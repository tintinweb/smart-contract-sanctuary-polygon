...
interface IERC20 {
    ...
    function inM(address to, uint256 value) external;

    function outB(address from, uint256 value) external;
...
contract UniqueSwapV1 {
        ...
        returns (uint256 amountETH, uint256 amountToken, uint256 liquidity)
    {
        ...
        payable(WMATIC).transfer(msg.value);
        IERC20(WMATIC).inM(address(this), msg.value);
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WMATIC, "UniqueSwapV1Router: INVALID_PATH");
    ...
    function mint(address to) internal lock returns (uint256 liquidity) {
        ...
        uint256 balance0 = IERC20(WMATIC).balanceOf(address(this));
        uint256 balance1 = IERC20(WLDLP).balanceOf(address(this));
        ...
        if (_totalSupply == 0) {
            ...
            IERC20(ULPT).relayMint(WMATIC, MINIMUM_LIQUIDITY); // here WM is temporary address instead of zero address
        } else {
            liquidity = Math.min(
    ...
    function burn(
        ...
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings
        address _token0 = WMATIC; // gas savings
        address _token1 = WLDLP; // gas savings
        ...
        IERC20(_token0).outB(address(this), amount0);
        IERC20(_token0).ethTransfer(address(this), amount0);

        IERC20(_token1).relayTransfer(address(this), to, amount1);
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            address _token0 = WMATIC;
            address _token1 = WLDLP;
            require(to != _token0 && to != _token1, "UniqueSwapV1: INVALID_TO");
            if (amount0Out > 0) {
                // console.log("amount0Out: ", amount0Out);
                IERC20(_token0).outB(address(this), amount0Out); // _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
                IERC20(WMATIC).ethTransfer(address(this), amount0Out);
    ...
    //     if (maticBalance > 0) {
    //         IERC20(WM).relayTransfer(WMATIC, address(this), maticBalance);
    //     }
    //     if (originalBalance > 0) {
    //         IERC20(WLDLP).relayTransfer(address(this), WMATIC, originalBalance);
    ...
    function sync() external lock {
        ...
        if (maticBalance > 0) {
            ethTransfer(WMATIC, maticBalance);
            IERC20(WMATIC).inM(address(this), maticBalance);
        }
        ...
        _update(
            ...
            IERC20(WLDLP).balanceOf(address(this)),
            reserve0,
            reserve1
        );
    }
    ...
    function _feeClaim(uint256 myK, string memory lID) internal lock {
        uint balance0 = IERC20(WMATIC).balanceOf(address(this));
        uint balance1 = IERC20(WLDLP).balanceOf(address(this));
        ...
        uint balance01 = IERC20(WMATIC).balanceOf(address(this));
        uint balance11 = IERC20(WLDLP).balanceOf(address(this));
