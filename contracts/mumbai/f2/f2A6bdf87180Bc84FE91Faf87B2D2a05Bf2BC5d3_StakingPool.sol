...
contract StakingPool is Ownable, ReentrancyGuard, Metadata {
    ...
    event WithdrawTimestampUpdated(uint256 _withdrawTimestamp);
    ...
    function init(bytes memory extraData) external {
        ...
        require(
            farmInfo.endTimestamp >= withdrawTimestamp,
    ...
    function updateWithdrawLock(uint256 _newWithdrawTimestamp)
        external
        onlyOwner
    {
        require(
            block.timestamp <= _newWithdrawTimestamp,
            "New withdraw timestamp must be greater than current timestamp"
        );

        require(
            farmInfo.endTimestamp >= _newWithdrawTimestamp,
            "New withdraw timestamp must be lower than farm end timestamp"
        );

        withdrawTimestamp = _newWithdrawTimestamp;
        emit WithdrawTimestampUpdated(withdrawTimestamp);
    }
    ...
    function canHarvest(address _user) public view returns (bool) {
        ...
        return ((block.timestamp >= user.nextHarvestUntil) &&
            (block.timestamp > withdrawTimestamp));
    }
    ...
    function _withdraw(
        ...
        require(
            block.timestamp >= withdrawTimestamp,
            "Cannot withdraw before withdraw timestamp"
        );
