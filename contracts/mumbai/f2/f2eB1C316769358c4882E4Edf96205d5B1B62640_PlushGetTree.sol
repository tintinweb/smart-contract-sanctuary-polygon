...
contract PlushGetTree is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, IPlushGetTree {
    ...
    function getTreesTypes() external view returns(string memory)
    {
        ...
        for(uint256 i = 0; i < treesTypes.length; i++){
            resultString = concatenate(resultString, bytes32ToString(treesTypes[i]));
            resultString = concatenate(resultString, concatenate('-', bytes32ToStringBytes32(treesTypes[i])));

            if(i + 1 < treesTypes.length){
                resultString = concatenate(bytes32ToString(treesTypes[i]), concatenate('-', bytes32ToStringBytes32(treesTypes[i])));
     ...
     * @param _bytes32 Tree type in bytes32
     */
    ...
    /**
     * @notice Convert bytes32 to string(bytes32)
     * @param _bytes32 Tree type in bytes32
     */
    function bytes32ToStringBytes32(bytes32 _bytes32) private pure returns (string memory) {
...
/// @custom:security-contact [email protected]
contract PlushForest is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
