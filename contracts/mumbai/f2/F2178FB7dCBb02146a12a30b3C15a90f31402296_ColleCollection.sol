...
contract ColleCollection is IColleCollection, ERC721URIStorage, SignatureValidator, ColleAccess {
    ...
    constructor(string memory name, string memory symbol) ERC721(name, symbol) {
        __SignatureValidator_init(name, "v0.3");
    }

    function mint(string memory _uri, address _receiver) public override onlyColle {
    ...
    function updateSaleMetadata(uint256 _tokenId, string memory _ipfsHash) public override {
        require(
            isColle(msg.sender) || ownerOf(_tokenId) == msg.sender,
    ...
    function permitApprove(
        ...
        public
        override
        isValidSignature(_from, _getPermitApprove(_from, _to, _tokenId, _deadline), _deadline, _signature)
    {
    ...
    function permitSafeTransfer(
        ...
        public
        override
        isValidSignature(_from, _getPermitSafeTransfer(_from, _to, _tokenId, _deadline), _deadline, _signature)
    {
    ...
    // It avoids accidental breaking, as 3rd party market places will not assume these functions exist.
    ////
    function safeTransfer(address _from, address _to, uint256 _tokenId) public override {
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "ERC721: caller is not token owner or approved");
        _safeTransfer(_from, _to, _tokenId, "");
    }

    ////
    // Reverting ERC721 transfers to break 3rd party marketplace integration
    // Items should only be transfered in tangent with their mirrored physical goods.
    ////
    function transferFrom(
        address /*from*/,
        address /*to*/,
        ...
        revert("ColleNFT: Not supported, use transfer instead.");
    }
    ...
    function safeTransferFrom(
        ...
        revert("ColleNFT: Not supported, use safeTransfer instead.");
    }
    ...
    function _getPermitSafeTransfer(
        address _from,
        address _to,
        uint256 _tokenId,
        uint256 _deadline
