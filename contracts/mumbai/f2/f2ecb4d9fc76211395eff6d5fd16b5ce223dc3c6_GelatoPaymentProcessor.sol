...
contract GelatoPaymentProcessor is GelatoRelayContext, Ownable {
    ITrustedForwarder public immutable trustedForwarder;

    IERC20 public immutable paymentToken;
    IERC20 public immutable feeToken;

    uint256 paymentValuePerFeeGwei;

    constructor(
        ITrustedForwarder _trustedForwarder,
        IERC20 _paymentToken,
        IERC20 _feeToken,
        uint256 _paymentValuePerFeeGwei,
        address owner
    ) {
        require(
            _trustedForwarder.supportsInterface(
                type(ITrustedForwarder).interfaceId
            ),
            "target is not a valid ITrustedForwarder"
        );
        trustedForwarder = _trustedForwarder;
        paymentToken = _paymentToken;
        feeToken = _feeToken;
        paymentValuePerFeeGwei = _paymentValuePerFeeGwei;
    ...
    function relayCall(
        ...
        trustedForwarder.execute(req, signature);
        if (_isGelatoRelay(msg.sender)) {
            _payGelatoRelayFee(req.from);
        }
    }

    function setPaymentValuePerFeeGwei(uint256 _paymentValuePerFeeGwei)
        external
        onlyOwner
    {
        paymentValuePerFeeGwei = _paymentValuePerFeeGwei;
    ...
    function _payGelatoRelayFee(address account) internal virtual {
        require(address(feeToken) == _getFeeToken(), "feeToken mismatch");
        _chargeAccount(account);
        _transferRelayFee();
    }

    function _chargeAccount(address account) internal virtual {
        paymentToken.transferFrom(
            account,
            address(this),
            _getPaymentValue(_getFee())
        );
    }

    function _getPaymentValue(uint256 fee)
        internal
        view
        returns (uint256 value)
    {
        value = (fee / 1e9) * paymentValuePerFeeGwei;
    }
