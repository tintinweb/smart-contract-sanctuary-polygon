...
contract AccessControl {
    ...
    modifier onlySERAPHIM() {
        require(seraphims[msg.sender] == true, 'You are not a seraph');
        _;
    }
...
contract VSBattle is AccessControl, SafeMath {
    //Data Structures
    ...
    uint8 smallAuraEffect = 6;
    uint8 bigAuraEffect = 12;
    uint8 lureEffect = 15;
    ...
    function updateAuras(uint64 battleId) public {
        IABToken ABTokenData = IABToken(ABTokenDataContract);
    ...
    function auraPassive(uint64 battleId) internal {
        ...
        if (Battles[battleId].attacker.aura == 1) {
            Battles[battleId].attacker.hp += 5;
        }
        ...
        if (Battles[battleId].defender.aura == 1) {
            Battles[battleId].defender.hp += 5;
        }
    ...
    function beforeRound(uint64 battleId) internal {
    ...
    function attackerTurn(uint64 battleId, uint8 _action) public {
        ...
        if (Battles[battleId].attacker.action == 2) {
        ...
        if (Battles[battleId].attacker.action == 12) {
            Battles[battleId].attacker.speed += 20;
        }
        ...
        if (Battles[battleId].attacker.action == 13) {
            Battles[battleId].attacker.defenseBuff += 25;
        }
    ...
    function getOrder(uint64 battleId) public returns (bool) {
        //todo: use both teams speed to determine randomnesss
        uint8 chance = getRandomNumber(100, 0, msg.sender);

        if (chance >= (10 + Battles[battleId].attacker.speed)) {
    ...
    function defenderTurn(uint64 battleId) internal {
        ...
        if (Battles[battleId].defender.action == 12) {
            Battles[battleId].defender.speed += 20;
        }
        ...
        if (Battles[battleId].defender.action == 13) {
            Battles[battleId].defender.defenseBuff += 25;
        }
