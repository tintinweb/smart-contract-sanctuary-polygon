...
contract Airdrop is Ownable {
    ...
    //todo data na start pobierania drugiego 50%
    //ustawienie przelicznika
    //sprawdzenie ile powinineneś mieć lp tokena
    //sprawdzenie ile masz zalockowanego lp tokena
    //możliwość zmniejszenia przelicznika
    //wydanie airdropa jak masz wystarczająco lp
    //ustawnienie daty vestingu (jeśli nie jest rozpoczęty)
    //vesting liniowy przez 3 miesiące


    uint256 public constant LP_MULTIPLAYER_DIVIDER = 1000000;
    ...
    uint256 public lpMultiplayer;


    event NewMerkleRoot(address user, bytes32 merkleRoot);
    event NewMultiplayer(address user, uint256 lpMultiplayer);
    ...
    function setLpMultiplayer(uint256 _lpMultiplayer) external onlyOwner {
        lpMultiplayer = _lpMultiplayer;
        emit NewMultiplayer(msg.sender, _lpMultiplayer);
    }
    ...
    function getClaimableAmount(address _address) public view returns (uint256) {
        uint256 totalEntitled = totalUserAirdrop[_address];
        ...
        if (block.timestamp > vestingStartDate) {
            claimable += getVestedAmount(halfOfAirdrop, msg.sender);
        }

        if (block.timestamp > lockdropAirdropReleaseDate && lpMultiplayer > 0) {
            uint256 userLpLock = getLock(_address);
            uint256 amountReq = halfOfAirdrop * lpMultiplayer / LP_MULTIPLAYER_DIVIDER;
    ...
    function getVestedAmount(uint256 amountToVest, address userAddress) internal view returns (uint256) {
        if (block.timestamp < vestingStartDate) {
            return 0;
            ...
            uint256 vestedAmount = amountToVest * elapsedTime / vestingDuration;
            if (claimedAmounts[userAddress] >= amountToVest) {
                return vestedAmount + amountToVest - claimedAmounts[userAddress];
            }
            return vestedAmount - claimedAmounts[userAddress];
        }
    }
    ...
    function getLock(address checkedAddress) public view returns (uint tokenAmount) {
        ...
        for (uint i = 1; i <= allocationsCount; i++) {
            ...
            if (address(pairToken) == pairAddress && !taken) {//todo sprawedzić taken!!!
                lockedTokens += value;
            }
        }
        return lockedTokens;
