...
contract MBOSpecialCollection is ERC721Opensea {
    ...
    event RegularClaimed(address indexed receiver, uint256 indexed tokenId);
    ...
    uint256 public DISCOUNTED_MINT_PRICE;
    uint256 public GENESIS_DISCOUNT_DIVISOR_VALUE;
    ...
    // =============== MINTING ===============
    /**
     * @dev Mint NFT with Genesis Token usage
     * @param genesisTokenId The genesis token id
    ...
    function genesisClaimMint(
        ...
        // Update the genesis claim data
        genesisClaimedIdToOwner[genesisTokenId] = msg.sender;
        genesisClaimTotalQty += 1;

        emit GenesisClaimed(msg.sender, totalSupply(), genesisTokenId);
    }

    /**
     * @dev Regular mint of NFT
     * @param mintRequest The mint request
     * @param signature The signature
     */
    function regularMint(
        ...
        emit RegularClaimed(msg.sender, totalSupply());
    }
    ...
    function batchMint(
        ...
        for (uint256 i = 0; i < mintRequests.length; i++) {
            if (mintRequests[i].isWithGenesis == true) {
                genesisClaimMint(
                    mintRequests[i].genesisTokenId,
                    mintRequests[i],
                    signatures[i]
                );
            } else {
                regularMint(mintRequests[i], signatures[i]);
            }
        }
    }
    ...
    /**
     * @dev Get discounted price for genesis claim
     * @param price The price of the token
     * @return uint256 The discounted price
     */
    function getGenesisDiscountedPrice(
        uint256 price
    ) internal view returns (uint256) {
        return price - (price / GENESIS_DISCOUNT_DIVISOR_VALUE);
    }

    /**
     * @dev Get the price of a single mint
     * @param mintRequest The mint request
     * @return uint256 The price of the mint
     */
    function getMintPrice(
        MintRequest calldata mintRequest
    ) public view returns (uint256) {
        uint256 price = 0;
        if (mintRequest.isWithGenesis == true) {
            price = getGenesisDiscountedPrice(MINT_PRICE);
        } else {
            price = MINT_PRICE;
        }
    ...
    /**
     * @dev Get the price of a batch mint
     * @param mintRequests The mint requests
     * @return uint256 The price of the batch mint
     */
    function getBatchMintPrice(
        ...
        for (uint256 i = 0; i < mintRequests.length; i++) {
            totalMintPrice += getMintPrice(mintRequests[i]);
        }
    ...
    constructor(
        ...
        uint256 mintPrice,
        uint256 discountedMintPrice,
        uint256 genesisDiscountDivisorValue,
        ...
        DISCOUNTED_MINT_PRICE = discountedMintPrice;
        GENESIS_DISCOUNT_DIVISOR_VALUE = genesisDiscountDivisorValue;
