...
contract PreSale is Ownable {
    ...
    function newCampaign(
        ...
        require(bytes(info.website).length > 0, "Website is required!");
        ...
        returns (bool)
    {
        ...
        if (c.balance == 0) completeCampaign(token);

        return true;
    }

    function completeCampaign(address token) public enable returns (bool) {
        CampaignData storage c = campaigns[activeCampaigns[token]];
        _checkCampaign(c);
        ...
        if (c.balance > 0) tokencontract.transfer(c.owner, c.balance);

        c.isCompleted = true;

        delete activeCampaigns[c.token];
        emit CompleteCampaign(c.id);
    ...
    function rescissionCampaign(
        ...
        CampaignData memory c = campaigns[activeCampaigns[token]];
        _checkCampaign(c);
    ...
    function getFundersIdByCampaignId(uint campaignId)
        public
        view
        returns (uint256[] memory)
    {
    ...
    function getFunderCountByCampaignId(uint campaignId)
        public
        view
        returns (uint256)
    {
        return campaignFunders[campaignId].length;
    }
