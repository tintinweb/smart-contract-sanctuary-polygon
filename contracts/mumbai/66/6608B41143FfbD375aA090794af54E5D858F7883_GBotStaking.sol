...
abstract contract ERC721Stakeable is Ownable{
    ...
    * unclaimedPrizes are the prizes users have accumulated over the previous periods, but haven't claimed
    ...
    /**
    * @dev 
    * unclaimedPrizes are the prizes users have already claimed
     */
    ...
    */
    function _stake(uint256 tokenId, uint256 rarity) internal returns (uint256) {        
        ...
        _totalWeight += _calculateStakeWeight(rarity);
        return index;
    }
    ...
    function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256) {
        if (_current_stake.tokenId == 0 || _current_stake.since == 0)
            return 0;
        ...
        if (timestamp > _periodFinish ){
            timestamp = _periodFinish;
        }
        uint256 stakeWeight = _calculateStakeWeight(_current_stake.rarity);
        uint256 rewardPerMinute = _rewardPerMinute().mul(stakeWeight);
        uint256 stakeReward = ((timestamp.sub(_current_stake.since)).div(1 minutes)).mul(rewardPerMinute);
    ...
    */
    function _withdrawStake(uint256 tokenId, address owner) internal {
        ...
        _totalWeight -= _calculateStakeWeight(current_stake.rarity);
    }

    /**
     * @dev
     * Called upon change of stakes/withdraws
     * Recalculates reward and writes in unclaimed rewards
    */
     function updateReward (address owner) internal {
        // Get reward for this period and reset timers
        uint256 periodPrize = _getTotalPrizesForPeriod(owner);
         _resetStakerTimers(owner);
        _totalRewardsPool -= periodPrize;
        unclaimedPrizes[owner] += periodPrize;
    }
    ...
    */
    function _claimAllPrizes(address owner) internal returns(uint256) {
        ...
        claimedPrizes[owner] = periodPrize + unclaimedPrize;
    ...
    */
    function _saveUnclaimedPrizes() internal {
        ...
        for (uint256 s = 0; s < totalStakes; s += 1) {
            ...
            uint256 periodPrize = _getTotalPrizesForPeriod(currentUser);
            _restartTimers(currentUser);
            _totalRewardsPool -= periodPrize;
            unclaimedPrizes[currentUser] += periodPrize;
        }
    }
    ...
    * _getTotalPrizes gets the total prizes for a staker
    */
    function _ownerUnclaimedPrizes(address owner) internal view returns (uint256) {
        return _getTotalPrizesForPeriod(owner) + unclaimedPrizes[owner];
    }

        /**
    * @dev
    * _getTotalPrizes gets the total claimed prizes for a staker
    */
    function _ownerClaimedPrizes(address owner) internal view returns (uint256) {
    ...
    *  
    */
    function _calculateStakeWeight(uint256 rarity) internal pure returns (uint256 rewardPoints) {
    ...
    /**
    * @dev
    * Shows rewardPerMinute on 1 token weight
    */
    function _rewardPerMinute() internal view returns (uint256 rewardPerMinute) {
        // If no pool or event not started, rate is 0
        if (_totalRewardsPool == 0)
        return 0;
        // Check if total weight 0, then return usual reward for 1 weight
        uint256 weight = 1;
        if (_totalWeight != 0) 
        weight = _totalWeight;

        uint256 fullReward = _totalRewardsPool.div(weight);
        return fullReward.div(((_periodFinish - _periodStarted) / 1 minutes));
    }
    ...
    */
    function _resetStakerTimers(address owner) internal {
        ...
        // If calculation is after the event finish, its since is the finish time
        uint256 timestamp = block.timestamp;
        if (timestamp > _periodFinish) {
            timestamp = _periodFinish;
        }
        ...
        stakeholders[user_index].address_stakes[s].since = timestamp;   
       }
    }

        /**
    * @dev
    * _restartTimers is used only after starting a new period, to restart stakes that remain
    */
    function _restartTimers(address owner) internal {
        // Grab user_index which is the index to use to grab the Stake[]
        uint256 user_index = stakes[owner];
        Stake[] memory existingStakes = stakeholders[user_index].address_stakes;
    ...
    function _ownerTotalWeight(address owner) internal view returns(uint256) {
        ...
        for (uint256 s = 0; s < existingStakes.length; s += 1) {
            ownerWeight += _calculateStakeWeight(existingStakes[s].rarity);
       }
        return ownerWeight;
    }
...
contract GBotStaking is ERC721Receiver, ERC721Stakeable, ReentrancyGuard  {
    ...
    function claimPrizes() nonReentrant  public {
        ...
        uint256 reward = unclaimedPrizes[_msgSender()];
        if (reward == 0) return;
    ...
    function withdrawAll()  virtual public {
        ...
        for (uint256 i = 0; i < currentTokens.length; i += 1) {
            ...
            if (currentTokens[i] != 0){
                ...
                emit GBotWithdrawn(_msgSender(), currentTokens[i]);
            }
        }
    ...
    function getUnclaimedReward(address owner) virtual public view returns (uint256) {
        return _ownerUnclaimedPrizes(owner);
    }

    function getClaimedReward(address owner) virtual public view returns (uint256) {
        return _ownerClaimedPrizes(owner);
    }

    function getStakedGBots(address owner) virtual public view returns (uint256[] memory) {
        return _ownerTokenIds(owner);
    }

    function getTotalWeight(address owner) virtual public view returns (uint256) {
        return _ownerTotalWeight(owner);
    } 
     ...
     */
    function confirmPoolAndStart(uint256 amount, uint256 duration) external stakingFinished {
        _requireOwnership(_msgSender());
