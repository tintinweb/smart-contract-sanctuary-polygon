...
contract limitOrderAndNormalSwapTokenProxy is OpsTaskCreator, Ownable {
    ...
    function deposit(uint256 _usdtAmount) external payable {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.deposit.selector, _usdtAmount));
        _getRevertMsg(success, result);
    }

    function withdraw(uint256 _usdtAmount, uint256 _ethAmount) external {
        (bool success,bytes memory result) = OrderAdminAddress.delegatecall(abi.encodeWithSelector(limitOrderAndNormalSwapToken.withdraw.selector, _usdtAmount, _ethAmount));
        _getRevertMsg(success, result);
    }
