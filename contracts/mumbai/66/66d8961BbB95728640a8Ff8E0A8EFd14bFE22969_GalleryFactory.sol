...
contract Gallery is Ownable, EIP712, IGallery {
	...
	modifier onlyOwnerorTokenOwner(uint256 tokenid) {
		...
		if (tokenowner == msg.sender || owner() == msg.sender) return;
		else revert('Need to be either token owner or gallery owner');
	...
	function mintAndSellNft(
		...
		sellNft(tokenId, amount, artistFee, galleryOwnerFee, thirdPartyFee);
	...
	// function getListOfNfts() public view override returns (bytes32[] memory) {
	...
	function retreiveBalance() public override onlyOwner {
		uint256 amount = this.getBalance();
