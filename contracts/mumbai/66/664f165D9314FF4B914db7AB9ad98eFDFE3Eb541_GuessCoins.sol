...
contract GuessCoins is Ownable {
    ...
    event Buy(bytes32 id_, address user_, uint256 share_);
    event Draw(bytes32 id_, bytes hash, uint256 value_, address[] winners_);
    ...
    mapping(uint256 => address[]) private _chices;
    mapping(uint256 => uint256) private _chiceShares;
    ...
    function _reset() internal {
        ...
        delete _chices[0];
        delete _chices[1];
        delete _chiceShares[0];
        delete _chiceShares[1];
    }
    ...
    function chiceTokens() public view returns (uint256, uint256) {
        return (_chiceShares[0] * _price, _chiceShares[1] * _price);
    }
    ...
    function buy(uint256 value_, uint256 share_) public returns (bytes32) {
        require(value_ == 0 || value_ == 1, "the value is must 0 or 1");
        _chices[value_].push(_msgSender());
        _chiceShares[value_] += share_;
        _shares[_msgSender()] += share_;
        bytes32 id = _generateId();
        IPlatform(_platform).deosit(id, _msgSender(), _token, share_ * _price);
        emit Buy(id, _msgSender(), share_);
    ...
    function draw() public {
        ...
        bytes32 id = _generateId();
        bytes memory hash = abi.encodePacked(blockhash(block.number - 1));
        uint256 value;
        for (uint8 i = 0; i < hash.length; i++)
            value += uint256(uint8(hash[i]));
        address[] memory winners = _chices[value % 2];
        uint256 totalShare;
        for (uint256 i = 0; i < winners.length; i++)
            totalShare += _shares[winners[i]];
        uint256 balance = IPlatform(_platform).balanceOf(address(this), _token);
        uint256[] memory amounts = new uint256[](winners.length);
        for (uint256 i = 0; i < winners.length; i++)
            amounts[i] = (balance * _shares[winners[i]]) / totalShare;
        IPlatform(_platform).withdraw(id, winners, _token, amounts);
        emit Draw(id, hash, value, winners);
        _reset();
    }
}
