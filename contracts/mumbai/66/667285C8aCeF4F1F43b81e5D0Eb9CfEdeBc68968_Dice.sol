...
interface IVault {
    ...
    function sendWinner(address payable _to, uint256 _amount) external;
    function getOwner() external view returns (address);
}
...
contract Common is ReentrancyGuard {
    ...
    function transferFees(address to) external nonReentrant {
        ...
        // }
        if (msg.sender != (IVault(vault)).getOwner()) {
            revert NotOwner((IVault(vault)).getOwner(), msg.sender);
        }
        uint256 fee = VRFFees;
        VRFFees = 0;
    ...
    function _transferPayout(
        ...
        IVault(vault).sendWinner(payable(player), payout);
