    ...
    ReentrancyGuard
{
    ...
    function claimWomen(address _receiver, uint256 _quantity) public payable virtual nonReentrant {
        require(_currentIndexWomen + _quantity <= nextTokenIdToLazyMint, "Not enough lazy minted tokens.");
        verifyClaim(msg.sender, _quantity); // Add your claim verification logic by overriding this function.

        uint256 startTokenId = _transferTokensOnClaimWomen(_receiver, _quantity); // Mints tokens. Apply any state updates by overriding this function.
        emit TokensClaimed(msg.sender, _receiver, startTokenId, _quantity);
    }
    ...
    function _transferTokensOnClaimWomen(address _receiver, uint256 _quantity)
        internal
        virtual
        returns (uint256 startTokenId)
    {
        startTokenId = _currentIndexWomen;
        _safeMintWomen(_receiver, _quantity);
    }
...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    uint256 internal _currentIndexWomen;
    ...
    constructor(string memory name_, string memory symbol_) {
        ...
        _currentIndexWomen = _startTokenIdWomen();
    }
    ...
    function _startTokenIdWomen() internal view virtual returns (uint256) {
        return 2;
    }
    /**
    ...
    function totalSupply() public view override returns (uint256) {
        ...
        unchecked {
            return _currentIndexWomen - _burnCounter - _startTokenId();
        }
    }
    ...
    function _ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        ...
        unchecked {
            if (_startTokenId() <= curr || _startTokenIdWomen() <= curr){
                if (curr < _currentIndexWomen) {
    ...
    function _safeMintWomen(address to, uint256 quantity) internal {
        _safeMintWomen(to, quantity, "");
    }
    ...
    function _safeMintWomen(
        address to,
        uint256 quantity,
        bytes memory _data
    ) internal {
        uint256 startTokenId = _currentIndexWomen;
        if (to == address(0)) revert MintToZeroAddress();
        if (quantity == 0) revert MintZeroQuantity();
        ...
        unchecked {
            ...
            if (to.isContract()) {
                ...
                // Reentrancy protection
                if (_currentIndexWomen != startTokenId) revert();
            } else {
                do {
                    emit Transfer(address(0), to, updatedIndex++);
                } while (updatedIndex < end);
            }
            _currentIndexWomen = updatedIndex;
        }
        _afterTokenTransfers(address(0), to, startTokenId, quantity);
    }
    ...
    function _transfer(
        ...
        unchecked {
            ...
            if (nextSlot.addr == address(0)) {
                ...
                if (nextTokenId != _currentIndexWomen) {
                    nextSlot.addr = from;
...
function claimNFT(address _reciever, uint256 _quantity, uint256 _index)
        public
        payable
        virtual
    {
        super.claim(_reciever, _quantity);
    }

    function claimNFTWomen(address _reciever, uint256 _quantity, uint256 _index)
        public
        payable
        virtual
    {
        super.claimWomen(_reciever, _quantity);
    }
