...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function setTokenDetails(uint256 id, uint256 newDetails) external onlyRole(MINTER_ROLE) {
  ...
  function processTokenRequests() external {
    ...
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      bool isHeroS = ((request.details >> 35) & 1) == 1;
      ...
      if (available < request.count) {
        ...
        createRandomToken(to, available, rarity, isSuperBox, isHeroS, seed, dropRateOption);
        break;
      }
      available -= request.count;
      createRandomToken(to, request.count, rarity, isSuperBox, isHeroS, seed, dropRateOption);
      requests.pop();
      if (available == 0) {
        break;
      }
  ...
  function createRandomToken(
    ...
    for (uint256 i = 0; i < count; ++i) {
      ...
      (, details) = design.createRandomToken(tokenSeed, id, rarity, isSuperBox, isHeroS, dropRateOption);
      createTokenWithId(to, id, details);
    }
  }
