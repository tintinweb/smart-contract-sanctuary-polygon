...
contract PredictionWorld5 is Initializable {
    ...
    // 2nd Key: <MARKET_ID>
    mapping(address => mapping(uint => Bet[])) public userYesBets;
    mapping(address => mapping(uint => Bet[])) public userNoBets;
    ...
    function updateUserBets(
        ...
        for (uint i = 0; i < market.yesBets.length; i++) {
            if (market.yesBets[i].user == msg.sender) {
        ...
        for (uint i = 0; i < market.noBets.length; i++) {
            if (market.noBets[i].user == msg.sender) {
