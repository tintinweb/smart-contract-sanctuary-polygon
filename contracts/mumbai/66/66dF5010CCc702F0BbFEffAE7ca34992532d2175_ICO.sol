...
contract ICO is Ownable, Initializable {
    ...
    uint[3] public claimPeriod = [180 days, 30 days];
    ...
    function startSale() external onlyOwner {
        require(isSalePaused, "ICO : isSalePaused should be true");
        isSalePaused = false;
    }

    function finishSale() external onlyOwner {
        require(!isSalePaused, "ICO : isSalePaused should be false");
        isSalePaused = true;
    ...
    function claim() external {
        userStruct storage user_ = user[msg.sender];
        require(isSalePaused, "ICO : isSalePaused should be true");
        require(saleClosedTime != 0, "ICO : saleClosedTime < block.timestamp");
        require(user_.totalClaimed < user_.balance, "ICO : total claim < total balance");
