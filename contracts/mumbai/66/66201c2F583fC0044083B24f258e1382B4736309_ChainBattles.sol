...
contract ChainBattles is ERC721URIStorage{
    ...
    function generateCharacter(uint256 tokenId)public view returns(string memory){
        ...
        return string(
            abi.encodePacked(
                // to specify to the browser that Base64 string is an SVG image and how to open it.
                "data:image/svg+xml;base64,",//tell the browser how to handle the data
                 Base64.encode(svg)//contains the actuals data
            )
        );
    }
    ...
    function getTokenURI(uint256 tokenId)public view returns(string memory){
        ...
        '{',
            ...
            //generateCharacter return the image url type.mean image path the image that display on browser .
        '}'
    );
     return string(
        abi.encodePacked(
    ...
    function train(uint256 tokenId) public{
        require(_exists(tokenId),"please enter the existing id");
