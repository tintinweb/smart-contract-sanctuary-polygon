...
contract BoleroManager is AccessControl {
    ...
    * */
    ...
    function getArtistICOAddress(address artistAddress) public view returns (address) {
        return IBoleroArtistToken(getArtistTokenAddress(artistAddress)).icoContract();
    }


    function getPaymentAddress(address artistAddress) public view returns (bool, address) {
        bool isWithPaymentSplitter = IBoleroArtist(getArtistContractAddress(artistAddress)).isWithPaymentSplitter();
        if (isWithPaymentSplitter) {
            address paymentSplitterAddress = IBoleroArtist(getArtistContractAddress(artistAddress)).artistPayment();
            return (true, paymentSplitterAddress);
        } else {
            address paymentAddress = IBoleroArtist(getArtistContractAddress(artistAddress)).artistPayment();
