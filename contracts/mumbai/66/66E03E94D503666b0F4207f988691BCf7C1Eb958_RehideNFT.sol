...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    function mintPass(
        ...
        _passTtlMapping[newTokenId] = ttl + block.timestamp;
    ...
    function readSharedNote(uint256 tokenId) public payable returns (uint256 readsAvailable) {
        ...
        if (noteReadDenylistAddresses.length > 0) {
            ...
            require(isDenied, "RehideNFT: Address is in blacklist");
        }
    ...
    function addToReadDenylist(uint256 tokenId, address[] memory toAddAddresses) public {
        ...
        require(ownerOf(tokenId) == _msgSender(), "RehideNFT: Trying to set another wallet's token blacklist");
        require(toAddAddresses.length > 0, "RehideNFT: No addresses to add");
    ...
    /**
     * @dev Get how many mint passes are held by address
     */
    ...
    /**
     * @dev Get all mint passes created by address
     */
    function getWalletActiveMintPass(address account) public view returns (uint256[] memory tokenIds) {
        uint256 counter = 0;
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 tokenId = tokenIds[i];
            uint256 ttl = _passTtlMapping[i];
            if (ownerOf(tokenId) == account && (ttl == 0) || ttl > block.timestamp) {
