...
contract GoldStake is Ownable, IERC721Receiver, Pausable {
    ...
    // Counters
    uint16 public goldsInOceanCounter;
    uint16 public raftsInOceanCounter;
    uint16 public goldsArrivedCounter;
    uint16 public raftsArrivedCounter;
    uint16 public tokensArrivedCounter;

    event TokenStaked(address owner, uint16 goldId, uint16[] tokenIds, uint16 rum, uint16 speed, uint timestamp);
    event GoldArrived(address owner, uint16 goldId, uint16[] tokenIds, uint timestamp);
    ...
    constructor() {
        ...
        speedMapping[1] = 30; // default gold
        speedMapping[2] = 45; // pirate gold
    ...
    function setGolds(address _address) external onlyOwner {
        addManager(_address);
        goldToken = IToken(_address);
    }
    ...
    function stakeTokens(address _account, uint16 _goldId, uint16[] calldata _tokenIds, uint16 _rumAmount) public whenNotPaused {
        require(_account == msg.sender, "You do not have a permission to do that");

        if (_goldId != 0) {
            require(goldToken.ownerOf(_goldId) == msg.sender, "This NTF does not belong to address");
        }

        uint16 speed = speedMapping[0]; // raft speed
        if (_goldId != 0) {
            if (goldToken.isPirate(_goldId)) {
                speed = speedMapping[2]; // pirate gold speed
                require(_tokenIds.length <= 7, "Cannot stake more than 7 NFTs");
            } else {
                speed = speedMapping[1]; // default gold speed
                require(_tokenIds.length <= 5, "Cannot stake more than 5 NFTs");
            }

            goldToken.transferFrom(msg.sender, address(this), _goldId);
            waterproof[_goldId] = true;
        }
        ...
        goldsInOceanCounter += 1;
        if (_goldId == 0) raftsInOceanCounter += 1;
        emit TokenStaked(msg.sender, _goldId, _tokenIds, _rumAmount, uint16(speed), block.timestamp);
    }
    ...
    function unstakeTokens(address _owner, uint16 _index) external whenNotPaused {
        ...
        emit GoldArrived(userStake.owner, userStake.tokenIds[0], userStake.tokenIds, block.timestamp);
    }
    ...
    function unstakeTokensIntoLand(address _owner, uint16 _index) external whenNotPaused {
        ...
        if (!goldToken.isApprovedForAll(address(this), address(newLand))) goldToken.setApprovalForAll(address(newLand), true);
        if (!goldHunter.isApprovedForAll(address(this), address(newLand))) goldHunter.setApprovalForAll(address(newLand), true);

        uint arrivedTimestamp = userStake.startTimestamp + distance / (userStake.speed + speedMapping[5]) * 1 minutes;
        require(block.timestamp >= arrivedTimestamp, "The gold has not arrived yet");
