...
contract MetaMaster {
    ...
    uint256 public withdrawalInterval = 5 minutes;
    ...
    function deposit(uint256 _USDTAmount, address _referrer) public {
		...
		if (user.referrer == address(0)) {
			...
			if (users[_referrer].deposits.length > 0 && _referrer != msg.sender) {
                ...
                uint256 commission = _USDTAmount.mul(oneTimeCommissionPercent).div(PERCENTS_DIVIDER);
                users[_referrer].directIncome = users[_referrer].directIncome.add(commission);
			}
    ...
    function getUserTotalReturns(address _userAddress) public view returns(uint256){
        uint256 dailyRewardPercentage = 7;
    ...
    function getUserPendingMintingWithdrawalAmount(address _userAddress) public view returns(uint256) {
        return getUserTotalReturns(_userAddress).sub(mintingAmountWithdrawn[_userAddress]);
    }

    function getUserPendingBonusWithdrawalAmount(address _userAddress) public view returns(uint256) {
        return users[_userAddress].bonus.sub(bonusAmountWithdrawn[_userAddress]);
    }

    function withdraw(address _userAddress, uint256 _mintAmount, uint256 _bonusAmount) public {
        uint256 getTime = block.timestamp;
        require(msg.sender == _userAddress, "Caller should be the owner of withdraw account");
        require(getUserPendingMintingWithdrawalAmount(_userAddress) >= _mintAmount, "Invalid Minting Amount");
        require(getUserPendingBonusWithdrawalAmount(_userAddress) >= _bonusAmount, "Invalid Bonus Amount");
        require(_mintAmount + _bonusAmount >= 10000000,"The Amount should be greater than 10 USDT");
        require(getTime >= lastWithdrawal + withdrawalInterval, "Cannot withdraw before the interval has passed."   );
        
        lastWithdrawal = getTime;
        
        mintingAmountWithdrawn[_userAddress] = mintingAmountWithdrawn[_userAddress] + _mintAmount;
        bonusAmountWithdrawn[_userAddress] = bonusAmountWithdrawn[_userAddress] + _bonusAmount;
        
        // amountWithdrawn[_userAddress] = amountWithdrawn[_userAddress] + _mintAmount + _bonusAmount;
        uint256 creatorTax = _mintAmount.mul(withdrawTax).div(PERCENTS_DIVIDER);
        uint256 withdrawAmount = _mintAmount.sub(creatorTax);
    ...
    function getUserTotalEarning(address _userAddress) public view returns(uint256){
        return getUserTotalLevelIncome(_userAddress) + getUserTotalReturns(_userAddress) + getUserDirectIncome(_userAddress);
    }
}