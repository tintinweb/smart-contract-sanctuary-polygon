...
contract MrPixel is ERC721Enumerable, Ownable {
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        require(!paused || msg.sender == owner(), "Sale must be active to mint tokens");
		...
		returns (string memory)
    {
        ...
        return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(
			currentBaseURI, _tokenId.toString(), baseExtension
		)) : "";
    }

	function tokenAlreadyRevealed(uint256 _tokenId) 
		public
		view
		returns (bool) 
	{
	...
	function setAlreadyRevealedTokens(uint256 lastTokenRevealed)
		public
		onlyOwner
	{
