    ...
    DefaultOperatorFilterer
{
    ...
    uint256 public airdropMax = 150;
    uint256 public airdropped = 0;

    uint256 public treasuryMax = 30;
    uint256 public treasuryMinted = 0;

    uint256 public publicSupply = 320;
    ...
    function mint() external payable noContract nonReentrant {
        ...
        require(publicMinted < publicSupply, "NO_MORE_SUPPLY");
    ...
    function treasuryMint(address _wallet, uint256 _amount) public onlyOwner {
        require(state == State.Closed, "MINT_ALREADY_OPEN");
        require(
            treasuryMinted + _amount <= treasuryMax,
            "MORE_THAN_TREASURY_MAX"
        );

        _safeMint(_wallet, _amount);
        treasuryMinted += _amount;
    }
    ...
    function airdrop(
        ...
        require(
            airdropped + totalAmount <= airdropMax,
            "MORE_THAN_MAX_AIRDROP_SUPPLY"
        );

        for (uint256 i = 0; i < _wallets.length; i++) {
            _safeMint(_wallets[i], _amounts[i]);
        }
        airdropped += totalAmount;
    }
