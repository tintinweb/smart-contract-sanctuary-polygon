...
import "../TokenForge721gov.sol";
//import "./interfaces/ILayerZeroEndpoint.sol";
//import "./interfaces/ILayerZeroReceiver.sol";


error NotTokenOwner();
error InsufficientGas(uint256 messageFee);
error SupplyExceeded();

contract LzTokenForge721gov is TokenForge721gov, NonblockingLzApp {

    event LzReceivedToken(
        uint16 _srcChainId,
        address _from,
        uint256 _tokenId
    );
    ...
    ) TokenForge721gov(name_, symbol_, signer_, baseUri_) NonblockingLzApp(endpoint_) {
    }

    function crossChainDebug1(uint16 /*dstChainId*/, uint256 tokenId) public payable {
        if (msg.sender != ownerOf(tokenId)) revert NotTokenOwner();
    }

    function crossChainDebug2(uint16 /*dstChainId*/, uint256 tokenId) public payable {
        if (msg.sender != ownerOf(tokenId)) revert NotTokenOwner();
    ...
    function crossChainDebug3(uint16 dstChainId, uint256 tokenId) public payable {
        if (msg.sender != ownerOf(tokenId)) revert NotTokenOwner();
        ...
        if (msg.value <= messageFee) revert InsufficientGas(messageFee);
    }

    function crossChain(uint16 dstChainId, uint256 tokenId) public payable {
        if (msg.sender != ownerOf(tokenId)) revert("Not TokenOwner");
    ...
    function _nonblockingLzReceive(
        ...
        emit LzReceivedToken(_srcChainId, from, tokenId);
    }
