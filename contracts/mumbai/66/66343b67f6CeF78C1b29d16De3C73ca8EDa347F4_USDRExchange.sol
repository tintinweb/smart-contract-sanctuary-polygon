...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function swapFromTNGBL(
        ...
        uint256 multiplier;
        {
            ...
            multiplier = getTNGBLMultiplier(usdrMarketCap * 1e9, stable + rwa);
        ...
        returns (uint256)
    {
        assembly {
            if iszero(iszero(fee)) {
                amount := sub(amount, div(mul(amount, fee), 100))
            }
        }
        return amount;
    }
    ...
    function _checkTNGBLToUSDRMarketCapRatio() private view {
        ...
        (uint256 stable, uint256 rwa, ) = ITreasury(treasury)
            .getTreasuryValue();
        require(
            stable + rwa >= IStaking(staking).usdrMarketCap() * 88e7,
        ...
        )
    {
        ...
        (uint256 stable, uint256 rwa, uint256 tngblVal) = ITreasury(treasury)
            .getTreasuryValue();
        require(
            stable + rwa < _scaleAmount(amountOut, tngblDecimals, 18),
            "sufficient backing"
