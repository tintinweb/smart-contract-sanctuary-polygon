...
contract Marketplace is ERC721Holder, AccessControl, ReentrancyGuard {
    ...
    uint256 public feeBasisPoint = 500; //4 decimals, applies to listings. Fees collected are held in contract
    uint256 public resaleFeeBasisPoint = 100; //4 decimals, applies to listings. Fees collected are held in contract
     ...
     */
    function approveBooking(uint256 requestId, uint256 ownerFee) external {
    ...
    function createBnftAuction(
        ...
        IERC721 bnftToken = IERC721(address(BnftContract));
