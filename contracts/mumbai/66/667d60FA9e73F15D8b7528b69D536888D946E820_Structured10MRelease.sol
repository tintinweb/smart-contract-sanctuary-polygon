...
import "./Structured10MRouter.sol";
 ...
 * registered.
 *
 * See [Structured10Router.sol] for more info.
 *
 */
contract Structured10MRelease is ERC1155Upgradeable, OwnableUpgradeable {
  /**
   ...
   *
   * See [Structured10MRouter] for more info.
   */
  Structured10MRouter private _router;

  function initialize(address routerAddress, address minterAddress, address owner) public initializer {
    __ERC1155_init("_");
  ...
  function _setRouterAddress(address routerAddress) internal {
    _router = Structured10MRouter(routerAddress);
  }

  /**
   * Mints a NFT on the contract (node that only the `minter` wallet can execute this action)
   */
  function mintNFT(uint256 tokenId, address ownerId) public {
    require(_msgSender() == minter, "Only the `minter` wallet can mint");
    _mint(ownerId, tokenId, 1, "");
  }

  /**
   * Sets (updates) the `minter` wallet address on the contract (node that only the `owner` wallet can execute this action)
   */
  ...
  function batchMint(uint256[] calldata tokenIds, address[] calldata ownerIds) public {
    address operator = _msgSender();
    require(operator == minter, "Only the `minter` wallet can mint");
    require(tokenIds.length == ownerIds.length, "tokenIDs length need to match ownerIDs length");
    for (uint256 i=0; i < tokenIds.length; i++) {
      _mint(ownerIds[i], tokenIds[i], 1, "");
    }
  }
}
 ...
 *
 * This contract is a delegate contract used by the [Structured10MRelease] contract
 * for its `uri` contruction code. The purpose of it is to determine what IPFS CID
 * value to use for a given NFT ID.
 *
 * The reason why this code exists is to allow the Structured10MRelease ERC1155 contract
 ...
 * Structured10MRelease contract will be relinquished so that the NFT collection stays immutable.
 *
 ...
 *
 * The parent contract (Structured10MRelease) then uses the CID value and places that into the
 ...
 *
 */
contract Structured10MRouter is OwnableUpgradeable {
  string[] private _routes;
  uint256 private _itemsPerRoute;

  /**
   * @param initialRoutesCount used to pre-fill the routes array with empty values
   * @param itemsPerRoute used to specify how many entries will live in each route (in each IPFS dir)
   */
  function initialize(uint256 initialRoutesCount, uint256 itemsPerRoute, address owner) public initializer {
    _routes = new string[](initialRoutesCount);
   ...
   */
  function registerRoute(uint256 index, string memory cid) public onlyOwner {
    while (index >= _routes.length) {
      _routes.push("");
    }
    _routes[index] = cid;
  }
  ...
  function lookup(uint256 itemIndex) public view returns (string memory) {
    uint256 index = itemIndex / _itemsPerRoute;
    string memory cid = index < _routes.length ? _routes[index] : "";
    require(bytes(cid).length > 0,
      string(abi.encodePacked("Route boundary for \"itemIndex=", Strings.toString(itemIndex), "\" is not defined")));
    return cid;
  }
