...
//updateX6Referrer
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        address[] memory refReferral = users[referrerAddress].matrices[level].Referrals;
        ...
        if (refReferral[1] == userAddress) {
            updateX6(userAddress, referrerAddress, level, false);
            return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
        } else if (refReferral[0] == userAddress) {
            updateX6(userAddress, referrerAddress, level, true);
            return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
        }
        
        if (users[refReferral[0]].matrices[level].Referrals.length <= 
            users[refReferral[1]].matrices[level].Referrals.length) {
            updateX6(userAddress, referrerAddress, level, false);
        } else {
            updateX6(userAddress, referrerAddress, level, true);
        }
    ...
    function updateX6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        address[] memory refReferrals = users[referrerAddress].matrices[level].Referrals;

        if (!x2) {
            users[refReferrals[0]].matrices[level].Referrals.push(userAddress);
            //set current level
            users[userAddress].matrices[level].currentReferrer = refReferrals[0];
        } else {
            users[refReferrals[1]].matrices[level].Referrals.push(userAddress);
            //set current level
            users[userAddress].matrices[level].currentReferrer = refReferrals[1];
        }
    }
    
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level) private {
        Matrix storage refMatrix = users[referrerAddress].matrices[level];
        address refcurrentReferrer =  users[referrerAddress].matrices[level].currentReferrer;
        ...
        address[] memory x6 = users[refcurrentReferrer].matrices[level].Referrals;
        
        if (x6.length == 2) {
            if (x6[0] == referrerAddress ||
                x6[1] == referrerAddress) {
                users[refcurrentReferrer].matrices[level].closedPart = referrerAddress;
            } else if (x6.length == 1) {
                if (x6[0] == referrerAddress) {
                    users[refcurrentReferrer].matrices[level].closedPart = referrerAddress;
                }
        ...
        refMatrix.Referrals = new address[](0);
        refMatrix.secondLevelReferrals = new address[](0);
        refMatrix.closedPart = address(0);
