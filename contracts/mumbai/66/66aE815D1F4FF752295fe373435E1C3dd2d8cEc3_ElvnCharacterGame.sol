...
interface INFTParts is IERC1155{
    ...
    function getRandomBase(uint _tokenId) external view returns (uint[2] memory);
    function partSet(uint _partId) external view returns(uint);
}
...
contract ElvnCharacterGame is Ownable {
   ...
   struct StakingBoost{
    ...
    mapping(uint => mapping(uint => uint)) public tokenSetCount;
    mapping(uint => mapping(uint => uint)) public partSetBoosts;
    mapping(uint => uint) public activeSet;
    ...
    uint public rewardPerPowerPoint;
    bool internal initiateRewardCalculation=false;
    bool public claimStakingRewardsActive = false;
    bool internal stakingRewardsBaseCalculated;
    ...
    */
    ...
    //FUNCTION NEEEDS TO BE DELATED AFTER TESTS
    function setCharacterPartsManually(uint _tokenId, uint[5] calldata _parts) public onlySetter{
        tokenParts[_tokenId].hoodyId = _parts[0];
        tokenParts[_tokenId].maskId = _parts[1];
        tokenParts[_tokenId].shortId = _parts[2];
        tokenParts[_tokenId].shoeId = _parts[3];
        tokenParts[_tokenId].gloveId = _parts[4];
    }
    ...
    function equip(uint _tokenId, uint _part) public {
        ...
        uint _partSet = INFTParts(partContract).partSet(_part);
        tokenSetCount[_tokenId][_partSet] += 1;
        if(tokenSetCount[_tokenId][_partSet] >= 3){
            activeSet[_tokenId] = _partSet;
        }
    ...
    function equipGlove(uint _tokenId, uint _part) internal {
        if(tokenParts[_tokenId].gloveId != 0){
            unequip(_tokenId, tokenParts[_tokenId].gloveId);
        }
        tokenParts[_tokenId].gloveId = _part;
    }
    ...
    function unequip(uint _tokenId, uint _part) internal {
        ...
        uint _partSet = INFTParts(partContract).partSet(_part);
        tokenSetCount[_tokenId][_partSet] -= 1;
        if(activeSet[_tokenId]!=0 && activeSet[_tokenId] == _partSet && tokenSetCount[_tokenId][_partSet] <3){
            activeSet[_tokenId] = 0;
        }
    }
    ...
    function createBaseCharacter(uint _start, uint _stop) public onlyOwner{
        ...
        {
            uint[2] memory _baseParts = INFTParts(partContract).getRandomBase(i);
            for(uint j = 0; j<= 1;j++)
            {
                _baseClass = INFTParts(partContract).class(_baseParts[j]);
                if(_baseClass == 1){
                    tokenParts[i].hoodyId = _baseParts[j];
                }
                    else if(_baseClass == 2)
                    {
                        tokenParts[i].maskId = _baseParts[j];
                    }
                        else if(_baseClass == 3)
                        {
                            tokenParts[i].shortId = _baseParts[j];
                        }
                            else if(_baseClass == 4)
                            {
                                tokenParts[i].shoeId = _baseParts[j];
                            }
                            else
                            {
                                tokenParts[i].gloveId = _baseParts[j];
                            }
            tokenSetCount[i][INFTParts(partContract).partSet(_baseParts[j])] += 1;                
            }
        }
    }

    function setPartSetBoost(uint _setId, uint _count, uint _boost) public onlySetter{
        partSetBoosts[_setId][_count] = _boost;
    }

    function setPartSetBoostBatch(uint[] calldata _setId, uint[] calldata _count, uint[] calldata _boost) public onlySetter{
        require(_setId.length == _count.length && _setId.length == _boost.length,"ELVN Char: Length of lists are not the same");
        for(uint i=0;i< _setId.length;i++){
            partSetBoosts[_setId[i]][_count[i]] = _boost[i];
        }
    }    
    
   
    
    function getPlayersOfOwner(address _owner) public view returns(uint[][] memory){
        uint[][] memory _players;
        uint[] memory _characters = walletOfOwner(_owner);
        for(uint i=0; i < _characters.length;i++){
            _players[i][0] = _characters[i];
            _players[i][1] = INFTParts(partContract).partSet(tokenParts[_characters[i]].hoodyId);
            _players[i][2] = INFTParts(partContract).partSet(tokenParts[_characters[i]].maskId);
            _players[i][3] = INFTParts(partContract).partSet(tokenParts[_characters[i]].shortId);
            _players[i][4] = INFTParts(partContract).partSet(tokenParts[_characters[i]].shoeId);
            _players[i][5] = INFTParts(partContract).partSet(tokenParts[_characters[i]].gloveId);
        }
        return _players;
    }
    ...
    function levelUp(uint _tokenId) public {
        ...
        require(_currentLevel < maxLevel, "ELVN Char: You can't level up more");
    ...
    function setMaxLevel(uint _level) public onlyOwner{
        require(maxLevel < _level, "ELVN Character: The new max level can't be lower");
        maxLevel = _level;
    }
    ...
    function totalStakingPower(uint _tokenId) public view returns (uint){
        ...
        if(activeSet[_tokenId]!= 0){
            _stakingPower += _stakingPower * partSetBoosts[activeSet[_tokenId]][tokenSetCount[_tokenId][activeSet[_tokenId]]] / 100;
        }
        return _stakingPower;
    }

    function calculateStakingRewardsBase() public onlySetter{
        require(!initiateRewardCalculation,"ELVN Character: Reward calculation was already initiated");
        initiateRewardCalculation = true;
        availableBalance = IERC20(gelvnAddress).balanceOf(address(this)) - reservedRewards - summedServiceFee;
        rewardPerPowerPoint = availableBalance / totalSummedStakingPower - 1;
        reservedRewards += availableBalance;
        stakingRewardsBaseCalculated = true;
    }

    function calculateStakingRewards(uint _start, uint _stop) public onlySetter{
        require(stakingRewardsBaseCalculated,"ELVN Character: Staking Reward Base was not calculated yet");
        uint _calculatedStakingPower;
        for(uint i=_start; i <= _stop; i++){
            stakingRewards[i] += rewardPerPowerPoint * stakingPowerSum[i];
            _calculatedStakingPower += stakingPowerSum[i];
            stakingPowerSum[i] = 0;
        }
        totalSummedStakingPower -= _calculatedStakingPower;
    }

    function endStakingRewardCalculation() public onlySetter{
        require(totalSummedStakingPower == 0,"ELVN Character: There is still staking Power left");
        initiateRewardCalculation = false;
        stakingRewardsBaseCalculated = false;
    }

    function editStakingRewards(uint _tokenId, uint _newStakingReward) public onlySetter{
        stakingRewards[_tokenId] = _newStakingReward;
    }

    function activateStakingRewardClaiming() public onlySetter{
        require(!claimStakingRewardsActive,"ELVN Character: Claiming is already active");
        claimStakingRewardsActive = true;
    }

    function deactivateStakingRewardClaiming() public onlySetter{
        require(claimStakingRewardsActive,"ELVN Character: Claiming is already deactivated");
        claimStakingRewardsActive = false;
    }

    function claimStakingRewards(uint _tokenId) public{
        require(claimStakingRewardsActive, "ELVN Character: Staking Rewards Claiming is not active");
        require(msg.sender == tokenOwner[_tokenId],"ELVN Character: You are not the Character Owner");
