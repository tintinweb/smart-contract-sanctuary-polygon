...
interface ICategory {
    ...
    function createCategory(string memory tokenURI) external returns (uint256);
}
...
interface IYear {
    ...
    function createYear(string memory tokenURI) external returns (uint256);
}
...
interface IDayMonth {
    ...
    function createDayMonth(string memory tokenURI) external returns (uint256);
}
...
interface ICardPack {
    ...
    function createCard(string memory tokenURI) external returns (uint256);
...
contract PackOpener is Ownable {
    ...
    function openPack(
        ...
        for (uint i = 0; i < dayMonthAmount; i++) {
            dayMonthContract.createDayMonth(_tokenDayMonth);
        }
        for (uint j = 0; j < yearAmount; j++) {
            yearContract.createYear(_tokenYear);
        }
        for (uint k = 0; k < categoryAmount; k++) {
            categoryContract.createCategory(_tokenCategory);
        }
        for (uint l = 0; l < craftingCardAmount; l++) {
            craftingCardContract.createCraftingCard(_tokenCraftingCard);
        }
    }
}