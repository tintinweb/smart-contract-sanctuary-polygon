...
contract RWACalculator is AddressAccessor, IRWACalculator {
    ...
    uint256 public initialReSellerThreshold;
    ...
    function setPriceAboveMarketInitialReSellerThreshold(uint256 threshold)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(threshold >= 100000, "incorrect threshold");
        initialReSellerThreshold = threshold;
    }
        ...
        priceManager = ITNFTPriceManager(
            abi.decode(
                addressProvider.getAddresses(
                    abi.encode(TANGIBLE_PRICE_MANAGER_ADDRESS)
                ),
                (address)
            )
        );
        ...
        returns (ITangibleMarketplace marketplace)
    {
        marketplace = ITangibleMarketplace(
            abi.decode(
                addressProvider.getAddresses(
                    abi.encode(TANGIBLE_MARKETPLACE_ADDRESS)
                ),
                (address)
            )
        );
        ...
        )
    {
        ...
        uint256 thresholdToUse = lot.seller == factory.initReSeller()
            ? initialReSellerThreshold
            : priceThreshold[address(tnft)];
        require(thresholdToUse >= 100000, "price threshold missing");

        uint256 topPrice = ((((hs.weSellAt[0] + hs.lockedAmount[0]) * share) /
            10000000) * thresholdToUse) / fullPercent;
        if (amount <= topPrice) {
            inRange = true;
        } else {
            inRange = false;
