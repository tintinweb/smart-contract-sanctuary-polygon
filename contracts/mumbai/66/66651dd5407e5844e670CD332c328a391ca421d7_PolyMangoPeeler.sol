...
contract MangoPass is ERC721Enumerable, Ownable {
    address public immutable mangoPeeler;
    ...
    constructor(
        ...
        address mangoPeeler_
    ) ERC721("Mango Pass", "MANGOPASS") {
        baseTokenURI = baseURI_;
        contractURI = contractURI_;
        mangoPeeler = mangoPeeler_;
    }

    function mint(address to, uint256 tokenId) external {
        require(totalSupply() < 2**14, "max exceeded");
        require(msg.sender == mangoPeeler, "only peeler");
        _mint(to, tokenId);
    }
    ...
    function changeContractURI(string memory newContractURI) external onlyOwner {
...
import "./MangoPass.sol";

contract PolyMangoPeeler is Ownable, Initializable {
    using PackedUint256 for uint256;

    event Peel(address user, uint256[] tokenIdList, uint256 requestId);
    ...
    uint256 public constant MAGIC_NUMBER = 5;

    MangoPass public immutable mangoPass;
    address public immutable polyMango;
    VRFConsumerV2 public immutable vrfConsumer;

    mapping(uint256 => Request) private _tokenIdToRequest;
    mapping(address => uint256[]) private _peeledTokenIds;
    ...
    constructor(
        ...
        polyMango = polyMango_;
        mangoPass = MangoPass(mangoPass_);
        vrfConsumer = VRFConsumerV2(vrfConsumer_);
    }
    ...
    function tokenIdToRequest(uint256 tokenId) external view returns (Request memory) {
        return _tokenIdToRequest[tokenId];
    }

    function peeledTokenIds(address user) external view returns (uint256[] memory) {
        return _peeledTokenIds[user];
    }
    ...
    function peel(uint256[] calldata tokenIdList) external {
        require(0 < tokenIdList.length && tokenIdList.length <= MAX_LENGTH, "length");
        require(!isKilled, "killed");
        uint256 requestId = vrfConsumer.requestRandomWords();
        for (uint8 i = 0; i < tokenIdList.length; ++i) {
            IERC721(polyMango).safeTransferFrom(msg.sender, BURN_ADDRESS, tokenIdList[i]);
            _peeledTokenIds[msg.sender].push(tokenIdList[i]);
            _tokenIdToRequest[tokenIdList[i]] = Request({id: requestId, owner: msg.sender, index: i, isClaimed: false});
        }
        emit Peel(msg.sender, tokenIdList, requestId);
    }

    function claim(uint256[] calldata tokenIdList) external {
        for (uint256 i = 0; i < tokenIdList.length; ++i) {
            Status status = getPeeledMangoStatus(tokenIdList[i]);
            Request storage request = _tokenIdToRequest[tokenIdList[i]];
            if (status == Status.PEELED) {
                request.isClaimed = true;
                mangoPass.mint(request.owner, tokenIdList[i]);
                emit Claim(request.owner, tokenIdList[i]);
            }
        }
    }

    function getPeeledMangoStatus(uint256 tokenId) public view returns (Status) {
        Request memory request = _tokenIdToRequest[tokenId];
        if (request.id == 0) {
            return Status.UNPEELED;
        }
        uint256 randomNumber = vrfConsumer.randomNumberMap(request.id);
        if (randomNumber == 0) {
            return Status.PEELING;
        }
        uint256 number = randomNumber.get8(request.index);
        if ((number % MAGIC_NUMBER) != 0) {
            return Status.ROTTEN;
        }
        if (request.isClaimed) {
            return Status.CLAIMED;
        }
        return Status.PEELED;
    }

    function getBatchPeeledMangoStatus(uint256[] memory tokenIds) public view returns (Result[] memory result) {
        result = new Result[](tokenIds.length);
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            result[i] = Result({status: getPeeledMangoStatus(tokenIds[i]), tokenId: tokenIds[i]});
        }
        return result;
    }
}
...
contract VRFConsumerV2 is VRFConsumerBaseV2 {
    ...
    uint32 public constant S_NUM_WORDS = 1;
    ...
    mapping(uint256 => uint256) public randomNumberMap;
    ...
    function requestRandomWords() external returns (uint256 requestId) {
        require(burner == msg.sender, "only burner");
        ...
        require(requestId > 0, "INVALID_ID");
    }
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        // S_NUM_WORDS is 1, so the length of randomWords must be 1.
        randomNumberMap[requestId] = randomWords[0];
