...
contract PlushCoinWallets is Ownable {
    ...
    function bet(uint256 _amount) public
    {
        ...
        require(plush.approve(address(this), _amount), "Approve has failed.");

        increaseWalletAmount(msg.sender, _amount);
        plush.transferFrom(msg.sender, address(this), _amount);
    }

    function withdrawToUser(uint256 _amount) external
    {
        uint256 percent = _amount * fee / 100;
        walletInfo[msg.sender].balance -= _amount;
        walletInfo[plushFeeWallet].balance += percent;
        plush.approve(msg.sender, _amount);
        plush.transferFrom(address(this), msg.sender, _amount - percent);
    }
    ...
    function decreaseWalletAmount(address _wallet, uint256 _amount) public
    {
        ...
        require(plushApps.isInStruct(msg.sender), 'You have no rights.');

        uint256 percent = _amount * fee / 100;

        walletInfo[_wallet].balance -= _amount;
        walletInfo[plushApps.getPayAddress(msg.sender)].balance += _amount - percent;
        walletInfo[plushFeeWallet].balance += percent;
    }
    ...
    function setFeePercent(uint256 _amount) external onlyOwner
    {
        fee = _amount;
    }
...
contract PlushApps is Ownable {
    ...
    function isInStruct(address _appAddress) public view returns(bool)
    {
        if(appsInfo[_appAddress].active){
            return true;
        }
