...
contract PoolFactory is Initializable, AccessControlEnumerableUpgradeable, TimeboundEIP712Upgradeable, UUPSUpgradeable, IPoolFactory, IContractFactory {
    ...
    function deployProxyByImplementation(address _impl, bytes calldata _data, bytes32 _salt) external onlyRole(DEFAULT_ADMIN_ROLE) returns (address poolAddr) {
        // ignore factory, packcontract and poolId
        (,,,uint128 _openStartTimestamp, uint128 _amountDistributedPerOpen) = abi.decode(_data[4:], (address, address, uint256, uint128, uint128));
