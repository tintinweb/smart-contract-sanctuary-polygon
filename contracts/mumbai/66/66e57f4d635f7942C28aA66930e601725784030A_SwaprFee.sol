    ...
    AccessControlUpgradeable
{
    ...
    function __BaseGovernanceWithUser_init(
        address governer
    ...
    function __BaseGovernanceWithUser_init_unchained(
        address governer
    ) internal onlyInitializing {
    ...
    function _authorizeUpgrade(
        ...
        require(
            hasRole(GOVERNANCE_ROLE, msg.sender) ||
                (hasRole(UPGRADE_MANAGER_ROLE, msg.sender) &&
                    (newImplementation == _proposedUpgrade)),
            "UPGR_NT_AUTH"
        );
    }
...
contract SwaprFee is BaseGovernanceWithUserUpgradable {
    ...
    struct FeePricing {
        ...
        uint finalFeeCap; //Cap is USD, 1 USD == 1e18
        uint priceCap;
    }
    ...
    function setLKRToken(
        address _lkrToken,
        address _lkrOracle,
        uint256 __discountAmount
    ) external onlyAdmin {
    ...
    function addPaymentToken(
        ...
        for (uint256 i; i < _paymentTokens.length; ) {
            if (
                !isTokenSupported(_paymentTokens[i]) &&
                _paymentTokens[i] != address(0)
            ) {
                paymentTokens.push(
                    IERC20MetadataUpgradeable(_paymentTokens[i])
                );
    ...
    function setNativeTokenPriceFeed(address _priceFeed) external onlyAdmin {
        require(_priceFeed != address(0), "Cant set address 0");
        _cryptoPriceFeed = AggregatorV3Interface(_priceFeed);
    }

    function removeNativeTokenPriceFeed() external onlyAdmin {
        delete _cryptoPriceFeed;
    }

    function getTokenFeed(
        address token
    ) public view returns (AggregatorV3Interface priceFeed) {
        if (token == address(0)) {
    ...
    function isTokenSupported(
        ...
        if (_paymentToken == address(0)) {
            address(_cryptoPriceFeed) != address(0)
                ? exists = true
                : exists = false;
        } else if (_paymentToken == address(lkrToken)) {
            exists = true;
        } else {
    ...
    function addDiscountTokens(
        address[] memory _discountToken,
        uint256[] memory __discountAmount
    ) external onlyAdmin {
        require(
            _discountToken.length == __discountAmount.length,
            "Invalid input"
        );
    ...
    function removeDiscountToken(address _discountToken) public onlyAdmin {
        (bool exists, ) = _exists(_discountToken);
        require(exists, "This is not a payment token");
    ...
    function getBaseOrderFee(
        ...
        public
        view
        returns (uint totalBaseFee, uint percentBaseFee, uint fixedBaseFee)
    {
        (totalBaseFee, percentBaseFee, fixedBaseFee) = _getBaseFee(
            OrderFee,
            subjectAmount,
            token
        );
    ...
    function getBaseAuctionFee(
        ...
        public
        view
        returns (uint baseFee, uint percentBaseFee, uint fixedBaseFee)
    {
        (baseFee, percentBaseFee, fixedBaseFee) = _getBaseFee(
            AuctionFee,
            subjectAmount,
            token
        );
    ...
    function getFinalOrderFee(
        uint subjectAmount,
        address token
    ) public view returns (uint finalFee) {
    ...
    function setOrderFinalFeeCap(uint finalFeeCap) external onlyAdmin {
    ...
    function setAuctionFinalFeeCap(uint finalFeeCap) external onlyAdmin {
    ...
    function payNow(uint fee, address paymentToken) public payable {
        if (isTokenSupported(paymentToken)) {
            if (fee > 0) {
                ...
                if (_discountAmount[paymentToken] > 0) {
                    feeToPay = (fee * _discountAmount[paymentToken]) / _EXP;
                }
                if (paymentToken == address(0)) {
                    require(msg.value >= feeToPay, "ERROR: LOW_VALUE_OBSERVED");
                    payable(feeReceiver).transfer(feeToPay);
                } else if (paymentToken == address(lkrToken)) {
                    uint allowance = lkrToken.allowance(
                        _msgSender(),
                        address(this)
                    );
                    require(allowance >= fee, "ERROR: INSUFFICIENT_ALLOWANCE");
                    lkrToken.safeTransferFrom(
                        _msgSender(),
                        feeReceiver,
                        feeToPay
                    );
                } else {
                    IERC20MetadataUpgradeable fundToken = IERC20MetadataUpgradeable(
                            paymentToken
                        );
                    uint allowance = fundToken.allowance(
                        _msgSender(),
                        address(this)
                    );
                    require(allowance >= fee, "ERROR: INSUFFICIENT_ALLOWANCE");
    ...
    function disposeFeeRecord(bytes calldata data) external onlySwapr {
        (uint fee, address user, address paymentToken) = abi.decode(
            data,
    ...
    function _extractFeePricingInfo(
        bytes memory data
    ) internal pure returns (FeePricing memory feePricing) {
        ...
        returns (uint totalBaseFee, uint percentBaseFee, uint fixedBaseFee)
    {
        ...
        if (token == address(0)) {
            uint baseRecount = ((feePricing.fixedBaseFee * _EXP) /
                _getPrice(_cryptoPriceFeed));
            fixedBaseFee =
                (baseRecount * _getFeedDecimals(_cryptoPriceFeed)) /
                _EXP;
        } else if (token == address(lkrToken)) {
            uint baseRecount = ((feePricing.fixedBaseFee * _EXP) /
                _getLkrPrice(lkrOracle));
            fixedBaseFee = (baseRecount * 10 ** 8) / _EXP;
        } else {
            uint baseRecount = ((feePricing.fixedBaseFee * _EXP) /
                _getPrice(oracleFeedForToken[token]));
            fixedBaseFee =
                (baseRecount * _getFeedDecimals(oracleFeedForToken[token])) /
                _EXP;
        }
    ...
    /// @param feePricing refer to type FeePricing type
    /// @return finalFee final calculated fee
    function _getFinalFee(
        FeePricing memory feePricing,
        uint subjectAmount
    ) internal pure returns (uint finalFee) {
        require(feePricing.finalFeePercentage <= _EXP, "BIGGER_THAN_MAX");
    ...
    function _exists(
        address _paymentToken
    ) internal view returns (bool exists, uint idx) {
    ...
    function _calculateFinalFeeCap(
        ...
        if (address(lkrToken) != address(0) && token == address(lkrToken)) {
            uint capRecount = ((feePricing.finalFeeCap * _EXP) /
                _getLkrPrice(lkrOracle));
            priceCapTokens = (capRecount * 10 ** 8) / _EXP;
        } else {
            uint capRecount = ((feePricing.finalFeeCap * _EXP) /
                _getPrice(getTokenFeed(token)));
            priceCapTokens =
                (capRecount * _getFeedDecimals(getTokenFeed(token))) /
                _EXP;
        }
    }
    ...
    function _calculatePriceCap(
        ...
        if (address(lkrToken) != address(0) && token == address(lkrToken)) {
            uint capRecount = ((feePricing.priceCap * _EXP) /
                _getLkrPrice(lkrOracle));
            priceCapTokens = (capRecount * 10 ** 8) / _EXP;
        } else {
            uint capRecount = ((feePricing.priceCap * _EXP) /
    ...
    function _getFeedDecimals(
        AggregatorV3Interface priceFeed
    ) internal view returns (uint priceFeedDecimals) {
