...
contract VisitedByDeployScript is Script {
    ...
    function run() external {
        ...
        PoVFactory _povFactory = new PoVFactory();
        PoVFactoryProxy _povFactoryProxy = new PoVFactoryProxy(address(_povFactory), abi.encodeWithSignature("initialize()"));
        PoVFactory povFactory = PoVFactory(address(_povFactoryProxy));
...
contract CheckContractScript is Script {
    ...
    function run() external {
        ...
        /// 4. PoVFactory
        PoVFactory povFactory = PoVFactory(address(POV_FACTORY_CONTRACT));
        _owner = povFactory.owner();
...
contract RegisterScript is Script {
    ...
    function run() external {
        ...
        string memory SPOT_BASEURI = vm.envString("SPOT_BASEURI");

        vm.startBroadcast();
        SpotRegistry spotRegistry = SpotRegistry(REGISTRY_CONTRACT);
        spotRegistry.register(SPOT_OWNER, SPOT_ADDRESS, SPOT_BASEURI);

        vm.stopBroadcast();
    }
}

contract ResolverDeployScript is Script {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VERIFIER_CONTRACT = vm.envAddress("VERIFIER_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    ...
    address SPOT_OWNER = vm.envAddress("SPOT_OWNER");
    address SPOT_ADDRESS = vm.envAddress("SPOT_ADDRESS");
    string SPOT_BASEURI = vm.envString("SPOT_BASEURI");

    function run() external {
        vm.startBroadcast();
        SpotResolver _resolver = new SpotResolver(msg.sender, SPOT_OWNER, SPOT_ADDRESS, SPOT_BASEURI, VISITEDBY_CONTRACT, POV_FACTORY_CONTRACT);
...
contract MintWithExtendedScript is Script {
    ...
    function run() external {
        vm.startBroadcast();
        VisitedBy visitedBy = VisitedBy(VISITEDBY_CONTRACT);
        visitedBy.mint(SIGNATURE, bytes32(EXTENDED));

        vm.stopBroadcast();
    }
}

contract AddPoVScript is Script {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VERIFIER_CONTRACT = vm.envAddress("VERIFIER_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    ...
    // argument
    address SPOT_ADDRESS = vm.envAddress("SPOT_ADDRESS");
    int256 LATITUDE = vm.envInt("LATITUDE");
    int256 LONGITUDE = vm.envInt("LONGITUDE");
    ...
    function run() external {
        ...
        SpotResolver(_resolverAddress).add(LATITUDE, LONGITUDE, POV_ID);
        vm.stopBroadcast();
    }
}
