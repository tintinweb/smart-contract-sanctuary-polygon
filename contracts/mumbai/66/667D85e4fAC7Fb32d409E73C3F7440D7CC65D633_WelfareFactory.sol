...
contract WelfareFactory is AccessControl {
    ...
    function newLotteryWithRNG(uint256 _limit, address payable _lottery)
        external
        onlyRole(FACTORY_ROLE)
        returns (address)
    {
...
contract Lottery is IRandomConsumer, Ownable, AccessControl {
    ...
    {
        for (uint256 index = 0; index < winningNumbers.length; index++) {
