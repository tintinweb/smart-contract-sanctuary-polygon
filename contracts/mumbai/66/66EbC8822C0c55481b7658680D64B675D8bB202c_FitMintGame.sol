...
contract FitMintGame is Ownable {
    ...
    mapping(address => uint256) public FITTBalances;
    ...
    function setadminAddr(address _adminAddress) public onlyOwner {
    ...
    function setNGAllowList(address[] calldata addresses, uint8 numAllowedToMint) public {
        require(msg.sender == adminAddress, "Only set Admin Address can set the NG allow list");
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowNGList[addresses[i]] = numAllowedToMint;
        }
    }

    function setOGAllowList(address[] calldata addresses, uint8 numAllowedToMint) public {
        require(msg.sender == adminAddress, "Only set Admin Address can set the OG allow list");
        for (uint256 i = 0; i < addresses.length; i++) {
            _allowOGList[addresses[i]] = numAllowedToMint;
        }
    }

    function setFITTBalances(address[] calldata addresses, uint256[] calldata balancelist) public {
        require(msg.sender == adminAddress, "Only set Admin Address can set the FITT balance");
        for (uint256 i = 0; i < addresses.length; i++) {
            FITTBalances[addresses[i]] = balancelist[i];
        }
    }

    function addFITTEarnings(address[] calldata addresses, uint256[] calldata balancelist) public {
        require(msg.sender == adminAddress, "Only set Admin Address can add FITT token to the user balance");
        for (uint256 i = 0; i < addresses.length; i++) {
            FITTBalances[addresses[i]] += balancelist[i];
        }
    }

    function burnFITTEarnings(address[] calldata addresses, uint256[] calldata balancelist) public {
        require(msg.sender == adminAddress, "Only set Admin Address can reduce FITT token from the user balance");
        for (uint256 i = 0; i < addresses.length; i++) {
            FITTBalances[addresses[i]] -= balancelist[i];
        }
    }
    ...
    function withdraw(uint _withdrawalAmount, address payable destAddr) public onlyOwner{
        require(address(this).balance >= _withdrawalAmount, "Insufficient funds");
        destAddr.transfer(_withdrawalAmount);
        emit TransferSent(msg.sender, destAddr, _withdrawalAmount);
    }
    ...
    function reserveNG(address _address) public {
        require(msg.sender == adminAddress, "Only set Admin Address can reserve NG NFT's");
        gameNFTInstance.mintNGNFT(_address);
    }
    
    function reserveOG(address _address) public {
        require(msg.sender == adminAddress, "Only set Admin Address can reserve OG NFT's");
        gameNFTInstance.mintOGNFT(_address);
    }

    function reserveToken(address _address, uint256 _tokenAmount) public {
        require(msg.sender == adminAddress, "Only set Admin Address can tranfer reserved FITT tokens");
