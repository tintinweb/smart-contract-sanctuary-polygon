...
contract Dao {
    ...
    function checkVotee() public
    {
        for(uint256 i = 0; i < validTokens.length; i++){
            votee[i] = daoContract.ownerOf(validTokens[i]);
        }
    }
    ...
    function createProposal(string memory _description) public {
        ...
        address[] memory vote;
        for(uint256 i = 0; i < validTokens.length; i++){
            vote[i] = daoContract.ownerOf(validTokens[i]);
        }
        ...
        newProposal.canVote  = vote;
        newProposal.maxVotes = vote.length;

        emit proposalCreated(nextProposal, _description, vote.length, msg.sender);
        nextProposal++;
    }
