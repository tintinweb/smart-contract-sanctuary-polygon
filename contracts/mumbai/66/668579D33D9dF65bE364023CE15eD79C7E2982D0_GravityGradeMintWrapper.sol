...
contract GravityGradeMintWrapper is Ownable, ITrustedMintable, IGravityGradeMintWrapper{
    ...
    constructor() Ownable(){
        s_trustedAddresses[msg.sender] = true;
    }

    function setGravityGradeContract(address _gravityGradeContract) external onlyTrusted{
    ...
    function trustedBatchMint(
        ...
        if(_tokenIds.length != _amounts.length)
            revert GravityGradeMintWrapper__TokenIdsAndAmountsNotSameLength();
