...
contract C2EDID {
    ...
    function createDID(
        ...
        DID memory _did = DID(_didID, _username, _fullname, msg.sender, _doc);
        dids[msg.sender] = _did;
        controllers[_didID].push(msg.sender);
        usernames[_username] = _didID;

        emit DIDCreated(msg.sender, _didID, _doc, _username, _fullname);
    }
    ...
    function deleteDID() external onlyController {
    ...
    function getName(
        ...
        DID memory _did = dids[_address];
        return (_did.username, _did.fullname);
    }
