    ...
    DefaultOperatorFilterer
{
    ...
    function batchMintTo(
        ...
        _safeMint(_to, _quantity, _data, 0);
    }
    ...
    ReentrancyGuard
{
     ...
     *  @param _quantity  The number of NFTs to mint.
     */
    function claim(address _receiver, uint256 _quantity, uint256 index) public payable virtual nonReentrant {
        require(_currentIndex + _quantity <= nextTokenIdToLazyMint, "Not enough lazy minted tokens.");
        if (index == 0)
        {
        require(_currentIndex + _quantity <= 2, "Cannot mint women NFT");
        verifyClaim(msg.sender, _quantity); // Add your claim verification logic by overriding this function.
        uint256 startTokenId = _transferTokensOnClaim(_receiver, _quantity, 0); // Mints tokens. Apply any state updates by overriding this function.
        ...
        verifyClaim(msg.sender, _quantity); // Add your claim verification logic by overriding this function.
        uint256 startTokenId = _transferTokensOnClaim(_receiver, _quantity, 1); // Mints tokens. Apply any state updates by overriding this function.
        emit TokensClaimed(msg.sender, _receiver, startTokenId, _quantity);
        }
    }
    ...
    /// @notice The tokenId assigned to the next new NFT to be claimed.
    function nextTokenIdToClaimWomen() public view virtual returns (uint256) {
        return _currentIndexWomen;
    }
     ...
     */
    function _transferTokensOnClaim(address _receiver, uint256 _quantity, uint256 index)
        internal
        virtual
        returns (uint256 startTokenId)
    {
        if (index == 0){
            ...
            _safeMint(_receiver, _quantity, 0);
        }
        else if (index == 1){
            startTokenId = _currentIndexWomen;
            _safeMint(_receiver, _quantity, 1);
        }
    }
...
contract ERC721A is Context, ERC165, IERC721A {
    ...
    function _totalMinted() internal view returns (uint256) {
        ...
        unchecked {
            return _currentIndexWomen - _startTokenId();
        }
    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        return _startTokenId() <= tokenId && tokenId < _currentIndexWomen && !_ownerships[tokenId].burned;
    }
    ...
    function _safeMint(
        ...
        unchecked {
            ...
            if (to.isContract()) {
                ...
                if (index == 0 && _currentIndex != startTokenId) revert();
                if (index == 0 && _currentIndexWomen != startTokenId) revert();
            } else {
                do {
                    emit Transfer(address(0), to, updatedIndex++);
                } while (updatedIndex < end);
...
interface IClaimableERC721 {
     ...
     *  @param _quantity  The number of NFTs to mint.
     */
    function claim(address _receiver, uint256 _quantity, uint256 index) external payable;
...
contract Contract is ERC721LazyMint {
    ...
    ERC721Base public immutable burnable;
    ...
    constructor(
        ...
        burnable = ERC721Base(_burnAddress);
    }
    ...
    override
    {
        require(burnable.balanceOf(_claimer) <= _quantity, "get some nfts brokie");
    }

    function claimNFT(address _reciever, uint256 _quantity, uint256 index)
        public
        payable
        virtual
    {
        burnable.burn(index);
        super.claim(_reciever, _quantity, 0);
    }

    function claimNFTWomen(address _reciever, uint256 _quantity, uint256 index)
        public
        payable
        virtual
    {
        burnable.burn(index);
        super.claim(_reciever, _quantity, 1);
    }
}