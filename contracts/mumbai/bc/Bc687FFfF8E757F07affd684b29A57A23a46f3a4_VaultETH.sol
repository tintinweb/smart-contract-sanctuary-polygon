...
contract VaultETH is Ownable {
    ...
    AggregatorV2V3Interface public ethOracle;
    ...
    constructor(
        ...
        ethOracle = AggregatorV2V3Interface(_ethOracle);
    }
    ...
    function getLatestPrices() public view returns (uint256, uint256) {
        ...
        int256 price1 = ethOracle.latestAnswer();
    ...
    function updatePrice() public {
        ...
        (uint256 changeInPrice, bool isIncreased) = priceChange(currentPrice1);
          if(isIncreased) {
            price[ETH] = lastPrice.mul(leverage.mul(changeInPrice).add(denominator)).div(denominator);
            price[eETH] = lastPrice.mul(denominator.sub(leverage.mul(changeInPrice))).div(denominator);
          }
          if(!isIncreased){
            price[ETH] = lastPrice.mul(denominator.sub(leverage.mul(changeInPrice))).div(denominator);
            price[eETH] = lastPrice.mul(leverage.mul(changeInPrice).add(denominator)).div(denominator);
          }
    }
    ...
    function geteETHToken() public view returns (address) {
        return (eETH);
    }

    function getETHToken() public view returns (address) {
        return (ETH);
    }
    ...
    function setTokens(address eTYAddress, address eITYAddress)
        ...
        onlyOwner
    {
        require(ETH == address(0) || eETH == address(0));
        (eETH, ETH) = (eITYAddress, eTYAddress);
        price[ETH] = initialPrice;
        price[eETH] = initialPrice;
    }
