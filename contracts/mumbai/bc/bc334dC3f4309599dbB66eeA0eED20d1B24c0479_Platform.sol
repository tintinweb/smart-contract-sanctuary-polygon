    ...
    IPlatform
{
    ...
    event Transfer(address[] tos_, address indexed token_, uint256[] amounts_);
    ...
    function _transfer(
        ...
        require(
            tos_.length == amounts_.length,
            "the length of users is different from the length of amounts"
        );
        for (uint256 i = 0; i < tos_.length; i++) {
            if (token_ == address(0x0)) payable(tos_[i]).transfer(amounts_[i]);
            else IERC20(token_).transfer(tos_[i], amounts_[i]);
        }
        emit Transfer(tos_, token_, amounts_);
    }
    ...
    function _creditBalanceOf(address token_) internal view returns (uint256) {
        if (token_ == address(0x0)) return _msgSender().balance;
        else {
            uint256 allowance = IERC20(token_).allowance(
                _msgSender(),
                address(this)
            );
            uint256 balance = IERC20(token_).balanceOf(_msgSender());
    ...
    mapping(address => mapping(address => uint256)) private _projectBalances;
    ...
    function _projectBeneficiaryOf(address project_)
        internal
        view
        returns (address)
    {
        return IProject(project_).beneficiary();
    }

    function _projectDownloadUrlOf(address project_)
        internal
        view
        returns (string memory)
    {
        return IProject(project_).downloadUrl();
    }

    function _projectFeeRateOf(address project_)
        internal
        view
        returns (uint256)
    {
        return _projectDetails[project_].feeRate;
    }
    ...
    function _projectBalanceOf(address project_, address token_)
        internal
        view
        returns (uint256)
    {
        return _projectBalances[project_][token_];
    }
    ...
    {
        return
            _projectFeeRateOf(project_) == 0
                ? amount_
                : (amount_ * _projectFeeRateOf(project_)) / 10000;
    }

    function _projectPay(
        address project_,
        bytes32 id_,
        address user_,
        address token_,
        ...
        _projectBalances[project_][token_] += amount_;
        _projectDetails[project_].fundFlow[token_] += amount_;
        emit Pay(project_, id_, user_, token_, amount_);
        IAgentExtension(
            _getExtension(keccak256("EXTENSION_AGENT")).contractAddress
        ).addFundFlow(user_, token_, amount_);
    }

    function _projectSettle(
        address project_,
        bytes32 id_,
        ...
        for (uint256 i = 0; i < users_.length; i++) {
            _projectBalances[project_][token_] -= amounts_[i];
            realAmounts[i] = _realAmountByProjectFeeRate(project_, amounts_[i]);
        }
        for (uint256 i = 0; i < users_.length; i++)
            _increaseReward(users_[i], token_, realAmounts[i]);
        emit Settle(project_, id_, users_, token_, amounts_);
    }

    function _queryProjectFundFlow(address project_)
        internal
        view
        returns (mapping(address => uint256) storage)
    {
        return _projectDetails[project_].fundFlow;
    }
    ...
    function _userBlocked(address user_) internal view returns (bool) {
    ...
    function transferExtensionOwnership(bytes32 extension_, address newOwner)
        external
        override
        onlyHasAuthority(AUTHORITY_SUPER, _msgSender())
        onlyExtensionDefined(extension_)
    {
        IExtension(_getExtension(extension_).contractAddress)
            .transferExtensionOwnership(newOwner);
    }
    ...
    function transferDefaultTokenOwnership(address newOwner_)
        ...
        onlyHasAuthority(keccak256("MANAGE_FUND"), _msgSender())
    {
        IPlatformToken(_defaultToken).transferTokenOwnership(newOwner_);
    }
    ...
    function transfer(
        ...
        onlyHasAuthority(keccak256("MANAGE_FUND"), _msgSender())
    {
        _transfer(tos_, token_, amounts_);
    }
        ...
        onlyUserNotBlocked(_msgSender())
    {
        ...
        amounts[0] = amount_;
        _transfer(tos, token_, amounts);
    }

    function creditBalanceOf(address token_)
        external
        view
        override
        returns (uint256)
    {
        return _creditBalanceOf(token_);
    }
    ...
    function projectBeneficiaryOf(address project_)
        external
        view
        override
        ...
        returns (address)
    {
        return _projectBeneficiaryOf(project_);
    }

    function projectDownloadUrlOf(address project_)
        external
        view
        override
        onlyProjectExist(project_)
        returns (string memory)
    {
        return _projectDownloadUrlOf(project_);
    }
    ...
    function projectFeeRateOf(address project_)
        external
        view
        override
        onlyProjectExist(project_)
        returns (uint256)
    {
        return _projectFeeRateOf(project_);
    }
    ...
    function projectPay(
        ...
        onlyUserRegistered(user_)
    {
        _projectPay(_msgSender(), id_, user_, token_, amount_);
    }

    function projectSettle(
        bytes32 id_,
        address[] calldata users_,
        address token_,
        uint256[] calldata amounts_
    ) external override onlyProjectExist(_msgSender()) {
        _projectSettle(_msgSender(), id_, users_, token_, amounts_);
    }

    function projectBalanceOf(address project_, address token_)
        external
        view
        override
        onlyProjectExist(project_)
        returns (uint256)
    {
        return _projectBalanceOf(project_, token_);
    }

    function queryProjectFundFlow(address project_, address token_)
        external
        view
        override
        onlyProjectExist(project_)
        returns (uint256)
    {
        return _queryProjectFundFlow(project_)[token_];
    }
    ...
    function transferProjectOwnership(address project_, address newOwner_)
        ...
        onlyProjectExist(project_)
    {
        IProject(project_).transferProjectOwnership(newOwner_);
    }
    ...
    function register(address inviter_)
        external
        override
        whenNotPaused
        onlyUserNotRegistered(_msgSender())
    ...
    function isBlocked(address user_)
        external
        view
        override
        onlyUserRegistered(_msgSender())
    ...
    function queryUserFundFlow(address user_, address token_)
        external
        view
        override
        returns (uint256)
...
struct ProjectDetail {
    bool existed;
    uint256 feeRate;
    mapping(address => uint256) fundFlow;
}
...
interface IExtension {
    function transferExtensionOwnership(address newOwner_) external;
}
...
interface IAgentExtension is IExtension {
...
interface IPlatform {
    ...
    function creditBalanceOf(address token_) external view returns (uint256);
    ...
    function projectFeeRateOf(address project_) external view returns (uint256);
    ...
    // USER
    function register(address inviter_) external;
...
interface IPlatformToken {
    function transferTokenOwnership(address newOwner_) external;
...
interface IProject {
    function transferProjectOwnership(address newOwner_) external;
