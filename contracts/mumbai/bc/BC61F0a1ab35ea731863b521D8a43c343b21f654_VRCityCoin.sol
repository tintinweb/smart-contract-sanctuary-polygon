    ...
    ReentrancyGuard
{
    ...
    /// @return Fee percentage, which is deducted twice when purchasing tokens.
    uint256 public feePercentage; 
    /// @dev The wallet address where part of the fee goes.
     ...
     * @param _feePercentage is the fee percentage, which is deducted 
     twice when purchasing tokens.
     * @param _walletAddress is the wallet address where part of the fee goes.
     */
    constructor(
    ...
    ) ERC20("VR City Coin", "VRCITY") {
        require(_feePercentage * 2 < 1e18, "Too much fee percentage");
        ...
        uint256 quotient = 10**(decimals() - USDC.decimals());
        precision = quotient;

        feePercentage = _feePercentage / (1e18 / quotient);
    ...
    function initVesting(
        ...
        IVesting(_vestingAddress).initVesting(_honors);

        emit InitializationVesting(_initSupply, _initReserve, _vestingAddress);
    }

    function purchase(uint256 _usdcAmount) external override nonReentrant {
        ...
        uint256 purchaseAmount = getPurchaseTotal(_usdcAmount - feeAmount * 2);
        _mint(msg.sender, purchaseAmount);
        reserve += _usdcAmount - feeAmount;
        emit PurchaseToken(msg.sender, purchaseAmount, _usdcAmount);
    }

    function sell(uint256 _tokenAmount) external override nonReentrant {
    ...
    function setFeePercentage(uint256 _newFeePercentage)
        ...
        onlyRole(ADMIN_ROLE)
    {
        require(_newFeePercentage * 2 < 1e18, "Too much fee percentage");
    ...
    function getPurchaseAmount(uint256 _usdcAmount)
        ...
        returns (uint256)
    {
        return getPurchaseTotal((_usdcAmount * (precision - feePercentage)) / precision);
    }
    ...
    function getReserveBalance() public view override returns (uint256) {
        return reserve;
    }

    function getReserveRatio() public view override returns (uint32) {
        return reserveRatio;
    }
}
