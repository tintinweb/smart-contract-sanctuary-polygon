...
contract TokenitySwap is Swap, SwapNative, Withdraw {
    constructor() {}
...
import "./helpers/SanctionsListContract.sol";
    ...
    SanctionsListContract
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  check if the Vendor Contract has enough amount of tokens for the transaction
        require(
            tokenA.balanceOf(address(this)) >= _tokenAmountToSell,
            "SellTokensFor: Vendor contract has not enough tokens in its balance"
        );

        require(
            tokenA.transfer(_msgSender(), _tokenAmountToSell),
    ...
    SanctionsListContract
{
    ...
    function buyToken(IERC20 _token)
        external
        payable
        noReentrant
        returns (uint256 tokenAmount)
    {
        ...
        if (fee > 0) {
            ...
            require(
                payable(address(owner())).send(_amountfee),
        ...
        // @dev  valid the amount of tokens to buy
        bool _limitValid = limitBuy(tokenAmountToBuy);
        require(_limitValid, "BuyToken: Exceeded TOKENETY token buy limit");

        // @dev check if the Vendor Contract has enough amount of tokens for the transaction
        require(
            _token.balanceOf(address(this)) >= tokenAmountToBuy,
            "BuyToken: Vendor contract has not enough tokens in its balance"
        );

        // @dev Transfer token to the msg.sender
        require(
            _token.transfer(_msgSender(), tokenAmountToBuy),
    ...
    function sellTokens(IERC20 _token, uint256 _amountTokens)
        external
        noReentrant
        returns (uint256 tokenAmount)
    {
        ...
        // Check that the user's token balance is enough to do the swap
        require(
            _token.balanceOf(_msgSender()) >= _amountTokens,
            "SellTokens: Your balance is lower than the amount of tokens you want to sell"
        );

        // @dev allowonce to execute the swap
        require(
            _token.allowance(_msgSender(), address(this)) >= _amountTokens,
            "SellTokens: You don't have enough tokens to sell"
        );

        // Transfer token to the sender -> sc
        require(
            _token.transferFrom(_msgSender(), address(this), _amountTokens),
        ...
        // @dev  valid the amount of tokens to Sell
        bool _limitValid = limitSell(tokenSendSub);
        require(_limitValid, "SellTokens: Exceeded TOKENETY token selll limit");
        ...
        require(
            ...
            "SellTokens_ Vendor has not enough funds to accept the sell request"
        );

        // @dev  we send matic to the sender
        (bool success, ) = _msgSender().call{value: tokenAmountToSell}("");
        require(success, "sellTokens: receiver rejected ETH transfer");
...
contract SanctionsListContract {
    address public SANCTIONS_CONTRACT = 0x40C57923924B5c5c5455c48D93317139ADDaC8fb;

    // @dev Address Oracle
    function setAddressSanctions(address _address) external returns (bool) {
...
contract TransferHistory is Context, Ownable {
    ...
    function limitSell(uint256 sellAmount) public returns (bool) {
        ...
        //     "Sell: Exceeded TOKENETY token sell limit"
        // );
        // _;
    }
    ...
    // @dev
    uint256 public dayBuyLimit = 10000000000000000000000;
    mapping(address => BuyOnDay) public buyInADay;

    // @dev  Throws if you exceed the Buy limit
    function limitBuy(uint256 buyAmount) public returns (bool) {
