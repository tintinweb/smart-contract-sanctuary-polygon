...
// File: contracts/16_Final_contractV2.sol
...
contract adminMain is Ownable {
    ...
    function addOrganization(string memory organizationName) public onlyOwner returns (address) { //also think of returning orginization insterd of address (i think its the same thing)
        ...
        Organization OrganizationAddress = Organization(Clones.clone(OrganizationImplementationAddress));

        // since the clone create a proxy, the constructor is redundant and you have to use the initialize function
        OrganizationAddress.initialize(EventImplementationAddress,contractOwnerAddress); //done //pass in the parameters of their address later (also pass in the implementation address) <-----------------------------------------
...
contract Organization {
    ...
    function initialize(address _EventImplementationAddress,address _contractOwnerAddress) public{ //change this to contract only and only owners 
...
contract Event { // is Ownable, AccessControl
    ...
    function _assignCertificate(
        ...
        require(contractOwnerAddress == msg.sender);
        require(certCreate >100); //just in case 2 prevent underflow attacks (not a problem in this vertion of solidity)
        ...
        holdersCertificate[assignTo].push(Certificate(name, organization, url, assignTo));
