...
contract DeceaLink is Ownable {
    ...
    uint256 licenseTimerMs = 2592000000;
    ...
    function activatedBot(uint8 id) public payable onlyOwner {
        // only the customer can use this function
        require(msg.sender == ownerAddress);
        require(botExists[id], "Bot does not exist");
    ...
    function deactivatedBot(uint8 id) public payable onlyOwner {
        // only the customer can use this function
        require(msg.sender == ownerAddress);
        require(botExists[id], "Bot does not exist");
        uint8 currentid = id - 1;
    ...
    function withdrawToken(address _tokenContract) public payable onlyOwner {
    ...
    function payLicense(
        ...
        if (msg.value >= bot.botLicensePay) {
            ...
            if (isLicenseExist == true) {
                licExpired = insertLicenseData.licenseExpired + licenseTimerMs;
                insertLicenseData.licenseExpired = licExpired;
                insertLicenseData.licenseActivated = isActivated.YES;
                ...
                totalLicense++;
                licExpired = block.timestamp + licenseTimerMs;
                licenseDataList[deviceId].push();
    ...
    function activatedBot(uint256 licenseTime) public payable onlyOwner {
        // only the customer can use this function
        require(msg.sender == ownerAddress);
