...
interface IEZR is IYousovStructs {
  ...
  function transferFromFromRecoveryFactoryOrRecoveries(address from, address to, uint256 amount) external;
...
contract Recovery is IRecovery, YousovRoles, ERC2771Context {
    ...
    function startTheRecovery() external recoveryPanicOff override {
        ...
        require(
            recoveryStatus == RecoveryStatus.READY_TO_START &&
                ( _msgSender() == user || IRecoveryFactory(recoveryFactory).temporaryWalletMappings( _msgSender()) == user )
                &&
                (agentList.length ==
                    IUser(
                        IUserFactory(
    ...
    function agentCheckUserAnswer(
        ...
        if (actualRecoveryStats.isAllAnswersAgentsAnswered) {
            if (
                ...
                address _tempWalletAddress = IRecoveryFactory(recoveryFactory).temporaryRecoverytemporaryWallet(address(this));
                
                if (_tempWalletAddress != address(0)) {
                // transfer all finds to user
                IEZR(ezr).transferFromFromRecoveryFactoryOrRecoveries(_tempWalletAddress, user,  IERC20(ezr).balanceOf(_tempWalletAddress));
                }
...
contract RecoveryFactory is IRecoveryFactory, YousovRoles, ERC2771Context {
    ...
    mapping(address => address) public temporaryWalletMappings;
    mapping(address => address) public temporaryRecoverytemporaryWallet;
    ...
    constructor(
        ...
        managerController = _managerController;
        recoveryBroadcast = _recoveryBroadcast;
        IRecoveryBroadcast(_recoveryBroadcast).setRecoveryFactoryAddress(address(this));
    }
    ...
    function createRecovery(
        ...
        require(
            !userHasRecovery(_msgSender()) &&
                !userHasRecovery(recoveredAccount) &&
                ((YousovAccessControl(yousovAccessControl).hasRole(
                    ...
                    (YousovAccessControl(yousovAccessControl).hasRole(
                        YOUSOV_USER_ROLE,
                        _msgSender()
                    ) && _msgSender() == recoveredAccount)) && (IUserFactory(userFactory).yousovUserList().length >=
                IManagerController(managerController).getValueInt(
                    "minimumAgentsAppear"
                ) &&
                isTrustedForwarder(msg.sender) ),
            "YOUSOV : Operation not authorized, not the trustedForwarder"
        ...
        uint256 recoveryPriceEZR = IManagerController(managerController)
            .getValueInt("minimumRecoveryPriceEzr");
        uint256 toAgentAPW = (recoveryPriceEZR *
            IManagerController(managerController).getValueDec(
                "agentPayrollWalletFromRecovery"
            )) / 100;
        uint256 toBurn = (recoveryPriceEZR *
            IManagerController(managerController).getValueDec("burn")) / 100;
        uint256 toEST = recoveryPriceEZR - (toAgentAPW + toBurn);

        IEZR(ezr).transferFromFromRecoveryFactoryOrRecoveries(
            _msgSender(),
            agentPayrollWallet,
            toAgentAPW
        );
        ...
        IEZR(ezr).transferFromFromRecoveryFactoryOrRecoveries(
            _msgSender(),
            IManagerController(managerController).treasuryAddress(),
            toEST
        );
        ...
        if (
            ...
            temporaryWalletMappings[_msgSender()] = recoveredAccount;
            temporaryRecoverytemporaryWallet[_newRecovery] = _msgSender();
        }
        recoveries.push(_newRecovery);
        if (currentCreatedRecoveries[_msgSender()] != address(0)) {
          delete currentCreatedRecoveries[_msgSender()];            
        }
    ...
    function userHasRecovery(address userAddress) public view returns (bool) {
            ...
            if (
                ((YousovAccessControl(yousovAccessControl).hasRole(
                    ...
                    (YousovAccessControl(yousovAccessControl).hasRole(
                        ...
                        IRecovery(recoveries[i]).user() ==
                        temporaryWalletMappings[userAddress])) &&
    ...
    function recoveryExist(address _recovery) external view override returns (bool) {
        for (uint i = 0; i < recoveries.length; ++i) {
    ...
    function addActiveAgent(
        address newActiveAgent,
        address linkedRecovery
    ...
    function getLegalSelectableAgents(
        ...
        require(
            isTrustedForwarder(msg.sender),
            "YOUSOV :Recovery don't exist or Operation not authorized, not the trustedForwarder"
        );
       IRecoveryBroadcast(recoveryBroadcast).getLegalSelectableAgents(_msgSender(), _currentRecovery);
    }
