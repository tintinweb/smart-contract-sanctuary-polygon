...
contract DragonMutation is BaseAccessControl, ReentrancyGuard, Pausable {

    string public constant BAD_CALLER_ERROR = "DragonMutation: the caller is not an owner of the dragon";
    string public constant BAD_DRAGON_ERROR = "DragonMutation: the given dragon cannot be mutated";
    string public constant BAD_EGGS_ERROR = "DragonMutation: bad eggs";
    string public constant BAD_AMOUNT_ERROR = "DragonMutation: incorrect amount sent to the contract";

    address private _dragonCreatorAddress;
    address private _dragonTokenAddress;
    address private _eggTokenAddress;
    address private _coinContractAddress;

    uint private _mutationPrice;
    uint[] private _ingredientSets;
    mapping(uint => bool) private _ingredientSetsMap;

    event MutantCreated(
        address indexed sender, 
        uint originalDragonId, 
        uint mutantDragonId, 
        uint[] eggs, 
        uint set);
    ...
    BaseAccessControl(accessControl) {
        _mutationPrice = mutationPrice;

        _dragonCreatorAddress = dragonCreator;
        _dragonTokenAddress = dragonToken;
        _eggTokenAddress = eggToken;
        _coinContractAddress = coinContract;

        // 4 Random + 1 Epic 
        // 4 Random + 1 Legendary 
        // 4 Random + 1 Epic + 1 Legendary
        _ingredientSets = [0x410, 0x401, 0x411];
    }
    ...
    function dragonTokenAddress() public view returns (address) {
        return _dragonTokenAddress;
    }

    function setDragonTokenAddress(address newAddress) external onlyRole(COO_ROLE) {
        address previousAddress = _dragonTokenAddress;
        _dragonTokenAddress = newAddress;
        emit AddressChanged("dragonToken", previousAddress, newAddress);
    }
    ...
    function coinContractAddress() public view returns (address) {
        return _coinContractAddress;
    }

    function setCoinContractAddress(address newAddress) external onlyRole(COO_ROLE) {
    ...
    function setPrice(uint newPrice) external onlyRole(CFO_ROLE) {
        uint previousPrice = _mutationPrice;
        _mutationPrice = newPrice;
        emit ValueChanged("price", previousPrice, newPrice);
    }

    function ingredientSets() external view returns (uint[] memory) {
        return _ingredientSets;
    }

    function setIngredientSets(uint[] calldata newSets) external onlyRole(CFO_ROLE) {
        //clear the previous sets
        for (uint i = 0; i < _ingredientSets.length; i++) {
            delete _ingredientSetsMap[_ingredientSets[i]];
        }
        //set new sets
        _ingredientSets = newSets;
        for (uint i = 0; i < newSets.length; i++) {
            _ingredientSetsMap[newSets[i]] = true;
        }
    } 
    ...
    function mutate(uint dragonId, uint[] calldata eggs, uint amount) 
    external
    nonReentrant
    whenNotPaused {
        DragonToken dt = DragonToken(dragonTokenAddress());
        require(dt.ownerOf(dragonId) == _msgSender(), BAD_CALLER_ERROR);
        require(dt.canBeMutated(dragonId), BAD_DRAGON_ERROR);
        require(amount >= price(), BAD_AMOUNT_ERROR);

        uint resultSet = _checkEggs(eggs);
        require(resultSet > 0, BAD_EGGS_ERROR);

        if (amount > 0) {
            IERC20(coinContractAddress()).transferFrom(_msgSender(), address(this), amount);
        }
        
        DragonInfoV2.Details memory details = dt.dragonInfo(dragonId);
        uint mutantDragonId = DragonCreator(dragonCreatorAddress())
            .giveBirth(dragonId, 0, details.genes, _msgSender());

        emit MutantCreated(_msgSender(), dragonId, mutantDragonId, eggs, resultSet);
    }

    function _checkEggs(uint[] calldata eggs) internal view returns (uint) {
        EggToken et = EggToken(eggTokenAddress());
        uint[3] memory eggSet = [uint(0), 0, 0];
        for (uint i = 0; i < eggs.length; i++) {
            if (et.ownerOf(eggs[i]) == _msgSender()) {
                EggInfo.Details memory info = et.eggInfo(eggs[i]);
                if (info.hatchedAt > 0) {
                    if (info.dragonType == DragonInfo.Types.Unknown) 
                        eggSet[0]++;
                    else if (info.dragonType == DragonInfo.Types.Epic20)
                        eggSet[1]++;
                    else if (info.dragonType == DragonInfo.Types.Legendary)
                        eggSet[2]++;
                    else
                        return 0;
                }
        ...
        uint resultSet = (eggSet[0] << 8) | (eggSet[1] << 4) | eggSet[2];
        return _ingredientSetsMap[resultSet] ? resultSet : 0;
    }

    function withdrawCoins(uint amount, address to) external virtual onlyRole(CFO_ROLE) {
        IERC20(coinContractAddress()).transfer(to, amount);
