...
contract ABTProofVerifier is FunctionsClient, ConfirmedOwner {
    ...
    function verifyProof(Request memory request) public returns (bytes32) {
        require(abtTokenAddress != address(0), "ABTToken address is not set");
        require(bytes(request.source).length > 0, "Source is not set");
        require(bytes(request.key).length > 0, "Key is not set");
        require(
            request.targetAddress != address(0),
            "Target address is not set"
        );
        require(request.gasLimit > 0, "Gas limit is not set");
        require(request.subscriptionId > 0, "Subscription ID is not set");
        ...
        ) = abtToken.getProof(request.targetAddress, request.key);
        ...
        if (request.secrets.length > 0) {
            req.addRemoteSecrets(request.secrets);
        }
