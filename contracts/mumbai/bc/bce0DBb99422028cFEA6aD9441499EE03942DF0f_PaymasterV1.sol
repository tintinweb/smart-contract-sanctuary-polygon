...
contract PaymasterV1 is BasePaymaster {
    ...
    uint256 public constant CUSTOM_FORWARDER_HUB_OVERHEAD = 50000;

    //These parameters are documented in IPaymaster.GasAndDataLimits
    uint256 public constant CUSTOM_PRE_RELAYED_CALL_GAS_LIMIT = 150000;
    uint256 public constant CUSTOM_POST_RELAYED_CALL_GAS_LIMIT = 160000;
    uint256 public constant CUSTOM_PAYMASTER_ACCEPTANCE_BUDGET =
        CUSTOM_PRE_RELAYED_CALL_GAS_LIMIT + CUSTOM_FORWARDER_HUB_OVERHEAD;
    uint256 public constant CUSTOM_CALLDATA_SIZE_LIMIT = 10500;
    ...
    {
        return "2.2.3+opengsn.custom-paymaster.ipaymaster";
    }
    ...
    function setFeePayer(address _feePayer) public onlyOwner {
        feePayer = _feePayer;
    }
        ...
        returns (IPaymaster.GasAndDataLimits memory limits)
    {
        return
            IPaymaster.GasAndDataLimits(
                CUSTOM_PAYMASTER_ACCEPTANCE_BUDGET,
                CUSTOM_PRE_RELAYED_CALL_GAS_LIMIT,
                CUSTOM_POST_RELAYED_CALL_GAS_LIMIT,
                CUSTOM_CALLDATA_SIZE_LIMIT
            );
    }
    ...
    function addToken(
        ...
        require(_token != address(0), "token address must not zero");
        require(_oracle != address(0), "oracle address must not zero");
        require(
            _feeCollector != address(0),
            "fee collector address must not zero"
        );

        oracles[_token] = _oracle;
        feeCollectors[_token] = _feeCollector;
    ...
    ) internal view returns (uint256 tokenPreCharge, uint256 tokenPerEth) {
        uint256 maxCharge = relayHub.calculateCharge(
            maxPossibleGas,
            relayRequest.relayData
        ) + relayRequest.request.value;

        (, int256 answer, , , ) = oracle.latestRoundData();
        uint8 decimal = oracle.decimals();
        uint256 price = uint256(10**decimal).div(convertToUint(answer));

        return (maxCharge.mul(price), price);
    }

    function _calculateActualCharge(
        uint256 gasUseWithoutPost,
        ...
        internal
        view
        returns (uint256 tokenActualCharge, uint256 ethActualCharge)
    {
        uint256 charge = relayHub.calculateCharge(
            gasUseWithoutPost.add(gasUsedByPost),
            relayData
        );

        return (charge.mul(tokenPerEth), charge);
    }
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        address token = MinLibBytes.readAddress(
            relayRequest.relayData.paymasterData,
            12
        );

        address oracle = oracles[token];
        address collector = feeCollectors[token];
        require(
            oracle != address(0) && collector != address(0),
            "not valid token"
        );
        ...
        (bool success, ) = token.call(
            relayRequest.relayData.paymasterData[32:]
        );
        require(success, "preRelayedCall - permit call reverted");

        (uint256 tokenPreCharge, uint256 tokenPerEth) = _calculatePreCharge(
            relayRequest,
            maxPossibleGas,
            AggregatorV3Interface(oracle)
        );
        ...
        return (abi.encode(payer, collector, tokenPerEth), false);
    }
    ...
    function postRelayedCall(
        ...
        (address payer, address collector, uint256 tokenPerEth) = abi.decode(
            context,
        ...
        IERC20(token).safeTransfer(collector, tokenActualCharge);

        IERC20(WETH).safeTransferFrom(feePayer, address(this), ethActualCharge);
        IWETH(WETH).withdraw(ethActualCharge);
    ...
    function convertToUint(int256 val) internal pure returns (uint256) {
        if (val < 0) {
            return uint256(-val);
        } else {
            return uint256(val);
