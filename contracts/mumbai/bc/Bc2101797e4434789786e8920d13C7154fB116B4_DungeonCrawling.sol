...
contract DungeonCrawling is Initializable, DungeonCrawlingSettings {
    ...
    private
    {
        require(_entities.length >= dungeonNameToInfo[_dungeonName].minEntitiesPerCrawl, "Too few entities");
        require(dungeonNameToInfo[_dungeonName].maxEntitiesPerCrawl >= _entities.length, "Too many entities");

        require(_entities.length == _entityInputQuantities.length, "Bad entity input length");
        ...
        for(uint256 i = 0; i < _entities.length; i++) {
            ...
            _storeEntity(_dungeonName, _entities[i], ORC_EQUIPPABLE, _level);
    ...
    private
    {
        ...
        for(uint256 i = 0; i < _inputQuantities.length; i++) {
            ...
            if(_quantity == 0 && _inputRequirement.minQuantity == 0) {
                ...
                require(_quantity >= _inputRequirement.minQuantity, "Quantity too small");
                require(_inputRequirement.maxQuantity == 0 || _quantity <= _inputRequirement.maxQuantity, "Quantity too large");
    ...
    contractsAreSet
    {
        ...
        if(_dungeonInfo.dungeonCooldownPeriod > 0) {
        ...
        if(_dungeonInfo.dungeonUnlockPeriod == 0) {
        ...
        emit DungeonEnded(msg.sender, _userInfo.activeDungeonName, _entityCooldown, _unlockTimeReady, _outcome);
    }
    ...
    function _unstakeItemsAndEntitiesIfPossible() private returns(uint256) {
        ...
        for(uint256 i = _userInfo.lockedInputIndexes.length; i > 0; i--) {
            ...
            DungeonCrawlingInputs storage _inputs = dungeonCrawlingInputs[_lockedEntityInfo.inputIndex];
    ...
    function activeDungeonInfoForUser(address _user) external view returns(DungeonCrawlingInfoForUser memory) {
        if(!_isUserDungeonCrawling(_user)) {
            return DungeonCrawlingInfoForUser(
                "",
                new DungeonCrawlingEntity[](0),
                new DungeonSuppliedInput[](0),
                new DungeonSuppliedInput[][](0)
            );
        }

        return DungeonCrawlingInfoForUser(
            userToInfo[_user].activeDungeonName,
            dungeonCrawlingInputs[userToInfo[_user].activeInputIndex].entities,
            dungeonCrawlingInputs[userToInfo[_user].activeInputIndex].inputs,
            _suppliedEntityInputsForUser(_user)
        );
    }

    function _suppliedEntityInputsForUser(address _user) private view returns(DungeonSuppliedInput[][] memory) {
        if(!_isUserDungeonCrawling(_user)) {
            return new DungeonSuppliedInput[][](0);
        }
        DungeonCrawlingInputs storage _inputs = dungeonCrawlingInputs[userToInfo[_user].activeInputIndex];

        DungeonSuppliedInput[][] memory _entityInputs = new DungeonSuppliedInput[][](_inputs.entities.length);
        ...
        for(uint256 i = 0; i < _inputs.entities.length; i++) {
            ...
            _entityInputs[i] = _entityInputsStorage;
        }
...
abstract contract DungeonCrawlingSettings is Initializable, DungeonCrawlingContracts {
    ...
    requiresEitherRole(ADMIN_ROLE, OWNER_ROLE)
    {
        ...
        for(uint256 i = 0; i < _info.inputs.length; i++) {
            require(_info.inputs[i].itemId > 0, "Bad input item ID");
...
abstract contract DungeonCrawlingState is ERC721HolderUpgradeable, ERC1155HolderUpgradeable, UtilitiesUpgradeable {
    ...
    event DungeonStarted(
        ...
        DungeonSuppliedInput[][] _entityInputs,
...
struct DungeonCrawlingInfoForUser {
    string activeDungeonName;
    DungeonCrawlingEntity[] entities;
    DungeonSuppliedInput[] inputs;
    DungeonSuppliedInput[][] entityInputs;
}
