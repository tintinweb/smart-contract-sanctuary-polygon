...
/**
 * @title FakeAltroTradeChecker
 * @author FakeAltro Developer
 * @dev Contract that acts as a trade checker for initial ERC721 token sales on 0x. It verifies that the buyer is in the FakeAltro allowlist
 * before allowing the trade to proceed.
 */
contract FakeAltroTradeChecker is ERC721Holder {
	...
	modifier onlyAllowListed() {
		require(allowList.isAddressAllowed(msg.sender), "FakeAltroTradeChecker: address not allowed");
		_;
	}

	/**
	 * @dev Constructor function that initializes the FakeAltroTradeChecker contract
	 * @param _zeroExContract Address of the 0x contract to interact with
	 * @param _allowList Address of the contract that manages the FakeAltro allowlist
	 */

	constructor(address payable _zeroExContract, address _allowList, address _feeManager) {
		require(_zeroExContract != address(0) && _allowList != address(0), "FakeAltroTradeChecker: cannot be null address");
		require(_allowList.supportsInterface(type(IAllowList).interfaceId), "FakeAltroTradeChecker: does not support IAllowList interface");
		require(_feeManager.supportsInterface(type(IFeeManager).interfaceId), "FakeAltroTradeChecker: does not support IFeeManager interface");
