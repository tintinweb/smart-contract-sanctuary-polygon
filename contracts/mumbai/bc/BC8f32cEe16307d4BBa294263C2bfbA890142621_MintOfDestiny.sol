...
contract MintOfDestiny is Pausable, AccessControl, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    */
    ...
    // Set the roles
    bytes32 private constant _admins = keccak256("_admins");
    bytes32 private constant _trustedContracts = keccak256("_trustedContracts");
    ...
    // Array of tokens vaulted in the contract for the MINTOFDESTINY owner to claim
    ...
    */
    ...
    function addRole(string memory roleName, uint256 tokenLimit, bool allowPublicMint, uint256 price, string memory mandatoryRole, bool hasAttachedMint, string memory attachedMintRole) public onlyAdmins whenNotPaused {
        ...
        // The first role must be MINTOFDESTINY
        if (roleNames.length == 0){
            require(stringsEqual(roleName, "MINTOFDESTINY"), "First role must be MINTOFDESTINY");
            tokenLimit = 1;
        ...
        // The first role is MINTOFDESTINY token (1of1), so we skip that one
        if (roleNames.length > 1){
            // Mint the first one and stake it to the contract for MINTOFDESTINY Owner to claim
    ...
    function updateRoleDetails(string memory roleName, bool allowPublicMint, uint256 price, string calldata mandatoryRole, bool hasAttachedMint, string calldata attachedMintRole) public onlyAdmins {
    ...
    function killRole(string memory roleName) public onlyAdmins {
    ...
    function pushTheBigGreenButton() public {
        // Only puny hoomans can call this (stops scammers from phishing MOD owner)
        require(tx.origin == msg.sender);

        // Only allow the owner of MINTOFDESTINY to claim all the tokens in the vault.
        require(mod.ownerOf(roles["MINTOFDESTINY"].tokenStartID) == msg.sender, "Hmm, don't have time to play with myself.");
    ...
    function bigGreenButtonFailSafe(uint256 tokenID) public {
        // Only puny hoomans can call this (stops scammers from phishing MOD owner)
        require(tx.origin == msg.sender);
        ...
        mod.safeTransferFrom(address(this), mod.ownerOf(roles["MINTOFDESTINY"].tokenStartID), tokenID);
    }
