...
contract QuestPoly is KeeperCompatibleInterface { // proxy implementation
    ...
    function performLeaderboard(
        ...
        require(
            ...
            && (_perfLastActionTimestamp >= lastActionTimestamp_ && _perfLastActionTimestamp < timestamp)
    ...
    function checkUpkeepView() external view returns (bool upkeepNeeded) {
...
contract TestQuestPoly is QuestPoly {
    ...
    function performLeaderboard(
        ...
        uint88 timestamp = uint88(block.timestamp);
        require(block.timestamp >= (((lastLeaderboardTimestamp / 1 hours) + 1) * 1 hours), "should wait more");
        require(
            _perfLastCombatId >= lastCombatId_
            && (_perfLastActionTimestamp >= lastActionTimestamp_ && _perfLastActionTimestamp < timestamp)
            && performId_ == _currentPerformId,
    ...
    function checkUpkeep(bytes calldata /* checkData */) external override virtual returns (bool upkeepNeeded, bytes memory /* performData */) {
        upkeepNeeded = block.timestamp >= (((lastLeaderboardTimestamp / 1 hours) + 1) * 1 hours);
        // We don't use the checkData in this example. The checkData is defined when the Upkeep was registered.
    }
}