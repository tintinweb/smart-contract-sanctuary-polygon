...
contract GensoV2 is ERC721Mintable, ERC721Permit("GensoKishiOnline.v2") , Ownable, ERC721URIStorage{
    string public _defaultURI;
    event UpdateDefaultURI(string defaultURI);

    constructor() ERC721("GensoKishiOnline.v2", "Genso_NFT_v2") {
        setDefaultURI("https://api01.genso.game/api/metadata/");
    }
    ...
    function bulkTransfer( address _from,  address  _to, uint256[] memory tokenIds) public {
            for (uint256 i; i < tokenIds.length;) {
                safeTransferFrom(_from, _to, tokenIds[i]);
                unchecked { i++; }
            }
    }
}
