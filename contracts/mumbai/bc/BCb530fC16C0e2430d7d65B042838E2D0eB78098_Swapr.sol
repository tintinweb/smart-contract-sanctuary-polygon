...
contract Marketplace is BaseGovernanceUpgradeable, Orders, Auctions {
    ...
    */
    ...
    function buyNowOrder(address lock, uint nftId, uint split) external inProgressGuard(lock, nftId) {
        ...
        Swapr.PayNow memory payOps = Swapr.PayNow(order.toEOA, false, order.acceptedToken, buyer, order.seller, swaprWallet.getBalance(buyer,order.acceptedToken), price);
    ...
    */
    ...
    function makeOffer(address lock, uint nftId, uint offer) external {
        ...
        require(swaprWallet.getBalance(bidder, auction.acceptedToken) >= offer, "ERROR: INSUFFICIENT_DEPOSITS");
    ...
    function buyNowAuction(address lock, uint nftId) external inProgressGuard(lock, nftId) {
        ...
        //MakePayment
        Swapr.PayNow memory payOps = Swapr.PayNow(auction.toEOA, false, auction.acceptedToken, buyer, auction.seller, swaprWallet.getBalance(buyer,auction.acceptedToken), auction.buyNowPrice);
        require(swaprContract.payNow(payOps), "ERROR: PAYMENT_FAILED");
...
contract Swapr is BaseGovernanceUpgradeable {
    ...
    function addFunds() public payable {
        ...
        (bool sent,) = payable(address(swaprWallet)).call{gas :10000, value: msg.value}("");
        require(sent, "ERROR: DEPOSIT_FAILED");
    ...
    function claim(address claimant, address lock, uint nftId) external onlyMarketplace returns(bool claimed, string memory res) {
        ...
        if(claimant == seller){
            if(marketplaceContract.bidExists(lock, nftId)){
                ...
                PayNow memory payOps = PayNow(toEOA, true, token, claimant, seller, swaprWallet.getLockedBalance(claimant,token), amount);
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    */
    ...
    // function addNative(address depositor, uint amount) public payable {
    //     require(msg.value > 0, "ERROR: LOW_VALUE_OBSERVED");
    //     nativeBank[depositor] += amount;
    // }
    ...
    // function addERC(address depositor, address token) public {
    //     IERC20MetadataUpgradeable fundToken = IERC20MetadataUpgradeable(token);
    //     uint allowance = fundToken.allowance(depositor, address(this));
    //     require(allowance > 0, "ERROR: ZERO_ALLOWANCE");
    //     fundToken.safeTransferFrom(depositor, address(this), allowance);
