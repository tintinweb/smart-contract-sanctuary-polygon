...
contract Marketplace is Ownable{
    ...
    function renewRental(Order memory sellOrder, address to, uint128 start, uint128 end) external payable{
        ...
        (uint256 totalPayment, uint256 platformPayment) = calculateTotalPayment((sellOrder.payment.amount * numberOfRentals) / sellOrder.numberOfTokens);
        doPaymentTransfer(totalPayment, sellOrder.maker, sellOrder.payment, platformPayment, true);
        IProperty(collectionAddress).renewRental(sellOrder.rentalId, to, start, end);
    }
    ...
    function distributeRevenue(uint pdrId, uint amountPerNFT, address[] calldata accounts, Payment memory payment) public{
        // IProperty.Rental memory r = getUserFromContractA(pdrId);
    
        require(IERC1155(collectionAddress).balanceOf(msg.sender, pdrId) > 0);       
        uint256 fee;
        address ownerRental;
        ...
        for(uint i=0; i< rentalIds.length; i++){
            ...
            ownerRental = currentRental.renter;

            if(searchIfRentalOwnerMatches(accounts, ownerRental)){
                fee = amountPerNFT * (IERC1155(collectionAddress).balanceOf(ownerRental, rentalIds[i].id));

                (uint256 totalPayment, uint256 platformPayment) = calculateTotalPayment(fee);
                doPaymentTransfer(totalPayment, msg.sender, payment, platformPayment, false);
            }  
        }
    ...
    //collectionAddress - property.sol
