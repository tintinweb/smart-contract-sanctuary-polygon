 ...
 */
contract FearGlobalExchange is AccessControl, Pausable, Withdrawable, ContextMixin, NativeMetaTransaction {

    IExchangeContract[] public exchangeContracts;
    mapping (string => IExchangeContract) private exchangeContractsByName;
    ...
    function exchange(string memory contractName, string memory positionName, uint256 amount, bytes calldata data) payable external whenNotPaused {
        IExchangeContract exchangeContract = exchangeContractsByName[contractName];
        require(address(exchangeContract) != address(0), "Exchange contract doesn't exist");
        exchangeContract.exchange{value:msg.value}(_msgSender(), positionName, amount, data);
    }

    function addTradeContract(address _address) external onlyRole(MANAGER_ROLE) {
        IExchangeContract newContract = IExchangeContract(_address);
        exchangeContracts.push(newContract);
        exchangeContractsByName[newContract.name()] = newContract;
    }
    
    function removeTradeContract(uint256 index) public onlyRole(MANAGER_ROLE) {
        string memory name = exchangeContracts[index].name();
        exchangeContracts[index] = exchangeContracts[exchangeContracts.length - 1];
        exchangeContracts.pop();

        delete exchangeContractsByName[name];
    }
}