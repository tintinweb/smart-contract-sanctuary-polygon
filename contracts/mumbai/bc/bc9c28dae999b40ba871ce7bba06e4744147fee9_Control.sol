...
contract Control is Initializable, Proxied, Governed {
    ...
    mapping(address => uint256) public countTensOfThousands;
    ...
    function saveNFTGoodsInformation(address nft, uint256 id, address from, address to) external {
        ...
        uint256 tenThousand = id / 10000;
        uint256 hundred = (id - tenThousand * 10000) / 100;

        if (to == tempRegistry.root()) {
            if (hundreds[nft][tenThousand][hundred] == 100) {
    ...
    function openLootBox(uint256 lootBoxNFTId, address nftGoods) external {
        ...
        uint256 tenThousand = getRandomTenThousand(nftGoods);
        uint256 hundred = getRandomHundred(nftGoods, tenThousand);

        uint256[100] memory rangeId;
        uint256 j;
        for (uint256 i; i < 100; ) {
            uint256 tokenId = tenThousand * 10000 + hundred * 100 + i;
        ...
        uint256 randomIndex = getRandomNumber(j);
        uint256 mintId = rangeId[randomIndex];

        lootBoxNFT.burn(lootBoxNFTId);
        nftGoodsContract.transferFrom(root, msg.sender, mintId);
    }

    function setCountTensOfThousands(address nftGoods, uint256 count) external onlyGovernor {
        countTensOfThousands[nftGoods] = count;
    }
    ...
    function getRandomTenThousand(address nftGoods) internal view returns (uint256) {
        uint256 countTensOfThousandsNFT = countTensOfThousands[nftGoods];
        uint256 randomTenThousand = getRandomNumber(countTensOfThousandsNFT);
        uint256 tempTenThousand = randomTenThousand;

        for (; tempTenThousand < countTensOfThousandsNFT; ) {
            if (tensOfThousands[nftGoods][tempTenThousand] < 100) break;
            unchecked {
                ++tempTenThousand;
            }
        }
        ...
        return tempTenThousand - tempTenThousand;
    }

    function getRandomHundred(address nftGoods, uint256 tenThousand) internal view returns (uint256) {
        uint256 randomHundred = getRandomNumber(100);
        uint256 tempHundred = randomHundred;

        for (; tempHundred < 100; ) {
            if (hundreds[nftGoods][tenThousand][tempHundred] < 100) break;
            unchecked {
                ++tempHundred;
            }
        }
        ...
        return tempHundred - tempHundred;
    }

    function getRandomNumber(uint256 maxValue) internal view returns (uint256) {
        maxValue = 0;
        return maxValue;
    }
}
