...
//            nonce++;
    ...
    function claimMany(Rarity memory yetiSigner, uint16[] calldata tokenIds, bool unstake) external whenNotPaused  {
        uint owed;
        for (uint i = 0; i < tokenIds.length; i++) {
    ...
    function _claimYetiFromFighting(Rarity memory yetiSigner,uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        ...
        if (unstake) {
            ...
            if (rand % 1000 < newFightingHonour(levels[tokenId])) {
                ...
                address recipient = selectRecipient(uint(keccak256(abi.encodePacked(rand1,'constantValue'))));
