...
contract NameRegistry_Covest is Initializable, PausableUpgradeable, UUPSUpgradeable {
    ...
    event ContractChanged(address caller,string contractName,address contractAddress);
    event InitContract();
     ...
     *  @dev `setupCount == 0` => [Factory, policyManager, policyDetail,Referral, UnderWriter, ClaimAssessors, ProductDesign, ComplianceProvider, PlatformFee, Reserve, RiskManager, Subsidized, VerifyAddress] ||
    ...
    function setup(address[] memory _contract) public isSuperManager {
        ...
        if(setupCount == 0) {
            ...
            contracts["CP"] = _contract[7];
            contracts["PF"] = _contract[8];
            contracts["R"] = _contract[9];
    ...
    function _init() internal {
        ...
        INameRegistry.InitStructerParams memory initData = INameRegistry.InitStructerParams(
            contracts["F"],
            contracts["RF"],
            contracts["UW"],
            contracts["CA"],
        ...
        for(uint256 i = 0; i < 12; i++) {
            if(contracts[initCaller[i]] == address(0)) {
                revert(string(abi.encodePacked("Not found contract address: ",initCaller[i])));
            }
     ...
     *   - `msg.sender` Must be a SuperManager.
     */
    function setContract(string memory _contractName,address _addr) public isSuperManager {
     ...
     */
    function getContract(string memory _contractName) public view returns (address) {
