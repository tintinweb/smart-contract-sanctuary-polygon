...
contract EtheremonMonsterTokenBasic is
    IERC721,
    ERC721,
    ERC165,
    BasicAccessControl
{
    ...
    function _canOperate(address _tokenOwner) internal view {
        require(
            _tokenOwner == msgSender() ||
                ownerToOperators[_tokenOwner][msgSender()]
        );
    }

    function _canTransfer(uint256 _tokenId, address _tokenOwner) internal view {
        EtheremonTradeInterface trade = EtheremonTradeInterface(tradeContract);
        require(!trade.isOnTrading(uint64(_tokenId)));
        require(_tokenOwner != address(0));
        require(
            _tokenOwner == msgSender() ||
                idToApprovals[_tokenId] == msgSender() ||
                ownerToOperators[_tokenOwner][msgSender()]
        );
    }
    ...
    function isApprovable(address _owner, uint256 _tokenId)
        public
        view
        returns (bool)
    {
        ...
        if (obj.monsterId != uint64(_tokenId)) return false;
        if (obj.trainer != _owner) return false;
        // check battle & trade contract
        EtheremonTradeInterface trade = EtheremonTradeInterface(tradeContract);
        return (!trade.isOnTrading(obj.monsterId));
    }
    ...
    function approve(address _approved, uint256 _tokenId) external override {
        ...
        EtheremonTradeInterface trade = EtheremonTradeInterface(tradeContract);
        if (trade.isOnTrading(obj.monsterId)) revert();
...
abstract contract EtheremonMonsterEnumerable is
    EtheremonMonsterTokenBasic,
    IERC721Enumerable
{
    constructor() public {
...
abstract contract EtheremonMonsterStandard is
    EtheremonMonsterEnumerable,
    IERC721Metadata
{
...
contract EtheremonMonsterToken is
    EtheremonMonsterStandard("EtheremonMonster", "EMONA"),
    NativeMetaTransaction
{
    ...
    uint256 public gapFactor = 0.001 ether;
    ...
    function setFactor(uint256 _gapFactor) public onlyModerators {
    ...
    function setAddressWhitelist(address _smartcontract, bool _status)
        external
        onlyModerators
    {
    ...
    function clearApproval(uint256 _tokenId) external onlyModerators {
        _clearApproval(_tokenId);
    }
    ...
    function getMonsterCP(uint64 _monsterId)
        external
        view
        returns (uint256 cp)
    {
        ...
        returns (bool catchable, uint256 price)
    {
        ...
        if (class.catchable == false) {
            if (
                addressWhitelist[msgSender()] == true &&
                classWhitelist[_classId] == true
            ) {
                return (true, price);
            }
        }
    ...
    function catchMonster(
        ...
        string calldata _name,
        uint256 amount
    ) external isActive returns (uint256 tokenId) {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterClassAcc memory class;
        (
            class.classId,
        ...
        if (class.catchable == false) {
            if (
                addressWhitelist[msgSender()] == false ||
                classWhitelist[_classId] == false
            ) {
                revert();
            }
        }
...
contract EthermonBulkTransfer is BasicAccessControl {
    ...
    function bulkTransfer(address[] calldata _to, uint256[] calldata _tokenId)
        ...
        onlyModerators
    {
        EtheremonMonsterToken monster = EtheremonMonsterToken(
