...
contract SSC is ERC721Enumerable, Ownable {
  ...
  // mint function -- buffer modifier to observe current stage before minting -- allowing only whitelist to participate during 'presale'.
  function mint(uint256 _mintAmount) public payable buffer(msg.sender){
    ...
    modifier buffer(address _msgSender) {
      require(openingTime != 0);
      require(checkStage() != Stage.locked);
      require((checkStage() == Stage.publicsale || iswhitelisted(_msgSender)));
      _;
  }
...
//set time here [unix time stamp] once time is set, before the time means sales are locked. in between the times is presale, and afterwards is public
...
// input whitelist addresses like this without brackets [addr, addr, addr]
    function addManyToWhitelist(address[] memory _beneficiaries) public onlyOwner {
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
        whitelist[_beneficiaries[i]] = true;
    }
  }
//this function returns a boolean. this is checked during presale stage for whitelist wallets
