...
contract FactoryFacet is Modifiers {
    ...
    function initialize()external {
        if(initialized != true){
            ...
            BigBoardManagerLib.setFactoryHostToken(0x523F4e35105d6F8082DD7da889A415B5a7cb3497);
            initialized = true;
        } else{
            revert("Done");
        }
    ...
    function getGameBoardzCount(string memory gameId) external view returns(uint256){
        return BigBoardManagerLib.getGameBoardzCount(gameId);
    }
    ...
    function getGameBoard(string memory gameId, uint256 index) external view returns(address){
        return BigBoardManagerLib.getGameBoard(gameId, index);
    }

    function isAirdropEnabled() external view returns(bool){
        return BigBoardManagerLib.isAirdopEnabled();
    }
    ...
    function disableAirdrop() external onlyFounder {
        BigBoardManagerLib.disableFactoryAirdrop();
    }
}
   ...
   event ScoreUpdated(string gameId);
    ...
    function initializeGameData() external {
        if(initialized != true){
            ...
            c.oracle = 0x3E62b566165654386e36dE7590F662D6419362Ac;
...
library BigBoardManagerLib {
    ...
    struct Factory {
        ...
        bool airdropEnabled;
        mapping (string => address[]) gameBoardz;
    }
    ...
    function getGameBoardzCount(string memory gameId) internal view returns(uint256){
        Factory storage f = getFactory();
        return f.gameBoardz[gameId].length;
    }

    function getGameBoard(string memory gameId, uint256 index) internal view returns(address){
        Factory storage f = getFactory();
        return f.gameBoardz[gameId][index];
    }
    ...
    function createBoard(string memory gameId, uint256 boardFee, address feeToken, address airdDropToken, uint256 airDropamt) internal returns (address) {
        ...
        f.gameBoardz[gameId].push(address(f.board));
