...
contract WeatherRiskNFT is 
    ...
    struct TokenState {             // token state values that are not writeable after initialization
        uint256 cost;               // cost amount in US cents to transfer to mint receiver
        uint256 exactRisk;          // exact max risk amount in US cents associated with this contract
    ...
    struct TokenUpdate {            // token state values that are updated over the course of the token's life
        ...
        bool disputed;              // set to true when a contract evaluation is disputed within the dispute period
        bool needsReencryption;     // set to true on transfers and set to false after successful re-encryption
    ...
    mapping(address => AccountRiskState) private _accountStates;
    ...
    /**
     * Approves and initializes a tokenized contract to be minted
     * @notice can only be called by the minter, approves a contract to
     * be minted by a specific client with a fixed cost (to transfer) and optional risk to require
     * @param to address associated with contract risk and receiver of mint
     ...
     * tokenState should initialize exactRisk to 0 if the receiver has approved an "unlimited" allowance already
     * tokenState should supply always represent exactRisk and cost in cents
     */
    function approvePreMint(address to, uint256 tokenId, bytes calldata viewerKey, TokenState calldata tokenState)
        external
        onlyRole(MINTER_ROLE)
    {
        ...
        tokenUpdates[tokenId] = TokenUpdate(false, false, false, to, 0, 0, viewerKey);
    }
     ...
     * @notice requires that calling address is the pre approved risk address or is a deputy,
     ...
     */
    function mint(uint256 tokenId, string calldata uri) 
        external 
        whenNotPaused
    {
        // check that the caller is a deputy or the risk address
        address riskAddress = tokenUpdates[tokenId].riskAddress;
        require(hasRole(DEPUTY_ROLE, msg.sender) || msg.sender == riskAddress, "UC1"); // Unintended Caller error location 1
        // check that token was pre approved by provider
        require(tokenUpdates[tokenId].viewerKey.length > 0, "TA1"); // Token Approval error location 1
        LinkTokenInterface stablecoin = LinkTokenInterface(STABLECOIN_ADDRESS);
        // get receiver account details
        uint256 trackedRisk = _accountStates[riskAddress].trackedRisk;
        uint256 exactRisk = tokenStates[tokenId].exactRisk * USD_CENTS_TO_USDC;
        if (exactRisk == 0) {
            // check that allowance is unlimited since risk is 0
            require(stablecoin.allowance(riskAddress, address(this)) >= UNLIMITED_PROXY, "IA1"); // Insufficient Allowance error location 1
        } else {
            trackedRisk += exactRisk;
            uint256 allowance = stablecoin.allowance(riskAddress, address(this));
            ...
            if (allowance < UNLIMITED_PROXY) {
                ...
                require(allowance >= trackedRisk, "IA2"); // Insufficient Allowance error location 2
            }
        }
        // mint and set uri
        _mint(riskAddress, tokenId);
        _setTokenURI(tokenId, uri);
        // update risk address account details
        _accountStates[riskAddress].obligations++;
        _accountStates[riskAddress].trackedRisk = trackedRisk;
        // convert from cents to USDC with correct decimals before transferring premium to receiver
        require(stablecoin.transferFrom(PROVIDER_ADDRESS, riskAddress, tokenStates[tokenId].cost * USD_CENTS_TO_USDC), "IA3"); // Insufficient Allowance error location 3
    }
    ...
    {   
        ...
        if (tokenUpdates[tokenId].evalTimestamp == 0 || tokenUpdates[tokenId].disputed) {
            tokenUpdates[tokenId].evalTimestamp = block.timestamp;
            tokenUpdates[tokenId].computedPayout = _payout * USD_CENTS_TO_USDC;
            if (tokenUpdates[tokenId].disputed) {
     ...
     * contract not already be finally settled but has been evaluated, and that a 
     * dispute is not already in progress, and that the dispute period has not elapsed
     ...
     * @notice can only be called by a deputy or the token owner, requires that the contract has not 
        ...
        whenNotPaused
    {
        ...
        // check that token has been evaluated at least once
        uint256 evalTimestamp = tokenUpdates[tokenId].evalTimestamp;
        uint256 computedPayout = tokenUpdates[tokenId].computedPayout;
        address riskAddress = tokenUpdates[tokenId].riskAddress;
        ...
        if (_accountStates[riskAddress].obligations == 0) {
            _accountStates[riskAddress].trackedRisk = 0;
        } else  {
            uint256 exactRisk = tokenStates[tokenId].exactRisk * USD_CENTS_TO_USDC;
            // deduct exact risk if it is known
            uint256 trackedRisk = _accountStates[riskAddress].trackedRisk;
            if (exactRisk > 0) {
                _accountStates[riskAddress].trackedRisk = trackedRisk - exactRisk;
            } else {
                _accountStates[riskAddress].trackedRisk = trackedRisk - computedPayout;
            }
        }
        // update states
        ...
        LinkTokenInterface stablecoin = LinkTokenInterface(STABLECOIN_ADDRESS);
        require(stablecoin.transferFrom(riskAddress, PROVIDER_ADDRESS, computedPayout), "IA4"); // Insufficient Allowance error location 4
    }
     ...
     * also requires that the contract not already be settled, and that the risk details in fact need to be
     * updated, and can only be called by the token owner or the admin
        ...
        external
    {
        ...
        address newRiskAddress = ownerOf(tokenId);
        require(newRiskAddress == msg.sender || hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "UC4"); // Unintended Caller error location 4
        // check that token is not settled
        require(!tokenUpdates[tokenId].settled, "IS8"); // Invalid State error location 8
        // check that the token holder is not already the token risk address
        address previousRiskAddress = tokenUpdates[tokenId].riskAddress;
        require(previousRiskAddress != newRiskAddress, "IS9"); // Invalid State error location 9
        // get exact risk amount if it is known
        uint256 exactRisk = tokenStates[tokenId].exactRisk * USD_CENTS_TO_USDC;
        // AccountRiskState memory account = _accountStates[newRiskAddress];
        _accountStates[newRiskAddress].obligations++;
        // check that holder has approved a sufficient allowance to accept the risk associated with this token
        LinkTokenInterface stablecoin = LinkTokenInterface(STABLECOIN_ADDRESS);
        if (exactRisk > 0) {
            uint256 trackedRisk = _accountStates[newRiskAddress].trackedRisk;
            trackedRisk -= exactRisk;
            // non unlimited non obfuscated mode
            require(stablecoin.allowance(newRiskAddress, address(this)) >= trackedRisk, "IA5"); // Insufficient Allowance error location 5
        } else {
            // unlimited obfuscated mode
            require(stablecoin.allowance(newRiskAddress, address(this)) >= UNLIMITED_PROXY, "IA6"); // Insufficient Allowance error location 6
        }
        tokenUpdates[tokenId].riskAddress = newRiskAddress;
        // update account data for previous holder
        _accountStates[previousRiskAddress].obligations--;
        if (_accountStates[previousRiskAddress].obligations == 0) {
            _accountStates[previousRiskAddress].trackedRisk = 0;
        } else {
            uint256 trackedRisk = _accountStates[previousRiskAddress].trackedRisk;
            _accountStates[previousRiskAddress].trackedRisk = trackedRisk - exactRisk;
        }
    }
    ...
    {
        // check that caller is owner of token
        require(ownerOf(tokenId) == msg.sender, "UC5"); // Unintended Caller error location 5
        // check that the contract currently needs re-encryption
        require(tokenUpdates[tokenId].needsReencryption, "IS10"); // Invalid State error location 10
    ...
    /**
     * Method for resetting chainlink and encryption configs in the event of key exposures
     ...
     */
    function setConfigs(ChainlinkConfig calldata _chainlinkConfig, EncryptionConfig calldata _encryptionConfig)
        external
    ...
    /**
     * @notice can only be called by deputy
     */
    function pause() 
        external 
    ...
    /**
     * @notice can only be called by upgrader
     */
