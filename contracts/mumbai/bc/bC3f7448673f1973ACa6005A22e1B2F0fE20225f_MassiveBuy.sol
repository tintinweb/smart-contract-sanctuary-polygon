...
contract MassiveBuy is Ownable {
    IWosMarketplace private wosMarketplace;
    ...
    constructor(
        IWosMarketplace _wosMarketplace,
        IERC20 _collateral,
    ...
    function buy(BuyWell[] memory wells) external {
        for (uint i = 0; i < wells.length; i++) {
            ...
            uint256 price = mkRates.getOilWellSalePrice(well.bars);
            collateral.transferFrom(msg.sender, address(this), price);
            collateral.approve(address(wosMarketplace), price);
            wosMarketplace.BuyOilWellWithOwner(
                msg.sender,
                well.bars,
                well.nonce,
                well.intent,
    ...
    function setMarketplace(address mkplace) external onlyOwner {
        require(mkplace != address(0), "address cannot be 0");
        wosMarketplace = IWosMarketplace(mkplace);
    }
}