...
// File: opengsn/contracts/forwarder/IForwarder.sol
...
// File: opengsn/contracts/forwarder/Forwarder.sol
...
// File: opengsn/contracts/DiviForwarder.sol
...
contract DiviForwarder is Forwarder {

  address public swapRouter;

  function setSwapRouter(address _address) public {
    swapRouter = _address;
  }

  uint256 public amountToPay;

  function setAmountToPay(uint256 _amountToPay) public {
  ...
  function approve(address token, address cntr) public {
    IERC20(token).approve(cntr, type(uint256).max);
  }

  function setup() public  {
    this.registerDomainSeparator("GSN Relayed Transaction", "3");
    this.registerRequestType("ForwardRequest", ")");
    this.registerRequestType("RelayRequest", "RelayData relayData)RelayData(uint256 maxFeePerGas,uint256 maxPriorityFeePerGas,uint256 transactionCalldataGasUsed,address relayWorker,address paymaster,address forwarder,bytes paymasterData,uint256 clientId)");
  }

  address public constant DiViToken = 0x363A8d1D02b9F4e5084284BAC8857C7C510bf702;

  event FREvent(uint256 item, uint256 idex, ForwardRequest req);
  ...
  returns (bool success, bytes memory ret) {
    ...
    emit FREvent(3, 3, req);

    // Transfer the specified amount of DAI to this contract.
    // TransferHelper.safeTransferFrom(targetToken, walletAddress, address(this), amountToPay);
    // Approve the router to spend DAI.
    // TransferHelper.safeApprove(targetToken, swapRouter, amountToPay);


    // string memory _domainSeparatorName;
    // uint256 _maxAcceptanceBudget;
    // GsnTypes.RelayData memory  _relayRequest;
    // (_domainSeparatorName, _maxAcceptanceBudget, _relayRequest) = abi.decode(req.data, (string, uint256, GsnTypes.RelayData));

    // ExactInputSingleParams memory _exactInputSingleParams = abi.decode(_relayRequest.paymasterData, ExactInputSingleParams);

    // // Transfer the specified amount to this contract.
    // TransferHelper.safeTransferFrom(_exactInputSingleParams.tokenIn, _exactInputSingleParams.recipient, address(this), req.value);

    // // Approve the router to spend
    // TransferHelper.safeApprove(_exactInputSingleParams.tokenIn, req.to, req.value);

     return super.execute(req, domainSeparator, requestTypeHash, suffixData, sig);
  }
