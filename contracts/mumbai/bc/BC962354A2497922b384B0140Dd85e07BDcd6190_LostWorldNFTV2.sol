    ...
    UUPSUpgradeable
{
    ...
    function setLCNFT(address lostchainNFTAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostchainNFTAddress != address(0x0), 'LWNFT: Lost chain NFT is zero address');
        LCNFT = ILostChainNFT(lostchainNFTAddress);
    }
    ...
    function editLand(
        ...
        require(rewardList.length == rewardAmount.length, 'LWNFT: Array lengths mismatch');

        for (uint256 i = 0; i < enemyList.length; i++) {
            require(LWD.checkEnemyAvailable(enemyList[i]), 'LWNFT: Selected enemy is disabled');
        }
        for (uint256 i = 0; i < rewardList.length; i++) {
            require(LCNFT.checkNFTRewardable(rewardList[i]), 'LWNFT: Selected NFT is not rewardable');
        }
    ...
    function clearLand(uint256 landId) public onlyRole('TRUSTED_FORWARDER', msg.sender) {
        ...
        landData[landId].rewardAmount = new uint256[](0);
    ...
    function TESTING53() external pure returns (bool) {
        return true;
    }
}
