...
contract ROIContract {
    ...
    function isCapitalWithdrawalRequestValid(uint256 requestId) public view returns (bool) {
        CapitalWithdrawalRequest memory request = getCapitalWithdrawalRequestById(requestId);

        // Check if the request exists (not an empty struct) and is approved and the amount is greater than zero
        if (request.id != 0 && request.approved && request.amount > 0) {
            return true;
        } else {
            return false;
        }
   ...
   /// 
    function approveCapitalWithdrawal(uint256 requestId) external onlyAdmin {
        ///
        require(isCapitalWithdrawalRequestValid(requestId) == true,"Invalid request ID");
        CapitalWithdrawalRequest memory request = getCapitalWithdrawalRequestById(requestId);
        ...
        Withdrawal memory withdrawObj = Withdrawal({ amount: request.amount, timestamp: block.timestamp });
        user.withdrawals.push(withdrawObj);
        // update deposits
        for (uint256 i = 0; i < user.deposits.length; i++) 
        {
            Deposit storage deposit = user.deposits[i];
        ...
        payable(request.user).transfer(request.amount - projectFee - devFee);
    }
    ...
    function blockUser(address _user) external onlyAdmin {
        require(_user != developerAddr, "Cannot block developer");
