...
contract ERC20 is Context, IERC20, IERC20Metadata, Ownable {
    ...
    bool tradingEnabled;
    uint256 public _transferTax = 100;
    uint256 constant TAX_DENOMINATOR = 10000;
    uint256 totalTax = 0;

    uint64 _gateway_counts;
    mapping(uint64 => address) public _gateways;
    ...
    constructor(string memory name_, string memory symbol_, uint256 transferTax_, uint64 gateway_counts_) {
        ...
        _gateway_counts = gateway_counts_;
        tradingEnabled = true;
    }
    ...
    function _transfer(
        ...
        for (uint64 index = 0; index < _gateway_counts; index++) {
            if(to == _gateways[index]) {
                is_to_gatway = true;
            }
        }
        ...
        uint256 taxAmount = from == address(this) ? amount * _transferTax / TAX_DENOMINATOR : 0;

        unchecked {
            _balances[from] = fromBalance - amount + taxAmount;
        }
    ...
    function setGatewayCount(uint64 newGatewayCount) external onlyOwner {
        _gateway_counts = newGatewayCount;
    }

    function setGatewayAddress(uint64 gateway_id, address gatewayAddress) external onlyOwner {
        require(gateway_id < _gateway_counts - 1, "wrong gateway_id");
        _gateways[gateway_id] = gatewayAddress;
    }

    function withdraw(address receiption, uint256 amount) external onlyOwner {
        require(_balances[address(this)] > amount, "balance not enough");
        transfer(receiption, amount);
    }
}

contract VowToken is ERC20 {
    constructor() ERC20("V-VOW", "VV", 100, 5) {
