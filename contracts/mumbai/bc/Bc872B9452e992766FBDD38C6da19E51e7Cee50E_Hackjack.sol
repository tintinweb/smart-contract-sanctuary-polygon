...
library Queue {
    ...
    function isMetaEmpty(MetaRequestQueue storage queue) internal view returns (bool) {
        return Metalength(queue) == 0;
    }

    function Metalength(MetaRequestQueue storage queue) internal view returns (uint256) {
        if (queue._last < queue._first) {
            return 0;
        }
        return queue._last - queue._first + 1;
    ...
    function Metaenqueue(MetaRequestQueue storage queue, uint256[] memory data) internal {
        queue._data[++queue._last] = data;
    }

    function Metadequeue(MetaRequestQueue storage queue) internal isMetaNotEmpty(queue) returns (uint256[] memory data) {
        data = queue._data[queue._first];
        delete queue._data[queue._first++];
    } 

    function Metapeek(MetaRequestQueue storage queue) internal view returns (uint256[] memory data) {
        return queue._data[queue._first];
    }

    function Metapeeklast(MetaRequestQueue storage queue) internal view isMetaNotEmpty(queue) returns (uint256[] memory data) {
        return queue._data[queue._last];
    }
...
contract RequestQueue {
    ...
    event MetaOperationResult(uint256[] data);
    ...
    function metaLength() external view returns (uint256) {
        return _Metaqueue.Metalength();
    }

    function isMetaEmpty() external view returns (bool) {
        return _Metaqueue.isMetaEmpty();
    }

    function metaEnqueue(uint256[] memory data) external {
        _Metaqueue.Metaenqueue(data);
    }

    function metaDequeue() external returns (uint256[] memory data) {
        data = _Metaqueue.Metadequeue();
        emit MetaOperationResult(data);
    }

    function metaPeek() external view returns (uint256[] memory data) {
        return _Metaqueue.Metapeek();
    }

    function metaPeekLast() external view returns (uint256[] memory data) {
        return _Metaqueue.Metapeeklast();
    }
