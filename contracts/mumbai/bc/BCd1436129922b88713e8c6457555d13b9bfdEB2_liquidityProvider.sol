 ...
 */
    function removeLiquidity(address lp, uint amount) public returns(uint amountA, uint amountB){
        IUniswapV2Pair(lp).transferFrom(msg.sender,address(this),amount);
        IUniswapV2Pair(lp).approve(router, amount);
    ...
    function depositBUSDToLP(address user,uint amount,address lp) public onlyRole(BUSDTOLPCALL) returns(uint){
        ...
        IERC20(path[0]).approve(router, IERC20(path[0]).balanceOf(address(this)));
        IERC20(path[1]).approve(router, IERC20(path[1]).balanceOf(address(this)));
        IUniswapV2Router02(router).addLiquidity(
            path[0],
            path[1],
            ...
            IERC20(Admin.BUSD()).approve(router,IERC20(Admin.BUSD()).balanceOf(address(this)));
        ...
        IERC20(lp).transfer(Admin.Treasury(), IERC20(lp).balanceOf(address(this)));
