...
contract CoresVerse is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    mapping (address => bool) public _isNoLimit;
    ...
    constructor(
        ...
        address[2] memory addrs, //router, marketing
        ...
        require(buyMarketingFee.add(buyDeadFee) <= 5, "Total buy fee is over 5%");
        require(sellMarketingFee.add(sellDeadFee) <= 5, "Total sell fee is over 5%");

        uint256 totalSupply = totalSupply_.mul(10 ** 18);
        swapTokensAtAmount = totalSupply.mul(5).div(100000); // 0.005% define as 1000 = 1%
    ...
    function whitelistLimit(address account, bool value) external onlyOwner {
        _isNoLimit[account] = value;
    }
    ...
    function recoverCORE() public onlyOwner {
        uint256 amount = address(this).balance;
        require(amount > 0, "No CORE available");       
        payable(msg.sender).transfer(amount);
    }

    function recoverERC20(address tokenAddress, uint256 tokenAmount) public onlyOwner {
        require(tokenAddress != address(this), "not allowing recovering self token");
        IERC20(tokenAddress).transfer(msg.sender, tokenAmount);
    }   
    // ** end Only owner function
    ...
    function _transfer(
        ...
        if(from != owner() && to != owner() && !_isNoLimit[from] && !_isNoLimit[to]){
            require(amount <= _maxTxAmount, "Transfer amount exceeds the maxTxAmount.");
        }
        if(from != owner() && to != owner() && to != uniswapV2Pair && to != deadWallet && !_isNoLimit[to]){
