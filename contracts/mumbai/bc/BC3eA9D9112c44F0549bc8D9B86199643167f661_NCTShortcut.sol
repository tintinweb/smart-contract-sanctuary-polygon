...
contract NCTShortcut is IShortcut{
    ...
    uint public constant SENKEN_FEE = 3;
    ...
    returns(uint){
        if(_checkTokenTCO2(_toToken)){
            ...
            (uint forSenken, uint forTrade) = _calculateSenkenFees(_amIn);
            /// todo send to Senken
            /// idea send remainder in contract at end of both
            ...
            // buy NCT
            _buyNCT(_fromToken, _toToken, forTrade, _amOut);
            // exchange the NCT tokens for TCO2s and send to user
            return _exchangeNCTForTCO2(_fromToken, _toToken, forTrade, _amOut);
            // emit complete event
        }else{            
            if(_checkTokenTCO2(_fromToken)){
                ...
                _exchangeTCO2ForOutToken(_fromToken, _toToken, _amIn, _amOut);
                return _amOut;
                }
            }
    ...
    * @dev function to check if a sell shortcut will be valid, only returns true if
    ...
    function _checkSellShortcutValid(        
        ...
        try nct.checkEligible(_fromToken) returns(bool eligible){
            ...
            (,uint amountBack) = _calculateSenkenFees(amounts[0]);
            return(amountBack >= _amOut);
    ...
    * @dev function to check if a buy shortcut will be valid, only returns true if
    ...
    function _checkBuyShortcutValid(        
        ...
        (,uint amIn) = _calculateSenkenFees(_amIn);
    ...
    function _exchangeTCO2ForOutToken(
        ...
        IERC20(_fromToken).safeTransferFrom(msg.sender, address(this), _amIn);
        ...
        try dex.swapExactTokensForTokens(nctAfter-nctBefore, _amOut, path, address(this), block.timestamp) returns(uint[] memory amounts){
            require(amounts[1] >= _amOut, "Shortcut Error: Order failed to retreive TCO2s");
            return amounts[1];
        }catch (bytes memory _err){
    ...
    * @dev function to buy an Order with NCT
    ...
    function _calculateSenkenFees(uint amount) internal pure returns(uint forSenken, uint forUser){
        forSenken = amount * SENKEN_FEE / ONE_HUNDRED_PERCENT;
        forUser = amount - forSenken;
    }
