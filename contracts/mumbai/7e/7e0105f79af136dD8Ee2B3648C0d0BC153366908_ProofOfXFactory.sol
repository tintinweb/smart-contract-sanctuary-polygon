...
import "./ProofOfX.sol";
import './INgageN.sol';
import './INgageNFactory.sol';

contract ProofOfXFactory is AccessControl {
    bytes32 public constant CONTRACT_CREATOR = keccak256("CONTRACT_CREATOR");
    address public ngageNFactory;
    address public upgradeAdmin;

    mapping(string => address) public proofOfXContracts;

    event FactoryContracts(
        address _owner, 
        string _factoryName,
        address _ngageN,
        address _proofOfX,
     ...
     */
    constructor(address _ngageNFactory) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(CONTRACT_CREATOR, _msgSender());
        ngageNFactory = _ngageNFactory;
    }
    ...
    function createContracts(
        ...
        require(hasRole(CONTRACT_CREATOR, _msgSender()), "PoXF: Only the contract creator can create a factory");
        require(
            INgageNFactory(ngageNFactory).factoryCreated(_factoryName) == _creator, 
            "PoXF: Creator cannot be different for proxy and proofOfX"
        );
        address proxy = INgageNFactory(ngageNFactory).ngageNContracts(_factoryName);
        require(proxy != address(0), "PoXF: Proxy cannot be null.");

        CustomForwarder forwarder = new CustomForwarder();

        ProofOfX proofOfX = new ProofOfX(INgageN(proxy), forwarder);

        proofOfX.grantRole(DEFAULT_ADMIN_ROLE, _creator);
        forwarder.grantRole(DEFAULT_ADMIN_ROLE, _creator);

        // CustomForwarder(address(forwarder)).grantRole(CustomForwarder(address(forwarder)).RELAYER(), _relayer);

        proofOfXContracts[_factoryName] = address(proofOfX);
        emit FactoryContracts(_creator, _factoryName, address(proxy), address(proofOfX), address(forwarder));
    }

    function setNgageNFactory(address _ngageNFactory) public onlyAdmin {
        ngageNFactory = _ngageNFactory;
    }
}
...
interface INgageNFactory is IAccessControl {

    function CONTRACT_CREATOR() external pure returns (bytes32);
    function ngageN() external pure returns (address);
    function upgradeAdmin() external pure returns (address);
    function factoryCreated(string memory _factoryName) external pure returns (address);
    function ngageNContracts(string memory _factoryName) external pure returns (address);
