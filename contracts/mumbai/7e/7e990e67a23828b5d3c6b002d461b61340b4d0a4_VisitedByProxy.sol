...
import "../src/VisitedBy.sol";
import "../src/SpotRegistry.sol";
import "../src/SpotVerifier.sol";
import "../src/SpotResolver.sol";
import "../src/lib/IPoVFactory.sol";
import "../src/lib/PoV5192.sol";
import "../src/pov/PoV5192MetadataFactory.sol";
import "../src/lib/VisitedByTypes.sol";

// =================================================================================================
// Proxy contracts for deployment
// =================================================================================================

contract SpotRegistryProxy is ERC1967Proxy {
    // solhint-disable-next-line no-empty-blocks
    constructor(address _logic, bytes memory _data) ERC1967Proxy(_logic, _data) {}
}

contract SpotVerifierProxy is ERC1967Proxy {
    // solhint-disable-next-line no-empty-blocks
    constructor(address _logic, bytes memory _data) ERC1967Proxy(_logic, _data) {}
}

contract VisitedByProxy is ERC1967Proxy {
    // solhint-disable-next-line no-empty-blocks
    constructor(address _logic, bytes memory _data) ERC1967Proxy(_logic, _data) {}
}

contract PoVFactoryProxy is ERC1967Proxy {
    // solhint-disable-next-line no-empty-blocks
    constructor(address _logic, bytes memory _data) ERC1967Proxy(_logic, _data) {}
}

contract BytesUtil {
    function _splitStringAsBytes32(string memory _str) internal pure returns (bytes32) {
        bytes memory _bytes = bytes(_str);
        uint256 _length = _bytes.length;
        bytes32 _bytes32;
        for (uint256 i = 0; i < 32; i++) {
            if (i < _length) {
                _bytes32 |= bytes32(_bytes[i]) >> (i * 8);
            }
        }
        return _bytes32;
    }

    function _splitStringAsBytes32Array2(string memory _str) internal pure returns (bytes32[2] memory) {
        ...
        for (uint256 i = 0; i < 2; i++) {
            ...
            for (uint256 j = 0; j < 32; j++) {
                if (_index < _length) {
                    _bytes32 |= bytes32(_bytes[_index]) >> (j * 8);
                    _index++;
                }
            }
            result[i] = _bytes32;
    ...
    function _splitStringAsBytes32Array4(string memory _str) internal pure returns (bytes32[4] memory) {
...
/**
 * @dev This is a deploy script for VisitedBy.
 * It deploys SpotRegistry, SpotVerifier, and VisitedBy.
 * It also initializes SpotRegistry and SpotVerifier.
 * @notice SpotRegistry and SpotVerifier, VisitedBy are deployed as proxies.
 * So, user should call proxy address as a contract address.
 */
contract VisitedByDeployScript is Script {
    string CONTRACT_ENV_FILE = ".env.contracts";

    function _replaceEnv(string memory key, string memory val) public {
        string[] memory script = new string[](5);
        script[0] = "sed";
        script[1] = "-i";
        script[2] = "-e";
        script[3] = string.concat("s/^export ", key, "=.*$/export ", key, "=", val, "/");
        script[4] = CONTRACT_ENV_FILE;
        vm.ffi(script);
    }
    ...
    function run() external {
        ...
        /// 1. Deploy registry and proxy
        SpotRegistry _spotRegistry = new SpotRegistry();
        SpotRegistryProxy _spotRegistryProxy = new SpotRegistryProxy(address(_spotRegistry), "");
        SpotRegistry spotRegistry = SpotRegistry(address(_spotRegistryProxy));
        _replaceEnv("REGISTRY_CONTRACT", vm.toString(address(spotRegistry)));

        /// 2. Deploy verifier and proxy
        SpotVerifier _spotVerifier = new SpotVerifier();
        SpotVerifierProxy _spotVerifierProxy = new SpotVerifierProxy(address(_spotVerifier), "");
        SpotVerifier spotVerifier = SpotVerifier(address(_spotVerifierProxy));
        _replaceEnv("VERIFIER_CONTRACT", vm.toString(address(spotVerifier)));

        /// 3. Deploy VisitedBy and proxy. set registry and verifier for VisitedBy.
        VisitedBy _visitedBy = new VisitedBy();
        VisitedByProxy _visitedByProxy = new VisitedByProxy(address(_visitedBy), abi.encodeWithSignature("initialize(address,address)", spotRegistry, spotVerifier));
        VisitedBy visitedBy = VisitedBy(address(_visitedByProxy));
        _replaceEnv("VISITEDBY_CONTRACT", vm.toString(address(visitedBy)));

        /// 4. Deploy PoVFactory and proxy.
        PoV5192MetadataFactory povFactory = new PoV5192MetadataFactory();
        _replaceEnv("POV_FACTORY_CONTRACT", vm.toString(address(povFactory)));

        vm.stopBroadcast();
    }
}

contract ProxyInitializeScript is Script {
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VERIFIER_CONTRACT = vm.envAddress("VERIFIER_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    address POV_FACTORY_CONTRACT = vm.envAddress("POV_FACTORY_CONTRACT");

    function run() external {
        vm.startBroadcast();

        /// initialize spotRegistry and spotVerifier. set visitedBy address.
        SpotRegistry(REGISTRY_CONTRACT).initialize(VISITEDBY_CONTRACT, POV_FACTORY_CONTRACT);
        SpotVerifier(VERIFIER_CONTRACT).initialize(VISITEDBY_CONTRACT);
...
contract CheckContractScript is Script {
    ...
    function run() external {
        ...
        /// 1. registry
        SpotRegistry spotRegistry = SpotRegistry(address(REGISTRY_CONTRACT));
        address _owner = spotRegistry.owner();
        spotRegistry.totalSpot();

        /// 2. verifier
        SpotVerifier spotVerifier = SpotVerifier(address(VERIFIER_CONTRACT));
        _owner = spotVerifier.owner();

        /// 3. VisitedBy
        VisitedBy visitedBy = VisitedBy(address(VISITEDBY_CONTRACT));
        _owner = visitedBy.owner();
        ViSItedToken vsit = ViSItedToken(visitedBy.vsitContract());
        _owner = vsit.owner();
        string memory _name = vsit.name();
        string memory _symbol = vsit.symbol();
        uint256 _total = vsit.totalSupply();

        /// 4. PoVFactory
        PoV5192MetadataFactory povFactory = PoV5192MetadataFactory(address(POV_FACTORY_CONTRACT));

        vm.stopBroadcast();
    }
}

contract RegisterScript is Script, VisitedByTypes, BytesUtil {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");

    // Spot Info
    string SPOT_NAME = vm.envString("SPOT_NAME");
    string SPOT_DESCRIPTION = vm.envString("SPOT_DESCRIPTION");
    string SPOT_IMAGE = vm.envString("SPOT_IMAGE");
    string SPOT_URL = vm.envString("SPOT_URL");
    string SPOT_PROVIDER = vm.envString("SPOT_PROVIDER");
    string SPOT_EXTRA = vm.envString("SPOT_EXTRA");

    function run() external {
        // argument
        address SPOT_OWNER = vm.envAddress("SPOT_OWNER");
        address SPOT_ADDRESS = vm.envAddress("SPOT_ADDRESS");

        vm.startBroadcast();
        SpotRegistry spotRegistry = SpotRegistry(REGISTRY_CONTRACT);
        spotRegistry.register(
            SPOT_OWNER,
            SPOT_ADDRESS,
            SpotInfo({
                name: _splitStringAsBytes32(SPOT_NAME),
                description: _splitStringAsBytes32Array4(SPOT_DESCRIPTION),
                imageUrl: _splitStringAsBytes32Array4(SPOT_IMAGE),
                url: _splitStringAsBytes32Array2(SPOT_URL),
                provider: _splitStringAsBytes32(SPOT_PROVIDER),
                extra: _splitStringAsBytes32(SPOT_EXTRA)
            })
        );
...
contract ResolverDeployScript is Script, VisitedByTypes, BytesUtil {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    address POV_FACTORY_CONTRACT = vm.envAddress("POV_FACTORY_CONTRACT");
    ...
    string SPOT_URL = vm.envString("SPOT_URL");
    string SPOT_PROVIDER = vm.envString("SPOT_PROVIDER");
    int256 SPOT_LATITUDE = vm.envInt("SPOT_LATITUDE");
    int256 SPOT_LONGITUDE = vm.envInt("SPOT_LONGITUDE");
    string SPOT_EXTRA = vm.envString("SPOT_EXTRA");

    function run() external {
        vm.startBroadcast();
        SpotResolver _resolver = new SpotResolver(
            msg.sender,
            SPOT_OWNER,
            SPOT_ADDRESS,
            VISITEDBY_CONTRACT,
            POV_FACTORY_CONTRACT,
            SpotInfo({
                name: _splitStringAsBytes32(SPOT_NAME),
                description: _splitStringAsBytes32Array4(SPOT_DESCRIPTION),
                imageUrl: _splitStringAsBytes32Array4(SPOT_IMAGE),
        ...
        vm.setEnv("RESOLVER_CONTRACT", vm.toString(address(_resolver)));
...
contract MintScript is Script {
    ...
    // argument
    bytes SIGNATURE = vm.envBytes("SIGNATURE");
    bytes32 USER_DATA = bytes32(vm.envBytes("USER_DATA"));

    function run() external {
        vm.startBroadcast();
        VisitedBy visitedBy = VisitedBy(VISITEDBY_CONTRACT);
        visitedBy.mint(SIGNATURE, USER_DATA);

        vm.stopBroadcast();
    }
}

contract MintWithExtendedScript is Script {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VERIFIER_CONTRACT = vm.envAddress("VERIFIER_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    ...
    // argument
    bytes SIGNATURE = vm.envBytes("SIGNATURE");
    bytes EXTENDED = vm.envBytes("EXTENDED");
    bytes32 USER_DATA = bytes32(vm.envBytes("USER_DATA"));

    function run() external {
        vm.startBroadcast();
        VisitedBy visitedBy = VisitedBy(VISITEDBY_CONTRACT);
        visitedBy.mint(SIGNATURE, bytes32(EXTENDED), USER_DATA);

        vm.stopBroadcast();
    }
}

contract AddPoVScript is Script, VisitedByTypes, BytesUtil {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    uint256 POV_FACTORY_ID = vm.envUint("POV_FACTORY_ID");

    // argument
    address SPOT_ADDRESS = vm.envAddress("SPOT_ADDRESS");

    // PoV Info
    string POV_NAME = vm.envString("POV_NAME");
    string POV_DESCRIPTION = vm.envString("POV_DESCRIPTION");
    string POV_IMAGE = vm.envString("POV_IMAGE");
    string POV_URL = vm.envString("POV_URL");
    string POV_PROVIDER = vm.envString("POV_PROVIDER");
    int256 POV_LATITUDE = vm.envInt("POV_LATITUDE");
    int256 POV_LONGITUDE = vm.envInt("POV_LONGITUDE");
    string POV_EXTRA = vm.envString("POV_EXTRA");

    function run() external {
        vm.startBroadcast();
        SpotRegistry spotRegistry = SpotRegistry(REGISTRY_CONTRACT);
        address _resolverAddress = spotRegistry.getResolver(SPOT_ADDRESS);

        SpotResolver(_resolverAddress).add(
            POV_FACTORY_ID,
            PoVInfo({
                name: _splitStringAsBytes32(POV_NAME),
                description: _splitStringAsBytes32Array4(POV_DESCRIPTION),
                imageUrl: _splitStringAsBytes32Array4(POV_IMAGE),
                url: _splitStringAsBytes32Array2(POV_URL),
                provider: _splitStringAsBytes32(POV_PROVIDER),
                latitude: POV_LATITUDE,
                longitude: POV_LONGITUDE,
                extra: _splitStringAsBytes32(POV_EXTRA)
            })
        );
        vm.stopBroadcast();
    }
...
contract AddCustomPoVScript is Script, VisitedByTypes, BytesUtil {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address POV_FACTORY_CONTRACT = vm.envAddress("POV_FACTORY_CONTRACT");
    uint256 POV_ID = vm.envUint("POV_ID");
    ...
    function run() external {
        vm.startBroadcast();
        // create PoV
        IPoVFactory povFactory = IPoVFactory(POV_FACTORY_CONTRACT);

        SpotRegistry spotRegistry = SpotRegistry(REGISTRY_CONTRACT);
        address _resolverAddress = spotRegistry.getResolver(SPOT_ADDRESS);

        PoVInfo memory _povInfo = PoVInfo({
            name: _splitStringAsBytes32(POV_NAME),
            description: _splitStringAsBytes32Array4(POV_DESCRIPTION),
            imageUrl: _splitStringAsBytes32Array4(POV_IMAGE),
            url: _splitStringAsBytes32Array2(POV_URL),
        ...
        address _pov = povFactory.createNewPoV(SPOT_OWNER, _resolverAddress, _povInfo);

        SpotResolver(_resolverAddress).addCustomPoV(_pov, POV_ID);
        vm.stopBroadcast();
    }
}

contract ShowBalancesScript is Script {
    // read env
    address REGISTRY_CONTRACT = vm.envAddress("REGISTRY_CONTRACT");
    address VERIFIER_CONTRACT = vm.envAddress("VERIFIER_CONTRACT");
    address VISITEDBY_CONTRACT = vm.envAddress("VISITEDBY_CONTRACT");
    ...
    function run() external {
        ...
        /// 1. registry
        SpotRegistry spotRegistry = SpotRegistry(address(REGISTRY_CONTRACT));
        spotRegistry.getOwnSpots(msg.sender);

        /// 3. VisitedBy
        VisitedBy visitedBy = VisitedBy(address(VISITEDBY_CONTRACT));
        visitedBy.experienceOf(msg.sender);
        visitedBy.complaintOf(msg.sender);

        ViSItedToken vsit = ViSItedToken(visitedBy.vsitContract());
        vsit.balanceOf(msg.sender);
...
abstract contract PoV5192 is PoVBase, IERC5192 {
    // ================================================================
    //  mappings
    // ================================================================
    /// lock mapping
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(PoVBase) returns (bool) {
...
import "../lib/IPoVFactory.sol";
import "./PoV5192Metadata.sol";

contract PoV5192MetadataFactory is IPoVFactory {
    // ================================================================
    //  user functions
    // ================================================================
    /**
    ...
    function createNewPoV(
        ...
        PoV5192Metadata pov = new PoV5192Metadata(povOwner, resolver, poVInfo);
        emit CreatePoV(address(pov), povOwner);
        return address(pov);
    }
}
...
import "../lib/PoV5192.sol";

contract PoV5192Metadata is PoV5192, Ownable {
    // ================================================================
    //  usings
    // ================================================================
    using Counters for Counters.Counter;
    ...
    constructor(
        ...
        super._povInit(resolverContractAddress);
        _povInfo = povInfo;
    }
    ...
    function mint(
        ...
        super._mint(to, tokenId, userData);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (address(0) == super.ownerOf(tokenId)) revert("PoV: URI query for nonexistent token");

        // separate metadata for avoiding stack too deep error
        bytes memory _metadata1 = abi.encodePacked(
            '{"name":"',
        ...
        bytes memory _metadata2 = abi.encodePacked(
            '"url":"',
            string(abi.encodePacked(_povInfo.url)),
            '",'
            '"provider":"',
            string(abi.encodePacked(_povInfo.provider)),
            '",',
            '"owner":"',
            Strings.toHexString(uint160(super.ownerOf(tokenId)), 20),
            '",'
        );

        bytes memory _metadata3 = abi.encodePacked('"extra":"', string(abi.encodePacked(_povInfo.extra)), '",', '"userData":"', Strings.toHexString(uint256(super.getUserData(tokenId))), '",');

        string memory _latitude = _int2string(_povInfo.latitude);
        string memory _longitude = _int2string(_povInfo.longitude);

        bytes memory _metadata4 = abi.encodePacked('"latitude":', _latitude, ",", '"longitude":', _longitude, ",", '"spot":"', Strings.toHexString(uint160(super.getResolver()), 20), '"}');

        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(abi.encodePacked(_metadata1, _metadata2, _metadata3, _metadata4))));
    }
     ...
     * @param __name bytes32, name of pov
     */
    function setName(bytes32 __name) public onlyOwner {
     ...
     * @notice msg.sender should be pov owner or admin.
     * @param description bytes32[4] memory, description of pov
     */
    function setDescription(bytes32[4] memory description) public onlyOwner {
     ...
     * @notice msg.sender should be pov owner or admin.
     * @param imageUrl bytes32[4] memory, image of pov
     */
    function setImage(bytes32[4] memory imageUrl) public onlyOwner {
     ...
     * @dev set Url of pov
     * @notice msg.sender should be pov owner or admin.
     * @param url bytes32[2] memory, url of pov
     */
    function setUrl(bytes32[2] memory url) public onlyOwner {
        _povInfo.url = url;
    }
     ...
     * @notice msg.sender should be pov owner or admin.
     * @param provider bytes32, provider of pov
     */
    function setProvider(bytes32 provider) public onlyOwner {
        _povInfo.provider = provider;
    }

    /**
     * @dev set Extra of pov
     * @notice msg.sender should be pov owner or admin.
     * @param extra bytes32 memory, extra of pov
     */
    function setExtra(bytes32 extra) public onlyOwner {
        _povInfo.extra = extra;
    }

    function changePosition(int256 latitude, int256 longitude) public onlyOwner {
     ...
     * @dev Returns initial name
     */
    function _initialName() internal pure override returns (string memory) {
        return "ProofOfVisitMetadata";
    }

    /**
     * @dev Returns initial symbol
     */
    function _initialSymbol() internal pure override returns (string memory) {
        return "POVM";
    }
