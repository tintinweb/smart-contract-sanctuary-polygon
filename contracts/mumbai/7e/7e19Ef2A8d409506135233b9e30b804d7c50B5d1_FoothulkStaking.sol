    ...
    ERC20PresetMinterPauserUpgradeable
{
    ...
    function claim(uint256[] memory ids) external whenNotPaused {
        uint256 total = _calculateYield(ids);
        require(total > 0, "No tokens available");

        mint(msg.sender, total);
    }

    function _valueIfOverflow(uint256 amount) public view returns (uint256) {
    ...
    function _getDelta(uint256 id) public view returns (uint256) {
        uint256 lastTimestamp = _timestamps[id] > 0 ? _timestamps[id] : startTime;
        return block.timestamp - lastTimestamp;
    }

    function _getTotal(uint256 id) virtual public view returns (uint256) {
