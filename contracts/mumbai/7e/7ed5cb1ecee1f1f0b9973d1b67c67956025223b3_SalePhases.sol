...
contract SalePhases is Context, Ownable, Initializable {
    ...
    function standardUnlock() internal {
        if(whatUnlockPhaseWeAre() >= 3) { releaseAll(true,false,false,false); }
        if(whatUnlockPhaseWeAre() >= 6) { releaseAll(true,true,false,false); }
        if(whatUnlockPhaseWeAre() >= 8) { releaseAll(true,true,true,false); }
        ...
        _pendingpurchasedPrivateSaleTokens[msg.sender] += (_purchasedPrivateSaleTokens[msg.sender] / 100 * 10) * whatUnlockPhaseWeAre();
        _pendingpurchasedPrivateSale2Tokens[msg.sender] += (_purchasedPrivateSale2Tokens[msg.sender] / 100 * 15) * whatUnlockPhaseWeAre();
        _pendingpurchasedPreSaleTokens[msg.sender] += (_purchasedPreSaleTokens[msg.sender] / 100 * 20) * whatUnlockPhaseWeAre();
    }
