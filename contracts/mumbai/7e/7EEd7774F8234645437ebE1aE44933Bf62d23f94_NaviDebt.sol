...
/**
 * @title Debt token representing 1 USD of reserves.
 * @notice Can be minted by NaviController contract for liqudating an unfunded accounts (with negative balance).
 * @dev Can be exchanged for reserves in the NaviController or nToken contracts.
 **/
contract NaviDebt is ERC777, ERC20Permit, IBridged, INaviDebt {
    using SafeERC20 for IERC20;
    address public immutable _bridgeContract;
    modifier onlyBridge() {
        require(
            _bridgeContract == msg.sender,
            "NaviDebt: caller is not a bridge"
        );
        _;
    }

    address public immutable _controllerContract;
    modifier onlyController() {
        require(
            _controllerContract == msg.sender,
            "NaviDebt: caller is not a controller"
        );
        _;
    }

    constructor(address bridgeContract, address controllerContract)
        ERC777("NaviDebt", "NAVIDEBT", new address[](0))
        ERC20Permit("NaviDebt")
    {
    ...
    function controllerMint(address account, uint256 amount)
        external
        override
        onlyController
    {
