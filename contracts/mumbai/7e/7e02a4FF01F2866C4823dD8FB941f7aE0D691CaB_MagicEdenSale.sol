...
/**
 * @title MagicEden Metarun Sale
 * @dev Ensures purchase of MysteryBoxes
 */

contract MagicEdenSale is AccessControl {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    MetarunCollection public collection;

    // Sellable kind (defines range of sellable ids)
    uint256 private collectionKind;

    uint256 public mysteryBoxPrice;
    uint256 public currentBoxPosition;
    uint256 public totalMinted;

    event BoxBought(address owner, uint32 qty, uint256 price);

    /**
     * @dev the constructor arguments:
     * @param _collection ERC1155 token of NFT collection
     */
    constructor(MetarunCollection _collection) {
        collection = _collection;
        collectionKind = collection.MYSTERY_BOX_KIND();
     ...
     * @dev Buy mystery box
     */
    function mint(uint32 qty) public payable {
        require(mysteryBoxPrice != 0, "MysteryBox price not set");
        require(msg.value == qty * mysteryBoxPrice, "Incorrect msg.value");

        collection.mintBatch(msg.sender, collectionKind, qty, currentBoxPosition);
        totalMinted += qty;
        currentBoxPosition += qty;

        emit BoxBought(msg.sender, qty, mysteryBoxPrice);
    }
    ...
    /**
     * @dev Sets the value of the variable totalMinted. Start position for mintBatch
     */
    function setCurrentBoxPosition(uint256 value) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You should have DEFAULT_ADMIN_ROLE");
        currentBoxPosition = value;
    }

    /**
     * @dev Send all native tokens from the contract address to msg.sender with DEFAULT_ADMIN_ROLE
     */
...
contract MetarunCollection is ERC1155Upgradeable, AccessControlUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
    ...
    uint256 public constant IGNIS_CLASSIC_HALLOWEEN = 0x00010603;
    uint256 public constant IGNIS_EPIC_HALLOWEEN = 0x00010703;
    uint256 public constant IGNIS_LEGENDARY_HALLOWEEN = 0x00010803;

    uint256 public constant PENNA_CLASSIC_HALLOWEEN = 0x00020603;
    uint256 public constant PENNA_EPIC_HALLOWEEN = 0x00020703;
    uint256 public constant PENNA_LEGENDARY_HALLOWEEN = 0x00020803;

    uint256 public constant ORO_CLASSIC_HALLOWEEN = 0x00030603;
    uint256 public constant ORO_EPIC_HALLOWEEN = 0x00030703;
    uint256 public constant ORO_LEGENDARY_HALLOWEEN = 0x00030803;

    uint256 public constant IGNIS_CLASSIC_CHRISTMAS = 0x00010604;
    uint256 public constant IGNIS_EPIC_CHRISTMAS = 0x00010704;
    uint256 public constant IGNIS_LEGENDARY_CHRISTMAS = 0x00010804;

    uint256 public constant PENNA_CLASSIC_CHRISTMAS = 0x00020604;
    uint256 public constant PENNA_EPIC_CHRISTMAS = 0x00020704;
    uint256 public constant PENNA_LEGENDARY_CHRISTMAS = 0x00020804;

    uint256 public constant ORO_CLASSIC_CHRISTMAS = 0x00030604;
    uint256 public constant ORO_EPIC_CHRISTMAS = 0x00030704;
    uint256 public constant ORO_LEGENDARY_CHRISTMAS = 0x00030804;
    ...
    function mintBatch(
        ...
        uint256 currentTokenId = (kind << 16) | startPosition;
