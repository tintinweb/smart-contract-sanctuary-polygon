...
contract WorktreeFactory {
    event NewTree(address tree, address claimModule);
    ...
    function createERC20(
        ...
        IClaimModule claimModule = new ERC20Claim{ salt: salt }(token, claimAmount);
        Worktree worktree = new Worktree{ salt: salt }(verifier, claimModule, root);
        worktree.transferOwnership(msg.sender);
        address treeAddress = address(worktree);
        address claimAddress = address(claimModule);
        emit NewTree(treeAddress, claimAddress);
