...
contract Treasury is Ownable, ReentrancyGuard {
    ...
    event GameStateSet(uint gameId, uint state, address player);
    event GameEnded(uint gameId, uint gameResult, address player);
    ...
    */
    function SetNFTVault(address _newVault) onlyOwner() external returns(address) {
        NFT_VAULT = _newVault;
        return NFT_VAULT;
    }
    ...
    */
    function DistributeFeesBackToVaults() nonReentrant onlyOwner() external {
        ...
        uint leftFeeBalance = address(this).balance - FeeVaultBalance;
        ...
        emit DistributedFeeBackToVaults(teamVault, nftVault);
    }
    ...
    */
    function SetWager(uint _gameAction) nonReentrant external payable returns(uint gameId) {
        ...
        emit GameStateSet(gameId, 1, Games[gameId].PlayerAddress);
        return gameId;
    }
     ...
     * @return gameId the new gameId for player
    */
    function WithdrawWager(uint _gameId) nonReentrant external returns(uint gameId) {
        _validGame(_gameId);
        _requireState(_gameId, 1); // game state 2: game updated
        ...
        Games[_gameId].State = 4; // game abandoned
        emit GameStateSet(_gameId, 4, Games[_gameId].PlayerAddress); // game abandoned
    ...
    */
    function UpdateGame(uint _gameId, uint _result) TreasuryOwner() nonReentrant external returns(uint gameId) {
        _validGame(_gameId);
        ...
        if (_result == 0) {
            ...
            emit GameStateSet(_gameId, 2, Games[_gameId].PlayerAddress);
        } else if (_result == 1) {
            // Player loses
            ...
            if (TAKE_FEE) {
                ...
                FeeVaultBalance += gameFee;
            }

            // update game state
            Games[_gameId].State = 3; // game ended
            emit GameStateSet(_gameId, 3, Games[_gameId].PlayerAddress); // game ended
            emit GameEnded(_gameId, Games[_gameId].Result, Games[_gameId].PlayerAddress);
        } else if (_result == 2) {
            // TIE
     ...
     * @return gameId the gameId
    */
    function GameResult(uint _gameId) nonReentrant external returns(uint gameId) {
...
struct Game{
    ...
    uint Wager;
    uint State; // 0 init, 1 wager set, 2 results set, 3 game played, 4 abandoned
    bool Exists;
}
