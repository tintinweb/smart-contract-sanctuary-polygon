    ...
    Ownable
{
    ...
    event CancelListing(uint256 _tokenId, uint256 _nonce, address _seller);
    event MatchListing(uint256 _tokenId, uint256 _nonce, address _buyer, address _seller);
    ...
    // keccak256("ServerAuthHash(address buyer,uint256 tokenId,uint256 shardPrice,uint256 txnExpiry)")
    bytes32 public constant SERVER_AUTH_TYPEHASH = 0x5b43859ee0614dc801f4fcbe35adccc9745c29f27f79ebf8248ce80edfe6add8;
    ...
    bytes32 public constant ADD_LISTING_TYPEHASH = 0xe03a3d655fda03b186b889ec549966697994237d3b02c1ec0635fd5b65591948;

    mapping (address => bool) private allowedBaseTokens;

    constructor () {
        uint256 chainId;
        name = "Venari Marketplace";
    ...
    function cancelListing(uint256 tokenId) external {
        ...
        emit CancelListing(tokenId, _sellerNonces[msg.sender][tokenId], msg.sender);
    ...
    function matchListing(
        ...
        require(IERC20(baseTokenAddress).balanceOf(msg.sender) >= basePrice, "Insufficient base balance");
        require(IERC20(baseTokenAddress).allowance(msg.sender, address(this)) >= basePrice, "Insufficient base allowance");
        require(shardInterface.balanceOf(msg.sender) >= shardPrice, "Insufficient shard balance");
        require(shardInterface.allowance(msg.sender, address(this)) >= shardPrice, "Insufficient shard allowance");
        require(venariInterface.isApprovedForAll(seller, address(this)), "Marketplace not approved for transfer");

        // Transfer funds from buyer to this contract.
        IERC20(baseTokenAddress).safeTransferFrom(msg.sender, address(this), basePrice);
        shardInterface.safeTransferFrom(msg.sender, address(this), shardPrice);

        // Verifies signatures of seller and server
        {   // Scoping for stack too deep error
            bytes32 sellerMessage = generateAddListingHash(seller, tokenId, listingExpiry, baseTokenAddress, basePrice).toEthSignedMessageHash();
        ...
        emit MatchListing(tokenId, _sellerNonces[seller][tokenId], msg.sender, seller);
