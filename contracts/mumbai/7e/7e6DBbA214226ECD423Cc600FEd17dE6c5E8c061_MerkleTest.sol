...
contract MerkleTest is ERC721A, Ownable {
    ...
    uint256 public constant MAX_TOKEN_SUPPLY = 11111;
    address public constant DEV_ADDRESS = 0xBDC1396201845BF3f4Fd2791C9C57888D6F0b369;
    ...
    constructor(string memory baseURI, bytes32 _merkleRoot) ERC721A("MerkleTest", "MERKLE") {
        setBaseURI(baseURI);
    ...
    function mint(
        ...
        require(total <= MAX_TOKEN_SUPPLY, "MerkleTest: Max limit");
        require(total + _count <= MAX_TOKEN_SUPPLY, "MerkleTest: Max limit");
        require(total + _count <= _maxSale, "MerkleTest: Max sale limit");
        require(_sale, "MerkleTest: Sale is not active");
        require(msg.value >= getPrice(_count), "MerkleTest: Value below price");

        if (_whitelist == true) {
            // Verify if the account has already claimed
            require(
                !isPresaleListClaimed(msg.sender),
                "MerkleTest: account already claimed"
            );

            // Verify we cannot claim more than the max amount
            require(
                _count <= _presaleMaxAmount,
                "MerkleTest: can only claim less than or equal to the max amount"
            );
    ...
    function _mintElements(address _to, uint256 _amount) private {
        ...
        require(totalSupply() <= MAX_TOKEN_SUPPLY, "MerkleTest: limit reached");
    }
    ...
    function _payout(address _address, uint256 _amount) private {
        ...
        require(success, "MerkleTest: transfer failed");
    }
    ...
    function mintReserve(uint256 _count, address _to) public onlyOwner {
        uint256 total = totalSupply();
        require(total <= MAX_TOKEN_SUPPLY, "MerkleTest: sale ended");
        require(total + _count <= MAX_TOKEN_SUPPLY, "MerkleTest: max limit");
    ...
    function withdrawAll() public payable onlyOwner {
        ...
        require(balance > 0, "MerkleTest: malance should be above 0");
