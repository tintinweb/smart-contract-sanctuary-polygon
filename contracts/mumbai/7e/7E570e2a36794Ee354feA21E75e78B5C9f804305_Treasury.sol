...
contract Treasury is ITreasury, Ownable, ERC721Holder, ReentrancyGuard, Initializable {
  ...
  function approveERC20(
    ...
    emit ERC20Allowance(erc20ContractAddr, spender, amount);
  }
  ...
  function transferERC20BatchWithVesting(
    ...
    uint64[] calldata vestingStartTimestamps,
    uint64[] calldata vestingDurationSeconds
  ) external override onlyOwner nonReentrant {
    ICoCreateLaunch coCreate = project.getCoCreate();
    address vestingWalletImpl = coCreate.getImplementationForType("VestingWallet");
    require(vestingWalletImpl != address(0), "Invalid vesting wallet component");
    require(amount.length == transferTo.length, "Invalid amount and transfer length");
    require(amount.length == vestingStartTimestamps.length, "Invalid amount and vestingStartTimestamps length");
    require(amount.length == vestingDurationSeconds.length, "Invalid amount and vestingDurationSeconds length");
...
interface ITreasury {
  ...
  event ERC20Allowance(address erc20, address spender, uint256 amount);
  ...
  /**
   * @notice Approve funds from the ERC20 balance to the given address
   *
   * @param erc20ContractAddr Contract address of the erc20 token
  ...
  /**
   * @notice Burns the project token
   *
   * @param projectToken to be burned
   * @param amount of token that needs to be burned
   */
  ...
  /**
   * @notice Deploys vesting wallet for the vesting parameters and transfers the specified amount to the vesting wallets
   *
   * @param erc20ContractAddr address of erc20 to be transferred
   * @param amount array of amounts to be transferred
   * @param transferTo array of address who could withdraw from the vesting wallets
   * @param vestingStartTimestamps array of starting time for vesting
   * @param vestingDurationSeconds array of duration for vesting
   */
  function transferERC20BatchWithVesting(
    address erc20ContractAddr,
...
interface ICoCreateProject {
  ...
  /**
   * @notice returns the Co:Create Launch associated with the Co:Create project
   */
  function getCoCreate() external view returns (ICoCreateLaunch);

  /**
   * @notice returns the project token associated with the Co:Create project
   */
  function getProjectToken() external view returns (ProjectToken);

  /**
   * @notice returns the Treasuries associated with the Co:Create project
   */
  function getTreasuries() external view returns (address[] memory);

  /**
   * @notice returns the name of the Co:Create project
   */
  function name() external view returns (string memory);

  /**
   * @notice returns the owner of the Co:Create project
   */
  function getContractOwner() external view returns (address);

  /**
   * @notice returns true if the address is a componenet deployed by Co:Create project
   */
  function isExistingComponent(address component) external view returns (bool);

  /**
   * @notice Deploys a proxy contract using the implementation address defined for the component type
   *
   * @param componentType for which the proxy to be deployed
   * @param data to be used for deploying the proxy
   */
  function deployProxy(string memory componentType, bytes memory data) external returns (address);

  /**
   * @notice Deploys a UUPS contract using the implementation address defined for the component type
   *
   * @param componentType for which the proxy to be deployed
   * @param data to be used for deploying the proxy
   */
  ...
  /**
   * @notice Sets the owner for the component contracts deployed from Co:Create Project. This is not related to the
   * AccessControl of Co:Create Project. This wouldn't change the owners for the existing deployed components. The new
   * Owner will only be applied for new components deployed
   *
   * @param newOwner address for the new Owner
   */
...
contract ProjectToken is OwnableUpgradeable, ERC20BurnableUpgradeable, ERC20VotesUpgradeable, UUPSUpgradeable {
  ...
  function initialize(
    ...
    require(!isFixedSupply_ || initialSupply_ > 0, "invalid initial supply");
...
interface ICoCreateLaunch {
  event CoCreateLaunchInitialized(address upgradeGate, address owner, address coCreateProjectFactory);
  event CoCreateProjectDeployed(address indexed project, string name, string description, address admin);
  event ComponentAdded(string componentType, address implementation);
  event ComponentRemoved(string componentType, address implementation);

  /**
   * @notice returns name of the Co:Create launch
   */
  function name() external view returns (string memory);

  /**
   * @notice returns the Factory to create Co:Create Project
   */
  function getCoCreateProjectFactory() external view returns (address);

  /**
   * @notice returns the UpgradeGate used by Co:Create. Any upgrade path should be registered to UpgradeGate
   */
  function getUpgradeGate() external view returns (UpgradeGate);

  /**
   * @notice returns true if the address is a valid Co:Create project created from Co:Create Launch
   */
  function isCoCreateProject(address coCreateProject) external view returns (bool);

  /**
   * @notice returns implementation address for the component type
   */
  function getImplementationForType(string memory componentType) external view returns (address);

  /**
   * @notice adds component implementation to Co:Create
   *
   * @param componentType name of the component Type to add
   * @param implementationAddress of the component to add
   */
  function addComponentContract(string memory componentType, address implementationAddress) external returns (bool);

  /**
   * @notice remove component implementation from Co:Create
   *
   * @param componentType name of the component Type to remove
   * @param implementationAddress of the component remove
   */
  function removeComponentContract(string memory componentType, address implementationAddress) external returns (bool);

  /**
   * @notice update component implementation from Co:Create. removes existing implementation address and adds the new
   * implementation address
   *
   * @param componentType name of the component Type to update
   * @param implementationAddress of the component update
   */
...
contract UpgradeGate is Ownable {
  ...
  function validateUpgrade(address newImpl, address currentImpl) external view {
  ...
  function registerPath(address newImpl, address currentImpl) external onlyOwner {
    require(!validUpgradePath[newImpl][currentImpl], "already registered");
  ...
  function unregisterPath(address newImpl, address currentImpl) external onlyOwner {
    require(validUpgradePath[newImpl][currentImpl], "not registered");
