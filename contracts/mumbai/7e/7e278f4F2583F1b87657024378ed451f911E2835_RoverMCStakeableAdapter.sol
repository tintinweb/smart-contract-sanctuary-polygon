...
contract RoverMCStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC721HolderUpgradeable, VRFConsumerBaseV2Upgradable {
    event ParametersUpdated(address _stakeable, uint256 _id, uint256 _cap, bool _isBase);
    ...
    mapping(int256 => mapping(int256 => TokenInfo)) public tokenInfo;
    ...
    */
    ...
    function setResourceParameters(
        ...
        emit ParametersUpdated(address(this), _resourceId, _resourceCap, _isBase);
    }
        ...
        )
    {
        ...
        (uint256 rate, uint256 m3tam0dChance) = getRoverRate(tokenId, _x, _y);
        uint256 blockTime = _pausedAt > 0 ? _pausedAt : block.timestamp;
        uint256 elapsed = blockTime - localSeed;
        uint256 ret;
        if (elapsed > maxCutoff) {
            ret = (rate * maxCutoff) / 1 days;
        } else {
            ret = (rate * elapsed) / 1 days;
        }
        return (ret, tokenInfo[_x][_y].resourceId, m3tam0dChance);
    }
