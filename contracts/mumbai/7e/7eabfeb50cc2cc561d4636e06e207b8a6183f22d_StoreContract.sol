...
contract WhitelistContract is AccessControlEnumerable {
    ...
    // Define a function to get the members of a role
    ...
    function isPurchasable(address account, uint256 storeId, uint256 dropId, uint256 itemIndex) public view returns (bool){
        ...
        if (whitelistedStoreItem.maxMint > 0){
            uint256 timestamp = block.timestamp;
            if (timestamp > whitelistedStoreItem.date)
                return true;
        }
        return false;
    }
...
//import "./StoreContract.sol";
   ...
   */
contract MintContract is DefaultOperatorFilterer, ERC721, ERC721Enumerable, ERC721URIStorage, AccessControlEnumerable, IERC2981 {
...
/*
    ...
    address private storeContractAddress;
*/
    ...
    event NFTTransfered(address indexed from, address indexed to, uint256 indexed tokenId);
    constructor(address admin) ERC721("IRL Smart Collectibles", "IRL") {
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
...
/*
    ...
    function getStoreContract() public view onlyRole(DEFAULT_ADMIN_ROLE) returns (address){
...
//import "./sol";
   ...
   * @title ContractName
   * @dev ContractDescription
   * @custom:dev-run-script contracts/StoreContract.sol
   */
contract StoreContract is AccessControlEnumerable {
    ...
    function publicMintNFTFromStore(uint256 storeId, uint256 dropIndex, uint256 itemIndex, int256[] memory token, string memory modelId) public payable{
        ...
        if (storeItem.drop.date > timestamp){
            ...
            if (token[0] != -1 && (IERC721(mintContractAddress).ownerOf(uint256(token[0])) == msg.sender ||
                                   IERC721(mintContractAddress).ownerOf(uint256(token[0])) ==  IRLWallet)){
            ...
            if (storeItem.utils.isWhitelisted){
                isWhitelisted = whitelistContractInstance.isPurchasable(msg.sender, storeId, dropIndex, itemIndex);
            }            
        ...
        if (bytes(storeItem.utils.allowPassIdMint).length != 0){
            require(token[1] != -1 && (IERC721(mintContractAddress).ownerOf(uint256(token[1])) == msg.sender ||
                                   IERC721(mintContractAddress).ownerOf(uint256(token[1])) ==  IRLWallet) && compareStrings(tokenStoreItemList[uint256(token[1])], storeItem.utils.allowPassIdMint), "User does not own allowPassId");        
        }
        ...
        if (token[2] != -1 && (IERC721(mintContractAddress).ownerOf(uint256(token[2])) == msg.sender ||
                                   IERC721(mintContractAddress).ownerOf(uint256(token[2])) ==  IRLWallet) 
    ...
    function getStoreItemPriceInWei(uint256 storeId, uint256 dropIndex, uint256 itemIndex, int256 discountToken, address sender) public view returns (uint256){
        ...
        uint256 discountDate = storeItem.utils.discount.date;
        string memory discountPassId = storeItem.utils.discount.passId;
        if (discountToken != -1 && (IERC721(mintContractAddress).ownerOf(uint256(discountToken)) == IRLWallet || 
            IERC721(mintContractAddress).ownerOf(uint256(discountToken)) == sender) 
    ...
    function writeTokenStoreItem(uint256 tokenId, uint256 storeId, uint256 dropIndex, uint256 itemIndex) public onlyRole(MANAGER){
