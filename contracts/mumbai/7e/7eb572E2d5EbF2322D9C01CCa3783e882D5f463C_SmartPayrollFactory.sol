...
contract DAOVault is AccessControl{
  ...
  function transfer(address _receiver) internal  {
    ...
    require(tokenBalance>0||ethBalance>0, "no funds");

      if(tokenBalance>0){
        IERC20(ERC20Token).transfer(_receiver,tokenBalance);
...
contract SmartPayrollFactory {
  ...
  function createUpkeepContract(
    ...
    SmartPayrollByTime smartPayrollByTime = new SmartPayrollByTime(_upkeepContractParams, _amount,address(this),_credit,_DAOAddress);
  ...
  function createDAOVault(address _employer,address _employee,address _erc20) external {
    DAOVault daoVault = new DAOVault(_employer,_employee,_erc20);
    emit DAOVaultCreate(address(daoVault),_employer,_employee);
  }
}
