...
contract Router is IRouter {
    ...
    constructor(address _vault, address _usda, address _weth) public {
        vault = _vault;
        usda = _usda;
        weth = _weth;
    ...
    function _vaultSwap(address _tokenIn, address _tokenOut, uint256 _minOut, address _receiver) private returns (uint256) {
        ...
        if (_tokenOut == usda) { // buyUSDA
            amountOut = IVault(vault).buyUSDA(_tokenIn, _receiver);
        } else if (_tokenIn == usda) { // sellUSDA
            amountOut = IVault(vault).sellUSDA(_tokenOut, _receiver);
        } else { // swap
...
interface IVaultUtils {
    ...
    function getMarketPrice(address _indexToken, uint256 delta, bool isLong) external view returns (uint256);
