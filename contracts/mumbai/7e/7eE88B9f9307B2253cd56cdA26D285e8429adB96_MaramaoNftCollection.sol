...
import "../mixin/ERC721TokenPausable.sol";

contract MaramaoNftCollection is
    ERC721,
    ERC721Enumerable,
    ERC721URIStorage,
    Pausable,
    ...
    constructor(string memory _name, string memory _symbol, address _oracleAddress) ERC721(_name, _symbol) {
        require(_oracleAddress != address(0), "Cannot be null address");

        oracleAddress = _oracleAddress;
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(PAUSER_ROLE, _oracleAddress);
    ...
    function pauseToken(uint256 tokenId) public onlyRole(PAUSER_ROLE) onlyRole(MINTER_ROLE) {
        require(_exists(tokenId), "MaramaoNftCollection: pausing a nonexistent token");
        _pauseToken(tokenId);
    }

    function unpauseToken(uint256 tokenId) public onlyRole(PAUSER_ROLE) onlyRole(MINTER_ROLE) {
        require(_exists(tokenId), "MaramaoNftCollection: unpausing a nonexistent token");
