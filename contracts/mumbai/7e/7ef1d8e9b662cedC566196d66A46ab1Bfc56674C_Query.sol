...
contract Query is AccessControl {
    ...
    */
    ...
    function getMultiBalance(address[] memory _addresses) view external returns (uint256[] memory) {
        uint256[] memory balances = new uint256 [](_addresses.length);
        for (uint256 i = 0; i < _addresses.length; i++) {
            balances[i] = vaultBalance.getBalance(_addresses[i]);
        }
        return balances;
    }
    //Get Multi Item

    function getMultiItem(address[] memory _userAddress , uint256 [] memory _itemId) external view returns (uint256[] memory) {
    uint256[] memory multiItem = new uint256[](_userAddress.length * _itemId.length);
    for(uint256 i = 0; i < _userAddress.length; i++) {
      for (uint256 j = 0; j < _itemId.length; j++) {
        uint256 addrIdx = j + _itemId.length * i;
          multiItem[addrIdx] = vaultItem.getItemAmountbyId(_userAddress[i],_itemId[j]);  
      }  
    }
    return multiItem;
  }
