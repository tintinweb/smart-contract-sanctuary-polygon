...
contract EthermonStaking is EthermonStakingBasic, EthermonEnum {
    ...
    function updateTokens(uint256 _lockId) external {
        ...
        if (timeElapsed > 0 && currentTime > data.endTime) {
            ...
            data.balance +=
                (((data.teamPower * 10) / stakingData.SumTeamPower()) * 600) /
                10;

            emit Deposite(data.owner, address(this), data.emons);
            data.emons += data.balance;

            emit Withdraw(data.owner, address(this), data.balance);
            emit Deposite(data.owner, address(this), data.emons);
            emit Withdraw(data.owner, address(this), timeElapsed);
        ...
        //     600 *
        //     timeElapsed;
        uint256 hourlyEmon = (((data.teamPower * 10) /
            stakingData.SumTeamPower()) * 600) / 10;
