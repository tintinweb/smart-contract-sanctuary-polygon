...
contract LostChainUserDataMasterV2 is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    function initialize(LostChainForwarder forwarder, address rbaLinkerAddress) public initializer {
        require(address(forwarder) != address(0x0), 'LCUDM: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCUDM: RBA linker is zero address');
        LCRBALinker = ILostChainRBALinker(rbaLinkerAddress);
    ...
    function registerUser(
        ...
        require(isTrustedForwarder(msg.sender), 'LCUDM: TX only allowed from forwarder');
        require(!userWalletLink[uid].enabled, 'LCUDM: User already exists');
        require(!checkUsernameExists(username), 'LCUDM: Username is taken');
