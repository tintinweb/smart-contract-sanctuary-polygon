...
contract Knockout {
    ...
    mapping(uint => TournamentConfig) tournaments;

    mapping(uint => uint) totalAmount; // tournament index => total value
    mapping(uint => uint) currentStep; // tournament index => current step (0 = not started, 1 = first round, 2 = second round)
    mapping(uint => mapping(uint => address[])) steps; // tournament index => current step => participants list
    mapping(uint => mapping(uint => mapping(address => bool))) claimWon; // tournament index => current step => address => claim won
    mapping(uint => mapping(address => bool)) participating; // tournament index => address => is participating
    mapping(uint => mapping(address => bool)) hasWithdrawn; // tournament index => address => has withdrawn price or ticket cost
