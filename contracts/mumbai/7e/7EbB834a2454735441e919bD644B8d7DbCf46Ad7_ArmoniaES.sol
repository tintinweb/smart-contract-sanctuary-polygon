...
*/
  ...
  function registerByAdmin(string memory _nomalias , address payable _addr , string memory _ipfs  ) public onlyAdmins {
    _nomalias = string(abi.encodePacked('arm.per.', _nomalias ));
    require( genteByAlias[_nomalias].addr == address(0) , 'Alias Duplicado');
    require( genteByAddr[_addr].addr == address(0) , 'Pasaporte ya creado');
    Gente memory oGente = Gente( _addr , _ipfs , _nomalias  );  
