...
contract Tierraland is ERC721URIStorage, ERC2981, Ownable  {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIds;
  uint256 private MAX_TOKEN = 10000;
  uint256 private _price = 10_0000_0000_0000_0000;
  mapping(address => uint256[]) private _nftOfWallet;
  mapping(uint256 => uint256) private _nftIndex;

  constructor() ERC721("Tierraland", "TRL") {}
  ...
  function mintBatch(uint256 number, string[] memory tokenURI) public payable returns(uint256) {
    require(MAX_TOKEN >= number + _tokenIds.current(), "Not enough tokens left to buy.");
    ...
    for (uint256 i = 0; i < number; i++) {
      ...
      _nftOfWallet[msg.sender].push(newItemId);
      _nftIndex[newItemId] = _nftOfWallet[msg.sender].length - 1;
    }
    return newItemId;
  }
