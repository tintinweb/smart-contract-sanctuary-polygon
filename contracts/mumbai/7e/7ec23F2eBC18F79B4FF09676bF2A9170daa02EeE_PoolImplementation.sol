...
contract PoolImplementation is PoolStorage, NameVersion {
    ...
    modifier onlyManager() {
        require(isPoolManager[msg.sender] , 'Pool: only manager');
        _;
    }
    ...
    function setPoolManager(address manager, bool isManager) public _onlyAdmin_ {
        require( manager != address(0) , "PoolImplementation.setPoolManager: manager must not zero address");
    ...
    function _transferIn(Data memory data, uint256 amount) internal {
        ...
        // }
        // else {
        IERC20(data.underlying).safeTransferFrom(msg.sender, address(this), amount);
