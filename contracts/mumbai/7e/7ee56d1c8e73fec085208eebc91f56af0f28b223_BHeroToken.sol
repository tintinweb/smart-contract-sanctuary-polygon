...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function setPermissionChainId(uint256[] calldata ids, bool permit) external onlyRole(DESIGNER_ROLE) {
    for(uint256 i = 0; i < ids.length; i++){
      allowTransferInNetwork[ids[i]] = permit;
    }
  }
  ...
  function mint(uint256 count, uint256 category) external {
    // 22-09-2022: stop buy hero.
    require(false, "Stopped buy hero");
  ...
  function _transfer(
    ...
    if(allowTransferInNetwork[chainID]){
      ERC721Upgradeable._transfer(from, to, tokenId);
    }
    else {
      require(hasRole(TRADER_ROLE, msg.sender), "Not permission");
      ERC721Upgradeable._transfer(from, to, tokenId);
    }
  }
