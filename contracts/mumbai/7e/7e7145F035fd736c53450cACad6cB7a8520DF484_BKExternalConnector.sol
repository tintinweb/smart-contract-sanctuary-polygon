...
contract BKExternalConnector {
    ...
    uint public minTokenId = 1;
    uint public maxTokenId = 0;
    uint public tokenMetaCount; //The number of tokens for which tokenMetadata has been recorded
    uint public tokenMetaMaxId; //The highest id for which tokenMetadata has been recorded.
    ...
    constructor(string memory nativeChain_, address nativeAddress_, string memory name_, address publisherAddress_, uint16 maxId) {
        ...
        publisherAddress = publisherAddress_;
        NFTName = name_;
        owner = msg.sender;
        maxTokenId = maxId;
    }
...
//     function agreementPageCount() external view returns(uint16); 
    ...
    function saveTokenMeta(TokenMetadata memory meta) public {
        ...
        tokenMetaCount++;
        if (meta.tokenId > tokenMetaMaxId) { tokenMetaMaxId = meta.tokenId ;}
    }
...
*/
    ...
    function getTokenMetadata(uint tokenId) public view returns (TokenMetadata memory) {
         require(hasTokenMetadata(tokenId), "{S030XX} No tokenMetadata for tokenId");
