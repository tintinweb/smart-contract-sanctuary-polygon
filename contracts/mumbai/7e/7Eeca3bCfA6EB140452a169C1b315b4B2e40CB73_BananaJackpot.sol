...
abstract contract VRFConsumer is VRFConsumerBaseV2, Ownable {
    uint8 public constant NUMBER_OF_WORDS = 4;
...
contract BananaJackpot is VRFConsumer {
    ...
    constructor(
        ...
        uint8 _itemsPerReel,
        uint8 _jackpotPercentagePoints,
        ...
        jackpotPercentagePoints = _jackpotPercentagePoints;
    ...
    function spin() external {
        ...
        emit SpinInProgress(msg.sender, requestId, _costPerSpin, jackpot());
    }
    ...
    function setItemsPerReel(uint8 _itemsPerReel) external onlyOwner {
        if (_itemsPerReel < 10) revert NotEnoughItemsPerReel();
        itemsPerReel = _itemsPerReel;
    }

    function jackpot() public view returns (uint256) {
        uint256 balance = _banana.balanceOf(address(this));
        return _calculatePercentage(balance, jackpotPercentagePoints);
    }

    // function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
    //     uint256 winnings = 0;
    //     uint8[4] memory randomNumbers = [
    //         _randomWordToNumber(randomWords[0]),
    //         _randomWordToNumber(randomWords[1]),
    //         _randomWordToNumber(randomWords[2]),
    //         _randomWordToNumber(randomWords[3])
    //     ];

    //     // In order to hit the jackpot you must roll four sevens.
    //     if (
    //         randomNumbers[0] == 11
    //         && randomNumbers[1] == 9
    //         && randomNumbers[2] == 1 
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        uint256 winnings = 0;
        bytes4 randomNumbers = bytes4(abi.encodePacked(
            _randomWordToNumber(randomWords[0]),
            _randomWordToNumber(randomWords[1]),
            _randomWordToNumber(randomWords[2]),
            _randomWordToNumber(randomWords[3])
