...
// File: contracts/LuxOn/NewIERC721LUXON.sol


pragma solidity ^0.8.13;

interface IERC721LUXON {
    function mintLuxOn(address mintUser, uint32 quantity, uint32[] memory characterId) external returns(uint256);
}
...
// File: contracts/LuxOn/OnChainGachaMachine.sol
...
contract GachaMachine is Payback, SuperOperators, ReentrancyGuard {
    ...
    address private mintGoodsAddress;

    uint256 private mintPrice;
    uint8 private onceMintCount;

    uint32[] private tierRatio;
    uint32[] private gachaGradeRatio;
    uint256 private tierRatioSum;
    uint256 private gachaGradeRatioSum;

    mapping(uint8 => mapping(uint8 => uint32[])) private characterInfoTable;
    mapping(uint32 => CharacterData) private characterInfo;
    ...
    constructor(
        ...
        tierRatio = _tierRatio;
        gachaGradeRatio = _gachaGradeRatio;

        for (uint8 i = 0; i < _tierRatio.length; i++) {
            tierRatioSum += _tierRatio[i];
        }
        for (uint8 i = 0; i < _gachaGradeRatio.length; i++) {
            gachaGradeRatioSum += _gachaGradeRatio[i];
        }
    }
    ...
    function getTierRatio() public view returns (uint32[] memory) {
        return tierRatio;
    }

    function getGachaGradeRatio() public view returns (uint32[] memory) {
        return gachaGradeRatio;
    }

    function getCharacterInfo(uint32 characterId) public view returns (uint8, uint8) {
        return (characterInfo[characterId].tier, characterInfo[characterId].gachaGrade);
    }

    function getCharacterCount() public view returns (uint256) {
        uint256 characterCount = 0;
        for (uint8 i = 0; i < tierRatio.length; i++) {
            for (uint8 j = 0; j < gachaGradeRatio.length; j++) {
                characterCount += characterInfoTable[i][j].length;
            }
        }
        return characterCount;
    }

    function getCharacterCountByTireAndGachaGrade(uint8 _tier, uint8 _gachaGrade) public view returns (uint256) {
    ...
    function setOnceMintCount(uint8 _onceMintCount) external onlyOwner {
        onceMintCount = _onceMintCount;
    }
    ...
    function setTierRatio(uint32[] memory _tierRatio) external onlyOwner {
        tierRatio = _tierRatio;
        for (uint8 i = 0; i < _tierRatio.length; i++) {
            tierRatioSum += _tierRatio[i];
        }
    ...
    function setGachaGradeRatio(uint32[] memory _gachaGradeRatio) external onlyOwner {
        gachaGradeRatio = _gachaGradeRatio;
        for (uint8 i = 0; i < _gachaGradeRatio.length; i++) {
            gachaGradeRatioSum += _gachaGradeRatio[i];
        }
    ...
    function setCharacterInfo(uint8 _tier, uint8 _gachaGrade, uint32[] memory _characterIds) external onlyOwner {
        characterInfoTable[_tier][_gachaGrade] = _characterIds;
        CharacterData memory characterData = CharacterData(_tier, _gachaGrade);
        for (uint8 i = 0; i < _characterIds.length; i++) {
            characterInfo[_characterIds[i]] = characterData;
        }
    }

    //------------------ gacha ------------------//

    function gacha(address mintUser, uint32 quantity) external payable onlySuperOperator returns (uint256) {
        require(quantity <= onceMintCount, "Once Exceeded the limit");
        return mintPay(mintUser, quantity);
    }

    function withdrawMoney() external onlyOwner nonReentrant {
        (bool success) = IERC20(mintGoodsAddress).transferFrom(address(this), msg.sender, IERC20(mintGoodsAddress).balanceOf(address(this)));
        require(success, "Transfer failed.");
    }

    //------------------ private ------------------//

    function mintPay(address mintUser, uint32 quantity) private returns (uint256) {
        uint256 price = getMintPrice() * quantity;

        require(IERC20(mintGoodsAddress).balanceOf(mintUser) >= price, "Need to send more mint goods.");
        IERC20(mintGoodsAddress).transferFrom(mintUser, address(this), price);
        mintPayback(mintUser, price);
        uint32[] memory gachaIds;
        for (uint8 i = 0; i < quantity; i++) {
            gachaIds[i] = characterGacha(i);
        }
        return IERC721LUXON(mintAddress).mintLuxOn(mintUser, quantity, gachaIds);
    }

    function mintPayback(address mintUser, uint256 price) private {
        paybackByMint(mintUser, price);
    }

    function characterGacha(uint8 seed) public view returns (uint32) {
        uint8 _tier = randomTier(seed);
        uint8 _gachaGrade = randomGachaGrade(_tier);
        return randomCharacterNum(_tier, _gachaGrade);
    }

    function randomTier(uint8 seed) public view returns (uint8 index) {
        uint ratio = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, seed))) % tierRatioSum;
        index = 0;
        uint ratioSum = 0;
        for (uint8 i = 0; i < tierRatio.length; i++) {
    ...
    function randomGachaGrade(uint8 _tier) public view returns (uint8 index) {
        uint ratio = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, _tier))) % gachaGradeRatioSum;
        index = 0;
        uint ratioSum = 0;
        for (uint8 i = 0; i < gachaGradeRatio.length; i++) {
            ratioSum += gachaGradeRatio[i];
            if (ratio <= ratioSum) {
                break;
            }
            index++;
    ...
    function randomCharacterNum(uint8 _tier, uint8 _gachaGrade) public view returns (uint32) {
        uint256 characterCount = characterInfoTable[_tier][_gachaGrade].length;
        uint index = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, characterCount))) % characterCount;
