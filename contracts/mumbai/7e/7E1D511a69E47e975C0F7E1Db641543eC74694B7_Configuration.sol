...
contract Configuration is IConfig {
    ...
    uint public tokenStatisticsVersionCounter;

    struct TokenStatistics {
        mapping(uint => uint) juryTokensSharePerStrike;
    }
    ...
    mapping(uint => TokenStatistics) internal tokenStatisticsVersion;
    ...
    function setJuryTokensSharePerStrike(uint[] memory strike_,uint[] memory requiredShare_) external returns(bool) {
        tokenStatisticsVersionCounter += 1;
        for(uint i=0;i<strike_.length;i++) {
            tokenStatisticsVersion[tokenStatisticsVersionCounter].juryTokensSharePerStrike[strike_[i]] = requiredShare_[i];
        }        
    ...
    function getJuryTokensShare(uint strike_,uint version_) external view override returns(uint) {
        return tokenStatisticsVersion[version_].juryTokensSharePerStrike[strike_];
    }
    ...
    function setWalletAddress(address developer_,address escrow_) external isAdmin override returns(bool) {
    ...
    function getFeeDeductionConfig() public override view returns(uint,uint,uint,uint,uint,uint,uint,uint,uint,uint) {
