...
contract Administrator is AccessControl, ERC2771Context {
   ...
   */
  constructor(INgageN _ngagenAddress, CustomForwarder _forwarder)
  ...
  /**
   * @dev Public function to mint a single new token and approve ADMN contract
  ...
  function mintAndApproveNFT(
    ...
    require(
      ...
      'ADMN: Only CREATOR with valid signature'
    );
  ...
  /**
   * @dev Public function to mint new tokens in batches and approve them to the ADMN contract
   * Reverts if the caller is not trusted forwarder contract
...
contract CustomForwarder is EIP712, AccessControl {
    ...
    modifier onlyRelayer() {
        require(hasRole(RELAYER, msg.sender), "CF: Only relayer");
        _;
    }
    ...
    {
        require(verify(req, signature), "CF: signature does not match request");
