...
contract XPNft1155OpenSea is ERC1155, Ownable, BridgeNFTBatch, ERC2981, DefaultOperatorFilterer {
    ...
    modifier onlyAdmin {
        require(msg.sender == _admin, "You're not an admin. Unauthorised funciton call");
        _;
    }

    modifier onlyMinter {
        require(msg.sender == _minter, "You're not a Minter. Unauthorised funciton call");
        _;
    }
	...
	function mint(address to, uint256 id, bytes calldata rawInfo) override external onlyMinter {
		if (rawInfo.length != 0) {
	...
	function mintBatch(address to, uint256[] calldata ids, uint256[] calldata amounts, bytes calldata rawInfo) override external onlyMinter {
		if (rawInfo.length != 0) {
			SetRoyalty[] memory setRoyaltyInfo = abi.decode(rawInfo, (SetRoyalty[]));
    ...
    function burnBatchFor(address from, uint256[] calldata ids, uint256[] calldata amounts) override external onlyMinter {
    ...
    function updateURI(string memory newURI) external onlyAdmin {
        _baseURI = newURI;
    }
