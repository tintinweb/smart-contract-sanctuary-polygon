...
contract BetHistory is IBetHistory, AccessHandler {
        ...
        onlyRole(BETTER_ROLE)
    {
        ...
        uint256 pot = betAmount.mul(bet.decimalOdds).div(BetData.ODDS_PRECISION);
        uint256 matchedAmount = pot.sub(betAmount);
        ...
        // market hash => tokenAdd => amount betted
        marketBetted[bet.marketHash][bet.token] = marketBetted[bet.marketHash][bet.token].add(betAmount);
        ...
        returns (BetData.BetSettleResult memory result)
    {
        ...
        uint256 betAmount = bet.amount;
        console.log("potSize:", potSize.div(1 ether), "amount:", betAmount.div(1 ether));
        uint256 lockedAmount =
            betLockBox.getLockedAmount(result.better, result.tokenAdd);
        console.log("locked:", lockedAmount.div(1 ether));
        if (lockedAmount == 0 || potSize == 0) {
            return result;
        }

        console.log("Unlock for better ....", betAmount.div(1 ether));
        ...
        uint256 matchedAmount = potSize.sub(betAmount);
        ...
        if (marketHistory.isMarketOutcome(
            ...
            uint256 profit = potSize.sub(betAmount);
            result.paidToFee = profit.mul(feePermille).div(1000);
            result.paidToBetter = potSize.sub(result.paidToFee);
        ...
        if (marketHistory.isMarketOutcome(
            ...
            result.paidToLP = matchedAmount.div(2);
            uint256 profit = potSize.sub(betAmount).div(2);
            result.paidToFee = profit.mul(feePermille).div(1000);
            result.paidToBetter =
                potSize.sub(result.paidToFee).sub(result.paidToLP);
            // Add the fee to the common reward pools
            _payFees(result.better, result.tokenAdd, result.paidToFee);
            // Pay the better
            _payBetter(result.better, result.tokenAdd, result.paidToBetter);
        ...
        if (marketHistory.isMarketOutcome(
            ...
            result.paidToBetter = betAmount.div(2);
            result.paidToLP = potSize.sub(result.paidToBetter);
            // Pay the better
            _payBetter(result.better, result.tokenAdd, result.paidToBetter);
            // Return to LP
            _payLP(result.tokenAdd, result.paidToLP);
        ...
        returns (BetData.BetSettleResult memory result)
    {
        ...
        uint256 potSize = unsettledPots[betHash];
        uint256 matchedAmount = potSize.sub(betAmount);
        marketBetted[bet.marketHash][bet.token] =
            marketBetted[bet.marketHash][bet.token].sub(betAmount);
        marketMatched[bet.marketHash][bet.token] =
            marketMatched[bet.marketHash][bet.token].sub(matchedAmount);
        unsettledPots[betHash] = 0;
        ...
        private
    {
        if (feePermille > 0) {
            ...
            if (feeSplitPercent1 > 0) {
                ...
                uint256 fee1 = inAmount.mul(feeSplitPercent1).div(100);
                feeRewards1.addRewards(inBetter, fee1);
            ...
            if (feeSplitPercent2 > 0) {
                // Add the fee to the common reward pool
                uint256 fee2 = inAmount.mul(feeSplitPercent2).div(100);
                feeRewards2.addRewards(inBetter, fee2);
                // Transfer the fee to the reward handler
                bool success = transferViaProxy(
                    address(betLockBox),
                    tokenAdd,
