...
contract TokenLockerContract is ERC721Holder, Context, ReentrancyGuard{
    ...
    IMarketplace public Marketplace; // Main Contract address for the IMarketplace interface
    ...
    modifier isProtocol(){
        bool isProtocolSupported = Marketplace.isSupportedProtocol(_msgSender());
    ...
    modifier isSupportedCollection(address _collection){
        require(Marketplace.getCollectionData(_collection).collectionAddress != address(0));
        _;
    }
    ...
    function _deposit(address _collection, uint[] calldata _tokens, address _ownerOf) private{
        IERC721 nft = IERC721(_collection);
        uint length = _tokens.length;
        for(uint i; i < length;){ 
            uint tokenId = _tokens[i];
    ...
    function liquidate(bytes32 _lockId, address _recipient) external nonReentrant isProtocol{
        LockedTokens storage lockTokens = lockedTokens[_lockId];
    ...
    function updateMarketplace(address _Marketplace) external isAdmin{
        Marketplace = IMarketplace(_Marketplace);
    }
