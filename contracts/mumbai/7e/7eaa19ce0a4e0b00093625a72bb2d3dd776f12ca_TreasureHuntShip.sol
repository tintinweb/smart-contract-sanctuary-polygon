...
contract TreasureHuntShip is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function burnShips(uint256[] memory tokenIDs) public {
        for (uint256 i = 0; i < tokenIDs.length; i++){
            ...
            _burn(tokenIDs[i]);
            if (isActiveShip[tokenIDs[i]]) {
                tokenIndex[activeShipsByOwner[msg.sender][activeShipsByOwner[msg.sender].length - 1]] = tokenIndex[tokenIDs[i]];
                activeShipsByOwner[msg.sender][tokenIndex[tokenIDs[i]]] = activeShipsByOwner[msg.sender][activeShipsByOwner[msg.sender].length - 1];
                activeShipsByOwner[msg.sender].pop();
            } else {
