...
contract PledgeStakeV2 {
    ...
    function _stake(address _user, uint256 _amount) private {
        ...
        uint256 directReward = _amount.mul(directRewardPercentage).div(100);
    ...
    function _updateRewards(address _user, uint256 _amount) private {
        ...
        for (uint256 i = lastProcessedStakeIndex[_user]; i < stakes[_user].length; i++) {
            StakeInfo storage userStake = stakes[_user][i];
            uint256 roundNumber = (block.timestamp - userStake.startTime) / roundDuration;

            if (block.timestamp >= userStake.startTime + (roundDuration * (roundNumber + 1))) {
                if (roundNumber < totalRounds) {
                    uint256 roundReward = (_amount * roundRewardPercentages[roundNumber]) / 100;
                    rewards[_user].roundRewards = rewards[_user].roundRewards.add(roundReward);

                    while (upline != address(0) && userInfo[upline].totalReferrals >= 2) {
                        uint256 level = userInfo[upline].level;
                        uint256 levelReward = (roundReward * levelPercentage[level - 1]) / 100;
        ...
        lastProcessedStakeIndex[_user] = stakes[_user].length;
    }
