 ...
 * and maintains the state of Buyout, TokenSaleStatus, FakeAltroFractionsSale, ClosingTimeForTokenSale.
 */

contract FakeAltroFractions is AccessControl, ERC1155Burnable, ERC1155Supply, IFractions, Initializable {
	...
	function setContractSale(address contractSale_) external initializer onlyRole(DEFAULT_ADMIN_ROLE) {
		require(contractSale_ != address(0), "FakeAltroFractions: cannot be null address");
		require(contractSale_.supportsInterface(type(IFractionsSale).interfaceId), "FakeAltroFractions: does not support IFractionsSale interface");
	...
	function setClosingTimeForTokenSale(uint256 tokenId, uint256 closingTime) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(closingTime > block.timestamp, "FakeAltroFractions: closing time cannot be set in the past");
		...
		if (isTokenSaleClosed(tokenId)) {
			if (tokenSaleStatus[tokenId] == TokenSaleStatus.SUCCESSFUL) {
				require(
					...
					"FakeAltroFractions: cannot transfer bought out token id"
				);
			} else if (tokenSaleStatus[tokenId] == TokenSaleStatus.FAILED) {
				require(hasRole(BURN_MANAGER_ROLE, operator) && to == address(0), "FakeAltroFractions: cannot trade token whose sale failed");
			} else {
	...
	function uri(uint256 id) public view override returns (string memory) {
		require(exists(id), "FakeAltroFractions: non existent token");
