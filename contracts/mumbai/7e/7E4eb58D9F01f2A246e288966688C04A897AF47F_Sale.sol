	...
	ReentrancyGuardUpgradeable
{
	...
	event AddUsersToVesting(uint256,uint256,address,uint256[],uint256[]);
	...
	function buyToken(uint256 _tokenAmount, uint256 _phaseIndex)
		external
		payable
	...
	function buyTokenWithBUSD(uint256 _tokenAmountDesired, uint256 _phaseIndex) public isPublicSale(_phaseIndex)
		nonReentrant {
		uint256 finalPrice = _tokenAmountDesired.mul(phases[_phaseIndex].price).div(10**18);
		require(
			block.timestamp < phases[_phaseIndex].endAt,
		...
		require(
			phases[_phaseIndex].supply >= _tokenAmountDesired,
			"Not enough supply"
		);

		require(
			_tokenAmountDesired >= phases[_phaseIndex].minimunEntry,
			"There are too few tokens"
		);
		...
		emit Purchase(msg.sender, _tokenAmountDesired, id,_phaseIndex);
	}
	...
	function addUsersToVesting(uint256 _phaseIndex,uint256 _tokenAmount,address _address,uint256[] memory _timesToRelease,
		uint256[] memory _percentsToRelease) external onlyRole(DEFAULT_ADMIN_ROLE) {
			require(
			block.timestamp < phases[_phaseIndex].endAt,
			"This phase is over, wait for the next"
		);
		...
		require(
			...
			tokenLocksForSale[id].initAmount = _tokenAmount;
			tokenLocksForSale[id].remainAmount = _tokenAmount;
			getTimeToPresent(_timesToRelease);
			tokenLocksForSale[id].percentsToRelease = _percentsToRelease;
			phases[_phaseIndex].supply -= _tokenAmount;
		...
		tokensRemainForSale -= _tokenAmount;
		emit AddUsersToVesting(_phaseIndex,_tokenAmount,_address,_timesToRelease,_percentsToRelease);
	}
