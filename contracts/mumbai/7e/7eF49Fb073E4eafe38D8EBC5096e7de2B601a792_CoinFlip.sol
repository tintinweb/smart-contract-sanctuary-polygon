    ...
    ERC2771Context
{
        ...
        VRFManager(_oracleContract)
    {
        ...
        uint256 i;
        for (; i < _gameManagerConstructorArgs.approvedTokens.length; ) {
            require(
                treasuryContract.isTokenApproved(
                    _gameManagerConstructorArgs.approvedTokens[i]
                ),
                "coinFlipConstructor: token not approved by treasury"
            );
    ...
    function startGame(
        ...
        initializeGameData(sender, _token, _ante, _guess);
    ...
    function fulfillRandomWords(
        ...
        uint256 modifiedRandomNumber = (_randomWords[0] % 1000);
    ...
    function _endGame(uint256 _gameId, uint256 _randomNumber) private {
        ...
        if (gameData[_gameId].chosen == gameData[_gameId].outcome) {
            ...
            if (
                ...
                // payout 1:1 to player, return balance to house
                payout(
                    PayoutArgs({
                        _player: gameData[_gameId].player,
                        _tokenContract: gameData[_gameId].anteTokenContract,
                        _playerReleaseAmount: gameData[_gameId].ante,
                        _houseReleaseAmount: gameData[_gameId].ante *
                            RESERVE_RATIO,
                        _playerPayout: (gameData[_gameId].ante * 2),
                        _housePayout: (gameData[_gameId].ante * RESERVE_RATIO) -
                            gameData[_gameId].ante
                    })
                );
            } else {
                // payout 6:1 to player
                payout(
                    PayoutArgs({
                        _player: gameData[_gameId].player,
                        _tokenContract: gameData[_gameId].anteTokenContract,
                        ...
                        _houseReleaseAmount: gameData[_gameId].ante *
                            RESERVE_RATIO,
                        _playerPayout: (gameData[_gameId].ante +
                            gameData[_gameId].ante *
                            RESERVE_RATIO),
                        _housePayout: 0
                    })
            ...
            payout(
                PayoutArgs({
                    ...
                    _tokenContract: gameData[_gameId].anteTokenContract,
                    _playerReleaseAmount: gameData[_gameId].ante,
                    _houseReleaseAmount: gameData[_gameId].ante * RESERVE_RATIO,
                    _playerPayout: 0,
                    _housePayout: gameData[_gameId].ante * (RESERVE_RATIO + 1)
                })
            );
        }
    ...
    function addApprovedToken(address _token)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        require(
            treasuryContract.isTokenApproved(_token),
            "addApprovedToken: approved token must also be approved on the treasury"
        );
        super.addApprovedToken(_token);
    }
    ...
    function removeApprovedToken(address _token)
        ...
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.removeApprovedToken(_token);
    }
    ...
    function setMaximumAnte(address _token, uint256 _minimumAnte)
        public
        override
        onlyRole(GAME_MANAGER_ROLE)
    {
        super.setMaximumAnte(_token, _minimumAnte);
    }
...
contract TreasuryManager {
    ...
    function holdFundsToStartGame(
        ...
        treasuryContract.debitHouseBalance(
            _tokenContract,
            _ante * RESERVE_RATIO
        );
        treasuryContract.creditHouseHold(_tokenContract, _ante * RESERVE_RATIO);
    }

    function payout(PayoutArgs memory _payoutArgs) internal {
        require(
            treasuryContract.houseHolds(_payoutArgs._tokenContract) >=
                _payoutArgs._houseReleaseAmount,
        ...
        treasuryContract.creditHouseBalance(
            _payoutArgs._tokenContract,
            _payoutArgs._housePayout
        );

        uint256 totalReleased = _payoutArgs._houseReleaseAmount +
            _payoutArgs._playerReleaseAmount;
        uint256 totalPayout = _payoutArgs._housePayout +
            _payoutArgs._playerPayout;
        ...
        require(
            _payoutArgs._playerReleaseAmount != 0,
...
contract GameManager {
    ...
    event MinimumAnteUpdated(address _tokenContract, uint256 _minimumAnte);
    event MaximumAnteUpdated(address _tokenContract, uint256 _maximumAnte);
    event GameEnabled();
    event GameDisabled();
    ...
    constructor(GameManagerConstructorArgs memory _gameManagerConstructorArgs) {
        ...
        require(
            ...
            "gameManagerConstructor: array length mismatch"
        );
        require(
    ...
    function initializeGameData(
        ...
        require(
            ...
            "startGame: token must be approved for this game"
        );
        require(
            _ante >= minimumAnte[_token],
            "startGame: ante less than token requirement"
        );
        require(
            _ante <= maximumAnte[_token],
            "startGame: ante greater than token requirement"
        );
        require(
        ...
        g.player = _player;
        g.ante = _ante;
        g.anteTokenContract = _token;
        g.chosen = _guess;
        playerInGame[_player] = gameCounter;
    }

    function addApprovedToken(address _token) public virtual {
        approvedTokens[_token] = true;
    }

    function removeApprovedToken(address _token) public virtual {
        ...
        virtual
    {
        ...
        emit MinimumAnteUpdated(_token, _minimumAnte);
    }
        ...
        virtual
    {
        ...
        emit MaximumAnteUpdated(_token, _maximumAnte);
    }
