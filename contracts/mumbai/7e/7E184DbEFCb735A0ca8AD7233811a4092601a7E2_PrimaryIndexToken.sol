...
contract PrimaryIndexToken is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable
{
    ...
    function getCollateralAvailableToWithdraw(address account, address projectToken, address lendingToken) public view returns(uint collateralProjectToWithdraw) {
        ...
        // 13919311 *100 *1e18/663456/54 = 38851863351679466086
        ...
        DepositPosition storage _depositPosition = depositPosition[account][projectToken][usdcToken];
        uint256 depositedProjectTokenAmount = _depositPosition.depositedProjectTokenAmount;
        uint256 _totalOutstandingInUSD = lendingToken == address(0) ? 0 : totalEstimatedOutstandingInUSD(account, projectToken, lendingToken);
        //12236723 *100 *1e18/663456/54
        uint256 depositedAmountSatifyHF= depositedProjectTokenAmount - _totalOutstandingInUSD  * lvrDenominator * (10 ** ERC20Upgradeable(projectToken).decimals()) / getTokenEvaluation(projectToken, 10 ** ERC20Upgradeable(projectToken).decimals()) / lvrNumerator;
        collateralProjectToWithdraw =  collateralProjectRemaining > depositedAmountSatifyHF? 0: depositedAmountSatifyHF;
    }

    function getCollateralAvailableToWithdrawTest(address account, address projectToken, address lendingToken) public view returns(uint collateralProjectToWithdraw) {
        // (uint256 lvrNumerator, uint256 lvrDenominator) = getLoanToValueRatio(projectToken, lendingToken);
        // uint256 depositRemaining = pitRemaining(account, projectToken, lendingToken);  // 
        // // 13919311 *100 *1e18/663456/54 = 38851863351679466086
        // uint256 collateralProjectRemaining = depositRemaining * lvrDenominator * (10 ** ERC20Upgradeable(projectToken).decimals()) / getTokenEvaluation(projectToken, 10 ** ERC20Upgradeable(projectToken).decimals()) / lvrNumerator;
