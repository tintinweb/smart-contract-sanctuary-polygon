...
contract MixieNftSaleIntegratedContractTest4 is 
ERC1155Supply, ERC2981ContractWideRoyalties, ReentrancyGuard, EmergencyPausable {
    ...
    uint constant units = 1; //FOR TESTING ONLY - COMMENT THIS LINE OUT FOR PRODUCTION
    ...
    internal requiresUpdate view returns(uint) {
        require(addressHasType1Coupon(prospectiveBuyer, discountRate), string(abi.encodePacked(
    ...
    external pushesUpdate whenNotPaused nonReentrant {
		...
		for (uint i = 0; i < amount; i++) {
			...
			totalPrice += prices[i];
            useType1Coupon(msg.sender, discountRate[i]);
        }
