...
interface IMarketplace {
    ...
    function acceptBid(
        IMarketplace.Bid calldata _bid,
        bytes calldata _signature
    ) external;
    ...
    OwnableUpgradeable
{
    ...
    function initialize(address _payee, address _wrappedToken) public initializer {
    ...
    function acceptBid(
        ...
        require(bidder == _bid.bidder, "Invalid signature");
        require(_bid.seller == msg.sender, "Seller not match");
        require(
            IERC721(_bid.tokenAddress).ownerOf(_bid.tokenId) == msg.sender,
            "You not own nft"
        );

        IERC721(_bid.tokenAddress).transferFrom(
            msg.sender,
            bidder,
            _bid.tokenId
        );

        uint256 marketRevenue = _bid.price.mul(marketPercent).div(10000);

        // pay for _seller
        wrappedToken.transferFrom(
            bidder,
            _bid.seller,
            _bid.price.sub(marketRevenue)
        );

        // pay for _market
        wrappedToken.transferFrom(bidder, marketPayee, marketRevenue);

        // pay for _royalty: TODO
        emit NFTBought(
            bidder,
            _bid.seller,
            _bid.tokenAddress,
    ...
    function _hashBid(IMarketplace.Bid calldata _bid)
        private
        view
        returns (bytes32)
    {
        return
          _hashTypedDataV4(
              keccak256(
                  abi.encode(
                      keccak256(
                          "BidParams(address tokenAddress,uint256 tokenId,uint256 price,address bidder,address seller,uint256 expiresAt)"
                      ),
                      _bid.tokenAddress,
                      _bid.tokenId,
                      _bid.price,
