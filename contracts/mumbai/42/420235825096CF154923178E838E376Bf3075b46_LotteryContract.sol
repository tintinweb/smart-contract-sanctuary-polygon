...
contract LotteryContract {
    ...
    function pickWinner() public onlyAdmin {
        ...
        address winner = players[index]; //Get the winner address
        emit Winner(winner); //Emit the event that a winner was picked

        if (nftContract != address(0)) {
            ERC721Base(nftContract).transferFrom(address(this), winner, tokenId);
        }
    ...
    function getNFTContract() public view returns (address) {
        return nftContract; //Return the NFT contract address
    }
