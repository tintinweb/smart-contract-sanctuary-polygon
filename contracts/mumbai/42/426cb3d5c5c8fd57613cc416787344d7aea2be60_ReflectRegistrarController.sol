...
contract ReflectRegistrarController is Ownable {
    ...
    event SubdomainFee(bytes32 indexed tld, uint indexed SubdomainFee, bytes32 indexed label, address payment);
    event TldOwnerFee(bytes32 indexed tld, uint indexed tldOwnerFee, bytes32 indexed label, address payment);
    ...
    function registerWithConfig(string memory name, string memory tld, address owner, uint duration, address payment, address resolver, address addr) public {
        ...
        emit TldOwnerFee(StringToBytes32.stringToBytes32(tld), tldOwnerFee, StringToBytes32.stringToBytes32(name), payment);
        emit SubdomainFee(StringToBytes32.stringToBytes32(tld), cost, StringToBytes32.stringToBytes32(name), payment);
