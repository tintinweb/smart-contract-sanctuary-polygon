...
contract TheCryptoStudio is ERC1155
{
    ...
    event tokenDeclaredEvent(address creator,uint256 tokenID,uint256 timestamp);
    ...
    function declareNFT(string memory name , string memory imageCID , string memory animationCID , string memory description ,string memory spaceName, uint256 mintPrice, bool token_type, uint256 maxSupply) public   {
        ...
        emit tokenDeclaredEvent(msg.sender, tokenID.current(),block.number);
    }
