...
interface IERC2981Royalties {
    ...
    function xrRoyaltyInfo(uint256 _tokenId, uint256 _value)
        external
        view
        returns (address _receiver, uint256 _royaltyAmount);
}
...
// Metarix NFT lanchpad smartcontract
contract MetarixLaunchpadNFT is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC2981Base { 
    ...
    // mapping xrRoyalties
    mapping(uint256 => XrRoyaltyInfo) internal _xrRoyalties;
    ...
    function mint(uint256 amount, uint256 royalty, uint256 xrRoyalty, string memory uri)
        public
        whenNotPaused
    {   
        ...
        _setTokenXrRoyalty(tokenIDs, msg.sender, xrRoyalty);
        _mint(msg.sender, tokenIDs, amount, "0x00");
    }
    
    // mint batch items
    function mintBatch(uint256[] memory amounts, uint256 royalty, uint256 xrRoyalty, string[] memory uri)
        public
        whenNotPaused
    {
    ...
    function _setTokenXrRoyalty(
        uint256 tokenId,
        address recipient,
        uint256 value
    ) internal {
        require(value <= 10000, 'ERC2981Royalties: Too high');
        _xrRoyalties[tokenId] = XrRoyaltyInfo(recipient, uint24(value));
    }
    ...
    function xrRoyaltyInfo(uint256 tokenId, uint256 value)
        external
        view
        override
        returns (address receiver, uint256 royaltyAmount)
    {
        XrRoyaltyInfo memory xrRoyalties = _xrRoyalties[tokenId];
        receiver = xrRoyalties.recipient;
        royaltyAmount = (value * xrRoyalties.amount) / 100;
    }
