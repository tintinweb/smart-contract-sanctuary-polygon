...
contract Economics is IEconomics, AuthModifiers, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    uint32 public spentFuelTicketCount;
    uint256 public spentFuel;
    SpentFuelDestination[] public spentFuelDestinations;
    ...
    function __Economics_init(
        ...
        SpentFuelDestination[] calldata _spentFuelDestinations
    ) public initializer {
        __Ownable_init();
        __AuthModifiers_init(_registry);
        __Economics_init_unchained(_fuelToken, _basicTaxRate, _spentFuelDestinations);
    }
    ...
    function __Economics_init_unchained(
        ...
        for (uint256 i = 0; i < _spentFuelDestinations.length; i++) {
            spentFuelDestinations.push(_spentFuelDestinations[i]);
        }
    }
    ...
    @param _amount amount of fuel tokens that will be topped up
    ...
    function topUpIntegrator(
        ...
        require(_amount > 0, "Economics: zero amount");
        require(_price > 0, "Economics: incorrect price");
        require(fuelToken.allowance(_sender, address(this)) >= _amount, "Economics: sender lacks allowance");

        bool topUpFuel = fuelToken.transferFrom(_sender, address(this), _amount);
        require(topUpFuel, "Economics: transfer failed! Perhaps balance might be too low");

        uint256 _newAveragePrice = _calculateAveragePrice(_amount, _price, integrator.availableFuel, integrator.price);
        integrator.availableFuel += _amount;
        integrator.price = (_newAveragePrice);

        emit IntegratorToppedUp(_integratorIndex, _amount, _price, _newAveragePrice);
        ...
        returns (uint256)
    {
        ...
        if (!integrator.isBillingEnabled) {
            return 0;
        }
        ...
        uint256 _min = uint256(integrator.rates.minFee) * 1e15;
        uint256 _max = uint256(integrator.rates.maxFee) * 1e15;
        ...
        // Full workings:
        //
        // _fuelUsd[1e18] = (basePrice[1e3] * 1e15) * (mintRate[1e6] / 1e6);
        //   => _fuelUsd[1e18] = (basePrice[1e3] * 1e9) / mintRate[1e6];
        //
        // _fuel[1e18] = (_fuelUsd[1e18] * 1e18) / price[1e18];
        for (uint256 i = 0; i < _ticketActions.length; i++) {
            uint256 _fuelUsd = uint256(_ticketActions[i].basePrice) * integrator.rates.mintRate * 1e9;
        ...
        emit FuelReserved(integrator.index, uint32(_ticketActions.length), _fuel);
        return _fuel;
    }

    /** @notice tax a relayer for a basic action
    @param _relayerAddress - relayer address of an integrator
    */
    function spendBasicAction(address _relayerAddress, uint32 _ticketCount) external onlyEvent returns (uint256) {
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        if (!integrator.isBillingEnabled) {
            return 0;
        }

        uint256 _fuelToSpend = (basicTaxRate * _reservedFuelPerTicket(integrator.index) * _ticketCount) / 1e6;

        integrator.reservedFuel -= _fuelToSpend;
        spentFuel += _fuelToSpend;

        emit BasicTaxCharged(integrator.index, _ticketCount, _fuelToSpend);
        return _fuelToSpend;
    }

    /** @notice spends a relayer's reserved fuel for a number of tickets
    @param _relayerAddress - relayer address of an integrator
    */
    function spendTicketReserved(address _relayerAddress, uint32 _ticketCount) external onlyEvent returns (uint256) {
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        if (!integrator.isBillingEnabled) {
            return 0;
        }

        uint256 _fuelToSpend = _reservedFuelPerTicket(integrator.index) * _ticketCount;
        ...
        require(integrator.activeTicketCount >= _ticketCount, "Economics: spending more tickets than active");

        integrator.reservedFuel -= _fuelToSpend;
        spentFuel += _fuelToSpend;
        integrator.activeTicketCount -= _ticketCount;
        spentFuelTicketCount += _ticketCount;

        emit TicketFuelEmptied(integrator.index, _ticketCount, _fuelToSpend);
        return _fuelToSpend;
    }

    /**
    @notice moves GET from the spent fuel balance to the destination addresses
    @dev this function can be called by anyone
     */
    function collectSpentFuel() external nonReentrant {
        require(spentFuel > 0, "Economics: nothing to collect");
        require(fuelToken.balanceOf(address(this)) >= spentFuel, "Economics: insufficient token balance");

        // State-change before transfer to cover reentrancy.
        uint256 _spentFuel = spentFuel;
        uint32 _spentFuelTicketCount = spentFuelTicketCount;
        spentFuel = 0;
        spentFuelTicketCount = 0;

        for (uint256 i = 0; i < spentFuelDestinations.length; i++) {
            uint256 _collectionAmount = (_spentFuel * spentFuelDestinations[i].percentage) / 1e6;
            require(
                fuelToken.transfer(spentFuelDestinations[i].destination, _collectionAmount),
                "Economics: fuel token transfer failed"
            );
            emit SpentFuelCollected(spentFuelDestinations[i], _spentFuel, _collectionAmount, _spentFuelTicketCount);
        }
    }

    /** 
    @param _relayerAddress an address of the integrator in question
    @param _newBalance the correct/intended balance of the silo in GET 
    @notice the spentFuel balance will be used as 'counter post' 
     */
    function correctAccountBalance(
        ...
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        uint256 _oldBalance = integrator.availableFuel;
        uint256 _difference;

        // calculate the difference between new and old, to correct the spentFuel balance.
        // Could be positive and negative
        if (_newBalance > _oldBalance) {
            // Process refund to relayer
            // _difference is negative so remove from the spentFuel by getting the positive difference.
            _difference = (_newBalance - _oldBalance);
            require(_difference < spentFuel, "Economics: not enough spent fuel");
            spentFuel = spentFuel - _difference;
        } else {
            // Process balance correction to spent fuel
            // _difference is positive, reverse _old + _new to get positive difference and add.
            spentFuel = spentFuel + (_oldBalance - _newBalance);
        }

        integrator.availableFuel = _newBalance;

        emit AccountBalanceCorrected(integrator.index, _oldBalance, _newBalance, _difference);
    }
    ...
    */
    ...
    function setupIntegrator(
        ...
        integrator.rates = _dynamicRates;
    ...
    function disableIntegrator(address _relayerAddress) external onlyIntegratorAdmin {
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        integrator.isConfigured = false;
        integrator.isBillingEnabled = false;
        emit IntegratorDisabled(integrator.index);
    }
    ...
    /**
    @param _relayerAddress a relayer address of the integrator the new relayerRateStruct belongs to
    @param  _dynamicRates array containing all the dyanmic rates
     */
    function updateDynamicRates(address _relayerAddress, DynamicRates calldata _dynamicRates)
        external
        onlyIntegratorAdmin
    {
        integratorData[relayerData[_relayerAddress].integratorIndex].rates = _dynamicRates;
        emit UpdateDynamicRates(relayerData[_relayerAddress].integratorIndex, _dynamicRates);
    }

    function enableIntegratorBilling(address _relayerAddress) external onlyIntegratorAdmin {
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        emit EnableIntegratorBilling(integrator.index);
        integrator.isBillingEnabled = true;
    }

    function disableIntegratorBilling(address _relayerAddress) external onlyIntegratorAdmin {
        IntegratorData storage integrator = integratorData[relayerData[_relayerAddress].integratorIndex];
        emit DisableIntegratorBilling(integrator.index);
        integrator.isBillingEnabled = false;
    }

    /** resets the spentFuel balance.
    @notice function is useful for if for whatever reason the balance doesn't reflecft what has been truely collected
     */
    function setBasicTaxRate(uint24 _basicTaxRate) external onlyOwner {
    ...
    function setSpentFuel(uint256 _spentFuel) external onlyOwner {
        require(_spentFuel > 0, "Economics: new balance invalid");
        emit UpdateSpentFuel(spentFuel, _spentFuel);
        spentFuel = _spentFuel;
    }

    function setSpentFuelDestinations(SpentFuelDestination[] calldata _spentFuelDestinations) external onlyOwner {
        emit UpdateSpentFuelDestinations(spentFuelDestinations, _spentFuelDestinations);

        delete spentFuelDestinations;
        for (uint256 i = 0; i < _spentFuelDestinations.length; i++) {
            spentFuelDestinations.push(_spentFuelDestinations[i]);
        }
    }
    ...
    function _reservedFuelPerTicket(uint32 _integratorIndex) internal view returns (uint256) {
        IntegratorData storage integrator = integratorData[_integratorIndex];
        return integrator.reservedFuel / integrator.activeTicketCount;
    }

    function viewIntegratorAveragePrice(uint32 _integratorIndex) external view returns (uint256) {
        return integratorData[_integratorIndex].price;
    }

    function viewIntegratorAvailableFuel(uint32 _integratorIndex) external view returns (uint256) {
        return integratorData[_integratorIndex].availableFuel;
    }
...
interface IEconomics {
    ...
    function spentFuelDestinations(uint256 _index) external returns (address payable, uint24);
...
interface IEventImplementation {
    ...
    struct InventoryFinancing {
        // the palletIndex (of PalletFactory) the inventory is collateraized on
        uint64 palletIndex;
        // address of the contract overseeing the collateraization
        address bondModerator;
        // bool that is true if an event/inventory is involved in financing
        bool isFinanced;
        // bool that codifies as per the agreement on the collatarization if pre-repayment primary sales are allowed
        bool primaryAllowed;
        // bool that codifies as per financing agreement if pre-repayment scanning is allowed
    ...
    event UpdateFinancing(InventoryFinancing indexed financing);
    ...
    function setFinancing(InventoryFinancing memory _financing) external;
}
