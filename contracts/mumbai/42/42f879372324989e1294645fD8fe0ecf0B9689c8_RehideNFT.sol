...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    function mintNFT(
        ...
        // Don't enforce min price to owner and high tier addresses
        ...
        if (mintFee > 0) {
            ...
            if (referrer != _msgSender()) {
                ...
                if (primaryReferrer != address(0)) {
                    ...
                    // TODO: Check it hasn't been more than 6 months since activation
    ...
    */
    function getNotesSharedWithAddressCount(address account) external view returns (uint256 notesCount) {        
        uint256[] memory tokenIds = getTokensSharedWithAddress(account);
        notesCount = tokenIds.length;
    } 

    /**
    * @dev Get all tokens shared with an address
    */
    function getTokensSharedWithAddress(address account) public view returns (uint256[] memory tokenIds) {
        uint256[] memory allTokenIds = _notesSharedWithAddress[account];
        uint256 tokenIdCount = 1;
        for (uint256 i = 0; i < allTokenIds.length; i++) {
            if (ownerOf(allTokenIds[i]) != address(0)) {
                tokenIds[tokenIdCount - 1] = allTokenIds[i];
                tokenIdCount++;
            }
        }
    }
