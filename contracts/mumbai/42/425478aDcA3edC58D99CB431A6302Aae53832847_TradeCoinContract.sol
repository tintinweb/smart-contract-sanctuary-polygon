    ...
    ITradeCoinContract
{
    ...
    modifier isLegalOwner(uint256 _tokenId) {
    ...
    function initiateCommercialTx(
        ...
        Documents memory _documents,
        bool _payInFiat
    ) external override onlyLegalOwner(msg.sender, _tokenId) {
    ...
    function unitConversion(
        ...
        require(_previousAmountUnit != _newUnite, "Invalid Conversion");
        require(
            _previousAmountUnit == tradeCoin[_tokenId].unit,
            "Invalid Match: unit"
        );

        tradeCoin[_tokenId].amount = _newAmount;
        tradeCoin[_tokenId].unit = _newUnite;
    ...
    function addTransformation(
        ...
        Documents memory _documents,
        string memory _geoLocation
    ) external override isLegalOwnerOrCurrentHandler(msg.sender, _tokenId) {
        require(
    ...
    function changeProductHandler(
        ...
        Documents memory _documents,
        string memory _geoLocation
    ) external override isLegalOwner(_tokenId) {
        require(
            _documents.docHash.length == _documents.docType.length,
            "Document hash amount must match document type"
        );
    ...
    function batchProduct(
        ...
        tempArray[_tokenIds.length] = _tokenIdCounter;
        ...
        nonReentrant
    {
        ...
        // else transfer
        transferFrom(legalOwner, msg.sender, _tokenId);
    ...
    function massApproval(uint256[] memory _tokenIds, address to)
        external
        override
    {
    ...
    function bulkTransferFrom(
        ...
        for (uint256 i; i < lengthOfIds; ) {
            transferFrom(from, to, _tokenIds[i]);
