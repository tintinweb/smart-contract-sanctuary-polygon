    ...
    AccessControlUpgradeable
{
    ...
    function otherParticipantBuy(
        ...
        Paza.buyWithApproval(postpaidUsdcWallet, address(this), _pazaAmount);
        Paza.safeTransfer(
            defiWallet,
     ...
     * @param _borrower array of borrowers address
    ...
    function loanTransferOfBlacklistedBank(
        ...
        for (uint i = 0; i < _borrower.length; i++) {
            ...
            // (, bytes memory data) = (pools[_blacklistedBank]).staticcall(
            ...
            //     )
            // );
            // LoanLib.Loan memory loanDetails = abi.decode(data, (LoanLib.Loan));
    ...
    function completeTenure(
        ...
        // (, bytes memory data) = (pools[_bank]).staticcall(
        //     abi.encodeWithSignature("getUserDetails()", _borrower, _loanId)
        // );
        // LoanLib.Loan memory loanDetails = abi.decode(data, (LoanLib.Loan));
    ...
    // function borrowerUnstake(address _bank, uint _loanId) external {
    //     (uint rewardAmount, uint stakedAmount) = IPool(pools[_bank]).unstake(
    //         _msgSender(),
    //         _loanId
    //     );
    //     emit borrowerUnstaked(
    //         _bank,
    //         _msgSender(),
    //         _loanId,
    //         rewardAmount,
    ...
    // function claimReward(address _bank, uint _laonId) external {
    //     uint rewardAmount = IPool(pools[_bank]).claimReward(
    //         _msgSender(),
    //         _laonId
    //     );
    //     emit RewardClaimed(_bank, _msgSender(), _laonId, rewardAmount);
    // }
...
interface IPool {
    ...
    // function unstake(address _borrower, uint256 _loanId) external returns(uint rewardAmount, uint stakedAmount);
    // function claimReward(address _borrower, uint _loanId) external returns (uint);
}
