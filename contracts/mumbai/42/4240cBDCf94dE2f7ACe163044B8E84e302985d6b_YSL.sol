    ...
    function tax(uint256 taxAmount) internal {
        address[] memory path = new address[](2);
            ...
            if(IUniswapV2Router02(router).getAmountsOut(balanceOf(address(this)),path)[1] >= YSL_ThresholdLimit){
                ...
                IERC20(BUSD).transfer(Admin.Treasury(), (convertedBUSD * YSL_Tax_Allocation[1])/(YSL_Tax * 2));
                IERC20(BUSD).transfer(Admin.TeamAddress(), (convertedBUSD * YSL_Tax_Allocation[1])/(YSL_Tax * 2));
            }
