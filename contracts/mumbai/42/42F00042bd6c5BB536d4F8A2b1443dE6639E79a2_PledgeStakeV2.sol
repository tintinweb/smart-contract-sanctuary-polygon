    ...
    function _updateRewards(address _user) private {
        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            ...
            if (!stake.unstaked || stake.lastCalculatedRound < TOTAL_ROUNDS) {
                ...
                for (uint256 j = stake.lastCalculatedRound; j <= roundNumber && j < TOTAL_ROUNDS; j++) {
                    ...
                    for (uint256 k = 0; k < LEVEL_COUNT; k++) {
                        ...
                        else if ((upline == defaultReferrer || uplineLevel > 0) && uplineLevel >= k && userInfo[upline].totalStake > 0 ) {
                    ...
                    stake.lastCalculatedRound = j + 1; // Increase the last calculated round
                }
            }
        }
    }
