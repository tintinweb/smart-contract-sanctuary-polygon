...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
    ...
    function payNbirth(address user) internal returns(uint) {

        uint recParentIndx = nextMemberParentFill;
        ...
        if (autoPoolCycles[recMindex].isFullBirth){
            ...
            if (cycleOver(recMindex)){

                return 0;

            }else{

                 payNbirth(user); // recursive
            }
            ...
            // split rebirth
            if(syncCycle(recMindex)){

                // only run if cycle is not over
    ...
    function fullCycleOver(uint index) internal view returns(bool){
          ...
          if (autoPoolCycles[index].target==autoPoolCycles[index].achieved){
                ...
                if (autoPoolCycles[index].cycle==sysInfos.maxCycle){
    ...
    function syncCycle(uint recMindex) internal returns(bool) {
        ...
        if (autoPoolCycles[recMindex].target==autoPoolCycles[recMindex].achieved){
                ...
                if (autoPoolCycles[recMindex].cycle==sysInfos.maxCycle){
    ...
    function reBirthPosition(address _poolUser,uint _mIndex,uint _from, uint _to) internal {
        ...
        if(syncCycle(_mIndex)){

             syncIndex();          // update index

            
            autoPoolCycles[_mIndex].achieved++;
            autoPoolCycles[_mIndex].indexRebirth++;
    ...
    function payNbirth3x(address _user) internal {

        uint recParentIndx = nextMember3xParentFill;
