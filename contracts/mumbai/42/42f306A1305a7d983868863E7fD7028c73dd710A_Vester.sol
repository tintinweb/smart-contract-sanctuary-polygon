...
contract Vester {
    ...
    function _createVestingSchedule(
        ...
        for (uint256 i = 0; i < _receivers.length; i++) {
            ReceiverData storage r = receivers[vestingIds][_receivers[i]];
            r.amount = _amounts[i];
            receiversVesting[_receivers[i]].push(vestingIds);
            emit ReceiversData(
                vestingIds,
                _receivers[i],
                _amounts[i],
    ...
    function _getReleasableAmount(
        ...
        ReceiverData memory receiverData = receivers[_vestingId][_receiver];
        ...
        if ((currentTime < (vestingSchedule.start + vestingSchedule.cliff))) {
            ...
            uint256 releasableAmount = (receiverData.amount * vestedSeconds) /
