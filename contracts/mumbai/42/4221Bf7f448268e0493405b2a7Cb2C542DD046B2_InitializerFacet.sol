...
import {IERC173} from "../../external/diamond/interfaces/IERC173.sol";
...
contract InitializerFacet {
    ...
    modifier initializer() {
        LibInitializable.DiamondStorage storage ds = LibInitializable
            .diamondStorage();
        require(!ds.initialized, "InitializableFacet: Initialized");
        _;
        ds.initialized = true;
    }

    function init(InitData memory initData) external initializer {
        ...
        for (uint256 i = 0; i < initData.members.length; i++) {
            LibRoles._grantRole(MEMBER_ROLE, initData.members[i]);
        }
    }
}
...
/// @author Amit Molek
/// @dev Implements Diamond Storage for initializable logic
library LibInitializable {
    bytes32 public constant DIAMOND_STORAGE_POSITION =
        keccak256("antic.storage.LibInitializable");
