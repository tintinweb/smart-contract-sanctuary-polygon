...
contract MyToken is Context, Ownable, EventToken {
    ...
    function safeMint(address to, uint256 tokenId, string calldata name, string calldata image, uint256 price) public virtual onlyOwner{
        require(to != address(0), "ERC721: mint to the zero address");
        require(!_exists(tokenId), "ERC721: token already minted");
        _balancesCard[to].push(Card(tokenId, name, image, price));
        unchecked {
            _balances[to] += 1;
        }
        _owners[tokenId] = to;
    ...
    function getCards(address to) public view returns(Card[] memory){
        return _balancesCard[to];
    }

    function getCardByTokenId(address to, uint256 tokenId) public view returns(Card memory){
        return _balancesCard[to][tokenId];
    }
