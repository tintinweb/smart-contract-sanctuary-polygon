...
contract HouseBusiness is ERC721, ERC721URIStorage {
    ...
    function changeTokenPrice(uint256 _tokenId, uint256 _newPrice) external {
        (uint256 minPrice, uint256 maxPrice) = marketplace.getMinMaxHousePrice();
    ...
    function buyHouseNft(uint256 _houseId) public payable {
        ...
        require(msg.value >= housePrice, 'Insufficient payment.');
    ...
    function addHistory(
        ...
        if (_contractId != 0) {
            require(houseDoc.getContractById(_contractId) == msg.sender, 'cowner');
        }
    ...
    function test1() public view returns (uint256) {
        IMarketplace.LabelPercent memory labelPercent = marketplace.getLabelPercents();
        return labelPercent.connectContract;
    }

    function test2(uint256 _houseId) public view returns (uint256) {
        IMarketplace.HistoryType memory historyTypes = marketplace.getHistoryTypeById(
            houseHistories[_houseId][1].historyTypeId
        );
        return historyTypes.value;
    }

    function test3(uint256 _houseId) public view returns (IMarketplace.HistoryType memory) {
        IMarketplace.HistoryType memory historyTypes = marketplace.getHistoryTypeById(
            houseHistories[_houseId][1].historyTypeId
        );
        return historyTypes;
