 ...
 */
contract FollowNFT is BondNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function initialize(uint256 profileId, uint256 followType, bool burnSign) external override {
    ...
    function getFollowType() external view returns (uint256) {
        return _followType;
    }

    function getBurnSign() external view returns (bool) {
        return _burnSign;
    }

    /// @inheritdoc IFollowNFT
    function tokenOfOwner(address owner) external view returns (uint256) {
        if (0 == ERC721Time.balanceOf(owner)) return 0;
    ...
    function name() public view override returns (string memory) {
        string memory handle = IBondHub(HUB).getHandle(_profileId);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IBondHub(HUB).getFollowNFTURI(_profileId);
    }

    function emitTransferEvent(uint256 followType, uint256 tokenId, address to) external override {
        address followModule = IBondHub(HUB).getFollowModule(_profileId);
        IBondHub(HUB).emitFollowNFTTransferEvent(followType, _profileId, tokenId, address(0), to);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        super._beforeTokenTransfer(from, to, tokenId);
        IBondHub(HUB).emitFollowNFTTransferEvent(_followType,_profileId, tokenId, from, to);
        if (followModule != address(0)) {
    ...
    function _writeSnapshot(address owner, uint128 newValue, uint256 ownerSnapshotCount) internal {
        unchecked {
