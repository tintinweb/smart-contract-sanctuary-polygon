...
contract ERC721Upgradeable is Initializable, ContextUpgradeable, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
    ...
    // v1 multimint
    function _multimint(address to, uint256[] memory tokenIds) internal virtual {
...
interface ISnookGame is IDescriptorUser {
  ...
  function mint2(uint count) external;
...
contract SnookGame is ISnookGame, AccessControlEnumerableUpgradeable, PausableUpgradeable {
    ...
    uint public constant LIVES_PER_SNOOK = 5;
    ...
    function updateUsdcSkillAddress(address uniswap) public onlyRole(PAUSER_ROLE) {
      _uniswap = IUniswapUSDCSkill(uniswap);
    }
    ...
    function mint2(uint count) external override whenNotPaused() {
      ...
      for (uint i=0; i<count; i++) {
        (string memory tokenURI, uint traitId) = _generateTokenURI();
      ...
      _afterdeath.updateOnMint(TRAITCOUNT_MINT2*count);
    ...
    function reportKill(
      ...
      emit Death(_snook.ownerOf(tokenId), tokenId, killerTokenId, d.lives);
    }
    ...
    //     require(_snook.exists(killerTokenId) == true, 'Killer token does not exist');
    //     require(_snook.isLocked(tokenId) == true, 'Not in play'); // prevent wallet server from errors
    //     require(d.deathTime == 0, 'SnookGame: death time already not 0');
    //     d.deathTime = block.timestamp;

    //     // resurrection price is based on traits of dying snook 
    //     d.resurrectionPrice = _afterdeath.getResurrectionPrice(tokenId);

    //     // remember what traits should be assigned to snook on resurrection
    //     d.onResurrectionTraitCount = onResurrectionTraitCount;
    //     d.onResurrectionStars = onResurrectionStars;
    //     d.onResurrectionScore = onResurrectionScore;
    //     d.onResurrectionTokenURI = onResurrectionTokenURI;
    //     _state.setDescriptor(tokenId, d);
    ...
    //     }
    //     _snook.setKillerTokenId(tokenId, killerTokenId);
    //     emit Death(_snook.ownerOf(tokenId), tokenId, killerTokenId);
    // }
