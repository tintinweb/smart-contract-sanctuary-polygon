...
contract LottoX is Ownable{
  ...
  function setResult(uint256 oddID) public onlyOwner{
    ...
    for(uint i=_startBetID; i<_lastBetID; i++){
      ...
      betWin = betAmt.mul(totalPool).div(totalOddBet);
    ...
    if(totaPayout > 0 && totalPool >= totaPayout){
      totalPool = totalPool.sub(totaPayout);
    }
  }
  ...
  function _payout(address uplineAddress, uint256 amount, uint levelCounter) internal{
    if(amount > 0 && rates.length > levelCounter && _members[uplineAddress].totalDownlines > 0 && uplineAddress != address(0) && uplineAddress != address(this)){
  ...
  function withdrawMyWonBal() public returns(bool){
  ...
  function withdrawMyWonBonus() public returns(bool){
    uint256 winBonus = _winnerBonuses[msg.sender];
    require(winBonus > 0, "Insuffient amount");
    ERC20(_token).transfer(msg.sender, winBonus);
    _winnerBonuses[msg.sender] = _winnerBonuses[msg.sender].sub(winBonus);
    return true;
  }

  function withdrawMySponsorBonus() public returns(bool){
    uint256 spBonus = _sponsorBonuses[msg.sender];
    require(spBonus > 0, "Insuffient amount");
    ERC20(_token).transfer(msg.sender, spBonus);
    _sponsorBonuses[msg.sender] = _sponsorBonuses[msg.sender].sub(spBonus);
    return true;
  }

  function checkIncome() public view returns(uint256, uint256, uint256){
    return (
      _winnerRecords[msg.sender],
      _winnerBonuses[msg.sender],
      _sponsorBonuses[msg.sender]
    );
  }
