...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    */
	...
	event BUY(address seller, uint256 amount);

    function buy()public payable returns (uint amount) {
        require(msg.value > 0, "Insufficient payment");
       
        uint256 Matic = tokenPrice + (amount * priceIncrement);// calculate the total cost of the tokens
    ...
    function calculateCostFirstTime(uint256 amount) public view returns (uint256) {
        uint256 matic = tokenPrice * amount;
        return matic;
    }

    function calculateCostSecondTime(uint256 amount) public view returns (uint256) {
        uint256 Matic = tokenPrice + (amount * priceIncrement);// calculate the total cost of the tokens
        return Matic;
    }


    function calculateUpdatedTokenPrice( uint tokenAmount) internal view returns (uint) {
        uint256 Matic = tokenPrice + (tokenAmount * priceIncrement);// calculate the total cost of the tokens
        return Matic * (_tTotal / 1000 + 1);
    }
