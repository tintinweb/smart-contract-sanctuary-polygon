...
contract Status {
    ...
    function status(address _holder) public view returns (uint256) {
        if (balances[_holder] >= 100000 * 10 ** _token.decimals()) return 5;
        if (balances[_holder] >= 30000 * 10 ** _token.decimals()) return 4;
        if (balances[_holder] >= 10000 * 10 ** _token.decimals()) return 3;
