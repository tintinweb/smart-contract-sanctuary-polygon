...
contract ProofOfXFactory is AccessControl, ERC2771Context {
    ...
    mapping(string => addresses) public factories;
    ...
    /**
     * @dev Constructor for PoXFactory contract
     */
    constructor(address _ngageNFactory, CustomForwarder _forwarder)
    ...
    /**
     @dev modifier to check if the sender is the default admin of PoXFactory contract
     * Revert if the sender is not the contract admin
     */
    modifier onlyAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "PoXF: Only Admin");
        _;
    }
    ...
    modifier onlyTrustedForwarder() {
        require(isTrustedForwarder(msg.sender), "PoXF: Only Trusted Forwarder");
        _;
    }
        ...
        onlyTrustedForwarder
    {
        require(
            ...
            "PoXF: Only the contract creator can create a factory"
        );
        require(
            !factoryCreated[_factoryName],
            "PoXF: Factory already created for this factory name"
        );
        require(
            INgageNFactory(ngageNFactory).factoryCreator(_factoryName) == _owner,
            "PoXF: Contract Creator cannot be different for NgageN and proofOfX"
        );
        address proxy = INgageNFactory(ngageNFactory).ngageNContracts(
            _factoryName
        );
        require(proxy != address(0), "PoXF: NgageN Proxy cannot be null.");

        CustomForwarder forwarder = new CustomForwarder();
        ProofOfX proofOfX = new ProofOfX(INgageN(proxy), forwarder);

        proofOfX.grantRole(DEFAULT_ADMIN_ROLE, _owner);
        proofOfX.grantRole(CREATOR, _assetCreator);
        for(uint i = 0; i < _operators.length; i++) {
            proofOfX.grantRole(OPERATOR, _operators[i]);
        }

        forwarder.grantRole(DEFAULT_ADMIN_ROLE, _owner);
        forwarder.grantRole(RELAYER, _relayer);

        factoryCreated[_factoryName] = true;
        factories[_factoryName] = addresses(
            _owner,
            _factoryName,
            address(proxy),
            address(proofOfX),
...
interface INgageNFactory is IAccessControl {
    ...
    function factoryCreator(string memory _factoryName) external pure returns (address);
