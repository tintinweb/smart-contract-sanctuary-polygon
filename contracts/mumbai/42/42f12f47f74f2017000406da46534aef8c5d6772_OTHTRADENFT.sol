...
contract OTHTRADENFT is IBidNFT, ERC721Holder, Ownable, Pausable {
    using SafeMath for uint256;
    ...
    function sellTokenTo(uint256 _tokenId, address _to) public override whenNotPaused {
        ...
        quoteErc20.transfer(_seller, price.sub(total_fees));
        quoteErc20.transfer(nft.minterInfo(_tokenId) , royaltyAmount);
        _asksMap.remove(_tokenId);
        _userSellingTokens[_tokenSellers[_tokenId]].remove(_tokenId);
