    ...
    ReentrancyGuard
{
    ...
    uint16 private index = 1;

    uint16 public phaseForClaim;

    uint8 private maxFusionsPerDay = 19;
    ...
    mapping(bytes => uint8) private throttling;

    mapping(address => uint8) public bonuses_fusion;
    ...
    modifier throttleLimit() {
        ...
        if (throttling[key] > maxFusionsPerDay + bonuses_fusion[msg.sender])
            revert("too many fusions for today");
        _;
    }

    function setBonusFusion(address to, uint8 quantity) external onlyOwner {
    ...
    function _afterTokenTransfers(
        ...
        if (!Address.isContract(from) && from!=0x0000000000000000000000000000000000000000) {
