...
contract PWCC is Context, IERC20, Ownable {
    ...
    uint256 public swapTokensAtAmount = 15_000 * 10**9;
    uint256 public maxBuyLimit = 100_000 * 10**9;
    uint256 public maxSellLimit = 100_000 * 10**9;
    uint256 public maxWalletLimit = 100_000 * 10**9;
    
    uint256 public genesis_block;
    
    address public marketingWallet = 0xEC6a8A7b181A02CeCF95465346b5278F435996A7;
    address public devWallet = 0xEC6a8A7b181A02CeCF95465346b5278F435996A7;
    address public burnWallet = 0x0000000000000000000000000000000000000000;

    string private constant _name = "PWCC";
    string private constant _symbol = "PWCCCC";
    ...
    Taxes public taxes = Taxes(0, 3, 2, 0, 0);
    Taxes public sellTaxes = Taxes(0, 3, 2, 0, 0);
    ...
    constructor (address routerAddress) {
        ...
        allowedTransfer[burnWallet] = true;
    ...
    function setTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _burn) public onlyOwner {
       taxes = Taxes(_rfi,_marketing,_liquidity,_dev,_burn);
        emit FeesChanged();
    }
    
    function setSellTaxes(uint256 _rfi, uint256 _marketing, uint256 _liquidity, uint256 _dev, uint256 _burn) public onlyOwner {
       sellTaxes = Taxes(_rfi,_marketing,_liquidity,_dev,_burn);
        emit FeesChanged();
    }
    ...
    function _getValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory to_return) {
        ...
        (to_return.rDev, to_return.rburn) = _getRValues2(to_return, takeFee, _getRate());
        return to_return;
    }
    ...
    function _getTValues(uint256 tAmount, bool takeFee, bool isSell) private view returns (valuesFromGetValues memory s) {
        ...
        s.tTransferAmount = tAmount-s.tRfi-s.tMarketing-s.tLiquidity-s.tDev-s.tburn;
        return s;
    }
    ...
    function _getRValues1(valuesFromGetValues memory s, uint256 tAmount, bool takeFee, uint256 currentRate) private pure returns (uint256 rAmount, uint256 rTransferAmount, uint256 rRfi,uint256 rMarketing, uint256 rLiquidity){
        ...
        rTransferAmount =  rAmount-rRfi-rMarketing-rLiquidity-rDev-rburn;
        return (rAmount, rTransferAmount, rRfi,rMarketing,rLiquidity);
    }
    
    function _getRValues2(valuesFromGetValues memory s, bool takeFee, uint256 currentRate) private pure returns (uint256 rDev,uint256 rburn) {
    ...
    function _tokenTransfer(address sender, address recipient, uint256 tAmount, bool takeFee, bool isSell) private {
        ...
        if(s.rLiquidity > 0 || s.tLiquidity > 0) {
            ...
            emit Transfer(sender, address(this), s.tLiquidity + s.tMarketing + s.tDev+ s.tburn);
        }
        if(s.rMarketing > 0 || s.tMarketing > 0) _takeMarketing(s.rMarketing, s.tMarketing);
        if(s.rburn > 0 || s.tburn > 0) _takeburn(s.rburn, s.tburn);
    ...
    function swapAndLiquify(uint256 contractBalance, Taxes memory temp) private lockTheSwap{
        uint256 denominator = (temp.liquidity + temp.marketing + temp.dev + temp.burn) * 2;
        ...
        uint256 burnAmt = unitBalance * 2 * temp.burn;
        if(burnAmt > 0){
