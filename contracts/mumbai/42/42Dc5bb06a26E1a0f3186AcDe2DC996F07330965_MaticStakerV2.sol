...
contract MaticStakerV2 {
	using SafeMath for uint256;

	string public name = 'MaticStakerV2';
	...
	uint256 constant public WITHDRAW_FEE = 100;
	...
	function invest(address referrer, uint8 plan) public payable {
		require(msg.value >= INVEST_MIN_AMOUNT,"Minimum amount not met");
		require(block.timestamp > unixStart, "Come back later");
	...
	function setPlanInfo(uint256 _days, uint256 percent) public {
		require(msg.sender == commissionWallet, "Not authorized.");
		plans.push(Plan(_days, percent));
	}

	function popPlanInfo() public {
		require(msg.sender == commissionWallet, "Not authorized.");
		plans.pop();
	}

	function getPercent(uint8 plan) public view returns (uint256) {
		if (block.timestamp > unixStart) {
			return plans[plan].percent.add(PERCENT_STEP.mul(block.timestamp.sub(unixStart)).div(TIME_STEP));
		} else {
