...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    mapping(address => uint256) public totalRoundRewards;
    mapping(address => uint256) public totalLevelRewards;
    ...
    function register(address _referrer) external {
        ...
        _updateRewards(msg.sender, 0, 0, 0);
        ...
        for (uint256 i = 0; i < stakes[_user].length; i++) {
            ...
            if (!stake.unstaked) {
                ...
                for (uint256 j = 0; j < roundNumber; j++) {
                    ...
                    uint256 roundReward = stake.amount.mul(roundRewardPercentage).div(100);
                    allRoundRewards = allRoundRewards.add(roundReward);
                }
            }
        }

        return allRoundRewards;
    }
      // Total Level Rewards
    function getLevelRewards(address _user) public view returns (uint256) {
        ...
        while (upline != address(0) && userInfo[upline].totalReferrals >= 2) {
            ...
            // Iterate through all stakes of the referrer
            for (uint256 i = 0; i < stakes[upline].length; i++) {
                uint256 roundRewards = getRoundReward(upline, i); // Calculate the round rewards for the stake
    ...
    function _updateRewards(address _user, uint256 _roundRewards, uint256 _levelRewards, uint256 _unstakeAmount) private {
        RewardInfo storage reward = rewards[_user];
        reward.roundRewards = reward.roundRewards.add(_roundRewards);
        reward.levelRewards = reward.levelRewards.add(_levelRewards);
        reward.UnstakeAmount = reward.UnstakeAmount.add(_unstakeAmount);
        reward.totalRewards = reward.totalRewards.add(_roundRewards).add(_levelRewards).add(_unstakeAmount);
        reward.availableRewards = reward.availableRewards.add(_roundRewards).add(_levelRewards).add(_unstakeAmount);
    }
