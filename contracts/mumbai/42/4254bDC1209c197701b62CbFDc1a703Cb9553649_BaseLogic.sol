...
contract BaseLogic is Initializable, OwnableUpgradeable, IBaseLogic {
    ...
    address public landManager;
    address public gameResource;
    address public gameController;
    address public eventContract;
    address public removeLogic;
    address public destroyLogic;
    address public gameSupport;
    ...
    function setEvent(address _event) public onlyOwner {
        eventContract = _event;
    }

    function setRemoveLogic(address _remove) public onlyOwner {
        removeLogic = _remove;
    }

    function setDestroyLogic(address _destroy) public onlyOwner {
        destroyLogic = _destroy;
    }

    function setGameSupport(address _support) public onlyOwner {
        gameSupport = _support;
    }
    ...
    function boostPayout(
        ...
        uint256 index = IGameController(gameController).getIndex(id, position);
        IGameController(gameController).updatePayout(id, position, 0, 0, 1);
        IInventory.SpecialResource memory resource = IInventory(inventory)
            .getASpecialResource(id, resourceId, index);

        IInventory(inventory).updateSpecialResource(
            id,
            resourceId,
            index,
            resource.payout + addedValue*10,
            resource.prop
        );
    }
    ...
    function addNewResource(
        ...
        if (_isItemExist(id, uint256(Items.ID.IB10), false)) {
            IGameController(gameController).updateNewAddedResource(id);
        }
        IInventory(inventory).addResource(id, resourceId, quantity);
    }

    function rewardCoin(
        uint256 id,
        string memory category
    ) external view override returns (int256 coin) {
    ...
    function mulPayoutResource(
        ...
        uint16[] memory res_pos = IGameController(gameController).findPosition(
            id,
            resourceId
        );
        if (res_pos.length > 0) {
    ...
    function destroyItem(
        ...
        IInventory(inventory).removeItem(id, itemId, index);
        IInventory(inventory).addDestroyedItems(id, itemId);
    }
    ...
    function checkAllAdjacent(
        uint256 id,
        uint16 position
    ) external override onlyGameContract returns (bool valid) {
        if (
            _isItemExist(id, uint256(Items.ID.IP6), false) ||
            _isItemExist(id, uint256(Items.ID.IT2), false)
        ) {
            uint256[] memory propIP6 = IInventory(inventory).getSpecialItems(
                id,
                uint256(Items.ID.IP6)
            );
            uint256[] memory propIT2 = IInventory(inventory).getSpecialItems(
                id,
            ...
            for (uint256 i = 0; i < propIP6.length; ++i) {
                ...
                if (
                    ...
                    // TODO
                    _updateSpecialItem(id, uint256(Items.ID.IP6), false, i, 3);
                    valid = true;
                } else if (propIP6[i] != 3) {
                    _updateSpecialItem(
                        id,
            ...
            for (uint256 i = 0; i < propIT2.length; ++i) {
                ...
                if (propIT2[i] == 0) {
                    // TODO
                    _updateSpecialItem(id, uint256(Items.ID.IT2), false, i, 3);
                    valid = true;
                } else if (propIT2[i] != 3) {
                    _updateSpecialItem(
                        id,
    ...
    function getASpecialItem(
        ...
        itemId = isEssence ? itemId + 2000 : itemId + 1000;
        return IInventory(inventory).getASpecialItem(id, itemId, index);
    }
    ...
    function isItemExist(
        ...
        return _isItemExist(id, itemId, isEssence);
    }

    function _isItemExist(
        uint256 id,
        uint256 itemId,
        bool isEssence
        ...
        return IInventory(inventory).isItemExist(id, itemId);
    }
    ...
    function updateSpecialItem(
        ...
        return _updateSpecialItem(id, itemId, isEssence, index, newProp);
    }

    function _updateSpecialItem(
        uint256 id,
        uint256 itemId,
        bool isEssence,
        uint256 index,
        ...
        itemId = isEssence ? itemId + 2000 : itemId + 1000;
        return
            IInventory(inventory).updateSpecialItem(id, itemId, index, newProp);
    }
    ...
    function _rewardCoin(
        ...
        uint256[] memory resId = IGameResource(gameResource).getCategory(
            category
        );
        for (uint256 i = 0; i < resId.length; ++i) {
            uint16[] memory res_pos = IGameController(gameController)
                .findPosition(id, resId[i]);
            if (res_pos.length > 0) {
    ...
    function _updatePayout(
        ...
        for (uint256 i = 0; i < position.length; ++i) {
            IGameController(gameController).updatePayout(
                id,
                position[i],
                mul,
                divide,
