...
contract swapHelperDiyTask4 is OpsTaskCreator, Ownable {
    ...
    modifier onlyEditer(bytes32 _taskID) {
        ...
        require(msg.sender == owner() || msg.sender == y.taskConfigData._owner, "e003");
        _;
    }

    modifier onlyTime(bytes32 _md5) {
        taskConfig memory y = taskConfigList[md5TaskList[_md5]];
        require(block.timestamp >= (lastExecutedTimeList[_md5]).add(y.taskConfigCreate._timeIntervalList[lastTimeIntervalIndexList[md5TaskList[_md5]]]), "m001");
        require(getInTimeZone(y.taskConfigCreate._start_end_Time, y.taskConfigCreate._timeList), "m002");
        _;
    }
    ...
    function swapToken(address _user, bytes32 _md5) external onlyTime(_md5) {
        ...
        require(txHistoryList[x.taskID][x.day]._totalTx.add(1) <= y.taskConfigCreate._maxtxAmount, "p001");
        require(txHistoryList[x.taskID][x.day]._totalSpendTokenAmount.add(x.claimAmount * 2) <= y.taskConfigCreate._maxSpendTokenAmount, "p002");
        ...
        if (x.swapInAmount == 0) {
            ...
            if (address(USDTPoolAddress.USDT()) == x._TokenItem.swapInToken) {
                ...
                require(userInfoList[_user].usdtAmount >= x._feeItem.allFee, "m003");
    ...
    function cancelTask(bytes32 _taskID) external onlyEditer(_taskID) {
        taskConfig storage y = taskConfigList[_taskID];
        require(y.taskConfigData._status, "e002");
        _cancelTask(_taskID);
