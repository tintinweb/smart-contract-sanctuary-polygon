...
contract TransferStep is BaseStep {
    ...
    // constructor --> set the fee controller
    constructor(address _dremHub, address _assetRegistry, address _feeController) BaseStep(_dremHub, _assetRegistry) {
        _setFeeController(_feeController);
    }

    // require init to first step, to remove risk of double calculation
    function init(uint256 _stepNodeKey, bytes memory) external override canBeLegacied {
        // Transfer step must be the root of the stepTree
        if (_stepNodeKey != 1) revert Errors.InvalidStepPosition();
    }

    // wind function --> transfer some assets
    function wind(address _caller, uint256, uint256 _amountIn, bytes memory)
        external
        override
        canBeDeprecated
        returns (uint256)
    {
        if (_amountIn == 0) revert TransferLib.InsufficientFunds();

        // split the funds and fees
        TransferLib.Distribution memory fundSplit = _splitDepositFunds(_amountIn);
        ...
        address denominationAsset = IVault(msg.sender).getDenominationAsset();

        _receiveFunds(_caller, msg.sender, fundSplit.funds, denominationAsset);

        // transfer fees for drem (if necessary)
        if (fundSplit.fees.dremFee > 0) {
            _receiveFunds(_caller, feeController.getDremCollector(), fundSplit.fees.dremFee, denominationAsset);
        }

        // transfer vault fees (if necessary)
        if (fundSplit.fees.adminFee > 0) {
            _receiveFunds(
                _caller, feeController.getVaultCollector(msg.sender), fundSplit.fees.adminFee, denominationAsset
            );
        }

        return fundSplit.funds;
    }

    // unwind function --> transfer the assets back
    function unwind(address _caller, uint256, uint256 _amountIn, bytes memory)
        external
        override
        canBeDisabled
        returns (uint256)
    {
        ...
        TransferLib.Distribution memory fundSplit = _splitWithdrawalFunds(_caller, _amountIn);

        // push the funds from the vault to the user (should be done in generalist function)
        _sendFunds(_caller, fundSplit.funds, denominationAsset);

        // transfer fees to drem
        if (fundSplit.fees.dremFee > 0) {
            _sendFunds(feeController.getDremCollector(), fundSplit.fees.dremFee, denominationAsset);
        }

        // transfer fees to admin
        if (fundSplit.fees.adminFee > 0) {
            _sendFunds(feeController.getVaultCollector(msg.sender), fundSplit.fees.adminFee, denominationAsset);
        }
    ...
    function value(uint256, address _denominationAsset) external view override returns (uint256) {
        // just return the value of the denomination asset from the vault
        return IERC20(_denominationAsset).balanceOf(msg.sender);
    }
    ...
    // decode the variable args
    function _decodeVariableArgs(bytes memory _variableArgs)
        internal
        pure
        returns (TransferLib.VariableArgData memory)
    {
        uint256 funds = abi.decode(_variableArgs, (uint256));
        return TransferLib.VariableArgData({funds: funds});
    }

    // get the funds split for deposits
    function _splitDepositFunds(uint256 _funds) internal view returns (TransferLib.Distribution memory) {
        // allocate some memory to the distribution
        TransferLib.Distribution memory fundSplit;

        // This needs to be done with a precision factor of 10^18
        // get the fee that the user will incur (will be handled by the fee controller)
        fundSplit.fees = feeController.calculateDepositFees(_funds, msg.sender);

        // get the remaining purchasing power
        fundSplit.funds = _funds - fundSplit.fees.dremFee - fundSplit.fees.adminFee;

        // return this information back to the caller
        return fundSplit;
    }

    // get the funds split for withdrawals
    function _splitWithdrawalFunds(address _caller, uint256 _funds)
        internal
        view
        returns (TransferLib.Distribution memory)
    {
        // allocate some memory to the distribution
        TransferLib.Distribution memory fundSplit;
        ...
        // This needs to be done with a precision factor of 10^18
        // get the fee that the user will incur (will be handled by the fee controller)
        fundSplit.fees = feeController.calculateWithdrawalFees(_funds, _caller, msg.sender);
    ...
    function _sendFunds(address _recipient, uint256 _amount, address _denominationAsset) internal {
        ...
        bytes memory transferData = abi.encodeWithSelector(IERC20.transfer.selector, _recipient, _amount);

        // send the transfer
        bytes memory returnData = IVault(msg.sender).execute(_denominationAsset, transferData);

        bool transferSuccess = (returnData.length == 0 || abi.decode(returnData, (bool)));

        // revert if this transfer has not been executed
        if (!(transferSuccess)) revert TransferLib.TransferFailed();
    }

    function _receiveFunds(address _sender, address _recipient, uint256 _amount, address _denominationAsset) internal {
        // encode the ERC20 transfer
        bytes memory transferData = abi.encodeWithSelector(IERC20.transferFrom.selector, _sender, _recipient, _amount);
    ...
    function _setFeeController(address _feeController) internal {
        if (_feeController == address(0)) revert Errors.ZeroAddress();
...
abstract contract BaseStep is IStep, HubOwnable {
    ...
    function unwind(
        ...
        uint256 stepNodeKey,
        uint256 amountIn,
        bytes memory encodedVariableArgs
    ) external virtual canBeDisabled returns (uint256) {}

    function value(uint256 stepNodeKey, address denominationAsset) external view virtual returns (uint256) {}
    ...
    function _validateStepNotDeprecated() internal view {
        _validateStepNotDisabled();
        if (DREM_HUB.getStepState(address(this)) == DataTypes.StepState.Deprecated) revert Errors.StepDeprecated();
    }

    function _validateStepNotLegacied() internal view {
        _validateStepNotDisabled();
        _validateStepNotDeprecated();
        if (DREM_HUB.getStepState(address(this)) == DataTypes.StepState.Legacy) revert Errors.StepLegacied();
    }
}
...
library TransferLib {
    ...
    // struct for what the Fixed Args will be is not necessary, as it will take no data
