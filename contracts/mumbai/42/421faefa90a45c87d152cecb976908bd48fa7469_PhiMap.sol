...
contract PhiMap is AccessControlUpgradeable, IERC1155ReceiverUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice Require that Already created.
     */
    ...
    function changeWallPaper(
        ...
        address contractAddress,
        uint256 tokenId
    ) public onlyIfNotLocked nonReentrant onlyIfNotPhilandOwner(name) {
        address lastWallPaperContractAddress = wallPaper[name].contractAddress;
        uint256 lastWallPaperTokenId = wallPaper[name].tokenId;
        // Withdraw the deposited WALL OBJECT at the same time if it has already been set up
        if (lastWallPaperContractAddress != address(0)) {
     ...
     */
    function mapInitialization(string memory name) external onlyIfNotLocked onlyIfNotPhilandOwner(name) {
    ...
    function batchDepositObject(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            depositObject(name, contractAddresses[i], tokenIds[i], amounts[i]);
        }
    }
    ...
    function writeLinkToObject(
        ...
        uint256 objectIndex,
        Link memory link
    ) external onlyIfNotLocked onlyIfNotPhilandOwner(name) onlyIfNotReadyObject(name, objectIndex) {
