...
contract Test {
    ...
    function getUserBoard(address user, uint8 level) public view returns(uint256[] memory, address[] memory, uint8 reinvestCount , uint256 bUser) {
        ...
        uint8 k=0;
        if(users[user].levelActive[level] && users[user].boardMatrix[level].reinvestCount<2)
        {
            while(true) {
     ...
     function getUserBoardDetail(address user, uint8 level) public view returns(address bReferrer, address[] memory, address[] memory, uint8 reinvestCount , bool boardActive) {
       return(users[user].boardMatrix[level].boardReferrer, users[user].boardMatrix[level].firstReferrals, users[user].boardMatrix[level].secondReferrals, users[user].boardMatrix[level].reinvestCount, users[user].boardMatrix[level].isBoardActive);
    }
