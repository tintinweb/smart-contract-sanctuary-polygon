    ...
    OwnableUpgradeable
{
    ...
    mapping(bytes32 => bool) public externalSources;

    /// @notice A mapping that keeps track of the energy ID a source can increase.
    mapping(bytes32 => uint8) public sourceEnergyIds;

    /// @notice A mapping that keeps track of the multiplier a source can increase.
    mapping(bytes32 => uint256) public sourceEnergyMultiplier;

    /// @notice A mapping that keeps track of which sources have been redeemed per identifier.
    mapping(bytes32 => mapping(bytes32 => bool)) public redeemedSources;

    /// @notice Addresses that can sign redeem requests.
    mapping(address => bool) public authorizedSigners;

    /// @notice A mapping that keeps track of which request ids have been used.
    mapping(bytes32 => mapping(bytes32 => bool)) private usedRequestIds;
    ...
    error AlreadyRedeemed(bytes32 source, bytes32 id);
    ...
    /// @notice A modifier that checks if a redeem request is valid.
    ...
    modifier onlyValidSignature(
        ...
        if (
            usedRequestIds[request.externalSourceId][request.uid] ||
            !authorizedSigners[signer]
        ) revert InvalidSignature();
        _;
    }

    /// @notice Adds a source that can be redeemed.
    ...
    function addExternalSource(
        ...
        externalSources[sourceId] = true;
        sourceEnergyIds[sourceId] = energyId;
        sourceEnergyMultiplier[sourceId] = multiplier;

        emit ExternalSourceAdded(sourceId, energyId, multiplier);
    }
    ...
    function removeExternalSource(bytes32 sourceId) external onlyOwner {
        ...
        delete sourceEnergyIds[sourceId];
        delete sourceEnergyMultiplier[sourceId];
    ...
    function clearBadge(bytes32 sourceId, bytes32 id) external onlyOwner {
        delete redeemedSources[sourceId][id];
    }
    ...
    function redeem(
        RedeemRequest calldata request,
        bytes calldata signature
    ) external onlyValidSignature(request, signature) {
        if (!externalSources[request.externalSourceId]) revert UnknownSource();
        if (redeemedSources[request.externalSourceId][request.id])
            revert AlreadyRedeemed(request.externalSourceId, request.id);

        // Mark id as redeemed
        redeemedSources[request.externalSourceId][request.id] = true;

        // Mark request as used
        usedRequestIds[request.externalSourceId][request.uid] = true;

        uint8 energyId = sourceEnergyIds[request.externalSourceId];
        uint256 amount = request.score *
            sourceEnergyMultiplier[request.externalSourceId];
