    ...
    UUPSUpgradeable
{
    ...
    mapping(address => VestingStruct) internal _vestingMap;

    event EtherReleased(uint256 amount);
    event ERC20Released(address indexed token, uint256 amount);

    IERC20 public aprsToken;
    ...
    function setVestingToken(address _token) external onlyOwner {
        ...
        aprsToken = IERC20(_token);
    }

    function setVestingAccount(
        address beneficiary,
        uint256 cliff,
        uint256 start,
        uint256 duration,
        ...
        VestingStruct memory _vestingStruct = VestingStruct(
            cliff,
            start,
            duration,
            released,
        ...
        _vestingMap[beneficiary] = _vestingStruct;
    }

    function deposit(uint256 value) external {
        // SafeERC20.safeTransfer(aprsToken, address(this), value);
        aprsToken.transfer(address(this), value);
    }

    function contractAmount() external view returns (uint256) {
        return aprsToken.balanceOf(address(this));
    }

    /**
     * @dev Getter for the cliff time.
     */
    function cliff(address beneficiary) external view returns (uint256) {
        VestingStruct memory vesting = _vestingMap[beneficiary];
    ...
    /**
     * @dev Getter for the vesting amount total.
     */
    function amountTotal(address beneficiary) external view returns (uint256) {
        VestingStruct memory vesting = _vestingMap[beneficiary];
        return vesting.amountTotal;
    }
     ...
     */
    function release(address beneficiary) external virtual {
        VestingStruct memory vesting = _vestingMap[beneficiary];
        ...
        // SafeERC20.safeTransfer(aprsToken, beneficiary, releasable);
        aprsToken.transfer(beneficiary, releasable);
    }

    /**
     * @dev Calculates the amount of tokens that has already vested. Default implementation is a linear vesting curve.
     */
    function vestedAmount(address beneficiary, uint256 timestamp)
        public
        view
        virtual
        returns (uint256)
     ...
     */
    function _vestingSchedule(address beneficiary, uint256 timestamp)
        internal
        view
        virtual
        returns (uint256)
    {
        ...
        if (timestamp < vesting.start + vesting.cliff) {
            ...
            return
                (vesting.amountTotal * (timestamp - vesting.start)) /
                vesting.duration;
        }
    }
}
