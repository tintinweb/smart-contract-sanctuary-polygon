...
contract GBotBreeder is ERC721Receiver, Ownable, Pausable, GBotBabyLock, BreedingProperties {
  ...
  event BreedingStarted(address indexed owner, uint256[] parentsTokenIds, uint256[] parentMetadatas, uint256 tokenId, uint256 lockTimestamp);
    ...
    function breed(address sender, bytes calldata sig, uint256[] memory tokenIds) public virtual whenNotPaused {
        ...
        uint256 seed = 10000; //checkSignature(sender, sig, tokenIds);
        ...
        counter++;
        nonces[_msgSender()] ++;
        emit BreedingStarted(_msgSender(), tokenIds, parentMetadatas, babyBotMetadata, block.timestamp + babyLockTime);
    }
