  ...
  // function purchaseLevels9(uint8 _level) external isUserAccount(_msgSender()) {
    ...
    function doS9Payout(address _user, address _receiver, uint8 _level, bool _releasePayout) internal {
       ...
       uint price =  prices[_level];
        ...
        sendETHDividends(_user, _receiver,_level);
 ...
 function buyNewLevels4(address _child, uint8 level, uint _matrix)  internal {
       ...
       // require(msg.value == levelPrice[level], "invalid price");
        ...
        require(!userss[_child].activeX6Levels[level], "EC-13"); 
            ...
            if (_matrix==0){
                // s4
                 address freeXReferrer = findFreeX6Referrer(_child, level);
...
function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (x6Matrix[referrerAddress][level].firstLevelReferrals.length < 2) {
            ...
            address x6firstLevelRef0 = x6Matrix[ref][level].firstLevelReferrals[0];
            address x6firstLevelRef1 = x6Matrix[ref][level].firstLevelReferrals[1];
            uint x6firstLevelLength = x6Matrix[referrerAddress][level].firstLevelReferrals.length;
            
            if ((len == 2) && 
                (x6firstLevelRef0 == referrerAddress) &&
                (x6firstLevelRef1 == referrerAddress)) {
            ...
            } else if (len == 2 && x6firstLevelRef1 == referrerAddress) {
                if (x6firstLevelLength == 1) {
                    //emit NewUserPlace(userAddress, ref, 2, level, 5);
                } else {
                    //emit NewUserPlace(userAddress, ref, 2, level, 6);
        ...
        address x6MatrixFirstLevelReferrals0 = x6Matrix[referrerAddress][level].firstLevelReferrals[0];
        address x6MatrixFirstLevelReferrals1 = x6Matrix[referrerAddress][level].firstLevelReferrals[1];

        if (x6MatrixClosePart != address(0)) {
            if ((x6MatrixFirstLevelReferrals0 == 
                x6MatrixFirstLevelReferrals1) &&
                (x6MatrixFirstLevelReferrals0 ==
                x6MatrixClosePart)) {

                updateX6(userAddress, referrerAddress, level, true);
                return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
            } else if (x6MatrixFirstLevelReferrals0 == 
                x6MatrixClosePart) {
                updateX6(userAddress, referrerAddress, level, true);
                return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
            } else {
        ...
        if (x6MatrixFirstLevelReferrals1 == userAddress) {
            updateX6(userAddress, referrerAddress, level, false);
            return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
        } else if (x6MatrixFirstLevelReferrals0 == userAddress) {
            updateX6(userAddress, referrerAddress, level, true);
            return updateX6ReferrerSecondLevel(userAddress, referrerAddress, level);
        }
        
        if (x6Matrix[x6MatrixFirstLevelReferrals0][level].firstLevelReferrals.length <= 
            x6Matrix[x6MatrixFirstLevelReferrals1][level].firstLevelReferrals.length) {
            updateX6(userAddress, referrerAddress, level, false);
        } else {
            updateX6(userAddress, referrerAddress, level, true);
        }
 ...
 function usersX6Matrix(address userAddress, uint8 level) public view returns(address, address[] memory, address[] memory, bool, address, bool) {
    ...
    function sendETHDividends(address userAddress, address _from, uint8 level) private {
       ...
       // }
        ...
        uint Fee = levelPrice>=distPrice? levelPrice-distPrice:0;

        if (Fee>0){

             tokenMetaRider.transferFrom(_msgSender(), address(this), Fee);

        }

        // commenting this because I think the fund should only transfer to receiver if this is part of your plan then remove the comment

        // if (!tokenMetaRider.transferFrom(_msgSender(), receiver, distPrice )) {

        //     tokenMetaRider.transferFrom(_msgSender(), owner(), distPrice);
        // }
