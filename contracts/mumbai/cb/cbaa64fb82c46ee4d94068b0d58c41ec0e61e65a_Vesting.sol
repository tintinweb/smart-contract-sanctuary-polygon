...
/**
@title 2112 Team Vesting
@author 2112.run
@notice This contract releases DATA to the team safe linearly over the period of 2 years
*/
contract Vesting is VestingWallet {
    ...
    */
    address public safe;
    /**
    @notice Address of the Registry used to resolve ecosystem addresses
    */
    IRegistry public registry;
    /**
    @notice Reference to the DATA contract
    */
    IERC20 public data;
    /**
    @notice Constructor of the Vesting contract
    @param safe_ Address of the team safe
    @param registry_ Address of the Registry used to resolve ecosystem addresses
    @param nonce_ Prevent re-using signature
    @param signature_ Signature from deployer for Registry
    */
    constructor(address safe_, address registry_, uint256 nonce_, bytes memory signature_)
        VestingWallet(safe_, uint64(block.timestamp), uint64(730 days)) {
        registry = IRegistry(registry_);
        registry.register(Constants.VESTING, nonce_, signature_);
    }
    /**
    @notice Claim any ERC20 in the contract sent by mistake that is not DATA
    @param token Address of the ERC20
    */
    function release(address token) public override {
        if(address(data) == address(0))
            data = IERC20(registry.getEntry(Constants.DATA));
        if(address(data) == token)
            revert CannotRelease();
        IERC20(token).transfer(safe, IERC20(token).balanceOf(address(this)));
    }
    /**
    @notice Claim any ETH in the contract
    */
    function release() public override {
        payable(safe).transfer(address(this).balance);
    }
    /**
    @notice Release of DATA based on vesting schedule
    */
    function releaseData() public {
        if(address(data) == address(0))
            data = IERC20(registry.getEntry(Constants.DATA));
        super.release(address(data));
    }
}
