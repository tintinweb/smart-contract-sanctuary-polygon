...
contract Coin_MyCoin is ERC20 {
	...
	struct referralRecord { bool hasDeposited; address referringAddress; uint256 referralsAtLevel0; uint256 referralsAtLevel1; }
 ...
 * calls addReferral with variable _amt as amount of native currency sent to contract
 ...
 * updates referralRecordMap (Element referringAddress) (Entity referralsAtLevel0) as (referralRecordMap with element referringAddress with element referralsAtLevel0) + (_amt)
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * if referringAddress is equals to Address 0 then ()
 * updates referralRecordMap (Element referringAddress) (Entity referralsAtLevel1) as (referralRecordMap with element referringAddress with element referralsAtLevel1) + (_amt)
 * updates referringAddress as referralRecordMap with element referringAddress with element referringAddress
 * 
*/
	function addReferral(uint256 _amt) internal {
		...
		referralRecordMap[referringAddress].referralsAtLevel0  = (referralRecordMap[referringAddress].referralsAtLevel0 + _amt);
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		if ((referringAddress == address(0))){
			return;
		}
		referralRecordMap[referringAddress].referralsAtLevel1  = (referralRecordMap[referringAddress].referralsAtLevel1 + _amt);
		referringAddress  = referralRecordMap[referringAddress].referringAddress;
		return;
	}
