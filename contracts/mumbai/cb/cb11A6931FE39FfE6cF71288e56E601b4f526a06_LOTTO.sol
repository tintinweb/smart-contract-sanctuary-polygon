...
contract LOTTO {
    ...
    constructor(address payable _ownerAddress) {
        ...
        player[msg.sender].userAddress = msg.sender;
    ...
    function buyFirstNft(address _referral) public payable {
        ...
        IERC20(busd).transfer(LottoGameAddress, 23.5 ether);
        ...
        if (player[msg.sender].playerId == 0) {
            ...
            _playerStruct.userAddress = msg.sender;
    ...
    function buySecondNft() public payable {
        ...
        if (plyrRnds_[_referral][accumulatRoundID].referrers % 5 <= 4) {
            if (player[_referral].isBuySecondNft == true) {
                ...
                emit EarningEvent(secondNftBuyAmount, _referral, address(0),0, block.timestamp);
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for (uint256 i = 0; i < BonusLinesCount; i++) {
            ...
            player[up].referralEarningsL[i] = sendAmount;

            up = player[up].userAddress;

            emit EarningEvent(sendAmount, _addr, up,0, block.timestamp);
        }
    }
        ...
        )
    {
        ...
        return (referralEarningsL, _player.nft200Earning, _player.totalReward);
    }
}
