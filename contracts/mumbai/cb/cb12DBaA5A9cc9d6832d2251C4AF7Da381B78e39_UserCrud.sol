...
contract UserCrud is OwnableUpgradeable {
    struct UserStruct {
        string fullName;
        string userEmail;
        string firstName;
    ...
    function initialize(address _pst) public initializer {
        require(_pst != address(0), "TOKEN: INVALID_PST");
        __Ownable_init();
        pst = IERC20Upgradeable(_pst);
    }
    ...
    // function getUser(
    //     address userAddress
    // ) public view returns (string calldata userEmail, uint userAge, uint index) {
    //     require(isUser(userAddress));
