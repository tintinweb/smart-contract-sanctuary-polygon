...
interface NFTToken{
    //function mint(address _owner, uint256 newTokenId) external;
...
contract UnikNFTMarketPlace is ownable{
    ...
    //test address on mumbai
    address payable Unikfame = payable(0xce2f09F6762341e1764729d4cf65A6a1B8Bbd18D); // 100%
    ...
    function resellToken(uint256 [] calldata tokenIds, uint256 price) public whenNotPaused{
            ...
            if(idToMarketItem[tokenId].seller == msg.sender){
              if(idToMarketItem[tokenId].isAuction == false){
                if(idToMarketItem[tokenId].isMarket == true){
                  ...
                  emit MarketTrx(tokenId,msg.sender,address(0),0,
                                  "Cancelled",
                                  block.timestamp
                  );
                }
