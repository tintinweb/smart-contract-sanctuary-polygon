...
contract Rewarder is IRewarder, Initializable {
	...
	modifier isAuth() {
		require(auths[msg.sender], 'Rewarder: caller is not allowed to call');
		_;
	}

	/// @dev proxy initialize function
	/// @param _token ERC20 token address
	/// @param _auths authorized addresses
	function initialize(IERC20 _token, address[] memory _auths) external initializer {
		_setToken(_token);
	...
	function mint(address to, uint256 amount) external override isAuth {
		token.transfer(to, amount);
		emit Minted(msg.sender, to, amount);
	}

	function _auth(address auth) internal {
		auths[auth] = true;
		emit AuthUpdated(auth);
	}
