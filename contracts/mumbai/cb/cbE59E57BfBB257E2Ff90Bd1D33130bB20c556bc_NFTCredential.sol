...
contract NFTCredential is ERC721A, Ownable {
    ...
    mapping(uint256 => bool) private _showFlg;
    mapping(uint256 => address[]) private _showAddressList;
    mapping(uint256 => address[]) private _showNameList;
    ...
    function mintAndTransfer(
        ...
        for (uint i = 0; i < requestNum; i++) {
            ...
            _setTokenURI(
                tokenId,
                generateTokenURI(_name, _description, _imageURIs[i], setexternalurl)
            );
            */
            string memory setexternalurl = generateExternalURL(_externalURIs[i], tokenId, _credentialId);
            _setTokenURI(
                tokenId,
                generateTokenURI(_name, _description, _imageURIs[i], setexternalurl)
            );
            ...
            addrlist[0] = _toAddresses[i];
            
            _setShowFlg(tokenId, false);
            _setShowAddressList(tokenId, addrlist);
    ...
    function updateNFT(
        ...
        // update the tokenURI
        _setTokenURI(tokenId, generateTokenURI(_name, _description, _imageURI, _externalURI));
    }
    ...
    function getShowFlg(uint256 tokenId)
        public
        view
        returns (bool)
    {
        ...
        return _showFlg[tokenId];
    }

    function setShowFlg(uint256 tokenId, bool showflg) public {
        /*
        address _tokenOwner = ownerOf(tokenId);
        require(
            _msgSender() == _tokenOwner || _msgSender() == owner(),
            "The sender is NOT the token ID's owner."
        );
        */

        _setShowFlg(tokenId, showflg);
    }

    function _setShowFlg(uint256 tokenId, bool showflg) internal {
        require(_exists(tokenId), "ShowFlg set of nonexistent token");
        _showFlg[tokenId] = showflg;
    }

    function getShowAddressList(uint256 tokenId)
        public
        view
        returns (address[] memory)
    {
        require(_exists(tokenId));
        uint list_length = _showAddressList[tokenId].length;
        address[] memory retAddress = new address[](list_length);
        retAddress = _showAddressList[tokenId];

        return retAddress;
    }

    function setShowAddressList(uint256 tokenId, address[] memory addrlist) public {
        /*
        address _tokenOwner = ownerOf(tokenId);
        require(
            _msgSender() == _tokenOwner || _msgSender() == owner(),
    ...
    function _setShowAddressList(uint256 tokenId, address[] memory addrlist) internal {
        require(_exists(tokenId), "URI set of nonexistent token");

        uint list_length = _showAddressList[tokenId].length;

        for (uint i = 0; i < list_length; i++) {
            _showAddressList[tokenId].pop();
        }

        for (uint i = 0; i < addrlist.length; i++) {
            _showAddressList[tokenId].push(addrlist[i]);
        }
    }

    function getShowNameList(uint256 tokenId)
        public
        view
        returns (address[] memory)
    {
        require(_exists(tokenId));
        uint list_length = _showNameList[tokenId].length;
        address[] memory retNameList = new address[](list_length);
        retNameList = _showNameList[tokenId];

        return retNameList;
    }

    function setShowNameList(uint256 tokenId, address[] memory namelist) public {
        /*
        address _tokenOwner = ownerOf(tokenId);
        require(
            _msgSender() == _tokenOwner || _msgSender() == owner(),
        ...
        */
        ...
        _setShowNameList(tokenId, namelist);
    }

    function _setShowNameList(uint256 tokenId, address[] memory namelist) internal {
        require(_exists(tokenId), "URI set of nonexistent token");

        uint list_length = _showNameList[tokenId].length;

        for (uint i = 0; i < list_length; i++) {
            _showNameList[tokenId].pop();
        }

        for (uint i = 0; i < namelist.length; i++) {
            _showNameList[tokenId].push(namelist[i]);
        }
    }
    ...
    function getShowNFTs()
        public
        view
        returns (string[] memory)
    {
        uint total_token_num = _totalMinted();
        string[] memory nfts = new string[](total_token_num + 1);

        uint nft_num = 0;
        for (uint i = 0; i <= total_token_num; i++) {
            if (_exists(i)) {
        ...
        string[] memory retNftList = new string[](nft_num);
        for (uint i = 0; i < nft_num; i++) {
            retNftList[i] = nfts[i];
        }
