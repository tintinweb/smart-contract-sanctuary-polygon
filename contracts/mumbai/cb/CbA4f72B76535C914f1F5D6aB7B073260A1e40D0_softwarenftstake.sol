...
contract softwarenftstake {
    ...
    nftContract.transferFrom(msg.sender, address(this), _tokenId);
        ...
        stakingContractDetails[index] = stakingContract(
          ...
          stakingRewards[msg.sender][index] =  rewardDetails(_tokenId,block.timestamp,block.timestamp,block.timestamp + 15 minutes,0,0,block.number,0);
...
function ShowmintProduction(address _add, uint256 _index) public view returns(uint256 ,uint256){
uint256 baseTokenEmission =(stakingContractDetails[_index].baseTokenEmission);
uint256 _mimValue = mimValue(_add ,_index)  ;
uint256 _tokenId =  stakingRewards[_add][_index].tokenId;
uint256 diff = nftContract.getEmmissionDate(_tokenId) - block.timestamp ;
uint256 mintProductionValue;
if(p2>p1){
mintProductionValue = (baseTokenEmission * ( _mimValue - carryingCapacity())  * p2/p1  / 10);
...
mintProductionValue = (baseTokenEmission * ( _mimValue - carryingCapacity())  / 10) ;
    ...
    return (mintProductionValue, diff);
}


    function claim(uint256 _index) public {
        require(stakingRewards[msg.sender][_index].lastRewardClaim  + 60 < block.timestamp , "reward claim after 60 second ") ;
        uint256 amount = mintProduction(msg.sender , _index);
        require(amount > 0, " amount greater than 0");
        LFI.transfer(msg.sender, amount);
        stakingRewards[msg.sender][_index].claimedReward += amount ;
