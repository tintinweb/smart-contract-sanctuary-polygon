...
╚╗╔╝║║ ║║║╚═╝║║║   ╔╣╠╗║╚═╝║║║║║║║
 ╚╝ ╚╝ ╚╝╚═══╝╚╝   ╚══╝╚═══╝╚╝╚╝╚╝
$HOPIUM - a People's Treasury(TM) token, in conjunction with "Farming Is A Habit" community and "Defi Informer" media company.
...
/// @notice The $HOPIUM Token is awarded in NFT's and is a precursor to the $FAM token.
...
contract Hopium is ReentrancyGuard, ERC20, ERC20Burnable, ERC20Snapshot, AccessControl, Taxable, PaymentSplitter {
    ...
    bytes32 public constant EXCLUDER_ROLE = keccak256("EXCLUDER_ROLE");   // Excluder role is a lite admin role that can add/remove addresses to the exclusion list.
    ...
    constructor(address[] memory _payees, uint256[] memory _shares, address _multisig, address _treasury, bool taxed_, uint default_tax_, uint maxtax_, uint mintax_)
        ERC20("$HOPIUM", "$HOPIUM")
        ...
        payable
    {
        ...
        _grantRole(SNAPSHOT_ROLE, _treasury);
        _grantRole(SNAPSHOT_ROLE, _multisig);
        ...
        _mint(_treasury, 10000 * 10 ** decimals()); // Mint 10,000 $HOPIUM to treasury hot wallet.
    }
...
/// @dev Add the public function for the EXCLUDER_ROLE to add/remove excluded addresses from tax:

    function addExcludedAddress(address newexcluded) public onlyRole(EXCLUDER_ROLE) {
        _grantRole(EXCLUDED_ROLE, newexcluded);
    }

    function removeExcludedAddress(address oldexcluded) public onlyRole(EXCLUDER_ROLE) {
