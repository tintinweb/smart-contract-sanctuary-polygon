...
contract ProtoPoolStaking is Ownable, ReentrancyGuard {
    ...
    address public SSVTokenADDR;
    address public SSVContractAddress;
    uint256[] operatorIDs;
    Validator[] public validators;
    address public oracleNodeAddress;
    uint256 public ETHBalance;
    uint256 public beaconETHBalance;

    event UserStaked(address user_address, uint256 amount);
    event PubKeyDeposited(bytes pubkey);
    event operatorIDsChanged(uint256[] newOperators);
    ...
    constructor(
        ...
        SSVContractAddress = ssvContract;
        SSVTokenADDR = ssvToken;
        operatorIDs = ids;
    }
     ...
     */
    function getOperators() public view returns (uint256[] memory) {
        return operatorIDs;
    }
    ...
    */
    ...
    /**
     * @notice Get the total ETH amount
     */
    function totalETH() public view returns (uint256) {
        return beaconETHBalance + ETHBalance;
    }
     ...
     */
    function updateOperators(uint256[] memory _newOperators) external onlyOwner {
        operatorIDs = _newOperators;
        emit operatorIDsChanged(_newOperators);
    }

    /**
     * @notice setting oracle node address
     * @param _oracleNodeAddress: address of oracle node
     */
    ...
    /**
     * @notice Throws if called by any sender that isn't a trusted node DAO member
    */
    modifier onlyTrustedNode(address _nodeAddress) {
        require(_nodeAddress == oracleNodeAddress, "ProtoPoolStaking: Invalid trusted node");
        _;
    }

    /**
     * @notice Throws if called by any sender that isn't a white list key generator address
    */
    modifier onlyWhitelistAddress() {
        require(msg.sender == whitelistKeyGenerator, "ProtoPoolStaking: address is not Key Generator Address");
        _;
    }

    /**
     * @notice set whitelist key generation address
     * @param _whitelistKeyGenerator: whitelist address
    */

    function setWhitelistKeyGenerator(address _whitelistKeyGenerator) external onlyOwner {
        whitelistKeyGenerator = _whitelistKeyGenerator;
    }

    /**
     * @notice oracle will update the total beacon chain ETH balances
     * @param _beaconETHBalance: beacon chain ethereum total supply (including: validator + rewards)
    */

    function updateBalances(uint256 _beaconETHBalance) external onlyTrustedNode(msg.sender) {
        beaconETHBalance = _beaconETHBalance;
    }

    /**
     * @notice Get PETH amount
     * @param _ethAmount: ethereum amount
     */
    function getPETHValue(uint256 _ethAmount) public view returns (uint256) {
        // Use 1:1 ratio if no pETH is minted
        if (totalETH() == 0) { return _ethAmount; }

        // Calculate and return
        return _ethAmount.mul(pETH.totalSupply()).div(totalETH());
    }

    /**
     * @notice Get ETH amount
     * @param _pETHAmount: PETH amount
     */
    function getETHValue(uint256 _pETHAmount) public view returns (uint256) {
        // Use 1:1 ratio if no pETH is minted
        if (pETH.totalSupply() == 0) { return _pETHAmount; }

        // Calculate and return
        return _pETHAmount.mul(totalETH()).div(pETH.totalSupply());
    }
    ...
    function stake() external payable {
        require(msg.value > 0, "ProtoPoolStaking: shouldn't stake zero amount");
        pETH.mint(msg.sender, getPETHValue(msg.value));
     ...
     * @param _pETHAmount: PETH Amount to be swap
     */

    function unStake(uint256 _pETHAmount) public {
        pETH.transferFrom(msg.sender, address(this), _pETHAmount);
        uint256 _amount_to_transfer = getETHValue(_pETHAmount);

        require(ETHBalance >= _amount_to_transfer, "ProtoPoolStaking: can't swap PETH");

        payable(msg.sender).transfer(_amount_to_transfer);

        // update the balances
        ETHBalance -= _amount_to_transfer;
        pETH.burn(_pETHAmount);
    }

    /**
     * @notice Deposit a validator to the deposit contract, the validator will be taken from validator's pool
     */
    function depositValidator() external {
        require(ETHBalance >= VALIDATOR_AMOUNT, "ProtoPoolStaking: not enough balance");
        require(validators.length > 0, "ProtoPoolStaking: not active validator");

        Validator memory validator = validators[validators.length - 1];
        validators.pop();

        ETHBalance -= VALIDATOR_AMOUNT;
        ...
        emit PubKeyDeposited(validator.pubkey);
    }
    ...
    function depositShares(
        ...
        IERC20(SSVTokenADDR).approve(SSVContractAddress, _amount);
        // Register the validator and deposit the shares
        ISSVNetwork(SSVContractAddress).registerValidator(
            _pubkey,
            _operatorIDs,
            _shares,
            _amount,
