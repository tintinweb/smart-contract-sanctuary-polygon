...
contract GeniosClub is IGeniosClub {
  ...
  function sendTokenDividends(
    ...
    if (matrix == 1) {
      ...
      if (userAddr == Id1) {
        updateG3X7Pool(userAddr, level);
        updateG3X7Club(userAddr, level);
        updateG3X7Academy(userAddr, level);

        uint256 amount = calPerc(LevelPrice[level], 50);
        TransferHelper.safeTransfer(TOKEN, userAddr, amount);

        Platform.G3X7TotalEarnings += amount;
        Users[userAddr].G3X7Earnings += amount;
        Users[userAddr].G3X7Matrix[level].Earnings += amount;
      }
