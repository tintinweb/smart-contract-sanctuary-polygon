...
contract BridgeShibes is Ownable {
    ...
    event Bridge(address owner, uint tokenID, uint timestamp) ; 

    function bridge(uint tokenID) public returns (bool) {
        require(online, "bridge offline") ; 
        require(shibes.ownerOf(tokenID) == msg.sender, "sender not owner") ; 

        emit Bridge(msg.sender, tokenID, block.timestamp) ;
