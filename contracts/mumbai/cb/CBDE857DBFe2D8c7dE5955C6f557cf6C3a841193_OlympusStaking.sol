...
contract OlympusStaking is OlympusAccessControlled {
    ...
    event TaxesUpdated(uint256 stakeTax, uint256 earlyUnstakeTax, uint256 unstakeTax);
    ...
    uint256 public stakingTax = 6;  // non wei
    uint256 public unstakingTaxEarly = 24; // non wei
    uint256 public unstakingTax = 12; // non wei
    ...
    function stake(
        ...
        uint256 tax = _amount.mul(stakingTax).div(100);
        OHM.safeTransferFrom(msg.sender, address(this), tax);
        uint newAmount = _amount.sub(tax);
        OHM.safeTransferFrom(msg.sender, address(this), newAmount);
        newAmount = newAmount.add(rebase()); // add bounty if rebase occurred
        if (_claim && warmupPeriod == 0) {
            return _send(_to, newAmount, _rebasing);
        } else {
            ...
            warmupInfo[_to] = Claim({
                deposit: info.deposit.add(newAmount),
                gons: info.gons.add(sOHM.gonsForBalance(newAmount)),
                expiry: epoch.number.add(warmupPeriod),
                lock: info.lock
            });

            gonsInWarmup = gonsInWarmup.add(sOHM.gonsForBalance(newAmount));
    ...
    function unstake(
        ...
        if(epoch.number < info.expiry){
            tax = (info.deposit).mul(unstakingTaxEarly).div(100);
        }
        else if(epoch.number > info.expiry){
            tax = (info.deposit).mul(unstakingTax).div(100);
        }
    ...
    function setTaxes (uint256 _stakingTaxPercentage, uint256 _unstakingTaxPercentageEarly, uint256 _unstakingTaxPercentage) external onlyGovernor {
        stakingTax = _stakingTaxPercentage;
        unstakingTaxEarly = _unstakingTaxPercentageEarly;
        unstakingTax = _unstakingTaxPercentage;
        emit TaxesUpdated(stakingTax, unstakingTaxEarly, unstakingTax);
    }
}
