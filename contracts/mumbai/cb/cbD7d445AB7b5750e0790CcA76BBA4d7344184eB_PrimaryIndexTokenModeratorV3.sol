...
contract PrimaryIndexTokenModeratorV3 is Initializable, AccessControlUpgradeable {
	...
	event SetDepositLimitPerProjectAsset(address projectToken, uint256 _depositLimit);
	...
	event LendingTokenLoanToValueRatioSet(
	...
	function removeProjectToken(
		...
		require(
			primaryIndexToken.totalDepositedPerProjectToken(projectToken) == 0,
			"PIT: projectToken amount exist on PIT"
		);
	...
	function addLendingToken(
		...
		require(
			_lendingToken != address(0) && _bLendingToken != address(0),
			"PIT: invalid address"
		);
	...
	function removeLendingToken(
		...
		for (uint256 i = 0; i < primaryIndexToken.projectTokensLength(); i++) {
			require(
				primaryIndexToken.totalBorrow(primaryIndexToken.projectTokens(i), lendingToken) ==
					0,
				"PIT: exist borrow of lendingToken"
			);
		}
	...
	function setProjectTokenInfo(
		...
		require(
			_loanToValueRatioNumerator <= _loanToValueRatioDenominator,
			"invalid loanToValueRatio"
		);
	...
	function setLendingTokenInfo(
		...
		primaryIndexToken.setLendingTokenInfo(
			_lendingToken,
			_bLendingToken,
			_isPaused,
			_loanToValueRatioNumerator,
		...
		require(
			IBLendingToken(_bLendingToken).underlying() == _lendingToken,
			"PIT: underlyingOfbLendingToken!=lendingToken"
		);
		emit SetPausedLendingToken(_lendingToken, _isPaused);
		emit LendingTokenLoanToValueRatioSet(
			_lendingToken,
			_loanToValueRatioNumerator,
			_loanToValueRatioDenominator
		);
	...
	function setDepositLimitPerProjectAsset(
		...
		require(_depositLimit > 0, "PIT: borrowLimit=0");
		require(projectToken != address(0), "PIT: invalid address");
		primaryIndexToken.setDepositLimitPerProjectAsset(projectToken, _depositLimit);
		emit SetDepositLimitPerProjectAsset(projectToken, _depositLimit);
	}
}