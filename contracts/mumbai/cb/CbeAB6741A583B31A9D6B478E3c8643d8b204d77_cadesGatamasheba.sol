...
contract cadesGatamasheba is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function callAllWinners() public view returns (uint256[] memory) {
        return winners;
    }


    function pickWinnerIndex(uint256 _playerslength) public onlyOwner returns (uint) {
        uint index = requestRandomWords() % _playerslength;
        winners.push(index);
        return index;
    }
