...
contract Demo is AccessControl  {
    ...
    bytes32 public constant DOCOTOR_ROLE = keccak256("DOCOTOR_ROLE");
    bytes32 public constant PATIENT_ROLE = keccak256("PATIENT_ROLE");
  
    // `Admin` address that has every access
    ...
    constructor() {
        ...
        _setupRole(DOCOTOR_ROLE,    doctors[msg.sender].doctorAddress);
    ...
    function updateHospitalInfo(
       string memory _name,
        ...
        require(hasRole(DOCOTOR_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender), "Caller is not a minter");
        // require(msg.sender == doctors[msg.sender].doctorAddress || msg.sender == ADMIN,"Only `Doctor` Or `Admin` have the Access" );
    ...
    function addNurse(
       ...
       function updateNurseInfo(  
        ...
        require(hasRole(NURSE_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender), "Caller is not a minter");

        // require(msg.sender == nurses[_address].nurseAddress || msg.sender == ADMIN ,"Only `Nurse` Or `Admin` can have the Access");
    ...
    function addDoctor(
     ...
     function updateDoctorInfo(
        ...
        // require(msg.sender == doctors[_address].doctorAddress || msg.sender == ADMIN,"Only `Doctor` Or `Admin` have the Access" );
    ...
    function addPatient(
     ...
     function UpdatePatientInfo(  
        ...
        require(hasRole(NURSE_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender) || hasRole(PATIENT_ROLE, msg.sender), "Caller is not a minter");
        // require(msg.sender == patients[_address].patientAddress || msg.sender == nurses[msg.sender].nurseAddress || msg.sender == ADMIN,"Only Patient Registered patients Or Nurses can Access");
  ...
  // [MEDICAL-HISTORY- SECTION]
    ...
    function updateMedicalHistory(
        ...
        require(hasRole(DOCOTOR_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender) || hasRole(NURSE_ROLE, msg.sender), "Caller is not a minter");

        // require(msg.sender == doctors[msg.sender].doctorAddress || msg.sender == nurses[msg.sender].nurseAddress || msg.sender == ADMIN, "Only `Doctor`, `Nurse` and `Admin have the Access");
