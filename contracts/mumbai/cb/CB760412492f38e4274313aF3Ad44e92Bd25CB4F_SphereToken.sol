...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    function getUserTotalOnDifferentContractsSphere(address sender) public view returns (uint256) {
        ...
        balanceOfAllSphereGamesContracts = getBalanceOfAllSphereGamesContracts(sender);

        userTotal.add(balanceOfAllSubContracts).add(balanceOfAllSphereGamesContracts);
        return userTotal;
    }
    ...
    returns (uint256)
    {
        ...
        for (uint256 i = 0; i < sphereGamesContracts.length; i++) {
            sphereGamesTotal += IPublicBalance(sphereGamesContracts[i]).balanceOf(
                sender
            );
        }
    ...
    function _transferFrom(
        ...
        automatedMarketMakerPairs[recipient])
            ...
            require(amount <= maxSellTransactionAmount.div(50), 'Party Sell Error amount');
        }
        ...
        if (partyArrayCheck[sender] || partyArrayCheck[recipient]) {
            require(amount <= maxBuyTransactionAmount.div(50), 'Party Buy Error amount');
        }
    ...
    function takeFee(
        ...
        if (
            (partyArrayCheck[sender] || partyArrayCheck[recipient])
        ) {
    ...
    function addPartyAddies(address _partyAddy, bool _value) private {
    ...
    function setFeeReceivers(
        ...
        require(_riskFreeValueReceiver != address(0), '_riskFreeValueReceiver not set');
    ...
    function setStablecoin(address _stableCoin) external onlyOwner {
        require(_stableCoin != address(0x0),
            'can not use 0x0 address'
        );
