    ...
    Date
{
    ...
    returns (uint256)
    {
        uint256 birthdayYear = getYear(lifeSpan.getBirthdayDate(lifeSpanId));
        uint256 birthdayMonth = getMonth(lifeSpan.getBirthdayDate(lifeSpanId));
        uint256 birthdayDays = getDay(lifeSpan.getBirthdayDate(lifeSpanId));

        uint256 mintYear = getYear(lifeSpan.getDateOfMint(lifeSpanId));
        uint256 mintMonth = getMonth(lifeSpan.getDateOfMint(lifeSpanId));
        uint256 mintDays = getDay(lifeSpan.getDateOfMint(lifeSpanId));

        uint256 dateYear = getYear(block.timestamp);
        uint256 dateMonth = getMonth(block.timestamp);
        uint256 dateDays = getDay(block.timestamp);
