...
contract Referral_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private _countReferral;
    bytes32 public constant SUPER_MANAGER_ROLE = keccak256("SUPER_MANAGER_ROLE");
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    event FundInFlow(address indexed caller,address indexed asset,address indexed referer,string policyId, uint256 value);
    ...
    function allReferral() public view returns(address[] memory) {
        ...
        for (uint i = 0;i<_allReferral.length;i++){
    ...
    function applyWorking(address _user) public  isSuperManager {
        ...
        // (,bool isaccessreferral) = S.accessRole(_user,IStaking.typeUser.RF);
        // require(isaccessreferral == true,"Staking : You not have access for register.");
    ...
    function resignation(address _user) public  isSuperManager {
        require(PDS.isBlacklistOfUser(_user) != true,"Policy: This _user are Blacklist.");
        // (,bool isaccessreferral) = S.accessRole(_user,IStaking.typeUser.RF);
        // require(isaccessreferral == true,"Staking : You are not UnderWriter this moment.");
        ...
        for (uint256 i = 0; i < _allReferral.length; i++) {
            if (_user == _allReferral[i]) {
                _allReferral[i] = _allReferral[
                    _allReferral.length - 1
                ];
    ...
    function fundInFlow(address _caller,address _asset, address _referer,string memory _policyId, uint256 _value) public isPolicy { // only Policy can Manage // 
        require(_caller != _referer,"Referral: _from != _referer");

        if(_referer != address(0)){
            require(isReferral(_referer) == true,"You Refferal are not Refferer");
            S.update(_referer, IStaking.typeUser.RF);
        }

        historyValueOfUser[_policyId] = _value;
        valueOfRefer[_asset][_referer] += _value;
        referTo[_policyId] = _referer;


        emit FundInFlow(_caller,_asset,_referer,_policyId,_value);
    }
    ...
    function isReferral(address _user) public view returns(bool) {
        (IStaking.StakingData memory _StakingData) = S.stakingData(_user,IStaking.typeUser.RF);
        (IStaking.WorkType memory _dataType) = S.workData(IStaking.typeUser.RF);
    ...
    function withdraw(address _asset, uint256 _value) public nonReentrant checkAsset(_asset) { // for Referral //
        IERC20 _ERC20 = IERC20(_asset);
        IPolicyDetails _PDS_ = PDS;
        require(_PDS_.isBlacklistOfUser(msg.sender) != true,"Policy: This _user are Blacklist.");
        require(_PDS_.isBlacklistAssetsOfUser(msg.sender,_asset) != true,"Policy: This Assets of you are Blacklist.");
        ...
        emit Withdraw(_asset,msg.sender,_value,valueOfRefer[_asset][msg.sender]);
    }

    function withdrawOther(address _asset, uint256 _value) public nonReentrant  isSuperManager checkAsset(_asset) {
        IERC20 _ERC20 = IERC20(_asset);
        require(valueOfRefer[_asset][address(0)] >= _value);

        valueOfRefer[_asset][address(0)] -= _value;
        _ERC20.transfer(msg.sender,_value);
        emit Withdraw(_asset,address(0),_value,valueOfRefer[_asset][address(0)]);
    }
    ...
    function balanceOfReferral(address _asset,address _user) public view returns(uint256) {
