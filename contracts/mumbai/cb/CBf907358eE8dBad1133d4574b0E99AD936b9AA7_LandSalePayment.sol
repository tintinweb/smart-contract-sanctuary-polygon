...
contract LandSalePayment is Ownable, ReentrancyGuard{
    ...
    constructor(address _FTaddress, address _NFTaddress, string[] memory _category,bool[] memory _slotDependency, uint256[] memory _priceInUSD, uint256[] memory _maxCategorySupply/*, uint256[] memory _slot, uint256[] memory _startTime, uint256[] memory _endTime, uint256[][] memory _slotSupply*/){
        ...
        //     //addNewSlot(_slot[index], _startTime[index], _endTime[index], _category, _slotSupply[index]);
        //     slot[_slot[index]].startTime = _startTime[index];
        //     slot[_slot[index]].endTime = _endTime[index];
        //     slotCount++;

        //     for(uint index2=2; index2<_category.length; index2){

        //         slot[_slot[index]].slotSupply[_category[index2]].maxSlotCategorySupply = _slotSupply[index][index2];
                
        //     }
        // }

        // set(_category,_slot,_startTime,_endTime,_slotSupply);
        set();

    }

    // function set(string[] memory _category,uint256[] memory _slot, uint256[] memory _startTime, uint256[] memory _endTime, uint256[][] memory _slotSupply) internal {
    ...
    function mintLand(uint256 _slot,string memory _category, uint256 _tokenId) internal {
        ...
        require(slot[_slot].slotSupply[_category].mintedSlotCategorySupply.add(1) < slot[_slot].slotSupply[_category].maxSlotCategorySupply,"");
        require(totalSupply.add(1) < maxSupply,"");
