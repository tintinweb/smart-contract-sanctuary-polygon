...
contract UserStorage {
    ...
    int256 public activeUsers;
    int256 public deletedUsers;
    ...
    function isUser(address userAddress) external view onlyOwner returns (bool) {
    ...
    function _isUser(address userAddress) private view onlyOwner returns (bool) {
        // if (userIndex.length == 0) return false;
        // require(userIndex.length != 0, "Whitelist: You need to be whitelisted");
        if (userStructs[userAddress].createdAt > 0) {
            return true;
