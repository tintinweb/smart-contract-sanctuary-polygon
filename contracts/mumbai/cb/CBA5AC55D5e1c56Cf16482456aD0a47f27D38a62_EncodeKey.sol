...
contract EncodeKey is ERC721, Ownable {
  ...
  function lastIndex() public view returns (uint256) {
    return startOfBlock[3] + keyTypes[3];
  }
  ...
  function tokensOfOwner(address _owner) public view returns (uint256[] memory) {
	...
	function mint(address _to, uint8 _quantity, uint8 _type) public onlyOwner {
    require(_type == 3 || startOfBlock[_type] + keyTypes[_type] + _quantity <= startOfBlock[_type + 1], "not enough tokens to mint");
  ...
  function multiMint(address[] calldata _to, uint8[] calldata _quantity, uint8[] calldata _types) public onlyOwner {
    ...
    for (uint8 i = 0; i < _to.length; i++) {
      address to = _to[i];
      uint8 quantity = _quantity[i];
      uint8 _type = _types[i];
      require(_type == 3 || startOfBlock[_type] + keyTypes[_type] + quantity <= startOfBlock[_type + 1], "not enough tokens to mint");
  ...
  function _baseURI() internal pure override returns (string memory) {
    return "https://octopus-app-du49r.ondigitalocean.app/metadata/keys/";
  }
}
