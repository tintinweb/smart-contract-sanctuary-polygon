...
contract Mutants is ERC721Enumerable, Ownable {
  ...
  function setAddresses(address _mintableToken, address _burnNFT) public onlyOwner{
  ...
  function mintETH(address _to, uint256 _mintAmount) public payable {
    ...
    require(erc20_supply + _mintAmount <= ETH_MAX_SUPPY, "Exceeds total eth supply");
  ...
  function mintERC20(
    ...
    require(_mintAmount <= maxMintAmount, "Mint amount exceeds max per transaction");
    require(supply + _mintAmount <= maxSupply, "Mint amount exceeds max supply");
    require(erc20_supply + _mintAmount <= ERC20_MAX_SUPPY, "Exceeds total token supply");

    require(IERC20(mintableToken).balanceOf(msg.sender) >= costOfMintableToken * _mintAmount, 'not enough chiz');
    require(IBurnNFT(burnNFT).balanceOf(msg.sender) >= _mintAmount, 'not enough incubator nfts');


    IERC20(mintableToken).transferFrom(msg.sender, address(this), costOfMintableToken * _mintAmount);

    for (uint256 i = 1; i <= _mintAmount; i++) {
        IBurnNFT(burnNFT).burn(IBurnNFT(burnNFT).tokenOfOwnerByIndex(msg.sender, 0));
        _safeMint(_to, supply + i);
