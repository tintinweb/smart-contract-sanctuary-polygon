    ...
    PausableUpgradeable
{
    ...
    /// @custom:oz-renamed-from totalSubscription
    uint256[] private totalSubscriptions;
    ...
    function initialize(address gameStateToken) public initializer {
        ...
        contractDeploymentTime = (block.timestamp / day) * day;
    ...
    function addScheme(
        ...
        scheme.name = name;
        scheme.vestingDuration = vestingDuration * day;
        scheme.period = period * day;
    ...
    function addSubscription(
        ...
        subscription.cliffTime = cliffTime * day;

        if (scheme.schemeType == SchemeType.investor) {
            subscription.startTimeVesting =
                ((subscription.startTime + subscription.cliffTime) / day) *
                day;
        } else if (scheme.schemeType == SchemeType.member) {
            subscription.startTimeVesting =
                ((contractDeploymentTime + subscription.cliffTime) / day) *
                day;
        }
        ...
        totalSubscriptions.push(subscriptionID);
    ...
    function claim(uint256 subscriptionID) external whenNotPaused {
        ...
        _claim(subscriptionID, block.timestamp);
    }

    function autoClaim() external whenNotPaused onlyOperator {
        for (uint256 index = 0; index < totalSubscriptions.length; index++) {
            Subscription memory subscription = _subscriptions[
                totalSubscriptions[index]
            ];
            if (
                block.timestamp > subscription.nextTimeClaim &&
                subscription.isActive == true
            ) _claim(totalSubscriptions[index], block.timestamp);
            else require(false, "No subscriptions can be vested at this time");
        }
    }
    ...
    function isWalletOnVesting(address wallet) external view returns (bool) {
    ...
    function getClaimableAmount(uint256 totalAmount, uint256 totalDays)
        public
        pure
        returns (uint256 claimableAmount)
    {
        claimableAmount = _calculateClaimableAmount(totalAmount, totalDays);
    }
    ...
    function getTotalSubscription() public view returns (uint256[] memory) {
        return totalSubscriptions;
    }

    function _calculateClaimableAmount(uint256 totalAmount, uint256 totalDays)
        internal
        pure
        returns (uint256 claimAvailable)
    {
        claimAvailable = (totalAmount / totalDays);
    }

    function _calculateDaysPassed(uint256 endTime, uint256 nowTime)
        internal
        pure
    ...
    function _claim(uint256 subscriptionID, uint256 blockTime) internal {
        Subscription storage subscription = _subscriptions[subscriptionID];
        Scheme memory scheme = _schemes[subscription.schemeId];

        require(subscription.isActive == true, "vesting is unavailable");
        require(subscriptionID > 0, "Requiring at least one subscription ID");
        require(subscription.totalAmount > 0, "totalAmount is zero");
        require(
            block.timestamp > subscription.nextTimeClaim,
            "It's not time to claim"
        );

        if (blockTime > subscription.endTimeVesting) {
            uint256 totalClaim = subscription.totalAmount;
            subscription.vestedAmount += totalClaim;
            subscription.totalAmount -= totalClaim;
            subscription.numberOfclaimed += 1;
            subscription.nextTimeClaim = 0;
            subscription.isActive = false;

            uint256 index = _findIndexArray(
                totalSubscriptions,
                totalSubscriptions.length,
                subscriptionID
            );
            _remove(totalSubscriptions, index);

            erc20Token.safeTransfer(subscription.wallet, totalClaim);
            emit ClaimSucceeded(
                subscription.wallet,
                totalClaim,
            ...
            uint256 currentTotalDays = _calculateDaysPassed(
                subscription.endTimeVesting,
                block.timestamp
            );
            uint256 claimAvailable = _calculateClaimableAmount(
                subscription.totalAmount,
                currentTotalDays
            );
            ...
            subscription.nextTimeClaim =
                ((block.timestamp + scheme.period) / day) *
                day;
            ...
            subscription.numberOfclaimed += 1;
            erc20Token.safeTransfer(subscription.wallet, claimAvailable);
