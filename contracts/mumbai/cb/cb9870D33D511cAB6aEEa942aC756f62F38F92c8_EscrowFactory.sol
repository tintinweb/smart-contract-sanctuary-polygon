...
contract EscrowFactory is Ownable{
    ...
    function listToken(address _contract,uint _token,uint _price,address _reserve,address _paymentToken) external {
        ...
        listingInfo[_contract][_token] = tokenInfo(msg.sender,_reserve,FEE,_price,userListings[msg.sender].length,_paymentToken);
    ...
    function buyToken(address _contract,uint _token) external payable{
        ...
        if(currInfo.paymentToken == address(0)){
            require(msg.value >= currInfo.price,"Price not paid");
            uint fee = msg.value * FEE/1000;
            (bool sent,) = (currInfo.owner).call{value: msg.value - fee}("");
            require(sent, "Failed to send Ether");
        }
        else{
            require(msg.value == 0,"Ether payment not required");
            uint fee = currInfo.price * FEE/1000;
            IERC20(currInfo.paymentToken).transferFrom(msg.sender, address(this), currInfo.price);
            IERC20(currInfo.paymentToken).transfer(currInfo.owner,currInfo.price-fee);
        }
        IERC721(_contract).transferFrom(address(this),msg.sender,_token);
        popListing(_contract, _token);
        delete listingInfo[_contract][_token];
    ...
    function withdrawTokens(address _contract) external onlyOwner{
