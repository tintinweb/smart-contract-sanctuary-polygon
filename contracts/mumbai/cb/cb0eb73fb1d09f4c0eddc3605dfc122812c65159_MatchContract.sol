...
contract MatchContract is MatchInterface{ 
	...
	MainControllerInterface.VoteRecord memory record = MainControllerInterface.VoteRecord(flag, teamA, teamB, 0, 0, block.timestamp, amount, true);
	...
	//  issue NFT ticket to account
	MainControllerInterface.VoteRecord memory record = MainControllerInterface.VoteRecord(0, teamA, teamB, _teamAScore, _teamBScore, block.timestamp, amount, false);
	ITickets(ticketNFT).newTicket(account, address(this), record);
