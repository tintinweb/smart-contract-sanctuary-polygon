...
contract LandSalePayment is Ownable, ReentrancyGuard{
    ...
    constructor(address _TVKaddress, address _NFTaddress, string[] memory _category,bool[] memory _slotDependency, uint256[4][5] memory _categoryDetail, uint256[3][13] memory _slot, uint256[5][13] memory _slotSupply){
        ...
        for(uint index=0; index<_category.length; index++){
            addNewLandCategory(_category[index],_slotDependency[index], _categoryDetail[index][0], _categoryDetail[index][1], _categoryDetail[index][2], _categoryDetail[index][3]);
        }
    ...
    function addNewLandCategory(string memory _category, bool _slotDependency, uint256 _priceInUSD, uint256 _maxCategorySupply, uint256 _categoryStartRange, uint256 _categoryEndRange) public onlyOwner{
    ...
    function decreaseLandCategorySupply(string memory _category, uint256 _additionalSupply) public onlyOwner{
        require(landCategory[_category].status,"Landsale: Invalid caetgory!");
        require(_additionalSupply>0, "Landsale: Invalid value!");
        require(landCategory[_category].maxCategorySupply.sub(landCategory[_category].mintedCategorySupply) >= _additionalSupply,"Landsale: Decreasing supply impacts the minted supply!");
