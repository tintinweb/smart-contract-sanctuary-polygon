...
contract BGI_DAO is Ownable {   
    ...
    function valuateDAO(uint valNew) public payable onlyOwner {
        valCents = valNew;
    ...
    function UserDeposit(address user, uint amount) public payable onlyOwner {
    ...
    function UserwithdrawAllMoney(address user, uint amount) public payable onlyOwner {
    ...
    function UserwithdrawMoney(address user, uint amount) public payable onlyOwner {
         require(amount <= balanceReceived[user].totalBalance, "not enough funds");
         balanceReceived[user].totalBalance -= amount;
    }  // processes capital withdrawals of (single) user from the DAO

    function transfer(address to, address from , uint amount) public payable onlyOwner {
    ...
    function addMember(address user, string memory _choice) public payable onlyOwner {
    ...
    function getCurrentMemberBalance(address user) public view onlyOwner returns (uint) {
