...
contract DexFlash is IGrid, Context{
    string public  info;

    address private immutable token0 = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB;
    address private immutable token1 = 0x1A26d803C2e796601794f8C5609549643832702C;
    ...
    function flash(address recipient, uint256 amount0, uint256 amount1, bytes calldata data) external override {
        ...
        IGridFlashCallback(msg.sender).gridexFlashCallback(data);
    ...
    function claimToken(address token, uint256 amount) payable external {
        IERC20(token).approve(address(this), amount);
        IERC20(token).transfer(msg.sender, amount);
    }


    function transfer(bool t, uint256 amount) external {
        if(t){
            SafeERC20.safeTransfer(IERC20(token0), msg.sender, amount);
        }
    }
}
