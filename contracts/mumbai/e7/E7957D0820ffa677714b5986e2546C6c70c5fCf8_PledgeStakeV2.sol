...
contract PledgeStakeV2 {
    ...
    function _updateRewards(address _user) private {
        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            ...
            if (!stake.unstaked) {
                ...
                // Check if the stake is still active before calculating the roundNumber
                if (block.timestamp < stake.startTime + ROUND_DURATION * TOTAL_ROUNDS) {
                    roundNumber = (block.timestamp - stake.startTime) / ROUND_DURATION;
                } else {
            ...
            if (user.level > 0) { // Add this condition
                address upline = user.referrer;
                for (uint256 k = 0; k < LEVEL_COUNT; k++) {
                    if (upline != address(0)) {
                        uint256 levelRewardPercentage = levelPercentage[k];
                        uint256 levelReward = roundRewards.mul(levelRewardPercentage).div(100);

                        // Check if upline's level is greater than or equal to the current level
                        if (userInfo[upline].level >= k + 1) { // Subtract 1 from the upline's level
                            // Only skip stake check for default referrer
                            ...
                            } else if (userInfo[upline].totalStake > 0) {
                                rewardInfo[upline].levelRewards = rewardInfo[upline].levelRewards.add(levelReward);
                                rewardInfo[upline].totalRewards = rewardInfo[upline].totalRewards.add(levelReward);
                                rewardInfo[upline].availableRewards = rewardInfo[upline].availableRewards.add(levelReward);
                            }
    ...
    // Function to calculate round rewards for a user
    function calculateTotalRewards(address _user) public view returns (uint256, uint256) {
        uint256 roundRewards;
        uint256 levelRewards;
        uint256 userLevel = userInfo[_user].level; // Get the user's level
        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            ...
            if (!stake.unstaked) {
                ...
                // Check if the stake is still active before calculating the roundNumber
                if (block.timestamp < stake.startTime + ROUND_DURATION * TOTAL_ROUNDS) {
                    roundNumber = block.timestamp.sub(stake.startTime).div(ROUND_DURATION);
                } else {
                    roundNumber = TOTAL_ROUNDS;
                }
                ...
                for (uint256 j = 0; j < roundNumber; j++) {
                    ...
                    if (userLevel > 0 || _user == defaultReferrer) { // Add this condition
                        ...
                        for (uint256 k = 0; k < LEVEL_COUNT; k++) {
                            if (upline != address(0)) {
                                ...
                                // Get the upline's level from the UserInfo struct
                                uint256 uplineLevel = userInfo[upline].level;

                                // Check if upline's level is greater than or equal to the current level
                                if (uplineLevel >= k + 1 || upline == defaultReferrer) { // Subtract 1 from the upline's level
                                    if (upline == defaultReferrer || userInfo[upline].totalStake > 0) {
                                        levelRewards = levelRewards.add(levelReward);
                                    }
                                }
                                upline = userInfo[upline].referrer;
                            }
