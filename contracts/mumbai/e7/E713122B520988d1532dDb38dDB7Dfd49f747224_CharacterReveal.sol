...
contract CharacterReveal is ICharacterReveal, GameBase, Random {
    ...
    function _characterReveal(uint256 worldId, uint256 tokenId) private {
        ...
        require(record.characterDefinitionIds.length == record.weights.length);
        require(record.itemPackDefinitionIds.length == record.weights.length);

        uint256 index = indexByRandom(record.weights);
