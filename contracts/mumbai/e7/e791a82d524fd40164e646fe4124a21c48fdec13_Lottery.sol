...
contract Lottery {
    ...
    function checkIfUserWin(uint _lotteryId,uint _userId,uint _numberOfTickets) private{        
        ...
        for(uint i=0;i<lotteries[_lotteryId].awards.length;i++){
            ...
            )
            {
                ...
                users[_userId].balance = users[_userId].balance + lotteries[_lotteryId].awards[i];
            }
        }
    }
    ...
    function finishLottery(uint _lotteryId) private{
        ...
        lotteries[_lotteryId].lotteryWinnerAddresses = winners[_lotteryId]; //new randon needed + add winners balance ???????????????????
