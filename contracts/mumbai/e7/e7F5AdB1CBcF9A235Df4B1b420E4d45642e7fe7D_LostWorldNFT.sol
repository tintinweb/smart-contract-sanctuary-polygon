...
import './interface/ILostWorldValidator.sol';
import './lib/CustomERC2771ContextUpgradeable.sol';

contract LostWorldNFT is
    ...
    UUPSUpgradeable
{
    ...
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LWNFT: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LWNFT: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LWNFT: forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LWNFT: RBA linker is zero address');
        require(worldDataAddress != address(0x0), 'LWNFT: World data is zero address');
        require(validatorAddress != address(0x0), 'LWNFT: Validator is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
        LWD = ILostWorldData(worldDataAddress);
        LWV = ILostWorldValidator(validatorAddress);
        initializeERC2771(address(forwarder));

        __ERC721_init('Lost Land', 'LL');
    ...
    {
        sender = CustomERC2771ContextUpgradeable._msgSender();
    }
    ...
    ILostWorldValidator private LWV;

    mapping(uint256 => string) private _tokenURIs;
    CountersUpgradeable.Counter private _landId;
    CountersUpgradeable.Counter private _worldId;
    ...
    mapping(uint256 => LostLand) lostLands;
    mapping(uint256 => LostWorld) lostWorlds;
    ...
    mapping(uint256 => LandData) landData;

    function setLWD(address lostWorldDataAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostWorldDataAddress != address(0x0), 'LWNFT: Lost world data is zero address');
        LWD = ILostWorldData(lostWorldDataAddress);
    }
    ...
    function addWorld(
        ...
        uint256 worldId = _worldId.current();
        require(!lostWorlds[worldId].exists, 'LWNFT: World already exists');

        _worldId.increment();
        lostWorlds[worldId].worldId = worldId;
        lostWorlds[worldId].exists = true;
        lostWorlds[worldId].worldName = worldName;
        lostWorlds[worldId].worldDescription = worldDescription;
        lostWorlds[worldId].sizeX = sizeX;
        lostWorlds[worldId].sizeY = sizeY;
        lostWorlds[worldId].imageUri = imageUri;
    }
    ...
    function mintLand(
        ...
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');
        require(
            landId.length == x.length && x.length == y.length && y.length == uri.length,
            'LWNFT: Array lengths mismatch'
        );

        for (uint256 i = 0; i < landId.length; i++) {
            lostLands[landId[i]].exists = true;
            lostLands[landId[i]].worldId = worldId;
            lostLands[landId[i]].x = x[i];
            lostLands[landId[i]].y = y[i];
            lostWorlds[worldId].landIds.push(landId[i]);

            LWD.initRecharge(msg.sender, landId[i]);

            _landId.increment();
            _safeMint(to, landId[i]);
            _setTokenURI(landId[i], uri[i]);
        }
    }

    function getLatestLandId() public view returns (uint256) {
        return _landId.current();
    }

    function getWorlds() public view returns (LostWorld[] memory) {
        uint256 worldAmount = _worldId.current();
        require(worldAmount > 0, 'LWNFT: No world exists');

        LostWorld[] memory ret = new LostWorld[](worldAmount);
        for (uint256 i = 0; i < worldAmount; i++) {
            ret[i] = lostWorlds[i];
        }
    ...
    function getLandNewPos(uint256 worldId) public view returns (uint16 x, uint16 y) {
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');
        LostWorld memory world = lostWorlds[worldId];
        for (uint16 i = 0; i < world.sizeX; i++) {
            for (uint16 j = 0; j < world.sizeY; j++) {
                uint256 landId = i * world.sizeY + j;
                if (!lostLands[landId].exists) {
                    x = i;
                    y = j;
                    return (x, y);
                }
    ...
    function getWorldMaxMint(uint256 worldId) public view returns (uint256) {
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');
        LostWorld memory ret = lostWorlds[worldId];
        uint256 maxSize = ret.sizeX * ret.sizeY;

        return maxSize - lostWorlds[worldId].landIds.length;
    }

    function getLostWorld(uint256 worldId) public view returns (LostWorld memory) {
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');
        return lostWorlds[worldId];
    }

    function getLostWorldLands(uint256 worldId) public view returns (LostLand[] memory) {
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');

        uint256[] memory landIds = lostWorlds[worldId].landIds;
        require(landIds.length > 0, 'LWNFT: World does not contain and land');

        LostLand[] memory ret = new LostLand[](landIds.length);
        for (uint256 i = 0; i < landIds.length; i++) {
            LostLand memory land = lostLands[landIds[i]];
            land.owner = ownerOf(landIds[i]);
            ret[i] = land;
        }
    ...
    function editLand(
        ...
        uint256 landPrice,
        bool playable
    ) public onlyRole('TRUSTED_FORWARDER', msg.sender) {
        require(lostLands[landId].exists, 'LWNFT: Land does not exists');
        require(_msgSender() == ownerOf(landId), 'LWNFT: Not the land owner');
        require(landPrice > 0, 'LWNFT: Land price is 0');
        LWV.validateLand(rewardAmount, maxRewardAmount, enemyList, rewardList);

        landData[landId].rewardAmount = rewardAmount;
        landData[landId].enemyIdList = enemyList;
        landData[landId].rewardIdList = rewardList;
        landData[landId].maxRewardAmount = maxRewardAmount;
        landData[landId].dungeonId = dungeonId;
        landData[landId].landPrice = landPrice;
        landData[landId].playable = playable;

        if (enemyList.length > 0 && rewardList.length > 0 && rewardAmount.length > 0) landData[landId].playable = true;
        else landData[landId].playable = false;
    }

    function clearLand(uint256 landId) public onlyRole('TRUSTED_FORWARDER', msg.sender) {
        require(lostLands[landId].exists, 'LWNFT: Land does not exists');
        require(_msgSender() == ownerOf(landId), 'LWNFT: Not the land owner');
        landData[landId].rewardAmount = new uint8[](0);
        landData[landId].maxRewardAmount = new uint8[](0);
        landData[landId].enemyIdList = new uint256[](0);
        landData[landId].rewardIdList = new uint256[](0);
        landData[landId].playable = false;
        landData[landId].dungeonId = 0;
        landData[landId].landPrice = 0;
    }

    function viewLand(uint256 landId) public view returns (LandData memory) {
        LandData memory land = landData[landId];
        land.owner = ownerOf(landId);
        land.x = lostLands[landId].x;
        land.y = lostLands[landId].y;
        land.landId = landId;
        if (land.landPrice <= 0) land.playable = false;
    ...
    {
        LandData memory land = landData[landId];
        require(land.playable, 'LWNFT: Land is not playable');
        require(land.landPrice > 0, 'LWNFT: Land price is zero');
        return (
            land.rewardAmount,
            land.maxRewardAmount,
            land.enemyIdList,
            land.rewardIdList,
            land.landPrice,
            land.owner
    ...
    function getLandHash(uint256 landId) external view returns (bytes32) {
        return generateHash(landId);
    }

    function getHash(uint256 landId) public view returns (bytes32) {
        return generateHash(landId);
    }

    function generateHash(uint256 landId) internal view returns (bytes32) {
        LandData memory land = landData[landId];
        return
            keccak256(
                abi.encodePacked(
    ...
    function viewAllLandInWorld(uint256 worldId) public view returns (LandData[] memory) {
        require(lostWorlds[worldId].exists, 'LWNFT: World does not exists');
        uint256[] memory landIds = lostWorlds[worldId].landIds;
        require(landIds.length > 0, 'LWNFT: World does not contain and land');

        LandData[] memory ret = new LandData[](landIds.length);
        for (uint256 i = 0; i < landIds.length; i++) {
            LandData memory land = landData[landIds[i]];
            land.owner = ownerOf(landIds[i]);
            land.landId = landIds[i];
            land.x = lostLands[i].x;
            land.y = lostLands[i].y;
            ret[i] = land;
        }
