...
contract Vesting is Ownable {
    ...
    IERC20 GeniusToken;
    //@param: totalClaimPeriod: Total vesting time of 5 years is divided into 5 claim periods of 1 year each
    uint256 public totalClaimPeriod = 5;

    //@param percentReleasePerPeriod 20% of the tokens vested for each user will be released after every period
    uint256 public percentReleasePerPeriod = 20; // 20 percent
    uint256 startTime;

    //@param yearCalc: Division Factor to convert the time into years
    // uint256 yearCalc = 31556952; //Year in seconds
    uint256 yearCalc = 60; //Year in seconds
    ...
    //@notice Checks the claimable amount the user is entitled for the period elapsed
        ...
        returns (uint256)
    {
        ...
        uint256 claimAmount;
        uint256 claimPeriod = (
            (block.timestamp.sub(startTime)).div(yearCalc)
        );
        if (claimPeriod < 1) return 0;
        if (claimPeriod <= totalClaimPeriod) {
            claimAmount = (user.total).mul(claimPeriod).div(
                totalClaimPeriod
            );
    ...
    //@notice: Checks the Genius token balance in the vesting contract
