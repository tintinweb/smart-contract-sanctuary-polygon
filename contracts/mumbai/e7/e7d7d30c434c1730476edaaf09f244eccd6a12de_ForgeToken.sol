...
// File: ForgeTokenOwnerMintPolygon.sol
...
//-----------------------------------------------------------------------------
contract ForgeToken is ERC721Enumerable, Ownable {
    ...
    uint private constant NUM_XP_VALUES = 10;
    // [0064 = 100, 01F4 = 500, 03E8 = 1000, 1338 = 5000, 1D4C = 7500, 2710 = 10000, 3A98 = 15000, 4E20 = 20000, 61A8 = 25000, 7FF8 = 32760]
    bytes32 private constant XP_VALUES = 0x006401F403E813381D4C27103A984E2061A87FF8000000000000000000000000;
    // 05 = ~2%, 08 = ~3%, 0D = ~5%, 26 = ~15%, 40 = ~25%
    bytes32 private constant XP_OCCURRENCE_RATES = 0x05080D264040260D080500000000000000000000000000000000000000000000;
    // 5/256 = ~1.95%
    uint8 constant SPECIAL_VARIANT_RATE = 5;

    address public minterAddress = 0xED605eA81904A0D7537767878ef94bFB343Fd8D5;
    string public baseURI = "https://forge.gg/";
    mapping (uint=>TokenAttributes) public tokenAttributes;
    ...
    constructor() ERC721("ForgeToken", "FORGE") {}
    ...
    //-------------------------------------------------------------------------
    /// @notice Mints a new Forge Badge and sends it to the specified address.
    ///  The token ID of the new badge is determined by the totalSupply when
    ///  minted. Randomly rolls XP and specialVariant value.
    /// @dev Throws if sender is not the Minter address
    /// @param _to The recipient of the minted NFT
    //-------------------------------------------------------------------------
    function safeMint(address _to, uint256 nonce) external {
        require (
        ...
        bool isSpecialVariant = uint8(randomSeed[1]) < SPECIAL_VARIANT_RATE;
        tokenAttributes[tokenId] = TokenAttributes(xpValue, isSpecialVariant);
    ...
    function burn(uint _tokenId) external {
        ...
        uint16 amountBurned = tokenAttributes[_tokenId].xp;
        ...
        returns (uint16)
    {
        ...
        uint index;
        for (uint i = 0; i < NUM_XP_VALUES; ++i) {
            if (_rand < uint8(XP_OCCURRENCE_RATES[i])) {
                index = i;
                break;
            }
            _rand -= uint8(XP_OCCURRENCE_RATES[i]);
        }

        // convert index into a bytes16 value
        uint16 xpValue = uint16(uint8(XP_VALUES[index * 2])) << 8;
        xpValue += uint8(XP_VALUES[index * 2 + 1]);
