    ...
    VRFConsumerBaseV2
{
    ...
    mapping(uint256 => bytes32) private _vrfRequests;
    ...
    function mint(
        ...
        if (!hasRole(CLAIMER_ROLE, _msgSender())) {
            ...
            require(
                balanceOf(to) <= subcollection.params.maxMintPerAddress,
    ...
    /**
     * @dev This function initializes the process to reveal the random words generated
     * by VRF Coordinator with a callback. It requires a trusted signer role and it should
     * not be called after revealing has already been done.
     * @param subcollectionName subcollection name.
     */
    function initReveal(
        string memory subcollectionName
    ) external onlyRole(TRUSTED_SIGNER_ROLE) {
        require(!isSubcollectionRevealed(subcollectionName), "Already revealed");
        ...
        _vrfRequests[vrfRequestId] = keccak256(
            abi.encodePacked(subcollectionName)
        );
    }
    ...
    function fulfillRandomWords(
        ...
        bytes32 subcollectionNameHash = _vrfRequests[requestId];
        require(subcollectionNameHash.length != 0, "Invalid request ID");
        if (!subcollections[_subIdByNameHash[subcollectionNameHash]].isRevealed) {
            _reveal(subcollectionNameHash, randomWords[0]);
        }
    }
...
interface ICollectionToken  {
    ...
    function mint(string memory subcollectionName, address to, bytes32[] memory proof) external payable returns (uint256);
