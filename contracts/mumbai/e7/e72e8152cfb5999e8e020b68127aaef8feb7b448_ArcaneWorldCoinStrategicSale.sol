...
contract ArcaneWorldCoinStrategicSale is Ownable {
    ...
    uint constant LOCKUP_FREEZE = 60 days;
    ...
    uint public lockupTokens = 0;
    //number of tokens withdrawn    
    uint public withdrawTokens = 0;

    event NewHolder(address indexed _to, uint amount);
    event UnlockTokens(address indexed _to, uint _amount);
    ...
    function lockUp(address _target, uint256 _amount) public onlyOwner {
        ...
        require(_amount > 0);
        require(lockup_record[_target].amount == 0 &&  lockup_record[_target].complete == false, 'The recipient already exists');

        uint256 balance = token.balanceOf(address(this));
        
        if (lockupTokens != 0) {
            balance = balance - lockupTokens;
        }
        ...
        lockupTokens += _amount;
        countHolders++;
        emit NewHolder(_target, _amount);
    }
    ...
    function unlock() public isNotFreeze {
        ...
        if (lock.withdrawSumm <= lock.amount) {
            ...
            emit UnlockTokens(lock.owner, withdrawalBalance);
        }

        lockupTokens -= withdrawalBalance;
        withdrawTokens += withdrawalBalance;
    ...
    function massLockup(address[] memory _targets, uint256[] memory _amounts) public onlyOwner {
        ...
        for (uint256 i = 0; i < count; i++){
            ...
            lockupTokens += _amounts[i];
            countHolders++;
            emit NewHolder(_targets[i], _amounts[i]);
        }
    }    
}