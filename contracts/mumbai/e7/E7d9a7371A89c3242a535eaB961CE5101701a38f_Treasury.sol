...
contract Treasury is Context, AccessControlled {
    ...
    function deposit(
        ...
        require (approvedTokens[_token], invalidToken);
     ...
     */
    function withdraw(address _token, address _user, uint256 _amount, uint256 _fee) external returns (uint256) {
        require(permissions[STATUS.RESERVESPENDER][msg.sender], notApproved); // check if it's Songchain's contract calling
      
        uint256 payment = _amount - getFee(_amount, _fee);
        collectFee(_token, _amount, _fee);
        IERC20(_token).transfer(_user, payment);
        
        emit WithdrawERC20(_token, _user, payment);
    ...
    function collectFee(address _token, uint256 _amount, uint256 _fee) internal {             
        erc20Treasury[_token] += getFee(_amount, _fee);
    }

    function getFee(uint256 _amount, uint256 _fee) public view returns (uint256) {
        return (_amount * _fee) / feeDivisor;
    }
    ...
    function setFeeDivisor(uint256 _feeDivisor) public onlyGovernor {
        feeDivisor = _feeDivisor;
    }

    function setApprovedToken(address _token, bool _approved) public onlyGovernor {
