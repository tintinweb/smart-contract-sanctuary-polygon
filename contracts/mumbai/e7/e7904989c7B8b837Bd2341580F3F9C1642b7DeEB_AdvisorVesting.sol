...
contract NWTVestingHub is Ownable, ReentrancyGuard, Pausable {
    ...
    IERC20 public token = IERC20(0xD88121fF2F241a183b33AFf4D031873CEfCaB3f6);
    ...
    mapping(address => Buyer) public investors;
    ...
    function claim(
        ...
        require(
            block.timestamp > start.add(vestingCliff),
            "NWTVestingHub: Cliff Period is not over yet"
        );
        ...
        if (elapsedTime >= duration == false) {
            require(
                block.timestamp >=
                    investors[benificery].lastClaimTime.add(claimCooldown),
        ...
        investors[_msgSender()].claimedAmount = investors[_msgSender()]
            .claimedAmount
            .add(claimableTokens);
        investors[_msgSender()].lastClaimTime = block.timestamp;
