...
contract HarfangExchange is ReentrancyGuard, Ownable {
        ...
        nonReentrant
    {
        ...
        _transferNonFungibleToken(makerAsk.signer, msg.sender, takerBid.id, makerAsk.sid, makerAsk.strategy);
...
contract Harfang is Ownable {
    ...
    function setBatchWhitelisted(bool addWhitelist, address[] calldata addresses) onlyOwner external {
        for(uint256 i = 0;i<addresses.length;i++){
            isWhitelisted[addresses[i]] = addWhitelist;
        }
    }

    function createElement(bytes calldata cid, uint256 _copies, Utils.ElementType t, address currency) onlyWhitelisted external {
        require(cid[0] == 0x12 && cid[1] == 0x20 && cid.length == 34, "Incorrect cid (v0 only)");
    ...
    function createSpecificElement(uint256 id, address to) onlyWhitelisted public {
        ...
        emit SpecificElementCreated(global.lastId-1, id, to);
    }

    function send(uint256 id, uint256 sid, address to, bytes calldata messageURI) onlyWhitelisted external {
        ...
        if (global.t == Utils.ElementType.stamp){
            ...
            _tokenApprovals[Utils.encode(id, sid)] = address(0);
            emit Transfer(msg.sender, to, id, sid);
            emit Transfer(msg.sender, to, element.twin, element.twinSid);
        }else{
            revert("Element does not have a type");
        }
    }

    function transfer(uint256 id, uint256 sid, address from, address to) onlyWhitelisted external {
    ...
    function stamp(uint256 cardID, uint256 cardSID, uint256 stampID, uint256 stampSID, address currency) onlyWhitelisted external {
    ...
    function unstamp(uint256 id, uint256 sid) onlyWhitelisted public {
    ...
    function burn(uint256[2][] calldata elementsToBurn) onlyWhitelisted external {
    ...
    function approve(address to, uint256 id, uint256 sid) onlyWhitelisted external {
    ...
    function setApprovalForAll(address operator, bool approved) onlyWhitelisted external {
