...
contract ENFVesting is AccessControl, ReentrancyGuard {
    ...
    uint128 public immutable vestingContractStartingDate;
    ...
    constructor(IERC20 tokenAddress, uint128 vestingContractStartingDateParam) {
        _token = tokenAddress;
        vestingContractStartingDate = vestingContractStartingDateParam;
    ...
    function getReleaseAmount(bytes32 vestingScheduleId) public view returns (uint256) {
        ...
        uint256 vestingStartDate = vestingContractStartingDate + vestingSchedule.vestingDetails.cliff;
        uint256 tgeAmount;

        if (currentTime < vestingContractStartingDate) {
            return 0;
        }
    ...
    function enfOfCliff(bytes32 vestingScheduleId) external view returns (uint256) {
        VestingSchedule memory vest = _vestings[vestingScheduleId];
        return vestingContractStartingDate + vest.vestingDetails.cliff;
    }

    function endOfVesting(bytes32 vestingScheduleId) external view returns (uint256) {
        VestingSchedule memory vest = _vestings[vestingScheduleId];
        return vestingContractStartingDate + vest.vestingDetails.cliff + vest.vestingDetails.duration;
    }
