...
contract CommentPosts {
    ...
    // Map the user address to a list of message IDs and the address bucket of the poster
    // Address is a string for address "buckets" 0x123, 0x123-1, 0x123-2 ...
    mapping (string => uint256[2][]) public commentPostsMap;
    mapping (string => mapping (uint256 => bool)) public commentPostsMapMap;
    ...
    */
    ...
    /**
    * Returns a list of messages that are posted by a given user
    * @param usrAddress : the address to retrieve the details for
    * @param startFrom : the number to start getting records from
    */
    function getCommentIDs(address usrAddress, uint256 startFrom) public view returns(uint256[2][] memory) {
        ...
        if (startFrom == 0){
            startFrom = ((commentPostsMap[bucketKey].length) + (bucketKeyID * maxItemsPerBucket));
            if (startFrom != 0){
                startFrom -= 1;
            } else {
                // It's empty, so end
                uint256[2][] memory empty = new uint256[2][](0);
                return empty;
            }
        }
        ...
        for (uint i=0; i <= bucketKeyID; i++) {
            ...
            if (startFrom >= (i * maxItemsPerBucket) && startFrom <= ((commentPostsMap[bucketKey].length) + (i * maxItemsPerBucket))) {
                bucketKeyID = i;
        ...
        for (uint i=(startFrom + 1); i > (startFrom + 1 - _maxRecords); i--) {
            ...
            if (commentPostsMapMap[bucketKey][commentPostsMap[bucketKey][i - 1][0]]){
                postCount += 1;
            }
        ...
        if (bucketKeyID != 0 && amountLeft > 0){
            ...
            for (uint i=commentPostsMap[remainderBucketKey].length; i > commentPostsMap[remainderBucketKey].length - amountLeft; i--) {
                // Add it if the item is still enabled
                if (commentPostsMapMap[remainderBucketKey][commentPostsMap[remainderBucketKey][i - 1][0]]){
                    postCount += 1;
                }
            }
        }
        ...
        // Start the array
        uint256[2][] memory msgIDs = new uint256[2][](postCount);
        ...
        for (uint i=(startFrom + 1); i > (startFrom + 1 - _maxRecords); i--) {
            if(commentPostsMapMap[bucketKey][commentPostsMap[bucketKey][i - 1][0]]){
                msgIDs[counter] = [commentPostsMap[bucketKey][i - 1][0], commentPostsMap[bucketKey][i - 1][1]];
                counter += 1;
            }
        }
        ...
        if (bucketKeyID != 0 && amountLeft > 0){
            ...
            for (uint i=commentPostsMap[remainderBucketKey].length; i > commentPostsMap[remainderBucketKey].length - amountLeft; i--) {
                // Add it if the item is still enabled
                if (commentPostsMapMap[remainderBucketKey][commentPostsMap[remainderBucketKey][i - 1][0]]){
                    msgIDs[counter] = [commentPostsMap[remainderBucketKey][i - 1][0], commentPostsMap[remainderBucketKey][i - 1][1]];
                    counter += 1;
                }
            }
        }

        delete usrAddressStr;
        delete postCount;
        delete _maxRecords;
        delete bucketKeyID;
        delete bucketKey;
    ...
    function addCommentPost(uint256 msgID, address addressPoster, uint256 isCommentOf) public onlyAdmins {
        // Stringify the address
        string memory addressPosterStr = KUtils.addressToString(addressPoster);

        // Add the post to the bucket for the poster
        uint256 bucketKeyID = getBucketKey(addressPosterStr, true);
        string memory postBucketKey = KUtils.append(addressPosterStr,'-',KUtils.toString(bucketKeyID),'','');

        // Update the Posts Mapping with this user address
        commentPostsMap[postBucketKey].push([msgID,isCommentOf]);

        // Update the Posts Map Map for quick removal
        commentPostsMapMap[postBucketKey][msgID] = true;

        delete addressPosterStr;
        delete postBucketKey;
        delete bucketKeyID;
    }


    function removeCommentPost(uint256 msgID, address addressPoster) public onlyAdmins {
        // Stringify the address
        string memory addressPosterStr = KUtils.addressToString(addressPoster);

        // Get the latest bucket on that user
        uint256 bucketKeyID = getBucketKey(addressPosterStr, false);

        // Loop through each bucket and set the address to false
        for (uint i=0; i <= bucketKeyID; i++) {
            string memory postBucketKey = KUtils.append(addressPosterStr,'-',KUtils.toString(i),'','');

            // Update the map map for quick removal
            commentPostsMapMap[postBucketKey][msgID] = false;
        }

        // Update their total post count
        userProfiles.updatePostCount(addressPoster, true);
    ...
    function getBucketKey(string memory mapKey, bool toInsert) private view returns (uint256){
        ...
        while (mapID == 99999999999999999999){
            ...
            if (commentPostsMap[bucketToCheck].length > 0){
                // exists
                b++;
                prevBucketLen = commentPostsMap[bucketToCheck].length;
            } else if (b == 0) {
...
interface IUserProfiles {
    ...
    function logPost(address posterAddress, uint256 tipPerTag, address[] calldata tipReceivers, bool isComment) external;
    ...
    function getUserContracts(address usrAddress) external view returns(address[] memory);
