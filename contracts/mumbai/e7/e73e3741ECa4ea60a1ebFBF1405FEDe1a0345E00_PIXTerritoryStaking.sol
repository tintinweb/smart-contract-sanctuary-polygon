...
contract PIXTerritoryStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, IPIXTerritoryStaking {
    ...
    function earnedByAccountUntilTimestamp(address account, uint256 untilTimestamp) public override view returns (uint256) {
        uint256[] memory tokens = stakedNFTs[account];
        _quickSort(tokens, 0, tokens.length - 1);
        
        uint256 reward = 0;
        for (uint256 i = 0; i < tokens.length; i += 1) {
            if (i == 0 || tokens[i] != tokens[i-1]) {
                if (stakers[tokens[i]] != account) continue;
                reward += earnedRewardUntilTimestamp(tokens[i], untilTimestamp);
            }
        }
    ...
    function getStakedNFTsLength(address user) external override view returns(uint256) {
        uint256[] memory tokens = stakedNFTs[user];
        _quickSort(tokens, 0, tokens.length - 1);
    ...
    function _quickSort(uint256[] memory arr, uint256 left, uint256 right) internal view {
        uint256 i = left;
        uint256 j = right;
        if(i==j) return;
