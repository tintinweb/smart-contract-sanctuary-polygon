...
contract CrowdFunding {
    ...
    struct Campaign {
        ...
        string image;
        EnumerableSet.AddressSet donators;
    ...
    function createCampaign(
        ...
        campaignIds.push(numberOfCampaigns);
    ...
    function donateToCampaign(uint256 _id) public payable {
        ...
        require(block.timestamp <= campaign.deadline, "The campaign has expired.");

        uint256 amount = msg.value;

        campaign.donators.add(msg.sender);
        campaign.donations[msg.sender] = campaign.donations[msg.sender].add(amount);

        (bool sent, ) = payable(campaign.owner).call{value: amount}("");

        require(sent, "Failed to send the donation to the campaign owner.");
    ...
    function getDonators(uint256 _id) public view returns (address[] memory, uint256[] memory) {
        ...
        for (uint256 i = 0; i < campaign.donators.length(); i++) {
            address donator = campaign.donators.at(i);
            donators[i] = donator;
            donations[i] = campaign.donations[donator];
        }
