...
contract NCSubscription is Ownable {
    ...
    function subscribe() external {
        uint256 subInEther = poolSize / 1 ether;
        require(token.allowance(msg.sender, address(this)) >= subInEther, 'Not enough tokens');
        token.transferFrom(msg.sender, address(this), subInEther);
        nft.mint(msg.sender);
    ...
    function unsubscribe() external {
        uint256 subInEther = poolSize / 1 ether;
        // Check if they have a nft
        require(nft.balanceOf(msg.sender) >= 0, 'No Subscription');
        // token.approve(msg.sender, subInEther);
        token.transferFrom(address(this), msg.sender, subInEther);
        // BURN NFT
