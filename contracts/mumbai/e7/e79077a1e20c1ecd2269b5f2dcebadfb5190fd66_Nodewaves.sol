...
contract Nodewaves is ERC20, ERC20Burnable, Ownable, ReEntrancyGuard {
    ...
    uint256 public deposit_one_percentage = 10;
    ...
    function buyNode(address[10] memory _friend, uint256 _paymentType)
        public
        payable
    {
        ...
        for (uint256 i = 0; i < 10; i++) {
            if (_friend[i] != address(0)) {
                referral_balances[_friend[i]] +=
                    (msg.value * deposit_one_percentage) /
                    100;
            }
        }
        ...
        totalUser += 1;
        company.transfer((msg.value * (100 - deposit_one_percentage)) / 100);
    ...
    function buyMembership(address[10] memory _friends, uint256 _paymentType)
        public
        payable
    {
        require(_paymentType == 1 || _paymentType == 2, "Invalid payment type");
        ...
        if (_paymentType == 1) {
            ...
            nws.transferFrom(msg.sender, address(this), membershipPriceWithNws);
            nws.transfer(
                company,
                (membershipPriceWithNws * paymentSplit) / 10000
            );
        }

        referral_balances[_friends[0]] += (msg.value * f_percentages[0]) / 100;
        referral_balances[_friends[1]] += (msg.value * f_percentages[1]) / 100;
        referral_balances[_friends[2]] += (msg.value * f_percentages[2]) / 100;
        referral_balances[_friends[3]] += (msg.value * f_percentages[3]) / 100;
        referral_balances[_friends[4]] += (msg.value * f_percentages[4]) / 100;
        referral_balances[_friends[5]] += (msg.value * f_percentages[5]) / 100;
        referral_balances[_friends[6]] += (msg.value * f_percentages[6]) / 100;
        referral_balances[_friends[7]] += (msg.value * f_percentages[7]) / 100;
        referral_balances[_friends[8]] += (msg.value * f_percentages[8]) / 100;
        referral_balances[_friends[9]] += (msg.value * f_percentages[9]) / 100;

        isUser[msg.sender] = true;
        timestamp[msg.sender] = block.timestamp;
        totalUser += 1;
        uint256 total_percentages = f_percentages[0] +
            f_percentages[1] +
            f_percentages[2] +
            f_percentages[3] +
            f_percentages[4] +
            ...
            f_percentages[8] +
            f_percentages[9];
        company.transfer((msg.value * (100 - total_percentages)) / 100);
    }
    ...
    function claimWithdraw(uint256 _amount) public noReentrant {
    ...
    function addClaimReward(address _address, uint256 _amount) public {
        require(msg.sender == company, "You are not othorised");
        referral_balances[_address] = _amount;
    }

    function buyNodePercentage(uint256 _percentage) public {
        require(msg.sender == company, "You are not othorised");
