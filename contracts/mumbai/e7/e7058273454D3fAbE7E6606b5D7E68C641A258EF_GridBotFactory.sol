...
contract GridBotFactory  {
  ...
  function factoryNewGrid(
    ...
    swap.swapExactMaticInputSingle{ value: msg.value }(wmatic.balanceOf(address(this)),address(registerKeeps),address(wmatic), linkToken);
  ...
  function wDeposit() public payable {
    wmatic.deposit{ value: msg.value }();
  }
  function wBalance() public view returns(uint){
...
contract Swap {
    ...
    function swapExactMaticInputSingle(uint256 amountIn, address to, address tokenIn,address tokenOut) public payable returns (uint256 amountOut) {
        // msg.sender must approve this contract
