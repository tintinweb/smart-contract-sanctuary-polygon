...
contract BrunchMint is ERC721, ERC721URIStorage, Ownable {
    ...
    constructor(address mintPassAddress) ERC721("BrunchMint", "BBM") {
        mintPass = MintPass(mintPassAddress);
    }

    function mintpassOwner(uint256 tokenId) public view returns (address) {
        return mintPass.ownerOf(tokenId);
    }

    function claim(uint256 mintPassTokenId) public {
        _checkSubscription(mintPassTokenId);
    ...
    function _checkSubscription(uint256 mintPassTokenId) private view {
        require(
            msg.sender == mintPass.ownerOf(mintPassTokenId),
            "Not the subscriber"
        );
        require(
            mintPass.expiresAt(mintPassTokenId) > block.timestamp,
...
import "@openzeppelin/contracts/utils/Counters.sol";
import "@openzeppelin/contracts/access/Ownable.sol";
import "lib/ERC5643/ERC5643.sol";

contract MintPass is ERC5643, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;

    constructor() ERC5643("MintPass", "MTP") {
        _setMinimumRenewalDuration(900); // 15 mins
        _setMaximumRenewalDuration(3600); // 1 hr
    }

    uint256 public pricePerSecond = 10000000000000; // 0.00001 ETH

    function subscribe(uint64 duration) public payable {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();

        _safeMint(msg.sender, tokenId);
        _createSubscription(tokenId, duration);
    }

    function _createSubscription(uint256 tokenId, uint64 duration) private {
        ...
        if (msg.value < subscriptionPrice(tokenId, duration)) {
    ...
    function subscriptionPrice(uint256 tokenId, uint64 duration)
        public
        view
        returns (uint256)
    {
        return _getRenewalPrice(tokenId, duration);
    }
    ...
    function setMinimumRenewalDuration(uint64 duration) public {
        _setMinimumRenewalDuration(duration);
    }

    function setMaximumRenewalDuration(uint64 duration) public {
