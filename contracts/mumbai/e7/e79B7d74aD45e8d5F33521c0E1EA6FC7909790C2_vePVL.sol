...
interface IValveNFT is IERC721Enumerable, IERC2981Royalties {
    struct Valve {
        uint32 power;
        uint16 level;
        uint16 score;
    ...
    function getValveDetails(uint256 tokenId)
        external
        view
        returns (
            uint16 level,
    ...
    function valvesLength() external view returns (uint256);
    ...
    event ValveCreation(uint256 from, uint256 length);
...
import './interfaces/IValveNFT.sol';

/// @title VePvl
/// @notice Valve Venom: the staking contract for PVL, as well as the token used for governance.
/// Note Venom does not seem to hurt the Valve, it only makes it stronger.
/// Allows depositing/withdraw of pvl and staking/unstaking ERC721.
/// Here are the rules of the game:
/// If you stake pvl, you generate vePvl at the current `generationRate` until you reach `maxCap`
/// If you unstake any amount of pvl, you loose all of your vePvl.
    ...
    IVePvl
{
    ...
    struct UserInfo {
        uint256 amount; // pvl staked by user
        uint256 lastRelease; // time of last vePvl claim or first deposit if user has not claimed yet
        // the id of the currently staked nft
        // important: the id is offset by +1 to handle tokenID = 0
        uint256 stakedNftId;
    }

    /// @notice the pvl token
    IERC20 public pvl;

    /// @notice the masterValve contract
    IMasterValve public masterValve;

    /// @notice the NFT contract
    IValveNFT public nft;

    /// @dev Magic value for onERC721Received
    /// Equals to bytes4(keccak256("onERC721Received(address,address,uint256,bytes)"))
    bytes4 private constant ERC721_RECEIVED = 0x150b7a02;

    /// @notice max vePvl to staked pvl ratio
    /// Note if user has 10 pvl staked, they can only have a max of 10 * maxCap vePvl in balance
    uint256 public maxCap;

    /// @notice the rate of vePvl generated per second, per pvl staked
    ...
    /// @dev contract addresses are by default unable to stake pvl, they must be previously whitelisted to stake pvl
    ...
    function initialize(
        IERC20 _pvl,
        // IMasterValve _masterValve,
        IValveNFT _nft
    ) public initializer {
        // require(address(_masterValve) != address(0), 'zero address');
        require(address(_pvl) != address(0), 'zero address');

        // Initialize vePVL
        __ERC20_init('Valve Venom', 'vePVL');
        __Ownable_init();
        __ReentrancyGuard_init_unchained();
        __Pausable_init_unchained();

        // set generationRate (vePvl per sec per pvl staked)
    ...
    /// @notice sets masterValve address
    /// @param _masterValve the new masterValve address
    function setMasterValve(IMasterValve _masterValve) external onlyOwner {
        require(address(_masterValve) != address(0), 'zero address');
        masterValve = _masterValve;
    }
    ...
    function setNftAddress(IValveNFT _nft) external onlyOwner {
    ...
    /// @notice checks wether user _addr has pvl staked
    /// @param _addr the user address to check
    /// @return true if the user has pvl in stake, false otherwise
    function isUser(address _addr) public view override returns (bool) {
        return users[_addr].amount > 0;
    }

    /// @notice returns staked amount of pvl for user
    /// @param _addr the user address to check
    /// @return staked amount of pvl
    function getStakedPvl(address _addr) external view override returns (uint256) {
    ...
    /// @notice deposits PVL into contract
    /// @param _amount the amount of pvl to deposit
    function deposit(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        if (isUser(msg.sender)) {
            // if user exists, first, claim his vePVL
            _claim(msg.sender);
        ...
        // Request Pvl from user
        pvl.safeTransferFrom(msg.sender, address(this), _amount);
    }
    ...
    /// @notice claims accumulated vePVL
    ...
    /// @notice Calculate the amount of vePVL that can be claimed by user
    /// @param _addr the address to check
    /// @return amount of vePVL that can be claimed by user
    function claimable(address _addr) external view returns (uint256) {
    ...
    function _claimable(address _addr) private view returns (uint256) {
        ...
        // get user's vePVL balance
        uint256 userVePvlBalance = balanceOf(_addr);

        // user vePVL balance cannot go above user.amount * maxCap
        uint256 maxVePvlCap = user.amount * maxCap;

        // first, check that user hasn't reached the max limit yet
        if (userVePvlBalance < maxVePvlCap) {
            // then, check if pending amount will make user balance overpass maximum amount
            if ((userVePvlBalance + pending) > maxVePvlCap) {
                return maxVePvlCap - userVePvlBalance;
            } else {
                return pending;
            }
        }
    ...
    /// @notice withdraws staked pvl
    /// @param _amount the amount of pvl to unstake
    /// Note Beware! you will loose all of your vePVL if you unstake any amount of pvl!
    function withdraw(uint256 _amount) external override nonReentrant whenNotPaused {
        ...
        // update his balance before burning or sending back pvl
        users[msg.sender].amount -= _amount;

        // get user vePVL balance that must be burned
        uint256 userVePvlBalance = balanceOf(msg.sender);

        _burn(msg.sender, userVePvlBalance);

        // send back the staked pvl
        pvl.safeTransfer(msg.sender, _amount);
    }

    /// @notice hook called after token operation mint/burn
    /// @dev updates masterValve
    /// @param _account the account being affected
    /// @param _newBalance the newVePvlBalance of the user
    function _afterTokenOperation(address _account, uint256 _newBalance) internal override {
        masterValve.updateFactor(_account, _newBalance);
    }

    /// @notice This function is called when users stake NFTs
    /// When Valve NFT sent via safeTransferFrom(), we regard this action as staking the NFT
    ...
    function onERC721Received(
        ...
        require(msg.sender == address(nft), 'only valve NFT can be received');
    ...
    function getVotes(address _account) external view virtual override returns (uint256) {
        uint256 vePvlBalance = balanceOf(_account);

        // check that user has more than voting treshold of maxCap and has pvl in stake
        if (vePvlBalance * invVoteThreshold > users[_account].amount * maxCap && isUser(_account)) {
            return vePvlBalance;
        } else {
            return 0;
        }
