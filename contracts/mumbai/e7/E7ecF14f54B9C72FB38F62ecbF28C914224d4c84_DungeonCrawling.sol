...
contract DungeonCrawling is Initializable, DungeonCrawlingSettings {
    ...
    contractsAreSet
    {
        ...
        DungeonSuppliedInput[][] memory _entityInputs = new DungeonSuppliedInput[][](_entities.length);

        for(uint256 i = 0; i < _entities.length; i++) {
            uint256 _tokenId = _entities[i].tokenId;
            DungeonSuppliedInput[] storage _entityInputsStorage = dungeonCrawlingInputs[userToInfo[msg.sender].activeInputIndex].tokenIdToEntityInputs[_tokenId];
    ...
    private
    {
        ...
        for(uint256 i = 0; i < _entities.length; i++) {
            ...
            dungeonCrawlingInputs[_userInfo.activeInputIndex].entities.push(_entities[i]);

            _stakeInputs(
                dungeonNameToInfo[_dungeonName].entityInputs,
                dungeonCrawlingInputs[_userInfo.activeInputIndex].tokenIdToEntityInputs[_entities[i].tokenId],
    ...
    contractsAreSet
    {
        require(isUserDungeonCrawling(msg.sender), "Not dungeon crawling");
    ...
    function ownerOfEntity(uint256 _tokenId) private view returns(address) {
...
abstract contract DungeonCrawlingSettings is Initializable, DungeonCrawlingContracts {
    ...
    requiresEitherRole(ADMIN_ROLE, OWNER_ROLE)
    {
        ...
        require(_info.startTime > 0, "Bad start time");
        ...
        for(uint256 i = 0; i < _info.inputs.length; i++) {
            ...
            require(_input.maxQuantity == 0 || _input.maxQuantity >= _input.minQuantity, "Max quantity smaller than min");
