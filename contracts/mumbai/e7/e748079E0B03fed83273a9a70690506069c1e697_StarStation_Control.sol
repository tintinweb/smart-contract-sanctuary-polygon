...
// File: Star Stations/StarStationControl.sol
...
contract StarStation_Control is Context, Ownable{
    ...
    struct StationClass{
        ...
        uint256 maxUpgrade;         // Max upgrade for the class
    }
    ...
    address constant star = 0x7Ba798157147e37Dc4c54bDFa2aF013BAf3C02c2; //0x8440178087C4fd348D43d0205F4574e0348a06F0;
    ...
    constructor(uint256 _emissionRate, address _stationAddress) {
    ...
    function updateWeight(uint256 _startId, uint256 _endId) external {
        for (uint256 pid = _startId; pid <= _endId; pid++) {                                            // Cycle through all Spacestations
            ...
            if(SpaceStation.lastUpdate < totalMinted){                                                  // Check that new Nfts have been minted since stations last update
                if(SpaceStation.weight < stationClass.maxUpgrade){                                      // Check that station weight is less then max weight
                    uint256 newStations = totalMinted.sub(SpaceStation.lastUpdate);                     // Get the number of new nfts minted since last update
                    uint256 currentWeight = SpaceStation.weight;                                        // Get the current weight of the Station
                    ...
                    SpaceStation.weight = currentWeight.add(newWeight);                                 // Update the ships weight.
    ...
    // Mint requested NFT
    function mintNFT(uint256 _classPID, string memory _uri) external returns(uint256){
        ...
        uint256 tokenId = spaceStationContract.safeMint(_msgSender(),_uri,station.startingWeight);  // Pay for the NFT
    ...
    function addClass(StationClass calldata _station )external onlyOwner{
        stationClasses.push(StationClass({                      // Add new station to the SpaceStations array
            ...
            maxUpgrade:_station.maxUpgrade                      // Set Max weight for class
        }));
    }
    ...
    // Allow for owner of stations contract change incase of new controller
    function updateNftWeight(uint256 tokenId,uint256 increase)external onlyOwner {
        spaceStationContract.updateWeight(tokenId,increase);
    }

    function setMaxIncrease(uint256 _increase) external onlyOwner{
        for(uint256 pid; pid < stationClasses.length; pid++){
            StationClass storage class = stationClasses[pid];
            class.maxUpgrade = class.startingWeight.mul(_increase);
        }
    }
}