...
contract ZAMNFT is ERC721Enumerable, Ownable {
    ...
    function _presale(
        ...
        if (!secondLevel[account] && !thirdLevel[account]) {
            require(level1 <= 1400, "NFT LEVEL 1 SOLD OUT");
            require(
                _mintAmount + level1 <= 1400,
                "NFT is left or mint amount is too big"
            );
            for (uint256 i = 0; i < _mintAmount; i++) {
                _safeMint(account, level1 + i);
            }
            level1 += _mintAmount;
        }
        if (secondLevel[account]) {
            require(level2 <= 500, "NFT LEVEL 2 SOLD OUT");
            require(
                _mintAmount + level2 <= 500,
                "NFT is left or mint amount is too big"
            );

            for (uint256 i = 0; i < _mintAmount; i++) {
                _safeMint(account, level2 + i);
            }
            level2 += _mintAmount;
        }
