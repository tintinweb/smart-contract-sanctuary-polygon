...
contract FlashMinter is IFlashLender, Pausable, Ownable {
    ...
    event FlashloanTaken(uint256 amount);
    event FlashloanPaidBack(uint256 amount, uint256 fee);
    ...
    function flashLoan(
        ...
        emit FlashloanPaidBack(amount, _fee);
        token.poolBurn(address(receiver), amount + _fee);
        return true;
    }
