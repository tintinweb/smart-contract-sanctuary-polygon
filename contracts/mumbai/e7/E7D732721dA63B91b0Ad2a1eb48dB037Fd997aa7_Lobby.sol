...
interface GamesInterface {
    function startGame(uint[] memory _playerIndexes, uint _numPlayers, uint _debugRoomId) external returns (uint);
}
...
contract Lobby is Ownable {
    ...
    event StartedGame(uint lobbyNumber, uint[] playerIds);
    ...
    // TODO: Consider DRYing with regular
    // NOTE:  ONLY LAST PLAYER TO ENLIST WILL SELECT DEBUG ROOM
    function debugEnlist(uint _charId, address charOwner, uint _debugRoomId) public {
        require(msg.sender == address(chars), "Not character contract");
        ...
        uint playerIndex = players.addPlayer(address(chars), _charId, charOwner);
        ...
        if (newLobby.numberOfPlayers == MAX_PLAYERS) {
            uint gameStarted = games.startGame(newLobby.playerIndexes, MAX_PLAYERS, _debugRoomId);
            emit StartedGame(gameStarted, newLobby.playerIndexes);
        }
    ...
    function enlistForMission(uint _charId, address charOwner) public {
        ...
        if (newLobby.numberOfPlayers == MAX_PLAYERS) {
            uint gameStarted = games.startGame(newLobby.playerIndexes, MAX_PLAYERS, 0);  // TODO: Remove debug
            emit StartedGame(gameStarted, newLobby.playerIndexes);
        }
    ...
    // TODO: DRY and or validate if this needs a lobby or should be elsewhere
    function enlistSolo(uint _charId, address charOwner) public {
        require(msg.sender == address(chars), "Not character contract");

        uint playerIndex = players.addPlayer(address(chars), _charId, charOwner);
        uint[] memory player = new uint[](1);
        player[0] = playerIndex;
        uint gameStarted = games.startGame(player, 1, 0);  // TODO: Remove debug
        emit StartedGame(gameStarted, player);
    }
