...
contract MembershipsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function publishMemberships(Models.NewAssetSaleInfo[] calldata memberships)
        external
        whenNotPaused
        nonReentrant
        returns (uint256[] memory membershipIds)
    {
        ...
        for (uint256 i = 0; i < quantity; i++) {
            ...
            offers[msg.sender][membershipId] = Models.Offer(memberships[i].amountToSell, memberships[i].price);
            newMembershipIds[i] = membershipId;
            organizerMemberships[msg.sender].push(membershipId);
        ...
        IMembership(membershipAddress).mintBatch(msg.sender, newMembershipIds, amounts, uris, '');

        for (uint256 i = 0; i < quantity; i++) {
            emit MembershipPublished(msg.sender, newMembershipIds[i], amounts[i], memberships[i], uris[i]);
        }

        return organizerMemberships[msg.sender];
    }
