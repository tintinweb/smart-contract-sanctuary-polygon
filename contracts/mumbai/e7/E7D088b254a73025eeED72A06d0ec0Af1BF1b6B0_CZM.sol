...
contract CZM is ERC20, Ownable {
    ...
    constructor() ERC20("CycleZero Meme", "CZM") {
        _mint(msg.sender, CYCLE_DURATION * 10 ** decimals());
        aiEnabled = true; // Enable the AI Time Assistant by default
    }

    function createTimeCapsule(uint256 startTime, uint256 endTime, uint256 deposit) external {
        require(startTime >= block.timestamp, "Start time must be in the future");
        require(endTime > startTime, "End time must be after start time");
        require(deposit > 0, "Deposit amount must be greater than 0");

        TimeCapsule memory newCapsule = TimeCapsule(startTime, endTime, deposit);
        timeCapsules.push(newCapsule);
        userTimeCapsules[msg.sender].push(timeCapsules.length - 1);

        _transfer(msg.sender, address(this), deposit);
    }

    function getTimeCapsuleCount(address user) external view returns (uint256) {
        return userTimeCapsules[user].length;
    }

    function getTimeCapsuleDetails(address user, uint256 index) external view returns (uint256, uint256, uint256) {
        require(index < userTimeCapsules[user].length, "Invalid time capsule index");

        TimeCapsule memory capsule = timeCapsules[userTimeCapsules[user][index]];
        return (capsule.startTime, capsule.endTime, capsule.deposit);
    }
