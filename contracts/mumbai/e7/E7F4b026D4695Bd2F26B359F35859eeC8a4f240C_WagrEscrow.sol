...
import "./interfaces/IWagrNFT.sol";

// TODO allow topup
// TODO collect fee for treasury
// TODO collect fee for Resolver
// TODO Accept NFTs
// TODO accept multiple currencies

contract WagrEscrow is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
    bytes32 public constant ROLE_ADMIN = keccak256("ROLE_ADMIN");

    IWagrNFT NFTContract;
    ...
    address payable public wagrFeeVault;

    uint256 public wagrFeeBps;
    uint256 private maxFeeBps;

    /**
     * @dev The asset deposited for a wager
     ...
     * @param partyResolver The address of the person who is handling the case
     * @param closed Whether the escrow has been closed
     * @param description The text describing what the wager is
     * @param determineTime The time the Wager can be judged. This is optional if set to 0.
    ...
    event WagerCancelled(uint256 escrowId);
    event WagerStarted(uint256 escrowId);
    event WagerDetermined(uint256 escrowId, address winner);
    event FundsDeposited(uint256 escrowId, address depositor, IERC20Upgradeable currency, uint256 amount);
    ...
    modifier onlyCaseResolver(uint256 escrowId) {
        require(
            escrows[escrowId].partyResolver == msg.sender,
    ...
    modifier onlyOpenCase(uint256 escrowId) {
        require(escrowIds.contains(escrowId), "Wager does not exist");
        require(!escrows[escrowId].closed, "Wager has been closed");
        _;
    }

    modifier onlyParticipatingParty(uint256 escrowId) {
        require(
            escrows[escrowId].partyA == msg.sender || escrows[escrowId].partyB == msg.sender,
    ...
    // -- FUNCTIONS
    function initialize(address _admin, address payable _wagrFeeVault) public initializer {
        require(_wagrFeeVault != address(0), "Fee Treasury wallet cannot be 0 address");
        _setupRole(ROLE_ADMIN, _admin);
        _setRoleAdmin(ROLE_ADMIN, ROLE_ADMIN);

        wagrFeeVault = _wagrFeeVault;
        maxFeeBps = 10000;
        wagrFeeBps = 100;
    }

    // TODO set fee for Arbitrator
    /**
    * @dev Create a new wager assigning counterparty, arbitrator and amounts
    * @param partyB The counterparty
    * @param partyResolver The Arbitrator
    ...
    */
    function createWager(
        ...
        uint256 determineTime,
        IERC20Upgradeable currencyToDepositA,
        uint256 amountToDepositA,
        IERC20Upgradeable currencyToDepositB,
        uint256 amountToDepositB
    ) external payable returns (uint256) {
        require(partyB != address(0) && partyResolver != address(0), "Cannot set the address to 0x for other parties");
        require(msg.sender != partyResolver && partyB != partyResolver, "Cannot set as Resolver the parties themselves");
        ...
        if (address(currencyToDepositA) != address(0)) {
            currencyToDepositA.safeTransferFrom(msg.sender, address(this), amountToDepositA);
        } else {
            // GAS token
    ...
    */
    ...
    onlyParticipatingParty(escrowId)
    {
        ...
        if (address(assetToDeposit.currency) != address(0)) {
            assetToDeposit.currency.safeTransferFrom(msg.sender, address(this), assetToDeposit.amount);
        } else {
            // GAS token
        ...
        emit FundsDeposited(escrowId, msg.sender, assetToDeposit.currency, assetToDeposit.amount);
        NFTContract.mintEscrowNft(escrows[escrowId].partyA, escrowId, true, "this should be a url to metadata");
        NFTContract.mintEscrowNft(escrows[escrowId].partyB, escrowId, false, "this should be a url to metadata");
        // TODO collect fee for treasury
    }
    ...
    */
    ...
    onlyCaseResolver(escrowId)
    {
        ...
        DepositAsset storage assetsA = escrowAssetsA[escrowId];
        DepositAsset storage assetsB = escrowAssetsB[escrowId];

        require(assetsA.amount > 0 && assetsB.amount > 0, "The parties have not completed their fund deposits yet");

        if (partyAWon) {
            transferFundsToParty(assetsA, escrows[escrowId].partyA);
            transferFundsToParty(assetsB, escrows[escrowId].partyA);
        } else {
            transferFundsToParty(assetsA, escrows[escrowId].partyB);
            transferFundsToParty(assetsB, escrows[escrowId].partyB);
        }
        escrows[escrowId].closed = true;
        // TODO collect fees for resolver
    }
    ...
    */
    function updateNftContractAddress(address _nftContractAddress) external onlyAdmin {
        require(_nftContractAddress != address(0));
        NFTContract = IWagrNFT(_nftContractAddress);
    }
    ...
    **/
    function transferFundsToParty(DepositAsset memory asset, address winner) internal {
        // NOT GAS token
        if (address(asset.currency) != address(0)) {
            asset.currency.safeTransfer(winner, asset.amount);
        } else { // GAS token
            (bool success, ) = winner.call{value: asset.amount}("");
            require(success, "Something went wrong with the transfer to the winner of the wager. Gas token");
        }
    }

    /**
     * @dev generate a new wager/escrow id (iterates by 1)
