...
/// @dev 
contract RequestMembers is Initializable {
    ...
    address[] private requestedAccounts;
    ...
    /// @param governanceTokenContract Addres of Governance Smart Contract.
    function initialize (address roleContract, address governanceTokenContract) initializer public  {
        roles = UsersGroups(roleContract);
        governanceToken = GovernanceToken(governanceTokenContract);
    }
    ...
    /// @notice Get UsersGroups Address.
    /// @dev Get UsersGroups Address.
    /// @return address Address of UsersRoles Contract.
    function getUsersRolesAddress()
    public
    view
    returns (UsersGroups) {
        return roles;
        ...
        if(!roles.isMember(_address) && ifRequestedAccounts(_address)){
                for(uint i=0;i < requestedAccounts.length; i++){
                    if(requestedAccounts[i] == _address){
                        ...
                        governanceToken.transfer_and_delegate(_address,1);
...
contract UsersGroups is Initializable {
  ...
  /// @notice List groups users's belongs.
  /// @dev List groups users's belongs.
  /// @param user User Address.
  /// @return list_groups Array of groups in bytes32.
  ...
  returns (bytes32[] memory)
  {
    ...
    for(uint8 i =0;i<groups.length;i++)
    {
      ...
      bytes32[] memory users_groups = new bytes32[](count);
      uint8 count_array = 0;
      ...
      for(uint8 i =0;i<groups.length;i++)
      {
        if(ifUserHasGroup(user, groups[i].group_name))
        {
          users_groups[count_array] =groups[i].group_name;
          count_array++;
        }
      }
      return users_groups;
...
contract GovernanceToken is Initializable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable{
    ...
    uint256 private s_maxSupply = 1000;
