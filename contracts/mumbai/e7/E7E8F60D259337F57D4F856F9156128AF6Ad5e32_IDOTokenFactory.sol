...
library dataTypes {
    ...
    struct tokenData {
        ...
        string __metacrypt_name;
        string __metacrypt_symbol;
...
import "../service/FactoryHelper.sol";

import "@uniswap/v2-periphery/contracts/interfaces/IUniswapV2Router02.sol";
import "@uniswap/v2-periphery/contracts/interfaces/IERC20.sol";
import "@uniswap/v2-core/contracts/interfaces/IUniswapV2Factory.sol";

contract reflectionToken is IERC20, ERC20Ownable, FactoryHelper {
    ...
    constructor(
        ...
        uint8 __metacrypt_tax_liquidity,
        uint8 __metacrypt_tax_buyback
    ) payable FactoryHelper("Metacrypt_B_TR_TAX_NC_X", __metacrypt_target) {
...
contract simpleToken is ERC20Decimals, ERC20Mintable, ERC20Burnable, TokenRecover, FactoryHelper {
    constructor(
        address __metacrypt_target,
        string memory __metacrypt_name,
        string memory __metacrypt_symbol,
        ...
        FactoryHelper("Metacrypt_B_TR_MB_US_NC_X", __metacrypt_target)
    {
...
abstract contract FactoryHelper {
    ...
    constructor(string memory _id, address _target) payable {
        __target = _target;
        __identifier = _id;
        payable(_target).transfer(msg.value);
    }

    function createdByZoopad() public pure returns (bool) {
        return true;
    }
...
import "./Mocks/simpleToken.sol";
import "./Mocks/reflectionToken.sol";
import "./lib/zoopadTokenDataType.sol";


contract IDOTokenFactory {
    ...
    function createToken(
        dataTypes.TokenTypes tokenType,
        dataTypes.tokenData memory tokenData
    ) external returns (address) {
        if (tokenType == dataTypes.TokenTypes.Simple) {
            address newContract = address(
                new simpleToken(
                    tokenData.__metacrypt_target,
                    tokenData.__metacrypt_name,
                    tokenData.__metacrypt_symbol,
                    tokenData.__metacrypt_decimals,
                    tokenData.__metacrypt_initial
                )
            );
            return newContract;
        } else {
            address newContract = address(
                new reflectionToken(
                    ...
                    tokenData.__metacrypt_decimals,
                    tokenData.__metacrypt_initial,
                    tokenData.__metacrypt_router,
                    tokenData.__metacrypt_tax_reflection,
                    tokenData.__metacrypt_tax_wallet,
                    tokenData.__metacrypt_tax_burn,
                    tokenData.__metacrypt_tax_liquidity,
                    tokenData.__metacrypt_tax_buyback
                )
            );
            return newContract;
        }
