...
contract RPSFeverNFT is ERC721Enumerable, Ownable {
    ...
    function game(uint _tokenId) public payable returns (uint256[] memory) {
        ...
        results[0] = player;
        results[1] = randomNo;
        results[2] = gameResults(player, randomNo);
        if (results[2] == 1) {
            rewardPlayer();
        }
        return results;
