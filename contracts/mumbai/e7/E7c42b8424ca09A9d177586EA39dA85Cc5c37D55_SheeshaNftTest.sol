...
import "./SheeshaNft.sol";

contract SheeshaNftTest is SheeshaNft {
    constructor(string memory uri_, address shvault_, address lpvault_) 
        SheeshaNft(uri_, shvault_, lpvault_) {
    }
...
contract SheeshaNft is
    ...
    string public constant NAME = "Shesha NFT 1";
    ...
    constructor(string memory uri_, address shvault_, address lpvault_) ERC721(NAME, SYMBOL) {
    ...
    /**
     * @dev Blocks user.
     * @param to Address to block
     */
    function blockMint(address to) public onlyOwner {
        require(!blocked[to], "SNFT: already blocked");
        blocked[to] = true;
        emit Blocked(_msgSender(), to);
    }

    /**
     * @dev Unblocks user.
     * @param to Address to unblock
     */
    function unblockMint(address to) public onlyOwner {
        require(blocked[to], "SNFT: already unblocked");
        delete blocked[to];
        emit Unblocked(_msgSender(), to);
    }
     ...
     */
    function canMint(address to) public view {
        require(!blocked[to], "SNFT: blacklisted");
        require(!_minters[to], "SNFT: already minted");
