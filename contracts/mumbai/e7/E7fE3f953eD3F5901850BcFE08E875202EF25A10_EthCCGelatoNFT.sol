...
contract EthCCGelatoNFT is ERC721MetaTxEnumerable, Proxied {
    ...
    modifier onlyRelay() {
        require(
            ...
            "EthCCGelatoNFT: Only gelato relay"
        );
        _;
    }
    ...
    ) ERC721MetaTx("EthCC Gelato", "ETHCCGELATO", _gelatoRelay) {
        maxSupply = _maxSupply;
        raffle = _raffle;
    }

    // Todo: function mint() external onlyRelay {
    function mint() external {
        uint256 supplyTotal = totalSupply();

        if (maxSupply > 0)
            require(supplyTotal < maxSupply, "EthCCGelatoNFT: Max Supply");
        ...
        returns (string memory)
    {
        ...
        (uint256 gelatoId, uint256 gelatoState) = raffle.getGelatoIdAndState(
            owner,
            tokenId
        );
    ...
    function _onePerAccount(address _account) private view {
        // Todo: require(balanceOf(_account) == 0, "EthCCGelatoNFT: One per account");
    }
}
