...
contract Groups is Pausable, ReentrancyGuard {
    ...
    constructor(address _kutils, uint256 _maxMembersPerGroup) {
    ...
    */
    ...
    function addTrustedContract(address newContract) public onlyAdmins{
        trustedContracts[newContract] = true;
    }

    function removeTrustedContract(address oldContract) public onlyAdmins{
    ...
    */
    ...
    function getOwnerOfGroupByID(uint256 groupID) public view returns (address){
        return groupDetails[groupID].ownerAddress;
    }

    function getMembersOfGroupByID(uint256 groupID) public view returns (address[] memory){
    ...
    function getGroupAddressFromID(uint256 groupID) public view returns (address){
        return groupDetails[groupID].groupAddress;
    }

    function getGroupIDFromAddress(address groupAddress) public view returns (uint256){
        return groupAddressToID[groupAddress];
    }

    function getOwnerOfGroupByAddress(address groupAddress) public view returns (address){
        return groupDetails[groupAddressToID[groupAddress]].ownerAddress;
    }

    function getGroupMemberships(address usrAddress) public view returns (uint256[] memory){
        return groupMemberships[usrAddress];
    }
    ...
    function addMemberToGroup(uint256 groupID, address member) public onlyGroupOwners(groupID) nonReentrant{
    ...
    function leaveGroup(uint256 groupID) public onlyGroupMembers(groupID, msg.sender) nonReentrant{
        removeMember(groupID, msg.sender);
    }

    // This allows users to change the case of their group name (mygroup => MyGroup)
    function updateGroupNameFormat(uint256 _groupID, string calldata _groupName) public onlyGroupOwners(_groupID) nonReentrant {
        // Make sure the name is still the same
        require(_groupID == getGroupID(_groupName), "Can only change the case");

        // Set the group name
        groupDetails[_groupID].groupName = _groupName;
    }

    function removeMemberFromGroup(uint256 groupID, address member) public onlyGroupOwners(groupID) nonReentrant {
        removeMember(groupID, member);
    }

    function removeMember(uint256 groupID, address member) internal {
        console.log('leaving group - inside');
    ...
    */
    ...
    function setBaseURI(string memory _baseURI, uint256 _tokenID) public onlyTrustedContracts {
        groupDetails[_tokenID].baseURI = _baseURI;
    }

    function setInitialDetails(uint256 _groupID, address _owner, string memory groupName, string memory _baseURI, address tokenContractAddress) public onlyTrustedContracts {
        ...
        // Set the base URI
        groupDetails[_groupID].baseURI = _baseURI;
        ...
        UserProfiles.setupNewGroup(groupAddress, groupName, _groupID, tokenContractAddress, _baseURI);
    }
}
