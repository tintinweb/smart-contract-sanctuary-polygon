...
contract Roulette is VRFConsumerBaseV2, Ownable {
  ...
  uint32 private constant CALL_BACK_GAS_LIMIT = 2500000;  
  ...
  */
  function checkWinner() external onlyOwner {
    uint256 number = betIdToBets[currentBetId].randomNumber;
      for (uint i = 0; i < players[currentBetId].length; i++) {
        bool won = false;
        UserBet storage b = players[currentBetId][i];
      if (number == 0) {
      ...
      emit WinAmount(currentBetId, b.winningAmount);
    }
  }
