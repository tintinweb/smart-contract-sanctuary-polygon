...
contract RollerskateQuotes is ERC721A, Ownable, ReentrancyGuard {
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
    ...
    if (keccak256(abi.encodePacked(seedPhrase)) == keccak256(abi.encodePacked(''))) {
      ...
      bytes32 _bytes32 = keccak256(abi.encodePacked(_tokenId.toString(),seedPhrase));

      for (uint256 i; i < 32; i++) {
        bytesArray[i] = _bytes32[i];
      }      
      metadataJsonFilename = string(bytesArray);
    }
