...
contract SwapV2 is AccessControl, ReentrancyGuard {
    ...
    address public minter; // aave minter
    ...
    function mint(address acceptedToken, uint256 amount, uint256 minAmount) external {
        ...
        require((amountCRUS >= MIN_AMOUNT || _msgSender() == factoryDIF) , "Minimum 1 USD");
     ...
     * @return success Is operation successful
     */
    function _fundReservationMint(uint256 amount) internal returns(bool success){
        (uint256 taa, , , uint256 vaultBalance) = getBalances();
     ...
     * @return success Is operation successful
     */
    function _fundReservationMintEth(uint256 amount) internal returns(bool success){
        (uint256 taa, , , uint256 vaultBalance) = getBalances();
        uint256 state = _getReservationState(vaultBalance, taa + amount);
        success = true;
        if (state == 0) {
    ...
    function _withdrawLP(uint256 amount) internal {
        if (ICurveFiGauge(gauge).balanceOf(address(this)) < amount) revert NotEnoughLiquidity();
