  ...
  OwnableUpgradeable
{
  ...
  event NewTokensAvailable(uint256 indexed NFTHash, uint256 available);
  event NewNFTHashAvailable(uint256 indexed NFTHash, uint256 price);
  event UpdateNFTHashPrice(uint256 indexed NFTHash, uint256 price);
  event NFTHashSell(
    uint256 price,
    uint256 discount,
  ...
  uint256 public initiatedNFTHash;
  ...
  function _addTokensHashToNFTHash(
    uint256 NFTHashIndex,
    bytes32[] memory tokensHash
  ) internal {
    require(NFTHashs[NFTHashIndex].exist, "NFTHash not exist");
    for (uint256 i = 0; i < tokensHash.length; i++) {
      NFTHashs[NFTHashIndex].tokensHash[
        NFTHashs[NFTHashIndex].appendIndex
      ] = tokensHash[i];
      NFTHashs[NFTHashIndex].appendIndex++;
    }
    emit NewTokensAvailable(
      NFTHashIndex,
      NFTHashs[NFTHashIndex].appendIndex - NFTHashs[NFTHashIndex].sellIndex
    );
  }

  function _addNFTHash(uint256 price, bytes32[] memory tokensHash) internal {
    NFTHashs[initiatedNFTHash].exist = true;
    NFTHashs[initiatedNFTHash].price = price;
    NFTHashs[initiatedNFTHash].sellIndex = 0;
    NFTHashs[initiatedNFTHash].appendIndex = 0;
    _addTokensHashToNFTHash(initiatedNFTHash, tokensHash);
    emit NewNFTHashAvailable(initiatedNFTHash, price);
    initiatedNFTHash++;
  }

  function addNFTHash(uint256 price, bytes32[] memory tokensHash)
    external
    onlyOwner
  {
    _addNFTHash(price, tokensHash);
  }

  function addTokensHashToNFTHash(
    uint256 NFTHashIndex,
    bytes32[] memory tokensHash
  ) external onlyRole(STOCKER) {
    _addTokensHashToNFTHash(NFTHashIndex, tokensHash);
  }

  function updateNFTHashPrice(uint256 NFTHashIndex, uint256 price)
    external
    onlyOwner
  {
    require(NFTHashs[NFTHashIndex].exist, "NFTHash not exist");
    NFTHashs[NFTHashIndex].price = price;
    emit UpdateNFTHashPrice(initiatedNFTHash, price);
  }
  ...
  function _buyNFTHash(
    ...
    emit NFTHashSell(price, paymentTokenDiscount, paymentToken, tokenHash);
    ...
    HashNFT.mint(_msgSender(), tokenHash);
  }

  function buyAvailableNFTHash(uint256 NFTHashIndex, address paymentToken)
    external
    payable
  {
    NFTHash storage NFTHash = NFTHashs[NFTHashIndex];
    require(NFTHash.appendIndex > NFTHash.sellIndex, "No available stock");

    bytes32 tokenHash = NFTHash.tokensHash[NFTHash.sellIndex];
    NFTHash.sellIndex++;

    _buyNFTHash(NFTHash.price, tokenHash, paymentToken);
  }

  function buyMetaNFTHash(
    uint256 price,
    bytes32 tokenHash,
    address paymentToken,
    ...
    _buyNFTHash(price, tokenHash, paymentToken);
  }
