    ...
    Ownable
{
    ...
    event CancelListing(uint256 _tokenId, address _seller, uint256 _sellerNonce, uint256 _globalNonce);
    event MatchListing(uint256 _tokenId, address _buyer, address _seller, uint256 _sellerNonce, uint256 _globalNonce);
    event MatchEnabled(bool _enabled);
    ...
    bytes32 public constant SERVER_AUTH_TYPEHASH = keccak256("ServerAuth(address buyer,address seller,uint256 tokenId,uint256 baseFeeNumerator,uint256 txnExpiry,uint256 sellerNonce,uint256 globalNonce)");
    bytes32 public constant ADD_LISTING_TYPEHASH = keccak256("AddListing(address seller,uint256 tokenId,uint256 listingExpiry,address baseTokenAddress,uint256 basePrice,uint256 sellerNonce,uint256 globalNonce)");

    bytes32 private constant EIP712_SALT = 0x074d6bce6505163759a773f6983edc99ac75c0206038232dad435b20e7a42f40; 
    ...
    function setMatchEnabled(bool enabled) external onlyOwner {
        matchEnabled = enabled;
        emit MatchEnabled(matchEnabled);
    }
    ...
    function getNonces(address owner, uint256 tokenId) public view returns (uint256, uint256) {
        return (_sellerNonces[owner][tokenId], globalNonce);
    }

    /**
     * @dev Generates the EIP712 hash that was signed for a seller listing their venari in exchange for an ERC20
     */ 
    function generateAddListingDigest(AddListing memory addListing) public view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(
                        ...
                        addListing.seller,
                        addListing.tokenId,
                        addListing.listingExpiry,
                        addListing.baseTokenAddress,
                        addListing.basePrice,
                        addListing.sellerNonce,
                        addListing.globalNonce
                    )
                )
            )
        );
     ...
     */ 
    function generateServerAuthDigest(ServerAuth memory serverAuth) public view returns (bytes32) {
        return keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(
                    abi.encode(
                        ...
                        serverAuth.seller,
                        serverAuth.tokenId,
                        serverAuth.baseFeeNumerator,
                        serverAuth.txnExpiry,
                        serverAuth.sellerNonce,
                        serverAuth.globalNonce
                    )
                )
            )
        );
    ...
    function matchListing(
        ...
        require(matchEnabled, "Matching not enabled");
        ...
        require(baseFeeNumerator <= feeDenominator, "Invalid baseFeeNumerator");
        ...
        {   
            ...
            AddListing memory addListingArgs = AddListing(seller, tokenId, listingExpiry, baseTokenAddress, basePrice, _sellerNonces[seller][tokenId], globalNonce);
            bytes32 addListingDigest = generateAddListingDigest(addListingArgs);
            require(ECDSA.recover(addListingDigest, sellerSig) == seller, "Recovered address does not match seller");
        }
        
        {   
            // Scoping for stack too deep error
            ServerAuth memory serverAuthArgs = ServerAuth(msg.sender, seller, tokenId, baseFeeNumerator, txnExpiry, _sellerNonces[seller][tokenId], globalNonce);
            bytes32 serverAuthDigest = generateServerAuthDigest(serverAuthArgs);
            require(ECDSA.recover(serverAuthDigest, serverSig) == signVerifier, "Recovered address does not match server");
        }
        ...
        {
            // Scoping for stack too deep error
            uint256 sellerNonce = _sellerNonces[seller][tokenId];
            venariInterface.transferFrom(seller, msg.sender, tokenId);
            emit MatchListing(tokenId, msg.sender, seller, sellerNonce, globalNonce);
        }
    ...
    function cancelListing(uint256 tokenId) external {
        ...
        emit CancelListing(tokenId, msg.sender, _sellerNonces[msg.sender][tokenId], globalNonce);
