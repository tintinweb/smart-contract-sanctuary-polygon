...
contract ClaimNFT is Initializable, ERC2771ContextUpgradeable, UUPSUpgradeable {
    ...
    function registerERC1155(string memory _sessionId, address _contractAddress, uint256 _tokenId, address _owner, uint256 _supply, uint256 _from, uint256 _to) external {
    ...
    function claimSingleERC721(string memory _sessionId) public {
        ...
        contract721.safeTransferFrom(data.owner, data.toAddress, data.tokenId);
        emit ERC721Claim(data.toAddress, data.tokenId, data.contractAddress);
    }
    ...
    function cancel(string memory _sessionId) external onlyOperator {
    ...
    function editSessionTime(string memory _sessionId, uint8 claimType, uint256 _from, uint256 _to) external {
        if(claimType == 0) {
            RegisterListERC1155 memory data = registrationListRecord[_sessionId];
            require(_msgSender() == data.owner, "Permission denied to edit!");
            data.from = _from;
            data.to = _to;
            registrationListRecord[_sessionId] = data;
        } else if(claimType == 1) {
