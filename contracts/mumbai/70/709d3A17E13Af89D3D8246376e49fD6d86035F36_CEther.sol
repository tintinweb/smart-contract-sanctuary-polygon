...
contract CEther is CToken {
    ...
    function borrow(address payable holder, uint borrowAmount) external returns (uint) {
        return borrowInternal(holder, borrowAmount);
    }
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowInternal(address payable holder, uint borrowAmount) internal  returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
        ...
        return borrowFresh(holder, msg.sender, borrowAmount);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowFresh(address payable borrower, address payable holder, uint borrowAmount) internal returns (uint) { // holder => margin
         ...
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
        doTransferOut(holder, borrowAmount);
    ...
    /**
     * @dev Split off from `` to keep contract below the 24 KB size limit.
