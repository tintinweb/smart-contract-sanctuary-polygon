...
contract ERC721Token is ERC721URIStorage, Pausable, WhitelistMintableTokenBase , IERC721TokenContract,IERC721WhitelistMintable 
{
    ...
    bytes32 private _unreveal_token_uri_hash;
    ...
    constructor(string memory name_, string memory symbol_,uint256 max_supply_,string memory unreveal_name_,string memory unreveal_token_uri_) 
        ...
        _unreveal_token_uri_hash = keccak256(bytes(unreveal_token_uri_));
    }
    ...
    function _get_owned_tokenids_of(address _addr) internal view returns(uint256[] memory) {
    ...
    function firstOwnedTokenOf(address _addr,uint256 count) external view override returns (uint256[] memory,uint256) {
        uint256 counter = 0;
        uint256[] memory outTokenIds = new uint256[](count);
        uint256 lastId =_tokenIds.current();
        for (uint256 tId = 1; tId <= lastId; tId++) {
            if(_ownerOf(tId) == _addr){
                outTokenIds[counter] = tId;
                counter++;
                if(counter >= outTokenIds.length)
                    break;
            }
        }
        return (outTokenIds,counter);
    }
    ...
    function ownedTokenOf(address _addr) external virtual override view returns(uint256[] memory){
       return _get_owned_tokenids_of(_addr);
    }
    ...
    function updateUnrevealTokenUri(string memory token_uri) external virtual onlyOwner {
       bytes32 _new_hash = keccak256(bytes(token_uri));
       if(_unreveal_token_uri_hash != _new_hash){
            _apply_unreveal_uri_changed(token_uri);
            _unreveal_token_uri = token_uri;
            _unreveal_token_uri_hash = _new_hash;
       }
    }
    ...
    function updateTokenURI(uint256 tokenId,string memory tokenURI) external virtual override onlyOwnerAndOperatableTemplate {
    ...
    function getUnrevealTokenOf(address _addr,uint256 count) external view override returns (uint256[] memory){
       uint256[] memory token_ids = _get_owned_tokenids_of(_addr);
       uint256 _count = (token_ids.length < count) ? token_ids.length : count;
       uint256 counter = 0;
       uint256[] memory out_token_ids = new uint256[](_count);
       for(uint256 i = 0 ; i < token_ids.length; i++){
           if(_is_unreveal_uri(tokenURI(token_ids[i]))){
                out_token_ids[counter] = token_ids[i];
                counter++;
                if(counter >= out_token_ids.length)
                    break;
           }
       }
    ...
    function _is_unreveal_uri(string memory uri) internal view returns (bool){
        return (bytes(uri).length == bytes(_unreveal_token_uri).length) && (keccak256(bytes(uri)) == _unreveal_token_uri_hash);
    }
    function _apply_unreveal_uri_changed(string memory new_uri) internal {
        uint256 last_token_id = _tokenIds.current();
        for (uint256 tId = 1; tId <= last_token_id; tId++){
            if(_is_unreveal_uri(tokenURI(tId)))
                _setTokenURI(tId, new_uri);
        }
    }
    function isTokenRevealed(uint256 token_id) external view override returns (bool){
        return (!_is_unreveal_uri(tokenURI(token_id)));
    }
}
