...
contract DegenDNSFactory is IBaseDegenDNSFactory, Ownable, ReentrancyGuard {
  ...
  address public metadataAddress; // default DegenDNSMetadata address
  
  uint256 public price; // price for creating a new TLD
  uint256 public royalty = 0; // royalty for Degen Domains when new domain is minted 
...
contract DegenDNS is IBaseDegenDNS, ERC721, Ownable, ReentrancyGuard {
  ...
  function tokenURI(uint256 _tokenId) public view override returns (string memory) {
    return IDegenDNSMetadata(metadataAddress).getMetadata(
  ...
  function _sendPayment(uint256 _paymentAmount, address _referrer) internal {
    if (royalty > 0 && royalty < 3000) { 
      // send royalty - must be less than 30% (3000 bips)
    ...
    if (_referrer != address(0) && referral > 0 && referral < 7000) {
      // send referral fee - must be less than 70% (7000 bips)
  ...
  function changeReferralFee(uint256 _referral) external override onlyOwner {
    require(_referral < 7000, "Referral fee cannot be 70% or higher");
