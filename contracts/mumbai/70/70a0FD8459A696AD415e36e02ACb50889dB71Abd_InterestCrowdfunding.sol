...
import "./Calcs.sol";
import "./ICrowdfundingControl.sol";
import "./IVaultInterestFunders.sol";
import "./IFounders.sol";


/// @title A Contract to claim Interests to be paid to Crowdfunding.
/// @author Panoram Finance / Voha.
/// @notice You can use this contract to claim the interest earning for any user in Crowdfunding Section.
contract InterestCrowdfunding is ReentrancyGuard, Calcs, IFounders {
    ...
    /// @param _CrowControl - the address of the CrowFunding Control contract.
    constructor(address _tokenInfo, address _CrowControl) Calcs(_CrowControl) {
        //Set Roles
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(DEV_ROLE, msg.sender);
        _setupRole(DEFAULT_ADMIN_ROLE, 0x30268390218B20226FC101cD5651A51b12C07470); // cambiar por el Multisign
    ...
    /// @param _idCrowFunding the user's CrowFunding ID.
    /// @param _token - the allowed token address for withdrawals.
    function SetClaimRewardsRequest(uint256 _idCrowFunding, address _token) public validToken(_token) isPaused nonReentrant{
        (uint256 amountRewards, uint256 timeOfCalc) = calcInterestAccumulated(msg.sender, _idCrowFunding);
        if(amountRewards == 0 && timeOfCalc == 0){
            revert("CrowdFunding interest was Fully Paid");
        }
        uint256 totalRewards = amountRewards + CrowdControl.getPendingRewards(msg.sender, _idCrowFunding);
        totalRewards /= 1e12;
        (,address FoundersRewards) = tokenInfo.getCrowdVaults(_token);
        if(totalRewards == 0){ 
            revert("You have not enough rewards yet");
        }
        uint256 amountVault = IVaultInterestFunders(FoundersRewards).getTotalRewards();
        if(totalRewards > amountVault){
            revert("Not enough rewards in the vault");
        }
        uint96 date = CrowdControl.getLastSetClaimRewards(msg.sender, _idCrowFunding);
         if((block.timestamp - uint256(date)) < waitTimeForRewards){
            revert("You must wait at least 24 hours to Set a Rewards Claim Again");
        } 

        (uint256 amountClaimed, , , uint256 totalToPaid) = CrowdControl.getRewardsClaimed(msg.sender, _idCrowFunding);
        if((amountClaimed + totalRewards) > totalToPaid){
            totalRewards = totalToPaid - amountClaimed; // the user can claim only totalToPaid as the maximum interest amount to claim.
        }
        

        IVaultInterestFunders(FoundersRewards).addRequest(totalRewards);
        uint256 _id = CrowdControl.createRequest(msg.sender, 0, totalRewards,2);
        CrowdControl.updateLastTimeClaim(msg.sender, _idCrowFunding, timeOfCalc);
        // Actualizar el valor de los rewards pendientes
        CrowdControl.updatePendingRewards(msg.sender, _idCrowFunding, timeOfCalc);
    ...
    /// @param _idCrowdfund the user's lending ID.
    /// @param _token - the allowed token address for withdrawals.
    function claimRewards(uint256 _idRequest, uint256 _idCrowdfund, address _token) public nonReentrant validToken(_token) isPaused{
        (,uint256 rewards, Status state, uint96 date) = CrowdControl.getRequest(msg.sender, _idRequest);
        ...
        if(checkLastClaimTime <= waitTimeForRewards){
         ...
         (,address FoundersRewards) = tokenInfo.getCrowdVaults(address(_token));
        if(rewards > IVaultInterestFunders(FoundersRewards).getTotalRewards()){
            revert("Not enough rewards in the vault");
        } 
        IVaultInterestFunders(FoundersRewards).claimRequest(rewards, _token);
        IERC20(_token).safeTransfer(msg.sender, rewards);

        CrowdControl.updateClaimed(_idCrowdfund, msg.sender, rewards, block.timestamp);
        CrowdControl.closeRequest(msg.sender, _idRequest, Status.complete, _idCrowdfund);

         (uint256 amountClaimed, , , uint256 totalToPaid) = CrowdControl.getRewardsClaimed(msg.sender, _idCrowdfund);
        if(amountClaimed >= totalToPaid){
            CrowdControl.setAllInteresPaid(msg.sender, _idCrowdfund, true);
        } 

        emit ClaimRewardsSuccess(msg.sender, _idCrowdfund, rewards); 
    }
    ...
    function updateCrowdControl(address _LendingControl) external onlyDev checkAddress0(_LendingControl) {
        CrowdControl = ICrowdfundingControl(_LendingControl);
    }
...
/// @title Vault Interest Funders Interface.
/// @author Panoram Finance.
/// @notice You can use this interface to connect to the Vault Interest for Funders contract.
interface IVaultInterestFunders {
