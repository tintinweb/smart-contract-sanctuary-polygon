...
contract Jackpot is Ownable, Pausable, IJackpot {
    ...
    constructor(address _tickets, address _token) {
        tickets = _tickets;
        token = _token;
    }
    ...
    function _transfer(address to, uint256 amount) internal {
        if(token == address(0)) {
            require(payable(to).send(amount), "Jackpot: token send failed");
        } else {
