...
contract PlaytestSessionWildeventHook is IWildeventHook {
    event PlaytestSessionComplete(
        uint32[] wildfileIds,
        uint256 started,
        string venue,
        string specialEvent,
        uint16 durationMinutes,
        uint16[] wildfileMinutesWatched
    );
    ...
    function onWildevent(uint32[] calldata wildfileIds, bytes calldata data) external override {
        ...
        (
            uint256 started,
            string memory venue,
            string memory specialEvent,
            uint16 durationMinutes,
            uint16[] memory wildfileMinutesWatched
        ) = decode(data);
        emit PlaytestSessionComplete(wildfileIds, started, venue, specialEvent, durationMinutes, wildfileMinutesWatched);
    }

    function encode(
        uint256 started,
        string calldata venue,
        string calldata specialEvent,
        uint16 durationMinutes,
        uint16[] memory wildfileMinutesWatched
    ) public pure returns (bytes memory data) {
        data = bytes.concat(data, LibEncode.encodeUint256(started));
        data = bytes.concat(data, LibEncode.encodeString(venue));
        data = bytes.concat(data, LibEncode.encodeString(specialEvent));
        data = bytes.concat(data, LibEncode.encodeUint16(durationMinutes));
        data = bytes.concat(data, LibEncode.encodeUint16Array(wildfileMinutesWatched));
        return data;
    }
    ...
    function decode(bytes memory data)
        public
        pure
        returns (uint256, string memory, string memory, uint16, uint16[] memory)
    {
        uint40 offsetBytes = 0;
        uint256 started;
        string memory venue;
        string memory specialEvent;
        uint16 durationMinutes;
        ...
        (started, offsetBytes) = LibEncode.decodeUint256(data, offsetBytes);
        (venue, offsetBytes) = LibEncode.decodeString(data, offsetBytes);
        (specialEvent, offsetBytes) = LibEncode.decodeString(data, offsetBytes);
        (durationMinutes, offsetBytes) = LibEncode.decodeUint16(data, offsetBytes);
        (wildfileMinutesWatched,) = LibEncode.decodeUint16Array(data, offsetBytes);

        return (started, venue, specialEvent, durationMinutes, wildfileMinutesWatched);
    }
}
