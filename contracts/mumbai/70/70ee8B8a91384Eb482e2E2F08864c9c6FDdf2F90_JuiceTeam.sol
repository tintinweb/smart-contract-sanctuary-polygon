...
import 'EthereumContracts/contracts/utils/BlacklistRegistry.sol';
import 'EthereumContracts/contracts/utils/ContractState.sol';
import 'EthereumContracts/contracts/utils/ProxyAccess.sol';
...
BlacklistRegistry, ProxyAccess, ERC2981, ERC173, ContractState, Whitelist_ECDSA {
    /**
    ...
    * @dev Ensures that `id_` is a valid series
    ...
    /**
    * @dev Ensures that `operator_` is not forbidden to transfer tokens.
		*
		* @param operator_ : the operator to validate
		*/
    modifier onlyAllowedOperator(address operator_) {
        if (!isAllowedOperator(operator_)) {
            revert BlacklistRegistry_NOT_ALLOWED(operator_);
        }
        _;
    }
    // **************************************
    ...
    * @notice Removes a proxy registry from the list of accepted proxy registries.
		...
		*
		* - Caller must be the contract owner.
		* - `address_` must be blacklisted.
		*/
    function removeBlacklistAddress(address address_) public onlyOwner {
        _removeBlacklistedAddress(address_);
    }
