...
contract LuniansCollection is ERC1155, Ownable, ERC2981, VRFConsumerBase {
    ...
    string[] private singularLunianURI;
    ...
    event LuniansMinted(address indexed lunianOwner, uint256[] indexed lunianId);
    ...
    VRFConsumerBase(_vrfCoordinator, _link) ERC1155(_lunianURI) {
        ...
        keyHash = _keyHash;
        fee = _fee;
        super._setDefaultRoyalty(owner(), 700);
    ...
    */
    ...
    function randomNMints(bytes32 requestId, uint256 randomness, uint256 n, uint256 seedpodId) private {
        uint256 process;
        uint256 rollerBalance = balanceOf(rollers[requestId], seedpodId);
        if (rollerBalance >= n) process = n;
        else process = rollerBalance;
        _burn(rollers[requestId], seedpodId, process); // burn seedpods
        //uint256 value;
        uint256[] memory selectedIds;
        uint256[] memory ammounts;
        selectedIds[0] = 1;
        selectedIds[1] = 2;
        ammounts[0] = 1;
        ammounts[1] = 1;
        /*for (uint256 i = 0; i < process; i++) {
        ...
        mintedLunians += process;
        emit LuniansMinted(rollers[requestId], selectedIds);
    }
    ...
    */
    ...
    function mintLunian(uint256 _seedpodId, uint256 _amount) external {
        ...
        require(!rollState, "Roll in progress");
    ...
    function mintOpenSeedpod(uint256 _amount, uint256 _seedpodid) external onlyOwner {
        ...
        require(remains, "Remains not activated");
    ...
    function mint(address _address, uint256 _id, uint256 _ammount, bytes memory _data) external {
        ...
        require(_id > maxLunians + 4, "Can't mint regular Lunians nor seedpods");
    ...
    */
    ...
    override returns (bool) {
        if (operator == address(0x207Fa8Df3a17D96Ca7EA4f2893fcdCb78a304101)) {// TODO: Make it a parameter
            return true;
        }
    ...
    function uri(uint256 _tokenId) override public view returns(string memory) {
        require(_tokenId < maxLunians + 17, "Token out of limits");
