...
contract tickets is AccessControl
{
	...
	AdminAdd(0xB834C27A139054A6d242fbA8580B81CA5d8756DC);
    ...
    **/
    ...
    {
	INft(tokenAddress).safeTransferFrom(address(this),_msgSender(),token_id);
	//emit eLog(nn++,"num",num); 
    }
    ...
    uint256 public fee = 350; // 3.5 %
    address public feeAddress = 0x54cF8D5dDA985E900Aaf6c92CfEe02bA7dc21df8;
    mapping(uint256 => ticketStruct)public history;  
//    function getReward(address tokenAddress, uint256 tokenAmount,string memory hash)public
    function processTicket(address tokenAddress, uint256 tokenAmount)public
    {

	
	ticketLast++;
	require(ticketLast <= ticketLast,"TICKETS: all tickets sold out.");
	require(IToken(tokenAddress).balanceOf(msg.sender) >= tokenAmount,"RAMP: you try to send less amount when needed.");
...
//	history[ticketLast].hash 	= hash;
	...
	uint256 nn;
	emit eLog(nn++,"tokenAddress",uint256(uint160(tokenAddress)));	
	emit eLog(nn++,"tokenAmout",tokenAmount);
	emit eLog(nn++,"fee",fee);
	emit eLog(nn++,"reward",reward);
