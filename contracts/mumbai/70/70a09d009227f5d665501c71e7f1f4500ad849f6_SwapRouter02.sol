...
// File: contracts/base/AdminLogic.sol
...
abstract contract AdminLogic {
  ...
  modifier OnlyAdmin() {
      require(isAdmin(msg.sender), "DiviRouter: FORBIDDEN");
      _;
  }

  constructor(address[] memory _admins) {
    require(_admins.length > 0, "DiviRouter: YOU_HAVE_TO_PROVIDE_ADMINS");
    admins = _admins;
  }

  // **** ADMINS LOGIC ****
  function addAdmin(address _newAdmin) external OnlyAdmin {
      require(!isAdmin(_newAdmin), "DiviRouter: ADMIN_ALREADY_EXIST");
  ...
  function removeAdmin(address _adminAddress)
      ...
      returns (bool)
  {
      require(isAdmin(_adminAddress), "DiviRouter: INVALID_ADMIN_ADDRESS");
      require(
          msg.sender != _adminAddress,
          "DiviRouter: YOU_CANNOT_REMOVE_YOURSELF"
      );
      require(
          admins.length > 1,
          "DiviRouter: YOU_CANNOT_REMOVE_THE_LAST_ADMIN"
      );
...
// File: contracts/base/FeeLogic.sol


pragma solidity =0.7.6;


abstract contract FeeLogic is AdminLogic {
  address public feeWallet;
  uint public feeRate = 100; // 1.00% in basis points (parts per 10,000)

  constructor(address _feeWallet, address[] memory _admins) AdminLogic(_admins) {
    feeWallet = _feeWallet;
  }

  // **** FEE RATE FUNCTIONS ****
  function estimateFee(uint amount) public view returns (uint) {
      return feeRate == 0 ? amount : (amount * feeRate) / 10000;
  }

  function getUpdatedAmount(uint amount, uint amountFee)
      public
      pure
      returns (uint)
  {
      return amount - amountFee;
  }

  function changeFeeRate(uint newFeeRate) external OnlyAdmin {
      require(
          newFeeRate < 10000 && newFeeRate >= 0,
          "DiviRouter: Invalid feeRate"
      );
      feeRate = newFeeRate;
  }

  function changeFeeWallet(address newFeeWallet) external OnlyAdmin {
...
abstract contract V3SwapRouter is IV3SwapRouter, PeripheryPaymentsWithFeeExtended, OracleSlippage, FeeLogic, ERC2771Recipient {
    using Path for bytes;
    using SafeCast for uint256;

    event LogDataItem(uint256 value, address tokenIn, address payer, address target, uint256 amout);
    ...
    function uniswapV3SwapCallback(
        ...
        emit LogDataItem(0, tokenIn, data.payer, msg.sender, amountToPay);
        ...
        returns (uint256 amountOut)
    {
        ...
        amountOut = exactInputInternal(
            ...
            SwapCallbackData({
                ...
                payer: hasAlreadyPaid ? address(this) : _msgSender()
            })
        );
    ...
    function exactInput(ExactInputParams memory params) external payable override returns (uint256 amountOut) {
        ...
        address payer = hasAlreadyPaid ? address(this) : _msgSender();
        ...
        returns (uint256 amountIn)
    {
        ...
        amountIn = exactOutputInternal(
            ...
            params.recipient,
            params.sqrtPriceLimitX96,
            SwapCallbackData({path: abi.encodePacked(params.tokenOut, params.fee, params.tokenIn), payer: _msgSender()})
        );
    ...
    function exactOutput(ExactOutputParams calldata params) external payable override returns (uint256 amountIn) {
        exactOutputInternal(
            ...
            params.recipient,
            0,
            SwapCallbackData({path: params.path, payer: _msgSender()})
        );
...
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended, FeeLogic {
    using LowGasSafeMath for uint256;
    ...
    function swapExactTokensForTokens(
        ...
        uint amountFee = estimateFee(amountIn);
        amountIn = getUpdatedAmount(amountIn, amountFee);
    ...
    function swapTokensForExactTokens(
        ...
        uint amountFee = estimateFee(amountOut);
        amountOut = getUpdatedAmount(amountOut, amountFee);
        pay(path[0], msg.sender, feeWallet, amountFee);
...
// File: @uniswap/v3-periphery/contracts/base/SelfPermit.sol
...
contract SwapRouter02 is ISwapRouter02, V2SwapRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    ...
    ) ImmutableState(_factoryV2, _positionManager) PeripheryImmutableState(factoryV3, _WETH9) FeeLogic(_feeWallet, _admins) {}
}