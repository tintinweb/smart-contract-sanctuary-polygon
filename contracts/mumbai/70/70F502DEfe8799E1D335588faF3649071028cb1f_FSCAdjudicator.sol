...
// once a contentBond is challenged, the adjudication is underway; and from that time the bonder can no longer withdraw his contentBond.
// funds related to content (contentBond, challengeFee, commentFee) are maintained in content.holdoverAmount. from here funds are deducted
// as they are distributed for eg. LINK fees. at the end of an adjudication, the vindication award, challenge award, lottery award, and
// revealer award are all paid out if the content.holdoverAmount.
//
contract FSCAdjudicator is AccessControl, VRFConsumerBase {
  ...
  event CommentFeeUpdated(uint256  oldCommentFee, uint256 newCommentFee);
  ...
  event ContentBondEv(uint256 indexed hash, uint256 indexed timestamp, address indexed bonder, bytes title, bytes comment);
  event CommentEv(uint256 indexed hash, uint256 indexed timestamp, address indexed commenter, bytes title, bytes comment);
  event ChallengeBondEv(uint256 indexed hash1, uint256 indexed timestamp, address indexed challenger, bytes title, bytes comment);
  event EncryptedVoteEvent(uint256 indexed _hash1, address indexed jurorAddr,
			   bytes32 encryptedVote0, bytes32 encryptedVote1, bytes32 encryptedVote2, bytes32 encryptedVote3,
			   bytes32 encryptedVote4, bytes32 encryptedVote5, bytes32 encryptedVote6, bytes32 encryptedVote7);
  event AllVotesCastEvent(uint256 indexed hash, uint256 noVotes);
  event VerdictEvent(uint256 indexed hash, uint256 verdict, uint256 isFreeVoteCount, uint256 notFreeVoteCount);
  event AwardsEvent(uint256 indexed hash, uint256 vindicationAward, uint256 challengeAward, uint256 voterRewardReserve, uint256 revealAward, uint256 dividendAmount);
  ...
  // Revealer can reveal certified votes
  bytes32 public constant REVEALER_ROLE = keccak256("REVEALER_ROLE");
  bytes32 public constant REVEALER_ADMIN_ROLE = keccak256("REVEALER_ADMIN_ROLE");
  ...
  // values for content.verdict
  uint8   constant VERDICT_UNADJUDICATED   = 0x00;
  uint8   constant VERDICT_CHALLENGED      = 0x01;
  uint8   constant VERDICT_VOTE_PENDING    = 0x02;
  uint8   constant VERDICT_REVEAL_PENDING  = 0x03;
  uint8   constant VERDICT_NOT_FREE_SPEECH = 0x04;
  uint8   constant VERDICT_IS_FREE_SPEECH  = 0x05;

  // mask certified vote to get verdict
  uint8   constant VOTE_FREE_SPEECH_MASK   = 0x01;
  ...
  //
  //                                                lottery
  //                                     award*votes   | challenge/vindication
  //                                             |     |    |   rand  reveal
  //                                             V     V    V    V     V
  // adjudication cost (is free speech):   100 * 0.2 + 10 + 10 + 0.1 + 1 = 41.1 ether
  // adjudication cost (not free speech):  100 * 0.2 + 10 + 25 + 0.1 + 1 = 56.1 ether
  // adjudication cost (direct challenge): 100 * 0.2 + 10 +  0 + 0.1 + 1 = 31.1 ether  
  // dividend/burn (is free speech):   45 ether - 41.1 ether =  3.9 ether
  // dividend/burn (not free speech):  60 ether - 56.1 ether =  3.9 ether
  // dividend/burn (direct challenge): 45 ether - 31.1 ether = 13.9 ether
  // any comment fees are added to the dividend/burn amount
  ...
  uint256 public revealAward         =  1 ether;
  uint256 public commentFee          = 100 * (10**6 * 1 gwei);
  ...
  // eventually it should be possible to revoke the GOVERNOR_ADMIN_ROLE, and finally the DEFAULT_ADMIN_ROLE.
  ...
  // bonder set to zero if the content is not bonded
  function isContentBonded(uint256 contentHash)
    external
    view
    returns(bool isBonded, address bonder)
  {
    Content storage _content = contents[contentHash];
    isBonded = _content.bonder != address(0);    
    bonder = _content.bonder;
  }

  // challenger set to zero if the content is not challenged
  function isContentChallenged(uint256 contentHash)
    external
    view
    returns(bool isChallenged, address challenger)
  {
    Content storage _content = contents[contentHash];
    isChallenged = _content.challenger != address(0);
    challenger = _content.challenger;
  }
  ...
  function postContentBond(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment)
    public
  {
    Content storage _content = contents[contentHash];
    require(_content.bonder == address(0), "content already bonded");
    require(amount == contentBondAmount + swarmMaintenanceFee, "incorrect amount of FSC");
    require(fscToken.transferFrom(msg.sender, address(this), amount), "failed to xfer bond");
    // deduct maintenance fee for posting to SWARM
    accumulatedFees += swarmMaintenanceFee;
    _content.holdoverAmount = contentBondAmount;
    _content.bonder = msg.sender;
    emit ContentBondEv(contentHash, block.timestamp, msg.sender, title, comment);
  }


  // same as postContentBond, but approval (to xfer FSC) provided via draft-IERC20Permit
  function postContentBondAuthorized(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment,
				     uint256 deadline, uint8 v, bytes32 r, bytes32 s)
    external
  {
    fscToken.permit(msg.sender, address(this), amount, deadline, v, r, s);
    postContentBond(amount, contentHash, title, comment);
  }
  ...
  // each content.
  // note: swarm maintenance fee is due the first time content is specified; paid in addition to contentBondAmount
  function postComment(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment)
    public
  {
    Content storage _content = contents[contentHash];
    require(_content.bonder != address(0), "content not bonded");
    require(amount == commentFee, "incorrect amount of FSC");
    require(fscToken.transferFrom(msg.sender, address(this), amount), "failed to xfer bond");
    _content.holdoverAmount = commentFee;
    emit CommentEv(contentHash, block.timestamp, msg.sender, title, comment);
  }


  // same as postComment, but approval (to xfer FSC) provided via draft-IERC20Permit
  function postCommentAuthorized(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment,
				 uint256 deadline, uint8 v, bytes32 r, bytes32 s)
    external
  {
    fscToken.permit(msg.sender, address(this), amount, deadline, v, r, s);
    postComment(amount, contentHash, title, comment);
  }
  
  
  // a bonder can reclaim his bond to the specified content, thereby un-bonding the content
  function withdrawContentBond(uint256 contentHash)
    external
  {
    Content storage _content = contents[contentHash];
    require(_content.bonder == msg.sender, "content not bonded by you");
    require(_content.challenger == address(0), "content is already challenged");
    _content.bonder = address(0);
    _content.holdoverAmount = 0;
    require(fscToken.transfer(msg.sender, contentBondAmount), "failed to xfer bond");
  }


  // post a challenge-bond against the specified content bond
  // a prospective challenger can initiate a challenge w/o a bonder. in that case the challenger will never get a reward.
  // note: if posting a challenge to unbonded content, then amount is increased to pay swarm fees
  function postChallengeBond(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment)
    public
  {
    Content storage _content = contents[contentHash];   
    if ((_content.verdict == VERDICT_IS_FREE_SPEECH || _content.verdict == VERDICT_NOT_FREE_SPEECH) && allowReadjudication) {
      _content.bonder = _content.challenger = address(0);
    ...
    uint256 _xferAmount = challengeBondAmount + randMaintenanceFee;
    if (_content.bonder == address(0)) {
      // unbonded content - direct call for adjudication so pay swarm fees
      accumulatedFees += swarmMaintenanceFee;
      _xferAmount += swarmMaintenanceFee;
    }
    require(amount == _xferAmount, "incorrect amount of FSC");
    require(fscToken.transferFrom(msg.sender, address(this), _xferAmount), "failed to xfer challenge fee");
    accumulatedFees += randMaintenanceFee;
    _content.holdoverAmount += challengeBondAmount;
    _content.challenger = msg.sender;
    _content.challengeBlock = block.number;
    _content.verdict = VERDICT_CHALLENGED;
    emit ChallengeBondEv(contentHash, block.timestamp, msg.sender, title, comment);
  ...
  function postChallengeBondAuthorized(uint256 contentHash, uint256 amount, bytes memory title, bytes memory comment,
				       uint256 deadline, uint8 v, bytes32 r, bytes32 s)
    external
  {
    fscToken.permit(msg.sender, address(this), amount, deadline, v, r, s);
    postChallengeBond(contentHash, amount, title, comment);
  }
    ...
    internal
  {
    ...
    content.verdict = VERDICT_VOTE_PENDING;
  }


  function encryptedVote(uint256 contentHash, bytes32 voteHash,
			 bytes32 encryptedVote0, bytes32 encryptedVote1, bytes32 encryptedVote2, bytes32 encryptedVote3,
			 bytes32 encryptedVote4, bytes32 encryptedVote5, bytes32 encryptedVote6, bytes32 encryptedVote7)
    external
  {
    ...
    require(_content.verdict == VERDICT_VOTE_PENDING, "adjudication not in progress");
    require(_juror.vote == VERDICT_UNADJUDICATED || _juror.voteBlock < _content.challengeBlock, "already voted");
    require(fscToken.isSelectedForJury(contentHash, msg.sender), "not eligible to vote");
    require(fscToken.transferFrom(msg.sender, address(this), voteWagerAmount), "failed to xfer vote wager");
    _content.holdoverAmount += voteWagerAmount;
    emit EncryptedVoteEvent(contentHash, msg.sender,
			    encryptedVote0, encryptedVote1, encryptedVote2, encryptedVote3,
			    encryptedVote4, encryptedVote5, encryptedVote6, encryptedVote7);
    _juror.voteHash = voteHash;
    _juror.voteBlock = block.number;
    _juror.vote = VERDICT_REVEAL_PENDING;
    ++_content.voteCount;
    if (_content.voteCount >= juryQuorum) {
      _content.verdict = VERDICT_REVEAL_PENDING;
      emit AllVotesCastEvent(contentHash, _content.voteCount);
    }
  }


  function revealVote(uint256 contentHash, address revealerAddr, address jurorAddr, uint256 certifiedVote)
    external
    onlyRole(REVEALER_ROLE)
  {
    Content storage _content = contents[contentHash];
    Juror storage _juror = _content.jurors[jurorAddr];
    bytes32 voteHash = keccak256(abi.encodePacked(certifiedVote));
    require(voteHash == _juror.voteHash, "vote hash mismatch");
    require(_content.verdict == VERDICT_REVEAL_PENDING, "reveal not in progress");
    require(_juror.vote == VERDICT_REVEAL_PENDING, "already voted");
    uint8 jurorVote = ((certifiedVote & VOTE_FREE_SPEECH_MASK) == VOTE_FREE_SPEECH_MASK) ? VERDICT_IS_FREE_SPEECH : VERDICT_NOT_FREE_SPEECH;
    _juror.vote = jurorVote;
    if (jurorVote == VERDICT_IS_FREE_SPEECH)
    ...
    if (_content.isFreeVoteCount + _content.notFreeVoteCount >= juryQuorum) {
      _setFreeSpeechProtected(contentHash, revealerAddr);
    }
  }


  function _setFreeSpeechProtected(uint256 contentHash, address revealerAddr)
    internal
  {
    uint256 _voterRewardReserve = 0;
    Content storage _content = contents[contentHash];
    _content.verdict = ((uint256)(_content.notFreeVoteCount) * 100 / juryQuorum > freeSpeechBiasPct) ? VERDICT_NOT_FREE_SPEECH : VERDICT_IS_FREE_SPEECH;
    if (_content.verdict == VERDICT_IS_FREE_SPEECH) {
      if (_content.bonder != address(0)) {
	uint256 _bonderAmount = contentBondAmount + vindicationAward;	
	_content.holdoverAmount -= _bonderAmount;
	require(fscToken.transfer(_content.bonder, _bonderAmount), "failed to xfer vindication award");
	_voterRewardReserve += _content.isFreeVoteCount * (voteWagerAmount + voteAward);
      }
    } else {
      // only award challenger if there was a bonder
      if (_content.bonder != address(0)) {      
    ...
    // award a lottery prize
    bytes32 _requestId = getRandomNumber();
    vrfReqIdToContent[_requestId] = contentHash;
    _voterRewardReserve += voteLotteryAward;
    _content.holdoverAmount -= revealAward;
    require(fscToken.transfer(revealerAddr, revealAward), "failed to xfer reveal award");
    uint256 _dividendAmount = _content.holdoverAmount - _voterRewardReserve;
    accumulatedDividends += _dividendAmount;
    emit VerdictEvent(contentHash, _content.verdict, _content.isFreeVoteCount, _content.notFreeVoteCount);
    emit AwardsEvent(contentHash, vindicationAward, challengeAward, _voterRewardReserve, revealAward, _dividendAmount);
  }
    ...
    internal
  {
    ...
    uint256 lotterySize = (content.verdict == VERDICT_IS_FREE_SPEECH) ? content.isFreeVoteCount : content.notFreeVoteCount;
  ...
  {
    Content storage _content = contents[contentHash];
    require(_content.verdict == VERDICT_IS_FREE_SPEECH || _content.verdict == VERDICT_NOT_FREE_SPEECH, "no verdict");
    ...
    external
  {
    ...
    dividendToken.payFSCDividend(_dividendAmount);
  }
  ...
  function setCommentFee(uint256 _commentFee) public onlyRole(GOVERNOR_ROLE) {
    emit CommentFeeUpdated(commentFee, _commentFee);
...
interface IDividendToken {
  function payFSCDividend(uint256 amount) external;
  function withdrawFSCDividend() external returns (uint256 amount);
  function payDAIDividend(uint256 amount) external;
  function withdrawDAIDividend() external returns (uint256 amount);
}
