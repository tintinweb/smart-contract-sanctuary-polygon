...
abstract contract ERC721Stakeable is Ownable{
    ...
    */
    ...
    function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256) {
        ...
        uint256 rewardPerMinute = fullReward.div(((_periodFinish - _periodStarted) / 1 minutes));
    ...
    */
    ...
    /**
    * @dev
    * Shows rewardPerMinute on 1 token weight, per minute
    *  
    */
    function _rewardPerMinute() internal view returns (uint256 rewardPerMinute) {
        uint256 fullReward = _totalRewardsPool.div(_totalWeight).mul(1);
...
contract GBotStaking is ERC721Receiver, ERC721Stakeable {
    ...
    //================================== PUBLIC FUNCTIONS ==================================//
    function ownerTotalPrize(address owner) virtual public view returns (uint256) {
        return _getTotalPrizes(owner);
    }

    function ownerStakedNfts(address owner) virtual public view returns (uint256[] memory) {
        return _ownerTokenIds(owner);
    }

    function ownerTotalWeight(address owner) virtual public view returns (uint256) {
