...
contract BlueBird is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable {
    ...
    function mintFromMatic() external payable checkBalance(address(msg.sender).balance,msg.value){
        ...
        if(minter4minted["ecosystem"]*1e18+ value*mintPrice/1e18 > minter4amount["ecosystem"]*1e18){
            revert BlueBird__ErrorMint("ecosystem",value/1e18*mintPrice/1e18);
        }
        (bool callSuccess,) = owner.call{value: msg.value}('');
        if(!callSuccess){
            revert BlueBird__ErrorCall();
        }
        minter4minted["ecosystem"] = minter4minted["ecosystem"]+value*mintPrice/1e18;
        _mint(msg.sender, value*mintPrice);
        emit mintFromMaticEvent(msg.sender, value*mintPrice/1e18, mintPrice);
    }
