...
contract Popin is ERC1155, Ownable, IPopfeed {
    ...
    mapping(uint256 => mapping(uint256 => IPopfeed.Feed)) public items;
    ...
    function getFeeds(
        uint256 tokenId
    ) public view returns (IPopfeed.Feed[_ITEM_NUM] memory) {
        IPopfeed.Feed[_ITEM_NUM] memory feeds;
        for (uint8 i = 0; i < _ITEM_NUM; i++) {
            feeds[i] = items[i][tokenId];
        }
        return feeds;
    }
    ...
    function setFeeds(
        ...
        for (uint8 i = 0; i < _ITEM_NUM; i++) {
            items[i][_tokenId] = IPopfeed.Feed(
                _itemTokenIds[i],
