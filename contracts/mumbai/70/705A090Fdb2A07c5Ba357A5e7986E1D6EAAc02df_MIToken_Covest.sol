...
contract MIToken_Covest is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable , UUPSUpgradeable, ReentrancyGuardUpgradeable {
     ...
     */
    function mint(address _user,address _asset, uint256 _amount) public nonReentrant isSuperManager assetsChecker(_asset) {
        IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_asset);
        require(_user != address(0), "user address is not valid.");
        require(_ERC20.allowance(_user,address(this)) >= _amount,"MIToken: You are not allowance");
        require(_ERC20.balanceOf(_user) >= _amount,"MIToken: Insufficient Balance");
        uint _rate = exchangeRate;

        _ERC20.transferFrom(_user,address(this),_amount);
        //* (600*10^5)*10**(18-5) * rate / 100
        uint _amountToMint = (( _amount * 10**(decimals() - _ERC20.decimals())) * _rate ) / 100; 
     ...
     *   - `_asset` Check the asset is approve from adminRouter for this pool.
     */
    function unWrap(address _user,address _asset,uint256 _amount) public nonReentrant isSuperManager assetsChecker(_asset) {
        uint _rate = exchangeRate;
        IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_asset);
        uint256 _saveAmount = (_amount*_rate)/100;
        uint _willburn = _saveAmount * 10**(decimals() - _ERC20.decimals());
        require(_user != address(0), "user address is not valid.");
        require(balanceOf(_user) >= _willburn,"MIToken: Balance Insufficient");
        _burn(_user, _willburn);
        require(_ERC20.balanceOf(address(this)) >= _saveAmount,"MIToken: Contract Balance Insufficient");
