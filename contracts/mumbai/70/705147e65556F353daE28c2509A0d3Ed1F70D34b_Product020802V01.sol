...
contract Product020802V01 is OwnableV01 {
    ...
    bytes private constant PRODUCT_ID = "0208_02";
    int private constant TIME_ZONE_OFFSET = 1;
    ...
    function createPolicy(
        ...
        require(!policyExists[policyParams.onChainPolicyId], "policy already in storage");
        ...
        for (uint256 index = 0; index < policyParams.selectedCoverages.length; index = index.unsafe_inc()) {
        ...
        policyExists[policyParams.onChainPolicyId] = true;
        totalPolicies += 1;
    ...
    function endorsePolicy(
        Product020802Lib.EndorsementParams calldata endorsementParams
    ) external onlyPartnerOrWakam(allPolicies[endorsementParams.onChainPolicyId].partnershipCodeHash) {
        doesPolicyExist(endorsementParams.onChainPolicyId);
        Product020802Lib.endorsePolicy(
            endorsementParams,
            allPolicies[endorsementParams.onChainPolicyId],
            TIME_ZONE_OFFSET
        );
    }
    ...
    function cancelPolicy(
        ...
        doesPolicyExist(onChainPolicyId);
    ...
    // should we change the order of the returned field?
    ...
    function doesPolicyExist (string calldata onChainPolicyId) private view {
        require(policyExists[onChainPolicyId], "Unknown policy");
    }

    function getPolicyStatus(uint operationType, uint operationEffectiveDate) private view returns (uint) {
        if (operationType == 1 || operationType == 3 || operationType == 4 || operationType == 5) {
...
library Product020802Lib {
    ...
    function checkBusinessRulesToCreatePolicy(
        PolicyParams calldata policyToBeChecked,
        uint256 subscriptionDate,
        uint256 coverageStartDate
    ) external pure {
    ...
    function checkEndorsePolicyBusinessRules(EndorsementParams memory endorsementParams, Policy storage policy) private view {
        checkIfOngoingWithdrawal(policy.operationType);
        ...
        if (endorsementParams.requestedCoverageStartDate != 0) {
            // que si le contract n'a pas commencé
            checkIfTheCoverageIsNotAlreadyActive(policy.coverageStartDate);
            checkIfCoverageStartDateIsNotThePast(endorsementParams.requestedCoverageStartDate);
        }
        checkOperationEffectiveDate(endorsementParams.operationEffectiveDate);
    }
    ...
    function checkIfTheCoverageIsNotAlreadyActive(uint256 coverageStartDate) private view {
        require(coverageStartDate > block.timestamp, "coverage has already started");
    }

    function checkOperationEffectiveDate(uint256 operationEffectiveDate) private view {
        require(
            operationEffectiveDate.dailyCompareTo(block.timestamp) >= 0,
            "operationEffectiveDate cannot be in the past"
        );
        require(
            operationEffectiveDate.dailyCompareTo(block.timestamp + 8 weeks) > 0,
            "operationEffectiveDate must be less than now + 2 months"
        );
    }
    ...
    function checkIfCoverageStartDateIsNotThePast(uint256 coverageStartDate) private view {
        bool coverageStartDateIsInThePast = coverageStartDate.dailyCompareTo(block.timestamp) >= 0;
    ...
    function endorsePolicy(
        ...
        checkEndorsePolicyBusinessRules(endorsementParams, policy);
        ...
        if (endorsementParams.requestedCoverageStartDate != 0) {
            ...
            policy.coverageStartDate = coverageStartDate;
            policy.expirationDate = coverageStartDate.addYears(1, timezoneOffset);
        }
        if (bytes(endorsementParams.package).length != 0) {
            policy.package = endorsementParams.package;
        }
        if (endorsementParams.exposure.vehicleType != 0) {
            policy.exposure.vehicleType = endorsementParams.exposure.vehicleType;
        }
        if (bytes(endorsementParams.exposure.brand).length != 0) {
            policy.exposure.brand = endorsementParams.exposure.brand;
        }
        if (bytes(endorsementParams.exposure.model).length != 0) {
            policy.exposure.model = endorsementParams.exposure.model;
        }
        if (endorsementParams.exposure.purchaseDate != 0) {
            policy.exposure.purchaseDate = endorsementParams.exposure.purchaseDate;
        }
        if (endorsementParams.exposure.purchasePrice != 0) {
            policy.exposure.purchasePrice = endorsementParams.exposure.purchasePrice;
        }
        if (endorsementParams.exposure.usage != 0) {
            policy.exposure.usage = endorsementParams.exposure.usage;
        }
        if (endorsementParams.exposure.householdSize != 0) {
            policy.exposure.householdSize = endorsementParams.exposure.householdSize;
        }
        if (endorsementParams.exposure.maxSpeed != 0) {
            policy.exposure.maxSpeed = endorsementParams.exposure.maxSpeed;
        }
        if (endorsementParams.split != 0) {
            policy.split = endorsementParams.split;
        }
        if (bytes(endorsementParams.commonSensitiveData).length != 0) {
            policy.commonSensitiveData = endorsementParams.commonSensitiveData;
        }
        if (bytes(endorsementParams.claimsAdminSensitiveData).length != 0) {
            policy.claimsAdminSensitiveData = endorsementParams.claimsAdminSensitiveData;
        }
        if (bytes(endorsementParams.policiesAdminSensitiveData).length != 0) {
            policy.policiesAdminSensitiveData = endorsementParams.policiesAdminSensitiveData;
        }
        if (bytes(endorsementParams.distributorSensitiveData).length != 0) {
            policy.distributorSensitiveData = endorsementParams.distributorSensitiveData;
        }
        if (bytes(endorsementParams.insurerSensitiveData).length != 0) {
            policy.insurerSensitiveData = endorsementParams.insurerSensitiveData;
        }

        if (endorsementParams.updateSelectedOptions) {
            policy.selectedOptions = endorsementParams.selectedOptions;
        }
        if (endorsementParams.updateCoverages) {
            delete policy.selectedCoverages;
            for (uint256 index = 0; index < endorsementParams.selectedCoverages.length; index = unsafe_inc(index)) {
                policy.selectedCoverages.push(endorsementParams.selectedCoverages[index]);
            }
        }

        policy.operationType = endorsementParams.operationType;
        policy.operationEffectiveDate = block.timestamp;
    }

    function unsafe_inc(uint256 x) public pure returns (uint256) {
        unchecked {
            return x + 1;
        }
    }
...
// ["0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553","1111",1688563210,"ABCD",[1,"Xiaomi2","PR266O2",2235465,40000,1,5,22],["FV_2"],[["ABC","CCC","BBB"]],2,"chiffre144444444ZHGDF","cdcQKLJLKDQJVLKDJhiffre2","chiLKSDVQJLKDJKLSQJVLKQJLKDVJLffre3","cDLKJVSLKJVDLKJDVLKJDlkvlkjvdslhiffre4","vldksjlvdskjvlkdsjlvkjdlkschiffre5",2,1688563210]

// ["1114", "0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553", 1688563210 ,1,1,1,"Formula 1",[1,"Xiaomi","PRO2",2235465,50000,"€ cents",1,5,18],["FV_1"],[["ABC","jhkcs","sjdhk"]],3,true,1,"VApX7aKNMzkk0N/sjtETGEy46vPKmwVHwVnqrwqDxHu3UDln5pFCypEhDWbVNQNu","rRCUP5gXd0S1tv4mzKaocw==","GDt/LJNaHBQpj/BgWV5h6Q==","vd98N5HRgx/5xpVEKbzSFA==", "f5/kEnFN/nBJ5J2AEgUrag=="]
// ["0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553","1114",1688563210,"ABCD",[1,"Xiaomi2","PR266O2",2235465,40000,1,5,22],["FV_2"],[["ABC","CCC","BBB"]],2,"VApX7aKNMzkk0N/sjtETGEy46vPKmwVHwVnqrwsdvml3UDln5pFCypEhDWbVNQNu","rRCUP5gXd0S1tv4mzKacWW==","GDt/LJNaHBQpj/BgWV5h6A==","vd98N5HRgx/5xpVEKdslSFA==", "f5/kEnFN/nBJvsd2AEgUrag==",3,1685971210,true,true]

//["1111","0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553", 1688563210,1,1,1,"Formula 1",[1,"Xiaomi","PRO2",2235465,50000,"€ cents",1,5,18],["FV_1"],[["ABC","jhkcs","sjdhk"]],3,true,1,"chiffre1","chiffre2","chiffre3","chiffre4", "chiffre5"]
//[true,"0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553", 1688563210,1,1,1,1,3,"1111","Formula 1","chiffre1","chiffre2","chiffre3","chiffre4", "chiffre5",["FV_1"],[1,2235465,50000,1,18,5,"€ cents","Xiaomi","PRO2"],[["ABC","jhkcs","sjdhk"]]]

// [true,true,"0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553",1688563210,2,2,1688563210,"ABCD","1111","fqsfqsdfsqdfqs","fqsdfsqdfqsdf","fsqdfqsdfsqdfsqd","fsdfsqdfsqdfqsdfsqd","fsdqfsqdfsqdfsqdf",["FV_2"],[1,2235465,40000,1,5,22,"Xiaomi2","PR266O2"],[["ABC","CCC","BBB"]]]
// ["0x64499a2015da168f281a5976dfcaa97db11e5f482e4076d1cf6b76eebfcee553","1111",1688563210,"ABCD",[1,"Xiaomi2","PR266O2",2235465,40000,1,5,22],["FV_2"],[["ABC","CCC","BBB"]],2,"fqsfqsdfsqdfqs","fqsdfsqdfqsdf","fsqdfqsdfsqdfsqd","fsdfsqdfsqdfqsdfsqd","fsdqfsqdfsqdfsqdf",3,1685971210,true,true]
