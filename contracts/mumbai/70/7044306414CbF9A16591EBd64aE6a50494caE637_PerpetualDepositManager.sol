    ...
    IPerpetualDepositManager
{
     ...
     *                    (not ABDK)
     */
    function brokerDepositToDefaultFund(uint8 _poolId, uint16 _iLots) external override {
        require(_poolId <= iPoolCount, "pool index out of range");
        require(_iLots > 0, "invalid lots");
        PerpStorage.LiquidityPoolData storage pool = liquidityPools[_poolId];
        int128 fAmount = pool.fBrokerCollateralLotSize.mul(int128(int16(_iLots)) * ONE_64x64);
        ...
        uint16 brokerLots = brokerMap[_poolId][msg.sender];
        if (brokerLots >= 65534) {
            // avoid overflow
            _iLots = 0;
        }
