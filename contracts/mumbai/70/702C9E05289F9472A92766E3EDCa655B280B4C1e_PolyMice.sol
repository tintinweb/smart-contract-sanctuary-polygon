...
contract PolyMice is ERC721Enumerable {
    /*
...
*/
    ...
    constructor() ERC721("PolyMice", "PMICE") {
        _owner = msg.sender;
     ...
     */
    function currentJoeyCost() public view returns (uint256) {
        uint256 _totalSupply = totalSupply();

        if (_totalSupply <= 200) return 0;
        if (_totalSupply > 200 && _totalSupply <= 400)
            return 100000000000000000;
        if (_totalSupply > 400 && _totalSupply <= 600)
            return 200000000000000000;
        if (_totalSupply > 600 && _totalSupply <= 800)
            return 300000000000000000;
        if (_totalSupply > 800 && _totalSupply <= 1000)
    ...
    function mintInternal() internal {
        ...
        require(!MiceLibrary.isContract(msg.sender));
    ...
    function mintMouse() public {
        ...
        //Burn this much joey
        IJoey(joeyAddress).burnFrom(msg.sender, currentJoeyCost());
    ...
    function hashToSVG(
        ...
        for (uint8 i = 0; i < 9; i++) {
            uint8 thisTraitIndex = MiceLibrary.parseInt(
                MiceLibrary.substring(_hash, i, i + 1)
            );
            ...
            for (
                ...
                string memory thisPixel = MiceLibrary.substring(
                ...
                uint8 x = letterToNumber(
                    MiceLibrary.substring(thisPixel, 0, 1)
                );
                uint8 y = letterToNumber(
                    MiceLibrary.substring(thisPixel, 1, 2)
                );
                ...
                svgString = string(
                    abi.encodePacked(
                        svgString,
                        "<rect class='c",
                        MiceLibrary.substring(thisPixel, 2, 4),
                        "' x='",
                        x.toString(),
                        "' y='",
                        y.toString(),
                    ...
                    MiceLibrary.encode(
                        bytes(
                            string(
                                abi.encodePacked(
                                    '{"name": "PolyMice #',
                                    MiceLibrary.toString(_tokenId),
                                    '", "description": "PolyMice is a collection of 1000 unique mice. All the metadata and images are generated and stored 100% on-chain. No IPFS, no API. Just the Polygon blockchain.", "image": "data:image/svg+xml;base64,',
                                    MiceLibrary.encode(
    ...
    function _tokenIdToHash(
        ...
        if (ownerOf(_tokenId) == 0x000000000000000000000000000000000000dEaD) {
            tokenHash = string(
                abi.encodePacked("1", MiceLibrary.substring(tokenHash, 1, 9))
            );
        }
    ...
    /**
     * @dev Sets the joey ERC20 address
     * @param _joeyAddress The joey address
     */

    function setJoeyAddress(address _joeyAddress) public onlyOwner {
        joeyAddress = _joeyAddress;
    }
