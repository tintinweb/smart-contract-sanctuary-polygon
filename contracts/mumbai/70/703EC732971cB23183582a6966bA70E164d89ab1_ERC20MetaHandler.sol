...
contract ERC20MetaHandler is BaseCombinedGsnHandler {
    IV3SwapRouter public swapRouter;

    IWrappedChainToken public wrappedChainToken;
    uint256 public preApprovedGasDiscount = 0;

    mapping(IERC20Meta => bool) registeredTokens;
    mapping(IERC20Meta => uint24) registeredTokenPoolFee;
    uint256 registeredTokenCount = 0;

    mapping(address => uint256) nonces;

    modifier onlyRegisteredToken(IERC20Meta token) {
        require(registeredTokens[token], "Meta: token not registered");
        _;
    }

    modifier onlyWithSwapRouter() {
        require(address(swapRouter) != address(0), "Meta: no swap router");
        _;
    }

    modifier onlyWithWrappedChainToken() {
        require(address(wrappedChainToken) != address(0), "Meta: no swap router");
        _;
    }

    function setSwapRouter(IV3SwapRouter _swapRouter) public onlyOwner {
        require(registeredTokenCount == 0, "Meta: tokens registered");
    ...
    function registerToken(IERC20Meta token, uint24 poolFee) public onlyOwner onlyWithSwapRouter {
        require(address(swapRouter) != address(0), "Meta: No swap router defined");
        require(!registeredTokens[token], "Meta: token already registered");
        require(token.approve(address(swapRouter), type(uint256).max), "Meta: approval for swap router failed");
        require(token.approve(owner(), type(uint256).max), "Meta: approval for owner failed");
    ...
    function isRegisteredToken(IERC20Meta token) public view returns (bool) {
        return registeredTokens[token];
    }

    function executeFeePrivate(ExecuteRequestData memory requestData) private {
        require(requestData.token.transferFrom(requestData.userAddress, address(this), requestData.fee), "Meta: Fee transfer failed");
        if (requestData.chainTokenFee > 0) {
            IV3SwapRouter.ExactOutputSingleParams memory params = IV3SwapRouter.ExactOutputSingleParams({
                tokenIn : address(requestData.token),
                tokenOut : address(wrappedChainToken),
                fee : registeredTokenPoolFee[requestData.token],
                recipient : address(this),
                amountOut : requestData.chainTokenFee,
                amountInMaximum : requestData.fee,
                sqrtPriceLimitX96 : 0
            });
    ...
    function finishFeePrivate(ExecuteRequestData memory requestData) private {
        if (requestData.chainTokenFee > 0) {
            wrappedChainToken.withdraw(requestData.chainTokenFee);
            relayHub.depositFor{value : requestData.chainTokenFee}(address(this));
        }
    }

    function executePrivate(ExecuteRequestData memory requestData) private {
        require(requestData.token.transferFrom(requestData.userAddress, requestData.target, requestData.amount), "Meta: Value transfer failed");
    }

    function checkPrivate(ExecuteRequestData memory requestData) private view onlyWithSwapRouter onlyWithWrappedChainToken onlyRegisteredToken(requestData.token) {
        require(requestData.token.balanceOf(requestData.userAddress) >= requestData.amount + requestData.fee, "Meta: balance too low");
    }

    function checkWithApproval(ExecuteRequestData memory requestData, ApprovalRequestData memory approvalRequestData) internal view {
        checkPrivate(requestData);
        if (requestData.token.allowance(requestData.userAddress, address(this)) < requestData.amount + requestData.fee) {
            require(approvalRequestData.approval >= requestData.amount + requestData.fee, "Meta: approval too low");
        }
    }

    function executeApprovalPrivate(ExecuteRequestData memory requestData, ApprovalRequestData memory approvalRequestData) private {
        if (requestData.token.allowance(requestData.userAddress, address(this)) < requestData.amount + requestData.fee) {
            bytes memory functionSignature = abi.encodeCall(IERC20.approve, (address(this), approvalRequestData.approval));
            requestData.token.executeMetaTransaction(requestData.userAddress, functionSignature, approvalRequestData.sigR, approvalRequestData.sigS, approvalRequestData.sigV);
        }
    }

    bytes4 private EXECUTE_WITH_APPROVAL_METHOD_ID = bytes4(keccak256("executeWithApproval(address,address,uint256,address,uint256,uint256,uint256,bytes32,bytes32,uint8)"));

    function executeWithApproval(IERC20Meta token, address userAddress, uint256 amount, address target, uint256 fee, uint256 chainTokenFee, uint256 approval, bytes32 sigR, bytes32 sigS, uint8 sigV) public {
        ExecuteRequestData memory requestData = ExecuteRequestData(token, userAddress, amount, target, fee, chainTokenFee);
        ApprovalRequestData memory approvalRequestData = ApprovalRequestData(approval, sigR, sigS, sigV);
        checkWithApproval(requestData, approvalRequestData);
        executeApprovalPrivate(requestData, approvalRequestData);
        executeFeePrivate(requestData);
        executePrivate(requestData);
        finishFeePrivate(requestData);
    }

    function check(ExecuteRequestData memory requestData) view internal {
        checkPrivate(requestData);
        require(requestData.token.allowance(requestData.userAddress, address(this)) >= requestData.amount + requestData.fee, "Meta: allowance too low");
    }

    bytes4 private EXECUTE_METHOD_ID = bytes4(keccak256("execute(address,address,uint256,address,uint256,uint256)"));

    function execute(IERC20Meta token, address userAddress, uint256 amount, address target, uint256 fee, uint256 chainTokenFee) public {
        ExecuteRequestData memory requestData = ExecuteRequestData(token, userAddress, amount, target, fee, chainTokenFee);
        check(requestData);
        executeFeePrivate(requestData);
        executePrivate(requestData);
    ...
    function decodeExecuteRequestData(bytes memory data) private view returns (bytes4 methodId, ExecuteRequestData memory requestData) {
        methodId = GsnUtils.getMethodSig(data);
        if (methodId == EXECUTE_METHOD_ID || methodId == EXECUTE_WITH_APPROVAL_METHOD_ID) {
            requestData = ExecuteRequestData({
                token : IERC20Meta(address(uint160(GsnUtils.getParam(data, 0)))),
                userAddress : address(uint160(GsnUtils.getParam(data, 1))),
                amount : uint256(GsnUtils.getParam(data, 2)),
                target : address(uint160(GsnUtils.getParam(data, 3))),
                fee : uint256(GsnUtils.getParam(data, 4)),
                chainTokenFee : uint256(GsnUtils.getParam(data, 5))
            });
        } else {
    ...
    function verifyCall(IForwarder.ForwardRequest memory request, GsnTypes.RelayData calldata relayData, bytes memory signature, bytes memory approvalData) private view returns (bytes4 methodId, ExecuteRequestData memory requestData, ApprovalRequestData memory approvalRequestData) {
        (approvalData);
        ...
        (methodId, requestData) = decodeExecuteRequestData(request.data);
        require(requestData.userAddress == request.from, "Meta: from mismatch");

        if (methodId == EXECUTE_METHOD_ID) {
            require(requestData.chainTokenFee >= relayHub.calculateCharge(requiredRelayGas() - preApprovedGasDiscount, relayData), "Meta: fee too low");
            check(requestData);
        } else if (methodId == EXECUTE_WITH_APPROVAL_METHOD_ID) {
            require(requestData.chainTokenFee >= relayHub.calculateCharge(requiredRelayGas(), relayData), "Meta: fee too low");
            approvalRequestData = ApprovalRequestData({
                approval : uint256(GsnUtils.getParam(request.data, 6)),
                sigR : bytes32(GsnUtils.getParam(request.data, 7)),
                sigS : bytes32(GsnUtils.getParam(request.data, 8)),
                sigV : uint8(GsnUtils.getParam(request.data, 9))
            });
            checkWithApproval(requestData, approvalRequestData);
        }
    }
    ...
    function preRelayedCall(GsnTypes.RelayRequest calldata relayRequest, bytes calldata signature, bytes calldata approvalData, uint256 maxPossibleGas) external override virtual onlyRelayHub onlyToSelf(relayRequest) returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        (bytes4 methodId, ExecuteRequestData memory requestData, ApprovalRequestData memory approvalRequestData) = verifyCall(relayRequest.request, relayRequest.relayData, signature, approvalData);
        (methodId);
    ...
    function execute(ForwardRequest calldata request, bytes32 domainSeparator, bytes32 requestTypeHash, bytes calldata suffixData, bytes calldata signature) public override payable returns (bool success, bytes memory ret) {
        ...
        (bytes4 methodId, ExecuteRequestData memory requestData) = decodeExecuteRequestData(request.data);
        (methodId);
        require(requestData.userAddress == request.from, "Meta: from mismatch");
...
abstract contract BaseCombinedGsnHandler is IRelayRecipient, IPaymaster, IForwarder, Ownable {
    ...
    uint256 private _preRelayedCallGasLimit = 35000;
