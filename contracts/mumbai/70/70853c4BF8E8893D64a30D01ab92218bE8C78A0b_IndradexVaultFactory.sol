...
// File contracts/IndradexBaseVault.sol
    ...
    returns (TokenTotalBalance[] memory)
  {
    ...
    if (currentWatermark > _highWatermark) {
      ...
      for (uint256 j = 0; j < tokensLength; j++) {
        tokenTotalBalances[j].balance =
          tokenTotalBalances[j].balance -
          (tokenTotalBalances[j].balance -
            (tokenTotalBalances[j].balance).mul(_highWatermark).div(
              currentWatermark
...
// File contracts/IndradexSingleExternalVault.sol
...
contract IndradexSingleExternalVault is IndradexBaseVault {
  ...
  function adminDeposit(address token, uint256 amount)
    ...
    onlyPresentToken(token)
  {
    uint256 inputTokenExternalBalance_ = tokenExternalBalance(
      address(inputToken)
    );
    uint256 tokenAmountScaledValue_ = tokenAmountScaledValue(token, amount);
    require(
      inputTokenExternalBalance_ >= tokenAmountScaledValue_,
      'Vault: Deposit amount value cannot be greater than the external balance of the input token'
    );
    ERC20(token).safeTransferFrom(msg.sender, address(this), amount);
    setInputTokenExternalBalance(
      inputTokenExternalBalance_ - tokenAmountScaledValue_
    );
  }

  /**
   * @dev Set the external balance for the input token.
   */
  function setInputTokenExternalBalance(uint256 newExternalBalance)
    public
    onlyOwner
  {
    ...
    tokenInfo[inputTokenAddress].externalBalance = newExternalBalance;
  }
  ...
  function _adminWithdraw(
    ...
    setInputTokenExternalBalance(
      tokenAmountScaledValue(token, amount) +
        tokenExternalBalance(address(inputToken))
    );
  }
  ...
  /**
   * @dev Calculate the price of an amount of a token in input token.
   */
  function tokenAmountScaledValue(address token, uint256 amount)
    internal
    view
    returns (uint256)
  {
    return
      (amount * 10**(18 - tokenInfo[token].tokenDecimals)).mul(
        scaledChainlinkPrice(
          tokenInfo[token].aggregator,
          tokenInfo[token].aggregatorDecimals
        )
      ) / (10**(18 - tokenInfo[address(inputToken)].tokenDecimals));
  }
}

// File contracts/IndradexLibrarySingleVaultFactory.sol
...
library IndradexLibrarySingleVaultFactory {
  ...
  function createVault(
    ...
    uint256 maxCapDeposit,
    uint256 performanceFeePercentage
  ) external returns (IndradexSingleExternalVault) {
    return
      new IndradexSingleExternalVault(
        newOwner,
        name_,
        symbol_,
        inputToken_,
...
// File contracts/IndradexMultiExternalVault.sol
...
// File contracts/IndradexLibraryMultiVaultFactory.sol
...
// File contracts/IndradexVaultFactory.sol

pragma solidity 0.8.9;

contract IndradexVaultFactory is Ownable {
  enum VaultStatus {
    NOT_EXISTING,
    ACTIVE,
    DEPRECATED
  ...
  address[] private _indradexMultiExternalVaults;
  address[] private _indradexSingleExternalVaults;
  mapping(address => VaultStatus) private _vaultStatus;
  ...
  function createVault(
    ...
    uint256 maxCapDeposit,
    uint256 performanceFeePercentage,
    bool isSingleExternalVault
  ) external onlyOwner returns (address newVault) {
    if (isSingleExternalVault) {
      newVault = address(
        IndradexLibrarySingleVaultFactory.createVault(
          newOwner,
          name_,
          symbol_,
          inputToken_,
      ...
      _indradexSingleExternalVaults.push(newVault);
    } else {
      newVault = address(
        IndradexLibraryMultiVaultFactory.createVault(
          newOwner,
          name_,
          symbol_,
          inputToken_,
      ...
      _indradexMultiExternalVaults.push(newVault);
    }
    _vaultStatus[newVault] = VaultStatus.ACTIVE;
  }

  /**
   * @dev Set a vault as deprecated or not
   */
  function setDeprecated(address vaultAddress, bool deprecated)
    external
    onlyOwner
  {
    require(
      _vaultStatus[vaultAddress] != VaultStatus.NOT_EXISTING,
      'Factory: vault not existing'
    );
    if (deprecated) {
      _vaultStatus[vaultAddress] = VaultStatus.DEPRECATED;
    } else {
      _vaultStatus[vaultAddress] = VaultStatus.ACTIVE;
    }
  }

  /**
   * @dev Returns if a vault is deprecated or not
   */
  function vaultStatus(address vaultAddress)
    external
    view
    returns (VaultStatus)
  {
    return _vaultStatus[vaultAddress];
  }

  /**
   * @dev Returns the array of IndradexMultiExternalVault
   */
  function getIndradexMultiExternalVaults()
    external
    view
    returns (address[] memory)
  {
    return _indradexMultiExternalVaults;
  }

  /**
   * @dev Returns the array of IndradexSingleExternalVault
   */
  function getIndradexSingleExternalVaults()
    external
    view
    returns (address[] memory)
  {
    return _indradexSingleExternalVaults;
  }
}