...
contract ERC721Opal is OpalBase, IERC721OpalBase, ERC721AUpgradeable {
    function initialize(
        ...
        string memory layersTable_,
        string memory attributesTable_,
        string memory tokensTable_
    )
        external
        initializerERC721A
        initializer
    {
        ...
        _layersTable = layersTable_;
        _attributesTable = attributesTable_;
        _tokensTable = tokensTable_;

        _withdrawalAddress = msg.sender;

        _collectionId = collectionId_;

        setTablelandBaseURI("https://testnet.tableland.network/query?s=");
    }

    // TODO Make this payable
    function mint(uint256 _quantity) external {
        if (_quantity > 5) {
            revert InvalidMintQuantity(_quantity, _maxMintQuantity);
        }
        // if (msg.value != _mintPrice) {
        //     revert InvalidPaymentSent(msg.value, _mintPrice);
        // }
        ...
        returns (string memory)
    {
        ...
        string memory query = string.concat(
            ...
            _tokensTable,
            ".id%2C%27name%27%2Cname%2C%27image%27%2C",
            _tokensTable,
            ".image%2C%27description%27%2Cdescription%2C%27attributes%27%2Cjson_group_array%28json_object%28%27trait_type%27%2Ctrait_type%2C%27value%27%2Cvalue%29%29%29%20FROM%20",
            _tokensTable,
            "%20JOIN%20",
            _attributesTable,
            "%20ON%20",
    ...
    {
        return interfaceId
            == type(IERC721OpalBase).interfaceId
    ...
    PausableUpgradeable
{
    ...
    string internal _tokensTable;
    string internal _layersTable;
    string internal _attributesTable;

    // Base URI for the Tableland Network
    string internal _tablelandBaseURI;

    address internal _withdrawalAddress;

    uint256 internal _collectionId;

    uint8 internal constant _maxMintQuantity = 10;
    // uint256 internal constant _mintPrice = 0.0001 ether;
    ...
    function withdrawETH() external onlyOwner {
        (bool sent,) =
            payable(_withdrawalAddress).call{value: address(this).balance}("");
        require(sent, "Withdraw failed");
    }

    function tableURI(string calldata _table)
        external
        view
        override
        returns (string memory)
    {
        return string.concat(_tablelandBaseURI, "SELECT%20*%20FROM%20", _table);
    }
    ...
    function contractURI() public view override returns (string memory) {
        return string.concat(
            _tablelandBaseURI,
            "SELECT+json_object%28%27",
            "name%27%2C+name%2C+%27",
            "description%27%2C+description%2C+%27",
            "image%27%2C+image%2C+%27",
            "external_link%27%2C+external_link%2C+%27",
            "seller_fee_basis_points%27%2C+seller_fee_basis_points%2C+%27",
            "fee_recipient%27%2C+fee_recipient%29",
            "+FROM+",
            "opal_test_collections_80001_3898",
            "+WHERE+",
            "id%3D",
            LibString.toString(_collectionId)
        );
    }
...
interface IOpalBase is IERC165 {
    error InvalidMintQuantity(uint256 minted, uint256 allowed);
    error InvalidPaymentSent(uint256 sent, uint256 required);

    event SetTablelandBaseURI(string oldUri, string newUri);

    function setTablelandBaseURI(string memory _baseURI) external;

    /// URI to query a project-specific table on the Tableland network
    function tableURI(string calldata _table)
        external
        view
        returns (string memory);
    ...
    function setTableNames(
        string memory tokensTable,
        string memory layersTable,
        string memory attributesTable
    )
        external;
}
