...
contract BetsFuzzed is UltibetsCore, ReentrancyGuard {
    ...
    function placeBet(uint256 _eventID, EventResult _eventValue)
        external
        payable
    {
        require(
        ...
        BetData memory bet = BetData(
            msg.sender,
            _eventID,
            betDataList[betDataByBettor[msg.sender][_eventID][_eventValue]].betAmount +
                betAmount,
            0,
            _eventValue,
            block.timestamp
        ...
        dailyBetsByBettor[msg.sender][eventList[_eventID].eDate].push(totalBetNumber);
    ...
    function claimBetCancelled(uint256 _eventID, EventResult _eventResult)
        external
        nonReentrant
    {
        require(
        ...
        uint256 betAmount = betDataList[betDataByBettor[msg.sender][_eventID][_eventResult]]
            .betAmount;
    ...
    function withdrawGain(uint256 _eventID, EventResult _betValue)
        external
        nonReentrant
    {
        require(
        ...
        betDataList[betDataByBettor[msg.sender][_eventID][_betValue]].paidAmount = gain;
