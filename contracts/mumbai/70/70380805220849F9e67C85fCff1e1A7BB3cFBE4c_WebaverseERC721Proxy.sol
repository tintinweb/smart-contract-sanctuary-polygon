...
contract WebaverseERC721 is ERC721 {
    ...
    WebaverseERC20 public erc20Contract; // ERC20 contract for fungible tokens
    uint256 public mintFee; // ERC20 fee to mint ERC721
    address public treasuryAddress; // address into which we deposit minting fees
    ...
    mapping(address => bool) public allowedMinters; // addresses allowed to mint in this copy of the contract
    uint256 public nextTokenId = 0; // the next token id to use (increases linearly)
    mapping(uint256 => string) public tokenIdToHash; // map of token id to hash it represents
    mapping(string => uint256) public hashToStartTokenId; // map of hashes to start of token ids for it
    mapping(string => uint256) public hashToTotalSupply; // map of hash to total number of tokens for it
    mapping(string => Metadata[]) public hashToMetadata; // map of hash to metadata key-value store
    mapping(string => address[]) public hashToCollaborators; // map of hash to addresses that can change metadata
    mapping(uint256 => uint256) public tokenIdToBalance; // map of tokens to packed balance
    mapping(uint256 => address) public minters; // map of tokens to minters
    mapping(uint256 => Metadata[]) public tokenIdToMetadata; // map of token id to metadata key-value store
    mapping(uint256 => address[]) public tokenIdToCollaborators; // map of token id to addresses that can change metadata
