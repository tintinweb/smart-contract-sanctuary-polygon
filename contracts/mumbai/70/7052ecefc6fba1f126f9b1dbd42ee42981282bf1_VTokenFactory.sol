...
contract VTokenFactory is IVTokenFactory {
    mapping(address => address) public override getVToken;

    function createVToken(address token,string memory name,string memory symbol,uint8 decimals,address bridgeControl) external override returns (address vToken) {//TODO 是否需要鉴权
        ...
        VToken(vToken).initialize(token,name,symbol,decimals,bridgeControl);
