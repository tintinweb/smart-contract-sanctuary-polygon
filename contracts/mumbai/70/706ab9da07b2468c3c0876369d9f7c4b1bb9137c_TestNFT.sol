...
// File src/contracts/NFT.sol

pragma solidity ^0.8.0;

contract TestNFT is ERC721, IERC721Receiver, Pausable, IERC2981 {
    ...
    function mint(uint256 nftAmount) public payable whenNotPaused {
    ...
    //check the balance of users
    function getNftBalanceOfUser(
        address _owner
    ...
    //Update recipient members.
    function updateRecipient1(
        address payable recipient1,
        uint256 percentage1
    ) external onlyOwner {
        require(recipient1 != address(0), "Recipient address cannot be zero");
        require(percentage1 > 0, "Percentage must be greater than zero");
        uint totalPercentage = percentage1;
        for (uint i = 1; i < recipients.length; i++) {
            if (recipients[i] == recipient1) {
                revert("Recipient already exists");
            }
            totalPercentage += percentages[i];
        }
        require(totalPercentage <= 100, "Percentages cannot exceed 100%");
        recipients[0] = recipient1;
        percentages[0] = percentage1;
        emit RecipientsUpdated(recipients, percentages);
    }
    ...
    function updateRecipient2(
        ...
        require(recipient2 != address(0), "Recipient address cannot be zero");
        require(value > 0, "Percentage must be greater than zero");
        uint totalPercentage;
        for (uint i = 0; i < recipients.length; i++) {
    ...
    //Update recipient members.
    function updateRecipient3(
        address payable recipient3,
        uint256 value
    ) external onlyOwner {
        require(recipient3 != address(0), "Recipient address cannot be zero");
        require(value > 0, "Percentage must be greater than zero");
        uint totalPercentage;
        for (uint i = 0; i < recipients.length; i++) {
            if (recipients[i] == recipient3) {
                percentages[i] = value;
            }
            totalPercentage += percentages[i];
        }
    ...
    //Update recipient members.
    function updateRecipient4(
        address payable recipient4,
        uint256 value
    ) external onlyOwner {
        require(recipient4 != address(0), "Recipient address cannot be zero");
        require(value > 0, "Percentage must be greater than zero");
        uint totalPercentage;
        for (uint i = 0; i < recipients.length; i++) {
            if (recipients[i] == recipient4) {
                percentages[i] = value;
            }
            totalPercentage += percentages[i];
        }
