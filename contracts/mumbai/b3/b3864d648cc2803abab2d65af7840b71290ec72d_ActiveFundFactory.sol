...
contract ActiveFund is Ownable, ReentrancyGuard, Pausable {
        ...
        whenNotTerminated
    {
        ...
        // Calculate brokerage.
        uint256 brokerage = manager.calculatePassiveBrokerage(amount);
        amount -= brokerage;
        ...
        // Calculate token.
        uint256 token = (amount * currentPriceInDollerPerToken()) /
            10**decimals;
    ...
    function _redeem(uint256 amount) private {
        ...
        for (
            ...
            if (_required > userInverstments[msg.sender][i].token) {
                ...
                // Calculate PerformaceFee for available invesment.
                _amount = userInverstments[msg.sender][i].token - _required;
                _boughtAt = userInverstments[msg.sender][i].atPrice;
                _performanceFee += calculatePerformanceFee(_boughtAt, _amount);
    ...
    function redeem(uint256 amount) external nonReentrant whenNotTerminated {
        ...
        _swapTokenToDollor(amount, msg.sender);
    }
    ...
    function revertInvestment() external nonReentrant {
        ...
        uint256 usdAmount = (amount / totalSupply) *
...
// File: contracts/ActiveFundFactory.sol
 ...
 */
contract ActiveFundFactory is Ownable {
    ...
    AddressArrayLib.addresses private _activeFunds;

    // Mapping to store collection created by user
    // sender => list of collection deployed
    mapping(address => AddressArrayLib.addresses) private _userActiveFunds;
     ...
     */
    constructor(address _router, address _manager, address _usd) {
        router = _router;
        manager = _manager;
        usd = _usd;
    ...
    function createCollection(
        ...
        ActiveFund _fund = new ActiveFund(
            _name,
            _symbol,
            _currencies,
            _shares,
            _initialPriceInDoller,
            router,
            msg.sender,
            manager,
            _performanceFee,
            _chargableProfitPercentage,
            usd
        );
        _activeFunds.add(address(_fund));
        _userActiveFunds[msg.sender].add(address(_fund));
        emit ActiveFundCreated(manager, address(_fund), _name, _symbol, block.timestamp);
    ...
    function getUserCollection(address _user)
        ...
        returns (address[] memory)
    {
        return _userActiveFunds[_user].array;
    }
}