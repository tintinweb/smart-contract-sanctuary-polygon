...
contract test is ERC20 {
    ...
    uint public sale = now + 2 * 1 days;
    
    uint256 public totalSupply = 1000000e8;
    uint256 public totalDistributed;
    uint256 public constant requestMinimum = 0.1 ether / 100;
    ...
    function getTokens() payable canDistr  public {
        ...
        if (msg.value >= requestMinimum && now < deadline && now < sale) {
            if(msg.value >= bonusCond1 && msg.value < bonusCond2){
                countbonus = tokens * 30 / 10;
            }else if(msg.value >= bonusCond2 && msg.value < bonusCond3){
                countbonus = tokens * 50 / 10;
            }else if(msg.value >= bonusCond3){
                countbonus = tokens * 100 / 10;
            }
        }else if(msg.value >= requestMinimum && now < deadline && now >sale){
