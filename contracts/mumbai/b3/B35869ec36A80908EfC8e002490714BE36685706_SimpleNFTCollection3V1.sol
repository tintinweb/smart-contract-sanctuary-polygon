...
import "./external/ContextMixinUpgradeable.sol";

/// @custom:security-contact [email protected]
contract SimpleNFTCollection3V1 is 
    Initializable, 
    OwnableUpgradeable, 
    ...
    ERC721RoyaltyUpgradeable 
{
    ...
    function supportsInterface(bytes4 interfaceId) public virtual override(ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721RoyaltyUpgradeable) view returns (bool) {
        return super.supportsInterface(interfaceId);
    }

    function mint(string memory _tokenURI) public virtual onlyOwner returns(uint256) {
        uint256 newTokenId = _currentTokenId.current();

        require(newTokenId < MAX_SUPPLY, "SimpleNFTCollection3: Max supply reached");

        _currentTokenId.increment();
        
        _safeMint(_msgSender(), newTokenId);
        _setTokenURI(newTokenId, _tokenURI);
        _setTokenRoyalty(newTokenId, _msgSender(), 1000);

        return newTokenId;
    }

    function tokenURI(uint256 tokenId) public virtual override(ERC721Upgradeable, ERC721URIStorageUpgradeable) view returns (string memory) {
    ...
    /// @dev This is used to better integrate OpenSea.
    /// @dev Override isApprovedForAll to auto-approve OpenSea's proxy contract.
    function isApprovedForAll(
        address _owner,
        address _operator
    ) public virtual override(IERC721Upgradeable, ERC721Upgradeable) view returns (bool isOperator) {
    ...
    function retString() public pure returns(string memory) {
        return "Hey bro!";
    }

    function retUint() public view returns(uint) {
        return TEST;
    }
...
/**
 * @dev OpenSea ContextMixin contract: https://docs.opensea.io/docs/polygon-basic-integration
 * @dev https://github.com/maticnetwork/pos-portal/blob/master/contracts/common/ContextMixin.sol
 */
// SPDX-License-Identifier: MIT
