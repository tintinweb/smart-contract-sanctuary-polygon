 ...
 */
contract VCPoCNft is IVCPoCNft, ERC721Enumerable, AccessControl, CanWithdrawERC20 {
    ...
    uint256 _tokenId;
    uint256 _boostDuration = 5 days;

    uint256 constant _DENOMINATOR = 10_000;
    uint240 constant _USDC_MULTIPLICATOR = 10 ** 6;
    Range[] _boostRanges;

    /// @notice Maps the token id of the NFT to its corresponding contribution
    mapping(uint256 => Contribution) private _pocData;
    ...
    constructor(
        ...
        if (admin == address(this) || admin == address(0)) {
            revert PoCUnexpectedAdminAddress();
        }
        _admin = admin;
        _setTo(pool);

        _grantRole(MINTER_ROLE, starter);
        _grantRole(MINTER_ROLE, pool);
        _grantRole(MINTER_ROLE, marketManager);
        _grantRole(MINTER_ROLE, marketplaceFixedPrice1155);
        _grantRole(MINTER_ROLE, marketplaceFixedPrice721);
        _grantRole(MINTER_ROLE, marketplaceAuction721);
        _grantRole(MINTER_ROLE, marketplaceFixedPricePocNft);
        _grantRole(MINTER_ROLE, marketplaceAuctionPocNft);

        _grantRole(APPROVER_ROLE, marketManager);

        _boostRanges.push(Range(10_000 * _USDC_MULTIPLICATOR, 2_500));
        _boostRanges.push(Range(30_000 * _USDC_MULTIPLICATOR, 5_000));
        _boostRanges.push(Range(70_000 * _USDC_MULTIPLICATOR, 7_500));
        _boostRanges.push(Range(150_000 * _USDC_MULTIPLICATOR, 10_000));
    }
    ...
    function _onlyAdmin() internal view {
        if (msg.sender != _admin) {
            revert PoCOnlyAdminAllowed();
        }
    }

    function setAdmin(address admin) external {
        _onlyAdmin();
        if (admin == address(this) || admin == address(0) || admin == _admin) {
            revert PoCUnexpectedAdminAddress();
        }
        _admin = admin;
    }
    ...
    function changeBoostDuration(uint256 newBoostDuration) external {
        _onlyAdmin();
        if (newBoostDuration == 0 || newBoostDuration == _boostDuration) {
            revert PoCUnexpectedBoostDuration();
        }
        _boostDuration = newBoostDuration;
    }

    function changeBoostRanges(Range[] calldata newBoostRanges) external {
        _onlyAdmin();
        _deleteBoostRanges();
        _checkAndSaveBoostRanges(newBoostRanges);
        emit PoCBoostRangesChanged(newBoostRanges);
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(IERC165, ERC721Enumerable, AccessControl) returns (bool) {
    ...
    function votingPowerBoost(address _user) external view returns (uint256) {
        return _diminish(_totalEffectiveDonation(_user));
    }
    ...
    function mint(address user, uint256 amount, bool isPool) external onlyRole(MINTER_ROLE) {
        uint256 tokenId = _tokenId;
        _tokenId++;
        _mint(user, tokenId);
        _pocData[tokenId] = Contribution(amount, block.timestamp);
        emit PoCNFTMinted(user, amount, _tokenId, isPool);
    }
    ...
    function _deleteBoostRanges() private {
        if (_boostRanges.length > 0) {
            delete _boostRanges;
        }
    }

    function _checkAndSaveBoostRanges(Range[] memory newBoostRanges) private {
        for (uint256 i = 0; i < newBoostRanges.length; i++) {
            if (
                i > 0 &&
                (newBoostRanges[i].maxDonation <= newBoostRanges[i - 1].maxDonation ||
                    newBoostRanges[i].maxBps <= newBoostRanges[i - 1].maxBps)
            ) {
                revert PoCInvalidBoostRangeParameters();
            }
            _boostRanges.push(newBoostRanges[i]);
        }
    }

    function _totalEffectiveDonation(address user) internal view returns (uint256 ted) {
        uint256 balance = balanceOf(user);
        for (uint256 i = 0; i < balance; i++) {
            uint256 tokenId = tokenOfOwnerByIndex(user, i);
            ted += _effectiveDonation(tokenId);
        }
    }

    function _effectiveDonation(uint256 tokenId) internal view returns (uint256) {
        uint256 timestamp = block.timestamp;
        uint256 pocTimestamp = _pocData[tokenId].timestamp;
        if (timestamp < pocTimestamp || timestamp >= pocTimestamp + _boostDuration) {
            return 0;
        } else {
            uint256 amount = _pocData[tokenId].amount;
            return amount - (amount * (timestamp - pocTimestamp)) / _boostDuration;
        }
    }

    function _diminish(uint256 amount) internal view returns (uint256) {
        uint256 high = _boostRanges.length;
        uint256 low = 0;

        while (low < high) {
            uint256 mid = (low + high) / 2;
            if (_boostRanges[mid].maxDonation > amount) {
                high = mid;
            } else {
                low = mid + 1;
            }
        ...
        if (high == 0) {
            // enters here if amount < _boostRanges[0].maxDonation
            return (amount * _boostRanges[0].maxBps) / _boostRanges[0].maxDonation;
        } else {
            // enters here if _boostRanges[high-1].maxDonation <= amount < _boostRanges[high].maxDonation  (high can be equal to _zones.length)
            uint256 index = high - 1;
            if (high == _boostRanges.length) {
                index--;
            }

            uint240 xi = _boostRanges[index].maxDonation;
            uint16 yi = _boostRanges[index].maxBps;
            uint240 xf = _boostRanges[index + 1].maxDonation;
            uint16 yf = _boostRanges[index + 1].maxBps;

            return yi + ((yf - yi) * (amount - xi)) / (xf - xi);
        }
    }
}
