...
contract PerpetualLiquidator is PerpetualRebalanceFunctions, IFunctionList, IPerpetualLiquidator {
    ...
    int128 internal constant THREEQUARTER_64x64 = 0xc000000000000000;

    /**
     * Liquidation can be initiated by anyone. The AMM takes over the defaulting
     * trader position.
     *
     * @param   _perpetualId  The reference for the perpetual.
    ...
    function liquidateByAMM(
    ...
    ) external payable virtual override whenNotPaused returns (int128) {
        ...
        if (_getMarginViewLogic().isMaintenanceMarginSafe(_perpetualId, _traderAddr)) {
            // no revert but return zero fee earned
            return 0;
        }

        require(_traderAddr != address(this), "cannot liquidate AMM");
        return _liquidateByAMM(_getPerpetual(_perpetualId), _liquidatorAddr, _traderAddr);
    }

    function _liquidateByAMM(
        PerpetualData storage _perpetual,
        address _liquidatorAddr,
        address _traderAddr
    )
        internal
        updateFundingAndPrices(_perpetual.id, _getPoolIdFromPerpetual(_perpetual.id))
        returns (int128 fLiquidatedAmount)
    {
        ...
        int128 fFee;
        {
            ...
            fLiquidatedAmount = _getPositionAmountToLiquidate(_perpetual, _traderAddr).neg();
            // we liquidate at mark-price

            (fTraderPos, fPrice, fRealizedPnL, fFee) = _liquidateAndPenalize(
                _perpetual,
                _traderAddr,
                _liquidatorAddr,
                fLiquidatedAmount
            ...
            require(
                _isTraderMarginSafe(_perpetual, _traderAddr, false),
                "trader unsafe after liquidation"
            );
        }
        {
            MarginAccount storage account = marginAccounts[_perpetual.id][_traderAddr];
            // int128 newpos = account.fPositionBC;
            // bytes16 posID = account.positionId;
            emit Liquidate(
                _perpetual.id,
                _liquidatorAddr,
                _traderAddr,
        ...
        )
    {
        ...
        // returns: fTraderPos, fPrice, fRealizedPnL, fPenaltyCC
    }
    ...
    function _payLiquidationPenalty(
        ...
        int128 fPenaltyCC;
        {
            ...
            // pay liquidator
            _transferFromVaultToUser(liqPool, _liquidatorAddr, fHalfPenaltyCC);
            // pay default fund
