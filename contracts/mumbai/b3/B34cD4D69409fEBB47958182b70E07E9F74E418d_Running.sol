...
contract Running is Ownable {
    ...
    function result(string[] memory domains) public onlyOwner {
        for (uint256 i = 0; i < domains.length; i++) {
            ...
            if (position <= chargeNumber) {
                bool isDouble = _history[domain] > 0 && _history[domain] <= position;
    ...
    function widthdrawFromDomain(string memory domain, uint256 value) public {
        ...
        require(keccak256(abi.encodePacked(_wallets[wallet])) == keccak256(abi.encodePacked(domain)), "Invalid withdraw wallet address");
        ...
        onlyOwner
    {
        ...
        accountPayable.transfer(value - (value * chargeFee / 100));
