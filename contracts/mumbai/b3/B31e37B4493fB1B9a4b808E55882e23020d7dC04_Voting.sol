...
contract Voting is GSNRecipient {
    ...
    IRelayHub hub = IRelayHub(0xD216153c06E857cD7f72665E0aF1d7D82172F494);
    ...
    modifier onlyOwner() {
        require(_msgSender() == owner, "Voting: Not an owner");
        _;
    }

    modifier onlyWhitelisted() {
        require(_whitelist[_msgSender()], "Voting: Not authorized to vote");
        _;
    }

    function checkWhitelisted() external view returns (bool) {
    ...
    function getAllData() external view returns (uint[] memory) {
        return counterArray;
    }

    function registerVoter(Voter calldata voter) external onlyWhitelisted {
        require(voter.walletAddress == address(0), "Voter already registered");
        _voters[_msgSender()] = voter;
    }

    function castVote(bool support) external onlyWhitelisted {

        Voter storage voter = _voters[_msgSender()];
        require(voter.walletAddress != address(0), "Voter doesn't exist");
        require(voter.index == 0, "Already voted");
        
        uint index = calculateIndex(voter);
        if(support) {
            index += 90;
        }
    ...
    function getVoterInfo(address wallet) external view returns (Voter memory) {
        require(_voters[wallet].walletAddress != address(0), "Voting: Voter not found");
    ...
    function _postRelayedCall(bytes memory context, bool success, uint256 actualCharge, bytes32 preRetVal) internal override {}
    ...
    function depositIntoHub() payable external onlyOwner {
        hub.depositFor{value: msg.value}(address(this));
    }

    function withdrawFromHub(uint amount, address payable to) external onlyOwner {
        hub.withdraw(amount, to);
    }

    function hubBalance() external view onlyOwner returns(uint) {
