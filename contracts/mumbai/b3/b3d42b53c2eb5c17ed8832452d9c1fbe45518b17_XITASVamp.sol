...
contract XITASVamp is Ownable {
    ...
    {
        require(_pid < lpTokensInfo.length, "XITASVamp: _pid should be less than lpTokensInfo");
    ...
    function addAllowedToken(address _token) external onlyOwner {
        require(_token != address(0),"XITASVamp: _token address should not be 0");

        for (uint256 i = 0; i < allowedTokens.length; i++) {
            if (address(allowedTokens[i]) == _token) {
                require(false, "XITASVamp: Token already exists!");
            }
        }
    ...
    function removeAllowedToken(uint _idx) external onlyOwner {
        require(_idx < allowedTokens.length, "XITASVamp: _idx out of range");
    ...
    {
        require(_token != address(0),"XITASVamp: _token address should not be 0!");
        require(_tokenType < 2,"XITASVamp: wrong token type!");
    ...
    function changeLPToken(uint _idx, address _token, uint16 _tokenType) external onlyOwner {
        require(_idx < lpTokensInfo.length, "XITASVamp: _idx out of range");
        require(_token != address(0), "XITASVamp: _token address should not be 0!");
        require(_tokenType < 2, "XITASVamp: wrong tokenType");
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        require(_pid < lpTokensInfo.length, "XITASVamp: _pid out of range!");
    ...
    function isPairAvailable(address _token0, address _token1)
        ...
        returns (bool)
    {
        require(_token0 != address(0), "XITASVamp: _token0 address should not be 0!");
        require(_token1 != address(0), "XITASVamp: _token1 address should not be 0!");
    ...
    function transferAnyERC20Token(
        ...
        require(tokenAddress != address(0), "XITASVamp: Token address cannot be 0");
