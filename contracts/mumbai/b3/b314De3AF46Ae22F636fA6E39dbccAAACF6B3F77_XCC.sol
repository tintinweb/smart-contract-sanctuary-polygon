...
contract XCC is ERC721A, Ownable, ReentrancyGuard {
    ...
    ) ERC721A("X Chimps Club", "XCC", _maxBatchSize, _collectionSize) {
        baseURI = initBaseURI;
        _signer = signer;
    ...
    function mint() external payable {
        ...
        require(
            ...
            "XCC: The wallet has already minted during public sale."
        );
        require(
    ...
    function withdraw(address to_, uint256 amount_) external nonReentrant onlyOwner {
        ...
        if (to_ == address(0x0000000000000000000000000000000000000000)) {
            to_ = msg.sender;
        }
        if (amount_ == 0) {
