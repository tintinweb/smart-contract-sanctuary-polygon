...
contract Product020802 {
    ...
    function createPolicy(PolicyParams memory policyParams) external {
        ...
        checkDaysOfrequestedCoverageStartDateIsNotInThePast(policyParams.requestedCoverageStartDate, subscriptionDate);
    ...
    function checkDaysOfrequestedCoverageStartDateIsNotInThePast(
        uint256 requestedCoverageStartDate,
        uint256 subscriptionDate
    ) private pure {
        (uint yearOfRequestedCoverageStartDate, uint monthOfRequestedCoverageStartDate, uint dayOfRequestedCoverageStartDate) = requestedCoverageStartDate.timestampToDate();
        (uint yearOfSubscriptionDate, uint monthOfSubscriptionDate, uint dayOfSubscriptionDate) = subscriptionDate.timestampToDate();
        require(
            yearOfRequestedCoverageStartDate >= yearOfSubscriptionDate || monthOfRequestedCoverageStartDate >= monthOfSubscriptionDate || dayOfRequestedCoverageStartDate >= dayOfSubscriptionDate,
