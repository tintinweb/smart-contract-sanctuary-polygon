...
contract RoyaltyStream is Initializable, OwnableUpgradeable {
    Member[] private _members;
    uint256 private total;
    uint256 private maxMembers;
    IWmatic public wmatic;

    function initialize(address _wmaticAddress, uint256 _maxMembers)
        public
        initializer
    {
        __Ownable_init();
    ...
    function remove(uint256 index) internal onlyOwner {
        require(index < _members.length, "Index out of bounds");
        total = total - _members[index].value;

        for (uint256 i = index; i < _members.length - 1; i++) {
    ...
    function addMember(address _account, uint256 _value) public onlyOwner {
        require(_account != address(0), "Invalid account");
        require(_value > 0, "Invalid value");
        require(_members.length < maxMembers, "Too many members");

        _members.push(Member(_account, _value));
        total = total + _value;
    }
    ...
    function changeMemberRate(address account, uint256 value) public onlyOwner {
        for (uint256 i = 0; i < _members.length; i++) {
            if (_members[i].account == account) {
                total = total - _members[i].value;
                _members[i].value = value;
                total = total + value;
                break;
            }
        }
