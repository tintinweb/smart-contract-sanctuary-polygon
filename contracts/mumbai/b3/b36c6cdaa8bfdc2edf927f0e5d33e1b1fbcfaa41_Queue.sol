...
contract AgentStore is Ownable {
    ...
    function begin(uint256 _tokenId, File calldata file) public {
        // Covenants expire after one year. Source: https://qr.ae/pyPsqx
        uint expiration = block.timestamp + 31556952;
        agents[_tokenId].begin(Covenant(file, expiration));
        afterBegin(_tokenId);
    }
    ...
    // Here's an ad-hoc partial LinkedList implementation - a separate contract would be more readable
    uint256 private nextExpiringAgent = COUNT;
    uint256 private lastExpiringAgent = COUNT;
    uint256[COUNT + 1] private nextExpiringAgents;
    uint256[COUNT + 1] private prevExpiringAgents;
    constructor() {
        nextExpiringAgents[COUNT] = COUNT;
        nextExpiringAgents[COUNT] = COUNT;
    }
    function afterEnd(uint256 _tokenId) internal {
        nextExpiringAgents[prevExpiringAgents[_tokenId]] = nextExpiringAgents[_tokenId];
        prevExpiringAgents[nextExpiringAgents[_tokenId]] = prevExpiringAgents[_tokenId];
    }
    function afterBegin(uint256 _tokenId) internal {
        nextExpiringAgents[prevExpiringAgents[COUNT]] = _tokenId;
        prevExpiringAgents[COUNT] = _tokenId;
    }
}
