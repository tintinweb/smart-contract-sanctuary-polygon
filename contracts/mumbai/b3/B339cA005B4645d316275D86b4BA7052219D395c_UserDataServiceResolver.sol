 ...
 */
    ...
    function createNewAccount(address _accountWalletAddress, string memory accountName) public returns (uint256) {
        uint256 tokenId = _createNewAccount(_accountWalletAddress);
        setOptionalName(_accountWalletAddress, tokenId, accountName);
        return tokenId;

    }

    function createNewAccount(address _accountWalletAddress) public returns (uint256) {
        emit Log("create new account requested");
        uint256 tokenid = _createNewAccount(_accountWalletAddress);
        emit Log("got tokenid");
        return tokenid;
    }

    function _createNewAccount(address accountOwner) internal returns (uint256) {
        emit Log("entering internal create account and sending register");
        uint256 tokenId = super._register(accountOwner, true);
        emit Log("super._register is successful... requesting ident");
        string memory _ident = string(abi.encodePacked(tokenId, accountOwner));
        super.setIdent(tokenId, _ident);
        emit Log("identity created");
