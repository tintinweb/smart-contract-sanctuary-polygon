...
contract SportSevenShop is Pausable, Ownable {
    ...
    mapping(Rarity => uint256) public usdPrice;
    mapping(myIERC20 => bool) public allowedStable;

    event BuyPlayer(uint256 _id, Rarity _rarity, address _to);
    ...
    constructor(
        ...
        usdPrice[Rarity.COMMON] = 10000;
        usdPrice[Rarity.RARE] = 100000;
        usdPrice[Rarity.LEGENDARY] = 500000;
    }
    ...
    function buyInETH(
        ...
        uint256 ethPrice = getNFTPriceInETH(_rarity);
        require(
        ...
        for (uint256 i; i < _amount; i++) {
            ...
            emit BuyPlayer(tokenId, _rarity, _to);
        }

        // 0.05 Matic is sent to the user for each bought card
        uint256 valueBack = _amount * 5 * 10 ** 16;
        // The value is immediately transferred to the funds recipient
        (bool sent,) = payable(fundsRecipient).call{value : msg.value - valueBack}("");
     ...
     * Please check for token allowance before calling this function.
    ...
    function buyInUSD(
        ...
        _stableAddress.safeTransferFrom(
            msg.sender,
            fundsRecipient,
            _amount * usdPrice[_rarity] * 10 ** (_stableAddress.decimals() - 2)
        );
        for (uint256 i; i < _amount; i++) {
            uint256 tokenId = _mint(_to);
            emit BuyPlayer(tokenId, _rarity, _to);
    ...
    /**
     * @dev Set the price in USD (no decimals) of a given rarity
     * @param _rarity rarity level
     * @param _price New price in USD (no decimals) for the token
     */
    function setPrice(Rarity _rarity, uint256 _price) external onlyOwner {
        usdPrice[_rarity] = _price;
    }
    ...
    function getNFTPriceInETH(
        ...
        uint256 priceInUsd = usdPrice[_rarity];
        uint256 ethToUsd = _getETHtoUSDPrice();
        // Convert price in ETH for US Dollar price
        priceInETH =
        (priceInUsd * 10 ** ethToUsdFeed.decimals() * 10 ** 16) /
        ethToUsd;
    }
