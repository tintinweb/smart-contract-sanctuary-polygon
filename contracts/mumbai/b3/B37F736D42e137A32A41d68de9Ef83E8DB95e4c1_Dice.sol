...
contract Dice is VRFConsumerBaseV2, Ownable {
  ...
  event winAmount(uint256 _winningAmount);
   ...
   * @param _sliderValue to choose the slider value while placing bet. 
   * @param _isRollOver whether the user wants to roll over or roll under the chosen slider value.
   */
  function placeBet(address ERC20Address, uint amount, bool _isMatic, uint8 _sliderValue, bool _isRollOver) external payable {
    ...
    if (_isRollOver) {
      ...
      uint256 _multiplier; 
      _multiplier = _calcMultiplier(_sliderValue, _isRollOver);
    ...
    if(_isMatic == false){
      ...
      placedBet(_sliderValue, _isRollOver);
...
function placedBet(uint8 _sliderValue, bool _isRollOver) internal {
  ...
  //Internal function called by VRFCoordinator when it receives a valid VRFproof.
  ...
  */
  ...
  function _checkWinner(uint256 _requestId) internal {
    ...
    payable(bet.betterAddress).transfer(_winningAmount);
    }
    else {
      require(
      checkTokenBalance(_tokenAddress) >= _winningAmount,
      'Sorry, Contract does not have enough balance'
    );
    IERC20(_tokenAddress).transfer(bet.betterAddress, _winningAmount);
    }
    emit winAmount(_winningAmount);
  }
  ...
  function checkTokenBalance(address ERC20Address) public view returns(uint) {
    return IERC20(ERC20Address).balanceOf(address(this));
  }

  //Owner is allowed to withdraw the contract's matic balance.
  function withdrawal(address _receiver, uint256 _amount) external onlyOwner {
    require(
  ...
  function TokenWithdraw(address ERC20Address, address _receiver, uint256 _amount) external onlyOwner {
    require(
      checkTokenBalance(ERC20Address) >= _amount,
      'Sorry, Contract has not Enough Balance'
    );
