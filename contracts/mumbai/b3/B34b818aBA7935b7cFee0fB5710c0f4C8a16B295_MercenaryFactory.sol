...
contract MercenaryFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    bytes32 internal constant UPGRADE_HASH = keccak256("Upgrade(address to,uint256 id,uint256 orderid)");
    ...
    uint256 public upgrade_base = 10;

    uint256 public upgrade_max = 20;
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    // 合约部署者设置upgrade_base
    function setUpgradeMin(uint256 _min) external nonReentrant onlyOwner returns(bool){
        upgrade_base = _min;
        return true;
    }

    // 合约部署者设置upgrade_max
    function setUpgradeMax(uint256 _max) external nonReentrant onlyOwner returns(bool){
        upgrade_max = _max;
        return true;
    }
    ...
    //升级验签 
    modifier requiresUpgrade(address to, uint256 id,uint orderid, bytes calldata signature) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(UPGRADE_HASH,to,id,orderid))
            )
        );
    ...
    function upgradeMetaData(
        ...
        requiresUpgrade(_to, _id, _orderid, signature)
        returns(uint[] memory)
    {
        randomResult = uint256(keccak256(abi.encode(randomResult, block.timestamp)));

        //先随机选出哪一个是强化的属性
        uint rand = randomResult % 3; // 0-2
        uint newValue = upgrade_base + randomResult % upgrade_max;
        ...
        charactor[0] =  ch.attack;
        charactor[1] =  ch.defense;
        charactor[2] =  ch.hp;
