...
contract MissionManager is AccessControl, IMissionManager {
    ...
    mapping(string => NFTRental.Mission) public pendingMissions;
    ...
    function setMission(NFTRental.Mission memory mission)
        ...
        onlyRentalPool
    {
        pendingMissions[mission.uuid] = mission;
    ...
    function cancelMission(string calldata _uuid) external onlyRentalPool {
        NFTRental.Mission memory curMission = pendingMissions[_uuid];
    ...
    function acceptMission(string calldata _uuid) external override {
        NFTRental.Mission memory missionToAccept = pendingMissions[_uuid];
        ...
        delete pendingMissions[missionToAccept.uuid];
        missionDates[missionToAccept.uuid].startDate = block.timestamp;
    ...
    function ownerTerminatingMission(string calldata _uuid) external override {
        ...
        missionDates[curMission.uuid].terminationDate = block.timestamp;
    ...
    {
        if (pendingMissions[_uuid].owner != address(0)) {
            return "pending";
        } else if (ongoingMissions[_uuid].owner != address(0)) {
            if (missionDates[_uuid].terminationDate > 0) {
                return "terminating";
            } else {
                return "ongoing";
            }
        } else if (missionDates[_uuid].creationDate > 0) {
            return "done";
        } else {
            return "nonExisting";
        }
    ...
    function retrieveTenantPendingMissions(address _tenant)
        ...
        view
        override
        returns (NFTRental.Mission[] memory tenantPendingMission)
    {
        string[] memory tenantPendingMissionsUuid = tenantPendingMissionUuid[
            _tenant
        ];
        NFTRental.Mission[]
        ...
        for (uint256 i; i < tenantPendingMissionsUuid.length; i++) {
            tenantPendingMissions[i] = pendingMissions[
    ...
    function tenantHasPendingMissionForDapp(
        address _tenant,
        string memory _dappId
    ) public view override returns (bool hasMissionForDapp) {
        string[] memory tenantMissionsUuid = tenantPendingMissionUuid[_tenant];
        for (uint32 i; i < tenantMissionsUuid.length; i++) {
            NFTRental.Mission memory curMission = pendingMissions[
                tenantMissionsUuid[i]
            ];
            if (
                keccak256(bytes(curMission.dappId)) == keccak256(bytes(_dappId))
    ...
    function isTerminatingFunction(string calldata _uuid)
        ...
        returns (bool)
    {
        return missionDates[_uuid].terminationDate > 0;
    }
    ...
    function isMissionCreated(string calldata _uuid)
        public
        view
        override
        returns (bool)
    {
        return missionDates[_uuid].creationDate > 0;
    }
...
interface IMissionManager {
    ...
    function retrieveTenantPendingMissions(address _tenant)
        external
        view
        returns (NFTRental.Mission[] memory tenantPendingMissions);
