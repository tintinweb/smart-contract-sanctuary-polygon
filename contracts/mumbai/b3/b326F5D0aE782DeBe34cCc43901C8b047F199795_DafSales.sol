...
contract DafSales is Ownable {
    ...
    IERC20 public immutable dafCoin;
    ...
    */
    ...
    )
    {
        ...
        dafCoin = IERC20(DAFCoinAddress_);
    }
    ...
    */
    ...
    function buy(uint256 _batchNumber, uint256 _amount, Coin coinType) external payable {
        SalesInformation storage batch = batches[_batchNumber];
        require(_batchNumber < batchCount, "Daf: invalid batch number");
        require(_amount + batch.sold <= batch.initialAmount, "Daf: exceeds total coin number");
        require(batch.active, "Daf: batch sale is not active");

        uint256 payedAmount;
        if (coinType == Coin.ETH) {
            payedAmount = (_amount * batch.priceUSD) / getLatestETHPrice();
            require(msg.value >= payedAmount, "Daf: insufficient balance of ethereum");
        }
        else if (coinType == Coin.WBTC) {
            payedAmount = (_amount * batch.priceUSD) / getLatestWBTCPrice();
            ERC20(WBTCAddress).transferFrom(msg.sender, address(this), payedAmount);
        }
        else if (coinType == Coin.USDC) {
            payedAmount = (_amount * batch.priceUSD) / getLatestUSDCPrice();
            ERC20(USDCAddress).transferFrom(msg.sender, address(this), payedAmount);
        }

        dafCoin.transfer(msg.sender, _amount);
        batch.sold += _amount;
    ...
    /**
     * @dev hedges from the batch
     * @param _batchNumber - target sale batch number
     * @param _amount - coin amount
    */

    function hedge(uint256 _batchNumber, uint256 _amount, HedgeCoin coinType) external payable {
        SalesInformation storage batch = batches[_batchNumber];
        require(_batchNumber < batchCount, "Daf: invalid batch number");
        require(_amount + batch.sold <= batch.initialAmount, "Daf: exceeds total coin number");
        require(batch.active, "Daf: batch sale is not active");
        ...
        if (coinType == HedgeCoin.WBTC) {
            ...
            payedAmount = (_amount * batch.priceUSD) / getLatestWBTCPrice();
            uint256 hedgingFee = payedAmount * BTCHedgingPercent / 10 ** percentageDecimals;
            ...
            payedAmount = (_amount * batch.priceUSD) / getLatestETHPrice();
            uint256 hedgingFee = payedAmount * ETHHedgingPercent / 10 ** percentageDecimals;

            require(msg.value >= hedgingFee + payedAmount, "Daf: insufficient eth balance");
            // fee should be transferred into Daf contract wallet
