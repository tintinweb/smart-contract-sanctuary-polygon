    ...
    IDelegator
{
    constructor(
        ...
        uint256[] memory _absMaxLtvRatios,
        uint256 _pusdPriceCeiling,
        uint256 _pusdPriceFloor
    ) {
    ...
    function getCollateralRatioModel(address asset) external view returns (address model) {
    ...
    function getCurrentMaxLtvRatios(address[] calldata assets) external returns (uint256[] memory ratios) {
        ...
        emit GetCurrentMaxLtvRatios(ratios);
    }

    function getCurrentMaxLtvRatio(address asset) external view returns (uint256 ratio) {
        bytes memory data = _staticcall(abi.encodeWithSelector(
...
contract CRM is ICRM, CRMStorage {
  ...
  function initialize(
    ...
    ICRM[] memory _collateralRatioModels = new ICRM[](_assets.length);
    for (uint256 i = 0; i < _assets.length; i++) {
            _collateralRatioModels[i] = ICRM(this);
    }
    _setCollateralRatioModels(_assets, _collateralRatioModels);
    _setAbsMaxLtvRatios(_assets, _absMaxLtvRatios);
    primeOracle = IPrimeOracle(_primeOracle);
    pusdPriceCeiling = _pusdPriceCeiling;
    pusdPriceFloor = _pusdPriceFloor;
  }

  /* external getters */

  function getCollateralRatioModel(address asset)
        external
        view
        override returns (address){
        return address(collateralRatioModels[asset]);
  }

  function getCurrentMaxLtvRatios(address[] calldata assets)
        external
        override
        returns (uint256[] memory){
        uint256[] memory ltvRequirements = new uint256[](assets.length);
        for (uint256 i = 0; i < assets.length; i++) {
            ICRM crm = collateralRatioModels[assets[i]];
            ltvRequirements[i] = crm.getCurrentMaxLtvRatio(assets[i]);
        }
    ...
    function getCurrentMaxLtvRatio(
      address asset
        ...
        //price <= 0.99
            ...
            return (priceDelta * absMaxLtvRatios[asset]) / 1e4;
        }
    }

    /* external setters */

    function setPusdPriceCeiling(uint256 price) external override onlyAdmin returns (uint256) {
        return _setPusdPriceCeiling(price);
    }

    function setPusdPriceFloor(uint256 price) external override onlyAdmin returns (uint256) {
    ...
    function _getPusdPrice() internal view returns (uint256) {
        return primeOracle.getPusdPrice();
    }

    /* internal setters */

    function _setPusdPriceCeiling(uint256 price) internal returns (uint256) {
        pusdPriceCeiling = price;
        return pusdPriceCeiling;
    }

    function _setPusdPriceFloor(uint256 price) internal returns (uint256) {
    ...
    function _setAbsMaxLtvRatios(
        ...
        require(
            _assets.length == _absMaxLtvRatios.length,
            "ERROR: Length mismatch between 'assets' and 'assetLtvRatios'"
        );
        for (uint256 i = 0; i < _assets.length; i++) {
            absMaxLtvRatios[_assets[i]] = _absMaxLtvRatios[i];
            emit AssetLtvRatioUpdated(_assets[i], _absMaxLtvRatios[i]);
        }
    }

    function _setCollateralRatioModels(
       address[] memory _assets,
       ICRM[] memory _collateralRatioModels
    ) internal {
        require(
            _assets.length == _collateralRatioModels.length,
            "ERROR: Length mismatch between 'assets' and 'assetLtvRatios'"
        );
        for (uint256 i = 0; i < _assets.length; i++) {
            collateralRatioModels[_assets[i]] = _collateralRatioModels[i];
            emit CollateralRatioModelUpdated(_assets[i], address(_collateralRatioModels[i]));
        }
    }
}
    ...
    DelegatorErrors
{
    ...
    function setDelegateeAddress(
        address _delegeteeAddress
    ) public onlyAdmin() {
        (delegateeAddress, _delegeteeAddress) = (_delegeteeAddress, delegateeAddress);

        emit DelegateeAddressUpdate(_delegeteeAddress, delegateeAddress);
    }
}
