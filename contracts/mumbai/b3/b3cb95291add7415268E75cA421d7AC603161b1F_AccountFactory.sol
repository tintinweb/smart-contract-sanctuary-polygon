...
contract AccountFactory is Ownable, IAccountFactory {
  ...
  //mapping of id to account address
  ...
  function deploy(uint256 _id, IStorage _store, address _owner) 
    external onlyOwner 
  {
    _accounts[_id] = new Account(_id, _store, _owner);
  }
  ...
  function transferOwnership(address newOwner) 
    public virtual override(IAccountFactory, Ownable) onlyOwner 
  {
  ...
  IProduct 
{
   ...
   */
  constructor(uint256 id, IStorage store, address admin) 
    ERC721StoreChild(id, store) 
  {
   ...
   */
  function product(uint256 productId) public view returns(
    //fixed metadata uri
    string memory uri,
    //max supply size
    uint256 maxSupply,
   ...
   */
  function productOf(uint256 tokenId) public view returns(uint256) {
    return _storage().productOf(tokenId);
  }
  ...
  function supportsInterface(bytes4 interfaceId) 
    ...
    view 
    virtual 
    override(AccessControlEnumerable, ERC721StoreChild) 
    returns(bool) 
  {
  ...
  function mint(
    ...
    _storage().mint(productId, tokenId, recipient);
  }
}
  ...
  IERC721Metadata
{
  ...
  constructor(uint256 id, IStorage store) {
    _childId = id;
    _store = store;
  }
