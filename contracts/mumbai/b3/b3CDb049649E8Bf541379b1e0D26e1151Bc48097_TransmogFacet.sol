...
/******************************************************************************\
/// @title Sipher Portal - Transmog Facet
/// @author Ather Labs
/// @dev  This facet provides functions to mint NFTs to the portal.
/******************************************************************************/
...
contract TransmogFacet is ReentrancyGuard {
    /* ------ TYPE & VARIABLES ------ */
    ///@dev A fixed deadline timestamp for ERC1155C minting Tx
    uint32 internal constant DEADLINE_TO_MINT_ERC1155C = 4294967295;

    /* ------ EVENTS ------ */
    /// @dev Event emitted when an NFT is minted into the portal.
    event MintedToPortal(uint8 listingId, uint88 tokenId, uint88 amount);

    /// @dev Event emitted when a batch of NFTs is minted into the portal.
    event MintedBatchToPortal(uint8 listingId, uint88[] tokenId, uint88[] amount);

    /* ------ EXTERNAL FUNTIONS ------ */
    /// @notice Mint ERC721 NFTs into the portal
    ...
    /// @notice Mint ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    ...
    /// @notice Mint a batch of ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenIds Array of tokenIds.
    /// @param _amounts Array of amount to mint of each tokenId.
    ...
    /// @notice Mint Sipher Spaceship ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    /// @param _amount Amount to mint.
    /// @param _salt A random number convert to string.
    /// @param signature A signature from Sipher server.
    ...
    /// @notice Mint a batch of Sipher Spaceship ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenIds Array of tokenIds.
    /// @param _amounts Array of amount to mint of each tokenId.
    /// @param _salt A random number convert to string.
    ...
    /// @param _listingId Token's listing Id.
    /// @param _tokenType Token's type.
    /// @return address Address of the validated token.
