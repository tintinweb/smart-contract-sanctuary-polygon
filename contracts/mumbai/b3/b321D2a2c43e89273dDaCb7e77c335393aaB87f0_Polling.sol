...
interface PollingInterface {
     ...
     * @param pollFeeForPoll The poll fee paid at the time of poll creation.
     */
    function createPoll(
        uint256 pollId,
        ...
        uint256 startDate,
        uint256 endDate,
        uint256 pollFeeForPoll
    ) external;

    /**
     * @dev Creates a new poll with the specified parameters.
     * @dev Only owner can call this function.
     * @param pollIds The unique identifier of the poll.
     * @param pollTitles The title of the poll.
     * @param pollChoices An array of choices for the poll.
     * @param pollCreators The address of the poll creator.
     * @param startDates The start date of the poll.
     * @param endDates The end date of the poll.
     * @param feeForPoll The array of poll fee paid at the time of poll creation.
     */
    function createPolls(
        ...
        uint256[] memory feeForPoll
    ) external;

    /**
     * @dev Ends the specified polls by providing the necessary information.
     * @dev Only owner can call this function.
     * @param pollId The unique identifier of the poll to be ended.
    ...
    PollingInterface
{
    ...
    function initialize(
        ...
        require(minFee > 0, "Polling: Minimum Fee should be greater than zero");
        require(
            minFee <= feeForPoll,
     ...
     * @param startDate The start date of the poll.
     * @param endDate The end date of the poll.
     * @param feeForPoll The poll fee paid at the time of poll creation.
     */
    function createPoll(
        uint256 pollId,
        string memory pollTitle,
     ...
     * @param feeForPoll The polls fee paid at the time of polls creation.
     */
    function createPolls(
        uint256[] memory pollIds,
        string[] memory pollTitles,
    ...
    function endPoll(
        ...
        PollingDetail storage updatePoll = pollingDetails[pollId];
        ...
        toknContract.burn((updatePoll.pollFee * burnPercent) / MAX_PERCENTAGE);
    ...
    function endPolls(
        ...
        for (uint8 index = 0; index < pollIds.length; index++) {
            if (
                ...
                block.timestamp >= pollingDetails[pollIds[index]].endDate
            ) {
                ...
                toknContract.burn(
                    (updatePoll.pollFee * burnPercent) / MAX_PERCENTAGE
                );
    ...
    function updatePollFee(
        ...
        require(
            minPollFee <= feeForPoll,
            "Polling: Fee should be greater than minimum fee"
        );
        // require(feeForPoll > 0, "Polling: Fee should be greater than zero");
        require(
            feeForPoll != pollFee,
