...
contract Votonx is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Votonx", "VTNX") {}

    function mint(address to, uint256 amount) external {
        require(_controllers.contains(msg.sender) || msg.sender == owner(), "Only controllers and owner can mint");
    ...
    function _calculateSplitFee(uint256 fee) private pure returns (uint256) {
        return fee.div(3);
    }

    function _takeFee(uint256 fee) private {
        uint256 splitFee = _calculateSplitFee(fee);
        _balances[marketingWallet] = _balances[marketingWallet].add(splitFee);
        _balances[researchWallet] = _balances[researchWallet].add(splitFee);
        _balances[developmentWallet] = _balances[developmentWallet].add(splitFee);
        emit Transfer(address(this), marketingWallet, splitFee);
        emit Transfer(address(this), researchWallet, splitFee);
        emit Transfer(address(this), developmentWallet, splitFee);
    }
    ...
    function setResearchWallet(address wallet) external onlyOwner {
