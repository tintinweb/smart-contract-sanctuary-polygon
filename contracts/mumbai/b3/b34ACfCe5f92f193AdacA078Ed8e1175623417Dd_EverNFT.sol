    ...
    ERC165StorageUpgradeable
{
    ...
    event NewPrintMinted(uint256 dropId, uint256 tokenId);

    // Store the base uri for NFT metadata ${_baseTokenURI}/:nftId
    string private _baseTokenURI;
    // Stores the counter for NFt id
    CountersUpgradeable.Counter private _tokenIds;
    // store the address of the Drop Manager
    ...
    whenNotPaused
    {
        ...
        for (uint256 i = 0; i < _quantity; ++i) {
            ...
            dropIdPerToken[newItemId] = drop.dropId;
            _safeMint(_to, newItemId);

            emit NewPrintMinted(drop.dropId, newItemId);
        }

        IEverDropManager(dropManager).updateDropCounter(drop.dropId, _quantity);

        // transfer Fee to the owner address
        if (msg.value > 0) {
            payable(drop.owner).transfer(msg.value);
        }
    }
    ...
    internal override(ERC721PausableUpgradeable, ERC721EnumerableUpgradeable)
    whenNotPaused
    {
...
interface IEverNFT {
     ...
     * @param _quantity : amount of tokens to be minted
     */
    function mint(address _to, uint256 _dropId, uint128 _quantity) external payable;
