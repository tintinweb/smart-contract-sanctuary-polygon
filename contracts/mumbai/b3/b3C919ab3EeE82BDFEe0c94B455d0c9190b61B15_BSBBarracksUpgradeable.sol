...
contract BSBBarracksUpgradeable is Initializable, BSBBaseGameModeUpgradeable {
    ...
    function _claimRewards(CollectionItems calldata collectionItems_) internal {
        bool applyMintPassBuff = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != uint(0);
        ...
        for (uint j = 0; j < collectionItems_.assets.length; ++j) {
            uint referenceDate_ = _getReferenceDateFromStamina(
                collectionItems_.collection,
                collectionItems_.assets[j],
                bearsStats[j].stamina,
                applyMintPassBuff
            );

            uint resultingLevel_ = bearsStats[j].level.value + _calculateLevelReward(
                                                                    collectionItems_.collection,
                                                                    collectionItems_.assets[j],
                                                                    referenceDate_
                                                                );
            resultingLevel_ = resultingLevel_ <= bearsStats[j].level.max
                ? resultingLevel_
                : bearsStats[j].level.max;

            bearsStats[j].level.value = resultingLevel_;

            bearsStats[j].stamina.value = bearsStats[j].stamina.min + _calculateRemainingStamina(
                                                collectionItems_.collection,
                                                collectionItems_.assets[j],
                                                referenceDate_,
                                                applyMintPassBuff
            ...
            BSBStorageCaller.setBearStats(collectionItems_.collection, collectionItems_.assets[j], bearsStats[j]);

            emit LevelClaimed(_msgSender(), collectionItems_.assets[j], resultingLevel_);

            stateChangeDates[collectionItems_.collection][collectionItems_.assets[j]] = block.timestamp;
    ...
    function calculateRemainingStamina(address collection_, uint tokenId_, uint referenceDate_) external view returns(uint) {
