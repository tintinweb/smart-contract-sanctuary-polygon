...
contract RaffleTest is VRFConsumerBaseV2, Ownable{
   ...
   // IERC721 public token;
    ...
    struct Raffle{
        address creator;
        uint totalRounds;
        // uint winnersPerRound;
        uint256[2] startingRounds; // first index for the rounds number and second for the winners of that round.
   ...
   // mapping(uint => mapping(address => bool)) isParticipate;
    ...
    function startRaffle(uint256 Raffle_id, uint256 totalRounds, uint256 totalWinners, uint256 threshold ,bool regRequired) external onlyOwner {
        require(threshold > 0, "Invalid threshold.");
        require(raffle[Raffle_id].status == RaffleStatus.NotInitialized, "Raffle is already Initialized");
        require(totalWinners > totalRounds, "Invalid amount.");
    ...
    function getQuantityOfWinnersOfTheRound(uint256 Raffle_id, uint256 roundNumber) public view returns(uint256 NumberOfWinners){
    ...
    function getRaffle(uint256 roundNumber) public view returns(Raffle memory){
        return raffle[roundNumber];
    }
    function setParticipantsAndPrizes(uint Raffle_id, bytes memory _participants, bytes32[] memory _prizes) external onlyOwner checkRaffle(Raffle_id) {
      
        require(_prizes.length == getQuantityOfWinnersOfTheRound(Raffle_id, raffle[Raffle_id].currentRound), "wrong amount of parameters.");
        prizes  = _prizes; //prize tags set horhe hen a
        console.log("_prizes.length", _prizes.length);
        address[] memory addresses = abi.decode(_participants, (address[]));
        for(uint i=0; i<addresses.length; i++){      
            participants[Raffle_id].push(addresses[i]);
        } 
       
    }
    
    function UserRegister(uint Raffle_id) checkRaffle(Raffle_id) external {
        ...
        isRegistered[Raffle_id][msg.sender] = true;
    }
    ...
    function pickWinners(uint Raffle_id ) external onlyOwner checkRaffle(Raffle_id) returns (uint256 requestId) {
        ...
        uint256 winnersOfThisRound = getQuantityOfWinnersOfTheRound(Raffle_id, raffle[Raffle_id].currentRound);        
        RaffleID = Raffle_id;
        // RoundNo = raffle[Raffle_id].currentRound;
   
        uint32 numWords = uint32(winnersOfThisRound);
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        for(uint256 i=0; i<randomWords.length; i++){
            ...
            winners[RaffleID][RoundNo].push(winner);
        ...
        RaffleID = 0;
        RoundNo = 0;
        emit ReceivedRandomness(requestId, winners[RaffleID][RoundNo]);
    }
    function getWinners(uint Raffle_id, uint Round_No) external view returns(address[] memory){
        return winners[Raffle_id][Round_No];
    }
}
