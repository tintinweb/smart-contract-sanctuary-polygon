...
interface IArtFluenceNFT721{
     function createCollection721(string memory _name ,string memory _symbol,string memory _uri,uint expiry,address owner,address payable _royaltyAddress,uint _royalFee,address signer)external returns(address);
}
...
contract ArtFluenceNFTFactory {
    ...
    function createCollection(string memory _name ,string memory _symbol,string memory _uri,uint expiry,address payable _royaltyAddress,uint _royalFee,address _signer,Sig memory sig)public{
        ...
        address _art721 = artFluenceNFT721.createCollection721(_name,_symbol,_uri,expiry,msg.sender,_royaltyAddress,_royalFee,_signer);
