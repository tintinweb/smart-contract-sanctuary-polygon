...
/*
    AssetTokenIssuer contract is designed for instantly minting AssetTokens.
    It's overwriting the asynchronous minting processes that required in the AssetToken.
    Normally, for minting AssetToken: minter should first call requestMint on the AssetToken
...
contract AssetTokenIssuer is AccessControlUpgradeable, ERC1155HolderUpgradeable {
    ...
    /* Pauses mint functionality */
    function pauseMint() external requireAdmin {
        isMintPaused = true;
    }

    /* Unpauses mint functionality */
    function unpauseMint() external requireAdmin {
        isMintPaused = false;
    }

    /* 
        Authorizes an asset with its USD price feed.
        The asset will be allowed to use in mint function as a payment token.
    */
    ...
    /* 
        Sets the AssetToken address and its USD price feed.
        The asset token will be the underlying asset here and it will be issued to the minter.
    */
    ...
    /* Sets the custody account which funds will be transferred to */
    ...
    /* Sets the AssetToken's kya string */
    ...
    /* Calls the approveRedemption method of the AssetToken */
    ...
    /* Returns the AssetToken's getCurrentRate method's result */
    function getCurrentRate() public view returns (uint256) {
        return _getAssetTokenData().getCurrentRate(assetTokenAddress);
    }

    /* 
        Sets the AssetToken's interestRate.
        interestRate should be set as per-second value with positive or negative rate boolean.
    */
    ...
    /* Returns the AssetToken's getInterestRate method result */
    function getInterestRate() external view returns (uint256, bool) {
        return _getAssetTokenData().getInterestRate(assetTokenAddress);
    }

    /* Transfer the issuer role to a new account by calling the AssetToken's transferIssuer method */
    ...
    /* Sets the AssetTokenData contract of the AssetToken */
    ...
    /* Sets the minimum redemption amount of the AssetToken */
    ...
    /* Freezes the AssetToken contract */
    ...
    /* Unfreezes the AssetToken contract */
    ...
    /* Adds an agent to AssetTokenData contract for the AssetToken */
    ...
    /* Removes an agent from AssetTokenData contract for the AssetToken */
    ...
    /* Blacklists an account for the AssetToken */
    ...
    /* Removes an account from the blacklist of the AssetToken */
    ...
    /* Requests asynchronous minting of the AssetToken by calling the AssetToken's requestMint method */
    ...
    /* 
        Requests redemption of the AssetToken by calling the AssetToken's requestRedemption method.
        Issuer role can redeem instantly. So, when a user is calling requestRedemption; redeem will be instant.
        It will transfer the given asset token amount to the issuer contract and then will execute the 
        requestRedemption method which redeems the staked asset in.
    */
    function requestRedemption(uint256 _assetTokenAmount, string memory _destination) external returns (uint256) {
        require(IERC20(assetTokenAddress).balanceOf(_msgSender()) >= _assetTokenAmount, "Not enough balance");
        require(
            IERC20(assetTokenAddress).allowance(_msgSender(), address(this)) >= _assetTokenAmount,
    ...
    /* Prevents the AssetToken's transfers to a specific account on its safe guard mode */
    ...
    /* Returns AssetTokenData contract of the AssetToken for internal usage */
    ...
    /* Returns the AssetToken contract set in this issuer contract for internal usage */
    function _getAssetToken() private view returns (IAssetToken) {
        return IAssetToken(assetTokenAddress);
    }

    /* 
        Returns how much AssetToken will be minted with the given asset and amount.
        It's useful when an estimation/quote needed on the user interface.
    */
    ...
    /* 
        Calculates how much AssetToken needs to be requested from the AssetToken contract.
        This method calculates the fee amount and deduct it from the asset that goes in.
        Afterwards, it reads the price feeds of the AssetToken and the given asset.
        Lastly, calculates the AssetToken amount will be paid to an account.
    */
    ...
    /*
        Instantly mints AssetTokens for the account that calls this method.
        It will transfer the given asset in to the custody address.
        Then it calls the requestMint function of the AssetToken with the calculated amoun to mint.
