...
import "./utils/UserMintableCollectionWhitelist.sol";
...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    function _setTokenRoyalty(
        ...
        require(
            ...
            "UserMintableCollection ERC2981: royality must be less than max limit"
        );
        require(
            feeNumerator <= _feeDenominator(),
            "UserMintableCollection ERC2981: royalty fee will exceed salePrice"
        );
        require(
            receiver != address(0),
            "UserMintableCollection ERC2981: Invalid parameters"
        );
        _tokenRoyaltyInfo[tokenId] = RoyaltyInfo(receiver, feeNumerator);
        emit RoyaltyInfoSet(tokenId, receiver, feeNumerator);
    } //after meeting they need to set up
...
contract UserMintableCollection is
    ERC2771Context,
    ERC721URIStorage,
    ERC2981,
    Whitelist
{
    ...
    constructor(
        ...
        require(
            ...
            "UserMintableCollection: address must be valid"
        );
    ...
    /**
     *@dev modifer to check only owner and forwarder contract
     */
    modifier onlyValidAddress() {
        require(
            msg.sender == forwarder || _msgSender() == owner(),
            "UserMintableCollection: only owner can call this function"
        );
        _;
    }
    ...
    modifier onlyInitialOwner(uint256 _tokenId) {
        require(
            ...
            "UserMintableCollection: only initial owner can set the royality"
        );
        _;
    }
    ...
    function changeLimitOfRoyality(uint256 _limit) external onlyOwner {
        require(
            ...
            "UserMintableCollection: royalty must be less than equal to 100%"
        );
        uint256 _oldLimit = maxRoyalty;
        require(
            _limit != _oldLimit,
            "UserMintableCollection: limit must be different"
        );
        maxRoyalty = _limit;
    ...
    function mint(
        ...
        uint96 _feeNumerator
    ) external onlyValidAddress returns (uint256 tokenId) {
        require(
            _to != address(0),
            "UserMintableCollection: _to address not valid"
        );
        require(
            bytes(_tokenURI).length > 0,
            "UserMintableCollection: Token URI is not valid"
        );
        counter++;
        tokenId = counter;
        ...
        if (_feeNumerator != 0) {
            _setTokenRoyalty(tokenId, _to, _feeNumerator);
        }
        initialOwner[tokenId] = _to;
    }

    /**
     *@dev owner can change forwerder address
     */
    function changeForwarder(address _newOwner) external onlyOwner {
        forwarder = _newOwner;
    }
    ...
    function burn(uint256 _tokenId) external {
        require(
            ...
            "UserMintableCollection: burn caller is not owner nor approved"
        );
    ...
    function approve(address to, uint256 tokenId) public override {
        if (to.isContract()) {
            require(
                whitelisted(to),
                "UserMintableCollection: Only whitelist is allowed"
            );
        }
    ...
    /**
     * @dev disable owner to set approve for those operator not in whitelist.
     */
    ...
    function transferFrom(
        ...
        require(
            ...
            "UserMintableCollection: transfer caller is not owner nor approved"
        );
...
contract Whitelist is Ownable {
    ...
    function addAddress(address _address) external onlyOwner {
        require(
            ...
            "UserMintableCollection WhiteList: address already whitelisted"
        );
        whitelistedMap[_address] = true;
    ...
    function removeAddress(address _address) external onlyOwner {
        require(
            ...
            "UserMintableCollection WhiteList: address already removed"
        );
        whitelistedMap[_address] = false;
