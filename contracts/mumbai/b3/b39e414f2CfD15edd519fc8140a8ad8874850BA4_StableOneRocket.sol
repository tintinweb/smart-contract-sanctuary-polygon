...
// File: contracts/StableOneRocket.sol
...
contract StableOneRocket is ERC20, ERC20Permit, ERC20Votes, Ownable {
     constructor() ERC20("Stable One Rocket", "SROCKET") ERC20Permit("Stable One Rocket") {
    ...
    uint256 public publicSaleSupply = 40000* 10 ** decimals();
    
    //Supply limit of presale
    uint256 public preSaleSupply = 25000* 10 ** decimals();
    ...
    // Minimum Tokens a user can buy
    uint256 minBuyLimit = 100000000000000000;
    ...
    function _validatePurchase (address _beneficiary, uint256 weiAmount, State currentState ) internal returns (uint256){
        ...
        if(currentState== State.PreSale){
            requiredAmount = weiAmount.div(preSalePrice).mul(10 ** decimals());
            require(requiredAmount<=(preSaleSupply-preSaleTokensSold),"Cannot transfer more than supply for presale");
            preSaleTokensSold = preSaleTokensSold + requiredAmount;
        }
        else if(currentState == State.PublicSale){
            requiredAmount = weiAmount.div(publicSalePrice).mul(10 ** decimals());
            require(requiredAmount<=(publicSaleSupply-publicSaleTokensSold),"Cannot transfer more than supply for publicsale");
            publicSaleTokensSold = publicSaleTokensSold + requiredAmount;            
        }
    ...
    function buyToken() external payable{
        ...
        IERC20(address(this)).safeTransfer(msg.sender, tokensAmount);
    }
    ...
    function setPreSale(bool _condition) external onlyOwner{
        require(_condition!=preSaleOpen);
        preSaleOpen = _condition;
    }

    function setPublicSale (bool _condition) external onlyOwner{
        require(_condition!=publicSaleOpen);
        publicSaleOpen = _condition;
        if(preSaleTokensSold<preSaleSupply){
            publicSaleSupply = publicSaleSupply + (preSaleSupply-preSaleTokensSold);
        }
    }
    ...
    function withdrawNotSoldTokens() public payable onlyOwner {
