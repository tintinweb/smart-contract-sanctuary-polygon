...
error MetaStarNFT__TransferFailed();
error MetaStarNFT__NotEnoughMoneySent();
error MetaStarNFT__CollectionSoldOut();
error MetaStarNFT__NFTAlreadyMinted();
error MetaStarNFT__NFTIndexDoesNotExist();
error MetaStarNFT__StarTypeNotExists();
error MetaStarNFT__PackTypeNotExists();
error MetaStarNFT__StarTypeSoldOut();
error MetaStarNFT__IncorrectNumStars();

contract MetaStarNFTV2 is ERC721URIStorage, Ownable {
    ...
    uint256 private immutable s_DISCOUNT_FACTOR;
    ...
    uint256 private immutable i_mintFee_type0;
    uint256 private immutable i_mintFee_type1;
    uint256 private immutable i_mintFee_type2;
    uint256 private immutable i_mintFee_type3;
    uint256 private immutable i_mintFee_type4;
    ...
    event DiscountFactorChanged(uint256 newDiscount);
    event PackMinted(uint256 packType, uint256 numStars);

    /// @notice Mappings
    mapping(address => bool) private isAdmin;
    mapping(uint256 => string) private s_metaStarTokenUris;
    ...
    constructor(uint256 threshold0, uint256 threshold1, uint256 threshold2, uint256 threshold3, uint256 mintFee_type0, uint256 mintFee_type1, uint256 mintFee_type2, uint256 mintFee_type3, uint256 mintFee_type4) ERC721("METASTAR NFT", "MST") {
        /**
        ...
        */
        ...
        i_mintFee_type0 = mintFee_type0;
        i_mintFee_type1 = mintFee_type1;
        i_mintFee_type2 = mintFee_type2;
        i_mintFee_type3 = mintFee_type3;
        i_mintFee_type4 = mintFee_type4;
    }
    ...
    */
    function reserveStar(uint256 tokenId, address recipient) public onlyAdmin {
        if (tokenId >= i_MAX_STARS) {
            revert MetaStarNFT__NFTIndexDoesNotExist();
        }
        if (s_tokenCounter >= i_MAX_STARS) {
            revert MetaStarNFT__CollectionSoldOut();
        }
        if(_exists(tokenId)) {
            revert MetaStarNFT__NFTAlreadyMinted();
        }
    ...
    */
    function mintNFT(uint256 tokenId, address recipient) internal {
        ...
        _setTokenURI(tokenId, s_metaStarTokenUris[tokenId]);
        emit NftMinted(tokenId, recipient);
    }

    /**
    @notice Basic function for minting a super-small NFTs without specifying tokenId
    */
    function mintSuperSmallNFT() onlyActive public payable {
        if (s_superSmallTokenCounter >= i_threshold0) {
            revert MetaStarNFT__StarTypeSoldOut();
        }
        mintMetaStarNFT(s_superSmallTokenCounter);
    }

    /**
    @notice Basic function for minting a small NFT without specifying tokenId
    */
    function mintSmallNFT() onlyActive public payable {
        if (s_smallTokenCounter >= i_threshold1 - i_threshold0) {
            revert MetaStarNFT__StarTypeSoldOut();
        }
        uint256 tokenId = s_smallTokenCounter + i_threshold0;
        mintMetaStarNFT(tokenId);
    }

    /**
    @notice Basic function for minting a medium NFT without specifying tokenId
    */
    function mintMediumNFT() onlyActive public payable {
        if (s_mediumTokenCounter >= i_threshold2 - i_threshold1) {
            revert MetaStarNFT__StarTypeSoldOut();
        }
        
        uint256 tokenId = s_mediumTokenCounter + i_threshold1;
        mintMetaStarNFT(tokenId);
    }

    /**
    @notice Basic function for minting a big NFT without specifying tokenId
    */
    function mintBigNFT() onlyActive public payable {
        if (s_bigTokenCounter >= i_threshold3 - i_threshold2) {
            revert MetaStarNFT__StarTypeSoldOut();
        }
        
        uint256 tokenId = s_bigTokenCounter + i_threshold2;
        mintMetaStarNFT(tokenId);
    }

    /**
    @notice Basic function for minting a special NFT without specifying tokenId
    */
    function mintSpecialNFT() onlyActive public payable {
        if (s_specialTokenCounter >= i_MAX_STARS - i_threshold3) {
            revert MetaStarNFT__StarTypeSoldOut();
        }
        
        uint256 tokenId = s_specialTokenCounter + i_threshold3;
        mintMetaStarNFT(tokenId);
    }

    /**
    * @notice Function to mint a MetaStarNFT
    * @dev 
    ...
    */
    function mintMetaStarNFT(uint256 tokenId) onlyActive public payable {
        if (tokenId >= i_MAX_STARS) {
            revert MetaStarNFT__NFTIndexDoesNotExist();
        }
        if (s_tokenCounter >= i_MAX_STARS) {
        ...
        if (tokenId < i_threshold0){ 
            if (msg.value < i_mintFee_type0) {
                revert MetaStarNFT__NotEnoughMoneySent();
            }
            s_superSmallTokenCounter += 1;
        } else if (tokenId < i_threshold1 && tokenId >= i_threshold0) {
            if (msg.value < i_mintFee_type1) {
                revert MetaStarNFT__NotEnoughMoneySent();
            }
            s_smallTokenCounter += 1;
        } else if (tokenId < i_threshold2 && tokenId >= i_threshold1) {
            if (msg.value < i_mintFee_type2) {
                revert MetaStarNFT__NotEnoughMoneySent();
            }
            s_mediumTokenCounter += 1;
        } else if (tokenId < i_threshold3 && tokenId >= i_threshold2) {
            if (msg.value < i_mintFee_type3) {
                revert MetaStarNFT__NotEnoughMoneySent();
            }
            s_bigTokenCounter += 1;
        } else {
            if (msg.value < i_mintFee_type4) {
                revert MetaStarNFT__NotEnoughMoneySent();
            }
            s_specialTokenCounter += 1;
        }
    ...
    */
    function mintPack(uint256 packType, uint256 numStars) onlyActive public payable {
        if((packType != 5 || packType != 4) && (numStars < 5 || numStars > 30)){
            revert MetaStarNFT__IncorrectNumStars();
        } else if ((packType == 4) && (numStars < 3 || numStars > 30)){
            revert MetaStarNFT__IncorrectNumStars();
        } else {
        ...
        if (packType == 1) { 
            ...
            revert MetaStarNFT__PackTypeNotExists();
        }
    ...
    */
    function addUris(string[] memory uris, uint256 first_index) public onlyAdmin {
        ...
        require(first_index == (s_uriAddedCounter), "Index incorrect.");
        }
        for (uint256 i = 0; i < uris.length; i++){
            s_metaStarTokenUris[first_index + i] = uris[i];
            s_uriAddedCounter += 1;
        }
    }
    ...
    */
    function withdraw() public onlyAdmin {
        ...
        if(!success) { 
            revert MetaStarNFT__TransferFailed();
        }
    ...
    */
    ...
    function getStarTokenUris(uint256 index) public view returns (string memory) {
        return s_metaStarTokenUris[index];
    }
