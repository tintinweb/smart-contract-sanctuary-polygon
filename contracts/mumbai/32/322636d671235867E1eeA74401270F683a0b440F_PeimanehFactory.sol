...
import "../../interface/IPeimanehFactory.sol";
import "./Peimaneh.sol";

contract PeimanehFactory is IPeimanehFactory {
  address public lastPeimaneh;

  event PeimanehCreated(address value);

  function createPeimaneh(
    address _pool,
    address _padash,
    address _ve,
    address[] memory _allowedRewardTokens
  ) external override returns (address) {
    address _lastPeimaneh = address(new Peimaneh(_pool, _padash, _ve, msg.sender, _allowedRewardTokens));
    lastPeimaneh = _lastPeimaneh;
    emit PeimanehCreated(_lastPeimaneh);
    return _lastPeimaneh;
  }

  function createPeimanehSingle(
    address _pool,
    address _padash,
    address _ve,
    address _voter,
    ...
    address _lastPeimaneh = address(new Peimaneh(_pool, _padash, _ve, _voter, _allowedRewardTokens));
    lastPeimaneh = _lastPeimaneh;
    emit PeimanehCreated(_lastPeimaneh);
    return _lastPeimaneh;
  }
...
import "../../interface/IPeimaneh.sol";
...
/// @title Peimanehs are used to incentivize pools, they emit reward tokens over 7 days for staked LP tokens
contract Peimaneh is IPeimaneh, MultiRewardsPoolBase {
  using SafeERC20 for IERC20;

  /// @dev The ve token used for peimanehs
  address public immutable ve;
  address public immutable padash;
  ...
  function _claimFees() internal returns (uint claimed0, uint claimed1) {
    ...
    if (claimed0 > 0 || claimed1 > 0) {
      ...
      if (_fees0 > IMultiRewardsPool(padash).left(_token0)) {
        fees0 = 0;
        IERC20(_token0).safeIncreaseAllowance(padash, _fees0);
        IPadash(padash).notifyRewardAmount(_token0, _fees0);
      } else {
        fees0 = _fees0;
      }
      if (_fees1 > IMultiRewardsPool(padash).left(_token1)) {
        fees1 = 0;
        IERC20(_token1).safeIncreaseAllowance(padash, _fees1);
        IPadash(padash).notifyRewardAmount(_token1, _fees1);
      } else {
        fees1 = _fees1;
      }
  ...
  function _lockVeToken(address account, uint tokenId) internal {
    ...
    if (tokenIds[account] == 0) {
      ...
      IVoter(voter).attachTokenToPeimaneh(tokenId, account);
    }
  ...
  function _unlockVeToken(address account, uint tokenId) internal {
    ...
    tokenIds[account] = 0;
    IVoter(voter).detachTokenFromPeimaneh(tokenId, account);
    emit VeTokenUnlocked(account, tokenId);
  }

  /// @dev Similar to Curve https://resources.curve.fi/reward-peimanehs/boosting-your-crv-rewards#formula
