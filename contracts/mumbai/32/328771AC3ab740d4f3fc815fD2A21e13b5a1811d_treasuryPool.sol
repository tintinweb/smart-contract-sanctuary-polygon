...
contract treasuryPool is Ownable, whitelistChecker{
    ...
    function depositToken (Whitelist memory users) external payable {
        ...
        if (!users.inBnb) {
            busd.transferFrom(msg.sender, address (this), users.amount);
            investorInvestedAmountPerProjectInBUSD[users.projectId][msg.sender] += users.amount;
            projectTotalCollectedInBUSD[users.projectId] += users.amount;
        }
        else {
    ...
    function withDrawProjectOwner (Whitelist memory users) external {
            require (getSigner(users) == designatedSigner,'Invalid Signer');
            require (users.userAddress == msg.sender,'Invalid User');
            require (!nonceStore[users.userAddress][users.timestamp],'!Nonce Used');
            nonceStore[users.userAddress][users.timestamp] = true;
            if (!users.inBnb) {
                require (msg.sender == projectIdsToOwner[users.projectId]);
                fundsWithdrawn[users.projectId][users.inBnb] += users.amount;
                ...
                busd.transfer(projectIdsToOwner[users.projectId], users.amount);
            }
            else {
                require (msg.sender == projectIdsToOwner[users.projectId]);
                fundsWithdrawn[users.projectId][users.inBnb] += users.amount;
                ...
                require (deducedAmount >= fundsWithdrawn[users.projectId][users.inBnb]);
                payable(projectIdsToOwner[users.projectId]).transfer(users.amount);
            }
    }
...
contract whitelistChecker is EIP712 {
    ...
    function _hash(Whitelist memory whitelist) internal view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(
                keccak256("Whitelist(address userAddress,uint256 timestamp,uint256 projectId,uint256 amount,uint256 amountOfProjectToken,bool inBnb)"),
                whitelist.userAddress,
                whitelist.timestamp,
                whitelist.projectId,
                whitelist.amount,
                whitelist.amountOfProjectToken,
                whitelist.inBnb
            )));
    }
