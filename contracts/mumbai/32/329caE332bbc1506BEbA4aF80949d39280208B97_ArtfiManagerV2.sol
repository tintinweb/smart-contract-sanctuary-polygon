  ...
  PausableUpgradeable
{
  ...
  ArtfiICollectionV2 private _artfiCollectionV2;
  ...
  modifier isArtfiMarketplace() {
    if (
      msg.sender != address(_artfiCollectionV2) &&
  ...
  function setConfiguration(
    ...
    if (blockList[msg.sender] == true) revert GeneralError("AF:126");
    if (serviceFeeWallet_ != address(0)) serviceFeeWallet = serviceFeeWallet_;
   ...
   *@param artfiCollectionV2_ address of artfiCollectionV2.
   */
  function setContractAddress(
    address artfiMarketplace_,
    address artfiCollectionV2_,
    address artfiFixedPrice_
  ) external {
    if (!isAdmin(msg.sender)) revert GeneralError("AF:101");
    if (artfiMarketplace_ != address(0)) _marketplace = artfiMarketplace_;
    if (artfiCollectionV2_ != address(0))
      _artfiCollectionV2 = ArtfiICollectionV2(artfiCollectionV2_);
  ...
  function addCryptoToken(
    ...
    ) revert GeneralError("AF:114");
    if (tokenValue_ == 0) revert GeneralError("AF:115");

    _cryptoTokenList[tokenName_] = CryptoTokens(address_, tokenValue_, true);
  ...
  function isAdmin(address caller_) public view returns (bool isAdmin_) {
    if (hasRole(DEFAULT_ADMIN_ROLE, caller_) || hasRole(CFO_ROLE, caller_))
      return true;
    else return false;
  }
  ...
  function isPauser(address caller_) public view returns (bool isPauser_) {
    if (hasRole(COO_ROLE, caller_)) return true;
    else return false;
  }
    ...
    )
  {
    ...
    artfiCollectionV2_ = address(_artfiCollectionV2);
  }

  function tokenExist(
    string memory tokenName_
  ) public view returns (bool tokenExist_) {
    if (_cryptoTokenList[tokenName_].tokenAddress != address(0)) return true;
    else return false;
  }
   ...
   *@return contractType_ type of contract such as ARTFI_V1,ARTFI_V2,COLLECTOR,EXTERNAL,UNSUPPORTED.
   */
  function getContractDetails(
    address contractAddress_
  ) public isArtfiMarketplace returns (ContractType contractType_) {
    if (contractAddress_ == address(_artfiCollectionV2))
      return (ContractType.ARTFI_V2);
    else {
      bytes memory payload = abi.encodeWithSignature("isCollectionContract()");
      (bool success, ) = contractAddress_.call(payload);
      if (success) return (ContractType.COLLECTOR);
      else {
        if (_is721(contractAddress_)) return (ContractType.EXTERNAL);
        else return (ContractType.UNSUPPORTED);
      }
    }
  }
   ...
   *@param lazyData_ contains tokenAddress,uri,seller address,creater addresses, royalties percentage, investors addresses, stakeRoyalty percentage
  ...
  ) external view isArtfiMarketplace returns (address) {
    bytes32 digest = _hashTypedDataV4(
      keccak256(
        abi.encode(
          keccak256(
            "NFT(string uri, address[] creators,uint256[] royalties,address[] investors,uint256[] stakeRoyalty,uint256 minPrice,uint256 quantity)"
          ),
          ...
          keccak256(abi.encodePacked(lazyData_.stakeRoyalty)),
  ...
  function _isNftTransferApproved(
    ...
    if (nftContract_ == address(_artfiCollectionV2)) return true;
  ...
  function _calculatePayout(
    ...
    view
    virtual
    returns (uint256 serviceFee_, uint256[] memory payoutFees_, uint256 netFee_)
  {
  ...
  function _is721(address tokenContract) internal returns (bool) {
    return _checkInterface(tokenContract, type(ArtfiICollectionV2).interfaceId);
  }
}
