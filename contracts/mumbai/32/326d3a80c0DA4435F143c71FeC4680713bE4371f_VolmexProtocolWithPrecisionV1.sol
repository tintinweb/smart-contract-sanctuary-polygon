 ...
 */
contract VolmexProtocolV1 is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    ) external virtual onlyActive onlyNotSettled returns (uint256 qtyToBeMinted, uint256 fee) {
        require(
    ...
    function redeemSettled(
        ...
        uint256 collQtyToBeRedeemed = (_volatilityIndexTokenQty * settlementPrice) +
     ...
     */
    function settle(uint256 _settlementPrice) public virtual onlyOwner onlyNotSettled {
        require(
    ...
    function _redeem(
        ...
        if (_receiver != address(this)) collateral.transfer(_receiver, collateralRedeemed);
...
contract VolmexProtocolWithPrecisionV1 is VolmexProtocolV1 {
    ...
    // Stores whitelist V2 protocols for migration of volatility tokens
    mapping (address => bool) public protocolWhitelist;
    // Checks mutex for protocol set
    bool private _executedOnce;
    /**
    ...
    function migrateToV2(
        uint256 _volatilityTokenAmount,
        IVolmexProtocol _v2Protocol
    ) external virtual onlyActive onlySettled {
        require(protocolWhitelist[address(_v2Protocol)], "Volmex: v2Protocol not whitelisted");
        require(_volatilityTokenAmount != 0, "Volmex: amount should be non-zero");
        (uint256 collQtyToBeRedeemed, ) = redeemSettled(
            _volatilityTokenAmount,
            _volatilityTokenAmount,
            address(this)
        );
    ...
    /**
     * @notice Sets v2Protocol at upgrade of V1 protocol
     * @param _v2Protocols Array of addresses of v2 protocols used in AMM
     */
    function setV2Protocol(address[2] memory _v2Protocols) external {
        require(!_executedOnce, "Volmex: v2Protocol already added");
        protocolWhitelist[_v2Protocols[0]] = true;
        protocolWhitelist[_v2Protocols[1]] = true;
        _executedOnce = true;
    }
    ...
    function _redeem(
        ...
        require(_collateralQtyRedeemed > precisionRatio, "Volmex: Collateral qty is less");
        ...
        if (redeemFees > 0) {
            unchecked {
                fee = (_collateralQtyRedeemed * redeemFees) / (precisionRatio * 10000);
            }
            effectiveCollateralQty = effectiveCollateralQty - fee;
            accumulatedFees = accumulatedFees + fee;
        }

        if (_receiver != address(this)) collateral.transfer(_receiver, effectiveCollateralQty);
