...
contract InWritingPromoCodes is Ownable{
    ...
    function mint_and_send_promo_NFT(string memory str, string memory promo) public payable returns (uint256) {
    ...
    function mint_and_send_affiliate_NFT(string memory str, string memory promo, address addr) public payable returns (uint256) {
        ...
        require(msg.value >= ((write.get_minting_cost() * (100 - affiliateDiscount[promo])) / 100), string(abi.encodePacked("payment not sufficient, this affiliate code is only ", Strings.toString(affiliateDiscount[promo]), "%")));
        ...
        payable(affiliateAddress[promo]).transfer(write.get_minting_cost() * affiliateCommission[promo] / 100);
        return tokenId;
    }
