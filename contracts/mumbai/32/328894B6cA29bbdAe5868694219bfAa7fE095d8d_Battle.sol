...
contract Battle is VRFManager, GameManager, AccessControlEnumerable {
    ...
    // home on right or left
    // who goes first
    // left window
    // right window
    // roll weather for now
    ...
    function moveTank(uint256 gameId, int256 _numberOfSpaces) external {
        ...
        if (gameData[gameId].gameState == GameState.HOME) {
            require(
                msg.sender == gameData[gameId].home.id,
                "moveTank: caller not home player"
            );
            ...
            require(
                newTankPosition >= 0 &&
                    newTankPosition <= ITerrain(terrainContract).width()
            );
            ...
            require(
                msg.sender == gameData[gameId].away.id,
                "moveTank: caller not away player"
            );
