...
/**
 * @title [Upgradeable] CWForwarder contract for Meta-Tx support
 *
 * @author FORCO LLC
 */
contract CWForwarder is
  Initializable,
  EIP712Upgradeable,
  OwnableUpgradeable,
  UUPSUpgradeable
{
   ...
   */
  function __CWForwarder_init() external initializer {
    __Ownable_init();
    __EIP712_init("CWForwarder", "1.0");
  ...
  function execute(
    ...
    require(
      ...
      "CWForwarder: signature does not match request"
    );
    require(msg.value >= req.value, "CWForwarder: not enough ETH sent");
    ...
    if (success == false) {
      if (returndata.length > 0) {
        ...
        revert("CWForwarder: failed to execute");
      }
    }
