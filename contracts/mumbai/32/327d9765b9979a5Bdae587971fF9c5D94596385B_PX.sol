...
//    StoragePackage[] public packages;
...
//        packages.push(sp);
    ...
    function getTokenURI(uint256 tokenId, uint256 currentUsage, uint256 storageSize, string memory storageUnit) public view returns (string memory){
        bytes memory dataURI = abi.encodePacked(
            '{"name": "PollinationX #', tokenId.toString(), '","description": "Decentralized Storage On Demand","image": "', getProgressImage(tokenId, storageUnit), '","attributes": [{"display_type": "boost_percentage","trait_type": "Usage","value": ',currentUsage.toString(), '},{"trait_type": "Capacity","value": "',storageSize.toString(),storageUnit,'"}]}'
        );
        return string(
            abi.encodePacked(
    ...
    function mint(uint256 packageId) public virtual payable {
        ...
        if(packageId == 0){
            require(pxFreePackage[msg.sender].alreadyMinted != 1, "Already minted");
    ...
    function addExtraContent(uint256 tokenId, string memory hash, string memory content) public {
        require(_isApprovedOrOwner(_msgSender(), tokenId),"!NFT owner");
    ...
    function getExtraContent(uint256 tokenId, string memory hash) public view returns (string memory){
        bytes32 contentHash = keccak256(abi.encodePacked(hash));
        require(pxStorage[tokenId].extraContent[contentHash].active == 1, "Invalid Hash");
