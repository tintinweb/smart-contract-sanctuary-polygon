...
contract CBY is ERC20 {
    ...
    modifier onlyOwner() {
        require(_msgSender() == owner, "ERC20: Caller on Owner");
        _;
    }
    ...
    function setPairAddress(address _addr, bool _status) external onlyOwner {
    ...
    function setRouterAddress(address _addr, bool _status) external onlyOwner {
    ...
    // Transfer functions are modified to add 2% buy & sell on DEX 
    ...
    function transferFrom(address from, address to, uint256 amount) public override returns (bool) {
        ...
        if(isRouter[_msgSender()] && isPair[to]) {
            ...
            if(fee > 0) {
                require(balanceOf(from) >= amount + fee, "ERC20: balance less than Amount (to be transfered) + DEX sell fee");
