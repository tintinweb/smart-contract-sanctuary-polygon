...
contract MumbaiFantasy is ChainlinkClient, ConfirmedOwner {
  ...
  function requestFirstId() public returns (bytes32 requestId) {
    ...
    function frontEndData() public view returns(uint, uint, string memory) {
      LinkTokenInterface link = LinkTokenInterface(chainlinkTokenAddress());
      uint amountOfLink = link.balanceOf(address(this));
      uint amountOfMatic = address(this).balance;
      string memory _topPlayer = topPlayer;

      return (amountOfLink, amountOfMatic, _topPlayer);
    }
