...
interface ICatRaffle_3_0 {
    ...
    /**
     * @notice Used for user to enter a raffle.
     * @param _raffleId id of raffle to enter.
     * @param _numTickets number of tickets to enter raffle with.
     *
     * Throws ICatRaffle_3_0__InvalidId when raffleId doesn't exist or when raffle is already finished.
     *
     * Emits RaffleEntered
     */
    ...
    /**
     * @notice Draws partial winners for raffle.
     * @param _raffleId id of the raffle to draw winners on.
     *
     * Throws ICatRaffle_3_0__NotDrawable when raffle is is not finished.
     * Throws ICatRaffle_3_0__InvalidId when raffleId doesn't exist.
     * Throws ICatRaffle_3_0__IsDrawn when all the winners are drawn.
     *
     * Emits RaffleDrawn
     */
    ...
    /**
     * @notice Claim user winnings from finished raffles.
     *
     * Throws ICatRaffle_3_0__NoRewards when user doesn't have any claimable rewards.
     *
     * Emits RaffleClaimed
     */
     ...
     * @param _raffle Raffle struct with desired input parameters.
     *
     * Throws ICatRaffle_3_0__ValueError "InvalidTicketId" when ticketId isn't recognized.
     * Throws ICatRaffle_3_0__ValueError "InvalidPrizeId" when prizeId isn't recognized.
     * Throws ICatRaffle_3_0__ValueError "InvalidNumPrizes" when numPrizes is less than one.
     * Throws ICatRaffle_3_0__ValueError "InvalidUserEntries" userMinEntries is higher than userMaxEntries or when either of them
       are below one or when userMaxEntries are higher than totalEntries.
     * Throws ICatRaffle_3_0__ValueError "InvalidTotalEntries" when totalEntries is less than one.
     * Throws ICatRaffle_3_0__ValueError "InvalidStartTime" when startTime is less than current time or finishTime.
     * Throws ICatRaffle_3_0__ValueError "InvalidRaffleTreasury" when input raffle treasury doesn't conform to IRaffleTreasury.
     *
     * Emits RaffleRegistered
     */
    function registerRaffle(RaffleInput memory _raffle) external returns (uint256 _raffleId);

    /**
     * @notice Updates active raffle with new parameters.
     * @param _raffle Raffle struct with desired input parameters.
     *
     * Throws ICatRaffle_3_0__ValueError "InvalidTicketId" when ticketId isn't recognized.
     * Throws ICatRaffle_3_0__ValueError "InvalidPrizeId" when prizeId isn't recognized.
    ...
    /**
     * @notice Used to pause new entries to a raffle
     * @param _raffleId id of the raffle that should be paused.
     * @param _isPaused used for pausing and unpausing raffle.
     *
     * Throws ICatRaffle_3_0__NotPausable when raffle is already paused.
     * Throws ICatRaffle_3_0__InvalidId when raffleId doesn't exist.
     *
     * Emits RaffleSetPaused
     */
    function setPaused(uint256 _raffleId, bool _isPaused) external;

    /**
     * @notice Deletes specified raffle. If tickets are already entered they must be manually reimbursed.
     * @param _raffleId id of the raffle that should be deleted.
     *
     * Throws ICatRaffle_3_0__InvalidId when raffleId doesn't exist.
     *
     * Emits RaffleDeleted
    ...
    /**
     * @notice Get information about specified raffle.
     * @param _raffleId id of the raffle that should be fetched.
     *
     * Throws ICatRaffle_3_0__InvalidId when raffleId doesn't exist.
     */
    ...
    /**
     * @notice Get list of all active raffles. Active raffle is defined as when finishTime is less than current time.
     * @return raffles list of active raffles.
     */
    function getActiveRaffles() external view returns (Raffle[] memory raffles);

    /**
     * @notice
     * @return returns true if a draw can be called on the raffle.
    ...
    PausableUpgradeable
{
    ...
    mapping(uint256 => mapping(uint256 => address)) public ticketToUser;
    mapping(uint256 => uint256) public raffleEntries;
    ...
    function enterRaffle(uint256 _raffleId, uint256 _numTickets) external {
        ...
        if (raffleEntries[_raffleId] + _numTickets > raffles[_raffleId].totalEntries)
        ...
        cumulativeEntries[_raffleId].push(raffleEntries[_raffleId] + _numTickets);
        userAtIndex[_raffleId][cumulativeEntries[raffleId].length - 1] = msg.sender;

        unchecked {
            userRaffleEntries[msg.sender][_raffleId] += _numTickets;
            raffleEntries[_raffleId] += _numTickets;
        }
    ...
    function claimRewards(uint256 _raffleId, uint256[] memory _prizeIds) external {
        for (uint i; i < _prizeIds.length; i++) {
            ...
            _givePrize(winningAddress, _raffleId, _prizeIds[i]);
    ...
    /**
     * @notice Admin register a new raffle with desired parameters.
     * @param r Raffle struct with desired input parameters.
     *
     * Throws ICatRaffle_3_0__ValueError "InvalidTicketId" when ticketId isn't recognized.
     * Throws ICatRaffle_3_0__ValueError "InvalidPrizeId" when prizeId isn't recognized.
     * Throws ICatRaffle_3_0__ValueError "InvalidNumPrizes" when numPrizes is less than one.
    ...
    function registerRaffle(RaffleInput calldata r) external onlyGov returns (uint256 _raffleId) {
        if (r.ticketId != 16 && r.ticketId != 17 && r.ticketId != 18)
            revert ICatRaffle_3_0__InvalidId(r.ticketId);
        if (r.prizeIds.length != r.prizeAmounts.length) revert ICatRaffle_3_0__InvalidInputs();
        if (r.userMinEntries > r.userMaxEntries) revert ICatRaffle_3_0__InvalidUserEntries();
        if (r.userMinEntries == 0) revert ICatRaffle_3_0__InvalidUserEntries();
        if (r.userMaxEntries == 0) revert ICatRaffle_3_0__InvalidUserEntries();
        if (r.userMaxEntries > r.totalEntries) revert ICatRaffle_3_0__InvalidUserEntries();
        if (r.userMinEntries > r.totalEntries) revert ICatRaffle_3_0__InvalidUserEntries();
        if (r.totalEntries == 0) revert ICatRaffle_3_0__InvalidUserEntries();
    ...
    /**
     * @notice Get list of all active raffles. Active raffle is defined as when finishTime is less than current time.
     * @return _activeRaffles list of active raffles.
     */
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        multiplier[_raffleId] =
            (raffleEntries[_raffleId] / 2) +
            (uint256(keccak256(abi.encode(randomWords[0], 1))) % (raffleEntries[_raffleId] / 2));

        while (gcd(multiplier[_raffleId], raffleEntries[_raffleId]) != 1) multiplier[_raffleId]++;

        shifter[_raffleId] =
            uint256(keccak256(abi.encode(randomWords[0], 2))) %
            raffleEntries[_raffleId];
        // console.log("Raffle ", raffleId);
        // console.log("Multipler ", multiplier[_raffleId]);
        // console.log("Shifter ", shifter[_raffleId]);
        // console.log("Total Entries: ", raffleEntries[_raffleId]);
    }
    ...
    function getWinningAddress(uint256 _raffleId, uint256 _prizeId) public view returns (address) {
        ...
        uint256 N = raffleEntries[_raffleId];
        ...
        // console.log("---------------------------------------------------------------------------");
        // console.log("winnerIndex ", uint(winnerIndex), "denseIndex ", denseIndex);
        // console.log("winningTicket: ", winningTicket + 1);

        if (winnerIndex == -1) revert ICatRaffle_3_0__TicketNotFound(winningTicket, N, K);
        // console.log("USER IS: ", userAtIndex[_raffleId][uint256(winnerIndex)]);
