...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
  ...
  // Fired when the deployer mints tokens
  event MintedTokens(uint previousSupply, uint amount, string id);  

  // Only the deployer of this contract
...
contract NFT is ERC721 {
  ...
  function inicialize(string memory name_, string memory symbol_, uint royalties_, address market_) public onlyDeployer {
    require(!inicializated, 'Contract already inicializated');
    _name = name_;
    _symbol = symbol_;
    royalties = royalties_;
    setApprovalForAll(market_, true);
    inicializated = true;
  }

  function mint(uint amount, string memory id) public onlyDeployer {
    require(amount > 0, 'ERC721: Invalid amount');
    _mint(_msgSender(), amount, id);
  }
  ...
  function setURI(string memory URI) public onlyDeployer {
    require(!URI_FLAG, 'Base URI already setted');
    _URI = URI;
    URI_FLAG = true;
  }
