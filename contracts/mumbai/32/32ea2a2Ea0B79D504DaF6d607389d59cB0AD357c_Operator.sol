...
contract Operator is Ownable {
    ...
    function callContract(
        ...
        if (!success) {
            ...
            string memory error = parseError(data);
            revert(error);
        }
    ...
    function parseError(bytes memory _data) internal pure returns (string memory) {
        ...
        bytes memory prefix = '\nError: ';
        bytes memory messageBytes = new bytes(_data.length - 4 - prefix.length);
        for (uint i = 4 + prefix.length; i < _data.length; i++) {
            messageBytes[i - 4 - prefix.length] = _data[i];
        }
        string memory messageString = string(messageBytes);
        return string(abi.encodePacked(prefix, messageString));
    }
