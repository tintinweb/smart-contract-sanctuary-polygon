    ...
    ChainlinkClient
{
     ...
     */
    function Update(uint256 tokenid) public returns (bytes32 requestId) 
    {
        
        UpdateId = tokenid;
        string memory url = bytes32ToString(bytes32(uint256(UpdateId)));
        
        Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fulfill.selector);

        // Set the URL to perform the GET request on
        request.add("get", string(abi.encodePacked(baseURI,'',url)));
      //  request.add("get", "https://min-api.cryptocompare.com/data/pricemultifull?fsyms=ETH&tsyms=USD");

        request.add("path", "data.value");

       // request.addBytes("emojiUnicode", _unicode);
//        request.addStringArray("path", path);
    ...
    function fulfill(bytes32 _requestId, bytes32  _volume) public recordChainlinkFulfillment(_requestId) {
         //   var s = bytes32ToString(_volume);
        //volume = _volume;
        string memory value = string(abi.encodePacked(baseURI,'', bytes32ToString(_volume)));
        _setURI(UpdateId, value);
        UpdateId =0;
    }
        ...
        
    {
        ...
        baseURI = string(abi.encodePacked(_url,'', "?tokenId="));
