...
contract SBTFacet is IERC1155MetadataURI {
    ...
    function mintBatch(address[] calldata _tos, uint256 tokenId_) external {
        ...
        // Not authorized.
        require(LibProjects.getAdminByProjectId(projectId, LibERC2771Context.msgSender()) == 1);
        ...
        for (uint256 i; i < count; ) {
            ...
            require(s.mintDetails[to][tokenId_].isMinted != 1);
    ...
    function burn(address _from, uint256 tokenId_) external {
        ...
        require(s.mintDetails[_from][tokenId_].isMinted == 1);

        delete s.userCardsIds[_from][s.mintDetails[_from][tokenId_].index - 1];
        s.mintDetails[_from][tokenId_].isMinted = 2;
        s.mintDetails[_from][tokenId_].mintedAt = 0;
        s.mintDetails[_from][tokenId_].mintedBy = address(0);
