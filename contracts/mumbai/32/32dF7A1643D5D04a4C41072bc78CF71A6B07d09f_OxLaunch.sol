...
import "../Oxlibrary/SafeMath.sol";
import "../invest/InvestorData.sol";
import "../Oxpool/Pools.sol";

contract OxLaunch is InvestorData {
    ...
    function WithdrawERC20ToInvestor(uint _PoolId) isPoolId(_PoolId) public {
        ...
        for(uint i=1;i<=distributionCount;i++){
            ...
            if(pools[_PoolId].MoreData.DistributionData[i].distributionTime < block.timestamp){
...
import "../Oxtoken/ERC20Helper.sol";
import "../Oxtoken/ETHHelper.sol";
import "./GovManager.sol";
import "../Oxutils/Pausable.sol";
...
contract Pools is Manageable {
    ...
    function isPoolLocked(uint256 _id) public view returns(bool){
        return pools[_id].BaseData.StartTime > block.timestamp || pools[_id].MoreData.distributionCount == 0;
    }
    ...
    function CreatePool(
        ...
        require(_StartTime > block.timestamp, "Need Valid startTime");
        ...
        //register the pool
        pools[poolsCount].BaseData = PoolBaseData(
            _Token,
            msg.sender,
            _MainCoin,
            _StartAmount,
        ...
        pools[poolsCount].MoreData.distributionCount = 0;
    ...
    function UpdatePoolDistribution(
        ...
        require(pools[_id].BaseData.StartTime > block.timestamp,"You could not do this,when the pool is open");
...
contract PoolsData is Pools {
        ...
        returns (PoolStatus)
    {
        ...
        if (block.timestamp < pools[_id].BaseData.StartTime) return PoolStatus.PreOpen;
        if (
            block.timestamp < pools[_id].BaseData.EndTime &&
            pools[_id].BaseData.StartTime < block.timestamp
        ) {
        ...
        if (
            block.timestamp > pools[_id].BaseData.EndTime &&
            pools[_id].MoreData.distributionCount >0 &&
            block.timestamp < pools[_id].MoreData.DistributionData[1].distributionTime
