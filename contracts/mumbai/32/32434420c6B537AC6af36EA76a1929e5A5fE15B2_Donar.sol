...
contract Donar is ERC20 {
    ...
    constructor(uint256 acceptedNgoRegisterationFee,string memory _name,string memory _symbol) ERC20(_name,_symbol) {
        i_ngoRegistrationFee = acceptedNgoRegisterationFee;
        i_owner =msg.sender;
     
    }

    function getReturnSameAddress() onlyOwner  public view returns(uint256){
         for(uint256 i= 0 ; i < s_donorsList.length ; i++ ){
             if(s_donorsList[i] == msg.sender ){
                 return i;
             }
             if(s_donorsList[i] == address(0)){
                 s_donorsList.length - 1;
             } 
        }
    }
...
//10,"Donar","Don"
        ...
        _totalAccumulatedDonorFund[msg.sender] += (msg.value + (profitMade * 10 ** decimals()));
    ...
    function getTransactionRecordsOfDonor() public view returns (donorDetails[] memory) {
    ...
    function getNgoBalance(address owner) public view returns(uint256){
    ...
    function withdraw(address owner, uint256 amount) public {
       require(owner!=address(0),"please use a valid address");
       (bool sucess ,) = payable(owner).call{value:amount}("");
       require(sucess ,"Transaction failed to execute");
    }

    

    function getAmountDonarHasDonatedPlusProfit(address owner) public view returns (uint256){
      require(owner != address(0),"please use a valid address");
        return _totalAccumulatedDonorFund[owner];
    }

    function HowManyDonorExist() public onlyOwner view returns (uint256) {
             return s_donorsList.length ;
        
    }

    function checkIfIhaveDonatedBefore(address owner) public view returns(bool){
        require(owner != address(0),"please use a valid address");
    ...
    function getDonarTotalProfit() public onlyOwner view returns(uint256){
        return interestRate;
    }

    function getDonorAddress(uint256 _Id) public onlyOwner view returns(address){
       return _donorIdToAddress[_Id];
    }

    function getDonorId(address owner) public onlyOwner view returns(uint256){
        require(owner != address(0),"please use a valid address");
    ...
    function HowManyNgoExist() onlyOwner public view returns(uint256){
        return s_NgoList.length;
    }

    function getDonorToAmountDonated(address owner) public view returns(uint256){
         require(owner != address(0),"please use a valid address");
         return _addressToDonorAmount[owner];
    }

    function getNumberOfDonationsByDonorMade() onlyOwner public view returns(uint256){
         return _NumberOfDonationsMade;
    }

    function NgoSendToHelpNeeded (address _ngo , uint256 amount , address _to) public{
       ...
       for(uint256 i= 0 ; i < s_donorsList.length ; i++ ){
          if(s_donorsList[i] == _ngo  || s_donorsList[i] ==_to ){
                revert Donar__InvalidAddress();
             }
        }
    ...
    function getDonorArray(uint256 _i) public view returns(address){
        return s_donorsList[_i];
    }

    function getNgoArray(uint256 _i) public view returns(address){
        return s_NgoList[_i];
    }

    function getHowManyDonarTokensYouHave(address owner) public view returns(uint256){
         require(owner != address(0),"please use a valid address");
         uint256 TokenWorth = balanceOf(owner);
         uint256 numberOfDonarTokens =uint256(TokenWorth/decimals());
         return numberOfDonarTokens;
    }

    function getDonorLeastFee() public onlyOwner view returns(uint256){
