...
contract UsernameNFTArtProxy is IUsernameArtProxy, OwnableUpgradeable {
    ...
    function _tokenURI(string memory _username, uint _date, uint _length, string[] memory traitsCategories, string[] memory traits) external pure returns (string memory) {
        string memory json = Base64.encode(
            bytes(
                string(
                    abi.encodePacked('{',
                        //start json
                        '"name":"', _username,'.thena",', //FIXME do we want the '.thena'?
                        '"description":"Thena Username Ecosystem.",', 
                        _getImageSVG(_username), '",',
                         '"attributes": [',
                            // first attribute
                            '{"display_type": "date","trait_type":"creation","value":"',toString(_date),'"},', 
                            _getExtraAttributes(traits, traitsCategories),
                            // last attribute 
                            '{"display_type": "number","trait_type":"length","value":"',toString(_length),'"}', 
    ...
    function _getExtraAttributes(string[] memory traits, string[] memory traitsCategories) public pure returns (bytes memory) {
    ...
    function _getImageSVG(string memory _username) internal pure returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                        abi.encodePacked(
                            ...
                            '<text id="username.eth" transform="translate(208 1641)" fill="#fff" font-size="210" font-family="Inter-Bold, Inter" font-weight="700"><tspan x="0" y="0">',
                            _username,
                            '</tspan></text>',
                            '</svg>'
                        )
