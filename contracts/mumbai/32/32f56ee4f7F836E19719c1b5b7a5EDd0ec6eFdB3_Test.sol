...
contract Test{    
    ...
    function testCalc(uint256 _amt, uint8 level,uint8 _type) public view returns(uint256,uint256,uint256,uint256){
        if(_type==1)
        {
          uint256 totalMatrixValue=(mlevelPrice[level-1]*(level+3));
          uint256 maticPerDollor=(_amt*1e18)/totalMatrixValue;   
          uint256 levelUSD=levelPrice[level];  
          return (totalMatrixValue, maticPerDollor, levelUSD, ((levelUSD*REFERRAL_PERCENTS[0])*maticPerDollor)/1e21);
        }
    }
