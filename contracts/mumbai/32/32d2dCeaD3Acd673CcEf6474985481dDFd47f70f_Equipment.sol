...
import './interfaces/IEquipmentDescriptor.sol';
import './interfaces/IEquipment.sol';

contract Equipment is ERC1155SupplyUpgradeable, BaseEquipmentNFT, IEquipment {
	using CountersUpgradeable for CountersUpgradeable.Counter;

	mapping(uint256 => string) _imageURIs;
		...
		returns (string memory _uri)
	{
		...
		_uri = ITokenDescriptor(tokenDescriptor).tokenURI(
			tokenId,
			_tokenIndices[tokenId]
		);
	}

	function setImageURI(uint256 tokenId, string memory _uri) public onlyAdmin {
		require(exists(tokenId), 'Token does not exist');
		_imageURIs[tokenId] = _uri;
	...
	function exists(uint256 tokenId)
		public
		view
		override(BaseEquipmentNFT, ERC1155SupplyUpgradeable)
		returns (bool)
	{
	...
	function create(
		...
		setMetadata(Metadata(name, description), tokenId);
		setTokenIndices(tokenId, [start, end]);

		setImageURI(tokenId, _imageUri);
	}
	...
	function pureImage(uint256 tokenId)
		external
		view
		override
	...
	{
		return
			IEquipmentDescriptor(tokenDescriptor).pureImage(
				_tokenIndices[tokenId]
			);
	}
}
...
interface IEquipmentDescriptor {
