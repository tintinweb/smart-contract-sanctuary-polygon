...
contract SFUSDStake is Ownable {
    ...
    modifier onlyAdmin() {
        require(
            owner() == msg.sender || moderatorAddress == msg.sender || isAdmin[msg.sender],
    ...
    function setSFUSDaddress(address _SFUSDaddress)
        external
        onlyOwner
    {
        require(_SFUSDaddress != address(0), "Invalid tokenAddress");
    ...
    function setIsAdmin(address _walletAddress, bool adminStatus) external onlyModerator{
    ...
    function deposit(uint256 _amount) external {
        ...
        if (investors[msg.sender].investor == address(0)) {
            ...
            investors[msg.sender].lastClaimedDate = block.timestamp;
        ...
        if (investors[msg.sender].totalRewardAmount >= _amount) {
            ...
            capitalLockedPool = capitalLockedPool.add(
                _amount.sub(depositFee).sub(investors[msg.sender].totalRewardAmount)
            );
    ...
    function claimRewards() external {
        require(
            depositsLength[msg.sender] > 0,
            "you need to deposit first"
        );
        require(
            investors[msg.sender].lastCalculationDate.add(WITHDRAW_PERIOD) <=
                block.timestamp,
        ...
        investors[msg.sender].lastClaimedDate = block.timestamp;
        investors[msg.sender].lastCalculationDate = block.timestamp;
        investors[msg.sender].totalRewardAmount = investors[msg.sender].totalRewardAmount.add(claimableAmount);
    ...
    function withdrawCapital(uint256 id) external {
        ...
        uint256 claimableReward = getAllWithdrawableReward(msg.sender);
        ...
        investors[msg.sender].lastCalculationDate = block.timestamp;
        investors[msg.sender].lastClaimedDate = block.timestamp;
        investors[msg.sender].totalRewardAmount = investors[msg.sender].totalRewardAmount.add(claimableReward);

        totalReward = totalReward.add(claimableReward);
        capitalLockedPool = capitalLockedPool.sub(depositState[id].depositAmount);
        capitalWithdrawn = capitalWithdrawn.add(depositState[id].depositAmount);
        depositState[id].state = false;

        emit CapitalWithdrawn(msg.sender, claimableReward+depositState[id].depositAmount, id, block.timestamp);
    }

    // calculate all claimable reward for user's investments
    ...
    function getAllWithdrawableReward(address _investorAddress)
        public
        view
        returns (uint256 allClaimableAmount)
    {
        uint256 length = depositsLength[_investorAddress];
        for (uint256 i = 0; i < length; i++) {
            allClaimableAmount += getWithdrawableReward(
                ownedDeposits[_investorAddress][i]
            );
        }
    }

    // calculate all claimable reward for user's particular investment
    ...
    function getWithdrawableReward(uint256 _id)
        public
        view
        returns (uint256 reward)
    {
        ...
        uint256 lastROIDate = investors[investor].lastCalculationDate;
        uint256 profit;
        if (lastROIDate.add(WITHDRAW_PERIOD) <= block.timestamp) {
            profit = depositState[_id]
                .depositAmount
                .mul(DAILY_ROI * 7)
                .mul(block.timestamp.sub(lastROIDate))
        ...
        )
    {
        ...
        lastClaimedDate = investors[_investorAddress].lastClaimedDate;
    ...
    // used by admin to add deposit for the investments
    function migrateInvestorToV2(
        ...
        if (investors[_investor].investor == address(0)) {
            ...
            investors[_investor].lastClaimedDate = _depositAt;
