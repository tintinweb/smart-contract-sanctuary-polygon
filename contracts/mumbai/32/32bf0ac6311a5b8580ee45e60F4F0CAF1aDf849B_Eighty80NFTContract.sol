...
contract Eighty80NFTContract is ERC721EnumerableUpgradeable, Ownable, Pausable, NonblockingLzApp {
    ...
    mapping (address => bool) public isLegitSource;
    mapping (uint => bytes) public routes;
    ...
    function init(string memory name, string memory symbol, address _owner, address endpointAddress) external onlyOnce
    {
        Pausableinitialize();
        Ownableinitialize();
    ...
    function mintNewTokens() external {
            currentIndex ++;
    ...
    function traverseChains (uint[] memory tokenIds, bytes memory userAddress, uint16 destinationChainId) external payable{
        address convertedBytesToAddress;
        assembly {
            convertedBytesToAddress := mload(add(userAddress,20))
        }
        ...
        _lzSend(destinationChainId,payload,payable(msg.sender),address(0),adaptorParams,routes[destinationChainId]);
    }
    ...
    function _nonblockingLzReceive(
        ...
        require (isLegitSource[sourceAddress],"Error: Source Not Whitelisted");
    ...
    function whitelistSourceAddress(address sourceAddress) external onlyOwner {
        isLegitSource[sourceAddress] = true;
    }

    function forceMintTokens(address _to, uint id) external onlyOwner{
    ...
    function setTraversingRoutes(uint16 chainId, bytes memory route) external onlyOwner {
 ...
 * {ERC721Enumerable}.
 */
contract ERC721Upgradeable is Context, ERC165Upgradeable, IERC721Upgradeable, IERC721MetadataUpgradeable {
...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function _lzSend(uint16 _dstChainId, bytes memory _payload, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams, bytes memory trustedDestinationAddress) internal virtual {
        lzEndpoint.send{value: msg.value}(_dstChainId, trustedDestinationAddress, _payload, _refundAddress, _zroPaymentAddress, _adapterParams);
    }
