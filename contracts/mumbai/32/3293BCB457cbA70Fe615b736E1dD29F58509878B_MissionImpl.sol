...
contract MissionImpl is MissionBase {
    ...
    modifier whenRewardsNotMintedYet() {
        require(!rewardsMinted, "ERROR: Rewards have already been minted");
        _;
        rewardsMinted = true;
    }

    function mintRewards() external whenRewardsNotMintedYet {
        _mintBronzeRewards();
        _mintSilverRewards();
        _mintGoldRewards();
    }

    function _mintGoldRewards() internal {
    ...
    function _mintSilverRewards() internal {
        for (uint i = 0; i < 13; i++) {
    ...
    function _mintBronzeRewards() internal {
        for (uint i = 0; i < 5; i++) {
