...
contract TakatufCollectionV2 is ERC721Upgradeable, AccessControlUpgradeable {
    ...
    bytes32 public constant TAKATUF_ADMIN_ROLE =
        keccak256("TAKATUF_ADMIN_ROLE");
    bytes32 public constant TAKATUF_MARKETPLACE_ROLE =
        keccak256("TAKATUF_MARKETPLACE_ROLE");
    ...
    modifier isTakatufAdmin() {
        if (
            ...
            !hasRole(TAKATUF_ADMIN_ROLE, msg.sender)
        ) revert GeneralError("NS:102");
        _;
    }

    modifier isTakatufAdminOrOwner() {
        if (
            !hasRole(DEFAULT_ADMIN_ROLE, msg.sender) &&
            !hasRole(TAKATUF_ADMIN_ROLE, msg.sender) &&
    ...
    modifier isTakatufMarketplace() {
        if (!hasRole(TAKATUF_MARKETPLACE_ROLE, msg.sender))
    ...
    //*********************** Admin Functions ***********************//
    /**
     * @notice Initializes the contract by setting 'name','symbol','baseURI','title','description','imageURI','coverImageURI' of collection and addresses of 'takatufAdmin','takatufMarketplace'.
     ...
     * @param takatufAdmin_ address of takatuf admin.
     * @param takatufMarketplace_ address of takatufMarketplace.
     */
    function initialize(
        uint256 version_,
        string memory name_,
        ...
        address owner_,
        address takatufAdmin_,
        address takatufMarketplace_
    ) external initializer {
        __ERC721_init(name_, symbol_);
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, takatufAdmin_);
        _setupRole(TAKATUF_ADMIN_ROLE, takatufAdmin_);
        _setupRole(COLLECTION_OWNER_ROLE, owner_);
        _setupRole(TAKATUF_MARKETPLACE_ROLE, takatufMarketplace_);
    ...
    function updateMetadata(
        ...
        string memory imageURI_,
        string memory coverImageURI_
    ) external isTakatufAdminOrOwner {
        _title = title_;
        _description = description_;
        _imageUri = imageURI_;
        _coverImageUri = coverImageURI_;
     ...
     * @dev only takatuf admin can change uri.
     * @param baseUri_ baseuri of token.
     */
    function setBaseURI(string memory baseUri_) external isTakatufAdmin {
        if (
...
import "./TakatufCollectionProxy.sol";
import "./TakatufCollection.sol";
...
contract CollectionFactory is Initializable {
    ...
    /** @notice Initializes the contract by setting the address of takatuf Marketplace.
     * @dev used instead of constructor.
     * @param takatufMarketplace_ address of takatuf Marketplace.
     */
    function initialize(address takatufMarketplace_) external initializer {
     ...
     * @param  takatufMarketplace_ address of takatuf Marketplace.
     */
    function updateMarketplaceaddress(address takatufMarketplace_)
        external
        onlyAdmin
    {
        _takatufMarketplace = takatufMarketplace_;
    ...
    function createNewCollection(
        ...
        TakatufCollectionV2 collectionV2 = new TakatufCollectionV2();
        TakatufProxy collectionProxy = new TakatufProxy(
            address(collectionV2),
            _admin,
            abi.encodeWithSelector(
                TakatufCollectionV2(address(0)).initialize.selector,
                _version,
                name_,
                symbol_,
                baseURI_,
 ...
 */
contract TakatufProxy is ERC1967Proxy {
    error GeneralError(string errorCode);
