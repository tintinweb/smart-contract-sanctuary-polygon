...
contract DecaCrush {
    ...
    constructor() {
        ...
        for(uint256 i=1;i<=10;i++){
            ...
            userList[currUserID[i]] = ownerWallet;
        }
    }
    ...
    function regUser(address _referer) public payable {
        ...
        require(_referrerID > 0 && _referrerID <= currUserID[1], 'Incorrect referrer Id');
        ...
        userList[currUserID[1]] = msg.sender;
        ...
        emit EarningsMatrix(_referer,LEVEL_PRICE[1]*40/100,3,1);
        payable (ownerWallet).transfer(LEVEL_PRICE[1]*10/100);
        emit Registration(msg.sender, userList[_referrerID],currUserID[1], block.timestamp);
    }
    ...
    function buyLevel(uint256 _slot) public payable {
        ...
        userList[currUserID[_slot]] = msg.sender;
       ...
       payable (ownerWallet).transfer(LEVEL_PRICE[_slot]*10/100);
         payable (userList[_referrerID]).transfer(LEVEL_PRICE[_slot]*40/100);

        emit EarningsMatrix(userList[_referrerID],LEVEL_PRICE[_slot]*40/100,3,_slot);
        emit Upgrade(msg.sender, userList[_referrerID],3,_slot);
    }
    ...
    function reinvest(address _user,uint256 _slot) internal {
        ...
        userList[currUserID[_slot]] = _user;
    ...
    function payForLevel(uint _level, address _referrer,uint256 _slot,address _user) internal {
        address referer=userList[users[_referrer][_slot].referrerID];
            ...
            if(!users[referer][_slot].isExist){
             ...
             for(uint256 i=_level;i<=5;i++){
                ...
                emit NewUserPlace(msg.sender, userList[users[_user][_slot].referrerID],referer, 3, _slot, _level,users[referer][_slot].cycle);
                if(_level<5){
