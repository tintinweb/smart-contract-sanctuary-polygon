...
contract ReferenceSeedModule is IReferenceModule, ModuleBase {
     ...
     * @param users The users to add seed rewards.
     * @param amounts The amounts of seed rewards.
     * @param pubIds The pubIds of seed rewards.
     */
    function addSeedRewards(address[] calldata users, SeedReward[] calldata amounts, uint256[] calldata pubIds)
        external
        onlyRelayer
    {
        require(users.length == amounts.length && users.length == pubIds.length, "length mismatch");
        for (uint256 i = 0; i < users.length; i++) {
            SeedReward storage reward = addressToReward[users[i]];
            reward.sun += amounts[i].sun;
            reward.rain += amounts[i].rain;
            reward.soil += amounts[i].soil;

            emit AddSeedReward(users[i], pubIds[i], reward.sun, reward.rain, reward.soil, block.timestamp);
        }
    }

    /**
     * @notice Allows a user to claim a specified amount from their reward.
     *  It checks that the user has enough sun, rain, and soil to claim the specified amount.
     *  It also ensures that the contract has enough of the specified currency to pay the claim.
     * @param amountToClaim_ The amount to claim.
     */
    function claim(uint256 amountToClaim_) external {
        SeedReward storage reward = addressToReward[msg.sender];
     ...
     */
    function getReward(address user_) external view returns (SeedReward memory) {
        return addressToReward[user_];
    }
     ...
     * @param _amount The amount of currency.
     */
