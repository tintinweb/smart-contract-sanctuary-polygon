                ...
                if (!mortgage.isMonthlyPaymentDelayed) {
                    //uint256 dailyInterestToPay = ((mortgage.totalDebt * interestrate) / 100000);
                     if(mortgage.strikes > 0){
                        uint256 dailyDelayedInterestToPay = (((mortgage.totalDebt * interestrate) * penalization) / 1000);
                ...
                if(!mortgageControl.getMortgageStatus(_userWallet[i], _idMortgage[i])){
                    ...
                    if (numdays > 1 days) {
                        ...
                        if (!mortgage.isMonthlyPaymentDelayed) {
                            ...
                            uint256 dayDelayedInterestToPay = (((mortgage.totalDebt * interestrate) * penalization) / 1000);
                            uint256 newAmountVault = mortgageControl.getCapitalPay(_userWallet[i],_idMortgage[i]);
                            mortgage.amountToVault += newAmountVault;
