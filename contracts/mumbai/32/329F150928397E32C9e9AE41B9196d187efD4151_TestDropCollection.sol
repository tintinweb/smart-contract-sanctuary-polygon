...
contract TestDropCollection is
    TestBaseCollection,
    ERC1155Upgradeable,
    OwnableUpgradeable,
    ITestDropCollection
{
    ...
    function uri(uint256 _id) public view override returns (string memory) {
        return string(abi.encodePacked(getBaseUri(), _id.toString()));
    }

    function getBaseUri() public view returns (string memory) {
        return ITestMasterCollection(_aiskinMasterCollection()).getBaseUri();
    }
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(ERC1155Upgradeable, TestBaseCollection)
        returns (bool)
    {
        return
            interfaceId == type(ITestDropCollection).interfaceId ||
            super.supportsInterface(interfaceId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (ids.length == 1) {
            ITestMasterCollection(_aiskinMasterCollection()).emitTransferSingle(
                    operator,
            ...
            ITestMasterCollection(_aiskinMasterCollection()).emitTransferBatch(
                operator,
                from, // from is always address(0) for new minted item
                to,
                ids,
    ...
    function _afterTokenTransfer(
        ...
        ITestMasterCollection(_aiskinMasterCollection()).emitLocked(ids[0]);
        // if (ids.length == 1) {
        //     ITestMasterCollection(_aiskinMasterCollection()).emitTransferSingle(
        //             operator,
        //             from, // from is always address(0) for new minted item
    ...
    function _aiskinMasterCollection() internal view returns (address) {
        return ITestFactory(aiskinFactory()).getAiskinMasterCollection();
    }
}