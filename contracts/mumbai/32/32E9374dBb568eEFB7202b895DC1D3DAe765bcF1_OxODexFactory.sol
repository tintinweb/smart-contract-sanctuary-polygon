...
contract OxODexFactory {
    ...
    constructor(address _managerAddress, address _treasurerAddress, address _token) {
...
contract OxODexPool is Pausable {
    ...
    uint private _lastWithdrawal = 0;
    ...
    constructor(address _factory, address payable _wethAddress) Pausable() {
    ...
    function deposit(uint _amount, uint256[4] memory _publicKey) external payable whenNotPaused pauseWhenCalled {
    ...
    ) public whenNotPaused pauseWhenCalled chargeForGas(relayerGasCharge)
    {
...
contract OxODexTokenPool is Pausable {
    ...
    /// @param _publicKey The public key of the participant
    function deposit(uint _amount, uint256[4] memory _publicKey) external whenNotPaused pauseWhenCalled {
        require(_amount > 0, "AMOUNT_MUST_BE_GREATER_THAN_ZERO");
    ...
    function withdraw(
        ...
        WithdrawalData memory withdrawalData,
        uint256 relayerFee
    ) public whenNotPaused pauseWhenCalled chargeForGas(relayerFee)
    {
        Ring storage ring = rings[withdrawalData.amount][withdrawalData.ringIndex];
