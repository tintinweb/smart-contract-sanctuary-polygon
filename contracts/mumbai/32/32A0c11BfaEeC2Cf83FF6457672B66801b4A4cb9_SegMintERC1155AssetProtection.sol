...
/**
 *Submitted for verification at testnet.snowtrace.io on 2023-03-20
*/
...
interface SegMintERC1155DBInterface {
    ...
    // check if global freeze status
    function isGlobalFreeze() external view returns (bool);

    // check if global freeze for TokenID status
    function isGlobalFreezeTokenID(uint256 TokenID_)
        external
        view
        returns (bool);

    // check if global freeze for address is on
    function isFreezeAccount(address account_) external view returns (bool);

    // check if freeze for address & for a specific TokenID is on
    function isfreezeAccountTokenID(address account_, uint256 TokenID_)
        external
        view
        returns (bool);
    ...
    function freezeGlobalTransactions(address sender_) external;

    // unfreeze global transactions
    function unFreezeGlobalTransactions(address sender_) external;

    // freeze global transaction for specific TokenID
    function freezeGlobalTransactionsSpecificTokenID(
        address sender_,
        uint256 TokenID_
    ) external;

    // unfreeze global transaction for specific TokenID
    function unFreezeGlobalTransactionsSpecificTokenID(
        address sender_,
        uint256 TokenID_
    ) external;

    // update freeze an account status
    function updateFreezeAccountStatus(address account_, bool status_)
        external
        returns (bool);

    // update freeze an account transactions for specific Token ID
    function updateFreezeAccountSpecificTokenIDStatus(
        address account_,
        uint256 TokenID_,
        bool status_
    ) external returns (bool);

    // function lock TokenID for an account
    function lockToken(
        uint256 TokenID_,
        address holder_,
        address locker_,
    ...
    // function unlock locked token balance
...
contract SegMintERC1155AssetProtection is SegMintERC1155AssetProtectionInterface {
        ...
        returns (bool)
    {
        ...
        require(
            SegMintERC1155DBInterface(_SegMintERC1155DBContractAddress)
                .isfreezeAccountTokenID(account_, TokenID_),
        ...
        returns (bool)
    {
        ...
        // freeze
        SegMintERC1155DBInterface(_SegMintERC1155DBContractAddress)
            .updateFreezeAccountSpecificTokenIDStatus(account_, TokenID_, true);
