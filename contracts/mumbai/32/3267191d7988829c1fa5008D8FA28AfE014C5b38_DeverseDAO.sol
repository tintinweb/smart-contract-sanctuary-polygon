...
contract DeverseDAO {
    ...
    event MembersAdded(uint256 daoId, address[] addedMembers);
    event MembersRemoved(uint256 daoId, address[] removedMembers);
    ...
    function addMemberToDAO(uint256 _daoId, address[] memory _membersToAdd)
        public
        onlyDAOAdmin(_daoId)
    {
        require(
            _membersToAdd.length <= 5,
            "Upto 5 members can be added at a time"
        );
        for (uint256 i = 0; i < _membersToAdd.length; i++) {
            isMemberofDAO[_daoId][_membersToAdd[i]] = true;
        }
        emit MembersAdded(_daoId, _membersToAdd);
    }

    function removeMemberFromDAO(
        uint256 _daoId,
        address[] memory _membersToRemove
    ) public onlyDAOAdmin(_daoId) {
        require(
            _membersToRemove.length <= 5,
            "Upto 5 members can be removed at a time"
        );
        for (uint256 i = 0; i < _membersToRemove.length; i++) {
            isMemberofDAO[_daoId][_membersToRemove[i]] = false;
        }
        emit MembersRemoved(_daoId, _membersToRemove);
    }
