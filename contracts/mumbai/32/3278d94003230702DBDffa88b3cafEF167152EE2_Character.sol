...
contract Character is ERC721Base, ERC721Sig {
    ...
    function mintWithSignature(
        ...
        require(_req.quantity == 1, "Unseen: quantiy must be 1");
    ...
    Royalty
{
    ...
    constructor(
        ...
        require(
            ...
            "Unseen: Sale recipient cannot be address 0"
        );
        require(_signer != address(0), "Unseen: Minimum 1 signer required");
        require(
            _royaltiesRecipient != address(0),
            "Unseen: Royalties recipient cannot be address 0"
        );
        require(_runToken != address(0), "Unseen: Token cannot be address 0");
        _setupDefaultRoyaltyInfo(_royaltiesRecipient, _royaltiesBps);
    ...
    function rent(uint256 tokenId, uint64 expires) public payable override {
        uint256 amount = super._rent(tokenId, expires);
        CurrencyTransferLib.transferCurrency(
            msg.value == amount ? CurrencyTransferLib.NATIVE_TOKEN : runToken,
            msg.sender,
            ownerOf(tokenId),
            amount
        );
    ...
    function _changeTreasuryWallet(address _treasuryWallet) external onlyOwner {
        require(
            treasuryWallet != _treasuryWallet,
            "Unseen: cannot be same as previous"
        );
        require(_treasuryWallet != address(0), "Unseen: cannot be address 0");
        treasuryWallet = _treasuryWallet;
    }

    function changeRentCurrency(address asset) external onlyOwner {
        require(runToken != asset, "Unseen: same as previous");
        runToken = asset;
    }
...
abstract contract ERC4907A is ERC721A, IERC4907A {
    ...
    function _rent(uint256 tokenId, uint64 expires)
        internal
        virtual
        returns (uint256 dueAmount)
    {
        require(userOf(tokenId) == address(0), "Rented");
    ...
    function setRentFee(uint256 tokenId, uint128 _ratePerMinute)
        public
        virtual
        override
        isAuthorized(tokenId)
    {
        rentablesInfo[tokenId].ratePerMinute = _ratePerMinute;
    }

    function setRentable(uint256 tokenId, bool rentable)
        public
        virtual
        override
        isAuthorized(tokenId)
    ...
    function getTokenRentInfo(uint256 tokenId)
        external
        view
...
interface IERC4907A is IERC721A {
    ...
    function rent(uint256 tokenId, uint64 _expires) external payable;
