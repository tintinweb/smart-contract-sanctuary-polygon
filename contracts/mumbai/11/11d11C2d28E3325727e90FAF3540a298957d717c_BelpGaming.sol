...
contract BelpGaming is Ownable, ReentrancyGuard {
    ...
    event DeleteGamerInfo(address indexed owner, address _to, Tier _tier);
     ...
     */
    function getGamerInfo(address to, Tier _tier) public view returns(address,bool,Tier) {
        uint256 gamerIndex = getGamerIndex(to);
     ...
     */
    function getGamerIndex(address _to) public view returns(uint256) {
    ...
    function updateGamerInfo(address _to, bool _hasReward, Tier _tier) external onlyOwner {
        ...
        uint256 gamerIndex = getGamerIndex(_to);
        require(gamerIndex != 9999, "gamer not found");
        GamerInfo memory gamerInfo;
    ...
    /**
     * @dev Function to delete gamer info.
     * @param _to is a address.
     * @param _tier is a tier type.
     */
    function deleteGamerInfo(address _to, Tier _tier) external onlyOwner {
        require(_to != address(0),"sender is zero address");
        uint256 gamerIndex = getGamerIndex(_to);
        require(gamerIndex != 9999, "gamer not found");
        ...
        emit DeleteGamerInfo(msg.sender, _to, _tier);
    }
     ...
     */
    function claim(Tier _tier) public nonReentrant {
        uint256 gamerIndex = getGamerIndex(_msgSender());
        require(gamerIndex != 9999, "gamer not found");
        require(checkValidReward(_msgSender(), _tier), "you dont have a reward");
