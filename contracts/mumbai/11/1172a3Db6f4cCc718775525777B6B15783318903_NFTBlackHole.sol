...
contract NFTBlackHole is IERC721Receiver, KeeperCompatibleInterface {
	...
	function debug(uint256 index) public view returns (address collection) {
		return nftList[index].collection;
	}

	function debug2() public view returns (uint256 length) {
		return nftList.length;
	}

	function debug3(uint256 index) public view returns (uint256 length) {
		return nftList[index].expiredBlock;
	}
