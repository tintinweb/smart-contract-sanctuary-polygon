...
contract DefaultPool is Ownable, CheckContract, IDefaultPool {
    ...
    */
    ...
    function sendROSEToActivePool(IaMATICToken amatic_Token, uint _amount) external override {
        _requireCallerIsVaultManager();
        address activePool = activePoolAddress; // cache to save an SLOAD
        emit DefaultPoolROSEBalanceUpdated_before(ROSE);
        ROSE -= _amount;
        ...
        if (_amount>0){
            bool sucess = amatic_Token.transfer(activePool, _amount);
            require ( sucess, "Default Pool sendROSEToActivePool failed");
            active_pool.receive_amatic(_amount);
        }
    }
    ...
    function receive_amatic_DP(uint new_coll) external override {
        _requireCallerIsVM();
        ROSE += new_coll;
        emit DefaultPoolROSEBalanceUpdated(ROSE);
    }

    function _requireCallerIsVM() internal view {
        require(
            msg.sender == vaultManagerAddress,
            "DefaultPool: Caller is neither BO nor Default Pool or VM");
    }
