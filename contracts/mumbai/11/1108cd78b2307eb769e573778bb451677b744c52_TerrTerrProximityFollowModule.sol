    ...
    FollowValidatorFollowModuleBase
{
    ...
    {
        (address tokenAddress, address followingAddress) = abi.decode(
            data,
            (address, address)
        );
    ...
    function _checkProximity(address _user, uint256 _profileId) private view {
        if (mapFollowByProfile[_profileId].tokenAddress != address(0)) {
            if (
                ITerrTerrMap(mapFollowByProfile[_profileId].tokenAddress)
