...
abstract contract PoolBase is PoolBaseInfo {
        ...
        returns (uint256)
    {
        ...
        // Base delta is difference between last borrows and provisional default utilization minimal borrows
        uint256 baseDelta = _poolSize(_info).mulDecimal(
            provisionalDefaultUtilization
        ) - _info.borrows;
        // Delta factor is factor on which base delta should be divided to consider reserves & insurance
        // Based on following formula:
        // (IFPD = InterestForProvisionalDefault, BFPD = BorrowsForProvisionalDefault)
        // borrows + IFPD + IFPD * (reserveFactor + insuranceFactor) = BFPD
        // baseDelta = IFPD * (1 + reserveFactor + insuranceFactor)
        // Interest for provisional default is actually accrued interest after which provisional default occures
        uint256 interestForProvisionalDefault = baseDelta.divDecimal(
