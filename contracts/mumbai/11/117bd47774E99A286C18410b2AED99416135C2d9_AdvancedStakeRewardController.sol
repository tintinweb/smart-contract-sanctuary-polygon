    ...
    IRewardAdviser
{
    ...
    // struct RewardingTimes {
    //     uint32 start;
    //     uint32 period;
    //     uint32 end;
    //     // $ZKP APY drop (scaled by 1e9) per second of period
    //     uint32 scZkpApyPerSecondDrop;
    // }
    ...
    /// @dev Emitted when rewarding times are updated
    event RewardingTimesUpdate(uint256 rewardingStart, uint256 rewardingEnd);
    ...
    constructor(
        ...
        address zkpToken,
        address nftToken,
        uint32 _rewardingStart,
        uint32 _rewardedPeriod
        ...
        require(
            uint256(_rewardingStart + _rewardedPeriod) > timeNow(),
            "ARC:E4"
        );
        rewardingStart = _rewardingStart;
        rewardedPeriod = _rewardedPeriod;
        rewardingEnd = rewardingStart + rewardedPeriod;

        scZkpApyPerSecondDrop = uint40(
            ((START_ZKP_APY - FINAL_ZKP_APY) * 1e9) / uint256(rewardedPeriod)
        );
    ...
    function getZkpApyAt(uint256 time) public view returns (uint256) {
        if (time < uint256(rewardingStart)) return 0;

        // overflow/underflow impossible due to uint32 input and `if` above
        unchecked {
            uint256 duration = time - uint256(rewardingStart);
            if (duration >= uint256(rewardedPeriod)) return FINAL_ZKP_APY;

            return START_ZKP_APY - (scZkpApyPerSecondDrop * duration) / 1e9;
        }
    }
    ...
    /// @notice Update the rewarding start, rewarded period, rewarding end, and, scaled amount of droped zkp per second
    function updateRewarding(uint32 _rewardingStart, uint32 _rewardedPeriod)
        external
        onlyOwner
    {
        uint32 _rewardingEnd = _rewardingStart + _rewardedPeriod;
        require(
            _rewardingStart > rewardingStart &&
                uint256(_rewardingEnd) > timeNow()
        );

        rewardingStart = _rewardingStart;
        rewardedPeriod = _rewardedPeriod;
        rewardingEnd = _rewardingEnd;
        ...
        emit RewardingTimesUpdate(
            uint256(_rewardingStart),
            uint256(_rewardingEnd)
        );
    }
    ...
    function rescueErc20(
        ...
        require(
            (token != ZKP_TOKEN) ||
                (block.timestamp >=
                    (uint256(rewardingStart) + ZKP_RESCUE_FORBIDDEN_PERIOD)),
    ...
    function _generateRewards(bytes memory message) internal {
        ...
        require(
            stakedAt >= uint256(rewardingStart),
            "ARC: unexpected stakedAt"
        );
    ...
    function _computeZkpReward(
        ...
        if (stakedAt > uint256(rewardingEnd)) return 0;
        uint256 rewardedTill = lockedTill > uint256(rewardingEnd)
            ? uint256(rewardingEnd)
            : lockedTill;
