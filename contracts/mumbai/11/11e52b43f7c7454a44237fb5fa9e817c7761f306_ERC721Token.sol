...
import {ERC721AQueryable} from "./ERC721A/ERC721AQueryable.sol";
import {ERC721A} from "./ERC721A/ERC721A.sol";
import {IERC721A} from "./ERC721A/IERC721A.sol";

// this is a normal 721A contract as a main collection
contract ERC721Token is ERC721AQueryable, Ownable, AccessControl {
    ...
    uint256 public allowlistSaleStartTime = 1648425600;
    uint256 public allowlistSaleEndTime = 1714271024;
    uint256 public publicSaleStartTime = 1648425600;
    uint256 public publicSaleEndTime = 1714271024;
    uint256 public mintLimitPerWallet = 2;

    /// @dev Only transfers to or from TRANSFER_ROLE holders are valid, when transfers are restricted.
    bytes32 public constant withdrawRole = keccak256("WITHDRAW_ROLE");
    /// @dev Only MINTER_ROLE holders can sign off on `MintRequest`s and lazy mint tokens.
    bytes32 public constant minterRole = keccak256("MINTER_ROLE");
    bytes32 public merkleRoot =
        0xdb4c59429a46a1e3350e99a3ca40e11edb876fc1f661154d20f643d89c6b27ac;
    ...
    constructor(
        ...
        // both of this will be the WMG wallet
        _setupRole(withdrawRole, msg.sender);
    ...
    function mint(uint256 amount) external payable {
        require(
            isSaleActive(publicSaleStartTime, publicSaleEndTime),
            "Sale is not active."
        );
        require(msg.value >= mintPrice * amount, "Not enough ETH sent");
        require(totalSupply() + amount <= maxSupply, "Not enough supply left.");
        require(
            mintedAmount[msg.sender] + amount <= mintLimitPerWallet,
    ...
    function allowlistMint(
        ...
        require(
            isSaleActive(allowlistSaleStartTime, allowlistSaleEndTime),
            "Sale is not active."
        );

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        bool isValidLeaf = MerkleProof.verify(proof, merkleRoot, leaf);
        require(isValidLeaf, "You are not on the allow list.");
    ...
    function withdraw() external onlyRole(withdrawRole) {
        require(vault != address(0), "Vault has not been set yet.");
        SafeTransferLib.safeTransferETH(vault, address(this).balance);
    }

    function setVault(address _vault) external onlyRole(withdrawRole) {
        vault = _vault;
    }
    ...
    function setAllowlistTime(
        uint256 _allowlistSaleStartTime,
        uint256 _allowlistSaleEndTime
    ) external onlyOwner {
        allowlistSaleStartTime = _allowlistSaleStartTime;
        allowlistSaleEndTime = _allowlistSaleEndTime;
    }

    function setPublicTime(
    ...
    function getMintedAmount(
        ...
        return mintedAmount[_userAddress];
    }
    ...
    /// @param _startTime is the uint256 unix timestamp from internal variables.
    /// @param _endTime is the uint256 unix timestamp from internal variables.
