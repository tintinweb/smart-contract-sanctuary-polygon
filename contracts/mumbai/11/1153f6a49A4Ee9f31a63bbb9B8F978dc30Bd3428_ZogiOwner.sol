...
contract ZogiOwner is Ownable{
    ...
    function transferFrom(address from, address to, uint256 amount) public onlyOwner returns (bool){
        require(!zogi.isBlackListed(from), "Owner can not transfer black listed funds");
        require(!zogi.isBlackListed(to), "Owner can not transfer black listed funds");
        return  zogi.transferFrom(from, to, amount);
    }

    function transferZogiOwnership(address newOwner) public onlyOwner{
        zogi.transferOwnership(newOwner);
    }
