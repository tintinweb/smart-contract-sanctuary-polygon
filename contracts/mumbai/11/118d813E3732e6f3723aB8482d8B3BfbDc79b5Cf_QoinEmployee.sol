...
contract QoinEmployee is ERC721Soulbound {
    ...
    modifier onlyChiefStaff() {
        require(
            _msgSender() == _chief,
            "Only Chief Staff can used this function"
        );
        _;
    }
    ...
    modifier onlyStaff() {
        require(
            _staff[_msgSender()] || _msgSender() == _chief,
            "Only Staff can used this function"
        );
        _;
    }
    ...
    function editData(uint256 noID, string memory data) public onlyStaff {
    ...
    function getDataByAddress(address user)
        public
        view
    ...
    function validate(bytes32 messagehash, bytes memory signature, uint256 _id)
        public
        view
        returns (bool isValid)
    {
        address user = _recoverPublicAddress(messagehash, signature);
        uint256 noID = _ownedNoID[user];

        if (noID > 0 && noID == _id) return true;
    ...
    function _recoverPublicAddress(bytes32 messagehash, bytes memory signature)
        internal
        pure
        ...
        )
    {
        ...
        if (isLatest) {
            startIndex =
                int256(_listNoID.length) -
                (_itemsPerPage * (page - 1)) -
                1;
