...
contract CollectionFactory is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function removeCollection(uint256 id) public whenNotPaused {
        // Check conditions
        require(id>0,"invalid collection Id");
        address collection = _collections[id];
        require(collection != address(0), "Collection doesn't exists");
        (uint256 index, bool exits) = findElementInArray(id, _userCollections[msg.sender]);
    ...
    function findElementInArray(uint id, uint256[] memory arr) private pure returns (uint256, bool) {
