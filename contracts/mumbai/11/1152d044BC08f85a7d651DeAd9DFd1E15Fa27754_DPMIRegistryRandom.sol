  ...
  VRFConsumerBase
{
  ...
  //uint128 constant uuidBitMask  = 0xff ff ff ff ff ff 0f ff 3f ff ff ff ff ff ff ff;
  //uint128 constant uuidFinalAdd = 0x00 00 00 00 00 00 40 00 80 00 00 00 00 00 00 00;
  uint128 constant uuidBitMask  = 0xffffffffffff0fff3fffffffffffffff;
  uint128 constant uuidFinalAdd = 0x00000000000040008000000000000000;
  ...
  mapping(bytes32 => uint128) mintRequestToRandomness;
  ...
  {
    require(LINK.balanceOf(address(this)) >= getLinkMintFee(), INSUFICIENT_FUNDS_FOR_MINTING);
    require(to != address(0), "ERC721: transfer to the zero address");
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    ...
    mintRequestToRandomness[requestId] = uint128(randomness);
  ...
  function mintFinish(bytes32 requestId) public returns (uint128 tokenId) {
    ...
    uint128 randomness = mintRequestToRandomness[requestId];
    
    tokenId = (randomness & uuidBitMask) | uuidFinalAdd;
    super.safeMint(to, tokenId, uri);
    
    delete mintRequestToUri[requestId];
    delete mintRequestToAddress[requestId];
    delete mintRequestToRandomness[requestId];
  }
