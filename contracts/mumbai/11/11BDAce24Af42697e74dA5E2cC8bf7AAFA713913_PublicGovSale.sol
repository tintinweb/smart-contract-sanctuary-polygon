...
contract PublicGovSale is Initializable, UUPSUpgradeable, PausableUpgradeable, OwnableUpgradeable, AdminProxyManager {
    ...
    function getRemoteData(uint256 _index) external view returns(
        address user,
        uint128 bought,
        uint128 received,
        uint128 fee
    ){
        if( _index >= getRemoteBuyersLength() ||
            !isRemoteUser[remoteUsers[_index]]
        ) return(address(0),0,0,0);
        
        user = remoteUsers[_index];
        bought = remoteUsersBought[_index];
        received = remoteUsersReceived[_index];
        fee = remoteUsersFee[_index];
    }
    ...
    function estimateFees() public view returns(uint256 fees){
        (fees,) = endpoint.estimateFees(dstChainId, factory.saleGateway(), getPayload(), false, getAdapterParams());
    }
    ...
    function finalizeRemote() external whenPaused {
        ...
        ISaleGateway(factory.saleGateway()).finalizeRemote(getPayload(), getAdapterParams(), estimateFees());
    }
