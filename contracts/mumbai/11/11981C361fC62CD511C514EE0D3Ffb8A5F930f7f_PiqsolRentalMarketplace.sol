...
contract PiqsolRentalMarketplace is  Initializable ,ERC721HolderUpgradeable ,OwnableUpgradeable,UUPSUpgradeable  {
    //** Tax Listing **//
    uint256 listingTax;
    uint256 buyitemTax;
    uint256 biditemTax;
...
// ************* Checks *****************//
     ...
     * List Nft to Rent 
     */
    function lendNFT_CollateralFee(
        address nftContract,
        uint256 tokenId,
        uint256 pricePerDay,
        uint256 collateralFee,
        ...
        require(IERC721Upgradeable(nftContract).ownerOf(tokenId) == msg.sender, "Piqsol: Not a nft owner");   
         require(
            LentActiveItems[nftContract][tokenId] == false,
            "Piqsol:Item is already up for Sale"
        );  
        require(pricePerDay > 0, "Piqsol:Rental price should be greater than 0");
        require(startDateUNIX >= block.timestamp, "Piqsol:Start date cannot be in the past");
        require(endDateUNIX >= startDateUNIX, "Piqsol:End date cannot be before the start date");
     ...
     *  Rent Nft
     */
    function rentNFT_CollateralFee(
        uint256 id,
        uint64 expires
    ) public payable 
       IsItemExists(id)
        ...
        require(block.timestamp > LentNFT_CollateralFeeForSale[id].expires, "Piqsol:NFT already rented");
        require(LentNFT_CollateralFeeForSale[id].expires <= LentNFT_CollateralFeeForSale[id].endDateUNIX, "Piqsol:Rental period exceeds max date rentable");
        ...
        require(msg.value >= totalfee, "Piqsol:Not enough ether to cover rental period");
        ...
        LentNFT_CollateralFeeForSale[id].expires = expires;
         IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract).safeTransferFrom(address(this),msg.sender,LentNFT_CollateralFeeForSale[id].tokenId);
         LentNFT_CollateralFeeForSale[id].isRented = true;
        emit RentedNFT_CollateralFee(
            id,
            IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract).ownerOf(LentNFT_CollateralFeeForSale[id].tokenId),
            msg.sender,
    ...
    function redeemFunds(uint256 id) public   IsItemExists(id){
         require(block.timestamp > LentNFT_CollateralFeeForSale[id].expires, "Piqsol:time is not ended yet");
         require(
            msg.sender == LentNFT_CollateralFeeForSale[id].owner,
            "Piqsol: Only Owner can call this method"
        );
          require(
             msg.sender != IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract).ownerOf(LentNFT_CollateralFeeForSale[id].tokenId),
            "Piqsol:can't Redeem"
        );
     ...
     */
      function claimCollateral(uint256 id) public  IsItemExists(id) {
           require(
            msg.sender == LentNFT_CollateralFeeForSale[id].renter,
            "Piqsol: Only Renter can claim"
        );
        require(block.timestamp > LentNFT_CollateralFeeForSale[id].expires, "Piqsol: time is not ended yet");
        require(
        ...
        IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract).safeTransferFrom(msg.sender,LentNFT_CollateralFeeForSale[id].owner,LentNFT_CollateralFeeForSale[id].tokenId);
     ...
     * unlist your rental
     */
     function cancelLending_CollateralFee(uint256 id)
        external
        IsItemExists(id)
        ItemIsForSale(id)
    {
        ...
        LentNFT_CollateralFeeForSale[id].isRented== true;
        LentActiveItems[LentNFT_CollateralFeeForSale[id].nftContract][
            LentNFT_CollateralFeeForSale[id].tokenId
        ] = false;
        IERC721Upgradeable(LentNFT_CollateralFeeForSale[id].nftContract)
            .safeTransferFrom(
                address(this),
                msg.sender,
                LentNFT_CollateralFeeForSale[id].tokenId
