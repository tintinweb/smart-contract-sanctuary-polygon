...
contract CowsGoneMad is ERC721Enumerable, Ownable, Admin {
  ...
  // public
  function mint(uint256 _mintAmount, address _buyerAddress) public payable {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    require(supply + _mintAmount <= maxSupply, "max NFT limit exceeded");
    _buyerAddress == address(0) ? _buyerAddress = msg.sender : _buyerAddress;

    if (_buyerAddress != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(_buyerAddress), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[_buyerAddress];
            require(ownerMintedCount + _mintAmount <= nftPerAddressLimit, "max NFT per address exceeded");
        }
        if (_buyerAddress == msg.sender) {
            require(msg.value >= cost * _mintAmount, "insufficient funds");
        }
    }

    for (uint256 i = 1; i <= _mintAmount; i++) {
      addressMintedBalance[_buyerAddress]++;
      _safeMint(_buyerAddress, supply + i);
    }
  }
  ...
  function setOnlyWhitelisted(bool _state) public onlyAdmin {
