	...
	ReentrancyGuardUpgradeable
{
	...
	mapping(uint256 => uint256) internal _amountRequest;
	mapping(uint256 => address) internal _whoIsRequest;
	mapping(uint256 => uint256[]) internal _weightRequest;
	mapping(uint256 => RedeemData) internal _redeemData;
	mapping(string => PolicyIdData) internal _policyIdData;
	mapping(uint256 => string) private _requestPolicyId;
	mapping(string => bool) private _existPolicies;
	mapping(address => string[]) internal _keepPolicyByUser;
	mapping(address => uint256[]) internal _keepRedeemIdByUser;
	mapping(string => uint256[]) internal _keepRedeemIdByPolicyId;
	 ...
	 */
	function whoIsRequest(uint256 _redeemId) public view returns (address) {
		return _whoIsRequest[_redeemId];
	}
	...
	{
		return _weightRequest[_redeemId];
	}
	 ...
	 */
	function amountRequest(uint256 _redeemId) public view returns (uint256) {
		return _amountRequest[_redeemId];
	}
		...
		returns (bool isActive)
	{
		if (
			...
			_policyIdData[_policyId].isRedeemed == false
		) {
			return true;
		} else {
			return false;
	...
	/**
	 *  @notice issuePolicy.
	 *  @param _issuePolicyParams The struct of IssuePolicyParams.
	 *
	 *  Requirements:
	 ...
	 */
	function issuePolicy(IssuePolicyParams memory _issuePolicyParams)
		public
		isReady
		onlyPolicyManager
	{
		...
		require(
			exist(_issuePolicyParams.policyId) != true,
			"This policyId exist"
		);
		require(
			_issuePolicyParams.pricing[2] ==
				IERC20(_issuePolicyParams.asset).decimals(),
			"Invalid Decimals of assets."
		);

		uint256 decimal = decimals();

		_existPolicies[_issuePolicyParams.policyId] = true;

		_policyIdData[_issuePolicyParams.policyId] = PolicyIdData(
			_issuePolicyParams.user,
			_issuePolicyParams.asset,
			_issuePolicyParams.referrer,
			_issuePolicyParams.policyId,
			_issuePolicyParams.orgSub,
			block.timestamp,
			block.timestamp +
				(1 hours * _issuePolicyParams.coveragePeriodHours),
			_issuePolicyParams.pricing[0] * 10**decimal,
			_issuePolicyParams.pricing[1] * 10**decimal,
			_issuePolicyParams.valueSub,
			_issuePolicyParams.percentSub,
			mempercent,
			false
		);

		_keepPolicyByUser[_issuePolicyParams.user].push(
			_issuePolicyParams.policyId
		);
		_keepPolicyPool.push(_issuePolicyParams.policyId);

		totalPolicies++;
		allPoliciesBeforeCalculate++;

		totalPoliciesValue += _issuePolicyParams.pricing[0] * 10**decimal;
		allPoliciesValueBeforeCalculate +=
			_issuePolicyParams.pricing[0] *
			10**decimal;

		totalPoliciesCoverageValue +=
			_issuePolicyParams.pricing[1] *
			10**decimal;
		...
		_keepPoliciesIndex[countPolicyIndex] = _issuePolicyParams.policyId;
		...
		emit BoughtPolicy(
			...
			_issuePolicyParams.policyId,
			_issuePolicyParams.asset,
			_issuePolicyParams.pricing,
			_issuePolicyParams.referrer,
			mempercent
		);

		if (_issuePolicyParams.valueSub > 0) {
			SUB.claimSubsidized(
				_issuePolicyParams.user,
				_issuePolicyParams.asset,
				_issuePolicyParams.pricing[0] *
					10**_issuePolicyParams.pricing[2],
				poolId
			);
	 ...
	 *  @param _policyId The policyId.
	 *  @param _redeemPercentage The percentage to redeem.
	 *
	 *  Requirements:
	 ...
	 *   - `_redeemPercentage` The uint256 of _redeemPercentage must greather than 0 and lessthan or equal to 100.
	 ...
	 */
	function checkRedeemPolicy(string memory _policyId, uint256 _redeemPercentage)
		internal
		view
		returns (bool, string memory)
	{
		...
		if (!(_redeemPercentage <= 100 && _redeemPercentage > 0)) {
	 ...
	 *  @notice redeemPolicy.
	 *  @param _policyId The policyId.
	 *  @param _redeemPercentage The percentage that will cash back to the user.
	 *
	 *  Requirements:
	 *   - `isReady` The pool must be ready.
	 *   - `msg.sender` Must be a PolicyManager address.
		...
		)
	{
		...
		_amountRequest[redeemId] =
			(_policyIdData[_policyId].buyValue * _redeemPercentage) /
			100;

		_whoIsRequest[redeemId] = _user;
		_weightRequest[redeemId] = _policyIdData[_policyId]
			.fundAllocationWeight;
		_requestPolicyId[redeemId] = _policyId;

		_redeemData[redeemId] = RedeemData(
			_user,
			redeemId,
			true,
			_amountRequest[redeemId],
			_weightRequest[redeemId],
			_user
		);

		_keepRedeemIdByUser[_user].push(redeemId);
		_keepRedeemIdByPolicyId[_policyId].push(redeemId);
	...
	function policyUser(address _user) public view returns (string[] memory) {
		return _keepPolicyByUser[_user];
	}

	/**
	 *  @notice Get redeemId array of the user in this pool.
	 *  @param _user The user address.
	 *  @return The redeemId array.
	 */
	function redeemIdUser(address _user)
		...
		returns (uint256[] memory)
	{
		return _keepRedeemIdByUser[_user];
	}

	/**
	 *  @notice Get redeemId array of the policyId in this pool.
	 *  @param _policyId The policyId.
	 *  @return The redeemId array.
	 */
	function redeemIdPolicy(string memory _policyId)
		...
		returns (uint256[] memory)
	{
		return _keepRedeemIdByPolicyId[_policyId];
	}
