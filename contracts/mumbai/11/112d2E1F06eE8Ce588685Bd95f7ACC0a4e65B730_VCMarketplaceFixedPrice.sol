...
contract VCMarketplaceFixedPrice is VCMarketplaceBase {
    ...
    function listFixedPrice(
        ...
        _setFees(_tokenId, msg.sender, _poolFeeBps, _projectIds, _projectFeesBps);
        if (!listed(_tokenId, msg.sender)) {
            _newList(_tokenId, _amount, _listPrice, marketFee);
            emit ListedFixedPrice(address(artNft), _tokenId, msg.sender, _amount, _listPrice, marketFee);
        } else {
            _updateList(_tokenId, _amount, _listPrice, marketFee);
            emit UpdatedFixedPrice(address(artNft), _tokenId, msg.sender, _amount, _listPrice, marketFee);
        }
    }
    ...
    function _newList(
        ...
        if (!minted) {
            artNft.requireCanRequestMint(msg.sender, _tokenId, _amount);
        } else {
            artNft.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");
        }
    ...
    function _updateList(
        ...
        if (!listing.minted) {
            artNft.requireCanRequestMint(msg.sender, _tokenId, listing.amount);
        } else {
            artNft.safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");
        }
    ...
    function unlistFixedPrice(uint256 _tokenId, uint256 _amount) public {
        ...
        if (listing.minted) {
            artNft.safeTransferFrom(address(this), listing.seller, _tokenId, _amount, "");
        }
    ...
    function purchase(uint256 _tokenId, address _seller) public whenNotPaused {
        ...
        if (!listing.minted) {
            artNft.mint(_tokenId, listing.amount);
            fixedPriceListings[_tokenId][_seller].minted = true;
        }
        artNft.safeTransferFrom(address(this), msg.sender, _tokenId, 1, "");

        pocNft.mint(msg.sender, listing.marketFee);
        pocNft.mint(listing.seller, starterFee + poolFee);

        emit Purchased(msg.sender, address(artNft), _tokenId, listing.seller, listing.price, listing.marketFee);
    }
...
abstract contract VCMarketplaceBase is FeeBeneficiary, Pausable, ERC1155Holder {
    ...
    function _chargeRoyalty(uint256 _tokenId, uint256 _amount) internal returns (uint256) {
        (address receiver, uint256 royaltyAmount) = IERC2981(address(artNft)).royaltyInfo(_tokenId, _amount);
        if (royaltyAmount > 0) {
...
interface IArtNft is IERC1155 {
    ...
    function setMarketplace(address _address) external;
...
contract FeeBeneficiary is AccessControl {
    ...
    function _setFees(
        ...
        _tokenFeesData[_tokenId][_seller] = TokenFeesData(_poolFeeBps, starterFeeBps, _projectIds, _projectFeesBps);
    }
