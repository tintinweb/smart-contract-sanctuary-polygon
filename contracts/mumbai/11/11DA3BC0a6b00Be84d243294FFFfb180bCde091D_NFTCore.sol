...
contract NFTCore is ERC1155, Pausable {
    ...
    function multiMint(uint256 series, uint256 amounts) whenNotPaused external payable {
    ...
    /// @param series specified series NFT to mint
    function mint(uint256 series) whenNotPaused external payable {
        require(capsuleContracts[series] != address(0), 'Have not set this series capsule contact');
    ...
    function exchange(uint256 series, uint256 pic) whenNotPaused external {
    ...
    function _burnExchanged(
        ...
        for (uint256 tokenId = _tokenId + 1; tokenId <= _tokenId + maxFragment; tokenId++) {
    ...
    function _addOwnedTokens(
        ...
        if (tokenIdToBool[tokenId][to]) {
            ...
            NFTInstance memory _NFTInstance = NFTInstance({tokenId: tokenId, count: amount});
