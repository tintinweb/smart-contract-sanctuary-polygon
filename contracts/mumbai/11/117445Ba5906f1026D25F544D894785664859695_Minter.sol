...
contract Minter is Ownable, ERC1155Holder {
    ...
    constructor(address venlyAddress, uint256 mintPrice, uint256 maxTokenPerAccount) {
        require(mintPrice > 0, "Mint price must be > 0");
        require(mintPrice > 0, "Max token per account must be > 0");
        venlyContract = venlyAddress;
        venlySc = VenlyERC1155Interface(venlyAddress);
        mintingPrice = mintPrice;
        maxMintPerAccount = maxTokenPerAccount;
    }
    ...
    function mintToken(bytes calldata signature, uint signedAt) external payable {
        require(signedAt >= block.timestamp - 60 minutes, "Obsolete signature");
        require(recoverSigner(msg.sender, signature, signedAt) == owner(), "Address is not whitelisted");
        require(paused == false, "Minting event is paused");
        require(msg.value >= mintingPrice, "Ether received must >= mint price");
    ...
    function totalTokenTypes() public view returns (uint)  {
        uint _total = 0;
        for (uint i = 0; i < tokenTypes.length; i = i + 1) {
            _total = _total + tokenTypes[i].maxSupply;
        }
        return _total;
    }
    ...
    // Withdraw all fund inside this contract to the receiver wallet address
