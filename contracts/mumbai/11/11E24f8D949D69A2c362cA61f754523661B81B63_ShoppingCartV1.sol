    ...
    DroppableUpgradeable
{
    ...
    modifier onlyOwnerOrAdmin() {
        require(_msgSender() == owner() || _isAdmin(), "Not authorized");
        _;
    }
    ...
    function _canSetMintingPhases() internal virtual override returns (bool) {
        return _msgSender() == owner() || _isAdmin();
    }
...
contract BacanoGoGiveawayV1 is BacanoGoCollection {
    ...
    function _isAdmin() internal view override returns (bool) {
        return contractWhitelist[_msgSender()];
    }

    // Droppable implementation
    function _checkOrigin() internal view override {
        bool isWhitelisted = contractWhitelist[_msgSender()];
        if (!isWhitelisted) {
    ...
    function tokenURI(
        ...
        if (token.cardType == CardType.HOLOFOIL) {
            nameSuffix = "Holofoil ";
        } else if (token.cardType == CardType.GOLD) {
            nameSuffix = "Gold ";
        }
    ...
    ERC2771ContextUpgradeable
{
    ...
    function setMintingPhases(
        DroppableUpgradeable.MintPhase[] calldata _phases,
        address[] memory _contracts
    ) external onlyOwner {
        for (uint256 i = 0; i < _contracts.length; i++) {
            require(isWhitelisted[_contracts[i]], "Not whitelisted");
            DroppableUpgradeable(_contracts[i]).setMintingPhases(_phases);
        }
    }

    function setPrice(
        uint256 _mintPhaseId,
        uint256 _pricePerToken,
        address[] memory _contracts
    ) external onlyOwner {
        for (uint256 i = 0; i < _contracts.length; i++) {
            require(isWhitelisted[_contracts[i]], "Not whitelisted");
            DroppableUpgradeable(_contracts[i]).setPrice(
                _mintPhaseId,
                _pricePerToken
            );
        }
    ...
    function setDefaultRoyaltyInfo(
        ...
        for (uint256 i = 0; i < _contracts.length; i++) {
            require(isWhitelisted[_contracts[i]], "Not whitelisted");
            BacanoGoCollection(_contracts[i]).setDefaultRoyaltyInfo(
                _royaltyRecipient,
                _royaltyBps
            );
        }
    ...
    function setPrimarySaleRecipient(
        ...
        for (uint256 i = 0; i < _contracts.length; i++) {
            require(isWhitelisted[_contracts[i]], "Not whitelisted");
            BacanoGoCollection(_contracts[i]).setPrimarySaleRecipient(
                _saleRecipient
            );
        }
    }
    ...
    function claim() external {
        require(giveawayContract != address(0), "Giveaway contract is not set");
        address claimer = _msgSender();
        BacanoGoGiveawayV1(giveawayContract).claim(
            claimer,
            1,
            new bytes32[](0)
        );
    ...
    function checkout(
        ...
        for (uint256 i = 0; i < cart.length; i++) {
            ...
            require(
                purchase.contractAddress != giveawayContract,
                "Invalid contract"
            );
        ...
        for (uint256 i = 0; i < claims.length; i++) {
            if (claims[i].amount == 0) {
                continue;
            }
            claims[i].droppable.claim{value: claims[i].value}(
                recipient,
                claims[i].amount,
                proof
            );
        ...
        if (freeClaims > 0 && giveawayContract != address(0)) {
            BacanoGoGiveawayV1 giveaway = BacanoGoGiveawayV1(giveawayContract);
