...
contract ERC721MCStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    uint256 public resourceEV;
    uint256 public resourceCap;
    int256 maxCutoff;
    ...
    function __update_cutoff() private {
        if (resourceEV > 0) {
            maxCutoff = int256(CUTOFF_COEF * (resourceCap / resourceEV) + CUTOFF_INTERCEPT);
        }
    }
    ...
    */
    ...
    function onCollect(
        ...
        (uint256 amount, uint256 id) = __checkTile(tokenId);
    ...
    function __checkTile(uint256 tokenId) internal view virtual returns (uint256, uint256) {
        if (resourceEV == 0) {
            return (0, resourceId);
        }
        int256 localSeed = int256(seeds[tokenId]);
        require(localSeed > 0, "MCS_ADAPTER: No such token staked");
        int256 blockTime = int256(block.timestamp);
        if (blockTime - localSeed >= maxCutoff) {
            return (resourceCap, resourceId);
        }
        uint256 ret = 0;
        int256 T = _getT(tokenId, localSeed);
    ...
    */
    ...
    function _getT(uint256 tokenId, int256 seed) internal view returns (int256 T) {
