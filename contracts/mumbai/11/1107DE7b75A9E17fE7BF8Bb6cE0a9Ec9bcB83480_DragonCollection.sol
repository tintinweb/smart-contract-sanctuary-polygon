 ...
 */
contract DragonCollection is Ownable {
   ...
   * @notice GameTreasury used in the game
   */
  AdditionalTreasury public additionaltreasury;


  uint256 public prize = 10000000000000000000; // 10 ETH
...
/**
 ...
 /**
   ...
   *      the reserve
   */
  constructor(address _card, address payable _daoAddress,  address payable _gameTreasury, address payable _additionalTreasury, address payable _partnerDAOAddress) payable {
    card = CardNFT(_card);
    gametreasury = GameTreasury(_gameTreasury);
    additionaltreasury = AdditionalTreasury(_additionalTreasury);
...
function mintCards (uint256 _amount) public payable {
    ...
    //treasury can be 0 if playe is in free Mint list 
...
function upgradeCardCost(uint256[] calldata _tokenIds, uint8 _targetCardType, uint16 _collectionId) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 cardsTypeValueDiffCost) {
    uint256 feeValue;
...
/**
   ...
   */
  function upgradeCards(uint256[] calldata _tokenIds, uint8 _targetCardType, uint16 _collectionId) public payable  validCardType(_targetCardType) {
   ...
   uint32 highestCardValue = 0;
    ...
    if(cardsTypeValueDiffCost > 0){
      if(gametreasury.balance() < cardsTypeValueDiffCost){
          uint256 cardsTypeValueDiffCostFromAdditionalTreasury = cardsTypeValueDiffCost - gametreasury.balance();
          additionaltreasury.giveUpgradeCacheBack(msg.sender, cardsTypeValueDiffCostFromAdditionalTreasury);
          cardsTypeValueDiffCost -= cardsTypeValueDiffCostFromAdditionalTreasury;
      }
      if(cardsTypeValueDiffCost > 0){
      ...
      if(gametreasury.balance() < upgradeCommission){
          uint256 upgradeCommissionFromAdditionalTreasury = upgradeCommission - gametreasury.balance();
          additionaltreasury.upgradeCommission(daoAddress, upgradeCommissionFromAdditionalTreasury);
          upgradeCommission -= upgradeCommissionFromAdditionalTreasury;
      }
...
function calculateBuyback(uint256[] calldata _tokenIds) public view returns (uint256 floorValue, uint256 bonusEarned, uint256 buyback) {
  ...
  function claimPrize (uint256[] calldata _tokenIds) public {

    (,,uint256 buyback) = calculateBuyback (_tokenIds);

  uint256  claimCommission = 0;

  if(buyback > 0){
    gametreasury.buyBackPayout(msg.sender, buyback);
    uint256 buybackFromAdditionalTreasury = 0;
      if(gametreasury.balance() < buyback){
          buybackFromAdditionalTreasury = buyback - gametreasury.balance();
          additionaltreasury.buyBackPayout(msg.sender, buybackFromAdditionalTreasury);
          buyback -= buybackFromAdditionalTreasury;
      }
      if(buyback > 0){
        gametreasury.buyBackPayout(msg.sender, buyback);
      }
      ...
      if(claimCommission > 0){
        if(gametreasury.balance() < claimCommission){
            uint256 claimCommissionFromAdditionalTreasury = claimCommission - gametreasury.balance();
            gametreasury.claimCommission(daoAddress, claimCommissionFromAdditionalTreasury);
            claimCommission -= claimCommissionFromAdditionalTreasury;
        }
        if(claimCommission > 0){
...
// ---------------------------------------------------------------------------
// Additional Treasury smart contract. 
// ---------------------------------------------------------------------------

contract AdditionalTreasury is AccessControl, Ownable{
