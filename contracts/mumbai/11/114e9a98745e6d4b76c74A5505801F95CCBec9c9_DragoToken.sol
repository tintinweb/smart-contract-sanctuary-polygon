...
contract DragoToken is Ownable, ERC20("Drago Token", "DRAGO") {
	using SafeMath for uint256;

    IDragos public dragosGenesisPartOneContract;
    IDragos public dragosGenesisPartTwoContract;

    IDragosBloodToken public dragosBloodTokenContract;
    ...
    uint256 public rewardRate = 1.6 ether;
    ...
    function claimTokensWithDragosBloodToken(uint256 amount) external
    {
        require(dragosBloodTokenContract.balanceOf(_msgSender()) >= amount);
        dragosBloodTokenContract.burn(_msgSender(), amount);
        _mint(_msgSender(), amount);
    }

    function claimTokensFromDungeon(uint256 amount, address to) external 
    {
        require(adminContracts[msg.sender]);
        _mint(to, amount);
    }

    function getDragosContractByTokenId(uint256 tokenId) internal view returns (IDragos) {
        if (tokenId <= 2297) {
            return dragosGenesisPartOneContract;
        } else {
            return dragosGenesisPartTwoContract;
        }
    }

    function stakeDragos(uint256[] memory dragosTokenId) external 
    {
        ...
        {
            IDragos dragosContract = getDragosContractByTokenId(dragosTokenId[i]);
    ...
    {
        return (((block.timestamp - dragosStakeDate[tokenId]) * rewardRate) / 1 days) / 2;
    }
    ...
    function setDragosPartOneContract(address _address) public onlyOwner {
        dragosGenesisPartOneContract = IDragos(_address);
    }

    function setDragosPartTwoContract(address _address) public onlyOwner {
        dragosGenesisPartTwoContract = IDragos(_address);
    }

    function setDragosBloodTokenContract(address _address) public onlyOwner {
        dragosBloodTokenContract = IDragosBloodToken(_address);
    }

    function setAdminContracts(address _address, bool isAdmin) public onlyOwner {
        adminContracts[_address] = isAdmin;
    }

	function burn(address _from, uint256 _amount) external {
		require(adminContracts[_msgSender()], "You are not approved to burn tokens");
		_burn(_from, _amount);
	}
}