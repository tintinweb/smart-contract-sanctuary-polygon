...
interface IBreedBabies {
  function safeMint(string memory _uri) external returns (uint256);
}

// FunctionsClient
contract Relationship is AutomationCompatibleInterface, VRFConsumerBaseV2 {
  using Functions for Functions.Request;
  using UintToString for uint256;

  struct CreateRelationshipParams {
    string sourceCode;
    IBreedBabies breedBabies;
  ...
  KeeperRegistrarInterface public constant REGISTRAR =
    KeeperRegistrarInterface(0xDb8e8e2ccb5C033938736aa89Fe4fa1eDfD15a1d);
  ...
  uint256 private frequency;
  IBreedBabies public breedBabies;

  // Chainlink Automation Variables
  uint256 private _upKeepId;
  ...
  event BabiesBred(bytes32 requestId, uint256 tokenId, string uri);
  event FunctionsError(bytes32 requestId, bytes error);

  constructor(
    // string memory sourceCode,
    IBreedBabies _breedBabies,
    address _relationshipRegistry
  ) VRFConsumerBaseV2(address(COORDINATOR)) {
    // FunctionsClient(ORACLE_ADDRESS)
    // source = sourceCode;
    breedBabies = _breedBabies;
  ...
  function testingSetter(bool set) public {
    condition = set;
  }

  // Initializer Function
  // onlyRelationshipRegistry
  function initialize(RegistrationParams memory registerAutomationParams) public {
    frequency = 1 days;
    latestTimestamp = block.timestamp;
    registerAutomationParams.upkeepContract = address(this);
    // Create VRF subscription
    _createNewSubscription();
    // Create UpKeep Subscription
    _registerAndPredictID(registerAutomationParams);
  }
  ...
  function _registerAndPredictID(RegistrationParams memory params) internal {
    LINK_TOKEN.approve(address(REGISTRAR), type(uint256).max);
    ...
    )
  {
    ...
    // upkeepNeeded = (block.timestamp - latestTimestamp) > frequency;
  }
  ...
  function performUpkeep(
    ...
    // if ((block.timestamp - latestTimestamp) > frequency) {
  ...
  function setFrequency(uint256 _frequencyInSeconds) public onlyRelationship {
    frequency = _frequencyInSeconds;
  }
  ...
  // function executeFunctionsRequest(
  ...
  //     uint256 tokenId = breedBabies.safeMint(metadataUri);
  //     emit BabiesBred(requestId, tokenId, metadataUri);
  //   } else {
  ...
  function getApprovedLinkAmount() public view returns (uint256) {
    return LINK_TOKEN.allowance(address(this), address(REGISTRAR));
  }
}
