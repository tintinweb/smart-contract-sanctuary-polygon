...
contract HelixSubs is
    ...
    event BillingSuccess( 
       bytes32[] successHashs
    );
...
//PUBLIC FUNCTIONS -----------------------------------------------------------------------------------
    ...
    function Billing(
        bytes32[100] calldata subsHash,
        uint256[100] calldata merchantValue,
        address[100] calldata merchantAddress,
        uint256[100] calldata hellixValue,
        address[100] calldata helixAddress,
        uint256[100] calldata creatorValue,
        address[100] calldata creatorAddress
    ) public returns (bool)
    {
        require(msg.sender == owner(),"Helix::Only owner can call billing function");
        bytes32[] memory successHashs;
        bytes32[] memory errorHashs;
        string[] memory errorReasons;
           ...
           if(subscription.exists)
           {
                if(subscription.nextDueTimestamp <= block.timestamp) {
                    ...
                    {                
                        ...
                        successHashs[i] = subsHash[i]; 
                    }
                    catch(bytes memory reason)
                    {
                        errorHashs[i] = subsHash[i];
                        errorReasons[i] = string(reason);
                    }
                }
                else
                {
                    errorHashs[i] = subsHash[i];
                    errorReasons[i] = "Future due date";
                }
            }
            else
            {
                errorHashs[i] = subsHash[i];
                errorReasons[i] = "Not found";
            }
        }

        emit BillingSuccess(successHashs);
        emit BillingError(errorHashs, errorReasons);
