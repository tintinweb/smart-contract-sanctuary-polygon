...
contract ffwc22lotto_router is ChainlinkClient {
    ...
    //current loading of seanson
    string private _SEASONID;
    //pending data from chainlink=>sportapi.com
    //team name in final
    ...
    // Top scorer
    uint8[] public _TOPSCORE = new uint8[](4);
    bytes32[] private _TOPSCOREReqId = new bytes32[](4);
    ...
    constructor() {
        ...
        _ticketPriceRaisePerDay = 500000000000000000; // raise 0.5 USD / Per day
    ...
    function buyTicket(
        ...
        require(
            !_isAnnounced,
            "FFWC22LTRouter: TICKETS_ARE_NOT_ON_SALE_AFTER_ANNOUNCING"
        );
        require(
            !_isTicketSold[number],
            "FFWC22LTRouter: THIS_TICKET_IS_SOLD_OUT"
        );
        ...
        //add ticket to this ticketTaker
        //check how many tickets this ticketTaker has
        uint32 curLength = _ticketHoldingLength[ticketTaker];
        //save ticket data for this ticketTaker
        ...
        returns (uint256 amounts)
    {
        require(
            ...
            "FFWC22LTRouter: THIS_TICKET_IS_CLAIMED"
        );
        ...
        returns (uint256 amounts)
    {
        require(
            _isAnnounced,
            "FFWC22LTRouter: DEV_CAN_CLAIM_ONLY_AFTER_ANNOUNCING"
        );
        ...
        returns (bool success)
    {
        ...
        //request first 4 top scorer #goal (Int)
        {
            ...
            req.addInt("times", 1);
            _TOPSCOREReqId[0]  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.addInt("times", 1);
            _TOPSCOREReqId[1]  = sendChainlinkRequest(req, LINK_fee);
            ...
            req.addInt("times", 1);
           _TOPSCOREReqId[2]  = sendChainlinkRequest(req, LINK_fee);
           ...
           //get SCORENO4
            ...
            req.addInt("times", 1);
           _TOPSCOREReqId[3]  = sendChainlinkRequest(req, LINK_fee);
        }
        ...
        recordChainlinkFulfillment(requestId)
    {
        ...
        } else if (requestId == _TOPSCOREReqId[0] ) {
            _TOPSCORE[0] = uint8(response);
        } else if (requestId == _TOPSCOREReqId[1] ) {
            _TOPSCORE[1] = uint8(response);
        } else if (requestId == _TOPSCOREReqId[2] ) {
            _TOPSCORE[2] = uint8(response);
        } else if (requestId == _TOPSCOREReqId[3] ) {
            _TOPSCORE[3] = uint8(response);
        }
    ...
    function updateIfFullyFulfill() private {
        ...
        uint256 SQUAREMULSCORE = uint256 ( _TOPSCORE[0] * _TOPSCORE[1] * _TOPSCORE[2] * _TOPSCORE[3] );
        ...
        if(
            ...
            uint8 SUMSCORE = uint8 ( _TOPSCORE[0] + _TOPSCORE[1] + _TOPSCORE[2] + _TOPSCORE[3] );
