...
struct WithdrawalInfo {
  uint256 withdrawPercentage1;
  uint256 withdrawPercentage2;
  address withdrawAddress1;
  address withdrawAddress2;
}

contract AlphaPharaohs is DefaultOperatorFilterer, ERC721AQueryable, ERC2981, BaseTokenURI {
  using Strings for uint256;
  using ECDSA for bytes32;

  TokenInfo public tokenInfo;
  WithdrawalInfo public withdrawalInfo;
  ...
  function mintX(address _to, uint256 _mintAmount) private {
    ...
    bool isFreeToMint = _to == owner() || _msgSender() == owner();
    if (!isFreeToMint) {
      require(msg.value >= totalCost, "Ether sent is less than minting cost");
      ...
      uint256 transferAmount1 = (totalCost * withdrawalInfo.withdrawPercentage1) / 10000;
      uint256 transferAmount2 = totalCost - transferAmount1;

      if (transferAmount1 > 0) {
        payable(withdrawalInfo.withdrawAddress1).transfer(transferAmount1);
      }
      if (transferAmount2 > 0) {
        payable(withdrawalInfo.withdrawAddress2).transfer(transferAmount2);
      }
    }

    if (isFreeToMint && msg.value != 0) {
      (bool success, ) = address(_msgSender()).call{value: msg.value}("");
      require(success, "Unable to refund excess Ether");
    }
  ...
  function setWithdrawalInfo(
    ...
    require(percentage1 + percentage2 == 10000, "Percentage must be 100");
    withdrawalInfo.withdrawAddress1 = wallet1;
    withdrawalInfo.withdrawAddress2 = wallet2;
    withdrawalInfo.withdrawPercentage1 = percentage1;
    withdrawalInfo.withdrawPercentage2 = percentage2;
  }
