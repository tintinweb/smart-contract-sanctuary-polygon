...
contract YETI2 is ERC1155, Ownable {

    YetiTown public yeti;
    FRXST public frxst;

    uint8 public RedHammer = 1;
    uint8 public GreenHammer = 2;

    mapping(uint => bool) public isTokenPresent;

    constructor (address _frxst, address _yetiTown, string memory tokenURI) ERC1155(tokenURI) {
        frxst = FRXST(_frxst);
        yeti = YetiTown(_yetiTown);
        isTokenPresent[RedHammer] = true;
        isTokenPresent[GreenHammer] = true;
    }

    function mintNewToken(uint tokenId) public onlyOwner{
    ...
    function buyNFTs(uint _amount, uint _tokenId) public {
        require(frxst.balanceOf(_msgSender()) >= 10, "Not enough FRXST");
        require(isTokenPresent[_tokenId], "Token does not exist");
        frxst.burn(_msgSender(), _amount*10 ether);
        mintTokens(_tokenId, _amount);
    }
