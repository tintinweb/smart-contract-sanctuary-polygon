...
contract TestnetWhitelist is Managable {
    address public bot;
    address public botMetadata;
    mapping(address => bool) public claimed;

    constructor (
        address _bot,
        address _botMetadata
    ) {
        bot = _bot;
    ...
    function isClaimed(address _owner) public view returns(bool) {
        return claimed[_owner];
    }

    function mint(address _owner, uint256[] calldata _bots) external onlyManager {
        require(!isClaimed(_owner), "already claimed");

        claimed[_owner] = true;
        for(uint256 i = 0;i < _bots.length; i++) {
            _mint(_owner, _bots[i]);
        }
    }

    function _mint(address _to, uint256 _dna) internal onlyManager {
        LibBot.Bot memory _b = _blankBot(_dna);

        uint256 _tokenId = IBot(bot).mint(_to);
        _b.id = _tokenId;
        IBotMetadata(botMetadata).setBot(_tokenId, _b);
    }    

    function _blankBot(uint256 _dna) internal view returns(LibBot.Bot memory) {
        LibBot.Bot memory _b;
        _b.genes = _dna;
        _b.generation = 0;
        _b.breedCount = 0;
        _b.birthTime = block.timestamp;
        _b.lastBreed = 0;
        _b.matronId = 0;
        _b.sireId = 0;
        _b.revealCooldown = 0;
