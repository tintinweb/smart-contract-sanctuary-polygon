...
contract Staker is IStaker, AccessControlledAndUpgradeable {
  ...
  mapping(address => mapping(address => uint256)) public userAmountStakedWithoutPendingActions;
  ...
  */
  ...
  function _changeBalanceIncentiveParameters(
    ...
    (uint128 lockedLong, uint128 lockedShort) = ILongShortOriginal(longShort)
      .marketSideValueInPaymentToken(marketIndex);
  ...
  */
    ...
    returns (uint256 floatReward)
  {
    ...
    uint256 amountStakedLong = userAmountStakedWithoutPendingActions[longToken][user];
    uint256 amountStakedShort = userAmountStakedWithoutPendingActions[shortToken][user];
    ...
    if (usersNextPriceActionIndex > 0 && usersNextPriceActionIndex <= currentRewardIndex) {
      ...
      userAmountStakedWithoutPendingActions[longToken][user] = amountStakedLong;
      userAmountStakedWithoutPendingActions[shortToken][user] = amountStakedShort;
  ...
  function claimFloatCustom(uint32[] calldata marketIndexes) external gemCollecting(msg.sender) {
    ILongShortOriginal(longShort).updateSystemStateMulti(marketIndexes);
  ...
  */
    ...
    gemCollecting(from)
  {
    ...
    ILongShortOriginal(longShort).updateSystemState(marketIndex);
    ...
    userAmountStakedWithoutPendingActions[msg.sender][from] += amount;
  ...
  */
    ...
    gemCollecting(msg.sender)
  {
    ...
    ILongShortOriginal(longShort).checkIfUserIsEligibleToTrade(
      msg.sender,
      marketIndex,
      isShiftFromLong
    );
    ILongShortOriginal(longShort).setUserTradeTimer(msg.sender, marketIndex, !isShiftFromLong);
    ...
    require(
      userAmountStakedWithoutPendingActions[token][msg.sender] >= totalAmountForNextShift,
      "Not enough tokens to shift"
    );

    ILongShortOriginal(longShort).shiftPositionNextPrice(
      marketIndex,
  ...
  */
  ...
  function _withdrawPrepLogic(
    ...
    uint256 currentAmountStaked = userAmountStakedWithoutPendingActions[token][msg.sender];
    ...
    unchecked {
      ...
      userAmountStakedWithoutPendingActions[token][msg.sender] = currentAmountStaked - amount;
    }
  }
  ...
  */
  function withdrawAll(uint32 marketIndex, bool isWithdrawFromLong) external {
    ...
    uint256 userAmountStakedBeforeWithdrawal = userAmountStakedWithoutPendingActions[token][
      msg.sender
    ];
    ...
    userAmountStakedWithoutPendingActions[token][msg.sender] = amountToShiftForThisToken;
  ...
  function upgradeToUsingCompactStakerState() public {
    ...
    if (accumulativeFloatPerSyntheticTokenSnapshots[1][0].timestamp == 0) {
      uint256 latestMarket = ILongShortOriginal(longShort).latestMarket();
  ...
  function userAmountStaked(address token, address user)
    public
    view
    returns (uint256 amountStaked)
  {
    amountStaked = userAmountStakedWithoutPendingActions[token][user];
