...
import "./interfaces/IMoDVault.sol";

contract MintOfDestiny is Pausable, ReentrancyGuard {
    ...
    */
    ...
    // Contract address for MoD Vault
    IMoDVault private modVault;

    struct role {
        uint totalTokensToMint;
        uint totalTokensAwarded;
        uint tokenStartID;
        uint tokenEndID;
    ...
    mapping (uint => string) tokensAwardedToRoleMap;
    ...
    address public modVaultAddress;


    constructor(IERC20 _paymentToken, address _mod, address _modVault){
        ...
        // Update the MoD Vault contract address
        modVault = IMoDVault(_modVault);
        admins[_modVault] = true;
        modVaultAddress = _modVault;

        // Setup the default Admin
        admins[msg.sender] = true;

        // Setup this contract to interact with MoD
        mod.updateMoDContract(address(this));

        // Setup this contract to interact with MoD Vault
        modVault.updateMoDContract(address(this));
    }
    ...
    */
    ...
    function addRole(string memory roleName, string memory baseURI, uint tokenLimit, bool allowPublicMint, uint price, uint priceERC20, uint priceWETH, string memory mandatoryRole, bool hasAttachedMint, string memory attachedMintRole) public onlyAdmins onlyHumans whenNotPaused {
        ...
        if (roleNames.length > 2){
            ...
            mint(modVaultAddress, roleName, true);
        }
    }

    function updateRoleDetails(string memory roleName, bool allowPublicMint, uint price, uint priceERC20, uint priceWETH, string calldata mandatoryRole, bool hasAttachedMint, string calldata attachedMintRole) public onlyAdmins {
    ...
    function updateRoleOwnership(uint tokenId, address addr, bool flag) external onlyAdmins {
    ...
    function updateContracts(address _mod, address _modVault) public onlyAdmins onlyHumans {
        // Update the MoD Token contract address
        mod = IMoD(_mod);
        admins[_mod] = true;
    ...
    */
    ...
    function mint(address sendTo, string memory roleName, bool addToVault) private whenNotPaused requireContractsSet {
        ...
        uint tokenID = roles[roleName].tokenStartID + roles[roleName].totalTokensAwarded;
        ...
        if (addToVault){
            modVault.addToVault(tokenID);
        }

        // Update the baseURI of this token to the token contract
        mod.setBaseURI(roles[roleName].baseURI, tokenID);

        if (stringsEqual(roleName, "Boss")) {
        ...
        // Only perform attached mints if not initial mints to the vault (one is vaulted by default on role create)
        if (roles[roleName].hasAttachedMint == true && tokensRemainingForRole(roleName) > 0 && sendTo != modVaultAddress){
    ...
    */
    ...
    function getCostOfTokenByRole(string memory roleName) public view returns(uint[] memory){
    ...
    function tokensRemainingForRole(string memory roleName) public view returns(uint){
    ...
    function getLevel(address player) public view returns(uint){
        return levels[player];
    }

    function tokenLimitForRole(string memory roleName) public view returns (uint){
        return roles[roleName].totalTokensToMint;
    }

    function totalTokensMintedInRole(string memory roleName) public view returns (uint){
        return roles[roleName].totalTokensAwarded;
    }

    function getRoleFromTokenID(uint tokenID) public view returns(string memory){
        return tokensAwardedToRoleMap[tokenID];
    }

    function getBaseURI(uint tokenID) public view returns(string memory){
        return roles[tokensAwardedToRoleMap[tokenID]].baseURI;
    }
    ...
    function getNextTokenID(string calldata roleName) public view returns(uint){
        return roles[roleName].tokenStartID + roles[roleName].totalTokensAwarded;
    }
    ...
    function modSendValue(address payable recipient, uint amount) internal {
...
interface IMoDVault {
    ...
    // Adds a token to the vault
    function addToVault(uint256 tokenID) external;
