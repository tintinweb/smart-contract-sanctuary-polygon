    ...
    ReentrancyGuard
{
    ...
    constructor(address _userContract) ERC1155("https://example/api/item/") {
        userContract = User(_userContract);
        _baseURI = uri(1);
    }

    mapping(uint256 => address) TokenOwner;
    mapping(address => bool) public MintCopiesAccess;
    mapping(uint256 => string) private _tokenURIs;

    function updateUserContractAddress(address _newContractAddress)
        public
        onlyOwner
        returns (bool)
    {
        userContract = User(_newContractAddress);
        return true;
    }
    ...
    function _setURIBatch(uint256[] memory tokenIds, string[] memory tokenURIs)
        internal
        virtual
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            _tokenURIs[tokenIds[i]] = tokenURIs[i];
            emit URI(uri(tokenIds[i]), tokenIds[i]);
        }
    }
    ...
    function addMintCopiesAccess(address walletAddress) public onlyOwner {
        MintCopiesAccess[walletAddress] = true;
    }

    function removeMintCopiesAccess(address walletAddress) public onlyOwner {
        MintCopiesAccess[walletAddress] = false;
    }
    ...
    function updateTokenOwnerBatch(
        ...
        require(
            MintCopiesAccess[msg.sender],
            "You do have access to call this function"
        );
        for (uint256 i = 0; i < _newOwners.length; i++) {
            TokenOwner[_tokenIds[i]] = _newOwners[i];
        }
    }
    ...
    function mint(string memory _tokenURI) public nonReentrant {
        ...
        require(
            userContract.getUserKYCLevel(msg.sender) == 3,
            "User doesn't have access to mint NFT"
        );
        _tokenIdCounter.increment();
        TokenOwner[_tokenIdCounter.current()] = msg.sender;
        _mint(msg.sender, _tokenIdCounter.current(), 1, "");
        _setURI(_tokenIdCounter.current(), _tokenURI);
    }

    function mintBatch(uint256 _totalNFT, string[] memory _tokenURI)
        public
        nonReentrant
    {
        require(
        ...
        uint256[] memory tempIds = new uint256[](_totalNFT);
        uint256[] memory NFTamounts = new uint256[](_totalNFT);
        for (uint256 i = 0; i < _totalNFT; i++) {
            _tokenIdCounter.increment();
            TokenOwner[_tokenIdCounter.current()] = msg.sender;
            tempIds[i] = _tokenIdCounter.current();
            NFTamounts[i] = 1;
        }
        _mintBatch(msg.sender, tempIds, NFTamounts, "");
        _setURIBatch(tempIds, _tokenURI);
    }
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            TokenOwner[_ids[i]] = _to;
        }
        _safeBatchTransferFrom(_from, _to, _ids, _amounts, data);
    }
    ...
    function mintCopiesBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                ((exists(ids[i])) || (ids[i] == 0)),
                "Token Id doesn't exists!"
            );
        }
        require(
            MintCopiesAccess[msg.sender],
            "Caller doesn't have access to mint copies."
        );
        _mintBatch(_buyerAddress, ids, amounts, "");
    }
...
contract Marketplace is Ownable, ReentrancyGuard, ERC1155Holder {
    EnvisionToken public nftContract;
    User public userContract;
    ...
    struct NFTSale {
        ...
        uint8 status; //1-new, 2-completed
    }
    ...
    constructor(
        ...
        nftContract = EnvisionToken(_nftContractAddress);
        tokenContract = IERC20(_tokenContractAddress);
        userContract = User(_userContractAddress);
        minPrice = 1 * (10**18);
        maxPrice = 10000 * (10**18);
        txnFee = 5;
    }

    function updateTokenAddress(address _newTokenContractAddress)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function updateNFTContractAddress(address _newContractAddress)
        public
        onlyOwner
        returns (bool)
    {
        nftContract = EnvisionToken(_newContractAddress);
        return true;
    }
    ...
    function updateFeeCollectionWallet(address _newAddress)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function updatPriceConstaraints(uint256 _minPrice, uint256 _maxPrice)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function markNFTForSale(
        ...
        require(
            ((_itemPrice >= minPrice) && (_itemPrice <= maxPrice)),
        ...
        require(
            nftContract.tokenOwner(_tokenId) == msg.sender,
        ...
        if (_saleType == 1) {
            ...
            NFTSale memory currentSale = NFTSale(
                ...
                _tokenId,
                _itemPrice,
                _itemPrice + ((_itemPrice * txnFee) / 100),
                block.timestamp,
                msg.sender,
                address(0),
                1,
    ...
    function buyNFT(uint8[] memory _saleIds, uint8[] memory _saleTypes)
        public
        nonReentrant
    {
        require(
        ...
        require(
            _saleIds.length == _saleTypes.length,
            "Wrong sale Ids and sale type counts"
        );
        uint256[] memory exclusiveIds = new uint256[](_saleIds.length);
        uint256[] memory licenseIds = new uint256[](_saleIds.length);
        uint256[] memory exclusiveAmounts = new uint256[](_saleIds.length);
        uint256[] memory licenseAmounts = new uint256[](_saleIds.length);
        NFTSale[] memory currentSale = new NFTSale[](_saleIds.length);
        uint256 exclusiveCounter = 0;
        uint256 licenseCounter = 0;

        for (uint256 i = 0; i < _saleIds.length; i++) {
            require(
                allSales[_saleIds[i]].status == 1,
                "Current sale is not available for some saleId"
            );
            require(
                (allSales[_saleIds[i]].saleType == 1) ||
                    (allSales[_saleIds[i]].saleType == 2),
                "Some NFT is not available for Exclusive or license sale type"
            );
            currentSale[i] = allSales[_saleIds[i]];
            if (_saleTypes[i] == 1) {
                exclusiveIds[exclusiveCounter] = currentSale[i].tokenId;
                exclusiveAmounts[exclusiveCounter] = 1;
                exclusiveCounter++;
            } else {
        ...
        if (exclusiveIds.length > 0) {
            require(
                (userContract.getUserKYCLevel(msg.sender) == 2) ||
                    (userContract.getUserKYCLevel(msg.sender) == 3),
                "Not eligible for purchase"
            );
        } else if (licenseIds.length > 0) {
            require(
                (userContract.getUserKYCLevel(msg.sender) == 1) ||
                    (userContract.getUserKYCLevel(msg.sender) == 2) ||
                    (userContract.getUserKYCLevel(msg.sender) == 3),
                "Not eligible for purchase"
            );
        ...
        if (userContract.getUserStakingStatus(msg.sender))
            for (uint256 i = 0; i < _saleIds.length; i++) {
                {
                    saleConsiderationPrice += currentSale[i].itemPrice;
                    itemTotalPrice += currentSale[i].itemPrice;
                }
            }
        else
            for (uint256 i = 0; i < _saleIds.length; i++) {
        ...
        uint256 contractFee = saleConsiderationPrice - itemTotalPrice;
        ...
        for (uint256 i = 0; i < _saleIds.length; i++) {
            sellerBalance[currentSale[i].sellerAddress] += currentSale[i]
                .itemPrice;
        ...
        emit NFTSold(_saleIds, saleConsiderationPrice, msg.sender);
    }

    function withdrawSellerBalance() public nonReentrant {
        require(sellerBalance[msg.sender] > 0, "Zero balance for the caller");
        tokenContract.safeTransfer(msg.sender, sellerBalance[msg.sender]);
        // require(tokenContract.transfer(msg.sender, sellerBalance[msg.sender]), "Some Error");
        emit SellerWithdraws(
            sellerBalance[msg.sender],
            block.timestamp,
            msg.sender
        );
    ...
    function getAllUserBuys(address _userAddress)
        public
        view
    ...
    function getAllUserSells(address _userAddress)
        public
        view
        returns (uint256[] memory saleIds)
    {
...
contract User is Ownable {
    ...
    modifier atleastJrAdmin() {
        require(juniorAdmins[msg.sender] || owner() == msg.sender, "Need atleast jr admin status");
        _;
    }

    // Modifier to require caller to have access to contract methods
    modifier needAccess() {
        require(
            contractAccessAddresses[msg.sender] || owner() == msg.sender,
            "Need access to this contract get methods"
        );
        _;
    }
    ...
    modifier KYCAdmin() {
        require(
            kycAdmins[msg.sender] || juniorAdmins[msg.sender] || owner() == msg.sender,
    ...
    function updateJuniorAdminStatus(address _jrAdminAddress, bool _status)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function updateKycAdminStatus(address _address, bool status)
        public
        onlyOwner
        returns (bool)
    {
    ...
    function blackListUser(address _userAddress)
        public
        atleastJrAdmin
        returns (bool)
    {
    ...
    function removeBlackListedUser(address _userAddress)
        public
        atleastJrAdmin
        returns (bool)
    {
    ...
    function updateUserKYCLevel(address _userAddress, uint8 _kycLevel)
        public
        KYCAdmin
        returns (bool)
    {
    ...
    function whitelistCreator(address _userAddress)
        public
        atleastJrAdmin
        returns (bool)
    {
    ...
    function addStakingUser(address _userAddress)
        public
        atleastJrAdmin
        returns (bool)
    {
    ...
    function removeStakingUser(address _userAddress)
        public
        atleastJrAdmin
        returns (bool)
    {
    ...
    // This function allows the owner of the contract to update the contract access status of a user
    // The owner can add or remove a user's access to the contract
    function updateAccessStatus(address _address, bool _status)
        public
        onlyOwner
        returns (bool)
    {
        // Update the contract access status for the specified address
        contractAccessAddresses[_address] = _status;
    ...
    function getUserBlackListStatus(address _userAddress)
        public
        view
        needAccess
        returns (bool)
    ...
    function getUserStakingStatus(address _userAddress)
        public
        view
        needAccess
        returns (bool)
    ...
    function getjrAdminStatus(address _jrAdminAddress)
        public
        view
        needAccess
        returns (bool)
    ...
    function getKycAdminStatus(address _address)
        public
        view
        needAccess
        returns (bool)
    ...
    function getUserKYCLevel(address _userAddress)
        public
        view
        needAccess
        returns (uint8)
    ...
    function getContractAccessStatus(address _userAddress)
        public
        view
        needAccess
        returns (bool)
    {
        // This function returns the contract access status of a user
        // If the user has access to the contract, it will return true
        // If the user does not have access to the contract, it will return false
        return contractAccessAddresses[_userAddress];
    }
}