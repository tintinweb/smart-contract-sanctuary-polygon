...
contract Claim {
    ...
    function claimTokens() public {
        uint nftCount = IERC721(nftContractAddress).balanceOf(msg.sender);
        require(nftCount > 0, "You do not have NFTs to claim token");

        uint totalClaimAmount = 0;
        for (uint i = 0; i < nftCount; i++) {
            uint tokenId = IERC721Enumerable(nftContractAddress).tokenOfOwnerByIndex(msg.sender, i);
    ...
    function getTokensClaimedByTokenId(uint tokenId) public view returns (uint) {
        require(tokenId < IERC721Enumerable(nftContractAddress).totalSupply(), "Invalid TokenId");
        return tokensClaimedByTokenId[tokenId];
    }

    function getTokensClaimedByAddress(address wallet) public view returns (uint) {
