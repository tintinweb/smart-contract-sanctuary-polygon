...
contract DiceGame is Initializable, RandomNumber {
    ...
    function playGame(
        ...
        processInfluencerReferral(influencer, betAmount, address(token));
    ...
    function processInfluencerReferral(
        ...
        address referredBy = userReferredBy[msg.sender];
        if (referredBy == address(0)) {
            ...
            require(
                ...
                "DiceGame: influencer cannot change"
            );
            influencerData[influencer].totalBetsByToken[token] += betAmount;
            //
        }
    ...
    {
        require(
            getAvailablePrize(_token) >= _amount,
    ...
    /// @return parsedNumber raw number formated to be in the range from 1 to 10000
...
contract RandomNumber is ReferralSystem, VRFConsumerBaseV2Upgradeable {
        ...
        override
    {
        ...
        address influencer = userReferredBy[userGuess.user];
        // If the user guessed incorrectly then stop
        if (
            userGuess.lowerNumber > winningNumber ||
            userGuess.upperNumber < winningNumber
        ) {
            // Increasing user loses of influencer
            influencerData[influencer].totalUserLossesByToken[
        ...
        if (userGuess.token == IERC20Upgradeable(address(0))) {
            ...
            if (!success) {
                // An error with the transfer was found, Increasing user loses of influencer
                influencerData[influencer].totalUserLossesByToken[
                    address(userGuess.token)
                ] += userGuess.betAmount;
                return;
            ...
            if (isRefund) {
                // A refund ocurred, Increasing user refunds of influencer
                influencerData[influencer].totalUserRefundsByToken[
                    address(userGuess.token)
                ] += userGuess.betAmount;
                return;
            }
            // Sucessful win payment, Increasing user wins of influencer
            influencerData[influencer].totalUserWinsByToken[
                address(userGuess.token)
            ] += userGuess.betAmount;
        }
    }
...
contract ReferralSystem is OwnableUpgradeable {
    ...
    mapping(address => address) public userReferredBy; // What influencer referred this user
    mapping(address => uint256) totalPaidToInfluencersByToken; // All the influencers
    ...
    struct InfluencerData {
        ...
        mapping(address => uint256) totalInfluencerEarns;
        mapping(address => uint256) totalCasinoEarns;
    }

    function __RandomNumber_init() internal {
        __Ownable_init();
    }
    ...
    function addInfluencers(
        ...
        require(
            percentages.length == influencers.length &&
                names.length == influencers.length,
            "ReferralSystem: inputs have different length"
        );
        ...
        for (uint256 i = 0; i < influencers.length; i++) {
            ...
            influencerData[influencers[i]].name = names[i];
        }
    }

    // TODO: check if it could cause gas limit issues, one solution is to pass the index and address, validate and delete
    function deleteInfluencers(address[] calldata influencers)
        external
        onlyOwner
    {
        for (uint256 i = 0; i < influencers.length; i++) {
            for (uint256 j = 0; j < influencerWallets.length; j++) {
                if (influencerWallets[j] == influencers[i]) {
                    influencerWallets[j] = influencerWallets[
                        influencerWallets.length - 1
                    ];
                    influencerWallets.pop();
                    delete influencerData[influencers[i]];
                }
            }
        }
    }

    function editInfluencerName(address influencer, string calldata name)
        external
        onlyOwner
    {
        influencerData[influencer].name = name;
    }

    function editInfluencersPercentage(
        address[] calldata influencers,
        uint256[] calldata percentages
    ) external onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < influencers.length; i++) {
            influencerData[influencers[i]].referralPercentage = percentages[i];
        }
    }
        ...
        )
    {
        ...
        totalLosses = influencerData[influencer].totalUserRefundsByToken[token];
    }
}
