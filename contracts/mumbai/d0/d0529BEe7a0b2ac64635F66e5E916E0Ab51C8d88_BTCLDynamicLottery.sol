...
function performUpkeep(bytes calldata performData) external nonReentrant override {
    // Decode the performData to retrieve the round numbers
    uint256[] memory roundsToProcess = abi.decode(performData, (uint256[]));
    require(roundsToProcess.length > 0, "No rounds to draw");

    // Loop through all rounds that need to be drawn and check if they are open
    for (uint256 i = 0; i < roundsToProcess.length; i++) {
        uint256 roundNr = roundsToProcess[i];
        require(roundNr >= nextRound && roundNr < nextRound + maxRounds, "Round out of range");
        require(rounds[roundNr].status.roundStatus == BTCLCore.Status.Open, "Round not open");
        require(rounds[roundNr].status.totalBets >= maxPlayers, "Not enough players");

        // Set the round status to Drawing and request the randomness
        rounds[roundNr].status.roundStatus = BTCLCore.Status.Drawing;
        rounds[roundNr].status.requestId = requestRandomness(uint32(totalWinners));
    ...
    if (roundsToProcess[roundsToProcess.length - 1] + 1 == nextRound + maxRounds) {
        nextRound += maxRounds;
        round++;
        rounds[round].status.roundStatus = BTCLCore.Status.Open;
...
function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
    ...
    for (uint256 i = nextRound; i < nextRound + maxRounds; i++) {
        if (rounds[i].status.requestId == requestId && rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
            // Set the round status to Completed and save the randomness
            roundsProcessed++;
