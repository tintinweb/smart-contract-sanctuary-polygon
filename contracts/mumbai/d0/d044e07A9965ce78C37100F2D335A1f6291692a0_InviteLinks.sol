    ...
    EIP712Upgradeable
{
        ...
        nonZeroAddress(publicKey)
    {
        ...
        // Emit the event
        emit LinkCreated(id, referrer, sparks, publicKey);
    }
    ...
    function redeemLink(
        ...
        if (request.recipient != _msgSender()) {
            revert Unauthorized(request.recipient);
        }
        ...
        // Assign the referrer
        if (referrers[request.recipient] == address(0)) {
        ...
        // Emit the event
        emit LinkRedeemed(linkId, link.referrer, request.recipient, link.sparks);
    }
}