...
contract dice {
    ...
    event SettlementEnd(string indexed orderIdIdx, string orderId, uint256 randomNum, uint256[] results,BetRegion[] winRegions, BetRecord[] bets); //结算完成
    ...
    function doSettlement(uint256 randomNum, uint256[] memory results) private {
        require(state == GameState.Settlement, "doSettlement can be called only in settlement state.");
        ...
        emit SettlementEnd(orderId, orderId, randomNum, results, winRegions, bets);
