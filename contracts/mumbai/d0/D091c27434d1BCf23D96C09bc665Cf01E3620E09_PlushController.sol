...
contract PlushController is Ownable {
    ...
    function withdrawalToUser(address _address, uint256 _amount) external onlyOwner
    {
        require(withdrawalAddressExist(_address), 'Withdrawal is not available for this address.');
        require(getAvailableBalanceForWithdrawal() >= _amount, 'Not enough balance.');

        plushCoinWallets.withdrawByController(_amount, _address);
