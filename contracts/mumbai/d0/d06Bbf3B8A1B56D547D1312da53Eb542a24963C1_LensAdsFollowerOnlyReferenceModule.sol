...
error LensAds__NotEnoughBudget();
...
// debo recibir numeros que cuadren
...
{
    // Modulens Fee
    address private immutable i_modulens;
    uint256 private immutable i_fee = 1; // Fee in %
    // Lens
    using SafeERC20 for IERC20;
    mapping(uint256 => mapping(uint256 => PostData)) internal _dataByProfileByPost;
    mapping(uint256 => mapping(uint256 => uint256)) private s_profileToPostToCurrentBudget; // Current budget by profile by post
    mapping(uint256 => mapping(uint256 => uint256)) private s_profileToPostToNumberOfMirrors; // Mirrors done

    constructor(address hub, address moduleGlobals) FeeModuleBase(moduleGlobals) ModuleBase(hub) {
        i_modulens = payable(msg.sender);
    }
    ...
    function initializeReferenceModule(
        ...
        address postOwner = IERC721(HUB).ownerOf(profileId);

        if (
            !_currencyWhitelisted(_currency) ||
            postOwner == address(0) ||
            _budget == 0 ||
            _reward == 0
        ) revert Errors.InitParamsInvalid();

        _dataByProfileByPost[profileId][pubId].budget = _budget - ((_budget * i_fee) / 100);
        _dataByProfileByPost[profileId][pubId].reward = _reward;
        _dataByProfileByPost[profileId][pubId].currency = _currency;

        fundThisPost(postOwner, _budget, _currency);
        s_profileToPostToCurrentBudget[profileId][pubId] = _budget;
    ...
    function processMirror(
        ...
        uint256 __currentBudget = s_profileToPostToCurrentBudget[profileIdPointed][pubIdPointed];
        ...
        if (__currentBudget > __reward) {
            IERC20(__currency).safeTransferFrom(address(this), mirrorCreator, __reward);

            s_profileToPostToCurrentBudget[profileIdPointed][pubIdPointed] -= __reward;
            s_profileToPostToNumberOfMirrors[profileIdPointed][pubIdPointed]++;
        } else {
            revert LensAds__NotEnoughBudget();
        }
    }

    function fundThisPost(address _profile, uint256 _budget, address _currency) private {
        IERC20(_currency).safeTransferFrom(_profile, address(this), _budget);
        IERC20(_currency).safeTransferFrom(address(this), i_modulens, (_budget * i_fee) / 100);
    }
