...
contract GameOwner is Ownable {
  ...
  event RoleChange(address indexed _game,bool flag);
  function addGameRole(address _game) public onlyOwner {
      gameRoles[_game] = true;
      emit RoleChange(_game,true);
  }
  function GetGameRole(address _game) public view onlyOwner  returns(bool){
    return   gameRoles[_game];
...
contract SunflowerLand is ERC721Enumerable, Pausable, GameOwner {
    /*
     *每个农场都部署了自己的合约地址 这使 NFT 能够实际拥有资源和代币
    */
    ...
    /**
     * 请勿使用 - 它会导致根据 totalSupply 生成的 ID 重叠 首选转移到死地址
     */
...
contract SunflowerLandSession is Ownable, GameOwner {
    ...
    SunflowerLand farm;
    // Enable for deploy - 禁用测试 IUniswapV2Router02 public immutable uniswapV2Router；
    ...
    // 这被明确命名为“批量”函数，因为函数参数与普通的“批处理”函数（例如 getMintedAtBatch）不一致
