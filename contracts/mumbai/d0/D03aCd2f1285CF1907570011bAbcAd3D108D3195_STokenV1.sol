...
contract STokenV1 is Context, IERC20Metadata, Pausable, Deployerable, Roles {
    ...
    /// @notice Event emited when locked tockens unlock
    event UnlockTokens(
        address tokensOwner,
    ...
    ///         moving the whole balance of tokens, setting proper limits for old and new treasuties
    ...
    /// @dev Allowed only for ComplianceOfficer.
    ///      Function blocked when contract is paused.
    ///      Function just changes flag {hasOwnSecondaryLimit} to `false`
    ///      for the addresses from `_accountsToReset`
    ///      and contract will ignore value which is set in
    ///      the parametr {individualSecondaryTradingLimit} for each account from array
    ...
    ///      And lock `_amountToLock` of tokens or the whole avaliable balance of `_account`.
    ...
    ///      Function checks the opportunity to lock `_amountToLock`=> see
    ///      comments to function {_checkAmountToLock} to know how is it checked.
    ///      And lock `_amountToLock` of tokens or the whole avaliable balance of account from `_bundleTo`.
    ...
    ///      whole avaliable balance of account from `_bundleTo`.
    ...
    /// @notice Returns the whole personal data of `_account`
    /// @return array with such data of this account(see struct {PersonalInfo}):
    /// *userAddress,
    /// *is address whitelisted(true/false),
    /// *hasOwnSecondaryLimit(true/false),
    /// *hasOwnTransactionCountLimit(true/false),
    /// *individualSecondaryTradingLimit,
    /// *individualTransactionCountLimit,
    ...
    /// @dev Function takes two numbers: {_avaliableBalance} & {_avaliableLimit}
    ///      and compaire them.
    ///      {_avaliableBalance} - subtracts from the total balance value of lockUps
    ///      {_avaliableLimit} - returns currently available limit for this address
    ...
    {
        _avaliableBalance(_account) < _avaliableLimit(_account)
            ? result = _avaliableBalance(_account)
            : result = _avaliableLimit(_account);
    }
    ...
    /// @dev Returns currently avaliable Secondary Trading limit of `_account`:
    ///      calculates difference between the current limit of `_account` and
    ///      the amount already sent by this address
    function _avaliableLimit(address _account) internal view returns (uint256) {
    ...
    /// @dev Returns currently avaliable amount of tokens to use (transfer) by `_account`:
    ///      calculates the difference between the whole balance of tokens and
    ///      locked tokens on the `_account`
    function _avaliableBalance(address _account)
        internal
        view
        returns (uint256)
    {
    ...
    ///      compare {_avaliableBalance} of `_account` and `_amountToLock`
    ...
    {
        _avaliableBalance(_account) > _amountToLock
            ? resultedAmount = _amountToLock
            : resultedAmount = _avaliableBalance(_account);
    }
