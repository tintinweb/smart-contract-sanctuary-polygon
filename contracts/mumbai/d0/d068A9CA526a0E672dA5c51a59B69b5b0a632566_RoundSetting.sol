...
contract RoundSetting is IRoundSetting, AccessController {
    ...
    function getCurrentRound(uint256 _consumptionType) external override view returns (RoundInfo memory){
      return roundInfoByIndex[currentRoundByConsumptionType[_consumptionType]];
    }

    function getRoundInfo(uint256 _roundIndex) external override view returns (RoundInfo memory){
      return roundInfoByIndex[_roundIndex];
    }

    function getTotalSoldByConsumptionType(uint256 _consumptionType) external override view returns (uint256){
      return totalSoldByConsumptionType[_consumptionType];
    }

    function getTotalBuyOfUserPerRound(address _addr, uint256 _roundIndex) external override view returns (uint256){
      return totalBuyOfUserPerRound[_addr][_roundIndex];
    }

    function getTotalItemSoldInRound(uint256 _roundIndex) external override view returns (uint256){
      return totalItemSoldInRound[_roundIndex];
    }

    function updateRoundSetting(address _address, uint256 _roundIndex, uint256 amount) external override onlyOperator {
    ...
    function cancelRound(uint256 _roundIndex) external override onlyOperator returns (uint256, uint256) {
    ...
    function getRoundAvailable(uint256[] memory _roundIndexes) external override view returns (RoundInfo[] memory){
