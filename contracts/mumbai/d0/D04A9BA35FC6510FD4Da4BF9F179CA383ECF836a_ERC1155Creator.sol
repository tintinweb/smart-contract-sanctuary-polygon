...
contract ERC1155Creator is AdminControl, ERC1155, ReentrancyGuard {
    ...
    function mint(
        ...
        uint256 tokenId,
        uint256 amount
    ) public adminRequired nonReentrant {
        // 不允許增發
        require(
            _totalSupply[tokenId] == 0,
            "This contract does not allow to increase the number of minted NFTs."
        );
        // 檢查是否已經設定好Token URI
        require(
            bytes(_uris[tokenId]).length != 0,
            "Please set token uri before mint."
        );
        // 鑄造
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < tokenId.length; i++) {
            require(
                bytes(_uris[tokenId[i]]).length != 0,
                "Please set token uri before mint."
            );
        }
        // 鑄造
