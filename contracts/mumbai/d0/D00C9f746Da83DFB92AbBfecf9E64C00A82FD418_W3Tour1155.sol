...
// File: evm/contracts/W3Tour1155.sol
...
contract W3Tour1155 is ERC1155, AccessControl {
    ...
    event W3ItemRegistered(uint256 indexed _id,  uint256 _maxSupply, string _metadata);
    event W3ItemUpdated(uint256 indexed _id,  uint256 _maxSupply, string _metadata);

    constructor(string memory _nameArg, string memory _symbolArg) ERC1155("") {
        _symbol = _symbolArg;
        _name = _nameArg;
    ...
    function safeMint(address _account, uint256 _id, uint256 _quantity) public {
        require (hasRole(MINTER_ROLE, msg.sender),"not authorized");
        _safeMint(_account, _id, _quantity);
    }

    function safeSingleMintBatch(address _account, uint256[] memory _ids) public {
        require (hasRole(MINTER_ROLE, msg.sender),"not authorized");
        for (uint256 i = 0; i < _ids.length; ++i) {
             _safeMint(_account, _ids[i], 1);
        }
    }    

    function safeMultiMintBatch(address _account, uint256[] memory _ids, uint256[] memory _quantities) public {
        require (hasRole(MINTER_ROLE, msg.sender),"not authorized");
        require(_ids.length == _quantities.length, "ids and quantities length mismatch");
        for (uint256 i = 0; i < _ids.length; ++i) {
             _safeMint(_account, _ids[i], _quantities[i]);
        }
    }

    function _safeMint(address _account, uint256 _id, uint256 _quantity) private {
        require (_items[_id].date > 0, "item is not registered");
        require (_items[_id].quantity + _quantity  <= _items[_id].maxSupply, "exceeded max supply");
        _items[_id].quantity +=_quantity;
