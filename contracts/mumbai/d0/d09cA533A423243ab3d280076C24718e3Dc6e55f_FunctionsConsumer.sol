...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner, IOracle {
  ...
  function fulfillRequest(
    ...
    uint256 length = uint256(stringToUint(getElementAtIndex(string(response), 0)));
    ILightClientMock.OracleResponse[] memory responses = new ILightClientMock.OracleResponse[](length);
    ...
    while (keccak256(abi.encode(result)) != keccak256(abi.encode(""))) {
      ...
      if (resultIndex == 2) {
        ...
        ILightClientMock.OracleResponse memory res = ILightClientMock.OracleResponse(chainId, height, root);
  ...
  function getElementAtIndex(string memory _str, uint256 _index) public pure returns (string memory) {
