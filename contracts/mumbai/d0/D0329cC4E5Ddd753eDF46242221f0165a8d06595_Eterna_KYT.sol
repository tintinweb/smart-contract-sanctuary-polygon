...
// File: contracts/lock.
contract Eterna_KYT is BasicMetaTransaction,Initializable{
    ...
    function deposit(uint256 _amount, address token) external payable {
        ...
        if (token != address(1)){
            ...
            TransferHelper.safeTransferFrom(token,_msgSender(),address(this),_amount);
    ...
    function spottingInBatch(Transactions[] calldata txArray ) external {
        ...
        for (uint256 i = 0; i < txArray.length; i++) {
            ...
            if(x.token == address(1)){
                ...
                TransferHelper.safeTransfer(x.token,x.vault,multipleToken[x.depositer][x.token]);
                //  IERC20(x.token).transfer( x.vault ,   multipleToken[x.depositer][x.token] );
            }
