...
contract Referal_Matching is RestrictedFunctions {
   ...
   function join (address _ref,uint256 _package, uint8 _flag) external isLock {
      ...
      ROI_DATA[ref1].earned += _amount * ref_1_Percent / 100e18;
      ROI_DATA[ref2].earned += _amount * ref_2_Percent / 100e18;
      emit ReferalCommission ([ref1,ref2],[_amount * ref_1_Percent / 100e18,_amount * ref_2_Percent / 100e18]);
      _match(user,_ref,_flag);
      emit Join(user, _ref, ref2, _package, _amount, roi.depositTime);
   }
   ...
   function addressValidate (address[]memory _ref) internal view returns(address,address) {
      transformedAddress[_ref[0]] == address(0) ? _ref[0]:transformedAddress[_ref[0]];
      transformedAddress[_ref[1]] == address(0) ? _ref[1]:transformedAddress[_ref[1]];
