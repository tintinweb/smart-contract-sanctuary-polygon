...
contract TuneAIv3 is Ownable, ReentrancyGuard {
    ...
    */
    ...
    Counters.Counter public songIds; // private tracker for song ID's
    uint public totalTracks; // all tracks uploaded
    mapping(address => Rating[]) public ratings; // tracks ratings
   ...
   */
    ...
    function uploadTrack(
        ...
        string memory songIPFSURL = string(
            abi.encodePacked("https://gateway.pinata.cloud/ipfs/", _songCID)
        );
        string memory coverIPFSURL = string(
            abi.encodePacked("https://gateway.pinata.cloud/ipfs/", _coverCID)
        );
   ...
   */
    ...
    function rateTrack(uint256 _index, uint256 _rating) external {
        ...
        Rating[] storage userRatings = ratings[msg.sender];
        if (userRatings.length > 0) {
            bool ratingFound = false;
            for (uint i = 0; i < userRatings.length; i++) {
                if (userRatings[i].index == _index) {
                    ratingFound = true;
                    track.totalRatings += _rating - userRatings[i].rating;
                    userRatings[i].rating = _rating;
                    break;
                }
            }
            if (!ratingFound) {
                // user has not rated this track before
                userRatings.push(Rating(_index, _rating));
                track.totalRatings += _rating;
                track.numRatings++;
            }
        } else {
            // user has not rated any tracks before
            userRatings.push(Rating(_index, _rating));
            track.totalRatings += _rating;
            track.numRatings++;
        }
