...
// Base
import '../interfaces/PaymentSplitterUpgradeable.sol';
import '../interfaces/IThirdwebContract.sol';
// Meta-tx
import '../interfaces/ERC2771ContextUpgradeable.sol';
// Access
import "@openzeppelin/contracts-upgradeable/access/AccessControlEnumerableUpgradeable.sol";
// Utils
import "@openzeppelin/contracts-upgradeable/utils/MulticallUpgradeable.sol";
import '../interfaces/FeeType.sol';
...
contract Winners is Split {

    using SafeMath for uint256;
    using Address for address;

    uint256 public distributeThreshold = 0.1 ether;
    ...
    event distributeBalances(address user, uint256 amount);
     ...
     constructor(
        ...
        function setPayees(address[] memory _payees, uint256[] memory _shares) public onlyRole(DEFAULT_ADMIN_ROLE) {
        __PaymentSplitter_init(_payees, _shares);
        }

        function claimBalance(uint256 tokenId) external {
            require(finalChapter.balanceOf(msg.sender, tokenId) > 0, "You must have FinalChapter token");
        ...
        function distributeBalance() internal {
            if (address(this).balance >= distributeThreshold) {
                ...
                uint256 amountPerTopUser = amountForTopUsers.div(numTopUsers);
                uint256 amountForNonTopUsers = amount.sub(amountForTopUsers);
                uint256 amountPerNonTopUser = numUsers > 99 ? amountForNonTopUsers.div(numUsers.sub(numTopUsers)) : 0;
                ...
                for (uint256 i = 0; i < rankingAddresses.length; i++) {
                    ...
                    if (rankingIndex[user] <= 98) {
                     ...
                     emit distributeBalances(user, amount);
                }
            }
        }
        ...
        function getRanking() public view returns (address[] memory) {
            return rankingAddresses;
        }
        ...
        function getContractBalance() public view returns (uint256 maticBalance, uint256 usdBalance) {
        ...
        function distribute() public override {
            ...
            for (uint256 i = 0; i < count; i++) {
                ...
                if (_pendingPayment(account, totalReceived, alreadyReleased) > 0) {
                    _release(account);
                }
            }
            distributeBalance();
       ...
       function distribute(IERC20Upgradeable token) public override {
            uint256 count = payeeCount();
            for (uint256 i = 0; i < count; i++) {
