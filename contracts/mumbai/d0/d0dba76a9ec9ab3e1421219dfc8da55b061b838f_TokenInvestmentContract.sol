...
contract TokenInvestmentContract {
    ...
    address public feeCollector = 0xA29998d66c45123448715Bf46a8870D2EC2a5BA4;
    ...
    function changeDailyProfitPercentage(
        ...
        require(
            newPercentage >= 1 && newPercentage <= 2,
            "Please enteer in the range of 1-2%"
        );
    ...
    function invest(uint tokenAmount, address referral) external {
        ...
        uint taxAmount = (depositAmount * depositFee) / 100;
        ...
        if (investors[msg.sender].initialCapital == 0) {
            investorCount++;
        }
        ...
        // Transfer tax amount to the taxCollector address
        IERC20(tokenAddress).transfer(feeCollector, taxAmount);

        investors[msg.sender].initialCapital += (depositAmount - taxAmount);
    ...
    function withdrawProfit() external {
        ...
        investors[msg.sender].totalProfit += investors[msg.sender].profit;
    ...
    function withdrawCapital() external {
        ...
        investors[msg.sender].lastProfitDistributionTime = 0;

        IERC20(tokenAddress).transfer(msg.sender, withdrawAmount);
        emit WithdrawCapital(msg.sender, withdrawAmount);
    }

    function distributeProfit(address investor) public {
    ...
    function depositOwner(uint tokenAmount) external onlyOwner {
        ...
        ownerDeposit += tokenAmount;

        emit Deposit(msg.sender, tokenAmount);
    }

    function withdrawTokensOwner(uint amount) external onlyOwner {
    ...
    function getTotalProfit() public view returns (uint) {
        ...
        return ownerDeposit - ownerWithdraw;
    }
