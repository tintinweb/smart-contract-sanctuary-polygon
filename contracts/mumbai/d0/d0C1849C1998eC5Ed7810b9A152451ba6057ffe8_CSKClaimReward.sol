    ...
    Initializable
{
    ...
    event ClaimDigital(claimInfo _info, RewardType);
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "orderInfo(address claimer,bytes32 itemId,uint256 shipPrice, uint256 timestamp)"
                        ),
                        _info.claimer,
                        _info.itemId,
                        _info.timestamp
    ...
    function claimItem(claimInfo calldata _info, RewardType _type) public payable nonReentrant {
        ...
        //transfer
        if(_type == RewardType.Btc) {
            uint256 _amount = 0.1 ether;
            WBtc.transferFrom(rewardWallet, _info.claimer, _amount);
        } else if (_type == RewardType.Phy) {
