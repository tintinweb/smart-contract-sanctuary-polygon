...
/**
 * @title FakeAltroFractionsBuyout
 * @author FakeAltro Developer
 * @dev This contract allows for the buyout of fractional token sales on the FakeAltro platform.
 */
contract FakeAltroFractionsBuyout is AccessControl, ReentrancyGuard {
	...
	/**
	 * @dev The FakeAltroFractions contract
	 */
	IFractions public immutable altrFractions;
	/**
	 * @dev The FakeAltroFractionsSale contract
	 */
	...
	/**
	 * @dev The FakeAltroFeeManager contract.
	 */
	...
	modifier onlyIfSaleClosed(uint256 saleId) {
		require(altrFractionsSale.isSaleClosed(saleId), "FakeAltroFractionsBuyout: sale not finished yet");
		_;
	}
	/**
	...
	modifier onlyBeforeBuyoutOpen(uint256 buyoutId) {
		require(isBeforeBuyoutOpen(buyoutId), "FakeAltroFractionsBuyout: buyout already started");
		_;
	}
	/**
	...
	modifier onlyWhileBuyoutOpen(uint256 buyoutId) {
		require(isBuyoutOpen(buyoutId), "FakeAltroFractionsBuyout: buyout not open");
		_;
	}

	/**
	 * @dev Constructor function to initialize the FakeAltroFractionsBuyout contract
	...
	constructor(IFractions _altrFractions, IFractionsSale _altrFractionsSale, address _feeManager, uint256 _buyoutMinFractions, uint256 _buyoutOpenTimePeriod) {
		require(address(_altrFractions) != address(0) && address(_altrFractionsSale) != address(0), "FakeAltroFractionsBuyout: cannot be null address");
	...
	function requestBuyout(uint256 saleId) external nonReentrant onlyIfSaleClosed(saleId) {
		...
		require(fractionsSale.fractionsSold >= fractionsSale.saleMinFractions, "FakeAltroFractionsBuyout: sale unsuccessful");
		require(canDoBuyout(msg.sender, saleId), "FakeAltroFractionsBuyout: not enough fractions");
	...
	function setBuyoutParams(uint256 buyoutId, uint256 buyoutPrice) external onlyRole(DEFAULT_ADMIN_ROLE) onlyBeforeBuyoutOpen(buyoutId) {
		require(_buyoutsCounter.current() > buyoutId, "FakeAltroFractionsBuyout: invalid buyout id");
	...
	function buyoutUnsupervised(uint256 saleId) external nonReentrant onlyIfSaleClosed(saleId) {
		...
		require(fractionsBalance == fractionsSale.fractionsSold, "FakeAltroFractionsBuyout: not enough fractions");
		require(fractionsSale.fractionsSold >= fractionsSale.saleMinFractions, "FakeAltroFractionsBuyout: sale unsuccessful");
	...
	function executeBuyout(uint256 buyoutId) external onlyWhileBuyoutOpen(buyoutId) {
		Buyout storage buyout = buyouts[buyoutId];
		require(buyout.initiator == msg.sender, "FakeAltroFractionsBuyout: must be buyout initiator");
		...
		require(canDoBuyout(msg.sender, buyout.fractionSaleId), "FakeAltroFractionsBuyout: not enough fractions");
	...
	function _setBuyoutMinFractions(uint256 _buyoutMinFractions) internal {
		require(_buyoutMinFractions >= _MIN_FRACTIONS && _buyoutMinFractions <= _MAX_FRACTIONS, "FakeAltroFractionsBuyout: buyout min fractions exceed boundaries");
	...
	function _setFeeManager(address _feeManager) internal {
		require(_feeManager != address(0), "FakeAltroFractionsBuyout: cannot be null address");
		require(_feeManager.supportsInterface(type(IFeeManager).interfaceId), "FakeAltroFractionsBuyout: does not support IFeeManager interface");
	...
	function _setBuyoutOpenTimePeriod(uint256 _buyoutOpenTimePeriod) internal {
		require(_buyoutOpenTimePeriod > _MIN_BUYOUT_OPEN_TIME_PERIOD, "FakeAltroFractionsBuyout: open time period cannot be less than minimum");
 ...
 * @author FakeAltro Developer
 * @dev Contract used to manage the tokens used to purchase FakeAltro fractions.
 */
 ...
 * @title TokenSplitter
 * @author FakeAltro Developer
 * @dev Contract for managing tokens used to buyout FakeAltro fractions.
 */
