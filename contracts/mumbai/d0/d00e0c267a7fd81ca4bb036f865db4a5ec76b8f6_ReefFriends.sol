...
contract ReefFriends is ERC721, Ownable {
  using Strings for uint256;
  ...
  uint256 public maxPerPresaleWallet = 3;
  uint256 public maxPerPublicWallet = 6;
  ...
  constructor() ERC721("Reef Friends", "Reef Friends") {}
  ...
  function mint(bytes32[] calldata _merkleProof, uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    ...
    if(presaleActive==true){
      ...
      require(balanceOf(msg.sender) + _mintAmount <= maxPerPresaleWallet, "Mint limit exceeded." );

    }
    else{
      require(msg.value >= publicSaleCost * _mintAmount, "Insufficient funds!");
      require(balanceOf(msg.sender) + _mintAmount <= maxPerPublicWallet, "Mint limit exceeded." );
    }
