...
interface IFactory {
    ...
    function createTokenReward(address tokenerc, address tokennft, uint256 id, uint fee) external returns (address tokenReward);
}
...
interface ITokenReward {
    ...
    function getFeeOfExchange() external view returns (uint);
}
...
import "../interface/ITokenReward.sol";
...
contract Router is ERC1155Holder, Ownable {
    ...
    constructor(address _factory, address _referral, address _WETH){
    ...
    function _addLiquidity(
        ...
        if(factory.getTokenReward(tokenerc, tokennft,id) == address(0)){
           factory.createTokenReward(tokenerc, tokennft, id, _fee);
        }
        ...
        if(_reserveerc == 0 && _reservenft == 0){
            ...
            require(amountErcOptimal <= amountErcDesired && amountErcOptimal >= amountErcMin, 'DeMaskRouter: INSUFFICIENT_ERC_AMOUNT');
    ...
    function removeLiquidity(
        ...
        require(amounterc >= amountErcMin, 'DeMaskRouter: INSUFFICIENT_ERC_AMOUNT');
    ...
    function getAmountBuy(
        ...
        require(_reserveerc > 0 && _reservenft > 0, 'DeMaskRouter: INSUFFICIENT_LIQUIDITY');
        ...
        feeBuy = amountIn * ITokenReward(_token).getFeeOfExchange() / 1000000;
    ...
    function getAmountSell(
        ...
        feeSell = amountOut * ITokenReward(_token).getFeeOfExchange() / 1000000;
    ...
    function buyETH(
        ...
        (,, address _token) = getReserves(WETH, tokennft,id);
        (uint amount, uint feeBuy) = getAmountBuy(WETH, tokennft, id, amountNft);
        require(amount <= msg.value, 'DeMaskRouter: EXCESSIVE_INPUT_AMOUNT');
        IWETH(WETH).deposit{value: amount }();
        _safeTransferETH(_token, amount, feeBuy);
        _updateTokenReward(0, amountNft, to, 0, msg.sender, feeBuy, _token);
    ...
    function sellETH(
        ...
        (,, address _token) = getReserves(WETH, tokennft,id);
        (uint amount, uint feeSell) = getAmountSell(WETH, tokennft, id, amountNft);
        require(amount >= amountOutMin, 'DeMaskRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IERC1155(tokennft).safeTransferFrom(msg.sender, _token, id, amountNft, '0x0');
        _updateTokenReward(amount, 0, to, feeSell, address(this), feeSell, _token);
    ...
    function _updateTokenReward(uint amountercOut, uint amountnftOut, address to, uint amountFee, address from, uint _fee, address _token) internal {
        ...
        uint amounFeeReferral = _fee *  IReferral(referral).getFee() / 10000; // 1% <-> 100
        ITokenReward(_token).updateReward(_fee.sub(amounFeeReferral));
    }
    ...
    function _safeTransafer(address _token, address tokenerc, uint amount, uint feeBuy) internal {
        ...
        uint feeReferral = IReferral(referral).getFee();
        uint amounFeeReferral = feeBuy *  feeReferral / 10000; // 1% <-> 100
        IERC20(tokenerc).transferFrom(msg.sender, IReferral(referral).getReceiver(msg.sender) , amounFeeReferral);
        IERC20(tokenerc).transferFrom(msg.sender, ITokenReward(_token).getPool(), feeBuy.sub(amounFeeReferral));
    }

    function _safeTransferETH(address _token, uint amount, uint feeBuy) internal {
        IWETH(WETH).transfer(_token, amount.sub(feeBuy));
        uint feeReferral = IReferral(referral).getFee();
        uint amounFeeReferral = feeBuy *  feeReferral / 10000; // 1% <-> 100
        IWETH(WETH).transfer(IReferral(referral).getReceiver(msg.sender), amounFeeReferral);
        IWETH(WETH).transfer(ITokenReward(_token).getPool(), feeBuy.sub(amounFeeReferral));
    }
