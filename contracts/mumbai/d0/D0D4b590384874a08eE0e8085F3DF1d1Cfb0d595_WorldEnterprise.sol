...
contract WorldEnterprise is IERC20 {
    ...
    modifier checkInfo(Enterprise.Info memory info_) {
        ...
        require(
            bytes(info_.description).length < Enterprise.MAX_LENGTH,
     ...
     *
     **/
    function joinWorldEnterprise(uint256 amount, uint256 price) external payable{
        ...
        if(price > 0) {
          require(
            price == msg.value,
            "WorldEnterprise: Insufficiant fund"
        );  
        }
