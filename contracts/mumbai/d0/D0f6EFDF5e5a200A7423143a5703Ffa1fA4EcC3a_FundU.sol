...
contract FundU is ReentrancyGuard, FundUtils {
    ...
    function withdraw(uint256 id) public nonReentrant onlyStreamBeneficiary(id) {
        ...
        if (stream.stopTime <= block.timestamp) {
            ...
            require(balance > 0, "FundU: No balance available");
        ...
        emit Withdraw(id, stream.owner, stream.beneficiary, balance);
    }
    ...
    function _withdrawPauseAndResume(uint256 _id, address _who) private {
        ...
        if (_balance > 0) {
            ...
            emit Withdraw(_id, stream.owner, stream.beneficiary, _balance);
        }
    }
