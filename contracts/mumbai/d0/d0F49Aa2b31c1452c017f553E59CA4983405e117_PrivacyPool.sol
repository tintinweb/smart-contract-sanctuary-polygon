...
contract PrivacyPool is ReentrancyGuard, MerkleTree, Verifier {
    using Pairing for bytes;
        ...
        returns (bool)
    {
        ...
        // uint[2] memory a = [flatProof[0], flatProof[1]];
        // uint[2][2] memory b = [[flatProof[2], flatProof[3]], [flatProof[4], flatProof[5]]];
        // uint[2] memory c = [flatProof[6], flatProof[7]];
        uint[5] memory input = [root, subsetRoot, nullifier, assetMetadata, withdrawMetadata];

        if (!verifyProof(flatProof, input)) revert InvalidZKProof();
...
contract Verifier {
    ...
    function verifyingKey() internal pure returns (VerifyingKey memory vk) {
        ...
        vk.IC[0] = Pairing.G1Point( 
            3709819338034394992351931193574361286047020808926008285394817693194359492932,
            10700126753315410368605817583707442682053930082726970092476084268903238095838
        );                                      
        
        vk.IC[1] = Pairing.G1Point( 
            1392961279004659496453774271225248599110755520446593538187132018630339220805,
            14757083777691947523325674745087506823366809512964294745021625736604779145222
        );                                      
        
        vk.IC[2] = Pairing.G1Point( 
            9016422286038831481546107221069759076890811787526002531206992088281882340194,
            12949162567942042187558873032494354881131620726213917185407863768005939184396
        );                                      
        
        vk.IC[3] = Pairing.G1Point( 
            15411371666175404298698422922427088232137377934590382432585601298084001218610,
            16236079821346211422407885087732745404588845905335332439456841773188578791652
        );                                      
        
        vk.IC[4] = Pairing.G1Point( 
            9411271819521899946147710946502139526722281433495063516594225225969940268066,
            12448565309283982693735590975015007125111196374853390432223413985073075281110
        );                                      
        
        vk.IC[5] = Pairing.G1Point( 
            4266273535521171700411791565542107965799858198377560899451873090397324136017,
            15143698420178204420514434530852536314463958569391339345991109212447760513463
        );                                      
