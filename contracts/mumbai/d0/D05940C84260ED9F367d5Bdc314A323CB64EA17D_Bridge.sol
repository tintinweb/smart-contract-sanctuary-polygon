...
interface MintGatewayWithStatus {
    function status(bytes32 hash) external view returns (bool);
}

contract Bridge is Context, AccessControlEnumerable {
    ...
    mapping(bytes32 => bool) public nHashStatus;
    ...
    function forwardBalanceToLender(IERC20 erc20) public {
        erc20.transfer(address(lender), erc20.balanceOf(address(this)));
    }
    ...
    struct ContractCallParams {
        /// The contract to call after minting the Ren-asset
        address contractAddress;
        /// The contract call's data
        bytes data;
        address transactedToken;
    }
    ...
    function mintAndCall(
        ...
        if (borrowAsset) {
            require(
                ...
                "Bridge: only relayer can mint from lender"
            );
        } else {
            require(
        ...
        require(
            contractCalls.length == 0 ||
                (contractCalls.length == 1 &&
                    allowedContracts[contractCalls[0].contractAddress]),
            "Bridge: invalid contractCalls length or unapproved contract call recipient"
        );
        require(
            nHashStatus[nHash] == false,
            "Bridge: transaction already submitted"
        );

        // Update nHashStatus.
        nHashStatus[nHash] = true;

        uint256 transactedTokenBalanceBefore = 0;
        if (
            contractCalls.length > 0 &&
            contractCalls[0].transactedToken != address(0x0)
        ) {
            transactedTokenBalanceBefore = IERC20(
                contractCalls[0].transactedToken
            ).balanceOf(address(this));
        }

        IERC20 token = registry.getRenAssetBySymbol(asset);
        ...
        // Add `0` to replace the fee and confirmations, which aren't in the payload.
        {
            ...
            _mint(asset, borrowAsset, pHash, amount, nHash, signature);
        }
        uint256 amountAfterFee = amount - fee;

        if (contractCalls.length > 0) {
            {
                token.approve(contractCalls[0].contractAddress, amountAfterFee);
                (bool success, bytes memory returnData) = contractCalls[0]
                    .contractAddress
                    .call{gas: gasleft() - 3000}(contractCalls[0].data);
                if (!success) {
                    assembly {
            ...
            {
                uint256 approvalRemaining = token.allowance(
                    address(this),
                    contractCalls[0].contractAddress
                );
                if (approvalRemaining > 0) {
                    token.transfer(fallbackRecipient, approvalRemaining);
                    token.approve(contractCalls[0].contractAddress, 0);
                }
            }

            if (contractCalls[0].transactedToken != address(0x0)) {
                uint256 transactedTokenBalanceAfter = IERC20(
                    contractCalls[0].transactedToken
                ).balanceOf(address(this));
                uint256 transactedTokenReceived = transactedTokenBalanceBefore -
                    transactedTokenBalanceAfter;
                if (transactedTokenReceived > 0) {
                    IERC20(contractCalls[0].transactedToken).transfer(
                        fallbackRecipient,
                        transactedTokenReceived
                    );
                }
            }
        } else {
            token.transfer(fallbackRecipient, amountAfterFee);
        }
    ...
    function mintAndRepay(
        ...
        IERC20 token = registry.getRenAssetBySymbol(asset);
        registry.getMintGatewayBySymbol(asset).mint(
            pHash,
            amount,
            nHash,
            signature
        );
        uint256 returned = lender.repay(address(token), nHash, amount);
        if (returned > 0) {
            token.transfer(fallbackRecipient, returned);
        }
    }

    /// @dev burnParams.amount is treated as the contractCall.transactedToken's amount rather than the Ren-asset's amount.
    function callWithTokenAndBurn(
        ContractCallParams calldata contractCall,
        BurnParams calldata burnParams
    ) external {
        require(
            allowedContracts[contractCall.contractAddress],
            "Bridge: contract not allowed"
        );

        IERC20 renAsset = registry.getRenAssetBySymbol(burnParams.asset);
        uint256 assetBalanceBefore = renAsset.balanceOf(address(this));

        uint256 transactedTokenBalanceBefore = 0;
        if (contractCall.transactedToken != address(0x0)) {
            transactedTokenBalanceBefore = IERC20(contractCall.transactedToken)
                .balanceOf(address(this));
            IERC20(contractCall.transactedToken).transferFrom(
                _msgSender(),
                address(this),
                burnParams.amount
            );
            IERC20(contractCall.transactedToken).approve(
                contractCall.contractAddress,
                burnParams.amount
            );
        }

        (bool success, bytes memory returnData) = contractCall
            .contractAddress
            .call{gas: gasleft() - 3000}(contractCall.data);
        if (!success) {
            assembly {
                revert(add(returnData, 32), returnData)
            }
        ...
        uint256 assetBalanceAfter = renAsset.balanceOf(address(this));

        uint256 amountReceived = assetBalanceBefore - assetBalanceAfter;

        uint256 feeAmount = (amountReceived * maxFeeBips) / 10000;
        uint256 amountToBurn = amountReceived - feeAmount;

        require(amountToBurn > 0, "Bridge: nothing to burn from contract call");

        if (
            String.isNotEmpty(burnParams.recipientChain) ||
            burnParams.recipientPayload.length > 0
        ) {
            registry.getMintGatewayBySymbol(burnParams.asset).burnWithPayload(
                burnParams.recipientAddress,
                burnParams.recipientChain,
                burnParams.recipientPayload,
                amountToBurn
            );
        } else {
            registry.getMintGatewayBySymbol(burnParams.asset).burn(
                burnParams.recipientAddress,
                amountToBurn
            );
        }
        ...
        if (contractCall.transactedToken != address(0x0)) {
            uint256 transactedTokenBalanceAfter = IERC20(
                contractCall.transactedToken
            ).balanceOf(address(this));
            uint256 transactedTokenBalanceRemaining = transactedTokenBalanceBefore -
                    transactedTokenBalanceAfter;
            if (transactedTokenBalanceRemaining > 0) {
                IERC20(contractCall.transactedToken).transfer(
                    _msgSender(),
                    transactedTokenBalanceRemaining
                );
            }
        }
    ...
    function burn(BurnParams calldata params) external {
        require(
            params.amount > 0,
            "Bridge: nothing to burn from contract call"
        );

        uint256 feeAmount = (params.amount * maxFeeBips) / 10000;
        uint256 amountToBurn = params.amount - feeAmount;

        if (
            String.isNotEmpty(params.recipientChain) ||
            params.recipientPayload.length > 0
        ) {
            registry.getMintGatewayBySymbol(params.asset).burnWithPayload(
            ...
            registry.getMintGatewayBySymbol(params.asset).burn(
                params.recipientAddress,
                params.amount
            );
        }
    ...
    function _mint(
        ...
        if (borrowAsset) {
            lender.borrow(
                address(registry.getRenAssetBySymbol(asset)),
                nHash,
                amount
            );
        } else {
...
contract Lender is Context, AccessControlEnumerable {
    ...
    constructor(address roleAdminAddress) {
        AccessControlEnumerable._grantRole(
            AccessControl.DEFAULT_ADMIN_ROLE,
            roleAdminAddress
        );
