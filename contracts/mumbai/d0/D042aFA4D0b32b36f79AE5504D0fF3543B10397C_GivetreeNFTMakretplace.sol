...
contract GivetreeNFTMakretplace is Ownable, ReentrancyGuard {
	...
	modifier isApprovedCollection(
		...
		require(IERC721(_collection).getApproved(tokenId) == address(this), "Givetree: NFT isn't approved marketplace");
		_;
	}

	modifier isApprovedSingleNFT {
		require(IERC1155(SingleNFT).isApprovedForAll(_msgSender(), address(this)), "Givetree: NFT isn't approved marketplace");
		_;
	}
	...
	modifier isValidCharity(
		...
		if (charity != address(0)) {
			...
			require(bValid, "Givetree: Invalid charity address");
		}
		_;
	}
	...
	function listItemForFixed(address _collection, uint256 tokenId, uint256 price, address charity, uint8 percent)
		external 
		isApprovedCollection(_collection, tokenId)
	...
	function listSingleForFixed(uint256 tokenId, uint256 amount, uint256 price, address charity, uint8 percent)
		external
		isApprovedSingleNFT
		notListed(SingleNFT, tokenId, _msgSender())
		isValidCharity(charity)
		isValidPercent(percent)
	{
		require(price > 0, "Givetree: Price must be above 0");
		...
		s_listings[SingleNFT][tokenId] = Listing(0, 0, price, _msgSender(), charity, percent, 1, amount);

		emit SingleListedForFixed(SingleNFT, _msgSender(), tokenId, amount, price, charity, percent);
	}

	function listSingleForAuction(uint256 tokenId, uint256 amount, uint256 initialPrice, uint256 endAuctionTimestamp, address charity, uint8 percent)
		external
		isApprovedSingleNFT
		notListed(SingleNFT, tokenId, _msgSender())
		isValidCharity(charity)
		isValidPercent(percent)
	{
		...
		s_listings[SingleNFT][tokenId] = Listing(1, endAuctionTimestamp, initialPrice, _msgSender(), charity, percent, 1, amount);

		emit ItemListedForAuction(SingleNFT, _msgSender(), tokenId, initialPrice, endAuctionTimestamp, charity, percent);
	}

	function buySingleNFT(uint256 tokenId)
		external
		payable
		isListed(SingleNFT, tokenId)
		isListedForFixed(SingleNFT, tokenId)
		nonReentrant 
	{
		Listing memory listedItem = s_listings[SingleNFT][tokenId];
		require(IERC1155(SingleNFT).isApprovedForAll(listedItem.seller, address(this)), "Givetree: NFT isn't approved marketplace");
		require(msg.value == listedItem.price, "Givetree: Insufficient balance");
		require(listedItem.seller != _msgSender(), "Givetree: Caller is this token's owner");
		...
		singleNFT.Royalty memory royalty = singleNFT(SingleNFT).getRoyalty(tokenId);
		uint256 toRoyalCharity = msg.value * royalty.charityPercent / 100;
		uint256 toRoyalCreator = msg.value * royalty.creatorPercent / 100;
		(success, ) = payable(royalty.charity).call{ value: toRoyalCharity}("");
		(success, ) = payable(royalty.creator).call{ value: toRoyalCreator}("");
		...
		IERC1155(SingleNFT).safeTransferFrom(listedItem.seller, _msgSender(), tokenId, listedItem.amount, "");
		delete s_listings[SingleNFT][tokenId];

		emit SingleBought(SingleNFT, _msgSender(), tokenId, listedItem.amount, listedItem.price);
	}

	function bidForAuctionSingle(uint256 tokenId, uint256 amount, uint256 bidPrice)
		external
		isListed(SingleNFT, tokenId)
		isListedForAuction(SingleNFT, tokenId)
		nonReentrant
	{
		Listing memory listedItem = s_listings[SingleNFT][tokenId];
		require(listedItem.seller != _msgSender(), "Givetree: Caller is this token's owner");
		...
		BiddingForAuction[] memory _biddings = s_biddings[SingleNFT][tokenId];
		bool bUpdated = false;

		// Replace the last sender's bid with this new one
		for (uint i = 0; i < _biddings.length; i ++) {
			if (_biddings[i].bider == _msgSender()) {
				s_biddings[SingleNFT][tokenId][i] = bidding;
				bUpdated = true;
				break;
			}
		}
		...
		if (!bUpdated) {
			s_biddings[SingleNFT][tokenId].push(bidding);
		}

		emit BiddedForAuction(SingleNFT, _msgSender(), tokenId, bidPrice);
	}
		...
		isTokenOwner(_collection, tokenId)
	{
		...
		if (_biddings.length > 0) {
			...
			else if (listedItem.NFTtype == 1) {
				singleNFT.Royalty memory royalty = singleNFT(_collection).getRoyalty(tokenId);
				uint256 toRoyalCharity = highestBid.bidPrice * royalty.charityPercent / 100;
				uint256 toRoyalCreator = highestBid.bidPrice * royalty.creatorPercent / 100;
				paymentToken.transferFrom(highestBid.bider, royalty.charity, toRoyalCharity);
				paymentToken.transferFrom(highestBid.bider, royalty.creator, toRoyalCreator);
	...
	function updateFactory(address _factory) external onlyOwner {
		require(_factory != address(0), "Givetree: Invalid address");
		
		factory = giveTreeFactory(_factory);

		emit UpdatedFactory(_factory);
	}
