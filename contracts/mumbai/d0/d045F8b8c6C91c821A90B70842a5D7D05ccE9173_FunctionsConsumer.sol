...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function addSimulatedRequestId(address oracleAddress, bytes32 requestId) public onlyOwner {
    ...
    event ProfileWithdraw(WithdrawResponse withdrawResponse);
    ...
    function withdraw(address _profile) public {
        ...
        require(
            msg.sender == profile.i_owner(),
            "Only onwer of contract allow withdraw"
        );

        uint256 balance = profile.balance();
        WithdrawResponse memory withdrawResponse = WithdrawResponse(
            _profile,
            msg.sender,
            balance
        );
        ...
        emit ProfileWithdraw(withdrawResponse);
    }
