...
contract ClaimTokenFacet {
    ...
    function setLootboxId(uint256 pool) external {
        LibDiamond.enforceIsContractOwner();
        LibTerminusToken.setLootboxId(pool);
    }

    function claimTokenGenerateMessageHash(
        address player,
        uint256 amount,
        uint256 bundleId,
        ...
        bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
                    "claimTokenPayload(address player, uint256 amount, uint256 bundleId, uint256 blockDeadline)"
                ),
                player,
                amount,
                bundleId,
    ...
    function claimTokenWithSignature(
        uint256 poolId,
        uint256 amount,
        uint256 bundleId,
        uint256 blockDeadline,
        ...
        LibTerminusToken.TerminusStorage storage ts = LibTerminusToken.terminusStorage();

        bytes32 hash = claimTokenGenerateMessageHash(
            msg.sender,
            amount,
            bundleId,
            blockDeadline
        ...
        require(
            ...
            "ClaimTokenFacet: claimTokenWithSignature -- Payload must be signed by game server"
        );
        require(
            !LibServerSideSigning._checkRequest(bundleId),
            "ClaimTokenFacet: claimTokenWithSignature -- Request has already been fulfilled"
        );
        require(
            block.number <= blockDeadline,
            "ClaimTokenFacet: claimTokenWithSignature -- Block deadline has expired"
        );
        LibServerSideSigning._completeRequest(bundleId);

        LibTerminusToken.stashLootBox(msg.sender, poolId, bundleId, amount);
    }
}
