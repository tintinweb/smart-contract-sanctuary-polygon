  ...
  OwnableUpgradeable
{
    ...
    onlyRole(ADMIN_ROLE)
  {
    ...
    for (uint256 i = 0; i < loopLimit; i++) {
      ...
      UUPSUpgradeable(proxy).upgradeTo(_implementation);
      emit ProxyUpgraded(proxy, _implementation, msg.sender);
    }
  }

  function pauseAll() public onlyRole(ADMIN_ROLE) {
    uint256 loopLimit = proxyDetails.length < loopProxyCall
      ? proxyDetails.length
      : loopProxyCall;
    for (uint256 i = 0; i < loopLimit; i++) {
      ...
      if (!NFT(proxy).paused()) {
        NFT(proxy).pause();
        emit ProxyPaused(proxy, msg.sender, true);
      }
    }
  }

  function pauseSingle(address _proxy)
    public
    onlyRole(ADMIN_ROLE)
    validProxy(_proxy)
  {
    NFT(_proxy).pause();
    emit ProxyPaused(_proxy, msg.sender, true);
  }

  function unpauseAll() public onlyRole(ADMIN_ROLE) {
    uint256 loopLimit = proxyDetails.length < loopProxyCall
      ? proxyDetails.length
      : loopProxyCall;
    for (uint256 i = 0; i < loopLimit; i++) {
      ...
      if (NFT(proxy).paused()) {
        NFT(proxy).unpause();
        emit ProxyUnpaused(proxy, msg.sender, true);
      }
    }
  }

  function unpauseSingle(address _proxy)
    public
    onlyRole(ADMIN_ROLE)
    validProxy(_proxy)
  {
    NFT(_proxy).unpause();
    emit ProxyUnpaused(_proxy, msg.sender, true);
  }

  function grantRoleAll(bytes32 _role, address _account)
    public
    onlyRole(ADMIN_ROLE)
  {
    uint256 loopLimit = proxyDetails.length < loopProxyCall
  ...
  function revokeRoleAll(bytes32 _role, address _account)
    public
    onlyRole(ADMIN_ROLE)
  {
    uint256 loopLimit = proxyDetails.length < loopProxyCall
    ...
    for (uint256 i = 0; i < loopLimit; i++) {
      address proxy = proxyDetails[i].proxyAdd;
      if (_role == NFT(proxy).ADMIN_ROLE() && _account == address(this)) {
        revert('Restricted: ProxyManager cannot revoke its MINTLAB_ADMIN_ROLE');
      }
  ...
  function revokeRoleSingle(
    address _proxy,
    bytes32 _role,
    address _account
  ) public onlyRole(ADMIN_ROLE) validProxy(_proxy) {
    if (_role == NFT(_proxy).ADMIN_ROLE() && _account == address(this)) {
      revert('Restricted: ProxyManager cannot revoke its MINTLAB_ADMIN_ROLE');
    }
  ...
  function renounceRole(bytes32 role, address account) public override {
    ...
    if (role == ADMIN_ROLE && account == owner()) {
      revert('Restricted: Owner cannot renounce his ADMIN_ROLE');
    }
    if (role == DEFAULT_ADMIN_ROLE && account == owner()) {
      revert('Restricted: Owner cannot renounce his DEFAULT_ADMIN_ROLE');
    }
