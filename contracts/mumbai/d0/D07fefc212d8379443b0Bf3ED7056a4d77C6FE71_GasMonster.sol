...
contract GasMonster is ERC721A, VRFConsumerBaseV2, ChainlinkClient, Ownable {
	...
	function upgrade(bytes32 requestId, bool isUpgraded) public recordChainlinkFulfillment(requestId) {
		...
		// Chainlink VRF call
		emit UpgradeTriggeredFulfillment(requestId, tokenId);
	...
	event UpgradeTriggeredFulfillment(bytes32 requestId, uint256 tokenId);
	...
	function testChainlinkRequest(uint256 tokenId) external onlyOwner {
		require(msg.sender == ownerOf(tokenId), 'Only owner can upgrade NFT');
		require(Tokens[tokenId].level < 7, 'Token had alreay reached max level'); // all requirements take 7314 gas units
