...
contract LendingStorage is AccessControl, ReentrancyGuard, Pausable {
  ...
  address public _transactionWallet; // public to test
  address public _commissionWallet; // public to test
  address public _verifier; // public to test
  address public _currencyToken; // public to test
  uint256 public _commissionPercent;

  mapping(uint256 => Counters.Counter) public _orderIds; // public to test
  mapping(uint256 => mapping(uint256 => OrderERC4907)) public orders; // public to test
  mapping(string => bool) public _orderUUids; // public to test
  address[] public _contractAddresses; // public to test
  uint256 public _percentDecimals;
  uint256 public _maxWaitingTime;
  ...
  function setMaxWaitingTime(uint256 maxWaitingTime) external onlyAdmin {
    _maxWaitingTime = maxWaitingTime * 60;
  }

  function setPercentDecimals(uint256 _decimals) external onlyAdmin {
    require(_decimals > 0 && _decimals <= 10000, "VALUE_INVALID");
    _percentDecimals = _decimals;
  }

  function setTransactionWallet(address _address) external onlyAdmin {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _transactionWallet = _address;
  }

  function setCommisionWallet(address _address) external onlyAdmin {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _commissionWallet = _address;
  }

  function setCurrencytoken(address _token) external onlyAdmin {
    require(_token != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _currencyToken = _token;
  }

  function setCommissionVal(uint256 _value) external onlyMarket {
    _commissionPercent = _value;
  }

  function getCommistionVal() external onlyMarket view returns(uint256) {
    return _commissionPercent;
  }

  function setAddressVerify(address _address) external onlyAdmin {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _verifier = _address;
  }

  function addNFTContractAddress(address[] memory _nftContractAddresses) external onlyAdmin {
    for(uint256 i = 0; i < _nftContractAddresses.length; i++) {
      _contractAddresses.push(_nftContractAddresses[i]);
    }
  }

  function reloadNFTContractAddress(address[] memory _nftContractAddresses) external onlyAdmin {
  ...
  function getOrder(uint256 _nftId, uint8 _nftType) external onlyMarket view returns (OrderERC4907 memory) {
  ...
  function getInfor() onlyMarket external view returns (StorageInfor memory) {
    StorageInfor memory info = StorageInfor({
      ...
      commissionPercent: _commissionPercent / _percentDecimals,
      percentDecimals: _percentDecimals,
      adminVerify: _verifier,
      transactionWallet: _transactionWallet,
