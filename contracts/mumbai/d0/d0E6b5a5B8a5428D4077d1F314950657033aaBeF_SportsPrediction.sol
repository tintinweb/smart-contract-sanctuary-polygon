...
contract SportsPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public intervalSeconds; // interval in seconds between two prediction  match
    ...
    mapping(uint256 => MatchInfo) public fixtureResult;
    ...
    event EndMatch(uint256 indexed fixture, uint256 indexed matchId, int256 price);
    ...
    function claimable(uint256 fixture, address user) public view returns (bool) {
        ...
        if (mm.lockPrice == mm.closePrice) {
            return false;
        }
        return
        betInfo.amount != 0 &&
        !betInfo.claimed &&
        ((mm.closePrice > mm.lockPrice && betInfo.position == Position.Home) ||
        (mm.closePrice < mm.lockPrice && betInfo.position == Position.Draw) ||
        (mm.closePrice < mm.lockPrice && betInfo.position == Position.Away));
    }
     ...
     * @param price: price of the match
     */
    function _safeEndMatch(
        ...
        emit EndMatch(fixture, matchId, mm.closePrice);
    }
     ...
     * @param price: price of the match
     */
    function _safeLockMatch(
        uint256 fixture,
        uint256 matchId,
        int256 price
    ) internal {
        require(matches[fixture].startTimestamp != 0, "Can only lock match after match has started");
        require(block.timestamp >= matches[fixture].lockTimestamp, "Can only lock match after lockTimestamp");
        require(
            block.timestamp <= matches[fixture].lockTimestamp + bufferSeconds,
            "Can only lock match within bufferSeconds"
        );
        Match storage mm = matches[fixture];
        mm.closeTimestamp = block.timestamp + intervalSeconds;
        mm.lockPrice = price;

        emit LockMatch(fixture, matchId, mm.lockPrice);
    }

    /**
     * @notice Start match
     * Previous match n-2 must end
     * @param fixture: fixture
     */
    function _safeStartMatch(uint256 fixture) internal {
        require(genesisStartOnce, "Can only run after genesisStartMatch is triggered");
        require(matches[fixture - 2].closeTimestamp != 0, "Can only start match after match n-2 has ended");
        require(
            block.timestamp >= matches[fixture - 2].closeTimestamp,
            "Can only start new match after match n-2 closeTimestamp"
        );
        _startMatch(fixture);
    }
     ...
     * @param fixture: fixture
     */
    function _startMatch(uint256 fixture) internal {
        Match storage mm = matches[fixture];
        mm.startTimestamp = block.timestamp;
        mm.lockTimestamp = block.timestamp + intervalSeconds;
        mm.closeTimestamp = block.timestamp + (2 * intervalSeconds);
        mm.fixture = fixture;
        mm.totalAmount = 0;
    ...
    function setFixtureResult(uint256 fixture, uint8 result, bool finished) public onlyOracle {
        require(finished, "After match is finished, we can set result");

        fixtureResult[fixture].fixture = fixture;
        fixtureResult[fixture].result = result;
        fixtureResult[fixture].finished = finished;
    }
