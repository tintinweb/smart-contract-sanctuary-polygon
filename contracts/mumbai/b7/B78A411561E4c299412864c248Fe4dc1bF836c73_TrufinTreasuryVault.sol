...
import {
    TrufinTreasuryVaultStorage
} from "../../storage/TrufinTreasuryVaultStorage.sol";
import {ShareMath} from "../../libraries/ShareMath.sol";
import {IWETH} from "../../interfaces/IWETH.sol";
import {GnosisAuction} from "../../libraries/GnosisAuction.sol";
import {IERC20Detailed} from "../../interfaces/IERC20Detailed.sol";

contract TrufinTreasuryVault is
    ReentrancyGuardUpgradeable,
    OwnableUpgradeable,
    ERC20Upgradeable,
    TrufinTreasuryVaultStorage
...
library Vault {
    ...
    struct VaultParams {
        ...
        /// @dev Asset used in Theta / Delta Vault
        address asset;
...
library VaultLifecycleTreasury {
    ...
    struct CloseParams {
        ...
        /// @dev oToken address
        address currentOption;
        /// @dev the delay between commitAndClose and rollToNextOption
        uint256 delay;
        /// @dev the last round the strike price was overridden
        uint16 lastStrikeOverrideRound;
        /// @dev overridden strike price
        uint256 overriddenStrikePrice;
        /// @dev It is the time period between each option sales
        uint256 period;
    }
...
abstract contract TrufinTreasuryVaultStorageV1 {
    /**
...
/**
 * @dev We are following Compound's method of upgrading new contract implementations
 * When we need to add new storage variables, we create a new version of TrufinTreasuryVaultStorage
 * e.g. TrufinTreasuryVaultStorage<versionNumber>, so finally it would look like
 * contract TrufinTreasuryVaultStorage is TrufinTreasuryVaultStorageV1, TrufinTreasuryVaultStorageV2
 */
abstract contract TrufinTreasuryVaultStorage is TrufinTreasuryVaultStorageV1 {
...
interface IWETH {
    ...
    /**
     * @notice Returns the WETH balance of @param account
     */
    function balanceOf(address account) external view returns (uint256);
...
library GnosisAuction {
    ...
    */
    ...
    /**
     * @dev function to get oToken premium in stables
     * @param oTokenAddress the address of the oToken
     * @param optionsPremiumPricer OptionPremiumPricer addres
     * @param premiumDiscount the vault discount applied to the premium
     * @return the premium paid for single option
     */
...
interface ITrufinVault {
    ...
    /**
     * @notice Getter function of struct Vault.VaultParams.cap
     */
    function cap() external view returns (uint256);
    ...
    /**
     * @notice Getter function of struct Vault.VaultParams
     */
...
library DateTime {
    ...
    /**
     * @dev Calculate the number of days from 1970/01/01 to year/month/day using
     ...
     * @return _days number of days from 1970/01/01 to `year`/`month`/`day`
     */
     ...
     * @param _days number of days from 1970/01/01
     * @return year year from `_days` days since 1970/01/01
     * @return month month from `_days` days since 1970/01/01
     * @return day day from `_days` days since 1970/01/01
     */
    ...
    /**
     * @dev returns if the year is leap year
     * @param timestamp is unix timestamp
     * @return leapYear bool true if the year is leap year
     */
    ...
    /**
     * @dev returns how many days the month has
     * @param timestamp is unix timestamp
     * @return daysInMonth how many days the month has
     */
     ...
     * @param timestamp is unix timestamp
     * @return dayOfWeek is the day of the week
     */
    ...
    /**
     * @dev returns the timestamp year
     * @param timestamp is unix timestamp
     * @return year is the year
     */
    ...
    /**
    * @dev return the timestamp month
     * @param timestamp is unix timestamp
     * @return month is the month
     */
    ...
    /**
     * @dev returns the timestamp day
     * @param timestamp is unix timestamp
     * @return day is the day
     */
    ...
    /**
     * @dev returns the timestamp of the date
     * @param year is the year
     * @param month is the month
     * @param day is the day
     * @return timestamp is unix timestamp of the date
     */
...
library DSMath {
    ...
    /**
     * @notice Return the smaller number of x and y.
     */
    ...
    /**
     * @notice Return the larger number of x and y.
     */
    ...
    /**
    * @dev A Ray is a decimal number with 27 digits of precision that is being represented as an integer.
    */
    ...
    /**
     * @notice Multiply two Rays and return a new Ray with the correct level of precision. 
     * @dev rounds to zero if x*y < WAD / 2
     */
    ...
    /**
       @notice Divide two Rays and return a new Ray with the correct level of precision
       @dev rounds to zero if x*y < RAY / 2
     */
    ...
    /**
     * @notice Raise a Ray to the n^th power and return a new Ray with the correct level of precision
