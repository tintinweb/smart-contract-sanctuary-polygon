...
import "./ArtistCollectionImplementation.sol";

contract ArtistCollectionFactory {
    address public immutable implementation;

    event ArtistCollectionCreated(address indexed collection);
    ...
    function createArtistCollection(
        ...
        address artist,
        address payable defaultRoyaltyReceiver,
        uint256 defaultRoyaltyBPS
    ) external returns (address) {
        address clone = Clones.clone(implementation);
        ArtistCollectionImplementation(clone).initialize(
            baseURI,
            contractName,
            tokenSymbol,
            artist,
        ...
        emit ArtistCollectionCreated(clone);
        return clone;
    }
}
...
abstract contract Royalties {
        ...
        returns (address receiver, uint256 amount)
    {
        ...
        return (_receiver, (_getRoyaltyBps(tokenId) * value) / 10000);
    }

    function _getRoyaltyBps(uint256 tokenId) internal view returns (uint256) {
        (address payable receiver, uint256 bps) = _getDefaultRoyalties();
    ...
    {
        (address payable receiver, ) = _getDefaultRoyalties();

        if (receiver != address(0x0)) {
            return receiver;
        }

        return _tokenRoyaltyReceiver[tokenId];
    }
...
// Version: 5.0
contract ArtistCollectionImplementation is
    Initializable,
    ERC721Upgradeable,
    ...
    Royalties
{
    ...
    address payable private _defaultRoyaltyReceiver;
    uint256 private _defaultRoyaltyBPS;
    ...
    function initialize(
        ...
        _defaultRoyaltyReceiver = defaultRoyaltyReceiver;
    ...
    function mintSignable(
        ...
        require(
            _signableMinting == true,
            "Global signable minting must be turned on"
        );
        require(basisPoints < 10001, "Total royalties exceeds 100%");
        uint256 tokenId = getNextTokenId();
        require(
        ...
        address authorizer = Signable.recoverPersonalAddress(
            tokenId,
            _tokenURI,
            v,
            r,
    ...
    function _getDefaultRoyalties()
        ...
        returns (address payable, uint256)
    {
        return (_defaultRoyaltyReceiver, _defaultRoyaltyBPS);
    }
