...
contract UploadTransaction is Ownable {
    ...
    function upload(Model memory record) public onlyController {
       address account = record.account;
       Data[] memory data = record.datas;
       uint256 length = datas.length;
       for(uint256 i=0;i<length;i++){
            accountRecords[account].push(data[i]);
       }
       emit Upload(account,data);
    }
