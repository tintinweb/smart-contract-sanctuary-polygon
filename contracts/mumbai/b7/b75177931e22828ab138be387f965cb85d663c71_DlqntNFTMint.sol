...
import "./modules/VRFPolygonChainlink.sol";

contract DlqntNFTMint is ERC721, ERC2981, ERC721Burnable, ReentrancyGuard, Ownable, Whitelist, VRFPolygonChainlink {
    ...
    constructor() ERC721("DlqntDrop01", "TOKEN") {}

    /*
     * Community API
     * Purpose is for community use.
    */
    ...
    function _safeMint(address to, uint256 _tokenId) internal override nonReentrant {
        require(_maxSupply >= 1, "Developer Configuration: Max supply must be greater or equal to 1");
        require(_tokenId <= _maxSupply, "Mint closed: Maximum supply has been reached.");
        require(_mintActive == true, "Mint closed: Minter is turned off");
        require(msg.value >= _mintPrice, "Insufficient Funds: Please provide enough funds equal to the mint price including gas");
    ...
    /*
     * Owner Functions
     * Purpose is for development use.
    */
...
contract  VRFPolygonChainlink is VRFConsumerBase, Ownable  {
    bytes32 internal keyHash;
    uint256 internal fee;
    
    uint256 private randomResult;
    uint256 private randomnessTolerance = 0; //seconds

    event RandomnessFulfilled(address indexed previousOwner, address indexed newOwner);
    /**
    ...
    function getRandomNumberInRange(uint256 min, uint256 max) public returns(uint256) {
        getRandomNumber();
        uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, randomResult))) % max;
