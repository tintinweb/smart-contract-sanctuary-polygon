...
// File: contracts/IBlatformMarketAuction.sol



pragma solidity ^0.8.0;

interface IBlatformMarketAuction {
    /**
...
// File: contracts/IBlatformRoyaltyRegistry.sol
 ...
 */
interface IBlatformRoyaltyRegistry is IERC1155TokenCreator {
    /**
    ...
    /**
     * @dev Sets the royalty percentage set for an Blatform token
     * Requirements:
...
/**
 * @title Payments contract for Blatform Marketplaces.
 */
...
/**
 * @dev Interface for interacting with the Blatform contract that holds Blatform beta tokens.
 */
interface IBlatform {
...
// File: contracts/BlatformMarketAuction.sol
...
contract BlatformMarketAuction is IBlatformMarketAuction, Ownable, Payments {
    using SafeMath for uint256;
    ...
    IBlatformRoyaltyRegistry public iERC1155CreatorRoyalty;

    // Blatform contract
    IBlatform public blatform;
    //erc1155 contract
     ...
     * @param _blatform address of the Blatform contract
     */
    constructor(address _iMarketSettings, address _iERC1155CreatorRoyalty, address _blatform)
    public
    {
        require(
        ...
        require(
            _blatform != address(0),
            "constructor::Cannot have null address for _blatform"
        );
        ...
        iERC1155CreatorRoyalty = IBlatformRoyaltyRegistry(_iERC1155CreatorRoyalty);

        blatform = IBlatform(_blatform);
        erc1155 = IERC1155(_blatform);
     ...
     * @param _address address of the IMarketplaceSettings.
     */
    function setBlatform(address _address) public onlyOwner {
        require(
            _address != address(0),
            "setBlatform::Cannot have null address for _IBlatform"
        );

        blatform = IBlatform(_address);
    ...
    function setIERC1155CreatorRoyalty(address _address) public onlyOwner {
        ...
        iERC1155CreatorRoyalty = IBlatformRoyaltyRegistry(_address);
    }
    ...
    view
    {
        ...
        require(isOwner || msg.sender == address(blatform), 'sender must be the token owner');
    }
    ...
    function setSalePrice(
        ...
        blatform.setPrice(_amount, _tokenId, _owner);
        // blatform.putOnSale(0, _tokenId, _owner);
        // blatform.setPriceType(0, _tokenId);
        // blatform.setIsForSale(true, _tokenId, _owner);
        emit SetSalePrice(_amount, _tokenId);
    }
     ...
     * @param _oldBlatformAddress get the token ids from the old creato contract.
     * @param _startIndex start index of array
     * @param _endIndex end index of array
     */
    function restore(address _oldAddress, address _oldBlatformAddress, uint256 _startIndex, uint256 _endIndex) external onlyOwner {
        BlatformMarketAuction oldContract = BlatformMarketAuction(_oldAddress);
        IBlatform oldBlatformContract = IBlatform(_oldBlatformAddress);

        uint256 length = oldBlatformContract.getTokenIdsLength();
        require(_startIndex < length, "wrong start index");
        require(_endIndex <= length, "wrong end index");

        for (uint i = _startIndex; i < _endIndex; i++) {
            uint256 tokenId = oldBlatformContract.getTokenId(i);

            address[] memory owners = oldBlatformContract.getOwners(tokenId);
    ...
    function buy(uint256 _tokenId, address _owner) public payable {
        require(blatform.getIsForSale(_tokenId, _owner) == true, "bid::not for sale");
        // The owner of the token must have the marketplace approved
        ownerMustHaveMarketplaceApproved(_owner);
        ...
        uint8 priceType = blatform.getPriceType(_tokenId, _owner);
        ...
        Payments.payout(
            ...
            blatform.getServiceFee(_tokenId),
        ...
        blatform.removeFromSale(_tokenId, _owner);
    ...
    function bid(
        ...
        // Set the new bid.
        _setBid(_newBidAmount, payable(msg.sender), _tokenId, _owner);
        blatform.setBid(_newBidAmount, msg.sender, _tokenId, _owner);
    ...
    internal
    {
        salePrice[_tokenId][_owner] = SalePrice(payable(address(0)), 0);
    }
    ...
    function _setBid(
        ...
        //add bid balance
        uint256 serviceFee = _amount.mul(blatform.getServiceFee(_tokenId)).div(100);
