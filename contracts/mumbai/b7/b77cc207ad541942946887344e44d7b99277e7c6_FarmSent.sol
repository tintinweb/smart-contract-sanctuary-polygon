...
contract FarmSent is Crowdsale, FinalizableCrowdsale, UUPSUpgradeable{
    ...
    */
     ...
     * @param _beneficiary Address performing the token purchase
     */
    function buyToken(address _beneficiary, uint256 _USDTTokenAmount) external onlyWhileOpen whenNotPaused{
      ...
      require(_USDT.balanceOf(_beneficiary) >= _USDTTokenAmount, "USDT Token balance limit exceeded");

      require((_purchasedAmount[_beneficiary][round] + _USDTTokenAmount) <= purchaseLimitInWei, "Maximum purchase Limit exceeded for this round");

      buyTokens(_beneficiary, _USDTTokenAmount);

      // calculate token amount to be created
      uint256 token_amount = _getTokenAmount(_USDTTokenAmount);
      token.safeTransfer(_beneficiary, token_amount);
      _purchasedAmount[_beneficiary][round] += _USDTTokenAmount;
    }
