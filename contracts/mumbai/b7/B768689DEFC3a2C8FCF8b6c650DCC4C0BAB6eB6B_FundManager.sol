...
contract FundManager is EIP712MetaTransaction("FundManager", "1") {
    ...
    mapping(address => mapping(uint256 => Bet)) public betList;
    ...
    function depositDwin(address a, uint256 amount) public {
        dwin.transferFrom(msgSender(), a, amount);
    ...
    function palcebet(
        ...
        counter += 1;
        addressToBalance[msgSender()] -= _stake;
        Bet memory bet = Bet(counter, _odds, _stake * 100, _payout);
        Betlist.push(bet);
        betList[msgSender()][counter] = bet;
        emit BetPlaced(counter, _odds, _stake, _payout);
    }
    ...
    function getbet(address _address, uint256 _id) external view returns (Bet memory) {
        return betList[_address][_id];
    }
}
