...
contract NFTStaking is Ownable, ReentrancyGuard {
    ...
    // save the total rarity of the nfts staked by a certain user
    mapping(address => uint16) public totalRarity;
    ...
    // the numerator of the multiplier is the sum of the nft multipliers
    // the divider is the same for all NFTs
    ...
    function stake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        if (stakes[msg.sender].amountStaked > 0) {
            // if there is an amount staked calculate the rewards and update unclaimed rewards
            uint256 rewards = calculateRewards(msg.sender);
            stakes[msg.sender].unclaimedRewards += rewards;
        } else {

            stakersArray.add(msg.sender);
        }
        ...
        for (uint256 i; i < len; ++i) {        
            ...
            totalRarity[msg.sender] += (IDsRarity[i] + 1 );
    ...
    function unstake(uint256[] calldata _tokenIds) external nonReentrant {
        ...
        stakes[msg.sender].timestampLastUpdate = block.timestamp;
        if (stakes[msg.sender].amountStaked == 0) {
            stakersArray.remove(msg.sender);
        }
    ...
    function claimRewards() external {
        uint256 rewards = calculateRewards(msg.sender) +
            stakes[msg.sender].unclaimedRewards;
     ...
     * @param range : range of stakers whose rewards will be updated
     * @param updateRewards : boolean to choose wether to update the rewards or just update the unclaimed rewards 
     */
    function setRewardsPerHour(
        ...
        address[] memory _stakers = stakersArray.values();

        
        //uint256 len = _stakers.length;
        // update given range of stakers 

        // bounded loop with custom range to not incurr in the gas limit
        for (uint256 i=range[0]; i < range[1]; ++i) {
            address user = _stakers[i];
            stakes[user].unclaimedRewards += calculateRewards(user);
            stakes[msg.sender].timestampLastUpdate = block.timestamp;
        }
        if(updateRewards)
    ...
    /**
     * @dev Get the available rewards for the param _user
     * @param _user Address of the user
    ...
    function availableRewards(address _user) internal view returns (uint256) {
        if (stakes[_user].amountStaked == 0) {
            return stakes[_user].unclaimedRewards;
        }
        uint256 _rewards = stakes[_user].unclaimedRewards + calculateRewards(_user);
        return _rewards;
    }
    ...
    /**
     * @dev Calculate rewards for param _staker
     * Calculate rewards for param _staker by calculating the time passed
     * since last update in hours and mulitplying it to ERC721 Tokens Staked
     * and rewardsPerHour.
     * @param _staker Address of the staker
     * @return _rewards Rewards for param _staker
     */
        ...
        returns (uint256 _rewards)
    {
        ...
        if( endStakingTime < block.timestamp && endStakingTime > _stake.timestampLastUpdate )
            return ( 
                ( 
                (endStakingTime - _stake.timestampLastUpdate) * _stake.amountStaked * rewardsPerHour * (uint256(totalRarity[_staker]) / _stake.amountStaked )
                ) / 3600 / totalNFTStaked
            );
        else
            return (
                (
                (block.timestamp - _stake.timestampLastUpdate) * _stake.amountStaked * rewardsPerHour * ( uint256(totalRarity[_staker]) / _stake.amountStaked )
                ) / 3600 / totalNFTStaked   
            );

        // this could also be written as :
        // block.timestapm - lastUpdateTime * amountStaked * rewardsPerHour * totalRarity as
        // total rarity account for the total amount of NFT staked
    }

    
    /**
     * @dev Change the staking end block for the current period
     * @param _newStakingEndBlock: new end block for the staking  
     */
    function changeStakingEndBlock(uint _newStakingEndBlock) public onlyOwner {
        if(_newStakingEndBlock < block.number)
