...
interface ITokens {
    // Mints a new token
    function mint(address sendTo, uint256 seed) external;
...
contract DrownTheClown is ReentrancyGuard {
    ...
    mapping(address => uint256) gbarBalanceOf;
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost, uint seed) public onlyOwner nonReentrant {
        ...
        // console.log("Game Prize Pool to start: " , houseCut);

        for (uint i = 0; i <= gamePurses.length - 1; i++) {
            if (gamePurses[i] > 0){

                // console.log("game purse: ", gamePurses[i]);
                ...
                // console.log("Referrals Paid: (WA/TP/HO) ", winningAmount, totalPaidAfterRef, houseCut);
                ...
                // console.log("Local Balance Updated to: ", balanceOf[winnerAddresses[i]]);

                // console.log("Total Payment Tokens Won updated to: ", totalPaymentTokenWon);
                ...
                // console.log("Users Profile tokens won updated: ", userProfiles.getTokensWonAmount(winnerAddresses[i], gamePaymentToken));
        ...
        if (sideBetsCheck[triWinners] > 0){
            ...
            // console.log("Winner:", sideBetWinner);
            ...
            // console.log("Side Bet Balance:", winSideBetPoolBal);
            ...
            // console.log("New Side bet pool:", newSideBetPrizePool);

            // Pay the referrals if there are any and update the winning amount as adjusted
            (winSideBetPoolBal, totalPaidAfterRef, houseCut) = payReferrals(winSideBetPoolBal, sideBetWinner, 90);

            // console.log("Referrals Paid: (WA/TP/HO) ", winSideBetPoolBal, totalPaidAfterRef, houseCut);

            // console.log("Paide referrals and returned (SBPB / TP):", winSideBetPoolBal, totalPaidAfterRef);

            // Pay out the winner
            balanceOf[sideBetWinner] +=  winSideBetPoolBal;

            // console.log("total paid winner: ", balanceOf[sideBetWinner]);
            ...
            // console.log("total tokens won: ", totalPaymentTokenSideBetWon);
            ...
            // console.log("Done with side bets");
        }
        ...
        for (uint i = 0; i < players.length; i++) {
            ...
            // console.log("Updating GBAR for", players[i]);

            // Update the players GBAR balance to claim
            gbarBalanceOf[players[i]] +=  gbarsEmit;

            // console.log("Updated GBAR");

            // Update the total amount of GBAR gifted
            totalGBarGifted = totalGBarGifted + gbarsEmit;

            // console.log("total gifted:", totalGBarGifted);
        }
        ...
        // console.log("Checking if we can send NFT: ", tokens.tokensRemaining());

        // Send the NFT if available las as to not get DoS'd
        if (tokens.tokensRemaining() > 0){
            // console.log("Sending NFT to winner");
            tokens.mint(firstPlace, seed);
    ...
    function payReferrals(uint256 winningAmount, address winner, uint256 percentOfTotalToPay) public returns(uint256, uint256, uint256){
        ...
        if (referrers.length > 0) {
            ...
            // console.log("balanceOf[referrers[0]] ", balanceOf[referrers[0]]);
            ...
            // console.log("Total Tokens won for", referrers[0], userProfiles.getTokensWonAmount(referrers[0], gamePaymentToken));

            // Level 2
            if (referrers.length > 1){

                // console.log("Referrer 2: ", referrers[1]);

                refAmountWon = (refCut * 75) / 1000;

                // console.log("Winning amount for level 2: ", refAmountWon);

                totalPaidOut += refAmountWon;

                // console.log("Total Paid out now: ", totalPaidOut);

                // Pay the referrer their 1%
                balanceOf[referrers[1]] += refAmountWon;

                // console.log("balanceOf[referrers[1]] ", balanceOf[referrers[1]]);

                // Update the user's global profile with their referral winnings
                userProfiles.updateTokensWon(referrers[1], gamePaymentToken, refAmountWon);

                // console.log("Total Tokens won for  ", referrers[1], userProfiles.getTokensWonAmount(referrers[1], gamePaymentToken));
            }

            // Level 3
            if (referrers.length > 2){

                // console.log("Referrer 3: ", referrers[2]);

                refAmountWon = (refCut * 25) / 1000;

                // console.log("Winning amount for level 3: ", refAmountWon);
                ...
                // console.log("balanceOf[referrers[2]] ", balanceOf[referrers[2]]);

                // Update the user's global profile with their winnings
                userProfiles.updateTokensWon(referrers[2], gamePaymentToken, refAmountWon);

                // console.log("Total Tokens won for  ", referrers[2], userProfiles.getTokensWonAmount(referrers[2], gamePaymentToken));
            }
        ...
        // console.log("Winning amount for actual winner: ", winningAmount);

        // console.log("Total Paid out: ", totalPaidOut);

        // console.log("House Cut: ", (totalPool - totalPaidOut));

        // console.log("Done paying referrals");
