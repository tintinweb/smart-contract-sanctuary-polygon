...
contract Token3RDS is Roles, ERC20, ERC20Pausable, ERC20Capped {
    constructor(
        uint _initialSupply,
        uint _totalSupply,
    ...
    Roles(_admins)
    {
        require(_initialSupply <= _totalSupply, "3RDS: Invalid params");
     ...
     */
    function mint(address to, uint256 amount) public virtual onlyRole(GUARDIAN_ROLE) {
        _mint(to, amount);
    }
     ...
     */
    function pause() public virtual  onlyRole(GUARDIAN_ROLE) {
        _pause();
    }
     ...
     * - the caller must have the `PAUSER_ROLE`.
     */
    function unpause() public virtual onlyRole(GUARDIAN_ROLE) {
        _unpause();
    }
