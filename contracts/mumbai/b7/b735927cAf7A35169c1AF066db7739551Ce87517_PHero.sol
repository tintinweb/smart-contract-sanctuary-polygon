    ...
    VRFV2WrapperConsumerBase
{
        ...
        ERC721("My Hero For Testing", "MHFT")
    {
        ...
        _counter = PHeroDetails.initCounter();
    }
    ...
    function fulfillRandomWords(
        ...
        // build nft's metadata
        // string memory uri = PHeroDetails.generateMetadata(_tokenIdCounter.current(), detail);
        string memory uri = "";
    ...
    function getCounter() public view returns (PHeroDetails.Counter memory) {
        return _counter;
    }

    function getCounterBy(uint8 opposition) public view returns (PHeroDetails.DecodeCounter memory) {
        if (opposition == PHeroDetails.METAL)
            return PHeroDetails.decodeCounter(_counter.metal);
        else if (opposition == PHeroDetails.CARPENTRY) 
            return PHeroDetails.decodeCounter(_counter.carpentry);
        else if (opposition == PHeroDetails.WATER)
            return PHeroDetails.decodeCounter(_counter.water);
        else if (opposition == PHeroDetails.FIRE)
            return PHeroDetails.decodeCounter(_counter.fire);
        return PHeroDetails.decodeCounter(_counter.earth);
        
    }

    function detailLandBy(uint256 tokenId) public view returns(PHeroDetails.Details memory) {
        uint256 enc = _heroDetails[tokenId];
        return PHeroDetails.decode(enc);
    }
}