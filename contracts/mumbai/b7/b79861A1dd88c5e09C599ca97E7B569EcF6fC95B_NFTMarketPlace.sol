...
contract NFTMarketPlace is ReentrancyGuard {
    ...
    function createItemForSale(
        ...
        //require(msg.value ==  price ,"The Market Fees is 0.010 Ether");
        require(nftContract != address(0), "address should not be equal 0x0");

        // require(checkOwnershipExist(msg.sender),"Main NFT only mint by owner");
        itemId.increment();
...
//     function pausetoken(address from, address to, uint256 tokenId) public 
//     {
//         require(canPause == false, "Paused a token");

//         emit  Paused( from ,  to ,  tokenId);
//     }
...
//        paused = _paused;
//    }
    // function addOwnerInList(address newAddress) public payable nonReentrant {

    //     require(owner == address(newAddress), "only contract owner can add the sub owners");
    //     ownerList.increment();
    //     uint256 id = ownerList.current();
    //     contractOwnerList[id] = payable(newAddress);
    // }

    // function checkOwnershipExist(address addressToCheck) private  view returns (bool isExist){
    //     uint256 totalItemCount = ownerList.current();

    //      isExist = false;

    //     for (uint256 i = 0; i < totalItemCount; i++) {
    //         if (contractOwnerList[i + 1] == payable(addressToCheck)) {
    //             isExist = true;
    //         }
    //     }
    //     return isExist;
    ...
    // function convertPriceToMatic(uint256 mainId,  uint256 inrPrice,uint256 percentage) public  view returns (uint256 price){
    //     price =   idForMarketItem[mainId].price/inrPrice*percentage/100;
    //     return price;
    // }
    ...
    // For fraction create
    function createItemForFractionSale(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        uint256 percentage,
        ...
        //require(msg.value ==  price ,"The Market Fees is 0.010 Ether");
        require(nftContract != address(0), "address should not be equal 0x0");
        //uint256 userEnteredPrice=convertPriceToMatic(mainId,inrPrice,percentage);
        //require(msg.value >=userEnteredPrice, "Price should be same as  NFT price");
        uint256 mainPercentange = idForMarketItem[mainId].ownedPercentage;
        uint256 newPercentange = mainPercentange + percentage;        
        require(
            newPercentange <= 100,
            "Percentage should be less than total owned percentage"
        );
        itemId.increment();
        uint256 id = itemId.current();
        idForMarketItem[mainId].seller.transfer(msg.value);
        ...
        idForMarketItem[mainId].ownedPercentage = newPercentange;
        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId); //buy
        itemsSold.increment();
  ...
  // For fraction create for NEFT and RTGS feature
    function createItemForTrasnferFractionSale(
        address nftContract,
        uint256 tokenId,
        uint256 price,
        uint256 percentage,
        ...
        idForMarketItem[mainId].ownedPercentage = newPercentange;
        IERC721(nftContract).transferFrom(address(this), receiver, tokenId); //buy
        itemsSold.increment();
    ...
    // function getOwnerName() public view returns (address) {
    //     return owner;
    // }
