...
contract Sale is Ownable2Step {
    ...
    uint32 private constant _DIVISOR = 100_000;

    uint32 public fee;

    // The addresses of the price setting accounts
    address private _approvedAccountAddress; 
    ...
    constructor(
        ...
        fee = uint32(fee_);
        _approvedAccountAddress = approvedAccount;
    }
     ...
     * @param nftAmount The amount of fractionalized nfts to buy
     */
    event Bought(
    ...
    /**
     * @notice Emit when the owner successfuly set price for each fractionalized nft of a give sale id
     * @param saleId The id of the sale
    ...
    /**
     * @notice Emit when the seller successfuly cancel a sale
     * @param saleId The id of the sale
     */
    event SaleCancelled(uint256 indexed saleId);

    /**
     * @notice Emit when the owner update the fee successfully
    ...
    modifier onlyApproved() {
        require(msg.sender == _approvedAccountAddress, "Not Approved Account");
        _;
    }
     ...
     * Will transfer unsold nfts from the treasury back to the seller on success
     * @param saleId The id of the sale
     */
    function cancelSale(
        uint256 saleId
    ...
    /**
     * @notice Buy nfts from a particular sale id
     * @param saleId The id of the sale
     * @param nftAmountToBuy The amount of nfts to buy
     */
    function buy(
        uint256 saleId,
        ...
        SaleInfo storage saleIdToSaleInfo_ = saleIdToSaleInfo[saleId];
        if (saleIdToSaleInfo_.isForBullion) {
            if (msg.sender != owner()) revert OnlyBullionCanBuy();
        }
        ...
        /// Update the amount of nfts nftAmountSold
        unchecked {
        ...
        _TOKEN.safeTransferFrom(msg.sender, address(_TREASURY), systemFee);
    ...
    /**
     * @notice Buy nfts from a particular sale id as Bullion
     * @param saleId The id of the sale
     * @param nftAmount The amount of nfts to buy
     */
    function buyAsBullion(uint256 saleId, uint256 nftAmount) external onlyOwner {
        if (saleId > saleIdCounter) revert SaleDoesNotExsit();
        ...
        uint256 available;
        unchecked {
            available = saleIdToSaleInfo_.nftAmountToSell - saleIdToSaleInfo_.nftAmountSold;
        }
        if (nftAmount > available) revert NotEnoughNftsToBuy();

        uint256 nftPrice = saleIdToSaleInfo_.nftPrice;
        if (nftPrice == 0) revert PriceIsNotSet();

        uint256 payment = nftPrice * nftAmount;
        /// Note: Solidity will round down in divisions. e.g. 2/3 will result in 0
        uint256 systemFee = (payment * fee) / _DIVISOR;

        /// Update the amount of nfts nftSold
        unchecked {
            saleIdToSaleInfo_.nftAmountSold += uint64(nftAmount);
        }

        /// Reset the nftPrice
        delete saleIdToSaleInfo_.nftPrice;

        /// Keep system fee in the treasury, 
        /// Transfer the rest to the seller from the treasury
        _TOKEN.safeTransferFrom(
            address(_TREASURY),
            saleIdToSaleInfo_.seller,
            payment - systemFee
    ...
    */
    function updateApprovedAccount(address _toApprove) external onlyOwner {
      _approvedAccountAddress = _toApprove;
    }
     ...
     * @return _approvedAccountAddress
     */
    function getApprovedAccountAddress() external view onlyOwner returns (address) {
        return _approvedAccountAddress;
    }
}
...
contract Treasury is Ownable2Step, ERC1155Holder {
    ...
    function withdraw(uint256 _amountUSDC) external onlyOwner isActive {
        ...
        if (_USDC.balanceOf(address(this)) < _amountUSDC)
    ...
    //   _USDC.safeTransfer(owner(), _USDC.balanceOf(address(this)));
      ...
      for (uint256 i; i < sale.saleIdCounter(); ++i) { 
        ( ,,
        address seller,
        address collectionAddress,
        uint64 nftId,
        uint64 nftAmount,
        uint64 nftSold, ) = sale.saleIdToSaleInfo(i);
