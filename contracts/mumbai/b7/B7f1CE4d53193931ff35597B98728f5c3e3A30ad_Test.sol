...
contract Test is ERC721A, Ownable, Pausable, ERC2981 {
    uint256 public immutable collectionSize;
    uint256 public immutable freezeDuration;
    string private baseTokenURI;
    string public provenanceHash;

    event TransferAfterLock(address indexed from, address indexed to, uint256 indexed tokenId);
    ...
    function _beforeTokenTransfers(address from, address to, uint256 tokenId, uint256 quantity) internal whenNotPaused override(ERC721A) {
        ...
        if (this.lockedUntil(tokenId) > 0) {
            ...
            emit TransferAfterLock(from, to, tokenId);
        } else {
            if (from != address(0)) {
