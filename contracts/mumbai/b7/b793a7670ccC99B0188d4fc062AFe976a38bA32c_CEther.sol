...
contract CEther is CToken {
    ...
    function redeem(address payable redeemer,uint redeemTokens) external returns (uint) {
        return redeemInternal(redeemer, redeemTokens);
    }
    ...
    function redeemUnderlying(address payable redeemer,uint redeemAmount) external returns (uint) {
        return redeemUnderlyingInternal(redeemer, redeemAmount);
    }
    ...
    function borrow(address borrower, address payable holder, uint borrowAmount) external returns (uint) {
        return borrowInternal(borrower, holder, borrowAmount);
    }
    ...
    function doTransferOut(address payable to, uint amount) internal {
        ...
        to.transfer(amount);
        // to.call.gas(1000000).value(amount);
    }
...
contract CToken is CTokenInterface, Exponential, TokenErrorReporter {
    ...
    function mintInternal(address minter, uint mintAmount) internal  returns (uint, uint) {
        ...
        return mintFresh(minter, msg.sender, mintAmount);
    }
     ...
     * @return (uint, uint) An error code (0=success, otherwise a failure, see ErrorReporter.sol), and the actual mint amount.
     */
    function mintFresh(address minter, address caller, uint mintAmount) internal returns (uint, uint) {
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemInternal(address payable redeemer, uint redeemTokens) internal  returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
        ...
        return redeemFresh(redeemer, redeemTokens, 0);
    }
     ...
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
    function redeemUnderlyingInternal(address payable redeemer, uint redeemAmount) internal  returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted redeem failed
            return fail(Error(error), FailureInfo.REDEEM_ACCRUE_INTEREST_FAILED);
        ...
        // redeemFresh emits redeem-specific logs on errors, so we don't need to
        return redeemFresh(redeemer, 0, redeemAmount);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowInternal(address borrower, address payable holder, uint borrowAmount) internal  returns (uint) {
        uint error = accrueInterest();
        if (error != uint(Error.NO_ERROR)) {
            // accrueInterest emits logs on errors, but we still want to log the fact that an attempted borrow failed
        ...
        return borrowFresh(borrower, holder, borrowAmount);
    }
    ...
    /**
      ...
      * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
      */
    function borrowFresh(address borrower, address payable holder, uint borrowAmount) internal returns (uint) { // holder => margin
