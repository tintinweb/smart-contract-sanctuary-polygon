...
contract Foo {
    ...
    function sendCall(uint32 _nonce, uint16 _destination, address _to, bytes calldata _data) external {
        bytes memory _payload = abi.encode(MessageType.Call, _destination, _to, _data);
    ...
    function verifyVm(bytes memory _vm) public view returns (bytes memory, uint16, uint16, bytes memory, address, uint8) {
        ...
        return (_dvm.payload, _destination, chainId, _data, _to, uint8(_type));
    }
