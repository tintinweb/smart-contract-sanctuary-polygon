...
contract MajrIDPolygon is ERC20Splitter, ERC721A, Pausable {
   ...
   * @param _price uint256
   * @dev Only owner can call it and only when the contract is paused and the price can't be zero
   */
  function setPrice(uint256 _price) external onlyOwner whenPaused {
    require(_price > 0, "MajrIDPolygon: Mint price must be greater than zero.");
  ...
  function mint(address to, uint256 quantity) external whenNotPaused {
    require(
      ...
      "MajrIDPolygon: Not enough tokens to mint."
    );
    ...
    require(sent, "MajrIDPolygon: Failed to send tokens.");
  ...
  function mintWithReferrer(
    ...
    require(
      to != referrer,
      "MajrIDPolygon: Cannot mint with the recipient as the referrer."
    );
  ...
  function burn(uint256 _tokenId) external whenNotPaused {
    require(
      ...
      "MajrIDPolygon: You do not own this token."
    );
  ...
  function tokensOfUserIn(
    ...
    require(start >= 0 && start < stop, "MajrIDPolygon: Invalid query range.");
