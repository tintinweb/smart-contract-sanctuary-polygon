...
contract AJMarketplace is ReentrancyGuard {
    ...
    mapping (uint256 => mapping(uint8 => address[])) public  allBidersAddress;  // NFT->AuctionIndex->UserAddress
    address public WETH;
    struct AuctionData {
        uint256 nftId;
        address seller;
   ...
   // --------------------------------- Auction Start ------------------------------------------ //
    ...
    function getBidderAddress(uint256 _nftId, uint8 _auctionIndex) public view returns(address [] memory){
        return allBidersAddress[_nftId][_auctionIndex];
    }
    ...
    function bid(uint256 _nftId, uint256 _payAmount) external {
        ...
        (bool result, ) = isAddressInArray(allBidersAddress[_nftId][auctionIndex], msg.sender);
        ...
        if (!result){
            allBidersAddress[_nftId][auctionIndex].push(msg.sender);
        }
    }
    ...
    function getAllBidders(uint256 _nftId) public view returns(address[] memory){
        uint8 auctionIndex = nftAuctionCount[_nftId];
        return allBidersAddress[_nftId][auctionIndex];
    }
    ...
    function transferToHigherBidder(uint256 _nftId) external  {
        ...
        if (AuctionDataset[_nftId].highestBidder != address(0)) {
            ...
            for(uint256 i=0; i<allBidersAddress[_nftId][auctionIndex].length; i++){
                if (allBidersAddress[_nftId][auctionIndex][i] != AuctionDataset[_nftId].highestBidder){
                    // Transfer 
                    address userAddress = allBidersAddress[_nftId][auctionIndex][i];
                    IWETH(WETH).transfer(allBidersAddress[_nftId][auctionIndex][i], bids[_nftId][userAddress]);
                    bids[_nftId][allBidersAddress[_nftId][auctionIndex][i]] = 0;
                }else{
        ...
        AuctionDataset[_nftId].started = false;
    }
