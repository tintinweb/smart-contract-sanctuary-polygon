...
import "./Access.sol";
import "./IYadeckCards.sol";
import "./IYadeckTokens.sol";
import "./NftLib.sol";
    ...
    IYadeckTokens
{
    ...
    IYadeckCards public yadeckCards;
    INextContract public nextContract;

    constructor(address yadeckCardsAddress, address trustedForwarder)
        ERC2771Context(trustedForwarder)
        ERC721("Yadeck", "YDK")
    {
        require(
            yadeckCardsAddress != address(0),
            "YadeckCards address is required"
        );
        yadeckCards = IYadeckCards(yadeckCardsAddress);

        // Set 10% default fees in bips
    ...
    {
        return
            interfaceId == type(IYadeckTokens).interfaceId ||
    ...
    function createCardAndMintToken(Card memory card, address to)
        public
        override
        onlyMinter
        returns (uint256)
    {
        uint256 cardId = yadeckCards.createCard(card);
    ...
    function mintToken(uint256 cardId, address to)
        ...
        returns (uint256)
    {
        require(yadeckCards.cardExists(cardId), "Card does not exist");
    ...
    function evolution(
        ...
        uint256 newCardId = yadeckCards.evolveCard(
            cardId,
            metadata,
            status,
            certification
...
contract CapperAccess is Ownable, AccessControl {
    ...
    function revokeCapper(address account) external {
        revokeRole(CAPPER_ROLE, account);
    }
}
...
interface IYadeckCards {
    function createCard(Card calldata card) external returns (uint256);

    function getCard(uint256 _cardId) external view returns (Card memory card);

    function getArtist(uint256 artistId)
        external
        view
        returns (string memory name, string memory dept);
...
///  Note: the ERC-165 identifier for this interface is 0x47ca7504
interface IYadeckTokens {
    function createCardAndMintToken(Card memory card, address to)
        external
        returns (uint256);
...
library NftLib {
    ...
    function requireERC721(address _candidate) public view {
        require(
            IERC721(_candidate).supportsInterface(interfaceIdERC721),
            "IS_NOT_721_TOKEN"
        );
    }
    ...
    function evolveNft(
        ...
        uint256 tokenId,
        uint8 newTokenStatus,
        uint8 newTokenCertification,
        address signer,
        bytes memory signature
        ...
        require(
            nftContract.ownerOf(tokenId) == signer,
            "Signer is not the token owner"
        );
    ...
    function getMessageHash(
        ...
        return
            keccak256(
                abi.encodePacked(tokenId, newTokenStatus, newTokenCertification)
            );
    }
