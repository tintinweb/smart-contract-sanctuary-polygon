...
contract GaiaLand is ERC721, Ownable {
  ...
  uint256 public gaiaGoodLandPrice = 75e13;

  uint256 public gaiaRegularLandPrice = 25e13;
  ...
  uint256 public gaiaLandReserve = 1550;

  mapping(uint256 => uint256) public price;
  mapping(uint256 => uint256) public mintMap;
  ...
  address private immutable WMATICAddress = 0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1;
  ...
  function withdraw() external onlyOwner {
    ...
    uint256 gaiaBalance = IERC20(WMATICAddress).balanceOf(address(this));
    IERC20(WMATICAddress).transfer(ownerAddress, gaiaBalance);
    ...
    require(
      msg.value >=
        (gaiaGoodLandPrice.mul(numberOfGoodLands) +
          gaiaRegularLandPrice.mul(numberOfRegularLands)),
  ...
  function mintGaiaLandToken(
    ...
    uint256 tPrice = gaiaGoodLandPrice.mul(numberOfGoodLands) +
      gaiaRegularLandPrice.mul(numberOfRegularLands);
    ...
    require(
      IERC20(WMATICAddress).transferFrom(msg.sender, address(this), tPrice.mul(numberOfRate).div(1000))
    );
  }
  ...
  function closeTrade(uint256 _id) external {
    require(ownerMap[_id] == msg.sender, 'sender is not owner');
    require(listedMap[_id] == true, 'Already closed');
    delete listedMap[_id];
    delete auctions[_id];
  }

  function buy(uint256 _id, uint256 _price) external payable {
    _validate(_id);
    require(listedMap[_id] == true, 'Already closed');
  ...
  function buyToken(uint256 _id, uint256 _price) external {
    ...
    require(
      IERC20(WMATICAddress).transferFrom(
        msg.sender,
        address(this),
        _commissionValue
      )
  ...
  function transferMATICOrWMATIC(address payable _to, uint256 value) private {
    if (!attemptMATICTransfer(_to, value)) {
      IWMATIC(WMATICAddress).deposit{value: value}();
      require(IWMATIC(WMATICAddress).transfer(_to, value));
    }
  }

  function attemptMATICTransfer(address payable _to, uint256 value)
    private
    returns (bool)
  {
    (bool success, ) = _to.call{value: value, gas: 30000}('');
    return success;
  }

  function _validate(uint256 _id) internal view {
    require(listedMap[_id] == true, 'Item not listed currently');
