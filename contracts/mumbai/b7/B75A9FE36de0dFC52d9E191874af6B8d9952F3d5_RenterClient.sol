...
contract RenterClient is Context, IRentingContract, Ownable, ReentrancyGuard {
...
//    uint private constant RENTING_RENEWAL_PERIOD_GAP = 4 hours;
    ...
    function rentBattleSet(uint256 landId) nonReentrant external returns (bool) {
    ...
    function rentFromCollection(uint256 id, uint256 landId, uint256[] memory botsIds) nonReentrant external returns (bool) {
        ...
        storageContract.createRenting(bs, collection.rentingType, collection.chargeCoin, collection.price, collection.owner, _msgSender(),
            block.timestamp + DEFAULT_RENTING_DURATION, collection.id, collection.perpetual, new address[](0), collection.revenueShare);
    ...
    function renewRental(uint256 landId) nonReentrant external returns (bool){
