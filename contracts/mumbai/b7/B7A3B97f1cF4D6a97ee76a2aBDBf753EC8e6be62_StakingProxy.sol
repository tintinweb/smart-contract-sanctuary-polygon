...
contract StakingProxy is AccessControlEnumerable, ReentrancyGuardUpgradeable {
    ...
    event Claimed(address user, uint256 poolId, uint256 round, uint256 tokenAmount, uint256 rewardAmount);
    ...
    function createPool(
        ...
        require(_daysPerRound != 0, Error.DAY_PER_ROUND_MUST_NOT_EQUAL_ZERO);
        ...
        uint256 totalReward = (_maxPoolStake * (_cliff + _releaseTotalRounds * _daysPerRound) * _apr) /
            (daysOfYear * 1 ether);

        require(
            IERC20(_rewardAddress).allowance(_msgSender(), address(this)) >= totalReward,
            Error.REWARD_TOKEN_IS_NOT_ALLOWED_BY_ADMIN
        );
        require(
        ...
        uint256 newPoolId = _pools.length;
     ...
     */
    function _getActivePools() internal view returns (StakePool[] memory) {
        StakePool[] memory activePools = new StakePool[](_getCountActivePools());
        uint256 count = 0;
    ...
    function stake(uint256 _poolId, uint256 _amount) external nonReentrant {
        ...
        require(
            stakeInfo.amount == 0 || stakeInfo.unStakeTime > 0 || stakeInfo.releasedRounds == pool.releaseTotalRounds,
            Error.DUPLICATE_STAKE
        );
        ...
        require(
            IERC20(pool.stakeAddress).allowance(_msgSender(), address(this)) >= _amount,
            Error.TOKEN_IS_NOT_ALLOWED_BY_USER
        );
        require(
        ...
        uint256 reward = (_amount * (pool.cliff + pool.releaseTotalRounds * pool.daysPerRound) * pool.apr) /
            (daysOfYear * 1 ether);

        // 07:00 UTC next day
        uint256 valueDate = (block.timestamp / 1 days) * 1 days + 1 days + 7 hours;

        stakeInfo = StakeInfo(_poolId, block.timestamp, valueDate, _amount, reward, 0, 0);
    ...
    function _getCountStakeAvailable(address _user) internal view returns (uint256 count) {
        ...
        for (uint256 i = 0; i < _stakeHistories[_user].length; i++) {
            if (
                _stakeHistories[_user][i].unStakeTime == 0 &&
                _stakeHistories[_user][i].releasedRounds < _pools[_stakeHistories[_user][i].poolId].releaseTotalRounds
            ) count++;
        }
    }

    function getStakeAvailableList(address _user) external view returns (StakeInfo[] memory stakeAvailableList) {
        stakeAvailableList = new StakeInfo[](_getCountStakeAvailable(_user));
        uint256 count = 0;
        ...
        for (uint256 i = 0; i < _stakeHistories[_user].length; i++) {
            if (
                ...
                stakeAvailableList[count++] = _stakeHistories[_user][i];
            }
        }
    }

    function _getRewardClaimable(uint256 _poolId, address _user)
        internal
        view
        returns (
            uint256 releaseRound,
            uint256 releaseTokenAmount,
            uint256 releaseRewardAmount
        )
    {
        StakeInfo memory stakeInfo = _stakeInfo[_poolId][_user];
        StakePool memory pool = _pools[_poolId];

        if (stakeInfo.amount == 0 || stakeInfo.unStakeTime != 0 || stakeInfo.releasedRounds == pool.releaseTotalRounds)
            return (0, 0, 0);
        if (stakeInfo.valueDate + pool.cliff > block.timestamp) return (0, 0, 0);

        uint256 lockedDays = ((block.timestamp - stakeInfo.valueDate) / 1 days) + 1;
        uint256 roundsPassed = ((lockedDays - pool.cliff) / pool.daysPerRound) + 1;

        if (roundsPassed > pool.releaseTotalRounds) roundsPassed = pool.releaseTotalRounds;

        if (stakeInfo.releasedRounds == roundsPassed) return (roundsPassed, 0, 0);

        releaseRound = roundsPassed;
        releaseTokenAmount = ((roundsPassed - stakeInfo.releasedRounds) * stakeInfo.amount) / pool.releaseTotalRounds;
        releaseRewardAmount =
            ((roundsPassed - stakeInfo.releasedRounds) * stakeInfo.totalReward) /
            pool.releaseTotalRounds;
    }

    function getRewardClaimable(uint256 _poolId, address _user)
        external
        view
        returns (
            uint256 releaseRound,
    ...
    function unStake(uint256 _poolId) external nonReentrant {
        ...
        require(
            stakeInfo.amount > 0 && stakeInfo.unStakeTime == 0 && stakeInfo.releasedRounds < pool.releaseTotalRounds,
            Error.NOTHING_TO_UN_STAKE
        );

        (, uint256 releaseTokenAmount, uint256 releaseRewardAmount) = _getRewardClaimable(_poolId, _msgSender());

        require(releaseTokenAmount == 0 && releaseRewardAmount == 0, Error.HAVE_NOT_CLAIM_REWARD_BEFORE_UN_STAKE);

        uint256 claimedTokenAmount = (stakeInfo.releasedRounds * stakeInfo.amount) / pool.releaseTotalRounds;
        uint256 claimedRewardAmount = (stakeInfo.releasedRounds * stakeInfo.totalReward) / pool.releaseTotalRounds;

        require(
            IERC20(pool.stakeAddress).transfer(_msgSender(), stakeInfo.amount - claimedTokenAmount),
            Error.TRANSFER_TOKEN_FAILED
        );

        _pools[_poolId].totalStaked -= (stakeInfo.amount - claimedTokenAmount);

        _stakedAmounts[pool.stakeAddress] -= (stakeInfo.amount - claimedTokenAmount);
        _rewardAmounts[pool.rewardAddress] -= (stakeInfo.totalReward - claimedRewardAmount);

        _stakeHistories[_msgSender()][_stakeHistoryIndex[_poolId][_msgSender()]].unStakeTime = block.timestamp;
    ...
    function claim(uint256 _poolId) external nonReentrant {
        ...
        (uint256 releaseRound, uint256 releaseTokenAmount, uint256 releaseRewardAmount) = _getRewardClaimable(
            _poolId,
            _msgSender()
        );

        require(releaseTokenAmount > 0 || releaseRewardAmount > 0, Error.NOTHING_TO_CLAIM);

        if (pool.stakeAddress == pool.rewardAddress) {
            require(
                IERC20(pool.rewardAddress).transfer(_msgSender(), releaseTokenAmount + releaseRewardAmount),
                Error.TRANSFER_REWARD_FAILED
            );
        } else {
            require(IERC20(pool.stakeAddress).transfer(_msgSender(), releaseTokenAmount), Error.TRANSFER_TOKEN_FAILED);
            require(
                IERC20(pool.rewardAddress).transfer(_msgSender(), releaseRewardAmount),
                Error.TRANSFER_REWARD_FAILED
            );
        }

        _stakedAmounts[pool.stakeAddress] -= releaseTokenAmount;
        _rewardAmounts[pool.rewardAddress] -= releaseRewardAmount;

        _stakeInfo[_poolId][_msgSender()].releasedRounds = releaseRound;
        _stakeHistories[_msgSender()][_stakeHistoryIndex[_poolId][_msgSender()]].releasedRounds = releaseRound;

        emit Claimed(_msgSender(), _poolId, releaseRound, releaseTokenAmount, releaseRewardAmount);
    }

    /**
        @dev all token in all pools holders staked
     */
    function getTotalStakedAmount(address _tokenAddress) external view returns (uint256) {
     ...
     */
    function getTotalRewardAmount(address _tokenAddress) external view returns (uint256) {
     ...
     */
    function withdrawERC20(address _tokenAddress) external nonReentrant onlyAdmin {
        uint256 balances = IERC20(_tokenAddress).balanceOf(address(this));
        require(balances > 0, Error.NOTHING_TO_WITHDRAW);
        ...
        require(IERC20(_tokenAddress).transfer(_msgSender(), balances), Error.TRANSFER_TOKEN_FAILED);
    }
}
...
library Error {
    ...
    string public constant DAY_PER_ROUND_MUST_NOT_EQUAL_ZERO = "Error: Day per round must be not equal 0";
    ...
    string public constant REWARD_TOKEN_IS_NOT_ALLOWED_BY_ADMIN = "Error: Reward token is not allowed by admin";
    string public constant TOKEN_IS_NOT_ALLOWED_BY_USER = "Error: Reward token is not allowed by user";
    ...
    string public constant HAVE_NOT_CLAIM_REWARD_BEFORE_UN_STAKE = "Error: Have not claim reward before un stake";

    string public constant NOTHING_TO_CLAIM = "Error: Nothing to claim";
    string public constant NOTHING_TO_UN_STAKE = "Error: Nothing to un stake";
