...
contract Keeper48 is AutomationCompatibleInterface, Ownable {
    ...
    function checkUpkeep(
        ...
        int d = 1e18;
        while (i < _n - 1) {
            points[i] = d; //oracle.getAmountOracle(oracleTimestamp) + d;

            d += 1e15;
            console.log(uint(points[i]));
            oracleTimestamp += 3600;
            i++;
        }

        points[i] = 1e18; //oracle.getAmountOracle(block.timestamp);

        console.log(1);

        (int256 std, int256 mu) = points.calculateStd();

        console.log(uint(std), uint(mu));
