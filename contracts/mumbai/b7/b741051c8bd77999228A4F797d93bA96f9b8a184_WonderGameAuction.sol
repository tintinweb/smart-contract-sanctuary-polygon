...
contract WonderGameAuction is Ownable, INFTMintInitiator {
    ...
    constructor(IWonderGameCharacterInventory _wonder,IWonderGameMinter _nftMinter ,address _treasury)
    {
        wonderGame = _wonder;
    ...
    function acknowledgeMint(uint256 _requestId,
        address _user,
        uint256[] memory _tokenIds) external override {
    ...
    function _startNewAuction () internal {
        require(
            ...
            "Previous auction period not ended"
        );
        ...
        requestIds[requestId] = auctionId;
    }
    ...
    function getBidHistoryById(
        ...
        for (uint256 i = start; i <= endindex; i++) {
            History[i] = auctionRegistry[_auctionid].bidHistorybyId[i];
        }
        return History;
    }
