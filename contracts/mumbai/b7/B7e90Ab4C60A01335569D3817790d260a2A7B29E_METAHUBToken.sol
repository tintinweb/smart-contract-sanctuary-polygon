...
contract METAHUBToken is IBEP20, TokenAuth {
  ...
  mapping (address => uint) public lastReleaseAdvisorsAndTeamBlock; // TODO remove on next deployment:
  ...
  uint public stakingReleased; // TODO change to developmentReleased on net deployment
...
//  }
  ...
  function releaseMintingAllocation(uint _amount) external onlyVault returns (bool) {
    require(stakingReleased + _amount <= maxMintingBeMinted, "Max staking allocation had released");
    stakingReleased += _amount;
...
contract TokenAuth is Context, Initializable {
  ...
  IVault public vault;
  mapping (address => uint) public advisorAddresses; // TODO remove on next deployment
  uint private advisorAllocated; // TODO remove on next deployment
  uint private founderAllocated; // TODO remove on next deployment
