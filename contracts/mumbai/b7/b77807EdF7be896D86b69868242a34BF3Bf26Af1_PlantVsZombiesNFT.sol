...
contract FactoryPlant is VRFConsumerBaseV2, Ownable, Pausable {
    ...
    function _randomNumber(
        ...
        uint16 _result = min +
            uint16((requestRandomNumber + block.timestamp) % (max - min + 1));
        return _result;
    }

    function updatePlantRule(
        uint16[3][5] memory _newPlantRule
    ) public onlyOwner whenPaused {
        plantRule = _newPlantRule;
        emit UpdatePlantRule(_newPlantRule);
    }

    function getPlantRule()
        public
        view
        onlyOwner
        returns (uint16[3][5] memory)
    {
        return plantRule;
    }

    function _randomPropertiesPlant(
        bool isVip
    ) internal returns (uint16[5] memory) {
        ...
        for (uint _i = 0; _i < 5; _i++) {
            if (propertieHigh[_i] != isVip) {
                plantProperties[_i] = _randomNumber(
                    plantRule[_i][1],
                    plantRule[_i][2],
                    requestRandomNumber
                );
