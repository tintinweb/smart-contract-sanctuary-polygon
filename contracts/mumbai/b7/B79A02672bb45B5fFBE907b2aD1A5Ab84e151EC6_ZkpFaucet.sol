...
contract ZkpFaucet is Claimable, ImmutableOwnable {
    ...
    modifier validatePrice() {
        require(
            maxAmountToPay == 0 || msg.value <= maxAmountToPay,
            "High value"
        );
    ...
    /**
     * @notice update the amount that can be received by users
     * @param _maxAmountToPay the amount that can be paid by users
     */
    function updateMaxAmountToPay(uint256 _maxAmountToPay) external onlyOwner {
        maxAmountToPay = _maxAmountToPay;
    }
