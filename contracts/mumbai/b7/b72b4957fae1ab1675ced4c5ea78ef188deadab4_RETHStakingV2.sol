...
contract RETHStaking is SMAuth, ReentrancyGuard  {
    ...
    uint256 private constant ONE_MONTH_SEC = 300 ; //2592000;
    ...
    function _stake(uint256 amount, uint256 months) private {
        ...
        uint256 duration = block.timestamp.add( months.mul(300 days));   
    ...
    function _unstake(uint256 stakeId) private {
        ...
        uint256 gtreward = getTotalRewards(msg.sender, stakeId) > Stakes[msg.sender][stakeId].claimed ? getTotalRewards(msg.sender, stakeId) : Stakes[msg.sender][stakeId].claimed;
    ...
    function addStakes(address owner, uint256 amount, uint256 st, uint256 et, uint256 m, bool collected, uint256 claimed) public onlyAuth {
        userstakes[owner]++;
        Stakes[owner].push(stakes(owner, amount, st, et, m, collected, claimed));
        emit StakingUpdate(owner, amount, st, et, false, claimed);
    }

}

contract RETHStakingV2 is RETHStaking {
    RETHStaking public STAKINGV1;
    mapping(address=>bool) private migrated;
    event LogMigration(stakes stake);
    constructor(IERC20 add_) RETHStaking(add_){}

    function migrateStakes(address[] memory wallet) public onlyAuth {
        for(uint256 i=0;i<wallet.length;i++){
            stakes[] memory staked = STAKINGV1.getStakes(wallet[i]);
            for(uint256 j=0;i<staked.length;j++){
                Stakes[wallet[i]].push(staked[j]);
                emit LogMigration(staked[j]);
            }
        }
    }

    function checkStatus(address wallet) public view onlyAuth returns(bool) {
