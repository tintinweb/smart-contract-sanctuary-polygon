...
contract DSponsor is AccessControl, ContextMixin, IDSponsor {
  ...
  bytes32 public constant SET_PROPERTIES_ROLE = keccak256('SET_PROPERTIES_ROLE');
  ...
  modifier onlySponsor(uint256 tokenId) {
    if (_msgSender() != NFT_CONTRACT.ownerOf(tokenId)) revert UnallowedSponsorOperation();
    _;
  }
   ...
   */
  constructor(IERC721 ERC721Contract, string memory rulesURI, address sponsee) {
    if (sponsee == address(0)) revert SponseeCannotBeZeroAddress();

    try ERC721Contract.supportsInterface(0x80ac58cd) returns (bool) {} catch (bytes memory) {
   ...
   */
  function setProperty(string memory propertyString, bool allowed) external onlyRole(SET_PROPERTIES_ROLE) {
   ...
   */
  function setSponsoData(uint256 tokenId, string memory property, string memory data) external onlySponsor(tokenId) {
  ...
  function setSponsoDataValidation(
    ...
    string memory data;
    data = _setSponsoDataValidation(tokenId, _propertyStringToBytes32(property), validated, reason);

    emit NewSponsoDataValidation(tokenId, property, validated, data, reason);
  }
  ...
  function getSponsoData(
    uint256 tokenId,
    string memory propertyString
  )
    external
    view
    returns (string memory lastDataValidated, string memory lastDataSubmitted, string memory lastRejectionReason)
  {
    SponsoData storage sponsoData = _sponsoDatas[tokenId][_propertyStringToBytes32(propertyString)];
  ...
  function isAllowedProperty(string memory propertyString) external view returns (bool) {
  ...
  function _setSponsoData(uint256 tokenId, bytes32 property, string memory data) private onlyAllowedProperty(property) {
    string memory lastDataValidated = _sponsoDatas[tokenId][property].lastDataValidated;
    _sponsoDatas[tokenId][property] = SponsoData(lastDataValidated, data, '');
  }
  ...
  function _isAllowedProperty(bytes32 propertyBytes) private view returns (bool) {
    return _allowedProperties[propertyBytes];
  }

  function _propertyStringToBytes32(string memory p) private pure returns (bytes32) {
...
interface IDSponsor {
  event NewSponsoData(uint256 indexed tokenId, string indexed property, string data);
  ...
  function setSponsoData(uint256 tokenId, string memory property, string memory data) external;
