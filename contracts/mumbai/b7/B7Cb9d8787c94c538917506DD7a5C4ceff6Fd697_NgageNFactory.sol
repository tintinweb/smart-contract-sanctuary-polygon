...
contract NgageNFactory is AccessControl, ERC2771Context {
    ...
    mapping(string => address) public factoryCreator;

    mapping(string => address) public ngageNContracts;

    event FactoryCreated(address _owner, string _factoryName, address _ngageN);
    ...
    function create(
        ...
        require(
            hasRole(CONTRACT_CREATOR, _msgSender()),
            "NF: Only the contract creator can create a factory"
        );
        require(
            factoryCreator[_factoryName] == address(0),
            "NF: Factory already created"
        );
        require(
            _creator != _msgSender(),
            "NF: Creator cannot be the proxy admin"
        );
        ...
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            ...
            abi.encodeWithSelector(
                NgageNV2(address(0)).initialize.selector,
                _uri,
                _contractUri
            )
        );

        NgageNV2(address(proxy)).grantRole(DEFAULT_ADMIN_ROLE, _creator);

        factoryCreator[_factoryName] = _creator;
