...
contract Erc1155BaseUri is ERC1155URIStorage, ERC2981, Ownable {
    ...
    function airdrop(
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            safeTransferFrom(_from, _to[i], id, 1, "0x0");
        }
    }
    ...
    function airdropMultiple(
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            safeTransferFrom(_from, _to[i], _ids[i], 1, "0x0");
        }
    }

    // Creates a new token type and assings _initialSupply to minter
    function create(uint256 _initialSupply)
        external
        ownerOrManager
        returns (uint256 _id)
    {
        ...
        _mint(msg.sender, _id, _initialSupply, "0x0");
    }
    ...
    {
        firstTokenId = _tokenIds.current();
        ...
        _safeBatchTransferFrom(address(0x0), msg.sender, _ids, _values, "0x0");
        firstTokenId = _ids[0];
    }
    ...
    function mint(
        ...
        for (uint256 i = 0; i < _to.length; ++i) {
            _mint(_to[i], _id, _quantities[i], "0x0");
        }
    }
