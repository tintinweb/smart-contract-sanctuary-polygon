    ...
    ERC1155HolderUpgradeable
{
    ...
    function createSale(
        ...
        ProjectInfo memory _project,
        SaleInput memory _saleInput
    ) external nonReentrant onlyProject returns (uint256) {
        ...
        if (!_project.isFixed) {
            ...
            require(
                _saleInput.priceDecrementAmt > 0 &&
                    _saleInput.priceDecrementAmt <= _saleInput.maxPrice - _saleInput.minPrice,
                "Invalid price"
            );
        }
        ...
        if (_project.isSingle) {
            if (_isCreateNewToken) {
                sale.tokenId = _isSetRoyalty
                    ? IOSB721(_project.token).mintWithRoyalty(
                        address(this),
                        _saleInput.tokenUri,
                        _saleInput.royaltyReceiver,
                        _saleInput.royaltyFeeNumerator
                    )
            ...
            if (_isCreateNewToken) {
                sale.tokenId = _isSetRoyalty
                    ? IOSB1155(_project.token).mintWithRoyalty(
                        address(this),
                        _saleInput.amount,
                        _saleInput.tokenUri,
                        _saleInput.royaltyReceiver,
    ...
    function close(
        ...
        ProjectInfo memory _project,
        SaleInfo memory _sale,
        uint256 _totalBuyersWaitingDistribution,
        bool _isGive
    ) external onlyProject nonReentrant returns (uint256) {
        ...
        for (uint256 i = 0; i < buyersWaiting.length; i++) {
            ...
            if (_isGive || _project.sold < _project.minSales) {
                Helper.safeTransferNative(
                    billInfo.account,
                    billInfo.royaltyFee + billInfo.superAdminFee + billInfo.sellerFee
                );
            } else {
            ...
            // Transfer tokens
            address receiver = (_project.minSales > 0 && _project.sold < _project.minSales && !_isGive)
                ? _project.manager
                : billInfo.account;
            _project.isSingle
    ...
    function buy(
        ...
        if (!projectInfo.isFlashSale) {
            require(
                MerkleProofUpgradeable.verify(_merkleProof, merkleRoots[_saleId], keccak256(abi.encodePacked(_to))),
                "Invalid winner"
            );
        }
        ...
        _sharing(
            PaymentInput(
                _to,
                projectInfo.isCreatedByAdmin,
                projectInfo.isInstantPayment,
                projectInfo.id,
                saleInfo.id,
                saleInfo.tokenId,
                projectInfo.minSales,
                projectInfo.profitShare,
                _amount,
                total
            )
        );
    ...
    function _calculateSale(uint256 _projectId, uint256 _saleId, uint256 _amount) private returns (uint256, uint256) {
        ...
        if (projectInfo.isFixed) {
            ...
            price = getCurrentDutchPrice(
                projectInfo.saleStart,
                projectInfo.saleEnd,
                saleInfo.dutchMaxPrice,
                saleInfo.dutchMinPrice,
                saleInfo.priceDecrementAmt
            );
        }
    ...
    function _sharing(PaymentInput memory _paymentInput) private {
        ...
        // Calculate royal fee
        (address royaltyReceiver, uint256 royaltyFee) = getRoyaltyInfo(
            _paymentInput.projectId,
            _paymentInput.tokenId,
            _paymentInput.payAmount
        );
        ...
        emit Buy(
            _paymentInput.buyTo,
            _paymentInput.saleId,
            _paymentInput.tokenId,
            _paymentInput.amount,
            _paymentInput.profitShare,
            supperAdminProfit,
            royaltyProfit,
            sellerProfit
        );
