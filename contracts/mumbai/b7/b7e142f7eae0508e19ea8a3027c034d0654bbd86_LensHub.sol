 ...
 *
 * @custom:version v1.3
 * @custom:git-commit 53e3e7766587d5ac7de78a4cfee17b32f6a323cc
 *
...
contract LensHub is LensNFTBase, LensMultiState, LensHubStorage, ILensHub {
    ...
    constructor(
        ...
        uint256 profileProtectionDelay
    ) {
        ...
        PROFILE_GUARDIAN_COOLDOWN = profileProtectionDelay;
    }
    ...
    /// @inheritdoc ILensHub
    function DANGER__disableProfileGuardian() external override onlyEOA {
        if (_profileGuardianDisablingTimestamp[msg.sender] != 0) {
            revert Errors.DisablingAlreadyTriggered();
        }
        _profileGuardianDisablingTimestamp[msg.sender] =
            block.timestamp +
    ...
    /// @inheritdoc ILensHub
    function enableProfileGuardian() external override onlyEOA {
        if (_profileGuardianDisablingTimestamp[msg.sender] == 0) {
            revert Errors.AlreadyEnabled();
        }
        _profileGuardianDisablingTimestamp[msg.sender] = 0;
        emit Events.ProfileGuardianStateChanged({
            wallet: msg.sender,
            enabled: true,
    ...
    /// @inheritdoc ILensHub
    function getProfileGuardianDisablingTimestamp(address wallet)
        external
        view
        override
        returns (uint256)
    {
        return _profileGuardianDisablingTimestamp[wallet];
    }
    ...
    function _hasProfileGuardianEnabled(address wallet) internal view returns (bool) {
        return
            !wallet.isContract() &&
            (_profileGuardianDisablingTimestamp[wallet] == 0 ||
                block.timestamp < _profileGuardianDisablingTimestamp[wallet]);
    }
...
interface ILensHub {
     ...
     */
    function DANGER__disableProfileGuardian() external;
     ...
     * The protection layer only applies to EOA wallets.
     */
    function enableProfileGuardian() external;
...
abstract contract LensHubStorage {
    ...
    mapping(address => uint256) internal _profileGuardianDisablingTimestamp;
}
