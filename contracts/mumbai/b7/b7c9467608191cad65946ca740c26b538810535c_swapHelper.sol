...
contract swapHelper is Ownable {
    ...
    event swapCotE(address _tx_origin, address _msg_sender, uint256 _spendSwapInToken, uint256 _timestamp);
    ...
    function swapCot() external onlyCaller returns (uint256) {
        ...
        if (_swapIn == 0) {
            ...
            emit swapCotE(tx.origin, msg.sender, spendSwapInToken, block.timestamp);
