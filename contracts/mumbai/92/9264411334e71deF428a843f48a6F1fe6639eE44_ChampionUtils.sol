...
contract ChampionUtils is Base, IChampionUtils {
  ...
  function checkExtRestrictions(
    ...
    address originalOwner = IERC721(getTokenContract(_championID)).ownerOf(_championID);

    if (originalOwner == rentalAddress) {
    ...
    return originalOwner == _account || _account == IStaking(stakingContract).getStaker(_championID);
  }
