...
contract Maticpro {
    ...
    function invest(address referrer) public payable {
		...
		if (user.referrer != address(0)) {	   
            ...
            for (uint i = 0; i < ref_bonuses.length; i++) {
                if (upline != address(0)) {
                    users[upline].refStageIncome[i] = users[upline].refStageIncome[i];
                    if(user.checkpoint == 0){
  ...
  function withdrawal(uint256 _amount) external{
    ...
    _safeTransfer(msg.sender,actualAmountToSend.mul(_amount).div(100));
    
    emit Withdrawn(msg.sender,actualAmountToSend.mul(_amount).div(100));
