...
contract MyContract is ERC1155LazyMint {
    ...
    function registerPlayer(string calldata _name) external {
        require(players[msg.sender].isRegistered == false, "Player already registered");
        require(paymentToken.balanceOf(msg.sender) >= mintRate, "Insufficient token balance to register player");
        require(paymentToken.allowance(msg.sender, address(this)) >= mintRate, "Insufficient allowance to register player");
        paymentToken.transferFrom(msg.sender, address(this), mintRate);
        players[msg.sender] = Player(true, _name);
    }
    ...
    function verifyClaim(
        ...
        require(isPlayer(_claimer), "Player is not registered!");
