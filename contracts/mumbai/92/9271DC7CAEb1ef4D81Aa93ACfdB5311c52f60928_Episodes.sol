...
/*
    ...
    the limitations contained in the certificate:
    https://explorer.solana.com/address/6mNPiDntrDtcyCiWmY3ZsG77BCcj8gisXtgTdj2BAQV.

    MagnificTower is free software and part of Mysteries Land Projects: 
    you can redistribute it and/or modify it under the terms of the 
    GNU General Public License as published by the Free Software Foundation, 
    either version 3 of the License, or (at your option) any later version, 
    respecting the above limitation.
...
contract Episodes is Multicall, Permissions, BatchMintMetadata, ERC1155Drop {
    
    ICards cards;
    uint256 public numTokenIds;
    mapping(uint256 => string) public E1Passwords;
    mapping(uint256 => uint256[]) public E1C1Requirements;
    mapping(uint256 => uint256) public E1E2Correspondence;
    mapping(address => mapping(uint256 => bool)) public burnedTokens;
    ...
    {
        cards = ICards(_cardsAddress);
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
    }

    function burnToEarn(uint256 _tokenId) external {
        require(balanceOf[msg.sender][_tokenId] > 0, "You do not own this Episode");
        string memory password = getPasswordForId(_tokenId);
        require(keccak256(abi.encodePacked(password)) == keccak256(abi.encodePacked(E1Passwords[_tokenId])), "Incorrect password");
        for (uint i=0; i<E1C1Requirements[_tokenId].length; i++) {
            require(cards.balanceOf(msg.sender, (E1C1Requirements[_tokenId][i])) >= 10, "You don't have all the Cards for this Episode");
        }
        burnedTokens[msg.sender][_tokenId] = true;
    ...
    function getPasswordForId(uint256 _tokenId) internal view returns (string memory) {
        return E1Passwords[_tokenId];
    }

    function setE1Password(uint256[] memory _e1TokenIds, string[] memory _passwords) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i=0; i<_e1TokenIds.length; i++) {
            E1Passwords[_e1TokenIds[i]] = _passwords[i];
        }
    }

    function setE1C1Requirement(uint256[] memory _e1TokenIds, uint256[][] memory _c1TokenIds) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i=0; i<_e1TokenIds.length; i++) {
            E1C1Requirements[_e1TokenIds[i]] = _c1TokenIds[i];
         }
    }

    function setE1E2Correspondence(uint256[] memory _e1TokenIds, uint[] memory _e2Values) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i=0; i<_e1TokenIds.length; i++) {
            E1E2Correspondence[_e1TokenIds[i]] = _e2Values[i];
        }
    }
     
     function mint(address to, uint id, uint amount, string memory data ) external onlyRole(DEFAULT_ADMIN_ROLE) {
         super._mint(to, id, amount, bytes(data));
