...
// Replace below address with main token token
   ...
   struct userInfo {
    ...
    uint mIndex3x;
    // uint parentIndx;
    uint64 nextMemberParentFill;
    uint64 nextMemberDownlineFill;
    ...
    function buyTopup(uint64 position) public returns(bool) {
        ...
        // Comman function.
         if (userInfos[msg.sender].joined==false){
            ...
            userInfos[ref].AR_VaildityIndex=royalty_Index+sysInfos.AR_validity;
            }
    ...
    function reTopup(uint64 position) external returns(bool) {
        ...
        else if(userInfos[msg.sender].joined==true){
		userGains[msg.sender].withdrawLimit += totalAmount*2;
        }
    ...
    // Transfer from deposit credits fund .
        ...
        function depositFund(uint _amount) external  returns(bool) {
...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
    ...
    function setPrams(uint8 withdrwalFee, uint setPoolTime, uint8 setPoolLimit,uint8 _maxCycle, uint8 _maxDepth, uint8 _AR_validity) external onlyOwner returns(bool){
    ...
    function _autoPool2xPosition(address user) internal returns (bool)
    {
        ...
        uint64 idx = nextMemberParentFill;
    ...
    function payNbirth(address user,uint recParentIndx) internal returns(uint) {
        ...
        uint64 recMindex = autoPool2xDataList[recParentIndx].mIndex;
    ...
    function reBirthPosition(address _poolUser,uint64 _mIndex,uint _from, uint _to) internal {
     ...
     for (uint i=1;i<=5;i++){
        ...
        if(ref!=address(0)){
            ...
            if (directAct>=GAP_DIRECT ){
                ...
                for (uint j=0;j<userInfos[ref].referralList.length;j++){
  ...
  function _closingRoyality() internal {
    ...
    function currentRoyaltyStatus()public view returns(uint GR_Collection, uint GR_Qualifier, uint AR_Collection, uint AR_Qualifier){
    return(GR_collection,GR_Qualifier_Count,AR_collection,AR_Qualifier_Count);

    }
    function viweRoyaltyPotential() public view returns (uint globalRoyalty,uint activeRoyalty){
       
       // Global royalty pay.
       if(royalty_Index>userInfos[msg.sender].GR_index && userInfos[msg.sender].GR_Qualify==true){
         uint gpi =userInfos[msg.sender].GR_index;
         globalRoyalty=royaltys[royalty_Index].GR_total_Share-royaltys[gpi].GR_total_Share;
         
       }
        // Active royalty pay.
       if( royalty_Index>userInfos[msg.sender].AR_index && userInfos[msg.sender].AR_VaildityIndex>=royalty_Index){
    ...
    function PayRoyalty() internal {
       (uint GR_Amount, uint AR_Amount)=viweRoyaltyPotential();
        if(GR_Amount>0){
         // Global royalty pay.
...
// global royalty
  function _royaltyQualify(address _ref) internal {
      ...
      //active Royalty
      if(directCount>=AR_DIRECTS && userInfos[_ref].AR_VaildityIndex==0){
            //Qualified
            userInfos[_ref].AR_index=royalty_Index;
            userInfos[_ref].AR_VaildityIndex=uint64(royalty_Index+sysInfos.AR_validity);
            AR_Qualifier_Count++;
      }
      ...
      if (directCount>=GR_DIRECT && userInfos[_ref].GR_Qualify==false){
        ...
        if ((totalLeg-GR_STRONG_LEG)>=GR_OTHER_LEG && strongLeg>=GR_STRONG_LEG){
            ...
            userInfos[_ref].GR_Qualify=true;
            GR_Qualifier_Count++;
        }
