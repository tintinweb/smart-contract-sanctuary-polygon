...
contract HToken is HTokenInternal, ReentrancyGuard {
  ...
  string public constant name = "HToken Test";
   ...
   * @param _protocolFeeMantissa protocol fee rate used on borrows, scaled by 1e18
...
contract HTokenInternal is ERC1155, IERC721Receiver, ErrorReporter, AccessControl {
  ...
  event CollateralDeposited(address indexed _initiator, uint256 _collateralId);
  ...
  function depositCollateralInternal(uint256 _collateralId) internal {
    ...
    _mintCoupon(msg.sender, _collateralId);
   ...
   */
  function _mintCoupon(address _to, uint256 _collateralId) internal {
  ...
  function liquidateBorrowInternal(uint256 _collateralId) internal {
    ...
    accrueInterestToCoupon(activeCoupon.id);
   ...
   * @dev Simulates accrual of interest
   * @param _couponId token ID of the coupon
  ...
  function accrueInterest() public {
    ...
    for (uint256 i = 1; i <= length; ) {
      ...
      if (coupon.active == COUPON_ACTIVE) {
        interestAccumulated += _accrueInterestToCoupon(collateralId);
      }
      unchecked {
        ++i;
      }
  ...
  function accrueInterestToCoupon(uint256 _couponId) public returns (uint256) {
    ...
    Collateral storage collateral = collateralPerBorrowCouponId[_couponId];

    uint256 earned = _accrueInterestToCoupon(collateral.collateralId);
  ...
  function accrueInterestToCollateral(uint256 _collateralId) external returns (uint256) {
    ...
    uint256 earned = _accrueInterestToCoupon(_collateralId);
  ...
  function _accrueInterestToCoupon(uint256 _collateralId) internal returns (uint256) {
  ...
  function accrueInterestToCouponsInternal(uint256[] calldata _ids) internal returns (uint256) {
    ...
    for (uint256 i = 1; i <= length; ) {
      ...
      if (coupon.active == COUPON_ACTIVE) {
        interestAccumulated += _accrueInterestToCoupon(collateral.collateralId);
      }
      unchecked {
        ++i;
      }
  ...
  function _addReserves(uint256 _addAmount) external {
    if (!hasRole(SUPPLIER, msg.sender)) revert Unauthorized();
  ...
  function _withdrawAdminFees(uint256 _amount) external {
    if (!hasRole(MARKET_ADMIN, msg.sender)) revert Unauthorized();
