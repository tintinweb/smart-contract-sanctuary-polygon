...
/**
 * @dev Use to made payment for ico service.
 * Has a function to approve user's ICO request.
 * @notice Utilize balance vault to minimize gas cost in token transfer.
 * @notice Is pausable to prevent malicious behavior.
 */
contract ICOPayment is Ownable, AccessControl, Pausable {
    ...
    receive() external payable {
        revert("[ICOPayment] Revert receive function.");
    }

    fallback() external payable {
        revert("[ICOPayment] Revert fallback function.");
    }

    /**
     * @dev Approve ico service purchasing made by user.
     * Transfer fee using balance vault.
     * @param _chainId - Chain id of ico service.
    ...
    */
    function approveICO(
        ...
        if (_fees != 0) {
            require(
                balanceVault.getBalance(_projectOwner) >= _fees,
                "[ICOPayment.approveICO] Insufficient user balance"
            );
            balanceVault.decreaseBalance(_projectOwner, _fees);
            balanceVault.increaseBalance(adminAddress, _fees);
        }
        require(
            bytes(_chainId).length > 0,
            "[ICOPayment.approveICO] Invalid chain id"
        );
        require(
            _initialTokenAddress != address(0) &&
            _totalBuyingLimit > 0 &&
            _rate > 0,
            "[ICOPayment.approveICO] Invalid initial token arguments"
        );
        require(
            _startDate < _endDate &&
            (_startDate > block.timestamp && _endDate > block.timestamp),
            "[ICOPayment.approveICO] Invalid date range"
        );
        require(
            _paidIntervals.length == _paidPortions.length &&
            _paidIntervals.length > 0,
            "[ICOPayment.approveICO] Invalid paid interval length"
        );
        uint256 totalPaidPortion;
        uint256 prevPaidInterval;
        for (uint256 i = 0; i < _paidIntervals.length; i++) {
            require(
                _paidIntervals[i] > _endDate &&
                _paidIntervals[i] > prevPaidInterval &&
                _paidPortions[i] > 0,
                "[ICOPayment.approveICO] Invalid paid interval and paid portion members"
            );
            prevPaidInterval = _paidIntervals[i];
            totalPaidPortion += _paidPortions[i];
        }
        require(
            totalPaidPortion == 100,
            "[ICOPayment.approveICO] Invalid total paid portions"
        );
    ...
    /**
     * @dev Set ico payment in to pause state (only setter allowed).
     */
    function pauseICOPayment() external onlyOwner {
        _pause();
    }

    /**
     * @dev Set ico payment in to normal state.
     */
    function unpauseICOPayment() external onlyOwner {
        _unpause();
    }
}
