...
contract billionMoney is owned {
    ...
    constructor(address payable ownerAddress, address payable ID1address) public {
        ...
        priceOfLevel[1] = 1000000;
        priceOfLevel[2] = 2000000;
        priceOfLevel[3] = 3000000;
...
*/
    ...
    function updateROI(address userAddress, uint _amount) internal {
        ...
        }else if (userGains[userAddress].expReturn==0 && userGains[userAddress].returnShare!=0 && userInfos[userAddress].isDividendDeactive==false ){
        ...
        }else if (userInfos[userAddress].isDividendDeactive==true && userGains[userAddress].expReturn==0 ){
        ...
        }else if (userGains[userAddress].expReturn!=0 && userInfos[userAddress].isDividendDeactive==false){

            userGains[userAddress].expReturn+=_amount; // update exp return on new level buy
        }
    ...
    function claimPassiveIncome() public returns(bool) {
        ...
        require( userGains[msg.sender].expReturn>0,"invalid");
