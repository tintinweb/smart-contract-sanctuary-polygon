...
abstract contract App is UpgradeableApp, Initializable {
    ...
    uint8 public immutable APP_UPDATER = _initNextRole();
    ...
    function upgrade(address appCode) external requirePermission(APP_UPDATER) {
...
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        bytes32 tier = _getPropertyValue(uint32(tokenId), PROPERTY_TIER);
...
interface IAppManager {
    ...
    function connectApp(bytes32 id, address appAddress) external;
