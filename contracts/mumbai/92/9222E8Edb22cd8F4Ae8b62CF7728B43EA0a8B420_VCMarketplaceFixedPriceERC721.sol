...
contract VCMarketplaceFixedPriceERC721 is VCMarketplaceBase {
    ...
    function listFixedPrice(
        ...
        if (!listed(_tokenId)) {
            _newList(_tokenId, _listPrice, marketFee);
            emit ListedFixedPrice(address(artNft), _tokenId, msg.sender, _listPrice, marketFee);
        } else {
            _updateList(_tokenId, _listPrice, marketFee);
            emit UpdatedFixedPrice(address(artNft), _tokenId, msg.sender, _listPrice, marketFee);
        }
    }
    ...
    function _newList(
        ...
        bool minted = IArtNftERC721(address(artNft)).exists(_tokenId);
        if (!minted) {
        ...
        fixedPriceListings[_tokenId] = FixedPriceListing(minted, msg.sender, _listPrice, marketFee);
    }
