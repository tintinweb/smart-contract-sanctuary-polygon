...
contract Test{    
    ...
    function testCalc(uint256 _amt, uint8 level,uint8 _type) public view returns(uint256){
        if(_type==1)
        {
          uint256 totalMatrixValue=(mlevelPrice[level-1]*(level+3));
          uint256 maticPerDollor=(_amt/totalMatrixValue)*1e18;   
          uint256 levelUSD=levelPrice[level];  
          return ((levelUSD*REFERRAL_PERCENTS[0])*maticPerDollor)/1e21;
        }
    }
