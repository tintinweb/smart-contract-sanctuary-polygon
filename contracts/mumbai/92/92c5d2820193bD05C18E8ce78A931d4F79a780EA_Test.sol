...
contract Test{    
    ...
    event CommunityBonus(address indexed receiver, address indexed _from, uint _amount, uint usdamount, uint8 level, uint8 _type);
    event MatrixBonus(address indexed receiver,  uint256 _amount, uint8 level,  uint256 usdamount);
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) 
        {
            require(msg.value>=((levelPrice[level]*1e18)/maticRate), "invalid price!");         
            ...
            uint256 levelUSD=levelPrice[level];
        ...
        else 
        {
            ...
            require(msg.value>=((mlevelPrice[level]*1e18)/maticRate), "invalid price!");            
    ...
    function registration(address userAddress, address referrerAddress, uint256 id) private{
        ...
        //=========== start mt3 matrix ==========
            address freeX3Referrer = findFreeX3Referrer();
            users[userAddress].x3Matrix[1].currentReferrer = freeX3Referrer;
            if (users[freeX3Referrer].x3Matrix[1].referrals.length == 2) {
                currentMT3Id++;
            }
    ...
    function updateMT3(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x3Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress, referrerAddress, 0, level, uint8(users[referrerAddress].x3Matrix[level].referrals.length));
            return address(uint160(referrerAddress)).transfer((20e18*1e18)/maticRate);
        }          

        emit NewUserPlace(userAddress, referrerAddress, 0, level, 3);
        //close matrix
        users[referrerAddress].x3Matrix[level].referrals = new address[](0);
       
               //create new one by recursion
        if (referrerAddress != owner) {
            address freeReferrerAddress = users[referrerAddress].x3Matrix[level].currentReferrer;            
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level, uint256 amount) private{
        ...
        {                
          ...
          {
             if(!users[referrerAddress].activeX3Levels[level+1])
        ...
        else
        {
            ...
            if(users[referrerAddress].x6Matrix[level].referrals.length < level+3) 
            {
                users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
                emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
                emit MatrixBonus(referrerAddress, amount, (level),(amount*1e18)/maticRate);
                return address(uint160(referrerAddress)).transfer(amount);   
            }
            users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
            emit MatrixBonus(referrerAddress, amount, (level),(amount*1e18)/maticRate);           
            address(uint160(referrerAddress)).transfer(amount);           
    ...
    function autoUpgradeLevel(address _user, uint8 level) private{
        if(!users[_user].activeX3Levels[level])
        {
            ...
            uint256 totalMatrixValue=(mlevelPrice[level-1]*(level+3));         
            uint256 maticPerDollor=(users[_user].holdAmount[level-1]*1e18)/totalMatrixValue;         
    ...
    function autoUpgrade(address _user, uint8 level) private{
             uint256 totalMatrixValue=(mlevelPrice[level-1]*(level+3));         
             uint256 maticPerDollor=(users[_user].holdAmount[level-1]*1e18)/totalMatrixValue;      
             uint256 UpgradeAmount=(mlevelPrice[level]/1e18)*maticPerDollor;
             uint256 userAmount=(ilevelPrice[level-1]/1e18)*maticPerDollor;
             address(uint160(_user)).transfer(userAmount);
             emit MatrixBonus(_user, userAmount, (level-1),ilevelPrice[level-1]);
             users[_user].holdAmount[level-1]=0;
             address freeX6Referrer = findFreeX6Referrer(level);

             users[_user].activeX6Levels[level] = true;
             users[_user].x6Matrix[level-1].referrals =new address[](0);
             users[_user].activeX6Levels[level-1]=false;

             updateX6Referrer(_user, freeX6Referrer, level, UpgradeAmount);
             x3CurrentvId[level-1]=x3CurrentvId[level-1]+1;
    ...
    function withdrawLevelIncome() public{
        ...
        for(i=0;i<10;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payUpId], msg.sender, rest/100, ((rest/100)*1e18)/maticRate, i+1,1);
        }
        ...
        for(i=0;i<10;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payDownId], msg.sender, rest/100, ((rest/100)*1e18)/maticRate, i+1,2);
        }
    ...
    function findFreeX3Referrer() public view returns(address) {
        return idToAddress2[currentMT3Id];
    }
