...
contract Swap is ISwap, OwnableUpgradeable, Proxiable {
    /// @dev An array of all the Swap structs ever created by the ParadigmSwap
    ISwap.Swap[] internal allSwaps;

    /// @notice Monotonically incrementing index, used when creating Swaps.
    ...
    // This should be immutable and constants
    bytes32 public  DOMAIN_NAME = keccak256("SIREN SWAP");
    bytes32 public  DOMAIN_VERSION = keccak256("1");
    uint256 public  DOMAIN_CHAIN_ID;
    bytes32 public  DOMAIN_SEPARATOR;


    function initialize(IAddressesProvider _addressesProvider) public override {
        ...
        // emit SwapInitialized(lpToken, address(0x0), address(_seriesController));
    }
    ...
    function createSwap(
        ...
        uint256 bidSize,
        address poolAddress,
        ISwap.ProductType productType
    ) external override {
        //TODO add requires that we want to have here

        // store variable in memory for reduced gas costs when reading
        uint64 _latestIndex = latestIndex;
        SwapStatus _status = ISwap.SwapStatus.OPEN;
        ISwap.Swap memory _swap = ISwap.Swap(
            productId,
            poolAddress,
            _status,
            expirationDate,
        ...
        // Emit the event
        emit SwapCreated(_latestIndex, _swap);
    ...
    function settleSwap(
        ...
        // We can call checkBid, because it has only require statements
        // I think we should make a generic settlement piece here instead of calling to the amm
        (
            uint64 _productId,
            address poolAddress,
            ,
        ...
        IMinterAmm(poolAddress).bTokenDirectAuctionBuy(
            _productId,
            expirationDate,
            buyAmount,
            nonce,
    ...
    function checkBid(
        ...
        //   'SIGNER_BALANCE_LOW']
        Swap memory swap = allSwaps[swapId];
        require(swap.poolAddress != address(0), "Swap does not exist");

        ISeriesController.Series memory series = getProduct(swap.productId);
        require(series.expirationDate != 0, "Product does not exist");
        ...
        require(signer == signatory, "UNAUTHORIZED");
        // Check if authorized?
        // require(authorized[signer] ==  signatory, "...")


        // Check nonce?


        require(buyAmount >= swap.bidSize, "BID_TOO_SMALL");
        // minBidSize rename?

        // Do we want to check for available size ? availableSize
         uint256 pricePerOption = (sellAmount * 1e18) / buyAmount;
        require(swap.minBidPrice <= pricePerOption, "PRICE_TOO_LOW");

        // Check Singer allowance

        // Check Singer balance    

        // Check Seller/AMM allowance  

        // Check seller/AMM balance   
    ...
    {
        //Return swap + series
        ISwap.Swap memory _swap = allSwaps[swapId];
        return (
            _swap.productId,
            _swap.poolAddress,
            _swap.status,
            _swap.expirationDate,
            _swap.bidSize,
            _swap.minBidPrice,
            _swap.productType
        );
    ...
    {
        return (
            ISeriesController(addressesProvider.getSeriesController()).series(
                productId
            )
        );
    }
...
interface ISwap {
    ...
    /// @notice All possible states a Swap can be in with regard to its expiration date
    enum SwapStatus {
        OPEN,
        CLOSED
    }

    /// @notice All possible ProductTypes a Swap can have
    enum ProductType {
        /**
    ...
    /// @notice Emitted when the owner creates a new Swap
    event SwapCreated(uint64 swapId, ISwap.Swap newSwap);

    /// @notice Emitted when the SeriesController gets initialized
    event SwapUpdated(address swapId, ISwap.SwapStatus);

    ///////////////////// VIEW/PURE FUNCTIONS /////////////////////

    function initialize(IAddressesProvider _addressesProvider) external;
...
interface IMinterAmm {
    ...
    function bTokenDirectAuctionBuy(
        uint64 seriesId,
        uint256 expiry,
        uint256 buyAmount,
        uint256 nonce,
