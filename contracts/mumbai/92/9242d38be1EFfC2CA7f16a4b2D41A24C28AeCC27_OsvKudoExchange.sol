...
contract OsvKudoExchange is Initializable {
    ...
    function setOSVContracts(address osvKudoTokenAddress) public {
    ...
    function approveExchangeForCSA(address userToBeApproved, uint256 amount) public {
        ...
        totalUSDExchanged[userToBeApproved] += getUSDValue(amount);
    }
}
...
contract OsvKudo is Initializable, ERC20Upgradeable, DateTime {
    ...
    function _beforeTokenTransfer(
        ...
        // osvNFT.checkERC1155Holder(to, osvNFT.OSV_EMPLOYEE_TOKEN_ID()); // TODO: maybe only can send to other employees
        super._beforeTokenTransfer(from, to, amount);

        // make sure users can claim every month a bonus up to a specific amount
        // make sure we don't add on mint or burn
        if (from != address(0) && to != address(0) && to != address(osvKudoExchange) && amount != 0) {
...
contract OsvNFT is ERC1155Upgradeable {
    ...
    function initialize(string memory uri) public initializer {
        __ERC1155_init(uri);
        _mint(msg.sender, _OSV_ADMIN_TOKEN_ID, 1, "");
    }
