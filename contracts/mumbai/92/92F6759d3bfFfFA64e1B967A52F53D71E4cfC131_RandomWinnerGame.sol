...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    // Amount of the links needed to be sent with request
    uint256 public fee;
    ...
    // Emitted when player joined
    event PalyerJoined(uint256 gameId, address player);
    ...
    function joinGame() public payable {
        ...
        require(msg.value == entryFee, "Value sent is not smae as entry fee!");
        require(players.length < maxPlayers, "Game is full");
        players.push(msg.sender);
        emit PalyerJoined(gameId, msg.sender);
        if (players.length == maxPlayers) {
            getRandomWinner();
        }
    }

    // Get the randome winner out of the players
