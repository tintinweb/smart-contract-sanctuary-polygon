...
contract StabilityPool is IStabilityPool, Ownable {
    ...
    uint256 public lastOMATICError_Offset;
    uint256 public lastRewardsError_Offset;
    ...
    mapping(uint128 => mapping(uint128 => uint256)) public epochToScaleToRewards;
    ...
    function withdrawFromStabilityPool(uint _amount) external override {
        ...
        uint depositorOMATICGain = getDepositorOMATICGain(msg.sender);
        uint depositorOMATICRewards = getDepositorOMATICRewards(msg.sender);
        ...
        // Transfer of OMATIC and MATIC rewards
        uint totalOMAATICRewards = depositorOMATICGain + depositorOMATICRewards;
        _sendOMATICGainToDepositor(totalOMAATICRewards);
    }
    ...
    */
    ...
    // --- Staking rewards calculator functions for depositor and front end ---
    function getDepositorOMATICRewards(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor];
        ...
        uint oMATICGain = _getOMATICRewardsFromSnapshots(initialDeposit, snapshots);
        return oMATICGain;
    }

    function _getOMATICRewardsFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        /*
        ...
        */
        ...
        uint oMATICGain = ((initialDeposit* (firstPortion + secondPortion)) / P_Snapshot) / DECIMAL_PRECISION;

        return oMATICGain;
    }

    // --- Liquidation rewards calculator functions for depositor and front end ---
    ...
    */
    function getDepositorOMATICGain(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor];
        ...
        uint oMATICGain = _getOMATICGainFromSnapshots(initialDeposit, snapshots);
        return oMATICGain;
    }

    function _getOMATICGainFromSnapshots(uint initialDeposit, Snapshots memory snapshots) internal view returns (uint) {
        /*
        * Grab the sum 'S' from the epoch at which the stake was made. The MATIC gain may span up to one scale change.
        * If it does, the second portion of the MATIC gain is scaled by 1e9.
        * If the gain spans no scale change, the second portion will be 0.
    ...
    function _allowDeposit() internal view returns (bool) {
        ...
        allow = utilisationRatio >= (depositorUTR * DECIMAL_PRECISION) || _getUSDCinSP() < MIN_USDC_REQUIRED ? true : false;
        return allow;
    }

    // USDC getter functions
    function getUSDCinSP() external override view returns (uint) {
        return _getUSDCinSP();
    }

    function _getUSDCinSP() internal view returns (uint) {
        return usdc_token.balanceOf(address(this));
    }

    // --- Block reward functions ---
    function rewardsOffset(uint _rewardsToAdd) external override {
        uint totalUSDC = totalUSDCDeposits;
        if(_rewardsToAdd == 0 || totalUSDC == 0) {
            return;
        }

        _triggerOrumIssuance(communityIssuance);

        uint oMATICRewardsPerUnitStaked = _computeOMATICRewardsPerUnitStaked(_rewardsToAdd, totalUSDC);

        _updateStakingRewardSumAndProduct(oMATICRewardsPerUnitStaked);

        oMATIC += _rewardsToAdd;
    }

    function _computeOMATICRewardsPerUnitStaked(uint _rewardsToAdd, uint _totalUSDCDeposits) internal returns (uint){
        uint RewardsNumerator = (_rewardsToAdd * DECIMAL_PRECISION) + lastRewardsError_Offset;

        uint oMATICRewardGainPerUnitStaked = RewardsNumerator / _totalUSDCDeposits;
        lastRewardsError_Offset = RewardsNumerator - (oMATICRewardGainPerUnitStaked * _totalUSDCDeposits);

        return oMATICRewardGainPerUnitStaked;    
    }

    function _updateStakingRewardSumAndProduct(uint _OMATICRewardsPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
        ...
        uint marginalOMATICRewards = _OMATICRewardsPerUnitStaked * currentP;
        uint newR = currentR + marginalOMATICRewards;
        ...
        override
    {
        ...
        (uint oMATICGainPerUnitStaked,
            uint USDCLossPerUnitStaked) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalUSDC);

        _updateRewardSumAndProduct(oMATICGainPerUnitStaked, USDCLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal 
        returns (uint oMATICGainPerUnitStaked, uint USDCLossPerUnitStaked)
    {
        /*
        ...
        */
        uint oMATICNumerator = (_collToAdd * DECIMAL_PRECISION) + lastOMATICError_Offset;
        ...
        oMATICGainPerUnitStaked = oMATICNumerator / _totalUSDCDeposits;

        lastOMATICError_Offset = oMATICNumerator - (oMATICGainPerUnitStaked* _totalUSDCDeposits);

        return (oMATICGainPerUnitStaked, USDCLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _oMATICGainPerUnitStaked, uint _USDCLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
        ...
        * The oMATIC gain for any given depositor from a liquidation depends on the value of their deposit
        * (and the value of totalDeposits) prior to the Stability being depleted by the debt in the liquidation.
        *
        * Since S corresponds to oMATIC gain, and P to deposit loss, we update S first.
        */
        uint marginalOMATICGain = _oMATICGainPerUnitStaked * currentP;
        uint newS = currentS + marginalOMATICGain;
    ...
    function _moveOffsetCollAndDebt(
        ...
        activePool.decreaseUSDCDebt(_debtToOffset);

        activePool.sendoMATIC(address(this), _collToAdd);
        oMATIC += _collToAdd;
    }
    ...
    */
    ...
    function _sendOMATICGainToDepositor(uint _amount) internal {
        if (_amount == 0) {return;}
        oMATIC -= _amount;

        bool success = oMATICToken.transfer(msg.sender, _amount);

        require(success, "StabilityPool: sending oMATIC failed");
    }

    /* withdrawoMATICGainToVault:
    * - Transfers the depositor's entire oMATIC gain from the Stability Pool to the caller's vault
    * - Leaves their compounded deposit in the Stability Pool
    * - Updates snapshots for deposit */
    function withdrawoMATICGainToVault(address _upperHint, address _lowerHint) external override {
        uint initialDeposit = deposits[msg.sender];
        require(initialDeposit > 0, "StabilityPool: Initial deposit is 0");
        require(vaultManager.getVaultStatus(msg.sender) == 1, "StabilityPool: caller must have an active vault to withdraw oMATICGain to");

        uint totalOMATICRewards = getDepositorOMATICGain(msg.sender) + getDepositorOMATICRewards(msg.sender);
        require(totalOMATICRewards > 0, "StabilityPool: caller must have non-zero oMATIC Gain");
        ...
        _updateDepositAndSnapshots(msg.sender, compoundedUSDCDeposit);

        oMATIC -= totalOMATICRewards;
        oMATICToken.approve(address(borrowerOps), totalOMATICRewards);
        borrowerOps.moveoMATICGainToVaultFromStabilityPool(msg.sender, totalOMATICRewards, _upperHint, _lowerHint);
    }
