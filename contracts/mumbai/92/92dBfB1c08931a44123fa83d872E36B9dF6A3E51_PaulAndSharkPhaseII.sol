...
contract PaulAndSharkPhaseII is ERC1155, Ownable {
    IPaperKeyManager paperKeyManager;

    uint8 public constant PLATINUM = 10;
    uint8 public constant GOLD = 115;
    uint16 public constant SILVER = 375;
    uint256 public constant MINT_FEE = 0.001 ether;

    string public contractURI;

    mapping (uint8 => uint16) public mintLimit;
    ...
    modifier checkLimit(uint8 _id, uint16 _qty) {
        require(mintLimit[_id] >= _qty, "cannot mint excess of limit");
        _;
    }


    modifier checkTokenId(uint8 _id) {
        require(_id == 0 || _id == 1 || _id == 2, "token does not exists");
        _;
    }
    ...
    constructor(string memory _contractURI, address _paperKeyManagerAddress) ERC1155("https://ipfs.io/xyz/{id}.json") Ownable() {
        mintLimit[0] = SILVER;
        mintLimit[1] = GOLD;
        mintLimit[2] = PLATINUM;
    ...
    function mint( uint8 _id, uint16 _qty, bytes32 _nonce, bytes calldata _signature, address _walletAddress) public payable checkFee
        checkTokenId(_id) checkLimit(_id, _qty)
        onlyPaper(keccak256(abi.encode(_walletAddress, _id, _qty)), _nonce, _signature)
    {
        mintLimit[_id] = mintLimit[_id] - _qty;
        _mint(_walletAddress, _id, _qty, "");
    }
...
// Sample json at contract - uri 
// {
//   "name": "Paul and Shark",
//   "description": "{description}",
//   "image": "{ipfs link}",
//   "external_link": "{ipfs url}",
//   "seller_fee_basis_points": 1000, # Indicates a 1% seller fee # 10 percent to both Wonderbox and P&S.
//   "fee_recipient": "{recipient address}" # Where seller fees will be paid to.
// }
