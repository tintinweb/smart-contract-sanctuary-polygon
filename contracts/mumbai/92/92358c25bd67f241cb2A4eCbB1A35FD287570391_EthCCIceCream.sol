...
import {IEthCCGelatoRaffle} from "./interfaces/IEthCCGelatoRaffle.sol";

contract EthCCIceCream is ERC721MetaTxEnumerable, Proxied {
    using Strings for uint256;

    uint256 public immutable maxSupply;
    IEthCCGelatoRaffle public immutable raffle;

    uint256 public constant GOLD_INDEX = 7;
    uint256 public constant NOT_GOLD_VARIANT = 6;
    string public baseUri;

    mapping(address => bool) public goldenIceCream;

    modifier onlyMetaBox() {
        require(isTrustedForwarder(msg.sender), "EthCCIceCream: Only metabox");
        _;
    }
    ...
    constructor(
        ...
        IEthCCGelatoRaffle _raffle
    ) ERC721MetaTx("EthCC Ice Cream", "ETHCCREAM", _gelatoMetaBox) {
        maxSupply = _maxSupply;
        raffle = _raffle;
    }

    // Todo: function mint() external onlyMetaBox {
    function mint() external {
        uint256 supplyTotal = totalSupply();

        if (maxSupply > 0)
            require(supplyTotal < maxSupply, "EthCCIceCream: Max Supply");
        ...
        returns (string memory)
    {
        ...
        uint256 round = raffle.roundNow();

        string memory baseURI = _baseURI();

        address owner = ownerOf(tokenId);
        uint256 uriId = goldenIceCream[owner]
            ? GOLD_INDEX
            : (tokenId + round) % NOT_GOLD_VARIANT;

        uint256 iceCreamState = raffle.getIceCreamState();
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0)) _onePerAccount(to);
    ...
    function _onePerAccount(address _account) private view {
        // Todo: require(balanceOf(_account) == 0, "EthCCIceCream: One per account");
    }
}
...
//solhint-disable no-empty-blocks
interface IEthCCGelatoRaffle {
    function joinRaffle(address _player) external;
    ...
    function setAdmins(address[] memory _user, bool _admin) external;
    ...
    function isRaffleLive() external view returns (bool);

    function roundNow() external view returns (uint256);

    function roundInterval() external view returns (uint256);

    function getIceCreamState() external view returns (uint256);
