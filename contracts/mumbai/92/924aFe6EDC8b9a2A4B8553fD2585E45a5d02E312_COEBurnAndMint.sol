...
contract COEBurnAndMint is
  ...
  ReentrancyGuard
{
  // COERoles public rolesContract;
  event Mint(uint256 _tokenId, address sender);
  event RequestFulfilled(address indexed opener, AssetType assetType);
  ...
  mapping(uint256 => RequestStatus) public s_requests; //requestId to request status struct
  mapping(address => uint256[]) public s_userRequests; //user to request ids
  ...
  uint32 private callbackGasLimit = 2000000; //TODO: MAKE HIGHER?
  uint16 private requestConfirmations = 3;

  //LOCAL VRF CONSUMER BASE 0X = 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266
  //MUMBAI VRF CONSUMER BASE 0X = 0x7a1BaC17Ccc5b313516C5E16fb24f7659aA5ebed
  constructor(
    address _aegAddress,
  ...
  {
    COORDINATOR = VRFCoordinatorV2Interface( // <------------------NEED TO SWITCH FOR VRF
      0x7a1BaC17Ccc5b313516C5E16fb24f7659aA5ebed //mumbai
      // 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266 //local test
    );
    s_subscriptionId = subscriptionId;
    // rolesContract.grantRole(rolesContract.ADMIN(), msg.sender);
    // rolesContract.grantRole(rolesContract.OWNER(), msg.sender);
  ...
  // uint256[] public basicCardTypes;
  // uint256[] public commonCardTypes;
  // uint256[] public uncommonCardTypes;
  // uint256[] public rareCardTypes;
  // uint256[] public epicCardTypes;
  // uint256[] public legendaryCardTypes;

  mapping(Library.Rarity => uint256[]) public rarityToCardTypes;
  ...
  function addCardType(uint256 _cardType, Library.Rarity _rarity) public {
    //only cards contract can call this
    require(
      msg.sender == cardsAddress,
      "Only the cards contract can call this function"
    );

    rarityToCardTypes[_rarity].push(_cardType);
  }

  /** MINTING **/
  //mapping for address to minted token count
  ...
  function startOpenFP() public nonReentrant whenNotPaused {
    ...
    //check if user already has a request for this asset type
    // uint256[] memory fulfilledNotUsed = getFulfilledByAddressAndType(
    //   msg.sender,
    //   AssetType.FounderPack
    // );

    requestRandomWords(AssetType.FounderPack);
    console.log("STARTING OPENING FP");
  }

  function finishOpenFP() public nonReentrant {
    console.log("FINISHING OPENING FP");
    require(
      FunctionInterface(burnTokenAddress).balanceOf(msg.sender, 1) > 0,
      "No pack."
    );
    ...
    console.log(
      "minting ethernal...",
      NftInterface(ethernalsAddress).totalTypes()
    );
    NftInterface(ethernalsAddress).adminMint(
      msg.sender,
      randomWords[1] % NftInterface(ethernalsAddress).totalTypes(),
      1,
      1
    );
    console.log("minting adventurer...");
    NftInterface(adventurersAddress).adminMint(
      msg.sender,
      randomWords[2] % NftInterface(adventurersAddress).totalTypes(),
      1,
      1
    );
    console.log("minting card backs...");
    NftInterface(cardBacksAddress).adminMint(msg.sender, 1, 1, 1); //silver
    NftInterface(cardBacksAddress).adminMint(msg.sender, 2, 1, 1); //gold

    console.log("getting cards...");

    uint256[] memory cardsToMint = new uint256[](37);

    //MINT 5 common rarity cards
    for (uint256 i = 0; i < 5; i++) {
      cardsToMint[i] = rarityToCardTypes[Library.Rarity.Common][
        randomWords[i + 3] % rarityToCardTypes[Library.Rarity.Common].length
      ];
    }
    // for (uint256 i = 0; i < 5; i++) {
    //   cardsToMint[i] = commonCardTypes[
    //     randomWords[2 + i] % commonCardTypes.length
    //   ];
    // }

    //MINT 1 uncommon rarity card
    cardsToMint[5] = rarityToCardTypes[Library.Rarity.Uncommon][
      randomWords[8] % rarityToCardTypes[Library.Rarity.Uncommon].length
    ];
    // cardsToMint[5] = uncommonCardTypes[
    //   randomWords[7] % uncommonCardTypes.length
    // ];

    //MINT 1 rare/epic/leggo PROMO card
    uint256[] memory promoCardIds = CardInterface(cardsAddress).getPromoTypes();
    uint256 randomPromoCardId = promoCardIds[
      randomWords[8] % promoCardIds.length
    ];
    cardsToMint[6] = randomPromoCardId;

    //add basic cards to mint
    for (
      uint256 i = 0;
      i < rarityToCardTypes[Library.Rarity.Basic].length;
      i++
    ) {
      cardsToMint[7 + i] = rarityToCardTypes[Library.Rarity.Basic][i];
    }
    // for (uint256 i = 0; i < basicCardTypes.length; i++) {
    //   cardsToMint[i + 7] = basicCardTypes[i];
    // }

    console.log("minting cards...", cardsToMint.length);
    CardInterface(cardsAddress).packMint(msg.sender, cardsToMint);

    console.log("minted cards...", cardsToMint.length);

    addressOpenedFP[msg.sender]++;
  }

  /** OTHER ASSETS MINTING **/

  function startMintRandom(
    AssetType _assetType
  ) public nonReentrant whenNotPaused {
    //only allow adventurer and ethernal for now
    require(
      _assetType == AssetType.Adventurer || _assetType == AssetType.Ethernal,
      "Invalid asset type."
    );
    requestRandomWords(_assetType);
  }

  function finishMintRandom(
    AssetType _assetType,
    address _paymentTokenAddress
  ) public nonReentrant {
    //need to check asset type they wish to finish opening
    uint256[] memory requests = getFulfilledByAddressAndType(
      msg.sender,
      _assetType
    );
    require(requests.length > 0, "No fulfilled requests.");
    //require they have enough aeg tokens to open
    address _address;
    if (_assetType == AssetType.Adventurer) {
      _address = adventurersAddress;
    } else if (_assetType == AssetType.Ethernal) {
      _address = ethernalsAddress;
    } else if (_assetType == AssetType.CardBack) {
      _address = cardBacksAddress;
    } else {
      revert("Invalid asset type.");
    }
    uint256 assetPrice = FunctionInterface(_address).randomPrice();
    ...
    NftInterface(_address).adminMint(
      msg.sender,
      s_requests[requests[0]].randomWords[0] %
        NftInterface(_address).totalTypes(),
      1,
      1
    );
  }
  ...
  /** GETTERS **/
  // frontend can use this to display how many packs are available to open
  function getFulfilledByAddressAndType(
    address _address,
    AssetType _assetType
  ) public view returns (uint256[] memory) {
    //get all s_userRequests for this address
    ...
    for (uint256 i = 0; i < requestIds.length; i++) {
      if (
        s_requests[requestIds[i]].fulfilled &&
        !s_requests[requestIds[i]].used &&
        s_requests[requestIds[i]].assetType == _assetType
      ) {
    ...
    return
      FunctionInterface(helperAddress).trim(
        fulfilledRequestIdsCount,
        fulfilledRequestIds
      );
  }
  ...
  function requestRandomWords(
    ...
    //get number of words to return from asset type
    uint32 numWords = 10; // FP requires 9
    if (
      _assetType == AssetType.Ethernal ||
      _assetType == AssetType.Adventurer ||
      _assetType == AssetType.CardBack
    ) {
      numWords = 1;
    }
    ...
    // Will revert if subscription is not set and funded. <------------------PROD: NEED TO UNCOMMENT FOR VRF
    ...
    // console.log("msg.sender in requestRandomWords", msg.sender);
    ...
    // console.log(s_userRequests[msg.sender][0]);
  ...
  function fulfillRandomWords(
    ...
    // console.log("IN FULFILLRANDOMWORDS");
    require(s_requests[_requestId].exists, "request not found");
    s_requests[_requestId].fulfilled = true;
    s_requests[_requestId].randomWords = _randomWords;

    // console.log("fulfillRandomWords DONE", _requestId, _randomWords.length);
    emit RequestFulfilled(
      s_requests[_requestId].opener,
      s_requests[_requestId].assetType
    );
  }
...
interface CardInterface {
  function getbasicCardIds() external view returns (uint256[] memory);

  function packMint(address, uint256[] memory) external;

  function getPromoTypes() external view returns (uint256[] memory);
}

interface BnmInterface {
  function addCardType(uint256 _cardType, Library.Rarity _rarity) external;
}
