...
contract Subscription is ReentrancyGuard {
    ...
    function setPlan(uint256 _planId, uint256 _price) public onlyOwner {
        if (plans[_planId].id == 0) {
            planIds.push(_planId);
        }
        plans[_planId] = Plan(_planId, _price);
    }

    function getPlans() public view returns (uint256[] memory, uint256[] memory) {
    ...
    function pay(uint256 _planId) public nonReentrant {
        ...
        token.transferFrom(msg.sender, owner, plan.price);
