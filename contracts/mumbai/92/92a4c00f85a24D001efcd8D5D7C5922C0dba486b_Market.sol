...
contract Market is IMarket, Context {
  ...
  */
  function registerPaperKey(address _paperKey) external onlyVotationModule() { //! Porque este only owner
  ...
  */
  function _paperValidateBuyParams(uint256 offerId_, address buyer_) internal view {
  ...
  function _paperTransactAllTokens(uint256 offerId_, address buyer_) internal  {
    ...
    for (uint256 i = 0; i < offer.collectionAddresses.length; i++) {
      ...
      IERC721(offer.collectionAddresses[i]).safeTransferFrom(offer.seller, buyer_, auxTokenIds[i], '');
    }
  }
  ...
  */
  ...
  {
    if (isAuction(offerId_) && !directBuy_) _paperValidateBuyParams(offerId_,buyer_);
