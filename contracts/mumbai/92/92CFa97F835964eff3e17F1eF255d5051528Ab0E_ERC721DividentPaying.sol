...
contract ERC721DividentPaying is ERC721, IERC721DividendPaying {
	...
	function withdrawDividend() external override {
        require(!_msgSender().isContract(), "ERC721DividentPaying: withdrawal from contract is not allowed");
	...
	function dividendOf(address owner) public override view returns(uint256) {
		return withdrawableDividendOf(owner);
	}

	function withdrawableDividendOf(address owner) public override view returns(uint256) {
		return accumulativeDividendOf(owner) - _withdrawnDividends[owner];
	}

	function accumulativeDividendOf(address owner) public override view returns(uint256) {
	...
	function withdrawnDividendOf(address owner) public override view returns(uint256) {
		return _withdrawnDividends[owner];
	}

	function vault() public override view returns(uint256) {
		return _vault;
	}
