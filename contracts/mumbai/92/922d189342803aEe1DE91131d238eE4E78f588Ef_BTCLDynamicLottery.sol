...
function checkUpkeep(bytes calldata) external view override returns (bool upkeepNeeded, bytes memory performData) {
    ...
    for (uint256 i = round; i < round + maxRounds; i++) {
...
function performUpkeep(bytes calldata performData) external nonReentrant override {
    ...
    // Request randomness for all rounds that need to be drawn
...
function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
    ...
    for (uint256 i = round; i < round + maxRounds; i++) {
        if (rounds[i].status.requestId == requestId && rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
            ...
            // Copy the required number of elements from the randomness array to the storage array for the current round
            uint256 startIndex = (roundsProcessed - 1) * totalWinners;
            for (uint256 j = 0; j < totalWinners; j++) {
                rounds[i].status.randomness[j] = randomness[startIndex + j];
            }
    ...
    // Set next round if all rounds have been drawn
