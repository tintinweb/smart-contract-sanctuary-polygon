...
contract RelationshipRegistry is SismoConnect {
  ...
  function createRelationship(CreateRelationshipParams memory params) public returns (address) {
    ...
    validatezkProofs(params.zkProofs, params.groupIds);
  ...
  function validatezkProofs(bytes[2] memory zkProofs, bytes16[2] memory groupIds) public {
    for (uint256 i = 0; i < 2; i++) {
      require(validatezkProof(zkProofs[i], groupIds[i]), "Invalid zkProof");
    }
  }

  function validatezkProof(bytes memory zkProof, bytes16 groupId) public returns (bool) {
    verify({responseBytes: zkProof, claim: buildClaim(groupId)});
    return true;
  }
