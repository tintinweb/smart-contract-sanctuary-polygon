...
contract AuctionWithAdmin is Ownable {
    ...
    // Declare a set state variable
    EnumerableMap.UintToAddressMap private auctionSaleId;
    // Declare a set state variable
    EnumerableMap.UintToAddressMap private dealSaleId;
    ...
    uint256 private curr_auctionSale_id;
    uint256 private curr_dealSale_id;
    ...
    function createAuction(
        ...
        EnumerableMap.set(auctionSaleId, _tokenId, msg.sender);
    ...
    function createDeal(
        ...
        nft_token.transferFrom(msg.sender, address(this), _tokenId);
        EnumerableMap.set(dealSaleId, _tokenId, msg.sender);
    ...
    function buyDeal(uint256 _tokenId, uint256 _amount) public {
        ...
        EnumerableMap.remove(dealSaleId, _tokenId);
        emit BuyDeal(
            msg.sender,
            _tokenId,
            token_seller[_tokenId],
    ...
    function cancelAuction(uint256 _tokenId) public {
        ...
        delete auction[_tokenId];
        delete token_seller[_tokenId];
        curr_auctionSale_id -= curr_auctionSale_id;
        EnumerableMap.remove(auctionSaleId, _tokenId);
        emit AuctionCancelled(msg.sender, _tokenId, block.timestamp);
    }

    function viewCurrSaleId() public view returns (uint256){
        return curr_sale_id;
    }

    function viewCurrAuctionSaleId() public view returns (uint256){
        return curr_auctionSale_id;
    }

    function viewDealSaleId() public view returns (uint256){
        return curr_dealSale_id;
    }

    function viewDealLength() public view returns (uint256){
        return EnumerableMap.length(dealSaleId);
    }

    function viewSaleLength() public view returns (uint256){
        return EnumerableMap.length(saleId);
    }

    function viewAuctionSaleLength() public view returns (uint256){
        return EnumerableMap.length(auctionSaleId);
    }

    function viewDealDetails(uint256 index) public view returns (uint256){
        (uint256 id,) = EnumerableMap.at(dealSaleId, index);
        return id;
    }

    function viewSaleDetails(uint256 index) public view returns (uint256){
        (uint256 id,) = EnumerableMap.at(saleId, index);
        return id;
    }

    function viewAuctionSaleDetails(uint256 index) public view returns (uint256){
        (uint256 id,) =  EnumerableMap.at(auctionSaleId, index);
        return id;
    }
