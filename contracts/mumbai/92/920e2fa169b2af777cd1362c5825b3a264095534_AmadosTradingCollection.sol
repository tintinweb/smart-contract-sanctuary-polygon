  ...
  SupportsInterface
{
    ...
    virtual
  {
    ...
    delete ownerToId[_from][_tokenId];
  }
...
contract AmadosTradingCollection is NFTokenMetadata, Ownable 
{
    // Properties
    ...
    mapping(uint256 => string) private ipfsById;
    // Vars
    uint256 public current_minted = 0;
    uint public mintLimit = 5;
    
    // URI Data
    string private metaAddress = "https://ipfs.io/";
    ...
    constructor(string memory nftNameCollection, string memory symbol)
    {
        nftName     = nftNameCollection;
        nftSymbol   = symbol;
    ...
    {   // @dev Token URIs are generated dynamically on view requests. 
        ...
        bytes memory concat;
        concat = abi.encodePacked(metaAddress, ipfsById[tokenID]);
    ...
    function mint(string memory ipfs) public payable
    {
        require(isMintingActive, "Minting not available yet or paused for next round");
        require(nftClaimed[msg.sender] + 1 <= mintLimit, "Can't mint more than 5 limit per Wallet.");
        //require(msg.value >= nftPrice, "NFTs are expensive, need more ETH to afford that.");
        require(current_minted + 1 <= maxNft, "Not enough of those left in stock.");
    ...
    function updateNftPrice(uint newPrice) external onlyOwner
    {
        nftPrice = newPrice;
    }
