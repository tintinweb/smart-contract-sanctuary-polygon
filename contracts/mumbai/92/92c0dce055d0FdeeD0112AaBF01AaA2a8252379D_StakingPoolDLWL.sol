...
contract StakingPoolDLWL is Ownable, ReentrancyGuard, Metadata {
    ...
    event BlockRewardUpdated(uint256 _blockPerSec, uint256 _rewardPoolIndex);
    ...
    function init(bytes memory extraData) external {
        ...
        require(
            ...
            "End timestamp no must be greater than current"
        );

        require(
            farmInfo.endTimestamp >= farmInfo.withdrawTimestamp,
            "End block timestamp must be greater/eqal than withdraw timestamp"
        );
        ...
        rewardPool.push(
            RewardInfo({
                ...
                lastRewardBlock: block.timestamp > _localVars._startTimestamp
                    ? block.timestamp
                    : _localVars._startTimestamp,
                blockReward: _localVars._blockPerSec,
                accRewardPerShare: 0
            })
        );
    ...
    function getMultiplier(
        ...
        uint256 _from = _fromBlock >= rewardInfo.startTimestamp
            ? _fromBlock
            : rewardInfo.startTimestamp;
        uint256 to = rewardInfo.endTimestamp > _to
            ? _to
    ...
    function addRewardToken(
        ...
        require(farmInfo.endTimestamp > _startTimestamp, "Invalid start block");
        require(farmInfo.endTimestamp >= _endTimestamp, "Invalid end block");
        require(_endTimestamp > _startTimestamp, "Invalid end block");
        ...
        require(
            _lastRewardBlock >= block.timestamp,
        ...
        returns (uint256)
    {
        ...
        if (block.timestamp > rewardInfo.lastRewardBlock && lpSupply != 0) {
    ...
    function updatePool(uint256 _rewardInfoIndex) public {
        ...
        if (block.timestamp <= rewardInfo.lastRewardBlock) {
            return;
        }
        uint256 lpSupply = totalInputTokensStaked;

        if (lpSupply == 0) {
            rewardInfo.lastRewardBlock = block.timestamp;
            return;
        }
        uint256 multiplier = getMultiplier(
            rewardInfo.lastRewardBlock,
        ...
        rewardInfo.lastRewardBlock = block.timestamp < rewardInfo.endTimestamp
            ? block.timestamp
            : rewardInfo.endTimestamp;
    ...
    function updateBlockReward(
        ...
        rewardPool[_rewardTokenIndex].blockReward = _blockPerSec;
        emit BlockRewardUpdated(_blockPerSec, _rewardTokenIndex);
    }
