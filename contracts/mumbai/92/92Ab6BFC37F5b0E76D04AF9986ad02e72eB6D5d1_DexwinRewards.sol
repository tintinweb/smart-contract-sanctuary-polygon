...
error InvalidRoleValue();
error MaxClaimed();
error NotYet();

contract DexwinRewards is ERC20Capped, ERC20Burnable, Authorizable {
    struct User {
        address account;
        bool flag;
        string role;
    ...
    ) ERC20("DexwinToken", "DXT") ERC20Capped(cap * (10**decimals())) Authorizable(_owner) {
        require(ERC20.totalSupply() + _initialSupply <= cap, "ERC20Capped: cap exceeded");
        ERC20._mint(getOwner(), _initialSupply * (10**decimals()));
    }
    ...
    function claimReward() public {
        require(addressToUser[msg.sender].flag, "Cannot claim");
        User storage userAccount = addressToUser[msg.sender];
        string memory userRole = userAccount.role;
        ...
        if (claimedTokens == 0) {
            _mint(msg.sender, (claimLimit * (10**decimals())));
            addressToUser[msg.sender].lastClaimedAt = block.timestamp;
            addressToUser[msg.sender].claimedTokens = claimLimit;
            emit UserClaimed(
                msg.sender,
                true,
                userRole,
                ...
                claimFrequency,
                claimLimit,
                addressToUser[msg.sender].lastClaimedAt
            );
            ...
            if (pendingClaims > 0) {
                ...
                if (claimAble <= (tokensAssigned - claimedTokens)) {
                    ...
                    addressToUser[msg.sender].lastClaimedAt = block.timestamp;
                    addressToUser[msg.sender].claimedTokens += claimAble;
                    emit UserClaimed(
                        msg.sender,
                        true,
                        userRole,
                    ...
                    _mint(msg.sender, ((tokensAssigned - claimedTokens) * (10**decimals())));
                    addressToUser[msg.sender].lastClaimedAt = block.timestamp;
                    addressToUser[msg.sender].claimedTokens += (tokensAssigned - claimedTokens);
                    emit UserClaimed(
                        msg.sender,
                        true,
                        userRole,
                        ...
                        claimLimit,
                        claimFrequency,
                        claimedTokens + (tokensAssigned - claimedTokens),
                        addressToUser[msg.sender].lastClaimedAt
                    );
                }
            } else {
    ...
    function ownerMints(uint256 amount) public onlyAuthorized {
