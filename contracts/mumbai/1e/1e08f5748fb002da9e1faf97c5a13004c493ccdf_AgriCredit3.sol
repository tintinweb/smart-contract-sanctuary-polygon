...
contract AgriCredit3 is ERC721, AccessControl {
    ...
    constructor(uint256 mp) ERC721("AgriCredit3", "VICC3") { 
        mintPrice = mp; //200000000000000; //0.0002 ether;
        transferFee = 150; //1.5%
    ...
    function mintCredit(bytes32 _guid, uint256 _price, uint256 _newExchangeRate) whenNotPaused public payable returns (uint256) {
    ...
    function mintCreditInternal(bytes32 _guid, uint256 _price, uint256 _newExchangeRate, uint256 individualMsgValue) internal returns (uint256) {
        tokenCounter++;
        guidToTokenId[_guid] = tokenCounter;
        tokenToPrice[tokenCounter] = _price;
        tokenStatus[tokenCounter] = 0;
    ...
    function mintCredits(bytes32[] memory _guid, uint256 _price, uint256 _newExchangeRate, uint256 amount) whenNotPaused external payable returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < amount; i++) {
            res[i] = mintCreditInternal(_guid[i], _price, _newExchangeRate, individualMsgValue);
        }
        return(res);
    }
