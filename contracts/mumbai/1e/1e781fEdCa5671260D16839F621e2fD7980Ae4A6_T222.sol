...
contract T222 is Ownable, TimeForDay, SortList {
    ...
    mapping(uint256 => EnumerableSet.AddressSet) private Day_Top20InviteNum; //记录每天的推荐数前20名
    //用于处理日大单排行榜
    mapping(uint256 => mapping(uint256 => EnumerableSet.AddressSet)) private Day_InVestNum_AddressList; //记录每天每个推荐数的前20个地址
    mapping(uint256 => EnumerableSet.AddressSet) private Day_Top10InVestNum; //记录每天的推荐数前20名
    ...
    function updateReferBangList(uint256 _day, address _user) private {
        uint256 _takeNum = 20;
        uint256 _inviteNumber = Day_User_InviteNum[_day][_user] + 1;
        Day_User_InviteNum[_day][_user] = _inviteNumber;
        if (Day_Top20InviteNum[_day].contains(_user)) {
            return;
        }
        uint256 _num = Day_Top20InviteNum[_day].length();
        if (_num < _takeNum) {
            Day_Top20InviteNum[_day].add(_user);
        } else {
            address[] memory x = Day_Top20InviteNum[_day].values();
            for (uint256 i = 0; i < _takeNum; i++) {
                address _addressItem = x[i];
                if (Day_User_InviteNum[_day][_addressItem] < _inviteNumber) {
                    Day_Top20InviteNum[_day].remove(_addressItem);
                    Day_Top20InviteNum[_day].add(_user);
                    break;
                }
            }
    ...
    function updateInvestBangList(uint256 _day, address _user, uint256 _amount) private {
        uint256 _takeNum = 10;
        uint256 _investAmount = Day_User_InvestAmount[_day][_user] + _amount;
        Day_User_InvestAmount[_day][_user] = _investAmount;
        if (Day_Top10InVestNum[_day].contains(_user)) {
            return;
        }
        uint256 _num = Day_Top10InVestNum[_day].length();
        if (_num < _takeNum) {
            Day_Top10InVestNum[_day].add(_user);
        } else {
            address[] memory x = Day_Top10InVestNum[_day].values();
            for (uint256 i = 0; i < _takeNum; i++) {
                address _addressItem = x[i];
                if (Day_User_InvestAmount[_day][_addressItem] < _investAmount) {
                    Day_Top10InVestNum[_day].remove(_addressItem);
                    Day_Top10InVestNum[_day].add(_user);
                    break;
                }
            }
    ...
    function getDayTop2(address[] memory _List, uint256 _day) private view returns (address[] memory y){
        ...
        for (uint256 i = 0; i < _num; i++) {
            x[i] = Day_User_InviteNum[_day][_List[i]];
        }
        x = unique(x);
        x = sortUint256List(x);
        ...
        for (uint256 j = 0; j < _num2; j++) {
            ...
            for (uint256 k = 0; k < _num; k++) {
                if (Day_User_InviteNum[_day][_List[k]] == _num2Item) {
                    y[p] = _List[k];
                    p += 1;
                }
            }
    ...
    function getDayTop3(address[] memory _List, uint256 _day) private view returns (address[] memory y){
        ...
        uint256[] memory x = new  uint256[](_num);
        for (uint256 i = 0; i < _num; i++) {
            x[i] = Day_User_InvestAmount[_day][_List[i]];
        }
        x = unique(x);
        x = sortUint256List(x);
        uint256 _num2 = x.length;
        ...
        for (uint256 j = 0; j < _num2; j++) {
            uint256 _num2Item = x[j];
            for (uint256 k = 0; k < _num; k++) {
                if (Day_User_InvestAmount[_day][_List[k]] == _num2Item) {
                    y[p] = _List[k];
                    p += 1;
                }
            }
    ...
    function getDayTop(uint256 _timestamp, uint256 _type) public view returns (address[] memory _List, uint256 _day) {
        ...
        _day = getYearMonthDay(_timestamp);
        if (_type == 20) {
            _List = Day_Top20InviteNum[_day].values();
            _List = getDayTop2(_List, _day);
        } else if (_type == 10) {
            _List = Day_Top10InVestNum[_day].values();
