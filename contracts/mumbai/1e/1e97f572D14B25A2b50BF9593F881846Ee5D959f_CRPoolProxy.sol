...
contract CRPoolProxy is Ownable, ERC1155Holder {
    ...
    function createPool(
        ...
        require(ICRPool(_crpool).owner() == _msgSender(), "ONLY_POOL_OWNER");

        address[] memory tokens = ICRPool(_crpool).initialTokens();
        uint256[] memory balances = ICRPool(_crpool).initialBalances();

        // Pull the xTokens required for the pool creation
        for (uint256 i = 0; i < tokens.length; i++) {
            _pullXTokenToPool(ICRPool(_crpool), IXToken(tokens[i]), balances[i]);
        }

        // Execute the operation
        ICRPool(_crpool).createPool(
            _initialSupply,
            _minimumWeightChangeBlockPeriodParam,
            _addTokenTimeLockInBlocksParam
        );

        // Send the resulting xLPT from the pool creation
        _pushXLptToSender(ICRPool(_crpool));
    }

    function createPool(address _crpool, uint256 _initialSupply) external {
        require(ICRPool(_crpool).owner() == _msgSender(), "ONLY_POOL_OWNER");
        ...
        // Execute the operation
        ICRPool(_crpool).createPool(_initialSupply);
    ...
    function getUpdateWeightCost(
        ...
        return
            SmartPoolManager.getUpdateWeightCost(
                IConfigurableRightsPool(_crpool),
                ICRPool(_crpool).bPool(),
                _token,
                _newWeight
            );
    }
    ...
    function updateWeight(
        ...
        IBPool bPool = ICRPool(_crpool).bPool();
        ...
        uint256 cost = getUpdateWeightCost(_crpool, _token, _newWeight);

        // In order to keep the pool stable
        // - Provide xLPT to the pool if reducing weight
        // - provide xToken if increasing weight
        if (_newWeight < currentWeight) {
            _pullXLptToPool(ICRPool(_crpool), cost);
        } else {
            _pullXTokenToPool(ICRPool(_crpool), IXToken(_token), cost);
        }

        // Execute the operation
        ICRPool(_crpool).updateWeight(_token, _newWeight);
    ...
    function applyAddToken(address _crpool) external {
        require(ICRPool(_crpool).owner() == _msgSender(), "ONLY_POOL_OWNER");

        SmartPoolManager.NewTokenParams memory newToken = ICRPool(_crpool).newToken();

        // Pull from the sender the amount of xToken needed to add the new token
        _pullXTokenToPool(ICRPool(_crpool), IXToken(newToken.addr), newToken.balance);

        // Execute operation
        ICRPool(_crpool).applyAddToken();
     ...
     */
    function getRemoveTokenCost(address _crpool, address _token) public view returns (uint256) {
        return SmartPoolManager.getRemoveTokenCost(IConfigurableRightsPool(_crpool), ICRPool(_crpool).bPool(), _token);
    }

    /**
     * @notice Computes the earnings from removing a token from the pool
     */
    function getRemoveTokenEarnings(address _crpool, address _token) external view returns (uint256) {
        return
            SmartPoolManager.getRemoveTokenEarnings(IConfigurableRightsPool(_crpool), ICRPool(_crpool).bPool(), _token);
    }

    function removeToken(address _crpool, address _token) external {
        require(ICRPool(_crpool).owner() == _msgSender(), "ONLY_POOL_OWNER");

        // Pull the xLPT cost from the sender
        uint256 xLPTCost = getRemoveTokenCost(_crpool, _token);
        _pullXLptToPool(ICRPool(_crpool), xLPTCost);

        // Execute operation
        ICRPool(_crpool).removeToken(_token);
    ...
    /**
     * @notice Computes the amounts in required to join a smart pool and get a certain LPT amount out
     * while providing up to a maximum amount of tokens in.
     */
    function getJoinPoolAmountsIn(
        address _crpool,
        uint256 _poolAmountOut,
        uint256[] calldata _maxAmountsIn
    ) external view returns (uint256[] memory) {
        return
            SmartPoolManager.joinPool(
                IConfigurableRightsPool(_crpool),
                ICRPool(_crpool).bPool(),
                _poolAmountOut,
                _maxAmountsIn
    ...
    /**
     * @dev Join the `_crpool`, getting `_poolAmountOut` pool tokens. This will pull some of each of the currently
    ...
    function joinPool(
        ...
        address[] memory tokens = ICRPool(_crpool).bPool().getCurrentTokens();

        // Pull the xTokens required for joining the pool
        for (uint256 i = 0; i < tokens.length; i++) {
            _pullXTokenToPool(ICRPool(_crpool), IXToken(tokens[i]), _maxAmountsIn[i]);
        }

        // Execute the operation
        ICRPool(_crpool).joinPool(_poolAmountOut, _maxAmountsIn, msg.sender);
        ...
        emit JoinPool(_msgSender(), _crpool, _poolAmountOut);
    }

    /**
     * @notice Computes the amounts out obtained by exiting a smart pool while providing a certain LPT amount in.
     */
    function getExitPoolAmountsOut(
        address _crpool,
        uint256 _poolAmountIn,
        uint256[] calldata _minAmountsOut
    ) external view returns (uint256[] memory) {
        (, , uint256[] memory amountsOut) = SmartPoolManager.exitPool(
            IConfigurableRightsPool(_crpool),
            ICRPool(_crpool).bPool(),
            _poolAmountIn,
            _minAmountsOut
    ...
    /**
     * @dev Exit the `_crpool`, paying `_poolAmountIn` pool tokens and getting some of each of the currently trading
     * tokens in return. These values are limited by the array of `_minAmountsOut` in the order of the pool tokens.
     *
     * @param _crpool Pool address.
    ...
    function exitPool(
        ...
        _pullXLptToPool(ICRPool(_crpool), _poolAmountIn);

        // Execute the operation
        ICRPool(_crpool).exitPool(_poolAmountIn, _minAmountsOut);
        ...
        emit ExitPool(_msgSender(), _crpool, _poolAmountIn);
    }

    /**
     * @notice Computes the LPT amount out obtained by joinswapExternAmountIn a certain token with a certain amount.
     */
    function getJoinswapExternAmountInPoolAmountOut(
        address _crpool,
        address _tokenIn,
        ...
        return
            SmartPoolManager.joinswapExternAmountIn(
                IConfigurableRightsPool(_crpool),
                ICRPool(_crpool).bPool(),
                _tokenIn,
                _tokenAmountIn,
    ...
    function joinswapExternAmountIn(
        ...
        _pullXTokenToPool(ICRPool(_crpool), IXToken(_tokenIn), _tokenAmountIn);

        // Execute the operation
        _poolAmountOut = ICRPool(_crpool).joinswapExternAmountIn(
            _tokenIn,
            _tokenAmountIn,
            _minPoolAmountOut,
            msg.sender
    ...
    /**
     * @notice Computes the token in amount required to obtain a certain LPT amount out by joinswapPoolAmountOut
     * while providing up to a maximum amount of tokens in.
     */
    function getJoinswapPoolAmountOutTokenInAmount(
        address _crpool,
        address _tokenIn,
        uint256 _poolAmountOut,
        ...
        return
            SmartPoolManager.joinswapPoolAmountOut(
                IConfigurableRightsPool(_crpool),
                ICRPool(_crpool).bPool(),
                _tokenIn,
                _poolAmountOut,
    ...
    function joinswapPoolAmountOut(
        ...
        // Pull required xToken from the sender
        _pullXTokenToPool(ICRPool(_crpool), IXToken(_tokenIn), _maxAmountIn);

        // Execute the operation
        _tokenAmountIn = ICRPool(_crpool).joinswapPoolAmountOut(_tokenIn, _poolAmountOut, _maxAmountIn, msg.sender);
    ...
    /**
     * @notice Computes the token out amount obtained by exitswapPoolAmountIn while providing a certain LPT amount in.
     */
    function getExitswapPoolAmountInTokenOutAmount(
        address _crpool,
        address _tokenOut,
        uint256 _poolAmountIn,
        uint256 _minAmountOut
    ) external view returns (uint256) {
        (, uint256 tokenOutAmount) = SmartPoolManager.exitswapPoolAmountIn(
            IConfigurableRightsPool(_crpool),
            ICRPool(_crpool).bPool(),
            _tokenOut,
            _poolAmountIn,
    ...
    function exitswapPoolAmountIn(
        ...
        // Execute the operation
        _tokenAmountOut = ICRPool(_crpool).exitswapPoolAmountIn(_tokenOut, _poolAmountIn, _minAmountOut);
    ...
    /**
     * @notice Computes the pool amount in amount required to obtain a certain token amount out
     * by exitswapExternAmountOut.
     */
    function getExitswapExternAmountOutPoolAmountIn(
        address _crpool,
        address _tokenOut,
        uint256 _tokenAmountOut,
        uint256 _maxPoolAmountIn
    ) external view returns (uint256) {
        (, uint256 tokenOutAmount) = SmartPoolManager.exitswapExternAmountOut(
            IConfigurableRightsPool(_crpool),
            ICRPool(_crpool).bPool(),
            _tokenOut,
            _tokenAmountOut,
    ...
    function exitswapExternAmountOut(
        ...
        // Pull required xLPT from the sender
        _pullXLptToPool(ICRPool(_crpool), _maxPoolAmountIn);

        // Execute the operation
        _poolAmountIn = ICRPool(_crpool).exitswapExternAmountOut(_tokenOut, _tokenAmountOut, _maxPoolAmountIn);
    ...
    function _pullXTokenToPool(
        ...
        _xToken.approve(address(_crpool), _amount);
    }

    function _pullXLptToPool(ICRPool _crpool, uint256 _amount) internal {
        IXToken xLPT = IXToken(xTokenWrapper.tokenToXToken(address(_crpool)));
        ...
        _crpool.approve(address(_crpool), _amount);
    }
    ...
    function _pushXLptToSender(ICRPool _crpool) internal {
        uint256 amount = _getContractBalance(_crpool);

        // Wrap the amount
        _crpool.approve(address(xTokenWrapper), amount);
        require(xTokenWrapper.wrap(address(_crpool), amount), "ERR_WRAP_POOL");

        // Send the wrapped amount
        _transferFromContractToSender(IXToken(xTokenWrapper.tokenToXToken(address(_crpool))), amount);
    }
