...
contract VCMarketplaceFixedPriceERC1155 is IVCMarketplaceFixedPriceERC1155, VCMarketplaceBase, ERC1155Holder {
    ...
    function unlistFixedPrice(uint256 _tokenId, uint256 _amount) external {
        ...
        if (listing.minted) {
            IVCArtNftERC1155(address(artNft)).safeTransferFrom(address(this), listing.seller, _tokenId, _amount, "");
        }
    ...
    function _newList(ListingData memory data, uint256 _amount) internal {
        bool minted = IVCArtNftERC1155(artNft).exists(data.tokenId);
        if (!minted) {
            IVCArtNftERC1155(artNft).requireCanRequestMint(msg.sender, data.tokenId, _amount);
        } else {
            IVCArtNftERC1155(artNft).safeTransferFrom(msg.sender, address(this), data.tokenId, _amount, "");
        }
    ...
    function _updateList(uint256 _tokenId, uint256 _amount, uint256 _listPrice) internal {
        ...
        if (!listing.minted) {
            IVCArtNftERC1155(address(artNft)).requireCanRequestMint(msg.sender, _tokenId, listing.amount);
        } else {
            IVCArtNftERC1155(address(artNft)).safeTransferFrom(msg.sender, address(this), _tokenId, _amount, "");
        }
    ...
    function _minting(FixedPriceListing memory listing, uint256 _tokenId) internal {
        if (!listing.minted) {
            IVCArtNftERC1155(address(artNft)).mintTo(_tokenId, address(this), listing.amount);
            fixedPriceListings[_tokenId][listing.seller].minted = true;
        }
        IVCArtNftERC1155(address(artNft)).safeTransferFrom(address(this), msg.sender, _tokenId, 1, "");
