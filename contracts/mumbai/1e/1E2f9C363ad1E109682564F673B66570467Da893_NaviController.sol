    ...
    IBridgedData
{
    ...
    uint256 public offChainReserves;
    ...
    modifier onlyBridge() {
        require(_bridgeContract == msg.sender, "Navi: caller is not a bridge");
        _;
    }
    ...
    function bridgedData() external view override returns (bytes memory) {
        return abi.encode(getTotalReserves());
    }
    ...
    function receiveBridgedData(bytes memory data)
        ...
        onlyBridge
    {
        offChainReserves += abi.decode(data, (uint256));
    }
    ...
    function tokensReceivedInternal(
        ...
        if (msg.sender == daoToken) {
            amountDebt = wmul(
                ...
                getTotalReserves() + offChainReserves
            );
            //both offchain NaviDao token supply and nToken reserves are updated at once by the bridge
            //so this is safe, they are always in sync. You can think of amountDebt as:
            //amountDebt = wmul(_reservesClaimableFactor, wmul(_oracle.price(1), amount))
