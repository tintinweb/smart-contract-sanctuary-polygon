...
contract LedgerManager is ReentrancyGuard {
    ...
    */
    function addOrder(LedgerTypes.OrderInfo memory orderInfo, uint256 endsInSec, uint256 deadline, uint256 rewardBaseAmount, bytes memory signature) external payable returns (uint256) {
        ...
        dailyLimit.withdrawReward(msg.sender, rewardReduction.reduceReward(rewardToken.totalSupply(), rewardBaseAmount));
...
contract DailyLimit {
    ...
    constructor (SystemContext systemContext_, uint256 startingPoolSize_, uint256[] memory startTimestamps_, uint256[] memory maxPerDays_, uint256[] memory maxExistingInPools_) {
        systemContext = systemContext_;
        rewardToken = systemContext_.brightPoolToken();
        require(startTimestamps_.length == maxPerDays_.length, "Arr len incorrect");
        require(startTimestamps_.length == maxExistingInPools_.length, "Arr len incorrect");
        require(startTimestamps_.length != 0, "Arrays empty");

        lastReward = startTimestamps_[0];
        currentlyInPool = startingPoolSize_;

        for (uint256 i = 0; i < startTimestamps_.length; i++) {
            schedules.push(Schedule(startTimestamps_[i], maxPerDays_[i], maxExistingInPools_[i]));
            if (startTimestamps_.length > i + 1) {
    ...
    function _recalculatePool() internal {
        // solhint-disable-next-line not-rely-on-time
        if (lastReward == block.timestamp) {
            return;
        }

        uint256 firstScheduleIndex = scheduleIndex;

        for (uint256 i = scheduleIndex; i < schedules.length; i++) {
            // solhint-disable-next-line not-rely-on-time
            if (schedules[i].startTimestamp < block.timestamp) {
                continue;
            }
            scheduleIndex = i - 1;
            break;
        ...
        for (uint256 i = scheduleIndex + 1; i > firstScheduleIndex; i--) {
            Schedule memory schedule = schedules[i - 1];
            // solhint-disable-next-line not-rely-on-time
            currentlyInPool += (block.timestamp - _max(schedule.startTimestamp, lastReward)) * schedule.maxPerDay / 86400;
            if (currentlyInPool > schedule.maxExistingInPool) {
                currentlyInPool = schedule.maxExistingInPool;
                break;
            }
        }
    ...
    function _calculateReward(uint256 reward) internal view returns(uint256) {
        Schedule memory schedule = schedules[scheduleIndex];
        if (currentlyInPool > schedule.maxPerDay) {
            return reward * _findMiddle(schedule.maxPerDay, currentlyInPool) / 10000;
        } else {
            return reward * _findMiddle(currentlyInPool, schedule.maxPerDay) / 10000;
        }
    }

    function withdrawReward(address userAddress, uint256 userReward) external onlyManagerRole() {
        _recalculatePool();
        assert(rewardToken.transfer(userAddress, _calculateReward(userReward)));
    }
}
