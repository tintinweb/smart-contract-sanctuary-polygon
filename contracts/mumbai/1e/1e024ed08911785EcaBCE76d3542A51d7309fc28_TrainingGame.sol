...
abstract contract TrainingGameRewards is Initializable, TrainingGameContracts {
    ...
    function getMultiplierMinForReward(uint256 _rewardId, uint8 _proficiency) external view returns(uint16) {
        return rewardIdToMultiplier[_rewardId].min[_proficiency];
    }
    
    function getMultiplierMaxForReward(uint256 _rewardId, uint8 _proficiency) external view returns(uint16) {
        return rewardIdToMultiplier[_rewardId].max[_proficiency];
    }
