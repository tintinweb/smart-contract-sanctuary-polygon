...
/// @title Interface for VVSErc1155Factory


pragma solidity ^0.8.9;


interface IVVSErc1155Factory {
    /**
    ...
    * @param owner The address of the creator which owns the new collection.
    ...
    */
    event CollectionCreated(address collectionContract, address owner, uint256 version, string name, string symbol, uint256 nonce);
...
/// @title Interface for VVSErc1155Factory Initializer

pragma solidity ^0.8.9;


interface IVVSErc1155FactoryInitializer {
...
import { IVVSErc1155FactoryInitializer } from "./interfaces/IVVSErc1155FactoryInitializer.sol";
import { IVVSErc1155Factory } from "./interfaces/IVVSErc1155Factory.sol";
 ...
 */ 
contract VVSErc1155Factory is IVVSErc1155Factory, IVVSErc1155FactoryInitializer, OwnableUpgradeable {
    ...
    */
    ...
    function createCollection(
        ...
        require(bytes(name).length != 0, "VVSErc1155Factory: name is required");
        require(bytes(symbol).length != 0, "VVSErc1155Factory: symbol is required");

        // This reverts if the NFT was previously created using this implementation version + msg.sender + nonce
        collectionAddress = implementation.cloneDeterministic(_getSalt(_msgSender(), nonce));
        console.log("collection address to:", collectionAddress);

        IVVSErc1155Initializer(collectionAddress).initialize(name, symbol, baseuri);
        
        emit CollectionCreated(collectionAddress, _msgSender(), version, name, symbol, nonce);
    }
    ...
    */
    function _updateImplementation(address _implementation) private {
        require(_implementation.isContract(), "VVSErc1155Factory: implementation is not a contract");
        implementation = _implementation;
        
        unchecked {   // Version cannot overflow 256 bits.
            version++;
        }
