...
import "../../contracts/libraries/OSBToken.sol";

contract $OSBToken is OSBToken {
...
contract OSBToken is ERC20Upgradeable, OwnableUpgradeable {
    ...
    /// @dev Emit an event when the minTo is successfully.
    event MintTo(address indexed to, uint256 indexed amount);

    /// @dev Emit an event when the mintToList is successfully.
    event MintToList(address[] addresses, uint256[] amounts);
    ...
    function initialize(
        ...
        if (_issueAmount > 0) _mintTo(_owner, _issueAmount);
        emit ContractDeployed(_owner, _name, _symbol, _contractUri, _issueAmount, _maxTotalSupply);
    }
    ...
    modifier exceededTotalSupply(uint256 amount) {
        if (maxTotalSupply > 0 && totalSupply() + amount > maxTotalSupply) revert ExceededMaximumTotalSupply();
        _;
    }
    ...
    function mintToList(address[] memory _addresses, uint256[] memory _amounts) external onlyOwner {
        if (!(_addresses.length > 0 && _addresses.length == _amounts.length)) revert InvalidParameters();

        for (uint256 i = 0; i < _addresses.length; ++i) {
            _mintTo(_addresses[i], _amounts[i]);
        }

        emit MintToList(_addresses, _amounts);
    }

    function _mintTo(address _to, uint256 _amount) private exceededTotalSupply(_amount) {
        _mint(_to, _amount);
    }
    ...
    function setContractURI(string calldata _uri) external onlyOwner {
        ...
        contractURI = _uri;
        emit SetContractURI(oldUri, _uri);
    }
}