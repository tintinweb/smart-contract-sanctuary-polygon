...
contract pacificrimcontract is Ownable {
    ...
    function presaleMint(uint256 _tokenId) public payable{
        require(msg.value == ethAmount,"invalid payment");
        require(block.timestamp >= preSaleTime,"sale not started yet");
        require(block.timestamp<= preSaleTime.add(preSaleDuration),"sale ended");
        require(mintBalancePreSale[msg.sender].add(1) < preSaleMintLimit,"cannot mint more then wl limit");
        require(mintedSupply.add(1) <= cappedSupply,"cannot mint capped limit reached");
    ...
    function whitelistMint(uint256 _tokenId) public payable{
        require(msg.value == ethAmount,"invalid payment");
        require(block.timestamp >= whitelistSaleTime,"sale not started yet");
        require(block.timestamp<= whitelistSaleTime.add(whitelistSaleDuration),"sale ended");
        require(mintBalanceWhitelistSale[msg.sender].add(1) < whitelistSaleMintLimit,"cannot mint more then wl limit"); //= sign
        require(mintedSupply.add(1) <= cappedSupply,"cannot mint capped limit reached");
    ...
    function publicMint(uint256 _tokenId) public payable{
        require(msg.value == ethAmount,"invalid payment");
        require(block.timestamp >= publicSaleTime,"sale not started yet");
        require(block.timestamp<= publicSaleTime.add(publicSaleDuration),"sale ended");
        require(mintedSupply.add(1) <= cappedSupply,"cannot mint capped limit reached");
