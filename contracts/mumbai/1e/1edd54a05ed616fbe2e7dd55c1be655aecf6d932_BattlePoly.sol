...
contract BattlePoly {
    ...
    function initialize(address dogewood_, address vrf_, address quest_) external {
        ...
        policyVariables[PolicyVariables.ENEMY_STAT_POINTS_PERC] = 200000; // Enemy Stat Points %	2000.00%
        ...
        // for (uint16 level_ = 1; level_ <= 20; level_++) 
        {
            for (uint8 class_ = classFrom_; class_ < classTo_; class_++) { // for (uint8 class_ = 0; class_ < 8; class_++) {
                for (uint8 breed_ = 0; breed_ < 8; breed_++) {
                    ...
                    int coat_ = (classInfo[class_].statInfo.coat + breedInfo[breed_].coat) * int(levelInfo[level_].heroStatPoints) / int(PERCENT_MULTIPLIER);
                    int magic_ = (classInfo[class_].statInfo.magic + breedInfo[breed_].magic) * int(levelInfo[level_].heroStatPoints) / int(PERCENT_MULTIPLIER);
                    int sneak_ = (classInfo[class_].statInfo.sneak + breedInfo[breed_].sneak) * int(levelInfo[level_].heroStatPoints) / int(PERCENT_MULTIPLIER);
    ...
    function updateEnemyStatPointsPercent(uint256 perc_, uint16 levelFrom_, uint16 levelTo_) external { // perc_ = percent * 100 (60%=6000)
        require(msg.sender == admin);
            ...
            for (uint8 class_ = 0; class_ < 8; class_++) {
                for (uint8 breed_ = 0; breed_ < 8; breed_++) {
                    ...
                    statInfo[level_][class_][breed_][1] = StatInfo(
                            ...
                            ds_.coat * int(perc_) / int(PERCENT_MULTIPLIER),
                            ds_.magic * int(perc_) / int(PERCENT_MULTIPLIER),
                            ds_.sneak * int(perc_) / int(PERCENT_MULTIPLIER),
    ...
    function combatSingle(uint256 dogeId) external {
        ...
        // if(lbPoints_ > 0) {
        //     // // add leaderboard log
        //     // uint256[] memory doges_ = new uint256[](1); doges_[0] = dogeId;
        //     // address[] memory owners_ = new address[](1); owners_[0] = msg.sender;
        //     // uint256[] memory scores_ = new uint256[](1); scores_[0] = lbPoints_;
