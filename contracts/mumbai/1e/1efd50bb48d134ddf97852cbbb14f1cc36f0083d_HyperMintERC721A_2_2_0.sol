    ...
    OperatorFilterer
{
    ...
    function name()
        public
        view
        override(IHyperMintERC721A_2_2_0, ERC721A, IERC721A)
    ...
    /// @dev no need for the onlyContractManager modifier
    ///      in each call address, msg.value, msg.sender,
    ///      and storage are all inherited
    function multicall(bytes[] calldata calls) external {
        for (uint256 i = 0; i < calls.length; ) {
            (bool success, bytes memory r) = address(this).delegatecall(
                calls[i]
            );
            if (!success) {
