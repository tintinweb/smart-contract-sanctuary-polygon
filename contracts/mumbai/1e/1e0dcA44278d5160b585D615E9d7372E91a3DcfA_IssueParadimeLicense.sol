...
import "./DigitalAudioTracks.sol";
import "./DigitalAudioTracksERC20.sol";

contract IssueParadimeLicense {
    ...
    //Fees
    uint256 private standardLicenseFee; 
    mapping(address => uint256) private registeredLicenseFees;
    mapping(bytes32 => bool) private registeredProjects;

    constructor() {
        //TODO: Add From Input
        standardLicenseFee = 1;
    }

    function issueNativePaymentLicense(
        bytes32 _projectId,
        address _protocol,
        //Shares
        ...
        uint[] memory _royalties, //_exportRoyalty,_sampleRoyalty,_remixRoyalty,_resaleRoyalty,_coverRoyalty
        bool[] memory _permissions //_publishPermission,_exportPermission,_samplePermission,_remixPermission,_coverPermission
    ) public payable { //onlyProjectAdmin payable //isValidCreatorStakes(_owners, _residualOwners) 
        require(!registeredProjects[_projectId], "A License was issued for this project already.");
        ...
        //Todo: Charge Standard Fee
        (bool paymentSuccess, ) = payable(address(this)).call{ value: msg.value}("");
        require(paymentSuccess, "Payment was not successful, failed to issue token");
        ...
        DigitalAudioTracks _factory = DigitalAudioTracks(_protocol);
        _factory.issueToken(
            _projectId,
            address(_agreement),
            msg.sender,
        ...
        registeredProjects[_projectId] = true;
    }

function issueERC20PaymentLicense(
        bytes32 _projectId,
        address _protocol,
        //Shares
        ParadimeTokens.Shares[] memory _owners,
        ...
        DigitalAudioTracksERC20 _factory = DigitalAudioTracksERC20(_protocol);
        _factory.issueToken(
            _projectId,
            address(_agreement),
            msg.sender,
