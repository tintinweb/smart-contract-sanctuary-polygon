...
import "./ABTProofVerifier.sol";

contract ABTProofTestContract {
    ABTProofVerifier public verifier;

    constructor(address _verifierAddress) {
        verifier = ABTProofVerifier(_verifierAddress);
    }

    function testVerifyProof(
        uint64 _subscriptionId,
        bytes calldata _apiKey,
        string calldata _key
    ) public {
        // Prepare calldata for the testFunction call
        bytes memory data = abi.encodeWithSignature("testFunction()");

        bytes32 requestId = verifier.verifyProof({
        ...
        emit ProofRequestSubmitted(requestId);
    }

    function testFunction() public returns (bool) {
        uint64 value = 1;
        emit ProofVerified(keccak256(abi.encodePacked(value)));
        return true;
    }
    ...
    event ProofRequestSubmitted(bytes32 id);
    event ProofVerified(bytes32 result);
}
...
contract ABTProofVerifier is FunctionsClient, ConfirmedOwner {
    using Functions for Functions.Request;

    string public testValue = "not filled";

    address public abtTokenAddress;
    ...
    constructor(
        ...
        abtTokenAddress = _abtTokenAddress;
        source = _source;
    }
    ...
    event RequestSend(bytes32 indexed requestId);
    ...
    function verifyProof(
        ...
        require(abtTokenAddress != address(0), "ABTToken address is not set");
        require(bytes(source).length > 0, "Source is not set");
        require(bytes(key).length > 0, "Key is not set");
        require(targetAddress != address(0), "Target address is not set");
        require(subscriptionId > 0, "Subscription ID is not set");

        IABTTokenV1 abtToken = IABTTokenV1(abtTokenAddress);
        Functions.Request memory req;

        (
            string memory _key,
            string memory _zkproof,
            string memory _publicSignals
        ) = abtToken.getProof(targetAddress, key);
        ...
        string[] memory dynamicArgs = new string[](3);
        dynamicArgs[0] = _key;
        dynamicArgs[1] = _publicSignals;
        dynamicArgs[2] = _zkproof;

        if (dynamicArgs.length > 0) req.addArgs(dynamicArgs);

        bytes32 assignedReqID = sendRequest(req, subscriptionId, 300000);

        callData[assignedReqID] = data;

        emit RequestSend(assignedReqID);
    ...
    function fulfillRequest(
        ...
        bytes memory data = callData[requestId];

        if (err.length > 0 || keccak256(data) == keccak256(bytes(""))) {
            // The request was not successful, delete the call data
            delete callData[requestId];
            emit RequestFailed(requestId);
            return;
        }

        // Call the function with the original call data
    ...
    function updateABTTokenAddress(address _abtTokenAddress) public onlyOwner {
        abtTokenAddress = _abtTokenAddress;
    }
}
