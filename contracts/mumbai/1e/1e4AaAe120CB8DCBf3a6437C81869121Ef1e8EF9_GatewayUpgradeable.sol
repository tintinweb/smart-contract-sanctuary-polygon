    ...
    ReentrancyGuardUpgradeable
{
    ...
    // destChainId{string} => (destChainType{uint64} => bridgeFees)
    mapping(string => mapping(uint64 => uint256)) public bridgeFees;
    ...
    function setBridgeFees(
        uint64 _destChainType,
        string memory _destChainId,
        uint256 _bridgeFee
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        bridgeFees[_destChainId][_destChainType] = _bridgeFee;
    }
    ...
    function requestToDest(
        ...
        uint256 _crossTalkFees = bridgeFees[destChainParams.destChainId][destChainParams.destChainType];
        require(msg.value >= _crossTalkFees, "insufficient fees sent");
    ...
    function readQueryToDest(
        ...
        uint256 _crossTalkFees = bridgeFees[destChainParams.destChainId][destChainParams.destChainType];
        require(_crossTalkFees != 0, "fee not set for this chain");
        require(msg.value >= _crossTalkFees, "insufficient fees sent");
    ...
    function crossTalkAck(
        ...
        emit CrossTalkAckSubmitEvent(crossTalkAckPayload.eventIdentifier, chainType, chainId, success, data);
    }
...
interface IGateway {
    ...
    function getCrossTalkFees(string memory destChainId, uint64 chainType) external view returns (uint256 fees);
