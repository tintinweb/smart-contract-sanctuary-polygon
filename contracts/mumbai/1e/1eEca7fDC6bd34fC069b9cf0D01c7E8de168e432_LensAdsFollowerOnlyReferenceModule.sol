...
// debo recibir numeros que cuadren o permitir withdraw del balance
    ...
    FeeModuleBase
{
    ...
    mapping(uint256 => mapping(address => uint256)) private s_profileToGlobalBudget; // Current global budget by profile by currency
    mapping(uint256 => mapping(uint256 => uint256)) private s_profileToPostToNumberOfMirrors; // Mirrors by profile by post
    mapping(uint256 => mapping(uint256 => bool)) private s_followerHasMirrored; // follower to publicacion to boolean, true if has already mirrored before
    ...
    function initializeReferenceModule(
        ...
        if (_budget == 0) revert Errors.LensAds__BudgetEqualZero();
        if (_budget > s_profileToGlobalBudget[profileId][_currency])
            revert Errors.LensAds__BudgetBiggerThanGlobalBudget();
        if (_reward == 0) revert Errors.LensAds__RewardEqualZero();
    ...
    function processMirror(
        ...
        if (s_followerHasMirrored[profileId][pubIdPointed] == true) {
        ...
        if (__currentBudget < __reward) {
            ...
            s_profileToGlobalBudget[profileIdPointed][__currency] -= __reward;
    ...
    function fundThisPost(uint256 _profileId, uint256 _pubId, uint256 _budget) private {
        s_profileToPostToCurrentBudget[_profileId][_pubId] = _budget;
    }

    function fundMyGlobalBudget(uint256 profileId, uint256 globalBudget, address _currency) public {
        address budgetOwner = IERC721(HUB).ownerOf(profileId);
        IERC20(_currency).safeTransferFrom(budgetOwner, address(this), globalBudget);
        IERC20(_currency).safeTransferFrom(address(this), i_modulens, (globalBudget * i_fee) / 100);
        s_profileToGlobalBudget[profileId][_currency] +=
            globalBudget -
            ((globalBudget * i_fee) / 100);
    }

    // withdraw reminder, function to empty the funds in the mirror
    function withdrawBudget(address _currency, uint256 _profileId, uint256 _amount) public {
        if (IERC721(HUB).ownerOf(_profileId) != msg.sender)
            revert Errors.LensAds__NotSenderProfile();
        if (_amount > s_profileToGlobalBudget[_profileId][_currency])
            revert Errors.LensAds__NotEnoughGlobalBudget();
        else {
            IERC20(_currency).safeTransferFrom(address(this), msg.sender, _amount);
            s_profileToGlobalBudget[_profileId][_currency] -= _amount;
        }
    }

    function getGlobalBudget(address _currency, uint256 _profileId) public view returns (uint256) {
        return s_profileToGlobalBudget[_profileId][_currency];
    }

    function getPostBudget(uint256 _pubId, uint256 _profileId) public view returns (uint256) {
        return s_profileToPostToCurrentBudget[_profileId][_pubId];
    }
