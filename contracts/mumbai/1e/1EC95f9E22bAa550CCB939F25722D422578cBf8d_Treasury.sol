 ...
 */
contract Treasury is Ownable, ISwapReceiver, ERC20 {
    ...
    uint public totalFundsInvested;
    mapping(address => uint256) public availableCollateral;
    mapping(address => uint256) public strategiesApprovedBalances;

    event NewDepositCollateral(uint amount, address user);
    event NewWithdrawalCollateral(uint amount, address user);
    event NewDeposit(uint amount, address user);
    event NewWithdrawal(uint amount, address user);
    event NewSettlement(int amount);

    constructor(address _stableCoin, address _swaps) Ownable() ERC20("Boardwalk Treasury Token","tBORD"){
    ...
    function withdrawal(uint _amount) external{
        ...
        uint total = stablecoin.balanceOf(address(this)) + totalFundsInvested;
        stablecoin.safeTransfer(msg.sender, _amount * total / totalSupply());
        emit NewWithdrawal(_amount, msg.sender);
    }

    function addCollateral(uint256 _amount) external{
        stablecoin.safeTransferFrom(msg.sender, address(this), _amount);
        _addCollateralForUser(msg.sender, _amount);
        emit NewDepositCollateral(_amount, msg.sender);
    }

    function withdrawalCollateral(uint256 _amount) external{
        require(availableCollateral[msg.sender] >= _amount, "You cannot remove this much collateral");
        _removeCollateralForUser(msg.sender, _amount);
        stablecoin.safeTransfer(msg.sender, _amount);
        emit NewWithdrawalCollateral(_amount, msg.sender);
    }
    ...
    function settle(int _usdSettlement, uint _collateralToFree, address _recipient, address _strategy) override external{
        ...
        if(_usdSettlement > 0){
            if(int(getBalance()) >= _usdSettlement){
                ...
                totalFundsInvested = 
                (uint(_usdSettlement) > totalFundsInvested) 
                ? 0 
                : totalFundsInvested - uint(_usdSettlement);
