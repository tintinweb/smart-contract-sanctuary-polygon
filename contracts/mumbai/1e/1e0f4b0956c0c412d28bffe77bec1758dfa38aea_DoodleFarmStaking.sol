...
contract DoodleFarmStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    modifier requireERC20Token() {
        require(address(erc20Token) != address(0), "erc20Token not set");
        _;
    }

    modifier requireErc20Token() {
        require(address(erc721Token) != address(0), "erc721Token not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setERC20TokenAddress(address erc20TokenAddr) public onlyOwner() {
      erc20Token = IERC20Token(erc20TokenAddr);
    }

    function setErc20TokenAddress(address erc721Addr) public onlyOwner() {
      erc721Token = IERC721(erc721Addr);
    }
      ...
      returns (uint256[] memory)
    {
      ...
      uint256[] memory depositSetOtherContract = erc20Token.depositsOf(account);
    ...
    {
      uint256[] memory tokensInOldContract = erc20Token.depositsOf(account);
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        uint256 lastBlock = arrayContains(tokensInOldContract, tokenId) ? 
          erc20Token._depositBlocks(account, tokenId) 
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused requireERC20Token {
      ...
      uint256[] memory tokensInOldContract = erc20Token.depositsOf(msg.sender);
      ...
      erc20Token.claimRewards(tokensInOldContract);
    ...
    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireErc20Token requireERC20Token {
        require(msg.sender != address(erc20Token), "Invalid address");
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireErc20Token requireERC20Token nonReentrant() {
        claimRewards(tokenIds);
