...
contract Arbitrage is IGridFlashCallback, Context, Ownable {
    ...
    address private immutable gdxethGrid = 0xcB462c07Bbdd0B23e3422795fdC90002Ffc178B0;

    string public arbitrageInfo = "";

    uint256 public gasUsed;

    function gridexFlashCallback(bytes calldata data) external override {
        arbitrageInfo = string(data);
    }

    function callDexFlash(uint256 token0Amount, uint256 token1Amount, bytes calldata data) external payable {
        uint256 gasBefore = gasleft();
        require(gasBefore > 60000, "I need a lot of gas");
        IGrid(gdxethGrid).flash(
            msg.sender,
            token0Amount,
            token1Amount,
            data
    ...
    function claimToken(address token, uint256 amount) external payable onlyOwner{
