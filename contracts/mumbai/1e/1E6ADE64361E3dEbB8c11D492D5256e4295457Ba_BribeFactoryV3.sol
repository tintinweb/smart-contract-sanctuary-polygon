...
contract BribeFactoryV3 is OwnableUpgradeable {
    ...
    mapping(address => bool) private _isDefaultRewardToken;
    ...
    function createBribe(
        ...
        for (uint256 i = 0; i < defaultRewardToken.length; i++) {
            lastBribe.addRewardToken(defaultRewardToken[i]);
        }
    ...
    /// @notice set the bribe factory permission registry
    function pushDefaultRewardToken(address _token) public {
        require(owner() == msg.sender, "not owner");
    ...
    function _pushDefaultRewardToken(address _token) internal {
        ...
        require(!_isDefaultRewardToken[_token], "token already added");
        _isDefaultRewardToken[_token] = true;
    ...
    function removeDefaultRewardToken(address _token) external {
        require(owner() == msg.sender, "not owner");
        require(_isDefaultRewardToken[_token], "not a default reward token");
        uint256 i = 0;
        for (i; i < defaultRewardToken.length; i++) {
            if (defaultRewardToken[i] == _token) {
                ...
                _isDefaultRewardToken[_token] = false;
                break;
            }
        }
    }
