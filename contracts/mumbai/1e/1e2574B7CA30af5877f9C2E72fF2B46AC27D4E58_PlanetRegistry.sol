...
contract PlanetRegistry {
    ...
    event PlanetCreated(bytes32 indexed planetCode, uint256 planetId);
        ...
        onlyPlanetStatus(_code, PlanetStatus.REGISTERED)
    {
        ...
        emit PlanetCreated(_code, planetId);
    }
        ...
        for (uint256 i = 1; i < shareholders.length; i++) {
            if (shareholders[i].shareholder == _shareholder) {
                ...
                if (shareholders[i].depositedAmount > 0) {
                    refundAmount = shareholders[i].depositedAmount;
                    _refund(
                ...
                shareholders.pop();
                emit PlanetShareholderRemoved(_code, _shareholder, refundAmount);
                return;
            }
        }
