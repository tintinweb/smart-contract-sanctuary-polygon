...
interface IHyperswapRouter01 {
    ...
    function addLiquidityMUMBAI(
        address token,
        uint amountTokenDesired,
        uint amountTokenMin,
        uint amountMUMBAIMin,
        address to,
        uint deadline
    ) external payable returns (uint amountToken, uint amountMUMBAI, uint liquidity);
    ...
    function removeLiquidityMUMBAI(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountMUMBAIMin,
        address to,
        uint deadline
    ) external returns (uint amountToken, uint amountMUMBAI);
    ...
    function removeLiquidityMUMBAIWithPermit(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountMUMBAIMin,
    ...
    function swapExactMUMBAIForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
    function swapTokensForExactMUMBAI(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapExactTokensForMUMBAI(uint amountIn, uint amountOutMin, address[] calldata path, address to, uint deadline)
        external
        returns (uint[] memory amounts);
    function swapMUMBAIForExactTokens(uint amountOut, address[] calldata path, address to, uint deadline)
        external
        payable
        returns (uint[] memory amounts);
...
interface IPangolinRouter {
    ...
    function removeLiquidityMUMBAISupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountMUMBAIMin,
        address to,
        uint deadline
    ) external returns (uint amountMUMBAI);
    function removeLiquidityMUMBAIWithPermitSupportingFeeOnTransferTokens(
        address token,
        uint liquidity,
        uint amountTokenMin,
        uint amountMUMBAIMin,
    ...
    function swapExactMUMBAIForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        address[] calldata path,
        address to,
        uint deadline
    ) external payable;
    function swapExactTokensForMUMBAISupportingFeeOnTransferTokens(
        uint amountIn,
        uint amountOutMin,
        address[] calldata path,
        address to,
...
library Address {
     ...
     * Requirements:
     *
     * - the calling contract must have an MUMBAI balance of at least `value`.
...
// helper methods for interacting with ERC20 tokens and sending MUMBAI that do not consistently return true/false
library TransferHelper {
    ...
    function safeTransferMUMBAI(address to, uint value) internal {
        (bool success,) = to.call{value:value}(new bytes(0));
        require(success, 'TransferHelper: MUMBAI_TRANSFER_FAILED');
    }
}
...
contract Zap is Ownable {
    ...
    function zapOut(address _from, uint amount, address routerAddr, address _recipient) external {
        ...
        if (token0 == WNATIVE || token1 == WNATIVE) {
            ...
            uint amtToken;
            uint amtMUMBAI;
            (amtToken, amtMUMBAI) = IPangolinRouter(routerAddr).removeLiquidityMUMBAI(token, amount, 0, 0, address(this), block.timestamp);
            ...
            TransferHelper.safeTransferMUMBAI(_recipient, amtMUMBAI);
        } else {
    ...
    function _swapHalfNativeAndProvide(address token, uint amount, address routerAddress, address recipient) private returns (uint, uint, uint) {
            ...
            if (useNativeRouter[routerAddress]) {
                ...
                return router.addLiquidityMUMBAI{value : amount.sub(swapValue)}(token, tokenAmount, 0, 0, recipient, block.timestamp);
            }
            else {
    ...
    function _swapNativeForToken(address token, uint value, address recipient, address routerAddr) private returns (uint) {
        ...
        uint[] memory amounts = router.swapExactMUMBAIForTokens{value : value}(0, path, recipient, block.timestamp);
    ...
    function _swapTokenForNative(address token, uint amount, address recipient, address routerAddr) private returns (uint) {
        ...
        if (tokenBridgeForRouter[token][routerAddr] != address(0)) {
            ...
            path[0] = token;
            path[1] = tokenBridgeForRouter[token][routerAddr];
            path[2] = router.WMATIC();
        } else {
            path = new address[](2);
            path[0] = token;
            path[1] = router.WMATIC();
        }

        uint[] memory amounts = router.swapExactTokensForMUMBAI(amount, 0, path, recipient, block.timestamp);
        return amounts[amounts.length - 1];
    }
