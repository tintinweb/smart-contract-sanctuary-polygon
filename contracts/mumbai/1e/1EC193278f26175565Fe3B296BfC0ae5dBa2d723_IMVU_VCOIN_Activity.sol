...
contract IMVU_VCOIN_Activity is Ownable {
    ...
    event NFTQuestGrant(address indexed from, string indexed provider, string indexed txId, address to, string amount);
    ...
    function nftQuestGrant(address from, string calldata provider, string calldata txId, address to, string calldata amount) external onlyOwner {
        emit NFTQuestGrant(from, provider, txId, to, amount);
    }
    ...
    function addUrl(string calldata provider, string calldata url) external onlyOwner {
    ...
    function getUrl(string calldata provider, string calldata txId) public view returns(string memory url){
        return string(abi.encodePacked(providerBaseUrls[provider], txId));
    }
