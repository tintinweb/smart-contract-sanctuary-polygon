...
contract PolygonVoucher is ERC721Enumerable, Ownable {
    ...
    mapping(uint=>EggVoucher) public VoucherList;
    ...
    constructor() ERC721("Oneverse Egg Vouchers", "OV-EGG"){ }

    function enablePremint() external onlyOwner {
        premintEnabled = true;
    }

    function premint(uint256[] calldata _tokenID,bool[] calldata _paid) external {
        require(msg.sender == owner() || approvedAddress[msg.sender],"Not owner or approved");
        for (uint256 i = 0; i < _tokenID.length; i++) {
            VoucherList[_tokenID[i]].premintPaid = _paid[i];
        }
    }
    ...
    function mintToUser(address[] calldata to,uint[] memory tokenIds,uint[] memory redeemable) external {
        require(tokenIds.length == redeemable.length,"Length mismatch");
        require(msg.sender == owner() || approvedAddress[msg.sender],"Not owner or approved");
        uint length = tokenIds.length;
        for(uint i=0;i<length;i++){
            _safeMint(to[i], tokenIds[i]);
            VoucherList[tokenIds[i]] = EggVoucher(redeemable[i],block.timestamp,false);
        }
    }

    function approveAddress(address _toApprove,bool _approve) external onlyOwner{
        approvedAddress[_toApprove] = _approve;
    }
