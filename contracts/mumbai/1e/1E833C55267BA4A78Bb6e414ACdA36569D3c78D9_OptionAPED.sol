...
contract OptionAPED is Ownable {
    ...
    event BetPlaced(uint256 id, address user, uint256 amount, BidType outcome);
    ...
    function placeBid(uint256 _betAmount, BidType _selectedBid) external {
        ...
        emit BetPlaced(optionsCounter, msg.sender, _betAmount, _selectedBid);
    }
    ...
    function claim(uint256 id) public {
        ...
        uint256 totalAmount = getWonAmount(msg.sender, id) + betAmount;
        bidTokenTransfer(address(this), msg.sender, getWonAmount(msg.sender, id) + betAmount);
        emit FundsWithdrawn(id, msg.sender, totalAmount);
    }
