...
interface Episodes {
    function balanceOf(address _owner, uint256 _id) external view returns (uint256);
    function numTokenIds() external view returns (uint256[] memory);
    function burnedTokens(address account) external view returns (uint256[] memory);

}

contract FinalChapter is Multicall, Permissions, ERC1155Drop {

    Episodes public episodes;
    uint256 public _minBurnedTokens;
    ...
    function setAllowedTokens(uint256[][] memory tokensList) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function MinBurnedTokens() private view returns (bool) {
        ...
        uint256[] memory burnedTokens = episodes.burnedTokens(msg.sender);
   ...
   function mintFinal(uint256 tokenId) external {
        require(episodes.balanceOf(msg.sender, tokenId) >= 1, "Must own at least 1 burned token in Episodes");
        require(MinBurnedTokens(), "You need to validate the 99 Episodes to claim the FinalChapter");
