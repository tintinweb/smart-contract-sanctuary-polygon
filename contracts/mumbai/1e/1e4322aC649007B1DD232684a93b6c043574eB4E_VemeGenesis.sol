...
contract VemeGenesis is ERC721, Ownable {
    ...
    modifier onlyHoldersOrOwner() {
        require(
            (isDao && balanceOf(_msgSender()) > 0) || _msgSender() == owner()
        );
        _;
    }
    ...
    function preMint(uint256 amount) public onlyContractOrOwner {
        ...
        returns (bool success)
    {
        ...
        require(
            block.timestamp <= votings[_index].timestamp + votingDuration,
            "t"
        );
    ...
    function activateVoting(uint256 _index) external {
        ...
        (bool b, bytes memory result) = _contractToCall.call{value: _value}(
            _data
        );
