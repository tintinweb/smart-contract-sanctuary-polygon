...
contract Crafting is Initializable, CraftingSettings {
    ...
    onlyEOA
    {
        require(_startCraftingParams.length > 0, "Zero crafting params");
    ...
    returns(uint256, bool)
    {
        ...
        require(block.timestamp >= _craftingRecipe.recipeStartTime, "Recipe has not started");
        require(_craftingRecipe.recipeStopTime == 0
            || _craftingRecipe.recipeStopTime > block.timestamp,
            "Recipe has stopped");
        ...
        emit CraftingStarted(
            ...
            _craftingId,
            _userCrafting.timeOfCompletion,
            _craftingParams.recipeId,
            _userCrafting.randomRequestKey,
            _userCrafting.suppliedInputs);
    ...
    returns(uint64, uint256, uint256)
    {
        ...
        for(uint256 i = 0; i < _craftingRecipe.inputs.length; i++) {
            ...
            if(_startCraftingItemInfo.collection == address(0) && !_recipeInput.isRequired) {
                ...
                require(_inputOption.itemInfo.amount > 0, "Collection/ItemId is not an option");
                require(_inputOption.itemInfo.amount == _startCraftingItemInfo.amount, "Item amounts do not match");
                require(_inputOption.itemInfo.itemId == _startCraftingItemInfo.itemId, "Item ids do not match");
                require(_inputOption.itemInfo.collection == _startCraftingItemInfo.collection, "Item collections do not match");
    ...
    function endCrafting(
        uint256[] calldata _craftingIds)
    external
    whenNotPaused
    contractsAreSet
    onlyEOA
    {
        require(_craftingIds.length > 0, "No ids");
        for(uint256 i = 0; i < _craftingIds.length; i++) {
            _endCrafting(_craftingIds[i]);
        }
    }
...
abstract contract CraftingSettings is Initializable, CraftingContracts {
    ...
    {
        require(_craftingRecipe.recipeStartTime > 0, "No start time for recipe");
        require(_craftingRecipe.recipeStopTime == 0
            || _craftingRecipe.recipeStopTime > _craftingRecipe.recipeStartTime,
            "Stop time must be 0 or greater than start time");
        ...
        // Input validation.
        require(_craftingRecipe.inputs.length > 0, "Recipe requires inputs");
        for(uint256 i = 0; i < _craftingRecipe.inputs.length; i++) {
            ...
            for(uint256 j = 0; j < _input.inputOptions.length; j++) {
                ...
                require(_inputOption.itemInfo.collection == address(etherOrcsItems)
                    ...
                    "Only EOI and DCI are supported as inputs");

                require(_inputOption.itemInfo.amount > 0, "Option amount must be greater than 0");
...
abstract contract CraftingState is ICrafting, ERC1155HolderUpgradeable, UtilitiesUpgradeable {
