...
contract Resources_3 is Initializable, IResourceLogic {
    IBaseLogic public baseLogic;
    using Resources for Resources.ID;

    modifier onlyGameContract() {
        require(msg.sender == baseLogic.gameSupport(), "n");
        _;
    }
    ...
    {
        if (resourceId == uint16(Resources.ID.E3)) {
            e3(id, position);
        } else if (resourceId == uint16(Resources.ID.E5)) {
            destroyPos = e5(id, position);
        } else if (resourceId == uint16(Resources.ID.F1)) {
            f1(id, position);
        } else if (resourceId == uint16(Resources.ID.F4)) {
            destroyPos = f4(id, position);
        } else if (resourceId == uint16(Resources.ID.G2)) {
            g2(id, position);
        } else if (resourceId == uint16(Resources.ID.G4)) {
            destroyPos = g4(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.G5)) {
            g5(id, position);
        } else if (resourceId == uint16(Resources.ID.G6)) {
            destroyPos = g6(id, position);
        } else if (resourceId == uint16(Resources.ID.G7)) {
            destroyPos = g7(id, position);
        } else if (resourceId == uint16(Resources.ID.G8)) {
            g8(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.H1)) {
            h1(id, position);
        } else if (resourceId == uint16(Resources.ID.H2)) {
            destroyPos = h2(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.H3)) {
            h3(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.H6)) {
            h6(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.H7)) {
            h7(id, position);
        } else if (resourceId == uint16(Resources.ID.H8)) {
            h8(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.H11)) {
            destroyPos = h11(id, position);
        } else if (resourceId == uint16(Resources.ID.H12)) {
            destroyPos = h12(id, position);
        } else if (resourceId == uint16(Resources.ID.I1)) {
            destroyPos = i1(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.J2)) {
            j2(id, position);
        }
    }

    function e3(uint256 id, uint16 position) internal {
        uint256 count = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.E3));
        if (count > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            ).findPosition(id, uint256(Resources.ID.E3));
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                listPos
    ...
    function f1(uint256 id, uint16 position) internal {
        ...
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.V2 ||
                adjResource == Resources.ID.B1 ||
                adjResource == Resources.ID.C4 ||
                adjResource == Resources.ID.C7 ||
                adjResource == Resources.ID.C8 ||
                adjResource == Resources.ID.C12 ||
                adjResource == Resources.ID.S4 ||
                adjResource == Resources.ID.E1 ||
                adjResource == Resources.ID.F3 ||
                adjResource == Resources.ID.M11 ||
                adjResource == Resources.ID.C9 ||
                adjResource == Resources.ID.O2 ||
                adjResource == Resources.ID.P1 ||
                adjResource == Resources.ID.S11 ||
                adjResource == Resources.ID.G5 ||
                adjResource == Resources.ID.C16 ||
                adjResource == Resources.ID.A3 ||
                adjResource == Resources.ID.W1
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
            ...
            } else if (adjResource == Resources.ID.P2) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
                    2,
    ...
    function f4(
        ...
        if (
            (countSpin == 5 &&
                baseLogic.isItemExist(id, uint256(Items.ID.IT4), false)) ||
            countSpin == 0
        ) {
            destroyPos = new uint16[](1);
            destroyPos[0] = position;
        ...
        // TODO check destroy if belong to fossilike then resuce 5 roll
    }
    ...
    function g4(
        ...
        return
            IResource_0(baseLogic.resourceLogic(0)).arrow(
                id,
                position,
                3,
                random
    ...
    function g5(uint256 id, uint16 position) internal {
    ...
    function g6(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (adjResource == Resources.ID.B15) {
                destroyPos[countDestroy] = adjPosition;
                ++countDestroy;
                IEventContract(baseLogic.eventContract()).emitEffect(
                    id,
    ...
    function g8(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC12), false) &&
                (percent > position - 2 && percent < position + 1)) ||
            percent == position
        ) {
    ...
    function h1(uint256 id, uint16 position) internal {
        IResource_0(baseLogic.resourceLogic(0)).suits(id, position, 2);
    }
    ...
    function h2(
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IH2), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
                0,
            ...
            if (percent < 30) {
                ...
                uint256 randPosition = ((random / 100 ** position) % 100000) %
                    adjacentPosition.length;
                uint16[] memory destroyPos = new uint16[](1);
                destroyPos[0] = adjacentPosition[randPosition];
                IEventContract(baseLogic.eventContract()).emitEffect(
                    id,
                    1,
                    position,
                    adjacentPosition[randPosition],
                    0
                );
                return destroyPos;
            }
    ...
    function h3(uint256 id, uint16 position, uint256 random) internal {
        if (baseLogic.isItemExist(id, uint256(Items.ID.IH2), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            if (percent < 30) {
                ...
                uint256 randPosition = ((random / 10 ** position) % 100000) %
                    adjacentPosition.length;
    ...
    function h6(uint256 id, uint16 position, uint256 random) internal {
        if (baseLogic.isItemExist(id, uint256(Items.ID.IH2), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
    ...
    function h7(uint256 id, uint16 position) internal {
        if (baseLogic.isItemExist(id, uint256(Items.ID.IH2), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            // TODO hex of tedium update chance of roll better symbol
            IGameController(baseLogic.gameController()).updateRarity(
                id,
                0,
                -20
            );
    ...
    function h8(uint256 id, uint16 position, uint256 random) internal {
        if (baseLogic.isItemExist(id, uint256(Items.ID.IH2), false)) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
    ...
    function h11(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.U1 ||
                adjResource == Resources.ID.B11 ||
                adjResource == Resources.ID.T7
            ) {
                destroyPos[countDestroy] = adjPosition;
                ++countDestroy;
                coin += 6;
    ...
    function h12(
        ...
        IInventory(baseLogic.inventory()).addItem(id, uint256(Items.ID.IP4));
        IEventContract(baseLogic.eventContract()).emitEffect(
            id,
            1,
            position,
    ...
    function i1(
        ...
        // TODO add 1 common item
    }

    function j2(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
