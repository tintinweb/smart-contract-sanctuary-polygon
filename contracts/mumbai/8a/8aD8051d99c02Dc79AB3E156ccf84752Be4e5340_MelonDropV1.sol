...
import { ERC721AUpgradeable, ERC721AStorage } from "../erc721a//ERC721AUpgradeable.sol";
import { ERC721AQueryableUpgradeable } from "../erc721a/ERC721AQueryableUpgradeable.sol";
import { ERC721ABurnableUpgradeable } from "../erc721a/ERC721ABurnableUpgradeable.sol";
...
import { MintRandomnessLib } from "../helpers/MintRandomnessLib.sol";
import { LicenseVersion, CantBeEvilLib} from "../helpers/CantBeEvil.sol";
import { ICantBeEvil } from "../helpers/ICantBeEvil.sol";
...
/**
 * @title MelonDropV1
 * @notice The Melon Drop contract - a creator-owned, modifiable implementation of ERC721A.
 */
contract MelonDropV1 is IMelonDropV1, ERC721AQueryableUpgradeable, ERC721ABurnableUpgradeable, OwnableRoles, ICantBeEvil {
     ...
     * @dev NFT License
     */
    LicenseVersion internal _license;

    /**
     * @dev The upper bound of the max mintable quantity for the drop.
     */
    uint32 public dropMaxMintableUpper;

    /**
     * @dev The lower bound for the maximum tokens that can be minted for this drop.
     */
    uint32 public dropMaxMintableLower;

    /**
     * @dev The timestamp after which `dropMaxMintable` drops from
     *      `dropMaxMintableUpper` to `max(_totalMinted(), dropMaxMintableLower)`.
     */
    uint32 public dropCutoffTime;
    ...
    function initialize(
        ...
        if (dropMaxMintableLower_ > dropMaxMintableUpper_) revert InvalidDropMaxMintableRange();
        ...
        dropMaxMintableUpper = dropMaxMintableUpper_;
        dropMaxMintableLower = dropMaxMintableLower_;
        dropCutoffTime = dropCutoffTime_;
        ...
        emit MelonDropInitialized(
            ...
            fundingRecipient_,
            royaltyBPS_,
            dropMaxMintableLower_,
            dropMaxMintableUpper_,
            dropCutoffTime_,
            version_,
            flags_
        );
     ...
     * @inheritdoc IMelonDropV1
     */
    function setDropMaxMintableRange(uint32 dropMaxMintableLower_, uint32 dropMaxMintableUpper_)
        external
        onlyRolesOrOwner(ADMIN_ROLE)
    {
        ...
        if (currentTotalMinted != 0) {
            dropMaxMintableLower_ = uint32(FixedPointMathLib.max(dropMaxMintableLower_, currentTotalMinted));

            dropMaxMintableUpper_ = uint32(FixedPointMathLib.max(dropMaxMintableUpper_, currentTotalMinted));

            // If the upper bound is larger than the current stored value, revert.
            if (dropMaxMintableUpper_ > dropMaxMintableUpper) revert InvalidDropMaxMintableRange();
        }
        ...
        emit DropMaxMintableRangeSet(dropMaxMintableLower, dropMaxMintableUpper);
    }

    /**
     * @inheritdoc IMelonDropV1
     */
    function setDropCutoffTime(uint32 dropCutoffTime_) external onlyRolesOrOwner(ADMIN_ROLE) {
        if (mintConcluded()) revert MintHasConcluded();

        dropCutoffTime = dropCutoffTime_;

        emit DropCutoffTimeSet(dropCutoffTime_);
    }
     ...
     * @inheritdoc IMelonDropV1
     */
    function dropInfo() external view returns (DropInfo memory info) {
        info.baseURI = baseURI();
        info.contractURI = contractURI();
        info.name = name();
        info.symbol = symbol();
        info.fundingRecipient = fundingRecipient;
        info.dropMaxMintable = dropMaxMintable();
        info.dropMaxMintableUpper = dropMaxMintableUpper;
        info.dropMaxMintableLower = dropMaxMintableLower;
        info.dropCutoffTime = dropCutoffTime;
     ...
     * @inheritdoc IMelonDropV1
     */
    function dropMaxMintable() public view returns (uint32) {
        if (block.timestamp < dropCutoffTime) {
            return dropMaxMintableUpper;
        } else {
            return uint32(FixedPointMathLib.max(dropMaxMintableLower, _totalMinted()));
        }
    }
    ...
    function mintConcluded() public view returns (bool) {
        return _totalMinted() == dropMaxMintable();
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(IMelonDropV1, ERC721AUpgradeable, IERC721AUpgradeable)
        returns (bool)
    {
        return
            interfaceId == type(ICantBeEvil).interfaceId ||
            interfaceId == type(IMelonDropV1).interfaceId ||
    ...
    modifier requireMintable(uint256 totalQuantity) {
        unchecked {
            ...
            uint256 currentDropMaxMintable = dropMaxMintable();
            ...
            if (currentTotalMinted + totalQuantity > currentDropMaxMintable) {
                // Won't underflow.
                //
                // `currentTotalMinted`, which is `_totalMinted()`,
                // will return either `dropMaxMintableUpper`
                // or `max(dropMaxMintableLower, _totalMinted())`.
                //
                // We have the following invariants:
                // - `dropMaxMintableUpper >= _totalMinted()`
                // - `max(dropMaxMintableLower, _totalMinted()) >= _totalMinted()`
                uint256 available = currentDropMaxMintable - currentTotalMinted;
                revert ExceedsDropAvailableSupply(uint32(available));
            }
        }
        _;
    }
     ...
     */
    modifier updatesMintRandomness(uint256 quantity) {
    ...
    function getLicenseURI() public view returns (string memory) {
        return CantBeEvilLib.getLicenseURI(_license);
    }

    function getLicenseName() public view returns (string memory) {
        return CantBeEvilLib.getLicenseName(_license);
    }
        ...
        assembly {
            ...
            if mul(maxMintable, quantity) {
                ...
                for {} 1 {} {
                    ...
                    // is implemented, the `prevrandao()` will be determined by the beacon chain.
