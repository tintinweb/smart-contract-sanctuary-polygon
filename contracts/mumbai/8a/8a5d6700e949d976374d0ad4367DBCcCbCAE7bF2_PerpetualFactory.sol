    ...
    IPerpetualFactory
{
     ...
     * @param   _baseQuoteS2            Base/Quote currency of S2 in bytes4 array
     * @param   _baseQuoteS3            Base/Quote currency of S3 in bytes4 array
    ...
    ) external virtual override onlyMaintainer {
        _createPerpetual(
            _iPoolId,
            _baseQuoteS2,
            _baseQuoteS3,
            ...
            _defaultFundRiskParams,
            _eCollateralCurrency
        );
    }
    ...
    function setPerpetualOracles(
        ...
        _validateOracles(_baseQuoteS2, _baseQuoteS3, uint256(perpetual.eCollateralCurrency));
        _setOracles(perpetual, _baseQuoteS2, _baseQuoteS3);
        emit SetOracles(_iPerpetualId, _baseQuoteS2, _baseQuoteS3);
    }

    function setPerpetualBaseParams(uint24 _iPerpetualId, int128[7] calldata _baseParams)
        external
        override
        onlyMaintainer
    {
    ...
    function setPerpetualRiskParams(
        ...
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        _validateUnderlyingRiskParams(
            _underlyingRiskParams,
            uint256(perpetual.eCollateralCurrency)
        );
        _validateDefaultFundRiskParams(
            _defaultFundRiskParams,
            uint256(perpetual.eCollateralCurrency)
        );
        _setUnderlyingRiskParams(perpetual, _underlyingRiskParams);
        _setDefaultFundRiskParams(perpetual, _defaultFundRiskParams);
        emit SetPerpetualRiskParameters(
            _iPerpetualId,
            _underlyingRiskParams,
            _defaultFundRiskParams
        );
    ...
    function _createPerpetual(
        ...
        require(_iPoolId != 0, "pool id > 0");
        require(_iPoolId <= iPoolCount, "liquidity pool not found");
        require(
            _eCollateralCurrency <= uint256(AMMPerpLogic.CollateralCurrency.QUANTO),
            "invalid eCollateralCurrency"
        );
        LiquidityPoolData storage pool = liquidityPools[_iPoolId];

        _validateOracles(_baseQuoteS2, _baseQuoteS3, _eCollateralCurrency);
        _validateBaseParams(_baseParams);
        ...
        //id, base data
        uint24 iPerpetualIndex = pool.iPerpetualCount++;
        ...
        perpetual.fSettlementS2PriceData = ONE_64x64;
        perpetual.fSettlementS3PriceData = ONE_64x64;
    ...
    function _validateOracles(
        bytes4[2] calldata _baseQuoteS2,
        bytes4[2] calldata _baseQuoteS3,
        uint256 _eCollateralCurrency
    ) internal view {
        require(_baseQuoteS2[0] != bytes4(0), "invalid oracleS2Addr");
        require(_baseQuoteS2[1] != bytes4(0), "invalid oracleS2Addr");
        require(
            (_baseQuoteS3[0] != bytes4(0) && _baseQuoteS3[1] != bytes4(0)) ||
                _eCollateralCurrency != uint256(AMMPerpLogic.CollateralCurrency.QUANTO),
            "invalid oracleS3Addr"
        );
        // ensure we have an oracle
        require(oracleFactoryAddress != address(0x0), "no oracle factory");
        OracleFactory factory = OracleFactory(oracleFactoryAddress);
        require(factory.existsRoute(_baseQuoteS2[0], _baseQuoteS2[1]), "no route S2");
        require(
            _baseQuoteS3[0] == bytes4(0) || factory.existsRoute(_baseQuoteS3[0], _baseQuoteS3[1]),
            "no route S3"
        );
    }

    function _validateBaseParams(int128[7] calldata _baseParams) internal pure {
        require(_baseParams[0] < ONE_64x64, "invalid fInitialMarginRate");
        require(
            _baseParams[1] > 0 && _baseParams[1] < _baseParams[0],
            "invalid fMaintenanceMarginRate"
        );
        require(_baseParams[2] >= 0, "invalid fReferralRebateCC");
        require(_baseParams[3] > 0 && _baseParams[3] < 10000, "invalid liquidation penalty");
        require(_baseParams[4] >= 0 && _baseParams[4] < ONE_64x64 >> 3, "invalid fMinimalSpread");
        require(_baseParams[5] > 0, "invalid fLotSizeBC");
        require(
            _baseParams[6] >= 0 && _baseParams[6] < ONE_64x64 >> 3,
            "invalid fIncentiveSpread"
        );
    }

    function _validateUnderlyingRiskParams(
        int128[5] calldata _underlyingRiskParams,
        uint256 _eCollateralCurrency
    ) internal pure {
        ...
        require(
            _underlyingRiskParams[0] > 0 && _underlyingRiskParams[0] < ONE_64x64 >> 5,
            "invalid fFundingRateClamp"
        );
        require(
            _underlyingRiskParams[1] > 0 && _underlyingRiskParams[1] < ONE_64x64,
            "invalid fMarkPriceEMALambda"
        );
        require(
            _underlyingRiskParams[2] > 0 && _underlyingRiskParams[2] < ONE_64x64,
            "invalid fSigma2"
        );
        require(
            !isQuanto || (_underlyingRiskParams[3] > 0 && _underlyingRiskParams[3] < ONE_64x64),
            "invalid fSigma3"
        );
        require(
            !isQuanto ||
                (_underlyingRiskParams[4] > ONE_64x64.neg() &&
                    _underlyingRiskParams[4] < ONE_64x64),
            "invalid fRho23"
        );
    }

    function _validateDefaultFundRiskParams(
        int128[13] calldata _defaultFundRiskParams,
        uint256 _eCollateralCurrency
    ) internal pure {
        bool isQuanto = _eCollateralCurrency == uint256(AMMPerpLogic.CollateralCurrency.QUANTO);
        require(
            _defaultFundRiskParams[0] > (ONE_64x64).neg() && _defaultFundRiskParams[0] < 0,
            "invalid fStressReturnS2[0]"
        );
        require(
            _defaultFundRiskParams[1] > 0 && _defaultFundRiskParams[1] < ONE_64x64,
            "invalid fStressReturnS2[1]"
        );
        require(
            !isQuanto ||
                (_defaultFundRiskParams[2] > (ONE_64x64).neg() && _defaultFundRiskParams[2] < 0),
            "invalid fStressReturnS3[0]"
        );
        require(
            !isQuanto ||
                (_defaultFundRiskParams[3] > (ONE_64x64).neg() &&
                    _defaultFundRiskParams[3] < ONE_64x64),
            "invalid fStressReturnS3[1]"
        );
        require(
            _defaultFundRiskParams[4] > 0 && _defaultFundRiskParams[4] < ONE_64x64,
            "invalid fDFCoverNRate"
        );
        require(
            _defaultFundRiskParams[5] > 0 && _defaultFundRiskParams[5] < ONE_64x64,
            "invalid fDFLambda[0]"
        );
        require(
            _defaultFundRiskParams[6] > 0 && _defaultFundRiskParams[6] < ONE_64x64,
            "invalid fDFLambda[1]"
        );
        require(
            _defaultFundRiskParams[7] > (ONE_64x64 << 2).neg() &&
                _defaultFundRiskParams[7] < (ONE_64x64).neg(),
            "invalid fAMMTargetDD"
        );
        require(
            _defaultFundRiskParams[8] > (ONE_64x64 << 2).neg() &&
                _defaultFundRiskParams[8] < (ONE_64x64).neg(),
            "invalid fAMMTargetDD"
        );
        require(
            _defaultFundRiskParams[7] <= _defaultFundRiskParams[8],
            "fAMMTargetDD[0]<fAMMTargetDD[1]"
        );
        require(_defaultFundRiskParams[9] > 0, "minimal AMM size>0");
        require(_defaultFundRiskParams[10] > 0, "minimal trader exposure EMA>0");
        require(_defaultFundRiskParams[11] > 0, "minimal AMM exposure EMA>0");
        require(_defaultFundRiskParams[12] > ONE_64x64, "trade size bump>1");
    }
     ...
     * @param   _baseQuoteS2 Array of the S2 base/quote currencies.
     * @param   _baseQuoteS3 Array of the S3 base/quote currencies.
     */
    function _setOracles(
        ...
        _perpetual.S2BaseCCY = _baseQuoteS2[0];
        _perpetual.S2QuoteCCY = _baseQuoteS2[1];
        _perpetual.S3BaseCCY = _baseQuoteS3[0];
        _perpetual.S3QuoteCCY = _baseQuoteS3[1];
    }

    function _setBaseParams(PerpetualData storage _perpetual, int128[7] calldata _baseParams)
        internal
    {
        _perpetual.fInitialMarginRate = int32(_baseParams[0] >> 35);
        _perpetual.fMaintenanceMarginRate = int32(_baseParams[1] >> 35);
        _perpetual.fReferralRebateCC = _baseParams[2];
        _perpetual.liquidationPenaltyRateTbps = uint16(uint128(_baseParams[3]));
        _perpetual.minimalSpreadTbps = ConverterDec18.ABDKToTbps(_baseParams[4]);
        _perpetual.fLotSizeBC = _baseParams[5];
        _perpetual.incentiveSpreadTbps = ConverterDec18.ABDKToTbps(_baseParams[6]);
    }

    function _setUnderlyingRiskParams(
        PerpetualData storage _perpetual,
        int128[5] calldata _underlyingRiskParams
    ) internal {
        _perpetual.fFundingRateClamp = int32(_underlyingRiskParams[0] >> 35);
        _perpetual.fMarkPriceEMALambda = int32(_underlyingRiskParams[1] >> 35);
        _perpetual.fSigma2 = int32(_underlyingRiskParams[2] >> 35);
        _perpetual.fSigma3 = int32(_underlyingRiskParams[3] >> 35);
        _perpetual.fRho23 = int32(_underlyingRiskParams[4] >> 35);
    }

    function _setDefaultFundRiskParams(
        PerpetualData storage _perpetual,
        int128[13] calldata _defaultFundRiskParams
    ) internal {
        ...
        _perpetual.fDFCoverNRate = int32(_defaultFundRiskParams[4] >> 35);
        ...
        _perpetual.fMaximalTradeSizeBumpUp = int32(_defaultFundRiskParams[12] >> 35);
    }
     ...
     */
    function activatePerpetual(uint24 _perpetualId) external override onlyMaintainer {
     ...
     * @param _perpetualId   Id that points to the perpetual.
     */
    function _activatePerpetual(uint24 _perpetualId) internal {
        ...
        require(
            perpetual.state == PerpetualState.INVALID || perpetual.state == PerpetualState.CLEARED,
        ...
        if (pool.isRunning) {
            ...
            // set fund allocation weights
            _setInitialFundAllocationWeight(perpetual);
        } else {
    ...
    function setPerpetualParam(
        ...
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        if (_isStringEqual(_name, "fInitialMarginRate")) {
            require(_value < ONE_64x64, "invalid fInitialMarginRate");
            perpetual.fInitialMarginRate = int32(_value >> 35);
        } else if (_isStringEqual(_name, "fMaintenanceMarginRate")) {
            require(
                _value > 0 && _value < int128(perpetual.fInitialMarginRate) << 35,
                "invalid fMaintenanceMarginRate"
            );
            perpetual.fMaintenanceMarginRate = int32(_value >> 35);
        ...
        } else if (_isStringEqual(_name, "liquidationPenaltyRateTbps")) {
            require(_value > 0 && _value < 10000, "invalid liquidation penalty");
            perpetual.liquidationPenaltyRateTbps = uint16(uint128(_value));
        } else if (_isStringEqual(_name, "fMinimalSpread")) {
            require(_value >= 0 && _value < ONE_64x64 >> 3, "invalid fMinimalSpread");
            perpetual.minimalSpreadTbps = ConverterDec18.ABDKToTbps(_value);
        } else if (_isStringEqual(_name, "fIncentiveSpread")) {
            require(_value >= 0 && _value < ONE_64x64 >> 3, "invalid fIncentiveSpread");
            perpetual.incentiveSpreadTbps = ConverterDec18.ABDKToTbps(_value);
    ...
    function setPerpetualParamPair(
        ...
        if (_isStringEqual(_name, "fStressReturnS2")) {
            ...
            require(
                _value2 > ONE_64x64.neg() && _value2 < ONE_64x64,
                "invalid fStressReturnS3[1]"
            );
            ...
            require(_value1 <= 0, "fCurrentAMMExposureEMA[0]<=0");
            require(_value2 >= 0, "fCurrentAMMExposureEMA[1]>=0");
            ...
            require(
                _value1 > (ONE_64x64 << 2).neg() && _value1 < (ONE_64x64).neg(),
                "invalid fAMMTargetDD[0]"
            );
            require(
                _value2 > (ONE_64x64 << 2).neg() && _value2 < (ONE_64x64).neg(),
                "invalid fAMMTargetDD[1]"
            );
            require(_value1 < _value2, "fAMMTargetDD[0]<fAMMTargetDD[1]");
    ...
    function updatePriceFeeds(
        ...
        if (numPythOracles == _priceIds.length || numPythOracles == 0) {
            ...
            bool needed;
            {
                ...
                uint256 pythFee = IPyth(pyth).getUpdateFee(updates);
            ...
            }
            {
                ...
                require(onDemandFee <= msg.value, "insufficient fee");
                needed = needed || _submitUpdates(false, onDemandFee, updates, ids, times);
            }
...
contract PerpetualUpdateFunctions is PerpetualBaseFunctions {
    ...
    function _accumulateFundingInPerp(PerpetualData storage _perpetual) internal {
        if (uint32(block.timestamp) <= _perpetual.iLastFundingTime) {
            // already updated
            return;
        }
     ...
     */
    function _getMarkPremiumRateEMA(PerpetualData storage _perpetual)
        internal
        view
        returns (int128)
    {
     ...
     */
    function _getLockedInValue(PerpetualData storage _perpetual, address _traderAddr)
        internal
        view
        returns (int128)
    {
     ...
     */
    function _updateAMMTargetFundSize(PerpetualData storage _perpetual, int128 _fNewTargetFundSize)
        internal
    {
    ...
    function _updateDefaultFundTargetSize(uint24 _iPerpetualId) internal {
        ...
        if (
            ...
            emit UpdateDefaultFundTargetSize(
                liquidityPool.id,
                liquidityPool.fDefaultFundCashCC,
                liquidityPool.fTargetDFSize
            );
        }
    }
     ...
     */
    function _getDefaultFundTargetSize(PerpetualData storage _perpetual)
        internal
        view
        returns (int128)
    {
        int128[2] memory fIndexPrices;
        fIndexPrices[0] = _getSafeOraclePriceS2(_perpetual);
     ...
     */
    function _increaseDefaultFundCash(LiquidityPoolData storage _liquidityPool, int128 _fAmount)
        internal
    {
    ...
    /**
     * @dev Calculate volatility spread
     * @param _perpetual reference to perpetual data
     */
    function _calculateVolatilitySpread(PerpetualData storage _perpetual)
        internal
        view
        returns (int128)
    {
        uint256 numSecSinceJump = block.timestamp - _perpetual.iLastPriceJumpTimestamp;

        return
            _getAMMPerpLogic().volatilitySpread(
                _perpetual.jumpSpreadTbps,
                _perpetual.minimalSpreadTbps,
                numSecSinceJump,
                LAMBDA_SPREAD_CONVERGENCE
            );
    }

    /**
     * @dev Check whether oracle is terminated. If so set emergency state.
     * Otherwise
     * - store settlement prices (if price not zero)
     * - update jumpSpreadTbps
     * @param  _perpetual    reference to perpetual
     * @return isMarketClosed (price is zero)
     */
    function _checkOracleStatus(PerpetualData storage _perpetual)
        internal
        returns (bool isMarketClosed)
    {
        OracleFactory oracleFactory = OracleFactory(oracleFactoryAddress);
        if (oracleFactory.isRouteTerminated(_perpetual.S2BaseCCY, _perpetual.S2QuoteCCY)) {
        ...
        if (fPrice > 0) {
            int128 fCurrSpread = _calculateVolatilitySpread(_perpetual);
            if (fPrice != _perpetual.fSettlementS2PriceData) {
                ...
                if (_perpetual.iLastSettlementPriceUpdateTimestamp + 6 > uint32(block.timestamp)) {
                    int128 fJump = fPrice
                        .div(_perpetual.fSettlementS2PriceData)
                        .sub(ONE_64x64)
                        .abs();
                    if (fJump > fCurrSpread.add(fCurrSpread)) {
                        // we cap the spread increase at 8bps
                        _perpetual.jumpSpreadTbps = fJump > 14757395258967641
                            ? 80
                            : ConverterDec18.ABDKToTbps(fJump);
                        _perpetual.iLastPriceJumpTimestamp = uint32(block.timestamp);
                    }
                }
                _perpetual.fSettlementS2PriceData = fPrice;
            }
            _perpetual.iLastSettlementPriceUpdateTimestamp = uint32(block.timestamp);
        } else {
            isMarketClosed = true;
        }
        if (_perpetual.S3BaseCCY[0] != bytes4(0)) {
            if (oracleFactory.isRouteTerminated(_perpetual.S3BaseCCY, _perpetual.S3QuoteCCY)) {
                _getRebalanceLogic().rebalance(_perpetual.id);
                _setEmergencyState(_perpetual);
            }
    ...
    function _setEmergencyState(PerpetualData storage _perpetual) internal {
        ...
        // fund allocation <- zero
        {
            LiquidityPoolData storage pool = _getLiquidityPoolFromPerpetual(_perpetual.id);
            pool.fFundAllocationNormalizationCC = pool.fFundAllocationNormalizationCC.sub(
                _perpetual.fFundAllocationWeightCC
            );
            _perpetual.fFundAllocationWeightCC = 0;
        }
    ...
    /**
     * @dev Sets the LP fund allocation weight of a given perpetual in its liquidity pool
     * @param _perpetual Perpetual storage reference
     */
    function _setInitialFundAllocationWeight(PerpetualData storage _perpetual) internal {
        require(_perpetual.state == PerpetualState.NORMAL, "perp state should be normal");
        LiquidityPoolData storage pool = _getLiquidityPoolFromPerpetual(_perpetual.id);
        int128 fWeight = _perpetual.fMinimalTraderExposureEMA.mul(
            _getBaseToCollateralConversionMultiplier(_perpetual, false)
        );
        pool.fFundAllocationNormalizationCC = pool
            .fFundAllocationNormalizationCC
            .sub(_perpetual.fFundAllocationWeightCC)
            .add(fWeight);
        _perpetual.fFundAllocationWeightCC = fWeight;

        emit UpdateFundAllocationWeight(
            pool.id,
            _perpetual.id,
            fWeight.div(pool.fFundAllocationNormalizationCC)
        );
    }
     ...
     */
    function _getPriceInfo(uint24 _iPerpetualId)
        internal
        view
        returns (bytes32[] memory, bool[] memory)
    {
        ...
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        // S2 first
        (bytes32[] memory feedIdsS2, bool[] memory isPythS2) = factory.getRouteIds(
            [perpetual.S2BaseCCY, perpetual.S2QuoteCCY]
        );
        ...
        // S3
        (bytes32[] memory feedIdsS3, bool[] memory isPythS3) = factory.getRouteIds(
            [perpetual.S3BaseCCY, perpetual.S3QuoteCCY]
        );
        // count unique feeds
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
    ...
    function _rebalanceToTarget(PerpetualData storage _perpetual) internal {
        ...
        if (_perpetual.iLastDefaultFundTransfer == uint32(0)) {
            // we are about to transfer fund into the AMM pool - reset counter
            _perpetual.iLastDefaultFundTransfer = uint32(block.timestamp);
        }
...
contract PerpetualBaseFunctions is PerpStorage, ILibraryEvents {
        ...
        returns (uint24, uint24)
    {
        ...
        // idx doesn't have to be random, and it could be the same (nothing would happen)
        // slither-disable-next-line weak-prng
