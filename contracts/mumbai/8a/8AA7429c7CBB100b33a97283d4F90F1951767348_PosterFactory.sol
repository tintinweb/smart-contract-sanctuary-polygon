...
contract PosterFactory is ERC1155OnChainMetadata {
    ...
    function initialize(
        ...
        name = name_;
        symbol = symbol_;
        _addValue(_contractMetadata, key_contract_name, abi.encode(name));
        _addValue(
            _contractMetadata,
        ...
        _addValue(
            _contractMetadata,
            key_contract_image,
            abi.encode("ipfs://QmWoaiiNB9NoDfj3q1xhMt6DJSAU8fMsNePuhH8gwbaKND")
        );
        _addValue(
            _contractMetadata,
            key_contract_external_link,
            abi.encode("https://mintsongs.com")
        );
        _addValue(
            _contractMetadata,
    ...
    modifier createPreCheck(uint256 _maxSupply) {
        uint256 price = 500000000000000000 * _maxSupply;
        require(
            msg.value >= price,
            string(
                abi.encodePacked(
    ...
    function createPosterMetadata() internal {
        _setValue(
            1,
            key_token_name,
     ...
     */
    function createPoster(address _royaltyRecipient, uint256 _maxSupply)
        external
        payable
        createPreCheck(_maxSupply)
        returns (uint256)
    {
        tokenId.increment();
