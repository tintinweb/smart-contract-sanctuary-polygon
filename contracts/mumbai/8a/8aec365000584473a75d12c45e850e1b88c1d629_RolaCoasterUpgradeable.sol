...
contract RolaCoasterUpgradeable is ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, BlackListUpgradeable, AdvisorSupplyUpgradeable, TeamSupplyUpgradeable, SaleSupplyUpgradeable, IRolaCoasterUpgradeable{
    ...
    function initialize(
        ...
        __AdvisorSupplyEnabled_init(advisorAddress, block.timestamp);
        __TeamSupplyEnabled_init(teamAddress, block.timestamp);
        __SaleSupplyEnabled_init(treasuryAddress, usdcContractAddress, rolaTokenRatePerUSDC, decimals(), block.timestamp);
    ...
    function mintRolaforPublicSale(uint256 amountUSDC) external override whenNotPaused nonReentrant whenNotBlackListedUser(_msgSender()) {
        // Change here
        uint256 currentEligibleRola = safeMintRolaforPublicSale(amountUSDC, block.timestamp);
    ...
    function mintRolaforPrivateSale(uint256 amountUSDC, uint8 saleId) external override whenNotPaused nonReentrant whenNotBlackListedUser(_msgSender()) onlyPrivateUser(saleId) {
        // Change here
        uint256 currentEligibleRola = safeMintRolaforPrivateSale(amountUSDC, block.timestamp, saleId);
        _mint(_msgSender(), currentEligibleRola);
        emit RolaMinted(_msgSender(), currentEligibleRola);
    }
    ...
    function startPrivateSale(address privateAddress, uint256 rolaRateForSale, uint256 maxRolaAllowed) external override whenNotPaused onlyMaintainer{
        // Change here
        uint8 privateSaleIndex = safeAddPrivateSaleDetails(privateAddress, rolaRateForSale, maxRolaAllowed, block.timestamp);
...
interface IRolaCoasterUpgradeable {
     ...
     */
    function mintRolaforPublicSale(uint256 amountUSDC) external;
     ...
     */
    function mintRolaforPrivateSale(uint256 amountUSDC, uint8 saleId) external;
     ...
     */
    function startPrivateSale(address privateAddress, uint256 rolaRateForSale, uint256 maxRolaAllowed) external;
