...
contract AllowanceTransfer is IAllowanceTransfer, EIP712 {
    ...
    mapping(address => mapping(uint256 => uint256)) public allowanceNonceBitmap;
    ...
    function permit1(
        ...
        _useAllowanceUnorderedNonce(owner, permitSingle.details.nonce);
    ...
    function invalidateAllowanceUnorderedNonces(
        ...
        allowanceNonceBitmap[msg.sender][wordPos] |= mask;

        emit UnorderedAllowanceNonceInvalidation(msg.sender, wordPos, mask);
    }

    /// @notice Checks whether a nonce is taken and sets the bit at the bit position in the bitmap at the word position
    /// @param from The address to use the nonce at
    /// @param nonce The nonce to spend
    function _useAllowanceUnorderedNonce(address from, uint256 nonce) internal {
        (uint256 wordPos, uint256 bitPos) = allowaneBitmapPositions(nonce);
        uint256 bit = 1 << bitPos;
        uint256 flipped = allowanceNonceBitmap[from][wordPos] ^= bit;
    ...
    function allowaneBitmapPositions(
        uint256 nonce
