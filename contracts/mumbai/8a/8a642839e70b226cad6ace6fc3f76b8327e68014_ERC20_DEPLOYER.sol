...
contract ERC20_DEPLOYER is Ownable {
    ...
    function deployERC20(string memory _name, string memory _symbol) external {
        ERC20 newContract = new ERC20(_name, _symbol, 1_000_000 ether, msg.sender);
    ...
    function getPaginatedDeployed(uint256 _offset, uint256 _nbElements, address _valid, uint256 _value) 
        view 
        external 
        returns (
            address[] memory addresses, 
            string[] memory names,
            string[] memory symboles,
            uint256[] memory timestamps, 
            bool[] memory valids
    ) {
        if (_offset >= deployed.length) {
            // _offset too high, return empty array
            return (addresses, names, symboles, timestamps, valids);
        }
        ...
        addresses = new address[](_nbElements);
        names = new string[](_nbElements);
        symboles = new string[](_nbElements);
        timestamps = new uint256[](_nbElements);
        valids = new bool[](_nbElements);

        // fetch data and assign values
        for (uint256 i = 0; i < _nbElements; i++) {
            addresses[i] = deployed[i + _offset].adr;
            names[i] = IERC20(addresses[i]).name();
            symboles[i] = IERC20(addresses[i]).symbol();
            timestamps[i] = deployed[i + _offset].timestamp;
            valids[i] = IERC20(addresses[i]).balanceOf(_valid) >= _value;
        }
    }
}