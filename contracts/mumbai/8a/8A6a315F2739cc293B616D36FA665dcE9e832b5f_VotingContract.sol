...
contract VotingContract is Context, Ownable {
    ...
    constructor(address _membershipToken, uint8 _voteBalance) Ownable() {
        require(
    ...
    function castVote(
        ...
        require(
            ...
            "Question Voting period not active"
        );
        require(
        ...
        hasVoted[keccak256(abi.encodePacked(_questionHash, _msgSender()))] = true;
    ...
    function getOptionIndex(
        ...
        if(i < question.optionCount) return i;
    ...
    function getQuestionResult(
        ...
        require(
            q.questionHash != bytes32(0),
            "Question does not exist."
        );
        require(
            !_isQuestionActive(_questionHash),
            "Voting period for the question is still active."
        );
    ...
    function calculateQuestionResult(bytes32 _questionHash) external onlyOwner {
        ...
        Question storage question = questions[_questionHash];
        questionResults[_questionHash] = new uint[](question.optionCount);
        for (uint8 i = 0; i < question.optionCount; i++) {
            questionResults[_questionHash][i] = 0;
        }

        for (uint8 i = 0; i < question.optionCount; i++) {
            questionResults[_questionHash][i] += question.votes[i].length;
        }
