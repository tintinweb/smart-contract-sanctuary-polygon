...
library LibDiamond {
    ...
    event OwnershipTransferred(bool indexed state, address indexed newOwner);

    function flipContractOwnerStatus(address _newOwner) internal {
        DiamondStorage storage ds = diamondStorage();
        bool previousState = ds.contractOwner[_newOwner];
        ds.contractOwner[_newOwner] = !ds.contractOwner[_newOwner];
        emit OwnershipTransferred(previousState, _newOwner);
    }

    function contractOwner(address addr) internal view returns (bool contractOwner_) {
        contractOwner_ = diamondStorage().contractOwner[addr];
    }

    function enforceIsContractOwner() internal view {
        require(diamondStorage().contractOwner[msg.sender], "LibDiamond: Must be contract owner");
    }
...
library Constants {
	string public constant NAME = "tern.crypto Plus | Pass";
