...
contract drop3 is ERC1155, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
    string public name = "drop3";
    string public symbol = "DROP3";
    ...
    uint8 startingTokenId = 0;
    uint8 redeemedTokenId = 1;
    ...
    function airdrop(address[] calldata _addresses) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            _mint(_addresses[i], startingTokenId, 1, "");
            totalSupply++;
        }
    }
        ...
        nonReentrant
    {
       ...
       _mint(to, startingTokenId, quantity, "");
    ...
    function publicMint(uint8 quantity, address to)
        ...
        nonReentrant
    {
        require(_publicActive, "Mint is not active");
        require(quantity > 0, "Must mint more than 0 tokens");
        require(msg.value == (PRICE * quantity), "Incorrect ETH amount");
        require(totalSupply + quantity <= MAX_SUPPLY, "Exceeds max supply of Tokens");
    ...
    // Presale
    function mintPreSaleTokens(uint8 quantity, bytes32[] calldata _merkleProof, address to)
        external
        payable
        callerIsUser
        nonReentrant
    ...
    function redeem() external nonReentrant {
        ...
        // burn 1, mint 1
        _burn(msg.sender, startingTokenId, 1);
