...
interface IDAOYieldVault {
    ...
    function withdrawamount (address member, uint256 amount, uint256 threshold) external; 
...
contract ElectionDAO is ERC721, ERC721URIStorage, KeeperCompatibleInterface {
    ...
    uint public mintPrice = 0.02 ether; //Minimum stake price
   ...
   // mapping (address => uint) public lastCandidate;
    ...
    uint [] public s_tiecandidatetokenid;
    ...
    //events
    event Mint(address indexed _from, uint _id); //change this to daocardattributes
 
    event CandidateCreated(address indexed _from, string _description, uint _id);
 
    event CandidateElected(bool _passed, address indexed _to, uint _id);
    ...
    event TokenWeight(uint256 _ID, address indexed  _OWNER, uint256 _WEIGHT);
    ...
    function withdraw(uint256 _tokenId) external
    {
        ...
        require(dAOCardAtrributes[_tokenId].term == 120, "mods cant cancel" ); //this might be a modifier or bool
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    //THESE ARE MY ADD MORE AND WITHDRAW FROM EACH TOKENID FUNCTIONS - THERE IS NOT ENOUGH SPACE RIGHT NOW

    //we may have to make this an NTT in before transfer not in what I have it right now or we could do it in both of them
    //function withdrawAmount(uint256 _tokenId, uint256 amount) external
    //{
        //require(ownerOf(_tokenId) == msg.sender, "not owner");
        //require(dAOCardAtrributes[_tokenId].term == 120, "mods cant withdraw" ); //this might be a modifier or bool
   
            //daoYieldVault.withdrawamount(msg.sender, amount, mintPrice);
            //dAOCardAtrributes[_tokenId].tokenStakeAmount -= amount;
    //}

    //deposit more value in token 
    //function depositMore(uint256 _tokenId) external payable
    //{
        //require(ownerOf(_tokenId) == msg.sender, "not owner");
   
            //daoYieldVault.stake(msg.sender);
            //dAOCardAtrributes[_tokenId].tokenStakeAmount += msg.value;      
    //}
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function stake() external payable {
        ...
        dAOCardAtrributes[tokenId] = DAOAttributes ({
                ...
                tokenStakeAmount: stakedAmount, //this may be uneeded for the dao card again to show this we could have an event like MINT or staked
    ...
    //we will get the amount when we refactor using the dao yield contract 
    function sendFundsToUser(uint256 _amount, uint256 _tokenID) internal {
        ...
        //if balance of vault has money send the agreed upon value else do a percentage of the dao vault 
        //TESTING
    ...
    function enterCandidate (string memory _description,  uint256 _tokenID) public {
        ...
        require(ownerOf(_tokenID) == msg.sender, "Not Owner Of Entrance Token");
         require(dAOCardAtrributes[_tokenID].term == 120, "mods cant transfer" );
    ...
    function getLosers(uint256 _winningCandidate) internal {
          ...
          //for now we will just set this to eqaul to
          //we add another list here that else if its == we push that ID to tie breakers in perform upkeep we select random
          for (uint i = 0; i <= s_candidateid.length; i++) {
                ...
                //tie mechancis 
                //else if (candidates[i].yesVotes == candidates[_winningCandidate].yesVotes){ 
                    //will use this to select random winning token ID when contract is more balanced
                    //s_tiecandidatetokenid.push(candidates[i].tokenId);
                //}
            } 
    }
 
    //In this function we could just set the mod in the DAOYieldVault From Here And Make Our Life Slightley Easier - Calculate And Set Winner
    function calculateWinner() public returns (uint256) {
        s_electionState = Election.OPEN;

        uint256 winningCandidate = getHighestVote();
        ...
        //this should fix our problem because the candidate ids were begin miscounted rather then starting from 1 i mean we could always go back but we need to keep track of the ids
        delete totalCandidates;
        return winnerId;
    ...
    function voteOnCandidate(bool _yes, uint _candidateId, uint tokenID) public {
        ...
        //set the yes no votes plus this instead of the function this could also be a modifier 
        uint256 tokenWeight = calculateWeight(tokenID);
           ...
           //the token weight is caclulated and captured on each vote in a store online it would be captured on each redeemable except we will change it to rate of earn instead of the points themselves
            emit TokenWeight(tokenID, msg.sender, tokenWeight);
        ...
        )
    {
        ...
        //we need the same checks and structure as the raffle here 
        ...
        bytes calldata /* performData */
            ...
            if((block.timestamp - lastTimeStamp) > intervalWeek){
                ...
                if(modNeeded && s_candidateid.length > 0){
                    ...
                    //Passtime is not working its over 
    ...
    //Might need before tokentransfer in here instead like GUIDL 
     function _transfer(
        address from,
        address to,
        uint256 tokenId
