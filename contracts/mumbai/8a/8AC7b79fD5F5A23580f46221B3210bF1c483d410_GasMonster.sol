...
contract GasMonster is ERC721A, ChainlinkInfrastructureClient, TokenURIResolver, PausableMint, Withdrawable, Royalty {
	using Chainlink for Chainlink.Request;

	// NULL definition that suits for NFT properties
	uint8 private constant NULL = 10;

	uint256 public priceOfNFT = 0.00001 ether;
	...
	// NFT IDs by owner address
	mapping(address => uint256[]) public ownersNFTs;
	...
	function mint(uint256 quantity) external payable whenNotPaused {
		require(msg.value >= priceOfNFT * quantity, 'GasMonster: Not enough ethers to mint NFTs');
		require(quantity <= 10, 'GasMonster: You are trying to mint more than 10 NFTs');
		// require(_totalMinted() + quantity <= maxSupply, 'Tried to mint more than last');

		_safeMint(msg.sender, quantity);
	}

	/// Called after mint or transfer called
	/// Rcords new NFTs or changes NFT owner
	function _afterTokenTransfers(
		...
		// Token mint transfer
		if (from == address(0)) {
			recordMintedTokens(startTokenId, quantity);
			return;
		}

		// User transfered token ot another user/contract
		// Update records about ownership
		for (uint256 i = 0; i < ownersNFTs[from].length; i++) {

			if (ownersNFTs[from][i] != startTokenId) {
				continue;
			}

			ownersNFTs[from][i] = ownersNFTs[from][ownersNFTs[from].length - 1];
			ownersNFTs[from].pop();
		}

		ownersNFTs[to].push(startTokenId);
	}

	function recordMintedTokens(uint256 startTokenId, uint256 quantity) internal {
		...
		for (uint256 i = 0; i < quantity; i++) {
			...
			ownersNFTs[msg.sender].push(currentIndex);
	...
	function triggerUpgrade(uint256 tokenId) external {
		...
		require(
			canRequestUpgrade(tokenId),
			'GasMonster: Upgrade already in process. If request will not complete in 15 minutes, you will be able request upgrade again'
		);

		Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.upgrade.selector);

		request.add('get', string(abi.encodePacked(TokenURIResolver.backendBaseURL, '/canUpgrade/', Strings.toString(tokenId))));
		...
		LastUpgrades[tokenId] = Upgrade(true, block.timestamp);
	}
	 ...
	 */
	function canRequestUpgrade(uint256 tokenId) internal view returns (bool) {
		Upgrade memory lastTokenUpgrade = LastUpgrades[tokenId];
		return !lastTokenUpgrade.inProgress || block.timestamp >= lastTokenUpgrade.timestamp + 15 minutes;
	}
	...
	function upgrade(bytes32 requestId, bool canBeUpgraded) public recordChainlinkFulfillment(requestId) {
		...
		if (token.level == 7) {
			fillInMissingPropsOfToken(tokenId, 12345678910111213);
			emitSuitingEvent(tokenId);
	 ...
	 */
	function requestNFTPropsFullfillmentWithRandom(uint256[] memory tokenIds) internal {
		...
		for (uint8 i = 0; i < tokenIds.length; i++) {
			LastUpgrades[tokenIds[i]] = Upgrade(true, block.timestamp);
		}
	}
	 ...
	 */
	function hasAllParamsAccordingToLevel(uint256 tokenId) internal view returns (bool) {
		NFT storage token = Tokens[tokenId];
		...
		if (token.level == 1) {
			...
			return countFilledRoboPartsOfToken(tokenId) == 3 && token.background >= 5 && token.background <= 9;
	 ...
	 */
	function fillInMissingPropsOfToken(uint256 tokenId, uint256 _randomNumber) internal {
		NFT storage token = Tokens[tokenId];
		...
		if (token.level >= 5) {
			requireNumberToBeMoreThan(_randomNumber, 99999);
		} else if (token.level < 3) {
			requireNumberToBeMoreThan(_randomNumber, 999);
		} else {
			requireNumberToBeMoreThan(_randomNumber, 9);
		}
		...
		if (token.level == 1) {
			// Define gender
			if (block.timestamp < timestampOfSexChangeAtMint) {
				// Male
				token.gender = 1;
			} else {
				// Female
	 ...
	 */
	function countFilledRoboPartsOfToken(uint256 tokenId) internal view returns (uint8 counter) {
		NFT storage token = Tokens[tokenId];
	 ...
	 */
	function fillInRoboPartOfToken(uint256 tokenId, uint256 randomNumber) internal {
		requireNumberToBeMoreThan(randomNumber, 9);
	 ...
	 */
	function emitSuitingEvent(uint256 tokenId) internal {
		NFT storage token = Tokens[tokenId];
	...
	/** Updates price of NFT
	 * @param newPrice price for NFT
	...
	function requireNumberToBeMoreThan(uint256 number, uint256 minValue) internal pure {
	...
	function testChainlinkRequest() external {
		...
		Chainlink.Request memory request = buildChainlinkRequest(jobId, address(this), this.fullfilChainlinkRequest.selector);

		request.add('get', string(abi.encodePacked(TokenURIResolver.backendBaseURL, '/canUpgrade/0')));
		request.add('path', 'data');
...
contract Royalty is IERC2981, ERC165 {
    ...
    function _changeRoyaltyReceiver(address receiver_) internal {
        require(msg.sender == _receiver, "Only current receiver can change royalty address");
        _receiver = receiver_;
    }
}
