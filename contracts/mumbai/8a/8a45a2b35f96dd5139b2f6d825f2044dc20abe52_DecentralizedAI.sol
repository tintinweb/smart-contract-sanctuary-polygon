...
contract DecentralizedAI {
    ...
    event ApiKeyGenerated(uint256 model_id, address indexed user, address indexed owner, uint256 api_key);
    ...
    function requestApiKey(uint256 model_id, address payable owner, uint256 max_tokens, string calldata request_source) public payable returns (uint256) {
        ...
        require(!private_keys[model_id][msg.sender], "API key already generated");
        ...
        emit ApiKeyGenerated(model_id, msg.sender, owner, api_key);
        return api_key;
    }
    ...
    function getLastApiKey(uint256 model_id, address user) public view returns (uint256) {
        require(private_keys[model_id][user], "API key not generated");
        uint256 apiKey = 0;
        for (uint256 i = 1; i < next_model_id; i++) {
            if (private_keys[i][user]) {
                apiKey = generateApiKey(i, user, models[i].owner);
            }
        }
        return apiKey;
    }
