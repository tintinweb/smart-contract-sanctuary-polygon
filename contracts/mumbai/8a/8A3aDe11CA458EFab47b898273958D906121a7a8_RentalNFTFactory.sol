...
contract RentalNFT is ERC721Holder, ERC721URIStorage, SuperAppBaseCFA, Ownable {
    ...
    // bool public immutable Transferable = false;

    // Mapping from owner address to token ID
    mapping(address => uint256) public _ownerToTokenId;
    // Mapping if owner has already minted
    ...
    constructor(
        ...
        string memory originBaseURI
    ) ERC721(name, symbol) SuperAppBaseCFA(host, true, true, true) {
        acceptedToken = _acceptedToken;
        flowRate = _flowRate;
        baseURI = originBaseURI;
    }

    // need to be minted when create flow
    function _mintNft(address to) internal returns (uint256) {
        uint256 newItemId = _tokenIds.current();
        _safeMint(to, newItemId);
        _ownerToTokenId[to] = newItemId;
        _hasNFT[to] = true;
        _tokenIds.increment();
        return newItemId;
    }
    ...
    function _burnNft(address owner) internal {
        uint256 ownerTokenId = _ownerToTokenId[owner];
        _burn(ownerTokenId);
        _hasNFT[owner] = false;
    }
    ...
    function transferFrom(
        ...
        require(false, "non-transferable NFT");
        //solhint-disable-next-line max-line-length
        // require(
    ...
    function onFlowCreated(
        ...
        require(
            decompiledContext.msgSender == address(this),
            "Please create flow by contract function"
        );

        require(!_hasNFT[msg.sender], "already minted");
    ...
    function onFlowUpdated(
        ...
        require(
            decompiledContext.msgSender == address(this),
            "Please update flow by contract function"
        );
    ...
    function onFlowDeleted(
        ...
        // ISuperfluid.Context memory decompiledContext = host.decodeCtx(ctx);
        // console.log("origin sender:", sender);
        // console.log("onFlowDeleted:", decompiledContext.msgSender);
        _burnNft(sender);
        return ctx;
    }

    //user need to set permission for contract first
    function createNftFlow() public {
        require(!_hasNFT[msg.sender], "already minted");
        (
            bool allowCreate,
            bool allowUpdate,
            bool allowDelete,
            int96 flowRateAllowance
        ) = acceptedToken.getFlowPermissions(msg.sender, address(this));

        require(allowCreate == true, "Contract has no permission");
        require(
            flowRateAllowance >= flowRate,
            "Contract has't enough flowRateAllowance"
        );
        acceptedToken.createFlowFrom(msg.sender, address(this), flowRate);
    }

    //user need to set permission for contract first
    // function deleteNftFlow() public {
    //     require(_hasNFT[msg.sender], "You haven't minted yet");
    ...
    //     require(allowDelete == true, "Contract has no permission");
...
contract RentalNFTFactory {
    ...
    uint[] private _allContractIds;

    // id to NFT contract info
    ...
    function generateNewRentalNFT(
        ...
        RentalNFT rentalNft = new RentalNFT(
            acceptedToken,
            host,
            flowRate,
            name,
        ...
        contracts[contractId]._contractAddress = address(rentalNft);
        contracts[contractId]._owner = msg.sender;
        contracts[contractId]._acceptedToken = address(acceptedToken);
        contracts[contractId]._host = address(host);
        contracts[contractId]._flowRate = flowRate;
        contracts[contractId]._name = name;
        contracts[contractId]._symbol = symbol;
        contracts[contractId]._baseURI = baseURI;

        rentalNft.transferOwnership(msg.sender);
        _allContractIds.push(contractId);

        _contractIds.increment();

        return address(rentalNft);
    }

    function getAllContractIds() public view returns (uint[] memory) {
        return _allContractIds;
    }

    function getContractAddress(uint id) public view returns (address) {
        return contracts[id]._contractAddress;
    }

    function getOwner(uint id) public view returns (address) {
        return contracts[id]._owner;
    }

    function getFlowRate(uint id) public view returns (int96) {
        return contracts[id]._flowRate;
    }

    function getAcceptedToken(uint id) public view returns (address) {
        return contracts[id]._acceptedToken;
    }

    function getName(uint id) public view returns (string memory) {
        return contracts[id]._name;
    }

    function getSymbol(uint id) public view returns (string memory) {
        return contracts[id]._symbol;
    }

    function getBaseUri(uint id) public view returns (string memory) {
        return contracts[id]._baseURI;
    }
}