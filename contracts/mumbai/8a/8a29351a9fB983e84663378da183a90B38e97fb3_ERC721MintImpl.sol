...
contract ERC721MintImpl is ERC721MintProxyStorage, Ownable, ERC721Init {
    ...
    function initialize(string memory _name, string memory _symbol, string memory _uri) public {
        require(bytes(uri).length == 0, 'ERC721MintImpl::initialize: may only be initialized once');
        uri = _uri;
