...
contract SwarmDOTCEscrow is ERC1155Holder, IEscrow, AccessControl {
     ...
     *   @param _offerId uint256 the offer ID
     */
    function setNFTDeposit(uint256 _offerId) external override isEscrowFrozen onlyEscrowManager {
        ...
        onlyEscrowManager
    {
        ...
        uint256 _amount = unstandardisedNumber(IdOTC(dOTC).getTakerOrders(orderId).amountToReceive, token);
        ...
        onlyEscrowManager
    {
        ...
        address _nftAddress = IdOTC(dOTC).getNftOffer(_nftOfferId).nftAddress;
        IERC1155(_nftAddress).setApprovalForAll(dOTC, true);
        emit WithdrawNFT(_nftOfferId, _nftOrderId, _receiver);
    }

    function cancelNftDeposit(uint256 nftOfferId) external override isEscrowFrozen onlyEscrowManager {
        address nftAddress = IdOTC(dOTC).getNftOffer(nftOfferId).nftAddress;
        IERC1155(nftAddress).setApprovalForAll(dOTC, true);
