...
contract Minter is IMinter, Ownable {
     ...
     * dev
     */
    function setDevReserve(uint16 reserve) external override onlyOwner {
        require(
            (reserve * 5 <= _getOverall()),
    ...
    function _verify(
        ...
        return (_recover(_hash(salt, _uai, sender), token) == _signer);
    }
    ...
    function _getMaxSupply() internal returns (uint256) {
        return IUAI(_uai).getMaxSupply();
    }

    function _getOverall() internal returns (uint256) {
        return IUAI(_uai).getOverall();
    }
}
...
interface IMinter {
    ...
    function setDevReserve(uint16 reserve) external;
