...
contract KubzWardrobe is ERC721xDummy, DefaultOperatorFiltererUpgradeable {
    ...
    function transferCheck(uint256 _tokenId) internal view {
        try IERC6551Account(ownerOf(_tokenId)).token() returns (
            uint256 chainId,
            address tokenContract,
            uint256 tokenId
        ) {
            require(
                block.timestamp > tokenKWRLockedUntil[tokenContract][tokenId],
                "KWR locked"
            );
        } catch {}
    }
    ...
    function updateTraitLock(
        uint256 collection, // 1 = kzg, 2 = kubz
        uint256[] calldata collectionTokenIds,
        uint256 secondsFromBlockTimestamp
    ) external {
        require(collection == 1 || collection == 2, "unsupported collection");
        require(secondsFromBlockTimestamp < 259200, "locking for too long");

        IERC721 collectionContract = collection == 1
            ? kzgContract
            : kubzContract;
        for (uint256 i = 0; i < collectionTokenIds.length; ) {
            uint256 tokenId = collectionTokenIds[i];
            require(
                collectionContract.ownerOf(tokenId) == msg.sender,
                "Not owner of collection token"
            );
            uint256 cur = tokenKWRLockedUntil[address(collectionContract)][
                tokenId
            ];
            uint256 newt = block.timestamp + secondsFromBlockTimestamp;
            uint256 larger = cur > newt ? cur : newt;
            tokenKWRLockedUntil[address(collectionContract)][tokenId] = larger;
            unchecked {
                i++;
            }
        }
