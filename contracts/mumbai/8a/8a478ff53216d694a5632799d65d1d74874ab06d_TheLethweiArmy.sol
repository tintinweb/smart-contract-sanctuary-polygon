...
// File: contracts/TheLethweiArmy.sol
...
error TheLethweiArmy__InvalidMintAmount();
error TheLethweiArmy__MaxSupplyExceeded();
error TheLethweiArmy__InsufficientFunds();
error TheLethweiArmy__AllowlistSaleClosed();
error TheLethweiArmy__AddressAlreadyClaimed();
error TheLethweiArmy__InvalidProof();
error TheLethweiArmy__PublicSaleClosed();
error TheLethweiArmy__TransferFailed();
error TheLethweiArmy__NonexistentToken();

/// @title The LethweiArmy collection 
/// @notice This contract reduces gas for minting NFTs using the ERC721A standard
/// @dev This contract includes allowlist mint
contract TheLethweiArmy is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 _maxMintAmount) {
        if (_mintAmount < 1 || _mintAmount > _maxMintAmount)
            revert TheLethweiArmy__InvalidMintAmount();

        if (totalSupply() + _mintAmount > iMaxSupply)
            revert TheLethweiArmy__MaxSupplyExceeded();
    ...
    modifier mintPriceCompliance(uint256 _mintAmount) {
        if (msg.value < sMintPrice * _mintAmount)
            revert TheLethweiArmy__InsufficientFunds();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.AllowlistOnly)
            revert TheLethweiArmy__AllowlistSaleClosed();

        if (sAllowlistClaimed[_msgSender()])
            revert TheLethweiArmy__AddressAlreadyClaimed();

        bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));

        if (!MerkleProof.verify(_merkleProof, sMerkleRoot, leaf))
            revert TheLethweiArmy__InvalidProof();
        ...
        mintPriceCompliance(_mintAmount)
    {
        if (sSaleState != SaleState.PublicOpen)
            revert TheLethweiArmy__PublicSaleClosed();
    ...
    function withdraw() public onlyOwner nonReentrant {
        ...
        if (!success) revert TheLethweiArmy__TransferFailed();
    }
    ...
    {
        if (!_exists(_tokenId)) revert TheLethweiArmy__NonexistentToken();
