...
contract Product020802V01 is OwnableV01 {
    ...
    event PolicyEndorsed(string onChainPolicyId, uint256 operationType, uint256 operationEffectiveDate);
    ...
    function endorsePolicy(
        ...
        emit PolicyEndorsed(
            endorsementParams.onChainPolicyId,
            allPolicies[endorsementParams.onChainPolicyId].operationType,
            allPolicies[endorsementParams.onChainPolicyId].operationEffectiveDate
        );
    }
    ...
    function cancelPolicy(
        ...
        require(
            ...
            "date of notice cannot be in the future"
        );
...
library Product020802Lib {
    ...
    function endorsePolicy(
        EndorsementParams memory endorsementParams,
    ...
    function checkEndorsePolicyBusinessRules(
        EndorsementParams memory endorsementParams,
        Policy storage policy
    ) private view {
        ...
        if (endorsementParams.requestedCoverageStartDate != 0) {
            checkIfCoverageIsNotAlreadyActive(policy.coverageStartDate);
            checkIfRequestedCoverageStartDateIsAfterEffectiveDate(
                endorsementParams.requestedCoverageStartDate,
                endorsementParams.operationEffectiveDate
            );
        }
    }
    ...
    function checkIfRequestedCoverageStartDateIsAfterEffectiveDate(
        uint256 requestedCoverageStartDate,
        uint256 operationEffectiveDate
    ) private pure {
        require(
            requestedCoverageStartDate.dailyCompareTo(operationEffectiveDate) >= 0,
            "requestedCoverageStartDate need to be after effectiveDate"
        );
    }

    function checkIfCoverageIsNotAlreadyActive(uint256 coverageStartDate) private view {
    ...
    function checkOperationEffectiveDate(uint256 operationEffectiveDate) private view {
        ...
        require(
            ...
            "operationEffectiveDate must be less than now + 8 weeks"
        );
    }
