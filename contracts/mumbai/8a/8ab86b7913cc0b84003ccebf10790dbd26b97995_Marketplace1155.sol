  ...
  UUPSUpgradeable 
{
  ...
  struct Offers {
    uint256 count;
    mapping(uint256 => ResaleOffer) resaleOffers;
  }

  mapping(uint256 => MarketItem) private idToMarketItem;
  mapping(uint256 => Offers) public idToResaleOffers;
  ...
  event MarketItemSellerChanged (
    uint256 indexed itemId,
    address indexed previousSeller,
    ...
    whenNotPaused
  {
    ...
    emit MarketItemCreated(itemId, nftContract, tokenId, 0, minter, address(0), 0, address(0), 0, ListingStatus.Cancelled);

    Offers storage newOfferList = idToResaleOffers[itemId];
    newOfferList.count += 1;
    uint256 resaleId = newOfferList.count;
    newOfferList.resaleOffers[resaleId] = ResaleOffer({
    ...
    emit NewResaleOffer(itemId, 1, msg.sender, amount, price, ListingStatus.Active);
  }
    ...
    whenNotPaused
  {
    ...
    require(isNFTOwner(marketItem.nftContract, marketItem.seller), "Sorry, it looks like you are not the current owner of the NFT contract. Please cancel your offer and try again.");
  ...
  {
    MarketItem storage marketItem = idToMarketItem[itemId];
    require(marketItem.seller == msg.sender || (isNFTOwner(marketItem.nftContract, msg.sender)), "Only current seller or contract owner can cancel the listing");
    require(marketItem.status != ListingStatus.Blacklisted, "Item is currently blacklisted");
    ...
    IERC1155Upgradeable(marketItem.nftContract).safeTransferFrom(address(this), marketItem.seller, marketItem.tokenId, cancelledAmount, "");

    emit MarketItemCancelled(itemId, marketItem.seller, cancelledAmount, marketItem.status);

    if (marketItem.seller != msg.sender) {
      marketItem.seller = payable(msg.sender);
      emit MarketItemSellerChanged(itemId, marketItem.owner, marketItem.seller, marketItem.status);
    } 
    ...
    whenNotPaused
  {
    ...
    require(getResellerOfferId(itemId, msg.sender) == 0, "Offer previously created on this item"); // getResaleId != 0
    ...
    emit NewResaleOffer(itemId, resaleId, msg.sender, amount, price, ListingStatus.Active);
  }

  //! =============== MODIFY RESALE OFFER OR CANCEL ============================================

  function modifyResaleOffer(
    uint256 itemId,
    uint256 amount,
    uint256 price
  ) 
    ...
    whenNotPaused
  {
    ...
    MarketItem memory marketItem = idToMarketItem[itemId];
    require(marketItem.status != ListingStatus.Blacklisted, "Sorry, this item has been blacklisted");

    uint256 resaleId = getResellerOfferId(itemId, msg.sender);
    require(resaleId != 0, "No offer previously registered");

    ResaleOffer storage offer = idToResaleOffers[itemId].resaleOffers[resaleId];
    require(offer.seller == msg.sender, "Only seller can modify offer");
    ...
    if (amount > 0) {
      ...
      IERC1155Upgradeable(marketItem.nftContract).safeTransferFrom(msg.sender, address(this), marketItem.tokenId, amount, "");
      emit ResaleOfferModified(itemId, resaleId,  amount, offer.price,  msg.sender, offer.status);
    } else {
      offer.amountToSell = 0;
      offer.price = 0;
      offer.status = ListingStatus.Cancelled;
      IERC1155Upgradeable(marketItem.nftContract).safeTransferFrom(address(this), msg.sender, marketItem.tokenId, returnedAmount, "");
      emit ResaleOfferCancelled(itemId, resaleId, offer.seller, offer.amountToSell, offer.status);
    }
    ...
    whenNotPaused
  {
    ...
    emit ResaleItemSale(itemId, resaleId, msg.sender, offer.seller, amount, price, mainFee, marketFee, royaltyFee, royaltyReceiver);
    ...
    if (offer.amountToSell == 0) {
      ...
      emit ResaleItemSold(itemId, resaleId, offer.seller, offer.price, offer.status);
    }
  }

  //! =============== FETCH NFT SINGLE RESALE OFFER ============================================

  function getNFTResaleOffer(uint256 itemId, uint256 resaleId)
    public view
  ...
  //! =============== FETCH NFT ALL RESALE OFFERS ============================================

  function fetchNFTResaleOffers(uint256 itemId)
    public view 
    returns (ResaleOffer[] memory) 
  {
    Offers storage offerList = idToResaleOffers[itemId];

    uint256 activeOffersCount;
    uint256 currentIndex;

    for (uint256 i = 0; i < offerList.count; i++) {
      if (offerList.resaleOffers[i + 1].status == ListingStatus.Active) {
        activeOffersCount++;
      }
    }

    ResaleOffer[] memory activeOffers = new ResaleOffer[](activeOffersCount);

    for (uint256 i = 0; i < offerList.count; i++) {
      if (offerList.resaleOffers[i + 1].status == ListingStatus.Active) {
        ResaleOffer storage currentOffer = offerList.resaleOffers[i + 1];
  ...
  //! =============== FETCH ALL USER RESALE OFFERS ============================================

  function fetchMyResaleOffers()
    public view
    returns (ResaleOffer[] memory)
  {
    uint256 totalItemCount = ITEM_IDS.current();
    ...
    ResaleOffer[] memory myOffers = new ResaleOffer[](userOffersCount);
    uint256 resaleId;
    for (uint256 i = 0; i < totalItemCount; i++) {
      if(getResellerOfferId(i + 1, msg.sender) != 0) {
        resaleId = getResellerOfferId(i + 1, msg.sender);
        myOffers[currentIndex] = idToResaleOffers[i + 1].resaleOffers[resaleId];
        currentIndex++;
      }
    }
    return myOffers;
  ...
  // Check if seller is nft contract owner
  function isNFTOwner(address nftAddress, address seller)
    internal view
    returns (bool)
  {
    return OwnableUpgradeable(nftAddress).owner() == seller;
  }
  ...
  function getResellerOfferId(
    ...
    returns (uint256)
  {
    // ResaleOffer[] storage offerList = idToResaleOffers[itemId];
    Offers storage offerList = idToResaleOffers[itemId];
    uint256 resaleId;

    for (uint256 i = 0; i < offerList.count; i++) {
      if (offerList.resaleOffers[i + 1].seller == sellerAccount) {
        resaleId = offerList.resaleOffers[i + 1].resaleId;
      }
    }
    return resaleId;
  }
    ...
    onlyRole(CENSOR_ROLE)
  {
    ...
    emit MarketItemBlacklisted(itemId, marketItem.nftContract, marketItem.tokenId, marketItem.seller, marketItem.amountToSell, msg.sender, ListingStatus.Blacklisted);
  }
    ...
    onlyRole(CENSOR_ROLE)
  {
    ...
    emit SellerBlacklisted(sellerAccount, msg.sender);
  }
