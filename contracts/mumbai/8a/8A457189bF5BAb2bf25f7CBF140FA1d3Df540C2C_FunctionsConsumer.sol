...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner, ERC721, ERC721URIStorage {
  ...
  function convertStringToAddress(string memory addressString) public pure returns (address) {
    bytes memory strBytes = bytes(addressString);
    require(strBytes.length == 42, "Invalid address string length");

    uint160 convertedAddress = 0;
    uint256 base = 1;
    for (uint8 i = 40; i > 0; i--) {
      uint8 charValue = uint8(strBytes[i + 1]);
      require(
        (charValue >= 48 && charValue <= 57) || (charValue >= 97 && charValue <= 102),
        "Invalid character in the address string"
      );
      ...
      convertedAddress += uint160(charValue * base);
      base *= 16;
    }
