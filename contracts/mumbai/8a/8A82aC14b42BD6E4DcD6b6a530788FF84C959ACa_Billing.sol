...
contract Billing is IBilling, OwnableUpgradeable, UUPSUpgradeable {
        ...
        returns (uint64)
    {
        ...
        emit CreateSystemSubscriptionEvent(
            uint64(contractSubscriptions.length) - 1,
            _sub.frequencyOfDeduction,
            _sub.deductionIN,
            _sub.amount,
            _sub.isActive,
            _sub.code
    ...
    function cancelSystemSubscription(uint64 subID) external virtual override onlyOwner {
        ...
        emit DeactivateSystemSubscriptionEvent(subID, false, contractSubscriptions[subID].code);
    }
    ...
    function subscriptionStatus(address account) public view returns (bool, uint64) {
        ...
        if (
            ...
            return (false, userToSubscription[account].systemDefinedSubscriptionID);
        }
    }
    ...
    function isSubscriptionActive(address account) external {
        (bool status, uint64 id) = subscriptionStatus(account);
        if (!status && id < type(uint64).max) {
            purchaseSubscription(account);
            emit SubscriptionStatusEvent(account, true, id);
        } else {
            emit SubscriptionStatusEvent(account, status, id);
        }
    }
...
interface IBilling {
     ...
     * Note  account must a subscription
     */
    event SubscriptionStatusEvent(address indexed account, bool active, uint64 indexed subscriptionID);


    /**
     * @dev Emitted when a system Subcription/plan is created
     *
     *
     * Note  account must a subscription
     */
    event CreateSystemSubscriptionEvent(
        uint64 planID,
        uint32 frequencyOfDeduction,
        uint32 deductionIN,
        uint128 amount,
        bool isActive,
        bytes7 code
    ...
    /**
     * @dev Emitted when a system Subcription/plan is deactivated
     *
     *
     * Note  account must a subscription
     */
    event DeactivateSystemSubscriptionEvent(uint64 planID, bool isActive, bytes7 code);
