...
contract TrueOwnerGuard is Guard, SignatureDecoder, TrueOwnerGetter {
    /// @dev Mapping to store the true owner for each Safe. The mapping is empty if it has never been called by the Safe.
    mapping(address => address) internal trueVaultOwners;
    ...
    /// @notice it saves trueOwner address into trueVaultOwners mapping to compare it in checkAfterExecution
    function checkTransaction(
        address to,
        uint256 value,
        bytes memory data,
        ...
        bytes32 txHash =
            safe.getTransactionHash(to, value, data, operation, safeTxGas, baseGas, gasPrice, gasToken, refundReceiver, safe.nonce() - 1);

        address trueOwner = getTrueOwner(msg.sender);
        bool signedByTrueOwner = isAddressInSignatures(txHash, signatures, safe.getThreshold(), trueOwner);

        require(signedByTrueOwner, "EDCC00"); // TrueOwnerGuard: must be signed by the true owner
        // set the true owner to check it in the checkAfterExecution
        trueVaultOwners[msg.sender] = trueOwner;
    }

    /// @dev This function is called after the transaction is executed.
    /// @notice with this check enabled it is not allowed to change true owner with Safe tx
    function checkAfterExecution(bytes32, bool) external view override {
        // in final decision now, it's allowed
        // require(trueVaultOwners[msg.sender] == getTrueOwner(msg.sender), "EDCC01"); // TrueOwnerGuard: use recovery to change trueOwner
    }
}
