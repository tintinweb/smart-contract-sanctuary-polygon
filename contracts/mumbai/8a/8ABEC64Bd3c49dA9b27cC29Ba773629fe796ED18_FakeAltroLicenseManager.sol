 ...
 */
contract FakeAltroLicenseManager is Ownable, ERC165, ILicenseManager {
	...
	function setDiscount(address user, uint256 discount) external onlyOwner {
		require(discount > MIN_DISCOUNT && discount <= MAX_DISCOUNT, "FakeAltroLicenseManager: discount not in accepted range");
	...
	/**
	 * @dev This function is responsible for setting the staking service for FakeAltroLicenseManager. The address passed as an argument must not be a null address and must support the IStakingService interface
	 *
	...
	function _setStakingService(address stakingService_, uint256 pid_) internal {
		require(stakingService_ != address(0), "FakeAltroLicenseManager: cannot be null address");
		require(stakingService_.supportsInterface(type(IStakingService).interfaceId), "FakeAltroLicenseManager: does not support IStakingService interface");

		address stakingToken = IStakingService(stakingService_).poolStakingToken(pid_);
		require(stakingToken != address(0), "FakeAltroLicenseManager: pool id not valid");
