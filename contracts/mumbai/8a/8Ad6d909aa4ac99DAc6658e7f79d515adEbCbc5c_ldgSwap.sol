...
contract LDG_Defi is ERC20, ERC20Burnable, Pausable, AccessControl {
...
contract ldgSwap is Ownable, AccessControl {
    ...
    constructor(address _USDAddress, address _CAPSAddress, address _CAPSATokenAddress, address _LDGTokenAddress, address _fundWallet, address _feeWallet, address _feeRoleLDG) {
        require(_USDAddress != address(0x0), "USDC contract address cannot be 0x0.");
        ...
        token = LDG_Defi(_LDGTokenAddress);
    ...
    uint256 public balances_capsa; // total capsa registered
    ...
    function setFeeWallet(address _token) external onlyOwner { // SET FEE ADDRESS
        feeWallet = address(_token);
    }
    ...
    function setFeePeriod(uint256 _feePeriod, uint32 _ratio_time_fee) external onlyOwner { // Set the fee Period
        feePeriod = _feePeriod;
        ratio_time_fee = _ratio_time_fee;
    }

    function setFundWallet(address _token) external onlyOwner { // SET FUND WALLET
        fundWallet = address(_token);
    }
    ...
    function withdraw(uint256 amount) external isTokenSet {
        require(depositPaused == false, "The withdraw is paused.");
    ...
    function takeFee() external isTokenSet { // Function to take the fee
