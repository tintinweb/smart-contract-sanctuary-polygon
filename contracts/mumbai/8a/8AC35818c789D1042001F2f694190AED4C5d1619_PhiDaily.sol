...
interface IEmissionLogic {
    function determineTokenId(uint256 logicVersion) external view returns (uint8);
}
...
/// @title Users claim Daily Objects
contract PhiDaily is AccessControlUpgradeable {
    /* --------------------------------- ****** --------------------------------- */
    ...
    mapping(address sender => uint256 count) public DailyClaimedHistory;
    mapping(address sender => mapping(uint256 tokenid => uint256 count)) public DailyClaimedEachHistory;
    /* --------------------------------- ****** --------------------------------- */
    ...
    event SetDailyObject(address indexed dailyContract);
    ...
    function initialize(
        ...
        _dailyObject = dailyContract;
        _emissionlogic = emissionlogic;
    ...
    /// @dev get EmissionLogic
    function getEmissionLogic() external view returns (address) {
        return _emissionlogic;
    }

    /// @dev set DailyContract
    function setEmissionLogic(address emissionlogic) external onlyOwner {
        require(emissionlogic != address(0), "cant set address(0)");
        _emissionlogic = emissionlogic;
        emit SetEmissionLogic(emissionlogic);
    }

    /// @dev get DailyContract
    function getDailyContract() external view returns (address) {
        return _dailyObject;
    }

    /// @dev set DailyContract
    function setDailyContract(address dailyObject) external onlyOwner {
        require(dailyObject != address(0), "cant set address(0)");
        _dailyObject = dailyObject;
        emit SetDailyObject(dailyObject);
    }
    ...
    /*                                   OBJECT                                   */
    /* -------------------------------------------------------------------------- */
    /// @dev claim daily object
    function claimDailyObject(uint8 box, Coupon memory coupon) external {
        // to prevent bots minting from a contract
        require(msg.sender == tx.origin);

        // Check that the coupon sent was signed by the admin signer
        bytes32 digest = keccak256(abi.encode(box, msg.sender));
        require(isVerifiedCoupon(digest, coupon), "Invalid coupon");

        uint256 tokenid = IEmissionLogic(_emissionlogic).determineTokenId(box);
        ++DailyClaimedHistory[msg.sender];
        ++DailyClaimedEachHistory[msg.sender][tokenid];
        IDailyObject(_dailyObject).getObject(msg.sender, tokenid);
        emit LogClaimObject(msg.sender, tokenid);
    }

    function checkClaimedEachStatus(address sender, uint256 tokenId) external view returns (uint256) {
        return DailyClaimedEachHistory[sender][tokenId];
    }
}