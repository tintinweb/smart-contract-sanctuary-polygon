...
contract TprNetwork is Ownable {
    ...
    function multisendBusd(address payable[]  memory  _contributors, uint256[] memory _balances, uint256 membcode, uint256 rcode, uint64 plan) public payable {
    ...
    function multisendEmpire(address _senderads, uint256 _amttoken, uint256 membcode, uint256 rcode, uint64 plan) public {
       // uint256 total = msg.value;
    ...
    function RewardGeneration(address _senderads, uint256 _amttoken, address mainadmin) public onlyOwner {
       ...
       // require(total >= _amttoken );
       // total = total.sub(_amttoken);
        tokenInterface(token).transferFrom(mainadmin,_senderads,_amttoken);
        //emit Multireceivers(_amttoken,_senderads,membcode,rcode,plan);
       // emit Multisended(_amttoken, msg.sender, membcode, rcode, plan);
    }
