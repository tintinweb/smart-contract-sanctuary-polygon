...
/**
 * StavmeSaMinimalProxyFactory is a reusable contract factory
 * that uses a minimal proxy pattern
 * 
 * The implemntation contract address is immutable which ensures consistency
 * in the generated instances, for new type/version of instance new deployment
 * of this proxy is required
 * 
 * @title StavmeSaMinimalProxyFactory
 * @author StavmeSa
 */
contract StavmeSaMinimalProxyFactory is Ownable, Pausable {
    ...
    function setPaused(bool _paused) external payable onlyOwner(msg.sender) {
        _setPaused(_paused);
    }
}
