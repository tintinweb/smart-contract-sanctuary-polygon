...
contract ManualOrder is OrderBase {
    ...
    function createManualOrder(TccItemV2 calldata _tcc, uint256 _tokenAmount) external nonReentrant payable {
        ...
        _y._taskConfig.tcd = TcdItem({
            ...
            _swapInDecimals: IERC20(_y.swapInToken).decimals(),
            _swapOutDecimals: IERC20(_y.swapOutToken).decimals(),
    ...
    function doManualOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        //判断是否有授权额度,如果没有则授权
        if (!y.tcc.isV3) {
            //            if (IERC20(x._TokenItem.swapInToken).allowance(address(this), V2Dex) < x.swapInAmount) {
            //                IERC20(x._TokenItem.swapInToken).approve(V2Dex, approveAmount);
            //            }
            approveDex(IERC20(x._TokenItem.swapInToken), V2Dex, x.swapInAmount);
        } else {
        ...
        if (y.tcc._swapPathConfig2._swapPathList.length > 0) {
            if (!y.tcc.isV3) {
                x.swapInAmount = MultiDexRouter(V2Dex).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            ...
            //                x.swapInAmount = MultiDexRouter(V2Dex).swapExactETHForTokensSupportingFeeOnTransferTokens{value : x.swapInAmount}(
            ...
            if (y.tcc._swapPathConfig1._swapPathList[y.tcc._swapPathConfig1._swapPathList.length - 1] == weth) {
                if (!y.tcc.isV3) {
                    x.swapInAmount = MultiDexRouter(V2Dex).swapExactTokensForETHSupportingFeeOnTransferTokens(
                        y.tcc._swapPathConfig1._routerAddressListV2,
                        x.swapInAmount,
                        0,
                        y.tcc._swapPathConfig1._swapPathList,
        ...
        if (y.tcc._swapPathConfig2._swapPathList.length > 0) {
            ...
            //判断是否有授权额度,如果没有则授权
            if (!y.tcc.isV3) {
                approveDex(IERC20(x._TokenItem.swapOutToken), V2Dex, x.swapInAmount);
                //                if (IERC20(x._TokenItem.swapOutToken).allowance(address(this), V2Dex) < x.swapInAmount) {
                //                    IERC20(x._TokenItem.swapOutToken).approve(V2Dex, approveAmount);
                //                }
            } else {
            ...
            uint256 out;
            if (!y.tcc.isV3) {
                out = MultiDexRouter(V2Dex).swapExactTokensForTokensSupportingFeeOnTransferTokens(
