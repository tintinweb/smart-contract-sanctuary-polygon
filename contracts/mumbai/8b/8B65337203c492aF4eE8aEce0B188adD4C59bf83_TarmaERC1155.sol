...
contract TarmaERC1155 is ERC1155, Ownable, ReentrancyGuard {
    ...
    mapping(address => Tarma[]) public playerTarmas;
    uint256 public tarmaCount;
    ...
    function checkin() public nonReentrant {
        for (uint i=0; i<playerTarmas[msg.sender].length; i++) {
            require(playerTarmas[msg.sender][i].memories.lastCheckin + MILLIS_PER_DAY < block.timestamp, "Too soon!");
    ...
    function spawnTarma(address recipient, string memory name) public onlyOwner {
        tarmaCount += 1;

        _mint(recipient, tarmaCount, 1, "");

        Tarma memory tarma = Tarma({
            name: name,
            id: tarmaCount,
            memories: Memories({
    ...
    function _timeTick(address playerAddress, uint256 playerTarmaId) internal {
        Tarma memory tarma = playerTarmas[playerAddress][playerTarmaId];
        ...
        playerTarmas[playerAddress][playerTarmaId].body_values.lowerBody.fitness-=1;
        playerTarmas[playerAddress][playerTarmaId].body_values.upperBody.fitness-=1;
        ...
        emit TarmaAged(
            ...
            playerTarmas[playerAddress][playerTarmaId].body_values.lowerBody.fitness,
            playerTarmas[playerAddress][playerTarmaId].body_values.upperBody.fitness,
    ...
    function feedMeal(uint256 tarmaId) public nonReentrant {
        {
            for(uint i = 0; i<playerTarmas[msg.sender].length; i++){
                if (playerTarmas[msg.sender][i].id == tarmaId) {
                    ...
                    require(playerTarmas[msg.sender][i].memories.bornDate != 0, "No born tarma found");
