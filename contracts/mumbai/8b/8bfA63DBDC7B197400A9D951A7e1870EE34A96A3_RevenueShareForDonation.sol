...
contract RevenueShareForDonation is AccessControl, ReentrancyGuard{
  ...
  function withdrawETH(address account) external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    if(getMaterialIdFromAddress(account) > 10)revert InvalidAccount();
    uint256 materialId = getMaterialIdFromAddress(account);
    if(claimablePerMaterial[materialId].amountETH == 0) revert AcountHasNoClaimableAmount();
    uint256 amount = claimablePerMaterial[materialId].amountETH;
  ...
  function withdrawWETH(address account) external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    if(getMaterialIdFromAddress(account) > 10)revert InvalidAccount();
    uint256 materialId = getMaterialIdFromAddress(account);
    if(claimablePerMaterial[materialId].amountWETH == 0) revert AcountHasNoClaimableAmount();
    uint256 amount = claimablePerMaterial[materialId].amountWETH;
