...
contract Alligators is
    ...
    uint32 private constant NUM_WORDS = 40;
    ...
    */
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        internalFulfill(_requestId, _randomWords);
        emit RequestFulfilled(_requestId, _randomWords);
    }

    function internalFulfill(uint256 _requestId, uint256[] memory _randomWords) internal {
        ...
        if (uint8(saleStatus) == 2 || uint8(saleStatus) == 1) {
            if (moddedRng < MIN_CHANCE_VALUE) {
                alligatorMint(minter, perThisTx, _requestId, _randomWords);
            } else {
                jokerMint(minter, _requestId, _randomWords);
            }
        }
        if (uint8(saleStatus) == 3) {
            alligatorMint(minter, perThisTx, _requestId, _randomWords);
        }
        if (uint8(saleStatus) == 4) {
            jokerMint(minter, _requestId, _randomWords);
        }  
    }

    function alligatorMint(address to,uint256 _quantity, uint256 _requestId, uint256[] memory _randomWords) internal {
    ...
    function jokerMint(address to, uint256 _requestId, uint256[] memory _randomWords) internal {
