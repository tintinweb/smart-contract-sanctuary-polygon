...
abstract contract IUniverseFactory is UniverseFactoryStorage {
    function allPairLength() external view virtual returns (uint256 length);
    function createSingleUniverseStorage(address userAddress) external virtual returns (address newStorageAddress);
    function createBulkUniverseStorage(UserData[] memory userDataList) external virtual ;
}
