...
contract Reserve_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
	...
	mapping(string => bool) private _existsIpfs;
	mapping(bytes32 => bool) private _existsHashMessage;

	mapping(uint256 => mapping(address => SnaphotCA)) internal claimIdCASnapshotAddress;

	// status = [ Pending (0), Voting (1), Finalizing (2), Accepted (3), Rejected (4), Canceled (5), Expired (6) ] //
	mapping(uint256 => uint8) internal _claimIdStatus;
	mapping(uint256 => ClaimIdRequestData) internal _claimIdData;

	mapping(address => uint256[]) internal _keepClaimIdUser;
	mapping(string => uint256[]) internal _keepClaimIdPolicy;
	...
	event Withdraw(address indexed caller, address indexed to, address indexed asset, uint256 value);
	...
	event ClaimRequested1st(
		address indexed asset,
		uint256 valueForRequest,
		uint256 indexed claimId,
		bool indexed boolean,
		uint256 valueForApprove
	);
	...
	event ClaimIdCancel(address indexed from, address indexed asset, uint256 indexed claimId);
	event ReInsuranceFeeNative(address indexed from, address indexed to, uint256 value);
	event ReInsuranceFeeAsset(address indexed from, address indexed to, address indexed asset, uint256 value);
	...
	/**
	 * @notice Get countClaimForRequest.
	 * @return The uint256 value.
	 */
	uint256 public countClaimForRequest;

	/**
	 * @notice Get percentageWillPassWhenRequest.
	 * @return The uint256 value.
	 */
	uint256 public percentageWillPassWhenRequest;

	/**
	 * @notice Get totalClaimValuePaid.
	 * @return The uint256 value.
	 */
	...
	function _authorizeUpgrade(address newImplementation) internal override isSuperManager {}

	/**
	 * @notice Get claimIdLastedUserWithPolicyId.
	 * @return claimId The uint256 value.
	 */
	...
	function init(INameRegistry.InitStructerParams memory initData) public {
		require(address(NR) == msg.sender, "CNNR");
	...
	/**
	 * @notice Get totalClaimValueReserve.
	 * @return The uint256 value.
	 */
	function totalClaimValueReserve() public view returns (uint256) {
		uint256 savedtotalClaimValueReserve = _totalClaimValueReserve;
		uint256 savedClaimId = countClaimForRequest; // start 1 //
		uint256 savedMinusExpires;

		for (uint256 i = 1; i <= savedClaimId; i++) {
			if (claimIdStatus(i) == 6) {
				savedMinusExpires += _claimIdData[i].approveAmount;
			}
		}
	 ...
	 *  - `_to` Must not be blacklisted "user" and "asset".
	 *
	 * Warning:
	 *  ! SUPERMANAGER USE THIS WITH REASONABLE ONLY. !
	 */
	function withdraw(
		address _to,
		address _asset,
		uint256 _value
	) public nonReentrant isSuperManager {
		require(AR.isSupportAssets(poolId, _asset), "A");
		IERC20 _ERC20 = IERC20(_asset);
		require(PDS.isBlacklistUser(_to) != true);
		require(PDS.isBlacklistAssetsUser(_to, _asset) != true);

		_ERC20.transfer(_to, _value);

		emit Withdraw(msg.sender, _to, _asset, _value);
	}

	/**
	 * @notice Get verifyClaimRequest.
	 * @dev This function for verifyMessage from backend.
	 * _generatedAt Timestamp in second.
	 * _expiresAt Timestamp in second.
	 * @param _user The user address.
	 * @param _asset The asset address.
	 * @param _ipfsHash The ipfsHash.
	 * @param _value The uint256 value.
	...
	function verifyClaimRequest(
		...
		return keccak256(abi.encodePacked(_user, _asset, _ipfsHash, _value, _generatedAt, _expiresAt, address(this)));
	}

	/**
	 * @dev decodedClaimPolicy for decode `_data` and put it into tuple.
	 * @return The tuple value that is ClaimPolicyParams.
	 */
	function decodedClaimPolicy(bytes memory _data) internal pure returns (ClaimPolicyParams memory) {
		(
			...
			bytes32 r,
			bytes32 s
		) = abi.decode(_data, (address, address, string, uint256, uint256, uint8, bytes32, bytes32));
		ClaimPolicyParams memory decodedData = ClaimPolicyParams(
			user,
			asset,
			ipfsHash,
			generatedAt,
	...
	/**
	 * @dev policyClaimInternal for buyPolicy is valid or not.
	 * @return The boolean value that tell the policy param for claimPolicy is valid or not.
	 */
	function policyClaimInternal(
		address _user,
		address _assets,
		string memory _ipfsHash,
		uint256 _value,
		...
		bytes32 hashMessage = verifyClaimRequest(
			_user,
			_assets,
			_ipfsHash,
			_value,
		...
		require(_existsHashMessage[hashMessage] == false, "HMAU");
	...
	/**
	 * @dev Set CAVoting for who is ca active in that claimId.
	 * @param _claimId The uint256 value of claimId.
	 * @return The boolean value that can set successful or not.
	 */
	function setCAvoting(uint256 _claimId) private returns (bool) {
		address[] memory allareca = CA.allCA();
		require(allareca.length > 0);
		for (uint256 i = 0; i < allareca.length; i++) {
			claimIdCASnapshotAddress[_claimId][allareca[i]].haveAccess = true;
			claimIdCASnapshotAddress[_claimId][allareca[i]].alreadyVote = false;
		}
	 ...
	 * @notice existstIpfs.
	 * @param _ipfsHash The ipfsHash.
	 * @return The boolean value that ipfsHash is exists or not.
	 */
	function existsIpfs(string memory _ipfsHash) public view returns (bool) {
		return _existsIpfs[_ipfsHash];
	}

	/**
	 * @notice reInsuranceFeeNative.
	 * @dev for withdraw Native to reInsurer.
	 * @param _to The destination address.
	 ...
	 *  - `msg.sender` Must be a SuperManager.
	 */
	function reInsuranceFeeNative(address _to, uint256 _value) public nonReentrant isSuperManager {
	...
	/**
	 * @notice reInsuranceFeeAsset.
	 * @dev for withdraw asset to reInsurer.
	 * @param _to The destination address.
	 * @param _asset The asset address.
	 * @param _value The value.
	 *
	...
	function policyClaimRequest(
		...
		require(NR.paused() == false, "CL");
		if (PDS.isOnlyDistributor() == true) {
			require(AR.isPolicyDistributor(poolId, msg.sender) == true, "ND");
		}
		(IPolicyDetails.PolicyIdData memory policyData, bool isActive, bool isRedeemed) = PDS.policyData(_policyId);
		require(isActive == true, "PNA");
		require(isRedeemed == false, "PIR");
		require(policyData.user == msg.sender, "YAOP");
		...
		require(
			maxCoverageOfThisUser > 0 &&
				(maxCoverageOfThisUser -
					_historyOfPolicyId[_policyId].claimAmountPaid -
					_historyOfPolicyId[_policyId].claimPending >=
					_value),
			"DBP-PRE"
		);
		require(existsIpfs(_ipfsHash) == false, "HD");
		// PDS decimals //
		require(_value >= 10**18, "D18");
		require(policyClaimInternal(msg.sender, policyData.asset, _ipfsHash, _value, _data), "ED");
		_existsIpfs[_ipfsHash] = true;
		_historyOfPolicyId[_policyId].claimPending += _value;
		uint256 _claimId = countClaimForRequest + 1;
		countClaimForRequest += 1;
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		require(setCAvoting(_claimId), "SE");
		claimIdDataThisId.caSnapshotCount = CA.countCA();
		claimIdDataThisId.caSnapshotValue = S.workData(IStaking.StakingTypes.CA).powerCount;

		_claimIdStatus[_claimId] = 0;
		...
		_keepClaimIdPolicy[_policyId].push(_claimId);
	...
	/**
	 * @notice firstCAVote.
	 * @param _claimId The number value of claimId.
	 * @param _boolean The boolean value that mean this claimId will pass or not from the first CA.
	 * @param _valueApprove The value approve, if `_boolean` = false => _valueApprove must be zero.
	 *
	 * Requirement:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *  - `msg.sender` Must be worker of ClaimAssessors.
	 */
	function firstCAVote(
		uint256 _claimId,
		bool _boolean,
		uint256 _valueApprove
	) public nonReentrant isCA {
		require(NR.paused() == false, "CL");
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		require(_claimIdStatus[_claimId] == 0, "NP");
		require(claimIdDataThisId.user != msg.sender && claimIdDataThisId.user != address(0), "CAC-CFC");
		require(claimIdDataThisId.asset != address(0), "IA");
		require(_valueApprove >= 10**18 || (_valueApprove == 0 && _boolean == false), "D18");
		require(claimIdDataThisId.requestAmount >= _valueApprove, "A>U");
		require(
			claimIdCASnapshotAddress[_claimId][msg.sender].haveAccess == true &&
				claimIdCASnapshotAddress[_claimId][msg.sender].alreadyVote == false,
			"NHA-AV"
		);
		if (_boolean == false) {
			require(_valueApprove == 0, "VAZ");
		}
		bool policyIsActive = PDS.isPolicyActive(claimIdDataThisId.policyId);
		require(policyIsActive == true, "PNA");

		claimIdCASnapshotAddress[_claimId][msg.sender].alreadyVote = true;
		S.update(msg.sender, IStaking.StakingTypes.CA);
		...
		IStaking.StakingData memory dataCA = S.stakingData(msg.sender, IStaking.StakingTypes.CA);

		claimIdDataThisId.votingPass += dataCA.balance - dataCA.penalty;

		claimIdDataThisId.caSnapshotVoteCount += 1;
		_claimIdStatus[_claimId] = 1;
		...
		emit ClaimRequested1st(
			claimIdDataThisId.asset,
			claimIdDataThisId.requestAmount,
			_claimId,
			_boolean,
			_valueApprove
		);
	...
	/**
	 * @notice caProposalVoting.
	 * @dev The second vote it will vote of the result of the first vote CA.
	 * such as The first vote pass the others of CA will vote for proposal is it true the answer of the first CA.
	 * @param _claimId The number value of claimId.
	 * @param _boolean The boolean value that mean this claimId will pass or not from the first CA.
	 *
	 * Requirement:
	...
	function caProposalVoting(uint256 _claimId, bool _boolean) public nonReentrant isCA {
		require(NR.paused() == false, "CL");
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		require(_claimIdStatus[_claimId] == 1, "NV");
		...
		emit CAProposal(claimIdDataThisId.asset, _claimId, _boolean);
	...
	/**
	 * @notice Set passPercentage.
	 * @param _percentage The percentage of the pass claim.
	 *
	 * Requirement:
	 *  - `msg.sender` Must be a SuperManager.
	 */
	...
	/**
	 * @notice claimIdCancel.
	 * @param _claimId The number value of claimId.
	 *
	 * Requirement:
	 *  - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *  - `_claimId` Must owner of msg.sender or Must be Manager can be cancel.
	 *  - `_claimId` Must status is Pending or Expired.
	 *  - `_claimId` Must not status Canceled.
	 */
	function claimIdCancel(uint256 _claimId) public nonReentrant {
		require(NR.paused() == false, "CL");
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		require(claimIdDataThisId.user == msg.sender || AR.hasRole(MANAGER_ROLE, msg.sender) == true, "NOMR");
		require(
			_claimIdStatus[_claimId] != 5 && (_claimIdStatus[_claimId] == 0 || claimIdStatus(_claimId) == 6),
			"AC-NP"
		);
		address _asset = claimIdDataThisId.asset;
		require(_asset != address(0), "IA");
		_claimIdStatus[_claimId] = 5;
		...
		emit ClaimIdCancel(msg.sender, _asset, _claimId);
	}

	/**
	 * @dev _finalize, if status of `_claimId` is Voting it must caSnapshotCount equal to caSnapshotVoteCount
	 * else it must Finalizing.
	 * @param _claimId The number value of claimId.
	 *
	 * Requirement:
	 *  - `_claimId` Must status is Voting or Finalizing.
	 *  - `policyId` Must active it will get from _claimId.
	 */
	function _finalize(uint256 _claimId) private {
		require(NR.paused() == false, "CL");
		ClaimIdRequestData storage claimIdDataThisId = _claimIdData[_claimId];
		if (_claimIdStatus[_claimId] == 1) {
			require(claimIdDataThisId.caSnapshotVoteCount == claimIdDataThisId.caSnapshotCount, "NECA-C");
		} else {
			require(claimIdStatus(_claimId) == 2, "SDF");
		}
	 ...
	 *
	 * Requirement:
	 *  - `msg.sender` Must be a this contract or Must be a Manager.
	 *  - `_claimId` Must status is Finalizing.
	 */
	function finalize(uint256 _claimId) public ForFinalize {
		require(claimIdStatus(_claimId) == 2, "NF");
		require(block.timestamp > _claimIdData[_claimId].timeout, "NTO");

		_finalize(_claimId);
	}

	/**
	 * @notice Get accessForClaimId.
	 * @param _user The user address.
	 * @param _claimId The number value of claimId.
	 * @return haveAccess The boolean value that tell this user have a permission for this claimId ( only for CA ).
	 * @return alreadyVote The boolean value that tell this user is voted or not for this claimId ( only for CA ).
	 */
	function accessForClaimId(address _user, uint256 _claimId) public view returns (bool haveAccess, bool alreadyVote) {
		haveAccess = claimIdCASnapshotAddress[_claimId][_user].haveAccess;
		alreadyVote = claimIdCASnapshotAddress[_claimId][_user].alreadyVote;
		return (haveAccess, alreadyVote);
	}

	/**
	 * @notice Get claimIdStatus.
	 * @dev status must be one of the following:
	 *  - Pending (0)
	 *  - Voting (1)
	 *  - Finalizing (2)
	 *  - Accepted (3)
	 ...
	 *
	 * @param _claimId The number value of claimId.
	 * @return The uint8 value of status.
	 */
	function claimIdStatus(uint256 _claimId) public view returns (uint8) {
		require(_claimIdData[_claimId].user != address(0));
		if (_claimIdStatus[_claimId] == 0 && block.timestamp > _claimIdData[_claimId].timeout) {
			return 6;
		} else if (_claimIdStatus[_claimId] == 1 && block.timestamp > _claimIdData[_claimId].timeout) {
			return 2;
		} else {
			return _claimIdStatus[_claimId];
		}
	}

	/**
	 * @notice Get allClaimStatus.
	 * @dev Get allClaimStatus by filter status and you will received claimId of the filter status.
	 * @param _status The uint8 value of the status.
	 * @return The tuple array value of ClaimIdRequestData.
	 * @return The claimId array value.
	 */
	function getAllClaimStatus(uint8 _status) public view returns (ClaimIdRequestData[] memory, uint256[] memory) {
		uint256 _keepNumberLength;
		uint256 _keepIndexNumber;
		uint256[] memory _claimIdArrayBefore = new uint256[](countClaimForRequest);

		for (uint256 index = 1; index <= countClaimForRequest; index++) {
			uint8 statusClaimId = claimIdStatus(index);
			if (statusClaimId == _status) {
				_claimIdArrayBefore[_keepNumberLength] = index;
				_keepNumberLength++;
			}
		}

		ClaimIdRequestData[] memory arrayStatus = new ClaimIdRequestData[](_keepNumberLength);
		uint256[] memory _claimIdArray = new uint256[](_keepNumberLength);

		for (uint256 index = 0; index < _keepNumberLength; index++) {
			ClaimIdRequestData memory claimIdMetadataIndex = claimIdData(_claimIdArrayBefore[index]);
			arrayStatus[_keepIndexNumber] = claimIdMetadataIndex;
			_claimIdArray[_keepIndexNumber] = _claimIdArrayBefore[index];
			_keepIndexNumber++;
		}

		return (arrayStatus, _claimIdArray);
	}

	/**
	 * @notice Get pendingClaimRequest.
	 * @param _policyId The policyId.
	 * @return The uint256 value of pendingClaimRequest of that policyId.
	 */
	...
	/**
	 * @notice Get claimIdData.
	 * @param _claimId The number value of claimId.
	 * @return The tuple value of ClaimIdRequestData.
	 */
	function claimIdData(uint256 _claimId) public view returns (ClaimIdRequestData memory) {
		require(_claimIdData[_claimId].user != address(0), "NFCID");
		return _claimIdData[_claimId];
	}

	/**
	 * @notice Get claimPolicyDataId.
	 * @param _policyId The policyId.
	 * @return The tuple value of HistoryOfPolicyIdData.
	 */
	...
	/**
	 * @notice Get claimAmountPaid.
	 * @param _policyId The policyId.
	 * @return The uint256 value of claimAmountPaid of that policyId.
	 */
	...
	/**
	 * @notice Get userClaimId.
	 * @param _user The uesr address.
	 * @return The uint256 array value that is all history of that user.
	 */
	function userClaimId(address _user) public view returns (uint256[] memory) {
		return _keepClaimIdUser[_user];
	}

	/**
	 * @notice Get policyClaimId.
	 * @param _policy The policyId.
	 * @return The uint256 array value that is all history of that user.
	 */
	function policyClaimId(string memory _policy) public view returns (uint256[] memory) {
		return _keepClaimIdPolicy[_policy];
	}
}
...
interface IClaimAssessors {
	...
	function balanceOfPolicyId(string memory _policyId) external view returns (uint256);
	...
	function isClaimAssessorsNonCheckStake(address _user) external view returns (bool);
