...
import "../premium/IFrameItPremiumList.sol";
import "../nfts/IFrameItLootBox.sol";
import "../dex/IRungieDEX.sol";
import "../utils/Ownable.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";

contract FrameItMarketplace is Ownable {
    ...
    address private rungieDEX;
    address private premiumUsersList;
    bool private activatePremium;
    ...
    constructor(address _rungieToken, address _rungieDEX, address _premiumUsersList, uint256 _primarySaleIDNonceCounter, uint256 _secondarySaleIDNonceCounter) {
        ...
        rungieDEX = _rungieDEX;
        premiumUsersList = _premiumUsersList;
        activatePremium = true;
    }

    function setActivatePremium(bool _value) external onlyOwner {
        activatePremium = _value;
    }
    ...
    function setPrimarySaleFullLootBox(address _nftAddress, uint24 _numberOfNFTs, uint256 _price, address _token, uint64 _startDate, uint64 _endDate, uint16 _maxAllowedSale) external {
        ...
        emit PrimarySaleLootBoxEvent(msg.sender, _nftAddress, _numberOfNFTs, _price, _token, primarySaleIDNonceCounter, _startDate, _endDate);
    }

    function cancelPrimarySaleLootBox(uint256 _saleID) external {
        PrimarySaleStruct memory sale = primarySales[_saleID];
        require(sale._owner == msg.sender, "BadSaleOwner");
    ...
    // TODO. Limitar que solo se pueda comprar X sobres al minuto????
    function buyPrimarySaleLootBox(uint256 _saleID, uint24 _amount) external payable {
        ...
        for (uint256 i=0; i<_amount; i++) {
            ...
            IERC721(sale._nft).safeTransferFrom(sale._nft, msg.sender, _id);
            _ids[i] = _id;
        }
        sale._sold += _amount;

        bool isUserPremium = IFrameItPremiumList(premiumUsersList).isPremiumUser(sale._owner);
        address salesWallet = IFrameItLootBox(sale._nft).salesWallet();

        if (activatePremium && isUserPremium) {
            if (sale._token == rungieToken) {
                IERC20(sale._token).safeTransferFrom(msg.sender, address(this), sale._price);
                IRungieDEX(rungieDEX).swapRungiesToStableCoin(salesWallet, block.timestamp + 10, sale._price, 0, 0);    // TODO. Estimate better minimum output
            } else {
                IERC20(sale._token).safeTransferFrom(msg.sender, salesWallet, sale._price);
            }
        } else {
        ...
        emit BuyLootBoxEvent(sale._owner, msg.sender, _saleID, _amount, msg.value, sale._token, _ids);
    }

    function setSecondarySale(address _nftAddress, uint256 _id, uint256 _price, address _token, uint256 _expirationDate) external {
        require(IERC721(_nftAddress).ownerOf(_id) == msg.sender, "BadOwner");
        ...
        emit SecondarySaleLootBoxEvent(msg.sender, _nftAddress, _id, _price, _token, _expirationDate, secondarySaleIDNonceCounter);
    }
    ...
    function buySecondarySale(uint256[] calldata _saleIDs) external payable {
        ...
        for (uint256 i=0; i<_saleIDs.length; i++) {
            ...
            else IERC20(sale._token).safeTransferFrom(msg.sender, salesWallet, sale._price);
            
            IERC721(sale._nft).safeTransferFrom(sale._owner, msg.sender, sale._id);

            emit BuySecondaryEvent(sale._owner, msg.sender, sale._nft, sale._id, sale._price, sale._token);
        }
    ...
    function claimOffer(bytes calldata _message, bytes memory _messageLen, bytes calldata _signature) external payable {
        ...
        else IERC20(_token).safeTransferFrom(msg.sender, salesWallet, _price);
...
interface IFrameItPremiumList {

    function addPremiumUser(address _account) external;
    function addMultiplesPremiumUsers(address[] calldata _accounts) external;
    function removePremiumUser(address _account) external;
    function removeMultiplesPremiumUsers(address[] calldata _accounts) external;
    function isPremiumUser(address _account) external view returns (bool);
}
...
interface IRungieDEX {

    function swapRungiesToStableCoin(address _recipient, uint256 _deadline, uint256 _amountIn, uint256 _amountOutMinimum, uint160 _sqrtPriceLimitX96) external;
    function estimateRungiesForStableCoin(uint256 _amountIn) external returns (uint256);
}
