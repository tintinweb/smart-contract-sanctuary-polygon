...
contract VCMarketManager is IVCMarketManager, CanWithdrawERC20 {
    address public admin;
    ...
    address public marketplaceFixedPricePocNft;
    address public marketplaceAuctionPocNft;
    address public pool;
    ...
    IVCArtNftERC1155 public artNftERC1155;
    IVCArtNftERC721 public artNftERC721;
     ...
     * @param _marketplaceFixedPriceERC1155 Address of the fixedPrice marketplace
     * @param _marketplaceFixedPriceERC721 Address of the fixedPrice marketplace
     * @param _marketplaceAuctionERC721 Address of the auction marketplace
     * @param _marketplaceFixedPricePocNft Address of the POC fixedPrice marketplace
     * @param _marketplaceAuctionPocNft Address of the POC auction marketplace
     * @param _artNftERC1155 Address of the ERC1155 Art NFT
     * @param _artNftERC721 Address of the ERC721 Art NFT
     */
    constructor(
        address _admin,
        ...
        marketplaceFixedPricePocNft = _marketplaceFixedPricePocNft;
        marketplaceAuctionPocNft = _marketplaceAuctionPocNft;

        artNftERC1155 = IVCArtNftERC1155(_artNftERC1155);
        artNftERC721 = IVCArtNftERC721(_artNftERC721);

        admin = _admin;
    }

    /**
     * @dev See {IVCMarketManager-setAdmin}.
     */
    function setAdmin(address _admin) external {
        _onlyAdmin();
        _checkAddress(_admin);
    ...
    /**
     * @dev See {IVCMarketManager-setPool}.
     */
    function setPool(address _pool) external {
        _onlyAdmin();
        _checkAddress(_pool);
        _setTo(_pool);
        pool = _pool;
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setPool(_pool);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setPool(_pool);
        IFeeBeneficiary(marketplaceAuctionERC721).setPool(_pool);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setPool(_pool);
        IFeeBeneficiary(marketplaceAuctionPocNft).setPool(_pool);
    }

    /**
     * @dev See {IVCMarketManager-setStarter}.
     */
    function setStarter(address _starter) external {
        _onlyAdmin();
        _checkAddress(_starter);
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setStarter(_starter);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setStarter(_starter);
        IFeeBeneficiary(marketplaceAuctionERC721).setStarter(_starter);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setStarter(_starter);
        IFeeBeneficiary(marketplaceAuctionPocNft).setStarter(_starter);
    }

    /**
     * @dev See {IVCMarketManager-changeMarketManager}.
     */
    function changeMarketManager(address _mktManager) external {
        _onlyAdmin();
        _checkAddress(_mktManager);
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setAdmin(_mktManager);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setAdmin(_mktManager);
        IFeeBeneficiary(marketplaceAuctionERC721).setAdmin(_mktManager);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setAdmin(_mktManager);
        IFeeBeneficiary(marketplaceAuctionPocNft).setAdmin(_mktManager);
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceFixedPriceERC721}.
     */
    function setMarketplaceFixedPriceERC721(address _marketplaceFixedPriceERC721) external {
        _onlyAdmin();
        _checkAddress(_marketplaceFixedPriceERC721);
        marketplaceFixedPriceERC721 = _marketplaceFixedPriceERC721;
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceAuctionERC721}.
     */
    function setMarketplaceAuctionERC721(address _marketplaceAuctionERC721) external {
        _onlyAdmin();
        _checkAddress(_marketplaceAuctionERC721);
        marketplaceAuctionERC721 = _marketplaceAuctionERC721;
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceFixedPriceERC1155}.
     */
    function setMarketplaceFixedPriceERC1155(address _marketplaceFixedPriceERC1155) external {
        _onlyAdmin();
        _checkAddress(_marketplaceFixedPriceERC1155);
        marketplaceFixedPriceERC1155 = _marketplaceFixedPriceERC1155;
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceFixedPricePocNft}.
     */
    function setMarketplaceFixedPricePocNft(address _marketplaceFixedPricePocNft) external {
        _onlyAdmin();
        _checkAddress(_marketplaceFixedPricePocNft);
        marketplaceFixedPricePocNft = _marketplaceFixedPricePocNft;
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceAuctionPocNft}.
     */
    function setMarketplaceAuctionPocNft(address _marketplaceAuctionPocNft) external {
        _onlyAdmin();
        _checkAddress(_marketplaceAuctionPocNft);
        marketplaceAuctionPocNft = _marketplaceAuctionPocNft;
    }

    /**
     * @dev See {IVCMarketManager-getMarketplaces}.
     */
    function getMarketplaces() external view returns (address[5] memory marketplaces) {
        marketplaces[0] = marketplaceFixedPriceERC1155;
        marketplaces[1] = marketplaceFixedPriceERC721;
        marketplaces[2] = marketplaceAuctionERC721;
        marketplaces[3] = marketplaceFixedPricePocNft;
        marketplaces[4] = marketplaceAuctionPocNft;
    }

    /**
     * @dev See {IVCMarketManager-setArtNftERC721}.
     */
    function setArtNftERC721(address _artNftERC721) external {
        _onlyAdmin();
        _checkAddress(_artNftERC721);
        artNftERC721 = IVCArtNftERC721(_artNftERC721);
    }

    /**
     * @dev See {IVCMarketManager-setArtNftERC1155}.
     */
    function setArtNftERC1155(address _artNftERC1155) external {
        _onlyAdmin();
        _checkAddress(_artNftERC1155);
        artNftERC1155 = IVCArtNftERC1155(_artNftERC1155);
    }

    /**
     * @dev See {IVCMarketManager-callContract}.
     */
    function callContract(address _contract, bytes calldata _data) external {
        _onlyAdmin();
        (bool status, ) = _contract.call(_data);
        if (!status) {
            revert MktMgrCallContractFailedError();
        }
    }

    // IDEA:
    // Admin should call these ones only, not the ones at each marketplace smart contract.
    // function all_other_sets() onlyAdmin {};

    /**
     * @dev See {IVCMarketManager-approveForAllMarketplaces}.
     */
    function approveForAllMarketplaces() external {
        ...
        pocNft.setApprovalForAllCustom(msg.sender, marketplaceFixedPricePocNft, true);
        pocNft.setApprovalForAllCustom(msg.sender, marketplaceAuctionPocNft, true);
        emit MktTransfersApproved(msg.sender);
    }

    /**
     * @dev See {IVCMarketManager-accrueRoyalty}.
     */
    function accrueRoyalty(address _receiver, uint256 _royaltyAmount) external {
        _onlyMarketplaces();
        royalties[_receiver] += _royaltyAmount;
        _balanceNotWithdrawable[currency] += _royaltyAmount;
        emit MktRoyaltiesAccrued(_receiver, _royaltyAmount);
    }

    /**
     * @dev See {IVCMarketManager-claimRoyalties}.
     */
    function claimRoyalties(address _to, uint256 _poolAmount) external {
        uint256 claimAmount = royalties[msg.sender];
        if (claimAmount == 0) {
            revert MktNoRoyaltiesError();
        }
        if (_poolAmount > claimAmount) {
            revert MktMgrInvalidPoolAmountError();
        }

        royalties[msg.sender] = 0;
        _balanceNotWithdrawable[currency] -= claimAmount;

        claimAmount -= _poolAmount;
        if (claimAmount > 0 && !currency.transfer(_to, claimAmount)) {
            revert MktMgrFailedToClaimError();
        }
        if (_poolAmount > 0) {
            if (!currency.transfer(pool, _poolAmount)) {
                revert MktMgrFailedToClaimError();
            }
            pocNft.mint(msg.sender, _poolAmount, true);
        }

        emit MktRoyaltiesClaimed(msg.sender, _to, claimAmount, _poolAmount);
    }

    /**
     * @dev See {IVCMarketManager-setCurrency}.
     */
    function setCurrency(IERC20 _currency) external {
        _onlyAdmin();
        currency = _currency;
        IFeeBeneficiary(marketplaceAuctionERC721).setCurrency(_currency);
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setCurrency(_currency);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setCurrency(_currency);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setCurrency(_currency);
        IFeeBeneficiary(marketplaceAuctionPocNft).setCurrency(_currency);
    }

    /**
     * @dev See {IVCMarketManager-setPoCNft}.
     */
    function setPoCNft(address _pocNft) external {
        _onlyAdmin();
        pocNft = IVCPoCNft(_pocNft);
        IVCMarketplaceBase(marketplaceAuctionERC721).setPoCNft(_pocNft);
        IVCMarketplaceBase(marketplaceFixedPriceERC1155).setPoCNft(_pocNft);
        IVCMarketplaceBase(marketplaceFixedPriceERC721).setPoCNft(_pocNft);
        IVCMarketplaceBase(marketplaceFixedPricePocNft).setPoCNft(_pocNft);
        IVCMarketplaceBase(marketplaceAuctionPocNft).setPoCNft(_pocNft);
    }

    /**
     * @dev See {IVCMarketManager-setMinPoolFeeBps}.
     */
    function setMinPoolFeeBps(uint96 _minPoolFeeBps) external {
        _onlyAdmin();
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setMinPoolFeeBps(_minPoolFeeBps);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setMinPoolFeeBps(_minPoolFeeBps);
        IFeeBeneficiary(marketplaceAuctionERC721).setMinPoolFeeBps(_minPoolFeeBps);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setMinPoolFeeBps(_minPoolFeeBps);
        IFeeBeneficiary(marketplaceAuctionPocNft).setMinPoolFeeBps(_minPoolFeeBps);
    }

    /**
     * @dev See {IVCMarketManager-setMarketplaceFeeBps}.
     */
    function setMarketplaceFeeBps(uint256 _marketplaceFeeBps) external {
        _onlyAdmin();
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setMarketplaceFeeBps(_marketplaceFeeBps);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setMarketplaceFeeBps(_marketplaceFeeBps);
        IFeeBeneficiary(marketplaceAuctionERC721).setMarketplaceFeeBps(_marketplaceFeeBps);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setMarketplaceFeeBps(_marketplaceFeeBps);
        IFeeBeneficiary(marketplaceAuctionPocNft).setMarketplaceFeeBps(_marketplaceFeeBps);
    }

    /**
     * @dev See {IVCMarketManager-setMaxBeneficiaryProjects}.
     */
    function setMaxBeneficiaryProjects(uint96 _maxBeneficiaryProjects) external {
        _onlyAdmin();
        IFeeBeneficiary(marketplaceFixedPriceERC1155).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IFeeBeneficiary(marketplaceFixedPriceERC721).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IFeeBeneficiary(marketplaceAuctionERC721).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IFeeBeneficiary(marketplaceFixedPricePocNft).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
        IFeeBeneficiary(marketplaceAuctionPocNft).setMaxBeneficiaryProjects(_maxBeneficiaryProjects);
    }

    /**
     * @dev See {IVCMarketManager-setListStatusERC721}.
     */
    function setListStatusERC721(uint256 _tokenId, bool listed) external {
        if (msg.sender == marketplaceFixedPriceERC721) {
            ...
            revert MktMgrOnlyMarketplaceAllowedError();
        }
    }

    /**
     * @dev See {IVCMarketManager-getListStatusERC721}.
     */
    function getListStatusERC721(uint256 _tokenId) external view returns (ListStatus) {
        return _listStatusERC721[_tokenId];
    }

    function _onlyMarketplaces() internal view {
        if (
     ...
     */
    function _checkAddress(address _addr) internal view {
        if (_addr == address(this) || _addr == address(0)) {
            revert MktMgrUnexpectedAddressError();
        }
    }
    ...
    function _onlyAdmin() internal view {
        if (msg.sender != admin) {
            revert MktMgrOnlyAdminAllowedError();
        }
    }
}
