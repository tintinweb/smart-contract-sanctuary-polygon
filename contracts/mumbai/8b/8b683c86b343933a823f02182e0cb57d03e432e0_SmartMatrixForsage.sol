...
constructor(address ownerAddress, address secondOwnerAddress, address thirdOwnerAddress) public {
    ...
    // Update second owner's matrix current referrer to OwnerAddress
    for(uint8 i = 1; i <= LAST_LEVEL; i++){
    users[secondOwnerAddress].matrices[i].currentReferrer = ownerAddress;
    }
for(uint8 i = 1; i <= 5; i++){
    // Update third owner's matrix current referrer to SecondOwnerAddress
    users[thirdOwnerAddress].matrices[i].currentReferrer = secondOwnerAddress;
}
}
