...
contract SafeTime {
	...
	function _initTime(uint256 _externalTimeStampTrusted)  private returns (Time.time memory) {
		...
		Time.time memory _t = Time.time(0, _externalTimeStampTrusted, block.number, 0);
		return _t;
	}

	function currentTime() private returns(uint256) {
		return Time.getSafeTime(_t, block.number);
	}
}
...
library Time {
	...
	**/
	...
	function _setTime(time memory _t, uint256 _blockNumber) internal {
		_t._value = _blockToTime(_t, _blockNumber);
		_t.lastBlock = _blockNumber;
	}

	function getSafeTime(time memory _t, uint256 _blockNumber) public returns(uint256) {
		_setTime(_t, _blockNumber);
		return _t._value;
	}
