    ...
    AccessControl
{
    ...
    constructor(Token _tokenContract, address childChainManager)
        ERC721("MerchCoin NFT", "MRCC-NFT")
        EIP712("MerchCoin-NFT", "1")
    {
    ...
    function purchaseItem(address originalSigner, NFTVoucher calldata voucher, bytes memory signature)
        external
    // bytes memory signature
    {
        // make sure signature is valid and get the address of the signer
        address signer = _verify(voucher, signature);
        // make sure that the voucher & signature has not been modified
        require(signer == originalSigner, "signature or message been modified");
        ...
        require(!withdrawnTokens[voucher.tokenId], "ChildMerchCoinNFT: TOKEN_EXISTS_ON_ROOT_CHAIN");
        // first assign the token to the signer, to establish provenance on-chain
        _mint(originalSigner, voucher.tokenId);
        _setTokenURI(voucher.tokenId, voucher.uri);

        // transfer the token to the redeemer
        _transfer(originalSigner, msg.sender, voucher.tokenId);
...
contract Token is ERC20, ReentrancyGuard, Pausable{
    string  public standard = "MRCC Token v1.0.0";

    constructor() ERC20("MerchCoin", "MRCC") ReentrancyGuard() Pausable(){
