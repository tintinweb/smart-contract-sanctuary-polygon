...
contract SuperfluidEndResolver is AccessControl {
    ...
    function addToChecker(address _sender, address _receiver, uint256 timestamp) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        ibAlluoStreamDataToTimestamp[msg.sender][_sender][_receiver]= timestamp;
    }
    ...
    function liquidateSender(address _sender, address _receiver, address _token) external onlyRole(GELATO) {
        IIbAlluo(_token).stopFlowWhenCritical(_sender, _receiver);
