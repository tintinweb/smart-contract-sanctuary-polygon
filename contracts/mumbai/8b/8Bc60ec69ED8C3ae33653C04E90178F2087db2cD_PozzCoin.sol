    ...
    ERC20Upgradeable
{
    ...
    uint256 public prevFeeAmount;
    uint256 public prevBackingAmount;
    address public prevTransferTo;
    ...
    modifier onlyRift() {
        require(msg.sender == rift, "PozzCoin: invalid rift");
        _;
    }
    ...
    function initialize(
        ...
        _mint(_premintAddr, 100000000e18);
    }
    ...
    function _balanceAdjustment(bool increase, uint256 amount) internal {
        if (increase) {
            require(
                ...
                "PozzCoin: mint amount exceeds balance"
            );
            ...
            require(
                ...
                "PozzCoin: burn amount exceeds balance"
            );
            unchecked {
                _balances[address(pair)] = _balances[address(pair)] - amount;
                _balances[vault] = _balances[vault] + amount;
    ...
    // returns the price of poz to usdc times 1e18(for decimal issue)
    function getPoolPrice() public view returns (uint256 price) {
        bool isToken0 = pair.token0() == address(this);
        ERC20 stableToken = isToken0 ? ERC20(pair.token1()) : ERC20(pair.token0());
        uint256 stableBalance = stableToken.balanceOf(address(pair));
        price = (stableBalance * (10**decimals()) * 1e18) / (balanceOf(address(pair)) * (10**stableToken.decimals()));
    }
    ...
    function _transfer(
        ...
        if (!isWhitelist && isTrade) {
            isPauseTrade = isPauseTrade || prevBackingAmount > 0;
        ...
        if (!isWhitelist && isTrade) {
            ...
            prevFeeAmount = feeAmount;
            prevBackingAmount = feeAmount - emissionAmount;
        }
    }
    ...
    function dynamicAdjustment(bool isTrade) public onlyRift {
        require(prevBackingAmount > 0, "PozzCoin: unknown backing amount");

        if(!isTrade) {
            require(prevTransferTo != address(0), "PozzCoin: unknown to address");
            require(_balances[backing.reserveAddress] >= prevBackingAmount, "PozzCoin: insufficient backing poz");
            require(_balances[emission.reserveAddress] >= prevFeeAmount - prevBackingAmount, "PozzCoin: insufficient emission poz");
            _balances[backing.reserveAddress] = _balances[backing.reserveAddress] - prevBackingAmount;
            _balances[emission.reserveAddress] = _balances[emission.reserveAddress] - (prevFeeAmount - prevBackingAmount);
            _balances[prevTransferTo] = _balances[prevTransferTo] + prevFeeAmount;
            if(prevTransferTo == address(pair)) {
                pair.sync();
            }
        } else {
            bool isToken0 = pair.token0() == address(this);
            ...
            uint256 truePrice = prevPrice + (prevBackingAmount * prevPrice / circSupply);
            uint256 adjustedPozAmount = (stableBalance * 1e18 / truePrice) * 10**(decimals() - stableToken.decimals());
            bool isIncrease = adjustedPozAmount > balanceOf(address(pair));
            if (isIncrease) {
                _balanceAdjustment(true, adjustedPozAmount - balanceOf(address(pair)));
            } else {
                _balanceAdjustment(false, balanceOf(address(pair)) - adjustedPozAmount);
            }
            uint256 adjustedPrice = getPoolPrice();
            require(adjustedPrice > prevPrice, "PozzCoin: price logic fails");
            prevPrice = adjustedPrice;
        }
        prevTransferTo = address(0);
        prevFeeAmount = 0;
        prevBackingAmount = 0;
        isPauseTrade = false;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "PozzCoin: backing reserve address cannot be zero"
        );
        require(
            _reserveRate <= 10000,
            "PozzCoin: backing reserve rate cannot be greater than 100%"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "PozzCoin: emission reserve address cannot be zero"
        );
        require(
            _emissionRate <= 10000,
            "PozzCoin: emission reserve rate cannot be greater than 100%"
        );
        ...
        onlyOwner
    {
        require(
            ...
            "PozzCoin: locked address cannot be zero"
        );
        bool flag = false;
        for (uint256 i = 0; i < lockedAddress.length; i++) {
    ...
    function setIsPauseTrade(bool _isPauseTrade) external onlyRift {
        if(_isPauseTrade == false) {
            require(prevBackingAmount == 0, "PozzCoin: price is still lower");
        }
        isPauseTrade = _isPauseTrade;
    }
