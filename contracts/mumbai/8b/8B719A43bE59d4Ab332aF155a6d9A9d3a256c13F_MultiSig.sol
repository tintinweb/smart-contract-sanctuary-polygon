...
contract MultiSig is EIP712, ReentrancyGuard {
    ...
    function recoverSigner(address _to, uint256 _value, bytes memory _data, bytes memory userSignature, uint256 _nonce) public view returns (address) {
    ...
    // @param _nonce - transaction nonce
    function executeTransaction(bytes[] memory signatures, address _to, uint256 _value, bytes memory _data, uint256 _nonce) public onlySigner nonReentrant returns (bytes memory) {
