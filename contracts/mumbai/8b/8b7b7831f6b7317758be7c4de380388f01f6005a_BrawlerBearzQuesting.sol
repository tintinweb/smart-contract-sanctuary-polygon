    ...
    IBrawlerBearzQuesting
{
    ...
    uint256[] public shopItemIds;

    // @dev An array of rarities ordered by most rare to least
    uint256[] public shopRarities;

    // @dev An array of shop drop item ids ordered by most rare to least
    uint256[] public crawlerIds;

    // @dev An array of rarities ordered by most rare to least
    uint256[] public crawlerRarities;
    ...
    function initialize() public initializer {
        ...
        requestNonce = 1;
        lastQuestId = 0;
        shopItemIds = [1, 2, 3, 4, 5, 6, 7];
        shopRarities = [2, 7, 18, 30, 50, 75, 999];
        crawlerIds = [1, 2, 3, 4, 5, 6, 7];
        crawlerRarities = [2, 7, 18, 30, 50, 75, 999];
    }
        ...
        onlyRole(QUEST_EXECUTOR_ROLE)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(currentQuest.isActive, "!active || !exists");
        ...
        onlyRole(QUEST_EXECUTOR_ROLE)
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(tokenQuests[tokenId].endAt > block.timestamp, "!finished");
            // Reset token quest data
            uint256 duration = block.timestamp - tokenQuests[tokenId].startAt;

            // Add to rewards to claim
            uint256 xpReward = ((tokenQuests[tokenId].seed / 100) %
                currentQuest.maxXP) + 1;

            uint256 tokenReward = ((tokenQuests[tokenId].seed / 10) %
                currentQuest.maxCredits) + 1;

            Reward memory shopItemReward;

            if (currentQuest.canDropItems) {
                shopItemReward = seedToItemReward(tokenQuests[tokenId].seed);
            }

            Reward memory crawlerzReward;

            if (currentQuest.canFindCrawlerz) {
                crawlerzReward = seedToCrawlerReward(tokenQuests[tokenId].seed);
            }
            ...
            emit QuestEnd(
                ...
                xpReward,
                tokenReward,
                shopItemReward.tokenId,
                crawlerzReward.tokenId
            );

            // Add to rewardsToClaim
            ...
            if (shopItemReward.tokenId > 0) {
                rewardsToClaim[_address].push(shopItemReward);
            }

            if (crawlerzReward.tokenId > 0) {
                rewardsToClaim[_address].push(crawlerzReward);
            }
        }
    }
    ...
    function seedToItemReward(uint256 seed)
        internal
        view
        returns (Reward memory)
    {
        ...
        uint256 randomness = (seed / 1000);
        uint256 chance = (randomness % 100) + 1; // 1 - 100

        if (chance < shopRarities[0]) {
            // LEGENDARY
            itemReward.tokenId = shopItemIds[0];
        } else if (chance < shopRarities[1]) {
            // SUPER RARE ITEM
            itemReward.tokenId = shopItemIds[1];
        } else if (chance < shopRarities[2]) {
            // RARE ITEM
            itemReward.tokenId = shopItemIds[2];
        } else if (chance < shopRarities[3]) {
            // UNCOMMON ITEM
            itemReward.tokenId = shopItemIds[3];
        } else if (chance < shopRarities[4]) {
            // COMMON ITEM 1
            itemReward.tokenId = shopItemIds[4];
        } else if (chance < shopRarities[5]) {
            // COMMON ITEM 2
            itemReward.tokenId = shopItemIds[5];
        } else if (chance < shopRarities[6]) {
            // COMMON ITEM 3
            itemReward.tokenId = shopItemIds[6];
        }
    ...
    function seedToCrawlerReward(uint256 seed)
        internal
        view
        returns (Reward memory)
    {
        ...
        uint256 randomness = (seed / 100000);
        uint256 chance = (randomness % 100) + 1; // 1 - 100

        if (chance < crawlerRarities[0]) {
            // LEGENDARY
            itemReward.tokenId = crawlerIds[0];
        } else if (chance < crawlerRarities[1]) {
            // SUPER RARE ITEM
            itemReward.tokenId = crawlerIds[1];
        } else if (chance < crawlerRarities[2]) {
            // RARE ITEM
            itemReward.tokenId = crawlerIds[2];
        } else if (chance < crawlerRarities[3]) {
            // UNCOMMON ITEM
            itemReward.tokenId = crawlerIds[3];
        } else if (chance < crawlerRarities[4]) {
            // COMMON ITEM 1
            itemReward.tokenId = crawlerIds[4];
        } else if (chance < crawlerRarities[5]) {
            // COMMON ITEM 2
            itemReward.tokenId = crawlerIds[5];
        } else if (chance < crawlerRarities[6]) {
            // COMMON ITEM 3
            itemReward.tokenId = crawlerIds[6];
        }

        itemReward.amount = 1;
        itemReward.typeOf = "CRAWLERZ";
        ...
        returns (QuestMetadata[] memory)
    {
        ...
        for (uint256 j = 0; j < lastQuestId; j++) {
            if (availableQuests[j].isActive) {
        ...
        onlyRole(QUEST_CREATOR_ROLE)
    {
        ...
        availableQuests[nextId] = QuestMetadata({
            ...
            isActive: currentQuest.isActive,
            canDropItems: currentQuest.canDropItems,
            canFindCrawlerz: currentQuest.canFindCrawlerz,
            duration: currentQuest.duration,
            maxCredits: currentQuest.maxCredits,
            maxXP: currentQuest.maxXP
        });
    }
    ...
    /**
     * @notice Sets item config
     * @param _shopItemIds An array of shop drop item ids
     * @param _shopRarities An array of rarities linked to the shop items
     */
    function setQuestItemConfig(
        uint256[] calldata _shopItemIds,
        uint256[] calldata _shopRarities
    ) external onlyRole(QUEST_CREATOR_ROLE) {
        shopItemIds = _shopItemIds;
        shopRarities = _shopRarities;
    }

    /**
     * @notice Sets crawlers config
     * @param _crawlerIds the ids of crawlerz
     * @param _crawlerRarities An array of rarities linked to crawlerz
     */
    function setCrawlersConfig(
        uint256[] calldata _crawlerIds,
        uint256[] calldata _crawlerRarities
    ) external onlyRole(QUEST_CREATOR_ROLE) {
        crawlerIds = _crawlerIds;
        crawlerRarities = _crawlerRarities;
    }

    function setQuestIsActive(uint256 questId, bool _isActive)
        external
        override
        onlyRole(QUEST_CREATOR_ROLE)
    {
        require(questId < lastQuestId, "Invalid quest id");
        availableQuests[questId].isActive = _isActive;
    }
     ...
     */
    function getQuestData(uint256 tokenId)
        external
        view
        override
