...
contract Factory is AccessControl {
    ...
    mapping (string => NFT) nftContractAddress;
    ...
    function deployMintingContract(string memory _name, string memory _symbol, address _minterAddress, address _validatorAddress) public  {
    // function deployMintingContract(string memory _name, string memory _symbol, address _roleAddress) public  {
        require(hasRole(DEPLOYER_ROLE, msg.sender), "0x0f1");
        NFT nftContract = new NFT(_name, _symbol, _minterAddress, _validatorAddress);
        // NFT nftContract = new NFT(_name, _symbol, _roleAddress);
    ...
    function getContractAddress(string memory _contractName) view public returns (address){
...
contract NFT is ERC721URIStorage, EIP712, AccessControl {
    ...
    /**
        * @dev check the user has MINTER_ROLE
        * @dev Error Details
        * - 0x00: User has not MINTER_ROLE
    */
    ...
    constructor(
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _roleAddress);
        grantRole(MINTER_ROLE, _roleAddress, _roleAddress);
        grantRole(VALIDATOR_ROLE, _roleAddress, _validator);
    }

    /**
     * @dev Grant roles
     * @param _role Role
     * @param _account address, Role to be assigned
     * @dev Error Details
        * - 0x01: User is not Admin
     */

    function grantRole(bytes32 _role, address _admin, address _account) public {
        require(isAdmin(_admin), "0x01" );
     ...
     * @param _account address, Role to be assigned
     * @dev Error Details
        * - 0x02: User is not Admin
     */

    function revokeRole(bytes32 _role, address _account) public override {
        require(isAdmin(msg.sender), "0x02" );
        super._revokeRole(_role, _account);
    }

    /**
     * @dev returns bool 
     * @param _account address to check is admin or not
     */
     ...
     * @param marketplace NFT Marketplace contract address
     * @dev Error Details
        * - 0x03: check VALIDATOR_ROLE
        * - 0x04: check VALIDATOR_ROLE
     */
    ...
    function lazyMint(
        ...
        require(hasRole(VALIDATOR_ROLE, verifyValidator), "0x03");
        require(msg.value >= voucher.price, "0X04");
        tokenId++;
        _safeMint(signer, tokenId);
        _setTokenURI(tokenId, voucher.uri);
        _transfer(signer, minter, tokenId);
        IERC20(voucher.token).transferFrom(minter, signer, voucher.price);
        setApprovalForAll(marketplace, true);
        ...
        returns (bytes32)
    {
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256("NFTVoucher(string uri, uint256 price, address token)"),
