...
contract SurveyContract {
    ...
    function submitSurveyResponse(uint _surveyId, string[] memory _answers) public {
        ...
        // Approve the smart contract to spend participant's tokens
        uint tokenReward = surveys[_surveyId].tokenReward;
        require(token.balanceOf(msg.sender) >= tokenReward, "Insufficient custom tokens");

        token.approve(address(this), tokenReward);
