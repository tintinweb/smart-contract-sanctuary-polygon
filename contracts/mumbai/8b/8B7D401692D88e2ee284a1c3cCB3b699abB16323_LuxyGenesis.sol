...
contract LuxyGenesis is ERC721URIStorage, Pauser {
    ...
    function getGenesisCirculantSupply() public view returns (uint) {
        uint genesisCirculantSupply = nativeGenesisSupply + genesisSupply;
        return genesisCirculantSupply;
    }
    ...
    function bridgeMint(
        ...
        _mint(to, id);
        (uint32 id, uint32 level, uint rarity) = abi.decode(bytes(uri), (uint32, uint32, uint));       

        Rarity decodedRarity = Rarity(rarity);
    ...
    function crossChain(
        ...
        string memory _uri = string(abi.encode(_id, metadata[_id].level, metadata[_id].level));
        _burn(_id);
   ...
   /**
    ...
    function tokenURI(
        ...
        bytes memory json = abi.encodePacked(
            ...
            '", ',
            string(description),
            ', "image": "https://ipfs.io/ipfs/QmdnY45NLj8n69rVB8PG6G6q9zXgJv6oq3PBd4QqTiGpe1',
            '"}'
        );
        return
            string(
