...
contract VaultRent is ReentrancyGuard, AccessControl {
    ...
    uint256 private lastTimeCall;

    mapping(address => bool) public contracts;

    constructor(address _tokenInfo, address _token, string memory _name) {
        ...
        setmultisig(0x30268390218B20226FC101cD5651A51b12C07470);
    ...
    function deposit(uint256 _amount,  address _token) external onlyApprovedContract nonReentrant validToken(_token){
    ...
    function setLastTimeCall() public onlyApprovedContract {
        lastTimeCall = block.timestamp;
    }

    function getLastTimeCall() public view onlyApprovedContract returns(uint256) {
        return lastTimeCall;
    }
    ...
    // ONLY FOR TESTNET - DELETE BEFORE DEPLOY TO MAINNET
    /* function updateLastTimeCall(uint256 _lastTime) public {
        if (!hasRole(DEV_ROLE, msg.sender)) {
            revert("have no dev role");
        }
        lastTimeCall = _lastTime;
    } */
