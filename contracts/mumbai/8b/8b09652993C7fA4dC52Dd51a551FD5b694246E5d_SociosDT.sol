...
contract SociosDT is ERC1155Supply,  Ownable {
    ...
    event AmountTransfered(address indexed _to, uint256 indexed _tokenId, uint256 _amount);
    event Redeemed(address indexed _from, uint256 _amount);

    constructor(string memory _name, string memory _symbol) ERC1155("ipfs://QmW7buEn1WiEQvYuTaZVv6zgVfGTXHRNP5rinZC4oQMjMv/") {
        sdtName =_name;
        sdtSymbol = _symbol;
        saleOpen = false;
        redeemAllowed = false;
    ...
    function setAmountEarnedPerFruits(uint256 _sdtIdx, uint256 _amountEarnedPerFruits) external onlyOwner {
        require(exists(_sdtIdx), "SETAMOUNT: Token does not exist");
    ...
    function setLockedTree(uint256 _sdtIdx, bool _locked) external onlyOwner {
        require(exists(_sdtIdx), "LOCK: Token does not exist");
    ...
    function purchase(uint256 _numOfTrees, uint256 _sdtIdx) external payable noReentrant {
        ...
        require (!sdtTreesData[_sdtIdx].locked, "Tree Locked");
    ...
    function withdrawAmountEarnedPerFruits(uint256 _tokenId, address payable _account) external noReentrant{
        uint256 balance = balanceOf(msg.sender, _tokenId);
        require(balance > 0, "Account without tree");
        ...
        require(address(this).balance >= sdtTrees[_tokenId].amountEarnedPerFruits, "Not enough funds");

        payable(_account).transfer(sdtTrees[_tokenId].amountEarnedPerFruits);
        lastStage[msg.sender][_tokenId] = currentStage;
        emit AmountTransfered(msg.sender, _tokenId,  sdtTrees[_tokenId].amountEarnedPerFruits);
    }
