...
contract NPMPriceOracle is IPriceOracle {
  ...
  using OZSafeMath for uint256; // Renamed to fix the namespace conflict with Uniswap
  ...
  constructor(
    ...
    IUniswapV2Pair _pair = IUniswapV2Pair(UniswapV2Library.pairFor(_factory, _npm, _stablecoin));
  ...
  function consult(address token, uint256 amountIn) public view override returns (uint256) {
    ...
    require(token == npm, "Invalid token");
  ...
  function getFairLpPrice(uint256 lpTokens) external view returns (uint256) {
    uint256 totalSupply = pair.totalSupply();
    (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();

    uint256 sqrtK = Babylonian.sqrt(reserve0.mul(reserve1)).mul(2**112).div(totalSupply);

    uint256 px0 = consult(token0, 1);
    uint256 px1 = consult(token1, 1);

    uint256 price = sqrtK.mul(2).mul(Babylonian.sqrt(px0)).div(2**56).mul(Babylonian.sqrt(px1)).div(2**56);
    return lpTokens * price;
  }
}
