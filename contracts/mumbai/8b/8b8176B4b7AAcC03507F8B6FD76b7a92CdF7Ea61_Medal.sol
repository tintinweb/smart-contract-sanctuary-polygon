...
contract Medal is ERC1155, ERC1155Pausable, ERC1155Burnable, Ownable {
    ...
    constructor(
        ...
        uint _totalMedalType) ERC1155(_baseURI) {
        name = "Medal";
        symbol = "MDL";
        _setURI(_baseURI);
    ...
    function changeMedalTypes(uint _totalMedalType) external onlyOwner() whenNotPaused(){
        require(
    ...
    function dropMedal(address[] memory _addresses,uint256[] memory  _quantities,uint256 _id) external onlyOwner() whenNotPaused(){
        require(
    ...
    function mint(address _to,uint256 _id,uint256 _quantity) public onlyOwner() validTokenId(_id) whenNotPaused(){
        require(
            _to != address(0),
    ...
    function batchMint(address _to,uint256[] memory _ids,uint256[] memory _quantities) external onlyOwner() whenNotPaused(){
        require(
            _to != address(0),
            "Medal: enter valid address"
        );
    ...
    function burn(address _account,uint256 _id,uint256 _amount) public override validTokenId(_id) onlyOwner() whenNotPaused(){
        require(
    ...
    function batchBurn(address _account,uint256[] memory _ids,uint256[] memory _quantities) external onlyOwner() whenNotPaused(){
