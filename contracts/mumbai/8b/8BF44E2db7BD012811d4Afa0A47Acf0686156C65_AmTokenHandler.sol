...
// File: contracts\handlers\AmTokenHandler.sol
...
contract AmTokenHandler is IDepositExecute, HandlerHelpers, Ownable, ERC20Safe {
    ...
    address public aaveTokenHandler;
    ...
    function setAaveTokenHandler(address _aaveTokenHandler) external onlyOwner {
        aaveTokenHandler = _aaveTokenHandler;
    }

    function setAmTokenResources(bytes32 aaveTokenResourceId, bytes32 amTokenResourceId) external onlyOwner {
        amTokenResources[aaveTokenResourceId] = amTokenResourceId;
    }
    ...
    function executeProposal(
        ...
        require(IERC20(tokenAddress).balanceOf(address(this)) >= amount, "Not enough amount to deposit");

        IERC20(tokenAddress).approve(AAVE_MUMBAI_LP_ADDR, amount);
        uint256 amountToWithdraw = ILendingPool(AAVE_MUMBAI_LP_ADDR).withdraw(tokenAddress, amount, aaveTokenHandler);
