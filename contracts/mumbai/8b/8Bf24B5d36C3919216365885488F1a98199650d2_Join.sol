...
contract Join is IExecutePlugin, BasePluginWithRules {
    ...
    function execute(
        ...
        (address _communityId, uint256 _subscriptionId) = abi.decode(_data,(address, uint256));

        require(_communityId != address(0), "Join: wrong community");
        checkPlugin(_version, _communityId);

        checkRuleWithNftId(RulesList.COMMUNITY_JOINING_RULES, _communityId, _sender, _subscriptionId);
