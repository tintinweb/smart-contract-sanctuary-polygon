...
interface IBuckMintCarbonOffsets {
...
import '../interfaces/IBuckMintCarbonOffsets.sol';
import '../interfaces/IBuckMintContractRegistry.sol';
import './BaseCarbonTonneStorage.sol';

/// @notice Base Carbon Tonne for Buckmint
/// Contract is an ERC20 compliant token that acts as a pool for BCO2 tokens
/// It is possible to whitelist BuckMint Protocol external tokenized carbon
    ...
    BaseCarbonTonneStorage
{
    ...
    event BuckMintRegistrySet(address ContractRegistry);
    ...
    event BCO2ScoringUpdated(address[] bco2s);
    event AddFeeExemptedBCO2(address bco2);
    event RemoveFeeExemptedBCO2(address bco2);
    ...
    function initialize() external virtual initializer {
        ...
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        __ERC20_init_unchained('BuckMint Protocol: Base Pool Tonne', 'BPT');
    ...
    modifier onlyWithRole(bytes32 role) {
        require(hasRole(role, msg.sender) || owner() == msg.sender, 'Unauthorized');
        _;
    }
    ...
    function setBuckMintContractRegistry(address _address) external virtual onlyOwner {
        ...
        emit BuckMintRegistrySet(_address);
    }
    ...
    function switchMapping(string memory _mappingName, bool accepted) external virtual onlyOwner {
    ...
    /// @notice Function to whitelist selected external non-BCO2 contracts by their address
    /// @param erc20Addr accepts an array of contract addresses
    function addToExternalWhiteList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
    ...
    /// @notice Function to whitelist certain BCO2 contracts by their address
    /// @param erc20Addr accepts an array of contract addresses
    function addToInternalWhiteList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < erc20Addr.length; ++i) {
    ...
    /// @notice Function to blacklist certain BCO2 contracts by their address
    /// @param erc20Addr accepts an array of contract addresses
    function addToInternalBlackList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < erc20Addr.length; ++i) {
    ...
    /// @param erc20Addr accepts an array of contract addresses
    function removeFromExternalWhiteList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < erc20Addr.length; ++i) {
    ...
    /// @notice Function to remove BCO2 addresses from internal blacklist
    /// @param erc20Addr accepts an array of contract addresses
    function removeFromInternalBlackList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < erc20Addr.length; ++i) {
    ...
    /// @notice Function to remove BCO2 addresses from internal whitelist
    /// @param erc20Addr accepts an array of contract addressesc
    function removeFromInternalWhiteList(address[] memory erc20Addr) external onlyOwner {
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < erc20Addr.length; ++i) {
    ...
    function setFeeRedeemPercentage(uint256 _feeRedeemPercentageInBase) external virtual onlyOwner {
        require(_feeRedeemPercentageInBase < feeRedeemDivider, 'Invalid fee percentage');
    ...
    function setFeeRedeemReceiver(address _feeRedeemReceiver) external virtual onlyOwner {
    ...
    function setFeeRedeemBurnPercentage(uint256 _feeRedeemBurnPercentageInBase) external virtual onlyOwner {
        require(_feeRedeemBurnPercentageInBase < feeRedeemDivider, 'Invalid burn percentage');
    ...
    function setFeeRedeemBurnAddress(address _feeRedeemBurnAddress) external virtual onlyOwner {
    ...
    function addRedeemFeeExemptedAddress(address _address) external virtual onlyOwner {
    ...
    function removeRedeemFeeExemptedAddress(address _address) external virtual onlyOwner {
        redeemFeeExemptedAddresses[_address] = false;
    }

    /// @notice Adds a new BCO2 for redeem fees exemption
    /// @param _bco2 BCO2 to be exempted on redeem fees
    function addRedeemFeeExemptedBCO2(address _bco2) external virtual onlyOwner {
        redeemFeeExemptedBCO2s[_bco2] = true;
        emit AddFeeExemptedBCO2(_bco2);
    }

    /// @notice Removes a BCO2 from redeem fees exemption
    /// @param _bco2 BCO2 to be removed from exemption
    function removeRedeemFeeExemptedBCO2(address _bco2) external virtual onlyOwner {
        redeemFeeExemptedBCO2s[_bco2] = false;
        emit RemoveFeeExemptedBCO2(_bco2);
    }
    ...
    /// @notice Determines the minimum vintage start time acceptance criteria of BCO2s
    /// @param _minimumVintageStartTime unix time format
    function setMinimumVintageStartTime(uint64 _minimumVintageStartTime) external virtual onlyOwner {
        minimumVintageStartTime = _minimumVintageStartTime;
        emit MinimumVintageStartTimeUpdated(_minimumVintageStartTime);
    }

    /// @notice Allows MANAGERs or the owner to pass an array to hold BCO2 contract addesses that are
    /// ordered by some form of scoring mechanism
    /// @param bco2s array of ordered BCO2 addresses
    function setBCO2Scoring(address[] calldata bco2s) external onlyWithRole(MANAGER_ROLE) {
        require(bco2s.length > 0, '!bco2s');
        scoredBCO2s = bco2s;
        emit BCO2ScoringUpdated(bco2s);
    }

    /**
     * @notice method to set router address
     * @dev use this method to set router address
     * @param _router address of BuckMintCrosschainMessenger
     */
    ...
    // -------------------------------------
    //   BuckMintCrosschainMessenger functions
    // -------------------------------------
     ...
     * @dev invoked only by the BuckMintCrosschainMessenger (Router)
    ...
    /// @notice Deposit function for BCT pool that accepts BCO2s and mints BCT 1:1
    ...
    /// for each BCO2 separately
    function deposit(address erc20Addr, uint256 amount) external virtual whenNotPaused {
    ...
    function checkEligible(address erc20Addr) public view virtual returns (bool) {
        bool isBuckMintContract = IBuckMintContractRegistry(contractRegistry).checkERC20(erc20Addr);

        if (isBuckMintContract) {
            if (internalWhiteList[erc20Addr]) {
                return true;
            }

            require(internalBlackList[erc20Addr] == false, 'Blacklisted BCO2');

            require(checkAttributeMatching(erc20Addr) == true, 'Non-matching attributes');
        }
        /// @dev If not BuckMint native contract, check if address is whitelisted
        else {
    ...
    function checkAttributeMatching(address erc20Addr) public view virtual returns (bool) {
        ...
        (projectData, vintageData) = IBuckMintCarbonOffsets(erc20Addr).getAttributes();

        /// @dev checks if any one of the attributes are blacklisted.
        /// If mappings are set to "whitelist"-mode, require the opposite
        require(vintageData.startTime >= minimumVintageStartTime, 'Start time too old');
        require(regions[projectData.region] == regionsIsAcceptedMapping, 'Region not accepted');
        require(standards[projectData.standard] == standardsIsAcceptedMapping, 'Standard not accepted');
        require(methodologies[projectData.methodology] == methodologiesIsAcceptedMapping, 'Methodology not accepted');
    ...
    /// @param bco2s Array of BCO2 contract addresses
    /// @param amounts Array of amounts to redeem for each bco2s
    /// @return Total fees amount
    function calculateRedeemFees(address[] memory bco2s, uint256[] memory amounts) external view virtual whenNotPaused returns (uint256) {
        if (redeemFeeExemptedAddresses[msg.sender]) {
            return 0;
        }
        require(bco2s.length == amounts.length, 'Length of arrays differ');
        ...
        //slither-disable-next-line uninitialized-local
        for (uint256 i; i < bco2s.length; ++i) {
    ...
    /// @notice Redeem a whitelisted BCO2 without paying any fees and burn
    /// the BCO2. Initially added to burn HFC-23 credits, can be used in the
    /// future to dispose of any other whitelisted credits.
    /// @dev User needs to approve the pool contract in the BCO2 contract for
    /// the amount to be burnt before executing this function.
    /// @param bco2 BCO2 to redeem and burn
    /// @param amount Amount to redeem and burn
    function redeemAndBurn(address bco2, uint256 amount) external whenNotPaused {
        require(redeemFeeExemptedBCO2s[bco2], 'Not exempted');
        redeemSingle(bco2, amount);
        // User has to approve the pool contract in the BCO2 contract
        // in order for this function to successfully burn the tokens
        IBuckMintCarbonOffsets(bco2).burnFrom(msg.sender, amount);
    }

    /// @notice Redeems Pool tokens for multiple underlying BCO2s 1:1 minus fees
    /// @dev User specifies in front-end the addresses and amounts they want
    /// @param bco2s Array of BCO2 contract addresses
    /// @param amounts Array of amounts to redeem for each bco2s
    /// BCT Pool token in user's wallet get burned
    function redeemMany(address[] memory bco2s, uint256[] memory amounts) external virtual whenNotPaused {
        uint256 bco2Length = bco2s.length;
        require(bco2Length == amounts.length, 'Length of arrays differ');
        ...
        for (uint256 i; i < bco2Length; ++i) {
            ...
            redeemSingle(bco2s[i], amounts[i] - feeAmount);
        }
        if (totalFee != 0) {
    ...
    /// BCO2s from an array of ranked BCO2 contracts
    ...
    function redeemAuto(uint256 amount) external virtual whenNotPaused {
        ...
        uint256 scoredBCO2Len = scoredBCO2s.length;
        while (amount > 0 && i < scoredBCO2Len) {
            address bco2 = scoredBCO2s[i];
            uint256 balance = tokenBalances[bco2];

            // Only BCO2s with a balance should be included for a redemption.
            if (balance != 0) {
                uint256 amountToRedeem = amount > balance ? balance : amount;
                redeemSingle(bco2, amountToRedeem);
    ...
    /// @notice Automatically redeems an amount of Pool tokens for underlying
    /// BCO2s from an array of ranked BCO2 contracts starting from contract at
    ...
    /// contracts.
    /// @param amount Total amount to be redeemed
    /// @return bco2s amounts The addresses and amounts of the BCO2s that were
    /// automatically redeemed
    function redeemAuto2(uint256 amount) external virtual whenNotPaused returns (address[] memory bco2s, uint256[] memory amounts) {
        //slither-disable-next-line uninitialized-local
        uint256 i;
        // Non-zero count tracks BCO2s with a balance
        //slither-disable-next-line uninitialized-local
        ...
        while (amount > 0 && i < scoredBCO2Len) {
            ...
            if (amount == 0) {
                bco2s = new address[](nonZeroCount);
                amounts = new uint256[](nonZeroCount);

                bco2s[nonZeroCount - 1] = bco2;
        ...
        for (uint256 j; j < i - 1; ++j) {
            address bco2 = scoredBCO2s[j];
            // This second loop only gets called when the `amount` is larger
            // than the first bco2 balance in the array. Here, in every iteration the
            // bco2 balance is smaller than the remaining amount while the last bit of
            // the `amount` which is smaller than the bco2 balance, got redeemed
            // in the first loop.
            uint256 balance = tokenBalances[bco2];
            ...
            bco2s[nonZeroCount] = bco2;
            amounts[nonZeroCount] = balance;
            redeemSingle(bco2, balance);
            unchecked {
                ++nonZeroCount;
            }
        }
    ...
    function redeemSingle(address erc20, uint256 amount) internal virtual whenNotPaused {
    ...
    function transfer(address recipient, uint256 amount) public virtual override validDestination(recipient) returns (bool) {
        super.transfer(recipient, amount);
        return true;
    }

    function transferFrom(address sender, address recipient, uint256 amount) public virtual override validDestination(recipient) returns (bool) {
    ...
    function getScoredBCO2s() external view returns (address[] memory) {
        return scoredBCO2s;
    }
}
...
abstract contract BaseCarbonTonneStorageV1 {
    ...
    /// @dev mapping to whitelist external non-BCO2 contracts by address
    mapping(address => bool) public externalWhiteList;

    /// @dev mapping to include certain BCO2 contracts by address,
    /// overriding attribute matching checks
    mapping(address => bool) public internalWhiteList;

    /// @dev mapping to exclude certain BCO2 contracts by address,
...
abstract contract BaseCarbonTonneStorageV1_2 {
    ...
    /// @notice array used to read from when redeeming BCO2s automatically
    address[] public scoredBCO2s;
}

abstract contract BaseCarbonTonneStorageV1_3 {
    /// @notice BCO2s exempted from redeem fees
    mapping(address => bool) public redeemFeeExemptedBCO2s;
}
