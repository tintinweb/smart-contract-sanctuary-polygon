...
struct Item {
    /// @notice what kind of item (post, comment, or poll)
    ItemKind kind;
    ...
    /// @notice Please remove this unused property at the moment of fresh redloy
...
struct AppStorage {
    ...
    mapping(uint256 => mapping(uint256 => bytes32)) groupUsers;

    /// @dev maps item id to Poll
    mapping(uint256 => Poll) polls;

    mapping(uint256 => mapping(uint256 => uint256)) groupUserNotes;
}
...
contract DiamondInit {    
    ...
    function init(address semaphoreAddress, address _verifier, address[] memory _admins, address[] memory _moderators, Unirep _unirep) external {
        ...
        appStorage.repRequirement.comment = 0;
