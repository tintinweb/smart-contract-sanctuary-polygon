...
contract ScholarMarket is SMAuth, ReentrancyGuard {
    ...
    function hireScholar(address wallet, 
        ...
        uint256 stopLoss 
            ...
            //scholarManagerLimit[msg.sender][wallet]=1;
    ...
    function respondOffer(uint256 nftId, uint256 response) public {
    ...
    function cancelOffer(uint256 nftId) public {
    ...
    function settleNFT(uint256 nftId) public {
    ...
    function updateMatch(address wallet, uint256 nftId, bool won) public onlyAuth {
        require(nftManager[nftId].scholar == wallet);
        uint256 smp_ = scholar[wallet].matchesPlayed;
        uint256 smw_ = scholar[wallet].matchesWon;
        if(won==true) {
        scholar[wallet] = Scholar(smp_, smw_++);
        nftManager[nftId].won++;
    ...
    function startMatch(address wallet, uint256 nftId, uint256 matchid) public nonReentrant {
        require(matchData[matchid].matchFee != 0);
        require(isScholarAllowed(wallet, nftId) == true);
        scholar[wallet].matchesPlayed++;
        nftManager[nftId].matchesPlayed++;
    ...
    function createMatch() public onlyAuth returns(uint256 matchId) {
    ...
    function rewardDistribution(uint256[] memory  nftid, uint256 matchId) public nonReentrant onlyAuth {
    ...
    function getReward() public nonReentrant {
        require(block.timestamp - rewards[msg.sender].time < 7 days, "Already claimd once in a week");
    ...
    function setfundingWallet( address wallet) external onlyAuth {
