...
contract POAPtest is ERC721, Pausable, ERC721Enumerable, AccessControl {
    ...
    function takeTest(string memory _testAnswers) public{
        require(alreadyWon[_msgSender()]==false,"already won");
        if(keccak256(abi.encodePacked(testAnswers))==keccak256(abi.encodePacked(_testAnswers))){
            safeMint(_msgSender());
            alreadyWon[_msgSender()]=true;
            testWinners.push(_msgSender());
        }
    }
