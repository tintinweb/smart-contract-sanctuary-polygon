...
// imports
import "hardhat/console.sol"; //Allows console.log
import "@openzeppelin/contracts/utils/Strings.sol";
import "@solvprotocol/erc-3525/ERC3525.sol";

contract EmissionsToken is ERC3525 {

    // TokenURIs
    mapping (uint256 => string) private _tokenURIs;

    // Update event for when tokenURI is updated
    event Update(string indexed field, string indexed oldData, string indexed newData);
    event UpdateRecord(address indexed user, uint256 indexed id);
    event Mint(address indexed recipient, uint256 indexed newTokenID);
    ...
    // Mapping of ids to SlotData
    mapping(uint256 => SlotData) _slotData;

    // Constructor
    constructor() ERC3525("EmissionsToken", "EmiT", 4) {}

    /* 
        * Returns slot data as object
        * @param slotId_ id of slot
        * @returns the SlotData object
    */
    function slotURIdata(uint256 slotId_) public view returns(SlotData memory) {
        return _slotData[slotId_];
    }

    /* 
        * Returns slot data as object, overrids slotURI function from 3525
        * @param slotId_ id of slot
        * @returns slot data as a string
    */
    function slotURI(uint256 slotId_) public view override returns(string memory) {
        return string(
            abi.encodePacked(
                '{ company: ',
                _slotData[slotId_].company,
                ', activity: ',
                _slotData[slotId_].activity,
                ', quality: ',
                Strings.toString(_slotData[slotId_].quality),
                '}'
            ));
    }

    /*
        * Updates metadata for a Slot
        * @param slotId_ id of slot being updated
        * @param company company in control of slot
        * @param activity activity associated with slot
        * @param quality data quality score of the slot
    */
    function updateSlotData(uint256 slotId_, string memory company, string memory activity, uint256 quality) external {
        emit UpdateRecord(msg.sender, slotId_);
        emit Update("Slot URI - Company", _slotData[slotId_].company, company);
        _slotData[slotId_].company = company;
        emit Update("Slot URI - Activity", _slotData[slotId_].activity, activity);
        _slotData[slotId_].activity = activity;
        emit Update("Slot URI - Quality", Strings.toString(_slotData[slotId_].quality), Strings.toString(quality));
        _slotData[slotId_].quality = quality;
    }

    /* 
        * Mint a new Emissions token
        * @param to_ the recipient address
        * @param tokenId_ ID of new token
        * @param slot_ slot of the token (Ideally address of the minter or recipient)
        * @param value_ value of token in mg
        * @param uri_ URI of token metadata
    */
    function mint( address to_, uint256 tokenId_, uint256 slot_, uint256 value_, string memory uri_) public {
        _mint(to_, tokenId_, slot_, value_);
        updateURI(tokenId_, uri_);
    }

    /* 
        * Get Emissions tokenURI
        * @param tokenId_ ID of token retrieving URI for
        * @return URI of token
    */
    ...
    /*
        * Update token URI
        * @param tokenId_ ID of token being updated
        * @param uri_ new URI for token
    */
    function updateURI(uint256 tokenId_, string memory uri_) public {
        _requireMinted(tokenId_);
        emit UpdateRecord(msg.sender, tokenId_);
        emit Update("Token URI", tokenURI(tokenId_), uri_);
