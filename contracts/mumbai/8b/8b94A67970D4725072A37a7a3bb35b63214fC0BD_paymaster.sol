...
contract paymaster is BasePaymaster {
    ...
    function preRelayedCall(GsnTypes.RelayRequest calldata relayRequest, bytes calldata signature, bytes calldata approvalData, uint256 maxPossibleGas ) external override virtual returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        require(relayRequest.request.to == proxy, "not support");
        emit relaycall(relayRequest.request.to);
        return ("", false);
    }
    function postRelayedCall(bytes calldata context,bool success, uint256 gasUserWithOutPost,GsnTypes.RelayData calldata relayData ) external relayHubOnly override {
        (context, success, gasUserWithOutPost, relayData);
    }
}