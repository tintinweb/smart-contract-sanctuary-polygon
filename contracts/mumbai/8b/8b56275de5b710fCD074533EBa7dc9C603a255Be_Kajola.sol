...
contract Kajola is Ownable {
    ...
    function addSyndicate(string memory _name, uint256 _lotteryType, uint256 _ticketPrice, uint256 _multiplier, uint256 _tableFee, uint256 _expiryTime) public onlyOwner {
        ...
        require(_lotteryType >= 2 && _lotteryType <= 5, "Lottery types should be from 2 to 5." );
    ...
    function joinSyndicate(address _player, uint256 _syndicateId, uint256[] memory _numbers) public onlyOwner {
        uint256 totalSyndicatesCount = _syndicateCount.current();
        ...
        if(_syndicate.ticketsByPlayer[_player].length == 0) {
            _syndicate.players.push(_player);            
        }

        uint256 totalTicketsCount = _syndicate._ticketCount.current();
        _syndicate.ticketsByPlayer[_player].push(totalTicketsCount);
