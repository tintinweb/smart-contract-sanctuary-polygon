...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    function mintNFT(
        ...
        uint256 newTokenId = doMintNFT(recipient, uri, note.package);
    ...
    function readSharedNote(uint256 tokenId) public payable returns (string memory noteSmd) {
        require(_exists(tokenId), "RehideNFT: Token not found");

        Note storage note = _notesMapping[tokenId];

        // _noteReadAllowlist
        require((bytes(note.smd).length != 0), "RehideNFT: Data for note not found (empty)");
