...
contract FabweltNftM is ERC1155, Ownable {
  ...
  function createToken(uint _amount, string memory tokenURI) public returns (uint256) {
        ...
        _mint(msg.sender,newTokenId,_amount,"");
         setURIInternal(newTokenId, tokenURI); // set the tokenURI to the tokenId.
  ...
  function setURIInternal(uint _id, string memory _uri) internal {
    tokenURI[_id] = _uri;
    emit URI(_uri, _id);
  }
