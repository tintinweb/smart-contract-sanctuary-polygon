...
contract SkizZONFT is IERC721, Ownable {
    ...
    address internal preSaleToken;
    bool public publicSale = false;
    bool public preSale = false;
    
    uint private mintPrice1 = 1 ether; // in brackets of 1000 ski. prezzonft
    uint private mintPrice2 = 2 ether;
    uint private mintPrice3 = 3 ether;
    ...
    constructor(address payable _marketer, address payable _developer, address _presaletoken, string memory _imageHash) {
  ...
  function isWhitelisted(address _wallet) public view returns(bool) {
    IERC721 token = IERC721(preSaleToken); // token is cast as type IERC721
    ...
    function PublicSaleState(bool _isRunning) external onlyOwner {
        saleStartTime = block.timestamp;
        publicSale = _isRunning;
        if (publicSale == true) {preSale = false;}
        emit SaleChange();
    }


        function PreSaleState(bool _isRunning) external onlyOwner {
        saleStartTime = block.timestamp;
        preSale = _isRunning;
        if (preSale == true) {publicSale = false;}
        emit SaleChange();
    }
    ...
    function mintsRemaining() external view returns (uint) {
        return token_limit.sub(numTokens); //ski. aggiunto submarketingmints
    }
    ...
    function calcMintPrice(uint numberOfNfts) public view returns (uint) {
        ...
        for(uint c = 1; c<=(numberOfNfts); c++ ){
            if (numTokens < (token_limit.mul(33)).div(100)){
                _cost = _cost.add(mintPrice1);}
            else if (numTokens < (token_limit.mul(66)).div(100)) { 
                _cost = _cost.add(mintPrice2);
            } else { _cost = _cost.add(mintPrice3); }
    ...
    function setPrices(uint _mntP1, uint _mntP2, uint _mntP3 ) external onlyOwner {
    require(!contractSealed, "Contract sealed.");
    mintPrice1 = _mntP1;
    mintPrice2 = _mntP2;
    mintPrice3 = _mntP3;
