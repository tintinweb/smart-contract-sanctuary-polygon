...
contract HST {
    ...
    constructor() public {
        signer = msg.sender;
        packages[25].isExists = true;
        packages[50].isExists = true;
        packages[100].isExists = true;
        packages[150].isExists = true;
        packages[200].isExists = true;
    }
    ...
    function buy(uint256 _package) public security{
        require(packages[_package.div(1e18)].isExists==true,"Invalid Package.");
    ...
    // @dev Add/Remove packages on this contract
    function packageSettlement(uint8 _package, uint8 _isExists) external onlySigner security{
        bool isExists = (_isExists==1)?true:false;
        packages[_package].isExists = isExists;
    }
