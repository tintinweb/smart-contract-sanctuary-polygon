...
contract RUFFRAFFLE {
    ...
    function createRaffle(
        ...
        newRaffle.nftContract = nftContract;
        ...
        emit RaffleCreated(raffleId, msg.sender, nftContract, tokenContract, tokenId, ticketPrice, maxTickets);
    }
    ...
    function selectWinner(uint256 raffleId) internal onlyRaffleCreator(raffleId) {
        ...
        transferNFT(raffle.nftContract, raffle.tokenId, winner); // Передаем NFT победителю
    ...
    function purchaseTicketsWithTokens(uint256 raffleId, uint256 numTickets, address tokenAddress) external {
        ...
        uint256 tokenDecimals = supportedTokens[tokenAddress][admin];
        require(tokenDecimals > 0, "Token not supported.");
    ...
    function withdrawAllTokens(address recipient) external onlyAdmin {
        ...
        uint256 balance = IERC20(admin).balanceOf(address(this));
        IERC20(admin).transfer(recipient, balance);
    ...
    function withdrawServiceFee(address payable recipient) external onlyAdmin {
        require(recipient != address(0), "Invalid recipient address.");

        uint256 balance = address(this).balance;
        uint256 serviceFee = balance.mul(serviceFeePercentage).div(100);

        recipient.transfer(serviceFee);

        emit ServiceFeeWithdrawn(recipient, serviceFee);
    }

    function transferNFT(address nftContract, uint256 tokenId, address recipient) internal {
