...
contract CSMRouter is ICSMRouter, AccessControl, ReentrancyGuard {
    ...
    function lzReceive(
        ...
        address _srcAddress = bytesToAddress(_srcAddressBytes);
        console.log("_srcAddress ", _srcAddress);
        console.logBytes(_srcAddressBytes);
        // require(_approvedAssetIds[_srcAddress][_srcChainId] > 0, "SRC_CHAIN_NOT_APPROVED");
        require(
            _approvedRouters[_srcChainId] == address(_srcAddress),
            string.concat("SRC_CHAIN_NOT_APPROVED_", string(abi.encodePacked(address(_srcAddress))))
        );
        (
            address sender,
            uint16 srcChainId,
    ...
    function togglePoolPerAssets(address _asset, IPoolCrossChain _pool) external onlyRole(DEFAULT_ADMIN_ROLE) {
        _poolsPerAssets[_asset] = _pool;
    }
