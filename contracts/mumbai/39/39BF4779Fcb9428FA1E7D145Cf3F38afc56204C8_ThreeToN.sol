...
contract ThreeToN {
    ...
    function start(string memory sid, int96 rate, ISuperfluidToken token) external {
        // todo: assert EOA
    ...
    function hasJoined(string memory sid) external view returns (bool) {
        ...
        (timestamp, flowRate, deposit, owedDeposit) = cfaV1.cfa.getFlow(data.token, msg.sender, data.owner);
