...
// ReentrancyGuard, 
contract PharoReservePool is IReservePool, PharoEmits, AccessControl, Ownable
{
    ...
    constructor(address pharoTokenAddress, address newOwner) {
                     ...
                     address asset) public
    {
        ...
        emit ReservePoolDeposit(amount, msg.sender);
    }
    ...
    // todo: access control...
    // nonReentrant
    function withdraw(uint256 pharoId, 
                      uint256 amount,
                      string memory memo) public
    {
        // get the right pool
        PoolInfo storage pool = pharoIdToPoolInfo[pharoId];
        require(amount >= pool.balance, "you dont have that much");
        pool.balance = pool.balance.sub(amount);

        IERC20(pool.asset).safeTransfer(msg.sender, amount);

        emit ReservePoolWithdraw(amount, msg.sender, memo);
    }
}
