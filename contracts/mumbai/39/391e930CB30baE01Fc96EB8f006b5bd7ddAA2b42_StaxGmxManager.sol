...
contract StaxGmxManager is IStaxGmxManager, Ownable, Operators, Pausable {
    ...
    function buyStxGlpQuote(
    ...
    ) external view override returns (uint256 stxGlpAmountOut, uint256[] memory investFeeBps, bytes memory otherQuoteParams) {
        ...
        if (_amount == 0) return (stxGlpAmountOut, investFeeBps, otherQuoteParams);
        ...
        (investFeeBps[0], usdgAmountOut) = buyUsdgQuote(_amount, _token);

        stxGlpAmountOut = (aumInUsdg == 0) ? usdgAmountOut : usdgAmountOut * glpSupply / aumInUsdg;
        otherQuoteParams = abi.encode(usdgAmountOut);
    }
    ...
    /// otherQuoteParams are unused for a sale quote.
    function sellStxGlpQuote(
        uint256 _stxGlpAmount,
        address _toToken
    ) external view returns (uint256 tokenAmountOut, uint256[] memory exitFeeBps, bytes memory otherQuoteParams) {
        // GMX.io don't provide on-contract external functions to obtain the quote. Logic extracted from:
        // https://github.com/gmx-io/gmx-contracts/blob/83bd5c7f4a1236000e09f8271d58206d04d1d202/contracts/core/GlpManager.sol#L183

        exitFeeBps = new uint256[](2);  // [STAX's exit fee, GMX's exit fee]
        exitFeeBps[0] = sellFeeRate.asBasisPoints();
        (, uint256 glpAmount) = sellFeeRate.split(_stxGlpAmount);

        if (glpAmount == 0) return (tokenAmountOut, exitFeeBps, otherQuoteParams);
