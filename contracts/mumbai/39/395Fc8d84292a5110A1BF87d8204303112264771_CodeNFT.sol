...
contract CodeNFT is ERC721URIStorage, Ownable {
        ...
        returns (uint256)
    {
        ...
        require(tokenURIbytes.length != 0, "token URI is not set");

        _tokenIds.increment();

        uint256 newItemId = _tokenIds.current();
        _safeMint(recipient, newItemId);
        _setTokenURI(newItemId, string(abi.encodePacked(tokenURI, newItemId.toString(), tokenURIend)));
        return newItemId;
    }
