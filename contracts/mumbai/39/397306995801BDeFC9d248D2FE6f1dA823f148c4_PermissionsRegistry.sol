...
/*
    This contract handles the accesses to the various Pearl contracts.
*/
...
contract PermissionsRegistry {
    ...
    function addRole(string memory role) external onlyMultisig {
    ...
    function removeRole(string memory role) external onlyMultisig {
        bytes memory _role = bytes(role);
    ...
    function setRoleFor(address c, string memory role) external onlyMultisig {
        bytes memory _role = bytes(role);
        require(_checkRole[_role], 'not a role');
    ...
    function removeRoleFrom(address c, string memory role) external onlyMultisig {
        bytes memory _role = bytes(role);
        require(_checkRole[_role], 'not a role');
    ...
    *************************************************************/
    ...
    function setEmergencyCouncil(address _new) external {
        require(msg.sender == emergencyCouncil || msg.sender == multisig);
