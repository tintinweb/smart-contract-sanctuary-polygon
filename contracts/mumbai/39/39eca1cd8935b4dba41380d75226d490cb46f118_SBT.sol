...
contract SBT is ERC721, Signer {
  ...
  ) ERC721(_name, _symbol) Signer(_signer) { 
   ...
   *
   * Requirements:
   * - `_sig` must be signed off signer wallet
   * @param _sig signature
   */
  function mint(bytes calldata _sig) external {
    ...
    emit LogMinted(msg.sender, newTokenId);
  }

  /**
   * Override {ERC721:_beforeTokenTransfer}
   */
  function _beforeTokenTransfer(
    address _from,
    address _to,
