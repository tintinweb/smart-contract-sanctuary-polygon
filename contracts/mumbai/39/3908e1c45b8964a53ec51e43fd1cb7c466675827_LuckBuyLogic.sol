...
contract LuckBuyStorage {
  ...
  mapping (address=>LuckBuyUserLog[]) private userLogs; 
  mapping (address=>LuckBuyInviteLog[]) private inviteLogs; 
  ...
  function getInviteLogs(address user, uint page, uint size) public view returns (LuckBuyInviteLog[] memory, uint256) {
    uint256 start; 
    uint256 end;
    uint tag = 0;
    if (inviteLogs[user].length > (page+1) * size) { 
      start = inviteLogs[user].length - (page+1) * size;
      end = inviteLogs[user].length - page * size;
    } else {
      start = 0;
      if (inviteLogs[user].length > page * size) {
        end = inviteLogs[user].length - page * size;
      } else {
        end = 0;
      }
    }
    LuckBuyInviteLog[] memory logs = new LuckBuyInviteLog[](end - start);
    for(uint i = end; i > start; i--) {
      logs[tag] = inviteLogs[user][i - 1];
      tag++;
    }
    return (logs, inviteLogs[user].length);
  }
  ...
  function addItemLog(uint256 itemID, address winner, uint256 quantity) private {
  ...
  function addUserLog(address user, uint256 itemID, bool finished, bool win) private {
    userLogs[user].push(
      LuckBuyUserLog(
        itemID,
        items[itemID].tokenContract,
        items[itemID].startTime,
        items[itemID].buyQuantity[user],
        finished,
        win,
        items[itemID].bonusAmount
      )
  ...
  function addInviteLog(address user, uint256 itemID, uint256 reward) private {
    inviteLogs[user].push(
      LuckBuyInviteLog(
        itemID,
        items[itemID].tokenContract,
        items[itemID].startTime,
  ...
  function rewardItem(uint256 itemID,uint256 winnerIndex, address platform) public onlyLogicContract returns (bool, address, uint256, uint256) {
    ...
    for (uint256 i = 0; i < items[itemID].rewardUserSets.length; i++) {
      ...
      addInviteLog(user, itemID, amount); 
      balance -= amount;
    }
    ...
    addItemLog(itemID, winner, quantity);
...
contract LuckBuyLogic is ReentrancyGuardUpgradeable {
  ...
  event ItemSettle(uint256 indexed itemID,  uint256 startTime, address winner, uint256 quantity, uint256 bonusAmount);

  
  function storageAddress() internal pure returns (address) {
    return 0x38B6f8F3942aEb40DCBE0a853352fE9f323701AA;
  }
  ...
  function getKingAddress() internal pure returns (address) {
    return  0xc28E0E7e2Ef94e75e41fB82cA42b1600c47dE360;
  }
  ...
  function getItemTemplates() public pure returns (LuckBuyItemTemplate[] memory) {
    LuckBuyItemTemplate[] memory items = new LuckBuyItemTemplate[](12);
    items[0] = LuckBuyItemTemplate(1, address(0x0), 5 ether, 4.75 ether,1 ether, true, 1 days, true);
    items[1] = LuckBuyItemTemplate(2, address(0x0), 10 ether, 9.5 ether,1 ether, true, 20 seconds, true);
    items[2] = LuckBuyItemTemplate(3, address(0x0), 20 ether, 19 ether,1 ether, true, 1 days, true);
    items[3] = LuckBuyItemTemplate(4, address(0x0), 50 ether, 47.5 ether, 1 ether, true, 1 days, true);
    items[4] = LuckBuyItemTemplate(5, address(0x0), 100 ether, 95 ether, 1 ether, true, 1 days, true);
    items[5] = LuckBuyItemTemplate(6, address(0x0), 200 ether, 19 ether, 1 ether, true, 1 days, true);
    items[6] = LuckBuyItemTemplate(7, address(0x0), 500 ether, 475 ether, 1 ether, true, 1 days, true);
    items[7] = LuckBuyItemTemplate(8, address(0x0), 1000 ether, 950 ether, 1 ether, true, 1 days, true);
    items[8] = LuckBuyItemTemplate(9, address(0x0), 2000 ether, 1900 ether, 1 ether, true, 1 days, true);
    items[9] = LuckBuyItemTemplate(10, address(0x0), 5000 ether, 4750 ether, 1 ether, true, 1 days, true);
    items[10] = LuckBuyItemTemplate(11, address(0x0), 0.05 ether, 0.0475 ether, 0.01 ether, true, 1 days, true);
    items[11] = LuckBuyItemTemplate(12, address(0x0), 0.005 ether, 0.00475 ether, 0.001 ether, true, 1 days, true);
  ...
  function initItems() public {
    ...
    for (uint256 i = 0; i < itemArray.length; i++) {
      ...
      if(! storageContract().checkItemExist(item.itemID) && item.online) { 
        assert(storageContract().addItem(item.itemID, item.tokenContract, item.totalAmount, item.bonusAmount, item.price, item.period));
      } else if (!item.online) { 
  ...
  function getItemLogs(uint256 itemID, uint page, uint size) public view returns (LuckBuyItemLog[] memory itemArray, uint256 total) {
    return storageContract().getItemLogs(itemID, page, size);
  }

  
  function getInviteLogs(address player, uint page, uint size) public view returns (LuckBuyInviteLog[] memory itemArray, uint256 total) {
    return storageContract().getInviteLogs(player, page, size);
  }
  ...
  function buyItem(uint256 itemID, uint256 quantity, address inviter) public payable {
    ...
    if (inviter != address(0)) { 
      assert(storageContract().addRewardUser(itemID, inviter, amount * getInviteRate() / getBaseRate()));
    }
  ...
  function settleItems() public nonReentrant {
    ...
    for (uint256 i = 0; i < itemArray.length; i++) {
      ...
      uint256 quantity;
      uint256 bonus;
      uint256 itemStartTime = item.startTime;
      if(item.finished) {
        ...
        (result, winnerAddress, quantity, bonus) = storageContract().rewardItem(item.itemID, winnerIndex, getKingAddress());
        assert(result);
        
        emit ItemSettle(item.itemID, itemStartTime, winnerAddress, quantity, bonus);
