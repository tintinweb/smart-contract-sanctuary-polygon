...
contract AuctionWithAdmin is Ownable {
    ...
    function getAuction(uint256 _tokenId)
        ...
        view
        virtual
        returns (AuctionDetails memory, address seller)
    {
        return (auction[_tokenId], EnumerableMap.get(auctionId, _tokenId));
    }
    ...
    function dealDetails(uint256 index) public view returns (DealDetails memory dealInfo,  address seller, uint256 tokenId){
        (uint256 id,) = EnumerableMap.at(dealId, index);
        return (deal[id], EnumerableMap.get(dealId, id), id);
    }
    ...
    function auctionDetails(uint256 index) public view returns (AuctionDetails memory auctionInfo, address seller, uint256 tokenId){
        (uint256 id,) =  EnumerableMap.at(auctionId, index);        
        return (auction[id], EnumerableMap.get(auctionId, id), id);
    }
    ...
    function dealDetailsByTokenId(uint256 tokenId) public view returns (DealDetails memory dealInfo, address seller){             
        return (deal[tokenId], EnumerableMap.get(dealId, tokenId));
    }
