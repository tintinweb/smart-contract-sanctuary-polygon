...
contract BatchBet is ERC1155Holder {
    IERC20 public immutable token;

    event FPMMBatchBuy(address indexed buyer, address[] markets, uint256[] investmentAmount);
    event FPMMSingleBuy(
    ...
    function getPositionId(bytes32 _conditionId, uint256 _outcomeIndex) internal view returns (uint256) {
        uint256 indexSet = 1 << _outcomeIndex;
        bytes32 collectionId = getCollectionId(0, _conditionId, indexSet);

        return CTHelpers.getPositionId(token, collectionId);
    }

    function batchBuy(
        address[] calldata _markets,
        uint256[] calldata _investmentAmount,
        uint256[] calldata _outcomeIndex,
        uint256[] calldata _minOutcomeTokensToBuy
    ) external {
        uint256 length = _markets.length;
        require(
            length > 0 &&
                _investmentAmount.length == length &&
                _outcomeIndex.length == length &&
                _minOutcomeTokensToBuy.length == length,
            "invalid input"
        );

        for (uint256 i = 0; i < length; i++) {
            address market = _markets[i];
            uint256 investmentAmount = _investmentAmount[i];
            uint256 outcomeIndex = _outcomeIndex[i];
            IFixedProductMarketMaker fpmm = IFixedProductMarketMaker(market);

            token.transferFrom(msg.sender, address(this), investmentAmount);
            token.approve(market, investmentAmount);

            address conditionalToken = fpmm.conditionalTokens();
            bytes32 conditionId = fpmm.conditionIds(0);
            uint256 positionId = getPositionId(conditionId, outcomeIndex);
            uint256 outcomeTokensToBuy = fpmm.calcBuyAmount(investmentAmount, outcomeIndex);

            fpmm.buy(investmentAmount, outcomeIndex, _minOutcomeTokensToBuy[i]);
            IERC1155(conditionalToken).safeTransferFrom(address(this), msg.sender, positionId, outcomeTokensToBuy, "");

            emit FPMMSingleBuy(msg.sender, market, investmentAmount, outcomeIndex, outcomeTokensToBuy);
        }

        emit FPMMBatchBuy(msg.sender, _markets, _investmentAmount);
    }
}
...
library CTHelpers {
    ...
    /// @param outcomeSlotCount The number of outcome slots which should be
    /// used for this condition. Must not exceed 256.
...
interface IFixedProductMarketMaker {
    ...
    function conditionIds(uint256 outcomeIndex) external view returns (bytes32);
