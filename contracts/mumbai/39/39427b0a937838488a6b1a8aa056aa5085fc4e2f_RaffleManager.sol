    ...
    ReentrancyGuard
{
    ...
    function onTokenTransfer(
        ...
        if (data.length == 32) {
            ...
            string memory name = string(
                abi.encodePacked("Raffle ", Strings.toString(_id))
            );
            _registerAutomation(
                name,
    ...
    function getRaffle(
        uint256 raffleId
    ) external view returns (RaffleInstance memory) {
    ...
    function getUserEntries(
        ...
        return userEntries[user][raffleId];
    }
     ...
     *
     */
    function updateRaffleOwner(
        uint256 raffleId,
        address newAdmin
    ) external onlyRaffleOwner(raffleId) {
    ...
    function withdrawLink(
        uint256 raffleId
    ) external onlyRaffleOwner(raffleId) nonReentrant {
        require(
            ...
            if (
                ...
                if (
                    ...
                    if (
                        ...
                        raffles[raffleId].winners.length
                    ) {
                        raffles[raffleId].raffleState = RaffleState.CANCELLED;
                        emit RaffleCancelled(raffleId);
                    } else {
