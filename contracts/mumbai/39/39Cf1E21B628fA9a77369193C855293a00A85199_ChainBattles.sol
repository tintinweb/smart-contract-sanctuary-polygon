...
contract ChainBattles is ERC721URIStorage  {
    ...
    mapping(uint256 => NPC) public tokenIdToLevels;
...
function generateCharacter(uint256 tokenId) public view returns(string memory){
    ...
    bytes memory svg = abi.encodePacked(
        ...
        '<text x="50%" y="50%" class="base" dominant-baseline="middle" text-anchor="middle">', "Strength: ",getStr(tokenId),'</text>',
...
function getLevel(uint256 tokenId) public view returns (string memory) {
    NPC memory _npc = tokenIdToLevels[tokenId];
    return _npc.level.toString();
}

function getSpeed(uint256 tokenId) public view returns (string memory) {
    NPC memory _npc = tokenIdToLevels[tokenId];
    return _npc.speed.toString();
}
function getStr(uint256 tokenId) public view returns (string memory) {
    NPC memory _npc = tokenIdToLevels[tokenId];
    return _npc.strength.toString();
}
function getLife(uint256 tokenId) public view returns (string memory) {
    NPC memory _npc = tokenIdToLevels[tokenId];
    return _npc.life.toString();
}
...
function mint() public {
    ...
    NPC storage _npc = tokenIdToLevels[newItemId];
    _npc.level = 1;
    _npc.speed = 3;
    _npc.strength = 5;
    _npc.life = 2;
 ...
 function train(uint256 tokenId) public {
   ...
   require(ownerOf(tokenId) == msg.sender, "You must own this NFT to train it!");
    ...
    NPC storage _npc = tokenIdToLevels[tokenId];

    uint256 currentLevel = _npc.level;
    _npc.level = currentLevel + 1;

    uint256 currentSpeed = _npc.speed;
    _npc.speed = currentSpeed + getRandomNumber(currentSpeed);

    uint256 currentStr = _npc.strength;
    _npc.strength = currentStr + getRandomNumber(currentStr);

    uint256 currentLife = _npc.life;
    _npc.life = currentLife + getRandomNumber(currentLife);
