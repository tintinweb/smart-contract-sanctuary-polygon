...
interface IERC721 {
    ...
    function mint(address to, uint256 tokenId, uint256 numberOfShares) external;
}

contract BuySharesNFT is Ownable {
    ...
    event  BuyNFT(address indexed user, uint256 tokenID, uint256 numberOfShares);

    constructor(IERC721 _NFTContract, uint256 _pricePerShare) {
    ...
    function buyNFT() lock public payable {
        require(msg.value.mod(pricePerShare) == 0, "The amount of MATIC is incorrect.");

        uint256 salesVolumeAdd = msg.value.div(pricePerShare);
    ...
    function buyMinNFT() lock public payable {
        require(msg.value.mod(pricePerShare) == 0, "The amount of MATIC is incorrect.");
        uint256 creatNumber = msg.value.div(pricePerShare);
        require(creatNumber > 0, "You need pay enough MATIC.");
    ...
    function buyMidNFT() lock public payable {
        require(msg.value.mod(pricePerShare * 10) == 0, "The amount of MATIC is incorrect.");
        uint256 creatNumber = msg.value.div(pricePerShare * 10);
        require(creatNumber > 0, "You need pay enough MATIC.");
    ...
    function buyMaxNFT() lock public payable {
        require(msg.value.mod(pricePerShare * 100) == 0, "The amount of MATIC is incorrect.");
        uint256 creatNumber = msg.value.div(pricePerShare * 100);
        require(creatNumber > 0, "You need pay enough MATIC.");
    ...
    function setPricePerShare(uint256 _pricePerShare) public onlyOwner() {
        pricePerShare = _pricePerShare;
    }
