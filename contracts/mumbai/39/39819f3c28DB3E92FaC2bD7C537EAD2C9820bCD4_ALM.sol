...
contract ALM is ERC20, Ownable, ReentrancyGuard {
    ...
    */
    function airDropReward() external onlyWhenAirDropIsNotFinished nonReentrant returns(bool isComplete){
    ...
    */
    function getUserRewardAmount(uint256 unclaimedRewardAmount) private view returns(uint256 amountOwed){
    ...
    */
    function getRewardAmountToTransfer(address user) private returns(uint256 amountToTransfer){
        ...
        if(userClaimedRewardMapping[user][address(currentRewardToken)].totalRewardMapping < totalRewardMapping[address(currentRewardToken)]){
            amountToTransfer = getUserRewardAmount(balanceOf(user));
        }else{
            // Claim unclaimedRewards on user struct if already claimed
            amountToTransfer = getUserRewardAmount(userClaimedRewardMapping[user][address(currentRewardToken)].unclaimedReward);
