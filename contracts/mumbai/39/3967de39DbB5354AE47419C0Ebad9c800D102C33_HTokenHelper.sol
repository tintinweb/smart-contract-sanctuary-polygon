...
contract HTokenHelper is Ownable {
  ...
  function getMarketOraclePrices(HTokenI _hToken) external view returns (uint256, uint256) {
    ...
    uint256 underlyingPriceInUSD = uint256(priceOracle.getUnderlyingPriceInUSD(_hToken.underlyingToken(), _hToken.decimals()));
    uint256 ethPrice = uint256(priceOracle.getEthPrice(_hToken.decimals()));
