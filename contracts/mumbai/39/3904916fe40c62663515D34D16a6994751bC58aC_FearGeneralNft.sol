...
contract FearNft1155 is ERC1155, AccessControl, OpenSeaCompatible, NativeMetaTransaction, ContextMixin
{
    ...
    constructor(string memory name, string memory ticker, string memory domainName, uint64 _mintingChainId) ERC1155(name, ticker, domainName) {
    ...
    */
    function initialMint(address account, uint256 id, uint256 amount, bytes calldata data) external onlyRole(MINTER_ROLE) {
        require(block.chainid == mintingChainId || block.chainid == 31337, "Minting is allowed only on Polygon."); // or hardhat internal (for testing)
...
contract FearGeneralNft is FearNft1155
{
    constructor() FearNft1155("FearGeneralNft", "NEWFEARNFT", "https://fear.io", 80001) {
    }
}