...
import {AdvancedConfig, DropuStorage} from "../libraries/DropuStorage.sol";
import {MerkleProofUpgradeable} from "@openzeppelin/contracts-upgradeable/utils/cryptography/MerkleProofUpgradeable.sol";
import {IERC721} from "@openzeppelin/contracts/token/ERC721/IERC721.sol";

contract DropuERC721AExtensionD is DropuERC721AUpgradeable {
    ...
    // Address of the Dropu admin address
    address public constant dropuAdminAddress =
    ...
    /**
     * @notice Allows the dropuAdminAddress to set the dropu fee per token
     * @param _dropuFeePerToken The new fee per token in wei
     */
    function setDropuFeePerToken(uint256 _dropuFeePerToken) external {
        require(msg.sender == dropuAdminAddress, "MUST_BE_DROPU_ADMIN");
        DropuStorage.state().data.dropuFeePerToken = _dropuFeePerToken;
    }

    // ============ DROPU DEPOSIT TOKEN REDEMPTION ============
    ...
    function remainingDepositPaymentInWei() public view returns (uint256) {
        return
        uint256(DropuStorage.state().advCfg.remainingDepositPayment) *
        10 ** 13;
    }
    ...
    function setDepositMerkleRoot(
        ...
        DropuStorage.state().advCfg.depositMerkleRoot = _depositMerkleRoot;
    }

    /**
     * @notice Set the address of the Dropu deposit contract eligible for burning to mint
    ...
    function fundingTargetInWei() public view returns (uint256) {
        return uint256(DropuStorage.state().cfg.fundingTarget) * 10 ** 16;
    }
    ...
    function setLoaningActive(bool _loaningActive) external onlyOwner {
        DropuStorage.state().advCfg.loaningActive = _loaningActive;
    }
    ...
    function totalLoaned() public view returns (uint256) {
        return DropuStorage.state().data.currentLoanTotal;
    }
    ...
    function loanedBalanceOf(address _owner) public view returns (uint256) {
        return DropuStorage.state().data.totalLoanedPerAddress[_owner];
    }
    ...
    function loanedTokensByAddress(
        ...
        for (
            ...
            if (DropuStorage.state().data.tokenOwnersOnLoan[i] == _owner) {
    ...
    function refundPriceInWei() public view returns (uint256) {
        return uint256(DropuStorage.state().advCfg.refundPrice) * 10 ** 13;
    }
    ...
    function setRefundAddress(address _refundAddress) external onlyOwner {
        ...
        DropuStorage.state().advCfg.refundAddress = _refundAddress;
    }
    ...
    function increaseRefundEndsAt(uint32 _newRefundEndsAt) external onlyOwner {
        ...
        DropuStorage.state().advCfg.refundEndsAt = _newRefundEndsAt;
    }
