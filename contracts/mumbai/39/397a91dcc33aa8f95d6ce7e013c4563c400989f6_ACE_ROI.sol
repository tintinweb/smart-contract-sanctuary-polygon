...
contract ACE_ROI{
    ...
    address owner;
    IERC20 public MATIC;
    uint256 public finalDate = 21; // change seconds to days on finalization
    uint256 public minAmount = 0.1 ether;
    uint256 public plan1FinalTime = 14;
    ...
    function calNoOfDays(address _staker) public view returns(uint256) {
        ...
        uint256 noOfDays = noOfSeconds.div(10 seconds); // change seconds to days on finalization

        if(noOfDays > finalDate){
            noOfDays = finalDate;
        }
        // condition for plan 1 max days which are 14 days

        /// Not working
        if(investers[_staker].plan == 1){
    ...
    function holdBonus(address _staker) public view returns(uint256){
        uint256 noOfDays = calNoOfDays(_staker);
        uint256 bonus = noOfDays.mul(investers[_staker].amount.div(1000).mul(1)); // 0.1 % bonus
    ...
    function withdrawReward(uint256 _plan) public {
        ...
        investers[msg.sender].rewardWithdrawed += reward; // error will be -=
        ...
        if(_plan == 2 || _plan == 3){
            rewardFor21Days(msg.sender);
        }
        
        MATIC.transfer(msg.sender, reward);
    }

    function rewardFor21Days(address _staker) private view {
        require(block.timestamp >= investers[_staker].time + 210 seconds, "Wait 21 Days");
    }
