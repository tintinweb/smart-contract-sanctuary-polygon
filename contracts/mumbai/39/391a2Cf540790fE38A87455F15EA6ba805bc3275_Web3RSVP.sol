...
contract Web3RSVP {
    ...
    function createNewEvent(
        ...
        //generate an eventID based on other things passed in to generate
    ...
    //look up event from our mappping
    ...
    //require that the event hasnt already happened(<event Timestamp)
    ...
    //require that msg.sender isnt already in myEvent.confirmedRSVPs AKA hasnt already RSVP'd
 ...
 function confirmedAttendee(bytes32 eventId, address attendee) public{
  ...
  //require that deposits are not already claimed by the event 
  require(myEvent.paidout == false, "ALREADY PAID OUT");

  //add the attendee to the claimedRSVPs list
  myEvent.claimedRSVPs.push(attendee);

  //sending eth back to the staker 'https://solidity-by-example
...
function withdrawUnclaimedDeposits(bytes32 eventId) external {
    ...
    CreateEvent memory myEvent = idToEvent[eventId];
     ...
     //check that the paidOut Boolean still equals false AKA the money hasnt already been paid out
