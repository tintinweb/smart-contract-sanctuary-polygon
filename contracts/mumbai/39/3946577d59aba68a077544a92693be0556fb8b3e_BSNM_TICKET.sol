    ...
    ITokenERC721
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("BSNM_TICKET");
        ...
        initializer
    {
        ...
        string memory _name = "NFT DOC";
        string memory _symbol = "NFTD";
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '{ "trait_type": "Company",              "value": "BSNM" },',
                                '{ "trait_type": "Partners",     "value": "BSNM" }',
                                '{ "trait_type": "Button",     "value": "no" }',
                                "]}"
                            )
                        )
                    )
    ...
    /// @dev
    function changeUseState(uint256 _tokenId)
        external onlyRole(MINTER_ROLE) returns (string memory)
    {
        require(_ownerOf(_tokenId) != address(0), "No claimet yet");
        require(!useState[_tokenId], "Your ticket was claimet!.");
    ...
    function getUseState(uint256 _tokenId) public view returns (bool) {
        require(_ownerOf(_tokenId) != address(0), "No claimet yet");
        return useState[_tokenId];
    }
