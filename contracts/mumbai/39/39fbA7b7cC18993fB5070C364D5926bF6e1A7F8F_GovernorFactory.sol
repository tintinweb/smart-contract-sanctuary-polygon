...
contract GovernorFactory is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function test(
        ...
        require(voteTokenExist, "GovernorFactory: vote token preset not exist");
        require(voteTokenPreset != address(0), "GovernorFactory: preset address wrong");
        emit testingFirst(voteTokenPreset);
        address voteToken = voteTokenPreset.cloneDeterministic(salt);
        require(voteToken != address(0), "GovernorFactory: vote token not initialized");
        (bool success, bytes memory result) = voteToken.call(
            _voteTokenInitializeData
        );
        _result = result;
        ...
        emit testing(voteToken, result, success);
        return result;
    }
