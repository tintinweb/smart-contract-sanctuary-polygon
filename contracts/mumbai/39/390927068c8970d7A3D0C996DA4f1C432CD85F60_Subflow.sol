...
contract Service is Ownable {
    ...
    function removePlan(uint8 _planId) public {
...
contract Subflow {
    ...
    function getServiceAddressNyId(uint256 _serviceId) public view returns (address) {
    ...
    function testLoop(address _s) public view returns (bool) {
        for(uint256 i = 0; i < serviceAddresses.length; i++) {
            if (serviceAddresses[i] == _s) {
                return true;
            }          
        } 
        return false;
