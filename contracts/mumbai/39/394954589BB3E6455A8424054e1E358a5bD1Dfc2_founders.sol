...
contract founders is Ownable, ReentrancyGuard, AccessControl {
    ...
    function leaderboardMint(
        ...
        //     "Founder Contract: Invalid category"
        // );
        require(
        ...
        require(
            _tokenId >= categories[_category].leaderboardSupplyPoint &&
                _tokenId <= categories[_category].maxSupplyPoint,
            "Founder Contract: Invalid token id for leaderboard supply range"
        );

        bytes32 msgHash = getMessageHash(msg.sender, _tokenId, _category);
        ...
        emit LeaderboardMint(msg.sender, _tokenId, _category);
    }
    ...
    function reservedMint(
        ...
        require(
            _tokenId >= categories[_category].reservedSupplyPoint &&
                _tokenId < categories[_category].leaderboardSupplyPoint,
            "Founder Contract: Invalid token id for leaderboard supply range"
        );
        bytes32 msgHash = getMessageHash(msg.sender, _tokenId, _category);
        ...
        totalMintedSupply++;
        NFT.mint(msg.sender, _tokenId);
        emit ReservedMint(msg.sender, _tokenId, _category);
    }
    ...
    function publicMint(
        ...
        // );
        require(
            msg.value == _tokenIds.length * categories[_category].categoryPrice,
            "Founder Contract: Invalid Price"
        );
        require(
            totalPublicMintedSupply.add(_tokenIds.length) <= totalPublicSupply,
            "Founder Contract: Public supply max minted"
        );
        for (uint256 index = 0; index < _tokenIds.length; index++) {
            require(
                categories[_category].publicMintedSupply.add(1) <=
                    categories[_category].publicSupply,
                "Founder Contractt: Max supply of public in this category minted"
            );
            require(
                _tokenIds[index] >= categories[_category].publicSupplyPoint &&
                    _tokenIds[index] <
            ...
            totalMintedSupply++;
            NFT.mint(msg.sender, _tokenIds[index]);
            emit PublicMint(msg.sender, _tokenIds[index], _category);
        }
    }
