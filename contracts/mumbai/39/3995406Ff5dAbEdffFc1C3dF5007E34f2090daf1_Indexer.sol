...
contract Indexer is Initializable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    function getStudentId(uint256 institutionId, uint256 courseId, address student) internal view returns (uint256) {
        return getInstitutionContract(institutionId).getStudentId(courseId, student);
    }

    function getStudentURI(uint256 institutionId, uint256 courseId, address student) internal view returns (string memory) {
        Institutions intitution = getInstitutionContract(institutionId);
        return intitution.getStudentURI(courseId, student);
    }

    function getAllStudentIds(uint256 institutionId, address student) internal view returns (uint256[] memory) {
