...
contract CrossChainDepositor is LogicUpgradeable {
    ...
    modifier isUsedMultichainToken(address token) {
        require(multichainTokenPoolId[token] != 0, "CD1");
        _;
    }

    modifier onlyExecutor() {
        require(
            msg.sender == IMultichainRouter(stargateRouter).executor(),
     ...
     * @param poolId Pool ID defined in Stargate
     */
    function addMultichainToken(address token, uint8 poolId) external onlyOwner {
        require(token != address(0), "CD2");
        require(multichainTokenPoolId[token] == 0, "CD3");

        multichainTokenPoolId[token] = poolId;

        emit SetMultichainTokenPoolId(token, poolId);
    }
