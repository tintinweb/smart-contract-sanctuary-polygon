...
contract NFT is ERC721, PullPayment, Ownable {
        ...
        returns (string memory)
    {
        ...
        require(
            bytes(uris[ownerAddress]).length > 0,
            "Token URI is not yet set"
        );

        return uris[ownerAddress];
    }

    /// @dev Sets the base token URI prefix.
    function setTokenURI(address _address, string memory _tokenURI)
        public
        onlyOwner
    {
        uris[_address] = _tokenURI;
    }
