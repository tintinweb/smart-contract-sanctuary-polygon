...
abstract contract Role is ISLProject, ProjectBase {
    ...
    function isProviderMaster(address _account) public view returns (bool) {
        return provider.isMaster(_account);
    }

    function isProvider(address _account) public view returns (bool) {
        return provider.hasRole(_account);
    }
    ...
    function _onlyProviderMaster(address _account) private view {
        if (!isProviderMaster(_account)) revert OnlyProviderMaster();
    }

    function _onlyProvider(address _account) private view {
        if (!isProvider(_account)) revert OnlyProvider();
    }
     ...
     *  Role
     */
    function setProvider(ISLRoleWallet _provider) external onlyProviderMaster {
        if (
            !address(_provider).supportsInterface(
        ...
        provider = _provider;
        emit SetProvider(address(_provider), block.timestamp);
    }

    function setOperator(ISLRoleWallet _operator) external onlyProviderMaster {
        if (
    ...
    function setNextOperator(
        ...
        operator = _operator;
        operatorExpirationTimestamp = _expirationTimestamp;
        operatorSequence += 1;
        emit SetNextOperator(
            address(_operator),
    ...
    // DAO governance
    function setDAO(IDAOFundWallet _DAO) external onlyProviderMaster {
        if (
            !address(_DAO).supportsInterface(type(IDAOFundWallet).interfaceId)
        ) {
            revert InvalidDAO();
        }

        DAO = _DAO;
        emit SetDAO(address(DAO), block.timestamp);
    }

    function setAssociator(address _associator) external onlyOperator {
        associator = _associator;

        emit SetAssociator(associator, block.timestamp);
    }
...
abstract contract Royalty is Role {
    function setRoyalty(
        uint32 _providerRate,
        uint32 _operatorRate,
        uint32 _associatorRate,
        ...
        royalty.providerRate = _providerRate;
        royalty.operatorRate = _operatorRate;
        royalty.associatorRate = _associatorRate;
        royalty.creatorRate = _creatorRate;
        royalty.DAORate = _DAORate;
    ...
    function getTotalRoyalty() external view returns (uint256) {
        return
            royalty.providerRate +
            royalty.operatorRate +
            royalty.associatorRate +
            royalty.creatorRate +
            royalty.DAORate;
    }
}
...
abstract contract Universe is Role {
    ...
    function getUniverseById(
        uint256 _universeId
    ) external view returns (Universe memory, uint256[] memory) {
        if (!isExistUniverseById(_universeId)) {
            revert InvalidUniverseId();
        }
...
contract SLProject is Royalty, Collection, UUPSUpgradeable {
    function initialize(
        ISLRoleWallet _provider,
        ISLRoleWallet _operator,
        IDAOFundWallet _DAO,
        address _associator
    ) public initializer {
    ...
    function getAccountsByCollectionId(
        ...
        accounts[uint256(AccountType.Provider)] = address(provider);
        accounts[uint256(AccountType.Operator)] = address(operator);
        accounts[uint256(AccountType.Associator)] = associator;
        accounts[uint256(AccountType.Creator)] = collections[_collectionId]
            .creator;
        accounts[uint256(AccountType.DAO)] = address(DAO);
