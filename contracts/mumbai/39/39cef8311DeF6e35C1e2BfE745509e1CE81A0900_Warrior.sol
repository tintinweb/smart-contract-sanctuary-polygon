...
contract Warrior is IWarrior, ERC721Enumerable, Ownable, Pausable {
    ...
    uint256 public MINT_PRICE_GROAT = 1000 ether;
    uint256 public WL_PRICE_GROAT = 1000 ether;
    ...
    mapping(address => uint256) public userToRewardGroat;
    uint256 public totalReward = 0;
    uint256 public totalRewardGroat = 0;
    ...
    constructor(
        ...
        MINT_PRICE = _mintPrice*1 ether/1000;
        MINT_PRICE_GROAT = _mintPriceGroat*1 ether;
    }
    ...
    function setMintPrice(uint256 _price) external onlyOwner {
        MINT_PRICE = _price*1 ether/1000;
    }

    function setWLMintPrice(uint256 _price) external onlyOwner {
        WL_PRICE = _price * 1 ether;
    }

    function setMintPriceGroat(uint256 _priceGroat) external onlyOwner {
        MINT_PRICE_GROAT = _priceGroat * 1 ether;
    }

    function setWLMintPriceGroat(uint256 _priceGroat) external onlyOwner {
        WL_PRICE_GROAT = _priceGroat * 1 ether;
    }
    /** EXTERNAL */
     ...
     */
    function mint(uint256 amount, bool stake) public payable whenNotPaused {
        require(minted + amount <= MAX_TOKENS, "All tokens minted");
        require(amount > 0, "Invalid mint amount");
        if (isOpened()) {
            if(msg.value==0){
                IERC20(MINT_PAY_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), amount * WL_PRICE_GROAT);
            }else{
                require(
            ...
            if(msg.value==0){
                IERC20(MINT_PAY_TOKEN_ADDRESS).transferFrom(msg.sender, address(this), amount * MINT_PRICE_GROAT);
            }else{
                require(
    ...
    function core_mint(uint256 amount, bool stake) private {
        ...
        if(msg.value>0){
            userToReward[msg.sender] += MINT_PRICE *discount_amount/100;
        }else{
            userToRewardGroat[msg.sender] += MINT_PRICE_GROAT *discount_amount/100;
        }
        
        totalReward += MINT_PRICE *discount_amount/100;
        totalRewardGroat += MINT_PRICE_GROAT *discount_amount/100;
    ...
    function getUserRewardGroat(address user) public view returns (uint256){
        return userToRewardGroat[user];
    }
    ...
    function withdrawRewardGroat() external{
        uint256 this_groat_balance = IERC20(MINT_PAY_TOKEN_ADDRESS).balanceOf(address(this));
        require(
            userToRewardGroat[msg.sender]>0 && this_groat_balance >= userToRewardGroat[msg.sender],
            "You don't have reward balance or there is not enough $GROAT in the reward pool"
        );
        IERC20(MINT_PAY_TOKEN_ADDRESS).transfer(msg.sender, userToRewardGroat[msg.sender]);
        totalRewardGroat -= userToRewardGroat[msg.sender];
        userToRewardGroat[msg.sender] = 0;
    }

    function withdrawReward() external{
        require(
            userToReward[msg.sender]>0 && address(this).balance >= userToReward[msg.sender],
            "You don't have reward balance or there is not enough $GROAT in the reward pool"
        );
