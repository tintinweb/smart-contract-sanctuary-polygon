...
contract PresaleUpgradeable is OwnableUpgradeable {
    ...
    function _getTokenAmount(
        ...
        if (priceFeed != address(0)) {
            ...
            uint256 usdAmount = (amount * usdPrice) / 10 ** supportCoinInfo[token].decimals;
            return usdAmount / price;
        }
        return amount / price;
    }

    function _estimateTokenAmount(address inputToken, uint8 roundId, uint256 daoTokenAmount) private view returns (uint256) {
        require(daoTokenAmount > 0, "Invalid token amount");
        uint256 usdTotalPrice = daoTokenAmount * round[roundId].usdPricePerToken / 1e18;
        address priceFeed = supportCoinInfo[inputToken].chainlink;
        // Check stableCoin
        if (priceFeed != address(0)) {
            uint256 usdPrice = uint256(getLatestPrice(priceFeed));
            return usdTotalPrice * 10 ** supportCoinInfo[inputToken].decimals / usdPrice;
        }
        return usdTotalPrice * 1e6 / 1e8;
    }
