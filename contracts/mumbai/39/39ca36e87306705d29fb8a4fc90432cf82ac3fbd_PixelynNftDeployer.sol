...
// TODO split this up to multiple abstract contracts
contract PixelynNft is ReentrancyGuard, Context, IERC165, IERC721, IERC721Metadata, IERC2981 {
    ...
    uint256 private _numMintedTokens;
     ...
     */
    constructor(address __pixelynNftDeployer, SharedStructs.NewCollectionInitialize memory _collectionInitialize) {
        _name = _collectionInitialize.name;
        _symbol = _collectionInitialize.symbol;

        _pixelynNftDeployer = __pixelynNftDeployer;
        _mintStartTimestamp = _collectionInitialize.mintStartTimestamp;
        __baseURI = _collectionInitialize.baseURI;
        _price = _collectionInitialize.price;
        _maxSupply = _collectionInitialize.maxSupply;
        _numAvailableTokens = _collectionInitialize.maxSupply;

        // set the beneficiaries
        require(_collectionInitialize.beneficiaries.length > 0, 'PixelynNft: beneficiaries length must be at least 1');
        require(_collectionInitialize.beneficiaries.length < 4, 'PixelynNft: beneficiaries length must be less than 4');

        uint256 totalPercentage = 0;
        for (uint256 i = 0; i < _collectionInitialize.beneficiaries.length; i++) {
            require(
                _collectionInitialize.beneficiaries[i].percentage <= 10000,
                'PixelynNft: beneficiary percent must be less than or equal to 10000'
            );
            totalPercentage += _collectionInitialize.beneficiaries[i].percentage;
            _beneficiaries.push(_collectionInitialize.beneficiaries[i]);
        }

        require(totalPercentage == 10000, 'PixelynNft: total percent must be 10000');

        require(
            _collectionInitialize.royalties.percentage <= 10000,
            'PixelynNft: royalties percent must be less than or equal to 10000'
        );

        // set the royalty recipient and percent
        _royaltyRecipient = _collectionInitialize.royalties.to;
        _royaltyPercentage = _collectionInitialize.royalties.percentage;
    }
    ...
    modifier isPixelynSuperAdmin() {
        require(
            IPixelynNftDeployer(_pixelynNftDeployer).getSuperAdmin() == _msgSender(),
            'PixelynNft: caller is not a super admin'
        );
        _;
    }
    ...
    modifier isAirdropper() {
        require(
            IPixelynNftDeployer(_pixelynNftDeployer).getAirdropperState(_msgSender()),
            'PixelynNft: caller is not a airdropper'
        );
        _;
    }
    ...
    function balanceOf(address owner) public view returns (uint256) {
        require(owner != address(0), 'PixelynNft: address zero is not a valid owner');
    ...
    function ownerOf(uint256 tokenId) public view virtual override returns (address) {
        ...
        require(owner != address(0), 'PixelynNft: invalid token ID');
        return owner;
    }
     ...
     */
    function setName(string memory newName) external isPixelynSuperAdmin {
        _name = newName;
    }
     ...
     */
    function setSymbol(string memory newSymbol) external isPixelynSuperAdmin {
        _symbol = newSymbol;
    }
     ...
     */
    function setMintPrice(uint256 _mintPrice) external isPixelynSuperAdmin {
        _price = _mintPrice;
    }
     ...
     */
    function setMintStartTimestamp(uint256 newMintStartTimestamp) external isPixelynSuperAdmin {
     ...
     */
    function setBaseURI(string memory newBaseURI) external isPixelynSuperAdmin {
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        ...
        require(to != owner, 'PixelynNft:approval to current owner');

        require(
            _msgSender() == owner || isApprovedForAll(owner, _msgSender()),
            'PixelynNft:approve caller is not token owner or approved for all'
        );
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'PixelynNft:caller is not token owner or approved');
    ...
    /**
     * @dev Safely transfers `tokenId` token from `from` to `to`, checking first that contract recipients
     * are aware of the PixelynNft: protocol to prevent tokens from being forever locked.
     *
    ...
    function _safeTransfer(address from, address to, uint256 tokenId, bytes memory data) internal virtual {
        _transfer(from, to, tokenId);
        require(
            _checkOnERC721Received(from, to, tokenId, data), 'PixelynNft: transfer to non ERC721Receiver implementer'
        );
    }
    /**
    ...
    function _getRandomAvailableTokenId(address to, uint256 updatedNumAvailableTokens) internal returns (uint256) {
        uint256 randomNum = uint256(
            keccak256(
                abi.encode(
                    ...
                    block.number,
                    block.timestamp,
                    // TODO: look at prevrandao here instead of difficulty
                    block.difficulty,
    ...
    function _getAvailableTokenAtIndex(uint256 indexToUse, uint256 updatedNumAvailableTokens)
        internal
        returns (uint256)
    {
     ...
     */
    function reserve(address to, uint256 index) public isPixelynSuperAdmin {
        require(_msgSender() == tx.origin, 'PixelynNft: Contracts cannot mint');
        require(to != address(0), 'PixelynNft: mint to the zero address');
        require(_numAvailableTokens >= 1, 'PixelynNft: minting more tokens than available');

        uint256 tokenId = _getAvailableTokenAtIndex(index, _numAvailableTokens);
        --_numAvailableTokens;
     ...
     *
     */
    function mintRandom(address to, uint256 _numToMint) public payable nonReentrant {
        require(block.timestamp >= _mintStartTimestamp, 'PixelynNft: mint has not started yet');
        // require(_msgSender() == tx.origin, 'PixelynNft: Contracts cannot mint');
        require(to != address(0), 'PixelynNft: mint to the zero address');
        require(_numToMint > 0, 'PixelynNft: need to mint at least one token');
        require(_numAvailableTokens >= _numToMint, 'PixelynNft: minting more tokens than available');
        require(msg.value == _price * _numToMint, 'PixelynNft: Did not send correct amount of funds');

        _splitPayments();

        _mintRandomInternal(to, _numToMint);
    }

    function _mintRandomInternal(address to, uint256 _numToMint) private {
     ...
     *
     */
    function airdrop(address[] memory to) public isAirdropper {
        require(_numAvailableTokens >= to.length, 'PixelynNft: minting more tokens than available');

        for (uint256 i; i < to.length; ++i) {
            address toAddress = to[i];

            _mintRandomInternal(toAddress, 1);
        }
    }
    ...
    function _setRoyalties(address newRecipient, uint256 newPercentage) internal {
        require(newRecipient != address(0), 'Royalties: new recipient is the zero address');
        require(newPercentage < 10000, 'Token royalty > 100%');
        _royaltyRecipient = newRecipient;
        _royaltyPercentage = newPercentage;
    }

    function setRoyalties(address newRecipient, uint256 percent) external isPixelynSuperAdmin {
        _setRoyalties(newRecipient, percent);
    }
    ...
    {
        // TODO - check `_royaltyPercentage` logic!
        uint256 amount = _royaltyPercentage * salePrice / 10000;
        return (_royaltyRecipient, amount);
    }
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        require(PixelynNft.ownerOf(tokenId) == from, 'PixelynNft:transfer from incorrect owner');
        require(to != address(0), 'PixelynNft:transfer to the zero address');
    ...
    function _setApprovalForAll(address owner, address operator, bool approved) internal virtual {
        require(owner != operator, 'PixelynNft:approve to caller');
    ...
    function _requireMinted(uint256 tokenId) internal view virtual {
        require(_exists(tokenId), 'PixelynNft: invalid token ID');
    }
        ...
        returns (bool)
    {
        if (to.isContract()) {
            try IERC721Receiver(to).onERC721Received(_msgSender(), from, tokenId, data) returns (bytes4 retval) {
                ...
                if (reason.length == 0) {
                    revert('PixelynNft: transfer to non ERC721Receiver implementer');
                } else {
                    /// @solidity memory-safe-assembly
                    assembly {
    ...
    /**
     * @dev Hook that is called before any token transfer. This includes minting and burning. If {PixelynNft:Consecutive} is
     ...
     */
    function _beforeTokenTransfer(address from, address to, uint256 batchSize) internal virtual {
        if (batchSize > 1) {
            if (from != address(0)) _balances[from] -= batchSize;
            if (to != address(0)) _balances[to] += batchSize;
        }
    }
}
...
contract PixelynNftDeployer is IPixelynNftDeployer, Context, Ownable {
     ...
     */
    modifier isAirdropper() {
        require(_allAirdroppers[_msgSender()], 'PixelynNftDeployer: caller is not a allowed airdropper');
        _;
    }
     ...
     */
    modifier isAdmin() {
        require(_admins[_msgSender()], 'PixelynNftDeployer: caller is not a admin');
        _;
    }
    ...
    modifier isAdminOrSuperAdmin() {
        require(
            _admins[_msgSender()] || this.owner() == _msgSender(),
            'PixelynNftDeployer: caller is not a admin or super admin'
        );
        _;
    }
    ...
    function getRoles(address[] memory addresses) external view returns (SharedStructs.Roles[] memory) {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            SharedStructs.Roles memory hasRole = SharedStructs.Roles({
                user: addresses[i],
                isSuperAdmin: this.owner() == addresses[i],
                isAirdropper: getAirdropperState(addresses[i]),
                isAdmin: getAdminState(addresses[i])
            });
            roles[i] = hasRole;
        }
