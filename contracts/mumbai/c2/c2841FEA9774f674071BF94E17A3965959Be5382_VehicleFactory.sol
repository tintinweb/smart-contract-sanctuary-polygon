  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
        ...
        onlyAuthorizationContract //只允许主合约调用
    {
        ...
        if (rand == 0) {
            movement_speed = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_durability = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            max_capacity = 300 - movement_speed - max_durability;
        } else if (rand == 1) {
            max_durability = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_capacity = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            movement_speed = 300 - max_durability - max_capacity;
        } else if (rand == 2) {
            movement_speed = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_capacity = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            max_durability = 300 - movement_speed - max_capacity;
        } else {
            movement_speed = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_durability = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            max_capacity = 300 - movement_speed - max_durability;
        ...
        requiresUpgrade(_to, _id, _max, _orderid, signature)
    {
        ...
        uint rand = uint256(keccak256(abi.encode(randomResult, 1))) % 3; // 0,1,2
         ...
         if (rand == 0) {
            characters[_id].movement_speed += uint256(keccak256(abi.encode(randomResult, 2))) % _max; 
         } else if (rand == 1) {
            characters[_id].max_durability += uint256(keccak256(abi.encode(randomResult, 3))) % _max; 
         } else if (rand == 2)  {
            characters[_id].max_capacity += uint256(keccak256(abi.encode(randomResult, 4))) % _max; 
         }
    }
        ...
        requiresRevert(_to, _id, _attributes, _orderid, signature)
    {
        ...
        characters[_id].movement_speed = _attributes[0];
        characters[_id].max_durability = _attributes[1];
        characters[_id].max_capacity = _attributes[2];
    ...
    // 获取载具总铸造数量
    function totalVehicles() public view returns (uint) {
