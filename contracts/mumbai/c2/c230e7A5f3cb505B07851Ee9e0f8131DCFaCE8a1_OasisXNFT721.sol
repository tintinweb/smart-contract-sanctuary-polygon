...
contract OasisXNFT721 is ERC721A, Pausable, PaymentSplitter {
     ...
     * @param root merkle tree root of the hashed whitelist addresses
     ...
     * @param owner_ address of proxy's owner
     **/
    function initialize(
        string memory name_,
    ...
    function mint(
        ...
        require(to != address(0), "OasisXNFT721: Address cannot be 0");
        require(
            received == mintPrice[_phase].mul(quantity),
    ...
    function limitNotExceeded() public view returns (bool) {
        require(
            _currentIndex.sub(1) <= _maxTokenId,
    ...
    {
        require(
            _numberMinted(minter).add(quantity) <= _mintsPerAddressLimit,
...
contract PaymentSplitter is Context, Initializable {
    ...
    function getDuePayment(address account_) public view returns (uint256) {
        return
            ((address(this).balance + totalReleased()) * shares(account_)) /
            totalShares() -
            released(account_);
    }
 ...
 */
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata, Initializable {
    using Address for address;
    using Strings for uint256;
    ...
    function initialize721A(string memory name_, string memory symbol_)
        internal
        onlyInitializing
    {
        _name = name_;
