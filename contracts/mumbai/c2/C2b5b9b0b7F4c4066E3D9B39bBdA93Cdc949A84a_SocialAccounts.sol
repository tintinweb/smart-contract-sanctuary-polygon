...
contract SocialAccounts is VRFConsumerBaseV2, ChainlinkClient, ERC677Receiver {
    ...
    // ORACLE
    address private oracle = 0xc71Ff0575DFBE3aDfaf50aE29611E1b8377d6c7B;
    bytes32 private jobId = "89b71cd80bb342669fd98eda3509acd5";
    ...
    event VerificationForAddress(address requester, string verificationTimestamp, string account);
    ...
    function getVerificationsForAddress(address addr) public {
        ...
        for (uint i=0; i<arrayLength; i++) {
            emit VerificationForAddress(addr, verificationsForAddr[i].timestamp,
                verificationsForAddr[i].account);
        }
    }

    function requestVerification(string calldata account) public {
    ...
    function verify(bytes32 test) public
    {
        Request storage rq = requests[msg.sender];
        ...
        if (bytes(rq.account).length == NULL)
            revert("Missing required parameters for verification : account)");

        Chainlink.Request memory oracle_request = buildChainlinkRequest(test, address(this), this.fulfill.selector);
    ...
    function fulfill(bytes32 _requestId, bool _value) public recordChainlinkFulfillment(_requestId)
    {
        ...
        if (_value == true) {
            Verification memory vr = Verification({account: requests[initiator].account, timestamp: Strings.toString(block.timestamp) });
