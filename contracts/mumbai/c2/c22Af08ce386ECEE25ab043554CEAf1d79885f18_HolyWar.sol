    ...
    ERC2771Context
{
    ...
    function fulfillRandomWords(
        ...
        if (gameData[gameId].state == GameState.GETTING_VRF_1) {
            ...
            uint256 playerCard = gameData[gameId].chosenCards[0] % 13;
            uint256 houseCard = gameData[gameId].chosenCards[1] % 13;
            ...
            uint256 playerCard = gameData[gameId].chosenCards[8] % 13;
            uint256 houseCard = gameData[gameId].chosenCards[9] % 13;
    ...
    function shuffleDeck(uint256 _numberOfCards, uint256 _gameId) private {
        ...
        for (; g.drawIndex < startingIndex + _numberOfCards; ) {
            ...
            g.chosenCards[g.drawIndex] = index;
            unchecked {
                --cardsRemaining;
                ++g.drawIndex;
            }
    ...
    function playerDeposit(uint256 _amount) public {
        address sender = _msgSender();
        playerDeposit(sender, _amount);
    }

    function playerWithdraw(uint256 _amount) public {
        address sender = _msgSender();
        playerWithdraw(sender, _amount);
    }
...
contract TreasuryManager {
    ...
    function playerDeposit(address _player, uint256 _amount) internal {
        anteTokenContract.safeTransferFrom(_player, address(this), _amount);
        balances[_player] += _amount;
        emit Deposit(_player, _amount);
    }

    function playerWithdraw(address _player, uint256 _amount) internal {
        require(
            _amount <= balances[_player],
            "playerWithdraw: amount exceeds players balance"
        );

        anteTokenContract.safeTransfer(_player, _amount);
        balances[_player] -= _amount;
        emit Withdraw(_player, _amount);
    }
...
contract GameManager {
    ...
    {
        return gameData[_gameId].chosenCards;
    }
