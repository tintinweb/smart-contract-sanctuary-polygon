...
contract QatarWorldCup is ERC721Enumerable, Ownable {
    ...
    function mintBatch721Token(uint256 _numberOfTokens, uint256 _lowBreakpoint, uint256 _highBreakpoint) public payable hasMintRunning{
        ...
        if( _tokenOverRange == 0 ){
            ...
            uint256 _mintPriceOverRange = getMintPriceByBreakpoint(_highBreakpoint + 1);
            require(((_tokenInRange * _mintPriceInRange) + (_tokenOverRange * _mintPriceOverRange)) <= msg.value, "Ether value sent is not correct");
        }
    ...
    function getMintPriceByBreakpoint(uint256 _breakpoint) public view returns (uint256 ){
            unchecked { return  mintPrice - (mintPrice * discountByBreakpoint[breakpoint[_breakpoint]])/denominator;}
    }
