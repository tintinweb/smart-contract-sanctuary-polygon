...
contract KeeperArctic is AccessControlledAndUpgradeable, KeeperCompatibleInterface {
  ...
  function getOracleInfoForSystemStateUpdate(
    ...
    //      And 18 for mumbai (because it can handle bigger transactions)
    if (_numberOfMissedEpochs > (6 * (block.chainid == 80001 ? 3 : 1))) {
      _numberOfMissedEpochs = 6 * (block.chainid == 80001 ? 3 : 1);
    }
    ...
    while (currentUpdateIndex < _numberOfMissedEpochs) {
      ...
      if (
        ...
        // console.log("We found an update !");
