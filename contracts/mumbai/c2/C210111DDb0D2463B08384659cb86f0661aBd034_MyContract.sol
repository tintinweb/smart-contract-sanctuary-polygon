...
contract MyContract is VRFV2WrapperConsumerBase {
    event BetRequest(uint indexed requestId);
    event BetResult(BetSelection indexed result);
    ...
    // mapping to track each address's stakes
    uint256 public houseStake; // Variable to track total staked amount
    ...
    uint constant entryFees = 10_000_000;
    ...
    function bet(BetSelection choice) external payable returns (uint) {
    ...
    function fulfillRandomWords(
        ...
        uint256 scaledNumber = randomWords[0] % 10000;

        if (scaledNumber < 4586) {
            BetSelection result = BetSelection.Banker;
            if (statuses[requestId].choice == result) {
                statuses[requestId].didWin = true;
                uint256 transferAmount = (entryFees * 195) / 100; // Calculate the transfer amount
                require(
                    address(this).balance >= transferAmount,
                    "Not enough balance in the contract"
                ); // Ensure the contract has enough balance
                payable(statuses[requestId].player).transfer(transferAmount); // Transfer the funds
                houseStake = houseStake - transferAmount;
            }
        } else if (scaledNumber < 9048) {
            BetSelection result = BetSelection.Player;
            if (statuses[requestId].choice == result) {
                statuses[requestId].didWin = true;
                uint256 transferAmount = (entryFees * 200) / 100; // Assuming no extra gain on winning for player
                require(
                    address(this).balance >= transferAmount,
                    "Not enough balance in the contract"
                ); // Ensure the contract has enough balance
            ...
            BetSelection result = BetSelection.Tie;
            if (statuses[requestId].choice == result) {
                statuses[requestId].didWin = true;
                uint256 transferAmount = (entryFees * 900) / 100; // Assuming 8x gain on winning for tie
                require(
                    address(this).balance >= transferAmount,
                    "Not enough balance in the contract"
                ); // Ensure the contract has enough balance
