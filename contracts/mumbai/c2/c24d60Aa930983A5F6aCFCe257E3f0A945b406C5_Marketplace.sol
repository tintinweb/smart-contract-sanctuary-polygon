    ...
    IERC1155ReceiverUpgradeable
{
    ...
    /// @dev Mapping from uid of a direct listing => Mapping from address of allowed order buyers
    mapping(uint256 => mapping(address => bool)) public allowedBuyers;
    ...
    // @dev Checks whether caller is a listing creator or the contract owner.
    modifier onlyListingCreatorOrOwner(uint256 _listingId) {
        require(listings[_listingId].tokenOwner == _msgSender(), "!OWNER");
    ...
    /// @dev Checks wheather caller is allowed to buy a listing.
    modifier onlyAllowedBuyer(uint256 _listingId) {
        require(allowedBuyers[_listingId][_msgSender()], "!BUYER");
        _;
    }
    ...
    /// @dev Adds address to the order allowlist.
    function addAddressToOrderAllowlist(address _address, uint256 _listingId)
        external
        onlyExistingListing(_listingId)
        onlyListingCreatorOrOwner(_listingId)
    {
        allowedBuyers[_listingId][_address] = true;
        emit AddedToOrderAllowlist(_address, _listingId);
    }

    /// @dev Removes address from the order allowlist.
    function removeAddressFromOrderAllowlist(
        address _address,
        uint256 _listingId
    )
        external
        ...
        onlyListingCreatorOrOwner(_listingId)
    {
        allowedBuyers[_listingId][_address] = false;
        emit RemovedFromOrderAllowlist(_address, _listingId);
    }
...
import "../extension/IPlatformFee.sol";
...
interface IMarketplace is IThirdwebContract, IPlatformFee {
    ...
    /// @dev Emitted when an address is added to the allowed buyers list of a listing.
    event AddedToOrderAllowlist(address wallet, uint256 listingId);

    /// @dev Emitted when an address is removed from the allowed buyers list of a listing.
    event RemovedFromOrderAllowlist(address wallet, uint256 listingId);
    ...
    /**
     * @notice Add a new address to the allowlist of addresses that can buy listings.
    ...
    /**
     * @notice Remove an address from the Allowlist of addresses that can buy listings.
     * @dev Only the order owner can call this function.
     *
