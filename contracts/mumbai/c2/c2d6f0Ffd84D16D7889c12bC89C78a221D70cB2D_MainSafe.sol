...
contract MainSafe is Ownable, IMainSafe {
    ...
    constructor() {
        priceFeed["USDC"] = 0x572dDec9087154dC5dfBB1546Bb62713147e0Ab0;
        tokens["USDC"] = 0xe6b8a5CF854791412c1f6EFC7CAf629f5Df1c747;
    }
    ...
    function checkEmail(string memory _email) external returns(address) {
        require(emailToUser[_email] != address(0), "Invalid email: User not registered!");
        emit ShowContractAddress(emailToUser[_email]);
...
contract User is Ownable, IUser {
    ...
    function deposit(string memory tokenName, uint256 amount, string memory _email) external {
        ...
        Gencoin(gencoin).mintToken(amount/25, email);
    ...
    function sendTokensAddress(string memory tokenName, uint256 amount ,address receiverAddress, string memory _email) external {
     ...
     function swap(string memory _email, string memory from, string memory to, uint256 amount) external {
        ...
        uint256 fromVal = ((amount * 1 ether) /uint256(fromPrice));
        ERC20(fromTokenAddress).transfer(address(mainSafe), fromVal);
        uint256 toAmount = amount - ((amount * 2)/100);
