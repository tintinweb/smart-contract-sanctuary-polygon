...
contract gPhroToken is ERC20Burnable, Ownable, AccessControl
{
    ...
    {
        require(hasRole(MINTER_ROLE, msg.sender), "Not an approved minter");
        _;
    }

    modifier isPermittedDao()
    {
        require(hasRole(DAO_ROLE, msg.sender), "Not an approved DAO");
        _;
    }

    modifier isPermittedOperator()
    {
        require(hasRole(PharoConstants.OPERATOR_ROLE, msg.sender), "Not an approved minter");
        _;
    }

    modifier isPermittedDistributor()
    {
        require(hasRole(DISTRIBUTOR_ROLE, msg.sender), "Not an approved distributor");
        _;
    }
    
    modifier isAdminOrOwner()
    {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || owner() == msg.sender, "You are not admin or owner");
        _;
    }
    ...
    constructor(address _newOwner) ERC20("Pharo Governance Token", "gPHRO") ERC20Burnable() {
        _setupRole(DEFAULT_ADMIN_ROLE, _newOwner);
        _setupRole(MINTER_ROLE, _newOwner);
        _mint(0xc316Efa8aD1046b0DEf528e17E6f4136072731F7, 1000000 ether); // Jax Pharo Foundry Account
     ...
     * See {ERC20-_mint}.
     */
    function mintStakerTokens(address to, uint256 amount) public isPermittedMinter
    {
    ...
    function permit(
        ...
        require(
            ...
            "gPHRO :: past the fucking deadline bro!"
        );
        ...
        require(
            signatory != address(0),
            "gPHRO :: permit: invalid signature ~ zero address mofo!"
        );
        require(
            signatory == owner,
            "gPHRO :: permit: unauthorized ~ must be the owner of the signature..."
        );
    ...
    function delegateBySig(
        ...
        require(
            signatory != address(0),
            "gPHRO::delegateBySig: invalid signature"
        );
        require(
            nonce == _nonces[signatory]++,
            "gPHRO::delegateBySig: invalid nonce"
        );
        require(
            block.timestamp <= expiry,
            "gPHRO::delegateBySig: signature expired"
        );
    ...
    function getPriorVotes(
        ...
        require(
            ...
            "gPHRO::getPriorVotes: not yet determined"
        );
    ...
    function _transferTokens(address src, address dst, uint96 amount) internal {
        require(
            src != address(0),
            "gPHRO::_transferTokens: cannot transfer from the zero address"
        );
        require(
            dst != address(0),
            "gPHRO::_transferTokens: cannot transfer to the zero address"
        );

        _balances[src] = sub96(
            _balances[src],
            amount,
            "gPHRO::_transferTokens: transfer amount exceeds balance"
        );
        _balances[dst] = add96(
            _balances[dst],
            amount,
            "gPHRO::_transferTokens: transfer amount overflows"
        );
    ...
    function _writeCheckpoint(
        ...
        uint32 blockNumber = safe32(
            block.number,
            "gPHRO::_writeCheckpoint: block number exceeds 32 bits"
        );
