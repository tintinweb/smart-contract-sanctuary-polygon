...
import {ERC721} from "@maticnetwork/fx-portal/contracts/lib/ERC721.sol";

/**
 * @title FxERC721RootTunnel - Polygon Minted Tokens bridge
 * @dev to be deployed on Polygon
 */
contract FxERC721RootTunnelv2 {
    // constructor
    // map Token
    // Process message from child
    // deposit -> lock the token on contract and its has to be minted on Goerli
    ...
    function mapToken(address rootToken) public {
        ...
        require(
            ...
            "FxERC721RootTunnel: ALREADY_MAPPED"
        );
        ...
        bytes memory message = abi.encode(
            MAP_TOKEN,
            abi.encode(rootToken, name, symbol)
        );

        // Handle Send message to root - Polygon to Goerli
    ...
    function deposit(
        ...
        // require token mapping
        require(
            rootToChildTokens[rootToken] != address(0x0),
            "FxERC721RootTunnel: Mapping required for Root Token"
        );
        ...
        bytes memory message = abi.encode(
            DEPOSIT,
            abi.encode(rootToken, msg.sender, user, tokenId, data)
        );
        sendMessageToChild(message);
    ...
    function _processMessageFromChild(
        ...
        if (syncType == WITHDRAW) {
            ...
            revert("FxERC721RootTunnel: INVALID_SYNC_TYPE");
        }
    }
    ...
    modifier validateSender(address sender) {
        require(
            sender == fxChildTunnel,
            "FxBaseRootTunnel: INVALID_SENDER_FROM_CHILD"
        );
        _;
    }

    //(rootToken, childToken, receiver, tokenId, data)

    function _withdraw(bytes memory syncData) internal returns (address) {
        (
            address rootToken,
            address childToken,
            address to,
        ...
        require(
            rootToChildTokens[rootToken] == childToken,
            "FxERC721RootTunnel: INVALID_MAPPING_ON_EXIT"
        );
    ...
    function minimalProxyCreationCode(
        address logic
