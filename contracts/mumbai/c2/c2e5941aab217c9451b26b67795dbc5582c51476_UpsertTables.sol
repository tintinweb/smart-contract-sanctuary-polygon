...
contract UpsertTables is IERC721Receiver {
    ITablelandTables internal _tableland;
    string internal _tablePrefix;
    uint256 internal _tableId;
    string public tableName;

    constructor(address payable _tbl) payable {
        _tableland = ITablelandTables(_tbl);
        _tablePrefix = "upsert_example";
    }

    event CreateTable(string name, uint256 id);
    event Upsert(string name, string statements);
    ...
    function create() public payable {
        ...
        emit CreateTable(tableName, _tableId);
    }


    function upsert(string memory statements)
      public
      payable
    {
        // assumes an insert then update statement are ordered correctly
        _tableland.runSQL(
            address(this),
            _tableId,
            statements
        ...
        emit Upsert(tableName, statements);
