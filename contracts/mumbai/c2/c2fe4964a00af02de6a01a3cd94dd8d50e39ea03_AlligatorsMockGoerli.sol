...
contract Alligators is
    ...
    */
    ...
    function merge3alligators(uint256 _1st, uint256 _2nd, uint256 _3rd, address owner_) external {
        if (tokenIdType[_1st] != 1 || tokenIdType[_2nd] != 1 || tokenIdType[_3rd] != 1) revert callErr();
        if (ownerOf(_1st) != owner_ || ownerOf(_2nd) != owner_ || ownerOf(_3rd) != owner_) revert callErr();
        uint8[] memory pullDataTraits = _compareTrait(_1st, _2nd, _3rd);
        (uint8[] memory pullData, bool state) = _decide(_1st, _2nd, _3rd);
        if (!state) revert callErr();
        createAnatomy(pullDataTraits);
        createLevels(pullData);
    ...
    function mergeWjoker(uint256 _1st, uint256 _2nd, uint256 _3rd, address owner_) external onlyMerger() {
        if (tokenIdType[_1st] != 2 || tokenIdType[_2nd] != 1 || tokenIdType[_3rd] != 1) revert callErr();
        if (ownerOf(_1st) != owner_ || ownerOf(_2nd) != owner_ || ownerOf(_3rd) != owner_) revert callErr();
    ...
    */
    ...
    function createAnatomy(uint8[] memory data_) internal {
        _anatomy[_currentIndex] = NFTAnatomy(
            {
                trait1: Trait1(data_[0]), 
                trait2: Trait2(data_[1]),
                trait3: Trait3(data_[2]),
                trait4: Trait4(data_[3]),
                trait5: Trait5(data_[4]),
                trait6: Trait6(data_[5]),
                trait7: Trait7(data_[6])
            });
    }

    function createLevels(uint8[] memory data_) internal {
        _lvl[_currentIndex] = NFTLevel(
            {
                trait1Lvl: data_[0],
                trait2Lvl: data_[1],
