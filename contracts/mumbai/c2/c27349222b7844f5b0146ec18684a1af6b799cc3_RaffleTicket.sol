...
contract RaffleTicket is ERC20, Pausable {
    ...
    event BurnAccessGranted(address);
    event BurnAccessRevoked(address);
    event TransferFunctionEnabled();
    event TransferFunctionDisabled();

    constructor() ERC20("RaffleTicket", "RT") {
        _owner.push(address(0x32CB7819ABA6DBb3fAcA62793D775AB722396402));
        _owner.push(address(0xD675C5ec93fdD2235Bf8ba4A4530a482Ce72e6eC));
    }
    ...
    modifier requiresMintAccess() {
        require(
            ...
            "Raffle#requiresMintAccess: No Mint Access"
        );
        _;
    }
    ...
    modifier requiresBurnAccess() {
        require(
            ...
            "Raffle#requiresBurnAccess: No Burn Access"
        );
        _;
    }

    modifier whenTransferAllowed() {
        require(TransferStatus, "RaffleTicket#whenTransferAllowed: Transfer is not allowed");
        _;
    }
    ...
    function setTransferStatus(bool status) public onlyOwner{
        ...
        if(status){
            emit TransferFunctionEnabled();
        }else{
            emit TransferFunctionDisabled();
        }
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override whenTransferAllowed{
