...
contract Testament {
    ...
    function update(
        ...
        uint8[] memory _allocationPercentages,
        address[] memory _witness
    ) external onlyOwner onlyNonActivated {
        require(
            _beneficiaries.length == _allocationPercentages.length,
            "Beneficiaries and percentages must have the same length"
        );
        ...
        if (
            keccak256(abi.encodePacked(will.beneficiaries)) !=
        ...
        if (
            keccak256(abi.encodePacked(will.allocationPercentages)) !=
            keccak256(abi.encodePacked(_allocationPercentages))
        ) {
            will.allocationPercentages = _allocationPercentages;
            hasChange = true;
        }
        if (
            keccak256(abi.encodePacked(will.witness)) !=
            keccak256(abi.encodePacked(_witness))
        ) {
            will.witness = _witness;
            hasChange = true;
        }
        if (hasChange) {
            will.lastConfirmationTimestamp = block.timestamp;
            emit UpdatedSuccess(msg.sender);
        }
    }
