...
contract NBP is SBT {
    ...
    function mint(
        ...
        uint256 projectId = _metadata.projectId;
        _mint(_to, _tokenId, projectId);
        _setTokenMetadata(_tokenId, _metadata);
        _addUserToProject(projectId, _to);
    }
    ...
    function mintBatch(
        ...
        uint256 projectId = _metadata.projectId;
        _mintBatch(_tos, _tokenId, projectId);
        _setTokenMetadata(_tokenId, _metadata);
        _addUsersToProject(projectId, _tos);
...
contract Projects {
    ...
    function createProject(
        ...
        // TODO: Nameに対する正規表現+trim
        require(!_equal(_name, ""), "Name must be at least 0 characters.");
        require(!_equal(_imageUrl, ""), "ImageUrl must be at least 0 characters.");
        require(getProjectIdByName(_name) == 0, "That name has been taken.");
    ...
    function updateProjectById(
        ...
        require(!_equal(getProjectById(_projectId).name, ""), "Project does not exist.");
        require(getAdminByProjectId(_projectId, msg.sender), "Not authorized.");

        ProjectStruct memory currentProject = getProjectById(_projectId);
        string memory currentName = currentProject.name;
        require(_equal(currentName, _name) || getProjectIdByName(_name) == 0, "That name has been taken.");

        if (!_equal(_name, "") && !_equal(currentName, _name)) {
            _projects[_projectId].name = _name;
            _projectMapping[_name] = _projectId;
            delete _projectMapping[currentName];
        }

        if (!_equal(_imageUrl, "") && !_equal(currentProject.imageUrl, _imageUrl)) {
            _projects[_projectId].imageUrl = _imageUrl;
        }

        if (!_equal(_description, "") && !_equal(currentProject.description, _description)) {
            _projects[_projectId].description = _description;
        }
    }

    function updateProjectByName(
        string memory _currentName,
        string memory _name,
        string memory _imageUrl,
        string memory _description
    ) external {
        uint256 projectId = getProjectIdByName(_currentName);
        require(projectId > 0, "Project does not exist.");
        updateProjectById(projectId, _name, _imageUrl, _description);
    }
    ...
    function getProjectByName(string memory _name) public view returns(ProjectStruct memory) {
        uint256 projectId = getProjectIdByName(_name);
    ...
    //  ==========  user logic    ==========
    function _addUserToProject(uint256 _projectId, address _walletAddress) internal {
        require(!_equal(getProjectById(_projectId).name, ""), "Project does not exist.");

        if (!existProjectUserById(_projectId, _walletAddress)) {
            _projectUsers[_projectId].push(_walletAddress);
            _projectUserExists[_projectId][_walletAddress] = true;
            _operatorAdmins[_projectId][_walletAddress] = false;
        }

        if (!existUserProjectById(_walletAddress, _projectId)) {
            _userProjects[_walletAddress].push(_projectId);
            _userProjectExists[_walletAddress][_projectId] = true;
        }
    }
    ...
    function _addUsersToProject(uint256 _projectId, address[] memory _addressList) internal {
        ...
        for(uint256 i; i < count; i++) {
            ...
            _addUserToProject(_projectId, walletAddress);
        }
    }
    ...
    function getProjectUsersByName(
        string memory _name
        ...
        uint256 projectId = getProjectIdByName(_name);
        return getProjectUsersById(projectId);
    }
    ...
    function getUsersByProjectName(
        string memory _projectName
    ) public view returns(
        ProjectUserStruct[] memory
    ) {
        uint256 projectId = getProjectIdByName(_projectName);
        return getUsersByProjectId(projectId);
    }

    function existProjectUserById(
        uint256 _projectId,
        address _walletAddress
    ) public view returns(bool) {
        return _projectUserExists[_projectId][_walletAddress];
    }

    function existProjectUserByName(
        string memory _projectName,
        address _walletAddress
    ) public view returns(bool) {
        uint256 projectId = getProjectIdByName(_projectName);
        return existProjectUserById(projectId, _walletAddress);
    }
    ...
    function existUserProjectById(
        address _walletAddress,
        uint256 _projectId
    ) public view returns(bool) {
        return _userProjectExists[_walletAddress][_projectId];
    }

    function existUserProjectByName(
        address _walletAddress,
        string memory _projectName
    ) public view returns(bool) {
        uint256 projectId = getProjectIdByName(_projectName);
        return existUserProjectById(_walletAddress, projectId);
    }
    ...
    function getAdminByProjectName(string memory _projectName, address _walletAddress) public view returns(bool) {
        uint256 projectId = getProjectIdByName(_projectName);
        return getAdminByProjectId(projectId, _walletAddress);
    }
    ...
    function updateAdminByProjectId(
        ...
        require(createdBy == msg.sender, "Not authorized.");
        require(createdBy != _walletAddress, "Permissions of the user who created the Project cannot be changed.");

        if (_operatorAdmins[_projectId][_walletAddress] != _isAdmin) {
            _operatorAdmins[_projectId][_walletAddress] = _isAdmin;
        }
    }

    function updateAdminByProjectName(
        string memory _projectName,
        address _walletAddress,
        bool _isAdmin
    ) external {
        uint256 projectId = getProjectIdByName(_projectName);
        updateAdminByProjectId(projectId, _walletAddress, _isAdmin);
    }
...
contract SBT is Projects, ERC165, IERC1155, IERC1155MetadataURI {
    ...
    //  ==========  Additional logic    ==========
    function _mint(address _to, uint256 _id, uint256 _projectId) internal {
    ...
    function _mintBatch(address[] memory _tos, uint256 _id, uint256 _projectId) internal {
        for (uint256 i = 0; i < _tos.length; i++) {
            _mint(_tos[i], _id, _projectId);
        }
    }
    ...
    function _setTokenMetadata(
        ...
        if (bytes(_tokenMetadata[_id].name).length == 0) {
