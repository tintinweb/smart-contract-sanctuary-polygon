...
interface IRecovery is IYousovStructs{
    ...
    function sendRecoveryAnswerToAnswerAgent(string[] memory userAnswers) external;
...
contract Recovery is IRecovery, YousovRoles {
    address public user;
    ...
    address[] public answerAgents ;
    mapping (address => AnswerAgentsDetails) public answerAgentsDetails;
    mapping (address => bool) public questionAgents;
    ...
    function clearContenderAgents() external override {
        require(
    ...
    function addContenderAgent(address contenderAgent) external override {
        require(
            msg.sender == recoveryFactory,
            "Yousov: Operation not authorized"
        );
    ...
    function addNewAgentToRecovery() external override {
        ...
        require(!this.isUserIsAnActifAgent(msg.sender),"User is already an actif agent");
        agentList.push(msg.sender);
        IRecoveryFactory(recoveryFactory).addActiveAgent(msg.sender, address(this));
        emit AgentAddedToRecovery(address(this), msg.sender);
        if (
            agentList.length ==
            IUser(
    ...
    function isUserIsAnActifAgent(address userAgent) external view override  returns (bool) {
        for (uint i = 0; i < agentList.length; i++) {
    ...
    function isUserIsAnAnswerAgent(address userAgent) external view override  returns (bool) {
        for (uint i = 0; i < answerAgents.length; i++) {
            if (answerAgents[i] == userAgent) {
                return true;
            }
        }
        return false;
    ...
    function startTheRecovery() external override {
        require(recoveryStatus == RecoveryStatus.CREATED && tx.origin == user  && (agentList.length ==
            IUser(
                IUserFactory(IRecoveryFactory(recoveryFactory).userFactory())
                    .userContract(user)
            ).userChallenges().length *
                2), "YOUSOV: Operation not authorized");
        uint256 randomIndex;
        for(uint256 i = 0; i < agentList.length; i++) {
            randomIndex = uint256(keccak256(abi.encodePacked(block.timestamp, i, block.difficulty))) % agentList.length;
            
            if(i < agentList.length / 2) {
                string memory challengeID =IUser(
                 IUserFactory(IRecoveryFactory(recoveryFactory).userFactory())
                    .userContract(user)
                ).userChallenges()[i];
                Challenge memory affectedChallenge = IUser(
                 IUserFactory(IRecoveryFactory(recoveryFactory).userFactory())
                    .userContract(user)
                ).userChallengesDetails(challengeID);
                answerAgents.push(agentList[randomIndex]);
                answerAgentsDetails[agentList[randomIndex]] = AnswerAgentsDetails(affectedChallenge.answer,"",false, AnswerStatus.NOT_ANSWERED);
                emit AgentRoleForRecovery(agentList[randomIndex], address(this), RecoveryRole.ANSWER_AGENT, affectedChallenge.answer );
            } else {
                questionAgents[agentList[randomIndex]] = true;
                emit AgentRoleForRecovery(agentList[randomIndex], address(this), RecoveryRole.QUESTION_AGENT,"");
            }
        }
    }


    function sendRecoveryAnswerToAnswerAgent(string[] memory userAnswers) external override {
        require(tx.origin == user && recoveryStatus == RecoveryStatus.IN_PROGRESS , "YOUSOV : Not authorised operation");
        for (uint i = 0; i < userAnswers.length; i++) {
            address answerAgent = answerAgents[i];
            AnswerAgentsDetails memory lastStat =  answerAgentsDetails[answerAgent] ;
            answerAgentsDetails[answerAgent] = AnswerAgentsDetails(lastStat.initialAnswer,userAnswers[i],false,AnswerStatus.NOT_ANSWERED);
            emit SendAnswersToAnswerAgents(answerAgent, address(this), userAnswers[i]);
        }
    }

    function _isAllAnswersAgentsHaveAnswered() view private returns(RecoveryStats memory) {
        uint256 totalValidAnswers = 0;
        for (uint i = 0; i < answerAgents.length; i++) {
            if (answerAgentsDetails[answerAgents[i]].answerStatus == AnswerStatus.NOT_ANSWERED) {
                return RecoveryStats(false,0);
            }
            if (answerAgentsDetails[answerAgents[i]].answer) {
    ...
    function agentCheckUserAnswer(address answerAgent,bool userAnswer) external override {
        require(msg.sender == answerAgent && recoveryStatus == RecoveryStatus.IN_PROGRESS && this.isUserIsAnAnswerAgent(answerAgent) && answerAgentsDetails[answerAgent].answerStatus == AnswerStatus.NOT_ANSWERED , "YOUSOV : Not authorised operation");
        answerAgentsDetails[answerAgent] = AnswerAgentsDetails(answerAgentsDetails[answerAgent].initialAnswer,answerAgentsDetails[answerAgent].actualAnswer,userAnswer,AnswerStatus.ANSWERED);   
        // check if all answers agents have answered than 
        RecoveryStats memory actualRecoveryStats = _isAllAnswersAgentsHaveAnswered();
        if (actualRecoveryStats.isAllAnswersAgentsAnswered) {
            if (actualRecoveryStats.totoalValidAnswers >= IUser(
                 IUserFactory(IRecoveryFactory(recoveryFactory).userFactory())
                    .userContract(user)
                ).threashold() ) {
...
contract RecoveryFactory is IRecoveryFactory, YousovRoles {
    using SafeMath for uint256;
    address public ezr;
    ...
    function createRecovery() external override {
        require(
            ...
            "YOUSOV: Not authorized to this action"
        );
        // Pay the recovery
        TransferHelper.safeTransfer(ezr, agentPayrollWallet, 5 * 10 ** 17);

        EZR(ezr).burn(3 * 10 ** 17);

        TransferHelper.safeTransfer(
            ezr,
            0xD44d5088f6eb0a0Aed56B2065478737030acEA94,
            2 * 10 ** 17
        );

        // //  create the recovery
        address _newRecovery = address(
            new Recovery(msg.sender, address(this), yousovAccessControl)
        );

        recoveries.push(_newRecovery);
        currentCreatedRecoveries[msg.sender] = _newRecovery;
        emit RecoveryCreated(msg.sender, _newRecovery);
        this.getLegalSelectableAgents(_newRecovery);
    }

    function userHasRecovery(address userAddress) public returns (bool) {
        for (uint i = 0; i < recoveries.length; i++) {
            if (
                userAddress == IRecovery(recoveries[i]).user() &&
                (IRecovery(recoveries[i]).getRecoveryStatus() ==
    ...
    function addActiveAgent(address newActiveAgent, address linkedRecovery) external override {
    ...
    function deleteActiveAgent(address _agentAddress) external override {
        ...
        for (uint i = 0; i < activeAgentsInRecoveries.length; i++) {
            if (activeAgentsInRecoveries[i] == _agentAddress) {
                activeAgentsInRecoveries[i] = activeAgentsInRecoveries[
                    activeAgentsInRecoveries.length - 1
                ];
                _agentExists = true;
                break;
            }
    ...
    // TODO add conditions => msg.sender == recovery.user and recovery satus != Over
    function getLegalSelectableAgents(
        address _currentRecovery
    ) external override {
        require(
            recoveryExist(_currentRecovery) &&
                tx.origin == IRecovery(_currentRecovery).user() &&
