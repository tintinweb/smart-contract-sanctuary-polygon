...
contract ERC721SignatureExtension is AccessControlEnumerable, ReentrancyGuard {
     ...
     */
    constructor(address _admin, address _signer, address _feeOperator, address payable _feeCustodian) payable{
        ...
        _grantRole(FEE_OPERATOR_ROLE, _feeOperator);
    }
     ...
     */
    function batchMintPayable(BatchMintPayableParams calldata _params) public nonReentrant payable {
        if(_params.tos.length != _params.tokenIds.length || (_params.tokenUris.length > 0 && _params.tos.length != _params.tokenUris.length) || _params.tos.length != _params.prices.length){
        ...
        bytes32 hash = batchMintPayableHash(BatchMintPayableHashParams(_params.baseContract, _params.tos, _params.tokenIds, _params.tokenUris, _params.prices, _params.expiresAt, _params.payees, _params.shareBps, _params.feeShareBps));
        
        if(validateSignature(hash, _params.sig, _params.signer)==false){
            revert InvalidSignature();
        }        

        IBaseERC721Extendable(_params.baseContract).batchMintExtension(_params.tos, _params.tokenIds, _params.tokenUris);
        ...
        for (uint256 i; i < _params.payees.length; ++i) {
            ...
            (bool payeeSent,) = payeeAddress.call{value : priceSum * _params.shareBps[i] / 10000}("");
            if(payeeSent==false){
     ...
     * @param _baseContract erc721 contract where the extension is registered and where the token check should be executed     
     * @param _tokenIds that should be checked. If at least one token is minted a information will be returned
     * @param _expiresAt timestamp that will compared to current block time.
     */
    function checkClaimEligibility(address _baseContract, uint256[] calldata _tokenIds, uint256 _expiresAt) external view returns (string memory){
        ...
        for (uint256 i; i < len; ++i) {
            try IERC721(_baseContract).ownerOf(_tokenIds[i]) returns (address) {
     ...
     */
    function batchMintPayableHash(BatchMintPayableHashParams memory _params) public pure returns (bytes32) {
     ...
     */
    function batchUpdateTokenUri(address _baseContract, uint256[] calldata _tokenIds, string[] calldata _tokenUris, bool _emitBatchMetadataUpdatedEvent) public onlyRole(DEFAULT_ADMIN_ROLE) {
        IBaseERC721Extendable(_baseContract).batchUpdateTokenUriExtension(_tokenIds, _tokenUris, _emitBatchMetadataUpdatedEvent);
    }
     ...
     */
    function updateBaseURI(address _baseContract, string memory _baseURI, bool _emitBatchMetadataUpdatedEvent) public onlyRole(DEFAULT_ADMIN_ROLE) {
        IBaseERC721Extendable(_baseContract).updateBaseURIExtension(_baseURI, _emitBatchMetadataUpdatedEvent);
    }
    ...
    function setFeeCustodian(address payable _feeCustodian) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if(_feeCustodian == address(0)){
            revert ZeroAddressNotAllowed();
        }
...
interface IBaseERC721Extendable is IERC165 {
     ...
     */
    function batchMintExtension(address[] calldata _tos, uint256[] calldata _tokenIds, string[] calldata _tokenUris) external;

    /**
     * @dev executes a single mint by an extension     
     */
    function mintExtension(address _to, uint256 _tokenId, string calldata _tokenUri) external;

    /**
     * @dev executes a base uri update by an extension     
     */
    function updateBaseURIExtension(string memory _baseURI,bool _emitBatchMetadataUpdatedEvent) external;

    /**
     * @dev executes a batch update of token uris by an extension     
     */
    function batchUpdateTokenUriExtension(uint256[] calldata _tokenIds, string[] calldata _tokenUris, bool _emitBatchMetadataUpdatedEvent) external;
