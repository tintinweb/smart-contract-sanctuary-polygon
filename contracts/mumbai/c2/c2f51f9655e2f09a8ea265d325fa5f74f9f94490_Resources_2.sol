...
contract Resources_2 is Initializable, IResourceLogic {
    IBaseLogic public baseLogic;
    using Resources for Resources.ID;

    modifier onlyGameContract() {
        require(msg.sender == baseLogic.gameSupport(), "m");
        _;
    }
    ...
    {
        if (resourceId == uint16(Resources.ID.C10)) {
            c10(id, position);
        } else if (resourceId == uint16(Resources.ID.C11)) {
            c11(id, position);
        } else if (resourceId == uint16(Resources.ID.C15)) {
            c15(id, position);
        } else if (resourceId == uint16(Resources.ID.C16)) {
            c16(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.C17)) {
            c17(id, position);
        } else if (resourceId == uint16(Resources.ID.C18)) {
            c18(id, position);
        } else if (resourceId == uint16(Resources.ID.C19)) {
            c19(id, position);
        } else if (resourceId == uint16(Resources.ID.D1)) {
            destroyPos = d1(id, position);
        } else if (resourceId == uint16(Resources.ID.D2)) {
            d2(id, position);
        } else if (resourceId == uint16(Resources.ID.D3)) {
            d3(id, position);
        } else if (resourceId == uint16(Resources.ID.D4)) {
            removePos = d4(id, position);
        } else if (resourceId == uint16(Resources.ID.D5)) {
            d5(id, position);
        } else if (resourceId == uint16(Resources.ID.D7)) {
            destroyPos = d7(id, position);
        } else if (resourceId == uint16(Resources.ID.D8)) {
            destroyPos = d8(id, position);
        } else if (resourceId == uint16(Resources.ID.E1)) {
            destroyPos = e1(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.E2)) {
            destroyPos = e2(id, position);
        } else if (resourceId == uint16(Resources.ID.F2)) {
            destroyPos = f2(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.T4)) {
            destroyPos = t4(id, position, randomNumber);
        }
    }

    function c10(uint256 id, uint16 position) internal {
        IResource_0(baseLogic.resourceLogic(0)).suits(id, position, 1);
    }

    function c11(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function c15(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
        bool isID1Exist = baseLogic.isItemExist(
            id,
            uint256(Items.ID.ID1),
            false
        );
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                ...
                adjResource == Resources.ID.B2 ||
                adjResource == Resources.ID.D5 ||
                adjResource == Resources.ID.M15 ||
                adjResource == Resources.ID.T6 ||
                adjResource == Resources.ID.J2
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
            ...
            if (
                isID1Exist &&
                (adjResource == Resources.ID.B9 ||
                    adjResource == Resources.ID.B11 ||
                    adjResource == Resources.ID.B4 ||
                    adjResource == Resources.ID.U1 ||
                    adjResource == Resources.ID.W4 ||
    ...
    function c16(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC12), false) &&
                percent < 30) || percent < 15
        ) {
            baseLogic.addNewResource(id, uint256(Resources.ID.M11), 1);
        }
    }

    function c17(uint256 id, uint16 position) internal {
        ...
        for (uint256 i = 0; i < 5; ++i) {
            ...
            if (resource == Resources.ID.C17) {
                listPos[uint256(countCrab)] = uint16(pos);
                ++countCrab;
            }
        }
        if (countCrab > 1) {
            uint16[] memory resPos = Helper.getArray(
                uint256(countCrab),
                listPos
            );
            IGameController(baseLogic.gameController()).updatePayout(
    ...
    function c18(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function c19(uint256 id, uint16 position) internal {
        uint256 countCultist = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.C19));
        int256 coin;
        if (countCultist > 1) {
            ...
            ).findPosition(id, uint256(Resources.ID.C19));
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                listPos
    ...
    function d1(
        ...
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            } else if (adjResource == uint256(Resources.ID.M3)) {
        ...
        if (countDestroy > 0) {
            IGameController(baseLogic.gameController()).updatePayout(
                ...
                0,
                0,
                40 * int256(countDestroy - 1)
            );
            return Helper.getArray(countDestroy, destroyPos);
        }
    }

    function d2(uint256 id, uint16 position) internal {
        uint256 count = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.D2));
        if (count > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
    ...
    function d3(uint256 id, uint16 position) internal {
    ...
    function d4(
        ...
        uint256 countRemove;
        uint16[] memory removePos = new uint16[](20);
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            uint16 adjPosition = adjacentPosition[i];
            Resources.ID adjResource = Resources.findResource(
                IGameController(baseLogic.gameController()).atPosition(
            ...
            if (
                adjResource == Resources.ID.S8 ||
                adjResource == Resources.ID.T9 ||
                adjResource == Resources.ID.A2 ||
                adjResource == Resources.ID.C17 ||
                adjResource == Resources.ID.G6 ||
                adjResource == Resources.ID.P3 ||
                adjResource == Resources.ID.S2 ||
                adjResource == Resources.ID.O5
            ) {
                baseLogic.boostPayout(id, uint16(Resources.ID.D4), position, 1);
                removePos[countRemove] = adjPosition;
                ++countRemove;
            }
        }
        return Helper.getArray(countRemove, removePos);
    }

    function d5(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
    ...
    function d8(
        ...
        bool isID4Exist = baseLogic.isItemExist(
            id,
            uint256(Items.ID.ID4),
            false
        );
        uint256 countDestroy;
        uint16[] memory destroyPos = new uint16[](20);
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.B9 || adjResource == Resources.ID.W4
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
    ...
    function e1(
        ...
        uint256 percent = ((random / 10 ** position) % 10000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC4), false) &&
                percent < 30) || percent < 10
        ) {
            IGameController(baseLogic.gameController()).updateMap(
                id,
                position,
                uint256(Resources.ID.C8)
            );
        }
        ...
        if (baseLogic.isItemExist(id, uint256(Items.ID.IF7), false)) {
            ...
            uint256 countDestroy;
            uint16[] memory destroyPos = new uint16[](20);
            for (uint256 j = 0; j < adjacentPosition.length; ++j) {
                uint16 adjPosition = adjacentPosition[j];
                Resources.ID adjResource = Resources.findResource(
                    IGameController(baseLogic.gameController()).atPosition(
                        id,
                        adjPosition
                ...
                if (
                    ...
                    adjResource == Resources.ID.G4 ||
                    adjResource == Resources.ID.O1
                ) {
    ...
    function e2(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.C19 ||
                adjResource == Resources.ID.W5 ||
    ...
    function t4(
        ...
        return
            IResource_0(baseLogic.resourceLogic(0)).rollDice(
                id,
                position,
                3,
                random
