...
contract JarvixERC721TokenAutoMinter is SecuredProcessProxy, PausableImpl {
    /** IERC721Auto interface ID definition */
    bytes4 public constant IERC721AutoInterfaceId = type(IERC721Auto).interfaceId;
    /** @dev Address of the NFT collection to which to mint NFTs */
    address public immutable nftAddress;
    bytes32 public immutable mintProcessName;

    constructor(address proxyHubAddress_, address nftAddress_, address securedProcessAddress_, bytes32 mintProcessName_)
    ProxyDiamond(proxyHubAddress_)
    SecuredProcessProxy(securedProcessAddress_, true, true, true) {
        if(!ERC165(nftAddress_).supportsInterface(IERC721AutoInterfaceId)) revert ProxyHub_ContractIsInvalid(IERC721AutoInterfaceId);
        nftAddress = nftAddress_;
        mintProcessName = mintProcessName_;
    }

    function mint(address to, uint256 amount) external whenNotPaused() {
        doProcess(mintProcessName, _msgSender(), amount);
        JarvixERC721TokenAuto(nftAddress).safeMint(to, amount);
    }

    /**
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(SecuredProcessProxy, AccessControlEnumerable) returns (bool) {
        return SecuredProcessProxy.supportsInterface(interfaceId) ||
               AccessControlEnumerable.supportsInterface(interfaceId);
    }
}

error JVX_ERC721_SetIDNotAllowed();
error JVX_ERC721_SetURINotAllowed();

contract JarvixERC721TokenManualMinter is SecuredProcessProxy, PausableImpl {
    /** IERC721Manual interface ID definition */
    bytes4 public constant IERC721ManualInterfaceId = type(IERC721Manual).interfaceId;
    /** @dev Address of the NFT collection to which to mint NFTs */
    address public immutable nftAddress;
    bytes32 public immutable mintProcessName;
    bool public immutable setID;
    bool public immutable setURI;

    constructor(address proxyHubAddress_, address nftAddress_, address securedProcessAddress_,
                bytes32 mintProcessName_, bool setID_, bool setURI_)
    ProxyDiamond(proxyHubAddress_)
    SecuredProcessProxy(securedProcessAddress_, true, true, true) {
        if(!ERC165(nftAddress_).supportsInterface(IERC721ManualInterfaceId)) revert ProxyHub_ContractIsInvalid(IERC721ManualInterfaceId);
        nftAddress = nftAddress_;
        mintProcessName = mintProcessName_;
        setID = setID_;
        setURI = setURI_;
    ...
    //function mintNext(address to) external {
        /*_beforeMint(false, false, 1);
        JarvixERC721TokenManual(nftAddress).safeMintNext(to);*/
    //}
    //function mint(address to, uint256 tokenID) external {
        /*_beforeMint(true, false, 1);
        JarvixERC721TokenManual(nftAddress).safeMint(to, tokenID);*/
    //}
    //function mintNextAndDefine(address to, string memory tokenURI_) external {
        /*_beforeMint(false, true, 1);
        JarvixERC721TokenManual(nftAddress).safeMintNextAndDefine(to, tokenURI_);*/
    //}
    //function mintAndDefine(address to, uint256 tokenID, string memory tokenURI_) external {
        /*_beforeMint(true, true, 1);
        JarvixERC721TokenManual(nftAddress).safeMintAndDefine(to, tokenID, tokenURI_);*/
    //}

    function mintNext(address[] memory to) public {
        _beforeMint(false, false, to.length);
        JarvixERC721TokenManual(nftAddress).safeMintNext(to);
    }
    function mint(address[] memory to, uint256[] memory tokenIDs) public {
        _beforeMint(true, false, to.length);
        JarvixERC721TokenManual(nftAddress).safeMint(to, tokenIDs);
    }
    function mintNextAndDefine(address[] memory to, string[] memory tokenURIs) public {
        _beforeMint(false, true, to.length);
        JarvixERC721TokenManual(nftAddress).safeMintNextAndDefine(to, tokenURIs);
    }
    function mintAndDefine(address[] memory to, uint256[] memory tokenIDs, string[] memory tokenURIs) public {
        _beforeMint(true, true, to.length);
        JarvixERC721TokenManual(nftAddress).safeMintAndDefine(to, tokenIDs, tokenURIs);
    }

    function _beforeMint(bool setID_, bool setURI_, uint256 amount) internal whenNotPaused() {
        if(setID != setID_) revert JVX_ERC721_SetIDNotAllowed();
        if(setURI != setURI_) revert JVX_ERC721_SetURINotAllowed();
        doProcess(mintProcessName, _msgSender(), amount);
    }
...
contract JarvixERC721TokenManual is BaseJarvixERC721Token, IERC721Manual {
    ...
    function safeMintNext(address[] memory to) external {
        ...
        for(uint256 i = 0 ; i < to.length ; i++) {
            tokenIDs[i] = totalSupply() + 1 + i;
        }
        safeMint(to, tokenIDs);
    }
    /**
    ...
    function safeMintNextAndDefine(address[] memory to, string[] memory tokenURIs) external {
        ...
        safeMintAndDefine(to, tokenIDs, tokenURIs);
    }
    /**
     * @dev This method is to be used to mint NFTs "manually", ie explicitly chosen by the caller, associated to their
     * applicable URIs
 ...
 * see: https://cryptomarketpool.com/erc721-contract-that-supports-sales-royalties/
 */
abstract contract RoyaltyImplementerProxy is ProxyDiamond, Ownable, IERC2981, RoyaltiesV2 {
     ...
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return interfaceId == type(IERC2981).interfaceId || // = 0x2a55205a Interface ID for Royalties from IERC2981, 0x2a55205a=bytes4(keccak256("royaltyInfo(uint256,uint256)"))
