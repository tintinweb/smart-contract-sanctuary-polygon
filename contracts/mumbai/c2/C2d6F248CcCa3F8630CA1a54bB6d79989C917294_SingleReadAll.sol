...
contract SingleReadAll is BasePluginWithRules {
    using UserLib for IRegistry;

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SINGLE_READ_ALL_COMMENT;
        registry = IRegistry(_registry);
    }

    function read(
        uint256 _postId
    ) external view returns(DataTypes.CommentInfo[] memory) {
        ...
        require(registry.hasAnyPayment(sender, postInfo.paymentType),
            "SingleReadAll: the user did not pay"
        );

        uint256 commentCount = ICommentData(registry.commentData()).getCommentCount(_postId);
        DataTypes.CommentInfo[] memory comments = new DataTypes.CommentInfo[](commentCount);

        for(uint256 i = 0; i < commentCount; i++ ) {
            vars.data = abi.encode(_postId, i + 1);
            DataTypes.CommentInfo memory comment = ICommentData(registry.commentData()).readComment(vars);
            comments[i] = comment;
        }
