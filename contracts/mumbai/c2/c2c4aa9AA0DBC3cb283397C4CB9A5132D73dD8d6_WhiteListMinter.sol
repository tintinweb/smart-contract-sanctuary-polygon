...
contract WhiteListMinter is Ownable{
    ...
    function setMaxMints(uint8 _maxMints) external onlyOwner {
     ...
     function validateEID(uint eID, uint dropEID) internal pure returns (bool valid){
        uint pants = dropEID%1000;
        uint shirts = (dropEID/1000)%1000;
        uint creature = (dropEID/1000000)%1000;

        if (pants > 4 || shirts > 5 || creature > 5 || pants < 1 || shirts < 1 || creature < 1 ){
                return false;
            }
        return true;
    ...
    function whiteListMint(
        ...
        external
        {
            ...
            require( validateEID( whitelistEID, _eID), "Not allowed to mint this creature");
