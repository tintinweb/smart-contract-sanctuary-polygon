...
contract MoonlitDao is ERC1155, AccessControlEnumerable, ERC1155Supply {
  ...
  function deposit(uint256 _amount) external payable {
    ...
    require(fundedAmount + _amount <= currentEditionMax - author.genesisEditionReserved, "Investment exceeds cap.");
  ...
  function setProjectData(string calldata _subtitle, string calldata _genre) external onlyRole(AUTHOR_ROLE) {
    require(fundedAmount == 0, "Investing already started");
    setSubtitle(_subtitle);
    setGenre(_genre);
  }
 
  function addContributor(address _contributor, uint256 _share) internal {
  ...
  function mintForCommunity(uint256 _amount) public onlyRole(AUTHOR_ROLE) {
    require(currentEdition == 1, "Only genesis edition minting");
    require(fundedAmount < currentEditionMax, "Cannot mint, genesis edition sold out");
