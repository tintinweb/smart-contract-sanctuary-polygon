...
contract Staking is ReentrancyGuard , Ownable  , AccessControl{
    ...
    event userStakeEvent( address indexed user , uint256 indexed stakeId , uint256 indexed amount , uint256 timestamp) ;
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function createStakingRound (   uint256 startTime , uint256 endTime , address rewardToken ,  uint256 stakedFund , uint256 rewardFund  , bool enabled , bool revokable ) public  onlyAdmin
    {
        uint256 id = stakingRoundId.current() ;
        StakingRound memory newRound = StakingRound ( id , startTime , endTime , rewardToken , stakedFund , rewardFund  , true , enabled , false , false , false , true , revokable );
        stakeRoundInfo[id] = newRound ; 
        stakingRoundId.increment();

    }

    function setRewardFundStakingRound ( uint256 id  , uint256 rewardFund) public  onlyAdmin
    {
    ...
    function changeStakingRoundTime ( uint256 id , uint256 startTime , uint256 endTime ) public  onlyAdmin
    {
    ...
    function setWithdrawProfitStakingRound ( uint256 id  , bool withdrawProfit ) public  onlyAdmin
    {
        stakeRoundInfo[id].withdrawProfit = withdrawProfit ; 
    }
    ...
    function createUserStake ( uint256 id  , uint256 stakeAmount) public 
    {
        ...
        require( success);
        UserStake memory stake = UserStake( msg.sender , block.timestamp , block.timestamp + 60 days , stakeAmount , 0  , 0 , 0 ) ; 
        StakeUserInfo[ msg.sender][id] = stake  ;

        stakeRoundInfo[id].stakedFund = stakeRoundInfo[id].stakedFund.add(stakeAmount ) ; 

        emit userStakeEvent ( msg.sender , id , stakeAmount , block.timestamp ) ;
    ...
    // function depositStake ( uint256 id , uint256 stakeAmount ) public
    // {
    //     require(  stakeRoundInfo[id].exist == true , "Stake Round Does Not Exist") ;
    //     require(  stakeRoundInfo[id].enabled == true , "Stake Round Is Disabled") ;
    //     require(  stakeRoundInfo[id].finished == false , "Stake Round Is Finished") ;
    ...
    function withdrawStake ( uint256 id , uint256 stakeAmount ) public
    {
    ...
    function getProfit( uint256 id ) public 
    {
        require(  stakeRoundInfo[id].exist == true , "Stake Round Does Not Exist") ;
        require(  stakeRoundInfo[id].enabled == true , "Stake Round Is Disabled") ;
        ...
        uint256 reward = StakeUserInfo[ msg.sender][id].stakedFund.mul( stakeRoundInfo[id].rewardFund  ).div( stakeRoundInfo[id].stakedFund ) ;
