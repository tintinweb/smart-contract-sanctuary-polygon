...
import "../entrypoint/EntryPoint.sol";
import "./BasePaymaster.sol";
/**
 * Paymaster that accepts USDC tokens as payment.
...
contract TokenPaymaster is BasePaymaster {
    ...
    function validatePaymasterUserOp(
        ...
        require(
            ...
            "USDC-TokenPaymaster: gas too low for postOp"
        );
        ...
        if (userOp.initCode.length != 0) {
            ...
            require(
                USDCToken.allowance(sender, address(this)) >= requiredPreFund,
                "USDC-TokenPaymaster: not enough allowance"
            );
        }
        require(
            USDCToken.balanceOf(sender) >= requiredPreFund,
            "USDC-TokenPaymaster: not enough balance"
        );
    ...
    function _validateConstructor(
        ...
        */
        ...
        //the 6th parameter is USDC token
        bytes32 tokenParam = bytes32(
            userOp.initCode[userOp.initCode.length - 110:] /* 64+46=110 */
        );
        require(
            address(uint160(uint256(tokenParam))) == address(USDCToken),
