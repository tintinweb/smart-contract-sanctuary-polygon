...
contract BreedPlanet is BreedPlanetBase, ChainlinkRng {
    ...
    function _breed(
        ...
        if (
            ...
            require(
                aprsToken.allowance(userAddress, address(this)) >= _aprsPrice,
                "Grant aprsToken approval to Sale Contract"
            );
            require(
                animaToken.allowance(userAddress, address(this)) >= _animaPrice,
                "Grant animaToken approval to Sale Contract"
            );
            if (!isDryRun && _aprsPrice > 0) {
                aprsToken.transferFrom(userAddress, address(this), _aprsPrice);
            }
        ...
        returns (uint256[] memory)
    {
        ...
        } else if (elementStruct.domainValue == elementStruct.earth) {
    ...
    // admin can set price
    // function setAprsAndAnimaPrices(uint256 _aprsPrice, uint256[] memory _animaPrices) external onlyOwner {
    function setAprsAndAnimaPrices(
        uint256 _aprsPrice,
        uint256[] memory _animaPrices
    ) public onlyOwner {
        require(_animaPrices.length == 5, "Prices length are wrong");
    ...
    function setApeironGodiverseCollectionNumber(
        ...
        require(
            _apeironGodiverseCollectionNumbers.length == 5,
            "Number length are wrong"
        );
        apeironGodiverseCollectionNumbers[
            0
        ] = _apeironGodiverseCollectionNumbers[0];
        apeironGodiverseCollectionNumbers[
            1
        ] = _apeironGodiverseCollectionNumbers[1];
        apeironGodiverseCollectionNumbers[
            2
        ] = _apeironGodiverseCollectionNumbers[2];
        apeironGodiverseCollectionNumbers[
            3
        ] = _apeironGodiverseCollectionNumbers[3];
        apeironGodiverseCollectionNumbers[
            4
        ] = _apeironGodiverseCollectionNumbers[4];
    }
