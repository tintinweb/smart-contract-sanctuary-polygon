...
contract Hhdd is Ownable, AccessControl {
    using SafeMath for uint256;

    bytes32 public constant SECOND_OWNER_ROLE = keccak256("SECOND_OWNER_ROLE");
    ...
    constructor() {
        _setupRole(SECOND_OWNER_ROLE, msg.sender);
    }
    ...
    function addSecondOwner(address account) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not an admin");
        grantRole(SECOND_OWNER_ROLE, account);
    }

    function removeSecondOwner(address account) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not an admin");
        revokeRole(SECOND_OWNER_ROLE, account);
    }
}