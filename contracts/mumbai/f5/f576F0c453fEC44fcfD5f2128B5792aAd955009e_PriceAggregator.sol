...
contract PriceAggregator is ChainlinkClient {
    using Chainlink for Chainlink.Request;
    
    // Contracts (constant)
    // GFarmTradingStorageV5 0xaee4d11a16B2bc65EDD6416Fb626EB404a6D65BD
    address tradingStorageAddress;
    address tokenDaiLPAddress;

    StorageInterface storageT = StorageInterface(tradingStorageAddress);
    LpInterface tokenDaiLp = LpInterface(tokenDaiLPAddress);

    // Contracts (adjustable)
    PairsStorageInterface public pairsStorage;
    ...
    constructor(address[] memory _nodes, PairsStorageInterface _pairsStorage, address _nftRewards, address _tradingStorageAddress, address _tokenDaiLPAddress) {
        ...
        tradingStorageAddress = _tradingStorageAddress;
        tokenDaiLPAddress = _tokenDaiLPAddress;
    }
    ...
    // Manage contracts
    function updatePairsStorage(PairsStorageInterface _pairsStorage) external onlyGov{
    ...
    function fulfill(bytes32 _requestId, uint _price) external recordChainlinkFulfillment(_requestId){
        ...
        if(r.initiated){
            ...
            PairsStorageInterface.Feed memory f = pairsStorage.pairFeed(r.pairIndex);
            (, int feedPrice1, , , ) = ChainlinkFeedInterface(f.feed1).latestRoundData();

            if(f.feedCalculation == PairsStorageInterface.FeedCalculation.DEFAULT){
                feedPrice = uint(feedPrice1*int(PRECISION)/1e8);
            }else if(f.feedCalculation == PairsStorageInterface.FeedCalculation.INVERT){
                feedPrice = uint(int(PRECISION)*1e8/feedPrice1);
            }else{
                (, int feedPrice2, , , ) = ChainlinkFeedInterface(f.feed2).latestRoundData();
            ...
            if(_price == 0 || priceDiff * PRECISION * 100 / feedPrice <= f.maxDeviationP){
                ...
                if(answers.length == minAnswers){
                    ...
                    CallbacksInterface.AggregatorAnswer memory a = CallbacksInterface.AggregatorAnswer(
                        orderId,
                        median(answers),
