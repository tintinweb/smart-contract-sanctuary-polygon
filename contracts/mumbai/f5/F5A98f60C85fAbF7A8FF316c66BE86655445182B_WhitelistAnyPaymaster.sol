...
contract WhitelistAnyPaymaster is BasePaymaster {

    mapping(address => bool) public contractWhitelist;
    mapping(address => bool) public callerWhitelist;

    function versionPaymaster() external view override virtual returns (string memory) {
        return "3.0.0-beta.3+brillianz.simple.ipaymaster";
    }

    function setContractWhitelist(address addr, bool isAllowed) public onlyOwner {
        contractWhitelist[addr] = isAllowed;
    }

    function setCallerWhitelist(address addr, bool isAllowed) public onlyOwner {
        callerWhitelist[addr] = isAllowed;
    }
    ...
    returns (bytes memory context, bool revertOnRecipientRevert) {
        ...
        if (callerWhitelist[relayRequest.request.from]) {
            return ("", false);
        }
        if (contractWhitelist[relayRequest.request.to]) {
            return ("", false);
        }
        revert('Neither the caller nor the contract was whitelisted.');
    }
