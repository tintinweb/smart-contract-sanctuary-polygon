...
interface IProxyReader is IERC721 {
  // Registry Functions
...
import "../interfaces/IProxyReader.sol";
import "../interfaces/IRegistry.sol";
import "../interfaces/IRecord.sol";
import "../interfaces/IERC721.sol";

contract ProxyReader is IProxyReader, OwnableUpgradeable {
    ...
    function tokenURI(uint256 tokenId) external view override returns (string memory) {
      ...
      return IERC721(_registry).tokenURI(tokenId);
    }
    ...
    function isApprovedOrOwner(address spender, uint256 tokenId) external view override returns (bool) {
      ...
      return IERC721(_registry).isApprovedOrOwner(spender, tokenId);
    }
    ...
    function resolverOf(uint256 tokenId) external view override returns (address) {
      ...
      return IRegistry(_registry).resolverOf(tokenId);
    }

    function namehash(string[] calldata labels) external view override returns (uint256) {
      return IRegistry(_registry).namehash(labels);
    }

    function balanceOf(address owner) external view override returns (uint256) {
      return IERC721(_registry).balanceOf(owner);
    }
    ...
    function getApproved(uint256 tokenId) external view override returns (address) {
      ...
      return IERC721(_registry).getApproved(tokenId);
    }

    function isApprovedForAll(address, address) external pure override returns (bool) {
      revert('ProxyReader: Unsupported');
    }
    ...
    function registryOf(uint256 tokenId) external view returns (address) {
      if (IRegistry(_registry).exists(tokenId)) return _registry;
      return address(0);
    }

    function _exists(uint256 tokenId) private view returns (bool) {
      return IRegistry(_registry).exists(tokenId);
    }

    function _ownerOf(uint256 tokenId) private view returns (address) {
      return IERC721(_registry).ownerOf(tokenId);
    }
