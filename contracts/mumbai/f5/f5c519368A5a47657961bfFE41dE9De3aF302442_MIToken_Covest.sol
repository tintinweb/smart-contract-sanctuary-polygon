...
contract MIToken_Covest is Initializable, ERC20Upgradeable, ERC20BurnableUpgradeable, PausableUpgradeable , UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event MintMI(address indexed admin,address indexed from,address indexed coin,uint amountIn,uint amountMIReceive,uint ExchangeRate);
    event UnWrapped(address indexed admin,address indexed from,address indexed coin,uint amountMIUnWrap,uint amountOut,uint ExchangeRate);
    event RateChanged(uint rate);

    function initialize(address _nameRegistry,uint256 _BalanceRate) public initializer {
       NR = INameRegistry(_nameRegistry);
       __ERC20_init(NR.poolName(), NR.poolId());
       __ERC20Burnable_init();
       __Pausable_init();
       __UUPSUpgradeable_init();
       
       exchangeRate = _BalanceRate;

       emit RateChanged(_BalanceRate);
    }
    ...
    modifier assetsChecker(address _coin) {
        require(AR.isSupportAssets(symbol(),_coin), "AdminRouter: This CU we not receive this CU.");
        _;
    }
    ...
    function mint(address user,address _coin, uint256 _value) public nonReentrant assetsChecker(_coin) isSuperManager {
        IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_coin);
        require(user != address(0), "user address is not valid.");
        require(_ERC20.allowance(user,address(this)) >= _value,"MIToken: You are not allowance");
        require(_ERC20.balanceOf(user) >= _value,"MIToken: Insufficient Balance");
        uint rate = exchangeRate;

        _ERC20.transferFrom(user,address(this),_value);
        //* (600*10^5)*10**(18-5) * rate / 100
        uint amount = (( _value * 10**(decimals() - _ERC20.decimals())) * rate ) / 100; 
        
        _mint(user, amount);

        emit MintMI(msg.sender,user,_coin,_value,amount,rate);
    }

    function unWrap(address user,address _coin,uint256 _amount) public nonReentrant assetsChecker(_coin) isSuperManager {
        uint rate = exchangeRate;
        uint256 saveAmount = (_amount*rate)/100;
        IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_coin);
        uint willburn = saveAmount * 10**(decimals() - _ERC20.decimals());
        require(user != address(0), "user address is not valid.");
        require(balanceOf(user) >= willburn,"MIToken: Balance Insufficient");
        _burn(user, willburn);
        require(_ERC20.balanceOf(address(this)) >= saveAmount,"MIToken: Contract Balance Insufficient");
        _ERC20.transfer(user,saveAmount);

        emit UnWrapped(msg.sender,user,_coin,_amount,saveAmount,rate);
    }

    function setRateBalance(uint _rate) public isSuperManager {
        require(_rate > 0);
        exchangeRate = _rate;
