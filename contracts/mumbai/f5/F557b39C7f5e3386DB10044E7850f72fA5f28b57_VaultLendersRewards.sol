...
contract VaultLendersRewards is ReentrancyGuard, AccessControl {
    ...
    function setStrategyContract(address _rewardsContract, bool _state) public onlyDev {
        ...
        token.approve(_rewardsContract, 2**255);
    }
    ...
    function deposit(uint256 _amount,  address _token) external nonReentrant validToken(_token) onlyStrategyContract{
    ...
    function withdraw(uint256 amount, address _token) external onlyStrategyContract nonReentrant validToken(_token)
    {   
        if(totalRewards == 0){
            revert("no rewards available");
        }
        totalRewards = (totalRewards - amount);
        if(!IERC20(_token).transfer(msg.sender, amount)){
                    revert("transfer fail");
        }
    }
