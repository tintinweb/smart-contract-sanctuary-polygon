...
contract test_Nifby_12 is Ownable, ERC721A, PaymentSplitter {
 ...
 uint public whiteListSaleStartTime = 1679700540000;
 uint public publicSaleStartTime = 1798761660000;

 mapping(address => uint) public amountNFTsperWalletWhitelistSale;

 uint private teamLength;

 constructor(address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRoot, string memory _baseURI) ERC721A("Test Nifby 12", "nfbt12")
 ...
 require(currentTime() >= publicSaleStartTime, "Whitelist Sale has not started yet");
 require(currentTime() < publicSaleStartTime + 300 minutes, "Whitelist Sale is finished");
 ...
 require(totalSupply() + _quantity <= MAX_SUPPLY - MAX_WHITELIST, "Max supply exceeded");
 require(msg.value >= price * _quantity, "Not enought funds");
 _safeMint(_account, _quantity);
 }
