...
contract GameResources is OwnableUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;
    using Resources for Resources.ID;
    mapping(Resources.ID => int256) public resourcesInitPayout;
    ...
    function setPayout(
        Resources.ID[] memory _res,
        int256[] memory payout
    ) public onlyOwner {
        for (uint256 i = 0; i < _res.length; ++i) {
            resourcesInitPayout[_res[i]] = payout[i];
        }
    }

    function setCategory(
        string memory sort,
        Resources.ID[] memory resource
    ) public onlyOwner {
