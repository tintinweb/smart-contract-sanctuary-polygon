...
contract Trade {
    ...
    function tradeAssetNative(Order memory order, Fee memory fee) internal virtual {
        if(order.nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721safeTransferFrom(IERC721(order.nftAddress), order.seller, order.buyer, order.tokenId);
        }
        if(order.nftType == BuyingAssetType.ERC1155)  {
        ...
        bool success;
        if(fee.platformFee > 0) {
            (success,) = owner.call{value: fee.platformFee}('');
            require(success);
        }
        if(fee.royaltyFee > 0) {
            (success,) = fee.tokenCreator.call{value: fee.royaltyFee}('');
            require(success);
        }
        (success,) = order.seller.call{value: fee.assetFee}('');
        require(success);
    }
    ...
    function buyAsset(Order memory order, Sign memory sign) public payable returns(bool) {
        ...
        require(((order.erc20Address != address(0)) || (msg.value == order.amount)), "Order/Value mismatch");
        verifySellerSign(order.seller, order.tokenId, order.unitPrice, order.erc20Address, order.nftAddress, sign);
        order.buyer = msg.sender;
        tradeAssetNative(order, fee);
