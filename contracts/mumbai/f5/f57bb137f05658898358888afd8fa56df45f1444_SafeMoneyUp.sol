...
contract SafeMoneyUp is ERC20, Ownable {
    ...
    function testvariables () public view returns (uint256 perTokenPrice){
    ...
    uint256 ethAmountInPool = address(this).balance;
     perTokenPrice = PRECISION.mul(PRECISION).div(supplyWithoutContractAndBurn).div(ethAmountInPool);
    }

    function getrealvalue (uint256 weiA) public view returns (uint256 tokensToBuy) {
        uint256 supplyWithoutContractAndBurn = totalSupply() - balanceOf(address(this)) - balanceOf(address(0)) - adminFeeTokens;
        uint256 ethAmountInPool = address(this).balance;
     uint256 perTokenPrice = PRECISION.mul(PRECISION).div(supplyWithoutContractAndBurn).div(ethAmountInPool);
     tokensToBuy = weiA * perTokenPrice;
    ...
    uint256 ethAmountInPool = address(this).balance;
    uint256 perTokenPrice = ethAmountInPool.mul(PRECISION).div(supplyWithoutContractAndBurn);
    uint256 tokensToBuy = msg.value.mul(PRECISION).div(perTokenPrice);
   ...
   function sellTokens(uint256 tokensToSell) public {
    ...
    if (totalSupply() > balanceOf(address(this)) + balanceOf(address(0))){
       ...
       uint256 ethAmountInPool = address(this).balance;
       uint256 perTokenPrice = ethAmountInPool.mul(PRECISION).div(supplyWithoutContractAndBurn);
       uint256 ethAmount = tokensToSell.mul(perTokenPrice).div(PRECISION);
