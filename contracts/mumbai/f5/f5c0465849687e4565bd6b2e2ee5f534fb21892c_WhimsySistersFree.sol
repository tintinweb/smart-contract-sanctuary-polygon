...
contract WhimsySistersFree is ERC721A, Ownable {
    ...
    uint public reservedSupply = 500;
    ...
    function mintFromReserve(uint amount, uint reserveMints, bytes calldata signature) public payable {
        require(block.timestamp < publicSaleStartTimestamp + mintFromReservePeriod, "Mint from reserve is not available");
        require(signature.length > 0 && recoverSignerAddress(_msgSender(), reserveMints, signature) == authorizedSigner, "tx sender is not allowed to presale");

        mint(amount, reserveMints);
    }
    ...
    function mint(uint amount, uint amountFromReserve) internal {
        ...
        require(mintedNFTs[_msgSender()] + amount <= maxMintsPerWallet + amountFromReserve, "No more mints for this wallet!");
        ...
        require(totalSupply() + amount + reserved <= MAX_SUPPLY, "Tokens supply reached limit");
