...
// File: contracts/REDREAMERProtocol.sol
...
contract REDREAMERProtocol is Ownable {
    mapping(address => bool) public validContracts;
    mapping(address => mapping(uint256 => bool)) public isRedeemed;

    event Redeem(address indexed _contract, uint256 indexed _tokenId, address indexed _to, string _type);
    ...
    function redeemWithTransfer(address _contract, uint256 _tokenId, address _to) public {
        ...
        emit Redeem(_contract, _tokenId, _to, "transfer");
    }
    ...
    function redeemWithBurn(address _contract, uint256 _tokenId) public {
        ...
        isRedeemed[_contract][_tokenId] = true;
        emit Redeem(_contract, _tokenId, address(0), "burn");
    }

    function redeemWithMark(address _contract, uint256 _tokenId) public {
        require(validContracts[_contract], "invalid contract address");
        require(!isRedeemed[_contract][_tokenId], "already redeemed");
        require(_isApprovedOrOwner(_contract, _tokenId, msg.sender), "ERC721: transfer caller is not owner nor approved");
        isRedeemed[_contract][_tokenId] = true;
        emit Redeem(_contract, _tokenId, address(0), "mark");
    }
    ...
    function setValidContract(address _contract) public onlyOwner {
