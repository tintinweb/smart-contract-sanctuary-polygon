    ...
    RoyaltiesV1Luxy
{
    ...
    uint256 public constant DROP_START_TIME = 1654199809;
    ...
    function mint(uint8 num) external payable {
        require(block.timestamp < DROP_START_TIME, "Drop hasn't started yet");
        ...
        require(
            bought[_msgSender()] <= bought[_msgSender()] + num,
            "Exceeds max mint per account"
        );
        if (block.timestamp < DROP_START_TIME + 300) {
            require(luxy.balanceOf(_msgSender()) > 1000 ether, "");
        }
        ...
        payable(teamAddress).call{value: fee}("");
        payable(artistAddress).call{value: msg.value - fee}("");
    }
