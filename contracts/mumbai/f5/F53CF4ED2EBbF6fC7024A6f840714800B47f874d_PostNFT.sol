 ...
 */
contract PostNFT is OxtripNFTBase, IPostNFT, VersionedInitializable {
    ...
    uint256 internal constant REVISION = 0;
    ...
    function initialize(string calldata name, string calldata symbol) external override {
        _royaltyBasisPoints = 1000; // 10% of royalties
        super._initialize(name, symbol);
        emit Events.PostNFTInitialized(block.timestamp);
    }

    function mint(address to, uint256 pubId) external override returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IOxtripHub(HUB).getPostContentURI(tokenId);
    }
