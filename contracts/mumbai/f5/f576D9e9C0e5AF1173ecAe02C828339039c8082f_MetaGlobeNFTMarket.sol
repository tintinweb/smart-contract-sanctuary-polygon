...
contract MetaGlobeNFTMarket is AccessControlEnumerable, Ownable, ReentrancyGuard, Pausable{
    ...
    returns(bool)
  {
    ...
    idToMarketItem[_itemId].buyer = _msgSender();
  ...
  //撤单
  function cancelOrder(address nftContract, uint256 itemId)
     public 
     nonReentrant 
     whenNotPaused
     onlyNonContract
     returns (bool) 
  {
    ...
    //触发撤单事件
    emit OnCancelOrder(idToMarketItem[itemId].seller, item.id, item.amount);
    ...
    returns (MarketItem[] memory) 
  {
    ...
    for (uint256 index = 0; index < itemCount; index++) {
      //合约内
      if(idToMarketItem[index+1].buyer == address(0)) {
