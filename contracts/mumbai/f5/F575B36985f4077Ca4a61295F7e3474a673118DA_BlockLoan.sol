...
contract BlockLoan is ERC20{

    using SafeMath for uint256;
      constructor (uint256 initialSupply) ERC20("HopeAccelerated Token", "HAT") {
        _mint(msg.sender, initialSupply);
    }


    mapping (address => uint) balances; //Lender or Borrower => balance
    ...
    function deposit (uint amount) public payable returns(uint){
        require(msg.value >= 1e16, 'Error, deposit must be >= than 0.01 ETH');
        balances[msg.sender] += amount;
        //isDeposited[msg.sender] = true;
        //emit Deposited(msg.sender, msg.value, nlock.timestamp);
        return amount;
    }
    ...
    public {
        ...
        applications[numApplications] = LoanApplication(true, numApplications, 
        msg.sender, duration, credit_amount, interest_rate, otherData);
        
        numApplications += 1;
        hasOngoingApplication[msg.sender] = true;
    }
    function grantLoan(uint appId) public payable{
        //check balance
        ...
        loans[numLoans] = Loan(true, 
        numLoans, applications[appId].borrower, 
        msg.sender, applications[appId].interest_rate, 
        applications[appId].duration,
        applications[appId].credit_amount,
        applications[appId].credit_amount, 0,
        block.timestamp, 0, appId);
    ...
    function repayLoan( uint amount, uint estimatedInterest, uint timeSinceLastPayment) public payable{
        //check balance
        ...
        // Update balance for interest
        balances[msg.sender] -= interest;
        balances[loan.Lender] += interest;
    ...
    function isApplicationOpen(uint index) public returns (bool) {

        LoanApplication memory app = applications[index];
        if(app.openApp) return true;
        else return false;

    }
    function isLoanOpen(uint index) public returns (bool) {

        Loan memory loan= loans[index];
        if(loan.openLoan == true) return true;
        else return false;
    ...
    function getLoanData(uint index) public returns (uint[] memory, address, address){
        ...
        return (numericalData, loans[index].borrower, loans[index].Lender);
    }

    function getNumApplications() public returns (uint){
        return numApplications;
    }
    function getNumLoans() public returns (uint){
        return numLoans;
    }
    function isBorrower(address account) public returns (bool){
        return borrowers[account].EXISTS;
    }
