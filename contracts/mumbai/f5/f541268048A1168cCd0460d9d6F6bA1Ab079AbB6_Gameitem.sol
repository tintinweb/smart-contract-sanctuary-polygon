...
contract Gameitem is ERC1155, Ownable {
  ...
  //minting event
  event minting(uint256 id, string indexed uri, uint256 amount);

  constructor(string memory _name) public ERC1155("") {
  ...
  function mint(string memory _uri, uint256 _amount) public payable{
    require(msg.value >= mintPrice, "Insufficient mint price");
    require(bytes(_uri).length > 0, "uri should be set");
    require(_amount > 0, "amount has to be greater than 0.");
    _mint(msg.sender, currentID, _amount, _uri, "");
    _tokenOwners[currentID].push(msg.sender);
    _tokenOwner[currentID][msg.sender] = true;
    _tokenIDs[msg.sender].push(currentID);
    currentID++;
    emit minting(currentID, _uri, _amount);
  }
