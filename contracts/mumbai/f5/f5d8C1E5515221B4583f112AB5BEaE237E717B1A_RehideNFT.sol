...
interface IRehideNFT {
    ...
    event ReadNote(uint256 indexed tokenId, address indexed reader, uint256 timestamp);
    event ReadNoteRewardsTransferred(address indexed creator, uint256 indexed tokenId, uint256 reward);
...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    function readNote(uint256 tokenId) public payable returns (string memory noteSmd) {
        ...
        require(note.tokenId > 0, "RehideNFT: Data for token not found");
        require(note.readsAvailable > 0, "RehideNFT: Data for token not found (max. reads exceeded)");
        require(note.expiry > block.timestamp, "RehideNFT: Data for token not found (expired)");
        require((bytes(note.smd).length != 0), "RehideNFT: Data for token not found (empty)");
        ...
        _noteReads[note.tokenId].push(noteRead);
        emit ReadNote(note.tokenId, _msgSender(), block.timestamp);

        note.readsAvailable -= 1;

        if (note.readsAvailable == 0){
            // burn(note.tokenId);
        ...
        if (readFee > 0) {
            ...
            emit ReadNoteRewardsTransferred(_platformWallet, tokenId, platformFee);

            address payable payableCreator = payable(note.creator);
            payableCreator.transfer(readFee - platformFee);
            emit ReadNoteRewardsTransferred(payableCreator, tokenId, readFee - platformFee);
        }
    }
    ...
    function doMintNFT(address recipient, string memory uri, string memory package) private returns (uint256) {
        _tokenIds.increment();
