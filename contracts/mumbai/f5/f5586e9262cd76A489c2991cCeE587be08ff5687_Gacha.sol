...
contract Gacha is IERC721Receiver {
    ...
    function checkRewardConditions(uint256 _indexOfTokenReward, address _challengerAddress) public view returns(bool){
        ...
        address requireBalanceNftAddress = IChallenge(IChallenge(_challengerAddress).erc721Address(0)).requireBalanceNftAddress();
        // check balance NFT, require Balance Nft Address
        if(IChallenge(requireBalanceNftAddress).balanceOf(IChallenge(_challengerAddress).challenger()) > 0) {
