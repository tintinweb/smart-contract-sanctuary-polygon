...
import "./interface/IERCPermit.sol";

contract AVANFT is ERC721PsiBurnable, ERC2981, Pausable, Ownable, IERC2612 {

    uint96 private DEFAULT_FEE_NUMERATOR = 500; // 500/10000 = 5%
    address private DEFAULT_ROYALTY_ADDRESS =
        0xED973Be655aDA7FF5EC1dB69A0d77a9Debb2Cb05; // royalty address
    ...
    constructor(string memory _name, string memory _symbol, string memory _baseUri) ERC721Psi(_name, _symbol) {
        _setDefaultRoyalty(DEFAULT_ROYALTY_ADDRESS, DEFAULT_FEE_NUMERATOR);
        baseUri = _baseUri;
    ...
    // IERC2612 - permit
    function permit(address owner, address spender, uint tokenId, uint deadline, uint8 v, bytes32 r, bytes32 s) external override {
        require(deadline > block.timestamp, "AVANFT: the signature is expired.");
        require(owner == ownerOf(tokenId), "AVANFT: the tokenId is not owned by owner");
        // recover signature
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                _DOMAIN_SEPARATOR,
                keccak256(abi.encode(_PERMIT_TYPEHASH, owner, spender, tokenId, _nonces[owner]++, deadline))
            )
        );
        address signerAddress = ecrecover(digest, v, r, s);
        require(signerAddress != address(0), "AVANFT: INVALID_SIGNATURE: RECOVER ERROR");
        require(signerAddress == owner, "AVANFT: INVALID_SIGNATURE: NOT MATCH");
    ...
    // End of Pausable contract

    function supportsInterface(bytes4 interfaceId)
        public
        view
        virtual
        override(ERC2981, ERC721Psi)
        returns (bool)
    {
        return
