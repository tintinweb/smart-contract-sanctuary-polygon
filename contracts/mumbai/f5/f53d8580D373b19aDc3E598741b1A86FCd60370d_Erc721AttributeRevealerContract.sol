...
abstract contract ItemRevealerBaseContract is OperatableDriverTemplateManagerContract {
    ...
    function add_interaction_driver(address _driver_addr) external onlyOwner{
        ...
        require(_driver_addr.supportsInterface(type(ITokenInteractionDriver).interfaceId));
...
//Contract to make on chain reveal.
//Example review pet appearance and attributes.
contract Erc721AttributeRevealerContract is ItemRevealerBaseContract {
    
    using ERC165Checker for address;

    mapping(address => mapping(uint256 => uint256[])) private to_reveals;

    event OnRevealed(address indexed addr,address indexed token_contract,uint256 token_id,string token_uri);

    constructor(string memory name_,string memory description_,address price_currency_contract_) 
        ItemRevealerBaseContract(name_,description_,price_currency_contract_) {
    }

    function is_support_input(address token_contract,uint256) internal view virtual override returns (bool){
        return token_contract.supportsInterface(type(IERC721TokenContract).interfaceId);
    }
    function is_support_contributor(address contibutor) internal view virtual override returns (bool){
        return contibutor.supportsInterface(type(IItemDistributeString).interfaceId);
    }
    function consume_input(address _addr_,uint256 template_id,uint256 applyCount) internal virtual override returns (bool){        
        
        to_reveals[_addr_][template_id] = new uint256[](0);
        ...
        uint256[] memory tokenIds = erc721.getUnrevealTokenOf(_addr_,applyCount);
        for(uint256 i = 0; i < tokenIds.length; i++)
            valid_count += (tokenIds[i] > 0) ? 1 : 0;

        require(valid_count >= applyCount,"INSUFFICIENT_TOKEN");
        to_reveals[_addr_][template_id] = tokenIds;
        return true;
    }  
    function operate_reveal(address _addr_,uint256 template_id,uint256 applyCount) internal virtual override returns (bool){
        ...
        address _erc721_driver_addr = get_interaction_driver(formula.input_token_contract);
        require(_erc721_driver_addr != address(0),"NO_DRIVER");

        IERC721InteractionDriver _erc721_driver = IERC721InteractionDriver(_erc721_driver_addr);
        uint256[] memory tokenIds =  to_reveals[_addr_][template_id];
        IItemDistributeString contributor = IItemDistributeString(formula.contributor_address);
        for(uint256 i = 0; i < applyCount; i++){
           string memory revealed_uri = contributor.consume_distributed(_addr_, false);
           _erc721_driver.updateURIOf(_addr_,tokenIds[i], revealed_uri,true);
            emit OnRevealed(_addr_,formula.input_token_contract,tokenIds[i],revealed_uri);
        }
        to_reveals[_addr_][template_id] = new uint256[](0);
        return true;
    }
