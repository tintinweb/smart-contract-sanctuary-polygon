...
contract Finixx {
    ...
    uint256 private constant poolTimeStep = 10 minutes;
    ...
    uint256[8] private PoolBusinessRequired = [100e18, 150e18, 200e18, 250e18, 300e18, 350e18, 400e18, 450e18];
    ...
    uint256 private constant lotteryDuration = 10 minutes;
    ...
    function register(address _ref) public {  
        ...
        for(uint256 i=0; i<registrationRewards.length; i++) {
            if(upline != address(0)) {
                ...
                if(userInfo[upline].registered[2] >= i && userInfo[upline].isActive && (userInfo[upline].curRevenue < userInfo[upline].selfDeposit.mul(_rewarding).div(baseDivider) || userInfo[upline].curPackage <= 0)) {
                    if(userInfo[upline].curRevenue.add(reward) > userInfo[upline].selfDeposit.mul(_rewarding).div(baseDivider) && userInfo[upline].curPackage > 0) {
                    ...
                    rewardInfo[upline].growth = rewardInfo[upline].growth.add(reward.mul(growthPercents).div(baseDivider));
                    rewardInfo[upline].lottery = rewardInfo[upline].lottery.add(reward.mul(lotteryPercents).div(baseDivider));
                    userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(reward);
                    userInfo[upline].curRevenue = userInfo[upline].curRevenue.add(reward);
                    userInfo[upline].registered[1] = userInfo[upline].registered[1].add(reward); 
                }

                userInfo[upline].registered[0] = userInfo[upline].registered[0].add(1); 
        ...
        for(uint256 i = 0; i < winners.length; i++){
            if(winners[i] != address(0)){
                ...
                if(curUser.isActive) {
                    ...
                    if(curUser.curRevenue.add(reward) > curUser.selfDeposit.mul(freezePercents).div(baseDivider)) {
