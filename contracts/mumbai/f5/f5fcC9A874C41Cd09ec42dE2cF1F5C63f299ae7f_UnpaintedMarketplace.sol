...
contract UnpaintedMarketplace is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    Unpainted721 private UERC721;   
    Unpainted1155 private UERC1155;   
    ...
    constructor(address _erc721, address _erc1155) {
        ...
        UERC721 = Unpainted721(_erc721);
        UERC1155 = Unpainted1155(_erc1155);
    }
    ...
    {
        (receiver, royaltyAmount) = UERC721.royaltyInfo(tokenId, price);
    ...
    function getRoyalties1155(uint256 tokenId, uint256 price)
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        (receiver, royaltyAmount) = UERC1155.royaltyInfo(tokenId, price);
        if (receiver == address(0) || royaltyAmount == 0) {
            return (address(0), 0);
        }
        return (receiver, royaltyAmount);
...
//-----------------------------------------------------------------------------ERC721--------------------------------------------------------------------------------//
    ...
    {
        require(
            UERC721.ownerOf(tokenId) == msg.sender,
            "Sender does not own the item"
        );
        require(price > 0, "Price must be at least 1 wei");

        require(
            UERC721.getApproved(tokenId) == address(this),
            "Market is not approved"
        );
        UERC721.transferFrom(msg.sender, address(this), tokenId);
        ...
        nonReentrant
    {
        ...
        UERC721.transferFrom(address(this), msg.sender, tokenId);
        ...
        nonReentrant
    {
        ...
        UERC721.transferFrom(address(this), msg.sender, idToMarketItem_.tokenId);
...
function createTokenAuction721(
        ...
        require(msg.sender == UERC721.ownerOf(_tokenId), "Not the owner of tokenId");
        ...
        UERC721.safeTransferFrom(owner, address(this), _tokenId);
    ...
    */
    ...
    function cancelAuction721(uint256 _tokenId) external nonReentrant {
        ...
        UERC721.safeTransferFrom(address(this), auction.seller, _tokenId);
    }
...
//-----------------------------------------------------------------------------ERC1155--------------------------------------------------------------------------------//
     ...
     function listToken1155(uint256 tokenId, uint256 amount, uint256 price, address[] memory privateBuyer) external nonReentrant returns(uint256) {
        
        require(amount > 0, "Amount must be greater than 0!");
        require(UERC1155.balanceOf(msg.sender, tokenId) >= amount, "Caller must own given token!");
        require(UERC1155.isApprovedForAll(msg.sender, address(this)), "Contract must be approved!");
    ...
    function purchaseToken1155(uint256 listingId, uint256 amount) public payable nonReentrant {
        ...
        require(UERC1155.balanceOf(idToListing1155[listingId].seller, idToListing1155[listingId].tokenId) >= amount, "Seller doesn't have enough tokens!");
        ...
        UERC1155.safeTransferFrom(idToListing1155[listingId].seller, msg.sender, idToListing1155[listingId].tokenId, amount, "");
    ...
    function viewListingById1155(uint256 _listId, uint256 _tokenId) public view returns(Listing1155 memory) {
        for (uint256 i = 0; i < listingsArray1155.length; i++){
            if (listingsArray1155[i].tokenId == _tokenId && listingsArray1155[i].listingId == _listId) {
