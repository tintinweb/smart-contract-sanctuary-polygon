...
/**
 * @file Market.sol
 * @author lakshmi <[email protected]>
...
contract LIFEVERSEMarket is ReentrancyGuard {
 ...
 function getadminaddress(address payable adminaddress, uint256 FEE) external payable returns(address, uint256){
   ...
   //     payable(owner).transfer(listingprice);
     ...
     function Cancel(address nftContract, uint256 itemId)public payable nonReentrant{
            uint tokenId = idToMarketToken[itemId].tokenId;
            require(tokenId == tokenId,'please Enter the valid TokenId');
             IERC721(nftContract).transferFrom(address (this),msg.sender , tokenId);
       }
