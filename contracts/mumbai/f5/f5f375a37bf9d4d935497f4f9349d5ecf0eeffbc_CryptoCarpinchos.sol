    ...
    function _beforeTokenTransfers(
        ...
        do {
            if (from != address(0) && from != to) {
                _removeTokenFromOwnerEnumeration(from, updatedIndex);
            }

            if (to != address(0) && to != from) {
                _addTokenToOwnerEnumeration(to, updatedIndex);
            }
...
contract CryptoCarpinchos is ERC721A, Ownable {
    ...
    function buyWithCard(address user, uint256 amount) external onlyAllowed {
        require(open, "Contract closed");
        if (amount > mintMax) {
            amount = mintMax;
        }
