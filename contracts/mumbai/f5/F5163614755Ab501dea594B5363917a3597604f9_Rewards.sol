...
/**
 * @title Rewards - Contract to facilitate reward redemption in the PV Gaming Ecosystem
 * 
 * @author Jack Chuma
 */
contract Rewards is IRewards, AccessControl {
    ...
    event CraftingFeeSet(uint256 fee);
    event ResourcesAddressSet(address resourcesAddress);
    event CraftingAddressSet(address craftingAddress);
    ...
    /**
     * @notice Called by contract owner to update stored address for GameItems contract
    ...
    function submitAssetPack(
        ...
        pow.transferFrom(msg.sender, address(this), _powIn);
        resources.mintBatch(address(assetPool), _resourceIds, _resourceAmounts);
        gameItems.mintBatch(address(assetPool), _itemIds, _itemAmounts);
    ...
    function claimResourceRewards(
        ...
        for (uint i = 0; i < _rewards.length; ) {
            ...
            pow.transfer(address(assetPool), _reward.powIn);
    ...
    function claimGameItemRewards(
        ...
        for (uint i = 0; i < _rewards.length; ) {
            ...
            uint256 _powFee;
            uint256 _powToTransfer;
            unchecked{ 
                _powFee = _powIn * craftingFee / 1000000000000000000;
                _powToTransfer = _powIn - _powFee;
            }

            pow.transfer(address(assetPool), _powToTransfer);
...
interface IRewards {
    ...
    function claimResourceRewards(ResourceReward[] calldata _rewards) external;

    function claimGameItemRewards(GameItemReward[] calldata _rewards) external;
}
