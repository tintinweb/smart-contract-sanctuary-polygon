...
// File: .deps/remix-tests/1_test.sol
...
contract Certisure is Ownable, AccessControl {
    ...
    function checkURL(string memory _url) private pure returns (bool) {
        string memory ipfsPrefix = "https://certisure.infura-ipfs.io/ipfs/";
        require(bytes(_url).length > bytes(ipfsPrefix).length);
        string memory urlPrefix = substring(_url, 0, bytes(ipfsPrefix).length);
...
contract CertisureCloner {
    //Creating an array of neb crew addresses
    Certisure[] public CertisureAddresses;
    address public implementationAddress;
    function addCertisure() public {

        //Creating a new crew object, you need to pay //for the deployment of this contract everytime - $$$$
        Certisure CertisureAddress = Certisure(Clones.clone(implementationAddress));

        // since the clone create a proxy, the constructor is redundant and you have to use the initialize function
        CertisureAddress.initialize(); 

        //Adding the new crew to our list of crew addresses
        CertisureAddresses.push(CertisureAddress);
    }

    function testCertisure(uint256 arr) public returns (string memory){
        string memory result = Certisure(CertisureAddresses[arr]).test();
        return (result);
    }
}
