...
contract KnightFacet is IKnight {
  ...
  function testAddr() external view returns (address) {
    return address(this);
  }

  function testItemAddr() external view returns (address) {
    return address(s.Items);
  }
  ...
  function randomKnightId() private view returns (uint256 item_id) {
    ...
    do {
      salt++;
      item_id = uint256(keccak256(abi.encodePacked(block.timestamp, tx.origin, salt)));    
