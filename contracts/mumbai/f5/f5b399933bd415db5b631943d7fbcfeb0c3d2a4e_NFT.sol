...
contract NFT is ERC721, Ownable {
    ...
    function mintTo(address recipient, uint256 _value) public returns (uint256) {
        if (_value < price-((price*maxDiscount)/10000)) {
            revert MintPriceNotPaid();
        }

        require(IERC20(paymentToken).transferFrom(msg.sender, address(this), _value));
