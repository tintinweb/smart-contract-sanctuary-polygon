...
contract EventTicketSystem is ERC721, Pausable, Ownable {
    ...
    /**
    * @dev Smart Contract parameterrs provided at deployment
    */ 

    // The owner address (organizer)
    address payable public withdrawalAddress;

    /**
    * @dev Event parameterrs provided at deployment
    */ 

    // Event identifier
    string public eventId;

    // The UNIX timestamp of the doorTime
    uint64 public eventDoorTime;
    ...
    // The number of tickets available (implies maximumAttendeeCapacity)
    uint64 public ticketSupply;

    // Initial ticket price
    uint256 public initialTicketPrice;

    // Maximum ticket resell price
    uint64 public maxPriceFactor;

    // Percentage of the ticket price counted as fee when it's transferred
    ...
    */
    ...
    {
        withdrawalAddress = _owner != address(0) ? payable(_owner) : payable(msg.sender);
        eventId = _eventID;
        eventDoorTime = uint64(_eventDoorTime);
        eventStartDate = uint64(_eventStartDate);
        eventEndDate = uint64(_eventEndDate);
...
/* MODIFIERS */
    ...
    */
    modifier priceCap(uint256 _price) {
        uint256 _maxPrice =  initialTicketPrice.div(100).mul(maxPriceFactor);
    ...
    */
    modifier EventNotStarted() {
        require((uint64(block.timestamp) < eventStartDate),"event has already started");
        _;
    }
...
/* GETTERS */
    ...
    */
    ...
    returns (
        ...
        bool forSale,
        bool used,
        string memory ticketSeat,
        address underName,
        address owner
    )
    {
        string memory _ticketSeat = string(tickets[_id].ticketSeat);
        return (
            ticketNumber = uint256(tickets[_id].ticketNumber),
            price = uint256(tickets[_id].price),
            forSale = bool(tickets[_id].forSale),
            used = bool(tickets[_id].used),
            ticketSeat = _ticketSeat,
            underName = address(tickets[_id].underName),
            owner = address(tickets[_id].owner)
        );
    }
    ...
    */
    ...
    /** 
    * @dev Returns the seat number of a specific ticket
    */
    function getTicketSeat(uint256 _ticketId) 
    public 
    view 
    returns (string memory) 
    {

        return tickets[_ticketId].ticketSeat;
    }
    ...
    */
    function _createTicket(
        ...
        string memory ticketSeat // TODO: implement ticket seats functionality
    ) 
    internal 
    EventNotStarted 
    isAvailable 
    returns (uint256) 
    {
        // TODO: Implement other ticket details
    ...
    */
    ...
    {   
        ...
        {
            payable(msg.sender).transfer(msg.value.sub(initialTicketPrice));
        }

        uint256 _ticketId = _createTicket(msg.sender, "");
    ...
    */
    ...
    whenNotPaused 
    {
        ...
        {
            payable(msg.sender).transfer(msg.value.sub(_priceToPay));
        }
