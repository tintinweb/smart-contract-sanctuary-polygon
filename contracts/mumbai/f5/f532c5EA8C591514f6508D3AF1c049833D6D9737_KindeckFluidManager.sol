...
contract KindeckFluidManager is Ownable, IERC777Recipient {
    ...
    /**
     * @dev Kan addresses be hardcoded with addresses here or how does that work?
     */
    function initialize(
        ISuperfluid _host,
        ...
        // require(owner == address(0), "ERROR: Contract already initalized");
        ...
        console.log("Beneficary address ", _beneficiary);
    ...
    function setSuperToken(ISuperfluidToken _token) external onlyOwner {
        token = _token;
    }
    ...
    function setCommissionUnits(address _beneficiary, uint128 _comissionUnits)
        external
        onlyAdmin
    {
        _idaLib.updateSubscriptionUnits(
    ...
    function tokensReceived(
        ...
        /**
         * @dev Check that tokens equal our supertoken. Need to be able to update that.
         * require(msg.sender == token, "Invalid token SuperToken");
         */
        distribute();
    }
    ...
    function distribute() internal {
        ...
        console.log("Balance to distribute", balanceToDistribute);

        _idaLib.distribute(token, INDEX_ID, balanceToDistribute);
    }

    /**
     * @dev what if the parent hasn't approved the subscription?
     * - Do we need another claim for parent subscribers?
     */
    function claim() external {
        _idaLib.claim(token, address(this), INDEX_ID, beneficiary);
    }

    /**
     * @dev do we still need to approve subscriptions? Maybe do that on tokenReceived as well?
     * - How to approve parent's subscription?
     */
    function approveSubscription(address _publisher) external {
        _idaLib.approveSubscription(token, _publisher, INDEX_ID);
    }

    function getSubscriptionByIndexId(address _subscriber)
        external
        view
        returns (
            bool exist,
    ...
    /**
     * @dev If we want to cancel a subscription. Maybe if a user doesn't pay their kindeck subscription?
     * - We will then manully (server side) need to update subscriptionUnits again when it's activated
     */
    function revokeSubscription() external onlyAdmin {
        _idaLib.revokeSubscription(token, address(this), INDEX_ID);
    }
}
