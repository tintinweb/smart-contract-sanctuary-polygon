...
import "./ITokenVestingUpgradeable.sol";

/**
 * @title Bitorio Vesting
 */
contract BitorioVesting is
    Initializable,
    ITokenVestingUpgradeable,
    OwnableUpgradeable,
    ReentrancyGuardUpgradeable
{
    ...
    /**
     * @dev Initailizes a vesting contract.
    ...
    /**
     * @dev Returns the vesting schedule ids for given hodler address
    ...
    {
        uint256 vestingSchedulesCount = getVestingSchedulesCountByBeneficiary(
            holder
        );
        bytes32[] memory vestingScheduleIds = new bytes32[](
            vestingSchedulesCount
        );
        uint256 count;
        for (uint256 i = 0; i < getVestingSchedulesCount(); i++) {
            bytes32 vestingScheduleId = getVestingIdAtIndex(i);
            VestingSchedule memory vestingSchedule = getVestingSchedule(
                vestingScheduleId
            );
            if (vestingSchedule.beneficiary == holder) {
                vestingScheduleIds[count++] = vestingScheduleId;
            }
        }
     ...
     */
    function withdraw(uint256 amount) public override nonReentrant onlyOwner {
        require(
    ...
    /**
     * @notice Release vested amount of tokens for all vesting scheduled
     */
    function releaseAll() public override onlyOwner {
        for (uint256 i; i < vestingSchedulesIds.length; i++) {
            bytes32 vestingScheduleId = vestingSchedulesIds[i];
            uint256 amount = computeReleasableAmount(vestingScheduleId);
            if (amount > 0) release(vestingScheduleId, amount);
        }
    }
     ...
     */
    function getWithdrawableAmount() public view override returns (uint256) {
