...
contract VLFIV2 is ERC20Upgradeable, ERC20PermitUpgradeable, AccessControlUpgradeable, ERC20VotesUpgradeable {
...
/// @notice initialize function called by Openzeppelin Hardhat upgradeable plugin
 ...
 /*** Functions to call after upgradation */
    ...
    function setPoolName(string memory _poolName) external onlyRole(MANAGER_ROLE) {
        poolName = _poolName;
    }
    ...
    function initLpTokenPrice(uint256 _initLpTokenPrice) external onlyRole(MANAGER_ROLE) {
    ...
    function stake(address onBehalfOf, uint256 amount) public {
        require(amount > 1 && amount <= STAKED_TOKEN.balanceOf(msg.sender), "HOUSEPOOL: INVALID AMOUNT");
    ...
    function unStake(address to, uint256 amount) public {
        require(amount > 1  , "HOUSEPOOL: INVALID AMOUNT");
        if (cooldownActive) {
    ...
    function getMaxWithdrawal(address _user) public view returns (int256) {
        ...
        if (((userShare * voi.expectedValue)) < userShare * userEVTracker[_user]) {
            maxUserWithdrawalAmount = (userShare) * int256(totalValueLocked) / int256(10**pool_decimals);
        } else {
             maxUserWithdrawalAmount =  ((userShare * (int256(totalValueLocked) - voi.expectedValue)) /int256(10**pool_decimals) +
                (userShare * userEVTracker[_user]) / int256(10**pool_decimals));
        }
