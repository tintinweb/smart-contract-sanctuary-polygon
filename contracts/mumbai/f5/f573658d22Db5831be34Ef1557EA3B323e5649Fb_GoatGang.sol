...
//(_, () /\ ~|~   (_, /\ |\| (_,
contract GoatGang is ERC721AQueryable, Ownable, ReentrancyGuard {
	...
	//prices
	uint256 public tokenPrice = 1 ether; //normal price (in MILK)
	uint256 public tokenPriceWithItems = 0.5 ether; //normal price (in MILK)

	uint256 public maxSupply = 5000;
	uint256 public publicSaleStartTime = block.timestamp - 1; //change to real;
	address private _signerAddress;
	mapping(string => bool) private _usedNonces;
	address public milk = 0x1F3f3Aba9982570143C7B3F8E91091a3E5A41fc1; //MILK token

	event CustomAction(uint256 nftID, uint256 value, uint256 actionID, string payload);

	function delete2Me2() internal {}
	...
	constructor() ERC721A("Goat Gang", "GoatG") {
		_mint(msg.sender, 1);
	...
	function withItemsMint(
		...
		require(totalSupply() + 1 <= maxSupply, "presale out of stock");
		require(!_usedNonces[nonce], "nonce already used");

		IMilk(milk).burn(msg.sender, tokenPriceWithItems);
	 ...
	 * @dev admin mint
	 */
	function adminMint(uint16 qty, address to) external onlyOwner {
	...
	/**
	@dev sets the contract address for milk
	 */
	function setMilkToken(address cAddress) external onlyOwner {
		milk = cAddress;
	}
