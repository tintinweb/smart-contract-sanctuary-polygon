...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function _judgeAuth() internal view returns (bool){
        if(msg.sender != owner()) {
            return isVisit;
            // require(_judgeAuth(), "not allow visit");
        }
        return true;
    }
    function _judgeMint() internal view returns (bool){
        if(msg.sender != owner()) {
            return isMint;
            // require(_judgeMint(), "not allow mint");
        }
        return true;
    }
    function _setIsMint() public onlyOwner {
        isMint = !isMint;
    }
    function _setVisit() public onlyOwner {
        isVisit = !isVisit;
    }
    /**
     * @dev See {IERC165-supportsInterface}.
...
contract MetaCommon is ERC721 {
    using Counters for Counters.Counter;
    using Strings for uint256;
    Counters.Counter _tokenIds;
    mapping(uint256 => string) _tokenURIs;
    mapping(uint256 => uint256) isSale;
    uint256 minMintprice = 0.02 ether;
    uint256 minFromTransfer = 0.01 ether;
    ...
    constructor() ERC721("MetaCommon", "MC") {}
    ...
    function mintNFT(address recipient, string memory uri) public payable returns (uint256) {
        require(_judgeAuth(), "not allow visit");
        require(_judgeMint(), "not allow mint");
        require(msg.value >= minMintprice, "not allow mint");
    ...
    function mintManyNFT(uint256 num, address recipient, string memory uri) public payable{
        require(_judgeAuth(), "not allow visit");
        require(_judgeMint(), "not allow mint");
        require(msg.value >= num*minMintprice, "not allow mint");
    ...
    function manyStartSale(uint256[] memory tokenIds, uint256 price) public {
    ...
    function tokenprice(uint256 tokenId) public view returns (uint256 price){
        uint256 tokenPrice = isSale[tokenId];
        return tokenPrice; 
    }
    ...
    function safeTransfer(address from, address to, uint256 tokenId) public payable {
        ...
        //IERC20(0x0000000000000000000000000000000000001010).transfer(to, msg.value/2);
        (bool os,) = payable(from).call{value: msg.value * 95 / 100}("");
    ...
    function fromTransfer(address from, address to, uint256 tokenId) public payable {
        // require(_isApprovedOrOwner(_msgSender(), tokenId), "ERC721: transfer caller is not owner nor approved");
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(from != address(0), "from is the zero address");
        require(to != address(0), "to is the zero address");
        require(!(isSale[tokenId] > 0), "token is sell");
        require(ownerOf(tokenId) == msg.sender, "not owner");
        require(msg.value >= minFromTransfer, "token is not enough");
        _safeTransfer(from, to, tokenId, "");
    }
}