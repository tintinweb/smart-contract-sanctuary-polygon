...
contract PowerUpScorer is Initializable, OwnableUpgradeable {
    ...
    /// @notice The energy ID that will be increased when a user is powered up.
    uint8 public powerUpEnergyId;

    /// @notice The energy ID that will be increased when a user powers up any other user daily.
    uint8 public dailyEnergyId;

    /// @notice The energy ID that will be increased when a user powers up any other user daily.
    uint8 public experienceEnergyId;
    ...
    function initialize(address _energyMinter) public initializer {
        ...
        powerUpEnergyId = EnergyConstants.PWATT;

        dailyEnergyScore = 1 ether;
        dailyEnergyId = EnergyConstants.LWATT;

        experienceScore = 1 ether;
        experienceEnergyId = EnergyConstants.XWATT;
    ...
    function powerUp(address to) external {
        ...
        if (userLastPowerUpDate != currentDay) {
            ...
            // Mint dailyEnergyId based on the power-up streak
            uint256 dailyEnergyAmount = dailyEnergyScore * userPowerUpStreak;
            energyMinter.mint(from, dailyEnergyId, dailyEnergyAmount);
        }

        userPowerUpCounter += 1;
        if (experienceMilestones[userPowerUpCounter]) {
            // Mint epxerience energy
            energyMinter.mint(from, experienceEnergyId, experienceScore);
        }

        powerUpCounter[from] = userPowerUpCounter;

        // Powerup a new user
        energyMinter.mint(to, powerUpEnergyId, powerUpScore);
