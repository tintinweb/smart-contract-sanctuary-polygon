...
import "./EpicFootball.sol";

contract EpicFootballEvent is ReentrancyGuard, Ownable {
    EpicFootball public nftContract;
    ...
    constructor(
        ...
        nftContract = EpicFootball(_nftContract);
    }
    
    function withdrawAmount() external payable{
        require(msg.sender == recipientAddress,"Sender is not Recipient Address");
     ...
     * @param _limitBox uint16: limit quantity of the NFT
     ...
     * @param _roundId uint8: id of the event
     */
    function getWhitelisted(uint8 _roundId) public payable {
        require(
            _purchaseRoundExist[_roundId] == true,
            "Purchase round does not exist"
        );
        ...
        PurchaseRound storage purchaseRound = _purchaseRounds[_roundId];
        require(msg.value >= purchaseRound.price, "Not enough MATIC");
     ...
     */
    function buyNFT(uint8 _roundId, string memory _tokenUri)
        public
        nonReentrant
        roundAvailable(_roundId)
    {
        ...
        require(
            purchaseRound.supply >= purchaseRound.minted + 1,
        ...
        if (roundIsPrivate[_roundId]) {
            require(
                roundWhiteList[_roundId][buyer] == true,
                "Buyer is not in the white list"
            );
        }
        purchaseRound.minted += 1;
        nftContract.mint(buyer, _tokenUri);
...
contract EpicFootball is ERC721Tradable,AccessControl{
    using SafeMath for uint256;
    ...
    constructor(address _proxyRegistryAddress)
        ERC721Tradable("Epic Football", "EFB", _proxyRegistryAddress)
    {
    ...
    /**
     * @dev Mints a token to sender address with a tokenURI.
     * @param _tokenUri uri of the token
     */
    function whitelistMint(string memory _tokenUri) public {
        require(block.timestamp >= whitelistTime,"Sale not started");
