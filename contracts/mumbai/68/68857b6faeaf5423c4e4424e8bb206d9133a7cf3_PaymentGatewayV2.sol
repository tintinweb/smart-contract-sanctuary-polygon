    ...
    UUPSUpgradeable
{
    ...
    modifier onlyWhiteList(address _addr) {
        require(whitelists[_addr], "not whitelist address");
        _;
    }
    ...
    function userPay(
        ...
        return
            ERC20Upgradeable(vndtToken).transferFrom(
                msg.sender,
                address(this),
                amount
            );
    ...
    function userPayWithPermit(
        ...
        // Process pay
        return userPay(amount, sessionId);
    }
    ...
    function merchantSend(
        ...
        address recipient,
        string calldata sessionId
    ) public onlyWhiteList(msg.sender) returns (bool) {
    ...
    function merchantBatchSend(
        ...
        string calldata sessionId
    ) public onlyWhiteList(msg.sender) {
    ...
    function addWhiteLists(address[] calldata _addrs) external onlyOwner {
    ...
    function removeWhiteLists(address[] calldata _addrs) external onlyOwner {
        for (uint256 i = 0; i < _addrs.length; i++) {
            address addr = _addrs[i];
