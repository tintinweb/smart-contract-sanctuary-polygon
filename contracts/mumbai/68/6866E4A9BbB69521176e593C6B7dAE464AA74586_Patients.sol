...
contract Patients{
    ...
    mapping(address=>mapping(address=>bool)) dataAccess;
    ...
    function viewPatientInfo(address _address) external view returns(Patient memory){
        ...
        require(onlyOwnerAccess(_address) || dataAccess[_address][msg.sender]  || (msg.sender==owner),"You don't have access to info.");
    ...
    function makeAppointment(address payable _docAddress, string calldata _date, string calldata _time, uint _fee, bool _online, bool _shareData)
     payable external returns(bool){
        ...
        Appointment memory appointment = Appointment(_docAddress, msg.sender, _date, _time, _fee, _online, _shareData);
        patientAppointments[msg.sender].push(appointment);
        dataAccess[msg.sender][_docAddress] = _shareData;
    ...
    //Function through which a patient can grant or revoke acces to their data
    function grantOrRevokeDataAccess(address _address, uint _index, bool _access) external{
        require(onlyOwnerAccess(_address),"Access denied");
        require(_index>=0 && _index<=patientAppointments[msg.sender].length,"Invalid appointment id");
        dataAccess[_address][patientAppointments[msg.sender][_index].docId] = _access;
    }
    ...
    function updateMedicalRecords(address _address, uint _index, string memory _record) external {
        require(onlyOwnerAccess(_address) || dataAccess[_address][msg.sender],"Access denied");
