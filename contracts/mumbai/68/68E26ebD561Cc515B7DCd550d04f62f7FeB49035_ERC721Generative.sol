...
import "./interfaces/IGenerativeCollection.sol";
import "./ERC721General.sol";

contract ERC721Generative is IGenerativeCollection, ERC721GeneralBase {
    using EnumerableSet for EnumerableSet.AddressSet;

    /**
     * @notice Script for generative token
     */
    string public script;

    /**
     * @notice Imported files being used
     */
    FileImport[] public imports;
    ...
    function getGenerativeCode() public view returns (string memory) {
        return string.concat(generateImportCode(), generateFullScript());
    }

    /// @notice generate the full script for the token
    function generateFullScript() public view returns (string memory) {
        return string.concat("<script>", string(script), "</script>");
    }

    /// @notice Returns the HTML for the given imports
    function generateImportCode() public view returns (string memory) {
        string memory code = "";

        for (uint256 i = 0; i < imports.length; i++) {
            code = string.concat(code, generateFileImport(imports[i]));
        }
    ...
    function generateFileImport(FileImport memory _import) public view returns (string memory) {
        if (_import.fileType == FILE_TYPE_JAVASCRIPT_PLAINTEXT) {
            return string.concat("<script>", _import.link, "</script>");
        } else if (_import.fileType == FILE_TYPE_JAVASCRIPT_BASE64) {
            return string.concat('<script src="data:text/javascript;base64,', _import.link, '"></script>');
        } else if (_import.fileType == FILE_TYPE_JAVASCRIPT_GZIP) {
            return
                string.concat(
    ...
    function addImport(FileImport memory _import) external {
        _addImport(_import);
    }

    function addImports(FileImport[] memory _imports) external {
    ...
    function _initialize(
        ...
        __MarketplaceFilterer__init__(false);
    ...
    function _addImport(FileImport memory _import) private {
        imports.push(_import);
    }

    /// @notice adds many imports
    function _addManyImports(FileImport[] memory _imports) private {
...
interface IGenerativeCollection {
    struct FileImport {
        string name;
        uint8 fileType;
        string link;
