...
contract ClampSwap {
    ...
    function registerUser(string memory _name) external {
        require(!usersByAddress[msg.sender].Exists, "USER ALREADY REGISTERED");
        usersByAddress[msg.sender] = User({
            Exists: true,
            UserName: _name,
            UserAddress: msg.sender
        });
        usersByUserName[_name] = User({
            Exists: true,
            UserName: _name,
            UserAddress: msg.sender
        });
    ...
    function indexSwapToNativeWithReceiver(
        ...
        uint[] memory amountsOut = indexSwapWithReceiver(
            _indexIdentifier,
            _percentage,
            address(wNative),
            _receiver
        );
        uint256 amount = sum(amountsOut);
    ...
    function indexSwapToERC20WithReceiver(
        ...
        uint sumAmountsOut = sum(amountsOut);
        ...
        IERC20(_returnBaseToken).safeTransfer(_receiver, sumAmountsOut);
    }

    function indexSwapWithReceiver(
        uint _indexIdentifier,
        uint _percentage,
        address _returnBaseToken,
        address _receiver
    ...
    function getUserIndex(
        ...
        require(
            userIndexes[_user][_indexIdentifier].Exists == true,
            "CLAMPV1: NO-INDEX-EXISTS"
        );
        return (
            userIndexes[_user][_indexIdentifier].TokenHoldings,
            userIndexes[_user][_indexIdentifier].TokenAddresses
        );
    }
