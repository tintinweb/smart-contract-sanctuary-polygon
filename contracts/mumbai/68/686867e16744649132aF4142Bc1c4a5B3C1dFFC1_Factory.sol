...
contract Factory is Ownable {
    ...
    function getAddressTotal() public view returns(uint userTotal){
        for(uint i; i < deployed.length; i++){
            userTotal += ERC20(deployed[i]).balanceOf(msg.sender);
        }
    }
}
...
contract Investment is ERC20, Ownable, ReentrancyGuard {
    ...
    //-----CONSTRUCTOR------
    ///
    constructor(uint256 _totalInvestment, address lgentry, uint number) ERC20("InvestmentCurrency", "IC"){
