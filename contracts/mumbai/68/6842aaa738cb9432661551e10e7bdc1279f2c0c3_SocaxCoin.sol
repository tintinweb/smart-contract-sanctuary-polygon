...
contract SocaxCoin is MintableToken{
	string public constant name = "Socax";
	string public constant symbol = "SCX";
	uint public constant decimals = 12;
	uint public constant totalSupply =  500000000 * 10**18;

	
}

/**
 * @title SocaxCoinCrowdsale
 * @dev SocaxCoinCrowdsale is a base contract for managing a token crowdsale.
 * Crowdsales have a start and end timestamps, where investors can make
 * token purchases and the crowdsale will assign them tokens based
 * on a token per ETH rate. Funds collected are forwarded to an owner
 * as they arrive.
 */
contract SocaxCoinCrowdsale is Ownable {
  using SafeMath for uint256;
  ...
  function SocaxCoinCrowdsale(uint256 _rate) public {
    require(_rate > 0);
	require (_rate < 1000);
  ...
  function createTokenContract() internal returns (SocaxCoin) {
    return new SocaxCoin();
  }
