    ...
    ReentrancyGuard
{
    ...
    uint256 public fusionPrice = 2 ether;
    ...
    event UpdateFusionPrice(uint256 newPrice);
    ...
    modifier checkPriceFusion(uint256 value) {
        if (value < fusionPrice) revert InvalidPriceForFusion();
        gameFunds += value;
        _;
    }

    /**
     * @notice update the new price for fusion.
     *
     * Requirements:
     *
     * - Only owner of contract can call this function
     */
    function changePriceFusion(uint256 newValue) external onlyOwner {
        fusionPrice = newValue;
        emit UpdateFusionPrice(newValue);
    }
    ...
    function privatesalesmint(
        ...
        if (indexMint + quantity > maxSupplyPrivateSale + maxSupplyPremint)
        ...
        checkPriceFusion(msg.value)
    {
        ...
        if (metadatas[tokenId0].evolution != metadatas[tokenId1].evolution)
            revert InvalidTokenIdsForFusion();
        else {
            uint8 nextEvo = metadatas[tokenId1].evolution+1;
            uint8 metadata0 = uint8(tokenId0 % 2);
            uint8 metadata1 = uint8(tokenId1 % 2);
            if ((metadata0 == metadata1)) revert InvalidTokenIdsForFusion();
            if (nextEvo < 7) {
