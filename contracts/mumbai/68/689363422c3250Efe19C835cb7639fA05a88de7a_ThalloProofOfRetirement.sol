...
import {ProofOfRetirementData, RetirementBlock} from "./ThalloStructs.sol";
...
contract ThalloProofOfRetirement is UUPSUpgradeable, OwnableUpgradeable, ERC721Upgradeable {
    address public thalloBridgeEvents;
    uint256 public currentTokenId;
    mapping(uint256 => ProofOfRetirementData) public proofs;
    event FinalizeRetirement(ProofOfRetirementData newProofData, uint256 indexed tokenId);

    modifier onlyBridge() {
        require(msg.sender == thalloBridgeEvents, "TPR:NB");
        _;
    }
    ...
    function initialize(address _owner, address _bridge) external initializer {
        __Ownable_init();
        transferOwnership(_owner);
        __ERC721_init("Thallo Proof of Retirement", "TPR");
        thalloBridgeEvents = _bridge;
    }
    ...
    /**
     *@notice Mints a proof of retirement to a user for retired carbon credits
     *@param _proofData Struct containing the data for the proof of retirement
     *@param _to The address to mint the NFT to
     */
    function issueFinalProof(ProofOfRetirementData calldata _proofData, address _to) external onlyBridge {
        unchecked {
            ++currentTokenId;
        }
        proofs[currentTokenId] = _proofData;
        _mint(_to, currentTokenId);
        emit FinalizeRetirement(_proofData, currentTokenId);
    }

    ///@dev Seperate getter so the retirement blocks are returned with the proof data
    function getProof(uint256 _tokenId) external view returns (ProofOfRetirementData memory) {
        return proofs[_tokenId];
    }

    /**
     *@notice Returns the URI of a proof
     *@param tokenId The token id of the proof to return
     */
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return proofs[tokenId].tokenURI;
    }
}
...
/**
 *@notice Struct that stores information related to retirement and uncustody requests
 *@member requestor The address that requested the retirement or uncustody
 *@member quantity The amount of carbon credits that were requested to be retired or uncustodied (This will be 0 if returned and processed)
 *@member processed Whether the request has been processed or not
 */
struct Request {
    address requestor;
    uint256 quantity;
