...
contract lbtestmv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    uint256 internal TICKET_VAL = 10 * 10**stableDecimals; //minimum amount (in wei) for getting registered on list
   ...
   // Mumbai 0xeeB482F3981D004C6199cA21F331Dc33EE639f0a // Mainnet 0x527a819db1eb0e34426297b03bae11F2f8B3A19E
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lottoWeekly = IluckblocksWeekly(0xedC7Ac96Bcd3fd7f31843789f717Ec635bf3409B);
        weeklyContract = 0xedC7Ac96Bcd3fd7f31843789f717Ec635bf3409B;
    ...
    function withdrawExcessBalance () public onlyOwner() {
        ...
        wmatic.safeTransfer(msg.sender,contractBalance);
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 amountInToken = (minimumPot * 10**18) / tprice;  // 50 usd in token

        minimumPrize = amountInToken;

        require(amount >= minimumPrize,"not enough amount - below 50 usd");
        require(wmatic.balanceOf(msg.sender) >= amount,"not enough balance");
    ...
    function accumulateJackpot(uint256 amount) internal {
        ...
        if(amount > minimumPrize){
         ...
         wmatic.safeTransfer(weeklyContract,amount.mul(15).div(100));
    ...
    function swapTokensToUSD(uint256 tokenAmount, address user, uint typeSwap) private {
        ...
        wmatic.approve(address(uniswapRouter), (amountInToken+1000000000000000000000));
        ...
        if(typeSwap == 1){
         ...
         prizeValueJackpot = (wmatic.balanceOf(address(this)) - contractBalance);

        } else if (typeSwap == 0){

         contractBalance = (wmatic.balanceOf(address(this)) - prizeValueJackpot);
    ...
    function autoSpin(address _caller, uint256[] calldata _nodes) public nonReentrant {
        ...
        require(block.timestamp > lastAutoSpin + 86400,"draw timestamp still not reached.");
         ...
         if(getJackpotinUSD() >= 100 * 10**18 && registeredsAccts.length >= 15){ 

            if(lbNodes.getUserActivation(_caller) == false){
                lbNodes.activateNodes(_caller,_nodes,3,0);
            }

            if(block.timestamp > lastAutoSpin + 86700 && reset == false){    // if no node activates in 5 minutes period queue resets
                lbNodes.resetQueue(3,0);
                reset = true;
            } else if(lbNodes.updateNodeInfo(_caller,botReward,0,3) == true){
    ...
    function getRandomNumber() internal returns (uint256 requestId) {
        ...
        if(contractBalance >= minimumPrize*3){
            ...
            swapTokensToUSD(10 * 10**18,address(this),0);
    ...
    function BuyTicket(address player,uint quantity) public nonReentrant {
        ...
        // update balances
         ...
         prizeValueJackpot = (wmatic.balanceOf(address(this)) - contractBalance); // ((dailyFeed * 10**12) / tprice) / 10**10;
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
        ...
        require(getJackpotinUSD() >= 10 * 10**18, "Contract need to have atleast 10 USD in value.");
