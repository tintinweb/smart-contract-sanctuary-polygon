...
contract GUTradeMatcher {
    ...
    event AuctionSuccessful(uint indexed nftId, bytes indexed bidSig, address bidder, address collector,  uint listingId, uint amount, uint price);
    event PurchaseSuccessful(uint indexed nftId, address buyer, address collector, uint listingId, uint amount, uint price);
    event OfferAccepted(uint indexed nftId, bytes indexed offerSig, address offeror, address collector,  uint listingId, uint amount, uint price);
    event ManagerAddressChanged(address indexed managerAddress);

    mapping (uint => bool) public finalizedListings;
    ...
    {
        TradeLib.checkFinalizeOfferPreconditions(manager.cancellations(), manager, data, finalizedListings);
            
        TradeLib.verifyFinalizeOfferSignatures(data);

        TradeLib.transferERC20(data.nftData, data.sale.tokenAddress, data.offer.offeror, data.seller, manager.beneficiary(), data.offer.amount, data.auth.fee);
        ...
        emit OfferAccepted(data.sale.nftId, data.offerSignature, data.offeror, data.offer.to, data.offer.listingId, data.sale.amount, data.offer.amount);
    }

    function finalizeBatchOffers(TradeLib.FinalizeOffer[] calldata data) external
    {
        for(uint i; i < data.length; i++){
            TradeLib.checkFinalizeOfferPreconditions(manager.cancellations(), manager, data[i], finalizedListings);

            TradeLib.verifyFinalizeOfferSignatures(data[i]);

            TradeLib.transferERC20(data[i].nftData, data[i].sale.tokenAddress, data[i].offer.offeror, data[i].seller, manager.beneficiary(), data[i].offer.amount,  data[i].auth.fee);
            ...
            emit OfferAccepted(data[i].sale.nftId, data[i].offerSignature, data[i].offeror, data[i].offer.to, data[i].offer.listingId, data[i].sale.amount, data[i].offer.amount);
    ...
    {
        TradeLib.checkPurchasePreconditions(manager.cancellations(), manager, data, finalizedListings);
        ...
        emit PurchaseSuccessful(data.sale.nftId, msg.sender, data.to, data.sale.listingId, data.sale.amount, data.sale.price);
    }
    ...
    function batchPurchase(TradeLib.FinalizePurchase[] calldata data) external
    {
        for (uint i; i<data.length; i++)
        {
            TradeLib.checkPurchasePreconditions(manager.cancellations(), manager, data[i], finalizedListings);
            ...
            emit PurchaseSuccessful(data[i].sale.nftId, msg.sender, data[i].to, data[i].sale.listingId, data[i].sale.amount, data[i].sale.price);
        }
    }
    
    function finalizeAuction(TradeLib.FinalizeAuction calldata data) external
    {
        TradeLib.checkFinalizeAuctionPreconditions(manager.cancellations(), manager, data, finalizedListings);
        ...
        emit AuctionSuccessful(data.auction.nftId, data.bidSignature, data.bidder, data.bid.to, data.auction.listingId, data.auction.amount, data.bid.amount);
    }
    ...
    function finalizeBatchAuctions(TradeLib.FinalizeAuction[] calldata data) external
    {
        for (uint i; i<data.length; i++)
        {
            TradeLib.checkFinalizeAuctionPreconditions(manager.cancellations(), manager, data[i], finalizedListings);
            ...
            emit AuctionSuccessful(data[i].auction.nftId, data[i].bidSignature, data[i].bidder, data[i].bid.to,  data[i].auction.listingId, data[i].auction.amount ,data[i].bid.amount);
...
library TradeLib {
    ...
    function checkFinalizeAuctionPreconditions(address cancellationsAddress, GUTradeManager manager, FinalizeAuction calldata data, mapping (uint=>bool) storage listings) internal {
        ...
        require(!listings[data.auction.listingId], "Listing already finalized");
        listings[data.auction.listingId] = true;
    }

    function checkPurchasePreconditions(address cancellationsAddress, GUTradeManager manager, FinalizePurchase calldata data, mapping (uint=>bool) storage listings) internal
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        ...
        require(!listings[data.sale.listingId], "Listing already finalized");
        listings[data.sale.listingId] = true;
    }

    function checkFinalizeOfferPreconditions(address cancellationsAddress, GUTradeManager manager, FinalizeOffer calldata data, mapping (uint=>bool) storage listings) internal
    {
    ...
    function checkMintToPreconditions(GUCollections collections, GUTradeManager manager, MintTo calldata data) internal view
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
    ...
    {
        bytes memory toHash = abi.encodePacked(offer.to, offer.listingId, offer.amount, offer.expiry, offer.sellersOffer);
        bytes32 _hash = keccak256(toHash);
        return recoverSigner(getEthSignedMessageHash(_hash), signature) == signer;
    }
