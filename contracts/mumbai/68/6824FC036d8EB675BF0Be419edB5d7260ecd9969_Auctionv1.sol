...
contract Auctionv1 is Initializable, OwnableUpgradeable {
    ...
    */
    ...
    modifier checkForBuyAuction(Auctions storage _auc) {
        require(
            _auc.startTime < block.timestamp && _auc.endTime > block.timestamp
        );
        _;
    }
    ...
    modifier checkForBidAuction(Auctions storage _auc, uint256 _tokenId) {
        require(
            _auc.startTime < block.timestamp && _auc.endTime +  _auc.tokenBid[_tokenId].length * fifteenMinute > block.timestamp
        );
        _;
    }
    ...
    function buyNow(
        ...
        onlyIfAuctionExist(auctionId)
        checkForBuyAuction(auctionsDetails[auctionId])
    {
        Auctions storage _auc = auctionsDetails[auctionId];
