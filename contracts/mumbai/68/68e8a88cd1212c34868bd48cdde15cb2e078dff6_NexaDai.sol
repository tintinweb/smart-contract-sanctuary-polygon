...
// Replace below address with main token token
   ...
   struct userInfo {
    ...
    function buyTopup() public returns (bool) {
        ...
        sysInfos.totalFee+=uint128(JOIN_PRICE*sysInfos.withdrawFee/100);
    ...
    function _activateUser(address msgSender) internal {
        ...
        else _autoPool2xPosition(msgSender,sysInfos.poolSpan);
    ...
    function transferCredit(address _to, uint128 _amount) external  returns(bool) {
      ...
      // Deposit credits fund .
        ...
        require(userInfos[msg.sender].referral!=address(0),"EC3");
...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
    ...
    function setPrams(uint8 withdrwalFee, uint64 setPoolTime, uint8 setPoolLimit,uint8 _maxCycle,uint64 _royaltyClosingTime, uint8 _AR_validity, bool _poolSpan)  external  returns(bool){
        require(msg.sender==defaultAddress,"EC-4");
   ...
   function _updateTeamNum(address ref) internal  {
    ...
    function _poolPosition()internal{
        ...
        require(usr.poolLimit == sysInfos.pool2Entrylimit,"EC-6");
   ...
   //Incomming transaction
    function _transfer(address _from, address _to,uint _amount) internal {
        ...
        require(_from!=address(0) && _to!=address(0) && _amount>0 && tokenAddress != address(0) ,"EC-7");
    ...
    function _transfer(address _to,uint _amount) internal {
       ...
       require(_to!=address(0) && _amount>0 ,"EC-7");
  ...
  function _distributeGapBonus(address _user,address ref)  internal returns(bool){
       ...
       for (uint i ; i <=TEAM_DEPTH; i++){ 
            //variable data
            address usr = ref;
            userInfo memory user=userInfos[usr]; // here we can also optimize the code 
            ref = userInfos[usr].referral;
    ...
    function _transferTo(address user,uint128 amot)internal {
        if(!dataFetchDone)return;
          ...
          if(tokenAddress!=address(0)) _transfer(user,(amot-(amot*sysInfos.withdrawFee/100))); // zero balance call    
         // else console.log(user,(amot-(amot*sysInfos.withdrawFee/100))); 

    }

    function _payCulculate (address to_usr,uint8 typ,uint128 amot)internal  returns (uint128){
        ...
        if (nextMemberDownlineFill==0){
            ...
            if(cycle || recMindex==0 && defID!=1){
                ...
                while (cycle || recMindex==0 && defID!=1){
                    ...
                    recMindex = autoPool2xDataList[nextMemberParentFill].mIndex;
                    defID = autoPool2xDataList[nextMemberParentFill].userID;
                }
