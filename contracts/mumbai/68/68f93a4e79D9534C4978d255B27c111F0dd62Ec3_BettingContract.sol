...
contract BettingContract is RrpRequesterV0,Ownable,ReentrancyGuard {
    ...
    /// @param _airnodeRrp Airnode RRP contract address
    constructor(address _airnodeRrp,address _token,uint _decimals) RrpRequesterV0(_airnodeRrp) {
       threshold = 50;
       maintoken = IERC20(_token);
       maindecimal = _decimals;
       tokens[_token]  = true;
    ...
    function makeRequestUint256() external {
        ...
        userbytes[msg.sender] = requestId;
        useraddress[requestId] = msg.sender;
        expectingRequestWithIdToBeFulfilled[requestId] = true;
        emit RequestedUint256(requestId);
    } 

    function makeRequestUint2562() private {
        bytes32 requestId = airnodeRrp.makeFullRequest(
            airnode,
            endpointIdUint256,
            address(this),
        ...
        onlyAirnodeRrp
    {
        ...
        revealCoin(qrngUint256,requestId);

        emit ReceivedUint256(requestId, qrngUint256);
    }

    function flipCoin(uint256 _amount) public payable nonReentrant returns (string memory) {
        require(userbytes[msg.sender] == bytes32(0), "one bet at a time buddy!");
        uint transferAmount = _amount * 10**maindecimal;
        maintoken.transferFrom(msg.sender, address(this), transferAmount);
    ...
    function flipCoin2(uint256 _amount) public nonReentrant returns (string memory) {
        require(userbytes[msg.sender] == bytes32(0), "one bet at a time buddy!");
        uint transferAmount = _amount * 10**maindecimal;
        maintoken.transferFrom(msg.sender, address(this), transferAmount);
        betsize[msg.sender] = (_amount * 2);
    ...
    function revealCoin(uint256 randomNumber,bytes32 requestId) private {
        uint256 randomNum = uint256(keccak256(abi.encodePacked(randomNumber)));
        address getAddress = useraddress[requestId];
        if (randomNum % 100 < threshold) {
           
            
            winpot[getAddress] += betsize[getAddress];
            delete useraddress[requestId];
            delete userbytes[getAddress];
            delete betsize[getAddress];
            emit win(getAddress, randomNumber, true, betsize[getAddress]);
            // return "heads";  
        } else {
            winpot[_owner] += betsize[getAddress] / 2;
            delete useraddress[requestId];
            delete userbytes[getAddress];
            delete betsize[getAddress];
            emit win(getAddress, randomNumber, false, betsize[getAddress]);
            // return "tails"; 
        }
    }

    function claimPot() public nonReentrant {
       require(winpot[msg.sender] != 0, "no wins to claim");
       uint transferAmount = winpot[msg.sender] * 10**maindecimal;
       maintoken.transfer(msg.sender,transferAmount);
    }

    function checkWins() public view returns (uint balance) {
        return winpot[msg.sender] ;
    }

    function checkWins(address _address) public view returns (uint balance) {
        return winpot[_address] ;
    }
