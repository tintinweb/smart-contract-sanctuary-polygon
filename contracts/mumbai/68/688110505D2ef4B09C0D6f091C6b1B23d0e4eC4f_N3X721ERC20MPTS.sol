...
import { IN3XClone } from "../interfaces/clone/IN3XClone.sol";
    ...
    IN3XClone 
{
    ...
    /// @notice Withdraw ERC20 role
    bytes32 internal constant WITHDRAWER_ROLE = keccak256("WITHDRAWER_ROLE");

    /// @notice Contract type used by clone factory 
    bytes32 internal constant CONTRACT_TYPE = bytes32("N3X721ERC20MPTS");
    
    /// @notice Contract version used by clone factory
    ...
    mapping(address => uint256) internal erc20AddressToMintPrice;
    ...
    function initialize(string memory name_, string memory symbol_, address owner_) initializer public {
        ...
        __EIP712MetaTransactionUpgradeable_init(name_, "1");
    ...
    /// @notice Returns the module type of the contract.
    ...
    */
    function setBaseURI(string memory newBaseTokenURI) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _baseTokenURI = newBaseTokenURI;
    }

    /// @notice Gets the maximum total supply.
    function getMaxTotalSupply() external virtual view returns(uint256) {
        return _maxTotalSupply;
    }

    /// @notice Admin can set the maximum total supply.
    function setMaxTotalSupply(uint256 newMaxTotalSupply) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    function getErc20MintPrice(address erc20Address) public virtual view returns (uint256) {
        return erc20AddressToMintPrice[erc20Address];
    } 
    ...
    */
    function setErc20MintPrice(address erc20Address, uint256 price_) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    function withdrawErc20(address erc20Address, uint256 amount) external virtual onlyRole(WITHDRAWER_ROLE) {
    ...
    */
    function mintWithErc20(address erc20Address)
        ...
        whenNotPaused
    {
        uint256 price = getErc20MintPrice(erc20Address);
        ...
        unchecked {
            ...
            require(newTokenId < _maxTotalSupply, "Maximum supply reached");
    ...
    /**
    @notice Takes address and returns the tokens owned
    ...
    */
    function getTokensByOwner(address owner_) external view returns (uint256[] memory tokenIds) {
        uint256 numTokens = balanceOf(owner_);
    ...
    */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        return string(
            bytes.concat(
                bytes(_baseTokenURI), 
                bytes(StringsUpgradeable.toHexString(uint256(uint160(address(this))), 20)), 
                "/", 
