...
contract CraftLogic is MultiOwner, ReentrancyGuard {
    ...
    function craft(uint256 recipeId) external nonReentrant {
        ...
        if (catalyst.tokenAddress != address(0)) {
            // Check if catalyst is set
            // ERC20 Interface
            if (catalyst.tokenType == 0) {
                if (ICatalyst(catalyst.tokenAddress).balanceOf(_msgSender()) < catalyst.amount)
                    revert CatalystConditionNotSatisfied();
            }
            // ERC721 Interface
            else if (catalyst.tokenType == 1) {
                if (ICatalyst(catalyst.tokenAddress).ownerOf(catalyst.tokenid) != _msgSender())
                    revert CatalystConditionNotSatisfied();
            }
            // ERC1155 Interface
            else if (catalyst.tokenType == 2) {
                if (ICatalyst(catalyst.tokenAddress).balanceOf(_msgSender(), catalyst.tokenid) < catalyst.amount)
                    revert CatalystConditionNotSatisfied();
            }
        }

        // Burn the required materials
        for (uint256 i = 0; i < recipe.materials.length; ) {
        ...
        // Mint the artifacts
        for (uint256 i = 0; i < recipe.artifacts.length; ) {
     ...
     *
     * @param id the ID of the recipe to create
    ...
    function createRecipe(
        ...
        if (recipes[id].id == id) revert ExistentCraft(id);

        // Check that materials and artifacts arrays have at least one element
        if (materials.length == 0) revert EmptyMaterialsArray();
        if (artifacts.length == 0) revert EmptyArtifactsArray();
        ...
        for (uint256 i = 0; i < materials.length; ) {
        ...
        for (uint256 i = 0; i < artifacts.length; ) {
        ...
        emit RecipeCreated(id, name, _msgSender());
    }
    ...
    function updateRecipe(
        ...
        // Check if the recipe already exists
        if (recipes[id].id != id) revert NonExistentCraft(id);
        ...
        // Emit the RecipeCreated event
        emit RecipeUpdated(id, name, _msgSender());
    }

    function changeRecipeStatus(uint256 id, bool active) public onlyOwner {
        // Check if the recipe already exists
        if (recipes[id].id != id) revert NonExistentCraft(id);
        ...
        recipe.active = active;
        // Emit the RecipeCreated event
        emit ChangeRecipeStatus(id, active, _msgSender());
    }
}
