...
contract UhtToken is Initializable, ERC20Upgradeable, UUPSUpgradeable {
    ...
    function isApproved(address account) public view returns (bool) {
        return approvedUsers[account];
    }

    function approveUser(address user) public  {
         require(approvedUsers[msg.sender], "Sender is not approved");
        approvedUsers[user] = true;
    }

    function revokeApproval(address user) public onlyOwner {
        approvedUsers[user] = false;
    }

    function transferBulk(address[] calldata recipients, uint256[] calldata amounts) external {
    ...
    function allowance(address owner, address spender) public view onlyOwner override returns (uint256) {
        require(approvedUsers[owner], "Owner is not approved");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public override returns (bool) {
        require(approvedUsers[sender] && approvedUsers[msg.sender], "Sender or spender is not approved");
