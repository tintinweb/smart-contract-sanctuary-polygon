...
uint[] memory tokeTypes=requestToTokenTypeMap[requestId];
 uint[] memory randomNumber=randomNumbers;
   for (uint i=0;i< tokeTypes.length;i++){
                uint tokenId = getTokenNumber(tokeTypes[i],randomNumber[i]);
                tokentier[tokenId] = tokeTypes[i];
                tokenTaken[tokenId] = true;
                token.transferFrom(requestToUserMap[requestId],address(this),tokentierToPrice[tokeTypes[i]]);
