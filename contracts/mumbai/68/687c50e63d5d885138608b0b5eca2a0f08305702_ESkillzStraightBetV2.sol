...
contract ESkillzStraightBetV2 is ESkillzStraightBet {
   ...
   function  CreateMPGameV2(address _creator, address _joiner, uint256 amount) external onlyReceiver{
      ...
      totalAmountsOfDay[(block.timestamp- startTimeStamp)/ 1 days] = totalAmountsOfDay[(block.timestamp- startTimeStamp)/ 1 days] + 2 * amount;
      GameIDs++; 
    ...
    function getClaimAmountsOfPlayerV2(address _player) external view returns (uint256) {
        return  calcClaim(_player);
    }

    function claimFromContractV2() external {
        require(calcClaim(msg.sender) > 0, "claim amounts must be bigger than zero.");
        sport.mintMore(msg.sender, calcClaim(msg.sender));
        staking[msg.sender] = Stake(staking[msg.sender].amount + calcClaim(msg.sender), block.timestamp, 0);
    }
}
