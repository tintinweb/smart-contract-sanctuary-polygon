...
contract PassiveIncomeNFTSwap is Ownable {
    ...
    constructor(
        ...
        stakingToken = IERC20(stakingContractAddress);
    }
    ...
    function swap(uint256 tokenId, uint8 lockDurationInMonths) external {
        ...
        //passiveIncomeNFT.mint(msg.sender, amount, lockDurationInMonths);
    }
...
contract PassiveIncomeNFT is ERC721Enumerable, Ownable {
    struct Lock {
        uint256 startTime;
        uint256 endTime;
    ...
    function setMaxLockDuration(uint8 months) external onlyOwner {
    ...
    function setNextSnapshot(
        uint256 snapshot /*onlyRole(SNAPPER)*/
    ) external {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "lock #',
                        Strings.toString(tokenId),
                        '", "description": "Nidhi Passive Income NFTs generate ... you guessed it ... passive income.", "image": "data:image/svg+xml;base64,',
...
contract NidhiLegacyNFT is ERC721Enumerable, AccessControl {
        ...
        onlyRole(MINTER_ROLE)
    {
        ...
        emit Minted(_tokenIds, tokenURI);
    }

    function setImageBaseURL(string memory baseURL) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(bytes(_images[tokenId]).length > 0, "URI query for nonexistent token");
        return string(
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            ...
                            abi.encodePacked(_imageBaseURL, _images[tokenId]),
                            '"}'
                        )
                    )
                )
