...
contract Sale is ERC721Holder, ERC1155Holder, Ownable, Initializable {
    ...
    function _sharing(bool isAdmin, uint256 projectId, address _token, uint256 _tokenId) private returns(uint256, uint256, uint256) {
        ...
        if (isAdmin) {
            royaltyFee = getRoyaltyFee(_token, _tokenId, msg.value);
            ...
            royaltyFee = getRoyaltyFee(_token, _tokenId, msg.value);
            adminFee   = _getAdminFee(msg.value, percent);
        ...
        return (percent, adminFee, amount);
    }
	...
	function getRoyaltyFee(address _token, uint256 _tokenId, uint256 _salePrice) public view returns (uint256 royaltyAmount) { 
        if (nftChecker.isImplementRoyalty(_token)) {
            (, royaltyAmount) = IOSB721(_token).royaltyInfo(_tokenId, _salePrice);
        } else {
		    royaltyAmount = 0;
        }
	}
