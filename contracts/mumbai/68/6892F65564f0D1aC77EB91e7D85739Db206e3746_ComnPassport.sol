    ...
    Ownable
{
    ...
    /// @dev Only transfers to or from TRANSFER_ROLE holders are valid, when transfers are restricted.
    bytes32 public constant TRANSFER_ROLE = keccak256("TRANSFER_ADMIN_ROLE");
    ...
    function claimWithCoupon(
        ...
        uint256 discountedPrice = this._calcPrice(coupon, _currency, _tier);
        coupon.usedCount += 1;
   ...
   function getCouponIdx(string calldata _coupon)
    ...
    function calcPrices(
        ...
        for (uint256 i = 0; i < prices.length; i++) {
            if (strcmp(prices[i].tier, _tier)) {
                ...
                cpd[j].price = this._calcPrice(coupon, prices[i].currency, _tier);
    ...
    function getPriceSize() public view returns (uint256) {
    ...
    //     address _currency,
    //     string calldata _tier
    // ) public view returns (uint256 discountedPrice, uint8 discount) {
    //     Coupon memory coupon = _getCoupon(_coupon);
    //     discountedPrice = this._calcPrice(coupon, _currency, _tier);
