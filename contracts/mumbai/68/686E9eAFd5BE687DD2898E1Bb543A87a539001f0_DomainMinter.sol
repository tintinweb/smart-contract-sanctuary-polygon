...
contract DomainMinter is Ownable, ReentrancyGuard {
  ...
  function mintDomain(
    string memory _domainName, address _holder
  ) external nonReentrant payable returns(uint256 tokenId) {
    // mint a domain
    tokenId = tldContract.mint(price,_domainName,_holder, address(0));
  }
  ...
  function ownerFreeMint(
    ...
    // mint a domain
    tokenId = tldContract.mint(0,_domainName, _domainHolder, address(0));
  }
