...
contract DYTIC is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public total_special_limit = 2000;
    
    bytes32 public whitelistSigner;

    mapping(address => bool) public specialMinted; 
    ...
    constructor(string memory _initBaseURI, string memory _initNotRevealedUri, string memory _contractURI) ERC721A("Do You Think I'm Crazy?", "DYTIC") {
    ...
    function mint(uint256 quantity) public payable  {
        ...
        if (msg.sender != owner()) {
            ...
            if (nft.balanceOf(msg.sender) > 0 && nft2.balanceOf(msg.sender) > 0){

            require(total_special_count + quantity <= total_special_limit, "Special giveaway Limit Reached");
            require(msg.value >= (specialNFTHoldersCost * quantity), "Not Enough ETH Sent"); 
            require(specialMinted[msg.sender] != true, "Already Minted"); 

            total_special_count = total_special_count + quantity;
            specialMinted[msg.sender] == true; 

            } else {

            require(total_PS_count + quantity <= total_PS_limit, "Publicsale Limit Reached");
