...
contract RewardsWinningFound is Ownable, ReentrancyGuard {
    ...
    event AddresseUpdated(address fight, address pvpAddress, address rankingAddress, address payments);
    ...
    function updateInterfaces() external {
        ...
        rankingRewardAddress = gameAddresses.getAddressOf(AddressesInit.Addresses.REWARDS_RANKING);
        paymentsAddress = gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS);
        emit AddresseUpdated(fightAddress, pvpRewardAddress, rankingRewardAddress, paymentsAddress);
    }
    ...
    function setBonusMult(uint256 _bonus) external onlyOwner {
        require(_bonus >= 100 && _bonus <= 140, "bonus multiplicator not match");
