...
contract WEBWIZ is ERC721Enumerable, Ownable, IRamperInterface721 {
    ...
    function fetchMaticPrice() internal view returns (uint256) {
        AggregatorV3Interface priceFeed = AggregatorV3Interface(CHAINLINK_AGGREGATOR_ADDRESS);
        (, int256 priceValue, , , ) = priceFeed.latestRoundData();
        require(priceValue > 0, "Invalid price");
        uint256 decimals = uint256(priceFeed.decimals());
        uint256 priceInMatic = uint256(priceValue) * (10**decimals);
        return priceInMatic;
    }

    function price() override external view returns (uint256) {
        uint256 nftPriceInMatic = (33 * 10**18) / fetchMaticPrice();
        return nftPriceInMatic;
    }

    function mint(address _userWallet, uint256 _quantity) override external payable {
        uint256 nftPriceInMatic = (33 * 10**18) / fetchMaticPrice();
        require(msg.value >= nftPriceInMatic * _quantity, "Insufficient funds for requested tokens");
