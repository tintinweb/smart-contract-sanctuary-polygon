...
contract LandRegister is ERC721, Ownable{
    struct Land{
        uint256 id;
        string location;
        uint256 area;
    ...
    constructor() ERC721("LandRegister", "LAND"){
        landCount = 0;
    ...
    function transferLand(uint _landId, address payable _newOwner) public{
        ...
        _transfer(msg.sender, _newOwner, _landId);
        land.isListed = false;
        lands[_landId] = land;
    ...
    function buyLand(uint _landId, address payable _newOwner) public{
        Land memory land = lands[_landId];
        require(msg.sender != land.owner, "You are already owner");
        require(land.isListed == false, "Land is already sold");
        require(maticToken.balanceOf(_newOwner) >= land.pricePerArea * land.area, "Insufficient balance");
        
        maticToken.transferFrom(_newOwner, land.owner, land.pricePerArea * land.area);
        _transfer(msg.sender, _newOwner, _landId);
    ...
    function listLand(uint _landId) public{
        Land memory land = lands[_landId];
    ...
    function getLands(address _owner) public view returns(Land[] memory){
     ...
     function getUnSoldLand() public view returns(Land[] memory){
        Land[] memory landsOfUser = new Land[](landCount);
        uint count = 0;
        for(uint i = 1; i <= landCount; i++){
