...
contract SheratonGovernor is Governor, GovernorSettings, GovernorCompatibilityBravo, GovernorVotesComp, GovernorTimelockCompound, Ownable {
    ...
    mapping(address => bool) public trusties;   //3 members
    
    event UpdateTrusties(address account, bool value);
    constructor(
    ...
    function updateTrusties(address _account) public onlyOwner
    {
        trusties[_account] = !trusties[_account];
        emit UpdateTrusties(_account, trusties[_account]);
    }
    ...
    function propose(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, string memory description)
        ...
        returns (uint256)
    {
        require(trusties[msg.sender] || token.balanceOf(msg.sender) >= 500, "Address is not trusties or do not have 500 Sheraton NFTs");
