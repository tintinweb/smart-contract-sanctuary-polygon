...
contract AlterOsUpgradable is Initializable,
  ...
  function initialize(uint256 _feePercent) external initializer {
  ...
  function addAcceptableContracts(
    ...
    if (
      keccak256(
        abi.encodePacked(_collectionContracts[_contractID].contractAddress)
    ...
    emit AllowedContractsAdded(contractAddress, contractName, creatorAdress);
  ...
  function createListing(
    ...
    for (uint256 i; i < tokenIds.length; i++) {
      require(_listedTokens[tokenIds[i]] != true, "Token is already listed");
    }
    if (!isErc721) {
      require(amount > 0);
      for (uint256 i; i < tokenIds.length; ) {
        require(
          IERC1155Upgradeable(nftAddress).balanceOf(msg.sender, tokenIds[i]) >=
            amount,
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        require(
          IERC721Upgradeable(nftAddress).ownerOf(tokenIds[i]) == msg.sender,
    ...
    if (isErc721) {
      for (uint256 i; i < tokenIds.length; i++) {
        IERC721Upgradeable(nftAddress).safeTransferFrom(
          ...
          address(this),
          tokenIds[i],
          abi.encodePacked("SafeTransferFrom", "'''###'''", _uint2str(price))
        );
      }
    } else {
      for (uint256 i; i < tokenIds.length; i++) {
    ...
    nonReentrant
  {
    ...
    if (listing.isErc721) {
      for (uint256 i; i < listing.tokenIds.length; i++) {
        if (
          IERC721Upgradeable(listing.nftAddress).getApproved(
            listing.tokenIds[i]
          ) != address(this)
        ) {
          // if (msg.value > 0) {
    ...
    if (listing.erc20Address == address(0)) {
      ...
      AddressUpgradeable.sendValue(payable(listing.seller), listing.price);
