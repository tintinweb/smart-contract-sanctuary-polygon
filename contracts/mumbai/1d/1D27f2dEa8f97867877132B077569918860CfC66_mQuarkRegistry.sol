...
contract mQuarkRegistry is ImQuarkRegistry, mQuarkProjectDeployer, AccessControl, NoDelegateCall {
  ...
  function setSubscriber(address _subscriber) external onlyRole(CONTROL_ROLE) {
    s_subscriber = _subscriber;
    emit SubscriberSet(_subscriber);
  }

  function setImplementation(uint8 _id, address _implementation) external onlyRole(CONTROL_ROLE) {
  ...
  function registerProject(
    ...
    address m_project = deploy(address(this), s_subscriber, msg.sender, m_projectId);
