...
import './interface/ILostChainGameSettings.sol';

contract LostWorldDataV2 is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LWD: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LWD: RBA linker is zero address');
    ...
    function initialize(
        ...
        address nftAddress,
        address gameSettingsAddress
    ) public initializer {
        require(address(forwarder) != address(0x0), 'LWD: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LWD: RBA linker is zero address');
        require(nftAddress != address(0x0), 'LWD: NFT is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
        LCNFT = ILostChainNFT(nftAddress);
        LCGS = ILostChainGameSettings(gameSettingsAddress);
    ...
    mapping(uint256 => Enemy) enemyList;
    CountersUpgradeable.Counter private _enemyIdCounter;

    struct Recharge {
        bool initialized;
        uint8 chargeAmount;
    }
    mapping(uint256 => Recharge) rechargeAmount;

    event RechargeLand(address chargee, uint256 nftId, uint8 amount);

    function setLWNFT(address lostWorldNFTAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(lostWorldNFTAddress != address(0x0), 'LWD: Lost world NFT is zero address');
        LWNFT = ILostWorldNFT(lostWorldNFTAddress);
    }
    ...
    function setLCGD(address gameDataAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(gameDataAddress != address(0x0), 'LWD: Game data is zero address');
        LCGD = gameDataAddress;
    }

    function setLCGS(address gameSettingAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(gameSettingAddress != address(0x0), 'LCUD: Game settings is zero address');
        LCGS = ILostChainGameSettings(gameSettingAddress);
    }
    ...
    function addEnemy(
        ...
        uint256 attackSpeed,
        uint256 cleaveChance
    ) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(bytes(name).length > 0, 'LWD: Name cannot be empty');
        require(bytes(image).length > 0, 'LWD: Image cannot be empty');
        require(health > 0, 'LWD: Health cannot be empty');
        require(damage > 0, 'LWD: Damage cannot be empty');
        require(defense > 0, 'LWD: Defemse cannot be empty');
        require(criticalChance > 0, 'LWD: Critical chance cannot be empty');
        require(attackSpeed > 0, 'LCLCGDED: Attack speed cannot be empty');
        require(cleaveChance > 0, 'LWD: Cleave chance cannot be empty');

        uint256 enemyId = _enemyIdCounter.current();
        _enemyIdCounter.increment();
        require(!enemyList[enemyId].exists, 'LWD: Enemy already exists with that id');

        enemyList[enemyId].exists = true;
        enemyList[enemyId].enemyId = enemyId;

        enemyList[enemyId].name = name;
        enemyList[enemyId].health = health;
        enemyList[enemyId].damage = damage;
        enemyList[enemyId].defense = defense;
        enemyList[enemyId].criticalChance = criticalChance;
        enemyList[enemyId].attackSpeed = attackSpeed;
        enemyList[enemyId].cleaveChance = cleaveChance;
        enemyList[enemyId].image = image;
    }

    function getAllEnemy(bool includeDisabled) public view returns (Enemy[] memory) {
        uint256 maxEnemyId = _enemyIdCounter.current();
        require(maxEnemyId > 0, 'LWD: No enemy available');

        Enemy[] memory tempRet = new Enemy[](maxEnemyId);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < maxEnemyId; i++) {
            if (enemyList[i].exists && (!enemyList[i].disabled || includeDisabled)) {
                tempRet[totalIndex] = enemyList[i];
                totalIndex++;
            }
        }

        // Only return non empty index
        require(totalIndex > 0, 'LWD: No enemy available');
        Enemy[] memory ret = new Enemy[](totalIndex);
        for (uint256 i = 0; i < totalIndex; i++) {
            ret[i] = tempRet[i];
        }
    ...
    function getEnemyById(uint256[] memory ids) external view returns (Enemy[] memory) {
        return _getEnemyById(ids, true);
    }

    function getEnemy(uint256[] memory ids, bool includeDisabled) public view returns (Enemy[] memory) {
        return _getEnemyById(ids, includeDisabled);
    }

    function _getEnemyById(uint256[] memory ids, bool includeDisabled) internal view returns (Enemy[] memory) {
        uint256 maxEnemyId = _enemyIdCounter.current();
        require(maxEnemyId > 0, 'LWD: No enemy available');

        Enemy[] memory tempRet = new Enemy[](ids.length);
        uint256 totalIndex = 0;
        for (uint256 i = 0; i < ids.length; i++) {
            if (enemyList[ids[i]].exists && (!enemyList[ids[i]].disabled || includeDisabled)) {
                tempRet[totalIndex] = enemyList[ids[i]];
                totalIndex++;
            }
        }
    ...
    function disableEnemy(uint256 enemyId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(enemyList[enemyId].exists, 'LWD: Enemy does not exist');
        require(!enemyList[enemyId].disabled, 'LWD: Enemy is already disabled');

        enemyList[enemyId].disabled = true;
    }

    function enableEnemy(uint256 enemyId) public onlyRole('GAME_MANAGER', _msgSender()) {
        require(enemyList[enemyId].exists, 'LWD: Enemy does not exist');
        require(enemyList[enemyId].disabled, 'LWD: Enemy is not disabled');

        enemyList[enemyId].disabled = false;
    }

    function checkEnemyAvailable(uint256 enemyId) external view returns (bool) {
        require(!enemyList[enemyId].disabled, 'LWD: Enemy is disabled');
        return true;
    }

    function initRecharge(address operator, uint256 landId)
        external
        onlyRole('TRUSTED_FORWARDER', operator)
        nonReentrant
    {
        require(_msgSender() == address(LWNFT), 'LWD: caller is not LWNFT');
        require(!rechargeAmount[landId].initialized, 'LWD: Land is already initialized');

        rechargeAmount[landId].initialized = true;
        rechargeAmount[landId].chargeAmount = ~uint8(0);
    }
    ...
    ) external onlyRole('TRUSTED_FORWARDER', operator) {
        require(_msgSender() == address(LCGD), 'LWD: caller is not LWNFT');
        require(rechargeAmount[landId].initialized, 'LWD: Land is not initialized');

        uint8 totalCharge = _calculateChargeAmount(rewardList, maxRewardAmount);
        require(rechargeAmount[landId].chargeAmount >= totalCharge, 'LWD: Land does not have enough charge');

        uint8 newChargeAmount = rechargeAmount[landId].chargeAmount - totalCharge;
        rechargeAmount[landId].chargeAmount = newChargeAmount;
    }

    function checkChargeAvailable(
        uint256 landId,
        uint8[] memory maxRewardAmount,
        uint256[] memory rewardList
    ) public view returns (bool) {
        uint8 totalCharge = _calculateChargeAmount(rewardList, maxRewardAmount);
        return rechargeAmount[landId].chargeAmount >= totalCharge;
    }

    function _calculateChargeAmount(uint256[] memory rewardList, uint8[] memory maxRewardAmount)
        private
        view
        returns (uint8)
    {
        require(rewardList.length == maxRewardAmount.length, 'LWD: Reward list and reward amount mistmatch');
        uint8 totalCharge = 0;
        for (uint256 i = 0; i < rewardList.length; i++) {
            string memory rewardRarity = LCNFT.getRewardableRarity(rewardList[i]);
            uint8 charge = LCGS.getRechargeUsage(rewardRarity) * maxRewardAmount[i];
            totalCharge += charge;
        }
    ...
    function getRechargeRates(uint256[] memory landIds) public view returns (uint8[] memory) {
        uint8[] memory ret = new uint8[](landIds.length);
        for (uint256 i = 0; i < landIds.length; i++) {
            ret[i] = rechargeAmount[i].chargeAmount;
        }

        return ret;
    }

    function recharge(uint256 landId, uint256 nftId) public onlyRole('TRUSTED_FORWARDER', msg.sender) nonReentrant {
        require(rechargeAmount[landId].initialized, 'LWD: Land recharge does not exists');
        (, , , , , address landOwner) = LWNFT.getLandData(landId);
        require(landOwner == _msgSender(), 'LWD: User does not own that land');
        require(LCNFT.checkNFTRecharger(nftId), 'LWD: NFT is not rechargeable type');

        uint256 balance = LCNFT.userNFTBalance(_msgSender(), nftId);
        require(balance > 0, 'LWD: User does not own that NFT');
        ...
        rechargeAmount[landId].chargeAmount = ~uint8(0);
        LCNFT.gameDataBurnNFT(msg.sender, _msgSender(), id, amount);

        emit RechargeLand(_msgSender(), nftId, rechargeAmount[landId].chargeAmount);
    }

    function getEnemyRarity(uint256 enemyId) external view returns (string memory) {
        require(enemyList[enemyId].exists, 'LWD: Enemy does not exist');
        Enemy memory enemy = enemyList[enemyId];
    }

    // TEST FUNCTION TO ALTER BYTECODE (RE-VERIFY)
    function TESTING59() external pure returns (bool) {
        return true;
    }
}
...
interface ILostChainGameSettings {
    function getMaxEnergy() external view returns (uint8);

    function getRechargeDuration() external view returns (uint64);

    function getEnemyMaxRewardWeight(string memory rarity) external view returns (uint8);

    function getRewardWeight(string memory rarity) external view returns (uint8);

    function getRechargeUsage(string memory rarity) external view returns (uint8);
}
