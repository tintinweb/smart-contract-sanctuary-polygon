...
contract AnimeTokenStakingPool is Ownable, ReentrancyGuard {
    ...
    uint256 public MINIMUM_CLAIMABLE_TIME = 10; // 3 days;
     ...
     */
    constructor(address _stakingToken, address _animeStudio) {
    ...
    /**
     * @dev estimate time when to archive the tagert reward
     */
    function estimateTime(uint256 targetReward, uint256 balance) public view returns (uint256){
      uint256 targetRewardPerToken = targetReward.div(balance).mul(1e18);
      return ((targetRewardPerToken.sub(rewardPerTokenStored)).mul(_totalSupply).div(1e18).div(rewardRate)).add(lastUpdateTime);
    }

    /**
     * @dev get current timestamp
     */
    function estimateDuration(uint256 targetReward, uint256 balance) external view returns (uint256){
        return estimateTime(targetReward, balance).sub(block.timestamp);
    }
    ...
    function claim() public nonReentrant updateReward(msg.sender) {
        require(initialize == true, "AnimeBankStakingPool: not initialized");
        require(block.timestamp > _depositTime[msg.sender].add(MINIMUM_CLAIMABLE_TIME), "AnimeBankStakingPool: not reach claimable time");
        ...
        if (reward > 0) {
            ...
            uint256 rewardToAnimeStudio = reward.div(2);
            IERC20(stakingToken).safeTransfer(animeStudio, rewardToAnimeStudio);
            IERC20(stakingToken).safeTransfer(msg.sender, reward.sub(rewardToAnimeStudio));
    ...
    function notifyRewardAmount(uint256 reward, uint256 duration) public onlyOwner updateReward(address(0)) {
        ...
        require(reward > 0, "AnimeBankStakingPool: reward must over zero");
        require(duration > 0, "AnimeBankStakingPool: duration must over zero");
        require(balance >= reward, "AnimeBankStakingPool: not enough reward balance");
        require(periodFinish <= block.timestamp, "AnimeBankStakingPool: reward in duration, must wait until finish");
        ...
        require(rewardRate <= balance.div(duration), "AnimeBankStakingPool: Provided reward too high");
     ...
     */
    function recover(uint256 tokenAmount) external onlyOwner {
        IERC20(stakingToken).safeTransfer(owner(), tokenAmount);
        emit Recovered(stakingToken, tokenAmount);
    }
