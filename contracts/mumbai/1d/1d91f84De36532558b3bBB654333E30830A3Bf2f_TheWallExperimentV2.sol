...
contract TheWallExperimentV2 is ERC721, ERC721Enumerable, Ownable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    mapping(uint => uint) values; 
    uint MAX_TOKENS;
    uint MAX_TOKENS_PER_SALE;
    uint price = 10**15;
    address public sendBalance;
    uint balance;

    constructor() ERC721("The wall experiment", "TWE") {
        _tokenIdCounter.increment();                                
        MAX_TOKENS =10;
        MAX_TOKENS_PER_SALE=5;
        sendBalance = 0x8eE886FefAd32e3e2998C148A4d0FdaCB538667D;
    ...
    function setValue(uint tokenId, uint newValue ) public {
        require(msg.sender == ownerOf(tokenId),"You are not the owner") ; 
        require(newValue>31, "Value must be > 31");
        require(newValue<255, "Value must be < 255");
        values[tokenId]=newValue;
    }
    function getValue()public view returns(uint[] memory){
        uint[] memory text = new uint[](_tokenIdCounter.current()); 
    ...
    function getTokensMsgSender()public view returns(uint[] memory){
        uint[] memory text = new uint[](_tokenIdCounter.current()); 
        uint index =0;
        for(uint i = 1; i<=_tokenIdCounter.current();i++){
    ...
    function payoutToSeller() payable public returns(bool) {
        require(isOwner());
            payable(sendBalance).transfer(balance);
