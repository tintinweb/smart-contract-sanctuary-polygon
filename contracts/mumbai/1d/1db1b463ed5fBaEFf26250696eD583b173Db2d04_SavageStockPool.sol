...
contract SavageStockPool is Operator {
    ...
    function _checkAllowance(uint256 amount) private {
        ...
        address thisAddress = address(this);
        uint256 allowance = token.allowance(thisAddress, stockAddress);
        if (allowance >= amount) {
            return;
        }
        if (amount == 0) {
            token.safeApprove(stockAddress, amount);
        } else {
            token.safeIncreaseAllowance(stockAddress, allowance - amount);
        }
    }
