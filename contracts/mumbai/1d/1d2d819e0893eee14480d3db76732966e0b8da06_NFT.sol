...
contract NFT is ERC721A, Ownable {
    ...
    uint256 private max_nft_supply;

    //maximum amount of mints allowed per person
    // uint256 public max_nft_per_wallet = 5;
    ...
    // extention
    string public baseURLExtention = ".json";

    constructor(uint256 maxNftSupply, string memory _uri) ERC721A("NFT", "Nft") {
        max_nft_supply = maxNftSupply;
        baseTokenURI =_uri;
    }

    // public minting
    function mint(uint256 numberOfNFT) public onlyOwner {
        require(saleIsActive, "Sale in not open yet!");
        require(totalSupply() + numberOfNFT <= max_nft_supply, "Not enough NFTs left to mint.");
    ...
    function tokenURI(uint256 tokenId_) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId_.toString(), baseURLExtention)) : "";
    }
    ...
    function withdraw() public onlyOwner {
        ...
        _withdraw(payable(address(this)), balance); 
    }
