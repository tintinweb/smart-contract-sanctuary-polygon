...
contract Running is Ownable {
    ...
    //Result for running
    mapping(string => uint256) private _result0;
    mapping(string => uint256) private _result1;
    //Index của last result là 0 hoặc 1
    uint8 private lastIndex;
    //Số member bị charge tiền
    uint256 private chargeNumber = 2;
    ...
    function positionOf(string memory domain) public view returns (uint256[] memory) {
        uint256[] memory positions = new uint256[](2);
        positions[0] = _result0[domain];
        positions[1] = _result1[domain];
        return positions;
    }
    ...
    function result(string[] memory domains, uint256 chargeValue) public onlyOwner {
        for (uint256 i = 0; i < domains.length; i++) {
            string memory domain = domains[i];
            require(_wallets[domain] != address(0), "Invalid domain");
            bool isDouble = _result0[domain] > 0 && _result0[domain] <= chargeNumber && _result1[domain] > 0 && _result1[domain] <= chargeNumber;
            uint256 position = domains.length - i;
            if (position <= chargeNumber) {
                uint256 value = isDouble ? chargeValue * 2 : chargeValue;
                charge(domain, value);
            }
            if (lastIndex == 0) {
                _result1[domain] = position;
            } else {
                _result0[domain] = position;
            }
        }
