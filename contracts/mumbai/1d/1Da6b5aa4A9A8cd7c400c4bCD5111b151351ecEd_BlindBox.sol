...
contract BlindBox is OwnableUpgradeable, ItemPool, ReentrancyGuardUpgradeable {
    ...
    uint256 constant public price_n = 0.002 ether;
    uint256 constant public price_w_n = 0.001 ether;
    uint256 constant public price_s = 0.002 ether;
    uint256 constant public price_w_s = 0.001 ether;

    uint256 public lock_amount_n;
    uint256 public lock_amount_s;

    mapping(address => bool) public admin;

    event eventBlindBoxMint(address indexed toAddress, uint256 indexed tokenId, uint256 indexed tokenType);
    event eventOnlyBlindBoxMint(address indexed toAddress, uint256 indexed tokenId, uint256 indexed tokenType);
    ...
    {
        require(msg.value >= price * _amount, "value error");
    ...
    function openBox(address _beneficiary, uint256 _amount, uint256 _boxType) internal
    {
        ...
        if (_boxType == ITEM_TYPE_1)
        {
            ...
            lock_amount_n -= _amount;
        }
        else
        {
            total_weight_s = init_total_weight;
            valid_count_s -= _amount;
            lock_amount_s -= _amount;
            user_buy_count_s[_beneficiary] += _amount;
        }
    }

    function onlyBuyBox(uint256 _amount, uint256 _boxType) external payable nonReentrant
    {
        require(msg.value >= price * _amount, "value error");
        require(_msgSender() == tx.origin, "sender error");
        // require(block.timestamp >= start_time && block.timestamp < end_time, "time limit");
        ...
        if (_boxType == ITEM_TYPE_1)
        {
            require(valid_count_n >= _amount, "amount error");
            require(user_buy_count_n[_msgSender()] + _amount <= max_count_user_n, "amount limit");
            require(lock_amount_n + _amount <= valid_count_n, "total amount limit");
            lock_amount_n += _amount;
        }
        else
        {
            require(valid_count_s >= _amount, "amount error");
            require(user_buy_count_s[_msgSender()] + _amount <= max_count_user_s, "amount limit");
            require(lock_amount_s + _amount + (all_count_s - valid_count_s) <= all_count_n, "total amount limit");
            lock_amount_s += _amount;
        }

        emit eventOnlyBlindBoxMint(_msgSender(), _amount, _boxType);
    }
    ...
    function initLock(uint256 _amount, uint256 _boxType) external checkAdmin
    {
        if (_boxType == ITEM_TYPE_1)
        {
            lock_amount_n -= _amount;
