...
contract MyERC721DropConditional is
    ERC721A,
    LazyMint,
    ContractMetadata,
    Drop,
    ...
    Multicall
{
    ...
    function _getCouponIdx(uint256 _coupon) private view returns (uint256) {
    ...
    function addCoupon(
        ...
        onlyRole(couponAdminRole)
    {
        if (_getCouponIdx(_coupon) < couponSize) {
            couponSize += 1;
        }
        coupons[couponSize] = Coupon(_coupon, _discount, _totalSupply, _usedCount, _expireTimestamp);
    }
    ...
    function removeCoupon(
        ...
        onlyRole(couponAdminRole)
    {
        uint256 _id = _getCouponIdx(_coupon);
        if (_id < couponSize) {
            delete coupons[_id];
            couponSize -= 1;
            return;
        }
        revert("no coupon found");
    }
    ...
    {
        uint256[] memory couponIds = new uint256[](couponSize);
        for (uint256 i = 0; i < couponSize; i++) {
    ...
    function getCoupon(uint256 _coupon)
        public view
        onlyRole(couponAdminRole)
    ...
    function _getPriceIdx(address _currency) private view returns (uint256) {
    ...
    function addPrice(
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        uint256 _id = _getPriceIdx(_currency);
        if (_id < priceSize) {
            priceSize += 1;
        }
        prices[priceSize] = CurrencyPrice(_currency, _price);
    }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        revert("no price available for currency");
    }
    ...
    function getPrice(address _currency)
        public view
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (uint256)
    {
    ...
    function claimWithCoupon(
        ...
        // Update contract state.
        Coupon memory coupon = coupons[_coupon];

        if (coupon.expireTimestamp < block.timestamp && coupon.expireTimestamp != 0) {
            revert("coupon already expired");
        }

        if (coupon.usedCount >= coupon.totalSupply && coupon.totalSupply != 0) {
            revert("total coupons used");
        }

        coupon.usedCount += 1;

        uint256 priceIdx = priceSize + 1;
        for (uint256 i = 0; i < priceSize; i++) {
            if (prices[i].currency == _currency) {
                priceIdx = i;
            }
        ...
        if (priceIdx > priceSize) {
            revert("invalid currency");
        }

        uint256 basePrice = prices[priceIdx].price;
        uint256 discountedPrice = basePrice - ((basePrice * coupon.discount)/100);

        // If there's a price, collect price.
        _collectPriceOnClaim(primarySaleRecipient(), 1, _currency, discountedPrice);

        // Mint the relevant tokens to claimer.
        uint256 startTokenId = _transferTokensOnClaim(_receiver, 1);

        // emit TokensClaimed(_conditionId, _dropMsgSender(), _receiver, startTokenId, 1);
    }
