...
contract RandomReachDebug6 is ERC721URIStorage, Ownable, RrpRequesterV0, EIP712 {
    ...
    function requestRandomNFT(
        ...
        // Check that the cost to mint is being met
        require(msg.value >= mintCost, ERR_MINT_COST_NOT_MET);

        // Check that the user hasn't reached the max mint limit
        require(minterToMintCount[user] < MAX_MINTS_PER_ADDRESS, ERR_MINT_LIMIT_REACHED);

        // Hash the user's data
        ...
        requestToMinter[requestId] = user;
        minterToMintCount[user]++;

        emit RequestedRandom(requestId);
    }

    function fulfillRandom(bytes32 requestId, bytes32 response) external onlyAirnodeRrp {
        ...
        uint256 randomNumber = uint256(response);
        if (randomNumber % 3 == 0) {
