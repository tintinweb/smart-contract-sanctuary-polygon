...
contract ION is ERC20, AccessControl {
    ...
    uint256 trickleTime = 280; // in seconds
 ...
 //Percent base is in 1000
    ...
    function _setEpoch() internal {
        epochEndAmt = (marketCap() * marketCapPer) / (PCT_BASE * 10000); //todo Remove 1000 value
    ...
    function _setEpochEndAmt() internal {
        epochEndAmt = (marketCap() * marketCapPer) / (PCT_BASE * 10000); //todo remove 1000 for deployment
...
interface ISuperCharge {
    ...
    function superChargeRewards(uint) external returns(uint,uint,bool);
    ...
    function canClaim(address,uint) external returns(bool);
    function claimSuperCharge(address user) external;
    function startEpoch() external;
    function endEpoch(uint amount) external;
    function userRewards(address user, uint stakedAmount) external view returns(uint amount, uint end);
}
