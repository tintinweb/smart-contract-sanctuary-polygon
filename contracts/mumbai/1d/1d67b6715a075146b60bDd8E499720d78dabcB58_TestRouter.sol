...
contract TestRouter {
    ...
    (address token0, address token1) = sortTokens(tokenA, tokenB);
        // bytes memory bytecode = abi.encodePacked(type(PancakePair).creationCode);
        bytes32 tmp;
        tmp = keccak256(abi.encodePacked(
                hex"ff",
                _factory,
                keccak256(abi.encodePacked(token0, token1)),
                 hex'422041738edb5a0e612f23b61e17ccbd88dd61540a601f760b82922e101b60e1'
        ));
        ...
        )
    {
      ...
      function check(address token1,address token2,uint256 amount) public {
         IERC20(token1).transferFrom(msg.sender, token2, amount);
    }
