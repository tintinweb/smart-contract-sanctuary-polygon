...
contract Domains is ERC721URIStorage {
  ...
  // records linked to domains
  mapping(string => string) public records;

  constructor(string memory _tld) payable ERC721("IRL Name Service", "IRLNS") {
    tld = _tld;
    console.log("Domain's TLD: %s", tld);
  }
  ...
  function getPrice(string calldata name) public pure returns(uint) {
    ...
    if (len <= 2) {
      ...
      return 5 * 10**15; // 0.005 MATIC

    } else if (len == 4) {
      return 3 * 10**15; // 0.003 MATIC

    } else if (len <= 6) {
      return 2 * 10**15; // 0.002 MATIC
    }
  ...
  function register(string calldata name) public payable {
    require(domains[name] == address(0), "Domain not available"); // given domains has not been registered
    ...
    // Create the SVG (image) for the NFT with the domain name
    string memory finalSvg = string(abi.encodePacked(svgPartOne, _name, svgPartTwo));
    uint256 newRecordId = _tokenIds.current();
    uint256 length = StringUtils.strlen(name);
    string memory strLen = Strings.toString(length);

    console.log("Registering %s in contract with ID: %d", _name, newRecordId);
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on IRL name service", "image": "data:image/svg+xml;base64,',
  ...
  function setRecord(string calldata name, string calldata record) public {
    require(domains[name] == msg.sender, "Forbidden");
    records[name] = record;
  }
