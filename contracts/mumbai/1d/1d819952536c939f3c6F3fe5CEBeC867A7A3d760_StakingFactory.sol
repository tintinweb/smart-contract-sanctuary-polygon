...
contract StakingFactory is Ownable {
    ...
    event DeployChild(address indexed user, address indexed child, uint256 id);

    uint256 private _offerId; 

    StakingMaster private _staking;
    address[] internal _deployed;
    mapping(address => address[]) internal _userChilds;

    constructor(address staking, uint256 offerId) {
        _staking = StakingMaster(staking);
        _offerId = offerId;
    }

    function deploy() external returns(address) {
        address user = _msgSender();
        require(isStaking(user) > 0, "Not allowed");
        StakingChild child = new StakingChild(user);
        _deployed.push(address(child));
        _userChilds[user].push(address(child));
        emit DeployChild(user, address(child), _deployed.length - 1);
        return address(child);
    }

    function isStaking(address user) public view returns (uint256){
        return _staking.isStaking(user, _offerId);
    }
    ...
    function getUserChilds(address user) public view returns(address[] memory) {
        return _userChilds[user];
    }

    function getChild(uint256 id) public view returns(address) {
        return _deployed[id];
    }
...
contract StakingMaster is Ownable {
    ...
    function createOffer(
        ...
        require(fee >= 0 && fee <=100, "Wrong Fee");
        _offersCount += 1;
    ...
    function resignFromOffer(uint256 id) public virtual {
        ...
        require(isStaking(user, id) > 0, "Not in offer");
    ...
    // If returns greater than 0 is because user is staking
    function isStaking(address user, uint256 id) public view returns (uint256) {
        return _stakes[user][id];
    }
...
contract StakingChild is StakingMaster {

    StakingFactory internal _factory;

    modifier isMasterStaking {
        require(_factory.isStaking(owner()) > 0, "Owner not staking");
        _;
    }

    constructor(address owner) {
        _factory = StakingFactory(_msgSender());
    ...
    function createOffer(
        ...
        uint256 duration, 
        string memory membershipURL
    ) public override onlyOwner isMasterStaking {
        super.createOffer(name, token, amount, fee, duration, membershipURL); 
    }

    function removeOffer(uint256 id) public override onlyOwner isMasterStaking {
        super.removeOffer(id);
    }

    function applyForOffer(uint256 id) public override isMasterStaking {
        super.applyForOffer(id);
    }

    function resignFromOffer(uint256 id) public override isMasterStaking {
        super.resignFromOffer(id);
    }
    ...
    function getMasterAddress() public view returns(address) {
        return _factory.getMasterAddress();
    }
}
