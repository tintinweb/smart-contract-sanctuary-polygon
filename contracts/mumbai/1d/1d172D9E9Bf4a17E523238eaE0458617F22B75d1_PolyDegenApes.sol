...
contract PolyDegenApes is 
    ERC721, 
    ERC721Enumerable,
    Ownable 
    {
        event TokenIsMint(uint256 indexed _tokenId, address indexed _owner,uint256 indexed _tokenPrice);
        event TokenIsFreeMint(uint256 indexed _tokenId, address indexed _owner);
    ...
    //=============== variable =======================
        ...
        uint16  private NFTPerAddressLimit = 10;
        uint256 private __MintPrice = 50 ether ; // Default mint price
        ...
        mapping(address => uint256) public __addressMintedBalance;

        address __devAddress = 0xC34B6AD4463255b719524A868766E754e2cc7183;
    //=============== calc function ======================
    ...
    //====================================================
        constructor() ERC721("PolyDegenApes", "PDAPE") {
            _setDefault();
        }
    ...
    //=========================================
        function MintFree(uint256 _numberToMint) external payable whenMintIsLive{
            uint256 totalMinted = totalSupply();
            uint256 ownerMintedCount = __addressMintedBalance[msg.sender];
            require(ownerMintedCount + _numberToMint <= NFTPerAddressLimit, "Max NFT per address exceeded");
            require(_numberToMint > 0, "Cant mint 0" );
            require(_numberToMint <= NFTPerAddressLimit, "Cant mint more then Free max supply" );
            require(totalMinted + _numberToMint <= MaxFree, "Cant go over free supply" );
            for (uint256 i = 0; i < _numberToMint; ++i) {
                uint256 newTokenID=_getNewTokenId();
                _safeMint(msg.sender, newTokenID);
                _useToken(newTokenID);
                __addressMintedBalance[msg.sender]++;
                emit TokenIsFreeMint(newTokenID, msg.sender);
            }
            delete totalMinted;
        ...
        function Mint(uint256 _numberToMint) external payable whenMintIsLive {
            ...
            require(_numberToMint <= 20, "You cannot mint more than 20 Tokens at once!");
            ...
            for (uint256 i = 0; i < _numberToMint; i++) {
                ...
                _safeMint(msg.sender, newTokenID);
                _useToken(newTokenID);
                emit TokenIsMint(newTokenID, msg.sender,__MintPrice);
            }
        ...
        function Withdraw() public onlyOwner {
            ...
            require(ContractBalance>0,"Balanse is 0 .");
            require(payable(__devAddress).send(ContractBalance),"Transfer not Execute .");
            delete ContractBalance;
        }        
        function WithdrawPrice(uint256 _amount) public onlyOwner {
            uint256 ContractBalance = address(this).balance;
            require(ContractBalance>_amount,"Balanse is low .");
            require(ContractBalance>0,"Balanse is 0 .");
