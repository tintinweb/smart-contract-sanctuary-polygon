...
contract RPFMarketplace is ReentrancyGuard {
    ...
    mapping (uint => mapping(uint => uint)) rfpToMilestone;
    mapping (uint => uint) milestoneNums;
    ...
    function createMilestone(
        ...
        idToMilestone[idMilestone].rfpID = _rfpId;
        uint curNum = milestoneNums[_rfpId];
        rfpToMilestone[_rfpId][curNum] = idMilestone;
        milestoneNums[_rfpId] += 1;
        idMilestone += 1;
    ...
    function setSellerID(uint _rfpID, uint _buyerID, uint _sellerID, uint _sellerWalletID, address _sellerAddress) public {
        ...
        idToRFP[_rfpID].sellerAddress = payable(_sellerAddress);
    ...
    function getTotalRFPCreated() external view returns (RFP[] memory) {
        
        RFP[] memory rfps = new RFP[](idRFP);
        uint curId = 0;
        for (uint i = 0; i < idRFP; i++){

            if(idToRFP[i].ended == false){
                RFP storage currentRFP = idToRFP[i];
                rfps[curId] = currentRFP;
                curId += 1;
            }
        }
        return rfps;
    ...
    function getRFP(uint _rfpid) external view returns (RFP memory) {
        return idToRFP[_rfpid];
    }

    function getTotalRFPs() external view returns(uint) {
        return idRFP;
    }

    function getTotalMilestones() external view returns(uint) {
        return idMilestone;
    }
}