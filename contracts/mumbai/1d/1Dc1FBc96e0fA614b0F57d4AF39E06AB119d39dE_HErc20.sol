...
contract HToken is ERC1155, IERC721Receiver, ExponentialNoError, TokenErrorReporter, AccessControlEnumerable {
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    uint256 allowed = hivemind.redeemAllowed(address(this), msg.sender, underlyingToRedeem);
    if (allowed != 0) {
      revert HivemindError(Error.COMPTROLLER_REJECTION);
    }
  ...
  function _mintCoupon(
    ...
    coupon.couponId = tokenId.current();
  ...
  function borrowInternal(uint256 _borrowAmount, uint256 _tokenId) internal {
    ...
    require(hivemind.borrowAllowed(address(this), msg.sender, _tokenId, _borrowAmount) == 0, "Borrow error");
  ...
  function repayBorrowInternal(uint256 _repayAmount, uint256 _tokenId) internal {
    uint256 allowed = hivemind.repayBorrowAllowed(address(this), msg.sender, _tokenId, _repayAmount);
    ...
    require(_repayAmount > 0, "Amount can not be 0");
  ...
  function repayBorrowBehalfInternal(
    ...
    // Can't repay 0 amount
    require(_repayAmount > 0, "Can't repay 0.");
