...
import {ProtocolInterfaces} from '../Constant.sol';
...
contract SynthereumFixedRateFactory is IDeploymentSignature, ReentrancyGuard, SynthereumFixedRateCreator {
    //----------------------------------------
    // Storage
    //----------------------------------------
     ...
     */
    constructor(address synthereumFinder) SynthereumFixedRateCreator(synthereumFinder) {
    ...
    {
        address deployer = synthereumFinder.getImplementationAddress(ProtocolInterfaces.Deployer);
        require(msg.sender == deployer, 'Sender must be Synthereum deployer');
        console2.log(params.syntheticName);
        console2.log('xxxx1');
...
import {IMintableBurnableTokenFactory} from '../tokens/erc20/factories/interfaces/IMintableBurnableTokenFactory.sol';
import {IProtocolFinder} from '../core/interfaces/IFinder.sol';
import {IMintableBurnableERC20} from '../tokens/interfaces/IMintableBurnableERC20.sol';
import {BaseControlledMintableBurnableERC20} from '../tokens/interfaces/BaseControlledMintableBurnableERC20.sol';
import {ProtocolInterfaces} from '../Constant.sol';
import {SynthereumFixedRateWrapper} from './FixedRateWrapper.sol';
import {ISynthereumFixedRateWrapper} from './IFixedRateWrapper.sol';
...
contract SynthereumFixedRateCreator {
    ...
    IProtocolFinder public immutable synthereumFinder;

    //----------------------------------------
    // Events
    //----------------------------------------
    event CreatedFixedRate(address indexed fixedRateAddress, uint8 indexed version, address indexed deployerAddress);
    ...
    constructor(address _synthereumFinder) {
        synthereumFinder = IProtocolFinder(_synthereumFinder);
    }
     ...
     */
    function createFixedRate(Params calldata params) public virtual returns (SynthereumFixedRateWrapper fixedRate) {
        console2.log(params.syntheticName);
        console2.log(bytes(params.syntheticName).length);
        console2.log(bytes(params.syntheticSymbol).length);
        console2.log('asim3');
        require(bytes(params.syntheticName).length != 0, 'Missing synthetic name');
        require(bytes(params.syntheticSymbol).length != 0, 'Missing synthetic symbol');
        console2.log('Amm her1.3');
        if (params.syntheticToken == address(0)) {
            console2.log('Amm her2');
            console2.log(params.roles.admin);
            IMintableBurnableTokenFactory tokenFactory = IMintableBurnableTokenFactory(
                IProtocolFinder(synthereumFinder).getImplementationAddress(ProtocolInterfaces.TokenFactory)
            );
            BaseControlledMintableBurnableERC20 tokenCurrency =
                tokenFactory.createToken(params.syntheticName, params.syntheticSymbol, 18);
            console2.log('Amm her3');
            fixedRate = new SynthereumFixedRateWrapper(
        _convertParams(params, tokenCurrency)
      );
            console2.log('Amm her4');
            ...
            tokenCurrency.addAdmin(synthereumFinder.getImplementationAddress(ProtocolInterfaces.Manager));
            ...
            require(
                keccak256(abi.encodePacked(tokenCurrency.symbol())) == keccak256(abi.encodePacked(params.syntheticSymbol)),
    ...
    function _convertParams(Params memory params, BaseControlledMintableBurnableERC20 tokenCurrency)
        internal
        view
        returns (SynthereumFixedRateWrapper.ConstructorParams memory constructorParams)
    {
        console2.log(address(params.collateralToken));
        ...
        constructorParams.fixedRateToken = IMintableBurnableERC20(address(tokenCurrency));
        constructorParams.roles = params.roles;
        constructorParams.rate = params.rate;
        console2.log('am here 3.1');
    }
}
    ...
    ReentrancyGuard
{
    ...
    IProtocolFinder private finder;
    ...
    modifier onlyMaintainer() {
        require(hasRole(MAINTAINER_ROLE, _msgSender()), 'Sender must be the maintainer');
        _;
    }
    ...
    constructor(ConstructorParams memory params) nonReentrant {
        console2.log('am here 3.2');
        require(params.pegCollateralToken.decimals() <= 18, 'Collateral has more than 18 decimals');
        console2.log('am here 3.3');
        require(params.fixedRateToken.decimals() == 18, 'FixedRate token has more or less than 18 decimals');
        console2.log('am here 3.4');
        rate = params.rate;
        ...
        returns (uint256 amountTokens)
    {
        ...
        amountTokens = (_collateral * (10 ** (18 - pegCollateralToken.decimals())) * rate) / PRECISION;
    ...
    {
        require(fixedRateToken.balanceOf(_msgSender()) >= _tokenAmount, 'Not enought tokens to unwrap');
     ...
     */
    function collateralToken() external view override returns (IERC20 collateralCurrency) {
     ...
     */
    function syntheticTokenSymbol() external view override returns (string memory) {
     ...
     */
    function synthereumFinder() external view override returns (IProtocolFinder) {
        return finder;
    }
     ...
     */
    function totalSyntheticTokensMinted() external view override returns (uint256) {
    ...
    function isTrustedForwarder(address forwarder) public view override returns (bool) {
        try finder.getImplementationAddress(ProtocolInterfaces.TrustedForwarder) returns (address trustedForwarder) {
