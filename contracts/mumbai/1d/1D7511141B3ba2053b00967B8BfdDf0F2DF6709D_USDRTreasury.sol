...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
        ...
        )
    {
        ...
        tngbl_ =
            IPriceOracle(oracle).quote(IERC20(tngbl).balanceOf(address(this))) *
        ...
        rwa =
            ...
            underlyingMultiplier;
        liquidity = underlyingLPAmount * underlyingMultiplier;
        if (includeUSDRLiquidity) {
            liquidity += usdrLPAmount * 1e9;
        }
        debt =
    ...
    function burnTNGBL() external onlyRole(CONTROLLER_ROLE) {
        ...
        if (tngblBalance > 0) {
            ...
            if ((scaledMarketCap * tngblBurnThreshold) / 100 <= backing) {
                ...
                IExchange(exchange).afterBurnTNGBL(0);
            } else {
                uint256 tngblPrice = IPriceOracle(oracle).quote(1e18);
                uint256 avgMintPrice = IExchange(exchange).avgTNGBLMintPrice();
                if (tngblPrice > avgMintPrice) {
                    burnAmount =
                        (tngblBalance * tngblPrice) /
                        avgMintPrice -
                        tngblBalance;
                    /*if (burnAmount < tngblBalance / 100) { // todo uncomment for production
                        burnAmount = 0;
                    } else {*/
                        IExchange(exchange).afterBurnTNGBL(burnAmount);
                    //}
                }
            }
        }
