...
contract PodTokenUpgradeable is
  ERC721AUpgradeable,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(string memory _name, string memory _symbol) public initializer initializerERC721A {
    maxSupply = 1000;
  ...
  function setBaseURI(uint256 _maxSupply, string memory baseURI_) public onlyRole(DEFAULT_ADMIN_ROLE) {
    baseURI = baseURI_;
    maxSupply = _maxSupply;
  }

  function setMaxSupply(uint16 _maxSupply) public onlyRole(DEFAULT_ADMIN_ROLE) {
    maxSupply = _maxSupply;
  }

  function airdropTokens(address[] memory wallets) external payable nonReentrant onlyRole(MINTER_ROLE) {
    require(totalSupply() + wallets.length <= maxSupply, "max supply exceeded");
