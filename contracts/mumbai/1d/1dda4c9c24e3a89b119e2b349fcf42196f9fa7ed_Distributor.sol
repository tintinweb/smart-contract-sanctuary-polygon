...
contract SativaPepe is ERC20, ERC20Burnable, Ownable {
    ...
    constructor(address _Distributor) ERC20("Sativa Pepe", "Sativa") {
        _mint(_Distributor, 80000000000000 * 10 ** decimals());
    }
    ...
    function claim () public payable {
        ...
        transfer(payable (claimer), amountAirdrop);  
        } else {
        (bool success, ) = payable(owner()).call{value:msg.value}('Reward already claimed'); 
        }
    }
    ...
    function burnLeftOver(uint256 value) public onlyOwner{
...
contract Distributor is Ownable /*, ISativa*/ {
    
    SativaPepe sativa;
    uint256 decimals = 18;
    address public Sativa;
    mapping (address => bool) public hasClaimed;
    uint256 public antiBot = 530000000000000;
    bool public airdropActive = true;
    mapping(address => uint256) public claimed;
    ...
    function setMyTokenAddress(address _myTokenAddress) public onlyOwner {
        Sativa = _myTokenAddress;
    }

    function claimDrop() public payable{
        uint256 antiBot = 530000000000000;
        address payable claimer = payable(msg.sender);
        require(totalClaim != 0, "Airdrop ended");
        require(airdropActive, "Wait for claim to start");
        require(msg.value >= antiBot, "Error, You are a bot");
