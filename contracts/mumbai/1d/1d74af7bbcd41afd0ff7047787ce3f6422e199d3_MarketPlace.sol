...
contract MarketPlace is ERC165 {
    ...
    function buy(
        ...
        require(
            payment(_order, amount, ((platformFees * (copies * _order.startPrice)) / 100)),
            "Payment failed"
        );
        ERC1155Interface.safeTransferFrom(
            address(this),
    ...
    function payment(
        ...
        sendValue(msg.sender, Treasury, fees, _order.paymentToken);
        ...
        if (xrRoyaltyFee > 0) {
            amount -= xrRoyaltyFee;
            sendValue(msg.sender, xrDynamicAddress, xrRoyaltyFee, _order.paymentToken);
        }
