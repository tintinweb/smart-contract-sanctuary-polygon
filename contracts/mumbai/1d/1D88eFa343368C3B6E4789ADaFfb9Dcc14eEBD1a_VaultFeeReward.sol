...
contract VaultFeeReward is Governable, ReentrancyGuard, Pausable {
    ...
    event SetKiloPerp(address kiloPerp);
    event ClaimedReward(
        address user,
        address rewardToken,
        uint256 amount
    ...
    constructor(address _kiloPerp, address _rewardToken, uint256 _rewardTokenBase) {
        owner = msg.sender;
        kiloPerp = _kiloPerp;
    ...
    function updateReward(address account) public {
        ...
        uint256 vaultReward = IKiloPerp(kiloPerp).distributeVaultReward();
        uint256 supply = IKiloPerp(kiloPerp).getTotalShare();
        if (supply > 0) {
        ...
        //我持有的股份在一小周期内的收益
        claimableReward[account] += IKiloPerp(kiloPerp).getShare(account) * (cumulativeRewardPerTokenStored - previousRewardPerToken[account]) / PRECISION;
    ...
    function reinvest() external returns (uint256 reinvestAmount){
        ...
        if (reinvestAmount > 0) {
            if (rewardToken == address(0)) {
                IKiloPerp(kiloPerp).stake{value: reinvestAmount}(reinvestAmount * BASE / rewardTokenBase, msg.sender);
            } else {
                IERC20(rewardToken).safeApprove(kiloPerp, 0);
                IERC20(rewardToken).safeApprove(kiloPerp, reinvestAmount);
                IKiloPerp(kiloPerp).stake(reinvestAmount * BASE / rewardTokenBase, msg.sender);
            }
    ...
    function getClaimableReward(address account) external view returns(uint256) {
        ...
        uint256 _pendingReward = IKiloPerp(kiloPerp).getPendingVaultReward();
        ...
        return currentClaimableReward + IKiloPerp(kiloPerp).getShare(account) * (_rewardPerTokenStored - previousRewardPerToken[account]) / PRECISION;
    }
