...
contract AdvisorVault is Ownable {
  ...
  function claim() public onlyAdvisor {
    ...
    require(nextClaimEpoch < block.timestamp, "Can not claim for now" );

    AdvisorInfo storage infos = advisorInfos[msg.sender];
    require(infos.totalClaimedAmount < infos.lockedAmount, "Empty vault");

    uint256 periodCount = (block.timestamp - infos.nextClaimEpoch) / unlockPeriod + 1;

    uint256 amountToUnlock = periodCount * unlockAmountPerPeriod;
    uint256 amount = infos.lockedAmount * amountToUnlock / unlockAmountPerPeriodDenominator;

    infos.totalClaimedAmount += amount;

    if(infos.nextClaimEpoch == 0) {
      infos.nextClaimEpoch = lockPeriodStart + unlockPeriod;
    } else {
      infos.nextClaimEpoch += unlockPeriod;
    }
  ...
  function addAvisor(address _addr, uint256 _amount) public onlyOwner {
