...
contract PolygonBSG {
    ...
    uint256 private constant timeStep = 1 days; //1 days
    uint256 private constant dayPerCycle = 15 days; //15 days
    uint256 private constant maxAddFreeze = 50 days; //2%
    ...
    uint256 private constant boosterPoolPercents = 50; //4%
    uint256 private constant insurancePoolPercents = 100; //0.5%
    ...
    uint256 private constant maxInfinityL1toL5 = 5000e6;
    ...
    function _calLevelNow(address _user) private view returns (uint256) {
        ...
        if(user.maxDeposit >= 2500e6 && directTeam >=15 && user.maxDirectDeposit >= 15000e6 && totalTeam >= 400 && maxTeam >= 150000e6 && otherTeam >= 150000e6){
            level = 4;
    ...
    function _isBooster(address _user) private {
        if(orderInfos[_user].length > 2 && !(userInfo[_user].isbooster)) {
