...
// File: contracts/vortem-staking.sol
...
contract VortemStaking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    mapping(uint256 => uint256) public _firstDepositBlocks;
    ...
    // 10 $VORTEM-SHARD PER DAY
    // n Blocks per day = 43200, Token Decimal = 18
    // Rate = 10 * 10 ** 18 / 43200 = 2.31481481e14 wei
    function setRate(uint256 _rate) public onlyOwner {
        rate = _rate;
    }

    // n Blocks per day = 43200
    // starts from 1, everyday increase by 0.1 up to 2.0
    function rateMultiplier(uint256 _depositBlock)
        public
        view
        returns (uint256 mutiplier)
    {
        uint256 blockSinceDeposit = Math.min(block.number, expiration) -
            _depositBlock;
        uint256 multiplier = 100 + Math.min(blockSinceDeposit / 43200, 100);
        return multiplier;
    }

    // Set this to a block to disable the ability to continue accruing tokens past that block number.
    // Currently at 43200 blocks per day
    // Set pool to run for 5 years, 5 * 365 * 43200 = 78840000
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            rewards[i] =
                (rate *
                    ...
                    rateMultiplier(_firstDepositBlocks[tokenId])) /
                100;
        }
        ...
        returns (uint256)
    {
        ...
        return
            (rate *
                ...
                (Math.min(block.number, expiration) -
                    _depositBlocks[account][tokenId]) *
                rateMultiplier(_firstDepositBlocks[tokenId])) / 100;
    }
    ...
    function deposit(uint256[] calldata tokenIds) external whenNotPaused {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _firstDepositBlocks[tokenIds[i]] = Math.min(
                block.number,
                expiration
            );
        }
        ...
        nonReentrant
    {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            ...
            _firstDepositBlocks[tokenIds[i]] = 0;
            IERC721(stakingDestinationAddress).safeTransferFrom(
                address(this),
                msg.sender,
                tokenIds[i],
