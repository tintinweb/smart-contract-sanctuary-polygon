...
// Interface para acceder a funciones de Rents Payer
interface IRentsPayer {
...
contract MigrationsReward is AccessControl, IUserRewards {
    bytes32 private constant DEV_ROLE = keccak256("DEV_ROLE");
    IRewardsOld public oldRewards;
    IRewardsControl public newRewards;
    IRentsPayer public rentsPayer;
    ...
    constructor(address _oldRewards, address _newRewards, address _rentsPayer) {
        ...
        rentsPayer = IRentsPayer(_rentsPayer);
    }
    ...
    /// @dev Function to migrate the Builder and Holders Rewards from the Old Rewards Control to the New Rewards Control.
    function migrateBH(address[] calldata _wallets) external onlyDev {
        ...
        for (uint256 i = 0; i < length;) {
            ...
            RB = oldRewards.getBuilderRewards(_wallets[i]);
            RH = oldRewards.getHolderRewards(_wallets[i]);
    ...
    /// @dev Function to migrate the Rents from Old Rewards Control to the New Rewards Control.
    function migrateRents(address[] calldata _wallets, address _collection) external onlyDev {
        uint256 length = _wallets.length;
        UserRewardsRent memory userRents;

        for (uint256 i = 0; i < length;) {
            // get the data from old contract.
            (uint256 rewardPerRent, uint256 lastTimePayRent, uint256 lastTimeCalcRent, uint256 rewardPerRentNFT) =
                rentsPayer.getRewardsPerUserRent(_wallets[i], _collection);

            userRents.rewardPerRent = rewardPerRent;
            userRents.lastTimePayRent = lastTimePayRent;
            userRents.lastTimeCalcRent = lastTimeCalcRent;
            userRents.rewardPerRentNFT = rewardPerRentNFT;
            userRents.rentsRetained = 0;
            // write the data to new rewards control contract.
            newRewards.setMigrationRentData(userRents, _collection, _wallets[i]);
