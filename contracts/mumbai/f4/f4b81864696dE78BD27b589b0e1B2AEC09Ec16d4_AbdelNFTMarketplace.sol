...
contract AbdelNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    constructor(
        ...
        _bidRate = bidRate_;
        require(blueToken_ != address(0), "constructor: token address zero");
        blueToken = IERC20(blueToken_);
    }
    ...
    function getUserRoyalty() external view returns (uint256) {
    ...
    function listItem(
        ...
        require(
            listData.nftAddress != address(0),
            "listItem: nft address is zero"
        );
        require(listData.itemsAvailable != 0, "listItem: nft amount is zero");

        if (listData.lazyMint) {
            _check(
                listData.tokenId,
                listData.nftAddress,
                _nftMaxCopies,
        ...
        require(
            listData.expirationTime > listData.listingTime,
            "listItem: Expiration date invalid"
        );
        require(
            listData.listingTime >= block.timestamp - 300,
            "listItem: listing time invalid"
        );
        require(listData.basePrice != 0, "listItem: Price cannot be zero");
        require(
            listData.itemsAvailable != 0,
            "listItem: cannot list zero numner of items"
        );

        if (listData.saleKind == SaleKind.Auction) {
            require(
                listData.reservePrice >= listData.basePrice,
                "listItem: Reserve price is lower than base price"
            );
            require(
                listData.itemsAvailable == 1,
        ...
        if (!listData.lazyMint) {
            IERC1155(listData.nftAddress).safeTransferFrom(
                _msgSender(),
                address(this),
                listData.tokenId,
                listData.itemsAvailable,
        ...
        nonReentrant
    {
        ...
        blueToken.safeTransferFrom(_msgSender(), address(this), _bidAmount);
    }
    ...
    function buyItem(
        ...
        blueToken.safeTransferFrom(_msgSender(), address(this), _totalPrice);
    ...
    function retractOffer(uint256 _itemId) external itemExists(_itemId) {
        ...
        blueToken.safeTransfer(_msgSender(), amount);
    ...
    function withdrawServiceFee(address account) external onlyOwner {
        ...
        blueToken.safeTransfer(account, amount);
    ...
    function _purchaseWithLazyMinting(
        ...
        blueToken.safeTransfer(item_.seller, payment);
    ...
    function _refund(uint256 amount, address receiver) internal {
        blueToken.safeTransfer(receiver, amount);
    }
