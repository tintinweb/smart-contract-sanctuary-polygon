...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    event SameChainSwap(uint256 amount, IERC20Upgradeable[] tokens, uint256[] flags);
    ...
    // add wrapper contract for samechain swap - not to be used by bridge

    /**
     * To be called from UI only for same chain swap
     */
    function swapSameChainFromUI(
        IERC20Upgradeable[] memory tokens,
        uint256 amount,
        uint256 minReturn,
        uint256[] memory flags,
        ...
        emit SameChainSwap(returnAmount, tokens, flags);
    }
