...
contract GenesisStoneEveStaking is AccessControl {
    ...
    struct StakedNFTInfo {
        ...
        bool normalMinted; // whether the staker has minted a normal Eve
        bool mythicMinted; // whether the staker has minted a mythic Eve
    }
    ...
    /// @dev Checks if the given tokenId has been used to mint a normal Eve
    /// @param tokenId_ The tokenId of the GenesisStone NFT
    function _isNormalMinted(uint256 tokenId_) internal view returns (bool) {
        return _stakedNFTInfos[tokenId_].normalMinted;
    }

    /// @dev Checks if the given tokenId has been used to mint a mythic Eve
    /// @param tokenId_ The tokenId of the GenesisStone NFT
    function _isMythicMinted(uint256 tokenId_) internal view returns (bool) {
        return _stakedNFTInfos[tokenId_].mythicMinted;
    }

    /// @notice Checks if the given tokenIds have been used for normal Eve minting
    /// @param tokenIds_ An array of GenesisStone NFT tokenIds
    /// @return results An array of bool values indicating whether each tokenId has been used for normal Eve minting
    function hasBeenUsedForMinting(
        ...
        for (uint256 i = 0; i < tokenIds_.length; ) {
            usedForMinting[i] = _isNormalMinted(tokenIds_[i]);
            unchecked {
                i++;
            }
        }
    ...
    /// @notice Checks if the given tokenIds have been used for mythic Eve minting
    /// @param tokenIds_ An array of GenesisStone NFT tokenIds
    /// @return results An array of bool values indicating whether each tokenId has been used for mythic Eve minting
    function hasBeenUsedForMythicMinting(
        uint256[] calldata tokenIds_
    ) external view returns (bool[] memory) {
        bool[] memory usedForMinting = new bool[](tokenIds_.length);

        for (uint256 i = 0; i < tokenIds_.length; ) {
            usedForMinting[i] = _isMythicMinted(tokenIds_[i]);
            unchecked {
                i++;
            }
        }
    ...
    function numOfNormalEveMintable(
        ...
        for (uint256 i = 0; i < stakedCount; ) {
            uint256 tokenId = stakedTokens.at(i);
            unchecked {
                if (_stakingComplete(tokenId) && !_isNormalMinted(tokenId)) {
                    result++;
                }
                i++;
            }
    ...
    function mintEve(uint256[] calldata tokenIds_) external {
        for (uint256 i = 0; i < tokenIds_.length; ) {
            if (_isNormalMinted(tokenIds_[i])) {
            ...
            // Mint the NFT
            _stakedNFTInfos[tokenIds_[i]].normalMinted = true;
            ...
            emit EveMinted(tokenIds[0], MINT_TYPE_NORMAL);
    ...
    /// @notice Mints a number of mythic Eve NFTs for the caller
    /// @param tokenIds_ An array of GenesisStone NFT tokenIds
    function mintMythicEve(uint256[] calldata tokenIds_) external {
        for (uint256 i = 0; i < tokenIds_.length; ) {
            if (!_isMythicStone(tokenIds_[i])) {
                revert Utils.NotAMythicStone();
            }

            if (_isMythicMinted(tokenIds_[i])) {
                revert Utils.AlreadyMinted();
            }
            ...
            // Mint the NFT
            _stakedNFTInfos[tokenIds_[i]].mythicMinted = true;
            uint256[] memory tokenIds = EVE_CONTRACT.mintMythic(msg.sender, 1);

            // Emit event for the minted Eve tokenId
            _stakerInfos[msg.sender].mintedNFTs.add(tokenIds[0]);
            emit EveMinted(tokenIds[0], MINT_TYPE_MYTHIC);
    ...
    /// @dev Unstake a genesis stone NFT
    /// @param tokenIds_ The tokenIds of the genesis stone NFT to unstake
    function unstake(uint256[] calldata tokenIds_) external {
        EnumerableSet.UintSet storage stakedTokens = _stakerInfos[msg.sender]
            .stakedNFTs;

        for (uint256 i = 0; i < tokenIds_.length; ) {
            uint256 tokenId = tokenIds_[i];
            if (!stakedTokens.contains(tokenId)) {
                revert Utils.NotStaked();
            }
            ...
            _stakedNFTInfos[tokenId].stakedTime = 0;
