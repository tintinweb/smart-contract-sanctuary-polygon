...
contract Lighthouse is ERC721A {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://lighters.live/api/nft/metadata/";
    }
    ...
    function tokensOfOwner(
        ...
        uint256[] memory sortedTokens = new uint256[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            sortedTokens[i] = tokens[i];
        }

        sortAscending(sortedTokens);
        return sortedTokens;
    }
