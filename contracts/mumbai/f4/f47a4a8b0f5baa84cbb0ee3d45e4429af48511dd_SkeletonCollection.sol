 ...
 */
contract SkeletonCollection is
    Initializable,
    ICollection,
    ...
    OwnableUpgradeable
{
    ...
    function initialize(
        ...
        collectionId = 3;
        buyBackValue = 10 * 10 ** 18; // 10 Matic
    ...
    function depositForFreeMint(uint32 _mintCount) public payable {
        require(_mintCount >= 1 && _mintCount <= 1000, "wrong mintCount");
        ...
        returns (uint256 cost, uint256 collectionMintFee, uint256 partnerFee)
    {
        ...
        require(_amount >= 1 && _amount <= 100, "wrong amount");
    ...
    function commit(
        ...
        if (_freeMint) {
            require(_amount >= 1 && _amount <= freeMintCount, "wrong amount");
        }
        ...
        require(msg.value >= cost, "not enough f");
    ...
    function reveal(bytes32 _revealHash) public {
        ...
        require(getHash(_revealHash) == commits[msg.sender].commit, "wrong C");
        ...
        )
    {
        ...
        for (uint8 i = 1; i < payoutDeck.length; ) {
            if (payoutDeck[i]) {
                if(bonusCardCount > 1){
                    if(i < 5){
                        bonusPercent += ((1_000 * 10 ** 4) / 100_000);
                    }else{
        ...
        if(payoutDeck[1] && payoutDeck[2] && payoutDeck[3] && payoutDeck[4]){
            allAces = true;
        }
        floorValue = comboValue / 2;
        floorValue = (floorValue * buyBackValue) / 10 ** 8;
        uint256 winValue = (comboValue * (10 ** 4 + bonusPercent)) /
            2 /
            10 ** 4;
        uint64 buybackDIM = 10 ** 10;
        buyback =
            (winValue * (buyBackValue / buybackDIM) * buybackDIM) /
            10 ** 8;
    ...
    function claimBuyBack(uint160[] calldata _tokenIds) public {
        ...
        if (buybackMandatoryAces) {
            require(allAces, "A not provided");
        }
    ...
    function checkDiscount(
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(collectionId, _discount, msg.sender)
        );
        fee = _fee;
        if (_isVerifiedCoupon(digest, _coupon)) {
    ...
    function setPartnerFee(uint32 _newFee) external onlyOwner {
        // Update partnerFee
        require(_newFee < 100_000, "should be < 100");
    ...
    function setPartnerCommission(uint32 _newCommission) external onlyOwner {
        // Update partnerFee
        require(_newCommission < 100_000, "should be < 100");
    ...
    function setPartnerMarketplaceFee(
        ...
        require(_newPartnerMarketplaceFee < 100_000, "should be < 100");
