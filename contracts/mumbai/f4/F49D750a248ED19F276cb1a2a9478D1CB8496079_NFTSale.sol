...
contract NFTSale is ERC721, Ownable, ReentrancyGuard {
    using Strings for uint256;
    ...
    function toggleWlMintActive(bool _status) external onlyOwner {
        wlMintActive = _status;
    }

    // Set whitelist
    function toggleWhitelist(address _address, bool status) external onlyOwner {
    ...
    function whitelistMint(
        ...
        require(_quantity == 1, "only 1 mint per user");
        require(wlMintActive, "Whitelist sale is closed at the moment.");
        require(!wlMintStatus[msg.sender], "already minted");

        address _to = msg.sender;
        require(whitelists[_to], "You're not whitelisted.");

        wlMintStatus[msg.sender] = true;
