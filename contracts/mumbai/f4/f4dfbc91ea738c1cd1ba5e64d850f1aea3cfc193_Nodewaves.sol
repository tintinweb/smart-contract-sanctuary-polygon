...
contract Nodewaves is ERC20, ERC20Burnable, Ownable {
    ...
    mapping(address => uint) public nodeAffiliateRewards;

    uint public maxSupply = 10e9 * 10 ** decimals();
    ...
    uint public distributionTime = 365;
    ...
    mapping(address => uint) public previousToken;

    uint public daysPassed;
    ...
    uint public APY = 40000;
    uint public burnAmount = 1000;
    uint public burnAffiliatePercentage = 2500;

    uint public poolReward =0;
    address public poolAddress;
    uint public polRewardPercentage = 1000;
    uint public development = 0;
    address public developmentAddress;
    uint public developmentPercentage = 300;
    uint public systemOperation = 0;
    address public systemOperationAddress;
    uint public systemOperationPercentage = 200;
    ...
    function PoolDevAndSysWithdraw(uint _index) public {
        if (_index == 0) {
    ...
    function setPoolRewardPercentage(uint _percentage) public onlyOwner {
        polRewardPercentage = _percentage;
    }

    function setDevelopmentPercentage(uint _percentage) public onlyOwner {
        developmentPercentage = _percentage;
    }

    function setSystemOperationPercentage(uint _percentage) public onlyOwner {
        systemOperationPercentage = _percentage;
    }

    uint[10] public burnAfflilatePercentages = [
        1000,
        1000,
        1000,
        1000,
    ...
    mapping(address => uint) public burnAffiliateRewards;
    ...
    function burnWithdraw() public {
        ...
        uint _amount = _burning.amount;

        require(_amount > 0, "You don't have any burning amount");

        uint _totalRewards = _amount.mul(APY).div(10000);

        uint _rewardPerdays = _totalRewards.div(distributionTime);

        uint _days = (block.timestamp.sub(_burning.lastwithdraw)).div(
            payoutTime
        );

        uint _reward = _days.mul(_rewardPerdays);

        require(_reward <= tokenForPoB, "Not enough rewards in the PoB");

        tokenForPoB = tokenForPoB.sub(_reward);

        uint _affiliateReward = _reward.mul(burnAffiliatePercentage).div(10000);
        ...
        uint _burnReward = _reward.sub(_affiliateReward);
    ...
    function burn(uint amount, address[10] memory _affliliates) public {
        require(
    ...
    function nodeWithdraw() public {
        ...
        uint _days = (block.timestamp.sub(node.lastWithdraw)).div(payoutTime);
        uint _dayilyAmount = tokenForNode.div(distributionTime);
        uint _amount = _days.mul(_dayilyAmount).div(numberOfNodes);

        require(_amount <= tokenForNode, "Not enough tokens in the node");
        tokenForNode = tokenForNode.sub(_amount);

        uint _previous = previousToken[msg.sender];
        previousToken[msg.sender] = 0;

        if (node.affiliate != address(0)) {
            uint _affiliateAmount = _amount.mul(10).div(10 ** decimal);
            nodeAffiliateRewards[node.affiliate] =
                nodeAffiliateRewards[node.affiliate] +
                _affiliateAmount;
    ...
    function team() public view returns (uint) {
        ...
        uint _dayilyAmount = tokenForTeam.div(distributionTime);
        return _days * _dayilyAmount;
    }
    ...
    function advisors() public view returns (uint) {
        ...
        uint _dayilyAmount = tokenForAdvisors.div(distributionTime);
        return _days * _dayilyAmount;
    }
    ...
    function liquidity() public view returns (uint) {
        ...
        uint _dayilyAmount = tokenForLiquidity.div(distributionTime);
        return _days * _dayilyAmount;
    }
    ...
    function devAndEco() public view returns (uint) {
        ...
        uint _dayilyAmount = tokenForDevAndEco.div(distributionTime);
        return _days * _dayilyAmount;
    }

    function sp() public view returns (uint) {
        return distributedateSP;
    }

    function withdrawToken(uint _index) public {
        if (_index == 0) {
            require(
    ...
    function affiliateWithdraw() public {
        uint _affiliateAmount = nodeAffiliateRewards[msg.sender];
    ...
    function setNodeAffiliateRewards(
        ...
        uint[] memory _nodeAffiliateRewards
    ) public onlyOwner {
        for (uint i = 0; i < _nodeAffiliates.length; i++) {
