...
contract Token is IERC20, Ownable {
    ...
    function _transferFee(address from, uint256 tax) private returns (bool _success) {
        ...
        uint256 taxAmountNewMovies;
        uint256 taxAmountProfit; 
        uint256 taxAmountUserCredit;
        uint256 totalDivided;

        uint256 taxPercentagesDevelopment = taxPercentages[uint256(taxAddressType.taxAddressDevelopment)];
        uint256 taxPercentagesNewMovies = taxPercentages[uint256(taxAddressType.taxAddressNewMovies)];
        uint256 taxPercentagesProfit = taxPercentages[uint256(taxAddressType.taxAddressProfit)];
        uint256 taxPercentagesUserCredit = taxPercentages[uint256(taxAddressType.taxAddressUserCredit)];

        if(taxPercentagesDevelopment > 0)  {
            // Calculate Tax Amount
            taxAmountDevelopment = (tax * taxPercentagesDevelopment) / (100);
            // Transfer Tax Amount
            _balances[taxAddresses[uint256(taxAddressType.taxAddressDevelopment)]] += taxAmountDevelopment;
            //Transfer Event for Development Tax Address
            emit Transfer(from, taxAddresses[uint256(taxAddressType.taxAddressDevelopment)], taxAmountDevelopment);
        }

        if(taxPercentagesNewMovies > 0)  {
            // Calculate Tax Amount
            taxAmountNewMovies = (tax * taxPercentagesNewMovies) / (100);
            // Transfer Tax Amount
            _balances[taxAddresses[uint256(taxAddressType.taxAddressNewMovies)]] += taxAmountNewMovies;
            //Transfer Event for New Movies Tax Address
            emit Transfer(from, taxAddresses[uint256(taxAddressType.taxAddressNewMovies)], taxAmountNewMovies);

        }

        if(taxPercentagesProfit > 0)  {
            // Calculate Tax Amount
            taxAmountProfit = (tax * taxPercentagesProfit) / (100);
            // Transfer Tax Amount
            _balances[taxAddresses[uint256(taxAddressType.taxAddressProfit)]] += taxAmountProfit;
            //Transfer Event for Profit Tax Address
            emit Transfer(from, taxAddresses[uint256(taxAddressType.taxAddressProfit)], taxAmountProfit);

        }

        if(taxPercentagesUserCredit > 0)  {
            // Calculate Tax Amount
            taxAmountUserCredit = (tax * taxPercentagesUserCredit) / 100;
        }
        // Calculate Total Divided
        totalDivided = taxAmountDevelopment + taxAmountNewMovies + taxAmountProfit + taxAmountUserCredit;
        //Calculate remaining fee to be divided
        uint256 residual = (tax - totalDivided);
        //Transfer the remaining fee
        taxAmountUserCredit += residual;
        if(taxAmountUserCredit > 0){
            // Transfer Tax Amount
            _balances[taxAddresses[uint256(taxAddressType.taxAddressUserCredit)]] += taxAmountUserCredit;
            //Transfer Event for Profit Tax Address
            emit Transfer(from, taxAddresses[uint256(taxAddressType.taxAddressUserCredit)], taxAmountUserCredit);
        }
        return true;
    }
