...
contract LottoGame is VRFConsumerBaseV2 {
    ...
    function buyTickets(string calldata _referrerUsername, string calldata _username, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket memory) {
        ...
        if(keccak256(abi.encodePacked(_referrerUsername)) != keccak256(abi.encodePacked())){
        ...
        (uint8 num1, uint8 num2, uint8 num3, uint8 num4, uint8 num5, uint8 num6) = getTicket(uint256(((gameCount-1)*poolCapacity) + _userIndex));
        ticketCount++;
