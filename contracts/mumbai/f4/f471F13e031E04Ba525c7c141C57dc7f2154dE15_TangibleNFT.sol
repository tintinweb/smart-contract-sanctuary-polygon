    ...
    IERC721Receiver
{
    ...
    function lockTNGBL(
        ...
        if (toLock > IFactory(factory).passiveNft().maxLockDuration()) {
            toLock = IFactory(factory).passiveNft().maxLockDuration();
        }
    ...
    function _destroyTNFT(uint256 tokenId, address burningFrom) internal {
        ...
        console.log("lastPaidDate timestamp %s", storageEndTime[tokenId]);
        console.log(
            "lastPaidDate plus 180 days %s",
            (storageEndTime[tokenId] + 180 days)
        );
        if (
            ownerAllowedToBurn ||
            ...
            // if (
            //     ((IFactory(factory).isFactoryOperator(tokenOwner) || IFactory(factory).isFactoryAdmin(tokenOwner) ||
            //         (!_isStorageFeePaid(tokenId) &&
            //             (storageEndTime[tokenId] + 180 days) < block.timestamp &&
            //             IFactory(factory).isFactoryOperator(burningFrom))) &&
            //         !_isTokenMinter(tokenOwner, tokenId)) ||
            //     (IFactory(factory).isFactoryAdmin(burningFrom) &&  tokenOwner == burningFrom)||
            //     !storageRequired
            // ) {
            _burn(tokenId);
            console.log("prosao burn");
    ...
    function adjustStorageAndGetAmount(
        ...
        console.log("current timestamp %s", block.timestamp);
        console.log("lastPaidDate timestamp %s", lastPaidDate);

        //calculate to which point storage will last
        lastPaidDate += _years * 365 days;
        storageEndTime[tokenId] = lastPaidDate;

        console.log("lastPaidDate timestamp endDate is %s", lastPaidDate);
