...
contract VCProject is Initializable {
    ...
    uint256 _projectBalance; // in USD

    // Raised amounts: only increase after funding, never decrease
    uint256 _raisedAmountOutsideCampaigns; // in USD
    ...
    function startCampaign(
        ...
        _onlyStarter();
        if (_target == 0) {
            revert Errors.ProjInvalidCampaignTarget();
        }
    ...
    function publishCampaignResults() external {
        ...
        if (campaignStatus != CampaignStatus.SUCCEEDED || _campaigns[currentCampaignId].resultsPublished) {
    ...
    // function closeCampaign() external returns (uint256 campaignId) {
    //     _onlyStarter();
    //     campaignId = _numberOfCampaigns - 1;

    //     // check if this campaign can be closed, related to the duration of the campaign
    //     if (block.timestamp < _campaigns[campaignId].endTime) {
    //         revert Errors.ProjCampaignCannotBeClosed();
    //     }
    //     if (_campaigns[campaignId].isClosed) {
    //         revert Errors.ProjCampaignAlreadyClosed();
    //     }
    //     if (!_closeCampaign(campaignId)) {
    //         revert Errors.ProjCampaignBalanceIsNotZero();
    //     }
    // }

    // function _closeCampaign(uint256 _campaignId) internal returns (bool) {
    //     if (_campaigns[_campaignId].balance == 0) {
    //         _campaigns[_campaignId].isClosed = true;
    //         return true;
    //     }
    //     return false;
    // }
    ...
    function backerWithdrawDefeated(address _user, IERC20 _currency)
        external
        returns (
            uint256 campaignId,
        ...
        )
    {
        ...
        campaignId = _numberOfCampaigns - 1;
        uint256 backersDeadline = _campaigns[campaignId].backersDeadline;
        bool canWithdraw = getCampaignStatus(campaignId) == CampaignStatus.NOTFUNDED && block.timestamp <= backersDeadline;

        if (!canWithdraw) {
            revert Errors.ProjCampaignNotNotFunded();
        }

        backerBalance = _backers[campaignId][_user][_currency];
        if (backerBalance == 0) {
            revert Errors.ProjBackerBalanceIsZero();
        }

        _backers[campaignId][_user][_currency] = 0;
        _updateBalances(campaignId, _currency, backerBalance, false);
        if (_campaigns[campaignId].balance == 0) {
            statusDefeated = true;
        }
    ...
    function transferUnclaimedFunds(IERC20 _currency, address _pool)
        external
        returns (
            uint256 campaignId,
            uint256 _amountToPool,
            bool _statusDefeated
        )
    {
        _onlyStarter();
        ...
        campaignId = _numberOfCampaigns - 1;
        uint256 backersDeadline = _campaigns[campaignId].backersDeadline;
        bool canTransfer = getCampaignStatus(campaignId) == CampaignStatus.NOTFUNDED && block.timestamp > backersDeadline;

        if (!canTransfer) {
            revert Errors.ProjCannotTransferUnclaimedFunds();
        }

        _amountToPool = _campaignBalance[campaignId][_currency];
        if (_amountToPool == 0) {
            revert Errors.ProjBalanceIsZero();
        }

        _updateBalances(campaignId, _currency, _amountToPool, false);
        if (_campaigns[campaignId].balance == 0) {
    ...
    function getCampaignStatus(uint256 _campaignId) public view returns (CampaignStatus currentStatus) {
        ...
        } else if ((raisedAmount == target) || (raisedAmount >= softTarget && currentTime > endTime)) {
            if (balance > 0) {
...
library Errors {
    ...
    error ProjInvalidCampaignTarget();
    error ProjBackerBalanceIsZero();
    error ProjAlreadyClosed();
    error ProjBalanceIsNotZero();
    error ProjLastCampaignNotClosed();
    error ProjCampaignCannotBeClosed();
    error ProjCampaignBalanceIsNotZero();
    error ProjCampaignAlreadyClosed();
}
