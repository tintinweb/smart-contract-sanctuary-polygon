  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(uint256 _feePercent, uint256 _royaltyPercent)
    external
    initializer
  {
    __Ownable_init();
  ...
  function buyAssetFromListing(
    ...
    if (listing.erc20Address == address(0)) {
      ...
      if (listing.price > token.allowance(msg.sender, address(this))) {
        revert(
      ...
      for (uint256 i; i < listing.tokenIds.length; i++) {
        AddressUpgradeable.sendValue(payable(listing.creators[i]), royalityFee);
        token.transferFrom(msg.sender, listing.creators[i], royalityFee);
      }
