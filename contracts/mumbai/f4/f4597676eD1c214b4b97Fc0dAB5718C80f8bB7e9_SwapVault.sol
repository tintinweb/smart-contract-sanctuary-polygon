    ...
    ISwapVault
{
    ...
    */
    ...
    modifier onlySwapManager() {
        require(
            IMasterWhitelist(whitelistAddress).isSwapManagerWhitelisted(
                msg.sender
            ),
            "Not Swap Manager"
        );
    ...
    */
    function createSwapOrder(
        ...
        uint256 _amount,
        uint256 _rate
    ) external onlySwapManager onlyDuringPoolState(_poolId, PoolStatus.LOCKED) {
    ...
    */
    function resetPoolState(bytes32 poolId) external onlySwapManager {
        require(
            _changePoolStatus(poolId, PoolStatus.WAITING_FOR_RATE),
            "Pool Status Change Failed"
        );
