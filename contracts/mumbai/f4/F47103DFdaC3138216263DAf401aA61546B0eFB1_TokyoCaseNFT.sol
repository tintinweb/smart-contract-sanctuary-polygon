...
contract BaseNFT is ERC721Enumerable, ERC2981, IERC4906, IERC4907, Ownable, Pausable {
    ...
    function _afterTokenTransfer(
        ...
        if (from_ != to_) {
            if (to_ == address(0)) {
                delete _holdingStartedAts[firstTokenID_];
            } else {
                _holdingStartedAts[firstTokenID_] = block.timestamp;
            }
...
contract TokyoCaseNFT is BaseNFT {
    ...
    constructor(uint256 maxSupply_) BaseNFT("TOKYO CASE NFT", "TCN") {
    ...
    function airdrop(address to_) external onlyMinter whenNotPaused {
        require(
            ...
            "TCN: minted amount exceeds the upper limit"
        );
        require(!_isAirdroppeds[to_], "TCN: already airdropped");

        _airdrop(to_);
    }

    function bulkAirdrop(address[] calldata tos_)
        external
        onlyMinter
        whenNotPaused
    {
        ...
        for (uint256 i = 0; i < tos_.length; i++) {
            require(!_isAirdroppeds[tos_[i]], "TCN: already airdropped");
