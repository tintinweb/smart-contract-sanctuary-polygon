...
contract PledgeStakeV2 {
    ...
    function _updateRewards(address _user) private {        
        uint256 roundRewards = getRoundRewards(_user);
        uint256 levelRewards = getLevelRewards(_user);

        rewards[_user].roundRewards = roundRewards;
        rewards[_user].levelRewards = levelRewards;

        // Update the total rewards
        rewards[_user].totalRewards = rewards[_user].directRewards.add(roundRewards).add(levelRewards);

        // Calculate the withdrawable rewards
        uint256 withdrawnRewards = rewards[_user].withdrawnRewards;
        uint256 withdrawable = rewards[_user].totalRewards.sub(withdrawnRewards);
        rewards[_user].availableRewards = withdrawable;

        // Update the stake index for the user
        lastProcessedStakeIndex[_user] = stakes[_user].length;
    }

    function getRoundRewards(address _user) public view returns (uint256) {
        ...
        for (uint256 i = lastProcessedStakeIndex[_user]; i < stakes[_user].length; i++) {
            ...
            uint256 completedRounds = elapsedTime.div(roundDuration);
            uint256 remainingTime = elapsedTime.mod(roundDuration);

            if (completedRounds >= 1 && completedRounds <= totalRounds) {
                roundRewards += (userStake.amount * roundRewardPercentages[completedRounds - 1]) / 100;
            }

            // Handle partial round rewards if stake is currently active
            if (completedRounds < totalRounds && remainingTime > 0) {
                roundRewards += (userStake.amount * roundRewardPercentages[completedRounds]) / 100;
            }
        }
        return roundRewards;
    }

function getLevelRewards(address _user) public view returns (uint256) {
        uint256 levelRewards = 0;
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        while (upline != address(0) && userInfo[upline].totalReferrals >= 2) {
            uint256 level = userInfo[upline].level;
            uint256 roundRewards = getRoundRewards(upline); // Calculate the round rewards for the referrer
            levelRewards += (roundRewards * levelPercentage[level - 1]) / 100;
            upline = userInfo[upline].referrer;
        }
        return levelRewards;
    }
