...
contract XtraMATIC is Ownable {
	...
	uint256 public passivePlanStartUNIX;
	...
	constructor(address payable wallet,uint256 _startTime) {
		...
		passivePlanStartUNIX = _startTime.add(TIME_STEP.mul(14));
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		require(plan<3 ||(plan<6 && block.timestamp > passivePlanStartUNIX),"Passive plans not activated yet!");
	...
	function getPercent(uint8 plan) public view returns (uint256) {
		if(plan<3){
			...
			if (block.timestamp > passivePlanStartUNIX) {
				return plans[plan].percent.add(PERCENT_STEP.mul(block.timestamp.sub(passivePlanStartUNIX)).div(TIME_STEP));
			} else {
				return plans[plan].percent;
			}
		}
