...
contract BackerPayV1Union is IBackerPayV1Union {
        ...
        ensure(deadline)
    {
        ...
        uint256 commission = (msg.value * p.rate) / 1000000;
        p.balanceOf[address(0)] = p.balanceOf[address(0)].add(msg.value.sub(commission));
        emit Ledger(
            paymentId,
            commission,
            msg.value,
    ...
    function paymentToken(
        ...
        uint256 commission = (amount * p.rate) / 1000000;
        p.balanceOf[tokenAddr] = p.balanceOf[tokenAddr].add(amount.sub(commission));
        emit Ledger(
            paymentId,
            commission,
            amount,
