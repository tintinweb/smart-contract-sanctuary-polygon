...
contract BinarySystemContract is Ownable {
    ...
    function activate(address parent, string memory record) external {
        if (IERC20(usdtToken).balanceOf(msg.sender) >= 110000000) {
            IERC20(usdtToken).transferFrom(msg.sender, parent, 50000000);
            IERC20(usdtToken).transferFrom(msg.sender, address(this), 50000000);
            IERC20(usdtToken).transferFrom(msg.sender, adminA, 5000000);
            IERC20(usdtToken).transferFrom(msg.sender, adminC, 5000000);
            // BBAB
    ...
    function purchase(
        ...
        if (IERC20(usdtToken).balanceOf(msg.sender) >= purchaseAmount) {
