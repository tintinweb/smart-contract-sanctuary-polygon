...
import "@polychainmonsters/nfbs/contracts/interfaces/INFBOpenerExternal.sol";
...
contract SummoningEffectNFBOpener is
    INFBOpenerExternal,
    VRFConsumerBaseV2,
    ConfirmedOwner
{
    struct RequestStatus {
        ...
        uint256[] randomWords;
        address to; // receiver of the revealed NFT
    }
    ...
    function openViaNfb(
        ...
        /// @dev only permit summoning effect NFBs to be opened and not items that might be launched in the future
        require(
            seriesId == 1 && editionId == 1,
            "SummoningEffectNFBOpener: Invalid series or edition"
        );

        uint32 numWords = uint32(amount);
    ...
    function fulfillRandomWords(
        ...
        require(
            ...
            "SummoningEffectNFBOpener: Request not found"
        );
        require(
        ...
        for (uint256 i = 0; i < _randomWords.length; i++) {
            ...
            ISummoningEffectTypes.SummoningEffect memory summoningEffect = SummoningEffectTraits
                .getSummoningEffect(randomWord);
            polygear.mint(
                requests[_requestId].to,
                1,
                1,
                SummoningEffectTraits.packSummoningEffect(summoningEffect.elementType, summoningEffect.summoningLevel)
            );
        }
