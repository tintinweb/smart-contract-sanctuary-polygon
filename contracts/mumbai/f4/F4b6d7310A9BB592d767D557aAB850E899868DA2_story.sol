...
// File: storyTeller/storyTeller_0.0.2_nft.sol
...
contract story is ERC721 {
    ...
    function writeStory (
        ...
        // Check if the payment is at least 0.01 MATIC
        require(msg.value >= 0.01 * 10 ** 18, "Minimum payment of 0.01 MATIC required");

        require(
            _backStoryId == 0 || (
                storyList[_backStoryId].year < currentYear() &&
        ...
        Story memory newStory = Story({
            ...
            year: (block.timestamp - startTime) / 60,
            incident: _incident,
            isFinal: _isFinal,
            teller: msg.sender
        });
    ...
    function withdraw(uint256 amount) public {
        require(msg.sender == 0xfbc853EeF4Ad085Efb60A66f2aAc0fB976af71F6, "Only the owner can withdraw MATIC");
    ...
    function mintNFT(uint256 year, string memory incident, uint256 storyId) internal {
     ...
     * Metadata processing.
     */
    function tokenURI(uint256 _storyId) public view override returns (string memory) {
        require(_exists(_storyId), "ERC721Metadata: URI query for nonexistent token");

        Story memory incident = storyList[_storyId];
        //year, storyId, incident
        //#storyId
        string memory svg = getSVG(incident, _storyId);
        bytes memory json = abi.encodePacked(
            '{"name": "',
            abi.encodePacked("#", Strings.toString(_storyId)),
    ...
    function getSVG(Story memory _incident, uint256 _storyId) private view returns (string memory) {
        
        return
            string(
                abi.encodePacked(
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300"><path d="M0 0h300v300H0z"/><text fill="#fff" font-family="Times" font-size="10" font-weight="400"><tspan x="10" y="20">',
                ...
                abi.encodePacked("After ", getStory(_incident.backStoryId).incident, "..."),
                '</tspan></text><text fill="#fff" font-family="Times" font-size="8" font-weight="400"><tspan x="10" y="80">',
                _incident.incident,
                '</tspan></text><text fill="#fff" font-family="times" font-size="8" font-weight="400"><tspan x="10" y="270">',
                abi.encodePacked("STORY: #", Strings.toString(_storyId)),
                '</tspan></text><text fill="#fff" font-family="times" font-size="8" font-weight="400"><tspan x="10" y="290">',
                abi.encodePacked("BACKED STORY: #", Strings.toString(_incident.backStoryId), ", in ", Strings.toString(_incident.year)),
                '</tspan></text></svg>'
         )
 );
    }
