...
contract Runiverse is Pausable, ReentrancyGuard, EIP712, RuniverseAdministrative, IDomainSeparator {
    ...
    function checkSignature(bytes memory signature, SignedStartGame calldata player, uint256 gameId) internal {
        ...
        {
            bytes32 structHash = keccak256(abi.encode(_JOLLYUSED_TYPEHASH,player.sender, player.playerAddress, player.tokenChosen,player.amountBetted, gameId, player.deadline));
            address signer = ECDSA.recover(_hashTypedDataV4(structHash), v, r, s);
            require(signer == player.sender, "invalid signature");
        } 
...
abstract contract CryptorunFeeSharer is CryptorunNFTLinker  {
	...
	function _setArenaFee(uint256 gameId, address currencyFee, uint256 fee) internal {
        ...
        if(_totalArenaMinted==0){
            arenaTokenId = 1;
        }
        else
        {
            arenaTokenId = (gameId % _totalArenaMinted) + 1;
        }
