...
enum CrowdsaleStage {Before,Stage1,Between,Stage2,After,Finalize}
    ...
    function claim(address beneficiary_) external {
        ...
        if(contributionMap.get(beneficiary_, true) != 0) {
            sablier.withdrawFromStream(_stage1StreamId[beneficiary_], sablier.balanceOf(_stage1StreamId[beneficiary_], beneficiary_));

        } if (contributionMap.get(beneficiary_, false) != 0) {
            sablier.withdrawFromStream(_stage2StreamId[beneficiary_], sablier.balanceOf(_stage2StreamId[beneficiary_], beneficiary_));
        }
    }
    ...
    function deliverTokens(uint256 startIndex_, uint256 endIndex_, uint256 startTime_) external onlyOwner{
        ...
        uint256 maxIndex = getTotalInvestors();
        ...
        for (uint i = startIndex_; i < endIndex_+1; i++) {
    ...
    function getTokenClaimable(address _beneficiary ) public view returns (uint256) {
        uint256 balance = 0;
        if(contributionMap.get(_beneficiary, true)> 0) {
            balance += sablier.balanceOf(_stage1StreamId[_beneficiary], _beneficiary);
        } if(contributionMap.get(_beneficiary, false)> 0) {
            balance += sablier.balanceOf(_stage2StreamId[_beneficiary], _beneficiary);
        }
        return balance;
    }
