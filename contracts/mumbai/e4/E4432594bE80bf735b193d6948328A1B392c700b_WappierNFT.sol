...
contract WPaymentSplitter is Context {
    ...
    event PayeeReplaced(address prev, address account);
    ...
    /**
     * @dev Replaces the `index` payee by `account`.
     * @param index The index of payee in `_payees`.
     * @param account The address of the new payee.
     */
    function _replacePayee(uint256 index, address account) internal virtual {
        address prev = _payees[index];

        _shares[account] = _shares[prev];
        delete _shares[prev];

        _released[account] = _released[prev];
        delete _released[prev];

        _payees[index] = account;
        emit PayeeReplaced(prev, account);
    }
...
abstract contract WP721Marketplace is WPEIP712, WPERC721, WPaymentSplitter, Pausable {
     ...
     * @param voucher A signed `NFTVoucher` that describes the token to be redeemed.
     */
    function redeemVoucher(NFTVoucher calldata voucher) public payable whenNotPaused {
     ...
     */
    function releaseAll() external whenNotPaused onlyRole(getRoleAdmin(MINTER_ROLE)) {
        _releaseAll();
    }

    /**
     * @dev Retuns the amount of Ether available to the caller to release.
     * @return The pending withdrawal balance of the caller.
     */
    function availableToRelease() external view whenNotPaused returns (uint256) {
        return _pendingPayment(msg.sender);
    }

    /**
     * @dev Replaces the `index` payee by `account`. Reverts if the caller is not an authorized minter.
     */
    function replacePayee(uint256 index, address account) external whenNotPaused onlyRole(getRoleAdmin(MINTER_ROLE)) {
        _replacePayee(index, account);
    }
