...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base, Pausable {
    ...
    mapping(uint256 => CancelledLoan) public isLoancancelled;
    ...
    event LoaneeCancelledLoan(address loanee, uint256 loanId);
    ...
    function issueLoan(
        ...
        emit LoanIssued(offer.loanee, _loanId);
    }
    
    /**
     * Loanee can cancel loan
     *
     * @param _loanId - Id of the loanable item
     */
    
    function cancelLoanByLoanee(uint256 _loanId ) external nonReentrant whenNotPaused{
        address sender = _msgSender();
        require(loanItems[_loanId].owner != address(0), "Loanable Item Not Found");
        require(sender == loanItems[_loanId].loanee, "Only lonee can cancel the loan");
        require(
            (block.timestamp - loanItems[_loanId].startingTime) <= loanItems[_loanId].timePeriod,
            "Inactive loan item"
        );
        isLoancancelled[_loanId].isCancellled=true;
        isLoancancelled[_loanId].loanee=sender;
        loanItems[_loanId].loanee=address(0);
        loanItems[_loanId].startingTime=0;
        
        emit LoaneeCancelledLoan(sender,_loanId);
    }
    ...
    function claimNFTRewards(uint256 _loanId) external nonReentrant whenNotPaused {
        ...
        require(loanItems[_loanId].startingTime > 0 || isLoancancelled[_loanId].isCancellled==true, "Inactive loan item");
        require(
            sender == loanItems[_loanId].owner || sender == loanItems[_loanId].loanee ||sender== isLoancancelled[_loanId].loanee,
            "Either loaner or loanee can claim nft rewards"
        );
        require(
            (block.timestamp - loanItems[_loanId].startingTime) >= loanItems[_loanId].timePeriod || isLoancancelled[_loanId].isCancellled==true,
        ...
        if (loanItems[_loanId].claimer == NFTRewardsClaimer.loaner) {
            ...
            require(sender == loanItems[_loanId].loanee || sender== isLoancancelled[_loanId].loanee, "Only Loanee can claim NFT rewards");
