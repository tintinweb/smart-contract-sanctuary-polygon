...
import "./ICrossTalkApplication.sol";
import "./Utils.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint64 public stateLastValsetNonce;
    ...
    bytes32 constant REQUEST_FROM_ROUTER_METHOD_NAME =
        0x7265717565737446726f6d526f75746572000000000000000000000000000000;
    // bytes32 encoding of "requestFromSource"
    bytes32 constant REQUEST_FROM_SOURCE_METHOD_NAME =
        0x7265717565737446726f6d536f75726365000000000000000000000000000000;
    // bytes32 encoding of "crossTalkAck"
    bytes32 constant CROSS_TALK_ACK_METHOD_NAME =
        0x63726F737354616C6B41636B0000000000000000000000000000000000000000;

    mapping(string => mapping(uint64 => uint8)) public outboundNonceMapping;
    // cross-talk sequencial nonce mapping
    // (destChainid{string} => (destChainType{uint64} => nonce{uint64}))
    mapping(string => mapping (uint64 => uint64)) public destChainNonces;
    // cross-talk nonce validation mapping for incoming requests from different chains
    // (srcChainid{string} => (srcChainType{uint64} => (nonce{uint64} => handled{uint8})))
    mapping(string => mapping (uint64 => mapping (uint64 => uint8))) public crossTalkNonceMapping;

    event RequestToRouterEvent(
        address indexed applicationContract,
    ...
    event RequestToDestEvent(
        bytes applicationContract,
        uint64 indexed eventIdentifier,
        Utils.SourceChainParams srcChainParams,
        Utils.AckGasParams ackGasParam,
        Utils.DestinationChainParams destChainParams,
    ...
    event ValsetUpdatedEvent(
        uint64 indexed _newValsetNonce,
        uint64 indexed _eventNonce,
        uint64 srcChainType,
        string srcChainId,
        address[] _validators,
        uint64[] _powers
    ...
    error ExecuteCallsRevert(bool[] execFlags, bytes[] execData);
    
    function validateValsetPower(uint64[] memory _powers) private pure {
        uint64 cumulativePower = 0;
        for (uint64 i = 0; i < _powers.length; i++) {
    ...
    modifier isSelf() {
        require(_msgSender() == address(this), "ExecuteHandleCalls : Can only be called by Current Contract");
        _;
    }
    ...
    function makeCheckpoint(Utils.ValsetArgs memory _valsetArgs, string memory _chainId)
        internal
        pure
        returns (bytes32)
    {
    ...
    function updateValset(
        ...
        // pass a vote
        validateValsetPower(_newValset.powers);
    ...
    /**
        ...
        @param _chainType chain type such as evm, substrate, cosmos etc.
     */
    function initialize(
        ...
        stateLastValsetCheckpoint = newCheckpoint;
        byteChainId = keccak256(abi.encodePacked(_chainId));
        eventNonce++;
        emit ValsetUpdatedEvent(valsetNonce, eventNonce, chainType, chainId, _validators, _powers);
        // Constructor Fx
    }

    function _authorizeUpgrade(address newImplementation) internal virtual override onlyRole(DEFAULT_ADMIN_ROLE) {}

    function requestToRouter(bytes memory payload, string memory routerBridgeContract)
        external
        whenNotPaused
        returns (uint64)
    {
        eventNonce++;
        emit RequestToRouterEvent(msg.sender, eventNonce, chainType, routerBridgeContract, chainId, payload);
        return eventNonce;
    }
    ...
    function requestToDest(
        ...
        returns (uint64)
    {
        destChainNonces[destChainParams.destChainId][destChainParams.destChainType]++;
        uint64 crossTalkNonce = destChainNonces[destChainParams.destChainId][destChainParams.destChainType];
        eventNonce++;
        Utils.SourceChainParams memory srcChainParams = Utils.SourceChainParams(
            crossTalkNonce,
            expTimestamp,
            isAtomicCalls,
            chainType,
    ...
    function requestFromRouter(
        ...
        Utils.Signature[] calldata _sigs,
        Utils.RouterRequestPayload memory requestPayload
    ) external nonReentrant whenNotPaused {
        ...
        require(
            outboundNonceMapping[requestPayload.routerBridgeAddress][requestPayload.outboundTxNonce] == 0,
        ...
        bytes memory encodedABI = abi.encode(
            // bytes32 encoding of "requestFromRouter"
            REQUEST_FROM_ROUTER_METHOD_NAME,
            chainType,
            chainId,
            requestPayload.routerBridgeAddress,
            requestPayload.outboundTxNonce,
        ...
        outboundNonceMapping[requestPayload.routerBridgeAddress][requestPayload.outboundTxNonce] = 1;
        eventNonce++;
        uint64 currentNonce = eventNonce;
        if (block.timestamp > requestPayload.expTimestamp) {
            bytes memory emptyData = "";
            emit EventOutboundAck(
                requestPayload.relayerRouterAddress,
                requestPayload.routerBridgeAddress,
                chainId,
                chainType,
                currentNonce,
    ...
    function executeHandlerCalls(
        ...
        bool isAtomic
    ) external isSelf returns (bool[] memory) {
        ...
        for (uint8 i = 0; i < handlers.length; i++) {
            ...
            if (success) {
                (success, data) = handler.call(
                    abi.encodeWithSelector(IApplication.handleRequestFromRouter.selector, sender, payloads[i])
                );
                console.logBytes(data);
            }
            successFlags[i] = success;
            if (isAtomic && !success) {
                bytes memory encodedData = abi.encode(successFlags);
    ...
    function requestFromSource(
        ...
        // These are arrays of the parts of the validators signatures
        Utils.Signature[] calldata _sigs,
        Utils.CrossTalkPayload memory crossTalkPayload
    ) external nonReentrant whenNotPaused {
        // chainId, chainType data is present inside the signature validation
        // valsetArgs validation will be handled by the makeCheckPoint function
        require(
            crossTalkNonceMapping[crossTalkPayload.sourceParams.chainId][crossTalkPayload.sourceParams.chainType][crossTalkPayload.crossTalkNonce] == 0,
            "cross-talk request message already handled"
        );
        ...
        bytes memory encodedABI = abi.encode(
            // bytes32 encoding of "requestFromSource"
            REQUEST_FROM_SOURCE_METHOD_NAME,
            crossTalkPayload.eventIdentifier,
            crossTalkPayload.crossTalkNonce,
            chainType,
            chainId,
            crossTalkPayload.sourceParams.chainId,
            crossTalkPayload.sourceParams.chainType,
            crossTalkPayload.sourceParams.caller,
            crossTalkPayload.isAtomic,
            crossTalkPayload.expTimestamp,
            crossTalkPayload.contractCalls.destContractAddresses,
            crossTalkPayload.contractCalls.payloads
        );
        bytes32 messagehash = keccak256(encodedABI);
        bytes32 digest = _make_digest(messagehash);
        SignatureUtils.checkValidatorSignatures(_currentValset, _sigs, digest, Utils.constantPowerThreshold);

        crossTalkNonceMapping[crossTalkPayload.sourceParams.chainId][crossTalkPayload.sourceParams.chainType][crossTalkPayload.crossTalkNonce] = 1;
        eventNonce++;
        uint64 currentNonce = eventNonce;
        destChainNonces[crossTalkPayload.sourceParams.chainId][crossTalkPayload.sourceParams.chainType]++;
        uint64 crossTalkNonce = destChainNonces[crossTalkPayload.sourceParams.chainId][crossTalkPayload.sourceParams.chainType];
        
        if (block.timestamp > crossTalkPayload.expTimestamp) {
            bytes memory emptyData = "";
            emit CrossTalkAckEvent(
                crossTalkPayload.relayerRouterAddress,
                crossTalkPayload.sourceParams.caller,
                crossTalkPayload.sourceParams.chainId,
                crossTalkPayload.sourceParams.chainType,
                chainId,
        ...
        (bool success, bytes memory data) = address(this).call(
            abi.encodeWithSelector(
                this.executeCrossTalkCalls.selector,
                crossTalkPayload.sourceParams.caller,
                crossTalkPayload.contractCalls.destContractAddresses,
                crossTalkPayload.contractCalls.payloads,
                crossTalkPayload.isAtomic,
    ...
    function executeCrossTalkCalls(
        ...
        uint64 srcChainType
    ) external isSelf returns (bool[] memory, bytes[] memory) {
        bool[] memory execFlags = new bool[](handlers.length);
        bytes[] memory execData = new bytes[](handlers.length);
        for (uint8 i = 0; i < handlers.length; i++) {
            address handler = toAddress(handlers[i]);
            bool success;
            bytes memory data;
            ...
            if (success) {
                (success, data) = handler.call(
                    abi.encodeWithSelector(ICrossTalkApplication.handleRequestFromSource.selector, srcContractAddress, payloads[i], srcChainId, srcChainType)
                );
                console.logBytes(data);
            }
            execFlags[i] = success;
            execData[i] = data;
            if (isAtomic && !success) {
                revert ExecuteCallsRevert({
                    execFlags: execFlags,
                    execData: execData
                });
            }
        }
        return (execFlags, execData);
    }

    function crossTalkAck(
        // The validators that approve the call
        Utils.ValsetArgs calldata _currentValset,
        // These are arrays of the parts of the validators signatures
        Utils.Signature[] calldata _sigs,
        Utils.CrossTalkAckPayload memory crossTalkAckPayload
    ) external nonReentrant whenNotPaused {
        // chainId, chainType data is present inside the signature validation
        // valsetArgs validation will be handled by the makeCheckPoint function
        require(
            crossTalkNonceMapping[crossTalkAckPayload.destChainId][crossTalkAckPayload.destChainType][crossTalkAckPayload.crossTalkNonce] == 0,
            "cross-talk request message already handled"
        );
        ...
        bytes memory encodedABI = abi.encode(
            // bytes32 encoding of "crossTalkFromRouter"
            CROSS_TALK_ACK_METHOD_NAME,
            crossTalkAckPayload.eventIdentifier,
            crossTalkAckPayload.crossTalkNonce,
            chainType,
            chainId,
            crossTalkAckPayload.destChainType,
            crossTalkAckPayload.destChainId,
            crossTalkAckPayload.srcContractAddress,
            crossTalkAckPayload.execFlags,
            crossTalkAckPayload.execData
        );
        ...
        crossTalkNonceMapping[crossTalkAckPayload.destChainId][crossTalkAckPayload.destChainType][crossTalkAckPayload.crossTalkNonce] = 1;
        
        (bool success, bytes memory data) = address(this).call(
            abi.encodeWithSelector(
                this.executeCrossTalkAck.selector,
                crossTalkAckPayload.eventIdentifier,
                crossTalkAckPayload.srcContractAddress,
                crossTalkAckPayload.execFlags,
                crossTalkAckPayload.execData
    ...
    function executeCrossTalkAck(
        ...
        address handler = toAddress(srcContractAddress);
        bool success;
        bytes memory data;
        success = isContract(handler);
        if (success) {
            (success, data) = handler.call(
                abi.encodeWithSelector(ICrossTalkApplication.handleCrossTalkAck.selector, eventIdentifier, execFlags, execData)
            );
            console.logBytes(data);
        }
    }
