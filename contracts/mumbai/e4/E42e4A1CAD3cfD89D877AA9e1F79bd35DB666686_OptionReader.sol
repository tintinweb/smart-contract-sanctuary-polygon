...
contract OptionReader {
    ...
    {
        IBinaryOptions options = IBinaryOptions(optionsContract);
    ...
    function getPayout(
        ...
        uint256 maxAmount = options.getMaxUtilization();
        // Calculate the max fee due to the max txn limit
        uint256 maxPerTxnFee = ((pool.availableBalance() *
            config.optionFeePerTxnLimitPercent()) / 100e2);
        uint256 newFee = min(amount, maxPerTxnFee);
        ...
        (uint256 unitFee, , ) = options.fees(10**(options.decimals()), isAbove);
        payout = (newFee * 10**(options.decimals())) / unitFee;

        if (payout > maxAmount || newFee < amount) {
            payout = min(payout, maxAmount);
        }
