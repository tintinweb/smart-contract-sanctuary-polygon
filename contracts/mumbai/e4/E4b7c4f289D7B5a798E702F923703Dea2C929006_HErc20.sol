...
contract HToken is ERC1155, IERC721Receiver, ErrorReporter, AccessControlEnumerable {
   ...
   */
  mapping(uint256 => uint256) public borrowCouponsIdPerCollateral;
  ...
  */
  ...
  function getUserCoupons(address _user) public view returns (Coupon[] memory) {
    unchecked {
      ...
      uint256 counter;
      for (; i < length; i++) {
        if (borrowCoupons[borrowCouponsIdPerCollateral[i]].owner == _user) counter++;
      }
  ...
  function accrueInterestToCoupons() public returns (uint256) {
    ...
    for (index = 1; index <= length; ) {
      Coupon memory coupon = borrowCoupons[borrowCouponsIdPerCollateral[index]];
      if (coupon.active == 2) {
  ...
  function _mintCoupon(
    ...
    borrowCouponsIdPerCollateral[currentId] = _collateralId;
    // userToCoupons[msg.sender]++;
    // Mint NFT
  ...
  function liquidationAllowedByCollateralId(uint256 _collateralId) internal view returns (bool allowed) {
    unchecked {
      ...
      uint256 length = idCounter.current();
      for (; i < length; i++) {
        Coupon memory coupon = borrowCoupons[borrowCouponsIdPerCollateral[i]];
  ...
  function _beforeTokenTransfer(
    ...
    for (j; j < _ids.length; j++) {
      ...
      if (_ids[j] > 0) {
        borrowCoupons[borrowCouponsIdPerCollateral[_ids[j]]].owner = _to;
        lengthToModify++;
      }
    }
