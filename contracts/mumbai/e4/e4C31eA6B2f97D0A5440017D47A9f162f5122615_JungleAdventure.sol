...
contract JungleAdventure is VRFConsumerBaseV2 {
	...
	struct RunReturnData {
		uint256 gameData;
		address[3] rewardAddress;
		uint256[3] tokenId;
		uint256[3] amounts;
		uint256[3] tokenTypes;
	}
	...
	mapping(uint256 => MultiRunData) public multiRunData;
	...
	event ActivateManyCharm(uint256 teamId, uint256 charm, uint256 amount);
	...
	function activateCharm(uint256 _teamId, uint256 _charm, uint256 _amount) external {
		require(_teamId < teamCount[msg.sender], "JungleAdventure: Index does not exist");
		charmInUse[msg.sender][_teamId] = CharmEffect(_charm, 10000 * _amount + 1);
		charms.safeTransferFrom(msg.sender, BURN_ADDRESS, _charm, _amount, "");
		emit ActivateManyCharm(_teamId, _charm, _amount);
	}
	...
	function pendingEnergy(address _user) external view returns(uint256) {
		...
		uint256 extra;
		if (timerUser == 0)
			extra = weights[_user].mul(1e6);
		return weights[_user].mul(1e6).mul(time.sub(timerUser)).div(86400) + extra;
	}
	...
	function runAdventure(uint256 _teamId, bool _energy) external {
		...
		(uint256 gameData, 
		 ...
		 uint256[3] memory tokenTypes) = logic.run(_getActiveCharm(msg.sender, _teamId) | (_energy ? 128 : 0), _generateSeed(0), team, getLevelsInTeam(team));
	...
	// function runManyAdventuresVRF(uint256[] calldata _teamIds, bool _energy) external returns(uint256[] memory) {
	// 	require(_teamIds.length > 1);
	// 	uint256[] memory requestIds = new uint256[](_teamIds.length);
	// 	for(uint256 i = 0; i < _teamIds.length; i++)
	// 		requestIds[i] =  runAdventureVRF(_teamIds[i], _energy);
	// 	return requestIds;
	// }

	// function runManyAdventuresVRF(uint256 _teamId, bool _energy, uint256 _amount) external returns(uint256[] memory) {
	// 	require(_amount > 1);
	// 	uint256[] memory requestIds = new uint256[](_amount);
	// 	for(uint256 i = 0; i < _amount; i++)
	// 		requestIds[i] =  runAdventureVRF(_teamId, _energy);
	// 	return requestIds;
	// }

	function queryManyAdventuresVRF(uint256 _teamId, bool _energy, uint256 _amount) external returns(uint256) {
		require(useVrf, "Don't use VRF");
		require(!isPaused, "!paused");
		uint256 team = teams[msg.sender][_teamId];
		require(team > 0, "JungleAdventure: team not set");
		require(_amount > 1 && _amount < 26);

		_syncEnergy(msg.sender);
		if (_energy)
			_spendEnergy(msg.sender, _vxInTeam(team) * energyFee * _amount);
		else {
			uint256 fee = _vxInTeam(team).mul(nanaFee).mul(_amount);
			nanas.transferFrom(msg.sender, address(this), fee);
			seasonRewardBalance[season] = seasonRewardBalance[season].add(fee.div(2));
			nanas.transfer(owner(), fee.div(10));
			nanas.transfer(BURN_ADDRESS, fee.sub(fee.div(2)).sub(fee.div(10)));
		...
		multiRunData[requestId] = MultiRunData(false, msg.sender, _amount, _teamId, 0, _energy ? 0 : _amount);
		return requestId;
	}

	function settleAdventureRuns(uint256 _requestId) external {
		MultiRunData memory data = multiRunData[_requestId];
		address user = data.user;
		uint256 team = teams[user][data.teamId];
		require(user != address(0) && !data.fulfilled && data.seed > 0);

		multiRunData[_requestId].fulfilled = true;
		uint256 amount = data.amount;
		uint256 kongiumToMint;
		for (uint256 j = 0; j < amount; j++) {
			_consumeCharm(user, data.teamId, data.expGranted == 0);
			RunReturnData memory returnData;
			(returnData.gameData, 
			returnData.rewardAddress,
			returnData.tokenId,
			returnData.amounts,
			returnData.tokenTypes) = logic.run(_getActiveCharm(user, data.teamId) | (data.expGranted == 0 ? 128 : 0), _generateSeed(data.seed, j), team, getLevelsInTeam(team));
			uint256 expFlag = _increaseXp(team, (((returnData.gameData >> 32) & 0xffffffff) + data.expGranted > 0 ? 1 : 0) * (1 + ((returnData.gameData >> 224) & 1)));
			for (uint256 i = 0; i < 3; i++) {
				if (returnData.tokenTypes[i] == 1155) {
					IERC1155(returnData.rewardAddress[i]).safeTransferFrom(address(this), user, returnData.tokenId[i], returnData.amounts[i], "");
				}
				else if (returnData.tokenTypes[i] == 20) {
					IERC20(returnData.rewardAddress[i]).transfer(user, returnData.amounts[i]);
				}
			}
			kongiumToMint += returnData.gameData & 0xffffffff;
			emit AdventureDone(
				user,
				team,
				returnData.gameData |
				(expFlag << 64) |
				(data.expGranted > 0 ? 1 : 0 << 69) |
				(_encodeTeam(getLevelsInTeam(team)) << 70) |
				(_getActiveCharm(msg.sender, data.teamId) << 95),
				block.timestamp);
		}
		_mintKongium(user, kongiumToMint);
	}

	function runAdventureVRF(uint256 _teamId, bool _energy) public returns(uint256) {
		require(useVrf, "Don't use VRF");
		require(!isPaused, "!paused");
		uint256 team = teams[msg.sender][_teamId];
		require(team > 0, "JungleAdventure: team not set");
	...
	function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
		if (multiRunData[requestId].user != address(0)) {
			multiRunData[requestId].seed = randomWords[0];
			return;
		}
		...
		(uint256 gameData,
		 ...
		 uint256[3] memory tokenId,
		 uint256[3] memory amounts,
		 uint256[3] memory tokenTypes) = logic.run(((data >> 185) & 3) | (((data >> 187) ^ 1) << 7), _generateSeed(randomWords[0]), data & (2 ** 160 - 1), _decodeTeamLevels((data >> 160) & 33554431));
	...
	function _generateSeed(uint256 _seed, uint256 _salt) internal returns(uint256 rand) {
		rand = uint256(keccak256(abi.encodePacked(_seed, _salt)));
	}
	...
	function _syncEnergy(address _user) internal {
		...
		if (time > timerUser)
			energyCount[season][_user] += extra.add(weights[_user].mul(1e6).mul(time.sub(timerUser)).div(86400));
...
contract JungleLogic is IJungleLogic, Ownable {
	...
	function editEvent(uint256 _eventId, uint256 __chance) external onlyOwner {
		require(_eventId < eventCounter, "Outside of range");
	...
	function disableEvent(uint256 _eventId) external onlyOwner {
		require(_eventId < eventCounter, "Outside of range");
	...
	function charmDisableEvent(uint256 _eventId) external onlyOwner {
	...
	function charmEditEvent(uint256 _eventId, uint256 __chance) external onlyOwner {
		require(_eventId < charmEventCounter, "Outside of range");
	...
	function rodDisableEvent(uint256 _eventId) external onlyOwner {
	...
	function rodEditEvent(uint256 _eventId, uint256 __chance) external onlyOwner {
		require(_eventId < rodEventCounter, "Outside of range");
	...
	function rodEnableEvent(uint256 _eventId) external onlyOwner {
		require(_eventId < rodEventCounter, "Outside of range");
		GameEvent storage _event = rodGameEvents[_eventId];
