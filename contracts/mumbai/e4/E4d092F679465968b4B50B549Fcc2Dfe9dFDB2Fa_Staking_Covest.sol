	...
	ReentrancyGuardUpgradeable
{
	...
	/**
	 * @dev decode Constructor.
	 * @return The tuple value of ConstructorParams.
	 */
	function decodedConstructor(bytes memory _data)
		internal
		pure
		returns (ConstructorParams memory)
	{
		(
			uint256 _CAMaxStake,
			uint256 _UWMaxStake,
			uint256 _ReferMaxStake,
			uint256 _CPMaxStake,
			uint256 _CAUserMinStake,
			uint256 _UWUserMinStake,
			uint256 _ReferUserMinStake,
			uint256 _CPUserMinStake,
			uint256 _CAUserMaxStake,
			uint256 _UWUserMaxStake,
			uint256 _ReferUserMaxStake,
			uint256 _CPUserMaxStake
		) = abi.decode(
				_data,
				(
					uint256,
	 ...
	 */
	function checkRoleAccess(address _user, StakingTypes _type)
		public
		view
		returns (bool)
	{
	...
	{
		require(
			checkRoleAccess(msg.sender, _type) == true,
			'Your are not access'
		);
		require(
	...
	function payPenaltyBehalf(
		...
		address MITokenAddress = address(MI);
		require(checkRoleAccess(_user, _type) == true, 'User are not access');
		require(
			MI.allowance(msg.sender, address(this)) >= _amount,
			'Staking: You are not allowance.'
		);
