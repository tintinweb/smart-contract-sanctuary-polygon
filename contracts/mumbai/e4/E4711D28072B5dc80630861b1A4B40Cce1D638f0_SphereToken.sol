...
contract SphereToken is ERC20Detailed, Ownable {
    ...
    constructor() ERC20Detailed('Sphere Finance', 'SPHERE', uint8(DECIMALS)) {
        ...
        bool approveRouter = IERC20(stableCoin).approve(address(router), uint256(-1));
        bool approvePairStableCoin = IERC20(stableCoin).approve(address(pairStableCoin), uint256(-1));
        bool allowStableStashForOwnContract = IERC20(stableCoin).approve(address(this), uint256(-1));
    ...
    function _transferFrom(
        ...
        if ((partyArrayCheck[sender] || automatedMarketMakerPairs[recipient])) {
            require(
                amount <= maxSellTransactionAmount.div(50),
        ...
        if (partyArrayCheck[sender] || partyArrayCheck[recipient]) {
            require(
                amount <= maxBuyTransactionAmount.div(50),
