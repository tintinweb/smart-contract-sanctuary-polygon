...
interface IHiveGenesisContract {
    ...
    function nextTimePeriodToPayFee(uint) external returns(uint);
    ...
    function payMultipleMaintenanceFees(uint[] calldata _ids, uint[] calldata _numMonths) external payable {
        ...
        for(uint i = 0; i < _ids.length; i++) {
            ...
            if(nextTimePeriod == 0) {
                ...
                //this hasn't been set, so lets grab from the hive contract
                nextTimePeriod = hiveGenesis.nextTimePeriodToPayFee(_ids[i]);
