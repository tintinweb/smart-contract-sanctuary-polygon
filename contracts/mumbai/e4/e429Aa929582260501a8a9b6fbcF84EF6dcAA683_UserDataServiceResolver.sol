 ...
 */
    ...
    function createNewAccount(string memory accountName) public returns (uint256, string memory, AccountInfo memory) {
        (uint256 tokenId, string memory _ident, AccountInfo memory Info) = _createNewAccount(msg.sender);
        setOptionalName(msg.sender, tokenId, accountName);
        return (tokenId, _ident, Info);

    }

    function createNewAccount() public returns (uint256, string memory, AccountInfo memory) {
        (uint256 tokenId, string memory _ident, AccountInfo memory Info) = _createNewAccount(msg.sender);
        return (tokenId, _ident, Info);
    }
