...
contract CarvMemberships is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    // Used for subscribe with permit.
    struct Permit {
        address member;
        string uri;
        uint128 expiredTime;
    ...
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address member,string uri,uint128 expiredTime,bytes32 id)");
    // Mapping used permit ids, signer could use this id to avoid duplicate subscription.
    ...
    function initialize() initializer public {
        ...
        _HASHED_NAME = keccak256(bytes("CarvMemberships"));
    ...
    function subscribe(address to, string memory uri) public onlyRole(MINTER_ROLE) returns (uint256) {
    ...
    function subscribeWithPermit(Permit calldata _req, bytes calldata _signature) external returns (uint256) {
        address signer = recoverPermitSigner(_req, _signature);
        emit MembershipSubscribedWithPermit(signer, _req.member, 0);
        require(hasRole(MINTER_ROLE, signer), "missing minter role");
        require(!_permits[_req.id], "duplicated permit id");
        _permits[_req.id] = true;

        uint256 tokenId = _subscribe(_req.member, _req.uri);
        emit MembershipSubscribedWithPermit(signer, _req.member, tokenId);
        return tokenId;
    }
