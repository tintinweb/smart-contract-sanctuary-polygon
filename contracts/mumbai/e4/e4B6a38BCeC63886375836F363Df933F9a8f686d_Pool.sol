...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  /**
   * @dev Only risk pool admin can call functions marked by this modifier.
   **/
  ...
  function initialize(IPoolAddressesProvider provider) external virtual initializer {
    ...
    _reduxInterval = 1435 minutes; // 1 days - 5 mins
  }
  ...
  function recoverToken(
    ...
    ParticularLogic.executeRecoverToken(
      _reserves,
      DataTypes.ExecuteRecoverTokenParams({
        user: msg.sender,
        asset: token,
        amount: amount
      })
  ...
  function getSuppliers(
    ...
    uint256 start, 
    uint256 n
  ) external view override returns (address[] memory suppliers) {
    uint256 len = supplierList[asset].list.length - start;
    if(len > n) len = n;
    suppliers = new address[](len);
    for(uint256 i = 0; i < len; i++) {
      suppliers[i] = supplierList[asset].list[start + i];
    }
  }
    ...
    for(uint256 i = 0; i < _reservesCount; i++) {
      ...
      ParticularLogic.executeSplitReduxInterest(
        ...
        DataTypes.ExecuteSplitReduxInterestParams({
          asset: asset,
          daoWallet: ADDRESSES_PROVIDER.getDaoWallet(),
          oracle: ADDRESSES_PROVIDER.getPriceOracle(),
          reservesCount: _reservesCount,
  ...
  /// @inheritdoc IPool
  function setReserveInterestRates(address asset, uint256 borrowRate, uint256 prxyRate, uint256 prxyRate2) external virtual override onlyPoolConfigurator {
  ...
  /// @inheritdoc IPool
  function getPrxyInterestRate2(address supplyingAsset) 
    external view override 
    returns(uint256)
  {
    uint256 rate = _reserves[supplyingAsset].prxyRate2 * WadRayMath.RAY;
    rate /= SECONDS_PER_YEAR * GeneralLogic.RATE_PRECISION;
...
library ParticularLogic {
  ...
  /**
   * @notice Function to transfer the tokens to the admin wallet. The caller (admin)
   * @dev Emits the `RecoverToken()` event
   * @param reservesData The state of all the reserves
   * @param params The additional parameters needed to execute the RecoverToken function
   **/
  function executeRecoverToken(
    mapping(address => DataTypes.ReserveData) storage reservesData,
    DataTypes.ExecuteRecoverTokenParams memory params
  ) external {
    DataTypes.ReserveData storage reserve = reservesData[params.asset];
    IPToken(reserve.pTokenAddress).transferUnderlyingTo(params.user, params.amount);
    emit RecoverToken(params.asset, params.amount);
  }
   ...
   * @param params The additional parameters needed to execute the SplitReduxInterest function
   **/
  function executeSplitReduxInterest(
    ...
    DataTypes.ReserveData storage reserve = reservesData[params.asset];
    uint256 totalInterestBaseCurrency = 0;
    for(uint256 i = 0; i < params.reservesCount; i++) {
      address supplyingAsset = reservesList[i];
      DataTypes.ReserveData storage reserveX = reservesData[supplyingAsset];
      uint256 interestAmount = IPToken(reserveX.pTokenAddress).getReduxInterestOf(params.asset);
      totalInterestBaseCurrency += GeneralLogic.getUserBalanceInBaseCurrency(supplyingAsset, interestAmount, params.oracle);
    }

    uint256 totalInterestAmount = GeneralLogic.getBaseCurrencyInBalance(params.asset, totalInterestBaseCurrency, params.oracle);
    IPToken(reserve.pTokenAddress).reduxInterestTransfer();
    ...
    if(totalUncollateralAmount == 0) {
      ...
      uint256 realTotalReward = reserve.splitRate * totalInterestAmount / GeneralLogic.RATE_PRECISION;
      reserve.supplyRate = realTotalReward * MathUtils.SECONDS_PER_YEAR * GeneralLogic.RATE_PRECISION / (totalUncollateralAmount * params.elapsedTime);
    ...
    if(totalRewardAmount > 0) IERC20(params.asset).transfer(reserve.pTokenAddress, totalRewardAmount);
    uint256 daoAmount = totalInterestAmount - totalRewardAmount;
    if(daoAmount > 0) IERC20(params.asset).transfer(params.daoWallet, daoAmount);
    emit ReduxInterestSplit(params.asset, totalRewardAmount, daoAmount, totalUncollateralAmount);
  }
...
library LiquidationLogic {
  event LiquidationCall(
    ...
    uint256[] liquidatedBorrowedAmounts
  );
  ...
  function executeLiquidationCall(
    ...
    uint256[] memory borrowBalances = new uint256[](params.reservesCount);
    for(uint256 i = 0; i < params.reservesCount; i++) {
      borrowBalances[i] = IPToken(reserve.pTokenAddress).borrowBalanceOf(params.user, reservesList[i]);
    }
