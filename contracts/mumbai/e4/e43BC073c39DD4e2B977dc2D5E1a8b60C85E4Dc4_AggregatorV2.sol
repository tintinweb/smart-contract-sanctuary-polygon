...
contract AggregatorV2 is ReentrancyGuard, Ownable, EIP712 {
    ...
    event CallNftDexSwapFromETH(uint256 fromAmt, uint256 toMinAmt, address toToken, uint256 toAmt,address _nftDex, uint256 tokenId,address nftToken);
    event CallNftDexFromETH(address _nftDex, uint256 tokenId,address nftToken);
    ...
    event SwapTokenToNFT( uint256 fromAmount,  address fromToken, uint256 toTokenAmount, address toToken, address destination, address _dex,address _nftDex, uint256 tokenId,address nftToken);
    ...
    function swapToETHDexNft(SwapNftOrder memory order, bytes memory _sign) external nonReentrant
    {
        require(
    ...
    function swapTokenToTokenDexNft(SwapNftOrder memory order, bytes memory _sign) external nonReentrant {
        require(
            owner() == _hashTypedDataV4(_hashNftSwapOrder(order)).recover(_sign),
            "CALL_DATA_MUST_SIGNED_BY_OWNER"
        );
    ...
    /// @notice single chain - eth to token
    function swapETHToDexNft(SwapNftOrder memory order, bytes memory _sign) external payable nonReentrant
    {
        require(
            owner() == _hashTypedDataV4(_hashNftSwapOrder(order)).recover(_sign),
            "CALL_DATA_MUST_SIGNED_BY_OWNER"
...
//    function callNftDexSwapFromETH(CallNftDex memory order) external payable onlyOwner {
//        require(order._dex != address(0), "DEX_CAN_NOT_BE_0");
//        require(order.destination != address(0), "DESTINATION_CANNT_BE_0_ADDRESS");
//        require(order.toMinAmt > 0, "MIN_AMOUNT_MUST_BE_MORE_THAN_0");
//        require(order.fromAmt > 0, "VALUE_MUST_BE_MORE_THAN_0");
//        uint256 ethBalance = address(this).balance;
//        require(order.fromAmt <= ethBalance, "INSUFFIENT_BALANCE");
//        uint256 _toBalanceOrigin = IERC20(order.toToken).balanceOf(address(this));
//        (bool success, ) = order._dex.call{value: order.fromAmt}(order.data);
//        require(success, "DEX_SWAP_FAILED");
//        uint256 toTokenAmt = IERC20(order.toToken).balanceOf(address(this)).sub(_toBalanceOrigin);
//        require(toTokenAmt >= order.toMinAmt, "LESS_THAN_MIN_AMT");
//        require(toTokenAmt >= order.nftBaseAmount, "LESS_THAN_NFT_NEED_AMOUNT");
//        TransferHelper.safeApprove(order.toToken, order._nftDex, order.nftBaseAmount);
//        (bool result, ) = order._nftDex.call(order.nftData);
//        require(result, "NFT_BUY_FAILED");
//        if (order.destination != address(this)) {
//            IERC721(order.nftToken).transferFrom(
//                address(this),
...
//            );
//        }
//        emit CallNftDexSwapFromETH( order.fromAmt, order.toMinAmt, order.toToken,toTokenAmt,order._nftDex,order.tokenId,order.nftToken);
//    }
//    function callNftDexFromETH(CallNftDex memory order) external payable onlyOwner {
//        require(order.destination != address(0), "DESTINATION_CANNT_BE_0_ADDRESS");
//        (bool result, ) = order._nftDex.call{value:order.nftBaseAmount}(order.nftData);
//        require(result, "NFT_BUY_FAILED");
//        if (order.destination != address(this)) {
//            IERC721(order.nftToken).transferFrom(
//                address(this),
...
//            );
//        }
//        emit CallNftDexFromETH( order._nftDex,order.tokenId,order.nftToken);
//    }
