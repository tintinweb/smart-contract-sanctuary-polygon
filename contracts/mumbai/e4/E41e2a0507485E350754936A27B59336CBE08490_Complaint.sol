...
contract Complaint {
    ...
    struct complaint {
        ...
        string title;
        string description;
        bytes approvalRemark;
        bytes resolutionRemark;
        bool isApproved;
        bool isResolved;
        bool exists;
    }
    ...
    function approveComplaint(uint256 _id, bytes memory _approvalRemark)
        public
        onlyOfficer
    {
        require(
        ...
        onlyOfficer
    {
      ...
      Complaints[_id].approvalRemark = bytes.concat(
    bytes("This complaint is rejected. Reason: "),
    bytes(_approvalRemark)
);
...
Complaints[_id].resolutionRemark = bytes(_resolutionRemark);
    }
