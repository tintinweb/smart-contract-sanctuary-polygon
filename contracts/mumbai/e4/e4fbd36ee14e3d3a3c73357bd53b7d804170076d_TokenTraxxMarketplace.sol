    ...
    ErrorCodes
{
    ...
    function removeListing(uint256 listingId) external onlyAdmin {
        ...
        returns (uint256 listingCount, uint256 activeCount)
    {
        ...
        for (uint256 i = 0; i < listingIdTracker; i++) {
            Listing memory currentListing = listings[i];
            if (
                currentListing.assetContract == _assetContract &&
                currentListing.tokenId == _tokenId
            ) {
                listingCount++;
        ...
        returns (Listing[] memory activeListings)
    {
        ...
        // 20,000 ->  =====> 30 NFTs x 20,000
        // 70,000 -> Totallistings
        // mapping(assetContract(ERC721 [active] | ERC1155) -> tokenId [10] ) => [ 70,001 ] _tokenListingMaps
        // totalTokenListings = _tokenListingMaps[_assetContract][_tokenId] => [ 70,001 ].length
        // Total quantity of active listing;
        //  for (uint256 i = 0; i < listingIdTracker; i++) {
        //     Listing memory currentListing = listings[i];
        //     if (
        //         currentListing.assetContract == _assetContract &&
        ...
        // for(uint256 index=0; index<= totalTokenListings.length; index++ ) {
        //      Listing currentlisting = listings[totalTokenListings[index]];
        //      currentlisting.quantity > 0 active activeListings[index] = currentlisting;
        // }
