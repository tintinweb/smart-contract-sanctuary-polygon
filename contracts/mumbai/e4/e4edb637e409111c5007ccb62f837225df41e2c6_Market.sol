    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @return auctions (Auction[] memory) Auction IDs
     * @return nextOffset (uint256) Next offset index
     */
    function getAuctions(uint256 offset_, uint256 length_)
        external
        view
        returns (AmberfiLib.Auction[] memory auctions, uint256 nextOffset)
    {
        uint256 auctionLength = getAuctionsCount();
        ...
        auctions = new AmberfiLib.Auction[](length_);

        for (uint256 i; i < length_; ++i) {
            auctions[i] = _auctions[offset_ + i];
        }
