...
import "./FactoringContractTickets.sol";
...
uint256 constant BASIS_POINT_DENOMINATOR = 100 * 100;
...
contract FactoringContract is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    using {basisPointMultiply} for uint256;
    ...
    uint256 public initialAuditorBalance;
    uint256 public finalAuditorBalance;
    uint256 internal _auditReserve;  // amount the contract is holding to pay for audits.

    FactoringContractParams public p;
    FactoringContractTickets public t;
    ...
    modifier onlyInStates4(State[4] memory allowed) {
     ...
     modifier onlyInStates5(State[5] memory allowed) {
        for (uint256 i = 0; i < allowed.length; i++) {
            if (_currentState == allowed[i]) {
                _;
                return;
    ...
    function initialize(address params, address tickets)
        initializer
        whenNotPaused
        external
    {
        p = FactoringContractParams(params);
        t = FactoringContractTickets(tickets);
        p.attach();
        t.attach(params);
   ...
   /* ------------------------------
    ...
    /// @notice Exchanges the currency token for an NFT representing the acquired portion of the contract. Can be called multiple times to acquire multiple NFTs, but there is no particular reason to do this.
    /// @param amountToSpend The amount of currency to spend buying part of this income stream.
    function buyTicket(uint256 amountToSpend)
        external
        whenNotPaused
        advancesState
        onlyInState(State.Fundraising)
    {
        if (amountToSpend < p.minimumPurchase() ||
            amountToSpend > p.maximumFundraisingGoal() - totalSold) {
            revert RequestedPurchaseOutOfRange(p.minimumPurchase(), p.maximumFundraisingGoal() - totalSold);
        }
        ...
        uint256 auditReserveTarget = p.initialAuditPrice() + p.finalAuditPrice();
        ...
        sellerBalance += amountToSpend - forAuditReserve;
        emit TicketBought(msg.sender, t.createTicket(msg.sender, amountToSpend), amountToSpend);
    }
    ...
    {
        CollectionRound storage currentRound = _getAssuredActiveRound();
        if (block.timestamp < currentRound.startTs + p.timeToInfoDefaultInSec()) {
            revert AuditBlockedAssertedTooEarly(currentRound.startTs + p.timeToInfoDefaultInSec());
        }
    ...
    /// @param amountAssessed The full value of the relevant income stream for the relevant time period, denominated in the currency used by this contract. NOTE that this number should NOT account for the portion of that income stream sold or any fees or penalties. (i.e., if the seller offered 10% of their income for $1000, but only sold $500 of tickets, the auditor should still report the full income number.)
    ...
    /// @param amountAssessed The full value of the relevant income stream for the relevant time period, denominated in the currency used by this contract. NOTE that this number should NOT account for the portion of that income stream sold or any fees or penalties. (i.e., if the seller offered 10% of their income for $1000, but only sold $500 of tickets, the auditor should still report the full income.)
    ...
    /// @notice A function for the seller to submit payment for the current collection round. The amount paid will be determined by the most reccent audit submitted. The amount owed can be determined from getPaymentsOwed() and must be approved before calling this.
        ...
        onlyInStates4([State.AwaitingInitialCollection, State.AwaitingFinalCollection, State.DefaultInitialPayment, State.DefaultFinalPayment])
    {
        ...
        uint256 treasuryShare = baseOwed.basisPointMultiply(p.collectionFeeBasisPoints());
        ...
        require(_currencyContract.transferFrom(p.sellerAddress(), address(this), baseOwed + feesOwed + initialAuditPriceOwed
            + finalAuditPriceOwed));
        require(_currencyContract.transfer(p.treasuryAddress(), treasuryShare));
        initialAuditorBalance += initialAuditPriceOwed;
        finalAuditorBalance += finalAuditPriceOwed;
    }
   ...
   /* ------------------------------
            ...
            State.DefaultFinalPayment, State.Complete])
    {
        ...
        for (uint256 i = 0; i < t.balanceOf(msg.sender); i++) {
            uint256 tokenId = t.tokenOfOwnerByIndex(msg.sender, i);
            (uint256 amountInvested, uint256 roundsClaimed) = t.tickets(tokenId);
            uint256 totalBalanceForTicket = 0;
            for (uint256 j = roundsClaimed; j < _collectionRounds.length; j++) {
                ...
                totalBalanceForTicket += (amountInvested * round.proceedsForBuyers) / totalSold;  // TODO: consider phantom overflow here.
            }
            totalBalance += totalBalanceForTicket;
            emit TicketRedeemed(msg.sender, tokenId, totalBalanceForTicket, roundsCompleted);
            t.updateRoundsClaimed(tokenId, roundsCompleted);
        }
        if (totalBalance == 0) { revert NoBalance(); }
        require(_currencyContract.transfer(msg.sender, totalBalance));
    }
        ...
        onlyInState(State.FundraisingFailed)
    {
        ...
        while (t.balanceOf(msg.sender) > 0) {
            uint256 tokenId = t.tokenOfOwnerByIndex(msg.sender, 0);
            (uint256 bought, ) = t.tickets(tokenId);
            totalBought += bought;
            t.burn(tokenId);
        }
        if (totalBought == 0) { revert NoBalance(); }
    ...
    function withdrawFundraisingProceeds()
        ...
        onlyInStates9([State.NoActiveCollection, State.DefaultInfo, State.AwaitingInitialAudit,
            ...
            State.DefaultFinalPayment, State.Complete])
    {
        if (sellerBalance == 0) { revert NoBalance(); }
        uint256 treasuryShare = totalSold.basisPointMultiply(p.fundraiseFeeBasisPoints());
        ...
        require(_currencyContract.transfer(p.treasuryAddress(), treasuryShare));
        require(_currencyContract.transfer(p.sellerAddress(), sellerShare));
    }

    /// @notice A function for the initial auditor to withdraw any payment.
    function withdrawInitialAuditPayments()
        external
        whenNotPaused
        onlyRole(INITIAL_AUDITOR_ROLE)
    {
        uint256 forAuditor = initialAuditorBalance;
        initialAuditorBalance = 0;
        require(_currencyContract.transfer(p.initialAuditorAddress(), forAuditor));
    }

    /// @notice A function for the final auditor to withdraw any payment.
    function withdrawFinalAuditPayments()
        external
        whenNotPaused
        onlyRole(FINAL_AUDITOR_ROLE)
    {
        uint256 forAuditor = finalAuditorBalance;
        finalAuditorBalance = 0;
        require(_currencyContract.transfer(p.finalAuditorAddress(), forAuditor));
    }
    ...
    {
        if (_auditReserve == 0) { revert NoBalance(); }
        uint256 forSeller = _auditReserve;
        _auditReserve = 0;
        require(_currencyContract.transfer(p.sellerAddress(), forSeller));
    }
    ...
    function withdrawMistakenCurrency(address mistakenCurrencyContractAddress)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        if (mistakenCurrencyContractAddress == p.currencyAddress()) { revert NotMistakenCurrency(); }
        IERC20 mistaken = IERC20(mistakenCurrencyContractAddress);
        require(mistaken.transfer(p.treasuryAddress(), mistaken.balanceOf(address(this))));
    }
        ...
        returns (uint256)
    {
        ...
        if (_collectionRounds[_collectionRounds.length - 1].paymentTs != 0) {
            return _collectionRounds.length;
        }
        return _collectionRounds.length - 1;
    }
        ...
        returns (uint256)
    {
        ...
        if (_collectionRounds.length >= p.numTargetCollectionRounds() && totalCollected >= scaledFloor()) {
            return _collectionRounds.length;
        }
        ...
        returns (State)
    {
       // These are states that cannot be exited due to the passage of time.
        if (_currentState == State.AwaitingInitialAudit ||
            _currentState == State.DefaultInfo ||
            _currentState == State.DefaultInitialPayment ||
            _currentState == State.DefaultFinalPayment ||
            _currentState == State.Dispute ||
            _currentState == State.FundraisingFailed ||
        ...
        uint256 numCompleted = collectionRoundsCompleted();
        if (totalRoundsSoFar() != numCompleted) {  // at least one required round is not complete.
            if (numCompleted == _collectionRounds.length) {
                // the struct for the active round has not been created yet.
                return State.AwaitingInitialAudit;
            }
            return _stateFromActiveRound(_collectionRounds[_collectionRounds.length - 1]);
        }
    ...
    function _stateFromActiveRound(CollectionRound memory round)
        internal
        view
        returns (State)
    {
    ...
    function _getAssuredActiveRound()
        internal
        returns (CollectionRound storage)
    {
        if (_collectionRounds.length == 0 || _collectionRounds[_collectionRounds.length - 1].paymentTs != 0) {
            // the struct for the active round has not been created yet, so we create it here.
            _collectionRounds.push(CollectionRound({
                startTs: p.firstCollectionTs() + p.collectionCadenceInSec() * _collectionRounds.length,
        ...
        return _collectionRounds[collectionRoundsCompleted()];
    }
   ...
   /* ------------------------------------
    ...
    function getPaymentsOwed()
        external
        view
        onlyInStates5([State.AwaitingInitialCollection, State.DefaultInitialPayment, State.Dispute,
            State.AwaitingFinalCollection, State.DefaultFinalPayment])
        ...
        returns (uint256)
    {
        ...
        baseAmount = baseAmount.basisPointMultiply(p.basisPointsOffered());
        // If collection is only continuing because of a need to meet the floor, treat the floor as a cap.
        if (_collectionRounds.length > p.numTargetCollectionRounds() && scaledFloor() - totalCollected < baseAmount) {
            baseAmount = scaledFloor() - totalCollected;
        // If a cap is specified, don't exceed it.
        } else if (p.proposedCollectionCap() > 0 && scaledCap() - totalCollected < baseAmount) {
        ...
        returns (uint256)
    {
        uint256 baseAmount = _scaleBySoldPercentage(round.finalAuditTs == 0 ? round.initialAuditAmount : round.finalAuditAmount);
        baseAmount = baseAmount.basisPointMultiply(p.basisPointsOffered());
        return baseAmount.basisPointMultiply(_feeBasisPointsForRound(round));
    }
        ...
        returns (uint256 forInitial, uint256 forFinal)
    {
        ...
        bool floorMet = totalCollected + baseOwed >= _scaleBySoldPercentage(p.proposedCollectionFloor());
        bool capExistsAndMet = p.proposedCollectionCap() > 0 && totalCollected + baseOwed >= scaledCap();
        if (floorMet && (capExistsAndMet || collectionRoundsCompleted() + 1 >= p.numTargetCollectionRounds())) {
            return (0, 0);  // if final round, no need to pay for next round's audit.
        }

        forFinal = round.finalAuditTs != 0 ? p.finalAuditPrice() : 0;
    ...
    function _feeBasisPointsForRound(CollectionRound storage round)
        internal
        view
        returns (uint256)
    {
    ...
    function scaledFloor()
        ...
        returns (uint256)
    {
        return _scaleBySoldPercentage(p.proposedCollectionFloor());
    }
    ...
    function scaledCap()
        ...
        returns (uint256)
    {
        return _scaleBySoldPercentage(p.proposedCollectionCap());
    }

    /// @notice The basis points of the income stream associated with this user by their held tickets.
    function basisPointsOfIncomeStreamHeld()
        public
        view
        returns (uint256)
    {
        return (t.totalValueHeld(msg.sender) * p.basisPointsOffered()) / p.maximumFundraisingGoal();
    }
    ...
    function _scaleBySoldPercentage(uint256 toScale)
        ...
        returns (uint256)
    {
        return (toScale * totalSold) / p.maximumFundraisingGoal();
    }
...
struct FactoringContractParamsInput {
    ...
    uint16 basisPointsOffered;  // the number of basis points of the income flow that were offered for sale.
    ...
    string informationalSellerName;
    string informationalIncomeFlowDescription;
    string informationalCurrencySymbol;
}
...
contract FactoringContractParams {
    ...
    function informationalSellerName() public view returns (string memory) { return _p.informationalSellerName; }
    function informationalIncomeFlowDescription() public view returns (string memory) { return _p.informationalIncomeFlowDescription; }
    function basisPointsOffered() public view returns (uint16) { return _p.basisPointsOffered; }
    function informationalCurrencySymbol() public view returns (string memory) { return _p.informationalCurrencySymbol; }
}
...
contract FactoringContractTickets is ERC721, ERC721Enumerable, AccessControl, Pausable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    bytes32 public constant FC_ROLE = keccak256("FC_ROLE");
    ...
    error BadTokenId();
    error AlreadyAttached(address attachedTo);
    error ParamsAddressMismatch(address callerParams, address ticketParams);
    error DisallowedTransferDestination(address disallowed);

    // Stop people from making possibly common errors of transferring the ticket to either the
    // ticket address or the main contract address.
    modifier allowedTransferDestination(address to) {
        if (to == address(this) ||
            to == _attachedTo) {
            revert DisallowedTransferDestination(to);
        }
        _;
    }

    constructor(string memory name, string memory symbol, address params)
        ERC721(name, symbol)
    {
        _p = FactoringContractParams(params);
        _grantRole(DEFAULT_ADMIN_ROLE, _p.adminAddress());
    }

    /// @notice Used in setup only.
    /// @dev Called from the FactoringContract to establish a 1-1 correspondence.
    function attach(address callerParams)
        external
    {
        if (_attachedTo != address(0)) {
            revert AlreadyAttached(_attachedTo);
        }
        if (callerParams != address(_p)) {
            revert ParamsAddressMismatch(callerParams, address(_p));
        }
        _attachedTo = msg.sender;
        _grantRole(FC_ROLE, msg.sender);
    }

    /// @dev Reentrancy here should not hurt anything, but we use nonReentrant out of an abundance of caution.
    function createTicket(address to, uint256 value)
        external
        onlyRole(FC_ROLE)
        nonReentrant
        whenNotPaused
    ...
    {
        _tokenIds.increment();
        tickets[_tokenIds.current()] = ClaimTicket({amountInvested: value, roundsClaimed: 0});
        _safeMint(to, _tokenIds.current());
        return _tokenIds.current();
    }

    function updateRoundsClaimed(uint256 tokenId, uint256 roundsClaimed)
        external
        onlyRole(FC_ROLE)
        whenNotPaused
    {
        tickets[tokenId].roundsClaimed = roundsClaimed;
    }

    function totalValueHeld(address addr)
        external
        view
        returns (uint256)
    {
        uint256 total = 0;
        uint256 numTickets = balanceOf(addr);
        for (uint256 i = 0; i < numTickets; i++) {
            total += tickets[tokenOfOwnerByIndex(addr, i)].amountInvested;
        }
        return total;
    }
        ...
        returns (string memory)
    {
        ...
        ClaimTicket memory t = tickets[tokenId];
        string memory name = string(abi.encodePacked('ISA Redemption Ticket #', tokenId.toString()));
        string memory description = string(abi.encodePacked(
            _p.informationalSellerName(), ' is selling ',
            uint256(_p.basisPointsOffered()).toString(), ' basis points of "', _p.informationalIncomeFlowDescription(),
            '" for ', _p.maximumFundraisingGoal().toString(), ' ', _p.informationalCurrencySymbol(),
            '.\nThis ticket was purchased for ', t.amountInvested.toString(), ' ', _p.informationalCurrencySymbol(),
