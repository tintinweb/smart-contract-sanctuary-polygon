...
contract NFTMARKETPLACE {
    ...
    struct userVerification{
        ...
        string  email;
        uint256 idCard;
        address[] verificationAgainstUser;
        bool isUserExist;

    }
    mapping(uint256 => nft) public nftData;
    mapping (address=>userVerification) public verification;
    // mapping (uint256=>userVerification) public verificationOnID;
    ...
    function setUserRecord(string memory _name,string memory _email,uint256 _number,uint256 _idCard) public {
     if(verification[msg.sender].isUserExist==false){
        userIdCount ++;
        verification[msg.sender].userId=userIdCount;
         verification[msg.sender].Name=_name;
          verification[msg.sender].email=_email;
          verification[msg.sender].phoneNumber=_number;
          verification[msg.sender].idCard=_idCard;
           verification[msg.sender].verificationAgainstUser.push(msg.sender);
            verification[msg.sender].isUserExist=true;

        }
    }

         function getUserAllAddress(address _address) public view returns(address[] memory){
       address[] memory data = new address[](verification[_address].verificationAgainstUser.length);
      for (uint i = 0; i < verification[_address].verificationAgainstUser.length; i++) {
            data[i] = verification[_address].verificationAgainstUser[i];
        }
        return data;
   }

     function checkUserId(address _address) public view returns(uint256 _id){
         bool success;
      for (uint i = 0; i < verification[_address].verificationAgainstUser.length; i++) {
            if(verification[_address].verificationAgainstUser[i]==_address){
                success=true;
            }
            if(success==true){
              return   verification[_address].userId;
            }
      }
       
   }
   function resetUserDetail(address _user,bool _erase) public {
          verification[_user].isUserExist=_erase;
   }
    ...
    function bidWithOtherCurrency(uint256 _auctionIndex, uint256 _newBid)
        external
        returns (bool)
    {
        require(_auctionIndex < auctionData.length, "Invalid auction index");
     ...
     function bidWithNative(uint256 _auctionIndex)
        external
        payable
        returns (bool)
    {
        ...
        // IERC20 paymentToken = IERC20(auction.addressErc20Token);
        require(isOpen(_auctionIndex), "Auction is not open");
        require(msg.value > auction.currentBid,"New bid price must greater than last bid price");
        // require( msg.sender != auction.creator, "Creator of the auction cannot place new bid" );
        // require(block.timestamp < auction.endAuction,"Auction is closed");
    ...
    function finalizeAuction(uint256 _auctionIndex) external {
        require(auctionData[_auctionIndex].isClaimed==false,"already finalized");
        require(_auctionIndex < auctionData.length, "Invalid auction index");
        ...
        _nftToken.transferFrom(
         ...
         auctionData[_auctionIndex].isClaimed=true;
