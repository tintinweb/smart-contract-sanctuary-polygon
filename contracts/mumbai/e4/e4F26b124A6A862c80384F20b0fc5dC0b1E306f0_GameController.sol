...
import "./interfaces/IGameSupport.sol";
...
contract GameController is OwnableUpgradeable, IGameController, IStruct {
    ...
    function _reset(uint256 id) internal {
        gameSupport.reset(id);
        delete countAddedNewResource[id];
        for (uint256 i = 0; i < statMapResources[id].length(); ++i) {
            (, uint256 value) = statMapResources[id].at(i);
            statMapResources[id].remove(value);
        }
    }
    ...
    function updateMap(
        ...
        (int256 payout, uint256 prop) = IGameResource(baseLogic.gameResource())
    ...
    function setGameSupport(address _support) public onlyOwner{
        gameSupport = IGameSupport(_support);
    }
...
contract GameSupport is OwnableUpgradeable, IGameSupport {
    ...
    mapping(uint256 => mapping(uint8 => int256)) private rarityUpdate;
    mapping(uint256 => IStruct.InfoNextRoll) private info;
    ...
    function executeRemoveRes(
        uint256 id,
        uint256 randomNumber
    ) external override onlyGameController returns (uint256 coin) {
    ...
    function executeItem(
        ...
        uint256 randomNumber
    ) external override onlyGameController returns (int256 coin) {
    ...
    function initInfo(uint256 id) external override onlyGameController {
        // IGame.RequestResult memory landRequest = IGame(gameContract)
        //     .getLandRequest(id);
        info[id] = IStruct.InfoNextRoll(1, 1, 0, 0, 0, 0);
    }

    function updateNumShop(
        uint256 id,
        uint8 sort, // 0 : item shop, 1 - res shop, 2 - shop2Res
        uint256 value,
        bool increase
    ) external override onlyLogicContract {
        if (sort == 0) {
            info[id].numItem = increase
                ? info[id].numItem + value
                : info[id].numItem - value;
        } else if (sort == 1) {
            info[id].numRes = increase
                ? info[id].numRes + value
                : info[id].numRes - value;
        } else if (sort == 2) {
            info[id].shop2Res = increase
                ? info[id].shop2Res + value
                : info[id].shop2Res - value;
        }
    }

    function skipNextShop(
        uint256 id,
        bool skip // must 0 = tự do, 1 = phải chọn 1 resource, 2 = phải skip resource
    ) external override onlyLogicContract {
        if (skip) {
            info[id].skipResShop = 2;
        } else {
            info[id].skipResShop = 1;
        }
    }

    function updateNextOption(
        uint256 id,
        uint256 option, //option = 0: bình thường, option = 1: rare + veryRare, 2: veryRare, 3 : uncommon and better
        uint256 ratio
    ) external override onlyLogicContract {
        if (info[id].option < option) {
            info[id].option = option;
            info[id].ratio = ratio;
        }
        // TODO compare ration
    }
    ...
    function reset(uint256 id) external override {
        for (uint256 i = 0; i < destroyedPosition[id].length(); ++i) {
            destroyedPosition[id].remove(destroyedPosition[id].at(i));
        }
        for (uint256 i = 0; i < removedPosition[id].length(); ++i) {
            removedPosition[id].remove(removedPosition[id].at(i));
        }
    }

    function _resetAll(uint256 id) internal {}
...
interface IGameSupport {
    ...
    function initInfo(uint256 id) external;
    ...
    function skipNextShop(uint256 id, bool skip) external;

    function updateNextOption(uint256 id, uint256 option,uint256 ratio) external;
...
contract DestroyLogic is Initializable, OwnableUpgradeable, IDestroy {
    ...
    function destroy(
        ...
        for (uint i = 0; i < destroyPos.length; ++i) {
            ...
            if (resource == uint256(Resources.ID.B1)) {
                ...
                t5_destroy(id, destroyPos[i],destroyPos, randomNumber);
    ...
    function t2_destroy(uint256 id, uint256 position) internal {
        ...
        IGameSupport(baseLogic.gameSupport()).updateNextOption(id, 0, 100);
    }
    ...
    function t5_destroy(
        ...
        uint256 rand = ((random / 1e37) % 10000) % destroyPos.length;
...
contract RemoveLogic is Initializable, OwnableUpgradeable, IRemove {
    ...
    function remove(
        ...
        for (uint i = 0; i < removePos.length; ++i) {
            ...
            _removeResource(id, removePos[i], uint256(resource));
        }
    }
    ...
    function _removeResource(
     ...
     // check any resource have insert res then remove
    }
