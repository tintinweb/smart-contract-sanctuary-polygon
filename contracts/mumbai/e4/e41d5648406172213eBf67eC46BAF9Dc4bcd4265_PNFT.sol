...
contract AccountRegistryUpgradeable is Initializable {
    ...
    ERC721Upgradeable
{
    ...
    function burn(uint256 tokenId) public virtual {
        if (!_isApprovedOrOwner(msg.sender, tokenId))
            revert ERC721BurnableUpgradeable__NotOwnerNorApproved();
        _burn(tokenId);
    }
...
abstract contract NonFungibleType {
    ...
    function _setSold(uint256 typeNFT_, uint256 quantity_) internal {
        ...
        if (_sold[typeNFT_] > _typeInfo[typeNFT_].limit)
            revert NonFungibleType__ExceedLimit(typeNFT_);
    }
    ...
    function _setType(
        ...
        _typeInfo[type_] = TypeInfo(paymentToken_, price_, limit_, operation_);
        emit SetTypeNFT(type_, paymentToken_, price_, limit_, operation_);
    }
...
import { NonFungibleType } from "./internal-upgradeable/NonFungibleType.sol";
    ...
    ERC721WithPermitUpgradable
{
    ...
    event Registered(address recipient, uint256 tokenId, uint256 quantity);
    ...
    function initialize(
        ...
        __AccountRegistryUpgradeable_init_unchained(registry_, implementation_);
        _setType(1, address(0), 10 ether, 100, operation_);
        _setType(2, address(0), 5 ether, 100, operation_);
        _setType(3, address(0), 1 ether, 100, operation_);
        _setType(4, address(0), 3 ether, 100, operation_);
    ...
    function setType(
        ...
        _setType(type_, paymentToken_, price_, limit_, operation_);
    }

    function buy(uint256 typeNFT_, uint256 quantity_) external payable nonReentrant {
        ...
        address sender = _msgSender();
        {
            ...
            if (typeInfo.paymentToken == address(0)) {
                ...
                if (refund > 0.1 ether) {
                    ...
                    CurrencyTransferLib.safeTransferNativeToken(owner(), total);
                } else {
                    CurrencyTransferLib.safeTransferNativeToken(owner(), value);
                }
            } else {
        ...
        emit Registered(sender, tokenId, quantity_);
    ...
    function getTypeNFT(
        uint256 typeNFT
    ) external view returns (address, uint256, uint256, Operation memory) {
        return (
    ...
    function _batchProcess(
        ...
        address tba = registry.account(implementation, chainId, tokenCollection, tokenId_, 0);
    ...
    function supportsInterface(
        ...
        public
        view
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC721WithPermitUpgradable)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }

    function _authorizeUpgrade(address implement_) internal virtual override onlyOwner {}
}
