...
contract SecureBank is ERC721, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721("SecureBank", "SBT") {}
    ...
    function withdraw(uint256 amount) public nonReentrant {
        ...
        if (_deposits[msg.sender] == 0 && _exists(_userTokenId[msg.sender])) {
    ...
    function depositBalanceOf(address account) public view returns (uint256) {
    ...
    function transferFrom(
        ...
        require(_deposits[from] != 0, "No deposit to transfer");
        require(!_exists(_userTokenId[to]), "Recipient already has a deposit");

        _deposits[to] = _deposits[from];
        _deposits[from] = 0;
        _userTokenId[to] = tokenId;
        delete _userTokenId[from];
