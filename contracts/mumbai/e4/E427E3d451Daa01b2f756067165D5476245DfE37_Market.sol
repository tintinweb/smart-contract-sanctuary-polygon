...
contract Market is Context, ERC721Holder, Ownable {
    ...
    struct ReserveAuction {
        ...
        bool auctionEnd;
        address[] allowList;
        mapping(address => bool) mapAllowList;
    }

    uint32 private bidIncreasePercentage = 5;
    ...
    constructor(IERC20 tokenAddr, IERC721 nftAddr) {
    ...
    function createPrivateBidList(
        ...
        for (uint256 i = 0; i < allowList.length; i++) {
            act.mapAllowList[allowList[i]] = true;
        }

        emit PrivateBidCreated(_auctionID, _msgSender(), nftId, reservePrice);
    }

    function setBidIncreasePercentage(uint32 amount) public onlyOwner {
        bidIncreasePercentage = amount;
    }

    function setAuctionBidPeriod(uint32 amount) public onlyOwner {
        auctionBidPeriod = amount;
    }

    function setNFTAddr(address nftAddr) public {
        nft = IERC721(nftAddr);
    }
    ...
    function _checkValueLength(uint256 val, int256 minus)
        internal
        pure
        returns (int256)
    {
        if (val < 1 ether) {
            ...
            bytes memory valByte = new bytes(val);
            return int256(valByte.length) - minus;
        }
    }
    ...
    function _multiplyAndPow(
        ...
        return multiply * (1**uint256(_checkValueLength(val, minus)));
    }

    function getMinBid(uint256 _auctionID) public view returns (uint256) {
        ReserveAuction storage act = auctionIdToReserveAuction[_auctionID];
        // uint256 increaseBid = (act.amount / bidIncreasePercentage) * 100;
        if (act.amount >= _multiplyAndPow(act.amount, 5, -1)) {
            return _multiplyAndPow(act.amount, 0.5 ether, -2);
        } else if (
            act.amount >= _multiplyAndPow(act.amount, 1 ether, -1) &&
            act.amount < _multiplyAndPow(act.amount, 2 ether, -1)
        ) {
            return _multiplyAndPow(act.amount, 1, -3);
        } else {
            return _multiplyAndPow(act.amount, 2.5 ether, -3);
        }
        
        // return act.amount + increaseBid;
    }
    ...
    modifier bidRequirements(uint256 _auctionID, uint256 _tokenAmount) {
        ...
        require(
            _tokenAmount < minBid,
            string(abi.encodePacked("require minimum bid volume : ", minBid))
        );
        require(
    ...
    function _isWhitelistedSale(uint256 _auctionID)
        ...
        returns (bool)
    {
        return auctionIdToReserveAuction[_auctionID].mapAllowList[msg.sender];
    }
    ...
    function endBid(uint256 _auctionID) public isAuctionEnd(_auctionID) {
        ReserveAuction storage act = auctionIdToReserveAuction[_auctionID];
        if (act.auctionEnd == false && act.bidder != address(0)) {
