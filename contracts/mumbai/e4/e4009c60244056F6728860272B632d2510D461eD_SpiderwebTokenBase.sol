...
/// @notice This contract is only for Spiderweb.AI's BASE chain = Polygon. It locks tokens on source, on outgoing send(), and unlocks tokens when receiving from other chains.
contract SpiderwebTokenBase is OFTV2, ERC20Capped {
    constructor(string memory _name, string memory _symbol, uint _initialSupply, uint _cap, uint8 _sharedDecimals, address _lzEndpoint) OFTV2(_name, _symbol, _sharedDecimals, _lzEndpoint) ERC20Capped(_cap) {
