...
contract DoodlePalNFTv2Staking is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    using EnumerableSet for EnumerableSet.UintSet; 
    
    //addresses 
    IERC721 public stakingNftToken;
    ...
    modifier requireRewardToken() {
        require(address(rewardToken) != address(0), "rewardToken not set");
        _;
    }

    modifier requireStakingNftToken() {
        require(address(stakingNftToken) != address(0), "stakingNftToken not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setRewardTokenAddress(address rewardTokenAddr) public onlyOwner() {
      rewardToken = Erc20Token(rewardTokenAddr);
    }

    function setStakingNftTokenAddress(address stakingNftAddr) public onlyOwner() {
      stakingNftToken = IERC721(stakingNftAddr);
    }
      ...
      returns (uint256[] memory)
    {
      ...
      uint256[] memory depositSetOtherContract = rewardToken.depositsOf(account);
    ...
    {
      uint256[] memory tokensInOldContract = rewardToken.depositsOf(account);
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        uint256 lastBlock = arrayContains(tokensInOldContract, tokenId) ? 
          rewardToken._depositBlocks(account, tokenId) 
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused requireRewardToken {
      uint256 reward; 
      uint256 blockCur = Math.min(block.number, expiration);
      uint256[] memory tokensInOldContract = rewardToken.depositsOf(msg.sender);
      ...
      rewardToken.claimRewards(tokensInOldContract);
    ...
    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireStakingNftToken requireRewardToken {
        require(msg.sender != address(rewardToken), "Invalid address");
        claimRewards(tokenIds);
        for (uint256 i; i < tokenIds.length; i++) {
            stakingNftToken.safeTransferFrom(
                msg.sender,
                address(this),
                tokenIds[i],
                ""
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireStakingNftToken requireRewardToken nonReentrant() {
        claimRewards(tokenIds);
    ...
    function withdrawTokens() external onlyOwner {
        uint256 tokenSupply = rewardToken.balanceOf(address(this));
        rewardToken.transfer(msg.sender, tokenSupply);
    }
