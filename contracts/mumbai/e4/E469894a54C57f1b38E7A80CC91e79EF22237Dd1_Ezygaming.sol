...
contract Ezygaming is ERC721Enumerable, Ownable, Pausable {
    ...
    constructor(string memory notReveal) ERC721("Ezygaming", "EZY") {
        notRevealURI = notReveal;
    }
    ...
    function changeNotRevealURI(string memory URI) public onlyOwner {
        notRevealURI = URI;
    }
    ...
    function tokenURI(uint256 tokenId) public view  override returns (string memory)
    {
        ...
        if(lastNFTBuy + revealTime >= block.timestamp){
            return notRevealURI;
        }else {
