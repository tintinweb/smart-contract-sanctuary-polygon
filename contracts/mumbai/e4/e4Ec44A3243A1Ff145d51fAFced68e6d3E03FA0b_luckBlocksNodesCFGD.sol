...
contract luckBlocksNodesCFGD is Ownable,ReentrancyGuard {
  ...
  function resetQueue(uint lottery, uint weekly) external {
      ...
      if(weekly == 1){
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 900,"autoSpin parameters not met");
        ...
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 600,"autoSpin parameters not met");
        //send the waiting nodes to the active nodes array and delist the active stucked nodes
        uint[] memory _activenodes = activenodes;
        uint[] memory _waitingnodes = waitingnodes;
   ...
   function updateNodeInfo(address caller, uint256 reward, uint weekly,uint lottery) external returns(bool success) {
      ...
      if(weekly == 1){
       ...
       require(block.timestamp > lbcontract.autoSpinTimestamp() + 300,"autoSpin parameters not met");
