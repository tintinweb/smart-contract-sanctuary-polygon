...
contract DuelStake is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 totalAllocation; // Total allocation for the pool (total seconds * tokenPerSecond)
    ...
    // withdrawal fee (100 for 1%)
    uint256 public withdrawalFee = 1000;

    // entrance fee
    uint256 public entranceFee = 50;
    ...
    function deposit(uint256 _pid, uint256 _amount) external {
        ...
        if(pay_entrance) {
            if(_amount>entranceFee) {
                pool.token.safeTransferFrom(address(msg.sender),address(feeRecipient),entranceFee*(10**18));
                pool.token.safeTransferFrom(address(msg.sender), address(this), _amount.sub(entranceFee*(10**18)));
                user.amount = user.amount.add(_amount.sub(entranceFee*(10**18)));
                pool.supply = pool.supply.add(_amount.sub(entranceFee*(10**18)));
            }
        } else {
    ...
    /**
     * @dev Withdraw Tokens from DuelStaking.
     */
    function withdraw(uint256 _pid, uint256 _amount) external {
        ...
        if (pending > 0) {
            ...
            {
                fee = withdrawalFee;
                feeAmount = calculateFee(fee, pending);
            }
            if (feeAmount > 0) {
                pool.token.safeTransfer(address(feeRecipient), feeAmount);
            }
            safeTokenTransfer(msg.sender, pending.sub(feeAmount));
        }
    ...
    /**
     * @dev Returns base reward per second.
     */
