...
interface ILooxooryNft{
    ...
    function getNFT(uint256 _tokenId) external view returns(string memory uri,uint256 price,address creatorAddress,uint256 campaignId);
}
...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    event Minted(uint256 tokenId,string uri,uint256 price,address creatorAddress,uint256 campaignId);
    ...
    function createMarketSale(NFTVoucher calldata voucher,bool donation) public payable{
        ...
        (string memory uri,uint256 price,address creatorAddress,uint256 campaignId) = ILooxooryNft(NFTContract).getNFT(tokenId);

        emit Minted(tokenId,uri,price,creatorAddress,campaignId);
    }
