...
contract Exchange is ExchangeDomain, Initializable, ERC2771ContextUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, FeeComp, RoyaltyComp, BidComp {
    event CreateAndSell(address seller, address token, uint256 tokenId, uint256 tomenAmount, uint256 price);
    ...
    function sell(SellRequest calldata sellReq) public {
        ...
        sellinfos[_msgSender()][sellReq.asset.token][sellReq.asset.tokenId] = SellInfo(sellReq.asset.tokenAmount, sellReq.currency, sellReq.price);
    ...
    // 创建并售卖NFT
    function createAndSell(CreateAndSellRequest calldata createAndSellReq) public {
        if (createAndSellReq.asset.assetType == LibAsset.AssetType.ERC721) {
            ...
            sellinfos[_msgSender()][createAndSellReq.asset.token][createAndSellReq.asset.tokenId] = SellInfo(1,  createAndSellReq.currency, createAndSellReq.price);
        }
            ...
            sellinfos[_msgSender()][createAndSellReq.asset.token][createAndSellReq.asset.tokenId] = SellInfo(createAndSellReq.asset.tokenAmount, createAndSellReq.currency, createAndSellReq.price);
        }

        emit CreateAndSell(_msgSender(), createAndSellReq.asset.token, createAndSellReq.asset.tokenId, createAndSellReq.asset.tokenAmount, createAndSellReq.price);
    }
    ...
    function bid(BidRequest calldata bidReq) 
        public
        payable
    {
        BidInfo storage bidInfo = bidinfos[bidReq.seller][bidReq.asset.token][bidReq.asset.tokenId][bidReq.asset.tokenAmount];

        require(block.timestamp < bidInfo.expirationTime, "Exchange: The time for bids expired");
        require(bidReq.price > bidInfo.price, "Exchange: Bid price is too low");

        _bid(_msgSender(), bidInfo.currency, bidReq.price);

        if(bidInfo.lastBidder != address(0)) {
            _returnBid(bidInfo.lastBidder, bidInfo.currency, bidInfo.price);
        }

        bidInfo.price = bidReq.price;
        bidInfo.lastBidder = _msgSender();
    }
    ...
    function _swap(BuyRequest calldata buyReq) internal {
        ...
        require((sellInfo.price <= msg.value),"Exchange: match price faild");

        delete sellinfos[buyReq.seller][buyReq.asset.token][buyReq.asset.tokenId];
        ...
        uint256 feeAmount = _payFee(_msgSender(), sellInfo.currency, msg.value);
        uint256 royaltyAmount = _payRoyalty(buyReq.asset.token, buyReq.asset.tokenId, _msgSender(), sellInfo.currency, msg.value);
        uint256 remainingAmount = msg.value - feeAmount - royaltyAmount;
        ...
        if(sellInfo.currency == address(0)){
            ...
            require(erc20.transferFrom(_msgSender(), address(this), royaltyAmount), "BidComp: Bid with ERC20 token failed");
        }
...
contract ExchangeDomain {
    ...
    mapping (address => mapping (address => mapping (uint256 => mapping (uint256 => BidInfo)))) public bidinfos;
}
...
contract FeeComp is Initializable {
    ...
    function _payFee(address buyer, address currency, uint256 salePrice) internal returns(uint256) {
        ...
        if (currency == address(0)){
            ...
            require(erc20.transferFrom(buyer, address(this), feeAmount), "BidComp: Bid with ERC20 token failed");
        }
...
contract RoyaltyComp {
    function _payRoyalty(address token, uint256 _tokenId, address buyer, address currency, uint256 _salePrice) internal returns(uint256){
        ...
        if (currency == address(0)){
            ...
            IERC20Upgradeable erc20 =  IERC20Upgradeable(currency);
            require(erc20.transferFrom(buyer, receiver, royaltyAmount), "BidComp: Bid with ERC20 token failed");
        }
