...
interface IRequest {
    ...
    function acceptRequest(address _requestAddress) external payable;
...
contract Fee is ControlNode{
    ...
    function setFee(uint _price) onlyRole(STAFF_ROLE) internal{
...
contract Artscape721 is ERC721, ERC721URIStorage, AccessControl, RoyaltiesV2Impl  {
...
contract Request is ReentrancyGuard,Initializable{
    ...
    constructor(address payable _owner, address payable _buyer, address payable _market, uint _price, uint _tokenId, address _tokenAddress)initializer{
        require (Artscape721(_tokenAddress).ownerOf(_tokenId) != _market, "Token is already up on sale");
        require(Artscape721(_tokenAddress).ownerOf(_tokenId) != msg.sender, "You are owner");
    ...
    function getDetail() public view returns(address, address, address,address, uint, uint, bool){
        return (owners, buyer, market, tokenAddress, tokenId, price, isComplete);
    }
    ...
    function rejected() external nonReentrant{
        require((msg.sender == owners || msg.sender == market), "Only Owner or Market");
        isComplete = true;
    }
}
...
contract Offer is ReentrancyGuard, EscowVault, Fee{
    using Counters for Counters.Counter;
    ...
    function createSellItems(uint _tokenId, address _tokenAddress, uint _price) nonReentrant public returns(uint) {
    //     //to resell owner need to setApprovalForAll to marketplaceAddress
        ...
        require (activeSell[_tokenAddress][_tokenId]==false, "AlreadyOnSale");
    ...
    //     //assert(Itemss[newTokenId].itemId == newTokenId);
    //     emit MarketTransaction(1, msg.sender, _tokenId, newTokenId, _tokenAddress, address(0), _price, false);
        return newTokenId;
    }
    //  function reTokentoMarket(address _tokenAddress, uint _tokenId, uint _price) TokenExist(_tokenAddress,_tokenId) nonReentrant external {
    //     require (_price > 0, "Price must be higher than 1 Wei");
    //     require (Artscape721(_tokenAddress).ownerOf(_tokenId) == msg.sender, "Must be owner");
    //     Items memory old_Items =  tokenIdToItems[_tokenAddress][_tokenId];
    //     Items memory _Items = Items({
    //         seller: payable(msg.sender),
    //         price: _price,
    //         isSold: false,
    //         tokenId: old_Items.tokenId,
    //         tokenAddress: old_Items.tokenAddress,
    //         itemId: old_Items.itemId,
    //         owners: payable(address(0)),
    //         isAuction: false
    //     }
    //         );
    //     Artscape721(_tokenAddress).transferFrom(msg.sender, address(this), _tokenId);
    //     tokenIdToItems[_tokenAddress][_tokenId]=_Items;
    //     activeTokens[_tokenAddress][_tokenId]=true;
    //     emit MarketTransaction(3, msg.sender, _tokenId, _Items.itemId, _tokenAddress, address(0), _price, false);
    // }
    function _changePrice(uint _tokenId, address _tokenAddress, uint _nprice) public virtual{
...
contract AuctionV1 is Fee{
    address payable public beneficial;
    ...
    constructor(uint _biddingTime, address payable _beneficial, uint _start, uint _tokenId, address _tokenAddress) {
        beneficial = _beneficial;
        highestBid = _start;
        tokenid = _tokenId;
        tokenAddress = _tokenAddress;
        auctionEndTime = block.timestamp + _biddingTime;
        owner_address = getWallet();
        marketFee = getFee();
        emit AuctionDetail(auctionEndTime, _beneficial, _start, _tokenId, _tokenAddress);
    }
    function bid() external payable{
        require(block.timestamp < auctionEndTime,"AlreadyEnded");
        require(msg.value > highestBid,"There is already higher bid or equal bid");
    ...
    function withdrawal() public returns (bool){
        ...
        require(amount > 0, "The amount must be more than 0");
    ...
    function auctionTimeNow() public view returns(uint){
        return block.timestamp;
    }
    function withdrawal(address _address) public returns (bool){
    ...
    function auctionEnd() external{
        ...
        require(!ended,"The fuction auctionEnded has already been called");
        _auctionEnd();
    }
    function checkReturn() public view returns(uint){
        return pendingReturns[msg.sender];
    }
    function _auctionEnd() internal{
        ended = true;
        emit AuctionsEnd(highestBidder, highestBid);
        uint percent_price = (highestBid*marketFee*100) / 10000;
        uint last_price = highestBid-percent_price;
        (bool sent, ) = beneficial.call{value: last_price}("");
        require(sent, "Failed send");
        emit AuctionSend(beneficial, last_price);
        (bool sent2, ) = owner_address.call{value: percent_price}("");
        require(sent2, "Failed send");
        emit AuctionSend(owner_address, percent_price);
...
contract MarketAuction is ReentrancyGuard, Offer{
    ...
    function setAuctionV1(uint _biddingTime, uint _tokenId, address _tokenAddress, uint start_price) public{
        require (Artscape721(_tokenAddress).ownerOf(_tokenId) == msg.sender, "MustBowner");
        require (activeSell[_tokenAddress][_tokenId]==false, "AlreadyOnSale");
        require (activeAuction[_tokenAddress][_tokenId]==false, "AlreadyOnAuction");
        setStatusAuction(_tokenId,_tokenAddress,start_price);
        AuctionV1 auction = new AuctionV1(_biddingTime, payable(Artscape721(_tokenAddress).ownerOf(_tokenId)),start_price, _tokenId, _tokenAddress);
    ...
    function bid(address _tokenAddress, uint _tokenId ) public payable {
        require(nft2Auction[_tokenAddress][_tokenId]!= address(0x0), "Not on Auction");
        AuctionV1(nft2Auction[_tokenAddress][_tokenId]).bid{value: msg.value}(msg.sender);
    ...
    function end(address _tokenAddress, uint _tokenId, address _receiver) public{
        require(activeSell[_tokenAddress][_tokenId] == false,"!available");
        require(_receiver != address(0x0), "Receiver must not be address zero");
        AuctionV1(nft2Auction[_tokenAddress][_tokenId]).auctionEnd();
        _sendToken(_tokenAddress,_tokenId,_receiver);
        super.transferEscowOnSell(_tokenAddress,_tokenId);
        super.withdrawalAuction(_tokenAddress,_tokenId);
        nft2Auction[_tokenAddress][_tokenId] = address(0);
        emit AuctionTransaction(3, _tokenId, _tokenAddress, nft2Auction[_tokenAddress][_tokenId], AuctionV1(nft2Auction[_tokenAddress][_tokenId]).getHighestBid(),AuctionV1(nft2Auction[_tokenAddress][_tokenId]).getHighestBidder(), AuctionV1(nft2Auction[_tokenAddress][_tokenId]).getEndTime(), true);
    }
    function _sendToken(address _tokenAddress, uint _tokenId, address _receiver) internal{
        Artscape721(nft2Auction[_tokenAddress][_tokenId]).transferFrom(address(this), _receiver,_tokenId);
    }
    function getHighestBid(address _tokenAddress, uint _tokenId) public view returns(uint highestbid) {
        ...
        return AuctionV1(nft2Auction[_tokenAddress][_tokenId]).highestBid();
    }
    function getHighestBidder(address _tokenAddress, uint _tokenId) public view returns(address highestbidder) {
        require(nft2Auction[_tokenAddress][_tokenId] != address(0x0), "!Auction");
        return AuctionV1(nft2Auction[_tokenAddress][_tokenId]).highestBidder();
    }
...
contract VoucherV1 is ReentrancyGuard, Initializable{
    ...
    modifier isOwner(){
        require(msg.sender == beneficial, "You are not the owner");
        _;
    }
    modifier isReceiver(){
        require(msg.sender == receiver || msg.sender == market , "You are not allow");
        _;
    }
    event voucherEvent(string TxType, address indexed owner, address receiver,address market, address indexed tokenAddress, uint indexed tokenId, uint price);
    event TransactionTransfer(string TxType, address to, uint value, uint indexed itemId,address indexed tokenAddress);

    constructor(address payable _beneficial, address payable _receiver, address payable _market, uint _tokenId, address _tokenAddress, uint _price) initializer {
        beneficial = _beneficial;
        receiver = _receiver;
        market = _market;
        tokenId = _tokenId;
        ...
        emit voucherEvent("Create", beneficial, receiver, market, tokenAddress, tokenId, price);
    }
    function withdrawal() isReceiver() external{
        require(Artscape721(tokenAddress).ownerOf(tokenId) == address(this), "Token is not here");
        require(isWithdraw == false, "already withdrawal");
        
        Artscape721(tokenAddress).transferFrom(address(this), receiver, tokenId);
        isWithdraw = true;
        emit voucherEvent("Withdrawal", beneficial, receiver, market, tokenAddress, tokenId, price);
    }
    ...
    function _getRoyaltyData() internal view returns (address artist, uint percent){
        LibPart.Part[] memory royalty = Artscape721(tokenAddress).getRaribleV2Royalties(tokenId);
        address _address;
        uint _percent;
        if(royalty.length != 0){
            _address = royalty[0].account;
            _percent = royalty[0].value;
        }
...
contract MarketRequest is IRequest, EscowVault,Initializable{
    ...
    function createRequest(uint _tokenId, address _tokenAddress, uint _price) override external returns(address){
        require(Artscape721(_tokenAddress).ownerOf(_tokenId) !=  tx.origin, "must!owner");
        if(_price < 0){revert();}
        if(activeSell[_tokenAddress][_tokenId]){revert("AOS");}
        Request _reqcontract= new Request(payable(Artscape721(_tokenAddress).ownerOf(_tokenId)), payable( tx.origin), payable(address(this)), _price, _tokenId, _tokenAddress);
        address2Request[_tokenAddress][_tokenId][tx.origin] = address(_reqcontract);
        nft2Request[_tokenAddress][_tokenId].push(address(_reqcontract));
        emit RequestTx(0, Artscape721(_tokenAddress).ownerOf(_tokenId), tx.origin, address(_reqcontract), address(0), _tokenId, _tokenAddress, _price, false);
        _reqIds.increment();
        return address(_reqcontract);
    }
   function acceptRequest(address _requestAddress) external payable override{
        Request _reqcontract = Request(_requestAddress);
        require( tx.origin == _reqcontract.getOwner(), "!token_owner");
        VoucherV1 vouchers = new VoucherV1(payable(_reqcontract.getOwner()),payable(_reqcontract.getBuyer()),payable(address(this)), _reqcontract.getTokenId(),_reqcontract.getTokenAddress(), _reqcontract.getPrice());
        tokenId2Accept[_reqcontract.getTokenAddress()][_reqcontract.getTokenId()] = address(vouchers);
        Artscape721(_reqcontract.getTokenAddress()).transferFrom(tx.origin, address(vouchers), _reqcontract.getTokenId());
    ...
    function withdrawVoucher(address _voucherAddress) external payable{
        VoucherV1 vouchers = VoucherV1(_voucherAddress);
        require(tx.origin == vouchers.getBuyer(), "NotAllow");
        require(msg.value == vouchers.getPrice(),"!price");
        vouchers.withdrawal();
    }
...
contract ArtscapeMarket is IMarketplace, Offer{
    ...
    // mapping(address => mapping(uint=> address)) public tokenId2Request;
    // mapping(address => mapping(uint=> address)) public tokenId2Accept;
    mapping(address=>mapping(uint=> bool))public activeTokens;
    mapping(address => mapping(uint=> address)) public tokenId2Auction;
    // mapping(address => mapping(uint=> Bid[])) public tokenId2Bid;
    // mapping(address => mapping(uint=> address[])) public AuctionParticipant;
    // mapping(address => uint)public pendingReturns;
    ...
    modifier isAuction(address _tokenAddress,uint _tokenId){
        require(super.escowAuctionCheck(_tokenAddress, _tokenId), "!onAuction");
        _;
    }
    modifier isHuman() {
    ...
    returns ( address seller, address owners, uint price, uint price_fee, uint index, uint tokenId, address tokenAddress, bool isSold){
        ...
        uint fee = offer.price*listFee/100;
    ...
    function addTokentoMarket(uint _tokenId, address _tokenAddress, uint _price) nonReentrant override external {
        uint newTokenId = createSellItems(_tokenId,_tokenAddress,_price);
        emit MarketTransaction(1, msg.sender, _tokenId, newTokenId, _tokenAddress, address(0), _price, false);
    }
    ...
    function _buyArt(uint _tokenId, address _tokenAddress,  uint _value, uint price, address seller) internal{
        ...
        uint royalty_price;
        uint price_send; 
        uint fee = price*listFee/100;
        (_royalty, _percent) = _getRoyaltyData(_tokenId,_tokenAddress);               
        royalty_price = ((_value-fee)*100) / 10000;
    ...
    function getprice(uint _tokenId, address _tokenAddress)public view returns(uint){
        return nft2Item[_tokenAddress][_tokenId].price + (nft2Item[_tokenAddress][_tokenId].price*listFee/100);
    }
    //TODO: TokenExist(_tokenAddress,_tokenId)
...
contract WrapperArtscape is ArtscapeMarket, MarketAuction{
    ...
    constructor(
        ...
        market_request = new MarketRequest();
        market_request.initialize();
    }
    function create_request(uint _tokenId, address _tokenAddress, uint _price) public returns(address){
        return market_request.createRequest(_tokenId, _tokenAddress, _price);
    }
    function accept_request(address _requestAddress) public payable{
        market_request.acceptRequest(_requestAddress);
    }
    function reject_request(address _requestAddress) public{
        market_request.rejectedRequest(_requestAddress);
    }
    function redeem_voucher(address _voucher) public payable{
        market_request.withdrawVoucher(_voucher);
    }
    function request_detail(address _tokenAddress, uint _tokenId, uint _index) public view returns(address){
        return market_request.get_request(_tokenId, _tokenAddress, _index);
    }
}