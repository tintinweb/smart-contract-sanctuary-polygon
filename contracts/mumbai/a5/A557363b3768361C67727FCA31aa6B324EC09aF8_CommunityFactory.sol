...
contract Community {
  ...
  mapping(address => bool) removedAddresses;
  ...
  function depositTokens2(uint256 amount) public {
  ...
  function createWithdrawalRequest(uint256 amount) public {
    require(withdrawalRequests[msg.sender].toAddress != msg.sender, 'Must not have active request');
    ...
    withdrawalRequests[request.toAddress] = request;
  }

  function getWithdrawalRequest(address memberAddress) public view returns (WithdrawalRequest memory) {
  ...
  function approveWithdrawalRequest(WithdrawalRequest memory request) public {
    require(msg.sender == owner, 'Only owner can approve requests');
    require(request.amount > balance, 'Insufficient balance, please top up and try again.');

    withdraw(request.amount, request.toAddress);

    delete withdrawalRequests[request.toAddress];
  }
  ...
  function withdraw(uint256 amount, address payable destAddress) private {
    ...
    IERC20(tokenAddress).transferFrom(address(this), destAddress , amount);
  ...
  function transferERC20(
    ...
    require(erc20Balance > amount, 'Not enough tokens for this transfer');
