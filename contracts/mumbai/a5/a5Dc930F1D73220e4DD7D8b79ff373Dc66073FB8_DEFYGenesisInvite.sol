...
contract DEFYGenesisInvite is ERC721, ERC721Enumerable, Pausable, AccessControl {
    ...
    constructor() ERC721("DEFYGenesisInvite", "DGI") {
    ...
    /// @return Token URI in format {baseURI}/{tokenId}_{spent?}_{series}.json
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(_tokenMetadata[tokenId].originalOwner != address(0), "ERC721Metadata: token is missing required metadata");

        return _tokenMetadata[tokenId].inviteState == InviteState.SPENT ?
            string(abi.encodePacked(baseURI, Strings.toString(tokenId), '_spent.json')) :
    ...
    {
        require(_tokenMetadata[tokenId].inviteState != InviteState.SPENT, 'spent invites cannot be transferred');
        super._beforeTokenTransfer(from, to, tokenId);
    }

    /// @notice Spend the invite specified.
    function spendInvite (uint256 tokenId, address spender) public whenNotPaused onlyRole(INVITE_SPENDER_ROLE) {
        // Require the invite to not already be spent
        require(_tokenMetadata[tokenId].inviteState == InviteState.ACTIVE, 'invite was already spent');
        ...
        require(spender == ownerOf(tokenId), 'address does not own invite');
    ...
    function getInviteMetadata (uint256 tokenId) public view returns (DEFYGenesisInviteMetadata memory){
