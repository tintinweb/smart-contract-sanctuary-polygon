...
contract Keeper48 is AutomationCompatibleInterface, Ownable {
    ...
    function checkUpkeep(
        ...
        return (
            k > 0,
            abi.encode(token0, token1, block.timestamp, lm, oracles)
        );
    }
    ...
    function getAmountOracle(
        ...
        uint256 timestamp,
        uint256 roundIndex
    ) external view returns (int256 amount, uint256 lastRoundIndex) {
        uint256 l = roundIndex;
        // uint256 latestRoundId = oracle.latestRound();
        uint256 h = oracle.latestRound(); // Not n - 1
        ...
        while (l < h) {
            uint256 mid = l + (h - l) / 2;
            uint256 t = oracle.getTimestamp(mid);
        ...
        amount = oracle.getAnswer(l) * 1e10;
        lastRoundIndex = l;
    }
}
