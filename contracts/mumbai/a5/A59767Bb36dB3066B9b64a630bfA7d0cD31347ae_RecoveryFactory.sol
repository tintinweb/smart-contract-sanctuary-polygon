...
contract RecoveryFactory is IRecoveryFactory, YousovRoles {
    ...
    constructor(address _ezr, address _agentPayrollWallet, address _userFactory, address _yousovAccessControl){
        ezr = _ezr;
    ...
    function createRecovery() external override {
        require(!userHasRecovery(msg.sender) && (YousovAccessControl(yousovAccessControl).hasRole(TEMPORARY_YOUSOV_USER_ROLE, msg.sender) ||YousovAccessControl(yousovAccessControl).hasRole(YOUSOV_USER_ROLE, msg.sender)), "YOUSOV: Not authorized to this action" );
        // Pay the recovery
        TransferHelper.safeTransfer(ezr, agentPayrollWallet, 5 * 10**16 );        
        
        EZR(ezr).burn(3 * 10**16);       
        
        TransferHelper.safeTransfer(ezr, 0xD44d5088f6eb0a0Aed56B2065478737030acEA94, 2 * 10**16 );
        
        // //  create the recovery
        address _newRecovery = address(new Recovery(msg.sender, address(this), yousovAccessControl));
    ...
    function getLegalSelectableAgents(address _currentRecovery) external  override {
        require(recoveryExist(_currentRecovery) && tx.origin == IRecovery(_currentRecovery).user() && IRecovery(_currentRecovery).getRecoveryStatus() !=  RecoveryStatus.OVER ,"YOUSOV : Recovery don't exist");
        ...
        emit LegalAgentsToNotify(_currentRecovery,IRecovery(_currentRecovery).contenderAgentsList());
    }
