...
/// @title The N00uns DAO auction house
...
// LICENSE
// N00unsAuctionHouse.sol is a modified version of Zora's AuctionHouse.sol:
// https://github.com/ourzora/auction-house/blob/54a12ec1a6cf562e49f0a4917990474b11350a2d/contracts/AuctionHouse.sol
//
// AuctionHouse.sol source code Copyright Zora licensed under the GPL-3.0 license.
// With modifications by N00unders DAO.
...
contract N00unsAuctionHouse is
    IN00unsAuctionHouse,
    PausableUpgradeable,
    ReentrancyGuardUpgradeable,
    OwnableUpgradeable
{
    // The N00uns ERC721 token contract
    IN00unsToken public n00uns;
 ...
 IERC20 public biddingToken;
    ...
    /**
     * @notice Settle the current auction, mint a new N00un, and put it up for auction.
     */
    ...
    function createBid2(uint256 n00unId) external payable  nonReentrant {
        ...
        emit AuctionBid(_auction.n00unId, msg.sender, msg.value, extended);
    ...
    /**
     * @notice Unpause the N00uns auction house.
     * @dev This function can only be called by the owner when the
    ...
    function _createAuction() internal {
        try n00uns.mint() returns (uint256 n00unId) {
            ...
            emit AuctionCreated(n00unId, startTime, endTime);
    ...
    function _settleAuction() internal {
        ...
        if (_auction.bidder == address(0)) {
            n00uns.burn(_auction.n00unId);
        } else {
            n00uns.transferFrom(address(this), _auction.bidder, _auction.n00unId);
        }
...
/// @title Interface for N00un Auction Houses
...
interface IN00unsAuctionHouse {
    struct Auction {
        // ID for the N00un (ERC721 token ID)
        uint256 n00unId;
    ...
    event AuctionCreated(uint256 indexed n00unId, uint256 startTime, uint256 endTime);

    event AuctionBid(uint256 indexed n00unId, address sender, uint256 value, bool extended);

    event AuctionExtended(uint256 indexed n00unId, uint256 endTime);

    event AuctionSettled(uint256 indexed n00unId, address winner, uint256 amount);
...
/// @title Common interface for N00unsDescriptor versions, as used by N00unsToken and N00unsSeeder.
...
interface IN00unsDescriptorMinimal {
    ///
    /// USED BY TOKEN
    ///

    function tokenURI(uint256 tokenId, IN00unsSeeder.Seed memory seed) external view returns (string memory);

    function dataURI(uint256 tokenId, IN00unsSeeder.Seed memory seed) external view returns (string memory);
