...
contract Blog is Ownable {
  ...
  function updateTitle(uint256 _id, string memory _title) public {
    _idToPost[_id].title = _title;
    _afterPostUpdate(_id);
  }

  function updateContentHash(uint256 _id, string memory _contentHash) public {
    _idToPost[_id].contentHash = _contentHash;
    _afterPostUpdate(_id);
  }

  function updateImageHash(uint256 _id, string memory _imageHash) public {
    _idToPost[_id].imageHash = _imageHash;
    _afterPostUpdate(_id);
  }

  function updateIsPublished(uint256 _id, bool _isPublished) public {
    _idToPost[_id].isPublished = _isPublished;
    _afterPostUpdate(_id);
  }

  function banPost(uint256 _id, bool _isBanned) public onlyOwner {
    require(_isPostBanned[_id] != _isBanned, "Post is already banned/not banned");
    if (_isBanned) {
  ...
  function _afterPostUpdate(uint256 _id) internal {
