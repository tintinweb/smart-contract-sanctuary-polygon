...
import { IAirdropOnPurchaseHook } from "./IAirdropOnPurchaseHook.sol";

/// @title AirdropOnPurchaseHook
/// @notice Hook contract that enables setting token airdrops on each purchase on a lock
/// @author Coinvise
contract AirdropOnPurchaseHook is ILockKeyPurchaseHook, IAirdropOnPurchaseHook {
    ...
    function setAirdrop(
        ...
        if (address(airdropToken) == address(0) || airdropAmount == 0) revert InvalidAirdrop();
        if (address(airdrops[address(_lock)].airdropToken) != address(0)) revert AirdropExists();
    ...
    function onKeyPurchase(
        ...
        if (address(airdrop.airdropToken) != address(0) && airdrop.airdropAmount != 0) {
