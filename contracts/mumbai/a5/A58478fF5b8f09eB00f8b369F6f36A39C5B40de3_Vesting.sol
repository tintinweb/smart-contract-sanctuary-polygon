...
contract Vesting {
    ...
    uint256 constant INITIAL_UNLOCK = 10; // 10% : released percent at TGE stage
    uint256 constant UNLOCK_UNIT = 30; // 30% of the total allocation will be unlocked
    uint256 constant CLIFF_PERIOD = 2 hours; // cliff period
    uint256 constant INITIAL_LOCK_PERIOD = CLIFF_PERIOD + 1 hours; // No tokens will be unlocked for the first 2 hours + 1 hours
    uint256 constant vestingPeriod = 21 hours; // rest vesting period
    ...
    constructor(address _SBCToken, address _presaleContract, address payable _multiSigAdmin) {
        ...
        SBCToken.approve(presaleContract, MAX_UINT256);
    ...
    function getVested(address beneficiary) public view virtual returns (uint256 _amountVested) {
        ...
        if (_vestingSchedule.totalAmount == 0) {
            ...
            return (_vestingSchedule.totalAmount).mul(INITIAL_UNLOCK).div(100);
        }
        ...
        if (block.timestamp > firstVestingPoint && block.timestamp <= secondVestingPoint) {
            vestedPercent = INITIAL_UNLOCK.add(UNLOCK_UNIT) + (block.timestamp - firstVestingPoint).mul(90-UNLOCK_UNIT).div(vestingPeriod);
