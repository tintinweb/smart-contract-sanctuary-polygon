...
library Constants {
    ...
    uint16 constant MAX_DESCRIPTION_LENGTH = 280;
    uint8 constant MAX_BIO_LENGTH = 30;
}
...
library Errors {
    ...
    error OnlyAdmins();
    error MaxLength(uint16 _maxLength);
}
...
library Events {
    ...
    event AvatarChanged(uint256 id, string avatar);
    event DescriptionChanged(uint256, string description);
    event BioChanged(uint256 id, string key, string value);
}
    ...
    ERC721EnumerableUpgradeable
{
    ...
    mapping(address => bool) private _adminPermissions;

    mapping(bytes32 => mapping(string => string)) bios;
    ...
    modifier onlyAdmins(address _caller, bool _whitelistEnabled) {
        if (_whitelistEnabled) {
            if (!_adminPermissions[_caller]) revert Errors.OnlyAdmins();
            _;
        } else {
            _;
        }
    ...
    function createProfile(
    ...
    ) external onlyAdmins(msg.sender, whitelistEnabled) {
        ...
        bytes32 usernameHash = _usernameToHash(args.username);
    ...
    function changeAvatar(string memory _newAvatar) external {
        DataTypes.ProfileStruct memory _profile = getProfileByAddress(
            msg.sender
        );
        ...
        _profileById[_profile.tokenId].imageURI = _newAvatar;

        emit Events.AvatarChanged(_profile.tokenId, _newAvatar);
    }

    function setDescription(string memory _newDescription) external {
        DataTypes.ProfileStruct memory _profile = getProfileByAddress(
            msg.sender
        );

        if (bytes(_newDescription).length > Constants.MAX_DESCRIPTION_LENGTH)
            revert Errors.MaxLength(Constants.MAX_DESCRIPTION_LENGTH);

        _profileById[_profile.tokenId].description = _newDescription;

        emit Events.DescriptionChanged(_profile.tokenId, _newDescription);
    }

    function setBio(DataTypes.ProfileBioStruct memory _newBio) external {

        if (bytes(_newBio.value).length > Constants.MAX_BIO_LENGTH)
            revert Errors.MaxLength(Constants.MAX_BIO_LENGTH);

        bytes32 usernameHash = _usernameToHash(_newBio.username);

        bios[usernameHash][_newBio.key] = _newBio.value;

        emit Events.BioChanged(_profileIdByUsernameHash[usernameHash], _newBio.key, _newBio.value);
    }
    ...
    /*------------------------------------------------------Operations methods-------------------------------------------------------*/
    function _usernameToHash(
        string memory _username
    ) internal pure returns (bytes32) {
        return keccak256(bytes(_username));
    }
    ...
    function getBios(
        ...
        return bios[_usernameToHash(_username)][key];
    }
    ...
    function toggleWhitelisted() public onlyOwner {
        whitelistEnabled = !whitelistEnabled;
    }

    function addAdmin(address _newAdmin) public onlyOwner {
        _adminPermissions[_newAdmin] = true;
    }

    function deleteAdmin(address _admin) public onlyOwner {
        delete _adminPermissions[_admin];
    }
}