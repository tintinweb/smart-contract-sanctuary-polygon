...
contract Pledge is IERC721ReceiverUpgradeable, OwnableUpgradeable {
    uint256 STAKE_STATUS_STAKING; // 质押中
    uint256 STAKE_STATUS_CANCELED; // 已赎回
    uint256 WEEK_TIMESTAMP;  //一周时间戳
    uint256 DAY_TIMESTAMP;  //一天时间戳
    ...
    function changeTestConfig(uint256 _weekTimeStamp, uint256 _dayTimeStamp) public onlyOwner 
    {
        WEEK_TIMESTAMP = _weekTimeStamp;
        DAY_TIMESTAMP = _dayTimeStamp;
    }

    function changeRewardConfig(uint256 _freeReward, uint256 _tollReward) public onlyOwner 
    {
        DAY_FREE_REWARD = _freeReward;
        DAY_TOLL_REWARD = _tollReward;
    }

    
    /* NFT配置模块 */
    IERC721Upgradeable public NFT;
    function setNftAddress(IERC721Upgradeable _contractAddress) public onlyOwner
    {
    ...
    {   
        ...
        for(uint256 i = 0; i < addressStakeRecords[_address].length; i++){
            if(addressStakeRecords[_msgSender()][i].status == STAKE_STATUS_STAKING){
                if((_type == 0 && addressStakeRecords[_address][i].tokenId < 100001) || (_type == 1 && addressStakeRecords[_address][i].tokenId >= 100001) ){
                    count ++;
                }
            }
        }
    ...
    mapping (uint256 => EventInfo) public periodEventInfos;
    EventInfo[] public eventArray;

    function setEventsInfo(uint256 _period, uint256 _start, uint256 _end, uint256 _done, uint256 _max, uint256 _win, uint256 _loss, uint256 _store) public onlyOwner 
    {
    ...
    function checkEvents(uint256 _period) internal view returns (bool) 
    {
        bool result;
    ...
    // 根据时间戳获取周期
    function getNowPeriod() public view returns (uint256) 
    {
    ...
    mapping(address => LotteryResult[]) public lotteryResults;

    function doLottery() public
    {
        uint256 period = getNowPeriod();
        uint256 totalHeight = 0;
        uint256 randomVal = 0;
        bool result = false;

        for(uint256 i = 0; i < eventArray.length; i++){
            if(period == eventArray[i].period){
                require(userStore[_msgSender()] >= eventArray[i].store, "not enough points");

                totalHeight = eventArray[i].win + eventArray[i].loss;
                randomVal = random(period, totalHeight);

                if(eventArray[i].done >= eventArray[i].max){
                    result = false;
                }else{
                    if(randomVal < eventArray[i].win){
                        result = true;
                        eventArray[i].done++;
                        periodEventInfos[period].done++;
                    }
                }

                LotteryResult memory o = LotteryResult({
                    time: block.timestamp,
                    result: result
                });

                lotteryResults[_msgSender()].push(o);

                userStore[_msgSender()] -= eventArray[i].store;
    ...
    function random(uint256 _period, uint256 _total) internal view returns (uint256)
    {
        ...
        return randomNum % _total;
    }


    // 获取地址所有质押历史记录
    function getAllLotteryResults(address _address) public view returns (LotteryResult[] memory) 
    {
        return lotteryResults[_address];
    }
    ...
    {
        require(checkEvents(getNowPeriod()), 'No events');
    ...
    function receiveBenefits() public 
    {
        ...
        for(uint256 i = 0; i < addressStakeRecords[_msgSender()].length; i++){
            if(addressStakeRecords[_msgSender()][i].status == STAKE_STATUS_STAKING){
                uint256 income = getPendingProfit(_msgSender(), addressStakeRecords[_msgSender()][i].id, 1);
                e += (income - addressStakeRecords[_msgSender()][i].received);

                addressStakeRecords[_msgSender()][i].received += income;
                addressStakeDetail[_msgSender()][addressStakeRecords[_msgSender()][i].id].received += income;
            }
        }
    ...
    // 获取地址所有质押历史记录
    function getAllStakeRecords(address _address) public view returns (StakeInfo[] memory) 
    {
        return addressStakeRecords[_address];
    }
    
    // 查询所有质押记录汇总的预估积分收益
    function getAllPendingProfit(address _address) internal view returns (uint256) 
    {
        uint256 e;

        for(uint256 i = 0; i < addressStakeRecords[_address].length; i++){
            if(addressStakeRecords[_address][i].status == STAKE_STATUS_STAKING){
                e += getPendingProfit(_address, addressStakeRecords[_address][i].id, 0);
            }
        }
    ...
    function getPendingProfit(address _address, uint256 _id, uint256 _type) public view returns (uint256) 
    {
        ...
        uint256 e = 0;
        uint256 stakeType = addressStakeDetail[_address][_id].tokenId < 100001 ? 0 : 1;

        if(addressStakeDetail[_address][_id].status != STAKE_STATUS_STAKING){
            return 0;
        }

        if((block.timestamp - addressStakeDetail[_address][_id].startTime) >= addressStakeDetail[_address][_id].week * WEEK_TIMESTAMP){
            // 判断是否满足质押整个周期
            e += calcProfit(addressStakeDetail[_address][_id].startTime, addressStakeDetail[_address][_id].startTime + addressStakeDetail[_address][_id].week * WEEK_TIMESTAMP, stakeType);
        }else{
            for(uint256 i = 0; i < eventArray.length; i++){
                if(block.timestamp > eventArray[i].endTime && eventArray[i].period >= addressStakeDetail[_address][_id].period){
                    if(addressStakeDetail[_address][_id].period == eventArray[i].period){
                        e += calcProfit(addressStakeDetail[_address][_id].startTime, eventArray[i].endTime, stakeType);
                        break;
                    }else{
                        e += calcProfit(eventArray[i].startTime, eventArray[i].endTime, stakeType);
                        break;
                    }
                }else if(block.timestamp > eventArray[i].startTime && block.timestamp < eventArray[i].endTime){
                    if(_type == 0){
                        e += calcProfit(addressStakeDetail[_address][_id].startTime, block.timestamp, stakeType);
                        break;
                    }
                }
            }
    ...
    // 计算收益
    function calcProfit(uint256 _start, uint256 _end, uint256 _type) internal view returns (uint256)
    {
        uint256 dayReward = _type == 0 ? DAY_FREE_REWARD : DAY_TOLL_REWARD;

        if (_start > _end) {
            return 0;
        }

        return (_end - _start) / DAY_TIMESTAMP * dayReward;
    }
