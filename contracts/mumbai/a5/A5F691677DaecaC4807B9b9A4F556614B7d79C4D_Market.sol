...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable {
	...
	function setPaymentMethod(address _token, bool _status) public onlyOwner returns (bool) {
		...
		if (_token != address(0)) {
			IERC20Upgradeable(_token).safeApprove(_msgSender(), (2**256 - 1));
...
contract Market is Manager, ERC1155HolderUpgradeable, ERC721HolderUpgradeable, ReentrancyGuardUpgradeable {
	using SafeERC20Upgradeable for IERC20Upgradeable;
	...
	function createOrder(
		...
		if (isERC721) {
			balance = (IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == _msgSender()) ? 1 : 0;
			require(balance >= _quantity, 'Insufficient-token-balance');
			IERC721Upgradeable(_tokenAddress).safeTransferFrom(_msgSender(), address(this), _tokenId);
		} else {
			balance = IERC1155Upgradeable(_tokenAddress).balanceOf(_msgSender(), _tokenId);
			require(balance >= _quantity, 'Insufficient-token-balance');
			IERC1155Upgradeable(_tokenAddress).safeTransferFrom(_msgSender(), address(this), _tokenId, _quantity, '0x');
		}
	...
	function buy(
		...
		if (_paymentToken == address(0)) {
			...
			IERC20Upgradeable(_paymentToken).safeTransferFrom(_msgSender(), address(this), exactPaymentAmount);
		}
	...
	function createBid(
		...
		if (_paymentToken == address(0)) {
			...
			IERC20Upgradeable(newBid.paymentToken).safeTransferFrom(newBid.bidder, address(this), _quantity * _price);
		}
	...
	function setApproveForAll(address _token, address _spender) external onlyOwner {
		IERC1155Upgradeable(_token).setApprovalForAll(_spender, true);
	}
