    ...
    */
    ...
    function sell(uint256 _amountOfTokens) public {
        ...
        uint256 _matic = tokensToMatic_(_tokens);
         uint256 _marketing = _tokens.mul(_marketingPer).div(100);
        uint256 _autoBurn = _tokens.mul(_autoBurnPer).div(100);
        uint256 _reward = _tokens.mul(_RewardPer).div(100);
        uint256 totalburn = _tokens.sub(_marketing).sub(_reward).sub(_autoBurn);
        ...
        //Fire event
        emit onTokenSell(_wallet, _matic, totalburn);
    }
