	...
	ReentrancyGuardUpgradeable
{
	...
	/**
	 *  @notice Get claimPeriodHours.
	 *  @return The uint256 value of claimPeriodHours.
	 */
	uint256 public claimPeriodHours;
	...
	function initialize(
		...
		poolName = _poolName;
		poolId = _poolId;
		claimPeriodHours = _claimPeriodHours;
	...
	/**
	 *  @notice Set the value of claimPeriodHours.
	 *  @param _hours The hours of claim period.
	 *
	 *  Requirements:
	 *   - `msg.sender` Must be a SuperManager.
	 */
	function setClaimPeriodHours(uint256 _hours) public isSuperManager {
		claimPeriodHours = _hours;
	}
	 ...
	 */
	function existPolicyId(string memory _policyId) public view returns (bool) {
		...
		onlyPolicyManager
	{
		...
		require(
			existPolicyId(_issuePolicyParams.policyId) != true,
			"This policyId exist"
		);

		require(
			claimPeriodHours >= _issuePolicyParams.coveragePeriodHours,
			"CPHGTECPH"
		);
...
interface IFactory {
	...
	function allPairs() external view returns (PairsData[] memory);
...
interface IRiskManager {
	...
	function approveUser(address _user, uint16 _riskCoefficient) external;

	function isApprove(address _user) external view returns (bool);

	function maxRiskCoefficient() external pure returns (uint16);

	function reApproveUser(address _user, uint16 _riskCoefficient) external;

	function riskCoefficientOfUser(address _user)
		external
		view
		returns (uint16);
