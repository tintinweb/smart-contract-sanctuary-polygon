...
import "../managers/LiquidityManager.sol";
...
contract MockCurveFactory is ICurveFactory {
    ...
    function deploy_metapool(
        ...
        _coin = coin;
        address[3] memory meta;
        meta[0] = _dai;
        meta[1] = _usdc;
        meta[2] = _usdt;
        ICurvePool pool = new MockCurvePool(name, symbol, coin, meta);
        _basePool = address(pool);
        return address(pool);
    }
        ...
        returns (uint256[] memory)
    {
        ...
        balances[0] = IERC20(_coin).balanceOf(pool);
        balances[1] = IERC20(_dai).balanceOf(pool);
        balances[2] = IERC20(_usdc).balanceOf(pool);
        balances[3] = IERC20(_usdt).balanceOf(pool);
        return balances;
    }
        ...
        returns (uint256[] memory)
    {
        ...
        decimals[0] = IERC20Metadata(_coin).decimals();
        decimals[1] = IERC20Metadata(_dai).decimals();
        decimals[2] = IERC20Metadata(_usdc).decimals();
        decimals[3] = IERC20Metadata(_usdt).decimals();
        return decimals;
    }
}
...
contract LiquidityManager is AddressAccessor, Pausable {
        ...
        whenNotPaused
    {
        ...
        _splitTokenAmounts(exchange, underlying, usdr);
        ...
        returns (uint256 usdrBalance, uint256 underlyingBalance)
    {
        ...
        uint256[] memory balances = ICurveFactory(curveFactory)
            .get_underlying_balances(curvePool);
        uint256[] memory decimals = ICurveFactory(curveFactory)
            .get_underlying_decimals(curvePool);
...
contract MockCurvePool is ERC20, ICurvePool {
    address[] public meta;
    ...
    function sendTokens(
        ...
        IERC20(index == 0 ? coin : meta[index - 1]).transfer(receiver, amount);
    }
}
