...
import "https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.7.0/contracts/token/ERC721/extensions/ERC721Enumerable.sol";
...
contract Akkorokamui is ERC721, ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    constructor() ERC721("Akkorokamui", "AKK") {
        // _tokenIdCounter.increment(); // uncomment to start mint at ID 1
    }

    function safeMint() public {
        require(_tokenIdCounter.current() <= MAX_ID, "Max supply minted");
        require(balanceOf(msg.sender) == 0, "Max mint per wallet reached");
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(msg.sender, tokenId);

        string memory urinumber = Strings.toString(tokenId);
        string memory fullTokenURI = string.concat(
            "ipfs://QmYygSYisCjKN1ji82C1Dy89mu2k2QXisnEg4PkMkK9bMe/",
            urinumber,
            ".json"
        );
