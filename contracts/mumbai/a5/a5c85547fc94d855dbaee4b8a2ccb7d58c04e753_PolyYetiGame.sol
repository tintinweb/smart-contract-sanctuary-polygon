...
// File: contracts/Interfaces/IYeti.sol

pragma solidity ^0.8.7;


interface iYeti is IERC1155{
...
// File: contracts/PolyYetiGame.sol
...
contract PolyYetiGame is OwnableUpgradeable, YetiSigner, PausableUpgradeable, ReentrancyGuardUpgradeable{
...
//    bool public rescueEnabled = false;
    ...
    function setDesignatedSigner(address _signerAddress) external onlyOwner{
        designatedSigner = _signerAddress;
        nonce++;
    }
    ...
    function MintTokens(Rarity memory yetis) external {
        if(!onL2[yetis.tokenId]){
            ...
            ownerOfToken[yetis.tokenId] = yetis.userAddress;
    ...
    ///@dev This function is required so that when
    ///a player re-enters their token on L2, the stored data is queried from the mappings.
    function ReEnterMint(Rarity memory token) internal{
        yeti.mintNewTokens(token.tokenId, token.userAddress);
        ownerOfToken[token.tokenId] = token.userAddress;
        nonce++;
    }

    function BurnTokens(address from, uint tokenId) external onlyOwner{
        yeti.burnToken(tokenId, from);
        nonce++;
    }
    ...
    function levelup(uint[] memory tokenIds) external whenNotPaused {
        for(uint i = 0; i < tokenIds.length; i++){
            ...
            require(frxst.balanceOf(msg.sender) >= levelCost[levels[tokenIds[i]]], "Insufficient FRXST");
            frxst.transferFrom(_msgSender(), address(this), levelCost[levels[tokenIds[i]]]* 1 ether);
...
//                todo: find a  solution for the owner of
//                require(yeti.ownerOf(tokenIds[i]) == msg.sender, "AIN'T YO TOKEN");
                require(ownerOfToken[tokenIds[i]] == _msgSender(), "AIN'T YO TOKEN");
                yeti.safeTransferFrom(_msgSender(), address(this), tokenIds[i],1,'');
    ...
    function claimMany(Rarity memory yetiSigner, uint16[] calldata tokenIds, bool unstake) external whenNotPaused  {//whenNotPaused
        uint owed = 0;
        for (uint i = 0; i < tokenIds.length; i++) {
            require(fighters[tokenIds[i]].tokenId == tokenIds[i] || palace[tokenIds[i]].tokenId == tokenIds[i], "Yeti is not staked");
            owed += _claimYeti(yetiSigner, tokenIds[i], unstake);
        }
        require(owed > 0, "Claiming before 1 day");
        // frxst.mint(_msgSender(), owed);
        frxst.transfer(_msgSender(), owed);
        nonce++;
    }
...
//        frxst.mint(treasury, amount);
    ...
    function _claimYetiFromHospital(uint tokenId, bool healed) internal {
        ...
        yeti.safeTransferFrom(address(this), _msgSender(), tokenId, 1, "");
        popToken(tokenId, msg.sender);
        delete tokenToPosition[tokenId];
        delete hospital[tokenId];
    }

    function _claimYetiFromPalace(Rarity memory yetiSigner, uint tokenId, bool unstake) internal returns (uint owedFrxst) {
    ...
    function _claimYetiFromFighting(Rarity memory yetiSigner,uint tokenId, bool unstake) internal returns (uint owedFrxst) {
...
//      require (initiateGame[tokenId]==true,'Game Not Initiated');
        ...
        if (unstake) {
            ...
            if (rand % 1000 < newFightingHonour(levels[tokenId])) {
                ...
                yeti.safeTransferFrom(address(this), msg.sender, tokenId,1, "");
                popToken(tokenId, msg.sender);
                delete tokenToPosition[tokenId];
            }
    ...
    function _claimYeti(Rarity memory yetiSigner, uint tokenId, bool unstake) internal returns (uint owedFrxst) {
        if (fighters[tokenId].tokenId != tokenId) {
            return _claimYetiFromPalace(yetiSigner, tokenId, unstake);
        } else {
            return _claimYetiFromFighting(yetiSigner, tokenId, unstake);
        }
    }
...
//                require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
//                yeti.safeTransferFrom(address(this), _msgSender(), tokenId,1, ""); // send back Yeti
//                delete palace[tokenId];
//                if (stake.activityId == 1) {
//                    totalYetiStakedGathering -= 1;
//                } else {
