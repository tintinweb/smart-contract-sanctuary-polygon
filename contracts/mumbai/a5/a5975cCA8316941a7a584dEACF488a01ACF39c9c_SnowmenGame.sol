...
// 폴더를 만들어서 아이템을 넣는 방식 폴더 하나당 CID하나 생성(가스비 절약)
contract SnowmenGame is ERC1155Supply, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter public folderId;   // IPFS에 올라간 폴더 개수를 새는 변수

    mapping(uint256 => string) public metadataHash;
    mapping(address => bool) private _authorized;   // 권한받은 계정 정보 비공개

    modifier onlyAuthorized() {
        require(
            // 요청을 보낸 sender의 계정의 권한이 있거나, 컨트랙트를 배포한 계정이 호출할 경우 통과
    ...
    function getIds(uint256 numOfMeta) external view returns (uint256[] memory) {    
        // numOfMeta라는 uint256 배열을 만들어서 토큰 아이디를 저장
        uint256[] memory ids = new uint256[](numOfMeta);

        for(uint256 i=0; i<ids.length; ++i) {
            // 128비트 쉬프트 연산자 , 시프트된 folderId.current() 값을 변수 i와 비트 OR 연산
    ...
    // 오버라이드를 이용하여 부모 컨트랙트를 사용하는 것이 아닌 정의한 uri 함수를 사용
    function uri(uint256 tokenId) public view override returns (string memory) {
        return (
            // 
            string (
                // 여러 데이터 타입들을 하나로 합쳐서 byte 타입으로 리턴
                abi.encodePacked(
                    "ipfs://",
