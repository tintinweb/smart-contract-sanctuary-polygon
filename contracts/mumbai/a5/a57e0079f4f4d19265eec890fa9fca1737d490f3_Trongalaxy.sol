...
contract Trongalaxy {
        ...
        if (_amount > 0 && _receiver != address(0)) {
          uint contractBalance = address(this).balance;
            if (contractBalance > 0) {
                ...
                if(player.packageamount >= 100000000 && player.packageamount < 2100000000){
                    player.tronDeposit = player.tronDeposit.sub(payout.div(25).mul(10));
                }else if(player.packageamount >= 2100000000 && player.packageamount < 6000000000){
                    player.tronDeposit = player.tronDeposit.sub(payout.div(3));
                }else if(player.packageamount >= 6000000000){
    ...
    function collect(address _addr) internal {
       ...
       if (secPassed > 0 && player.time > 0) {
           ...
           //100 
            ...
            if(player.packageamount >= 2100000000 && player.packageamount < 6000000000){
                if (player.interestProfit >= player.tronDeposit.mul(3)){
                    player.interestProfit = player.tronDeposit.mul(3);
                }
            }
            if(player.packageamount >= 6000000000){
    ...
    function getvel(uint _tronDeposit) public pure returns (uint256) { 
        uint256 vel = 116000; //1%
