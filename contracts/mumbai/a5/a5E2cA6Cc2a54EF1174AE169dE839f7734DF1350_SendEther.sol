...
contract Marimo is Ownable, ERC721A {
  ...
  function getCurrentSize(uint256 tokenId) public view returns (uint256) {
    ...
    if (elapsedMinutes < 24 * 60 * 6) {
      return (100 * elapsedMinutes) / unitTime / 100  + _lastSize[tokenId];
    } else if (elapsedMinutes <= 24 * 60 * 15) {
      return (95 * elapsedMinutes + 432 * 100) / unitTime / 100 + _lastSize[tokenId];
    } else if (elapsedMinutes <= 24 * 60 * 24) {
      return (50 * elapsedMinutes + 10152 * 100) / unitTime / 100 + _lastSize[tokenId];
    } else if (elapsedMinutes <= 24 * 60 * 30) {
      return (10 * elapsedMinutes + 23976 * 100) / unitTime / 100 + _lastSize[tokenId];
    } else {
      return _lastSize[tokenId] + (28296 / unitTime);
    }
  }
