...
* @author DeCommas team
    ...
    */
contract ChunkRouter is ILayerZeroReceiver, UUPSUpgradeable, OwnableUpgradeable, ERC1155Upgradeable {

    event LocalActionExecuted(address target);
    event LocalTransfer(address indexed destination, uint256 amount);
    event DepositPicked(address indexed user, uint256 strategyId, uint256 deposit, uint256 tokens);
...
//        actionPool = actionPool_;
    ...
    function withdraw(uint256 strategyId, uint256 tokenAmount) external {
        _burn(_msgSender(), strategyId, tokenAmount);

        pendingWithdrawals[strategyId][msg.sender] += tokenAmount;

        emit RequestedWithdraw(_msgSender(), strategyId, tokenAmount);
    }

    function submitWithdrawal(address[] calldata users, uint256[] calldata amounts) external {
        require(users.length == amounts.length, "Router/submit-length-mismatch");
        for (uint256 i = 0; i < users.length; i++) {
            claimable[users[i]] += amounts[i];

            emit WithdrawalApproved(users[i], amounts[i]);
        }
    }

    function claim(uint256 amount) external {
        require(claimable[msg.sender] >= amount, "Router/not-enough-claimable");

        claimable[msg.sender] -= amount;

        IERC20(_currentUSDCToken).transfer(msg.sender, amount);
    ...
    function calcMintAmount(uint256 tvl, uint256 strategyId, uint256 depositAmount) internal view returns(uint256) {
        if (tvl > 100000 && totalSupply[strategyId] > 0) {
            uint256 share = depositAmount * 1e18 / tvl;
            return share * totalSupply[strategyId] /  1e18;
        }
        // user get token amount equal to USDC amount by default
        return depositAmount * 1e12;
    }
    ...
    function localTransfer(
        ...
        require(targets.length == amounts.length, "Router/invalid-data");
        uint256 totalTransferred;
        for (uint256 i = 0; i < targets.length; i++) {
            totalTransferred += amounts[i];
            require(IERC20(_currentUSDCToken).transfer(targets[i], amounts[i]), "Router/transfer-failed");

            emit LocalTransfer(targets[i], amounts[i]);
        }
        uint256 totalUsersDeposits;
        for (uint256 i = 0; i < users.length; i++) {
            uint256 userDeposit = pendingDeposits[strategyId][users[i]];
            totalUsersDeposits += userDeposit;
            pendingDeposits[strategyId][users[i]] = 0;

            uint256 mintingAmount = calcMintAmount(strategyTvl, strategyId, userDeposit);
            _mint(users[i], strategyId, mintingAmount, "");

            emit DepositPicked(users[i], strategyId, userDeposit, mintingAmount);
        }

        require(totalUsersDeposits == totalTransferred, "Router/amounts-mismatch");
        totalStrategyPendingDeposits[strategyId] -= totalUsersDeposits;
    }
    ...
    function lzReceive(
        ...
        //FIXME removed guard for a moment
        ...
        if(flag == 1) { // bridge
           bridge(actionData);
            ...
            (
                ...
                address[] memory targets,
                uint256[] memory amounts
            ) = abi.decode(actionData,
                (uint256, uint256, address[], address[], uint256[])
            );
            localTransfer(strategyId, strategyTvl, users, targets, amounts);
        } else if (flag == 3) {
