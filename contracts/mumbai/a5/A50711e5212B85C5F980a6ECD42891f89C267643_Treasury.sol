...
contract Treasury is ITreasury, AccessControl, ReentrancyGuard {
    ...
    function payRefund(address _to, uint256 _amount) external onlyRole(ADMIN_ROLE) {
...
contract DynamicNft is AccessControl, ICoreContract {
    ...
    uint256 public genesisPrice = 0.02 ether;
    ...
    function mintGenesis(address _to) public payable {
    ...
    function refund() public onlyRole(ADMIN_ROLE) {
        ...
        ITreasury treasury = ITreasury(payable(treasuryAddress));
        uint256[] memory toRefund = refundArray;
        
        for (uint256 i = 0; i < refundArray.length; i++) {
            address _to = GenesisNft.ownerOf(refundArray[i]);
            delete refundArray[i];
            Genesis storage token = genesisHolder[toRefund[i]];
            GenesisNft.burn(token.tokenId);
            delete genesisHolder[toRefund[i]];
            treasury.payRefund(_to, _amount);
        }

        emit Refunded();
    }

    function transferNft (address _from, address _to, uint256 _tokenId) public onlyRole(NFT_ROLE) {
        if (isNftHolder[_from][TOKEN.GENESIS] == true) {
            Genesis storage token = genesisHolder[_tokenId];
            token.owner = _to;
            genesisHolder[_tokenId] = token;
        } else if (isNftHolder[_from][TOKEN.SUBSCRIPTION] == true) {
            Subscription storage token = subsHolder[_tokenId];
            token.owner = _to;
            subsHolder[_tokenId] = token;
        }
    }
