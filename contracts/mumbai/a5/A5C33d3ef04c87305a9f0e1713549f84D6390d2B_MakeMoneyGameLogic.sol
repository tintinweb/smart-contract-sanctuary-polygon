...
// File contracts/MakeMoneyGameLogic.sol

pragma solidity >=0.4.22 <0.9.0;


 contract MakeMoneyGameLogic is Ownable{

  address public gameDataAddress; 
  address public gameTokenAddress; 
  uint256 private fee = 0.01 ether; 
  uint256 private makeMoneyRewardBase = 1 ether;

  constructor(address _gameDataAddress, address _gameTokenAddress) {
    gameDataAddress = _gameDataAddress;
    gameTokenAddress = _gameTokenAddress;
  }

  function getPlayerData() public view returns (PlayerData memory) {
    address player = msg.sender;
    return GameDataInterface(gameDataAddress).getPlayerData(player);
  }

  function getGameTokenBalance() public view returns (uint256) {
    address player = msg.sender;
    return IERC20(gameTokenAddress).balanceOf(player);
  }

  function getCapital() public view returns (uint256) {
    return IERC20(gameTokenAddress).balanceOf(gameDataAddress);
  }

  function makeMoney() public payable {
    require(msg.value == fee, "MakeMoneyGameLogic: fee error");
    address player = msg.sender;
    PlayerData memory playerData = GameDataInterface(gameDataAddress).getPlayerData(player);
    uint256 reward = makeMoneyRewardBase * (playerData.gameLevel + 1); 
    GameDataInterface(gameDataAddress).transferCoin(player, reward);
    GameDataInterface(gameDataAddress).incrGameCount(player, playerData.dataVersion);
  }
