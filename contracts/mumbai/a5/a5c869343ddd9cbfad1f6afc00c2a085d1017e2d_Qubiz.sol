...
interface IRandom {
    function getRandomIds(uint amount) external returns (uint256[] memory ids);
}
    ...
    UUPSUpgradeable
{
    ...
    mapping(uint256 => string) private _siteImageUrls;

    uint256 private constant MAX_PER_TX = 10;  
    uint256 private constant MAX_SUPPLY = 637;

    uint256 private qubizPrice = 1;
    address private randomContract = 0x09FFEaEF22F75C046d557a6330317BFAeA1700C5;
    ...
    function setSiteImageUrl(string calldata siteImageUrl, uint tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "NOT_THE_OWNER!");
        _siteImageUrls[tokenId] = siteImageUrl;
    }

    function mint(uint256 amount) external payable {
        require(amount <= MAX_PER_TX, "EXCEEDS_MAX_PER_TX!");
        require(totalSupply() + amount <= MAX_SUPPLY, "EXCEEDS_MAX_SUPPLY!");
        require(uint256(getLatestPrice()) * qubizPrice * amount == msg.value, "INCORRECT_VALUE!");
        uint[] memory ids = IRandom(randomContract).getRandomIds(amount);
