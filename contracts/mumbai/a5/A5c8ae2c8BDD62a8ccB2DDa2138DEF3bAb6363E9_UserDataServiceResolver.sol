 ...
 */
    ...
    event NewAccountCreated(string identity, address indexed owner, uint256 tokenId);
    ...
    function createNewAccount(string memory accountName) external returns (uint256, string memory, AccountInfo memory) {
        (uint256 tokenId, string memory ident, AccountInfo memory Info) = _createNewAccount(msg.sender);
        setOptionalName(msg.sender, tokenId, accountName);
        return (tokenId, ident, Info);

    }

    function createNewAccount() external returns (uint256, string memory, AccountInfo memory) {
        (uint256 tokenId, string memory ident, AccountInfo memory Info) = _createNewAccount(msg.sender);
        return (tokenId, ident, Info);
    }

    function _createNewAccount(address accountOwner) internal returns (uint256, string memory, AccountInfo memory) {
        uint256 tokenId = register(accountOwner);
        string memory ident = concat("AKX3_ECOUSER_", abi.decode(abi.encode(tokenId), (string)));
        setIdent(tokenId, ident);
        bytes32 metasId = setNewMetaDatas(tokenId, msg.sender);
        AccountInfo memory Info = setAccountInfoRecord(tokenId, ident, msg.sender, metasId);
        emit NewAccountCreated(ident, msg.sender, tokenId);
    ...
    function setAccountInfoRecord(uint256 tokenId,
        ...
        AccountInfo memory Info = AccountInfo(tokenId, identity, owner, metasId, block.timestamp);
        _info[owner] = Info;
        return Info;
    }

    function myAccountInfo() external returns(AccountInfo memory Info) {
