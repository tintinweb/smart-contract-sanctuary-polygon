    ...
    UUPSUpgradeable
{
    ...
    function initialize() public initializer {
        __ERC721_init("Cepheus_Node", "CND");
    ...
    function getNodeSold() external view returns (uint256) {
        return _tokenIdCounter.current();
    }

    function getNodeType(uint256 tokenId) external view returns (uint256) {
        return _nodeType[tokenId];
    }
    ...
    function _beforeTokenTransfer(
        ...
        // Updating from address's data if it is not address zero
        if (from != address(0)) {
            uint256 length = _tokenList[from].length;
            for (uint256 i; i < length; i++) {
                if (_tokenList[from][i] == tokenId) {
                    _tokenList[from][i] = _tokenList[from][length - 1];
                    _tokenList[from].pop();
                    break;
                }
            }
