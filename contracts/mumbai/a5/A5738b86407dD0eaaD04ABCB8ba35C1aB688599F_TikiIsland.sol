...
contract TikiIsland is ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    /// variables to store the mint prices
    /// @notice prices should be specified in smallest unit of the native token of the deployed blockchain (eg: ether(should specify in wei) for ethereum)
    ...
    constructor(string memory _name, string memory _symbol, string memory _hiddenURI,  uint256 _whitelistMintPrice, uint256 _publicMintPrice, uint256 _maxSupply, uint256 _whitelistMintLimit, uint256 _publicMintLimit, address payable _teamPaymentSplitterAddress, address payable _royaltyRecipient, uint96 _royaltyPercentageBasisPoints) ERC721(_name, _symbol) {
        maxSupply = _maxSupply;
        setHiddenURI(_hiddenURI);
        setWhitelistMintPrice(_whitelistMintPrice);
    ...
    function whitelistMint(uint amount) public payable {
        ...
        require(msg.value == whitelistMintPrice*amount, "Value doesnot match with whitelist mint price");
    ...
    function publicMint(uint amount) public payable {
        ...
        require(msg.value == publicMintPrice*amount, "Value doesnot match with mint price");
    ...
    /// function to set/update the public mint price
