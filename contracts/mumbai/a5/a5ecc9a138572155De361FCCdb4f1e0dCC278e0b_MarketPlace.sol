...
contract MarketPlace is Ownable, IMarketPlace, Multicall {
	...
	constructor(address _nft, address _platformfeeaddress) {
		nft = INFT(_nft);
		platformfee = 500; // 5%
	...
	function transferfees(uint256 _tokenId) internal {
		...
		(bool ownerrtxsucess, ) = owner.call{ value: ownerfee }('');
		require(ownerrtxsucess, 'Failed to pay token owner');
		if (_thirdpartyfee > 0) {
			(bool thirdpartytxsucess, ) = TokenInfo.thirdParty.call{ value: _thirdpartyfee }('');
			require(thirdpartytxsucess, 'Failed to pay Third Party');
		}
	}
		...
		)
	{
		...
		if (block.timestamp <= TokenInfo.feeExpiryTime) {
			_thirdpartyfee = cutPer10000(TokenInfo.thirdpartyfee, TokenInfo.minPrice);
		} else _thirdpartyfee = 0;
		uint256 ownerfee = TokenInfo.minPrice.sub(_artistfee).sub(_platformfee).sub(_galleryownerfee).sub(_thirdpartyfee);

		return (_galleryownerfee, _artistfee, _platformfee, _thirdpartyfee, ownerfee);
	}
