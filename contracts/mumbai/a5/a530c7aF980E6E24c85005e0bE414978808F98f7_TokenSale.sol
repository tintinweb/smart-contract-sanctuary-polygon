...
contract TokenSale is Initializable, OwnableUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event UpdatedRefRate(uint256 _rate);
    event UpdatedRewardRate(uint256 _rate);
    ...
    event Sold(address indexed account, address indexed _token, uint256 _amount, uint256 amount);
    ...
    function buy(address _token, uint256 _amount, address referrer) public nonReentrant returns (uint256) {
        ...
        if (!refTree.hasReferrer(_msgSender())) {
            ...
            referrer = refTree.linearTree[_msgSender()].referrer;
        }
        ...
        if (referrer != address(0) && token.balanceOf(referrer) > 100_000 * 10 ** token.decimals()) {
            uint256 rewardAmount = MathUpgradeable.mulDiv(_amount, refRate, DECIMALS);
            IERC20MetadataUpgradeable(_token).safeTransfer(referrer, rewardAmount);
            emit SentRefReward(_msgSender(), referrer, _token, rewardAmount);
        }

        return amount;
    }

    function sell(address _token, uint256 amount) public nonReentrant returns (uint256) {
        require(price[_token] > 0, "Invalid token");
        ...
        emit Sold(_msgSender(), _token, _amount, amount);
    ...
    function stake(uint256 amount) public nonReentrant {
        ...
        StakerData storage stakerData = stakers[_msgSender()];
        stakerData.reward = calcReward(_msgSender());
        stakerData.lastClaimedAt = uint48(block.timestamp);

        totalStakedAmount = totalStakedAmount.add(amount);
        stakerData.amount = stakerData.amount.add(amount);

        uint256 rate;
        uint256 _rate = rewardRate / 100;
        if (totalRewardRate > rewardRate) {
            rate = getRandomNumber(_rate - 2, _rate);
        } else {
            rate = getRandomNumber(_rate, _rate + 4);
        }
        stakerData.rewardRate = uint16(rate * 100);
        totalRewardRate = uint16(
            (totalStakedAmount.mul(totalRewardRate) + stakerData.amount.mul(stakerData.rewardRate)) /
                (totalStakedAmount.add(stakerData.amount))
        );

        emit Staked(_msgSender(), stakerData.amount, stakerData.rewardRate);
    }

    function claimReward() public nonReentrant {
        StakerData storage stakerData = stakers[_msgSender()];
        uint256 amount = calcReward(_msgSender());
        require(amount > 0, "No claimable reward");
        require(getMonth(block.timestamp) != getMonth(stakerData.lastClaimedAt), "Already claimed in this month");

        token.safeTransfer(_msgSender(), amount);
        stakerData.reward = 0;
        stakerData.lastClaimedAt = uint48(block.timestamp);

        emit ClaimedReward(_msgSender(), amount);
    }

    function calcReward(address account) public view returns (uint256) {
        StakerData storage stakerData = stakers[account];
        uint256 diffDays = (block.timestamp - stakerData.lastClaimedAt) / SECONDS_PER_DAY;
        return
            stakerData.reward.add(
                MathUpgradeable.mulDiv(stakerData.amount, stakerData.rewardRate, DECIMALS).mul(diffDays).div(30)
            );
    }
    ...
    function getRandomNumber(uint256 i, uint256 j) public view returns (uint256) {
        require(j > i, "Invalid range");
        uint256 range = j - i + 1;
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % range;
    ...
    function setRefRate(uint16 _rate) external onlyOwner {
        refRate = _rate;
        emit UpdatedRefRate(_rate);
    }

    function setRewardRate(uint16 _rate) external onlyOwner {
        rewardRate = _rate;
        emit UpdatedRewardRate(_rate);
    }
