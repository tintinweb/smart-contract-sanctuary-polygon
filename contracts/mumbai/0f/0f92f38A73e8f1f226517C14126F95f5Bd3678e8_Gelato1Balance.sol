...
contract Gelato1Balance is IGelato1Balance, Proxied, Initializable {
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for IERC20;

    /// STRUCTS
    struct BalanceTreeLeafNode {
    ...
    /// @dev mapping(sponsor => token => diffAmounts)
    /// @dev Ensures that sponsor cannot withdraw in excess
    mapping(address => mapping(address => uint256))
        public diffSponsorDepositsWithdraws;
    /// @dev mapping(root => sponsor => token => withdrawalAmountInEpoch)
    /// @dev Tracks sponsor withdrawal amounts per epoch
    mapping(bytes32 => mapping(address => mapping(address => uint256)))
        public epochSponsorWithdrawals;
    ...
    event LogWhitelistedToken(address indexed _token);

    event LogUnlistedToken(address indexed _token);
    ...
    /// MODIFIERS
    modifier isManager() {
        require(_managers.contains(msg.sender), "Only managers");
        _;
    }
    ...
    function init(
        ...
        require(managersLength > 0, "Init requires at leat one manager");

        for (uint256 i; i < managersLength; i++) {
            _managers.add(managers_[i]);
        }

        for (uint256 j; j < whitelistedTokens_.length; j++) {
            _whitelistedTokens.add(whitelistedTokens_[j]);
        }
    }

    /// EXTERNAL FUNCTIONS
    function depositNative(address _sponsor) external payable override {
        require(msg.value > 0, "Null deposit");
        require(_sponsor != address(0), "Invalid sponsor");

        diffSponsorDepositsWithdraws[_sponsor][NATIVE_TOKEN] += msg.value;
    ...
    function requestWithdrawal(address _token, uint256 _desiredAmount)
        external
        override
    {
        require(_desiredAmount > 0, "Invalid withdrawal amount");
        require(
            _desiredAmount <= diffSponsorDepositsWithdraws[msg.sender][_token],
            "Cannot withdraw in excess"
        );

        emit LogRequestWithdrawal(root, msg.sender, _token, _desiredAmount);
    }

    function cancelWithdrawalRequest(
        address _token,
        uint256 _desiredCancelledAmount
    ) external override {
        emit LogCancelWithdrawalRequest(
            root,
            msg.sender,
            _token,
            _desiredCancelledAmount
        );
    }

    function withdraw(
        address _token,
        uint256 _desiredWithdrawAmount,
        uint256 _remainingBalance,
        uint256 _frozenAmount,
        bytes32[] calldata _merkleProof
    ) external override {
        require(_desiredWithdrawAmount > 0, "Invalid withdraw amount");
        ...
        require(
            (epochSponsorWithdrawals[root][msg.sender][
                _token
            ] += _desiredWithdrawAmount) <= _frozenAmount,
            "Sponsor cannot withdraw more than _frozenAmount"
        );
        // Reverts if msg.sender tries to withdraw in excess
        diffSponsorDepositsWithdraws[msg.sender][
            _token
        ] -= _desiredWithdrawAmount;

        _transferTo(_token, msg.sender, _desiredWithdrawAmount);

        emit LogSponsorWithdrawal(
            rootCache,
            msg.sender,
            _token,
            _desiredWithdrawAmount
    ...
    function addManager(address _manager) external override isManager {
    ...
    function whitelistToken(address _token) external override isManager {
        require(_token != address(0), "Invalid token address");

        _whitelistedTokens.add(_token);

        emit LogWhitelistedToken(_token);
    }

    function unlistToken(address _token) external override isManager {
        require(_whitelistedTokens.contains(_token), "Token not whitelisted");

        _whitelistedTokens.remove(_token);

        emit LogUnlistedToken(_token);
    }

    function settle(bytes32 _root) external override isManager {
        root = _root;

        emit LogSettlement(_root, msg.sender);
    }
    ...
    function settleWithSignature(
        ...
        bytes32 digest = keccak256(abi.encode(chainId, _root, managerNonce));
        managerNonce++;
        ...
        emit LogSettlement(_root, manager);
    }
    ...
    function collectFees(
        ...
        uint256 _frozenAmount,
        bytes32[] calldata _merkleProof
    ) external override isManager {
        _collectFees(_token, _amount, _frozenAmount, _merkleProof);
    }
    ...
    function feeCollector()
        ...
        view
        override
        returns (address feeCollector_)
    {
        feeCollector_ = IGelato(gelato).getFeeCollector();
    }
    ...
    function _depositToken(
        ...
        require(_amount > 0, "Null deposit");
        require(_sponsor != address(0), "Invalid sponsor");
        require(
        ...
        diffSponsorDepositsWithdraws[_sponsor][address(_token)] += _amount;
    ...
    function _collectFees(
        ...
        bytes32 leafHash = keccak256(
            abi.encode(
                ...
                address(0),
                _token,
                0, // remainingBalance is 0 for Gelato's fee accounting
                _frozenAmount
            )
        );
        ...
        _validateMerkleProof(leafHash, _merkleProof, rootCache);
        require(
            (epochSponsorWithdrawals[rootCache][address(0)][
                _token
            ] += _amount) <= _frozenAmount,
            "Cannot collect fees in excess"
        );

        address feeCollector_ = IGelato(gelato).getFeeCollector();
        _transferTo(_token, feeCollector_, _amount);

        emit LogCollectFees(rootCache, feeCollector_, _token, _amount);
    }
    ...
    function _transferTo(
        ...
        if (_token == NATIVE_TOKEN) {
            ...
            require(success, "FeeCollector ETH withdrawal failed");
        } else {
    ...
    function _validateMerkleProof(
        ...
        bool isValid = MerkleProof.verify(_merkleProof, _root, _leafHash);
...
interface IGelato {
    ...
    function setFeeCollector(address _feeCollecor) external;

    function getGasPriceOracle() external view returns (address gasPriceOracle);
...
interface IGelato1Balance {
    ...
    function unlistToken(address _token) external;
