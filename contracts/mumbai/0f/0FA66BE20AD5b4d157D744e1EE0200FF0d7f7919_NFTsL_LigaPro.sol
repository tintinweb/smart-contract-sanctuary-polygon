...
abstract contract DropSinglePhase is IDropSinglePhase {
    ...
    function claim(
        ...
        _afterClaim(startTokenId, _quantity);
    }
    ...
    ERC721AUpgradeable
{
    ...
    function tokenURI(uint256 _tokenId) public view override returns (string memory) {
        ...
        require(_ownershipOf(_tokenId).addr != address(0), "No claimet yet");
    ...
    function claimPrize(uint256 _tokenId) public returns (string memory) {
        require (_ownershipOf(_tokenId).addr == _dropMsgSender(), "Your are not the owner");
        
        string memory URIclaimed = getBaseURI(_tokenId);
        claimNFT[_tokenId] = true;
        emit tokenClaimed (_tokenId, URIclaimed);

        return string(abi.encodePacked(URIclaimed, _tokenId.toString()));
    }

    function claimPrizeState(uint256 _tokenId) public view returns (bool) { 
        require(_ownershipOf(_tokenId).addr != address(0), "No claimet yet");
    ...
    event tokenClaimed (uint256 indexed index, string tokenURI);
}