...
contract MutagenResolver is Ownable {
    ...
    error AttributesLengthMismatch();
    error AttributesRulesLengthMismatch();
    error KeyExceedsResultCounter();
    ...
    /// This mapping will keep the track of added rule with project and mutagen nft
    /// uuid of project and mutagen nft => uuid of attribute types => Rule
    mapping(bytes32 => mapping(bytes32 => Rule)) public projectMutagenToRule;

    struct Rule {
        // Project NFT Matcher values 
        // attribute_type attribute_value value_op rarity_op rarity_score
        bytes30 pMatcher;
        // ADD , Delete , Replace
        uint16 resultOp;
        // Mutagen NFT Matcher values 
        // attribute_type attribute_value value_op rarity_op rarity_score
        bytes30 mMatcher;
        // MutagensisResultCounter
        uint16 resultKey;
    }
    ...
    /// @notice Encode the matcher of the rule
    /// @param attributeType Key of the attribute
    /// @param attributeValue Value of the attribute
    /// @param valueOp Rules operand on attribute value ex: gt,ge,lt,le etc..
    /// @param rarityOp Rules operand on rarity score ex: gt,ge,lt,le etc..
    /// @param rarityScore Rarity score
    function encodingMatcher(
        bytes4 attributeType, 
        bytes22 attributeValue, 
        bytes1 valueOp, 
        bytes1 rarityOp, 
        bytes2 rarityScore
    ) 
    ...
    {
        return abi.encode(attributeType, attributeValue, valueOp, rarityOp, rarityScore);
    }

    /// @notice Decode the matcher of the rule
    /// @param data encoded data of a matcher
    function decodingMatcher(bytes memory data) public view returns(bytes4, bytes22, bytes1, bytes1, bytes1, bytes2) {
        (bytes4 attributeType, 
        bytes22 attributeValue, 
        bytes1 valueOp, 
        bytes1 rarityOp, 
        bytes2 rarityScore) = abi.decode(data, (bytes4, bytes22, bytes1, bytes1, bytes2));
    }

    /// @notice Add the Rule for project and mutagen NFT with attribute types
    /// @param projectNFT Address of project NFT
    /// @param mutagenNFT Address of mutagen NFT
    /// @param projectAttributeType Type of attribute for project NFT
    /// @param mutagenAttributeType Type of attribute for mutagen NFT
    /// @param rules Rules for the project and mutagen NFT
    function addResolverRules(
        address projectNFT,
        address mutagenNFT,
        bytes4[] calldata projectAttributeType,
        bytes4[] calldata mutagenAttributeType,
        Rule[] calldata rules
    ) external onlyOwner {
        if (projectAttributeType.length != mutagenAttributeType.length) {
            revert AttributesLengthMismatch();
        }
        if (projectAttributeType.length != rules.length) {
            revert AttributesRulesLengthMismatch();
        } 

        bytes32 uuid = _getUUID(projectNFT, mutagenNFT);
        uint256 rulesLength = rules.length;

        for (uint256 i; i < rulesLength;) {
            if (rules[i].resultKey > mutagensisResultCounter[uuid]) {
                revert KeyExceedsResultCounter();
            }
            bytes32 uuidAttr = _getUUIDForAttributeType(projectAttributeType[i], mutagenAttributeType[i]);
            projectMutagenToRule[uuid][uuidAttr] = rules[i];
            unchecked {
                ++i;
            }
        }
    ...
    function _getUUIDForAttributeType(bytes4 projectAttributeType, bytes4 mutagenAttributeType) internal pure returns(bytes32) {
        return bytes32(abi.encode(projectAttributeType, mutagenAttributeType));
    }
