...
contract LendingNFT is AccessControl, ILendingNFT, ReentrancyGuard, Pausable, ERC721Holder
{
  ...
  function paymentOrder(
    ...
    _storageAddress.updateStatus(nftId, nftType, statusPaid);
    _storageAddress.updateExpiredTime(nftId, nftType, expiredTime);

    uint256 rentalPrice = orderItem.dailyRentalPrice * orderItem.rentalDuration;
    uint256 commissionPrice = (rentalPrice * info.commissionPercent) / 100;
  ...
  function lenderRewardClaim(
    ...
    // Update status expired time
    if (orderItem.status == statusPaid && orderItem.expiredTime < block.timestamp) {
      _storageAddress.updateStatus(_nftId, _nftType, statusExpired);

      // Claim NFT (confirm customer)
    }

    _storageAddress.updateStatus(_nftId, _nftType, uint8(LendingStorage.OrderStatus.IsClaimPrice));
    uint256 rentalPrice = orderItem.dailyRentalPrice * orderItem.rentalDuration;
    uint256 receivedPrice = (rentalPrice * (100 - info.commissionPercent)) / 100;
