...
contract StakeRewarder is Ownable {
    ...
    function pendingRewards(
        ...
        return user.amount.mul(accPerShare).div(1e12).sub(user.rewardDebt).sub(user.claimedAmount);
    }
    ...
    function claim(
        ...
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_beneficiary];
        user.claimedAmount = pendingRewards(_pid, _beneficiary);
