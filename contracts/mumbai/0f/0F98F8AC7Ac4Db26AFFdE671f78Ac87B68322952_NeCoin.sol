...
// @notice: NeCoin , official NePlay token 

contract NeCoin is Ownable, ERC677, BlackList {

    /** 
    * @notice mints 2.1 billion tokens to msg.sender during deployment
    */
    constructor() ERC677("NeCoin", "NE"){
        _mint(msg.sender,  2100000000 * 10**18);
    }


    /**
    * @notice let users burn their coins
    * @param amount: amount of coins to burn
    *reverts if amount is greater than balance of msg.sender
    */
    function burnCoins(uint amount) external{
        _burn(msg.sender, amount);
    }

    /** @notice revert the transaction if the sender/receiver is blacklisted 
     * @dev override the _beforeTokenTransfer hook from ERC20.sol. Regulates all token transfers
    */
       function _beforeTokenTransfer(
        address from,
        address to,
