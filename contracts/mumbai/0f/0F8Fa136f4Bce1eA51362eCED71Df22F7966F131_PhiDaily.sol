...
contract PhiDaily is MultiOwner, ReentrancyGuard {
    /* -------------------------------------------------------------------------- */
    /*                                   CONFIG                                   */
    /* -------------------------------------------------------------------------- */
    ...
    // The phiDailyRelay contract address.
    address public phiDailyRelay;
    ...
    /*                               INITIALIZATION                               */
    /* -------------------------------------------------------------------------- */
    constructor(address _adminSigner, address _materialObject, address _emissionLogic) {
    ...
    function setPhiDailyRelay(address _phiDailyRelay) external onlyOwner {
        if (_phiDailyRelay == address(0)) {
            revert InvalidAddress({ reason: "cant set address(0)" });
        }
        phiDailyRelay = _phiDailyRelay;
    }
    ...
    function batchClaimMaterialObject(
        ...
        for (uint i = 0; i < length; ) {
            // Check that the coupon sent was signed by the admin signer
            bytes32 digest = keccak256(abi.encode(eventids[i], logicids[i], _msgSender()));
            if (!isVerifiedCoupon(digest, coupons[i])) {
                revert InvalidCoupon();
            }

            uint256 tokenid = IEmissionLogic(emissionLogic).determineTokenbyLogic(logicids[i]);

            _updateClaimStatus(_msgSender(), eventids[i], tokenid, logicids[i]);
            emit LogClaimMaterialObject(_msgSender(), eventids[i], logicids[i], tokenid);
            unchecked {
                ++i;
            }
            IMaterialObject(materialObject).getObject(_msgSender(), tokenid, 1);
    ...
    modifier onlyPhiDailyRelay() {
        require(_isPhiDailyRelay(msg.sender), "onlyGelatoRelay");
        _;
    }

    function _isPhiDailyRelay(address _forwarder) internal view returns (bool) {
        return _forwarder == phiDailyRelay;
    }
    ...
    function claimMaterialObjectByRelayer(
        ...
        Coupon memory coupon,
        address userAddress
    ) external onlyPhiDailyRelay onlyIfAllreadyClaimed(eventid, logicid) nonReentrant {
        // Check that the coupon sent was signed by the admin signer
        bytes32 digest = keccak256(abi.encode(eventid, logicid, userAddress));
        if (!isVerifiedCoupon(digest, coupon)) {
            revert InvalidCoupon();
        }
        ...
        _updateClaimStatus(userAddress, eventid, tokenid, logicid);
        emit LogClaimMaterialObject(userAddress, eventid, logicid, tokenid);
        IMaterialObject(materialObject).getObject(userAddress, tokenid, 1);
    }
    ...
    function batchClaimMaterialObjectByRelayer(
        ...
        Coupon[] memory coupons,
        address userAddress
    ) external onlyPhiDailyRelay onlyIfAllreadyClaimedMultiple(eventids, logicids) nonReentrant {
        uint256 length = eventids.length;
        // Ensure input arrays have the same length
        if (length != logicids.length || logicids.length != coupons.length) {
            revert ArrayLengthMismatch();
        ...
        for (uint i = 0; i < length; ) {
            // Check that the coupon sent was signed by the admin signer
            bytes32 digest = keccak256(abi.encode(eventids[i], logicids[i], userAddress));
            if (!isVerifiedCoupon(digest, coupons[i])) {
                revert InvalidCoupon();
            }
            ...
            _updateClaimStatus(userAddress, eventids[i], tokenid, logicids[i]);
            emit LogClaimMaterialObject(userAddress, eventids[i], logicids[i], tokenid);
            unchecked {
                ++i;
            }
            IMaterialObject(materialObject).getObject(userAddress, tokenid, 1);
