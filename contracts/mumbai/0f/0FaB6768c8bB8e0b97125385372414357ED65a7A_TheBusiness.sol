...
contract TheBusiness is Ownable {
    ...
    function getID(address _owner, uint256 randomNumber) public view returns(uint256){
        ...
        if(number <= 9600){
            ...
            plantID = 100_000_000_000 + plantType * 10**10;
            uint256 plantNumber;
            ...
            // 2102020102
            if (number <= 9850){
                plantID += 1 * 10 ** 4 +1 * 10**2 + 1; // common
            } else if (number <= 9950){
                plantID += 2 * 10 ** 4 +1 * 10**2 + 2; // rare
            } else if (number <= 9990){
                plantID += 3 * 10 ** 4 +1 * 10**2 + 3; // epic
            } else {
                plantID += 4 * 10 ** 4 +1 * 10**2 + 4; // legendary
            }
            return plantID;
        }
