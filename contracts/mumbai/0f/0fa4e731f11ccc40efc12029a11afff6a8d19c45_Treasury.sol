...
contract Treasury is ITreasury, Initializable {
  ...
  // Ladder 
  address private _ladder;
  mapping(address=>mapping(uint=>ILadder.Winner)) private _accountSeasonWinner;
  uint _reservedTournamentsBalance;
  // End of ladder
  ...
  function mintLuckWheelSNOOK(address to) external override onlyLuckWheel returns(uint) {
    ...
    uint[] memory snookIds = ISnookGame(_game).mint(1,0); 
  ...
  function initialize4(
    ...
    require(_ladder == address(0), 'Treasury: Already executed');
    _ladder = ladder;
  }
  ...
  modifier onlyLadder {
    require(msg.sender == _ladder, 'Treasury: Not ladder contract');
    _;
  }
  ...
  // from Ladder or from AnyCall
  function acceptSeasonWinners(
    uint seasonNumber,
    ILadder.Winner[] memory winners
  ) onlyLadder() external override 
  {
    for (uint i=0; i<winners.length; i++) {
      ILadder.Winner memory winner = winners[i];
      _accountSeasonWinner[winner.account][seasonNumber] = winner; 
    }
  }

  function getWinnerFor(address account, uint seasonNumber) 
    external override view returns(ILadder.Winner memory) 
  {
    return _accountSeasonWinner[account][seasonNumber];
  } 

  function _claimPrizeFor(address account, uint seasonNumber) internal {
    ILadder.Winner storage winner = _accountSeasonWinner[account][seasonNumber];
    require(winner.prizeAmount > 0, "Ladder: nothing to claim");
    require(
      _reservedTournamentsBalance >= winner.prizeAmount,  // never should happen
      "Treasury: not enough reserved tournament balance"
    );
    _skill.transfer(winner.account, winner.prizeAmount);
    _reservedTournamentsBalance = _reservedTournamentsBalance - winner.prizeAmount;
    winner.prizeAmount = 0;
  }

  function claimPrizeFor(address account, uint seasonNumber) external override {
    _claimPrizeFor(account, seasonNumber);
  }
 
  function claimPrize(uint seasonNumber) external {
  ...
  function purgeLadderWinner(
    ...
    delete _accountSeasonWinner[account][seasonNumber];
  }

  // from Ladder or from Anycall
  function reserveTournamentsBalance(uint amount) onlyLadder() external override {
    require(amount <= _tournamentsBalance, "Amount exceeds tournament balance");
    _reservedTournamentsBalance += amount;
    _tournamentsBalance -= amount;
  } 
    
  function getReservedTournamentsBalance() external view override returns(uint) {
    return _reservedTournamentsBalance;
  }

  // called by distributePrizeAmountAmongChains
  function freeUnclaimableReservedTournamentsBalance(uint amount) onlyLadder() external override {
    _reservedTournamentsBalance -= amount;
    _tournamentsBalance += amount;
  }
  // End of ladder
