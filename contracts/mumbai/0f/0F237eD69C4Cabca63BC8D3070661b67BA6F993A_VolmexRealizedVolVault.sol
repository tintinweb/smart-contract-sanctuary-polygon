...
contract VolmexRealizedVolVault is VolmexVault, VolmexRealizedVolVaultStorage {
    ...
    function rollToNextVtoken(
        ...
        // It required to approve the VToken for Auction to start auction.
    ...
    IVolmexVault
{
        ...
        returns (bool)
    {
        ...
        if (withdrawalShare <= ownedShares && ownedShares != 0) {
            return true;
        } else {
            return false;
        }
     ...
     * @param _shares amount of shares want to withdraw
     * @param _redeemer is the account to lookup share
     * @return bool value
     */
    function previewRedeem(uint256 _shares, address _redeemer)
        public
        view
        virtual
        returns (uint256)
...
interface IVolmexVault {
    ...
    function previewRedeem(uint256 _shares, address _redeemer) external view returns (uint256);
