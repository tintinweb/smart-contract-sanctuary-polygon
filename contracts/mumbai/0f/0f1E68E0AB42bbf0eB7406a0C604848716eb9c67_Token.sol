...
contract Token is TokenERC20, Control, ReentrancyGuard {
	...
	event Log(string str, uint112 val1, uint112 val2);
	...
	constructor(
		...
		_gonBalances[_admin] = TOTAL_GONS;
		feeAddress = _feeTaker;
	}
	...
	function getFreeToSell(address user) external view initialized returns (uint256 limitAvail) {
		...
		if (limit.year == 0 || limit.year != year || limit.month != month) {
			limitAvail = (balanceOf(user) * feeFree) / 100;
		} else {
	...
	function rebase() private {
		...
		} else if (lastRebasePrice > currentPrice && currentPrice < (lastRebasePrice * 1005) / 1000 ) {

			uint256 currentSupply = (lastRebasePrice * _totalSupply) / currentPrice;
			uint256 supplyDelta = currentSupply - _totalSupply;
			uint256 previousSupply = _totalSupply;
			lastRebasePrice = currentPrice;
			_rebase(supplyDelta);
			IUniswapV2Pair(pair).skim(feeAddress);
	...
	function _transfer(
		...
		uint256 fee;
		uint256 feeGon;
		if (_isDexAddress(to) && from != whitelistAddress) {
		...
		if (_isDexAddress(to) && from != whitelistAddress && pair != address(0)) {
			rebase();
		}
	...
	/**
	 * @notice max(0, left - right)
	 * @param left: first var
	 * @param right: second var
	 * @return maximum from 0 and (left - right)
	 */
...
abstract contract Control is AccessControl {
	...
	modifier isPermitToChange() {
		require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || hasRole(MANAGER, msg.sender), "You are not allowed");
		_;
	}

	modifier isBackendRole() {
		require(hasRole(BACKEND, msg.sender), "Not backend");
		_;
	}
	...
	/**
	 * @notice Set whitelist for owner
	 * @param _whitelistAddress: address from which would be added liqudity
	 */
	function setWhitelistAddress(address _whitelistAddress) external isPermitToChange isValidAddress(_whitelistAddress) {
	...
	function setFee(uint8 _operateFee, uint8 _holderFee) external isValidPercent(_operateFee) isValidPercent(_holderFee) isPermitToChange {
		require(_operateFee + _holderFee <= 100, "Fee sum must be below 100");
	...
	function setInflactionKoeff(uint32 inflKoef) external isPermitToChange {
		require(inflKoef != 0, "Must not be 0");
