...
contract NsbLocker is Ownable, Lockable, ILocker {
    ...
    function checkLocked(address _user) internal view returns (bool) {
        return
            !whitelist[_user]
                ? true
                : (block.timestamp >= lockedItems[_user].end);
    }
    ...
    function isLocked(
        ...
        require(
            (msg.sender == ERC20Address) || (msg.sender == owner()),
            "You are not allowed"
        );

        // Unlock for all system
        if (!systemLock) return false;
    ...
    function getLockedAmount(address _user) public view returns (uint256) {
        // Unlock for all system
        if (!systemLock) return 0;

        if (checkLocked(_user)) return 0;
    ...
    function getLockedInfo(
        address _user
    ) external view returns (LockItem memory) {
