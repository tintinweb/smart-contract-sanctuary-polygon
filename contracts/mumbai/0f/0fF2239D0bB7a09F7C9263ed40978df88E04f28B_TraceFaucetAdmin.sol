...
// File contracts/TraceFaucetAdmin.sol
...
contract Escapable is Ownable, ReentrancyGuard {
  ...
  function escapeHatch(address _token, address payable _escapeHatchDestination) external onlyOwner nonReentrant {
    ...
    if (_token == address(0x0)) {
        ...
        (bool sucess, ) = _escapeHatchDestination.call {
            value: balance,
            gas: 200000
        } ("");
    ...
    token.transfer(_escapeHatchDestination, balance);
    emit EscapeHatchCalled(_token, balance);
  }
...
contract TraceFaucetAdmin is Pausable, Escapable {
    ...
    modifier validToken(address tokenAddress) {
        require(
            _isWhitelistedToken(tokenAddress),
            "No whitelisted token !!!"
        );
        _;
    }
    modifier validInput(address[] memory tokenAddress, uint256[] memory amounts) {
        require(
            tokenAddress.length == amounts.length,
            "Invalid input: address and amounts length are not matched !!!"
        );
        _;
    }

    function _isWhitelistedToken(address tokenAddress) private view returns(bool) {
        address[] memory _whitelistedTokens = whitelistedTokens;
        if (tokenAddress == address(0)) {
            return true;
        }
        for (uint256 i = 0; i < _whitelistedTokens.length; i++) {
            if (tokenAddress == _whitelistedTokens[i]) {
                return true;
            }
        }
        return false;
    ...
    ///@notice send token to mutiple address
    ///@dev
    ///@dev  
    ///@param tokenAddress - token address to send
    ///@param userAddress - array of accounts to which adming send token
    ...
    function bulkTransfer(
        ...
        validInput(userAddress, amounts)
        nonReentrant
        whenNotPaused
    {
        ...
        if (tokenAddress == address(0)) {
            for (uint256 i = 0; i < userAddress.length; i++) {
                (bool sucess, ) = payable(userAddress[i]).call {
                    value: amounts[i],
                    gas: 200000
                } ("");
                require (sucess, "Failed to send ETH");
            }
        } else {
            ...
            require (
                IERC20(tokenAddress).allowance(msg.sender, address(this)) >= totalAmount, 
                "total allowance is less than total amount to send"
            );
            for (uint256 i = 0; i < amounts.length; i++) {
    ...
    function setWhitelistedTokens(address[] memory newWhitelistedTokens)
        ...
        whenNotPaused
	{
		whitelistedTokens = newWhitelistedTokens;
	}
