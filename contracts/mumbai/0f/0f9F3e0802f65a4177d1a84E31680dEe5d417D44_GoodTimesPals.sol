...
contract GoodTimesPals is Ownable, ERC721A, ReentrancyGuard {
  using Strings for uint256;
  using ECDSA for bytes32;

  string public baseURI = "ipfs://bafybeicoth3x7b46oi5rtkt7mdvcf5lhc2hyszetvf6onzjamgx2bhm3xa/";
  uint16 public maxSupply = 7500;

  bool public isPublicSale = true;

  uint256 public mintPerWallet = 5;
  ...
  ) ERC721A("Good Times Pals", "GTP", maxSupply, maxSupply) {
    transferOwnership(_owner);
    _safeMint(_owner, _ownerMintAmount);
    signer = _signer;
  }

  function mint(uint256 _mintAmount, uint256 _price, bytes memory _signature) payable public {
    ...
    if (msg.sender != owner()) {
      require(isSignatureValid(msg.sender, _price, nonce[msg.sender], _signature), "Invalid signature");
      require(msg.value >= _price * _mintAmount, "Amount sent less than the cost of minting NFT(s)");
    }
  ...
  function isSignatureValid(
    ...
    bytes32 hash = keccak256(abi.encodePacked(_whitelistAddress, _price, _nonce));
