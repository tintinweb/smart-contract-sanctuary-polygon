...
contract LeslarMetaVerseV3 is ERC721A, ReentrancyGuardUpgradeable, AccessControlUpgradeable {

  using StringsUpgradeable for uint256;
  bytes32 public constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  bytes32 public merkleRoot;
  bytes32 public listPreSaleRoot;
  ...
  function preSaleMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    bytes32 leaf = keccak256(abi.encodePacked(_msgSender()));
    require(MerkleProofUpgradeable.verify(_merkleProof, listPreSaleRoot, leaf), 'The user is not logged in to the presale participant');
  ...
  function setListPreSaleRoot(bytes32 _preSaleRoot) public onlyRole(DEFAULT_ADMIN_ROLE) {
    listPreSaleRoot = _preSaleRoot;
  }
