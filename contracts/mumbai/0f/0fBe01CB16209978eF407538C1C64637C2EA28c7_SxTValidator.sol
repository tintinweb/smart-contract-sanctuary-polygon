...
/// @dev This contract will be deployed by SxT team, used fulfill the request by Oracle SxT Node

contract SxTValidator is ISxTValidator, AdminUpgradeable, PausableUpgradeable {
    ...
    uint256 public sxt_oracleMask;

    // Maximum number of oracles the offchain reporting protocol is designed for
    uint64 public sxt_maxNumOracles;

    // Request Expire time
    uint256 private sxt_requestExpireTime;
    ...
    //  * @param relay SxTRelay contract address
    //  */
     ...
     * @param relay SxTRelay contract address
     */
    function initialize(address relay) initializer public {
        sxt_oracleMask = 0x0001010101010101010101010101010101010101010101010101010101010101;
        sxt_maxNumOracles = 31;
        sxt_requestExpireTime = 5 minutes;
    ...
    /**
     * Set SxTRelay contract address
     * @param relay SxTRelay contract address
     */
    function setSxTRelay(address relay) public whenNotPaused onlyAdmin {
        sxtRelay = relay;
     ...
     */
    function setSXTRequestExpireTime(uint256 expireTime) external whenNotPaused onlyAdmin {
     ...
     */
    function setMaxOracleCount(uint64 count, uint256 oracleMask ) external whenNotPaused onlyAdmin {
     ...
     * @dev Only SxTRelay contract will be able to call this function
    ...
    ) external whenNotPaused onlySxTSigner{
     ...
     * @param request struct instance of a request
     */
    ...
    */
    ...
    /// @notice modifier for checking if caller is SxTRelay contract
    modifier onlySxTRelay() {
        require(msg.sender == sxtRelay, "SxTValidator: only SxTRelay can call");
        _;
    }

    /// @notice modifier for checking if caller is valid signer
    modifier onlySxTSigner() {        
        Signer memory signer = sxt_signers[msg.sender];
        require(signer.active, "SxTValidator: Invalid caller. Only valid signer can call.");
        _;
    }
}