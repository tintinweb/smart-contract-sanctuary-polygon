...
contract TokenVesting is Ownable, ReentrancyGuard {
 ...
 function lock (LockParams[] calldata _lock_params) external nonReentrant {
    require(_lock_params.length > 0, 'NO PARAMS');
    address _token = address(RETH);
    uint256 totalAmount = 0;
    for (uint256 i = 0; i < _lock_params.length; i++) {
        totalAmount += _lock_params[i].tokensDeposited;
    ...
    for (uint256 i = 0; i < _lock_params.length; i++) {
        ...
        uint256 amountInTokens = FullMath.mulDiv(lock_param.tokensDeposited, amountIn, totalAmount);
        balanceBefore += amountInTokens;

        TokenLock memory token_lock;
        token_lock.tokensDeposited = amountIn;
