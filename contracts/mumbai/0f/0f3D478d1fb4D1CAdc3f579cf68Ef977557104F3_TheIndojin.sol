...
contract TheIndojin is ERC721AQueryable, Ownable, ReentrancyGuard {
  ...
  uint256 public maxfreeMintsupply = 1;
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) {
    ...
    //The free token is not claimed yet
        ...
        //The buyer asked to mint N tokens, so he will pay N-1 tokens
            ...
            require(freemintCount[msg.sender] + (1-_mintAmount+_mintAmount) <= maxfreeMintsupply, 'Freemint supply is over');  

        }
    //Ok, this call is free but only this
        freeMintClaimed[_msgSender()]=true;   
        freemintCount[msg.sender] += (1-_mintAmount+_mintAmount);     
    }else{
