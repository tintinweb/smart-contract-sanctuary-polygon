...
contract Auction {
    ...
    function cancelListingAuction(
        ...
        if (stanFund[msg.sender] < feeSystem.userFee) revert InvalidBalance();
    ...
    function makeOfferFixedPrice(
        ...
        if (stanFund[_maker] < _amount) revert InvalidBalance();
        if (_expirationTime <= block.timestamp) revert InvalidTimestamp();
        if (Listings[_nftID].Amount > _amount) revert InvalidOffer();
    ...
    function placeBidAuction(
        ...
        if (Amount > _amount) revert InvalidOffer();
    ...
    function acceptOfferPvP(
        ...
        if (msg.sender != ownerOfNFT) revert InvalidOwner();
    ...
    function cancelOfferAuction(
        ...
        if (msg.sender != maker) revert InvalidOwner();
        if (auctionIdToAuction[_auctionId].offerAmount == 0)
    ...
    function cancelOfferAuto(
        ...
        if (_auctionId.length != 0) backFeeToUserFund(_indexId);

        processEvent(_tokenId, _requestId, "EXPIRED_FIX_PRICE", _isWeb);
    }

    function cancelListingAuto(
        bytes memory _requestId,
