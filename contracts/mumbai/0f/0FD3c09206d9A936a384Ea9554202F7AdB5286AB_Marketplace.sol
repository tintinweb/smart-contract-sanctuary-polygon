...
* @dev Fired in cancelOfferOnBuyOrder()
   ...
   * @param _collectionAddress the address of collection dao to get marketplace fee
   * @return amount with fees deducted
   */
  function takeFeesFrom(uint256 _amount, address _collectionAddress) internal returns (uint256) {
    // Calculate marketplaceFee
    uint256 marketplaceFee = Collection(_collectionAddress).checkDiscount();
    
    // Send fee portion to DAO
    (bool daoSuccess,) = daoAddress.call{value: marketplaceFee}("");
    require(daoSuccess, "could not transfer to dao");

    uint256 result = _amount - marketplaceFee;
    return result;
  }
 ...
 * @notice Cancelles an offer on a sell order
  ...
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    //     (_collectionAddress,,) = card.cardCollectionInfo(cardCollection);
    //   }
  ...
  function fulfillSellOrder(bytes32 _orderId) external payable onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    if(!sellOrder.owner.isContract()) {
      // Transfer Ether amount without fees to sell order owner
      // (fees are paid to DAO and reserve in the takeFeesFrom function)
      payable(sellOrder.owner).transfer(takeFeesFrom(sellOrder.highestOffer, _collectionAddress));
    }
...
function buySellOrder(bytes32 _orderId) external payable onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    if(!sellOrder.owner.isContract()) {
      // Transfer Ether amount without fees to sell order owner
      // (fees are paid to DAO and reserve in the takeFeesFrom function)
      payable(sellOrder.owner).transfer(takeFeesFrom(msg.value, _collectionAddress));
    }
 ...
 * @notice Cancelles an offer on a buy order
  ...
  function fulfilBuyOrder(bytes32 _orderId, uint256 _cardId) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    ...
    (address _collectionAddress,,,) = card.cardCollectionInfo(cardCollection);
    ...
    // (fees are paid to DAO and reserve in the takeFeesFrom function)
    payable(msg.sender).transfer(takeFeesFrom(buyOrder.bid, _collectionAddress));
...
/**
  * @notice Collection interface for getting collection data
   *
   * EXAMPLE:
   * Dao attribution
   * public checkDiscount function; 
   *
   */
interface Collection {
    function checkDiscount() external view returns(uint256);
}
