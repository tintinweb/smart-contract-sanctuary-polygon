  ...
  using SafeMath for uint256;
   ...
   event onBuy(address buyer , uint256 amount);
    ...
    function withdrawLostBNBFromBalance() public 
    {
	 ...
	 function SwapWithMatic(string memory userAddress,string memory member_user_id,uint256 totalAmt,string memory swaptype) public payable {
    ...
    address(uint160(owner)).transfer(msg.value);
	 ...
	 function SwapWithDAI(string memory userAddress,string memory member_user_id,uint256 busd_amt,string memory swaptype) public payable {
  	
		DAI.transferFrom(msg.sender,owner,busd_amt);
