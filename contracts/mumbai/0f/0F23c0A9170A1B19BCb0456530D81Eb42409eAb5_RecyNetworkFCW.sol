...
contract MetadataStorage is Multicall, Ownable {
    ...
    function getMetadataJSON(uint256 id) public view returns (string memory) {
        MetadataLibrary.Attribute[] memory arrAttributes = metadatas[id]
            .attributes;
        MetadataLibrary.Property[] memory arrProperties = metadatas[id]
            .properties;
        MetadataLibrary.Attribute[]
            memory arrAttributesTemp = new MetadataLibrary.Attribute[](
        ...
        for (uint256 i = 0; i < idx; i++) {
            ...
            if (!isEmpty(arrAttributesTemp[i].displayType)) {
                metadataParcial = abi.encodePacked(
                    '"',
                    arrAttributesTemp[i].tagType,
                    '":',
                    '"',
                    arrAttributesTemp[i].displayType,
                    '",'
                );
                metadata = abi.encodePacked(metadata, metadataParcial);
            }
            ...
            metadataParcial = abi.encodePacked(
                '"',
                arrAttributesTemp[i].tagKey,
                '":',
                '"',
                arrAttributesTemp[i].keyType,
                '",'
            );
            metadata = abi.encodePacked(metadata, metadataParcial);
            ...
            metadataParcial = abi.encodePacked(
                '"',
                arrAttributesTemp[i].tagValue,
                '":',
                '"',
                arrAttributesTemp[i].valueType,
                '"'
            );
            metadata = abi.encodePacked(metadata, metadataParcial);
            metadata = abi.encodePacked(metadata, "}");
...
contract NFTMetadata is ERC721, ERC721Burnable, MetadataStorage {
    constructor(
    ...
    function safeMintMetadata(
        ...
        require(!_exists(tokenId), "NFTMetadata: invalid token ID");
...
contract RecyNetworkFCW is NFTMetadata {
    ...
    // id status => year month => report
    mapping(uint256 => mapping(bytes32 => Report)) reports;
    // id status => year month => signatures
    mapping(uint256 => mapping(bytes32 => Signature[])) reportSignatures;
    // id status => year month => adress audit => index array signature
    mapping(uint256 => mapping(bytes32 => mapping(address => uint256))) indexSignatures;
    ...
    constructor(
        string memory nameToken,
        string memory symbolToken
    ) NFTMetadata(nameToken, symbolToken) {
    }
    ...
    function addReport(
        ...
        require(_exists(statusID), "RecyNetworkFCW: status not exist");
        require(!isEmpty(url), "RecyNetworkFCW: Report  url empty");
        require(
            yearMonth != bytes32(""),
            "RecyNetworkFCW: Report  yearMonth empty"
        );
        Report memory report = Report(yearMonth, url, block.timestamp);
        Report memory reportExist = reports[statusID][yearMonth];

        require(isEmpty(reportExist.url), "RecyNetworkFCW: Report exist");
        reports[statusID][yearMonth] = report;
    }
    ...
    function getReport(
        ...
        require(_exists(statusID), "RecyNetworkFCW: status not exist");
        Report memory reportExist = reports[statusID][yearMonth];
        require(!isEmpty(reportExist.url), "RecyNetworkFCW: Report not exist");
        return reports[statusID][yearMonth];
    }
    ...
    function signReport(
        ...
        bool check = checkReportSignature(
            statusID,
            yearMonth,
            auditorAddress,
            nonce,
        ...
        require(check, "RecyNetworkFCW: Report not checkReportSignature");
        ...
        reportSignatures[statusID][yearMonth].push(sign);
        uint256 idx = reportSignatures[statusID][yearMonth].length - 1;
        // id status => year month => adress audit => index array signature
        indexSignatures[statusID][yearMonth][auditorAddress] = idx;
    }

    function checkReportSignature(
        uint256 statusID,
        bytes32 yearMonth,
        address signer,
        uint256 nonce,
        ...
        require(_exists(statusID), "RecyNetworkFCW: status not exist");
        bytes32 messageHash = getReportMessageHash(statusID, yearMonth, nonce);
    ...
    function getReportSignature(
        ...
        Report memory reportExist = reports[statusID][yearMonth];
        require(!isEmpty(reportExist.url), "RecyNetworkFCW: Report not exist");
        uint256 idx = indexSignatures[statusID][yearMonth][auditorAddress];
        // id status => year month => signatures
        Signature memory signature = reportSignatures[statusID][yearMonth][idx];
        require(
            signature.auditorAddress == auditorAddress,
            "RecyNetworkFCW: Report not signed by auditor"
        );
    ...
    function getReportSignatures(
        ...
        require(!isEmpty(reportExist.url), "RecyNetworkFCW: Report not exist");
        // id status => year month => signatures
        Signature[] memory signatures = reportSignatures[statusID][yearMonth];
        return signatures;
    }

    function getReportMessageHash(
        uint256 statusID,
        bytes32 yearMonth,
        uint256 nonce
