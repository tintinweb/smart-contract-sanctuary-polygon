...
contract Marketplace is ERC721Holder, Ownable {
  ...
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    for(uint256 i = 0; i < sellOrder.assetIds.length; i++) {
      if (i == 0) {
        ...
        _collectionDaoAddress = card.cardCollectionDaoAddress(cardCollection);
      }
  ...
  function acceptBuyOrderLowestOffer(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    if(!buyOrder.lowestOfferor.isContract()) {
      ...
      address payable _collectionDaoAddress = card.cardCollectionDaoAddress(cardCollection);
...
/**
  * @notice CollectionDao interface for getting collection data
   * Collection Partner Dao should have public collectionAttribution value
   * Collection Partner Dao can have public marketplaceRoyalty value
   *
   * EXAMPLE:
   * Dao house edge
