...
contract MEMEBurunGame {
    uint256 public constant ticketPrice = 50 * 10**18; 
    uint256 public constant maxTickets = 100; 
    uint256 public constant ticketCommission = 0 * 10**18;
...
function WithdrawWinnings(address tokenAddress) public isWinner {
    ...
    // Calculate 50% of the winnings
    uint256 amountToSend = reward2Transfer / 2;
    ...
    // Send 50% of the winnings to the dead wallet
    address deadWallet = address(0x000000000000000000000000000000000000dEaD); // Replace with the actual dead wallet address
    if (tokenAddress == address(0)) {
        // Send 50% of the winnings in Ether to the dead wallet
        (bool success, ) = deadWallet.call{ value: amountToSend }("");
        require(success, "Failed to send tokens to the dead wallet");
    } else {
        // Send 50% of the winnings in tokens to the dead wallet
        IERC20 tokenToWithdraw = IERC20(tokenAddress);
        require(tokenToWithdraw.transfer(deadWallet, amountToSend), "Token transfer to dead wallet failed");
    }
}
