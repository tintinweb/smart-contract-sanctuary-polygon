...
struct Aavegotchi {
    uint16[16] equippedWearables; //The currently equipped wearables of the Aavegotchi
    // [Experience, Rarity Score, Kinship, Eye Color, Eye Shape, Brain Size, Spookiness, Aggressiveness, Energy]
    int8[6] temporaryTraitBoosts;
    int16[6] numericTraits; // Sixteen 16 bit ints.  [Eye Color, Eye Shape, Brain Size, Spookiness, Aggressiveness, Energy]
    string name;
    ...
    Ownable
{
    ...
    constructor() ERC721("AavegotchiMockNFT", "AMNFT") {}
    ...
    function getAavegotchi(
        uint tokenId
    ) external view returns (Aavegotchi memory) {
        return aavegotchis[tokenId];
    }

    function testAbiEncode() external view returns (bytes memory payload) {
        // allow 1 by default
        // require(_tokenIds.length > 0, "LzApp: tokenIds[] is empty");
        // require(
        //     _tokenIds.length == 1 ||
        ...
        //     _debitFrom(_from, _dstChainId, _toAddress, _tokenIds[i]);
        // }
        payload = abi.encode(aavegotchis[0]);
    }

    function testAbiDecode(bytes calldata data) external view returns (Aavegotchi memory aavegotchi) {
        aavegotchi = abi.decode(data, (Aavegotchi));
    }
...
import "./AavegotchiMockNFT.sol";
import {Aavegotchi} from "./AavegotchiMockNFT.sol";

contract Bridge is ProxyONFT721 {
    constructor(
    ...
    function _send(address _from, uint16 _dstChainId, bytes memory _toAddress, uint[] memory _tokenIds, address payable _refundAddress, address _zroPaymentAddress, bytes memory _adapterParams) internal override {
        ...
        for (uint i = 0; i < _tokenIds.length; i++) {
            _debitFrom(_from, _dstChainId, _toAddress, _tokenIds[i]);
            aavegotchis[i] = AavegotchiMockNFT(address(token)).getAavegotchi(_tokenIds[i]);
        }
