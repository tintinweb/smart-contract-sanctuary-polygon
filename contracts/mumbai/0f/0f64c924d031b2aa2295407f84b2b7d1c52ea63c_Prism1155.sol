 ...
 */
  ...
  returns (uint256[] memory)
  {
    require(tokens[_mNftId].isMaster == true ,"_mNFTId needs to be Master");
  ...
  function createTokens(string[] memory _name, uint256[] memory _price, uint256[] memory _projectId, uint256[] memory _collectionId, bool[] memory _isMaster, uint256[] memory _maxSupply, string[] memory _tokenURI) 
    public 
    onlyOwner
  {
  ...
  */
  ...
  function _adjustTokenHolding(
    ...
    for (uint256 i=0;i < addressToTokenIds[_from].length; i++){
      if(_id == addressToTokenIds[_from][i]){
