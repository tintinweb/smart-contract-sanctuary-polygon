...
uint256 private _minBetAmount = 1000; // Minimum bet amount
uint256 private _maxBetAmount = 100000; // Maximum bet amount
uint8 private _tokenDecimals; // Number of decimal places for the AMBER token
IERC20 private _amberToken;
mapping(uint256 => mapping(address => uint256)) private _bets; // Bet cells
...
constructor() {
    _owner = msg.sender;
    _amberToken = IERC20(0xf32d5D5FA5d2545071570c0EEf6E2C65A4c7e5c5); // Replace with the AMBER token address on the Mumbai network
    _tokenDecimals = 18; // Set the correct number of decimal places for the AMBER token
}
...
function endLobby(uint256 lobbyId) internal {
    ...
    require(_bets[lobbyId][_player1] > 0 && _bets[lobbyId][_player2] > 0, "Invalid lobby");

    address winner = determineWinner(lobbyId);
    uint256 winningAmount = (_bets[lobbyId][_player1].add(_bets[lobbyId][_player2])).mul(8) / 10; // 80% of the total bets as the winning amount
    ...
    // Transfer 15% of the total pot to a dead address
    uint256 deadAddressAmount = _totalPot.mul(15) / 100;
    _amberToken.transfer(address(0x000000000000000000000000000000000000dEaD), deadAddressAmount);
    _totalPot = _totalPot.sub(deadAddressAmount);

    // Transfer 5% of the total pot to the creator
    uint256 creatorAmount = _totalPot.mul(5) / 100;
    require(_amberToken.transfer(_player1, creatorAmount), "Failed to transfer AMBER tokens to the creator");
    _totalPot = _totalPot.sub(creatorAmount);
...
function determineWinner(uint256 lobbyId) internal view returns (address) {
    bytes32 randomHash = keccak256(abi.encodePacked(blockhash(block.number - 1), lobbyId));
    uint256 randomNumber = uint256(randomHash) % (_bets[lobbyId][_player1] + _bets[lobbyId][_player2]);

    return (randomNumber < _bets[lobbyId][_player1]) ? _player1 : _player2;
}
