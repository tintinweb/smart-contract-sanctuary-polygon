...
contract Zhuba is ERC721URIStorage, VRFConsumerBaseV2, Ownable {
    ...
    constructor(
        ...
        s_tokenCounter = 0;
        i_zhuAddress = zhuAddress;
    }

    function requestNft() public returns (uint256 requestId) {
        IERC20 zhuToken = IERC20(i_zhuAddress);
        if (
            zhuToken.allowance(msg.sender, address(this)) <
            i_mintFee * (10 ** 18)
        ) {
            revert Zhuba__NotEnoughAllowance();
        }
        zhuToken.transferFrom(
            msg.sender,
            address(this),
            i_mintFee * (10 ** 18)
        );
    ...
    function withdraw() public onlyOwner {
        IERC20 zhuToken = IERC20(i_zhuAddress);
        uint256 balance = zhuToken.balanceOf(address(this));

        bool success = zhuToken.transfer(msg.sender, balance);
        if (!success) {
