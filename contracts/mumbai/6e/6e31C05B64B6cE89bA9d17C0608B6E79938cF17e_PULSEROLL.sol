...
contract PULSEROLL is VRFConsumerBaseV2 {
    ...
    constructor(address vrfCoordinator, address link, bytes32 keyHash,address _stakingaddress,address _founderaddress,uint64 _sub)
   ...
   //Owner functions
    ...
    function ChangevrfCoordinator(address _vrfCoordinator)external onlyOwner{
        require(_vrfCoordinator != address(0),"coordinator should not equal to zero Address");
    ...
    function ChangeLinkAddress(address _linkaddress)external onlyOwner{
       require(_linkaddress != address(0),"Link address is not equal to zero Address");
    ...
    function ChangeSubscription(uint64 _sub)external onlyOwner{
        require(_sub !=0 ,"The enetered subscription is zero");
        require(s_subscriptionId != _sub,"The subscription should not be same");
        s_subscriptionId = _sub;
    }
