...
contract FALNFT is ERC20, Ownable, ERC20Permit, ERC721Holder {
    ...
    mapping(address => mapping(uint256 => FractionalizedNFT))
        public fractionalizedNFTs;

    constructor()
        ERC20("Fractional NFT Token", "FNT")
        ERC20Permit("Fractional NFT Token")
    {}
    ...
    function mintAndFractionalize(
        ...
        FractionalizedNFT storage fractionalizedNFT = fractionalizedNFTs[
            _collection
        ][_tokenId];

        require(
            fractionalizedNFT.tokenAmount == 0,
        ...
        require(
            fractionalizedNFT.tokenAmount >= _fractionAmount,
            "Fraction amount must be less than or equal to NFT token amount"
        );

        _mint(msg.sender, _fractionAmount);
    }
    ...
    //     require(
    //         msg.value == _fractionAmount * fractionalizedNFT.pricePerFraction,
    //         "Incorrect amount of ether sent"
    //     );
    //     require(balanceOf(msg.sender) == 0, "Buyer already owns fractions");

    //     _transfer(owner(), msg.sender, _fractionAmount);
    ...
    require(fractionalizedNFT.forSale == true, "Fraction not for sale");
    require(_fractionAmount >= 5, "Minimum 5 fractions can be purchased");
    require(_fractionAmount <= fractionalizedNFT.tokenAmount, "Not enough fractions available");

    uint256 purchasePrice = _fractionAmount * fractionalizedNFT.pricePerFraction;
    require(msg.value == purchasePrice, "Incorrect amount of ether sent");

    fractionalizedNFT.tokenAmount -= _fractionAmount; // Update the remaining token amount
    ...
    function redeem(address _collection, uint128 _tokenId) external {
    FractionalizedNFT storage fractionalizedNFT = fractionalizedNFTs[_collection][_tokenId];
    ...
    _burn(msg.sender, fractionalizedNFT.tokenAmount); // Burn all the fractions owned by the redeemer
