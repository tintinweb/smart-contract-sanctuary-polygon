...
// File contracts/uniswap/FullMath.sol
...
// File contracts/uniswap/LiquidityAmounts.sol
...
// File contracts/uniswap/TickMath.sol
...
// File contracts/RangeProtocolVault.sol
...
/// @dev [email protected]
contract RangeProtocolVault is Ownable, ERC20, ReentrancyGuard, IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    event Burned(address indexed receiver, uint256 burnAmount, uint256 amount0Out, uint256 amount1Out);
    ...
    constructor(
        ...
        ERC20(_name, _symbol)
    {
        require(_managerFee <= MAX_MANAGER_FEE, "FEE");
        ...
        // Upon deployment inThePosition status is true.
        inThePosition = true;
    ...
    function uniswapV3MintCallback(
        ...
        require(msg.sender == address(pool), "MCB");
    ...
    function uniswapV3SwapCallback(
        ...
        require(msg.sender == address(pool), "SCB");
        ...
        )
    {
        ...
        if (totalSupply > 0) {
            ...
            // that being out of the pool renders currently set ticks as outdated and totalSupply being zero does not allow
            // calculating correct amounts of amount0 and amount1 to be accepted from the user.
            revert("Not allowed to mint");
        }
    ...
    function removeLiquidity() external onlyManager {
        ...
        if (liquidity > 0) {
            ...
            (
                ...
                uint256 fee0,
                uint256 fee1
            ) = _withdraw(_lowerTick, _upperTick, liquidity);
    ...
    function swap(
        ...
        int256 swapAmount,
        uint160 sqrtPriceLimitX96
    ) external onlyManager returns (int256 amount0, int256 amount1) {
        (amount0, amount1) = pool.swap(address(this), zeroForOne, swapAmount, sqrtPriceLimitX96, "");
    ...
    function addLiquidity(
        ...
        uint256 amount0,
        uint256 amount1
    ) external onlyManager returns (uint256 remainingAmount0, uint256 remainingAmount1) {
        (uint160 sqrtRatioX96, , , , , , ) = pool.slot0();
        ...
        if (baseLiquidity > 0) {
            (uint256 amountDeposited0, uint256 amountDeposited1) =
                pool.mint(address(this), newLowerTick, newUpperTick, baseLiquidity, "");
    ...
    function setinThePositionStatus() external onlyManager {
        bool _inThePosition = !inThePosition;
        inThePosition = _inThePosition;

        emit InThePositionStatusSet(_inThePosition);
    }
    ...
    function updateManagerParams(int16 newManagerFee, address newManagerTreasury) external onlyManager {
        require(newManagerFee <= int16(MAX_MANAGER_FEE), "FEE");
        if (newManagerFee >= 0) {
            managerFee = uint16(newManagerFee);
        }
        if (newManagerTreasury != address(0)) {
        ...
        returns (uint256 amount0Current, uint256 amount1Current)
    {
        ...
        amount0Current += fee0 + token0.balanceOf(address(this)) - managerBalance0 - treasury0;
        amount1Current += fee1 + token1.balanceOf(address(this)) - managerBalance1 - treasury1;
    }
        ...
        )
    {
        ...
        if (amount0Current == 0 && amount1Current > 0) {
            ...
            revert("token0 and token1 amounts are zero");
        } else {
    ...
    function _applyFees(uint256 _fee0, uint256 _fee1) private {
        treasury0 += (_fee0 * treasuryFee) / 10000;
        treasury1 += (_fee1 * treasuryFee) / 10000;
        // managerFee is read from storage, so storing in a local variable saves gas cost.
        uint16 _managerFee = managerFee;
        managerBalance0 += (_fee0 * _managerFee) / 10000;
        managerBalance1 += (_fee1 * _managerFee) / 10000;
    }

    function _netFees(uint256 rawFee0, uint256 rawFee1) private view returns (uint256 fee0, uint256 fee1) {
        // managerFee is read from storage, so storing in a local variable saves gas cost.
        uint16 _managerFee = managerFee;
        uint256 deduct0 = (rawFee0 * (treasuryFee + _managerFee)) / 10000;
        uint256 deduct1 = (rawFee1 * (treasuryFee + _managerFee)) / 10000;
