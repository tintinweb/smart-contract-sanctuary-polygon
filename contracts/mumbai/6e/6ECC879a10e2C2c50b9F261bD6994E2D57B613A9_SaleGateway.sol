...
contract SaleGateway is
    ...
    address public dstSaleGatewayRemote; // dst sale gateway remote
    ...
    function init(address _endpoint) external initializer proxied {
        ...
        __NonblockingLzApp_init(_endpoint); // layer zero endpoint
	...
	function _nonblockingLzReceive(
        ...
        IPublicGovSale(targetSale).buyToken(amountIn, buyer);
    }
    ...
    function finalizeRemote(
        ...
        require(factory.isKnown(_msgSender()), "unknown");

        lzEndpoint.send{value: crossFee}( // {value: messageFee} will be paid out of this contract!
            dstChainId, // destination chainId
            abi.encodePacked(dstSaleGatewayRemote), // destination address of sale gateway remote contract
        ...
        emit Finalize(dstChainId, dstSaleGatewayRemote, _payload);
    }
    ...
    function setDstSaleGatewayRemote(address _dstSaleGatewayRemote) external onlyOwner {
        dstSaleGatewayRemote = _dstSaleGatewayRemote;
    }
