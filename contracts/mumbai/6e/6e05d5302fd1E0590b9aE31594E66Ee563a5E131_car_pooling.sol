...
contract car_pooling {
    ...
    function forceTransaction(uint256 _rideId, uint _type) public  { // approve : 1, cancel : 2
        ...
        require(_erc20Token.transfer(rUser, _price), "Insufficient ERC20 balance");
    }

    function requestTokens() public {
        require(is_user[msg.sender] && userRole[msg.sender] == 1,"User account not exist or user is not a passenger");
        require(_erc20Token.balanceOf(msg.sender)<100 ether,"user already have enough tokens");
    ...
    function completeRide(uint256 _ride_id) public returns(string memory){
        ...
        // require(payable(data.driver).send(_balance));
        uint256 _price = (_balance)*1 ether;
        require(_erc20Token.transfer(data.driver, _price), "Insufficient ERC20 balance");
