...
contract InfiniPack is ERC1155LazyMint, SignatureAction {
    address private sysAddress; // Server Address to get signatures from
    ...
    // override the burn function and add a signature check to ensure the signer is a specific address
    function burn(
        address _owner,
        uint256 _tokenId,
        uint256 _amount,
        ...
        // check the signer is the system address
        require(_isAuthorizedSigner(signer), "Not authorized: signer != sysAddress");

        // now call the original burn function
        _burn(_owner, _tokenId, _amount);
    }

    function changeSysAddress(address _sys) external onlyOwner {
        require(_sys != address(0), "sysAddress != address(0)");
        sysAddress = _sys;
    }
