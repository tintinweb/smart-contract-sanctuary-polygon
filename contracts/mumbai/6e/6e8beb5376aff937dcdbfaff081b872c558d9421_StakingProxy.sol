...
contract StakingProxy is Ownable {
    ...
    constructor(
        address _xdao, // XDAO Token contract address
        address _sxdao, // sXDAO Token contract address
    ...
    function stake(uint _amount, address _recipient) external returns (bool) {
        ...
        require(_amount != 0); // why would anyone need to stake 0 XDAO?
        ...
        claims[_recipient] = Claim({
            ...
            gons: claimInfo.gons.add(IsXDAO(sXDAO).gonsForBalance(_amount)),
    ...
    function claim(address _recipient) external {
        ...
        IERC20(sXDAO).transfer(_recipient, IsXDAO(sXDAO).balanceForGons(claimInfo.gons));
    }
