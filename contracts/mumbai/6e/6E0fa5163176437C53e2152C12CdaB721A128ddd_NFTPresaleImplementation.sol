...
/**
 * @dev {NFTPresaleImplementation} contract, including:
 *
 ...
 *  - allows to buy NFTs
 */
contract NFTPresaleImplementation is Ownable, Initializable {
    using SafeMath for uint256;

    event Redeemed(address _user, uint256 _nftId);

    event PresaleStatusChanged(bool _status);

    event OpenBuyStatusChanged(bool _status);
    ...
    event BuyTokenUpdated(address _buyToken);

    event BuyAmountUpdated(uint256 _buyAmount);
    ...
    function init(bytes memory _encodedData) external initializer {
        (
            ...
            _encodedData,
            (uint256, uint256, bytes32, ICNTNFT, IERC20, uint256, address)
        );
    }
        ...
        onlyOwner
    {
        ...
        emit MaxMintingAllowedUpdated(maxMintingAllowed);
    }
    ...
    function updateOpenBuy(bool _openBuy) external onlyOwner {
        openBuy = _openBuy;
        emit OpenBuyStatusChanged(_openBuy);
    }
    ...
    function updateBuyToken(IERC20 _buyToken) external onlyOwner {
        buyToken = _buyToken;
        emit BuyTokenUpdated(address(_buyToken));
    }
    ...
    function updateBuyAmount(uint256 _buyAmount) external onlyOwner {
        ...
        emit BuyAmountUpdated(_buyAmount);
    }
     ...
     */
    function redeemWithProof(address _account, bytes32[] calldata _proof)
        external
    {
     ...
     */
    function _redeem(address _account) private {
        ...
        buyToken.transferFrom(_account, address(this), buyAmount);
