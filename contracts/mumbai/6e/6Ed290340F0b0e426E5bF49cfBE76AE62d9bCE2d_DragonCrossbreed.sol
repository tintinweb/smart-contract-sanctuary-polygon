...
contract DragonCrossbreed is BaseAuctionReceiver, ReentrancyGuard, RandomConsumerBase {
    ...
    function platformPrice(uint count) public view returns (uint) {
        return _platformPrices[count];
    }
    ...
    whenNotPaused {
        ...
        require(
            ...
            "DC: the caller is not an owner of the dragon"
        );
        require(
        ...
        require(amount >= (_platformPrice + _rentPrice), "DC: incorrect amount sent to the contract");
    ...
    function addReward(address to, uint amount) external onlyRole(COO_ROLE) {
        require(!Address.isContract(to), "DC: address cannot be contract");
        _rewards[to] += amount;
    ...
    function claimReward(address to, uint amount) external nonReentrant {
        require(!Address.isContract(to), "DC: address cannot be contract");
        require(amount <= _rewards[_msgSender()], "DC: the given amount exceeded the allowed reward");
