...
// File contracts/AjiraPayStaing.sol

pragma solidity =0.8.4;
contract AjiraPayFinanceStaking is Pausable, Ownable, AccessControl, ReentrancyGuard{
    ...
    constructor(address _stakingToken, uint256 _minStaking, uint256 _maxStaking){
        require(_stakingToken != address(0),"AJP Staking: Invalid staking token address");
        ajiraPayFinanceToken = IERC20(_stakingToken);
        _grantRole(MANAGER_ROLE, msg.sender);
        minStake = _minStaking * 10 ** decimals;
        maxStake = _maxStaking * 10 ** decimals;
    }
    ...
    function stake(uint256 _amount, uint256 _stakingTierId) external whenNotPaused nonReentrant {
        ...
        require(_amount * 10 ** decimals >= minStake,"AJP Staking: Stake Below Minimum");
        require(_amount * 10 ** decimals <= maxStake,"AJP Staking: Stake Above Maximum");
        uint256 holderTokenBalance = ajiraPayFinanceToken.balanceOf(msg.sender);
        require(_amount * 10 ** decimals <= holderTokenBalance,"AJP Staking: Insufficient balance for staking");

        StakeTier storage _stakingTier = stakingTierById[_stakingTierId];
        require(_stakingTier.isPaused == false,"Staking tier closed");
        ...
        totalStakedTokens = totalStakedTokens.add(_amount * 10 ** decimals);
        ...
        _transferStakedTokens(msg.sender, _amount * 10 ** decimals);
        NewStake storage _stakeItem = stakingItemById[_currentStakeId];
        emit Stake(_currentStakeId, _stakeItem.beneficiary, _amount * 10 ** decimals, _stakeItem.apr, _stakeItem.stakeDuration, block.timestamp);
    }
    ...
    function emmergencyWithdraw(uint256 _stakingId) external canClaimRewards(msg.sender) hasNotClaimedStakeId(msg.sender, _stakingId) whenNotPaused nonReentrant {
        require(stakingItemIdExists[_stakingId] == true,"AJP Staking: Non-Existent Staking Id");
        NewStake storage _stakeItem = stakingItemById[_stakingId];
        uint256 itemLastStaked = _stakeItem.stakedAt;
        require(block.timestamp.add(itemLastStaked) <= 3 days,"AJP Staking: cannot emmergency withdraw after 3 days");
        require(msg.sender == _stakeItem.beneficiary,"AJP Staking: Unauthorized claim");
        uint256 totalSupply = ajiraPayFinanceToken.balanceOf(address(this));
        uint256 amount = _stakeItem.stakingAmount;
        require(amount <= totalSupply,"AJP Staking: Insufficient Supply");
        _withdraw(msg.sender,amount);
        isStakeWithdrawn[msg.sender][_stakeItem.id] = true;
        totalStakeClaimed = totalStakeClaimed.add(amount);
        emit EmmergencyWithdraw(msg.sender, amount, block.timestamp);
    }
    ...
    function recoverLostTokensForInvestor(address _token, uint _amount) external onlyRole(MANAGER_ROLE) nonReentrant{
        ...
        emit RecoverERC20Tokens(msg.sender, _token, _amount, block.timestamp);
    }

    receive() external payable{
        revert("Cannot accept external native token");
    }
