...
contract CrowdMargin {
    ...
    function findUnfilledBlob(uint32 poolNumber, uint32 blobNumber) internal view returns (uint32) {
    	Blob memory currentBlob = poolBlobs[poolNumber][blobNumber];
    	if(currentBlob.right == 0) return blobNumber;

    	Blob memory wingBlob = poolBlobs[blobNumber][currentBlob.left];
    	if(wingBlob.right == 0) return currentBlob.left;

    	wingBlob = poolBlobs[blobNumber][currentBlob.right];
    	if(wingBlob.right == 0) return currentBlob.right;
    ...
    function addToPool(address addr, uint32 poolNumber, uint32 origBlobNumber) internal {
		...
		if(origBlobNumber != 0) {
			...
			if(currentBlob.up != 0) {
				if(!isPoolBlobClaimed[poolNumber][currentBlob.up]) {
					Blob memory upperBlob = poolBlobs[poolNumber][currentBlob.up];

					if(upperBlob.up != 0) {
						if(!isPoolBlobClaimed[poolNumber][upperBlob.up]) {
							blobNumber = findUnfilledBlob(poolNumber, upperBlob.up);
						}
						else {
							blobNumber = findUnfilledBlob(poolNumber, currentBlob.up);
						}
					}
					else {
						blobNumber = findUnfilledBlob(poolNumber, currentBlob.up);
					...
					blobNumber = findUnfilledBlob(poolNumber, origBlobNumber);
				}
			}
			else {
				blobNumber = findUnfilledBlob(poolNumber, origBlobNumber);
