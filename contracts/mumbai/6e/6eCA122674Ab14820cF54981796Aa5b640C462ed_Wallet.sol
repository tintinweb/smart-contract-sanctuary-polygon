...
abstract contract EIP4337Wallet is IEIP4337Wallet {
     ...
     */
    function validateSignature(UserOperation calldata userOp, bytes32 requestId, address aggregator) internal virtual;
...
contract Wallet is EIP4337Wallet, BaseWallet, Initializable, TransactionManager, SecurityManager, GuardianManager {
    ...
    function onlyOwner() internal view override(BaseWallet, EIP4337Wallet) {
        if (msg.sender != getOwner()) revert NotTheOwner();
    }

    function onlyEntryPoint() internal view override(BaseWallet, EIP4337Wallet) {
        require(msg.sender == address(entryPoint), "Wallet: caller must be an entrypoint");
    }

    function onlyOwnerOrEntryPoint() internal view override(BaseWallet) {
