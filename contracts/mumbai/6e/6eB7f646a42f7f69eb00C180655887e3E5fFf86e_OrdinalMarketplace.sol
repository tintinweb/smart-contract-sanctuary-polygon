...
import "./interfaces/IOrdinalMarketplace.sol";
...
contract OrdinalMarketplace is IOrdinalMarketplace, Ownable, ReentrancyGuard, Pausable {

    address constant NATIVE_TOKEN = address(1);

    modifier nonZeroAddress(address _address) {
        require(_address != address(0), "OrdinalMarketplace: address is zero");
        _;
    }
    ...
    function setTransferDeadline(uint _transferDeadline) public override onlyOwner {
        ...
        require(_transferDeadline > _finalizationParameter * 2, "OrdinalMarketplace: low deadline");
    ...
    function setProtocolFee(uint _protocolFee) public override onlyOwner {
        require(MAX_PROTOCOL_FEE >= _protocolFee, "OrdinalMarketplace: invalid fee");
    ...
    /// @param _bitcoinPubKey Bitcoin PubKey of the Ordinal holder (without starting '04'). 
    ...
    function listOrdinal(
        ...
        require(!ordinals[txId][_msgSender()].isListed, "OrdinalMarketplace: already listed");
        require(_bitcoinPubKey.length == 64 || _bitcoinPubKey.length == 0, "invalid pub key");
        ...
        if (_scriptType == ScriptTypes.P2TR) { 
            ...
            revert("OrdinalMarketplace: invalid type");
        }
        ...
        if (isSignRequired) {
            if (_scriptType == ScriptTypes.P2TR) {
                require(
                    _verifySchnorr(_convertToBytes32(scriptHash), txId, _r, _s, _v),
                    "OrdinalMarketplace: not ordinal owner"
                );
            } else {
                require(
                    _compareBytes(
                        scriptHash, _doubleHash(abi.encodePacked(FOUR, _bitcoinPubKey))
                    ),
                    "OrdinalMarketplace: wrong pub key"
                );
    ...
    function putBid(
        ...
        require(!ordinals[_txId][_seller].isSold, "OrdinalMarketplace: sold ordinal");
        ...
        if (_paymentToken == NATIVE_TOKEN) {
            require(msg.value == _amount, "OrdinalMarketplace: wrong value");
        } else {
    ...
    function revokeBid(
        ...
        if (bids[_txId][_seller][_bidIdx].isAccepted) {
            // check that deadline is passed but ordinal hasn't been transffered
            require(!ordinals[_txId][_seller].isSold, "OrdinalMarketplace: ordinal sold");
            require(
                IBitcoinRelay(relay).lastSubmittedHeight() > bids[_txId][_seller][_bidIdx].deadline,
                "OrdinalMarketplace: deadline not passed"
            );
    ...
    function acceptBid(bytes32 _txId, uint _bidIdx) external nonReentrant whenNotPaused override returns (bool) {
        require(!ordinals[_txId][_msgSender()].hasAccepted, "OrdinalMarketplace: already accepted");
        require(bids[_txId][_msgSender()].length > _bidIdx, "OrdinalMarketplace: invalid idx");  
    ...
    /// @param _outputOrdinalIdx Index of output that includes Ordinal
    ...
    function sellOrdinal(
        ...
        require(!ordinals[_txId][_seller].isSold, "OrdinalMarketplace: sold ordinal");
        require(bids[_txId][_seller][_bidIdx].isAccepted, "OrdinalMarketplace: not accepted");
        ...
        require(
            ...
            "OrdinalMarketplace: not finalized"
        );
        ...
        uint fee = _checkOrdinalTransferAndSendFunds(
            _txId, 
            _seller, 
            _bidIdx, 
            _transferTx.vout, 
    ...
    function _verifySchnorr(
        ...
        require(sp != 0, "OrdinalMarketplace: wrong sig");
        address R = ecrecover(sp, _v, _pubKeyX, ep);
        require(R != address(0), "OrdinalMarketplace: ecrecover failed");
        return _e == keccak256(
    ...
    function _checkScriptType(bytes memory _script, ScriptTypes _scriptType) private pure {
        if (_scriptType == ScriptTypes.P2PK || _scriptType == ScriptTypes.P2WSH || _scriptType == ScriptTypes.P2TR) {
            require(_script.length == 32, "OrdinalMarketplace: invalid script");
        } else {
            require(_script.length == 20, "OrdinalMarketplace: invalid script");
        }
    }
    ...
    function _ordinalIdx(
        ...
        for (uint i = 0; i < _inputTxs.length; i++) {
            ...
            require(
                ...
                "OrdinalMarketplace: outpoint != input tx"
            );
        ...
        require(
            ...
            "OrdinalMarketplace: outpoint not match with _txId"
        );
    ...
    /// @notice Checks that weather the Ordinal is transffered to buyer or not
    /// @param _txId of the Ordinal
    /// @param _seller Address of the seller
    /// @param _bidIdx Index of the accepted bid in bids list
    /// @param _vout output of transaction that transffred Ordinal from seller to buyer
    /// @param _outputOrdinalIdx Index of output that includes Ordinal
    /// @param _ordinalIdxInput Index of satoshi Ordinal in input of tx
    /// @param _lockerLockingScript Locker's locking script (it will be passed if the payment token is teleBTC)
    function _checkOrdinalTransferAndSendFunds(
        bytes32 _txId,
        address _seller,
        uint _bidIdx,
        bytes memory _vout,
        uint _outputOrdinalIdx,
        uint _ordinalIdxInput,
        bytes memory _lockerLockingScript
    ) internal returns (uint _fee) {
        // find number of satoshis before the output that includes the Ordinal
        uint outputValue;
        for (uint i = 0; i < _outputOrdinalIdx; i++) {
            outputValue += BitcoinHelper.parseOutputValue(_vout, i);
        }

        if (_outputOrdinalIdx != 0) {
            require(
                _ordinalIdxInput > outputValue,
                "OrdinalMarketplace: not transffered"
            );
        }

        require(
            _ordinalIdxInput <= outputValue + BitcoinHelper.parseValueFromSpecificOutputHavingScript(
                _vout,
                _outputOrdinalIdx,
        ...
        return _sendTokens(_txId, _seller, _bidIdx, _lockerLockingScript);
    ...
    function _isConfirmed(
        ...
        require(msg.value >= feeAmount, "OrdinalMarketplace: relay fee is not sufficient");
    ...
    function _bytesToAddress(bytes memory _data) public pure returns (address) {
        require(_data.length == 20, "OrdinalMarketplace: Invalid len");
        address addr;
        assembly {
    ...
    function _convertToBytes32(bytes memory _data) public pure returns (bytes32) {
        require(_data.length == 32, "OrdinalMarketplace: Invalid len");
        bytes32 result;
        assembly {
