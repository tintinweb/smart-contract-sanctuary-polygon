  ...
  EIP712Upgradeable
{
  ...
  bytes32 constant CALL_TYPEHASH =
    keccak256("Call(address target,uint256 value)");
  bytes32 constant CALL_INFO_TYPEHASH =
    keccak256("CallInfo(Call[] callList)Call(address target,uint256 value)");
  ...
  function _hashCall(Call calldata call) internal pure returns (bytes32) {
    return keccak256(abi.encode(CALL_TYPEHASH, call.target, call.value));
  }
  ...
  function multiCall(
    ...
    require(callLength > 0, "MultiCall: empty call list");

    address recoverdAddress = _domainSeparatorV4()
      .toTypedDataHash(_hashCallInfo(callInfo))
      .recover(sign);
    require(isAdmin(recoverdAddress), "MultiCall: invalid signature");
