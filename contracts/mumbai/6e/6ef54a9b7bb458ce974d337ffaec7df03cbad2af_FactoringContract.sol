            ...
            State.DefaultFinalPayment, State.Complete])
    {
        ...
        for (uint256 i = 0; i < t.balanceOf(msg.sender); i++) {
            ...
            for (uint256 j = roundsClaimed; j < _collectionRounds.length; j++) {
                ...
                totalOwedForTicket += (amountInvested * round.proceedsForBuyers) / totalSold;  // TODO: consider phantom overflow here.
            }
...
contract FactoringContractTickets is ERC721, ERC721Enumerable, AccessControl {
    ...
    {
        _tokenIds.increment();
        _safeMint(to, _tokenIds.current()); // TODO: fix reentrancy here
        ...
        returns (string memory)
    {
        ...
        string memory description = string(abi.encodePacked(
            _p.infoSellerName(), ' is selling ',
            uint256(_p.infoBasisPointsOffered()).toString(), ' basis points of "', _p.infoIncomeFlowDescription(),
            '" for ', _p.maximumFundraisingGoal().toString(), ' ', _p.infoCurrencySymbol(),
            '.\nThis ticket was purchased for ', t.amountInvested.toString(), ' ', _p.infoCurrencySymbol(),
