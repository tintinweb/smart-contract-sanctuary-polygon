...
contract CurdDistribution is Ownable {
	...
	event WithdrawnPoolUnallocatedToken(address _to, PoolType _pool, uint256 _amount);
	...
	function initialize(address vesting) external {
		...
		if (_token.balanceOf(address(this)) != 10 ** 9 * 10 ** 18)
			revert InsufficientBalance(10 ** 9 * 10 ** 18, _token.balanceOf(address(this)));
		...
		_addPool(PoolType.ECOSYSTEM, (100 * ((10 ** 6) * (10 ** 18))), 24, 12, 4);
	...
	function _addPool(
		...
		emit PoolAdded(_msgSender(), _pool, _allocation, _cliff, _vestingSchedule, _slice);
	}

	function withdrawUnallocatedToken(address _to) external onlyOwner isInitialized {
		if (_to == address(0x0)) revert ZeroAddress();

		uint256 balanceOfContract = _token.balanceOf(address(this));
		uint256 unTransferredTokens = getTotalAllocatedToken() - getTotalTokenWithdrawn();
	...
	function withdrawAvailableToken(address _to, uint256 _value, PoolType _pool) external onlyOwner isInitialized {
		if (_to == address(0x0)) revert ZeroAddress();
		if (_value == 0) revert ZeroAmount();

		uint256 poolBalance = _poolInfo[_pool].totalAmountAllocated - _poolInfo[_pool].userAllocatedAmount;

		if (_value > poolBalance) revert InsufficientBalance(_value, poolBalance);
	...
	function getTotalAllocatedToken() public view returns (uint256) {
		...
		for (uint8 i = 0; i <= uint8(type(PoolType).max); i++) {
			totalAllocatedCurdTokens += getAllocatedToken(CurdDistribution.PoolType(i));
		}
	...
	function getAvailableToken(PoolType _pool) public view returns (uint256) {
		return (_poolInfo[_pool].totalAmountAllocated - _poolInfo[_pool].userAllocatedAmount);
	}
	...
	function getDistribution(PoolType _pool) external view returns (PoolInfo memory) {
	...
	function recoverMatic() external payable onlyOwner {
		...
		(bool status, bytes memory _data) = address(this).call{value: contractBalance}("");
