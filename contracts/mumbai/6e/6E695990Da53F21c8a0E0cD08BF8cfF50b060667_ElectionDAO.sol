...
contract ElectionDAO is ERC721, ERC721URIStorage, KeeperCompatibleInterface {
    ...
    //Testing variables 
    uint256 public winnerIdtemp;
    uint256 public winningCandidatetemp;
 
    uint public maxSupply;
    uint public mintPrice = 0.02 ether; //change this to 1 ether when submit 
   ...
   // mapping (address => uint) public lastCandidate;
    ...
    event CandidateCreated(address _from, string _description, uint _id);
 
    event CandidateElected(bool _passed, address _to, uint _id);
 
    event VoteCast(address _from, uint256 _votesUp, uint256 _votesDown, bool _yes, uint _proposalid);
 
    event OwnershipTransferred(address _from, address _to);
 
    event ModUpdated(uint256 _term, string _uri, uint256 _tokenId);

    //DOUBLE _ VOTING _ PREVENTION
    ...
    function passTime(uint256 _tokenId) public {
        ...
        updateURI(_tokenId);
        emit ModUpdated(dAOCardAtrributes[_tokenId].term,  dAOCardAtrributes[_tokenId].imageURI,  _tokenId);
    ...
    //In the future TokenURI May Need To be off chain
        ...
        returns (string memory)
    {
        ...
        string memory strStakedValue = Strings.toString(daoAttributes.tokenStakeAmount);
    ...
    function enterCandidate (string memory _description,  uint256 _tokenID) public {
        ...
        emit CandidateCreated(msg.sender, _candidate.description, _candidate.id);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function getHighestVote() internal returns (uint256) {
            ...
            for (uint i = 0; i < s_candidateid.length; i++) {
                ...
                if(candidates[i].yesVotes >= largestValue){
                    getLog = candidates[i].yesVotes;
                    winningID = candidates[i].id;   
                }
                else if (candidates[i].yesVotes < largestValue){

                        largestValue = getLog;
                }  
            }
            candidates[winningID].passed = true;
            emit CandidateElected(candidates[winningID].passed , candidates[winningID].candidate, candidates[winningID].id);
            return winningID;   
    }

    //this is where we will either set a tie or delete the losers 
    function getLosers(uint256 _winningCandidate) internal {

          for (uint i = 0; i < s_candidateid.length; i++) {
                if(candidates[i].yesVotes < candidates[_winningCandidate].yesVotes){
    ...
    function calculateWinner() public returns (uint256) {
        ...
        uint256 winningCandidate = getHighestVote(); // first we get the winning ID by grabbing the largest value token
        winningCandidatetemp = getHighestVote();

        uint256 winnerId = candidates[winningCandidate].tokenId;
        winnerIdtemp = candidates[winningCandidate].tokenId;

        getLosers(winningCandidate);
        
        delete candidates[winningCandidate];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function voteOnCandidate(bool _yes, uint _candidateId, uint tokenID) public {
        ...
        //dAOCardAtrributes[tokenID].tokenWeight = calculateWeight(tokenID);
            ...
            //this is showing up as false on the output because we had it set to false im not sure why it was set to false but we have to redeploy after testing 
            emit VoteCast(msg.sender, candidates[_candidateId].yesVotes,  candidates[_candidateId].noVotes, _yes, _candidateId);
