...
contract NFTRenting is OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    */
    function listRentingNFT(address _NFTAddress, uint256 _NFTId, uint256 _rentalPrice) public onlyNFTOwner(_NFTId, _NFTAddress) whenNotPaused{   
        ...
        require(getUnclaimedFee(_NFTAddress, _NFTId) == 0, "[NFTRenting.listRentingNFT] Need to claim remaining rental fee first");
    ...
    */
    function executeRent(address _NFTAddress, uint256 _NFTId, uint256 _rentalPeriod) public whenNotPaused{
        ...
        RentingNFTs[_NFTAddress][_NFTId].rentalEnd = _rentalEnd;
        RentingDetailsbyOwnerOrderIds[NFT.owner][NFT.orderIdRentingNFT].rentalEnd = _rentalEnd;
    ...
    * @param _NFTId -  ID of prefered NFT.
    */
    function getUnclaimedFee(address _NFTAddress, uint256 _NFTId) public view returns(uint256) {
        RentingNFTDetail memory NFT = RentingNFTs[_NFTAddress][_NFTId];
        uint256 currentDate = (block.timestamp - (block.timestamp % 86400)) / 86400;
        uint256 startDate = (NFT.rentalStart - (NFT.rentalStart % 86400)) / 86400;
        uint256 endDate = startDate + NFT.rentalPeriod;
        uint256 lastClaimedDay = (NFT.lastClaimedTime - (NFT.lastClaimedTime % 86400)) / 86400;
        if (currentDate > endDate) {
            currentDate = endDate;
        }
        uint256 dayPassed = currentDate - lastClaimedDay;
        uint256 totalFee = dayPassed * NFT.rentalPrice;
        return totalFee;
    }
    ...
    */
    function claimFeeRenting(address _NFTAddress, uint256 _NFTId) public onlyNFTOwnerANDadmin(_NFTId, _NFTAddress) whenNotPaused{
        ...
        uint256 unclaimedFee = getUnclaimedFee(_NFTAddress, _NFTId);
        uint256 platformFee = (getUnclaimedFee(_NFTAddress, _NFTId) * platformCommission) / 100;
