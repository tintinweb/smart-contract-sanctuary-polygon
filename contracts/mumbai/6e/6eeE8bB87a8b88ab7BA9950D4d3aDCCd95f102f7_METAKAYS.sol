...
contract METAKAYS is Ownable, ERC721, NonblockingReceiver {
    ...
    uint256 nextTokenId = 9008;
    uint256 MAX_MINT_MATIC = 9068;
    ...
    function mint() external payable {
        require(nextTokenId < MAX_MINT_MATIC, "SUPPLY LIMIT REACHED!");
    ...
    function CREATE(bytes memory artData, bytes memory colorData, bytes memory colorData2, uint256[4] memory colors) internal view returns (string memory) {
        bytes memory karmics = DynamicBuffer.allocate(2**16);
        for (uint i = 0; i < 512; i+=8) {
            ...
            for (uint256 ii=0; ii < 8; ii++) {
                if ((workingByte >> (7 - ii)) & 1 == 1) {
                    karmics.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
                        toString((y*50)+25),'" fill="',svgBackgroundColor[colors[0]]));
                } else {
                    if ((colorByte >> (7 - ii)) & 1 == 1) {
                        karmics.appendSafe(abi.encodePacked('<circle r="25"  cx="',toString((x*50) + (ii*50) +25),'" cy="',
                            toString((y*50)+25),'" fill="',svgBackgroundColor[colors[1]]));
                    } else {
                        if ((colorByte2 >> (7 - ii)) & 1 == 1) {
                            karmics.appendSafe(abi.encodePacked('<circle r="25"  cx="',toString((x*50) + (ii*50) +25),'" cy="',
                                toString((y*50)+25),'" fill="',svgBackgroundColor[colors[2]]));
                    } else {
                        karmics.appendSafe(abi.encodePacked('<circle r="25" cx="',toString((x*50) + (ii*50) +25),'" cy="',
                            toString((y*50)+25),'" fill="',svgBackgroundColor[colors[3]]));
                        }
                    }
                }
            }
         }
      return string(karmics);
    }
