    ...
    ERC2981
{
    ...
    mapping(string => bool) trustedRemotes;

    event RemoteStatusChanged(string chain, bool newStatus);
    ...
    ) ERC721("MyToken", "MTK") AxelarExecutable(gateway_) {
    ...
    function safeMint(
        ...
        //only backend can mint onlyMinter
    ...
    mapping(string => uint256) nameOfNetworkToChainId;
     ...
     * @param _chain Name of chain accepted for transferRecemote and execute
     * @param _status Is chain accepted for transferRemote and execute
     */
    function setTrustedRemote(string calldata _chain, bool _status) external onlyRole(ADMIN_ROLE) {
        trustedRemotes[_chain] = _status;
        emit RemoteStatusChanged(_chain, _status);
    }

    function setMetadata(uint256 tokenId, string calldata uri) public {
        _setTokenURI(tokenId, uri);
        emit TokenMetadataChanged(tokenId, uri);
    }
    ...
    //we need to lock token on opensea
    function tokenURI(uint256 tokenId) public view override(ERC721, ERC721URIStorage) returns (string memory) {
        //add lock
        require(!lockedTokens[tokenId], "TokenLocked");
