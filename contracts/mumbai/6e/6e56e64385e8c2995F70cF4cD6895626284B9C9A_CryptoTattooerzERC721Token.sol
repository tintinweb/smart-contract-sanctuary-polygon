...
error CTZ_ERC721_CapIsZero();
/** Cannot mint more than max cap */
error CTZ_ERC721_CapExceeded();
/** Cannot find token with given ID */
error CTZ_ERC721_NonexistentToken(uint256 tokenID);
/** Cannot mint token with given ID */
error CTZ_ERC721_ExistentToken(uint256 tokenID);
/** User not allowed to burn a specific token */
error CTZ_ERC721_BurnNotAllowed(address user, uint256 tokenID);
/** Cannot manually mint when mint is automatic */
error CTZ_ERC721_MintIsAuto();
/** Cannot automatically mint when mint is manual */
error CTZ_ERC721_MintIsManual();
/** Cannot automatically mint when mint is not ready */
error CTZ_ERC721_MintIsNotReady();

/**
 * @title This is the CryptoTattooerz ERC721 token contract.
 ...
 * @author tazous
 */
contract CryptoTattooerzERC721Token is TokenDataHandlerProxy, RoyaltyImplementerProxy, ERC721Enumerable, PausableImpl {
    ...
    ERC721(name_, symbol_) ProxyDiamond(proxyHubAddress_) TokenDataHandlerProxy(dataHandlerAddress_) RoyaltyImplementerProxy(royaltyHandlerAddress_) {
        if(cap_ == 0) revert CTZ_ERC721_CapIsZero();
        cap = cap_;
    ...
    function tokenURI(uint256 tokenID) public view virtual override returns (string memory) {
        if(!_exists(tokenID)) revert CTZ_ERC721_NonexistentToken(tokenID);
    ...
    function addTokenID2Mint(uint256 tokenID) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        if(_tokenIDs2Mint.length() >= mintableSupply()) revert CTZ_ERC721_CapExceeded();
        // Mint is manual as no defined token ID to be minted and mint already started
        if(_tokenIDs2Mint.length() == 0 && totalSupply() + burnedSupply() > 0) revert CTZ_ERC721_MintIsManual();
        // Token ID has already been minted
        if(_exists(tokenID)) revert CTZ_ERC721_ExistentToken(tokenID);
    ...
    /**
     * @dev This method is to be used if NFT mint is to be done "automatically", ie randomly chosen inside a predefined
     * list of token IDs to be minted. This list should be considered complete before the first mint and automatic minting
     * is not consistent with manual minting. Once one is chosen, minting should always be done the same way in order to
     * stay consistent with chosen policy
     * @param to Address of the future owner of the NFT about to be randomly chosen and minted
     */
    function mintTo(address to) external onlyRole(MINTER_ROLE) {
        // Available NFTs total number
        uint256 nftsNb = _tokenIDs2Mint.length();
        // NFT contract should be consistent with mint parameters
        if(nftsNb != mintableSupply()) {
            // Mint has been started and is manual
            if(totalSupply() + burnedSupply() > 0) revert CTZ_ERC721_MintIsManual();
            // Mint is "auto" but not all token IDs have been added or will be "manual" and not yet started
            revert CTZ_ERC721_MintIsNotReady();
        }
        // No more NFT to be minted
        if(nftsNb == 0) revert CTZ_ERC721_CapExceeded();
    ...
    /**
     * @dev This method is to be used if NFT mint is to be done "manually", ie explicitly chosen by the caller. Manual
     * minting is not consistent with automatic minting. Once one is chosen, minting should always be done the same way
     * in order to stay consistent with chosen policy
     * @param to Address of the future owner of the NFT about to be manually chosen and minted
     */
    function safeMint(address to, uint256 tokenID_) external onlyRole(MINTER_ROLE) {
        // NFT contract should be consistent with mint parameters, Mint is "auto"
        if(_tokenIDs2Mint.length() != 0) revert CTZ_ERC721_MintIsAuto();
        super._safeMint(to, tokenID_);
    }

    function burn(uint256 tokenID) external {
        if(!hasRole(BURNER_ROLE, _msgSender()) && _msgSender() != ownerOf(tokenID)) {
            revert CTZ_ERC721_BurnNotAllowed(_msgSender(), tokenID);
        }
        _burn(tokenID);
    }

    /**
     * @dev Redefine low-level _mint function if order to validate maximum cap and check that is case of automatic mint,
     * the token ID is one of the defined IDs to be minted
     */
    function _mint(address to, uint256 tokenID) internal virtual override {
        if(mintableSupply() == 0) revert CTZ_ERC721_CapExceeded();
        // All mintable supply should come from predefined list in case of automatic minting
        if(_tokenIDs2Mint.length() != 0 && !_tokenIDs2Mint.contains(tokenID)) revert CTZ_ERC721_NonexistentToken(tokenID);
        // NFT about to be minted should be removed from the list of available ones if present
        _tokenIDs2Mint.remove(tokenID);
