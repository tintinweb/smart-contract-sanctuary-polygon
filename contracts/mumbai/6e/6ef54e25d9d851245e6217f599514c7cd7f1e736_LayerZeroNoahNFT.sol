...
// File interfaces/ILayerZeroReceiver.sol
...
// File interfaces/ILayerZeroUserApplicationConfig.sol
...
// File interfaces/ILayerZeroEndpoint.sol
...
// File contracts/LayerZeroNoahNFT.sol
...
contract LayerZeroNoahNFT is ERC721Enumerable, NonblockingReceiver, ILayerZeroUserApplicationConfig {
    using Strings for uint256;
    ...
    function reserveNoahs() public onlyOwner {
        for(uint256 i = 0; i < 20; i++) {
            _safeMint(msg.sender, i+1);
        }     
    }

    function crossChains(
        uint16 _chainId,
        uint256 tokenId
    ) public payable {
        require(msg.sender == ownerOf(tokenId), "Message sender must own the NFT.");
