...
contract Vote is ReentrancyGuard {
    ...
    function initializeVote(
        ...
        require(!isInitialized, "initializeVote: Already initialized vote"); // TODO(don't need anymore)
    ...
    function approveFilms(uint256[] memory _filmIds) external onlyAuditor {
        for(uint256 i = 0; i < _filmIds.length; i++) {
            ...
            if(filmVoting[_filmIds[i]].voteCount < IProperty(DAO_PROPERTY).minVoteCount()) continue;
    ...
    function replaceAuditor(uint256 _agentIndex) external onlyStaker nonReentrant {
        ...
        require(_agentVoting.voteCount >= IProperty(DAO_PROPERTY).minVoteCount(), "replaceAuditor: Less than minimum vote count");
    ...
    function addFilmBoard(address _member) external onlyStaker nonReentrant {
        ...
        require(fbp.voteCount >= IProperty(DAO_PROPERTY).minVoteCount(), "addFilmBoard: Less than minimum vote count");
    ...
    function setDAORewardAddress(address _rewardAddress) external onlyStaker nonReentrant {
        ...
        require(rav.voteCount >= IProperty(DAO_PROPERTY).minVoteCount(), "addRewardAddress: Less than minimum vote count");
    ...
    function updateProperty(uint256 _propertyIndex, uint256 _flag) external onlyStaker nonReentrant {
        ...
        require(_propertyVoting.voteCount >= IProperty(DAO_PROPERTY).minVoteCount(), "updateProperty: Less than minimum vote count");
