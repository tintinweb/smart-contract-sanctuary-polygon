    ...
    function _doCalls(
        ...
        for (uint256 i = 0; i < calls.length; i++) {
            ...
            if (calls[i].inToken.isETH()) {
                (bool succeeded, bytes memory returndata) = calls[i].target.call{value: calls[i].amount}(
                    abi.encodePacked(
                        calls[i].selector,
                        calls[i].data
                    )
                ...
                if (!succeeded) {
                    if (returndata.length < 68) revert("ETH Subswap failed");
                    assembly {
                        returndata := add(returndata, 0x04)
                    }
                    revert(string.concat("ETH Subswap failed: ", abi.decode(returndata, (string))));
                }
            } else {
                ...
                (bool succeeded, bytes memory returndata) = calls[i].target.call(
                    abi.encodePacked(
                        calls[i].selector,
                        calls[i].prefix,
                        subswapInAmount,
                ...
                if (!succeeded) {
                    if (returndata.length < 68) revert("Subswap failed");
                    assembly {
                        returndata := add(returndata, 0x04)
                    }
                    revert(string.concat("Subswap failed: ", abi.decode(returndata, (string))));
                }
            }
        }
    }
