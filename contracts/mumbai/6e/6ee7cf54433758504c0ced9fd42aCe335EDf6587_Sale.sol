	...
	ReentrancyGuardUpgradeable
{
	...
	modifier isPublicSale() {
		if (!phases[currentPhaseIndex].isPublic) {
	...
	function buyToken(uint256 _tokenAmount)
		...
		require(
			block.timestamp < phases[currentPhaseIndex].endAt,
			"This phase is over, try next"
		);

		require(
			phases[currentPhaseIndex].supply >= _tokenAmount,
			"Not enough supply"
		);
		require(
			block.timestamp < phases[currentPhaseIndex].endAt,
		...
		require(
			_tokenAmount >= phases[currentPhaseIndex].minimunEntry,
			"There are too few tokens"
		);

		uint256 finalPrice =
			(_tokenAmount * phases[currentPhaseIndex].price) / (getLatestPrice());
		...
		id++;
		if (phases[currentPhaseIndex].timelock > 0) {
			...
			getTimeToPresent(phases[currentPhaseIndex].timesToRelease);
			tokenLocksForSale[id].percentsToRelease = phases[currentPhaseIndex]
				.percentsToRelease;
		} else {
		...
		if (phases[currentPhaseIndex].supply == 0) {
			phases[currentPhaseIndex].over = true;
			emit PhaseOver(true,currentPhaseIndex);
		}
	...
	function createPhase(
		...
		currentPhaseIndex++;
		phases[currentPhaseIndex] = p;

		emit PhaseCreated(_isPublic, _minimunEntry, _price, _endAt, _supply);
	}
	///@notice add a list of wallet to vesting
	function addUsersToVesting(uint256[] memory _tokenAmount,address[] memory _address) external onlyRole(DEFAULT_ADMIN_ROLE) {
			require(_tokenAmount.length==_address.length,"should be equal users and quantity");
			uint totalQuantity=0;
			for (uint i=0;i<_tokenAmount.length;i++) {
		...
		require(
			phases[currentPhaseIndex].supply >= totalQuantity,
			"Not enough supply"
		);
			...
			getTimeToPresent(phases[currentPhaseIndex].timesToRelease);
			tokenLocksForSale[id].percentsToRelease = phases[currentPhaseIndex].percentsToRelease;
			phases[currentPhaseIndex].supply -= _tokenAmount[i];
		...
		emit AddUsersToVesting(currentPhaseIndex,_tokenAmount,_address);
	}
	...
	function cancelPhase() external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(
			phases[currentPhaseIndex].over == false,
			"This phase is over,  try next"
		);
		if (phases[currentPhaseIndex].supply > 0) {
			ERC20(tokenAddress).transfer(
				dispatcher,
				phases[currentPhaseIndex].supply
			);
			phases[currentPhaseIndex].supply = 0;
		}
		phases[currentPhaseIndex].over = true;
		emit PhaseOver(true,currentPhaseIndex);
	}
