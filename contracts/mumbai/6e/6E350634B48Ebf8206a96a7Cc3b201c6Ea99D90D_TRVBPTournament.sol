...
contract TRVBPTournament is TRVBPWarrior, AdminControl, ReentrancyGuard {
  ...
  constructor() AdminControl() {
    ...
    setupRole(CANCEL_ROLE, 0x5F06CE9F9aB1be5FE3b5B84359FaC47ab75b43A3);
    setupRole(SET_WINNER_ROLE, 0x5F06CE9F9aB1be5FE3b5B84359FaC47ab75b43A3);

    setMaxFightingPerChamp(0, 10999, 100);

    setClass(0, 1999, CLASSES.BLOODING);
    setFee(0, 1999, 0.1 ether);
    setPayout(0, 1999, 0);
    setSize(0, 1999, 8);

    setClass(2000, 2999, CLASSES.CLASS_3);
    setFee(2000, 2999, 1 ether);
    setPayout(2000, 2999, 0);
    setSize(2000, 2999, 4);

    setClass(3000, 3999, CLASSES.CLASS_2);
    setFee(3000, 3999, 10 ether);
    setPayout(3000, 3999, 0);
    setSize(3000, 3999, 4);

    setClass(4000, 4999, CLASSES.CLASS_1);
    setFee(4000, 4999, 20 ether);
    setPayout(4000, 4999, 0);
    setSize(4000, 4999, 4);

    setClass(5000, 5999, CLASSES.BLOODBATH);
    setFee(5000, 5999, 1 ether);
    setPayout(5000, 5999, 0);
    setSize(5000, 5999, 8);
  }
  ...
  function eligibleJoin(uint256 _tournamentId, uint256 _warriorId) public view returns (bool, string memory) {
    if (tournaments[_tournamentId].state != TournamentState.AVAILABLE) {
      return (false, "Tournament not available");
    }
    (bool feeSuccess, ) = getFee(_tournamentId);
    if (!feeSuccess) {
      return (false, "Tournament Fee not available");
    }
    (bool sizeSuccess, ) = getSize(_tournamentId);
    if (!sizeSuccess) {
      return (false, "Tournament Size not available");
    }
    (bool classSuccess, uint256 class) = getClass(_tournamentId);
    if (!classSuccess) {
      return (false, "Tournament Class not available");
    }
