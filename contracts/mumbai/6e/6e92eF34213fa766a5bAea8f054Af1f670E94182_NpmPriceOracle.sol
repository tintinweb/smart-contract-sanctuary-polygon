...
contract NpmPriceOracle is IPriceOracle {
  ...
  uint256 public immutable stablecoinMultiplier;
  ...
  constructor(IUniswapV2Pair _pair, address _npm) public {
    ...
    uint8 stablecoinDecimals = ERC20(_pair.token0()).decimals();

    if (_pair.token0() == _npm) {
      stablecoinDecimals = ERC20(_pair.token1()).decimals();
    }

    stablecoinMultiplier = 10**uint256(stablecoinDecimals);
  ...
  function _calculateFairLpPrice(
    ...
    uint256 p = Babylonian.sqrt(p0.mul(stablecoinMultiplier));

    return uint256(2).mul(r).mul(p).mul(amountIn).div(supply.mul(stablecoinMultiplier));
  }
}
