...
contract GameRegistry is OwnableUpgradeable {
   ...
   * @param _gid Game ID
   */
  function removeGame(uint256 _gid) external  onlyActiveGame(_gid) {
    // remove game
   ...
   * @param _baseGameCreatorFee Base game creator fee
   */
  function updateBaseGameCreatorFee(uint256 _gid, uint256 _baseGameCreatorFee) external  onlyActiveGame(_gid) {
  ...
  function updateDepositTokenAmount(
    ...
    address _token,
    uint256 _amount
  ) external  onlyActiveGame(_gid) onlyValidTID(_gid, _tid) {
