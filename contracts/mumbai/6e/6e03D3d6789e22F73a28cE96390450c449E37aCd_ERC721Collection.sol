...
contract ERC721Collection is AccessControl, ERC721, ReentrancyGuard {
  string private contractUri;
  string private baseUri;
  bytes32 public immutable OPERATOR_ROLE;
  ...
  function setURI(string memory newuri) public onlyAdmin {
    if (bytes(newuri).length == 0) revert NullString();
    baseUri = newuri;
  }
  ...
  function setContractURI(string memory _contractUri) public onlyAdmin {
    if (bytes(_contractUri).length == 0) revert NullString();
    contractUri = _contractUri;
  }

  /**
   * @dev Public function to lazy mint a single new token and transfer it to buyer
   * Reverts if the caller is not an administrator
   * @param from The address to which the newly minted tokens will be assigned.
  ...
  function redeemNFT(
    ...
    if (bytes(_tokenUri).length == 0) revert NullString();
    if (from == address(0) || to == address(0)) revert NullAddress();
    _safeMint(from, id);
    if (!isApprovedForAll(from, address(this)))
      setApprovalForAll(from, address(this), true);
    tokenUris[id] = _tokenUri;
  ...
  function burnNFT(address from, uint256 id) external nonReentrant {
    if (_msgSender() != from && !hasRole(OPERATOR_ROLE, _msgSender()))
  ...
  function safeTransferFrom(
    ...
    if (from == address(0) || to == address(0)) revert NullAddress();
    if (_msgSender() != from && !_isApprovedOrOwner(_msgSender(), id))
      revert Unapproved();
    _safeTransfer(from, to, id, '');
  }
   ...
   * @param operator The address which will send the tokens.
  ...
  function setApprovalForAll(
    ...
    if (_msgSender() != account && !hasRole(OPERATOR_ROLE, _msgSender()))
      revert NotOwnerOrOperator();
