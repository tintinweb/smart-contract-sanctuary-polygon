...
contract ChainlinkFeedGetter is IPrimeOracleGetter {
    ...
    mapping(uint256 => mapping(IERC20 => AggregatorInterface)) private assetFeeds;
    ...
    **/
    ...
    /// @param denomCurrencyParam the denom currency used for the price quotes. If USD is used, denom currency is 0x0
    /// @param denomCurrencyUnitParam the unit of the denom currency
    constructor(
        ...
        emit DenomCurrencySet(denomCurrency, denomCurrencyUnit);
    }

    /// @inheritdoc IPrimeOracleGetter
    function getAssetPrice(uint256 chainId, IERC20 asset, IERC20) external view override returns (uint256) {
        AggregatorInterface feed = assetFeeds[chainId][asset];
    ...
    function _getAssetFeed(uint256 chainId, IERC20 asset) internal view returns (address){
        return address(assetFeeds[chainId][asset]);
    }

    function getAssetFeed(uint256 chainId, IERC20 asset) external view override returns (address)
    {
        return _getAssetFeed(chainId, asset);
    }

    /// @inheritdoc IPrimeOracleGetter
    function setAssetFeeds(uint256[] calldata chainIds, IERC20[] calldata assets, address[] calldata feeds)
        external
        override
        onlyAdmin
    {
    ...
    * @param feeds The address of the feed of each asset
    */
    function _setAssetFeeds(uint256[] memory chainIds, IERC20[] memory assets, address[] memory feeds) internal {
        ...
        for (uint256 i; i < assets.length; i++) {
            assetFeeds[chainIds[i]][assets[i]] = AggregatorInterface(feeds[i]);
