...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
    ...
    function _executeTrade(
        ...
        if (_isClose) {
            require(
                _getLockedInValue(_perpetual, _traderAddr) != 0,
    ...
    function _preTrade(
        ...
        if (closePos) {
            ...
            _fAmount = _fAmount < 0
                ? _fAmount.sub(_perpetual.fLotSizeBC)
                : _fAmount.add(_perpetual.fLotSizeBC);
        } else {
    ...
    {
        require(
            _perpetual.fCurrentTraderExposureEMA > 0,
        ...
        return
            isLong
                ? _perpetual.fCurrentTraderExposureEMA.mul(scale)
                : _perpetual.fCurrentTraderExposureEMA.mul(scale).neg();
    }
        ...
        internal
    {
        // (neg) AMM exposure (aggregated trader exposure)
        {
            ...
            int128 fLambda = fCurrentObs.abs() > _perpetual.fCurrentAMMExposureEMA[iIndex].abs()
                ? _perpetual.fDFLambda[1]
                : _perpetual.fDFLambda[0];
            if (fCurrentObs.abs() < _perpetual.fMinimalAMMExposureEMA) {
                fCurrentObs = iIndex == 0
                    ? _perpetual.fMinimalAMMExposureEMA.neg()
                    : _perpetual.fMinimalAMMExposureEMA;
            }
            int128 tmp = _getAMMPerpLogic().ema(
                _perpetual.fCurrentAMMExposureEMA[iIndex],
                fCurrentObs,
                fLambda
            );
        ...
        // trader exposure
        {
            ...
            int128 fLambda = fCurrentObs > _perpetual.fCurrentTraderExposureEMA.abs()
                ? _perpetual.fDFLambda[1]
                : _perpetual.fDFLambda[0];
            if (fCurrentObs < _perpetual.fMinimalTraderExposureEMA) {
                fCurrentObs = _perpetual.fMinimalTraderExposureEMA;
            }
            _perpetual.fCurrentTraderExposureEMA = _getAMMPerpLogic().ema(
    ...
    function _transferProtocolFee(
        ...
        (fAMMFundContribution, fDefaultFundContribution) = _calculateContributions(
            _liqPool,
            _fTreasuryFee
        );
        ...
        if (_liqPool.fPnLparticipantsCashCC != 0) {
            ...
            fAMMFundContribution = fAMMFundContribution.add(_fPnLparticipantFee);
        }

        _liqPool.fDefaultFundCashCC = _liqPool.fDefaultFundCashCC.add(fDefaultFundContribution);
        emit UpdateDefaultFundCash(
            _liqPool.id,
            fDefaultFundContribution,
            _liqPool.fDefaultFundCashCC
        );

        // contribution to AMM pool is recorded in perpetual and the aggregated amount in the liq-pool
        _liqPool.fAMMFundCashCC = _liqPool.fAMMFundCashCC.add(fAMMFundContribution);
        _perpetual.fAMMFundCashCC = _perpetual.fAMMFundCashCC.add(fAMMFundContribution);
        ...
        returns (int128)
    {
        ...
        // funding status
        int128 fMinSpread = int128(_perpetual.fMinimalSpread) << 35;
        int128 fIncentiveSpread = int128(_perpetual.fIncentiveSpread) << 35;
        return
     ...
     */
    function _calculateContributions(LiquidityPoolData storage _pool, int128 _fTreasuryFee)
        internal
        view
        returns (int128, int128)
    {
        ...
        if (fAMMFundCashNew > _pool.fTargetAMMFundSize) {
            ...
            if (fDefaultFundGap > 0) {
                ...
                int128 fDefaultFundContribution = fAMMFundGap > fDefaultFundGap
                    ? fDefaultFundGap
                    : fAMMFundGap;
    ...
    {
        PerpStorage.MarginAccount storage mgn = marginAccounts[_perpId][_trader];

        fBrokerFee = !_hasBroker
            ? int128(0)
            : ConverterDec18.tbpsToABDK(mgn.brokerFeeTbps).mul(_fDeltaPosCC);
        (fTreasuryFee, fPnLparticipantFee) = _getBrokerFeeLogic().splitProtocolFee(mgn.feeTbps);
...
contract PerpetualUpdateFunctions is PerpetualBaseFunctions {
    ...
    function _updateDefaultFundTargetSize(uint24 _iPerpetualId) internal {
        ...
        if (
            uint64(block.timestamp) - perpetual.iLastTargetPoolSizeTime >
            uint64(liquidityPool.iTargetPoolSizeUpdateTime) &&
    ...
    function _getUpdatedTargetAMMFundSize(
        ...
        if (_pool.fPnLparticipantsCashCC > 0) {
            int128 fPricingPnLCashCC = _pool.fPnLparticipantsCashCC.mul(
                int128(_pool.fPricingCashRatio) << 35
            );
            // if pricing cash in PnL fund already covers the target, no more cash is needed from the AMM fund
            fMStar = fMStar > fPricingPnLCashCC ? fMStar.sub(fPricingPnLCashCC) : int128(0);
        }
        return fMStar;
    }
    ...
    /**
     * Check whether oracle is terminated. If so set emergency state.
    ...
    {
        if (uint64(block.timestamp) > 60 + _perpetual.iPriceUpdateTimeSec) {
    ...
    /**
     * Set the state of the perpetual to "EMERGENCY".
    ...
    function _setNormalState(PerpetualData storage _perpetual) internal {
        ...
        // increment count of normal perps
        LiquidityPoolData storage pool = liquidityPools[_perpetual.poolId];
        pool.iNormalPerpetualCount++;
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
    ...
    function _rebalanceToTarget(PerpetualData storage _perpetual) internal {
        ...
        if (uint64(block.timestamp) <= _perpetual.iLastDefaultFundTransfer) {
            ...
            fMaxTransfer = ABDKMath64x64
                ...
                .div(ABDKMath64x64.fromUInt(uint256(pool.iFundTransferConvergenceHours) * 3600));
            // scale can't be more than 100%
            fMaxTransfer = fMaxTransfer > ONE_64x64 ? ONE_64x64 : fMaxTransfer;
            // apply scale to smaller between target size (total size, not gap) and max total transfer in period
            fMaxTransfer = fMaxTransfer.mul(
                fStressTarget > pool.fMaxTransferPerConvergencePeriod
                    ? pool.fMaxTransferPerConvergencePeriod
                    : fStressTarget
            );
    ...
    function _decreasePoolCash(LiquidityPoolData storage _pool, int128 _fAmount) internal {
        ...
        require(_pool.fPnLparticipantsCashCC >= 0, "partc fund cash !>0");

        // increase AMM fund cash in case virtual PnL funds were not sufficient
        if (_pool.iNormalPerpetualCount > 0 && fPricingCashCC > _pool.fPnLparticipantsCashCC) {
            // amount to be transferred from default fund to AMM fund
            int128 fAMMGapToFill = fPricingCashCC.sub(_pool.fPnLparticipantsCashCC);
            require(fAMMGapToFill < _pool.fDefaultFundCashCC, "withdrwl price impact");

            // transfer bulk first
            _pool.fDefaultFundCashCC = _pool.fDefaultFundCashCC.sub(fAMMGapToFill);
            _pool.fAMMFundCashCC = _pool.fAMMFundCashCC.add(fAMMGapToFill);

            // record transfer as equal fractions into each (normal) perp
            uint256 length = _pool.iPerpetualCount;
            fAMMGapToFill = fAMMGapToFill.div(int128(uint128(_pool.iNormalPerpetualCount) << 64));
            for (uint256 i = 0; i < length; i++) {
                uint24 idx = perpetualIds[_pool.id][i];
                PerpetualData storage perpetual = perpetuals[_pool.id][idx];
            ...
            fPricingCashCC = _pool.fPnLparticipantsCashCC;
        }

        // update pricing PnL fund cash ratio
        if (_pool.fPnLparticipantsCashCC == 0) {
            // no cash in pool, no pricing cash
            _pool.fPricingCashRatio = 0;
        } else if (
            _pool.fPnLparticipantsCashCC < _pool.fBrokerCollateralLotSize.div(0x0a0000000000000000)
        ) {
            // thenth of broker lot used as reference for a small amount of cash that can always be used for pricing
            _pool.fPricingCashRatio = int32(ONE_64x64 >> 35);
        } else {
            // otherwise the actual ratio
            _pool.fPricingCashRatio = int32(
                fPricingCashCC.div(_pool.fPnLparticipantsCashCC) >> 35
            );
        }
    ...
    function _updatePricingToTotalCashRatio(LiquidityPoolData storage _pool) internal {
        ...
        if (_pool.fPnLparticipantsCashCC == int128(0)) {
            _pool.fPricingCashRatio = 0;
        ...
        int128 fScale = ABDKMath64x64
            ...
            .div(ABDKMath64x64.fromUInt(uint256(_pool.iFundTransferConvergenceHours) * 3600));
        // a priori, the target is the max amount of non-protocol owned funds that we can use for pricing, 75% of DF
        int128 fTargetPricingCashCC = _pool.fDefaultFundCashCC.mul(CEIL_AMT_FUND_WITHDRAWAL);
        // we reduce it to the actual amount available in the fund
        ...
        // to avoid numerical issues, enforce P <= total LP
        fPricingCashCC = fPricingCashCC > _pool.fPnLparticipantsCashCC
            ? _pool.fPnLparticipantsCashCC
            : fPricingCashCC;
        // should not increase/decrease funds by more than the gap to target
        int128 fMaxDeltaCashCC = fTargetPricingCashCC.sub(fPricingCashCC).abs();
        // and should not move more funds than allowed by the convergence period
        {
            int128 fMaxDeltaCashGlobal = fScale.mul(_pool.fMaxTransferPerConvergencePeriod);
            fMaxDeltaCashCC = fMaxDeltaCashCC > fMaxDeltaCashGlobal
                ? fMaxDeltaCashGlobal
                : fMaxDeltaCashCC;
        }
        ...
        if (fPricingCashCC > fTargetPricingCashCC) {
            ...
            fDeltaP = fDeltaP > fMaxDeltaCashCC ? fMaxDeltaCashCC.neg() : fDeltaP.neg();
            ...
            fDeltaP = fDeltaP > fMaxDeltaCashCC ? fMaxDeltaCashCC : fDeltaP;
        }
        _pool.fPricingCashRatio = int32(
            fPricingCashCC.add(fDeltaP).div(_pool.fPnLparticipantsCashCC) >> 35
        );
        _pool.iLastPricingCashUpdate = uint64(block.timestamp);
        emit PricingLiquidityUpdated(_pool.id, fPricingCashCC, _pool.fPnLparticipantsCashCC);
    }
