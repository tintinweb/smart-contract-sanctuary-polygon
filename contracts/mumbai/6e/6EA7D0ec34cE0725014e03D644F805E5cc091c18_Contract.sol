...
contract Contract is ERC721Base {
    ...
    function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal override {
        tokenMetadata[_tokenId].image = _tokenURI;
    }

    function mintTokenWithCompressedSvg(address to, string memory compressedBase64) public {
        uint256 tokenId = _nextTokenIdToMint;

        _safeMint(to, tokenId);

        TokenMetadata memory metadata;
        metadata.name = "OpenTitled";
        metadata.description = "OpenTitled";
        metadata.image = compressedBase64;
        metadata.attributes = new string[](1);
        metadata.attributes[0] = "rarity: common";

        tokenMetadata[tokenId] = metadata;

        string memory metadataJson = generateMetadataJson(metadata);
        string memory metadataBase64 = Base64.encode(bytes(metadataJson));
        string memory finalCompressedBase64 = string(abi.encodePacked("data:application/json;base64,", compressedBase64, metadataBase64));

        _setTokenURI(tokenId, finalCompressedBase64);

        _nextTokenIdToMint++;
    }

    function generateMetadataJson(TokenMetadata memory metadata) internal pure returns (string memory) {
        string memory attributesJson = generateAttributesJson(metadata.attributes);
