...
contract shareTokenCx is Ownable {
    ...
    function setShare(IERC20 _token, uint256 _claimTime, uint256 _shareNumber) external onlyOwner {
        uint256 _shareAmount = getLeftToken(_token);
    ...
    function getLeftToken(IERC20 _token) public view returns (uint256) {
        if (_token.balanceOf(address(this)).add(tokenList[_token].claimedAmount) > tokenList[_token].sharedAmount) {
    ...
    function takeWrongToken(IERC20 _token, uint256 _amount) external onlyOwner {
        if (_amount == 0) {
            ...
            require(_amount <= _token.balanceOf(address(this)), "e008");
        }
