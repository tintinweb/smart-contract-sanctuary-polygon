...
contract SimpleRoyalties is IERC2981, ERC165 {
    ...
    constructor(address _recipient, uint256 _amount) {
        ...
        require(
            _amount == 0 || _recipient != address(0),
...
// File: ViciERC721WithTransferFee.sol

contract ViciERC721WithTransferFee is
    BaseViciContract,
    RoyaltiesClient,
    Recallable,
    ContextMixin
{
     ...
     * owner can "recall" the NFT.
     */
    function maxRecallPeriod() public pure returns (uint32) {
    ...
    function transferFrom(
        ...
        address _toAddress,
        uint256 _tokenId
    ) public payable virtual whenNotPaused noBannedAccounts {
        require(!_isBanned(_fromAddress) && !_isBanned(_toAddress));
        require(_isApprovedOrOwner(_msgSender(), _tokenId));
        _checkTransferFee(_fromAddress, _toAddress);
     ...
     * - Contract MUST NOT be paused.
     * - `_operator` MUST NOT be the zero address.
     * - Caller MUST NOT be the same as `_operator`.
     * - `_operator` and calling user MUST NOT be banned.
     *
     * Emits an {ApprovalForAll} event.
     */
    ...
    function setFeeExemption(address _address, bool _exempt)
        public
        onlyOwnerOrRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function _checkTransferFee(address _fromAddress, address _toAddress)
        internal
        view
    {
        require(
            feeExempt[_msgSender()] ||
                feeExempt[_fromAddress] ||
                feeExempt[_toAddress] ||
...
contract Goblins is ViciERC721WithTransferFee {
    constructor()
        ViciERC721WithTransferFee(
            "Goblins",
            "GOBO",
            10,
            new SimpleRoyalties(
                address(0x4A91AD92EF73458f70aBB1238141C20252a2aBa1),
                10000
            )
        ) {
            setTransferFee(100000000000000000);
            setContractURI("https://storage.googleapis.com/arrival-shadow-motion-newman/test-001/Goblins-Metadata.json");
            setBaseURI("https://storage.googleapis.com/arrival-shadow-motion-newman/test-001/goblins/");
        }
}