...
contract EnergyIXTStaking is IEnergyIXTStaking, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function stake(uint256 _amount, bool _ameliaCommitment) external nonReentrant whenNotPaused {
    ...
    function earned(address _walletAddress) public view returns (uint256) {
        return (userStakedAmount[_walletAddress] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[_walletAddress])) + s_rewardPool.rewards[_walletAddress];
    }

    function earnedAmelia(address _walletAddress) public view returns (uint256) {
        return (userAmeliaCommitment[_walletAddress] * (rewardPerTokenStored() - s_rewardPool.userRewardPerTokenPaid[_walletAddress])) + s_rewardPool.ameliaRewards[_walletAddress];
    }
