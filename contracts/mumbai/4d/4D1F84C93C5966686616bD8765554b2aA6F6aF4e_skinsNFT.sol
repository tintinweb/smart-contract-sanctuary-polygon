...
@author @Pedrojok01
@notice Simple design with skin NFTs for testing purposes. !!! Do not use in production !!!
*/
...
contract skinsNFT is ERC1155, Ownable, ReentrancyGuard, Pausable {
    ...
    constructor() ERC1155("ipfs://QmVHxkDb54YDvwzXHpC1WWYdgzEAGuKecx8vhXgTAprgpm/metadata/{id}.json") {
        skins.push();
    }
    ...
    function getSkinName(uint8 _id) external view returns (bytes32) {
        return getName[_id];
    }

    function getSkinId(bytes32 _name) external view returns (uint8) {
        return getId[_name];
    }

    function getSkinAmount(uint8 _id) external view returns (bytes32) {
        return getName[_id];
    }
    ...
    function addNewSkin(uint8 _id, bytes32 _name) external onlyOwner {
        require(_id == skins.length, "_id ivalid");
        require(getId[_name] == 0, "Name already exist");
    ...
    */
    function mint(
        ...
        require(msg.sender == owner() || msg.sender == gamifyStaking, "Not authorised");
        require(_isValidSkin(id), "wrong parameter");
    ...
    function setStakingAddress(address _newStakingAddress) external onlyOwner {
        require(_newStakingAddress != address(0), "address 0");
    ...
    // function _isApprovedForMint(address _sender) private pure returns (bool) {
    //     if (_sender == true) {
    //         // implement logic with staking contract
    //         return true;
    //     } else return false;
    // }
