...
contract NFTSwap2 is ReentrancyGuard, Ownable, ERC1155Holder {
    ...
    function createSwapPair(
        ...
        require(
            whitelistedContracts[source] == true,
            "contract is not whitelisted"
        );
        swapPair memory pair = swapPair(source, _count, _tokenId, 0, 0, true,_minterContract);
        pairs[count] = pair;
    ...
    function swapMultiple(
        ...
        require(pair.count == total, "not enough nfts");
        ...
        mock1155 = MockSoulbound1155(pair.source);
    ...
    function swapSingle(
        ...
        require(!isPaused, "Swap cannot be performed when contract is paused");
        swapPair memory pair = pairs[pairId];
        require(pair.count == amount, "not enough nfts");
        require(pair.isActive, "Cannot perform swap when pair is inactive");
        require(
            pair.startTime < block.timestamp && pair.endTime > block.timestamp,
            "Not within pair swap times"
        ...
        mock1155 = MockSoulbound1155(pair.source);
        mock1155.burn(address(this), id, amount);
        mock1155 = MockSoulbound1155(pair.minterContract);
        mock1155.swapMint(recipient, pair.targetTokenID, 1);
        emit swapPerformed(pairId, recipient);
    }
    ...
    function onERC1155Received(
        ...
        bytes32 test = bytes32(_data);
        uint pair = uint(test);
        swapSingle(pair, _id, _amount, _from);   
    ...
    function onERC1155BatchReceived(
        ...
        swapMultiple(pair, _ids, _amounts,_from, amt);
    ...
    function getUserPairs(address _user) public view returns (uint256[] memory) {
        ...
        if(pair.startTime < block.timestamp && pair.endTime > block.timestamp && pair.isActive){
            uint256 total = 0;
            for(uint h=0; h < 20;h++) {
                uint256 cnt = ERC1155(pair.source).balanceOf(_user, h);
                total = total + cnt;
            }
            if(total>=pair.count){
                pairids[counter] = i;
