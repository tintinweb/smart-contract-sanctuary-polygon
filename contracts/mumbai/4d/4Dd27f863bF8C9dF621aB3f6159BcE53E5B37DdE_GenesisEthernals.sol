  ...
  DefaultOperatorFilterer
{
  ...
  constructor(
    ...
    address _helper
  ) ERC721("Aether Games Genesis Ethernals", "AGGEN") {
  ...
  function airdropToAddresses(address[] memory addresses) public onlyOwner {
    require(
    ...
    for (uint256 i = 0; i < addresses.length; i++) {
      _mint(addresses[i], _tokenIdCounter.current());
      _tokenIdCounter.increment();
      addressMinted[addresses[i]]++;
    }
  }

  function mint(address _paymentTokenAddress) public payable whenNotPaused {
    require(totalSupply() < maxSupply, "Amount exceeds supply.");
  ...
  function tokenURI(
    ...
    return
      bytes(customBaseURI).length > 0
        ? string(
          abi.encodePacked(customBaseURI, Strings.toString(tokenId), ".json")
        )
        : "";
  }
