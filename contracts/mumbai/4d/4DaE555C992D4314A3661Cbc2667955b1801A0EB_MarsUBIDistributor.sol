...
contract MarsUBIDistributor is Ownable, Pausable, FxBaseChildTunnel {
  ...
  function setParameters(uint256 _start, uint256 _period, uint256 _rate) public onlyOwner {
    require(start == 0 || block.timestamp < start, "UBI HAS ALREADY BEGUN");
    start = _start;
    period = _period;
    rate = _rate;
  ...
  */
  ...
  function getClaimable(uint16[] calldata tokenIds) public view returns(uint256 totalClaimable) {
    ...
    uint256 _end = end > 0 ? end : block.timestamp;
    for (uint i = 0; i < tokenIds.length; i++) {
      totalClaimable += rate * getSeconds(start, _end, lastClaim[tokenIds[i]]) / period;
    }
  }
  ...
  function _processMessageFromRoot(uint256 , address sender, bytes memory data)
		internal override validateSender(sender) 
	{
		 ...
		 claim(tokenIds, recipient, claimId);
	}
  ...
  function claim(uint256[] memory tokenIds, address recipient, uint256 claimId) internal whenNotPaused {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      totalClaimable += getSeconds(start, _end, lastClaim[tokenIds[i]]);
      
      lastClaim[tokenIds[i]] = uint32(_end);
    }
