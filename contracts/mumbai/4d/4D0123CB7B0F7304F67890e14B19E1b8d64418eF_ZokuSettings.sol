...
contract ZokuSettings is ISettings, Ownable, Initializable {
  ...
  uint256 public override tokenMaxSupply;
  address public override tokenOwner;
  uint256 public override adminFee;
  uint256 public override royaltyFee;
  address payable public override adminFeeReceiver;
  address payable public override royaltyFeeReceiver;
  address payable public override fundReceiver;
  address public immutable nativeToken = 0x0000000000000000000000000000000000000000;
  bool public override refundable;
  bool public override refundableIncludeFee;
  ...
  modifier onlyAdmin() {
    require(_admins.contains(_msgSender()), "Not a whitelisted admin");
    _;
  }
  ...
  function protocols() public view returns(address, address, address) {
    return (_factory, _sales, _marketplace);
  }
  ...
  function isAdmin(address _admin) public view returns(bool) {
    return _admins.contains(_admin);
  }
  ...
  function addAdmin(address admin) public onlyOwner {
    _admins.add(admin);
    emit AdminCreated(admin, _msgSender());
  }

  function removeAdmin(address admin) external onlyOwner {
    _admins.remove(admin);
    emit AdminRemoved(admin, _msgSender());
  }

  function setRoyaltyFeeReceiver(address payable _royaltyFeeReceiver) external onlyOwner {
    royaltyFeeReceiver = payable(_royaltyFeeReceiver);
    emit RoyaltyFeeReceiverUpdated(_royaltyFeeReceiver, _msgSender());
  }

  function setRoyaltyFee(uint256 _royaltyFee) external onlyOwner {
    royaltyFee = _royaltyFee;
    emit RoyaltyFeeUpdated(_royaltyFee, _msgSender());
  }

  function setAdminFeeReceiver(address payable _adminFeeReceiver) external onlyOwner {
    adminFeeReceiver = payable(_adminFeeReceiver);
    emit AdminFeeReceiverUpdated(_adminFeeReceiver, _msgSender());
  }

  function setFundReceiver(address payable _fundReceiver) external onlyOwner {
    fundReceiver = payable(_fundReceiver);
    emit FundReceiverUpdated(_fundReceiver, _msgSender());
  }

  function setAdminFee(uint256 _adminFee) external onlyOwner {
    adminFee = _adminFee;
    emit AdminFeeUpdated(_adminFee, _msgSender());
  }
  ...
  function setTokenOwner(address _tokenOwner) external onlyOwner {
    ...
    emit TokenOwnerUpdated(_tokenOwner, _msgSender());
  }

  function setPaymentToken(address _paymentToken) external onlyOwner {
    paymentToken = _paymentToken;
    emit PaymentTokenUpdated(_paymentToken, _msgSender());
  }

  function setRefundable(bool _refundable, bool _includeFee) external onlyOwner {
    refundable = _refundable;
    refundableIncludeFee = _includeFee;
    emit RefundableUpdated(_refundable, _includeFee, _msgSender());
  }
  ...
  function transferOwnership(address newOwner) public override(Ownable, ISettings) onlyOwner {
    require(newOwner != address(0), "Ownable: new owner is the zero address");
    _transferOwnership(newOwner);
  }

  function renounceOwnership() public override(Ownable, ISettings) onlyOwner {
