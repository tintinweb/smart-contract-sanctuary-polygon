...
contract ArtLandToken is ERC721, LandMint, Pausable, ERC721Burnable, IERC2981, ReentrancyGuard {
    string private _baseTokenURI;
    string private _contractURI;

    constructor(uint16 _maximumLand) ERC721("ArtLandToken", "ALT") LandMint(_maximumLand) {}
    ...
    function royaltyInfo(uint256, uint256 _salePrice) external view override returns (address receiver, uint256 royaltyAmount) {
        return (owner(), (_salePrice * 700) / 10000);
    }
...
abstract contract LandMint is AdminOwnable, ERC721Enumerable {
  ...
  function _mintLand(
    ...
    bytes calldata signature // prevent alteration of intended mint data
  ) internal returns (uint256[] memory) {
    // console.log('mintData 1', uint8(mintData.minX), uint8(mintData.maxX), uint8(mintData.minY));
    // console.log('mintData 2', uint8(mintData.maxY), mintData.timestampStart, mintData.timestampEnd);
    // console.log('mintData 3', mintData.value, mintData.prevMintCount, msg.value);
    ...
    for (int32 x = mintData.x0; x <= mintData.x1; x++) {
      for (int32 y = mintData.y0; y <= mintData.y1; y++) {
        ...
        console.log("index", index, uint(int(x)), uint(int(y)));
        console.log("assigned", tokenId);
        index++;
      }
    }
...
abstract contract AdminAccessControl is Context, AccessControl, AdminSignature {
    ...
    function transferAdminship(address newAdmin) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
