...
contract ColexionMarketPlace is ReentrancyGuardUpgradeable, IERC1155ReceiverUpgradeable, OwnableUpgradeable {
    ...
    function placeOrderForSell(uint256 tokenId, address nftContract, uint16 copies, uint256 pricePerNFT, address paymentToken, uint256 endTime, uint8 saleType) external {
    ...
    function cancelOrder(uint256 orderId) external {
        ...
        if (_order.saleType == uint8(SaleType.OpenForOffers)) {
            returnAmountToRemainingBidder(orderId, _order.paymentToken == address(0));
        }
    ...
    function withdrawRejectBid(uint256 orderId, uint256 bidId, bool isReject) external {
        Order storage _order = order[orderId];
        Bid storage _bid = bids[orderId][bidId];
        ...
        require(_bid.status == 0, "cant process");
    ...
    function bulkBuy(uint256[] calldata orderIds, uint256[] calldata amounts) payable external OnlyAdminOwner {
        uint256 payAmount = 0;
        ...
        require(amountSupplyIssue == false && payAmount <= msg.value, "Iv Amount");
    ...
    function withdrawMoney(uint256 amount, address tokenAddress) onlyOwner external {
    ...
    function returnAmountToRemainingBidder(uint256 orderId, bool isNative) private {
        Order storage _order = order[orderId];
