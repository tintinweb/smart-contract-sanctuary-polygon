    ...
    ReentrancyGuardUpgradeable
{
    ...
    ///         campaign.
    /// @param  stakingLandType - The ID was configured within this contract for
    ///         determinting the suitable rewards.
    ...
    /// @notice Reward Info of each campaign for each staking item.
    ///         (campaignId + stakingToken + stakingTokenType)
    mapping(uint8 => mapping(address => mapping(uint256 => CampaignReward)))
        public campaignRewardInfo;

    /// @notice Total ERC721 tokens were deposited in a campaign with type.
    ///         (campaignId + stakingTokenType)
    mapping(uint8 => mapping(uint256 => uint256))
        public campaignERC721BalanceByTokenType;

    /// @notice Total ERC1155 tokens were deposited in a campaign with type.
    ///         (campaignId + stakingTokenType)
    mapping(uint8 => mapping(uint256 => uint256))
        public campaignERC1155BalanceByTokenType;
    ...
    /// @notice Total tokens in a campaign of an user with type.
    ///         (campaignId + userAddress + stakingToken + stakingTokenType)
    mapping(uint8 => mapping(address => mapping(address => mapping(uint256 => uint256))))
        public userBalanceByTokenType;

    /// @notice Get type of a particular token. (stakingToken + tokenId)
    mapping(address => mapping(uint256 => uint256)) public stakingTokenTypes;
    ...
    function initialize(
        ...
        // Campaign 1
        campaignRewardInfo[1][_erc721Token][1] = CampaignReward({
            boxType: 1,
            boxAmount: 10,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][2] = CampaignReward({
            boxType: 2,
            boxAmount: 15,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][3] = CampaignReward({
            boxType: 3,
            boxAmount: 10,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][4] = CampaignReward({
            boxType: 4,
            boxAmount: 15,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][5] = CampaignReward({
            boxType: 5,
            boxAmount: 10,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][6] = CampaignReward({
            boxType: 6,
            boxAmount: 15,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        });
        campaignRewardInfo[1][_erc721Token][7] = CampaignReward({
            boxType: 7,
            boxAmount: 10,
            buildingBoxType: 0,
            buildingBoxAmount: 0
        ...
        // Campaign 2
        campaignRewardInfo[2][_erc721Token][1] = CampaignReward({
            boxType: 1,
            boxAmount: 10,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][2] = CampaignReward({
            boxType: 2,
            boxAmount: 15,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][3] = CampaignReward({
            boxType: 3,
            boxAmount: 10,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][4] = CampaignReward({
            boxType: 4,
            boxAmount: 15,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][5] = CampaignReward({
            boxType: 5,
            boxAmount: 10,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][6] = CampaignReward({
            boxType: 6,
            boxAmount: 15,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[2][_erc721Token][7] = CampaignReward({
            boxType: 7,
            boxAmount: 10,
            buildingBoxType: 1,
            buildingBoxAmount: 1
        ...
        // Campaign 3
        campaignRewardInfo[3][_erc721Token][1] = CampaignReward({
            boxType: 1,
            boxAmount: 10,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][2] = CampaignReward({
            boxType: 2,
            boxAmount: 15,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][3] = CampaignReward({
            boxType: 3,
            boxAmount: 10,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][4] = CampaignReward({
            boxType: 4,
            boxAmount: 15,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][5] = CampaignReward({
            boxType: 5,
            boxAmount: 10,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][6] = CampaignReward({
            boxType: 6,
            boxAmount: 15,
            buildingBoxType: 2,
            buildingBoxAmount: 1
        });
        campaignRewardInfo[3][_erc721Token][7] = CampaignReward({
            boxType: 7,
            boxAmount: 10,
            buildingBoxType: 2,
            buildingBoxAmount: 1
    ...
    /**
     * @dev   This function is called by the user when they want to see the rewards they can
     *        claim with a particular NFT Land type.
     * @param campaignId - An unique identifier for this particular campaign. There are 3
     *        campaign represent for 3 kinds of duration: 30, 60 and 90 days.
     * @param token - The NFT Land address
     * @param stakingTokenType - The ID was configured within this contract for determinting
     *        the suitable rewards.
     */
    function getUserRewardByTokenType(
        uint8 campaignId,
        address token,
        uint256 stakingTokenType
    )
        public
        view
        returns (
    ...
    {
        CampaignReward memory reward = campaignRewardInfo[campaignId][token][
            stakingTokenType
        ];
        uint256 userBalance = userBalanceByTokenType[campaignId][msg.sender][
            token
        ][stakingTokenType];
        boxType = reward.boxType;
        boxAmount = reward.boxAmount * userBalance;
        buildingBoxType = reward.buildingBoxType;
        buildingBoxAmount = reward.buildingBoxAmount * userBalance;
    }
    ...
    /**
     * @dev   This function is called by the Admin when they want to configure the token
     *        type for NFT Land.
     * @param token - The NFT Land contract address.
     * @param tokenIds - The list of NFT Land ID.
     * @param types - The corresponding type for each NFT Land ID.
     */
    function setStakingTokenTypes(
        address token,
        uint256[] calldata tokenIds,
        uint256[] calldata types
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(tokenIds.length == types.length, "length not match");
        require(tokenIds.length > 0, "length equals 0");
        for (uint256 i; i < tokenIds.length; i++) {
            stakingTokenTypes[token][tokenIds[i]] = types[i];
        }
    }
    ...
    function _deposit(
        ...
        uint256 tokenType = stakingTokenTypes[_token][_tokenId];
        require(tokenType > 0, "invalid token");
        if (_token == address(ERC721_STAKING_TOKEN)) {
            ...
            campaignERC721BalanceByTokenType[_campaignId][tokenType] += _amount;
            userBalanceByTokenType[_campaignId][_beneficiary][_token][
                tokenType
            ] += _amount;
            userBalanceByTokenId[_campaignId][_beneficiary][_token][
                _tokenId
            ...
            campaignERC1155BalanceByTokenType[_campaignId][
                tokenType
            ] += _amount;
            userBalanceByTokenType[_campaignId][_beneficiary][_token][
                tokenType
