...
contract Resources_6 is Initializable, IResourceLogic {
    using Items for Items.ID;
    using Resources for Resources.ID;
    IBaseLogic public baseLogic;
    modifier onlyGameContract() {
        require(msg.sender == baseLogic.gameSupport(), "q");
        _;
    }
    ...
    {
        if (resourceId == uint16(Resources.ID.T2)) {
            destroyPos = t2(id, position);
        } else if (resourceId == uint16(Resources.ID.T3)) {
            t3(id, position);
        } else if (resourceId == uint16(Resources.ID.T5)) {
            destroyPos = t5(id, position);
        } else if (resourceId == uint16(Resources.ID.T6)) {
            destroyPos = t6(id, position);
        } else if (resourceId == uint16(Resources.ID.T7)) {
            t7(id, position, randomNumber);
        } else if (resourceId == uint16(Resources.ID.T9)) {
            t9(id, position);
        } else if (
            resourceId == uint16(Resources.ID.V1) ||
            resourceId == uint16(Resources.ID.V2) ||
            resourceId == uint16(Resources.ID.V3)
        ) {
            destroyPos = void(id, position);
        } else if (resourceId == uint16(Resources.ID.W1)) {
            w1(id, position);
        } else if (resourceId == uint16(Resources.ID.W4)) {
            w4(id, position);
        } else if (resourceId == uint16(Resources.ID.W5)) {
            w5(id, position);
        } else if (resourceId == uint16(Resources.ID.L3)) {
            destroyPos = l3(id, position);
        } else if (resourceId == uint16(Resources.ID.N2)) {
            n2(id, position);
        } else if (resourceId == uint16(Resources.ID.A4)) {
            a4(id, position);
        } else if (resourceId == uint16(Resources.ID.N3)) {
            n3(id, position);
        } else if (resourceId == uint16(Resources.ID.A5)) {
            a5(id, position);
        } else if (resourceId == uint16(Resources.ID.S10)) {
            s10(id, position);
        } else if (resourceId == uint16(Resources.ID.S11)) {
            s11(id, position);
        } else if (resourceId == uint16(Resources.ID.S12)) {
            s12(id, position, randomNumber);
        }
    }
    ...
    function t3(uint256 id, uint16 position) internal {
    ...
    function t6(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.C1 ||
                adjResource == Resources.ID.B15 ||
                adjResource == Resources.ID.P5
            ) {
        ...
        if (countDestroy > 0) {
            IGameController(baseLogic.gameController()).updatePayout(
                ...
                0,
                0,
                6 * int256(countDestroy - 1)
            );
            return Helper.getArray(countDestroy, destroyPos);
        }
    }

    function t7(uint256 id, uint16 position, uint256 random) internal {
        uint256 percent = ((random / 10 ** position) % 100000) % 100;
        if (
            (baseLogic.isItemExist(id, uint256(Items.ID.IC12), false) &&
                percent < 12) || percent < 6
        ) {
            IInventory(baseLogic.landManager()).addResource(
                id,
                uint16(Resources.ID.S10),
                1
            );
        }
    }

    function t9(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function void(
        ...
        for (uint8 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (adjResource == Resources.ID.E4) {
                ++count;
            }
        }
        if (count == 0) {
    ...
    function w1(uint256 id, uint16 position) internal {
        uint256 count = IGameController(baseLogic.gameController())
            .countResource(id, uint256(Resources.ID.W1));
        if (count > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            ).findPosition(id, uint256(Resources.ID.W1));
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                listPos
    ...
    function w4(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
        if (countSpin > 0) {
            ...
            if (countSpin == 0) {
                baseLogic.boostPayout(id, uint16(Resources.ID.W2), position, 1);
            }
        }
    }

    function w5(uint256 id, uint16 position) internal {
        uint16[] memory adjacentPosition = Helper.getAdjacentPositions(
            position,
            baseLogic.checkAllAdjacent(id, position)
        );
        for (uint256 i = 0; i < adjacentPosition.length; ++i) {
            ...
            if (
                adjResource == Resources.ID.C3 ||
                adjResource == Resources.ID.O4 ||
                adjResource == Resources.ID.C18 ||
                adjResource == Resources.ID.A3 ||
                adjResource == Resources.ID.H2 ||
                adjResource == Resources.ID.H3 ||
                adjResource == Resources.ID.H4 ||
                adjResource == Resources.ID.H5 ||
                adjResource == Resources.ID.H6 ||
                adjResource == Resources.ID.H7 ||
                adjResource == Resources.ID.H8 ||
                adjResource == Resources.ID.E2 ||
                adjResource == Resources.ID.S10
            ) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    adjPosition,
    ...
    function n2(uint256 id, uint16 position) internal {
        uint256 row = position / 5;
        int256 count;
        uint16[] memory listPos = new uint16[](5);
        for (uint256 i = 0; i < 5; ++i) {
            uint256 pos = row * 5 + i;
            Resources.ID resource = Resources.findResource(
                IGameController(baseLogic.gameController()).atPosition(id, pos)
            );
            if (resource == Resources.ID.N2) {
                listPos[uint256(count)] = uint16(pos);
                ++count;
            }
        }
        if (count > 1) {
            ...
            uint16[] memory resPos = Helper.getArray(uint256(count), listPos);
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                resPos
    ...
    function a4(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
        ...
        if (countSpin == 0) {
            baseLogic.boostPayout(id, uint16(Resources.ID.W2), position, 4);
            IGameController(baseLogic.gameController()).updateProp(
                id,
                position,
                2
    ...
    function n3(uint256 id, uint16 position) internal {
        uint256 row = position / 5;
        int256 count;
        uint16[] memory listPos = new uint16[](5);
        for (uint256 i = 0; i < 5; ++i) {
    ...
    function a5(uint256 id, uint16 position) internal {
        uint256 countSpin = IGameController(baseLogic.gameController()).getProp(
            id,
            position
        );
    ...
    function s10(
        ...
        --countSpin;
        if (
            !baseLogic.isItemExist(id, uint256(Items.ID.IU1), false) &&
            countSpin == 0
        ) {
            destroyPos = new uint16[](20);
            destroyPos[0] = position;
    ...
    function s11(uint256 id, uint16 position) internal {
        uint256 count = IGameController(baseLogic.gameController())
            .countResource(id, uint16(Resources.ID.S11));
        if (count > 1) {
            IGameController(baseLogic.gameController()).updatePayout(
                id,
                position,
            ...
            uint16[] memory listPos = IGameController(
                baseLogic.gameController()
            ).findPosition(id, uint16(Resources.ID.S11));
            IEventContract(baseLogic.eventContract()).emitSuperEffect(
                id,
                position,
                listPos
    ...
    function s12(uint256 id, uint16 position, uint256 random) internal {
        ...
        if (isIC7Exist) {
            uint16[] memory flower_pos = IGameController(
                baseLogic.gameController()
            ).findPosition(id, uint16(Resources.ID.F3));
            for (uint256 i = 0; i < flower_pos.length; ++i) {
                IGameController(baseLogic.gameController()).updatePayout(
                    id,
                    flower_pos[i],
                    5,
            ...
            uint16[] memory seed_pos = IGameController(
                baseLogic.gameController()
            ).findPosition(id, uint16(Resources.ID.S4));
            for (uint256 i = 0; i < seed_pos.length; ++i) {
                uint256 percent = ((random / 100 ** (position + i)) % 5000) %
                    100;
                if (percent < 50) {
                    IResource_0(baseLogic.resourceLogic(0)).s4_grow(
                        id,
                        seed_pos[i],
                        random
                    );
            ...
            for (uint256 i = 0; i < adjacentPosition.length; ++i) {
                ...
                if (adjResource == Resources.ID.F3) {
                    IGameController(baseLogic.gameController()).updatePayout(
                        id,
                        position,
                        5,
                ...
                } else if (adjResource == Resources.ID.S4) {
                    // uint256 percent = ((random / 100 ** position) % 100000) %
                    //     100;
                    uint256 percent = Helper.calRandomHelper(
                        position,
                        random,
                        100000,
                        100
