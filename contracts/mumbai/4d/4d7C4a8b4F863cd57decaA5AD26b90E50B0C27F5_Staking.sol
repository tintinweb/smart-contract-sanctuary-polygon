...
error BCBCT__Stake__NotApproved();
...
contract Staking is APR, ReentrancyGuard {
    ...
    function stake(uint256 stakeAmount) external nonReentrant {
        uint8 _apr = getApr(stakeAmount);
        bool istenureRight = _apr > 0;
        ...
        if (isApproved(msg.sender, address(this)) < stakeAmount) {
            revert BCBCT__Stake__NotApproved();
        }
    ...
    function unstake(uint256 _stakeID) external nonReentrant {
        ...
        uint256 stakedamount = stakeStatus[sender]
            ...
            .staketokens;
        uint256 stakedtime = stakeStatus[sender].stakeDetails[_stakeID].end;
        uint256 penalty = 0;
        uint256 reward = 0;
        uint256 cliamable = 0;
        ...
        if (stakedamount == 0) revert BCBCT__UnStake__AlreadyClaimed();
        if (getStakesID().length <= 1)
            stakeStatus[sender].isStaked = Status.UNSTAKED;
        if (stakedtime > block.timestamp) {
            penalty = getPenalty(_stakeID); // If applicable, the penalty is the same as the earned interest.
        } else if (stakedtime < block.timestamp) {
            reward = getReward(_stakeID); //if applicable
        }
        cliamable = (stakedamount + reward) - penalty;
        stakeStatus[sender].count -= 1;
    ...
    function setApr(
        ...
        bool check = (tenure <= 0 && apr <= 0);
        if (check) {
            revert BCBCT__Staking__InvalidArgument();
        }
        emit UpdatedAPR(_id, apr);
        _setApr(_id, tenure, apr); // Only the APR is adjustable, whereas the tenure is fixed at one year.
    }
    ...
    function getReward(uint256 _stakeID) public view returns (uint256) {
        uint256 stakedamount = stakeStatus[msg.sender]
            .stakeDetails[_stakeID]
            .staketokens;
        uint8 tenure = stakeStatus[msg.sender].stakeDetails[_stakeID].tenure;
        uint8 apr = getApr(stakedamount);
        uint256 denominatorAPR = 100;
        uint256 lockedDays = uint256(tenure) * 365;
        uint256 rewardClaimable = (stakedamount * lockedDays * uint256(apr)) /
            (365 * denominatorAPR);
    ...
    function getPenalty(uint256 _stakeID) public view returns (uint256) {
        return getReward(_stakeID);
    }
    ...
    //--------------withdraw---------------------//
    function withdraw(address owner, uint256 _amount)
        external
        onlyOwner
        checkForZeroArgument(owner, _amount)
    {
        emit Withdrawn(owner, _amount);
        BCBCT.transfer(owner, _amount);
    }
