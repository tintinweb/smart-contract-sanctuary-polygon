...
contract AgentRoleUpgradeable is OwnableUpgradeable, AccessControl {
    ...
    function _setRoleToOwner(address _owner) internal {
        _setupRole(Owner_Role, _owner);    }

    function addSubOwner(address _SubOwner) external onlyOwner {
        _setupRole(Owner_Role, _SubOwner);
    }

    function setRolesToUser(address _userAddress, bytes32[] memory roles) external onlyOwner{
        require(isAgent(_userAddress), "Roles can be assigned only to the Agent");
    ...
    function addAgent(address _agent) public onlyRole(Owner_Role) {
        _agents.add(_agent);
        emit AgentAdded(_agent);
    }

    function removeAgent(address _agent) public onlyRole(Owner_Role) {
...
contract TokenST is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    function _transfer(
        ...
        require(_amount < transferLimit, 'Amount is over the Limit');
        uint16 countrylist = tokenIdentityRegistry.investorCountry(_to);
        require(!_blacklistCountries[countrylist],'Country blacklisted');
        require(_authorizedCountries[countrylist], 'Country not Authorized');
    ...
    function setTransferLimit(uint _limit) external onlyOwner {
        transferLimit = _limit;
    }

    function banCountry(uint16 _country) external onlyOwner {
        _blacklistCountries[_country] = true;
    }

    function unBanCountry(uint16 _country) external onlyOwner {
        _blacklistCountries[_country] = false;
    }

    function isCountryBan(uint16 _country) external view returns (bool){
        return _blacklistCountries[_country];
    }

    function authorizeCountry(uint16 _country) external onlyOwner {
        _authorizedCountries[_country] = true;
    }

    function unAuthorizeCountry(uint16 _country) external onlyOwner {
        _authorizedCountries[_country] = false;
    }

    function isCountryAuthorized(uint16 _country) external view returns (bool){
        return _authorizedCountries[_country];
    }
...
contract TokenStorage {
    ...
    mapping(uint16 => bool) _blacklistCountries;
    mapping(uint16 => bool) _authorizedCountries;
