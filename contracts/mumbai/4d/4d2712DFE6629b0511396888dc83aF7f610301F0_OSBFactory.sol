...
contract OSB721 is ERC721EnumerableUpgradeable, ERC2981Upgradeable, OwnableUpgradeable {
    ...
    /// @dev Emit an event when the contract is deployed
    event ContractDeployed(address indexed owner, string contractUri, string name, string symbol, address indexed receiverRoyaltyFee, uint96 indexed percentageRoyaltyFee);

    /// @dev Emit an event when mint success
    event Mint(address indexed to, uint256 indexed tokenId, string indexed tokenUri);
    ...
    event MintWithRoyalty(address indexed to, uint256 tokenId, string tokenUri, address indexed receiverRoyaltyFee, uint96 indexed percentageRoyaltyFee);

    /// @dev Emit an event when mintBatchWithRoyalty success
    event MintBatchWithRoyalty(address indexed to, uint256[] tokenIds, string[] tokenUris, address[] receiverRoyaltyFees, uint96[] percentageRoyaltyFees);
    ...
    event SetContractURI(string indexed oldUri, string indexed newUri);

    /// @dev Emit an event when updated new token URI
    event SetTokenURI(uint256 indexed tokenId, string indexed oldUri, string indexed newUri);

    /**
     * @notice Setting states initial when deploy contract and only called once
     * @param _owner Contract owner address
     * @param _contractUri Contract URI metadata
     * @param _name Token name
     * @param _symbol Token symbol
     * @param _receiverRoyaltyFee Default royalty receiver address
     * @param _percentageRoyaltyFee Default percent royalty
     */
    function initialize(address _owner, string memory _contractUri, string memory _name, string memory _symbol, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) public initializer {
        ...
        emit ContractDeployed(_owner, _contractUri, _name, _symbol, _receiverRoyaltyFee, _percentageRoyaltyFee);
    }
    ...
    /**
     * @notice Update new contract URI
    ...
    /**
     * @notice Mint as many tokens to an address
     * @param _to Mint to address
     * @param _tokenUris Metadata URIs per token
     * @param _amount Token quantity
     * @return tokenIds The list of owned tokens
     */
    function mintBatch(address _to, string[] memory _tokenUris, uint256 _amount) external onlyOwnerOrController returns (uint256[] memory tokenIds) {
        require(_amount > 0, "Invalid amount");
        require(_tokenUris.length == _amount, "Invalid param");
     ...
     * @return tokenId Token id of owner
     */
    function mintWithRoyalty(address _to, string memory _tokenUri, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) external onlyOwnerOrController returns (uint256 tokenId) {
        tokenId = mint(_to, _tokenUri);
        if (_receiverRoyaltyFee == address(0)) _setTokenRoyalty(tokenId, defaultRoyaltyInfo.receiver, _percentageRoyaltyFee);
        else _setTokenRoyalty(tokenId, _receiverRoyaltyFee, _percentageRoyaltyFee);
        emit MintWithRoyalty(_to, tokenId, _tokenUri, _receiverRoyaltyFee, _percentageRoyaltyFee);
    }

    /**
     * @notice Mint as many tokens to an address and set royalty
     * @param _to Mint to address
     * @param _tokenUris Metadata URIs per token
     ...
     * @return tokenIds The list of owned tokens
     */
    function mintBatchWithRoyalty(address _to, string[] memory _tokenUris, address[] memory _receiverRoyaltyFees, uint96[] memory _percentageRoyaltyFees) external onlyOwnerOrController returns (uint256[] memory tokenIds) {
        require(_tokenUris.length > 0 && _tokenUris.length == _receiverRoyaltyFees.length && _tokenUris.length == _percentageRoyaltyFees.length, "Invalid param");

        tokenIds = new uint256[](_receiverRoyaltyFees.length);

        for (uint256 i = 0; i < _receiverRoyaltyFees.length; i++) {
            tokenIds[i] = mint(_to, _tokenUris[i]);
            if (_percentageRoyaltyFees[i] == 0) continue;
            if (_receiverRoyaltyFees[i] == address(0)) _setTokenRoyalty(tokenIds[i], defaultRoyaltyInfo.receiver, _percentageRoyaltyFees[i]);
            else _setTokenRoyalty(tokenIds[i], _receiverRoyaltyFees[i], _percentageRoyaltyFees[i]);
        }
        emit MintBatchWithRoyalty(_to, tokenIds, _tokenUris, _receiverRoyaltyFees, _percentageRoyaltyFees);
    }
...
contract OSB1155 is ERC1155Upgradeable, ERC2981Upgradeable, OwnableUpgradeable {
    ...
    /// @dev Emit an event when mint success
    event Mint(address indexed to, uint256 indexed tokenId, string indexed tokenUri, uint256 amount);

    /// @dev Emit an event when mintBatch success
    event MintBatch(address indexed to, string[] tokenUris, uint256[] tokenIds, uint256[] amounts);

    /// @dev Emit an event when mintWithRoyalty success
    event MintWithRoyalty(address indexed to, string tokenUri, uint256 amount, address indexed receiverRoyaltyFee, uint96 indexed percentageRoyaltyFee);

    /// @dev Emit an event when MintBatchWithRoyalty success
    event MintBatchWithRoyalty(address indexed to, string[] tokenUris, uint256[] tokenIds, uint256[] amounts, address[] receiverRoyaltyFees, uint96[] percentageRoyaltyFees);
     ...
     * @return tokenId Token id of owner
     */
    function mint(address _to, string memory _tokenUri, uint256 _amount) public onlyOwnerOrController returns (uint256 tokenId) {
        require(_amount > 0, "Invalid amount");
        require(bytes(_tokenUri).length > 0, "Invalid tokenUri");
        lastId.increment();
        tokenId = lastId.current();
     ...
     * @return tokenIds The list of owned tokens
     */
    function mintBatch(address _to, string[] memory _tokenUris, uint256[] memory _amounts) external onlyOwnerOrController returns (uint256[] memory tokenIds) {
        require(_amounts.length > 0 && _amounts.length == _tokenUris.length, "Invalid param");
        tokenIds = new uint256[](_amounts.length);
        for (uint256 i = 0; i < _amounts.length; i++) {
            tokenIds[i] = mint(_to, _tokenUris[i], _amounts[i]);
        }
        emit MintBatch(_to, _tokenUris, tokenIds, _amounts);
    }
     ...
     * @return tokenId Token id of owner
     */
    function mintWithRoyalty(address _to, string memory _tokenUri, uint256 _amount, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) external onlyOwnerOrController returns (uint256 tokenId) {
        tokenId = mint(_to, _tokenUri, _amount);
        if (_receiverRoyaltyFee == address(0)) _setTokenRoyalty(tokenId, defaultRoyaltyInfo.receiver, _percentageRoyaltyFee);
        else _setTokenRoyalty(tokenId, _receiverRoyaltyFee, _percentageRoyaltyFee);
        emit MintWithRoyalty(_to, _tokenUri, _amount, _receiverRoyaltyFee, _percentageRoyaltyFee);
    }

    /**
     * @notice Mint as many tokens with quantity to an address and set royalty
     * @param _to Mint to address
     * @param _tokenUris Metadata URIs per token
     * @param _amounts Quantity per token
     * @param _receiverRoyaltyFees Royalty receiver address per token
     ...
     * @return tokenIds The list of owned tokens
     */
    function mintBatchWithRoyalty(address _to, string[] memory _tokenUris, uint256[] memory _amounts, address[] memory _receiverRoyaltyFees, uint96[] memory _percentageRoyaltyFees) external onlyOwnerOrController returns (uint256[] memory tokenIds) {
        require(_amounts.length > 0 && _amounts.length == _tokenUris.length && _amounts.length == _receiverRoyaltyFees.length && _receiverRoyaltyFees.length == _percentageRoyaltyFees.length, "Invalid param");
        tokenIds = new uint256[](_amounts.length);
        for (uint256 i = 0; i < _amounts.length; i++) {
            tokenIds[i] = mint(_to, _tokenUris[i], _amounts[i]);
            if (_percentageRoyaltyFees[i] == 0) continue;
        ...
        emit MintBatchWithRoyalty(_to, _tokenUris, tokenIds, _amounts, _receiverRoyaltyFees, _percentageRoyaltyFees);
    }
