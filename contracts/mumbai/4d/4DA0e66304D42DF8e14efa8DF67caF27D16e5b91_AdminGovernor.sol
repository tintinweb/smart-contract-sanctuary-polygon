...
import "./AdminGovernorStorage.sol";
...
contract AdminGovernor is 
    AdminGovernorStorage,
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    IGovernor {
    ...
    function initialize(uint256 _votingDelay,uint256 _votingPeriod) public initializer {
    ...
    function castVoteWithReason( uint256 proposalId, uint8 support, string calldata reason) public override {
        _castVote(proposalId, msg.sender, support, reason);
    }

    function castVoteBySig(IMessage.Message memory message, bytes memory signature, IMessage.DomainSeparator memory domain) public override returns (uint256 balance){
        address voter ; 
        voter = VerifySignature.verify(message, domain, signature);
        _castVote(message.proposalId, voter, message.support, message.reason);
        return address(this).balance;
    }

    function _castVote(uint256 proposalId, address account, uint8 support, string memory reason) internal {
    ...
    function _quorumReached(uint256 proposalId) internal view returns (bool) {
        ProposalVote storage proposalvote = _proposalVotes[proposalId];
        return  proposalvote.forVotes >= validatorCnt / 2 + 1 ;
    }

    function _fullQuorum(uint256 proposalId) internal view returns (bool) {
        ProposalVote storage proposalvote = _proposalVotes[proposalId];
        return
            validatorCnt == (proposalvote.forVotes + proposalvote.againstVotes + proposalvote.abstainVotes);
    }
    ...
    function state(uint256 proposalId) public view override returns (ProposalState) {
        ...
        if (proposal.executed) {
            ...
            block.timestamp <= proposal.voteEnd.getDeadline() && !_fullQuorum(proposalId)) {
    ...
    function _execute(uint256 proposalId) internal virtual returns (bool) {
        ProposalDetails storage proposal = _proposalDetails[proposalId];
        return _transferToken(proposal.candidate, proposal.tokenOffer);
    }

    

    function _transferToken(address receiver, uint256 amount) internal returns (bool) {
        return IERC20(USDCToken).transfer(receiver, amount * 10 ** 18);
    }

    function registerValidator(address candidate) public onlyOwner {
        isValidator[candidate] = true;
        validatorCnt += 1;
    }

    function updateVotingDelay(uint256 newValue) external onlyOwner {
        votingDelay = newValue;
    }

    function updateVotingPeriod(uint256 newValue) external onlyOwner {
        votingPeriod = newValue;
    }

    function updateUSDTToken(address newValue) external onlyOwner {
        USDCToken = newValue;
    }

    function addDaoToList(address dao) external onlyOwner  {
        DAOs[DaosCount] = dao;
        DaosCount += 1;
    }

    function DAOsList() external view returns (address[] memory) {
        address[] memory daosAddress = new address[](DaosCount);
        for (uint8 i =0 ; i < DaosCount ; i++) {
            daosAddress[i] = DAOs[i];
        }
        return daosAddress;
    }
...
contract AdminGovernorStorage {
    ...
    struct ProposalVote {
        ...
        EnumerableSet.AddressSet hasVoted; 
    }
    ...
    mapping(uint256 => ProposalDetails) internal _proposalDetails; //proposalID to details
    mapping(address => bool) internal isValidator; 
    uint32 public validatorCnt;
    mapping(uint256 => ProposalVote) internal _proposalVotes;
    mapping(uint256 => ProposalCore) internal _proposalCores; 
    mapping (uint8 => address) internal DAOs;
    uint8 DaosCount;
...
abstract contract IGovernor{
    ...
    function castVoteBySig(IMessage.Message memory message, bytes memory signature, IMessage.DomainSeparator memory domain) public virtual returns (uint256 balance);
    ...
    function _cancel(uint256 proposalId) internal virtual returns (bool); //TODO


    function execute(uint256 proposalId) public payable virtual returns (bool);
...
library VerifySignature {
    ...
    bytes32 private constant MESSAGE_TYPEHASH =
        keccak256(
            "Vote(uint256 proposalId,uint256 support,string reason)"
        );

    function getMessageHash(IMessage.Message memory _message)
        internal
        pure
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(
                    ...
                    _message.proposalId,
                    _message.support,
                    keccak256(abi.encodePacked(_message.reason))
                )
            );
    }

    function getDomainSeparatorHash(
        IMessage.DomainSeparator memory _domain
    ...
    function verify(
        IMessage.Message memory _message,
        IMessage.DomainSeparator memory _domain,
...
what i need is signature and then you have to call
