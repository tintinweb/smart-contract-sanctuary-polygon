...
contract ThalloProofOfRetirement is UUPSUpgradeable, OwnableUpgradeable, ERC721Upgradeable {
    ...
    event NewProofOfRetirement(ProofOfRetirementData newProofData, uint256 tokenId);
    event UpdateProofOfRetirement(ProofOfRetirementData newProofData, uint256 tokenId);
    ...
    /**
     *@notice Mints a new proof of retirement
     *@param _projectId The project id
     *@param _registry The registry the project is custodied on
     *@param _vintage The vintage year
     *@param _tccAddress The address of the TCC contract
     *@param _to The address to mint the proof to
     *@dev Called from the bridge via a TCC contract
     */
    function mintProof(
        uint24 _projectId,
        uint16 _registry,
        ...
        ProofOfRetirementData storage newProof = proofs[currentTokenId];
        newProof.projectId = _projectId;
        newProof.registry = _registry;
        newProof.vintage = _vintage;
        newProof.tccAddress = _tccAddress;

        _mint(_to, currentTokenId);
        emit NewProofOfRetirement(newProof, currentTokenId);
        return currentTokenId;
    }

    /**
     *@notice Updates the final proof of retirement update with registry data and URI
     *@param _tokenId The token id of the proof to update
     *@param _retirementBlocks Data containing registry serial numbers and quantity
     *@param _tokenURI URI pointing to any proof metadata, artwork, etc.
     */
    function issueFinalProof(
        ...
        require(_tokenId <= currentTokenId, "TPR:Token does not exist");
        ProofOfRetirementData storage proofData = proofs[_tokenId];
        for (uint256 i = 0; i < _retirementBlocks.length; i++) {
            proofData.retirementBlocks.push(_retirementBlocks[i]);
        }
        proofData.tokenURI = _tokenURI;
        emit UpdateProofOfRetirement(proofData, _tokenId);
    }
