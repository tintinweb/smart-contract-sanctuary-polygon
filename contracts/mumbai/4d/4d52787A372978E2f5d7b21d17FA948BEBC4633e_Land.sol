...
contract Check is ERC1155, ReferralSystem {
  ...
  function createLevels(Level[] calldata _levels) external onlyOwner {
    require(_levels.length > 0, "Checks: Zero Levels");
    for (uint256 i = 0; i < _levels.length; i++) {
      // every ascending level should have a greater sales number than the previous one
      if (levels.length != 0) {
        require(_levels[i].requiredSales > levels[levels.length - 1].requiredSales, "Check: Levels are not in order");
      }
  ...
  function mint(
    ...
    require(mintEnabled && block.timestamp < mintingStopTime, "Check:Minting paused");

    require(_amount != 0, "Check:Invalid amount");
    require(bytes(_referrerTwitterHandle).length <= 15, "Check:Invalid referrer Twitter handle");

    require(bytes(_buyerTwitterHandle).length <= 15, "Check:Invalid buyer Twitter handle");
  ...
  function _referrerAirdrop(address _referrer, address _brandReferrer, uint256 amount) internal {
    require(_referrer != address(0) || _brandReferrer != address(0), "Check: Referrer does not exist");
  ...
  function _airdrop(uint256 _airDropQuantity, uint256 _airDropAmount, address _referrer) internal {
    if (ethAirdropEnabled) {
      ...
      require(address(this).balance >= _amount, "Check:Insufficient balance");
  ...
  function calculateUserDiscount(
    ...
    require(referrer != address(0) || brandReferrer != address(0), "Check: Referrer does not exist");
...
interface ILand is IERC721,IBaseStruct {
  function mintLand(uint256 _amount, address _buyer, CoordinatesData memory _coordinates) external;
...
contract Land is ILand, ERC721, Ownable, ERC1155Holder, AccessControlEnumerable {
  ...
  //Only given amount of checks are allowed to mint land
  uint256[] public allowedChecksAmount = [8, 32, 128, 512, 2048];

  event CheckContract(Check check);
  event MintLand(address indexed buyer, uint256 indexed tokenId, CoordinatesData coordinatesData);

  constructor(string memory _name, string memory _symbol, Check _check) ERC721(_name, _symbol) {
    check = _check;
  ...
  /* A user can only buy if
    - He approves his checks to land contract before buying land
    - Gives valid amount of checks
    - Has enough balance of checks to buy land
  */
  ...
  function mintLand(uint256 _amount, address _buyer, CoordinatesData memory _coordinates) external virtual override {
    ...
    uint checkTokenId = check.TOKENID();
    require(check.balanceOf(_buyer, checkTokenId) >= _amount, "Land:Not enough check balance");

    tokenIdCoordinatesData[nextTokenId] = _coordinates;

    //Send checks to land contract
    check.safeTransferFrom(_buyer, address(this), checkTokenId, _amount, "");

    //Burn checks
    check.burn(checkTokenId, _amount);
  ...
  function isValidAmount(uint256 _amount) internal view returns (bool) {
    for (uint256 i = 0; i < allowedChecksAmount.length; i++) {
      if (_amount == allowedChecksAmount[i]) {
        return true;
      }
    }
    return false;
  ...
  function addAllowedChecksAmount(uint256 _amount) external onlyOwner {
    allowedChecksAmount.push(_amount);
  }

  function setCheck(Check _check) external onlyOwner {
    check = _check;
  }

  function removeChecksAmount(uint256 _amount) external onlyOwner {
    uint256 indexToRemove = allowedChecksAmount.length + 1;

    for (uint256 i = 0; i < allowedChecksAmount.length; i++) {
      if (allowedChecksAmount[i] == _amount) {
        indexToRemove = i;
        break;
      }
    }

    require(indexToRemove < allowedChecksAmount.length, "Land: Amount not found");

    allowedChecksAmount[indexToRemove] = allowedChecksAmount[allowedChecksAmount.length - 1];
    allowedChecksAmount.pop();
  }
...
abstract contract ReferralSystem is Ownable {
  ...
  //Holds the amount of token sales by using referrer referral code
  mapping(address => uint256) public referrerTokenSales;
  //Holds the current sales amount of referrer from which we calculate referrer airdrop
  ...
  //Holds the current sales amount of brand referrer from which we calculate referrer airdrop
  mapping(address => uint256) public brandReferrerCurrentTokenSales;

  //Holds the amount of token sales by using brand referrer referral code
  mapping(address => uint256) public brandReferrerTokenSales;

  //Twitter handle Brand
  mapping(string => address) public twitterHandleToBrand;
  mapping(address => string) public brandToTwitterHandle;

  //Events

  event BrandReferral(address account, string twitterHandle);
  event AccountReferral(address account, string twitterHandle);
  ...
  function _validateReferrer(
    ...
    address _brandReferrer = twitterHandleToBrand[_twitterHandle];
  ...
  function createReferralForAccount(address _account, string calldata _twitterHandle) public {
    ...
    emit AccountReferral(_account, _twitterHandle);
  }

  //for brands addresses must be save seperately
  function createReferralCodeForBrand(address _account, string calldata _twitterHandle) external onlyOwner {
    require(_account != address(0), "ReferralSystem:Invalid address");
    require(bytes(brandToTwitterHandle[_account]).length == 0, "ReferralSystem: Referrer already exists");
    require(
      bytes(_twitterHandle).length > 0 && bytes(_twitterHandle).length <= 15,
      "ReferralSystem: Invalid twitter handle"
    );
    require(twitterHandleToBrand[_twitterHandle] == address(0), "ReferralSystem: Twitter handle already in use");
    brandToTwitterHandle[_account] = _twitterHandle;
    twitterHandleToBrand[_twitterHandle] = _account;

    emit BrandReferral(_account, _twitterHandle);
  }
}
