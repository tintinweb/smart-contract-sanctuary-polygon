...
contract TESTR4 is NFTokenEnumerableMetadata, Ownable {
	...
	RCA4 public referralContract;
	uint256 public maxMintAmount = 30;
	uint256 public incrementAt = 10;
	uint256 public incrementPrice = 1000;
	...
	function mint(uint256 _mintAmount, address _referrer) external payable {
		...
		for (uint256 i = 0; i < _mintAmount; i++) {
			...
			uint256 modulus = currentSupply % incrementAt;
			
			if (modulus == 0 && currentSupply != 0) {
				price += (price * incrementPrice) / 10000;
			}
	...
	function setReferralAddress(RCA4 _referralAddress) public onlyOwner {
	...
	function updateIncrementPrice(uint16 _incrementPrice) public onlyOwner {
...
contract RCA4 is IReferral, Ownable {
