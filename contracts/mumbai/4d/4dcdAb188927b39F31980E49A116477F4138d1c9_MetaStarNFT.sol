...
import "./MetaStarPremiumWLNFT.sol";
import "./MetaStarWLNFT.sol";
import "./MetaStarWhitelist.sol";
...
contract MetaStarNFT is ERC721URIStorage, Ownable {
    ...
    MetaStarWhitelist METASTARWHITELIST;
    ...
    /// @notice Modifiers
    modifier onlyAdmin {
        require(msg.sender == owner() || isAdmin[msg.sender], "00");
        _;
    }
    ...
    constructor(address _metaStarWhitelist, uint256 threshold0, uint256 threshold1, uint256 threshold2, uint256 threshold3, uint256 mintFee_type0, uint256 mintFee_type1, uint256 mintFee_type2, uint256 mintFee_type3, uint256 mintFee_type4) ERC721("METASTAR NFT", "MST") {
        /**
        ...
        */
        METASTARWHITELIST = MetaStarWhitelist(_metaStarWhitelist);
    ...
    */
    function mintTypeMetaStarNFT(uint256 starType) onlyActive public payable {
        uint256 tokenId;
        if (starType == 0) {
        ...
        mintMetaStarNFT(tokenId, false);
    }

    /**
    * @notice Function to mint a MetaStarNFT by choosing tokenId
    * @dev 
    * - Check if Whitelist is Active, if so -> check that user is Whitelisted
    * - Check that contract is ACTIVE
    ...
    * - Call internal mintNFT function
    */
    function mintMetaStarNFT(uint256 tokenId, bool direct) onlyActive public payable {
        if (s_whitelistState == WhitelistState.ACTIVE) {
            if (direct == true) {
                require(METASTARWHITELIST.isPremiumWhitelisted(msg.sender) == true, "03");
            } if (direct == false) {
                require(METASTARWHITELIST.isSimpleWhitelisted(msg.sender) == true, "02");
            }
        }
        if (tokenId >= i_MAX_STARS) {
    ...
    * @dev 
    * - Check that contract is active and user is whitelisted
    * - Check that contract is active
    ...
    */
    ...
    /**
    * @notice Change contract and whitelist state from Active to Inactive and viceversa
    * @dev Only callable by the Owner or an admin
    * @param objetive should be:
    *           0: ContractState
    *           1: WhitelistState
    */
    function flipStates(uint256 objetive) public onlyAdmin {
        if (objetive == 0) {
            ...
            s_whitelistState = WhitelistState.ACTIVE;
            }
        } 
    }
...
error MetaStarPremiumWL__TransferFailed();
error MetaStarPremiumWL__NotEnoughMoneySent();

contract MetaStarPremiumWLNFT is ERC721, Ownable {
    ...
    constructor() ERC721("MetaStarPremiumWL", "MSPW") {
        s_contractState = ContractState.INACTIVE;
        s_tokenCounter = 0;
    ...
    function mintNFT() onlyActive public payable {
        if (msg.value < s_mintFee) {
            revert MetaStarPremiumWL__NotEnoughMoneySent();
        }
    ...
    */
    function withdraw() public onlyAdmin {
        ...
        (bool success, ) = payable(msg.sender).call{value: amount}("");
        if(!success) { 
            revert MetaStarPremiumWL__TransferFailed();
        }
...
contract MetaStarWhitelist is Ownable{

    /// @notice ERC721 Tokens
    MetaStarPremiumWLNFT PREMIUM_WHITELIST_NFT;
    MetaStarWLNFT WHITELIST_NFT;
    ...
    constructor(address _metaStarPremiumWLNFT, address _metaStarWLNFT) {
        /**
        * Initialization of ERC721 Variables
        */
        PREMIUM_WHITELIST_NFT = MetaStarPremiumWLNFT(_metaStarPremiumWLNFT);
        WHITELIST_NFT = MetaStarWLNFT(_metaStarWLNFT);
    }
    ...
    /**
    * @notice Function used to check if user is whitelisted
    * @dev User can be whitelisted if:
    *       1: Owns a PremiumWhitelistNFT
    *       2: Owns a normal WhitelistNFT
    *       3: Was manually whitelisted
    */
    function isSimpleWhitelisted(address user) external returns(bool) {
    ...
    *       1: Owns a PremiumWhitelistNFT
    */
    function isPremiumWhitelisted(address user) external view returns(bool) {
