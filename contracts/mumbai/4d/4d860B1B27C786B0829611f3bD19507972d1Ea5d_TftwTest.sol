...
contract TftwTest is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://nfts.buns.land/signumrd/";
    }

    function safeMint(address to, uint256 tokenId) public payable {
        uint256 currentPrice = price;
        if (silverSaleStarted) {
            require(silverWhitelist[msg.sender] == true, "You're not in the whitelist.");
            currentPrice = price - (price * 25 / 100);
        } else if (goldSaleStarted) {
            require(goldWhitelist[msg.sender] == true, "You're not in the whitelist.");
            currentPrice = price - (price * 50 / 100);
        } else if (ogSaleStarted) {
            require(ogWhitelist[msg.sender] == true, "You're not in the whitelist.");
            currentPrice = price - (price * 50 / 100);
        } else if (privateSaleStarted) {
            require(privateWhitelist[msg.sender] == true, "You're not in the whitelist.");
            currentPrice = price - (price * 10 / 100);
        } else {
