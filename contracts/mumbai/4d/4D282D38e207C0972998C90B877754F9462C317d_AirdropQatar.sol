...
contract AirdropQatar is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    struct CampaignInfo {
        address whitelist; // whitelist contract addresses
        uint256 totalTokenClaimed; // total token
        uint256 release; // time release
        uint256 endDate; // time end
    }
    ...
    CampaignInfo[] private campaignInfo;
    mapping(uint256 => mapping(address => ClaimInfo)) public claim;

    event Claimed(uint256 _cid, address indexed _user, uint256 _amount);

    constructor() {}

    // Withdraw purchased tokens after release time
    // function Claim(uint256 _cid) external nonReentrant returns(uint256) {
    //     address _sender = msg.sender;
    //     CampaignInfo storage campaign = campaignInfo[_cid];
    //     ClaimInfo storage _claim = claim[_cid][_sender];
    //     require(
    //         block.timestamp >= _claim.release,
    //         "Please wait until release time"
    //     );
    //     require(_claim.amount == 0,"All tokens were released");
    //     IWhitelist whitelist = IWhitelist(campaign.whitelist);
    //     if(whitelist.isWhitelisted(_sender)) {
    //         _claim.amount = whitelist.whitelistedAmountFor(_sender);
    //         _claim.release = block.timestamp;
    //     }
    //     token.safeTransfer(_sender, _claim.amount);
    //     emit Claimed(_cid, _sender, _claim.amount);
    //     return _claim.amount;
    // }
    function claimAirdrop(uint256 _cid, uint256 _tokenId) public nonReentrant returns(uint256) {
        address _sender = msg.sender;
        CampaignInfo storage campaign = campaignInfo[_cid];
        ClaimInfo storage _claim = claim[_cid][_sender];
        IWhitelist whitelist = IWhitelist(campaign.whitelist);
        ...
        require(
            block.timestamp >= _claim.release,
            "Airdrop: Please wait until release time"
        );
        require(_claim.amount == 0,"Airdrop: All tokens were released");
        require(whitelist.isWhitelisted(_sender, _tokenId), "Airdrop: Sender is not whitelist!");
        require(!whitelist.isWhitelistedClaim(_sender, _tokenId), "Airdrop: Sender claimed before!");

        //store user info
        _claim.amount = whitelist.whitelistedAmountFor(_sender, _tokenId);
        _claim.release = block.timestamp;
        // update status whitelist info 
        whitelist.updateClaimed(_sender, _tokenId);
        payable(_sender).transfer(_claim.amount);
        
        emit Claimed(_cid, _sender, _claim.amount);
        return _claim.amount;
    }

    function claimAllAirdrop(uint256 _cid, uint256[] memory _tokenIds) public nonReentrant returns(uint256[] memory _amountList , uint256 _total){ 
        address _sender = msg.sender;
        for (uint i = 0; i < _tokenIds.length; i++) {
            ...
            require(_claim.amount == 0,"All tokens were released");
            require(whitelist.isWhitelisted(_sender, _tokenIds[i]), "Airdrop: Sender is not whitelist!");
            require(!whitelist.isWhitelistedClaim(_sender, _tokenIds[i]), "Airdrop: Sender claimed before!");

            _claim.amount = whitelist.whitelistedAmountFor(_sender, _tokenIds[i]);
            _claim.release = block.timestamp;
            // update status whitelist info 
            whitelist.updateClaimed(_sender, _tokenIds[i]);

            _amountList[i] = _claim.amount;
            _total += _claim.amount;
            emit Claimed(_cid, _sender, _claim.amount);
        }
            payable(_sender).transfer(_total);
    }
    ...
    function withdraw(uint256 amount) public onlyOwner {
        require(amount < address(this).balance, "Airdrop: Insufficient Balance");
    ...
    function addCampaign(
        ...
        campaignInfo.push(
            CampaignInfo({
                ...
                totalTokenClaimed: _totalTokenClaimed,
                release: _release,
                endDate: _endDate
            })
        );
    ...
    function getCampaignInfo(uint256 _cid)
    external
    view
    returns(
        address,
        ...
        return (
        campaignInfo[_cid].whitelist,
        campaignInfo[_cid].totalTokenClaimed,
        campaignInfo[_cid].release,
        campaignInfo[_cid].endDate
        );
    }
}
...
contract Whitelist is IWhitelist {
    ...
    onlyBy(airdropAddress)
    {
        ...
        for(uint256 i = 0; i < _whitelists.length; i++){
            if(_listWhitelist[account][i].tokenId == tokenId){
