...
contract community_registry is AccessControl {
    // dummy contract
    ...
    constructor(
        ...
        _setupRole(CONTRACT_ADMIN,_community_admin);
        
    }
}


contract community_registry_factory is AccessControl {
    ...
    function  launchCommunityRegistry(
        uint256 community_id, 
        address community_admin, 
        string memory community_name
    ) external onlyRole(LAUNCHER_ROLE) returns (address) {
        community_registry cr =  new community_registry(community_id, community_admin, community_name);
        return address(cr);
    }
