...
contract LostChainUserData is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCUD: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LCUD: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LCUD: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCUD: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    mapping(string => User) userWalletLink;
    ...
    mapping(address => Energy) playerEnergy;

    function setLCGD(address gameDataAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(gameDataAddress != address(0x0), 'LCUD: Game data is zero address');
    ...
    function checkWalletExists(address wallet) public view returns (bool) {
        return wallets[wallet];
    }

    function getUser(string calldata uid) public view returns (User memory) {
        require(userWalletLink[uid].enabled, "LCUD: User doesn't exists");
        return userWalletLink[uid];
    }
    ...
    function bindWallet(
        ...
        require(!userWalletLink[uid].enabled, 'LCUD: User already exists');
        require(!checkUsernameExists(username), 'LCUD: Username is taken');
        usernames[username] = true;
        wallets[walletAddress] = true;

        userWalletLink[uid].username = username;
        userWalletLink[uid].walletAddress = walletAddress;
        userWalletLink[uid].enabled = true;
    }

    function getCurrentEnergy(address playerAddress) internal view returns (uint8) {
        require(playerAddress != address(0x0), 'LCUD: Addess is zero address');
        uint8 energy = playerEnergy[playerAddress].energy;

        // First time energy
        if (playerEnergy[playerAddress].usedAt == 0) energy = LCGS.getMaxEnergy();

        // Refilled
        if (playerEnergy[playerAddress].usedAt + LCGS.getRechargeDuration() >= block.timestamp)
            energy = LCGS.getMaxEnergy();

        return energy;
    }

    function getNextRechargeTime(address playerAddress) internal view returns (uint256) {
        require(playerAddress != address(0x0), 'LCUD: Addess is zero address');

        if (playerEnergy[playerAddress].usedAt == 0) return 0;
        else {
            uint256 rechargeTime = playerEnergy[playerAddress].usedAt + LCGS.getRechargeDuration();
            if (block.timestamp >= rechargeTime) return 0;
            else return rechargeTime - block.timestamp;
        }
    }

    function getEnergyDetail(address playerAddress) public view returns (uint8, uint256) {
        require(playerAddress != address(0x0), 'LCUD: Addess is zero address');
        return (getCurrentEnergy(playerAddress), getNextRechargeTime(playerAddress));
    }

    function playerHasEnergy(address playerAddress) public view returns (bool) {
        require(_msgSender() == LCGD, 'LCUD: caller is not LCGD');
        if (getCurrentEnergy(playerAddress) > 0) return true;
        else return false;
    }

    function consumeEnergy(address operator, address playerAddress) external onlyRole('TRUSTED_FORWARDER', operator) {
        require(_msgSender() == LCGD, 'LCUD: caller is not LCGD');
        require(playerHasEnergy(playerAddress), 'LCUD: Out of energy');

        if (playerEnergy[playerAddress].usedAt != 0) {
            // Decrease energy
            if (playerEnergy[playerAddress].energy == LCGS.getMaxEnergy()) {
                playerEnergy[playerAddress].usedAt = block.timestamp;
                _removeEnergy(playerAddress);
            } else _removeEnergy(playerAddress);
        } else {
            // First time use of energy
            playerEnergy[playerAddress].usedAt = block.timestamp;
            playerEnergy[playerAddress].energy = LCGS.getMaxEnergy() - 1;
        }
    }

    function _removeEnergy(address playerAddress) internal {
        // Energy just recharged, deduct by max energy
        if (playerEnergy[playerAddress].energy == 0) playerEnergy[playerAddress].energy = LCGS.getMaxEnergy() - 1;
        else playerEnergy[playerAddress].energy--;
    }
