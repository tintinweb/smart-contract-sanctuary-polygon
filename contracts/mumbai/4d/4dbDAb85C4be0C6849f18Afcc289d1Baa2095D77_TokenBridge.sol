...
interface ITokenBridge {
    ...
    function sendETHToAptos(bytes32 _toAddress, uint _amountLD) external payable;

    event Send(address indexed token, address indexed from, bytes32 indexed to, uint amountLD);
    event SendPayload(bytes payload, uint fee);
...
contract TokenBridge is ITokenBridge, Ownable, ReentrancyGuard {
    ...
    function sendToAptos(
        ...
        // send to aptos
        _sendToken(_token, _toAddress, amountSD, msg.value);
    ...
    function sendETHToAptos(
        ...
        // send to aptos
        _sendToken(_weth, _toAddress, amountSD, msg.value - _amountLD);
    ...
    // ---------------------- internal functions ----------------------
    function msgReceive(uint16, bytes memory _payload) external {
    ...
    function _redeemETHTo(address _weth, address payable _to, uint _amountLD) internal {
    ...
    function _sendToken(address _token, bytes32 _toAddress, uint64 _amountSD, uint _fee) internal {
        bytes memory payload = _encodeSendPayload(_token, _toAddress, _amountSD);
        emit SendPayload(payload, _fee);
    }
    ...
    ) internal pure returns (address token, address to, uint64 amountSD, bool unwrap) {
