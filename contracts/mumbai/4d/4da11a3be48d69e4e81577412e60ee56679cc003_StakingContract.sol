...
contract StakingContract {
    ...
    // Additional ABI methods for reading contract information
    ...
    function getUserLastRoundCompleted(address _user) external view returns (uint256) {
        return users[_user].lastRoundCompleted;
    }

    function getUserTotalReward(address _user) external view returns (uint256) {
        return users[_user].totalReward;
    }

    function getUserAvailableWithdrawal(address _user) external view returns (uint256) {
        return users[_user].availableWithdrawal;
    }
    ...
    function getUserReferralReward(address _user, address _referrer) external view returns (uint256) {
        return users[_referrer].referralRewards[_user];
    }

    function getUserLevelReferralReward(address _user, uint256 _level) external view returns (uint256) {
        return users[_user].levelReferralRewards[_level];
    }
}