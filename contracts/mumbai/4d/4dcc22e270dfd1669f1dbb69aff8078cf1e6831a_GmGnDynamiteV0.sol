...
import "../../util/migrations/Erc1155/Erc1155MigrationToolsV1.sol";
import "../../storage/implementations/GmGnDynamite/GmGnDynamiteStorageV1.sol";

contract GmGnDynamiteV0 is Erc1155V1, Erc1155MigrationToolsV1 {
    error MigrationNotCompleteYet();
    ...
    function initialize(
        ...
        GmGnDynamiteStorageV1.layout()._degenToonzCollection = _degenToonzCollection;
    }

    function importBalances(uint256[] memory _ids, BalancesData[] memory _mintData) public onlyOwner {
        _importBalances(_ids, _mintData);
    }
    ...
    function _beforeTokenTransfer(
        ...
        if (_from != address(0)) {
            revert MigrationNotCompleteYet();
        }
    }

    function importClaimStatuses(uint256[] memory _bitMapValues) public onlyOwner {
        uint256 i = 0;
        for (;;) {
            GmGnDynamiteStorageV1.layout()._claimStatusesBitMap[i] = _bitMapValues[i];

            if (_bitMapValues.length == ++i) break;
        }
    }
    ...
    function _mintBatch(
        ...
        uint256[] memory amounts,
        bytes memory data
    ) internal override(ERC1155Upgradeable, Erc1155MigrationToolsV1) {
...
abstract contract Erc1155MigrationToolsV1 {
    struct BalancesData {
        address owner;
        uint256[] balances;
    }

    function _importBalances(uint256[] memory _ids, BalancesData[] memory _mintData) internal {
        uint256 i = 0;
        for (;;) {
            _mintBatch(_mintData[i].owner, _ids, _mintData[i].balances, "");

            if (_mintData.length == ++i) break;
        }
    }
