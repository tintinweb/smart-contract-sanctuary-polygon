...
library LibGameData {
        ...
        returns (uint256[] memory _context)
    {
        ...
        _context[4] = uint256(uint160(claimContext_.flowENERGY_WIN_XP)); // address
        _context[5] = uint256(claimContext_.id_ENERGY_WIN_XP); // ID
        
        // _context[6] = uint256(uint160(claimContext_.flowGM));
        // _context[7] = claimContext_.id_GM; // ID
        
        // _context[8] = uint256(uint160(claimContext_.flowIMPROVE));
        // _context[9] = claimContext_.id_IMPROVE; // ID
    }
}
...
contract LiChessReward is StandardInterpreter, LiChessIClaim {
    ...
    function calculateReward(GameData memory context_)
        public
        view
        returns (
            uint256[]memory
        ...
        external
    {
        ...
        uint256[] memory stack = calculateReward(context_);
...
struct GameData {
    ...
    bool isImproved;
    bool isBeatenGM;
    address flowENERGY_WIN_XP;
    uint256 id_ENERGY_WIN_XP;
    // address flowGM;
    // uint256 id_GM;
    // address flowIMPROVE;
    // uint256 id_IMPROVE;
...
import { Flow} from "../flow/basic/Flow.sol";
import "../flow/libraries/LibFlow.sol";
import "../flow/interpreter/FlowInterpreter.sol";
import {ReentrancyGuardUpgradeable as ReentrancyGuard} from "@openzeppelin/contracts-upgradeable/security/ReentrancyGuardUpgradeable.sol";

contract IFlow is ReentrancyGuard{
