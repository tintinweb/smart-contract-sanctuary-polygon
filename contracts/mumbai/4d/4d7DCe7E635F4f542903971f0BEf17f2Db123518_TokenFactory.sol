...
contract TokenFactory is NonblockingLzApp {
    ...
    function _lzAction(bool isReq, bytes memory pload, uint16 chId, uint256 gas, uint256 nativeGas, address gasTo) private {
        ...
        bytes memory adapter = _getAdapter(isReq, gas, nativeGas, remote);
    ...
    function _resAction(bytes memory payload, uint16 chId) private {
        _lzAction(false, payload, chId, 0, 0, address(this));
    }
    ...
    function estimateFees(bool isReq, string memory chSym, uint256 price, uint256 gas, uint256 nativeGas) external view returns (uint) {
    ...
    function _getAdapter(bool isReq, uint gas, uint256 nativeGas, bytes memory remote) private pure returns (bytes memory) {
        if (gas == 0) {
            gas = 1000000;
        }

        if (nativeGas == 0) {
            nativeGas = isReq ? 555555555555555 : 555555;
        }
        uint16 v = 2;
