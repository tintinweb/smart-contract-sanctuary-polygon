...
/// @author Howdy Games
/// @title Contract infrastructure for cross-contract invocation
contract RegistryFarmerV2 is IRegistryFarmerV2, OwnableUpgradeable {
	mapping(string => address) private _contractAddressByContractName;

	function initialize() public initializer {
		__Ownable_init();
	}

	function setContract(string memory contractName, address _address)
		public
		onlyOwner
	{
		_contractAddressByContractName[contractName] = _address;

		emit SetContract(contractName, _address);
	}
	...
	function getContract(string memory contractName)
		...
		returns (address)
	{
		require(_contractAddressByContractName[contractName] != address(0));

		return _contractAddressByContractName[contractName];
	}
}
