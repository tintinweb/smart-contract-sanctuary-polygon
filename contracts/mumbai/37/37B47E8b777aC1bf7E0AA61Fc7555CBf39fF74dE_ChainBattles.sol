...
contract ChainBattles is ERC721URIStorage{
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<text x="50%" y="70%" class="base" dominant-baseline="middle" text-anchor="middle">', "Levels: ",getStrength(tokenId),'</text>',
    ...
    // First method to generate random number
    function createRandom(uint number) public view returns(uint){
        return uint(blockhash(block.number-1)) % number;
    }

    // Second method to generate random number
    function createRandomPartTwo(uint number) public returns(uint){
        return uint(keccak256(abi.encodePacked(initialNumber++))) % number;
    }

    // Third method to generate random number
    function createRandomPartThree(uint number) public view returns(uint){
    ...
    function mint() public{
        ...
        tokenIdToLevels[newItemId].speed = createRandom(newItemId);
        tokenIdToLevels[newItemId].strength =createRandomPartTwo(newItemId);
        tokenIdToLevels[newItemId].life =createRandomPartThree(newItemId);
