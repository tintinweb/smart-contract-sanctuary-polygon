...
interface Cards {
    function balanceOf(address _owner, uint256 _id) external view returns (uint256);
    function hasEligibleTokenCards() external view returns (bool);
}

contract Episodes is Permissions, ReentrancyGuard, ERC1155Drop {
    
    using EnumerableSet for EnumerableSet.UintSet;

    Cards public immutable cards;
    uint256 public numTokenIds;
    mapping(uint256 => uint256[]) public E1C1Requirements;
    mapping(uint256 => uint256) private E1E2Correspondence;
    mapping(address => EnumerableSet.UintSet) internal burnedTokens;
    ...
    function burnToEarn(uint256 _tokenId, uint256[] calldata _decryptedData) external {
    // Verifique se o usuário possui o token Episode 
    require(balanceOf[msg.sender][_tokenId] > 0, "You do not own this Episode");

    // Verifique se os dados descriptografados correspondem aos dados criptografados armazenados na blockchain
    require(keccak256(abi.encodePacked(E1C1Requirements[_tokenId])) == keccak256(abi.encodePacked(_decryptedData)), "Invalid decrypted data");

    // Verifique se o usuário possui pelo menos 10 tokens do contrato Cards com, IDs diferentes
    uint256 differentIds = 0;
    for (uint i = 0; i < _decryptedData.length; i++) {
        if (cards.balanceOf(msg.sender, _decryptedData[i]) > 0) {
            differentIds++;
        }
    }
    require(differentIds >= 10, "You don't have all the Cards for this Episode");

    // Queime o token Episode
    burnedTokens[msg.sender].add(_tokenId);
    emit Burned(msg.sender,_tokenId);
    super._burn(msg.sender, _tokenId, 1);
    // Cunhe o token Episode correspondente
    emit Minted(msg.sender,E1E2Correspondence[_tokenId]);
    super._mint(msg.sender, E1E2Correspondence[_tokenId], 1,"");
    }

    function setE1C1Requirement(uint256[] calldata _e1TokenIds, uint256[][] calldata _c1TokenIds) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i=0; i<_e1TokenIds.length; i++) {
            E1C1Requirements[_e1TokenIds[i]] = _c1TokenIds[i];
        }
    }

    function getE1C1Requirement(uint256 _tokenId) public view returns (uint256[] memory) {
    return E1C1Requirements[_tokenId];
    }

    function setE1E2Correspondence(uint256[] calldata _e1TokenIds, uint[] calldata _e2Values) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint i=0; i<_e1TokenIds.length; i++) {
            E1E2Correspondence[_e1TokenIds[i]] = _e2Values[i];
        }
    }

    function setNumTokenIds(uint256 _numTokenIds) external onlyRole(DEFAULT_ADMIN_ROLE) {
        numTokenIds = _numTokenIds;
    }
     
    function getBurnedTokenCount(address user) public view returns (uint256) {
        return burnedTokens[user].length();
    }

    function getBurnedTokens(address user) public view returns (uint256[] memory) {
        uint256 tokenCount = getBurnedTokenCount(user);
        uint256[] memory result = new uint256[](tokenCount);
        for (uint256 i = 0; i < tokenCount; i++) {
            result[i] = burnedTokens[user].at(i);
        }
        return result;
    }
}