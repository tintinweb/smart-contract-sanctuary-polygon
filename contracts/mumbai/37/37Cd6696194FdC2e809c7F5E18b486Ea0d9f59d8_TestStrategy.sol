...
contract TestStrategy is IStrategy, StrategyBase {
    ...
    function deposit(
        ...
        _mint(receiptToken, _recipient, _shares, sharesDecimals);
    ...
    function withdraw(
        ...
        uint256 shares = recipients[_recipient].totalShares;
        uint256 initial = shares - (shares * 1e4) / 1e5; //simulate 10% loss
        IERC20(_asset).safeTransfer(_recipient, initial);
    ...
    {
        uint256 initial = recipients[_recipient].totalShares;
        initial = (initial * 1e4) / 1e5; //simulate 10% rewards
        recipients[_recipient].totalShares -= initial;
        totalInvestments -= initial;
        ...
        IERC20(tokenIn).safeTransfer(_recipient, initial);

        emit Rewards(_recipient, amounts, tokens);
...
abstract contract StrategyBase is Ownable, ReentrancyGuard {
    ...
    function _undoComponents(bytes[] calldata _componentsData, address _holding)
        internal
    {
        if (_componentsData.length > 0) {
            for (uint256 i = 0; i < _componentsData.length; i++) {
                (
                    address _componentAsset,
                    address _component,
                    uint256 _rate,
                    IVault.RepaymentStrategyData memory _repaymentData
                        ...
                        (
                            ...
                            address,
                            uint256,
                            IVault.RepaymentStrategyData
                        )
                    );

                IComponent(_component).undo(
                    _componentAsset,
                    _repaymentData,
                    _rate,
                    _holding
