    ...
    AccessControlUpgradeable
{
    ...
    event AdminRoleAdded(
        address indexed superAdmin,
        address indexed adminAddress
    );
    event CFORoleAdded(address indexed superAdmin, address indexed cfoAddress);
    event PermissionGranted(bytes32 permission, address account);
    event PermissionRevoked(bytes32 permission, address account);
    event AdminRoleRevoked(
        address indexed superAdmin,
        address indexed adminAddress
    );
    event CFORoleRevoked(
        address indexed superAdmin,
        address indexed cfoAddress
    );
    ...
    modifier onlyRegisteredToken(address _tokenAddress) {
        require(
            tokenRegister[_tokenAddress].decimals != 0,
            "FC: Token not registered"
        );
        _;
    }
    ...
    modifier onlySuperAdmin() {
        require(
            ...
            "FC: Caller is not an Super Admin "
        );
        _;
    }
    ...
    modifier onlyMasterAdmin() {
        require(
            hasRole(MASTER_ADMIN_ROLE, _msgSender()),
            "FC: Caller is invalid "
        );
        _;
    }
    ...
    /**
     * @dev Adds an admin role to the specified address with the given permissions.
     * @param _address The address to which the admin role will be added.
     */

    function addAdminRole(
        address _address
    ) external onlySuperAdmin notZeroAddress(_address) {
        require(!isAdmin(_address), "FC:Admin role Exist");

        _setupRole(ADMIN_ROLE, _address);
        emit AdminRoleAdded(_msgSender(), _address);
    }
    ...
    */
    ...
    function addCFORole(
        ...
        emit CFORoleAdded(_msgSender(), _address);
    }
    ...
    function revokeAdminRole(
        ...
        emit AdminRoleRevoked(_msgSender(), _address);
    }
    ...
    //     emit PoolAddressChanged(_oldPoolAdddress, _newPoolAddress);
    // }
        ...
        onlyUnregisteredToken(_tokenAddress)
    {
        require(
            ...
            "FC:Supply must be greater than zero"
        );
        require(
            tokenDetails.decimals > 0,
            "FC:Decimals must be greater than zero"
        );
        require(
    ...
    /**
     * @dev Performs a cross-border transfer of tokens.
     *
     * @param _fromTokenAddress The address of the token being transferred from.
     * @param _toTokenAddress The address of the token being transferred to.
     * @param _fromAddress The address from which the tokens are being transferred.
     * @param _toAddress The address to which the tokens are being transferred.
     * @param _fromAmount The amount of tokens to be transferred from the `_fromAddress`.
     * @param _toAmountFxRate The exchange rate between `_fromTokenAddress` and `_toTokenAddress`.
     * @param directTransfer A boolean indicating whether to perform a direct transfer of tokens.
     */
    ...
    function internationalTransfer(
        ...
        uint256 _fromAmount,
        address _poolAddress,
        uint256 _poolTransferAmount,
        address _toTokenAddress,
        address _toAddress,
        uint256 _toAmountFxRate,
        bool directTransfer
    )
        public
        onlyMasterAdmin
        onlyRegisteredToken(_fromTokenAddress)
        onlyRegisteredToken(_toTokenAddress)
        amountGreaterThanZero(_fromAmount)
        amountGreaterThanZero(_toAmountFxRate)
    {
        if (_fromAmount > 0) {
            IToken(_fromTokenAddress).mint(_fromAddress, _fromAmount);
        }

        IToken(_fromTokenAddress)._transferToken(
            _fromAddress,
            _poolAddress,
            _poolTransferAmount
        );
        ...
        if (!directTransfer) {
            IToken(_toTokenAddress).mint(_toAddress, _toAmountFxRate);
        }
    ...
    function domesticTransfer(
        ...
        onlyMasterAdmin
        onlyRegisteredToken(_tokenAddress)
        onlyRegisteredToken(_fromAddress)
        amountGreaterThanZero(_fromAmount)
        amountGreaterThanZero(_toAmount)
    {
        if (_fromAmount > 0) {
            IToken(_tokenAddress).mint(_fromAddress, _fromAmount);
        }

        IToken(_tokenAddress)._transferToken(
            _fromAddress,
            _toAddress,
            _toAmount
        );
    ...
    function unregisterTokens(
        address _tokenAddress
    ) external onlyAdmin onlyRegisteredToken(_tokenAddress) {
    ...
    function isTokenRegistered(
        ...
        return tokenRegister[_tokenAddress].decimals != 0;
    }
    ...
    function platformBlackList(
        ...
        bool _status,
        string calldata _uniqueId
    ) external onlyMasterAdmin notZeroAddress(_userAddress) {
        require(
