...
contract OmniseaDropsScheduler is IOmniseaDropsScheduler, ReentrancyGuard {
    ...
    mapping(address => mapping(uint8 => Phase)) internal phases; // ID => Phase
    mapping(address => mapping(uint8 => mapping(address => uint24))) internal phaseMintedCount; // Phase ID => minted
    ...
    function setPhase(
        ...
        require(_from < _to && block.timestamp < _to);
