...
contract StepsHunter is ChainlinkClient, ConfirmedOwner, AutomationCompatibleInterface {
    ...
    function createGoal(
        ...
        Goal storage goal = goalsMap[goalParams.id];

        require(goal.id != goalParams.id, 'goal is already created');
        ...
        emit LogUint('totalGoalPayment', totalGoalPayment);
        emit LogUint('userBalance', i_link.balanceOf(msg.sender));
        ...
        emit LogUint('stepsHunterUserWalletAllowance', stepsHunterUserWalletAllowance);
        ...
        string memory upkeepName = string(abi.encodePacked('goal#', Strings.toString(goalParams.id)));
         ...
         uint256 upkeepID = registerAndPredictID(
            ...
            abi.encode(goalParams.id),
            5000000000000000000,
            0
        );

        goal.id = goalParams.id;
    ...
    function checkUpkeep(
        ...
        bool upkeepNeededInCheckup = 
            ((block.timestamp - goal.lastDayRecordTimestamp) > interval) &&
