...
contract Vibe is ERC20, Ownable {
    ...
    function sendETHToFee(uint256 totalETH) private {
        uint256 feetotal = _liquidityFee.add(_teamFee).add(_postMintFee);
        uint256 liquidityETH = totalETH.mul(_liquidityFee).div(feetotal);
        uint256 teamAmountETH = totalETH.mul(_teamFee).div(feetotal);
        uint256 postMintETH = totalETH.sub(liquidityETH).sub(teamAmountETH);

        lpWallet.transfer(liquidityETH);
        ...
        // Add funds to postMintCampaign
        IPostMintCampaign.Funds[] memory funds = new IPostMintCampaign.Funds[](1);
        funds[0] = IPostMintCampaign.Funds(ERC20(address(this)), wethBalance);
        postMintCampaign.addFunds(funds);
    }
