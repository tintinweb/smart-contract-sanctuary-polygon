...
contract OrigamiGmxEarnAccount is IOrigamiGmxEarnAccount, Ownable, Operators {
    ...
    IERC20 public override stakedGlp;
    ...
    constructor(address _gmxRewardRouter, address _glpRewardRouter, address _esGmxVester, address _stakedGlp) {
    ...
    function mintAndStakeGlp(uint256 fromAmount, address fromToken, uint256 minUsdg, uint256 minGlp, uint256 slippageBps) external override onlyOperators returns (uint256) {
        IERC20(fromToken).safeIncreaseAllowance(glpRewardRouter.glpManager(), fromAmount);
    ...
    /// @notice Execute is provided for the operator (OrigamiGmxManager),
