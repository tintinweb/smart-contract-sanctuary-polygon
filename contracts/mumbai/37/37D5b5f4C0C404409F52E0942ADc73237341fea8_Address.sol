...
// File: contracts/MurderManHunt.sol

pragma solidity >=0.8.0 <0.9.0;

contract MurderManHunt is ERC721, Ownable {
    using Strings for uint256;
    ...
    uint256 public cost = 143 ether;
    uint256 public marketingSupply = 50;

    Counters.Counter private _tokenSupply;

    event NewMurdererMinted(address sender, uint256 tokenId);
    ...
    function _processMint(address _address) internal {
        uint256 id = _tokenSupply.current();
        _safeMint(_address, id);
        emit NewMurdererMinted(_address, id);
        _tokenSupply.increment();
    }
    ...
    function mint(uint256 _mintAmount) public payable {
        require(
            (!isPaused || msg.sender == owner()) &&
                _mintAmount > 0 &&
                (_mintAmount <= MAX_MINT_AMOUNT || msg.sender == owner()) &&
    ...
    function airdrop(address[] calldata _users) public payable onlyOwner {
        ...
        require(
            marketingSupply > 0 &&
                ...
                _users.length > 0 &&
                (supply + _users.length - 1 <= MAX_SUPPLY),
            marketingSupply <= 0
                ? "Marketing supply is empty"
                : (
                    _users.length > marketingSupply
    ...
    function withdraw() public payable onlyOwner {
        ...
        (bool success, ) = payable(0x28eBb057F382A1883089ada739e9E54B656EBF50)
