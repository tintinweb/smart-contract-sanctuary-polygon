...
function collectionAdmin() external view returns(address){
    return admin;
}

function burnOnDelivery(uint _tokenId, address _to) external onlyAdmin {
    address STO = STOForTokenId[_tokenId];
    require(IERC20(STO).balanceOf(_to)==IFractionToken(STO).maxSupply(),"Address does not own the totalSupply().");
    IFractionToken(STO).burnOnCommand();
    _burn(_tokenId);
}
