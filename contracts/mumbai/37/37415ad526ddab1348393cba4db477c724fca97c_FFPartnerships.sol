...
import './libs/FFPartnerships/Structs.sol';
import './libs/FFPartnerships/Errors.sol';

/**
 * @title FFPartnerships
 * @author Futures Factory
 * @notice ERC1155 for the Futures Factory partnerships
 */
contract FFPartnerships is ERC1155Supply, Ownable, AccessControl {
  using Counters for Counters.Counter;

  event CreateDrop(
    uint256 indexed dropId,
    DropType indexed dropType,
    bytes dropData,
    string dropUri
  );
  ...
  mapping(uint256 => mapping(address => bool)) public dropsClaims;
  ...
  constructor(
    ...
    _setupRole(DROP_CREATOR, _dropCreator);
  }
  ...
  function updateDropUri(uint256 dropId, string memory _dropUri)
    external
  ...
  ) external payable onlyValidMinter {
    Drop memory drop = drops[dropId];
  ...
  function _mintERC721GatedDrop(
    uint256 dropId,
    bytes memory dropData,
    bytes memory mintData
  ) internal {
    ERC721GatedDropMintSettings memory mintSettings = abi.decode(
      mintData,
      (ERC721GatedDropMintSettings)
    );

    ERC721GatedDropSettings memory dropSettings = abi.decode(
      dropData,
      (ERC721GatedDropSettings)
    );
    ...
    if (
      balanceOf(mintSettings.to, dropId) + mintSettings.amount >
      dropSettings.maxPerWallet
  ...
  function _mintPublicDrop(
    uint256 dropId,
    bytes memory dropData,
    bytes memory mintData
  ) internal {
    PublicDropMintSettings memory mintSettings = abi.decode(
      mintData,
      (PublicDropMintSettings)
    );

    PublicDropSettings memory dropSettings = abi.decode(
      dropData,
      (PublicDropSettings)
    );
  ...
  function _mintAllowlistDrop(
    uint256 dropId,
    bytes memory dropData,
    bytes memory mintData
  ) internal {
    AllowlistDropMintSettings memory mintSettings = abi.decode(
      mintData,
      (AllowlistDropMintSettings)
    );

    AllowlistDropSettings memory dropSettings = abi.decode(
      dropData,
    ...
    if (
      MerkleProof.verify(
        mintSettings.proof,
        dropSettings.listMerkleRoot,
        keccak256(abi.encodePacked(mintSettings.to, mintSettings.amount))
      ) == false
    ...
    dropsClaims[dropId][mintSettings.to] = true;
  }
