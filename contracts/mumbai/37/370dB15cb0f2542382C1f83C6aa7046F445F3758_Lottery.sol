...
contract Lottery is VRFConsumerBaseV2, ILottery, Roles {
    ...
    function burnShares(uint _value) external override {
        ...
        sharesPool = sharesPool.sub(_value);
    ...
    function setMyCode(string memory _code) external payable override {
        address sender = _msgSender();
        uint value = msg.value;
        require(value >= 10 ** (decimals - 1), "Insufficient value");
        ...
        payable(manager).transfer(value);
    }
    ...
    function sendWhitelist(address _player, uint _value) external override onlyManager {
        _updateWLQuota(_player, _whitelist[_player] + _value);
        ...
        emit SendWhitelist(_player, _value, account.topAgent);
    }
...
interface ILottery {
    ...
    function setMyCode(string memory _code) external payable;
    ...
    function sendWhitelist(address _player, uint _value) external;
