...
contract OmniCounter is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory _payload) internal override {
        ...
        (
            ...
            item.name,
            item.kg
        ) = abi.decode(_payload, (uint256,string,uint256));
        emit ReceivedData(item.id, item.name, item.kg);
        counter += 1;
    }
    ...
    function estimateFee(uint16 _dstChainId, bool _useZro, bytes calldata _adapterParams) public view returns (uint nativeFee, uint zroFee) {
        Item memory MyStruct = Item(1, "test", counter*2);
        bytes memory encodeData = abi.encode(MyStruct.id, MyStruct.name, MyStruct.kg);
        return lzEndpoint.estimateFees(_dstChainId, address(this), encodeData, _useZro, _adapterParams);
    }

    function incrementCounter(uint16 _dstChainId) public payable {
        Item memory MyStruct = Item(1, "test", counter*2);
        bytes memory encodeData = abi.encode(MyStruct.id, MyStruct.name, MyStruct.kg);
        _lzSend(_dstChainId, encodeData, payable(msg.sender), address(0x0), bytes(""), msg.value);
    }
