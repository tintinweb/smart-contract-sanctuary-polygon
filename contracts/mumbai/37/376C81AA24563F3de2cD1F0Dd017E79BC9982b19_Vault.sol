...
contract Vault{
   ...
   mapping(uint => userRewardsInfo[]) private packageUsersRewards;
   ...
   */
   function transferpackageTokensValue(uint _packageId) external{
      ...
      userRewardsInfo memory _userRewards;
      for(uint i = 0; i < addresses.length; i++){
         if(!userRewardStatus[_packageId][addresses[i]]){
            ...
            _userRewards._userAd = addresses[i];
            _userRewards._reward = userTotalValue;
            packageUsersRewards[_packageId].push(_userRewards);
   ...
   function getPackageUserRewards(uint _packageId) external view returns(userRewardsInfo[] memory){
      return packageUsersRewards[_packageId];
   }
}
