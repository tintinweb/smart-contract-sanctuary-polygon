...
contract RajuToken is Initializable {
    ...
    // fees
    uint256 public _buyliquifierFee;
    uint256 public _buystabilizerFee;
    uint256 public _buytreasuryFee;
    uint256 public _buyblackHoleFee;
    uint256 public _p2pFee;

    uint256 public _sellliquifierFee;
    uint256 public _sellstabilizerFee;
    uint256 public _selltreasuryFee;
    uint256 public _sellblackHoleFee;
    uint256 public _sellmoreSellFee;
    ...
    function runInit() external limited {
        ...
        if((_phase1starttime + _phase1period) >= block.timestamp)
        {
            _buyliquifierFee = 400;
            _buystabilizerFee = 500;
            _buytreasuryFee = 300;
            _buyblackHoleFee = 200;
            _p2pFee = 0;
            _sellliquifierFee = 0;
            _sellstabilizerFee = 0;
            _selltreasuryFee = 0;
            _sellblackHoleFee = 0;
        }
        else if((_phase1starttime + _phase1period) < block.timestamp && (_phase2starttime + _phase2period) >= block.timestamp)
        {
            _buyliquifierFee = 400;
            _buystabilizerFee = 500;
            _buytreasuryFee = 300;
            _buyblackHoleFee = 200;
            _p2pFee = 0;
            _sellliquifierFee = 400;
            _sellstabilizerFee = 500;
            _selltreasuryFee = 500;
            _sellblackHoleFee = 600;
        }
        else if((_phase2starttime + _phase2period) < block.timestamp && (_phase3starttime + _phase3period) >= block.timestamp)
        {
            _buyliquifierFee = 400;
            _buystabilizerFee = 500;
            _buytreasuryFee = 300;
    ...
    function _maxTxCheck(address sender, address recipient, uint r1, uint amount) internal pure {
        ...
        require(impact <= 1000, "buy/sell/tx should be lower than criteria(10%)"); // _maxTxNume(antiwhale)
    }
    ...
    function accuTaxSystem(uint amount) internal {
		...
		if (curcuitBreakerFlag_ == 2) { // circuit breaker activated
			if (_curcuitBreakerTime + 1200 < block.timestamp) { // certain duration passed. everyone chilled now?
		...
		
        {
            ...
            if (timeDiffGlobal < 1200) { // still in time window
    ...
    function _swapBack(uint r1) private returns (uint) {
        ...
        // save gas
        uint liquifierFee = _buyliquifierFee + _sellliquifierFee;
        uint stabilizerFee = _buystabilizerFee + _sellstabilizerFee;
        uint treasuryFee = _buytreasuryFee + _selltreasuryFee + _p2pFee; // handle sell case
        uint blackHoleFee = _buyblackHoleFee + _sellblackHoleFee;
    ...
    function _takeFee(address sender, address recipient, uint256 fAmount) internal returns (uint256) {
        ...
        uint liquifierFee = _buyliquifierFee + _sellliquifierFee;
        uint stabilizerFee = _buystabilizerFee + _sellstabilizerFee;
        uint treasuryFee = _buytreasuryFee + _selltreasuryFee;
        uint blackHoleFee = _buyblackHoleFee + _sellblackHoleFee;
    ...
    function setphase1starttime(uint256 phase1starttime) external limited {
        require(phase1starttime > block.timestamp);
    ...
    function setphase2starttime(uint256 phase2starttime) external limited {
        require((_phase1starttime + _phase1starttime) <= phase2starttime);
    ...
    function setphase3starttime(uint256 phase3starttime) external limited {
        require((_phase2starttime + _phase2starttime) <= phase3starttime);
