...
/// @author Coinvise
contract MembershipsFactory is Ownable {
    ...
    event MembershipsDeployed(address membershipsClone, address indexed owner, address indexed implementation);
    ...
    function deployMemberships(
        ...
        IMemberships(membershipsClone).initialize(
            _owner,
            _treasury,
            _name,
            _symbol,
        ...
        if (_membership.airdropToken != address(0) && _membership.airdropAmount != 0) {
            // if cap == 0, memberships is uncapped, can have unlimited supply of memberships
            // so require transferring airdrop tokens for at least one purchase
            // Creator needs to refill the Membership contract with more when it runs out
            if (_membership.cap == 0)
                IERC20(_membership.airdropToken).safeTransferFrom(
                    msg.sender,
                    membershipsClone,
                    _membership.airdropAmount
                );
            else
                IERC20(_membership.airdropToken).safeTransferFrom(
                    msg.sender,
