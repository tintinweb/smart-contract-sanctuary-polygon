...
contract PlushGetTree is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function addTreeType(string memory _type, uint256 _price, uint256 _count) external onlyRole(DEVELOPER_ROLE) {
        require(!treeMap[_type].isValid, "This type of tree already exists");

        treeMap[_type] = Tree(true, _type, _price, _count);
    }

    function removeTreeType(string memory _type) external onlyRole(DEVELOPER_ROLE) {
    ...
    function setTreeTypePrice(string memory _type, uint256 _price) external onlyRole(DEVELOPER_ROLE) {
        require(treeMap[_type].isValid, "Not a valid tree type.");
        require(_price > 0);
        treeMap[_type].price = _price;
    }

    function setTreeTypeCount(string memory _type, uint256 _count) external onlyRole(DEVELOPER_ROLE) {
        require(treeMap[_type].isValid, "Not a valid tree type.");
