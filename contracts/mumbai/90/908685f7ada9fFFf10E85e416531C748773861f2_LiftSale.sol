...
contract LiftSale {
    constructor(address token) {
        minBuy = 10**9; // 1 gwei
        maxBuy = 10**18; // 1 ether
        liftToken = IERC20Metadata(token);
        owner = payable(msg.sender);
        liftDecimals = liftToken.decimals();
        maticDecimals = 18;
    }
    ...
    uint8 private liftDecimals;
    uint8 private maticDecimals;

    function buy1pra1() public payable returns (bool) {
        require(
            maxBuy >= msg.value,
            "Por favor, tenha pena do seu rico dinheiro..."
        );
        require(
            minBuy <= msg.value,
            "Por favor, n\xC3\xA3o seja t\xC3\xA3o m\xC3\xA3o de vaca, pelo menos 1 gwei..."
        );

        liftToken.transferFrom(
            owner,
            msg.sender,
