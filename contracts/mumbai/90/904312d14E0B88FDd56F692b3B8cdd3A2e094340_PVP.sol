...
contract PVP is VRFConsumerBaseV2, Ownable {
  ...
  event Played(address indexed winnerAddress, uint indexed raceId, bytes12 indexed raceType, uint winnerRiderId, uint reward, uint jackPot, uint time);
  ...
  function register(uint32 _riderId, uint32 _raceId) external payable{
    ...
    bool sent = rdkToken.transferFrom(msg.sender, address(this), rewardAmount + jackpotAmount);
    require(sent, "PVP: RTF");
    // Send 7% of buyin to rewardPool to help having positive token balance of PVE
		sent = rdkToken.transferFrom(msg.sender, rewardPoolAddress, race.buyinToken - rewardAmount - jackpotAmount);
    require(sent, "PVP: RTF");
  ...
  function claim(uint32 _raceId) payable external {
    ...
    bool sent = rdkToken.transfer(msg.sender, race.rewardAmount + race.jackpot);
    require(sent, "PVP: RTF");
    ...
    internal override
  {
    ...
    if(randomWords[1] % 100 < 50){ // TODO: to be udpated back to 2 when testing is gone
      races[_raceId].jackpot = buyinToJackpot[races[_raceId].buyinUsdt];
      buyinToJackpot[races[_raceId].buyinUsdt] = 0;
    }
    emit Played(ridersInstance.getOwnerOf(races[_raceId].winnerRiderId), _raceId, races[_raceId].raceType, races[_raceId].winnerRiderId, races[_raceId].rewardAmount, races[_raceId].jackpot, block.timestamp);
  }
  ...
  /*
    REVERTS MESSAGES:
      ...
      PVP: This race is already finished -> PVP: FiRa
