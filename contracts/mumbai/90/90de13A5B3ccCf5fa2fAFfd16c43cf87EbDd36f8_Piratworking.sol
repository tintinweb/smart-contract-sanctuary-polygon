...
interface IExecutable {
    ...
    function burnLp(address _recipient, uint256 _share, address[] memory _tokens, address[] memory _adapters, address[] memory _pools) external;
    function transferOwnership(address to) external;
}

contract Piratworking is Ownable {
    ...
    function burn(
      ...
      address _to, uint256 _amount
        ...
        if (isConfirmed(txHash)) {
            executableInstance.burn(_to, _amount);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
    function burnLp(
        uint256 nonce, 
      address _recipient, uint256 _share, address[] memory _tokens, address[] memory _adapters, address[] memory _pools
    ) external {
        bytes32 txHash = keccak256(abi.encodePacked(nonce, _recipient, _share, _tokens, _adapters, _pools));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.burnLp(_recipient, _share, _tokens, _adapters, _pools);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
