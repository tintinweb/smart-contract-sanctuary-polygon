...
contract VRFBeacon is VRFBeaconOCR, VRFBeaconDKGClient {
    ...
    mapping(address => bytes[]) private enrollmentCiphers;
    ...
    function postCipher(
        ...
        enrollmentCiphers[user].push(cipher1);
        enrollmentCiphers[user].push(cipher2);
    }

    function getEnrollmentCiphers(address user) external view returns (bytes[] memory) {
        return enrollmentCiphers[user];
    }
}
