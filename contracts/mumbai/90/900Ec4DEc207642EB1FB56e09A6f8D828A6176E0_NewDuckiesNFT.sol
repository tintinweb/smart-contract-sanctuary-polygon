    ...
    RandomUpgradeable
{
    ...
    function meld(uint256[5] calldata meldingTokenIds) external UseRandom returns (uint256) {
    ...
    function _meldTraits(uint256[5] memory genes, Gene.Traits trait) internal returns (uint8) {
        ...
        if (meldedTraitIdx == 5) {
            ...
            return bestTraitValue == traitWeights[uint8(trait)].length - 1 ? bestTraitValue : bestTraitValue + 1;
        }
    ...
    function _requireCorrectMeldingClasses(uint256[5] memory genes) internal pure {
        ...
        for (uint256 i = 1; i < genes.length; i++) {
            require(genes[i].getTrait(Gene.Traits.Class) == uint8(class), 'melding Classes must be equal');
        }

        require(class != Gene.Classes.SuperLegendary && class != Gene.Classes.Zombie, 'wrong Class to meld');
    }
    ...
    function _requireCorrectMeldingTraits(uint256[5] memory genes) internal pure {
        ...
        for (uint256 i = 1; i < genes.length; i++) {
            require(genes[i].getTrait(Gene.Traits.Background) == color, 'melding legendary Backgrounds must be equal');
        ...
        require(elementsPresentBitfield == allElementsBitfield, 'mending legendary requires all Elements');
    }
    ...
    function _burnTokensAndGetGenes(uint256[5] memory tokenIds) internal returns (uint256[5] memory) {
