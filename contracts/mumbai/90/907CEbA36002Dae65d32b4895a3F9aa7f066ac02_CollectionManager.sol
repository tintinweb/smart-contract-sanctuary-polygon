...
contract CollectionManager is ICollectionManager, Ownable, Base {
        ...
        returns (address newCol)
    {
        ...
        collectionCreationData[newCol] = CreationData(address(collectionImpl));

        ICollectionBase(newCol).initialize(_vars);

        emit CollectionCreated(newCol, address(collectionImpl));
    }
...
interface ICollectionManager {
    ...
    function createCollection(bytes memory data)
        external
        returns (address newCol);
