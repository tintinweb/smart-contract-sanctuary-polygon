...
contract FactoryFacet is Modifiers {
    ...
    function getBBHToken() public view returns(address){
        return address(BigBoardManagerLib.getFactory().hostToken);
    }

    function isFounder(address pFounder) public view returns (bool){
        return BigBoardManagerLib.getFactory().founderz[pFounder];
    } 

    function getGameIds(uint256 index) external view returns(string memory){
        return BigBoardManagerLib.getFactory().gameIds[index];
    }

    function getBoardCount() external view returns(uint256){
        return BigBoardManagerLib.getApprovedBoardCount();
    }

    function getBoard(uint256 i) external view returns(address){
        return BigBoardManagerLib.getApprovedBoard(i);
    }
}
...
library BigBoardManagerLib {
    ...
    function getApprovedBoardCount() internal view returns(uint256){
        Factory storage f = getFactory();
        return f.approvedBoardContracts.length;
    }

    function getApprovedBoard(uint256 index) internal view returns(address){
         Factory storage f = getFactory();
        return f.approvedBoardContracts[index];
    }
