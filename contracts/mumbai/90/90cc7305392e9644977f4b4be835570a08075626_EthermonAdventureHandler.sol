...
interface EtheremonMonsterNFTInterface {
    ...
    function getMonsterCurrentStats(
        uint64 _monsterId
    )
        external
        view
...
interface EtheremonWorldNFT {
    function STAT_MAX_CLASS(
        uint32 _classId
    ) external returns (uint256 _statMax);
}
...
contract EthermonAdventureHandler is BasicAccessControl, EthermonEnum {
    ...
    function handleMultipleItems(
        ...
        uint8 statMax = uint8(
            (_statMaxClass > 0) ? (_statMaxClass) : STAT_MAX_VALUE
        );
        // // Make it to +3
    ...
    function getMonCurrentSeasonExp(
        uint64 _monId,
        uint32 _season
    ) external returns (uint256) {
        EthermonSeasonDataInterface seasonData = EthermonSeasonDataInterface(
    ...
    function monsterWorldTokenOfOwnerByIndex(
        address _trainer,
        uint256 _index
    )
        public
