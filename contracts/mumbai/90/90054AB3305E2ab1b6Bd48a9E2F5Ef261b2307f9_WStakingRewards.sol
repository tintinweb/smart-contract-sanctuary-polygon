...
contract WStakingRewards is FeeBase, ERC20, ReentrancyGuard, IERC20Wrapper {
  ...
  function accumulatedRewards() external pure override returns (uint256) {
    return 1;
  }

  function accumulatedRewardsFor(address _user) external pure override returns (uint256) {
    require(_user != address(0), 'Fake addr');
    return 2;
  }
}
