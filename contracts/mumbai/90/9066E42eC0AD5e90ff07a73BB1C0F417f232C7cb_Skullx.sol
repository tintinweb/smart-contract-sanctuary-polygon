...
contract Skullx is ERC721Enumerable, Ownable, ReentrancyGuard {
  ...
  AeonsContract private aeonsContract;
  ...
  address private w0 = 0xF65b1bC72ffe0c8BcbC91da87abc53aC8Cf884FD;
  address private w1 = 0x7057092E6fB32683c7aB292420fFFe11B9929a9b;
  address private w2 = 0x74F963A0741BCc06112AFAb7B0863eA2Ce405e8C;
  address private w3 = 0xE0D0c735A5779919363dB1DEEE102fa5536C93fE;
  ...
  function summon(
    uint256 numSkullx
  ) external payable whenPublicSaleActive nonReentrant {
    uint256 supply = totalSupply();
    require(numSkullx <= SUMMON_LIMIT, "Exceeds SUMMON_LIMIT");
    require(
      supply + numSkullx <= MAX_SKULLX - reservedLeft,
      "Exceeds MAX_SKULLX"
    );
  ...
  function getMintPrice() public view whenPublicSaleActive returns (uint256) {
    ...
    if (elapsed >= publicSaleDuration) {
      ...
      return
        currentPrice > SKULLX_BASE_PRICE ? currentPrice : SKULLX_BASE_PRICE;
    }
  }
  ...
  function aeonsPresaleSummon(
    uint256[] calldata aeonIds
    ...
    for (uint256 i = 0; i < aeonIds.length; i++) {
      require(
        aeonsContract.ownerOf(aeonIds[i]) == msg.sender,
        "Not the Aeon owner"
      );
    ...
    require(
      supply + aeonIds.length <= MAX_SKULLX - reservedLeft,
      "Exceeds MAX_SKULLX"
    );
    require(
      SKULLX_BASE_PRICE * aeonIds.length <= msg.value,
  ...
  // ----------------------------------
  function addToOriginsList(
    address[] calldata addresses,
    uint256[] calldata numOrigins
  ) external onlyOwner {
    require(
      addresses.length == numOrigins.length,
  ...
  function removeFromOriginsList(
    address[] calldata addresses
  ) external onlyOwner {
    for (uint256 i = 0; i < addresses.length; i++) {
      require(addresses[i] != address(0), "Can't add the null address");
  ...
  function getOriginsListClaimAvailable(
    address owner
  ...
  function originsFreeSummon(uint256 numSkullx) external payable nonReentrant {
    uint256 supply = totalSupply();
    require(
      originsFreeMintActive,
      "Free summon phase for Skullx role is not active"
    );
    require(
      originsListClaimAvailable[msg.sender] >= numSkullx,
      "Exceeds free mints available"
    );
    require(
      supply + numSkullx <= MAX_SKULLX - reservedLeft,
      "Exceeds MAX_SKULLX"
