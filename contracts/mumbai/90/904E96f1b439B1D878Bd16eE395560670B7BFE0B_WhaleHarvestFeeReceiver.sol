...
import './interfaces/IWhaleHarvestFactory.sol';
import './interfaces/IWhaleHarvestPair.sol';
import './interfaces/IWETH.sol';
import './libraries/TransferHelper.sol';
import './libraries/SafeMath.sol';


contract WhaleHarvestFeeReceiver {
    using SafeMath for uint;

    address public owner;
    IWhaleHarvestFactory public factory;
    address public WETH;
    ...
    constructor(
        ...
        owner = _owner;
        factory = IWhaleHarvestFactory(_factory);
        WETH = _WETH;
    ...
    function transferOwnership(address newOwner) external {
        require(msg.sender == owner, 'WhaleHarvestFeeReceiver: FORBIDDEN');
        owner = newOwner;
    }
    ...
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'WhaleHarvestFeeReceiver: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'WhaleHarvestFeeReceiver: ZERO_ADDRESS');
    }
    ...
    // Taken from WhaleHarvestLibrary, removed the factory parameter
    ...
    // Done with code form WhaleHarvestRouter and WhaleHarvestLibrary, removed the deadline argument
    function _swapTokensForETH(uint amountIn, address fromToken)
        internal
    {
        IWhaleHarvestPair pairToUse = IWhaleHarvestPair(pairFor(fromToken, WETH));
        ...
        require(reserveIn > 0 && reserveOut > 0, 'WhaleHarvestFeeReceiver: INSUFFICIENT_LIQUIDITY');
    ...
    // Take what was charged as protocol fee from the WhaleHarvest pair liquidity
    function takeProtocolFee(IWhaleHarvestPair[] calldata pairs) external {
