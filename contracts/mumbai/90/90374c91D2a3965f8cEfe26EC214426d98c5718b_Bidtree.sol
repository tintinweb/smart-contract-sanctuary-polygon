...
contract Bidtree is Ownable {
    ...
    Counters.Counter _countOffsetBids; // Compensated bids number counter
    ...
    uint private _multiplier; // The multiplier of the reserve currency price during the bet, when exceeding it, it is allowed to compensate for the bets before completing
    ...
    /// Structures
    struct Bid {
        uint128 _amount; // Contribution amount
        uint128 _price; // Reserved currency price at the the share of the bank per user after (determined after the completing of the gamecontribution moment
        bool _returned; // Event indicator for the contribution refund
    }

    struct Contribution {
        uint _amountContributions; // Total user's contribution amount
        uint _amountIncome; // Total user's income amount (referals and compensations)
        uint _activeReferrals; // Active refaral links counter
    ...
    constructor(
        ...
        uint waitingTime,
        uint depositAmount,
        uint percentageOwner,
        uint percentageMarketing,
        uint percentageLottery,
        uint percentageReferral,
        uint percentageFund
    ) {
        _usd = usd;
        _bitcoin = bitcoin;
    ...
    /// @param multi - the number of bids
    ...
    /// @notice Contribute
    /// @param multi - the number of bids
    /// @param base - bid indicator (base/referral)
    /// @param referral - referral address (if it is a base bid then the address is not applied, you can put any or null)
    function contribute(uint multi, bool base, address referral) external {
        ...
        locker = false;
        emit Contributed(msg.sender, referral, _deposit, _referral, _fond, _lottery, _marketing, _toOwner, _refund);
    }
