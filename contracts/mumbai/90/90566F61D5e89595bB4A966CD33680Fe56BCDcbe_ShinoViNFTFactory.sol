...
contract ShinoViNFT is ERC721, ERC721Burnable, ERC721URIStorage, Ownable {
    ...
    constructor(
        ...
        minters[0xb06F50Cd017f9D351F8101f206ADA392811C2B51] = true;
    }

    function safeMint(address to, string memory uri) public onlyOwner {
        require(to != address(0), "To address must not be null");
        require(tokens[uri].length > 0);
        uint index = tokens[uri].length-1;
        uint256 tokenId = tokens[uri][index].id;
        tokens[uri].pop();
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, uri);
    }

    function safeMintPublic(address to, string memory uri) public payable isMinter {
        require(to != address(0), "To address must not be null");
        require(tokens[uri].length > 0);
        uint index = tokens[uri].length-1;
        require(msg.value >= tokens[uri][index].price, "Mint price not met!");
        uint256 tokenId = tokens[uri][index].id;
    ...
    function addToCollection(token[] memory _tokens) public onlyOwner {
        for (uint i = 0; i < _tokens.length; i++) {
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            tokens[_tokens[i].uri].push(_tokens[i]);
        }
    }
    ...
    modifier isMinter() {
        require(minters[msg.sender] == true, "access denied");
        _;
    }
...
contract ShinoViNFTFactory is Initializable {
    ...
    function removeNFT(address _contract) isAdmin public returns (address[] memory) {
    ...
    function addShinoViAdmin(address _address) isOwner public returns (bool) {
        shinoViAdmins[_address] = true;
        return true;
    }

    function removeShinoViAdmin(address _address) isOwner public returns (bool) {
        delete shinoViAdmins[_address];
        return true;
    }

    function isShinoViCreator() external view returns (bool) {
        return shinoViCreators[msg.sender];
    }

    function isShinoViAdmin() external view returns (bool) {
        return shinoViAdmins[msg.sender];
    }
