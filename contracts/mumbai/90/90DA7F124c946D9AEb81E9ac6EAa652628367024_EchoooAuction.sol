    ...
    Pausable,Initializable,UUPSUpgradeable
{
    ...
    function cancel(
        ...
        uint256 _highPrice = 0;
        if(order.executionDelegate.delegateType() == 1){
            address _owner = IERC721(order.tokenAddress).ownerOf(order.tokenId);
            _highPrice = _bidlisting721s[order.tokenAddress][order.tokenId][_owner].highPrice;
            require(_highPrice == 0,'there is a bidder,cannot cancel');
        }else if(order.executionDelegate.delegateType() == 2){
            _highPrice = _bidlisting1155s[order.tokenAddress][order.tokenId][order.user].highPrice;
            require(_highPrice == 0,'there is a bidder,cannot cancel');
        }
