...
contract RealtyOracleTangible is RWAGenericOracle, PriceConverter {
    ...
    function marketPriceNativeCurrency(uint256[] calldata fingerprints)
        public
        view
        returns (uint256[] memory nativePrices, string memory currency)
    {
        ...
        for (uint256 i = 0; i < length; i++) {
            nativePrices[i] =
                oraclePrices[fingerprints[i]].weSellAt +
                additionals[fingerprints[i]];
        }
    }
}
