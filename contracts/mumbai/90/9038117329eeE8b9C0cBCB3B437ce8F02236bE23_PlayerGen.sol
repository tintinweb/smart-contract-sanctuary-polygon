...
contract PlayerGen is ERC721URIStorage {
    ...
    function generatePlayer(uint256 tokenId) public returns (string memory) {
        bytes memory svg = abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350">',
            "<style>.base { fill: white; font-family: serif; font-size: 18px; }</style>",
            "<style>.name { fill: white; font-family: serif; font-size: 24px; }</style>"
    ...
    function mint(string memory _name) public {
        ...
        tokenIdToAbilities[newItemId].Shooting = 0;
        tokenIdToAbilities[newItemId].Passing = 5;
        tokenIdToAbilities[newItemId].Tackling = 5;
        tokenIdToAbilities[newItemId].Saves = 10;
        _setTokenURI(newItemId, getTokenURI(newItemId));
    }

    // Practice All
    function practiceAll(uint256 tokenId) public {
        ...
        tokenIdToAbilities[tokenId].Shooting = currentShooting + 1;

        uint256 currentPassing = tokenIdToAbilities[tokenId].Passing;
        tokenIdToAbilities[tokenId].Passing = currentPassing + 1;
    ...
    // Practice Shooting
    function practiceShooting(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing player-token");
        require(
            ownerOf(tokenId) == msg.sender,
    ...
    // Practice Passing
    function practicePassing(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing player-token");
        require(
            ownerOf(tokenId) == msg.sender,
    ...
    // Practice Tackling
    function practiceTackling(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing player-token");
        require(
            ownerOf(tokenId) == msg.sender,
    ...
    // Practice Saves
    function practiceSaves(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing player-token");
        require(
            ownerOf(tokenId) == msg.sender,
