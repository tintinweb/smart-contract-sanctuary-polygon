...
contract Escrow is Initializable,Ownable,Signature{
    ...
    event ContractCreated(address _from, address _to, uint _dueDate, uint _amount);
    ...
    function initialize(
        ...
        require(_end_time > block.timestamp,"end time can't be less");
        ...
        account_info.time.init = block.timestamp;
        account_info.time.end = _end_time;
        account_info.content_id = cid;
        account_info.amount = msg.value;
        account_info.status = Status.in_progress;
        
        emit ContractCreated(_client, _freelancer, _end_time, msg.value);
    }
...
contract EscrowFactory is Escrow{
        ...
        event ContractCreated(address _to);
