...
// File: contracts/Market 2/marketmarket.sol
...
contract WalletMonstersMarketplace {
    ...
    event itemAdded(uint256 id, uint256 tokenId, address tokenAddress,uint amount ,uint256 askingPrice, uint256 date);
    event itemSold(uint256 id, address buyer,uint256 amount, uint256 askingPrice, uint256 date);
    ...
    function Randomness() private view returns (uint) {
      ...
      function approveContract(bool status, address contractAddy) public returns(bool success){

            ERC1155 tokenContract = ERC1155(contractAddy);
            tokenContract.setApprovalForAll(address(this), status);
            
            return success;

      }

      function additemToMarket(uint256 tokenId, address tokenAddress, uint256 units, uint256 askingPrice) OnlyItemOwner(tokenAddress,tokenId) HasTransferApproval(tokenAddress) public returns(uint256){
            ...
            emit itemAdded(newItemId, tokenId, tokenAddress, units ,askingPrice, block.timestamp);
     ...
     function buyItem(uint256 id) payable public ItemExist(id) IsForSale(id) {
            ...
            ERC1155(itemsForSale[id].tokenAddress).safeTransferFrom(itemsForSale[id].seller, msg.sender, itemsForSale[id].tokenId, itemsForSale[id].units, '');
            itemsForSale[id].seller.transfer(msg.value); //send funds to seller
           
            emit itemSold(id, msg.sender, itemsForSale[id].units ,itemsForSale[id].askingPrice, block.timestamp);
