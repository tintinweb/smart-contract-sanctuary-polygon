...
contract HyperMintERC1155 is ERC1155, Ownable, ERC1155Burnable {
    string public name;
    ...
    constructor (string memory _name, string memory _symbol, string memory _uri, bool _allowBuy,
    ...
    function setPriceAndTotalSupply(uint256 id, uint256 _price, uint256 _supply) public onlyOwner {
    ...
    function addTokens(uint256[] memory newSupplies, uint256[] memory newPrices) public onlyOwner {
    ...
    function _buy(uint256 id, uint256 amount) internal {
        ...
        if(purchaseTokenAddress == address(0)){
            ...
            token.transferFrom(msg.sender, primaryRoyaltyReceiver, royaltyAmount);
            token.transferFrom(msg.sender, customerAddress, saleAmount - royaltyAmount);
        }
    }
    ...
    function buyPresale(uint256 id, uint256 amount, uint8 _v, bytes32 _r, bytes32 _s) external payable {
        require(allowBuy, "Buy disabled");
