...
contract Groups is ERC721, ERC721Enumerable, Pausable, Ownable, ReentrancyGuard {
    ...
    mapping (uint256 => GroupDetails) groupDetails;

    // Group Address to Group ID mapping
    mapping(address => uint256) groupAddressToID;
    ...
    uint256 public costToMintTier1 = 100 ether;
    uint256 public costToMintTier2 = 10 ether;
    uint256 public costToMintTier3 = 5 ether;
    uint256 public costToMintTier4 = 1 ether;
    ...
    */
    ...
    function getOwnerOfGroup(uint256 groupID) public view returns (address){
    ...
    function leaveGroup(uint256 groupID) public onlyGroupMembers(groupID, msg.sender) nonReentrant{
        ...
        isMemberOf[msg.sender][groupID] = false;

        // Emit to the logs for external reference
        emit logLeaveGroup(groupID, msg.sender);
    }

    function addMemberToGroup(uint256 groupID, address member) public onlyGroupOwners(groupID) nonReentrant{
        require(groupDetails[groupID].memberCount < maxMembersPerGroup, "You have reached the max amount of members for this group");

        // Add them to the group
        isMemberOf[member][groupID] = true;

        // Add to the group member count
        groupDetails[groupID].memberCount ++;
    ...
    function removeMemberFromGroup(uint256 groupID, address member) public onlyGroupOwners(groupID) nonReentrant{
        ...
        // Remove from the group member count
        groupDetails[groupID].memberCount --;
    ...
    function mintGroup(string calldata groupName) public payable nonReentrant {
        ...
        groupDetails[groupID].ownerAddress = msg.sender;
        ...
        groupDetails[groupID].baseURI = append(baseURIPrefix, KUtils.addressToString(msg.sender),'/',KUtils.toString(groupID),'.json');
        ...
        groupAddressToID[groupAddress] = groupID;
        groupDetails[groupID].groupAddress = groupAddress;
    ...
    function tokenURI(uint256 _tokenID) public view virtual override(ERC721) returns (string memory) {
        return groupDetails[_tokenID].baseURI;
    }
