...
contract Domains is ERC721URIStorage {
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10**16; // 0.0005 MATIC = 5 00 000 000 000 000 000 (17 decimals). We're going with 0.0005 Matic cause the faucets don't give a lot
        } else if (len == 4) {
            return 3 * 10**16; // To charge smaller amounts, reduce the decimals. This is 0.0003
        } else {
            return 1 * 10**16;
        }
    }
