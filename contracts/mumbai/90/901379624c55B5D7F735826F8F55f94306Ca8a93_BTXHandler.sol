...
contract BTXHandler is OwnableUpgradeable {
    ...
    event BTXMinted(address indexed user, uint256 amount, string mintTag);
    ...
    function mintBTX(
        ...
        IBTXToken(btxToken).mint(to, amount);
        emit BTXMinted(to, amount, mintTag);
    }

    function mintBTXReward(
        address to,
        uint256 amount,
        string memory mintTag
    ) public onlyOwner {
        IBTXToken(btxToken).mintReward(to, amount);
        emit BTXRewardMinted(to, amount, mintTag);
    }
    ...
    function burnBTX(
        ...
        IBTXToken(btxToken).burnFrom(user, amount);
        emit BTXBurned(user, amount, burnTag);
    }

    function burnBTXReward(
        address user,
        uint256 amount,
        string memory burnTag,
        uint8 v,
        ...
        IBTXToken(btxToken).burnUsingRewardsFrom(user, amount);
        emit BTXBurned(user, amount, burnTag);
    }
