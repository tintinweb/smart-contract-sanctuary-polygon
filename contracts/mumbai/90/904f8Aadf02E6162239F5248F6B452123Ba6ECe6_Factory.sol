    ...
    AccessControlUpgradeable
{
    ...
    function mintLoanNft(
        ...
        if (_details.isRefinance) {
            ...
            if (
                ...
                IPool(pools[_details.bank]).transferAuctionedRewards(
                    _details.bank,
                    _details.refinanceTokenID
                );
    ...
    function auctionStakedAmountTransfer(
        ...
        require(NFT.nftStatus(_loanId) == false, "NFT Freezed");
    ...
    function poolLoanTransfer(
        ...
        for (uint256 i = 0; i < _lenders.length; i++) {
            require(NFT.nftStatus(_ids[i]) == false, "NFT Freezed");
     ...
     * @param auctionreward boolena to check is this for auction Reward transfer
     * @param _borrower address of borrower
     * @param _loanId token Id of the loan
     * @param _auctionWinner address of auction winner
     * @param _isNormalRefinance boolena to check if this is normal refinance or not
     **/
    ...
    function prepaidPazaBuy(
        ...
        uint pazaAmount;
        if (auctionreward) {
            receiver = pools[_auctionWinner];
            LoanLib.Loan memory temp = IPool(pools[_lender]).getUserDetails(
                _borrower,
                _loanId
            );
            pazaAmount = temp.discount - temp.borrowerShare;
            IPool(pools[_auctionWinner]).updateAuctionedRewardAmount(
                _loanId,
                pazaAmount
            );
        } else if (_isNormalRefinance) {
...
interface IPool {
    ...
    function updateAuctionedRewardAmount(uint _loanId, uint _amount)external;
    function transferAuctionedRewards(address _bank, uint _loanId) external;
