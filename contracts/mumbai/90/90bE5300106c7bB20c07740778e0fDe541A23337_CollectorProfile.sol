...
import "./RandomColorGenerator.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/utils/Base64.sol";

contract CollectorProfile is RMRKNestable, Ownable  {
    ...
    function createProfile() public {
        ...
        //string memory color = RandomColorGenerator.generateColorCode(tokenId);
    ...
    function generateTokenId(address sender_) public pure returns (uint256){
        return uint256(uint160(sender_));
    }
    

    function generateSVG(uint256 tokenId) public pure returns(string memory){
        bytes memory svg = abi.encodePacked(
            '<svg xmlns="http://www.w3.org/2000/svg" fill="#', RandomColorGenerator.generateColorCode(tokenId) ,'">',
...
library RandomColorGenerator {
    function generateColorCode(uint256 seed) internal pure returns (string memory) {
        bytes32 hash = keccak256(abi.encodePacked(seed));
        bytes memory buffer = new bytes(6);
        for (uint256 i = 0; i < 3; i++) {
            uint8 value = uint8(hash[i]);
            buffer[i * 2] = byteToHex(value >> 4);
            buffer[i * 2 + 1] = byteToHex(value & 0x0f);
        }
        return string(buffer);
    }

    function byteToHex(uint8 byteValue) internal pure returns (bytes1) {
        if (byteValue < 10) {
            return bytes1(byteValue + 0x30);
        } else {
            return bytes1(byteValue + 0x57);
        }
    }
}