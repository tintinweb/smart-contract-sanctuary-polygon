...
contract InfinitEquity_Crossmint is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable, AccessControl {
	...
	address public USDCAddress = 0xFEca406dA9727A25E71e732F9961F680059eF1F9;
	...
	function crossmint(address _to, string memory _referralTest) public payable {
		
		require(msg.sender == crossmintAddress, "This function is for Crossmint only.");
		require(IERC20(USDCAddress).balanceOf(msg.sender) >= price, "Insufficient USDC Funds.");
		
		total++;
		
		referralTest = _referralTest;
		
		IERC20(USDCAddress).safeTransferFrom(_msgSender(), address(this), price);

		// Mint the token
        _lockedMint(_to, total);
