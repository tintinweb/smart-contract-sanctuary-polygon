...
contract SMWHexGen1Bags is ERC721A, ERC2981, DefaultOperatorFilterer, Ownable {
    uint256 public MAX_SUPPLY = 111;
    uint256 public MINT_PRICE = 0 ether;
    uint256 public MAX_PER_WALLET = 1;

    string public baseURI = "ipfs://QmYQCuB8RaL8mCrjZcSRsZQADGRSewTgUpAQUAXgHUS1YS/";
    ...
    constructor() ERC721A("Stealth Meta Wear - HEX Gen1", "SMWHB") {}
...
contract NFTStaking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 rewardmath = 10000 ether;
    bool Pool_Pause = false;
    ...
    mapping(address => uint256) public initialRewards;
    mapping(uint256 => bool) public claimInitial;

    constructor(SMWHexGen1Bags _nft, IERC20 _token) {
        nft = _nft;
        token = _token;
    }

    function stake(uint256[] calldata tokenIds) external nonReentrant {
        require(!Pool_Pause, "Staking Pool is Paused.");
        uint256 tokenId;
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            if (claimInitial[tokenId] == false) {
                initialRewards[msg.sender] = 50 ether;
            }
            claimInitial[tokenId] = true;
    ...
    function _claim(
        ...
        uint256 tokenId;
        uint256 earned = 0;
        uint256 reward = rewardmath;

        for (uint256 i = 0; i < tokenIds.length; i++) {
            tokenId = tokenIds[i];
            Stake memory staked = vault[tokenId];
            require(staked.owner == account, "not an owner");
            uint256 stakeListReward = initialRewards[account];
            if (stakeListReward > 0) {
            initialRewards[msg.sender] = 0;
            earned += stakeListReward;
        }
            
            uint256 stakedAt = staked.timestamp;
            reward = (100 ether * (block.timestamp - stakedAt)) / 86400;
            earned = reward / 100;
            vault[tokenId] = Stake({
                owner: account,
                tokenId: uint24(tokenId),
    ...
    function getSMWBalance () external view returns (uint256) {
        uint256 balance = IERC20(token).balanceOf(address(this));
        return balance;
    }

    function withdraw_token(address tokenAddress, uint256 _amount) public onlyOwner {
