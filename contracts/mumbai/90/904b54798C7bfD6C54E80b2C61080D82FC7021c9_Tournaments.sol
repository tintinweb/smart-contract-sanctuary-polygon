...
using SafeMath for uint;
    ...
    struct Tournament{
        ...
        uint status; //0 announced 1 registration  2 started 3 distribute rewards 4 finished  7 canceled
    ...
    address bangerAddress=0x2E48479cDDEF205B2df644d397358c45105969f6; //TEMP

    function createTournament(string memory _name, uint _pricePerPlayer, uint _playersPerTeam, uint _numberOfTeams,address _prizeToken, uint _daysTillRevert, address _publisherAddress) public onlyOwner returns (uint){
            ...
            Tournament memory t = Tournament(tournamentId,_name, msg.sender,_pricePerPlayer,_playersPerTeam,_numberOfTeams,tempIds,tempJoinedIds,0,0,tempPlacements,IERC20(_prizeToken),tempDr,addMinutes(block.timestamp,_daysTillRevert),_publisherAddress);
            tournaments.push(t);
    ...
    function setStatus(uint _tournamentId, uint _status) public {
        ...
        require(_status>tournaments[_tournamentId].status,"Invalid status request");
        require(tournaments[_tournamentId].distributionRules.length>0,"Distribution rules not set");
        require(_status<=2,"Invalid status request");
    ...
    function cancelTournament(uint _tournamentId) public {  //onlyOwner for now
        ...
        for (uint i=0;i<tournaments[_tournamentId].teamIds.length;i++){
            if (teams[tournaments[_tournamentId].teamIds[i]].teamOwner!=tournaments[_tournamentId].ownerAddress) //owner already reverted //members not included atm
            {
    ...
    function joinTournament(uint _teamId, uint _tournamentId) public {
    ...
    function payTournament(uint _teamId, uint _tournamentId, uint _amount) public {
        ...
        if (teamPaid>=tournaments[_tournamentId].pricePerPlayer*tournaments[_tournamentId].playersPerTeam){
            tournaments[_tournamentId].joinedIds.push(_teamId); 
        }
    ...
    function setDistributionRules(uint _tournamentId, uint[] memory _distribution) public {
        ...
        require(tournaments[_tournamentId].status==0,"Tournament not in announced status");
        uint sum=0;
    ...
    function distributeRewards(uint _tournamentId) public {
        ...
        require(tournaments[_tournamentId].status==3, "Tournament not in distribute rewards status.");
        ...
        for (uint i=0;i<tour.distributionRules.length;i++){
            ...
            if (i==0)  
            {
                sendToken(_tournamentId, bangerAddress,amount); //banger
                 ...
                 sendToken(_tournamentId, tournaments[_tournamentId].publisherAddress,amount); //publisher
            }
            else if (i==2){
                 sendToken(_tournamentId, tournaments[_tournamentId].ownerAddress,amount); //creator
            }
            else {
