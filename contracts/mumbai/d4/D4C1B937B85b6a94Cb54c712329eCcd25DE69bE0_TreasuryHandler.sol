    ...
    ExchangePoolProcessor
{
    ...
    /**
     * @notice Set new fees swap to token address.
     * @param newFeesSwapToTokenAddress New fees to swap token address.
     */
    function setFeesSwapToTokenAddress(address newFeesSwapToTokenAddress) external onlyOwner {
        require(
            newFeesSwapToTokenAddress != address(0),
            "TreasuryHandlerAlpha:setTreasury:ZERO_TREASURY: Cannot set zero address as farm."
        );

        feesSwapToToken = IERC20(newFeesSwapToTokenAddress);
    }
