...
contract StakeSO_TestV29 is Ownable {
    ...
    mapping(uint256 => uint256) idToInitialTimeOfStaking;
 ...
 /*-------------  Staking, Unstaking & Claiming -------------*/
    ...
    */
    ...
    function stake(uint256 _tokenId) public lockContract{
        ...
        // is this error? nope
        /*
        require(remainingForRarity > 0 || removableForRarity > 0, "no more slots available!");
        if(remainingForRarity == 0) {
            checkMaxMonths(_tokenId);
        }
        */
...
/*------------- Calculation, Logics -------------*/
    ...
    */
    ...
    function getLongestStaked(uint256 _rarity) internal view returns(uint256){
        ...
        // testing
        uint256 timeLimit = block.timestamp - maxMinutes*60*60;
        if(oldestTime < block.timestamp - timeLimit) {
            return oldestID;
        }
        else {
            return 0;
    ...
    */
    function checkMaxMonths(uint256 tokenIdToBeStaked) internal {
        uint256[] memory slots = getRemainingSlots();
        uint256 rarity = getRarity(tokenIdToBeStaked);
   ...
   function getSlotsRemovable() public view returns(uint256[] memory) {
        ...
        for(uint256 i = 1; i <= 10000; i++) {
            ...
            */
            ...
            // testing hours
            uint256 timeLimit = block.timestamp - maxMinutes*60*60;
            if(initialTime < timeLimit && initialTime != 0) {
