...
contract Jackpot {
    ...
    uint256 public constant maxTickets = 100; // maximum tickets per jackpot
    uint256 public constant ticketCommission = 0.001 ether; // commition per ticket
    uint256 public constant duration = 30 minutes; // The duration set for the jackpot

    uint256 public expiration; // Timeout in case That the jackpot was not carried out.
    address public jackpotOperator; // the crator of the jackpot
    uint256 public operatorTotalCommission = 0; // the total commission balance
    address public lastWinner; // the last winner of the jackpot
    uint256 public lastWinnerAmount; // the last winner amount of the jackpot
    ...
    // modifier to check if caller is the jackpot operator
    modifier isOperator() {
        require(
            (msg.sender == jackpotOperator),
            "Caller is not the jackpot operator"
        );
        _;
    }
    ...
    function RefundAll() public {
        require(block.timestamp >= expiration, "the jackpot not expired yet");
