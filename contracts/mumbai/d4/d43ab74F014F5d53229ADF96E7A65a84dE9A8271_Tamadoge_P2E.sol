...
contract Tamadoge_P2E is Initializable, AccessControlUpgradeable, OwnableUpgradeable {
    ...
    event ArcadeCreditsBought(address indexed by, uint256 indexed arcadeCreditBuyPlan, uint256 arcadeCreditsBought, uint256 tamaPaid, uint256 timestamp);
    
    // Events for users staking/unstaking tama tokens in contract for receiving arcade credits.
    event TamaStaked(address indexed by, uint256 indexed planId, uint256 stakeId, uint256 amount, uint256 timestamp);
    event TamaUnstakedBatch(address indexed by, uint256[] stakeIds, uint256 totalUnstakedTamaAmount, uint256 timestamp);
    
    // Events for owner/admin withdrawing user-deposited tama from contract or depositing it back.
    event TamaTokensWithdrawnFromUserStakes(address indexed by, uint256 amount, uint256 timestamp);
    event TamaTokensDepositedToUserStakes(address indexed by, uint256 amount, uint256 timestamp);

    // Events for activating, deactivating, updating an existing ArcadeCreditBuyPlan and for adding a new one.
    event ActivatedArcadeCreditBuyPlans(address indexed by, uint256[] planIds, uint256 timestamp);
    event DeactivatedArcadeCreditBuyPlans(address indexed by, uint256[] planIds, uint256 timestamp);
    event UpdatedArcadeCreditBuyPlan(address indexed by, uint256 indexed planId, uint256 arcadeTokens, uint256 tamaRequired, uint256 timestamp);
    event AddedNewArcadeCreditBuyPlan(address indexed by, uint256 indexed planId, uint256 arcadeTokens, uint256 tamaRequired, bool isActivated, uint256 timestamp);

    // Events for activating, deactivating, updating an existing TamaStakePlan and for adding a new one.
    event ActivatedTamaStakePlans(address indexed by, uint256[] planIds, uint256 timestamp);
    event DeactivatedTamaStakePlans(address indexed by, uint256[] planIds, uint256 timestamp);
    event UpdatedTamaStakePlan(address indexed by, uint256 indexed planId, uint256 stakeDurationInSeconds, uint256 timestamp);
    event AddedNewTamaStakePlan(address indexed by, uint256 indexed planId, uint256 stakeDurationInSeconds, bool isActivated, uint256 timestamp);

    // Event for tama payouts from p2e reward pool balance.
    event TamaPayoutFromP2eRewardPool(address indexed by, address[] addresses, uint256[] amounts, uint256 p2eRewardPoolBalanceLeft, uint256 timestamp);

    // Event for publishing game results on IPFS.
    event GameResultPublished(address indexed by, uint256 indexed gameId, string result, uint256 timestamp);
    ...
    function initialize(
        ...
        for(uint256 i=0; i<_arcadeCreditBuyPlans.length; i++) {
            ...
            require(
                _arcadeCreditBuyPlans[i].arcadeTokens * _tamaPerCreditToSendToRewardPool <= _arcadeCreditBuyPlans[i].tamaRequired,
                "arcadeTokens * tamaPerCredit is greater than tama required."
            );
    ...
    function buyArcadeCredits(uint256 _planId) external {
        // Plan must be valid.
        require(_planId != 0 && _planId <= arcadeCreditBuyPlansAvailable, "Invalid plan id.");
        ...
        stakingRewardsPoolBalance += ((plan.tamaRequired - tamaToSendToP2EPool) * tamaPercentageToSendToStakingRewardPool) / 10000;
        
        // Increment p2eRewardPoolBalance.
        p2eRewardPoolBalance += tamaToSendToP2EPool;

        // Emit ArcadeCreditsBought event.
        emit ArcadeCreditsBought(msg.sender, _planId, plan.arcadeTokens, plan.tamaRequired, block.timestamp);
    ...
    function stakeTama(uint256 _planId, uint256 _amountToStake) external {
        ...
        emit TamaStaked(msg.sender,  _planId, userStake.totalStakes , _amountToStake, block.timestamp);
    ...
    function batchUnstakeTama(uint256[] memory _stakeIds) external {
        ...
        require(currentStakedAmountAvailableInContract >= totalTamaUnstaked, "Insufficent staked balance in contract.");
        ...
        emit TamaUnstakedBatch(msg.sender, _stakeIds, totalTamaUnstaked, block.timestamp);
    ...
    function withdrawStakedTamaTokens(uint256 _tokenAmount) external onlyOwner {
        // currentStakedAmountAvailableInContract must be equal/greater than amount being withdrawn.
        require(currentStakedAmountAvailableInContract >= _tokenAmount, "Insufficient staked balance.");
        ...
        // Emit the event.
        emit TamaTokensWithdrawnFromUserStakes(msg.sender, _tokenAmount, block.timestamp);
    ...
    function depositTamaTokensWithdrawnFromUserStakes(uint256 _tokenAmount) external onlyOwner {
        // currentStakedAmountAvailableInContract + _tokenAmount should be <= totalStakedAmountInContract
        require(currentStakedAmountAvailableInContract + _tokenAmount <= totalStakedAmountInContract, "Excessive deposit amount");

        // Increment currentStakedAmountAvailableInContract storage variable value.
        currentStakedAmountAvailableInContract += _tokenAmount;

        // Emit the event.
        emit TamaTokensDepositedToUserStakes(msg.sender, _tokenAmount, block.timestamp);
    ...
    function batchActivateArcadeCreditBuyPlans(uint256[] memory _planIds) external onlyAdminOrOwner {
        for(uint256 i=0; i<_planIds.length; i++) {
            ...
            require(planId != 0 && planId <= arcadeCreditBuyPlansAvailable, "Invalid plan id.");

            // Activate the plan id.
            arcadeCreditBuyPlans[planId].isActive = true;
        }
        emit ActivatedArcadeCreditBuyPlans(msg.sender, _planIds, block.timestamp);
    }
    ...
    function batchDeactivateArcadeCreditBuyPlans(uint256[] memory _planIds) external onlyAdminOrOwner {
        ...
        emit DeactivatedArcadeCreditBuyPlans(msg.sender, _planIds, block.timestamp);
    }
    ...
    function updateArcadeCreditBuyPlan(uint256 _planId, uint256 _arcadeTokens, uint256 _tamaRequired) external onlyAdminOrOwner {
        ...
        // _tamaRequired should be >= (_arcadeTokens * tamaPerCreditToSendToRewardPool)
        require(
            _arcadeTokens * tamaPerCreditToSendToRewardPool <= _tamaRequired,
            "arcadeTokens * tamaPerCredit is greater than tama required."
        );
        ...
        emit UpdatedArcadeCreditBuyPlan(msg.sender, _planId, _arcadeTokens, _tamaRequired, block.timestamp);
    }
    ...
    function addNewArcadeCreditBuyPlan(uint256 _arcadeTokens, uint256 _tamaRequired, bool _activatePlan) external onlyAdminOrOwner returns(uint256) {
        ...
        arcadeCreditBuyPlans[arcadeCreditBuyPlansAvailable] = ArcadeCreditBuyPlan(_arcadeTokens, _tamaRequired, _activatePlan);

        emit AddedNewArcadeCreditBuyPlan(msg.sender, arcadeCreditBuyPlansAvailable, _arcadeTokens, _tamaRequired, _activatePlan, block.timestamp);
    ...
    function batchActivateTamaStakePlans(uint256[] memory _planIds) external onlyAdminOrOwner {
        for(uint256 i=0; i<_planIds.length; i++) {
            ...
            require(planId != 0 && planId <= tamaStakePlansAvailable, "Invalid plan id.");

            // Activate the plan id.
            tamaStakePlans[planId].isActive = true;
        }
        emit ActivatedTamaStakePlans(msg.sender, _planIds, block.timestamp);
    }
    ...
    function batchDeactivateTamaStakePlans(uint256[] memory _planIds) external onlyAdminOrOwner {
        ...
        emit DeactivatedTamaStakePlans(msg.sender, _planIds, block.timestamp);
    }
    ...
    function updateTamaStakePlan(uint256 _planId, uint256 _stakeDurationInSeconds) external onlyAdminOrOwner {
        ...
        emit UpdatedTamaStakePlan(msg.sender, _planId, _stakeDurationInSeconds, block.timestamp);
    }
    ...
    function addNewTamaStakePlan(uint256 _stakeDurationInSeconds, bool _activatePlan) external onlyAdminOrOwner returns(uint256) {
        ...
        // Add the new plan to the mapping.
        tamaStakePlans[tamaStakePlansAvailable] = TamaStakePlan(_stakeDurationInSeconds, _activatePlan);

        emit AddedNewTamaStakePlan(msg.sender, tamaStakePlansAvailable, _stakeDurationInSeconds, _activatePlan, block.timestamp);
    ...
    function payoutTama(address[] memory _addresses, uint256[] memory _amounts) external onlyAdminOrOwner {
        ...
        emit TamaPayoutFromP2eRewardPool(msg.sender, _addresses, _amounts, p2eRewardPoolBalance, block.timestamp);
    }
    ...
    function publishIpfsResult(uint256 _gameId, string memory _result) external onlyAdminOrOwner {
        ...
        require(bytes(gameResults[_gameId]).length == 0, "Result already declared for this game id.");

        // Result string passed should not be empty.
        require(keccak256(bytes(_result)) != keccak256(bytes("")), "Empty string.");

        // Store the result in gameResults mapping.
        gameResults[_gameId] = _result;

        emit GameResultPublished(msg.sender, _gameId, _result, block.timestamp);
    }
    ...
    function getTotalActiveStakes(address _address) public view returns(uint256) {
        ...
        for(uint256 i=1; i<=totalUserStakes; i++) {
            if (userStakesInfo.tamaStakes[i].stakeTime + tamaStakePlans[userStakesInfo.tamaStakes[i].tamaStakePlanId].stakeDurationInSeconds > block.timestamp) {
                totalActiveStakes += 1;
            }
        }
