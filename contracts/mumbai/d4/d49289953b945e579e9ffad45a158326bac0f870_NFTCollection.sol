    ...
    ReentrancyGuardUpgradeable
{
    ...
    function setCollectionIsDestroyed(address collection) external whenNotPaused{
        require(msg.sender == collection,"Error : caller has no access to destroy collection.");
    ...
    ERC2981Upgradeable
{
     ...
     * @param collectionName.
     * @param collectionSymbol.
     * @param _receiver royalty receiver wallet address
        ...
        whenNotPaused
    {
        ...
        require(
            msg.sender == ERC721Upgradeable.ownerOf(tokenId) || msg.sender == creator[tokenId],
    ...
    function burn(uint256 tokenId) public virtual override{
        require(whitelisted[msg.sender],"Account is not whitelisted.");
        require(
            _isApprovedOrOwner(_msgSender(), tokenId) || msg.sender == creator[tokenId],
    ...
    */
    function includeInWhitelist(address account) external onlyAdmin {
    ...
    /**
       ...
       @param account whitelisting address
    */
    function excludeFromWhitelist(address account) external onlyAdmin {
        require(account != address(0), "Account cant be zero address");
    ...
    */
    function includeAllInWhitelist(address[] memory accounts) external onlyAdmin {
        for (uint256 account = 0; account < accounts.length; account++) {
            if(!whitelisted[accounts[account]] && accounts[account]!=address(0)) {
    ...
    */
    function excludeAllFromWhitelist(address[] memory accounts) external onlyAdmin {
        for (uint256 account = 0; account < accounts.length; account++) {
             if(whitelisted[accounts[account]] && accounts[account]!=address(0)) {
    ...
    function destroyCollection(address factory) public onlyAdmin {
