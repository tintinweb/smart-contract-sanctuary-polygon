...
contract lbtestkv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
    ...
    uint256 internal TICKET_VAL = 15 * 10**stableDecimals; //minimum amount (in wei) for getting registered on list
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lottoWeekly = IluckblocksWeekly(0x976222b48565C39B62FB80d6d23e6aeAb779cAb5);
        weeklyContract = 0x976222b48565C39B62FB80d6d23e6aeAb779cAb5;
    ...
    // updates minimumPot in USD to draw and amount of tokens in usd that starts on every new draw - Default 200000000000000000000 - 150
    function setMinimumPot(uint256 _minimumPot, uint256 _amountToFill) public onlyOwner() {
        require(_minimumPot >= 100 * 10**18 && _amountToFill >= 40, "minimumPot need to be minimum 100 * 10**18 and amountToFill minimum 40");
        minimumPot = _minimumPot;
        amountToFill = _amountToFill;
    }

    function setTicketValue(uint256 _priceInStable) public onlyOwner() {
        TICKET_VAL = _priceInStable * 10**stableDecimals;
    }
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 tprice = getLatestPrice(krstmTrading,usdTrading,1 * 10**18);
        tprice = tprice * 10**12;
        uint256 amountInToken = (amountToFill * 10**18 * 10**18) / tprice;  // {configuredAmount} usd in token

        minimumPrize = amountInToken;

        require(amount >= minimumPrize,"not enough amount - below configured amount in usd");
    ...
    function getRandomNumber() internal returns (uint256 requestId) {
        ...
        uint256 tprice = getLatestPrice(krstmTrading,usdTrading,1 * 10**18);
        tprice = tprice * 10**12;
        uint256 amountInToken = (amountToFill * 10**18 * 10**18) / tprice;  // {configured amount} usd in token
        minimumPrize = amountInToken;
