...
contract Ethermon3DItemData is Ethermon3DItemBasic {
    event ItemSold(address _buyer, uint32[] _itemId, uint256[] _qty);
    ...
    function ItemUpdate(uint32 _itemId, uint256 _qty) external onlyModerators {
        ItemData storage item = items[_itemId];
        item.qty = _qty;
    }

    function UpdateEvent(Items calldata _items) external onlyModerators {
