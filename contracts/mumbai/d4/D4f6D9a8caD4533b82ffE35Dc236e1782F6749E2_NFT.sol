...
abstract contract ERC721WithPermit is ERC721A,IERC721WithPermit{
...
function ownerOf(uint256 tokenId) public view virtual override returns (address) {
    ...
    function permit(
       ...
       // require(spender != recoveredAddress, 'ERC721Permit: approval to current owner');
        require(recoveredAddress!=address(0),"recoveredAddress cannot be zero");
        require(recoveredAddress == ownerOf(tokenId),"you are not the owner of the token");
        require( SignatureChecker.isValidSignatureNow(ownerOf(tokenId), digest, signature), '!INVALID_PERMIT_SIGNATURE: failed to recover signature using SignatureChecker!' );
...
contract NFT is ERC721A,ERC721WithPermit, AccessControl {
    ...
    constructor() ERC721A("ToryNFT", "TNT"){
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(AccessControl,ERC721A,ERC721WithPermit)
        returns (bool)
    {
    ...
    function _burn(uint256 tokenId) internal virtual override(ERC721A) {
        super._burn(tokenId);
    ...
    function _transfer(
        ...
        address to,
        uint256 tokenId
    ) internal virtual override (ERC721A,ERC721WithPermit) {
        // increment the nonce to be sure it can't be reused
        _incrementNonce(tokenId);
...
function ownerOf(uint256 tokenId) public view virtual override(ERC721A,ERC721WithPermit) returns (address) {
        return _ownershipOf(tokenId).addr;
    }
