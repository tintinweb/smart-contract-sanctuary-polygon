...
contract LaboManagement is ERC721Holder, Ownable {
    ...
    mapping(uint256 => LabDetails) public labDetails;
    ...
    function initSpotsNumber(uint256 _tokenId) external returns(bool){
        require(msg.sender == address(ILabNFT),"Only");
    ...
    function getLabDetails(uint256 _tokenId) external view returns(LabDetails memory){
    ...
    function workInASpot(
        ...
        WorkInstance storage w = labDetails[_laboId].workingSpot[_spotId];
        require(
    ...
    {
        LabDetails storage lab = labDetails[_laboId];
    ...
    function cleanSlotsBeforeClosing(uint256 _laboId) external returns (bool) {
        ...
        if (lab.employees == 0) {
            ...
            for (uint256 i = 0; i < numberOfSpots; ) {
                WorkInstance storage w = lab.workingSpot[i];
                if (w.zaiId != 0) {
