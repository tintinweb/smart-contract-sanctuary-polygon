...
contract LooxooryNFTMarketplace is ReentrancyGuard,Ownable {
    ...
    function createCampaign(
      CreateCampaignStruct memory _campaignDTO
        ...
        for(uint256 i=0;i<_campaignDTO.signatures.length;i++){
          voucherToCampaignID[_campaignDTO.signatures[i]] = currentCampaignId;
        }
        ...
        campaigns[currentCampaignId] = Campaign({
          ...
          signatures:_campaignDTO.signatures,
          totalNfts:_campaignDTO.signatures.length,
  ...
  function updateAuctionEndTime (uint256 _bidEndTime ) external onlyOwner {
      require(isOnAuction,"No nft is on auction at this moment");

     //  require(!isOnAuction,"An item is already on auction");
        require(_bidEndTime>block.timestamp,"Invalid bid end time");        

       // currentlyOnAuction[tokenId].biddingTime = _bidEndTime;
        currentlyOnAuction[currentlyOnAuctionTokenID].biddingTime = _bidEndTime;
