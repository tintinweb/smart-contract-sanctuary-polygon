    ...
    AccessControlUpgradeable
{
    ...
    //address => tokenId + 1
    mapping(address => uint256) public preorderIndexFor;
    ...
    function initialize() public initializer {
        ...
        baseUri = "ipfs://bafybeihy2sdmcucwaallhjp7rdlgbmbyjgseutie4i5xvt2zvc3ctynt3u/";
        hiddenUrl = "ipfs://bafybeihy2sdmcucwaallhjp7rdlgbmbyjgseutie4i5xvt2zvc3ctynt3u/1.json";
        baseExtension = ".json";
        maxSupply = 10000;
    ...
    function processPreorder(uint256 _tokenId) public payable {
        ...
        require(
            preorderSupply.current() < maxPreorderCount,
            "Preorders sold out"
        );
    ...
    // ) public payable {
    //     require(!paused, "Contract is temporary paused");
    //     require(msg.value >= preorderPrice * tokens.length, "Not enough ether");
    //     require(
    //         (preorderSupply.current() * tokens.length) < maxPreorderCount,
    //         "Preorders sold out"
    //     );
    //     for (uint256 index = 0; index < tokens.length; index++) {
    //         _completePreorder(toAddress, tokens[index]);
    //     }
    // }
    ...
    function removeReserveFor(uint256 tokenId) public payable onlyAdmin {
        require(!paused, "Contract is temporary paused");
        require(tokenId <= maxSupply);
        require(preorders[tokenId].preordered != true, "Already preordered");
        require(preorders[tokenId].reserved != true, "Already reserved");
    ...
    function _completePreorder(address toAddress, uint256 tokenId) private {
        ...
        require(preorderIndexFor[toAddress] == 0, "Token already reserved");
        preorderSupply.increment();
        preorderIndexFor[toAddress] = tokenId + 1;
        preorders[tokenId].owner = toAddress;
        preorders[tokenId].preordered = true;
    }

    function getPreorderFor(address forAddress) public view returns (uint256) {
        return preorderIndexFor[forAddress] - 1;
    }
