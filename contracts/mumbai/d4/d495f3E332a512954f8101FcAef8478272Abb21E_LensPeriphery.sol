...
contract LensPeriphery {
     ...
     */
    function setProfileMetadataURI(uint256 profileId, string calldata metadata)
        external
    {
     ...
     * and an EIP712Signature struct.
     */
    function toggleFollowWithSig(
        DataTypes.ToggleFollowWithSigData calldata vars
    ) external {
        unchecked {
            _validateRecoveredAddress(
                _calculateDigest(
    ...
    function _setProfileMetadataURI(uint256 profileId, string calldata metadata)
        internal
    {
    ...
    function _toggleFollow(
        ...
        for (uint256 i = 0; i < profileIdsLength; ) {
            address followNFT = HUB.getFollowNFT(uint(DataTypes.FollowType.Follow),profileIds[i]);
            if (followNFT == address(0)) revert Errors.FollowInvalid();
            if (!IERC721Time(address(HUB)).exists(profileIds[i]))
                revert Errors.TokenDoesNotExist();
            if (IERC721Time(followNFT).balanceOf(follower) == 0)
                revert Errors.FollowInvalid();
            unchecked {
                ++i;
            }
        }
        emit Events.FollowsToggled(
            follower,
            profileIds,
            enables,
            block.timestamp
