 ...
 */
contract BShare is ERC20, AccessControl, Pausable,IEvents,ReentrancyGuard {
    ...
    uint public lockTransactionTime = 10 seconds;  //For testing 24 hours = 10s; for deployment 24 hours = 86400s;
    uint public priceImpactProtection = 100; //Price impact protection is multiply by 10 for testing, For deployment 10 = 1
    uint public BShare_ThresholdLimit = 1000 * 10**18; //amount after BShare swap to BUSD
    uint256 public BShare_Tax = 1500; //Tax always multiply by 100
    uint [] public BShare_Tax_Allocation = [1000, 500]; // index 1 => BShare Vault, index 2 => treasury contract
   ...
   **/
    ...
    **/
    ...
    modifier _isMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "BShare: Caller is not a minter");
        _;
    }
 ...
 /**
    ...
    @param _BShare_ThresholdLimit set amount of threshold

    Note only admin can call
     */

    function setBshareThresholdLimit(uint _BShare_ThresholdLimit) external _isAdmin {
        require(_BShare_ThresholdLimit > 0, 'Bshare: Threshold limit for Bshare can not be zero');
        emit setterForUint("BshareToken",address(this),BShare_ThresholdLimit,_BShare_ThresholdLimit,block.number,block.timestamp);
        BShare_ThresholdLimit = _BShare_ThresholdLimit;
    }
  ...
  /**
    ...
    **/
    ...
    function setMinter(address _minter) external _isAdmin {
        require(_minter != address(0), "BShare: Null address provided");
    ...
    function setBurner(address _burner) external _isAdmin {
        require(_burner != address(0), "BShare: Null address provided");
    ...
    */
    ...
    function _transfer(
        ...
        if(sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD()) || recipient == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD())){
            address user = sender == IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD()) ? recipient : sender; 
            if(IWhitelist(Admin.whitelist()).getAddressesOfSwap(user)){
                super._transfer(sender, recipient, amount);
            }
             else{
                if(sender==IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD())){
                    ...
                    super._transfer(sender, address(this), taxAmount);
                    amount -= taxAmount;
                    uint256 prevAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD())); 
                    super._transfer(sender, recipient, amount);
                    uint256 currentAmount = balanceOf(IUniswapV2Factory(IUniswapV2Router02(Admin.ApeswapRouter()).factory()).getPair(address(this), Admin.BUSD()));  
 ...
 function blockNumber(address user) internal {
    ...
    **/
    ...
    function tax( uint256 taxAmount) internal {
        address[] memory path = new address[](2);
            path[0] = address(this);
            path[1] = Admin.BUSD();
            if(IUniswapV2Router02(Admin.ApeswapRouter()).getAmountsOut(balanceOf(address(this)),path)[1] >= BShare_ThresholdLimit){
                IERC20(address(this)).approve(Admin.ApeswapRouter(), taxAmount); //BSC Testnet pancake router address
                swap((taxAmount * BShare_Tax_Allocation[0])/BShare_Tax,path,Admin.BShareBUSDVault());
                swap(((taxAmount * BShare_Tax_Allocation[1])/(BShare_Tax * 2)),path,Admin.Treasury());
                swap(((taxAmount * BShare_Tax_Allocation[1])/(BShare_Tax * 2)),path,Admin.TeamAddress());
            }  
    }
    ...
    **/
    ...
    function swap(uint256 amount, address[] memory path, address sendTo) internal{
