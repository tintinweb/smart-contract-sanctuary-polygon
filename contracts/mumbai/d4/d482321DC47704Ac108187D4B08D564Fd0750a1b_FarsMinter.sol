...
contract FarsMinter is IMinter {
  ...
  constructor(
    ...
    address controller_, // controller with veFars and voter addresses
  ...
  function _veFars() internal view returns (IVeFars) {
    return IVeFars(IController(controller).veFars());
  }
  ...
  /// @dev Calculate circulating supply as total token supply - locked supply - veFars balance - minter balance
  ...
  function _circulatingSupply() internal view returns (uint) {
    ...
    // exclude veFars token balance from circulation - users unable to claim them without lock
    // late claim will lead to wrong circulation supply calculation
    - token.balanceOf(address(_veFars()))
  ...
  function updatePeriod() external override returns (uint) {
    ...
    if (block.timestamp >= _period + _WEEK && initializer == address(0)) {
      ...
      * _WEEKLY_EMISSION_DECREASE
      ...
      IERC20(address(token)).safeTransfer(address(_veFars()), _growth);
      // checkpoint token balance that was just minted inveFars      _veFars().checkpointToken();
      // checkpoint supply
      _veFars().checkpointTotalSupply();
