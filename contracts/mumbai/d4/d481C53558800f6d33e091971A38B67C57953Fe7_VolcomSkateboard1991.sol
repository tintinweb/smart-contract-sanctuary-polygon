...
contract VolcomSkateboard1991 is PunchableERC721{
    ...
    uint16 constant maxSkateboards = 90;
    ...
    constructor(string memory _base_uri,
    ...
    function setStartingIndex() external onlyOwner {
        ...
        while( startingIndex == 0) {
            startingIndex = uint16(uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % (maxSkateboards - 1));
        }
    }

    function mint(uint8 amount) external payable {
        require(amount <= maxMintsAtOnce, "Trying to mint too many");
        require(supply + amount <= maxSkateboards, "exceeds max supply");
        //if (whitelist[_msgSender()] ){
        //    require(msg.value >= ((price*(10000 - discount))/10000) * amount , "Not enough Matic");
       // }else{
        require( msg.value >= price * amount, "Dont be cheap");
        //}
        ...
        for (uint16 i; i < amount; i++){
            nextTID++;
            supply++;
            require(supply <= maxSkateboards, "exceeds max supply");
    ...
    *****************************************************************************************/
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "Nonexistent NFT");
        if (!_globalUriEmpty()){
            ...
            uint16 i = uint16(((tokenId + startingIndex) % maxSkateboards) + 1);
            //console.log("uri: ", uri);
            //console.log("_uris[uri]: ", _uris[uri]);
            string memory uri = string(abi.encodePacked(base_uri, toString(i)));
