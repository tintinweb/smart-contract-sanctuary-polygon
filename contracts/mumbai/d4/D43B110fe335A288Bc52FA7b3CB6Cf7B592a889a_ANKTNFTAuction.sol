...
// File: contracts\ANKTNFTAuction.sol

pragma solidity ^0.8.0;
contract ANKTNFTAuction is Ownable, Pausable, ReentrancyGuard {
  ...
  modifier onlyIfAuctionIsActive(address _nftAddress, uint256 _tokenId) {
      Auction memory _auction = auctions[_nftAddress][_tokenId];
      require(_auction.startedAt > 0, "ANKTNFTAuction: Provided TokenId and NFTaddress are not in auction");
      _;
  }
    
  constructor(uint256 auctioneerFee_) {
    require(auctioneerFee_ >= 1 && auctioneerFee_ <= 1000, "ANKTNFTAuction: auctioneerFee must be between 1 - 1000");
    auctioneerFee = auctioneerFee_;
  }

  function setAuctioneerFee(uint256 auctioneerFee_) external onlyOwner {
    require(auctioneerFee_ >= 1 && auctioneerFee_ <= 1000, "ANKTNFTAuction: auctioneerFee must be between 1 - 1000");
    auctioneerFee = auctioneerFee_;
  }

  receive() external payable{
    require(msg.value == 0, "ANKTNFTAuction: you can not transfer money to this contract!");
  }
  ...
  function getAuction(
    ...
    external
    onlyIfAuctionIsActive(_nftAddress, _tokenId)
    view
  ...
  function getCurrentPrice(
    ...
    view
    returns (uint256) {
        return _getCurrentPrice(auctions[_nftAddress][_tokenId]);
  }
  ...
  function createAuction(
    ...
    Auction memory _auction = auctions[_nftAddress][_tokenId];
    require(_auction.startedAt ==  0, "ANKTNFTAuction: auction is already exist for given tokenid");
    require(_owns(_nftAddress, _seller, _tokenId), "ANKTNFTAuction: sender must be owner of NFT to create auction");
    require(_duration >= 1 minutes, "ANKTNFTAuction: minimum auction duration must be > 60 ");
  ...
  function cancelAuction(address _nftAddress,
    uint256 _tokenId)
    external
    onlyIfAuctionIsActive(_nftAddress, _tokenId) {
  ...
  function _getCurrentPrice(
    ...
    returns (uint256) {
        ...
        _secondsPassed = block.timestamp.sub(_auction.startedAt);
        }
    ...
    returns (uint256)
  {
    if (_secondsPassed >= _duration) {
      ...
      return uint256(_startingPrice) + uint256(_currentPriceChange);
    }
  }
  ...
  function _computeAuctioneerFee(uint256 _price) internal view returns (uint256) {
    return _price.mul(auctioneerFee).div(100);
  }
    ...
    returns (uint256)
  {
    ...
    require(_bidAmount >= _price, "ANKTNFTAuction: new bid amount must be  >= current price");
    address payable _seller = payable(_auction.seller);
    _removeAuction(_nftAddress, _tokenId);
    if (_price > 0) {
      uint256 _auctioneerFee = _computeAuctioneerFee(_price);
      uint256 _finalAmount = _price.sub(_auctioneerFee);
     _seller.transfer(_finalAmount);
    }
