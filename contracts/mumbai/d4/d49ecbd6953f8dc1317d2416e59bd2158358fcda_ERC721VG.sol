...
contract ERC721VG is ERC721A, Ownable, ERC2981 {
    ...
    /**
     * @notice Event emitted when `maxMintsPerTransaction` has been modified
     */
    event MaxMintsPerTransactionChanged(uint256 newMaxMintsPerTransaction);
     ...
     * @member amountForFounders: amount tokens per founders (founding team)
    ...
    constructor(
        ...
        // Check collection size is larger than zero
        require(saleSettings_.collectionSize > 0, "Collection size must be bigger than 0");
        // Check amount of NFTs for developers size
        require(saleSettings_.amountForFounders <= saleSettings_.collectionSize, "larger collection size needed");
        // check withdraw entry data
        require(saleSettings_.withdrawAddresses.length > 1, "withdrawAddresses size must be minimum 2");
        require(
            saleSettings_.withdrawAddresses.length == saleSettings_.withdrawPercentages.length,
            "Number of withdraw addresses not equal to percent"
        );
        ...
        // amount of NFTs secured for founders
        amountForFounders = saleSettings_.amountForFounders;
        // total collection size
        collectionSize = saleSettings_.collectionSize;

        // premint and transfer to founders
        // Cheaper than a loop to mint one by one to only one wallet address
        // In case of founder's list, first wallet in the list must be = msg.sender
        if (saleSettings_.amountForFounders != 0) {
            require(
                (saleSettings_.amountForFounders % saleSettings_.founders.length == 0),
                "amount not div by num wallets"
            );
            _mintERC2309(msg.sender, saleSettings_.amountForFounders);
            if (saleSettings_.founders.length > 1) {
                uint256 amountPerFounder = saleSettings_.amountForFounders / saleSettings_.founders.length;
                for (uint256 i = 1; i < saleSettings_.founders.length; ) {
                    for (uint256 j = 1; j <= amountPerFounder; ) {
                        uint256 tokenIndex = amountPerFounder * i + j;
                        ERC721A.transferFrom(msg.sender, saleSettings_.founders[i], tokenIndex);
                        unchecked {
                            j++;
                        }
                    }
    ...
    function mint1(uint256 roundId, uint256 amount) external payable virtual {
        ...
        require(amount <= maxMintsPerTransaction);
        require(
            _waveToRoundsToOwnerTotalMinted[waveId][roundId][msg.sender] + amount <= maxMintsPerWallet,
            "Max allowed per address"
        );
    ...
    function mintWithWl(
        ...
        require(waveId > 0, "No active wave");
        bytes32 _merkleRoot = _waveToRounds[waveId][roundId].merkleRoot;
        require(_merkleRoot != bytes32(0), "No Round proof required");
        require(amount <= maxMintsPerTransaction);
        require(
            _waveToRoundsToOwnerTotalMinted[waveId][roundId][msg.sender] + amount <= maxMintsPerWallet,
            "Max allowed per address"
    ...
    function setupRound(
        ...
        emit RoundSetup(roundWaveId, roundId, supply, startTime, duration, price, _merkleRoot);
    }

    /**
     * @notice Change number of tokens that a wallet can mint in a public wave
     */
    function setMaxMintsPerWallet(uint256 newMaxMints) external virtual onlyOwner {
     ...
     * @notice Change number of tokens that a wallet can mint in a public wave
     */
    function setMaxMintsPerTransaction(uint256 newMaxMints) external virtual onlyOwner {
        maxMintsPerTransaction = newMaxMints;
        emit MaxMintsPerTransactionChanged(newMaxMints);
    }
