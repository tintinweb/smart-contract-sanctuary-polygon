...
contract GabrielV3 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function compoundArcha(uint256 _pid) external {
        ...
        if (pending > 0) {
            ...
            storeUnclaimedReward(_pid, msg.sender, 0, excess);
    ...
    function reset(uint256 _pid, uint256 startIndex, uint256 endIndex) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        address[] memory harvestList = new address[](pool.harvestList.length);
    ...
    function storeUnclaimedReward(uint256 _pid, address userAddress, uint256 index, uint256 excess) internal {
        ...
        if (arrayLength == 0) {
            ...
            if (pending > 0) {
                if (index == 0) {
                    user.nonHarvestedRewards[index] = excess;
                } else {
        ...
        if (arrayLength == NORT) {
            ...
            if (pending > 0) {
                if (index == 0) {
                    user.nonHarvestedRewards[index] = user.nonHarvestedRewards[index].add(excess);
                } else {
