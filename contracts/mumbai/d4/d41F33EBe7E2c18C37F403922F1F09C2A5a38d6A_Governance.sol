...
// File: DaoContracts/Governance.sol
...
contract Governance is Ownable{
    ...
    event AddProposal(uint256 indexed index,string proposalMetadataURI, uint256 votingDeadline,uint256 rewardDistributionDeadline,uint256 knightReward, string[] optionsMetadataURI);
    ...
    function addProposal(string memory proposalMetadataURI, uint256 votingDeadline,uint256 rewardDistributionDeadline, uint256 knightReward, string[] memory optionsMetadataURI) external onlyOwner {
        ...
        proposals[mappingNonce].knightReward = knightReward;
        ...
        emit AddProposal(mappingNonce, proposalMetadataURI, votingDeadline, rewardDistributionDeadline, knightReward, optionsMetadataURI);
    ...
    function vote(uint256 indexOfProposal, uint256[] memory votedOptionsIndices, uint256[] memory votedPower) external {
        ...
        require(block.timestamp - lockerState.lockTime > 14 days, "You can not vote until 14 days after your stake!");
        Options[] storage options = proposals[indexOfProposal].options;
        uint256 cumulativeVotePower;
        for (uint256 index = 0; index < votedOptionsIndices.length; index++) {
            require(votedOptionsIndices[index] < options.length, "Invalid votedOptionsIndices");
            options[votedOptionsIndices[index]].voted += votedPower[index];
            cumulativeVotePower += votedPower[index];
        }
        require(cumulativeVotePower == lockerState.currentLockingAmount,"Your voting fractions is not 100%");
        proposals[indexOfProposal].totalVotedPower += lockerState.currentLockingAmount;
        voteStatus[indexOfProposal][msg.sender].votePower += lockerState.currentLockingAmount;
