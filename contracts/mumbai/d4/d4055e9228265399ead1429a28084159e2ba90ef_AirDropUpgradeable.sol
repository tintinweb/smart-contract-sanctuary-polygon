...
interface IAirdropUpgradeable {

    struct LockupSummary {
        uint256 amount; // Total amount airdropped to given account
        bool claimed; // If the airdropped tokens have been claimed
    }

    /**
     * @dev Emitted when `amount` of tokens are set to `account` for airdropping.
     */
    event AmountSet(address account, uint256 amount);

    /**
     * @dev Emitted when this airdrop is started with `globalLockupDuration` and `airdropExpiryTime`
     * with the total amount in this airdrop being `totalAmountToAirdrop`.
     */
    event AirdropStarted(uint256 globalLockupDuration, uint256 airdropExpiryTime, uint256 totalAmountToAirdrop);

    /**
     * @dev Emitted when `account` locked their tokens for gaining voting power.
     */
     ...
     * @param globalLockupDuration the duration sets the end timestamp after which 
     * airdropped tokens are eligible to be withdrawn directly to users' accounts.
     ...
     */
    function setAmounts(address[] memory accounts, uint256[] memory amounts)
        external;

    /**
     * @dev Locks airdropped tokens directly to Voting Escrow to gain voting power.
     */
    ...
    UUPSUpgradeable 
{
    ...
    IERC20Upgradeable public token; // MVDAO token contract address
    IVotingEscrowUpgradeable public veContract; // Voting Escrow contract address
    ...
    function initialize(
        ...
        treasury = _treasury;
        veContract = IVotingEscrowUpgradeable(_veContract);        
    }
    ...
    function startAirdrop(
        ...
        require(
            _globalLockupDuration > 0, 
            "Airdrop: invalid lockup duration"
        );
        require(
    ...
    function setAmounts(address[] memory _accounts, uint256[] memory _amounts)
        external
        override
        onlyAdmin
    {
