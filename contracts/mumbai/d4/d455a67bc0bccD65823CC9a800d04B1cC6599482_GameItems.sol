...
import "./interfaces/IPVWhitelist.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/token/ERC1155/ERC1155.sol";

/**
 * @title GameItems - ERC1155 contract to represent GameItems in the PV Gaming Ecosystem
 * 
 * @author Jack Chuma
 */
contract GameItems is ERC1155, AccessControl {
    ...
    modifier onlyWhitelisted(address _address) {
        if (!whitelist.isWhitelisted(_address))
    ...
    constructor(
        ...
        whitelist = IPVWhitelist(_whitelist);
    ...
    /**
     * @notice Admin function to update stored whitelist contract address
     * @param _whitelistAddr New address for whitelist contract
     */
    function setWhitelist(address _whitelistAddr) external onlyRole(USER_ROLE) {
        if (_whitelistAddr == address(0)) revert ZeroAddress();
        whitelist = IPVWhitelist(_whitelistAddr);
        emit WhitelistSet(_whitelistAddr);
    }

    /**
     * @notice Called by whitelisted address to mint new items
     * @param _to Address to mint assets to
     * @param _id Item ID to add
     * @param _amount Amount of item to mint
     */
    function mint(
        address _to,
        uint256 _id,
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public pure override(AccessControl, ERC1155) returns (bool) {
    ...
    function uri(
        ...
        return string(
            abi.encodePacked(
                super.uri(_tokenId), Strings.toString(_tokenId)
            )
        );
    }
}
...
interface IPVWhitelist {
    function modifyWhitelist(
        address _whitelistAddress, 
        bool _isWhitelisted
    ) external;

    function isWhitelisted(address _whitelistAddr) external returns (bool);
}
