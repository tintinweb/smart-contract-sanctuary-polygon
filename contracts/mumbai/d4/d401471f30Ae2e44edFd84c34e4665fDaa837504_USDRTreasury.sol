...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    ...
    function migrate(address to) external onlyRole(TREASURY_WITHDRAW_ROLE) {
    ...
    function migrateNFT(address to, bool ftnft)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        ...
        address[] memory nfts = !ftnft
            ? treasuryTracker.getTnftCategoriesInTreasury()
            : treasuryTracker.getFractionContractsInTreasury();
        uint256 length = nfts.length;
        for (uint256 i; i < length; i++) {
            uint256[] memory tokenIds = !ftnft
                ? treasuryTracker.getTnftTokensInTreasury(nfts[i])
                : treasuryTracker.getFractionTokensInTreasury(nfts[i]);
    ...
    function withdrawTnft(
        ...
        bool ftnft
    ) external onlyRole(TREASURY_WITHDRAW_ROLE) {
        require(emergencyStop, "emergency stop required");
    ...
    function burnTNGBL() external onlyRole(CONTROLLER_ROLE) {
        address tngbl = _fetchAddress(TNGBL_ADDRESS);
    ...
    {
        address rentShare = _fetchAddress(TANGIBLE_RENT_SHARE_ADDRESS);
