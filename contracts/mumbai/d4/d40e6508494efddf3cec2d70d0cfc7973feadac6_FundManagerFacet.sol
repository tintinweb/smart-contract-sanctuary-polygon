...
contract FundManagerFacet is FundManagerModifiers {
    ...
    function setTokenContract(address _tokenContract) external notZeroAddress(_tokenContract) {
        s.tokenContract = IERC20(_tokenContract);
    }
    
    function charge(uint256 _amount, address buyer) public validCollector() {
        bool resultTx = s.tokenContract.transferFrom(buyer, s.collector, _amount);
