...
contract ERC1155DaoProxy is IERC1155DaoProxy, Ownable {
    ...
    function checkListPriceForFixedPriceRange(
        ...
        unchecked {
            if (tokenOwner.startOfLife == 0) {
                fixedPrice = (tokenDet.tokenPrice / 100) * 95;
            } else {
                fixedPrice =
                    ((tokenDet.tokenPrice / 100) *
                        95 *
                        ((tokenOwner.startOfLife * 100) /
                            tokenOwner.endOfLife)) /
                    100;
            }
        }

        return ListingPriceTuple(fixedPrice / 10000, fixedPrice / 10);
    }
    ...
    function getBorrower(
        ...
        return tokenOwner.borrower;
    }

    /**
     * @notice returns the borrowers
     *
     * @param tokenIds       id of the token.
     * @param serialNos      serial Number of the token.
     *
     * @return borrowers.
     */
    function getBorrowerBatch(
        uint256[] memory tokenIds,
        uint256[] memory serialNos
    ) public view returns (address[] memory) {
        address[] memory bor = new address[](tokenIds.length);

        for (uint256 i = 0; i < tokenIds.length; ++i) {
            bor[i] = getBorrower(tokenIds[i], serialNos[i]);
        }
    ...
    function checkLendingPrice(
        ...
        unchecked {
            ...
            (lendingPriceFloor, lendingPriceCeil) = (
                lendingPrice / 100000 + _erc1155dao.getPlatformFeesInWei(),
    ...
    function getTokenDetailsBatch(
        uint256[] memory tokenIds
    ) external view returns (TokenDetails[] memory) {
        TokenDetails[] memory tokenDet;

        for (uint i = 0; i < tokenIds.length; i++) {
            tokenDet[i] = getTokenDetails(tokenIds[i]);
        }
        return tokenDet;
    }

    function getTokenDetailsWithRemainingBalance(
        uint256 tokenId
    ) public view returns (TokenDetails memory, uint256) {
        TokenDetails memory tokenDet = _erc1155dao.getTokenDetails(tokenId);
        uint256 balance = _erc1155dao.getBalance(tokenId, _erc1155daoproxy);
        return (tokenDet, balance);
    }

    function getTokenDetailsWithRemainingBalanceBatch(
        uint256[] memory tokenIds
    ) public view returns (TokenDetails[] memory, uint256[] memory) {
        TokenDetails[] memory tokenDet;
        uint256[] memory balance;

        for (uint i = 0; i < tokenIds.length; i++) {
            (tokenDet[i], balance[i]) = getTokenDetailsWithRemainingBalance(
                tokenIds[i]
            );
        }
        return (tokenDet, balance);
    ...
    function getTokenBearer(
        uint256 tokenId,
        uint256 serialNo
    ) public view returns (TokenBearer memory) {
        TokenBearer memory tokenBearer = _erc1155dao.getTokenBearer(
            tokenId,
            serialNo
        );
        return tokenBearer;
    ...
    function getTokenBearerBatch(
        uint256[] memory tokenIds,
        uint256[] memory serialNos
    ) external view returns (TokenBearer[] memory) {
        TokenBearer[] memory tokenBearer;

        for (uint i = 0; i < tokenIds.length; i++) {
            tokenBearer[i] = getTokenBearer(tokenIds[i], serialNos[i]);
        }
        return tokenBearer;
    }

    function getTokenBearerDetailsWithRemainingBalance(
        uint256 tokenId,
        uint256 serialNo
    ) public view returns (TokenDetails memory, TokenBearer memory, uint256) {
        TokenDetails memory tokenDet = _erc1155dao.getTokenDetails(tokenId);
        TokenBearer memory tokenBearer = _erc1155dao.getTokenBearer(
            tokenId,
            serialNo
        ...
        uint256 balance = _erc1155dao.getBalance(tokenId, _erc1155daoproxy);
        return (tokenDet, tokenBearer, balance);
    }

    function getTokenBearerDetailsWithRemainingBalanceBatch(
        uint256[] memory tokenIds,
        uint256[] memory serialNos
    )
        public
        view
        returns (TokenDetails[] memory, TokenBearer[] memory, uint256[] memory)
    {
        TokenDetails[] memory tokenDet;
        TokenBearer[] memory tokenBearer;
        uint256[] memory balance;
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            (
                ...
                tokenBearer[i],
                balance[i]
            ) = getTokenBearerDetailsWithRemainingBalance(
                tokenIds[i],
                serialNos[i]
            );
        }
