...
contract CBS is ERC1400 {
    ...
    address public feeAddr1; // Address to receive the 1% fee
    address public feeAddr2; // Address to receive the 0.25% fee
    address public feeAddr3; // Address to receive the 0.125% fee
    mapping(address => bool) public whitelist;

    /**
     * @dev Sets the values for {_owner}, {_to}, {_feeAddr1} and {_feeAddr2}.
     *
    ...
    constructor(
        ...
        address _feeAddr2, 
        address _feeAddr3
    ) ERC1400("CITD Bond STO", "CBS", 1, _to, 100_000_000 * 1e18) {
     ...
     */
    function upgradeFeeAddr(address _feeAddr1,  address _feeAddr2,  address _feeAddr3) public onlyOwner {
        feeAddr1 = _feeAddr1;
        feeAddr2 = _feeAddr2;
        feeAddr3 = _feeAddr3;
    }
    ...
    function _transferByPartition(
        ...
        if(_checkWhiteList(from, to)) return super._transferByPartition(fromPartition, operator, from, to, value, data, operatorData);
        ...
        uint256 feeAmount1 = value.mul(1000).div(100000); // 1%
        uint256 feeAmount2 = value.mul(250).div(100000); // 0.25%
        uint256 feeAmount3 = value.mul(125).div(100000); // 0.125%

        _removeTokenFromPartition(from, fromPartition, value);
        value = value.sub(feeAmount1).sub(feeAmount2);
        ...
        //transfer fee1
        if(feeAmount1 > 0 && feeAddr1 != address(0)) {
            _transferWithData(from, feeAddr1, feeAmount1);
            _addTokenToPartition(feeAddr1, toPartition, feeAmount1);
            emit TransferByPartition(fromPartition, operator, from, feeAddr1, feeAmount1, data, operatorData);
        }

        //transfer fee2
        if(feeAmount2 > 0 && feeAddr2 != address(0)) {
            _transferWithData(from, feeAddr2, feeAmount2);
            _addTokenToPartition(feeAddr2, toPartition, feeAmount2);
            emit TransferByPartition(fromPartition, operator, from, feeAddr2, feeAmount2, data, operatorData);
        }

        //transfer fee3
        if(feeAmount3 > 0 && feeAddr3 != address(0)) {
            _transferWithData(from, feeAddr3, feeAmount3);
            _addTokenToPartition(feeAddr3, toPartition, feeAmount3);
            emit TransferByPartition(fromPartition, operator, from, feeAddr3, feeAmount3, data, operatorData);
        }
    ...
    function _checkWhiteList(address from, address to) internal view returns(bool) {
        ...
        if(from == feeAddr1 || from == feeAddr2 || from == feeAddr3 || from == owner()) return true;
        if(to == feeAddr1 || to == feeAddr2 || to == feeAddr3 || to == owner()) return true;
