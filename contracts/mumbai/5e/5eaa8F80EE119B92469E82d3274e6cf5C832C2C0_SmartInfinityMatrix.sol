...
contract SmartInfinityMatrix {
    ...
    function getMatrixType(uint8 level) internal pure returns (uint8) {
...
function updateX3Referrer(address userAddress, address referrerAddress, uint8 level) internal {
    ...
    if (referrerAddress != owner) {
        ...
        sendDividendstoOwner(userAddress, level);
        refMatrix.reinvestCount++;
...
function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) internal {
    ...
    if (referrerMatrix.Referrals.length < 2) {
        ...
        if (referrerAddress == owner) {
            return sendDividendstoOwner(userAddress, level);
        }
...
function updateX6(address userAddress, address referrerAddress, uint8 level, bool x2) internal {
    ...
    function findReceiver(address userAddress, address _from, uint8 level) internal returns(address, bool) {
    ...
    function sendDividendstoOwner(address userAddress, uint8 level) internal {
        uint256 amount = levelPrice[level];
        userWallet[userAddress].tokenBalance[DaiAddress] -= amount; 
        userWallet[owner].tokenBalance[DaiAddress] += amount;

    }

    function sendDividends(address userAddress, address _from, uint8 level, uint8 matrix, uint length) internal {       
        ...
        if(level == 12){
        transferToken(userAddress, receiver , level, matrix, length);
    ...
    function transferToken(address from, address to, uint8 level, uint8 matrix, uint length) internal { 
        uint256 amount = levelPrice[level];
