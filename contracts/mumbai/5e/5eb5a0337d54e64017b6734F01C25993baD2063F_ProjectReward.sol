...
contract ProjectReward is AccessControl, IAirdropReward, Initializable {
    ...
    uint256 public airdropMATICamount;
    ...
    function addAirdrop(
        ...
        if (token != address(1)) {
            ...
            airdropMATICamount += totalAmount;
        }
    }
    ...
    function claimAirdrop() external {
        ...
        uint256 lastClaimed;
        for (; i < end; i++) {
            if (userAirdropDetails[msg.sender].airdropAmount[i + 1] > 0) {
                if (airdropsDetails[i + 1].token != address(1)) {
                    IERC20D(airdropsDetails[i + 1].token).safeTransfer(
                        msg.sender,
                        userAirdropDetails[msg.sender].airdropAmount[i + 1]
                    );
                } else {
                    (bool sent, ) = payable(msg.sender).call{
                        value: userAirdropDetails[msg.sender].airdropAmount[
                            i + 1
                        ]
                    }("");
                    require(sent, "Failed to send MATIC");
                    airdropMATICamount -= userAirdropDetails[msg.sender]
                        .airdropAmount[i + 1];
                }

                lastClaimed = i+1;
                emit ClaimedAirdrop(
                    airdropsDetails[i + 1].token,
                    msg.sender,
                    userAirdropDetails[msg.sender].airdropAmount[i + 1]
                );
            }
    ...
    function takeLockedMATIC() external onlyAdmin {
        require(address(this).balance > airdropMATICamount, "No MATIC amount");
