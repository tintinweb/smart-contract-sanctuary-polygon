...
contract EpillioVesting is OwnableUpgradeable,ReentrancyGuardUpgradeable{
    ...
    uint[] public initialAmountReleased=[0,0,100,100,20,30,0,1000];
    uint[] public linearVestingAmountReleased=[0,1000,900,900,980,970,1000,0]; // stores percentage
    uint[] public linearVestingTimePeriod=[0,120 minutes,2 minutes,130 minutes,270 minutes,270 minutes ,150 minutes,0];

    receive()external payable{}


    function initialize(address _epillioToken,address _usdt, uint _usdtRate,uint tokenRate, uint[] memory _minMaxMatic, uint[] memory _minMaxUsdt)external initializer{
    ...
    function setSingleDate(uint  startTime)external onlyOwner{
        globalStartTime=startTime;
        initialVestingStartTime = [0,startTime,startTime,startTime,startTime,startTime,startTime,startTime];
        linearVestingStartTime= [0,startTime+60 minutes ,startTime +2 minutes,startTime + 30 minutes,startTime + 365 minutes,startTime + 180 minutes,startTime+365 minutes,0 ];
    }
    ...
    function getVestingBalance(address _userAddress) public view returns (uint, uint) {
        ...
        else if (!InvestorDetails[_userAddress].hasLinearClaimed && InvestorDetails[_userAddress].linearToBeClaimed > 0 && block.timestamp >= linearVestingStartTime[InvestorDetails[_userAddress].investorType]) return (linearVestingDetails(_userAddress),3);
        else return (0,4);
    }
