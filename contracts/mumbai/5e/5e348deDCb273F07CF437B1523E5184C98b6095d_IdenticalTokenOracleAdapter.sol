...
contract IdenticalTokenOracleAdapter is AccessControl, IOracleAdapter {
    ...
    function getPrice(address asset1, address asset2) external view returns (bool found, uint256 price) {
        address underlyingToken = getUnderlyingToken(asset1);
        if (underlyingToken != address(0)) {
            try _priceOracle.getPrice(underlyingToken, asset2) returns (uint256 price_) {
                return (true, price_);
            } catch {}
        }

        underlyingToken = getUnderlyingToken(asset2);
        if (underlyingToken != address(0)) {
            try _priceOracle.getPrice(asset1, underlyingToken) returns (uint256 price_) {
                return (true, price_);
            } catch {}
        }
    }
