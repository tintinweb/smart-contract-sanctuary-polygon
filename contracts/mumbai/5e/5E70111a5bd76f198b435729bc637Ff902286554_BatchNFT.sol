...
import { BatchReveal } from "./BatchReveal.sol";

contract BatchNFT is Ownable, ERC721Enumerable, BatchReveal {
    ...
    constructor(string memory _baseURI, string memory _unrevealedURI)
     ...
     function provideRandomness(uint randomNumber) public onlyOwner {
    ...
    function tokenURI(uint256 id) public view override returns (string memory) {
        if(id >= lastTokenRevealed){
            ...
            return string(abi.encodePacked(baseURI, getShuffledTokenId(id).toString(), baseExtension));
        }
    }
     ...
     require(numberNFT+numberOfTokens<=15,"Maximum amount of Nft-s per account is 15");
...
abstract contract BatchReveal {
    uint constant public TOKEN_LIMIT = 15;
    uint constant public REVEAL_BATCH_SIZE = 3;
    ...
    function addRange(Range[RANGE_LENGTH] memory ranges, int128 start, int128 end, uint lastIndex) pure private returns (uint) {
        ...
        if(end > intTOKEN_LIMIT){
            addRange(ranges, 1, end - intTOKEN_LIMIT, lastIndex);
            lastIndex++;
        }
        return lastIndex;
    }
