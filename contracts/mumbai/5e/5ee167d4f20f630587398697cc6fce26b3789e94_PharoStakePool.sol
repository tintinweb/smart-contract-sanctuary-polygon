...
import "./interfaces/pharo/IGPHROToken.sol";
...
contract PharoStakePool is AccessControl, TokenRecover
{
    ...
    constructor(
        ...
        addStakePool(10, _phroToken);

        _setupRole(PharoConstants.OPERATOR_ROLE, msg.sender);
        _setupRole(DAO_ROLE, msg.sender);
        _transferOwnership(0xc316Efa8aD1046b0DEf528e17E6f4136072731F7);
    }
    ...
    function addStakePool(
        ...
        require(
            _allocationPoint > 0 && _allocationPoint < 101,
    ...
    function pendingPhro(
        ...
        return
            staker.amount.mul(accPhroPerShare).div(1e12).sub(staker.rewardDebt);
    }
    ...
    function deposit(uint256 _poolId, uint256 _amount) public {
        ...
        phroToken.transferFrom(address(msg.sender), address(this), _amount);
    ...
    function withdraw(uint256 _poolId, uint256 _amount) public
    {
        ...
        updatePool(_poolId);
        uint256 pending = user.amount.mul(pool.accPhroPerShare).div(1e12).sub(user.rewardDebt);
    ...
    /// @dev transfers GPHRO from contract if available and mints if not
    ...
    function poolDetails(uint256 _poolId) external view returns(uint256[4] memory)
    {
        PoolInfo storage pool = pools[_poolId];
        return [pool.allocationPoint,
                pool.lastRewardBlock,
                pool.depositSum,
...
interface IGPHROToken
{
    ...
    function lock(bytes32 _reason, uint256 _amount, uint256 _time) external;
    function transferWithLock(address _to, bytes32 _reason, uint256 _amount, uint256 _time) external returns(bool);
    function tokensLocked(address _of, bytes32 _reason) external view returns(uint256);
    function tokensLockedAtTime(address _of, bytes32 _reason, uint256 _time) external view returns(uint256);
    function totalBalanceOf(address _of, bytes32 _reason) external view returns(uint256);
    function extendLock(bytes32 _reason, uint256 _time) external view returns(bool);
    function increaseLockAmount(bytes32 _reason, uint256 _amount) external;
    function tokensUnlockable(address _of, bytes32 _reason) external view returns(uint256);
    function unlock(address _of) external;
    function getUnlockableTokens(address _of) external returns(uint96);
}
