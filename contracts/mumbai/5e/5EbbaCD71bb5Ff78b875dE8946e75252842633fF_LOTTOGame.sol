...
contract LOTTOGame is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    event JackpotClaimed(address user,uint256 winPrice,uint256 gameId,uint256 nftId,uint256 timestamp);
    ...
    function claimJackpot(uint256 gameId,uint256 _nftId) external
    {
        ...
        emit JackpotClaimed(msg.sender,amount,gameId,_nftId,block.timestamp);
        address ref = lottoContract.getUpline(msg.sender);
        if(ref!=address(0)){
