...
contract CraftLogic is MultiOwner, ReentrancyGuard {
    ...
    modifier onlyPhiGelatoRelay() {
        require(_isPhiGelatoRelay(msg.sender), "onlyGelatoRelay");
        _;
    }

    function _isPhiGelatoRelay(address _forwarder) internal view returns (bool) {
        return _forwarder == phiGelatoRelay;
    }
    ...
    function craftByRelayer(uint256 recipeId) external onlyPhiGelatoRelay nonReentrant {
        _craft(recipeId);
    }
}
