...
abstract contract Base is ICampaign, FeesDistributable, SuperCertsClaimable, Whitelistable, Accessable, Providers, ReentrancyGuard {
    ...
    function _isCancelled() internal view returns (bool) {
    ...
    function _requireNonZero(uint a, uint b, uint c) internal pure {
        _require (a > 0 && b > 0 && c > 0, "Cannot be 0");
    }
...
abstract contract SaleBase is Base {
    ...
    function finishUp() external nonReentrant onlyDeployer {
        _require(!_isFinished() && !_isFailed() && !_isCancelled(), "Finished, failed or cancelled");
    ...
    function getClaimStatus(address user) public view returns (bool purchaseClaimed, bool certsEnabled, bool certClaimed, bool certsClaimable) {
        ...
        if (_isFinished() && !certClaimed && _sales.userPurchaseMap[user].totalFund > 0) {
            certsClaimable = true;
        }
    }
    
    function returnFund() external nonReentrant {
        // Only failed or cancelled campaign can be refundable.
        _require(_isFailed() || _isCancelled(), "Not failed or cancelled");
    ...
    function claimCerts(address referer, bytes memory signature) external override nonReentrant {
        ...
        bool hasUpline = (referer != Constant.ZERO_ADDRESS && _verifyUpline(msg.sender, referer, signature));
    ...
    function _setup(
        ...
        uint customUserFeePcnt) internal { 
            ...
            _global.stakerRebateMinPcnt = data[2];
            _global.stakerRebateCapPcnt = data[3];
    ...
    function _getGlobalParams() private view returns (uint[] memory) {
        ...
        keys[2] = ParamKey.StakerDiscountMinPcnt;
        keys[3] = ParamKey.StakerDiscountCapPcnt;
    ...
    function _getStakerRebatePcnt(uint stake) private view returns (uint) {
        return _interpolate(stake, _global.minLaunch, _global.capLaunch, _global.stakerRebateMinPcnt, _global.stakerRebateCapPcnt);
    }

    function _verifyStake(address user, uint stake, bytes memory signature) internal view returns (uint) {
        bytes32 hash =  keccak256(abi.encodePacked(this, user, stake));
        return _verifyHash(hash, signature) ? stake : 0;
    }

    function _verifyUpline(address user, address upline, bytes memory signature) internal view returns (bool) {
...
library Types {
    ...
    struct GlobalParams {
        ...
        uint stakerRebateMinPcnt;   // Eg: 1000 svLaunch gets 0.6% discount on total Fee.
        uint stakerRebateCapPcnt;   // Eg: 50,000 svLaunch gets 30%% discount on total Fee.
...
contract FcfsCampaign is SaleBase {
    ...
    function setup(
        ...
        uint unitPrice,
        uint[5] calldata alloc, // min, maxPublic, stakerLower, stakerUpper, maxFund
        ...
        _setupCustomData(preOpenDuration, unitPrice, alloc);
    }

    function setupLpV2(address locker, address router, ILpLocker.LpScheme scheme, uint lpValue, uint rate, uint lockDuration) public canConfigure {
        _require(_isConfigured(), "Setup first");
        ...
        _lp.params.lockDuration = lockDuration;
    ...
    function setupLpV3(address locker, address router, ILpLocker.LpScheme scheme, uint lpValue, uint rate, uint lockDuration, uint24 poolFee, uint160 poolPrice, int24 minTick, int24 maxTick) external canConfigure {
        setupLpV2(locker, router, scheme, lpValue, rate, lockDuration);
    ...
    // If finalize() is called and we need to change parameter (even in the middle of a sale). 
    // modifyParam:
    // - Timings:       v1 = startTime, v2 = endTime
    // - Caps:          v1 = SoftCap, v2 = HardCap
    // - SvLaunchRange: v1 = minSvLaunch, v2 = cappedSvLaunch
    // - PublicAlloc    v1 = min, v2 = max
    // - FundAlloc      v1 = min, v2 = max
    // - StakerAlloc    v1 = lower, v2 = upper
    function modifySale(FcfsTypes.modifyParams param, uint v1, uint v2) external onlyDaoOrApprover {
        _require(getState(State.Finalized), "Not yet finalize. Use setup");

         if (param == FcfsTypes.modifyParams.Timings) {
            _config.startTime = v1;
            _config.endTime = v2; // v2 can be 0 (ie no ending time, manual stop)
            return;
        } 

        _require(v2 >= v1, "Invalid range");
        if (param == FcfsTypes.modifyParams.Caps) {
            _require(v2 >= _sales.totalFundRaised, "HardCap less than sold");
            _config.softCap = v1; // SoftCap can be 0
            _config.hardCap = v2;
            return;
        } 
        
        _requireNonZero(v1, v2);
        if (param == FcfsTypes.modifyParams.SvLaunchRange) {
            _global.minLaunch = v1;
            _global.capLaunch = v2;
        } else if (param == FcfsTypes.modifyParams.PublicAlloc) {
            _data.alloc.min = v1;
            _data.alloc.maxPublic = v2;
        } else if (param == FcfsTypes.modifyParams.FundAlloc) {
            _data.alloc.min = v1;
            _data.alloc.maxFund = v2;
        } else if (param == FcfsTypes.modifyParams.StakerAlloc) {
            _data.alloc.stakerRange.lower = v1;
            _data.alloc.stakerRange.upper = v2;
        }
    }
    ...
    function getStakerAllocation(uint stake) public virtual override view returns (uint) { 
        return _interpolate(stake, _global.minLaunch, _global.capLaunch, _data.alloc.stakerRange.lower, _data.alloc.stakerRange.upper);
    }
    ...
    function _setupCustomData(uint preOpenDuration, uint unitPrice, uint[5] calldata alloc) private {
        _requireNonZero(unitPrice);
        _requireNonZero(alloc[0], alloc[1], alloc[2]);
        _requireNonZero(alloc[3], alloc[4]);

        uint min = alloc[0];
        _require(alloc[1] >= min && alloc[2] >= min && alloc[4] >= min, "Invalid max");
        _require(alloc[3] >= alloc[2], "Invalid min max");
        ...
        _data.alloc.min = min;
        _data.alloc.maxPublic = alloc[1];
        _data.alloc.stakerRange.lower = alloc[2];
        _data.alloc.stakerRange.upper = alloc[3];
        _data.alloc.maxFund = alloc[4];
    }

    function _buy(uint fund, uint totalPayable, uint stake, bytes memory signature) private {

        _require(fund > 0 && _isConfigured(), "Cannot buy");
        ...
        if (stake > 0) {
            stake = _verifyStake(msg.sender, stake, signature);
    ...
    function _isLive(bool isPreOpenUser) private view returns (bool) {
        ...
        return (opened && !ended && !_isFinished() && !_isCancelled());
    }
}
...
library FcfsTypes {
    ...
    struct Range {
        uint lower;   // In currency unit
        uint upper;   // In currency unit
    }
