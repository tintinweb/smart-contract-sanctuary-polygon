    ...
    ERC20Upgradeable
{
    ...
    uint256 private prevFeeAmount;

    bytes4 public constant swapSig1 = bytes4(keccak256("swapExactTokensForTokens(uint256,uint256,address[],address,uint256)"));
    bytes4 public constant swapSig2 = bytes4(keccak256("swapExactTokensForTokensSupportingFeeOnTransferTokens(uint256,uint256,address[],address,uint256)"));
    ...
    function _transfer(
        ...
        bool isTrade = msg.sig == swapSig1 || msg.sig == swapSig2;
        uint256 feeAmount;
        ...
        if (!isWhitelist && isTrade) {
            ...
            prevFeeAmount = feeAmount - emissionAmount;
        }
    }
    ...
    function dynamicAdjustment(bool tradeType, uint256 amount) public onlyRift {
        bool isToken0 = pair.token0() == address(this);
        ERC20 stableToken = isToken0 ? ERC20(pair.token1()) : ERC20(pair.token0());
        uint256 stableBalance = stableToken.balanceOf(address(pair));
        uint256 circSupply = _getCirculatingSupply();
        
        uint256 truePrice = prevPrice + (prevFeeAmount * prevPrice / circSupply);
        ...
        require(adjustedPrice > prevPrice, "PozToken: price logic fails");
