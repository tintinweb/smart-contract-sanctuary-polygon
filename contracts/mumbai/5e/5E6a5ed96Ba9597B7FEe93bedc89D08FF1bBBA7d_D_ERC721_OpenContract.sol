...
contract D_ERC721_OpenContract is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function mint(
        ...
        uint256 tokenId = LibDiamond.diamondStorage().erc721Storage.tokenId;
        LibDiamond.diamondStorage().erc721Storage.tokenId++;
        console.log(LibDiamond.diamondStorage().erc721Storage.tokenId);
        LibDiamond.diamondStorage().erc721Storage.tokens[tokenId].uri = uri_;
        console.log(
            LibDiamond.diamondStorage().erc721Storage.tokens[tokenId].uri
        );
        LibDiamond.diamondStorage().erc721Storage.backendTokenKeyToId[
            backendTokenKey_
        ] = tokenId;
        LibDiamond.diamondStorage().erc721Storage.keyFallback.push(
            backendTokenKey_
        );
    ...
    function getTokenData(
        uint256 tokenId_
    ) external view returns (SharedStructs.Token memory) {
        SharedStructs.Token storage tokenStorage = LibDiamond
            .diamondStorage()
            .erc721Storage
            .tokens[tokenId_];
        SharedStructs.Token memory tokenMemory = SharedStructs.Token({
