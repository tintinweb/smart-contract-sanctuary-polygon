...
contract RevealTest is ERC721Mintable, ERC721Permit("RevealTest NFT:Token") , Ownable, ERC721URIStorage, ReentrancyGuard{
    // string public _defaultURI;
    string public _beforeURI;
    string public _afterURI;
    uint16 constant maxNumber = 10000;
    uint256 counter = 1;
    ...
    constructor() ERC721("RevealTest NFT:Token", "CAL") {
        setBeforeURI("https://metadata.nft-studio.com/castle/");
    }
    ...
    // }
    function setBeforeURI(string memory beforeURI_) public {
        _beforeURI = beforeURI_;
        emit UpdateDefaultURI(_beforeURI);
    }
    function setAfterURI(string memory afterURI_) public {
        _afterURI = afterURI_;
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory){
        ...
        if (ownerOf(tokenId)==owner()) {
            return string(abi.encodePacked(_beforeURI, tokenId+48));
        } else {
            return string(abi.encodePacked(_afterURI, tokenId+48));
        }
    }

    // function claim(uint256 _tokenId) public nonReentrant onlyOwner{
    //     require(_tokenId > 0 && _tokenId <= maxNumber, "Token ID invalid");
    //     require(_tokenId < counter, "Set Information");
    //     require(balanceOf(msg.sender) < 2, "you can't");
    //     _safeMint(_msgSender(), _tokenId);
    // }
    function claim() public nonReentrant {
        require(counter <= maxNumber, "you can't");
        require(balanceOf(msg.sender) < 2, "you can't");
        _safeMint(_msgSender(), counter);
        counter += 1;
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC721Mintable) returns (bool) {
...
import "../roles/AccessOperatableWithMinter.sol";
...
abstract contract AccessOperatable is AccessControl {
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER, msg.sender), "Must be minter");
        _;
    }
