...
import "./IPublicFactory.sol";
import "./PublicSale.sol";

contract PublicFactory is IPublicFactory{
    ...
    function createProject(
        ...
        project = address(new PublicSale());

        allProjects.push(project);
        
        PublicSale(project).initialize(
            _calculation,
            _start,
            _sale,
            _target,
...
contract PublicSale {
    IPublicFactory public immutable factory = IPublicFactory(msg.sender);
    ...
    function getUserAllocation(address _user, uint64 _boosterRunning) public view returns(uint128 userAlloc){
        ...
        if(_boosterRunning == 1){
            ...
            uint128 left = saleAmount - booster[1].achieve;
    ...
    function boosterProgress() public view returns (uint64 running) {
        ...
        for(uint64 i=1; i<=4; i++){
            if( (block.timestamp >= booster[i].start && block.timestamp <= booster[i].end) ||
                (i == 4 && block.timestamp >= booster[i].start)
            ){
                running = i;
                break;
            }
    ...
    function migrateCandidates(address[] calldata _candidates) external returns (bool) {
        require(msg.sender == gov && block.timestamp >= calculation && block.timestamp < booster[1].start, "bad");
    ...
    function setStart(uint128 _start) external onlyOwner {
        require(block.timestamp < booster[1].start, "bad");
