...
contract SoloService is BaseTournamentService {
  ...
  {
    isJoinedBetNFT = IChampionUtils(championUtils).isOriginalOwnerOf(address(this), _championID);
  ...
  function createChallenge(
    ...
    require(!IChampionUtils(championUtils).isOriginalOwnerOf(address(this), _restrictions.whitelist[1]), "TRV: Opponent already joined BN"); // require owner of token
...
contract ChampionUtils is Base {
  ...
  uint256 public maxFightPerChampion = 3;
