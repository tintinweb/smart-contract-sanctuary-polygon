...
/// @notice Market contract that sales ERC1155 tokens using an ERC20 as payment.
...
contract GOBMarket is AccessControl {
	...
	/**
	 * @notice Buy tokens of a ginven ERC1155.
	 * @param erc1155 ERC1155 addresss. Must exist as a valid address in `erc1155List()`.
	 * GOBMarket must have minting access on it.
	...
	function buyItem(
		...
		currency.safeTransferFrom(
			_msgSender(),
			address(this),
			items[erc1155][tokenId].price * quantity
		);

		// Mint erc1155
		IERC1155(erc1155).mint(to, tokenId, quantity, "");
	...
	/**
	 * @notice Changes the currency used to sell tokens. It cannot be changed
	...
	/**
	 * @notice Adds an ERC1155 contract that will hold the tokens to be sold (minted).
	...
	/**
	 * @notice Removes an ERC1155 contract. Reverts if `erc1155` has tokens listed for sale.
	 * @param erc1155 ERC1155 addresss
	 */
	...
	/**
	 * @notice Changes the priced of token listed for sale.
	 * @param erc1155 ERC1155 addresss.
	...
	/**
	 * @notice Withdraws all available funds of `currency`.
	...
	function withdraw(address _currency, uint256 amount) public onlyRole(WITHDRAW_ROLE) {
		...
		emit Withdraw(_currency, amount);
	}
