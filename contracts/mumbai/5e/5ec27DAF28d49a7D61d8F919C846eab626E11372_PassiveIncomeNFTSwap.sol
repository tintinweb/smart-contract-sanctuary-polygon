...
contract PassiveIncomeNFTSwap is Ownable {
    ...
    function swap(uint256 tokenId, uint8 lockDurationInMonths) external {
        nidhiNFT.safeTransferFrom(msg.sender, address(this), tokenId);
...
contract PassiveIncomeNFT is ERC721Enumerable, Ownable {
    ...
    function mint(
        ...
        mint(minter, lockedAmount, lockDurationInMonths, "");
    }
...
contract PassiveIncomeCalculator is IPassiveIncomeCalculator {
    ...
    function _totalPayout(
        ...
        return (((sqrt(
            ((startBalance * multiplier)) / ((endTime - startTime)**2)
        ) * (currentTime - startTime)) / 1e9)**2); // todo simplify
    }
