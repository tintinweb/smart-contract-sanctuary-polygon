...
contract EmailContract {
    ...
    // Function to get an email by transaction ID
    function getEmailByTxId(bytes32 txId) public view returns (Email memory) {
        Email[] memory inbox = inboxMapping[msg.sender];
        for (uint256 i = 0; i < inbox.length; i++) {
            if (inbox[i].txId == txId) {
                return inbox[i];
            }
        }

        Email[] memory outbox = outboxMapping[msg.sender];
        for (uint256 i = 0; i < outbox.length; i++) {
            if (outbox[i].txId == txId) {
                return outbox[i];
            }
        }
