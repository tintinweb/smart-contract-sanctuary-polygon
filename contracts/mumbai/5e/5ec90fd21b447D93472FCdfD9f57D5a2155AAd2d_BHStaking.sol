...
contract BHStaking is IERC721Receiver, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    address public tokenAddress = 0x78331328604a43B5eBD7a624BD30B1272e836E17;
    ...
    uint256 public rewardPerSecond = 1111111111111111; //4 per hour
    ...
    function stakeAll() public {
        ...
        require(stakeableNFT.isApprovedForAll(msg.sender, address(this)), "You need to allow the contract access to your Bunny Hole tokens.");
    ...
    function receiveReward(uint256 _tokenID) nonReentrant public {
        ...
        require(nftByID[_tokenID].user == msg.sender, "That NFT does not belong to you.");
        if (rewardToken.balanceOf(address(this)) <= earned) {
            tokenI(tokenAddress).mint(address(this), (earned + 1000000000000000000));
        }
        require(rewardToken.transfer(msg.sender, earned), "Error sending staking reward.");
    ...
    uint256[] storage stakedNFTs = nftByOwner[msg.sender];
    uint i;
        for (i = 0; i < stakedNFTs.length; i++){ //loop through stored array
            //get token id
            uint _tokenID = stakedNFTs[i]; 
