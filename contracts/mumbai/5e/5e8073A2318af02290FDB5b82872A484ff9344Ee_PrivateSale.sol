...
contract PrivateSale is OwnableUpgradeable, PausableUpgradeable{
    ...
    function roundWiseWhitelist(Round round, address account, bytes32[] memory proof) external view returns(bool) {
         bytes32 root = MerkleProofUpgradeable.processProof(proof, keccak256(abi.encodePacked(account)));
        return merkleRoots[round][root];
    }

    function buyRumToken(uint256 amountOfUsdc, bytes32[] memory proof) external whenNotPaused { 
        bytes32 root = MerkleProofUpgradeable.processProof(proof, keccak256(abi.encodePacked(msg.sender)));
    ...
    function withdrawUsdcToken(uint256 amount)
        external
        onlyOwner
    {
        usdc.transfer(withdrawAddress, amount);
    }
