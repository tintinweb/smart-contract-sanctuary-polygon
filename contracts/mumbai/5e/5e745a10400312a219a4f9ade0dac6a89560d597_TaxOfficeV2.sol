...
interface ITaxable {
    ...
    function setSharkOracle(address _SharkOracle) external returns (bool);
...
contract TaxOfficeV2 is Operator {
    ...
    address public Shark = address(0xb5AD37c87a6Fcb0EFbc2eeAA6F86719dc664B79b);
    address public WMatic = address(0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889);
    address public uniRouter = address(0x3a6eD047db6543b646474028AfCfED1d39b9B06a);

    mapping(address => bool) public taxExclusionEnabled;

    function setTaxTiersTwap(uint8 _index, uint256 _value) external onlyOperator returns (bool) {
        return ITaxable(Shark).setTaxTiersTwap(_index, _value);
    }

    function setTaxTiersRate(uint8 _index, uint256 _value) external onlyOperator returns (bool) {
        return ITaxable(Shark).setTaxTiersRate(_index, _value);
    }

    function enableAutoCalculateTax() external onlyOperator returns (bool) {
        return ITaxable(Shark).enableAutoCalculateTax();
    }

    function disableAutoCalculateTax() external onlyOperator returns (bool) {
        return ITaxable(Shark).disableAutoCalculateTax();
    }

    function setTaxRate(uint256 _taxRate) external onlyOperator returns (bool) {
        return ITaxable(Shark).setTaxRate(_taxRate);
    }

    function setBurnThreshold(uint256 _burnThreshold) external onlyOperator returns (bool) {
        return ITaxable(Shark).setBurnThreshold(_burnThreshold);
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) external onlyOperator returns (bool) {
        return ITaxable(Shark).setTaxCollectorAddress(_taxCollectorAddress);
    }
    ...
    function _excludeAddressFromTax(address _address) private returns (bool) {
        if (!ITaxable(Shark).isAddressExcluded(_address)) {
            return ITaxable(Shark).excludeAddress(_address);
        }
    }
    ...
    function _includeAddressInTax(address _address) private returns (bool) {
        if (ITaxable(Shark).isAddressExcluded(_address)) {
            return ITaxable(Shark).includeAddress(_address);
        }
    }

    function taxRate() external returns (uint256) {
        return ITaxable(Shark).taxRate();
    }
    ...
    {
        require(amtShark != 0 && amtToken != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);

        IERC20(Shark).transferFrom(msg.sender, address(this), amtShark);
        IERC20(token).transferFrom(msg.sender, address(this), amtToken);
        _approveTokenIfNeeded(Shark, uniRouter);
        _approveTokenIfNeeded(token, uniRouter);

        _includeAddressInTax(msg.sender);

        uint256 resultAmtShark;
        uint256 resultAmtToken;
        uint256 liquidity;
        (resultAmtShark, resultAmtToken, liquidity) = IUniswapV2Router(uniRouter).addLiquidity(
            Shark,
            token,
            amtShark,
            amtToken,
        ...
        if (amtShark.sub(resultAmtShark) > 0) {
            IERC20(Shark).transfer(msg.sender, amtShark.sub(resultAmtShark));
        }
        if (amtToken.sub(resultAmtToken) > 0) {
            IERC20(token).transfer(msg.sender, amtToken.sub(resultAmtToken));
        }
        return (resultAmtShark, resultAmtToken, liquidity);
    }
    ...
    {
        require(amtShark != 0 && msg.value != 0, "amounts can't be 0");
        _excludeAddressFromTax(msg.sender);
        ...
        uint256 liquidity;
        (resultAmtShark, resultAmtEth, liquidity) = IUniswapV2Router(uniRouter).addLiquidityETH{value: msg.value}(
            Shark,
            amtShark,
            amtSharkMin,
            amtEthMin,
        ...
        return (resultAmtShark, resultAmtEth, liquidity);
    }

    function setTaxableSharkOracle(address _SharkOracle) external onlyOperator returns (bool) {
        return ITaxable(Shark).setSharkOracle(_SharkOracle);
    }

    function transferTaxOffice(address _newTaxOffice) external onlyOperator returns (bool) {
        return ITaxable(Shark).setTaxOffice(_newTaxOffice);
    }
    ...
    function taxFreeTransferFrom(
        ...
        IERC20(Shark).transferFrom(_sender, _recipient, _amt);
