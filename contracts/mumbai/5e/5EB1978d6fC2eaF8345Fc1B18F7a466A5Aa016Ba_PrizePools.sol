...
contract PrizePools is IPrizePools, VRFConsumerBaseV2, Ownable {
    ...
    uint256 private immutable i_userCap;
    ...
    function registerUser(
        ...
        if (s_pools[poolId].registeredUsers.length >= i_userCap) {
            revert PrizePools_PoolFull(poolId);
        }
    ...
    function getPrizePool(uint256 id) external view returns (PrizePool memory) {
        return s_pools[id];
    }
