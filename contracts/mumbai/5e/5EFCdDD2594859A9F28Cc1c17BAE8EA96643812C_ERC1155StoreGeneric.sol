    ...
    EIP712Upgradeable
{
    ...
    uint256 private s_adminEthPayout;
    mapping(address => uint256) private s_adminTokenPayout;
    ...
    function createSale(
        ...
        if (!ERC165CheckerUpgradeable.supportsInterface(_token, type(IERC1155).interfaceId))
            revert SG__NotERC1155(_token);
    ...
    function withdraw(address _walletAddress, address _currency) external nonReentrant onlyGov {
        if (_currency == address(0)) {
            if (s_adminEthPayout == 0) revert SG__ZeroValue();
            uint256 amount = s_adminEthPayout;
            s_adminEthPayout = 0;
            (bool success, ) = payable(_walletAddress).call{value: amount}("");
            if (!success) revert SG__WithdrawalFailed();
        } else {
            if (s_adminTokenPayout[_currency] == 0) revert SG__ZeroValue();
            uint256 amount = s_adminTokenPayout[_currency];
            s_adminTokenPayout[_currency] = 0;
            IERC20(_currency).approve(address(this), amount);
            bool success = IERC20(_currency).transferFrom(address(this), _walletAddress, amount);
            if (!success) revert SG__WithdrawalFailed();
        }
    ...
    function toggleState() external onlyGov {
    ...
    function onPaper(
        ...
        external
        onlyPaper(
            keccak256(abi.encode(buyer, tokenAddressRebate, tokenIdRebate, numPurchases, saleId)),
            _nonce,
            _signature
        )
    {
        ...
        if (_tokenAddress != address(0)) {
            ...
            for (uint256 i; i < ownershipDiscounts.length; ++i) {
                if (
                    ...
                    (ownershipDiscounts[i].tokenType == TokenType.ERC721 ||
    ...
    function _buyTokens(
        ...
        if (_currency == address(0)) {
            _ethPayment(saleInputs.buyer, sale.defaultCurrency, _currency, balance);
        } else {
            _erc20Payment(
                saleInputs.buyer,
                sale.defaultCurrency,
                _currency,
                balance,
                sale.profitState
            );
    ...
    function _ethPayment(
        ...
        if (ethPrice > msg.value) {
            ...
            s_adminEthPayout += (ethPrice - total);
    ...
    function _distributeBeneficiaryTokens(address _currency, uint256 _price) internal {
        ...
        s_adminTokenPayout[_currency] += (_price - total);
    }
...
interface IERC1155StoreGeneric {
     ...
     * @param _defaultCurrency Default currency (contract 
