...
contract MockRandomNumberGenerator is IRandomNumberGenerator, Ownable {
    ...
    /**
     * @notice Set the address for the ArkaLottery
     * @param _arkaLottery: address of the Arka lottery
     */
    function setLotteryAddress(address _arkaLottery) external onlyOwner {
        arkaLottery = _arkaLottery;
    }
     ...
     * @param _seed: seed provided by the Arka lottery
     */
    function getRandomNumber(uint256 _seed) external override {
        require(msg.sender == arkaLottery, "Only ArkaLottery");
    ...
    function changeLatestLotteryId() external {
        latestLotteryId = IArkaLottery(arkaLottery).viewCurrentLotteryId();
    }
...
interface IArkaLottery {
    ...
    /**
     * @notice Draw the final number, calculate reward in NativeAsset per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in NativeAsset token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInNativeAsset: price of a ticket in NativeAsset
    ...
    function startLottery(
        uint256 _endTime,
        uint256 _priceTicketInNativeAsset,
