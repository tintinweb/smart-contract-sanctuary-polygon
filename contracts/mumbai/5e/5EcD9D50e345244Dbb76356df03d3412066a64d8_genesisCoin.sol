...
contract NFToken is ERC721, SupportsInterface {
    ...
    function getOwnerOf(
        uint256 _tokenId
    ) internal virtual returns (address _owner) {
        _owner = idToOwner[_tokenId];
        require(_owner != address(0), NOT_VALID_NFT);
    }
...
contract NFTokenMetadata is NFToken, ERC721Metadata {
    ...
    function tokenURI(
        uint256 _tokenId
    ) public view override validNFToken(_tokenId) returns (string memory) {
...
contract genesisCoin is NFTokenMetadata, Ownable {
    ...
    address public gameCoverContract=0x3A4565a9cA59c6999614A0c5AC1F124F5D41B122;
    ...
    function createNewCharacter(uint256 _id, string memory folderHash) public {
        ...
        characterToCoinURI[_id] = string(
            abi.encodePacked(folderHash, "/coin.json")
        );
    ...
    function createNewCharacterIn(
        uint256 _id,
        string memory folderHash
    ) internal {
        characterToCoinURI[_id] = string(
            abi.encodePacked(folderHash, "/coin.json")
        );
    ...
    function mintIn(address _to, uint256 _id) internal {
        ...
        uint256 _tokenId = characterToStartingID[_id] +
            characterToCurrentID[_id];
    ...
    function mintGenesis(address _to, uint256 _id) public {
        // require(msg.sender == gameCoverContract, "Invalid Caller");
        require(gameCoverToCurrentID[_id] < 2500);
        uint256 _tokenId = gameCoverToStartingID[_id] +
            gameCoverToCurrentID[_id];
        super._mint(_to, _tokenId);
        super._setTokenUri(_tokenId, gameCoverToCoinURI[_id]);
        gameCoverToCurrentID[_id] += 1;
    }

    function mintCharacterGenesis(address _to, uint256 _id) public {
        // require(msg.sender == charContract, "Invalid Caller");
        require(characterToCurrentID[_id] < 2500);
        uint256 _tokenId = characterToStartingID[_id] +
            characterToCurrentID[_id];
    ...
    function setGameCoverHash(uint256 _id, string memory _hash) public {
        require(isTeam(msg.sender));
        gameCoverToCoinURI[_id] = _hash;
    }
