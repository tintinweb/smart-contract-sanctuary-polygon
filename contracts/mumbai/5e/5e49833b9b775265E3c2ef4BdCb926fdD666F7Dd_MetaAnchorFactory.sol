 ...
 *    4. Call `SlimFactory.updateAuthorizedProvider(providerAddr)`. The version will be requested from `providerAddr` onchain.
 *    5. Register
...
contract SlimFactory is ERC165, IInitArgsProvider {
    ...
    /**
     * @notice Registers a new Contract deployment of contract with version X.Y.Z, returns the predicted address 
    ...
    function register(string memory _version, bytes memory _deployArgs, bytes memory _initArgs) public onlyRegistrar() whenOperational() returns (address registeredAddress) {    
        ...
        registeredAddress = _predictAddress(_provider, _deployArgs);
    ...
    function _predictAddress(address _provider, bytes memory _deployArgs) internal view returns (address) {
        BaseProvider p = BaseProvider(_provider);
        return getAddress(p.getBytecode(_deployArgs), _staticSalt);        
    }

    /**
     * 
     * @param deploymentAddress Previously registered address for deployment
     */
    function _deploy(address deploymentAddress) internal whenOperational() returns (address)  {
        BaseProvider p = BaseProvider(providerForAddress[deploymentAddress]);
        
        bytes memory argsForDeployment = deployArgs[deploymentAddress]; // TODO get from registered Deployments
        bytes memory byteCodeForDeployment = p.getBytecode(argsForDeployment); 

        require(p.verifyByteCode(byteCodeForDeployment, bytecodeHash[deploymentAddress]), "Bytecode verification failed, version mismatch?");

        address deployedAddress = _deployBinary(byteCodeForDeployment, _staticSalt);

        require(deployedAddress == deploymentAddress, "Predicted and deployed address mismatch");

        ProvidableContract pc = ProvidableContract(deployedAddress);
        delete pendingDeployments[deploymentAddress];
    ...
    /**
     * @notice Actually deploys the previously registered contract.
    ...
    function deploy(address registeredAddress) public virtual 
        ...
        returns (address)
    {
        return _deploy(registeredAddress);
    }

    /**
     * @notice Adds or updates a provider contract
     ...
     * @param _providerAddress Address of the deployed ProviderContract (which extends BaseProvider)
     */
    function updateProvider(address _providerAddress) public onlyMaintainer() {
        BaseProvider bp = BaseProvider(_providerAddress);
        string memory _version = bp.getVersion();
        require(providerByVersion[_version] != _providerAddress, "No change required");
        emit ProviderUpdate(_providerAddress, _version, msg.sender );
        providerByVersion[_version] = _providerAddress;
    }
    ...
    constructor(address _appHub) {
        authorizedMaintainer[msg.sender] = true;
    ...
    mapping (address => bool) public authorizedMaintainer;
    mapping (address => bool) public authorizedRegistrar;

    function isAuthorizedDeployer(address a) public virtual view returns (bool) {
        return authorizedDeployer[a];
    }
    ...
    function updateAuthorizedDeployer(address a, bool _add) public onlyMaintainer() {
        require(authorizedDeployer[a] != _add, "No change required");
        emit DeployerUpdate(a, _add, msg.sender);
        authorizedDeployer[a] = _add;
    }

    function isAuthorizedRegistrar(address a) public virtual view returns (bool) {
        return authorizedRegistrar[a];
    }
    ...
    function updateAuthorizedRegistrar(address a, bool _add) public onlyMaintainer() {
        require(authorizedRegistrar[a] != _add, "No change required");
        emit RegistrarUpdate(a, _add, msg.sender);
        authorizedRegistrar[a] = _add;
    }
    ...
    function isAuthorizedMaintainer(address a) public virtual view returns (bool) {
        return authorizedMaintainer[a];
    }

    function updateAuthorizedMaintainer(address a, bool _add) public onlyMaintainer() {
        require(authorizedMaintainer[a] != _add, "No change required");
        emit MaintainerUpdate(a, _add, msg.sender);
        authorizedMaintainer[a] = _add;
    }
