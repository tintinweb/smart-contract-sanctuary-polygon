...
contract TangibleRevenueShare2 is AccessControl {
    ...
    mapping(uint256 => int256) public total;
    mapping(address => int256) public share;
    mapping(address => mapping(uint256 => int256)) public changes;
    ...
    function updateTotal(int256 amount) external onlyRole(SHARE_MANAGER_ROLE) {
        _total = uint(int(_total) + amount);
        total[currentCycle()] += amount;
    }

    function updateShare(address account, int256 amount)
        external
        onlyRole(SHARE_MANAGER_ROLE)
    {
        share[account] += amount;
        changes[account][currentCycle()] += amount;
    }

    function claimable(address account) public view returns (uint256 amount) {
        int currentShare = share[account];
        int currentTotal = int(_total) - total[currentCycle()];
        uint256 numCycles = cycles.length;
        uint256 lastCycle = lastClaim[account];
        mapping(uint256 => int256) storage change = changes[account];
        for (uint256 i = 2; i < numCycles; i++) {
            uint256 cycle = cycles[numCycles - i];
            if (cycle <= lastCycle) break;
            currentShare -= change[cycle];
            currentTotal = total[cycle];
            amount += (revenue[cycle] * uint(currentShare)) / uint(currentTotal);
        }
    }
}
