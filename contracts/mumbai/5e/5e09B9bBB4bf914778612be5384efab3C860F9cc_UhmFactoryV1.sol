...
contract StemV1 is ERC1155, ERC1155Supply, Ownable {
    string[] public names; //string array of names
    uint[] public ids; //uint array of ids
    uint[] public fees; //uint array of fees
    uint[] public supplies; //uint array of supplies
    ...
    mapping(uint => Stem) public idToStem; //id to Stem mapping
    ...
    */
    function _createMapping() private {
        for (uint id = 0; id < ids.length; id++) {
            nameToId[names[id]] = ids[id];
            idToName[ids[id]] = names[id];
            idToStem[ids[id]] = Stem(ids[id], names[id], supplies[id], fees[id]);
        }
    }
    ...
    */
    function setFee(uint _id, uint _fee) public onlyOwner {
        idToStem[_id].mintFee = _fee;
    }
    ...
    */
    function mint(address account, uint _id, uint256 amount) public payable returns (uint) {
        if (msg.value < (idToStem[_id].mintFee * amount)) {
            revert InsufficientBalance({ available: msg.value, required: (idToStem[_id].mintFee * amount) });
        }

        if ((amount + totalSupply(_id)) > idToStem[_id].supply) {
            revert SupplyError();
        }
...
contract UhmFactoryV1 is Ownable {
    StemV1[] public stems; //an array that contains different StemV1 tokens deployed
    mapping(uint256 => address) public indexToContract; //index to contract address mapping
    mapping(uint256 => address) public indexToOwner; //index to StemV1 owner address

    event StemV1Created(address owner, address tokenContract); //emitted when StemV1 token is deployed
    event StemV1Minted(address owner, address tokenContract, uint amount); //emmited when StemV1 token is minted

    constructor() {
        console.log("Deploying UhmFactoryV1 Contract");
    }

    /*
    deployStemV1 - deploys a StemV1 token with given parameters - returns deployed address

    _contractName - name of our StemV1 token
    _uri - URI resolving to our hosted metadata
    _ids - IDs the StemV1 token should contain
    ...
    */
    function deployERC1155(
        ...
        StemV1 t = new StemV1(_contractName, _uri, _names, _ids, _fees, _supplies);
        stems.push(t);
        indexToContract[stems.length - 1] = address(t);
        indexToOwner[stems.length - 1] = tx.origin;
        emit StemV1Created(msg.sender, address(t));

        console.log("Deploying StemV1 Contract deployer: '%s' to adddres: '%s'", msg.sender, address(t));
    ...
    */
    function mintERC1155(uint _index, string memory _name, uint256 amount) public payable {
        uint id = getIdByName(_index, _name);
        stems[_index].mint{ value: msg.value }(indexToOwner[_index], id, amount);
        emit StemV1Minted(stems[_index].owner(), address(stems[_index]), amount);
    }
    ...
    */
    function getCountERC1155byIndex(uint256 _index, uint256 _id) public view returns (uint amount) {
        return stems[_index].balanceOf(indexToOwner[_index], _id);
    }

    function getCountERC1155byName(uint256 _index, string calldata _name) public view returns (uint amount) {
        uint id = getIdByName(_index, _name);
        return stems[_index].balanceOf(indexToOwner[_index], id);
    }

    function getIdByName(uint _index, string memory _name) public view returns (uint) {
        return stems[_index].nameToId(_name);
    }

    function getNameById(uint _index, uint _id) public view returns (string memory) {
        return stems[_index].idToName(_id);
    }

    function getERC1155byIndexAndId(
        uint _index,
        uint _id
    ) public view returns (address _contract, address _owner, string memory _uri, uint supply) {
        StemV1 stem = stems[_index];
        return (address(stem), stem.owner(), stem.uri(_id), stem.balanceOf(indexToOwner[_index], _id));
    }
}
