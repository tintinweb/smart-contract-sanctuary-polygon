...
// File: contracts/interfaces/IYodaLottery.sol

pragma solidity ^0.8.4;

interface IYodaLottery {
    /**
     ...
     * @param _priceTicketInToken: price of a ticket in Token
...
// File: contracts/PanTokenSwapLottery.sol
 ...
 */
contract YodaLottery is ReentrancyGuard, IYodaLottery, Ownable {
    ...
    uint256 public maxPriceTicketInToken = 50 ether;
    uint256 public minPriceTicketInToken = 0.005 ether;
        ...
        nonReentrant
    {
        ...
        token.safeTransferFrom(address(msg.sender), address(this), amountTokenToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInToken += amountTokenToTransfer;
    ...
    function claimTickets(
        ...
        token.safeTransfer(msg.sender, rewardInTokenToTransfer);

        emit TicketsClaim(msg.sender, rewardInTokenToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInToken) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInToken - amountToShareToWinners);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        _lotteries[_lotteryId].amountCollectedInToken += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInToken >= minPriceTicketInToken) && (_priceTicketInToken <= maxPriceTicketInToken),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInToken: _priceTicketInToken,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            TokenPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(token), "Cannot be Token token");
     ...
     * @dev Only callable by owner
     * @param _minPriceTicketInToken: minimum price of a ticket in Token
     * @param _maxPriceTicketInToken: maximum price of a ticket in Token
     */
    function setMinAndMaxTicketPriceInToken(uint256 _minPriceTicketInToken, uint256 _maxPriceTicketInToken)
        external
        onlyOwner
    {
        require(_minPriceTicketInToken <= _maxPriceTicketInToken, "minPrice must be < maxPrice");

        minPriceTicketInToken = _minPriceTicketInToken;
        maxPriceTicketInToken = _maxPriceTicketInToken;
    }
