...
library ParticularLogic {
  event ReduxInterestToTreasuryCall(
   ...
   **/
  function executeReduxInterestToTreasuryCall(
    mapping(address => DataTypes.ReserveData) storage reservesData,
    mapping(uint256 => address) storage reservesList,
    DataTypes.ExecuteReduxInterestToTreasuryCallParams memory params
  ) external {
    for (uint16 i = 0; i < params.reservesCount; i++) {
      address asset = reservesList[i];
      DataTypes.ReserveData storage reserve = reservesData[asset];  
      uint256 amount = IPToken(reserve.pTokenAddress).reduxInterestToTreasury();
      emit ReduxInterestToTreasuryCall(asset, amount);
    }
  }
  ...
  function executeClaimPrxy(
    ...
    ValidationLogic.validateClaimPrxyCall(reserve, params.amount);
    IPToken(reserve.pTokenAddress).claimPrxy(msg.sender, params.amount);
    IPrxyTreasury(params.prxyTreasury).transferPrxyTo(msg.sender, params.amount);
    emit ClaimPrxy(params.asset, msg.sender, params.amount);
  }
}
