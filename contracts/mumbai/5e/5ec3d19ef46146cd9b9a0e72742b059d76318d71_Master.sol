...
interface IMaster {
    ...
    event PlayGame(address indexed gameAddress, uint256 indexed ticketId, bool indexed isWin, address ticketOwner);
    ...
    function emitPlayGame(address gameAddress, uint256 ticketId, bool isWin, address ticketOwner) external;
    ...
    ERC721URIStorage
{
    ...
    function playGame(uint256 ticketId) external {
        require(
    ...
    */
    function addComments(string memory comment) public {
    ...
    */
    ...
    /*
        Store Rating  
    */
    function addRatings(uint8 rating) public {
        TicketDetails[] memory id = new TicketDetails[](ticketCounter);
        bool isPurchasedOwner;
        for (uint256 i = 0; i < ticketCounter; i++) {
            if(id[i].ticketOwner == msg.sender) {
...
contract Master is IBase, IMaster {
    ...
    function createGame(
        ...
        ownedGames[creatorAddress].push(address(cg));
    ...
    function emitPlayGame(
        address gameAddress,
        uint256 ticketId,
        bool isWin,
        address ticketOwner
    ) external {
        emit PlayGame(gameAddress, ticketId, isWin, ticketOwner);
    }
    ...
    function emitCommentsAdded(
        ...
        emit AddComment(gameAddress, commenterAddress, comment, time);
    }
    ...
    function emitRatingsAdded(
        ...
        emit AddRating(gameAddress, ratingAddress, rating, time);
    }
