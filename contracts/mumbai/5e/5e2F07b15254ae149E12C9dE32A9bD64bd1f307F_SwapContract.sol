...
contract SwapContract is Ownable {
    ...
    // MetacToken : ; //for mainnet yet to deploy on polygon mainnet
    // MetacToken : 0xA6c208D6276f88f5BBe25582a5DE9b2B35C81eaB; for testnet polygon 
  
        
    constructor(MetacToken _token) public {
        token = _token;
    ...
    **/
    ...
    function read(address add) public view returns(uint256 _depositMATIC, uint256 _depositBUSD, uint256 _lockedMETAC, uint256 _restTime, bool _checkTime) {
        _depositMATIC = depositsMATIC[add];
        _depositBUSD = depositsUSDC[add];
    ...
    // //Shows Matic amount equals to in usdc (6 decimal)
    // function theUSDCAmount(uint256 maticAmount) private view returns(uint256){
    //     uint256 usdcAmount = maticAmount.mul(getUSDCPerMATIC());
    //     return (usdcAmount).div(1e2);
    // }

    //make new function for matic 
    //Calculate the value of METAC user will get against their USDC
    function getTokensAmount(uint256 usdcAmount) public view returns(uint256) {
        uint256 tokensAmount = ((usdcAmount*1e12).mul(1e18)).div(tokensAmountForUSDC);
        return tokensAmount;
    }

    function MaticTokensAmount(uint256 usdcAmount) public view returns(uint256) {
