...
contract MaxBet is Pool {
    ...
    */
    ...
    function placeBet(
        ...
        playerLastBet[msg.sender] = commitment;
    ...
    function settleBet(
        ...
        if ((bet.isOver && luckyNum > bet.number) || (!bet.isOver && luckyNum < bet.number)) {
            emit DrawBet(commitment, bet.luckyNumber, winAmount);
        } else {
            emit DrawBet(commitment, bet.luckyNumber, 0);
        }
    }
...
contract Pool is Ownable {
    ...
    function getPoolStake() public view returns (address[] memory, uint256[] memory) {
        uint256[] memory listAmounts = new uint256[](stakers.length);
        for (uint256 i = 0; i < stakers.length; i++) {
            listAmounts[i] = stakes[stakers[i]].amount;
        }
        return (stakers, listAmounts);
    }
