...
contract web3domain is Ownable, ERC721, ERC721URIStorage, ERC721Enumerable {
    using strings for *;
    uint256 private nft_price = 1;
     uint256 private nft_comm_web3 = 80;
       uint256 private nft_comm_dealer = 10;
    struct Metadata {
        uint256 id;
        string title;
        uint256 allow;
    ...
    function claim(
        ...
        if (keccak256(bytes(primary)) != keccak256(bytes(""))) {
            ...
            if (getAllow(iid) == 0) {
                ...
                payable(getOwner(iid)).transfer((msg.value * getComm_Web3()) / 100);
                payable(owner()).transfer((msg.value * getComm_Dealer()) / 100);
            }
        } else {
            payable(owner()).transfer(msg.value);
        }
    ...
    constructor() ERC721("web3domain", "Web3Domain") {
        // mint(0, "");
    }
