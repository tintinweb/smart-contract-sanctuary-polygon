...
contract Mix {
    ...
    // Fee wallet
    address payable public feeWallet = payable(0x5Be794A7DcFccF77B8505260256cF2156C392d50);

    // SafeMath library instance
    ...
    function deposit() public payable {
        ...
        mixingPool = mixingPool.add(msg.value);
    }
    ...
    function withdraw(uint _amount) public {
        ...
        // Calculate 2% fee
        uint fee = _amount.mul(2).div(100);
        // Subtract withdrawn amount and fee from user balance
        balances[msg.sender] = balances[msg.sender].sub(_amount.add(fee));
        // Send withdrawn amount toto user
...
function mix() public payable {
    ...
    // Subtract mixing amount and fee from user balance
    balances[msg.sender] = balances[msg.sender].sub(msg.value.add(fee));
    // Add mixing amount to mixing pool
    mixingPool = mixingPool.add(msg.value);
    // Withdraw deposited amount from mixing pool
    mixingPool = mixingPool.sub(msg.value);
    // Send deposited amount to user
