...
contract BreakTheSilence is ERC721URIStorage {
    ...
    constructor() ERC721("BreakTheSilence", "BRTS") {
        owner = msg.sender;
        mintFee =2500000000000000;
    }

    function createToken(string memory tokenURI) public payable  returns (uint) {
        require(msg.value == getMintfee(msg.sender), "Add More Eth");
    ...
    function getMintfee(address Minter) private view returns (uint){
        if (Minter==owner){
            return 0;
        } else {
            return mintFee;
    ...
    function setFee(uint fee) public {
        require(msg.sender == owner, "Only owner is allowed to Set Fee");
        mintFee = fee;
