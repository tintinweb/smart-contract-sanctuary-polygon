...
contract ApeCoinNFT is ERC721, SismoConnect {
  ...
  function claimWithSismo(bytes memory response) public {
    uint256 requiredApeAmount = 1 * 10 ** 18;
    if (
      apeToken.balanceOf(msg.sender) < requiredApeAmount ||
      !apeToken.transferFrom(msg.sender, address(this), requiredApeAmount)
    ) {
      revert ApeCoinNFT__PaymentNotMet();
    }
