...
contract USDRBonding is AddressAccessor {
    ...
    function reset() external {
        ...
        (address exchange, address staking) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(USDR_EXCHANGE_ADDRESS, STAKING_ADDRESS)
            ),
            (address, address)
        );
        uint256 scaledMarketCap = IExchange(exchange).scaleToUnderlying(
            IStaking(staking).usdrMarketCap()
        );
        available = (scaledMarketCap * percentage) / 10000;
    }
