...
contract EscrowStaking is Ownable {
    ...
    uint256 public minAmount = 200000 * 10 ** 18;
    ...
    function getApr(address account) public view returns (AprInfo memory _a) {
        ...
        uint256 nftBalance = INFT(nftToken).balanceOf(account);
        if (nftBalance >= 10) {
            _aprInfo.percent += 6;
        }
        else if (nftBalance >= 5) {
            _aprInfo.percent += 4;
        }
