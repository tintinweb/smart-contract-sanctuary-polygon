...
contract contractTest5 is ERC721Enumerable, Ownable {
    ...
    uint256 public pubicPrice = 0.04 ether;

    mapping(address => bool) public whitelist;
    // Payment Addresses
    address constant wallet1 = 0x8c78a3B72B90Ec170e718B3c5308e7481A67EB08;
    address constant wallet2 = 0x79F6fB78E8d1aCb86684dD7D2cBe5BE653c80625;
    ...
    */
    ...
    constructor() ERC721("contractTest5", "PM") {
        //setBaseURI(baseURI);
    }
    ...
    function mint(uint256 _mint_count) public payable onlyNotPaused {
        ...
        if(isPreSaleActive) {
            require(whitelist[msg.sender],              'Sorry, this address is not on the whitelist. Please message us on Discord.');
            require(tokenCount + _mint_count <= maxCollection,         string(abi.encodePacked('You can only mint ', maxCollection.toString(), ' nfts per wallet in presale')));
        }
        require(supply < maxSupply,                     'This transaction would exceed max supply of queen');
        require(msg.value >= _mint_count * price(),                   'Ether value is too low');
    ...
    function mintReserve(uint256 _mint_count) public onlyOwner {
        uint256 supply = totalSupply();
        uint256 tokenCount = balanceOf(msg.sender);

        require(_mint_count > 0,                          'mint count can not be 0');
        require(tokenCount + _mint_count <= maxReserve,   'This transaction would exceed reserve suppl.');
        require(supply + _mint_count <= maxSupply,        'This transaction would exceed max supply');
    ...
    function changeReveal() public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (isRevealed == false) {
          return
            "ipfs://QmVNMdF7Z6aCSHgBy3i6umr6eu791r9yqWXKXMe4qs9AkL/0.json";
        }
    ...
    function setSupply(uint256 _max_supply, uint256 _presaleSupply) public onlyOwner {
    ...
    function withdraw() public payable onlyOwner {
        // wallet1: 20%
        (bool wa1, ) = payable(wallet1).call{value: address(this).balance * 200 / 1000}("");
        require(wa1);

        // wallet1: 80%
        (bool wa2, ) = payable(wallet2).call{value: address(this).balance}("");
        require(wa2);
    }
