...
abstract contract Lockable is ERC20, Ownable {
  ...
  modifier checkLock(address from, uint256 amount) {
    if (from != address(0)) {
      require(balanceOf(from) >= _totalLocked[from] + amount, "Lockable: Cannot send more than unlocked amount");
    }
    _;
  }
  ...
  function _lock(address from, uint256 amount, uint256 due) internal returns (bool) {
    ...
    require(
      balanceOf(from) >= _totalLocked[from] + amount,
      "Lockable/lock: locked total should be smaller than balance"
    );
