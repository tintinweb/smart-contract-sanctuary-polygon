...
contract BattlePoly {
            ...
            for (uint8 class_ = classFrom_; class_ < classTo_; class_++) { // for (uint8 class_ = 0; class_ < 8; class_++) {
                for (uint8 breed_ = 0; breed_ < 8; breed_++) {
                    ...
                    statInfo[level_][class_][breed_][1] = StatInfo(
                        ...
                        heart_ * int(policyVariables[PolicyVariables.ENEMY_STAT_POINTS_PERC]) / int(PERCENT_MULTIPLIER),
            ...
            for (uint8 class_ = 0; class_ < 8; class_++) {
                for (uint8 breed_ = 0; breed_ < 8; breed_++) {
                    ...
                    statInfo[level_][class_][breed_][1] = StatInfo(
                            ...
                            ds_.heart * int(perc_) / int(PERCENT_MULTIPLIER),
    ...
    function combatSingle(uint256 dogeId) external {
        ...
        hp_[1] = uint(statInfo[doge_.level][doge_.class][doge_.breed][1].heart) * policyVariables[PolicyVariables.HP_PER_HEART];
        ...
        while(hp_[0] > 0 && hp_[1] > 0) { // turn play
            ...
            uint eSpeed_ = vrf.getRandom("enemy speed") % uint(statInfo[doge_.level][doge_.class][doge_.breed][1].speed);

            uint8[2] memory playerTypes_ = dSpeed_ >= eSpeed_ ? [0, 1] : [1, 0];
    ...
    function _getBaseDamage(IDogewood.Doge2 memory doge_, uint8 playerType_) internal returns (uint baseDamage_) {
        ...
        if(classInfo[doge_.class].attackType == AttackTypes.Melee) {
            ...
            maxDamage_ = uint(statInfo[doge_.level][doge_.class][doge_.breed][playerType_].smarts); // AttackTypes.Magic
        }
    ...
    function _rollDodge(IDogewood.Doge2 memory doge_, uint8 dodgePlayerType_) internal returns (bool) {
        ...
        uint maxDodge_;
        if(classInfo[doge_.class].attackType == AttackTypes.Melee) {
            stat_ = uint(statInfo[doge_.level][doge_.class][doge_.breed][dodgePlayerType_].speed);
