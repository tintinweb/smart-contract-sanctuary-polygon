...
contract Voting is Ownable, AccessControl, Pausable {
    ...
    event ContestInited(string indexed contestId, uint256 startDate, uint256 endDate, uint256 entryFee, uint256 entryRewardVoteLimit);
    event ContestAdjusted(string indexed contestId, uint256 startDate, uint256 endDate, uint256 entryFee, uint256 entryRewardVoteLimit);
    event ContestEntryAdded(string indexed contestId, string indexed projectId, uint256 entryVotingReward);
    event Voted(string indexed contestId, string indexed projectId, address indexed userAddress,uint256[SCORE_TYPE] scores);
    event UserContestRewardAdded(string indexed contestId, address indexed userAddress, uint256 rewardAmount);
    event UserContestRewardClaimed(string indexed contestId, address indexed userAddress, uint256 claimAmount);
        ...
        dateRangeValid(_startDate, _endDate)
    {
        ...
        contest.entryRewardVoteLimit = _entryRewardVoteLimit;

        emit ContestInited(_contestId, _startDate, _endDate, _entryFee, _entryRewardVoteLimit);
    }

    function addContestEntry(
        string memory _contestId,
        string memory _projectId,
        uint256 _entryVotingReward
    )
        external
        onlyRole(WORKER)
        ...
        beforeContestStarted(_contestId)
    {
        ...
        contest.reward += _entryVotingReward;
        contest.rewardPerParticipation = contest.reward / contest.entryCount / contest.entryRewardVoteLimit;

        emit ContestEntryAdded(_contestId, _projectId, _entryVotingReward);
    }
        ...
        userExist(_userAddress)
    {
        ...
        require(
            ...
            "[Voting.voteForContestEntry] Cannot vote before contest start"
        );

        require(
            block.timestamp < contest.endDate,
            "[Voting.voteForContestEntry] Cannot vote after contest end"
        );
        require(contest.entryIdByProjectId[_projectId] != 0, "[Voting.voteForContestEntry] Entry not exist for specifiy project id");
        ...
        require(vote.role == 0, "[Voting.voteForContestEntry] User already voted");

        updateVotingData(contest, entry, vote, _userAddress, _scores);
    ...
    function updateVotingData(
        ...
        // Update Contest Data
        if (_entry.voteCount < _contest.entryRewardVoteLimit) {
            _contest.userReward[_userAddress] += _contest.rewardPerParticipation;

            emit UserContestRewardAdded(_contest.contestId, _userAddress, _contest.rewardPerParticipation);
        }
        _contest.userParticipation[_userAddress].push(_entry.projectId);
    ...
    function claimUserContestReward(
        ...
        userExist(_userAddress)
    {
        uint256 claimAmount = contestByContestId[_contestId].userReward[_userAddress];
        ...
        emit UserContestRewardClaimed(_contestId, _userAddress, claimAmount);
        delete contestByContestId[_contestId].userReward[_userAddress];
    }
        ...
        // dateRangeValid(_startDate, _endDate)
    {
        ...
        emit ContestAdjusted(_contestId, _startDate, _endDate, _entryFee, _entryRewardVoteLimit);
    }
        ...
        )
    {
        ...
        rewardPerParticipation = contest.rewardPerParticipation;
        entryFee = contest.entryFee;
        entryRewardVoteLimit = contest.entryRewardVoteLimit;
    ...
    function getContestUserReward(
        ...
        return contestByContestId[_contestId].userReward[_userAddress];
    }
