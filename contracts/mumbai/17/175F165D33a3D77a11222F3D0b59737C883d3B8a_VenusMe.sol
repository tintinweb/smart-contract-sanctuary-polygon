...
contract VenusMe is ERC721, ERC721Royalty, ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    // 5555 NFTS available on Polygon blockchain.
    // 16.665 in total on three blockchains:
    ...
    // Price initially set to 140 MATIC (around 55€ at this time)
    uint256 public price = 144 ether;
    ...
    constructor(string memory baseURI) ERC721("VenusMe", "VenusMe") {
        setBaseURI(baseURI);
    ...
    // everyone able to read a smart contract can see that this is the only
    ...
    // that we cannot do anything else with the funds than donate 55% percent
    ...
    function withdraw(uint256 amount) public payable onlyOwner {
        ...
        require(payable(addressCharity).send(percent * 55)); // 55% for charity
        require(payable(addressJuanjo).send((percent * 45) / 2)); // 24% for juanjo
        require(payable(addressTokomo).send((percent * 45) / 2)); // 24% for tokomo
    }
