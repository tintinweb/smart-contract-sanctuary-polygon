...
contract UniverselleGenesis is ERC1155Upgradeable, IERC2981 {
    ...
    */
    function createToken(address _minter, uint256 _amount) virtual public onlyAdmin {
        require(_amount > 0, "Error: amount can't be 0");
        token_ID++;
        _mint(_minter, (token_ID - 1), _amount, "");
        emit URI(uri(token_ID - 1), token_ID - 1);
        emit NewMint(_minter, (token_ID - 1), _amount);
    }
    /**
    * @dev 2981 Royalties
    */
    function royaltyInfo(uint256 tokenId, uint256 _salePrice) virtual override external view returns (address _receiver, uint256 _royaltyAmount) {
        require(tokenId < token_ID, "Error: invalid token id");
