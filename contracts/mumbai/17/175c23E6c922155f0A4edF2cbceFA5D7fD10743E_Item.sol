...
contract Item is ERC1155, Ownable {
    ...
    // price in USD (item's ID => price, price format example: $4.99 = 499 * 10^16)
    uint256[] public pricesUSD;

    // minting can be paused for each type
    bool[] public paused;
    ...
    // sends the minted item's information to the back-end server
    event PlayerMinted(address player, uint256 id, uint256 amount);

    //-------------------------------------------------------------------------
    // CONSTRUCTOR ////////////////////////////////////////////////////////////
    //-------------------------------------------------------------------------

    constructor() ERC1155("https://empireofsight.com/api/item/{id}.json") {
        priceFeed = AggregatorV3Interface(
            0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada
        ); ///////////// must be changed depending on the blockchain
        pausedAll = false;
    ...
    function getItemsNativePrice(uint256 id) public view returns (uint256) {
    ...
    // input price must be: (normal price) * 100, example: $4.99 = 499
    ...
    function mint(uint256 id, uint256 amount) public payable {
        ...
        require(
            msg.value >= getItemsNativePrice(id),
            "the sent amount is not enough"
        );
        ...
        emit PlayerMinted(msg.sender, id, amount);
    }
    ...
    // input must be normal price * 100, example: $4.99 = 499
        ...
        onlyOwner
    {
        ...
        pricesUSD.push(priceUSD * 10**16); // it should be 16!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    }

    // mints items by IDs for specific users
    function mintAdmin(
        uint256 id,
        uint256 amount,
        address player
        ...
        emit PlayerMinted(player, id, amount);
    }
    ...
    function setItemsPrice(uint256 id, uint256 price) external onlyOwner {
        require(id < getLengthTypes(), "item doesn't exist");
