...
contract BreedPlanet is ApeironPlanetGenerator {
    ...
    uint256 currentPlanetId = 4586;
    uint256 normalBreedBaseInterval = 14 * 3600 * 24;
    uint256 additionBreedBaseInterval = 14 * 3600 * 24;
    ...
    function born(uint256 planetId) external {
        ...
        require(
            (planetData.createTime + (bornBaseInterval)) < block.timestamp,
            "Not born for 7 days"
        );
    ...
    // todo should be set admin
    function setNormalBreedBaseInterval(uint256 interval) external {
        normalBreedBaseInterval = interval;
    }

    function setAdditionBreedBaseInterval(uint256 interval) external {
        additionBreedBaseInterval = interval;
    }

    function setBornBaseInterval(uint256 interval) external {
        bornBaseInterval = interval;
    }

    function getNormalBreedBaseInterval() external view returns (uint256) {
        return normalBreedBaseInterval;
    }
    function getAdditionBreedBaseInterval() external view returns (uint256) {
        return additionBreedBaseInterval;
    }
    function getBornBaseInterval() external view returns (uint256) {
        return bornBaseInterval;
    }
