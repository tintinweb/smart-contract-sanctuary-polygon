...
contract ComptrollerInterfaceG1 {
    ...
    function setMintedCAIOf(address owner, uint amount) external returns (uint);
}

contract ComptrollerInterfaceG2 is ComptrollerInterfaceG1 {
    function liquidateCAICalculateSeizeTokens(
        address cTokenCollateral,
        uint repayAmount) external view returns (uint, uint);
}
...
/**
  * @title Chee's InterestRateModel Interface
  * @author Chee
  */
...
contract ComptrollerErrorReporter {
    ...
    enum FailureInfo {
        ...
        SET_CAI_MINT_RATE_CHECK,
        SET_CAICONTROLLER_OWNER_CHECK,
        SET_MINTED_CAI_REJECTION,
...
contract TokenErrorReporter {
    ...
    enum FailureInfo {
        ...
        REDUCE_RESERVES_CASH_NOT_ACAILABLE,
        ...
        REPAY_CAI_COMPTROLLER_REJECTION,
        REPAY_CAI_FRESHNESS_CHECK,
        CAI_MINT_EXCHANGE_CALCULATION_FAILED,
...
contract CAIControllerErrorReporter {
    enum Error {
        NO_ERROR,
        UNAUTHORIZED,
        REJECTION,
    ...
    enum FailureInfo {
        ...
        CAI_MINT_REJECTION,
        CAI_BURN_REJECTION,
        CAI_LIQUIDATE_ACCRUE_BORROW_INTEREST_FAILED,
        CAI_LIQUIDATE_ACCRUE_COLLATERAL_INTEREST_FAILED,
        CAI_LIQUIDATE_COLLATERAL_FRESHNESS_CHECK,
        CAI_LIQUIDATE_COMPTROLLER_REJECTION,
        CAI_LIQUIDATE_COMPTROLLER_CALCULATE_AMOUNT_SEIZE_FAILED,
        CAI_LIQUIDATE_CLOSE_AMOUNT_IS_UINT_MAX,
        CAI_LIQUIDATE_CLOSE_AMOUNT_IS_ZERO,
        CAI_LIQUIDATE_FRESHNESS_CHECK,
        CAI_LIQUIDATE_LIQUIDATOR_IS_BORROWER,
        CAI_LIQUIDATE_REPAY_BORROW_FRESH_FAILED,
        CAI_LIQUIDATE_SEIZE_BALANCE_INCREMENT_FAILED,
        CAI_LIQUIDATE_SEIZE_BALANCE_DECREMENT_FAILED,
        CAI_LIQUIDATE_SEIZE_COMPTROLLER_REJECTION,
        CAI_LIQUIDATE_SEIZE_LIQUIDATOR_IS_BORROWER,
        CAI_LIQUIDATE_SEIZE_TOO_MUCH,
...
// File: contracts/CBep20.sol

pragma solidity ^0.5.16;


/**
 * @title Chee's CBep20 Contract
 * @notice CTokens which wrap an EIP-20 underlying
 * @author Chee
 */
contract CBep20 is CToken, CBep20Interface {
    /**
     ...
     * @notice Sender supplies assets into the market and receiver receives cTokens in exchange
     * @dev Accrues interest whether or not the operation succeeds, unless reverted
     * @param receiver the account which is receiving the cTokens
     * @param mintAmount The amount of the underlying asset to supply
     * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
     */
...
// File: contracts/BEP20Interface.sol
...
// File: contracts/ICheeOraclePrice.sol
...
// File: contracts/CheePriceOracleNew.sol
...
contract CheePriceOracleNew is PriceOracle {
    ...
    function getUnderlyingPrice(CToken cToken) public view returns (uint) {
        ...
        if (compareStrings(symbol, "cMATIC")) {
