 ...
 * Has a function to verify user's ICO request.
...
contract ICOPayment is Ownable, AccessControl, Pausable {
    ...
    event ICOFeesUpdated(uint256 newICOFees);

    /**
     * @dev Set the address of Token token.
     * Setup role for deployer.
     * @param _fees - fees for Create ICO.
     * @param _balanceVaultAddress - Contract address of balance vault.
     */
    constructor(uint256 _fees, address _balanceVaultAddress) {
    ...
    /**
     * @dev verify ICO service purchasing made by user.
    ...
    function verifyICO(
        ...
        if (fees != 0) {
            balanceVault.payWithUpo(msg.sender, fees);
            totalFees += fees;
        }
        require(
            bytes(_chainId).length > 0,
            "[ICOPayment.verifyICO] Invalid chain id"
        );
        require(
            _initialTokenAddress != address(0) &&
                _totalBuyingLimit > 0 &&
                _rate > 0,
            "[ICOPayment.verifyICO] Invalid initial token arguments"
        );
        require(
            _startDate < _endDate &&
                (_startDate > block.timestamp && _endDate > block.timestamp),
            "[ICOPayment.verifyICO] Invalid date range"
        );
        require(
            _roundPaid.length == _roundPaid.length && _roundPaid.length > 0,
            "[ICOPayment.verifyICO] Invalid paid interval length"
        );
        uint256 totalPaidPortion;
        uint256 prevPaidInterval;
        for (uint256 i = 0; i < _roundPaid.length; i++) {
            require(
                _roundPaid[i].paidIntervals > _endDate &&
                    _roundPaid[i].paidIntervals > prevPaidInterval &&
                    _roundPaid[i].paidPortions > 0,
                "[ICOPayment.verifyICO] Invalid paid interval and paid portion members"
            );
            prevPaidInterval = _roundPaid[i].paidIntervals;
            totalPaidPortion += _roundPaid[i].paidPortions;

            emit RoundPaidVerifed(
                i,
                _roundPaid[i].paidIntervals,
                _roundPaid[i].paidPortions
            );
        }
        require(
            totalPaidPortion == 10000,
            "[ICOPayment.verifyICO] Invalid total paid portions"
        );
     ...
     */
    function setBalanceVaultAddress(
    ...
    /**
     * @dev Update ICOfees fees.
     * @param _fees - new fee for Create ICO.
     */
    function setICOFees(uint256 _fees) external onlyRole(WORKER) {
        fees = _fees;

        emit ICOFeesUpdated(_fees);
    }
    ...
    function adminFeesWithdraw() public onlyRole(WORKER) whenNotPaused {
        ...
        require(
            withdrawAmount > 0,
            "[ICOPayment.adminFeesWithdraw] Insufficient fees to withdraw"
        );
        totalFees = 0;
