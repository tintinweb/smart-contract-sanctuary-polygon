...
/**
 * @title BandContract
 * @author harsh singh <[email protected]>
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function claimAllDrop(uint tokenID)
        external
        nonReentrant
    ...
    function claimDrop(uint dropId, uint tokenID)
        external
        nonReentrant
        returns (bool)
    {
    ...
    {   
        ...
        uint price = fixedMintPrice * amount;
        ...
        uint feePercentage = Registry(aliveRegistry).feePercentage();
        uint fee = (price * feePercentage) / 100; // TODO upgradeable
...
contract AliveBondedUpgradeable is ERC721AUpgradeable, ERC2981Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable
                         {
    bytes32  constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");
    bytes32  constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
 ...
 address public aliveRegistry;
    ...
    event bondedCurveMintEvent(address minter, uint amount, uint atPrice);
    event newDropEvent(uint id, uint value, uint perNftDrop, uint DropAmount);

     modifier onlyGovernor() { 
        require(Registry(aliveRegistry).isManagerWallet(msg.sender) == true ||  hasRole(GOVERNANCE_ROLE, msg.sender) ||  hasRole(DEFAULT_ADMIN_ROLE, msg.sender ),  "only governor");
        _;
    } 
    ...
    function initialize(
        ...
        address artist, 
        address _aliveRegistry,
        uint[3] memory configureBondedCurve // total supply, price increase per sale, start price
    ...
    {   
        require(amount <= Registry(aliveRegistry).maxMintPerTxn()[1], "amount limit per txn" );
        require(totalSupply() + amount <= maxBondedCurve, "max supply reached");
        require(amount>0);
         uint price = currentPriceBondedCurve;
   unchecked{
        ...
        uint feePercentage = Registry(aliveRegistry).feePercentage();
        uint fee = price*feePercentage/100; // TODO upgradeable
        payable(Registry(aliveRegistry).aliveHouseRoyaltyReceiver()).transfer(fee); // Todo - Upgradeble to alive, use a beacon meachnaism
        payable(mintRoyaltyReceiver).transfer(price - fee);
        if(msg.value > price) {
              payable(msg.sender).transfer(msg.value-price);
        }
   ...
   function claimDrop(uint dropId, uint tokenID) external nonReentrant returns(bool) {
    ...
    function _baseURI() internal view override returns (string memory) {
     ...
     function setRoyalties(uint _tokenId, address payable _royaltiesReceipientAddress, uint96 _percentageBasisPoints) public  onlyGovernor {
    ...
    function batchAirDrop(address[] memory _arrayOfReceivers, uint _amountForEach) public  onlyGovernor {
               
       require(totalSupply() + _arrayOfReceivers.length <= maxBondedCurve, "can't mint more than supply");
        for (uint256 index = 0; index < _arrayOfReceivers.length;) {
 ...
 function setURI(string memory _uri) public onlyGovernor {
  ...
  function contractURI() public view onlyAdmin returns (string memory) {
    ...
    function pause() public  {
        require(hasRole(PAUSER_ROLE, msg.sender) || Registry(aliveRegistry).isManagerWallet(msg.sender) == true, "Pauseable: must have pauser role to pause");
        _pause();
    }

    function unpause() public  {
        require(hasRole(PAUSER_ROLE, msg.sender) || Registry(aliveRegistry).isManagerWallet(msg.sender) == true, "Pauseable: must have pauser role to unpause");
        _unpause();
    }
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(
        ...
        address artist,
        address _aliveRegistry,
        uint[5] memory _configureDutchAuction // total supply, price increase per sale, start price
    ) public initializer initializerERC721A {
    ...
    function dutchAuction(address to, uint256 amount)
        ...
        whenNotPaused
        nonReentrant
    {   require(amount <= Registry(aliveRegistry).maxMintPerTxn()[2], "amount limit per txn" );
