...
// File: Resources/Staking/Owned.sol
...
// File: Resources/Utils/ReentrancyGuard.sol
...
// File: Resources/Uniswap/Interfaces/IUniswapV2Pair.sol
...
// File: Resources/Uniswap/TransferHelper.sol
...
// File: Resources/Utils/Address.sol
...
// File: Resources/Common/Context.sol
...
// File: Resources/Curve/IveSTAX.sol
...
interface IveSTAX {
    ...
    function totalStaxSupply() external view returns (uint256);
    function totalStaxSupplyAt(uint256 _block) external view returns (uint256);
...
// File: Resources/ERC20/IERC20.sol
...
// File: Resources/ERC20/SafeERC20.sol
...
// File: Resources/ERC20/ERC20.sol
...
// File: Resources/Staking/STAX_StakingRewardsDualV5.sol
...
// ====================================================================
// Includes veSTAX boost logic
// Unlocked deposits are removed to free up space

// Stax Finance: https://github.com/StaxFinance
...
// Originally inspired by Synthetix.io, but heavily modified by the Stax team
...
//import "../../Stax/Tokens/StaxToken.sol";
...
contract StakingRewardsDualV5 is Owned, ReentrancyGuard {
    ...
    // veSTAX related
    uint256 public vefxs_per_stax_for_max_boost = uint256(4e18); // E18. 4e18 means 4 veSTAX must be held by the staker per 1 FRAX
    ...
    constructor (
        ...
        // Uniswap related. Need to know which token stax is (0 or 1)
        address token0 = stakingToken.token0();
        if (token0 == _stax_address) stax_is_token0 = true;
        else stax_is_token0 = false;
    ...
    // Takes into account the locked stake time multiplier and veSTAX multiplier
    ...
    function staxPerLPToken() public view returns (uint256) {
        // Get the amount of FRAX 'inside' of the lp tokens
        uint256 stax_per_lp_token;
        {
            uint256 total_stax_reserves;
            (uint256 reserve0, uint256 reserve1, ) = (stakingToken.getReserves());
            if (stax_is_token0) total_stax_reserves = reserve0;
            else total_stax_reserves = reserve1;

            stax_per_lp_token = total_stax_reserves.mul(1e18).div(stakingToken.totalSupply());
        }
        return stax_per_lp_token;
    }

    function userStakedStax(address account) public view returns (uint256) {
        return (staxPerLPToken()).mul(_locked_liquidity[account]).div(1e18);
    }

    function minVeFXSForMaxBoost(address account) public view returns (uint256) {
        return (userStakedStax(account)).mul(vefxs_per_stax_for_max_boost).div(MULTIPLIER_PRECISION);
    }

    function veSTAXMultiplier(address account) public view returns (uint256) {
        // The claimer gets a boost depending on amount of veSTAX they have relative to the amount of FRAX 'inside'
        // of their locked LP tokens
        uint256 veSTAX_needed_for_max_boost = minVeFXSForMaxBoost(account);
        if (veSTAX_needed_for_max_boost > 0){ 
            uint256 user_vefxs_fraction = (veSTAX.balanceOf(account)).mul(MULTIPLIER_PRECISION).div(veSTAX_needed_for_max_boost);
        ...
        else return 0; // This will happen with the first stake, when user_staked_stax is 0
    }
        ...
        )
    {
        ...
        // Get the veSTAX multipliers
        // For the calculations, use the midpoint (analogous to midpoint Riemann sum)
        new_vefxs_multiplier = veSTAXMultiplier(account);
        
        uint256 midpoint_vefxs_multiplier;
        if (_locked_liquidity[account] == 0 && _combined_weights[account] == 0) {
            // This is only called for the first stake to make sure the veSTAX multiplier is not cut in half
    ...
    function _updateRewardAndBalance(address account, bool sync_too) internal {
        ...
        if (account != address(0)) {
            ...
            // Update the user's stored veSTAX multipliers
    ...
    function setMultipliers(uint256 _lock_max_multiplier, uint256 _vefxs_max_multiplier, uint256 _vefxs_per_stax_for_max_boost) external onlyByOwnGov {
        require(_lock_max_multiplier >= MULTIPLIER_PRECISION, "Mult must be >= MULTIPLIER_PRECISION");
        require(_vefxs_max_multiplier >= 0, "veSTAX mul must be >= 0");
        require(_vefxs_per_stax_for_max_boost > 0, "veSTAX pct max must be >= 0");

        lock_max_multiplier = _lock_max_multiplier;
        vefxs_max_multiplier = _vefxs_max_multiplier;
        vefxs_per_stax_for_max_boost = _vefxs_per_stax_for_max_boost;

        emit MaxVeFXSMultiplier(vefxs_max_multiplier);
        emit LockedStakeMaxMultiplierUpdated(lock_max_multiplier);
        emit veSTAXPerStaxForMaxBoostUpdated(vefxs_per_stax_for_max_boost);
    }
    ...
    event veSTAXPerStaxForMaxBoostUpdated(uint256 scale_factor);
}