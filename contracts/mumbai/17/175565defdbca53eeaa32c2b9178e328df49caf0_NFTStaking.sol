...
contract InvincibleContract is ERC20, ERC20Burnable, Ownable {

  mapping(address => bool) controllers;
  
   constructor() ERC20("Invincible Stone", "IES") { }
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  InvincibleContract token;

  mapping(uint256 => Stake) public vault; 

   constructor(ERC721Enumerable _nft, InvincibleContract _token) { 
    nft = _nft;
    token = _token;
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      rewardmath = 100 ether * (block.timestamp - stakedAt) / 864 ;
