...
import './Interfaces/IContractBridgeReward.sol';
import "./Common.sol";
    ...
    Common
{
    ...
    **/
    function fractionalize(uint256 tokenId) external virtual {
        ...
        // reward calling address to set the quantity of bridge of the user
        address rewardAddress = IContractAddresses(orchestratorAddress).bridgeReward();
        if(rewardAddress != address(0)) {
            IContractBridgeReward(rewardAddress).addAmountForWeekPerUser(list[tokenId].quantity, _msgSender());
        }
...
interface IContractBridgeReward {

    function addAmountForWeekPerUser(uint256 _amount, address _address) external;
