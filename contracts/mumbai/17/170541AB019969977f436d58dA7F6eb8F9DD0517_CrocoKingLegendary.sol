...
contract CrocoKingLegendary is ERC721Enumerable, ReentrancyGuard, Ownable {
    using Strings for uint256;
    uint256 public constant MaxSupply = 500;
    uint256 public constant Price = 0.001 ether;
    uint256 public constant tokenPerTransaction = 3;
    uint256 public reservedTokensCounter;
    uint256 public constant reservedTokens = 20;
    ...
    mapping(uint256 => NFTInformation) nftInfos;

    mapping(address => bool) whitelist;

    constructor(string memory _baseURI, string memory _notRevealedUri)
        ERC721("Croco King Legendary", "CKL")
    {
    ...
    function Mint(uint256 _amount) public payable nonReentrant {
        require(contractStatus != ContractStatus.After, "Sold Out");
        require(contractStatus == ContractStatus.Sale, "Sale Not Started");
        require(verifyAddress(msg.sender), "You can not Buy token");
        uint256 _price = Price * _amount;
        require(msg.value >= _price, "You send Wrong Value");
        require(_amount > 0, "you need to mint at least one token");
        require(
            tokensPerWallet[msg.sender] + _amount <= tokenPerTransaction,
            "you can only mint a fixed amount of tokens"
        );
        uint256 _reserved = reservedTokens - reservedTokensCounter;
        require(
            totalSupply() + _amount + _reserved <= MaxSupply,
            "you mint more than supply"
        );
    ...
    function mintByOwner(uint256 _amount) public onlyOwner {
        ...
        require(
            ...
            "max reserved Tokens limit exceeded"
        );
        if (totalSupply() + _amount == MaxSupply) {
            contractStatus = ContractStatus.After;
        }
