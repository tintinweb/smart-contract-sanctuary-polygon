...
contract TheSanctuary is ERC1155, Ownable, DefaultOperatorFilterer {
    ...
    */
    function _cost(uint _batchID, bytes32[] calldata proof) public view returns(uint256){
        if (!checkIfAdmin()) {
    ...
    function checkOut(uint _amount, uint _batchID, bytes32[] calldata proof) private {
        if (!checkIfAdmin()) {
            ...
            require(msg.value >= (_amount * _cost(_batchID, proof)), "$?");
        }
    }
    ...
    */
    function setTier(bool _create, uint8 _tID, uint256 _tCost, bytes32 _tRoot) external onlyAdmins {
