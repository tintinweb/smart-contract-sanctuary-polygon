...
contract Profile {
    ...
    function donate(address donatorAddress) public payable {
    ...
    function withdraw(address sender) public onlyOwner {
        require(sender == i_owner, "Only onwer of contract allow withdraw");
...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  function executeRequest(
    ...
    // Send request passing subscriptionID 
  ...
  function addSimulatedRequestId(address oracleAddress, bytes32 requestId) public onlyOwner {
    ...
    event Donation(DonationResponse donationResponse);

    function createProfile() public {
        Profile newProfile = new Profile(msg.sender);
        emit ProfileCreated(
    ...
    function withdraw(address _profile) public {
        Profile profile = Profile(_profile);
        profile.withdraw(msg.sender);
    }

    function donate(address _profile,  string calldata source, bytes calldata secrets, string[] memory args, uint64 subscriptionId, uint32 gasLimit) public payable {
        require(msg.value > 0, "Donation amount must be greater than 0");

        uint256 fee = msg.value / 10; // 10% fee (1/10)
        uint256 donationAmount = msg.value - fee;

        totalFees += fee;

        Profile profile = Profile(_profile);
        profile.donate{value: donationAmount}(msg.sender);
        address profileOwner = profile.i_owner();
        DonationResponse memory donationResponse = DonationResponse(
            _profile,
            msg.sender,
            profileOwner,
            msg.value
        );
        emit Donation(donationResponse);
    ...
    function getDonations(
        address _profile
    ) public view returns (Profile.donationInfo[] memory) {
        Profile profile = Profile(_profile);
        return profile.getDonations();
    }

   function getBalance(address _profile) public view returns (uint256) {
        Profile profile = Profile(_profile);
