...
contract VCMarketplaceAuction is VCMarketplaceBase {
    ...
    function listAuction(
        ...
        if (auctionListings[address(_token)][_tokenId].seller != address(0)) {
            revert MktAlreadyListed();
        }

        bool minted = _token.exists(_tokenId);

        _checkSupply(minted, _token, _tokenId);
        ...
        _newList(minted, _token, _tokenId, _initialPrice, maturity, marketFee);

        emit ListedAuction(address(_token), _tokenId, msg.sender, _initialPrice, marketFee, maturity);
    }
    ...
    function _checkSupply(
        ...
        uint256 totalSupply = _minted ? _token.totalSupply(_tokenId) : _token.lazyTotalSupply(_tokenId);
    ...
    function _newList(
        ...
        auctionListings[address(_token)][_tokenId] = AuctionListing(
            minted,
            msg.sender,
            _initialPrice,
            _maturity,
    ...
    function unlistAuction(IArtistNft _token, uint256 _tokenId) public onlyWhitelistedTokens(address(_token)) {
        AuctionListing memory listing = auctionListings[address(_token)][_tokenId];
        ...
        delete auctionListings[address(_token)][_tokenId];

        if (listing.minted) {
            _token.safeTransferFrom(address(this), listing.seller, _tokenId, 1, "");
        }

        emit UnlistedAuction(msg.sender, address(_token), _tokenId);
    }
    ...
    function settle(IArtistNft _token, uint256 _tokenId) public whenNotPaused {
        ...
        if (listing.highestBid != 0) {
            ...
            resultingAmount -= _transferRoyalty(address(_token), _tokenId, listing.highestBid);
        ...
        emit Settled(listing.highestBidder, listing.seller, address(_token), _tokenId, listing.highestBid);
    }
