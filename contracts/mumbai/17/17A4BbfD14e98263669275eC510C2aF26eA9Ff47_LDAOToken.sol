...
contract LDAOToken is ERC20, Ownable {
    /**
     * @dev
     */
    ...
    /**
     * @dev List of addresses which contract does not charge fees in transaction
     */
     ...
     */
    constructor(uint256 initialSupply_, uint128 taxPerc_, uint128 decimals_, string memory name_, string memory symbol_) ERC20(name_, symbol_) {
    ...
    function _transfer(
        ...
        require(address(_feeDistributor) != address(0), "LDAO: fee manager not setted yet");
        if (from != address(0) && (!whitelist[from] && !whitelist[to])) {
            uint256 _fee = (_taxPerc * amount) / 100;
            amount -= _fee;

            FeeDistributor(_feeDistributor).checkpointToken(ERC20(address(this)));
            super._transfer(from, _feeDistributor, _fee);
            FeeDistributor(_feeDistributor).checkpointToken(ERC20(address(this)));
        }
