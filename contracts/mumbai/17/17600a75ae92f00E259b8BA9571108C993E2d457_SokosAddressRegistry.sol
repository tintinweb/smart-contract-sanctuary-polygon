    ...
    NativeMetaTransaction
{
    ...
    constructor(
        ...
        name = _name;
        symbol = _symbol;
        transferOwnership(sokosAddressRegistry.owner());
        isPublic = _isPublic;
    }
    ...
    function setRoyalties(
        ...
        require(
            owner() == _msgSender() ||
                _msgSender() == sokosAddressRegistry.marketplace(),
        ...
        returns (bool isOperator)
    {
        ...
        if (_operator == sokosAddressRegistry.marketplace()) {
            return true;
        }
...
contract SokosAddressRegistry is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable
{
    ...
    /// @notice SokosMarketplace contract
    address public marketplace;

    /// @notice SokosPriceFeed contract
    address public sokosPriceFeed;

    /// @notice Sokos Matic Price Feed contract
    address public sokosMaticPriceFeed;

    /// @notice Structure for Sokos collection
    struct Collection {
        string _name;
        string _symbol;
        address _address;
    ...
    /**
     @notice Authorization to upgrade Implementation
     @dev Only admin
     */
    ...
    function updatePriceFeed(address _priceFeed) external onlyOwner {
        sokosPriceFeed = _priceFeed;
    }

    /**
     @notice Update Matic price feed contract
     @dev Only admin
     */
    function updateMaticPriceFeed(address _priceFeed) external onlyOwner {
        sokosMaticPriceFeed = _priceFeed;
    }

    /**
     @notice Helper function for front-end to get collection size
     */
    ...
    function createCollection(
        ...
        require(
            _msgSender() == marketplace || _msgSender() == owner(),
            string(abi.encodePacked("AR:", Utils.toAsciiString(_msgSender())))
        );
        for (uint256 i; i < collections.length; i++) {
            Collection memory collection = collections[i];
            require(
                (keccak256(abi.encodePacked(collection._name)) !=
                    keccak256(abi.encodePacked(_name))),
                "AR: Duplicate"
            );
            require(
                (keccak256(abi.encodePacked(collection._symbol)) !=
                    keccak256(abi.encodePacked(_symbol))),
                "AR: Duplicate"
            );
        }
        RoyaltyToken newCollection = new RoyaltyToken(
            address(this),
            _name,
            _symbol,
            _isPublic
        );
        collections.push(Collection(_name, _symbol, address(newCollection)));
    ...
    {
        for (uint256 i; i < collections.length; i++) {
            Collection memory coll = collections[i];
            if (
    ...
    modifier isERC1155(address _factory) {
        require(
            IERC165Upgradeable(_factory).supportsInterface(
