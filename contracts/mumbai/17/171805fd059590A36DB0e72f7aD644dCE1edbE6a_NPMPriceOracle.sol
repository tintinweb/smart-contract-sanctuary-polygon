...
contract NPMPriceOracle is IPriceOracle {
  ...
  using SafeMath for uint256; // Renamed to fix the namespace conflict with Uniswap
  ...
  function consultLP(uint256 amountIn) external view override returns (uint256) {
    uint256 supply = pair.totalSupply();
    (uint256 r0, uint256 r1, ) = pair.getReserves();

    uint256 p0 = consult(token0, 1 ether);

    return _getFairLPPricing(r0, r1, p0, supply, amountIn);
  }
   ...
   *
   * @param r0 Provide the existing reserve0 value
   * @param r1 Provide the existing reserve1 value
   * @param p0 Provide the average price of NPM token
   * @param supply Provide the LP token supply
   * @param amountIn Enter LP token units to get the pricing.
   */
  function _getFairLPPricing(
    uint256 r0,
    uint256 r1,
    uint256 p0,
    uint256 supply,
    ...
    uint256 p = Babylonian.sqrt(p0.mul(1 ether)); // p1 = stablecoin = $1
...
interface IPriceOracle {
  ...
  function consultLP(uint256 amountIn) external view returns (uint256);
}
