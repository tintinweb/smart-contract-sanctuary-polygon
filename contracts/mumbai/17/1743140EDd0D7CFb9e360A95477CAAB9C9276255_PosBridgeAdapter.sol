...
contract PosBridgeAdapter is IBridgeAdapter, Ownable {
    ...
    function _sendAssets(
        ...
        bytes memory data = abi.encode(
            keccak256(abi.encode(nonce++, uint256(block.chainid))),
            value,
            dstAsset,
            to
        );
        ...
        emit AssetsSent(transferId, address(worker), asset, dstAsset, value, to, data);
    }

    function sendAssetsToRootVault(uint256 value, address asset)
        public
        returns (bytes32 transferId)
    {
        return _sendAssets(value, asset, address(0));
    }

    function sendAssetsToAddress(
        uint256 value,
        address asset,
        address receiver
    ) public returns (bytes32 transferId) {
        return _sendAssets(value, asset, receiver);
    }
        ...
        returns (uint256 pooled)
    {
        ...
        (, uint256 value, address asset, address receiver) = abi.decode(
            data,
        ...
        if (receiver == address(0)) {
            ...
            crossLedgerVault.depositFundsToRootVault(
                asset,
                pooled,
                transferId
            );
            ...
            crossLedgerVault.transferCompleted(transferId);
        }
...
{
	function depositFundsToRootVault(address token, uint value, bytes32 transferId) external returns(uint256);

    function transferCompleted(bytes32 transferId) external;
}
