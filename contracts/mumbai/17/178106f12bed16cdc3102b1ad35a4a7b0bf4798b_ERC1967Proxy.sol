...
import {SafeERC20Upgradeable} from '@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol';
import {EnumerableSetUpgradeable} from '@openzeppelin/contracts-upgradeable/utils/structs/EnumerableSetUpgradeable.sol';
import {AbstractCarbonStation} from '../../multichain/abstracts/AbstractCarbonStation.sol';

contract TestCarbonStation is AbstractCarbonStation {
    function initialize(
        address sender_,
        address payable treasury_
    ) public initializer {
    ...
    function test__send(uint256 destination_, bytes memory payload_) external payable {
        _send(destination_, payload_);
    }
}
...
import '../../multichain/CentralCarbonStation.sol';
import {CarbonToken} from '../../CarbonToken.sol';

/**
 * @author Flowcarbon LLC
 * @title Test Case Helper for protocol
 */
contract TestRaceConditionToken is CarbonToken {

    function test__authorizeUpgrade() public onlyRole(OPERATOR_ROLE) {
        _authorizeUpgrade(address(this));
    }

    /** @dev Allows us to provoke an error by approving more than 50 ether. */
    function approve(
        address spender,
        uint256 amount
        ...
        revert('oh no, an unexpected error happened!');
    }
}
...
/**
 * @author Flowcarbon LLC
 * @title A test helper to test intra-solidity interactions
 * @dev We go a long way to bring coverage up to 100% ...
 */
contract TestHelper {

    /**
     * @dev This should revert, as we do not allow an empty checksum.
     * @param token_ - Any token for the test.
     */
    function mintCarbonTokenWithEmptyChecksum(CarbonToken token_) external {
        token_.mint(address(this), 1 ether, '');
    }
     ...
     * @param token_ - Any token for the test.
     */
    function finalizeOffsetForCarbonTokenWithEmptyChecksum(CarbonToken token_) external {
        token_.finalizeOffset(1 ether, '');
    }

    /**
     * @dev This should revert, as we do not allow an empty checksum.
     * @param bundle_ - Any bundle token for the test.
     * @param token_ - Any token for the test.
     */
    function finalizeOffsetForCarbonBundleTokenWithEmptyChecksum(
        CarbonBundleToken bundle_, CarbonToken token_
    ) external {
        bundle_.finalizeOffset(token_, 1 ether, '');
    }
}
...
/**
 * @author Flowcarbon LLC
 * @title A Test Carbon Bundle Token Factory
 */
contract TestCarbonBundleTokenFactory is CarbonBundleTokenFactory {

    function test_createBundle(
        uint blueprintId_,
        string memory name_,
        string memory symbol_,
        uint16 vintage_,
        ...
        bytes memory initializer = abi.encodeWithSelector(
            CarbonBundleToken(address(0)).initialize.selector,
            // NOTE: passing feePoints=1337 provokes an error in CarbonBundleToken.initialize
            name_, symbol_, vintage_, tokens_, feePoints_, operator_, feePoints_ == 1337 ? address(0) : address(this),
            operator_, tokenFactory_
        );
        return _createBeaconProxy(blueprintId_, initializer);
    }
}
 ...
 * @title A Test Carbon Bundle Token Factory
 */
contract TestCarbonTokenFactory is CarbonTokenFactory {
    ...
    function test_createToken(
        ...
        bytes memory initializer = abi.encodeWithSelector(
            CarbonToken(address(0)).initialize.selector,
            name_, symbol_, details_, accessList_, operator_, operator_, bundleFactory_
        );
        return _createBeaconProxy(blueprintId_, initializer);
    }
}
...
contract TestCarbonAccessListFactory is AbstractFactory {
 ...
 * @title Test Case Helper for protocol
 */
contract TestAccessList is CarbonAccessList {
    function test__authorizeUpgrade() public onlyRole(OPERATOR_ROLE) {
        _authorizeUpgrade(address(this));
    }
}
 ...
 * @title Test Case Helper for protocol
 */
contract TestFeeMaster is CarbonFeeMaster {
    function test__authorizeUpgrade() public onlyRole(OPERATOR_ROLE) {
        _authorizeUpgrade(address(this));
    }
}
...
import {AbstractRouteAdapter} from '../../multichain/abstracts/AbstractRouteAdapter.sol';
import {IRoute} from '../../multichain/interfaces/IRoute.sol';
import {IRouteReceiver} from '../../multichain/interfaces/IRouteReceiver.sol';

/**
 * @author Flowcarbon LLC
 * @title Staging Route Adapter Contract
 */
contract StagingRouteAdapter is AbstractRouteAdapter, IRoute, AccessControl {

    /** @notice Emitted on every message. */
    event MessageSent(uint256 destination, bytes payload);

    /** @notice Adapter name and version. */
    bytes32 constant public IDENTIFIER = keccak256('STAGING_v1.0.0');

    /** @notice Mapping of destination chain ID to remote routes. */
    mapping(uint256 => address) public remoteRoutes;

    /** @notice Message storage. */
    mapping(uint256 => bytes[]) public messageQueue;

    /** @param receiver_ - Route receiver. */
    constructor(IRouteReceiver receiver_) {
        receiver = receiver_;
    ...
    function receiveMessage(uint256 source_, bytes memory payload_) external onlyReceiver {
        require(remoteRoutes[source_] != address(0),
          'StagingBridgeAdapter: unregistered remote');
        receiver.handleSendMessage(source_, payload_);
    }
    ...
    /** @dev See IRoute. */
    function sendMessage(uint256 destination_, bytes memory payload_) external onlyReceiver payable {
        require(remoteRoutes[destination_] != address(0),
          'StagingBridgeAdapter: unregistered remote');
        messageQueue[destination_].push(payload_);
        emit MessageSent(destination_, payload_);
    }

    /**
     * @notice The number of messages dispatched to destination.
     * @param destination_ - The remote chain.
     */
    function messageCount(uint256 destination_) external view returns (uint256) {
        require(remoteRoutes[destination_] != address(0),
          'StagingBridgeAdapter: unregistered remote');
        return messageQueue[destination_].length;
    }

    /**
     * @notice The message at a specific index.
     * @param destination_ - The remote chain.
     * @param index_ - The index of the message in the queue.
     */
    function messageAt(uint256 destination_, uint256 index_) external view returns (bytes memory) {
        require(remoteRoutes[destination_] != address(0),
          'StagingBridgeAdapter: unregistered remote');
        return messageQueue[destination_][index_];
    }
}
...
/**
 * @author Flowcarbon LLC
 * @title Layer Zero Route Adapter Contract
 */
contract LayerZeroRouteAdapter is
        IRoute,
        AbstractRouteAdapter,
        NonblockingLzApp {
    ...
    /** @notice Adapter name and version. */
    bytes32 constant public IDENTIFIER = keccak256('LAYER_ZERO_v1.0.0');

    // =================
    // Public Properties
    // =================

    /** @notice Refunds are sent to this address. */
    address payable public refundAddress;

    /** @notice Payments are forwarded to this address. */
    address public zeroPaymentAddress;
    ...
    constructor(
        ...
        require(address(receiver_) != address(0),
            'LayerZeroRouteAdapter: receiver is required');
        require(operator_ != address(0),
            'LayerZeroRouteAdapter: operator is required');
        require(treasury_ != address(0),
            'LayerZeroRouteAdapter: treasury is required');
        transferOwnership(operator_);
        receiver = receiver_;

        refundAddress = treasury_;
        zeroPaymentAddress = address(0x0);
        adapterParams = bytes('');
    }
    ...
    /** @dev See IRoute. */
    function registerRemoteRoute(uint256 destination_, address contract_) external onlyReceiver {
        trustedRemoteLookup[uint16(destination_)] = abi.encodePacked(contract_);
    }

    /**
     * @notice Configures the send params of the LayerZero endpoint.
     * @dev See ILayerZeroEndpoint for param details.
     */
    function setSendParams(
        address payable refundAddress_,
        address zeroPaymentAddress_,
        bytes memory adapterParams_
    ) external onlyOwner {
        require(refundAddress_ != address(0),
            'LayerZeroRouteAdapter: refund address is required');
        require(zeroPaymentAddress_ != address(0),
            'LayerZeroRouteAdapter: zero payment address is required');
        refundAddress = refundAddress_;
        zeroPaymentAddress = zeroPaymentAddress_;
    ...
    function sendMessage(uint256 destination_, bytes memory payload_) external onlyReceiver payable {
        lzEndpoint.send{value: msg.value}(
            uint16(destination_),
            trustedRemoteLookup[uint16(destination_)],
            payload_,
            refundAddress,
            zeroPaymentAddress,
            adapterParams
    ...
    function _nonblockingLzReceive(
        ...
        receiver.handleSendMessage(uint256(source_), payload_);
    }
}
...
import '../../multichain/adapters/HyperlaneRouteAdapter.sol';

/**
 * @author Flowcarbon LLC
 * @title A Carbon Credit Token Factory
 */
contract TestHyperlaneRouteAdapter is HyperlaneRouteAdapter {

    constructor(
        address abacusConnectionManager_,
        address abacusInterchainGasPaymentMaster_,
        IRouteReceiver receiver_,
        address operator_
    ) HyperlaneRouteAdapter(abacusConnectionManager_, abacusInterchainGasPaymentMaster_, receiver_, operator_) { }

    function _initDomains() internal virtual override {
        _setDomain(13371, 13371);        // Test 1
        _setDomain(13372, 13372);        // Test 2
        _setDomain(13373, 13373);        // Test 3
    }

    /** @dev incoming message forwarded to the receiver. */
    function test__handle(uint32 source_, bytes32 hash_, bytes memory message_) public {
        _handle(source_, hash_, message_);
    }
}
 ...
 * @title Staging Route Adapter Contract
 */
contract LayerZeroEndpointMock is LZEndpointMock {

    constructor(uint16 chainId_) LZEndpointMock (chainId_) {
        // do nothing
    }
}