...
contract BSBForestUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function setLevelImpacts(uint[] memory milestones_, ImpactType[] calldata impacts_) external onlyOwner {
        ...
        require(milestones_.length == impacts_.length, "BSB400");
    ...
    function stackInHouse(CollectionItems[] calldata collectionItems_, uint256 houseId_) external returns(uint) {
        ...
        for (uint i = 0; i < collectionItems_.length; ++i) {
            ...
            require(
                collectionItems_[i].assets.length + getTreeHouseOccupancy(houseId_) <= treeHouseStats.size.value,
                "BSB450"
            );
    ...
    function unStackFromHouse(CollectionItems[] calldata collectionItems_, uint256 houseId_) external returns(uint){
        _checkIfOwnsItem(contracts["treeHouse"], houseId_, _msgSender());
        bool applyBuff_ = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != 0;
        ...
        for (uint i = 0; i < collectionItems_.length; ++i) {
            ...
            for (uint j = 0; j < collectionItems_[i].assets.length; ++j) {
                if (collectionItems_[i].collection == contracts["treeHouse"]) {
                    require(getTreeHouseOccupancy(collectionItems_[i].assets[j]) == 0, "BSB450");
                } else {
                    require(
                        !tokenHouseAppartenance[collectionItems_[i].collection][collectionItems_[i].assets[j]],
                        "BSB420"
                    );
                }
    ...
    function claimRewards(CollectionItems[] calldata collectionItems_) external returns(uint) {
        uint accumulatedYield_;
    ...
    function _calculateTokenGeneratorYield(
        ...
        return accumulatedYield_ + yieldBonus_ * accumulatedYield_ / 100;
    }
    ...
    function _getTokenGeneratorBonus(
        ...
        return _getTreeHouseBonus(treeHouseId_);
    }

    function calculateTokenGeneratorReward(uint tokenId_) public view returns (uint) {
    ...
    function calculateTokenGeneratorRewards(uint[] calldata tokenIds_) external view returns (uint[] memory) {
        ...
        for (uint64 i = 0; i < tokenIds_.length; ++i) {
            yieldRewards[i] = calculateTokenGeneratorReward(tokenIds_[i]);
        }
        return yieldRewards;
    }
    ...
    function calculateTreeHouseReward(uint tokenId_) public view returns (uint) {
    ...
    function calculateTreeHouseRewards(uint[] calldata tokenIds_) external view returns (uint[] memory) {
        uint[] memory yieldRewards = new uint[](tokenIds_.length);
        for (uint64 i = 0; i < tokenIds_.length; ++i) {
            yieldRewards[i] = calculateTreeHouseReward(tokenIds_[i]);
        }
        return yieldRewards;
    }
    ...
    function _claimBearRewards(CollectionItems calldata collectionItems_, bool applyBuff_) internal returns(uint) {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            bearStats[i].stamina.value = _calculateBearStamina(
                collectionItems_.collection,
                collectionItems_.assets[i],
                applyBuff_,
                bearStats[i].stamina
    ...
    function calculateBearReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        ...
        return (
            ...
            _calculateBearStamina(collection_, tokenId_, applyBuff_, bearStats.stamina)
        );
    }
    ...
    function calculateBearRewards(address collection_, uint[] calldata tokenIds_) external view returns (uint[] memory, uint[] memory) {
        ...
        for (uint64 i = 0; i < tokenIds_.length; ++i) {
            (yieldRewards[i], staminaRewards[i]) = calculateBearReward(collection_, tokenIds_[i]);
        }
    ...
    function _calculateBearYield(
        ...
        uint defaultYieldedAmount_ = _calculateYieldForRate(collection_, tokenId_, yield_);
    ...
    function _calculateBearStamina(
        ...
        uint finalStamina_ = stamina_.value + accumulatedStamina_;
        return finalStamina_ > stamina_.max ? stamina_.max : finalStamina_;
    }
    ...
    function _calculateYieldForRate(
        ...
        return (block.timestamp - stateChangeDates[collection_][tokenId_]) * yield_
        / (yieldDaysRate * DAYS);
    }
}
...
contract BSBBaseUpgradeable is Initializable, OwnableUpgradeable {
    ...
    modifier onlyAuthorised {
        require(owner() == _msgSender() || authorizedContracts[_msgSender()], "BSB403");
        _;
    }
