...
// File contracts/GatheringToken.sol
...
contract GatheringToken is Initializable, ERC20Upgradeable {
  ...
  function initialize() initializer public {
    __ERC20_init("On the Block Gathering", "gOTB");
...
//  }
  ...
  struct DocToVoteOn {
    address submitter; // address of user who added the doc
    uint256 timestamp; // uid of the doc
    string docUid; // uid of the doc
    uint8 docType; // 1 == doc, 2 == sheet, 3 == slide
    uint256 totalVotesCount; // votes count for this doc
    uint256 totalApproveVotesWeight; // approve votes weight for this doc
...
//  Doc[] public docs;
  ...
  function docsToVoteOn() public view returns (DocToVoteOn[] memory) {
    ...
    DocToVoteOn[] memory docsToVoteOnArr = new DocToVoteOn[](numDocsToVoteOn);
    uint voteDocIdx = 0;
    for (uint i; i < numDocs; i++) {
      if (docs[i].approved == false) {
        DocToVoteOn memory tmpDoc = DocToVoteOn({
          submitter: docs[i].submitter,
          timestamp: docs[i].timestamp,
          docUid: docs[i].docUid,
          docType: docs[i].docType,
          totalVotesCount: docs[i].totalVotesCount,
          totalApproveVotesWeight: docs[i].totalApproveVotesWeight,
          totalRejectVotesWeight: docs[i].totalRejectVotesWeight,
          totalAbstainVotesWeight: docs[i].totalAbstainVotesWeight
        });
        docsToVoteOnArr[voteDocIdx] = tmpDoc;
        voteDocIdx += 1;
      }
    }
