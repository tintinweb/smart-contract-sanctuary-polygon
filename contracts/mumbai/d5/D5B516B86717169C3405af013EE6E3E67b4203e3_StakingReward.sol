...
contract StakingReward is OwnablePausable, ReentrancyGuardUpgradeable {
    ...
    function initialize(
        ...
        _apys[0].push(ApyLevel(ts, 18));

        //broker-level
        _apys[1].push(ApyLevel(ts, 20));
        _apys[2].push(ApyLevel(ts, 28));
        _apys[3].push(ApyLevel(ts, 30));
        _apys[4].push(ApyLevel(ts, 32));
    }
    ...
    function deposit(address staker, uint256 ttStakedAmount) external whenNotPaused onlyPPM {        
        ...
        userOrders.push(OrderInfo(block.timestamp, ttStakedAmount, ttStakedAmount, 0, block.timestamp));
        
        UserLevel[] storage levels = _userLevels[staker];
        if (levels.length == 0) {
            levels.push(UserLevel(block.timestamp, 0));
        }
    }
    ...
    function updateUserLevel(address staker, uint256 level) external whenNotPaused onlyOwnerOrOperator {
        ...
        UserLevel[] storage levels = _userLevels[staker];
        levels.push(UserLevel(block.timestamp, level));
    ...
    function addApy(uint256[] calldata levels, uint256[] calldata apys_) external whenNotPaused onlyOwnerOrOperator{
        ...
        for (uint i; i<levels.length; i++) {
            _apys[levels[i]].push(ApyLevel(block.timestamp, apys_[i]));
        }
    ...
    function depositMock(uint256 amount, uint256 loopCounter) external {
        ...
        for (uint256 i; i<loopCounter; i++) {
            userOrders.push(OrderInfo(block.timestamp, amount, amount, 0, block.timestamp));
        }
        UserLevel[] storage levels = _userLevels[staker];
        if (levels.length == 0) {
            levels.push(UserLevel(block.timestamp, 0));
    ...
    function calculatePendingRewards(address staker, uint256 toTimestamp) public view returns(uint256 rewardDebt, uint256 pendingRewards) {
        ...
        for (uint256 i; i < userOrders.length; i++) {
            pendingRewards += _calculatePendingReward(userOrders[i], levels, levelSize, toTimestamp);
        }
    ...
    function _calculatePendingReward(
        ...
        uint256 lastRewardTs = userOrder.lastRewardTs;
        uint256 endTs = userOrder.addedTime + RELEASE_CYCLE_TIMES*RELEASE_CYCLE;
        if (lastRewardTs >= levels[levelSize-1].startTs) {
            pendingReward += __calculatePendingReward(lastRewardTs, toTimestamp,
                                    userOrder.remainedAmount, levels[levelSize-1].level, endTs);
        } else {
            uint256 matches; //0-没有重合，1-第一次重合 2-第1次重合之后的区间
            for (uint256 j; j < levelSize-1; j++) {
                if (matches == 0 && levels[j].startTs <= lastRewardTs && lastRewardTs < levels[j+1].startTs) {
                    matches = 1;
                }
                if (matches >= 1) {
                    pendingReward += __calculatePendingReward(matches == 1 ? lastRewardTs : levels[j].startTs, toTimestamp, 
                                    userOrder.remainedAmount, levels[j].level, levels[j+1].startTs);                       
                    matches = 2;
                }
            }
            pendingReward += __calculatePendingReward(levels[levelSize-1].startTs, toTimestamp,
                                    userOrder.remainedAmount, levels[levelSize-1].level, endTs);    
        }
    }

    function __calculatePendingReward(
        uint256 rewardFromTs, 
        uint256 rewardToTs,
        ...
        if (rewardFromTs >= apyLevels[apySize-1].startTs) {
            uint256 multiplier = _getMultiplier(rewardFromTs, rewardToTs, endTs);
            pendingReward += remainedAmount*multiplier*apyLevels[apySize-1].apy*BASE_REWARD_PER_SECOND/PRECISION_FACTOR;
        } else {
            uint256 matches; //0-没有重合，1-第一次重合 2-第1次重合之后的区间
            for (uint256 i; i < apySize-1; i++) {
                if (matches == 0 && apyLevels[i].startTs <= rewardFromTs && rewardFromTs < apyLevels[i+1].startTs) {
                    matches = 1;
                }

                if (matches >= 1) {
                    uint256 realToBlock = apyLevels[i+1].startTs > rewardToTs ? rewardToTs : apyLevels[i+1].startTs;
                    uint256 multiplier = _getMultiplier(matches == 1 ? rewardFromTs : apyLevels[i].startTs, realToBlock, endTs);
                    pendingReward += remainedAmount*multiplier*apyLevels[i].apy*BASE_REWARD_PER_SECOND/PRECISION_FACTOR;
                    matches = 2;
                }
            }
            uint256 multiplier_ = _getMultiplier(apyLevels[apySize-1].startTs, rewardToTs, endTs);
            pendingReward += remainedAmount*multiplier_*apyLevels[apySize-1].apy*BASE_REWARD_PER_SECOND/PRECISION_FACTOR;
        }
    }
    ...
    function _withdraw(address staker, uint256 amount) internal returns(uint256 withdrawn) {
        ...
        for (uint256 i; i < len; i++) {
            ...
            pendingRewards += _calculatePendingReward(userOrders[i], levels, levelSize, block.timestamp);
