...
import "./MorganningPolygon.sol";

contract MorganningMumbai is MorganningPolygon {
    // TESTNET ONLY
    // ALLOWS YOU TO MINT ANOTHER TOKEN WITH THE SAME LAYER COMBO AS PREVIOUS
    function resetCombo(
        uint8 layer1,
        uint8 layer2,
        uint8 layer3,
        ...
        uint32 combo = uint32(layer1) |
            (uint32(layer2) << 4) |
            (uint32(layer3) << 8) |
            (uint32(layer4) << 12) |
            (uint32(layer5) << 16);
        layerComboUsed[combo] = false;
    }

    // anyone can set greenlist status
    function setGreenlistStatus(bool status) public override {
        greenListing = status;
    }
    ...
    function mint(
        ...
        _mint(reciever, block.chainid, layer1, layer2, layer3, layer4, layer5);
    }
}
...
import "../MorganningDeployable.sol";

// Used for chain specific logic
contract MorganningPolygon is MorganningDeployable {
...
import "../Morganning.sol";

contract MorganningDeployable is Morganning {
    constructor() Morganning(false) {}
}
...
import "./interfaces/IMorganning.types.sol";
import "./interfaces/ILayerZeroEndpoint.sol";
import "./interfaces/ILayerZeroReceiver.sol";
import "./utils/SettableCountersUpgradeable.sol";

contract Morganning is
    Initializable,
    ERC721Upgradeable,
    ...
    ILayerZeroReceiver
{
    ...
    SettableCountersUpgradeable.Counter private _tokenIdCounter;
    ...
    mapping(uint32 => bool) public layerComboUsed;
    ...
    // Mapping from owner address to owner's preferred token
    mapping(address => uint256) public preferredToken;

    // Greenlist merkle root
    bytes32 public merkleRoot;

    // True if greenlisting is active - only greenlisted users can mint
    bool public greenListing;

    uint16 public endIndex;

    struct LayerCounts {
        uint8 numImagesLayer1;
    ...
    // @param startIndex - index for the first token mintable for a specific chain
    // @param _endIndex - index for the last token mintable for a specific chain
    // @param root1 - greenlist merkle root
    // @param numl1 - layer 1 identifier
    ...
    function initialize(
        ...
        require(
            numl1 < 16 && numl2 < 16 && numl3 < 16 && numl4 < 16 && numl5 < 16,
            "Layers out of bounds!"
        );
        endpoint = ILayerZeroEndpoint(_endpoint);

        // start ids from for sensible preferredToken mapping deletion.
        _tokenIdCounter.set(startIndex);

        layerCounts = LayerCounts({
            numImagesLayer1: numl1,
    ...
    function setMerkleRoot(bytes32 root) public onlyRole(GREENLIST_ADMIN_ROLE) {
        merkleRoot = root;
    }

    // @notice set the greenlist status
    // @param status - true if greenlist active, false if public mint active
    function setGreenlistStatus(bool status)
        public
        virtual
     ...
     */
    function tokensAndPrefferredToken(address user)
        public
        view
        returns (
            bool hasToken,
    ...
    function mintGreenList(
        ...
        require(greenListing, "Not greenlisting - use mint instead!");
        require(
            MerkleProof.verify(
                proof,
                merkleRoot,
    ...
    function _mint(
        ...
        LayerCounts memory _layerCounts = layerCounts;
        require(
            layer1 < _layerCounts.numImagesLayer1 &&
                layer2 < _layerCounts.numImagesLayer2 &&
                layer3 < _layerCounts.numImagesLayer3 &&
                layer4 < _layerCounts.numImagesLayer4 &&
                layer5 < _layerCounts.numImagesLayer5,
            "Layers out of bounds!"
        );
        ...
        require(id <= endIndex, "Max supply reached for chain!");

        _tokenIdCounter.increment();
        if (balanceOf(reciever) == 0) preferredToken[reciever] = id;

        tokenLayers[id] = LayerData(
            originatingChainId,
            layer1,
            layer2,
            layer3,
    ...
    function validateUniqueness(
        ...
        uint32 combo = uint32(originatingChainId) |
            (uint32(layer1) << 4) |
            (uint32(layer2) << 8) |
            (uint32(layer3) << 12) |
            (uint32(layer4) << 16) |
    ...
    // @param reciever - user who will receive token on receiving chain
    // @param id - token id to mint it with, note this can fall outside of the startIndex & endIndex
    ...
    function _mintCrossChain(
        ...
        if (balanceOf(reciever) == 0) preferredToken[reciever] = tokenId;
    ...
    function setPreferredToken(uint256 tokenId) public {
        require(ownerOf(tokenId) == msg.sender, "Don't own that token!"); // todo: add approval
    ...
    // @param _destination - the address of the contract on the other chain, NB to set correctly!
    // @param tokenId - id of token to bridge
    function transferCrossChain(uint16 _dstChainId, uint256 tokenId)
        public
        payable
    {
        require(
        ...
        require(
            ...
            "LZ: destination chain is not a trusted source"
        );
        ...
        bytes memory payload = abi.encode(
            msg.sender,
            tokenId,
            tokenLayers[tokenId].originatingChainId,
            tokenLayers[tokenId].layer1,
            tokenLayers[tokenId].layer2,
            tokenLayers[tokenId].layer3,
            tokenLayers[tokenId].layer4,
            tokenLayers[tokenId].layer5
        );
    ...
    function lzReceive(
        ...
        require(
            ...
            "LZ: invalid source sending contract"
        );
        ...
        (
            ...
            uint8 layer5
                ...
                _payload,
                (address, uint256, uint256, uint8, uint8, uint8, uint8, uint8)
            );
    ...
    // @notice layer zero feature - gets a quote in source native gas, for the amount that send() requires to pay for message delivery
    ...
    // @notice before token transfer hook override to specify which inheritence to use
    // @param from - sender
    // @param to - receiver
