...
import {BaseConfig, AdvancedConfig, BurnToken, DropuStorage} from "../libraries/DropuStorage.sol";

contract DropuERC721AExtensionA is DropuERC721AUpgradeable {
    ...
    function updateBaseConfig(
        ...
        require(
            _baseConfig.dropuFeeActive == state.cfg.dropuFeeActive,
            "CANNOT_UPDATE_CONSTANT_VARIABLE"
        );
    ...
    function freezePayoutAddresses() external onlyOwner {
        DropuStorage.state().advCfg.payoutAddressesFrozen = true;
    }
    ...
    function updatePayoutAddressesAndBasisPoints(
        ...
        AdvancedConfig storage advCfg = DropuStorage.state().advCfg;
    ...
    function setRoyaltyBasisPoints(uint16 _royaltyBps) external onlyOwner {
        DropuStorage.state().cfg.royaltyBps = _royaltyBps;
    }
    ...
    function gift(
        ...
        if (state.cfg.dropuFeeActive) {
            uint256 dropuFee = (totalMints * dropuFeePerToken()) / 10;
            require(msg.value == dropuFee, "PAYMENT_INCORRECT");
