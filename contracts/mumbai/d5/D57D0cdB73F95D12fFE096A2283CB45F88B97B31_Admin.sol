...
contract Admin is Initializable, AccessControl, BasicMetaTransaction {
    //--------------------Other Contracts--------------------//

    address public owner;
    address public tokenFactory;
    address public tradingFactory;
    address public forwardFactory;
    address public whitelisting;
    address public safeProxyFactory;
    address public safeL2;
    address public masterForward;
    address public masterTrading;
    address public usdc;
    ...
    event IdentityRegistered(address indexed investorAddress,address identity);
    ...
    modifier onlyOperator() {
        require(hasRole(OPERATOR, _msgSender()), "Not Operator");
        _;
    }
    ...
    function initialize(
        ...
        safeL2 = _safeL2;
        masterForward = _masterForward;
        forwardFactory = _forwardFactory;
        masterTrading = _masterTrading;
        usdc = _USDC;
    }

    //---------------------------Admin-setters------------------------//

    function addOperator(address _address)
        external
        virtual
        onlyAdmin
        validation(_address)
    ...
    function setTradingFactory(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        tradingFactory = _address;
    }

    function setTokenFactory(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        tokenFactory = _address;
    }

    function setWhitelisting(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        whitelisting = _address;
    }

    function setSafeProxyFactory(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        safeProxyFactory = _address;
    }

    function setSafeL2(address _address)
        external
        validation(_address)
        onlyAdmin
    {
    ...
    function setMasterForward(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        masterForward = _address;
    }

    function setForwardFactory(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        forwardFactory = _address;
    }

    function setMasterTrading(address _address)
        external
        validation(_address)
        onlyAdmin
    {
        masterTrading = _address;
    }
    ...
    function assetCreation(
        ...
        ITokenFactory(tokenFactory).createToken(
            _name,
            _symbol,
            _decimals,
            _totalSupply,
    ...
    function addAgentOnIdentityRegistry(address _agent, address _iR) external {
        IIdentityRegistry iR = IIdentityRegistry(_iR);
    ...
    function registerIdentityContract(address _userAddress, address _identity,uint16 _country,address _iR) external {
        IIdentityRegistry iR = IIdentityRegistry(_iR);
        iR.registerIdentity(_userAddress, IIdentity(_identity), _country);
...
interface IAdmin is IAccessControl {
    ...
    function tradingFactory() external returns (address);

    function forwardFactory() external returns (address);

    function whitelisting() external returns (address);
...
interface ITokenFactory {
    ...
    function initialize(address _masterToken, address _owner,address _admin) external;
