...
contract VaultUpgradeable is IVault, MarketHubRegistrarUpgradeable, UUPSUpgradeable {
    ...
    function _authorizeUpgrade(address newImplementation) internal override {
        require(
            ...
            "Vault: newImplementation must be set as the target in the UpgradeGatekeeper contract"
        );
        marketHub.getUpgradeGatekeeper().resetUpgradeTarget();
    }

    modifier onlyEscrow() {
        // TODO: Remove AdminRole check. Needed for early tests
        require(
            (hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || msg.sender == address(marketHub.getEscrow())),
            "Only Escrow can call this function."
        );
        _;
    }

    function depositERC20(address _erc20Address, uint256 _erc20Amount, address _sender) external override onlyEscrow {
        require(_erc20Address != address(0), "ERC20 address cannot be 0");
        require(_erc20Amount > 0, "ERC20 amount must be greater than 0");

        IERC20 erc20 = IERC20(_erc20Address);
        require(erc20.allowance(_sender, address(this)) >= _erc20Amount, "ERC20 allowance too low");

        erc20Balances[_erc20Address] += _erc20Amount;
        emit DepositedERC20(_erc20Address, _erc20Amount);

        require(erc20.transferFrom(_sender, address(this), _erc20Amount), "ERC20 transfer failed");
    }

    function depositColleNFT(address _erc721Address, uint256 _tokenId, address _sender) external override onlyEscrow {
        require(_erc721Address != address(0), "ERC721 address cannot be 0");

        IColleCollection erc721 = IColleCollection(_erc721Address);
        require(erc721.ownerOf(_tokenId) == _sender, "ERC721 owner is not the sender");

        erc721Balances[_erc721Address][_tokenId] = true;
        emit DepositERC721(_erc721Address, _tokenId);

        // TODO: safeTransfer
        erc721.transfer(_sender, address(this), _tokenId);
    }
    ...
    function withdrawERC20(
        ...
        IERC20 erc20 = IERC20(_erc20Address);
        require(erc20Balances[_erc20Address] >= _erc20Amount, "ERC20 balance too low");

        erc20Balances[_erc20Address] -= _erc20Amount;
        emit WithdrawERC20(_erc20Address, _erc20Amount);

        require(erc20.transfer(_receiver, _erc20Amount), "ERC20 transfer failed");
    }
    ...
    function withdrawColleNFT(
        ...
        IColleCollection erc721 = IColleCollection(_erc721Address);
        require(erc721Balances[_erc721Address][_tokenId], "ERC721 is not deposited");

        erc721Balances[_erc721Address][_tokenId] = false;
        emit WithdrawERC721(_erc721Address, _tokenId);

        // TODO: safeTransfer
        erc721.transfer(address(this), _receiver, _tokenId);
    }
}
