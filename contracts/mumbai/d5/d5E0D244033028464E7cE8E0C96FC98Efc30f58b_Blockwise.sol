...
contract Blockwise {
    ...
    function executeGroupRequest(address creator, uint groupRequestId) public {
        ...
        uint40 requiredAcceptances = uint40(groupRequest.participants.length * 40 / 100);
        require(groupRequest.numberOfAcceptances >= requiredAcceptances, "Not enough acceptances");

        // Split the total amount amongst all participants and convert to wei
        uint individualAmount = (groupRequest.totalAmount * 1 ether) / (groupRequest.participants.length + 1);
