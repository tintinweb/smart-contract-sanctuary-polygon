...
contract FairymintFactory is Ownable {
    ...
    function createRaffle(string calldata raffleDisplayName)
        external
...
contract Fairymint is IRandomiserCallback, Ownable {
    ...
    event RandomSeedRequested(uint256 indexed raffleId, string ipfsCid);
    ...
    /// @param ipfsCid_ IPFS CID
    function raffle(string calldata ipfsCid_) external onlyOwner {
        require(
            bytes(ipfsCid).length == 0 && randomSeed == 0,
            "Randomness has already been set or has already been requested"
        );
        ipfsCid = ipfsCid_;
        randomiser.getRandomNumber(address(this));
        emit RandomSeedRequested(raffleId, ipfsCid_);
    }

    /// @notice Get the nth winner index using current seed.
    /// @param n nth draw
    function drawNthWinner(uint256 n) external view returns (uint256) {
    ...
    function receiveRandomness(uint256 randomness) external {
        ...
        emit RaffleDrawn(raffleId, ipfsCid, randomness);
    }
    ...
    function isRaffleInProgress() external view returns (bool) {
        return bytes(ipfsCid).length > 0 && randomSeed == 0;
    }

    /// @dev Returns `true` if the raffle has finished
    function isRaffleFinished() external view returns (bool) {
        return randomSeed != 0;
    }
}
