 ...
 */
 ...
 contract Fantazy is Ownable, ReentrancyGuard{
    ...
    uint256[] public test100;
     ...
     constructor(address _treasury, uint256 _commission, address _honeypot){
         treasury = _treasury;
    ...
    function Joinevent(
        ...
        eventuserscount++;
        Createteam(user, eventID, _captain, teamname, _vicecaptain, _playersselected, _teamid);
        _user[eventID] = Users({
            eventid : eventID,
            user : user,
            userscore : 0,
    ...
    function editSelectedPlayers(uint256 eventID, uint256 team, uint256[9] calldata playersselected) public{
        ...
        emit Editteam(msg.sender, eventID, team);
    }
    ...
    function editCaptainandVice(uint256 eventID, uint256 team, uint256 captain, uint256 vicecaptain) public{
        ...
        emit Editteamleaders(msg.sender, eventID, team, captain, vicecaptain);
    }
    ...
    function returnentryfee(uint256 poolid, uint256 team, uint256 eventID) public nonReentrant{
    ...
    function submitPlayersByRank(address[] memory users, uint256 poolid, uint256 stakeToPrizeRatio, uint256 _skew) public onlyOwner{
        ...
        /*for(uint256 i=0; i < users.length; i++){
            address see = users[i];
            pools[poolid].playersrank.push(see);
        }*/
    }

    function seeusers(address[] memory users) public {//remove this only for testing purposes
    testcase = users;
    }

    function viewtestcase() public view returns (address[] memory){
        return testcase;
    }
    ...
    function withdrawPrizes(uint256 eventID, uint256 poolid, uint256 index) public onlyOwner{
        require(!pools[poolid].prizesPaid, "The prizes have already been paid.");
        require(pools[poolid].playersrank.length > 0);
        uint256[] memory me = pools[poolid].teamsarray;
        uint256 lastindex = me.length.sub(index);
        uint256 exempt = pools[poolid].entryfee / 10;
        ...
        for(uint256 i= 0 + index; i < endindex; i++ ){
            uint256 balance = poolbalance[eventID][poolid];
            if(balance > pools[poolid].entryfee && pools[poolid].prizeDistribution[i] > totalexempt){
