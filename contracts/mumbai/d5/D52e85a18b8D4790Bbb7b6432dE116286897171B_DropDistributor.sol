...
//   }
    ...
    function claim(address users)
        external onlyOwner
    {   
       address _users = users;
        if(addressesClaimed[_users] == 1){
            revert("Tokens Already Claimed");
        }
        // require(addressesClaimed[msg.sender] ==0,'Drop Tokens Already Claimed');

        if(addressesClaimed[_users] == 0){
        require(IERC20(token).transfer(_users,setAmount),'Transfer Failed');
        addressesClaimed[_users] = 1;
        }
        emit Claimed(_users,setAmount);
    }
