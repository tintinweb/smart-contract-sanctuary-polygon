...
contract NounsToken is INounsToken, Ownable, ERC721 {
    ...
    /**
     * @notice Override isApprovedForAll to whitelist
     */
    ...
    function tokenURI2(uint256 tokenId) public view returns (string memory) {
        require(
            _exists(tokenId),
            "NounsToken: URI query for nonexistent token"
        );
...
interface INounsDescriptorMinimal {
    ...
    function tokenURI2(uint256 tokenId, INounsSeeder.Seed memory seed)
        external
        view
        returns (string memory);
