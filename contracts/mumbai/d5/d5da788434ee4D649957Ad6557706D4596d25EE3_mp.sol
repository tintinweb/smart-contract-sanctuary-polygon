...
contract mp is ERC721Holder, Initializable, OwnableUpgradeable, UUPSUpgradeable, ERC165{
    ...
    */
    ...
    external 
    {
        ...
        require(IERC20(_erc20TokenAddress).balanceOf(msg.sender) >= _biddingAmount,"You don't have enough balance to buy the NFT");
    ...
    external
    {
        ...
        if(msg.sender == highestBidder[_nftTokenid]){
            ...
            for(uint i = 0; i < count; i++){
                uint split = (remainingBalance / 10000) * ps.returnRoyaltyTokens(_nftTokenid,_nftAddress,i);
            ...
            IERC20(_erc20TokenAddress).transfer(holdLandByLister[_nftTokenid], remainingBalance);
    ...
    */
    ...
    returns(bool submitted)
    {
        ...
        IERC721(_nftAddress).safeTransferFrom(msg.sender, address(this), _nftTokenid);
        emit LandListed(msg.sender, _nftTokenid);
    ...
    external
    {
        ...
        if (_amountForNFT == nftValue[_nftTokenid]){
            ...
            IERC20(_erc20TokenAddress).transferFrom(
                ...
                remainingBalance);
            emit LandBought(msg.sender, _nftTokenid, _amountForNFT);
        }else{
...
contract mpRoyalty{
    ...
    address public splitterAd;

    function storeSplitterAddress(address _sm) external{
        splitterAd = _sm;
    }

    function splitAndTransfer1(uint nftId, address _nftAddress, address erc20) external{
        paymentSplitter ps = paymentSplitter(splitterAd);
        // uint[] memory tokens = ps.GETTOKEN(nftId);
        uint count = ps.addressCount(nftId, _nftAddress);
        for(uint i = 0; i < count; i++){
            IERC20(erc20).transferFrom(
                address(this), 
                ps.returnRoyaltyAddress(nftId,_nftAddress, i), 
                ps.returnRoyaltyTokens(nftId,_nftAddress, i));
        }
    }
}
...
contract paymentSplitter is Initializable, UUPSUpgradeable, OwnableUpgradeable{
