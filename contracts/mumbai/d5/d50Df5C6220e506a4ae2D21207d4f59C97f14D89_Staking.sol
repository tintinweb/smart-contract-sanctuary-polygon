...
contract Staking is Ownable {
    ...
    uint256 private rewardTokensPerBlock; // Number of reward tokens minted per block 10000..00
    ...
    function stake(uint256 _amount) external {
        require(_amount > 0, "VaultV2: Deposit amount can't be zero");
        if(tokensStaked == 0) {
            startBlockNumber = block.number;
            endBlockNumber = startBlockNumber + rewardPool / rewardTokensPerBlock;
        }
    ...
    function unstake() external {
        require(block.number - startBlockNumber >= depositLock, "VaultV2: You cannot withdraw tokens yet");
        PoolStaker storage staker = poolStakers[msg.sender];
        uint256 amount = staker.amount;
        require(amount > 0, "VaultV2: Withdraw amount can't be zero");
    ...
    /**
     * @dev Harvest useraccumulatedRewardsPerShare rewards
     */
    ...
    function harvestable(address _stakerAddress, uint256 _blockNumber) public view returns (uint256) {
        require(tokensStaked > 0, "VaultV2: Pool is not active");
        if(_blockNumber == 0)
            _blockNumber = block.number;
        _blockNumber = _blockNumber > endBlockNumber ? endBlockNumber : _blockNumber;
        uint256 blocksSinceLastReward = _blockNumber - lastRewardedBlock;
        uint256 rewards = blocksSinceLastReward * rewardTokensPerBlock;
        uint256 accumulatedRewardsPerShareView = accumulatedRewardsPerShare + (rewards * REWARDS_PRECISION / tokensStaked);

        PoolStaker storage staker = poolStakers[_stakerAddress];
        uint256 rewardsToHarvest = (staker.amount * accumulatedRewardsPerShareView / REWARDS_PRECISION) - staker.rewardDebt;
    ...
    function updatePoolRewards() private {
        uint256 _blockNumber = block.number > endBlockNumber ? endBlockNumber : block.number; 
        if (tokensStaked == 0) {
            lastRewardedBlock = _blockNumber;
            return;
        }
        uint256 blocksSinceLastReward = _blockNumber - lastRewardedBlock; 
        uint256 rewards = blocksSinceLastReward * rewardTokensPerBlock;
    ...
    function blocksLeft() public view returns (uint256) {
        return endBlockNumber > block.number ? endBlockNumber - block.number : 0;
    }
}