...
import "../interfaces/IPublicGovSale.sol";

contract CrossChainManager is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, NonblockingLzApp, PausableUpgradeable, AdminProxyManager {
    struct Invoice{
        uint32 buyersIndex;
        uint32 boosterId;
        uint64 boughtAt;
    ...
    function init(
        address _endpoint // layerZero endpoint
    ...
    function getUserAllocationLeft(address _sale, uint32 _booster, address _buyer) internal view returns(bool isOutOfSale){
        uint128 userAllocation = IPublicGovSale(_sale).getUserAllocation(_buyer, _booster);
        uint128 userPurchase = IPublicGovSale(_sale).purchasePerRound(_buyer, _booster);

        uint128 leftAmount = userAllocation - userPurchase;

        uint128 leftSale = IPublicGovSale(_sale).sale() - IPublicGovSale(_sale).sold();

        isOutOfSale = leftAmount > 0 && leftSale > 0 ? false : true;
    }
	...
	function _nonblockingLzReceive(
        ...
        {
            (address targetSale, uint256 srcInvoiceIndex, uint128 amountIn, address buyer) = abi.decode(_payload, (address, uint256, uint128, address));

            (bool success, ) = targetSale.call(
                abi.encodeWithSelector(IPublicGovSale._buyToken.selector, amountIn, buyer)
            );
            ...
            if(success){
                (
                    ...
                    , received,
                    bought,
                ) = IPublicGovSale(targetSale).invoices(buyer, IPublicGovSale(targetSale).getBuyerHistoryLength(buyer) - 1);
            }

            bool isOutOfSale = getUserAllocationLeft(targetSale, boosterId, buyer);

            payload = abi.encode(buyer, boosterId, srcInvoiceIndex, received, bought, isOutOfSale);
        }
        ...
        uint messageFee;
        {
            // (messageFee, ) = lzEndpoint.estimateFees(_srcChainId, address(this), payload, false, adapterParams);
            (messageFee, ) = lzEndpoint.estimateFees(_srcChainId, address(this), payload, false, bytes(""));
            require(address(this).balance >= messageFee, "eth");
        }

        lzEndpoint.send{value: messageFee}( // {value: messageFee} will be paid out of this contract!
            _srcChainId, // destination chainId
            abi.encodePacked(sendBackToAddress), // destination address of PingPong contract
    ...
    function wdEth(
        ...
        (bool success,) = payable(_target).call{value: _amount}("");
        require(success, "bad");
    }
...
interface IPublicGovSale {    
	function invoices(address, uint256) external view returns(uint32, uint32, uint64, uint128, uint128, uint128);
	function sale() external view returns(uint128);
	function sold() external view returns(uint128);
	function getBuyerHistoryLength(address) external view returns(uint256);
	function getUserAllocation(address, uint32) external view returns(uint128);
	function purchasePerRound(address, uint32) external view returns(uint128);
    function boosterProgress() external view returns (uint32);
	
	function init(uint128, uint128, uint128, uint128, uint128, uint128[4] calldata, address, address) external;
	function _buyToken(uint128, address) external returns(uint128);
}
