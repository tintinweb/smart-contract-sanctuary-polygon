...
//   uint256 public gaiaUSDC = 79357452196816930849001 * (10**18) / uint256(1868548345305467327315244) * 1588085682360 * (10**12) / uint256(1586020149070416559561266);
  uint256 public gaiaUSDC = 40000 * (10**18);
  ...
  function mintGaiaLandToken(
    ...
    uint _price = (gaiaGoodLandPrice * numberOfGoodLands + gaiaRegularLandPrice * numberOfRegularLands) * (10**36) / gaiaUSDC;
    require(
  ...
  function buy(uint256 _id) external payable {
    ...
    transferGAIAOrGAIA(payable(_previousOwner), _sellerValue);
  ...
  function transferGAIAOrGAIA(address payable _to, uint256 value) private {
    if (!attemptGAIATransfer(_to, value)) {
      PGAIA(PGAIAAddress).deposit{value: value}();
      require(PGAIA(PGAIAAddress).transfer(_to, value));
    }
  }

  function attemptGAIATransfer(address payable _to, uint256 value)
    private
    returns (bool)
  {
