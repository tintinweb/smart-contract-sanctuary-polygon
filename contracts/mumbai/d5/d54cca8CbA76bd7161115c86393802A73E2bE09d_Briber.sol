...
contract Briber is AutomateReady, Pausable {
    ...
    function execBribe(
        ...
        if (amount > 0) {
            ...
            emit ExecutedBribe(key, proposal, fee, plan);
        }
        else {
            if (plan.canSkip)
    ...
    function _getBribeAmount(Plan storage plan) internal view returns (uint256) {
        // fixed plans can only have insufficient tokens if:
        // 1. an unsafe plan was added and/or
        // 2. an unsafe withdrawal was requested
        if (plan.style == PlanStyle.FIXED)
            return plan.amount <= plan.token.balanceOf(address(this))
                ? plan.amount
                : 0;

        uint256 available = _getAvailable(plan.token);
        return plan.style == PlanStyle.PERCENT
            ? available * 1000 / plan.amount
            : available;
    }
