...
// Written by Doge0420 and inspired by HashlipsLowerGasFee contract - MIT licence
...
contract BlotMap is Ownable, ERC721{
    ...
    constructor() ERC721("BlotMapCabinet", "BMC") {
        ...
        addWhitelistedAddress(0x1f7D475723543Ba5715D7AD33F5FD2304e006847);
        addWhitelistedAddress(0x125f2788fa587DF002150f0E773Bb108eA276059);
    }

    // MODIFIER requires the contract to be not paused
    modifier isPaused() {
        require(!pauseStatus, "Contract has been paused");
        _;
    }

    // MODIFIER limits, filters mint ammount and checks if max supply has been hit
    ...
    // MODIFIER requires the msg.sender to be in the whitelist
    modifier whitelisted(address _sender) {
        if(whitelistStatus == true) {require(whitelist[_sender], "You are not whitelisted.");
        _;
        }
        else {
        _;
    ...
    // ONLY OWNER add addresses to the whitelist
    ...
    // ONLY OWNER remove addresses from the whitelist
    ...
    // toggles whitelistmode on or off
    function toggleWhitelistMode() public onlyOwner {
        whitelistStatus = !whitelistStatus;
    }

    // ONLY OWNER pause the contract
    function pause() public onlyOwner {
        pauseStatus = !pauseStatus;
    }

    // ONLY OWNER set the real uri of all nfts
    ...
    // ONLY OWNER set the uri in hidden state of the nfts
    ...
    // ONLY OWNER reveal or hide the nft collection
    ...
    // ONLY OWNER mint nfts to an address
    function mintToAddress(address _to, uint256 _ammount) public mintable(_ammount) onlyOwner {
        _mintAll(_to, _ammount);
    }

    // internal function to handle the ammount variable in the mint function and to actually mint the nfts
    ...
    // ONLY OWNER allows the owner to split the contract's balance and withdraw it
