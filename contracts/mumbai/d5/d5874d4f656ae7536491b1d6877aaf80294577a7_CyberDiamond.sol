...
contract CyberDiamond is ERC721Enumerable, Ownable {
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIds;

    uint256 public MAX_SUPPLY = 10020;

    // Public sale
    uint256 public PRICE =100 ether;
    uint256 public specialPRICE =300 ether;

    // Pre-sale
    bool public presale = true;
    uint256 public PRE_PRICE =80 ether ;
    uint256 public specialPRE_PRICE =280 ether ;
    ...
    uint256[20] public specialNFTs = [10001, 10002, 10003, 10004, 10005, 10006, 10007, 10008, 10009, 10010,10011, 10012, 10013, 10014, 10015, 10016, 10017, 10018, 10019, 10020];
    constructor(string memory baseUR) ERC721("Cyber Diamond", "CD") {
        setBaseURI(baseUR);
    }
    ...
    function mintNFTSpecial(uint256 _count) public payable {
        uint256 totalSpecialMinted = specialNFTs.length;
        require(totalSpecialMinted.add(_count) <= 20, "Not enough NFTs left!");

        require(_count > 0, "Cannot mint 0 NFTs.");
        require(msg.value >= calculatePriceSpecial(_count),"Not enough ether to purchase NFTs.");        
        if (presale) {
            require(isWhitelisted[msg.sender], "Address is not whitelisted!");
            preNFTsMinted += _count;
            require(preNFTsMinted <= 3500, "Pre-sale is over.");             
        }

        for (uint256 i = 0; i < _count; i++) {
            _safeMint(msg.sender, specialNFTs[i]);
        }
    }
    ...
    function calculatePriceSpecial(uint256 _count) public view returns(uint256) {
        if (presale) {
            return _count * specialPRE_PRICE;
        }        
        return _count * specialPRICE;
    }
