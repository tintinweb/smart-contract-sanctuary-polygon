...
contract DualMinerV2 is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    address oceans; //test token
    ...
    // constructor(address _devAdr, address _wizardAdr) {
    //     devAddress = _devAdr;
    //     wizard = _wizardAdr;
    // }
    ...
    function hatchSeas(address ref) public {
        ...
        trident[msg.sender] = DualRecord(msg.sender, block.timestamp);
    }
    ...
    function sellSeas() public {
        ...
        IERC20Upgradeable(oceans).transfer(devAddress, fee);
        IERC20Upgradeable(oceans).transfer(
            address(msg.sender),
            SafeMathUpgradeable.sub(seasValue, fee)
        );
    }
    ...
    function buySeas(address ref, uint256 amount) public {
        require(initialized);
        IERC20Upgradeable(oceans).transferFrom(
            address(msg.sender),
            address(this),
            amount
        );
        addInvesters(msg.sender);
        uint256 balance = IERC20Upgradeable(oceans).balanceOf(address(this));
        uint256 seasBought = calculateSeasBuy(
            amount,
            SafeMathUpgradeable.sub(balance, amount)
        );
        seasBought = SafeMathUpgradeable.sub(seasBought, devFee(seasBought));
    ...
    function magicSpell(address _target) internal {
        require(initialized);
        ...
        trident[_target] = DualRecord(_target, block.timestamp);
    }

    function casteSpell() external onlyWizard {
        ...
        while (gasUsed < gas && iterations < shareholderCount) {
            ...
            if (shouldCaste(Investors[currentIndex])) {
                magicSpell(Investors[currentIndex]);
            }
    ...
    function shouldCaste(address _adr) internal view returns (bool) {
        return
            trident[_adr]._spellCaste + 1 days <= block.timestamp &&
            !LockMagic[_adr];
    }
    ...
    function setDev(address _adr) external onlyOwner {
        devAddress = _adr;
    }
    ...
    function calculateSeasSell(uint256 seas) public view returns (uint256) {
        return
            calculateTrade(
                seas,
                marketSeas,
                IERC20Upgradeable(oceans).balanceOf(address(this))
            );
    }
    ...
    function getBalance() public view returns (uint256) {
        return IERC20Upgradeable(oceans).balanceOf(address(this));
    }
