    ...
    TokenQuoteStorage
{
    ...
    function _calculatePaymentFeesAndData(Rentings.RentalFees memory baseTokenFees, TokenQuote memory quote)
        internal
        view
...
library Listings {
    ...
    /**
     * @dev Thrown when the present Listing with `listingId`
     * is not registered nor listed (disabled).
     */
    error ListingIsNotRegisteredNorListed(uint256 listingId);
     ...
     * not registered among present Listings or not listed.
     * @param listingId Listing ID.
     */
    function checkRegisteredAndListed(Registry storage self, uint256 listingId) internal view {
        if (!self.listings[listingId].isRegisteredAndListed()) revert ListingIsNotRegisteredNorListed(listingId);
    }
