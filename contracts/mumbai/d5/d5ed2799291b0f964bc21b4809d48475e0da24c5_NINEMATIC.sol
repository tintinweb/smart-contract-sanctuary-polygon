...
contract NINEMATIC is ERC20 {
    ...
    string public name                                      = "NINEMATIC";
    string public symbol                                    = "M9";
    ...
    uint256 constant internal tokenPriceInitial_            = 1 finney;
    uint256 constant internal tokenPriceIncremental_        = 0.01 finney;
    uint256 constant internal tokenPriceDecremental_        = 0.0095 finney;
    ...
    */
    constructor(address payable admin,address payable developeradd,address payable marketingadd) public {
    ...
    function sell(uint256 _amountOfTokens) onlybelievers () public {
        ...
        uint256 _taxedmatic                   = _matic;
        
        tokenSupply_                        = SafeMath.sub(tokenSupply_, _tokens);
        tokenBalanceLedger_[_customerAddress] = SafeMath.sub(tokenBalanceLedger_[_customerAddress], _tokens);
        
        payouts_[_customerAddress]        += _taxedmatic;       
    ...
    function calculateTokensReceived(uint256 _maticToSpend) public view returns(uint256) {
    ...
    function stakeprofit(address _customerAddress) public view returns(uint256){
        ...
        uint256 finish = deposits[_customerAddress][i].start.add(deposits[_customerAddress][i].time).mul(1 days);
				...
				uint256 share_2 = stakeBalanceLedger_[_customerAddress].mul(50).div(PLANPER_DIVIDER);
				uint256 from_2 = boostertime_2[_customerAddress] > stakingTime_[_customerAddress] ? boostertime_2[_customerAddress] : stakingTime_[_customerAddress];
				...
				uint256 from_4 = boostertime_4[_customerAddress] > stakingTime_[_customerAddress] ? boostertime_4[_customerAddress] : stakingTime_[_customerAddress];
				...
				uint256 from_6 = boostertime_6[_customerAddress] > stakingTime_[_customerAddress] ? boostertime_6[_customerAddress] : stakingTime_[_customerAddress];
                ...
                if(level <= 1){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        ...
                        if (stakingTime_[downline] < finish) {
                            uint256 share = stakeBalanceLedger_[downline].mul(deposits[downline][i].percent).div(PLANPER_DIVIDER);
    ...
    function purchaseTokens(address _referredBy) public payable returns(uint256) {
        ...
        uint256 _amountOfTokens             = maticToTokens_(incMATIC);
        ...
        if(dayscount <= 10)
        {
            ...
            if(boosterCount_[_referredBy] >= 7 ){
                if(boostertime_7[_referredBy] == 0){
					boostertime_7[_referredBy] = block.timestamp;
    ...
    function maticToTokens_(uint256 _matic) internal view returns(uint256) {
