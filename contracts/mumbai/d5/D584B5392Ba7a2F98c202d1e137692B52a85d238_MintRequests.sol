 ...
 */
contract MintRequests is Ownable, ReentrancyGuard, Pausable {
    ...
    modifier onlyFactoryOrAdmin() {
        bool isFactory = IWhitelist(whitelistAddress).hasRoleFactory(msg.sender);
        bool isAdmin = IWhitelist(whitelistAddress).hasRoleAdmin(msg.sender);
    ...
    modifier onlyAdmin() {
        require(IWhitelist(whitelistAddress).hasRoleAdmin(msg.sender), "not an admin");
        _;
    }
    ...
    */
    function submitRequest(
        ...
        uint256 _supply
    ) external onlyLazyOrFullOrAdmin nonReentrant whenNotPaused {
    ...
    */
    ...
    @param requestId the Id of the request
    */
    function setAsMinted(uint256 requestId) external onlyFactoryOrAdmin nonReentrant {
        Requests storage request = requests[requestId];
        require(request.isApproved, "Request already approved");
        require(!request.isMinted, "Request already minted");
