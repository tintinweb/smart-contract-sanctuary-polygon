...
contract Alligators is
    ...
    uint32 private constant NUM_WORDS = 44;
    ...
    */
    ...
    function joker_level(
        uint256 _tokenId
    ) external view override returns (uint8 lvl) {

        if (_tokenId >= _currentIndex) revert callErr();

        if (tokenIdType[_tokenId] == 3) {
            return NFTLvl[_tokenId];
        }
    }
    ...
    */
    ...
    function setAlligatorMerge(address mergeHub_) external onlyOwner {
        if (mergeHub_ == address(0)) revert callErr();
        AlligatorMerge = mergeHub_;
        emit mergeIsSet(mergeHub_);
    }

    function setJokerMerge(address mergeHub_) external onlyOwner {
        if (mergeHub_ == address(0)) revert callErr();
    ...
    function setServerUrl(string memory _base) external onlyOwner {
    ...
    */
    ...
    // Whitelist addresses verification
    function verify(address whitelisted) internal view {
