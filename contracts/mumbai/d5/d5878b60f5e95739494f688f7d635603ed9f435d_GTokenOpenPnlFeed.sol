...
contract GTokenOpenPnlFeed is ChainlinkClient, IOpenTradesPnlFeed {
    ...
    event NextEpochValueRequested(
        uint256 indexed currEpoch, uint256 indexed requestId, bytes32 job, uint256 oraclesCount, uint256 linkFeePerNode
    );
    ...
    event RequestMedianValueSet(
        uint256 indexed currEpoch, uint256 indexed requestId, int256[] requestValues, int256 medianValue
    );
    ...
    constructor(
        ...
        require(
            _LINK_FEE_BALANCE_DIVIDER > 0 && _linkToken != address(0) && address(_gToken) != address(0)
                && _oracles.length > 0 && _job != bytes32(0) && _minAnswers >= MIN_ANSWERS && _minAnswers % 2 == 1
    ...
    function forceNewEpoch() external {
        require(
            block.timestamp - gToken.currentEpochStart() >= requestsStart + requestsEvery * requestsCount, "TOO_EARLY"
        );
    ...
    function makeOpenPnlRequest() private {
        ...
        uint256 linkFeePerNode =
            IERC20(chainlinkTokenAddress()).balanceOf(address(this)) / LINK_FEE_BALANCE_DIVIDER / oracles.length;
    ...
    function startNewEpoch() private returns (uint256 newEpoch) {
        ...
        int256 newEpochOpenPnl =
            nextEpochValues.length >= requestsCount ? average(nextEpochValues) : int256(currentEpochPositiveOpenPnl);
