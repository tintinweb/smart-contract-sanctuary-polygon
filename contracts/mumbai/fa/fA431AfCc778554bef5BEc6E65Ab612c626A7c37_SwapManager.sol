...
contract SwapManager is ISwapManager,IUniswapV3SwapCallback,OddzOwnable,BlockContext{
    ...
    mapping(address => uint256) public firstSwapTimestamp;
    ...
    function initialize(address _balanceManager,address _orderManager,address _oddzMarket) external initializer {
        require(
    ...
    function swap(SwapParams memory _params) 
        ...
        onlyClearingHouse 
        override 
        returns(SwapResponseParams memory){

        IBalanceManager.PositionInfo memory accountInfo =IBalanceManager(balanceManager).traderPositionInfo(_params.positionID);
            
        
        IOddzMarket.MarketData memory marketInfo = 
            IOddzMarket(oddzMarket).fetchMarketData(_params.baseToken);
        ...
        (uint baseAmount,uint quoteAmount)= UniswapInteractor.swap(UniswapInteractor.SwapParams(
                marketInfo.pool,
                oddzClearingHouse,
                _params.isShort,
        ...
        if (_params.isShort) {
            ...
            swappedPostionSize = baseAmount.neg256();
            
            swappedPositionNotional = quoteAmount.toInt256();
        } else {
            // long: swappedPostionSize >= 0 && swappedPostionSize <= 0
            swappedPostionSize = baseAmount.toInt256();
           
            swappedPositionNotional = quoteAmount.neg256();
        }

         (uint256 sqrtPriceX96, , , , , , ) =
            UniswapInteractor.fetchSlot0(IOddzMarket(oddzMarket).fetchPool(_params.baseToken));

        bool isReducingPosition;
        if(accountInfo.takerBasePositionSize == 0){
            isReducingPosition=false;
        }else{
            isReducingPosition =accountInfo.takerBasePositionSize < 0 != _params.isShort;
        } 
        ...
        if (isReducingPosition) {
            ...
            pnlToBeRealized = _calculatePnl(
                accountInfo.takerBasePositionSize,
                accountInfo.takerQuoteSize,
        ...
        return
            SwapResponseParams({
                baseAmount:  swappedPostionSize.abs(),
                quoteAmount: swappedPositionNotional.abs(),
                swappedPositionSize:swappedPostionSize,
                swappedPositionNotional: swappedPositionNotional,
    ...
    function _calculatePnl(int256 _basePosition,int256 _quoteSize,int256 _quoteAmount,int256 _baseAmount) internal pure returns (int256 _pnlToBeRealized) {
        uint256  _FULLY_CLOSED_RATIO = 1e18;
        // closedRatio is based on the position size
        uint256 closedRatio = FullMath.mulDiv(_baseAmount.abs(), _FULLY_CLOSED_RATIO, _basePosition.abs());
        ...
        if (closedRatio <= _FULLY_CLOSED_RATIO) {
            int256 reducedOpenNotional = _quoteSize.mulDiv(closedRatio.toInt256(), _FULLY_CLOSED_RATIO);
            _pnlToBeRealized = _quoteAmount.add(reducedOpenNotional);
        } else {
            int256 closedPositionNotional = _quoteAmount.mulDiv(int256(_FULLY_CLOSED_RATIO), closedRatio);
            _pnlToBeRealized = _quoteSize.add(closedPositionNotional);
        }
