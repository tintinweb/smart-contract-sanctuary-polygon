...
{   
    ...
    ) external returns (address wallet) 
    {
        ...
        bytecode = abi.encodePacked(bytecode, abi.encode(msg.sender));
        bytes32 salt = keccak256(abi.encodePacked(msg.sender,_tokenA,_tokenB));
        assembly {
            wallet := create2(0, add(bytecode, 32), mload(bytecode), salt)  //endowment, memory_start, memory_length, salt EIP-1014
        }
    }   
