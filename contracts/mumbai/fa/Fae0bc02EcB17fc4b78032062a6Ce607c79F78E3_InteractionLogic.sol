...
import {IBondHub} from '../interfaces/IBondHub.sol';
import {ApplicationFollowModuleProxy} from '../upgradeability/ApplicationFollowModuleProxy.sol';
...
library InteractionLogic {
    ...
    function _followSingle(
        ...
        if (0 == tokenId) {
            ...
            if (followModule != address(0)) {
                IFollowModule(followModule).processFollow(follower, profileId, followModuleData);
            }
        }
        return tokenId;
    }
    ...
    function _getFollowNFT(
        ...
        address nftAddress;
        for (uint256 i = 0; i < _profileById[profileId].followType.length; i++) {
    ...
    function deployApplicationFollowModule(
       ...
       //return ownerAddress;
       return _deployFollowNFT(601, uint256(DataTypes.FollowType.Follow));
        // bytes memory functionData = abi.encodeWithSelector(
        //     ApplicationFollowModule.initialize.selector,
        //     ownerAddress,
        //     id
        // );
        // address followNFT = address(new ApplicationFollowModuleProxy(functionData));
...
contract ApplicationFollowModuleProxy is Proxy {
    using Address for address;
    address immutable HUB;

    constructor(bytes memory data) {
        HUB = msg.sender;
        IBondHub(msg.sender).getApplicationFollowModuleImpl().functionDelegateCall(data);
    }

    function _implementation() internal view override returns (address) {
        return IBondHub(HUB).getApplicationFollowModuleImpl();
    }
}
