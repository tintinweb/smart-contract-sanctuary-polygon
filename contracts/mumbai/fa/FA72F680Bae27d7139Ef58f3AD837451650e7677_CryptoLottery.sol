...
contract CryptoLottery is ERC20 {
    ...
    mapping(address => uint) private _free_tickets;


    Round[] public _rounds;
    

    constructor(uint256 round_interval, uint256 ticket_price, uint256 fee) ERC20("Crypto Lottery", "CL") {
    ...
    function buyTicket(uint256[6] memory _numbers) external payable {
        ...
        require(_rounds[_rounds.length - 1].status == RoundStatus.Start, "Error: the last round ended");
    ...
    function _random(uint key) internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
    ...
    function lastCombination() internal {
        ...
        if(_rounds[_rounds.length - 1].win.length == 0) {
          ...
          for(uint i = 0; i < _cache.length; i++) {
              for(uint z = 0; z < _cache.length; z++) {
    ...
    function _get_last_combination () internal view returns(uint256[] memory numbers) {
         return _rounds[_rounds.length - 1].win;
    }

    function _get_combination_by_id (uint id) external view returns(uint256[] memory numbers) {
    ...
    function findWinners () internal {
        uint[] memory _numbers =  _get_last_combination();

        for(uint i = 0;  i < _tickets[_rounds.length - 1].length; i++) {
  
           for(uint z = 0;  z < _tickets[_rounds.length - 1][i].numbers.length; z++) {

             for(uint y = 0; y < _numbers.length; y++) {
                 
                 if( _tickets[_rounds.length - 1][i].numbers[z] == _numbers[y]) {
     ...
     */
     for(uint i = 0;  i < _tickets[_rounds.length - 1].length; i++) {
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 1 && _tickets[_rounds.length - 1][i].win_last_digit == false) {
             _tickets[_rounds.length - 1][i].free_ticket = true;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 1 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 2 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 2 && _tickets[_rounds.length - 1][i].win_last_digit == false) {
             _tickets[_rounds.length - 1][i].eth_reward = _ticket_price * 2;
             _tickets[_rounds.length - 1][i].token_reward = 2000 * 10 ** 18;
             _token_reward += _tickets[_rounds.length - 1][i].token_reward;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 3 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 3 && _tickets[_rounds.length - 1][i].win_last_digit == false) {
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 4 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
             _tickets[_rounds.length - 1][i].eth_reward = _ticket_price * 100;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 4 && _tickets[_rounds.length - 1][i].win_last_digit == false) {
             _tickets[_rounds.length - 1][i].eth_reward = _ticket_price * 200;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 5 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
             _tickets[_rounds.length - 1][i].eth_reward = (2 * address(this).balance) / 100;
        
             _fee_value += _fee * ((2 * address(this).balance) / 100) / 100;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 5 && _tickets[_rounds.length - 1][i].win_last_digit == false) {
             _tickets[_rounds.length - 1][i].eth_reward = (10 * address(this).balance) / 100;

             _fee_value += _fee * ((10 * address(this).balance) / 100) / 100;
         ...
         if(_tickets[_rounds.length - 1][i].win_count == 6 && _tickets[_rounds.length - 1][i].win_last_digit == true) {
             _tickets[_rounds.length - 1][i].eth_reward = (30 * address(this).balance) / 100;

             _fee_value += _fee * ((30 * address(this).balance) / 100) / 100;
    ...
    function getLastTicketsCount () external view returns (uint){
    ...
    function getUserTickets () external view returns(Ticket[] memory tickets) {
          
          Ticket[] memory ticket_array;

          for(uint i = 0; i < _tickets_ref[msg.sender].length; i++) {
              ticket_array[i] = _tickets[ _tickets_ref[msg.sender][i].round][_tickets_ref[msg.sender][i].number];
          }

          return ticket_array;
    }

    function getLastTickets (uint count) external view returns(Ticket[] memory tickets) {
        Ticket[] memory ticket_array;

        for(uint i = _tickets[_rounds.length - 1].length - count; i < _tickets[_rounds.length - 1].length; i++) {
    ...
    function getLastRounds (uint count) external view returns(Round[] memory rounds)  {

        Round[] memory _rounds_array;

        for(uint i = _rounds.length - count; i < _rounds.length; i++) {
    ...
    function claimTicketReward (uint round, uint number) external {
        require(msg.sender == _tickets[round][number].owner, "You are not an owner");
        ...
        if(_tickets[round][number].eth_reward > 0) {
            payable(_tickets[round][number].owner).transfer(_tickets[round][number].eth_reward);
        }

        if(_tickets[round][number].token_reward > 0) {
           _mint(_tickets[round][number].owner, _tickets[round][number].token_reward);
        }
