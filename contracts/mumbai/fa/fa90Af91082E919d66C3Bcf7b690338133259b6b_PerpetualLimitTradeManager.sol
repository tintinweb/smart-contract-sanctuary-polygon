...
interface ILimitOrderBookFactoryReduced {
    function orderBooks(uint24 _iPerpetualId) external view returns (address);
}
    ...
    IPerpetualLimitTradeManager
{
    ...
    function tradeViaOrderBook(Order calldata _order)
        external
        override
        whenNotPaused
        ...
        returns (bool)
    {
        ...
        require(
            ILimitOrderBookFactoryReduced(orderBookFactory).orderBooks(_order.iPerpetualId) ==
                msg.sender,
            "only order-bk"
        );
        require(block.timestamp >= _order.submittedTimestamp + iTradeDelaySec, "delay required");
        ...
        /* Order book contract must test the following conditions
            ...
            require(!isOrderExecuted(digest), "order already executed");
            require(!isOrderCancelled(digest), "order was canceled");
        ...
        */
        ...
        if (_order.flags.isStopOrder()) {
            ...
            _getTradeLogic().validateStopPrice(
                _order.fAmount > 0,
                _getPerpetualMarkPrice(_getPerpetual(_order.iPerpetualId)),
        ...
        if (!_trade(_order, digest)) {
            // market order failed due to limit price, we rebate the referrer
            if (_order.referrerAddr != address(0x0)) {
                ...
                _transferFromVaultToUser(
                    liqPool,
                    _order.referrerAddr,
                    perpetual.fReferralRebateCC
                );
                emit TransferFeeToReferrer(
                    perpetual.id,
                    _order.traderAddr,
                    _order.referrerAddr,
                    perpetual.fReferralRebateCC
    ...
    function chargePostingFee(Order calldata _order, uint16 _feeTbps) external override {
        // ensure trade was sent via order-book
        require(
            ILimitOrderBookFactoryReduced(orderBookFactory).orderBooks(_order.iPerpetualId) ==
                msg.sender,
        ...
        // convert trade amount to collateral currency
        int128 fAmount = _order.fAmount.abs().mul(
            _getBaseToCollateralConversionMultiplier(perpetual, false)
        );
        // charge the refundable relative fee and non-refundable fixed fee (== referral rebate)
        int128 feeCC = fAmount.mul(ConverterDec18.tbpsToABDK(_feeTbps));
        // pay to DF
        pool.fDefaultFundCashCC = pool.fDefaultFundCashCC.add(feeCC);
        _transferFromUserToVault(pool, _order.traderAddr, feeCC);
        emit UpdateDefaultFundCash(pool.id, feeCC, pool.fDefaultFundCashCC);
    }

    function rebatePostingFee(Order calldata _order, uint16 _feeTbps) external override {
        // ensure trade was sent via order-book
        require(
            ILimitOrderBookFactoryReduced(orderBookFactory).orderBooks(_order.iPerpetualId) ==
                msg.sender,
        ...
        int128 feeCC = fAmount.mul(ConverterDec18.tbpsToABDK(_feeTbps));
        pool.fDefaultFundCashCC = pool.fDefaultFundCashCC.sub(feeCC);
        _transferFromVaultToUser(pool, _order.traderAddr, feeCC);
        emit UpdateDefaultFundCash(pool.id, feeCC, pool.fDefaultFundCashCC);
    }
    ...
    function executeCancelOrder(uint24 _perpetualId, bytes32 _digest) external {
        ...
        require(
            ILimitOrderBookFactoryReduced(orderBookFactory).orderBooks(_perpetualId) == msg.sender,
            "only order-bk"
        );
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[2] = this.chargePostingFee.selector;
        functionList[3] = this.rebatePostingFee.selector;
        functionList[4] = this.isOrderExecuted.selector;
        functionList[5] = this.isOrderCanceled.selector;
...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions, PerpetualHashFunctions {
    ...
    function _doOpeningTradeActions(
        ...
        marginAccounts[_order.iPerpetualId][_order.traderAddr].iLastOpenTimestamp = uint64(
            block.timestamp
        );
        if (_fTraderPos == 0) {
            // trader opens a new position, generate position id
            marginAccounts[_order.iPerpetualId][_order.traderAddr].positionId = bytes16(
    ...
    function _executeCancelOrder(uint24 _perpetualId, bytes32 _digest) internal {
        require(
            executedOrCancelledOrders[_digest] & MASK_ORDER_CANCELLED == 0,
            "order was canceled"
        );
