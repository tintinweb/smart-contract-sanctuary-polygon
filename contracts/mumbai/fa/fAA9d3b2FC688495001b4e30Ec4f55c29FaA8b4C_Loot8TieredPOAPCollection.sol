...
contract Loot8TieredPOAPCollection is ILoot8TieredPOAPCollection, ERC721Enumerable, Loot8POAPCollection {

    // Tiers Based on Token IDs from this collection
    // Starts from 1
    // tier = 0 is used for a token that does not belong to any tier
    // Eg: 10 tiers, tiers = 10(1 to 10)
    uint256 public tiers;

    // Mapping Tier => Tier Name
    // Tier 0 = NON-TIERED
    mapping(uint256 => string) public tierName;

    // Mapping Tier => Tier TokenURI
    mapping(uint256 => string) public tierTokenURI;

    // Mapping Tier => List of coupons the tier is eligible for
    // For Tier 0 tierCoupons=[] always
    mapping(uint256 => address[]) public tierCoupons;

    // Mapping patron => Tier => boolean
    // Indicates if patron holds a token from the tier
    mapping(address => mapping(uint256 => bool)) public isPatronFromTier;

    // Mapping tokenId => Coupon collection address => boolean
    // Mapping indicating if a coupon was airdropped for a given tokenId
    mapping(uint256 => mapping(address => bool)) public couponAirdroppedForTokenId;
    ...
    constructor(
        ...
        tierName[0] = "NON-TIERED";
    }

    // Identifies a range clash while setting range for a tier
    // function _rangeClash(uint256 _start, uint256 _end) internal view returns(bool) {
    //     for(uint256 i = 0; i <= tiers; i++) {
    //         if(
    //             (
    //                 (tierRange[i][0] > 0 && tierRange[i][1] == 0) &&
    //                 (
    //                     (_start >= 0 && _end == 0) ||
    //                     (_start >= tierRange[i][0] || _end >= tierRange[i][0])
    //                 )
    //             ) ||
    //             (
    //                 (tierRange[i][0] > 0) &&
    //                 (
    //                     (_start >= tierRange[i][0] && _start <= tierRange[i][1]) ||
    //                     (_end >= tierRange[i][0] && _end <= tierRange[i][1])
    //                 )
    //             )
    //         ) {
    //             return true;
    ...
    function isCouponFromTier(uint256 _tier, address _coupon) public view returns(bool) {
        require(_tier <= tiers , "INVALID TIER");
        for(uint256 i = 0; i < tierCoupons[_tier].length; i++) {
            if(tierCoupons[_tier][i] == _coupon) {
                return true;
            }
        }
    ...
    function addCouponForTier(uint256 _tier, address _coupon) external {
        require(_msgSender() == governor || _msgSender() == owner(), "UNAUTHORIZED");
        require(_tier <= tiers , "INVALID TIER");
        require(!isCouponFromTier(_tier, _coupon), "COUPON ALREADY ADDED TO TIER");
        tierCoupons[_tier].push(_coupon);
        emit CouponAddedToTier(_tier, _coupon);
    }

    function removeCouponForTier(uint256 _tier, address _coupon) external {
        require(_msgSender() == governor || _msgSender() == owner(), "UNAUTHORIZED");
        require(_tier <= tiers , "INVALID TIER");
        require(isCouponFromTier(_tier, _coupon), "COUPON NOT FROM TIER");
        for(uint256 i = 0; i < tierCoupons[_tier].length; i++) {
            if(tierCoupons[_tier][i] == _coupon) {
                tierCoupons[_tier][i] = tierCoupons[_tier][tierCoupons[_tier].length - 1];
            }

            tierCoupons[_tier].pop();
        }
        
        emit CouponRemovedFromTier(_tier, _coupon);
    }

    function setTier(string memory _name, string memory _tokenURI) external {
        require(_msgSender() == governor || _msgSender() == owner(), "UNAUTHORIZED");
        require(StringUtils.strlen(_tokenURI) > 0, "INVALID TOKEN URI");
    
        tiers++;

        tierName[tiers] = _name;
        tierTokenURI[tiers] = _tokenURI;

        emit SetTier(_name, tiers, _tokenURI);
    }

    function isValidCoupon(address _coupon) public view returns(bool) {
        for(uint256 i = 0; i <= tiers; i++) {
            if(isCouponFromTier(i, _coupon)) {
                return true;
            }
        }
    ...
    function setCouponAirdroppedForToken(uint256 _tokenId) external {
        _requireMinted(_tokenId);
        require(isValidCoupon(_msgSender()), "UNAUTHORIZED");
        couponAirdroppedForTokenId[_tokenId][_msgSender()] = true;
    }

    function checkCouponAirdroppedForToken(uint256 _tokenId, address _coupon) public view returns(bool) {
        _requireMinted(_tokenId);
        require(isValidCoupon(_coupon), "INVALID COUPON");
        return couponAirdroppedForTokenId[_tokenId][_coupon];
    }

    function _refreshPatronTierData(address _patron) internal {
            
        uint256 j;
        bool[] memory fromTier = new bool[](tiers + 1);
        uint256 patronBalance = balanceOf(_patron);
        
        while(j < patronBalance) {
            uint256 tokenId = ERC721Enumerable.tokenOfOwnerByIndex(_patron, j);
            uint256 uriLength = bytes(tokenURI(tokenId)).length;
            bytes32 tokenURIHash = keccak256(abi.encodePacked(tokenURI(tokenId)));

            for(uint256 i = 1; i <= tiers; i++) {
                if (
                    uriLength == bytes(tierTokenURI[i]).length &&
                    tokenURIHash == keccak256(abi.encodePacked(tierTokenURI[i]))
                ) {
                    isPatronFromTier[_patron][i] = true;
                    fromTier[i] = true;
                }
            }
            j++;
        ...
        bool tiered;
        for(uint256 k = 1; k < fromTier.length; k++) {
            if(!fromTier[k]) {
                isPatronFromTier[_patron][k] = false;
            } else {
                tiered = true;
            }
        }

        // Patron is from tier 0 if not from any other
        if(balanceOf(_patron) > 0 && !tiered) {
            isPatronFromTier[_patron][0] = true;
        } else if(balanceOf(_patron) > 0 && tiered){
            isPatronFromTier[_patron][0] = false;
        }
    ...
    function _afterTokenTransfer(
        ...
        address patron = (from == address(0)) ? to : from;
        _refreshPatronTierData(patron);

        if(from != address(0)) {
            // Re-calculate tier for recipient in case of transfer
            _refreshPatronTierData(to);
        }
    }

    function getTiersForPatron(address _patron) public view returns(uint256[] memory _tiers) {
        uint256 count;
        for(uint256 i = 0; i <= tiers; i++) {
            if(isPatronFromTier[_patron][i]) {
                count++;
            }
        }

        _tiers = new uint256[](count);
        uint256 j;
        for(uint256 i = 0; i <= tiers; i++) {
            if(isPatronFromTier[_patron][i]) {
                _tiers[j] = i;
    ...
    function getEligibleCouponsForPatron(address _patron) public view returns(address[] memory _coupons) {
        uint256[] memory _tiers = getTiersForPatron(_patron);

        uint256 count;
        for(uint256 i = 0; i < _tiers.length; i++) {
            count = count + tierCoupons[_tiers[i]].length;
        }

        _coupons = new address[](count);
        uint256 k;
        for(uint256 i = 0; i < _tiers.length; i++) {
            for(uint256 j = 0; j < tierCoupons[_tiers[i]].length; j++) {
                _coupons[k] = tierCoupons[_tiers[i]][j];
                k++;
            }
        }
    ...
    function _burn(uint256 tokenId) internal virtual override(ERC721, Loot8POAPCollection) {
        Loot8POAPCollection._burn(tokenId);
    }

   function _msgData() internal view virtual override(Context, Loot8POAPCollection) returns (bytes calldata) {
        return Loot8POAPCollection._msgData();
    }

    function _msgSender() internal view virtual override(Context, Loot8POAPCollection) returns (address sender){
        return Loot8POAPCollection._msgSender();
    }
    ...
    function _transfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, Loot8POAPCollection) {
        Loot8POAPCollection._transfer(from, to, tokenId);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Enumerable, Loot8POAPCollection) returns(bool) {
        return Loot8POAPCollection.supportsInterface(interfaceId);
    }
    
    function tokenURI(uint256 _tokenId) public view virtual override(ERC721, Loot8POAPCollection) returns(string memory) {
        return Loot8POAPCollection.tokenURI(_tokenId);
    }
}
