...
contract lbtestmwv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
    ...
    function autoSpin(address _caller, uint8[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,1) == false){
            ...
            uint256 amountInUSD = getOutputAmount(wmaticTrading,usdTrading,botReward,0);        
            ...
            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 0){ 
                ...
                if(block.timestamp > lastAutoSpin + 605100 && reset == false){   // if no node activates in 5 minutes period queue resets
                    ...
                    reset = true;
                } else if(lbNodes.updateNodeInfo(_caller,amountInUSD,1,3) == true){
     ...
     */
     function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
       ...
       raffle = uint16(_randomWords[0] % uint256(registeredsAccts.length));
        ...
        wmatic.safeTransfer(userWallet,prizeValueJackpot);
