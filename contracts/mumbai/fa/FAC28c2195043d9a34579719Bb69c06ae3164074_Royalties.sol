    ...
    IRoyalties
{
    ...
    bytes32 internal constant RECLAIM_TYPEHASH = keccak256(
        "Reclaim(uint128[] tierIds,address recipient,uint256 nonce,uint256 deadline)"
    );
    ...
    function settleUserExpiredRoyalties(
        uint128 tierId,
    ...
    function findYUcrId(
        ...
        returns (uint256)
    {
        uint256 j = _NUM_DEPOSITS_[tierId];
        ...
        returns (uint256)
    {
        ...
        if (locals.x == locals.y) {
            ...
            uint256 xiTcrDiff = (
                _TCR_[tierId][locals.i].value - _TCR_[tierId][xUcr.depositId].value
            );
            uint256 xiUcrDiff = _tcrDiffToUcrDiff(xiTcrDiff, locals.p1);
            uint256 yjTcrDiff = _TCR_[tierId][locals.y].value - _TCR_[tierId][locals.j].value;
     ...
     *
     *  The value `y` is a deposit ID, and `yUcrId` is the greatest UCR ID such that the
     *  corresponding UCR record has a deposit ID equal to `y`.
     */
    function _getY(
        uint128 tierId,
        address account,
    ...
    function _tcrDiffToUcrDiff(
        ...
        returns (uint256)
    {
        return tcrDiff * ldaBalance / PRO_RATA_BASE;
    }
}
