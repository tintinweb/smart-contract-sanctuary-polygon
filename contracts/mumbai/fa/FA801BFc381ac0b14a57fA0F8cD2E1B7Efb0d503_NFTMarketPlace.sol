...
contract NFTMarketPlace is ERC1155, Owned{
    ...
    function transferAnyPDRRentalNFT(address _from, address _to, uint _tokenId, uint _start, uint _end, uint _amount, address _erc20Token) public onlyAdminOrOwner{
        ...
        if (_to != address(0x0)) {
            ...
            makePayment(_erc20Token, _to, platformFeeAccumulatorAddress, (totalPayment.mul(1)).div(100));

            //Royalities: to owner of RWO owner
            makePayment(_erc20Token, _to, ownerRWO, (totalPayment.mul(1)).div(100));

            // Crypto payment: to existing rental
            makePayment(_erc20Token, _to, _from, (totalPayment.mul(98)).div(100));
        }
    ...
    function rentPDR(uint _tokenId, uint _amount, uint _start, uint _end, address _renter, address _erc20Token) public{
        ...
        // Platform Fee: to platformFeeAccumulatorAddress
        makePayment(_erc20Token, _renter, platformFeeAccumulatorAddress, (totalPayment.mul(1)).div(100));

        // Crypto payment: to existing rental
        makePayment(_erc20Token, _renter, tokenIdToTokenOwner[_tokenId], (totalPayment.mul(99)).div(100));
    ...
    function renewPDRRentTimePeriod(uint _tokenId, uint _amount, uint _start, uint _end, address _renter, address _erc20Token) public {
        ...
        // Crypto payment: to existing rental
        makePayment(_erc20Token, _renter, _ownerRWO, (totalPayment.mul(99)).div(100));
    ...
    function distribute(uint256 _tokenId, uint256 _amountPerNFT, address _ERC20TokenAddress, address[] calldata accounts) public {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            makePayment(_ERC20TokenAddress, msg.sender, holder, ((fee).mul(99)).div(100));
        }
    }

    // Distribute Revenue - PDR Rent NFT
    function distributeInMATIC(uint256 _tokenId, uint256 _amountPerNFT, address[] calldata accounts, address _MATICAddress) public {
        require(balanceOf(msg.sender, _tokenId) > 0);
        require(_MATICAddress != address(0));        
        uint256 fee;
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            // Platform Fee: to platformFeeAccumulatorAddress
            makePayment(_MATICAddress, msg.sender, platformFeeAccumulatorAddress, ((platformFeePercentage).mul(fee)).div(100));

            // Crypto payment: to holder
            makePayment(_MATICAddress, msg.sender, holder, ((fee).mul(99)).div(100));
        }
    }
}
