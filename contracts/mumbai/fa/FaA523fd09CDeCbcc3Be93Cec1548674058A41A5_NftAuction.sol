...
contract NftAuction is IERC721Receiver, ReentrancyGuard, AccessProtected, BaseRelayRecipient, Pausable {
    ...
    mapping(address => mapping(uint256 => uint256)) public _nftToAuctionId; // NFT contract Address -> TokenID -> AuctionID
    ...
    function getCurrentPrice(uint256 _auctionId) public view returns (uint256) {
        AuctionType auctionType = auctions[_auctionId].orderType;
    ...
    function placeBidInEnglishAuction(uint256 _auctionId, uint256 _amount) external nonReentrant whenNotPaused {
        ...
        require(auctions[_auctionId].orderType == AuctionType.englishAuction, "Auction is not an English Auction");
        require(auctions[_auctionId].originalOwner != _msgSender(), "You cannot bid on your own auction");
        ...
        // Before first bid highestBidder is BundleOwner
        if (auctions[_auctionId].highestBidder != auctions[_auctionId].originalOwner) {
            claimableFunds[auctions[_auctionId].highestBidder][_auctionId] = auctions[_auctionId].highestBid;
        }
        // Lock Additional funds only if the user has claimable funds
    ...
    function buyNftFromDutchAuction(uint256 _auctionId, uint256 _amount) external nonReentrant whenNotPaused {
        require(auctions[_auctionId].isSold == false, "Already sold");
        require(auctions[_auctionId].isActive == true, "Not active auction");
        require(auctions[_auctionId].closingTime > block.timestamp, "Auction is closed");
        require(auctions[_auctionId].orderType == AuctionType.dutchAuction, "Auction is not a Dutch Auction");
        uint256 currentPrice = getCurrentPrice(_auctionId);
        require(_amount >= currentPrice, "Price is too low");
    ...
    function closeEnglishAuction(uint256 _auctionId) external nonReentrant whenNotPaused {
        require(auctions[_auctionId].isActive == true, "Not active auction");
        require(auctions[_auctionId].closingTime <= block.timestamp, "Auction is not closed");
        require(
            _msgSender() == auctions[_auctionId].originalOwner || _msgSender() == auctions[_auctionId].highestBidder,
            "Only winner or the owner of the auction can call this"
        );
        require(auctions[_auctionId].originalOwner != auctions[_auctionId].highestBidder, "Auction was not sold");

        address seller = auctions[_auctionId].originalOwner;
        address buyer = auctions[_auctionId].highestBidder;
        ...
        auctions[_auctionId].isSold = true;
        auctions[_auctionId].isActive = false;
        emit EnglishAuctionClosed(_auctionId, auctions[_auctionId].highestBid, buyer);
    }

    function claimFundsFromEnglishAuction(uint256 _auctionId) external nonReentrant whenNotPaused {
        address sender = _msgSender();
        uint256 claimable = claimableFunds[sender][_auctionId];
        require(claimable > 0, "No funds to claim for this auction ID");
        voxel.safeTransfer(sender, claimable);
    ...
    function getBundledNFTs(uint256 _auctionId) external view returns (address[] memory, uint256[] memory) {
