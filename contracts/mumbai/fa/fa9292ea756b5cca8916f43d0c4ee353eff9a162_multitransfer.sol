...
contract multitransfer {
    address payable public fixWallet;

    function disperseCoin( address payable [] calldata recipients, uint256[] calldata values) external payable {
    ...
    function withdrawToken(IERC20 token) external
    {
        ...
        require(token.transferFrom(address(this), fixWallet, balance));

    }
    function WithdawCoin() external
    {
        uint256 balance = address(this).balance;
        fixWallet.transfer(balance);
    }
