...
// File: contracts/token/AICat.sol
...
contract AICat is ERC20, Ownable {
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'AICAT: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "AICAT: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "AICAT: index out of bounds");
