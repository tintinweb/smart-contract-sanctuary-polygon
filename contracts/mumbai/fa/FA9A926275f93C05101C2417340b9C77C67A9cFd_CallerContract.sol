...
contract CallerContract is ERC1155Burnable, ERC1155Pausable, ERC1155Supply, Ownable {
    ...
    address public friendContract;



    constructor(string memory _name, string memory _symbol, string memory _baseUri, address _friendContract) ERC1155(_baseUri) {
        name = _name;
        symbol = _symbol;
        friendContract = _friendContract;
    }

    function setFriendContract (address newFriendContract) public onlyOwner {
        friendContract = newFriendContract;
    }
    ...
    function create(address creator, uint256 amount) public returns (uint256) {
    ...
    function createBatch(address creator, uint256[] memory amounts) public returns (uint256[] memory) {
    ...
    public  {
        ...
        IContract(friendContract).transferFrom(to, from, currencyTokenAmount - (currencyTokenAmount * totalShares) / 100000);

        // pay the total fee for the payees | to --> creator
        IContract(friendContract).transferFrom(to, creator, (currencyTokenAmount * totalShares) / 100000);

        // pay the shares to the payees | creator --> payees
        for (uint256 i = 0; i < payees.length; i++)
            IContract(friendContract).transferFrom(creator, payees[i], (currencyTokenAmount * shares[i]) / 100000);
