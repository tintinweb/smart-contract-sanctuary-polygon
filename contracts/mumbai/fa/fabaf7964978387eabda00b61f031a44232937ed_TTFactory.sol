...
// File: interfaces/ITritonContract.sol


pragma solidity ^0.8.15;

interface ITritonContract {
...
contract TTRegistry is MulticallUpgradeable, ERC2771ContextUpgradeable, AccessControlEnumerableUpgradeable {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    /// @dev wallet address => [contract addresses]
    mapping(address => EnumerableSetUpgradeable.AddressSet) private deployments;
...
contract TTFactory is MulticallUpgradeable, ERC2771ContextUpgradeable, AccessControlEnumerableUpgradeable {
    /// @dev Only FACTORY_ROLE holders can approve/unapprove implementations for proxies to point to.
    bytes32 public constant FACTORY_ROLE = keccak256("FACTORY_ROLE");

    TTRegistry public immutable registry;
    ...
    constructor(address _trustedForwarder, address _registry) ERC2771ContextUpgradeable(_trustedForwarder) {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(FACTORY_ROLE, _msgSender());

        registry = TTRegistry(_registry);
    }
    ...
    function deployProxyByImplementation(
        ...
        deployedProxy = ClonesUpgradeable.cloneDeterministic(_implementation, salthash);
        ...
        if (_data.length > 0) {
            ...
            AddressUpgradeable.functionCall(deployedProxy, _data);
        }
    }
    ...
    function addImplementation(address _implementation) external {
        ...
        ITritonContract module = ITritonContract(_implementation);
