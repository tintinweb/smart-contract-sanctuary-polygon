...
contract Episodes is Multicall, Permissions, BatchMintMetadata, ERC1155Drop {
    ...
    event CardsCalled(address indexed account, uint256 indexed tokenId, uint256 balance);
    ...
    function burnToEarn(uint256 _tokenId) external {
        ...
        uint256 balance = cards.balanceOf(msg.sender, (E1C1Requirements[_tokenId][i]));
        emit CardsCalled(msg.sender, E1C1Requirements[_tokenId][i], balance);
        require(balance >= 10, string(abi.encodePacked("Missing card: ", E1C1Requirements[_tokenId][i])));
        }
