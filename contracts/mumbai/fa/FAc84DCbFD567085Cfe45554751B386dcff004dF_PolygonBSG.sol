...
contract PolygonBSG {
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        uint256 addFreeze = 50;
        if(reward.cycleNumber >= 50){
        ...
        _releaseUpRewards(_user, _amount); //membership reward
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        if (upline != address(0)) {
            ...
            if(userInfo[upline].membership == 2){
                //diamond reward
                upRewards.infinityBonusReleased = upRewards.infinityBonusReleased
                ...
                upRewards.infinityBonusReleased = upRewards.infinityBonusReleased
                    ...
                    .div(baseDivider).div(baseDivider)).sub(25e6);
                usdt.transfer(insuranceAccount, 25e6);
            }if(userInfo[upline].membership == 4){
                //crown diamond reward
                upRewards.infinityBonusReleased = upRewards.infinityBonusReleased
                    .add(_amount.mul(infiniteRewardPercents).div(baseDivider)).sub(50e6);
    ...
    function withdraw() external {
        ...
        userin.totalRevenue = userin.totalRevenue.add(withdrawable).add(splitAmt);
        userin.totalFreezed = userin.totalFreezed.sub(withdrawable.add(splitAmt));
    ...
    function _checkClaimable(address _user) public view returns(bool, uint256){
        ...
        for(i = orderlist.length - 1;i >= 0;i--){
            ...
            if(_order.cycle <= rewardInfo[_user].cycleNumber.sub(1) && block.timestamp >= _order.unfreeze){
                return (true,i);
            }
        }
        return (false,i);
