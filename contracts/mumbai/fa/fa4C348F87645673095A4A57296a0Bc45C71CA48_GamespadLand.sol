...
contract GamespadLand is ERC721Psi, AccessProtected {
    ...
    function mintLand(
        ...
        require(_x.length == _quantity, "_x Parameter length mismatch");
        require(_y.length == _quantity, "_y Parameter length mismatch");
        uint256 startTokenId = _minted;
        uint256 index = 0;
        for (uint256 tokenId = startTokenId; tokenId < startTokenId + _quantity; tokenId++) {
            uint256 x = _x[index];
            uint256 y = _y[index];
            uint256 coords = formatCoordinates(x, y);
            require(_coordsToId[coords] == 0, "NFT for {x,y} already minted");
            _coordsToId[coords] = tokenId;
            _idToCoords[tokenId] = coords;
            index++;
        }
     ...
     */
    function getBaseURI() external view onlyAdmin returns (string memory) {
        return _baseURI();
    }
