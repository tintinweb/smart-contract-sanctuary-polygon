...
contract Consents is IConsents, ConsentRegistry, Managers, Points {
    ...
    mapping(uint32 => mapping(bytes32 => ConsentState))
        internal _consentPersonsMap;
    mapping(uint32 => bytes32[]) internal _consentPersonsArr;

    function updatePersonConsents(bytes32 id, uint32[] memory agreedConsents)
        external
        override
    {
        for (uint32 i = 0; i < agreedConsents.length; i++) {
            require(
                consents[agreedConsents[i]].id > 0,
                "Consents: consent doesn't exist"
            );
        }

        uint32[] memory currentConsents = _personConsents[id];
        uint32[] memory newConsents = agreedConsents.filterNotContained(
            currentConsents
        );

        for (uint32 i = 0; i < newConsents.length; i++) {
            require(
                consents[newConsents[i]].active,
                "Consents: it is not possible to accept an inactive consent"
            );
        }

        if (newConsents.length > 0) {
            require(
                _isManagerAllowedToChangeConsents(newConsents) || _isOwner(),
                "Only contract owner or consents manager can add person consents"
            );

            emit PersonConsentsAgreed(id, newConsents);
        }

        uint32[] memory withdrawnConsents = currentConsents.filterNotContained(
            agreedConsents
        );

        if (withdrawnConsents.length > 0) {
            require(
                _isManagerAllowedToChangeConsents(withdrawnConsents) ||
                    _isOwner(),
                "Only contract owner or consents manager can remove person consents"
            );
        ...
        for (uint32 i = 0; i < newConsents.length; i++) {
            ...
            if (
                _consentPersonsMap[consentId][id] ==
                ConsentState.PREVIOUSLY_AGREED
            ) {
    ...
    function personConsents(bytes32 id)
        external
        view
    ...
    function personConsentsHashes(bytes32 id)
        external
        view
        returns (PersonConsentWithState[] memory)
    {
        PersonConsentWithState[] memory res = new PersonConsentWithState[](
            _consentsCount
        );
        uint32 count = 0;
        for (uint32 i = 1; i <= _consentsCount; i++) {
    ...
    function consentPersons(uint32 id)
        external
        view
    ...
    function updateManager(address manager, uint32[] memory consentsIds)
        external
        override
        onlyOwner
    {
        require(
            _checkAllConsentsExist(consentsIds),
            "Consents: not all consents exist, can not update manager."
        );
