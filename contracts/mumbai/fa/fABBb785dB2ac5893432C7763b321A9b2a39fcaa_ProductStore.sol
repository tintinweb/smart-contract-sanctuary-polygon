...
contract ProductStore is Ownable, Pausable {
    ...
    event ToolsChanged(address oldTools, address newTools);
    event MinerChanged(address oldMiner, address newMiner);
     ...
     * @param blindbox blindbox to draw token ID from
     */
    struct ProductInfo {
        uint256 publishBlock;
        uint256 expireBlock;
    ...
    modifier onlyProductExists(uint256 productId) {
        require(productInfos[productId].expireBlock > 0, "ProductStore: product not found");
        _;
    }
    ...
    modifier onlyProductActive(uint256 productId) {
        require(
            ...
            "ProductStore: product not active"
        );
        _;
    }
    ...
    constructor(address _tools, address _miner) {
        require(_tools != address(0) || _miner != address(0), "ProductStore: zero address");
        tools = ITools(_tools);
        miner = IMiner(_miner);
    }
    ...
    function createProduct(
        ...
        require(blindbox != address(0), "ProductStore: zero address");
        require(
            publishBlock > block.number && expireBlock > publishBlock ,
            "ProductStore: invalid block range"
        );
        require(amountPerNFT > 0, "ProductStore: amountPerNFT must be positive");
        require(
            rebatePercentage >= 0 && rebatePercentage <= 1 * e18,
            "ProductStore: rebatePercentage must be positive"
        );
        require(nftTypeId <= tools.lastTypeId(), "ProductStore: nft typeId not found");
    ...
    function setTools(address newTools) external onlyOwner {
        require(newTools != address(0), "ProductStore: zero address");
        address oldTools = address(tools);
        tools = ITools(newTools);

        emit ToolsChanged(oldTools, newTools);
    }

    function setMiner(address newMiner) external onlyOwner {
        require(newMiner != address(0), "ProductStore: zero address");
        address oldMiner = address(miner);
        miner = IMiner(newMiner);

        emit MinerChanged(oldMiner, newMiner);
    }

    function setCashierAddress(address _cashierAddress) external onlyOwner {
        require(_cashierAddress != address(0), "ProductStore: zero address");
    ...
    function getNFT(uint256 productId)
        external
        payable
        whenNotPaused
        onlyProductExists(productId)
        onlyProductActive(productId)
    {
        uint256 amount = msg.value;
        _getNFT(productId, msg.sender, amount);
        
        //here mint nft
        uint256 nftAmount = amount.div(productInfos[productId].amountPerNFT);
        
        tools.mintNFT(msg.sender, productInfos[productId].nftTypeId, nftAmount);
        
        uint16[] memory tokenIds = IBlindBox(productInfos[productId].blindbox).draw(nftAmount);
        miner.mintByIds(tokenIds, msg.sender);
    }
    ...
    function emergencyUnstake(uint256 productId) external whenPaused onlyProductExists(productId) {
        ...
        IERC20(productInfos[productId].token).transfer(msg.sender, amount);
    ...
    function _getNFT(
        ...
        require(amount > 0, "ProductStore: cannot getNFT zero amount");
        require(
            amount.mod(productInfos[productId].amountPerNFT) == 0,
            "ProductStore: amount not amountPerNFT multiple"
        );

        uint256 rebatePercentage = productInfos[productId].rebatePercentage;

        userData[productId][user].totalSpentAmount = userData[productId][user].totalSpentAmount.add(amount);
        productData[productId].totalTokenAmount = productData[productId].totalTokenAmount.add(amount);

        if (rebatePercentage != 1*e18){
            require(cashierAddress != address(0), "ProductStore: cashier is zero address");
            payable(cashierAddress).transfer(payable(address(this)).balance);
        } 
    ...
    function _rebate(
        ...
        uint256 rebatableAmount = _rebatable(user);
        require(amount > 0, "ProductStore: cannot rebate zero amount");
        require(rebatableAmount > 0, "ProductStore: no rebate amount");
        require(amount <= rebatableAmount, "ProductStore: not enough rebate amount");
        ...
        for (i; i <= userVestingInfo[user].lastVesting; i++ ){
            ...
            if (block.number >= userVesting[user][i].unlockBlock){
                if (remaining >= userVesting[user][i].amount) {
                    ...
                    userData[productId][user].totalSpentAmount = 
                        userData[productId][user].totalSpentAmount.sub(userVesting[user][i].amount);
                    productData[productId].totalTokenAmount = 
                        productData[productId].totalTokenAmount.sub(userVesting[user][i].amount);
                    ...
                    // No sufficiency check required as sub() will throw anyways
                    userData[productId][user].totalSpentAmount = 
                        userData[productId][user].totalSpentAmount.sub(remaining);
                    productData[productId].totalTokenAmount = 
                        productData[productId].totalTokenAmount.sub(remaining);
        ...
        require(remaining == 0 , "ProductStore: not enough rebate amount");
...
interface IMiner is IERC721 {

    function mintByIds(uint16[] memory tokenIds, address to) external;
