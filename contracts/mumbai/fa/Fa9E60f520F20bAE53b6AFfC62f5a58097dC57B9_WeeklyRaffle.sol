...
contract WeeklyRaffle is WeeklyRaffleAccessControl, INFTMintInitiator, WonderVRFGeneratorV2 {
    ...
    event WinnerIndicesSelected(uint256 _weekNumber, uint256 _requestId, uint256[] _numWords, uint256[] _indices);
    ...
    event RaffleRewardMintRequestCancelled(address _user, uint256 _requestId);
    ...
    mapping(uint256 => MintRequest) public requestToMint; //requestId => weekNumber
    ...
    function getWeeklyConfig(uint256 _weekNumber) public view returns (WeeklyConfigResponse memory) {
    ...
    function getRemainingCount(uint256 _weekNumber, address _user) public view returns (uint256) {
        return weeklyConfigs[_weekNumber].remainingClaimCount[_user];
    }

    function getInProgressCount(uint256 _weekNumber, address _user) public view returns (uint256) {
        return weeklyConfigs[_weekNumber].inProgressClaimCount[_user];
    }

    function getWinners(uint256 _weekNumber) public view returns (User[] memory) {
        WeeklyRaffleConfig storage config = weeklyConfigs[_weekNumber];
    ...
    function getPendingCount(uint256 _weekNumber, address _user) public view returns (uint256) {
        return weeklyConfigs[_weekNumber].inProgressClaimCount[_user];
    }
    ...
    function processRandomness(uint256 _requestId, uint256[] memory _numWords) internal override {
        ...
        emit WinnerIndicesSelected(weekNumber, _requestId, _numWords, config.indices);
    }
    ...
    function resetMintRequest(address _user, uint256 _requestId) external onlyRole(HANDLER_ROLE) {
        ...
        emit RaffleRewardMintRequestCancelled(_user, _requestId);
    }
    ...
    function temp_(string memory temp) public pure returns (bytes32 hash) {
        hash = keccak256(abi.encodePacked(temp));
        return hash;
    }
}
