...
contract KeeperArctic is AccessControlledAndUpgradeable {
  ...
  function shouldUpdateMarket() external view returns (bool canExec, bytes memory execPayload) {
    ...
    for (uint32 index = 0; index < latestMarket; index++) {
  ...
  function getMissedEpochPriceUpdates(
    ...
    while (_isSearchingForMissedEpochExecutionInfo) {
      ...
      if (
        ...
        if (
          _currentMissedEpochPriceUpdatesArrayIndex == _numberOfMissedEpochs ||
          latestRoundId == _currentOracleIndex
        ) {
...
contract GEMS is AccessControlledAndUpgradeable {
  ...
  function initialize() public initializer {
    ...
    // _setupRole(GEM_ROLE, DEFAULT_ADMIN_ROLE);
    // _setRoleAdmin(GEM_ROLE, DEFAULT_ADMIN_ROLE);
