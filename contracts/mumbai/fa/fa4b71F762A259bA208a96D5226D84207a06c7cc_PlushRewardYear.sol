    ...
    Date
{
    ...
    returns (uint256)
    {
        ...
        DateT memory now = DateT(getYear(block.timestamp), getMonth(block.timestamp), getDay(block.timestamp));
        DateT memory nextBirthday = DateT(now.year, birthday.month, birthday.day);

        if(toTimestamp(now.year, now.month, now.day, 0, 0, 0) > toTimestamp(now.year, birthday.month, birthday.day, 0, 0, 0)){
            nextBirthday.year += 1;
        }

        uint256 nextBirthdayTimestamp = toTimestamp(nextBirthday.year, nextBirthday.month, nextBirthday.day, 0, 0, 0);
        uint256 mintTimestamp = toTimestamp(mint.year, mint.month, mint.day, 0, 0, 0);
        uint256 dateNowTimestamp = toTimestamp(now.year, birthday.month, birthday.day, 0, 0, 0);

        uint256 periodInMonth = (dateNowTimestamp - mintTimestamp) / 1 days;
