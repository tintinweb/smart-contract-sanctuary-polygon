...
contract Marketplace {
    uint256 public itemCounter; // unique id for each NFT
    address payable owner; // address of the owener of smart contract
    uint256 public listingPrice; // price to list an NFT

    // an NFT that is listed in the contract
    struct MarketItem {
        uint256 itemId;
        address nftContractAddress; // smart contract address that used to mint NFT
        uint256 tokenId;
        address payable seller;
        address payable owner; // account owns NFT after it is sold
        uint256 price; // price of NFT
        bool isSold;
        bool isPresent;
    }

    mapping(uint256 => MarketItem) private marketItems; // map an unit256 to a MarketItem

    // action performed when an NFT is listed
    ...
    constructor() {
  		itemCounter = 0;
  		owner = payable(msg.sender); // owner of smart contract
    ...
    function listMarketItem(address nftContractAddress, uint256 tokenId, uint256 price) public payable {
        ...
        // transfer NFT from seller to smart contract
        ...
        // transfer listing price to owner of smart contract
