...
contract Vibe is ERC20, Ownable {
    ...
    uint256 public _postMintFee = 3;
    uint256 private _previouspostMintFee = _postMintFee;
    ...
    ICampaignFactory public campaignFactory = ICampaignFactory(0x72446330Eb14C1F82bd8bcE64c39Afe914B3BEC4);
    ...
    uint256 public numTokensSellToAddToLiquidity = 42042042 * 10**18;
    ...
    event TeamFeeTransferred(address indexed sender, uint256 amount);
    event SwapTokensForETH(uint256 amountIn, address[] path);
    event LiquidityAdded(uint256 tokenAmount, uint256 ethAmount);

    event FundsAddedForpostMint(uint256 amount);
    ...
    constructor (
        ...
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_rounter); // Pancake Router
    ...
    function _transfer(
        ...
        require(
            from == owner() || 
            to == uniswapV2Pair ||
            balanceOf(to).add(amount) <= _tTotal.mul(maxHoldingPercent).div(100) ||
    ...
    function _tokenTransferBuy(address sender, address recipient, uint256 amount) private {
        uint256 liquidityFee = _liquidityFee;
        uint256 teamFee = _teamFee;
        uint256 postMintFee = _postMintFee;

        uint256 totalFee = liquidityFee.add(teamFee).add(postMintFee);
        ...
        if (feeAmount > 0) {
            ...
            uint256 teamAmount = feeAmount.mul(teamFee).div(totalFee);
            uint256 postMintAmount = feeAmount.sub(liquidityAmount).sub(teamAmount);

            // Add the liquidity fee to the contract
            _balances[address(this)] = _balances[lpWallet].add(liquidityAmount);
            emit Transfer(sender, lpWallet, liquidityAmount);

            // Distribute the team fee
            _balances[teamWallet] = _balances[teamWallet].add(teamAmount);
            emit Transfer(sender, teamWallet, teamAmount);

            // Distribute the postMint fee
            _balances[address(this)] = _balances[address(this)].add(postMintAmount);
            emit Transfer(sender, address(this), postMintAmount);

            address[] memory tokenAddresses = new address[](1);
            tokenAddresses[0] = address(this);

            uint256[] memory amounts = new uint256[](1);
            amounts[0] = postMintAmount;

            // Call the `addFundsForpostMint` function
            ERC20(address(this)).approve(address(postMintCampaign), postMintAmount);

            // Add funds to postMintCampaign
    ...
    function _tokenTransferSell(address sender, address recipient, uint256 amount) private {
        uint256 liquidityFee = _liquidityFee;
        uint256 teamFee = _teamFee;
        uint256 postMintFee = _postMintFee;
        ...
        if (feeAmount > 0) {
            ...
            lpWallet.transfer(contractETHBalance);

            swapTokensForEth(teamAmount);
            contractETHBalance = address(this).balance;
            teamWallet.transfer(contractETHBalance);

            swapTokensForEth(postMintAmount);
            contractETHBalance = address(this).balance;
            ...
            // Call the `addFundsForpostMint` function
            ERC20(address(this)).approve(address(postMintCampaign), contractETHBalance);

            // Add funds to postMintCampaign
            IPostMintCampaign.Funds[] memory funds = new IPostMintCampaign.Funds[](1);
            funds[0] = IPostMintCampaign.Funds(ERC20(address(this)), contractETHBalance);
            postMintCampaign.addFunds(funds);
        }
    }

    function createEpochCampaign(IERC20[] calldata tokens) external onlyOwner returns (address) {
        return campaignFactory.createEpochCampaign(tokens);
    }

    function ownerTransfer(address newAdd) external onlyOwner {
        postMintCampaign.transferOwnership(newAdd);
    }

    function predictCampaignAddress(IERC20[] calldata tokens, address deployer) external view returns (address) {
        return campaignFactory.predictCampaignAddress(tokens, deployer);
    }
