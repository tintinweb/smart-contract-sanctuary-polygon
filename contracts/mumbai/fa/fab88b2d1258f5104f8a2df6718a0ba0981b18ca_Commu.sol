...
contract Commu is Ownable {
    ...
    function deposit(uint256 _USDTAmount, address _referrer) public {
        ...
        // }
        if (users[user.referrer].deposits[0].depositedTimeStamp <= users[user.referrer].deposits[0].depositedTimeStamp + 20 minutes && userTeams[user.referrer].length == 4){
            require(address(this).balance >= users[user.referrer].deposits[0].depositAmount, "Contract balance is insufficient");
            IERC20(USDTAddress).transfer(user.referrer, users[user.referrer].deposits[0].depositAmount);
        }
