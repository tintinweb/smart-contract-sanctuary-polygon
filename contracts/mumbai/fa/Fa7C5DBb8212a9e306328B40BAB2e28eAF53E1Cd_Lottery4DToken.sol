...
import "./interface/ILottery2DToken.sol";

contract Lottery4DToken is
    ERC1155,
    IERC1155Enumerable,
    Ownable,
    ContractMetadata,
    ...
    ILottery2DToken
{
    ...
    function batchMintTickets(
        ...
        uint256[] memory _drawDates,
        uint256[] memory _numberTickets,
        uint256[] memory _amounts,
        string memory _baseURI
    ) public onlyAdmin {
        require(_amounts.length > 0, "BatchMintTickets: Minting zero tokens.");
        require(
            _tokenIds.length == _amounts.length,
            "BatchMintTickets: amounts or quantities length mismatch."
        );
        require(
            _tokenIds.length == _drawDates.length,
            "BatchMintTickets: drawDates length mismatch."
        );
        require(
            _tokenIds.length == _numberTickets.length,
            "BatchMintTickets: number tickets length mismatch."
        );

        for (uint256 index = 0; index < _tokenIds.length; index += 1) {
            require(
                _tokenIds[index] >= _lotteryId * 10000 &&
                    _tokenIds[index] <= _lotteryId * 10000 + 10000,
                "BatchMintTickets: Token Id invalid."
            );
            _metas[_tokenIds[index]].lotteryId = _lotteryId;
            _metas[_tokenIds[index]].tokenId = _tokenIds[index];
            _metas[_tokenIds[index]].drawDate = _drawDates[index];
            _metas[_tokenIds[index]].numberTicket = _numberTickets[index];
            _metas[_tokenIds[index]].quantity = _amounts[index];
            _metas[_tokenIds[index]].createdAt = block.timestamp;
        }
        nextTokenIdToMint_ = _tokenIds[_tokenIds.length - 1];
        currentLotteryId = _lotteryId;
        _batchMintMetadata(_tokenIds[0], _tokenIds.length, _baseURI);
    ...
    function getMetadata(uint256 _tokenId)
        public
        view
        returns (ILottery2DToken.Metadata memory)
    {
...
interface ILottery2DToken is IERC1155 {
    struct Metadata {
        uint256 lotteryId;
        uint256 tokenId;
        uint256 drawDate;
