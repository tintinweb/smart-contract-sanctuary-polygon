...
contract Paymaster is IPaymaster, UpgradeableACL {
  ...
  address public entryPoint; // reference to entry point

  // Custom API Parameters
  enum Paymaster_Options {
    //TIME,
    TXS,
    VALUE
  }

  struct UserPaymasterInfo {
    string option;
    uint256 amount;
    uint256 amount_left;
  }

  // Mapping from address to PaymasterInfo for each user
  mapping(address => UserPaymasterInfo) public paymasterRegistry;
  ...
  /**
   Function to set the a user of registry mapping
   */
  function addToRegistry(
    ...
    UserPaymasterInfo memory userToAdd = UserPaymasterInfo(option, amount, amount);
    paymasterRegistry[userAddress] = userToAdd;
  }
  ...
  /**
    Functions to update the specific condition after user transaction is executed

   */
  function updateAmount(address user, uint256 cost) public {
    UserPaymasterInfo storage userInfo = paymasterRegistry[user];
    userInfo.amount_left -= cost;
  }

  /**
   Function to check if condition is still valid to transact

   * @param currentOpDelta change to amount due to current operation

  */
  function conditionChecker(address user, uint256 currentOpDelta) public view returns (bool) {
    return (paymasterRegistry[user].amount_left - currentOpDelta) > 0;
  }
  ...
  function validatePaymasterUserOp(
    ...
    // checks for paymaster payment conditions
    if (compareStrings(paymasterRegistry[op.sender].option, "TXS")) {
      require(conditionChecker(op.sender, 1), "Failed txs condition");
    }
    // if (paymasterRegistry[op.sender].option == Paymaster_Options.TIME) {
    //   require(conditionChecker(op.sender, 1), "Failed time condition");
    // }
    if (compareStrings(paymasterRegistry[op.sender].option, "VALUE")) {
      require(conditionChecker(op.sender, cost), "Failed value condition");
    }

    // Checks balance of paymaster
  ...
  function postOp(
    ...
    // Updates conditions
    if (compareStrings(paymasterRegistry[data.sender].option, "TXS")) {
      updateAmount(data.sender, cost);
    }
    if (compareStrings(paymasterRegistry[data.sender].option, "VALUE")) {
      updateAmount(data.sender, 1);
    }
  }
