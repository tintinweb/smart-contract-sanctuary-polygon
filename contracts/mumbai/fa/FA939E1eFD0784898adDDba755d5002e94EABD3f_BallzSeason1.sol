    ...
    ERC1155PausableUpgradeable
{
    ...
    uint256 public nftRoyalty;
    uint256 public tokensInCirculation;
    uint256 public nextTokenId;
    bool public pausedCheck;
    bool public revealed;
    string public minterType;
    ...
    function initialize() public initializer {
        ...
        ownerAddress = 0xdd9a0cdD608302E4D22d88643bc2599fb06eC51a;
        baseExtension = ".json";
        cost = 0.00 ether;
        preSaleCost = 0.00 ether;
        maxMintAmount = 5;
        maxSupply = 500;
        nftRoyalty = 1;
        nextTokenId = 1;
    ...
    mapping(uint256 => LXTBallz) nftLogs;
    ...
    modifier mintCompliance(uint256 _mintAmount, uint256 Royalty) {
        require(
            hasRole(MINTER_ROLE, _msgSender()),
            "ERC1155PresetMinterPauser: must have minter role to mint"
        );
        require(!pausedCheck, "The contract is paused!");
        require(
            tokensInCirculation + _mintAmount <= maxSupply,
            "Max supply exceeded!"
        );
        require(
            (Royalty >= 1) && (Royalty <= 10000),
            "LXTBallz: Percentage not sufficient, min is 1, max is 100"
        );
        _;
    }
    ...
    function mint(uint256 mintAmount, string memory tokenURI)
        public
        payable
        virtual
        mintCompliance(mintAmount, nftRoyalty)
    {
        for (uint256 i = 0; i < mintAmount; i++) {
            ...
            LXTBallz memory newNft = LXTBallz({
                id: nextTokenId,
                tokenURI: tokenURI,
            ...
            royalty[nextTokenId][msg.sender] = nftRoyalty;
            emit Mint(nextTokenId, tokenURI, msg.sender, newNft);
            nextTokenId++;
            tokensInCirculation++;
        }
    }
    ...
    function mintBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            LXTBallz memory newNft = LXTBallz({
                id: ids[i],
                tokenURI: string(abi.encodePacked(tokenURI[i])),
                creator: ownerAddress,
                owner: msg.sender,
                timeOfGenerate: block.timestamp
            });
            nftLogs[ids[i]] = newNft;
            royalty[ids[i]][msg.sender] = nftRoyalty;
            emit Mint(
                ids[i],
                string(abi.encodePacked(tokenURI[i])),
                msg.sender,
                newNft
            );
            nextTokenId++;
        ...
        virtual
    {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
        require(_exists(id), "LXTBallz: URI set of nonexistent token");
        nftLogs[id].tokenURI = tokenUri;
    }

    function setMinterType(string memory mintType) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
    ...
    function setRoyalty(uint256 _royalty) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function whitelistUser(address _user) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function removeWhitelistUser(address _user) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function addPresaleUser(address _user) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function removePresaleUser(address _user) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function setNotHiddenURI(string memory _notRevealedURI) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    function setReveal() public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, _msgSender()),
            "LXTBallz: must have admin role to change"
        );
    ...
    // {
    //     require(_exists(id), "LXTBallz: Nonexistent token");
    //     return (nftLogs[id].creator, (salePrice * nftLogs[id].discount) / 10000);
    // }

    function getNFTDetails(uint256 id) external view returns (LXTBallz memory) {
        return nftLogs[id];
    }
    ...
    function currentMinter() external view returns (string memory) {
        return minterType;
    }
