...
contract MUSIC_NFT is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    address public fractional;


    event onMint(uint256 indexed tokenid,address indexed creator,string cid,uint256 roylty);

    constructor(string memory _name, string memory _symbol,address _minter,address _fractional) ERC721(_name, _symbol) {
    ...
    function changefractional(address _f) public onlyOwner returns (bool){
        fractional = _f;
        return  true;
    }
