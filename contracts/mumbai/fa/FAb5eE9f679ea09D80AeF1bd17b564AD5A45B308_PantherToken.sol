...
contract PantherToken is Token {
    ...
    function enable_liquidity(bool _status) public {
        ...
        if (_status) {
            ...
            //For marketing
            allocated_users[marketing].lock_period = block.timestamp + 90 days;

            //For team
            allocated_users[team].lock_period = block.timestamp + 365 days;

            //For staking
            allocated_users[staking].lock_period = block.timestamp + 90 days;

            //For EcoSystem
            allocated_users[ecoSystem].lock_period = block.timestamp + 180 days;

            //For advisor
            allocated_users[advisor].lock_period = block.timestamp + 270 days;

            //For productDevelopment
            allocated_users[productDevelopment].lock_period = block.timestamp + 90 days;

            //For partnership
            allocated_users[partnership].lock_period = block.timestamp + 30 days;
        }
    }

    function claimToken() public {
        //For EcoSystem & advisor & productDevelopment
        uint256 release_period_1 = 90 days;

        //For team and marketing staking
        uint256 release_period_2 = 30 days;

        //For bounty
        uint256 release_period_3 = 30 days;

        //For partnership
        uint256 release_period_5 = 60 days;

        //For publicSale
        uint256 release_period_7 = 30 days;

        //For presale &
        uint256 release_period_8 = 60 days;
        //For Liquidity
        uint256 release_period_9 = 90 days;
