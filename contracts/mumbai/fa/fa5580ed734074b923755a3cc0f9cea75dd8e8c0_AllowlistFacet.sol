...
contract AllowlistFacet is AllowlistModifiers {
    ...
    ///      revert AllowlistFinished
    /// @param _allowlistId the id of allowlist to buy
        ...
        purchasable(_allowlistId)
    {
        ...
        uint256 allowlistsBalance = s.userInventory[msg.sender].allowlistsBalance[_allowlistId];
        
         _fundManagerFacet().charge(s.allowlists[_allowlistId].price, msg.sender);

        if (allowlistsBalance < 1)  
            s.userInventory[msg.sender].allowlistsIds.push(_allowlistId);
        s.userInventory[msg.sender].allowlistsBalance[_allowlistId]++;
        ...
        returns (uint256)
    {
        ...
        s.allowlists[s.allowlistsCount].id = s.allowlistsCount;
    ...
    /// @notice Return a Allowlist by ID
    /// @param _allowlistId the Allowlist id
    /// @return the Allowlist with _allowlistId ID
...
/// @title Allowlist Modifiers Facet
/// @author Kfish n Chips
/// @custom:security-contact [email protected]
abstract contract AllowlistModifiers is IAllowlistFacet, Listing {

    /// @notice Check if Allowlist can be purchase
    /// @dev Revert with:
    ///     AllowlistNotStarted
    ///     AllowlistFinished
    /// @param _allowlistId the ID of Allowlist
    modifier purchasable(uint256 _allowlistId) override {
        uint256 projectId = s.allowlists[_allowlistId].projectId;
        _purchasable( _allowlistId, projectId);
        if (s.allowlists[_allowlistId].startTime > block.timestamp) revert  AllowlistNotStarted(); // solhint-disable-line not-rely-on-time
        if (s.allowlists[_allowlistId].endTime < block.timestamp) revert  AllowlistFinished(); // solhint-disable-line not-rely-on-time
        _;
    }

    /// @notice Check if an Allowlist exits with ID _allowlistId
    /// @dev Revert QueryNonExistentAllowlist
    /// @param _allowlistId The Id of Allowlist
    modifier exists(uint256 _allowlistId) override {
        if (_allowlistId == 0 || _allowlistId > s.allowlistsCount) revert QueryNonExistentAllowlist();
        _;
    }

    /// @notice Check if an Allowlist is active with ID _allowlistId
    /// @dev Revert AllowlistNotActive
    /// @param _allowlistId The Id of Allowlist
    modifier isActive(uint256 _allowlistId) override {
        if (!s.allowlists[_allowlistId].active) revert AllowlistNotActive();
        _;
    }
    ...
    /// @dev Revert AllowlistNotActive
    /// @param _allowlistId The Id of Allowlist
    modifier hasStockAvailable(uint256 _allowlistId) override {
        if (!( (s.allowlists[_allowlistId].stock - s.allowlists[_allowlistId].purchased) > 0)) revert NoStockLeft();
        _;
    }

    // @notice Check if _date is 
    /// @dev Revert StartTimeBeforeCurrentTime
    /// @param _date The Id of Allowlist
    ...
    // @notice Check if _newStock is greater than Allowlist purchased
    /// @dev Revert StockUnderPurchase
    /// @param _allowlistId The Id of Allowlist
    /// @param _newStock The new Stock
    modifier checkStockOverPurchase(uint256 _allowlistId, uint256 _newStock) {
        if (_newStock < s.allowlists[_allowlistId].purchased) revert StockUnderPurchase(); 
        _;
    }
