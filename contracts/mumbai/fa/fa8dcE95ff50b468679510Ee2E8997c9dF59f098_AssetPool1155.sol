...
contract AssetPool1155 is IAssetPool1155, Ownable, ReentrancyGuard, Pausable, ERC1155Holder {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) userBalance;
    ...
    event Withdrew(address indexed by, address indexed asset, uint256[] tokenIds, uint256[] amounts);

    modifier onlyActivatedPool() {
        if (!poolManagement.isActivatedPool(pid)) revert DeactivatedPool(pid);
        _;
    }
    ...
    modifier onlyPlayerRental() {
        (, , , address playerRental, ) = poolManagement.getPoolInfo(pid);
    ...
    function withdraw(
        ...
        emit Withdrew(msg.sender, _asset, _tokenIds, _amounts);
    }
