...
/**
 * @title BiletsClubStatus Contract
 */
    ...
    ReentrancyGuard
{
    ...
    string public contractURI =
        "https://bilets.club/api/opensea_contracts_meta/contractStatus";
    ...
    mapping(uint256 => uint256) public NFTstamina;
    ...
    constructor() ERC721A("Bilets Club Status", "BLT-Club-Status") {
        _safeMint(0x000000000000000000000000000000000000dEaD, 5);
    ...
    function getStaminaUsed(uint256 tokenID) public view returns (uint256) {
        return getStamina(tokenID) - block.timestamp;
    }

    function getStamina(uint256 tokenID) internal view returns (uint256) {
        if (NFTstamina[tokenID] <= block.timestamp) {
            return block.timestamp;
        } else {
            return NFTstamina[tokenID];
        }
    }
    ...
    function changeStamina(
        ...
        if (earn) NFTstamina[tokenID] = getStamina(tokenID) + amountStamina;
        else {
            NFTstamina[tokenID] = getStamina(tokenID) - amountStamina;
        }
    }
