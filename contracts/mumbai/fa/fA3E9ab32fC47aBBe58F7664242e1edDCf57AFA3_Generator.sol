...
contract Generator is IGenerator, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ICreature public creatureContract;
    IRandomizer private randomizerContract;

    /// @dev START storage for group
    uint256[] internal groupToNftCounts;
    uint256[] internal groupToNftPrices;
    address[] internal groupToPaymentTokens;
    uint8[] internal groupToNftGen;
    ...
    /// @dev Contain chances information
    ...
    modifier onlyEOA() {
        ...
        require(_sender == tx.origin, "Gen: invalid sender (1)");

        uint256 size;
        assembly {
            size := extcodesize(_sender)
        }
        require(size == 0, "Gen: invalid sender (2)");
    ...
    function setMintBankerChance(uint128 _chanceToMintBanker) external override onlyOwner {
        require(
            ...
            "Gen: invalid mint banker chance"
        );
    ...
    function setGroups(
        ...
        require(_groupToNftCounts.length > 0, "Gen: arrays is empty");
        require(
        ...
        for (uint256 i = 0; i < _groupToNftCounts.length; i++) {
            require(_groupToNftCounts[i] != 0, "Gen: nft count can't be a zero");

            if (i > 0) require(_groupToNftCounts[i] > _groupToNftCounts[i - 1],
                "Gen: each next value should be bigger then previous");
    ...
    function getGroupInfo(uint256 _groupId) external view override returns (uint256, uint256, uint256, address) {
        return (
            groupToNftCounts[_groupId],
            groupToNftPrices[_groupId],
            groupToNftGen[_groupId],
            groupToPaymentTokens[_groupId]
        );
    }

    /// @dev Group count: [10, 20, 30]. 1-10: group 0, 11-20: group 1, 21-30: group 2
    function mint(address _to, uint256 _amount) external payable override onlyEOA nonReentrant {
        ...
        require(_amount > 0, "Gen: limit has been reached");
    ...
    function _mintProcess(
        ...
        if (_randomizerContract.random(_getDecimals()) < _chances.toMintBanker) {
            _creatureContract.addCreatureInfo(_num, _gen, 0);
            emit BankerCreated(_num, _to);
        } else {
            _creatureContract.addCreatureInfo(_num, _gen, uint8(5 + _randomizerContract.random(4)));
    ...
    function _paymentProcess(uint256[] memory _nftCountToGenerateInGroup) private {
        ...
        for (uint256 i = 0; i < _nftCountToGenerateInGroup.length; i++) {
            uint256 _totalGroupPrice = groupToNftPrices[i] * _nftCountToGenerateInGroup[i];
            if (_totalGroupPrice == 0) continue;

            address _paymentTokenAddress = groupToPaymentTokens[i];
            if (_paymentTokenAddress == address(0)) {
                _nativePrice += _totalGroupPrice;
            } else {
        ...
        if (_nativePrice > 0) {
            require(msg.value >= _nativePrice, "Gen: insufficient funds");
            if (msg.value > _nativePrice) {
                (bool _txStatus,) = payable(msg.sender).call{value: msg.value - _nativePrice}("");
                require(_txStatus, "Gen: transfer failed");
            }
        }
    }
...
interface IGenerator {
    ...
    function getGroupInfo(uint256 _groupId) external view returns (uint256, uint256, uint256, address);
