...
abstract contract GoldListCrossChain is
    OwnableUpgradeable,
    AccessControlUpgradeable
{
    ...
    constructor(
        ...
        maticUsdPriceFeed = AggregatorV3Interface(priceFeedAddress);
     ...
     */
    function addGoldList(address newAddress, bool status)
        public
        virtual
        onlyAdmin
    {
     ...
     *
     */
    function revokeGoldList() external virtual onlyOwner {
    ...
    function withdrawMatic() external payable virtual onlyOwner {
    ...
    function revokeAdmin(address revokedAdmin) external virtual onlyOwner {
    ...
    function _lockTokens(uint256 lockedTokens) internal virtual returns (bool) {
...
contract GoldListPolygon is GoldListCrossChain, Router {
    uint256 private _lockedTokens;
    address goldListBSCAddress;
    ...
    constructor(
        address _abacusConnectionManagerPolygon,
        address _interchainGasPaymasterPolygon,
        address _token,
        address _admin,
        address _priceFeedAddress,
        address _multiSigWallet
    )
        //   ARGS constructor:  address _token,  address admin,  address priceFeedAddress,  address multiSigWallet
        GoldListCrossChain(_token, _admin, _priceFeedAddress, _multiSigWallet)
    {      
        _setAbacusConnectionManager(_abacusConnectionManagerPolygon);
        _setInterchainGasPaymaster(_interchainGasPaymasterPolygon);
    }


    // We will have the tokens in one receiver contract(In Polygon)
    //  it will be called by two sender Contracts
    //  One will be in polygon and one will be in BSC

    function _lockTokens(uint256 lockedTokens)
        internal
        virtual
        override
        returns (bool)
    ...
    function claimTokens() external payable override {
        require(goldList[_msgSender()], "Caller is not in Gold list");

        uint256 srgTokens = getAmountOfTokens(msg.value);

        // check balance of unlockedTokens
        require(
            IERC20(token).balanceOf(address(this)) - _lockedTokens >= srgTokens,
            "Not enough tokens"
        );
