...
*/  
    ...
    event orderCreated(uint256 Type, uint256 Amount, uint256 Price, IERC20 Currency);
...
*/
    ...
    function createOrder(
        ...
        orN++;
        uint256 _price = _currencyAmount.div(_amount);
        ...
        emit orderCreated(_type, _amount, _price, _currency);
        return (orN, _type);
    }
    ...
    function takeOrder(uint256 _orN, uint256 _amount) external nonReentrant{
        ...
        uint256 priceUSD = p2p[_orN].currencyFalseAmount.div(p2p[_orN].orderFalseAmount);
        uint256 toPay = _amount.mul(priceUSD);
        uint256 toReceive = _amount.div(priceUSD);
        require(_currency.balanceOf(msg.sender) >= toPay.mul(USDdcml), "Not enought USD");
        ...
        require(_amount.mul(ISLAMIdcml) <= p2p[_orN].remainAmount, "Seller has less ISLAMI than order");
            if(_amount.mul(ISLAMIdcml) < p2p[_orN].remainAmount){
                ...
                //require approve from currency(USDT, USDC) contract
                _currency.transferFrom(msg.sender, seller, toPay.mul(USDdcml));
        ...
        require(_amount.mul(USDdcml) <= p2p[_orN].remainCurrency, "Seller has less USD than order");
            if(_amount.mul(USDdcml) < p2p[_orN].remainCurrency){
                ...
                //require approve from ISLAMICOIN contract
                ISLAMI.transferFrom(msg.sender, seller, toReceive.mul(ISLAMIdcml));
