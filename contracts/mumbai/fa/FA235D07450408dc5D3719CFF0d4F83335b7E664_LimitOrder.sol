...
contract LimitOrder is OrderBase {
    ...
    function limitOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        x._in = y._limitItem._swapInAmount;
        if (!y.tcc.isV3) {
            approveDex(IERC20(x.swapInToken), V2Dex, x._in);
        } else {
            approveDex(IERC20(x.swapInToken), V3Dex, x._in);
        }
        if (x.swapOutToken == weth) {
            if (!y.tcc.isV3) {
                x.swapOutAmount = _dov2swap(true, y.tcc._swapPathConfig1._routerAddressListV2, x._in, y.tcc._swapPathConfig1._swapPathList, _user);
            } else {
                x.swapOutAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _user, x._in, false);
            }
        } else {
            if (!y.tcc.isV3) {
                x.swapOutAmount = _dov2swap(false, y.tcc._swapPathConfig1._routerAddressListV2, x._in, y.tcc._swapPathConfig1._swapPathList, _user);
            } else {
                x.swapOutAmount = _dov3swap(y.tcc._swapPathConfig1._factoryV3[0], y.tcc._swapPathConfig1._swapPathList, y.tcc._swapPathConfig1._feeListV3, _user, x._in, true);
            }
        }
        y._limitItem._swapInAmount = 0;
        ...
        SwapEventItem memory t = SwapEventItem(x.swapInToken, x.swapOutToken, IERC20(x.swapInToken).decimals(), IERC20(x.swapOutToken).decimals(), 0, 0, 0, x._in, 0, x.swapOutAmount);
