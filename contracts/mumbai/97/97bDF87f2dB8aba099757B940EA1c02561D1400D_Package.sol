...
import "./utils/IStaking.sol";

contract Package is Ownable, AccessControl {
    ...
    uint256 public commentPercentage;
    ...
    constructor(
        ...
        setPackagePercentage(30, 15, 30, 25);
    ...
    function approvePackage(
        ...
        uint256 entryFee = voting.getContestEntryFee(_contestId);
        require(
            entryFee > 0,
            "[Package.approvePackagePurchase] Invalid contestId"
        );
        require(
            balanceVault.getBalance(_userAddress) >= entryFee,
            "[Package.approvePackagePurchase] Insufficient user balance"
        );
        (
            uint256 votePortion,
            uint256 commentPortion,
            uint256 stakePortion,
            uint256 adminPortion
        ) = calCulatePackageDistribution(entryFee);

        // Distribute package fund
        balanceVault.decreaseBalance(_userAddress, entryFee);

        // Voting and Commenting
        balanceVault.increaseReward(votePortion + commentPortion);
        ...
        // Staking
        balanceVault.increaseBalance(address(this), stakePortion);
        balanceVault.withdrawUpo(stakePortion);
        token.approve(stakingAddress, stakePortion);
        staking.addPoolReward(stakePortion);

        // Admin
        balanceVault.increaseBalance(adminAddress, adminPortion);

        emit PackageApproved(
            _contestId,
            _projectId,
            _userAddress,
            votePortion,
    ...
    function calCulatePackageDistribution(uint256 entryFee)
        internal
        view
        returns (
            uint256 votePortion,
    ...
    {
        votePortion = (entryFee * votePercentage) / 100;
        commentPortion = (entryFee * commentPercentage) / 100;
        stakePortion = (entryFee * stakePercentage) / 100;
        // marketing + service fee + worker gas + leftover
        adminPortion = entryFee - (votePortion + stakePortion + commentPortion);
    }

    function getContestPackageDistribution(string memory _contestId)
        external
        view
        returns (
            uint256 votePortion,
    ...
    function setPackagePercentage(
        uint256 _votePercentage,
        uint256 _commentPercentage,
        uint256 _stakePercentage,
        uint256 _adminPercentage
    ) public onlyRole(WORKER) {
        require(
            (   
                _adminPercentage +
                _votePercentage +
            ...
            _votePercentage >= 0 &&
            _stakePercentage >= 0 &&
            _commentPercentage >= 0,
            "[Package.setPackagePercentage] Invalid package percentage"
        );
        votePercentage = _votePercentage;
        commentPercentage = _commentPercentage;
...
interface IVoting{
    function addContestEntry(string memory _contestId, string memory _projectId, uint256 _entryVoteReward, uint256 _entryCommentReward) external;
...
interface IStaking{
    function checkValidLevel(uint256 _type, address _userAddress) external view returns(uint256);

    function checkLevel(uint256 _stakeAmount) external view returns(uint256);

    function getLevel(address _userAddress) external view returns(uint256);
