...
contract ERC721WithPermit is ERC721, EIP712Base, AccessControl, Pausable {
  ...
  /**
   * @dev Emitted when an `approver` approved a token for transaction
   */
  event TokenApproval(address indexed tokenAddress, address indexed approver);

  /**
   * @dev Emitted when an `approver` revoke approval of token
   */
  event RevokeTokenApproval(address indexed tokenAddress, address indexed approver);
  ...
  function _setTokenUri(uint256 tokenId, string memory tokenUri) internal onlyRole(DEFAULT_ADMIN_ROLE) {
  ...
  /**
    * @dev Approve `tokenaddress` for transactions
    *
    * Emits an {TokenApproval} event.
    */
  function _approveToken(address _addressToApprove) internal onlyRole(DEFAULT_ADMIN_ROLE) {
      _approvedTokens[_addressToApprove] = true;
      emit TokenApproval(_addressToApprove, msg.sender);
  }

  /**
    * @dev Revoke `tokenaddress` approval
    *
    * Emits an {RevokeTokenApproval} event.
    */
  function _revokeTokenApproval(address _addressToRevoke) internal onlyRole(DEFAULT_ADMIN_ROLE) {
      _approvedTokens[_addressToRevoke] = false;
      emit RevokeTokenApproval(_addressToRevoke, msg.sender);
  }
  ...
  /**
    ...
    *
    */
  function _verifyToken(address _tokenAddress) internal view {
      require(_approvedTokens[_tokenAddress], "Artifract: TOKEN_NOT_ALLOWED");
  }
}
