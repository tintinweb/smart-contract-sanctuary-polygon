...
// function MessageViews(uint256 messageId) public returns (address[] memory) {
//     Message storage message = messages[messageId];
//     address[] storage viewedBy = message.viewedBy;
//     address[] memory sharedWith = message.sharedWith;

//     for (uint i = 0; i < sharedWith.length; i++) {
//         address sharedWithAddress = sharedWith[i];
//         if (!contains(viewedBy, sharedWithAddress)) {
//             viewedBy.push(sharedWithAddress);
//         }
//     }

//     address[] memory result = new address[](viewedBy.length);
//     for (uint i = 0; i < viewedBy.length; i++) {
//         result[i] = viewedBy[i];
//     }
