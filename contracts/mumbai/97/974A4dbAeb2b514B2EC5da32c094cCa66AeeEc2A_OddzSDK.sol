...
contract OddzSDK is IOddzSDK, BaseRelayRecipient, AccessControl {
     ...
     */
    uint256 public override minimumGaslessBetAmount;
    ...
    constructor(
        ...
        require(_oddzPrediction.length == _underlying.length, "");
    ...
    function predict(
        ...
        require(
            address(predictionMap[_underlying][_strike]) != address(0),
            "Prediction Market for the asset is not active"
        );
        ...
        if (directionUp) predictionMap[_underlying][_strike].predictUp{ value: msg.value }(roundId, userAddress_);
        else predictionMap[_underlying][_strike].predictDown{ value: msg.value }(roundId, userAddress_);
    }
    ...
    function predictWithGasless(
        ...
        if (directionUp) predictionMap[_underlying][_strike].predictUp{ value: msg.value }(roundId, msgSender());
        else predictionMap[_underlying][_strike].predictDown{ value: msg.value }(roundId, msgSender());
    }
