...
contract NFTAuction is Ownable, ReentrancyGuard {
    ...
    // uint256 public platformFeePercentage=136363636364;   // undecided
    ...
    */
    ...
    {   
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20
            ...
            uint256 ownerAmount= totalAmount.mul(_ownerPercentage).div(1000000000000);
            uint royaltyAmount;
            // Reset Sale Data
...
function _payFeesAndSeller(
        ...
        // uint256 platformFee= _highestBid.mul(_platformFeePercentage).div(1000000000000);
        uint256 platformFee = _platformFeePercentage;
        _highestBid = _highestBid.sub(platformFee);
        uint256 sellerAmount= _highestBid;
        uint256 ownerAmount= sellerAmount.mul(_ownerPercentage).div(1000000000000);
        
        sellerAmount= sellerAmount.sub(ownerAmount);

        ownerAmount = ownerAmount.add(platformFee);
        ...
        if(_isERC20Auction(erc20Token)){    // if sale is ERC20 
            ...
            // IERC20(erc20Token).transfer(owner, platformFee);
        }
        else{
    ...
    function takeHighestBid(address _nftContractAddress, uint256 _tokenId)
        ...
        onlyNftSeller(_nftContractAddress, _tokenId)
    {
        uint256 _platformFeePercentage = 0;
        require(
            _isABidMade(_nftContractAddress, _tokenId),
            "cannot payout 0 bid"
        );
        _transferNftAndPaySeller(_nftContractAddress, _tokenId, _platformFeePercentage);
    }

    function settleAuction(address _nftContractAddress, uint256 _tokenId , uint256 _platformFeePercentage)
        external
        nonReentrant
    ...
    function settleAuctionOnlyOwner(address _nftContractAddress, uint256 _tokenId, uint256 _platformFeePercentage)
        external
        onlyOwner
        nonReentrant
        isAuctionOver(_nftContractAddress, _tokenId)
