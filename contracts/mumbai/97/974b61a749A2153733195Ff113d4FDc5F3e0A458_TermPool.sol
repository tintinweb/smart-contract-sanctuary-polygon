...
interface ITermPool {
  ...
  struct Term {
    ...
    uint256 maxPoolSize;
    uint256 size;
    uint256 depositWindowFrom;
    uint256 depositWindowTo;
  ...
  event RewardTopUp(uint256 amount);
  ...
  function createTerm(
    uint256 _minPoolValue, // minimum amount of liquidity that must be provided
    uint256 _maxPoolValue, // maximum amount of liquidity that can be provided
    uint256 _depositWindowFrom, // time when the deposit window opens
    uint256 _depositWindowTo, // time when the deposit window closes
    uint256 _maturity, // timestamp when the borrower using cpTokens
  ...
  /// Does not return anything (either it's a success or it reverts) but should emit an event for the frontend.
  function topupReward(uint256 _amount) external;
  ...
  // function getAllActiveTerms() external view returns (uint256[] memory);
...
interface ITermPoolFactory {
  ...
  event TermPoolCreated(address indexed _pool, address indexed _currency);
  ...
  function tokenToPool(
    address
  ) external view returns (address addr, address currency, bool isListed);
  ...
  function getPools() external view returns (Pool[] memory pools);

  function createTermPool(address _cpToken) external returns (address pool);

  // function isPoolFactoryOwner(address _poolAdmin)
  //     external
  //     view
  //     returns (bool);
}
...
interface ITpToken is IERC20MetadataUpgradeable {
  ...
  function lock(address to, uint256 currencyAmount) external;
  ...
  function unlock(address from, uint256 currencyAmount) external;
...
import {TpToken, ITpToken, IERC20MetadataUpgradeable} from './TpToken.sol';
...
contract TermPool is ITermPool, Initializable, TermUtils {
  ...
  address public borrower; // or get it from permissionless factory??

  /// @notice Is pool listed in TermPoolFactory
  bool public isListed;

  /// @notice Available reward for all terms
  ...
  function setListed(bool _isListed) external onlyFactory {
    ...
    // TODO: emit ListingStatusChanged(_isListed);
  }

  /// @notice Creates new term
  /// @dev Callable only by borrower
  /// @param _minPoolSize Minimum term size
  /// @param _maxPoolSize Maximum term value
  /// @param _depositWindowFrom Deposit window start
  /// @param _depositWindowTo Deposit window end
  /// @param _maturity Maturity of the term
  /// @param _rewardFactor Reward factor of the term
  function createTerm(
    uint256 _minPoolSize,
    uint256 _maxPoolSize,
    uint256 _depositWindowFrom,
    uint256 _depositWindowTo,
    uint256 _maturity,
    uint256 _rewardFactor
  ) external onlyBorrower {
    if (_minPoolSize > _maxPoolSize) revert WrongBoundaries(_minPoolSize, _maxPoolSize);
    if (_depositWindowFrom > _depositWindowTo || _depositWindowTo < block.timestamp)
      revert WrongBoundaries(_depositWindowFrom, _depositWindowTo);
    if (_maturity == 0) revert ValueIsZero(_maturity);
    if (_rewardFactor == 0) revert ValueIsZero(_rewardFactor);
    ...
    // how it works at Permissionless facory
    // _symbol = factory.getPoolSymbol(address(currency), address(manager));
    // __ERC20_init(string(bytes.concat(bytes("Pool "), bytes(_symbol))), "");

    // what we gets
    // name   == Pool cpAUR-USDC
    // symbol == cpAUR-USDC

    // TODO: make it prettier
    bytes memory cpSymbol = bytes(IERC20MetadataUpgradeable(cpToken).symbol());
    cpSymbol[0] = 't';
    string memory tpSymbol = string(bytes.concat(cpSymbol, hex'2d', uint2bytes(termId))); // tpAUR-USDC-0
    string memory tpName = string(bytes.concat(bytes('Term Pool '), cpSymbol)); // Term Pool tpAUR-USDC-0
    uint8 tpDecimals = IERC20MetadataUpgradeable(cpToken).decimals();

    TpToken(tpTokenAddr).__TpToken_init(tpName, tpSymbol, tpDecimals);
  ...
  function lock(uint256 _termId, uint256 _amount) public existingTerm(_termId) {
    // no borrower validation?
    Term storage term = terms[_termId];

    if (term.depositWindowFrom > block.timestamp || term.depositWindowTo < block.timestamp)
      revert NotInDepositWindow(term.depositWindowFrom, term.depositWindowTo, block.timestamp);
    if (term.status == TermStatus.Cancelled) {
      revert TermIsNotActive(_termId);
    }
    if (term.maxPoolSize < term.size + _amount)
      revert MaxPoolSizeOverflow(term.maxPoolSize, term.size);

    term.size += _amount;

    if (term.status == TermStatus.Created) {
      _changeStatus(_termId, TermStatus.Opened);
      // should change status twice if term is filled in one tx
    }
    if (term.size == term.maxPoolSize) {
      _changeStatus(_termId, TermStatus.Filled);
    }

    // TODO: concat two methods into tpToken.lock
    IERC20MetadataUpgradeable(cpToken).transferFrom(msg.sender, address(this), _amount);
    ITpToken(term.tpToken).lock(msg.sender, _amount);
  ...
  /// @param _termId Id of the term
  function unlock(uint256 _termId) public existingTerm(_termId) {
    Term storage term = terms[_termId];

    if (block.timestamp < term.maturityEnds)
      revert NotEndedMaturity(term.maturityEnds, block.timestamp);

    uint256 balance = ITpToken(term.tpToken).balanceOf(msg.sender);

    if (balance == 0) revert ZeroAmount();
    // TODO: modify tpToken to unlock with reward or not. Move this logic to tpToken
    ITpToken(term.tpToken).unlock(msg.sender, balance);

    if (term.status == TermStatus.Cancelled) {
      IERC20MetadataUpgradeable(cpToken).transfer(msg.sender, balance);
      emit LiquidityRedeemed(msg.sender, _termId, balance);
    } else {
      uint256 reward = (balance * term.rewardFactor) / MULTIPLIER;

      if (reward > availableReward) revert NotEnoughReward();
      unchecked {
      ...
      IERC20MetadataUpgradeable(cpToken).transfer(msg.sender, totalAmount);

      term.size -= balance;
      if (term.size == 0) {
        _changeStatus(_termId, TermStatus.Completed);
  ...
  function topupReward(uint256 _amount) external onlyBorrower {
    availableReward += _amount;

    IERC20MetadataUpgradeable(cpToken).transferFrom(msg.sender, address(this), _amount);

    emit RewardTopUp(_amount);
  }

  /// @notice Withdraws the reward from the contract
  /// @dev Only borrower can call this function
  /// @param _amount Amount of tokens to withdraw
  function withdrawReward(uint256 _amount) external onlyBorrower {
    if (_amount > availableReward) revert NotEnoughReward();

    IERC20MetadataUpgradeable(cpToken).transfer(msg.sender, _amount);
  ...
  function rewardOf(
    ...
    if (balance > 0) {
      uint256 rewardPerMaturity = (balance * term.rewardFactor) / MULTIPLIER;
      return rewardPerMaturity;
    } else {
      return 0;
    }
  }
  ...
  function cancelTerm(uint256 _termId) external existingTerm(_termId) onlyBorrower {
    Term storage term = terms[_termId];
    // TODO: add stop term conditions
    if (term.status == TermStatus.Created) {
      term.status = TermStatus.Cancelled;

      activeTermsIndex.remove(_termId);
      // TODO emit TermCancelled
    } else {
  ...
  /// @notice Transfers the borrower role to another address
  /// @dev Only borrower can call this function
  function transferBorrowerRole(
    address _newBorrower
  ) external onlyBorrower notZeroAddr(_newBorrower) notSameAddr(_newBorrower, borrower) {
    // todo: add event
    // todo: ?? remove this function, parse borrower address from the permissionless
...
contract TpToken is ITpToken, ERC20Upgradeable {
  ...
  function lock(address to, uint256 _amount) external onlyTermPool {
    _mint(to, _amount);
  }

  /// @notice Burns the tokens according to provided cpToken amount
  function unlock(address from, uint256 _amount) external onlyTermPool {
    _burn(from, _amount);
  }
