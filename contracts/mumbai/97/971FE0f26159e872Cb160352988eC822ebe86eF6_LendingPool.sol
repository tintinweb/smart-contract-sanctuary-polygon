...
abstract contract BaseParaSwapAdapter is FlashLoanReceiverBase, Ownable {
    ...
    constructor(ILendingPoolAddressesProvider addressesProvider) public FlashLoanReceiverBase(addressesProvider) {
        ...
        for (uint256 i = 0; i < reserves.length; i++) {
            ...
            try IAToken(baseData.aTokenAddress).getIncentivesController() returns (
                ...
                if (address(aTokenIncentiveController) != address(0)) {
                    ...
                    aUserIncentiveData.userUnclaimedRewards = aTokenIncentiveController.getUserUnclaimedRewards(user);
            ...
            try IVariableDebtToken(baseData.variableDebtTokenAddress).getIncentivesController() returns (
                ...
                if (address(vTokenIncentiveController) != address(0)) {
                    ...
                    vUserIncentiveData.userUnclaimedRewards = vTokenIncentiveController.getUserUnclaimedRewards(user);
            ...
            try IStableDebtToken(baseData.stableDebtTokenAddress).getIncentivesController() returns (
                ...
                if (address(sTokenIncentiveController) != address(0)) {
                    ...
                    sUserIncentiveData.userUnclaimedRewards = sTokenIncentiveController.getUserUnclaimedRewards(user);
...
contract CollectorController is Ownable {
   ...
   */
  constructor(address owner, address collectorProxy) public {
...
contract Collector is VersionedInitializable, ICollector {
  ...
  uint256 public constant override REVISION = 1;
  ...
  function getFundsAdmin() external view override returns (address) {
    return _fundsAdmin;
  }
...
contract UniswapPriceOracle is IPriceOracle {
    ...
    constructor(
        ...
        BASE_CURRENCY_UNIT = 10**(decimals - 1);
    }

    function getAssetPrice(address baseAsset) external view override returns (uint256) {
        address quoteAsset = _quoteAsset; // gas saver
        if (baseAsset == quoteAsset) {
            return BASE_CURRENCY_UNIT;
        }

        IUniswapV3Pool pool = IUniswapV3Pool(IUniswapV3Factory(_uniswapFactory).getPool(baseAsset, quoteAsset, _fee));
        (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();

        // uint result = uint(sqrtPriceX96).mul(uint(sqrtPriceX96)).mul(BASE_CURRENCY_UNIT) >> (96 * 2);
        uint256 result = uint256(sqrtPriceX96).mul(uint256(sqrtPriceX96)).mul(BASE_CURRENCY_UNIT) >> (96 * 2);

        // if(baseAsset < quoteAsset && result != 0) {
        //   result = (BASE_CURRENCY_UNIT).div(result);
        // }
    ...
    function setAssetPrice(address asset, uint256 price) external override {
        revert("Asset price is not updateable, it is retrieved from Uniswap DEX");
    }
}
...
contract FlashLiquidationAdapter is BaseUniswapAdapter {
    ...
    function _decodeParams(bytes memory params) internal pure returns (LiquidationParams memory) {
        (address collateralAsset, address borrowedAsset, address user, uint256 debtToCover, bool useEthPath) = abi
            .decode(params, (address, address, address, uint256, bool));
