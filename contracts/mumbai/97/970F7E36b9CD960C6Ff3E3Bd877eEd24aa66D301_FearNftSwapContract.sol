...
import "./BaseExchangeContract.sol";
import "./ExchangeStructs.sol";

interface IFearNft1155 {
...
/**
 * @dev Contract implementing Nft Swapping for Global Fear Exchange.
 */
contract FearNftSwapContract is BaseExchangeContract {
    using SafeMath for uint256;

    constructor(string memory _name) BaseExchangeContract(_name) {
    }

    function _transferInAssets(string memory positionName, address from, address /*to*/, uint256 amount) internal override {
        ...
        for (uint i = 0; i < position.inAssets.length; i++) {
            ...
            IOldFearNft oldNftContract = IOldFearNft(asset.assetAddress);
            oldNftContract.burn(from, asset.id, amount.mul(asset.amount));
        }
    }

    function _transferOutAssets(string memory positionName, address /*from*/, address to, uint256 amount) internal override {
        ExchangePosition storage position = exchangePositionsByName[positionName];

        for (uint i = 0; i < position.outAssets.length; i++) {
            AssetInfo memory asset = position.outAssets[i];

            require(asset.assetType == AssetType.ERC1155);

            IFearNft1155 newNtfContract = IFearNft1155(asset.assetAddress);
            newNtfContract.initialMint(to, asset.id, amount.mul(asset.amount), "");
        }
    }
}
...
abstract contract BaseExchangeContract is AccessControl, Pausable, Withdrawable, IExchangeContract {
    ...
    function exchange(address user, string memory positionName, uint256 amount) payable external whenNotPaused {
