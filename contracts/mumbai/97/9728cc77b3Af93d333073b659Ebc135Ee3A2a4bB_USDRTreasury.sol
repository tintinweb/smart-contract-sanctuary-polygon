...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
        ...
        )
    {
        ...
        liquidity = ILiquidityManager(liquidityManager).liquidity();
        {
            (uint256 tngblAmount, uint256 underlyingAmount) = ILiquidityManager(
                tngblLiquidityManager
            ).getTokenAmounts();
            tngblLiquidity =
                (tngblAmount * IPriceOracle(oracle).quote(10**tngblDecimals)) /
                (10**tngblDecimals) +
                underlyingAmount;
        }
        debt =
