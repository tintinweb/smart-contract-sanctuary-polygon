...
contract WitnetTest {
    ...
    function fetchRandomNumber(uint32 max) public {
        assert(latestRandomizingBlock > 0);
        randomness = witnet.random(max, nonce, latestRandomizingBlock);
        nonce++;
    }

    function estimateRandomizeFee(uint256 gasPrice) external {
        uint256 fee = witnet.estimateRandomizeFee(gasPrice);
        emit Fee(fee);
    }

    function randomTest(uint32 nTest) public {
        for (uint256 i = 0; i < nTest; i++) {
            fetchRandomNumber(nTest);
            result[randomness] = result[randomness]++;
        }
    }
}