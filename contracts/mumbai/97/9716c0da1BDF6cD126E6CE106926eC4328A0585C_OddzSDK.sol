...
contract OddzSDK is IOddzSDK, BaseRelayRecipient, AccessControl {
    ...
    function predict(
        ...
        IOddzPrediction oddzPrediction = predictionMap[_underlying][_strike];
        ...
        if (address(msg.sender).isContract()) {
            ...
            require(msg.sender == userAddress, "Buyer and msg.sender cannot be different");
            userAddress_ = msg.sender;
        }

        if (directionUp) oddzPrediction.predictUp{ value: msg.value }(roundId, userAddress_);
        else oddzPrediction.predictDown{ value: msg.value }(roundId, userAddress_);
    }
    ...
    function predictWithGasless(
        ...
        if (directionUp) oddzPrediction.predictUp{ value: msg.value }(roundId, msgSender());
        else oddzPrediction.predictDown{ value: msg.value }(roundId, msgSender());
    }

    function setMinimumGaslessBetAmount(uint256 _amount) external onlyTimeLocker(msg.sender) {
        require(_amount >= 0.1 ether && _amount < 2 ether, "invalid minimum gasless premium");
        minimumGaslessBetAmount = _amount;
    }

    function allocateOddzReward(uint256 _amount) external override {}

    function distributeReward(address[] memory _providers, uint256 _month) external override {}
}
