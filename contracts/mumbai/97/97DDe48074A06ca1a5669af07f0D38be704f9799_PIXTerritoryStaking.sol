...
contract PIXTerritoryStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable {
    ...
    function setRewardToken(address rewardTokenAddress) external onlyRewardDistributor {
     ...
     */
    function addEpochRewards(uint256 epochTotalReward, uint256 epochDuration) external onlyRewardDistributor {
        ...
        for(uint256 size=0; size<=uint256(type(IPIX.PIXSize).max); size++) {
            ...
            for(uint256 category=0; category<=uint256(type(IPIX.PIXCategory).max); category++) {
                ...
                rewardForPool += 
                    (epochTotalReward * rewardPool.rewardPercentage) / rewardPercentagesDenominator;
     ...
     */
    function earnedReward(uint256 tokenId) public view returns(uint256) {
        IPIX.PIXInfo memory info = IPIX(pixNFT).getInfo(tokenId);
        RewardPool storage rewardPool = rewardPools[info.size][info.category];
        uint256 totalReward = 
           rewardPool.totalPoolRewardPerToken + _rewardPerTokenFromLastUpdate(rewardPool);
     ...
     */
    function earnedByAccount(address account) public view returns (uint256) {
        uint256 reward = 0;
