...
interface ITree{
    ...
    function mint(uint8 _numberToMint, uint8 _cause, uint256 _treeValue, address _user)external;
    function fertilise(uint256 _amount, uint256 _tokenId)external;
    function addValue(uint256 _valueToAdd, uint256 _tokenId, uint8 _causeId)external;
    function dataReturn(uint256 _tokenId)external view returns(uint8, uint256, uint8);
...
interface ICauses {
    ...
    function contractSentToCause(uint8 _index, address _token, uint256 _amount)external;
    function causeDataCall(uint8 _index) external view returns(bool, address);
...
contract Minter is ReentrancyGuard, Ownable{
    ...
    struct UsableTokens{
        address token;
        uint256 treeCost;
        uint256 fertilserValue;
    }

    constructor(){
        tree = ITree(0xD9F17513756aD89b1e3f59A18fb61D95e22CF276);
        cause = ICauses(0x3b2D6013c7c3Ee116eaf03DdA24d9a8dA48dF43F);
        reserve = msg.sender;
        tokens.push();
    }
    ...
    */
    ...
    function mintTree(uint8 _numberToMint, uint8 _cause, uint256 _valueToSpend, address _token) external nonReentrant{
        ...
        require(token.balanceOf(user)>= _valueToSpend, "Error: Insufficient Funds");
        ...
        uint256 causeValue = 9 * _valueToSpend/10;
        token.safeTransferFrom(user, reserve, _valueToSpend/10);
        token.safeTransferFrom(user, c, causeValue);

        cause.contractSentToCause(_cause, _token, causeValue);
    ...
    function fertiliseTree(uint256 _amount, uint256 _tokenId, address _token)public nonReentrant{
        ...
        require(tokenToUse(_token), "Error: Token not approved.");
        (uint8 cId,,) = tree.dataReturn(_tokenId);
        (,address cA) = cause.causeDataCall(cId);
        UsableTokens memory a = tokens[tokenLocation[_token]];
        IERC20 token = IERC20(_token);

        uint256 toSpend = (_amount * a.fertilserValue)/10;
        address user= _msgSender();

        require(toSpend*10 <= token.balanceOf(user), "Error: Need more BTRC");
        token.safeTransferFrom(user, cA, toSpend*9);
        token.safeTransferFrom(user, reserve, toSpend);
        cause.contractSentToCause(cId, _token, toSpend*9);       

        tree.fertilise(_amount, _tokenId);

    }

    function addTreeValue(address _token, uint256 _amount, uint256 _tokenId, uint8 _causeID)public nonReentrant{
        require(tokenToUse(_token), "Error: Token not approved.");
        address user = _msgSender();
        UsableTokens memory a = tokens[tokenLocation[_token]];
        IERC20 token = IERC20(_token);
        require(token.balanceOf(user)>= _amount, "Error: Insufficient Funds");
        uint256 addValueTo = (10 *_amount) / a.treeCost;
        uint256 causeValue = 9 * (_amount / 10);
        (uint8 b,,) = tree.dataReturn(_tokenId);
        if(b == 0){
            b = _causeID;
        }
        ...
        token.safeTransferFrom(user, _cause, causeValue);
        token.safeTransferFrom(user, reserve, _amount/10);

        cause.contractSentToCause(b, _token, causeValue);
