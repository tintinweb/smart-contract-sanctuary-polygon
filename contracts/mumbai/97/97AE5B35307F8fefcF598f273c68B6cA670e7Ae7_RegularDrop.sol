...
contract RegularDrop is ERC1155Drop {
    ...
    /// @dev Call lazyMint function and register items
    function callLazyMint(
        uint256 _amount,
        string calldata _baseURIForTokens,
        uint256 _price,
        bytes calldata _data
    ) external onlyOwner returns (uint256 batchId) {
        items[totalTraits] = Item({
            supply: _amount,
            price: _price,
            baseURI: _baseURIForTokens
        });
        totalTraits++;
        return lazyMint(_amount, _baseURIForTokens, _data);
    }
    ...
    function _transferTokensOnClaim(
        ...
        // Store the timestamp when the token is claimed
        claimedAt[_to][_tokenId] = block.timestamp;
        // Transfer the NFTs being claimed
        _mint(_to, _tokenId, _quantityBeingClaimed, "");
    }
