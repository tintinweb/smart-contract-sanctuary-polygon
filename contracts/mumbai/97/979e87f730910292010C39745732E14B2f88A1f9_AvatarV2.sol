...
contract AvatarV2 is AvatarBase {
    ...
    function dress(Part[] calldata partsOn, bytes32[] calldata partsOff)
        ...
        override
    {
        IDavaV2 davaCore = IDavaV2(dava());
        davaCore.refreshToken(_props().davaId);
        davaCore.deregisterCombinationData(_props().davaId);

        super.dress(partsOn, partsOff);

        davaCore.registerCombinationData(_props().davaId, allParts());

        emit Dress(msg.sender, uint256(_props().davaId), partsOn, partsOff);
    }
    ...
    function externalImgUri() public view override returns (string memory) {
        ...
        for (uint256 i = 0; i < frames.length; i += 1) {
            queries[i] = URICompiler.Query(
                ...
                string(
                    abi.encodePacked(
                        (uint256(uint160(frameCollection)) >> (8 * 16))
                            .toHexString(4),
                        "[",
        ...
        for (uint256 i = 0; i < parts.length; i += 1) {
            if (parts[i].collection != address(0x0)) {
                queries[layerAmount] = URICompiler.Query(
                    ...
                    string(
                        abi.encodePacked(
                            (uint256(uint160(parts[i].collection)) >> (8 * 16))
                                .toHexString(4),
                            "[",
...
abstract contract AvatarBase is MinimalProxy, IAvatar, Account {
    ...
    function owner() public view override returns (address) {
        return IDavaV2(dava()).ownerOf(_props().davaId);
    }
    ...
    {
        IWearable _wearable = IWearable(dava());
        bytes32[] memory allCategories = _wearable.getAllSupportedCategories();
    ...
    function _putOn(Part memory part_) internal {
        require(
            _isEligible(part_),
            "AvatarBase: avatar does not own this part"
        );
