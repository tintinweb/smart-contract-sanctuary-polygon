...
contract BOMNFT is ERC721URIStorage, Ownable, BlackList {
    ...
    function mint(string memory _tokenURI, uint id) public payable returns(uint tokenID) {
        ...
        if(msg.sender == owner()) {}
            ...
            require(msg.value >= maticCost, "Matic Amount not enough");
    ...
    function mintNFTs(uint256 _count, string memory _tokenFolderURI, uint id, uint _mode) public payable returns(uint startingTokenID){
        require(!paused);
        ...
        //     require(msg.value >= nMaticCost, "Matic Amount not enough");
        ...
        if(_mode == 0) {
            for(uint _index = 0; _index < _count; _index++) {
                ...
                _tokenID = supply;
                _mint(msg.sender, _tokenID);
                _setTokenURI(_tokenID, _tokenFolderURI);
            }
        } else {
            // for(uint _index = 0; _index < _count; _index++) {
            //     supply++;
            //     _tokenID = supply;
            //     _mint(msg.sender, _tokenID);
            //     _setTokenURI(_tokenID, string(abi.encodePacked(_tokenFolderURI, _index)));
            // }
        }
