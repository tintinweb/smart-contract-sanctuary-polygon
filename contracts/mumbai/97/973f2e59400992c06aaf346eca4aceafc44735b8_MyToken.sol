...
contract MyToken is ERC721
{
    ...
    mapping(uint256=>NFTDetail) userNFTDetails;
    ...
    event CreateNft(address to,uint256 nftId);
    ...
    ) external 
    {
        ...
        userNFTDetails[tokenId] = NFTDetail(_to,_link,_copies);
        emit CreateNft(_to,tokenId);
    } 
    ...
    {
      require(userNFTDetails[tokenid].userAddress == msg.sender,"You are not owner");
    ...
    function buynft(uint256 tokenid) external payable
    {
        ...
        require(msg.value>=totalAmount,"less than expected amount");
        transferBNB(userNFTDetails[tokenid].userAddress,msg.value); 
        _transfer(address(this),msg.sender,tokenid);
        nftprice[tokenid].price  = 0;
        nftprice[tokenid].inlist = false;
    }
     ...
     function claim(uint256 tokenid) external
    {
        ...
        if(finalowner[tokenid] == msg.sender)
        {
            ...
            address firstowner    = userNFTDetails[tokenid].userAddress;
            (bool success,)  = firstowner.call{value: totalamount}("");
            require(success, "refund failed");
            _transfer(address(this),msg.sender,tokenid);
        }
    }
    ...
    {
        require(userNFTDetails[tokenid].userAddress == msg.sender,"You are not originalowner");
    ...
    function transferBNB(address _address,uint256 _amount) 
      internal
    {
    ...
    {
        return userNFTDetails[_tokenId];
    } 
