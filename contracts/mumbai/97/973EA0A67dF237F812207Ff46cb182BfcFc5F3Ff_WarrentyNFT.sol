...
contract WarrentyNFT is ERC721, ERC721URIStorage{
    ...
    function getTokenUri(string memory _name, string memory _serial, string memory _product) private pure returns(string memory){
        string memory json = Base64.encode(
            bytes(string(
                abi.encodePacked(
                    '{"name": "', _name, '",',
                    '"image_data": "', generateCharacter(), '",',
                    '"attributes": [{"trait_type": "Serial", "value": ', _serial, '},',
                    '{"trait_type": "Product Type", "value": ', _product, '},',
                    ']}'
                )
            ))
        );
    ...
    function mint(string memory _name, string memory _serial,string memory _product) public {
        uint256 unique_id = block.timestamp;
        _safeMint(msg.sender,unique_id);
        _setTokenURI(unique_id,getTokenUri(_name,_serial,_product));
    }
