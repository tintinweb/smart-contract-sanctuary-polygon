...
//(_, () /\ ~|~   (_, /\ |\| (_,
contract GoatGang is ERC721AQueryable, Ownable, ReentrancyGuard, PullPayment, PaymentSplitter {
	using Address for address;
	using Strings for uint256;

	string public _contractBaseURI = "https://api.goatgang.io/metadata/";
	string public _contractURI = "https://api.goatgang.io/contract_uri";

	//prices
	uint256 public tokenPrice = 0.01 ether; //normal price without affiliate link
	uint256 public tokenPriceWithAffiliateLink = 0.095 ether; //5% discount with affiliate link
	uint256 public comissionPerMintAffiliate = 0.001 ether; //normal affiliate gets 10%
	...
	constructor() ERC721A("Goat Gang", "GoatG") PaymentSplitter(addressList, shareList) {
		_mint(msg.sender, 20);
	}
	...
	/**
	 * @dev normal afiliate buy
	 */
	function buyWithReferral(uint256 qty, address refferedBy) external payable nonReentrant {
		require(msg.sender != refferedBy, "can't be yourself");
		require(tokenPriceWithAffiliateLink * qty == msg.value, "exact amount needed");
		require(qty < 6, "max 5 at once");
		require(totalSupply() + qty <= maxSupply, "out of stock");
		require(block.timestamp > publicSaleStartTime, "not live");

		_asyncTransfer(refferedBy, qty * comissionPerMintAffiliate);
		_mint(msg.sender, qty);
	}
	...
	/**
	 * @dev paid usage outside the blockchain
	 */
	...
	//can decrease the max supply if it doesn't sell, used only by DAO vote
