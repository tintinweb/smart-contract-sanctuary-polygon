...

{
    ...
    (
        ...
        /**
        * @dev sets the default NFT template
        **/
        metadata[0].title = attrs[0];
        metadata[0].description = attrs[1];
        metadata[0].collectible = attrs[2];
        metadata[0].collection = attrs[3];
        metadata[0].partner = attrs[4];
        metadata[0].rarity = attrs[5];
        metadata[0].signed = attrs[6];
        metadata[0].image_url = attrs[7];
        metadata[0].animation_url = attrs[8];
        // metadata[0].external_url = attrs[9];
    }

    /**
    * @dev mints quantity of tokens and transfers
    * them to each recipient in the array passed
    * mint ERC2309 more gas savings
    */
    function mint
    (
        ...
        payable
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            /**
            * @dev assigns the default NFT template until dynamic rule applies
            **/
            metadata[newItemId].title = metadata[0].title;
            metadata[newItemId].description = metadata[0].description;
            metadata[newItemId].collectible = metadata[0].collectible;
            metadata[newItemId].collection = metadata[0].collection;
            metadata[newItemId].partner = metadata[0].partner;
            metadata[newItemId].rarity = metadata[0].rarity;
            metadata[newItemId].signed = metadata[0].signed;
            metadata[newItemId].image_url = metadata[0].image_url;
            metadata[newItemId].animation_url = metadata[0].animation_url;
            // metadata[newItemId].external_url = metadata[0].external_url;

            uint256[] memory _tokensOfOwner = this.tokensOfOwner(address(this));

            for (uint256 j = 0; j < _tokensOfOwner.length; j++) {
                this.safeTransferFrom(address(this), recipients[j], _tokensOfOwner[j]);
            }
    ...
    /**
    * @dev Optional set metadata by tokenId
    */
    ...
    {
        require(hasRole(ADMIN_ROLE, _msgSender()), "AutographDynamic: must have admin role");

        metadata[tokenId].title = attrs[0];
        metadata[tokenId].description = attrs[1];
        metadata[tokenId].collectible = attrs[2];
        metadata[tokenId].collection = attrs[3];
        metadata[tokenId].partner = attrs[4];
        metadata[tokenId].rarity = attrs[5];
        metadata[tokenId].signed = attrs[6];
        metadata[tokenId].image_url = attrs[7];
        metadata[tokenId].animation_url = attrs[8];
        // metadata[tokenId].external_url = attrs[9];

        return true;
    }

    /**
    * @dev concats 2 string params svg helper function
    */
    ...
    */
        ...
        returns (string memory)
    {
        ...
        string memory name = concatenate(metadata[tokenId].title, tokenId.toString());
        string memory description = metadata[tokenId].description;

        string memory image_url = metadata[tokenId].image_url;
        string memory metaData = generateMetadata(tokenId);

        string memory animation_url = metadata[tokenId].animation_url;
        // string memory external_url = metadata[tokenId].external_url;
    ...
    */
        ...
        returns(string memory)
    {
        ...
        /** @dev if we want to have separate metadata by tokenId
        * we can pass the tokenId here and use the metadata struct
        * _tokenId
        **/
        ...
        (
            abi.encodePacked(
                '[',
                '{"trait_type":"Collectible", "value":"', metadata[_tokenId].collectible, '"},',
                '{"trait_type":"Collection", "value":"', metadata[_tokenId].collection, '"},',
                '{"trait_type":"Partner", "value":"', metadata[_tokenId].partner, '"},',
                '{"trait_type":"Rarity", "value":"', metadata[_tokenId].rarity, '"},',
                '{"trait_type":"Signed", "value":"', metadata[_tokenId].signed, '"}',
                ']'
            )
        );
    }
    ...
    (
        ...
        virtual
    {
        require(hasRole(MINTER_ROLE, _msgSender()), "AutographDynamic: must have minter role to set token uri");
    ...
    */
    ...
    {
        IERC721AQueryableUpgradeable qtoken = IERC721AQueryableUpgradeable(address(this));
        uint256[] memory _tokensOfOwner = qtoken.tokensOfOwner(_owner);
...
abstract contract ERC721A__Initializable {
    ...
    function _isConstructorA() private view returns (bool) {
