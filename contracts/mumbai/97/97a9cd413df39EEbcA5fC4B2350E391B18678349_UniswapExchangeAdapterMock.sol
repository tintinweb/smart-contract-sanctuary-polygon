...
contract UniswapExchangeAdapterMock is UniswapExchangeAdapter {

    constructor (IUniswapV2Factory factory_, IUniswapV2Router02 router_)
    UniswapExchangeAdapter(factory_, router_)
    {}

    function exchangeTokensMock(Erc20Asset from, uint256 fromAmount, Erc20Asset to, uint256 toAmount) public view returns(bytes memory) {
        return exchangeTokens(from, fromAmount, to, toAmount);
    }

    function exchangeTokenForNativeMock(Erc20Asset from, uint256 fromAmount, NativeAsset to, uint256 toAmount) public view returns(bytes memory) {
        return exchangeTokenForNative(from, fromAmount, to, toAmount);
    }

    function exchangeNativeForTokenMock(NativeAsset from, uint256 fromAmount, Erc20Asset to, uint256 toAmount) public view returns(bytes memory) {
        return exchangeNativeForToken(from, fromAmount, to, toAmount);
    }
}
