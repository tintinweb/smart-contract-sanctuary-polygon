...
contract EventMarket {
    ...
    function buyTicket(uint256 ticketId) public payable {
        require(Tickets[ticketId].isPresent, "Ticket is not present");
        require (Tickets[ticketId].isSold == false, "Item has already been sold");
        require(Tickets[ticketId].price == msg.value,
        "Incorrect amount payed");

        Tickets[ticketId].isSold = true;
        Tickets[ticketId].owner = payable(msg.sender);

        IERC721(Tickets[ticketId].nftContractAddress).transferFrom(
            address(this),
            msg.sender,
    ...
    function changeListingPrice(uint256 newPrice) public {
        ...
        require(msg.sender == owner, "Only the owner can change listing price");
