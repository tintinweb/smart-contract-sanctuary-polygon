...
contract Factory is Initializable, Ownable, IFactory {
    ...
    function deploy(
        ...
        address instance = nft.cloneDeterministic(
            _getSalt(creator, nonces[creator], totalSupply, name)
        );
        ...
        nonces[creator] += 1;
        emit NewNFTCollection(creator, instance, totalSupply, name);
    }
    ...
    function getFutureAddress(
        ...
        return
            nft.predictDeterministicAddress(
                _getSalt(creator, nonces[creator], totalSupply, name)
            );
    }
