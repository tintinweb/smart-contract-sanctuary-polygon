...
contract LullaboysPass is ERC2981, Pausable {
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    IERC20 private lullacoin;
    uint256 private limitMint = 0;
    uint256 private cost = 0.01 ether;
    string private _URI;
    uint256 public total = 250;

    constructor(string memory uri, address token, address originals) ERC2981("Lullaboys Pass", "LBP", originals ) {
        lullacoin = IERC20(token);
        _URI = uri;
        pause();
    }
    ...
    function setMaxMint(uint256 newLimit) public onlyOwner {
        require(limitMint < newLimit, "Limit is too low");
        limitMint = newLimit;
    }
    ...
    function contractURI() public pure returns (string memory) {
        return "https://invisiblefriends.io/api/contracts/InvisibleFriends";
    }

    function whitelistMint() public payable {
        uint256 tokenId = _tokenIdCounter.current();
        uint256 saletoken = lullacoin.balanceOf(msg.sender);
        require(tokenId < limitMint, "Mint not available");
        require(saletoken > 0, "Not in whitelist");
        uint256 toUse = (saletoken > 10) ? 10 : saletoken;
        require(msg.value >= (cost - (toUse * 0.001 ether)), "Not enough Ether");

        _tokenIdCounter.increment();
        lullacoin.transferFrom(msg.sender,owner(), saletoken);
        _unpause();
