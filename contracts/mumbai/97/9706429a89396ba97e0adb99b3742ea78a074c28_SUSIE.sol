...
contract SUSIE is Context, ERC20, ERC20Burnable , Ownable {
    using SafeMath for uint256;
    ...
    constructor (address _router, address _beth, address _btcb, address _gala, DividendTracker1 _forGALA, DividendTracker1 _forBTCB, DividendTracker1 _forBETH) ERC20("SUSIE Token","SUSIE") {
    ...
    function getTransferAmounts(uint256 _amount) private returns (uint256, uint256) {
        ...
        console.log(forDividends, forBTCB, forBETH, forGALA);
    ...
    function _transfer(
        ...
        require(canTransferMoreThanAllowed[from] || amount <= maxTxAmount,"SUSIE: transfer more than allowed amount");
        if (amount == 0) {
        ...
        require(canHoldMoreThanAllowed[to] || balanceOf(to) <= maxBalance,"SUSIE: Balance exceed max balance");
    ...
    function swapAndSendDividends(uint256 _flag, uint256 tokens) private {
        if(_flag == 0){
            ...
            console.log("swapTokenforBTCB", dividends);
            ...
            console.log("swapTokenforBETH", dividends);
            ...
            console.log("swapTokenforGALA", dividends);
    ...
    function updateDividendTrackerGALA(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerGALA), "SUSIE: The dividend tracker already has that address");

        DividendTracker1 newDividendTracker = DividendTracker1(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "SUSIE: The new dividend tracker must be owned by the HoneyPad token contract");
    ...
    function updateDividendTrackerBETH(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerBETH), "SUSIE: The dividend tracker already has that address");
    ...
    function updateDividendTrackerBTCB(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerBTCB), "SUSIE: The dividend tracker already has that address");
