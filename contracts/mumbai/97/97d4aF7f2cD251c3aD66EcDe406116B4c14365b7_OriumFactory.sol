...
contract OriumFactory is Initializable, AccessControlUpgradeable {
    ...
    /**
     * @notice set alchemica tokens of aavegotchi
     * @dev only the operator can set alchemica tokens of aavegotchi
     * @param _tokens alchemica tokens of aavegotchi
     */
    function setPlatformTokens(uint256 _platform, address[] memory _tokens)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
     ...
     */
    function addPlatform(uint256 _platform, address _vaultImplementation)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function removePlatform(uint256[] calldata _platform) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _platform.length; i++) {
            ...
            platformToNftVaultImplementation[_platform[i]] = address(0);
