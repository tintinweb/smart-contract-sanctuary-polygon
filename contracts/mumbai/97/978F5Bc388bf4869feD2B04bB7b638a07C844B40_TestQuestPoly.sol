...
contract QuestPoly is KeeperCompatibleInterface { // proxy implementation
    ...
    uint256 public _perfLastActionId;
    ...
    function performLeaderboard(
        uint256 performId_, uint256 lastActionId_, uint88 lastActionTimestamp_,
        ...
        require(
            (_perfLastActionId >= lastActionId_ && _perfLastActionId <= _currentActionId)
            && (_perfLastActionTimestamp >= lastActionTimestamp_ && _perfLastActionTimestamp < timestamp)
            && performId_ == _currentPerformId+1,
            "invalid last perf"
        );
        bytes32 hash = keccak256(abi.encodePacked(performId_, lastActionId_, lastActionTimestamp_, msg.sender));
        address signer = hash.toEthSignedMessageHash().recover(signature);
        ...
        _currentPerformId++;
        _perfLastActionId = lastActionId_;
...
contract TestQuestPoly is QuestPoly {
    ...
    function checkUpkeep(bytes calldata /* checkData */) external override virtual returns (bool upkeepNeeded, bytes memory /* performData */) {
        upkeepNeeded = block.timestamp >= (((lastLeaderboardTimestamp / 24 hours) + 1) * 1 hours);
        // We don't use the checkData in this example. The checkData is defined when the Upkeep was registered.
    }
}