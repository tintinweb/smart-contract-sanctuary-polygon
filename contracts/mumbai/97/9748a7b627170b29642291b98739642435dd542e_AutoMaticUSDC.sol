...
contract AutoMaticUSDC is Context, Ownable, IERC20 {
    using SafeMath for uint256;
	using SafeERC20 for IERC20;
    
    uint8 private _decimals = 6;
    string private _symbol = "CUT";
    string private _name = "Capital USDC Token";
    ...
    uint256 constant minInvest = 50 * 10**6;
    uint256 constant percentDivider = 1000;
    uint256 constant maxRewards = 5000 * 10**6;
    ...
    IERC20 private USDC = IERC20(0x60DEb1AD352f45928eD9fa129b014420D07a55F9);

    constructor(address addr1, address addr2, address addr3) {
        dev = addr1;
        proj = addr2;
        part = addr3;
        _balances[address(this)] = 500000000 * 10**6; 
        _balances[proj] = 250000000 * 10**6; 	
        _balances[part] = 250000000 * 10**6;
    ...
    function invest(address ref, uint256 amount) public isInitialized {
        ...
        if (ref != 0x000000000000000000000000000000000000dEaD || ref != address(0)) {
            USDC.safeTransfer(ref, refAmount); 
        ...
        USDC.safeTransfer(proj, devStakeFee);
        USDC.safeTransfer(part, projStakeFee);
    ...
    function drawLastDepositWinner() public {
        if(lastDepositEnabled && block.timestamp.sub(lastDepositLastDrawAction) >= lastDepositTimeStep && lastDepositPotentialWinner != address(0)) {
            ...
            USDC.safeTransfer(lastDepositPotentialWinner, adjustedAmt); 
    ...
    function drawTopDepositWinner() private {
        if(topDepositEnabled && block.timestamp.sub(topDepositLastDrawAction) >= topDepositTimeStep) {
            ...
            USDC.safeTransfer(topDepositPotentialWinner, adjustedAmt); 
  	...
  	function reinvest() public isInitialized {
        ...
        for (uint i = 0; i < user.depoList.length; i++){
          if (!user.depoList[i].initialWithdrawn && !user.depoList[i].initialWithdrawRequest) {
    ...
    function withdrawInitial(uint256 key) public isInitialized {
        ...
        USDC.safeTransfer(dev, devStakeFee);
        USDC.safeTransfer(proj, projStakeFee);
        USDC.transfer(msg.sender, refundAmount.sub(devStakeFee.add(projStakeFee)));
        user.activeStakesCount--;
        user.totalInvested -= amount;
    ...
    function sellTokens(uint256 amount) external {       
        ...
        USDC.safeTransfer(msg.sender, cvtAmt.sub(devFee.add(projFee)));
        USDC.safeTransfer(dev, devFee);
        USDC.safeTransfer(proj, projFee);
    }
