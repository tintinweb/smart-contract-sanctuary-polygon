...
interface IMain {
    ...
    /**
     * @dev to Migrate Stakeholder(s) current address to the new one in the specified plans
     ...
     * @param _currentRootHashes the current root hashes
     * @param _newRootHashes the new root hashes
     * @param _currentProofs an array of proofs for current address in plans
     * @param _newProofs an array of proofs for the new address in plans
     * @param _addresses an array of the current and new wallet address
     * @param _allocations the allocation of the stakeholder
     */
    function migrateStakeholderAddressInSpecificPlans(
        uint256[] memory _planIds,
        bytes32[] calldata _currentRootHashes,
        bytes32[] calldata _newRootHashes,
        bytes32[][] calldata _currentProofs,
        bytes32[][] calldata _newProofs,
     ...
     * @param _proofs an array of merkle proofs for each stakeholder
     */
    ...
    Config
{
    ...
    function verifyProofByHash(
        ...
        _verify(_rootHash, _stakeholder, _stakeholderAllocation, _proof);
        return true;
    }
    ...
    function verifyProofByPlanId(
        ...
        _verify(_getPlan(_planIndex).merkleRootHash, _stakeholder, _stakeholderAllocation, _proof);
        return true;
    }
    ...
    function computeTopUp(uint256 _timeframe) external view override returns (uint256 _requiredTokens) {
        ...
        for (uint i = 0; i < _lengthOfPlans; i++) {
            PlanStruct memory _plan = plans[i];
            // calculate vested until for the plan!
            if (_plan.isTrustless) {
                _requiredTokens += _plan.budget / PIP;
            } else {
                _requiredTokens += _computeVestedUntilForPlan(
                    plans[i],
                    HUNDRED_PERCENT - _plan.cancelledAllocationPercentage,
                    _plan.cancelledAt != 0 ? _plan.cancelledAt : _until // RU-08
                );

                if (_plan.cancelledAllocationPercentage > 0) {
                    _requiredTokens += _subHelper(_plan.reservedForCancelledStakeholders, _plan.totalClaimedByCancelledStakeholders);
                }
            }
            // reduce total claimed tokens of the plan
            _requiredTokens = _subHelper(_requiredTokens, _plan.totalClaimedByActiveStakeholders);
        }
    ...
    function claim(uint256 _planIndex, address _stakeholder, uint256 _allocation, uint256 _amount, bytes32[] calldata _proof) public {
        _verify(_getPlan(_planIndex).merkleRootHash, _stakeholder, _allocation, _proof);
    ...
    function migrateStakeholderAddressInSpecificPlans(
        ...
        uint256 _length = _planIndexes.length;
        for (uint i = 0; i < _length; i++) {
            ...
            _verify(_currentRootHashes[i], _addresses[0], _allocations[i], _currentProofs[i]);
            // validate new address
            _verify(_newRootHashes[i], _addresses[1], _allocations[i], _newProofs[i]);

            _p.merkleRootHash = _newRootHashes[i];

            _migrateWalletAddress(_planIndex, _addresses[0], _addresses[1]);
    ...
    function cancelStakeholders(
        ...
        for (uint i = 0; i < _stakeholdersAddress.length; i++) {
            // make sure the claimed allocation are correct
            _verify(_p.merkleRootHash, _stakeholdersAddress[i], _allocations[i], _proofs[i]);
            ...
            // @dev computing vested tokens are happening in lazy mode (calculated on the fly)
            // @dev so reduce the amount of claimed tokens in the plan (by the cancelled stakeholder) to keep the top-up computer correct.
            _p.totalClaimedByActiveStakeholders = _subHelper(
                _p.totalClaimedByActiveStakeholders,
                getStakeholderStatus(_planIndex, _stakeholdersAddress[i]).tokensClaimed
            );

            uint256 _alreadyClaimed = getStakeholderStatus(_planIndex, _stakeholdersAddress[i]).tokensClaimed;
            _p.totalClaimedByCancelledStakeholders += _alreadyClaimed;
     ...
     */
    function _verify(bytes32 _rootHash, address _addr, uint256 _allocation, bytes32[] calldata _proof) private pure {
    ...
    function _computeVestedForIndividual(PlanStruct storage _p, address _stakeholder, uint256 _allocationPercentage) private view returns (uint256) {
        ...
        return
            _computeVestedUntilForPlan(
                ...
                // compute until the moment the plan or stakeholder was cancelled
    ...
    function _computeAmountOfReservedTokensForTrustlessPlans() private view returns (uint256 _reservedTokens) {
        ...
        for (uint i = 0; i < _lengthOfPlans; i++) {
            PlanStruct memory _plan = plans[i];
            if (_plan.isTrustless) _reservedTokens += ((_plan.budget / PIP) - _plan.totalClaimedByActiveStakeholders);
        }
    }
    ...
    function _computeWithdrawable() private view returns (uint256) {
        ...
        for (uint i = 0; i < _length; i++) {
            ...
            if (_plan.isTrustless) continue;
            _reservedTokens += _subHelper(
                _computeVestedUntilForPlan(
                    plans[i],
                    HUNDRED_PERCENT - _plan.cancelledAllocationPercentage,
                    // RU-08
                    _plan.cancelledAt != 0 ? _plan.cancelledAt : block.timestamp
                ),
                _plan.totalClaimedByActiveStakeholders
            );

            //@dev if there are any cancelled stakeholders
            if (_plan.cancelledAllocationPercentage > 0)
                _reservedTokens += _subHelper(_plan.reservedForCancelledStakeholders, _plan.totalClaimedByCancelledStakeholders);
        }
    ...
    function _computeCustomVestedUntil(PlanStruct storage _p, uint256 _allocationPercentage, uint256 _until) private view returns (uint256 _res) {
        ...
        for (uint i = 0; i < _length; i++) {
            if (_until >= _customVesting.timestamps[i]) {
                if (i == _lastUnlock) _allocationsReleased = HUNDRED_PERCENT;
