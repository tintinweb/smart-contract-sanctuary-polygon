...
contract Graveyard_auction is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable { /* Contract inherits from ERC1155 spec */
    ...
    function createAuction(uint256 id, uint256 initial_price, uint256 end_time) external onlyOwner {
        _mint(owner(), id, 1, "");
        ...
        subauction.auction_end_time = end_time;
        subauction.initialized = true;
    }

    function distribute(uint256 id) external onlyOwner {
        require(block.timestamp > subauctions[id].auction_end_time, "Auction has not ended");

        BidLink memory current = subauctions[id].bids[subauctions[id].bidcount];
    ...
    function bid(uint256 id) payable public {
        ...
        require(block.timestamp < subauctions[id].auction_end_time, "Auction has ended");

        if(subauctions[id].bidcount > 0){
            uint256 amount = subauctions[id].bids[subauctions[id].bidcount].value - 2300 * tx.gasprice;
            subauctions[id].bids[subauctions[id].bidcount].value = 0;  /* stop re-entrancy vulnerability */
            subauctions[id].bids[subauctions[id].bidcount].bidder.transfer(amount);
        }
