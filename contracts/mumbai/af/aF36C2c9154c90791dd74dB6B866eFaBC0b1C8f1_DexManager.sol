...
contract DexManager is Ownable, ReentrancyGuard, IDexManager {
        ...
        returns (uint256, uint256)
    {
        ...
        require(_lp != address(0), "ERR: INVALID LP ADDRESS");
        require(amountParams._lpAmount > 0, "ERR: INVALID LP_AMOUNT");

        _performRemoveLiquidityApprovals(_lp, amountParams._lpAmount, _ammId);
        ...
        (uint256 obtainedA, uint256 obtainedB) = IDex(AMMs[_ammId])
            .removeLiquidity(_tokenA, _tokenB, amountsData, _data);
        require(obtainedA > 0 || obtainedB > 0, "ERR: SWAP FAILED");
        if (obtainedA > 0) {
            IERC20(_tokenA).safeTransfer(msg.sender, obtainedA);
        }
        if (obtainedB > 0) {
            IERC20(_tokenB).safeTransfer(msg.sender, obtainedB);
        }
        ...
        )
    {
        ...
        data.lpBalanceBefore = IERC20(_lp).balanceOf(address(this));
        bytes memory amountsData = abi.encode(
            amountParams._amountADesired,
            amountParams._amountBDesired,
            amountParams._amountAMin,
        ...
        (data.usedA, data.usedB, data.obtainedLP) = IDex(AMMs[_ammId])
            .addLiquidity(_tokenA, _tokenB, amountsData, _data);
        data.lpBalanceAfter = IERC20(_lp).balanceOf(address(this));
        require(
            data.obtainedLP == (data.lpBalanceAfter - data.lpBalanceBefore),
            "ERR: INCORRECT LP ADDRESS"
        );
        require(data.obtainedLP > 0, "ERR: ADD LIQUIDITY FAILED");

        //transfer to sender
        if (amountParams._amountADesired - data.usedA > 0) {
            IERC20(_tokenA).safeTransfer(
                msg.sender,
                amountParams._amountADesired - data.usedA
            );
        }
        if (amountParams._amountBDesired - data.usedB > 0) {
            IERC20(_tokenA).safeTransfer(
                msg.sender,
                amountParams._amountBDesired - data.usedB
            );
        }
        IERC20(_lp).safeTransfer(msg.sender, data.obtainedLP);
    ...
    /// @notice Peforms approvals and pre-transfers for the remove liquidity operation
    /// @param _lp LP token address
    /// @param _amountLp Amount of LP token
    /// @param _ammId Registered AMM wrapper id
    function _performRemoveLiquidityApprovals(
        address _lp,
        uint256 _amountLp,
        uint256 _ammId
    ) private {
        IERC20(_lp).safeTransferFrom(msg.sender, address(this), _amountLp);
        OperationsLib.safeApprove(_lp, AMMs[_ammId], _amountLp);
    }
