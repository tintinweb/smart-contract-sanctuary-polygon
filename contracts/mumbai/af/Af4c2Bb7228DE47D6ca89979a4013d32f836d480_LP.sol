...
contract LP is ERC721AQueryable, Ownable, ReentrancyGuard, Pausable, IERC2981 {
    ...
    */
    bytes32 public phase1Root =
        0x8a14fd0a8633ae9e222df337982f10f4ba5c53efb5eb062eda33065276e064b9;

    bytes32 public phase2Root =
        0xcf0a03e6f32bbbfcf5c04e7c5079db18817ce26a36502d65eb222e3bf9a51f89;

    uint256 public constant PHASE_1_MAX = 2;
    ...
    */
    function phase1Mint(
        ...
        require(phase1Start <= block.timestamp && block.timestamp < phase2Start, "Phase 1 inactive");
        require(isMintValid(msg.sender, proof, phase1Root), "Not in phase 1 whitelist");
        require(quantity <= 2, "Max 2 at once");
        require(userMinted[msg.sender] + quantity <= PHASE_1_MAX, "Phase 1 limit exceeded");
    ...
    function phase2Mint(
        ...
        require(phase2Start <= block.timestamp && block.timestamp < phase3Start, "Phase 2 inactive");

        require(isMintValid(msg.sender, proof, phase2Root), "Not in phase 2 whitelist");
    ...
    function phase3Mint(uint256 quantity) external payable nonReentrant whenNotPaused {
        require(phase3Start <= block.timestamp, "Phase 3 inactive");
    ...
    function platformMint(address to, uint256 qty) external whenNotPaused {
        require(msg.sender == minter, "Not eligible to call");
    ...
    */
    ...
    function setBlacklisted(address _blacklisted, bool _isBlacklisted) external onlyOwner {
        isBlacklisted[_blacklisted] = _isBlacklisted;
    }
