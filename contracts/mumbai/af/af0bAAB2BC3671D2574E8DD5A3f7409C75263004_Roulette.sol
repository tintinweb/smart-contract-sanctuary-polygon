...
contract Roulette is VRFConsumerBaseV2 {
        ...
        )
    {
        ...
        for (uint256 i = (gamesResult[player].length - 1); i > (gamesResult[player].length - n); i--){
            counter--;
    ...
    function startRoll(uint256 iter, uint8[] memory _choice) external payable returns (uint256 requestId) {
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override isVRFCoordinator {
        ...
        for (uint i = 0; i<(gamesStart[_requestId].numOfIterations); i++) {
            ...
            _randomWords[0] = _randomWords[0]/100;
        }
        ...
        if (reward > 0) {
            ...
            (bool _success, bytes memory _res) = pool.call(abi.encodeWithSignature("isHaveReferal(address)", msg.sender));
            // отчисление в 0.5% человеку, который привел реферала (работает только в случае поражения)
            if (_success && abi.decode(_res, (bool))){
    ...
    function calculatePrize(
        ...
        if (isIncluded(choice, num)) {
            return ((deposit * (360/choice.length))/10);
        } else {
            return 0;
        }
    }

    function isIncluded(uint8[] memory choice, uint256 num) public view returns (bool) {
