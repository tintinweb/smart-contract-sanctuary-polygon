...
contract TNFT is IERC721 {
    ...
    string internal nftName = "NameNFT";
    string internal nftSymbol = "TNFT";
     ...
     * Public sale minting.
     */
    function mint(uint256 tokenid) external reentrancyGuard {
        require(publicSale, "Sale not started.");
        require(!marketPaused);
         require(IERC20(USDV).balanceOf(msg.sender) >= mintPrice, "Insufficient balance");
         USDV.transferFrom(msg.sender, address(this), mintPrice); //all NFT price is 1 usdv
            uint id = _mint(tokenid,msg.sender);
            emit Transfer(address(0), msg.sender, id);
        
    }

    function _mint(uint id,address _to) internal returns (uint) {
    ...
    function setMintprice(uint _price) public onlyDeployer() {
     ...
     function Adminwithdraw(uint _amount) public onlyDeployer() {
         USDV.transfer(msg.sender, _amount);
    }
