...
contract TestCoin is Context, IERC20, Ownable2 {
    ...
    uint256 private supplyMinimumStartingAmount = 1000000000; // Start with 1 token
    ...
    // Allow the router to be modified after it's been added to chain
    ...
    address routerAddress) { 
        ...
        require(routerAddress != address(0), "routerAddress should not be the zero address");
        marketingWallet = _marketingWallet;
        devWallet = _developmentWallet;
        _name = _projectName;
        _symbol = _projectSymbol;
        swapRouterAddress = routerAddress;
        emit Transfer(address(0),owner(), supplyMinimumStartingAmount);
        _tTotal = supplyMinimumStartingAmount;
    }

    function setInitialSupply(uint256 mintSupplyTotal, uint256 maxWalletHoldingPercentage) external onlyOwner {
        require(supplyHasBeenMinted <= 1, "You have already minted you supply"); // You have 2 tries to get your supply correct on chain before it will fail
    ...
    function liftMaxTx() external {
        ...
        require(tradingEnableTime+ 1 minutes <  block.timestamp,"Transaction limit can only be lifted 10 mins after trading is enanbled");
    ...
    function openTrading() external onlyOwner {
        ...
        // Spooky swap router 0xF491e7B69E4244ad4002BC14e878a34207E38c29
    ...
    function changeMarketingWallet( address payable _address) external onlyOwner notLocked(Functions.changeMarketWallet){
     ...
     function changeDevelopmentWallet( address payable _address) external onlyOwner notLocked(Functions.changeDevWallet){
        require(_address != address(0),"Development Wallet can not be zero");
        devWallet = _address;
        timelock[Functions.changeDevWallet] = 0;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        uint256 tTeam = amount*transferTax/100;    // tax amount
    ...
    function changeBuyTax(uint256 _newBuyTax) external onlyOwner{
        require(_newBuyTax < 15, "New Buy tax have to be under 15%");
        buyTax = _newBuyTax;
    ...
    function changeSellTax(uint256 _newSellTax) external onlyOwner{
        require(_newSellTax < 15, "New Buy tax have to be under 15%");
        sellTax = _newSellTax;
        emit sellTaxUpdated(_newSellTax);
    }

    event transferTaxUpdated(uint256 _newTaxAmount);   
    function changeTransferTax(uint256 _newTransferTax) external onlyOwner{
        require(_newTransferTax < 15, "New Buy tax have to be under 15%");
        transferTax = _newTransferTax;
        emit sellTaxUpdated(_newTransferTax);
    }
...
contract Ownable2 is Context {
    ...
    // Add owner wallet, or timesig wallet, use defender.openzeppelin.com to setup an on chain multi signature wallet.
    ...
    enum Functions {changeOwnership,changeMarketWallet,changeDevWallet,pause }
