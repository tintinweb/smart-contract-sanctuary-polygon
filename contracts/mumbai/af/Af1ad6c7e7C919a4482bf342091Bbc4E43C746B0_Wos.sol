...
contract Wos is ERC20, OnlyAuthorized, IOperable {
    ...
    address private wosPackagesAddress;

    constructor(IECDSASignature2 _Signature) ERC20("WOS", "WOS") {
        Signature = _Signature;
    }

    function mint(uint256 amount) external {
        require(
            msg.sender == address(bars) || msg.sender == wosPackagesAddress,
    ...
    function changeAuditor(
        ...
        Signature.verifyMessage(
            abi.encodePacked(ecdsa, msg.sender).hash(),
            nonce,
            timestamp,
            signatures
        );
        Signature = IECDSASignature2(ecdsa);
    }

    function setBarsAddress(
        IOperable _bars,
        uint256 nonce,
        uint256 timestamp,
        bytes[] memory signatures
        ...
        Signature.verifyMessage(
            abi.encodePacked(address(_bars), msg.sender).hash(),
            nonce,
            timestamp,
            signatures
        );
        ...
        require(
            address(bars) == address(0),
            "You can't change the address again"
        );
        bars = _bars;
    }

    function setWosPackagesAddress(
        address addr,
        uint256 nonce,
        uint256 timestamp,
        bytes[] memory signatures
        ...
        Signature.verifyMessage(
            abi.encodePacked(addr, msg.sender).hash(),
            nonce,
            timestamp,
            signatures
        );
    ...
    function getWosPackagesAddress() external view returns (address) {
        return wosPackagesAddress;
    }
}
