...
contract SGForged is ERC721AQueryable, Ownable, Reclaimable, Pausable {
    using Strings for uint256;

    SG public unforgedContract;
    ...
    string private _contractBaseURI = "ipfs://yyy";

    // royalties
    uint256 private royaltyBps = 500;
    address private royaltyReceiver = 0x59033903712b93202170952D68BdB6C461DB4253;
    ...
    function forgeMultiple(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(unforgedContract.balanceOf(msg.sender, ids[i]) >= amounts[i], "Not the owner");
    ...
    function forge(uint256 id) external whenNotPaused {
        require(unforgedContract.balanceOf(msg.sender, id) > 0, "Not the owner");
