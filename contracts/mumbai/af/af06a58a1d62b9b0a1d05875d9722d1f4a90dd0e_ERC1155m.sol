...
// Root file: contracts/tokens/ERC1155m_Brand_Tokens_mp.sol
...
// custom errors
error MinterNotAllowed(address minter);
...
contract ERC1155m is ERC1155, Ownable {
    ...
    address public _moleculeMinter;
    ...
    function mintBatch(
        ...
        // Check if minter is allowed
        if (_moleculeMinter != address(0)) {
            if (!IMoleculeController(_moleculeMinter).check(msg.sender)) {
                revert MinterNotAllowed(msg.sender);
            }   
        }
    ...
    function updateMolecule(
        ...
        } else if (mtype == MoleculeType.Minter) {
            _moleculeMinter = molecule;
