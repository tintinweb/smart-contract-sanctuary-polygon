...
contract ddsmaiproverbs is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public ddsmaiproverbs_PRESALE = 1; //presale
    uint public ddsmaiproverbs_WALLET_PRESALE_LIMIT = 10; //ddsmaiproverbsperwalletInWholeDuration
    uint public ddsmaiproverbs_WALLET_PUBLICSALE_LIMIT = 20; //ddsmaiproverbsperwalletInWholeDuration
    uint public ddsmaiproverbs_MAX_COUNT = 150; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public ddsmaiproverbs_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public ddsmaiproverbs_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmVAsCq9cRKaoyFrW8L1LhAk8JJhEr3ouYw1MNRiiZU6uf/";
    ...
    constructor() ERC721("DDSMPrime Proverbs", "ddsmaiproverbs") {}

    function setddsmaiproverbs_MAX_COUNT(uint _maxCount) external onlyOwner{
        ddsmaiproverbs_MAX_COUNT = _maxCount;
    }

    function setddsmaiproverbs_PRESALE(uint _presale) external onlyOwner{
        ddsmaiproverbs_PRESALE = _presale;
    }

    function setddsmaiproverbs_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        ddsmaiproverbs_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setddsmaiproverbs_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        ddsmaiproverbs_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setddsmaiproverbs_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        ddsmaiproverbs_MAINSALE_PRICE = _mainSalePrice;
    }

    function setddsmaiproverbs_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        ddsmaiproverbs_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= ddsmaiproverbs_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= ddsmaiproverbs_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= ddsmaiproverbs_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (ddsmaiproverbs_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (ddsmaiproverbs_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= ddsmaiproverbs_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//ddsmaiproverbs_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= ddsmaiproverbs_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= ddsmaiproverbs_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(ddsmaiproverbs_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
