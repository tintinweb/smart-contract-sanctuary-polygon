 ...
 * @notice This contract handles minting test soul tokens.
 */
contract TestSoul is
    ERC1155Supply,
    Ownable,
    Pausable,
    ReentrancyGuard,
    ...
    RevokableOperatorFilterer
{
    ...
    address private presaleSignerAddress =
        0x06623f56ea58834245eE9E20151C86E5c985F501;
    ...
    // If true, all tokens are soulbound
    bool public allTokensSoulbound;
    ...
    // If true, tokens are soulbound, if false, tokens can be transferred
    mapping(uint256 => bool) public tokenTransfersDisabled;
    ...
    constructor(uint256 _heymintFeePerToken)
        ERC1155(
            "ipfs://bafybeidzehtbcuyzopvtmgkpdqrbsp3scq27yghpkm266jjsxexizirgiq/{id}"
        )
        RevokableOperatorFilterer(
        ...
        )
    {
        ...
        tokenMaxSupply[1] = 20000;
        tokenPublicPrice[1] = 0 ether;
        tokenPublicMintsPerAddress[1] = 20000;
        tokenTransfersDisabled[1] = true;
        require(
    ...
    /**
     * @notice Turn token transfers on or off for a given tokenId
     */
    function setTokenTransfersDisabled(
        uint256 _tokenId,
        bool _tokenTransfersDisabled
    ) external onlyOwner {
        tokenTransfersDisabled[_tokenId] = _tokenTransfersDisabled;
    }

    /**
     * @notice Turn soulbinding on or off for all tokens
     */
    function setAllTokensSoulbound(bool _allTokensSoulbound)
        external
        onlyOwner
    {
        allTokensSoulbound = _allTokensSoulbound;
    }
    ...
    {
        require(!allTokensSoulbound, "TOKEN_SOULBOUND");
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            require(
                !allTokensSoulbound || soulboundAdminTransferInProgress,
                "TOKEN_SOULBOUND"
            );
            for (uint256 i = 0; i < ids.length; i++) {
                require(
                    !tokenTransfersDisabled[ids[i]] ||
                        soulboundAdminTransferInProgress,
                    "TOKEN_SOULBOUND"
                );
            }
        }
