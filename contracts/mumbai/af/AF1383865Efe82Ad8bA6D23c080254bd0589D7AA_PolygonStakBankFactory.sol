...
contract PolygonStakBankFactory  is  PausableUpgradeable,OwnableUpgradeable{
    ...
    mapping(address => mapping(IERC20Upgradeable => address[])) public getStakBanks;
    ...
    function initialize(IJSTAK _jStak) public initializer {
        require(_jStak != IJSTAK(address(0)), "StakBankFactory::ZERO_ADDRESS");
        __Ownable_init();
     ...
     */
    function getCreatedStakBanksByToken(address _creator, IERC20Upgradeable _stakingToken)
        public
        view
    ...
    /**
     * @notice Owner can set the JStak contract(IERC20Upgradeable)
    ...
    function registerStakBank(
        ...
        require(
            _stakingToken != IERC20Upgradeable(address(0)),
            "StakBankFactory::ZERO_ADDRESS"
        );
        require(
            _rewardToken != IERC20Upgradeable(address(0)),
            "StakBankFactory::ZERO_ADDRESS"
        );
        bytes memory tempNameString = bytes(_name);
        require(tempNameString.length != 0, "StakBankFactory::NO_NAME");

        bytes memory bytecode = type(PolygonStakBank).creationCode;
