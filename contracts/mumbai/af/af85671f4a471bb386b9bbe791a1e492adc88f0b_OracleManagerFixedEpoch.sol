...
contract OracleManagerFixedEpoch is IOracleManagerFixedEpoch {
  ...
  /// @notice Returns end timestamp of current epoch
  /// @return getEpochEndTimestamp end timestamp of the current epoch
  ...
  /// @notice Returns start timestamp of current epoch and number of epochs between current timestamp and lastEpochEndTimestamp
  /// @param lastEpochEndTimestamp end timestamp of the last epoch
  /// @return currentEpochTimestamp start timestamp of the current epoch
  /// @return numberOfEpochsSinceLastEpoch number of epochs between current timestamp and lastEpochEndTimestamp
  function getCurrentEpochStartTimestampAndNumberOfEpochsSinceLastEpoch(
  ...
  // TODO look where this function is used and delete if not used at all
  // TODO: should we not be passing around epoch indexes rather than timestamps?
  ...
  /// @return roundId ID of latest round
  /// @return answer price of latest round
  /// @return startedAt start round of latest round
  /// @return updatedAt  update round of latest round
  /// @return answeredInRound round at which latest round was answered at
  ...
  /// @notice Returns the latest round ID from chainlink Oracle
  /// @return latestRoundId ID of latest round
  ...
  /// @notice Checks for whether the oracle price update happened before end of current epoch end timestamp
  /// @dev Called by internal functions to check whether oracle price update happened before end of current epoch end timestamp
  /// @param currentEpochStartTimestamp start timestamp of current epoch
  /// @param currentOracleUpdateTimestamp timestamp of current oracle price update
  ...
  /// @notice Returns the price of particular oracle update index
  /// @param index specific oracle update index
  /// @return price price of the specified oracle update index
  ...
  function getMissedEpochPriceUpdates(
    ...
    // check whether timestampReusableVariable is before end point of previous epoch
    // if met, then break
    if (getEpochStartTimestamp() - EPOCH_LENGTH > timestampReusableVariable) {
      MissedEpochExecutionInfo[] memory _missedEpochPriceUpdates = new MissedEpochExecutionInfo[](
        1
      );

      _missedEpochPriceUpdates[0] = MissedEpochExecutionInfo({
        oracleUpdateIndex: 0,
        oraclePrice: 0,
        timestampPriceUpdated: 0,
        associatedEpochIndex: 0,
        epochHasNoAvailablePrice: true
      });
      return _missedEpochPriceUpdates;
    }
    uint256 _numberOfMissedEpochs = Math.min(
      _getMaximumNumberOfMissedEpochs(_latestExecutedEpochIndex, timestampReusableVariable),
    ...
    while (
      ...
      if (
        ...
        // check whether price update is after end point of next epoch
        // if met, break the loop and send back the false
        if (
          !_isCurrentPriceBeforeCurrentEpochEndTimestamp(
            timestampReusableVariable,
            _currentOracleUpdateTimestamp
          )
        ) {
          ...
          _missedEpochPriceUpdates[
            ...
            associatedEpochIndex: _latestExecutedEpochIndex +
              1 +
              _missedEpochExecution._currentMissedEpochPriceUpdatesArrayIndex,
            epochHasNoAvailablePrice: false
          });
        }
  ...
  /// @notice Returns complete oracle information for executing historical epoch(s)
  /// @param _latestExecutedEpochIndex the most recent epoch index in which a price update has been executed
  /// @param _previousExecutionPriceOracleIdentifier the "roundId" used to reference the most recently executed oracle price on chainlink
  /// @param _previousEpochEndTimestamp end timestamp of the previous epoch
  /// @return currentEpochTimestamp start timestamp of the current epoch
  /// @return numberOfEpochsSinceLastEpoch number of epochs between current timestamp and lastEpochEndTimestamp
  /// @return previousPrice price that was used for execution of previous epoch
  /// @return _missedEpochPriceUpdates list of epoch execution information
    ...
    )
  {
    ...
    (
      currentEpochTimestamp,
      numberOfEpochsSinceLastEpoch
    ) = getCurrentEpochStartTimestampAndNumberOfEpochsSinceLastEpoch(_previousEpochEndTimestamp);
