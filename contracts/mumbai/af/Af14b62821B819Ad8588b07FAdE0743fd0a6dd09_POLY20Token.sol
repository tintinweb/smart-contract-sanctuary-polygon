  ...
  */
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if(takeFee==true) {
      ...
      uint256 _reflectionsValue = amount.mul(_reflectionsPer).div(100);
      uint256 _marketingValue = amount.mul(_marketingPer).div(100);
      uint256 _liquidityValue = amount.mul(_liquidityPer).div(100);
      uint256 _burnValue = amount.mul(_burnPer).div(100);
      ...
      _autoburn(recipient, _burnValue);

      netamount=amount.sub(_reflectionsValue).sub(_marketingValue).sub(_liquidityValue).sub(_burnValue);
   ...
   * - `account` must have at least `amount` tokens.
   */
  function _autoburn(address account, uint256 amount) internal {
    require(account != address(0), "POLY20: burn from the zero address");
    _totalSupply = _totalSupply.sub(amount);
    emit Transfer(account, address(0), amount);
  }
