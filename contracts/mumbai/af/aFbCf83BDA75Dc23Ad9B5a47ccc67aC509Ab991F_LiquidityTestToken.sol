...
/// @title ERC-20 Token for LTT
contract LiquidityTestToken is ERC20, Ownable, ReentrancyGuard {
    
    /// maximum token allowed to be 10 Billion LiquidityTestToken * decimals()
    ...
    constructor() ERC20("Liquidity Test Token", "LTT") {
        /// Minting 1 Billion LTT initially
    ...
    function mintTokens(uint256 tokenAmount, address _address) external onlyOwner{
        require(tokenAmount <= maxTokenSupply - totalSupply(), "Total supply can be a maximum of 10 Billion LTT tokens");
        _mint(_address, tokenAmount);
        emit TokenMinted(tokenAmount, _address);
    }
}
