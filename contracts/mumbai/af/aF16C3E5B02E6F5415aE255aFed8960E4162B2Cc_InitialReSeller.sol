...
interface IMarketplaceExt is IMarketplace {
...
contract InitialReSeller is IInitialReSeller, IERC721Receiver {
    ...
    mapping(ITangibleFractionsNFT => mapping(uint256 => FractionBuyer )) public saleBuyersData;
    ...
    function withdrawSale(ITangibleFractionsNFT ftnft) external onlyFactoryAdmin{
        require(factory.fractionToTnftAndId(ftnft).initialSaleDone, "Sale in progress");
        //send the sale amount, sale was success
        uint256 balance = 
            saleData[ftnft].askingPrice == saleData[ftnft].paidSoFar 
                ? saleData[ftnft].askingPrice : saleData[ftnft].paidSoFar;
        
        saleData[ftnft].paymentToken.safeTransfer(msg.sender, balance);
        saleData[ftnft].sale = false;
        emit SaleAmountTaken(ftnft, saleData[ftnft].paymentToken, balance);
    }

    function withdrawFtnft(ITangibleFractionsNFT ftnft, uint256[] calldata tokenIds) external onlyFactoryAdmin{
    ...
    function extendSale(ITangibleFractionsNFT ftnft, uint256 endDate) external onlyFactoryAdmin{
    ...
    external onlyFactoryAdmin{
        ...
        if(address(ftnft) != address(0)){
            require(!factory.fractionToTnftAndId(ftnft).initialSaleDone, "sale already done");
        }
        IMarketplace marketplace = IMarketplace(factory.marketplace());
        //take tnft
        ...
        emit SaleStarted(ftnft, endSaleDate, tokenToSell, askingPrice);
    }
    ...
    function modifySale(
        ...
        marketplace.sellFraction(ftnft, fractionId, new uint256[](0), askingPrice, minPurchaseShare);
    }

    function reemburse(ITangibleFractionsNFT ftnft, uint256[] calldata tokenIds) external onlyFactoryAdmin{
        ...
        for(uint256 i; i < length; i++){
            ...
            //send back the money
            saleData[ftnft].paymentToken.safeTransfer(owner, saleBuyersData[ftnft][tokenIds[i]].pricePaid);
            //delete records
