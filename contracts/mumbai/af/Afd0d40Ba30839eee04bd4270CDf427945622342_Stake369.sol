...
/*
        33333333333333      66666666666666      99999999999999
                    33      66                  99          99
                    33      66                  99          99
                    33      66                  99          99
        33333333333333      66666666666666      99999999999999
...
contract Stake369 is ERC721, Ownable, RoyaltiesV2Impl {
    using Strings for uint256;

    /*
     * Private Variables
     */
    // uint256 private constant MAX_NUMBER_OF_1 = 168;
    ...
    //DEV PURPOSES - #remove
    uint256 private constant MAX_NUMBER_OF_1 = 12;
    uint256 private constant MAX_NUMBER_OF_2 = 12;
    uint256 private constant MAX_NUMBER_OF_3 = 6;
    uint256 private constant MAX_NUMBER_OF_4 = 12;
    uint256 private constant MAX_NUMBER_OF_5 = 12;
    uint256 private constant MAX_NUMBER_OF_6 = 3;
    uint256 private constant MAX_NUMBER_OF_7 = 12;
    uint256 private constant MAX_NUMBER_OF_8 = 12;
    uint256 private constant MAX_NUMBER_OF_9 = 3;

    //DEV PURPOSES - #remove
    uint256 private constant MAX_SUPPLY = 84; // collection size
    ...
    uint256 private constant MAX_MINTS_PER_ADDRESS = 100; // this is the max total mints per account
    bytes4 private constant _INTERFACE_ID_ERC2981 = 0x2a55205a;
    struct MintTypes {
        uint256 _numberOfMintsByAddress;
    }

    //DEFAULT URI FOR UNREVEALED STAGE
    string private _defaultUri;

    //URI WILL BE INIT ONCE THE TIME OF THE REVEALING PHASE COMES
    ...
    uint256 public mintPrice = 36900000000000000000;

    mapping(address => MintTypes) public addressToMints;
    mapping(uint256 => uint256) public revealedTokens;
    mapping(uint256 => string) private _uris;
    mapping(address => bool) private moderatorAddresses;
    mapping(address => bool) private whitelistedAddresses;

    address[] mintedAddressesArray; 
    /*
     * Constructor
     */
    constructor(string memory uri) ERC721("Stake369", "CARD") {
        _defaultUri = uri;
    ...
    function addToWhiteList(address _addressToWhitelist)
        external
        onlyOwner
        onlyModerator(msg.sender)
    {
        ...
        function removeFromWhiteList(address _addressToWhitelist)
        external
        onlyOwner
        onlyModerator(msg.sender)
    {
    ...
    function mint() external payable validateEthPayment(1) {
        require(mintable == true, "Exceeds maximum allowable mints");
        if (whitelist) {
            require(
                (whitelistedAddresses[msg.sender] == true) ||
                    owner() == msg.sender,
                "Only whitelisted users can mint at this stage"
            );
        }
        require(mintedTokens < MAX_SUPPLY, "Exceeds maximum allowable mints");
        require(
            addressToMints[msg.sender]._numberOfMintsByAddress + 1 <=
        ...
        mintedAddressesArray.push(msg.sender);
    }
    ...
    function _mintRandomId(address to) private {
        uint256 id = mintedTokens++;
        _safeMint(to, id);
        LibPart.Part[] memory _royalties = new LibPart.Part[](1);
        _royalties[0].value = 1000;
        _royalties[0].account = payable(address(this));
        ...
        if (bytes(_uris[tokenId]).length == 0) {
            ...
            string memory uri = string(
                abi.encodePacked(
                    _tokenBaseURI,
                    "/",
                    (revealedTokens[tokenId]).toString(),
                    ".json"
                )
            );
            return uri;
    ...
    function revealSpecificAmount(uint256 from, uint256 to) public onlyOwner {
        require(reveal == true, "Not allowed to reveal yet");
    ...
    function revealNTF(uint256 tokenId, uint256 random)
        private
        onlyOwner
    {
        require(reveal == true, "Not allowed to reveal yet");
        require(bytes(_uris[tokenId]).length == 0, "NFT already revealed");

        random = (random + tokenId * 3) % 9;
        random++;
        uint256 repeated = 12;

        if (availableNumbers[random] == 0) {
            while ((availableNumbers[random] == 0) && (repeated > 0)) {
                random = ((random + 1) % 9) + 1;
                repeated--;
            }
        }

        require(repeated > 0, "No more nfts to reveal");

        revealedTokens[tokenId] = random;
        availableNumbers[random] = --availableNumbers[random];
        _uris[tokenId] = string(
            abi.encodePacked(_tokenBaseURI, "/", random.toString(), ".json")
        );
    }

    function revealOneNTF(uint256 tokenId) public returns (string memory) {
        require(reveal == true, "Not allowed to reveal yet");
        if (_exists(tokenId)) {
        require(bytes(_uris[tokenId]).length == 0, "NFT already revealed");
    ...
    // ======================================================== Getters
        function getAvailabilityOfToken(uint256 index)
        public
        view
        returns (uint256)
    {
    ...
    function getRemainingTokenNumber() public view returns (uint256) {
        return MAX_SUPPLY - mintedTokens;
    }

    function getMintedPeople() public view returns (address  [] memory){
        return mintedAddressesArray;
    }
    ...
    function setMintableState(bool state) public onlyOwner {
        mintable = state;
    }

    function setWhitelistOnlyState(bool state) public onlyOwner {
        whitelist = state;
    }
    ...
    modifier onlyModerator(address _addressOfModerator) {
        require(
            (moderatorAddresses[_addressOfModerator] == true) ||
                owner() == _addressOfModerator,
