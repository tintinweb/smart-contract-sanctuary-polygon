...
contract EthermonStaking is EthermonStakingBasic, EthermonEnum {
    ...
    function depositeTokens(
        ...
        //uint256 dayTime = block.timestamp + (daysToStake[uint8(_day)] * 1 days);
        ...
        data.endTime = now + (daysToStake[uint8(_day)] * 1 minutes);
        ...
        uint256 timeElapsed = (now - data.lastCalled) / (1 minutes);
    ...
    function updateTokens(uint256 _lockId) external {
        ...
        if (timeElapsed > 0 && now > data.endTime) {
            //7                         // 32 - 28 = 5
            timeElapsed = timeElapsed - ((now - data.endTime) / 1 minutes);
            data.balance +=
