...
contract NFTCollectible is ERC721Enumerable, Ownable {
    ...
    event NewTotalMintCount(uint256 tokenId);
    event NewSendMessage(string message);

    constructor(string memory baseURI) ERC721("NFT Collectible", "NFTC") payable {
        setBaseURI(baseURI);
        /*
         * 初期シードを設定
         */
    ...
    function mintNFTs(uint _count) public payable {
        ...
        if (seed <= probability) {
            ...
            //uint256 prizeAmount = PRICE.mul(_count);
            uint256 prizeAmount = msg.value;
            require(
        ...
        emit NewSendMessage( mes );
        emit NewTotalMintCount( _tokenIds.current() );
    }
