...
/**
 * @title BTRT | REI Token
 * @author REI Token
 * @dev BTRT ERC20 Token | This contract is opt for digital securities management.
 */
 
contract BTRTToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "BTRT: funds are frozen."
        );
        _;
    }

    /**
     * @notice BTRTToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "Build To Rent Security Token";
        _symbol = "BTRT";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
     ...
     * @param _value BTRTg that enables or disables the whitelist.
     */
     ...
     * @param _value BTRTg that enables or disables the transactions limit.
     */
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "BTRT: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "BTRT: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "BTRT: _x must not be equal to _y");
        require(_x > 0, "BTRT: _x must be greater than 0");
        require(_y > 0, "BTRT: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "BTRT: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "BTRT: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "BTRT: transfer from the zero address");
        require(_recipient != address(0), "BTRT: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "BTRT: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "BTRT: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "BTRT: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "BTRT: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "BTRT: approve from the zero address");
        require(_spender != address(0), "BTRT: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "BTRT: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "BTRT: transfer exceeds your transaction count limit");
        }
    }
}