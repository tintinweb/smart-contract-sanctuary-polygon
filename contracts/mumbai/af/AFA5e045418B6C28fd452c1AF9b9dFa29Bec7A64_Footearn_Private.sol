...
contract Footearn_Private is ReentrancyGuard, Ownable {
    ...
    function buy(uint256 amount) public nonReentrant {
        ...
        if (whitelistWiner[msg.sender] == true) {
            require(
                amount <= maxBuyWiner && nextBuy <= maxBuyWiner,
                "Error: Invalid balance of buy"
            );
        } else if (whitelistRandom[msg.sender] == true) {
            require(
                amount <= maxBuyRandom && nextBuy <= maxBuyRandom,
                "Error: Invalid balance of buy"
            );
        } else if (
            whitelistRandom[msg.sender] == true &&
            whitelistWiner[msg.sender] == true
        ) {
            require(
                amount <= maxBuyRandom + maxBuyWiner &&
                    nextBuy <= maxBuyRandom + maxBuyWiner,
                "Error: Invalid balance of buy"
            );
        } else {
            revert("Error: Invalid whitelist");
        }
    ...
    function addWhitelistRandom(address[] memory _userAddress)
        public
        onlyOwner
    {
        for (uint256 index = 0; index < _userAddress.length; index++) {
            whitelistRandom[_userAddress[index]] = true;
        }
    }

    function removeWhitelistRandom(address[] memory _userAddress)
        public
        onlyOwner
    {
        for (uint256 index = 0; index < _userAddress.length; index++) {
            whitelistRandom[_userAddress[index]] = false;
        }
    }
}
