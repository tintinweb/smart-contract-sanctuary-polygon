...
contract FearWolfDistributor is AccessControl, Pausable, ReentrancyGuard, ERC1155Holder, EmergencyWithdrawer {
    ...
    /**
     * @dev Emitted when FEAR Wolf is bought (used for both presale claiming and public sale direct buying).
     */
    ...
    */
    ...
    // Prices in ETH
    uint256 public presalePrice = 1 * 10 ** 17; // 0.1 ETH
    uint256 public salePriceMax = 2 * 10 ** 17; // 0.2 ETH
    uint256 public salePriceMin = 1 * 10 ** 17; // 0.1 ETH

    // Dutch Auction - 0.01 ETH price drop every 2 hours
    uint256 public dutchAuctionStep = 1 * 10 ** 16;
    ...
    */
    ...
    // address => extra allocation
    mapping (address => uint256) private extraAlloccation;
    ...
    function _buyWolf(uint256 wolvesCount, uint256 price) private {
        ...
        for (uint i = 0; i < wolvesCount; i++) {
            // FIXME: UNCOMMENT THIS CALL IN REAL CONTRACT
    ...
    */
    ...
    /**
    * @dev Returns number of FearWolves reserved for the caller.
    * @return Number of FearWolves.
    */
    ...
    */
    ...
    function reserveWolf(uint256 wolvesCount) external payable whenNotPaused nonReentrant {
        address caller = _msgSender();
    ...
    */
    ...
    function setSaleState(bool state) external onlyRole(MANAGER_ROLE) {
        DEBUG_SALE_ACTIVE = state;

        if (!DEBUG_SALE_ACTIVE) {
            if (presaleWolvesCount == 0) {
                presaleWolvesCount = 100;
                saleWolvesCount -= 100;
            }
        }
    }
