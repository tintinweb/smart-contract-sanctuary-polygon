...
contract CrowdSourcingFactory {
    ...
    event CampaignCreated(address campaignAddress, uint campaignId);

    function createCrowdFundingCampaign(string memory _name, address _tokenAddress, uint256 _goalAmount) external {
        CrowdFundingCampaign newCampaign = new CrowdFundingCampaign(_name, _tokenAddress, _goalAmount, msg.sender);

        campaigns[campaignCount] = address(newCampaign);
        campaignCount++;

        emit CampaignCreated(address(newCampaign), campaignCount);
    }
}
