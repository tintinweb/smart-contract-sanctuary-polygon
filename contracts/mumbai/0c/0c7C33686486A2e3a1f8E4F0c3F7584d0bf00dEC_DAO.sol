...
contract CrowdFund {
    ...
    constructor(address _USDC, address _ownersAddress, uint _time, address _nft, uint _amountProposed, string memory _topic) {
...
contract crowdFundFactory {
    ...
    function createCrowdfund( address _USDC, address _ownersAddress, uint256 _salt, uint256 deadline, address _nft, uint _amountProposed, string memory _topic) public returns(address crowdFundAddr, address manager){
        CrowdFund crowdfund = (new CrowdFund){salt: bytes32(_salt)}(  _USDC,  _ownersAddress, deadline, _nft, _amountProposed, _topic);
   ...
   /// @param _topic: the caller inputs the _topic, _description, and amount of funding for the project
    ...
    function voteProposal(uint prosalID) external {
        ...
        Pis.deadline = _deadline + block.timestamp;
         ...
         deployCrowdFund(crowdFundFactoryAddr, USDC, beneficiary , prosalID, _deadline, stabeleBankNFT, amountProposed, name);
    ...
    //Helper functions
    function deployCrowdFund(address _crowdFundFactoryAddr, address _USDC, address _ownersAddress, uint256 _salt, uint256 deadline, address _nft, uint _amountProposed, string memory _topic) internal returns(address crowdFundAddr, address manager ) {
        
      return crowdFundFactory(_crowdFundFactoryAddr).createCrowdfund(_USDC, _ownersAddress, _salt, deadline, _nft, _amountProposed, _topic);
