...
contract MiniNovel is OwnableUpgradeable, ERC721Upgradeable, ERC721EnumerableUpgradeable, ERC2771ContextUpgradeable, Governance{
    ...
    event EnergyHelp(address indexed caretaker, uint256 indexed tokenid, string indexed name);
    ...
    function eat(uint256 tokenId) public {
        ...
        require(getStrength(tokenId) < 80, "im feeling too gross to eat");
    ...
    function play(uint256 tokenId) public {
        ...
        require(getStrength(tokenId) < 80, "im feeling too gross to play");
    ...
    function sleep(uint256 tokenId) public {
        ...
        require(ownerOf(tokenId) == _msgSender(), "not your story");
        require(getHealth(tokenId), "rest in hospital");
        require(getStrength(tokenId) < 80, "im feeling too gross to sleep");
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721Upgradeable) returns (string memory) {
        ...
        // solhint-disable-next-line quotes
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', getName(tokenId), '", "LofiTown Story are virtual avatar living on the blockchain.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
    ...
    function energy(uint256 tokenId) public {
        ...
        emit EnergyHelp(_msgSender(), tokenId, getName(tokenId));
    }
    ...
    function setLimit(uint256 new_limit, uint256 _grow) public onlyGovernance {
