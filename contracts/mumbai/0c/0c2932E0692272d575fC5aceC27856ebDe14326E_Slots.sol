...
contract Slots is VRFConsumerBaseV2 {
    ...
    function startRoll(uint256 iter) external payable returns (uint256 requestId) {
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override isVRFCoordinator {
        ...
        for (uint i = 0; i<(gamesStart[_requestId].numOfIterations); i++) {
            reward += endRoll(
                _requestId,
                (_randomWords[0] / 10 % 6),
                (_randomWords[0] % 100 / 10 % 6),
                (_randomWords[0] % 1000 / 100 % 6)
            );
            _randomWords[0] = _randomWords[0]/1000;
        }
    ...
    function endRoll(uint256 _requestId,uint256 firstDegigt, uint256 secondDigit, uint256 thirdDigit) private returns(uint256) {

        uint256 result = calculatePrize(firstDegigt, secondDigit, thirdDigit, (gamesStart[_requestId].deposit/(gamesStart[_requestId].numOfIterations)));
        if (result != 0) {
        ...
        emit GameResEvent(result, gamesStart[_requestId].player, firstDegigt, secondDigit, thirdDigit, gamesResult[gamesStart[_requestId].player].length);
        return(result);
    }
    ...
    function calculatePrize(
        ...
        if ((rand1 == 5 && rand2 == 5 && rand3 == 5) || (rand1 == 0 && rand2 == 0 && rand3 == 0)) {
            return deposit * 5;
