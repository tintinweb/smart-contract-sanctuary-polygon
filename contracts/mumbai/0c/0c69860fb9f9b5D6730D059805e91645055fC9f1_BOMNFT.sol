...
contract BlackList is Ownable {
    ...
    function setBlackList (address _evilUser, bool flag) public onlyOwner {
        isBlackListed[_evilUser] = flag;
        if (flag == true) emit AddedBlackList(_evilUser);
        else emit RemovedBlackList(_evilUser);
    }
...
contract BOMNFT is ERC721URIStorage, Ownable, LPPrice, BlackList {
    ...
    //public
    function mint(string memory _tokenURI, uint id) public payable  {
        require(!paused);
        require(supply + 1 <= allowedSupply);
        
        uint maticCost = usdCost  * 10 ** matic.decimals().div(currentPrice);
        if (id == 0) {
            require(msg.value >= maticCost);
        }
        else {
        ...
        owners[supply] = msg.sender;
        _mint(msg.sender, _tokenID);
        _setTokenURI(_tokenID, _tokenURI);
        updatePriceFromLP(); // This will update current Price
    }

    function mintNFTs(uint256 _count, string memory _tokenURI, uint id) public payable {
        require(!paused);
        require(supply + _count <= allowedSupply);

        // use Current Price here = USD/Matic
        uint nMaticCost = usdCost  * 10 ** matic.decimals().div(currentPrice).mul(_count);
        if (id == 0) {
            require(msg.value >= nMaticCost);
        }
        else {
            require(withdrawAddress != address(0));
            if      (id == 1) busd.transferFrom(msg.sender, withdrawAddress, usdCost * 10 ** busd.decimals() / 10 ** _decimals);
        ...
        uint _tokenID;
        for(uint _index = 0; _index < _count; _index++) {
            _tokenID = supply;
    ...
    // This function is called when NFT is minted, burned or transferred
    ...
    function getDecVal() internal override view returns (uint256) {
        return 10 ** _decimals;
    }
}
