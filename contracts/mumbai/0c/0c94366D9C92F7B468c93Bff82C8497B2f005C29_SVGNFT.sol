...
contract SVGNFT is ERC721URIStorage, VRFConsumerBaseV2 {
    ...
    Counters.Counter private _svgIdCounter;
    uint[] public minted;
  ...
  return requestId;
    ...
    function checkPoints(string[18] memory points) private pure returns(string memory _returnVal) {
        string[16] memory cPoints;
        for(uint i = 0; i < points.length; i++){
            if((i % 2) != 0){
                        string memory together = string(abi.encodePacked(points[i - 1], ',', points[i]));
                        cPoints[i] = together;
                    }
        }
        if(points.length == 3){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[4]));
        }
        if(points.length == 4){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[5], cPoints[7]));
        }
        if(points.length == 5){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[5], cPoints[7], cPoints[9]));
        }
        if(points.length == 6){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[5], cPoints[7], cPoints[9], cPoints[11]));
        }
        if(points.length == 7){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[5], cPoints[7], cPoints[9], cPoints[11], cPoints[13]));
        }
        if(points.length == 8){
            _returnVal = string(abi.encodePacked(cPoints[1], cPoints[3], cPoints[5], cPoints[7], cPoints[9], cPoints[11], cPoints[13], cPoints[15]));
        }
    }
   ...
   /* function create() public {
        ...
        string memory svg = manipulate(s_requestIdToRandomWords[s_requestIdToAddress[msg.sender]][0]);
        uint256 svgId = _svgIdCounter.current();
        _safeMint(msg.sender, svgId);
