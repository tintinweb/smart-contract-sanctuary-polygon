...
contract Holder {
    ...
    function parseReceiver(address adr, uint page, uint size) public view returns (transactionsInfo[] memory){
        transactionsInfo[] memory toReturn = new transactionsInfo[](size);
        ...
        for (uint i = start; i < end; i++) {
            toReturn[i] = transactionsHolder[receivers[adr].transactions[i]];
        }
        return toReturn;
    ...
    function parseSender(address adr, uint page, uint size) public view returns (transactionsInfo[] memory){
        transactionsInfo[] memory toReturn = new transactionsInfo[](size);
        ...
        uint start = (end > size) ? (end - size) : 0;
        for (uint i = start; i < end; i++) {
            toReturn[i] = transactionsHolder[senders[adr].transactions[i]];
        }
        return toReturn;

    }

    function parseTransactions(uint page, uint size) public view returns (transactionsInfo[] memory){
        transactionsInfo[] memory toReturn = new transactionsInfo[](size);
        if (transactionHolderSize < page * size) {
            return toReturn;
        }

        uint end = transactionHolderSize - page * size;
        uint start = (end > size) ? (end - size) : 0;
        for (uint i = start; i < end; i++) {
            toReturn[i] = transactionsHolder[i];
        }
        return toReturn;
    ...
    function parseTotalPaid(uint txnId) public view returns (uint){
        return transactionsHolder[txnId].currentTransaction;
    }
