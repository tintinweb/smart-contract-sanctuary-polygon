...
contract dExcountManager is Ownable, Pausable{    
    ...
    */
    ...
    function createPool(Parameters calldata store) external payable whenNotPaused {
        ...
        require(store.discount >= 0 && store.discount <= 90, "dExcountManager: Discount is invalid");
        ...
        pairInfo[address(store.token)].add(address(newPool));
     ...
     * @dev Returns true if the contract is in the discountpool. 
     */
    function saleTokenContains(address saleToken,address pool) public view returns (bool) {
        return pairInfo[saleToken].contains(pool);
    }

    /**
     * @dev Returns the number of discountPools.
     */
    function saleTokenLength(address saleToken) public view returns (uint256) {
        return pairInfo[saleToken].length();
    }
     ...
     * - `index` must be strictly less than {length}.
     */
    function saleTokenAt(address saleToken,uint256 index) public view returns (address) {
        return pairInfo[saleToken].at(index);
    }

    /**
     * @dev Return the entire discountPool in an array
     */
    function allSaleToken(address saleToken) public view returns (address[] memory) {
        return pairInfo[saleToken].values();
    }
}
