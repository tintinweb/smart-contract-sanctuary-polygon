...
interface ERC20Interface {
    ...
    function transfer(address to, uint tokens, string memory iname, string memory tname) external returns (bool);
    function approve(address spender, uint tokens) external returns (bool);
    function transferFrom(address from, address to, uint tokens, string memory iname, string memory tname, string memory cname) external returns (bool);
...
contract Clic is ERC20Interface {
    ...
    function claim(address owner, uint tokens) public returns (bool){
        require(tokens <= balances[owner]);
        claimed[owner] += tokens;
        return true;
    }

    function getClaimedbalance(address owner) public view returns (uint){
        return claimed[owner];
    }

    function getUnclaimedbalance(address owner) public view returns (uint){
        return balances[owner] - claimed[owner];
    }

    function transfer(address to, uint tokens, string memory iname, string memory tname) public returns (bool) {
        require(tokens <= balances[wallet_address]);
        if(bytes(iname).length > 0 && bytes(tname).length > 0){
    ...
    function transferFrom(address from, address to, uint tokens, string memory iname, string memory tname, string memory cname) public returns (bool) {
        require(tokens <= balances[from] - claimed[from]);
