...
interface IUniswapV2Router {
    /***********************************************************************************************************************
     @notice This Method Is Used By Contract To Get Expected Amount Out In USDC To ETH Swap.
    ************************************************************************************************************************/
...
interface IUniswapV3 {
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used By Contract To Swap USDC For ETH,
     Obtained WETH Will Be Unwrapped And Then ETH Will Be Transferred To Stacking Contracts.
    ************************************************************************************************************************/
...
interface IWETH {
    /***********************************************************************************************************************
     @notice This Method Is Used To Unwrap WETH Which Obtains From Uniswap In USDC To WETH Swap,
    ************************************************************************************************************************/
...
contract FixlineBetting is Ownable, ReentrancyGuard {
    ...
    uint256 public minMakerAmountPerSlotForStableCoinBet = 1000000;
    ...
    uint256 public minStableCoinSwapToETH = 20000;
    uint256 public totalBets;
    uint24 public uniswapV3PoolFeeForSwap = 500;
    uint256 public nextRewardTime;
    uint256 public pendingRewardsToSendInCapsStackingContract; // Make It Private
    uint256 public pendingStableCoinsToSwapForCapsStackingContract;
    uint256 public pendingRewardsToSendInFixlineStackingContract;
    uint256 public pendingStableCoinsToSwapForFixlineStackingContract;
    ...
    constructor(
        ...
        nextRewardTime = block.timestamp + (60 minutes / maxRewardsInDay); // Here Add 24 Hours
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used To Create New Bet Using ETH.
     User Will Be Not Able To Create Bet In Given Conditions,
     (1) If Slot Value Is Lesser Than 'minSlotsInBet'.
     (2) If slotPrice Is 0.
     (3) If expiryTime Is Not Greater Than Current Time.
     (4) If makerAmountPerBet Is Lesser Than 'minMakerAmountPerSlot'.

     @param slots, Number Of Slots In Bet.
     If slots Is N So Total N Number Of People Can Participate In Single Bet And They Will Play Against Bet Maker.
     @param slotPrice, Price Of Each Slot,
     Every User Have To Pay slotPrice To Participate In Bet,
     Slot Price Will Be Calculated On Frontend Side As Per Bet Maker Odds. 
     @param expiryTime, Expiry Time Of Bet.
     After Bet Expiry Time Any User Will Be Able To Claim Loss And Maker Can Cancel Bet. 
     @param betType, Type Of Bet, This Input Will Be Just Stored In Contract To Specify Type Of Bet. 
     Input Can Be:-
     0 -> Sports Bet
     1 -> Financial Bet
     2 -> Custom Bet
     @param data, Extra Data Related To The Bet,
     If There Is A Custom Bet So We Can Store Any Info Related To Bet By Passing As Input.
     @return betId, Id Of Bet.

     @note Bet Maker Have To Pass Amount For Each Slot In Contract.
     If Slot Price Of Bet Is 1 ETH And Bet Maker Have Passed 1 ETH And Total Slots Are 2,
     So Bet Maker Amount Per Slot Will Be 0.5 ETH (1 ETH Passed Amount / 2 Slots).
     So When Bet Taker Will Win They Will Get 90% Of (1 ETH Taker Deposited Amount + 0.5 ETH Maker Per Slot Amount),
     And When Bet Maker Will Win They Will Get 90% Of (1 ETH Maker Deposited Amount + Slot Takers Deposited Amount),

     @note (slots*subReputationAmount) Number Of Maker's Reputation Will Be Decrease.
     Consider User Reputation Is 10, Slots Value Is 3 And subReputationAmount Is 5,
     So Now User Reputation Will Be -5 (10 Current Reputation - (3 slots * 5 subReputationAmount)).

     Simple Example:-
     Consider User 'A' Have Created Bet And They Have Passed Parameters As Given Below,
     slots -> 2
     slotPrice -> 1000
     expiryTime -> 1111111111 (In Epoch Time) 
     betType -> 0
     data -> 0x4131205673204132 (A1 Vs A2)
     So In This Bet Only 2 People Can Participate And To Participate They Have To Pay 1000 Because Slot Price Is 1000.
     And Any User Cannot Claim Loss And Maker Cannot Cancel Bet Before 1111111111, Because Expiry Time Is 1111111111.
     And (2 slots * subReputationAmount) Number Of Maker's Reputation Will Be Decrease.
    ************************************************************************************************************************/
    ...
    function createBetUsingETH(
        ...
        require(
            slots >= minSlotsInBet,
            "Slots Should Greater Or Equal To Min Slot Value"
        );
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used To Create Bet Using Stable Coin (USDC).
     @note Bet Maker Have To Give (makerAmountPerSlot * slots) Amount Of Allowance To Contract.

     @param slots, Number Of Slots In Bet.
     @param makerAmountPerSlot, Amount Of Maker For Each Slot.
     @param slotPrice, Price Of Each Slot.
     @param expiryTime, Expiry Time Of Bet.
     @param betType, Type Of Bet, This Input Will Be Just Stored In Contract To Specify Type Of Bet. 
     @param data, Extra Data Related To The Bet.
     @return betId, Id Of Bet.
     @note Check createBetUsingETH Method's Comments To Get More Info About Params And Normal Bet Flow
    ************************************************************************************************************************/
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used To Buy ETH And Stable Coin Based Bet Slots Or Participate In Bets.
     @note Bet Taker Have To Pass ETH Amount For All ETH Based Bet Slots And Provide Allowance For Stable Coin Based Bets.
     User Will Be Not Able To Participate In Any Bet In Given Conditions,
     (1) If Bet It Is Expired.
     (2) If All Slots Of Bet Is Purchased.
     (3) If Bet Is Cancelled.
     (4) If User Passing Less Amount Then Slot Price.
     (5) If Sufficient Allowance Is Not Provided To Contract And Sufficient ETH Are Not Passed To Contract.
     (6) If User Is Accidentally Passing ETH.

     @param betIds, Bet Id's Of Bets In Which User Wants To Participate.

     Consider User Wants To Participate In Two Bets.
     [{
        betId : 1,
        slotPrice: 1 ETH
      },
     ...
     So Here User Have To Pass Total 1.5 ETH (1 ETH + 0.5 ETH) In Contract,
     And Provide Total 3.5 USDC Allowance To Contract.

     @note participatedInTotalBets*subReputationAmount Number Of Bet Taker's Reputation Will Be Decrease.
     Consider User Reputation Is 10, participatedInTotalBets Or betIds.length Is 3 And subReputationAmount Is 5,
     So Now User Reputation Will Be -5 (10 Current Reputation - (3 slots * 5 subReputationAmount)).
    ************************************************************************************************************************/
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used To Cancel Bets.
     User Will Be Not Able To Cancel Any Bet In Given Conditions,
     (1) If User Is Not Maker Of Bet.
     (2) If Bet Is Not Expired.
     (3) If Bet Already Cancelled.

     @param betIds, Bet Id's Of Bets Which User Wants To Cancel.

     @note In Each Bet (unsoldSlots*makerAmountPerBet) Amount Will Be Added In Users Withdrawable Balance.
     @note In Each Bet (unsoldSlots*subReputationAmount) Number Of Bet Maker's Reputation Will Be Incremented.
     Consider User Reputation Is -5, unsoldSlots Is 3 And subReputationAmount Is 5,
     So Now User Reputation Will Be 10 (-5 Current Reputation+(3 slots * 5 subReputationAmount)).
    ************************************************************************************************************************/
    ...
    function sendRewardsToStackingContract(
        ...
        if (
            ...
            (block.timestamp >= (nextRewardTime + 60 minutes)) // Here Add 24 Hours
        ) {
            if (
                (totalPendingStableCoinsToSwapForCapsStackingContract +
                    totalPendingStableCoinsToSwapForFixlineStackingContract) >=
                ...
                pendingStableCoinsToSwapForCapsStackingContract = 0;
                pendingStableCoinsToSwapForFixlineStackingContract = 0;
            }
    ...
    ************************************************************************************************************************/
    function declareLooserByAdmin(
        uint256 betId,
        bool isMakerLoss
    ) public onlyOwner isValidBetId(betId) {
        Bet memory tempBetData = betsData[betId];
        require(
            tempBetData.expiryTime <= block.timestamp,
            "Loss Can Be Only Claimed After Bet Expiry Time"
        );
        require(
    ...
    /***********************************************************************************************************************
     ...
     @note (soldSlots*addReputationAmount) Number Of Bet Maker's Reputation Will Be Incremented,
     ...
     90%-> (90% / Total takers) Reward To Each Taker.
     5% -> Maker.
     3% -> Admin.
     ...
     Now Consider makerAmountPerSlot Is 1 ETH, slotPrice Is 0.5 ETH And Total Sold Slots Are 3.
    ...
    ************************************************************************************************************************/
    function claimLossByTaker(uint256 betId) public nonReentrant {
        ...
        (
            uint256 userBetSlotNum,
            uint256[] memory userBetSlotIndex
        ) = getUserBetSlotNumAndIndex(msgSender, betId);
        require(
            userBetSlotNum != 0,
            "You Cannot Claim Loss Because You Have Not Purchased Slot"
        );
        uint totalSlotsClaimed = 0;
        for (uint256 i = 0; i < userBetSlotNum; ) {
            if (tempBetData.isLossClaimed[userBetSlotIndex[i]] == false) {
                takerLoss(
                    betId,
                    msgSender,
                    userBetSlotIndex[i],
        ...
        require(
            totalSlotsClaimed != 0,
            "You Have Already Claimed Loss For Your All Slots"
        );
    }
    ...
    ************************************************************************************************************************/
    ...
    /***********************************************************************************************************************
     @notice This Method Is Used To Provide Allowance Of StableCoin To Uniswap SwapRouter Contract.
     @note Actually Allowance Of StableCoin To Uniswap SwapRouter Contract Is Provided At Time Of Contract Deployment,
     Using This Method Allowance Can Be Increased Or Decreased.
     @param amount, Amount Of Allowance Admin Wants To Give To Uniswap SwapRouter Contract.
    ************************************************************************************************************************/

    function approveToUniswapSwapRouter(uint256 amount) external onlyOwner {
        stableCoin_Ins.approve(uniswapV3SwapRouterAddress, amount);
    }

    /********** Setter Functions **********/

    function setSubReputationAmount(
        uint256 _subReputationAmount
    ) external onlyOwner {
        subReputationAmount = _subReputationAmount;
    }

    function setAddReputationAmount(
        uint256 _addReputationAmount
    ) external onlyOwner {
    ...
    function setWinnerRewardPortion(
        uint256 _winnerRewardPortion
    ) external onlyOwner {
        winnerRewardPortion = _winnerRewardPortion;
    }

    function setLooserRewardPortion(
        uint256 _looserRewardPortion
    ) external onlyOwner {
        looserRewardPortion = _looserRewardPortion;
    }

    function setAdminRewardPortion(
        uint256 _adminRewardPortion
    ) external onlyOwner {
    ...
    function setMinStableCoinSwapToETH(
        uint256 _minStableCoinSwapToETH
    ) external onlyOwner {
        minStableCoinSwapToETH = _minStableCoinSwapToETH;
    }

    function setUniswapV3PoolFeeForSwap(
        uint24 _uniswapV3PoolFeeForSwap
    ) external onlyOwner {
        uniswapV3PoolFeeForSwap = _uniswapV3PoolFeeForSwap;
    }
    ...
    function setCapsTokenContract_Ins(
        address _capsTokenContractAddress
    ) external onlyOwner {
        capsTokenContract_Ins = ICapsToken(_capsTokenContractAddress);
    }

    function setCapsStackingContract_Ins(
        address _capsStakingContractAddress
    ) external onlyOwner {
        capsStakingContract_Ins = ICapsStaking(_capsStakingContractAddress);
    }

    function setStableCoinContract_Ins(
    ...
    function getUserBetSlotNumAndIndex(
        address userAddress,
        uint256 betId
    ) public view returns (uint256, uint256[] memory) {
        address[] memory _slotTakers = betsData[betId].slotTakers;
        uint256 totalSlotTakers = _slotTakers.length;
        uint256 totalSlotsTakenByUser = 0;
        uint256[] memory slotIndex = new uint256[](totalSlotTakers);

        for (uint256 i; i < totalSlotTakers; ) {
            if (_slotTakers[i] == userAddress) {
                slotIndex[totalSlotsTakenByUser] = i;
                unchecked {
                    totalSlotsTakenByUser++;
                }
            }
            unchecked {
                i++;
        ...
        return (totalSlotsTakenByUser, slotIndex);
    }
...
FixlineBetting:- 0x52ee1a38322B7b1c9F8533b472877aFbf282Ea60
*/