...
contract LendingPool is Ownable, ReentrancyGuard {
    ...
    function deposit(
        ...
        require(
            _poolInfo.endDate > block.timestamp,
    ...
    function claim(
        ...
        uint256 withdrawableAmount = _claimInterest(recordId, _recordInfo, _poolInfo, priceFeed, signature);
    ...
    function _getWithdrawableAmount(
        ...
        return
            ((depositAmount * APY * rateSum) / 36500) - claimedAmount;
    }
    ...
    function _checkPriceFeedSignature(
        ...
        require(
            block.timestamp < priceFeed.expireAt,
            "Lending-pool: signature expired"
        );
        require(
        ...
        (, bool _isValid) = SignatureUtils.isValid(
            hashed,
            signer,
            signature
        );
    ...
    function getUserRecords(
        ...
        uint256 offset,
        uint256 limit
    ) public view returns (RecordInfo[] memory) {
        ...
        for (uint256 i = offset; i < _recordCount && i < offset + limit; i++) {
            _records[i] = userRecords[userAddr].recordMap[i];
        }
        return _records;
    }
}
