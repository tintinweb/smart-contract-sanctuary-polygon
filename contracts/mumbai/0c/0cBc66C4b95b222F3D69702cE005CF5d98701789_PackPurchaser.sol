...
interface IPackPurchaserCustomHandler {
...
import "./interfaces/IPackPurchaserCustomHandler.sol";

contract PackPurchaser is Initializable, AccessControlUpgradeable {
  ...
  mapping(uint16 => mapping(uint8 => Pack)) public packs;
  mapping(uint16 => mapping(uint8 => mapping(address => PaymentToken)))
    public paymentTokens;
  mapping(uint16 => mapping(uint8 => bool)) public enforceTokenPayment;
  mapping(uint16 => mapping(uint8 => IPackPurchaserCustomHandler))
    public customHandlers;
  mapping(uint16 => mapping(uint8 => uint256)) public packsSold;
  ...
  function purchasePacks(
    ...
    require(to != address(0), "PackPurchaser: Invalid recipient");

    Pack memory pack = packs[seriesId][editionId];

    require(amount <= pack.maxPerTx, "PackPurchaser: Too many packs");
    require(
      amount + packsSold[seriesId][editionId] <= pack.availableAmount,
      "PackPurchaser: Not enough packs"
    );

    processPayment(msg.sender, seriesId, editionId, amount, paymentToken);
    packsSold[seriesId][editionId] += amount;
    pack.collection.mint(to, amount, seriesId, editionId);
  }
  ...
  function processPayment(
    ...
    if (address(paymentToken) != address(0)) {
      require(
        paymentTokens[seriesId][editionId][address(paymentToken)]
          .priceInTokens > 0,
        "PackPurchaser: Payment token not accepted"
      );
        ...
        require(
          msg.value >=
            paymentTokens[seriesId][editionId][address(paymentToken)]
              ...
              amount,
          "PackPurchaser: Not enough payment"
        );
      }
    ...
    if (!paymentProcessed && pack.priceInNative > 0) {
      require(
        !enforceTokenPayment[seriesId][editionId],
        "PackPurchaser: Token payment required"
      );

      uint256 totalPrice = pack.priceInNative * amount;
      require(
        msg.value == totalPrice,
        totalPrice > msg.value
          ? "PackPurchaser: Not enough payment"
          : "PackPurchaser: Too much payment"
      );
    }

    if (address(customHandlers[seriesId][editionId]) != address(0)) {
      customHandlers[seriesId][editionId].onPackPurchase{value: msg.value}(
        purchaser,
        amount,
        seriesId,
        editionId
  ...
  {
    enforceTokenPayment[seriesId][editionId] = !enforceTokenPayment[seriesId][
      editionId
    ];
  ...
  function setCustomHandler(
    uint16 seriesId,
    uint8 editionId,
    IPackPurchaserCustomHandler handler
  ) external onlyRole(MANAGER_ROLE) {
  ...
  function withdrawTokens(IERC20 token) external onlyRole(WITHDRAWER_ROLE) {
    ...
    require(balance > 0, "PackPurchaser: Nothing to withdraw");
