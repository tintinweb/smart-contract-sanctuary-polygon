...
contract AuctionPureV2 is Initializable {
    ...
    function listingNFTAuction(
        ...
        bool _isWhiteList,
        AuctionStructure.feeSystem calldata _feeSystem
        ...
        require(
            (paramStanFund.result &&
                paramStanFund.userStanFund >= _feeSystem.stanFee) ||
                _isWhiteList,
            "InvalidBalance"
        );
    ...
    function cancelListingAuction(
        ...
        bool _isWhiteList,
        AuctionStructure.feeSystem calldata _feeSystem
    ) external onlyAuction returns (uint256, AuctionStructure.Currency) {
    ...
    function placeBidAuction(
        ...
        onlyAuction
        returns (uint256, AuctionStructure.Currency)
    {
        AuctionStructure.stanFundParams memory paramStanFund = stanFund.get(
            _maker
        );
        ...
        require(
            (paramStanFund.result &&
                paramStanFund.userStanFund >= (_amount + _feeSystem.stanFee)) ||
                _isWhiteList,
            "InvalidBalance"
        );
        AuctionStructure.Listing memory listingInstance = listing
        ...
        if (!_isWhiteList)
            stanFund.set(
                (_reOffer(paramsReOffer) + _feeSystem.stanFee),
                AuctionStructure.Operator.MINUS,
                _maker
            );
    ...
    function acceptOfferAuction(
        ...
        return (infoInstance.tokenId, listingInstance.currency);
    }
    ...
    function _processFinishAuction(
        ...
        AuctionStructure.feeSystem calldata feeSystem
    ...
    function finishAuction(
        ...
        onlyAuction
        checkStateOfAution(_auctionId)
        returns (AuctionStructure.Currency, uint256)
    {
        AuctionStructure.Listing memory listingInstance = listing
            .getInforListing(_auctionId);
        ...
        return (listingInstance.currency, tokenId);
    }
}
