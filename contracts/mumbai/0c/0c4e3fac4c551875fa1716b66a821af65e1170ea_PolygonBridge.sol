...
contract PolygonBridge is Auth, EIP712Upgradeable {
  ...
  event TokenReleased(address[] receivers, uint[] quantities, uint totalQuantity, uint timestamp);
  ...
  function release(address[] calldata _receivers, uint[] calldata _quantities, uint _totalQuantity, bytes memory _signature) external onlyContractAdmin {
    ...
    emit TokenReleased(_receivers, _quantities, _totalQuantity, block.timestamp);
  }
