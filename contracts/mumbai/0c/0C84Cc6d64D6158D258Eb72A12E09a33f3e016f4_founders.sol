...
contract founders is Ownable, ReentrancyGuard, AccessControl {
    ...
    event UpdatePerTransactionCap(uint256 indexed perTransactionCap);
    event UpdateSignerAddress(address indexed signerAddress);
    event UpdateNFTContractAddress(address indexed nftContractAddress);
    event UpdateWithdrawAddress(address indexed withdrawAddress);
    ...
    function leaderboardMint(
        ...
        require(
            _msgHash.recover(_signature) == signerAddress,
    ...
    function updateSignerAddress(address _signerAddress) public {
        require(
            hasRole(ADMIN_ROLE, _msgSender()),
            "Founder Contract: Must have admin role to update"
        );
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_to, _tokenId, _category));
    }
