...
import "./IRegisterController.sol";

/**
 * Register an DID name by pay ETH only once.
 */
contract ETHController is BaseController, IRegisterController {
    // pay register fee as eth:
    uint256 public fee = 0 ether;

    constructor(address _registry) BaseController(_registry) {}

    function setEthRegisterFee(uint256 _fee) external onlyOwner {
        fee = _fee;
    }

    // register by ETH:

    function register(string memory name) external payable override {
