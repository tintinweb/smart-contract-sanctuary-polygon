...
contract LedgerManager is ReentrancyGuard {
    ...
    */
    function addOrder(LedgerTypes.OrderInfo memory orderInfo, uint256 endsInSec, uint256 deadline, uint256 rewardBaseAmount, bytes memory signature) external payable returns (uint256) {
        ...
        dailyLimit.withdrawReward(msg.sender, reward);
...
contract ProfitSplitter {
    ...
    */
    function _setSplitConfig(address[] memory splitConfigAddresses_, uint16[] memory splitConfigFeePercent_) internal {
        require(splitConfigAddresses_.length == splitConfigFeePercent_.length, "Arr length mismatch");

        delete splitConfig;
        uint16 percentSum = 0;
        for (uint256 i = 0; i < splitConfigAddresses_.length; i++) {
    ...
    */
    function _splitReward(uint256 profit_, IAsset asset_) internal {
        for (uint256 i = 0; i < splitConfig.length; i++) {
...
import "./external/NonblockingLzAppUpgradeable.sol";
...
contract PaypoolV1ERC20 is NonblockingLzAppUpgradeable, ERC20Upgradeable, PausableUpgradeable, IOFTUpgradeable {
    ...
    function setEndpoint(ILayerZeroEndpoint _lzEndpoint) external onlyOwner {
...
contract DailyLimit {
    ...
    function _recalculatePool(uint256 time) internal view returns(uint256, uint256) {
        ...
        for (uint256 i = lastScheduleIndex; i < schedules.length; i++) {
        ...
        for (uint256 i = lastScheduleIndex + 1; i > firstScheduleIndex; i--) {
    ...
    function _calculateReward(uint256 reward_, uint256 currentlyInPool_, uint256 scheduleIndex_) internal view returns(uint256) {
        ...
        if (currentlyInPool_ > schedule.maxPerDay) {
            return reward_ * _findMiddle(schedule.maxPerDay, currentlyInPool_) / 10000;
        } else {
            return reward_ * _findMiddle(currentlyInPool_, schedule.maxPerDay) / 10000;
        }
    }
    ...
    function withdrawReward(address userAddress, uint256 userReward) external onlyManagerRole() {
        ...
        assert(rewardToken.transfer(userAddress, reward));
