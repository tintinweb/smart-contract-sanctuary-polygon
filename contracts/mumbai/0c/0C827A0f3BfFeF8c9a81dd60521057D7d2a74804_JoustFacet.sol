...
contract JoustFacet {
    ...
    function getBatchRawUnicornRecord(
    ...
    function getEmbeddedJoustLosses(
        uint256 unicornRecord,
        uint256 losses
    ) public view returns (uint256) {
    ...
    function getEmbeddedJoustTournamentWins(
        uint256 unicornRecord,
        uint256 tourneyWins
    ) public view returns (uint256) {
    ...
    function getEmbeddedJoustEloScore(
        uint256 unicornRecord,
        uint256 eloScore
    ) public view returns (uint256) {
        ...
        )
    {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            (
                ...
                tournamentsWon[i],
                eloScores[i]
            ) = LibElo._getJoustRecord(tokenIds[i]);
        }
