...
// File contracts/PancakePredictionV2.sol
...
contract PancakePredictionV2 is Ownable, Pausable, ReentrancyGuard {
    ...
    function executeRound() external whenNotPaused onlyOperator {
        ...
        (uint80 roundID, int256 price) = _getPriceFromOracle();

        oracleLatestRoundId = uint256(roundID);

        // CurrentEpoch refers to previous round (n-1)
        _safeLockRound(currentEpoch,roundID, price);
        _safeEndRound(currentEpoch - 1, roundID, price);
...
// File contracts/test/MockAggregatorV3.sol
