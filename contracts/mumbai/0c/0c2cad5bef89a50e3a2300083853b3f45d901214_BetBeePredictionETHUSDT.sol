...
contract PredictionAdministrator is Ownable, Pausable, ReentrancyGuard {
    ...
    */
    ...
    function claimTreasury() external nonReentrant onlyAdmin notContract {
        uint256 claimableTreasuryAmount = ((address(this).balance * claimableTreasuryPercent) / 100);
...
/**
 * @title BetBeePredictionETHUSDT
 */
contract BetBeePredictionETHUSDT is PredictionAdministrator {

    uint256 public currentRoundId;
    uint256 public roundTime = 300; //5 mintues of round
    uint256 public bufferSeconds = 30; // buffer in seconds. default 30 seconds

    bool public genesisStartOnce = false;
    bool public genesisCreateOnce = false;

    enum RoundState {UNKNOWN, CREATED, STARTED, ENDED, DISPERSED, CANCELLED}
    ...
    /**
    * @notice Set Buffer Seconds
    * @param _bufferSeconds: buffer in seconds
    */
    function setBufferSeconds(uint256 _bufferSeconds) external whenPaused onlyAdmin notContract {
        require(_bufferSeconds > 0, "BufferSeconds cannot be zero");
    ...
    */
    function _endRound(uint256 roundId, int256 endPrice) internal {
        ...
        rounds[roundId].endTimestamp = block.timestamp;
    ...
    */
    function cancelAndRefundRound(uint256 _roundId) external onlyOperator notContract {
        require(rounds[_roundId].roundState == RoundState.CREATED || rounds[_roundId].roundState == RoundState.STARTED, "Round is not created/started or ended already");
    ...
    */
    function genesisCreateRound() external whenNotPaused onlyOperator notContract {
    ...
    * @return EndPrice
    */
    function getRound(uint256 _roundId) view external returns(uint256, uint256, uint256, int256, int256) {
        Round storage round = rounds[_roundId];
        return(uint256(round.roundState), round.bullAmount, round.bearAmount, round.startPrice, round.endPrice);
    }
    ...
    * @return userBullAmount
    * @return userBearAmount
    */
    function getUserBetAmount(uint256 _roundId, address _userAddress) view external returns(uint256, uint256) {
        return(ledger[_roundId][_userAddress].bullAmount, ledger[_roundId][_userAddress].bearAmount);
    }

    /**
    * @notice get endtime of current round
    * @return endTimestamp
    */
    function getCurrentRoundEndTime() view external returns(uint256) {
