...
import "0xa-template-contracts/contracts/ERC1155/ERC1155AdminMintable.sol";
import "0xa-template-contracts/contracts/ERC1155/ERC1155AdminBurnable.sol";
import "0xa-template-contracts/contracts/ERC1155/ERC1155FreeMintable.sol";
import "0xa-template-contracts/contracts/ERC1155/ERC1155OperatorApprovable.sol";
import "0xa-template-contracts/contracts/common/MetaTX.sol";

contract FanPass is ERC1155AdminMintable, ERC1155AdminBurnable, ERC1155FreeMintable, ERC1155OperatorApprovable, MetaTX {
    uint256 constant TOKEN_ID = 0; // all tokens have the same id=0
    ...
    constructor(
        ...
        uint256[] memory _tokenMaxSupplies,
        address[] memory _adminAddresses
    )
        ERC1155FreeMintable(_name, _symbol, _baseURI, _contractURI, _tokenMaxSupplies)
        AdminControlled(_adminAddresses)
        MetaTX(_name, _version)
    {
     ...
     */
    function setFanPassTokenSupply(uint256 _maxSupply) external onlyAdminAddress {
        _setTokenMaxSupply(TOKEN_ID, _maxSupply);
    }
    ...
    function isApprovedForAll(address _owner, address _operator)
        public
        view
        override(ERC1155, ERC1155OperatorApprovable)
        returns (bool)
    {
        return ERC1155OperatorApprovable.isApprovedForAll(_owner, _operator);
    }

    /**
     * @dev This is used instead of msg.sender as transactions won't be sent by the original token owner, but by OpenSea.
     */
    function _msgSender() internal view override(MetaTX, Context) returns (address sender) {
        return MetaTX._msgSender();
    }
}
...
abstract contract ERC1155AdminMintable is ERC1155Base, AdminControlled {
    /**
    ...
    function adminBatchMint(address[] memory accounts, uint256[] memory tokenIDs) external onlyAdminAddress {
        ...
        for (uint256 i = 0; i < len; ) {
            require(totalSupply(tokenIDs[i]) < totalMaxSupply[tokenIDs[i]], "Max supply exceeded");
            _mint(accounts[i], tokenIDs[i], 1, "");
...
abstract contract ERC1155AdminBurnable is ERC1155Base, AdminControlled {
    /**
     * @notice Allows admin to burn NFTs from the provided addresses
     * @param accounts The list of recipient addresses
     * @param tokenIDs The list of token IDs to be burned
     */
    function adminBatchBurn(address[] memory accounts, uint256[] memory tokenIDs) external onlyAdminAddress {
        uint256 len = accounts.length;

        require(len == tokenIDs.length, "LENGTH_MISMATCH");

        for (uint256 i = 0; i < len; ) {
            _burn(accounts[i], tokenIDs[i], 1);
...
abstract contract ERC1155FreeMintable is ERC1155Base, ReentrancyGuard, Pausable {
    constructor(
        string memory _name,
        string memory _symbol,
        string memory _baseURI,
    ...
    ) ERC1155Base(_name, _symbol, _baseURI, _contractURI) {
        uint256 len = _maxSupplies.length;
        for (uint256 i = 0; i < len; ) {
            totalMaxSupply[i] = _maxSupplies[i];
    ...
    /**
     * @notice Allows user to mint a token for free
     * @param account The recipient address
     * @param tokenId The token ID to be minted
     */
    function mint(address account, uint256 tokenId) external nonReentrant whenNotPaused {
        require(totalSupply(tokenId) < totalMaxSupply[tokenId], "Maximum total supply exceeded");
    ...
    function _setTokenMaxSupply(uint256 tokenId, uint256 maxSupply) internal {
        totalMaxSupply[tokenId] = maxSupply;
    }
}
...
abstract contract ERC1155OperatorApprovable is ERC1155Base {
    address public approvedOperator;

    /**
     * @dev See {IERC1155-isApprovedForAll}. Returns true if `_operator` is approved operator
     */
...
abstract contract MetaTX is NativeMetaTransaction, ContextMixin {
    string public version;

    constructor(string memory _name, string memory _version) NativeMetaTransaction(_name, _version) {
        version = _version;
    }
...
abstract contract ERC1155Base is ERC1155Supply, Ownable {
    string public name;
    string public symbol;
    string public contractURI;

    mapping(uint256 => uint256) public totalMaxSupply;
