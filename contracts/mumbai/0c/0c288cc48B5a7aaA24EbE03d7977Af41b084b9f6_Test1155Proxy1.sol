...
//    ╚═╝   ╚══════╝╚══════╝   ╚═╝        ╚═╝ ╚═╝╚══════╝╚══════╝    ╚═╝     ╚═╝  ╚═╝ ╚═════╝ ╚═╝  ╚═╝   ╚═╝        ╚═╝
//                                                                                                                     
// test 1155 proxy 1 - generated with HeyMint.xyz Launchpad - https://nft-launchpad.heymint.xyz
// 
...
contract Test1155Proxy1 {
  ...
  fallback() external payable {
    ...
    assembly {
      ...
      let result := delegatecall(gas(), implContract, 0, calldatasize(), 0, 0)
...
abstract contract ERC1155UDS is EIP712PermitUDS('ERC1155Permit', '1') {
  ...
  function _mint(
    ...
    _beforeTokenTransfer(msg.sender, address(0), to, id, amount, data);
  ...
  function _batchMint(
    ...
    _beforeBatchTokenTransfer(msg.sender, address(0), to, ids, amounts, data);
  ...
  function _batchBurn(
    ...
    _beforeBatchTokenTransfer(msg.sender, from, address(0), ids, amounts, '');
  ...
  function _burn(address from, uint256 id, uint256 amount) internal virtual {
    _beforeTokenTransfer(msg.sender, from, address(0), id, amount, '');
  ...
  IERC2981Upgradeable
{
  ...
  function royaltyInfo(
    ...
    HeyMintStorage.State storage state = HeyMintStorage.state();
    address payoutAddress = state.advCfg.royaltyPayoutAddress != address(0x0)
  ...
  function setRoyaltyPayoutAddress(
    ...
    HeyMintStorage.state().advCfg.royaltyPayoutAddress = _royaltyPayoutAddress;
  }
}
  ...
  function withdraw() external nonReentrant onlyOwner {
    ...
    if (state.advCfg.payoutAddresses.length == 0) {
      ...
      for (uint256 i = 0; i < state.advCfg.payoutAddresses.length; i++) {
        uint256 amount = (balance * state.advCfg.payoutBasisPoints[i]) / 10000;
...
contract HeyMintERC1155ExtensionC is HeyMintERC1155Upgradeable {
  ...
  function presaleMint(
    ...
    require(tokenPresaleTimeIsActive(_tokenId), 'PRESALE_TIME_IS_NOT_ACTIVE');
    require(
      !state.data.tokenMintingPermanentlyDisabled[_tokenId],
      'MINTING_PERMANENTLY_DISABLED'
    );
...
contract HeyMintERC1155ExtensionD is HeyMintERC1155Upgradeable {
   ...
   * @notice Updates the base configuration for the contract
   */
  function updateBaseConfig(BaseConfig memory _baseConfig) external onlyOwner {
  ...
  function _updateAdvancedConfig(
    ...
    if (state.advCfg.payoutAddressesFrozen) {
      require(_advancedConfig.payoutAddressesFrozen, 'PAYOUT_ADDRESSES_FROZEN');
      bool payoutInfoChanged = false;
      for (uint256 i = 0; i < _advancedConfig.payoutAddresses.length; i++) {
        if (
          _advancedConfig.payoutAddresses[i] != state.advCfg.payoutAddresses[i]
        ) {
  ...
  function upsertToken(TokenConfig memory _tokenConfig) external onlyOwner {
    HeyMintStorage.State storage state = HeyMintStorage.state();
    require(
      _tokenConfig.maxSupply >= state.data.totalSupply[_tokenConfig.tokenId],
      'MAX_SUPPLY_LESS_THAN_TOTAL_SUPPLY'
    );
    require(
    ...
    require(
      state.tokens[_tokenConfig.tokenId].presaleMaxSupply == 0 ||
        state.tokens[_tokenConfig.tokenId].presaleMaxSupply >=
        _tokenConfig.presaleMaxSupply,
      'MAX_SUPPLY_CANNOT_BE_INCREASED'
    );
    require(
  ...
  function updateFullConfig(
    ...
    HeyMintStorage.State storage state = HeyMintStorage.state();
    require(
      _burnTokens.length == 0 || _burnTokens.length == _tokenConfigs.length,
      'BURN_TOKENS_LENGTH_MUST_MATCH_TOKENS_LENGTH'
    );
    ...
    for (uint256 i = 0; i < _tokenConfigs.length; i++) {
      state.tokens[_tokenConfigs[i].tokenId] = _tokenConfigs[i];
      state.data.tokenIds.push(_tokenConfigs[i].tokenId);
      if (_burnTokens.length > 0) {
        delete state.burnTokens[_tokenConfigs[i].tokenId];
        for (uint256 j = 0; j < _burnTokens[i].length; j++) {
          state.burnTokens[_tokenConfigs[i].tokenId].push(_burnTokens[i][j]);
        }
      }
    }
  }
  ...
  function setTokenUri(
    ...
    require(!state.data.tokenMetadataFrozen[_tokenId], 'TOKEN_METADATA_FROZEN');
...
contract HeyMintERC1155ExtensionE is HeyMintERC1155Upgradeable {
  ...
  function creditCardMint(
    ...
    for (uint256 i = 0; i < state.advCfg.creditCardMintAddresses.length; i++) {
...
contract HeyMintERC1155ExtensionF is HeyMintERC1155Upgradeable {
  ...
  function refundPriceInWei(uint16 _tokenId) public view returns (uint256) {
    return
      uint256(HeyMintStorage.state().tokens[_tokenId].refundPrice) * 10 ** 13;
  }
  ...
  function burnPaymentInWei(uint16 _tokenId) public view returns (uint256) {
    return
      uint256(HeyMintStorage.state().tokens[_tokenId].burnPayment) * 10 ** 13;
  }
  ...
  function burnToMint(
    ...
    require(msg.value == burnPaymentTotal + heymintFee, 'INVALID_PRICE_PAID');
    if (state.cfg.heyMintFeeActive) {
      (bool success, ) = heymintPayoutAddress.call{value: heymintFee}('');
      require(success, 'TRANSFER_FAILED');
    }
    for (uint256 i = 0; i < state.burnTokens[_tokenId].length; i++) {
      ...
      require(burnToken.contractAddress == _contracts[i], 'INCORRECT_CONTRACT');
      if (burnToken.tokenType == 1) {
        ...
        require(
          burnContract.balanceOf(msg.sender, burnToken.tokenId) >= amountToBurn,
          'MUST_OWN_TOKEN'
        );
    ...
    state.data.totalSupply[_tokenId] += uint16(_tokensToMint);
...
contract HeyMintERC1155Reference {
  ...
  function tokensMintedByAddress(address _address, uint16 _tokenId) external view returns (uint16) {}
  ...
  RevokableOperatorFiltererUpgradeable
{
  ...
  function _beforeTokenTransfer(
    ...
    uint256 amount,
    bytes memory data
  ) internal override(ERC1155UDS) notPaused {
    HeyMintStorage.State storage state = HeyMintStorage.state();
  ...
  ERC2981
{
  ...
  mapping(address => mapping(uint256 => uint256)) public tokensMintedByAddress;
  ...
  function freezeTokenMetadata(uint256 _tokenId) external onlyOwner {
    require(!tokenMetadataFrozen[_tokenId], 'METADATA_HAS_ALREADY_BEEN_FROZEN');
  ...
  function reduceMaxSupply(
    ...
    require(
      tokenMaxSupply[_tokenId] == 0 || _newMaxSupply < tokenMaxSupply[_tokenId],
   ...
   * @notice Lock a token id so that it can never be minted again
   */
  function permanentlyDisableTokenMinting(uint256 _tokenId) external onlyOwner {
  ...
  function presaleMint(
    ...
    require(
      tokenPresaleMaxSupply[_tokenId] == 0 ||
        totalSupply(_tokenId) + _numTokens <= tokenPresaleMaxSupply[_tokenId],
      'MAX_SUPPLY_EXCEEDED'
    );
    require(
