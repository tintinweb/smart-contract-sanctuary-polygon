...
contract CustomPaymaster is BasePaymaster {
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        require(
            relayRequest.relayData.paymasterData.length >= 4,
            "paymastaData: missing method sig"
        );

        require(
            IERC20Permit.permit.selector ==
                GsnUtils.getMethodSig(relayRequest.relayData.paymasterData),
        ...
        uint256 tokenPreCharge = _calculatePreCharge(
            relayRequest,
            maxPossibleGas
        );
    ...
    function postRelayedCall(
        ...
        uint256 tokenActualCharge = _calculateActualCharge(
            gasUseWithoutPost.add(gasUsedByPost),
            relayData
        );

        uint256 tokenBalance = token.balanceOf(address(this));
        token.safeTransfer(payer, tokenBalance - tokenActualCharge);
        token.transfer(feeCollector, tokenActualCharge);

        emit TokensCharged(gasUseWithoutPost, gasUsedByPost, tokenActualCharge);
    }
}
