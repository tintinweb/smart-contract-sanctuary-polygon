...
contract Product020802V01 is OwnableV01 {
    ...
    function endorsePolicy(
        Product020802Lib.EndorsmentPolicy memory endorsmentPolicy,
        bool updateSelectedOptions,
        bool updateCoverages
    ) external onlyPartnerOrWakam(allPolicies[endorsmentPolicy.onChainPolicyId].partnershipCodeHash) {
        if (endorsmentPolicy.coverageStartDate != 0) {
            uint256 coverageStartDate = endorsmentPolicy.coverageStartDate.computeCoverageStartDate(
                allPolicies[endorsmentPolicy.onChainPolicyId].subscriptionDate,
                TIME_ZONE_OFFSET
            );
            allPolicies[endorsmentPolicy.onChainPolicyId].coverageStartDate = coverageStartDate;
            allPolicies[endorsmentPolicy.onChainPolicyId].expirationDate = coverageStartDate.addYears(
                1,
                TIME_ZONE_OFFSET
            );
        }

        if (bytes(endorsmentPolicy.package).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].package = endorsmentPolicy.package;
        }

        if (endorsmentPolicy.exposure.vehicleType != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.vehicleType = endorsmentPolicy.exposure.vehicleType;
        }
        if (bytes(endorsmentPolicy.exposure.brand).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.brand = endorsmentPolicy.exposure.brand;
        }
        if (bytes(endorsmentPolicy.exposure.model).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.model = endorsmentPolicy.exposure.model;
        }
        if (endorsmentPolicy.exposure.purchaseDate != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.purchaseDate = endorsmentPolicy
                .exposure
                .purchaseDate;
        }
        if (endorsmentPolicy.exposure.purchasePrice != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.purchasePrice = endorsmentPolicy
                .exposure
                .purchasePrice;
        }
        if (endorsmentPolicy.exposure.usage != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.usage = endorsmentPolicy.exposure.usage;
        }
        if (endorsmentPolicy.exposure.householdSize != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.householdSize = endorsmentPolicy
                .exposure
                .householdSize;
        }
        if (endorsmentPolicy.exposure.maxSpeed != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].exposure.maxSpeed = endorsmentPolicy.exposure.maxSpeed;
        }
        if (endorsmentPolicy.split != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].split = endorsmentPolicy.split;
        }
        if (bytes(endorsmentPolicy.commonSensitiveData).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].commonSensitiveData = endorsmentPolicy.commonSensitiveData;
        }
        if (bytes(endorsmentPolicy.claimsAdminSensitiveData).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].claimsAdminSensitiveData = endorsmentPolicy
                .claimsAdminSensitiveData;
        }
        if (bytes(endorsmentPolicy.policiesAdminSensitiveData).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].policiesAdminSensitiveData = endorsmentPolicy
                .policiesAdminSensitiveData;
        }
        if (bytes(endorsmentPolicy.distributorSensitiveData).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].distributorSensitiveData = endorsmentPolicy
                .distributorSensitiveData;
        }
        if (bytes(endorsmentPolicy.insurerSensitiveData).length != 0) {
            allPolicies[endorsmentPolicy.onChainPolicyId].insurerSensitiveData = endorsmentPolicy.insurerSensitiveData;
        }
        if (updateSelectedOptions) {
            allPolicies[endorsmentPolicy.onChainPolicyId].selectedOptions = endorsmentPolicy.selectedOptions;
        }
        if (updateCoverages) {
            delete allPolicies[endorsmentPolicy.onChainPolicyId].selectedCoverages;
            for (uint256 index = 0; index < endorsmentPolicy.selectedCoverages.length; index++) {
                allPolicies[endorsmentPolicy.onChainPolicyId].selectedCoverages.push(
                    endorsmentPolicy.selectedCoverages[index]
                );
            }
        }

        allPolicies[endorsmentPolicy.onChainPolicyId].operationType = endorsmentPolicy.operationType;
        allPolicies[endorsmentPolicy.onChainPolicyId].operationEffectiveDate = block.timestamp;
    }

    // function endorsePolicy(
    //     string memory onChainPolicyId,
    //     Product020802Lib.EndorsementParamsStruct[] memory endorsmentParams,
    //     uint256 endorsementType,
    ...
    //     allPolicies[onChainPolicyId].operationType.checkIfOngoingWithdrawal();
    //     for (uint256 index = 0; index < endorsmentParams.length; index++) {
    //         Product020802Lib.endorsePolicyCheckBusinessRules(endorsmentParams[index]);
    //     }
    //     for (uint256 index = 0; index < endorsmentParams.length; index++) {
    //         if (
    //             keccak256(abi.encodePacked((endorsmentParams[index].key))) ==
    //             keccak256(abi.encodePacked(("coverageStartDate")))
    //         ) {
    //             uint256 coverageStartDate = endorsmentParams[index].uintValue.computeCoverageStartDate(
    //                 allPolicies[onChainPolicyId].subscriptionDate,
    //                 TIME_ZONE_OFFSET
    //             );
    //             allPolicies[onChainPolicyId].coverageStartDate = coverageStartDate;
    //             allPolicies[onChainPolicyId].expirationDate = coverageStartDate.addYears(1, TIME_ZONE_OFFSET);
    //         } else if (
    //             keccak256(abi.encodePacked((endorsmentParams[index].key))) ==
    //             keccak256(abi.encodePacked(("selectedCoverages")))
    //         ) {
    //             delete allPolicies[onChainPolicyId].selectedCoverages;
    //             for (
    //                 uint256 selectedCoveragesIndex = 0;
    //                 selectedCoveragesIndex < selectedCoverages.length;
    //                 selectedCoveragesIndex++
    //             ) {
    //                 allPolicies[onChainPolicyId].selectedCoverages.push(selectedCoverages[selectedCoveragesIndex]);
    //             }
    //         } else {
    //             Product020802Lib.endorsePolicy(onChainPolicyId, endorsmentParams[index], allPolicies);
    //         }
    //     }
    //     allPolicies[onChainPolicyId].operationType = endorsementType;
    //     allPolicies[onChainPolicyId].operationEffectiveDate = block.timestamp;
    // }
    ...
    function checkBusinessRulesToCreatePolicy(
        ...
        Product020802Lib.checkMaxSpeed(policyToBeChecked.exposure.maxSpeed);
        Product020802Lib.checkLegalAge(policyToBeChecked.isOfLegalAge);
        Product020802Lib.checkCoverageStartDate(coverageStartDate, subscriptionDate);
    }
}
...
library Product020802Lib {
    ...
    struct Policy {
        bytes32 partnershipCodeHash;
        string onChainPolicyId;
        uint256 expirationDate; // + 1 an apres la coverageStartDate
        uint256 coverageStartDate; // Ne peux pas etre dans le passé
        uint256 subscriptionDate;
        uint256 riskType;
        uint256 policyType;
        uint256 termination;
        ...
        string distributorSensitiveData; //
        string insurerSensitiveData; //
        uint256 operationType; // Impossible de faire un avenant si resiliation en cour
        uint256 operationEffectiveDate; //
    }
    ...
    function endorsePolicy(
        string memory onChainPolicyId,
        EndorsementParamsStruct memory endorsmentParams,
        mapping(string => Policy) storage allPolicies
    ) external {
        bytes32 key = keccak256(abi.encodePacked((endorsmentParams.key)));

        if (key == keccak256(abi.encodePacked(("package")))) {
            allPolicies[onChainPolicyId].package = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("resetSelectedOptions")))) {
            delete allPolicies[onChainPolicyId].selectedOptions;
            allPolicies[onChainPolicyId].selectedOptions.push(endorsmentParams.stringValue);
        } else if (key == keccak256(abi.encodePacked(("selectedOptions")))) {
            allPolicies[onChainPolicyId].selectedOptions.push(endorsmentParams.stringValue);
        } else if (key == keccak256(abi.encodePacked(("split")))) {
            allPolicies[onChainPolicyId].split = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("commonSensitiveData")))) {
            allPolicies[onChainPolicyId].commonSensitiveData = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("claimsAdminSensitiveData")))) {
            allPolicies[onChainPolicyId].claimsAdminSensitiveData = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("policiesAdminSensitiveData")))) {
            allPolicies[onChainPolicyId].policiesAdminSensitiveData = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("distributorSensitiveData")))) {
            allPolicies[onChainPolicyId].distributorSensitiveData = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("insurerSensitiveData")))) {
            allPolicies[onChainPolicyId].insurerSensitiveData = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("vehicleType")))) {
            allPolicies[onChainPolicyId].exposure.vehicleType = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("brand")))) {
            allPolicies[onChainPolicyId].exposure.brand = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("model")))) {
            allPolicies[onChainPolicyId].exposure.model = endorsmentParams.stringValue;
        } else if (key == keccak256(abi.encodePacked(("purchaseDate")))) {
            allPolicies[onChainPolicyId].exposure.purchaseDate = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("purchasePrice")))) {
            allPolicies[onChainPolicyId].exposure.purchasePrice = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("usage")))) {
            allPolicies[onChainPolicyId].exposure.usage = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("householdSize")))) {
            allPolicies[onChainPolicyId].exposure.householdSize = endorsmentParams.uintValue;
        } else if (key == keccak256(abi.encodePacked(("maxSpeed")))) {
            allPolicies[onChainPolicyId].exposure.maxSpeed = endorsmentParams.uintValue;
        }
    }

    function endorsePolicyCheckBusinessRules(EndorsementParamsStruct memory endorsmentParams) public view {
        bytes32 key = keccak256(abi.encodePacked((endorsmentParams.key)));

        if (key == keccak256(abi.encodePacked(("maxSpeed")))) {
            checkMaxSpeed(endorsmentParams.uintValue);
        } else if (key == keccak256(abi.encodePacked(("coverageStartDate")))) {
            checkIfCoverageStartDateIsNotThePast(endorsmentParams.uintValue);
        }
    }

    function checkMaxSpeed(uint256 maxSpeed) public pure {
        uint256 authorizedMaxSpeed = 25;
        require(maxSpeed <= authorizedMaxSpeed, "Authorized maxSpeed is 25KM/H");
    }

    function checkCoverageStartDate(uint256 coverageStartDate, uint256 subscriptionDate) public pure {
        require(
    ...
    function checkLegalAge(bool isOfLegalAge) public pure {
    ...
    function checkIfOngoingWithdrawal(uint256 operationType) external pure {
        require(operationType != 1, "No action with ongoing withdrawal");
    }
}
