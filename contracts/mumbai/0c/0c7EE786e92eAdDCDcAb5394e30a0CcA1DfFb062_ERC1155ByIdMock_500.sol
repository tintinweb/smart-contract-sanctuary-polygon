    ...
    IERC1155ById
{
     ...
     */
    function __ERC1155ById_init(string memory name_, string memory symbol_) internal onlyInitializing {
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId)public view virtual override(ERC1155Upgradeable, BaseURIStorage, NFTSupplyStorage, NFTLifeCycle, IERC165) returns (bool) {
        return
    ...
    /**
     * @dev See {NFTSupplyStorage-getMaxSupplyByIndex}.
     */
    ...
    /**
     * @dev See {NFTLifeCycle-getNFTLifeCycleState}.
     */
    ...
    function uri(uint256 id) public view virtual override returns (string memory) {
        // Check whether any token exist with a given id, or not.
        if (!ERC1155SupplyUpgradeable.exists(id)) revert IdDoesNotExist();
    ...
    function _mint(
        ...
        ERC1155Upgradeable._mint(to, id, amount, "");
    ...
    function _mintBatch(
        ...
        ERC1155Upgradeable._mintBatch(to, ids, amounts, "");
    ...
    function _balanceOfOwner(address owner, uint256 id) internal view {
        ...
        if (ERC1155Upgradeable.balanceOf(owner, id) == 0) revert BalanceOfIdIsZero();
    }
    ...
    function _validateMintInput(uint256 id, uint256 amount) internal virtual {
        // Check `id` correspond to baseURIByIndex from {BaseURIStorage}.
        ...
        if (_isMaxSupplyByIndexDefined[id]) {
            if (amount + ERC1155SupplyUpgradeable.totalSupply(id) > _maxSupplyByIndex[id]) revert ExceedMaxSupplyById();
        }
    }

    /**
     * @dev Validates  NFT life cycle state from `ids` and `from`.
     */
    function _validateBurnBatchInput(uint256[] calldata ids) internal virtual {
        uint256 idsLength = ids.length;
    ...
    /**
     * @dev Validates NFT life cycle state from `id` and `from`.
     */
    function _validateBurnInput(uint256 id) internal virtual {
        // Check NFT life cycle state for `id`.
        if (_nftLifeCycleState[id] != NFTLifeCycleState.TRANSFERABLE_BURNABLE) revert IdIsNotBurnable();
    }
    ...
    function _setNFTLifeCycleById(uint256[] calldata ids, uint8[] calldata stateValue) internal virtual {
        ...
        for (uint256 i = 0; i < idsLength;) {
            ...
            string memory _baseURIById = getBaseURI(ids[i]);

            // Check `id` correspond to index of baseURIByIndex from {BaseURIStorage}.
            if (bytes(_baseURIById).length == 0) revert("ID does not exist.");
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < idsLength;) {
            ...
            if (_nftLifeCycleState[_ids] == NFTLifeCycleState.NOT_TRANSFERABLE_NOT_BURNABLE) {
...
interface IERC1155ById is IERC165 {
     ...
     */
    function airdrop(address[] calldata receivers, uint256[] calldata amounts, uint256 id) external;
...
abstract contract BaseURIStorage is ContextUpgradeable, IBaseURIStorage {
...
interface INFTLifeCycle is IERC165 {
    ...
    /**
     * @dev Emitted when the `caller` update NFT life cycle state.
     */
    event NFTLifeCycleStateUpdated(address indexed caller, uint256 index, uint8 state);
...
abstract contract NFTLifeCycle is ContextUpgradeable, INFTLifeCycle {
    // Initialize enums type representing NFT life cycle state after mint event.
    enum NFTLifeCycleState {
        TRANSFERABLE_NOT_BURNABLE,      // uint8 value = 0
        TRANSFERABLE_BURNABLE,          // uint8 value = 1
        NOT_TRANSFERABLE_NOT_BURNABLE   // uint8 value = 2
    }

    // Mapping of index to NFT life cycle state.
    mapping (uint256 => NFTLifeCycleState) internal _nftLifeCycleState;
    ...
    function getNFTLifeCycle(uint256 index) public view virtual returns (string memory) {
        return _getNFTLifeCycleStateElement(_nftLifeCycleState[index]);
    }
    ...
    function _setNFTLifeCycleState(uint256 index, uint8 stateValue) internal virtual {
        ...
        _nftLifeCycleState[index] = NFTLifeCycleState(stateValue);

        emit NFTLifeCycleStateUpdated(_msgSender(), index, stateValue);
    }
...
abstract contract NFTSupplyStorage is ContextUpgradeable, INFTSupplyStorage {
...
contract ERC1155ByIdMock_500 is 
    ERC1155ById,
    ERC2771Recipient,
    ...
    function initialize(
        ...
        // Init upgradeables.
        __ERC1155ById_init(name, symbol);
        ...
        if (_initOwner != _msgSender()) {
            OwnableUpgradeable.transferOwnership(_initOwner);
        }

        // Init collection.
        _setMaxQuantity(_initMaxAmount);
    }
    ...
    function burn(
        ...
        _burnAmountOfId(owner, id, amount);
    }
    ...
    function burnBatch(
        ...
        _burnAmountsOfIds(owner, ids, amounts);
    }
    ...
    /**
     * @dev See {ERC1155ById-_setBaseURIById}.
     */
     ...
     * @dev See {IERC165-supportsInterface}.
     */
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC1155ById, AccessControlUpgradeable) returns (bool) {
        return 
