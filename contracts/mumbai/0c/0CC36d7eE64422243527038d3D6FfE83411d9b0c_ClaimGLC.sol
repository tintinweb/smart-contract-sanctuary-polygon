...
contract ClaimGLC is IERC721Receiver, Ownable  {

  uint256 immutable chainId;
  uint256[] receivedTokenIds;
  uint256 public minAgeForClaimInDays;
  uint256 private arrayFront;
  
  address public signer;
  address public acceptedTokenAddress;
  
  mapping (bytes  => bool) isVerified;
  mapping (uint256 => uint256) receivingTime;
  ...
  constructor(address _signer, address _acceptedTokenAddress, uint256 _minAgeForClaimInDays) {
    ...
    minAgeForClaimInDays = _minAgeForClaimInDays * 1 days;
  }
  ...
  function claim(bytes memory signature, uint256 deadline, uint256 nonce) public {
    ...
    uint256 deltaTime = block.timestamp - receivingTime[tokenIdOfTokenToSend];
    uint256 noOfDaysPassed = deltaTime / 1 days;
    if(deltaTime % 1 days != 0) {
      noOfDaysPassed-=1;  // To adjust if 44.1 days time passed, set it to 43.1 and likewise
    }
    require(noOfDaysPassed >= 44, "No token available for claim");
    
    
    delete receivedTokenIds[arrayFront];
    delete receivingTime[tokenIdOfTokenToSend]; 
    arrayFront += 1;
  ...
  function onERC721Received(address operator, address from, uint256 tokenId, bytes calldata data) external override returns (bytes4) {
    ...
    receivingTime[tokenId] = block.timestamp;
    return this.onERC721Received.selector;
  }

  function transferNonAcceptedNFTs(address _tokenAddress, uint256 _tokenId, address _receiver) public onlyOwner {
    IERC721(_tokenAddress).safeTransferFrom(address(this), _receiver, _tokenId);
  }

  function setAcceptedTokns(address _acceptedTokenAddress) public onlyOwner {
    acceptedTokenAddress = _acceptedTokenAddress;
  }
  ...
  function cancelDeposite() public onlyOwner {
    uint256 _tokenId = receivedTokenIds[receivedTokenIds.length - 1];
    receivedTokenIds.pop();
    delete receivingTime[_tokenId];
    IERC721(acceptedTokenAddress).safeTransferFrom(address(this), msg.sender, _tokenId);
  }
