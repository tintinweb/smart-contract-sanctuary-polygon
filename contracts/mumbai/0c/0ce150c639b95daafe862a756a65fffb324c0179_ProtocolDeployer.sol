...
import {IProtocolDeployer} from './interfaces/IDeployer.sol';
import {IProtocolFinder} from './interfaces/IFinder.sol';
import {ProtocolInterfaces, FactoryInterfaces} from '../Constant.sol';
import {IProtocolManager} from './interfaces/IManager.sol';
import {MintableBurnableTokenPermit} from '../tokens/erc20/MintableBurnableTokenPermit.sol';
import {IProtocolFactoryVersioning} from './interfaces/IFactoryVersioning.sol';
...
import {ISynthereumDeployment} from '../fixedRate/IFixedRateWrapper.sol';

contract ProtocolDeployer is IProtocolDeployer, AccessControlEnumerable, ReentrancyGuard {
    ...
    // State variables
    //----------------------------------------
    IProtocolFinder public immutable protocolFinder;

    //----------------------------------------
    // Events
    //----------------------------------------
    event FixedRateDeployed(uint8 indexed fixedRateVersion, address indexed fixedRate);
    ...
    constructor(IProtocolFinder _protocolFinder, Roles memory roles) {
        protocolFinder = _protocolFinder;
    ...
    function deployToken(string calldata tokenName, string calldata tokenSymbol, uint8 tokenDecimals)
        external
        override
        onlyMaintainer
        nonReentrant
        returns (MintableBurnableTokenPermit token)
    {
        token = _deployToken(getFactoryVersioning(), 1, tokenName, tokenSymbol, tokenDecimals);

        emit TokenDeployed(address(token));
    }

    function deployFixedRate(uint8 fixedRateVersion, bytes calldata fixedRateParamsData)
        external
        override
        onlyMaintainer
        nonReentrant
    ...
    {
        fixedRate = _deployFixedRate(getFactoryVersioning(), fixedRateVersion, fixedRateParamsData);
        emit FixedRateDeployed(fixedRateVersion, address(fixedRate));
    }

    function _deployFixedRate(
        IProtocolFactoryVersioning factoryVersioning,
        ...
        returns (ISynthereumDeployment fixedRate)
    {
        address fixedRateFactory =
            factoryVersioning.getFactoryVersion(FactoryInterfaces.FixedRateFactory, fixedRateVersion);

        // bytes memory fixedRateDeploymentResult =
        fixedRateFactory.functionCall(
            abi.encodeWithSelector(getDeploymentSignature(fixedRateFactory), fixedRateParamsData)
        );

        //  fixedRate = ISynthereumDeployment(abi.decode(fixedRateDeploymentResult, (address)));
    }

    function getFactoryVersioning() internal view returns (IProtocolFactoryVersioning factoryVersioning) {
        factoryVersioning =
            IProtocolFactoryVersioning(protocolFinder.getImplementationAddress(ProtocolInterfaces.FactoryVersioning));
    }

    function getManager() internal view returns (IProtocolManager manager) {
        manager = IProtocolManager(protocolFinder.getImplementationAddress(ProtocolInterfaces.Manager));
    }
    ...
    function _deployToken(
        ...
        returns (MintableBurnableTokenPermit token)
    {
        address poolFactory = factoryVersioning.getFactoryVersion(ProtocolInterfaces.TokenFactory, poolVersion);
        console2.log('am here');
        console2.log(address(poolFactory));
        bytes memory poolDeploymentResult = poolFactory.functionCall(
            abi.encodePacked(getDeploymentSignature(poolFactory), tokenName, tokenSymbol, tokenDecimals),
            'Wrong pool deployment'
        );
        console2.log('am here1');

        token = MintableBurnableTokenPermit(abi.decode(poolDeploymentResult, (address)));
    }

    function getDeploymentSignature(address factory) internal view returns (bytes4 signature) {
        signature = IDeploymentSignature(factory).deploymentSignature();
        console2.logBytes4(signature);
    }
}
...
interface IProtocolManager {
     ...
     */
    function grantProtocolRole(address[] calldata contracts, bytes32[] calldata roles, address[] calldata accounts)
        external;
     ...
     */
    function revokeProtocolRole(address[] calldata contracts, bytes32[] calldata roles, address[] calldata accounts)
        external;
     ...
     * @param roles Roles id
     */
    function renounceProtocolRole(address[] calldata contracts, bytes32[] calldata roles) external;
...
import {BaseControlledMintableBurnableERC20} from '../interfaces/BaseControlledMintableBurnableERC20.sol';

import {ERC20Permit} from '@openzeppelin/contracts/token/ERC20/extensions/draft-ERC20Permit.sol';

contract MintableBurnableTokenPermit is ERC20Permit, MintableBurnableToken {
    /* -------------------------------------------------------------------------- */
    /*                                  Constants                                 */
    /* -------------------------------------------------------------------------- */
    ...
    constructor(string memory tokenName, string memory tokenSymbol, uint8 tokenDecimals)
        MintableBurnableToken(tokenName, tokenSymbol, tokenDecimals)
    ...
    function decimals() public view virtual override (ERC20, BaseControlledMintableBurnableERC20) returns (uint8) {
...
/**
 * @title Provides addresses of different versions of
 * tokens factory and others
 */
interface IProtocolFactoryVersioning {
    /**
     ...
     */
    function setFactory(bytes32 factoryType, uint8 version, address factory) external;

    /**
     * @notice Removes a pool factory
     * @param factoryType The type of pool factory to be removed
     * @param version Version of the factory to be removed
     */
    function removeFactory(bytes32 factoryType, uint8 version) external;

    /**
     * @notice Gets a pool factory contract address
     * @param factoryType The type of pool factory to be checked
     * @param version Version of the pool factory to be checked
     * @return factory Address of the pool factory contract
     */
    function getFactoryVersion(bytes32 factoryType, uint8 version) external view returns (address factory);

    /**
     * @notice Gets the number of pool factory versions for a specific type
     * @param factoryType The type of pool factory to be checked
     * @return numberOfVersions Total number of versions for a specific factory
     */
    function numberOfFactoryVersions(bytes32 factoryType) external view returns (uint8 numberOfVersions);
}
...
import {IProtocolFinder} from '../core/interfaces/IFinder.sol';
...
interface ISynthereumDeployment {
     ...
     */
    function synthereumFinder() external view returns (IProtocolFinder finder);
...
contract MintableBurnableToken is BaseControlledMintableBurnableERC20, AccessControlEnumerable {
    /* -------------------------------------------------------------------------- */
    /*                                  Constants                                 */
    /* -------------------------------------------------------------------------- */
...
abstract contract BaseControlledMintableBurnableERC20 is ERC20, IMintableBurnableERC20 {
    /* -------------------------------------------------------------------------- */
    /*                                   Storage                                  */
    /* -------------------------------------------------------------------------- */
     ...
     */
    function decimals() public view virtual override (ERC20, IMintableBurnableERC20) returns (uint8) {
        return _decimals;
    }
    /* -------------------------------------------------------------------------- */
    /*                              Internal function                             */
