...
/******************************************************************************\
/// @title Sipher Portal - Execute Facet 
/// @author Ather Labs
/// @dev This facet provides functions that use the calldata and the latest commit data to update a new stage root and execute all required pending transactions. 
/******************************************************************************/
...
contract ExecuteFacet is IOperations, ReentrancyGuard {
    /* ------ EVENTS ------ */
    /// @dev Event emitted when an execution is successful.
    event Executed(bytes32 pendingOperationsHash, uint256 batchId, uint256 depositCounter, uint256 executedCounter);

    /// @dev Event emitted when an successfully execute a deposit request.
    event ExecutedDepositRequest(uint128 requestId);

    /// @dev Event emitted when a pending balance is stored under the user's withdrawal key.
    ...
    /* ------ MODIFIERS ------ */
    /// @dev This modifier ensures that the functions applied can only be called by the portal itself.
    modifier selfCallOnly() {
        require(msg.sender == address(this), "EF: Invalid caller");
        _;
    }

    /* ------ EXTERNAL FUNTIONS ------ */
    /*
     * requestType == 0 execute deposit request
     * requestType == 1 execute deposit batch request
     * requestType == 2 execute withdraw native tokens request
     * requestType == 3 execute withdraw ERC20 tokens request
     * requestType == 4 execute withdraw ERC721 tokens request
     * requestType == 5 execute withdraw ERC1155 tokens request
     */
    /// @notice Execute all pending on-chain Txs based on latest commit. Clear all pending deposit record when complete.
    /// @dev This implementation requires that the calldata must be prepared with specific structure for each request type.
    /// @dev Only Executor can call this function.
    /// @dev This function can only be called when Operation is enabled.
    /// @dev Input data must match with the latest commit data.
    /// @dev The commit is still unexecuted yet.
    /// @param _requests Data array of pending on-chain Txs, each index is a compressed data of a single Tx.
    function executeBatch(bytes[] calldata _requests) external nonReentrant {
        LibAccessControl._requireExecutor();
        LibStatusControl._requireOperationsEnabled();
        CommittedBatchInfo memory commit = s.latestCommittedBatch;
        require(commit.timestamp + s.config.executeTimeInterval < block.timestamp, "EF: Too soon to execute");
        require(!s.isExecuted[commit.batchId], "EF: Current batch already executed");

        bytes32 pendingOperationsHash_ = s.domainSeparator;
        uint256 depositCounter;
        uint256 executedCounter;

        for (uint256 i = 0; i < _requests.length; ++i) {
            bytes memory pubData = _requests[i];
            uint8 requestType = LibBytes.bytesToUInt8(pubData, 0);
            if (requestType == 0) // Case 0: Execute deposit request
            {
                Deposit memory deposit = LibOperations._readDepositRequest(pubData, 1);
                require(
                    s.pendingDepositRecord[deposit.requestId].hashedRequestData ==
                        keccak256(abi.encode(s.domainSeparator, LibOperations._hashDepositRequest(deposit))),
                    "EF: Invalid deposit request data"
                );
                delete s.pendingDepositRecord[deposit.requestId];
                depositCounter++;

                emit ExecutedDepositRequest(deposit.requestId);
            } else if (requestType == 1) // Case 1: Execute deposit batch request
            {
                DepositBatch memory depositBatch = LibOperations._readDepositBatchRequest(pubData, 1);
                require(
                    s.pendingDepositRecord[depositBatch.requestId].hashedRequestData ==
                        keccak256(abi.encode(s.domainSeparator, LibOperations._hashDepositBatchRequest(depositBatch))),
                    "EF: Invalid deposit batch request data"
                );
                delete s.pendingDepositRecord[depositBatch.requestId];
                depositCounter++;

                emit ExecutedDepositRequest(depositBatch.requestId);
            } else if (requestType == 2) // Case 2: Execute withdraw native token request
            {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(withdraw.listingId == 0, "EF: Withdraw token is not native token");
                LibTokenManagement._requireUnpaused(0);
                _withdrawOrStoreCryptoCurrency(withdraw.to, withdraw.listingId, withdraw.amount);
            } else if (requestType == 3) // Case 3: Execute withdraw ERC20 token request
            {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(s.tokenTypes[withdraw.listingId] == 1, "EF: Withdraw token is not ERC20 token");
                LibTokenManagement._requireUnpaused(withdraw.listingId);
                _withdrawOrStoreCryptoCurrency(withdraw.to, withdraw.listingId, withdraw.amount);
            } else if (requestType == 4) // Case 4: Execute withdraw ERC721 tokens request
            {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(s.tokenTypes[withdraw.listingId] == 2, "EF: Withdraw token is not ERC721 token");
                LibTokenManagement._requireUnpaused(withdraw.listingId);
                _releaseERC721(withdraw.to, withdraw.listingId, withdraw.tokenId);
            } else if (requestType == 5) // Case 5: Execute withdraw ERC1155 tokens request
            {
                Withdraw memory withdraw = LibOperations._readWithdrawRequest(pubData, 1);
                require(
                    s.tokenTypes[withdraw.listingId] == 3 || s.tokenTypes[withdraw.listingId] == 4,
                    "EF: Withdraw token is not ERC1155 token"
                );
                LibTokenManagement._requireUnpaused(withdraw.listingId);
                _releaseERC1155(withdraw.to, withdraw.listingId, withdraw.tokenId, withdraw.amount);
            } else {
                revert("EF: Invalid request type");
            }
            pendingOperationsHash_ = LibOperations._concatHash(pendingOperationsHash_, pubData);
            executedCounter++;
        }

        // Post-check the execution result.
        require(executedCounter == _requests.length, "EF: Invalid execution length");
        require(depositCounter <= s.numberOfPendingRequest, "EF: Invalid deposit count");
        require(commit.pendingOperationsHash == pendingOperationsHash_, "EF: Invalid pending operations hash");

        s.isExecuted[commit.batchId] = true;
        s.numberOfPendingRequest -= depositCounter;
        s.currentBatchId++;

        emit Executed(commit.pendingOperationsHash, commit.batchId, depositCounter, executedCounter);
    }

    /// @notice Send Native token from the portal to recipient.
    /// @dev This is an external self call only function. This implementation allows try-catch statement be applied while perform executeBatch.
    /// @param _to Address of recipient.
    /// @param _amount Amount to transfer.
    /// @param _minLiquidity Minumum liquidity limit, balance of this token on the portal can not go below this limit.
    /// @return uint128 Transfered amount.
    function transferNative(
        address _to,
        uint88 _amount,
        uint256 _minLiquidity
    ) external selfCallOnly returns (uint128) {
        return LibWithdrawBalance._transferNativeToken(_to, _amount, _minLiquidity);
    }
    ...
    function transferERC20(
        ...
        return LibWithdrawBalance._transferERC20(_tokenAddress, _to, _amount, _minLiquidity);
    }
    ...
    function transferERC721(
        ...
        return LibWithdrawBalance._transferERC721(_tokenAddress, _to, _tokenId);
    }
    ...
    function transferERC1155(
        ...
        return LibWithdrawBalance._transferERC1155(_tokenAddress, _to, _tokenId, _amount, _minLiquidity);
    }

    /* ------ INTERNAL FUNTIONS ------ */
    /// @notice Internal function to transfer Native or ERC20 token from the portal, if the Tx fails during execution, the pending balance will be stored under the user's withdrawal key for later withdrawal procedure.
    /// @param _listingId Token's listingId.
    /// @param _amount Amount to transfer.
    function _withdrawOrStoreCryptoCurrency(
        address _recipient,
        uint8 _listingId,
        uint88 _amount
    ) internal {
        bool sent = false;
        if (s.isBlacklisted[_recipient].status) {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, 0, _amount);
            return;
        }

        if (_listingId == 0) {
            try
                this.transferNative{gas: s.config.withdrawGasLimit}(
                    _recipient,
                    _amount,
                    s.config.minLiquidity[_listingId]
                )
            {
                sent = true;
            } catch {}
        } else {
            address tokenAddr = LibTokenManagement._validateListingId(_listingId);
            try
                this.transferERC20{gas: s.config.withdrawGasLimit}(
                    IERC20(tokenAddr),
                    _recipient,
                    _amount,
                    s.config.minLiquidity[_listingId]
        ...
        if (sent) {
            emit Withdrawn(_recipient, _listingId, 0, _amount);
        } else {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, 0, _amount);
        }
    }

    /// @notice Internal function to transfer ERC721 token from the portal, if the Tx fails during execution, the pending balance will be stored under the user's withdrawal key for later withdrawal procedure.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    function _releaseERC721(
        ...
        IERC721 tokenAddr = IERC721(LibTokenManagement._validateListingId(_listingId));
        if (s.isBlacklisted[_recipient].status) {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, _tokenId, 1);
            return;
        }

        try this.transferERC721{gas: s.config.withdrawGasLimit}(tokenAddr, _recipient, _tokenId) {
            emit Withdrawn(_recipient, _listingId, _tokenId, 1);
        } catch {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, _tokenId, 1);
        }
    }

    /// @notice Internal function to transfer ERC1155 token from the portal, if the Tx fails during execution, the pending balance will be stored under the user's withdrawal key for later withdrawal procedure.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    /// @param _amount Amount to transfer.
    function _releaseERC1155(
        ...
        IERC1155 tokenAddr = IERC1155(LibTokenManagement._validateListingId(_listingId));
        if (s.isBlacklisted[_recipient].status) {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, _tokenId, _amount);
            return;
        }

        try
            this.transferERC1155{gas: s.config.withdrawGasLimit}(
                tokenAddr,
                _recipient,
                _tokenId,
                _amount,
        ...
        {
            emit Withdrawn(_recipient, _listingId, _tokenId, _amount);
        } catch {
            LibWithdrawBalance._increasePendingBalance(_recipient, _listingId, _tokenId, _amount);
        }
    }
