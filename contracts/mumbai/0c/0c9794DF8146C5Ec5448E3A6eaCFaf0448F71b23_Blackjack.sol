...
contract Blackjack is VRFConsumerBaseV2, Constants, Ownable, BreakdownUint256 {
    // VRF parameters
    ...
    constructor()
        VRFConsumerBaseV2(address(configMap["mumbai"].vrfCoordinator))
    {
        COORDINATOR = configMap["mumbai"].vrfCoordinator;
        LINKTOKEN = configMap["mumbai"].linkToken;
        KEY_HASH = configMap["mumbai"].keyHash;
        CALLBACK_GAS_LIMIT = configMap["mumbai"].callbackGasLimit;
        REQUEST_CONFIRMATIONS = configMap["mumbai"].requestConfirmations;
        NUM_WORDS = configMap["mumbai"].numWords;
        vrfSubscriptionId = COORDINATOR.createSubscription();
        COORDINATOR.addConsumer(vrfSubscriptionId, address(this));
    }
    ...
    function fulfillInit(uint256 requestId_) private {
        ...
        Card playerCard1 = uint8ToCard(randomUint8s[0]);
        Card playerCard2 = uint8ToCard(randomUint8s[1]);
        Card dealerCard1 = uint8ToCard(randomUint8s[2]);
