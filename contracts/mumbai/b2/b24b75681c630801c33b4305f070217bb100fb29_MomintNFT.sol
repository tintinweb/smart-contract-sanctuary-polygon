...
contract MomintNFT is ERC721, Ownable {
    ...
    function _previousIdsPaginated(
        ...
        uint256 currentTokenId = previoustInBatch_[tokenId];
        uint256 originalToken = originalTokenId(tokenId);
        for(uint i = 0; i < amountToFetch; i = i + 1) {
            _returnTokens[amountToFetch-i] = currentTokenId;
            currentTokenId = previoustInBatch_[currentTokenId];
            if(currentTokenId == originalToken) return _returnTokens;
        }
