...
contract PaperERC1155Template is ERC1155, Ownable {
    ...
    function register(address _paperKey) external onlyOwner {
        require(
            paperKeyManager.register(_paperKey),
            "Failed to register paperKey"
        );
    }
    ...
    function paperMint(
        ...
        external
        payable
        mintCompliant(_data.tokenId, _data.quantity)
        priceCompliant(_data.tokenId, _data.quantity)
        tokenLive(_data.tokenId)
    {
        bool success = paperKeyManager.verify(
            keccak256(abi.encode(_data.to, _data.quantity, _data.tokenId)),
            _nonce,
            _signature
        );
        require(success, "Failed to verify signature");
        // todo: your mint method here.
        tokenTotalSupply[_data.tokenId] += _data.quantity;
        _mint(_data.to, _data.tokenId, _data.quantity, "");
    }
