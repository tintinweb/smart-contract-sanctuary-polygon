...
contract GameStaking is Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(address(_rewardToken) != address(0), 'GameStake: zero-check');
    ...
    function getTiketPrice() external view returns (uint256) {
        return tiketPrice;
    }

    function setTiketPrice(uint256 newTiketPrice) external onlyOwner returns (bool) {
        tiketPrice = newTiketPrice;
        return true;
    }

    function setRewardToken(IERC20 newRewardToken) external onlyOwner returns (bool) {
        require(address(newRewardToken) != address(0), 'GameStake: zero-check');
    ...
    function getUserBalance(address playerAddress) public view returns (uint256) {
        return players[playerAddress].balance + calculateReverds(playerAddress);
    }
    ...
    function stake() external payable nonReentrant {
        require(initialized, 'GameStake: is not active');
        require(block.timestamp < endTime, 'GameStake: is ended');
        rewardToken.safeTransferFrom(msg.sender, address(this), tiketPrice);
        Player storage player = players[msg.sender];
        if (player.deposit > 0) {
            updateBalance(msg.sender);
        }
        if (minimalDepositAmount != 0) {
            require(msg.value >= minimalDepositAmount, 'GameStake: minimum amount error');
        }
        if (maximumDepositAmount != 0) {
            require(player.deposit + msg.value <= maximumDepositAmount, 'GameStake: limit amount error');
        }
        player.deposit = player.deposit + msg.value;
    ...
    function updateBalance(address playerAddress) internal {
        // !!! SAVE REVARD EVERYTIME when change Multiplier
        Player storage player = players[playerAddress];
        player.balance = player.balance + calculateReverds(playerAddress);
        player.claimDate = block.timestamp;
    }

    function updatePlayersRates(address[] memory playersAddreses, uint256[] memory newRates)
        external
        onlyOwner
        returns (bool)
    {
        require(playersAddreses.length == newRates.length, 'GameStake: wrong params');
        for (uint256 i; i < playersAddreses.length; i++) {
            updateBalance(playersAddreses[i]);
            players[playersAddreses[i]].rate = newRates[i];
        }
        return true;
    }

    function calculateReverds(address playerAddress) internal view returns (uint256 revards) {
        ...
        uint256 calimTimestamp = player.claimDate > endTime ? endTime : player.claimDate;
        revards = ((((player.deposit * rate) / ONE_HUNDRED_PERCENT) * (lastTimestamp - calimTimestamp)) /
            SECONDS_IN_A_DAY);
    }
    ...
    function withdrawDeposit() external nonReentrant {
        ...
        Player storage player = players[playerAddress];
        require(player.activateTime + lockTime <= block.timestamp, 'GameStake: lock time error');
        updateBalance(playerAddress);
        uint256 deposit = player.deposit;
        require(deposit > 0, 'GameStake: zero deposit error');
        player.deposit = 0;
        emit DepositWithdrawed(playerAddress, deposit);
        sendEth(playerAddress, deposit);
    }
    ...
    function claimReward() external nonReentrant {
        ...
        require(rewardToken.balanceOf(address(this)) >= balance, 'GameStake: not enough tokens');
        require(balance > 0, 'GameStake: zero balance');
        player.balance = 0;
        player.claimDate = block.timestamp;
    ...
    function sendEth(address payable to, uint256 value) internal {
