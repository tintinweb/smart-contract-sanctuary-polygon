...
contract SlaveCrossLedgerVault is CrossLedgerVault {
        ...
        if (messageType == LzMessage.REDEEM) {
            ...
            uint256 opId;
            {
                ...
                uint256 amountToRedeem = (shares *
                    rootVault.balanceOf(address(this))) / totalSupply;
                uint256 value = (amountToRedeem > 0)
                    ? rootVaultZap.redeem(asset, amountToRedeem)
                    : 0;
                IBridgeAdapter adapter = bridgeAdapters[asset][srcChainId];
                transferId = adapter.sendAssetsToAddress(
                    value,
                    asset,
                    receiver
                );
    ...
    function depositFundsToRootVault(
        ...
        _lzSend(
            lzChainId,
            _encodeDepositedFromBridgeMessage(
                transferId,
                totalAssetsBefore,
                totalAssetsAfter
            ),
