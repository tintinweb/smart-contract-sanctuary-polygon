...
contract GToken is ERC20Upgradeable, ERC4626Upgradeable, OwnableUpgradeable, IGToken {
    ...
    function initialize(
        ...
        require(
            _contractAddresses.asset != address(0) && _contractAddresses.owner != address(0)
                && _contractAddresses.manager != address(0) && _contractAddresses.admin != address(0)
            /*&& _contractAddresses.owner != _contractAddresses.manager
                && _contractAddresses.manager != _contractAddresses.admin **/
            && _contractAddresses.gnsToken != address(0) && _contractAddresses.lockedDepositNft != address(0)
                && _contractAddresses.pnlHandler != address(0) && _contractAddresses.openTradesPnlFeed != address(0)
                ...
                && _maxSupplyIncreaseDailyP <= MAX_SUPPLY_INCREASE_DAILY_P && _lossesBurnP <= MAX_LOSSES_BURN_P
                && _maxGnsSupplyMintDailyP <= MAX_GNS_SUPPLY_MINT_DAILY_P && _maxDiscountP <= MAX_DISCOUNT_P
    ...
    function depositWithDiscountAndLock(uint256 assets, uint256 lockDuration, address receiver)
        external
        checks(assets)
        ...
        returns (uint256)
    {
        uint256 simulatedAssets =
            assets * (PRECISION * 100 + lockDiscountP(collateralizationP(), lockDuration)) / (PRECISION * 100);
    ...
    function mintWithDiscountAndLock(uint256 shares, uint256 lockDuration, address receiver)
        external
        checks(shares)
        validDiscount(lockDuration)
        returns (uint256)
    ...
    function unlockDeposit(uint256 depositId, address receiver) external {
        ...
        int256 accPnlDelta = int256(d.assetsDiscount.mulDiv(PRECISION, totalSupply(), MathUpgradeable.Rounding.Up));
    ...
    function receiveAssets(uint256 assets, address user) external {
        ...
        // transfer DAI from sender to this vault
        SafeERC20Upgradeable.safeTransferFrom(_assetIERC20(), sender, address(this), assets);
    ...
    function deplete(uint256 assets) external {
        ...
        uint256 amountGns = assets.mulDiv(GNS_PRECISION, gnsTokenToAssetsPrice(), MathUpgradeable.Rounding.Up);
    ...
    function refill(uint256 assets) external {
        ...
        uint256 amountGns = assets * GNS_PRECISION / gnsTokenToAssetsPrice();
