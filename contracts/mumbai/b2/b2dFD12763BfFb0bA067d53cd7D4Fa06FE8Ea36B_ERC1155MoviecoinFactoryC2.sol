...
contract ERC1155Moviecoin is ERC1155Base {
    ...
    function __ERC1155Moviecoin_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) internal {
        ...
        _setBaseURI(baseURI);
        __TokenUriValidator_init_unchained();
...
import "./TokenUriValidator.sol";

abstract contract ERC1155Base is ERC1155DefaultApproval, ERC1155BurnableUpgradeable, ERC1155Lazy, HasContractURI, TokenUriValidator {
    string public name;
    ...
    event TokenURIUpdated(uint256 tokenId, string oldTokenURI, string newTokenURI, address updatingAuthority);
    ...
    function updateMetadataOwnerOrAdmin(uint256 tokenId, string memory newTokenURI) external onlyOwnerOrAdmin{
        emit TokenURIUpdated(tokenId, _tokenURIs[tokenId], newTokenURI, msg.sender);
        _tokenURIs[tokenId] = newTokenURI;
    }

    function updateMetadataProducer(uint256 tokenId, string memory newTokenURI, bytes memory adminSignature) external {
        require(block.timestamp >= getNftRevealTime(getMovieId(tokenId)), "NFT Reveal time has not reached yet");

        // Admin signature verification
        validateTokenUriSignature(tokenId, newTokenURI, admin, adminSignature);
...
abstract contract ERC1155Lazy is OwnableUpgradeable, IERC1155LazyMint, ERC1155BaseURI, Mint1155Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl {
    ...
    /* Movie Reveal time */
    mapping(string => uint256) private nftRevealTime;
    /* Token id to Movie id */
    mapping(uint256 => string) private movieId;
    /* Producer address of movie id */
    mapping(string => address) private movieProducer;
    /* Film Equity Percentage for the NFT */
    mapping(uint256 => uint256) private filmEquityPercentageOfNft;

    event MovieRevealTimeUpdated(string movieId, uint256 OldNftRevealTime, uint256 NewNftRevealTime);
    ...
    function mintAndTransfer(LibERC1155LazyMint.Mint1155Data memory data, address to, uint256 _amount) public override virtual {
        ...
        if (supply[data.tokenId] == 0) {
            ...
            filmEquityPercentageOfNft[data.tokenId] = data.filmEquityPercentage;
    ...
    function _saveCreators(uint tokenId, LibPart.Part[] memory _creators) internal {
        ...
        for (uint i = 0; i < _creators.length; i++) {
            LibPart.Part memory _creator = _creators[i];
            require(_creator.account != address(0x0), "Account should be present");
            require(_creator.value != 0, "Creator share should be positive");
            creatorsOfToken.push(_creator);
            total = total.add(_creator.value);
        }
    ...
    function getNftRevealTime(string memory tokenId) public view returns(uint256){
        return nftRevealTime[tokenId];
    }

    modifier onlyOwnerOrAdminOrProducer(address producer){
        require(msg.sender == owner() || msg.sender == admin || msg.sender == producer, "Not owner or admin or producer");
        _;
    }
    ...
    function setMovieRevealTime(string memory _movieId, uint256 _revealTime) external onlyOwnerOrAdminOrProducer(getMovieProducer(_movieId)) {
        emit MovieRevealTimeUpdated(_movieId, getNftRevealTime(_movieId), _revealTime);
        nftRevealTime[_movieId] = _revealTime;
    }

    function getMovieProducer(string memory _movieId) public view returns(address){
        return movieProducer[_movieId];
    }
    ...
    function getFilmEquityPercentageOfNft(uint256 tokenId) public view returns(uint256){
        return filmEquityPercentageOfNft[tokenId];
    }

    function getMovieId(uint256 tokenId) public view returns(string memory){
        return movieId[tokenId];
    }
...
contract TokenUriValidator is Initializable {
    bytes32 public constant TOKENURI_TYPEHASH = keccak256("TokenUri(uint256 tokenId,string tokenURI)");
    ...
    function __TokenUriValidator_init_unchained() internal initializer {
    ...
    function validateTokenUriSignature(uint256 tokenId, string memory tokenUri, address _signer, bytes memory signature) internal view returns (bool) {

      bytes32 digest = keccak256(
        abi.encodePacked(
          "\x19\x01",
          DOMAIN_SEPARATOR,
          keccak256(abi.encode(TOKENURI_TYPEHASH,
                  tokenId,
                  keccak256(bytes(tokenUri))))
        )
      );
      ...
      require(signer != address(0) && signer == _signer,"Invalid signature");
...
library LibRoyaltiesV2 {
    /*
     * bytes4(keccak256('getMoviecoinV2Royalties(uint256)')) == 0xfab05437
     */
    bytes4 constant _INTERFACE_ID_ROYALTIES = 0xfab05437;
}
...
library LibERC1155LazyMint {
    ...
    bytes32 public constant MINT_AND_TRANSFER_TYPEHASH = keccak256("Mint1155(uint256 tokenId,string movieId,uint256 supply,uint256 nftRevealTime,address producer,string tokenURI,uint256 filmEquityPercentage,Part[] creators,Part[] royalties)Part(address account,uint96 value)");
