...
contract Tournaments is Initializable {
    ...
    Game[] public games;
    uint256[] public nPlayer;
    mapping(uint256 => bool) public gameOn;
    mapping(uint256 => mapping(address => bool)) public userIn;
    ...
    function nActive() public view returns (uint256) {
        return games.length;
    }
    ...
    {
        return (games[i].size, nPlayer[i], gameOn[i], userIn[i][msg.sender]);
    }

    function joinGame(uint256 i) external {
        if (gameOn[i] || userIn[i][msg.sender]) return;

        userIn[i][msg.sender] = true;
        if (++nPlayer[i] == games[i].size) _startGame(i);
    }
    ...
    function endGame(uint256 i) external {
        require(userIn[i][msg.sender], "only the player can end the game");
        gameOn[i] = false;
    }
