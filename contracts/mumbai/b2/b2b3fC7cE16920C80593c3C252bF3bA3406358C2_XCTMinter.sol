    ...
    ReentrancyGuardUpgradeable
{
    ...
    IERC20Mintable public XCTToken;
    ...
    event ChangedRouter(IUniswapV2Router02 oldRouter, IUniswapV2Router02 newRouter);
    ...
    {
        emit ChangedRouter(uniswapV2Router, IUniswapV2Router02(
            newRouter
        ));
        uniswapV2Router = IUniswapV2Router02(
            newRouter
        ...
        returns (uint256)
    {
        ...
        return amounts[1].mul(factor).div(100000);
    }
    ...
    returns (uint256)
    {
        ...
        xctAmount = xctAmount.mul(factor).div(100000);
        ...
        returns (uint256 XCTamount)
    {
        ...
        uint256 stackConv = percentStackConversion;

        if(stackConv > 0)
        {
            uint256 usdcAmountForStack = usdcReceived.mul(stackConv).div(100000);
            IERC20Upgradeable(USDCToken).approve(
                address(this),
                usdcAmountForStack
            );
            IERC20Upgradeable(USDCToken).transferFrom(address(this), TREASURY_ADDRESS, usdcAmountForStack);
        }
    ...
    function buyXCT(address _token, uint256 _tokenAmount)
        ...
        returns (uint256)
    {
        if(_token == address(XCTToken))
            revert("token cannot be XCT");
       ...
       uint256 slippageFactor = (SafeMathUpgradeable.sub(100000, slippage))
        ...
        if(_token != address(USDCToken))
        {
            ...
            uint256 amountForUSDC = _tokenAmount;

            amounts2 = uniswapV2Router.getAmountsOut(
                amountForUSDC,
                path2
            );
        ...
        usdcReceived = USDCToken.balanceOf(address(this)).sub(prevUSDC);
        uint256 totalXCTToMint = usdcReceived;
        ...
        uint xctToMint = totalXCTToMint.mul(stepUpFactor);
        XCTToken.mint(msg.sender, xctToMint);
        totalUSDCMintedByContract = totalUSDCMintedByContract.add(usdcReceived);
        totalXCTMintedByContract = totalXCTMintedByContract.add(xctToMint);

        emit XCTBought(WETH, _tokenAmount, xctToMint);
        return xctToMint;
    }
