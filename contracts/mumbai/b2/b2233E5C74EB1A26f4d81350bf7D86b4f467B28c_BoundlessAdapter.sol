...
contract BoundlessAdapter is Context, ERC2771Context, AccessControlEnumerable {
    ...
    // Mint assets before calling the contract calls.
    function mintAndCall(
        ...
        _call(token, mintedAmount, fallbackRecipient, contractCalls);
    ...
    function _call(
        ...
        for (uint256 i = 0; i < contractCalls.length; i++) {
            ...
            console.log("outputToken", address(outputToken));
        ...
        console.log("finalAmount", inputAmount);
        console.log("finalToken", address(inputToken));
    ...
    // Allow the relayer to submit a mint recovery.
    function _recover(
        //
        // Parameters from users (in payload)
        //
        ...
        registry.getRenAssetBySymbol(asset).transfer(
            fallbackRecipient,
            amountMinted
        );
    }
    ...
    function _mint(
        ...
        if (borrowAsset) {
            ...
            uint256 amountMinted = registry.getMintGatewayBySymbol(asset).mint(
                pHash,
                amount,
                nHash,
                signature
