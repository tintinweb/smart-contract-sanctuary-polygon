...
contract NFTMarketPlace is ERC1155, Owned{
    ...
    // ERC1155 functions
    function mintFunc(address to, uint256 amount, bytes calldata data, string calldata _uri, bool isPDR) public {
        ...
        //Not reqd if safeTransferFrom function is not used
        holders[counter].push(to);       
        indexOf[counter][to] = (holders[counter].length)-1;
    ...
    function addProperty(
        ...
        mintFunc(_originalOwnerAddress, 1, data, _uri, false);
    ...
    function addAdvertisment(
        ...
        mintFunc(_originalOwnerAddress, noOfCopies, data, _uri, true);
    ...
    function getOwnerOfNFT(uint _tokenId) public view returns(address[] memory){
        return holders[_tokenId];
    }

    // // TODO: change to transfer any using owner or admin 
    // // TODO: remove pdr transfer
    // function safeTransferFrom(address from, address to, uint256 _tokenId, uint256 _noOfTokens, bytes calldata data) public override{
    //     require(
    ...
    //     //Check point - required/not
    //     //TODO: add type instead of bool: property.isPDR
    //     require((property.typeOfNFT) == TypeOfNFT.PDR, "Only PDR NFT allowed");

    //     if(balanceOf(to, _tokenId) == 0){
    //         if((balanceOf(from, _tokenId)).sub(_noOfTokens) == 0){
    //             // update holder address and index at which holder address is stored
    //             uint256 _index = indexOf[_tokenId][from];
    //             holders[_tokenId][_index] = to;
       
    //             delete indexOf[_tokenId][from];
    //             indexOf[_tokenId][to] = _index;
    //         } else{
    //             holders[_tokenId].push(to);
    //             indexOf[_tokenId][to] = holders[_tokenId].length - 1;
    //         }
    //     }
    //     _safeTransferFrom(from, to, _tokenId, _noOfTokens, data);

    //     //transfer PDR alongwith RWO
    //     for(uint i=0; i< mapParent[_tokenId].length; i++){
    //         _safeTransferFrom(from, to, mapParent[_tokenId][i], balanceOf(from, mapParent[_tokenId][i]), '0x');
    //     }
    // }

    // //burn nfts by admin or owner
    // // Burn PDR Rent NFT
    // // Burn RWO NFT
    ...
    //burn nfts by token owner
    function burnNFT(uint256 id) public onlyOwnerOf(id){
        baseNFT memory property = entries[id];
        address _owner;
    ...
    // // In balanceOfPDR -> owner balance if no rental
    // function balanceOfRental(address _renter, uint256 _tokenId, uint256 _start, uint256 _end) public view returns (uint256){
    //     require(_start <= _end);
    ...
    // function getBalancePDR(uint256 _tokenId, uint256 _start, uint256 _end) public view returns (uint256){
    //     require(_start <= _end);
    //     uint _startIndex = _start.div(minRentTime);
    //     uint _endIndex = _end.div(minRentTime);
    //     uint rentalTokenCount = 0;
    ...
    // Add null address if not mapped to 
    ...
    function transferRWONFTByOwnerOrAdmin(address from, address to, uint256 _tokenId, bytes calldata data) public  onlyAdminOrOwner(){
        baseNFT memory property = entries[_tokenId];
        //Check point - required/not
        require((property.typeOfNFT) == TypeOfNFT.RWO);
    ...
    function rentPDR(uint _tokenId, uint _amount, uint _start, uint _end, address _renter, address _erc20Token) public onlyOwnerOf(_tokenId){
