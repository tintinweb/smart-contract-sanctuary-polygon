...
// File: @openzeppelin/[email protected]/token/ERC721/extensions/ERC721Votes.sol
...
// File: contracts/MagicalGoatLand.sol
...
contract MagicalGoatLand is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, EIP712, ERC721Votes {
    mapping(uint256 => uint256) private _tokenPrices;

    constructor()
        ERC721("Magical Goat Land", "MGL")
        EIP712("Magical Goat Land", "1")
    {
        _mint(msg.sender, 1);
        _setTokenURI(1, "https://ipfs.io/ipfs/QmT54oUziAjA1qruZ2RRTHpsvv4yVCj2TnD6NHYi1geRby");
    }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://gateway.pinata.cloud/ipfs/Qmeee53Ed9h6c7nXwaUvEztx7dH1T1KEVv5GcyK1D1fUc8";
    }

    function safeMint(address to, uint256 tokenId, string memory uri, uint256 price)
        public
        onlyOwner
    {
        _safeMint(to, tokenId);
        ...
        returns (string memory)
    {
        ...
        string memory baseURI = _baseURI();
        string memory tokenURI_ = ERC721URIStorage.tokenURI(tokenId);
        bytes memory tokenURIBytes = bytes(tokenURI_);
        if (tokenURIBytes.length > 0) {
            return string(abi.encodePacked(baseURI, tokenId, "/", tokenURI_));
        } else {
