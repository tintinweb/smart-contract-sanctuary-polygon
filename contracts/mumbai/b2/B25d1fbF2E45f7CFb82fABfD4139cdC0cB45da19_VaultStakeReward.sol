...
import "./interface/IPerpTrade.sol";
import "./interface/IKiloConfig.sol";
import "./access/OwnerGovernableUpgradeable.sol";
import "./lib/UniERC20Upgradeable.sol";
import "@openzeppelin/contracts/utils/math/Math.sol";
import "hardhat/console.sol";


contract VaultStakeReward is OwnerGovernableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20Upgradeable, ERC4626Upgradeable {
    ...
    enum StakeType {Stake, Redeem}

    IKiloConfig private kiloConfig;
    ...
    event OwnerSetPerpTrade(address perpTrade);

    event StakedRedeemItem(
        address indexed user,
        StakeType stakeType,
        uint256 amount,
        uint256 shares,
        uint256 redeemRewardAmount
    );

    event OwnerSetVaultCapAndPeriod(
        uint256 cap,
    ...
    function initialize(
        ...
        kiloConfig = IKiloConfig(_kiloConfigAddr);
    ...
    function setPerpTrade(address _perpTrade) external onlyOwner {
        ...
        emit OwnerSetPerpTrade(_perpTrade);
    }

    function _updateReward(address user) private {
        if (user == address(0)) return;
        uint256 vaultReward = IPerpTrade(perpTrade).distributeVaultReward();
    }

    function deposit(uint256 amount, address user) public override nonReentrant returns (uint256) {
        //校验用户和状态
        require(kiloConfig.getKiloConfig().canUserStake, "!stake");
        require(msg.sender == user || msg.sender == address(this), "!stake");
        require(IERC20Upgradeable(token).balanceOf(address(this)) * BASE / tokenBase + amount <= cap, "!cap");
        //更新下当前用户stake奖励
        _updateReward(user);
        IPerpTrade(perpTrade).distributePendingPnl();
        //记录用户stake数据
        ...
        emit StakedRedeemItem(user, StakeType.Stake, newAmount, shares, 0);
    ...
    function redeem(
        ...
        require(shares <= totalSupply(), "!redeem");
        ...
        IPerpTrade(perpTrade).distributePendingPnl();
        Stake storage _stake = stakes[user];
        require(block.timestamp - _stake.timestamp > stakingPeriod, "!period");
        uint256 assets = previewRedeem(shares);
        require(IPerpTrade(perpTrade).totalOpenInterest() <= (IERC20Upgradeable(token).balanceOf(address(this)) - assets) * BASE / tokenBase * kiloConfig.getKiloConfig().utilizationMultiplier / (10 ** 4), "!utilized");
        uint256 amount = shares * _stake.amount / userShare;
        _stake.amount -= uint96(amount);
        staked -= uint256(amount);
        _withdraw(_msgSender(), receiver, owner, assets, shares);
        emit StakedRedeemItem(user, StakeType.Redeem, amount, shares, assets);
        return assets * BASE / tokenBase;
    }

    function updateVaultCapAndPeriod(uint256 _cap, uint256 _stakingPeriod) external onlyOwner {
        require(_cap > 0 && _stakingPeriod > 0 && _stakingPeriod < 30 days, "!allowed");
        cap = _cap;
        stakingPeriod = _stakingPeriod;
        emit OwnerSetVaultCapAndPeriod(_cap, _stakingPeriod);
    }

    function incrVaultBalance(uint256 increasedBalance) external {
        require(msg.sender == perpTrade, "not allowed");
    }

    function decrVaultBalance(uint256 decreasedBalance) external {
        require(msg.sender == perpTrade, "not allowed");
        IERC20Upgradeable(token).safeTransfer(msg.sender, decreasedBalance * tokenBase / BASE);
    }

    function transfer(address to, uint256 amount) public override(IERC20Upgradeable, ERC20Upgradeable) returns (bool) {
        address owner = _msgSender();
        Stake storage _from_stake = stakes[owner];
        Stake storage _stake = stakes[to];
        _stake.timestamp = uint128(Math.max(_from_stake.timestamp, _stake.timestamp));
    ...
    function transferFrom(
        ...
        Stake storage _from_stake = stakes[from];
        Stake storage _stake = stakes[to];
        _stake.timestamp = uint128(Math.max(_from_stake.timestamp, _stake.timestamp));
        return true;
    }
    ...
    function getShare(address stakeOwner) external view returns (uint256) {
        return IERC20Upgradeable(address(this)).balanceOf(stakeOwner);
    }
    ...
    //获取perp trade pending pnl  + pending fee
    function getVaultBalance() external view returns (uint256) {
        (int256 pendingPnl,uint256 pendingVaultReward) = IPerpTrade(perpTrade).getPendingBalance();
        //正(用户盈利):vault需要补充到perp 负(用户亏损):perp补充到vault
        int256 pendingBalance = pendingPnl;
        if (pendingBalance > 0) {
            return IERC20Upgradeable(token).balanceOf(address(this)) * BASE / tokenBase - uint256(pendingBalance);
        } else if (pendingBalance < 0) {
            return IERC20Upgradeable(token).balanceOf(address(this)) * BASE / tokenBase + uint256(-1 * pendingBalance);
        } else {
            return IERC20Upgradeable(token).balanceOf(address(this)) * BASE / tokenBase;
        }
    }
}
...
interface IKiloConfig {
    struct KiloConfig {
        uint256 minMargin;//开仓最小保证金
        uint256 protocolRewardRatio;// = 2000;  //协议奖励比例，手续和清算费 20%
        uint256 maxShift;// = 0.002e8; // max shift (shift is used adjust the price to balance the longs and shorts)
        uint256 minProfitTime;// = 6 hours;
        uint256 exposureMultiplier;// = 10000;
        uint256 utilizationMultiplier;// = 10000; //金库利用率乘数
        uint256 maxExposureMultiplier;// = 3;
        uint256 liquidationBounty;// = 5000; //清算奖励比例 In bps. 5000 = 50%
        uint256 liquidationThreshold;// = 8000; //清算门槛 In bps. 8000 = 80%
        bool canUserStake;// = true;//用户是否可以stake
        bool allowPublicLiquidator;// = false;//是否公共清算人操作
        bool isTradeEnabled;
        uint256 adlMultiplier;
    }

    function token() external returns(address);
    function tokenBase() external returns(uint256);

    function feeCalculatorAddr() external view returns(address);
    function fundingManagerAddr() external view returns(address);
    function perpTradeAddr() external view returns(address);
    function kiloPriceFeedAddr() external view returns(address);
    function orderBookAddr() external view returns(address);
    function positionManagerAddr() external view returns(address);
    function stakeRewardAddr() external view returns(address);
    function productManagerAddr() external view returns(address);
    function borrowingManagerAddr() external view returns(address);
    function liquidationThreshold() external view returns(uint256);

    function canUserStake() external view returns(bool);
    function getKiloConfig() external view returns (KiloConfig memory kiloConfig);
}
...
interface IPerpTrade {
    ...
    function distributePendingPnl() external returns(int256);
    ...
    function getProduct(uint256 productId) external view returns (
        address,uint256,uint256,uint256,bool,uint256,uint256,uint256,uint256,uint256);
    function getPosition(
        address account,
        uint256 productId,
        bool isLong
    ) external view returns (uint256,uint256,uint256,uint256,uint256,address,uint256,bool,int256,uint256);
    ...
    function getPendingBalance() external view returns (int256,uint256);
}
...
contract OwnerGovernableUpgradeable is GovernableUpgradeable {
    ...
    modifier onlyOwner() {
        ...
        if (msg.sender != owner) revert SenderNotOwner(owner, msg.sender);
        _;
    }
...
library UniERC20Upgradeable {
    ...
    function uniTransferFrom(IERC20Upgradeable token, address from, address to, uint256 amount) internal {
        if (amount > 0) {
            if (isETH(token)) {
...
contract GovernableUpgradeable is Initializable {
    ...
    error SenderNotGov(address gov, address sender);
    ...
    modifier onlyGov() {
        if(msg.sender != gov) revert SenderNotGov(gov, msg.sender);
        _;
    }
