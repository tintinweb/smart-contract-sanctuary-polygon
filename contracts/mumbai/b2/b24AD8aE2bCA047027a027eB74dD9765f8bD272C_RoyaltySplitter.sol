...
contract RoyaltySplitter is IRoyaltySplitter, Initializable, AccessControlUpgradeable {
    ...
    // royaltyForwarder => royaltyInfo
    mapping(address => RoyaltyInfo) public royaltyInfos;

    event RoyaltySplitterSet(IRoyaltySplitter indexed royaltySplitter);
    event RoyaltyRegistered(address indexed royaltyForwarder);
    event EtherRoyaltyReleased(address indexed royaltyForwarder, uint256 amount);
    event ERC20RoyaltyReleased(address indexed royaltyForwarder, address token, uint256 amount);
    ...
    ) external onlyRole(REGISTRAR_ROLE) returns (address, uint96) {
        if (royalties.length == 0) {
            revert NoRoyaltiesFound();
        }

        bytes32 salt = _generateSalt(collection, tokenId);
        address royaltyForwarder = address(new RoyaltyForwarder{ salt: salt }(this));
        uint96 totalShares = _setRoyaltyInfo(royaltyForwarder, royalties);

        emit RoyaltyRegistered(royaltyForwarder);
        return (royaltyForwarder, totalShares);
    }

    function releaseRoyalty() external payable {
        uint256 amount = msg.value;
        if (amount == 0) {
            revert ZeroRoyaltyAmount();
        }
    ...
    function _setRoyaltyInfo(
        ...
        for (uint256 i = 0; i < royaltyCount; ) {
            ...
            royaltyInfos[royaltyForwarder].royalties.push(royalties[i]);
            unchecked {
                i++;
            }
        }

        royaltyInfos[royaltyForwarder].totalShares = totalShares;
        return totalShares;
    }

    function _generateSalt(address collection, uint256 tokenId) private pure returns (bytes32) {
