...
contract Hackjack is VRFConsumerBaseV2, BreakdownUint256, Destructible, RequestQueue {
    ...
    event RandomnessFulfilled(uint requestIndex, uint256 handId, uint8 randomCard, requestType _requestType);
    ...
    function requestStand(uint256 _handId) internal {
        ...
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHitFinal);
    }



    function sendRandomRequest(bool isPlayer,bool isDealer,uint _handId,requestType _requestType) public {
        // Check for space in _requestQueue 
        if (length() >= numWords*32) {
            revert("Error: Sorry, all of our dealers are busy. Please try again in a few minutes.");
        }

        // Create request and add to queue
        uint256 requestIndex = _requestCounter.current();
        requestInfo memory newRequest = requestInfo(false, isPlayer,isDealer,0,_handId, _requestType,requestIndex);
    ...
    function fulfillRandomWords(
        ...
        for (uint i = 0 ; i < numberOfRandomWordsNeeded; i++) {
            uint8[] memory randomUint8s = getUint256BrokenIntoUint8(randomWords[i]);
    ...
    function fulfillRandomRequest(
        ...
        uint8 randomWord
            ...
            emit RandomnessFulfilled(requestIndex, requestIndexToRequestInfo[requestIndex].handId, randomWord% 13, requestIndexToRequestInfo[requestIndex].requestType);

            // Mark fulfilled
            requestIndexToRequestInfo[requestIndex].isFulfilled = true;
            requestIndexToRequestInfo[requestIndex].randomCard = randomWord % 13;

            // Dispatch Request to the right type and player
            // Type
        
            if (requestIndexToRequestInfo[requestIndex].requestType == requestType.dealNewHandPlayer) {
                hands[requestIndexToRequestInfo[requestIndex].handId].playerCards.push(requestIndexToRequestInfo[requestIndex].randomCard);
            } 
            
            else if (requestIndexToRequestInfo[requestIndex].requestType == requestType.dealNewHandFinal) {
                hands[requestIndexToRequestInfo[requestIndex].handId].dealerCards.push(requestIndexToRequestInfo[requestIndex].randomCard);
                ...
                hands[requestIndexToRequestInfo[requestIndex].handId].isLocked = false;
            } 
            
            
            else if (requestIndexToRequestInfo[requestIndex].requestType == requestType.playerHit) {
                hands[requestIndexToRequestInfo[requestIndex].handId].isLocked = false;
                hands[requestIndexToRequestInfo[requestIndex].handId].playerCards.push(requestIndexToRequestInfo[requestIndex].randomCard);
                setHandTotal(requestIndexToRequestInfo[requestIndex].handId,true);
                if (hands[requestIndexToRequestInfo[requestIndex].handId].playerTotal > 21) {
                    hands[requestIndexToRequestInfo[requestIndex].handId].isBusted = true;
                    resolveBustedHand(requestIndexToRequestInfo[requestIndex].handId);
                }
            } 
            
            
            else if (requestIndexToRequestInfo[requestIndex].requestType == requestType.dealerHit) {
                hands[requestIndexToRequestInfo[requestIndex].handId].isLocked = false;
                hands[requestIndexToRequestInfo[requestIndex].handId].dealerCards.push(requestIndexToRequestInfo[requestIndex].randomCard);

            }

            else if (requestIndexToRequestInfo[requestIndex].requestType == requestType.dealerHitFinal) {
                hands[requestIndexToRequestInfo[requestIndex].handId].isLocked = false;
                hands[requestIndexToRequestInfo[requestIndex].handId].dealerCards.push(requestIndexToRequestInfo[requestIndex].randomCard);
                setHandTotal(requestIndexToRequestInfo[requestIndex].handId,false);
            }
    ...
    modifier onlyUnlocked() {
        ...
        require(hands[_handId].isLocked == false, "Error: Your hand is locked and waiting for cards to be dealt.");
        _;
    }
    ...
    modifier onlyOneHand() {
        require(!isPlaying[msg.sender],"Error: You can only play one hand at a time.");
        _;
    }

    /// Players must be playing a hand to hit or stand
    modifier onlyAlreadyPlaying() {
        require(isPlaying[msg.sender], "Error: You can only hit or stand while you're playing a hand.");
        _;
    }
