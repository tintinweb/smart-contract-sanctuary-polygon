...
contract Product020802V01 is OwnableV01 {
    ...
    struct PolicyParams {
        ...
        uint256 renewalType;
        string package;
        WkmDateTime.Exposure exposure;
        string[] selectedOptions;
        WkmDateTime.Coverage[] selectedCoverages;
        uint256 split;
        bool isOfLegalAge;
        uint256 policyholderType;
        string commonSensitiveData;
    ...
    mapping(string => WkmDateTime.Policy) private allPolicies;
    ...
    function createPolicy(
        ...
        WkmDateTime.Policy storage policy = allPolicies[policyParams.onChainPolicyId];
    ...
    function endorsePolicy(
        ...
        WkmDateTime.EndorsementParamsStruct[] memory endorsmentParams,
        ...
        for (uint256 index = 0; index < endorsmentParams.length; index++) {
            if (keccak256(abi.encodePacked((endorsmentParams[index].key))) == "coverageStartDate") {
            ...
            WkmDateTime.endorsePolicyLib(onChainPolicyId, endorsmentParams[index], allPolicies);
        }
    ...
    function cancelPolicy(
        ...
        WkmDateTime.Policy storage policy = allPolicies[onChainPolicyId];
        require(
    ...
    function getPolicyHolderData(
        ...
        WkmDateTime.Policy memory policy = allPolicies[onChainPolicyId];
    ...
    {
        WkmDateTime.Policy memory policy = allPolicies[onChainPolicyId_];
    ...
    function getPolicyCoverages(
        ...
        returns (
            ...
            WkmDateTime.Coverage[] memory selectedCoverages
        )
    {
        WkmDateTime.Policy memory policy = allPolicies[onChainPolicyId];
    ...
    {
        WkmDateTime.Exposure memory exposure = allPolicies[onChainPolicyId].exposure;
...
library WkmDateTime {
    ...
    function computeCoverageStartDate(
        ...
        int256 requestedCoverageStartDateComparedToSubscriptionDate = dailyCompareTo(
        ...
        return setDateAtMidnight(requestedCoverageStartDate, timeZoneOffset) + 1 minutes;
    }
    ...
    function endorsePolicyLib(
        string memory onChainPolicyId,
        EndorsementParamsStruct memory endorsmentParams,
        mapping(string => Policy) storage allPolicies
    ) public {
        if (keccak256(abi.encodePacked((endorsmentParams.key))) == "riskType") {
            allPolicies[onChainPolicyId].riskType = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "termination") {
            allPolicies[onChainPolicyId].termination = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "renewalType") {
            allPolicies[onChainPolicyId].renewalType = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "package") {
            allPolicies[onChainPolicyId].package = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "selectedOptions") {
            allPolicies[onChainPolicyId].selectedOptions = endorsmentParams.stringValueArray;
        }
        // else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "selectedCoverages") {
        //     allPolicies[onChainPolicyId].selectedCoverages = selectedCoverages;
        // }
        else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "split") {
            allPolicies[onChainPolicyId].split = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "isOfLegalAge") {
            allPolicies[onChainPolicyId].isOfLegalAge = endorsmentParams.uintValue == 1 ? true : false;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "commonSensitiveData") {
            allPolicies[onChainPolicyId].commonSensitiveData = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "claimsAdminSensitiveData") {
            allPolicies[onChainPolicyId].claimsAdminSensitiveData = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "policiesAdminSensitiveData") {
            allPolicies[onChainPolicyId].policiesAdminSensitiveData = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "distributorSensitiveData") {
            allPolicies[onChainPolicyId].distributorSensitiveData = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "insurerSensitiveData") {
            allPolicies[onChainPolicyId].insurerSensitiveData = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "vehicleType") {
            allPolicies[onChainPolicyId].exposure.vehicleType = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "brand") {
            allPolicies[onChainPolicyId].exposure.brand = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "model") {
            allPolicies[onChainPolicyId].exposure.model = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "purchaseDate") {
            allPolicies[onChainPolicyId].exposure.purchaseDate = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "purchasePrice") {
            allPolicies[onChainPolicyId].exposure.purchasePrice = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "purchaseCurrency") {
            allPolicies[onChainPolicyId].exposure.purchaseCurrency = endorsmentParams.stringValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "usage") {
            allPolicies[onChainPolicyId].exposure.usage = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "householdSize") {
            allPolicies[onChainPolicyId].exposure.householdSize = endorsmentParams.uintValue;
        } else if (keccak256(abi.encodePacked((endorsmentParams.key))) == "maxSpeed") {
