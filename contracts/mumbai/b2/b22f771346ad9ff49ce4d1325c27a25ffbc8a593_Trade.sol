...
contract Trade is Initializable,OwnableUpgradeable {
    ...
    uint256 deci;
      ...
      function initialize() public initializer  {
        ...
        // addTokenType("USDT",0xdAC17F958D2ee523a2206206994597C13D831ec7,2500000000000000000); 
    }
 ...
 function DeleteTokenType (string memory _type) public onlyOwner{
    ...
    function saleToken(address payable from, uint256 tokenId, uint256 amount, uint256 nooftoken, uint nftType, address _conAddr,uint256
    royal) public payable {
        require(amount == order_place[from][tokenId].price.mul(nooftoken) , "Invalid Balance");
        _saleToken(from, amount, "ETH",royal);
        if(nftType == 721){
    ...
    function _saleToken(address payable from, uint256 amount, string memory bidtoken,uint256 royal) internal {
        ...
        uint256 dyntokfee =  Tokendetail[bidtoken].tokenfee;
            ...
            (uint256 _adminfee,uint256 roy, uint256 netamount) = calc(amount,royal, serviceValue);
        ...
        // );
            ...
            require( approveValue >= val.div(10**Tokendecimals), "Insufficient Balance");
                  ...
                  (uint256 _adminfee,uint256 roy, uint256 netamount) = calc(amount,royal, dyntokfee);
    ...
    function saleWithToken(address payable from, uint256 tokenId, uint256 amount, uint256 nooftoken, string memory bidtoken, uint nftType, address _conAddr,uint256 royal) public{
        require(amount == order_place[from][tokenId].price.mul(nooftoken) , "Insufficent Balance");
        _saleToken(from, amount, bidtoken,royal);
        if(nftType == 721){
            IERC721Upgradeable(_conAddr).safeTransferFrom(from, msg.sender, tokenId);
        }
        else{
    ...
    function acceptBId(string memory bidtoken,address bidaddr, uint256 amount, uint256 tokenId, uint256 nooftoken, uint256 nftType, address _conAddr,uint256 royal) public{
        _acceptBId(bidtoken, bidaddr, owner(), amount,royal);
        if(nftType == 721){
    ...
    function _acceptBId(string memory tokenAss,address from, address admin, uint256 amount,uint256 royal) internal{
        uint256 val = pERCent(amount, serviceValue).add(amount);
