...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  // a cooldown time where proposals cannot be executed until it elapses
  uint256 public cooldown = 1 days;

  // a chainlink subscription ID used to fund the chainlink function requests
  uint64 public subscriptionId;

  event SubscriptionIdUpdated(uint64 indexed oldSubscriptionId, uint64 indexed newSubscriptionId);
  ...
  function getTxData(string memory proposalId, uint256 txIndex) public view returns (bytes32) {
    bytes32[] memory txData = proposals[proposalId];
    require(txData.length > 0, "No tx data found for index");
    return txData[txIndex];
  }
  ...
  // allows cooldown to be updated
  function updateCooldown(uint256 newCooldown) public onlyOwner {
    cooldown = newCooldown;
  }

  // allows the subscription ID to be updated
