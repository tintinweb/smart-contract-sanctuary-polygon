...
contract DaoPublic is LinkedList, OwnableUpgradeable {
    ...
    function initialize(IXenoverse xenoverse_,
    ...
    function voteNfts(uint256 index) external {
        require(xenoverse.balanceOf(msg.sender) >999  ether, "Must hold 100 XEN Tokens");
        ...
        xenoverse.transferFrom(msg.sender, fundsWallet, 10e18);
    ...
    function _announceWinner() internal {
        ...
        if (
            ...
            xenoverse.mint(nftInfoo[index].owner,720e18);
            emit claimed(
    ...
    function claimBatch(uint256[] calldata indexes) public {
        for (uint256 i; i < indexes.length; i++) {
            ...
            uint256 amount = 180e18 / (nftInfoo[indexes[i]].votersCount);
            isclaimed[indexes[i]][msg.sender] = true;
            xenoverse.mint( msg.sender , amount );
            emit claimed( msg.sender, indexes[i], amount, block.timestamp);
        }
    }
