    ...
    ERC721RoyaltyUpgradeable
{
    ...
    function initialize(string calldata baseURI_, uint256 maxsupply_)
        public
        initializer
    {
        __ERC721_init("Escada Cherry in Japan Art NFT", "ECIJNFT");
    ...
    function mintBatch(address[] memory recipients) external onlyOwner {
        uint256 tokenId = _tokenIdCounter.current();
        require(
            (tokenId + recipients.length) - 1 <= maxsupply,
            "Exceed max supply"
        );
    ...
    function mintAll(address to) external onlyOwner {
        uint256 toMint = maxsupply - _tokenIdCounter.current();
        require(toMint > 0, "All gone Pete");
        while (_tokenIdCounter.current() < toMint) {
