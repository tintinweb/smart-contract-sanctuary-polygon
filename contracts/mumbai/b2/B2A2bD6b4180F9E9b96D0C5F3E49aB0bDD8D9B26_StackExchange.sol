...
contract StackExchange is Ownable {
  uint256 constant internal INIT_SHARES = 2e12;
  uint256 constant internal OFFSET = 1e20;
  uint256 constant internal BASE_PERCENT = 1e6;
  uint256 constant private SWAP_FEE = 50000;
  //uint8 constant public MAX_RECRUITS = 8;
  ...
  struct User {
    address recruiter;
    address[8] recruitAccounts;
    uint[8] recruitPending;
  }
  ...
  address public incentiveSupply;
  ...
  /*function getUserInfo(address account) external view returns (address recruiter, address[MAX_RECRUITS] memory recruits, uint[MAX_RECRUITS] memory pending) {
    User memory user = userMap[account];
    recruiter = user.recruiter;
    for (uint8 i = 0; i < user.recruits.length; i++) {
      recruits[i] = user.recruits[i].account;
      pending[i] = user.recruits[i].pending;
    }
  }*/
  ...
  function setRecruit(uint8 index, address account) external {
    require(index < userMap[address(msg.sender)].recruitAccounts.length, "INVALID_INDEX");
    userMap[address(msg.sender)].recruitAccounts[index] = account;
  }
  ...
  function removeLiquidity(address token, uint shares) external {
    ...
    uint256 tokenValue = pool.tokenReserve * shares / pool.shares;
    pool.shares -= shares;
    poolUser.shares -= shares;
  ...
  function swap(address sellToken, address buyToken, address receiver, uint sellAmount) external {
    ...
    if (sellToken != baseToken) {
      ...
      sellPool.tokenReserve += amount;
      amount = sellPool.baseReserve - (((sellPool.tokenReserve - amount) * sellPool.baseReserve) / sellPool.tokenReserve);
    ...
    if (buyToken != baseToken) {
      ...
      amount = buyPool.tokenReserve - (((buyPool.baseReserve - amount) * buyPool.tokenReserve) / buyPool.baseReserve);
      buyPool.tokenReserve -= amount;
    }
  ...
  function harvestUser() public {
    ...
    uint pending = 0;
    for (uint8 i = 0; i < user.recruitPending.length; i++) {
      pending += user.recruitPending[i];
      user.recruitPending[i] = 0;
    }
    if (!IERC20(rewardToken).transfer(address(msg.sender), pending)) {
  ...
  function harvestPool(address token) public {
    ...
    if (poolUser.pending > 0) {
      if (!IERC20(rewardToken).transfer(address(msg.sender), poolUser.pending)) {
        revert("TRANSFER_FAILED");
      }
      poolUser.pending = 0;
    }
  ...
  function deductSwapFee(Pool storage pool, uint amount) private returns (uint) {
    ...
    uint reward = rewardPool.tokenReserve - (((rewardPool.baseReserve - amount) * rewardPool.tokenReserve) / rewardPool.baseReserve);
...
contract StackToken is ERC20, Ownable {
    constructor(uint256 initialSupply) ERC20("Stack Exchange", "STACK") {
      if (initialSupply > 0) {
