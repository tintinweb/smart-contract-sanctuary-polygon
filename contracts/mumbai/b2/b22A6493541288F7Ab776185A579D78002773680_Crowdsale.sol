...
contract Crowdsale is PullPayment, Ownable, AccessControl {
    ...
    function mint(uint256 _amount) public payable onlyPositive(_amount) {
        ...
        if (block.timestamp <= closingTime) {
            ...
            uint256 surplus = quotas[miner].sub(sold[miner]);
            require(surplus >= 0, "Over the limit");
        } else {
            sold[miner] = _amount.add(sold[miner]);
            uint256 surplus = max.sub(sold[miner]);
            require(surplus >= 0, "Exceeded maximum quantity limit");
        }
    ...
    function limit(address _account) public view returns (uint256) {
        if (block.timestamp <= closingTime) {
            return quotas[_account].sub(sold[_account]);
        } else {
            return max.sub(sold[_account]);
        }
    }
