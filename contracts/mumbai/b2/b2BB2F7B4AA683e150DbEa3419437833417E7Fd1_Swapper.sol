...
contract Swapper is Ownable {
    ...
    address inputTokenA = 0x53287e027dcC0bFd0E40675c7182C3Ea96B0f2ea;
    address inputTokenB = 0x836b7AF3D4D712A5524e0DD625582f00b6a6f139;
    address outputToken = 0x20eCCcd9474904cb75846c9B1D2e36D2794F2044;
    ...
    function swap(address _token, uint amount) external {
        ...
        require(ERC20(_token).balanceOf(msg.sender) >= amount, "Insufficient token balance to swap");

        TokenC(outputToken).mint(amount);
    ...
    function unswap(address _token, uint amount) external {
        require(_token == inputTokenA || _token == inputTokenB, "Your token cannot be swapped");
        require(ERC20(outputToken).balanceOf(msg.sender) >= amount, "Insufficient token balance to swap");

        ERC20(outputToken).approve(address(this), amount);
        ERC20(outputToken).transferFrom(msg.sender, address(this), amount);

        ERC20(_token).transferFrom(address(this), msg.sender, amount);
    }
}
