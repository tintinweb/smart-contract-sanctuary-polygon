    ...
    ITind3rMembership
{
    ...
    mapping(address => EnumerableSetUpgradeable.UintSet) private _likeSet;
    ...
    function ifLike(address userA, address userB) public view returns (bool) {
        ...
        return _likeSet[userA].contains(userIdB);
    }
    ...
    function _setLike(address userA, address userB) private {
        ...
        uint256 userIdB = getUserId(userB);
        if (_likeSet[userA].contains(userIdB)) revert AlreadyLike();
        _likeSet[userA].add(userIdB);
        ++_userElo[userB];
    }
