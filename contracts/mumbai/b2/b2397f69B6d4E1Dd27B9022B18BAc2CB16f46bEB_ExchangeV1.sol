...
contract ExchangeV1 is IExchange, IxBaseV1, FeeWorkerV1 {
    ...
    constructor(
        ...
        address forwarder,
        address manager
    ) IxBaseV1(admin, forwarder) {
        _grantRole(WITHDRAWER_ROLE, admin);
        _setFeeManager(manager);
    }

    function versionRecipient() external pure override returns (string memory) {
        return "intella-exchange-v1;opengsn-2.2.5";
    }

    function setFeeManager(address manager) external onlyRole(OWNER_ROLE) {
        // if manager is address(0) then fee recharge will be skipped
        _setFeeManager(manager);
    }

    // token whitelist management
    function registerToken(address token) external onlyRole(OWNER_ROLE) {
        require(token != address(0), "token can not be zero address");
        require(_allowedTokens.add(token), "token already registered");

        emit TokenRegistered(token);
    }

    function unregisterToken(address token) external onlyRole(OWNER_ROLE) {
        require(_allowedTokens.remove(token), "token is not registered");
    ...
    function registeredTokenAt(uint256 index) external view returns (address) {
        return _allowedTokens.at(index);
    }
    ...
    function registerPool(address pool) external onlyRole(OWNER_ROLE) {
        require(pool != address(0), "pool can not be zero address");
        require(_allowedPools.add(pool), "pool already registered");

        emit PoolRegistered(pool);
    }

    function unregisterPool(address pool) external onlyRole(OWNER_ROLE) {
        require(_allowedPools.remove(pool), "pool is not registered");

        emit PoolUnregistered(pool);
    }

    function registeredPoolCount() external view returns (uint256) {
        return _allowedPools.length();
    }

    function registeredPoolAt(uint256 index) external view returns (address) {
        return _allowedPools.at(index);
    }
    ...
    function depositToken(
        ...
        require(isRegisteredPool(pool), "pool not registered");
        require(isRegisteredToken(token), "token not registered");

        address user = _msgSender();

        IERC20(token).safeTransferFrom(user, pool, amount);

        emit Deposited(pool, user, token, amount, memo);
    }
    ...
    {
        try this.innerWithdrawToken(pool, user, token, amount, collector, fee) {
            // success
            _log(
                pool,
                user,
    ...
    function innerWithdrawToken(
        ...
        require(
            ...
            "only can be called by Exchange itself"
        );
        ...
        require(
            collector != address(0),
            "fee collector address can not be zero"
        );
        require(token != address(0), "token address can not be zero");
        require(amount > 0, "amount must be greather than zero");
        require(fee >= 0, "amount must be greather than or equal to zero");
        require(fee <= amount, "fee must not exceed amount");

        IERC20(token).safeTransferFrom(pool, user, amount.sub(fee));
        if (fee > 0) {
            IERC20(token).safeTransferFrom(pool, collector, fee);
        }
    }
}
    ...
    ReentrancyGuard
{
    ...
    modifier nonRelayable() {
        require(
            ...
            "IxBaseV1: meta transaction not allowed"
        );
    ...
    function setForwarderAddress(address forwarder)
        external
        onlyRole(OWNER_ROLE)
    {
        _setTrustedForwarder(forwarder);
...
abstract contract FeeWorkerV1 {
    ...
    function _setFeeManager(address manager) internal {
        feeManager = manager;
    }

    function _tryRechargeGasFee(address target) internal {
        if (feeManager != address(0)) {
            ...
            try IFeeManager(feeManager).rechargeGasFee(target) {
                ...
                // failed due to configuration (role, address, gas fee, etd) and has string reason
                revert(reason);
            } catch {
                // failed due to configuration (role, address, gas fee, etd)
                revert(
                    "check fee manager configuration, maybe invalid fee manager address"
                );
            }
        }
    }
...
interface IFeeManager {
    event GasFeeRecharged(
        address indexed target,
        uint256 amount,
        RechargeStatus indexed status,
        string reason,
        bytes lowLevelReason
    );
