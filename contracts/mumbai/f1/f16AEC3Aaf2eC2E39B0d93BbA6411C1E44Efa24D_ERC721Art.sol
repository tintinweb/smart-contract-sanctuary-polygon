    ...
    ERC2981Upgradeable
{
    ...
    function initialize(
        ...
        price[Coin.USDC_TOKEN] = _priceInUSD;
        price[Coin.DAI_TOKEN] = _priceInUSD;
    ...
    modifier onlyAuthorized() {
        require(
            ...
            "ERC721Art: caller not manager or creator"
        );
        _;
    }
    ...
    /** @notice mints given the NFT of given tokenId, using the given coin for transfer.
        @param _tokenId: tokenId to be minted 
        @param _coin: token/coin of transfer */
    function mint(uint256 _tokenId, Coin _coin)
        public
        payable
        virtual
        whenNotPaused
        ...
        checkPrice(_coin, msg.value, msg.sender)
    {
        if (onlyERC20) {
            require(
                _coin != Coin.ETH_COIN,
                "ERC721Art: only ERC20 tokens allowed"
            );
        }

        _doAllTransfersMint(_coin, price[_coin], _tokenId);
    ...
    function creatorsProSafeTransferFrom(
        ...
        whenNotPaused
        nonReentrant
        checkPrice(coin, msg.value, from)
    {
        ...
        _doAllTransfers(coin, _tokenPrice, tokenId, to);
    }
    ...
    /** @dev executes all the royalties and payment transfers
        @param _coin: which coin/token to use for transfer
        @param _price: price of NFT 
        @param _tokenId: token ID of the NFT to be transferred
        @param _NFTOwner: address of the NFT owner to be payed */
    function _doAllTransfers(
        Coin _coin,
        uint256 _price,
        uint256 _tokenId,
        address _NFTOwner
        ...
        uint256 CreatorsProRoyalty = (_price * management.fee()) /
            _feeDenominator();
        _coinTransfer(CreatorsProRoyalty, _coin, management.multiSig());

        // collection creator transfer
        (, uint256 collectionCreatorRoyalty) = royaltyInfo(_tokenId, _price);
        _coinTransfer(collectionCreatorRoyalty, _coin, owner());
        ...
        uint256 ownerValue = _price -
            CreatorsProRoyalty -
            collectionCreatorRoyalty;
        _coinTransfer(ownerValue, _coin, _NFTOwner);

        emit OwnerPaymentDone(_tokenId, _NFTOwner, ownerValue);
    }

    /** @dev executes all the royalties and payment transfers when minting
        @param _coin: which coin/token to use for transfer
        @param _price: price of NFT*/
    function _doAllTransfersMint(
        Coin _coin,
        uint256 _price,
        uint256 _tokenId
    ) internal {
        ...
        // collection creator transfer
        _coinTransfer(_price - CreatorsProRoyalty, _coin, owner());
    ...
    function _transfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721Upgradeable) onlyAllowedOperator(from) {
        require(
