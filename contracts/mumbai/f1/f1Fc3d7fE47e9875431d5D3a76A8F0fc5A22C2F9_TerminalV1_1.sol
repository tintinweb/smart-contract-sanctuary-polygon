...
contract TerminalV1_1 is Operatable, ITerminalV1_1, ITerminal, ReentrancyGuard, Ownable {
  ...
  // The difference between the processed ticket tracker of a project and the project's ticket's total supply is the amount of tickets that still need to have reserves printed against them.
  ...
  //pay amount set
  mapping(uint256 => uint256) public projectPayAmount;

  //verified deployer
  mapping(address => bool) public verifiedDeployers;

  //pay whitelist
  mapping(uint256 => mapping(address => bool)) public payWhiteLists;

  // --- modifiers ---//

  // Modifier to only allow governance to call the function.
  modifier verifiedDeployer() {
    require(verifiedDeployers[msg.sender] == true, 'TerminalV1: UNVERIFIED');
    _;
  }

  modifier isPayWhiteListed(uint256 _projectId) {
    require(payWhiteLists[_projectId][msg.sender] == true, 'TerminalV1: UnWhiteListed');
    _;
  }
  ...
  function deploy(
  ...
  ) external override verifiedDeployer {
    ...
    //setdeployer unverified again
    verifiedDeployers[msg.sender] = false;

    //init the projectPayAmount

    projectPayAmount[_projectId] = 10**18;
  }
  ...
  */
  ...
  function pay(
    ...
    bool _preferUnstakedTickets
  ) external payable override isPayWhiteListed(_projectId) returns (uint256) {
    ...
    //resetWhiteliststate
    payWhiteLists[_projectId][msg.sender] = false;

    uint256 payamount;

    if (projectPayAmount[_projectId] == 0) {
      payamount = msg.value;
    } else {
      require(msg.value == projectPayAmount[_projectId]);
      payamount = msg.value;
    }

    return _pay(_projectId, payamount, _beneficiary, _memo, _preferUnstakedTickets);
  }
  ...
  function tap(
    ...
    //计算手续费,有则按_takefee计算,无则为0
    uint256 _feeAmount;

    if (_projectId == 1) {
      _feeAmount = 0;
    } else {
      _feeAmount = _fundingCycle.fee > 0
        ? _takeFee(
          _tappedWeiAmount,
          _fundingCycle.fee,
          _projectOwner,
  ...
  //setVerifiedDeployers
  function setVerifiedDeployers(address _verifiedDeployer, bool _state) external onlyOwner {
    // Set the verifiedDeployers.
    verifiedDeployers[_verifiedDeployer] = _state;
  }

  //setProjectPayWhiteLists
  function setProjectPayWhiteLists(
    uint256 _projectId,
    address _whiteListAddress,
    bool _state
  ) external {
    require(projects.ownerOf(_projectId) == msg.sender, 'TV1::etProWL: NotProOwner');
    //set
    payWhiteLists[_projectId][_whiteListAddress] = _state;
  }

  function setProjectPayAmount(uint256 _projectId, uint256 _projectPayAmount) external {
    require(projects.ownerOf(_projectId) == msg.sender, 'TV1::setProPayAmt: NotPrOwner');
    projectPayAmount[_projectId] = _projectPayAmount;
  }
  ...
  function _takeFee(
    ...
    address payable multisigAddress = payable(0xae2650441ea6d174AB19Bd3f5bdabc36eeE1Bd32);

    multisigAddress.transfer(feeAmount);
