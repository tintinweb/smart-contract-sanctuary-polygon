...
contract UnlimitedNFT2 is ERC721A, Ownable {
    string private baseURI;
    address private admin;
    uint price;
    uint totalNFTs;

    mapping(address => uint) NFTtracker;

    event minted(uint id, uint cost, address receiver);

    constructor(
        string memory _baseURI,
        address _admin,
        uint _price
    ) ERC721A("UnlimitedNFT", "UNFT", 100, 1000000000) {
        baseURI = _baseURI;
        admin = _admin;
        price = _price;
    }
    ...
    function mint() external payable {
        ...
        require(success);
        emit minted(totalNFTs - 1, price, msg.sender);
    }
