...
contract PassHolderSale is Ownable {
    ...
    uint256 public maxCap = 200_000 ether;
    ...
    function _buyTokens(uint256 _amountToSpend) private {
        if (allocations[msg.sender].firstRoundUsedCap + _amountToSpend < minPurchaseAmount) revert InvalidAmount();
        ...
        if (!paymentToken.transferFrom(msg.sender, treasuryAddress, _amountToSpend)) {
    ...
    function _buyTokensFCFS(uint256 _amountToSpend) private {
        if (allocations[msg.sender].firstRoundMaxCap == 0) revert NoAllocation();
        if (allocations[msg.sender].secondRoundUsedCap + _amountToSpend < minPurchaseAmount) revert InvalidAmount();

        if (allocations[msg.sender].secondRoundUsedCap + _amountToSpend > allocations[msg.sender].secondRoundUserMaxCap) {
            revert ExceedsAllocation();
        }
