...
contract WowTPoints is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  function updateLeaderBoard(address account, uint256 totalPoints) private {
    ...
    for (uint256 i = topLeaderBoardAddress.length - 1; i > insertIndex; i--) {
...
contract WowTQuiz is OwnableUpgradeable {
  ...
  function createQuiz(
    ...
    require(
      userPoints > createEligibility || _msgSender() == owner(),
    ...
    points.reducePoints(_msgSender(), createEligibility);
  }
  ...
  function quizEval(string memory _quizName, bytes32 choice) public {
    ...
    require(!evalStatus[_quizName][_msgSender()], 'Already tried');
    Quiz memory qtemp = getQuizdetails(_quizName);
    require(_msgSender() != qtemp.creatorAddress, "Can't attend your own quiz");

    bool isCorrect = (qtemp.answer == choice);
    if (isCorrect) {
      points.addPoints(_msgSender(), pointsToAnswer, 'quiz');
    }
    points.addPoints(qtemp.creatorAddress, quizePoints, 'quiz');

    evalStatus[_quizName][_msgSender()] = true;
  ...
  function quizeEvaluatePoints(
    ...
    if (answer == choice) {
      points.addPoints(_userAddress, pointsToAnswer, 'quiz');
      emit Answer(true);
    } else {
      emit Answer(false);
    }
    points.addPoints(_creatorAddress, quizePoints, 'quiz');
  }
