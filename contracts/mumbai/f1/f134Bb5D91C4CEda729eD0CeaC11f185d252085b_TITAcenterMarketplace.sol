...
contract TITAcenterMarketplace is IERC721Receiver {
    ...
    function toggleSale(uint256 itemId, uint256 price) external {
        address tokenOwner = MarketItems[itemId].seller;
        ...
        if (MarketItems[itemId].sale) {
            require(_owner == address(this), "Not a owner of this token");
            MarketItems[itemId].sale = false;
    ...
    function buy(uint256 itemId) public payable {
        ...
        uint256 price = MarketItems[itemId].price + gasFee;
        address payable seller = MarketItems[itemId].seller;
        require(MarketItems[itemId].sale, "It's not ready for sale");
        require(
            msg.value >= price,
            "Please submit the asking price in order to complete the purchase"
        );
        IERC721(nftContract).transferFrom(tokenOwner, msg.sender, tokenId);
        payable(seller).transfer(MarketItems[itemId].price);
        payable(owner).transfer(gasFee);
    ...
    /* Get owenr by contract address with token id */
