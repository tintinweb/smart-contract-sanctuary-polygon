...
contract Exchange is ERC20 {
  // 1 state variable -> smart contract address of the crypto devs token
  address public cryptoDevsTokenAddress;

  constructor(address _cryptoDevsTokenAddress) ERC20("CryptoDev LP Token", "CDLP") {
    require(_cryptoDevsTokenAddress != address(0), "Cannot be address zero.");
    cryptoDevsTokenAddress = _cryptoDevsTokenAddress;
  }

  // function to get the reserved amount cryptodevs token held in this smart contract
  ...
  function addLiquidity(uint256 _amount) public payable returns (uint256) {
    ...
    uint ethBalance = address(this).balance; // balance of this contract
    ...
    if (cryptoDevsTokenReserve == 0) {
      ...
      uint cryptoDevsTokenAmount = (msg.value * cryptoDevsTokenReserve) / ethReserve;
      require(_amount >= cryptoDevsTokenAmount, "Amount of tokens is less than the minimum required");
      
      cryptoDevsToken.transferFrom(msg.sender, address(this), cryptoDevsTokenAmount);
  ...
  function removeLiquidity(uint256 _amount) public returns (uint, uint) {
    ...
    // token that will be returned to the contract/reserve
