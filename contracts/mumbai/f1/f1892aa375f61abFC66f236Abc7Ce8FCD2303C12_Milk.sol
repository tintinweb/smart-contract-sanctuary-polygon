...
//Milk is an utility token, with 0 economic value, used only in GoatGang game.
//it is fully decentralized, with only the rewards contract beeing able to mint it
//maxSupply is capped at 21 million
contract Milk is IMilk, Context, ERC20, ERC20Burnable, Ownable {
	...
	//usage of token outside the blockchain
	function cAction(
		uint256 nftID,
		uint256 id,
		string memory what
	...
	function transferFrom(
		...
		address recipient,
		uint256 amount
	) public virtual override(ERC20, IMilk) returns (bool) {
