...
contract BaseTournamentService is BaseService, ITournamentService {
  ...
  function joinTournament(bytes memory _signature, bytes memory _params) external virtual override onlyRoler("joinTournament") {
    ...
    require(IChampionUtils(championUtils).checkExtRestrictions(_serviceID, tournamentID, signer, championID), "TRV: Require owner"); // require owner of token
  ...
  function completeTournament(
    ...
    if (_additionalInfo.length > 0) {
      ...
      if (receivers.length > 0 && winnings > 0) {
        for (uint8 i; i < receivers.length; i++) {
          IChampionUtils(championUtils).disperseRewards(
            serviceID, //
            _tournamentID,
            _warriors[i].ID,
            (winnings * 70) / 100,
    ...
    for (uint256 i = 0; i < _warriors.length; i++) {
      ...
      if (_warriors[i].win_position == 1 && winnings > 0) {
        IChampionUtils(championUtils).disperseRewards(
          serviceID, //
          _tournamentID,
          _warriors[i].ID,
      ...
      if (_warriors[i].win_position == 2 && winnings > 0) {
        IChampionUtils(championUtils).disperseRewards(
          serviceID, //
          _tournamentID,
          _warriors[i].ID,
