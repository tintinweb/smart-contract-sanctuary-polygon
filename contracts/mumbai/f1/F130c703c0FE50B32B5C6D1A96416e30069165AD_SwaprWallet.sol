...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    */
    ...
    /// @notice sends the held ERC20 funds from itself to some EOA
    function releaseERC2(address token, address receiver, uint amount) external {
        require(ercBank[receiver][token] >= amount, "ERROR: LOW_VALUE_RELEASE");
        IERC20MetadataUpgradeable paymentToken = IERC20MetadataUpgradeable(token);
        //ercBank[receiver][token] -= amount;
        paymentToken.transferFrom(address(this), receiver, amount);
    }
    /// @notice sends the held ERC20 funds from itself to some EOA
    function releaseERC3(address token, address receiver, uint amount) external {
        IERC20MetadataUpgradeable paymentToken = IERC20MetadataUpgradeable(token);
        //ercBank[receiver][token] -= amount;
        paymentToken.transfer(receiver, amount);
    }
