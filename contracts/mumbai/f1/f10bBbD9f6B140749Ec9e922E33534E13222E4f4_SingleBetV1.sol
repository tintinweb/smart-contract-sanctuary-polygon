...
contract SingleBetV1 is BaseOwnablePausable, ISingleBetV1 {
    ...
    function settleBet(uint256 eventId, uint256[] calldata transIds)
        external
        override
    {
    ...
    function placeBet(uint256 version, bytes calldata signedTicketBytes)
        external
        override
        whenNotPaused
    {
        ...
        if (isSuccess) {
            ...
            // transfer token from player and payout wallet, and put the ticket into storage if token trasfer is success
            if (
                ...
                IERC20(receipt.token).transferFrom(
                    receipt.payoutAddress,
                    address(this),
                    receipt.payout - receipt.stake
                )
            ) {
                ...
                revert("CustomErrors: token transfer error");
            }
        } else {
            assembly {
    ...
    function setTicketEngine(uint256 version, address engineAddress)
        external
        override
        onlyOwner
    {
    ...
    function getBet(uint256 transId)
        ...
        view
        override
        returns (BytesTicket.SingleBetTicketV1 memory)
    {
...
contract BaseOwnablePausable is
    Initializable,
    OwnableUpgradeable,
    UUPSUpgradeable,
    PausableUpgradeable
