...
contract XMATIC {
	...
	constructor(address payable wallet, address payable _developer) public {
        ...
        plans.push(Plan(25, 70, 300)); // days , percent, booster
        plans.push(Plan(30, 50, 200)); // days , percent, booster
	...
	function _invest(address payable referrer, uint8 plan, address payable sender, uint256 value) private {
		...
		(uint256 percent, uint256 profit, uint256 finish, uint256 booster) = getResult(plan, value);
		
		user.deposits.push(Deposit(plan, percent, value, profit, block.timestamp, finish,booster));
	...
	function withdraw() public {
		...
		for (uint256 i = 0; i < user.deposits.length; i++) {
			if (user.checkpoint < user.deposits[i].finish) {
					...
					if (from < to) {
					    ...
					    uint256 booster = payout.mul(user.deposits[i].booster).div(PERCENTS_DIVIDER);
					    user.deposits[i].amount = user.deposits[i].amount.add(booster);
					    payout = payout.sub(booster);
						totalAmount = totalAmount.add(payout);
						totalBoosterAmount = totalBoosterAmount.add(booster);
					}
				
			}
		}
		
	    user.totalBooster = user.totalBooster.add(totalBoosterAmount);
	    ...
	    uint256 fee = totalBoosterAmount.mul(PROJECT_FEE).div(PERCENTS_DIVIDER);
		commissionWallet.transfer(fee);
		uint256 developerFee = totalBoosterAmount.mul(DEVELOPER_FEE).div(PERCENTS_DIVIDER);
		developerWallet.transfer(developerFee);
	...
	function getPlanInfo(uint8 plan) public view returns(uint256 time, uint256 percent, uint256 booster) {
		time = plans[plan].time;
		percent = plans[plan].percent;
		booster = plans[plan].booster;
	}
	...
	function getPercent(uint8 plan) public view returns (uint256) {
	 ...
	 function getBooster(uint8 plan) public view returns (uint256){
        
        uint256 time = block.timestamp.sub(startUNIX).div(TIME_STEP);
        
        if(plan == 0){
            return plans[plan].booster.sub(time.mul(0)); //reinvest decreases every day by 2%
        }
        
        if(plan == 1){
            return plans[plan].booster.sub(time.mul(0)); // reinvest decreases every day by 1%
        }
        
        if(plan == 2){
            return plans[plan].booster.sub(time.mul(0)); // reinvest decreases every day by 0.5%
        }
		if(plan == 3){
            return plans[plan].booster.sub(time.mul(0)); // reinvest decreases every day by 0.5%
        }
	...
	function getResult(uint8 plan, uint256 deposit) public view returns (uint256 percent, uint256 profit, uint256 finish, uint256 booster) {
		percent = getPercent(plan);
		booster = getBooster(plan);
	...
	function getUserBoosterAmount(address userAddress) public view returns(uint256){
	    return users[userAddress].totalBooster;
	}
	...
	function getUserDepositInfo(address userAddress, uint256 index) public view returns(uint8 plan, uint256 percent, uint256 amount, uint256 profit, uint256 start, uint256 finish, uint256 booster) {
	    User storage user = users[userAddress];
		...
		booster = user.deposits[index].booster;
	}
