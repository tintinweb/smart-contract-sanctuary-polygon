    ...
    Ownable
{
    ...
    string private constant SIGNING_DOMAIN = "Blockspace";
    string private constant SIGNATURE_VERSION = "1";
    address payable public _wallet;
    uint256 CurrentTokenId;

    mapping(address => uint256) pendingWithdrawals;
    mapping(uint256 => bool) NFTLock;
    mapping(string => address) public MailLink;

    constructor(address payable minter)
        ERC721("BlockSapce", "BlockSpace")
    ...
    function mint(address user, string memory uri, string memory mail) public onlyOwner {
        uint256 tokenId = CurrentTokenId + 1;
        CurrentTokenId = tokenId;
        _mint(user, tokenId);
        _setTokenURI(tokenId, uri);
        NFTLock[tokenId] = true;
        _setMailLink(user, mail);
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        require(NFTLock[tokenId] == false, "NFT is locked You can not Transfer this NFT");
        _transfer(from, to, tokenId);
    }
    ...
    function _setMailLink(address user, string memory userMail) internal {
        MailLink[userMail] = user;
    }

    // function setMailLink(string memory userMail) public {
    //     MailLink[userMail] = msg.sender;
    // }

    function tokensOwnedMail(string memory userMail) external view returns (uint256[] memory){
        address holder = MailLink[userMail];
        return tokensOwned(holder);
    }
    ...
    function transferMultiple(address from, address to, uint256[] calldata tokenId) public onlyOwner {
