...
// increment function
  ...
  function createMarketSale_erc721(uint256 itemId) public nonReentrant{
    ...
    if(markettokens[itemId].is_fixed==true ){
      IERC20 _exchangeToken = IERC20(0xf30A23E84E6e1DC121F19dEF24bB2804Ab9cd20C);
      require(_exchangeToken.allowance(msg.sender, address(this)) >= price , "Token allowance too low");
      ...
      _exchangeToken.transferFrom(msg.sender , markettokens[itemId].prev_owner, (price/10000)*(10000-commission-royality));
      _exchangeToken.transferFrom(msg.sender,markettokens[itemId].creater, (price/10000)*royality);
      _exchangeToken.transferFrom(msg.sender,owner, (price/10000)*(commission));
      ...
      IERC20 _exchangeToken = IERC20(0xf30A23E84E6e1DC121F19dEF24bB2804Ab9cd20C);
      require(check_auction_completed(itemId) == true , "still auction not completed");
      require(_exchangeToken.allowance(msg.sender, address(this)) >= price , "Token allowance too low");
  ...
  function fetchallnfts() public view returns(MarketToken[] memory) {
    ...
    function fetchallnftsinsale() public view returns(MarketToken[] memory) {
      uint totalItemCount =n;
      uint itemCount = 0;
      uint currentIndex = 0;
  ...
  function check_for_ownership(uint256 itemId) public returns(bool){
       require(markettokens[itemId].owner != payable(0x00));
