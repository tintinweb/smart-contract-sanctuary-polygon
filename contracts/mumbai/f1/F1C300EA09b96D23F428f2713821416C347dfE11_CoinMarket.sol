...
contract CoinMarket is Pausable {
    ...
    event CoinAddressesChanged(uint256 indexed coinType, address indexed _from, address indexed _to, uint256 _time);
    /// @dev This event should be fired whenever an order is set.
    // event OrderDone(uint256 indexed id, uint256 indexed coinType, address indexed launcher, uint256 amount, uint256 price, uint256 unit, bool orderType);
    event OrderDone(uint256 indexed id);
    ...
    /// @dev Mapping from id to amount of left target.
    mapping(uint256 => uint256) idToLeft;
        ...
        )
    {
        ...
        uint256[] memory coinTypes = new uint256[](l);
        uint256[] memory lefts = new uint256[](l);
        uint256[] memory units = new uint256[](l);
        uint256[] memory prices = new uint256[](l);
        address[] memory launchers = new address[](l);
        bool[] memory labels = new bool[](l);
        for (uint256 i = 0; i < l; i++) {
            Order memory _Order = Orders[i];
            ids[i] = _Order.id;
            coinTypes[i] = _Order.coinType;
            lefts[i] = idToLeft[_Order.id];
            units[i] = _Order.unit;
            prices[i] = _Order.price;
            launchers[i] = _Order.launcher;
            labels[i] = _Order.label;
        }
        return (ids, coinTypes, lefts, units, prices, launchers, labels);
    }

    /// @dev Change coinAddress.
    ///  Caller should always be superAdmin. addr is new address of coinType.
    function changeCoinAddresses(uint256 coinType, address addr) external onlySuperAdmin {
        emit CoinAddressesChanged(coinType, coinAddresses[coinType], addr, block.timestamp);
    ...
    function setSell(
        ...
        require(amount >= unit && unit > 0 && amount % unit == 0, WRONG_PARAMETER);
        address addr = coinAddresses[coinType];
        require(addr != address(0), ADDRESS_NOT_SET);
        idToLeft[orderCount] = amount;
        idToIndex[orderCount] = Orders.length;
        idExist[orderCount] = true;
        Order memory _Order = Order({id: orderCount, coinType: coinType, unit: unit, price: price, launcher: msg.sender, label: true});
        Orders.push(_Order);
        emit OrderDone(orderCount);
        orderCount++;
        IERC20 sc = IERC20(addr);
        sc.transferFrom(msg.sender, address(this), amount);
    }

    function bidSell(uint256 id, uint256 amount) external payable whenNotPaused {
    ...
    function setBuy(
        ...
        idToIndex[orderCount] = Orders.length;
        idExist[orderCount] = true;
        Order memory _Order = Order({id: orderCount, coinType: coinType, unit: unit, price: price, launcher: msg.sender, label: false});
        Orders.push(_Order);
        emit OrderDone(orderCount);
        // orderCount++;
        // if (msg.value > chargeAmount) {
    ...
    function bidBuy(uint256 id, uint256 amount) external whenNotPaused {
        require(idExist[id], ID_NOT_EXISTS);
        uint256 index = idToIndex[id];
        Order memory _Order = Orders[index];
        require(amount % _Order.unit == 0, WRONG_PARAMETER);
    ...
    function cancelSell(uint256 id) external whenNotPaused {
        require(idExist[id], ID_NOT_EXISTS);
    ...
    /// @dev Launcher cancel order.
    ///  Since sc is set by superAdmin, we don't have to worry about re-entrancy attack here.
    function cancelSellAdmin(uint256 id) external onlyAdmin {
        require(idExist[id], ID_NOT_EXISTS);
        removeOrder(id);
    }
    ...
    function removeOrder(uint256 id) internal {
        ...
        Order memory _Order = Orders[index];
        removeInternalState(id);
        uint256 amount = idToLeft[id];
        delete idToLeft[id];
        if (_Order.label) {
            IERC20 sc = IERC20(coinAddresses[_Order.coinType]);
            sc.transfer(_Order.launcher, amount);
        } else {
            payable(_Order.launcher).transfer((amount * 10525) / 10000);
        }
    }
}
