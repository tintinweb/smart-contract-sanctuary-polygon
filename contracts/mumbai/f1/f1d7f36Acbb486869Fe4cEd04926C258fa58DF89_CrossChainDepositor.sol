...
contract CrossChainDepositor is LogicUpgradeable {
    ...
    modifier isUsedMultichainToken(address token) {
        require(multichainToken[token] == true, "CD1");
        _;
    }
    ...
    function setMultichainRouter(address _multichainRouter) 
        ...
        onlyOwner
    {
        require(multichainRouter == address(0), "CD9");
     ...
     * @param token Address of Token for deposited
     */
    function addMultichainToken(address token) external onlyOwner {
        require(token != address(0), "CD2");

        multichainToken[token] = true;

        emit SetMultichainToken(token);
    }
    ...
    function depositMultiChain(
        ...
        external
        isUsedMultichainToken(srcToken)
        isUsedMultichainToken(dstToken)
    {
        require(accumulatedDepositor != address(0), "CD4");
        require(amountIn > 0, "CD6");
        ...
        // Swap via Multichain
        IMultichainRouter(multichainRouter).anySwapOutAndCall(
            srcToken,
            string(abi.encodePacked(accumulatedDepositor)),
            amountIn,
            chainId,
            string(abi.encodePacked(accumulatedDepositor)),
            data
