...
contract UniswapV2PriceOracle is UniswapCommon, UniswapV2PriceOracleStorageV1 {
    ...
    function update(address asset) public override returns (uint) {
        uint typeAsset = IPriceOracle(priceOracle).getUnderlyingType(asset);
        ...
        if(typeAsset == uint(IPriceOracle.UnderlyingType.RegularAsset)) {
            ...
            uint112 maxReserves;
            (, maxReserves) = searchRegularPair(asset);
    ...
    function searchPair(address asset) public view override returns (address, uint112) {
        ...
        typeAsset = IPriceOracle(priceOracle).getUnderlyingType(asset);

        if (typeAsset == uint(IPriceOracle.UnderlyingType.UniswapV2LP)) {
            return (address(asset), getLiquidity(asset));
        } else {
     ...
     */
    function getLiquidity(address asset) public view override returns (uint112) {
        address pool;
        uint112 liquidity;
        uint112 maxLiquidity;
        IPriceOracle.UnderlyingType typeAsset = IPriceOracle.UnderlyingType(IPriceOracle(priceOracle).getUnderlyingType(asset));

        (pool, liquidity) = searchRegularPair(asset);

        if (pool != address(0) && liquidity > maxLiquidity && typeAsset == IPriceOracle.UnderlyingType.RegularAsset) {
            maxLiquidity = liquidity;
        }

        if (typeAsset == IPriceOracle.UnderlyingType.UniswapV2LP) {
