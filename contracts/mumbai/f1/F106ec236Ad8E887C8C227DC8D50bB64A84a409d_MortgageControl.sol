...
// new added
    ...
    function getDebtInfo(address _user, uint256 _mortgageId) public view returns(uint256,uint256,uint64,uint256){
        LocalVars memory vars;
        return (
        vars._loan = usermortgage[_user][_mortgageId].loan,
        vars._period = usermortgage[_user][_mortgageId].period,
        vars._interestrate= usermortgage[_user][_mortgageId].interestrate,
   ...
   function addNormalMorgateInterestData(
    ...
    function getMortgagesForWallet(address _wallet, address _collection) public view returns (uint256[] memory _idMortgagesForCollection){
        ...
        uint24 counter = 0;
        for (uint24 i = 1; i <= mortgageId; i++) {
            if (idInfo[i] == _wallet) {
                // sacamos los mortgages relacionados a la coleccion que intenta cobrar
                (address collection, , , , , , , , , , , , ) = getUserInfo(_wallet,i);
                if (_collection == collection) {
                    _idMortgagesForCollection[counter] = i;
                    counter++;
                }
            }
        }
