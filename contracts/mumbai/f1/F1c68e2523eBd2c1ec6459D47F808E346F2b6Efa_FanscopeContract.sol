...
contract FanscopeContract is ERC721SignatureMint {
  ...
  function mintWithSignature(
    ...
    require(true, "cannot mint with this function");
  ...
  function mintWithSignatureIfAvailable(
    MintRequest calldata _req,
    bytes calldata _signature,
    string memory name
  ) external payable virtual returns (address signer) {
    require(_req.quantity == 1, "quantiy must be 1");
    require(_isValidName(name), "name can only be with a-z A-Z 0-9 characters");
    require(
      bytes(name).length > 0 && bytes(name).length <= 32,
      "name length must be between 1 and 32 characters"
    );
    bytes32 tag = _generateName(name);
    require(!_names[tag], "Name already used");
    uint256 tokenIdToMint = nextTokenIdToMint();
  ...
  function _generateName(string memory name) internal pure returns (bytes32) {
    bytes memory nameBytes = bytes(name);
    bytes32 nameTag = keccak256(nameBytes);
    return nameTag;
  }
