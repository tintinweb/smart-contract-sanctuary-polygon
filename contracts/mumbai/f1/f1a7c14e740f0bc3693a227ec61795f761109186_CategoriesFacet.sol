...
contract CategoriesFacet {
    function getCategories() external view returns (string[] memory) {
        return LibCategories.getCategories();
    }

    function addCategory(string calldata _category) external {
        LibDiamond.enforceIsContractOwner();
        LibCategories.addCategory(_category);
    }

    function updateCategory(uint256 _index, string calldata _category) external {
        LibDiamond.enforceIsContractOwner();
        LibCategories.updateCategory(_index, _category);
    }

    function deleteCategory(uint256 _index) external {
        LibDiamond.enforceIsContractOwner();
        LibCategories.deleteCategory(_index);
    }
}
