...
contract OyesMatic {
    ...
    event NewUserPlace(address indexed user,uint indexed userId, address indexed referrer,uint referrerId, uint8 matrix, uint8 level, uint8 place,uint256 time);
    ...
    function updateO3Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].O3Matrix[level].referrals.length < 3) {
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress, users[referrerAddress].id, 1, level, uint8(users[referrerAddress].O3Matrix[level].referrals.length),block.timestamp);
            return sendMaticDividends(referrerAddress, userAddress, 1, level);
        }
        
        emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id, 1, level, 3,block.timestamp);
        //close matrix
    ...
    function updateO6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length < 2) {
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id, 2, level, uint8(users[referrerAddress].O6Matrix[level].firstLevelReferrals.length),block.timestamp);
            ...
            if ((len == 2) && 
                ...
                if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id, 2, level, 5,block.timestamp);
                } else {
                    emit NewUserPlace(userAddress,users[userAddress].id,ref,users[ref].id, 2, level, 6,block.timestamp);
                }
            }  else if ((len == 1 || len == 2) &&
                    users[ref].O6Matrix[level].firstLevelReferrals[0] == referrerAddress) {
                if (users[referrerAddress].O6Matrix[level].firstLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id, 2, level, 3,block.timestamp);
                } else {
                    emit NewUserPlace(userAddress,users[userAddress].id, ref,users[ref].id, 2, level, 4,block.timestamp);
                }
    ...
    function updateO6(address userAddress, address referrerAddress, uint8 level, bool x2) private {
        if (!x2) {
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id, 2, level, 2 + uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[0]].O6Matrix[level].firstLevelReferrals.length),block.timestamp);
            //set current level
            ...
            emit NewUserPlace(userAddress,users[userAddress].id, referrerAddress,users[referrerAddress].id, 2, level, 4 + uint8(users[users[referrerAddress].O6Matrix[level].firstLevelReferrals[1]].O6Matrix[level].firstLevelReferrals.length),block.timestamp);
            //set current level
