...
contract AlligatorMerge is Ownable, VRFConsumerBaseV2, IMergeGators {
    ...
    function merge(uint256 _1st, uint256 _2nd, uint256 _3rd) public payable {
        ...
        if (IAlligators(alligatorsAddress).ownerOf(_1st) == msg.sender) revert Mergefailed();
        if (IAlligators(alligatorsAddress).ownerOf(_2nd) == msg.sender) revert Mergefailed();
        if (IAlligators(alligatorsAddress).ownerOf(_3rd) == msg.sender) revert Mergefailed();
        ...
        IAlligators(alligatorsAddress).merge(msg.sender);
        IAlligators(alligatorsAddress).burn(_1st);
        IAlligators(alligatorsAddress).burn(_2nd);
        IAlligators(alligatorsAddress).burn(_3rd);
