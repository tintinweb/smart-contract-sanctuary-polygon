...
// GOAL:
// only signature that comes from database should be used
...
contract Astronauts is ERC721 ,EIP712, IERC721Receiver,ERC721Holder{
    ...
    constructor(address mooseSociety, address alphaHeard) ERC721("Astronauts", "ASTRO") EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION){
    ...
    function calimInvestorNFT(uint256 uniqueID, uint _MaxtimeStamp, uint256 claimAmount, address _address, bytes memory signature) public payable{
        ...
        uint csf = claimedSoFar[msg.sender];
        (bool success, ) = (msg.sender).call{value: (claimAmount-csf)}("");
