...
contract PoolToken is Ownable, Pausable, ReentrancyGuard {
    ...
    function userInfo(address _user) public view returns(uint256, uint256, uint256){
    ...
    function termInfo(address _user, uint256 termID) public view returns(uint256, uint256, uint256) {
        return (
                _userInfo[_user].listOfTerms[termID].dailyReceived,
                _userInfo[_user].listOfTerms[termID].duration,
                _userInfo[_user].listOfTerms[termID].dayRemained
            );
    }

    function addTerm(address customer, uint256 ID, uint256 dailyReceived, uint256 duration) external onlyAdmins {
        uint256 dayRemained = dailyReceived;
        _userInfo[customer].listOfTerms.push(Term(ID, dailyReceived, duration, dayRemained));
    }
    
    function addUserInfo(address customer, uint256 maxTokenReceived) external onlyAdmins {
    ...
    function daysHavePassed(uint timeStartClaim) private view returns(uint256) {
        ...
        return (timeNow - timeStartClaim) / (PeriodTimeReceived * 86400);
    }
    ...
    function claimToken() external nonReentrant whenNotPaused  {
        ...
        Term[] storage _term = _userInfo[recepient].listOfTerms;
        uint256 totalToken = 0;

        for (uint256 i = 0; i < _term.length; i++) {
            if (daysPassed > 0 ){
                if (_term[i].duration - daysPassed <= 0 ) {
                    daysPassed -= _term[i].duration;
                    totalToken += _term[i].dailyReceived * _term[i].dayRemained;
                    _term[i].dayRemained = 0;
                } else {
                    totalToken += _term[i].dailyReceived*daysPassed;
                    _term[i].dayRemained = _term[i].duration - daysPassed;
                    daysPassed = 0;
                }
            }
        }
        ...
        require(IERC20(PARTHLABS).transfer(recepient, totalToken));
        emit ClaimedToken(recepient, totalToken);
    }

    function transferAnyBEP20Token(address _tokenAddress, uint256 _amount) external nonReentrant whenNotPaused onlyOwner {
