...
contract MapManager is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function stakeBeast(uint16 beastId) external {
        ...
        _beastStates[beastId] = BeastState({
            representation: IBeast(beastAddress).getBeastRepresentation(
                beastId
            ),
            groupId: 0,
            beastId: beastId
    ...
    function createPublicInstance(uint16 mapId)
        external
    ...
    function revokeInvite(uint240 groupId, address[] calldata revokes)
        external
    {
        Group storage group = _groups[groupId];
        require(group.leader == msg.sender, "only leader");
    ...
    function joinGroup(uint240 groupId, uint16 beastId)
        external
    ...
    function claimReward(uint16 beastId) external onlyBeastOwner(beastId) {
        ...
        beastState.representation = IBeast(beastAddress).giveExperience(
            beastId,
            map.expReward
        );
        IRagnarokERC20(skullAddress).mint(msg.sender, map.skullReward);
    ...
    function _getBeastStates(uint256[] memory beastIds)
        internal
        view
        returns (BeastState[] memory)
    {
    ...
    function getGroup(uint240 groupId)
        external
        view
        returns (PublicGroup memory)
    {
    ...
    function _isAnyBeastFromPlayer(uint256[] memory beastIds, address player)
        internal
        view
        returns (bool)
    {
    ...
    modifier onlyBeastOwner(uint16 beastId) {
        require(
            _playerBeasts[msg.sender].contains(beastId),
