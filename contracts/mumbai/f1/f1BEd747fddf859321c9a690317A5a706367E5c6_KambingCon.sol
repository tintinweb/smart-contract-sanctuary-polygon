...
contract KambingCon is ERC721Enumerable, Ownable{
  ...
  string public constant INITIAL_PROVENANCE="5bac7b3d7ac337a0251721ac013ade53";
  string public constant FINAL_PROVENANCE="56051b3f7fa79c30ace07df0d4de0103";

  uint256 public openingTime;
  uint256 public closingTime;

  uint public constant MAX_SUPPLY = 9776;
  uint private NFT_PRICE = 1;

  uint private saleState = 0;

  constructor(string memory baseURI) ERC721("MEEEH Meeeh", "MEEH") {
    Owner = msg.sender;
  ...
  function getNFTPrice() public view returns (uint) {
    ...
    uint usdToMatic =  uint(1).mul(10 ** 16).div(latestPrice);
    uint nftPrice = usdToMatic.mul(NFT_PRICE).mul(10 ** 10);
    return nftPrice;
  }
  ...
  function flipState(uint _state) public onlyOwner {
    saleState = _state;
  }
  ...
  function saleMaxSupply() public view returns (uint) {
    if(saleState == 1) {
      return 888;
    }
  ...
  function giveBirth(uint _count) public payable {
    uint totalMinted = _tokenIdCounter.current();
    uint PRICE = getNFTPrice();
    require(totalMinted.add(_count) <= saleMaxSupply(), "Mintable NFTs Exhausted");
