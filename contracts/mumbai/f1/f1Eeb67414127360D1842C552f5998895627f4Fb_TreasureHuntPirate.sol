...
contract TreasureHuntPirate is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function giveGift(address receiver, bool isDirect) public {
        require(mysteryChestAmt[msg.sender] > 0, "You have no chest!");
        
        if (isDirect) {
            ITreasureHunt(rewardPool).transferCost(giftFee, false);
        } else {
            ITreasureHunt(rewardPool).payUsingReward(msg.sender, giftFee);
        }
        mysteryChestAmt[msg.sender] --;
        mysteryChestAmt[receiver] ++;
    }
...
interface ITreasureHunt is ITreasureHuntTrait {
    ...
    //picker
    function randomPirate(uint256 tokenID, string memory) external returns(bool);
