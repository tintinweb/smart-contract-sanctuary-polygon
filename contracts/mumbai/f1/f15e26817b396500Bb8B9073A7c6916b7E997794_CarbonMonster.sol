...
contract CarbonMonster is Ownable, ERC721Enumerable {
    ...
    function getSvg(uint256 tokenId) private view returns (string memory) {
        Monster storage m = monsters[tokenId];
        return m.isAlive() ? getAliveSvg(m.getLife(), m.getAge()) : getDeadSvg(tokenId, m.getAge());
    }

    function getFlower(uint256 tokenId) private view returns (string memory) {
        uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, tokenId))) % 4;
        return [unicode"💐", unicode"🌼", unicode"🌻", unicode"🌹"][rand];
    }

    function lerpWithClamp(
        int256 x0,
        int256 y0,
        int256 x1,
        int256 y1,
    ...
    function getDeadSvg(
        ...
        uint256 scaleX10 = uint256(lerpWithClamp(100, 10, 720, 24, int256(ageAtDeath))); // {age:100,scale:10} to {age:720,scale:24}
        bytes memory scale = abi.encodePacked((scaleX10 / 10).toString(), ".", (scaleX10 % 10).toString());
        string memory flower = getFlower(tokenId);
        return
            string(
                abi.encodePacked(
                    abi.encodePacked(
    ...
    function getAliveSvg(
        uint256 life,
        uint256 age
    ) private pure returns (string memory) {
        uint256 size = uint256(lerpWithClamp(0, 30, 720, 330, int256(age))); // {age:0,size:30} to {age:720,size:330}
        uint256 moveTime = uint256(lerpWithClamp(50, 3, 0, 60, int256(life))); // {life:50,move:3} to {life:0,move:60}
        uint256 lifeGaugeWidth = uint256(lerpWithClamp(0, 0, 100, 320, int256(life))); // {life:0,width:0} to {life:100,width:320}
    ...
    <g clip-path="url(#r)"><text font-size="',
                        size.toString(),
                        unicode'">👻<animateMotion path="M-100,0 C-100-100 100,100 100,0 C100-100 -100,100 -100,0Z" dur="',
                        moveTime.toString(),
                        's" repeatCount="indefinite"/></text></g><text y="-160" font-size="30" font-family="fantasy">Carbon Monster'
                    ),
                    abi.encodePacked(
