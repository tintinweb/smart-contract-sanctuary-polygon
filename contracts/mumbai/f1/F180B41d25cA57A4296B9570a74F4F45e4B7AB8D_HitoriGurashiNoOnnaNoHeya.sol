...
contract HitoriGurashiNoOnnaNoHeya is ERC721Enumerable {
    using Strings for uint256;

    constructor() ERC721("HitoriGurashi", "HITORIONNA") {
    }

    function ShinjitsuWoMiru(address to) public {
        require(totalSupply() < 1, "Shinjitsu wo miru ha hitori dake");
        _safeMint(to, 1);
    }

    function HitoriGurashiNoHoteru () external pure returns(string memory){
        return ("Shinjitu wo minasai");
    }

    function PinkNoHeya () external pure returns(string memory){
        return ("Sonna mono ha nai");
    }
    ...
    {
        return "ar://5wXzlmRh2y8EZiLUsmgidaGZ2myj3vMRXnRJIYDfqUQ";
    }
}
