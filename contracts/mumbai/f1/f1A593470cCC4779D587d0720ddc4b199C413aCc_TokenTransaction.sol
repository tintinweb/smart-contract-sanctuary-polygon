...
contract TokenTransaction is Pausable, Ownable {
    ...
    function setDividePercentage(uint256 _dividePercentage) public onlyOwner {
    ...
    function tokenPay(
        ...
        require(
            hashTokenPay(msg.sender, amount, tokenAddress, orderId, toAddress, blockHeight, nonce) == hash,
            "Invalid hash!"
        );
        ...
        if (toAddress != address(0)) {
            divideAmount = amount.mul(dividePercentage).div(100);
        ...
        emit TokenPay(msg.sender, toAddress, amount, tokenAddress, orderId, divideAmount, nonce);
    }
