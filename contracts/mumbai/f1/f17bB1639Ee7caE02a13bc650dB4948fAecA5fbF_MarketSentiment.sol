...
contract MarketSentiment {
    ...
    function addTicker(string memory _ticker) public {
        require(msg.sender == owner, "Only the owner can add new tickers");
        ticker storage newTicker = Tickers[_ticker];
        newTicker.exists = true;
        tickersArray.push(_ticker); // is this right? push(newTicker)?
    }
    ...
    function getVotes(string memory _ticker) public view returns (
        ...
        require(Tickers[_ticker].exists, "This ticker does not exist");
        ticker storage t = Tickers[_ticker];
        return(t.up, t.down);
    }
}