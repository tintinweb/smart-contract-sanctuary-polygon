...
library VaultLifecycleTreasury {
        ...
        )
    {
        ...
        require(premium > 0, "Premium should be greater than zero");
    ...
    function verifyOtoken(
        ...
        require(
            ...
            "otoken should not be a zero address"
        );
        ...
        require(otoken.strikeAsset() == USDC, "strikeAsset should be in USDC");
    ...
    function settleShort(address gammaController) external returns (uint256) {
        ...
        require(vault.shortOtokens.length > 0, "No shortOtokens");
    ...
    function verifyInitializerParams(
        ...
        require(
            _initParams._owner != address(0),
            "Owner should not be a zero address"
        );
        require(
            _initParams._keeper != address(0),
            "Keeper should not be a zero address"
        );
        require(
            _initParams._feeRecipient != address(0),
        ...
        require(
            bytes(_initParams._tokenName).length > 0,
            "Token name cannot be empty"
        );
        require(
            bytes(_initParams._tokenSymbol).length > 0,
        ...
        require(
            ...
            "Period option is not available"
        );
        require(
            _initParams._optionsPremiumPricer != address(0),
            "optionsPremiumPricer should not be a zero address"
        );
        require(
            _initParams._strikeSelection != address(0),
            "strikeSelection should not be a zero address"
        );
        require(
            _initParams._premiumDiscount > 0 &&
                _initParams._premiumDiscount <
                100 * Vault.PREMIUM_DISCOUNT_MULTIPLIER,
            "premiumDiscount should be greater than zero and less than 100%"
        );
        require(
            _initParams._auctionDuration >= _min_auction_duration,
            "!auctionDuration should be greater than the min_auction duration"
        );
        require(
            _initParams._minDeposit > 0,
            "minDeposit should be greater than zero"
        );

        require(
            _vaultParams.asset != address(0),
            "asset should not be a zero address"
        );
        require(
            _vaultParams.underlying != address(0),
            "underlying  should not be a zero address"
        );
        require(
...
library DateTime {
    ...
    function _daysFromDate(
        ...
        require(
            year >= 1970,
            "Year must be after 1970, please verify the year"
        );
        ...
        returns (uint256)
    {
        ...
        uint256 quarterMonth = (month <= 3) ? 3 : (month <= 6)
            ? 6
            : (month <= 9)
            ? 9
            : 12;

        uint256 quarterDate = timestampFromDate(
            getYear(timestamp),
            quarterMonth,
            1
        );
        ...
        returns (uint256)
    {
        ...
        uint256 biannualDate = timestampFromDate(
            getYear(timestamp),
            biannualMonth,
            1
        );
