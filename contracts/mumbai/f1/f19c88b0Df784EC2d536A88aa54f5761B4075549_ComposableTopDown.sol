...
interface IERC998ERC721TopDown {
    ...
    function rootOwnerOf(uint256 _tokenId) external view returns (bytes20 rootOwner);
    function rootOwnerOfChild(address _childContract, uint256 _childTokenId) external view returns (bytes20 rootOwner);
...
contract ComposableTopDown is ERC721, IERC998ERC721TopDown, IERC998ERC721TopDownEnumerable,IERC998ERC20TopDown, IERC998ERC20TopDownEnumerable {
    ...
    function createBundle(address[] memory _childContract, uint256[][] memory _childTokenId) external returns(uint256 _tokenId) {
        uint256 tokenLength = _childTokenId.length;
        uint256 contractLength = _childContract.length;
        // require(tokenLength > 1, "Min 2 NFTs required");
        require(tokenLength == contractLength, "Invalid length");
        address _from = msg.sender;
        _tokenId = mint(_from);
        
        for (uint i=0; i<tokenLength; i++) {
            for (uint j=0; j<_childTokenId[i].length; j++) {
                receiveChild(_from, _tokenId, _childContract[i], _childTokenId[i][j]);
                ERC721(_childContract[i]).transferFrom(_from, address(this), _childTokenId[i][j]);
            }
        }
    ...
    function unbundle(uint256 _tokenId, address _to) public {
        // require(msg.sender == address(rootOwnerOf(_tokenId)), "Not Owner");
        address rootOwner = address(rootOwnerOf(_tokenId));
        require(rootOwner == msg.sender || tokenOwnerToOperators[rootOwner][msg.sender] ||
        rootOwnerAndTokenIdToApprovedAddress[rootOwner][_tokenId] == msg.sender, "Invalid caller");

        // address _to = msg.sender;
        uint numOfContracts = childContracts[_tokenId].length;
        require(numOfContracts > 0, "Not bundled");
        uint i = numOfContracts - 1;
        uint j = 0;
        uint numOfTokens;
        uint _childTokenId;
        ...
        while (i >= 0) {
            ...
            while (j >= 0) {
                ...
                emit TransferChild(_tokenId, _to, _childContract, _childTokenId);
                if (j == 0) break;
                else j -= 1;
            }
            if (i == 0) break;
    ...
    function unbundleAndBurn(uint256 _tokenId, address _to) external {
        uint numOfContracts = childContracts[_tokenId].length;
        if (numOfContracts > 0) {
            unbundle(_tokenId, _to);
        }
        _transferFromInternal(msg.sender, address(0), _tokenId);
    }
    ...
    function rootOwnerOf(uint256 _tokenId) public view returns (bytes20 rootOwner) {
    ...
    function rootOwnerOfChild(address _childContract, uint256 _childTokenId) public view returns (bytes20 rootOwner) {
    ...
    function bapprove(address _approved, uint256 _tokenId) public {
        address rootOwner = address(rootOwnerOf(_tokenId));
        require(rootOwner == msg.sender || tokenOwnerToOperators[rootOwner][msg.sender]);
        rootOwnerAndTokenIdToApprovedAddress[rootOwner][_tokenId] = _approved;
        emit Approval(rootOwner, _approved, _tokenId);
    ...
    function _transferFrom(address _from, address _to, uint256 _tokenId) private {
        ...
        _transferFromInternal(_from, _to, _tokenId);
    }

    function _transferFromInternal(address _from, address _to, uint256 _tokenId) internal {
    ...
    //     IERC998ERC721BottomUp(_childContract).transferToParent(address(address(this)), _toContract, _toTokenId, _childTokenId, _data);
    //     emit TransferChild(_fromTokenId, _toContract, _childContract, _childTokenId);
    // }


    // address(this) contract has to be approved first in _childContract
    // TODO: modifier
    function getChild(address _from, uint256 _tokenId, address _childContract, uint256 _childTokenId) external {
    ...
    function childContractByIndex(uint256 _tokenId, uint256 _index) external view returns (address childContract) {
        require(_index < childContracts[_tokenId].length, "Contract address does not exist for address(this) token and index.");
    ...
    // address(this) contract has to be approved first by _erc20Contract
    ...
    function erc20ContractByIndex(uint256 _tokenId, uint256 _index) external view returns (address) {
        require(_index < erc20Contracts[_tokenId].length, "Contract address does not exist for address(this) token and index.");
