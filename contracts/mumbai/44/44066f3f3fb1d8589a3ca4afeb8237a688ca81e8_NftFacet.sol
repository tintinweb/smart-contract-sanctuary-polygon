...
contract NftFacet is ERC721AUpgradeable {
   error ProjectNFT__isAddress0();
    error ProjectNFT__isAlreadyOwner();
    error ProjectNFT__isNotOwner();
    error ProjectNFT__tokenIdNotExists();
    error ProjectNFT__notCalledFromDiamond();
    error ProjectNFT__alreadyInitialized();

    event ProjectNFTInitialized();
    ...
    modifier onlyOwnerCan() {
		if(LibDiamond.contractOwner() != msg.sender) revert ProjectNFT__isNotOwner();
		_;
	}
    modifier callingFromDiamond() {
        if(address(this) != s()._diamondAddress) revert ProjectNFT__notCalledFromDiamond();
        _;
    }
    modifier idDoExists(uint256 id) {
        if(_ownerOf(id) == address(0)) revert ProjectNFT__tokenIdNotExists();
        _;
    }
    ...
    function init() external onlyOwnerCan {
        if (s()._isInitialized == true) revert ProjectNFT__alreadyInitialized();
        
        s()._name = "Metadev3 Le Club - Project NFT";
        s()._symbol = "M3CB";
        s()._currentIndex = 0;
        s()._burnCounter = 0;
