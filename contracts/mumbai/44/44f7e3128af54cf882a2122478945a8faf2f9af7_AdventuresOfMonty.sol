...
contract AdventuresOfMonty is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    uint256 public publicMintPriceERC20;
    uint256 public publicMintPriceETH;
    IERC20 public crwAddress;
    string private baseURI;
    string private notRevealedURI = "ipfs://Qme9wcxvuwau5g3qVsUHaAtpSkPZPhYb2VqRC1SoujxQCM";

    event CrownRegistration(address indexed emisor, uint256 indexed tokenID, string uri, string crwAddress);

    constructor(address erc20Address) ERC721("Adventures of Monty", "Monty") {
        publicMintPriceERC20 = 10000000000000000;
        publicMintPriceETH = 10000000000000000;
        crwAddress = IERC20(erc20Address);
    }
    ...
    function revealCollection(string memory newBaseUri) public onlyOwner {
    ...
    function mintForAllowedList(address to) public onlyOwner {
    ...
    function allowMint(address to) public onlyOwner {
        allowedList[to] = true;
    }

    function disallowMint(address to) public onlyOwner {
    ...
    function publicMintWithERC20(uint256 numNFTs) public {
        require(numNFTs > 0, "You must mint at least one NFT");
        require(crwAddress.balanceOf(msg.sender)  >= publicMintPriceERC20 * numNFTs , "Insufficient ERC20 token balance");
        require(crwAddress.allowance(msg.sender, address(this)) >= publicMintPriceERC20 * numNFTs, "ERC20 token allowance not enough");
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId + numNFTs <= MAX_SUPPLY, "Exceeds MAX_SUPPLY");
        require(mintedPerWallet[msg.sender] + numNFTs <= MAX_MINT, "Exceeds MAX_MINT");
        crwAddress.transferFrom(msg.sender, address(this), publicMintPriceERC20 * numNFTs);
        for (uint256 i = 0; i < numNFTs; i++) {
            tokenId = _tokenIdCounter.current();
             _tokenIdCounter.increment();
            _safeMint(msg.sender, tokenId);
