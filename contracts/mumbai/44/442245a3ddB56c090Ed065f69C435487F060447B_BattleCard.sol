...
/* Errors */
error Battle__UpkeepNotNeeded(
    uint256 currentBalance,
    uint256 players,
    uint256 battleStatus
);

/// @title BattleCard
/// @notice This contract handles the token management and battle logic for the BattleCard game
/// @notice Version 1.0.0
/// @dev This contract implements Chainlink VRF V2 & Chainlink Automation UpKeep
/// @author Dewangga Praxindo

contract BattleCard is
    ERC1155,
    Ownable,
    ERC1155Supply,
    VRFConsumerBaseV2,
    AutomationCompatible
{
    ...
    constructor(
        address vrfCoordinatorV2, // contract address or supported networks
    ...
    function getRandomValue(uint256, address) internal view returns (uint256) {
        return s_randomValue;
    }

    function getRandomValue2(uint256, address) internal view returns (uint256) {
        return s_randomValue2;
    }
    ...
    function _resolveBattle(Battle memory _battle) internal {
        ...
        gameTokens[playerTokenInfo[_battle.players[0]]]
            .defenseStrength = (MAX_ATTACK_DEFEND_STRENGTH -
            _randomAttackStrengthPlayer1);

        uint256 _randomAttackStrengthPlayer2 = getRandomValue2(
            MAX_ATTACK_DEFEND_STRENGTH,
            _battle.players[1]
        );
