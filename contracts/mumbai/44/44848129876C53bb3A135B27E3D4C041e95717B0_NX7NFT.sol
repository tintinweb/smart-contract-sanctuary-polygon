...
contract NX7NFT is ERC721Burnable, Ownable {
    ...
    modifier notLocked() {
        require(!locked, "NX7NFT: Metadata URIs are locked");
    ...
    function adminMint(address to) external onlyOwner {
        require(to != address(0), "NX7NFT: Address cannot be 0");
    ...
    function maticPrice() view public returns (int256) {
    ...
    function mint(address to, uint32 amount) external payable {
        uint256 received = msg.value;
        uint256 receivedInMatic = received / 10 ** 18;
        require(to != address(0), "NX7NFT: Address cannot be 0");

        require(
            receivedInMatic * amount * uint256(maticPrice()) == _mintPrice*(amount)*8,
            "NX7NFT: Matic sent is not the right amount"
        );
    ...
    function changeMintBeneficiary(address beneficiary) public onlyOwner {
        require(
            ...
            "NX7NFT: Minting beneficiary cannot be address 0"
        );

        require(
            beneficiary != _mintingBeneficiary,
            "NX7NFT: beneficiary cannot be the same as previous"
        );
    ...
    function changeMintCost(uint256 mintCost) public onlyOwner {
        require(
            mintCost != _mintPrice,
            "NX7NFT: mint cost cannot be same as previous"
        );
        _mintPrice = mintCost;
    }
        ...
        notLocked
    {
        require(
            ...
            "NX7NFT: Base URI cannot be same as previous"
        );
    ...
    function _forwardFunds(uint256 received) internal {
        ...
        require(success, "NX7NFT: Failed to forward funds");
    }
}
