...
library MerchantLibrary {
    ...
    /**
     * @dev Predict swap amount in / out of router
     * @notice prediction is done from the following assumption
     * - payingToken_ is different from receiveToken_
     * - when payingToken_ is zero address, receiveToken_ is not WETH
    ...
    function predictAmount(
        ...
        uint256 knownAmount_,
        address[] memory path_,
        bool getInAmount
    ) public view returns (uint256) {
        if (address(swapRouter_) == address(0)) {
            return 0;
        }

        path_ = rebuildSwappablePath(
            payingToken_,
            receiveToken_,
            swapRouter_,
            path_
        ...
        if (getInAmount) {
            try swapRouter_.getAmountsIn(knownAmount_, path_) returns (
            ...
            try swapRouter_.getAmountsOut(knownAmount_, path_) returns (
                uint256[] memory amounts
            ) {
    ...
    /**
     * @dev Rebuild swappable path from the inputted params
     * @return swappable path array
     */
    function rebuildSwappablePath(
        address payingToken_,
        address receiveToken_,
        IUniswapV2Router02 swapRouter_,
        address[] memory path_
    ) public pure returns (address[] memory) {
        // For the native token, considered to WETH
        if (payingToken_ == address(0)) {
            payingToken_ = swapRouter_.WETH();
        }

        // swap path should be valid one
        bool isPathValid = path_.length >= 2 &&
            path_[0] == payingToken_ &&
            path_[path_.length - 1] == receiveToken_;

        if (!isPathValid) {
            if (
                payingToken_ == swapRouter_.WETH() ||
                receiveToken_ == swapRouter_.WETH()
            ) {
                path_ = new address[](2);
                path_[0] = payingToken_;
                path_[1] = receiveToken_;
            } else {
                path_ = new address[](3);
                path_[0] = payingToken_;
                path_[1] = swapRouter_.WETH();
                path_[2] = receiveToken_;
            }
        }
        return path_;
    }
    ...
    /**
     * @notice get best output amount from several routers
     */
    function predictBestOutAmount(
        address payingToken_,
        address receiveToken_,
        uint256 _amountIn,
        address[] memory swapRouters_,
        address[] memory path_
    )
        public
        view
        returns (
            uint256 bestAmount,
            uint256 bestRouterIndex,
            address[] memory swappablePath
        )
    {
        for (uint256 i = 0; i < swapRouters_.length; i++) {
            IUniswapV2Router02 swapRouter = IUniswapV2Router02(swapRouters_[i]);
        ...
        IUniswapV2Router02 bestSwapRouter = IUniswapV2Router02(
            swapRouters_[bestRouterIndex]
        );
        swappablePath = rebuildSwappablePath(
            payingToken_,
            receiveToken_,
            bestSwapRouter,
            path_
...
contract SlashCore is ISlashCore, Ownable {
    ...
    function doMerchantDeposit(
        ...
        // paid by native token & receive token is WETH
        if (payingToken_ == address(0) && receiveToken == swapRouter.WETH()) {
            // Wrap ETH to WETH
            IWETH(receiveToken).deposit{value: amountIn_}();
            assert(IWETH(receiveToken).transfer(merchantWallet, amountIn_));
            return;
        }
        ...
        (
            ...
            uint256 bestRouterIndex,
            address[] memory swappablePath
        ) = MerchantLibrary.predictBestOutAmount(
                payingToken_,
                receiveToken,
                amountIn_,
        ...
        if (bestAmount > 0) {
            ...
            if (payingToken_ == address(0)) {
                ...
                IBEP20(payingToken_).approve(
                    address(bestSwapRouter),
                    amountIn_
                );
                // make the swap
                bestSwapRouter
    ...
    function submitTransaction(
        ...
        if (payingToken_ != address(0)) {
            // zero addresss for the native token
        ...
        if (payingToken_ == address(0)) { // when paid with native token, value includes fee and amountIn_
            require(
                msg.value >= taxFeeAmount.add(donationFeeAmount).add(amountIn_),
                "Insufficient fee"
            );
        } else {
            require(
