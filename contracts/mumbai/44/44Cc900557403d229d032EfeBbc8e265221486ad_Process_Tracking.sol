...
contract Process{
    ...
    //Methods
    function addInput(uint _id, uint _quantity, address _origin) public protected
    {
    ...
    function addResource(uint _id, uint _quantity) public restricted
    {
        require(inputs[_id].quantity == 0, "This id is already used");
        inputs[_id] = Information({
            quantity: _quantity,
    ...
    function sendOutput(uint _id, uint _quantity, address _addr) public restricted{
        ...
        output.quantity -= _quantity;
        Process(_addr).addInput(_id, _quantity, address(this));
    }
}