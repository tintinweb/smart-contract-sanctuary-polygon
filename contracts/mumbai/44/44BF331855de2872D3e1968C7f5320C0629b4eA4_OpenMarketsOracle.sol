...
contract OpenMarketsOracle is OwnableUpgradeable, UUPSUpgradeable {
    /* TYPES */
    ...
    modifier onlyDataFeedOwner(uint256 oracleId) {
        if (msg.sender != dataFeeds[oracleId].owner) {
            revert OnlyDataFeedOwner();
        }
        _;
    }

    modifier onlyScribes(uint256 oracleId) {
        if (
            msg.sender != dataFeeds[oracleId].owner &&
            !dataFeeds[oracleId].scribes[msg.sender]
        ) {
            revert OnlyScribes();
        }
        _;
    ...
    ) external onlyDataFeedOwner(oracleId) {
        if (!dataFeeds[oracleId].scribes[_scribeToRemove]) {
            revert ScribeNotFound();
        }

        delete dataFeeds[oracleId].scribes[_scribeToRemove];
        emit ScribeRemoved(oracleId, _scribeToRemove);
    }
