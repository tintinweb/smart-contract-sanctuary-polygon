...
contract Multisig is  IMultisig {
    ...
    constructor(bytes memory _data){
        (name, description, owners, factoryAddress, numConfirmationsRequired) = abi.decode(_data, (string, string, address[], address, uint256));
    ...
    function addMember(address newMember, uint256 _txIndex) external virtual onlyOwner txExists(_txIndex) notExecuted(_txIndex){
        Transaction storage transaction = transactions[_txIndex];
        bytes memory _data = abi.encodeWithSignature("addMember(address)", newMember);
        require(keccak256(transaction.data) == keccak256(_data));
        require(
            transaction.numConfirmations >= numConfirmationsRequired,
            "cannot execute tx"
        );
    ...
    function removeMember(address member, uint256 _txIndex) external virtual onlyOwner txExists(_txIndex) notExecuted(_txIndex){
        Transaction storage transaction = transactions[_txIndex];
        bytes memory _data = abi.encodeWithSignature("removeMember(address)", member);
        require(keccak256(transaction.data) == keccak256(_data));
        require(
            transaction.numConfirmations >= numConfirmationsRequired,
            "cannot execute tx"
    ...
    function isMultisigOwner(address owner)public returns(bool){
...
contract multisigFactory  {
    ...
    // string private constant COMPUTATION_SCHEMA = "wasmCID text, inputCID text, startCMD text, CMD text, JobId text, creator text, bacalhauJobID text, result text";
    string private constant MULTISIG_OWNERS_SCHEMA =
        "multisigAddress text, ownerAddress text";

    constructor(){
        tablelandContract = TablelandDeployments.get();
        createStatements.push(SQLHelpers.toCreateFromSchema(MULTISIG_SCHEMA, MULTISIG_TABLE_PREFIX));
        createStatements.push(SQLHelpers.toCreateFromSchema(MULTISIG_OWNERS_SCHEMA, MULTISIG_OWNERS_TABLE_PREFIX));
    ...
    function createWallet(
        ...
        addMultisig(name,description,address(multisigWallet), owners, numConfirmationsRequired, _proposalTable, _confirmationTable);
    }

    function getMultisigInitBytes(string memory name, string memory description, address[] memory owners,address factory, uint256 number) public pure returns (bytes memory) {
        return abi.encode(name, description, owners, factory, number);
    }
    ...
    function addMultisig(
        ...
        for(uint256 i = 0; i < _owners.length; i++){
            insertMember(multisigAddress, _owners[i]);
        }
        mutate(
            tableIDs[0],
            SQLHelpers.toInsert(
    ...
    function addMember(
        address  member
            ...
            insertMember(msg.sender, member);
    }

    function removeMember(address  member) public{
        require(multisigs.contains(msg.sender));
        string memory filter = string.concat("multisigAddress=", SQLHelpers.quote(Strings.toHexString(msg.sender))," and ", "ownerAddress=",SQLHelpers.quote(Strings.toHexString(member)));
