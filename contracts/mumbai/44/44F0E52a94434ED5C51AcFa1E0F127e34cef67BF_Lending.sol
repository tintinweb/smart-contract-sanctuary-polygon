...
contract Lending is AccessControl, ReentrancyGuard, Calcs, ILending {
    ...
    event Request(address wallet, uint256 id, uint256 amount);
    event Withdraw(uint256 id,address wallet, uint256 amount, address token, uint256 _penalization);
    event Cancel(address wallet, uint256 id, address who);
    ...
    function setWithdraw(uint256 _amount, uint256 id) public NotPaused nonReentrant returns(uint256){
       ...
       (uint256 _deposit,) = LControl.getCalcInfo(msg.sender, id);
       if(_amount > _deposit){
    ...
    function completeWithdraw(uint256 idrequest, uint256 idLending) public NotPaused nonReentrant returns(uint256 _amount){
        ...
        if((block.timestamp - depositTime) < LControl.getDaysLeft(msg.sender, idLending)){
           penalization = (calcPenalization(/* idLending, */ _quantity) / 1e12);
           ...
           IERC20(token).transfer(panoram, _panoram);
           IERC20(token).transfer(lenderRewards, lenders);

           emit Withdraw(idrequest,msg.sender,_quantity, address(token),penalization);
           return total;
        }else {
            IERC20(token).transfer(msg.sender, _quantity);
    ...
    function cancelWithdraw(uint256 idRequest, address _user, uint256 idLending) public who(idLending) returns(bool _complete) {
        ...
        (address lendersVault,,) = tokenInfo.getVaultInfo(address(token));
        LControl.UpdateInteresAccumulated(msg.sender, idLending); //update rewards before cancel request
        ...
        emit Cancel(_user, idRequest, msg.sender);
    ...
    function reinvestment(uint256 id, address _wallet, uint256 _amount) public NotPaused nonReentrant{    
        ...
        token.transferFrom(_wallet, address(this), _amount);   
        IVaultLenders(lendersVault).deposit(_amount,address(token)); 

        (,uint256 depositTime,,,,) = LControl.getLenderInfo(_wallet, id);
        uint256 time = (block.timestamp - depositTime); //10 dias
        uint32 left = LControl.getDaysLeft(_wallet, id);
        /* uint256 total;
        left > time ? total = left - time : total = time - left;  
        console2.logString("Dias");
       ...
       if(time >= dayForUpdate && left == LControl.getDaysBeforePenalization()) {
            ...
            payPersecond = calcInterestForSecond(_amount, LControl.interesLenders());
    ...
    function updateUsageRatio(uint256 _usageRate) public onlyDev {
        usageRate = _usageRate;
    }  

    function updateExitDays(uint256 _exitDays) public onlyDev {
        exitDays = _exitDays;
    }

    function updateDayForUpdate(uint32 _dayForUpdate) public onlyDev {
        dayForUpdate = _dayForUpdate;
    }

    function permissions(address _lenders) public onlyDev validToken(address(token)) {
        token.approve(_lenders, MAX_UINT);
    }

    function isPaused(bool _status) public onlyDev {
        _paused = _status;
    }

    function updateLendcontrol(address _newLControl) public onlyDev {
        LControl = ILendingControl(_newLControl);
    }

    function updatePanoramWallet(address _panoram) public onlyDev {
        panoram = _panoram;
    }
}
...
contract Calcs is AccessControl {
    ...
    uint256 internal percentagePenalization = 150; // 1.5% de 10 mil // Cambiarlo a Lending Control
    ...
    modifier onlyDev(){
     ...
     /// @dev Function to calculate the user's interest accumulated.
    ...
    /// @dev Function to calculate the interest paid to the user for every second.
    /// @param _amountDeposit cantidad depositada por el usuario.
    /// @param _interes interes a pagar en base 10 mil.
    ...
    function distFees(uint256 _fee) internal view returns(uint256 panoram, uint256 lenders){
    ...
    function updatePercentagePenalization(uint256 _newPenalization) public onlyDev {
...
interface ILendingControl is ILending {
    ...
    function resetRewards(address _wallet, uint256 _idRequest) external;
