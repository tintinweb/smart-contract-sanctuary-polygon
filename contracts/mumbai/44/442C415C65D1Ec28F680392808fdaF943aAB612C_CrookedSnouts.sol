...
contract CrookedSnouts is VRFConsumerBaseV2, ERC721URIStorage {
  ...
  string[12] eyes = ["sunglasses", "angry", "silly", "helpless", "dead", "empty", "pirate", "shocked", "lady", "cyclop", "eyeballs", "love"]; // 12
  string[13] heads = ["bald", "king", "semibald", "hairy", "punk", "unicorn", "curly", "blond", "grass", "double tails", "horns", "rufous", "angel"]; // 13
  string[12] mouths = ["tongue", "nope", "surprised", "goofy", "scream", "golden tooth", "squeezy", "rotten", "pacman", "sewn up", "mouth ball", "kiss"]; // 12
  string[7] noses = ["snot", "pointy", "piggy", "lack", "greek", "worm", "ring"]; // 7
  ...
  function requestNumbers() public {

    // require(addressToRequestStruct[msg.sender].requestId == 0 || addressToRequestStruct[msg.sender].minted, "You have active request");
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomNumbers) internal override {
    ...
    requestIdToRandomNumbers[_requestId] = randomNumbers({
      ...
      face: (_randomNumbers[1] % 8) + 1,
      eyes: (_randomNumbers[2] % 11) + 1,
      head: (_randomNumbers[3] % 11) + 1,
      mouth: (_randomNumbers[4] % 11) + 1,
      nose:(_randomNumbers[5] % 7) + 1
    });
  ...
  function create() public {
    ...
    // require(addressToRequestStruct[msg.sender].requestId != 0 && !addressToRequestStruct[msg.sender].minted, "You already minted");
    // require(!addressToRequestStruct[msg.sender].minted, "You already minted");

    choosenLayers memory layers;
    ...
    // update userRequest that he already minted so user can request for number and mint more
    requestIdMinted[addressToRequestId[msg.sender]] = true;


    // emit CreatedNFT(counter, tokenURI);
  }  
  ...
  function formatTokenURI(string memory imageURI, string memory name, choosenLayers memory layers) internal pure returns(string memory tokenURI) {
  ...
  //   // if requestId == 0 --> you can request random numbers
  //   // if userRequest.minted == true --> you minted, you can request for next one
  //   requestStruct memory userRequest = addressToRequestStruct[msg.sender];
  //   require(userRequest.requestId == 0 || userRequest.minted, "You have active request");
  ...
  //   // if requestId != 0 --> you have active request to mint
  //   // if userRequest.minted == false --> you have not minted yet
  //   requestStruct memory userRequest = addressToRequestStruct[msg.sender];
  //   require(userRequest.requestId != 0 && !userRequest.minted, "You already minted");
