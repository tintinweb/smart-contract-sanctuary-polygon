...
contract FlowMi is VRFConsumerBaseV2 {
    ...
    function flowmiFollow(address _profileid) public payable {
        ...
        if (
            ...
            //iaWmatic.approve(address(iWETHGateway), _withdrawAmmount); // aprueba aMatic tmb,
            iaWmatic.approve(address(this), _withdrawAmmount); // aprueba aMatic tmb,
            iaWmatic.approve(address(POOL), _withdrawAmmount); // aprueba aMatic tmb,

            iWmatic.approve(address(this), _withdrawAmmount); // aprueba aMatic tmb,
            iWmatic.approve(address(POOL), _withdrawAmmount); // aprueba aMatic tmb,
            //imatic.approve(address(this), _withdrawAmmount); // aprueba aMatic tmb,
            //imatic.approve(address(POOL), _withdrawAmmount); // aprueba aMatic tmb,*/
            // Saca el wraped de la pool
            withdraw(wmaticTokenAddress, _withdrawAmmount);
            // deswrapea
            ...
            iWmatic.approve(address(iWmatic), fraction);

            iWmatic.deposit{value: fraction}();
            //deposit(wmaticTokenAddress, fraction);
    ...
    function withdraw(address tokenAddr, uint256 amount) internal {
        IERC20 token = IERC20(tokenAddr);
