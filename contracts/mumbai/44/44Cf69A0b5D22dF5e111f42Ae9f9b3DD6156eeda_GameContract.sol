...
{
    // Stores address of the Septim contract, to be called
    address _septimContractAddr;
    // Stores address of the Outfit contract, to be called
    ...
    {
        balance = Septim(_septimContractAddr).getBalance();
    }

    function addSeptim(uint256 amount) public 
    {
        Septim(_septimContractAddr).addToBalance(amount);
    }

    function removeSeptim(uint256 amount) public 
    {
        Septim(_septimContractAddr).removeFromBalance(amount);
    }

    // Price comes from Unity
    function buyOutfit(uint256 price, string memory metadataUrl) public
    {
        uint256 septimBalance = getSeptimBalance();

        // If we don't have enough Septim, we can't mint the outfit
        require(price > septimBalance, "Insufficient Septim Balance");

        Outfit(_outfitContractAddr).mint(metadataUrl);

        // We should check if outfit minting has succeed
        Septim(_septimContractAddr).removeFromBalance(price);
    }
}
...
contract Septim is ERC20 
{
    ...
    function addToBalance(uint256 amount) public 
    {
        ...
        console.log("Septim amount added: ", amount);
    }

    function removeFromBalance(uint256 amount) public 
    {
        // We could check if amount we want to remove is bigger than balance
        _burn(msg.sender, amount);
        console.log("Septim amount removed: ", amount);
    }
}
