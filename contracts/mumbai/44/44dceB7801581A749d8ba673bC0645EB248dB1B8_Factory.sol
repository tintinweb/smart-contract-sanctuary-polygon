...
contract Factory is Ownable {
     ...
     */
    function deployNewCollectionContract(string memory _name, string memory _symbol,address trustedForwarder) onlyOwner()
        external
        returns (address)
    {
        require(bytes(_name).length != 0,"Factory: Use valid _name");
        require(bytes(_symbol).length != 0,"Factory: Use valid _symbol");
        Collection collection = new Collection(_name, _symbol,trustedForwarder);
...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    function _setTokenRoyalty(
        ...
        require(feeNumerator <= maxRoyalty,"royality must be less than max limit");
        require(
...
contract Collection is ERC2771Context, ERC721URIStorage, ERC2981, Whitelist {
     ...
     */
    function resetTokenRoyalty(uint256 _tokenId) public onlyInitialOwner(_tokenId) {
     ...
     */
    function changeLimitOfRoyality(uint _limit) external onlyOwner(){
        require(
