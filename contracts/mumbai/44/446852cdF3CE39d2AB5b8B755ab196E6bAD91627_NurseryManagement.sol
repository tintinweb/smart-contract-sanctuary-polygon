...
contract NurseryManagement is Nursery {

    constructor(IERC20 _BZAI, uint256 _preMint,address _NFTreserve,string memory _URI) Nursery(_preMint,_NFTreserve,_URI)  {
        BZAI = _BZAI;
    }
 
    mapping(uint256 => uint256) public nurseryRevenues;

    uint256[4] public maturities = [1 days,3 days,7 days,10 days];
    ...
    mapping(uint256 => mapping(uint256 => ZaiStruct.MintedData)) public offeredEggReference;


    function setMaturityDurations(uint256 _index, uint256 _numberOfDays) external onlyOwner{
    ...
    function _canMint(uint256 _tokenId) internal view returns (bool) {
        bool result;
        if (
            IOpenAndClose(gameAddresses.getOpenAndCloseAddress()).canNurserySell(_tokenId) &&
            ownerOf(_tokenId) != gameAddresses.getClaimNFTsAddress()
        ) {
            result = true;
        }
        return result;
    ...
    function nextStateToMint(uint256 _tokenId)
        external
        view
        returns (uint256)
    {
    ...
    function _updateNextMint(uint256 _tokenId, uint256 state) internal {
        require(state == _nextStateToMint[_tokenId],"State has changed since tx began");
        ...
        if(temp.bronzeMinted < 5){
            ...
            if(temp.bronzeMinted == 5 && temp.silverMinted == 5 && temp.goldMinted == 5){
                _next = 3;
                nextUnlock[_tokenId] = block.timestamp + 1 days;
            }
    ...
    function buyEgg(uint256 _tokenId, uint256 state) external returns(uint256){
        ...
        address _paymentContract = gameAddresses.getPaymentsAddress();

        require(payWithRewardOrWallet(msg.sender, _paymentContract, _price));
        require(IPayments(_paymentContract).payOwner(ownerOf(_tokenId), _price));

        nurseryRevenues[_tokenId] = nurseryRevenues[_tokenId] + _price;

        return IEggs(gameAddresses.getEggsAddress()).mintEgg(msg.sender, state, maturities[state]);
    }

    function reserveNextEgg(uint256 _tokenId, address _to) external returns(uint256) {
        require(msg.sender == ownerOf(_tokenId), "only nursery owner can reserve next egg");
        require(lastTimeReserveEgg[_tokenId] > block.timestamp + 1 days,"You can reserve only one egg by day");
        ...
        require(
            IPayments(gameAddresses.getPaymentsAddress()).burnRevenuesForEggs(msg.sender, averagePrices[state]),
            "Your center doesn't have enough balance");

        _updateNextMint(_tokenId,state);
        numberOfEggsOffered[_tokenId] += 1;
        offeredEggReference[_tokenId][numberOfEggsOffered[_tokenId]] = nurseryMintedDatas[_tokenId];
        return IEggs(gameAddresses.getEggsAddress()).mintEgg(_to, state, block.timestamp + maturities[state] * 2);
    }
...
contract Nursery is ERC721Enumerable, Ownable {
    ...
    uint256[4] _maxPrices = [100000 * 1E18, 200000 * 1E18, 500000 * 1E18, 1000000 * 1E18];

    string _tokenURI;

    IAddresses public gameAddresses;

    constructor(uint256 _preMint, address _NFTreserve,string memory _URI) ERC721("Nursery_NFT", "NURS") {
        ...
        for(uint256 i = 0; i < _preMint ; ){
            ...
            eggsPrices[_newItemId] = ZaiStruct.EggsPrices(1000 * 1E18,5000 * 1E18,10000 * 1E18,100000 * 1E18);
            unchecked{ ++i; }
        }
    }
    ...
    function _pricesOk(uint256[4] memory _prices) internal view returns(bool){
        return(_prices[0] >= _minPrices[0] && _prices[1] >= _minPrices[1] && _prices[2] >= _minPrices[2] && _prices[0] >= _minPrices[0]);
    }
}
