...
contract BaseV2Voter is ContractGuard, ReentrancyGuard, PausableUpgradeable, OwnableUpgradeable {
    address public _ve; // the ve token that governs these contracts
    uint256 public totalWeight; // total voting weight

    uint256 public poolLength; // all pools viable for voting (Protocol owned liquidity, Treasury, Profit share)
    ...
    function poke(uint256 _tokenId) external onlyOneBlock whenNotPaused {
        ...
        _vote(_tokenId, _weights);
    }

    function _vote(uint256 _tokenId, uint256[] memory _weights) internal {
        ...
        _reset(_tokenId);
        uint256 _weight = uint256(Ive(_ve).balanceOfNFT(_tokenId));
        uint256 _totalWeight = 0;
    ...
    function vote(uint256 tokenId, uint256[] calldata _weights) external onlyOneBlock whenNotPaused {
        require(Ive(_ve).isApprovedOrOwner(msg.sender, tokenId));
        require(_weights.length == poolLength, "length mismatch");
        _vote(tokenId, _weights);
    }
