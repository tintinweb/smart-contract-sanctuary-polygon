    ...
    PausableUpgradeable
{
    ...
    function initialize(string memory _baseTokenURI, string memory _projectID) public initializer {
    ...
    function setTokenState(uint256 _state) public onlyOwner {
    ...
    function mint(address user, uint256 quantity, string[] memory cidArr, string[] memory offChainIds) external onlyOwner {
        for (uint256 i = 0; i < quantity; i++) {
            _mint(owner(), sccCounter.current(), 1, '');
            exists[sccCounter.current()] = true;
            pendingBalance[user][sccCounter.current()] = 1;
            _setURI(sccCounter.current(), cidArr[i]);
            offChainToOnChainId[offChainIds[i]] = sccCounter.current();
            tokenState[sccCounter.current()] = 0;
            sccCounter.increment();
            tokensOwnedCount[user]++;
        }
    }

    // Allow smart-contract's admin to put a SCC on sale
    function putOnSale(address user, string memory offChainId, uint256 price) external {
        uint256 tokenId = offChainToOnChainId[offChainId];
        
        require(_exists(tokenId), "SCC does not exists.");
        require(balanceOf(owner(), tokenId) == 1, "Admin must owns your SCC to sell it.");
        require(pendingBalance[user][tokenId] == 1, "You must owns the SCC to sell it.");
        require(price > 0, "Price must be greater than 0.");

        onSale[tokenId] = true;
        tokenPrice[tokenId] = price;
        sellers[tokenId] = user;
        onSaleTokenIds.push(tokenId);

        emit PutOnSale(tokenId, price);
    }

    function removeFromSale(address user, string memory offChainId) external {
        uint256 tokenId = offChainToOnChainId[offChainId];

        require(_exists(tokenId), "SCC does not exists.");
        require(balanceOf(owner(), tokenId) == 1, "Admin must owns the SCC to sell it.");
        require(pendingBalance[user][tokenId] == 1, "You must owns the SCC to sell it.");
    ...
    function buy(address user, string memory offChainId) external {
        uint256 tokenId = offChainToOnChainId[offChainId];
        ...
        _safeTransferFrom(owner(), user, tokenId, 1, '');
