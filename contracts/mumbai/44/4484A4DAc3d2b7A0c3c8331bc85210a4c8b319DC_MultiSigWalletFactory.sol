...
contract MultiSigWallet {
	...
	event AddChangeProposal(uint indexed proposalId, address indexed proposer, uint required_votes, uint amount_above_voter, bool executed);
	...
	constructor (uint _amount_above_voter, uint _required_votes, address _owner) payable {
		...
		User memory user = User(_owner, msg.value, UserRoles.Voter);
	...
	function changeConstants(uint _required_votes, uint _amount_to_vote) public anOwner {
		...
		emit AddChangeProposal(changeProposalCount, msg.sender, _required_votes, _amount_to_vote, false);
...
contract MultiSigWalletFactory {
	...
	function create_wallet(uint amount_above_voter, uint required_votes) public payable {
		MultiSigWallet wallet = new MultiSigWallet{value: msg.value}(amount_above_voter, required_votes, msg.sender);
