...
interface PrizeMint {
    function isSilverMinterFunc(uint256) external view returns (bool);
    function isGoldMinterFunc(uint256) external view returns (bool);
}

contract OctopusPro is ERC721Base, PermissionsEnumerable {
    enum NFTType {Copper, Silver, Gold}
    ...
    address private prizeContractt;
    mapping(uint256 => NFT) public nfts;
    address public tokenPayment;
    uint256 public maxSupply;
    uint256 public platformFee;
    mapping(NFTType => uint256) public rewardAmounts;
    mapping(address => uint256) public walletMints;
    mapping(address => uint256) private tokenOfOwnerByIndex;
    ...
    { 
        ...
        rewardAmounts[NFTType.Silver] = 0.002 ether;
        rewardAmounts[NFTType.Gold] = 0.003 ether;
        prizeContractt = _addressPrize;
        platformFee = _platformFee;
        maxSupply = _maxSupply;
        mintStart = _timestampStart;
        mintEnd = _timestampEnd;
    }

    function setRewardAmount(NFTType nftType, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        rewardAmounts[nftType] = amount;
    }

    function setPlatformFee(uint256 _newFee) public onlyRole(DEFAULT_ADMIN_ROLE){
        platformFee = _newFee;
    }
    ...
    function uri1(uint256 _tokenid) private pure returns (string memory) {
        uint maxlength = 100;
        bytes memory reversed = new bytes(maxlength);
        return string.concat("https://nextjs-nftmarketplace-theta.vercel.app/api/nft/", uint2str(_tokenid));
    }

    function updateMintDate(uint256 _timestampStart, uint256 _timestampEnd) public onlyRole(DEFAULT_ADMIN_ROLE) {
        mintStart = _timestampStart;
        mintEnd = _timestampEnd;
    }
    

    function mintTo() public virtual payable{
        require(block.timestamp > mintStart && block.timestamp < mintEnd, "You cannot mint now.");
        require(walletMints[msg.sender] < 1, "You minted");
        uint256 tokenId = nextTokenIdToMint();
        require(platformFee == msg.value, "not enough fee");
        uint256 supply = totalSupply();
        require(supply <= maxSupply, "max NFT limit exceeded");

        NFTType nftType;
        if (PrizeMint(prizeContractt).isSilverMinterFunc(tokenId)) {
            nftType = NFTType.Silver;
        } else if (PrizeMint(prizeContractt).isGoldMinterFunc(tokenId)) {
            nftType = NFTType.Gold;
        } else {
            nftType = NFTType.Copper;
        }
        nfts[tokenId] = NFT(nftType, false);

        _setTokenURI(tokenId, uri1(tokenId));
        _safeMint(msg.sender, 1, "");
        tokenOfOwnerByIndex[msg.sender] = tokenId;
        walletMints[msg.sender] = tokenId;
    }

    function claimETH() public payable  {
        require(msg.sender!=address(0), "address doesn't exist");
        uint256 tokenId = tokenOfOwnerByIndex[msg.sender];
        NFT storage nft = nfts[tokenId];
        require(nft.nftType != NFTType.Copper, "NFT is not eligible for claim");
        require(!nft.claimed, "NFT has already been claimed");
        
        if (nft.nftType == NFTType.Silver) {
            (bool os, ) = payable(msg.sender).call{value: rewardAmounts[NFTType.Silver]}("");
            require(os);
        } else if (nft.nftType == NFTType.Gold) {
            (bool os, ) = payable(msg.sender).call{value: rewardAmounts[NFTType.Gold]}("");
            require(os);
        }
        nft.claimed = true;
    }
