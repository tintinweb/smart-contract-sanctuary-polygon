...
contract YourNftToken is ERC721A, Ownable, ReentrancyGuard {
  ...
  uint256 public requiredAmount = 2;
...
struct Text { 
  ...
  constructor(
    ...
    require(ownedAmount >= requiredAmount, "You Don't Own The Required NFT's");
    require(maxSupply + 1 <= 12500, "Max supply reached");
     Text memory newText = Text("");
    texts[maxSupply+ 1] = newText;
    _safeMint(msg.sender, maxSupply + 1);
  }
    ...
    Text storage currentText = texts[_tokenId];
    currentText.message = _newMessage;
  }

   function readMessage(uint256 _tokenId) public view returns (string memory message){
    Text memory currentText = texts[_tokenId];
 ...
 function setContractAddress(address _newAddress) public onlyOwner {
