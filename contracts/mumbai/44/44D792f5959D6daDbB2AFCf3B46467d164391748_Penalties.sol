...
contract Penalties is Game, VRF{
  ...
  uint256 public dailyReward = 25 ether;
  uint8 constant rounds = 5;

  bool public pending = false;
  uint256 public period = 1 hours;

  uint32 public baseRate = 60000; // ~90% base rate chance of scoring
  ...
  function requestRandomWords() external {
      require(block.timestamp > gameTime.start + (currentRound * period), "ROUND NOT OVER");
  ...
  function takePenalty(uint16[] calldata tokenIds, PenaltyQueue.Choice choice) public duringGame whenNotPaused originCheck {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
      ...
      bool onTarget = baseRate + attributeBoost(tokenId) > uint16(uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), tokenIds[i]))));
  ...
  function penaltyStatus(uint16 tokenId) public view returns (string memory) {
    PenaltyQueue.Penalty storage penalty = queue.tokenPenalty(tokenId);
    if(currentRound == penalty.round) return "ALREADY PLAYED";
    if(!penalty.onTarget) return "MISSED THE TARGET";
    if(penalty.choice == gafferChoices[penalty.round]) return "GAFFER SAVED";
    return "SCORED!";
  }
  ...
  function payout(uint256 amount) public {
    ...
    for (uint256 i = 0; i < min(amount, queue.length()); i++) {
      ...
      if(penalty.onTarget && penalty.choice != gafferChoices[penalty.round]) {
        reward = (penalty.priorGoals + 1) * dailyReward;
        reddies.mint(holder, reward);
      }
      emit Revealed(holder, penalty.tokenId, penalty.round + 1, reward);
    }
  }
  ...
  function attributeBoost(uint256 tokenId) public view returns (uint32) {
      ...
      return uint32(uint256(stats.finishing) * 1000); // 1000 extra chances per finishing point (will exceed maxUint16)
  }
