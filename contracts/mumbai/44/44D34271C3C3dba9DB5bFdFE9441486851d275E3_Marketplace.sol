...
contract Marketplace is Ownable{
    ...
    function renewRental(Order memory sellOrder, uint128 start, uint128 end) external payable{
        ...
        uint numberOfRentals = rentals.amount;
        updateOrderFill(sellOrder, numberOfRentals);
        (uint256 totalPayment, uint256 platformPayment) = calculateTotalPayment((sellOrder.payment.amount * numberOfRentals) / sellOrder.numberOfTokens);
        doPaymentTransfer(totalPayment, sellOrder.maker, sellOrder.payment, platformPayment, true);
        IProperty(collectionAddress).renewRental(sellOrder.maker, msg.sender, sellOrder.rentalId, numberOfRentals, start, end);
    }
    ...
    function distributeRevenue(uint pdrId, address[] calldata accounts, Payment memory payment) public{
        ...
        (uint256 totalPayment, uint256 platformPayment) = calculateTotalPayment(payment.amount*IERC1155(collectionAddress).balanceOf(msg.sender, pdrId));
        uint remainingAmount = payPlatformFee(totalPayment, payment, platformPayment);

        for(uint i=0; i< rentalIds.length; i++){
            IProperty.Rental memory currentRental = IProperty(collectionAddress).getRentalDetails(rentalIds[i].id);
            if(searchIfRentalOwnerMatches(accounts, currentRental.renter)){
                fee = payment.amount * (IERC1155(collectionAddress).balanceOf(currentRental.renter, rentalIds[i].id));
