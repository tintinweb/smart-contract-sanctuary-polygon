...
contract DaobiAccountability is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function mint(address _to, address _target) private {
        ...
        _setTokenURI(tokenId,URIcontract.generateURI(_target, grudgeBook[_target].accuser, daobi.chancellor(), grudgeBook[_target].supporters.length, uint32(block.timestamp % 2**32), generateSupporterString(_target) ));
    }
...
interface IDaobiAccountabilityURIs {
  ...
  function generateURI ( address target, address accuser, address chancellor, uint numSupporters, uint timestamp, string memory supporterList ) external view returns ( string memory );
