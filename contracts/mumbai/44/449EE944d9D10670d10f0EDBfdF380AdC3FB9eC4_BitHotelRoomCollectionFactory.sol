...
contract BitHotelRoomCollectionFactory is AccessControl {
    ...
    event Deployed(address newContract, address creator, string name, string symbol, uint256 replicas, uint256 price);
    ...
    function deployAuction(
        ...
        emit DeployedTimeAuction(address(newCollection), creator, name, symbol, replicas_, prices[0], duration);
    ...
    function deploy(
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            ...
            if (prices[1] == 0 || i == 1) {
                marketplace.addERC721Listing(creator, address(newCollection), i, prices[0]);
            } else {
                marketplace.addERC721Listing(creator, address(newCollection), i, prices[1]);
            }
        }
        ...
        emit Deployed(address(newCollection), creator, name, symbol, replicas_, prices[0]);
