...
contract KiloPerpView {
    ...
    function getTradeFee(
        ...
        (address productToken,,uint256 productFee,,,,,,) = KiloPerp(kiloPerp).getProduct(productId);
        return PerpLib._getTradeFee(margin, leverage, productFee, productToken, user, user, feeCalculator);
    }

    function getProducts(uint256[] memory ids) external view returns(Product[] memory) {
        Product[] memory products = new Product[](ids.length);
        for (uint256 i; i<ids.length; i++) {
            (
                ...
                uint256 openInterestShort,
                ,,
            ) = KiloPerp(kiloPerp).getProduct(ids[i]);
    ...
    function getPositions(address account, uint256[] memory ids) external view returns(KiloPerp.Position[] memory positions) {
        uint256 index;
        uint256[] memory positionIds = new uint256[](ids.length*2);
        for (uint256 i; i<ids.length; i++) {
            positionIds[index++] = KiloPerp(kiloPerp).getPositionId(account, ids[i], true);
            positionIds[index++] = KiloPerp(kiloPerp).getPositionId(account, ids[i], false);
        }
        positions = KiloPerp(kiloPerp).getPositions(positionIds);
    }
...
contract KiloPerp is ReentrancyGuard {
    ...
    mapping (address => mapping (address => bool)) public approvedManagers;//用户授权管理员
    // Events
    ...
    function stake(uint256 amount, address user) external payable nonReentrant {
        //校验用户和状态
//        require(
//            (canUserStake || msg.sender == owner)
//            &&
//            (msg.sender == user || _validateManager(user)),
//                "!stake");

        require((canUserStake || msg.sender == owner) && (msg.sender == user || msg.sender == vaultRewardDistributor), "!stake");
        //更新下当前用户stake奖励
        ...
        //console.log("balance", vault.balance, amount);
        ...
        if (stakes[user].amount == 0) {
            ...
            if (!_validateManager(user)) { //从vaultFeeReward调用过来的收益再投资 不重新计算时间
...
//        console.log("position.price",uint256(position.price));
    ...
    function _updateVaultAndGetFee(
        ...
        if (remainMargin <= 0) { //margin不够亏损，全部贡献给国库了
            ...
            //console.log("remainMargin", uint256(remainMargin));
            if (pnl >= 0) {
                //console.log("0pnl", uint256(pnl));
