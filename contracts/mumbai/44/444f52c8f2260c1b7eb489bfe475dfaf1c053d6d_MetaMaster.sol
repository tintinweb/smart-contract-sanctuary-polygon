...
contract MetaMaster is Ownable {
    ...
    function getRefund(address _userAddress) public {
        ...
        if (user.deposits[0].depositedTimeStamp <= user.deposits[0].depositedTimeStamp + 15 minutes && userTeams[_userAddress].length == 4 && user.referrer != address(0)){
            require(IERC20(USDTAddress).transfer(_userAddress, user.deposits[0].depositAmount));
            }
    }
