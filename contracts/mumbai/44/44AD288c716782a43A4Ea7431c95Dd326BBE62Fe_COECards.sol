  ...
  DefaultOperatorFilterer
{
  ...
  function airdrop(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      _intMint(_addresses[i], _type, _level);
    }
  }
  ...
  function packMint(
    ...
    for (uint256 i = 0; i < _types.length; i++) {
      ...
      _intMint(_to, _types[i], 1);
    }
  }

  function _intMint(address _to, uint256 _type, uint256 _level) internal {
  ...
  function _setType(
    ...
    if (!types[_typeId].exists) {
      //add to the rariry array that it belongs to on bnm contract
      BnmInterface(burnAndMintAddress).addCardType(_typeId, _rarity);
      if (_isPromo) promoTypes.push(_typeId);
      typeCount++;
    }
  ...
  function getPromoTypes() public view returns (uint256[] memory) {
    return promoTypes;
  }
