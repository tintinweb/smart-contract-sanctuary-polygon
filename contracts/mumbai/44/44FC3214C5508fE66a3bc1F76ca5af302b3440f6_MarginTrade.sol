...
contract MarginTrade is Ownable {
    ...
    function createPositionandTrade(
        ...
        uint256 depositWithoutReserve = (depositInfo.assetAmount -
            (depositInfo.assetAmount.mul(reserveMargin).div(100))); // (80 - 80*20/100)

        uint256 marginForPosition = ((amount * depositWithoutReserve) /
            (
                depositWithoutReserve.add(depositInfo.loanAmount).sub(
                    depositInfo.usedMargin
                )
            ));

        // uint256 loanForPosition = (amount.sub(marginForPosition));
        ...
        uint256 _liquidationPrice = (
            amount.sub((depositInfo.assetAmount.mul(80)).div(100))
        ).div(amount * (tradePriceAfterTrade));
    ...
    // function getliquidationPrice(uint256 amount, address tokenAddress)
    //     public
    //     view
    //     returns (uint256)
    // {
    //     Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
    //     uint256 _liquidationPrice = (
    //         amount.sub((depositInfo.assetAmount.mul(80)).div(100)).div(
    //             amount.div(tradePriceAfterTrade)
    //         )
    //     );
    ...
    function liquidateOpenPosition() external onlyOwner {
        uint256 _positionID;
        address user;
        ...
        for (uint256 i = 1; i < positionID; i++) {
            ...
            if (
                ...
                price[1] < positionInfo.liquidationPrice
            ) {
