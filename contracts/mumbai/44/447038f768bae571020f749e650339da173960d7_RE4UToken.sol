...
/**
 * @title RE4U | RE Tokens LLC
 * @author Stobox Technologies Inc.
 * @dev RE4U ERC20 Token | This contract is opt for digital securities management.
 */
 
contract RE4UToken is IERC20, Roles {
    ...
    modifier onlyWithUnfrozenFunds(address _account) {
        require(
            ...
            "RE4U: funds are frozen."
        );
        _;
    }

    /**
     * @notice RE4UToken simply implements a ERC20 token.
     */
    constructor() public Roles(msg.sender) {
        _name = "REtoken Security Token";
        _symbol = "RE4U";
        _decimals = 0;
        _kDecimals = 18;
        _k = 10**_kDecimals;
        _defaultTransferLimit = 999999;
        _defaultTransactionCountLimit = 0;

        _mint(msg.sender, 52000000);
    ...
    function burn(address _account, uint256 _amount) external onlySuperAdmin {
        require(balanceOf(_account) >= _amount, "RE4U: balance too low");
    ...
    function transferFunds(
        ...
        require(balanceOf(_from) >= _amount, "RE4U: not enough tokens");
    ...
    function splitOrMerge(uint256 _x, uint256 _y) external onlySuperAdmin {
        require(_x != _y, "RE4U: _x must not be equal to _y");
        require(_x > 0, "RE4U: _x must be greater than 0");
        require(_y > 0, "RE4U: _y must be greater than 0");
    ...
    */
        ...
        returns (bool)
    {
        ...
        _approve(
            ...
            _allowances[_sender][msg.sender].sub(
                ...
                "RE4U: transfer amount exceeds allowance"
            )
        );
        ...
        returns (bool)
    {
        _approve(
            ...
            _allowances[msg.sender][_spender].sub(
                ...
                "RE4U: decreased allowance below zero"
            )
        );
    ...
    function _transfer(
        ...
        require(_sender != address(0), "RE4U: transfer from the zero address");
        require(_recipient != address(0), "RE4U: transfer to the zero address");

        _beforeTokenTransfer(_sender, _recipient, _amount);

        _balances[_sender] = _balances[_sender].sub(
            _amount,
            "RE4U: transfer amount exceeds balance"
        );
    ...
    function _mint(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "RE4U: mint to the zero address");
    ...
    function _burn(address _account, uint256 _amount) internal virtual {
        require(_account != address(0), "RE4U: burn from the zero address");

        _beforeTokenTransfer(_account, address(0), _amount);

        _balances[_account] = _balances[_account].sub(
            _amount,
            "RE4U: burn amount exceeds balance"
        );
    ...
    function _approve(
        ...
        require(_owner != address(0), "RE4U: approve from the zero address");
        require(_spender != address(0), "RE4U: approve to the zero address");
    ...
    function _updateTransferLimit(address _account, uint256 _amount) private {
        ...
        _transferLimits[_account].allowedToTransfer = _transferLimits[_account]
            .allowedToTransfer
            .sub(_amount, "RE4U: transfer exceeds your transfer limit");
    }
    ...
    function _updateTransactionCountLimit(address _account) private {
        if (_isEnabledTransactionCount) {
            ...
            _transactionCountLimits[_account]
                ...
                .sub(1, "RE4U: transfer exceeds your transaction count limit");
        }
    }
}