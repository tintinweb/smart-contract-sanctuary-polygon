...
contract Rental is Initializable, ReentrancyGuardUpgradeable {
    ...
    function withdrawRoyalty() external nonReentrant onlyOwner {
        ...
        isOwner(_landId, _lordId)
    {
        ...
        _transfer(landContract, msg.sender, address(this), _landId);
    ...
    {
        require(landLordsInfo[_rewardId].status, "not valid rewardId");
        _claim(_rewardId);
    }
    ...
    function _claim(uint256 _rewardId) internal {
        uint256 numberOfMonth = monthCalculation(_rewardId);

        uint256 rewards = _rewardsCalculate(_rewardId);

        rewards = rewards * numberOfMonth;
    ...
    function _timeCheck(uint256 _rewardId) internal view returns (bool) {
        uint256 stakeTime = landLordsInfo[_rewardId].stakeTime;
        if (
            landLordsInfo[_rewardId].stakeTime < block.timestamp &&
            (block.timestamp - stakeTime) >= timeSlot
        ) {
            return true;
        }
    ...
    function monthCalculation(uint256 _rewardId) internal returns (uint256) {
        uint256 numberOfMonth;
        for (uint256 i = 0; i < month; i++) {
            uint256 stackTime = landLordsInfo[_rewardId].stakeTime;

            if (
                block.timestamp >= stackTime &&
                block.timestamp - stackTime >= timeSlot
            ) {
                numberOfMonth += 1;
