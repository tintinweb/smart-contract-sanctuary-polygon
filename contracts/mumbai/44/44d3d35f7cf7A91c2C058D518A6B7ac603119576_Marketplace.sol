  ...
  ReentrancyGuard
{
  ...
  constructor(address _tokenAddress, address _wethAddress)
    ...
    PriceCalculation(_tokenAddress)
  {
  ...
  function purchaseWithETHw3user(
    ...
    _validateAmount(tokenPriceFiat[_tokenId], _wethAmount);
    ...
    uint256 fee = _calcBuyerSellerFee(_wethAmount);
    _send(fee, dbiliaToken.dbiliaFee());

    // calculate royalty amount and send to creator
    (uint256 royaltyAmount, address receiver) = _calcRoyalty(
      _tokenId,
      _wethAmount
    );
  ...
  function placeBidWithETHw3user(
    ...
    _validateAmount(_bidPriceFiat, _wethAmount);
    ...
    (uint256 royaltyAmount, ) = _calcRoyalty(_tokenId, _wethAmount);
...
contract PriceCalculation is PriceConsumerV3 {
  using SafeMath for uint256;
   ...
   */
  function _validateAmount(uint256 _fiatPrice, uint256 _wethAmount)
    internal
    view
  {
   ...
   */
  function _calcBuyerSellerFee(uint256 _wethAmount)
    internal
    view
    returns (uint256)
  {
   ...
   * @param _wethAmount amount of weth
   */
  function _calcRoyalty(uint256 _tokenId, uint256 _wethAmount)
    internal
    view
