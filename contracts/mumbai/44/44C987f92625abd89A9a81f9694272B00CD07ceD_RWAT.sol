    ...
    PausableUpgradeable
{
    ...
    /**
     * @notice Mints asset units to this contract.
     */
        ...
        onlyRole(ADMIN)
    {
     ...
     /**
     * @notice Mints asset units to a specified address.
     */
    function mintAssetTo(uint256 _assetId, uint256 _amount, address _to)
        external
        onlyRole(ADMIN)
    {
        _mintAsset(_assetId, _amount, _to);
    }

    /**
     * @notice Mints asset units to a specified address, through a handler.
     */
    function handlerMintAsset(uint256 _assetId, uint256 _amount, address _to)
        external
        onlyRole(HANDLER)
    {
        _mintAsset(_assetId, _amount, _to);
    ...
    /**
     * @notice User mints asset units with a server signature.
     */
    function userMintAsset(
        uint256 _assetId,
        uint256 _amount,
        uint8 _v,
        bytes32 _r,
        ...
        bytes memory message = abi.encode(msg.sender, address(this), _assetId, _amount, internalNonce[msg.sender]);
        require(
            ecrecover(
        ...
        _mintAsset(_assetId, _amount, msg.sender);
    }

    function _mintAsset(uint256 _assetId, uint256 _amount, address _to) internal {
        uint256 newNextId = nextId[_assetId] + _amount;
        require(
            newNextId <= lastId[_assetId],
            "Amount exceeds max"
        );

        for (uint256 i = nextId[_assetId]; i < newNextId; i++) {
            _mint(_to, i);
        }
        nextId[_assetId] = newNextId;
    }
    ...
    function setTransfersPaused(bool _paused) external onlyRole(ADMIN) {
     ...
     function setWhitelistedTransfers(bool _whitelistedTransfers) external onlyRole(ADMIN) {
