...
contract NFTAttack is ERC1155LazyMint {
    ...
    event Attacked(address indexed attacker, address indexed victim, uint256 level);
    ...
    function verifyClaim(
        ...
        require(_quantity == 1, "Only one Weapon at a time!");
    ...
    /** 
     * @notice Burn a Weapon to either level up or attack another Soul
     */
    function burn(
        address account,
        uint256 id,
        ...
        require(msg.sender == account || balanceOf[msg.sender][2] > 0, "NOT_TOKEN_OWNER or Iron Sword");
    ...
    /**
     * @notice Lets a Iron Sword owner attack another user's to burn their waepons
     */
    function attack(address victim) external gameNotPaused {
        address attacker = msg.sender;
        // only a Iron Sword owner can attack
        require(balanceOf[attacker][2] > 0, "You need an Iron Sword to attack!");
        // find which weapon the victim has
        ...
        if(balanceOf[victim][0] > 0) {
            ...
            revert("Victim has no Weapon!");
        }
        // burn it
        _burn(victim, tokenToBurn, 1);
        // mint a badge of honor to the attacker
        _mint(attacker, 3, 1, "");
        emit Attacked(msg.sender, victim, tokenToBurn + 1);
    }
