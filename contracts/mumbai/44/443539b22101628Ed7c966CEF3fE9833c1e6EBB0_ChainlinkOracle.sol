...
contract ChainlinkOracle is ChainlinkClient, AccessControlEnumerable {
  ...
  bytes32 public constant jobId = 0x3737626435666230643065653433356138666331343562323634326238316365;
  ...
  mapping(string => Response) public prices;
  mapping(uint256 => Request) public lastRequests;

  /**
   * Special mapping to store each collection to a certain index as it's easier to pass uint256 than strings
   */
  mapping(address => uint256) public collectionAddressToIndex;

  error Unauthorized();
  event ToggleCollectionString(address _collectionAddress, string _collectionName);
  event RequestFulfill(bytes32 _requestId, uint256 _estimate, string indexed _collection, string indexed _pricingAsset);
  event RequestSubmited(bytes32 _requestId, uint256 _time, string indexed _collection, string indexed _pricingAsset);
  event AddedApprovedExecutor(address _executor, bool _approved);
  ...
  function requestFloor(
    ...
    requests[reqId] = Request(_time, _collection, _pricingAsset, reqId);
    emit RequestSubmited(reqId, _time, _collection, _network);
  }

  function fulfillEstimate(bytes32 _requestId, uint256 _estimate) external recordChainlinkFulfillment(_requestId) {
    Request memory request = requests[_requestId];
    prices[request.collection] = Response(request.collection, request.pricingAsset, request.time, _estimate, _requestId);
    emit RequestFulfill(_requestId, _estimate, request.collection, request.pricingAsset);
  }

  function getUnderlyingNFTPrice(address _collectionAddress, uint256 _decimals) external view returns (uint256) {
    uint256 price = prices[collections[_collectionAddress]].estimate;
   ...
   */
  function getUnderlyingPrice(HTokenI _token) external view returns (int256) {
    (
      ,
      /*uint80 roundID*/
  ...
  function addPriceFeed(HTokenI _token, address _oracle) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(FACTORY, msg.sender)) revert Unauthorized();
    priceFeeds[_token] = AggregatorV3Interface(_oracle);
    emit PriceFeedAdded(_token, _oracle);
  }
