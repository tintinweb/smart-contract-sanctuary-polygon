   ...
   function getCouponIdx(string calldata _coupon)
    ...
    function _calcPrice(
        ...
        if (_currency == address(0))
        {
            ...
            hash = keccak256(abi.encodePacked(_msgSender(), "+", _tier, substr(_coupon.id, 0, 15))).toEthSignedMessageHash();
            ...
            bytes memory signature = b64decode(substr(_coupon.id, 16, bytes(_coupon.id).length));
