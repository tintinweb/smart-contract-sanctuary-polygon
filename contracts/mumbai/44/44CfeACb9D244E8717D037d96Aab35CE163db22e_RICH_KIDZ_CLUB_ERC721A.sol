...
/* 	IERC20 public constant USDC = IERC20(0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48);
	...
	IERC20 public constant USDC = IERC20(0x80235af5FD3fA0Aeb5c9005fFD859C6E06061170);
	IERC20 public constant USDT = IERC20(0x80235af5FD3fA0Aeb5c9005fFD859C6E06061170);

	IChainlinkPriceFeed public constant ETH_ORACLE = IChainlinkPriceFeed(0x0715A7794a1dc8e42615F059dD6e406A6594651A);
	...
	function initialize() external initializer {
		__ERC721AUpgradeableOwnable_init("RICH KID$ CLUB by CH Pulgarin", "RKC");
		
		 _revealedBaseURI = "revealedURI/";
		 _unrevealedBaseURI = "UNrevealedURI/";
	}
	...
	function mint(
		...
		if(phase != Phase.PUBLIC) _addMintedToAux(msg.sender, phase, uint64(amount));
        
		// check whitelist restrictions and supply restrictions
        require(_mintAllowed(phase, permittedAmount, supplyAfterMint, proof), "mint disallowed");
	...
	// OWNER
    function allocate(address[] calldata recipients, uint256[] calldata amounts) external onlyOwner {
    ...
    function setPhaseConfig(Phase phase, PhaseConfig memory config) external override onlyOwner {
        require(phase != Phase.PAUSED, "paused has no config");
        require(config.mintingFee >= ETH_DECIMALS / 10, "fee not set");
        require(config.maxSupply > totalSupply() && config.maxSupply <= MAX_SUPPLY, "maxSupply out of bounds");
    ...
    function setActivePhase(Phase phase) external override onlyOwner {
        ...
        if(phase != Phase.PAUSED) {
            ...
            require(config.mintingFee >= ETH_DECIMALS / 10, "fee not set");
            require(config.maxSupply > totalSupply(), "need additional nfts to mint");
    ...
    function setUnrevealedBaseURI(string memory newUnrevealedURI) external onlyOwner {
    ...
    function _processPayment(IERC20 currency, uint256 feeInWei) internal {
        if(currency == USDT || currency == USDC) {
			...
			uint256 fiatFee = feeInWei * FIAT_TOKEN_DECIMALS * getLatestEthPrice() / ETH_DECIMALS / PRICE_FEED_DECIMALS;
	...
	function _addMintedToAux(address minter, Phase phase, uint64 amount) internal {
	...
	function _mintAllowed(
        ...
        if(phase != Phase.PUBLIC) {
            ...
            if(!_verifyProof(msg.sender, permittedAmount, config.root, proof)) return false;
	...
	function mintedInPhase(address who, Phase phase) public view returns(uint256) {
    ...
    function tokenURI(uint256 tokenId) public view override returns(string memory) {
        ...
        // QUESTION: Should we reveal once, when all minting rounds are done, or multiple times after every round?
        if(tokenId < _revealedUntil) {
            return string(abi.encodePacked(_revealedBaseURI, _toString(tokenId)));
        }
        return string(abi.encodePacked(_unrevealedBaseURI, _toString(tokenId)));
    }
	...
	function getStakingStats(uint256 tokenId) external view returns(StakingStats memory stk) {
		require(_exists(tokenId), "nft not minted");
		if(isStaked(tokenId)) {
			stk = _staking[tokenId];
		} else {
			stk = StakingStats(_staking[tokenId].baseRate, 0, 0, 0, 0, address(0));
		}
	}
	...
	function amountOfCurrentlyMintableNfts() public view returns(uint256) {
		...
		return currentMaxSupply > currentSupply ? currentMaxSupply - currentSupply : 0;
	}


	// STAKING
	function stake(uint256[] calldata tokenIds, uint256[] calldata lengths, bool isRestake) public {
	...
	function revealTypes(
		...
		for(uint256 i = 0; i < tokenIds.length; i = i.unsafeInc()) {
			require(proofs[i].verify(revealRoot, keccak256(abi.encodePacked(tokenIds[i], baseRates[i]))), "invalid proof");
	...
	function getRewardsAndRestake(uint256[] calldata tokenIds, uint256[] calldata lengths) external {
		getRewardFor(msg.sender, tokenIds);
		stake(tokenIds, lengths, true);
	}

	function _calculateClaimableAmount(StakingStats storage stk)
		internal
		view
		returns(uint256)
	{
	...
	function _calculateBonus(uint256 length, uint256 rate) internal pure returns(uint256) {
...
contract ERC721AUpgradeableOwnable is IERC721A, OwnableUpgradeable {
    ...
    function __ERC721AUpgradeableOwnable_init(string memory name_, string memory symbol_) internal onlyInitializing {
        __Ownable_init();
        _name = name_;
        _symbol = symbol_;
	...
	function _transferForStaking(address from, address to, uint256 tokenId, bool staking) internal {
        uint256 prevOwnershipPacked = _packedOwnershipOf(tokenId);
...
interface IERC721A {
    ...
    /**
     * @dev custom Minted event indicating who was the msg.sender and who the recipient.
     */
