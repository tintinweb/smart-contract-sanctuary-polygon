...
/// @title Lending
/// @notice Lets users lend their hunters and borrow lended hunters
/// @notice This information is later used in the Hunters OnChain game
/// @author zetsub0ii.eth
contract Lending is AccessControl, Pausable {
  bytes32 constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
  bytes32 constant PLATFORM_ROLE = keccak256("PAUSER_ROLE");
  ...
  uint256 commission; // max BASE_PRECISION
  ...
  /// @notice Sets studio address which can collect commissions
  /// @dev Requires PLATFORM_ROLE
  ...
  function getOperator(uint256 hunterId) public view returns (address) {
    ...
    if (lending.status == LendingStatus.FULFILLED) {
      if (block.timestamp > lending.borrowedAt + lending.borrowedDays * 1 days) return address(0);
      // Discuss
      // Lending is expired, return empty address
      // Could be owner again
      else return lending.borrower;
    }

    if (lending.status == LendingStatus.CANCELLED) {
      if (block.timestamp - lending.lastUpdatedAt < 48 hours) return lending.borrower;
  ...
  function lendHunter(uint256 hunterId, uint256 dailyPrice, uint16 revenueShare)
    external
    whenNotPaused
  {
  ...
  /// @dev * If revenue share is 10.07%, 10070 must be passed
  function editHunterLending(uint256 hunterId, uint256 newDailyPrice, uint16 newRevenueShare)
    external
    whenNotPaused
  {
  ...
  function borrowHunter(uint256 hunterId, uint256 expectedPrice, uint16 borrowDays)
    external
    whenNotPaused
  {
    ERC721Lending memory lending = hunterLendings[hunterId];
    ...
    emit HunterBorrowed(hunterId, expectedPrice, borrowDays);
  }
  ...
  function _claimPendingRewards(ERC721Lending memory lending, address lendingOwner)
    private
    returns (uint48, uint16)
  {
    uint16 dayDiff = _dayDifference(uint48(block.timestamp), lending.lastUpdatedAt);
    ...
    uint256 commissionAmount = (claimAmount * commission) / BASE_PRECISION;
    studioBalance += commissionAmount;

    require(boom.approve(lendingOwner, claimAmount - commissionAmount), "BOOM transfer failed");
  ...
  function claimPendingRewards(uint256 hunterId) external {
    ...
    (uint48 updatedTime, uint16 claimDays) = _claimPendingRewards(lending, msg.sender);
  ...
  function cancelBorrowing(uint256 hunterId) external {
    ...
    (, uint16 claimDays) = _claimPendingRewards(lending, hunters.ownerOf(hunterId));
    ...
    if (cancelledDays > 0) {
      ...
      uint256 studioCut = (claimableAmt * EARLY_CANCEL_PENALTY) / BASE_PRECISION;
  ...
  /// @param hunterId ID of the hunter
  /// @dev @pre Depends on lending.remainingDays
  function closeHunterLending(uint256 hunterId) external {
    ...
    require(
      (lending.status == LendingStatus.FULFILLED) || (lending.status == LendingStatus.CANCELLED),
      "Invalid lending status"
    );

    if (lending.status == LendingStatus.CANCELLED) {
      require(block.timestamp - lending.lastUpdatedAt >= 48 hours, "Not 48 hours after cancel");
      ...
      require(
        boom.approve(lending.borrower, lending.remainingDays * lending.dailyPrice),
  ...
  function distributeRevenue(uint256[] memory hunterIds, uint256[] memory hunterAmounts)
    external
    onlyStudio
    whenNotPaused
  {
