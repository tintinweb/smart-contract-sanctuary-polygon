...
contract xyz11Staking9 is Ownable {
    ...
    function withdrawRewards() public {
        require(!paused, "The contract is paused");
        uint256 _amount = getRefReward(msg.sender);
        setRefReward(msg.sender);
        require(_xyzToken.balanceOf(address(this)) >= _amount, "Limit Exceeded");
        _xyzToken.transfer(msg.sender, _amount);
    }
    ...
    function getRefReward(address _user) public view returns(uint256){
    ...
    function setRefReward(address _user) internal {
        for (uint i=0; i < rewards_info[_user]._addr.length; i++) {
            if(rewards_info[_user]._addr[i] == msg.sender){
