...
contract LittlebitsNFT is VRFConsumerBaseV2, ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function ADMIN_addAllowlistAddresses(address[] memory addresses) public onlyOwner {
        allowlistSize += addresses.length;
        for (uint i = 0; i < addresses.length; i++) {
            allowlistStatus[addresses[i]] = true;
        }
    }
 ...
 * @author gifMaker - [email protected]
 * @notice v1.01 / 2023
 * @dev Retrieves Character attributes in a human readable format - polygon release
 */
