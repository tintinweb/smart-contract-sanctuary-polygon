...
contract chain_scouts_ships_l2 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, NonblockingLzApp {
    ...
    function mintTokens(address _to, uint[] memory tokenIds) public {
    ...
    function addAddressToWhitelist(address[] memory _toBeWhitelisted) public onlyOwner{
              for (uint256 i = 0; i < _toBeWhitelisted.length; i++) {
                  whitelistedAddresses[_toBeWhitelisted[i]] = true;
              }
         }
    ...
    function transferTokensToL1(uint[] memory tokenIds, bytes memory _userAddress, uint16 destinationChainId) external payable{
        burnTokens(tokenIds);
