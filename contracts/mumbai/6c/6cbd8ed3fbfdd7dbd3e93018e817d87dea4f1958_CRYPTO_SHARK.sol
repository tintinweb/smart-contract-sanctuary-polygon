...
contract CRYPTO_SHARK is ERC721A, Ownable, Pausable {
    ...
    function Mint(uint256 num) public payable whenNotPaused {
      require((_price * num) <= msg.value, "LOW_ETHER");
      require( WL_Mint[msg.sender] + num <= maxPerTx,    "Exceeds minting limit" );
    ...
    function _airdrop(address _address, uint256 num) internal {
        require(num <= maxBatchSize, 'TRY TO MINT LESS');
        unchecked { require(totalSupply() + num <= Supply, 'MAX_REACHED'); }
    ...
    function WHITELIST_MINT(uint256 num) public payable {
      ...
      require( supply + num <= Supply - _reserved,             "Exceeds maximum PUNK supply" );
