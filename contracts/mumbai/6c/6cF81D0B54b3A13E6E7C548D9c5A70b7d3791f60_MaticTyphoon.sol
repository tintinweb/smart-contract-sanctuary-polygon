...
contract MaticTyphoon{    
    struct User {
        ...
        mapping(uint8 => uint256) matrixHoldAmount;
    }
    ...
    address payable public changer;
    ...
    constructor(address payable _changer) public {
        changer=_changer;
        ...
        users[owner].activeX3Levels[10] = true;
    }
    ...
    function setPrice(uint256 price) public {
        require(msg.sender == changer, "only Owner");
        maticRate=price;
    }
    ...
    function test(uint256 value) public view returns(uint256)
    {
      return value*(maticRate/1e18);  
    }
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level, uint256 amount) private{
        ...
        else
        {
            ...
            if(users[referrerAddress].x6Matrix[level].referrals.length < level+3) 
            {
                users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
                emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
                emit MatrixBonus(referrerAddress, amount, (level),(amount*maticRate)/1e18);
                if(users[referrerAddress].partnersCount==49)
                address(uint160(referrerAddress)).transfer(amount);   
                else
                users[referrerAddress].matrixHoldAmount[9]+=amount;
                return;
            }
            users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
    ...
    function autoUpgrade(address _user, uint8 level) private{
             ...
             if(level<4 || (level==4 && users[_user].partnersCount>=1) || (level==5 && users[_user].partnersCount>=4) || (level==6 && users[_user].partnersCount>=9) || (level==7 && users[_user].partnersCount>=16) || (level==8 && users[_user].partnersCount>=25) || (level==9 && users[_user].partnersCount>=36))
             address(uint160(_user)).transfer(userAmount);
             else
             users[_user].matrixHoldAmount[level-1]+=userAmount;
    ...
    function withdrawLevelIncome() public{
        ...
        uint8 i;
        uint256 payAble=users[msg.sender].withdrawable;
        
        msg.sender.transfer((payAble*80)/100);
        ...
        for(i=0;i<10;i++)
        {
           ...
           if(payUpId==0)
           break;
           users[idToAddress2[payUpId]].withdrawable+=(payAble/100); 
           emit CommunityBonus(idToAddress2[payUpId], msg.sender, payAble/100, ((payAble/100)*maticRate)/1e18, i+1,1);
        }
        ...
        for(i=0;i<10;i++)
        {
           ...
           break;
           users[idToAddress2[payDownId]].withdrawable+=(payAble/100); 
           emit CommunityBonus(idToAddress2[payDownId], msg.sender, payAble/100, ((payAble/100)*maticRate)/1e18, i+1,2);
        }
        users[msg.sender].withdrawable=0;
        emit Withdraw(msg.sender,payAble);
    }
    ...
    function getUserMatrixHoldAmount(address userAddress) public view returns(uint256[] memory) {
		uint256[] memory mlevelHold = new uint256[](12);
		for(uint8 j=0; j<12; j++)
		{
		  mlevelHold[j]=users[userAddress].holdAmount[j+1];
		}
		return (mlevelHold);
	}

    function claimMatrixHoldAmount(uint8 level) public {
		if(users[msg.sender].matrixHoldAmount[level]>0 && level==3 && users[msg.sender].partnersCount>=1)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==4 && users[msg.sender].partnersCount>=4)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==5 && users[msg.sender].partnersCount>=9)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==6 && users[msg.sender].partnersCount>=16)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==7 && users[msg.sender].partnersCount>=25)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==8 && users[msg.sender].partnersCount>=36)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }

        if(users[msg.sender].matrixHoldAmount[level]>0 && level==9 && users[msg.sender].partnersCount>=49)
        {
            msg.sender.transfer(users[msg.sender].matrixHoldAmount[level]);
            users[msg.sender].matrixHoldAmount[level]=0;
        }
