...
contract GangNFTCreator is ERC721, Ownable, ReentrancyGuard {
     ...
     *@return tokenHolders Return GangNFT contracts of tokens the wallet holds from the respective club.
     */
    function getAllTokenHolders(
        address wallet
    ) public view returns (address[] memory) {
        address[] memory gangNFTContracts = new address[](totalSupply());
        uint256 index = 0;
        for (uint256 i = 0; i < totalSupply(); i++) {
            GangNFT gangNFT = GangNFT(allClubs[i]);
            uint256 balance = gangNFT.balanceOf(wallet);
            if (balance > 0) {
                gangNFTContracts[index] = address(gangNFT);
                index++;
            }
        }
        assembly {
            mstore(gangNFTContracts, index)
        }
        return gangNFTContracts;
    }
