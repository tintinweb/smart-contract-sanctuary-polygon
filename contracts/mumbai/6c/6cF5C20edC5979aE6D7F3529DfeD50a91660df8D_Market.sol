...
/// @title Classic Chords NFT Contract
/// @notice A contract for minting and managing Classic Chords NFTs
contract ClassicChords is ERC5006, Ownable {

    uint256 tokenId; // Counter for token ID
    ...
    /// @param recordLimit_ The maximum number of records allowed
    /// @param _contractAddress The address of the approved marketplace contract
    constructor(uint recordLimit_, address _contractAddress) ERC5006("", recordLimit_) {
        approvedContractAddress = _contractAddress;
    }

    /// @notice Function to approve the marketplace to list tokens
    function approveMarket() public onlyOwner {
        setApprovalForAll(approvedContractAddress, true);
    }

    /// @notice Function to disapprove the marketplace from listing tokens
    function disapproveMarket() public onlyOwner {
        setApprovalForAll(approvedContractAddress, false);
    }

    /// @notice Function to get the minted NFTs of a user
    /// @param _usersAddress The user's address
    ...
    /// @notice Function to view the token URI of a given token ID
    ...
    /// @notice Function to get a user record by record ID
    /// @param _recordId The ID of the record
    /// @return The user record corresponding to the ID
    function _userRecordOf(uint256 _recordId) external view returns (UserRecord memory) {
...
/**
 * @title Market
 * @dev A decentralized marketplace contract for buying and renting non-fungible tokens (NFTs).
 */
contract Market is Ownable, ReentrancyGuard, ERC1155Holder {
    ...
    // Counter for tracking the number of items deleted
    Counters.Counter private _itemsDeleted;
    // Counter for tracking the number of items rented
    Counters.Counter private _itemsRent;

    // Listing price for placing items on the marketplace
    uint256 public listingPrice;
    // Address of the NFT token contract
    address public addressNft;
    // Instance of the NFT token contract
    ClassicChords private nft;

    // Mapping of item IDs to MarketItems
    ...
    /**
     * @dev Sets the listing price of the contract.
     ...
     * @param _quantity The quantity of the token being listed.
     * @param _price The listing price for the token.
     * @param _expiry The expiry timestamp for the listing.
     * @param _recordId The record ID associated with the token.
     * @param _isAvailableOnRent Boolean indicating if the token is available for rent.
     * @param _isAvailableOnSell Boolean indicating if the token is available for sale.
     * @param _royaltyPr The royalty percentage for the token.
     */
    ...
    function buyNft(uint256 _itemId) external payable nonReentrant {
        ...
        nft.transferFrom(address(this), msg.sender, item.tokenId, item.quantity);
    ...
    /**
     * @dev Rents an NFT from the marketplace.
     * @param _itemId The ID of the item being rented.
     */
    function rentNft(uint256 _itemId) external payable nonReentrant {
        // Retrieve the MarketItem from the mapping
        MarketItem memory item = marketItemsMapping[_itemId];

        // Check if the item is available for rent
        require(item.isAvailableOnRent, "Item not available for rent");
        require(msg.sender != address(0), "Invalid renter address");
        require(msg.value >= item.price, "Insufficient payment");
    ...
    /**
     * @dev Deletes an expired or sold item from the marketplace.
     * @param _itemId The ID of the item to delete.
     */
    function deleteItem(uint256 _itemId) external onlyOwner {
        // Retrieve the MarketItem from the mapping
        MarketItem memory item = marketItemsMapping[_itemId];
        ...
        nft.transferFrom(address(this), item.seller, item.tokenId, item.quantity);

        // Emit the NFTSold event with a price of 0
        emit NFTSold(
            item.tokenId,
            item.seller,
            address(0),
    ...
    /**
     * @dev Returns the total number of items sold.
    ...
    /**
     * @dev Returns the total number of items deleted.
     * @return The total number of items deleted.
     */
    function getItemsDeleted() public view returns (uint256) {
        return _itemsDeleted.current();
    }

    /**
     * @dev Returns the total number of items rented.
     * @return The total number of items rented.
     */
    function getItemsRent() public view returns (uint256) {
        return _itemsRent.current();
    }

    /**
     * @dev Returns the details of a specific market item.
     * @param _itemId The ID of the market item.
    ...
    /**
     * @dev Returns an array of all market item IDs.
     * @return An array of all market item IDs.
     */
    function getAllMarketItemIds() public view returns (uint256[] memory) {
        uint256[] memory itemIds = new uint256[](_itemIds.current());
