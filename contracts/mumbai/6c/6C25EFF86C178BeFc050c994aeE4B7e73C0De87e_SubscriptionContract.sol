...
contract SubscriptionContract is Ownable {
    ...
    mapping(address => mapping(address => mapping(uint => Subscription)))
    ...
    function cancel(uint planId) external {
        Plan storage plan = plans[planId];
        Subscription storage subscriptionplan = subscriptions[plan.artist][
            msg.sender
        ][planId];
        require(
    ...
    function isSubscriber(
        ...
        require(
            subscriptions[artistAddress][_address][planId].subscriber !=
                address(0),
            "You need to subscribe first"
        );
        require(
            block.timestamp <
                subscriptions[artistAddress][_address][planId].nextPayment,
