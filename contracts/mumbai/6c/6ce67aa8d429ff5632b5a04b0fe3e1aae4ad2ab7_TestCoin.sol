...
contract TestCoin is IERC20, Pausable, AccessControl {
    ...
    address private _GNOSIS_SAFE;
    address public _swap_router_address_on_chain;
    string public _name;
    ...
    constructor(
        ...
        uint256 _testcoin_initial_supply,
        ...
        address _gnosis_wallet_address,
        address _swap_router_address_init
    ) {
        // _mint(GNOSIS, _testcoin_initial_supply * 10 ** decimals());
        _swap_router_address_on_chain = _swap_router_address_init;
        _GNOSIS_SAFE = _gnosis_wallet_address;
        _name = _testcoin_name;
        _symbol = _testcoin_symbol;
        _decimals = _testcoin_decimals;
        _tTotal = _testcoin_initial_supply;
        _rTotal = (MAX - (MAX % _tTotal));
        _grantRole(DEFAULT_ADMIN_ROLE, _GNOSIS_SAFE); 
        _grantRole(PAUSER_ROLE, _GNOSIS_SAFE);

        buyBackUpperLimit = _testcoin_initial_supply.mul(1).div(10000);

        _rOwned[_GNOSIS_SAFE] = _rTotal;

        feeWallet = payable(_GNOSIS_SAFE);

        numTokensSellToAddToLiquidity = _testcoin_initial_supply.mul(1).div(1000);
        // Polygon router quickswap, change to BSC or ETH router
        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(_swap_router_address_on_chain);
        ...
        _isExcludedFromFee[_GNOSIS_SAFE] = true;
        // _isExcludedFromFee[_GNOSIS_SAFE] = true;
        ...
        emit Transfer(address(0), _GNOSIS_SAFE, _tTotal);
    }
    ...
    // Increase the allowance of a certain wallet. 
    // whenNotPaused stops the function from working when paused using Open Zeppelin Access Control.
    // Require the role of ADMIN from the Open Zeppelin _GNOSIS_SAFE safe wallet
    // This requires 2 or more authorizations
    // Dual approvals might be redundant, next step, remove _approve()    
    function increaseAllowance(address spender, uint256 addedValue) public whenNotPaused virtual returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You don't have access to increase the allowance.");
        _approve(_msgSender(), spender, _allowances[_msgSender()][spender].add(addedValue));
        return true;
    }
    // Decrease the allowance of a certain wallet. 
    // whenNotPaused stops the function from working when paused using Open Zeppelin Access Control. 
    // Require the role of ADMIN from the Open Zeppelin _GNOSIS_SAFE safe wallet
    // This requires 2 or more authorizations
    // Dual approvals might be redundant, next step, remove _approve() 
    function decreaseAllowance(address spender, uint256 subtractedValue) public whenNotPaused virtual returns (bool) {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "You don't have access to decrease the allowance.");
        _approve(
            _msgSender(),
            spender,
    ...
    function _approve(
        ...
        // Require the admin to be able to approve anything 
        // 
    ...
    function recoverToken(address tokenAddress, uint256 tokenAmount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IERC20(tokenAddress).transfer(_GNOSIS_SAFE, tokenAmount);
    }
}