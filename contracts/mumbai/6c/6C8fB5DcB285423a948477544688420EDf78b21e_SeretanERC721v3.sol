...
import "./SeretanMintable.sol";
import "./SeretanERC2981.sol";
import "operator-filter-registry/src/UpdatableOperatorFilterer.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/ERC721Enumerable.sol";

contract SeretanERC721v3 is SeretanMintable, SeretanERC2981, UpdatableOperatorFilterer, ERC721Enumerable {
  ...
  function _safeMint(
    ...
    internal
    override(ERC721, SeretanMintable)
  {
  ...
  function owner() public view override(Ownable, UpdatableOperatorFilterer, SeretanMintable) returns (address) {
    return super.owner();
  }
}
...
contract SeretanMinter is PullPayment {
  struct Phase {
    uint256 startTime;
  ...
  function setPhaseList(
    ...
    public
  {
    require(msg.sender == collection || msg.sender == SeretanMintable(collection).owner());
    ...
    internal
  {
    ...
    _asyncTransfer(SeretanMintable(collection).owner(), msg.value);

    uint256 tokenId = nextTokenId[collection];
    nextTokenId[collection]++;

    SeretanMintable(collection).safeMint(to, tokenId);
  }
}
...
import "./SeretanMinter.sol";

abstract contract SeretanMintable {
  ...
  {
    minter = minter_;
    SeretanMinter(minter).setPhaseList(address(this), phaseList_);
  }
