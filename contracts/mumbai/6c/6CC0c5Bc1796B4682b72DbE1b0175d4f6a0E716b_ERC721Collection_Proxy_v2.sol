...
import "./common/StateSetter.sol";

contract ERC721Collection_Proxy_v2 is
    StateSetter,
    OwnableUpgradeable
{
    ...
    // Initialize the initial data of a contract
    function initialize(
        address _owner,
        string memory _name,
    ...
    function mintToken(address _receiver) external onlyRole(MINTER_ROLE) {
        ...
        uint256 _tokenId = tokenMinted();
        require(_tokenId < TOKEN_SUPPLY, "Limit Reached");
        _safeMint(_receiver, _tokenId);
        tokenId.increment();
    }

    // Every marketplace looks for this function to get the uri of a token
    ...
    function tokenSupply() external view returns (uint _tokenSupply) {
        _tokenSupply = TOKEN_SUPPLY;
    }
...
abstract contract StateSetter is AccessControlUpgradeable, ERC2981, ERC721Upgradeable {
    ...
    // Can be used to set contract level metadata (required for opensea to set royalty fee)
    ...
    // to set or update royalty fee of a specific token id
    ...
    // to reveal the actual NFT. DEFAULT_VALUE: FALSE
    function setReveal(bool _revealed) external onlyRole(ADMIN_ROLE) {
        // require(revealed != _revealed, "Value should not be same");
        revealed = _revealed;
    }
    ...
    // to set or update the uri of a specific token id
