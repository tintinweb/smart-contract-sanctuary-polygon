...
import "./2StepOwnableUpgradeable.sol";
...
// token transfers
// ----------------------------------------------------------------------------
contract OnePlanetCarbonOffsetChildV6 is ERC20Interface, OwnableUpgradeable, UUPSUpgradeable {
	...
	address payable public childChainManagerProxy;
    mapping(address => uint) balances;
    mapping(address => mapping(address => uint)) allowed;
    uint256 public retireMax;
    ERC721Interface internal checkApproved;
...
function initialize() public initializer {
		...
		oracleAddress = payable(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0); //Polygon MATIC/USD address
		retireAddress = 0x0000000000000000000000000000000000000000;
		daiAddress = payable(0x0715A7794a1dc8e42615F059dD6e406A6594651A); //Polygon address
		priceFeed = AggregatorV3Interface(oracleAddress);
        gasCO2factor = 380000000000;
        __Ownable_init();
        
        //Matic Constructor Arguments
        childChainManagerProxy = payable(0xA6FA4fB5f76172d178d61B04b0ecd319C5d1C0aa);
        tetherAddress = payable(0xc2132D05D31c914a87C6611C10748AEb04B58e8F); //Polygon address
        usdcAddress = payable(0xdA5289fCAAF71d52a80A254da614a192b693e977); //Polygon address
    ...
    // Address on Matic: 0xcB1e72786A6eb3b44C2a2429e317c8a2462CFeb1
    //-------------------------------------------------------------------------------------------
    ...
    // Address on Matic: 0xeaBc4b91d9375796AA4F69cC764A4aB509080A58
    //-------------------------------------------------------------------------------------------
    ...
    // Address on Matic: 0xdA5289fCAAF71d52a80A254da614a192b693e977
    //-------------------------------------------------------------------------------------------
    ...
    function offsetDirect(string calldata message) external payable returns (bool success) {
        ...
        require(msg.value > 0, "You need to send at least some MATIC");
    ...
    //-------------------------------------------------------------------------------------------
    // Enables smart contracts to spend 1PLCO2 from a user's wallet that initiated the call on
    // that smart contract.
    // Verify transaction details at www.1PLANET.app
    //-------------------------------------------------------------------------------------------
    ...
    // MAKE THIS EXTERNAL FOR THE MAINNET VERSION
    function isApprovedAndOffset(address spender, address operator, uint tokens, string calldata message) external returns (bool success) {
        checkApproved = ERC721Interface(address(msg.sender));
        require(checkApproved.isApprovedForAll(spender, operator) == true, "Operator is NOT Approved to Transfer Tokens");  
        require(tokens <= retireMax, "1PLCO2 retirement too high for normal ecoNFT use."); //safety check
        _beforeTokenTransfer(spender, address(0), tokens);
        balances[spender] -= tokens;
        _totalSupply -= tokens;
        emit Transfer(spender, address(0), tokens);
        emit CarbonOffset(message);
        return true;   
    }
    ...
    function setDAIaddress(address payable newAddress) public onlyOwner {
        daiAddress = newAddress;
    }

    function setUSDCaddress(address payable newAddress) public onlyOwner {
        usdcAddress = newAddress;
    }

    function setUSDTaddress(address payable newAddress) public onlyOwner {
        tetherAddress = newAddress;
    }
    ...
    function getLatestPrice() public {
        ...
        ethPrice = uint(price) / (1e8 / sigDigits);
    ...
    //------------------------------------------------------------------------------------------------
    // Safety check for users in case of malicious/scam NFT marketplace operators. Initially 10 1PLCO2
    //------------------------------------------------------------------------------------------------   
    function setRetireMax(uint256 maximum) external onlyOwner {
        retireMax = maximum;
    }
    ...
    // Matic L2 Bridge functions
     ...
     // ------------------------------------------------------------------------
    // Matic Child Contract Functions
    // ------------------------------------------------------------------------
    // being proxified smart contract, most probably childChainManagerProxy contract's address
    // is not going to change ever, but still, lets keep it 
    
    function updateChildChainManager(address payable newChildChainManagerProxy) external onlyOwner {
