...
contract JuanFourRafCon is VRFConsumerBaseV2 {
    ...
    event FinishedGame(uint room_number,address[] players);

    event RakeTransfer(uint room_number,uint amount, address s_owner); 
    ...
    event NewLease(uint room_number,address lessee,uint newEntryfee,uint pot,uint playerCount,bool active,uint grandPrize);
    ...
    function hostARaffle(uint _amount, uint newEntryfee, uint room_number, uint grandPrize,uint limit) public {
        ...
        emit NewLease(room_number,msg.sender,newEntryfee,raffle_room[room_number].pot,raffle_room[room_number].s_players.length,true,raffle_room[room_number].prizes[0]);
