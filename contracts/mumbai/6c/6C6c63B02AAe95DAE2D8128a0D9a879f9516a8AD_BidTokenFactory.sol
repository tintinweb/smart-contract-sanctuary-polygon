...
contract BidTokenFactory is Initializable {
    using Array for address[];

    address public whitelistContractAddress;
    address[] private nftAddresses;

    event BidTokenContractCreated(address nftContractInstance);
    ...
    // constructor for BidToken contract deployment
    function createBidToken(
        ...
        BidToken bidTokenInstance = new BidToken(nftName, nftSymbol, whitelistContractAddress, fnftAddress, uri, expDate);
        //Add new instance to array
        nftAddresses.push(address(bidTokenInstance));
        //emit event of new instance created
        emit BidTokenContractCreated(address(bidTokenInstance));
    }

    function getBidContractInstance(uint256 index) external view returns (address) {
        require(index >= 0 && index < nftAddresses.length, "Invalid NFT contract identifier");
        return nftAddresses[index];
    }

    function isDeployed(address bidContractInstance) external view returns (bool) {
        return nftAddresses.contains(bidContractInstance);
    }

    function countNftInstances() external view returns (uint256) {
        return nftAddresses.length;
    }

    function getAllNftInstances() external view returns (address[] memory) {
        return nftAddresses;
    }

    function _removeFromArray(uint256 _index) external onlyAdmin {
        require(_index < nftAddresses.length, "index out of bound");

        for (uint256 i = _index; i < nftAddresses.length - 1; i++) {
            nftAddresses[i] = nftAddresses[i + 1];
        }
        nftAddresses.pop();
    }

    function _pushToArray(address _contract) external onlyAdmin {
        nftAddresses.push(address(_contract));
    }
}
...
contract BidToken is ERC1155Supply {

    address public whitelistContractAddress;
    address public fnftAddress;
    ...
    constructor(string memory _name, string memory _symbol, address _whitelistContractAddress, address _fnftAddress, string memory _uri, uint256 _expDate) ERC1155(_uri) {
        name = _name;
        symbol = _symbol;
        whitelistContractAddress = _whitelistContractAddress;
        fnftAddress = _fnftAddress;
        expDate = _expDate;
    }
    ...
    /// @notice Decimals are not naitively supported in ERC1155. In the frontend, the user should be aware
    // that purchasing 1 bid token = 0.01 hcx token, and 100 bid tokens = 1.00 hcx token. 100 bid tokens
    // should be represented with 2 decimals, like 1.00 bid tokens.
    function mint(address _to, uint256 _id, uint256 _amount) public onlyAdmin {
        _mint(_to, _id, _amount, bytes(""));
    }

    function mintBatch(address _to, uint256 []memory _ids, uint256[] memory _amounts) public onlyAdmin {
