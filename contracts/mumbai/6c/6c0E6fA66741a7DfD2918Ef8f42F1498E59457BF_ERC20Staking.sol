...
contract ERC20Staking is ReentrancyGuard {
    ...
    // mapping for the stakingId and address.
    ...
    function stake(uint256 _stakingId, uint256 _amount) external nonReentrant {
        ...
        // Create StakedToken
        StakedToken memory stakedToken = StakedToken(msg.sender, true, _stakingId, _amount, 0, block.timestamp + 999999999, block.timestamp);
    ...
    function fulfill(uint256 _stakingId, uint256 _rewardRate, uint256 _duration) external{
        ...
        address staker = stakerAddress[_stakingId];
        Staker memory _thisStaker = stakers[staker];
        for (uint256 m = 0; m < _thisStaker.stakedTokens.length; m++){
            if (_thisStaker.stakedTokens[m].stakingId == _stakingId && _thisStaker.stakedTokens[m].staked == true){
                stakers[staker].stakedTokens[m].rewardRate = _rewardRate;
                stakers[staker].stakedTokens[m].duration = _duration;
            }
        }
    }
    ...
    function withdraw(uint256 _stakingId) external nonReentrant {
        ...
        require(stakerAddress[_stakingId] == msg.sender, "You don't own this staking ID!");
        ...
        for (uint256 i = 0; i < _thisStaker.stakedTokens.length; i++) {
            if (
                _thisStaker.stakedTokens[i].stakingId == _stakingId 
                && 
       ...
       uint256 withdrawAmount = stakers[msg.sender].stakedTokens[index].amount;
        ...
        stakerAddress[_stakingId] = address(0);
    ...
    function calculateAllRewards(address _staker) internal view returns (uint256){
        ...
        for (uint256 k = 0; k < _thisStaker.stakedTokens.length; k++){
            ...
            uint256 thisamount = _thisStaker.stakedTokens[k].amount;
            uint256 thisRewardRate = _thisStaker.stakedTokens[k].rewardRate;
            uint256 thisDuration = _thisStaker.stakedTokens[k].duration;
            uint256 thisStakingStartDate = _thisStaker.stakedTokens[k].stakingStartDate;

            // Calculate the APY to rewardsPerHour
            uint256 rewardsPerHour = (thisamount * thisRewardRate) / 8760;
            // Setup amount variable
            uint256 amount = 0;

            if (block.timestamp > thisStakingStartDate + thisDuration) {
                // If the staking is ended
                amount = (thisStakingStartDate + thisDuration - _thisStaker.timeOfLastUpdate) * rewardsPerHour / 3600;
                rewards += amount;
            } else {
                // If the staking is not ended
                amount = (block.timestamp - _thisStaker.timeOfLastUpdate) * thisRewardRate / 3600;
                rewards += amount;
            }
        }
        return rewards;
