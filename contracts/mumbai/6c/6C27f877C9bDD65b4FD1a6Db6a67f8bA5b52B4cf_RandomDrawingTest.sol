...
/**
 * @title ViciAccess
 * @author Josh Davis <[email protected]cinft.com>
 */
...
contract RandomDrawingTest is MockRandomDrawing {
	...
	OldNFTContract public constant HODLERS = OldNFTContract(
    ...
    function loadTestContestants() public onlyOwnerOrRole(OOMPA_LOOMPA) {
        require(factoryState == STATE.INITIAL, "Invalid state");
        uint256 entrantsCount = HODLERS.distinctTokensMinted();
        require(entrantsCount > 0, "No entrants");
        
        for (uint256 eachToken = 0; eachToken < entrantsCount; eachToken++) {
        	if (!HODLERS.exists(eachToken)) {
        		continue;
        	}
        
        	address tokenOwner = HODLERS.ownerOf(eachToken);
            if (isIneligible(tokenOwner)) {
                continue;
            }
    ...
    function _selectNextWinner(uint256 _randomNumber) internal override {
        uint256 entrantsCount = nftDrawingPool.length();
        uint256 winningToken = nftDrawingPool.at(_randomNumber % entrantsCount);
        address nextWinner = HODLERS.ownerOf(winningToken);
