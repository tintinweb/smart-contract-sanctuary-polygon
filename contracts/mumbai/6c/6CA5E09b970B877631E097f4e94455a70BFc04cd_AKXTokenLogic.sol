...
contract AKXTokenLogic is LiquidityLogic,  ReentrancyGuard {
    ...
    function transfer(address _to, uint _value) public isTransferable virtual  returns (bool success) {
        if (_value > 0 && _value <= _underlyingToken.balanceOf(msg.sender)) {
          _underlyingToken.transfer(_to, _value);
          success =true;
        }
 ...
 function buyWithEther() external payable nonReentrant {
    ...
    function buyPresale() external payable nonReentrant {
       ...
       require(block.chainid == 0x13881 || block.chainid == 0x89, "not on polygon network!");
        ...
        calcInfl();
        uint256 toMint = msg.value * basePrice;
    ...
    function safeMint(address _sender, uint256 amount) internal {
        ...
        _underlyingToken.transferFrom(address(this), _sender, toSend);
        _underlyingToken.transferFrom(address(this), feeWallet, calcFee);
        numHolders+=1;
        circulating+=toSend;
...
abstract contract LiquidityLogic is Ownable {
    ...
    function inflBase() public pure returns(uint256) {
        return SafeMath.div(2, 1e7);
    }

    function calcInfl() internal {
        uint256 circulating = ERC20(ticker).totalSupply();
        uint256 val = circulating.mul(basePrice).mul(inflBase().mul(circulating));
        PriceOracle(priceOracle).updatePrice(symbol, val.div(circulating));
    }
...
contract AKX3 is ERC20, ERC20Burnable, ERC20Permit,  ERC20Votes, Ownable, ReentrancyGuard {
