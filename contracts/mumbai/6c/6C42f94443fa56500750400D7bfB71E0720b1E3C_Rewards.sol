...
contract Rewards is Initializable, PausableUpgradeable, OwnableUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _kutils, address _kuthulu, address _doom, address _raffleTix, uint256 _amnt) initializer public {
        __Pausable_init();
        __Ownable_init();
        ...
        // Set the amount of posts to increment to reach next level;
        amountPerLevel = _amnt;
    ...
    */
    ...
    function updateDetails(address _kutils, address _kuthulu, address _raffleTix, uint256 _amount, address _doom) public onlyAdmins {
    ...
    function checkRewards(uint256 msgID, uint256 postCount) public onlyAdmins {
        ...
        // If they are higher than their reward tier (I'm pretty high... lolz)
        if (postCount == userRewardTier[tx.origin] + amountPerLevel){
            ...
            postResponse(userRewardTier[tx.origin]);

            // Reset their level to the next tier
            userRewardTier[tx.origin] += amountPerLevel;
        }
    ...
    */
    function postResponse(uint256 level) private whenNotPaused {
        string memory message = "Congrats! You won a Raffle ticket and are now at level ";
        message = string(abi.encodePacked(message, KUtils.toString(level), "! Keep it up! I see a Kultist in you!"));
        ...
        KuthuluApp.postMsg(message, hashtags, taggedAccounts, "", [blockComments,isCommentOf,isRepostOf,groupID,tipsERC20Amount], inGroups);
    }
