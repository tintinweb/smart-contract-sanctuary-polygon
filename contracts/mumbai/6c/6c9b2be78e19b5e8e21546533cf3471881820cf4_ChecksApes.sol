...
contract ChecksApes is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint private constant MAX_PUBLIC = 2222;
    uint private constant MAX_WL = 2222;

    uint public publicSalePrice = 0 ether;
    uint public wlSalePrice = 0.004 ether;
    ...
    uint public maxMintAmountPerWhitelist = 5; 

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, address _signerAddressWL,string memory _baseURI) ERC721A("CheckApes", "CHECKSAPES")
    ...
    function WLMint(uint _quantity, bytes calldata signature) external payable {
        ...
        if(mintedAmountNFTsperWalletWhitelistSale[msg.sender] + _quantity > maxMintAmountPerWhitelist) revert("Max for Whitelist Sale exceeded");
            
        mintedAmountNFTsperWalletWhitelistSale[msg.sender] += _quantity;
        if(mintedAmountNFTsperWalletWhitelistSale[msg.sender] != 0){
           _mint(msg.sender, _quantity);
        } else {
            
            if(totalSupply() < 3000){
                uint newQuantity = _quantity + 1; // 1 Free!
                _mint(msg.sender, newQuantity);
            } else{
                _mint(msg.sender, _quantity);
            } 
        }
    ...
    function currentState() external view returns (Step, uint, uint, uint, uint) {
        return (sellingStep, publicSalePrice, wlSalePrice, maxMintAmountPerPublic, maxMintAmountPerWhitelist);
    }
