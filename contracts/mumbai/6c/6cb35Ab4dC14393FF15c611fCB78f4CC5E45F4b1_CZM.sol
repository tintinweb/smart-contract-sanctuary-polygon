...
contract CZM is ERC20, Ownable {
    ...
    constructor() ERC20("CycleZero Meme", "CZM") {
        ...
        uint256 rewardAllocationPercentage = totalSupply() * 10 / 100; // 10% of the total supply
        _transfer(msg.sender, address(this), rewardAllocationPercentage);
    }
    ...
    function getRewardAllocationPercentage() public view returns (uint256) {
        return totalSupply() * 10 / 100; // 10% of the total supply
    }
    ...
    function aiFunction() public view returns (string memory) {
        if (aiEnabled) {
            ...
            if (decentralizedAge < D_WEEK_DURATION) {
                return "Welcome to the TimeVerse! You are in the first week of CycleZero.";
            } else if (decentralizedAge < D_MONTH_DURATION) {
                return "Congratulations! You have completed the first week of CycleZero.";
            } else if (decentralizedAge < D_YEAR_DURATION) {
                return "Great progress! You have completed the first month of CycleZero.";
            } else {
                return "Keep up the good work! You have completed the first year of CycleZero.";
            }
        } else {
    ...
    function userBirthdateFunction() public view returns (string memory) {
        uint256 decentralizedAge = getDecentralizedAge();
        uint256 birthYear = userProfiles[msg.sender].birthdate / D_YEAR_DURATION;

        if (decentralizedAge < D_MONTH_DURATION) {
            return "Welcome to the TimeVerse! You are in the first month of your journey.";
        } else if (decentralizedAge >= (birthYear + 1) * D_YEAR_DURATION) {
            uint256 rewardAmount = (balanceOf(msg.sender) * 2) / 100; // 2% of the user's CMZ token balance
            return string(abi.encodePacked("Congratulations! You have completed one year in the TimeVerse. You receive a reward of ", uintToString(rewardAmount), " CMZ tokens."));
        } else {
            return "Keep up the good work! Your journey continues in the TimeVerse.";
        }
    }
