...
contract GameController is OwnableUpgradeable, IGameController, IStruct {
        ...
        returns (uint256, uint256, uint256, uint256, uint256, uint256, uint256)
    {
        ...
        InfoNextRoll memory nextRoll = gameSupport.getInfoNextRoll(id);
        _reset(id);
        return (
            uint256(nextRoll.numRes),
            uint256(nextRoll.numItem),
            uint256(nextRoll.shop2Res),
            uint256(nextRoll.skipResShop),
            uint256(nextRoll.option),
            uint256(nextRoll.ratio),
            0
        );
    }
    ...
    function _addition(uint256 id, int256 coin) internal {
        if (
            coin % 3 == 0 &&
    ...
    function _reset(uint256 id) internal {
        ...
        uint256 length = statMapResources[id].length();
        while (length > 0) {
            (uint256 key, ) = statMapResources[id].at(0);
            statMapResources[id].remove(key);
            --length;
        }
    }
...
contract GameSupport is OwnableUpgradeable, IGameSupport {
    ...
    function _checkFrozen(uint256 id) internal {
        if (
            IGameController(baseLogic.gameController()).countResource(
                id,
                uint256(Resources.ID.F4)
            ) > 0
        ) {
            uint256 count;
            for (uint256 i = 0; i < destroyedPosition[id].length(); ++i) {
                uint256 resId = IGameController(baseLogic.gameController())
                    .atPosition(id, destroyedPosition[id].at(i));
                if (
                    IGameResource(baseLogic.gameResource()).contains(
                        "fossillikes",
                        resId
            ...
            for (uint256 i = 0; i < removedPosition[id].length(); ++i) {
                uint256 resId = IGameController(baseLogic.gameController())
                    .atPosition(id, removedPosition[id].at(i));
                if (
                    IGameResource(baseLogic.gameResource()).contains(
                        "fossillikes",
                        resId
            ...
            if (count > 0) {
                uint16[] memory listPos = IGameController(
                    baseLogic.gameController()
                ).findPosition(id, uint256(Resources.ID.F4));
                for (uint256 i = 0; i < listPos.length; ++i) {
                    uint256 prop = IGameController(baseLogic.gameController())
                        .atPosition(id, listPos[i]);
                    if (
                        (prop == 1 &&
                            !destroyedPosition[id].contains(listPos[i])) ||
                        prop <= count * 5
                    ) {
                        destroyedPosition[id].add(listPos[i]);
                    } else {
    ...
    function updateNextOption(
        uint256 id,
        uint8 option, //option = 0: bình thường, option = 1: rare + veryRare, 2: veryRare, 3 : uncommon and better
        uint16 ratio
    ) external override onlyLogicContract {
    ...
    function reset(uint256 id) external override {
        uint256 length = destroyedPosition[id].length();
        while (length > 0) {
            destroyedPosition[id].remove(destroyedPosition[id].at(0));
            --length;
        }
        length = removedPosition[id].length();
        while (length > 0) {
            removedPosition[id].remove(removedPosition[id].at(0));
            --length;
        }
    }
    ...
    function getInfoNextRoll(
        uint256 id
    ) external view override returns (IStruct.InfoNextRoll memory) {
        return info[id];
    }
...
interface IGameSupport {
    ...
    function updateNextOption(uint256 id, uint8 option, uint16 ratio) external;
    ...
    function getInfoNextRoll(
        uint256 id
    ) external view returns (IStruct.InfoNextRoll memory);
...
contract DestroyLogic is Initializable, OwnableUpgradeable, IDestroy {
    ...
    function t5_destroy(
        ...
        uint256 length = destroyPos.length;
        uint256 rand = ((random / 1e37) % 10000) % length;
        uint256 resourceId = IGameController(baseLogic.gameController())
            .atPosition(id, destroyPos[rand]);
        while (resourceId == uint256(Resources.ID.T5) && length > 0) {
            rand = (rand + 1) % destroyPos.length;
            --length;
        }
        if (resourceId != uint256(Resources.ID.T5)) {
            baseLogic.addNewResource(id, resourceId, 1);
        }
    }
