...
contract Factory {
	...
	function deploy(bytes32 _salt, string memory name, string memory symbol, string memory tokenURIPrefix) external returns(address addr) {
		addr = address(new HeftyverseUserToken721{salt: _salt}(name, symbol, tokenURIPrefix));
		HeftyverseUserToken721 token = HeftyverseUserToken721(address(addr));
...
contract HeftyverseUserToken721 is ERC721 {
