...
contract VeryLongAnimalsMusic is
    ERC721,
    ...
    ERC2981
{
    ...
    constructor(string memory _uri, uint256 _maxSupply)
        ERC721("VeryLongAnimalsMusic", "VLAM")
    {
        setBaseTokenURI(_uri);
        setMaxSupply(_maxSupply);
        setRoyaltyInfo(_msgSender(), 1000); // 1000 == 10%
    }
    ...
    function airdrop(address[] calldata addList, uint256 _mintAmount)
        external
        onlyOwner
        mintCompliance(_mintAmount)
    {
        for (uint256 i = 0; i < addList.length; ) {
            address to = addList[i];
            require(
                amountMinted[to] + _mintAmount <= _mintLimitsPerAddress,
                "Mint limit reached"
            );
