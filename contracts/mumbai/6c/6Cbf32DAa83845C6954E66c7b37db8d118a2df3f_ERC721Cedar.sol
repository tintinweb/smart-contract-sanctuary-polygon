 ...
 * @notice The contract supports preminting and distribution. It suppoers user terms and checks the greenlist status before transfer
 * @author Monax Labs
 */

contract ERC721Cedar is Ownable, IERC721Cedar, ERC721URIStorage, TierManager {
    ...
    /// @dev This function mints 200 tokens per batch up to MAX_LIMIT. It sets the tier ID for the token ID and returns an array of the token IDs.
    ...
    function mintBatch(
        ...
        require(
            totalTokensPerTier[_tierId] + _mintCount <= MAX_LIMIT_PER_TIER[_tierId],
            "ERC721Cedar: Mint count exceeds tier token limit"
        );
        require(totalSupply() + _mintCount <= MAX_LIMIT, "ERC721Cedar: max limit exceeded, reverting batch call");
        totalTokensPerTier[_tierId] += _mintCount;
    ...
    function addTiersInfo(
        address receiver,
        uint96[] calldata _fee,
        uint256[] calldata _maxLimitPerTier
    ) external virtual onlyOwner returns (uint96) {
       uint96 latestTier = _addTiersInfo(receiver, _fee, _maxLimitPerTier);
       return latestTier;
    }
    ...
    function checkSignature(address _account, bytes calldata _signature) public view returns (bool) {
    ...
    function _beforeTokenTransfer(
        ...
        if (_to != owner()) {
            ...
            if (caller != owner() || _from != owner()) require(_to != address(0), "ERC721Cedar: Burn unauthorised");
        }
    }
    ...
    function checkGreenlist(address _operator) internal view {
        if (Address.isContract(_operator) && greenlistManager.isGreenlistOn()) {
    ...
    /// @notice gets the Royalty Info and override the ERC2981 function in order to get tier royalty info.
    /// @dev this function gets the tier royalty information from the token ID
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) public view virtual override returns (address, uint256) {
        uint96 tierId = _getTierByToken(_tokenId);
        RoyaltyInfo memory royalty = _getRoyaltyInfo(tierId);
    ...
    function upgradeBaseURI(string calldata baseURI_) external virtual override onlyOwner {
        baseURI = baseURI_;
    }
    ...
    function setOwnerDomain(string calldata _ownerDomain) external virtual override onlyOwner {
    ...
    function getAgreementStatus(address _address) public view returns (bool sig) {
        return termsAccepted[_address];
    }

    // The following functions are overrides required by Solidity.

    function supportsInterface(bytes4 interfaceId) public view override(ERC721, IERC165, ERC2981) returns (bool) {
...
interface IERC721Cedar is IERC165 {
    function mintBatch(
        uint256 _mintCount,
        uint96 _tierId,
        address _to
    ...
    function setOwnerDomain(string calldata _ownerDomain) external;
}
...
contract SignatureVerifier is EIP712, Ownable {
    ...
    function verifySignature(address _to, bytes memory _signature) public view returns (bool) {
...
contract GreenlistManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    event OperatorUpdated(address _newAddress);
    event Greenlist(bool _status);
    ...
    function setAspenOperator(address _operator) public onlyOwner {
        operator = _operator;
    ...
    function setGreenlistStatus(bool _status) public onlyOwner {
        greenlistStatus = _status;
        emit Greenlist(_status);
    }

    /// @notice delete Aspen Operator
    /// @dev this function will delete the address if Aspen is not greenlisted.
    function deleteAspenOperator(address _address) public onlyOwner {
        require(!greenlistStatus, "GreenlistManager: Aspen operator is greenlisted");
        delete operator;
...
abstract contract TierManager is ERC2981 {
    mapping(uint256 => uint96) _tokenIDToTierID;
    mapping(uint96 => RoyaltyInfo) _tierInfo;
    mapping(uint96 => uint256) public MAX_LIMIT_PER_TIER;
    mapping(uint96 => uint256) totalTokensPerTier;
    uint96 totalTiers;
    event TierInfo(uint96 tierId, address royaltyReceiver, uint96 fee, uint256 maxLimit);


    /// @dev this function takes an array of tier data and stores the royalty information and max token limit per tier.
    function _addTiersInfo(
        address receiver,
        uint96[] calldata _fee,
        uint256[] calldata _maxLimitPerTier
    ) internal returns (uint96){
        require(_fee.length == _maxLimitPerTier.length, "TierManager: tier and fee data must be of the same size");
        for (uint96 i = 0; i < _fee.length; i++) {
            RoyaltyInfo memory royalty = RoyaltyInfo(receiver, _fee[i]);
            totalTiers++;
            _tierInfo[totalTiers] = royalty;
            MAX_LIMIT_PER_TIER[totalTiers] = _maxLimitPerTier[i];
            emit TierInfo(totalTiers, receiver, _fee[i], _maxLimitPerTier[i]);
    ...
    /// @dev this function gets the royalty information for a tier ID
    /// @return the royalty receiver and percentage for a tier ID
    function _getRoyaltyInfo(uint96 _tierId) internal view returns (RoyaltyInfo memory) {
        return _tierInfo[_tierId];
    }
}
