...
interface IManager {
    ...
    function calculateTransferPassiveBrokerage(uint256 _passiveTransferBrokerage)
        external
        view
        returns (uint256);
}
...
contract Passive is ERC20, Ownable, ReentrancyGuard {
    // Basic details
    ...
    function invest(uint256 amount) external nonReentrant {
        ...
        require(
            ...
            "PassiveFund: Spending not allowed on usdc"
        );
    ...
    function swapTokenToDollor(uint256 _amount) private {
    ...
    // Method to redeem from fund.
    function redeem(uint256 amount) external nonReentrant {
        // Check for the balance
        require(
            balanceOf(msg.sender) >= amount,
            "PassiveFund: Redeem requested more than balance."
        );

        // swap the fund in dollor.
        swapTokenToDollor(amount);
    ...
    function _transfer(
        ...
        require(
            ...
            "PassiveFund: transfer amount exceeds balance"
        );

        // Calculate brokerage.
        uint256 brokerage = manager.calculateTransferPassiveBrokerage(amount);
        amount -= brokerage;

        // transfer the fund
        super._transfer(from, to, amount);

        // Transfer the brokerage to manager contract.
        super._transfer(from, address(manager), brokerage);
    }
}