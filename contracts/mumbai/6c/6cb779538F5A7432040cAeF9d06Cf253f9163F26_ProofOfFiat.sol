...
contract ProofOfFiat is AccessControl, ERC2771Context {
    ...
    /**
     @dev modifier to check if the sender is the default admin of PoF contract
     * Revert if the sender is not the admin
     */
    modifier onlyPoFAdmin() {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "PoF: Only Admin");
        _;
    }
     ...
     */
    function setNFTAddress(address _nftAddress) public onlyPoFAdmin {
        require(_nftAddress != address(0), "PoF: Invalid address");
    ...
    function mintNFT(
        ...
        require(
            ...
            "PoF: Only CREATOR with valid signature"
        );
        nftAddress.mintNFT(to, id, amount, data, tokenUri);
    }

    /**
     * @dev Public function to mint a single new token and approve PoF contract
     * Reverts if the caller is not trusted forwarder contract
     * @param to The address to which the newly minted tokens will be assigned.
     * Requirements -
     *   to cannot be a null address
     ...
     * Reverts if the caller is not PoF Admin
    ...
    function burnNFT(
        ...
        require(
            ...
            "PoF: Only token owner with valid signature can burn his tokens"
        );
    ...
    function creatorTransferNFT(
        ...
        require(
            ...
            "PoF: Only CREATOR with valid signature and sufficient balance"
        );
        require(!orderExists[orderId], "PoF: Repeated Order.");
        nftAddress.safeTransferFrom(from, to, id, amount, data);
        orderExists[orderId] = true;
    ...
    function approveAndTransferNFT(
        ...
        require(
            from == _msgSender(),
            "PoF: Only token owner with valid signature"
        );
    ...
    /**
     * @dev Public function to mint new tokens in batches and approve them to the PoF contract
     * Reverts if the caller is not trusted forwarder contract
     * Reverts if the length of ids and amounts is not equal
     * @param to The address to which the newly minted tokens will be assigned.
     * Requirements -
    ...
    function burnNFTBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                ...
                "PoF: Only token owner with sufficient balance can burn his tokens"
            );
        }
    ...
    function creatorTransferNFTBatch(
        ...
        require(
            ...
            "PoF: Only CREATOR valid signature"
        );
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                balanceOf(from, ids[i]) >= amounts[i],
                "PoF: Creator does not have sufficient balance to transfer"
            );
        }
        require(!orderExists[orderId], "PoF: Repeated Order.");
        nftAddress.safeBatchTransferFrom(from, to, ids, amounts, data);
