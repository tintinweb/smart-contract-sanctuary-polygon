...
contract FUN is ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, Ownable, IERC2981 {
    ...
    constructor(string memory collectionName, string memory collectionSymbol, uint96 _royaltyPercentage, address _allowedMinter) ERC721(collectionName, collectionSymbol) {
    ...
    function safeMint(address to, string memory _tokenURI, string memory terraTokenId) public {
        ...
        setRoyalty(tokenId, to, royaltyPercentage);
        
        emit BindNFT(tokenId, terraTokenId, to, _tokenURI);
    }

    function setRoyalty(uint256 tokenId, address creator, uint96 _royaltyPercentage) private {
        royaltyReceivers[tokenId] = creator;
        royaltyPerNFT[tokenId] = _royaltyPercentage;
    }

    function setRoyalty(uint96 _royaltyPercentage) private {
        royaltyPercentage = _royaltyPercentage;
    }
