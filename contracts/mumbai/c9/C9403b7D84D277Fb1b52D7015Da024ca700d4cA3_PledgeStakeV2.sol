 ...
 contract PledgeStakeV2 {
    ...
    function stake(uint256 _amount) external {
     ...
     function _stake(address _user, uint256 _amount) private {
        ...
        _calculateRoundRewards(msg.sender, _amount);
    }
    ...
    function _calculateRoundRewards(address _user, uint256 _index) private view returns (uint256) {
        ...
        for (uint256 i = 0; i < TOTAL_ROUNDS; i++) {
            if (startTime.add(ROUND_DURATION.mul(i.add(1))) <= block.timestamp) {
                uint256 rewardsPercentage = roundRewardPercentages[i];
                uint256 roundReward = amount.mul(rewardsPercentage).div(100);
