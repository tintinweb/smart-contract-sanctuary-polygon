...
contract BAGC is ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, BAGCCore {
    ...
    event Minted(address indexed to, uint256 indexed tokenId, uint256 indexed InvitationTokenId);
    ...
    event Locked(address indexed user, uint256 indexed tokenId, uint256 value);
...
contract BAGCCore is OwnableUpgradeable {
    ...
    function updateInvitationAddress(string memory invitationBaseURI_) public onlyOwner {
    ...
    function setPool(
        ...
        _poolInfo[poolId] = PoolInfo(status, _poolInfo[poolId].currentNum, maxNum, 0);
    }
...
interface IBAGC {
    ...
    function claimLock(uint256 tokenId, uint256 value) external;
}
