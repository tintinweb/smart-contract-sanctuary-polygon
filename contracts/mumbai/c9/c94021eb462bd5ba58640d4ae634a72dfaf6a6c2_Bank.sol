...
contract Bank is Ownable {
    ...
    event AddReward(address rewardToken, uint256 reward);
    event Claim(address indexed user, uint256 blockTimestamp, uint256 reward);
    event Paused();
    event UnPaused();
    ...
    constructor(IERC20 _staking, address _owner,  IERC20 _reward) public {
        require(address(_staking) != address(0x000), "zero address");
    ...
    function addReward(uint256 amount) public  returns (bool) {

        uint256 lpSupply = token.balanceOf(address(this));
        if (lpSupply == 0) {
            rewardToken.transferFrom(msg.sender, owner(), amount);
            return true;
        }
        ...
        emit AddReward( address(rewardToken), amount);
        return true;
    }
    ...
    function claim() public isPaused {
        UserInfo storage user = userInfo[msg.sender];
        address _userAddr = msg.sender;
        uint256 pendingReward;
        if (user.amount > 0) {
            pendingReward = user.amount.mul(accPerShare).div(1e12).sub(
                user.rewardDebt
            );
            safeRewardTransfer(_userAddr, pendingReward);
        }

        user.rewardDebt = user.amount.mul(accPerShare).div(1e12);

        emit Claim(_userAddr, block.timestamp, pendingReward);
    }
