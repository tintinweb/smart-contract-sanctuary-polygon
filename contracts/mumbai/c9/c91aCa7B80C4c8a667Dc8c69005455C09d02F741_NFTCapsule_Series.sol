...
contract NFTCapsule_Series is Pausable, INFTCapsule {
    ...
    function _buildTokenId(uint256 picId, uint256 fragmentId)
        public
        view
        returns (uint256)
    {
        ...
        returns (uint256 tokenId)
    {
        ...
        // 取得隨機編號
        uint256 picId = (uint256(
            sha256(
                abi.encodePacked(
                    uint160(msg.sender) + block.timestamp + totalSupply
                )
            )
        ) % picAmount) + 1;
        uint256 fragmentId = (uint256(
            sha256(
                abi.encodePacked(
                    uint160(msg.sender) + block.timestamp + picId + totalSupply
                )
            )
        ) % fragmentAmount) + 1;
    ...
    function crossChainSet(uint256 tokenId) external override {
        require(msg.sender == NFTCore, "Invalid Set FragmentMintAmount");

        uint256 seriesAndPicId = tokenId / 1e6;
        uint256 picId = seriesAndPicId % 1e6;
