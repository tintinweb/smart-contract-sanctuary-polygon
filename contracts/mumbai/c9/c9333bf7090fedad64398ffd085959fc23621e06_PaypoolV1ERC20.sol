...
contract PaypoolV1ERC20 is NonblockingLzAppUpgradeable, ERC20Upgradeable, PausableUpgradeable, IOFTUpgradeable {
    ...
    EnumerableSetUpgradeable.UintSet internal supportedChains;
    mapping (uint16 => bytes) internal chainToAddress;
    ...
    function addSupportedEvmChains(uint16[] calldata chainIds) external onlyOwner {
        for(uint256 i = 0; i < chainIds.length; i++) {
            if (!supportedChains.contains(chainIds[i])) {
                assert(supportedChains.add(chainIds[i]));
                chainToAddress[chainIds[i]] = toBytes(address(this));
            }
        }
    }

    function addSupportedChains(uint16 chainId, bytes calldata contractAddress) external onlyOwner {
        if(!supportedChains.contains(chainId)) {
            assert(supportedChains.add(chainId));
            chainToAddress[chainId] = contractAddress;
        }
    }

    function removeSupportedChains(uint16[] calldata chainIds) external onlyOwner {
        for(uint256 i = 0; i < chainIds.length; i++) {
            if (supportedChains.contains(chainIds[i])) {
                assert(supportedChains.remove(chainIds[i]));
                delete chainToAddress[chainIds[i]];
            }
        }
    }
