...
/**
 * @title wav3shackFollowerOnlyReferenceModule
 * @author Daniel Beltrán for wav3shack
...
contract wav3shack {
    ...
    constructor(uint256 fee, address wmatic /*, address wav3s , address wav3sHub*/) {
        i_fee = fee;
        i_wMatic = wmatic;
        owner = msg.sender;
    }
    ...
    function processMirror(string memory pubId, address mirrorerAddress) external {
        // Get the currency for the post pointed to
        uint256 budget = dataByPublication[pubId].budget;
        // Get the reward for the post pointed to
        uint256 reward = dataByPublication[pubId].reward;
        // Get the currency for the post pointed to
        address currency = dataByPublication[pubId].currencyAddress;

        // Check if the mirrorer is the wallet owner
        if (msg.sender != s_wav3sHub) {
            revert('wav3s__OnlyWav3sCanCallThisCleverBoi()');
        }

        // Check if the follower has already mirrored this post
        if (s_publicationToFollowerHasMirrored[pubId][mirrorerAddress]) {
            revert('wav3s__FollowerAlreadyMirrored()');
        }

        // Check that there is enough budget to pay the reward
        if (reward >= budget) {
            revert('wav3s__NotEnoughPostBudget()');
        }
    ...
    function fundMySuperReach(
        ...
        if (msg.sender != profileAddress) {
            revert('wav3s__SenderNotOwner()');
        }
        if (budget <= 0) {
            revert('wav3s__InvalidBudget()');
        }
        if (reward <= 0 || reward > dataByPublication[pubId].budget) {
            revert('wav3s__NotEnoughBudgetForThatReward()');
        }
        if (minFollowers <= 0) {
            revert('wav3s__InvalidMinimumFollowers()');
        }

        if (currencyAddress != i_wMatic) {
            revert('wav3s__OnlyWMatic()');
        }
        ...
        // Transfer half of the fees to the wav3s contract
    ...
    function getBudget(string memory pubId) public view returns (uint256) {
    ...
    function withdrawPostBudget(string memory pubId, uint256 amount) public {
