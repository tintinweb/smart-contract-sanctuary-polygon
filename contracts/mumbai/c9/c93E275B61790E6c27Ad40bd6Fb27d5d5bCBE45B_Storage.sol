...
{   
  ...
  {   
    ...
    function getAllFiles(address userAddress) public view returns(string[] memory)
    {
        ...
        for(uint i=0;i<fileCount;i++)
        {
          ...
          {
            if(fileList[i+1].userAddress[j]==userAddress)
            {
    ...
    function fetchUsers(address userAddress) public view returns(string[] memory)
    {
        string[] memory ret = new string[](userCount);
        for(uint i=0;i<userCount;i++)
        {
          if(usersList[i+1].userAddress!=userAddress)
          {
