...
contract RWACalculator is IRWACalculator, AccessControl {
        ...
        returns (uint256 usdValue)
    {
        ...
        for (uint256 i; i < categoriesTotal; i++) {
            ...
            for (uint256 j; j < fractionsTotal; j++) {
                ...
                tokenIds[0] = fData[j].tnftTokenId;
                (
                    pricesOracleArrays.weSellAt,
                    ,
                    ,
    ...
    function fetchPaymentTokenAndAmountFtnft(
        ...
        uint256 fractionId,
        uint256 share
    ) external view returns (IERC20 paymentToken, uint256 amount) {
    ...
    function fetchPaymentTokenAndAmountTnft(
        ...
        if (unminted) {
            ...
            uint256[] memory weSellAt = new uint256[](1);
            uint256[] memory lockedAmount = new uint256[](1);
            uint256[] memory fingerprints = new uint256[](1);
            fingerprints[0] = fingerprint;
            ITNFTPriceManager priceManager = _getPriceManager();
            (weSellAt, , , , lockedAmount) = priceManager
                .itemPriceBatchFingerprints(
                    ITangibleNFT(tnft),
                    paymentToken,
                    fingerprints
                );
            amount = weSellAt[0] + lockedAmount[0];
            //check for storage
