...
contract Manager is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function setPaymentMethod(
        ...
        require(
            _token != address(0),
            "Cannot set native token as payment method"
        );
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function createOrder(
        ...
        emit OrderCreated(_orderId, _tokenId, _price, _paymentToken, _expTime);
        return _orderId;
    }
    ...
    function createBid(
        ...
        require(
            IERC20Upgradeable(_paymentToken).balanceOf(msg.sender) >=
                _buyerCommissionOutput.amountToBuyer,
            "Balance not enough"
        );
        require(
            IERC20Upgradeable(_paymentToken).allowance(
                msg.sender,
                address(this)
            ) >= _buyerCommissionOutput.amountToBuyer,
    ...
    function acceptBid(uint256 _bidId) external whenNotPaused returns (bool) {
        ...
        uint256 amountToBuyer = bid.bidPrice +
            bid.commissionFromBuyer +
            bid.taxAmount;

        require(
            IERC20Upgradeable(bid.paymentToken).balanceOf(bid.bidder) >=
                amountToBuyer,
            "Balance not enough"
        );
        require(
            IERC20Upgradeable(bid.paymentToken).allowance(
                bid.bidder,
                address(this)
            ) >= amountToBuyer,
            "Allowance not enough"
    ...
    function updateBid(
        ...
        if (bid.bidPrice != _bidPrice) {
            ...
            require(
                IERC20Upgradeable(bid.paymentToken).balanceOf(msg.sender) >=
                    _buyerCommissionOutput.amountToBuyer,
                "Balance not enough"
            );
            require(
