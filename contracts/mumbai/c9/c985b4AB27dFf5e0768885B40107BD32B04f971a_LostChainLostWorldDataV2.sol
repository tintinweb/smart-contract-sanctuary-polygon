...
contract LostChainLostWorldDataV2 is CustomERC2771ContextUpgradeable, UUPSUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCLWD: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LCLWD: RBA linker is zero address');
    ...
    function initialize(LostChainForwarder forwarder, address rbaLinkerAddress) public initializer {
        require(address(forwarder) != address(0x0), 'LCLWD: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCLWD: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
    ...
    function addEnemy(
        ...
        string memory criticalChance,
        string memory attackSpeed,
        string memory cleaveChance,
        string memory image
    ) public onlyRole('MINTER', _msgSender()) {
        require(bytes(name).length > 0, 'LCLWD: Name cannot be empty');
        require(bytes(health).length > 0, 'LCLWD: Health cannot be empty');
        require(bytes(damage).length > 0, 'LCLWD: Damage cannot be empty');
        require(bytes(defense).length > 0, 'LCLWD: Defemse cannot be empty');
        require(bytes(criticalChance).length > 0, 'LCLWD: Critical chance cannot be empty');
        require(bytes(attackSpeed).length > 0, 'LCLCGDED: Attack speed cannot be empty');
        require(bytes(cleaveChance).length > 0, 'LCLWD: Cleave chance cannot be empty');
        require(bytes(image).length > 0, 'LCLWD: Image cannot be empty');

        uint256 enemyId = _enemyIdCounter.current();
        _enemyIdCounter.increment();
        require(!enemyList[enemyId].exists, 'LCLWD: Enemy already exists with that id');
    ...
    function getAllEnemy(bool includeDisabled) public view returns (Enemy[] memory) {
        ...
        require(maxEnemyId > 0, 'LCLWD: No enemy available');
        ...
        require(totalIndex > 0, 'LCLWD: No enemy available');
    ...
    function disableEnemy(uint256 enemyId) public onlyRole('MINTER', _msgSender()) {
        require(enemyList[enemyId].exists, 'LCLWD: Enemy does not exist');
        require(!enemyList[enemyId].disabled, 'LCLWD: Enemy is already disabled');

        enemyList[enemyId].disabled = true;
    }

    function enableEnemy(uint256 enemyId) public onlyRole('MINTER', _msgSender()) {
        require(enemyList[enemyId].exists, 'LCLWD: Enemy does not exist');
        require(enemyList[enemyId].disabled, 'LCLWD: Enemy is not disabled');
