    ...
    IERC1271Wallet
{
    ...
    function _subDigest(
        ...
        return
            keccak256(
                abi.encodePacked("\x19\x01", chainId, address(this), _digest)
            );
    }
...
abstract contract ModuleCalls is
    IModuleCalls,
    IModuleAuth,
    ModuleERC165,
    ModuleSelfAuth
{
     ...
     * @return The next nonce
     */
    function nonce() external view virtual override returns (uint256) {
        return readNonce(0);
    }
    ...
    function readNonce(
        ...
        return
            uint256(ModuleStorage.readBytes32Map(NONCE_KEY, bytes32(_space)));
    }
    ...
    function selfExecute(
        Transaction[] memory _txs
    ) public virtual override onlySelf {
        // Hash transaction bundle
     ...
     * @param _txs  Transactions to execute
     */
    function _execute(bytes32 _txHash, Transaction[] memory _txs) private {
        // Execute transaction
        for (uint256 i = 0; i < _txs.length; i++) {
            ...
            require(
                ...
                "ModuleCalls#_execute: NOT_ENOUGH_GAS"
            );
