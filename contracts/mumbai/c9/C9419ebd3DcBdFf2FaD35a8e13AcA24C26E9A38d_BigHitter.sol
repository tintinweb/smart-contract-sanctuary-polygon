...
contract BigHitter is Ownable, UriChanger, ERC721EnumMetadata, VerifyAttestation, ERC5169 {
    ...
    string constant _METADATA_URI = "https://resources.catchmax.io";
    string constant _CONTRACT_NAME = "Catch Max";
    string constant _CONTRACT_SYMBOL = "CATCHMAX";
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ERC721Enumerable, ERC5169) returns (bool) {
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor() ERC721EnumMetadata(_CONTRACT_NAME, _CONTRACT_SYMBOL, _METADATA_URI) UriChanger(_msgSender()) {
    ...
    function updateAttestationData(address attestor, address issuer, string memory conferenceId) public onlyOwner {
        require(attestor != address(0) && issuer != address(0), "Address required");
    ...
    function mintUsingAttestation(bytes memory attestation) public returns (uint256 tokenId) {
        ...
        (subject, tokenBytes, conferenceBytes, timeStampValid) = verifyTicketAttestation(
            attestation,
            attestorKey,
            issuerKey
        );
        ...
        require(
            subject != address(0) && tokenId != 0 && timeStampValid && _compareStrings(conferenceBytes, conferenceID),
    ...
    function mintDirect(
        ...
        if (erc20Contract_ == address(0)) {
            ...
            IERC20(erc20Contract_).safeTransferFrom(_msgSender(), beneficiary, price);
        }
    }
    ...
    function mintByOperator(MintData[] calldata data) public onlyUriChanger {
        for (uint i = 0; i < data.length; i++) {
            _mint(data[i].to, data[i].tokenId);
        }
    }
    ...
    function _compareStrings(bytes memory s1, string memory s2) private pure returns (bool) {
...
abstract contract ERC721EnumMetadata is ERC721Enumerable {
    ...
    ) ERC721(contractName_, contractSymbol_) {
    ...
    function contractURI() public view returns (string memory) {
        return string(abi.encodePacked(metadataURI, "/contract/", symbol(), _JSON_FILE));
    }
