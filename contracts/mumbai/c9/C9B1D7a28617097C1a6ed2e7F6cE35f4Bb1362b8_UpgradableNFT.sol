...
contract UpgradableNFT is ERC721URIStorage, ERC2981 {
    ...
    uint private randomCounter = 0;
    string[] private headgearPossibleValues;
    ...
    function getTokenURI(uint256 tokenId) view public returns (string memory){
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                '"description": "', _name, '",',
                '"image": "ipfs://bafkreiguqq34zrxbzyn6523vj5jx7miftfwimalqxmxmhhe2ankm7i4fqi",',
                '"external_url": "https://bafkreiguqq34zrxbzyn6523vj5jx7miftfwimalqxmxmhhe2ankm7i4fqi.ipfs.nftstorage.link/",',
                '"attributes": [',
                    '{',
                        '"trait_type": "Headgear",',
                        '"value": "', headgearPossibleValues[randInt(tokenId, headgearPossibleValues.length)], '"',
                    '}',
                ']',
            '}'
        );
    ...
    function randInt(uint256 seed, uint max) private view returns (uint) {
        return uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, seed))) % max;
    }
