...
contract CollectionProxy is ERC721Upgradeable{
    ...
    function initialize(address _marketplaceScAdr,
        address payable _collectionCreator ) public initializer returns (address) {
...
contract MarketplaceConfV2 {
    
string constant MARKETPLACE_CFG_VERSION = "2.0.0";
    ...
    function initialize(uint256 _listingFees, uint16 _royaltiesMarketplace) public {
    ...
    function setMarketplaceRoyalties(uint16 _royaltiesMarketplace) public onlyOwner {
        // Ensure that _royaltiesMarketplace are always under 100%
        require(_royaltiesMarketplace <= 5000, NFTErrors.ROYALTIES_RANGE_ERROR);
        royaltiesMarketplace = _royaltiesMarketplace;
    }
    ...
    function getMessage() public pure returns (string memory){
        return "MarketplaceConfVersion2";
    }
...
import "./CollectionProxyV2.sol";
import "./MarketplaceConfV2.sol";
import "./NFTErrors.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC721/ERC721Upgradeable.sol";
 ...
 * @dev A contract for managing non-fungible token marketplaces.
 */
contract MarketplaceProxy is Initializable, ERC721Upgradeable{
    string constant COLLECTION_VERSION = "1.0.0";
    ...
    event NewCollectionAdded(address indexed collectionAdr); 

    CollectionProxyInterface public collectionProxy;

    
    function initialize(address _marketplaceConfAdr) public initializer  {
    ...
    function getAddress (address _collectionProxy) public {
    collectionProxy = CollectionProxyInterface(_collectionProxy);
    }
    ...
    function newCreateCollection(string memory _collectionName, string memory _symbole) public returns (address) {
        ...
        collections[address(collectionProxy)] = true;

        // Emit a "NewCollectionAdded" event.
        emit NewCollectionAdded(address(collectionProxy));
        // Return the address of the newly created collection.
   ...
   /*
    ...
    function totalSupply(address _collectionAdr) public returns (uint256) {
        return CollectionProxy(_collectionAdr).totalSupply();
    }
    ...
    function getCurrentListingFees() public view returns (uint256) {
        return MarketplaceConfV2(marketplaceConfAdr).getListingFees();
    }
   */
}
