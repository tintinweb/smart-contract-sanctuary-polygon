...
contract SalvaICO {
    ...
    function invest() public payable returns (bool) {
        ...
        uint256 _amount = _valueSent / tokenPrice;

        uint256 _rightAmount = _amount * 10**18;

        // sending Salvacoin to buyer

        address _to = msg.sender;

        IERC20(salvaContract).transfer(_to, _rightAmount);

        // _sendTokens(_to, _rightAmount);

        emit Invest(_to, msg.value, _rightAmount);
    ...
    function _sendTokens(address _to, uint256 _amount) private {
        IERC20(salvaContract).transfer(_to, _amount);
    }
    ...
    function withdrawUnsoldTokens() external onlyAdmin returns (bool success) {
        ...
        address _to = msg.sender;
        uint256 _amount = IERC20(salvaContract).balanceOf(address(this));
        require(_amount > 0, "ICO: zero ERC20 balance.");
        IERC20(salvaContract).transfer(_to, _amount);
