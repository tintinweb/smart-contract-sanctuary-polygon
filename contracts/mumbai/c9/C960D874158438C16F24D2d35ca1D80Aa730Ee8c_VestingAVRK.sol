...
contract VestingAVRK is Ownable, ReentrancyGuard, Pausable {
     ...
     */
    mapping(address => RecipientInformation) private walletToVestAllocations;
        ...
        nonReentrant
    {
        ...
        RecipientInformation storage user = walletToVestAllocations[
            _msgSender()
        ];
        uint256 totalToPay;
    ...
    function updatePercentageRecipient(
        ...
        RecipientInformation storage user = walletToVestAllocations[_user];
    ...
    /**
    todo cek apakash vest start time sudah berjalan
     */
    ...
    function resetMonths() external onlyOwner {
        vestStartTime = block.timestamp - _daysInSeconds();
    }
    ...
    function resetUser(address _user) external onlyOwner {
        delete walletToVestAllocations[_user];
    }
}
