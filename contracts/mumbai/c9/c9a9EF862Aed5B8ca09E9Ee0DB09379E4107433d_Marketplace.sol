...
contract Marketplace is OwnableUpgradeable {
    ...
    event ListItem(uint256 indexed index, string category, string title, string imgHash, uint256 count, uint256 priceForBBOSS, uint256 priceForUSD);
    ...
    function listItem(string memory _category, string memory _title, string memory _imgHash, uint256 _count, uint256 _priceForBBOSS, uint256 _priceForUSD) public onlyOwner {
        require(_count > 0, "count must be over 0");
        items[index] = ItemInfo(_category, _title, _imgHash, _count, _priceForBBOSS, _priceForUSD);

        emit ListItem(index, _title, _category, _imgHash, _count, _priceForBBOSS, _priceForUSD);
        index = index + 1;
    }
