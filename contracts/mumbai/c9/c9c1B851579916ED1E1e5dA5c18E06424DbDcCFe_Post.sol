...
contract Post {
    ...
    address _owner;  
    PostStruct[] public listPosts;
    mapping(uint => PostStruct) public mappingPost;
    mapping(address => AuthType) accounts;
    
    using Counters for Counters.Counter;
    Counters.Counter private _postIds;

    
    event PostEvent(address createBy, uint id, string title, string typeEvent, uint time);

    event AccountEvent(address createBy, address account, AuthType AuthType, uint time);
    constructor () {
        _owner = msg.sender;
    }
    ...
    modifier onlyAdmin() {
        require(accounts[msg.sender] == AuthType.ADMIN, "You are not Amin");
        _;
    }

    modifier adminOrCensor() {
        require(accounts[msg.sender] == AuthType.ADMIN ||
        accounts[msg.sender] == AuthType.CENSOR, "You have not Admin or Censor");
        _;
    }

    function createPost(string memory _title, string memory _content, bool _publish) public adminOrCensor {
         _postIds.increment();
         uint postId = _postIds.current();
        PostStruct memory post = PostStruct(postId, _title, _content, _publish);

        listPosts.push(post);
        mappingPost[postId] = post;
        emit PostEvent(msg.sender, postId, _title, "created", block.timestamp);         
    }

    function updatePost(uint _id, string memory _title, string memory _content, bool _publish) public adminOrCensor {
        PostStruct memory post = mappingPost[_id];
        post.title = _title;
        post.content = _content;
        post.publish = _publish;
        listPosts[_id - 1] = post;
        emit PostEvent(msg.sender, _id, _title, "updated", block.timestamp);
    }
    ...
    function getPost(uint _id) public view returns(PostStruct memory) {
        require(mappingPost[_id].publish == true, "Has not post publish");
        return mappingPost[_id];
    }

    function deletePost(uint _id) public adminOrCensor {
         delete listPosts[_id];
         delete mappingPost[_id];
         
    }

    function addAccount(address _address, AuthType _authType) public onlyAdmin {
        require(accounts[_address] != AuthType.ADMIN, "You have not permission");
      accounts[_address] = _authType;
      emit AccountEvent(msg.sender, _address, _authType, block.timestamp);
    }
    
    function removeAccount(address _address) public onlyAdmin {
        require(accounts[_address] != AuthType.ADMIN, "You have not permission");
        delete accounts[_address];
    }

    function addAdmin(address _address) public onlyOwner {
      accounts[_address] = AuthType.ADMIN;
    }

    function checkAuth(address _address) public view returns(AuthType){
