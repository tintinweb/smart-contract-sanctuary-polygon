...
contract StableCoin6Decs is Ownable, ERC20 {
    using SafeMath for uint256;

    uint8 public _decimals;

    /**
     * @notice 6 decimals stablecoin token constructor
    ...
    constructor(string memory _name, string memory _sym, uint256 _initialSupply) ERC20(_name, _sym) {
        _decimals = 6;
        _mint(msg.sender, _initialSupply.mul(uint(1e6)));
    }
    ...
    function mint(uint256 _amount) external {
        _mint(msg.sender, _amount.mul(uint(1e6)));
    }
