   ...
   function buyToken() public payable{
       ...
       payable(owner()).transfer(msg.value);
        ...
        if(isExist){
            ...
            uint256 burnTokens = numberOfTokens.mul(burnFee).div(100);
            uint256 marketingTokens = numberOfTokens.mul(marketingFee).div(100);
            uint256 LPAmount = numberOfTokens.mul(LPFee).div(100);

            uint256 totalDeductedAmount = burnTokens.add(marketingTokens).add(LPAmount);
            uint256 transferableAmount = numberOfTokens.sub(totalDeductedAmount);

            _transfer(owner(),marketingAddress,marketingTokens);
            _transfer(owner(),LPAddress,LPAmount);
            _transfer(owner(),0x000000000000000000000000000000000000dEaD,burnTokens);
            _transfer(owner(),msg.sender,transferableAmount);
        }
   ...
   */
   ...
   function setDrop(uint256 _airdrop, uint256 _rewards) onlyOwner public returns(bool){
    ...
    function setBurnFees (uint256 _fee) public onlyOwner returns (bool) {
        require(_fee>0,"Fee must be greater then zero");
        burnFee=_fee;
        return true;
    }

    // function burnFees () public view returns (uint256) {
    //     return burnFee;
    // }
    ...
    // function marktingFees () public view returns (uint256) {
    //     return marketingFee;
    // }
    ...
    // function LPFees () public view returns (uint256) {
    //     return LPFee;
    // }
