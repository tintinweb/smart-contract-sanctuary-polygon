...
interface IAdmin {
    ...
    function assignMemberships(uint256[] calldata ticketsIds, uint256[][] calldata memberships) external;
    ...
    // @dev for retriving the memberships assigned to a ticket
 ...
 * @dev See https://github.com/Fanszoid/contracts/blob/main/src/Membership.sol
 */
...
/**
 * @title The Fanszoid's Tickets Marketplace
 * @dev The Fanszoid's Tickets Marketplace is a smart contract that allows you manage events and tickets.
...
contract TicketsMarketplace is Initializable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    modifier hasAllowedMembership(uint256 ticketId) {
        uint256[] memory allowedMemberships = IAdmin(adminAddress).membershipsAllowedForTicket(ticketId);
        bool allowed = false;
        for (uint256 i = 0 ; i < allowedMemberships.length ; i++){
            if(IMembership(membershipAddress).balanceOf(msg.sender, allowedMemberships[i]) > 0) {
                allowed = true;
                break;
            }
        }
        require(allowed, 'Need an allowed membership!');
        _;
    }
     ...
     *  @param seller The seller from whom would like to claim (should be a zero price Models.Offer setted)
    ...
    function claimFreeTicket(
        ...
        require(ITicket(ticketAddress).balanceOf(seller, ticketId) >= 1, 'Seller hasnt enough tickets.');
        require(ITicket(ticketAddress).balanceOf(claimer, ticketId) == 0, 'Claimer has one ticket.');
        require(offers[seller][ticketId].price == 0, 'Ticket is not free.');
        require(offers[seller][ticketId].amount >= 1, 'Not enough tickets for claim');

        ITicket(ticketAddress).safeTransferFrom(address(seller), claimer, ticketId, 1, '');
        offers[seller][ticketId].amount -= 1;

        emit TicketBought(ticketId, seller, claimer, 0, 1);
    }

     /**
     *  @dev Claim a ticket with a proper membership.
     *  @param ticketId The id of the ticket to be claimed
     */
    function claimTicketByMembership(
        uint256 ticketId
    ) external hasAllowedMembership(ticketId) whenNotPaused {
    ...
    function changeEventOwnerInTicketsForEvent(uint256 eventId, address newOwner) external onlyAdmin whenNotPaused {
        uint256[] memory ticket = eventTickets[eventId];
    ...
    function publishTicketsForOrganizer(
        ...
        BulkTicketsInfo memory bulkInfo = BulkTicketsInfo(new uint256[](ticketsLength), new uint256[](ticketsLength), new string[](ticketsLength));
        ...
        if (memberships.length > 0) {
            IAdmin(adminAddress).assignMemberships(bulkInfo.newTicketIds, memberships);
        }
