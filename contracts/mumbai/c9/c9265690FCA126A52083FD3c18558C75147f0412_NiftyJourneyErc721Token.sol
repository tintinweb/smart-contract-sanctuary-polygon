...
contract NiftyJourneyErc721Token is NiftyERC721Token {
    uint88 internal maxTouchedTokenId;
    string internal constant ERROR_INPUT_AMOUNT_ZERO =
        "Amount of nifties is zero needs to be more";
    string internal constant ERROR_IPFSH_HASH_EMPTY = "ipfsHash is empty";

    function mint(uint88 amount, string calldata ipfsHash) external {
        _requireOnlyValidSender();

        require(amount > 0, ERROR_INPUT_AMOUNT_ZERO);
        require(bytes(ipfsHash).length > 0, ERROR_IPFSH_HASH_EMPTY);
        ...
        uint88 nextIdCounter = lastNiftyType.idLast + 1;
        uint88 firstNewTokenId = nextIdCounter;
        uint88 lastIdCounter = 0;
        if (lastNiftyType.niftyType == 0) {
            lastNiftyType.niftyType = 1;
        }
        uint72 niftyType = lastNiftyType.niftyType;
        ...
        uint88 newlyMinted = lastIdCounter - lastNiftyType.idLast;

        balances[to] += newlyMinted;

        lastNiftyType.idLast = lastIdCounter;

        collectionStatus.amountCreated += newlyMinted;
    ...
    function burnUnusedTokens() public {
        uint88 amountMinted = collectionStatus.amountCreated;
        uint88 newMaxTokenId = maxTouchedTokenId + 1;
        ...
        collectionStatus.amountCreated = maxTouchedTokenId;
    }
    ...
    function _updateMaxTouched(uint88 tokenId) internal {
...
contract NiftyERC721Token is ERC721Omnibus, Royalties, Signable, Withdrawable, Ownable, NiftyMetadata, IERC2309 {    
...
abstract contract NiftyMetadata is NiftyPermissions, NiftyTypes {        
    ...
    function setIPFSHash(uint256 niftyType, string memory ipfsHash) external {
        _requireOnlyValidSender();
        require(bytes(niftyTypeIPFSHashes[niftyType]).length == 0, ERROR_IPFS_HASH_ALREADY_SET);
        niftyTypeIPFSHashes[niftyType] = ipfsHash;        
    }
    ...
    function setMetadataGenerator(address metadataGenerator_) external {  
        ...
        if(metadataGenerator_ == address(0)) {
            ...
            require(IERC165(metadataGenerator_).supportsInterface(type(IMetadataGenerator).interfaceId), ERROR_INVALID_METADATA_GENERATOR);
            metadataGenerator = IMetadataGenerator(metadataGenerator_);
        }        
    }
}
...
abstract contract Signable is NiftyPermissions {        
    ...
    function setSigner(address signer_, bytes32 saltedHash_) external {
        _requireOnlyValidSender();
        _validateSignerPreparation(signatureStatus.isVerified, signer_, saltedHash_);        
        signatureStatus.signer = signer_;
        signatureStatus.saltedHash = saltedHash_;
        signatureStatus.isSalted = true;
    }

    function addOrUpdateMultiSigner(address signer_, bytes32 saltedHash_) external {
        _requireOnlyValidSender();
        _validateSignerPreparation(multiSignatureStatus[signer_].isVerified, signer_, saltedHash_);        
    ...
    function sign(uint256 salt, bytes calldata signature_) external {
        (address expectedSigner, bytes32 expectedSaltedHash) = _validateSignature(salt, signature_, signatureStatus);        
    ...
    function signMultiSig(uint256 salt, bytes calldata signature_) external {                
        (address expectedSigner, bytes32 expectedSaltedHash) = _validateSignature(salt, signature_, multiSignatureStatus[_msgSender()]);
        multiSignature[expectedSigner] = signature_;
        multiSignatureStatus[expectedSigner].isVerified = true;
        _commitVerifiedSigner(expectedSigner, expectedSaltedHash, signature_);        
    }

    function _commitVerifiedSigner(address signer_, bytes32 saltedHash_, bytes calldata signature_) private {
        verifiedSigners.push(signer_);
        emit ContractSigned(signer_, saltedHash_, signature_);        
    }

    function _validateSignerPreparation(bool isAlreadySigned_, address signer_, bytes32 saltedHash_) private pure {
    ...
    function _validateSignature(uint256 salt_, bytes calldata signature_, SignatureStatus memory signatureStatus_) private view returns (address, bytes32) {
        ...
        require(keccak256(abi.encodePacked(salt_)) == expectedSaltedHash, ERROR_INCORRECT_SECRET_SALT);
...
abstract contract Royalties is NiftyPermissions, NiftyTypes, IERC2981 {
    ...
    event RoyaltyReceiverUpdatedForTokenId(uint256 indexed tokenId, address previousReceiver, address newReceiver);
    ...
    mapping (uint256 => RoyaltyRecipient) internal royaltyRecipientsTokenIdOverrides;
    ...
    function setRoyaltyBips(uint256 scope, uint256 niftyTypeOrTokenId, uint256 bips) public {        
        _requireOnlyValidSender();            
        ...
        if(scope == 1) {                        
            ...
            royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].bips = uint16(bips);
        }
    }
    
    function royaltyInfo(uint256 tokenId, uint256 salePrice) public virtual override view returns (address, uint256) {
        if(royaltyRecipientsTokenIdOverrides[tokenId].recipient != address(0)) {
            return (royaltyRecipientsTokenIdOverrides[tokenId].recipient, (salePrice * royaltyRecipientsTokenIdOverrides[tokenId].bips) / BIPS_PERCENTAGE_TOTAL);
        } 
        
        uint256 niftyType = _getNiftyType(tokenId); 
        if(royaltyRecipients[niftyType].recipient != address(0)) {
            return (royaltyRecipients[niftyType].recipient, (salePrice * royaltyRecipients[niftyType].bips) / BIPS_PERCENTAGE_TOTAL);
        }

        if(globalFallbackRoyaltyRecipient.recipient != address(0)) {
            return (globalFallbackRoyaltyRecipient.recipient, (salePrice * globalFallbackRoyaltyRecipient.bips) / BIPS_PERCENTAGE_TOTAL);
        }

        return (address(0), 0);
    }    

    function initializeRoyalties(uint256 niftyType, address splitterImplementation, address[] calldata payees, uint256[] calldata shares) public returns (address)  {        
        initializeRoyalties(2, niftyType, splitterImplementation, payees, shares);
    }      
    ...
    function initializeRoyalties(uint256 scope, uint256 niftyTypeOrTokenId, address splitterImplementation, address[] calldata payees, uint256[] calldata shares) public returns (address)  {
        _requireOnlyValidSender();        

        if(scope == 1) {            
            address previousReceiver = globalFallbackRoyaltyRecipient.recipient;
            globalFallbackRoyaltyRecipient.isPaymentSplitter = payees.length > 1;
            globalFallbackRoyaltyRecipient.recipient = payees.length == 1 ? payees[0] : _clonePaymentSplitter(splitterImplementation, payees, shares);        
            emit RoyaltyReceiverUpdated(0, previousReceiver, globalFallbackRoyaltyRecipient.recipient);
            return globalFallbackRoyaltyRecipient.recipient;
        } else if(scope == 2) {
            address previousReceiver = royaltyRecipients[niftyTypeOrTokenId].recipient;        
            royaltyRecipients[niftyTypeOrTokenId].isPaymentSplitter = payees.length > 1;
            royaltyRecipients[niftyTypeOrTokenId].recipient = payees.length == 1 ? payees[0] : _clonePaymentSplitter(splitterImplementation, payees, shares);        
            emit RoyaltyReceiverUpdated(niftyTypeOrTokenId, previousReceiver, royaltyRecipients[niftyTypeOrTokenId].recipient);                        
            return royaltyRecipients[niftyTypeOrTokenId].recipient;
        } else if(scope == 3) {
            address previousReceiver = royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].recipient;        
            royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].isPaymentSplitter = payees.length > 1;
            royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].recipient = payees.length == 1 ? payees[0] : _clonePaymentSplitter(splitterImplementation, payees, shares);        
            emit RoyaltyReceiverUpdatedForTokenId(niftyTypeOrTokenId, previousReceiver, royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].recipient);
            return royaltyRecipientsTokenIdOverrides[niftyTypeOrTokenId].recipient;
        }
    ...
    function getPaymentSplitterByTokenId(uint256 tokenId) public virtual view returns (address) {        
        if(royaltyRecipientsTokenIdOverrides[tokenId].recipient != address(0)) {
            return royaltyRecipientsTokenIdOverrides[tokenId].isPaymentSplitter ? royaltyRecipientsTokenIdOverrides[tokenId].recipient : address(0);
        }
    ...
    function _getPaymentSplitterByNiftyTypeOrFallback(uint256 niftyType) internal view returns (address) {
        if(royaltyRecipients[niftyType].recipient != address(0)) {
            return royaltyRecipients[niftyType].isPaymentSplitter ? royaltyRecipients[niftyType].recipient : address(0);        
        }

        if(globalFallbackRoyaltyRecipient.recipient != address(0)) {
            return globalFallbackRoyaltyRecipient.isPaymentSplitter ? globalFallbackRoyaltyRecipient.recipient : address(0);
        }
...
abstract contract NiftyTypes {        
    ...
    function getNiftyTypeDetails(uint256 niftyType) public virtual view returns (NiftyType memory) {
