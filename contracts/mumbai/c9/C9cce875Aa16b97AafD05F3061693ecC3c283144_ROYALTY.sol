...
contract ROYALTY is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(address _artist) ERC721A("12 Knights Collective", "TKC") {
        artist = _artist;
    }
    ...
    function publicSaleMint(uint256 tokens) public payable nonReentrant {
        ...
        require(
            _numberMinted(_msgSender()) + tokens <= MAX_PUBLIC_MINT,
    ...
    function whitelistMint(
        ...
        uint256 supply = totalSupply();
        require(
            _numberMinted(_msgSender()) + tokens <= MAX_WHITELIST_MINT,
            "Max NFT Per Wallet exceeded"
        );
        require(tokens > 0, "need to mint at least 1 NFT");
        require(supply + tokens <= maxSupply, "We Soldout");
