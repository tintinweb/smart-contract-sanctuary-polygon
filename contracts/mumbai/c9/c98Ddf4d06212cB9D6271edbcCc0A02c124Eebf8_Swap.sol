...
contract Swap is BaseContract {
  struct Config {
    ...
    uint nftStakingRate;
    bool toPancake;
    address uniswapV2Pair; // TODO remove on next deployment
    bytes32 rootHash;
    address treasury;
  }
  struct User {
  ...
  function _buyDNO(uint _quantity, uint _orderValue, ISwap.PaymentCurrency _paymentCurrency) private {
    ...
    rankingNft.mint(msg.sender, _quantity);
  ...
  function _initDependentContracts() override internal {
    ...
    rankingNft = IRankingNFT(addressBook.get("rankingNft"));
