...
contract WhiteList is IWhiteList, OwnableUpgradeable {
  //if status == tsbOnly, then only tsbListedContracts [small mapping]
  //if status == partners, then tsbListedContracts and partnerContracts [manageable mapping]
  //if status == open, then no whitelist [no mapping needed]. But then we need a removeListing function for contracts we subsequently
  bool public tsbOnly;
