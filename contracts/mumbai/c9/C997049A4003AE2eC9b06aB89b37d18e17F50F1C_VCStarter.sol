...
contract VCStarter {
    ...
    function whitelistLab(address lab) external onlyCoreTeam {
    ...
    function blacklistLab(address lab) external onlyCoreTeam {
    ...
    function createProject(address _lab, uint256 _amountToRaise) external onlyCoreTeam returns (address newProject) {
    ...
    function activateProject(address _project) external onlyCoreTeam {
        VCProject(_project).activateProject();
        emit SttrProjectActivated(_project);
    }

    function closeProject(address _project) external onlyLabOwner(_project) {
    ...
    function backerMintPoCNft(
        ...
        if (newStatus == CampaignStatus.ACTIVE || newStatus == CampaignStatus.NOTFUNDED) {
            revert Errors.SttrCampaignNotSucceededNorFundedNorDefeated();
        }
        _backerMintPoCNft(_project, _campaignId, _currency);
    }
    ...
    function backerWithdrawDefeated(
        ...
        if (newStatus != CampaignStatus.NOTFUNDED) {
            revert Errors.SttrCampaignNotNotFunded();
        }
    ...
    function labWithraw(address _project, IERC20 _currency) external onlyLabOwner(_project) {
    ...
    function withdrawToPool(address _project, IERC20 _currency) external {
        uint256 transferedAmount = VCProject(_project).withdrawToPool(_currency, msg.sender);
...
contract VCProject is Initializable {
    ...
    function withdrawToPool(IERC20 _currency, address _receiver) external returns (uint256 amountAvailable) {
        _onlyStarter();
        amountAvailable = _currency.balanceOf(address(this)) - _totalCampaignsBalance[_currency] - _balanceOutsideCampaigns[_currency];
...
library Errors {
    ...
    error SttrCampaignNotNotFunded();
    error SttrCampaignNotSucceededNorFundedNorDefeated();
