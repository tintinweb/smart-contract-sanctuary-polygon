...
contract KhizaTokenVendor is Ownable, ReentrancyGuard {
    ...
    uint8 public selledTokenDecimals;
    ...
    constructor(
        ...
        IERC20 _khizaToken,
        uint _endsAt,
        uint8 _selledTokenDecimals
    ) {
        acceptedToken = _acceptedToken;
        khizaToken = _khizaToken;
        endsAt = _endsAt;
        selledTokenDecimals = _selledTokenDecimals;
    ...
    function setSelledTokenDecimals(uint8 _selledTokenDecimals) public onlyOwner {
        selledTokenDecimals = _selledTokenDecimals;
    }
    ...
    function calculate(uint amountToBuy) public view returns (uint amountToPay) {
        amountToPay = (amountToBuy * pricePerToken) / (10 ** selledTokenDecimals);
    }
    ...
    function buyTokens(uint amountToBuy) public nonReentrant notPaused whitelistCompliance(amountToBuy) timeCompliance {
        ...
        uint amountToPay = (amountToBuy * pricePerToken) / (10 ** selledTokenDecimals);
        // TODO: verificar se saldo a pagar é maior do que algum valor mínimo
        ...
        // TODO: inverter ordem: primeiro recebe USDC, depois transfere KHIZA
    ...
    function setWhitelistAddress(address _address, uint limit) onlyOwner public {
        emit WhitelistAdded(_address, limit);
        whitelistWithLimits[_address] = limit;
    }

    event WhitelistAdded(address _address, uint limit);
    // =========== PAUSABLE ===========
