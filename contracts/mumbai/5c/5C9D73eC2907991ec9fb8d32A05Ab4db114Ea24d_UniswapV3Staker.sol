...
interface IUniswapV3Staker is IERC721Receiver, IMulticall {
    ...
    event IncentiveEnded(address indexed msgSender,uint256 timeStamp,bytes32 indexed incentiveId, uint256 refund);

    /// @notice Emitted when ownership of a deposit changes
    /// @param tokenId The ID of the deposit (and token) that is being transferred
    /// @param oldOwner The owner before the deposit was transferred
    /// @param newOwner The owner after the deposit was transferred
    event DepositTransferred(address indexed msgSender,uint256 timeStamp,uint256 indexed tokenId, address oldOwner, address indexed newOwner);

    /// @notice Event emitted when a Uniswap V3 LP token has been staked
    /// @param tokenId The unique identifier of an Uniswap V3 LP token
    /// @param liquidity The amount of liquidity staked
    /// @param incentiveId The incentive in which the token is staking
    event TokenStaked(address indexed msgSender,uint256 timeStamp,uint256 indexed tokenId, bytes32 indexed incentiveId, uint128 liquidity);

    /// @notice Event emitted when a Uniswap V3 LP token has been unstaked
    /// @param tokenId The unique identifier of an Uniswap V3 LP token
    /// @param incentiveId The incentive in which the token is staking
    event TokenUnstaked(address indexed msgSender,uint256 timeStamp,uint256 indexed tokenId, bytes32 indexed incentiveId);

    /// @notice Event emitted when a reward token has been claimed
    /// @param to The address where claimed rewards were sent to
    /// @param reward The amount of reward tokens claimed
    event RewardClaimed(address indexed msgSender,uint256 timeStamp,address indexed to, uint256 reward);
}
...
contract UniswapV3Staker is IUniswapV3Staker, Multicall {
    ...
    function createIncentive(IncentiveKey memory key, uint256 reward) external override {
        ...
        emit IncentiveCreated(msg.sender,block.timestamp,key.rewardToken, key.pool, key.startTime, key.endTime, key.refundee, reward);
    }
    ...
    function endIncentive(IncentiveKey memory key) external override returns (uint256 refund) {
        ...
        emit IncentiveEnded(msg.sender,block.timestamp,incentiveId, refund);
    }
    ...
    function onERC721Received(
        ...
        emit DepositTransferred(msg.sender,block.timestamp,tokenId, address(0), from);
    ...
    function transferDeposit(uint256 tokenId, address to) external override {
        ...
        emit DepositTransferred(msg.sender,block.timestamp,tokenId, owner, to);
    }
    ...
    function withdrawToken(uint256 tokenId, address to, bytes memory data) external override {
        ...
        emit DepositTransferred(msg.sender,block.timestamp,tokenId, deposit.owner, address(0));
    ...
    function unstakeToken(IncentiveKey memory key, uint256 tokenId) external override {
        ...
        emit TokenUnstaked(msg.sender,block.timestamp,tokenId, incentiveId);
    }
    ...
    function claimReward(
        ...
        emit RewardClaimed(msg.sender,block.timestamp,to, reward);
    }
    ...
    function _stakeToken(IncentiveKey memory key, uint256 tokenId) private {
        ...
        emit TokenStaked(msg.sender,block.timestamp,tokenId, incentiveId, liquidity);
    }
}