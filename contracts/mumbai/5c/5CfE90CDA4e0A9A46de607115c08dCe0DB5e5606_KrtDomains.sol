...
contract KrtDomains is ERC721URIStorage, Pausable {
    ...
    //将域名映射为TokenId
    mapping(string => uint256) public getDomainId;
    //白名单判断
    ...
    //以此类推！请记住，_tokenId是一个状态变量，这意味着如果我们更改它，该值将直接存储在合约中
    constructor(string memory _tld)
        payable
        ERC721("krypectro Name Service", "create")
    {
        owner = payable(msg.sender);
        tld = _tld;
        console.log(
            "Hello and welcome to the world of Krypectro domain names!"
        );
    }
    ...
    function register(string calldata name) public payable {
        ...
        if (whitelist[msg.sender] == true) {
            //必须限定次数，这是很重要的
            require(
                msg.value >= thisWhitePrice,
                "This whiteAddress not enough Matic paid"
            );
        ...
        string memory finalSvg = string(
            abi.encodePacked(tokenURI, _name, svgPartTwo)
        );
    ...
    // 根据域名获取所有者地址
    function getDomainAddress(string memory name)
        public
        view
        returns (address)
    {
    ...
    //根据tokenId返回域名
    function getTokenIdToName(uint256 chooseTokenId)
        public
        view
    ...
    //根据域名返回tokenId
    function getNameToTokenId(string calldata name)
        public
        view
        returns (uint256)
    {
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        if (len == 3) {
            return 20 * 10**17; //0.05matic.
        } else if (len == 4) {
            return 10 * 10**17; //0.03matic
        } else {
            return 1 * 10**17; //0.01matic
        }
    }
    ...
    //这个函数为白名单铸造域名价格
    function whitePrice(string calldata name) public pure returns (uint256) {
        uint256 len = StringUtils.strlen(name);
        require(len > 0 && 3 <= len && len <= 10);
        if (len == 3) {
            return 0 * 10**17; //0.000005matic.
        } else if (len == 4) {
            return 0 * 10**17; //0.000003matic
        } else {
            return 0 * 10**17; //0.000001matic
        }
    }
