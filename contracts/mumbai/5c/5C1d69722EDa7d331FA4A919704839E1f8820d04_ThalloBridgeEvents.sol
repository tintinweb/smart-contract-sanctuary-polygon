...
import {ActionTypes, RegistryBlockType, RegistryBlock, RetirementBlock, CustodyBlock, BridgeEvent, EventEdit, TCCInfo, ProofOfRetirementData} from "./ThalloStructs.sol";
...
contract ThalloBridgeEvents is UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    ///@dev Enforces a serial number cannot be minted twice unless specifically allowed
    mapping(uint16 => mapping(string => bool)) public serialNumberMinted;
    ...
    event TokensUncustodied(address indexed tcc, uint256 requestId, uint256 quantity, uint256 eventIndex);
    event ReturnUncustody(address indexed tcc, uint256 requestId);
    event ReturnRetirement(address indexed tcc, uint256 requestId);
    event EventEdited(
    ...
    function issueVintageTokens(
        ...
        uint16 registry = ITCC(tccAddress).registry();
        require(_allowDuplicate || !serialNumberMinted[registry][_registryBlock.serialNumber], "TBE:IVT:SNM");
        ...
        serialNumberMinted[registry][_registryBlock.serialNumber] = true;
    ...
    /**
     *@notice Finalize the retirement with a proof of retirement NFT with registry information
     *@param _proofData The struct containing proof of retirement data. See ThalloStructs.sol for details. No need to pass in values for tccAddress and registry as they'll get overwritten anyway - just pass blank strings.
     *@param _requestId The request id for the retirement (stored in the TCC contract)
    ...
    function finalizeRetirement(
        ProofOfRetirementData memory _proofData,
        uint256 _requestId,
        address _to,
        ...
        bytes32 vintageKey = keccak256(abi.encodePacked(_proofData.projectId, _proofData.vintage));

        TCCInfo storage tcc = tccInfo[vintageKey];
        _proofData.tccAddress = tcc.tccAddress;

        uint16 registry = ITCC(tcc.tccAddress).registry();
        _proofData.registry = registry;

        _populateRetirementEvent(vintageKey, _registryBlocks, _custodyIndices, _custodyBlocks, _memo);

        uint256 totalRetirement = 0;
        for (uint256 i = 0; i < _proofData.retirementBlocks.length; i++) {
            totalRetirement += _proofData.retirementBlocks[i].quantity;
        }
        tcc.totalRetirement += totalRetirement;
        ITCC(tcc.tccAddress).finalizeRetirement(_requestId, totalRetirement);
        ITPR(thalloProofOfRetirement).issueFinalProof(_proofData, _to);
    }

    function _populateRetirementEvent(
        bytes32 _vintageKey,
        RegistryBlock[] calldata _registryBlocks,
        uint256[] calldata _custodyIndices,
        CustodyBlock[] memory _custodyBlocks,
        ...
        uint256 eventIndex = ++eventsLength[_vintageKey];
        BridgeEvent storage retirement = bridgeEvents[_vintageKey][eventIndex];
        ...
        for (uint256 i = 0; i < _custodyIndices.length; i++) {
            custodyBlocks[_vintageKey][_custodyIndices[i]] = _custodyBlocks[i];
        }
     ...
     *@param _tcc The address of the TCC contract
     *@param _requestId The request id for the retirement (stored in the TCC contract)
     *@dev This function should only be called when the retirement request was made in error
     */
    function returnRetirement(address _tcc, uint256 _requestId) external onlyCustodian {
        ITCC(_tcc).returnRetirement(_requestId);
        emit ReturnRetirement(_tcc, _requestId);
    }
     ...
     *@param _projectId  The id of the project
     *@param _vintage The vintage year
     *@param _requestId The id of the uncustody request (stored in the TCC contract)
    ...
    function finalizeUncustody(
        ...
        ITCC(tccAddress).finalizeUncustody(_requestId, _quantity);
        emit TokensUncustodied(tccAddress, _requestId, _quantity, numEvents);
    }

    /**
     *@notice Returns carbon credits on chain to the requesting party
     *@param _tcc The address of the TCC contract
     *@param _requestId The request id for the uncustody (stored in the TCC contract)
     *@dev This function will be called when the requesting address does not have an external registry account
     */
    function returnUncustody(address _tcc, uint256 _requestId) external onlyCustodian {
        ITCC(_tcc).returnUncustody(_requestId);
        emit ReturnUncustody(_tcc, _requestId);
    }
...
interface ITCC {
    ...
    function finalizeUncustody(uint256 _requestId, uint256 _quantity) external;

    function returnUncustody(uint256 _requestId) external;

    function finalizeRetirement(uint256 _requestId, uint256 _quantity) external;

    function returnRetirement(uint256 _requestId) external;

    function totalSupply() external view returns (uint256);

    function registry() external view returns (uint16);
}

//SPDX-License-Identifier: BUSL-1.1
pragma solidity 0.8.7;

import {ProofOfRetirementData} from "../ThalloStructs.sol";

interface ITPR {
    function issueFinalProof(ProofOfRetirementData calldata _proofData, address _to) external;
}
