...
contract airdrop is Ownable {
	...
	uint256 public todayZeroClock = 1647360000;  // 2022-03-16 00:00:00
	bool public isAirdrop;
	uint256 public remainAmount;
	uint256 public everydayAmount;

	constructor() {
		// init todayZeroClock as deploy day's zero o'clock
		todayZeroClock = block.timestamp - (block.timestamp - todayZeroClock) % 86400;
	...
	function reset() internal virtual {
		isAirdrop = false; // for next day's airdrop
		todayZeroClock += 86400; // tomorrow's zero o'clock
	...
	function isManager(address _manager) public onlyOwner view returns (bool) {
	...
	function AirTransferDiffValue(address[] memory _recipients, uint[] memory _values, address _tokenAddress) public onlyManager returns (bool) {
		require( (block.timestamp > todayZeroClock) && (!isAirdrop), "Already airdropped for today");
		...
		require(remainAmount - _transferredAmount >= 0, "Everyday transfer amount insufficient");
