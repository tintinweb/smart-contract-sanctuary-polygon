...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    // Calculate level rewards for the user's upline
    // Calculate level rewards for the user's upline
    // Calculate level rewards for the user's upline
    function getLevelReward(address _user) public view returns (uint256) {
        ...
        uint256 roundReward = getRoundReward(_user, 0); // Get the user's round reward

        UserInfo storage uplineInfo = userInfo[upline];
        uint256 level = uplineInfo.level;

        if (upline == defaultReferrer) {
            if (level < levelPercentage.length) {
                levelReward = roundReward * levelPercentage[level] / 100;
            }
        } else if (upline != address(0)) {
            // Apply the level condition when upline is not the default referrer or an invalid address
            if (level < levelPercentage.length && uplineInfo.totalStake > 0) {
                levelReward = roundReward * levelPercentage[level] / 100;
            }
        }
    ...
    //     reward.totalRewards = reward.roundRewards.add(reward.levelRewards);
