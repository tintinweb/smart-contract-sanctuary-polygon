...
contract PaybLaunchPad is Ownable, ReentrancyGuard {
    ...
    function withdraw(uint24 launchpadId) external onlyOwner() nonReentrant{
        // check balances of launchpad
        require(balanceOfLaunchpad[launchpadId] > 0, "total 0");

        address _self = address(this);
        uint256 _balance = _self.balance;

        // check balances this >= balances launchpad
        require(_balance >= balanceOfLaunchpad[launchpadId], "balance !enought");

        LaunchpadInfo storage launchpad = launchpads[launchpadId];

        launchpad.stakeToken.safeTransfer(_msgSender(), balanceOfLaunchpad[launchpadId]);
        balanceOfLaunchpad[launchpadId] = 0;    
    }
