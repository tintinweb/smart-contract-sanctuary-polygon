...
/// @author Quinn Eschenbach/Jaypigs
...
contract ERC4907 is ERC721, ERC721URIStorage, IERC4907, Ownable {
    ...
    function mint(uint256 _id, string memory _tokenURI) external onlyOwner {
        _mint(msg.sender, _id);
        _setTokenURI(_id, _tokenURI);
    }

    function setTokenURI(uint256 _id, string memory _tokenURI)
        external
        onlyOwner
    {
        _setTokenURI(_id, _tokenURI);
...
contract AxelarExecutableUpgradeable is Initializable, IAxelarExecutable {
    IAxelarGateway public gateway;

    function __AxelarExecutable_init(address gateway_)
        internal
        onlyInitializing
    {
