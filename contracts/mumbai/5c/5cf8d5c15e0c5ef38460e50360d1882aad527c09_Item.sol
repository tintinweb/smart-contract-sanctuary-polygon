...
contract Item is UUPSUpgradeable, ERC1155Upgradeable, OwnableUpgradeable{
    ...
    function mintDirect(address _receiver, string memory _itemUUID, uint256 quantity, uint256 _auraAmount) public onlyOwner{
        require(_auraAmount > 0, "invalid aura amount");

        uint256 tokenId = _tokenIdCounter.current();
        _mint(_receiver, tokenId, quantity, "");
        _tokenIdCounter.increment();

        items[tokenId] = ItemInfo({itemUUID: _itemUUID, auraAmount: _auraAmount});
        IERC20Upgradeable(auraAddress).transferFrom(msg.sender, address(this), _auraAmount * quantity);
    }
}