...
contract PolygonNFT721Batch is ERC721URIStorage, ERC2771Context {
...
contract PolygonNFTCollectionBatch is ERC2771Context {
    ...
    function createCollection(address collector, string calldata _session, string memory _name, string memory _sybmol) external {
        ...
        PolygonNFT721Batch collection = new PolygonNFT721Batch(_name, _sybmol, collector, trustedForwarder, address(this));
    ...
    function mintUnderCollection(address collection, string calldata _session, address user, uint256 from, uint256 to, string memory _uri) external {
        if(_msgSender() == operator || collectionRecords[_msgSender()][_session] == address(collection)) {
            PolygonNFT721Batch(collection).mint(user, from, to, _uri);
        } else {
