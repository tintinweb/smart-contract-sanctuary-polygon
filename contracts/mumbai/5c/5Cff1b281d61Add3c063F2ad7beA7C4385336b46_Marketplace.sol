 ...
 */
contract Minter is StateValidator, IMinter, LaunchBase {
        ...
        returns (bool)
    {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            IERC20(_VALID_TOKENS[state.ticker]).safeTransferFrom(
                item.bookmaker,
                state.owner,
                buybackPrice
            );
 ...
 * @notice BetNFT marketplace
 */
contract Playground is IPlayground, StateValidator, LaunchBase {
    using SafeERC20 for IERC20;
