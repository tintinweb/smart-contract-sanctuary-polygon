...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function closePosition(uint256 positionId) public {
        ...
        if(positionInfo.asset == uniswapV2Router.WETH()) {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).closePositionAmount(positionInfo.userAddress, address(0), positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount, temp);
        }
        else {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).closePositionAmount(positionInfo.userAddress, positionInfo.asset, positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount, temp);
        }
    ...
    function closeAmount(uint256 positionId, uint256 result) internal returns(uint256, uint256) {
        Position storage positionInfo = positionIDs[positionId];
        uint256 temp;
        ...
        if(positionInfo.assetAmount >= result) {
            ...
            if(positionInfo.marginUsed > temp) {
                marginAmount = positionInfo.marginUsed - temp;
                temp = 0;
            }
            else {
                temp = temp - positionInfo.marginUsed;
                marginAmount = 0;
            }
        }
        else {
    ...
    function checkUserOpenPosition(address user, address assetAddress) public view returns(bool, uint256) {
        ...
        for(uint i = 0; i < positionIds.length; i++) { 
            ...
            if(positionInfo.assetAmount > 0 && positionInfo.status == 0 && positionInfo.asset == assetAddress) {
                //open position
                isPosition = false;
                return (isPosition, positionIds[i]);
                //break;
            }            
        }
        return (isPosition, 0);
