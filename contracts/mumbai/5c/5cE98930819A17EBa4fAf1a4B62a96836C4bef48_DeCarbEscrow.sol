...
contract DeCarbEscrow is Ownable {
    ...
    IDeCarbParams private approvedProjects;
    ...
    function openCarbonEscrow(
        ...
        require(
            approvedProjects.isProjectId(_vintage, _projectID),
            "project not approved"
        );

        uint256 co2PriceInUSDC = approvedProjects.getVintagePrice(
            _vintage,
            _projectID
        );
    ...
    function setApprovedProjects(address _approvedProjects) internal {
        ...
        approvedProjects = IDeCarbParams(_approvedProjects);
    }
...
interface IDeCarbParams {
    function isProjectId(uint256 _vintage, string memory _projectID)
        external
        view
        returns (bool);

    function getVintagePrice(uint256 _vintage, string memory _projectID)
        external
        view
        returns (uint256);
}
