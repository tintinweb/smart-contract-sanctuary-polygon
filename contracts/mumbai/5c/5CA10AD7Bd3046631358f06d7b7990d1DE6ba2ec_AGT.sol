...
contract AGT is Ownable {
    /// @notice BEP-20 token name for this token
    string public constant name = "Agira";
    ...
    uint public totalSupply = 1000000000e18; // 1 billion AGL

    /// @notice Reward eligible epochs
    uint32 public constant eligibleEpochs = 30; // changed from 60 epochs to 30 epochs
    ...
    function balanceInternal(address account) internal view returns (uint96){
        return balances[account];
    }


    function mint(uint96 amount) public onlyOwner {
        require(msg.sender != address(0));
        uint96 currentBalance = balanceInternal(msg.sender);
