...
contract WasteTrader is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWasteToCash {
        ...
        if (gwsTime > 0) {
            ...
            if (wasteOrders.length == 0) {
                ...
                for (uint256 orderIndex; orderIndex < wasteOrders.length; ) {
                    //if (!s_wasteOrders[msg.sender][orderIndex].completed) {
                    if (
                        s_wasteOrders[msg.sender][orderIndex].orderAmount + _wasteAmount <= gwsMax
                    ) {
    ...
    function createNewOrder(uint256 _wasteAmount) internal {
        ...
        s_wasteOrders[msg.sender].push(WasteOrder(_wasteAmount, block.timestamp, 0, s_gwsTime));
    }
    ...
    function claimAstroCredits() external nonReentrant {
        ...
        WasteOrder[] storage wasteOrders = s_wasteOrders[msg.sender];
        for (uint256 orderIndex; orderIndex < wasteOrders.length; ) {
            if (block.timestamp - s_wasteOrders[msg.sender][orderIndex].createdAt > gwsTime) {
                ...
                if (orderAmount > 0) {
                    ...
                    // s_wasteOrders[msg.sender][orderIndex].speedUpDeductedAmount = 0;
