...
contract Gigble {
    ...
    function createGig(
        ...
        // gig.gigCreator = _gigCreator;
        gig.email = _email;
        gig.title = _title;
        gig.description = _description;
        gig.deliveryDate = _deliveryDate;
        gig.price = _price;
        gigCount++;
    ...
    function abort(uint256 _id) external onlyPublisher inState(State.Created) {
        emit Aborted();
    ...
    function confirmPurchase(
        uint256 _id
    ) external payable inState(State.Created) condition(msg.value == (value)) {
    ...
    function acceptGig(
        uint256 _id
    ) external onlyGigCreator inState(State.Locked) {
        emit ItemReceived();
    ...
    function refund(uint256 _id) external onlyPublisher inState(State.Created) {
        emit SellerRefunded();
        // It is important to change the state first because
        // otherwise, the contracts called using `send` below
        // can call in again here.
