 ...
 */
contract CryptoTattooerzTokenDirectSale is CryptoTattooerzCurrencyHandler, Pausable, AccessControlEnumerable{
    ...
    /** @dev Address of the ERC20 TOKEN contract in sale by this contract */
     ...
     * @param tokenAddress Address of the ERC20 TOKEN contract in sale by this contract
     */
    constructor(address priceHandlerAddress, address tokenAddress) {
        super._setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        setPriceHandlerAddress(priceHandlerAddress);
        setTokenAddress(CryptoTattooerzPriceHandler(priceHandlerAddress).TOKEN(), tokenAddress);
    }
    ...
    function setPriceHandlerAddress(address priceHandlerAddress) public onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        if(priceHandlerAddress != address(0)) {
            CryptoTattooerzPriceHandler(priceHandlerAddress).transform(COIN, TOKEN, 1000000);
        }
        _priceHandlerAddress = priceHandlerAddress;
    }

    /**
     * @dev External getter of the address of the ERC20 token contract corresponding to given currency code (could be
     * proprietary token "TOKEN", or any other of the handled tokens such as "USDC"...)
     */
    function getTokenAddress(bytes32 currency) external view returns (address) {
        return _tokensAddress[currency];
    }
    /**
     * @dev Internal getter of the address of the ERC20 token contract corresponding to given currency code (could be
     * proprietary token "TOKEN", or any other of the handled tokens such as "USDC"...). Will revert if token contract
     * address is not defined
     */
    function _getTokenAddress(bytes32 currency) internal view returns (address) {
        address tokenAddress = _tokensAddress[currency];
        require(tokenAddress != address(0), "CryptoTattooerzTokenDirectSale: Unknown requested currency");
        return tokenAddress;
    }
    /**
     * @dev Setter of the address of the ERC20 token contract in sale by this contract, only accessible by admins. If direct
     * sale contract still have funds for ERC20 token contract about to be changed, update will revert
     */
    function setTokenAddress(bytes32 currency, address tokenAddress) public onlyRole(DEFAULT_ADMIN_ROLE){
        ...
        require(currency != COIN, "CryptoTattooerzTokenDirectSale: ERC20 token address cannot be set for COIN");
        ...
        if(_tokensAddress[currency] != address(0)) {
            require(getBalance(currency) == 0,
                    "CryptoTattooerzTokenDirectSale: Cannot change ERC20 token address with remaining funds");
//            require(currency != getPriceHandler().TOKEN(), "CryptoTattooerzTokenDirectSale: Cannot change TOKEN address");
        }
        if(tokenAddress == address(0)) {
    ...
    /**
     * @dev Getter of the contract handling tokens in sale
     */
    function getERC20() private view returns (ERC20) {
        return getERC20(getPriceHandler().TOKEN());
    }
    /**
     * @dev Getter of the contract handling requested tokens currency
     */
     ...
     * deduced from amount of COINs sent in the message value at the time the transaction in being processed by the contract
    ...
    function buyWithCoins() public payable returns (uint256) {
        ...
        CryptoTattooerzPriceHandler priceHandler = getPriceHandler();
        uint256 tokenAmount = priceHandler.transform(priceHandler.COIN(), priceHandler.TOKEN(), msg.value);
        // Perform the final buy
        _buy(msg.sender, tokenAmount, priceHandler.COIN(), msg.value);
        return tokenAmount;
    }
    /*function buyWithCoins(uint256 requestedTokenAmount) private payable returns (uint256) {
        CryptoTattooerzPriceHandler priceHandler = getPriceHandler();
        uint256 tokenAmount = priceHandler.transform(priceHandler.COIN(), priceHandler.TOKEN(), msg.value);
        // TODO better
        require((requestedTokenAmount * 100*10000 / tokenAmount) < 2*10000);
        _buy(msg.sender, requestedTokenAmount, priceHandler.COIN(), msg.value);
        return tokenAmount;
    }*/
    /**
     * @dev This is the method to call to directly buy TOKENs with other ERC20 tokens. Amount of purchased TOKENs will
     * directly be deduced from amount of tokens to be transfered from ERC20 contract at the time the transaction in being
     * processed by the contract
     * @param currency Currency code of the ERC20 token contract to be used to buy TOKENs
     * @param amount Amount of ERC20 tokens to be used to buy TOKENs
     * @return The amount of purchased TOKENs
     */
    function buyWithTokens(bytes32 currency, uint256 amount) public returns (uint256) {
        // Calculate the amount of tokens corresponding to sent amount of ERC20 tokens
        CryptoTattooerzPriceHandler priceHandler = getPriceHandler();
        uint256 tokenAmount = priceHandler.transform(currency, priceHandler.TOKEN(), amount);
        // Retrieve the amount of ERC20 tokens from its contract in order to by tokens with
        SafeERC20.safeTransferFrom(getERC20(currency), msg.sender, address(this), amount);
        // Perform the final buy
        _buy(msg.sender, tokenAmount, currency, amount);
        return tokenAmount;
    }
    /*function buyWithTokens(bytes32 currency, uint256 amount, uint256 requestedTokenAmount) private returns (uint256) {
        // Calculate the amount of tokens corresponding to sent amount of ERC20 tokens
        CryptoTattooerzPriceHandler priceHandler = getPriceHandler();
        uint256 tokenAmount = priceHandler.transform(currency, priceHandler.TOKEN(), amount);
        // TODO better
        require((requestedTokenAmount * 100*10000 / tokenAmount) < 2*10000);
        // Retrieve the amount of ERC20 tokens in order to by tokens with
        SafeERC20.safeTransferFrom(getERC20(currency), msg.sender, address(this), amount);
        // Perform the final buy
        _buy(msg.sender, tokenAmount, currency, amount);
        return tokenAmount;
     ...
     * paused
     */
    function _buy(address buyer, uint256 amount, bytes32 currency, uint256 price) private whenNotPaused() {
        require(buyer != address(0), "CryptoTattooerzTokenDirectSale: Buyer should provide a valid address");
     ...
     */
    function withdraw(bytes32 currency, uint256 amount) external onlyRole(FUNDS_ADMIN_ROLE) {
        require(msg.sender != address(0), "CryptoTattooerzTokenDirectSale: Should withdraw funds to a valid address");
        if(currency == getPriceHandler().COIN()) {
     ...
     */
    function getBalance(bytes32 currency) public view returns (uint256) {
        if(currency == getPriceHandler().COIN()) {
...
abstract contract CryptoTattooerzPriceHandler is ICryptoTattooerzPriceHandler, CryptoTattooerzCurrencyHandler, AccessControlEnumerable{
     ...
     * @return The amount of 'toCurrency' corresponding to given amount of 'fromCurrency'
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) public view override returns (uint256) {
        ...
        if(toCurrency == TOKEN) {
            ...
            (uint8 discountRate, uint8 discountDecimals) = calculateTokenPriceDiscountRate(fromPriceUSD, fromDecimalsUSD);
...
interface ICryptoTattooerzPriceHandler {
     ...
     * @return The amount of 'toCurrency' corresponding to given amount of 'fromCurrency'
     */
    function transform(bytes32 fromCurrency, bytes32 toCurrency, uint256 amount) external view returns (uint256);
    /**
     * @dev Getter of the USD price for given currency amount (could be default chain coin "COIN", proprietary token
