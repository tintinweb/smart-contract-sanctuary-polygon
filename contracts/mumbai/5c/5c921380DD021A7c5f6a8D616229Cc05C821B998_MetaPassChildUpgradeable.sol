...
contract MetaPassChildUpgradeable is Initializable, ERC1155SupplyUpgradeable, ERC1155PausableUpgradeable, IChildToken, AccessControlMixinUpgradeable, NativeMetaTransactionUpgradeable, ContextMixinUpgradeable {
    ...
    uint constant MINT_PASS_ID = 0;

    event Withdraw(address owner, uint256 id, uint256 amount);
    event Deposit(address owner, uint256 id, uint256 amount);
    event MintAdmin(address recipient, uint256 id, uint256 amount);
    event Burn(address sender, uint256 id, uint256 amount);
    event Claim(address owner, uint256 id, uint256 amount);

    function __MetaPassChild_init(
        string memory name_,
        ...
        _name = name_;
        _symbol = symbol_;
        _setupContractId("MetaPassChildUpgradeable");
    ...
    function deposit(address user, bytes calldata depositData) external override only(DEPOSITOR_ROLE) {
        ...
        for (uint i = 0; i < ids.length; ++i) {
            emit Deposit(user, ids[i], amounts[i]);
        }
    }

    function withdrawSingle(uint256 id, uint256 amount) external {
        _burn(_msgSender(), id, amount);
        emit Withdraw(_msgSender(), id, amount);
    }

    function withdrawBatch(uint256[] calldata ids, uint256[] calldata amounts) external {
        _burnBatch(_msgSender(), ids, amounts);
        for (uint i = 0; i < ids.length; ++i) {
            emit Withdraw(_msgSender(), ids[i], amounts[i]);
        }
    }
    ...
    function claim(uint256 amount, bytes32[] calldata proof) external {
        ...
        require(_verify(_leaf(amount, _msgSender()), passOwnerRoot, proof), "Not owned");

        claimedTokens[_msgSender()] = true;

        _mint(_msgSender(), MINT_PASS_ID, amount, "");

        emit Claim(_msgSender(), MINT_PASS_ID, amount);
    }

    function mintAdmin(uint256 id, uint256 amount, address recipient) external only(AUTHORISED_MINTER_ROLE) {
        _mint(recipient, id, amount, "");
        emit MintAdmin(recipient, id, amount);
    }

    function burn(uint256 id, uint256 amount, address wallet) external only(AUTHORISED_BURNER_ROLE) {
        safeTransferFrom(wallet, burnWallet, id, amount, "");
        _burn(burnWallet, id, amount);
        emit Burn(wallet, id, amount);
    }

    function setPassOwnerRoot(bytes32 _newRoot) external only(DEFAULT_ADMIN_ROLE) {
        passOwnerRoot = _newRoot;
    }
    ...
    function _leaf(uint256 amount, address walletAddress) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(amount, walletAddress));
    }
