...
interface IEmissionLogic {
    function determineTokenId(uint256 logic) external view returns (uint256 tokenid);
}
...
contract PhiDaily is AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(address sender => uint256 count) public ClaimedCount;
    mapping(address sender => mapping(uint256 tokenid => uint256 count)) public ClaimedEachCount;
    mapping(address sender => mapping(uint256 eventid => mapping(uint256 logicid => uint256 _status)))
        public DailyClaimedStatus;
    /* --------------------------------- ****** --------------------------------- */
    /* -------------------------------------------------------------------------- */
    /*                                   EVENTS                                   */
    /* -------------------------------------------------------------------------- */
    ...
    function initialize(
        ...
        _materialObject = materialContract;
    ...
    modifier onlyIfAllreadyClaimed(uint256 eventid, uint256 logicid) {
        if (DailyClaimedStatus[msg.sender][eventid][logicid] == _CLAIMED) {
            revert AllreadyClaimed({ sender: msg.sender, eventid: eventid, logicid: logicid });
        }
        _;
    }

    modifier onlyIfAllreadyClaimedMultiple(uint256[] memory eventids, uint256[] memory logicids) {
        require(eventids.length == logicids.length, "Array length mismatch");
        for (uint i = 0; i < eventids.length; i++) {
            if (DailyClaimedStatus[msg.sender][eventids[i]][logicids[i]] == _CLAIMED) {
                revert AllreadyClaimed({ sender: msg.sender, eventid: eventids[i], logicid: logicids[i] });
            }
        }
        _;
    }
    ...
    /// @dev get MaterialContract
    function getDailyContract() external view returns (address) {
        return _materialObject;
    }

    /// @dev set DailyContract
    function setDailyContract(address materialObject) external onlyOwner {
        require(materialObject != address(0), "cant set address(0)");
    ...
    function claimMaterialObject(
        ...
        bytes32 digest = keccak256(abi.encode(eventid, logicid, msg.sender));
        require(isVerifiedCoupon(digest, coupon), "Invalid coupon");

        uint256 tokenid = IEmissionLogic(_emissionLogic).determineTokenId(logicid);
        IMaterialObject(_materialObject).getObject(msg.sender, tokenid);
        _updateClaimStatus(msg.sender, eventid, tokenid, logicid);
        emit LogClaimMaterialObject(msg.sender, eventid, logicid, tokenid);
    }

    function batchClaimMaterialObject(
        uint256[] memory eventids,
        uint256[] memory logicids,
        ...
        require(eventids.length == logicids.length && logicids.length == coupons.length, "Array length mismatch");

        for (uint i = 0; i < eventids.length; i++) {
            // Check that the coupon sent was signed by the admin signer
            bytes32 digest = keccak256(abi.encode(eventids[i], logicids[i], msg.sender));
            require(isVerifiedCoupon(digest, coupons[i]), "Invalid coupon");

            uint256 tokenid = IEmissionLogic(_emissionLogic).determineTokenId(logicids[i]);
            IMaterialObject(_materialObject).getObject(msg.sender, tokenid);
            _updateClaimStatus(msg.sender, eventids[i], tokenid, logicids[i]);
            emit LogClaimMaterialObject(msg.sender, eventids[i], logicids[i], tokenid);
        }
    }
}