...
contract Crowdsale is  Initializable, Pausable, ReentrancyGuard {
    ...
    event SetGoal(uint256 softCap, uint256 hardCap);
    ...
    event Distribute(address[] investors, uint256[] amount);
    event Refund(address[] investors, uint256[] amount);
    event RemoveInvestment(address investor);
    ...
    {   
        address _accessControlAddress = getRoleAddr();
        require(IAccessControl(_accessControlAddress).hasRole(WHITELISTED_ROLE, _account), "Not whitelisted");
        require(!_isCanceled, "Sale is already canceled");
    ...
    function removeInvestment(address _account)
        external
        onlyRole(MANAGER_ROLE)
    {
        investorMap.reinitInvestment(
            _account
        );
        emit RemoveInvestment(_account);
    }
    ...
    function refund(
        ...
        require(_isCanceled, "The sale is not canceled.");
        ...
        address[] memory investorsAddress = new address[](size);
        uint256[] memory refundAmount = new uint256[](size);
        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            address key = investorMap.getKeyAtIndex(i);
            bool alreadyRefunded = investorMap.isRefunded(key);
            if(!alreadyRefunded) {
                uint256 investedAmount = investorMap.getTokenAmountFor(key);
                USDT.safeTransfer(key, investedAmount);
                investorsAddress[i] = key;
                refundAmount[i] = investedAmount;
                investorMap.refund(key);
            }
        }
    ...
    function withdrawFundsRaised(address _to, uint256 _amount)
        ...
        onlyRole(MANAGER_ROLE)
    {
        require(_isFinalized, "The sale is not yet finalized.");
        require(
    ...
    function distributeTokensAfterCrowdsale(
        ...
        uint256 size = _endIndex - _startIndex + 1;
        address[] memory investorsAddress = new address[](size);
        uint256[] memory purchasedAmounts = new uint256[](size);
        uint256[] memory investedAmounts = new uint256[](size);

        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            address key = investorMap.getKeyAtIndex(i);
            bool alreadySent = investorMap.isDistributed(key);
            if(!alreadySent) {
                uint256 purchased = investorMap.getSecurityTokenAmountFor(key);
                uint256 amount = investorMap.getTokenAmountFor(key);
                token.safeTransfer(key, purchased);
                investorsAddress[i] = key;
                purchasedAmounts[i] = purchased;
                investedAmounts[i] = amount;
                investorMap.distribute(key);
            }
        }
        address eventAddr = getEventAddr();
    ...
    function setCap(uint256 _softCap, uint256 _hardCap) external onlyRole(MANAGER_ROLE) {
        require(_softCap > 0 &&_hardCap > 0, "Values cannot be equal to zero");
        softCap = _softCap;
        hardCap = _hardCap;
        emit SetGoal(_softCap,_hardCap);
    }
        ...
        returns (AllInvestment[] memory)
    {
        ...
        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            ...
            investmentsStruct[i] = AllInvestment(
                ...
                investorMap.getSecurityTokenAmountFor(key)
            );
        }
...
library ContribMap {
    ...
    function refund(InvestorMap storage map, address key) external {
        map.refunded[key] = true;
    }

    function distribute(InvestorMap storage map, address key) external {
        map.distributed[key] = true;
    }
    ...
    function isRefunded(InvestorMap storage map, address key)
        external
        view
        returns (bool)
    {
        return map.refunded[key];
    }

    function isDistributed(InvestorMap storage map, address key)
        external
        view
        returns (bool)
    {
