...
contract PublicSale is Ownable, Pausable {
    ...
    // User allocation
    struct Allocation {
        address owner;                  // allocation owner
        uint256 usdtAmount;             // amount in usdt
        uint256 boughtTokens;           // bought tokens
        uint256 partialUnlockAmount;    // partial unlocking amount
        uint256 claimedTokens;          // claimed tokens
        uint256 lastClaimId;            // last claim id
        bool claimedFirst;              // claim first unlock
    }
    ...
    IERC20 public token;    // sales token
    IERC20 public USDT;     // stablecoin for payment
    IBlacklist public blacklist;    // blacklist contract
    address public paymentVault;    // wallet for accepting payment
    
    uint256 public startTs;     // timestamp start of sales
    uint256 public endTs;       // timestamp end of sales
    uint256 public amountToken; // number of tokens for sale
    uint256 public tokenSold;   // tokens sold
    uint256 private totalDeposited; // total stablecoin deposits

    // tokenPrice = 2
    // tokenDecimals = 100
    // 1 token = 0.02 USDT

    uint256 public tokenPrice;          // price token
    uint256 public tokenPriceDecimals;  // decimal place

    uint256 public maxPerUser;          // maximum tokens to be purchased per 1 user       
    uint256 public unlockPeriodTime = 30 days;  // Unlocking period
    uint256 public firstUnlockPercent = 20;     // Percentage of first unlock
    uint256 public vestingPeriodMonth = 12;     // months of vesting
    
    bool private isBurned; // whether unsold tokens were burned after sale

    mapping(address => Allocation) public allocations; // list of all allocations (user => allocation)
