...
// error For the allowed amount of mint
error LessOrMoreAllowed(uint256 input);

// error To avoid exceeding the Max Supply
error MaxSupply(uint256 maxSupply, uint256 userTotal);

contract blockCity is ERC721, ERC721URIStorage, Ownable {
    ...
    uint256 public PRICE_PUBLIC = 500000000000000000 wei; // 0.5 ether
    ...
    // set price wei for mint nfts
    function setPublicMintPrice (uint priceWei) public onlyOwner
    {
        PRICE_PUBLIC = priceWei;
    }

    // set status public mint for users
    function setPublicMintStatus(bool _status) public onlyOwner {
        publicMintStatus = _status;
    }

    // Withdrawing the balance of the contract for the cost of mints
    ...
    // Setting the metadata folder of NFTs
    ...
    function Public_Mint(uint256 _count) public payable mintOpen {
        ...
        require(msg.value == PRICE_PUBLIC * _count, "Value is over or under price.");
    ...
    // return total suplly collection
    function totalSupply() public view returns (uint256) {
        return mintCounter;
    }

    // return metadata folder NFTs
