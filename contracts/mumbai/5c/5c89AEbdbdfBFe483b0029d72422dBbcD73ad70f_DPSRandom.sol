...
contract DPSRandom is Ownable {
    ...
    function getRandomBatch(
        ...
        require(_blockNumber.length == _blockTimestamp.length, "Array length mismatch");
        require(_blockNumber.length == _signature.length, "Array length mismatch");
        require(_blockNumber.length == _entropy.length, "Array length mismatch");
