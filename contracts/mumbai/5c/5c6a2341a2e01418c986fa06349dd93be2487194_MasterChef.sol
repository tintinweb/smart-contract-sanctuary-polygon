...
/**
 *Official MasterChef smart contract of DeFiFarm.co
*/
...
// DeFiFarm Token with Governance.
contract DeFiToken is ERC20('DeFi', 'DFF') {
...
contract SyrupBar is ERC20('SyrupBar Token', 'SYRUP') {
    ...
    // Safe DeFi transfer function, just in case if rounding error causes pool to not have enough CAKEs.
    function safeDeFiTransfer(address _to, uint256 _amount) public onlyOwner {
...
interface IMigratorChef {
    // Perform LP token migration from legacy PanDeFiSwap to DeFiSwap.
    // Take the current LP token address and return the new LP token address.
    // Migrator should have full access to the caller's LP token.
    // Return the new LP token address.
    //
    // XXX Migrator must have allowance access to PanDeFiSwap LP tokens.
    // DeFiSwap must mint EXACTLY the same amount of DeFiSwap LP tokens or
    // else something bad will happen. Traditional PanDeFiSwap does not
...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accDeFiPerShare; // Accumulated CAKEs per share, times 1e12. See below.
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 DeFiReward = multiplier.mul(DeFiPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        DeFi.mint(devaddr, DeFiReward.div(10));
        DeFi.mint(address(syrup), DeFiReward);
    ...
    function safeDeFiTransfer(address _to, uint256 _amount) internal {
        syrup.safeDeFiTransfer(_to, _amount);
    }
