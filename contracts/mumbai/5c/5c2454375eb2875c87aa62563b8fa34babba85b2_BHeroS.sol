...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
   ...
   * request Create Token: 1: countHero; 0: category; isHeroS: 1
   */
  function fusion(uint256[] calldata mainMaterials, uint256[] calldata buffMaterials)
    external
    canFusion(mainMaterials, buffMaterials)
  {
    uint256 rarityMain;
    uint256 rarityTarget;
    uint256[] memory dropRateOption = new uint256[](6);
    (rarityMain, rarityTarget, dropRateOption) = _calculatorFusion(mainMaterials, buffMaterials);
    address to = msg.sender;
    requestCreateToken(to, 1, BHeroDetails.ALL_RARITY, 0, 1, dropRateOption);
    //add data fusion: isFusion = true
    bHeroToken.addFusionData(to, rarityTarget, true);
  ...
  function _calculatorFusion(uint256[] calldata mainMaterials, uint256[] calldata buffMaterials)
    internal
    view
    returns (
      uint256,
  ...
  function _getPercentBuffMaterials(uint256 rarityTarget, uint256[] calldata buffMaterials)
    internal
    view
    returns (uint256)
  {
...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function processTokenRequests() external returns (bool) {
    address to = msg.sender;
    ...
    uint256 countFalse; //count fusion number false
    CreateTokenRequest[] storage requests = tokenRequests[to];
    for (uint256 i = requests.length; i > 0; --i) {
      ...
      uint256 rarityNewHero = BHeroDetails.decodeRarity(details[0]);
      if (!fusionData[to][i - 1].isFusion || fusionData[to][i - 1].rarityTarget == rarityNewHero) {
        createRandomToken(to, details);
      } else {
        countFalse++;
        emit FusionFalse(to, countFalse, fusionData[msg.sender][i - 1].rarityTarget, rarityNewHero, details[0]);
      }
  ...
  function addFusionData(
    address to,
    uint256 rarityTarget,
    bool isFusion
  ) external onlyRole(MINTER_ROLE) {
    uint256 index = tokenRequests[to].length;
    fusionData[to][index].rarityTarget = rarityTarget;
    fusionData[to][index].isFusion = isFusion;
  }
}
