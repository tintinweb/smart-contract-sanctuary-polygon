...
// File: contracts/inocyx/InocyxMarketplace.sol
...
// 0x5c9c886d6074520CAEDd60729D286153A68930E9
// interface NftContractInterFace {
//   function getCreator(uint256) external returns (address);
// }

contract InocyxMarketplace is Ownable, ReentrancyGuard {
  ...
  function getCurrentListingID() public view virtual returns (uint256) {
    return _listingId;
  }
  ...
  function createListing(
    ...
    if (!isErc721) {
      ...
      for (uint256 i; i < tokenIds.length; ) {
        require(
          IERC1155(nftAddress).balanceOf(seller, tokenIds[i]) >= amount,
    ...
    address _creator = contractOwner;
    ...
    nonReentrant
  {
    ...
    if (listing.isErc721) {
      for (uint256 i; i < listing.tokenIds.length; i++) {
        if (
          IERC721(listing.nftAddress).getApproved(listing.tokenIds[i]) !=
          address(this)
        ) {
          // if (msg.value > 0) {
          //   Address.sendValue(payable(msg.sender), msg.value);
          // }
          revert(
            "Asset is not owned by this listing. Probably was not approved by the smart contract, or was already sold."
          );
        }
      }
    } else {
      for (uint256 i; i < listing.tokenIds.length; i++) {
        if (
          ...
          // }
          revert(
            "Insufficient balance of the asset in this listing. Probably was not approved by the smart contract, or was already sold."
          );
        }
      }
    }
