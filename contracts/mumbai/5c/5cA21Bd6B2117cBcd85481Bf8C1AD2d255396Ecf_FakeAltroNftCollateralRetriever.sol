...
/**
 * @title FakeAltroNftCollateralRetriever
...
contract FakeAltroNftCollateralRetriever is ERC721HolderUpgradeable, OwnableUpgradeable {
	...
	modifier onlyMinter(address nftContract) {
		require(IAccessControl(nftContract).hasRole(MINTER_ROLE, _msgSender()), "FakeAltroNftCollateralRetriever: only Minter can burn NFT");
		_;
	}
	...
	function burnNft(address nftContract, uint256 tokenId) external onlyMinter(nftContract) {
		require(FakeAltroNftCollectionFactory(nftCollectionFactory).isAKnownCollection(nftContract), "FakeAltroNftCollateralRetriever: unknown collection");
	...
	function initialize(address nftCollectionFactory_, address feeManager_) external initializer {
		__FakeAltroNftCollateralRetriever_init(nftCollectionFactory_, feeManager_);
	}
	...
	function onERC721Received(address operator, address from, uint256 tokenId, bytes memory data) public override returns (bytes4) {
		require(FakeAltroNftCollectionFactory(nftCollectionFactory).isAKnownCollection(msg.sender), "FakeAltroNftCollateralRetriever: unknown collection");
		uint256 deadline = INftCollectionVaultService(msg.sender).getVaultServiceDeadline(tokenId);
		require(deadline > block.timestamp, "FakeAltroNftCollateralRetriever: vault service expired");
		require(IFeeManager(feeManager).isRedemptionFeePaid(msg.sender, tokenId), "FakeAltroNftCollateralRetriever: platform fee has not been paid");
	...
	function _setNftCollectionFactory(address nftCollectionFactory_) internal {
		require(nftCollectionFactory_ != address(0), "FakeAltroNftCollateralRetriever: cannot be null address");
		require(
			nftCollectionFactory_.supportsInterface(type(INftCollectionFactory).interfaceId),
			"FakeAltroNftCollateralRetriever: does not support INftCollectionFactory interface"
		);
	...
	function _setFeeManager(address feeManager_) internal {
		require(feeManager_ != address(0), "FakeAltroNftCollateralRetriever: cannot be null address");
		require(feeManager_.supportsInterface(type(IFeeManager).interfaceId), "FakeAltroNftCollateralRetriever: does not support IFeeManager interface");
		feeManager = feeManager_;
	}

	/**
	 * @dev Initializes the FakeAltroNftCollateralRetriever contract.
	 * @param nftCollectionFactory_ The address of the NFT collection factory contract.
	 * @param feeManager_ The address of the fee manager contract.
	 */
	function __FakeAltroNftCollateralRetriever_init(address nftCollectionFactory_, address feeManager_) internal initializer {
		__Context_init();
		__Ownable_init();
		__ERC721Holder_init();
		__FakeAltroNftCollateralRetriever_init_unchained(nftCollectionFactory_, feeManager_);
	}

	/**
	 * @dev Initializes the FakeAltroNftCollateralRetriever contract without calling the internal chainable init functions.
	 * @param nftCollectionFactory_ The address of the NFT collection factory contract.
	 * @param feeManager_ The address of the fee manager contract.
	 */
	function __FakeAltroNftCollateralRetriever_init_unchained(address nftCollectionFactory_, address feeManager_) internal initializer {
		_setNftCollectionFactory(nftCollectionFactory_);
		_setFeeManager(feeManager_);
	}
}
 ...
 */
contract FakeAltroNftCollection is ERC721URIStorage, ERC721Enumerable, ERC721Burnable, AccessControl, ReentrancyGuard, INftCollectionVaultService {
	...
	/**
	 * @dev Constructor function that initializes the FakeAltroNftCollection contract
	...
	constructor(
		...
		require(
			...
			"FakeAltroNftCollection: cannot be null address"
		);
		require(insolvencyGracePeriod_ >= minGracePeriod_, "FakeAltroNftCollection: grace period too short");
	...
	function seize(uint256 tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(block.timestamp > _vaultServiceDeadline[tokenId] + insolvencyGracePeriod, "FakeAltroNftCollection: cannot seize token");
	...
	function setVaultServiceDeadline(uint256 tokenId, uint256 deadline) external onlyRole(VAULT_MANAGER_ROLE) {
		require((deadline > _vaultServiceDeadline[tokenId] && deadline > block.timestamp), "FakeAltroNftCollection: new deadline is lower than the current one");
	...
	function setInsolvencyGracePeriod(uint256 insolvencyGracePeriod_) external onlyRole(DEFAULT_ADMIN_ROLE) {
		require(insolvencyGracePeriod_ >= minGracePeriod, "FakeAltroNftCollection: grace period too short");
		insolvencyGracePeriod = insolvencyGracePeriod_;
...
/**
 * @title FakeAltroNftCollectionFactory
 * @author FakeAltro Developer
 * @dev Contract for generating FakeAltro NFT collections
 */
contract FakeAltroNftCollectionFactory is ERC165Upgradeable, OwnableUpgradeable, INftCollectionFactory {
	using ERC165CheckerUpgradeable for address;
	using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

	/**
	 * @dev Struct that contains information about an FakeAltroNftCollection contract created by this factory
	 * @param collection address of the FakeAltroNftCollection contract
	 * @param symbol symbol of the FakeAltroNftCollection
	 * @param name name of the FakeAltroNftCollection
	 * @param oracle address of the oracle used by the FakeAltroNftCollection
	...
	/**
	 * @dev Set of addresses of created FakeAltroNftCollection contracts
	 */
	EnumerableSetUpgradeable.AddressSet private _collectionAddressSet;

	/**
	 * @dev Array that contains information about all the FakeAltroNftCollection contracts created by this factory
	 */
	...
	function createCollection(
		...
		require(oracle != address(0) && vaultManager != address(0), "FakeAltroNftCollectionFactory: cannot be null address");
		require(insolvencyGracePeriod >= minGracePeriod, "FakeAltroNftCollectionFactory: grace period too short");
		require(
			ILicenseManager(licenseManager).isAQualifiedOracle(oracle) && ILicenseManager(licenseManager).isAQualifiedOracle(vaultManager),
			"FakeAltroNftCollectionFactory: requirements to become oracle not met"
		);
		...
		require(success, "FakeAltroNftCollectionFactory: adding collection address to set fails");
	...
	function initialize(address licenseManager_, address nftReserveAddress_) external initializer {
		__FakeAltroNftCollectionFactory_init(licenseManager_, nftReserveAddress_);
	}
	 ...
	 */
	function __FakeAltroNftCollectionFactory_init(address licenseManager_, address nftReserveAddress_) internal onlyInitializing {
		__Ownable_init();
		__FakeAltroNftCollectionFactory_init_unchained(licenseManager_, nftReserveAddress_);
	}
	 ...
	 * @param nftReserveAddress_ address The address of the NFT reserve contract
	 */
	function __FakeAltroNftCollectionFactory_init_unchained(address licenseManager_, address nftReserveAddress_) internal onlyInitializing {
		_setLicenseManager(licenseManager_);
		_setNftReserveAddress(nftReserveAddress_);
		tokenVersion = "1";
	}
	...
	function _setLicenseManager(address licenseManager_) internal {
		require(licenseManager_ != address(0), "FakeAltroNftCollectionFactory: cannot be null address");
		require(licenseManager_.supportsInterface(type(ILicenseManager).interfaceId), "FakeAltroNftCollectionFactory: does not support ILicenseManager interface");
	...
	function _setNftReserveAddress(address nftReserveAddress_) internal {
		require(nftReserveAddress_ != address(0), "FakeAltroNftCollectionFactory: cannot be null address");
		require(nftReserveAddress_.supportsInterface(type(INftReserve).interfaceId), "FakeAltroNftCollectionFactory: does not support INftReserve interface");
		nftReserveAddress = nftReserveAddress_;
	}
}
