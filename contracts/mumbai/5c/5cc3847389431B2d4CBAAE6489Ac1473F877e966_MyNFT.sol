...
contract BridgeNFT is OwnableUpgradeable{
    ...
    function setDestNftAddr(address _srcNft, uint256[] calldata _dstChIds, address[] calldata _dstNfts) external onlyAdmin{
    ...
    function setTxFee(uint256[] calldata _chids, uint256[] calldata _fees) external onlyAdmin {
    ...
    function bridge(address _srcNft, uint256 _tokenId, uint256 _dstChId, address _receiver) external payable{
        ...
        if(payee != address(0) && msg.value > 0) {
            require(payable(payee).send(msg.value), "tx fee collection failed");
        }
    ...
    function bridgeCallBack(address _sender, address _srcNft, uint256 _tokenId) external onlyAdmin {
        if(origNFT[_srcNft]){
            require(IBridgeNFT(_srcNft).ownerOf(_tokenId) == address(this), "tokenid owner is not bridge");
            IBridgeNFT(_srcNft).transferFrom(address(this), _sender, _tokenId);
            require(IBridgeNFT(_srcNft).ownerOf(_tokenId) == _sender, "callback NFT failed");
        } else {
    ...
    function sendTo(uint256 _srcChid, address _dstNft, uint256 _tokenId, address _receiver) external onlyAdmin{
        if (origNFT[_dstNft]) {
            IBridgeNFT(_dstNft).transferFrom(address(this), _receiver, _tokenId);
            require(IBridgeNFT(_dstNft).ownerOf(_tokenId) == _receiver, "transfer NFT failed");
        } else {
...
contract Erc721Proxy is OwnableUpgradeable{
    mapping (address => bool) public admin;
    uint256 public fee; 

    event PreMint(address from, address srcNft, uint256 tokenID);
    ...
    function preMint(address _srcNft, uint256 _tokenId) external payable {
        ...
        try IErc721Proxy(_srcNft).ownerOf(_tokenId) returns (address owner) {
            if(owner == address(0)){
                pass_ = true;
            }
        } catch {
        ...
        require(pass_, "tokenId always minted or failed to ownerOf");
