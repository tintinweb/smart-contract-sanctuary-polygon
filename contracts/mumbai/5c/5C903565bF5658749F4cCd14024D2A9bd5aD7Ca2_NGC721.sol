...
// File: Contracts\FiveTigerNFT.sol

pragma solidity ^0.8.0;


contract NGC721 is ERC721Enumerable, Ownable {
    struct Miner {
        string name;
        bool exist;
        bool enable;
    ...
    event MinerEnable(address indexed miner, uint256 totalSupply, bool enable);
    
    constructor(string memory name, string memory symbol, string memory uri, uint256 _maxSupply) ERC721(name, symbol) {
        _uri = uri;
    ...
    function mint(address to) external returns(uint256 tokenId) {
        ...
        require(totalSupply() < maxSupply, "NFT:over than maxSupply");
        tokenId = totalSupply();
        _mint(to, tokenId);
    }
