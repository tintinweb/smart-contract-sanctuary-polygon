...
// File contracts/token/onft/extension/UniversalONFT.sol
...
// File contracts/examples/AwesomeNFTs.sol
...
contract AWEA is UniversalONFT {
    mapping (uint256 => string) private _tokenURIs;

    constructor(address _layerZeroEndpoint, uint _startMintId, uint _endMintId)
    UniversalONFT("Awesome Assets", "AWEA", _layerZeroEndpoint, _startMintId, _endMintId) {}

    function setTokenURI(uint256 tokenId, string memory _metadataURI) external onlyOwner() {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
