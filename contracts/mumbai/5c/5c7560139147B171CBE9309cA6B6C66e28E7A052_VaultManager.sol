...
contract VaultManager is OrumBase, Ownable, CheckContract, IVaultManager {
    ...
    */
    function batchLiquidateVaults(address[] memory _vaultArray) public override {
        ...
        if (vars.recoveryModeAtStart) {
            totals = _getTotalFromBatchLiquidateRecoveryMode(activePoolCached, vars.price,  _vaultArray);
        } else { 
            totals = _getTotalsFromBatchLiquidateNormalMode(activePoolCached, vars.price, _vaultArray);
        }
        ...
        activePoolCached.swapoMATICAndSendMATIC(orumRevenueAddress, totals.totalCollToSendToOrumRevenue);
        emit TEST_liquidationfee(totals.totalCollToSendToSP, totals.totalCollToSendToOrumRevenue);

        if (totals.totalCollSurplus > 0) {
            activePoolCached.swapoMATICAndSendMATIC(address(collSurplusPool), totals.totalCollSurplus);
        }
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _vaultArray.length; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                singleLiquidation = _liquidateRecoveryMode(_activePool, vars.user, vars.ICR, TCR, _price);
                ...
                singleLiquidation = _liquidateNormalMode(_activePool, vars.user, _price);
                // Add liquidation values to their respective running totals
                totals = _addLiquidationValuesToTotals(totals, singleLiquidation);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        (singleLiquidation.entireVaultDebt, singleLiquidation.entireVaultColl) = getEntireDebtAndColl(_borrower);
        ...
        singleLiquidation.collToSendToOrumRevenue) = _getOffsetValues(singleLiquidation.entireVaultDebt, collToLiquidate, _price);
        ...
        returns (LiquidationValues memory singleLiquidation)
    {
        ...
        if (_ICR <= _100pct) {
             (singleLiquidation.debtToOffset,
            singleLiquidation.collToSendToSP,
            singleLiquidation.collToSendToOrumRevenue) = _getOffsetValues(singleLiquidation.entireVaultDebt, vars.collToLiquidate, _price);
        ...
        returns(LiquidationTotals memory totals)
    {
        ...
        for (vars.i = 0; vars.i < _n && vars.user != firstUser; vars.i++) {
            ...
            if (!vars.backToNormalMode) {
                ...
                uint TCR = OrumMath._computeCR(vars.entireSystemColl, vars.entireSystemDebt, _price);
                singleLiquidation = _liquidateRecoveryMode(_contractsCache.activePool, vars.user, vars.ICR, TCR, _price);
                // Update aggregate trackers
                vars.entireSystemDebt = vars.entireSystemDebt.sub(singleLiquidation.debtToOffset);
                vars.entireSystemColl = vars.entireSystemColl.
                    sub(singleLiquidation.collToSendToSP).
                ...
                singleLiquidation = _liquidateNormalMode(_contractsCache.activePool, vars.user, _price);
    ...
    */
    function liquidateVaults(uint _n) external override {
        ...
        if (vars.recoveryModeAtStart) {
            ...
            totals = _getTotalsFromLiquidateVaultsSequenceNormalMode(contractsCache.activePool, vars.price, _n);
        }
        require(totals.totalDebtInSequence > 0, "VaultManager: nothing to liquidate");
        ...
        stabilityPool.offset(totals.totalDebtToOffset, totals.totalCollToSendToSP);
        // Send fees to the orum revenue
        contractsCache.activePool.swapoMATICAndSendMATIC(orumRevenueAddress, totals.totalCollToSendToOrumRevenue);
        // Only for testing
        emit TEST_liquidationfee(totals.totalCollToSendToSP, totals.totalCollToSendToOrumRevenue);

        if (totals.totalCollSurplus > 0) {
            contractsCache.activePool.swapoMATICAndSendMATIC(address(collSurplusPool), totals.totalCollSurplus);
        }
        vars.liquidatedDebt = totals.totalDebtInSequence;
        vars.liquidatedColl = totals.totalCollInSequence.sub(totals.totalCollGasCompensation).sub(totals.totalCollSurplus);
        emit Liquidation(vars.liquidatedDebt, vars.liquidatedColl, totals.totalCollGasCompensation, totals.totalUSDCGasCompensation);
    ...
    */
    ...
    (
        ...
        internal
        // view
        returns (uint debtToOffset, uint collToSendToSP, uint collToSendToOrumRevenue)
    {
        /*
        ...
        */
            ...
            emit Coll_getOffsetValues(_debt, _coll);


            // get the debt to offset in terms of the collateral i.e, oMATIC
            uint _debtInoMATIC = debtToOffset.mul(DECIMAL_PRECISION).div(_price);
            uint _totalCollProfit ;
            if (collToSendToSP > _debtInoMATIC) {
               _totalCollProfit = 0;
               collToSendToOrumRevenue = 0;
               collToSendToSP = collToSendToSP;
            ...
            emit TEST_offsetValues(_debtInoMATIC, debtToOffset, collToSendToSP, collToSendToOrumRevenue, _totalCollProfit);
    }

    function _sendGasCompensation(IActivePool _activePool, address _liquidator, uint _USDC, uint _oMATIC) internal {
        if (_USDC > 0) {
        usdcToken.returnFromPool(gasPoolAddress, _liquidator, _USDC);
        }

        if (_oMATIC > 0) {
            _activePool.swapoMATICAndSendMATIC(_liquidator, _oMATIC);
        }
    }
    ...
    function _getCurrentVaultAmounts(address _borrower) internal view returns (uint, uint) {
        uint currentoMATIC = Vaults[_borrower].coll;
