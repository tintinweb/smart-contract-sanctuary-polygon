    ...
    Pausable
{
    ...
    constructor() ERC20("Foot Earn Token", "FEN") {
...
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import {FootEarn} from"./FootEarn.sol";
import "./FEN.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";

contract StarterPack is
    Initializable,
    AccessControlEnumerable,
    ReentrancyGuardUpgradeable
{
    ...
    modifier checkOpenBox(uint256 index) {
        ...
        require(
            FootEarn(FactoryNFT).hasRole(MINTER_ROLE, address(this)) == true,
    ...
    function openBox(uint256 boxIndex) public checkOpenBox(boxIndex) {
        ...
        for (
            uint256 i = buyHistory[boxIndex].fromId;
            i <= buyHistory[boxIndex].toId;
            i++
        ) {
            FootEarn(FactoryNFT).mint(_msgSender(), i);
        }
        buyHistory[boxIndex].isOpen = true;
        emit OpenBoxSuccess(
            boxIndex,
            buyHistory[nonce].fromId,
            buyHistory[nonce].toId,
            _msgSender()
        );
    }

    function openBox2(uint256 boxIndex) public  {
       FootEarn(FactoryNFT).mint(_msgSender(), 0);
        // for (
        //     uint256 i = buyHistory[boxIndex].fromId;
        //     i <= buyHistory[boxIndex].toId;
        //     i++
