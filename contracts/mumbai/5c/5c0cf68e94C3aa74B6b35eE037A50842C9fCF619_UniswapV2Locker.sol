...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
  ...
  struct FeeStruct {
    ...
    IERCBurn secondaryFeeToken; // ZKMORPH token
    uint256 secondaryTokenFee; // optional, ZKMORPH token
    ...
    if (!feeWhitelist.contains(msg.sender)) {
      if (_fee_in_eth) { // charge fee in eth
        ...
        if (ethFee != 0 && _referral != address(0)) { // referral fee
          ...
          (bool successR, ) = _referral.call{value: referralFee}("");
          require(successR, "REFERRAL FEE TRANSFER FAILED");
          devFee = devFee.sub(referralFee);
        }
        (bool success, ) = devaddr.call{value: devFee}("");
      ...
      (bool successTransfer, ) = msg.sender.call{value: msg.value}("");
      require(successTransfer, "FAILED TO TRANSFER");
    }
  ...
  function splitLock (address _lpToken, uint256 _index, uint256 _lockID, uint256 _amount) external payable nonReentrant {
    ...
    (bool success, ) = devaddr.call{value: gFees.ethFee}("");
    require(success, "FEE TRANFER FAILED");
    userLock.amount = userLock.amount.sub(_amount);
