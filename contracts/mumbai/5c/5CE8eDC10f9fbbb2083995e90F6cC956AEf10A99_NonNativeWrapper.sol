...
contract NonNativeWrapper is ITokenBridgeable, Initializable, OwnableUpgradeable, INonNativeWrapperInitializable {
    ERC721 public originalContract;
    SystemContext public systemContext;

    bytes32 public constant BRIDGE_ROLE = keccak256("BRIDGE_ROLE");
    bytes32 public constant TOKEN_UNLOCK_ROLE = keccak256("TOKEN_UNLOCK_ROLE");

    function initialize(bytes memory initializerEncoded_) initializer public {
        (address systemContext_, address originalContract_) = decodeInitializer_NonNativeWrapper(initializerEncoded_);
        systemContext = SystemContext(systemContext_);
        originalContract = ERC721(originalContract_);
        __Ownable_init();
    }

    function decodeInitializer_NonNativeWrapper(bytes memory initializerEncoded_) override pure public returns (address, address ){
       return abi.decode(initializerEncoded_, (address, address));
    }

    function encodeInitializerParams(address systemContext_, address originalContract_) public pure returns (bytes memory) {
        return abi.encode(systemContext_, originalContract_);
    }

    function encodeInitializer(address systemContext_, address originalContract_) override public pure returns (bytes memory){
        bytes memory params = encodeInitializerParams(systemContext_, originalContract_);
    ...
    function moveTo(uint16 _l0ChainId, bytes calldata _destinationBridge, uint256 _tokenId, uint256 _gasAmount) external override payable {
        originalContract.transferFrom(msg.sender, address(this), _tokenId);

        systemContext.getBridge().mintOnTargetChain{value : msg.value}(
            _l0ChainId, _destinationBridge, msg.sender, msg.sender, _tokenId, originalContract.tokenURI(_tokenId),_gasAmount
        );
    }
    ...
    * @dev Returns collection identifier`
    ...
    function collectionId() public view override returns (bytes32) {
        return systemContext.getCollectionRegistry().collections(address(this));
    }

    /**
    * @dev Unlocks an existing token.
     * @param _to address of the future owner of the token
     * @param _tokenId new token id
     */
    function mintTo(address _to, uint256 _tokenId) public {
        systemContext.getAccessControlList().checkRoleAddr(BRIDGE_ROLE, msg.sender);
        require(originalContract.ownerOf(_tokenId) == address(this), "Wrapper is not an owner of that token");

        originalContract.transferFrom(address(this), _to, _tokenId);
    }

    /**
    * @dev Unlocks token when stuck
     */
    function unlockToken(address _to, uint256 _tokenId) external override {
        systemContext.getAccessControlList().checkRoleAddr(TOKEN_UNLOCK_ROLE, _msgSender());
        originalContract.transferFrom(address(this), _to, _tokenId);
    }
...
import "./factory/ContractFactory.sol";
...
contract SystemContext is SystemContextSubscribable {
    ...
    modifier onlyRole(bytes32 role_) {
        accessControlList.checkRoleAddr(role_, msg.sender);
        _;
    }
    ...
    function setBridge(LayerZeroBridge bridgeContract_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        ...
        accessControlList.grantRoleAddr(accessControlList.BRIDGE_ROLE(), address(bridgeContract_));
    }
    ...
    function setContractFactory(ContractFactory contractFactory_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        ...
        accessControlList.grantRoleAddr(accessControlList.CONTRACT_FACTORY_ROLE(), address(contractFactory_));
    }
    ...
    function setCollectionRegistry(CollectionsRegistry collectionRegistry_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        ...
        accessControlList.grantRoleAddr(accessControlList.COLLECTION_REGISTRY_ROLE(), address(collectionRegistry_));
    }
    ...
    function setAccessControlList(AccessControlList accessControlList_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        ...
        accessControlList = accessControlList_;
        accessControlList.grantRoleAddr(accessControlList.ACCESS_CONTROL_ROLE(), address(accessControlList_));
    }
    ...
    function setOwnerVerifier(OwnerVerifier ownerVerifier_, bool triggerCallbacks) external onlyRole(accessControlList.SYSTEM_CONTEXT_DEFAULT_ADMIN_ROLE()) {
        ...
        accessControlList.grantRoleAddr(accessControlList.OWNER_VERIFIER_ROLE(), address(ownerVerifier_));
    }
...
interface INonNativeWrapperInitializable {
    ...
    function encodeInitializer(address systemContext_, address originalContract_) external pure returns (bytes memory);

    function decodeInitializer_NonNativeWrapper(bytes memory initializerEncoded_) external pure returns (address systemContext_, address originalContract_);
}
...
contract LayerZeroBridge is ILayerZeroReceiver, IBridge {
    ...
    string public ERC721_BLUEPRINT_NAME = "ERC721Mintable";
    string public ERC721_WITH_SLOTS_BLUEPRINT_NAME = "ERC721MintableWithSlots";
    string public ERC721_NON_NATIVE_WRAPPER_BLUEPRINT_NAME = "NonNativeWrapper";
    ...
    modifier onlyRole(bytes32 role_) {
        systemContext.getAccessControlList().checkRoleAddr(role_, msg.sender);
        _;
    }
    ...
    function setMinGas(uint256 minGas_) external onlyRole(systemContext.getAccessControlList().BRIDGE_DEFAULT_ADMIN_ROLE()) {
        minGas = minGas_;
    }
    ...
    function _deployNonNativeWrapper(DeployExternalParams memory deployData, bytes32 collectionId) internal {
        ...
        INonNativeWrapperInitializable wrapperBlueprint = INonNativeWrapperInitializable(factory.getLatestBlueprint(ERC721_NON_NATIVE_WRAPPER_BLUEPRINT_NAME));
        factory.createContractInstanceByName(
            ERC721_NON_NATIVE_WRAPPER_BLUEPRINT_NAME, wrapperBlueprint.encodeInitializer(address(systemContext), deployData.originalCollection), collectionId, deployData.collectionName, deployData.owner
        );
    ...
    function _handleDeploy(DeployData memory deployData, uint16 srcChainId, bytes memory fromAddress, uint64 nonce) internal {
        ...
        bytes memory rawCallData = abi.encodeWithSelector(
            contractFactory.createContractInstanceByName.selector, deployData.blueprintName,
    ...
    function lzReceive(
        ...
        if (systemContext.chainId() < 10000) {
            acl.checkRole(acl.BRIDGE_ROLE(), _fromAddress);
        }
        address(this).call(abi.encodeWithSelector(this.handleAll.selector, _srcChainId, _fromAddress, _nonce, _payload));
    }
    ...
    function _sendMessageWithValue(uint16 chainId_, bytes calldata bridge_, bytes memory buffer_, address refundAddress_, uint256 value_, uint256 gasAmount_) internal {
        require(gasAmount_ >= minGas, "Gas limit too little");
    ...
    function deployContractEncode(string memory blueprintName_, bytes32 collectionId_, bytes memory ctorParams_, string calldata collectionName_, address owner_) public view returns (bytes memory) {
        return abi.encode(
            Data({
    ...
    function deployExternalCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            _sendMessageWithValue(
                ...
                deployContractEncode(ERC721_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].ctorParams, params.collectionName, params.owner),
    ...
    function deployNativeCollection(
        ...
        for (uint256 i = 0; i < deploymentParams_.length; i++) {
            ...
            if(deploymentParams_[i].chainId == systemContext.chainId()) {
                systemContext.getContractFactory().createContractInstanceByName(
                    ERC721_WITH_SLOTS_BLUEPRINT_NAME, deploymentParams_[i].ctorParams, collectionId_, params.collectionName, params.owner);
            } else {
                _sendMessageWithValue(
                    deploymentParams_[i].chainId,
                    deploymentParams_[i].bridgeAddress,
                    deployContractEncode(ERC721_WITH_SLOTS_BLUEPRINT_NAME, collectionId_, deploymentParams_[i].ctorParams, params.collectionName, params.owner),
                    params.refundAddress,
                    deploymentParams_[i].value,
                    params.gasAmount
                );
...
contract ContractFactory {
    event NewBlueprintRegistered(address indexed blueprintAddress, string blueprintName);
    event NewBlueprintVersionRegistered(address indexed blueprintAddress, string blueprintName, uint16 version);
    ...
    modifier onlyBridgeRole() {
        if (systemContext.chainId() < 10000) {
            AccessControlList acl = systemContext.getAccessControlList();
            acl.checkRoleAddr(acl.BRIDGE_ROLE(), msg.sender);
        }
        _;
    }
     ...
     */
    function registerNewBlueprint(address blueprintAddress, string memory blueprintName, bool isNative) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        require(!blueprintExists(blueprintName), "Contract already registered");
     ...
     */
    function registerNewBlueprintVersion(address blueprintAddress, string memory blueprintName, uint16 version, bool forceUpdate) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        require(blueprintExists(blueprintName), "Blueprint doesn't exist.");
        require(blueprints[blueprintName][version-1] != address(0), "Blueprint with previous version doesn't exist");
        require(blueprints[blueprintName][version-1] != blueprintAddress, "Previous blueprint is already registered with this address");
        require(version == latestBlueprintVersion[blueprintName] + 1, string(
                abi.encodePacked("Next '", blueprintName, "' should have version '", latestBlueprintVersion[blueprintName] + 1, "'")
            ));
        ...
        emit NewBlueprintVersionRegistered(blueprintAddress, blueprintName, version);
    }

    /**
     * @dev Removes latest blueprint version
     */
    function deregisterLatestBlueprint(string memory name, bool forceUpdate) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        require(blueprintExists(name), "Blueprint doesn't exist.");
        if (latestBlueprintVersion[name] > 1) {
            delete blueprints[name][latestBlueprintVersion[name]];
            latestBlueprintVersion[name]--;
            beacons[name].upgradeTo(blueprints[name][latestBlueprintVersion[name]], forceUpdate);
        }
        else {
    ...
    function blueprintExists(string memory name) public view returns (bool){
        return latestBlueprintVersion[name] != 0 && blueprints[name][1] != address(0);
    }
    /**
     ...
     */
    function _deployProxy(string memory blueprintName, bytes memory constructorParams) internal returns(address) {
        return address(new BeaconProxy(address(beacons[blueprintName]), constructorParams));
    }
     ...
     */
    function createContractInstanceByName(string memory blueprintName, bytes memory constructorParams, bytes32 collectionId, string calldata name, address owner) external onlyBridgeRole returns(address) {
        require (blueprints[blueprintName][latestBlueprintVersion[blueprintName]]!=address(0), "Blueprint not registered");

        address newContract = _deployProxy(blueprintName, constructorParams);
    ...
    function _registerToken(address newContract, string memory blueprintName, bytes32 collectionId, string calldata name, address owner) internal {
        systemContext.getCollectionRegistry().registerCollection(collectionId, name, owner, newContract,
            blueprintName, latestBlueprintVersion[blueprintName]);
    }

    function getLatestBlueprint(string calldata blueprintName_) public view returns (address){
        return blueprints[blueprintName_][latestBlueprintVersion[blueprintName_]];
    }
}
...
contract CollectionsRegistry is ICollectionRegistry {
    ...
    modifier onlyFactoryRole() {
        AccessControlList acl = systemContext.getAccessControlList();
        acl.checkRoleAddr(acl.CONTRACT_FACTORY_ROLE(), msg.sender);
        _;
    }
...
import "../SystemContext.sol";
import "../interfaces/ITokenBridgeable.sol";
import "../interfaces/IERC721MintableInitializable.sol";

/**
 * @title ERC721Mintable
 * ERC721Mintable - ERC721 contract that allows to mint tokens
 */
contract ERC721Mintable is ERC721EnumerableUpgradeable, ITokenBridgeable, OwnableUpgradeable, IERC721MintableInitializable {
    ...
    modifier onlyBridgeRole() {
        system.getAccessControlList().checkRoleAddr(BRIDGE_ROLE, _msgSender());
        _;
    }
    ...
    function initialize(bytes memory initializerEncoded_) initializer virtual public {
    (string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) = decodeInitializer_ERC721Mintable(initializerEncoded_);
        _initializeUnchainedInternal(name_, symbol_, contractURI_, owner_, system_);
    }

    function decodeInitializer_ERC721Mintable(bytes memory initializerEncoded_) override public pure returns (string memory, string memory, string memory, address, address){
        return abi.decode(initializerEncoded_, (string, string, string, address, address));
    }

    function _initializeUnchainedInternal(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) onlyInitializing internal {
        system = SystemContext(system_);
    ...
    function encodeInitializerParams(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) public pure returns (bytes memory) {
        return abi.encode(
            name_,
            symbol_,
            contractURI_,
    ...
    function encodeInitializer(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) override public pure returns (bytes memory){
        bytes memory params = encodeInitializerParams(name_, symbol_, contractURI_, owner_, system_);
        return abi.encodeWithSelector(this.initialize.selector, params);
    }
    ...
    * @dev Returns collection identifier`
     */
    function collectionId() external view override returns (bytes32) {
    ...
    * @dev Unlocks token when stuck
     */
    function unlockToken(address _to, uint256 _tokenId) external override {
        system.getAccessControlList().checkRoleAddr(TOKEN_UNLOCK_ROLE, _msgSender());
     ...
     */
    function mintTo(address _to, uint256 _tokenId) public {
        if (system.getAccessControlList().hasRoleAddr(BRIDGE_ROLE, msg.sender)) {
...
interface IERC721MintableInitializable {
//    function initialize_ERC721Mintable(bytes calldata initializerEncoded_) external;
    function encodeInitializer(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) external pure returns (bytes memory);
    function decodeInitializer_ERC721Mintable(bytes calldata initializerEncoded_) external pure returns (string memory, string memory, string memory, address, address);
}
...
contract BeaconProxy is Proxy, ERC1967UpgradeSimplified {
    ...
    constructor(address beacon, bytes memory data) payable {
        require(_BEACON_SLOT == bytes32(uint256(keccak256("eip1967.proxy.beacon")) - 1), "BeaconProxy: _BEACON_SLOT err");
...
contract UpgradeableBeacon is Ownable, IBeacon {
    ...
    * @dev Emitted when new implementation candidate is registered.
    ...
    function _switchIfRequired() internal {
        if (_implementationCandidate != address(0) && block.timestamp >= _nextSwitch) {
     ...
     */
    function upgradeTo(address newImplementation, bool forceUpdate) public virtual onlyOwner {
        _setCandidate(newImplementation);
        if (forceUpdate) {
            _nextSwitch = block.timestamp + 1;
        } else {
            uint256 window = ((block.timestamp + _implementationSwitchDelay) / _implementationSwitchWindow) + 1;
    ...
    * @dev Sets the implementation contract address for this beacon
    ...
    function _setCandidate(address newImplementationCandidate) private {
        require(Address.isContract(newImplementationCandidate), "UpgradeableBeacon: implementation is not a contract");
