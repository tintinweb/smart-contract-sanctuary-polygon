...
contract KeeperArctic is AccessControlledAndUpgradeable, KeeperCompatibleInterface {
  ...
  // NOTE: this function is exactly the same as in the `OracleManagerUtils` library contract. However, foundry does gives errors when trying to deploy this library.
  function getOracleInfoForSystemStateUpdate(
    ...
    (, , uint256 previousOracleUpdateTimestamp, , ) = chainlinkOracle.latestRoundData();

    uint256 epochLength = oracleManager.EPOCH_LENGTH();
    uint256 mewt = oracleManager.MINIMUM_EXECUTION_WAIT_THRESHOLD();

    uint256 latestValidEpoch = ((previousOracleUpdateTimestamp - mewt - oracleManager.initialEpochStartTimestamp()) / epochLength);
    uint256 relevantEpochStartTimestampWithMEWT = oracleManager.initialEpochStartTimestamp() + (_latestExecutedEpochIndex * epochLength) + mewt;

    uint256 _numberOfMissedEpochs = latestValidEpoch - _latestExecutedEpochIndex - 1;
    ...
    while (currentUpdateIndex < _numberOfMissedEpochs) {
      ...
      if (currentOracleUpdateTimestamp == 0 && previousOracleUpdateTimestamp != 0) {
        ...
        while (currentOracleUpdateTimestamp == 0) {
          latestExecutedOracleRoundId = (((latestExecutedOracleRoundId >> 64) + 1) << 64) + 1;
          (, currentOraclePrice, currentOracleUpdateTimestamp, , ) = chainlinkOracle.getRoundData(latestExecutedOracleRoundId);
        }
      }
      ...
      if (
        ...
        _missedEpochOracleRoundIds[currentUpdateIndex] = latestExecutedOracleRoundId;

        relevantEpochStartTimestampWithMEWT += epochLength;
        ++currentUpdateIndex;
      }

      previousOracleUpdateTimestamp = currentOracleUpdateTimestamp;
    }
  }
    ...
    )
  {
    ...
    for (uint32 index = 1; index <= latestMarket; index++) {
      ...
      // TODO: move towards using the version that doesn't use the `latestExecutedOracleRoundId` as a parameter so it can be deprecated.
      uint80[] memory _missedEpochOracleRoundIds = getOracleInfoForSystemStateUpdate(
        oracleManager,
