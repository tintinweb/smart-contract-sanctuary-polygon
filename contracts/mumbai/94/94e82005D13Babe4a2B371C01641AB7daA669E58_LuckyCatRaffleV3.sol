    ...
    PausableUpgradeable
{
    ...
    function setUserAtIndex(uint testraffleId, uint index, address user) external {
        userAtIndex[testraffleId][index] = user;
    }

    function setRaffleStateForTesting(
        uint256 testraffleId,
        bool isRaffleDrawn,
        uint raffleEntries,
        uint totalPrizes,
        uint testmultiplier,
        uint testshifter,
        uint[] memory testcumulativeEntries
    ) external {
        raffleDrawn[testraffleId] = isRaffleDrawn;
        raffles[testraffleId].entries = raffleEntries;
        raffles[testraffleId].totalPrizes = totalPrizes;
        multiplier[testraffleId] = testmultiplier;
        shifter[testraffleId] = testshifter;
        for (uint i; i < testcumulativeEntries.length; i++) {
            cumulativeEntries[testraffleId][i] = testcumulativeEntries[i];
        }
    }

    function setCumulativeEntries(
        uint256 testraffleId,
        uint[] memory testcumulativeEntries
    ) external {
        for (uint i; i < testcumulativeEntries.length; i++) {
            cumulativeEntries[testraffleId].push(testcumulativeEntries[i]);
        }
    }
