...
contract PIXLandmark is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    ...
    event LandmarkMinted(address indexed account, uint256 indexed tokenId, uint256 amount);
    ...
    function batchMint(
        ...
        require(ids.length == amounts.length, "Landmark: INVALID_ARGUMENTS");
        for (uint256 i; i < ids.length; i += 1) _safeMint(to, ids[i], amounts[i]);
    }
    ...
    function _safeMint(
        ...
        _mint(to, id, amount, "");
        emit LandmarkMinted(to, id, amount);
    }
