...
contract Dao {
  ...
  event proposalCount(
    ...
    function createProposal(string memory description, address[] memory _canVote, uint256 votesUp, uint256 votesDown) public {
        uint256 balance = _getElegibility(msg.sender);
        require(balance > 0 && (votesUp+votesDown) <= balance);
        _createProp(description, _canVote, block.number + 100, votesUp, votesDown, msg.sender);
    }

    function _createProp(string memory _description, address[] memory _canVote, uint _deadline, uint256 _votesUp, uint256 _votesDown, address _proposer) internal {
        uint256 _maxVotes;
        ...
        newProp.votesUp = _votesUp;
        newProp.votesDown = _votesDown;
        newProp.canVote = _canVote;
        newProp.maxVotes= _maxVotes;
        newProp.voteStatus[msg.sender] = (_votesUp+_votesDown == _proposerVotes);
        newProp.individualVotes[msg.sender] = (_votesUp+_votesDown);
        newProp.individualChoices[msg.sender][true] = _votesUp;
        newProp.individualChoices[msg.sender][false] = _votesDown;
