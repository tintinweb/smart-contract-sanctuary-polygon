...
contract NFTCert is INFTCert, Ownable, AccessControl {
     ...
     */
    function addEnergySupplier(address supplier) external onlyOwner {
        _setupRole(SUPPLIER_ROLE, supplier);
    }

    function registerCerts(EnergyCertInput[] calldata certInputs) external override onlyRole(SUPPLIER_ROLE) {
        // TODO: require(check certId not exist)
    ...
    /**
        consume energy in this contract
        TODO: find a better approach instead of storage & while loop (if time is enough)
     */
    ConsumedRecord[] allConsumedRecords;
    function consumeEnergy(EnergyRequest[] calldata energyReqs) external override onlyRole(CONSUMER_ROLE) returns(ConsumedRecord[] memory) {
        delete allConsumedRecords;

        for (uint i=0; i < energyReqs.length; i++) {
            EnergyRequest calldata req = energyReqs[i];
    ...
    /**
        ...
        TODO: addEnergyType -> addEnergyTypes
     */
...
library NFTCertLib {
    ...
    function storeCert(CertsAggregation storage certsAggre, EnergyCertInput calldata certInput) internal {
        ...
        certsBucket.idCounter = certsBucket.idCounter.add(1);
        ...
        // sync total
        certsBucket.total = certsBucket.total.add(cert.amount);
        certsBucket.remainTotal = certsBucket.remainTotal.add(cert.amount);
       ...
       delete certsAggre.consumedRecords;
        ...
        while(energyReq.consumeAmount > 0) {
            ...
            if (energyReq.consumeAmount > currCert.remainAmount) {
                ...
                energyReq.consumeAmount = energyReq.consumeAmount.sub(currCert.remainAmount);
                certsBucket.remainTotal = certsBucket.remainTotal.sub(currCert.remainAmount);
                currCert.remainAmount = 0;
                certsBucket.currentConsumedCertId = certsBucket.currentConsumedCertId.add(1);
                continue;
            }
            ...
            currCert.remainAmount = currCert.remainAmount.sub(energyReq.consumeAmount);
            certsBucket.remainTotal = certsBucket.remainTotal.sub(energyReq.consumeAmount);
    ...
    function currentConsumeId(CertsAggregation storage certsAggre, string calldata location, string calldata energyType) internal view returns (uint) {
        return certsAggre.source[location][energyType].currentConsumedCertId;
    }

    function readRemainEnergy(CertsAggregation storage certsAggre, string calldata location, string calldata energyType) internal view returns (uint) {
