...
contract Coolb is Context, Ownable {
    ...
    uint256 private BOOST_CHANCE = 80; // 35
    ...
    event RewardsBoosted(address indexed user, uint256 boosted);
    ...
    function random(address adr) private view returns (uint256) {
        return uint256(keccak256(abi.encodePacked(block.timestamp, adr, fishingPower[adr], fishersHired))) % 100;
    }
