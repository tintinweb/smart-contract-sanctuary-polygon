...
contract ComplexGiftBox is IGiftBox, ERC721Holder, ERC165 {
  ...
  mapping(uint256 => address) public conservators;
  uint256 public numConservators;
  ...
  constructor(address _registry, address _recipient, address _from, address _conservator, uint256 _targetDate, uint256 _targetBalance, address _targetToken, uint256 _targetTokenBalance) {
    ...
    if(_conservator != address(0)){
      ...
      conservators[numConservators] = _conservator;
      numConservators++;
    }
  ...
  function unwrap() public virtual isGiftReady {
    ...
    // Send ERC-721 tokens
    for(uint256 j; j < erc721Tokens.length; j++){
    ...
    for(uint256 k; k < erc1155Tokens.length; k++){
    ...
    //Remove conservators from managed list
    for(uint256 l; l < numConservators; l++){
      registry.deregisterConservator(conservators[l]);
    }
  }
  ...
  // @name Get Number of Allowed Erc20 tokens
