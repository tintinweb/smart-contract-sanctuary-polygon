...
contract RunningUpgradeable is OwnableUpgradeable {
    ...
    uint256 private chargeNumber;
    //Phí rút tiền (%)
    uint256 private chargeFee;
    //Phí bet (%)
    uint256 private betFee;
    //Số tiền bị phạt
    uint256[] private chargeValue;
    //Số tiền bet
    ...
    function __Running_init(string[] memory domains, address[] memory wallets) internal onlyInitializing {
        __Running_init_unchained(domains, wallets);
    }

    function __Running_init_unchained(string[] memory domains, address[] memory wallets) internal onlyInitializing {
        require(
        ...
        chargeFee = 10;
        betFee = 30;
        chargeValue = [0.1 ether, 0.07 ether, 0.05 ether];
        betValue = 0.1 ether;
    }

    function initialize(string[] memory domains, address[] memory wallets)
        public
        initializer
    {
        __Ownable_init();
        __Running_init(domains, wallets);
    }
    ...
    function deposit() public payable isMember(msg.sender) {
