...
// File: contracts/WomenHackersClub_v2.sol
...
contract WomenHackersClub is Ownable, metonymy, ONFT721 {
    ...
    uint256 price = 76000000000000000; // 0.076 ether

    uint256 mintpassprice = 76000000000000000; // 0.076 ether

    uint256 allowlistprice = 76000000000000000; // 0.076 ether

    uint256 public perWalletLimit = 10;

    uint256 public perTxLimit = 2;

    MINTPASS private mntps;
    ...
    address private a1 = 0x7d52923Ca0135F59B15986FCADeC7107758BbeFd;

    address public signingAddress = 0x1048Ded3a542e064C82161Ab8840152393E0477E;



    /// @notice Constructor for the WomenHackersClub smart contract

    /// @param mintpassContractAddress the address of the dependent contract (only ETH Mainnet)

    /// @param _layerZeroEndpoint handles message transmission across chains

    /// @param startMintId the starting mint number on this chain

    /// @param endMintId the max number of mints on this chain

    constructor(address mintpassContractAddress, address _layerZeroEndpoint, uint startMintId, uint endMintId) ONFT721("Women Women Hackers Club", "WHC", _layerZeroEndpoint) {

        nextMintId = Counters.Counter(startMintId);

        maxMintId = endMintId;

        mntps = MINTPASS(mintpassContractAddress);
...
function allowlistMint(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {
        ...
        require(msg.value >= allowlistprice * mintCount, "Amount sent too small");
        ...
        require(ownerMintedCount + mintCount < mint_allowed + 1, "Individual minting limit for WHC on allowlist exceeded!");
    ...
    function mintpassMint(uint256 mintCount) external payable {
        ...
        require(msg.value >= mintpassprice * mintCount, "Amount sent too small");

        

        uint256 balance = mntps.balanceOf(msg.sender);

        require(balance > 0, "Must hold at least one Mintpass to mint your WHCs");



        require(mintCount < perTxLimit + 1, "WHC quantity per transaction too large");



        uint256 ownerMintedCount = addressMintedBalance[msg.sender];

        require(ownerMintedCount + mintCount < perWalletLimit + 1, "Maximum amount of WHC mints for this wallet exceeded!");
    ...
    function whcClaim(uint256 mintCount,uint8 v, bytes32 r,bytes32 s,uint256 mint_allowed,uint256 free) external payable {

        require(PresaleIsActive, "WHC Presale not active");



        require(verifySignature(v,r,s,mint_allowed,free), "Invalid signature");

        require(free == 1, "You are not allowed to claim a free WHC");

        uint256 ownerMintedCount = addressMintedBalance[msg.sender];

        require(ownerMintedCount + mintCount < mint_allowed +1, "This exceeds your allowed free claims");
    ...
    /// @notice Public Mint function for the WHCs

    function publicMint(uint256 mintCount) external payable {

        require(saleIsActive, "Sale not active");

        require(msg.value >= price * mintCount, "Amount sent is too small");
        ...
        require(ownerMintedCount + mintCount + 1 < perWalletLimit, "Maximum amount of WHC mints for this wallet exceeded!");
   ...
   // internal mint function
    ...
    /// @notice change the mintpass price
    ...
    /// @notice change the allowlist price
    ...
    /// @notice change the presale status
    ...
    /// @notice change perWalletLimit

    function setPerWalletLimit(uint256 _perWalletLimit) external onlyOwner {
   ...
   /// @notice change the signing address
