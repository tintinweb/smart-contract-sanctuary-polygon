...
// File: contracts/TrashPandazClubSteakingV2.sol
...
contract TrashPandazClubSteakingV2 is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function viewStakedIds(address _account, uint256[] memory tokenIds) public view returns (uint256[] memory){ // may need to add memory after uint256[]
        uint256[] memory _ids;
        uint count = 0;
        for (uint256 i = 0; i < tokenIds.length; i += 1) {
            if( stakedAssets[tokenIds[i]] == _account ){
                _ids[i] = tokenIds[i];
                count ++;
            }
        }
