...
contract Flex2Earn is OperatorAccessControlUpgradeable {
    ...
    function setPoap(address _poap) public onlyOwner {
        require(_poap != address(0), "Flex2Earn::setPoap: Poap address cannot be 0");
        poap = Poap(_poap);
    }
    ...
    function awardFlexer(uint256 _flexedNfcId) external onlyClerk {
        ...
        require(!partnerRegistry.isEventCancelled(eventId), "Flex2Earn::awardFlexer: event is cancelled");
...
contract PartnerRegistry is OperatorAccessControlUpgradeable {
    ...
    modifier onlyEventOwnerOrOperator(uint256 eventId) {
        require(
            events[eventId].partner == msg.sender ||
            isOperator(msg.sender),
            "PartnerRegistry::onlyEventOwnerOrOperator: sender is neither Event owner nor Operator");
        _;
    }
    ...
    function modifyEvent(uint256 eventId, uint256 startTime, uint256 endTime, uint256 rewardPerFlexer, uint256 another1TokenRewardPool, bool isPoapAssigned) public onlyEventOwnerOrOperator(eventId) {
    ...
    function addClerksToEvent(uint256 eventId, address[] memory _clerks) public onlyEventOwnerOrOperator(eventId) {
        for(uint256 i = 0; i < _clerks.length; ++i) {
            _addClerkToEvent(eventId, _clerks[i]);
        }
    }

    function _removeClerkFromEvent(address _clerk) private onlyEventOwnerOrOperator(getClerkEventId(_clerk)) {
    ...
    function addAllowedCollections(uint256 eventId, address[] memory colectionAddresses) public onlyEventOwnerOrOperator(eventId) {
    ...
    function removeAllowedCollections(uint256 eventId, address[] memory colectionAddresses) public onlyEventOwnerOrOperator(eventId) {
        for(uint256 i = 0; i < colectionAddresses.length; ++i) {
            if(allowedCollectionsByEventId[eventId][colectionAddresses[i]]) {
                allowedCollectionsCounterByEventId[eventId]--;
            }
    ...
    function setEventCancellation(uint256 eventId, bool isCancelled) public onlyEventOwnerOrOperator(eventId) {
