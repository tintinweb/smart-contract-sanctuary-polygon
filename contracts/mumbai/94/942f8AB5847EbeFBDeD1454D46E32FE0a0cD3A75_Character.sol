...
import "../EXTENSIONS/ERC721Sig.sol";
...
/**
 *  @title Unseen Characters
 *  @notice ERC721 NFTs representing 'Unseen' characters
 *  @author Unseen | decapinator.eth
 **/
contract Character is ERC721Base, ERC721Sig {
    constructor(
        string memory baseTokenUri,
        string memory contractUri,
        address _saleRoyaltiesRentFeesRecipient,
        address signer,
        address _runToken,
        uint128 royaltyBps,
        uint128 _protocolFeeBps
    )
        payable
        ERC721Base(
            baseTokenUri,
            _saleRoyaltiesRentFeesRecipient,
            signer,
            _runToken,
            royaltyBps,
            _protocolFeeBps
    ...
    function mintWithSignature(
        ...
        // Collect price
        _collectPriceOnClaim(_req.currency, _req.pricePerToken);
    ...
    function _collectPriceOnClaim(
        ...
        if (_currency == CurrencyTransferLib.NATIVE_TOKEN) {
            require(msg.value == _pricePerToken, "Must send total price.");
        }
        (address treasuryWallet, ) = getProtocolFeeInfo();
...
/**
 *  @title Unseen Characters
 *  @notice ERC721 NFTs representing unseen metaverse characters
 *  @author Unseen | decapinator.eth
 **/
    ...
    ProtocolFee
{
    ...
    /// @notice Address of the RUN token.
    address public runToken;

    /// @notice Mapping if address has minter role.
    mapping(address => bool) internal _minterRole;

    /// @notice Emitted when a new minter is added.
    event minterRoleGranted(address indexed _signer);

    /// @notice Emitted when a minter is revoked.
    event minterRoleRevoked(address indexed _signer);

    /// @notice Emitted when the base uri is changed.
    event baseUriChanged(string prevURI, string newURI);

    /**
     *  @notice Constructor
     *  @param baseTokenUri Base token uri.
     *  @param _saleRoyaltiesRentFeesRecipient Address of the sale recipient.
     *  @param _signer Address of the signer.
     *  @param _runToken Address of the RUN token.
     *  @param _royaltiesBps Royalties in basis points.
     **/
    constructor(
        string memory baseTokenUri,
        address _saleRoyaltiesRentFeesRecipient,
        address _signer,
        address _runToken,
        uint128 _royaltiesBps,
        uint128 _protocolFeeBps
    ) payable ERC721A("UNSEEN", "USN") {
        require(
            _saleRoyaltiesRentFeesRecipient != address(0),
            "USN: Treasury cannot be address 0"
        );
        require(_signer != address(0), "USN: Minimum 1 signer required");
        require(_runToken != address(0), "USN: Token cannot be address 0");
        _setupDefaultRoyaltyInfo(
            _saleRoyaltiesRentFeesRecipient,
            _royaltiesBps
        );
        _setupProtocolFeeInfo(_saleRoyaltiesRentFeesRecipient, _protocolFeeBps);
        _minterRole[_signer] = true;
     ...
     */
    function rent(uint256 tokenId, uint64 expires) public override {
        ...
        if (amount != 0) {
            ...
            if (protocolFeeBps != 0) {
                CurrencyTransferLib.safeTransferERC20(
                    ...
                    msg.sender,
                    treasuryWallet,
                    (amount * protocolFeeBps) / 10000
                );
                amount -= (amount * protocolFeeBps) / 10000;
            }
            CurrencyTransferLib.safeTransferERC20(
                runToken,
                msg.sender,
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(ERC4907A, ERC721A, IERC721A, IERC165)
        returns (bool)
    {
    ...
    function isApprovedOrOwner(
        ...
        isApprovedOrOwnerOf = (_operator == _owner ||
     ...
     */
    function _changeBaseUri(string memory _Uri) external virtual onlyOwner {
        string memory prevURI = _baseTokenUri;
        require(
            keccak256(abi.encodePacked((_Uri))) !=
                keccak256(abi.encodePacked(prevURI)),
            "USN: same as previous"
        );
        _baseTokenUri = _Uri;
        emit baseUriChanged(prevURI, _Uri);
    }

    /**
     * @notice Changes rent currency.
     * @param asset New rent currency.
     */
    function changeRentCurrency(address asset) external onlyOwner {
        require(runToken != asset, "USN: same as previous");
        require(asset != address(0), "USN: cannot be address 0");
        runToken = asset;
    }
     ...
     * @param _signer Address of the signer.
     */
    function _grantMinterRole(address _signer) external onlyOwner {
        require(!_minterRole[_signer], "USN: Signer already have minter role");
        require(_signer != address(0), "USN: Cannot be address 0");
        _minterRole[_signer] = true;
        emit minterRoleGranted(_signer);
    }
     ...
     * @param _signer Address of the signer.
     */
    function _revokeMinterRole(address _signer) external onlyOwner {
        require(_minterRole[_signer], "Signer doesn't have minter role");
        require(_signer != address(0), "USN: Cannot be address 0");
        _minterRole[_signer] = false;
        emit minterRoleRevoked(_signer);
    }

    /**
     *  @notice Lets a contract admin set the URI for contract-level metadata.
     *  @dev Caller should be authorized to setup contractURI, e.g. contract admin.
     *  @param _Uri contract uri holding it's data
     */
    function setContractURI(string memory _Uri) public override onlyOwner {
    ...
    // The following functions are overrides required by OpenSea OperatorFilterer.
...
abstract contract ERC4907A is ERC721A, IERC4907A {
    ...
    function setUser(
        ...
        require(expires != 0, "No expiry assgined");
     ...
     * @return dueAmount The amount of RUN to be paid
     */
    function _rent(
        uint256 tokenId,
        uint64 expires
        ...
        require(userOf(tokenId) == address(0), "Rented");
        require(expires != 0, "No expiry assigned");
        require(
     ...
     * @param ratePerMinute The rent fee in RUN per minute
     * @dev The rent fee is set in RUN per minute
     */
    function setRentFee(
        uint256 tokenId,
        uint128 ratePerMinute
        ...
        require(
            ratePerMinute != rentablesInfo[tokenId].ratePerMinute,
            "Fee is set"
        );
    ...
    function setRentable(
        ...
        require(rentablesInfo[tokenId].rentable != rentable, "Already set");
     ...
     * @return ratePerMinute The rent fee in RUN per minute
...
import "../INTERFACES/IERC721Sig.sol";
import "@openzeppelin/contracts/utils/cryptography/EIP712.sol";

/**
 *  @title Unseen Mint Signature
 *  @notice Contract holding the logic of minting signed payload
 *  @author Unseen | decapinator.eth
 **/
abstract contract ERC721Sig is EIP712("ERC721Sig", "1.0.0"), IERC721Sig {
    using ECDSA for bytes32;

    /// @dev The typehash of the mint request.
    bytes32 private constant TYPEHASH =
        keccak256(
            "MintRequest(address to,uint256 pricePerToken,address currency,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid,bool isMintable)"
        );
    ...
    function _processRequest(
        ...
        if (_req.validityStartTimestamp > block.timestamp) {
            revert("Req not yet valid");
        }

        if (block.timestamp > _req.validityEndTimestamp) {
            revert("Req expired");
        }
 ...
 *  @notice Interface for contract metadata.
 */
