...
contract ShoppingCartV1 is Initializable, OwnableUpgradeable {
    ...
    // Admin functions
    function checkout(address recipient, Purchase[] memory cart) external payable {
        uint256 totalPrice = 0;
        
        Claim[] memory claims = new Claim[](cart.length);
        for (uint256 i = 0; i < cart.length; i++) {
            Purchase memory purchase = cart[i];
            require(isWhitelisted[purchase.contractAddress], "Contract is not whitelisted");

            DroppableUpgradeable droppable = DroppableUpgradeable(purchase.contractAddress);
            (DroppableUpgradeable.MintPhase memory phase,) = droppable.getActiveMintPhase();
        ...
        for (uint256 i = 0; i < cart.length; i++) {
            ...
            claim.droppable.claim{value: claim.value}(recipient, claim.amount, proof);
        }
    }
}