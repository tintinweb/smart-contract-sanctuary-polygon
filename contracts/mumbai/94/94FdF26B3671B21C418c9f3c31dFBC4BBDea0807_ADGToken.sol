...
contract ADGToken is ERC721, ERC721URIStorage {
    ...
    function _getTokenMetadata(uint256 tokenId) private view returns (TokenMetadata memory) {
        return TokenMetadata (
            items[tokenId].nameOfItem,
            items[tokenId].imageURI,
            Strings.toString(items[tokenId].price),
            uint256(uint160(items[tokenId].seller)),
            Strings.toString(orders[tokenId].purchaseTimestamp)
        );
    }
    ...
    function tokenURI(uint256 itemId) public view virtual override(ERC721, ERC721URIStorage) returns (string memory) {
        ...
        TokenMetadata memory curTokenMetadata = _getTokenMetadata(itemId);

        bytes memory dataURI = abi.encodePacked(
            '{',
                '"name": "ADG #', Strings.toString(itemId), '",',
                '"description": "', curTokenMetadata.nameOfItem, '",',
                '"image": "', curTokenMetadata.imageURI, '",',
                '"attributes": [',
                    '{',
                        '"display_type": "number",',
                        '"trait_type": "price",',
                        '"value": ', curTokenMetadata.price,
                    '},',
                    '{',
                        '"display_type": "number",',
                        '"trait_type": "seller",',
                        '"value": ', Strings.toString(curTokenMetadata.seller),
                    '},',
                    '{',
                        '"display_type": "date",',
                        '"trait_type": "Purchase date",',
                        '"value": ', curTokenMetadata.purchaseTimestamp,
                    '}',
                ']',
            '}'
         );
