...
contract CryptoStudio is ERC721,Ownable
{
...
//    @@@    ::   :::  @@@@@@@   :: ::::  ::: ::::  @@@@@@@   :   : :  ::         ::  @@@@@@@     \\
    ...
    mapping(uint256 => bool) private tokenMetadata; 
    ...
    function SocialSpaceCreation(string memory spaceName , string memory space_group , string memory imageCID) public  {
        require(spaceArtistsMap[spaceName].length() == 0, "taken space");
    ...
    function spaceExists(string memory spaceName) public view returns(bool){
        ...
        if(spaceArtistsMap[spaceName].length() == 0){
            return false;
        }
        return true;
    }
    ...
    function mint_your_Art(string memory name , string memory imageCID , string memory animationCID , string memory audioCID, string memory description ,string memory spaceName) public payable {
        ...
        require(spaceArtistsMap[spaceName].contains(msg.sender), "anothorized space to mint");

        tokenID.increment();
        uint256 temp = tokenID.current();
        string memory insert_statement = insertMainStatement(temp,description,imageCID,audioCID,name,animationCID);
        string memory insert_statement2 = insertAttributeStatement(temp ,"spaceName", spaceName);
        string memory insert_statement3 = insertAttributeStatement(temp ,"Creator", Strings.toHexString(msg.sender));   
    ...
    function assignVisualizer(uint256 tokenid , string memory animationURL) public {
        require(tokenCreator[tokenid] == msg.sender && tokenMetadata[tokenid] == false);
        tokenMetadata[tokenid] = true;
    ...
    function addSpaceArtist(string memory spaceName, address artist) public {
        require(spaceArtistsMap[spaceName].contains(msg.sender), "Only space owners can add an Artist to the space");
    ...
    function isSpaceArtist(string memory spaceName, address sender ) public view returns (bool){
        if(spaceArtistsMap[spaceName].contains(sender)){
            return true;
        }
        return false;
    }
    ...
    // Used for the Lit Action Encryption Rule for Posts
    function isSpaceOwner(string memory spaceName, address sender) public view returns (bool){
    ...
    function insertMainStatement(uint256 tokenid ,string memory description, string memory imageCID, string memory audioCID, string memory name, string memory animationCID) internal view returns(string memory){
        return
        SQLHelpers.toInsert(
                MAIN_TABLE_PREFIX,
                mainTableID,
        ...
        function insertAttributeStatement(uint256 tokenid ,string memory trait_type, string memory value) internal view returns(string memory){
        return
        SQLHelpers.toInsert(
                ATTRIBUTE_TABLE_PREFIX,
                attributeTableID,
