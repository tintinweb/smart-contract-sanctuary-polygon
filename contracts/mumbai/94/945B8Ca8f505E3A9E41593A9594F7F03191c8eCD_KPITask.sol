...
interface IKPIProjectStates {
    ...
    function getMember(
        address user
    ) external view returns (bytes32 name, bool isMember, address alternate);

    /// @notice Get mapping uint256 of members
    /// @param index The index of member
    /// @return The address of member
...
contract KPITask is IKPITask {
    ...
    function createTask(
        ...
        require(
            IKPIProject(taskContractInfo.projectContract).isAdmin(msg.sender)
        );
    ...
    function receiveTask(uint256 taskID) external override {
        ...
        require(
            IKPIProject(taskContractInfo.projectContract).isMember(msg.sender)
        );
        require(
        ...
        if (guarantee < guaranteeAmount && depositChecking) {
            ...
            IKPIPayment(taskContractInfo.paymentContract)
                .depositGuaranteeAmount(msg.sender, missing);
            IKPITaskControl(taskContractInfo.controlContract)
                .changeMemberGuarantee(msg.sender, guaranteeAmount);
        }
    ...
    function penalizeTask(uint256 taskID) external override {
        ...
        uint256 guarantee = IKPITaskControl(taskContractInfo.controlContract)
            .guarantee(task.assignee);
        (uint256 totalPenalized1, uint256 totalPenalized2) = IKPIPayment(
            taskContractInfo.paymentContract
        ).penalized(
                taskID,
                task.assignee,
        ...
        IKPITaskControl(taskContractInfo.controlContract)
            .mapIndexToPenalizedTask(task.assignee, taskID);
    ...
    function cancelTask(uint256 taskID) external override {
        ...
        IKPIPayment(taskContractInfo.paymentContract).canceled(
            taskID,
            getTaskReward[taskID].remaining1,
            getTaskReward[taskID].remaining2
        );
    ...
    function approveTask(uint256 taskID) external override {
        ...
        uint256 taskReward1 = IKPIPayment(taskContractInfo.paymentContract)
            .withdrawTask(
                taskID,
                task.status,
                task.assignee,
        ...
        if (getTaskReward[taskID].remaining2 > 0) {
            taskReward2 = IKPIPayment(taskContractInfo.paymentContract)
                .withdrawTask(
                    taskID,
                    task.status,
                    task.assignee,
    ...
    function forceDone(uint256 taskID) external override {
        ...
        uint256 amount1 = IKPIPayment(taskContractInfo.paymentContract)
            .withdrawTask(
                taskID,
                task.status,
                task.assignee,
        ...
        uint256 amount2 = 0;
        if (getTaskReward[taskID].remaining2 > 0) {
            amount2 = IKPIPayment(taskContractInfo.paymentContract)
                .withdrawTask(
                    taskID,
                    task.status,
                    task.assignee,
        ...
        IKPITaskControl(taskContractInfo.controlContract)
            .mapIndexToCompletedTask(task.assignee, taskID, amount1, amount2);
    ...
    function transfer(
        ...
        uint256 amount,
        uint8 whichToken
    ) external override onlyPaymentContract {
    ...
    function dropTask(uint256 taskID) external override {
        ...
        require(
            msg.sender == task.assignee ||
                IKPIProject(taskContractInfo.projectContract).isAdmin(
                    msg.sender
                )
        );
        require(TaskStatus.canDropTask(task.status));
        require(
            getTaskReward[taskID].remaining1 > 0 ||
                getTaskReward[taskID].remaining2 > 0
        );
        if (task.status != TaskStatus.TODO) {
            if (msg.sender == task.assignee) {
                uint256 guarantee = IKPITaskControl(
                    taskContractInfo.controlContract
                ).guarantee(msg.sender);
                IKPITaskControl(taskContractInfo.controlContract)
                    .changeMemberGuarantee(task.assignee, 0);
    ...
    ///@inheritdoc IKPITaskActions
    function setTaskControlContract(
        address taskControl
