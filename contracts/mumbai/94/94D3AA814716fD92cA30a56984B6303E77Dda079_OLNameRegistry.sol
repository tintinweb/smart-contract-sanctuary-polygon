...
contract OLNameRegistry is Ownable, Seriality {
    ...
    function serializeRegistry(Registry memory props) internal pure returns (bytes memory) {
        uint256 bufferSize = sizeOfAddress() +
            32 + // bytes32 - nameHash
        ...
        bytes32ToBytes(offset, props.nameHash, buffer);
        offset -= 32;

        addressToBytes(offset, props.regAddress, buffer);
        offset -= sizeOfAddress();

        return buffer;
    }

    function deserializeRegistry(bytes memory buffer) internal pure returns (Registry memory) {
        ...
        bytesToBytes32(offset, buffer, props.nameHash);
        offset -= 32;

        props.regAddress = bytesToAddress(offset, buffer);
