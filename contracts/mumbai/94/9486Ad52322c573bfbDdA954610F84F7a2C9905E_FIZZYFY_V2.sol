...
contract FIZZYFY_V2 
{
    ...
    uint256 private constant dayRewardPercents = 5000;
    uint256 private constant baseSplitPercents = 3000;
    uint256 private constant baseFundPercents = 7000;
    uint256 private constant maxAddFreeze = 45 hours;
    ...
    constructor() public {
        ...
        feeReceivers = address(0x5dE021eb0464369E4D5F83555A6E634EBcCB3372);
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        Dai.transfer(feeReceivers, fee);
        Dai.transfer(liquidityAddr, liquidity);        
        Dai.transfer(insuranceAddr, insurance);        
	...
	function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 1000e18){
            ...
            if(user.teamNum >= 20 && maxTeam >= 75000e18 && otherTeam >= 75000e18){
                levelNow = 5;
            }else if(user.teamNum >= 4 && maxTeam >= 10000e18 && otherTeam >= 10000e18){
                levelNow = 4;
            }else{
                levelNow = 3;
            }
	...
	function _updateTeamNum(address _user, uint256 _amount) private {
		...
		for(uint256 i = 0; i < 1; i++)
		{
            if(upline != address(0))
			{
                if(userInfo[upline].maxDeposit == userInfo[upline].totalDeposit)
                {
                    if(userInfo[upline].directnum >= 1 && userInfo[upline].directnum < 5 )
                    {
                        ...
                        upRewards.tokenValue = upRewards.tokenValue.add(amount.mul(tokenBonusPercents[userInfo[upline].directnum]).div(baseDivider));
                        upRewards.tokenBonus = upRewards.tokenBonus.add(amount.mul(tokenBonusPercents[userInfo[upline].directnum]).div(baseDivider));
                    }
                }
    ...
    function withdraw() external {
        ...
        require(user.directnum > 0 || (user.directnum >= 2 && user.totalRevenue <= maxRevenue && user.directBusiness >= user.maxDeposit.mul(2)), "max revenue reached with no direct");
