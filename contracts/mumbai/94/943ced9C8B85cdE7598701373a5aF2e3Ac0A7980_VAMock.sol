    ...
    Ownable
{
    ...
    modifier projectTokens() {
        if (_tokenIdCounter.current() < 40) {
            require(msg.sender == owner(), "First 40 tokens are for VAMock.");
            _projectToken[_tokenIdCounter.current() + 1] = true;
        }
        _;
    }
    ...
    constructor(bytes3[16] memory _palette) ERC721("VAMock", "vamock") {
        _setPalette(_palette);
        uint256 tokenId = _tokenIdCounter.current();
        _safeMint(address(this), tokenId);
        _fixedAdr[tokenId] = address(this);
    ...
    function vamockToken() public payable projectTokens returns (uint256) {
        ...
        returns (address, bytes3[40] memory)
    {
        ...
        bytes3[40] memory colors = getColors(adr, palette);
        return (adr, colors);
    }
