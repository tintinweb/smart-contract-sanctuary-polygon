...
contract Lending is AccessControl, ReentrancyGuard, Calcs, ILending {
    ...
    function completeWithdraw(uint256 idrequest, uint256 idLending) public NotPaused nonReentrant returns(uint256 _amount){
        ...
        (uint256 _quantity,,Status _state,uint256 _withdrawTime)  = LControl.getRequest(msg.sender, idrequest);
        if((_withdrawTime + exitDays) < block.timestamp){
            revert("to soon");
        }
    ...
    function cancelWithdraw(uint256 id, address _user, uint256 idLending) public who(id) returns(bool _complete) {
        (uint256 _quantity,,Status _state,)  = LControl.getRequest(_user, id);
        if(_state == Status.complete || _state == Status.cancelled){
            revert("Request not pending");
        }
        (address lendersVault,,) = tokenInfo.getVaultInfo(address(token));
        IVaultLenders(lendersVault).deleteRequest(_quantity);
        LControl.updateMoney(id, _user, _quantity);
        LControl.closeRequest(_user,id,Status.cancelled, idLending);

        emit Cancel(_user, id, msg.sender);
