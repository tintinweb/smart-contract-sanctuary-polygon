...
contract TokenWithBal is ERC20, ERC20Burnable {
    ...
    //Stores the time that the admins will lose control of the contract
    //This is private becuase there is a function to retrieve it
    ...
    constructor(
        ...
        switchTime = uint32(block.timestamp + 5 minutes);
    ...
    //not be trying to mint tokens than the admin is allowed
    ...
    //This modifier is to make sure that a msg.sender is the admin of the contract
    modifier onlyAdmin() {
    ...
    //This function must only be callable once by the admin
    ...
    function mint(address to, uint256 amount) external onlyApproved(amount) {
        ...
        //If the msg.sender is a tempAdmin increase the amount that it has minted
        ...
        onlyApproved(sum(amounts))
    {
        ...
        for (i = 0; i < amounts.length; ) {
            //Checking that each address is not equal to the burn address;
            require(addresses[i] != address(0x0), "ERR:ZA"); //ZA => Zero Address

            //Checking that each amount in larger than zero
    ...
    //This function must only be callable by the _transfer function
    function addLiquidity(uint256 tokenAmount) internal {
        // approve token transfer for the Uniswap Router
        _approve(address(this), uniswapV2RouterAddr, tokenAmount);

        //define a uint variable to be used later
        uint256 maticAmount;
        ...
        IUniswapV2Router02(uniswapV2RouterAddr).addLiquidityETH{
            ...
            block.timestamp + 1 //When it should be processed by, for this we set the time as right now, this only needs to have extra time if calling this function on it's own transaction
        );
