...
contract NFT is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, ERC721Burnable, Ownable, SafeTransfer, ReentrancyGuard, INFT {
    ...
    function mint(address _to,uint256 _nftId) public override nonReentrant checkAdmin {
        doMint(_to, _nftId);
    }
    ...
    function multiMint(address _to, uint256[] memory _nftIds) external nonReentrant checkAdmin {
    ...
    function blindboxMint(address toAddress) external checkAdmin returns (uint256){
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
...
contract SafeTransfer{
     ...
     */
    function getPayableAmount(address token,address platfromAddress, uint256 amount) internal returns (uint256) {
        if (token == address(0)){
            amount = msg.value;
        }else if (amount > 0){
            IERC20 oToken = IERC20(token);
            oToken.safeTransferFrom(msg.sender, platfromAddress, amount);
        }
        return amount;
    }
