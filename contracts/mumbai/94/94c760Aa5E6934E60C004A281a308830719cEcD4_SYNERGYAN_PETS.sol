...
contract SYNERGYAN_PETS is Initializable, ERC721URIStorageUpgradeable, AccessControlUpgradeable {
    ...
    event newNFT(address creator, uint id,string ipfsHash,string name);

    function initialize(string memory _contractUri) public initializer {
      _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
      __ERC721_init("SYNERGYAN_PETS", "PSL");
         baseUri = "ipfs://";
        contractUri = string(abi.encodePacked("ipfs://", _contractUri));
    }
    ...
    function safeMint(address _to,string memory _name,string memory _tokenHash, uint _feeSeller) public onlyOwner {
        id++;
        tokenData memory p = tokenData(_name,_to,_feeSeller,_tokenHash);
        nfts[id]=p;
        _safeMint(_to, id);
        _setTokenURI(id,_tokenHash);
        emit newNFT(_to, id, _tokenHash,_name);
    }
