...
contract TableCollection is NFTCollection {
    event TokenBaseURISet(string indexed tokenBaseURI);
    using SafeMathUpgradeable for uint256;
    using MerkleProofUpgradeable for bytes32[];
    mapping(address => uint256) private _mintCount;
    string private baseURIString = "https://testnet.tableland.network/query?s=";
    //tables 
	...
	function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // SQL query to JOIN two tables to compose the metadata accross a 'main' and 'attributes' tablE
		...
		string memory query = string(
			abi.encodePacked(
				...
				mainTable,
				'%2Eid%2C%27name%27%2Cname%2C%27description%27%2Cdescription%2C%27image%27%2Cimage%2C%27attributes%27%2Cjson_group_array%28json_object%28%27trait_type%27%2Ctrait_type%2C%27value%27%2Cvalue%29%29%29%20',
				'FROM%20',
				mainTable,
				'%20JOIN%20',
				attributesTable,
    ...
    function setURIString(string memory uri) public onlyOwner {
        baseURIString = uri;
    }

    function setBaseUri(string memory _mainTable,string memory _attributesTable) public onlyOwner {
    ...
    INFTCollection
{
    ...
    function withdraw() external {
        ...
        uint256 fees = _nftc.getFees(address(this));
        _nftc.addFeesClaimed(fees);
        AddressUpgradeable.sendValue(payable(address(_nftc)), fees);
    ...
    function transferOwnership(address newOwner)
        public
        override(INFTCollection, OwnableUpgradeable)
    {
