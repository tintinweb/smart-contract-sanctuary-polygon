...
contract BondStakeDepository is Ownable {
    ...
    address public immutable XDAO; // intermediate reward token from treasury
    address public immutable sXDAO; // token given as payment for bond
    ...
    struct Bond {
        uint gonsPayout; // sXDAO gons remaining to be paid
        uint xdaoPayout; //xdao amount at the moment of bond
    ...
    function deposit( 
        ...
        IERC20( XDAO ).approve( stakingManager, payout );

        IStakingManager(stakingManager).stake( payout, address(this) );
        /* ---------------------------------------------------------- */
        
        uint stakeGons=ISXDAO(sXDAO).gonsForBalance(payout);

        // depositor info is stored
        _bondInfo[ _depositor ] = Bond({ 
            gonsPayout: _bondInfo[ _depositor ].gonsPayout.add( stakeGons ),
            xdaoPayout: _bondInfo[ _depositor ].xdaoPayout.add( payout ),
    ...
    function redeem( address _recipient, bool _stake) external returns ( uint ) {        
        ...
        uint _amount = ISXDAO(sXDAO).balanceForGons(info.gonsPayout);
        emit BondRedeemed( _recipient, _amount, 0 ); // emit bond data
        IERC20( sXDAO ).transfer( _recipient, _amount ); // pay user everything due
        return _amount;
    }
    ...
    /**
     *  @notice return bond info with latest sXDAO balance calculated from gons
     *  @param _depositor address
     *  @return payout uint
     *  @return vesting uint
    ...
    function bondInfo(address _depositor) public view returns ( uint payout,uint vesting,uint lastBlock,uint pricePaid ) {
        ...
        payout=ISXDAO(sXDAO).balanceForGons(info.gonsPayout);
    ...
    function pendingPayoutFor( address _depositor ) external view returns ( uint pendingPayout_ ) {
        ...
        uint payout = ISXDAO(sXDAO).balanceForGons(_bondInfo[ _depositor ].gonsPayout);
