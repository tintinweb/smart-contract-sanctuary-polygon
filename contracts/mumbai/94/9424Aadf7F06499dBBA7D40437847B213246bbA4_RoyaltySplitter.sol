...
contract RoyaltySplitter is IRoyaltySplitter, Initializable, AccessControlUpgradeable {
    ...
    event RoyaltyRegistered(address indexed forwarder);
    ...
    function registerRoyalty(
        ...
        address forwarder = address(new RoyaltyForwarder{ salt: salt }(this));
        uint96 totalShares = _setRoyaltyInfo(forwarder, royalties);

        emit RoyaltyRegistered(forwarder);
        return (forwarder, totalShares);
    }
    ...
    function releaseRoyalty() external payable {
        ...
        for (uint256 i = 0; i < royaltyCount; ) {
            ...
            uint256 amountSent = (amountReceived * royalty.share) / totalShares;
            royalty.payee.sendValue(amountSent);
            unchecked {
                i++;
            }
        }
