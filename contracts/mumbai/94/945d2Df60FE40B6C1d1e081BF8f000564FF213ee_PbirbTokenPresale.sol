...
contract PbirbTokenPresale is Crowdsale, Ownable {
    ...
    function _validateWhitelistSale(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(_whitelistSalePbirbSold + tokens <= _whitelistSalePbirbCap, "Exceeds whitelist sale cap");
        require(_pbirbPurchased[beneficiary] + tokens <= _maxPbirbPerAddress, "Exceeds maximum tokens per address");
    }
    ...
    function _validatePublicSale(address beneficiary, uint256 weiAmount) internal view {
        ...
        require(_publicSalePbirbSold + tokens <= _publicSalePbirbCap, "Exceeds public sale cap");
        require(_pbirbPurchased[beneficiary] + tokens <= _maxPbirbPerAddress, "Exceeds maximum tokens per address");
    }
