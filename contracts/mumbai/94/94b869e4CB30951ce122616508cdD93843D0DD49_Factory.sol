...
contract Factory is IFactory, IOwnable {
    ...
    bytes32 private constant NFT_MANAGER_ROLE = keccak256("NFT_MANAGER");
    ...
    function setRevenueShare(address _revenueShare) external onlyOwner {
        ...
        if (address(revenueShare) != _revenueShare) {
            emit RevenueShareSet(address(revenueShare), _revenueShare);
            revenueShare = RevenueShare(_revenueShare);
        }
        //set proper roles
        for(uint256 i = 0; i < _tnfts.length; i++){
            AccessControl(address(revenueShare)).grantRole(NFT_MANAGER_ROLE, address(_tnfts[i]));
        }
    }
    ...
    function newCategory(
        ...
        AccessControl(address(revenueShare)).grantRole(NFT_MANAGER_ROLE, address(tangibleNFT));
