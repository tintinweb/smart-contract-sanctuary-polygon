...
contract Sale is ISale, ContextUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, ERC1155HolderUpgradeable {
     ...
     */
    function createSale(address _caller, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _project, SaleInput memory _saleInput) external nonReentrant onlyProject {
        ...
        if (_project.isSingle) {
            ...
            if (_isCreateNewToken) {
                sale.tokenId = _isSetRoyalty ? 
                IOSB1155(_project.token).mintWithRoyalty(address(this), _saleInput.tokenUri, _saleInput.amount, _saleInput.royaltyReceiver, _saleInput.royaltyFeeNumerator) : 
                IOSB1155(_project.token).mint(address(this), _saleInput.tokenUri, _saleInput.amount);
            } else {
...
interface ISale {
    ...
    function createSale(address _caller, bool _isCreateNewToken, bool _isSetRoyalty, ProjectInfo memory _project, SaleInput memory _saleInput) external;
    function getSaleById(uint256 _saleId) external view returns (SaleInfo memory);
    function getSaleNotCloseLength(uint256 _projectId) external view returns (uint256);
...
interface IOSB1155 is IERC1155Upgradeable {
    function mint(address _to, string memory _tokenUri, uint256 _amount) external returns (uint256);
    function mintWithRoyalty(address _to, string memory _tokenUri, uint256 _amount, address _receiverRoyaltyFee, uint96 _percentageRoyaltyFee) external returns (uint256);
    function setController(address _account, bool _allow) external;
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (address, uint256);
}
