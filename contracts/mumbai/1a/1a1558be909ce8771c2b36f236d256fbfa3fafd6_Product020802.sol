...
contract Product020802 {
    ...
    function computeExpirationDate(uint256 coverageStartDate) public pure returns (uint256) {
        uint256 expirationDate = addYears(coverageStartDate, 1);
    ...
    function isLeapYear(uint256 year) private pure returns (bool leapYear) {
        leapYear = ((year % 4 == 0) && (year % 100 != 0)) || (year % 400 == 0);
    }

    function addYears(uint256 timestamp, uint256 _years) private pure returns (uint256 newTimestamp) {
    ...
    function checkBusinessRulesToCreatePolicy(
        ...
        require(
            coverageStartDate < policyToBeChecked.expirationDate,
