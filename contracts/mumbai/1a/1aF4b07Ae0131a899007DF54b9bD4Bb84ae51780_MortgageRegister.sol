...
contract MortgageRegister is AccessControl {
    ...
    IMortgageControl public mortgageControl;

    // contracts Permission
    mapping(address => bool) public allowedSC;

    constructor(address _mortgageControl) {
    ...
    modifier onlyAllowedSC() {
        if (!allowedSC[msg.sender]) {
            revert("You're not Allowed to use this contract");
        }
        _;
    }

    /// @dev function called by Mortgages to calc the interests an register the initial mortgage interest data in Mortgage Control.
    /// @param _id - the user's mortgage ID.
    /// @param _user - the user's address wallet.
    /// @param _totalDebt - the Total debt of the mortgage.
    function registerInitialMortgageData(uint256 _id, address _user, uint256 _totalDebt)
        external
        onlyAllowedSC
        isPaused
    {
        (uint256 dailyInterest, uint256 dailyPanoramInterest, uint256 dailyRewardLenderInterest) =
            calcDailyInterest(_totalDebt);
        (uint256 dailyInterest_D, uint256 dailyPanoramInterest_D, uint256 dailyRewardLenderInterest_D) =
            calcDelayed_DailyInterest(_totalDebt);

        mortgageControl.saveMortgageInterestData(
            _id,
            _user,
            dailyInterest,
            dailyPanoramInterest,
            dailyRewardLenderInterest,
            dailyInterest_D,
            dailyPanoramInterest_D,
            dailyRewardLenderInterest_D
        );
    }

    /// @dev Private Function to calculate the Daily Normal Interest to pay for the Mortgage.
    /// @param _totalDebt - the Total debt of the mortgage.
    /// @return dailyInterest - the total Daily Interest amount to charge to the user for the mortgage. (dailyPanInterest + dailyInterestForRewardLenders)
    /// @return dailyPanInterest - the Daily Interest amount to pay to Panoram for the mortgage.
    /// @return dailyInterestForRewardLenders - the Daily Interest amount to send to Vault Lender Rewards for the mortgage.
        ...
        returns (uint256 dailyInterest, uint256 dailyPanInterest, uint256 dailyInterestForRewardLenders)
    {
        ...
        uint64 mortgageInterest = mortgageControl.getInterestRate();
    ...
    /// @dev Private Function to calculate the Daily Normal Interest to pay for the Mortgage.
    /// @param _totalDebt - the Total debt of the mortgage.
    /// @return D_dailyInterest - the total Delayed Daily Interest amount to charge to the user for the mortgage. (D_dailyPanInterest + D_dailyInterestForRewardLenders)
    /// @return D_dailyPanInterest - the Delayed Daily Interest amount to pay to Panoram for the mortgage.
    /// @return D_dailyInterestForRewardLenders - the Delayed Daily Interest amount to send to Vault Lender Rewards for the mortgage.
    ...
    // ****************** Admin Functions to upgrate contract's variables ******************* //
    ...
    function updateNormalPercentages(uint16 _panoram, uint16 _LenderReward) external onlyDev isPaused {
        percentagePanoram = _panoram;
        percentageLenderRewards = _LenderReward;
    }

    ///@dev Function to update the percentages to pay to Panoram and the lenders rewards pool in past due payments.
    function updateDelayedPercentages(uint16 _panoram, uint16 _LenderReward) external onlyDev isPaused {
        percentagePanoramDelayedPay = _panoram;
        percentageDelayedPayLenderRewards = _LenderReward;
    }
    ...
    /// @dev function give permission to a contract to interact with this contract.
    function setAllowedContract(address _Sc, bool _state) external onlyDev isPaused {
        allowedSC[_Sc] = _state;
    }
}
