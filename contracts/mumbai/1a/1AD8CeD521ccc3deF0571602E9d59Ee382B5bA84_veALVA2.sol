...
contract veALVA2 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    /**
     * @dev Deposits staking token and mints new veTokens for infinite time
     * @param _amount Amount of staking token to deposit
     * @param _account Account for which lockup is created
     */
    function infiniteLockup(address _account, uint256 _amount) public {
        ...
        if (
            oldLockup2.end == 0 &&
            oldLockup2.amount == 0 &&
            _userNonDecayCheckpoints[_account].length == 0
        ) {
            count++;
        }
        ...
        if (userCurrentEpoch == 0) {
            _userNonDecayCheckpoints[_account].push(
                NonDecayCheckpoint({blockNumber: blockNumber - 5, sum: 0})
            );

            NonDecayCheckpoint memory _userNonDecayCheckpoint0;

            // Update checkpoint for the user
            _userNonDecayCheckpoint0.blockNumber = blockNumber;
            _userNonDecayCheckpoint0.sum = (2 * amount);

            _userNonDecayCheckpoints[_account].push(_userNonDecayCheckpoint0);
        } else {
            ...
            _userNonDecayCheckpoints[_account].push(_userNonDecayCheckpoint1);
        }

        userEpochI[_account] = userCurrentEpoch + 1;

        // Checkpoints for the user and allUser of infinite locking
        NonDecayCheckpoint memory _allUsersNonDecayCheckpoint0;

        _allUsersNonDecayCheckpoint0.blockNumber = blockNumber;
        _allUsersNonDecayCheckpoint0.sum =
            _allUsersNonDecayCheckpoints[
                _allUsersNonDecayCheckpoints.length - 1
            ].sum +
            (2 * amount);

        _allUsersNonDecayCheckpoints.push(_allUsersNonDecayCheckpoint0);
    ...
    /**
     * @dev Deposits staking token and mints new veTokens according to the lockup end time
     * @param _amount Amount of staking token to deposit
     * @param _account Account for which lockup is created
     * @param _end End time for lockup
     */
    function finiteLockup(
        address _account,
        uint256 _amount,
        uint256 _end
    ) public {
