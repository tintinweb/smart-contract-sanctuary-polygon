...
contract LiquidityPool is IERC777Recipient, DSMath, ReentrancyGuard {
    ...
    address public maticIn;
    event AddLiquidityToken1(address presale, uint256 amount1, address token1);
    event AddLiquidityToken0(address presale, uint256 amount0, address token0);
    ...
    receive() external payable {
        if (msg.sender != presale || msg.sender != owner) {
            buy(msg.sender, msg.value);
        } else if (msg.sender == presale || msg.sender == presale) {
            reserve0 += msg.value;
    ...
    function sell(address payable seller, uint256 tokenAmount) public payable nonReentrant {
        ...
        if (legalTrade) {
            // Token is desired, proceed with the sale
            ...
            // Token is not desired, revert with an error message
            revert("Only the desired token can be sold here.");
        }
    }

    function prevalidateTrading(uint256 tokenAmount) internal {
        require(tokenAmount >= 100 ether, "Minimum token amount not met");
        legalTrade = (tokenIn == token1);
    }
    ...
    function buy(address buyer, uint256 amount) public payable nonReentrant {
        ...
        if (legalTrade) {
            ...
            // Token is not desired, revert with an error message
            revert("Only the desired token can be bought here.");
        }
    }

    function prevalidatePurchase(uint256 amount) internal {
        require(amount >= 1, "Minimum purchase amount not met");
        legalTrade = (maticIn == token0);
    }
