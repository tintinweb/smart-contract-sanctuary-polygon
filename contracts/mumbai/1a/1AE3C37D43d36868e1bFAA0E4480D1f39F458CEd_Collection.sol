...
contract Collection is ERC721EnumerableUpgradeable {
	...
	modifier isHalted() {
		require(!management.halted(), "Under Maintenance");
		_;
	}

	modifier CollectionOwner() {
        require(
            management.isOwner(address(this), msg.sender), "Collection Owner only"
        );
        _;
    }
	...
	modifier NudgeAdmin() {
        require(
            management.isAdmin(msg.sender), "Nudge Admin only"
        );
        _;
    }
	...
	/**
       	@notice Update Address of NudgeManagement contract
       	@dev  Caller must be Nudge Admin
		@param	_management				Address of NudgeManagement contract
    */
	function setManagement(IManagement _management) external NudgeAdmin {
		require(address(_management) != address(0), "Set Zero Address");
	...
	/**
       	...
       	@dev  Caller must be Owner of this collection
       	@param _newURI		New string of `baseURI`
    */
	function setURI(string memory _newURI) external CollectionOwner {
	...
	/**
		...
		@param	_fromID			Starting number of TokenID
		@param	_amount			Amount of NFTs to be minted
    */
	function mint(address _to, uint256 _fromID, uint256 _amount) external CollectionOwner {
