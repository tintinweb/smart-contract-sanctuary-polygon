...
contract UTDigitalRFactory is Ownable, BlackHolePrevention {
    ...
    //     pick                                    // 2 - Nonrandom redeem from the vault (user may select which card to transfer from vault) (no VRF)
    // }
    ...
    struct inputTraitStruct {
        ...
        uint8   redeemMode;                     // 0 - Random redeem from the vault (with VRF)
                                                ...
                                                // 2 - Nonrandom redeem from the vault (user may select which card to transfer from vault) (no VRF)
    }
...
contract UTDigitalRedeemStorage is UTGenericDroppableStorage {
    ...
    uint8       public immutable redeemMode;                    // 0 - Random redeem from the vault (with VRF), 1 - Nonrandom redeem from the vault (transfer first card from vault) (no VRF), 2 - Nonrandom redeem from the vault (user may select which card to transfer from vault) (no VRF)
    ...
    constructor(
        ...
        uint8   _redeemMode,                        // 0 - Random redeem from the vault (with VRF), 1 - Nonrandom redeem from the vault (transfer first card from vault) (no VRF), 2 - Nonrandom redeem from the vault (user may select which card to transfer from vault) (no VRF)
...
contract UTDigitalRedeemController is UTGenericTimedController, UTDigitalRedeemVault {
    ...
    uint256     public constant version     = 202300602;
...
abstract contract UTDigitalRedeemVault is ERC721Holder, BlackHolePreventionForCROwner {
    ...
    //     uint256 balance = token.balanceOf(address(this));
    //     require(balance >= _tokensToDrop , "UTDigitalRedeemVault: No available tokens in vault");

    //     ITraitStorage traitStorage = ITraitStorage(_traitStorage);
    //     uint8 newTraitValue = traitStorage.getValue(_tokenId) + _tokensToDrop;
    //     require( newTraitValue <= TRAIT_OPEN_VALUE + traitStorage.maxTokensToRedeem(), "UTDigitalRedeemVault: claim over limit");
     ...
     *      Has an unbounded cost!
     */
    function getAllVaultsDetails() public view returns (vaultDetails[] memory)
    {
        uint256 vaultLength = tokenContracts.length();
        vaultDetails[] memory retval = new vaultDetails[](vaultLength);
        for(uint16 contractIdx = 0; contractIdx < vaultLength; contractIdx++) {
            ...
            if (erc165.supportsInterface(type(IERC721Metadata).interfaceId)) {
                ...
                if (balance > 0) {
                    ...
                    retval[contractIdx] = vaultDetails(tokenContract , metaData.name(), metaData.symbol(), balance, metaData.tokenURI(erc721.tokenOfOwnerByIndex(address(this), 1)));
                } else {
                    try metaData.tokenURI(0) returns (string memory uri) {
