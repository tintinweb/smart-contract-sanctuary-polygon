...
contract SK8Staking is Pausable, Ownable {
    ...
    function setInterestRate(uint pDays, uint256 newValue)
        external
        onlyOwner
        whenNotPaused
    {
    ...
    function addressTokenIdStakedDetails(address staker, uint256 tokenId)
        external
        view
    ...
    function addressStakedTokenIds(address account)
        external
        view
     ...
     * @param amount amount of SK8 tokens to stake
     */
    function increaseAmountStaked(uint256 tokenId, uint256 amount)
        external
        whenNotPaused
        moreThanZero(amount)
     ...
     */
    function claimReward(uint256 tokenId, uint256 amount)
        external
        whenNotPaused
        moreThanZero(amount)
        lockPeriodNotFinished(msg.sender, tokenId)
     ...
     */
    function increaseLockPeriod(uint256 tokenId, uint256 periodInDays)
        external
        whenNotPaused
        updateReward(msg.sender, tokenId)
    {
