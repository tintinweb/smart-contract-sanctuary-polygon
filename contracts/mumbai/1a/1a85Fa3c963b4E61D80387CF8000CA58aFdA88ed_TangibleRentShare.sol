...
contract TangibleRentShare is AccessControl {
    ...
    function _distributorForToken(address contractAddress, uint256 tokenId)
        private
        view
        returns (RentDistributor distributor)
    {
        bytes memory token = abi.encodePacked(contractAddress, tokenId);
        distributor = _contractForToken[token].rentDistributor;
        ...
        returns (RevenueShare)
    {
        ...
        if (address(_contractForToken[token].revenueShare) == address(0)) {
            ...
            IOps(ops).createTimedTask(
                uint128(
                    (block.timestamp / 86400 + 1) * 86400 + 10 * _tokens.length
                ),
                86400,
                address(distributor),
                distributor.distribute.selector,
                address(forwarder),
                abi.encodeWithSelector(forwarder.checker.selector),
