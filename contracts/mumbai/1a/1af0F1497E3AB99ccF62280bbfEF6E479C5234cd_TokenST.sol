...
interface ICompliance {
    ...
    function transferOwnershipOnComplianceContract(address pendingOwner) external;

    function acceptOwnershipOnComplianceContract() external;
...
interface IIdentityRegistry {
    ...
    event OwnerNominated(address _pendingOwner);
    ...
    function transferOwnershipOnIdentityRegistryContract(address _pendingOwner) external;

    function acceptOwnershipOnIdentityRegistryContract() external;
    /**
...
interface IToken is IERC20 {
    ...
    event FundAdded(address tokenFund);
    ...
    function transferOwnershipOnTokenContract(address _pendingOwner) external;

    function acceptOwnershipOnTokenContract() external;

    // function addAdmin(address _agent) external;
    function addSubAdmin(address _user) external;

    // function revokeAdmin(address _agent) external;
    function revokeSubAdmin(address _user) external;

    // function addTokenizationAgentOnToken(address _userAddress) external;
    function addTokenizationAgent(address _user) external;

    // function removeTokenizationAgentOnToken(address _userAddress) external;
    function removeTokenizationAgent(address _user) external;

    // function addTransferAgentOnToken(address _userAddress) external;
    function addTransferAgent(address _user) external;

    // function removeTransferAgentOnToken(address _userAddress) external;
    function removeTransferAgent(address _user) external;

    // function setRolesToTokenAgent(address _userAddress, bytes32[] memory roles) external;
    function setRolesToAgent(address _user, bytes32[] memory roles) external;

    // function revokeRolesOnTokenAgent(address _userAddress, bytes32[] memory roles) external;
    function revokeRolesOnAgent(address _user, bytes32[] memory roles) external;
...
contract AgentRoleUpgradeable is AccessControlUpgradeable, OwnableUpgradeable {
    ...
    function revokeSubAdmin(address _user) public virtual onlyAdmins {
    ...
    function setRolesToAgent(address _user, bytes32[] memory roles) public virtual onlyAdmins{
    ...
    function revokeRolesOnAgent(address _user, bytes32[] memory roles) public virtual onlyAdmins{
        for(uint i = 0; i < roles.length; i++)
        {
...
contract TokenST is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    /**
     *  @dev See {IToken-setFund}.
     */
    function setFund(address _fund) external override onlyAdmins {
        tokenFund = _fund;
        emit FundAdded(tokenFund);
    }
     ...
     */
    function transferOwnershipOnTokenContract(address _pendingOwner) external override onlyAdmins {
    ...
    function acceptOwnershipOnTokenContract() external {
        require(msg.sender == potentialOwner, "must be nominated as potential owner");
        transferOwnership(potentialOwner);
    ...
    function setRolesToAgent(address _user, bytes32[] memory roles) public override (IToken, AgentRoleUpgradeable) {
        super.setRolesToAgent(_user, roles);
    }

    function revokeRolesOnAgent(address _user, bytes32[] memory roles) public override (IToken, AgentRoleUpgradeable){
        super.revokeRolesOnAgent(_user, roles);
    }

    function revokeSubAdmin(address _user) public override (IToken, AgentRoleUpgradeable) {
        super.revokeSubAdmin(_user);
    }
