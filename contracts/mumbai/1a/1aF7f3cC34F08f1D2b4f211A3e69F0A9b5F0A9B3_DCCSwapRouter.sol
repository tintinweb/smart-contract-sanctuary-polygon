...
contract DCCSwapRouter is IDCCSwapRouter {
    using DCCSwapSafeMath for uint256;

    address public immutable override factory;
    address public immutable override WDCC;
    address public immutable  feeMethod;


    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "DCCSwapRouter::ensure::EXPIRED");
        _;
    }

    constructor(address _factory, address _WDCC,address _feeMethod) public {
        factory = _factory;
        WDCC = _WDCC;
    ...
    receive() external payable {
        assert(msg.sender == WDCC);
        // only accept ETH via fallback from the WDCC contract
    }
    ...
    {
        {
            IDCCSwapFeeMethod.AddLiquidityFeeStruct memory  addLiquidityFeeStruct;
            (amountA, amountB,  addLiquidityFeeStruct) = IDCCSwapFeeMethod(feeMethod)._addLiquidity(tokenA, tokenB, amountADesired, amountBDesired, amountAMin, amountBMin);
            if(addLiquidityFeeStruct.feeA>0 && addLiquidityFeeStruct.feeToA!=address(0)){
                TransferHelper.safeTransferFrom(tokenA, msg.sender, addLiquidityFeeStruct.feeToA, addLiquidityFeeStruct.feeA);
            }
            if(addLiquidityFeeStruct.feeB>0 && addLiquidityFeeStruct.feeToB!=address(0)){
                TransferHelper.safeTransferFrom(tokenB, msg.sender, addLiquidityFeeStruct.feeToB, addLiquidityFeeStruct.feeB);
            }
        }
        {
            address pair = DCCSwapLibrary.pairFor(factory, tokenA, tokenB);

            TransferHelper.safeTransferFrom(tokenA, msg.sender, pair, amountA);
            TransferHelper.safeTransferFrom(tokenB, msg.sender, pair, amountB);
            liquidity = IDCCSwapPair(pair).mint(to);
        }

        // amountA = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(amountA, tokenA, "computeAddLiquidityFee(uint256)");
        // amountB = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(amountB, tokenB, "computeAddLiquidityFee(uint256)");

        // (amountA , amountB) = DCCSwapFeeMethodLibrary.computeAddLiquidateTokenABAmount(amountA ,  tokenA,  amountB ,  tokenB);
        // amountA = DCCSwapFeeMethodLibrary.computeAddLiquidityFee(amountA, tokenA);
        // amountB = DCCSwapFeeMethodLibrary.computeAddLiquidityFee(amountB, tokenB);
    ...
    {
        IDCCSwapFeeMethod.AddLiquidityFeeStruct memory  addLiquidityFeeStruct;
        (amountToken, amountETH,  addLiquidityFeeStruct) = IDCCSwapFeeMethod(feeMethod)._addLiquidity(token, WDCC, amountTokenDesired, msg.value, amountTokenMin, amountETHMin);
        address pair = DCCSwapLibrary.pairFor(factory, token, WDCC);
        // amountToken = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(amountToken, token, "computeAddLiquidityFee(uint256)");
    //    amountToken = DCCSwapFeeMethodLibrary.computeAddLiquidityFee(amountToken, token);
        if(addLiquidityFeeStruct.feeA>0 && addLiquidityFeeStruct.feeToA!=address(0)){
            TransferHelper.safeTransferFrom(token, msg.sender, addLiquidityFeeStruct.feeToA, addLiquidityFeeStruct.feeA);
        }
        // feeB 不用扣，只是要减掉就好  amountETH 已经是同频减掉了的
        TransferHelper.safeTransferFrom(token, msg.sender, pair, amountToken);
        IWDCC(WDCC).deposit{value : amountETH}();
        assert(IWDCC(WDCC).transfer(pair, amountETH));
    ...
    function removeLiquidity(
        ...
        address pair = DCCSwapLibrary.pairFor(factory, tokenA, tokenB);
        IDCCSwapPair(pair).transferFrom(msg.sender, pair, liquidity);
        // send liquidity to pair
        (uint256 amount0, uint256 amount1) = IDCCSwapPair(pair).burn(to);
        (address token0,) = DCCSwapLibrary.sortTokens(tokenA, tokenB);
        (amountA, amountB) = tokenA == token0 ? (amount0, amount1) : (amount1, amount0);
        require(amountA >= amountAMin, "DCCSwapRouter::removeLiquidity::INSUFFICIENT_A_AMOUNT");
        require(amountB >= amountBMin, "DCCSwapRouter::removeLiquidity::INSUFFICIENT_B_AMOUNT");
    }
    ...
    function removeLiquidityETH(
        ...
        // removeLiquidity 里面已经burn 过了，不需要外面再收一次手续费
        TransferHelper.safeTransfer(token, to, amountToken);
        IWDCC(WDCC).withdraw(amountETH);
    ...
    function removeLiquidityWithPermit(
        ...
        IDCCSwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function removeLiquidityETHSupportingFeeOnTransferTokens(
        ...
        (, amountETH) = removeLiquidity(token, WDCC, liquidity, amountTokenMin, amountETHMin, address(this), deadline);
        // removeLiquidity 里面已经burn 过了，不需要外面再收一次手续费
        TransferHelper.safeTransfer(token, to, IDCCSwapBEP20(token).balanceOf(address(this)));
        IWDCC(WDCC).withdraw(amountETH);
        TransferHelper.safeTransferETH(to, amountETH);
    }
    ...
    function swapExactTokensForTokens(
        ...
        (amounts, amountFees) = DCCSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "DCCSwapRouter::swapExactTokensForTokens::INSUFFICIENT_OUTPUT_AMOUNT"
        );

        uint256 sellAmount = amounts[0];
        // sellAmount = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(sellAmount, path[0], "computeSellFee(uint256)");
        // sellAmount = IDCCSwapFeeMethod(feeMethod).computeSellFee(sellAmount, path[0]);
       sellAmount = DCCSwapFeeMethodLibrary.computeSellFee(sellAmount, path[0]);


        // 卖出回调  amounts[0] 应该是扣了手续费的
        TransferHelper.safeTransferFrom(path[0], msg.sender, DCCSwapLibrary.pairFor(factory, path[0], path[1]), sellAmount);
        // 买入回调，amounts[1:} 后续应该是扣除手续费的
        _swap(amounts, path, to,amountFees,amountIn, to);
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountOut, amountIn, to);
        ...
        //     IDCCSwapFeeCallBack(swapFeeCallBackAddress).AfterSwapFeeCallBack(amountIn, amountFees, path, to);
        // }
    }
    ...
    function swapTokensForExactTokens(
        ...
        uint256[] memory amountFees;
        (amounts, amountFees) = DCCSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "DCCSwapRouter::swapTokensForExactTokens::EXCESSIVE_INPUT_AMOUNT");

        uint256 sellAmount = amounts[0];
        // sellAmount = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(sellAmount, path[0], "computeSellFee(uint256)");
       sellAmount = DCCSwapFeeMethodLibrary.computeSellFee(sellAmount, path[0]);
       (amounts, amountFees) = DCCSwapLibrary.getAmountsOut(factory, sellAmount, path);

        // 卖出回调  amounts[0] 应该是扣了手续费的
        TransferHelper.safeTransferFrom(path[0], msg.sender, DCCSwapLibrary.pairFor(factory, path[0], path[1]), sellAmount);
        // 买入回调，amounts[1:} 后续应该是扣除手续费的
        _swap(amounts, path, to,amountFees,amountOut, to);
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountIn, amountOut, to);

        // address swapFeeCallBackAddress = IDCCSwapFactory(factory).swapFeeCallBackAddress();
        // bool swapFeeCallBackAddressOn = swapFeeCallBackAddress != address(0);
        // if (swapFeeCallBackAddressOn) {
        //     IDCCSwapFeeCallBack(swapFeeCallBackAddress).AfterSwapFeeCallBack(amountOut, amountFees, path, to);
        // }
    }
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == WDCC, "DCCSwapRouter::swapExactETHForTokens::INVALID_PATH");
        uint256[] memory amountFees;
        (amounts, amountFees) = DCCSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "DCCSwapRouter::swapExactETHForTokens::INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IWDCC(WDCC).deposit{value : amounts[0]}();
        assert(IWDCC(WDCC).transfer(DCCSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        //eth 不会存在卖出扣除手续费的问题
        _swap(amounts, path, to,amountFees,msg.value, to);
        // 买入回调，amounts[1:} 后续应该是扣除手续费的
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountOut, msg.value, to);

        // address swapFeeCallBackAddress = IDCCSwapFactory(factory).swapFeeCallBackAddress();
        // bool swapFeeCallBackAddressOn = swapFeeCallBackAddress != address(0);
        // if (swapFeeCallBackAddressOn) {
        //     IDCCSwapFeeCallBack(swapFeeCallBackAddress).AfterSwapFeeCallBack(msg.value, amountFees, path, to);
        // }
    }
    ...
    function swapTokensForExactETH(
        ...
        require(path[path.length - 1] == WDCC, "DCCSwapRouter::swapTokensForExactETH::INVALID_PATH");
        uint256[] memory amountFees;
        (amounts, amountFees) = DCCSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, "DCCSwapRouter::swapTokensForExactETH::EXCESSIVE_INPUT_AMOUNT");
       ...
       (amounts, amountFees) = DCCSwapLibrary.getAmountsOut(factory, sellAmount, path);
        // 卖出回调  amounts[0] 应该是扣了手续费的
        TransferHelper.safeTransferFrom(path[0], msg.sender, DCCSwapLibrary.pairFor(factory, path[0], path[1]), sellAmount);
        _swap(amounts, path, address(this),amountFees,amountOut, to);
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountIn, amountOut, to);

        // 买入回调，amounts[1:} 后续应该是扣除手续费的
        IWDCC(WDCC).withdraw(amounts[amounts.length - 1]);
    ...
    function swapExactTokensForETH(
        ...
        require(path[path.length - 1] == WDCC, "DCCSwapRouter::swapExactTokensForETH::INVALID_PATH");
        uint256[] memory amountFees;
        (amounts, amountFees) = DCCSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(
            amounts[amounts.length - 1] >= amountOutMin,
            "DCCSwapRouter:swapExactTokensForETH::INSUFFICIENT_OUTPUT_AMOUNT"
        );
       ...
       sellAmount = DCCSwapFeeMethodLibrary.computeSellFee(sellAmount, path[0]);
        // 卖出回调  amounts[0] 应该是扣了手续费的
        TransferHelper.safeTransferFrom(path[0], msg.sender, DCCSwapLibrary.pairFor(factory, path[0], path[1]), sellAmount);
        _swap(amounts, path, address(this),amountFees,amountIn, to);
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountOut, amountIn, to);
        ...
        //     IDCCSwapFeeCallBack(swapFeeCallBackAddress).AfterSwapFeeCallBack(amountIn, amountFees, path, to);
        // }
        // if (IDCCSwapFactory(factory).swapFeeCallBackAddress()!= address(0) ) {
        //     IDCCSwapFeeCallBack(IDCCSwapFactory(factory).swapFeeCallBackAddress()).AfterSwapFeeCallBack(amountIn, amountFees, path, to);
        // }
    }
    ...
    function swapETHForExactTokens(
        ...
        require(path[0] == WDCC, "DCCSwapRouter::swapETHForExactTokens::INVALID_PATH");
        uint256[] memory amountFees;
        (amounts, amountFees) = DCCSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= msg.value, "DCCSwapRouter::swapETHForExactTokens::EXCESSIVE_INPUT_AMOUNT");
        IWDCC(WDCC).deposit{value : amounts[0]}();
        assert(IWDCC(WDCC).transfer(DCCSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        // 卖出回调  amounts[0] 应该是扣了手续费的
        _swap(amounts, path, to,amountFees,amountOut, to);
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        // 卖出回调  amountIn 应该是扣了手续费的
        // amountIn = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(amountIn, path[0], "computeSellFee(uint256)");
       amountIn = DCCSwapFeeMethodLibrary.computeSellFee(amountIn, path[0]);
        TransferHelper.safeTransferFrom(path[0], msg.sender, DCCSwapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IDCCSwapBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IDCCSwapBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "DCCSwapRouter::swapExactTokensForTokensSupportingFeeOnTransferTokens::INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        ...
        require(path[0] == WDCC, "DCCSwapRouter::swapExactETHForTokensSupportingFeeOnTransferTokens::INVALID_PATH");
        uint256 amountIn = msg.value;
        IWDCC(WDCC).deposit{value : amountIn}();
        assert(IWDCC(WDCC).transfer(DCCSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IDCCSwapBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IDCCSwapBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            "DCCSwapRouter::swapExactETHForTokensSupportingFeeOnTransferTokens::INSUFFICIENT_OUTPUT_AMOUNT"
        );
    }
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(
        ...
        require(
            path[path.length - 1] == WDCC,
            "DCCSwapRouter::swapExactTokensForETHSupportingFeeOnTransferTokens::INVALID_PATH"
        );
        // 卖出回调  amountIn 应该是扣了手续费的
        // amountIn = IDCCSwapFeeMethod(feeMethod).computeFreeSwapFee(amountIn, path[0], "computeSellFee(uint256)");
       amountIn = DCCSwapFeeMethodLibrary.computeSellFee(amountIn, path[0]);
        ...
        uint256 amountOut = IDCCSwapBEP20(WDCC).balanceOf(address(this));
        require(
            amountOut >= amountOutMin,
            "DCCSwapRouter::swapExactTokensForETHSupportingFeeOnTransferTokens::INSUFFICIENT_OUTPUT_AMOUNT"
        );
        IWDCC(WDCC).withdraw(amountOut);
    ...
    function quote(
        ...
        return DCCSwapLibrary.quote(amountA, reserveA, reserveB);
    }
    ...
    function getAmountOut(
        ...
        uint256 amountFee;
        (amountOut, amountFee) = DCCSwapLibrary.getAmountOut(factory, amountIn, reserveIn, reserveOut);
        return amountOut;
    }
    ...
    function getAmountIn(
        ...
        uint256 amountFee;
        (amountIn, amountFee) = DCCSwapLibrary.getAmountIn(factory, amountOut, reserveIn, reserveOut);
        return amountIn;
    }
    ...
    function _swapSupportingFeeOnTransferTokens( address[] memory path, address _to) internal virtual  {
        ...
        // IDCCSwapFeeMethod(feeMethod).transferFeeTo(amountFees,path,IDCCSwapFeeMethod.AmountDirect.AmountOut, 0 , address(0));

        // address swapFeeCallBackAddress = IDCCSwapFactory(factory).swapFeeCallBackAddress();
        // bool swapFeeCallBackAddressOn = swapFeeCallBackAddress != address(0);
        if (IDCCSwapFactory(factory).swapFeeCallBackAddress()!= address(0) ) {
            IDCCSwapFeeCallBack(IDCCSwapFactory(factory).swapFeeCallBackAddress()).AfterSwapFeeSupportingFeeOnTransferTokensCallBack(0, amountFees, path, _to);
        }
    }
    ...
    function _swap(
        ...
        if (IDCCSwapFactory(factory).swapFeeCallBackAddress()!= address(0) ) {
            IDCCSwapFeeCallBack(IDCCSwapFactory(factory).swapFeeCallBackAddress()).AfterSwapFeeCallBack(amountOutOrIn, amountFees, path, target);
        }
    }
}
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"8e61e1102fd15901bb2d1773618a5159cdcf364983b9d44514b249ed351af48b" // polygontest
                        // hex"996fa0d12914138bf9134d4d0be3fd551c092765b68036484d33e466124d7dea"//rinkby
                        // hex"0127f97bd2757c1fbd402f0f9d6d3a5ea575447b403e33a17f4aee41a3026b74"
                    )
                )
            )
        );
