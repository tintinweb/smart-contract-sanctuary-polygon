...
contract AirdropToken is Ownable {
    ...
    uint256 public totalAirdropCount = 0;

    mapping(address => uint256) public userAirdrop;
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _recipients.length; i++) {
            if (userAirdrop[_recipients[i]] == 0) {
                IERC20(airdropToken).safeTransfer(_recipients[i], _amount);
                totalAirdrop += _amount;
                totalAirdropCount++;
                userAirdrop[_recipients[i]] = _amount;
            }
        }
    }
