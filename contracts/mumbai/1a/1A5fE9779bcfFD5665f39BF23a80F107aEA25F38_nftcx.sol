...
abstract contract ReentrancyGuard {
    ...
    enum typeSet {AllSet, GoldSet, SilverSet, BronzeSet, taskUserSet, allGoldUserSet, allSilverUserSet, allBronzeUserSet, GoldLastIndexSet, fatherAddressSet}
interface IERC721 {
    function transferFrom(
        address from,
        address to,
...
contract nftcx is Ownable {
    ...
    event getReward(uint256 _time, address _childAddress, address _parentAddress, uint256 _rewardAmount);
    ...
    function buyNFT(address _referer) external {
        ...
        if (nodeList[_referer] == address(1)) {
            ...
            emit getReward(block.timestamp, _user, _referer, rewardForGold);
        } else {
            //推荐人是铜卡
            ...
            emit BindReferer(block.timestamp, _user, _referer, _node);
            emit getReward(block.timestamp, _user, _node, rewardForGold);
            emit getReward(block.timestamp, _user, _referer, rewardForBronze);
    ...
    function getSet(uint256 _setType, address _user) external view returns (address[] memory _addressList, uint256[] memory _NodeAmountList, uint256 _num, string  memory _typeName) {
        if (_setType == 0) {
            _typeName = "AllSet";
        ...
        if (_setType == 8) {
            ...
            _NodeAmountList = GoldLastIndexList;
    ...
    function getSetByIndexList(uint256 _setType, address _user, uint256[] memory _indexList) external view returns (address[] memory _addressList, uint256[] memory _NodeAmountList, uint256 _num, string  memory _typeName) {
        ...
        if (_setType == 8) {
            _typeName = "GoldLastIndexList";
            for (uint256 i = 0; i < _num; i++) {
                _NodeAmountList[i] = GoldLastIndexList[_indexList[i]];
            }
        }
        if (_setType == 9) {
            _typeName = "fatherAddressSet";
