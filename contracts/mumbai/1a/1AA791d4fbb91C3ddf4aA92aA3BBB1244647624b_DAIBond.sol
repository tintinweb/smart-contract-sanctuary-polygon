...
contract DAIBond is AddressAccessor, Pausable {
    ...
    function deposit(uint256 amount) external whenNotPaused {
        ...
        IERC20(underlying).transferFrom(who, treasury, amount);
        uint256 amountOut = IExchange(exchange).scaleFromUnderlying(
            amount + (amount * apr) / 100
        );
        ...
        v.claimed = 0;
        vesting[who] = v;
        if (v.fullyVestedAmount > v.usdr) {
            uint256 amountToMint = v.fullyVestedAmount - v.usdr;
            ...
            v.usdr += IExchange(exchange).swapFromTNGBL(
                tngblAmount,
                amountToMint,
                address(this)
            );
    ...
    function claim(uint256 amount) public {
        ...
        (address usdr, address susdr) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(USDR_ADDRESS, sUSDR_ADDRESS)
            ),
            (address, address)
        );
        Vesting memory v = vesting[who];
        ...
        if (totalClaimed == v.fullyVestedAmount) {
            ...
            vesting[who].usdr = v.usdr - amount;
        }
