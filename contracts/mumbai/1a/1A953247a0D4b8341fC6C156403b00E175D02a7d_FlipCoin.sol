...
contract FlipCoin is VRFConsumerBase, Ownable {
        ...
        returns (bytes32 requestId)
    {
        ...
        if (_coinType == 0) {
            ...
            IERC20(MMFAddress).transfer(
                ...
                (2 * requestFlipConfirmList[_requestId].coinAmount * 965) / 1000
            );
            IERC20(MMFAddress).transfer(
                adminWallet1,
            ...
            IERC20(MMFAddress).transfer(
                adminWallet2,
                (2 * requestFlipConfirmList[_requestId].coinAmount * 2) / 100
            );
            IERC20(MMFAddress).transfer(
                adminWallet3,
                (2 * requestFlipConfirmList[_requestId].coinAmount * 45) / 10000
            );
        }
     ...
     * @param _coinType bet type 0: MATIC, 1: MMF
     */
    function test(uint256 _coinType, uint256 _coinAmountIndex)
        external
        payable
        returns (bytes32 requestId)
    {
        ...
        IERC20(MMFAddress).transfer(
            adminWallet1,
            (2 * coinType[_coinType][_coinAmountIndex] * 105) / 10000
        );
        IERC20(MMFAddress).transfer(
            adminWallet2,
            (2 * coinType[_coinType][_coinAmountIndex] * 2) / 100
        );
        IERC20(MMFAddress).transfer(
            adminWallet3,
            (2 * coinType[_coinType][_coinAmountIndex] * 45) / 10000
        );
        return 0;
    }
