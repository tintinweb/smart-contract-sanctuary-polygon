 ...
 * Has a function for recieving blog donation.
 * Has functions to claim user event reward, blog event reward and donation.
 * Has functions for retriving blog and event information.
...
contract Blogging is Ownable, AccessControl, Pausable {
    ...
    struct Blog {
        ...
        mapping(string => BlogEvent) blogEventByEventId;
    ...
    mapping(address => mapping(string => BlogEvent)) public blogEventByAddressByEventId;
    ...
    event UserEventRewardClaimed(address userAddress, string indexed eventId, uint256 eventReward);
    ...
    event AdminFeesWithdraw(address adminAddress, uint256 fees);
    ...
    /**
     * @dev Create blog and setup variables.
     * @param _blogId - Blog id.
    ...
    /**
     * @dev Update event reward data, transfer token to blog owner.
     * @param _blogId - Blog id.
     * @param _eventId - Relate event id for reward.
     * @param _eventReward - BlogEvent reward amount.
     */
    function claimBlogEventReward(
        string memory _blogId,
        string memory _eventId,
        uint256 _eventReward
    ) external onlyRole(WORKER) blogCreated(_blogId) {
        Blog storage blog = blogByBlogId[_blogId];
        BlogEvent storage blogEvent = blog.blogEventByEventId[_eventId];

        require(
            bytes(blogEvent.eventId).length == 0,
            "[Blogging.claimBlogEventReward] Blog event reward already claimed"
        );
        require(
            eventRewardBalance >= _eventReward,
            "[Blogging.claimBlogEventReward] Insufficient event reward balance"
        );
        eventRewardBalance -= _eventReward;
        blog.blogTotalEventReward += _eventReward;
        blogEvent.eventId = _eventId;
        blogEvent.eventReward = _eventReward;
        balanceVault.increaseBalance(blog.blogOwner, _eventReward);
        balanceVault.decreaseBalance(address(this), _eventReward);

        emit BlogEventRewardClaimed(_blogId, _eventId, _eventReward);
    }

    /**
     * @dev Update event reward data, transfer token to user.
     * @param _userAddress - User address.
     * @param _eventId - Relate event id for reward.
     * @param _eventReward - BlogEvent reward amount.
     */
    function claimUserEventReward(
        address _userAddress,
        string memory _eventId,
        uint256 _eventReward
    ) external onlyRole(WORKER) {
        BlogEvent storage blogEvent = blogEventByAddressByEventId[_userAddress][_eventId];

        require(
            bytes(blogEvent.eventId).length == 0,
            "[Blogging.claimUserEventReward] User event reward already claimed"
        );
        require(
            eventRewardBalance >= _eventReward,
            "[Blogging.claimUserEventReward] Insufficient event reward balance"
        );
        eventRewardBalance -= _eventReward;
        blogEvent.eventId = _eventId;
        blogEvent.eventReward = _eventReward;
        balanceVault.increaseBalance(_userAddress, _eventReward);
        balanceVault.decreaseBalance(address(this), _eventReward);

        emit UserEventRewardClaimed(_userAddress, _eventId, _eventReward);
    }
     ...
     * @param _depositAmount - BlogEvent reward deposit amount.
     */
    ...
    */
    ...
    {
        BlogEvent storage blogEvent = blogByBlogId[_blogId].blogEventByEventId[_eventId];
        
        eventId = blogEvent.eventId;
        eventReward = blogEvent.eventReward;
    }

    /** 
    * @dev Retrieve user event info.
    * @param _userAddress - User address.
    * @param _eventId - Event id.
    */
    function getUserEventInfo(address _userAddress, string memory _eventId)
        external
        view
        returns (
            string memory eventId,
    ...
    */
    function getBlogEventList(string memory _blogId, string[] memory _eventIdList)
        ...
        view
        returns (
            BlogEvent[] memory blogEventList
        )
    {
        Blog storage blog = blogByBlogId[_blogId];

        blogEventList = new BlogEvent[](_eventIdList.length);
        for (uint256 i = 0; i < _eventIdList.length; i++) {
            blogEventList[i] = blog.blogEventByEventId[_eventIdList[i]];
        }
    }

    /** 
    * @dev Retrieve user event specify in event id list.
    * @param _userAddress - User address.
    * @param _eventIdList - Array of event id.
    */
    function getUserEventList(address _userAddress, string[] memory _eventIdList)
        external
        view
        returns (
            BlogEvent[] memory blogEventList
        )
    {
        blogEventList = new BlogEvent[](_eventIdList.length);
        for (uint256 i = 0; i < _eventIdList.length; i++) {
            blogEventList[i] = blogEventByAddressByEventId[_userAddress][_eventIdList[i]];
        }
    }
}
