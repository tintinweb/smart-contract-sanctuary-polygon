...
/// These Retirement Certificate NFTs display how many BCO2s a user has burnt
...
contract RetirementCertificates is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable, RetirementCertificatesStorage {
    // ----------------------------------------
    //      Libraries
    // ----------------------------------------
    ...
    function initialize(address _contractRegistry, string memory _baseURI) external virtual initializer {
        ...
        __ERC721_init_unchained('BuckMint Protocol: Retirement Certificates for Tokenized Carbon Offsets', 'BUCKMINT-CERT');
        __Ownable_init_unchained();
    ...
    /// @param retiringEntity The entity that has retired BCO2 and is eligible to mint an NFT.
    /// @param projectVintageTokenId The vintage id of the BCO2 that is retired.
    /// @param amount The amount of the BCO2 that is retired.
    /// @param isLegacy Whether this event registration was executed by using the legacy retired
    /// amount in the BCO2 contract or utilizes the new retirement event design.
    /// @dev    The function can either be only called by a valid BCO2 contract.
    function registerEvent(address retiringEntity, uint256 projectVintageTokenId, uint256 amount, bool isLegacy) external returns (uint256) {
        // Logic requires that minting can only originate from a project-vintage ERC20 contract
        require(IBuckMintContractRegistry(contractRegistry).checkERC20(msg.sender), 'Caller not a BCO2');
        require(amount != 0 && amount >= minValidRetirementAmount, 'Invalid amount');
    ...
    function attachRetirementEvents(uint256 tokenId, uint256[] calldata retirementEventIds) external {
    ...
    function _attachRetirementEvents(uint256 tokenId, address retiringEntity, uint256[] calldata retirementEventIds) internal {
        ...
        for (uint256 i; i < retirementEventIds.length; ++i) {
            ...
            require(retirements[eventId].retiringEntity == retiringEntity, 'Invalid event to be claimed');
    ...
    /// @notice Mint new Retirement Certificate NFT that shows how many BCO2s have been retired.
    /// @param retiringEntity The entity that has retired BCO2 and is eligible to mint an NFT.
    /// @param retiringEntityString An identifiable string for the retiring entity, eg. their name.
    /// @param beneficiary The beneficiary address for whom the BCO2 amount was retired.
    ...
    /// @param retirementEventIds An array of event ids to associate with the NFT. Currently
    /// only 1 event is allowed to be provided here.
    /// @dev    The function can either be called by a valid BCO2 contract or by someone who
    ...
    function mintCertificate(
        ...
        // ensure the caller is at least a BCO2 contract. This is to
        // allow BCO2 contracts to call retireAndMintCertificate.
        require(retiringEntity == msg.sender || IBuckMintContractRegistry(contractRegistry).checkERC20(msg.sender) == true, 'Invalid caller');
    ...
    /// as they are read in BCO2 contracts.
...
abstract contract RetirementCertificatesStorage is RetirementCertificatesStorageV1 {}
