 ...
 */
contract EternityCollection is ICollection, AccessControl, Ownable {
  /**
...
/**
   ...
   */
 function depositForFreeMint(uint8 _mintCount) public payable {
    ...
    require(msg.value  >= _mintCount * cost, "not enough funds are sent");
...
function mintCards (uint256 _amount, bytes32 revealHash) internal {
   uint256 max = 4294967296;
    ...
    for(uint i = 0; i < _amount; i++) {
      ...
      //uint256 randomNumberOLD = uint256(rng(4294967296, i, blockHash));
...
function upgradeCardCost(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId, Coupon memory _coupon, uint _discount) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) {
    ...
    for(uint16 i = 0; i < _tokenIds.length; i++) {
      ...
      if(totalValueGivenCards > targetCardTypeValue ){
        ...
        if(_collectionId == collectionId){
          ...
          // 15482  --- 6 of Diamonds
          upgradecost.feeValue = buyBackValue * totalValueGivenCards * upgradeFee / 100_000 / 2;
          ...
          upgradecost.feeValue = (buyBackValue * targetCardTypeValue) * upgradeFee / 100_000 / 2;
          upgradecost.upgradeFeeCost = upgradecost.feeValue / 10**8;
    ...
    if(upgradecost.upgradeFeeCostWithDiscount > cardsTypeValueDiffCost){
      upgradecost.upgradeFeeCostWithDiscount -= cardsTypeValueDiffCost;
      cardsTypeValueDiffCost = 0;
    }else{
      cardsTypeValueDiffCost -= upgradecost.upgradeFeeCostWithDiscount;
      upgradecost.upgradeFeeCostWithDiscount = 0;
    }
...
/**
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount) public payable  validCardType(_targetCardType) {
    ...
    (uint256 upgradeFeeCost, uint256 partnerFeeCost, uint256 cardsTypeValueDiffCost) = upgradeCardCost(_tokenIds, _targetCardType, _collectionId, _coupon, _discount);
    {
    ...
    
    {
      ...
      if(partnerFeeCost > 0){
        ...
        sendUpgradeFeeToPartner(upgradeFeeCost, partnerFeeCost);
      }
    ...
    card.mint(msg.sender, targetTokenId, _targetCardType, _collectionId, true);
      //Emit event
    emit Upgrade(
      msg.sender,
      collectionId,
...
function sendUpgradeFeeToPartner(uint256 upgradeFeeCost, uint256 partnerFeeCost) private {
    if(msg.value - upgradeFeeCost < partnerFeeCost){
...
function claimUpgradeCommission(uint256 cardsTypeValueDiffCost) private {
      ...
      if(commissioninfo.upgradeCommission > 0){
        ...
        if(commissioninfo.upgradeCommission > 0){
          gametreasury.giveUpgradeCacheBack(msg.sender, commissioninfo.upgradeCommission);
        }


        commissioninfo.upgradeCommission = cardsTypeValueDiffCost - commissioninfo.upgradeCommissionPrtner;
        if(gametreasury.balance() < commissioninfo.upgradeCommission){
            commissioninfo.upgradeCommissionFromTreasuryReserve = commissioninfo.upgradeCommission - gametreasury.balance();
