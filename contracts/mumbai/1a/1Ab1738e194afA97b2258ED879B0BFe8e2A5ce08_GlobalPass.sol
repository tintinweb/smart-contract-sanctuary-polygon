...
import "./GlobalPassAuctions.sol";
import "./GlobalPassRoyalties.sol";
import "./GlobalPassSelling.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
contract GlobalPass is ERC1155Burnable, GlobalPassAuction, ReentrancyGuard {
   ...
   // mapping(uint => uint[]) eventticketTypeDetails;
    ...
    // tickets will be created on time of event registration
     function registerEvent(
        ...
        uint _organiserId) external whenNotPaused isOrganiserOrOwner  {
        eventsCount++;
       ...
       // require(eventExists[_eventID] == false, "Event with same ID Already Registered");
        ...
        addTicketType(  newEventID, _TicketTypes /*_auctionStartTimes, _auctionEndTimes */) ;
     ...
     function updateEvent(uint16 _eventID,
        uint32 _eventStartDate,
        uint32 _eventEndDate,
    ...
    //Delete Event 
    function deleteEvent(uint64 _eventID) external onlyOwner eventIsRegistered(_eventID) whenNotPaused{
       ...
       function addTicketType(
        uint64 eventID,
        TicketType[] memory _ticketTypes
    ) public whenNotPaused isOrganiserOrOwner  eventIsRegistered(eventID) {
        for(uint8 i=0; i < _ticketTypes.length ; i++)
         {
             ticketsCount++;
            require(ticketExists[ticketsCount] == false, "Ticket with same ID Already Exists");
            for(uint j=0; j<_ticketTypes[i].validDates.length; j++){
                require((eventDetails[eventID].eventStartDate <= _ticketTypes[i].validDates[j]) && (eventDetails[eventID].eventEndDate >= _ticketTypes[i].validDates[j]), "Validity dates must be within Event Dates");
            }
            ...
            ticketTypeDetails[ticketsCount].eventIDforTicket = eventID;
                ...
                eventDetails[eventID].eventTickets.push(ticketTypeDetails[ticketsCount]);
    ...
    ) external whenNotPaused onlyOwner  TicketExists(_ticketID) eventIsRegistered(_eventID){
        require(_ticket.totalCopies >= ticketTypeDetails[_ticketID].copiesSold, "These no of copies have already been sold");
    ...
    function getAllPackages() public view returns(ticketPackage[] memory){
        ticketPackage[] memory packagesList = new ticketPackage[](packagesCount);
        for(uint i = 0; i< packagesCount; i++){
            if(eventDetails[ticketsPackage[i].ticketIDs[0]].eventEndDate > block.timestamp)
    ...
    function placeTicketForFixedPrice(address owner, uint ticketID,  uint _ticketFixPrice) internal  isOrganiserOrOwner {
    ...
    function setBaseURI(string memory newuri) public isOrganiserOrOwner {
        _setURI(newuri);
    }
...
import "@openzeppelin/contracts/access/Ownable.sol";
 abstract contract GlobalPassAuction is Ownable, ERC1155,Pausable, GlobalPassRoyalties{  //is SpaceERC20 
    event availableForAuction (uint, string) ;
    event removeFormSale (uint, string );
    event packageCreated(uint packageID, ticketPackage);
    ...
    //To check who has bidden on a Package
    //PackageID=>BidderAddress=>true/false
    mapping(uint=>mapping(address => bool)) hasBidden;

     // PackageId to PackageDetails (Struct) 
    mapping(uint => ticketPackage) ticketsPackage;
   ...
   mapping(address=> mapping(uint => uint)) public CopiesOnBidOrSale;
    ...
    modifier eventIsNotPaused(uint64 _eventID){
        ...
        modifier isOrganiserOrOwner(){
        require(isOrganiser[_msgSender()] || (_msgSender() == owner()), "You are not owner or Organiser");
        _;
    }
    ...
    //To check if even already exists or not
    modifier TicketExists(uint ticketID){
        require(ticketExists[ticketID] == true, "Ticket Does not exist");
        _;
    }
     modifier PackageExists ( uint packageID){
        require(ticketsPackage[packageID].Exists == true , "Not Owner of Package or Package Does't Exist ");
        _;
    }

    modifier notOnSale (address owner,uint packageID) {
        require(ticketsPackage[packageID].salestatus == saleTypeChoice.NotOnSale, "Error! Ticket is Already on Sale");
        _;
    }

    modifier onFixedPrice (address owner, uint packageID){
        require( ticketsPackage[packageID].salestatus == saleTypeChoice.OnfixedPrice, "Ticket is Not Available for Fixed Price");
        _;
    }
    modifier onAuction (address owner, uint packageID){
        require( ticketsPackage[packageID].salestatus == saleTypeChoice.onAuction, "Ticket is Not Available for Auctions");
        _;
    }
//    
 ...
 function isAdmin() public view returns (bool){
    ...
    function setOrganiser(address _organiser) public onlyOwner whenNotPaused{
        isOrganiser[_organiser] = true;
    }
    function removeOrganiser(address _organiser) public onlyOwner whenNotPaused{
         isOrganiser[_organiser] = false;
    }

//     //Place NFT to Accept Bids
    // function _placePackageForBids(address _owner, uint packageID ) notOnSale(_owner,packageID) NftExist(_owner , packageID) internal {
    ...
    //Test this function
    function getPackageDetails ( uint packageID) public view returns (ticketPackage memory Package, address[] memory bidderAddresses, uint[] memory bidAmounts){
       address[] memory bidderAddressesArr =  ticketsPackage[packageID].bidderAddress;
       uint[] memory bidAmountsArr = ticketsPackage[packageID].bidAmount;
       
        return (ticketsPackage[packageID],bidderAddressesArr,bidAmountsArr);
    }
    
    function _placePackageForFixedPrice( uint packageID, uint16[] memory _ticketIDs , uint[] memory ticketCopiesCount, uint packageFixPrice) internal {
        CurrentStatus = saleTypeChoice(2);
 ...
 function buyPackageForFixedPrice( address to, uint packageID, uint _eventID) external payable  PackageExists(packageID) {
         ...
         for(uint i = 0; i<ticketsPackage[packageID].ticketIDs.length; i++)
        {
            ...
            percentage += ticketTypeDetails[ticketsPackage[packageID].ticketIDs[i]].royaltyPercentage + ticketTypeDetails[ticketsPackage[packageID].ticketIDs[i]].servicePercentage;
            Ticket[to][ticketsPackage[packageID].ticketIDs[i]].salestatus = CurrentStatus;
            this.safeTransferFrom(ticketsPackage[packageID].packageOwner, to, ticketsPackage[packageID].ticketIDs[i], ticketsPackage[packageID].numOfCopies[i], "");
            
        }
        _royaltyAndGlobalPassFee(ticketsPackage[packageID].minimumPrice, percentage, payable(contractOwner), payable(ticketsPackage[packageID].packageOwner) );
    ...
    // function placePackageForTimedAuction(uint _eventID, uint16[] memory _ticketIDs , uint[] memory ticketCopiesCount, uint32 _auctionStartTime, uint32 _auctionEndTime, uint packageMinPrice) public {
    ...
    //       require(Ticket[_msgSender()][_ticketIDs[i]].Exists, "Ticket not exist"); 
    //       require((super.balanceOf(msg.sender, _ticketIDs[i]) - CopiesOnBidOrSale[msg.sender][_ticketIDs[i]]) >= ticketCopiesCount[i], "Balance Problem");
    ...
    //    }
    //    packagesCount++;
    //     uint newPackageID = packagesCount;
       
    //     _placePackageForTimedAuction( newPackageID, _ticketIDs , ticketCopiesCount, _auctionStartTime, _auctionEndTime, packageMinPrice);
    // }

    function placePackageForFixedPrice(uint _eventID, uint16[] memory _ticketIDs , uint[] memory ticketCopiesCount, uint packageFixPrice) external {
        for(uint i = 0; i<_ticketIDs.length; i++)
       {
          // require(Ticket[owner][_ticketIDs[i]].Exists && (balanceOf(msg.sender, _ticketIDs[i]) - CopiesOnBidOrSale[msg.sender][_ticketIDs[i]]) <= ticketCopiesCount[i] &&   _eventID ==  ticketTypeDetails[_ticketIDs[i]].eventIDforTicket, "Error: Cannot Place package for Fixed Price");
          require(Ticket[_msgSender()][_ticketIDs[i]].Exists, "Ticket not exist"); 
       ...
       packagesCount++;
        uint newPackageID = packagesCount;
       _placePackageForFixedPrice( newPackageID,  _ticketIDs , ticketCopiesCount, packageFixPrice);

    }


    // function _placePackageForTimedAuction( uint packageID, uint16[] memory _ticketIDs , uint[] memory ticketCopiesCount, uint32 _auctionStartTime, uint32 _auctionEndTime, uint packageMinPrice) internal {
    ...
    //     ticketsPackage[packageID].Exists = true;
    //     emit packageCreated( ticketsPackage[packageID]);
    //     emit availableForAuction (packageID, " Accepting Bids");
    // }

        

     function _removeFromSale( uint packageID) PackageExists(packageID) internal {
    ...
    //      require(ticketsPackage[packageID].Exists, "You are not owner of the Package");
    //     _getIndexOfHighestBid( packageID);
    //     _deductBiddingAmount(ticketsPackage[packageID].bidAmount[ticketsPackage[packageID].index], ticketsPackage[packageID].bidderAddress[ticketsPackage[packageID].index]);   // Deduct Bidder Amount of Bidding 
    //     CurrentStatus = saleTypeChoice(3);
    //      for(uint i = 0; i<ticketsPackage[packageID].ticketIDs.length; i++)
    //     {
    //         require((balanceOf(owner, ticketsPackage[packageID].ticketIDs[i]) >= CopiesOnBidOrSale[owner][ticketsPackage[packageID].ticketIDs[i]]) &&   (_eventID ==  ticketTypeDetails[ticketsPackage[packageID].ticketIDs[i]].eventIDforTicket), "Error: Cannot Purchase package for Fixed Price");
    //         CopiesOnBidOrSale[owner][ticketsPackage[packageID].ticketIDs[i]] -= ticketsPackage[packageID].numOfCopies[i];
    //         ticketsPackage[packageID].Exists = false;
    //         Ticket[ ticketsPackage[packageID].bidderAddress[ticketsPackage[packageID].index]][ticketsPackage[packageID].ticketIDs[i]].Exists = true;
    //         Ticket[ ticketsPackage[packageID].bidderAddress[ticketsPackage[packageID].index]][ticketsPackage[packageID].ticketIDs[i]].salestatus = CurrentStatus;
    //         this.safeTransferFrom(owner,  ticketsPackage[packageID].bidderAddress[ticketsPackage[packageID].index], ticketsPackage[packageID].ticketIDs[i], ticketsPackage[packageID].numOfCopies[i], "");
    //     }   
    ...
    //     require(block.timestamp <= ticketsPackage[packageID].auctionEndTime, "Time is Overed");
    ...
    //    hasBidden[packageID][msg.sender]=true;
    //     _getIndexOfHighestBid(packageID);
    // }

    // function _getIndexOfHighestBid( uint packageID) internal returns (uint){
    //     uint temp = 0;
    //     for (uint i=0; i<ticketsPackage[packageID].bidAmount.length; i++){
    ...
    // }
    // function _releaseBiddingValue( uint packageID) internal {
    //     for (uint i=0; i<ticketsPackage[packageID].bidderAddress.length; i++){
    //         BidsAmount[ticketsPackage[packageID].bidderAddress[i]] -= ticketsPackage[packageID].bidAmount[i];
    //     }
    // }

    //  function _bidAccepted( uint packageID) internal {
    //     for(uint i=0;i<ticketsPackage[packageID].bidAmount.length;i++)
    //         {
    //            hasBidden[packageID][ticketsPackage[packageID].bidderAddress[i]]=false;
    //         }
    //     delete ticketsPackage[packageID].bidAmount;
    //     delete ticketsPackage[packageID].bidderAddress;
    // }
    function CheckSaleType( uint packageID) view public returns(saleTypeChoice){
        return ticketsPackage[packageID].salestatus;
    }
    //   function addAuctionBid(address owner, uint packageID, uint _bidAmount) public payable  PackageExists(packageID) {
    //     if(ticketsPackage[packageID].bidAmount.length != 0)
    //         require(_bidAmount >= (ticketsPackage[packageID].bidAmount[ticketsPackage[packageID].index] + (ticketsPackage[packageID].bidAmount[ticketsPackage[packageID].index]*10)/100), "Bid Amount Must be greater than 10% of current Highest Bid");
    ...
    // }
    //     function _removeNftFromSale( uint packageID) internal {
    //     ticketsPackage[packageID].Exists = false;
