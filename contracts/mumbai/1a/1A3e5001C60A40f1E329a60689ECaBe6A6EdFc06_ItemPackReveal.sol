...
contract ItemPackReveal is IItemPackReveal, GameBase, Random {
    ...
    function drawLots(uint256 worldId, uint256 tokenId, uint256 itemPackDefinitionId) public view returns(uint256[] memory, int64[] memory) {
        ...
        for (uint256 i; i < slotLength; i++) {
            ...
            uint256 index = indexByRandom(weights);
...
contract Random is Ownable {
    ...
    function indexByRandom(uint256[] memory weights) public view returns(uint256) {
        uint256 max;
        ...
        uint256 randomResult = random(max);
    ...
    function random(uint256 max) public view returns(uint256) {
        return uint256(keccak256(abi.encodePacked(_seed, blockhash(block.number)))) % max;
    }
}
