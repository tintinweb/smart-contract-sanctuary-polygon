...
contract PariFIDEX is Ownable, Trader {
    ...
    function withdraw(uint8 type_,address tokenaddr,uint256 amount) dexstatuscheck public returns(bool) {
        ...
        if(type_==0){
            tokenaddr = address(0);
            if(msg.sender != owner()) withdrawFee = tokendetails[address(0)].withdrawFee;
            require((amount > 0) && (amount <= (userDetails[msg.sender]._balance[address(0)].mul(tokendetails[address(0)].decimals).mul(1e18)).div(1e18).div(1e18)) && (withdrawFee < amount),"insufficent amount or amount > 0");
            require(amount <= address(this).balance, "insufficent polygon balance");
            
            payable(msg.sender).transfer(amount.sub(withdrawFee)); 
        }
        else{
            if(msg.sender != owner()) withdrawFee = tokendetails[tokenaddr].withdrawFee;
           
            require(tokenaddr != address(0) && tokendetails[tokenaddr].status==1, "token address is not registered"); 
            require((amount > 0) && (amount <= ((userDetails[msg.sender]._balance[tokenaddr].mul(tokendetails[tokenaddr].decimals).mul(1e18)).div(1e18).div(1e18))) && (withdrawFee < amount), "insufficient balance or amount > 0");
            IERC20(tokenaddr).transfer(msg.sender, (amount.sub(withdrawFee)));  
        }
        ...
        if(msg.sender != owner()) userDetails[owner()]._balance[tokenaddr] = userDetails[owner()]._balance[tokenaddr].add(withdrawFee.mul(1e18).div(tokendetails[tokenaddr].decimals)); 
    ...
    function makeOrder(uint[8] memory makerOrder, uint[8] memory takerOrder,address[] memory traderAddress, bytes32[2] memory msgHash,uint8[2] memory  v,bytes32[4] memory rs) onlyTrader public returns(bool){
        ...
        require(makerOrder[0] != takerOrder[0],"orderid is same");
        ...
        if(makerOrder[7]==1 ){
            ...
            if(traderAddress[0] == feeTokenAddress){
                ...
                userDetails[owner()]._balance[feeTokenAddress] = userDetails[owner()]._balance[feeTokenAddress].add(makerOrder[6]);
            }
            // If maker is buyer and token buy is fee token or maker is seller and receiving token is fee token.
                ...
                // trade amount < feeAmount
                    ...
                    userDetails[owner()]._balance[feeTokenAddress] = userDetails[owner()]._balance[feeTokenAddress].add(makerOrder[5]);
                    makerOrder[5] = 0;
                }
            }
            else{
        ...
        if(takerOrder[7]==1){
            ...
            if(traderAddress[2] == feeTokenAddress){
                ...
                userDetails[owner()]._balance[feeTokenAddress] = userDetails[owner()]._balance[feeTokenAddress].add(takerOrder[6]);
            }
            // If taker is buyer and token buy is fee token or taker is seller and receiving token is fee token.
                ...
                if(takerOrder[5]>=takerOrder[6]){
                    ...
                    userDetails[owner()]._balance[feeTokenAddress] = userDetails[owner()]._balance[feeTokenAddress].add(takerOrder[5]);
                    takerOrder[5]=0;
                }
            }
    ...
    function getUserBalance( address user_, address token) public view returns (uint) {
        return userDetails[user_]._balance[token];
    }
}