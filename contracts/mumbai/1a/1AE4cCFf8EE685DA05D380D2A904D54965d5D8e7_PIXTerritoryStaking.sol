...
contract PIXTerritoryStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, IPIXTerritoryStaking {
    ...
    function _rewardPerTokenFromLastUpdate(RewardPool storage rewardPool) internal view returns(uint256) {
        ...
        uint256 untilTime = (block.timestamp < currentEpochEndTimestamp) ? block.timestamp : currentEpochEndTimestamp;
    ...
    function _updateRewards(RewardPool storage rewardPool) internal {
        rewardPool.totalPoolRewardPerToken += _rewardPerTokenFromLastUpdate(rewardPool);
     ...
     */
    function stake(uint256 tokenId) public override nonReentrant whenNotPaused {
    ...
    function _deleteFromStakedNFTs(address user, uint256 tokenId) internal {
        ...
        for (uint256 i; i < len; i++) {
            if (stakedNFTs[user][i] == tokenId) {
                stakedNFTs[user][i] = stakedNFTs[user][len - 1];
                stakedNFTs[user].pop();
                return;
            }
        }
...
function unique(uint[] memory data, uint setSize) internal pure {
    ...
    function fixStakedNFTs(address user, uint256 setSize) external onlyMod {
        uint256 length = stakedNFTs[user].length;
        require(length > 0, "Staking: NOT_STAKER");
        for (uint256 i; i < length; i += 1) {
            if (stakers[stakedNFTs[user][i]] != user) _deleteFromStakedNFTs(user, stakedNFTs[user][i]);
        }

        for (uint i=0; i < length; i+=1) {
            for (uint j=i+1; j < length; j+=1) {
                if(stakedNFTs[user][i] == stakedNFTs[user][j]){
                   _deleteFromStakedNFTs(user, stakedNFTs[user][j]);
                   length -= 1;
                }
            }
    ...
    /**
     * @dev views claimable reward for the one token
    ...
    function earnedReward(uint256 tokenId) public override view returns(uint256) {
     ...
     /**
     * @dev views claimable reward for the list of tokens
     * @param tokenIds list of tokens to view rewards
     */
    function earnedBatch(uint256[] memory tokenIds) external override view returns (uint256[] memory) {
        uint256[] memory earneds = new uint256[](tokenIds.length);
        for (uint256 i; i < tokenIds.length; i += 1) {
            earneds[i] = earnedReward(tokenIds[i]);
        }
        return earneds;
    }

    /**
     * @dev views total claimable reward for account
    ...
    function earnedByAccount(address account) public override view returns (uint256) {
        ...
        for (uint256 i; i < stakedNFTs[account].length; i += 1) {
            if (stakers[stakedNFTs[account][i]] != account) continue;
            reward += earnedReward(stakedNFTs[account][i]);
        }
    ...
    function _claimAndUpdateToken(uint256 tokenId) internal returns(uint256) {
        ...
        uint256 reward = earnedReward(tokenId);
        tokenTotalRewardLastClaim[tokenId] = rewardPool.totalPoolRewardPerToken;
    ...
    function claimAll() external override {
        claimBatch(stakedNFTs[msg.sender]);
    }
