...
contract CredentialingContract is Ownable {
    ...
    */
    ...
    // type - Event
    event DataCredentialized(bytes32 indexed viewToken, string indexed ipfsUrl, uint indexed _status);

    // type - Mapping
    mapping(address => mapping(bytes32 => bool)) private storeCredentails;
    mapping (address => bool) private admins;
    mapping(bytes32 => CredentializeData) private allInfo;
    mapping(bytes32 => bool) private secretId;
    ...
    function credentialize(address _driverAddress, string memory _ipfsUrl) external onlyAdmin{
        ...
        bytes32 packedData = keccak256(abi.encodePacked(_driverAddress,_ipfsUrl));
        if(storeCredentails[_driverAddress][packedData]){
            revert SameDataAlreadyStored();
        }
        allInfo[packedData].driverAddress = _driverAddress;
        allInfo[packedData].ipfsURL = _ipfsUrl;
        allInfo[packedData].viewTokenId = packedData;
        storeCredentails[_driverAddress][packedData] = true;
        secretId[packedData] = true;
        emit DataCredentialized(packedData, _ipfsUrl, CouponAccepted_Code);
    }

    /**
     * @notice returns the detailed struct of credentials
     * @param _packedData pass the bytes32 value from the event.
     */
    function viewCredentializedData(bytes32 _packedData) external view 
    returns(CredentializeData memory driverData){
        if(secretId[_packedData] == true){
            return  allInfo[_packedData];
        }else{
