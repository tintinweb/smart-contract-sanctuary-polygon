...
// File: interfaces/ISolarexRouter02.sol


pragma solidity >=0.6.2;


interface ISolarexRouter02 is ISolarexRouter01 {
 ...
 * Implemented and modified by SolarexSwap teams.
 */
contract SolarexZapV1 is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    // Interface for Wrapped BNB (WBNB)
    IWETH public WBNB;

    // SolarexRouter interface
    ISolarexRouter02 public solarexRouter;

    // Maximum integer (used for managing allowance)
    uint256 public constant MAX_INT = 2**256 - 1;

    // Minimum amount for a swap (derived from SolarexSwap)
    uint256 public constant MINIMUM_AMOUNT = 1000;

    // Maximum reverse zap ratio (100 --> 1%, 1000 --> 0.1%)
    uint256 public maxZapReverseRatio;

    // Address SolarexRouter
    address private solarexRouterAddress;
     ...
     * @param _solarexRouter: address of the SolarexRouter
     * @param _maxZapReverseRatio: maximum zap ratio
     */
    constructor(
        address _WBNBAddress,
        address _solarexRouter,
        uint256 _maxZapReverseRatio
    ) {
        WBNBAddress = _WBNBAddress;
        WBNB = IWETH(_WBNBAddress);
        solarexRouterAddress = _solarexRouter;
        solarexRouter = ISolarexRouter02(_solarexRouter);
        maxZapReverseRatio = _maxZapReverseRatio;
    }

    /*
     * @notice Zap BNB in a WBNB pool (e.g. WBNB/token)
     * @param _lpToken: LP token address (e.g. SOLAX/BNB)
     * @param _tokenAmountOutMin: minimum token amount (e.g. SOLAX) to receive in the intermediary swap (e.g. BNB --> SOLAX)
     */
     ...
     * @param _lpToken: LP token address (e.g. SOLAX/BUSD)
     * @param _tokenAmountOutMin: minimum token to receive (e.g. SOLAX) in the intermediary swap (e.g. BUSD --> SOLAX)
     */
    function zapInToken(
        address _tokenToZap,
     ...
     * @param _lpToken: LP token address (e.g. SOLAX/WBNB)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum amount to receive (in BNB/WBNB) in the intermediary swap (e.g. SOLAX --> BNB)
     */
    function zapOutBNB(
        address _lpToken,
     ...
     * @param _lpToken: LP token address (e.g. SOLAX/BUSD)
     * @param _tokenToReceive: one of the 2 tokens from the LP (e.g. SOLAX or BUSD)
     * @param _lpTokenAmount: amount of LP tokens to zap out
     * @param _tokenAmountOutMin: minimum token to receive (e.g. SOLAX) in the intermediary swap (e.g. BUSD --> SOLAX)
     */
    ...
    {
        address token0 = ISolarexPair(_lpToken).token0();
        address token1 = ISolarexPair(_lpToken).token1();

        require(_tokenToZap == token0 || _tokenToZap == token1, "Zap: Wrong tokens");

        // Convert to uint256 (from uint112)
        (uint256 reserveA, uint256 reserveB, ) = ISolarexPair(_lpToken).getReserves();

        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB);
            swapAmountOut = solarexRouter.getAmountOut(swapAmountIn, reserveA, reserveB);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA);
            swapAmountOut = solarexRouter.getAmountOut(swapAmountIn, reserveB, reserveA);
        }
    ...
    {
        require(
            _token0ToZap == ISolarexPair(_lpToken).token0() || _token0ToZap == ISolarexPair(_lpToken).token1(),
            "Zap: Wrong token0"
        );
        require(
            _token1ToZap == ISolarexPair(_lpToken).token0() || _token1ToZap == ISolarexPair(_lpToken).token1(),
            "Zap: Wrong token1"
        );
        ...
        if (_token0ToZap == ISolarexPair(_lpToken).token0()) {
        ...
        )
    {
        ...
        if (token1 == _tokenToReceive) {
            // sell token0
            uint256 tokenAmountIn = (_lpTokenAmount * reserveA) / ISolarexPair(_lpToken).totalSupply();
            ...
            // sell token1
            uint256 tokenAmountIn = (_lpTokenAmount * reserveB) / ISolarexPair(_lpToken).totalSupply();
    ...
    function _zapIn(
        ...
        uint256[] memory swapedAmounts = solarexRouter.swapExactTokensForTokens(
            swapAmountIn,
            _tokenAmountOutMin,
            path,
            address(this),
        ...
        (, , lpTokenReceived) = solarexRouter.addLiquidity(
            path[0],
            path[1],
    ...
    function _zapOut(
        ...
        (uint256 amount0, uint256 amount1) = ISolarexPair(_lpToken).burn(address(this));

        require(amount0 >= MINIMUM_AMOUNT, "SolarexRouter: INSUFFICIENT_A_AMOUNT");
        require(amount1 >= MINIMUM_AMOUNT, "SolarexRouter: INSUFFICIENT_B_AMOUNT");
        ...
        // Swap tokens
        solarexRouter.swapExactTokensForTokens(swapAmountIn, _tokenAmountOutMin, path, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address _token) private {
        if (IERC20(_token).allowance(address(this), solarexRouterAddress) < 1e24) {
            // Re-approve
            IERC20(_token).safeApprove(solarexRouterAddress, MAX_INT);
        }
    }
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = solarexRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1);
        uint256 denominator = solarexRouter.quote(
            halfToken0Amount,
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = solarexRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1);
            uint256 denominator = solarexRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = solarexRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0);
