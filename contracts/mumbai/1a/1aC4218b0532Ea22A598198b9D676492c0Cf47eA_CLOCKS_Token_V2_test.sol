...
contract CLOCKS_Token_V2_test is
    Initializable,
    ERC20Upgradeable,
    UUPSUpgradeable
{
   //=====Access_Management=====//
    ...
    event Upgraded_CLOCKS_Token_contract(address newImplementation);
    ...
    //=====Constructor_and_initialize=====//
    // Not needed in upgrades
    ...
    function _authorizeUpgrade(address newImplementation)
        ...
        onlyOwnerRole
    {
        emit Upgraded_CLOCKS_Token_contract(newImplementation);
    }

    //=====Functions V2=====//

    function mint(uint256 amount) external onlyOwnerRole {
        uint256 mintingAmount = amount + 2;
        
        require(
            balanceOf(treasury) + mintingAmount <= MAX_SUPPLY,
            "Would reach MAX_SUPPLY"
        );

        _mint(treasury, mintingAmount);

        emit MintedTokens(treasury, mintingAmount);
    }

    function isOwner(address anAddress) external view returns (bool) {
        return owner == anAddress;
    }
}
