...
contract BEscrowMeta is BaseRelayRecipient, Ownable {
  ...
  // withdrawal limits
  mapping(AccountType => uint256) public WithdrawableLimit;
  ...
  constructor(address _trustedForwarder) {
    ...
    WithdrawableLimit[AccountType.WITHDRAWABLE] = uint256(0);
    WithdrawableLimit[AccountType.EARNING] = uint256(100);
  }
  ...
  function setWithdrawableLimits(AccountType _accountType, uint256 _amount) external onlyOwner {
    WithdrawableLimit[_accountType] = _amount;
  }
  ...
  function getWhitelistedTokenAddresses(bytes32 symbol) external view returns(address) {
    return whitelistedTokens[symbol];
  }

  function getAccountBalance(address walletaddress, bytes32 symbol, AccountType _accountType) external view returns(uint256) {
  ...
  function depositTokens(
    ...
    require(whitelistedTokens[symbol] != address(0x0), "TOKEN_NOT_WHITELISTED");
    ERC20Permit(whitelistedTokens[symbol]).permit(
      _msgSender(),
      address(this),
      amount,
      deadline,
  ...
  function withdrawTokens(uint256 amount, bytes32 symbol, AccountType _accountType) external {
    if (_accountType == AccountType.EARNING) {
      require(amount >= WithdrawableLimit[AccountType.EARNING], "WITHDRAW_NOT_ALLOWED_LOW");
      require(earningBalances[_msgSender()][symbol] >= amount, "WITHDRAW_LOW_BALANCE");
      ...
      require(amount >= WithdrawableLimit[AccountType.WITHDRAWABLE], "WITHDRAW_NOT_ALLOWED_LOW");
      require(accountBalances[_msgSender()][symbol] >= amount, "WITHDRAW_LOW_BALANCE");
  ...
  function moveToWithdrawableAccount(address advertiser, uint256 amount, bytes32 symbol) external onlyOwner {
    require(campaignBalances[advertiser][symbol] >= amount, "INSUFFICIENT_CAMPAIGN_BALANCE");
