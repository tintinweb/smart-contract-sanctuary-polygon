...
contract rigelNFTLaunchPadClaim is Ownable, ERC1155Holder {
 ...
 constructor(
    ...
    function claimLaunchPad(uint256 _tokenId, uint256 _projectID) external {   
        ...
        (address[] memory pair, uint256[] memory userAmt) = getLaunchPadData(_msgSender());
        uint256 stAmt = userCurrentStateOfClaim[_msgSender()][_tokenId];    
        if (userAmt.length > 0) {
            for (uint256 i = 0; i <= userAmt.length; i++) {  

                if (userAmt[i] >= __pairs.investmentOnPairOne && pair[i] == _token.token1 && stAmt == 0) {                                    
                    if (_tokenId == 1 ) {
                        userCurrentStateOfClaim[_msgSender()][_tokenId] = __pairs.investmentOnPairOne;
                        claim(_tokenId);
                        break;
                    }                 
                } else if (userAmt[i] >= __pairs.investmentOnPairTwo && pair[i] == _token.token2) {                                    
                    ...
                    break;
                } else if (userAmt[i] >= __pairs.investmentOnPairThree && pair[i] == _token.token3) {                                    
                    uint256 cAmt = userCurrentStateOfClaim[_msgSender()][_tokenId - 1];
                    uint256 currentAmount = userAmt[i] - cAmt;
                    if (currentAmount >= __pairs.investmentOnPairThree) {
    ...
    /*
        0x8E62F88FD28F95cE0dD0557c8419215b5599CE64 : RGP
        0xbbafE1b52845FfFe0A41863241a4817D91f23828 : NFT
        0x8D70F16b4dCe46a3697DD5375aB6930cA4408F43 : FACTORY
        0xa8Eff94Bf25c11C0B0472abd5D6434821C8Ff58B : ROUTER
    */

    function getLaunchPadData(address _user) public view returns( address[] memory, uint256[] memory) {  
        uint256 allPair = launchPadFactoryContract(launchPadFactory).allPairsLength();          
        address[] memory pair = new address[](allPair);
        uint256[] memory amountInEachPair = new uint256[](allPair);
        for (uint256 i = 0; i <= pair.length - 1; i++) {
            pair[i] = launchPadFactoryContract(launchPadFactory).allPairs(i);
            uint256 lockedAmount = IrigelProtocolLanchPadPool(pair[i]).userFunds(_user);
            if (lockedAmount > 0) {
               amountInEachPair[i] = lockedAmount;
               pair[i] = pair[i];
            }
        }
        return (pair, amountInEachPair);    
    }


    function getNumbersOfProjInvestedIn(address _user) public view returns(uint256 _num) {      
        uint256 allPair = launchPadFactoryContract(launchPadFactory).allPairsLength();  
        address[] memory amountInEachPair = new address[](allPair);
        address[] memory amountInEachPair2 = new address[](allPair);
        
        for (uint256 i = 0; i <= amountInEachPair.length - 1; i++) {
            address pair = launchPadFactoryContract(launchPadFactory).allPairs(i);
            uint256 lockedAmount = IrigelProtocolLanchPadPool(pair).userFunds(_user);
            if (lockedAmount > 0) {
                ( , address swapTokenTo, ,,,,) = IrigelProtocolLanchPadPool(pair).viewData();
                if (i == 0) {
                    amountInEachPair2[i] = swapTokenTo;
                } else {
                    address proj = amountInEachPair2[i-1];
                    if (proj != amountInEachPair2[i]) {
                        amountInEachPair2[i] = swapTokenTo;
                    }
                }
            }
        }
        return amountInEachPair2.length; 
    }
