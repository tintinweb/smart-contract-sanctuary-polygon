...
contract NFTListing {
    ...
    mapping(address => uint256) public stakedNFTs; // Mapping to store staked NFTs
    ...
    function stakeNFT(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(listings[tokenId].active, "Listing does not exist");
            ...
            // Store the staking time
            listings[tokenId].active = false;
            uint256 stakingTime = block.timestamp + TimePeriod;
            listings[tokenId].tokenId = stakingTime;

            stakedNFTs[msg.sender]++; // Increment the staked NFT count for the user
    ...
    function unstakeNFT(uint256[] memory tokenIds) external {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(listings[tokenId].active, "Listing does not exist");
            require(
                block.timestamp >= listings[tokenId].tokenId + TimePeriod,
                "The staking period has not passed yet"
            );
            ...
            uint256 reward = stakedNFTs[msg.sender] * 1 ether; // 1 BNB per staked NFT
    ...
    function getOwnedTokenIds(
        ...
        for (uint256 i = 0; i < nextListingId; i++) {
            ...
            if (listing.owner == wallet && listing.active) {
