...
contract EthermonAdventureDataProxy {
...
contract EtheremonAdventureData is BasicAccessControl {
    ...
    event AddedRevenue(uint256 _amount, uint256 fee);
    ...
    function addExploreData(
        ...
        /**
            ...
            emon = 10e18
            perecent_to_us = 10
            total_sites_minted = 11
            (emon - ((emon * perecent_to_us) / 100))/ total_sites_minted
        */
        uint256 perecentAmount = (_emonAmount * 30).div(100);
        uint256 subAmount = _emonAmount.sub(perecentAmount);
        uint256 currentRevenue = subAmount.div(tokenIds.length);
        emonFee += perecentAmount;

        for (uint256 i = 0; i < tokenIds.length; i++)
            landToken[tokenIds[i]] += currentRevenue;
        emit AddedRevenue(currentRevenue, emonFee);
        return exploreCount;
    }

    function withdrawEmon() external onlyModerators {
        emon.safeTransfer(msg.sender, emonFee);
        emonFee = 0;
    }
        ...
        )
    {
        ...
        _perecentAmount = (_emonAmount * 30).div(100);
        _subAmount = _emonAmount.sub(_perecentAmount);
        _currentRevenue = _subAmount.div(tokenIds.length);
    }

    function getClassSize(
        uint256 _emonAmount,
        uint256 _siteId
    ) external view returns (uint256 _totalSites) {
        _totalSites = siteClassToken[_siteId].length;
    }
}