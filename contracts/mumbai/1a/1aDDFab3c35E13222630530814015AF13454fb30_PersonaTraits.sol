...
contract PersonaTraits is IPersona, Ownable {
    ...
    event NewTrait(uint16 indexed traitID);
    ...
    function addTrait(
        ...
        emit NewTrait(_traitID);
    }

    function getNFTRarity(
        uint16[] memory _traits
    ) external view returns (PersonaNFTRarity rarity) {
        uint8 point = 0;
    ...
    function buyTraitsFromStoreByCredit(
        ...
        for (uint256 i; i < _traits.length; i++) {
            totalPoints += traitPriceOnStore[_traits[i]] * _amounts[i];
        }
        require(
