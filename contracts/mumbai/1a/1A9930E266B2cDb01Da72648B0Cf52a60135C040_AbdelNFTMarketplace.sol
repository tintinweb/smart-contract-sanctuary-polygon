...
contract AbdelNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    function listItem(
        ...
        require(_nftAddress != address(0), "listItem: nft address is zero");
        ...
        if (_lazyMint) {
            if (ILazyMinting(_nftAddress).getMaxTokens(_tokenId) == 0) {
                ...
                require(
                    ILazyMinting(_nftAddress).getCreator(_tokenId) ==
                        _msgSender(),
        ...
        require(_nftAmount != 0, "listItem: cannot list zero numner of items");

        if (_saleKind == SaleKind.Auction) {
            require(
