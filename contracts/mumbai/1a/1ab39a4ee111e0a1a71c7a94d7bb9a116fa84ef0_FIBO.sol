...
contract FIBO is ERC20, Ownable {
    ...
    uint256 public numTokensForSell = 50000e18;
    ...
    /**
     * @dev Sets the number of tokens to sell
     * - MUST be called by the owner
     * - MUST check if you're going to update with the same value
     * - MUST emit the NumTokensToSellUpdated event
     */
    function setNumTokensForSell(uint256 _numTokensForSell) external onlyOwner {
        require(numTokensForSell != _numTokensForSell, "Oops!, trying to update with the same value");
        emit NumTokensToSellUpdated(numTokensForSell, _numTokensForSell);
        numTokensForSell = _numTokensForSell;
    }    
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (feeAmount > 0) {
            ...
            if (swapAmount >= numTokensForSell && exchanges[recipient]) {
                _takeFee(numTokensForSell);
            }
