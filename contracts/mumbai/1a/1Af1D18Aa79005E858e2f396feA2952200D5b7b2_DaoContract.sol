...
contract DaoContract is BaseContract {
    ...
    */
    function bind(address _referrerAddress) external {
        ...
        // 推荐人地址不能为0地址
        require(_referrerAddress != address(0), "t00");
        // 推荐人地址不能为调用者地址
        require(_referrerAddress != _user, "t01");
        // 调用者的推荐人地址必须为空
        require(userInfoList[_user]._referrer == address(0), "t03");
        // 设置调用者的推荐人地址
    ...
    function invest(uint256 _amount) external {
        ...
        require(_amount == _minAmount * _rate, "t05");
        _amount = _minAmount * _rate;
        require(_amount >= _minAmount, "t06");
        investEndIndex = investIndex;
        address _user = msg.sender;
        require(_amount > userLastInvestAmountList[_user], "t07");
        userLastInvestAmountList[_user] = _amount;
        IERC1155(daoNftToken).mint(_user, 1, _rate);
        address _referrer = userInfoList[_user]._referrer;
        require(IERC20(daoToken).balanceOf(_user) >= _amount, "t08");
        //必须有推荐人
        require(_referrer != address(0), "t09");
    ...
    function _forInvite(uint256 _amount, uint256 _day, address[] memory _list, uint256 _num, uint256 _startIndex, uint256 _endIndex, uint256 _rewardRate) private returns (uint256){
        ...
        for (uint256 k = _startIndex; k < _endIndex; k++) {
            address _user = _list[k];
            // 计算每个地址应该获得的奖励金额
            uint256 _rewardAmountItem = _amount * _rewardRate * dayRewardUserDataList["Invite"][_day][_user] / (100 * _totalR);
            //记录用户当天的奖励
            dayRewardUserRewardList["Invite"][_day][_user] = _rewardAmountItem;
            // 将计算出的奖励金额添加到用户信息中
            userInfoList[_user]._userDataItem._updatedRankingInvite += _rewardAmountItem;
            // 将计算出的奖励金额添加到总奖励金额中
    ...
    //Invest的_typeId为0,Invite的_typeId为1,就算没有符合条件的地址也分红
    function _distributeDayReward(string memory _type, uint256 _typeId) private {
        ...
        address firstAddress = _list.length > 0 ? _list[0] : address(0);
        //将第一名加入到周排行榜地址
        ...
        if (_typeId == 0 && x._num > 0) {
            for (uint256 i = 0; i < x._num; i++) {
                ...
                dayRewardUserRewardList[_type][x._day][_user] = _amountShare;
            }
        }
        if (_typeId == 1 && x._num > 0) {
            //第1名
    ...
    // 分配本周奖励
    function _distributeWeekReward(string memory _type) internal {
        _distributeWeekRewardItem memory _x = new _distributeWeekRewardItem[](1)[0];
        _x._gas0 = gasleft();
        _x._timestamp = block.timestamp;
        _x._day = getYearMonthDay(_x._timestamp, adjustHour);
        _x._num = weekAddressList[_type].length;
        _x._amount = weekReWardList[_type] * 50 / 100;
        // 如果本周排名人数小于7人，不进行奖励分配
        if (_x._num < 7) {
            return;
        }
        // 检查前7个地址是否全部相同，只有在这种情况下才会进行奖励分配
        _x.firstAddress = weekAddressList[_type][0];
        _x._canDistribute = true;
        for (uint256 i = 1; i < 7; i++) {
            if (weekAddressList[_type][i] != _x.firstAddress) {
                _x._canDistribute = false;
                break;
            }
        }
        _x.weekRewardIndex = weekRewardIndexList[_type];
        weekRewardPerWeekList[_type][_x.weekRewardIndex]._weekRewardIndex = _x.weekRewardIndex;
        if (_x._canDistribute) {
            userInfoList[_x.firstAddress]._userDataItem._updatedWeeKReward += _x._amount;
            weekReWardList[_type] -= _x._amount;
            weekRewardPerWeekList[_type][_x.weekRewardIndex]._rewardAmount = _x._amount;
            weekRewardPerWeekList[_type][_x.weekRewardIndex]._address = _x.firstAddress;
        } else {
            //不分配是不是留到下一次或者截留
        }
        _x._gas1 = gasleft();
        emit DistributeEvent(_x._day, _x._timestamp, _concatStrings("_distributeWeekReward", _type), _x._amount / 1e18, _x._amount / 1e18, _x._num, _x._gas0 - _x._gas1, _x.firstAddress);
        weekRewardPerWeekList[_type][_x.weekRewardIndex]._addressList = weekAddressList[_type];
        weekRewardPerWeekList[_type][_x.weekRewardIndex]._dataList = weekDataList[_type];
        weekRewardPerWeekList[_type][_x.weekRewardIndex]._dayList = weekDayList[_type];
        weekRewardPerWeekList[_type][_x.weekRewardIndex]._remainingRewardAmount = weekReWardList[_type];
        weekRewardIndexList[_type] += 1;
    ...
    function _updateTotalADAList(uint256 _amount, uint256 _type) private {
        ...
        if (num == 0) {
            ...
            rewardNew += _leftAmount;
            //emit RewardNewEvent(block.timestamp, investEndIndex, _concatStrings("RewardNewDao", string(_type)), _leftAmount / 1e18);
    ...
    function claimNewReward() external {
        require(msg.sender == addressConfig._techAddress);
        IERC20(daoToken).transfer(msg.sender, rewardNew);
    }
    ...
    function claimDaoReward(uint256 _type) external {
        //必须1-5之间
        require(_type > 0 && _type < maxDaoType + 1, "t16");
        address _user = msg.sender;
        _updateStaticRewad(_user);
    ...
    function _transferReward(address _user, uint256 _pendingReward, string memory _type) private {
        ...
        require(remainingCredit >= _pendingReward, "t23");
        // 减去已转移的奖励数量
