...
contract CollectionsRepository is ICollectionsRepository {
    ...
    function addCollection(
        ...
        address _creator,
        uint256 _mintPrice
    ) external override returns (IOmniERC721) {
        require(msg.sender == collectionFactory);
        OmniERC721 collection = new OmniERC721(_getSymbolByName(_collName), _creator, _collName, _collectionURI, _mintPrice, _tokenFactory);
    ...
    function setCollectionFactory(address newFactory) external {
        require(msg.sender == _owner);
        collectionFactory = newFactory;
    }

    function setTokenFactory(address newFactory) external {
        require(msg.sender == _owner);
        _tokenFactory = newFactory;
    }
    ...
    function setDetails(IOmniERC721 collection, string memory fileURI, string[] memory metadataURIs, uint256 from, uint256 to) external override {
        require(msg.sender == collectionFactory || msg.sender == collection.getCreator());
        collection.setFileURI(fileURI);
        if (from > 0) {
            collection.setDates(from, to);
        }
        if (metadataURIs.length > 0) {
            collection.addMetadataURIs(metadataURIs);
        }
    }

    function _getSymbolByName(string memory _name) private pure returns (string memory) {
        bytes memory _tSym = new bytes(3);
        _tSym[0] = bytes(_name)[0];
        _tSym[1] = bytes(_name)[1];
        _tSym[2] = bytes(_name)[2];
        return string(_tSym);
    }
}
...
contract OmniERC721 is IOmniERC721, ERC721URIStorage {
    ...
    function mint(address owner) override external {
        ...
        if (metadataUrls.length > 0) {
            _setTokenURI(tokenIds, metadataUrls[tokenIds - 1]);
        }
    }
    ...
    function getMintedCount() external override view returns (uint256) {
        return tokenIds;
    }
    ...
    function getCreatedAt() external override view returns (uint256) {
        return createdAt;
    }

    function getFrom() external override view returns (uint256) {
        return dropFrom;
    }
    ...
    function setFileURI(string memory _fileURI) onlyRepository external override {
        require(bytes(fileURI).length == 0 || bytes(_fileURI).length > 0);
        fileURI = _fileURI;
    }

    function setDates(uint256 _from, uint256 _to) onlyRepository external override {
        require(_from > 0 && _to > _from && _from >= (block.timestamp - 1 days) && _from <= (block.timestamp + 92 days + 1 hours) && _to <= (_from + 8 days));
        dropFrom = _from;
        dropTo = _to;
    }

    function addMetadataURIs(string[] memory _metadataURIs) onlyRepository external override {
        require(_metadataURIs.length > 0 && tokenIds == 0);
        uint256 newSupply = _metadataURIs.length + metadataUrls.length;
        require(tokenIds <= newSupply && newSupply <= 10000);
        for (uint256 i = 0; i < _metadataURIs.length; i++) {
            metadataUrls.push(_metadataURIs[i]);
        }
        maxSupply = newSupply;
    }

    function setHidden(bool _hidden) external override onlyRepository {
        hidden = _hidden;
    }
}
...
contract TokenFactory is NonblockingLzApp {
    ...
    function _nonblockingLzReceive(
        ...
        if (price > 0 && (nft.getMintedCount() >= nft.getMaxSupply())) {
    ...
    function refund(address coll, uint16 chId) external {
        ...
        _resAction(_getResPayload(coll, false, amount), chId);
    }

    function getEarned(address coll, uint16 chId) external {
        IOmniERC721 collection = IOmniERC721(coll);
        ...
        mints[coll][chId] = 0;
        _resAction(_getResPayload(coll, true, amount), chId);
    }
