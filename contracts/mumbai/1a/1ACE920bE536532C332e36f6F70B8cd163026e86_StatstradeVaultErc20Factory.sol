...
contract StatstradeVaultErc20Factory {
  ...
  function site__vault_create(string memory vault_id,string memory name,address owner,uint16 vault_tax_rate,uint vault_tax_max,uint vault_withdraw_min,uint vault_withdraw_max,address vault_erc20) external {
    ut__assert_admin(msg.sender);
    require(
  ...
  function site__account_close(string memory vault_id,address user_address) external {
    ...
    require(
      (account.status == VaultAccountStatus.Active) || (account.status == VaultAccountStatus.Requested),
  ...
  function ut__withdraw_transfer(string memory vault_id,address user_address,string memory withdraw_id,WithdrawRequest memory request,bool arbitrated) internal {
    ...
    IERC20 erc20 = IERC20(g__VaultLookup[vault_id].vault_erc20);
    if(arbitrated){
      erc20.transfer(g__SiteAuthority,vault_tax);
    }
    else{
      erc20.transfer(g__VaultLookup[vault_id].owner,vault_tax);
    }
    erc20.transfer(user_address,request.amount - vault_tax);
  ...
  function user__account_deposit(string memory vault_id,uint amount) external {
    VaultAccount memory account = ut__get_account(vault_id,msg.sender);
    require(
      account.status == VaultAccountStatus.Active,
      "Account not active."
    );
    g__VaultLookup[vault_id].total_pool += amount;
    IERC20 erc20 = IERC20(g__VaultLookup[vault_id].vault_erc20);
    erc20.transferFrom(msg.sender,address(this),amount);
    emit VaultEvent("account_deposit",vault_id,msg.sender,amount);
  }
}