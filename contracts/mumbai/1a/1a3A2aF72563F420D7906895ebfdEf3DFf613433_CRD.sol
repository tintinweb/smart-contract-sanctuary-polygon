...
// Copyright 2023, Creator Networks Inc, All rights reserved.
...
/**
@title A Channel Royalty Distribution Contract (CRDC) to help securitize royalty payments from a media platform where
    each investor holds a Channel Royalty Token (CRT) which provides the features described below.
@author Jason Aubrey (GigaStar)
@notice Copyright 2023, Creator Networks Inc, All rights reserved.
...
@notice Terms: When an investor purchases a CRT they become the owner of that token; however, owning the token does not
    entitle them to any ownership in the underlying channel. The owner of a CRT is entitled to channel royalties paid by
    the media platform. The details of revenue distribution are outlined in the terms of the investment.
@notice Terms: To distinguish between 1) channel royalties paid by media platforms and 2) CRT royalties paid per trade,
    (1) may be referred to as 'revenue share' or 'dividends' ('divs') and (2) may be referred to as 'trade royalties'.
@notice Terms: The term 'dividend' is not meant to imply any ownership in the media channel that generates revenue.
@dev Features provided by this contract:
* Security Token: FINRA compliant where token owners are entitled to a % of revenue share paid as dividends
* Token reassignment: Per FINRA, reassign a token for an investor with a bad wallet (e.g. lost, stolen, inaccessible)
...
* NFTs: Every token uses ERC721 which provides a unique tokenId and metadata URI
* Dividends: Paid by the paying agent in phases: add to holding account, request allocation, allocate divs, push divs
...
* Unclaimed Dividend and CRT Custody: Divs and tokens remain in the holding account until claimed to the token owner
    ...
    Dividend
{
    ...
    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[42] private __gap; // The sum of all storage slots used (except this member) should be 50
    ...
    @param uriBase Location of token metadata to be appended with a tokenId as `${uriBase}${tokenId}`
    ...
    @param div Contract controls the currency for dividend distributions, see {Dividend}
    @param holding Account used as a holding area for 2 purposes: A) Dividend tokens before pushing to CRT owners,
    ...
    */
        ...
        if(!isMint) {
            ...
            if(isTokenFix) {
                ...
                if(to != _holding) // Not an unclaim
                    _pushDividend(tokenId, to); // isBurn would fail to transfer
            } else if(!isSameUser) {
                if(isClaim) {
                    _pushDividend(tokenId, to);
                } else {
                    if(isTrade) {
                    ...
                    _pushDividend(tokenId, from); // Claim divs before ownership change
                }
            }
            if(!isBurn)
    ...
    function setUriBase(string memory uriBase) public override onlyOwner {
        _checkUriBase(uriBase);
        string memory from = _uriBase;
        _uriBase = uriBase;
        emit BaseURIUpdate(from, uriBase, msg.sender);
    }
    ...
    // Dividends step 1: Enqueue a request to allocate dividends. The request allows paging vs doing it directly.
    function requestDivAllocation(uint256 divAmountIn, uint256 pageSize)
        external override onlyManager nonReentrant returns(bool)
    {
        _requestDivAllocation(divAmountIn);
        return pageSize == 0 ? true : _processAllocationRequest(pageSize);
    }

    // Dividends step 2: Process queued requests to allocate dividends. Caller should page requests while returns true
    function processAllocationRequest(uint256 pageSize) external onlyManager returns(bool) {
        return _processAllocationRequest(pageSize);
    }

    // Dividends step 3: Push allocated dividends to each token owner (if claimed)
    function pushDividends(uint256 tierId, uint256 tokenIndexBegin, uint256 tokenIndexEnd)
        external override onlyManager nonReentrant returns(uint256)
    {
        return _pushDividends(tierId, tokenIndexBegin, tokenIndexEnd);
    }
    ...
    function _destroyToken(uint256 tokenId) internal {
        ...
        uint256 dividend = _dividends[tokenId];
        if(dividend != 0)
            emit DividendForfeited(tokenId, token.owner, dividend, _msgSender());
        delete _dividends[tokenId];
        // slither-disable-next-line unused-return
        EnumerableSetUpgradeable.remove(_buyBacks, tokenId);
    ...
    // FINRA requires an ability to reassign a token for an investor with a bad wallet (e.g. lost, stolen, inaccessible)
...
import './utils/IDividend.sol';
import './utils/IRoyalty.sol';

interface ICRD is IAccess, ITier, IToken, IRoyalty, IDividend {
...
contract Access is IAccess, AccessControlEnumerableUpgradeable {
    ...
    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[14] private __gap; // The sum of all storage slots used (except this member) should be 15
...
/**
@title A CRD utility for dividend features
@author Jason Aubrey (GigaStar)
@notice Copyright 2023, Creator Networks Inc, All rights reserved.
@notice See terms in {CRD} contract comments.
...
*/
abstract contract Dividend is IDividend, Initializable {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    // UPGRADABILITY: https://docs.openzeppelin.com/upgrades-plugins/1.x/writing-upgradeable#modifying-your-contracts
    IERC20Upgradeable internal _div; // payment currency/token for dividends
    mapping(uint256 => uint256) internal _dividends; // key: tokenId, value: balance allocated for transfer
    uint256 internal _divTotalAllocated; // sum of all dividends allocated to tokens
    uint256 internal _divTotalTransferred; // sum of all dividends transferred to token owner wallets
    uint256 internal _divSlip; // slippage: sum of all dividend allocation remainders
    DivAllocationRequest internal _divAllocReq;
    // UPGRADABILITY: All new variables should be immediately above this line and decrement the gap array

    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[24] private __gap; // The sum of all storage slots used (except this member) should be 30
    ...
    /// @dev Upgradeable contracts have a 2 part initialization: constructor(empty) + init(args), for more info see:
    ///  https://docs.openzeppelin.com/contracts/4.x/upgradeable#usage
    function __Dividend_init(IERC20Upgradeable div) internal onlyInitializing {
        _div = div;
    }
    ...
    function _dividendUnclaimed() internal view returns(uint256) {
    ...
    // Dividends step 1: Enqueue a request to allocate dividends. The request allows paging vs doing it directly.
    function _requestDivAllocation(uint256 divAmountIn) internal {
        ...
        // Ensure holding account has a balance to pay current obligations and this new dividend (funds received)
        uint256 pendingUnclaimed = _dividendUnclaimed() + _divAllocReq.divAmountIn + _divAllocReq.nextDivAmount;
        address holding = _getHolding();
        _checkLessEqual(Tag.insufficientBalance, pendingUnclaimed, _div.balanceOf(holding));

        // Ensure the holding account approved sufficient allowance to this contract to transfer the dividend
        _checkLessEqual(Tag.insufficientAllowance, pendingUnclaimed, _div.allowance(holding, address(this)));
    }

    // Dividends step 2: Process requests to allocate dividends. Caller should page requests while returns true
    function _processAllocationRequest(uint256 pageSize) internal returns(bool) {
        _checkNonZero(Tag.pageSize, pageSize);
        ...
        for(uint256 i = 0; i < tierCount * MAX_DIV_AMOUNTS; ++i) {
            ...
            unchecked {
                while(nextTokenIndex < end) { // See TOKEN_COUNT, caller must page
                    ...
                    _dividends[tokenId] += divPerToken;
    ...
    // Dividends step 3: Push allocated dividends to each token owner (if claimed)
    function _pushDividends(uint256 tierId, uint256 tokenIndexBegin, uint256 tokenIndexEnd) internal returns(uint256) {
        if(_dividendUnclaimed() == 0) return 0;
    ...
    function _pushDividend(uint256 tokenId, address tokenOwner) internal virtual returns(uint256) {
        _pushDiv(tokenId, tokenOwner);
        return 0; // facilitates testing, could be removed
    }

    function _pushDiv(uint256 tokenId, address tokenOwner) private {
        address holding = _getHolding();
        ...
        uint256 balance = _dividends[tokenId];
        ...
        emit DividendPushed(holding, tokenOwner, tokenId, balance, msg.sender);
        _div.safeTransferFrom(holding, tokenOwner, balance); // emits Transfer and decreases allowance
    }

    function getUnclaimedDividend(uint256 tokenId) external view override returns(uint256) {
...
interface IAccess {
    ...
    // These helpers were nice but the contract size was too big
    ...
    // function addMarket(address account) external;
    // function removeMarket(address account) external;
}
...
interface IDividend is ITier {
    ...
    event DividendForfeited(uint256 indexed tokenId, address indexed owner, uint256 amount, address caller);
    event DividendPushed(
        address indexed from, address indexed to, uint256 indexed tokenId, uint256 amount, address caller);

    /// @dev Allow 1 or more dividend allocation requests to be conceptually queued such that a caller can service
    ///     these requests with repeated calls (paging) until complete
    struct DivAllocationRequest {
        uint256 divAmountIn; // Dividend to be allocated across tokens
        ...
        // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
        uint256[10] __gap; // The sum of all storage slots used (except this member) should be 15
    }

    function getDivSlippage() external view returns(uint256);
    function requestDivAllocation(uint256 divAmountIn, uint256 pageSize) external returns(bool);
    function processAllocationRequest(uint256 pageSize) external returns(bool);
    function pushDividends(uint256 tierId, uint256 tokenIndexBegin, uint256 tokenIndexEnd) external returns(uint256);
    function getUnclaimedDividend(uint256 tokenId) external view returns(uint256);
}
...
interface ITier {
    struct TierInfo {
        ...
        EnumerableSetUpgradeable.UintSet tokenIds; // to enumerate tokens in a tier during dividend allocation
    }
...
abstract contract Royalty is IRoyalty, Initializable {
    ...
    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[13] private __gap; // The sum of all storage slots used (except this member) should be 15
...
contract Tier is ITier, Initializable {
    ...
    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[12] private __gap; // The sum of all storage slots used (except this member) should be 15
...
abstract contract Token is IToken, Initializable {
    ...
    function _createToken(address tokenOwner, uint256 tierId, uint256 holdPeriodEnd) internal {
        _tokens[++_tokenIdSeed] = TokenInfo(tokenOwner, tierId, holdPeriodEnd);
    }
