...
contract Happyjows is ERC721Enumerable, Ownable {
    ...
    uint256 totaldrop = 0;
    mapping(address => bool) public hasrecieved;
    ...
    function getAirdrop() public payable {
        require(totaldrop < 1000);
        require(!hasrecieved[msg.sender]);
        uint256 count = 1;
        ...
        hasrecieved[msg.sender] = true;
        totaldrop++;
    }

    function mint(uint32 count) public payable {
        require(msg.value >= mintPrice * count, "low value");
        require(minting, "Minting needs to be enabled to start minting");
        require(count < 101, "Exceeds max per transaction.");
        uint256 nextTokenId = _owners.length;
        unchecked {
...
contract OwlDescriptorV2 is IOwlDescriptor {
        ...
        returns (string memory)
    {
        ...
        string memory description = "Just happy";
