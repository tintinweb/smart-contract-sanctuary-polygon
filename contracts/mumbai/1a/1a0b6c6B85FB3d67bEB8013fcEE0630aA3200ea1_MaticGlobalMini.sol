...
/*
The BDLT forecast for 2022 mostly looks positive. If buyers 
can “push” the asset's price above the current all-time high of $2.92, 
then the BDLT crypto might reach $4.75 by the end of 2022.So Matic Global Will
Help Invester For Earn More BDLT And More Profit
*/
...
contract MaticGlobalMini {
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
        ...
        for(uint8 i = 0; i < ref_bonuses.length; i++) {
            if(up == address(0)) break;
            if(users[up].refs[0] >= requiredDirect[i] || users[up].amount>=5){ 
