    ...
    BasicMetaTransaction
{
    ...
    event FactoryAddressSet(address _factory);
    event NftFreezed(uint256 _tokenId);
    event NftMinted(
        address _to,
        uint256 _tokenId,
        string _tokenURI,
        LoanLib.Loan _details
    );
    event BatchApproved(uint[] _ids);
    ...
    function mint(
        ...
        borrower[_details.borrower].push(_tokenId);
        emit NftMinted(_to, _tokenId, _tokenURI, _details);
    }
    ...
    function transferFrom(
        ...
        require(NFTfreeze[tokenId] == false, "NFT freezed");
        super.transferFrom(from, to, tokenId);
    }


 /**
     * @notice external function to give approval in batch
     * @param _ids tokenId of ERC721 
     **/

    function batchApproval(uint[] calldata _ids) external {
        require(_ids.length <= 20, "Invalid length");
        for(uint i = 0; i < _ids.length; i++){
            super.approve(address(factory), _ids[i]);
        }
        emit BatchApproved(_ids);
    }
