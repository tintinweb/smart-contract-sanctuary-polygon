...
contract FanList is Ownable {
  ...
  function add(address _address, string memory _campaign, string memory _encryptedData) public returns(uint listNumber) {
    require(_address == _msgSender(), 'Cannot add others.');
  ...
  function update(address _address, string memory _encryptedData) public returns(bool success) {
    require(_address == _msgSender(), 'Cannot update others.');
  ...
  function getListIndex(address _address) public view returns(uint) {
  ...
  function getList() public view onlyOwner returns(address[] memory) {
    return list;
  }
  function getFullList() public view onlyOwner returns(address[] memory, string[] memory) {
    string[] memory out = new string[](list.length);
    for (uint i = 0; i < list.length; i++) {
      out[i] = infos[list[i]].encryptedData;
    }
    return (list, out);
  }
}
