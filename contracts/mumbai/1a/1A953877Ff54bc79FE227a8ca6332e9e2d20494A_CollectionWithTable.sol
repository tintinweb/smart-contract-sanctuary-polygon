...
library TablelandDeployments {
    ...
    // TablelandTables address on Arbitrum.
...
contract CollectionWithTable is
    Initializable,
    ...
    Pausable
{
    ...
    string private _attributesPrefix;
    bytes32 public ADMIN = keccak256("ADMIN");
    bytes32 public COMPLEX_ADMIN = keccak256("COMPLEX_ADMIN");
    bool private _publicMint;

    string private _animationBaseURI = "help";

    event TokenImageUpdated(uint256 tokenId_, string imageURL_);
    ...
    function initialize(
        ...
        address admin_,
        bool publicMint_,
        address complexAdmin_
    ) public initializer {
        __ERC721_init(collectionName, collectionSymbol);
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(ADMIN, msg.sender);
        _grantRole(ADMIN, admin_);
        _grantRole(COMPLEX_ADMIN, complexAdmin_);
        _baseURIString = baseURI;
        _tablePrefix = "collection";
        _attributesPrefix = "attributes";
    ...
    function createTables(address registry) external payable whenNotPaused onlyRole(ADMIN) returns (uint256) {
        ...
        _metadataTableId = _tableland.createTable(
            ...
            string.concat(
                ...
                "_",
                Strings.toString(block.chainid),
                " (id int, thumb text, image text, animation_url text);"
            )
        );

        _metadataTable =
            string.concat(_tablePrefix, "_", Strings.toString(block.chainid), "_", Strings.toString(_metadataTableId));
        ...
        _attributesTableId = _tableland.createTable(
            ...
            string.concat(
                ...
                "_",
                Strings.toString(block.chainid),
                " (type text, value text, token_id text);"
            )
        );

        _attributesTable = string.concat(
            _attributesPrefix, "_", Strings.toString(block.chainid), "_", Strings.toString(_attributesTableId)
        );
        return _metadataTableId;
    }

    //MINTING
    function adminMint(address to, string memory thumb, string memory image, Attributes[] memory attributes)
        external
        whenNotPaused
        onlyRole(ADMIN)
        returns (uint256)
    {
        uint256 tokenId = _mint(to, thumb, image, attributes);
        return tokenId;
    }

    function publicMint(address to, string memory thumb, string memory image, Attributes[] memory attributes)
        public
        whenNotPaused
    {
        require(_publicMint, "ERR: Minting not available");
        _mint(to, thumb, image, attributes);
    }

    function _mint(address to, string memory thumb, string memory image, Attributes[] memory attributes)
        internal
        returns (uint256)
    {
        _tokenId.increment();
        ...
        _tableland.runSQL(
            ...
            string.concat(
                "INSERT INTO",
                _metadataTable,
                "(id, thumb, image, animation_url) VALUES (",
        ...
        if (attributes.length > 0) {
            for (uint256 i = 0; i < attributes.length; i++) {
                _insertAttribute(attributes[i]._type, attributes[i]._value, tokenIdString);
            }
        }
        _safeMint(to, tokenId, "");
        return tokenId;
    ...
    //UPDATE DATA
    function updateAnimationURL(string memory animationBaseURI_) external whenNotPaused onlyRole(ADMIN) {
        _animationBaseURI = animationBaseURI_;
    }

    function updateMetadataField(string memory field_, string memory updatedField_)
        external
        whenNotPaused
        onlyRole(ADMIN)
    {
        _tableland.runSQL(
            address(this), _metadataTableId, SQLHelpers.toUpdate(_tablePrefix, _metadataTableId, field_, updatedField_)
        );
    }

    function updateImage(string memory newImage, uint256 tokenId) external whenNotPaused onlyRole(ADMIN) {
        _tableland.runSQL(
            address(this),
            _metadataTableId,
            SQLHelpers.toUpdate(
                _tablePrefix, _metadataTableId, "image, token_id", string.concat(newImage, Strings.toString(tokenId))
            )
        );
        // string.concat(
        //     "UPDATE ", _metadataTable, " SET image = ", newImage, " WHERE id = ", Strings.toString(tokenId), ";"
        // )

        emit TokenImageUpdated(tokenId, newImage);
    }

    function updateAttributes(uint256 tokenId, Attributes memory attribute) external whenNotPaused onlyRole(ADMIN) {
        _tableland.runSQL(
            address(this),
            _attributesTableId,
            string.concat(
    ...
    function insertAttributes(Attributes[] memory attributes_, uint256 tokenId_)
        external
        whenNotPaused
        onlyRole(ADMIN)
    {
        string memory tokenIdString = Strings.toString(tokenId_);
        for (uint256 i = 0; i < attributes_.length; i++) {
            _insertAttribute(attributes_[i]._type, attributes_[i]._value, tokenIdString);
        }
    }

    //URL Stuff
    function setBaseURI(string memory baseURI) public whenNotPaused onlyRole(ADMIN) {
        _baseURIString = baseURI;
    }
    ...
    //INTERNAL FUNCTIONS
    function _insertAttribute(string memory attributeType, string memory value, string memory tokenId)
        internal
        whenNotPaused
    {
        _tableland.runSQL(
            ...
            SQLHelpers.toInsert(
                ...
                _attributesTableId,
                "type, value token_id",
                string.concat(attributeType, value, tokenId)
            )
        );
    }

    function _getMetadataURI(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "ERC721URIStorage: URI query for nonexistent token");
        //Add return here for when the contract is paused to show an error
        string memory base = _baseURI();
        string memory tokenIdString = Strings.toString(tokenId);
    ...
    //GETTERS
    function getMetadataTable() public view whenNotPaused returns (string memory) {
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override (AccessControl, ERC721Upgradeable)
        returns (bool)
    {
