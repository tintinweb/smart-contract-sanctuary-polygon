...
import "./utils/ERC721URIStorageEnumerable.sol";
...
contract LandRegistry is ERC721URIStorageEnumerable {
    ...
    event UpdatedTokenURI(uint256[] tokenIds);
    event UpdatedMetadata(uint256[] tokenIds);
    ...
    modifier onlyMinter() {
        //  Note: Should not allow metatransaction
        if (!gov.hasRole(MINTER_ROLE, msg.sender)) revert OnlyMinter();
        _;
    }
    ...
    */
    function setGovernance(IGovernance _newGov) external onlyManager {
        if (address(_newGov) == address(0)) revert ZeroAddress();

        gov = _newGov;
    }

    /**
        @notice Set Trusted Forwarder contract (support metatransaction)
        @dev  Caller must have MANAGER_ROLE
        @param _newForwarder         Address of Forwarder contract
        Note: If disable the metatransaction, `_trustedForwarder` can be set to 0x00
        Usage: Forwarder contract contains some bugs that need to be fixed. Thus, set `_trustForwarder` to 0x00
        Deploy a new Forwarder contract, then set a new address of Forwarder contract
    */
    function setForwarder(address _newForwarder) external onlyManager {
        trustedForwarder = _newForwarder;
    }

    /**
        @notice Validate whether `_forwarder` address is TrustedForwarder contract
        @dev  Caller can be ANY
        @param _forwarder         Address of a contract to check
    */
    ...
    /**
       	@notice Query a list of `_tokenIds` that owned by `_owner`
       	@dev  Caller can be ANY
    ...
    */
    function assignParcel(address _to, uint256 _tokenId, string calldata _tokenURI) public onlyMinter {
        _safeMint(_to, _tokenId);

        // It's optional to set `_tokenURI`
        if (bytes(_tokenURI).length > 0) _setTokenURI(_tokenId, _tokenURI);
    }
    ...
    */
    function assignParcels(
        ...
        if (_tokenURIs.length != _len) revert LengthMismatch();

        for (uint256 i; i < _len; i++)
            assignParcel(_to, _tokenIds[i], _tokenURIs[i]);
    }
    ...
    */
    function assignBatchParcels(
        ...
        if (_tokenIds.length != _len || _tokenURIs.length != _len) revert LengthMismatch();

        for (uint256 i; i < _len; i++)
            assignParcel(_receivers[i], _tokenIds[i], _tokenURIs[i]);
    }
    ...
    */
    function removeBatchParcels(uint256[] calldata _tokenIds) external {
        ...
        uint256 _len = _tokenIds.length;
        if (gov.hasRole(MINTER_ROLE, _caller)) authorized = true;
    ...
    /**
       	@notice Update a batch of `metadata`
       	@dev  Caller must have AUTHORIZER_ROLE
		@param	_tokenIds		        A list of `_tokenId` that need to be updated the URI
		@param	_metadata		        A list of metadata
    */
    function setMetadata(uint256[] calldata _tokenIds, string[] calldata _metadata) external onlyAuthorizer {
        uint256 _len = _tokenIds.length;
        if (_metadata.length != _len) revert LengthMismatch();

        for (uint256 i; i < _len; i++)
            metadata[_tokenIds[i]] = _metadata[i];

        emit UpdatedMetadata(_tokenIds);
    }
    ...
    /**
       	@notice Update a batch of `_tokenURIs`
       	@dev  Caller must have AUTHORIZER_ROLE
		@param	_tokenIds		        A list of `_tokenId` that need to be updated the URI
		@param	_tokenURIs		        A list of `_tokenURI`
    */
    function updateTokenURI(uint256[] calldata _tokenIds, string[] calldata _tokenURIs) external onlyAuthorizer {
        uint256 _len = _tokenIds.length;
        if (_tokenURIs.length != _len) revert LengthMismatch();
        ...
        emit UpdatedTokenURI(_tokenIds);
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override isLocked {
 ...
 * account.
 */
abstract contract ERC721URIStorageEnumerable is ERC721URIStorage, IERC721Enumerable {
