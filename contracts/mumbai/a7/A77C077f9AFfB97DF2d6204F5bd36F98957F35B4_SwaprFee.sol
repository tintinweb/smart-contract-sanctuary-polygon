...
contract SwaprFee is BaseGovernanceUpgradeable {
    ...
    function setLKRToken(address _lkrToken, address _lkrOracle, uint256 _discountAmount) external onlyAdmin {
        require(_lkrToken != address(0), "Cant set address 0");
        require(_lkrOracle != address(0), "Cant set address 0");
        require(_discountAmount <= EXP, "Discount cant be more than 100%");
        lkrToken = IERC20MetadataUpgradeable(_lkrToken);
        lkrOracle = IDIAOracleV2(_lkrOracle);
        discountAmount[_lkrToken] = _discountAmount;
    }
    ...
    function setNativeTokenPriceFeed(address _priceFeed) external onlyAdmin {
        ...
        cryptoPriceFeed = AggregatorV3Interface(_priceFeed);
    }

    function removeNativeTokenPriceFeed() external onlyAdmin {
        delete cryptoPriceFeed;
    }
    ...
    function isTokenSupported(address _paymentToken) public view returns(bool exists) {
        if(_paymentToken == address(0)) {
            address(cryptoPriceFeed) != address(0) ? exists = true : exists = false;
    ...
    function addDiscountTokens(
        ...
        uint256[] memory _discountAmount
    ) external onlyAdmin {
    ...
    function removeDiscountToken(
        ...
        (bool exists,) = _exists(_discountToken);
        require(exists, "This is not a payment token");
        require(discountAmount[_discountToken] > 0, "Discount not set");
    ...
    function setPercentBaseOrderFee(uint percentBaseFee) external onlyAdmin {
        require(percentBaseFee <= EXP, "Fee exceed 100%");
    ...
    function setFinalOrderFee(uint finalFeePercentage) external onlyAdmin {
        require(finalFeePercentage <= EXP, "Fee exceed 100%");
    ...
    function _getBaseFee(FeePricing memory feePricing, uint subjectAmount, address token) internal view returns(uint totalBaseFee, uint percentBaseFee, uint fixedBaseFee) {
        ...
        if(token == address(0)) {
            uint baseRecount = (feePricing.fixedBaseFee * EXP / _getPrice(cryptoPriceFeed));
            fixedBaseFee = baseRecount * _getFeedDecimals(cryptoPriceFeed) / EXP;
        } else if(token == address(lkrToken)) {
            uint baseRecount = (feePricing.fixedBaseFee * EXP / _getLkrPrice(lkrOracle));
            fixedBaseFee = baseRecount * 10**8 / EXP;
        } else{
            uint baseRecount = (feePricing.fixedBaseFee * EXP / _getPrice(oracleFeedForToken[token]));
            fixedBaseFee = baseRecount * _getFeedDecimals(oracleFeedForToken[token]) / EXP;
        }
    ...
    function _calculateFinalFeeCap(FeePricing memory feePricing, address token) internal view returns (uint priceCapTokens) {
        if(address(lkrToken) != address(0) && token == address(lkrToken)) {
            uint capRecount = (feePricing.finalFeeCap * EXP / _getLkrPrice(lkrOracle));
            priceCapTokens = capRecount * 10**8 / EXP;
        } else {
            uint capRecount = (feePricing.finalFeeCap * EXP / _getPrice(getTokenFeed(token)));
            priceCapTokens = capRecount * _getFeedDecimals(getTokenFeed(token)) / EXP;
        }
    }

    function _calculatePriceCap(FeePricing memory feePricing, address token) internal view returns (uint priceCapTokens) {
        if(address(lkrToken) != address(0) && token == address(lkrToken)) {
            uint capRecount = (feePricing.priceCap * EXP / _getLkrPrice(lkrOracle));
            priceCapTokens = capRecount * 10**8 / EXP;
        } else {
            uint capRecount = (feePricing.priceCap * EXP / _getPrice(getTokenFeed(token)));
            priceCapTokens = capRecount * _getFeedDecimals(getTokenFeed(token)) / EXP;
        }
    }
