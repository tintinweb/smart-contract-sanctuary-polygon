...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function borrow(address assetAddress, uint256 borrowAmount) nonReentrant external {
        Deposit storage depositInfo = depositDetails[msg.sender][assetAddress];
        uint256 maxBorrowAmount = maxBorrow(msg.sender, assetAddress);
        require(maxBorrowAmount >= borrowAmount, "Margin: Cannot give more loan");
    ...
    function maxBorrow(address user, address assetAddress) public view returns(uint256) {
    ...
    function ltmRatio(address user, address assetAddress) public view returns(uint256) {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
        uint256 totalMargin = depositInfo.marginAmount + depositInfo.marginReserve - depositInfo.lossValue +
            depositInfo.gainValue;
