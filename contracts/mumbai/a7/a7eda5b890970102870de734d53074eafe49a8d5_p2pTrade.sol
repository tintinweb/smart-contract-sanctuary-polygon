...
contract p2pTrade is Ownable, ReentrancyGuard {
    ...
    uint256 private constant MINIMUM_ASSET = 10**10;
    uint256 private constant RATIO__FACTOR = 10**18;
    ...
    address public constant USD = 0xC5DC2366997A1Db48ed0a909c12c778d717a1859;
    ...
    modifier onlyOperator() {
        require(_isOperator[msg.sender] == true, "Not operator");
        _;
    }
    ...
    function createOrder(
        ...
        require(minToBuy <= amountAsset0, "Check the limits of your order");
        ...
        for (uint256 i = 0; i < methods.length; i++) {
            ordersByMethod[methods[i]].openOrders.push(nextOrder);
        }
        ...
        order.ratio0 = ratio0;
        order.ratio1 = ratio1;
        order.ratio = (ratio0 * ratio1).div(RATIO__FACTOR);
        require(order.ratio > 0, "Ratio must be more than 0");

        order.usdLock = order.ratio.mul(amountAsset0).div(RATIO__FACTOR);
        ...
        IBEP20(USD).safeTransferFrom(msg.sender, address(this), order.usdLock);
        ...
        nonReentrant
    {
        ...
        deal.amountUsd = order.ratio.mul(amount0).div(RATIO__FACTOR);
    ...
    function cancellOrder(uint256 orderId) public {
        ...
        uint256 amountUsd = order.ratio.mul(amount0).div(RATIO__FACTOR);
    ...
    function setConflict(uint256 dealId) external nonReentrant {
        ...
        require(
            canConflict(dealId) == true,
            "Please wait or Conflict is Open!"
        );
    ...
    function updateOrderLimits(
        ...
        require(
            minToBuy <= order.leftAsset0 && maxToBuy >= minToBuy,
    ...
    function putSignBuyer(uint256 dealId) external {
        DealInfo storage deal = dealInfo[dealId];

        require(deal.buyer == msg.sender, "Not buyer");
        require(deal.sign < 2, "Deal already close");
        require(deal.buyerSigned == false, "You have already signed");
    ...
    function putSignSeller(uint256 dealId) external {
        DealInfo storage deal = dealInfo[dealId];

        require(deal.seller == msg.sender, "Not seller");
        require(deal.sign < 2, "Deal already close");
        require(deal.sellerSigned == false, "You have already signed");
    ...
    function deleteOrder(uint256 orderId) private {
        ...
        for (uint256 i = 0; i < order.methods.length; i++) {
            OrdersByMethod storage byMethod = ordersByMethod[order.methods[i]];
        ...
        uint256[] storage arrayOrders = sellerInfo[order.owner].openOrders;
        ...
        sellerInfo[order.owner].openOrders = arrayOrders;
    }
