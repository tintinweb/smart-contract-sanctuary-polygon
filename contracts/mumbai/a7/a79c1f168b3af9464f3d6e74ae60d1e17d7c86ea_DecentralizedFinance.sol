...
contract DecentralizedFinance  {
    ...
    uint256 private constant freezeIncomePercents = 3000;   ////30% will frezze
    uint256 private constant timeStep = 15 minutes;          // basically used to divide by 1 day to get actual values
    uint256 private constant dayPerCycle =25 minutes;       /////cycle duration is 10 MINUTES for now (7 days in original)
    uint256 private constant dayRewardPercents = 400;       /////4% weekly
    uint256 private constant maxAddFreeze = 30 minutes;     //max add freeze time >> (45 days in original) 
    uint256 private constant referDepth = 25;               //referDepth
    uint256 public Gassfee = 2;

    uint256 private constant directPercents = 600;  //6 %
    uint256[4] private level4Percents = [100, 200, 300, 100];  //1,2,3,1
    uint256[20] private level5Percents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50,50,50,50,50,50]; //2,1,1,1,1,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5,0.5
    ...
    constructor(IBEP20 _busdAddr, address _defaultRefer, address  _feeReceivers) public {
        ...
        lastDistribute = block.timestamp+timeStep;
