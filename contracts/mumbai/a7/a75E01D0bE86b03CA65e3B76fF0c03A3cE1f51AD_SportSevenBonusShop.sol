...
contract SportSevenBonusShop is Pausable, Ownable {
    ...
    mapping(uint8 => uint256) public usdPrice;
    mapping(myIERC20 => bool) public allowedStable;

    event BuyBonus(uint256 _id, uint8 _bonusCategory, address _to);
     ...
     * @param _bonusCategory Pack selected
     */
    function buyInETH(
        uint8 _bonusCategory,
        address _to,
        uint256 _amount
    ) public payable {
        uint256 ethPrice = getNFTPriceInETH(_bonusCategory);
        require(
        ...
        for (uint256 i; i < _amount; i++) {
            ...
            emit BuyBonus(tokenId, _bonusCategory, _to);
        }
    ...
    function buyInUSD(
        ...
        _stableAddress.safeTransferFrom(
            msg.sender,
            fundsRecipient,
            _amount * usdPrice[_bonusCategory] * 10 ** (_stableAddress.decimals() - 2)
        );
        for (uint256 i; i < _amount; i++) {
            uint256 tokenId = _mint(_to);
            emit BuyBonus(tokenId, _bonusCategory, _to);
     ...
     * @param _bonusCategory rarity level
     * @param _price New price in USD (no decimals) for the token
     */
    function setPrice(uint8 _bonusCategory, uint256 _price) external onlyOwner {
        usdPrice[_bonusCategory] = _price;
    }
    ...
    function getNFTPriceInETH(
        ...
        uint256 priceInUsd = usdPrice[_bonusCategory];
