...
contract RentNFT {
  ...
  function extendBorrowTime(uint256 _lendingId, uint256 _numOfDays) external payable onlyBorrower(_lendingId) {
    require(nftMapping[_lendingId].maxBorrowTime >= _numOfDays, "Exceeding borrow limit");
    require(msg.value >= _numOfDays*nftMapping[_lendingId].dailyRate, "Pay required amount");
