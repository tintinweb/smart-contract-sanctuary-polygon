...
//     function canTransfer(address from, address to, uint256 value) external view returns (uint8);
// }

contract EquityToken is ERC20, IERC1404, Ownable {
   ...
   constructor() ERC20("Equity Token", "EQTK") {
        company = msg.sender;
    ...
    function setDividentRate(uint256 _dividendRate) external onlyOwner() {
        dividendRate = _dividendRate;
    }

    // function transfer(address to, uint256 value) external override returns (bool) {
    //     require(canTransfer(msg.sender, to, value) == 0, "Transfer not allowed.");
    ...
    //     require(canTransfer(from, to, value) == 0, "Transfer not allowed.");
    //     _transfer(from, to, value);
    //     _approve(from, msg.sender, allowance(from, msg.sender) - value);
    //     return true;
    // }

    // function canTransfer(address from, address to, uint256 value) public view virtual override returns (uint8) {
    //     if (from == company) {
    //         return 0; // Allow company to transfer shares freely
    //     }
    ...
    function transfer (address to, uint256 value) public override notRestricted(msg.sender, to, value) returns (bool success)
    {
       require(
        ...
        if (msg.sender == company) {
            return true; // Allow company to transfer shares freely
        }
    ...
    function issueDividend() external {
        require(msg.sender == company, "Only the company can issue dividends");

        uint256 totalDividend = (balanceOf(company) * dividendRate) / 100;
        uint256 dividendPerShare = totalDividend / totalShares;

        for (uint256 i = 0; i < tokenHolders.length; i++) {
            address shareholder = tokenHolders[i];
            uint256 dividendAmount = balanceOf(shareholder) * dividendPerShare;
            _transfer(company, shareholder, dividendAmount);
        }
    }
