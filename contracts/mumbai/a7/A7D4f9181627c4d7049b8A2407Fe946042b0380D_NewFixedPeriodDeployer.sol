...
import "../base/NewFixedPeriod.sol";
import "../util/OwnableUpgradeable.sol";
import "../interfaces/INewFixedPeriodDeployer.sol";

contract NewFixedPeriodDeployer is INewFixedPeriodDeployer, OwnableUpgradeable {
  address public immutable FIXEDPERIOD_IMPL;

  constructor() {
    __Ownable_init(msg.sender);
    FIXEDPERIOD_IMPL = address(new NewFixedPeriod());
  }

  function newDeployFixedPeriod(
    address _timelock,
    address _erc20,
    ...
    NewFixedPeriod(clone).initialize(
      _timelock,
      _erc20,
      _platform,
      _receivingAddress,
...
contract NewFixedPeriod is
  ...
  event SetPlatformRate(uint256 newPlatformRate);
  ...
  function initialize(
    ...
    (repeatMintNum,mintDeadlineTime,isNotWhitelist,_name,_symbol,_bURI) = abi.decode(_newParameter,(uint256,uint256,bool,string,string,string));
...
//    (repeatMintNum,mintDeadlineTime,isNotWhitelist,erc20) = abi.decode(_newParameter,(uint256,uint256,bool,address));
//    require(repeatMintNum > 0, "Must be greater than 0");
  }

  // only contract admin can set platformRate
  function setPlatformRate(uint256 _platformRate) public onlyOwner{
    platformRate = _platformRate;
    emit SetPlatformRate(_platformRate);
  ...
  function mint() public nonReentrant returns (uint256 tokenId) {
    ...
    //  whiteLists mode
    if (isNotWhitelist) {
        require(isWhite(_msgSender()), "Not a whitelist user");
    }
    // cutoff mode
  ...
  function mintEth() public payable nonReentrant returns (uint256 tokenId) {
    ...
    if (msg.value - rate > 0) {
      (bool success, ) = receivingAddress.call{value: msg.value - rate}(   // update
        ""
      );
  ...
  function withdraw() public nonReentrant onlyOwner {
    if (address(erc20) == address(0)) {
      ...
      (bool success, ) = platform.call{value: amount}(""); // withdraw ETH to platform account
      require(success, "Failed to send Ether");

      emit Withdraw(platform, amount);
    } else {
      uint256 amount = IERC20Upgradeable(erc20).balanceOf(address(this));
      IERC20Upgradeable(erc20).safeTransfer(platform, amount); // withdraw erc20 tokens to platform account
...
interface INewFixedPeriodDeployer {
...
contract Whitelist is OwnableUpgradeable {
      ...
      EnumerableSetUpgradeable.AddressSet private _whiteLists;  // white users

      // batch add white user
      function addWhite(address[] memory _addWhite) public onlyOwner returns (bool) {
          for (uint i = 0; i < _addWhite.length; i++) {
              require(_addWhite[i] != address(0), "addWhite: _addWhite is the zero address");
              EnumerableSetUpgradeable.add(_whiteLists, _addWhite[i]);
          }
          return true;
        }

      // batch delete white user
      function delWhite(address[] memory _delWhites) public onlyOwner returns (bool) {
          for (uint i = 0; i < _delWhites.length; i++) {
              require(_delWhites[i] != address(0), "delWhite: _delWhite is the zero address");
              EnumerableSetUpgradeable.remove(_whiteLists, _delWhites[i]);
          }
          return true;
        }


      function getWhiteLength() public view returns (uint256) {
            return EnumerableSetUpgradeable.length(_whiteLists);
        }


      function isWhite(address account) public view returns (bool) {
            return EnumerableSetUpgradeable.contains(_whiteLists, account);
        }


      function getWhite(uint256 _index) public view  returns (address){
            require(_index <= getWhiteLength() - 1, "White: index out of bounds");
            return EnumerableSetUpgradeable.at(_whiteLists, _index);
        }
}
