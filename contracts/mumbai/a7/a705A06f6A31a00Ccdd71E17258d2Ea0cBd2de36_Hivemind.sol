 ...
 */
contract Hivemind is HivemindStorage, HivemindI, AccessControlEnumerable, ReentrancyGuard {
  ...
  uint256 public constant FEE_CAP = 0.02 ether; // 2%
  ...
  function depositAllowed(
    ...
    if (marketPausedInfo[_hToken].depositPaused) revert Paused();
    if (address(_oracles[_hToken]) == address(0)) revert HivemindError(Error.ORACLE_NOT_SET);
    ...
    if (!market.accountMembership[_depositor]) {
      // only hTokens may call depositAllowed if depositor not in market
  ...
  function redeemAllowed(
    ...
    if (!market.accountMembership[_redeemer]) {
      revert HivemindError(Error.NOT_FOUND);
    }
  ...
  function depositCollateralAllowed(
    ...
    _depositor;
    _collateralId;
    if (marketPausedInfo[_hToken].depositCollateralPaused) revert Paused();
    Market storage market = _markets[_hToken];
  ...
  function getCollateralPriceInUnderlying(HTokenI _hToken) internal view returns (uint256) {
    ...
    if (lastUpdated < block.timestamp && block.timestamp - lastUpdated > cachedOracle.updateThreshold()) revert OracleNotUpdated();
  ...
  {
    if (_redeemTokens > 0)
      (liquidityTillLiquidation, shortfall) = getHypotheticalAccountLiquidityRedeemInternal(_hToken, _account, _redeemTokens);
    else
  ...
  function repayBorrowAllowed(
    ...
    _repayAmount;
    _collateralId;
    if (marketPausedInfo[_hToken].repayBorrowPaused) revert Paused();
  ...
  function withdrawalCollateralAllowed(HTokenI _hToken, uint256 _collateralId) external view override {
    ...
    if (_hToken.getDebtForCollateral(_collateralId) != 0) revert HivemindError(Error.WITHDRAW_NOT_ALLOWED);
  }
  ...
  {
    Market storage market = _markets[_hToken];
    return (market.isListed, market.collateralFactorMantissa, market.maxLTVFactorMantissa);
  }
  ...
  function getCollateralFactor(HTokenI _hToken) external view override returns (uint256) {
    return _markets[_hToken].collateralFactorMantissa;
  }
  ...
  function getHypotheticalAccountLiquidityRedeemInternal(
    ...
    if (hTokenBalance < _redeemTokens) revert HivemindError(Error.BALANCE_INSUFFICIENT);
   ...
   * @return liquidityTillLiquidation - hypothetical account liquidity in excess of collateral requirements
   * @return shortfall - hypothetical account shortfall below collateral requirements
   * @return liquidityTillLTV
 - Loan to value shortfall
   */
    ...
    )
  {
    ...
    if (_borrowAmount > availableUnderlying) revert HivemindError(Error.INSUFFICIENT_LIQUIDITY);

    // computing collateral factor applied on the collateral total amount
    // collateral factor % from the NFT price
    uint256 ltvValue = (_markets[_hToken].maxLTVFactorMantissa * sumCollateral) / 1e18;
   ...
   */
  function oracle(HTokenI _hToken) external view override returns (PriceOracleI) {
   ...
   * @param _hToken The market we set the oracle for
  ...
  /**
   * @notice Sets the maxLTVFactor used when liquidating borrows
   * @dev Admin function to set maxLTVFactor, can not be 0
   * @param _newMaxLTVFactorMantissa New close factor, scaled by 1e18
   * @param _hToken Market to set the close factor for
   */
  function _setMaxLTVFactor(HTokenI _hToken, uint256 _newMaxLTVFactorMantissa) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(FACTORY_ROLE, msg.sender)) revert Unauthorized();
    if (_newMaxLTVFactorMantissa == 0) revert HivemindError(Error.INVALID_MAX_LTV_FACTOR);

    Market storage market = _markets[_hToken];
    if (!market.isListed) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
    if (market.collateralFactorMantissa < _newMaxLTVFactorMantissa) revert HivemindError(Error.INVALID_MAX_LTV_FACTOR);

    emit NewMaxLTVFactor(_hToken, market.maxLTVFactorMantissa, _newMaxLTVFactorMantissa);
    market.maxLTVFactorMantissa = _newMaxLTVFactorMantissa;
  }

  /**
   * @notice Sets the collateralFactor for a market
   * @dev Admin function to set per-market collateralFactor, can not be 0
  ...
  function _setCollateralFactor(HTokenI _hToken, uint256 _newCollateralFactorMantissa) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && !hasRole(FACTORY_ROLE, msg.sender)) revert Unauthorized();
    if (_newCollateralFactorMantissa == 0) revert HivemindError(Error.INVALID_COLLATERAL_FACTOR);
    ...
    if (_newCollateralFactorMantissa < market.maxLTVFactorMantissa) revert HivemindError(Error.INVALID_COLLATERAL_FACTOR);
    ...
    // Set market's collateral factor to new collateral factor
  ...
  function _supportMarket(HTokenI _hToken) external {
    ...
    if (address(underlying) == address(0) || address(collateral) == address(0)) revert HivemindError(Error.MARKET_INVALID);
  ...
  function _pauseComponent(
    ...
    if (_target == 0) {
      ...
      emit ActionPausedhToken(_hToken, "Deposit", _state);
    } else if (_target == 6) {
  ...
  function _setBorrowFeePerMarket(
    ...
    if (_fee > FEE_CAP || _referralFee > FEE_CAP) revert HivemindError(Error.AMOUNT_TOO_BIG);
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
  ...
  event CollateralFactorUpdated(HTokenI indexed _hToken, uint256 _oldCollateralFactorMantissa, uint256 _newCollateralFactorMantissa);

  /// @notice Emitted when price oracle is changed
  event PriceOracleUpdated(PriceOracleI _oldPriceOracle, PriceOracleI _newPriceOracle);
