...
contract UniswapV2PairPriceAdapter is AccessControl, IOracleAdapter {
    ...
    function getReserves1(address poolAddress) external view returns (uint256 token1Reserves, uint256 token2Reserves) {
        PoolSetting memory poolInfo = _poolSettings[poolAddress];

        return UniswapV2Library.getReserves(_uniswapFactory, poolInfo.token1, poolInfo.token2);
    }

    function getReserves2(address poolAddress) external view returns (uint256 token1Reserves, uint256 token2Reserves) {
        (token1Reserves, token2Reserves, ) = IUniswapV2Pair(poolAddress).getReserves();
    }
