...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
    ...
    // Start game by setting appropriate values
    function startGame(uint8 _maxPlayers, uint256 _entryFee) public onlyOwner {
        // check if game is already running
        require(!gameStarted, "Game is currently running");
        // empty the players array
        delete players;
        // set max players for game
        maxPlayers = _maxPlayers;
        // set game started to true
        gameStarted = true;
        // set up entry fee for game
        entryFee = _entryFee;
        gameId += 1;
    ...
    function joinGame() public payable {
        ...
        // check if value sent by user matches the entryFee
        require(msg.value == entryFee, "Must pay full entry fee");
        // check if there is still space to add another player
        require(players.length < maxPlayers, "Game is full");
        // add the sender to the players list
        players.push(msg.sender);
        emit PlayerJoined(gameId, msg.sender);
        // if list is full start the winner selection process
    ...
    // Called to start the process of selecting a random winner
    function getRandomWinner() private returns (bytes32 requestId) {
        // LINK is an internal interface for the Link token found within VRFConsumerBase
        // Here we use the balanceOf method from that interface to make sure our contract has enough Link so that we can request // the VRFCoordinator for randomness
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK");
        // make a request to VRFCoordinator, requestRandomness is a function in VRFConsumerBase; starts the random generation process
