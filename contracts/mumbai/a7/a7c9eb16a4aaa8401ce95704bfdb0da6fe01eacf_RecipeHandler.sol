...
struct RecipeSignature {
  uint16 chainId;  // To prevent crosschain signature attacks
...
contract RecipeHandler is Ownable, StudioSignature {
  uint16 immutable private chainId;
  IERC20 immutable public bgem;
  IERC20 immutable public boom;
  IERC721 immutable public hunters;
  IERC1155 immutable public perks;
  IERC1155 immutable public shards;
  IERC1155 immutable public equipments;
  ...
  constructor(
    ...
    require(initStudio != address(0), "INVALID_STUDIO");
    studio = initStudio;
  ...
  function setStudio(address newStudio) external onlyOwner {
    require(newStudio != address(0), "INVALID_STUDIO");
    studio = newStudio;
  }
  ...
  function openChest(RecipeSignature calldata packet) external {
    require(packet.chainId == chainId, "INVALID_CHAIN_ID");
    require(packet.request == CHEST_REQUEST, "INVALID_REQUEST");

    require(
      _useSignature(studio, packet.payload, packet.signature), 
      "INVALID_SIGNATURE"
    );
  ...
  function upgrade(RecipeSignature calldata packet) external {
    require(packet.request == UPGRADE_REQUEST, "INVALID_REQUEST");
    ...
    // IN - Booms
    require(boom.transferFrom(user, studio, inBoom), "TRANSFER_FAILED");
