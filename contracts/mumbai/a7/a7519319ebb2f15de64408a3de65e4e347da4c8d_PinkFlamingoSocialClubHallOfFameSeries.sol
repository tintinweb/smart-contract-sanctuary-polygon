...
contract PinkFlamingoSocialClubHallOfFameSeries is ERC721Enumerable {
  /**
  @dev events
 */
 ...
 */
  ...
  function contractURI() public view returns (string memory) {
    return
      string(
        abi.encodePacked(baseURI, 'QmVNnrkwb6dwkfznHwWyu87Cm5u6mSk6g2mWtH522gG8TB')
      );
  }
  ...
  {
    return string(abi.encodePacked(baseURI, _uris[_tokenId]));
  }
 ...
 */
  ...
  function mintMingo(string memory _uri) public onlyAdmin returns (uint256 _tokenId) {
    return mintMingoTo(msg.sender, _uri);
  }

  function mintMingoTo(address _to, string memory _uri) public onlyAdmin returns (uint256 _tokenId) {
  ...
  function redeemMingo(address _to) public returns (uint256 _tokenId) {
    require(nextTokenId <= maxTokenId, 'Must not exceed maximum mint on Fantom');
    require(!isMintPaused || msg.sender == admin, 'Purchases must not be paused');
    require(
