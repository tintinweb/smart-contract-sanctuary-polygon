...
contract CollectibleCreator {
    ...
    event GetCollectibles(uint[] collectibles);
    ...
    function getCollectiblesByOwner() external {
        uint[] memory result = new uint[](ownerCollectibleCount[msg.sender]);
        uint counter = 0;
        for (uint i = 0; i < collectibles.length; i++) {
            if (collectibleToOwner[i] == msg.sender) {
                result[counter] = i;
                counter++;
            }
        }
        emit GetCollectibles(result);
    }
