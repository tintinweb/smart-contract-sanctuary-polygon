...
// ganache-cli --accounts 10 --fork https://eth-mainnet.alchemyapi.io/v2/9OGSTexsUDfIZiuaFbFhoWtE6LXmN0Yn --mnemonic brownie --port 8545 --hardfork istanbul --i 1
import "ERC721A.sol";
    ...
    ReentrancyGuard
{
    ...
    //Maximum number of NFTs an address can mint
    uint256 public max_mint_allowed_presale = 2;
    ...
    function changeMaxMintAllowedPreSale(uint256 _maxMintAllowedPreSale)
        external
        onlyOwner
    {
        ...
        nonReentrant
    {
        ...
        require(
            nftsPerWallet[msg.sender] < max_mint_allowed_presale,
            string(
                abi.encodePacked(
                    "You can only get ",
                    Strings.toString(max_mint_allowed_presale),
                    " NFT on the Presale"
                )
            )
        );
        require(
            _ammount <= max_mint_allowed_presale,
            string(
                abi.encodePacked(
    ...
    function saleMint(uint256 _ammount) external payable nonReentrant {
        ...
        require(
            nftsPerWalletSale[msg.sender] < max_mint_allowed,
            string(
                abi.encodePacked(
                    "You can only get ",
