...
contract ScoreCast is FunctionsClient, ConfirmedOwner {
  ...
  string private source = "return Functions.encodeUint256(123)";
  ...
  function fulfillRequest(
    ...
    // latestError = err;
    // emit FulfillingRequest(requestToFixture[requestId]);
    // string memory fixtureId = requestToFixture[requestId];
    // emit FulfillingRequest(fixtureId);
    // emit FulfillingRequest2(response);
    // emit FulfillingRequest3(fixtureId, response);
    // fixtureToResults[fixtureId] = response;
    fixtureToResults["test"] = response;
