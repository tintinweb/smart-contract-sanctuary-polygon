...
contract TreasureHuntPirate is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function setPirate(uint256 tokenID, uint8 star, uint16 power) public {
        ...
        string memory uri = string(abi.encodePacked(pirateBaseURI[star], StringsUpgradeable.toString(tokenID)));
    ...
    function readMysteryAmount(address acc) public view returns(uint256) {
        return mysteryChestAmt[acc];
    }

    function setBaseURI(string[] memory _pirateBaseURI) public onlyOwner {
        for(uint8 i = 1; i <= 5; i++ ){
            pirateBaseURI[i] = _pirateBaseURI[i];
        }
    }
