...
contract UserManage is OwnableUnset {
    mapping(address => address) public identityContracts;
    mapping(address => uint256) public ids;
    uint256 public totalCount;
    IBlockShow public blockShowContract =
        IBlockShow(0x235Ea790c5ADae735438a90e306e048BCF3bDE58);
    ...
    function isValid(address _user) public view returns (bool) {
        return blockShowContract.isWhitelisted(_user);
    }

    function createIdentity(bytes32[] memory __keys, string[] memory __values)
        external
    {
        require(
            identityContracts[msg.sender] == address(0x0) &&
                isValid(msg.sender) == true,
            "You already have identity contract"
        );

        totalCount++;
        ids[msg.sender] = totalCount;

        OazizCitizenship identity = new OazizCitizenship(
            address(this),
            appendUintToString("Citizen#", totalCount)
        );
        identityContracts[msg.sender] = identity.CitizenshipAddr();
        identity.setData(__keys, __values);
        identity.transferOwnership(msg.sender);
    }

    function setBlockShowAddress(address _blockshowAddr) external onlyOwner {
        blockShowContract = IBlockShow(_blockshowAddr);
    }

    function uintToString(uint256 v) pure public returns (string memory str) {
    ...
    function toBytes(uint256 x) pure public returns (bytes1 b) {
