...
interface IPosition {
    ...
    function checkUserOpenPosition(address user, address assetAddress) external view returns(bool, uint256);
...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function deposit(
        ...
        uint256 marginReserve = (marginAmount * marginReservePercent) / 100;
        ...
        if(isUserAdded[assetAddress][msg.sender] == false) {
            assetUsers[assetAddress].push(msg.sender);
            isUserAdded[assetAddress][msg.sender] = true;
        ...
        returns (uint256)
    {
        ...
        uint256 maxLeverage = IAdminFunctions(adminContract).getBaseLtm();
        if(maxLeverage * totalMargin > loanAmount) {
        ...
        returns (uint256)
    {
        ...
        if(source == 1) {
            ...
            (, uint256 interest) = loanToInterest(user, assetAddress);
            if(depositInfo.marginAmount > interest) {
                totalMargin = depositInfo.marginAmount - interest;
            }
            //totalMargin = depositInfo.marginAmount;
    ...
    function repay(
        ...
        // Source = 1  => Repay from external wallet
        if (source == 1) {
            if(repayAmount >= ICErc20Delegator(delegator).borrowBalanceCurrent(msg.sender)) {
                //full repay
                // ----------------------------------TBD -----------------------------------------//
                //if(depositInfo.usedLoan) > 0) => repayamount = repayamount - depositInfo.usedLoan
                require(depositInfo.usedLoan == 0, "Margin: Excess loan repaid");
                depositInfo.repaidLoan += depositInfo.loanAmount;
                depositInfo.marginAmount += depositInfo.loanAmount;
                ...
                //-----------------------------------TBD -------------------------------------//
                require(repayAmount <= depositInfo.loanAmount + depositInfo.interestAccumulated, "Margin: Excess loan repaid");
                uint256 loanAmount;
                ...
                //Overflow checks
                if(loanAmount > depositInfo.loanAmount) {
                    depositInfo.interestAccumulated = depositInfo.interestAccumulated - interestAmount - (loanAmount - depositInfo.loanAmount);
                    depositInfo.marginAmount += depositInfo.loanAmount;
                    depositInfo.loanAmount = 0;
                }
                else {
                    depositInfo.marginAmount += loanAmount;
                    depositInfo.loanAmount -= loanAmount;
            ...
            // If matic 
            if (assetAddress == address(0)) {
                //require((msg.sender).balance >= repayAmount, "Margin: Not enough balance");
        ...
        //using the margin account
            ...
            uint256 maxInterestAmount = maxInterest(msg.sender, assetAddress); 
            // if(repayAmount > maxRepayAmount) {
            //     repayAmount = repayAmount - maxRepayAmount;
            // }
            require(
                depositInfo.marginAmount >= maxInterestAmount,
            ...
            if(repayAmount >= ICErc20Delegator(delegator).borrowBalanceCurrent(msg.sender)) {
                ...
                //full repay
                depositInfo.repaidLoan += depositInfo.loanAmount;
                uint256 interest = repayAmount - depositInfo.loanAmount;
                depositInfo.marginAmount -= interest;
                depositInfo.loanAmount = 0;
                depositInfo.interestAccumulated = 0;
            }
            else {
                ...
                if(loanAmount > depositInfo.loanAmount) {
                    depositInfo.interestAccumulated = depositInfo.interestAccumulated - interestAmount - (loanAmount - depositInfo.loanAmount);
                    depositInfo.marginAmount -= interestAmount;
                    depositInfo.loanAmount = 0;
                }
                else {
                    depositInfo.loanAmount -= loanAmount;
        ...
        emit Repaid(msg.sender, assetAddress, repayAmount);
    }

    function maxInterest(address user, address assetAddress)
        public
        returns (uint256)
    {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
        ...
        //uint256 loanAmount = depositInfo.loanAmount + depositInfo.usedLoan;
        //uint256 maxLeverage = IAdminFunctions(adminContract).getBaseLtm();
        // if(maxLeverage * totalMargin > (loanAmount + interestAmount)) {
        //     uint256 maxRepayAmount = (maxLeverage * totalMargin - (loanAmount + interestAmount)) / (maxLeverage - 1);
        //     return maxRepayAmount;
        // }
        if(totalMargin > interestAmount) {
            uint256 maxRepayAmount = interestAmount;
            return maxRepayAmount;
        }
        return 0;
    }
    ...
    function getAssetUsers(address assetAddress) public view returns(address[] memory) {
        return assetUsers[assetAddress];
        ...
        )
    {
        ...
        (, uint256 interest) = loanToInterest(user, assetAddress);
        if(depositInfo.marginAmount > interest) {
            marginAmount = depositInfo.marginAmount - interest;
        }
        else {
            marginAmount = 0;
        }
    ...
    function loanToInterest(address user, address assetAddress) public returns(uint256, uint256) {
        ...
        uint256 interestAccumulated = compoundBorrow - depositInfo.loanAmount - depositInfo.usedLoan;
    ...
    function liquidateMargin(address user, address assetAddress) external {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
        (, uint256 interest) = loanToInterest(user, assetAddress);
        uint256 marginAmount = depositInfo.marginAmount;
        if(marginAmount <= (interest + 1835591120114139)) { 
            //account is liquidated
            liquidate(user, assetAddress);
         }

    }

    function liquidate(address user, address assetAddress) internal {
        //check if user has any position
        (bool isPosition, uint256 positionId) = IPosition(IAdminFunctions(adminContract).getPositionContract()).checkUserOpenPosition(user, assetAddress);
        if(isPosition == true) {
            //call repay
            repayLoan(assetAddress, user);
        }
        else {
            IPosition(IAdminFunctions(adminContract).getPositionContract()).closePosition(positionId);
        }
        //In case of open position close the most recent open position
    }

    function repayLoan(address assetAddress, address userAddress) internal {
         Deposit storage depositInfo = depositDetails[userAddress][assetAddress];
         (uint256 repayLoanAmount, 
         uint256 repayInterestAmount) = loanToInterest(userAddress, assetAddress);
        depositInfo.interestAccumulated = repayInterestAmount;
        uint256 repayAmount = repayLoanAmount + repayInterestAmount;
        address maximillionContract = IAdminFunctions(adminContract).getMaximillionContract();
        address delegator = IAdminFunctions(adminContract).getDelegatorAddress(assetAddress);
        require(delegator != address(0), "Margin: Not a delegator address");
        //repay the loan
        ...
        depositInfo.loanAmount -= repayLoanAmount;
        depositInfo.interestAccumulated -= repayInterestAmount;
        if(depositInfo.marginAmount <= repayInterestAmount) {
            depositInfo.marginReserve = depositInfo.marginReserve - (repayInterestAmount - depositInfo.marginAmount);
            depositInfo.marginAmount = 0;
        }
        else {
            depositInfo.marginAmount -= repayInterestAmount;
        }
    }
...
contract MarginStorage {
    ...
    mapping(address => address[]) public assetUsers; 

    mapping(address => mapping(address => bool)) public isUserAdded; 
