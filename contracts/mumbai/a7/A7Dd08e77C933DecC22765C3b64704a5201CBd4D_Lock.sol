...
contract Lock is ERC20 {
   ...
   modifier onlyWhitelistedRecipient(address recipient) {
        require(
            whitelistRecipient[recipient],
            "Recipient wallet is not whitelisted"
        );
        _;
    }
    ...
    function mint(address recipient, uint256 amount, string calldata certificateIPFSHash)
        external onlyWhitelistedRecipient(recipient)
    ...
    function burn(uint256 amount) external onlyWhitelistedBurn {
        ...
        for (uint256 i = lotIds.length; i > 0 && remainingAmount > 0; i--) {
            ...
            if (balance > 0) {
                uint256 burnAmount = (balance >= remainingAmount) ? remainingAmount : balance;
    ...
    function transfer(address recipient, uint256 amount) public onlyWhitelistedRecipient(recipient) 
    onlyWhitelistedMintTransfer override returns (bool) {
  ...
  uint256[] memory recipientLotIds;
    if (getLotCount(recipient) > 0) {
        ...
        for (uint256 i = senderLotIds.length; i > 0 && remainingAmount > 0; i--) {
            uint256 lotId = senderLotIds[i - 1];
            uint256 senderBalance = walletTokenBalances[msg.sender][lotId];

            if (senderBalance > 0) {
                uint256 transferAmount = (senderBalance >= remainingAmount) ? remainingAmount : senderBalance;
        ...
        for (uint256 i = recipientLotIds.length; i > 0 && remainingAmount > 0; i--) {
            uint256 lotId = recipientLotIds[i - 1];
            uint256 recipientBalance = walletTokenBalances[recipient][lotId];

            if (recipientBalance > 0) {
                uint256 transferAmount = (recipientBalance >= remainingAmount) ? remainingAmount : recipientBalance;
                walletTokenBalances[recipient][lotId] -= transferAmount;
                walletTokenBalances[msg.sender][lotId] += transferAmount;
                remainingAmount -= transferAmount;
            }
    ...
    function balanceOfCertificate(address account, uint256 lotId) public view returns (uint256) {
   ...
   function addToWhitelistRecipient(address wallet) external onlyOwner {
    ...
    function getLotDetails(uint256 lotId) public view returns (uint256, string memory) {
   ...
   function getSortedLotIds(address wallet) private view returns (uint256[] memory) {
