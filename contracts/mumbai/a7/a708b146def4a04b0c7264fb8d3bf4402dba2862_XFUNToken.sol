  ...
  IEscrowCapable
{
  ...
  /// @dev user address => escrow contract address => allowed true/false
  mapping(address => mapping(address => bool)) public userAllowedEscrows;
  ...
  // @dev initialize() is required by the upgradable proxy patter
  function initialize(
    string memory name,
   ...
   * - the sender needs to have PAUSER_ROLE
   */
  ...
  /**
   * @dev Grants `Minter` to `_minter`
   *
   * @param _minter Minter address
   *
   * See - {AccessControlUpgradeable - _grantRole }
   *
   * Requirements:
   *
  ...
  function grantMinter(address _minter) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_minter != address(0), "_minter can't be zero address");
    _grantRole(MINTER_ROLE, _minter);
  }

  /**
   * @dev Revokes `Minter` to `_minter`
   *
   * @param _minter Minter address
   *
   * See - {AccessControlUpgradeable - _grantRole }
  ...
  /**
   * @notice Returns true if escrow contract is allowed
   * @param _escrow address of escrow contract
   */
  function _isEscrowAllowed(address _escrow) internal view returns (bool) {
    require(_escrow != address(0), "Escrow is the zero address");
    return allowedEscrows[_escrow];
  }

  /**
   * @notice returns true if the escrow contract is enabled by the  user
   * @param _user User address
   * @param _escrow address of escrow contract
   */
  function _userHasEscrow(address _user, address _escrow)
    internal
    view
    returns (bool)
  {
    require(_user != address(0), "User is the zero address");
    return allowedEscrows[_escrow] && userAllowedEscrows[_user][_escrow];
  }

  /**
   * @notice Lets admin add the escrow contract
   * @param _escrow address of escrow
   */
  function allowEscrow(address _escrow) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(!_isEscrowAllowed(_escrow), "This escrow is already allowed");
    allowedEscrows[_escrow] = true;
  }

  /**
   * @notice Lets admin remove the escrow contract
   * @param _escrow address of escrow
   */
  function removeEscrow(address _escrow) external onlyRole(DEFAULT_ADMIN_ROLE) {
    require(_isEscrowAllowed(_escrow), "This escrow is not allowed");
    allowedEscrows[_escrow] = false;
  }

  /**
   * @notice Lets user enable the escrow contract
   * @param _escrow address of escrow
   */
  function enableEscrow(address _escrow) external {
    require(_isEscrowAllowed(_escrow), "This escrow is not allowed");
    require(
      !_userHasEscrow(_msgSender(), _escrow),
      "This escrow was already enabled by user"
    );
    userAllowedEscrows[_msgSender()][_escrow] = true;
  }

  /**
   * @notice Lets user disable the escrow contract
   * @param _escrow address of escrow
   */
  function disableEscrow(address _escrow) external {
    ...
    require(
      _userHasEscrow(_msgSender(), _escrow),
      "This escrow was not enabled by user"
    );
  ...
  function escrowFrom(address _from, uint256 _value) external returns (bool) {
    require(
      _userHasEscrow(_from, _msgSender()),
      "This escrow was not enabled by user"
    );
    _transfer(_from, _msgSender(), _value);
    return true;
  }
  ...
  function escrowReturn(
    ...
    require(
      ...
      "The sender is not an escrow account."
    );
    require(_to != address(0) && _value > 0, "Invalid receiver or amount");
    if (_fee > 0) {
      //Decrease the total supply and escrow balance when _fee is bigger than 0
      _burn(_msgSender(), _fee);
    }
  ...
  {
    return "2.2.0+opengsn.recipient.irelayrecipient";
  }
