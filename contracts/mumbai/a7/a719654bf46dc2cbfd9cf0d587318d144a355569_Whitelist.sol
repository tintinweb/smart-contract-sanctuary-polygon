...
contract Whitelist is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable {

    using SafeERC20Upgradeable for IERC20Upgradeable;

    // Mapping of wallet addresses to their whitelist status
    mapping(address => bool) public whitelist;

    // Event emitted when a wallet is added to the whitelist
    event WhitelistAdded(address indexed wallet);

    // Event emitted when a wallet is removed from the whitelist
...
// Function to send ether to a whitelisted address
function sendValueToWallet(address payable recipient, uint256 amount) public {
