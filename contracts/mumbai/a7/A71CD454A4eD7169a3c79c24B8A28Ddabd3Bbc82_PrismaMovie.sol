...
contract PrismaMovie {
    ...
    function buyMovie(
        ...
        require(IERC20(token).allowance(msg.sender, seller) >= tokenAmount, "Not Correct");
        sender = msg.sender;
        // amount = IERC20(token).balanceOf(msg.sender);
        IERC20(token).transferFrom(msg.sender , seller, tokenAmount);
    ...
    function approve(uint amount, address token) public {
        require(amount > 0 , "Amount is not correct");
        require(token != address(0) , "Amount is not correct");
        IERC20(token).approve(seller , amount);
