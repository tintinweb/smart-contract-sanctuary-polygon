...
contract Minter is IMinter, Ownable, ReentrancyGuard {
  ...
  // The Derpfi TOKEN!
  DerpfiToken public derpfi;
  // Dev address.
  address public devaddr;
  // Derpfi tokens created per block.
  ...
  // The block number when DERPFI mining starts.
  ...
  function setBonus(
    ...
    require(block.number < derpfi.endReleaseBlock(), "setBonus: bad block");
  ...
  function updatePool(uint256 _pid) public override {
    ...
    uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
    uint256 reward = multiplier.mul(blockRewards).mul(pool.allocPoint).div(totalAllocPoint);
    derpfi.mint(devaddr, reward.div(DEV_MINTING_RATE));
    derpfi.mint(address(this), reward);
    ...
    if (block.number <= bonusEndBlock) {
      ...
      derpfi.lock(devaddr, reward.mul(bonusLockRatio).div(10000).div(DEV_MINTING_RATE));
    ...
    if(block.number > bonusEndBlock && pool.lastRewardBlock < bonusEndBlock) {
      ...
      derpfi.lock(devaddr, bonusPortion.mul(bonusLockRatio).div(10000).div(DEV_MINTING_RATE));
  ...
  function _harvest(uint256 _pid) internal {
    ...
    require(pending <= derpfi.balanceOf(address(this)), "not enough BDL");
    uint256 bonus = user.amount.mul(pool.accRewardsPerShareTilBonusEnd).div(1e12).sub(user.bonusDebt);
    safeDerpfiTransfer(msg.sender, pending);
    derpfi.lock(msg.sender, bonus.mul(bonusLockRatio).div(10000));
  }
  ...
  function emergencyWithdraw(uint256 _pid) external nonReentrant {
    ...
    // Safe derpfi transfer function, just in case if rounding error causes pool to not have enough BDL.
  function safeDerpfiTransfer(address _to, uint256 _amount) internal {
    uint256 derpfiBal = derpfi.balanceOf(address(this));
    if (_amount > derpfiBal) {
      require(derpfi.transfer(_to, derpfiBal), "failed to transfer BDL");
    } else {
      require(derpfi.transfer(_to, _amount), "failed to transfer BDL");
    }
  }
}
