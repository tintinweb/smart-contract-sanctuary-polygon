...
contract PriceOracle is AdminExecutorAccess, MarketErrors {
    struct PriceUpdate {
        address market;
        uint256[] prices;
    }
    ...
    constructor(address admin) AdminExecutorAccess(admin) { }

    /// @notice Update many market's fair prices in one transaction
    function batchUpdateFairPrices(PriceUpdate[] calldata priceUpdates) external onlyExecutor whenNotPaused {
        for (uint256 i = 0; i < priceUpdates.length; ++i) {
            IUpdateFairPrices(priceUpdates[i].market).updateFairPrices(priceUpdates[i].prices);
        }
    }
}
