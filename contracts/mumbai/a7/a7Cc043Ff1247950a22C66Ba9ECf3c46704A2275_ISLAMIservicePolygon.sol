...
*/
    ...
    constructor(address _feeReceiver, address _developers, ERC20 _ISLAMI) {
        ISLAMI = _ISLAMI;
    ...
    function hasLockedTokens2(uint256 _ID, address _token, address _investor)public view returns(bool){
        if(investor[_investor][_ID].tokenAddress == ERC20(_token)){
            return true;
        }
        else{
            return false;
...
*/
    function useRecovery(uint256 _vaultID, address _investor) internal {
        ...
        uint256 feeToPay = investor[_investor][_vaultID].amount.mul(ewFee).div(200);
        uint256 feeToDev = investor[_investor][_vaultID].amount.mul(ewFee).div(200);
