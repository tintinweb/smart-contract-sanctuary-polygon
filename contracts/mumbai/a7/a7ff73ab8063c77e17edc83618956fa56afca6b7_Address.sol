...
contract MAPE is ERC721A, Ownable {
    ...
    uint256 public constant maxBatchMintSize = 7888;
    uint256 private constant reservedAmount = 7888;

    constructor() ERC721A("MAPE", "MAPE", maxBatchMintSize) {}

    // uint256 public NFTPrice = 130 ether;
    // uint256 public preSaleNFTPrice = 99 ether;
    uint256 public NFTPrice = 0.0002 ether;
    uint256 public preSaleNFTPrice = 0.0001 ether;
    bool public reveal;
    ...
    */
    ...
    function mintNFT(uint256 amount, bytes32[] memory _merkleProofOGALOne, bytes32[] memory _merkleProofOGALTwo, bytes32[] memory _merkleProofOGALThr, bytes32[] memory _merkleProofAL, uint indexOGALOne, uint indexOGALTwo, uint indexOGALThr, uint indexAL) public payable {
        ...
        if(teamMembers[msg.sender]) {
            ...
            bool _OGALOne =  verify(_merkleProofOGALOne, merkleRootOGALOne, leaf, indexOGALOne);
            bool _OGALTwo =  verify(_merkleProofOGALTwo, merkleRootOGALTwo, leaf, indexOGALTwo);
            bool _OGALThr =  verify(_merkleProofOGALThr, merkleRootOGALThr, leaf, indexOGALThr);
            bool _AL =  verify(_merkleProofAL, merkleRootAL, leaf, indexAL);

            require(_OGALOne || _OGALTwo || _OGALThr || _AL, "Not Allowed user for PreSale");

            for (uint256 i = 1; i <= amount; i++) {
                if(_OGALOne && tokenAmountsPerAddy[msg.sender] >= 0) {
                    require(msg.value > preSaleNFTPrice * (amount - i), "Payment required for additional NFT(s)");
                }
                ...
                if(!_OGALOne && !_OGALTwo && !_OGALThr && _AL) {
                    require(msg.value > preSaleNFTPrice * amount, "Require Pay for AL Mint");
                }

                _safeMint(msg.sender, 1);
                tokenAmountsPerAddy[msg.sender] += 1;
            }
        } else if(_publicSale) {
            require(msg.value > NFTPrice * amount, "Insufficient Funds...");
            _safeMint(msg.sender, amount);
        }
    }
