...
contract PolyKicktNFT is ERC721 {
    ...
    uint256 public constant SPECIAL_NFT_PROBABILITY_DIVISOR = 6;
    ...
    function tokenDetails(address _holder)
        public
        view
        returns (
            uint256 _tokenId,
    ...
    {
        return (
            details[_holder].tokenId,
            details[_holder].userWhitelistedIlos,
            details[_holder].typeNFT,
            details[_holder].cat
        );
    }

    function useNFT(address _holder) external onlyAdmin {
        require(details[_holder].userWhitelistedIlos != 0, "No NFT found");
        details[_holder].userWhitelistedIlos -= 1;
        if (
            details[_holder].userWhitelistedIlos == 0 &&
            details[_holder].cat == 1
        ) {
            _burn(details[_holder].tokenId);
        }
    }
    ...
    function mint(address _to) public onlyAdmin {
        // require(balanceOf(_to) == 0, "already minted for user");
        // require(didTransfer[_to] != true, "User transfered their NFT");
        if (balanceOf(_to) != 0) {
            emit notMinted("User already have NFT");
            return;
        }
        ...
        if (
            ...
            uint256(keccak256(abi.encodePacked(_to, totalNftsMinted))) %
                ((NUM_SPECIAL_NFTS + totalNftsMinted) *
                    SPECIAL_NFT_PROBABILITY_DIVISOR) <
            NUM_SPECIAL_NFTS - specialNftsMinted
        ) {
    ...
    function mintRare(address _to, uint256 _numWhitelistedIlos)
        external
        onlyOwner
    {
        require(
            rareNftsMinted < rareNFT,
            "Maximum number of rare NFTs has been reached"
        );
        string memory typeNFT;
        uint256 tokenId = totalNftsMinted;
        typeNFT = string(
            abi.encodePacked("Rare Polykick NFT", uint2str(tokenId))
        );
    ...
    function transfer(address _to) external {
        uint256 _tokenId = details[msg.sender].tokenId;
        require(
            _isApprovedOrOwner(_msgSender(), _tokenId),
            "ERC721: transfer caller is not owner nor approved"
        );
        require(balanceOf(_to) == 0, "only one NFT can be owned");
        _transfer(_msgSender(), _to, _tokenId);
        details[_to].userWhitelistedIlos = details[msg.sender]
            .userWhitelistedIlos;
        details[_to].typeNFT = details[msg.sender].typeNFT;
        details[_to].tokenId = _tokenId;

        didTransfer[msg.sender] = true;

        delete details[msg.sender];
    }

    function canParticipate(address _user, uint256 _numWhitelistedIlos)
        external
        view
        returns (bool)
    {
    ...
    function addToWhiteListBulkFromNFT() external {
        ...
        for (uint256 batchIndex = 0; batchIndex < numBatches; batchIndex++) {
            ...
            address[] memory usersToAddToWhiteList = new address[](
                endIndex - startIndex
            );
