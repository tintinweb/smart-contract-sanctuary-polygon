...
contract Marketplace is Ownable{
    ...
    function renewRental(
        Order memory sellOrder, //original rental id
        uint rentalId,
        uint numberOfRentals,
        uint128 startTime, 
        uint128 endTime
    ) external payable{
         //get pdr from renter id
        //  uint pdrId = IProperty(collectionAddress).getPdrFromRental(rentalId);
        // sellOrder.maker, msg.sender, sellOrder.rentalId, numberOfRentals, startTime, endTime
        ...
        doPaymentTransfer(msg.sender, totalPayment, sellOrder.maker, sellOrder.payment, platformPayment, true);
        IProperty(collectionAddress).renewRental(sellOrder.maker, msg.sender, rentalId, numberOfRentals, startTime, endTime);
    }
    ...
    function doRentalTransfer(
        ...
        IProperty(collectionAddress).safeTransferRental(from, msg.sender, rentalId, numberOfTokens, startTime, endTime);
    }
