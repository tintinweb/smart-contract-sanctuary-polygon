...
abstract contract Equippable is GameRegistryConsumerUpgradeable, IEquippable {
    ...
    function _getItems(
        ...
        // Pull current slots from trait provider
        if (!traitsProvider.hasTrait(tokenContract, tokenId, slotType)) {
            // Initialize currentSlots if nonexistent
            return new uint256[](getSlotCount(parentEntity, slotType));
        }
        return
