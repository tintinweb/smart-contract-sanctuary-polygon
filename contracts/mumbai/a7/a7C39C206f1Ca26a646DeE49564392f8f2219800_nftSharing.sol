...
contract nftSharing {
    constructor(address _routerContractAddress,uint _minLiquidityEthAmount,uint _initLiquidityTokenAmount,uint _feeTokenAmount) {
        owner = msg.sender;
        feeTo = msg.sender;

        routerContractAddress = _routerContractAddress;
        minLiquidityEthAmount = _minLiquidityEthAmount; // 10*(10**18) minimum 10 ETH
        initLiquidityTokenAmount = _initLiquidityTokenAmount; // 100000*(10**6) minimum 10% of all tokens
        feeTokenAmount = _feeTokenAmount; // 50000*(10**6) minimum 5% of all tokens
    }
    ...
    uint public initLiquidityTokenAmount;
    function setInitLiquidityTokenAmount(uint _initLiquidityTokenAmount) onlyOwner public {
        initLiquidityTokenAmount = _initLiquidityTokenAmount;
    }
    ...
    uint public feeTokenAmount;
    function setFeeTokenAmount(uint _feeTokenAmount) onlyOwner public {
    ...
    //deposit nft
    function depositNft(address _nftAddress,uint _nftId) public payable returns(address){
        ...
        nftShareContract.approve(routerContractAddress,initLiquidityTokenAmount);
        ...
        routerContract.addLiquidityETH{value:msg.value}(
            ...
            initLiquidityTokenAmount, // amountTokenDesired,
            0, // amountTokenMin,
        ...
        nftTokenContract.transfer(feeTo,feeTokenAmount);
        nftTokenContract.transfer(msg.sender,nftTokenContract.balanceOf(address(this)));

        return nftShareContractAddress;
    }
    ...
    {
        nftShareToken c = new nftShareToken(owner,_nftAddress,_nftId);
