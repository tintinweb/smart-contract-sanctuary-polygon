...
contract Voting {
     ...
     */
    function castVote(bool _decision, uint _proposalId, address _account) external {
        require(msg.sender == _account, "please connect using same address");
        checkBal[_account] = _account.balance;
        if (_account.balance  < MIN_TOK_VOTE) {
            revert notEnoughBalanceToCastVote();
        }
        if (userStatus[_proposalId][_account]) {
            revert alreadyVoted();
        }
        if (
            ...
            calculateWeightedAvg[_proposalId].total_weight += _account.balance;
            if (_decision) {
                votedFor[_proposalId] = votedFor[_proposalId] + 1;
                voteState[_proposalId][_account].voteStatus = true;
                voteState[_proposalId][_account].timestamp = block.timestamp;
                onlyForVotes[_proposalId].push(forVotedData(_account, block.timestamp, _account.balance));
                votedYes[_proposalId].push(_account);
                calculateWeightedAvg[_proposalId].sumOfFor += _account.balance;
                emit Voted(_account, _proposalId, _decision);
            } else {
                votesAgainst[_proposalId] = votesAgainst[_proposalId] + 1;
                voteState[_proposalId][_account].voteStatus = false;
                voteState[_proposalId][_account].timestamp = block.timestamp;
                onlyAgainstVotes[_proposalId].push(againstVotedData(_account, block.timestamp, _account.balance));
                votedNo[_proposalId].push(_account);
                calculateWeightedAvg[_proposalId].sumOfAgainst += _account.balance;
                emit Voted(_account, _proposalId, false);
            }
