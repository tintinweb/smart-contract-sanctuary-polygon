...
contract CharityAuction is ERC721URIStorage, Pausable {
    ...
    function mintItem(string memory _tokenuri, uint256 _startingPrice) public {
    ...
    function createAuction(uint256 tokenId, uint256 _duration) public {
    ...
    function bidStart(uint256 tokenId) public payable {
        require(
            state[tokenId] == State.CreateAuction ||
    ...
    function releaseFund(uint256 tokenId) public payable {
    ...
    function viewallitems() public view returns (DonatedItem[] memory) {
        uint256 length = _tokenIdCounter.current();
        DonatedItem[] memory tempitem = new DonatedItem[](length);
        for (uint i = 0; i < length; i++) {
            tempitem[i] = idtodonar[i];
        }
        return tempitem;
    }
}