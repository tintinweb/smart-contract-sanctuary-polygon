...
contract TokenSale is Initializable, ITokenSale {
    ...
    function initialize(
        ...
        admin = IAdmin(_admin);
        //check if totalSupplyInValue is in usdc
        state.totalSupplyInValue = uint128(
            (uint256(_params.totalSupply) *
                uint256(_params.privateTokenPrice)) / 10**6
        );
        usdc = IERC20D(0xa36EF00CCC73f43dd83E38AEba806C6EFB2BcD6F); //TODO change this for mainnet
    }

  // allocation is amount
        ...
        public
    {
        ...
        require(users.length == allocations.length, "TokenSale: Invalid length");
    ...
    function _processPrivate(address _sender, uint256 _amount) internal {
        ...
        uint maxInFiat = stakingContract.getAllocationOf(_sender) > tokensaleTiers[_sender] ? 
                            stakingContract.getAllocationOf(_sender) : tokensaleTiers[_sender];

        require( maxInFiat > 0, "TokenSale: No allocation");
        ...
        usdc.safeTransferFrom(_sender, address(this), add);
    ...
    function takeUSDCRaised() external override {
        ...
        // for this totalSupplyInValue should be in usdc
