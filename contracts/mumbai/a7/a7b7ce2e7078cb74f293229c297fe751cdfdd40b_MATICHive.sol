...
import "./IMATICHiveVault.sol";

contract MATICHive is Ownable {
    ...
    uint256 public MAX_DEPOSIT_STEP = 200 ether;
    ...
    event Withdraw(address indexed _address, uint256 honeyAmount, uint256 maticAmount);
    ...
    function getMaxDeposit(address _address) public view returns (uint256) {
        ...
        uint256 maxDeposit = min(maxDepositSinceInitialisation, 100000 ether);
    ...
    function reinvest() public whenInitialized {
        ...
        if (notClaimedHoney > 0) {
            IMATICHiveVault(vaultAddress).fundHive(notClaimedHoney.div(RATIO_MULTIPLIER));
        }
    }
    ...
    function withdraw() external whenInitialized {
        ...
        uint256 maticValue = calculateHoneySell(hasHoney);

        require(getBalance() >= maticValue, "NOT ENOUGH BALANCE");
        ...
        maticValue = maticValue.sub(distributeDevFees(maticValue));

        payable (msg.sender).transfer(maticValue);

        emit Withdraw(msg.sender, hasHoney, maticValue);
    }
    ...
    function maticRewards(address _address) external view returns(uint256) {
    ...
    function extraFund(uint256 _amount) external onlyOwner {
        IMATICHiveVault(vaultAddress).fundHive(_amount);
    }
}
