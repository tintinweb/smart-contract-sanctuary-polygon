...
contract ClassicHomelessSociety is Ownable, ReentrancyGuard, IERC721, IERC721Metadata, IERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;
    using EnumerableSet for EnumerableSet.UintSet;

    string private constant _name = "ClassicHomelessSociety";
    string private constant _symbol = "CHS";
    ...
    address payable internal immutable _wallet2;
    ...
    uint internal constant maxPerTx = 20;
    ...
    constructor(string memory baseURI_, address payable wallet1, address payable wallet2) {
    ...
    function getMintPrice(uint256 amount) public view returns (uint256 price) {
        ...
        for (uint256 i = 1; i <= amount; i++) {
            if(currentSupply + i > maxPerTx) {
                price += mintingLevelPrices[1];
            } else {
                price += mintingLevelPrices[0];
            }
        }
    }

    function mint(uint256 amount) external payable nonReentrant mintingEnabled {
        require(amount >= 1 && amount <= maxPerTx, "amountLimit");
        ...
        uint256 comissionOne = nftPrice * 20 / 100;
       
        (bool sentOne, ) = _wallet1.call{ value: comissionOne }("");
        require(sentOne, "comissionOne failed");

        uint256 comissionTwo = nftPrice - comissionOne;
      
        (bool sentTwo, ) = _wallet2.call{ value: comissionTwo }("");
        require(sentTwo, "comissionTwo failed");
