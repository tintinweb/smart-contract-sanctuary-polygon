...
contract OHtestV8 is ERC721URIStorage, Ownable, ERC721Enumerable {
    
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    
    uint256 private price = 200000000000000000 ; // 0.2 Ether
    //structure for containing reveal requests
    struct RevealData {
        uint256 tokenid;
        string tokenuri;
    }
    ...
    function findAddress(address _address, address[] storage arr) internal view returns(int) 
    {
    ...
    constructor() ERC721("OHTest 7", "OHT") {
    ...
    {
        int superAdminIndex = findAddress(adminAddress,superAdmins);
        if(superAdminIndex != -1){
            remove(uint256(superAdminIndex),superAdmins);
        }
        int adminIndex = findAddress(adminAddress,admins);
        if(adminIndex != -1){
    ...
    {
        require(verifySuperAdmin(adminAddress), "You need to have admin priviliges");
        _;
    } 
    ...
    function addAdmins(address[] memory adminAddresses) public isSuperAdmin(msg.sender) 
    {
        if(adminAddresses.length >0)
        {
            for(uint256 i = 0; i < adminAddresses.length; i++)
    ...
    function addUsers(address[] memory _WhitelistedAddresses) public  isAdmin(msg.sender)
    {
    ...
    function deleteUser(address _whitelistedAddress) public isAdmin(msg.sender) 
    {
        int whitelistIndex = findAddress(_whitelistedAddress,whitelist);
        if(whitelistIndex != -1){
    ...
    require(verifyUser(msg.sender)|| isSalePublic, "You need to be whitelisted or wait for the minting to go public");
    require(msg.value >= price, "Not enough funds sent.");
    require(isSaleActive, "Sale is not active" );
    require(totalSupply() < maxSupply, "All tokens are minted!");
    _tokenIds.increment();
    _safeMint(msg.sender, tokenId);
    //Set image to pre-reveal preview
    _setTokenURI(tokenId, "https://ipfs.moralis.io:2053/ipfs/QmdXat79QVNY63fR5Jf2ceAXj56FsKskvtXWPeMvrhcfvp/OH/Preview.json");
    return tokenId;
    }   
     
    function updateURI(RevealData memory data) public isSuperAdmin(msg.sender)  {
        _setTokenURI(data.tokenid, data.tokenuri);
    }  
    function updateURIs(RevealData[] memory data) public isSuperAdmin(msg.sender) 
    {
        if(data.length > 0){
    ...
    {
        newOwner = 0x72e1bA7813a267377c8750d9C0b58856bDC5Ef1e;
