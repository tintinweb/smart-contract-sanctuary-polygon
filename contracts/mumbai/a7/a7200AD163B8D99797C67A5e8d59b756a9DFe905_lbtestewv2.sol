...
contract lbtestewv2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
    ...
    function getJackpotinUSD() public view returns (uint256) {
       if(prizeValueJackpot > 0){

        uint256 tprice = getLatestPrice();
        uint256 jackpot = prizeValueJackpot - (prizeValueJackpot.mul(percentageToNFTs).div(1000)); 
    ...
    function getJackpot() public view returns (uint256) {
       if(prizeValueJackpot > 0){
        return prizeValueJackpot - (prizeValueJackpot.mul(percentageToNFTs).div(1000));
       } else{
        return 0;
       }
    } 
    ...
    function autoSpin(address _caller, uint8[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,1) == false){
            ...
            require(block.timestamp > lastAutoSpin + 4800, "draw timestamp still not reached");   // 604800 1 week > 605100 1 week+5mins
            ...
            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length > 0){ 
                ...
                if(block.timestamp > lastAutoSpin + 5100 && reset == false){   // if no node activates in 5 minutes period queue resets
     ...
     */
     function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        eth.safeTransfer(userWallet,prizeValueJackpot);
