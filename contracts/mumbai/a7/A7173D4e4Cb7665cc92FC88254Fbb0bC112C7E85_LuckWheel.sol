...
contract LuckWheel is Initializable {  
  ...
  mapping (address=>uint) private _accountTimestamp;
  mapping (address=>uint) private _accountCount;

  function checkin() external {
    require(block.timestamp - _accountTimestamp[msg.sender] >= 1 days, 'LuckWheel: already checked in');
    _accountCount[msg.sender] += 1;
    _accountTimestamp[msg.sender] = block.timestamp;
  }
