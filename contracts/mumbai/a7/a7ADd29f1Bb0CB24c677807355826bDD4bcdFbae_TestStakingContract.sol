// STAKING TEST CONTRACT
// SIGNIFICANTLY LOWERED STAKING TIMES 1 DAY = 1 SEC
// ONLY FOR TESTNET DEPLOYMENT
...
contract TestStakingContract is Ownable, ReentrancyGuard {
    ...
    uint256 constant internal durationMonth = 30 seconds;
    uint256 constant internal duration2Months = 60 seconds;
    uint256 constant internal durationYear = 365 seconds;
    ...
    function getDepositInfo(address account, uint256 index) public view returns (uint256 amount, uint256 startTime, StakingPeriod period, uint256 rewardsAmount) {
        require(account != address(0), "Zero address");
    ...
    function createDeposit(uint256 amount, StakingPeriod stakingPeriod) external nonReentrant returns(uint256) {
        ...
        Deposit memory deposit = Deposit(amount, block.timestamp, stakingPeriod, 0);
    ...
    function calculateRewardsAmount(Deposit memory deposit) internal view returns (uint256 amount) {
        ...
        uint stakingDays = (block.timestamp - deposit.startTime) / 1 seconds;
        ...
        if (reachedPeriod == StakingPeriod.StakingPeriodInvalid) {
            ...
            amount = (deposit.amount * stakingRewardPercents[uint(reachedPeriod)] * stakingDays) / (getDurationByStakingPeriod(reachedPeriod) / 1 seconds) / 100;
