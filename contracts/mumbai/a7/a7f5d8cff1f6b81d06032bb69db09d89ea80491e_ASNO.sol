    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    function mintGoldBatchAndTransfer(address[] memory to,uint256[] memory _count) external {
        // require(adminRole[msg.sender] == true, "Only Admin");
        require(to.length == _count.length,'Please check count of address and amount');
        for (uint256 i; i < to.length; ++i) {
            for(uint256 j;j<_count[i];i++){
