...
contract RevenueShareForDonation is AccessControl, ReentrancyGuard{
  ...
  event RequestAdded(uint256 receiveId, bool isWETH, uint tokenId, uint256 amount, uint256 materialId);
  ...
  function addRequest(uint256 _receiveId, uint tokenId) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    Request memory request = Request(isWETH, tokenId, payment.amount);
    receiveIdToRequests[_receiveId] = request;
    if (isWETH) {
        claimablePerMaterial[materialId].amountWETH += payment.amount;
    } else {
        claimablePerMaterial[materialId].amountETH += payment.amount;
    }
    emit RequestAdded(_receiveId, isWETH, tokenId, payment.amount, materialId);
  }
  ...
  function batchWithdraw() external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    ...
    // to get the wallet list: receiveIdToRequests[requestId(itterable)].members
