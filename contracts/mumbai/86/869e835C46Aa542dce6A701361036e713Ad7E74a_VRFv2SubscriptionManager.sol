...
contract VRFv2SubscriptionManager is VRFConsumerBaseV2 {
  ...
  uint256 counter;
  mapping(uint256 => uint256) indexToRequestId;
  ...
  function getRequestId(uint256 index) external view returns(uint256) {
    return indexToRequestId[index];
  }
  // Assumes the subscription is funded sufficiently.
  function requestRandomWords(uint32 _numWords, bytes32 _keyHash, uint32 _callbackGasLimit, uint16 _requestConfirmations ) external returns(uint256){
    ...
    indexToRequestId[counter] = requestId;
    counter += 1;
