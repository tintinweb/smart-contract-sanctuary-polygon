...
contract InstallmentNFT is Initializable, UUPSUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable,ERC721Holder {
    ...
    */
    modifier whenInstallmentNotPaused() {
        require(!_paused, "[InstallmentNFT.whenInstallmentNotPaused] Not Paused");
        _;
    }
    ...
    */
    modifier whenInstallmentPaused() {
        require(_paused, "[InstallmentNFT.whenInstallmentPaused] Paused");
        _;
    }
    /**
    ...
    function executeOrderInstallmentNFT(address _sellerAccount, bytes32 _orderId , uint256 _period) external whenInstallmentNotPaused{
        ...
        require(order.id != 0, "[InstallmentNFT.executeOrderInstallmentNFT] Order not found.");
        require(order.seller != msg.sender,"[InstallmentNFT.executeOrderInstallmentNFT] Can't buy this order.");
        require(_period % 3 == 0 &&  _period >= 6 && _period <= 24, "[InstallmentNFT.executeOrderInstallmentNFT] Period must be more than or equal to 6 and mod 3 = 0");
    ...
    function payBillInstallmentNFT(bytes32 _billId , uint256 _period) external whenInstallmentNotPaused {
        ...
        require(bill.billId != 0, "[InstallmentNFT.payBillInstallmentNFT] Bill not found.");
        require(_period != 0 , "[InstallmentNFT.payBillInstallmentNFT] Period is not correct");
        require(bill.periodBalance >=  _period , "[InstallmentNFT.payBillInstallmentNFT] Period is not correct");
        require(bill.buyer ==  msg.sender, "[InstallmentNFT.payBillInstallmentNFT] Only buyer of installment contract can pay the bill");
        uint totalPay;
        IERC20Upgradeable token = IERC20Upgradeable(nakaTokenAddress);
    ...
    function cancelOrderInstallmentNFT(bytes32 _orderId) external  whenInstallmentNotPaused{
        ...
        require(order.id != 0, "[InstallmentNFT.cancelOrderInstallmentNFT] Order not found.");
        require(order.seller == msg.sender, "[InstallmentNFT.cancelOrderInstallmentNFT] Unauthorized user.");
    ...
    function cancelBillInstallmentNFT(address _buyerAccount ,bytes32 _billId) external onlyRole(WORKER_ROLE) whenInstallmentNotPaused {
        ...
        require(bill.billId != 0, "[InstallmentNFT.cancelBillInstallmentNFT] Bill not found.");
