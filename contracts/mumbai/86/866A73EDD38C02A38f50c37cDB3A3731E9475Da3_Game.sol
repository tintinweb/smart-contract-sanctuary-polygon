...
contract Game is OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    event Shot(uint256 number, address account, uint256 reward, uint256[12] positions, uint256 requestId);
    ...
    function shoot() external nonReentrant whenNotPaused {
        ...
        emit Shot(playNumbers, msg.sender, userReward, positions, requestId);
    }

    function calPrice(uint256[12] memory positions) public view returns (uint256) {
        uint256 value;
        for (uint256 i = 0; i < 12; i++) {
