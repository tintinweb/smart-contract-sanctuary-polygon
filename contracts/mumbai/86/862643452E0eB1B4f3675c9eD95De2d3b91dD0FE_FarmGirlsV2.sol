...
import "./interfaces/IFarmGirls.sol";
import "./interfaces/IMetaFarmerette.sol";
...
/// @author Howdy Games
/// @title Just some honest farm girls
contract FarmGirlsV2 is
	IFarmGirls,
	...
	ERC1155SupplyUpgradeable
{
	...
	uint256 public constant MAX_FARM_GIRL_SUPPLY = 3000;
	uint256 public constant WEDDING_RING_ID = 0;

	bool public breedingIsLive;
	CountersUpgradeable.Counter private numberOfFarmGirlsBreeded;
	uint256 public weddingRingCost;
	uint256 public numberOfWeddingRingsLeft;
		...
		initializer
	{
		...
		name = "Howdy Games Farm Girls";
		symbol = "HFG";
		contractURI = _contractURI;
		weddingRingCost = 10**18 * 750;

		_setRegistryFarmer(_registryFarmerV2);
	}

	modifier isLive() {
		require(breedingIsLive, "Breeding is not live yet");
		_;
	}

	modifier isFarmerHolder(uint256 husbandId, uint256 bestManId) {
		...
		bool isHusbandHolder = honestFarmerClub.balanceOf(
			msg.sender,
			husbandId
		) ==
			1 ||
			theFarm.getDepositor(husbandId) == msg.sender;
		bool isBestmanHolder = honestFarmerClub.balanceOf(
			msg.sender,
			bestManId
		) ==
			1 ||
			theFarm.getDepositor(bestManId) == msg.sender;

		require(
			isHusbandHolder && isBestmanHolder && husbandId != bestManId,
			"Must own husband and best man"
		);
		_;
	}

	modifier isValidTokenPrice() {
		require(weddingRingCost > 0, "Token price must be greater than 0");
		_;
	}
	...
	function mintWeddingRing(
		...
		uint256 husbandId,
		uint256 bestManId
	) public isLive isValidTokenPrice isFarmerHolder(husbandId, bestManId) {
		require(
			numberOfWeddingRingsLeft >= numberOfWeddingRings,
			"Not enough wedding rings left"
		);

		uint256 totalPotatoCost = numberOfWeddingRings * weddingRingCost;
		IPotato potato = IPotato(_getRegistryFarmer().getContract("Potato"));

		potato.burnFrom(msg.sender, totalPotatoCost);
		_mint(msg.sender, WEDDING_RING_ID, numberOfWeddingRings, "");

		numberOfWeddingRingsLeft -= numberOfWeddingRings;

		emit MintWeddingRings(
			msg.sender,
			husbandId,
			bestManId,
			numberOfWeddingRings,
			totalPotatoCost
		);
	}

	function breedFarmGirls(uint256 numberOfFarmGirls) public isLive {
		uint256 newFarmGirlSupply = numberOfFarmGirlsBreeded.current() +
			numberOfFarmGirls;
		require(
			newFarmGirlSupply <= MAX_FARM_GIRL_SUPPLY,
			"Breed would exceed MAX_FARM_GIRL_SUPPLY"
		);

		uint256[] memory ids = new uint256[](numberOfFarmGirls);
		uint256[] memory amounts = new uint256[](numberOfFarmGirls);

		for (uint256 i = 0; i < numberOfFarmGirls; i++) {
			numberOfFarmGirlsBreeded.increment();

			uint256 id = numberOfFarmGirlsBreeded.current();

			ids[i] = id;
			amounts[i] = 1;

			emit BreedFarmGirl(msg.sender, id);
		}

		_mintBatch(msg.sender, ids, amounts, "");
		_burn(msg.sender, WEDDING_RING_ID, numberOfFarmGirls);
	}

	function uri(uint256 id) public view override returns (string memory) {
		IMetaFarmerette metaFarmerette = IMetaFarmerette(
			_getRegistryFarmer().getContract("MetaFarmerette")
		);

		return metaFarmerette.uri(id);
	}
	...
	// Administrative
	function setBreedingIsLive(bool _breedingIsLive) public onlyOwner {
		breedingIsLive = _breedingIsLive;
	}

	function setWeddingRingCost(uint256 potato) public onlyOwner {
		weddingRingCost = potato;
	}
	...
	function setNumberOfWeddingRingsLeft(uint256 _numberOfWeddingRingsLeft)
		public
		onlyOwner
	{
		numberOfWeddingRingsLeft = _numberOfWeddingRingsLeft;
	}
