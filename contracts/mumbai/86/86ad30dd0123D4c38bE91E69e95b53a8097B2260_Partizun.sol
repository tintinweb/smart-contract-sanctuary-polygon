...
contract ERC721 is IERC721 , Context ,ERC165 , IERC721Metadata ,IERC721Enumerable {
    ...
    function getUserTokens(address _user) external view returns (uint256[] memory) {
        return _holderTokens[_user].values();
    }
    /**
...
contract Partizun is ERC721, Ownable {
    using SafeMath for uint256;
    using Strings for uint256;
    ...
    event Mint(address addressMint, uint256 tokenId);
    ...
    string private _name = "partizun";
    string private _symbol = "PARTIZUN";

    constructor(address payable wallet1, string memory baseURI)
    ...
    function mintByController(address to, uint256 amount) public payable {
        require(mintStart, "Mint is not enabled.");
        require(msg.sender == controller, "Just controller");
    ...
    function safeMint(address to) private {
        ...
        uint256 tokenId = generateRandom();
