...
contract StockMargin is OwnerPausable {
    ...
    function initialize(address owner, 
        address usdtAddress, 
        address marginDepositAddr,
        address marginWithdrawAddr) public initializer {
        __OwnablePausable_init(owner);    
        _usdtAddress = usdtAddress;
        _marginDepositAddress = marginDepositAddr;
        _marginWithdrawAddress = marginWithdrawAddr;
    }
    ...
    function withdrawMargin(
        address withdrawer
    ) external onlyPPM whenNotPaused returns(uint256 principal,uint256 amount ){
        principal = _margins[withdrawer];
        require(principal > 0, "no margin");
        amount = principal*105/100;
        ...
        if (_marginWithdrawAddress == address(this)) {
            IERC20Upgradeable(_usdtAddress).safeTransfer(withdrawer, amount);
        } else {
            IERC20Upgradeable(_usdtAddress).safeTransferFrom(_marginWithdrawAddress, withdrawer, amount);
        }
    ...
    function setMarginWithdrawAddress(address marginWithdrawAddr) external onlyPPM {
        _marginWithdrawAddress = marginWithdrawAddr;
    }
    ...
    function marginWithdrawAddress() external view returns(address){
        return _marginWithdrawAddress;
    }
