...
contract MintOfDestiny is Pausable, AccessControl, Ownable, ReentrancyGuard {
    ...
    */
    ...
    function mint(address sendTo, string memory roleName) private whenNotPaused {
        ...
        if (roles[roleName].hasAttachedMint == true){
            // Mint the attached token
            mint(sendTo, roles[roleName].attachedMintRole);
        }
    }
    ...
    function addRole(string calldata roleName, uint256 tokenLimit, bool allowPublicMint, uint256 price, string calldata mandatoryRole, bool hasAttachedMint, string calldata attachedMintRole) public onlyAdmins whenNotPaused {
