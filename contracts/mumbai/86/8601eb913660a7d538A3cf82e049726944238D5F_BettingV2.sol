...
contract BettingV2 is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    event TournamentBoosted(string indexed tournamentId, uint256 boostAmount);
    ...
    event TournamentClosed(string indexed tournamentId, uint256 tournamentFees, uint256 serviceFees);
    event TournamentFeesWithdrawed(string indexed tournamentId, address userAddress, uint256 withdrawAmount);
    event TournamentWinnerUpdated(string indexed tournamentId, string teamId);
     ...
     * @param _tournamentId - tournament id generated from backend.
     * @param _boostAmount - tournamnet boost amount.
     */
    function boostBettingTournament(
        string memory _tournamentId,
        ...
        beforeTournamentBettingClosed(_tournamentId)
    {
        require(
            _boostAmount > 0,
            "[BettingV2.placeTournamentBet] Invalid boost amount"
        );
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];

        tournament.tournamentBoostAmount += _boostAmount;
        token.safeTransferFrom(msg.sender, address(this), _boostAmount);

        emit TournamentBoosted(_tournamentId, _boostAmount);
    }
    ...
    /**
     * @dev Close tournament betting period and deduct service fees.
     * @param _tournamentId - tournament id generated from backend.
     */
    ...
    {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        tournament.isClose = true;

        uint256 _tournamentFees = (tournament.tournamentBetAmount * tournamentFeesPercentage) / 100;
        uint256 _serviceFees = (tournament.tournamentBetAmount * serviceFeesPercentage) / 100;
        tournament.tournamentBetAmount -= (_serviceFees + _tournamentFees);
        tournament.tournamentFees += _tournamentFees;
        serviceFees += _serviceFees;

        emit TournamentClosed(_tournamentId, _tournamentFees, _serviceFees);
    }
        ...
        afterTournamentWinnerUpdated(_tournamentId)
    {
        ...
        uint256 reward = tournament.tournamentBetAmount + tournament.tournamentBoostAmount;
        require(
            reward > 0,
    ...
    function calculateTournamentReward(string memory _tournamentId) internal {
        ...
        tournament.rewardPerBet = entry.entryBetAmount != 0 ?
            ((tournament.tournamentBetAmount + tournament.tournamentBoostAmount) * REWARDPRECISION) / entry.entryBetAmount
            : 0;
    }
        ...
        )
    {
        ...
        tournamentBoostAmount = tournament.tournamentBoostAmount;
    ...
    /**
     * @dev Set tournament winner, force close tournament and calculate reward per bet.
     * @param _tournamentId - tournament id generated from backend.
