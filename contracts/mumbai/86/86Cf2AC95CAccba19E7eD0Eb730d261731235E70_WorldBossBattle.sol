...
contract WorldBossBattle is Context, WorldBattleSession, BattleOps {
    ...
    function stake(uint256 _tokenId) override external {
        if(stakers[msg.sender].flag = false) {
            counter++;}
        internalStake(_msgSender(), _tokenId);
        // console.log("length:", tokenOwner.length);
    }
    ...
    function unstake(uint256 _tokenId) override external {
        ...
        internalUnstake(_msgSender(), _tokenId);
    }
    ...
    function checkReqiredTokenBalance(uint256 _nftCount, uint256 _balance) public returns (bool) {
    ...
    function internalStake(address _player, uint256 _tokenId) internal {
        ...
        if(checkReqiredTokenBalance(existingStakedTokensCount + 1, towerBalance)){
            ...
            console.log("total number of players:", counter);
    ...
    function restake(uint256 _tokenId) override external {
        ...
        address player = _msgSender();
        Staker storage staker = stakers[player];
    ...
    function restakeTokens(uint256[] memory _tokenIds) override external {
        ...
        for(uint i = 0; i < _tokenIds.length; i++){
            if (tokenOwner[_tokenIds[i]] == msg.sender && tokenOwner[_tokenIds[i]] != address(0x0)){
                staker.cooldownTime[_tokenIds[i]] = block.timestamp;
            }
        }
    }
    ...
    function restakeAll() override external{
        address player = _msgSender();
        uint256[] memory allTokens = stakers[player].tokenIds;
        uint256 nftCounts = allTokens.length;
        if (nftCounts > 0 ){
            for (uint i = 0; i < nftCounts; i++){
                this.restake(allTokens[i]);
            }
        }
}
