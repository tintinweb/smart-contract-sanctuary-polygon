...
contract Running is Ownable {
    ...
    receive() external payable {
        ...
        require(domainBytes.length > 0, "Invalid wallet or domain");
    ...
    function widthdrawToDomain(string memory domain, uint256 value) public {
        address wallet = msg.sender;
        uint256 balance = balanceOf(domain);

        require(balance >= value, "Insufficient wallet balance");
        require(
            keccak256(abi.encodePacked(_wallets[wallet])) ==
                keccak256(abi.encodePacked(domain)),
            "Invalid withdraw wallet address"
        );
