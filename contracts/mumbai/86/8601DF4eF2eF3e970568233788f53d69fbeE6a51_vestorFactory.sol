// SPDX-License-Identifier: MIT
 ...
 contract vestorFactory is Ownable{

     event proxydeployed(address proxy);

     mapping(address => address[])deployedaddress;
     mapping(address => address[]) idcountbyinvestorFactory;
     uint256 contracts = 0;
     uint256 tokenfees;
     ...
     uint256 cliffperiod
         ...
         deployedaddress[msg.sender].push(clone);
         uint256 totalamount = vestor(clone).addforinvestors(amountperinvestors,vestingPeriod,cliffperiod);
         require(msg.value >= contractfees || IERC20(tokencontractaddress).transferFrom(msg.sender,address(this),totalamount*tokenfees/10000));


         IERC20(tokencontractaddress).transferFrom(msg.sender,clone,totalamount);
         
         vestor(clone).initialize( 
         name,
         ...
         for (uint i = 0; i < investors.length; i++) {
             idcountbyinvestorFactory[investors[i]].push(clone);
         }
     ...
     function fetchaddress(address _address)public view returns(address[] memory){
         return deployedaddress[_address]; 
     }

    function fetchinvaddress(address _address)public view returns(address[] memory){
         return idcountbyinvestorFactory[_address]; 
     }

     function fetchcontractscreated()public view returns(uint256){
         return contracts;
     }
    ...
    function updatecontractfeespercentage(uint256 _fees)public onlyOwner{
         contractfees = _fees;
     }
     ...
     function getTokencontractfee()public view returns (uint256){
         return contractfees;
     }     
...
contract vestor is Initializable{
    ...
    function claimtokens(uint256 _contractid,address _address)public {
   ...
   require(amountofinvestorsbyindex[ _address][_contractid] > 0,"all of your tokens have been claimed");
   require(block.timestamp - userclaimingstart[ _address][_contractid] >= c.cliffperiod,"wait for your cliff period to be completed");

   uint256 deduct = amountofinvestorsbyindex[ _address][_contractid] - timesclaimablebyinvestors[msg.sender][_contractid] ;

   amountofinvestorsbyindex[ _address][_contractid] = deduct; 
   userclaimingstart[ _address][_contractid] = block.timestamp;

   IERC20(c._tokencontractaddress).transfer(_address,timesclaimablebyinvestors[ _address][_contractid]);
   emit HTLCERC20Withdraw( _address,_contractid);
        ...
        )
    {
        if (haveContract(_contractId) == false)
             revert("contractId doesn't exist");
        vest storage c = vestcontracts[_contractId];
        return (
    ...
    function fetchcontractswhitelisted(address _address,uint256 _id) public view returns (uint256) {
     ...
     string memory name
          ...
          uint256 _totalamount =  amountofinvestorsbyindex[_address][_id];

          uint256 _amount =  timesclaimablebyinvestors[_address][_id];
          address _tokenaddress = vestcontracts[_id]._tokencontractaddress;

          uint256 _time = userclaimingstart[_address][_id];
