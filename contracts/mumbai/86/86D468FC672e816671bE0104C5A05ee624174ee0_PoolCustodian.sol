...
interface IFeesController {
    ...
    /**
     * @notice Emitted when issuance fees are registered
     * @param token Token address of the fees taken
     * @param amount Amount of fees taken
     **/
    event IssuanceFeesRegistered(address token, uint256 amount);
     ...
     *
     * Emits a {IssuanceFeesRegistered} event
     **/
     ...
     * Emits a {IssuanceFeesRegistered} event
     **/
    function registerIssuanceFees(uint256 amount) external returns (uint256 fees);
...
contract PoolTokenFeesController is IFeesController, Managed {
    ...
    uint256 public immutable ISSUANCE_FEES_RATE;
     ...
     * @param issuanceFeesRate fee rate taken at borrow time
    ...
    constructor(
        ...
        ISSUANCE_FEES_RATE = issuanceFeesRate;
     ...
     * @inheritdoc IFeesController
     */
    function registerIssuanceFees(uint256 amount) external onlyPool returns (uint256 fees) {
        fees = amount.mul(ISSUANCE_FEES_RATE);
        dueFees += fees;
        totalFees += fees;

        emit IssuanceFeesRegistered(TOKEN, fees);
    }
...
interface IProductFactory {
    ...
    /**
     * @notice Thrown when min issuance is less than target issuance
     */
    error INVALID_ISSUANCE_PARAMETERS();
...
contract Bullet is LoanBase, IRepayableLoan {
    ...
    function getPositionCurrentValue(uint256 positionId) external view override returns (uint256 positionCurrentValue) {
        ...
        uint256 referenceTimestamp = NON_STANDARD_REPAY_TIMESTAMP > 0 ? NON_STANDARD_REPAY_TIMESTAMP : block.timestamp;
    ...
    function getCurrentAccruals() external view override returns (uint256 currentAccruals) {
        if (borrowTimestamp + LOAN_DURATION < block.timestamp) {
            revert LoanErrors.LOAN_MATURITY_REACHED();
        }
        // TODO check precision in operations
        // TODO block after maturity
        // TODO reference timestamp should be non standard repayment start
        uint256 timeUntilMaturity = borrowTimestamp + LOAN_DURATION - block.timestamp;
        currentAccruals =
            ((totalToBeRepaid - totalCurrentBorrowed) * (LOAN_DURATION - timeUntilMaturity)) /
            LOAN_DURATION;
    }
...
contract CouponBullet is LoanBase, IPayableLoan, IRepayableLoan {
     ...
     */
    function getPositionCurrentValue(uint256 positionId) external view override returns (uint256 positionCurrentValue) {
        return _getPositionPaymentsCurrentValue(positionId);
    }
     ...
     */
    function repay() external onlyBorrower {
        // TODO repayment period must be less than a payment period, otherwise there could be deadlocks
        if (totalPaymentsCount - 1 != paymentsDoneCount) revert LoanErrors.LOAN_COUPONS_NOT_PAID();
        _makePaymentsDue(1);
    ...
    function getPaymentAmountForTick(
        ...
        couponPaymentForTick = tick.borrowedAmount.mul(LoanLogic.getPaymentRate(rate, paymentPeriod));
...
contract InstallmentLoan is LoanBase, IPayableLoan {
     ...
     */
    function getPositionCurrentValue(uint256 positionId) external view override returns (uint256 positionCurrentValue) {
        positionCurrentValue = _getPositionPaymentsCurrentValue(positionId);

        uint256 outstandingBorrowedAmount = (totalPaymentsCount - _getNumberOfPaymentsExpired()) *
            (ticks[positions[positionId].rate].borrowedAmount / totalPaymentsCount);
    ...
    function makePaymentsDue() external onlyBorrower {
        ...
        // TODO: check that we are amounts substracted are right
        totalCurrentBorrowed -= (totalCurrentBorrowed * paymentsToMake).div(totalPaymentsCount * ONE);
        if (paymentsDoneCount == totalPaymentsCount) poolPhase = LoanTypes.PoolPhase.REPAID;
    }
    ...
    function getPaymentAmountForTick(
        ...
        paymentAmount = tick
            .borrowedAmount
            .mul(ONE + LoanLogic.getPaymentRate(rate, paymentPeriod) * totalPaymentsCount)
...
interface ILoanBorrower {
     ...
     */
    event PrincipalRepaid(address indexed borrower, address contractAddress, uint256 repaidAmount, uint256 atlendisFee);
...
interface ILoanGovernance is ITimelock {
     ...
     */
    event BondIssuanceCancelled(address contractAddress, uint256 remainingEscrow);
     ...
     *
     * Emits a {BondIssuanceCancelled} event
     */
...
library LoanErrors {
    ...
    error LOAN_EARLY_REPAY_NOT_ALLOWED(); // "Bond is not callable";
    ...
    error LOAN_ISSUANCE_PHASE_EXPIRED(); // "Issuance phase has expired"
    error LOAN_ISSUANCE_PERIOD_STILL_ACTIVE(); // "Issuance period not expired yet"
    error LOAN_WRONG_INPUT(); // The specified input does not pass validation
    error LOAN_FINAL_COUPON(); // "Final Coupon is paid back with principal"
    error LOAN_COUPONS_NOT_PAID(); // "Missing coupon payment"
    ...
    error LOAN_BORROW_AMOUNT_OUT_OF_RANGE(); // "Borrow amount has to be between min and target issuance"
...
library LoanLogic {
    ...
    function getNumberOfPaymentsExpired(
        ...
        uint256 paymentPeriod
    ) internal view returns (uint256 paymentsDueCount) {
        // TODO reference timestamp?
        uint256 maturityCappedTimestamp = block.timestamp > borrowTimestamp + loanDuration
            ? borrowTimestamp + loanDuration
            : block.timestamp;
        return uint256(maturityCappedTimestamp - borrowTimestamp) / paymentPeriod;
    }
     ...
     */
    function getPaymentRate(uint256 rate, uint256 paymentPeriod) internal view returns (uint256 paymentRate) {
        paymentRate = rate.mul(paymentPeriod.div(1)).div(uint256(365 days).div(1));
    }
     ...
     */
    function depositToTick(LoanTypes.Tick storage tick, uint256 amount) external {
    ...
    function withdrawFromTick(
        ...
        (uint256 unborrowedAmount, , bool partialWithdraw) = getPositionRepartition(
            tick,
            poolPhase,
            depositedAmount,
        ...
        )
    {
        ...
        if (
            ...
            unborrowedAmount = depositedAmount.mul(tick.depositedAmount - tick.borrowedAmount).div(
                tick.depositedAmount
            );
            borrowedAmount = depositedAmount - unborrowedAmount;
        ...
        // If full fill and repaid or issuance was cancelled
        if (
...
abstract contract LoanBase is ILoan, LoanState, LoanGovernance, LoanLender, LoanBorrower {
        ...
        returns (uint256 unborrowedAmount, uint256 borrowedAmount)
    {
        if (_ownerOf[positionId] == address(0)) {
            revert LoanErrors.LOAN_POSITIONS_NOT_EXIST();
        }
        ...
        (unborrowedAmount, borrowedAmount, ) = LoanLogic.getPositionRepartition(
            ...
            positions[positionId].unborrowedAmountWithdrawn
        );

        uint256 paymentOutstanding = paymentsDoneCount - positions[positionId].numberOfPaymentsWithdrawn;

        uint256 paymentAmount = ticks[positions[positionId].rate].singlePaymentAmount;

        unborrowedAmount += paymentOutstanding * paymentAmount;
    }
    ...
    function getPositionLoanShare(uint256 positionId) external view returns (uint256 positionLoanShare) {
        ...
        uint256 additionalPrecision = RAY / ONE;
        return (borrowedAmount * additionalPrecision).div(totalBorrowedStatic);
    }
    ...
    function _getPaymentsLoanCurrentAccruals() internal view returns (uint256 currentAccruals) {
        ...
        uint256 paymentsDue = _getNumberOfPaymentsExpired() - paymentsDoneCount;
        uint256 timeIntoCurrentPayment = (block.timestamp - borrowTimestamp) -
            _getNumberOfPaymentsExpired() *
            PAYMENT_PERIOD;

        uint256 totalMissedPayments;
        uint256 totalRunningPayments;
        ...
        while (currentInterestRate <= MAX_RATE) {
            ...
            totalMissedPayments += earnings * paymentsDue;
            totalRunningPayments = (earnings * timeIntoCurrentPayment) / PAYMENT_PERIOD;
            currentInterestRate += RATE_SPACING;
        }

        currentAccruals = totalMissedPayments + totalRunningPayments;
    }
     ...
     * @return positionCurrentValue Current value of the position
     */
    function _getPositionPaymentsCurrentValue(uint256 positionId) internal view returns (uint256 positionCurrentValue) {
        (uint256 unborrowedAmount, uint256 borrowedAmount) = getPositionRepartition(positionId);
...
abstract contract LoanFactoryBase is IProductFactory {
    ...
    function deployLoan(
        ...
        (uint256 loanDuration, , , , uint256 paymentPeriod) = abi.decode(
            durationsConfigTmp,
        ...
        if (minIssuance > targetIssuance) revert INVALID_ISSUANCE_PARAMETERS();
...
abstract contract LoanBorrower is LoanState, ILoanBorrower {
    ...
    function enableBookBuildingPhase() external onlyBorrower onlyInPhase(LoanTypes.PoolPhase.INACTIVE) {
        ...
        cancellationFeeEscrow = CANCELLATION_FEE_PC.mul(TARGET_ISSUANCE_AMOUNT);
    ...
    function borrow(uint256 borrowedAmount, address to)
        external
        onlyBorrower
        onlyInPhase(LoanTypes.PoolPhase.ISSUANCE)
    {
        // Validation
        if (block.timestamp > ISSUANCE_PHASE_START_TIMESTAMP + ISSUANCE_PERIOD_DURATION)
            revert LoanErrors.LOAN_ISSUANCE_PHASE_EXPIRED();
        if (deposits == 0) revert LoanErrors.LOAN_ZERO_BORROW_AMOUNT_NOT_ALLOWED();

        if (deposits > 0 && deposits < MIN_ISSUANCE_AMOUNT) {
            borrowedAmount = deposits;
        } else if ((borrowedAmount < MIN_ISSUANCE_AMOUNT) || (borrowedAmount > TARGET_ISSUANCE_AMOUNT)) {
        ...
        uint256 fees = FEES_CONTROLLER.registerIssuanceFees(borrowedAmount);
        ...
        emit Borrowed(msg.sender, address(this), borrowedAmount, fees, cancellationFeeEscrow);
    }
    ...
    function _getNumberOfPaymentsExpired() internal view returns (uint256 numberOfPaymentsDue) {
        numberOfPaymentsDue = LoanLogic.getNumberOfPaymentsExpired(borrowTimestamp, LOAN_DURATION, PAYMENT_PERIOD);
    }
    ...
    function _makePaymentsDue(uint256 paymentsToMake) internal {
        // TODO verify that the borrower can pay last coupon and pay separately or together
        // if (paymentsDoneCount == totalPaymentsCount - 1) revert LoanErrors.LOAN_FINAL_COUPON();

        if (poolPhase != LoanTypes.PoolPhase.ISSUED && poolPhase != LoanTypes.PoolPhase.REPAID)
            revert LoanErrors.LOAN_INVALID_PHASE();

        if (paymentsToMake == 0) revert LoanErrors.NO_PAYMENTS_DUE();
        uint256 totalPaymentAmount;
        uint256 totalEarnings;
        uint256 currentInterestRate = MIN_RATE;
        // TODO: implement late repay fee?
        while (currentInterestRate <= MAX_RATE) {
            LoanTypes.Tick storage tick = ticks[currentInterestRate];
            if (tick.borrowedAmount > 0) {
        ...
        emit PaymentMade(msg.sender, address(this), paymentsDoneCount, totalPaymentAmount);
    }
    ...
    function _repay(
        ...
        if (block.timestamp < borrowTimestamp + LOAN_DURATION - REPAYMENT_PERIOD_DURATION)
            revert LoanErrors.LOAN_EARLY_REPAY_NOT_ALLOWED();
        ...
        emit PrincipalRepaid(msg.sender, address(this), repaidAmount, fees);
    }
...
abstract contract LoanGovernance is ILoanGovernance, LoanState {
     ...
     */
    function enableIssuancePhase() external onlyGovernance onlyInPhase(LoanTypes.PoolPhase.BOOK_BUILDING) {
        if (block.timestamp <= CREATION_TIMESTAMP + BOOK_BUILDING_PERIOD_DURATION) {
            revert LoanErrors.LOAN_BOOK_BUILDING_TIME_NOT_OVER();
        }
        poolPhase = LoanTypes.PoolPhase.ISSUANCE;
     ...
     * @inheritdoc ILoanGovernance
     */
    function cancelLoan(bool redistributeCancelFee) external onlyGovernance onlyInPhase(LoanTypes.PoolPhase.ISSUANCE) {
        if (block.timestamp < ISSUANCE_PHASE_START_TIMESTAMP + ISSUANCE_PERIOD_DURATION) {
            revert LoanErrors.LOAN_ISSUANCE_PERIOD_STILL_ACTIVE();
        }
        ...
        for (
            ...
            uint256 cancelFeeForTick = LoanLogic.repayCancelFeeForTick(
                ...
                CANCELLATION_FEE_PC,
                remainingEscrow,
                redistributeCancelFee && (deposits > MIN_ISSUANCE_AMOUNT)
            );
        ...
        emit BondIssuanceCancelled(address(this), remainingEscrow);
    }
    ...
    function executeTimelock() external onlyGovernance {
        ...
        NON_STANDARD_REPAY_TIMESTAMP = block.timestamp;
...
abstract contract LoanLender is LoanState, SolmateERC721, ILoanLender {
    ...
    function updateRate(uint256 positionId, uint256 rate) external onlyLender {
        ...
        emit RateUpdated(positionId, msg.sender, address(this), oldRate, rate);
    }
    ...
    function withdraw(uint256 positionId) external {
        ...
        if (unborrowedAmountWithdrawn) {
            ...
            // TODO verify that any unborrowed amount here means that we withdraw after repayment
            unborrowedAmount > 0 ||
            poolPhase == LoanTypes.PoolPhase.CANCELLED
        ) {
            _burn(positionId);
        ...
        uint256 fees = earnings.mul(FEES_CONTROLLER.getRepaymentFeesRate()); // TODO wtf
        CUSTODIAN.withdraw(unborrowedAmount + paymentsToWithdraw - fees, msg.sender);

        emit Withdrawn(positionId, msg.sender, address(this), unborrowedAmount + paymentsToWithdraw - fees);
    }
    ...
    function validateWithdraw(uint256 positionId) internal view {
        ...
        if (poolPhase == LoanTypes.PoolPhase.ISSUANCE || poolPhase == LoanTypes.PoolPhase.NON_STANDARD)
...
abstract contract LoanState is Managed, ILoanState {
    ...
    uint256 public NON_STANDARD_REPAY_TIMESTAMP;
    ...
    uint256 public immutable MIN_ISSUANCE_AMOUNT;
...
library RevolvingCreditLineErrors {
    ...
    error RCL_HAS_SIGNALLED(); // Position that already signalled exit can not exit
...
library TickLogic {
    ...
    function prepareTickForNextLoan(DataTypes.Tick storage tick, DataTypes.Loan storage currentLoan) external {
        ...
        uint256 tickAdjusted = tick.baseEpochsAmounts.adjustedDeposits +
            ...
            tick.baseEpochsAmounts.adjustedSignalled;
     ...
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or trim loan of the position
    ...
    function getAdjustedAmount(
        ...
        return adjustedAmount = position.baseDeposit.div(getEquivalentYieldFactor(tick, epoch, referenceLoan));
    }
    ...
    function getPositionCurrentValue(
        ...
        if (position.signalLoanId > 0) {
            bool signalLoanRepaid = (position.signalLoanId < currentLoan.id) || currentLoan.maturity == 0;
            if (signalLoanRepaid) {
                return
                    getAdjustedAmount(tick, position, referenceLoan).mul(
                        tick.endOfLoanYieldFactors[position.signalLoanId]
                    );
            }
        }
        if (position.withdrawLoanId > 0) {
    ...
    function getPositionEndOfLoanValue(
        ...
        if ((position.epochId <= tick.loanStartEpochId) || position.withdrawn.borrowed > 0) {
            ...
            uint256 endOfLoanInterest = shareOfEpoch.mul(epoch.accruals);
    ...
    function getPositionRepartition(
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 adjustedAmount = getAdjustedAmount(tick, position, referenceLoan);
            shareOfEpoch = adjustedAmount.div(tick.baseEpochsAmounts.adjustedDeposits);
        ...
        shareOfEpoch = position.baseDeposit.div(epoch.deposited);
        unborrowedAmount = shareOfEpoch.mul(epoch.deposited - epoch.borrowed);
        borrowedAmount = shareOfEpoch.mul(epoch.borrowed);
        return (unborrowedAmount, borrowedAmount);
    }
    ...
    function withdraw(
        ...
        if (expectedWithdrawnAmount == type(uint256).max) {
            withdrawnAmount = getPositionCurrentValue(tick, position, referenceLoan, currentLoan);
        } else {
        ...
        if (currentLoan.maturity == 0 || (currentLoan.maturity > 0 && tick.latestLoanId < currentLoan.id)) {
            ...
            if (adjustedAmountToWithdraw > tick.baseEpochsAmounts.adjustedDeposits) {
    ...
    function trim(
        ...
        if (position.epochId <= tick.loanStartEpochId) {
            uint256 adjustedAmount = getAdjustedAmount(tick, position, referenceLoan);
            if (position.signalLoanId == currentLoan.id) {
                tick.baseEpochsAmounts.adjustedSignalled -= adjustedAmount;
            } else {
            ...
            if (position.signalLoanId == currentLoan.id) {
                epoch.signalled -= position.baseDeposit;
                tick.newEpochsAmounts.signalled -= position.baseDeposit;
            } else {
                tick.withdrawnAmounts.borrowed += borrowedAmount;
                tick.withdrawnAmounts.toBeAdjusted += borrowedAmount + accruals;
            }
            ...
            // @dev precision issue
            tick.newEpochsAmounts.toBeAdjusted = endOfLoanPositionValue >= tick.newEpochsAmounts.toBeAdjusted
                ? 0
                : tick.newEpochsAmounts.toBeAdjusted - endOfLoanPositionValue;
     ...
     * @dev any partially matched position can be trimmed to result in being fully matched
     * @param tick Target tick
     * @param position Target position
    ...
    function registerBaseEpochExit(
        ...
        if (adjustedAmount > tick.baseEpochsAmounts.adjustedDeposits)
            adjustedAmount = tick.baseEpochsAmounts.adjustedDeposits;
        tick.baseEpochsAmounts.adjustedDeposits -= adjustedAmount;
        tick.baseEpochsAmounts.borrowed -= borrowedAmount;
        tick.baseEpochsAmounts.available -= unborrowedAmount;
    ...
    function registerNewEpochExit(
        ...
        bool isPositionAllNewEpochs = (currentPositionValue > tick.newEpochsAmounts.toBeAdjusted) &&
            (currentPositionValue - tick.newEpochsAmounts.toBeAdjusted) < 10;
        if (isPositionAllNewEpochs) currentPositionValue = tick.newEpochsAmounts.toBeAdjusted;
    ...
    /**
     * @notice Mark the position as signalled and register the signalled amount in the ticks
     * @param tick Target tick
     * @param position Target position
     * @param referenceLoan Either first loan or trim loan of the position
     * @param currentLoan Current loan information
    ...
    function signal(
        ...
        // if the position is from the base epoch, the adjusted amount to remove at the end of the loan is known in advance
            ...
            tick.baseEpochsAmounts.adjustedSignalled += getAdjustedAmount(tick, position, referenceLoan);
        }
        ...
        position.signalLoanId = currentLoan.id;
    }
        ...
        returns (uint256 baseEpochsFees)
    {
        ...
        if (tick.baseEpochsAmounts.adjustedDeposits > 0)
    ...
    function validateExit(
        ...
        if (position.signalLoanId > 0) revert RevolvingCreditLineErrors.RCL_HAS_SIGNALLED();
    ...
    /**
     * @notice Validation logic for position signalling
     * @param position Target position
     * @param tick Target tick
     * @param referenceLoan Either first loan or trim loan of the position
     * @param currentLoan Current loan information
    ...
    function accrualsFor(
        ...
        accruals = ((amount * timeDelta) / 365 days).mul(rate);
    }
    ...
    function allocateBorrowAmounts(
        ...
        if (amountToBorrow + accrualsToAllocate >= epochAvailable) {
            uint256 downscalingFactor = epochAvailable.div(amountToBorrow + accrualsToAllocate);
            accrualsAllocated = accrualsToAllocate.mul(downscalingFactor);
...
interface IRCLLenders {
     ...
     * @param managementFees Amount of fees taken
     */
    event Trim(uint256 indexed positionId, uint256 receivedAmount, uint256 managementFees);
     ...
     */
    event Signal(uint256 indexed positionId, uint256 loanId);
     ...
     * Emits a {Trim} event
     */
    function trim(uint256 positionId) external;
...
interface IRCLOrderBook {
     ...
     * @return adjustedSignalled Adjusted amount opted out of the next loan
     */
     ...
     * @return signalled Amount opted out of the next loan
     */
...
abstract contract RCLBorrowers is IRCLBorrowers, RCLOrderBook {
    ...
    function borrow(address to, uint256 amount) external onlyBorrower onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        ...
        amendGlobalsOnBorrow(amount);

        uint256 remainingAmount = amount;
        uint256 rate = MIN_RATE;
        while (remainingAmount > 0 && rate <= MAX_RATE) {
            (uint256 borrowedAmount, , ) = TickLogic.borrow(
                ticks[rate],
                currentLoan,
            ...
            totalToBeRepaid +=
                borrowedAmount +
                TickLogic.accrualsFor(borrowedAmount, currentLoan.maturity - block.timestamp, rate);

            rate += RATE_SPACING;
        }
        // TODO clean precision tolerance approach
        if (remainingAmount > 10) revert RevolvingCreditLineErrors.RCL_NO_LIQUIDITY();

        uint256 fees = FEES_CONTROLLER.registerIssuanceFees(amount);
        FundsTransfer.chargedWithdraw({
            token: TOKEN,
            custodian: CUSTODIAN,
            recipient: to,
        ...
        emit Borrow(amount - fees, fees, to);
    }
    ...
    function amendGlobalsOnRepay() internal {
        ...
        uint256 precisionScaling = WAD / ONE;
        loans[currentLoan.id].repaymentFeesRate = FEES_CONTROLLER.getRepaymentFeesRate() / precisionScaling;
    ...
    /**
     * @notice Update global pool variables during a borrow
     * @param amount Borrowed amount
     */
    function amendGlobalsOnBorrow(uint256 amount) internal {
        if (currentLoan.maturity == 0) {
...
abstract contract RCLLenders is IRCLLenders, RCLOrderBook, SolmateERC721 {
        ...
        notInPhase(DataTypes.OrderBookPhase.NON_STANDARD)
    {
        ...
        if (amountToWithdraw == type(uint256).max || position.signalLoanId > 0) {
            burn(positionId);
        }
     ...
     * @inheritdoc IRCLLenders
     */
    function trim(uint256 positionId) public onlyLender notInPhase(DataTypes.OrderBookPhase.NON_STANDARD) {
        DataTypes.Position storage position = positions[positionId];
        DataTypes.Tick storage tick = ticks[position.rate];
        ...
        uint256 unborrowedAmount = TickLogic.trim(tick, position, loan, currentLoan, minDepositAmount);
        ...
        emit Trim(positionId, withdrawnAmount, fees);
    }

    /**
     * @inheritdoc IRCLLenders
     */
    function signal(uint256 positionId) external onlyLender onlyInPhase(DataTypes.OrderBookPhase.OPEN) {
        if (ownerOf(positionId) != msg.sender) revert RevolvingCreditLineErrors.RCL_ONLY_OWNER();
        ...
        TickLogic.signal({tick: tick, position: position, referenceLoan: referenceLoan, currentLoan: currentLoan});

        emit Signal(positionId, currentLoan.id);
    }
        ...
        notInPhase(DataTypes.OrderBookPhase.NON_STANDARD)
    {
        ...
        if (endOfLoanBorrowEquivalent > 10 || remainingAccrualsToAllocate > 10)
     ...
     * If the position has been trimmed, the reference loan is the trim loan
...
abstract contract RCLOrderBook is IRCLOrderBook, Managed {
        ...
        )
    {
        ...
        return (
            ...
            tick.newEpochsAmounts.borrowed,
            tick.newEpochsAmounts.available,
            tick.newEpochsAmounts.signalled
        );
    }
...
contract RevolvingCreditLine is IRevolvingCreditLine, RCLOrderBook, RCLGovernance, RCLBorrowers, RCLLenders {
        ...
        )
    {
        ...
        if (position.signalLoanId > 0) return (owner, position.rate, position.baseDeposit, PositionStatus.UNAVAILABLE);
