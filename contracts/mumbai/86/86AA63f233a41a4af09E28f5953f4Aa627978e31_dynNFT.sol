...
contract dynNFT is ERC721, IERC721Receiver, Ownable, AutomationCompatible {
        ...
        returns (string memory)
    {
        ...
        /**
         * A SQL query to JOIN two tables to compose the metadata accross a 'tokens' and 'flowers' table in ERC-721 compliant schema
         * 
         * Essentially, the metadata is created in this table for each NFT. As values get updated via `growFlower`,
         * the associated metadata query will automotically read those values from the table; this `tokenURI` query
         * is future-proof upon table mutations.
         * 
         * The query forms a `json_object` with two nested `json_object` values in a `json_array`. The top-level metadat fields include
         * the `name`, `image`, and `attibutes`, where the `attributes` hold the composed data from the "tokens" and "flowers" tables.
