...
contract VotingContract is Context, Ownable {
    ...
    uint8 private addQuestionBalance;
    ...
    modifier canAddQuestion() {
        require(
            _canAddQuestion(_msgSender()) || _msgSender() == owner(),
            "Low Balance"
        );
        _;
    }
    ...
    constructor(
        ...
        addQuestionBalance = _addQuestionBalance;
    }
    ...
    function _canAddQuestion(address _user) internal view returns (bool) {
        return IERC721(membershipToken).balanceOf(_user) >= addQuestionBalance;
    }
    ...
    function setAddQuestionBalance(uint8 _bal) external onlyOwner {
        require(_bal >= 0, "Invalid value");
        addQuestionBalance = _bal;
    }
     function setVoteBalance(uint8 _bal) external onlyOwner {
        require(_bal >= 0, "Invalid value");
        voteBalance = _bal;
    }
}