...
contract SnowmanSales is Ownable, ERC1155Holder {
    using SafeERC20 for IERC20;
    IERC20 public snowmanToken;
    IERC1155 public snowmanGame;

    uint256 private constant TICKET_PRICE = 0.01 ether;
    uint256 private constant TICKET_QUANTITY = 1;
    uint256 private constant TICKET_ID = (1 << 128);
    ...
    constructor(address snowmanERC1155, address snowmanERC20) {
        snowmanGame = IERC1155(snowmanERC1155);
        snowmanToken = IERC20(snowmanERC20);
    }
    ...
    function onERC1155Received(
        ...
        require(msg.sender == address(snowmanGame), "incorrect sender");
        require(value != 0, "quantity is zero");
        emit onERC1155ReceivedExecuted(operator, from, id, value);
        // 함수의 이름과 파라미터 이름을 hash화 해서 bytes4로 return
        return this.onERC1155Received.selector;
    }
    ...
    function buyItem(
        ...
        require(snowmanGame.balanceOf(address(this), tokenId) >= quantity, "balance not enough");
        require(snowmanToken.balanceOf(buyer) >= amount, "insufficient token");
        require(snowmanToken.allowance(buyer, address(this)) >= amount, "insufficient token approval");
        snowmanToken.safeTransferFrom(buyer, owner(), amount);
        snowmanGame.safeTransferFrom(address(this), buyer, tokenId, quantity, "");
        emit BuyItem(buyer, tokenId, amount, quantity, block.timestamp);
    }

    function buyTicket() external payable {
        require(msg.value >= TICKET_PRICE, "price not enough");
        (bool success, ) = address(snowmanGame).call(
