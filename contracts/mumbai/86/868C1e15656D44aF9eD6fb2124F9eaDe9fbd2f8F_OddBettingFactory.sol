...
contract OddBettingFactory is Ownable, ReentrancyGuard {
    ...
    function selectWinner(
        ...
        Competition storage competition = competitions[competitoinId];
        require(
            competition.validBefore != 0,
            "Cannot Choose for Non Exist Competition"
        );
        competition.winPrediction = prediction;
    ...
    function withDraw(
        ...
        Bet storage bet = bets[competitionId][betId];
        // Claim For Draw Tournament
        if (competition.winPrediction == 3) {
            if (bet.firstPlayer == msg.sender) {
                require(!bet.firstPlayerDrawFund, "Fund Already WithDrawn");
                payable(msg.sender).transfer(bet.firstPlayerBidAmount);
                bet.firstPlayerDrawFund = true;
            } else if (bet.secondPlayer == msg.sender) {
                require(!bet.secondPlayerDrawFund, "Fund Already WithDrawn");
                payable(msg.sender).transfer(bet.secondPlayerBidAmount);
                bet.secondPlayerDrawFund = true;
            } else {
                require(false, "Not an Participant");
            }
        }
        // Claim For Single Bidded
        else if (
            bet.firstPlayer == msg.sender && bet.secondPlayer == address(0)
        ) {
            ...
            require(!bet.isClaimed, "Bet Already Claimed");
            uint256 totalAmount = bet.firstPlayerBidAmount +
                bet.secondPlayerBidAmount;
    ...
    function getJoinedDetails(
        address user
    ) public view returns (JoinedDetail[] memory) {
