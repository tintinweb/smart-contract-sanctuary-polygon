...
contract OmniCounter is NonblockingLzApp {
    ...
    function estimateFees() public view returns(uint messageFee){
        (messageFee, ) = lzEndpoint.estimateFees(bsc, address(this), bytes(""), false, bytes(""));       
    }

    function incrementCounter() public payable {
        require(msg.value >= estimateFees(), "less");
        _lzSend(bsc, bytes(""), payable(msg.sender), address(0x0), bytes(""));
    }
