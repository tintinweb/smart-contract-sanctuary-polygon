...
contract FunPool   {
    ...
    struct Record{
        address userAddress;//Receiving address
        uint256 amount;//Received quantity
        bool isClaimed;//Has it been claimed
        uint256 createTime;//Collection time
    }
 
   
    uint256 private _amount = 5 * 10 ** 18; //
    ...
    mapping(address =>Record) private _userRecord;//New User Claim Record
    address private _challengAddress;//
    ...
    function getChallengAddress() public view virtual returns (address) {
        return _challengAddress;
    }

    function setChallengAddress(address challengAddress_ ) public onlyOwner {
         _challengAddress = challengAddress_;
    }
    ...
    function setAmount(uint256 amount_ ) public onlyOwner {
         _amount = amount_;
    }
    ...
    function getUserRecord(address addres) public view virtual returns(Record memory){
        return _userRecord[addres];
    }
   ...
   //Distribute tokens to the specified address.
    ...
    //New users receive rewards
    function claimRewards() public {
        uint256 userAllowance =  token().allowance(msg.sender, _challengAddress);
        require(userAllowance>0,"User is not authorized to claim rewards");

        Record memory userRecord = _userRecord[msg.sender];
        require(userRecord.isClaimed == false,"The current address has already received rewards");
        _userRecord[msg.sender].userAddress = msg.sender;
        _userRecord[msg.sender].amount = _amount ;
        _userRecord[msg.sender].isClaimed =true;
        _userRecord[msg.sender].createTime = block.timestamp;
