...
contract tuned1155 is ERC1155
{
    ...
    function DeclareArt(string memory name , string memory imageCID , string memory animationCID , string memory audioCID, string memory description ,string memory spaceName) public payable {
    ...
    function Mint(uint256 tokenid,uint256 amount) public payable {
        require(tokenid <= tokenID.current() && amount>0 && amount*mintPrice <= msg.value);
        address payable to = payable(tokenCreator[tokenid]);
    ...
    function updateAttribute(uint256 tokenid , string memory attributeName , string memory value, bool main_table) public {
        require(tokenid <= tokenID.current() && balanceOf(msg.sender,tokenid) > 0);
    ...
    function assignAnimationURI(uint256 tokenid , string memory animationURL) public {
        ...
        tokenCreator[tokenid] = address(0);
