    ...
    ReentrancyGuardUpgradeable
{
    ...
    function __LinearPool_init(IERC20 _acceptedToken) internal {
        __Ownable_init();
        unstakeFee = 25;
        feeReceiverAddress = msg.sender; //change me
     ...
     */
    function linearSetPool(uint256 _poolId, bool _extendedPeriod)
        external
        onlyOwner
        linearValidatePoolById(_poolId)
    {
     ...
     * @param _minInvestment the minimum investment amount users need to use in order to join the pool.
     */
    function linearSetMinInvestmentPool(uint256 _poolId, uint256 _minInvestment)
        external
        onlyOwner
        linearValidatePoolById(_poolId)
    {
     ...
     */
    function _linearDeposit(uint256 _poolId, uint256 _amount)
        internal
        nonReentrant
        linearValidatePoolById(_poolId)
    {
    ...
    function _linearExecuteDeposit(
        ...
        require(
            _amount >= stakingData.userMinInvestment,
            "LinearStakingPool: User must stake higher than (minstake amount) SBX"
        );
...
contract FeesAndRulePool is LinearPool {
    using SafeERC20 for IERC20;
    using SafeCastUpgradeable for uint256;
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.UintSet;
    ...
    function __FeesAndRulePool_init (address treasuryPool, IERC20 _acceptedToken) public initializer {
    ...
    function updateFeeDepositStatus(bool status) external onlyOwner {
        feeDepositEnabled = status;
    }

    function updateFeeUnstakeStatus(bool status) external onlyOwner {
        feeUnstakeEnabled = status;
    }
    ...
    function _setFees(uint256 treasury) internal {
        require(
            treasury >= 100 && treasury <= maxFee,
    ...
    function _calcAmountAndUnstake(uint256 baseAmount, address _receiver)
        internal
    {
        feeUnstakeEnabled
            ? _splitAndSendUnstake(baseAmount, _receiver)
            : linearAcceptedToken.safeTransfer(_receiver, baseAmount);
    }
    ...
    function _calcAmountAndDeposit(uint256 baseAmount)
        ...
        returns (uint256)
    {
        uint256 treasuryAmount = (baseAmount * _treasuryFee) / percentageConst;
        feeDepositEnabled
        ...
        return feeDepositEnabled ? baseAmount - treasuryAmount : baseAmount;
    }

    function _splitAndSendDeposit(uint256 baseAmount) internal {
    ...
    function _splitAndSendUnstake(uint256 baseAmount, address _receiver)
        internal
    {
        uint256 sendingAmount = baseAmount;
        uint256 treasuryAmount = (baseAmount * _treasuryFee) / percentageConst;
    ...
    function unstake(uint256 _poolId, uint256 _amount) external {
        uint256 _amountLeft = _linearWithdraw(_poolId, _amount);

        _calcAmountAndUnstake(_amountLeft, msg.sender);
    }

    function deposit(uint256 _poolId, uint256 _amount) external {
        uint256 _amountLeft = _calcAmountAndDeposit(_amount);

        _linearDeposit(_poolId, _amountLeft);
    }
}