...
contract PULSEROLL is VRFConsumerBase {
    ...
    mapping(bytes32 => bool)public playerRequestFullfilled;
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal payoutsAreActive override 
    {
        ...
        playerRequestFullfilled[requestId]=true;
    ...
    function ownerRefundPlayer(
        ...
        require(playerAddress[originalPlayerBetId] != address(0),"player address should not be equal to 0");
        require(playerRequestFullfilled[originalPlayerBetId] == false,"the requestId is already fullfilled");
        require(playerBetValue[originalPlayerBetId] <= address(this).balance,"The bet value should be less than the balance in this contract");
        /* safely reduce pendingPayouts by playerProfit[rngId] */
        maxPendingPayouts = (maxPendingPayouts - playerBetValue[originalPlayerBetId]);//take out the player profit from maxpendingpayouts
        /* send refund */
         payable(playerAddress[originalPlayerBetId]).transfer(playerBetValue[originalPlayerBetId]) ;//original betamount is betamount
        /* log refunds */
        emit LogRefund(originalPlayerBetId, playerAddress[originalPlayerBetId], playerBetValue[originalPlayerBetId]);
    }
