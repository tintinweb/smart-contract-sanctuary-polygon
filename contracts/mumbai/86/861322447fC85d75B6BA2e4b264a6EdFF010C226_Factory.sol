...
contract ALP is AlpToken, IAlp, Ownable, MixinResolver {
    ...
    function deposit(uint256 value) external returns (uint256) {
        ...
        if (stakeLimitData.isStakingLimitSet()) {
            ...
            require(value <= currentStakeLimit, "STAKE_LIMIT");

            STAKING_STATE_POSITION.setStorageStakeLimitStruct(
                stakeLimitData.updatePrevStakeLimit(currentStakeLimit - value)
            );
        }
    ...
    function removeAvailableTokens(address[] calldata tokens)
        external
        onlyOwner
    {
        uint256 index;
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (index > 0) {
                ...
                availableTokensArr[index - 1] = availableTokensArr[
    ...
    function _getCurrentStakeLimit(StakeLimitState.Data memory _stakeLimitData)
        internal
        view
        returns (uint256)
    {
...
abstract contract AlpToken is IERC20, Pausable {
    ...
    function getSharesByPooled(uint256 _amount) public view returns (uint256) {
        ...
        if (totalPooled == 0) {
            ...
            // console.log(_amount, _getTotalShares(), totalPooled);
        ...
        returns (uint256 newTotalShares, uint256 redeem)
    {
        ...
        console.log(accountShares, _sharesAmount);
...
library StakeLimitUtils {
    ...
    */
    function setStakingLimit(
        ...
        require(_maxStakeLimit <= 79228162514264337593543950335, "TOO_LARGE_MAX_STAKE_LIMIT");
        require(_maxStakeLimit >= _stakeLimitIncreasePerBlock, "TOO_LARGE_LIMIT_INCREASE");
        require(
            (_stakeLimitIncreasePerBlock == 0)
            || (_maxStakeLimit / _stakeLimitIncreasePerBlock <= 4294967295),
    ...
    */
    function updatePrevStakeLimit(
        ...
        assert(_newPrevStakeLimit <= 79228162514264337593543950335);
