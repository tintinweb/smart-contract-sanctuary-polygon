    ...
    PausableUpgradeable
{
    ...
    /**
     * @dev secondary sale mapping
     * @dev address is for the tokenAddress and uint256 tokenId
     */
    mapping(address => mapping(uint256 => SecondarySale))
        public secondarySaleTokens;
    ...
    event TransferSellersCut(address seller, uint256 amount);

    event LockedTokenForSecondarySales(address tokenAddress, uint256 tokenId);
    ...
    event TransferBiddersCut(address bidder, uint256 amount);
    ...
    function withdrawBiddingAmount(uint256 tokenId) public {
        ...
        for (uint256 i = 1; i < auction.bidderSize; i++) {
            if (auction.bidders[i].bidder == msg.sender) {
                ...
                emit TransferBiddersCut(
                    msg.sender,
                    auction.bidders[i].bidAmount
                );
                break;
            }
        }
    ...
    /**
     * @dev distributing the bid amount. Note: only users who losts the bid will be get the bid amount back
     * @param tokenId the NFT
     */
    function _distributeBiddersAmount(uint256 tokenId) private {
        Auction storage auction = tokenIdToAuction[tokenId];
        ...
        for (uint256 i = 0; i < auction.bidderSize; i++) {
            if (auction.bidders[i].bidder != auction.highestBidder) {
                if (auction.bidders[i].claimedCuts) {
                    require(
                        auction.bidders[i].bidAmount < address(this).balance,
                        "NFTAuctionV3: Insufficient contract balance"
                    );
                    (bool succeed, bytes memory data) = auction
                        .bidders[i]
                        .bidder
                        .call{value: auction.bidders[i].bidAmount}("");
                    require(succeed, "NFTAuctionV3: Failed to withdraw");
                    auction.bidders[i].claimedCuts = true;
                    emit TransferBiddersCut(
                        auction.bidders[i].bidder,
                        auction.bidders[i].bidAmount
                    );
                }
            }
    ...
    /**
     * @dev End auction. Distribute bidders amount, transfer NFT to winner and the owners cut
    ...
    function endAuction(uint256 tokenId) public {
        ...
        require(
            !_isOwnerOf(auction.highestBidder, tokenId),
            "NFTAuctionV3: Already claimed the NFT"
        );

        _transferNFT(auction.highestBidder, tokenId);

        _distributeBiddersAmount(tokenId);

        _transferSellersCut(tokenId);
    ...
    function _transferSellersCut(uint256 tokenId) private {
        Auction storage auction = tokenIdToAuction[tokenId];
        require(
            !auction.seller.claimedCuts,
            "NFTAuctionV3: Seller has already claimed his cut"
        );
        require(
            auction.currentPrice < address(this).balance,
            "NFTAuctionV3: Insufficient contract balance"
        );
        (bool succeed, bytes memory data) = auction.seller.seller.call{
            value: auction.currentPrice
        }("");
        require(succeed, "NFTAuctionV3: Failed to send sellers cut");
        ...
        emit TransferSellersCut(auction.seller.seller, auction.currentPrice);
    }
    ...
    function lockNFTForSale(address tokenAddress, uint256 tokenId) public {
        require(
            _isOwnerOf(msg.sender, tokenId),
            "NFTAuctionV3: Token owner mismatch"
        );

        _escrow(msg.sender, tokenId);

        SecondarySale storage sale = secondarySaleTokens[tokenAddress][tokenId];
        //SecondarySale memory sale =
        sale.tokenAddress = tokenAddress;
        sale.tokenId = tokenId;
        sale.tokenOwner = msg.sender;

        emit LockedTokenForSecondarySales(tokenAddress, tokenId);
    }
    ...
    function transferSaleToken(
        ...
        require(
            sale.tokenAddress == tokenAddress,
            "NFTAuctionV3: Invalid token address"
        );
        require(sale.tokenId == tokenId, "NFTAuctionV3: Invalid tokenId");

        _transferNFT(receiver, tokenId);

        delete secondarySaleTokens[tokenAddress][tokenId];
        emit TransferSaleToken(tokenAddress, tokenId, receiver);
    }
}
