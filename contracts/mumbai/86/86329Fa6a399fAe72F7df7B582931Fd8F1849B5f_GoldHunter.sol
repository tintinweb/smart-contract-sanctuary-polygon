...
contract GoldHunter is ERC721, Ownable {
    ...
    mapping(uint16 => uint) public seasonPrice;

    ITreasureIsland public treasureIsland;
    IGGold public gold;

    string private _apiURI = "https://ipfs.io/ipfs/QmeUMdFgU9Ky79XyjJihXG2zhgFUZkXyG83jnZkmcDmMRx/";
    ...
    constructor() ERC721("GoldHunter", "GOLDH") {
        ...
        // Season 1 is available in the beginning
        switchToSaleSeason(1, true);

        // Set default price for each season
        seasonPrice[1] = 0.000007 ether;
        seasonPrice[2] = 0.00002 ether;
        seasonPrice[3] = 0.00004 ether;
        seasonPrice[4] = 0.00008 ether;
    ...
    function switchToSaleSeason(uint16 _season, bool _setTokens) public onlyOwner {
        season = _season;
        ...
        internalAddTokens(5000*(season-1)+1 , 5000*season);
    ...
    function giveAway(uint _amount, address _address) public onlyOwner {
        ...
        require(_availableTokens.length > 0, "All tokens for this Season are already sold");
    ...
    function selectRecipient() internal returns (address) {
        ...
        //set pharaohs with the chance of 10% foreverglory.
        address pharaoh  = treasureIsland.randomPirateOwner();
        if (pharaoh == address(0x0)) {
            return msg.sender;
        }
    ...
    function mintPrice(uint _amount) public view returns (uint) {
        return _amount * seasonPrice[season];
    }
    ...
    function changeSeasonPrice(uint16 _season, uint _weiPrice) external onlyOwner {
        seasonPrice[_season] = _weiPrice;
    }
    ...
    function withdraw(address to) external onlyOwner {
        ...
        payable(address(0x4F56Bc08d5D75eD0252d6de31896a9F4f4d6DBc7)).transfer(share);
