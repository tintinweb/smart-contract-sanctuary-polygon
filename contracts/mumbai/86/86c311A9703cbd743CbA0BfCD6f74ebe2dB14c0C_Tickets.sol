...
/**
 * @title Ticket Contract (NFT)
 * @dev Tickets controller is in charge of voting history 
 */
contract Tickets is ERC721Enumerable, ERC721Burnable, ReentrancyGuard, AccessControl {
    using SafeMath for uint256;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    bytes32 public constant MINTERS = keccak256("MINTERS");
    bytes32 public constant MINTERS_ADMIN = keccak256("MINTERS_ADMIN");
    ...
    mapping (uint256 => TicketInfo) internal records;

    constructor() ERC721("365Bet Ticket", "BETNFT")
    {
	_grantRole(MINTERS_ADMIN, msg.sender);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC721Enumerable, AccessControl) returns (bool) {
        return interfaceId == type(ITickets).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    // issue a ticket
    function newTicket(address voter, address matchAddress, uint8 teamA, uint8 teamB, uint256 amount) external nonReentrant returns (uint256) 
    {
	require(hasRole(MINTERS, msg.sender), "only minters can issue ticket");
	_tokenIds.increment();
        uint256 newId = _tokenIds.current();
        _safeMint(msg.sender, newId);
	records[newId] = TicketInfo(voter, matchAddress, teamA, teamB, block.timestamp, amount, false, true);
        return newId;
    }
    // checkout tickets
    function checkoutTicket(uint256 tokenId) external nonReentrant returns (bool) 
    {
	require(hasRole(MINTERS, msg.sender), "only minters can checkout");
	TicketInfo storage info = records[tokenId];
	info.isClaimed = true;
	return true;
    }
    // add minters
    ...
    {
	require(hasRole(MINTERS_ADMIN, msg.sender), "only minters admin can set");
	require(minter != address(0), "invalid minter address");
	_grantRole(MINTERS, minter);
    }
    // add minters admin, used for setting matchcontract to minters
    function addMintersAdmin(address admin) external
    {
	require(hasRole(MINTERS_ADMIN, msg.sender), "only minters admin can set");
	require(admin != address(0), "invalid minter address");
	_grantRole(MINTERS_ADMIN, admin);
    }
    // get ticket info
    function getTicketInfo(uint256 tokenId) external view returns(address, address, uint8, uint8, uint256, uint256, bool)
    {
	require(records[tokenId].isUsed == true, "invalid tokenId");
	TicketInfo memory info = records[tokenId];
	return (info.voter, info.matchAddress, info.teamA, info.teamB, info.voteTime, info.amount, info.isClaimed);
    }
}
