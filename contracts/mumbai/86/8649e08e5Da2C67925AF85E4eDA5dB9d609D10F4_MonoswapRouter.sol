...
// contract MonoswapRouter is Initializable, OwnableUpgradeable {
  ...
  function setCore (address _core) onlyOwner external {
    core = _core;
  }
  ...
  function addLiquidityPair (address _token, 
    ...
    liquidity = IMonoswap(core).addLiquidityPair(_token, vcashAmount, tokenAmount, msg.sender, to);
  }
  ...
  function addLiquidity (address _token, uint256 _amount, address to) external override returns(uint256 liquidity) {
    liquidity = IMonoswap(core).addLiquidityPair(_token, 0, _amount, msg.sender, to);
  }  
  ...
  function addLiquidityETH (address to) external override payable returns(uint256 liquidity)  {
    ...
    liquidity = IMonoswap(core).addLiquidityPair(WETH, 0, msg.value, core, to);
  }  
  ...
  function _removeLiquidity (address user, address _token, uint256 liquidity) view public returns(
    ...
    return IMonoswap(core)._removeLiquidity(user, _token, liquidity);
  }
  ...
  function tokenInsurance (address _token) external view returns(uint coverage) {
    coverage = IMonoswap(core).tokenInsurance(_token);
    console.log("coverage");
    console.log(coverage);
  }

  function getPool (
    address _token) view external returns (
    uint256 poolValue, 
    uint256 tokenBalanceVcashValue, 
    uint256 vcashCredit, 
    uint256 vcashDebt) {
      return IMonoswap(core).getPool(_token);
  }
  ...
  function listNewToken (
    ...
    uint256 vcashAmount, 
    uint256 tokenAmount,
    address to) external returns(
    uint _pid, uint256 liquidity) {
      return IMonoswap(core).listNewToken(_token, _price, 
        vcashAmount, tokenAmount, msg.sender, to);
  }

  function pools(address _token) external view returns (IMonoswap.PoolInfo memory) {
    return IMonoswap(core).pools(_token);
  }

  function getAmountIn(
    address tokenIn, address tokenOut, 
    uint256 amountOut) public view returns (
    uint256 tokenInPrice, uint256 tokenOutPrice, 
    uint256 amountIn, uint256 tradeVcashValue) {
      return IMonoswap(core).getAmountIn(tokenIn, tokenOut, amountOut);
  }

  function getAmountOut(
    address tokenIn, address tokenOut, 
    uint256 amountIn) public view returns (
    uint256 tokenInPrice, uint256 tokenOutPrice, 
    uint256 amountOut, uint256 tradeVcashValue) {
      return IMonoswap(core).getAmountIn(tokenIn, tokenOut, amountIn);
  }
}
...
interface IMonoswap {
  ...
  function _removeLiquidity (
    address user, address _token, 
    uint256 liquidity) view external returns(
    uint256 poolValue, uint256 liquidityIn, 
    uint256 vcashOut, uint256 tokenOut);

  function tokenInsurance (address _token) external view returns(uint);
  ...
  function getAmountIn(
    address tokenIn, address tokenOut, 
    uint256 amountOut) external view returns (
    uint256 tokenInPrice, uint256 tokenOutPrice, 
    uint256 amountIn, uint256 tradeVcashValue);

  function getAmountOut(
    address tokenIn, address tokenOut, 
    uint256 amountIn) external view returns (
    uint256 tokenInPrice, uint256 tokenOutPrice, 
    uint256 amountOut, uint256 tradeVcashValue);
}
