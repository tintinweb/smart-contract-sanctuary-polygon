...
contract multisigAccountV2 is Account, verifySignature {
    ...
    mapping(bytes32 => bool) executedTransaction;
    ...
    function execute(
        ...
        bytes32 messageHash = getMessageHash(_target, _value, _calldata, _uuid);
         require(!executedTransaction[messageHash], "transaction already executed");
        ...
        require(
            ...
            "Not enough signatures from the multisig"
        );
        delete transactionToAddressesConfirmations[messageHash];
        executedTransaction[messageHash] = true;
    ...
    function executeBatch(
        ...
        require(
            _targets.length == _calldatas.length && _targets.length == _values.length,
        ...
        for (uint256 i = 0; i < _targets.length; i++) {
            // require(signatures.length )
             messageHash = getMessageHash(_targets[i], _values[i], _calldatas[i], _uuids[i]);
             require(!executedTransaction[messageHash]);
            for (uint256 j = 0; j < signatures.length; j++) {
                temp = recoverSigner(messageHash, signatures[i][j]);
                // If the signer of the signature is owner of that multisig and it didnt happened accidentally to be included more than one times in the signatures array
                // assign him us a true transaction confirmation
                if (
                    isOwner[temp] &&
                    !transactionToAddressesConfirmations[messageHash].contains(temp)
                ) {
                    transactionToAddressesConfirmations[messageHash].add(temp);
                }
            }
            require(
                numConfirmationsRequired <=
                    transactionToAddressesConfirmations[messageHash].length(),
                "Not enough signatures from the multisig"
            );
            delete transactionToAddressesConfirmations[messageHash];
            executedTransaction[messageHash] = true;

            _call(_targets[i], _values[i], _calldatas[i]);
        }
    }
...
contract verifySignature {
    ...
    */
    ...
    function getMessageHash(
        ...
        return getEthSignedMessageHash(keccak256(abi.encodePacked(_to, _amount, _data,_uuid)));
    }
    ...
    function getEncoded(        
        ...
        uint _amount,
        bytes memory _data,
        string memory _uuid)public pure returns(bytes memory){
        return abi.encodePacked(_to, _amount, _data,_uuid);
    }
