...
interface INFTCustom {
    function firstOwner(uint256 _id) external view returns (address);
}
...
import "./interfaces/INFTCustom.sol";
 ...
 * @dev This contract is used to create a listing of ERC721 and ERC1155 token, which users can buy in direct sale.
    ...
    ErrorCodes
{
    ...
    function createListing(
        ...
        require(
            tokenAmountToList > 0, 
            throwError(INVALID_TOKEN_AMOUNT)
        );

        require(
            _params.primarySplitAddress != address(0),
    ...
    function modifyListing(
        ...
        uint256 safeTokenAmountToList = getSafeQuantity(
            _tokenType,
            _params.quantityToList
        );
    ...
    function validateSaleParameters(
        ...
        if (
            INFTCustom(listingData.assetContract).firstOwner(
                listingData.tokenId
    ...
    function executeSale(
        ...
        if (_currencyAmountToTransfer > 0) {
            address payee = _targetListing.primarySplitAddress != address(0)
                ? _targetListing.primarySplitAddress
        ...
        if (
            INFTCustom(_targetListing.assetContract).firstOwner(
                _targetListing.tokenId
            ) == collectionData.owner
        ) {
     ...
     * @param _currency address of currency which is being transferred.
     ...
     * @param _currency address of ERC20 token which is being transferred.
     ...
     * Currency should be valid.
     */
    function _withdraw(address to, address currency, uint256 amount) internal {
    ...
    function getTokenListingCount(
        address _assetContract,
        uint256 _tokenId
    ) internal view returns (uint256 listingCount, uint256 activeCount) {
        ...
        for (uint256 i; i < listingCount; ++i) {
            ...
            if (
                listings[_tokenListings[_assetContract][_tokenId][i]].quantity >
                0
            ) {
                activeCount++;
            }
    ...
    function getTokenListing(
        address _assetContract,
        uint256 _tokenId
    ) public view returns (Listing[] memory activeListings) {
     ...
     */
    function setAuthenticatorContract(
    ...
    /// @dev Stores the maintainer role
