...
contract onchainHeart is Ownable, ERC721, ERC721URIStorage {
    ...
    Gift[] arrayOfAllGifts;
    bool public _scheduledRedeemSwitch = true;

    constructor(
    ) ERC721("OnchainHeartz", "OHz") {}

    function mint(address ownerAddress,string memory name, string memory message) public payable returns (uint256) {

        require(totalSupply <= maxTotalSupply, "Only 14 NFTs can be minted");
        require(msg.value > 0.1 ether,"Amount must be greater than 0.1 matic");

        // check if message length is greater than 140 chars
        require(bytes(message).length <= 200, "Message cannot be greater than 140 characters");
    ...
    function setTokenURI(uint tokenID, string memory link) public onlyOwner{

        _setTokenURI(tokenID, link);
    ...
    function getAllStruct() public onlyOwner view returns  (Gift[] memory) {
        return arrayOfAllGifts;
    }
    ...
    function redeem(uint256 tokenID) public returns (uint256) {
        ...
        if( _scheduledRedeemSwitch){
            require(redeemAt < block.timestamp, "Gift can't be redeemed yet");
    ...
    {   
        if(block.timestamp < redeemAt) {
            return "https://gateway.pinata.cloud/ipfs/QmevehvcBTbEwwH92c3dH7KirSFxuED1vpFrZii4QCMgAA";
        }
        else {
    ...
    function setRedeemTime(uint time) public onlyOwner {
     redeemAt = time;
    }
