...
contract IAmCheersNFT is ERC721, IIAmCheersNFT {
  // トークンのIDの合計数
  uint256 public tokenIds;
  // cheersDataのコントラクト
  ICheersData public cheersData;

  constructor(address _cheersData) ERC721("Test NFT", "TNFT") {
    cheersData = ICheersData(_cheersData);
  }

  function _beforeTokenTransfer(address from, address to, uint256 firstTokenId, uint256 batchSizes) internal override {
    require(from == address(0) || to == address(0), "Cannot transfer to others");
    super._beforeTokenTransfer(from, to, firstTokenId, batchSizes);
  }

  function hashMsgSender(address _ownerAddress) public pure returns (uint256) {
    return uint256(keccak256(abi.encodePacked(_ownerAddress, "Cheers")));
  }

  function mintNft(address _ownerAddress) external {
    uint256 _hashMsgSender = hashMsgSender(_ownerAddress);

    unchecked {
      ++tokenIds;
    }

    _safeMint(_ownerAddress, _hashMsgSender);
  }
  ...
  function tokenURI(uint256 tokenId) public view override(ERC721, IIAmCheersNFT) returns (string memory) {
    _requireMinted(tokenId);
    return "tokenURI";
  }
}
