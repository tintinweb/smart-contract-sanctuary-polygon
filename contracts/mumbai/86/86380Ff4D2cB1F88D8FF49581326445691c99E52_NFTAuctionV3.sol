    ...
    PausableUpgradeable
{
    ...
    function _createAuction(
        ...
        require(!_hasAuctionCreated(auction), "NFTAuctionV3: Auction exists");
        require(
            endTime >= 1 minutes,
            "NFTAuctionV3: Auction duration must be atleast 1 minutes"
        );
        ...
        Seller memory sellerStruct = Seller(seller, tokenId, false);

        auction.seller = sellerStruct;
    ...
    function _bid(uint256 tokenId, uint256 bidAmount) internal {
        ...
        require(
            ...
            "NFTAuctionV3: The auction has not created"
        );
        require(
            _isAuctionActive(auction),
            "NFTAuctionV3: The Auction has ended"
        );
        require(
            msg.sender != auction.seller.seller,
            "NFTAuctionV3: Auction owner cannot bid"
        );
        require(
            auction.startingPrice <= bidAmount,
            "NFTAuctionV3: The amount should be greater than starting price"
        );
        require(
            auction.currentPrice < bidAmount,
            "NFTAuctionV3: The bid amount should be greater than the previous bid amount"
        );

        //pushing the details of the bidder to the mapping to act according to the auction result
        Bidder memory bidder = Bidder(msg.sender, bidAmount, tokenId, false);
    ...
    /**
     * @dev withdraw sellers cut after auction ends
     * @param tokenId The NFT tokenId
     */
    function _withdrawSellerCuts(uint256 tokenId) internal {
        Auction storage auction = tokenIdToAuction[tokenId];

        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV3: Auction does not exist"
        );
        require(
            auction.seller.seller == msg.sender,
            "NFTAuctionV3: Only Auction creator can call"
        );
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV3: Auction has not ended yet"
        );
        require(
            !auction.seller.claimedCuts,
            "NFTAuctionV3: Seller has already withdrew cut"
        );
        require(
            auction.highestBidder != address(0),
            "NFTAuctionV3: The Auction does not have any bids"
        );

        (bool succeed, bytes memory data) = msg.sender.call{
            value: auction.currentPrice
        }("");
        require(succeed, "NFTAuctionV3: Failed to withdraw");
    }

    /**
     * @dev send sellers cut after auction has ended
     ** Note: function accessible only for contract owner
     * @param tokenId the NFT tokenId
     */
    function transferSellersCut(uint256 tokenId) public onlyOwner {
        Auction storage auction = tokenIdToAuction[tokenId];
        ...
        (bool succeed, bytes memory data) = auction.seller.seller.call{
            value: auction.currentPrice
        }("");
        require(succeed, "NFTAuctionV3: Failed to withdraw");
    }
    ...
    function claimNFT(uint256 tokenId) public {
        ...
        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV3: Auction has not been created yet"
        );
        require(
            auction.highestBidder == msg.sender,
            "NFTAuctionV3: Winner can only claim NFT"
        );
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV3: Auction has not ended"
        );
        require(
            !_isOwnerOf(msg.sender, tokenId),
            "NFTAuctionV3: Already claimed the NFT"
        );
    ...
    /**
     * @dev send the NFT to the auction winner. Note: function accessible only for contract owner
     * @param tokenId the NFT tokenId
     * @param nftClaimer the winner of the auction
     */
    function transferNFTToWinner(uint256 tokenId, address nftClaimer)
        public
        onlyOwner
    {
        Auction storage auction = tokenIdToAuction[tokenId];
        ...
        require(
            auction.highestBidder == nftClaimer,
            "NFTAuctionV3: Winner can only claim NFT"
        );
        require(
            !_isOwnerOf(msg.sender, tokenId),
        ...
        _transferNFT(nftClaimer, tokenId);

        emit ClaimNFT(nftClaimer, tokenId);
    }
    ...
    function setNFTInterface(address nftContract) external onlyOwner {
        require(nftContract != address(0), "NFTAuctionV3: Invalid Address");
    ...
    function withdrawBiddingAmount(uint256 tokenId) public {
        ...
        require(
            !_isAuctionActive(auction),
            "NFTAuctionV3: The auction is not ended yet"
        );
        require(
            auction.highestBidder != msg.sender,
            "NFTAuctionV3: Winner cannot withdraw"
        );

        // transfer bid amount respectively

        for (uint256 i = 1; i < auction.bidderSize; i++) {
            if (auction.bidders[i].bidder == msg.sender) {
                require(
                    !auction.bidders[i].claimedCuts,
                    "NFTAuctionV3: Already withdrew"
                );
                require(
                    auction.bidders[i].bidAmount < address(this).balance,
                    "NFTAuctionV3: Insufficient contract balance"
                );
                (bool succeed, bytes memory data) = msg.sender.call{
                    value: auction.bidders[i].bidAmount
                }("");
                require(succeed, "NFTAuctionV3: Failed to withdraw");
                auction.bidders[i].claimedCuts = true;
                break;
            }
        }
    ...
    /**
     * @dev send bidders amount after the auction has a winner
     ** Note: function accessible only for contract owner
     * @param tokenId the NFT
     * @param bidderAddress the address of the bidder
     */
    function sendBiddersAmounts(uint256 tokenId, address bidderAddress)
        public
        onlyOwner
    {
        Auction storage auction = tokenIdToAuction[tokenId];
        ...
        for (uint256 i = 1; i < auction.bidderSize; i++) {
            if (auction.bidders[i].bidder == bidderAddress) {
                require(
                    !auction.bidders[i].claimedCuts,
                    "NFTAuctionV3: Already withdrew"
                );
                ...
                (bool succeed, bytes memory data) = bidderAddress.call{
                    value: auction.bidders[i].bidAmount
                }("");
                require(succeed, "NFTAuctionV3: Failed to withdraw");
                auction.bidders[i].claimedCuts = true;
    ...
    function createAuction(
        ...
        require(
            ...
            "NFTAuctionV3: Only NFT owner can create auction"
        );
        ...
        )
    {
        ...
        require(
            _hasAuctionCreated(auction),
            "NFTAuctionV3: Auction not created"
        );
        return (
    ...
    function cancelAuction(uint256 tokenId) external {
        ...
        require(
            msg.sender == auction.seller.seller,
            "NFTauctionV3: Only auction creator can cancel the auction"
        );

        if (block.timestamp >= auction.startedAt) {
            require(
                auction.bidderSize == 0,
                "NFTauctionV3: Auction with bidders cannot be cancelled"
            );
        ...
        onlyOwner
    {
        ...
        // require(_isAuctionActive(auction), "NFTAuctionV3: Auction has ended");
