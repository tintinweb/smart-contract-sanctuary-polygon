...
contract Insurance is FunctionsClient, VRFConsumerBaseV2, ConfirmedOwner {
  ...
  mapping(bytes32 => ResponseData) public responseData;

  mapping(bytes32 => bytes) public insuranceQuoteResponses;
  mapping(bytes32 => address) public functionsRequestOwners;
  mapping(address => bytes) public latestUserResponse;
  ...
  function estimateInsurance(
    ...
    uint256 newPolicyIndex = insuranceDataMapping[msg.sender].length - 1;
    ...
    responseData[assignedReqID] = ResponseData({user: msg.sender, policyIndex: newPolicyIndex, response: ""});
    latestRequestId = assignedReqID;
    functionsRequestOwners[assignedReqID] = msg.sender;
    emit OCRRequest(assignedReqID);
    return assignedReqID;
  }

  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ResponseData storage data = responseData[requestId];
