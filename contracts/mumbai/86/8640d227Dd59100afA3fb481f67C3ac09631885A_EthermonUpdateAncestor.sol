...
// File: contracts/EthermonUpdateAncestors.sol
...
contract EthermonUpdateAncestor is EthermonEnum, BasicAccessControl {
    using SafeERC20 for IERC20;
    ...
    function setContract(address _worldContract)
        external
        onlyModerators
    {
        worldContract = _worldContract;
    ...
    function checkAncestors(
        uint32 _classId,  uint256 index
    ) public view returns ( uint32 ancestor
            ) {
        EtheremonWorld world = EtheremonWorld(worldContract);
        // check ancestor
        ancestor = world.getClassPropertyValue(_classId, PropertyType.ANCESTOR, index);
        return ancestor;
