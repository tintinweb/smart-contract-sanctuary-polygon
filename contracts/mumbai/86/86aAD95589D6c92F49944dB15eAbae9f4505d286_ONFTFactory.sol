...
contract ONFTFactory {
    ...
    function create(
        ...
        ONFT721 onft = new ONFT721(_name, _symbol, _collectionURI, _tokensURI, msg.sender, minGasToTransfer, lzEndpoint);
...
contract ONFT721 is ONFT721Core, ERC721, IONFT721, IERC2981Royalties {
    ...
    constructor(
        ...
        require(bytes(_collectionURI).length > 0 && bytes(_tokensURI).length > 0);
    ...
    function _debitFrom(address _from, uint16, bytes memory, uint _tokenId) internal virtual override {
        require(_isApprovedOrOwner(msg.sender, _tokenId), "ONFT721: send caller is not owner nor approved");
