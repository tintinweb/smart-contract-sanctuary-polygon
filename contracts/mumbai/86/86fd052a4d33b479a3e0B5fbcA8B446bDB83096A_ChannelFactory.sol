...
import "./FactoryInterfaces/IDropsFactory.sol";
import "./FactoryInterfaces/IMutualAccessFactory.sol";

contract ChannelFactory is Ownable {
    IDropsFactory private _dropsFactory;

    IMutualAccessFactory private _mutualAccessFactory;

    event FactoriesDeployed(address mutualAccessDeployed, address dropDeployed);

    constructor(address dropsFactory_, address mutualAccessFactory_) {
        _dropsFactory = IDropsFactory(dropsFactory_);
        _mutualAccessFactory = IMutualAccessFactory(mutualAccessFactory_);
    }
    ...
    function deployAll(
        ...
        address mutualAccess = deployMutualAccessFactory(
            tickets,
            royaltyRecipients,
            royalties,
            expireAfterHours,
        ...
        address dropFactory = deployDropFactory();

        emit FactoriesDeployed(mutualAccess, dropFactory);
    }

    function setDropFactory(address dropFactory) public onlyOwner {
        _dropsFactory = IDropsFactory(dropFactory);
    }

    function setMutualAccessFactory(address mutualAccessFactory)
        public
        onlyOwner
    {
        _mutualAccessFactory = IMutualAccessFactory(mutualAccessFactory);
    }

    function deployMutualAccessFactory(
        MutualAccess.TicketTierInput[] calldata tickets,
        MutualAccess.RoyaltyRecipient[] calldata royaltyRecipients,
        MutualAccess.FeePercents calldata royalties,
        uint256 expireAfterHours,
        ...
        address mutualAccess = _mutualAccessFactory.deployMutualAccessAndMint(
            tickets,
            royaltyRecipients,
            royalties,
            expireAfterHours,
        ...
        return address(mutualAccess);
    }

    function deployDropFactory() public returns (address) {
        return address(_dropsFactory.deployDrop());
    }
}
...
interface IDropsFactory {
    function deployDrop() external returns (address);
}
