...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, tokenId.toString(),".json")) : "ipfs://QmVC9jZeB2enRvLhnYp4JgKkMavjTChuV7YhUGSJsNqPW5";
    }
...
interface IAstroSapiens_NFT {
    ...
    function OwnershipChange(address _newOnwer) external;
}

contract FINALTESTING_NFT is ERC721Royalty, Ownable, ContextMixin, ReentrancyGuard, AccessControl {
    ...
    uint256 public MAX_VIP_1_List = 2000; // Maximum NFT minted
    ...
    bytes32 public constant ADDING_LIST_ROLE = keccak256("ADDING_LIST");
    bytes32 public constant FEE_CHANGER_ROLE = keccak256("FEE_CHANGER_ROLE");

    constructor() ERC721("FinalTesting","Final") {
        _grantRole(DEFAULT_ADMIN_ROLE, 0x49A1A31905AA9d039ff04937D94bF9E612b8ECB9);
        _grantRole(URI_SETTER_ROLE, 0x49A1A31905AA9d039ff04937D94bF9E612b8ECB9);
        _grantRole(ADDING_LIST_ROLE, 0x8d8d4ae71e3079F83C1d5e316810Ff214Ef88b0a);
        _grantRole(FEE_CHANGER_ROLE, 0x9e57Dc51b95a09A31e093B2ec19961Dd066032F6);
    }

    event Gifted(address indexed sender, uint256 nftStart, uint256 nftEnd);
    event OwnershipTransfer(address indexed oldAddresss, address indexed newAddress);
    ...
    event NewAllowList(uint256 indexed numberofUsers);
    event NewVIPList2(uint256 indexed numberofUsers);
    event NewVIPList(uint256 indexed numberofUsers);
    event MultiSigAddress(address indexed newMultiSigAddress);
    ...
    function withdraw() external {
        require(msg.sender == MultiSigWallet, "Calller is not the MultiSig");
        uint256 balance = address(this).balance;
        payable(owner()).transfer(balance);
        emit WithdrawFee(msg.sender, balance);
    }

    function AddMultiSigWallet(address _multiSigAddress) public onlyRole(FEE_CHANGER_ROLE) {
        MultiSigWallet = _multiSigAddress;
        emit MultiSigAddress(_multiSigAddress);
    }
    ...
    function addVIPList(address[] memory _users) public onlyRole(ADDING_LIST_ROLE) nonReentrant {
        ...
        emit NewAllowList(_users.length);
    }
    ...
    function VIPList(uint256 numberOfTokens) public saleFlip nonReentrant payable {
        require(numberOfTokens > 0,"The number of NFTs minted must be > 0");
        require(VIP1Supply.add(numberOfTokens) <= MAX_VIP_1_List, "Not enough NFTs remaining");
        require(msg.value >= price.mul(numberOfTokens), "Insufficient funds");
        require(isVIPlist[msg.sender].add(numberOfTokens) <= 5, "You cannot mint more than 5 NFTs");
        ...
        VIP1Supply += numberOfTokens;
        emit VIPMint(msg.sender, numberOfTokens, msg.value);
    }

    // @dev Only wallet addres added in addVIPList added able to mint
    function VIPList2(uint256 numberOfTokens) public saleFlip2 nonReentrant payable {
        require(numberOfTokens > 0,"The number of NFTs minted must be > 0");
        require(totalMintedNumber.add(numberOfTokens) <= MAX_VIP_Allow_List, "Not enough NFTs remaining");
        require(msg.value >= price.mul(numberOfTokens), "Insufficient funds");
        require(isVIPlist2[msg.sender].add(numberOfTokens) <= 5, "You cannot mint more than 5 NFTs");
        for(uint i = 0; i < numberOfTokens; i++) {
            uint256 newItemId = _tokenIds.current();
            _safeMint(msg.sender, newItemId+1);
            _setTokenRoyalty(newItemId+1, msg.sender, royaltyPer);
    ...
    function AllowList(uint256 numberOfTokens) public saleFlip3 nonReentrant payable {
        require(numberOfTokens > 0,"Number of NFTs minted must be > 0 ");
        require(totalMintedNumber.add(numberOfTokens) <= MAX_VIP_Allow_List, "Not enough NFTs remaining");
        require(msg.value >= price.mul(numberOfTokens), "Insufficient funds");
        require(isAllowlist[msg.sender].add(numberOfTokens) <= 5, "You cannot mint more than 5 NFTs");
        for(uint i = 0; i < numberOfTokens; i++) {
            uint256 newItemId = _tokenIds.current();
            _safeMint(msg.sender, newItemId+1);
            _setTokenRoyalty(newItemId+1, msg.sender, royaltyPer);
    ...
    function GiftList(uint256 _startingNumber, uint256 _endNumber) external onlyOwner nonReentrant {
        require(_startingNumber > 10000, "Starting number value must be > 10000");
        require(_endNumber <= 10500, "Ending number value must be <= 10500");
    ...
    // @dev Ownership change from MultiSig
    function OwnershipChange(address _newOnwer) external onlyOwner nonReentrant {
        transferOwnership(_newOnwer);
        emit OwnershipTransfer(msg.sender, _newOnwer);
    }
}
