...
import "../../interfaces/IBaseDegenDNSFactory.sol";
import "../../interfaces/IBaseDegenDNS.sol";
import "../../lib/strings.sol";
import "@openzeppelin/contracts/utils/Strings.sol";
import "@openzeppelin/contracts/token/ERC721/extensions/IERC721Metadata.sol";

/// @title Degen Domains Resolver v2
/// @author CipherLabs - forked from OG author Tempe Techie
/// @notice This contract resolves all degen domains and TLDs on the particular blockchain where it is deployed
contract DegenResolverNonUpgradable is Ownable {
  ...
  function getDefaultDomain(address _addr, string memory _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseDegenDNSFactory(factories[i]).tldNamesAddresses(_tld);

      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseDegenDNS(tldAddr).defaultNames(_addr));
      }
  ...
  function getDefaultDomains(address _addr) public view returns(string memory) {
    ...
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      string[] memory tldNames = IBaseDegenDNSFactory(factories[i]).getTldsArray();

      for (uint256 j = 0; j < tldNames.length; ++j) {
        string memory tldName = tldNames[j];
        address tldAddr = IBaseDegenDNSFactory(factories[i]).tldNamesAddresses(tldName);
        string memory defaultName = IBaseDegenDNS(tldAddr).defaultNames(_addr);
  ...
  function getDomainHolder(string memory _domainName, string memory _tld) public view returns(address) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return address(IBaseDegenDNS(tldAddr).getDomainHolder(_domainName));
      }
  ...
  function getDomainData(string memory _domainName, string memory _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        return string(IBaseDegenDNS(tldAddr).getDomainData(_domainName));
      }
  ...
  function getDomainTokenUri(string memory _domainName, string memory _tld) public view returns(string memory) {
    ...
    for (uint256 i = 0; i < fLength;) {
      ...
      if (tldAddr != address(0) && !isTldDeprecated[tldAddr]) {
        (, uint256 _tokenId, , ) = IBaseDegenDNS(tldAddr).domains(_domainName);
  ...
  function getTldAddress(string memory _tldName) public view returns(address) {
    uint256 fLength = factories.length;
    for (uint256 i = 0; i < fLength;) {
      address tldAddr = IBaseDegenDNSFactory(factories[i]).tldNamesAddresses(_tldName);
...
interface IBaseDegenDNSFactory {
...
interface IBaseDegenDNS is IERC721 {

  struct Domain {
    string name; // domain name that goes before the TLD name; example: "ilove" in "ilove.ben"
    uint256 tokenId;
    address holder;
    string data; // stringified JSON object, example: {"description": "Some text", "twitter": "@lovestacofish", "friends": ["0x123..."], "url": "https://degendomains.io"}
  }
