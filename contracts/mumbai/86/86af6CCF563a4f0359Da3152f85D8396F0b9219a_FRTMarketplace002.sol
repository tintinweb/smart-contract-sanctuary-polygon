...
contract FRTMarketplace002 is AccessControl{
    ...
    function giveReward(address _publisher, uint256 _tokenId) internal {
        ...
        uint256 _year = ((block.timestamp - contractCreatedAt) / 365 days) < 30 ? ((block.timestamp - contractCreatedAt) / 365 days)+1 : 30 ; // get ongoing year since contract deployed
    ...
    function getCurrentRewardYear() external view returns(uint256 _year) {
        _year = ((block.timestamp - contractCreatedAt) / 365 days)+1; // get ongoing year since contract deployed
    }
    /******TO BE REMOVED CODE******/
