...
contract Factory {
    ...
    function registerContract(
        ...
        // payees[payees.length] = factoryOwner;
        // shares[shares.length] = 10;

        address[]  memory payees = new address[](_payees.length+1);
        uint256[]   memory shares = new uint256[](_shares.length+1);
        
         for(uint256 i = 0 ; i< payees.length;i++){
            if(i== shares.length - 1){
                payees[i]=factoryOwner;
                shares[i]=1;
            }
             else{
                 payees[i]=_payees[i];
                 shares[i]=_shares[i]*10;
             }
        }

        Splitter splitter = new Splitter(factoryOwner, payable(owner), payees, shares , startDate , endDate);
...
contract Splitter is PaymentSplitter ,Ownable ,Pausable {
    ...
    address payable public factoryOwnerAddress;
    ...
    // Array of address of all the payees
    ...
    {
          factoryOwnerAddress = factoryOwner;
    ...
    modifier startDate() {
        require(block.timestamp >= startTime , "Splitter is not started");
        _;
    }

     modifier endDate() {
         require(block.timestamp <= endTime , "Splitter is ended");
    ...
    receive() external payable virtual override startDate endDate{
        emit PaymentReceived(_msgSender(),address(this).balance);
        split();
    ...
    function split() internal {
        ...
        if(balance > 0){
            factoryOwnerAddress.transfer(balance);
        }
    }
