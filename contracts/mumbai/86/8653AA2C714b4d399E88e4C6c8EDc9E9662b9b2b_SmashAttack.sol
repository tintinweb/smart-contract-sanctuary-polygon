...
contract SmashAttack is ERC1155LazyMint {
    ...
    function claimPowerUp() external {
        require(
            balanceOf[msg.sender][2] > 0,
            "You need an Iron Sword to claim a power-up!"
        );

        // Check the number of NFTs with tokenId 3
        uint256 numNFTs = balanceOf[msg.sender][3];

        // Calculate the number of claims available
        uint256 numClaims = numNFTs / 5;

        // Check if there are any claims available
        require(
            numClaims > 0,
            "You don't have enough Badges to claim a power-up"
        );
        ...
        if (randomNumber == 0) {
            ...
            _mint(msg.sender, POWER_UP_ID, numClaims, "");
        } else {
            // Claim a defense power-up
            _mint(msg.sender, DEFENSE_ID, numClaims, "");
        }
    }
    ...
    /**
     * @notice Use a defense power-up to defend against an attack
     * @dev The power-up is burned after all uses are exhausted
     */
