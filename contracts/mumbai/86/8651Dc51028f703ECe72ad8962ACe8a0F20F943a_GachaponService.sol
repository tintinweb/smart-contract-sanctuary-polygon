    ...
    ERC1155HolderUpgradeable
{
    ...
    function buyGachapon(uint256 _gachaId) external whenNotPaused {
        ...
        require(
            balanceVault.getBalance(msg.sender) >= gacha.gachaPrice + gachaponFees,
            "[GachaponService.buyGachapon] Insufficient balance"
        );
        balanceVault.decreaseBalance(msg.sender, gacha.gachaPrice + gachaponFees);
        balanceVault.increaseBalance(adminAddress, priceFees + gachaponFees);
        balanceVault.increaseBalance(gacha.gachaOwner, gacha.gachaPrice - priceFees);
        gacha.soldCount++;
    ...
    function _transferItems(
        ...
        for (uint256 i = 0; i < items.itemList.length; i++) {
            ...
            if (item.nftType == NftType.NFT721) {
                IERC721Upgradeable erc721 = IERC721Upgradeable(item.itemAddress);
                erc721.transferFrom(address(this), _reciever, item.itemTokenId);
            } else {
                IERC1155Upgradeable erc1155 = IERC1155Upgradeable(item.itemAddress);
                erc1155.safeTransferFrom(
                    address(this),
