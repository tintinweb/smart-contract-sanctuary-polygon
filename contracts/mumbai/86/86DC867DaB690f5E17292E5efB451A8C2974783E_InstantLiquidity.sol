...
interface IOnSaleTrackerExt is IOnSaleTracker{
    ...
    function isFtnftOnSale(ITangibleFractionsNFT ftnft) external view returns(ContractItem memory);
}
    ...
    IERC721Receiver
{
    ...
    function _sanityChecks(ITangibleNFT _nft, address _ftnft, uint256 _fingerprint, bool _sellInstant) internal returns(uint256 priceToPay){
        bool defIsUSD = !(defaultToken == IFactory(factory).TNGBL());
        if(!defIsUSD){ //it is TNGBL
            _volatilityCheck();
        }
        ...
        (uint256 weSellAt, , uint256 weBuyAt, uint256 weBuyAtStock, ) = _itemPrice(
            _nft,
        ...
        {
            if(_ftnft != address(0)){
                if(!onSaleTracker.isFtnftOnSale(ITangibleFractionsNFT(_ftnft)).selling){
    ...
    function sellInstant(
        ...
        uint256 priceToPay = _sanityChecks(_nft,address(0), _fingerprint, true);
    ...
    function buyInstant(ITangibleNFT _nft, uint256 _tokenId) external override {
        ...
        uint256 priceToTake = _sanityChecks(_nft,address(0),  _nft.tokensFingerprint(_tokenId), false);
        //take token
        IERC20(defaultToken).safeTransferFrom(msg.sender, address(this), priceToTake);
    ...
    function sellInstantFraction(
        ...
        uint256 priceFull = _sanityChecks(_nft,address(_ftnft), _fingerprint, true);

        uint256 priceToPay = (fractTokenShare * priceFull) /
            _ftnft.fullShare();
    ...
    function buyFractionInstant(
        ...
        uint256 priceFull = _sanityChecks(_nft,address(_ftnft), _fingerprint, false);
