    ...
    ReentrancyGuard
{
    ...
    uint256 public pricePresale = 0.01 ether;
    //Price of one NFT in sale
    uint256 public priceSale = 0.04 ether;
    ...
    mapping(address => uint256) public nftsWallet;

    //Addresses of all the members of the team
    address[] private _team = [0xC554C5DbaC4CF2Ebd55C586dDfdAB8D8Bee5EA83];

    //Shares of all the members of the team
    uint256[] private _teamShares = [100];

    constructor()
        ERC721A("Ball Project", "BALL")
    ...
    function setUpPresale() external onlyOwner {
        require(sellingStep == Steps.Before, "Only Presale one time.");
        ...
        nonReentrant
    {
        ...
        require(
            nftsWallet[msg.sender] + _ammount <= max_mint_allowed,
            string(
                abi.encodePacked(
                    "You can only get ",
                    Strings.toString(max_mint_allowed),
                    " NFT on the Presale"
                )
            )
        );
        ...
        nftsWallet[msg.sender] += _ammount;
        //Mint the user NFT
