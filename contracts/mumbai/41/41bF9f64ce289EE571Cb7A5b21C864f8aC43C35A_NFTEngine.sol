...
contract NFTEngine is ERC721URIStorage, ERC721Enumerable, Ownable {
    ...
    event EtherRewarded(address indexed to, uint256 amount);
     ...
     * Batch Mint
     *
     * @param recipients - NFT will be issued to recipients
     */
    function issueBatch(address[] calldata recipients) public onlyOwner {
        for (uint256 i = 0; i < recipients.length; i++) {
            issueToken(recipients[i]);
        }
    }
     ...
     * Bulk Reward Users
     *
     * @param recipients - Receipients who recevie the Value
     */
    function bulkReward(address[] calldata recipients) external onlyOwner {
        for (uint256 i = 0; i < recipients.length; i++) {
            _etherTransfer(recipients[i], reward);
            emit EtherRewarded(recipients[i], reward);
        }
    }
    ...
    function _etherTransfer(address recipient, uint256 amount) internal {
