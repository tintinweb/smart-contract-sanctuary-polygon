...
contract MockOrderFiller is Initializable {
    ...
    event OrderFilled(
        uint64[] makerOrderReferenceIds,
        uint64 takerOrderReferenceId,
        uint128[] makerMakerTokenAmountFilled,
        uint128[] makerTakerTokenAmountFilled,
        uint256 takerTokenFilledAmount
    );

    event IndividualOrderFilled(
        uint64 referenceId,
        uint256 takerFilledAmount,
        uint128 makerAmountFilled
    );

    struct OrderData {
        uint64[] makerOrderReferenceIds;
        uint64 takerOrderReferenceIds;
    ...
    function _fill(
        LimitOrder memory _makerOrder,
        Signature memory _makerSignature,
        uint128 _makerTakerTokenFillAmount
    ) internal returns (uint128, uint128) {
        return (_makerOrder.takerAmount, _makerOrder.makerAmount);
        //return IZeroEx(exchangeProxyAddress).fillLimitOrder(_makerOrder, _makerSignature, _makerTakerTokenFillAmount);
    }
    ...
    function fillBuyOrder(
        ...
        info.makerOrdersToFill = uint128(makerTakerTokenFillAmounts.length);
        info.makerMakerTokenAmountFilled = new uint128[](info.makerOrdersToFill);
        info.makerTakerTokenAmountFilled = new uint128[](info.makerOrdersToFill);

        // Fill stablecoin orders first
        takerOrder.makerToken.approve(exchangeProxyAddress, _orderData.totalMakerTokensToFill);
        // For every order submitted
        for (uint256 i = 0; i <= info.makerOrdersToFill - 1; i++) {
            // call exchange proxy and fill orders
            (info._takerFill, info._makerFill) = _fill(
                makerOrders[i],
                makerSignatures[i],
                makerTakerTokenFillAmounts[i]
            );
            emit IndividualOrderFilled(_orderData.makerOrderReferenceIds[i], info._takerFill, info._makerFill);

            //require((info._takerFill != 0) && (info._makerFill != 0), "One of the orders could not be filled");
            info.makerMakerTokenAmountFilled[i] = info._makerFill;
            info.makerTakerTokenAmountFilled[i] = info._takerFill;
        }

        // Fill talent token orders
        takerOrder.takerToken.approve(exchangeProxyAddress, takerTakerTokenFillAmount);
        // For every order submitted
        (info.takerFill, info.makerFill) = _fill(
            takerOrder,
            takerSignature,
            takerTakerTokenFillAmount
        );
        emit IndividualOrderFilled(_orderData.takerOrderReferenceIds, info._takerFill, info._makerFill);

        require((info.takerFill != 0) && (info.makerFill != 0), "One of the orders could not be filled");
        emit OrderFilled(
        _orderData.makerOrderReferenceIds,
        _orderData.takerOrderReferenceIds,
        info.makerMakerTokenAmountFilled,
        info.makerTakerTokenAmountFilled,
        info.makerFill
    );
        return (info.makerMakerTokenAmountFilled, info.makerFill);
    }

    function fillSellOrder(
        // taker has talent token, execute taker orders first
        LimitOrder[] calldata makerOrders,
        Signature[] calldata makerSignatures,
        uint128[] calldata makerTakerTokenFillAmounts,
        LimitOrder calldata takerOrder,
        Signature calldata takerSignature,
        uint128 takerTakerTokenFillAmount,
        OrderData memory _orderData
    ) public onlyAdmin returns (uint128[] memory, uint128) {
        OrderInfo memory info;
        ...
        emit IndividualOrderFilled(_orderData.takerOrderReferenceIds, info._takerFill, info._makerFill);
        //require((info.takerFill != 0) && (info.makerFill != 0), "One of the orders could not be filled");
        uint128 takerTakerFillAmount = info.makerFill;
        ...
        info.makerMakerTokenAmountFilled,
        info.makerTakerTokenAmountFilled,
        takerTakerFillAmount
    );
        return (info.makerMakerTokenAmountFilled, takerTakerFillAmount);
    }
