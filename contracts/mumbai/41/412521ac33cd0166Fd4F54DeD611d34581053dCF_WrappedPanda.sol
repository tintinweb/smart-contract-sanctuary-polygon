...
import "./ExtendWrappedERC721.sol";

contract WrappedPanda is ExtendWrappedERC721 {
    ...
    // 1: Normal , 2: Rare, 3: Super Rare, 4: Epic, 5: Legendary, 6: Mythic
    mapping(uint256 => uint8) public typeOfPanda;
    mapping(uint8 => uint256[]) public pandaByType;
    uint256 maxBatchSize = 9999;

    constructor() ExtendWrappedERC721("Wrapped NFTV Panda", "wPANDA") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    }

    modifier allowMint() {
        address _sender = _msgSender();
        require(hasRole(MINT_ROLE, _sender), "Revert: Must have mint role");
        _;
    }
    ...
    ) external allowMint returns (uint256) {
        require(shouldValidType(_type), "Revert: Invalid Type Of Panda");
        require(shouldValidMetadata(_metadataId), "Revert: Invalid MetadataId Of Panda");
        require(
            pandaByType[_type].length <= maxBatchSize,
            "Revert: Limit panda for each type is 1000"
        );
        uint256 newPandaId = increment();

        _mint(_to, newPandaId);
        _setMetadata(newPandaId, _metadataId);
        typeOfPanda[newPandaId] = _type;
        pandaByType[_type].push(newPandaId);
        emit Created(_to, newPandaId, _type, _metadataId);
        return newPandaId;
    }
    ...
    function shouldValidType(uint8 _type) internal pure returns (bool) {
        if (_type == 0 || _type > 6) return false;
        return true;
    }
    ...
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i != length; i++) {
            uint256 pandaId = ids[i];

            super.safeTransferFrom(from, to, pandaId);
        }
    }

    function getPandaType(uint256 id) external view returns (uint8) {
        require(_exists(id), "ExtendERC721: query for nonexistent token");
        return typeOfPanda[id];
    }
    ...
    function setupMintRole(address account) public onlyRole(DEFAULT_ADMIN_ROLE){
...
contract ExtendWrappedERC721 is ERC721Enumerable, IERC721Swappable, AccessControlEnumerable, Ownable {
