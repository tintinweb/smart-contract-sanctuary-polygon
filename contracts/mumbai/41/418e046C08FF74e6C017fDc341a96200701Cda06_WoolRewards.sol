...
// File: contracts/WoolState/WoolRewards.sol
...
contract WoolRewards is ERC721Holder, Pausable, Ownable {
    ...
    function validateProject(uint256 projectId) external onlyOwner {
        Project memory project = IWoolStateNFT(_nftRegistry).getProject(projectId);
        require (project.status == State.Created, "Invalid Project state");

        if (IWoolStateNFT(_nftRegistry).activateProject(projectId)) {
            require(IERC20(_aceptedToken).transfer(owner(), _collected[projectId]), "ERC20: Transfer has failed");
        }
    }
