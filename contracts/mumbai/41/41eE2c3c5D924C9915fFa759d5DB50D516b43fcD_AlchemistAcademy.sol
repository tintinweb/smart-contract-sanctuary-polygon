...
contract Alchemist is ReentrancyGuard, Initializable {
        ...
        returns (bool) 
    {
        ...
        (,uint fees,uint ratioOfSteady,,uint durationForMaturity) = IAcademy(address(academy)).getChymeInfo(chyme);
        uint chymeAmountToMerge;
        (_steady.amount, chymeAmountToMerge) = elixir.getSteadyRequired(tokenId);
        _steady.balance = steady.balanceOf(msg.sender);
        
        require(_steady.amount <= _steady.balance, "Need more Steady");
        
        if(durationForMaturity > block.timestamp){
            ...
            IERC20Upgradeable(chyme).approve(chymeBeneficiary, chymeAmountToMerge);
        }
            ...
            IERC20Upgradeable(chyme).approve(chymeBeneficiary, chymeAmountToMerge * (100 - ratioOfSteady) / 100);
            //approve Chyme from this address to the msg.sender proportional to the value in steady
            IERC20Upgradeable(chyme).approve(msg.sender, chymeAmountToMerge * ratioOfSteady / 100);
        }
