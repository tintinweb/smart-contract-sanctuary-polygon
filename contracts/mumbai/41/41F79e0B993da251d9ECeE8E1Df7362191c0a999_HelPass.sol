...
contract HelPass is ERC721A,Ownable{
    ...
    mapping(address => bool) public _SaleAddressExist;
     ...
     function doAirDrop( uint256 _amount) onlyOwner public {
    uint256 count = users.length;
    uint256 perPerson =_amount/count;
    for (uint256 i = 0; i < count; i++)
    {
      /* calling transfer function from contract */
    //   token.transfer(users[i],_amount);
      token.transfer(users[i],perPerson);

    //   if((users[i].balance == 0) && (this.balance >= _ethAmount))
    //   {
    //     require(users[i].send(_ethAmount));
    //   }
    }
  }
