...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  event Pokemon(string name, uint256 id);
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    (bytes32 _bytesString, uint256 _id) = abi.decode(response, (bytes32, uint256));
    string memory _string = string(abi.encodePacked(_bytesString));
    emit Pokemon(_string, _id);
  }
