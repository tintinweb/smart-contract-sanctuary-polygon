...
contract KakiOrder is ERC721URIStorage, Ownable {
    ...
    constructor() ERC721("KakiOrder", "kaki") {
        // NftTypeDetail memory detail = NftTypeDetail(50, 0, "", 0, 500);
        _nftTypeDetails[NftType.BASIC] = NftTypeDetail(50, 0, "", 0, 500);

        // NftTypeDetail memory detail = NftTypeDetail(100, 10, "", 0, 500);
        _nftTypeDetails[NftType.COMMON] = NftTypeDetail(100, 10, "", 0, 500);

        // NftTypeDetail memory detail = NftTypeDetail(5000, 100, "", 0, 500);
        _nftTypeDetails[NftType.RARE] = NftTypeDetail(5000, 100, "", 0, 500);

        // NftTypeDetail memory detail = NftTypeDetail(0, 0, "", 0, 500);
        _nftTypeDetails[NftType.LEGENDARY] = NftTypeDetail(0, 0, "", 0, 500);
    }
    ...
    function getCurrentMintPrice(NftType nftType, uint256 nftId) public view returns (uint256) {
        return _nftTypeDetails[nftType].price + _nftTypeDetails[nftType].increasePrice * nftId;
    }
    ...
    function mint(NftType nftType) external payable {
        ...
        _setTokenURI(
            newItemId,
            string(
                abi.encodePacked(_nftTypeDetails[nftType].baseURI, Strings.toString(newItemId))
            )
        );
		...
		function getNftDetails(address _owner) public view returns (NftItem[] memory) {  
        uint256 totalSupply = _nftIds.current();

        NftItem[] memory details = new NftItem[](balanceOf(_owner));
        
        for (uint256 i = 0; i < totalSupply; i++) {
            if(_nftItems[i + 1].owner == _owner) {
