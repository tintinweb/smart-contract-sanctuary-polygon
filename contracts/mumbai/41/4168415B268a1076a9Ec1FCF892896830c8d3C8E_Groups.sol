...
contract Groups is Initializable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    */
    ...
    function getGroupMemberships(address usrAddress) public view whenNotPaused returns (uint256[] memory){
    ...
    function updateGroupNameFormat(uint256 _groupID, string calldata _groupName) public onlyGroupOwners(_groupID) whenNotPaused  {
    ...
    // This allows users to change colors of their group image (Hex format)
    function updateGroupColors(uint256 _groupID, string calldata color1, string calldata color2, string calldata color3) public onlyGroupOwners(_groupID) whenNotPaused {
        ...
        groupDetails[_groupID].colors[0] = color1;
        groupDetails[_groupID].colors[1] = color2;
        groupDetails[_groupID].colors[2] = color3;
    }

    // This allows users to change colors of their group image (Hex format)
    function updateGroupDetails(uint256 _groupID, string calldata _details) public onlyGroupOwners(_groupID) whenNotPaused {
        // Make sure the details are within length limits
        require(bytes(_details).length <= maxDetailsLength, "Details too long");

        groupDetails[_groupID].details = _details;
    }

    // This allows users to change colors of their group image (Hex format)
    function updateGroupURI(uint256 _groupID, string calldata _uri) public onlyGroupOwners(_groupID) whenNotPaused {
        // Require a Avatar with RCF compliant characters only
        require(KUtils.isValidURI(_uri), "Bad characters in URI");

        groupDetails[_groupID].uri = _uri;
    }
