    ...
    ISharedData
{
    ...
    uint256 public claimOrRefundCount;
        ...
        whenNotPaused
    {
        ...
        _takeMoneyFromSender(currency, msg.sender, amount);
    ...
    function claim() public virtual whenNotPaused {
        ...
        while (block.timestamp <= vestingInfo[index]._time) {
            allowedPercentage += vestingInfo[index]._percent;
            index += 1;
        }

        uint256 claimAmount = _tokenInfo.presaleRate *
            allowanceToUserInfo[msg.sender].deposit;

        uint256 calculatedClaimAmount = (claimAmount * allowedPercentage) / 100;

        calculatedClaimAmount -= allowanceToUserInfo[msg.sender].claimedAmount;

        IERC20(tokenAddress).transfer(msg.sender, calculatedClaimAmount);
