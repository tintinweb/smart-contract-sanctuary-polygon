...
contract ERC1155SBTClaim is ForjClaimLogic {
    ...
    function initialize(
        ...
        _claimInitialize(_merkleRoot, _startTimestamp, _endTimestamp, _claimId);
...
contract ForjClaimLogic is ForjSoulboundERC1155 {
    ...
    function _checks(
        ...
        if(mintedAmount + _amount > supplyPerId[claimId].max) revert MaxSupplyReached();
    ...
    function _claimMint(address user, uint256 amount) internal {
        ...
        supplyPerId[claimId].total += amount;
        mintedAmount += amount;

        mint(user, claimId, amount, "");
    }
}
