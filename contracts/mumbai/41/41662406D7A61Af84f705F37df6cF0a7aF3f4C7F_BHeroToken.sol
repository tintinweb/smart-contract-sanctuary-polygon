    ...
    UUPSUpgradeable
{
    ...
    bytes32 public constant MONSTER_ROLE = keccak256("MONSTER_ROLE");
    ...
    function initialize(IBEP20 coinToken_) public initializer {
        ...
        _setupRole(MONSTER_ROLE, msg.sender);

        // Use with Chainlink VRF.
        // _setupRole(RANDOM_CREATOR_ROLE, msg.sender);
    }

    function tokenDetailsGet(uint256 nftId) public view returns (uint256) {
        return tokenDetails[nftId];
    }

    function setPVEContract(PVE _pve) external onlyRole(DEFAULT_ADMIN_ROLE) {
        pveContract = _pve;
    }
    ...
    function mint(uint256 count) external {
        ...
        pveContract.setDeposit(to, design.getMintCost() * count);
        // Create requests.
    ...
    function work(uint256 nftId) public returns (bool) {
        require(msg.sender == pve, "you dont have permission");
        uint256 baseDetails = tokenDetails[nftId];
        tokenDetails[nftId] = BHeroDetails.setWork(baseDetails);
        return true;
    }

    function unwork(uint256 nftId) public returns (bool) {
        require(msg.sender == pve, "you dont have permission");
        uint256 baseDetails = tokenDetails[nftId];
        tokenDetails[nftId] = BHeroDetails.setUnWork(baseDetails);
        return true;
    }

    function setPVE(address pveaddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        pve = pveaddress;
    }

    function listOfNfts() external view returns (uint256[] memory) {
        return tokenIds[msg.sender];
    }

    function decodeHeroDetails(uint256 _nft)
        external
        view
        returns (BHeroDetails.Details memory)
    {
        uint256 _nftDetail = tokenDetails[_nft];
        return BHeroDetails.decode(_nftDetail);
    }
}
...
import "@openzeppelin/contracts-upgradeable/access/AccessControlUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/UUPSUpgradeable.sol";
import "./BCoinToken.sol";
import "./BHeroToken.sol";
import "./BSlotToken.sol";
import "./BHeroDetails.sol";
import "./BSlotDetails.sol";
import "./Utils.sol";

contract PVE is AccessControlUpgradeable, UUPSUpgradeable {
    using SafeMath for uint256;
    using BHeroDetails for BHeroDetails.Details;
    using BSlotDetails for BSlotDetails.Details;

    BCoinToken private bcoin;
    BHeroToken private bhero;
    BSlotToken private bslot;
    struct Monster {
        uint16 index;
        uint40 hp;
        uint256 reward;
    ...
    mapping(uint256 => Monster) private ListOfMonsters;

    uint256[] private monsterRate;

    mapping(uint256 => Monster) private work;
    mapping(uint256 => uint256) private workSlot;
    ...
    function initialize(
        ...
        monsterRate = [8000, 1500, 400, 100];

        ListOfMonsters[0] = Monster(1, 600000, 1 ether, 0, false);
        ListOfMonsters[1] = Monster(2, 1000000, 2 ether, 0, false);
        ListOfMonsters[2] = Monster(3, 1800000, 4 ether, 0, false);
        ListOfMonsters[3] = Monster(4, 3000000, 6 ether, 0, false);
    }
    ...
    function createWork(uint256 nftId, uint256 slotId) external returns (bool) {
        require(bhero.ownerOf(nftId) == msg.sender, "you are not the owner");
        uint256 hero = bhero.tokenDetailsGet(nftId);
        BHeroDetails.Details memory data = BHeroDetails.decode(hero);
        uint256 multiplicator = 1;
        if (slotId > 0) {
            require(
                bslot.ownerOf(slotId) == msg.sender,
                "you are not the owner"
            );

            uint256 slot = bslot.tokenDetailsGet(slotId);
            BSlotDetails.Details memory dslot = BSlotDetails.decode(slot);
            multiplicator = dslot.multiplicator;
            require(data.work == 0 && dslot.work == 0, "already working");
        }

        require(data.work == 0, "already working");

        uint256 seed = uint256(blockhash(block.number));
        (uint256 seedx, uint256 monsterIndex) = Utils.weightedRandom(
            seed,
            monsterRate
        );
        require(bhero.work(nftId), "error do it again");
        if (slotId > 0) require(bslot.work(slotId), "error do it again");

        Monster storage monsterData = work[nftId];
        if (slotId > 0) workSlot[nftId] = slotId;

        Monster memory m = ListOfMonsters[monsterIndex];
        monsterData.index = m.index;
        monsterData.hp = m.hp;
        monsterData.reward = m.reward * multiplicator;
        monsterData.start = block.timestamp;
        monsterData.iswork = true;

        return true;
    }

    function rewardWork(uint256 nftId) external returns (bool) {
        require(bhero.ownerOf(nftId) == msg.sender, "you are not the owner");
        uint256 hero = bhero.tokenDetailsGet(nftId);
        BHeroDetails.Details memory data = BHeroDetails.decode(hero);
        require(data.work == 1, "not working");
        Monster memory workpve = work[nftId];
        uint256 date = block.timestamp;
        uint256 diff = date - workpve.start;
        uint256 hp = diff * data.Power * (data.speed / 10);
        require(hp >= workpve.hp, "doesnt finish his work");
        if (hp >= workpve.hp) {
            bhero.unwork(nftId);
            if (workSlot[nftId] > 0) {
                bslot.unwork(workSlot[nftId]);
                workSlot[nftId] = 0;
            }
            balance[msg.sender] = balance[msg.sender].add(workpve.reward);
            totalBalance[msg.sender] = totalBalance[msg.sender].add(
                workpve.reward
            );

            Monster storage workpveedit = work[nftId];
            workpveedit.iswork = false;
    ...
    function getRewards() external returns (bool) {
        require(balance[msg.sender] >= 40 ether, "you dont have balance");

        uint256 total = balance[msg.sender];
        balance[msg.sender] = 0;
        bcoin.transferReward(msg.sender, total);
        return true;
    }

    /** Sets the Monsters List. */
    function setMonsterLists(uint256 rarity, Monster memory monsters)
        external
        onlyRole(DESIGNER_ROLE)
    {
        ListOfMonsters[rarity] = monsters;
    }

    /** Sets the monster rate. */
    function setMonsterRate(uint256[] memory value)
        external
        onlyRole(DESIGNER_ROLE)
    {
        monsterRate = value;
    ...
    function getMonstersList() external view returns (Monster[] memory) {
        uint256 size = monsterRate.length;
        Monster[] memory result = new Monster[](size);
        for (uint256 i = 0; i < size; ++i) {
            result[i] = ListOfMonsters[i];
        }
        return result;
    }

    function getMonsterRate() external view returns (uint256[] memory) {
        return monsterRate;
    }

    function setDeposit(address to, uint256 amount) public {
        require(
            msg.sender == address(bhero) || msg.sender == address(bhero),
            "you dont have access"
        );
        totalDeposit[to] = totalDeposit[to].add(amount);
    }
    ...
    function getData()
        ...
        view
        returns (
            BHeroDetails.Details[] memory hero,
            BSlotDetails.Details[] memory slot,
            uint256[] memory heroSlot,
            uint256[][] memory heroMonster
        )
    {
        address player = msg.sender;
        uint256[] memory bheroDetails = bhero.getTokenDetailsByOwner(player);
        for (uint256 i = 0; i < bheroDetails.length; ++i) {
            hero[i] = BHeroDetails.decode(bheroDetails[i]);
        }

        uint256[] memory bslotDetails = bslot.getTokenDetailsByOwner(player);
        for (uint256 i = 0; i < bslotDetails.length; ++i) {
            slot[i] = BSlotDetails.decode(bslotDetails[i]);
        }

        for (uint256 i = 0; i < hero.length; i++) {
            Monster memory workoenot = work[hero[i].id];
            if (workoenot.iswork == true) {
                uint256 workornotslot = workSlot[hero[i].id];
                heroSlot[hero[i].id] = 0;
                if (workornotslot == hero[i].id)
                    heroSlot[hero[i].id] = workornotslot;

                heroMonster[hero[i].id][0] = workoenot.index;
                heroMonster[hero[i].id][1] = workoenot.hp;
                heroMonster[hero[i].id][2] = workoenot.reward;
                heroMonster[hero[i].id][3] = workoenot.start;
            }
        }
    }
...
contract BCoinToken is BEP20Detailed, BEP20 {
    ...
    constructor() BEP20Detailed("Buttons Coin", "BCOIN", 18) {
    ...
    function setPVE(address pveaddress) external onlyOwner {
        pve = pveaddress;
    }

    function transferReward(address to, uint256 amount) external {
        require(msg.sender == pve, "you dont have access");
        transfer(to, amount);
    }
}
...
import "./BEP20.sol";
import "./BSlotDetails.sol";
import "./IBSlotDesign.sol";
import "./PVE.sol";

contract BSlotToken is
    ERC721Upgradeable,
    AccessControlUpgradeable,
    PausableUpgradeable,
    UUPSUpgradeable
{
    ...
    IBSlotDesign public design;
    PVE public pveContract;

    function initialize(IBEP20 coinToken_) public initializer {
        __ERC721_init("Button Slot", "BSLOT");
        __AccessControl_init();
        __Pausable_init();
        __UUPSUpgradeable_init();
        coinToken = coinToken_;
    ...
    function setDesign(address contractAddress)
        ...
        onlyRole(DESIGNER_ROLE)
    {
        design = IBSlotDesign(contractAddress);
    }
    ...
    function mint(uint256 rarity) external {
        ...
        pveContract.setDeposit(to, design.getMintCost(rarity));
        uint256 id = tokenIdCounter.current();
    ...
    function _beforeTokenTransfer(
        ...
        if (from == address(0)) {
            ...
            ++tokenByRarity[BSlotDetails.decodeRarity(details)];
        } else {
            // Transfer or burn.
            // Swap and pop.
            uint256[] storage ids = tokenIds[from];
            uint256 index = BSlotDetails.decodeIndex(tokenDetails[id]);
            uint256 lastId = ids[ids.length - 1];
            ids[index] = lastId;
            ids.pop();

            // Update index.
            tokenDetails[lastId] = BSlotDetails.setIndex(
                tokenDetails[lastId],
                index
            );
        }
        if (to == address(0)) {
            ...
            uint256 index = ids.length;
            ids.push(id);
            tokenDetails[id] = BSlotDetails.setIndex(tokenDetails[id], index);
    ...
    function tokenDetailsGet(uint256 slotId) public view returns (uint256) {
        return tokenDetails[slotId];
    }

    function work(uint256 slotId) public returns (bool) {
        require(msg.sender == pve, "you dont have permission");
        uint256 baseDetails = tokenDetails[slotId];
        tokenDetails[slotId] = BSlotDetails.setWork(baseDetails);
        return true;
    }

    function unwork(uint256 slotId) public returns (bool) {
        require(msg.sender == pve, "you dont have permission");
        uint256 baseDetails = tokenDetails[slotId];
        tokenDetails[slotId] = BSlotDetails.setUnWork(baseDetails);
        return true;
    }
    ...
    function decodeSlotDetails(uint256 _nft)
        external
        view
        returns (BSlotDetails.Details memory)
    {
        uint256 _nftDetail = tokenDetails[_nft];
        return BSlotDetails.decode(_nftDetail);
    }
}
...
library BSlotDetails {
    ...
    function encode(Details memory details) internal pure returns (uint256) {
        ...
        require(details.multiplicator < (1 << 5));
        require(details.work < (1 << 5));

        uint256 value;
        value |= details.id;
        value |= details.index << 30;
        value |= details.rarity << 40;
        value |= details.multiplicator << 45;
        value |= details.work << 50;
        return value;
    }

    function setWork(uint256 details) internal pure returns (uint256) {
        uint256 work = decodeWork(details);
        details &= ~(uint256(31) << 50);
        details |= (work + 1) << 50;
        return details;
    }
        ...
        returns (Details memory result)
    {
        ...
        result.index = decodeIndex(details);
        result.rarity = decodeRarity(details);
        result.multiplicator = (details >> 45) & 31;
        result.work = (details >> 50) & 31;
    }
