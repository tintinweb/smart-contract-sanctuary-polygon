...
contract EthermonUpdateData is EthermonEnum, BasicAccessControl {
    ...
    */
    ...
    function reRollMonsterStatsWeth(uint32 _tokenId) external isActive {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        MonsterObjAcc memory obj;
        (
            obj.monsterId,
    ...
    */
    ...
    function setCustomClassBurnPrice(uint32 _classId, uint256 _price_in_emon)
        public
        onlyModerators
    {
        customClassBurnPrice[_classId] = _price_in_emon;
    }

    function getCustomClassBurnPrice(uint32 _classId)
        external
        view
        returns (uint256 price_in_emon)
    {
        price_in_emon = customClassBurnPrice[_classId];
    }
