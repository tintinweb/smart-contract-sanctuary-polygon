...
abstract contract HunterRankBase is SLBase, SLController, HunterRankError {
    address internal seasonContract;
...
interface ISLHunterRank {
     ...
     *  Base
     */
    function setSeasonContract(address _seasonContract) external;
    ...
    ERC1155Burnable
{
    ...
    constructor(
        ...
        address _seasonContract,
        string memory _baseTokenURI
    ) ERC1155(_baseTokenURI) SLController(_projectContract) {
    ...
    function mint(
        ...
        uint256 _tokenId,
        uint256 _amount
    ) external onlySeasonContract whenMintEnabled {
        if (!exists(_tokenId)) {
            revert DoesNotExistTokenId();
        }
    ...
    function _onlySeasonContract() private view {
        if (_msgSender() != seasonContract) {
            revert OnlySeasonContract();
        }
    }
     ...
     *  Base
     */
    function setSeasonContract(address _seasonContract) external onlyOperator {
        seasonContract = _seasonContract;
    }
