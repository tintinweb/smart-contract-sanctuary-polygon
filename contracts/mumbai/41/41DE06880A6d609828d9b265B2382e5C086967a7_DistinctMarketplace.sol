    ...
    Whitelist
{
    ...
    modifier validateInputs(
        ...
        // require(token.isContract(), 'Distinct: token should be a contract');
        // require(
        //     _msgSender() == IERC721Upgradeable(token).ownerOf(tokenId),
        //     "Distinct: Not owner"
        // );
        require(
        ...
        // require(
        //     tokenIdsLength == amountLength && tokenIdsLength == priceLength,
        //     "Distinct: number of tokenIds should equal to price and amount"
        // );
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            DistinctNFT.isAdmin(_msgSender()),
            "Distinct: Not Admin"
        );
        _;
    }
    ...
    modifier onlyOperator() {
        require(
            DistinctNFT.isOperator(_msgSender()),
            "Distinct: Not Operator"
        );
        _;
    }
    ...
    modifier onlyDropSale(uint256 saleId) {
        ...
        require(reserveSale[saleId].saleType == SaleType.Drop, "Distinct: not Drop sale");
        _;
    }

    modifier onlyAuctionSale(uint256 saleId) {
        require(reserveSale[saleId].saleType == SaleType.Drop, "Distinct: not Auction sale");
        _;
    }

    modifier onlyDirectSale(uint256 saleId) {
        require(reserveSale[saleId].saleType == SaleType.Drop, "Distinct: not Direct sale");
        _;
    }
    ...
    ) external validateInputs(startTime,endTime)
    {
        ...
        for (uint256 i = 0; i < numberOftokenIds; i++) {
            // require(DistinctNFT.balanceOf(_msgSender(),tokenIds[i]) >= amounts[i], 'Distinct: insufficient Token balance');
            reserveSale[saleId].tokenIds.push(tokenIds[i]);
            reserveSale[saleId].dropItems[tokenIds[i]].amount = amounts[i];
            reserveSale[saleId].dropItems[tokenIds[i]].priceInWETH = priceInWETH[i];
            reserveSale[saleId].dropItems[tokenIds[i]].priceInUSDT = priceInUSDT[i];
        }

        emit DropCreated(saleId);
    }

    function updateDrop(uint256 saleId, uint256 endTime) external {
        reserveSale[saleId].endTime = endTime;
    ...
    function buyDropItems(
        ...
        if(totalSellerRevenueInWETH > 0) 
            USDT.safeTransferFrom(_msgSender(), drop.seller, totalSellerRevenueInWETH);
        if(totalSellerRevenueInUSDT > 0)
            WETH.safeTransferFrom(_msgSender(), drop.seller, totalSellerRevenueInUSDT);
    ...
    function isDropSale(uint256 saleId) external view  onlyDropSale(saleId) returns(bool){
        return true;
    }
    ...
    function joinWhitelist(uint256 saleId) external {
        require(reserveSale[saleId].status == SaleStatus.open, 'DistinctMarket: Sale not available');
        _joinWhitelist(_msgSender(),saleId);
    }

    function _isUSDT(address paymentToken) internal view returns (bool) {
        return paymentToken == address(USDT);
    }
    ...
    function _getSellerRevenueAndDistinctFeeTotal(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            totalSellerRevenueInWETH = 10;
            totalSellerRevenueInUSDT =10;
            totalDistinctFeeInWETH =10;
            totalDistinctFeeInUSDT =10;
            // if( _isUSDT(paymentTokens[i]) ) {
            //     (
            //         uint256 distinctFee,
    ...
    function getSaleInfo(uint256 saleId) external view
    returns (
        address seller,
        SaleType saleType,
        SaleStatus status,
    ...
    function _getSaleInfo(uint256 saleId) internal view
    returns(
        address,
        SaleType,
        SaleStatus,
    ...
    function _getItemsPrice(uint256 saleId, uint256[] memory tokenIds) internal view returns(uint256[] memory, uint256[] memory){
        uint256 numberOftokenIds = tokenIds.length;
        ReserveSale storage sale = reserveSale[saleId];
...
contract Whitelist is Initializable, ContextUpgradeable {
    ...
    function isWhitelisted(uint256 saleId) external view returns(bool){
        bytes32 caller = keccak256(abi.encode(_msgSender(), saleId));
...
contract DistinctMarketFee is Constants, Initializable {
    ...
    function _getIsPrimary(
        ...
        return !tokenIdToFirstSaleCompleted[tokenId] && creator == seller;
    }

    function _getFees(
        IDistinctNFT distinctNft,
        uint256 tokenId,
        address seller,
        uint256 price
    )
    ...
    {
        (royaltyFeeTo, royaltyFee) = distinctNft.royaltyInfo(tokenId, price);

        address creator = distinctNft.getCreator(tokenId);

        uint256 distinctFeeBasisPoint;
        if (_getIsPrimary(tokenId, creator, seller)) {
