 ...
 * @author mario
 */
contract Hivemind is HivemindV1Storage, HivemindI, ErrorReporter, AccessControlEnumerable, ReentrancyGuard {
  ...
  event NewCollateralFactor(HTokenI _hToken, uint256 _oldCollateralFactorMantissa, uint256 _newCollateralFactorMantissa);

  /// @notice Emitted when price oracle is changed
  event NewPriceOracle(PriceOracleI _oldPriceOracle, PriceOracleI _newPriceOracle);

  /// @notice Emitted when an action is paused on a market
  event ActionPausedhToken(HTokenI _hToken, string _action, bool _pauseState);
   ...
   * @param _collection The 721 Collateral collection address
  ...
  function borrowAllowed(
    ...
    if (borrowPaused[_hToken]) revert Paused();

    if (!markets[_hToken].isListed) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
    if (!markets[_hToken].accountMembership[_borrower]) {
      ...
      assert(markets[_hToken].accountMembership[_borrower]);
    }

    if (oracle.getUnderlyingPrice(_hToken) == 0) {
    ...
    (, uint256 shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_hToken, _collection, _collateralId, _borrowAmount);
  ...
  function addToMarketInternal(HTokenI _hToken, address _borrower) internal {
    ...
    if (marketToJoin.accountMembership[_borrower] == true) {
      // already joined
      return;
    }
    ...
    accountAssets[_borrower].push(_hToken);
   ...
   */
  function _addMarketInternal(HTokenI _hToken) internal {
    uint256 i;
    for (i; i < allMarkets.length; i++) {
  ...
  function redeemAllowed(
    ...
    _redeemTokens;
    if (redeemPaused[_hToken]) revert Paused();

    if (!markets[_hToken].isListed) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
    // If the redeemer is not 'in' the market, then we can bypass the liquidity check
    if (!markets[_hToken].accountMembership[_redeemer]) {
      return;
    }
  ...
  function getHypotheticalAccountLiquidity(
    ...
    if (_redeemTokens > 0) (liquidity, shortfall) = getHypotheticalAccountLiquidityRedeemInternal(_account, _hToken, _redeemTokens);
    else if (_collection != address(0))
      (liquidity, shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_hToken, _collection, _collateralId, _borrowAmount);
  }
   ...
   */
  function checkMembership(address _account, HTokenI _hToken) external view returns (bool) {
  ...
  function transferAllowed(HTokenI _hToken) external view override {
    // Pausing is a very serious situation - we revert to sound the alarms
    if (transferPaused[_hToken]) revert Paused();
  }
  ...
  function repayBorrowAllowed(
    ...
    // (, uint256 shortfall) = getHypotheticalAccountLiquidityBorrowInternal(_hToken, _collection, _collateralId, 0);

    // if (shortfall > 0) revert HivemindError(Error.INSUFFICIENT_SHORTFALL);
  }
   ...
   */
  function getAllMarkets() external view returns (HTokenI[] memory) {
    return allMarkets;
  }
  ...
  function getHypotheticalAccountLiquidityRedeemInternal(
    ...
    HTokenI[] memory assets = accountAssets[_account];
    uint256 i;
    ...
    for (i; i < assets.length; i++) {
      ...
      uint256 tokensToDenom = (markets[asset].collateralFactorMantissa * exchangeRateMantissa * oraclePriceMantissa) / _hToken.decimals();

      // sumCollateral += tokensToDenom * hTokenBalance
      // sumCollateral = mul_ScalarTruncateAddUInt(tokensToDenom, hTokenBalance, sumCollateral);
      sumCollateral += (tokensToDenom * hTokenBalance) / 10**_hToken.decimals();

      // sumBorrowPlusEffects += oraclePrice * borrowBalance
      sumBorrowPlusEffects += (oraclePriceMantissa * borrowBalance) / 10**_hToken.decimals();

      // Calculate effects of interacting with _hToken
      if (asset == _hToken) {
        // redeem effect
        // sumBorrowPlusEffects += tokensToDenom * redeemTokens
        sumBorrowPlusEffects += (tokensToDenom * _redeemTokens) / 10**_hToken.decimals();
  ...
  function getHypotheticalAccountLiquidityBorrowInternal(
    ...
    uint256 sumCollateral = oracle.getUnderlyingFloorNFTPrice(_collection, _hToken.decimals());
    ...
    uint256 collateralFactorUint = markets[_hToken].collateralFactorMantissa;
    ...
    sumCollateral = (collateralFactorUint * sumCollateral) / 10**_hToken.decimals();
  ...
  function _setCollateralFactor(HTokenI _hToken, uint256 _newCollateralFactorMantissa) external {
    ...
    if (_newCollateralFactorMantissa != 0 && oracle.getUnderlyingPrice(_hToken) == 0) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
    ...
    emit NewCollateralFactor(_hToken, oldCollateralFactorMantissa, _newCollateralFactorMantissa);
  }
  ...
  function _supportMarket(HTokenI _hToken) external {
    ...
    _hToken.isHToken(); // Sanity check to make sure its really a HTokenI

    market.isListed = true;
    market.collateralFactorMantissa = 0;

    _addMarketInternal(_hToken);
...
interface PriceOracleI {
  function getUnderlyingPrice(HTokenI) external view returns (uint256);

  function getUnderlyingFloorNFTPrice(address _collection, uint256 _decimals) external view returns (uint256);
...
abstract contract HivemindI {
  /*** Assets You Are In ***/

  function enterMarkets(HTokenI[] calldata _hTokens) external virtual;

  function exitMarket(HTokenI _hToken) external virtual;
  ...
  function transferAllowed(HTokenI _hToken) external virtual;
}
...
contract HivemindV1Storage {
  ...
  /**
   * @notice pausing certain functionalities, market => paused?
   */
  mapping(HTokenI => bool) public borrowPaused;
  mapping(HTokenI => bool) public transferPaused;
  mapping(HTokenI => bool) public redeemPaused;
