...
contract GameEngineMumbai is GameEngine {
    ...
    function alertStake(uint tokenId) external override {
        require (isStaked[tokenId] == false);
        require (nftToken.ownerOf(tokenId)==address(this));
        uint randomNo = 2 + randomNumberGenerator.getRandom(tokenId)%5;
        // uint randomNo = 2 + randomNumberGenerator.initiateRandomness(tokenId,nftToken.actionTimestamp(tokenId))%5;
        // nftToken.setTimeStamp(tokenId);
        firstStakeLockPeriod[tokenId] = block.timestamp + 1; //convert randomNo from hours to sec
    ...
    function stake(uint[] memory tokenId) external override {
        ...
        if ( stakeConfirmation[tokenId[i]] == true ){
            ...
            uint randomNo =  2 + randomNumberGenerator.getRandom(tokenId[i]) % 5;
            // uint randomNo =  2 + randomNumberGenerator.initiateRandomness(tokenId[i],nftToken.actionTimestamp(tokenId[i])) % 5;
            // nftToken.setTimeStamp(tokenId[i]);
            firstStakeLockPeriod[tokenId[i]] = block.timestamp + 1; //convert randomNo from hours to sec
            // firstStakeLockPeriod[tokenId[i]] = block.timestamp + randomNo*1 hours; //convert randomNo from hours to sec
            nftToken.safeTransferFrom(msg.sender, address (this), tokenId[i]);
            stakeTime[tokenId[i]] = block.timestamp;
            isStaked[tokenId[i]] = true;
            tokenToRandomHourInStake[tokenId[i]]= 1; //conversion required
    ...
    function executeClaims (uint randomNumber, uint tokenId, uint firstHold, uint secondHold) internal override returns (bool) {
    ...
    function onSuccess(uint tokenId) internal override returns (bool) {
        ...
        uint randomNumber = randomNumberGenerator.getRandom(tokenId);
        // uint randomNumber = randomNumberGenerator.initiateRandomness(tokenId,nftToken.actionTimestamp(tokenId));
    ...
    function onCriticalSuccess(uint tokenId) internal override returns (bool) {
        (uint8 nftType,) = metadataHandler.getToken(tokenId);
        // require (lastClaim[tokenId] + 12 hours <= block.timestamp, "Claiming before 12 hours");
        token.mintFromEngine(msg.sender, calculateSUP(tokenId));
        lastClaim[tokenId] = block.timestamp;
        if (uint(keccak256(abi.encodePacked(randomNumberGenerator.getRandom(tokenId),"1")))%100 < 40 
        // if (uint(keccak256(abi.encodePacked(randomNumberGenerator.initiateRandomness(tokenId,nftToken.actionTimestamp(tokenId)),"1")))%100 < 40 
...
contract GameEngine is Ownable, ReentrancyGuard, ProxyTarget {
    ...
    function steal(uint8 tokenType,uint nonce) internal view returns (uint) {
        uint randomNumber = randomNumberGenerator.getRand(nonce);
        // uint randomNumber = randomNumberGenerator.stealRandomness();
    ...
    function calculateSUP (uint tokenId) internal returns (uint) {
        ...
        if (lastClaimTime == 0) {
            ...
            if (wasUnstakedRecently[tokenId] == true) {
                calculatedDuration = (block.timestamp - stakedTime)/1 hours; // todo -- divide by 1 hours?
...
//VITAL INTERNAL FUNCITONS
    function claimStake(uint tokenId) internal returns (bool){
        uint randomNumber = randomNumberGenerator.getRandom(tokenId)%100;
        // uint randomNumber = randomNumberGenerator.initiateRandomness(tokenId,nftToken.actionTimestamp(tokenId))%100;
        (,uint8 level) =
