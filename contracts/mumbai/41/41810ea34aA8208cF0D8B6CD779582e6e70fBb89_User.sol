...
contract User {
    ...
    */
     ...
     * @param _cid - cid of the course
     */
    function createCourse(string memory _name, string memory _desc, 
        string memory _url, uint8 _bounty, string memory _cid, string memory _ipfsURL) public {
        require(
    ...
    /**
     * @notice - Fetch IPFS URL of course ID
     *
     * @param courseID - ID of the course
     * @return ipfsURL - IPFS file URL
     */
    function fetchAssessmentQuestions(uint8 courseID) view public returns (string memory ipfsURL) {
        Course[] memory coursesList = employeeCourses[msg.sender];
        for (uint8 i = 0; i < coursesList.length; i++) {
            if(coursesList[i].id == courseID) {
                return coursesList[i].ipfsURL;
            }
        }
    }
