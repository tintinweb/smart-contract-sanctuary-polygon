    ...
    ClaimTopicsCollection
{
    ...
    function removeClaimByUser(
        ...
        IIdentity identity = getIdentity(msg.sender);
    ...
    function isUserVerified(
        ...
        IIdentity identity = getIdentity(userAddress);
...
interface IIdentitiesCollection {
     ...
     */
    function registerIdentity(address userAddress, IIdentity identity) external;
     ...
     */
    function containsIdentity(address userAddress) external view returns (bool);

    /**
     *  @dev Returns the identity address of user.
     *  @param userAddress The wallet of the user
     */
    function getIdentity(address userAddress) external view returns (IIdentity);
}
    ...
    IdentitiesFactoryDeployHook
{
    ...
    function _deployIdentity(address userAddress) internal returns (address) {
        ...
        Identity(clone).initializeWithInitialClaimKey(
            userAddress,
            address(this)
        );
    ...
    function _deployIdentityWithClaim(
        ...
        Identity(clone).initializeWithClaims(
            userAddress,
            address(this),
            topic,
            scheme,
 ...
 * allowing for it to be used as an upgradable logic contract.
 */
contract Identity is Initializable, ClaimHolder {
    /**
    ...
    function initializeWithInitialClaimKey(
        address initialManagementKey,
        address initialClaimKey
    ) external virtual initializer {
        require(
    ...
    function initializeWithClaims(
        ...
        __Key_init(initialManagementKey);
        this.addClaim(topic, scheme, issuer, signature, data, uri);
    }
    ...
    IdentitiesCollection
{
    ...
    function _afterIdentityDeployed(
        ...
        _registerIdentity(userAddress, IIdentity(identityAddress));
    }
    ...
    IIdentitiesCollection
{
    ...
    /**
     * @dev  Returns whether user identity was registered
     * @param   userAddress  - user wallet address
     * @return  bool  .
     */
