...
contract Darabase is NFTContract{
    ...
    bytes32 constant PAYMENT_TYPEHASH = keccak256(
        "Payment(bytes4 paymentClass,uint256 amount,string currency,bytes data)"
    );
    bytes32 private constant SELL_ORDER_TYPEHASH =
        keccak256("SellOrder(address maker,uint256 rentalId,uint256 pdrId,uint256 numberOfTokens,uint256 salt,Payment payment)Payment(bytes4 paymentClass,uint256 amount,string currency,bytes data)");
    ...
    function renewRental(
        ...
        uint256 numberOfRentals = getRentalDetails(pdrId, rentalId).amount;
        (uint256 totalPayment) = calculatePayment(sellOrder.payment.amount, numberOfRentals, sellOrder.numberOfTokens, buyerFee);
        uint256 platformPayment = buyerFee + sellerFee;

        uint remaining = doPlatformPayment(msg.sender, totalPayment, sellOrder.payment, platformPayment);
        doPaymentTransfer(msg.sender, remaining, sellOrder.maker, sellOrder.payment, true);
        updateRental(pdrId, rentalId, startTime, endTime);
    }
