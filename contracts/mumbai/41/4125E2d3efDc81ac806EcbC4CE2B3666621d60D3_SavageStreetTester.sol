...
contract SavageStreetTester is ERC721r, Ownable, Pausable, ReentrancyGuard {
    ...
    function checkSaleStage(bool) public view returns (bool _currentState){
    ...
    returns (string memory)
  {
    ...
    function productionMint (uint quantity) external payable {
      if(!onlyWhitelisted) {
    ...
    function newMint(uint quantity) external payable {
      address buyer = msg.sender;

      if(checkSaleStage(true)){
        require(iswhitelisted(buyer) == true);
        require(msg.value >= prePrice * quantity);
      }
