...
contract ReMintOrder is Owner {
    ...
    mapping(address => mapping(uint256 => bool)) public tokenStake;
    ...
    constructor(ITemplar templar_, address slaveToken_, address pdToken_, address pdPair_) Owner() {
        templar = templar_;
    ...
    function setpdTokenInfo(address newpdToken, address newpdPair) public onlyOwner {
        pdToken = newpdToken;
        pdPair = newpdPair;
    }
    ...
    function orderMint(address tokenAddress, uint256 nftAmount) public checkContractCall checkPaused onlyOwner {
        ...
        if (inviter != address(0) && IERC721(slaveToken).balanceOf(inviter) > 0) {
            IERC20(pdToken).transferFrom(
                msg.sender,
                inviter,
                fee.mul(5).div(100)
    ...
    function getSlaveEnergy(address tokenAddress, uint256 tokenId) public view returns (uint256) {
    ...
    function reMint(address tokenAddress, uint256[] memory tokenIds) public checkContractCall checkPaused {
        uint256 amount = 3;
        ...
        for (uint256 i=0; i<tokenIds.length; i++) {
            require(IERC721(tokenAddress).ownerOf(tokenIds[i]) == msg.sender, "one nft is not belong to you");
            IERC721(tokenAddress).transferFrom(msg.sender, destroyAddress, tokenIds[i]);
        }

        uint256 ind = queryTokenAddressIndex(tokenAddress);
        for (uint256 j=0; j<res; j++) {
            uint256 tokenid = INFTcustom(orderNFTList[ind+1]).mint(msg.sender);
            emit ReMint(orderNFTList[ind+1], tokenid);
        }
    }

    function composite(IERC721[] memory addrs, uint256[] memory tokenIds) external checkContractCall checkPaused {

    }

    function recovery(uint256 tokenId, uint256 energy) external checkContractCall checkPaused {
    }

    function queryTokenAddressIndex(address tokenAddress) private view returns (uint256) {
