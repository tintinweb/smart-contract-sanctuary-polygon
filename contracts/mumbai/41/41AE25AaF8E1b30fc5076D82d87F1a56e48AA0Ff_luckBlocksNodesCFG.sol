...
contract luckBlocksNodesCFG is Ownable,ReentrancyGuard {
  ...
  Iluckblocks lbcontract;
   ...
   uint8[] public activenodes;
   uint8[] public waitingnodes;

   // Golden NFTNodes queue
   uint8[] public activenodesG;
   uint8[] public waitingnodesG;
    
   // tokenId to Node Info
   mapping(uint8 => Node) public nodes;
  ...
  // Events list
  event NftStaked(uint8 indexed tokenId, address indexed owner);
  event NftUnstaked(uint8 indexed tokenId, address indexed owner);
  event NftRewarded(uint8 indexed tokenId, uint256 lottery, uint256 amount, address indexed owner);
  event UserActivated(uint8[] nodes, address indexed owner, uint256 weekly);
  ...
  // initialize contract while deployment
    ...
    function stake(uint8 tokenId) public virtual nonReentrant {
    ...
    function unstake(uint8 tokenId) public virtual nonReentrant {
        ...
        if(tokenId < 11){
          ...
          uint8 index = 0;
          uint8[] memory _activenodes = activenodesG;
          uint8[] memory _waitingnodes = waitingnodesG;

          bool didDisabled = false;

          for (uint8 i = 0; i < _activenodes.length; i++) {
              if (
          ...
          if(didDisabled == false){
            for (uint8 i = 0; i < _waitingnodes.length; i++) {
                if (
          ...
          // Find the index of this token id in the nodes array
          uint8 index = 0;
          uint8[] memory _activenodes = activenodes;
          uint8[] memory _waitingnodes = waitingnodes;
   ...
   function getUserActivation(address _caller,uint weekly) external view returns (bool active) {
  ...
  function resetQueue(uint lottery, uint weekly) external {
      ...
      if(weekly == 1){
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 2100,"autoSpin parameters not met"); // on mainnet 605100
        ...
        for(uint i = _waitingnodes.length; i > 0; i--){
          ...
          uint8 nodeId = _waitingnodes[i-1];
        ...
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 900,"autoSpin parameters not met"); // on mainnet 86700
        //send the waiting nodes to the active nodes array and delist the active stucked nodes
        uint8[] memory _activenodes = activenodes;
        uint8[] memory _waitingnodes = waitingnodes;
   ...
   function updateNodeInfo(address caller, uint256 reward, uint weekly,uint lottery) external returns(bool success) {
      ...
      if(weekly == 1){
       require(block.timestamp > lbcontract.autoSpinTimestamp() + 1800,"autoSpin parameters not met"); // on mainnet 604800

          //change from active to the waiting queue
          if(activenodesG.length > 0){

              uint8 randomIndex = uint8(randomNum % activenodesG.length);

              // get choosen node infos
              uint8 choosenNode = activenodesG[randomIndex];
            ...
            //send the waiting nodes to the active nodes array
              uint8 randomIndex = uint8(randomNum % waitingnodesG.length);

              // get choosen node infos
              uint8 choosenNode = waitingnodesG[randomIndex];
       ...
       require(block.timestamp > lbcontract.autoSpinTimestamp() + 600,"autoSpin parameters not met"); // on mainnet 86400

      //change from active to the waiting queue
      if(activenodes.length > 0){
       
          uint8 randomIndex = uint8(randomNum % activenodes.length);

          // get choosen node infos
          uint8 choosenNode = activenodes[randomIndex];
        ...
        //send the waiting nodes to the active nodes array
         uint8 randomIndex = uint8(randomNum % waitingnodes.length);

          // get choosen node infos
         uint8 choosenNode = waitingnodes[randomIndex];
   ...
   function getWaitingNodes(bool _golden) external view returns(uint8[] memory){
    ...
    function getActiveNodes(bool _golden) external view returns(uint8[] memory){
    ...
    function getNodeStatus(uint8 nftId) public view returns(string memory){
  ...
  function getStakedNodesFromUser(address _wallet) public view returns (uint8[] memory) {
      uint8[] memory nodeArray = new uint8[](50); // Initialize the array with a fixed size
      uint8 count = 0; // Variable to keep track of the number of matched nodes
      ...
      uint8[] memory trimmedArray = new uint8[](count);
