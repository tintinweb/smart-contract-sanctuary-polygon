...
contract DragonCrossbreed is BaseAuctionReceiver, ReentrancyGuard, RandomConsumerBase {
    ...
    uint constant MAX_BREED_COUNT = 5;

    uint[MAX_BREED_COUNT] private _platformPrices;
    ...
    function setPlatformPrices(uint[MAX_BREED_COUNT] calldata newPlatformPrices) 
    ...
    function processERC721(address from, uint tokenId, bytes calldata data) 
        ...
        require(info.dragonType != DragonInfo.Types.Legendary && info.mutatedFromId == 0, 
            "DragonCrossbreed: the dragon cannot be a legendary-type or mutant");
        require(dt.breedsCount(tokenId) < MAX_BREED_COUNT, 
            "DragonCrossbreed: crossbreed limit exceeded");
    ...
    whenNotPaused {
        ...
        require(
            dragonToken.breedsCount(myDragonId) < MAX_BREED_COUNT && 
            dragonToken.breedsCount(anotherDragonId) < MAX_BREED_COUNT, 
            "DragonCrossbreed: crossbreed limit exceeded"
        );
        ...
        require(amount >= (_platformPrice + _rentPrice), "DragonCrossbreed: incorrect amount sent to the contract");
    ...
    function fulfillRandomness(bytes32 requestId, uint randomness) internal override {
        ...
        if (breedDetails.rentPrice > 0) {
            IERC20(coinContract()).transfer(
                holderOf(breedDetails.dragon2Id),
        ...
        DragonCreator(dragonCreatorAddress()).giveBirth(
            ...
            newGenes, 
            dragonToken.ownerOf(breedDetails.dragon1Id)
        );

        if (isLocked(breedDetails.dragon2Id) && 
            dragonToken.breedsCount(breedDetails.dragon2Id) >= MAX_BREED_COUNT) {
            _transferTokenToHolder(breedDetails.dragon2Id);
        }
