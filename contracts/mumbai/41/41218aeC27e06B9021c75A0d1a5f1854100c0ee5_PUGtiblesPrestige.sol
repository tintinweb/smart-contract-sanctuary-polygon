...
contract PUGtiblesPrestige is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public pugCost = 0.002 ether;  
    uint256 public pugWLCost = 0.001 ether;
    ...
    // Public functions
    function pugMint(uint8 mintAmount) external payable {
        ...
        require(mintAmount > 0, "Provide mint amount major to 0");
        require(mintAmount <= maxDalisPerMint, "Max Dalis per mint reached"); 
        require(nextTokenId + mintAmount < seasonSupply, "Max season supply reached");
        require(nextTokenId + mintAmount < TOTAL_SUPPLY, "Max supply reached"); 

        if(isWhiteListActive && (whiteListAmount > 0)){
            if(mintAmount <= whiteListAmount){
                require(msg.value >= pugWLCost * mintAmount, "Insufficient funds"); 

                whiteList[msg.sender] -= mintAmount;
                _safeMint(msg.sender, mintAmount);
            }else if(isPublicSaleActive){
                require(msg.value >= (pugWLCost * whiteListAmount) + (pugCost * (mintAmount - whiteListAmount)), "Insufficient funds"); 

                whiteList[msg.sender] -= whiteListAmount;
                _safeMint(msg.sender, mintAmount);
            }else{
                require(mintAmount <= whiteListAmount, "Exceeded max available to purchase");
            }
        }else if(isPublicSaleActive){
            require(msg.value >= pugCost * mintAmount, "Insufficient funds");   
    ...
    function ownerMint(address _addr, uint256 _mintAmount) external onlyOwner {
        ...
        require(nextTokenId + _mintAmount < TOTAL_SUPPLY, "Max supply reached");
        require(nextTokenId + _mintAmount < seasonSupply, "Max season supply reached");
