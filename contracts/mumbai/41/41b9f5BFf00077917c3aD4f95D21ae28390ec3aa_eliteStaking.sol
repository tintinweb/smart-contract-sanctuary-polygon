...
contract eliteStaking is Ownable, Pausable {

    IERC20 public eliteToken;
    ...
    constructor (address _wallet, address _eliteToken) {
        
        wallet = payable(_wallet);
        eliteToken = IERC20(_eliteToken);
    ...
    function setEliteToken(address _eliteToken) external onlyOwner{
        require(_eliteToken != address(0x0),"Zero address appear");
        eliteToken = IERC20(_eliteToken);
    }
    ...
    function deposit(uint256 _tokenAmount, uint256 _planID, address _referal) external whenNotPaused {
        ...
        eliteToken.transferFrom(_msgSender(), address(this),_tokenAmount);        
        emit DepositTokens(msg.sender,_referal,address(eliteToken), _tokenAmount);
    }
    ...
    function withdraw(uint256 _stakeID) external payable whenNotPaused {
        ...
        eliteToken.transfer(msg.sender, user.amount);
        ...
        if(((user.planDays) > (block.timestamp - user.stakeTime)) && user.plan > 1 ) {
            ...
            eliteToken.transfer(msg.sender, reward );
            eliteToken.transfer(getReferal[msg.sender], referalReward);
        }

        emit WithdrawToken(msg.sender, address(eliteToken), _stakeID, user.amount, reward, referalReward);
    }
