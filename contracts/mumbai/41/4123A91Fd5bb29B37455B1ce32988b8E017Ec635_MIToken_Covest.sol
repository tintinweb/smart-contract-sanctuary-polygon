	...
	ReentrancyGuardUpgradeable
{
	 ...
	 * @dev The exchangeRate for wrap asset to MIToken and unwrap MIToken to asset.
	...
	event UnWrapped(
		...
		address indexed from,
		address indexed coin,
		uint256 amountMIUnWrap,
		uint256 amountOut,
		uint256 ExchangeRate
	);
	event RateChanged(uint256 rate);

	function initialize(address _nameRegistry, uint256 _BalanceRate)
		public
		initializer
	{
	...
	modifier assetsChecker(address _coin) {
		require(
			AR.isSupportAssets(symbol(), _coin),
			'AdminRouter: This CU we not receive this CU.'
		);
		_;
	}
	...
	/**
	 *  @notice Mint a MIToken.
	 *  @dev `_amount` must be decimals of `_asset`
	 *  @param _user The user address.
	 *  @param _asset The asset will be exchange.
	 *  @param _amount The amount value that decimals of The asset `_asset`.
	 *
	 *  Requirements:
	 *   - `msg.sender` Must be a SuperManager.
	 *   - `nonReentrant` Can't call multiple when the first tx is pending of a user.
	 *   - `_asset` Check the asset is approve from adminRouter for this pool.
	 */
	function mint(
		address _user,
		address _asset,
		uint256 _amount
	) public nonReentrant isSuperManager assetsChecker(_asset) {
		IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_asset);
		require(_user != address(0), 'user address is not valid.');
		require(
			_ERC20.allowance(_user, address(this)) >= _amount,
			'MIToken: You are not allowance'
		);
		require(
			_ERC20.balanceOf(_user) >= _amount,
			'MIToken: Insufficient Balance'
		);
		uint256 _rate = exchangeRate;

		_ERC20.transferFrom(_user, address(this), _amount);
		//* (600*10^5)*10**(18-5) * rate / 100
		uint256 _amountToMint = ((_amount *
			10**(decimals() - _ERC20.decimals())) * _rate) / 100;

		_mint(_user, _amountToMint);

		emit MintMI(msg.sender, _user, _asset, _amount, _amountToMint, _rate);
	}

	/**
	 *  @notice Unwrap a MIToken to asset.
	 *  @dev `_amount` must be decimals of `_asset`
	 *  @param _user The user address.
	 *  @param _asset The asset will be exchange.
	 *  @param _amount The amount value that decimals of The asset `_asset`.
	...
	function unWrap(
		...
		uint256 _rate = exchangeRate;
		IERC20MetadataUpgradeable _ERC20 = IERC20MetadataUpgradeable(_asset);
		uint256 _saveAmount = (_amount * _rate) / 100;
		uint256 _willburn = _saveAmount * 10**(decimals() - _ERC20.decimals());
		require(_user != address(0), 'user address is not valid.');
		require(balanceOf(_user) >= _willburn, 'MIToken: Balance Insufficient');
		_burn(_user, _willburn);
		require(
			_ERC20.balanceOf(address(this)) >= _saveAmount,
			'MIToken: Contract Balance Insufficient'
		);
		_ERC20.transfer(_user, _saveAmount);

		emit UnWrapped(msg.sender, _user, _asset, _amount, _saveAmount, _rate);
	}

	/**
	 *  @notice Set the ExchangeRate.
	 *  @dev `_rate` e.g. 1% is 1.
	 *  @param _rate The rate of number that is percentage.
	 *
	 *  Requirements:
	 *   - `msg.sender` Must be a SuperManager.
	 *   - `_rate` Musn't be less than 0.
	 */
	function setRateBalance(uint256 _rate) public isSuperManager {
		require(_rate > 0);
		exchangeRate = _rate;
