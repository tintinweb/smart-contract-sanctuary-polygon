...
contract SumoShowPool is Ownable
{
    ...
    uint256 public goldShare = 0;
    uint256 public blackShare = 0;
    uint256 public unallocatedAmount = 0;
    ...
    function updateShares(uint256 _value) external onlySumoShow {
        ...
        if (mintedGold == 0) {
            _unallocatedAmount += _value * GOLD_REWARD_PERCENTAGE / 100;
        } else {
            goldShare += (_value * GOLD_REWARD_PERCENTAGE / 100) / mintedGold;
        }

        mintedBlack -= BLACK_RESERVE_NUMBER;
        if (mintedBlack == 0) {
            _unallocatedAmount += _value * BLACK_REWARD_PERCENTAGE / 100;
        } else {
            blackShare += (_value * BLACK_REWARD_PERCENTAGE / 100) / mintedBlack;
        }
    ...
    function addNewRoyaltiesRound(uint256 _amount) external onlyOwner {
        royaltiesRewards[nextRoyaltiesRoundId++] = _amount;
    }

    // deposit partnership rewards
    function depositPartnershipRewards(uint256 _amount) external onlyOwner {
        partnershipRewards[nextPartnershipId++] = _amount;
        wrappedEther.transferFrom(msg.sender, address(this), _amount);
    }
