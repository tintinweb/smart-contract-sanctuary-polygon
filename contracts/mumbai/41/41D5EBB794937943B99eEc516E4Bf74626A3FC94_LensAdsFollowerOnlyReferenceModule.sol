...
import './GlobalBudget.sol';

/**
 * @title LensAdsFollowerOnlyReferenceModule
 * @author Daniel Beltrán for Modulens
 ...
 *
 * @param currency The currency associated with this post.
...
contract LensAdsFollowerOnlyReferenceModule is
    FollowValidationModuleBase,
    IReferenceModule,
    FeeModuleBase
{
    // Modulens Fee
    // The address of the Modulens contract
    address immutable i_modulens;
    // The fee that will be charged in percentage
    uint256 immutable i_fee;
    // Lens Periphery
    using SafeERC20 for IERC20;
    // Mapping to store the data associated with a post, indexed by the profile and post ID
    mapping(uint256 => mapping(uint256 => PostData)) _dataByProfileByPost;
    // Mapping to store the global budget data, indexed by the profile and currency
    mapping(bytes32 => uint256) globalBudgetData;
    // Mapping to store the number of mirrors for a given profile and post
    mapping(uint256 => mapping(uint256 => uint256)) s_profileToPostToNumberOfMirrors;
    // Mapping to store whether a given follower has mirrored a given post or not
    mapping(uint256 => mapping(uint256 => bool)) s_followerHasMirrored;
    ...
    function initializeReferenceModule(
        ...
        (uint256 _budget, uint256 _reward, address _currency) = abi.decode(
            data,
            (uint256, uint256, address)
        );

        // Check that the budget is not equal to 0
        if (_budget == 0) {
            revert Errors.LensAds__BudgetEqualZero();
        }

        // Check that the budget is not greater than the global budget for this profile and currency
        uint256 thisGlobalBudget = GlobalBudget.getGlobalBudget(
            globalBudgetData,
            profileId,
            _currency
        );
        if (_budget <= thisGlobalBudget) {
            revert Errors.LensAds__BudgetBiggerThanGlobalBudget();
        }

        // Check that the reward is greater than 0
        if (_reward < 0) {
            revert Errors.LensAds__RewardEqualZero();
        }

        // Set the budget, reward, and currency for this post
        _dataByProfileByPost[profileId][pubId].budget = _budget;
        _dataByProfileByPost[profileId][pubId].reward = _reward;
        _dataByProfileByPost[profileId][pubId].currency = _currency;

        // Fund this post with the provided budget
        fundThisPost(profileId, pubId, _budget);

        // Update the global budget for the profile pointed to and the specified currency
        GlobalBudget.setGlobalBudget(
            globalBudgetData,
            profileId,
            _currency,
            thisGlobalBudget - _budget
        );

        // Emit the PostBudgetFunded event
        emit Events.PostBudgetFunded(profileId, pubId, _budget, _currency);
    ...
    function processMirror(
        ...
        // Get the currency for the post pointed to
        address __currency = _dataByProfileByPost[profileIdPointed][pubIdPointed].currency;
        // Get the reward for the post pointed to
        uint256 __reward = _dataByProfileByPost[profileIdPointed][pubIdPointed].reward;
        // Get the current budget for the post pointed to
        uint256 __budget = _dataByProfileByPost[profileIdPointed][pubIdPointed].budget;

        // Bring the global budget
        uint256 thisGlobalBudget = GlobalBudget.getGlobalBudget(
            globalBudgetData,
            profileId,
            __currency
        ...
        if (s_followerHasMirrored[profileId][pubIdPointed]) {
            revert Errors.LensAds__FollowerAlreadyMirrored();
        }

        // Check that there is enough budget to pay the reward
        if (__reward >= __budget) {
            revert Errors.LensAds__NotEnoughPostBudget();
        }

        // Transfer the reward to the mirror creator
        IERC20(__currency).safeTransferFrom(address(this), mirrorCreator, __reward);

        // Emit the RewardGiven event
        emit Events.RewardGiven(profileId, profileIdPointed, pubIdPointed, __reward, __currency);

        // Update the current budget for the post pointed to
        _dataByProfileByPost[profileIdPointed][pubIdPointed].budget -= __reward;
        // Update the global budget for the profile pointed to and the specified currency
        GlobalBudget.setGlobalBudget(
            globalBudgetData,
            profileIdPointed,
            __currency,
            thisGlobalBudget - __reward
        );
        // Increment the number of mirrors for this profile and post
        s_profileToPostToNumberOfMirrors[profileIdPointed][pubIdPointed]++;
        // Set the flag indicating that the follower has mirrored this post
        s_followerHasMirrored[profileId][pubIdPointed] = true;
    }

    /**
     * @dev Funds the specified post with the provided budget.
     * @param _profileId The ID of the profile that owns the post to be funded.
     * @param _pubId The ID of the post to be funded.
     * @param _budget The amount to be added to the post's current budget.
     */
    function fundThisPost(uint256 _profileId, uint256 _pubId, uint256 _budget) private {
        // Update the current budget for the post pointed to
        _dataByProfileByPost[_profileId][_pubId].budget += _budget;
    }

    /**
     * @dev Funds the global budget for a given profile. This will transfer funds from the budget owner's
     *       address to the contract, calculate the fees to be charged, and update the global budget
     *       for the given profile and currency.
     * @param profileId The ID of the profile to fund the global budget for.
     * @param globalBudget The amount of funds to add to the global budget.
     * @param _currency The currency in which to fund the global budget.
     * @param _frontend The address of the frontend contract.
     */
    function fundMyGlobalBudget(
        uint256 profileId,
        ...
        address budgetOwner = IERC721(HUB).ownerOf(profileId);

        // Transfer funds from the budget owner to the contract
        IERC20(_currency).safeTransferFrom(budgetOwner, address(this), globalBudget);

        // Calculate the fees to be charged
        uint256 fees = (globalBudget * i_fee) / 100;

        // Transfer half of the fees to the modulens contract
        IERC20(_currency).safeTransferFrom(address(this), i_modulens, (fees / 2));

        // Transfer the other half of the fees to the frontend contract
        IERC20(_currency).safeTransferFrom(address(this), _frontend, (fees / 2));

        // Get the current global budget in this currency
        uint256 thisGlobalBudget = GlobalBudget.getGlobalBudget(
            globalBudgetData,
            profileId,
            _currency
        );
        // Update the global budget for the profile
        GlobalBudget.setGlobalBudget(
            globalBudgetData,
            profileId,
            _currency,
            thisGlobalBudget + globalBudget - fees
        );

        // Emit an event to notify that the global budget has been funded
        emit Events.GlobalBudgetFunded(profileId, globalBudget - fees, _currency, _frontend);
    }

    /**
     * @dev Withdraws funds from the global budget for a given profile. This will transfer funds from the contract to the
     *      budget owner's address, and update the global budget for the given profile and currency.
     * @param _currency The currency in which to withdraw funds from the global budget.
     * @param _profileId The ID of the profile to withdraw funds from the global budget for.
     * @param _amount The amount of funds to withdraw from the global budget.
     */
    function withdrawGlobalBudget(address _currency, uint256 _profileId, uint256 _amount) public {
        if (IERC721(HUB).ownerOf(_profileId) != msg.sender)
            revert Errors.LensAds__NotSenderProfile();

        // Bring the global budget for the given profile and currency
        uint256 thisGlobalBudget = GlobalBudget.getGlobalBudget(
            globalBudgetData,
            _profileId,
            _currency
        ...
        // Check that there is enough funds in the global budget to withdraw the specified amount
        if (_amount > thisGlobalBudget) revert Errors.LensAds__NotEnoughGlobalBudget();

        // Transfer the specified amount of funds from the contract to the budget owner's address
        IERC20(_currency).safeTransferFrom(address(this), msg.sender, _amount);

        // Emit the GlobalBudgetWithdrawn event
        emit Events.GlobalBudgetWithdrawn(_profileId, _amount, _currency);

        // Update the global budget for the given profile and currency
        GlobalBudget.setGlobalBudget(
            globalBudgetData,
            _profileId,
            _currency,
            thisGlobalBudget - _amount
        );
    }

    // withdraw reminder, function to empty the funds in the mirror
    /**
     ...
     * @param _profileId The ID of the profile associated with the post.
     * @param _pubId The ID of the post to withdraw funds from.
     * @param _amount The amount of funds to withdraw from the post budget.
     */
    function withdrawPostBudget(uint256 _profileId, uint256 _pubId, uint256 _amount) public {
        // Check that the sender is the owner of the given profile
        if (IERC721(HUB).ownerOf(_profileId) != msg.sender) {
            // Revert with the LensAds__NotSenderProfile error if the sender is not the owner of the profile
            revert Errors.LensAds__NotSenderProfile();
        }

        // Get the post budget and currency for the given profile and post
        uint256 thisPostBudget = _dataByProfileByPost[_profileId][_pubId].budget;
        address _currency = _dataByProfileByPost[_profileId][_pubId].currency;

        // Check that there is enough funds in the post budget to withdraw the specified amount
        if (_amount > thisPostBudget) {
            // Revert with the LensAds__NotEnoughGlobalBudget error if there are not enough funds in the post budget
            revert Errors.LensAds__NotEnoughGlobalBudget();
        } else {
            // Transfer the specified amount from the contract to the sender's address
            IERC20(_currency).safeTransferFrom(address(this), msg.sender, _amount);

            // Emit the PostBudgetWithdrawn event
            emit Events.PostBudgetWithdrawn(_profileId, _pubId, _amount, _currency);

            // Update the post budget for the given profile and post
            _dataByProfileByPost[_profileId][_pubId].budget -= _amount;
        }
    }

    /**
     * @dev This function retrieves and returns the global budget for a given profile in a specified currency.
     * @param currency The currency of the global budget.
     * @param profileId The ID of the profile.
     * @return The global budget amount in the specified currency for the given profile.
     */
    function getGlobalBudget(uint256 profileId, address currency) public view returns (uint256) {
        // Bring the global budget
        uint256 thisGlobalBudget = GlobalBudget.getGlobalBudget(
            globalBudgetData,
            profileId,
        ...
        return thisGlobalBudget;
    }

    /**
     * @dev Returns the budget for a given post and profile.
     * @param _pubId The ID of the post.
     * @param _profileId The ID of the profile.
     * @return The budget for the given post and profile.
     */
    function getPostBudget(uint256 _pubId, uint256 _profileId) public view returns (uint256) {
        // Get the current budget for the post pointed to
        return _dataByProfileByPost[_profileId][_pubId].budget;
    }

    /**
     * @dev Returns the number of mirrors for a given post and profile.
     * @param _pubId The ID of the post.
     * @param _profileId The ID of the profile.
     * @return The number of mirrors for the given post and profile.
     */
    function getMirrorCount(uint256 _pubId, uint256 _profileId) public view returns (uint256) {
        return s_profileToPostToNumberOfMirrors[_profileId][_pubId];
    }
...
library Errors {
    ...
    error LensAds__NotEnoughPostBudget();
    error LensAds__NotEnoughGlobalBudget();
    error LensAds__FollowerAlreadyMirrored();
    error LensAds__NotSenderProfile();
    error LensAds__BudgetBiggerThanGlobalBudget();
    error LensAds__BudgetEqualZero();
    error LensAds__RewardEqualZero();
}
...
library Events {
    ...
    /**
     * @dev Emitted when the global budget for a given profile is funded.
     * @param profileId The ID of the profile that the global budget is funded for.
     * @param globalBudget The amount of funds added to the global budget.
     * @param currency The currency in which the global budget is funded.
     * @param frontend The address of the frontend contract.
     */
    event GlobalBudgetFunded(
        uint256 profileId,
        uint256 globalBudget,
        address currency,
        address frontend
     ...
     */
    event PostBudgetFunded(uint256 profileId, uint256 pubId, uint256 budget, address currency);

    /**
     * @dev Emitted when the budget for a post is not enough to pay a reward.
     * @param profileId The ID of the profile that the post belongs to.
     * @param pubId The ID of the post that does not have enough budget.
     * @param budget The amount of funds in the post's budget.
     * @param currency The currency in which the post's budget is funded.
     */
    event NotEnoughPostBudget(uint256 profileId, uint256 pubId, uint256 budget, address currency);

    /**
     * @dev Emitted when the global budget for a given profile is not enough to fund a post.
     * @param profileId The ID of the profile that does not have enough global budget.
     * @param pubId The ID of the post that cannot be funded.
     * @param globalBudget The amount of funds in the global budget.
     * @param currency The currency in which the global budget is funded.
     */
    event NotEnoughGlobalBudget(
        uint256 profileId,
        uint256 pubId,
        uint256 globalBudget,
        address currency
    ...
    /**
     * @dev Emitted when funds are withdrawn from a profile's global budget.
     *
     * @param profileId The ID of the profile whose global budget was withdrawn from.
     * @param amount The amount of funds withdrawn.
     * @param currency The currency of the withdrawn funds.
     */
    event GlobalBudgetWithdrawn(uint256 profileId, uint256 amount, address currency);
     ...
     * @param currency The currency of the withdrawn funds.
     */
    event PostBudgetWithdrawn(uint256 profileId, uint256 pubId, uint256 amount, address currency);
}