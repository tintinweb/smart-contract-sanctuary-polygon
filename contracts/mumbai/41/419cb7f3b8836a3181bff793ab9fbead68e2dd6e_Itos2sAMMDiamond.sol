...
contract Itos2sAMMDiamond is IDiamond {
    ...
    constructor() {
...

        {
            ...
            bytes4[] memory adminFunctionSelectors = new bytes4[](7);
            ...
            adminFunctionSelectors[3] = AdminFacet.removeRights.selector;
            adminFunctionSelectors[4] = AdminFacet.halt.selector;
            adminFunctionSelectors[5] = AdminFacet.unhalt.selector;
            adminFunctionSelectors[6] = AdminFacet.confirmUnhalt.selector;
...
contract AdminFacet is BaseAdminFacet {
    /// There is no function here to add rights because that is time-gated in the ParamsFacet
...
contract LiqMakerFacet {
    ...
    function openWideMaker(address recipient, uint8 portID, uint128 liq) external returns (uint256 posId) {
        ...
        // 6. Install it as a position in the PM, now that we have the tokens.
        posId = PMLib.PMInstall(assetId, recipient, portID, false);

        // 7. Update the interest rates
    ...
    function openMaker(
        ...
        // 5. Tally the amount required to deposit.
        {
            ...
            // The above operations have committed their asset to our bookkeeping
        ...
        // 7. Now that we have the tokens, and they're in the right place, and the book keeping is inplace
        // We can tell the PM we have the asset.
        posId = PMLib.PMInstall(assetId, recipient, portID, false);

        // 8. Update the borrow rate.
...
contract LiqTakerFacet {
    ...
    function openTakerCall(
        ...
        // into a new function.
        posId = LiqLib.openTakerCallHelper(recipient, portID, data, rLiq);

        // We update interest rates at the very end of every non-view operation.
    ...
    function openTakerPut(
        ...
        // Solc can't drop the param stack vars summarized in rLiq without jumping
        // into a new function.
        posId = LiqLib.openTakerPutHelper(recipient, portID, data, rLiq);
...
library LiqLib {
    ...
    function openTakerCallHelper(
        ...
        // ALWAYS DO THIS LAST! See openMakerAsset for details.
        InfoStorage storage info = StorageLib.info();
        ...
        // 9. Now that we have the tokens, and they're in the right place, and the book keeping is inplace
        // We can tell the PM we have the asset.
        posId = PMLib.PMInstall(assetId, recipient, portID, true);
    }
    ...
    function openTakerPutHelper(
        ...
        uint256 assetId;
        {
            ...
            // 4. Tally the amount required to deposit.
            {
                ...
                console2.log(terminalX, holdingY);
                console2.log(borrowedX, borrowedY);
                // Make sure the terminal amount can be supplied later.
        ...
        if (borrowedX > 0) {
            console2.log(info.tokenX.balance(), borrowedX);
        ...
        // 10. Now that we have the tokens, and they're in the right place, and the book keeping is inplace
        // We can tell the PM we have the asset.
        posId = PMLib.PMInstall(assetId, recipient, portID, true);
    }
    ...
    function closeMaker(
        ...
        InfoStorage storage info = StorageLib.info();
        if (val.cX > 0) {
            info.tokenX.transfer(recipient, val.cX);
        }
        if (val.cY > 0) {
            info.tokenY.transfer(recipient, val.cY);
        }
    }
    ...
    function closeTakerCall(
        ...
        // Y can't be positive.
        if (x > 0) {
            info.tokenX.transfer(recipient, uint256(x));
        }
        TransferLib.takerExerciseReceive(recipient, info.tokenX, info.tokenY, -x, -y, instructions);
    }
    ...
    function valueTakerCall(
        ...
        takerX -= makerX;
        // We know we can always use the remaining takerX to sufficiently repay the maker amounts.
        if (makerY > 0) {
    ...
    function closeTakerPut(
        ...
        // 5. Give amounts
        InfoStorage storage info = StorageLib.info();
        // X can't be positive here.
        if (y > 0) {
            info.tokenY.transfer(recipient, uint256(y));
        }
        TransferLib.takerExerciseReceive(recipient, info.tokenX, info.tokenY, -x, -y, instructions);
    }
