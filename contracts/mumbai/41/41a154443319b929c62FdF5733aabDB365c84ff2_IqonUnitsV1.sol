...
/// @title The IkonUnits ERC20 Token.
...
error LengthShouldBeSame();
error AmountShouldNotBeZero();
error AddressShouldNotBeZero();
error AddressAlreadySetToValue();
error CannotTransferToUnverifiedAddress();

contract IqonUnitsV1 is Initializable, ERC20Upgradeable, OwnableUpgradeable {
    /// @dev Mapping for verified addresses.
    mapping(address => bool) public isVerifiedAddress;

    /// @dev Events.
    /**
     * `ManagerRoleUpdated` will be fired when an address is granted/revoked manager role.
     * @param account: The account granted or revoked.
     * @param status: The status of the address granted or revoked.
     */
    event ManagerRoleUpdated(address account, bool status);
    /**
    ...
    function initialize() public initializer {
        ...
        _addVerifiedAddress(msg.sender, true);
    }
     ...
     * Required: Has role of `MANAGER_ROLE`.
    ...
    function setVerifiedAddress(
        ...
        _addVerifiedAddress(_address, _isVerified);
    }
    ...
    function setVerifiedAddress(
        ...
        /// @dev Length checking
        if (_address.length != _isVerified.length) revert LengthShouldBeSame();

        /// Adding into verified list.
        for (uint256 i; i < _address.length; i++) {
            _addVerifiedAddress(_address[i], _isVerified[i]);
        }
    }
     ...
     * @param from: The from account address.
    ...
    function _beforeTokenTransfer(
        ...
        if (to != address(0) && !isVerifiedAddress[to])
            revert CannotTransferToUnverifiedAddress();
     ...
     * @param _isVerified: The current status of the address.
     */
    function _addVerifiedAddress(address _address, bool _isVerified) private {
        /// @dev Parameters check.
        if (_address == address(0)) revert AddressShouldNotBeZero();
        if (isVerifiedAddress[_address] == _isVerified)
            revert AddressAlreadySetToValue();
