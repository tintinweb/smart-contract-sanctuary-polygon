    ...
    RequestQueue
{
    ...
    bool isNextRequestCreateNewRequestQueue;
    ...
    uint32 numWords = 2;
    bool isOpenForRandomnessRequest;
    ...
    mapping(uint256 => requestInfo) public requestIndexToRequestInfo;
    ...
    // Request a new deal for a new hand with a given _handID
    // fires off 3 requests to VRF if space is available
    function requestNewDeal(uint256 _handId) internal {
        // Deal 2 cards to player, index preserves order
        sendRandomRequest(true, false, _handId, requestType.dealNewHandPlayer);
        sendRandomRequest(true, false, _handId, requestType.dealNewHandPlayer);

        // Deal 1 card to dealer
        sendRandomRequest(false, true, _handId, requestType.dealNewHandFinal);
    }

    // Request hit for a player
    function requestPlayerHit(uint256 _handId) internal {
        sendRandomRequest(true, false, _handId, requestType.playerHit);
    }

    /// Player has stood
    function requestStand(uint256 _handId) internal {
        // Do to request, response architecture,
        // Cards can only be called once for dealers
        // Dealer will always request 5 cards and use cards until he busts or gets 17 or more
        // Max cards for dealer is 5
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
        sendRandomRequest(false, true, _handId, requestType.dealerHit);
    ...
    function sendRandomRequest(
        ...
        // Check for space in _requestQueue
        if (length() >= numWords * 32) {
            revert(
                "Error: Sorry, all of our dealers are busy. Please try again in a few minutes."
            );
        }

        // Create request and add to queue
        uint256 requestIndex = _requestCounter.current();
        ...
        requestIndexToRequestInfo[requestIndex] = newRequest;
        _requestCounter.increment();
        enqueue(requestIndex);

        // Check for current request sent out for controller
        if (isOpenForRandomnessRequest) {
            requestRandomWords();
            isOpenForRandomnessRequest = false;
        }
    }
        ...
        override
    {
        ...
        uint requestQueueLength = length();
        uint numberOfRandomWordsNeeded;
        // Multiple of 32
        if (requestQueueLength % 32 == 0) {
            numberOfRandomWordsNeeded = requestQueueLength / 32;
        } else {
            numberOfRandomWordsNeeded = (requestQueueLength / 32) + 1;
        }

        for (uint i = 0; i < numberOfRandomWordsNeeded; i++) {
            uint8[] memory randomUint8s = getUint256BrokenIntoUint8(
                randomWords[i]
            );
            uint numberOfRandomIntsNeeded;
            if (length() >= 32) {
                numberOfRandomIntsNeeded = 32;
            } else {
                numberOfRandomIntsNeeded = length();
            }
            for (uint j = 0; j < numberOfRandomIntsNeeded; j++) {
                uint256 requestIndex = dequeue();
                fulfillRandomRequest(requestIndex, randomUint8s[j]);
            }
        }

        isOpenForRandomnessRequest = true;
    }

    // Method called by fulfillRandomWords, that actually
    // fulfills the random request and computes blackjack logic
    function fulfillRandomRequest(uint256 requestIndex, uint8 randomWord)
        internal
    {
        emit RandomnessFulfilled(
            requestIndex,
            requestIndexToRequestInfo[requestIndex].handId,
            randomWord % 13,
            requestIndexToRequestInfo[requestIndex].requestType
        );

        // Get hand
        Hand storage hand = hands[
            requestIndexToRequestInfo[requestIndex].handId
        ];

        // Get card
        uint8 card = randomWord % 13;

        // Mark fulfilled
        requestIndexToRequestInfo[requestIndex].isFulfilled = true;
        requestIndexToRequestInfo[requestIndex].randomCard = card;

        // Dispatch Request to the right type and player
        // Type

        if (
            requestIndexToRequestInfo[requestIndex].requestType ==
            requestType.dealNewHandPlayer
        ) {
            hands[requestIndexToRequestInfo[requestIndex].handId]
                .playerCards
                .push(requestIndexToRequestInfo[requestIndex].randomCard);
        } else if (
            requestIndexToRequestInfo[requestIndex].requestType ==
            requestType.dealNewHandFinal
        ) {
            hands[requestIndexToRequestInfo[requestIndex].handId]
                .dealerCards
                .push(requestIndexToRequestInfo[requestIndex].randomCard);
            setHandTotal(requestIndexToRequestInfo[requestIndex].handId, true);
            setHandTotal(requestIndexToRequestInfo[requestIndex].handId, false);
            hands[requestIndexToRequestInfo[requestIndex].handId]
                .isLocked = false;
        } else if (
            requestIndexToRequestInfo[requestIndex].requestType ==
            requestType.playerHit
        ) {
            hands[requestIndexToRequestInfo[requestIndex].handId]
                .isLocked = false;
            hands[requestIndexToRequestInfo[requestIndex].handId]
            ...
            requestIndexToRequestInfo[requestIndex].requestType ==
            requestType.dealerHit
        ) {
            hand.isLocked = false;
            // Check that hand has already been resolved, from dealer standing or busting
            if (hand.isWon || hand.isLostToDealer || hand.isTied) {
                emit Here();
                return;
            }
            ...
            setHandTotal(requestIndexToRequestInfo[requestIndex].handId, false);
            // Check for dealer bust or stand
            // Dealer bust , player wins
            if (hand.dealerTotal > 21) {
                hand.isWon = true;
                // resolveWonHand(requestIndexToRequestInfo[requestIndex].handId);
                return;
            } // Dealer stands on 17
            // else if (hands[requestIndexToRequestInfo[requestIndex].handId].dealerTotal >= 17) {
            //     // Player wins
            //     if (hands[requestIndexToRequestInfo[requestIndex].handId].playerTotal > hands[requestIndexToRequestInfo[requestIndex].handId].dealerTotal) {
            //         resolveWonHand(requestIndexToRequestInfo[requestIndex].handId);
            //     }
            //     else if (hands[requestIndexToRequestInfo[requestIndex].handId].playerTotal == hands[requestIndexToRequestInfo[requestIndex].handId].dealerTotal) {
            //         resolveTieHand(requestIndexToRequestInfo[requestIndex].handId);
            //     }
            //     else if (hands[requestIndexToRequestInfo[requestIndex].handId].playerTotal < hands[requestIndexToRequestInfo[requestIndex].handId].dealerTotal) {
            //         resolveLostHand(requestIndexToRequestInfo[requestIndex].handId);
            //     }
            //     return;
            // }
        }
    ...
    uint256 public minBet = 0.000000000000000001 ether;
    ...
    event NewHand(uint256 handId, address player, uint256 bet);
    ...
    event LostToDealer(uint256 handId, address player, uint256 value_lost);
    ...
    /// Player cannot interact with hand while randomness is being request for previous action
    modifier onlyUnlocked() {
        uint _handId = playerHand[msg.sender];
        require(
            hands[_handId].isLocked == false,
            "Error: Your hand is locked and waiting for cards to be dealt."
        );
        _;
    }
    ...
    modifier onlyOneHand() {
        require(
            ...
            "Error: You can only play one hand at a time."
        );
        _;
    }

    /// Players must be playing a hand to hit or stand
    modifier onlyAlreadyPlaying() {
        require(
            isPlaying[msg.sender],
            "Error: You can only hit or stand while you're playing a hand."
        );
        _;
    }
    ...
    function deal() public payable onlyBetWithinLimits onlyOneHand {
        ...
        // Request to deal first cards
        requestNewDeal(_handId);

        emit NewHand(_handId, msg.sender, msg.value);
    ...
    /// Detects players hands and hits that
    function hit() public onlyAlreadyPlaying onlyUnlocked {
        uint _handId = playerHand[msg.sender];
        // onlyPlayable
        require(
            hands[_handId].isBusted == false,
        ...
        hands[_handId].isLocked = true;
        requestPlayerHit(_handId);
    }
    ...
    function stand() public onlyAlreadyPlaying onlyUnlocked {
        uint _handId = playerHand[msg.sender];
        // onlyPlayable
        require(
            hands[_handId].isBusted == false,
    ...
    /// Resolved a busted hand
    function resolveBustedHand(uint _handId) private {
        emit HandOver(_handId);
        address player = handOwner[_handId];
        ...
        // Player loses hand
        // Nothing happens!
        emit Busted(_handId, player, hands[_handId].bet);
        // Better luck next time
        return;
    }

    function resolveLostHand(uint _handId) private {
        emit HandOver(_handId);
        address player = handOwner[_handId];
        ...
        // Player loses hand
        // Nothing happens!
        emit LostToDealer(_handId, player, hands[_handId].bet);
        // Better luck next time
        return;
    }

    function resolveWonHand(uint _handId) private {
        emit HandOver(_handId);
        address player = handOwner[_handId];
        ...
        // Player wins hand
        emit Winner(_handId, player, hands[_handId].bet);
        // Congrats
    ...
    function resolveTieHand(uint _handId) private {
        ...
        // Player wins hand
        emit Tie(_handId, player, hands[_handId].bet);
        // Come Again!
        (bool sent, ) = address(payable(msg.sender)).call{
    ...
    function resolveHand(uint256 _handId) private {
        // Resolve Dealer
