...
contract NFTMarketplace is ERC721, ERC721URIStorage {
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://bafybeig5ahbwbwsjgm5dghwekpuhtvreydest3pz75e4a2bwghwlnsc6li.ipfs.nftstorage.link/";
    }
    ...
    {
        return string(
                abi.encodePacked(uri, Strings.toString(_tokenId), ".json")
            );
    }
    ...
    function buyToken(uint256 tokenId) public payable {
        require(idToMarketItem[tokenId].owner != address(0));
        require(msg.sender != address(0));
        require(msg.sender != idToMarketItem[tokenId].owner);
        require(msg.value == idToMarketItem[tokenId].price);

        _safeTransfer(idToMarketItem[tokenId].owner, msg.sender, tokenId, "");

        idToMarketItem[tokenId].author.transfer((msg.value / 100) * comission);
        idToMarketItem[tokenId].owner.transfer((msg.value / 100) * (100 - comission));
