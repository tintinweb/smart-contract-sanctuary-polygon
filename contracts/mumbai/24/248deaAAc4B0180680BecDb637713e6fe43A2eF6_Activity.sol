...
contract SemanticSBT is Ownable, Initializable, ERC165, IERC721Enumerable, ISemanticSBT, ISemanticSBTMetadata {
    ...
    function _buildRDF(SPO storage spo) internal view returns (string memory _rdf){
        _rdf = _buildS(spo);
        ...
        for (uint256 i ; i < len; ) {
            ...
            if (FieldType.INT == fieldType) {
                _rdf = string.concat(_rdf, _buildIntRDF(pIndex, oIndex));
            } else if (FieldType.STRING == fieldType) {
                _rdf = string.concat(_rdf, _buildStringRDF(pIndex, oIndex));
            } else if (FieldType.ADDRESS == fieldType) {
                _rdf = string.concat(_rdf, _buildAddressRDF(pIndex, oIndex));
            } else if (FieldType.SUBJECT == fieldType) {
                _rdf = string.concat(_rdf, _buildSubjectRDF(pIndex, oIndex));
            } else if (FieldType.BLANKNODE == fieldType) {
                _rdf = string.concat(_rdf, _buildBlankNodeRDF(pIndex, oIndex));
            }
            string memory suffix = i == len - 1 ? "." : ";";
    ...
    function _buildS(SPO storage spo) internal view returns (string memory){
        uint256 sIndex = spo.sIndex;
        string memory _className = sIndex == 0 ? SOUL_CLASS_NAME : _classNames[sIndex];
