...
interface ISLSeasonQuest {
    ...
    function getMonsterSetQuestById(
        ...
        view
        returns (
            SeasonQuestBase.Quest memory,
            SeasonQuestBase.MonsterSet memory
        );
    ...
    function getMonsterTraitQuestById(
        ...
        returns (
            SeasonQuestBase.Quest memory,
            SeasonQuestBase.MonsterTrait memory
        );
    ...
    function comfirmMonsterSetQuest(uint256 _questId) external;

    function comfirmMonsterTraitQuest(
        uint256 _questId,
...
abstract contract Confirm is Quest {
    ...
    function confirmGeneralQuest(
        ...
        if (!_verifyQuestSignature(_questSignature, hunter, _questId))
    ...
    function _checkQuest(
        ...
        if (_quest.completableCount != 0) {
            if (
                getQuestCompletedCount(_hunter, _quest.id) >=
        ...
        if (_quest.requiredQuestId != 0) {
            if (getQuestCompletedCount(_hunter, _quest.requiredQuestId) < 1)
    ...
    function _completeQuest(address _hunter, Quest memory _quest) private {
        ...
        questCompleted[_hunter][_quest.id].increment();

        uint256 seasonId = _quest.seasonId;

        uint256 questBadgeCollectionId = seasonContract
            .getSeasonQuestBadgeCollectionId(seasonId);

        // mint quest badge
        if (_quest.questBadgeIds.length > 0) {
            address questBadge = projectContract.getTokenContractByCollectionId(
                questBadgeCollectionId
            );
            ISLMT(questBadge).mintBatch(
                _hunter,
                _quest.questBadgeIds,
                _asUintArray(1, _quest.questBadgeIds.length)
            );
        }
    ...
    function _verifyQuestSignature(
        ...
        bytes32 data = keccak256(abi.encodePacked(_questId, _hunter));
    ...
    function _burnMonster(
        ...
        uint256 monsterCount = _monsterIds.length;

        address[] memory accounts = _asAddressArray(_hunter, monsterCount);
        uint256[] memory balances = monsterContract.balanceOfBatch(
            accounts,
            _monsterIds
        );

        for (uint256 i = 0; i < monsterCount; i = i.increment()) {
            if (balances[i] < _monsterAmounts[i]) {
                revert NotEnoughMonster();
            }
        }
...
abstract contract Quest is ISLSeasonQuest, SeasonQuestBase {
    ...
    function _addQuest(
        ...
        // check quest badge ids
        uint256 questBadgeCollectionId = seasonContract
            .getSeasonById(_quest.seasonId)
            .questBadgeCollectionId;
        ...
        if (_quest.questBadgeIds.length > 0) {
            ...
            if (!ISLMT(questBadge).existsBatch(_quest.questBadgeIds))
                revert InvalidQuestBadgeId();
        }
        ...
        quests[questId] = Quest({
            ...
            questBadgeIds: _quest.questBadgeIds
        });
    ...
    function getQuestCompletedCount(
        ...
        return questCompleted[_hunter][_questId].current();
    }
    ...
    function getMonsterSetQuestById(
        uint256 _questId
    ) external view returns (Quest memory, MonsterSet memory) {
        if (!isExistQuestById(_questId)) revert InvalidQuestId();

        return (quests[_questId], monsterSets[_questId]);
    }

    function getMonsterTraitQuestById(
        uint256 _questId
    ) external view returns (Quest memory, MonsterTrait memory) {
        if (!isExistQuestById(_questId)) revert InvalidQuestId();

        return (quests[_questId], monsterTraits[_questId]);
    }
    ...
    function getQuestScore(
        ...
        return _getQuestScore(_seasonId, _hunter);
    }
    ...
    function getQuestScoreBatch(
        ...
        for (uint256 i = 0; i < hunterCount; i = i.increment()) {
            scores[i] = _getQuestScore(_seasonId, _hunters[i]);
        }

        return scores;
    }

    function _getQuestScore(
        uint256 _seasonId,
        address _hunter
    ...
    SeasonQuestError
{
    ...
    /// @notice hunter to questId to completed count
    mapping(address => mapping(uint256 => CountersUpgradeable.Counter))
