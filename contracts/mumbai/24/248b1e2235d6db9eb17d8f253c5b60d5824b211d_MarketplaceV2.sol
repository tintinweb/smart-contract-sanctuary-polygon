...
contract MarketplaceV2 is Ownable, ReentrancyGuard {
    ...
    */
    function matchTransaction(
        ...
        IMetaFarm721 _nftContr = IMetaFarm721(_addrs[1]);

        try _nftContr.ownerOf(_values[0]) returns (address _owner) {
            require(_owner == _addrs[0], "Item not owned by Seller");
        } catch Error(
            string memory /*reason*/
        ) {
            _nftContr.mint(_addrs[0], _values[0]);
        } catch (
    ...
    */
    function cancelSale(
        ...
        emit CancelSale(_addrs[0], _values[0], _values[2], _sigHash);
    }
    ...
    function getMintMultipleStructHash(
        address _minter,
        address _to,
        address _nft,
        ...
        return
            keccak256(
                abi.encode(
                    keccak256(
                        "MintMultiple(address nft,address minter,address to,uint256[] tokenIds)"
                    ),
                    _nft,
                    _minter,
                    _to,
    ...
    /**
        this function is use for minting new NFTs in the system
        @param _minter address of the minter
        @param _to address receive NFTs
        @param _tokenIds tokenIds of the NFT to mint
        @param _signatures 2 signatures of the minter and the BE verifier
     */
    function mintMultiple(
        ...
        bytes32 structHash = getMintMultipleStructHash(
            _minter,
            _to,
            _nftContract,
            _tokenIds
        ...
        _validateSig(_minter, structHash, _signatures[0]);

        bytes32 msghash = keccak256(_signatures[0]);
        bytes32 verifierMsg = ECDSA.toEthSignedMessageHash(msghash);

        _validateSig(gov.verifier(), verifierMsg, _signatures[1]);

        IMetaFarm721 _nftContr = IMetaFarm721(_nftContract);
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            _nftContr.mint(_to, _tokenIds[i]);
        }
    }
}
...
interface IGovernance {
    ...
    function pausedTransfer() external view returns (bool);
