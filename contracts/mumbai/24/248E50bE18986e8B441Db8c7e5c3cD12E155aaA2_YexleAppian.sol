...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    ...
    event OwnershipTransferOfLand(address indexed to, uint indexed tokenId);
    ...
    */
    function mint(address _l1, address _to, uint256 _tokenId, string memory _tokenUri) external onlyAdmin{
        ...
        if(_l1 == L1Approver){
            ...
            emit OwnershipTransferOfLand(_to, _tokenId);
        }else{
    ...
    */
    function setTokenURI(address _tokenOwner, uint256 _id, string memory _tokenUri) external onlyAdmin{
        require(_tokenOwner == _owners[_id],"you are not the owner");
    ...
    //         emit OwnershipTransferOfLand(_data._sellingTo, _data._tokenId);
    //     }
    // }

    //Write -> Land-view-approval: (L1 approver, letting the buyers to access to view).
    function landDocumentViewRequestApprove(address _l1approver, address _requester, uint tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
    ...
    function requestLandForSale(address _requester, uint _tokenId) external onlyAdmin{
        ...
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        if(viewAccessGranted[_requester][_tokenId]){
    ...
    function ownerDecisionforRaisedRequest(address _ownerOfNft, address _requester, uint _tokenId, bool _status) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_requester);
        if(!status){ revert userContractError();}
        require(_ownerOfNft == _owners[_tokenId],"you are not the owner of nft");
    ...
    function registrationForLandByBuyer(address _req, uint tokenId, string memory _DocumentUri) external onlyAdmin{
        require(ownerAcceptLandSales[_req] == true, "registration is not possible");
    ...
    function approveByL1(approverData memory _data, address _l1) external onlyAdmin{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_l1 != L1Approver){ revert notL1Approver();}
        if(voteRecord[msg.sender][_data._tokenId]){ revert alreadyApproved();}
    ...
    function approveByL2(approverDataForL2 memory _data, address _l2) external{
        UserContract useC = UserContract(userContract);
        (bool status) = useC.verifyUser(_data._sellingTo);
        if(!status){ revert userContractError();}
        if(_l2 != L2Approver){ revert notL2Approver();}
