...
contract Brainz_Kidz is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 10;

    
    uint256 public MAIN_SUPPLY = 9;
    
    bool public IS_MAIN_ACTIVE = false;

    
    uint256 public MAIN_TX_LIMIT = 1;

    
    uint256 public MAIN_WALLET_LIMIT = 1;

    
    uint256 public MAIN_PRICE = 0 ether;

    
    uint256 public MAIN_COUNT = 0;

    

    
    mapping(address => uint) public MAIN_Minted;

    bool public _revealed = false;

    string private baseURI = "";
    string private preRevealURI = "https://ipfs.w3bmint.xyz/ipfs/QmZXc2NCkP7YzYc3PGhjE8b858JPcMAkxABPyshFzX9CYf";
    ...
    constructor() ERC721A("Brainz_Kidz", "BKDZ") {
    ...
    function MAINMint(address to, uint256 numberOfTokens) external isSecured(MAIN_PRICE * numberOfTokens) payable {
        validateMint(IS_MAIN_ACTIVE, MAIN_COUNT, MAIN_SUPPLY, MAIN_Minted[msg.sender], MAIN_WALLET_LIMIT, MAIN_TX_LIMIT, numberOfTokens, true);
        addressBlockBought[to] = block.timestamp;
        MAIN_Minted[to] += numberOfTokens;
        MAIN_COUNT += numberOfTokens;
    ...
    function toggleMAINMintStatus() external onlyOwner {
        IS_MAIN_ACTIVE = !IS_MAIN_ACTIVE;
    }

    // SUPPLY CHANGERS
    
    function setMAINSupply(uint256 _supply) external onlyOwner {
        MAIN_SUPPLY = _supply;
    }
    ...
    function setMAINPrice(uint256 _price) external onlyOwner {
        MAIN_PRICE = _price;
    }
