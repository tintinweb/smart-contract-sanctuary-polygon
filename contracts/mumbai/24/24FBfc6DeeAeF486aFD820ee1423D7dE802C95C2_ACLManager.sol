...
contract ACLManager is AccessControl, IACLManager {
  ...
  bytes32 public constant override POOL_RESUPPLY_ROLE = keccak256('POOL_RESUPPLY');
  ...
  /// @inheritdoc IACLManager
  function addPoolSupplier(address supplier) external override {
    grantRole(POOL_RESUPPLY_ROLE, supplier);
  }

  /// @inheritdoc IACLManager
  function removePoolSupplier(address supplier) external override {
    revokeRole(POOL_RESUPPLY_ROLE, supplier);
  }

  /// @inheritdoc IACLManager
  function isPoolSupplier(address supplier) external view override returns (bool) {
    return hasRole(POOL_RESUPPLY_ROLE, supplier);
  }
