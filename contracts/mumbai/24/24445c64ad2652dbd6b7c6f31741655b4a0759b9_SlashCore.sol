...
interface ISlashCore {
     ...
     * @param info ISlashCore.PaymentInfo
...
contract SlashCore is ISlashCore, Ownable {
    ...
    function handleFee(
        ...
        if (feeAmount > 0 && !_merchantsRegistered[merchant_]) {
            _merchantsRegistered[merchant_] = true;
            _merchants.push(merchant_);
        }

        // Fee will be processed only when it is more than specific amount
        // When admin try to handle all fee, no need to check threshold
        if (!forced_ && feeAmount < merchant.viewMinAmountToProcessFee()) {
            _donationFeePerMerchant[merchant_] = donationFeeAmount;
            _taxFeePerMerchant[merchant_] = taxFeeAmount;
            return;
        }
    ...
    /**
     * @dev Admin process all fees
     * Specially needed this operation for the safe-migration of SlashCore
     */
    function forceProcessAllFees(
        address donationWallet_,
        address marketingWallet_
    ) external onlyOwner {
        uint256 merchantCount = _merchants.length;
        for (uint256 i = 0; i < merchantCount; i++) {
            IMerchantProperty merchant = IMerchantProperty(_merchants[i]);
            // Filter merchants only that were deployed from slash owner
            if (
                merchant.viewDonationWallet() == donationWallet_ &&
                merchant.viewMarketingWallet() == marketingWallet_
            ) {
                handleFee(_merchants[i], 0, 0, true);
            }
        }
        emit AdminProcessedAllFee();
    }
     ...
     * @param info: ISlashCore.PaymentInfo
     */
    function doMerchantDeposit(PaymentInfo memory info)
        private
        returns (uint256 redundantAmount, uint256 cashBackAmount)
    {
        // Blacklisted token can not be used as paying token
        IMerchantProperty merchant = IMerchantProperty(info.merchant);
        require(
            !merchant.isBlacklistedFromPayToken(info.payingToken),
            "Blacklisted token"
        );
        ...
        cashBackAmount = info
            .requiredAmount
            .mul(merchant.viewCashBackPercent())
            .div(10000);

        // In case of payng token is same as receive token
        if (
            info.payingToken == receiveToken ||
            (IERC20(info.payingToken).isETH() && IERC20(receiveToken).isETH())
        ) {
            require(
                info.amountIn >= info.requiredAmount,
                "Insufficient amountIn_"
            );
            // Transfer token directly to merchant wallet
            IERC20(receiveToken).universalTransfer(
                merchant.viewMerchantReceiveWallet(),
                info.requiredAmount.sub(cashBackAmount)
            );
            // Transfer redundant token and cashback token back to the user
            redundantAmount = info.amountIn.sub(info.requiredAmount);
            IERC20(receiveToken).universalTransfer(
                info.account,
                redundantAmount.add(cashBackAmount)
            );
        } else {
            (address[] memory exchanges, uint256[] memory flags) = merchant
                .viewExchanges();
            IERC20(info.payingToken).universalApprove(
                address(_dexAggregator),
                uint256(int256(-1))
            );
            // prevent deep stack error
            redundantAmount = IERC20(receiveToken).universalBalanceOf(
                address(this)
            );
            _dexAggregator.swap{
                value: IERC20(info.payingToken).isETH() ? info.amountIn : 0
            }(
                IERC20(info.payingToken),
                IERC20(receiveToken),
                info.path,
                info.amountIn,
                info.requiredAmount.sub(cashBackAmount),
            ...
            // prevent deep stack error
            redundantAmount = IERC20(receiveToken)
                .universalBalanceOf(address(this))
                .sub(redundantAmount);
            // refund redundant amount and cashback
            if (redundantAmount > 0) {
                IERC20(receiveToken).universalTransfer(
                    info.account,
                    redundantAmount
                );
                redundantAmount = redundantAmount.sub(cashBackAmount);
            }
        }
    }

    /**
     * @dev Swap token to receive token and call the method in merchant contract
     * @param info: ISlashCore.PaymentInfo
     * @return redundantAmount Redundant token amount that is refunded to the user
     */
    function doMerchantDepositContractCall(PaymentInfo memory info)
        private
        returns (uint256 redundantAmount, uint256 cashBackAmount)
    {}
    ...
    function getTxFeeAmount(
        ...
        if (
            ...
            web3Token.universalBalanceOf(account_) >= web3BalanceForFreeTx
        ) {
            return 0;
        }
    ...
    function getFeeAmount(
        ...
        if (IERC20(receiveToken).isETH() || receiveToken == swapRouter.WETH()) {
     ...
     * @return refFeeBal Redundant fee amount that is refunded to the user
     */
    function submitTransaction(PaymentInfo memory info)
        external
        payable
        override
        returns (
    ...
    {
        require(info.account == tx.origin, "account is not tx caller");
        require(info.amountIn > 0, "_amountIn > 0");
        require(
            !IMerchantProperty(info.merchant).isBlacklistedFromPayToken(
                info.payingToken
            ),
            "blacklisted"
        );
        ...
        {
            // <- added scope to prevent deep stack error
            uint256 balanceBefore = IERC20(info.payingToken).universalBalanceOf(
                address(this)
            );

            if (!IERC20(info.payingToken).isETH()) {
                IERC20(info.payingToken).universalTransferFrom(
                    info.account,
                    address(this),
                    info.amountIn
                );
                info.amountIn = IERC20(info.payingToken)
                    .universalBalanceOf(address(this))
                    .sub(balanceBefore);
            }
        }

        {
            ...
            uint256 ethRequired = taxFeeAmount.add(donationFeeAmount).add(
                IERC20(info.payingToken).isETH() ? info.amountIn : 0
            ); // msg.value required for this transaction

            require(msg.value >= ethRequired, "Insufficient msg.value");

            // Handle fee
            handleFee(info.merchant, taxFeeAmount, donationFeeAmount, false);

            // Return redundant ETH back to user
            if (msg.value > ethRequired) {
                refFeeBal = msg.value.sub(ethRequired);
                payable(info.account).transfer(refFeeBal);
            }
        }

        // Swap token to receive token and transfer to the merchant wallet
        // update below condition if merchant want to receive in their contract in future update.
        // it can be use MerchantProperty._reserved
        if (true) {
            (refTokBal, cashBack) = doMerchantDeposit(info);
        } else {
            (refTokBal, cashBack) = doMerchantDepositContractCall(info);
        }
    }
    ...
    function viewMerchantsCount() external view onlyOwner returns (uint256) {
        return _merchants.length;
    }

    function viewMerchantsPaginated(uint256 offset_, uint256 length_)
        external
        view
        onlyOwner
        returns (address[] memory merchants)
    {
        if (offset_ >= _merchants.length) {
            merchants = new address[](0);
        } else {
            uint256 start = offset_;
            uint256 finish = (offset_ + length_) >= _merchants.length
                ? _merchants.length
                : offset_ + length_;
            merchants = new address[](finish - start);
            uint256 i;
            for (uint256 index = start; index < finish; index++) {
                merchants[i] = _merchants[index];
                i++;
            }
        }
    }
