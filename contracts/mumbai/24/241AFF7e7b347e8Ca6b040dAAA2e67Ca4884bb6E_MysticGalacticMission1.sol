...
contract MysticGalacticMission1 is ERC721, Ownable, Pausable, AccessControlEnumerable, ReentrancyGuard {
    ...
    uint16  public immutable MAX_TOKENS_PER_DAY;

    uint16  private tokenCounter;
    uint256 private lastMintedDay;
    uint16  private tokensMintedToday;
    ...
    event TokenMinted(uint256 tokenId, address minter, string tokenURI, uint256 price, uint256 pricePaid);
     ...
     * @param _usdPrice The initial price for minting a token in USD.
    ...
    constructor(
        ...
        console.log("constructing with price is ", usdPrice.toString());
     ...
     */
    function setPrice(uint256 _usdPrice) external onlyOwnerOrAdmin {
        usdPrice = _usdPrice;
     ...
     */
    function setPriceFeed( address _priceFeedAddress ) public onlyOwnerOrAdmin {
    ...
    function getCurrentPrice() external view returns (uint256) {
        return Util.convertUSDtoMATIC(usdPrice, 8, priceFeed);
    }
    ...
    function mint() public payable nonReentrant whenNotPaused returns (string memory) {
        ...
        uint256 currentPrice = Util.convertUSDtoMATIC(usdPrice, 8, priceFeed);
        require(msg.value >= currentPrice, "Funds sent is less than the price of token");
        ...
        emit TokenMinted(tokenId, msg.sender, uri, currentPrice, msg.value);
     ...
     */
    function setLaunchEventDate(uint256 _launchEventDate) external onlyOwnerOrAdmin {
     ...
     */
    function getLaunchEventDate() external view returns (uint256) {
     ...
     */
    function getRecoveryEventDate() external view returns (uint256) {
...
library Util {
     ...
     * @param priceFeed Feed of price convert
     */
    function convertUSDtoMATIC(uint256 usdAmount, uint8 usdDecimals, AggregatorV3Interface priceFeed) public view returns (uint256) {
...
//    function convertUSDtoMATIC(uint256 usdAmount, usdDecimals, AggregatorV3Interface priceFeed) public view returns (uint256) {
//        (, int price, , ,) = priceFeed.latestRoundData();
//        priceDecimals = priceFeed.decimals();
//
//        require(price > 0, "Price feed error");
//
//        // Calculate the equivalent MATIC amount with 8 decimal precision
//        uint256 maticAmount = usdAmount * 1e8 / uint256(price);
//        return maticAmount;
//    }
