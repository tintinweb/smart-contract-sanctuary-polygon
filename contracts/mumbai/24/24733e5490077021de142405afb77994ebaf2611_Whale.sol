...
pragma solidity 0.6.12;
contract Whale is ERC20Burnable, Operator {
    using SafeMath for uint256;

    // TOTAL MAX SUPPLY = 50,000 Whales
    ...
    uint256 public constant DIGITS_DAO_ALLOCATION = 5000 ether;
    ...
    uint256 public digitsDaoRewardRate;

    address public communityFund;
    address public devFund;
    address public digitsDaoFund;

    uint256 public communityFundLastClaimed;
    uint256 public devFundLastClaimed;
    uint256 public digitsDaoFundLastClaimed;
    ...
    constructor(uint256 _startTime, address _communityFund, address _devFund, address _daoFund) public ERC20("Sea Finance", "Whale") {
        _mint(msg.sender, 100 ether); // mint 100 Whale for initial pools deployment
        ...
        digitsDaoFundLastClaimed = startTime;

        communityFundRewardRate = COMMUNITY_FUND_POOL_ALLOCATION.div(VESTING_DURATION);
        devFundRewardRate = DEV_FUND_POOL_ALLOCATION.div(VESTING_DURATION);
        digitsDaoRewardRate = DIGITS_DAO_ALLOCATION.div(VESTING_DURATION);
    ...
    function unclaimedDigitsDaoFund() public view returns (uint256 _pending) {
        uint256 _now = block.timestamp;
        if (_now > endTime) _now = endTime;
        if (digitsDaoFundLastClaimed >= _now) return 0;
        _pending = _now.sub(digitsDaoFundLastClaimed).mul(digitsDaoRewardRate);
    }
    ...
    function claimRewards() external {
        ...
        _pending = unclaimedDigitsDaoFund();
        if (_pending > 0 && digitsDaoFund != address(0)) {
            _mint(digitsDaoFund, _pending);
            digitsDaoFundLastClaimed = block.timestamp;
        }
    }
