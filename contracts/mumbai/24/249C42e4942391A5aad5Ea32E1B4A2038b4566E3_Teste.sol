...
//File： fs://008c665287a04b6c8560f70880dfde3f/Ticket.sol

// File: @openzeppelin/[email protected].5.0/utils/cryptography/MerkleProof.sol
...
contract Teste is ERC721A, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  bool private _isFreeMint = true;
  uint256 private _nftEtherValue = 1 ether;
  ...
  constructor(
  ...
  ) ERC721A("|__name__|", "|__symbol__|") {
    setBaseURI(_initBaseURI);
    root = _root;
    _contractUri = _contractURI;
    _contractOwner = msg.sender;
    walletsOwner[wMobiup] = wMobiup;
    ...
    function mintWhitelist(uint256 _mintAmount,
      ...
      uint256 _indicationType, //1=directlink, 2=affiliate, 3=recomendation
      address payable endUser,
      bytes32[] memory proof) public payable {
    ...
    function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
      ...
      uint256 amountM = ownerAmount * _royalties / 100;
        
      payable(wMobiup).transfer(amountM);
      ownerAmount = ownerAmount - amountM;
