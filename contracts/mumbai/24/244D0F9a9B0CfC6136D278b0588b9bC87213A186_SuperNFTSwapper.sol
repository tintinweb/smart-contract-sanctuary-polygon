...
contract SuperNFTSwapper {
    using SuperTokenV1Library for ISuperToken;

    ISuperToken payWithToken;

    constructor(
        ISuperToken payWithToken_ // token used to purchase an NFT, usually stable coin
    ) {
        payWithToken = payWithToken_;
    }
    ...
    function StartSwap(
        ...
        //pull a stream from buyer and to seller
        //pull a FFT steram from seller to buyer
        require(flowRate > 0, "Only Positive FlowRate Accepted");

        payWithToken.createFlowFrom(msg.sender, seller, flowRate);
        uint256 timeForCompletePurchase = uint(int(price) / flowRate); // time taken to completely purchase NFT
        int96 flowRateForFFToken = int96(
            int((100 * 1e18) / timeForCompletePurchase)
        ); // flowRate so that it's proportional to the time it takes to purchase NFT; 100 *1e18 is the total shares for a NFT;
        FFToken.createFlowFrom(seller, msg.sender, flowRateForFFToken);
    }
}