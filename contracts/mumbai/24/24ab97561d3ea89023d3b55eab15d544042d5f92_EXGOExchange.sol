...
// File: @openzeppelin/contracts/token/ETH721/IETH721.sol
...
// File: @openzeppelin/contracts/token/ETH20/IETH20.sol

pragma solidity ^0.6.0;

/**
 * @dev Interface of the ETH20 standard as defined in the EIP.
 */
interface IETH20 {
    /**
...
// File: @openzeppelin/contracts/token/ETH1155/IETH1155.sol

pragma solidity ^0.6.2;

/**
 * @dev Required interface of an ETH1155 compliant contract, as defined in the

 *
 * _Available since v3.1._
 */
interface IETH1155 is IETH165 {
    /**
     ...
     * returned by {IETH1155MetadataURI-uri}.
     */
    ...
    /**
     * @dev xref:ROOT:ETH1155.adoc#batch-operations[Batched] version of {balanceOf}.
     *
     * Requirements:
     *
     ...
     * - If `to` refers to a smart contract, it must implement {IETH1155Receiver-onETH1155Received} and return the
     * acceptance magic value.
     */
    function safeTransferFrom(address from, address to, uint256 id, uint256 amount, bytes calldata data) external;

    /**
     * @dev xref:ROOT:ETH1155.adoc#batch-operations[Batched] version of {safeTransferFrom}.
     *
     ...
     * - If `to` refers to a smart contract, it must implement {IETH1155Receiver-onETH1155BatchReceived} and return the
     * acceptance magic value.
     */
...
contract OrderBook is Ownable {
    ...
    struct Order {
        OrderKey key;
        /* how much has owner (in wei, or UINT256_MAX if ETH-721) */
        uint256 selling;
        /* how much wants owner (in wei, or UINT256_MAX if ETH-721) */
        uint256 buying;
...
contract OrderState is OrderBook {
    ...
    function setCompletedBidOrder(
        ...
        completed[
            getBidOrdETHompletedKey(order, buyer, buyingAmount)
        ] = newCompleted;
    }
    ...
    function getBidOrdETHompletedKey(
        OrderBook.Order memory order,
        address buyer,
        uint256 buyingAmount
    ...
    function validateBidOrderSignature(
        ...
        require(
            completed[getBidOrdETHompletedKey(order, bidder, buyingAmount)] !=
                true,
            "Signature exist"
        );
        if (sig.v == 0 && sig.r == bytes32(0x0) && sig.s == bytes32(0x0)) {
...
contract TransferSafe {
    ...
    function ETH721safeTransferFrom(
        IETH721 token,
        address from,
        address to,
        uint256 tokenId
    ...
    function ETH1155safeTransferFrom(
        IETH1155 token,
        address from,
        address to,
        uint256 id,
    ...
    function ETH1155safeMintTransferFrom(
        IEXGOStore token,
        address from,
        address to,
        uint256 id,
        ...
        require(
            ...
            "TransferSafe:ETH1155safeMintTransferFrom:: transaction Failed"
        );
    }

    function ETH1155safeMintwithuriTransferFrom(
        IEXGOStore token,
        address from,
        address to,
        uint256 id,
        ...
        require(
            ...
            "TransferSafe:ETH1155safeMintWithUriTransferFrom:: transaction Failed"
        );
    }
}
...
contract EXGOExchange is OrderState, TransferSafe {
    ...
    function transferToken(
        ...
        if (order.key.sellAsset.assetType == AssetType.ETH721 || order.key.buyAsset.assetType == AssetType.ETH721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ETH721safeTransferFrom(
                        IETH721(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId
                    );
                ...
                if (!isStore) {
                    ETH721safeTransferFrom(
                        IETH721(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId
                    );
        ...
        } else if (order.key.sellAsset.assetType == AssetType.ETH1155 || order.key.buyAsset.assetType == AssetType.ETH1155) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ETH1155safeTransferFrom(
                        IETH1155(order.key.sellAsset.token),
                        order.key.owner,
                        buyer,
                        order.key.sellAsset.tokenId,
                        order.selling
                ...
                if (!isStore) {
                    ETH1155safeTransferFrom(
                        IETH1155(order.key.buyAsset.token),
                        buyer,
                        order.key.owner,
                        order.key.buyAsset.tokenId,
                        order.buying
    ...
    function transferETH20Fee(
        ...
        if (remaining > 0) {
            require(
                IETH20(token).transferFrom(buyer, _seller, remaining),
                "Failed transfer"
            );
        }
    }
    ...
    function fiatMethod(uint256 _type,address _nftAddress , address _to , uint256 _id,string memory uri ,uint _nftType)external{
        ...
        if(_type == 1){
            ...
            require(_nftAddress == address(_EXGOStore),"Invalid Token Address");
             ETH1155safeMintwithuriTransferFrom(IEXGOStore(_EXGOStore),owner(),_to,_id,1,uri);  
        }
        else{
            require(_nftAddress != address(0x00),"Invalid Token Address");
            require(_nftType == 1 || _nftType == 2,"Invalid NFT Type");
            if (_nftType == 1){
                 ETH721safeTransferFrom(IETH721(_nftAddress),_msgSender(),_to,_id);
            }
            else{
                ETH1155safeTransferFrom(IETH1155(_nftAddress), _msgSender(),_to,_id,1);
            }
        }
    }
    ...
    function inCaseTokensGetStuck(address _token) external onlyOwner {
        if (_token != address(0x000)) {
            uint256 amount = IETH20(_token).balanceOf(address(this));
            IETH20(_token).transfer(msg.sender, amount);
        } else {
