...
// File: contracts/EtheremonBulkUpdateUri.sol
...
contract EtheremonBulkUpdateUri is BasicAccessControl {
    mapping(uint64 => uint32) public neglectedTokens;
    ...
    constructor(address _monsterNFTContract, address _dataContract) public {
        monsterNFTContract = _monsterNFTContract;
        dataContract = _dataContract;
    }

    function setContracts(address _monsterNFTContract) public onlyModerators {
        monsterNFTContract = _monsterNFTContract;
    }

    function setNeglectedToken(uint64 _tokenId, uint32 _classId)
        public
        onlyModerators
    {
        if (_classId < 1 || neglectedTokens[_tokenId] == _classId) return;
        neglectedTokens[_tokenId] = _classId;
    }

    function removeNeglectedToken(uint64 _tokenId) public onlyModerators {
        if (neglectedTokens[_tokenId] == 0) return;
        delete neglectedTokens[_tokenId];
    }

    function setBulkTokenURI(
        uint64 _startMonId,
        uint64 _endMonID,
        string memory _uri
    ) public onlyModerators {
        EtheremonMonsterNFTInterface monsterContract = EtheremonMonsterNFTInterface(
                monsterNFTContract
            );
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        uint64 temp = _startMonId + 1000;

        for (; _startMonId <= _endMonID * 10; _startMonId++) {
            if (_startMonId >= temp) {
                return limitReached(_startMonId, _endMonID, _uri);
            }
            ...
            (
                ...
                obj.lastClaimIndex,
                obj.createTime
            ) = data.getMonsterObj(_startMonId);

            if (neglectedTokens[_startMonId] != obj.classId) {
                monsterContract.setTokenURI(_startMonId, _uri);
            }
        }
    }
    ...
    function limitReached(
        ...
        for (; _startMonId <= _endMonID * 10; _startMonId++) {
            if (_startMonId >= temp) {
                temp += 1000;
                return setBulkTokenURI(_startMonId, _endMonID, _uri);
            }
            MonsterObjAcc memory obj;
            (
                obj.monsterId,
        ...
        returns (string memory)
    {
        ...
        return monsterContract.tokenURI(_tokenId);
    }
}