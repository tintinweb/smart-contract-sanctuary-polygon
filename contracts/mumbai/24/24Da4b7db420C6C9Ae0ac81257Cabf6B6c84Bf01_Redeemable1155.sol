...
contract Redeemable1155 is ERC1155, AccessControlEnumerable {
  ...
  // ----------------- Role Checks -------------------- //
    ...
    modifier onlyOwnerOrAdmin() {
        require(hasRole(OWNER_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender), "Unauthorized");
        _;
    }

    modifier onlyOwnerAdminOrOperator() {
        require(hasRole(OWNER_ROLE, msg.sender) || hasRole(ADMIN_ROLE, msg.sender) || hasRole(OPERATOR_ROLE, msg.sender), "Unauthorized");
        _;
    }
  ...
  // ----------------- Constructor -------------------- //
    ...
    constructor(
      ...
      address _ownerAddress
        ...
        if (_redemptionAgentAddress == address(0) || _ownerAddress == address(0) || _adminAddress == address(0)) {
    ...
    function updateTokenURI(string memory _newURI) external onlyOwnerOrAdmin {
    ...
    function addUsersToWhitelist(WhitelistedUser[] memory _users) public onlyOwnerAdminOrOperator {
        for (uint i = 0; i < _users.length; i++) {
            whitelistedUsers[_users[i].whitelistedAddress] = _users[i].mintsRemaining;
        }
    }
    ...
    function setMintingPeriod(uint256 startDate, uint256 endDate) public onlyOwnerAdminOrOperator {
   ...
   function mint() public  {
        ...
        require(block.timestamp>= mintStartDate && block.timestamp <= mintEndDate);
    ...
    function grantRedemptionAgentRole(address _address) public onlyOwnerOrAdmin
    {
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(AccessControlEnumerable, ERC1155) returns (bool) {
