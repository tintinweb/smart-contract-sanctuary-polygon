...
contract MyNFT is ERC721Drop {
    constructor(
        ...
        address _primarySaleRecipient,
        uint8 chainIndex,
        uint256 rangeSupplyCap,
        uint8 numberOfChains
    )
        ERC721Drop(
            _name,
            _symbol,
    ...
    function tokenMint(uint256 tokenId)public onlyOwner{
        require(!_exists(tokenId), "Token already exists");
        _bridgeMint(msg.sender, tokenId);
    }
    
    function _setBaseURI(string memory _URI) public override onlyOwner{
        _baseuri = _URI;
    }
}
 ...
 */
contract ERC721C is Context, ERC165, IERC721A {
    using TWAddress for address;
    ...
    // Cross Chain changes
    // ##NOTE: rangeSupplyCap should not have decimal value when devided by _numberOfChains 
    uint8 public _chainIndex;//Chain index, if deployed on 5 chains then within range 1-5
    uint256 public _rangeSupplyCap;//Total range of all chains eg 10000 for 10K collection
    uint256 public _totalSupply;
    uint256 public _numberOfChains;

    // Range restrictions for this contracts minting
    uint256 public _minIndex;
    uint256 public _maxIndex;
    event IndexIncreased(uint256 newIndex);

    constructor(string memory name_, string memory symbol_, uint8 chainIndex_, uint256 rangeSupplyCap_, uint8 numberOfChains_) {
        _name = name_;
        _symbol = symbol_;
        _chainIndex = chainIndex_;
        _rangeSupplyCap = rangeSupplyCap_;
        _numberOfChains = numberOfChains_;
        _minIndex = _currentIndex = (_chainIndex * (rangeSupplyCap_ / numberOfChains_)) - ((rangeSupplyCap_ / numberOfChains_) - 1);
        _maxIndex = _chainIndex * (rangeSupplyCap_ / numberOfChains_);
    }
     ...
     */
     function _ownershipOf(uint256 tokenId) internal view returns (TokenOwnership memory) {
        ...
        require(_exists(curr),"Token does not exist");
        unchecked {
            if (1 <= curr) {
                ...
                if(!ownership.burned && !inRange(curr)){
                    return ownership;
                }
                ...
                while (true) {
                    curr--;
                    if (!inRange(curr)) {
                        uint256 batch = getBatchNumber(curr+1);
                        if(batch > 1){
                            uint256 maxIndex = _chainIndex * (_rangeSupplyCap / _numberOfChains);
                            curr = ((batch - 1)*_rangeSupplyCap) + maxIndex;
                        }
                    }
    ...
    function getBatchNumber(uint256 tokenId) internal view returns (uint256) {
        uint256 batchNumber = tokenId / _rangeSupplyCap + 1;
        return batchNumber;
    }
    ...
    function _setBaseURI(string memory _URI) public virtual{
        _baseuri = _URI;
    }
    ...
    function _exists(uint256 tokenId) internal view returns (bool) {
        if (inRange(tokenId)) {
            //Token is within the range of Natively minted tokens
            return true;
        } else if (!_ownerships[tokenId].burned && _ownerships[tokenId].addr != address(0)) {
            //Token is not within range of natively minted tokens but is found in bridged tokens
            return true;
        }
        return false;
    }

    function inRange(uint256 tokenId) public view returns (bool) {
        //First check if the token does not exceed the currentIndex
        if(tokenId >= _currentIndex){
            return false;
        }
        uint256 minIndex = (_chainIndex * (_rangeSupplyCap / _numberOfChains)) - ((_rangeSupplyCap / _numberOfChains) - 1);
        uint256 maxIndex = _chainIndex * (_rangeSupplyCap / _numberOfChains);
        uint256 offset = tokenId % _rangeSupplyCap;
        return (offset >= minIndex && offset <= maxIndex);
    }
    ...
    function _safeMint(
        ...
        unchecked {
            ...
            uint256 endTokenId = calculateEndTokenId(startTokenId, quantity);
            ...
            if (to.isContract()) {
                do {
                    ...
                    // Check for TokenID Range overflow
                    if (updatedIndex == _maxIndex) {
                        _minIndex += _rangeSupplyCap;
                        updatedIndex = _minIndex;
                        _maxIndex += _rangeSupplyCap;
                        emit IndexIncreased(_minIndex);
                    }else{
                        updatedIndex++;
                    }
                } while (updatedIndex < endTokenId);
    ...
    function _mint(address to, uint256 quantity) internal {
        ...
        unchecked {
            ...
            uint256 updatedIndex = startTokenId;
            uint256 endTokenId = calculateEndTokenId(startTokenId, quantity);  // Call the new function
    ...
    function calculateEndTokenId(uint256 startTokenId, uint256 quantity) public view returns (uint256) {
        uint256 endTokenId = (startTokenId - 1) + quantity;
        if (endTokenId > _maxIndex) {
            uint256 offset = (endTokenId - _maxIndex)-1;
            endTokenId = _minIndex + _rangeSupplyCap + offset;
        }
        return endTokenId;
    }
    ...
    function _batchBridgeMint(address to, uint256[] memory tokenIds) internal {
        require(to != address(0), "Mint to the zero address");
        
        _addressData[to].balance += uint64(tokenIds.length);
        _addressData[to].numberMinted += uint64(tokenIds.length);
    ...
    function _transfer(
        ...
        unchecked {
            ...
            // check if the token is in the sequential range then update the next token
            // If tokenId is not in the sequential range then its a Bridge Mint, ignore it
            if(inRange(tokenId)){
                // If the ownership slot of tokenId+1 is not explicitly set, that means the transfer initiator owns it.
                // Set the slot of tokenId+1 explicitly in storage to maintain correctness for ownerOf(tokenId+1) calls.
                uint256 nextTokenId = tokenId + 1;

                // Added this check if the nextTokenId > currentToken Range max to get the minIndex of the next minting range
                if(!inRange(nextTokenId)){
                    uint256 batch = getBatchNumber(tokenId);
                    uint256 minIndex = (_chainIndex * (_rangeSupplyCap / _numberOfChains)) - ((_rangeSupplyCap / _numberOfChains) - 1);
                    nextTokenId = (batch * _rangeSupplyCap) + minIndex;
                }
    ...
    function _burn(uint256 tokenId, bool approvalCheck) internal virtual {
        ...
        unchecked {
            ...
            // Added this check if the nextTokenId > currentToken Range max to get the minIndex of the next minting range
            if(!inRange(nextTokenId)){
                uint256 batch = getBatchNumber(nextTokenId);
                uint256 minIndex = (_chainIndex * (_rangeSupplyCap / _numberOfChains)) - ((_rangeSupplyCap / _numberOfChains) - 1);
                nextTokenId = ((batch-1)*_rangeSupplyCap) + minIndex;
            }
 ...
 *
 *  The `ERC721Drop` contract implements the ERC721 NFT standard, along with the ERC721C optimization to the standard.
    ...
    DropSinglePhase
{
    ...
    constructor(
        ...
        uint256 rangeSupplyCap,
        uint8 numberOfChains
    ) ERC721C(_name, _symbol, chainIndex, rangeSupplyCap, numberOfChains) {
        _setupOwner(msg.sender);
        _setupDefaultRoyaltyInfo(_royaltyRecipient, _royaltyBps);
        _setupPrimarySaleRecipient(_primarySaleRecipient);
        _setOperatorRestriction(true);
        nextTokenIdToLazyMint = _currentIndex;
    }
     ...
     *  @dev            ERC721C's `_burn(uint256,bool)` internally checks for token approvals.
     *
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public virtual override(ERC721C) onlyAllowedOperator(from) {
