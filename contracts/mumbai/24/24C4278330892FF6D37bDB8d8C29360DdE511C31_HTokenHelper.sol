...
contract HTokenHelper is HTokenHelperI, Ownable {
  ...
  uint256 public updateThreshold = 1 days;
   ...
   */
  function getDebt(HTokenI _hToken) external view override returns (uint256) {
    uint256 totalBorrows = _hToken.totalBorrows();
    return totalBorrows + ((totalBorrows * (_hToken.interestIndex() - _hToken.interestIndexStored())) / 10**_hToken.decimals());
  }
  ...
  function getMarketOraclePrices(HTokenI _hToken) external view override returns (uint256, uint256) {
    ...
    (uint128 floorPriceInETH, uint128 lastUpdated) = priceOracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), decimals);

    if (lastUpdated < block.timestamp && block.timestamp - lastUpdated > updateThreshold) revert OracleNotUpdated();

    uint256 underlyingPriceInUSD = uint256(priceOracle.getUnderlyingPriceInUSD(_hToken.underlyingToken(), decimals));
    uint256 ethPrice = uint256(priceOracle.getEthPrice(decimals));

    return ((floorPriceInETH * ethPrice) / underlyingPriceInUSD, underlyingPriceInUSD);
  }
  ...
  function getFloorPriceInUnderlying(HTokenI _hToken) external view returns (uint256) {
    uint256 floorPrice = getFloorPriceInUSD(_hToken);
    uint256 underlyingPriceInUSD = getUnderlyingPriceInUSD(_hToken);
  ...
  function getMaxBorrowableAmountInUnderlying(HTokenI _hToken, HivemindI _hivemind) external view returns (uint256) {
    uint256 floorPrice = getFloorPriceInUSD(_hToken);
    uint256 underlyingPriceInUSD = getUnderlyingPriceInUSD(_hToken);
    return ((_hivemind.getCollateralFactor(_hToken) * floorPrice) * DENOMINATOR) / underlyingPriceInUSD / 10**_hToken.decimals();
  }
