...
contract Mortgages is AccessControl, ReentrancyGuard, IMortgageInterest, IERC721Receiver{
    ...
    uint256 period,address token) public returns(uint256 id){
       ...
       // console2.logString("Success Transfer Downpayment"); 
        //Distribution fees
        locals2.debt = loan + locals.fee + locals2.mintfee;
       // console2.logString("Debt");
       // console2.logUint(locals2.debt);
        vaultLenders.withdraw(locals2.debt, token);
       // console2.logString("success withdraw vault lenders");
      
        (locals.pan, locals.rewards, locals.lenders) = calcFees(locals.fee); 
     /* console2.logString("Fee Panoram");
        console2.logUint(locals.pan); 
        console2.logString("Fee Rewards");
        console2.logUint(locals.rewards);
        console2.logString("lenders Rewards");
        console2.logUint(locals.lenders); */
     
        (locals.pan2, locals.rewards2, locals.lenders2)= calcFees(locals2.mintfee); 
       /*  console2.logString("mintFee Panoram");
        console2.logUint(locals.pan2);
        console2.logString("mintFee Rewards");
        console2.logUint(locals.rewards2);
        console2.logString("MintFee lenders");
        console2.logUint(locals.lenders2); */
        ...
        IERC20(token).transfer(panoramWallet, locals2.amount); //transfer percentage fee and NFT cost
        //Create and wrap NFT
        locals2.tokenId = INFTMarket(market).mintingMortgage(locals._collection, address(this), locals.price);
        locals2.uri = ILazyNFT(locals._collection).tokenURI(locals2.tokenId);
        IwrapNFT(locals.wrapContract).wrapNFT(user, locals2.tokenId, locals2.uri);
        ILazyNFT(locals._collection).transferFrom(address(this),locals.wrapContract,locals2.tokenId);
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,user, locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt,locals.downPay,locals.price, block.timestamp,period,interest); 
        ...
        emit mortgageEvent(id,user,locals._collection, locals.wrapContract,locals2.tokenId, locals2.debt);
    }
    ...
    uint256 period,address token) public returns(uint256 id){
        ...
        IERC20(token).transfer(panoramWallet, locals2.amount); //transfer percentage fee and NFT cost
        //Create and wrap NFT
        locals2.tokenId = INFTMarket(market).mintingPresaleMortgage(collection, address(this), locals.presalePrice);
        locals2.uri = ILazyNFT(collection).tokenURI(locals2.tokenId);
        IwrapNFT(locals.wrapContract).wrapNFT(user, locals2.tokenId, locals2.uri);
        ILazyNFT(collection).transferFrom(address(this),locals.wrapContract,locals2.tokenId);
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,user, collection, locals.wrapContract,locals2.tokenId, locals2.debt,
        locals.downPay,locals.presalePrice, block.timestamp,period,interest); 
        IMortgageControl(mortgageControl).addIdInfo(id,user);
        IMortgageControl(mortgageControl).updateLastTimeCalc(user, id, block.timestamp);

        emit mortgageEvent(id,user,collection, locals.wrapContract,locals2.tokenId, locals2.debt);
    }
    ...
    uint256 period,address token) public returns(uint256 id){
     ...
     LocalMortgage2 memory locals2;
        ...
        (locals2.marketPrice,,,,,) = INFTMarket(market).getSale(collection,nftId);
        ...
        locals.downPay = locals2.marketPrice - loan;
        ...
        //Distribution fees
        locals2.debt = loan + locals.fee + locals2.buyFee;
        vaultLenders.withdraw(locals2.debt, token);
        {// scope to avoid stack too deep errors
        (locals.pan, locals.rewards, locals.lenders)= calcFees(locals.fee); 
       /*  console2.logString("Fee Panoram");
        ...
        locals2.uri = ILazyNFT(collection).tokenURI(nftId);
        IwrapNFT(locals.wrapContract).wrapNFT(user, nftId, locals2.uri);
        ILazyNFT(collection).transferFrom(address(this),locals.wrapContract,nftId);
        //Register information
        IMortgageControl(mortgageControl).addRegistry(id,user, collection, locals.wrapContract,nftId, locals2.debt,
        locals.downPay,locals2.marketPrice, block.timestamp,period,interest); 
        IMortgageControl(mortgageControl).addIdInfo(id,user);
        IMortgageControl(mortgageControl).updateLastTimeCalc(user, id, block.timestamp);

        emit mortgageEvent(id,user,collection, locals.wrapContract,nftId, locals2.debt);
    }
    ...
    uint256 period,address token) public returns(uint256 _newidMortgage){
       ...
       Localdebt memory debt;
        ...
        //Distribution fees
        vaultLenders.withdraw(locals2.money, token);
        ...
        IERC20(token).transfer(user,locals2.newLoan);  
