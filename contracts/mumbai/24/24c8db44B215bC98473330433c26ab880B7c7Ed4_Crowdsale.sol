...
pragma solidity ^0.8.17;
    ...
    contract Crowdsale {
        ...
        function buyTokens() payable public {//payable {
            require(paused == false);
            //require(msg.value > 0);
            uint256 tokens = 2; //msg.value.mul(rate);
        ...
        /*function claimVesting() payable public {
            require(vestingStart == true, "Vesting period has not started yet.");
            require(block.timestamp - claimtime >= 1 //*86400, "You can only claim once every 24 hours.");
            VestingInfo memory v = vestingInfo[msg.sender];
            uint256 daysSinceLastClaim = (block.timestamp - claimtime) / 1; //(86400);
            v.claimAmount = v.claimAmount.add(v.dailyVestedTokens.mul(daysSinceLastClaim));
            if (v.claimAmount <= v.totalVestedTokens){
                MinClaim = v.claimAmount;
                token.transfer(msg.sender, MinClaim);
                v.totalVestedTokens = v.totalVestedTokens.sub(MinClaim);
                vestingInfo[msg.sender] = v;
                claimtime = block.timestamp;
            }
        }*/
            ...
            function claimVesting() public {
                ...
                uint256 claimAmount = vestingInfo[msg.sender].claimAmount;
        ...
        function approve_claim() public returns (bool) {
            require(token.approve(msg.sender, MinClaim), "Failed to approve the transfer");
            return true;
        }
    }