...
contract Greenpeace is DefaultOperatorFilterer, ERC721A, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  address private wArtist = 0x532b88741DFb0BEa216fA614f4D415A49c06F31d;
  address private wCreator = 0x532b88741DFb0BEa216fA614f4D415A49c06F31d;
  uint256 royaltiesSpender;

  uint256 private _royaltyFundMarket = 1;  
  uint256 private _royaltyArtistSecondMarket = 0;
  uint256 private _royaltyCreatorSecondMarket = 2;

  mapping(address => uint256) private _addressLastMintedTokenId;

  bool private _isFreeMint = false;
  uint256 private _nftEtherValue = 0.001 ether;
  uint256 private _nftEtherValueWaitList = 0.001 ether;  

  mapping(uint256 => string) private cids;

  uint256 private initSaleWaitList = 1686325649;
  uint256 private endSaleWaitList = 1686325649;
  uint256 private startSalePublic = 1686325649;

  event _transferSend(address _from, address _to, uint _amount);

  constructor(    
    bytes32 _root    
  ) ERC721A("Greenpeace", "PEACE") {    
    root = _root;
    _contractUri = "";
