...
contract Eumlet is Ownable, ReentrancyGuard {
    ...
    // all supported tokens has limit > 0
    ...
    constructor(
        ...
        tokenToLimit[_tokens[0]] = _tokenLimits[0];
        tokenToLimit[_tokens[1]] = _tokenLimits[1];
        tokenToLimit[NATIVE] = _tokenLimits[2];
        limit = _limit;
    }
    ...
    function multisend(
        ...
        require(tokenToLimit[token] >= totalAmount, "Amount exceeds limit");
        if (token == NATIVE) {
    ...
    function setTokenLimit(address token, uint256 newLimit) external onlyOwner {
        require(token != address(0), "Zero token address");
        tokenToLimit[token] = newLimit;
    }
