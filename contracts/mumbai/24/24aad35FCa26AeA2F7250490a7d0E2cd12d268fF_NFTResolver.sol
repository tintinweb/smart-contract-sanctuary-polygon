       ...
       // returns(uint256 []  memory arr)
    {
       ...
       for(uint256 i=0;i<ans.length;i++){
           ...
           for(uint256 j=0 ;j<nfts.length;j++){
               uint256 expiry =  INFTWarranty(contract_add).getExpiry(ans[i],nfts[j]);
               uint256 status=  INFTWarranty(contract_add).getExpiry(ans[i],nfts[j]);
                //uint256 creationTime =  INFTWarranty(contract_add).getCreation(ans[i],nfts[i]);
            if( expiry <= block.timestamp && status==2 ){
         ...
         if( expiry > block.timestamp||status==0||status==1||status==3 ){
