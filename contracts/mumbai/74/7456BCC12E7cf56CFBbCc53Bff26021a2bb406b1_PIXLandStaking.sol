    ...
    ReentrancyGuardUpgradeable
{
    ...
    uint256 public constant LOCK_PERIOD = 10 seconds;
    ...
    mapping(address => mapping(uint256 => uint256)) public lastStakeTimestamp;
    ...
    function getStakedIDIndex(address account, uint256 tokenId) internal view returns (uint256) {
        for (uint256 i; i < stakedIDs[account].length; i += 1) {
            if (stakedIDs[account][i] == tokenId) return i; 
        }
    }

    function getStakerIndex(address account, uint256 tokenId) internal view returns (uint256) {
        for (uint256 i; i < stakers[tokenId].length; i += 1) {
            if (stakers[tokenId][i] == account) return i; 
        }
    }

    function stake(uint256 tokenId, uint256 amount) external nonReentrant {
        if (stakedAmounts[msg.sender][tokenId] == 0) { 
            stakers[tokenId].push(msg.sender);
            stakedIDs[msg.sender].push(tokenId);
        }
        
        stakedAmounts[msg.sender][tokenId] += amount;
        lastStakeTimestamp[msg.sender][tokenId] = block.timestamp;
    ...
    function unstake(uint256 tokenId, uint256 amount) external nonReentrant {
        ...
        require(
            lastStakeTimestamp[msg.sender][tokenId] + LOCK_PERIOD < block.timestamp,
            "LandStaking: LAND_LOCKED"
         );
        if (stakedAmounts[msg.sender][tokenId] == amount) {
            delete stakers[tokenId][getStakerIndex(msg.sender, tokenId)];
            delete stakedIDs[msg.sender][getStakedIDIndex(msg.sender, tokenId)];
        }
