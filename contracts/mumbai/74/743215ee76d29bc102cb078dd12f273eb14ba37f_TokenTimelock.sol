...
contract TokenTimelock is Ownable, ReentrancyGuard{     
    ...
    uint256 public coFoundersMonthlyAmount = 703125 ether;
    
    mapping(address => bool) private developers1Wallets;
    uint256 public developers1MonthlyAmount = 1875000 ether;
    
    mapping(address => bool) private developers2Wallets;
    uint256 public developers2MonthlyAmount = 750000 ether;
    
    mapping(address => bool) private developers3Wallets;
    uint256 public developers3MonthlyAmount = 150000 ether;

    event Claimed(address indexed claimerAddress, string typeOfClaimer, uint256 claimedAmount);
    ...
    function coFoundersClaim(uint monthToClaim) external nonReentrant{
        ...
        emit Claimed(msg.sender, "coFounder claim", coFoundersMonthlyAmount);
    }   
    ...
    function developers1Claim(uint monthToClaim) external nonReentrant{
        require(monthToClaim != 0, "Cannot claim month 0");
        ...
        emit Claimed(msg.sender, "developer 1 claim", developers1MonthlyAmount);
    }

    function developers2Claim(uint monthToClaim) external nonReentrant{
        require(monthToClaim != 0, "Cannot claim month 0");
        require(monthToClaim <= 12, "Cannot claim for more than 1 year");
        ...
        emit Claimed(msg.sender, "developer 2 claim", developers2MonthlyAmount);
    }

    function developers3Claim(uint monthToClaim) external nonReentrant{
        require(monthToClaim != 0, "Cannot claim month 0");
        require(monthToClaim <= 12, "Cannot claim for more than 1 year");
        ...
        emit Claimed(msg.sender, "developer 3 claim", developers3MonthlyAmount);
    }    
    
    function Approvetokens() external onlyOwner returns (bool){
   ...
   function getContractTokenBalance() public view returns (uint256){
    ...
    function checkClaimDate(uint256 month) external view returns (uint256){
        return deploymentTime + 30 days * month;
    }   
