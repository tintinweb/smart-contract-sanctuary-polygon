...
contract nftcx is Ownable {
    ...
    //每笔交易时的金卡数量,用于分红
    mapping(uint256 => uint256) public goldNumerList;
    //项目参数
    ...
    function buyNFT(address _referer) external {
        ...
        goldNumerList[idoAmount] = GoldSet.length();
        idoAmount = idoAmount.add(1);
        //不能是银卡
        AllSet.add(_user);
    ...
    function getSet(uint256 _setType, address _user) external view returns (address[] memory _addressList, uint256[] memory _uint256List, uint256 _num, string  memory _typeName) {
        ...
        if (_setType == 9) {
            ...
            _uint256List = new uint256[](idoAmount);
            for (uint256 i = 0; i < idoAmount; i++) {
                _uint256List[i] = goldNumerList[i];
            }
            _num = idoAmount;
        }
        if (_setType == 10) {
