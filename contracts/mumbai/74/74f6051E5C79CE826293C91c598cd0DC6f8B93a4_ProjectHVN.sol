...
contract ProjectHVN is ERC721Enumerable, Ownable {
    ...
    function setWhitelistMintEnabled(bool _enabled)
        external
        payable
        onlyOwner
    {
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < _addresses.length; i++) {
            ...
            for (uint256 j = 0; j < whitelistedAddresses.length; j++) {
                if (whitelistedAddresses[j] == addr) {
    ...
    function tokensInWallet(address wallet) external view returns (uint256) {
