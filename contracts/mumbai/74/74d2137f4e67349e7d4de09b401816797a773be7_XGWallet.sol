...
// File: contracts/interfaces/IXGHub.sol
...
// File: contracts/subscriptions/XGWallet.sol
...
contract XGWallet is OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 FREEZE_PERCENT_OF_MERCHANT_PAYMENT_IN_BP;
    uint256 DEPOSIT_FEE_IN_BP;
    uint256 WITHDRAW_FEE_IN_BP;
    ...
    function toggleStakeRevenueForUser(
        address _user,
        bool _stakeRevenue
    ...
    function _depositToken(
        ...
        userBalance[_user].balances[_token] = userBalance[_user]
            .balances[_token]
            .add(rest);
    }
    ...
    function depositToRestrictedTokenBalanceOfUser(
        ...
        userBalance[_user].restrictedBalances[_token] = userBalance[_user]
            .restrictedBalances[_token]
            .add(_amount);
        userBalance[_user].balances[_token] = userBalance[_user]
            .balances[_token]
            .add(_amount);
    ...
    function _withdrawToken(
        ...
        require(
            _amount <=
                userBalance[_user].balances[_token].sub(
    ...
    function _transferToken(
        ...
        require(
            tokens[_token].transfer(_receiver, _amount),
    ...
    function payWithToken(
        ...
        if (tokensLeft == 0) {
            ...
            totalCheckoutValue[_token] = totalCheckoutValue[_token].add(
                _amount
            );
            return true;
        }
    ...
    function _removeFromTokenBalance(
        address _user,
        address _token,
        uint256 _amount
    ) internal {
    ...
    function _removeMaxFromTokenBalance(
        address _user,
        address _token,
        uint256 _amount
    ...
    function _removeFromRestrictedTokenBalance(
        address _user,
        address _token,
        uint256 _amount
    ) internal {
    ...
    function _removeMaxFromRestrictedTokenBalance(
        address _user,
        address _token,
        uint256 _amount
    ) internal {
    ...
    function getUserTokenBalance(
        ...
        if (
            _token == XGT_ADDRESS &&
            userBalance[_user].merchantStakingDeposits > 0
        ) {
    ...
    function getUserRestrictedTokenBalance(
        address _user,
        address _token
    ) external view returns (uint256) {
