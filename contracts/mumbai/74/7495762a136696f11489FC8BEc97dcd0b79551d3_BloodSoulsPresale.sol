...
contract BloodSoulsPresale is Ownable, ReentrancyGuard {
    ...
    event PurchaseAmount(address indexed buyer, uint tokens);
    ...
    function purchase(uint256 tokens) external payable nonReentrant {
        unchecked {
            //this ensures we can't just buy fractional amounts of tokens and orphan some in the purchase interface.
            uint256 tokensFull = tokens * 10**uint(18);

            uint256 ticketsPurchasedCount = tokensPurchased[msg.sender];
            require(tokensFull <= remainingBalance(), "Exceeded remaining supply!");
            require((ticketsPurchasedCount + tokensFull) <= MAX_PER_ADDRESS, "Exceeded maximum purchases for wallet address!");
            // Ensure sufficient payment
            if((msg.value* getActiveRate()) < tokensFull){
                require(false, string(abi.encodePacked("Incorrect payment: ", UintToString(msg.value * getActiveRate()), " < ", UintToString(tokensFull))));
            }
            ...
            tokensPurchased[msg.sender] += tokensFull;
            souls.transfer(msg.sender, tokensFull);

            emit PurchaseAmount(msg.sender, tokens);
        }
    }
    ...
    /// @notice Allows contract owner to enable sales or presales
    ...
    function tokensRemainingToPurchase(address target) public view returns(uint256){
        return MAX_PER_ADDRESS - tokensPurchased[target];
    }

    function myTokensRemainingToPurchase() public view returns(uint256){
        return MAX_PER_ADDRESS - tokensPurchased[msg.sender];
    }
}
