...
contract SuperSpreader is Ownable {
    ...
    ISuperToken[] public faucetTokens;

    constructor(ISuperToken[] memory superTokens, ISuperfluid host) payable {
        _transferDone[address(0)] = true;
        for(uint i; i < superTokens.length; ++i) {
            require(address(superTokens[i]) != address(0), "SuperToken zero address");
            faucetTokens.push(superTokens[i]);
        }
    ...
    function getEther(address payable receiver) external {
        ...
        if(address(this).balance >= 1 ether) {
            ...
            receiver.transfer(0.1 ether);
            // send token and start streams
            uint256 i;
            uint256 length = faucetTokens.length;
            unchecked {
                for(i; i < length; ++i) {
                    ISuperToken superToken = faucetTokens[i];
                    //very optimistic mint and upgrade
                    IERC20Minter token = IERC20Minter(superToken.getUnderlyingToken());
                    token.mint(address(this), 2000 ether);
                    token.approve(address(superToken), 1000 ether);
                    superToken.upgrade(1000 ether);
                    token.transfer(receiver, 1000 ether);
                    //send around 1500/month
                    cfaV1.createFlow(receiver, superToken, 578703703703703);
                }
            }
    ...
    // Housekeeping
    function addSuperToken(ISuperToken superToken) external onlyOwner {
        require(superToken.getUnderlyingToken() != address(0), "zero address");
        if(_indexOf(address(superToken)) < 0) {
            faucetTokens.push(superToken);
        }
    }

    function removeErc20(IERC20Minter token) external onlyOwner {
    ...
    function withdraw() external onlyOwner {
        ...
        emit sendEther(_owner, balance);
    }

    function _indexOf(address superTokenAddress) view internal returns(int256 index) {
        uint256 i;
        uint256 length = faucetTokens.length;
        unchecked {
            for(i; i < length; ++i) {
                if(address(faucetTokens[i]) == superTokenAddress) {
                    // this cast can overflow
                    return int256(i);
                }
            }
        }
    ...
    function _removeErc20(int256 index) internal {
        if(index < 0) return;
        uint i = uint(index);
        uint length = faucetTokens.length - 1;
        unchecked {
            for (i; i < length; ++i){
                faucetTokens[i] = faucetTokens[i+1];
            }
        }
        faucetTokens.pop();
    }
    ...
    function deleteFlow(ISuperToken superToken, address receiver) external onlyOwner {
