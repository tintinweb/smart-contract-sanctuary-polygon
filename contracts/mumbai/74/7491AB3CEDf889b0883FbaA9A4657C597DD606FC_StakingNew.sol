...
contract StakingNew{
    ...
    UserRewards public userRewards;
    ...
    constructor (address tokenAddress_, uint256 stakingCap_){
        ...
        stakingEnds = stakingStarts.add(864000); //10 day 
        withdrawStarts = stakingEnds.add(120); // 2 minutes
        withdrawEnds = withdrawStarts.add(864000); //10 day
        ...
        rewardState.afterMaturityWithdrawRewardAPY = 60;
    ...
    */
    function _withdrawEarly(address from, uint256 amount) private _realAddress(from) returns (uint256) {
        ...
        uint256 reward = (
        ( stakingEnds.sub(block.timestamp).mul(rewardState.earlyWithdrawRewardAPY) ).mul(amount)
        ).div(denom);
        rewardState.rewardBalance = rewardState.rewardBalance.sub(reward);
        userRewards.earlyWithdrawReward = userRewards.earlyWithdrawReward.add(reward);
    ...
    * @output uint256 - amount of staked tokens withdrawn
    */
    function _withdrawAfterClose(address from, uint256 amount) public _realAddress(from) returns (uint256) {
        uint256 rewBal = rewardState.rewardBalance;
        uint256 denom = stakeState.stakedBalance;
        uint256 reward = ((amount).mul(rewardState.afterMaturityWithdrawRewardAPY)).div(denom);

        rewardState.rewardBalance = rewBal.sub(reward);
        userRewards.afterMaturityReward = (userRewards.afterMaturityReward).add(reward);
    ...
    /*
    * @notice getEarlyWithdrawReward is a function to fetch how many rewards a user owns
    * @output uint256 - amount of rewards
    */
    function checkClaimedEarlyWithdrawReward(address) public view returns (uint256) {
        return userRewards.earlyWithdrawReward;
    }
    
    function checkClaimedAfterMaturityReward(address) public view returns (uint256) {
        return userRewards.afterMaturityReward;
    }

    function checkClaimedTotalReward(address) public view returns (uint256) {
        return (userRewards.afterMaturityReward).add(userRewards.earlyWithdrawReward);
    }
    ...
    */
    function earlyWithdrawReward() public view returns (uint256) {
