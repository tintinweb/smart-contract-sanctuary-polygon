...
interface certhis_struct {
    struct CERTHIS_DEFAULT {
        ...
        uint256 max_tax_label; // max royalties for label (resale)
        uint256 max_tax_label_mint; // max royalties for label (mint)
        uint256 max_tax_collection; // max royalties collection (resale)
        uint16 default_collection_certhis_tax; //  for all collection (resale) (can be modify by certhis);
        uint16 default_collection_certhis_tax_mint; // tax mint (can be modify by certhis);
        uint256 max_royalties; //max royalties for simple collection;
        uint16 max_tax_affiliation; //max royalties for affiliate collection;
        uint16 max_tax_share_royalties; //max royalties for collection Standard / Bulk;
    ...
    struct Label {
        ...
        address affiliation; // affiliate of the label
        uint16 affiliation_tax; // royalties on certhis royalties for the affiliate for the resale
        uint16 affiliation_tax_mint; // royalties on certhis  royalties for the affiliate for the mint
    }
...
contract collection_contract is ERC721, Ownable, ERC721Enumerable {
    uint256 public MAX_SUPPLY; // max supply of the collection
    ...
    */
    ...
    // EIP-2981 royalties (return just certhis royalties informartion)
        ...
        returns (address, uint256)
    {
        ...
        certhis_struct.Collection memory collection_object = Icerthis_main(
            certhis_contract
        ).get_collection(collection_id);
        return (
            certhis_default.default_certhis_payout,
              (_salePrice * collection_object.collection_certhis_tax) / 10000 
        );
    }
        ...
        returns (certhis_struct.NFT memory)
    {
        ...
        require(
            _nft_id < index_nft ||
                (collection_object.collection_full == 2 && _nft_id < MAX_SUPPLY)
        );
    ...
    function current_minted(address _address) external view returns (uint256) {
        uint256 current_sell_id = Icerthis_main(certhis_contract)
            .get_collection(collection_id)
            .sell_id;
        return _max_mint[current_sell_id][_address];
    }
    ...
    function safeMint(
        ...
        //get parameters of the collection from certhis smart contract
        certhis_struct.Collection memory collection_object = Icerthis_main(
            certhis_contract
        ).get_collection(collection_id);
        ...
        if (collection_object.collection_full == 1) {
            ...
            _max_mint[collection_object.sell_id][msg.sender] =
                _max_mint[collection_object.sell_id][msg.sender] +
                1;
        }
        ...
        _nfts[new_nft_id].owner_address = msg.sender;
    ...
    */
    ...
    function calculate_payouts(
    ...
    ) internal view returns (certhis_struct.payment_send[] memory) {
        certhis_struct.CERTHIS_DEFAULT memory certhis_default = Icerthis_main(
            certhis_contract
        ).certhis_default();
        certhis_struct.Collection memory collection_object = Icerthis_main(
        ...
        uint16 certhis_tax = collection_object.collection_certhis_tax;
        uint16 affiliation_tax = label_object.affiliation_tax;
        ...
        certhis_struct.payment_send[]
            memory members_payout = new certhis_struct.payment_send[](8);

        members_payout[3].address_payout = label_object.label_payout;
        members_payout[0].address_payout = certhis_default
            .default_certhis_payout;
        if (type_split == 2) {
            certhis_tax = collection_object.collection_certhis_tax_mint;
            affiliation_tax = label_object.affiliation_tax_mint;
            label_tax = label_object.label_tax_mint;
            members_payout[3].address_payout = label_object.label_payout_mint;
            members_payout[0].address_payout = certhis_default
        ...
        address label_affiliation = label_object.affiliation;
        uint256 for_certhis = ( salePrice * certhis_tax ) / 10000;
        rest = rest - for_certhis;
        //certhis royaltie
        if (label_affiliation != address(0)) {
            members_payout[1].amount_to_send =
                (for_certhis * affiliation_tax ) / 10000
                ;
            for_certhis = for_certhis - members_payout[1].amount_to_send;
            members_payout[1].address_payout = label_affiliation;
        }
        ...
        if (
            ...
            members_payout[2].amount_to_send =
                ( salePrice *  collection_object.affiliation ) / 10000 ;
        
            members_payout[2].address_payout = payment_split_infos.affiliation;
        ...
        //label royalties
        if (label_tax > 0) {
            members_payout[3].amount_to_send = (salePrice * label_tax ) / 10000;
        ...
        if (type_split != 2) {
            ...
            if (
                ...
                for_collection =
                    (salePrice * collection_object.collection_tax) / 10000 ;
                rest = rest - for_collection;
            }

            if (nft_object.royalties > 0) {
                members_payout[4].amount_to_send =
                    (salePrice * nft_object.royalties ) / 10000;
                rest = rest - members_payout[4].amount_to_send;

                members_payout[4].address_payout = nft_object.creator_address;
            }

            if (type_split != 3) {
                members_payout[5].amount_to_send = rest;
                members_payout[5].address_payout = nft_object.owner_address;
            }
            members_payout[6].address_payout = collection_object
                .collection_payout;
        } else {
            members_payout[6].address_payout = collection_object
        ...
        members_payout[6].amount_to_send = for_collection;
    ...
    function send_payouts(
        ...
        for (uint256 i = 0; i <= 7; i++) {
            if (_members[i].amount_to_send > 0) {
                if (_type_sending == 1) {
                    require(IERC20(_currency).transfer(
