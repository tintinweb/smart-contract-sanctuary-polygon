...
interface IHolder {
    enum APE_TOKEN_STATUS{CLAIMABLE, CLAIMED}
    enum APE_TOKEN_TYPE{NONE, BAYC, MAYC, MAX}
    function getMintingCountPerToken(APE_TOKEN_TYPE tokenType) external view returns (uint);
    function setNFTStatusClaimed(address owner, APE_TOKEN_TYPE tokenType, uint256[] calldata tokenIds) external returns (bool);
}
