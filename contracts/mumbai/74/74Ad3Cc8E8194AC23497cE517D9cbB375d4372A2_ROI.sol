...
contract ROI {
    ...
    function _refPayout(address _addr, uint256 _amount) private {
        ...
        for (i = 0; i < BonusLinesCount; i++) {
            ...
            if (flag) {
                ...
                if(investorsMap[up].structure[0]>=15 && investorsMap[up].totalReferralInvested>=1500 ether && bonusLevels[up].level==0){
                    eagleFundUsers.push(up);
                   bonusLevels[up].level++;
                }
                if(investorsMap[up].structure[0]>=100 && investorsMap[up].totalReferralInvested>=10000 ether && bonusLevels[up].level==1){
                    bonusLevels[up].index=royaltyUsers.length;
                    royaltyUsers.push(up);
                     bonusLevels[up].level++;
                }
                if(investorsMap[up].totalReferralInvested>=100000 ether && bonusLevels[up].level==2){
    ...
    function deposit(uint256 _amount, address _upline) public payable {
        ...
        if (investorsMap[_upline].deposits.length == 0 && restartTime==0) {
            require(
    ...
    function getAvailabel(address _addr) external view returns (uint256) {
        ...
        uint256 amount = investor.totalRewards.add(payout);
