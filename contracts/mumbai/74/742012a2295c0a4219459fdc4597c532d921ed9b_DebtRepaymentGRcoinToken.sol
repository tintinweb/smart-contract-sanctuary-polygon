...
contract DebtRepaymentGRcoinToken is Ownable, ReentrancyGuard, ERC20 {
    ...
    modifier isUnlocked() {
        require(!locked, "Token: contract is currently locked");
        _;
    }
    ...
    constructor() ERC20("DebtRepaymentGRcoin", "DRGR") {
        locked = true;
    }
    ...
    function withdrawDividends() external nonReentrant isUnlocked {
    ...
    function getDividends(address account_) public view returns (uint256) {
        uint256 holderBalance_ = balanceOf(account_);
        uint256 amount_ = 0;
        if (holderBalance_ != 0){
            amount_ = ( (dividendPerToken - xDividendPerToken[account_]) * holderBalance_ / MULTIPLIER);
            amount_ += credit[account_];
            }
        return amount_;
    }
