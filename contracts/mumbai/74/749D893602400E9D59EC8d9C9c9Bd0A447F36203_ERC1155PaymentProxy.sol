...
interface IERC1155SelfMinter {
    ...
    function tiers(uint256 tierNo) view external returns (Tier memory);
    function saleId() view external returns (uint256);
    function supplyPerId(uint256 saleId) view external returns (Supply memory);
    function mintedPerAddress(address wallet) view external returns (uint256);
}
...
contract ERC1155PaymentProxy is ERC1155, ReentrancyGuard, DefaultOperatorFilterer, Ownable {
    ...
    IERC1155SelfMinter private originalContract;
    ...
    function initialize(
        ...
        originalContract = IERC1155SelfMinter(originalContractAddress);
    ...
    function _mintValidation(
        ...
        IERC1155SelfMinter.Tier memory tier = originalContract.tiers(tierNo);

        if(tier.start > block.timestamp) revert TierPeriodHasntStarted();
        if(tier.end < block.timestamp) revert TierPeriodHasEnded();

        if (tier.isPublic == false && !MerkleProof.verify(_merkleProof, tier.merkleRoot, keccak256(abi.encodePacked(mintAddress, tierNo)))) {
            revert InvalidProof();
        }

        uint256 saleId = originalContract.saleId();

        if (originalContract.supplyPerId(saleId).total + amount > originalContract.supplyPerId(saleId).max) {
            revert TotalSupplyGreaterThanMaxSupply();
        }

        if (originalContract.mintedPerAddress(mintAddress) + amount > tier.limitPerWalletPerTier) {
            revert MintLimitReached();
        }

        require(
            tokenContract.balanceOf(mintAddress) >= minimumUserBalance,
