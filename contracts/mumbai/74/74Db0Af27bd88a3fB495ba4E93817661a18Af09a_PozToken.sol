...
contract PozToken is ERC20, Ownable {
    ...
    event SwapFeeUpdated(uint256 swapFeeRate);
    ...
    function setSwapFeeRate(uint24 _swapFeeRate) external onlyOwner {
        require(
            _swapFeeRate <= DENOMINATOR,
            "POZTOKEN: swap fee rate can not be greater than 100%"
        );
        swapFeeRate = _swapFeeRate;

        emit SwapFeeUpdated(swapFeeRate);
    }
    ...
    function setPoolAddress(address _poolAddress) external onlyOwner {
        require(_poolAddress != address(0), "POZTOKEN: pool address cannot be zero");
        poolAddress = _poolAddress;

        emit PoolAddressUpdated(poolAddress);
    }

    function setBackingReserveAddress(address _backingReserveAddress) external onlyOwner {
        require(_backingReserveAddress != address(0), "POZTOKEN: backing reserve address cannot be zero");
    ...
    function setStableTokenAddress(address _stableTokenAddress) external onlyOwner {
        require(_stableTokenAddress != address(0), "POZTOKEN: stable token address cannot be zero");
        stableTokenAddress = _stableTokenAddress;

        emit StableTokenAddressUpdated(stableTokenAddress);
    }

    function setLockedAddress(address _lockedAddress, bool _status) external onlyOwner {
        require(_lockedAddress != address(0), "POZTOKEN: locked address cannot be zero");
        bool flag = false;
        for (uint i = 0; i < lockedAddress.length; i++) {
    ...
    function _transfer(
        ...
        uint256 transferFee = sender == poolAddress || recipient == poolAddress
        ...
        if (transferFee > 0 && poolAddress != address(0)) {
            ...
            //// transfer POZ token to treasury address for swapping with stable token
            super._transfer(sender, treasury, transferFee);

            //// get the pool price
            uint256 poolPrice = _getPrice(address(this), stableTokenAddress);
            //// get the stable token amount for POZ fee token
            uint256 stableAmount = poolPrice * transferFee;

            //// transfer stable token from treasury to Backing Reserve.
            uint256 currentBal = IERC20Upgradeable(stableTokenAddress).balanceOf(treasury);
            require(currentBal >= stableAmount, "POZTOKEN: not enough stable token amount in treasury");

            //// check allowance of stable token
            require(
                IERC20Upgradeable(stableTokenAddress).allowance(treasury, address(this)) >= stableAmount,
                "must be approved stable token from treasury to this address"
            );

            IERC20Upgradeable(stableTokenAddress).safeTransferFrom(treasury, backingReserveAddress, stableAmount);
            ...
            uint256 stableTokenTotalAmount = IERC20Upgradeable(stableTokenAddress).balanceOf(poolAddress).add(IERC20Upgradeable(stableTokenAddress).balanceOf(backingReserveAddress));

            uint256 expectedPozTokenAmountInPool = circulatingSupply.mul(IERC20Upgradeable(stableTokenAddress).balanceOf(poolAddress)).div(stableTokenTotalAmount);
            
            if (expectedPozTokenAmountInPool >= balanceOf(poolAddress)) {
                uint256 transferTokenAmount = expectedPozTokenAmountInPool.sub(balanceOf(poolAddress));
                //// transfer POZ token from treasury to pool address
                super._transfer(treasury, poolAddress, transferTokenAmount);
            } else {
                uint256 transferTokenAmount = balanceOf(poolAddress).sub(expectedPozTokenAmountInPool);
    ...
    // get price of POZ token from the pool
    function _getPrice(address _tokenIn, address _tokenOut)
        internal
        view
        returns (uint256 price)
    {
        IUniswapV3Pool pool = IUniswapV3Pool(factory.getPool(_tokenIn, _tokenOut, swapFeeRate));
