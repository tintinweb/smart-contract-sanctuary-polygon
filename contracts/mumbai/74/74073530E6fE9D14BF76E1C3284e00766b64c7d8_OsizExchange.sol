...
// File: contracts/lib/IwMATIC.sol
...
contract OsizExchange is OrderState, TransferSafe {
    ...
    function transferSellFee(
        ...
        } else if (order.key.buyAsset.token == wMATIC) {
            transferwMATICFee(
                order.buying,
                order.key.owner,
                buyer,
    ...
    function transferBuyFee(
        ...
        if (order.key.sellAsset.token == wMATIC) {
            transferwMATICFee(
                order.selling,
                buyer,
                order.key.owner,
    ...
    function transferwMATICFee(
        ...
        if ((secoundaryFee > 0) && (royaltyReceipt != address(0x00))) {
            require(
                IwMATIC(wMATIC).transferFrom(buyer, royaltyReceipt, secoundaryFee),
                "Failed royalty fee transfer"
            );
        }
        if (remaining > 0) {
            require(
                IwMATIC(wMATIC).transferFrom(buyer, _seller, remaining),
                "Failed transfer"
            );
        }
    }
