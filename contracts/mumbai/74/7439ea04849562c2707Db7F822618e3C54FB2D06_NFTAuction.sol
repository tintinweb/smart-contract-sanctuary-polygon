...
interface NFTToken{
    function mint(address _owner, uint256 newTokenId,uint256 _tier,string memory _tokenURI) external;
...
//contract NFTMarketplace is ERC1155 {
    ...
    address payable Unikfame = payable(0x394Ab6172dE75359cdB145E231Efb23a4f17e258); // 100%
    ...
    function setNftAdr(address _allwNftAdr ) public {
        require(msg.sender == owner,"You can't call this function");
        NftAddr = _allwNftAdr;
    }

    /* Mints a token and lists it in the marketplace */
    function createToken(string memory creator, string memory tokenURI, uint256 price, uint _flag, uint tier, uint num, address royalty, uint _time) public onlyOwner returns (uint) {
        ...
        require(tokenURITier[tokenURI] == tier,"URL provided have been assign to another tier");
        ...
        {
          newTokenId = NftTkn.getCurrentTknId(); //_tokenIds.current();
          NftTkn.mint(msg.sender, newTokenId,tier,_tokenURI);
          
          NftTkn.setTokenURI(newTokenId, tokenURI);          
          createMarketItem(tokenURINums[tokenURI] + i,newTokenId, price, flag, royalty, _tokenURI, _time);
    ...
    function createAuctionSale(
      uint256 [] calldata tokenIds
        ...
        require(msg.value > 0, "Bidding price should be higher than Zero");
        ...
        for(uint i =0; i < tokenIds.length; i ++)
        {
          ...
          if(pricefrom > price){
            if(idToMarketItem[tokenId].isAuction == true){
              if(idToMarketItem[tokenId].timestamp >= block.timestamp){
                _seller = lastBider;
                _price = price;
                
                //payable(address(this)).transfer(pricefrom);
                ...
                emit MarketTrx(tokenId,address(0),msg.sender,price,
                                        "New Bid",
                                        block.timestamp
                          );
              }
    ...
    function endAuctionBatch() public payable whenNotPaused{
        NFTToken NftTkn = NFTToken(NftAddr);
        uint itemCount = NftTkn.getCurrentTknId();//_tokenIds.current();      
        address _seller;
        for (uint i = 1; i <= itemCount; i++) {                        
            ...
            else        // case more than 1 bid`
            {
                ...
                NftTkn.transfer(address(this), lastBider, tokenId);
            }     
            ...
            emit MarketTrx(tokenId,seller,lastBider,price,
                                        "Auction ended (batch)",
                                        block.timestamp
                          );
        }        
    ...
    function endAuction(
        ...
        for(uint i =0; i < tokenIds.length; i ++)
        {
          ...
          if(idToMarketItem[tokenId].isAuction){
            if(seller == msg.sender){
              ...
              emit MarketTrx(tokenId,msg.sender,lastBider,price,
                                            "Auction ended",
                                            block.timestamp
                              );
            }
    ...
    function fetchOwnableItems(address _to) public view returns(uint256){
    ...
    function fetchAuctionItems() public view returns (MarketItem[] memory) {
        NFTToken NftTkn = NFTToken(NftAddr);
        uint totalItemCount = NftTkn.getCurrentTknId(); //_tokenIds.current();
        uint itemCount = 0;
        uint currentIndex = 0;
