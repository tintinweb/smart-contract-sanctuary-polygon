...
contract PizzaV2 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
   ...
   event Increment(address something, uint256 test);  
   ...
   function addUserOwnedTokens(address addr, string memory val) public{
        userOwnedTokens[addr] = val;
        emit Increment(msg.sender, 1);
   }
    
   function additionalFunc() external {
    require(slices > 1, "no slices left");
    slices +=2;
   }

   function addToWhitelist(address addr) external{
        require(!whitelist[addr],'already added to whitelist');
        whitelist[addr]=true;
   }
}