...
contract ICO is ReentrancyGuard, AccessControl{
    ...
    function sendMATIC(address walletAddress) external payable onlyRole(ADMIN_ROLE) {
        require(walletAddress != address(0), "Null address");
        uint256 balance = address(this).balance;
        payable(walletAddress).transfer(balance);
    }

    function sendToken(address _tokenAddress,address walletAddress, uint256 amount)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(walletAddress != address(0), "Null address");
