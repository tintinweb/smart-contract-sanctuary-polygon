    ...
    // ERC2771ContextUpgradeable(0x86C80a8aa58e0A4fa09A69624c31Ab2a6CAD56b8) - injected in code directly
{
    ...
    EnumerableSetUpgradeable.AddressSet private supportedTokens2;
   ...
   function initialize(
    ...
    function deposit(address _token, uint256 _amount) external {
        ...
        if (supportedTokens2.contains(_token) == false) {
    ...
    function withdrawTo(
        ...
        if (supportedTokens2.contains(_targetToken) == false) {
 ...
 /// @notice Wraps and creates flow 
    ...
    function getListSupportedTokens() public view returns (address[] memory) {
        return supportedTokens2.values();
    }
    ...
    {
        if (_status) {
            supportedTokens2.add(_token);
        } else {
            supportedTokens2.remove(_token);
        }
...
interface ISuperfluidEndResolver {
    function addToChecker(address _sender, address _receiver, uint256 timestamp) external;
    function removeFromChecker(address _sender, address _receiver) external;
}
