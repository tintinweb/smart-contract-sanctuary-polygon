...
*/
contract PettoTeamTokenDistribution  is Ownable, ReentrancyGuard {
    ...
    */
    ...
    uint256 private constant _DISTRIBUTION_PERIOD = 1 days; //One day
    
    uint256 private constant _DISTRIBUTION_VALUE = 15000 * (10**_DECIMALS);

    uint256 private constant _MAX_TOKEN_SUPPLY_LIMIT = 105000000 * (10**_DECIMALS);
    ...
    */
    constructor(address _tokenAddress){
        ...
        teamMembers = [
            ...
            0x6aCcb7A03B0cfD1526Cc1fe5c0C5C379fD2fBAcb, //Cung
    ...
    */
    function getTeamMembers() external onlyOwner view returns (address[] memory){
        return teamMembers;
    }
    ...
    * @return bool
    */
    function addTeamMember(address _member) external onlyOwner nonReentrant returns (bool){
    ...
    * @return bool
    */
    function removeTeamMember(address _member) external onlyOwner nonReentrant returns (bool){
        for(uint256 i = 0; i < teamMembers.length; i++){
            if(teamMembers[i] == _member){
    ...
    */
    function distributeToken() external nonReentrant returns (bool){
        ...
        uint256 tokenToSend = numberOfMember*_DISTRIBUTION_VALUE;
        if(tokenToSend > pot){
            revert("Pot exhausted");
        }
        for (uint256 i = 0; i<numberOfMember; i++){
            if(!pettoToken.transfer(teamMembers[i], _DISTRIBUTION_VALUE)){
