...
/// @notice Events 
    ...
    event OrderCreate(string _function, uint256 _order, address _sender, uint256 _tokenquantity, address _tokencontract, uint256 _maticamount, uint256 _time);
    event RedeemOrder(string _function, uint256 _order, address _sender, uint256 _tokenquantity, address _tokencontract, uint256 _maticamount, uint256 _time);
    event Cancellisting(string _function, uint256 _order, address _sender, uint256 _tokenquantity, address _tokencontract, uint256 _time);
...
/// @notice Creating Selling listings
    function createOrder(address _tokenContract, uint256 _tokenQuantity, uint256 _maticAmount) public {
            ...
            emit OrderCreate("Selling Listing", orderNumber.current(), msg.sender, _tokenQuantity, _tokenContract, _maticAmount, block.timestamp);
    }
...
/// @notice Purchase orders of listing
    function redeemOrder(uint256 _orderNumber) public payable {
            ...
            emit RedeemOrder("Purchase", _orderNumber, msg.sender, purchaseQuantity, Orders[_orderNumber].tokenContract , msg.value, block.timestamp);
...
/// @notice For cancelling Selling Listing
    function cancelOrder(uint256 _orderNumber) public {
            ...
            emit Cancellisting("Cancelling Listing", _orderNumber, msg.sender, Orders[_orderNumber].tokenQuantity, Orders[_orderNumber].tokenContract, block.timestamp);
