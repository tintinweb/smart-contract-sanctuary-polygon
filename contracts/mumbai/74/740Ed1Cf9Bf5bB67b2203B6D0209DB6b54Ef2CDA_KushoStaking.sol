...
contract KushoStaking is Ownable, ReentrancyGuard {
    ...
    function claimRewards() external {
        ...
        require(block.timestamp >= stakers[msg.sender].timeOfLastClaim * 1 days, "Wait 24 hours to claim again.");
        if (paused) {
            stakers[msg.sender].timeOfLastUpdate = timeOfLastPause;
    ...
    function canClaim(address _staker) public view returns (bool) {
        if (block.timestamp >= stakers[_staker].timeOfLastClaim * 1 days) {
            return true;
        } else {
            return false;
        }
