...
contract PremiumChallenge is ReentrancyGuard {
    ...
    constructor(
        ...
        metaData = MetaData(
            _challengeName,
            _gameName,
            string.concat(_baseUrl, Strings.toHexString(uint256(uint160(address(this))), 20)),
            _target,
            _startTime,
            _endTime,
            _entryFee
    ...
    modifier challengeStateIsValidForSettingWinners {
        ChallengeState _challengeState = forecastChallengeState();
        require(_challengeState == ChallengeState.COMPLETED || _challengeState == ChallengeState.RUNNING, 'Invalid challenge state for setting winners');
        _;
    }

    modifier challengeStateIsValidForCancel {
        ChallengeState _challengeState = forecastChallengeState();
        require(_challengeState == ChallengeState.DRAFT || _challengeState == ChallengeState.RUNNING, 'Invalid challenge state for cancel');
        _;
    }

    modifier challengeIsCancelled {
        require(isCancelled, 'Challenge is not cancelled');
        _;
    }

    modifier challengeIsNotCancelled {
        require(!isCancelled, 'Challenge is cancelled');
        _;
    }
    ...
    function cancel() public senderIsAddress onlyOwner challengeIsNotCancelled challengeStateIsValidForCancel {
        require(participantWallets.length == 0, "Partcipnants already joined");
        isCancelled = true;
    }

    // join to challenge
    function join() external nonReentrant senderIsAddress challengeStateIsValidForJoin participantNotJoined {
        if (metaData.entryFee > 0) {
    ...
    function joinByOwner(address _wallet) public senderIsAddress onlyOwner challengeStateIsValidForJoin {
        require(_wallet != address(0), 'Invalid arg wallet address');
        require(metaData.entryFee == 0, 'Not free to enter challenge');
        require(participants[_wallet] == address(0), 'Participant already joined');
        participants[_wallet] = _wallet;
        participantWallets.push(_wallet);
        emit ParticipantJoined(_wallet);
    }
    ...
    function setWinners(address[] memory _winnersWallets, uint256 _qualifiedParticipants) public senderIsAddress onlyOwner challengeStateIsValidForSettingWinners challengeWinnersIsNotSet {
        // validate wallets
    ...
    // get wallets
    function getParticipantWallets() public senderIsAddress view returns (address[] memory) {
