...
// File: stakingVikas/update_stake.sol
...
contract InterestEarner {
    ...
    uint256 _OneWeakTime = 300;
    uint256 _TwoWeakTime = 600;
    uint256 _ThreeWeakTime = 900;
    uint256 _FourWeakTime = 1200;
    ...
    uint256 public finaltotalExpectedInterest;

    // Token amount variables
    mapping(address => uint256) public balances;
    uint256 public totalStateStaked;

    mapping(address => uint256) public holdersStakingCount;

    //Staking Struct
    struct StakingSchedule{
        bool initialized;
        // owner of staked tokens after they are released
        address  beneficiary;
        // start time of the staking period
    ...
    // Stake new changes 
    bytes32[] private StakingSchedulesIds;
    mapping(bytes32 => StakingSchedule) private stakingSchedules;
    ...
    event intrestAmount(string data);
    event intrestAmountInFewDays(uint data);
    event hundredPercentTime(uint data);
    event calculatedTime(uint data);
    event logStakingId(bytes32 data);
    event logStaking(bool initialized, address  beneficiary, uint256  initialStakeTimestamp, uint256 totalExpectedInterest, uint256  balances);
    event intrestValue(uint256 iValue);
    event initialTime(uint256 iTime);
    event StakingCount(uint256 count);
    event iEPAp(uint256 interestEarnedPerAnnum_pre, uint256 amount, uint256 percentageBasisPoints);
    event iEPApo(uint256 interestEarnedPerAnnum_post, uint256 interestEarnedPerAnnum_pre);
    event iEPAWPS(uint256 weiPerSecond);
    event sRT(uint256 releaseEpoch, uint256 timePeriod, uint256 initialStakingTimestamp, uint256 timestamp, uint256 secondsRemaining, uint256 interestEarnedForThisStake);
    ...
    */
    modifier onlyIfStakingScheduleNotRevoked(bytes32 stakeScheduleId) {
        require(stakingSchedules[stakeScheduleId].initialized == true);
        require(stakingSchedules[stakeScheduleId].revoked == false);
        _;
    }
    ...
    modifier percentageNotSet() {
        require(
            percentageSet == false,
            "The percentageBasisPoints has already been set."
        );
        _;
    }
    ...
    modifier percentageIsSet() {
        require(
            percentageSet == true,
            "Please set the percentageBasisPoints variable first, then try again."
        );
        _;
    }
    ...
    function setTimePeriod(uint256 _timePeriodInSeconds)
        public
        onlyOwner
        timePeriodNotSet
    {
    ...
    function setPercentage(uint256 _percentageBasisPoints)
        public
        onlyOwner
        percentageNotSet
    {
        require(
            _percentageBasisPoints >= 1 && _percentageBasisPoints <= 10000,
            "Percentage must be a value >=1 and <= 10000"
        );
        percentageSet = true;
        ...
        noReentrant
    {
        ...
        require(
            ...
            "Not enough ERC20 tokens in your wallet, please try lesser amount"
        );
        ...
        require(
            amount > 315360000000,
            "Amount to stake must be greater than 0.00000031536 ETH"
        );
        // Similarly, in terms of maximum allowable value, we need the staked amount to be less than 2**256 - 1 / 10, 000 (to avoid overflow)
        require(
            amount < MAX_INT.div(10000),
            "Maximum amount must be smaller, please try again"
        );
        ...
        emit iEPAp(interestEarnedPerAnnum_pre, amount, percentageBasisPoints);
        ...
        uint256 interestEarnedPerAnnum_post = interestEarnedPerAnnum_pre.div(
            10000
        );
        emit iEPApo(interestEarnedPerAnnum_post, interestEarnedPerAnnum_pre);
        
        // Let's calculate how many wei are earned per second
        uint256 weiPerSecond = interestEarnedPerAnnum_post.div(31536000);
        emit iEPAWPS(weiPerSecond);
        require(
            weiPerSecond > 0,
            "Interest on this amount is too low to calculate, please try a greater amount"
        );
        // Let's calculate the release date
        uint256 releaseEpoch = initialStakingTimestamp[msg.sender].add(
            timePeriod
        );
        ...
        require(
            releaseEpoch.sub(block.timestamp) > 0,
            "There is not enough time left to stake for this current round, please un-stake first"
        );
        // uint256 secondsRemaining = releaseEpoch.sub(block.timestamp);
        uint256 secondsRemaining = yearSeconds;
        ...
        emit sRT(releaseEpoch, timePeriod, initialStakingTimestamp[msg.sender], block.timestamp, secondsRemaining, interestEarnedForThisStake);
        ...
        require(
            token.balanceOf(address(this)) >=
                totalStateStaked.add(totalExpectedInterest).add(
                    interestEarnedForThisStake
                ),
            "Not enough STATE tokens in the reserve pool, please contact owner of this contract"
        );

        bytes32 stakingScheduleId = this.computeNextStakingScheduleIdForHolder(msg.sender);

        // Adding this user's expected interest to the expected interest variable
        totalExpectedInterest = totalExpectedInterest.add(
            interestEarnedForThisStake
        );
        ...
        expectedInterest[msg.sender] = expectedInterest[msg.sender].add(
            interestEarnedForThisStake
        );
        
        uint256 currentStakingCount = holdersStakingCount[msg.sender];
        holdersStakingCount[msg.sender] = currentStakingCount.add(1);
        StakingSchedulesIds.push(stakingScheduleId);
        finaltotalExpectedInterest = totalExpectedInterest.div(holdersStakingCount[msg.sender]);
        stakingSchedules[stakingScheduleId] = StakingSchedule(
            true,
            msg.sender,
            block.timestamp,
            finaltotalExpectedInterest,
            amount,
            false
        );
        emit StakingCount(holdersStakingCount[msg.sender]);
        emit logStakingId(stakingScheduleId);
        emit logStaking(true, msg.sender, block.timestamp, finaltotalExpectedInterest, amount);
    ...
    function Unstake(IERC20 token, bytes32 stakeScheduleId)
        public
        timePeriodIsSet
        percentageIsSet
        noReentrant
        onlyIfStakingScheduleNotRevoked(stakeScheduleId){
        StakingSchedule storage stakingSchedule = stakingSchedules[stakeScheduleId];
        stakingSchedule.revoked = true;
        // Ensure that there is a current round of interest at play
        require(
            stakingSchedules[stakeScheduleId].revoked != false,
            "No tokens staked at present"
        );
        ...
        uint256 amountToUnstake = stakingSchedules[stakeScheduleId].balances;

        // Decrease the total STATE staked
        totalStateStaked = totalStateStaked.sub(amountToUnstake);
        uint initialTimeStamp = stakingSchedules[stakeScheduleId].initialStakeTimestamp;
        emit initialTime(initialTimeStamp);
        
        uint256 calcTime = block.timestamp.sub(initialTimeStamp);
        emit calculatedTime(calcTime);
        emit calculatedTime(block.timestamp);
        
        uint256 interestToPayOut = 0;
        emit intrestAmount("here");
        emit intrestAmountInFewDays(interestToPayOut);

        // if (calcTime < 604800) {
        //   interestToPayOut = 0;
        // } else if (calcTime > 604800 && calcTime < 2629743) {
        //   interestToPayOut = expectedInterest[msg.sender].div(2);
        // } else if (calcTime > 2629743) {
        //   interestToPayOut = expectedInterest[msg.sender];
        // }
        uint256 unstakeReward_pre = stakingSchedules[stakeScheduleId].totalExpectedInterest.div(yearSeconds);
        emit intrestValue(unstakeReward_pre);
        uint256 unstakeReward = unstakeReward_pre.mul(block.timestamp.sub(stakingSchedules[stakeScheduleId].initialStakeTimestamp));
        emit intrestValue(unstakeReward);
        
        if (calcTime < _OneWeakTime) {
          interestToPayOut = 0;
          emit intrestAmount("inside if");
          emit intrestValue(interestToPayOut);
        } else if (calcTime > _OneWeakTime && calcTime < _TwoWeakTime) {
          interestToPayOut = unstakeReward.div(2);
          emit intrestAmount("inside else if 2");
          emit intrestValue(interestToPayOut);
        }
        
        else if (calcTime > _TwoWeakTime && calcTime < _ThreeWeakTime){
           uint256 interestToPayOut_pre = unstakeReward.mul(3);
           interestToPayOut = interestToPayOut_pre.div(4);
           emit intrestAmount("inside else if 3");
           emit intrestValue(interestToPayOut);
        }
        
        //  else if (calcTime > _FourWeakTime) {
        else {
          interestToPayOut = unstakeReward;
          emit intrestAmount("inside else if 4");
          emit intrestValue(interestToPayOut);
        }
        
        // require(calcTime <= 604800 , "less than one week");
        //     uint256 interestToPayOut = 0;
        //   require(calcTime >= 604800 && calcTime <= 2629743,"more than one week and less than three month");
        // //    uint256 interestToPayOut = expectedInterest[msg.sender];
        //    require(calcTime >= 2629743 && calcTime <= 31556926,"more than one week and less than three month");
        //    uint256 interestToPayOut = expectedInterest[msg.sender];
        ...
        require(
            interestToPayOut.add(amountToUnstake) <=
                token.balanceOf(address(this)),
            "Not enough STATE tokens in the reserve pool to pay out the interest earned, please contact owner of this contract"
        );
        ...
        emit intrestAmount("Reduce the value which represents interest owed to the msg.sender");
        emit intrestValue(interestToPayOut);
        expectedInterest[msg.sender] = expectedInterest[msg.sender].sub(
            interestToPayOut
        );
        ...
        // Emit the event log
        stakingSchedules[stakeScheduleId].revoked = true;

        emit TokensUnstaked(msg.sender, amountToUnstake);
        emit InterestWithdrawn(msg.sender, interestToPayOut);
    }

    function earnedReward(bytes32 stakeScheduleId, address holder) public 
    view
    returns(uint256){
        // StakingSchedule storage stakingSchedule = stakingSchedules[stakeScheduleId];
        require(
            holder == stakingSchedules[stakeScheduleId].beneficiary, "Sender is not staking holder wrong wallet address"
        );
        uint initialTimeStamp = stakingSchedules[stakeScheduleId].initialStakeTimestamp;
        
        uint256 calcTime = block.timestamp.sub(initialTimeStamp);
        
        uint256 interestToPayOut;
        uint256 unstakeReward_pre = stakingSchedules[stakeScheduleId].totalExpectedInterest.div(yearSeconds);
        uint256 unstakeReward = unstakeReward_pre.mul(block.timestamp.sub(stakingSchedules[stakeScheduleId].initialStakeTimestamp));
    ...
    function reinvestAlreadyStakedTokensAndInterestEarned(IERC20 token)
        public
        timePeriodIsSet
        percentageIsSet
        noReentrant
    {
        ...
        require(
            block.timestamp >
                (initialStakingTimestamp[msg.sender].add(timePeriod)),
            "Locking time period is still active, please try again later"
        );
        // Ensure the official ERC20 contract is being referenced
        require(
            token == erc20Contract,
        ...
        require(
            expectedInterest[msg.sender].add(balances[msg.sender]) <=
                token.balanceOf(address(this)),
            "Not enough STATE tokens in the reserve pool to pay out the interest earned, please contact owner of this contract"
        );
        uint256 newAmountToInvest = expectedInterest[msg.sender].add(
            balances[msg.sender]
        );
        require(
            newAmountToInvest > 315360000000,
            "Amount to stake must be greater than 0.00000031536 ETH"
        );
        require(
            newAmountToInvest < MAX_INT.div(10000),
            "Maximum amount must be smaller, please try again"
        );
        ...
        totalExpectedInterest = totalExpectedInterest.sub(
            expectedInterest[msg.sender]
        );
        ...
        initialStakingTimestamp[msg.sender] = block.timestamp;
        // Let's calculate the maximum amount which can be earned per annum (start with mul calculation first so we avoid values lower than one)
        uint256 interestEarnedPerAnnum_pre = newAmountToInvest.mul(
        ...
        require(
            secondsRemaining > 0,
            "There is not enough time left to stake for this current round"
        );
        // There are 31536000 seconds per annum, so let's calculate the interest for this remaining time period
        uint256 interestEarnedForThisStake = weiPerSecond.mul(secondsRemaining);
        // Make sure that contract's reserve pool has enough to service this transaction. I.e. there is enough STATE in this contract to pay this user's interest (not including/counting any previous end user's staked STATE or interest which they will eventually take as a pay out)
        require(
            ...
            "Not enough STATE tokens in the reserve pool, to facilitate this restake, please contact owner of this contract"
        );
    ...
    function transferTokensOutOfReservePool(IERC20 token, uint256 amount)
        public
        onlyOwner
        noReentrant
    {
        ...
        require(
            token == erc20Contract,
            "Token address must be ERC20 address which was passed into the constructor"
        );
        ...
        require(
            amount <=
                token.balanceOf(address(this)).sub(
                    (totalExpectedInterest.add(totalStateStaked))
                ),
            "Can only remove tokens which are spare i.e. not put aside for end user pay out"
        );
    ...
    /**
    * @dev Computes the next Staking schedule identifier for a given holder address.
    */
    function computeNextStakingScheduleIdForHolder(address holder)
        public
        view
        returns(bytes32){
        return computeStakingScheduleIdForAddressAndIndex(holder, holdersStakingCount[holder]);
    }

    /**
    * @dev Computes the staking schedule identifier for an address and an index.
    */
    function computeStakingScheduleIdForAddressAndIndex(address holder, uint256 index)
        public
        pure
        returns(bytes32){
