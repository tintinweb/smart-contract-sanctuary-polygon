...
import {IZKBadgeMinter} from "./interfaces/IZKBadgeMinter.sol";
import "sismo-connect-solidity/SismoLib.sol";

/**
 * @title ZK Badge Minter contract
 * @author Sismo
 */
contract ZKBadgeMinter is IZKBadgeMinter, Initializable, AccessControl, ERC1155, SismoConnect {
  ...
  // ZK Badge Config for each appId
  struct ZKBadgeConfig {
    // chosen when registering the config
    SismoConnectConfig config;
    string uri;
    AuthRequest[] authRequests;
    ClaimRequest[] claimRequests;
    address owner;
    // not chosen when registering the config
    uint256 tokenId;
  }

  // Each appId is mapped to a ZK Badge configuration. The config can be registered one time only.
  ...
  mapping(bytes16 => ZKBadgeConfig) private _zkBadgeConfigs;

  // mapping to quickly know if a vaultId has minted a ZK Badge for a specific appId
  mapping(bytes16 => mapping(uint256 => bool)) private _isVaultIdUsedForAppId;
  // mapping to keep track of the config for a specific tokenId
  mapping(uint256 => bytes16) private _tokenIdToAppId;

  uint256 private _tokenIdCounter;

  error ZKBadgeConfigAlreadyRegistered(bytes16 appId);
  error NotZKBadgeConfigOwner(bytes16 appId, address configOwner, address attacker);
  error VaultIdAlreadyUsedForAppId(bytes16 appId, uint256 vaultId);
  ...
  ) ERC1155(uri) SismoConnect(buildConfig(bytes16(0), false)) {
  ...
  function claimWithSismoConnect(bytes memory responseBytes, address to, uint256 amount) external {
    SismoConnectResponse memory response = abi.decode(responseBytes, (SismoConnectResponse));
    bytes16 appId = response.appId;

    ZKBadgeConfig memory zkBadgeConfig = _zkBadgeConfigs[appId];
    AuthRequest[] memory _authRequests = zkBadgeConfig.authRequests;
    ClaimRequest[] memory _claimRequests = zkBadgeConfig.claimRequests;

    SismoConnectRequest memory request = buildRequest({
      auths: _authRequests,
      claims: _claimRequests,
      signature: buildSignature({message: abi.encode(to, amount)})
    });

    SismoConnectVerifiedResult memory result = _sismoConnectVerifier.verify({
      response: response,
      request: request,
      config: _zkBadgeConfigs[appId].config
    });

    // check if vaultId is already used for this appId
    uint256 vaultId = result.getUserId(AuthType.VAULT);
    if (_isVaultIdUsedForAppId[appId][vaultId]) {
      revert VaultIdAlreadyUsedForAppId(appId, vaultId);
    }

    // update state to keep track of the vaultId
    _isVaultIdUsedForAppId[appId][vaultId] = true;

    // allow to mint the ZK Badge
    // we use the appId to get the tokenId
    _mint(to, _getTokenId(appId), amount, "");
  }

  function registerZKBadgeConfig(
    bytes16 appId,
    ...
    if (_zkBadgeConfigs[appId].owner != address(0)) {
      revert ZKBadgeConfigAlreadyRegistered(appId);
    }
    _setZKBadge(appId, isImpersonationMode, baseTokenURI, authRequests, claimRequests, owner);
  }

  function updateZKBadgeConfig(
    bytes16 appId,
    bool isImpersonationMode,
    string memory baseTokenURI,
    AuthRequest[] memory authRequests,
    ...
    address configOwner = _zkBadgeConfigs[appId].owner;
    if (configOwner != msg.sender) {
      revert NotZKBadgeConfigOwner(appId, configOwner, msg.sender);
    }
    _setZKBadge(appId, isImpersonationMode, baseTokenURI, authRequests, claimRequests, owner);
  }

  function getTokenId(bytes16 appId) external view returns (uint256) {
    return _getTokenId(appId);
  }

  function getZKBadgeConfig(bytes16 appId) external view returns (ZKBadgeConfig memory) {
    return _zkBadgeConfigs[appId];
  }

  function isVaultIdUsedForAppId(bytes16 appId, uint256 vaultId) external view returns (bool) {
    return _isVaultIdUsedForAppId[appId][vaultId];
  }

  function uri(uint256 tokenId) public view override(ERC1155) returns (string memory) {
    if (_isTokenIdFromLegacy(tokenId)) {
      return ERC1155.uri(tokenId);
    }
    bytes16 appId = _tokenIdToAppId[tokenId];
    return _zkBadgeConfigs[appId].uri;
  }

  function _setZKBadge(
    bytes16 appId,
    bool isImpersonationMode,
    string memory baseTokenURI,
    AuthRequest[] memory authRequests,
    ...
    _zkBadgeConfigs[appId].config.appId = appId;
    _zkBadgeConfigs[appId].config.vault.isImpersonationMode = isImpersonationMode;
    _zkBadgeConfigs[appId].uri = baseTokenURI;
    _setAuths(appId, authRequests);
    _setClaims(appId, claimRequests);
    _zkBadgeConfigs[appId].owner = owner;

    _zkBadgeConfigs[appId].tokenId = _getTokenId(appId) == 0
      ? _setTokenId(appId)
      : _getTokenId(appId);
  }

  function _setAuths(bytes16 appId, AuthRequest[] memory auths_) private {
    AuthRequest[] storage _authRequests = _zkBadgeConfigs[appId].authRequests;
    for (uint256 i = 0; i < auths_.length; i++) {
      _authRequests.push(auths_[i]);
    }
  }

  function _setClaims(bytes16 appId, ClaimRequest[] memory claims_) private {
    ClaimRequest[] storage _claimRequests = _zkBadgeConfigs[appId].claimRequests;
    for (uint256 i = 0; i < claims_.length; i++) {
      _claimRequests.push(claims_[i]);
    }
  }

  function _getTokenId(bytes16 appId) private view returns (uint256) {
    return _zkBadgeConfigs[appId].tokenId;
  }

  function _setTokenId(bytes16 appId) private returns (uint256) {
    // the tokenId counter starts at 1
    _tokenIdCounter = _tokenIdCounter == 0 ? 1 : _tokenIdCounter;
    uint256 tokenId = 20_000_000 + _tokenIdCounter;
    _tokenIdToAppId[tokenId] = appId;
    _tokenIdCounter++;
    return tokenId;
  }

  function _isTokenIdFromLegacy(uint256 tokenId) private pure returns (bool) {
    // legacy tokenIds for front contract are between 0 and 10_000_000
    // legacy tokenIds for hydraS1Accountbound contract are between 10_000_001 and 20_000_000
    // legacy tokenIds for pythia1Simple contract are between 30_000_001 and 30_000_100
    return tokenId <= 20_000_000 || (30_000_001 <= tokenId && tokenId <= 30_000_100);
  }

  /**
   * For all legacy tokenIds, the balance of a user is equal to the value of the underlying attestation (attestationCollectionId == badgeId)
   * For ZK Badges V2 tokenIds, the balance is equal to the amount of token owned
  ...
  function balanceOf(
    address account,
    uint256 id
  ) public view virtual override(ERC1155, IZKBadgeMinter) returns (uint256) {
    return
      _isTokenIdFromLegacy(id)
        ? _attestationsRegistry.getAttestationValue(id, account)
        : ERC1155.balanceOf(account, id);
  }

  /**
   * @dev Reverts, this is a non transferable ERC115 contract
   * @notice The transfer is allowed if the tokenId is not from legacy contract
   * it represent ZK Badges V2 tokenIds claimable by SismoConnect
   */
...
/**
 * @title Interface for ZKBadgeMinter contract
 * @author Sismo
 */
interface IZKBadgeMinter {
