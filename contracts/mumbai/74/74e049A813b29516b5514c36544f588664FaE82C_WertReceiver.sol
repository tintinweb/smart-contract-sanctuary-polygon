...
/** 
 *  SourceUnit: /Users/iggyi/Git/culture-vault/cv-api/contracts/WertReceiver.sol
*/
...
interface MarketInterface {
    ...
    function setOrder(Order memory order) external;

    function orderMatch(bytes memory data) external;

    function setOrderMatch(Order memory sellOrder, bytes memory sellerSignature, Order memory buyOrder, bytes memory buyerSignature, uint256 amount) external; 
}

contract WertReceiver is EIP712, ReentrancyGuard {
    address public wertAddress; 
    address public adminAddress;
    address public marketAddress;
    address public marketInterfaceAddress; 
    ...
    // uint256 orderNum; 
    // mapping (bytes => Order) sellOrder;
    // mapping (bytes => Order) buyOrder;
    ...
    constructor(address _wertAddress, address _marketAddress) EIP712("Culture Vault", "1.0.0") {
        wertAddress = _wertAddress;
        marketAddress = _marketAddress;
        adminAddress = 0x338fe4DEe63031e61ac90CC21cC6604f2c306b23;
    }
    ...
    modifier onlyAdmin() {
        require(msg.sender == adminAddress, "msg call is not from wert's contract.");
        _;
    }
    ...
    // // payable function that execute Market's OrderMatch() and receive NFT on behalf of buyer
    // function wertIncoming(Market _market,
        // Order memory sellOrder,
        // bytes memory sellerSignature, 
        // Order memory buyOrder, 
        // bytes memory buyerSignature, 
        // uint256 amount
    // ) public payable onlyWert nonReentrant {
    //     // _sellOrder storage Order(sellOrder.tokenAddress, sellOrder.tokenId, sellOrder.maker, sellOrder.currency, sellOrder.price, sellOrder.amount, sellOrder.listingTime, sellOrder.expirationTime, sellOrder.salt, sellOrder.side);
    //     // bytes memory data = abi.encodeWithSignature(_market.orderMatch(sellOrder, sellerSignature, buyOrder, buyerSignature, amount), sin);
    //     // bytes32 _sellOrder = keccak256(abi.encode(sellOrder));
    //     // _sellOrder = MarketInterface(marketInterfaceAddress).setOrder(sellOrder);
    //     // _buyOrder = MarketInterface(marketInterfaceAddress).setOrder(buyOrder);
    //     abi.decode()
    //     _market.orderMatch(MarketInterface(marketAddress).setOrder(sellOrder), sellerSignature, MarketInterface(marketInterfaceAddress).setOrder(buyOrder), buyerSignature, amount); 
    //     // MarketInterface(marketAddress).setOrderMatch(sellOrder, sellerSignature, buyOrder, buyerSignature, amount);
    //     nftCollection(buyOrder.tokenAddress, buyOrder.tokenId, amount, buyOrder.maker);
    //     emit IncomingRequest(wertAddress, buyOrder.maker, amount, buyOrder.tokenAddress, buyOrder.tokenId);
    //     // bool successful = address(marketAddress).call(data);
    //     // bool successful = true;
    //     // return successful; 
    // }

    function wertIncoming(bytes calldata inputData) public payable onlyWert nonReentrant {
        // bytes memory sellOrder;
        // bytes memory sellerSignature; 
        // bytes memory buyOrder;
        // bytes memory buyerSignature;
        // uint256 amount;
        // address tokenAddress;
        // uint256 tokenId;
        ...
        // uint256 salt;
        // uint256 side;
        // (sellOrder, sellerSignature, buyOrder, buyerSignature, amount) = abi.decode(inputData, (bytes, bytes, bytes, bytes, uint256));
        // (tokenAddress, tokenId, maker, currency, price, _amount, listingTime, expirationTime, salt, side) = abi.decode(sellOrder, (address, uint256, address, address, uint256, uint256, uint256, uint256, uint256, uint256));
        MarketInterface(marketAddress).orderMatch(inputData);
        // Market.orderMatch(inputData);
        nftCollection(0xcBc9bA209Ff656F302daD8Ef0D3d5081450F7abf, 327, 1, 0xf1aB283Cee61e95Ddd58722aBE195ec8a34D0C5d);
    }

    // match NFT against buyer details and send NFT back to owner
    function nftCollection(address tokenAddress, uint256 tokenId, uint256 amount, address nftOwner) internal {
        // Add OpenZeppelin IERC721Receiver and IERC1155Receiver to ensure safeTransfer can be called 
        // Step 1: NFT Token Received 
        // Step 2: Verify NFT owner wallet address is correct
        require(nftOwner != address(0));
        require(nftOwner != adminAddress );
        require(nftOwner != wertAddress);
        require(nftOwner != marketAddress);
        // Step 3: Set approval from this middleware contract to transfer NFT (no need if NFT is now own by this contract)
        // Step 4: Safe transfer NFT to owner wallet
        uint256 tokenStandard = getTokenStandard(tokenAddress);
        require(tokenStandard != 0, "Not support.");
        if (tokenStandard == 721) {
            IERC721(tokenAddress).safeTransferFrom(address(this), nftOwner, tokenId);
        } else if (tokenStandard == 1155) {
            IERC1155(tokenAddress).safeTransferFrom(
                address(this),
                nftOwner,
    ...
    // only CV admin can update marketplace address
    function updateMarketplaceAddress(address newAddress) onlyAdmin public {
        marketAddress = newAddress;
    } 
    // only CV admin can update admin wallet address
    function updateAdminAddress(address newAddress) onlyAdmin public {
        adminAddress = newAddress;
    } 

    // only CV admin can update wert address
    function updateWertAddress(address newAddress) onlyAdmin public {
        wertAddress = newAddress;
    }

    // allow public to check the incoming nft details - view only method 
    function viewMarketplaceAddress() public view returns (address) {
        return marketAddress; 
    }

    function viewAdminAddress() public view returns (address) {
        return adminAddress; 
    }

    function viewWertAddress() public view returns (address) {
        return wertAddress; 
    }
}