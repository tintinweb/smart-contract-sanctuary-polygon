...
contract LongShort is ILongShort, AccessControlledAndUpgradeable {
  ...
  mapping(uint32 => MarketTieredLeverage) separatePoolMarketContracts;
  ...
  //TODO remove this function potentially when separate pools are ready
  ...
  function registerPoolMarketContract(
    ...
    separatePoolMarketContracts[marketIndex] = marketContract;
    marketExists[marketIndex] = true;
...
contract MarketTieredLeverage is AccessControlledAndUpgradeable {
  ...
  */
  ...
  function _updateSystemStateInternal() internal virtual {
    // TOOD: the below code should only make 1 call to the oracle manager
    (uint32 currentEpochTimestamp, uint32 numberOfEpochsSinceLastEpoch) = oracleManager
    ...
    if (epochInfo.latestExecutedEpochIndex == epochInfo.epochIndex) {
      // this means the current epoch already has it's price
      return;
    }

    // TODO possibly (may be worse) optimize by saving as state variable in epochInfo
    int256 previousPrice = oracleManager.priceAtIndex(epochInfo.latestOraclePriceIdentifier);

    OracleManagerFixedEpoch.MissedEpochExecutionInfo[] memory epochsToExecute = oracleManager
...
contract OracleManagerFixedEpoch {
  ...
  constructor(address _chainlinkOracle) {
    ...
    initialEpochStartTimestamp = block.timestamp - EPOCH_LENGTH();
  }

  /// @dev This is the amount of time after the epoch that we need to wait before a price update becomes valid
  function MINIMUM_EXECUTION_WAIT_THRESHOLD() public pure virtual returns (uint256) {
    return 10;
  }

  /// @dev This is the fixed length of the epoch.
  function EPOCH_LENGTH() public pure virtual returns (uint256) {
    return 3600;
  }
