...
contract JubidouNFT is ERC721RoyaltyUpgradeable,OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public cost ;
    uint256 private nbInWhiteList ;

    bool public paused ;
    bool public whitelistMintEnabled ;
    bool public revealed ;
    
    uint256 public maxSupplyPrivate ;
    uint256 public maxSupply ; //5661 total soit 4561 public 
    ...
    function initialize() external initializer {
        __Ownable_init();
         __ERC721_init("Jubidou NFT collection", "JUBIDOUNFT");
          hiddenMetadataUri = "https://gateway.pinata.cloud/ipfs/QmdtaNNdYsou72F6T9Rg8aeFSfMEVDenWcyEPbPwfoT4oh/hiddenBabyDolls.json";
        ...
        cost = 10 ether;
         ...
         revealed=false;
         maxSupply=5661; //10 for test
         maxSupplyPrivate=1000; //5 for test
         nbInWhiteList= 0 ;
    }
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        if (whitelistMintEnabled) {
            require(
                totalSupply.current() + _mintAmount <= maxSupplyPrivate,
                "Max supply whitelist exceeded!"
            );
        } else {
 ...
 function getNbPrelist() public view onlyOwner returns (uint256) {
        return address(this).balance;
    }
