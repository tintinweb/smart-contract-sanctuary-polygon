...
contract DomainVerify is Ownable {
    ...
    function removeDomain(string memory _rootDomain) public {
        require(
            msg.sender == owner() || domain[_rootDomain].owner == msg.sender,
            "You don't have the privilages to remove this domain!"
        );
        domain[_rootDomain].owner = address(0);
        
        domain[_rootDomain].websiteName = "";
        domain[_rootDomain].phoneNumber = "";
        domain[_rootDomain].emailAddress = "";
        domain[_rootDomain].webCategory = "";
        domain[_rootDomain].ownerName = "";

        domain[_rootDomain].verifiedTime = 0;
        domain[_rootDomain].validTill = 0;

        emit DomainRemoved(msg.sender, _rootDomain);
    }
    ...
    event DomainRemoved(address remover, string rootDomain);
