...
contract SustainImpact is AccessControl {
    ...
    mapping(address => mapping(uint => mapping(uint => ProvisionalSchedule))) public provisionalSchedule;
    ...
    address public isgdDadari = 0xdb85529DFFC6E49Afa08D68eb7b4eCa722385C3D;
    address public isgsBadaripur = 0x3367A16E0cc56BC7d902d2aFf56bF52Ea23adbB3;
    address public sldcDelhi = 0x7d1FD7A9d41420e67C9416675cA17e651860681B;
    address public sldcup = 0xA520dC40fE68B8D89b0B9A5D37C08385FEFEfb76;
    address public rldc = 0xEBbf1e841B9462eb3276816EADd881beeBDd82c8;
    ...
    event CapacitySheetUploaded(address _userAddress, bool success);
    event UploadedRequisitionBySLDC(string _timeCapacity, uint _requisitionEntry);
    event RequistionUploaded(address _userAddress, bool success);
    event EntitlementUploaded(address _userAddress, bool success);
    event ProvisionalScheduleUploaded(address _userAddress, bool success);
    event CapacitySheetRevisionUploaded(address _userAddress, bool success);
    event RequistionRevisionUploaded(address _userAddress, bool success);
    event EntitlementRevisionUploaded(address _userAddress, bool success);
    event ProvisionalScheduleRevisionUploaded(address _userAddress, bool success);
    ...
    function uploadEntitlementInBulk(address[] memory _isgsAddress, address[] memory _sldcAddress, uint[] memory _timeBlock, uint[] memory _entitledPower) public{
        ...
        emit EntitlementUploaded(msg.sender, true);
    }

    function uploadEntitlementRevision(address _isgsAddress, address _sldcAddress, uint _timeBlock, uint _entitledPower) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        entitlmentData[msg.sender][_timeBlock][block.timestamp] = Entitlement(
            msg.sender,
            _isgsAddress,
    ...
    function uploadEntitlementInBulkRevision(address[] memory _isgsAddress, address[] memory _sldcAddress, uint[] memory _timeBlock, uint[] memory _entitledPower) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        ...
        require(_isgsAddress.length == _entitledPower.length, "Parameters missing");
        for(uint i = 0; i < _isgsAddress.length; i++){
            uploadEntitlementRevision(_isgsAddress[i], _sldcAddress[i], _timeBlock[i], _entitledPower[i]);
        }
        emit EntitlementRevisionUploaded(msg.sender, true);
    }

    function viewEntitlement(address _RLDCAddress, uint _timeBlock, uint _timestamp) public view returns(address, address, uint, uint, bool) {
        ...
        return (
           ...
           entitlmentData[_RLDCAddress][_timeBlock][_timestamp].isRevision
        );
    }

    function uploadProvisionalSchedule(address _isgsAddress, address _sldcAddress, uint _timeBlock, uint _energy) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        provisionalSchedule[msg.sender][_timeBlock][block.timestamp] = ProvisionalSchedule(
            msg.sender,
            _isgsAddress,
            _sldcAddress,
            _timeBlock,
    ...
    function uploadProvisionalScheduleInBulk(address[] memory _isgsAddress, address[] memory _sldcAddress, uint[] memory _timeBlock, uint[] memory _energy) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");

        require(_isgsAddress.length == _sldcAddress.length, "Parameters missing");
        require(_isgsAddress.length == _timeBlock.length, "Parameters missing");
        require(_isgsAddress.length == _energy.length, "Parameters missing");
        for(uint i = 0; i < _isgsAddress.length; i++){
            uploadProvisionalSchedule(_isgsAddress[i], _sldcAddress[i], _timeBlock[i], _energy[i]);
        }
        emit ProvisionalScheduleUploaded(msg.sender, true);
    }

    function uploadProvisionalScheduleRevision(address _isgsAddress, address _sldcAddress, uint _timeBlock, uint _energy) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        provisionalSchedule[msg.sender][_timeBlock][block.timestamp] = ProvisionalSchedule(
            msg.sender,
            _isgsAddress,
    ...
    function uploadProvisionalScheduleInBulkRevision(address[] memory _isgsAddress, address[] memory _sldcAddress, uint[] memory _timeBlock, uint[] memory _energy) public{
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        ...
        require(_isgsAddress.length == _energy.length, "Parameters missing");
        for(uint i = 0; i < _isgsAddress.length; i++){
            uploadProvisionalScheduleRevision(_isgsAddress[i], _sldcAddress[i], _timeBlock[i], _energy[i]);
        }
        emit ProvisionalScheduleRevisionUploaded(msg.sender, true);
    }

    function viewProvisionalSchedule(address _RLDCAddress, uint _timeBlock, uint _timestamp) public view returns(address, address, uint, uint, bool) {
        
        require(hasRole(RLDC_ROLE, msg.sender), "Not Authorized");
        
        return (
           provisionalSchedule[_RLDCAddress][_timeBlock][_timestamp].isgsAddress,
           provisionalSchedule[_RLDCAddress][_timeBlock][_timestamp].sldcAddress,
           provisionalSchedule[_RLDCAddress][_timeBlock][_timestamp].timeBlock,
           provisionalSchedule[_RLDCAddress][_timeBlock][_timestamp].energy,
           provisionalSchedule[_RLDCAddress][_timeBlock][_timestamp].isRevision
        );
    }
    ...
    function uploadCapacitySheetISGSInBulk(
        ...
        emit CapacitySheetUploaded(msg.sender, true);
    }

    function uploadCapacitySheetISGSRevision
    (
        string memory _timeCapacity, 
        uint _power, 
        uint _techMin, 
    ...
    function uploadCapacitySheetISGSInBulkRevision(
        string[] memory _timeCapacity, 
        uint[] memory _power, 
        uint[] memory _techMin, 
        uint[] memory _rampUp, 
        ...
        for(uint i = 0; i < _timeCapacity.length; i++){
            uploadCapacitySheetISGSRevision(_timeCapacity[i], _power[i], _techMin[i], _rampUp[i], _rampDown[i], _onBarInstCap[i]);
        }
        emit CapacitySheetRevisionUploaded(msg.sender, true);
    }
    ...
    function viewCapacitySheetRevision(address _ISGSAddress, uint _timeBlock, uint _timestamp) public view returns(uint, uint, uint, uint, uint, bool) {
        ...
        return (
           ...
           declaredCapacityData[_ISGSAddress][_timeBlock][_timestamp].rampDown,
           declaredCapacityData[_ISGSAddress][_timeBlock][_timestamp].onBarInstCap,
           declaredCapacityData[_ISGSAddress][_timeBlock][_timestamp].isRevision
        );
    }
    ...
    function uploadRequisitionSLDC(string memory _timeCapacity, uint _requisitionEntry) public {
        ...
        requisitionData[msg.sender][requisitionTimeBlock][block.timestamp] = Requisition(_timeCapacity, _requisitionEntry, false, msg.sender);
    ...
    function uploadRequisitionSLDCInBulk(string[] memory _timeCapacity, uint[] memory _requisitionEntry) public {
        ...
        emit RequistionUploaded(msg.sender, true);
    }

    function uploadRequisitionSLDCRevision(string memory _timeCapacity, uint _requisitionEntry) public {
        require(hasRole(SLDC_ROLE, msg.sender), "Not Authorized");
        requisitionData[msg.sender][requisitionTimeBlock][block.timestamp] = Requisition(_timeCapacity, _requisitionEntry, true, msg.sender);
        sldcData.push(SLDCData(msg.sender, requisitionTimeBlock, block.timestamp));
        requisitionTimeBlock++;
        emit UploadedRequisitionBySLDC(_timeCapacity, _requisitionEntry);
    }

    function uploadRequisitionSLDCInBulkRevision(string[] memory _timeCapacity, uint[] memory _requisitionEntry) public {
        require(hasRole(SLDC_ROLE, msg.sender), "Not Authorized");

        require(_timeCapacity.length == _requisitionEntry.length, "Parameter Missing");

        for(uint i = 0; i < _timeCapacity.length; i++){
            uploadRequisitionSLDCRevision(_timeCapacity[i], _requisitionEntry[i]);
        }
        emit RequistionRevisionUploaded(msg.sender, true);
    }

    // SLDC and RLDC views uploaded requisition
    function viewRequisition(address _SLDCAddress, uint _requisitionTimeBlock, uint _timestamp) public view returns(string memory, uint, bool) {
        ...
        return (
            ...
            requisitionData[_SLDCAddress][_requisitionTimeBlock][_timestamp].isRevision
        );
    }

    function getSldcData(address user) public view returns(string[] memory _timeDescription, uint[] memory _requisitionEntry, bool[] memory _isRevision){
        _timeDescription = new string[](sldcData.length);
        _requisitionEntry = new uint[](sldcData.length);
        _isRevision = new bool[](sldcData.length);
        uint j;
        for(uint i = 0; i < sldcData.length; i++){
            ...
            if(sldcData[i].SLDCAddress == user){
                (TimeDescription, RequisitionEntry, isRevision) = viewRequisition(sldcData[i].SLDCAddress, sldcData[i].timeBlock, sldcData[i].timestamp);
                _timeDescription[j] = TimeDescription;
                _requisitionEntry[j] = RequisitionEntry;
                _isRevision[j] = isRevision;
                j++;
            }
        }
        return (_timeDescription, _requisitionEntry, _isRevision);
    }
