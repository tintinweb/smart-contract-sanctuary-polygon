...
contract Boxes is Ownable, VRFConsumerBaseV2 {
    ...
    function claimBoxes(uint256 contestId, uint256[] memory boxIds) external payable {
        ...
        require(contest.boxesCanBeClaimed == false, "Boxes cannot be claimed");
     ...
     */
    function _assignScores (uint256 _contestId) internal {
        // fetch the contest
        Contest memory contest = contests[_contestId];
        // do not allow for another chainlink request if one has already been fulfilled
        require(contest.rewardsCanBeClaimed == false, "Box values already assigned");
    ...
    function fulfillRandomWords(
        ...
        // this flag allows for boxes to be claimed
