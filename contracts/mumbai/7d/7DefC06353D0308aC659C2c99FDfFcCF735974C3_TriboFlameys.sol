...
// File: contracts/TriboFlameys.sol
...
contract TriboFlameys is ERC721, Ownable, Pausable, ERC2981 {
    using Strings for uint256;
     ...
     */
    constructor(uint256 _supplyCap, address _allowlistSignerPublicKey) ERC721("Tribo Flameys", "FLAMEYS") {
    ...
    function mintStatus(address minter, Signature calldata _signature)
        public
        view
        whenNotPaused
        returns (MintingStatus)
    {
        ...
        if (!_verifySignature(minter, _signature)) {
            return MintingStatus.InvalidSignature;
        }
        if (addressMinted[minter]) {
    ...
    /**
     * @dev Mint function intended to claim a batch of Flameys by the owner of the contract
     */
