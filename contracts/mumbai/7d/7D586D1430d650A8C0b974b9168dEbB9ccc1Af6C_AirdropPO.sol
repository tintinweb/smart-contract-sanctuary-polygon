...
contract AirdropPO is Ownable, Pausable {
    address public TokenPOAddr;
    ...
    event NewAmountAirdrop(uint256 amount);
    ...
    constructor(
        ...
        TokenPOAddr = TokenPO_;
        START_TIME = start_time_;
        airdropAmount = amount_;
    }
    ...
    function changeAmount(uint256 amount_) public onlyOwner {
        airdropAmount = amount_;
        emit NewAmountAirdrop(amount_);
    }

    function canClaim(address _user, uint256 _to) public view returns (bool) {
        if (_to < START_TIME) return false;
        uint256 userClaimTo = lastClaim[_user] > START_TIME
            ? (lastClaim[_user] - START_TIME) / SECONDS_IN_DAY + 1
            : 0;
    ...
    function claimToken() public whenNotPaused {
        require(canClaim(msg.sender, block.timestamp), "Can not claim");

        TokenPO(TokenPOAddr).mint(msg.sender, airdropAmount);
