...
contract VIBESStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address vibesContract, uint256 initialRate) {
    ...
    function stake(uint256 amount) public nonReentrant returns (bool){
        require(_token.balanceOf(msg.sender) >= amount * 10 ** 18, "Not enough balance!");

        _stake(amount * 10 ** 18);
        _token.safeTransferFrom(msg.sender, address(this), amount * 10 ** 18);
        return true;
    }
    ...
    function unstakeSelected(uint256 index) public nonReentrant returns (bool){
        ...
        _token.safeTransfer(msg.sender, amount_to_mint);
        return true;
    }
    ...
    function calculateStakeReward(Stake memory _current_stake) internal view returns(uint256){
        uint256 periodCount = (block.timestamp.sub(_current_stake.since)) / 1 minutes;
