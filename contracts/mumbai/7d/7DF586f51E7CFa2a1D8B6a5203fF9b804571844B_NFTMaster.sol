...
contract NFTMaster is INFTMaster, NFT101Experience {
    IMaster masterContract;
    PriceConverter private priceConverter;

    address private marketPlaceAddress;
    address private address101ExperienceContract;

    uint256 public series; // 0 for pre-order, 1 for first series, 2 for second series, 42 for specials, etc...

    mapping(bytes32 => uint256) private _mastersRafflingCount;
    mapping(uint256 => INFTMaster.NFTMaster) private _nftMasters;

    constructor(RroseForwarder forwarder) ERC2771Context(address(forwarder)) ERC721('101Experience Master', '101MASTER') {
        // _setDefaultRoyalty(payable(address(this)), 1000); // TODO verify this? Set by default to the contract address to collect fees and redispatch to artist off-chain - Default to 10% commission fees
    }

    modifier onlyValidContract(address contractAddress) {
        if (contractAddress == address(0)) {
            revert NotAValidContractAddress(contractAddress, _msgSender(), owner());
        }
        if (!((_msgSender() == contractAddress) || (_msgSender() == owner())))
            revert NotAValidContractAddress(contractAddress, _msgSender(), owner());
        _;
    }

    function setSeries(uint256 _series) external onlyValidContract(address101ExperienceContract) {
        series = _series;
    }

    function setMasterContract(address _addressMasterContract) external onlyOwner {
        masterContract = IMaster(_addressMasterContract);
    }

    function set101ExperienceContract(address _address101ExperienceContract) external onlyOwner {
        address101ExperienceContract = _address101ExperienceContract;
    }

    function setPriceConverterContract(address _addressPriceConverter) external onlyOwner {
    ...
    function isListableOnMarketPlace(uint256 _tokenId) public view override returns (bool) {
        // NFTMaster is only listable on MarketPlace if the template Master maxSupply is reached (not available to buy on first market anymore)
        return masterContract.get(_nftMasters[_tokenId].master.key).supplyReached;
    }

    /*
    function setStatus(Status newStatus, uint256 tokenId) external override validTokenId(tokenId) {
        if(!(_isContract(_msgSender()) && _msgSender() == marketPlaceAddress)) revert NotAValidContractAddress(marketPlaceAddress, _msgSender(), owner());
        _nftMasters[tokenId].status = Status(newStatus);
    }*/

    /* function getPrice(uint256 tokenId) public view returns (uint256) {
        return _nftMasters[tokenId].master.price;
    }
*/
    function getPrice(bytes32 key) public view returns (uint256) {
        uint256 price = masterContract.get(key).price;
        return priceConverter.getMaticInWeiFromEurCents(price);
    }

    function getMasterTemplate(bytes32 key) public view returns (IMaster.Master memory master) {
        return masterContract.get(key);
    }

    function getNFTMasterCurrentSupply(bytes32 key) public view returns (uint256 supply) {
        return _mastersRafflingCount[key];
    }

    function _isNFTMasterMaxSupplyReached(bytes32 key) private view returns (bool) {
        return masterContract.get(key).maxSupply <= _mastersRafflingCount[key];
    }

    function _canBuyMaster(bytes32 key) private returns (bool) {
        IMaster.Master memory m = getMasterTemplate(key);
        if (block.timestamp <= m.startSaleDate) revert SaleNotOpenYet();
        if (m.endSaleDate <= block.timestamp) revert SaleEnded();
        if (_isNFTMasterMaxSupplyReached(key)) revert MaxSupplyReached(address(this), key);
        if (m.price > 0) {
            if (msg.value < priceConverter.getMaticInWeiFromEurCents(m.price)) revert NeedMoreMoney();
        }
        return true;
    }

    function safeMint(address to, bytes32 key) external payable onlyValidContract(address101ExperienceContract) returns (uint256) {
        require(_canBuyMaster(key));

        // 1 get the configuration of Master
        IMaster.Master memory m = getMasterTemplate(key);

        // Update the raffling count for this Master
        _mastersRafflingCount[key]++;

        // 2 minting the Master
        uint256 collectionId = _mastersRafflingCount[key];
        uint256 tokenId = uint256(keccak256(abi.encodePacked(key, '_', collectionId)));

        _safeMint(to, tokenId);

        if (_isNFTMasterMaxSupplyReached(key)) {
            masterContract.setMaxSupplyReached(key);
        }

        // 3 linking the detail struct of masterpiece with the tokenId that has been minted
        _nftMasters[tokenId] = INFTMaster.NFTMaster({ collectionId: collectionId, master: m, series: series });

        // We set the royalty fees for this token according to EIP2981 - https://eips.ethereum.org/EIPS/eip-2981
        // For the secondary Market
        _setTokenRoyalty(tokenId, m.royaltyReceiver2, uint96(m.feesBasisPoints));

        // We transfert 100% of the paid value to the RoyaltyReceiver contract fees splitter for the primary Market
        payable(m.royaltyReceiver1).transfer(msg.value);

        emit NFTMasterMinted(to, tokenId, collectionId, key);

        return tokenId;
    }

    function getToken(uint256 _tokenId) public view returns (INFTMaster.NFTMaster memory) {
        return _nftMasters[_tokenId];
    }

    /*    function getFloorPriceForToken(uint256 _tokenId) external view override returns (uint256) {
        return masterContract.get(_nftMasters[_tokenId].master.key).floorPrice;
    }*/

    function getFeesBasisPointsForToken(uint256 _tokenId) external view override returns (uint256) {
        return masterContract.get(_nftMasters[_tokenId].master.key).feesBasisPoints;
    }

    function tokenURI(uint256 tokenId) public view override validTokenId(tokenId) returns (string memory) {
        return _nftMasters[tokenId].master.uri;
    }
    ...
    // TODO: this method is blocking every transfers when the max supply is not reached (event between two wallets of the same owner)
    // Users can't transfer their NFTMaster if the Masters are always available to buy on the 101Experience website
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId,
        uint256 batchSize
    ) internal override(NFT101Experience) whenNotPaused {
        // We authorize the transfer from 0x0 address (for minting) and transfer of the token to the new owner
        /*if (from != address(0)) {
            // if (!isListableOnMarketPlace(tokenId)) revert TransferBlocked(tokenId);
            require(isListableOnMarketPlace(tokenId), 'TransferBlocked');
        }*/
        /*// We block all transfers where the floor price is not reached
        uint256 floorPriceInEurCents = _nftMasters[tokenId].master.floorPrice;
        if (floorPriceInEurCents > 1) {
            uint256 floorPriceInMaticWei = priceConverter.getMaticInWeiFromEurCents(floorPriceInEurCents);
            //if (msg.value < floorPriceInMaticWei) revert TransferBlocked(tokenId);
            require(msg.value >= floorPriceInMaticWei, 'TransferBlocked');
        }*/
        super._beforeTokenTransfer(from, to, tokenId, batchSize);
    }
}