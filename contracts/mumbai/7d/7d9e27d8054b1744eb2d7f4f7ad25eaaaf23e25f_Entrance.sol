    ...
    GnosisSafeUpgradeable
{
    ...
    IERC20UpgradeableExt public immutable token;
    ...
    event Pay(address indexed account, uint indexed amount, uint indexed nonce);
    event ClaimReward(address indexed account, uint indexed amount, uint indexed nonce);

    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address _token) {
        _disableInitializers();

        token = IERC20UpgradeableExt(_token);
    }
    ...
    function pay(uint _amount, bytes calldata _signature) external {
        address account = _msgSender();
        uint nonce = nonces[account];
        nonces[account] = nonce + 1;
        bytes memory data = abi.encodePacked(keccak256(abi.encodePacked(account, _amount, nonce)));
        require(isValidSignature(admin, data, _signature), "Invalid signature");

        token.safeTransferFrom(account, vault, _amount);
        emit Pay(account, _amount, nonce);
    }

    function claimReward(uint _amount, bytes calldata _signature) external {
        address account = _msgSender();
        uint nonce = nonces[account];
        nonces[account] = nonce + 1;
        bytes memory data = abi.encodePacked(keccak256(abi.encodePacked(account, _amount, nonce)));
        ...
        emit ClaimReward(account, _amount, nonce);
    }
