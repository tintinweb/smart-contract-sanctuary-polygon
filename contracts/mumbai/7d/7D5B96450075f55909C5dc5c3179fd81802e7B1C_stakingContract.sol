...
contract stakingContract {
    ...
    constructor (IERC20 _token) {
        token = _token;
        stakingDetails[0] = Stake(100, 3, 3);
        stakingDetails[1] = Stake(150, 5, 5);
        stakingDetails[2] = Stake(200, 7, 7);
        stakingDetails[3] = Stake(250, 10, 10);
        owner = msg.sender;  
    }
    ...
    function stake(uint256 amount, uint256 level) external returns(bool) {
        ...
        users[msg.sender][level].endTime = block.timestamp + stakingDetails[level].stakeLimit * 1 minutes;        
