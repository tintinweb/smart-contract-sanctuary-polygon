...
contract AAVEMoneyMarketModule is WithStorage {
    ...
    function wrapAndRepayAll(uint256 interestRateMode) external payable returns(uint256 repaid) {
        ...
        _weth.deposit{value: sentAmount}();
        // returns the actual amount repaid
        repaid = IPool(aas().v3Pool).repay(_nativeWrapper, type(uint256).max, interestRateMode, msg.sender);
        // refund excess eth
        payable(msg.sender).transfer(sentAmount - repaid);
    }
