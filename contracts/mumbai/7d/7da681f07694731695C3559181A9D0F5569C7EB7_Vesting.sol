...
contract Vesting {
    ...
    function releaseTokens(address benificiary,uint256 vestingID) internal returns(uint256) {
    ...
    function calculateVestedAmount(address benificiary,uint256 vestingID) internal returns(uint256) {
        ...
        if(schedule._releasedTokens>=schedule._totalTokens){
            return 0;
        }
    ...
    function withdraw(address benificiary,uint256 withdrawAmount,uint256 vestingID ) public {
        releaseTokens(benificiary,vestingID);
        require(vestingSchedules[benificiary][vestingID]._cliff<= block.timestamp ,"Cliff is not over yet");
        ...
        require(withdrawAmount<=withdrawableAmount[benificiary][vestingID],"withdraw limit exceed");
