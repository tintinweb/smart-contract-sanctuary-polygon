...
contract NFT_ERC_721 is NFToken, Authorizable {
  ...
  function getEncodedSvg() private view returns (string memory) {
    return Base64.encode(bytes(svg));
  }

  function buildTokenURI() internal view returns (string memory) {
      string memory json = Base64.encode(
          bytes(string(
              abi.encodePacked(
                  "{"
                     "\"name\": \"Pussy\","
                     "\"image\": \"data:image/svg+xml;base64,", getEncodedSvg(), "\""
                  "}"
              )
          ))
      );
  ...
  function mint(address _to) external onlyAuthorized() {
    super._mint(_to, nextTokenId);
    super._setTokenUri(nextTokenId, buildTokenURI());
    nextTokenId += 1;
  }
}