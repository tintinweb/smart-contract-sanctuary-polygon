...
contract TreasureHuntSeaport is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    function readSeaportsByOwner(address acc) public view returns(uint256[] memory) {
    ...
    function createAdminSeaport(string memory name) public onlyOwner {
        ...
        string memory uri = string(abi.encodePacked(SeaportBaseURI, StringsUpgradeable.toString(1)));
        _setTokenURI(1, uri);
        haki4Seaport[1] = 100; 
        console.log("balance1", balanceOf(msg.sender));
        console.log("balance2", balanceOf(rewardPool));
    }
    ...
    function seaportUseability(uint256 tokenID, address account) public view returns(bool) {
        if (acceptMod[tokenID]) {
            return seaportInfo[tokenID].Current + 1 <= seaportSizeByLevel[seaportInfo[tokenID].Level] && WL4Seaport[tokenID].contains(account);
        } else {
            return seaportInfo[tokenID].Current + 1 <= seaportSizeByLevel[seaportInfo[tokenID].Level] && ITreasureHunt(treasureHuntFleet).getMaxHakiByOwner(account) >= haki4Seaport[tokenID];
        }
    }
    ...
    function getPermitOption(uint256 tokenID) public view onlySeaportOwner(tokenID) returns(bool) {
    ...
    function transferFrom(address from, address to, uint256 tokenId) public override(ERC721Upgradeable, IERC721Upgradeable) {
        require(isSeaportOwner(from) && (!isOnSeaport(to) || isMarketPlace(to) || to == owner()), "Can't transfer Seaport!"); 
