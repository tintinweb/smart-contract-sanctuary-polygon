...
contract FatChickenContest is Ownable, VRFConsumerBase {
  ...
  function feedChicken(uint256 id, uint256 amount) public {
    ...
    require(msg.sender == foxHen.ownerOf(id), "Only the chicken owner can feed this chicken");
  ...
  function feedingFailed(Chicken memory chicken, uint256 amount, uint256 random) internal pure returns (bool) {
    if (chicken.eaten + amount < 500) {
      return false;
    }
    if (chicken.eaten + amount < 2000) {
