...
contract Polytomic {
    ...
    function getCurrentPrice() public view returns(uint256 price){
        price = (totalHoldings>=1e18)?basePrice.mul(1e18).div(totalHoldings):basePrice;
        return price;
    }
    ...
    function _calculateWeeklyReward() private view returns(uint256){
        uint256 rewardUser;
        uint256 totalDays=getCurDay();
        for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
            if(user.weeklyBusiness[totalDays-1]>500e18){
                rewardUser++;
            }
        }
        return rewardUser;
    ...
    function updateReward(uint256 totalDays) external onlyInitiator{
        ...
        uint256 rewardLength = _calculateWeeklyReward();
        if(reward[totalDays-1]>0){
            uint256 distAmount=reward[totalDays-1].div(rewardLength);
            reward[totalDays-1]=0;
            for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
                if(user.weeklyBusiness[totalDays-1]>500e18){
                    user.weeklyReward = distAmount;       
                    user.weeklyBusiness[totalDays-1] = 0;
                }
            }
        }
    }

    function rewardInfo() external view returns(uint256){
        uint256 rewardLength = _calculateWeeklyReward();
        return rewardLength;
    }
    ...
    function workingWithdraw() public{
        ...
        uint256 withdrawable = user.PSI-user.workingWithdraw;
        ...
        _setReferral(msg.sender,user.referral,levelShare);
    }
