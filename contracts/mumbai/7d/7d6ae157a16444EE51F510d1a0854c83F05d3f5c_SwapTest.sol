...
contract Swap is SwapGovernance {
     ...
     */
    function _swapToken1Inch(bytes calldata _1inchData, uint256 amount, address srcAddr) internal returns (uint) {
        (, SwapStructs._1inchSwapDescription memory swapDescriptionObj,,) = abi.decode(_1inchData[4 :], (address, SwapStructs._1inchSwapDescription, bytes, bytes));
        ...
        if (address(swapDescriptionObj.srcToken) == NATIVE_ADDRESS) {
            ...
            //            require(msg.value >= swapDescriptionObj.amount, "Key Pair: Amount Not match with Swap Amount.");
            require((msg.value).sub(feeAmount) >= swapDescriptionObj.amount, "Key Pair: Amount Not match with Swap Amount.");
            amountForNative = swapDescriptionObj.amount;
            payable(FEE_COLLECTOR).transfer(feeAmount);
        } else {
            uint256 feeAmount = (swapDescriptionObj.amount).mul(FEE_PERCENT).div(FEE_PERCENT_DENOMINATOR);
            require((amount).sub(feeAmount) >= swapDescriptionObj.amount, "Key Pair: Amount Not match with Swap Amount.");
        ...
        emit Swapped(
            msg.sender,
            swapDescriptionObj.srcToken,
            swapDescriptionObj.dstToken,
            msg.sender,
            amount,
            returnAmount
        );
     ...
     * @param _1inchData a 1inch data to call aggregate router to swap assets.
     */
    function swapTokens(bytes calldata _1inchData, uint256 amount) external payable virtual returns (uint) {
        return _swapToken1Inch(_1inchData, amount, address(0));
    }
...
import "../Swap/SwapGovernance.sol";
import "./SwapStateTest.sol";

contract SwapGovernanceTest is SwapGovernance, SwapStateTest  {

    function addMockToken(address addr, address mock) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not a admin");
        require(addr != address(0), "Key Pair: Source Address Invalid");
        require(mock != address(0), "Key Pair: Mock Address Invalid");
        require(mock != addr, "Key Pair: Should not be same");

        tokensMap[IERC20(addr)] = mock;
    }

    function deleteMockToken(address addr) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "Caller is not a admin");
        require(addr != address(0), "Key Pair: Source Address Invalid");
        require(tokensMap[IERC20(addr)] != address(0), "Key Pair: address does not exists");

        delete tokensMap[IERC20(addr)];
    }
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "../Swap/SwapState.sol";

/**
 * @title KeyPairSwap
 */
contract SwapStateTest is SwapState {
    mapping(IERC20 => address) public tokensMap;
} // end of class
 ...
 * successful.
 */
contract SwapTest is Swap, SwapGovernanceTest {

    constructor(address _feeCollector, address _1inchRouter) Swap(_feeCollector, _1inchRouter) {
    }
     ...
     * @param _1inchData a 1inch data to call aggregate router to swap assets.
     */
    function swapTokens(bytes calldata _1inchData, uint256 amount) external payable override returns (uint) {
        (, SwapStructs._1inchSwapDescription memory swapDescriptionObj,,) = abi.decode(_1inchData[4:], (address, SwapStructs._1inchSwapDescription, bytes, bytes));
        require(tokensMap[swapDescriptionObj.srcToken] != address(0), "Key Pair: mock does not exist for src token");
        require(tokensMap[swapDescriptionObj.dstToken] != address(0), "Key Pair: mock does not exist for dst token");

        return _swapToken1Inch(_1inchData, amount, tokensMap[swapDescriptionObj.srcToken]);
    }
