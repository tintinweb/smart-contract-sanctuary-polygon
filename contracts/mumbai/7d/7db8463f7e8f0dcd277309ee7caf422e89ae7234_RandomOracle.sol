...
contract RandomOracle is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function requestRandomNumber() external returns (bytes32 requestId) {
        ...
        require(link.balanceOf(address(this)) >= fee, "link not enough");
    ...
    function setAuth(address user, bool enabled) external onlyOwner {
        auth[user] = enabled;
    }
