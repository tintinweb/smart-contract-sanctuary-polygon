...
contract Receiver {
    ...
    function executeMetaTransaction(
        ...
        for (uint256 i = 0; i < transactions.length; i++) {
            ...
            // if (ITarget(targetAddress).balanceOf(user) < amount) {
            //     emit TransactionInvalidAmount(amount, "User doesn't have enough balance");
            //     continue;
            // }

            // nonces[user]++;
            // ITarget(targetAddress).exampleFunction(signer, to, amount);
            // emit TransactionProcessed(to, signer);
        }
    }
}
