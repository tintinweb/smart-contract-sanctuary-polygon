...
contract StrategyCurveLP is StratManager, FeeManager, GasThrottler {
    ...
    function chargeFees() internal {
        uint256 nativeFeeBal = IERC20(native).balanceOf(address(this)).mul(TOTAL_FEE).div(TOTAL_FEE_DIVISOR);
        ...
        if (callFee > 0) {
            callFeeAmount = nativeFeeBal.mul(callFee).div(MAX_FEE);
            IERC20(native).safeTransfer(tx.origin, callFeeAmount);
        }

        uint256 strategistFee;
        if (STRATEGIST_FEE > 0) {
            strategistFee = nativeFeeBal.mul(STRATEGIST_FEE).div(MAX_FEE);
...
abstract contract FeeManager is StratManager {
    uint constant public TOTAL_FEE = 45; // TOTAL_FEE/TOTAL_FEE_DIVISOR = 45/1000 = 4.5% of profit
    uint constant public TOTAL_FEE_DIVISOR = 1000;
