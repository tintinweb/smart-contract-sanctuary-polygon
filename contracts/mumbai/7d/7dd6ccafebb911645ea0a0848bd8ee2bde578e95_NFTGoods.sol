...
import "./openzeppelin_for_proxied/ERC721.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "forge-deploy-proxy/ForgeDeploy_Proxied.sol";

contract NFTGoods is INFTGoods, Initializable, Proxied, ERC721, AccessControl {
    ...
    uint256 public countTensOfThousands;
    ...
    function upgradeStatus(uint256 tokenId) external override {
        ...
        IStaking(registry.getStaking()).nftStatusChanged(tokenId, currentStatus);
    ...
    function mint(address to, string memory uri) external override onlyRole(MINTER_ROLE) {
    ...
    function setCountTensOfThousands(uint256 _countTensOfThousands) external override onlyRole(DEFAULT_ADMIN_ROLE) {
        countTensOfThousands = _countTensOfThousands;
    }
    ...
    function initialize(
        ...
        address _minter,
        address _burner,
        uint256 _countTensOfThousands
    ) public proxied initializer {
        if (_countTensOfThousands == 0) {
            revert NFTGoodsCountNFTsIsZero();
        }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, AccessControl, IERC165) returns (bool) {
        return
            interfaceId == type(INFTGoods).interfaceId ||
    ...
    function _afterTokenTransfer(
        ...
        if (getStatus[firstTokenId] == Status.Packed && from != address(0)) {
            ...
            if (from == tempRegistry.root()) {
                IStaking(tempRegistry.getStaking()).startCountingToPassiveStakingForNFT(firstTokenId);
            }
            IControl(tempRegistry.getControl()).saveNFTGoodsInformation(address(this), firstTokenId, from, to);
        }
...
error NoRewardssForNFT(); //Ошибка вывода при отсутствии наград за стейкинг у NFT
error ThisNFTIsHaldedOnStacking(); //Ошибка проведения запрещенных операций над застейканными NFT
...
error IsNotNFTGoodsAddress(); //Ошибка для бота в случае сбоя времени на сервере
...
interface IStaking {
    ...
    function calcReward(address nftGoods, uint256 id) external view returns (uint256);
}
