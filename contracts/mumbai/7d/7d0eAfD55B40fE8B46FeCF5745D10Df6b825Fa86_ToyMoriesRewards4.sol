...
contract ToyMoriesRewards4 is ReentrancyGuard, Ownable  {
    ...
    function issueAirDropERC20(address[] calldata holder) public onlyOwner{
        require(totalSupply > supplyCounter, "All tokens have been distributed");
        for (uint256 i = 0; i < holder.length; i++) {
            users[holder[i]].unclaimedRewards = users[holder[i]].unclaimedRewards + airDrop;
            supplyCounter = supplyCounter + airDrop;
        }
    }
    ...
    function claimRewards() external {
        uint256 rewards = users[msg.sender].unclaimedRewards;
        require(rewards > 0, "You have no rewards to claim");
        users[msg.sender].unclaimedRewards = 0;
        rewardsToken.safeTransfer(msg.sender, rewards);
    }

    function seeSupplyCounter() public view returns (uint256) {
