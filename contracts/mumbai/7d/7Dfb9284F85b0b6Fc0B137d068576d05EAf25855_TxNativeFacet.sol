...
/******************************************************************************\
/// @title Portal - TxNative Facet
/// @dev This contract provides functions for user to deposit native token into the portal.
/******************************************************************************/
...
contract TxNativeFacet is IOperations {
    ...
    /// @notice Deposit native token to the portal.
    /// @dev Deposit-related features must be enabled.
    /// @dev Require the native token not paused.
    function depositNative() external payable {
        LibStatusControl._requireDepositEnabled();
        LibTokenManagement._requireUnpaused(0);
        require(msg.value > s.config.minDeposit[0], "TNF: Deposit amount must be greater than minDeposit amount");
