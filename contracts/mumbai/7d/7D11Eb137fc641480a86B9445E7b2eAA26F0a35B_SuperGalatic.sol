    ...
    ISuperGalatic
{
    ...
    // rarity => level => uapCost
    mapping(Rarity => mapping(uint256 => uint256)) public upaCostTable;
    /**
     * @notice event is omitted when user mint gensis NFT
     * @param nftId address of the pool
     */
    event mintGensisNFT(uint256 nftId, address owner, uint256 categoryId);

    /**
     * @notice event is omitted when user mint gensis NFT
     * @param nftId address of the pool
     */
    event updateBodyPart(address nftContract, uint256 nftId, uint256 bodyType, uint256 bodyLevel, uint256 bodyRarity);
    ...
    function initUAPCostTable() private {
        upaCostTable[Rarity.White][1] = 10;
        upaCostTable[Rarity.White][2] = 50;
        upaCostTable[Rarity.White][3] = 200;
        upaCostTable[Rarity.White][4] = 300;

        upaCostTable[Rarity.Green][1] = 12;
        upaCostTable[Rarity.Green][2] = 60;
        upaCostTable[Rarity.Green][3] = 240;
        upaCostTable[Rarity.Green][4] = 360;

        upaCostTable[Rarity.Blue][1] = 15;
        upaCostTable[Rarity.Blue][2] = 75;
        upaCostTable[Rarity.Blue][3] = 300;
        upaCostTable[Rarity.Blue][4] = 450;
        upaCostTable[Rarity.Blue][5] = 600;

        upaCostTable[Rarity.Purple][1] = 20;
        upaCostTable[Rarity.Purple][2] = 100;
        upaCostTable[Rarity.Purple][3] = 400;
        upaCostTable[Rarity.Purple][4] = 600;
        upaCostTable[Rarity.Purple][5] = 800;
    }
     ...
     */
    function createGenesis(address minter) external override onlyFactory {
        uint256 newID = totalSupply() + 1;
        _safeMint(minter, newID);

        nftInfos[newID][uint256(BodyType.Head)] = BodyInfo(1, Rarity.White);
        nftInfos[newID][uint256(BodyType.Arm)] = BodyInfo(1, Rarity.White);
        nftInfos[newID][uint256(BodyType.BackPack)] = BodyInfo(1, Rarity.White);
        nftInfos[newID][uint256(BodyType.Chest)] = BodyInfo(1, Rarity.White);
        nftInfos[newID][uint256(BodyType.Ear)] = BodyInfo(1, Rarity.White);
        nftInfos[newID][uint256(BodyType.Leg)] = BodyInfo(1, Rarity.White);

        emit mintGensisNFT(newID, minter, categoryId);
    }
    ...
    function updateBody(
        ...
        require(_ableToUpdate(info.level, info.rarity), Errors.CANNOT_UPDATE);
        uint256 uapAmount = upaCostTable[info.rarity][info.level] * 1e18;
        info.level = info.level + 1;
        ISuperGalaticFactory(factory).costUAPForBodyUpdate(uapAmount, nftOwner);
        emit updateBodyPart(address(this), tokenId, bodyType, info.level, uint256(info.rarity));
    }

    /**
     * @notice check body's level is upgradeable or not
     * @param originalLevel current body level
     * @param rarity body part rarity
     */
    function _ableToUpdate(uint256 originalLevel, Rarity rarity) private pure returns (bool) {
        if (rarity == Rarity.White || rarity == Rarity.Green) {
            return originalLevel < 5;
        } else {
            return originalLevel < 6;
        }
    }
...
interface ISuperGalatic {
    ...
    function createGenesis(address minter) external;
...
interface ISuperGalaticFactory {
    ...
    function costUAPForBodyUpdate(uint256 uapAmount, address nftOwner) external;
}
