...
contract distributeToken {
    ...
    function setAuthority(address auth_) public onlyAuth {
        auth = auth_;
    }
    ...
    function feedBulk(UserData[] calldata userdata) public onlyAuth{
        for(uint256 i=0; i<userdata.length; i++){
            userData[userdata[i].wallet] = userdata[i];
        }
    }


     function withdraw() nonReentrancy public {
        require(userData[msg.sender].claimed==false);
        userData[msg.sender].claimed = true;
        uint256 amt_ = userData[msg.sender].amt;
        token.transfer(msg.sender, amt_);
    }
    ...
    function withdrawOwner() public onlyAuth {
