...
contract SiloManagerFactory is Ownable, KeeperCompatibleInterface{
    ...
    {   uint registryCount = Registry.getUpkeepCount() - 1;
        if(registryCount > lastCheckedUpkeepId){
            //(address potentialManager,uint gas,bytes memory check,,,address admin,) = Registry.getUpkeep(lastCheckedUpkeepId + 1);
            //if(isManager[potentialManager] && gas == GAS_LIMIT && check.length == 0 && admin == address(this)){//next upkeep id is a manager
            //    performData = abi.encode(potentialManager, lastCheckedUpkeepId + 1);
            //}
            //else{
            //    performData = abi.encode(address(0), lastCheckedUpkeepId+1);
            //}
            //upkeepNeeded = true;
...
contract SiloManager is KeeperCompatibleInterface{
    ...
    function setUpkeepId(uint id) external{
        ...
        address target = ManagerFactory.getTarget(id);
        require(address(this) == target, "Manager address does not match upkeep id target");
        upkeepId = id;
    }
