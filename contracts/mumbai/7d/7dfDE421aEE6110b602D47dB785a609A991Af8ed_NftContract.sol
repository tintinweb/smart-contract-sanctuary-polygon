...
contract NftContract is ERC721Enumerable,Ownable {
    ...
    uint256 public constant MAX_PER_MINT = 10000;
    ...
    function mintNft(uint _quantity) external onlyOwner payable{
        uint256 totalMinted = _tokenIds.current();
        require(totalMinted.add(_quantity) <= MAX_SUPPLY, "Quantity Exceed MAX SUPPLY");
        require(_quantity > 0 && _quantity <= MAX_PER_MINT, "Cannot mint specified number of NFTs.");
        require(msg.value >= NFT_PRICE*_quantity, "Not enough balance");

        for (uint256 i = 0; i < _quantity; i++) {
            _safeMint(msg.sender, _tokenIds.current());
		    emit newNftMinted(_tokenIds.current(), msg.sender);
            emit PermanentURI("Frozen (IPFS)", _tokenIds.current());
            _tokenIds.increment();
        }
