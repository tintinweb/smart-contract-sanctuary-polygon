...
contract NoWarNFT is ERC1155Supply, Pausable, Ownable {
    ...
    uint256 public maxMintableToken = 0;
    uint256 public minMintPrice = 0.01 ether;

    uint256 private nonce = 0;
    uint256 private mintBucket = 1;
    uint256 private mintBase = 10;
    ...
    function setMintParams(
        ...
        mintCount = _max + 1 - _min;
        mintBase = _base**2;
        mintBucket = mintBase**(mintCount - 1);
    ...
    function mint() public payable whenNotPaused {
        ...
        // console.log("mintBucket %s mintCount %s minMintableToken %s", mintBucket, mintCount, minMintableToken);
        uint256 x = ((((_random % mintBucket) * minMintPrice) / msg.value) *
            minMintPrice) / msg.value;
        // console.log("%s", x);
        uint256 y = maxMintableToken;
        while (x > 0) {
            if (y > minMintableToken) y--;
            x /= mintBase;
        }
        // console.log("%s", y);
        _mint(_sender, y, 1, "");
