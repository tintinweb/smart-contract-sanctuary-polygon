...
contract NFTAuction is ERC721URIStorage, ReentrancyGuard {
    ...
    function bid(uint256 listingId,uint256 bidPrice) public  {
        require(isAuctionOpen(listingId), 'auction has ended');
        Listing storage listing = listings[listingId];
        require(msg.sender != listing.seller, "cannot bid on what you own");

        uint256 newBid = bids[listingId][msg.sender];        
        require(bidPrice >= newBid, "cannot bid below the latest bidding price");
        require(bidPrice >= listing.globalhighestbidder, "cannot bid below the latest bidding price");
        bids[listingId][msg.sender] += bidPrice;
        
        highestBidder[listingId] = msg.sender;
        listing.globalhighestbidder = bidPrice;
        listing.price = bidPrice;
    ...
    function buy(uint256 listingId) public payable nonReentrant {
        ...
        require( msg.sender == winner, 
            'only winner account can complete auction'
        );

        if(winner != address(0)) {

            uint256 amount = bids[listingId][winner]; 
            require(msg.value == amount,"amount is not equal to bidding amount");
            bids[listingId][winner] = 0;
            _transferFund(payable(listing.seller), msg.value);
    ...
    function nftnotsale(uint256 listingId) public  nonReentrant {
        require(!isAuctionOpen(listingId), 'auction is still open');

        Listing storage listing = listings[listingId];
        address previous_owner = highestBidder[listingId]; 
        require( msg.sender == previous_owner, 
            'only previous_owner account can complete the auction'
        );
        _transfer(address(this), previous_owner, listing.tokenId);
        listings[listingId] = Listing({
            seller: msg.sender,
            tokenId: listingId,
            price: 0,
        ...
        listing.status = STATUS_DONE;
        emit AuctionCompleted(listingId, listing.seller, previous_owner, bids[listingId][previous_owner]);
    }
