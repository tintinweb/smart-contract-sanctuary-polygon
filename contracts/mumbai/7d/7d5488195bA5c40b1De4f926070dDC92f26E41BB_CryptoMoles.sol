...
import "./utils/ContentMixin.sol";
import "./utils/NativeMetaTransaction.sol";


contract CryptoMoles is ERC721, IERC2981, Pausable, Ownable, ReentrancyGuard, NativeMetaTransaction, ContextMixin {
    ...
    address private _treasureAddress;
    ...
    modifier allocTokens(uint32 numToMint) {
        require(
            _tokenIds.current() + numToMint <= (MAX_NFT - (MAX_GIFT - GIFT_COUNT)),
    ...
    constructor(
        ...
        address _treasureAdd,
        address _proxyRegistryAddress
    ) ERC721(_name, _symbol) {
        _treasureAddress = _treasureAdd;
        proxyRegistryAddress = _proxyRegistryAddress;
        _baseTokenURI = _baseUri;
    }

    function setBaseTokenURI(string calldata _baseTknURI) external onlyOwner {
        _baseTokenURI = _baseTknURI;
    }
    ...
    function setTreasureAddress(address _treasureAdd) external onlyOwner {
        _treasureAddress = _treasureAdd;
    }
    ...
    function tglSale() external onlyOwner {
    ...
    function unpause() public onlyOwner {
     ...
     function withdrawFunds(uint256 _amt) public onlyOwner {
        ...
        (bool success, ) = payable(_treasureAddress).call{value: pay_amt}("");
    ...
    function _mintSingleMole() private {
        uint256 newTokenID = _tokenIds.current();
        _safeMint(_msgSender(), newTokenID);
        _tokenIds.increment();
    }

    function mintMole(uint32 _count) 
        external
        payable
        nonReentrant
        saleLive
        allocTokens(_count)
        correctPayment(PRICE, _count) {

        require(
            balanceOf(_msgSender()) + _count <= MAX_MINT,
    ...
    function reserveMoles(uint32 tokenCount) external
        onlyOwner
        allocTokens(tokenCount) {
    ...
    function airDrop(address[] calldata receivers) public onlyOwner {
        require(
            _tokenIds.current() + receivers.length <= MAX_NFT,
            "Sorry, there are not enough token remaining."
        );
        require(
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        require(contractRoyalties == true, "Royalties disabled");

        return (_treasureAddress, (salePrice * 7) / 100);
    }
