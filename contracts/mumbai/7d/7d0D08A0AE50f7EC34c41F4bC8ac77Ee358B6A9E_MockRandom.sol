...
//HANDLY2 = 70,71;
//HAVEHELMETLY2 = 60,61;
//EYELY2 = 80,81,82,83,84;
//BODYLY2 = 100,101;
//SKILLSPOOL =

contract MockRandom is AccessControl {
    using Strings for string;

    bytes32 private keyHash;
    VRF2 private vrf;
    RARITIES private rarities;

    uint32 curCollection = 1;

    event Result (string metadata);
    event ResultSorting (string metadata);
    event StartRanNum ( 
        uint32 finalHand,
        uint32 handRarity,
        uint32 finalEyes,
    ...
    function setVRF(VRF2 _vrf) public onlyRole(DEV_ROLE) {
        vrf = VRF2(_vrf);
    }
    ...
    {
        ResultStorage memory resultStorage = startRandomNumber(tokenId, _msgSender);
        result = sortingMetadata(resultStorage);
        emit Result(result);
        return result;
    }
    ...
    function startRandomNumber(uint256 tokenId, address _msgSender)
        public
        returns (ResultStorage memory resultStorage)
    {
        ...
        Layer2Storage memory layer2 = randomLayer2(resultStorage.haveHelmet, regenId);
        ...
        emit StartRanNum(
            resultStorage.finalHand, 
            resultStorage.handRarity,
            resultStorage.finalEyes,
            resultStorage.finalSkills
        );
