    ...
    AccessControl
{
    ...
    Counters.Counter public _nextItemId;
    ...
    event InfanityCreated(uint256 itemId, bool enabled, bool exist);
    ...
    mapping(address => mapping(uint256 => uint256)) mintedPerCollections;
    ...
    function mint(
        ...
        _mintItem(msg.sender, _itemId, _quantity);
    }
    ...
    function mint(
        ...
        _mintItem(_to, _itemId, _quantity);
    }
    ...
    function _mintItem(
        ...
        uint256 _itemId,
        uint256 _quantity
    ) internal onlyExistingItem(_itemId) {
        if (_to == address(0)) {
    ...
    function getCollectionInfo(
        uint256 itemId
    ) external view onlyExistingItem(itemId) returns (InfanityItem memory) {
    ...
    function enableCollection(
        uint256 _id
    ) external onlyExistingItem(_id) onlyHasRole(ADMIN_ROLE) {
    ...
    function createCollection(bool enabled) external onlyHasRole(ADMIN_ROLE) {
        _create(enabled);
    }

    function _create(bool enabled) private {
        ...
        emit InfanityCreated({itemId: itemId, enabled: enabled, exist: true});
    }
    ...
    function _isValidTokenAddress(address _addrs) internal view returns (bool) {
        return
            _addrs.supportsInterface(type(IERC1155).interfaceId) ||
            _addrs.supportsInterface(type(IERC721).interfaceId);
    }
    ...
    function resetTokenRoyalty(
        uint256 tokenId
    ) external onlyHasRole(ADMIN_ROLE) {
...
interface IInfanity {
    /// Check for brand exist
    error NonExistingBrand();

    /// Cannot query non existing item
    error NonExistingItem();

    /// Cannot set cap less than minted
    error UpdateLessThanCap();

    /// Cannot mint to zero address
    error MintToZeroAddress();

    /// Cannot mint brand that already disabled
    error MintDisableBrand();

    /// Cannot mint disabled item
    error MintDisableItem();

    /// Mint invalid collection type {CollectionType}
    error MintInvalidCollectionType();

    /// Cannot mint more than cap
    error MintQuantityExceedsLimit();

    /// Cannot mint more than limit per tx
    error MintQuantityExceedsLimitPerTx();

    /// Cannot query brand that already disabled
    error CallerQueryDisableBrand();

    /// Cannot query item that already disabled
    error CallerQueryDisableItem();

    /// Cannot create brand that already exist
    error CreateExistingBrand();

    /// Cannot add invalid token address
    error NonSupportedTokenGatedAddress();

    /// Cannot set uint to zero value
    error UintZeroValue();

    /// Cannot set data to zero address
    error SetToZeroAddress();

    /// @notice Mint base on collection type
    /// @param _itemId The collection Id
    /// @param _quantity The number of token to mint base on token Id
    function mint(uint256 _itemId, uint256 _quantity) external payable;

    /// @notice Mint function by allowed caller only
    /// @param _to The address where to mint the token
    /// @param _itemId The collection Id
    /// @param _quantity The collection quantity
    function mint(address _to, uint256 _itemId, uint256 _quantity) external;
}
...
interface IInfanityERC721 {
    function mint(address to, uint256 collectionId, uint256 quantity) external;
}
