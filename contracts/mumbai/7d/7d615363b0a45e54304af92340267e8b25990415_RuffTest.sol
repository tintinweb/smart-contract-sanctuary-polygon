...
contract RuffTest is ERC721Holder, Ownable {
    ...
    uint256 private constant DEFAULT_RAFFLE_DURATION = 60 minutes;
    ...
    function endRaffle(uint256 raffleId) public {
        ...
        uint256 creatorAmount = prizeAmount.sub(feeAmount);
    ...
    function endRaffleAfterExpiry(uint256 raffleId) external onlyOwner {
        require(raffles[raffleId].id > 0, "Raffle does not exist");
        require(raffles[raffleId].ended, "Raffle has not ended yet");

        uint256 endTime = raffles[raffleId].endTime.add(DEFAULT_RAFFLE_DURATION);
        require(block.timestamp >= endTime, "The raffle has not expired yet");

        IERC20 token = IERC20(tokenAddress);
        uint256 ticketSold = raffles[raffleId].ticketSold;
        uint256 ticketPrice = raffles[raffleId].ticketPrice;
        uint256 refundAmount = ticketSold.mul(ticketPrice).mul(TOKEN_DECIMALS);
        uint256 feeAmount = refundAmount.mul(FEE_PERCENTAGE).div(100);
        uint256 refundPerParticipant = refundAmount.sub(feeAmount).div(participants[raffleId].length);

        for (uint256 i = 0; i < participants[raffleId].length; i++) {
            address participant = participants[raffleId][i];
            token.transfer(participant, refundPerParticipant);
        }
