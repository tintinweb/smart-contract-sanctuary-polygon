...
def __init__(_token: address, _voting_escrow: address):
    ...
    @param _token `ERC-20 C3` contract address
...
def _get_corrected_info(addr: address) -> CorrectedPoint:
    ...
    @notice Get the most recently recorded rate of voting power decrease for `addr`.
            Corrected by VOTE_WEIGHT_MULTIPLIER (veC3).
    @param addr Address of the user wallet
    @return CorrectedPoint
    """
    escrow: address = self.voting_escrow
    vec3_balance: uint256 = VotingEscrow(escrow).balanceOf(addr)
    locked_balance: LockedBalance = VotingEscrow(escrow).locked(addr)
    locked_end: uint256 = locked_balance.end
    locked_c3: uint256 = convert(locked_balance.amount, uint256)

    corrected_slope: uint256 = 0

    # Decay to 0. Decaying to 1 veC3 <> 1 C3 gives wrong expected results
    if (locked_end > block.timestamp):
        corrected_slope = vec3_balance / (locked_end - block.timestamp)

    return CorrectedPoint({bias: vec3_balance, slope: corrected_slope, lock_end: locked_end, c3_amount: locked_c3})
...
def change_global_emission_rate(new_rate: uint256):
    """
    @notice Change C3 emission rate
    @param new_rate new emission rate (C3 per second)
    """
    assert msg.sender == self.admin
