...
contract RentHouse {
    ...
    function withdraw(uint256 property_id, uint256 booking_id) public payable {
        if (properties[property_id].owner != msg.sender) {
            revert RentHouse__NotOwner();
        }
        // uint256 price = properties[property_id].price;
        // (bool success, ) = msg.sender.call{value: price}("");
        // require(success);
        // transferFrom(address(this), msg.sender, price);
        (bool _sent, bytes memory _data) = msg.sender.call{
            value: ((properties[property_id].price) *
                (bookings[booking_id].checkoutDate - bookings[booking_id].checkinDate))
        }("");
        require(_sent, "Failed to send ether");
        // payable(address(this)).transfer(properties[property_id].price);
    }
