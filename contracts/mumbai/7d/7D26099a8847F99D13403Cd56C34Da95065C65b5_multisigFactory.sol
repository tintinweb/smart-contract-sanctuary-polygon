...
contract multisigAccountV2 is Account, verifySignature {
    ...
    function initialize(address _admin, bytes calldata _data) public override initializer {
        ...
        (_owners, _numConfirmationsRequired) = abi.decode(_data, (address[], uint256));
    ...
    function execute(
        ...
        bytes32 messageHash = getMessageHash(_target, _value, _calldata);
        ...
        require(
            numConfirmationsRequired <= transactionToAddressesConfirmations[messageHash].length()
        );
...
// Smart wallet implementation
import {multisigAccountV2} from "./multisigAccountV2.sol";

contract multisigFactory is BaseAccountFactory {
    /// @notice Emitted when a new Token Gated Account is created.
    event multisigAccountCreated(address indexed account, bytes indexed data);
    ...
    constructor(
        ...
        BaseAccountFactory(
            address(new multisigAccountV2(_entrypoint, address(this)))
        )
    ...
    function _initializeAccount(
        ...
        multisigAccountV2(payable(_account)).initialize(_admin,_data);
        emit multisigAccountCreated(_account, _data);
    }
}
