...
contract Kenobi {
    ...
    uint8 public rentalLimitMonths; // Months any enterprise can pay rent in advance for.
    ...
    event Rental(
        ...
        uint256 rentPaid,
        uint256 tax,
        uint256 distributableRevenue,
        uint256 rentedFrom,
        uint256 rentedUntill
    );
    event SharesOffered(
        address Seller,
        uint256 AmmountShares,
    ...
    modifier eligibleToPayRent() {
        //only one enterprise at a time can be allowed to pay rent.
    ...
    {
        //shows whether someone is a stakeholder.
    ...
    function currentTenantCheck(address _tenantcheck)
        public
        view
        returns (bool, uint256)
    {
        //only works if from block.number on there is just one enterprise, otherwise tells untill when rent is paid.
        require(
            occupiedUntill == rentpaidUntill[enterprise],
            "The entered address is not the current enterprise"
        );
        if (rentpaidUntill[_tenantcheck] > block.number) {
            uint256 daysRemaining = ((rentpaidUntill[_tenantcheck] -
                block.number) * avgBlockTime) / 86400; //86400 seconds in a day.
            return (true, daysRemaining); //gives enterprise paid status true or false and days remaining
        } else return (false, 0);
    }
    ...
    function banStakeholder(address _stakeholder) public onlyGov {
        // can remove stakeholder from stakeholders array and...
    ...
    function setTax(uint8 _x) public onlyGov {
        //set new tax rate (for incoming rent being taxed with %)
    ...
    function SetAvgBlockTime(uint8 _sPerBlock) public onlyGov {
        //we do not have a forgery proof time measurement in Ethereum. Therefore we count the ammount of blocks. One Block equals to 13s but this can be changed by the government.
    ...
    function distribute() public onlyGov {
        // accumulated funds are distributed into revenues array for each stakeholder according to how many shares are held by shareholders. Additionally, government gets tax revenues upon each rental payment.
    ...
    function seizureFrom(
        ...
        //government has unlimited allowance, therefore  can seize all assets from every stakeholder. Function also used to buyShares from Stakeholder.
    ...
    function offerShares(uint256 _sharesOffered, uint256 _shareSellPrice)
        public
    {
        //Stakeholder can offer # of Shares for  Price per Share
    ...
    function buyShares(uint256 _sharesToBuy, address payable _from)
        ...
        payable
    {
        //Stakeholder can buy shares from seller for sellers price * ammount of shares
        (bool _isStakeholder, ) = isStakeholder(msg.sender);
        require(_isStakeholder);
        require(
            msg.value == _sharesToBuy * shareSellPrice[_from] &&
                _sharesToBuy <= sharesOffered[_from] &&
                _sharesToBuy <= shares[_from] &&
                _from != msg.sender
        ); //
    ...
    function transfer(address _recipient, uint256 _amount)
        ...
        returns (bool)
    {
        //transfer of Token, requires isStakeholder
        ...
        eligibleToPayRent
    {
        ...
        require(
            msg.value == _rentdue &&
                ...
                block.number + rentalLimitBlocks
        ); //sent in Ether has to be _rentdue; additional blocks for rental cannot be higher than limit.
        ...
        if (rentpaidUntill[enterprise] == 0 && occupiedUntill < block.number) {
            //hasn't rented yet & flat is empty
            rentpaidUntill[enterprise] = block.number + _additionalBlocks; //rents from now on
            rentalBegin = block.number;
        } else if (
            rentpaidUntill[enterprise] == 0 && occupiedUntill > block.number
        ) {
            //hasn't rented yet & flat is occupied
            rentpaidUntill[enterprise] = occupiedUntill + _additionalBlocks; //rents from when it is free
            rentalBegin = occupiedUntill;
        } else if (rentpaidUntill[enterprise] > block.number) {
            //is renting, contract is runing
            rentpaidUntill[enterprise] += _additionalBlocks; //rents from when it is free
            rentalBegin = occupiedUntill;
        } else if (
            rentpaidUntill[enterprise] < block.number &&
            occupiedUntill > block.number
        ) {
        ...
        occupiedUntill = rentpaidUntill[enterprise]; //set new occupiedUntill
        emit Rental(
            block.timestamp,
            msg.sender,
            msg.value,
            _taxdeduct,
            (msg.value - _taxdeduct),
            rentalBegin,
            occupiedUntill
        );
    }
