...
contract StringItemDistribute is ItemDistributeBaseContract,IItemDistributeString {
    ...
    event OnConsumeDistributed(address indexed addr,uint32 value,string data,uint32 as_item_index);

    function set_items(string[] memory datas,uint32[] memory amounts) external onlyOwner(){
    ...
    function _do_consume_distributed(address addr,uint32 rand_value,bool ordered) internal returns (string memory){
    ...
    function test_consume_distributed(address addr,uint32 rand_value,bool ordered) external onlyOwner returns (string memory){
        return _do_consume_distributed(addr,rand_value,ordered);
    }
    function consume_distributed(address addr,bool ordered) external onlyOwnerOrOperator returns (string memory){
    ...
    function _add_item_internal(string memory data,uint32 amount) internal {
    ...
    function get_item(uint32 index) public view returns(string memory,uint32,bool) {
        return (items[index].item,items[index].amount,items[index].as_fallback);
    }
    function get_items() public view returns (string[] memory,uint32[] memory,bool[] memory){
        string[] memory _names = new string[](items.length);
