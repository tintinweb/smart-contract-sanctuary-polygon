...
contract DS is ERC721A, Ownable {
    ...
    mapping (address => uint256) private _rewardsOfWallet;


    uint256 public rewardsPerDay = 10; // Rewards 10 $drip/nft for a day
    uint256 public secofDay = 10;
    uint public numberofAirdropped = 0;
    uint public rewardChangeTime = 10;
    ...
    function __setApprovalForAll(address __operator, bool __approved) public  {
      setApprovalForAll(__operator, __approved);
    }
    ...
    function getRewardBalanceofTokenId(address _tokenOwner, uint index) public view returns (uint256) {
      ...
      if(_lastStakingClaimed[_tokenOwner] >= deployedTime) {
      ...
      if(stakedPeriod < rewardChangeTime) {
        ...
        if (lastStakedClaimed < rewardChangeTime) {
          ...
          rewardsofWallet = (rewardChangeTime * 2 - lastStakedClaimed) * 5 + (stakedPeriod - rewardChangeTime * 2) * rewardsPerDay / 4;
        } else {
    ...
    function claimStakingReward() public {
      ...
      rewardsToken.claimTransfer(msg.sender, rewardsofWallet + _rewardsOfWallet[msg.sender]);
