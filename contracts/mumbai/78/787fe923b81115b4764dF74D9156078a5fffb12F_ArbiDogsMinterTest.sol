...
contract ArbiDogsMinterTest is InterfaceArbiDogsMinter, Ownable, ReentrancyGuard {
    ...
    modifier onlyNotInitialized() {
        require(isInitialized == false, 'ArbiDogsMinterTest: contract already initialized');
        _;
    }
    ...
    modifier onlyInitialized() {
        require(isInitialized, 'ArbiDogsMinterTest: contract not initialized');
        _;
    }
    ...
    function initialize(
        ...
        require(_arbiDogsNft != address(0), 'ArbiDogsMinterTest: invalid ArbiDogsNft address');
        require(_maxMintableTokens > 0, 'ArbiDogsMinterTest: invalid _maxMintableTokens');
    ...
    function _claim() public payable onlyInitialized nonReentrant {
        if (isWhitelistEnabled) {
            require(whitelist[msg.sender], 'ArbiDogsMinterTest: User not on whitelist');
        }
        require(tokenClaimed < maxMintableTokens, 'ArbiDogsMinterTest: All tokens are sold');
        uint256 numberOfTokens = msg.value / fee;
        numberOfTokens = _Min(numberOfTokens, maxMintableTokens - tokenClaimed);
        require(numberOfTokens > 0, 'ArbiDogsMinterTest: Invalid number of tokens');
        ...
        require(treasurySuccess, 'ArbiDogsMinterTest: Transfer to treasuryAddress failed');
        ...
        if (totalPayValue < msg.value) {
            ...
            require(success, 'ArbiDogsMinterTest: Transfer user surplus failed');
        }
    }
    ...
    function increaseMaxMintableTokens(uint256 amount) external onlyOwner {
        require(amount > 0, 'ArbiDogsMinterTest: invalid _maxMintableTokens');
    ...
    function decreaseMaxMintableTokens(uint256 amount) external onlyOwner {
        ...
        require(
            ...
            'ArbiDogsMinterTest: amount should not be greater than current maxMintableTokens'
        );
    ...
    function _setFee(uint256 _fee) internal {
        require(_fee > 0, 'ArbiDogsMinterTest: invalid fee');
        fee = _fee;
    }

    function _setTreasuryAddress(address _treasuryAddress) internal {
        require(_treasuryAddress != address(0), 'ArbiDogsMinterTest: invalid treasury address');
