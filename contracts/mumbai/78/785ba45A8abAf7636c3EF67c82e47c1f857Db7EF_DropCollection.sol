...
abstract contract BaseCollection is ERC2981, IBaseCollection, ReentrancyGuard {
    ...
    function createItem(
        ...
        _mintItemOnPublish(creator, tokenId, preMinted);
    ...
    function _mintItemOnPublish(
        address creator,
        uint256 tokenId,
        uint256 preMinted
    ) internal {
        uint256 freebiesAmount = IAISkinFactory(aiskinFactory())
            .getPublishFreebiesAmount();
        address treasury = IAISkinFactory(aiskinFactory()).getTreasury();

        Item memory item = _items[tokenId];
        if (preMinted > freebiesAmount) {
            uint256 totalPrice = (preMinted - freebiesAmount) * item.priceInWei;
            uint256 preMintFee = (totalPrice * item.shareFee) /
                _feeDenominator();
            if (item.tokenPayment != address(0)) {
                require(
                    IERC20(item.tokenPayment).transferFrom(
                        creator,
                        treasury,
                        preMintFee
                    )
                ...
                if (msg.value != preMintFee) revert InsufficientBalance();
                Address.sendValue(payable(treasury), preMintFee);
            }
        }
    ...
    IDropCollection
{
    ...
    function initialize(
        ...
        __ERC1155_init("");
        __BaseCollection_init(aiskinFactory_, name_, symbol_);
        __BaseDropCollection_init(owner_);
    }

    function __BaseDropCollection_init(
        address owner_
    ...
    function supportsInterface(
        ...
        view
        virtual
        override(ERC1155Upgradeable, BaseCollection)
        returns (bool)
    {
        return
            interfaceId == type(IDropCollection).interfaceId ||
            super.supportsInterface(interfaceId);
    }
    ...
    function _beforeCreateItem(
        address creator,
        uint256 tokenId
    ) internal virtual override onlyCreator(creator) {}
...
interface IAISkinFactory {
    ...
    /**
     * @dev Publish new item either under aiskin master collection or creator's collection. Publishing new item emits {IBaseCollection-CreateItem} on target collection
     *
     * Requirements:
     ...
     * @param preMinted The amount of token to mint on creator address
     */
