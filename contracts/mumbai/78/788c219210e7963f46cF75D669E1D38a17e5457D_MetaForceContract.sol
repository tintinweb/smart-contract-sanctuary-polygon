...
contract MetaForceContract is Ownable, IMetaForceContract {
    ...
    mapping(address => uint256) public availableMFSToBuy;
    ...
    function burnMFSPool() external onlyOwner {
        ...
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            ...
            mfs.burn(registry.getMetaPool(), mfs.balanceOf(registry.getMetaPool()));
        } else {
            core.decreaseFrozenMFS(registry.getMetaPool(), core.getAmountFrozenMFS(registry.getMetaPool()));
        }
        emit MFCPoolMFSBurned();
    }
    ...
    function buyMFS(uint256 amountMFS) external override {
        ...
        if (amountMFS > availableMFSToBuy[msg.sender]) {
            revert MFCBuyLimitOfMFSExceeded(amountMFS, availableMFSToBuy[msg.sender]);
        }

        availableMFSToBuy[msg.sender] -= amountMFS;
    ...
    function activationPack(uint256 level) public override {
        ...
        if (core.getWorkflowStage() != Workflow.SaleOpen) {
            ...
            availableMFSToBuy[msg.sender] += pricePackInMFS;
            //increaseTotalEmission(pricePackInMFS);
    ...
    function renewalPack(uint256 level, TypeRenewalCurrency typeCurrency) public override {
        ICoreContract core = ICoreContract(registry.getCoreContract());
        ...
        if (core.getWorkflowStage() < Workflow.Presale) {
            revert MFCEarlyStageForRenewalPack();
        }
        if (!core.checkRegistration(msg.sender)) {
            revert MFCUserNotRegisteredYet();
        }
        ...
        uint256 priceRenewalPackInUSD = PRICE_PACK_LEVEL1_IN_USD.mul(
            uint256(COEFF_INCREASE_COST_PACK_FOR_NEXT_LEVEL).pow((level - 1).convertIntToFixPoint()).div(
        ...
        if (core.getWorkflowStage() == Workflow.SaleOpen) {
            if (typeCurrency == TypeRenewalCurrency.MFS) {
                ...
                transferMFSFrom(msg.sender, address(this), priceRenewalPackInMFS);
                transferMFSFrom(msg.sender, registry.getMetaPool(), priceRenewalPackInMFS);
                holding.holdOnBehalf(msg.sender, 1, priceRenewalPackInMFS);
            } else {
                if (uint256(typeCurrency) != countRenewal[msg.sender][level] + 1) {
                    revert MFCNeedPayOnlyHMFSLevel(typeCurrency);
                }
                IHMFS hFMS = IHMFS(registry.getHMFS(countRenewal[msg.sender][level] + 1));
                hFMS.burn(msg.sender, priceRenewalPackInMFS);
                if (countRenewal[msg.sender][level] < LEVELS_COUNT - 1) {
                    countRenewal[msg.sender][level] += 1;
                }
            }
        } else {
            transferMFSFrom(msg.sender, registry.getMetaPool(), priceRenewalPackInMFS);
            availableMFSToBuy[msg.sender] += priceRenewalPackInMFS;
        }
        IEnergy energy = IEnergy(registry.getEnergyCoin());
        energy.mint(msg.sender, priceRenewalPackInMFS.mul(core.getEnergyConversionFactor()));
        core.increaseTimestampEndPack(msg.sender, level, PACK_ACTIVITY_PERIOD);
        distributionRewardReferal(msg.sender, level, priceRenewalPackInMFS, priceRenewalPackInUSD);
    ...
    function distributionRewardReferal(
        ...
        address[] memory referrers = core.getReferrers(user, rewardsRefers.length);
