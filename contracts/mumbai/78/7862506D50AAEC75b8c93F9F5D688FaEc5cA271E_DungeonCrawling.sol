...
contract DungeonCrawling is Initializable, DungeonCrawlingSettings {
    ...
    private
    {
        require(!isUserDungeonCrawling(msg.sender), "User is already dungeon crawling");
    ...
    private
    {
        ...
        require(_totalEntities >= dungeonNameToInfo[_dungeonName].minEntitiesPerCrawl, "Too few entities sent to dungeon");
        require(dungeonNameToInfo[_dungeonName].maxEntitiesPerCrawl >= _totalEntities, "Too many entities sent to dungeon");
        ...
        for(uint256 i = 0; i < _orcEntities.length; i++) {
            ...
            require(_orcEntities[i].collection == address(orcs), "Bad address for entity collection");

            _storeEntity(_dungeonName, _orcEntities[i], ORC_EQUIPPABLE, _level);

            dungeonCrawlingInputs[_userInfo.activeInputIndex].entities.push(_orcEntities[i]);
        }
        ...
        for(uint256 i = 0; i < _allyEntities.length; i++) {
            ...
            _allyIds[i] = _allyEntities[i].tokenId;

            require(_orcEntities[i].collection == address(allies), "Bad address for entity collection");
            ...
            dungeonCrawlingInputs[_userInfo.activeInputIndex].entities.push(_allyEntities[i]);
        }
    ...
    {
        DungeonInfo storage _dungeonInfo = dungeonNameToInfo[_dungeonName];
        require(_level >= _dungeonInfo.minimumLevel, "Entity is not at the minimum level for the dungeon");
        ...
        require(_dungeonInfo.maxNumberOfCrawlsPerEntity == 0
            ...
            "Entity has reached maximum crawls for this dungeon");

        require(block.timestamp >= collectionToTokenIdToCooldownTime[_entity.collection][_entity.tokenId],
            "Entity has not finished cooling down");
    ...
    {
        DungeonInfo storage _dungeonInfo = dungeonNameToInfo[_dungeonName];
        require(_inputQuantities.length == _dungeonInfo.inputs.length, "Input length must match dungeon input length");
        ...
        for(uint256 i = 0; i < _inputQuantities.length; i++) {
            ...
            if(_quantity == 0 && _inputRequirement.minQuantity == 0) {
                ...
                require(_quantity >= _inputRequirement.minQuantity, "Quantity given less than minimum required");
                require(_inputRequirement.maxQuantity == 0 || _quantity <= _inputRequirement.maxQuantity, "Quantity given over maximum");
            ...
            dungeonCrawlingInputs[_userInfo.activeInputIndex].inputs.push(DungeonSuppliedInput(
    ...
    contractsAreSet
    {
        require(isUserDungeonCrawling(msg.sender), "User is not dungeon crawling");
        ...
        if(_dungeonInfo.dungeonCooldownPeriod > 0) {
            for(uint256 i = 0; i < _activeInputs.entities.length; i++) {
                ...
                collectionToTokenIdToCooldownTime[_entity.collection][_entity.tokenId]
                    = block.timestamp + _dungeonInfo.dungeonCooldownPeriod;
            }
        }
        ...
        if(_dungeonInfo.dungeonUnlockPeriod == 0) {
            ...
            _userInfo.lockedInputIndexes.push(
                LockedEntityInfo(
                    block.timestamp + _dungeonInfo.dungeonUnlockPeriod,
                    _userInfo.activeInputIndex
                )
            );
        }
    ...
    function activeEntitiesForUser(address _user) public view returns(DungeonCrawlingEntity[] memory) {
        if(!isUserDungeonCrawling(_user)) {
            return new DungeonCrawlingEntity[](0);
        }
        return dungeonCrawlingInputs[userToInfo[_user].activeInputIndex].entities;
    }
    ...
    function suppliedInputsForUser(address _user) public view returns(DungeonSuppliedInput[] memory) {
        ...
        return dungeonCrawlingInputs[userToInfo[_user].activeInputIndex].inputs;
    }
}
...
abstract contract DungeonCrawlingSettings is Initializable, DungeonCrawlingContracts {
    ...
    {
        require(!isKnownDungeon(_dungeonName), "Dungeon is already added");
        require(_info.startTime > 0, "Dungeon start time must be greater than 0");
        require(!compareStrings(_dungeonName, ""), "Dungeon name must not be blank");
        ...
        for(uint256 i = 0; i < _info.inputs.length; i++) {
            ...
            require(_input.itemId > 0, "Bad input item ID");
            require(_input.maxQuantity == 0 || _input.maxQuantity >= _input.minQuantity, "Max quantity must be greater or equal than min quantity");
            require(_input.collection == address(etherOrcsItems) || _input.collection == address(dungeonCrawlingItem), "Unknown input collection");
        }
...
abstract contract DungeonCrawlingState is IDungeonCrawling, ERC721HolderUpgradeable, ERC1155HolderUpgradeable, UtilitiesUpgradeable {
    ...
    mapping(address => mapping(uint256 => uint256)) public collectionToTokenIdToCooldownTime;

    // Globally unique that tracks the inputs to dungeon crawling. Starts from 1.
    uint256 public dungeonInputsIndexCur;
    ...
    mapping(uint256 => DungeonCrawlingInputs) internal dungeonCrawlingInputs;
