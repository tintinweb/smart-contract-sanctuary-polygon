...
interface IPhiDaily {
    ...
    function claimMaterialObject(uint32 eventid, uint16 logicid, Coupon memory coupon) external;
...
contract PhiDailyRelay is ReentrancyGuard, ERC2771Context {
    ...
    function claimMaterialObjectbyRelay(
        uint32 eventid,
        uint16 logicid,
        IPhiDaily.Coupon memory coupon
    ) external nonReentrant {
        IPhiDaily(phiDaily).claimMaterialObject(eventid, logicid, coupon);
        emit LogClaimMaterialObject(eventid, logicid, _msgSender());
    }

    /// @dev Allows user to batch claim material objects via a relayer.
    function batchClaimMaterialObjectbyRelay(
        uint32[] memory eventids,
        uint16[] memory logicids,
        IPhiDaily.Coupon[] memory coupons
    ) external nonReentrant {
        IPhiDaily(phiDaily).batchClaimMaterialObject(eventids, logicids, coupons);
