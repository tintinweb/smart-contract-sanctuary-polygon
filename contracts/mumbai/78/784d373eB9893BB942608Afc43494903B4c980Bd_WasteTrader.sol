...
contract WasteTrader is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWasteToCash {
    ...
    function resetWasteOrders(address _player) internal {
        WasteOrder[] storage wasteOrders = s_wasteOrders[_player];
        for (uint256 orderIdx; orderIdx < wasteOrders.length - 1; orderIdx++) {
            s_wasteOrders[_player].pop();
        }
    }
