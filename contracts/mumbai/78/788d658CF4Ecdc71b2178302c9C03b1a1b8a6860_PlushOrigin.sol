...
contract PlushOrigin is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable, IPlushOrigin {
    ...
    uint256[] public connectionsTypes;
     ...
     */
    function getOrigin(uint256 lifespanParentId) external view returns(Connection[] memory) {
        uint256[] memory idsByLifespan = connectionsById[lifespanParentId];
        Connection[] memory parentConnections;

        for(uint256 i = 0; i < idsByLifespan.length; i++) {
            parentConnections[i] = connections[idsByLifespan[i]];
        }
    ...
    function addConnection(uint256 lifespanParentId, uint256 lifespanChildId, uint256 typeConnectionId, uint256 dateStart, uint256 dateEnd) external {
        ...
        uint256 idParent = connections.length;
        connections[idParent] = Connection(lifespanParentId, lifespanChildId, typeConnectionId, dateStart, dateEnd, true, false);
        connectionsById[lifespanParentId].push(idParent);

        uint256 idChild = connections.length;

        if(!isConnectionExist(lifespanChildId, lifespanParentId)){
            connections[idChild] = Connection(lifespanChildId, lifespanParentId, typeConnectionId, dateStart, dateEnd, false, false);
            connectionsById[lifespanChildId].push(idChild);
        }
    }
    ...
    function isConnectionExist(uint256 lifespanParentId, uint256 lifespanChildId) private view returns(bool){
        ...
        for(uint256 i = 0; i < idsByLifespan.length; i++) {
            ...
            if(parent.lifespanParentId == lifespanParentId && parent.lifespanChildId == lifespanChildId && parent.isDeleted == false) {
                return true;
            }
        }
     ...
     */
    function addConnectionType(uint256 idType) external onlyRole(OPERATOR_ROLE){
        connectionsTypes.push(idType);
    }
     ...
     */
    function removeConnectionType(uint256 idType) external onlyRole(OPERATOR_ROLE){
        for(uint256 i = 0; i < connectionsTypes.length; i++){
            if(connectionsTypes[i] == idType){
     ...
     * @param idType id type of connection
     */
    function isConnectionTypeExist(uint256 idType) private view returns(bool){
        for(uint256 i = 0; i < connectionsTypes.length; i++){
            if(connectionsTypes[i] == idType){
                return true;
            }
