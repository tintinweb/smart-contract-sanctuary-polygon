    ...
    OwnerRecovery
{
    ...
    modifier onlyNFTManager() {
        ...
        require(
            ...
            "ResourceToken: Not NFTManager"
        );
        _;
    }

    constructor(address _NFTManager, address _treasury) ERC20("Wood Token", "WOOD") {
        require(
            _NFTManager != address(0),
            "ResourceToken: NFTManager is not set"
        );
        require(
            _treasury != address(0),
            "ResourceToken: treasury is not set"
        );
    ...
    function _transfer(
        ...
        require(from != address(0), "ResourceToken: transfer from the zero address");
        require(to != address(0), "ResourceToken: transfer to the zero address");
    ...
    function ethTransferToTreasury(uint256 amount) external onlyNFTManager {
    ...
    function excludeFromFee(address account, bool status) external onlyOwner {
        require(account != address(0), "ResourceToken: zero address");
