    ...
    ReentrancyGuard
{
    ...
    ERC2771Context private _erc2771context;

    constructor(MinimalForwarder forwarder)
        ERC721("BetaTest", "BT")
        ERC2771Context(address(forwarder))
    {
        setDefaultRoyalty(0x98a281494b374604E4C8241AD23E87FD198d13ab, 1000);
    }
    ...
    function addSupply(uint256 num) public onlyOwner {
        maxSupply = maxSupply + num;
    }

    function setIsOpenSeaProxyActive() public onlyOwner {
        isOpenSeaProxyActive = !isOpenSeaProxyActive;
    }

    function safeMint(address to) public {
        require(isTrustedForwarder(_msgSender()), "Not trusted");
    ...
    function buyNFT() public payable {
        require(totalSupply() + 1 <= maxSupply, "Alle NFTs vergriffen");
        require(msg.value >= salePrice, "MSG Value too low");
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
    ...
    function burn(uint256 _tokenID) public override {
        if (_msgSender() == ownerOf(_tokenID)) {
            _burn(_tokenID);
        } else if (isTrustedForwarder(_msgSender())) {
            _burn(_tokenID);
        }
    }
