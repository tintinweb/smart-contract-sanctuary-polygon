...
contract Trade {
    ...
    function buyAsset(Order calldata order, Sign calldata sign) external returns(bool) {
        ...
        verifySign(order.seller, tokenIdHash, order.amount, order.erc20Address, order.nftAddress, order.qty, sign);        
    ...
    function executeBid(Order calldata order, Sign calldata sign) external returns(bool) {
        ...
        Fee memory fee = getFees(order.amount, order.nftAddress, order.tokenId, order.isPacked);
        bytes memory tokenIdHash = _encode(order.tokenId);
        verifySign(order.buyer, tokenIdHash, order.amount, order.erc20Address, order.nftAddress, order.qty, sign);
    ...
    function getFees(uint paymentAmt, address buyingAssetAddress, uint[] calldata tokenId, bool isPacked) public view returns(Fee memory) {
        address tokenCreator;
        uint platformFee;
        uint royalty;
        uint royaltyFee;
