...
import "./NoodzToken.sol";
import "@openzeppelin/contracts/access/Ownable.sol";

contract PlayToEarnNoodz is Ownable {
  address private admin;
  uint256 public contractBalance;
  address constant noodzToken = 0x159578aF76796f14eA5431de5CA15D41D57Fa819;
  // address noodzToken;
  ...
  function withdrawIfUnlocked(uint256 _gameId) external returns (bool) {
    require(balances[msg.sender][_gameId].locked == false, "Escrow is locked");
...
contract NoodzToken is Context, ERC20Votes, AccessControlEnumerable, Ownable {
  using SafeMath for uint256;
  bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
  bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
  address[] internal stakeholders;
  address[] internal devWallets;
  ...
  modifier onlyHasRole(bytes32 _role) {
    require(
      ...
      "NoodzToken.onlyHasRole: msg.sender does not have role"
    );
    _;
  }
  ...
  function _transfer(
    ...
    require(
      _to != address(this),
      "NoodzToken._transfer: can not transfer to your self"
    );
  ...
  function makeStake(uint256 _stake) public {
