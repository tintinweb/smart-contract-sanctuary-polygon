...
contract Mechaversus is Initializable, OwnableUpgradeable {
    ...
    function getLobbyInfo( uint256 match_id ) public view returns ( uint256, uint256, uint256 ) {
        return (_matches[ match_id ]._players.length, _matches[ match_id ]._stake_amount, _matches[ match_id ]._block_to_respond);
    }
    ...
    function endMatchConditionsChecks( uint256 match_id ) public virtual callerIsPlayer(match_id) {
        ...
        for( uint256 i = 0; i < _matches[ match_id ]._active_players.length; i++){
            ...
            if( _entities[ match_id ][ nexus_id ] == uint16(0) ) _loseMatch( match_id, _matches[ match_id ]._active_players[ idx ] );
            else{
