...
//载具工厂合约
contract VehicleFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    string internal nftNameVehicle = "MetaGlobe Vehicle NFT";
    ...
    //服务端签名地址
    address internal serverSideSigningKey = address(0);
   ...
   /**
     ...
     * Note 
     *
     * @param _mgge_contract ERC11655合约地址
     * @param _serverSide 服务端签名地址
     * @param _salt 随机数加盐
     */    
    constructor(address _mgge_contract, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        //授权给游戏主合约
        _setupRole(AUTHORIZATION_ROLE, _mgge_contract);
        ...
        DOMAIN_SEPARATOR = keccak256(
            abi.encode(
                ...
                keccak256(bytes("VehicleFactory")),
                keccak256(bytes("1")),
                block.chainid,
                address(this)
            )
        ...
        //初始化
        //飞行器1（防御） 
        uint _type_id = _addItem(120001,200,300,"defense");
        _setPropValues(_type_id, "capacity",10,100);
        _setPropValues(_type_id, "defense",100,150);
        _setPropValues(_type_id, "hp",10,50);
        _setPropValues(_type_id, "duration",80,100);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",10,50);
        //飞行器2（生命） 
         _type_id = _addItem(120002,150,301,"hp");
        _setPropValues(_type_id, "capacity",10,100);
        ...
        _setPropValues(_type_id, "move",10,50);
        //飞行器3（耐久） 
         _type_id = _addItem(120003,100,302,"duration");
        _setPropValues(_type_id, "capacity",10,100);
        _setPropValues(_type_id, "defense",50,80);
        _setPropValues(_type_id, "hp",50,80);
        _setPropValues(_type_id, "duration",100,140);
        _setPropValues(_type_id, "potential",1,5);
        _setPropValues(_type_id, "move",10,50);
    }
    ...
    function _addItem(
        ...
        uint256  _type_id = _typeTracker.current();
		_typeTracker.increment();

        vehicles[_type_id] = Vehicle (
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    function setWeight(uint256 _type_id, uint256 _weight) external nonReentrant onlyOperator returns(bool){
        ...
        vehicles[_type_id].weight = _weight;
        return true;
    } 
     ...
     * Note: msg.sender是erc1155合约地址, 并不是调用者的钱包地址
     */
        ...
        returns(uint[] memory)
    {
        ...
        for (uint i = 0; i < _typeCount; i++) {
             if (vehicles[i].count < vehicles[i].prop_sum) {
                 _count ++;
             }
        }
        if (_count == 0) {
        ...
        Vehicle storage vehicle = vehicles[_type_id];
        vehicle.count +=1;
        uint new_rand;
        ...
        uint margin = vehicles[_type_id].prop_sum - (props[hash_duration] + props[hash_defense] + props[hash_hp]);
        props[keccak256(abi.encode(vehicles[_type_id].margin_name))] +=  margin;
        ...
        ch[0] =  props[hash_defense];
        ch[1] =  props[hash_hp];
        ch[2] =  props[hash_duration];
        ch[3] =  move;
        ch[4] =  capacity;
        ch[5] =  potential;
        ...
        returns(uint[] memory)
    {
        ...
        charactor[0] =  ch.defense;
        charactor[1] =  ch.hp;
        charactor[2] =  ch.duration;
        ...
        requiresRevert(_to, _id, _orderid, signature)
    {
        ...
        ch.capacity  = revertcharacters[_id].capacity;    //最大容量
