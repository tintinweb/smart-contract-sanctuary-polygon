...
contract PositionUniswapV3 is ReentrancyGuard, Ownable, PositionV2Storage {
    ...
    function trade(
        ...
        if (path[0] == WETH9) {
            ...
            require(address(this).balance >= tradeAmount, "INSUFICIENT MATIC AMOUNT");
            result = uniswapV3Router.exactInputSingle{value: tradeAmount}(params);
        }
        else {
            if(source == 1) {
                IMarginFunds(IAdminFunctions(adminContract).getMarginFundsContract()).withdrawERC20Tokens(path[0], address(this), tradeAmount);
            }
            require(IERC20(path[0]).balanceOf(address(this)) >= tradeAmount, "INSUFICIENT AMOUNT");
            IERC20(path[0]).approve(address(uniswapV3Router), tradeAmount);
