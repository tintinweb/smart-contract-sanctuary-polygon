...
//// Used for gas-less meta transactions for transfers
...
contract Kultists is ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(uint256 cost, uint256 max, uint256 maxWhale, uint256 maxWL, address kuthuluContract, address userProfiles, address amuletsContract) initializer public {
    ...
    */
    ...
    // 2 = White List
    // 3 = Open Mint
    function updateDetails(uint256 amuletCost, uint256 max, uint256 _openMint, uint256 _maxReRolls) public onlyAdmins {
    ...
    */
    function useReRoll(address owner, uint256 tokenID) public onlyAdmins whenNotPaused {

        // Ensure they have reRolls to use
        require(reRolls[owner] > 0, "They don't have any re-rolls left");
    ...
    */
    ...
    function KuthuluHook(MsgData memory newMsg) public onlyTrustedContracts returns (bool) {
        ...
        if (newMsg.msgID != 0){
            require(newMsg.taggedAccounts.length == 1, "Too many accounts tagged");
            require(newMsg.taggedAccounts[0] == vaultAddress, "Invalid account paid to");
            require(newMsg.msgStats.tipsReceived == costToMint, "Incorrect mint value sent");
    ...
    * @dev 3 = Open Mint
    */
    function mintKultist() internal whenNotPaused nonReentrant {

        // Check that they have at least one Amulet
        require(NFT.balanceOf(tx.origin) >= 0, "You must own an Amulet to mint a Kultist");
    ...
    function updateWhales(address tokenOwner, uint256 tokensOwned, uint256 tokensOwnedBefore) internal whenNotPaused {
