...
contract Rift is Initializable, RiftContracts {
    ...
    ) external whenNotPaused contractsAreSet {
        require(_saIds.length == _saAmounts.length, "Bad SA lengths");
        
        require(
            amountCurrentlyStaked >= amountNeededToOpenPortal,
            "Not enough GP staked"
        );

        bytes memory _message = _getMessageForChild(
            _gpAmount,
            _wndTokenIds,
            _saIds,
            _saAmounts
    ...
    function _getMessageForChild(
        ...
        if (_saIds.length > 0) {
            sacrificialAlter.adminSafeBatchTransferFrom(
                msg.sender,
                address(this),
                _saIds,
                _saAmounts
    ...
    function handleMessage(bytes calldata _data)
        external
        override
        onlyAdminOrOwner
    {
    ...
    function processMessageFromRoot(
        ...
        if (_saIds.length > 0) {
            ...
            uint256[] memory _balances = sacrificialAlter.balanceOfBatch(
                _addresses,
                _saIds
            );
            ...
            for (uint256 i = 0; i < _saIds.length; i++) {
                ...
                if (_currentBalance < _saAmounts[i]) {
                    sacrificialAlter.mint(
                        _saIds[i],
                        uint16(_saAmounts[i] - _currentBalance),
                        address(this)
                    );
                }
            }
    ...
    function _createWnDIfNeeded(uint256 _tokenId, WizardDragon memory _trait)
        private
    {
...
abstract contract RiftContracts is Initializable, IRift, RiftGP {
...
interface IWnD is IERC721EnumerableUpgradeable {
    ...
    function mintF(address _to, WizardDragon calldata _traits) external;
