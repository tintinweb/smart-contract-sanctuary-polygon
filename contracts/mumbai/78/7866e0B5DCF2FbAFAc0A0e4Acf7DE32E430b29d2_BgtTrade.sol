...
contract BgtTrade is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event Sell(address seller,  uint256 sellerIndex, address buyer, uint256 buyerIndex, uint256 bgtAmount, uint256 usdtReceivedAmount);
    ...
    function cancelBuyOrder(uint256 index) external nonReentrant {
        ...
        require(order.status == BuyStatus.ONGOING && buyer == order.buyer, "BgtOTC: status error");
    ...
    function sell(address buyer, uint256 buyerIndex, uint256 bgtAmount) external nonReentrant{
        ...
        require(order.status == BuyStatus.ONGOING, "BgtOTC: status error");
        require(bgtAmount <= order.bgtBuyAmount - order.bgtBoughtAmount, "BgtOTC: amount error");
        address seller = msg.sender;

        uint256 usdtReceivedAmount = order.usdtPrice*bgtAmount/PRECISION;
        order.bgtBoughtAmount += bgtAmount;
        order.usdtCost += usdtReceivedAmount;
        ...
        IERC20Upgradeable(_bgt).safeTransferFrom(
            seller, 
            buyer, 
            bgtAmount
        );
        ...
        emit Sell(seller, sellerIndex, buyer, buyerIndex, bgtAmount, usdtReceivedAmount);
