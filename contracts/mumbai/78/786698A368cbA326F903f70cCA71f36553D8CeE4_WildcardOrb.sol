...
contract WildcardOrb is ERC721Enumerable, Ownable {
    ...
    // NFT Name
    string public constant TOKEN_NAME = "Wildcard Orb";
    // NFT Symbol
    string public constant TOKEN_SYMBOL = "WO";
    ...
    mapping(uint256 => bool) public nftSoldList;
    ...
    // Status for show/hide minted NFT on BackEnd url
    bool public viewStatus;
    ...
    /**
     *  Emitted when set viewStatus 
     */
    event ViewStatus(bool status);
    ...
    constructor(string memory BASEURI) ERC721(TOKEN_NAME, TOKEN_SYMBOL) {
        ...
        presaleLimit = 5;
        publicSaleLimit = 20;
        presalePrice = 200_000_000_000_000_000_000; // 200 Matic
        publicSalePrice = 300_000_000_000_000_000_000; // 300 Matic
        viewStatus = false;
    }
    ...
    function setPresaleLimit(uint256 counter) external onlyOwner {
        require(counter > 0, "WildcardOrb: Presale amount limit can't be zero.");
    ...
    function setPublicSaleLimit(uint256 counter) external onlyOwner {
        require(counter > 0, "WildcardOrb: Public sale amount limit can't be zero.");
    ...
    function setPresalePrice(uint256 price) external onlyOwner {
        require(price > 0, "WildcardOrb: Presale amount can't be zero.");
    ...
    function setPublicSalePrice(uint256 price) external onlyOwner {
        require(price > 0, "WildcardOrb: Public sale amount can't be zero.");
        ...
        onlyOwner
    {
        for (uint256 i = 0; i < clients.length; i++) {
            require(
                clients[i] != address(0),
                "WildcardOrb: Zero address can't be added to the whitelist."
            );
        }
        for (uint256 i = 0; i < clients.length; i++) {
    ...
    function presaleMint(uint256 amount) internal {
        require(
            ...
            "WildcardOrb: You are not added to the whitelist."
        );
        require(
            presalePrice * amount == msg.value,
            "WildcardOrb: Your presale payment amount does not match required presale minting amount."
        );
        require(
            (amount + _whitelistSold[msg.sender]) <= presaleLimit,
            "WildcardOrb: Your presale payment amount exceeds our presale minting amount limit."
        );
        for (uint256 i = 0; i < amount; i++) {
            while (nftSoldList[_tokenIds.current()] == true) {
                _tokenIds.increment();
            }
            nftSoldList[_tokenIds.current()] = true;
    ...
    function publicSaleMint(uint256 amount) internal {
        require(publicSaleStatus == true, "WildcardOrb: Public sale is not live.");
        require(
            publicSalePrice * amount == msg.value,
            "WildcardOrb: Your public sale payment amount does not match required presale minting amount."
        );
        require(
    ...
    function clientMint(uint256 amount) external payable {
        require(amount > 0, "WildcardOrb: Mint amount can't be zero");
        require(
            (getTotalMintNumber() + amount) <= maxTotalSupply,
            "WildcardOrb: You can't mint that amount of tokens. Exceeds max supply."
        );
    ...
    function adminAirdrop(address client, uint256[] calldata tokenIdArray)
        ...
        onlyOwner
    {   
        require(client != address(0), "WildcardOrb: You can't airdrop to the zero address");
        for (uint256 i = 0; i < tokenIdArray.length; i++) {
            require(
                tokenIdArray[i] < maxTotalSupply,
                "WildcardOrb: You can't airdrop that amount of tokens. Exceeds max supply."
            );
            require(
                nftSoldList[tokenIdArray[i]] == false,
                "WildcardOrb: You can't airdrop token already minted."
            );
        }
        for (uint256 i = 0; i < tokenIdArray.length; i++) {
            nftSoldList[tokenIdArray[i]] = true;
    ...
    function withdrawAdmin(address payable to, uint256 value)
        ...
        onlyOwner
    {
        require(to != address(0), "WildcardOrb: Can't withdraw to the zero address.");
        require(
            value <= address(this).balance,
            "WildcardOrb: Withdraw amount exceed the balance of this contract."
        );
        to.transfer(value);
        emit Withdraw(owner(), to, value);
    }

    function setViewStatus(bool status) external onlyOwner {
        if(viewStatus != status) {
            viewStatus = status;
        }

        emit ViewStatus(status);
    }
}