...
contract TestERC20mint is ERC721A, Ownable { // PaperVerification
    ...
    ) ERC721A("Dems", "DEMS") { // PaperVerification(_paperKey) address _tokenAddress, 
    ...
    //     external
    //     payable
    //     mintCompliant(_mintData.quantity)
    //     onlyPaper(_mintData)
    // {
    //     amountNFTsperWallet[_mintData.recipient] += _mintData.quantity;
    ...
    function claimTo(
        address _recipient,
        uint256 _quantity
    ) external payable mintCompliant(_quantity, _recipient) {
