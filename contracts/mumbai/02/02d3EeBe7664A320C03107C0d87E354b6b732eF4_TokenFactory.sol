...
contract TokenFactory is AccessControl {
    ...
    event tokenCreated(string name, address _tokenProxy, address _tokenImpl, address identityRegistry,address onchainId,
    ...
    function _initProxy(address identityRegistry,string memory _name,string memory _symbol,uint8 _decimals,uint _cap, string memory mappingValue, uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser) internal{
        ...
        require(success, "Compliance Initialization Failed");

        emit tokenCreated(_name, proxy, implAuthorithyToken, identityRegistry, identity, mappingValue, block.timestamp);
    }
