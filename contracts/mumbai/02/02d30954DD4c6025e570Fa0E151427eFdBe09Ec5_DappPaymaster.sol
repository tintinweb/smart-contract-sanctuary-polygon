...
contract DappPaymaster is IPaymaster, UpgradeableACL {
  ...
  modifier onlyOwner() {
    require(isOwner(msg.sender), "ACL: operation allowed only by the owner");
    _;
  }
  ...
  function entryPointInteraction(bytes calldata data) external payable onlyOwner returns (bool) {
    (bool success, bytes memory data) = entryPoint.call{ value: msg.value }(data);
   ...
   */
  function initialize(address _owner, address[] memory _guardians, address _verifyingSigner) external initializer {
  ...
  function setSponsoredContract(address _sponsoredContract) external onlyOwner {
    require(isContract(_sponsoredContract), "Invalid sponsoredContract address");
  ...
  function validatePaymasterUserOp(
    ...
    require(
      verifyingSigner == hash.toEthSignedMessageHash().recover(op.paymasterData),
