...
//
contract InstallmentNFT is Initializable, UUPSUpgradeable, OwnableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable,ERC721Holder {
    ...
    address private constant ADMIN = 0xf478D89FDE97e9719A5eDD0E461dfaBaccfc3F7D;
    ...
    // interest6month = 5 ; //APY 5% | formula : (6/3)+3+(4*0) = 5
    // interest9month = 10 ; //APY 10% | formula : (9/3)+3+(4*1) = 10
    // interest12month = 15 ; //APY 15% | formula : (12/3)+3+(4*2) = 15
    // interest15month = 20 ; //APY 20% | formula : (15/3)+3+(4*3) = 20
    // interest18month = 25 ; //APY 25% | formula : (18/3)+3+(4*4) = 25
    
    bytes32 public getOrderId; //use for uint test
    address public OnftAddress; //use for uint test
    uint256 public OtokenId; //use for uint test
    address public Oseller; //use for uint test
    uint256 public Oprice; //use for uint test

    bytes32 public getBillId; //use for uint test
    address public Buyer;//use for uint test
    address public Seller;//use for uint test
    address public NftAddress;//use for uint test
    uint256 public TokenId;//use for uint test
    uint256 public Price;//use for uint test
    uint256 public Period;//use for uint test
    uint256 public PeriodBalance;//use for uint test
    uint256 public PrePay;//use for uint test
    uint256 public TotalBill;//use for uint test
    uint256 public BillBalance;//use for uint test
    uint256 public PayByperiod;//use for uint test
    ...
    */
     ...
     */
    function createOrderInstallmentNFT (address _nftAddress,uint256 _tokenId, uint256 _price) external whenInstallmentNotPaused returns (Order memory){
        ...
        getOrderId = _orderId; //use for uint test
        OnftAddress = _nftAddress;//use for uint test
        OtokenId = _tokenId; //use for uint test
        Oseller = msg.sender; //use for uint test
        Oprice = _price; //use for uint test
    ...
    function executeOrderInstallmentNFT(address _sellerAccount, bytes32 _orderId , uint256 _period) external whenInstallmentNotPaused{
        ...
        IERC20Upgradeable token = IERC20Upgradeable(nakaTokenAddress);
        ...
        delete orderIdByNFT[order.nftAddress][order.tokenId];
            ...
            getBillId = _billId; //use for uint test
            Buyer = msg.sender;  //use for uint test
            Seller = _sellerAccount;  //use for uint test
            NftAddress = order.nftAddress;  //use for uint test
            TokenId = order.tokenId;  //use for uint test
            Price = order.price;  //use for uint test
            Period = _period;  //use for uint test
            PeriodBalance = periods;  //use for uint test
            PrePay = prepay;  //use for uint test
            TotalBill = totalbill;  //use for uint test
            BillBalance = totalbillbalance;  //use for uint test
            PayByperiod = totalBillPayByperiod;  //use for uint test
    ...
    function payBillInstallmentNFT(bytes32 _billId , uint256 _period) external whenInstallmentNotPaused {
        ...
        if(bill.periodBalance == 0){
            IERC721Upgradeable(bill.nftAddress).safeTransferFrom(
                address(this),
                bill.buyer,
                bill.tokenId
            );
        ...
        billByBillId[msg.sender][_billId] = Bill({
            ...
            getBillId = _billId; //use for uint test
            Buyer = msg.sender;  //use for uint test
            Seller = bill.seller;  //use for uint test
            NftAddress = bill.nftAddress;  //use for uint test
            TokenId = bill.tokenId;  //use for uint test
            Price = bill.price;  //use for uint test
            Period = bill.period;  //use for uint test
            PeriodBalance = bill.periodBalance;  //use for uint test
            PrePay = bill.prePay;  //use for uint test
            TotalBill = bill.totalBill;  //use for uint test
            BillBalance = bill.billBalance;  //use for uint test
            PayByperiod = bill.payByperiod;  //use for uint test
    ...
    function cancelOrderInstallmentNFT(bytes32 _orderId) external  whenInstallmentNotPaused{
        ...
        IERC721Upgradeable(order.nftAddress).safeTransferFrom(
            address(this),
            order.seller,
            order.tokenId
        );
    ...
    */
    ...
    //use for uint test
    function showOrderid () external view  returns(bytes32){
            return getOrderId;
    }
    //use for uint test
    function showbillid () external view  returns(bytes32){
            return getBillId;
    }
     //use for uint test
    function checkOrderNftAddress () external view  returns(address){
       return OnftAddress;
    }
    //use for uint test
    function checkOrderTokenId () external view  returns(uint256){
       return OtokenId;
    }
    //use for uint test
    function checkOrderSeller () external view  returns(address){
       return Oseller;
    } 
     //use for uint test
    function checkOrderPrice () external view  returns(uint256){
       return Oprice;
    }

     //use for uint test
    function checkBillBuyer () external view  returns(address){
    return Buyer;
    }
    //use for uint test
    function checkBillSeller () external view  returns(address){
    return Seller;
    }
    //use for uint test
    function checkBillNftAddress () external view  returns(address){
    return NftAddress;
    }
     //use for uint test
    function checkBillTokenId () external view  returns(uint256){
    return TokenId;
    }
    //use for uint test
    function checkBillPrice () external view  returns(uint256){
    return Price;
    }
     //use for uint test
    function checkBillPeriod () external view  returns(uint256){
    return Period;
    }
     //use for uint test
    function checkBillPeriodBalance () external view  returns(uint256){
    return  PeriodBalance;
    }
     //use for uint test
    function checkBillPrePay () external view  returns(uint256){
    return  PrePay;
    }
    //use for uint test
    function checkBillTotalBill () external view  returns(uint256){
    return  TotalBill;
    }
    //use for uint test
    function checkBillBillBalance () external view  returns(uint256){
    return  BillBalance;
    }
     //use for uint test
    function checkBillBillPayByperiod () external view  returns(uint256){
    return  PayByperiod;
    }
}
