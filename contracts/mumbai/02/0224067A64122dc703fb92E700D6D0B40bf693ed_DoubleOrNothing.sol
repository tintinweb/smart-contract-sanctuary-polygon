...
contract DoubleOrNothing is ERC721TokenReceiver {
    getRandom getrand ;
   constructor(address _getrand) {
       getrand = getRandom(_getrand);
   }
  ...
  function deposit() external payable {
    ...
    function contributionsInfo(address _addr) view external returns( uint256[] memory amounts, uint256[] memory totalWithdraws, uint256[] memory issuccess) {
        ...
        uint256[] memory _totalWithdraws = new uint256[](txHistory.deposits.length);
...
function transactionInfo() view external returns( uint256[] memory amounts, address[] memory useradress, uint256[] memory issuccess) {
