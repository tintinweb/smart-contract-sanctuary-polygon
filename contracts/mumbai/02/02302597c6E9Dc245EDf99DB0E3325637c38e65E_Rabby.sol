...
contract Rabby is Ownable, ERC721A, NonblockingReceiver {
    ILayerZeroEndpoint public layerZeroEndpoint;
    uint256 gasForDestinationLzReceive = 350000;

    // pricing/mintint variables
    uint256 MAX_SUPPLY = 10000;
    mapping(address => uint256) public allowlist;

    constructor(address _endpoint) ERC721A("Cyber Rabby", "RABBY") {
        layerZeroEndpoint = ILayerZeroEndpoint(_endpoint);
    }

    // function initialize(address _endpoint) external initializer {
    //     __Ownable_init();
    //     ERC721A("Cyber Rabby", "RABBY");
    //     layerZeroEndpoint = ILayerZeroEndpoint(_endpoint);
    // }
    ...
    function price(uint8 quantity) external view returns (uint256) {
        // TODO:
        return quantity * 1 * 10**16;
    }

    function mint(uint8 numTokens) external payable {
        // require(price(numTokens) <= msg.value);
        _safeMint(msg.sender, numTokens);
    }

    // This function transfer the nft from source chain to dest chain
    // under the hood it will burn that tokenId and mint new one on dest chain
    function traverseChains(uint16 _destChainId, uint256 tokenId)
        public
        payable
    {
        require(msg.sender == ownerOf(tokenId));
        require(trustedSourceLookup[_destChainId].length > 0);

        // burn NFT from source chain
        _burn(tokenId);

        bytes memory payload = abi.encode(msg.sender, tokenId);

        // encode adapterParams to specify more gas for the dest chain
        uint16 version = 1;
        ...
        // get the fees, send it, and refund the rest
        ...
        endpoint.send{value: msg.value}(
            _destChainId,
            trustedSourceLookup[_destChainId],
            payload,
    ...
    // Receiver, which mean someone traverse to this chain
    function _LzReceive(
        uint16,
        bytes memory,
        uint64,
...
abstract contract NonblockingReceiver is Ownable, ILayerZeroReceiver {
    ...
    function setTrustedSource(uint16 _chainId, bytes calldata _trustedSource)
        external
        onlyOwner
    {
        require(
