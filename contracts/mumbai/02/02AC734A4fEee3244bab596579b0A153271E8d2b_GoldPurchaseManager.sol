...
abstract contract PurchaseManager is AddressAccessor {
    function _validatePurchase(uint256 amount) internal view {
        ...
        IUSDRExchange.MintingStats memory stats = IUSDRExchange(usdrExchange)
            .mintingStats();
        uint256 stableMintedRedeemedThreshold = uint256(
            ITreasury(treasury).purchaseStableMintedRedeemedThreshold()
        );
        uint256 stableMarketcapThreshold = uint256(
            ITreasury(treasury).purchaseStableMarketcapThreshold()
        );
        uint256 totalSTValue = IERC20(underlying).balanceOf(treasury);
        // we can spend 50% of minted - redeemed
        uint256 maxSpendMintedRedeemedValue = _convertToCorrectDecimals(
            (((stats.underlyingToUSDR - stats.usdrToUnderlying) *
                stableMintedRedeemedThreshold) / 100),
        ...
        uint256 percentMarketcapValue = _convertToCorrectDecimals(
            ((IUSDR(usdr).totalSupply() * stableMarketcapThreshold) / 100),
            IERC20Metadata(underlying).decimals(),
            IERC20Metadata(usdr).decimals()
        );
        ...
        uint256 ableToSpend = percentMarketcapValue < totalSTValue
            ...
            : 0;
        ableToSpend = ableToSpend < maxSpendMintedRedeemedValue
            ? ableToSpend
            : maxSpendMintedRedeemedValue;
