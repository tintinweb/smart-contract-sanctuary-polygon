...
library OracleManagerUtils {
  ...
  function getOracleInfoForSystemStateUpdate(
    ...
    uint256 relevantEpochStartTimestampWithMEWT = ((_latestExecutedEpochIndex + 2) * epochLength) + mewt + oracleManager.initialEpochStartTimestamp();
    ...
    while (currentUpdateIndex < _numberOfMissedEpochs) {
      ...
      // TODO: Remove these logs - Left this in for easy debugging for now.
      console.log(previousOracleUpdateTimestamp, relevantEpochStartTimestampWithMEWT, currentOracleUpdateTimestamp, latestExecutedOracleRoundId);
      console.log("difference", currentOracleUpdateTimestamp - previousOracleUpdateTimestamp);
      if (
        previousOracleUpdateTimestamp < relevantEpochStartTimestampWithMEWT &&
        currentOracleUpdateTimestamp >= relevantEpochStartTimestampWithMEWT &&
        currentOracleUpdateTimestamp < relevantEpochStartTimestampWithMEWT + epochLength
      ) {
        console.log("We found an update !", latestExecutedOracleRoundId);
        // Just a little sanity check that the price is correct!
        require(currentOraclePrice > 0, "invalid price");
...
library SimulationTests {
  ...
  function warpOutsideOfMewt(Vm vm, IOracleManager oracleManager) public {
    if (inMewt(oracleManager)) {
      vm.warp(oracleManager.getEpochStartTimestamp() + oracleManager.MINIMUM_EXECUTION_WAIT_THRESHOLD() + 1);
    }
  }

  function warpOneEpochLength(Vm vm, IOracleManager oracleManager) public {
    vm.warp(block.timestamp + oracleManager.EPOCH_LENGTH());
  }

  function mockChainlinkOracleNextPrice(
    Vm vm,
    AggregatorV3Interface chainlinkOracle,
    int256 newPrice
  ) internal {
    (uint80 currentRoundId, , , , ) = chainlinkOracle.latestRoundData();
    // uint80 currentRoundId = 10001;
    ...
    vm.mockCall(
      ...
      abi.encode(newRoundId, newPrice, startedAt, updatedAt, 1)
    );
    vm.mockCall(
      address(chainlinkOracle),
  ...
  function mockChainlinkOraclePercentPriceMovement(
    ...
    mockChainlinkOracleNextPrice(vm, chainlinkOracle, newPrice);
  }

  function mockChainlinkOracleNewPrice(
    Vm vm,
    IOracleManager oracleManager,
    int256 newPrice // e.g. 1e18 is 100%
  ) public {
    AggregatorV3Interface chainlinkOracle = oracleManager.chainlinkOracle();
    mockChainlinkOracleNextPrice(vm, chainlinkOracle, newPrice);
  }
  ...
  function dealPoolToken(
    ...
    IERC20 paymentToken = IERC20(market.get_paymentToken());

    require(amountInPaymentToken > 0 && amountInPaymentToken <= paymentToken.balanceOf(user), "must have enough payment tokens");

    AggregatorV3Interface chainlinkOracle = market.get_oracleManager().chainlinkOracle();

    vm.stopPrank();
    vm.startPrank(user);
    mint(market, poolType, poolIndex, amountInPaymentToken);
    // TODO get chainlink oracle from market
    console2.log("timestapm", block.timestamp);
    warpOutsideOfMewt(vm, market.get_oracleManager());
    mockChainlinkOraclePercentPriceMovement(vm, chainlinkOracle, 0);
    warpOneEpochLength(vm, market.get_oracleManager());
  ...
  function mint(
    ...
    if (poolType == IMarketCommon.PoolType.LONG) {
      console2.log("minting long tokens");
      market.mintLong(poolIndex, amountInPaymentToken);
    } else if (poolType == IMarketCommon.PoolType.FLOAT) {
      console2.log("minting float tokens");
      market.mintFloatPool(amountInPaymentToken);
    } else {
      console2.log("minting short tokens");
      console2.log("amountInPaymentToken", amountInPaymentToken);
      market.mintShort(poolIndex, amountInPaymentToken);
    }
  }
  ...
  function updateSystemBruteForceForTesting(IMarket market) public {
    ...
    for (uint256 i = 0; i < _missedEpochOracleRoundIds.length; i++) {
      console2.log("_missedEpochOracleRoundIds", i, _missedEpochOracleRoundIds[i]);
    }
    // HACK
    ...
    if (_missedEpochOracleRoundIds.length > 0) {
      ...
      for (
        ;
        epochsToSkip < _missedEpochOracleRoundIds.length && epochInfo.latestExecutedOracleRoundId >= _missedEpochOracleRoundIds[epochsToSkip];
        epochsToSkip++
      ) {
        console2.log(
