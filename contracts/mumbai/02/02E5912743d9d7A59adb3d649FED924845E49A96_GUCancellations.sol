// Owner of this software is Globally United FZE. All rights reserved.
...
contract GUCancellations {
    ...
    mapping(uint => mapping(uint => bool)) public cancelledPrices;
    mapping(uint => uint) cancelledPricesCountPerListingId;

    event ListingCancelled(uint indexed id);
    event OfferCancelled(bytes indexed offerId);
    event PriceCancelled(uint indexed id, uint price);

    function cancelAuction(address seller, GUNFTLib.AuctionData calldata auction, bytes calldata auctionSignature, address authorized, GUNFTLib.AuthData calldata auth, bytes calldata authSignature) external
    {
        require(seller == msg.sender, "You are not the seller");
        require(GUNFTLib.verifyAuctionSignature(seller, auction, auctionSignature), "Invalid seller's signature");
        require(GUNFTLib.verifyAuthSignature(authorized, auth, authSignature), "Invalid auth signature");
        cancelledListings[auction.listingId] = true;
        emit ListingCancelled(auction.listingId);
    }

    function cancelSale(address seller, GUNFTLib.SaleData calldata sale, bytes calldata saleSignature, address authorized, GUNFTLib.AuthData calldata auth, bytes calldata authSignature) external
    {
        require(seller == msg.sender, "You are not the seller");
        require(GUNFTLib.verifySaleSignature(seller,sale,saleSignature), "Invalid seller's signature");
        require(GUNFTLib.verifyAuthSignature(authorized, auth, authSignature), "Invalid auth signature");
        cancelledListings[sale.listingId] = true;
        emit ListingCancelled(sale.listingId);
    }

    function cancelOffer(address offerer, GUNFTLib.OfferData calldata offer, bytes calldata offerSignature, address authorized, bytes calldata authSignature) external
    {
        require(offerer == msg.sender, "You are not the offerer");
        require(!cancelledBidsAndOffers[offerSignature], "Offer already cancelled");
        require(GUNFTLib.verifyOfferSignature(offerer,offer,offerSignature), "Invalid offerer's signature");
        require(GUNFTLib.verifyAuthBidOfferSignature(authorized, offerSignature, authSignature), "Invalid auth signature");
        cancelledBidsAndOffers[offerSignature] = true;
        emit OfferCancelled(offerSignature);
    }

    function cancelSalePrices(address seller, GUNFTLib.SaleData[] calldata sale, bytes[] calldata saleSignature, address[] calldata authorized, GUNFTLib.AuthData[] calldata auth, bytes[] calldata authSignature) external {
        require(seller == msg.sender, "You are not the seller");
        
        for (uint i=0; i<sale.length; i++)
        {
            require(GUNFTLib.verifySaleSignature(seller, sale[i], saleSignature[i]), "Invalid seller's signature");
            require(GUNFTLib.verifyAuthSignature(authorized[i], auth[i], authSignature[i]), "Invalid auth signature");
            require(!(cancelledPrices[sale[i].listingId][sale[i].price]), "Price already cancelled");
            cancelledPrices[sale[i].listingId][sale[i].price] = true;

            emit PriceCancelled(sale[i].listingId, sale[i].price);
        }
    }

    function cancelAuctionPrices(address seller, GUNFTLib.AuctionData[] calldata auction, bytes[] calldata saleSignature, address[] calldata authorized, GUNFTLib.AuthData[] calldata auth, bytes[] calldata authSignature) external {
        require(seller == msg.sender, "You are not the seller");
        
        for (uint i=0; i<auction.length; i++)
        {
            require(GUNFTLib.verifyAuctionSignature(seller, auction[i], saleSignature[i]), "Invalid seller's signature");
            require(GUNFTLib.verifyAuthSignature(authorized[i], auth[i], authSignature[i]), "Invalid auth signature");
            require(!(cancelledPrices[auction[i].listingId][auction[i].startPrice]), "Price already cancelled");
            cancelledPrices[auction[i].listingId][auction[i].startPrice] = true;

            emit PriceCancelled(auction[i].listingId, auction[i].startPrice);
        }
    }

    function cancelBid(address bidder, GUNFTLib.BidData calldata bid, bytes calldata bidSignature, address authorized, bytes calldata authSignature) external
    {
        require(bidder == msg.sender, "You are not the offerer");
        require(!cancelledBidsAndOffers[bidSignature], "Offer already cancelled");
        require(GUNFTLib.verifyBidSignature(bidder, bid, bidSignature), "Invalid offerer's signature");
        require(GUNFTLib.verifyAuthBidOfferSignature(authorized, bidSignature, authSignature), "Invalid auth signature");
...
import "./GUManager.sol";
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";

library GUNFTLib {

    function checkFinalizeAuctionPreconditions(address cancellationsAddress, GUManager manager, FinalizeAuction calldata data, GUCollections collections) internal {
        ...
        require(!(GUCancellations(cancellationsAddress).cancelledPrices(data.auction.listingId, data.auction.startPrice)), "Price cancelled");
        ...
        require(!(GUCancellations(cancellationsAddress).cancelledBidsAndOffers(data.bidSignature)),"Bid cancelled");
        ...
        collections.setFinalizedListings(data.auction.listingId);
    }

    function checkPurchasePreconditions(address cancellationsAddress, GUManager manager, FinalizePurchase calldata data, GUCollections collections) internal
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        require(!(GUCancellations(cancellationsAddress).cancelledListings(data.sale.listingId)),"Listing cancelled");
        require(!(GUCancellations(cancellationsAddress).cancelledPrices(data.sale.listingId, data.sale.price)), "Price cancelled");
        ...
        collections.setFinalizedListings(data.sale.listingId);
    }

    function checkFinalizeOfferPreconditions(address cancellationsAddress, GUManager manager, FinalizeOffer calldata data, GUCollections collections) internal
    {
        data.offer.sellersOffer ? require(data.seller == data.offeror, "Unauthorized") : require(data.seller == msg.sender, "Unauthorized");
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        require(!(GUCancellations(cancellationsAddress).cancelledListings(data.sale.listingId)), "Listing cancelled");
    ...
    function checkMintToPreconditions(GUCollections collections, GUManager manager, MintTo calldata data) internal view
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        require(data.nftData.creator == msg.sender, "Unauthorized");
        require(data.nftData.royalty <= manager.maxRoyaltyFee(), "Royalty too high");
        require(checkForSupplyOverflow(collections, data.nftData.nftId, data.nftData.supply, data.amount), "Supply overflow");
        require(verifyNftSignature(data.authorized, data.nftData, data.nftSignature), "Invalid mint data / signature provided");
    }
    ...
    {
        bytes memory toHash = abi.encodePacked(offer.initialOfferor, offer.to, offer.listingId, offer.amount, offer.expiry, offer.sellersOffer);
        bytes32 _hash = keccak256(toHash);
        return recoverSigner(getEthSignedMessageHash(_hash), signature) == signer;
    }
    ...
    function verifyPurchaseSignatures(FinalizePurchase calldata data) internal pure {
    ...
    function verifyFinalizeAuctionSignatures(FinalizeAuction calldata data) internal pure 
    {
        require(verifyNftSignature(data.nftSigner, data.nftData, data.nftSignature), "Invalid nft data / signature");
    ...
    function verifyFinalizeOfferSignatures(FinalizeOffer calldata data) internal pure
    {
        require(verifyNftSignature(data.nftSigner, data.nftData, data.nftSignature), "Invalid nft data / signature");
        require(verifySaleSignature(data.seller, data.sale, data.saleSignature), "Invalid Sale data / seller signature provided");
        require(verifyAuthSignature(data.authorized, data.auth, data.authSignature), "Invalid auth signature");        
        require(verifyOfferSignature(data.offeror, data.offer, data.offerSignature), "Invalid Offer data / offeror signature provided");
    ...
    function transferOrMintERC1155(NFTData calldata nftData, TransferERC1155Data memory transferData) internal
    {   
    ...
    function transferERC20(NFTData calldata nftData, address tokenAddress, address sender, address seller, address beneficiary, uint amount, uint fee) internal
    {
...
contract GUCollections is ERC1155Supply, ERC1155URIStorage {
    ...
    mapping(uint256 => bytes) public nftSignatures;
    mapping(uint256 => bool) public finalizedListings;
    address public adminSafe;
    ...
    event Mint(uint256 indexed id, address indexed to, uint256 amount);
    ...
    function setFinalizedListings(uint256 listingId) public onlyTraders {
        require(!finalizedListings[listingId], "Listing already finalized");
        finalizedListings[listingId] = true;
    }

    function uri(uint256 tokenId) public view override(ERC1155URIStorage, ERC1155) returns (string memory)
    {
        return bytes(_tokenURIs[tokenId]).length > 0 ? string(abi.encodePacked(_baseURI, _tokenURIs[tokenId], "/metadata.json")) : super.uri(tokenId);
    }
    ...
    function mint(address to, uint256 id, uint256 amount, bytes calldata signature, string calldata cid) external onlyTraders {
        if (totalSupply(id) != 0)
            require(keccak256(signature) == keccak256(nftSignatures[id]),"Nft signature tampered");
        else {
            _setURI(id, cid);
