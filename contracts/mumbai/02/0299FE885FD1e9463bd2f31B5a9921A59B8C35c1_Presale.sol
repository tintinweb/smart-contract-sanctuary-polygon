...
contract Presale is OwnableUpgradeable {
    ...
    address public ssloth;
    address public exchangeToken; // Default is USDC
    address public weth;
    /// ETH convert to USDC using Uniswap
    /// Example: 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff (UniswapV2Router02 Polygon mainnet)
    address private uniswapRouter;

    address[] public stableCoins; // Support stable coin list: USDC, USDT, FRAX, etc
    address[] private buyers;

    uint256 public price; // USD price 1$ = 100000000
    uint256 public usd; // 1e8 - usd decimals is 8
    uint256 public totalClaimableCount; // default is 6
    ...
    function changeStableCoins(address[] memory _stableCoins) external onlyOwner {
        stableCoins = _stableCoins;
    }

    function changeClaimableCount(uint256 count) external onlyOwner {
        totalClaimableCount = count;
    }

    function changeExchangeToken(address token) external onlyOwner {
    ...
    function setPause(bool _paused) external onlyOwner {
        require(_paused != paused, "Already set");
        paused = _paused;
    }

    function startVesting() external isPaused onlyOwner {
        require(vestingStartTime == 0, "Already started vesting");
    ...
    function purchaseWithStableCoin(address token, uint256 amount) external isPaused {
        require(checkSupportToken(token), "Unsupported token");
        require(amount > 0, "Insufficient amount");
    ...
    function airdrop() external {
        require(vestingStartTime != 0, "Not started vesting");
        uint256 leftClaimableCount = totalClaimableCount - claimedCount;
        require(leftClaimableCount > 0, "Already all claimed");
        // Round is 1 month
        uint256 count = (block.timestamp - vestingStartTime) / (86400 * 30);
        require(count > 0, "Already claimed for this month");

        // At the starting of vesting, the first claim takes place. So + 1
        uint256 claimableCount = count - claimedCount + 1;
        if (claimableCount > leftClaimableCount) claimableCount = leftClaimableCount;

        _airdrop(claimableCount);
    }

    function checkSupportToken(address token) public view returns (bool) {
        for (uint256 i = 0; i < stableCoins.length; i++) {
    ...
    function getAmountsOutWithETH(uint256 amountIn) external view returns (uint256) {
        ...
        return getTokenAmount(exchangeToken, amountsOut[amountsOut.length - 1]);
    }

    function getAmountsOutWithStableCoin(address token, uint256 amountIn) external view returns (uint256) {
        require(checkSupportToken(token), "Unsupported token");
        return getTokenAmount(token, amountIn);
    }
    ...
    function getClaimableAmountOf(address user) external view returns (uint256) {
        ...
        uint256 claimableCount = (block.timestamp - vestingStartTime) / (86400 * 30);
        if (claimableCount == 0) return 0;
        if (claimableCount > leftClaimableCount) claimableCount = leftClaimableCount;

        return purchasedAmount[user] / totalClaimableCount * claimableCount;
    }
    ...
    function getTokenAmount(address token, uint256 amount) internal view returns (uint256) {
    ...
    function _purchase(address token, uint256 amount) internal {
        require(vestingStartTime == 0, "Not started vesting");
        uint256 outputAmount = getTokenAmount(token, amount);
        require(outputAmount > 0, "Insufficient amount");
    ...
    function _airdrop(uint256 count) internal {
        for (uint256 i = 0; i < buyers.length; i++) {
            ...
            IERC20Upgradeable(ssloth).safeTransfer(buyer, purchasedAmount[buyer] / totalClaimableCount * count);
        }
        claimedCount += count;
