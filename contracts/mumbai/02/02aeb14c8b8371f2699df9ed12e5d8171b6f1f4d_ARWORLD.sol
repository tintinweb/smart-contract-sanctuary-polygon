    ...
    Ownable
{
    ...
    mapping(uint256 => bool) public tokensWithMeta;
    uint256 public batchLimit = 20;
    
    string private _baseUri;
    string private _baseTokenURI;
    string private _contractURI;
    bool private isSetBaseURI;
    ...
    ) ERC721("ARWORLD NFT", "ARWNFT") {
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(MINTER_ROLE, _msgSender());
        _setupRole(DEPOSITOR_ROLE, childChainManager);
        _initializeEIP712("ARWORLD NFT");
        _baseTokenURI = "https://arworld.io/api/land/";
        _contractURI = "https://arworld.io/contract/";
    }


    // privileged public functions
    function setProxyRegistry(address proxyRegistry) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ARWORLD: must have admin role to set");
        require(proxyRegistry != address(0), "ARWORLD: null address for proxy");
        proxyRegistryAddress = proxyRegistry;      
    }

    function setBatchLimit(uint256 limit) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ARWORLD: must have admin role to set");
        require(limit > 0, "ARWORLD: null batch limit");
        batchLimit = limit;
    }
    ...
    function setBaseURI(string memory baseURI) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ARWORLD: must have admin role to set");
        require(!isSetBaseURI);
        require(bytes(baseURI).length > 0, "ARWORLD: empty baseURI");
        isSetBaseURI = true;
        _baseUri = baseURI;
    }

   // transfer to IPFS
    function setTokenMeta(uint256 tokenId) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "ARWORLD: must have minter role to set");
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory baseURI = _baseURI();
        require(bytes(baseURI).length > 0, "ARWORLD: empty baseURI");
        tokensWithMeta[tokenId] = true;
    }

    function setBaseTokenURI(string memory baseTokenURI_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ARWORLD: must have admin role to set");
        require(bytes(baseTokenURI_).length > 0, "ARWORLD: empty baseTokenURI");
        _baseTokenURI = baseTokenURI_;
    }

    function setContractURI(string memory contractURI_) public {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), "ARWORLD: must have admin role to set");
        require(bytes(contractURI_).length > 0, "ARWORLD: empty contractURI");
        _contractURI = contractURI_;
    }

    function mint(address user, uint256 tokenId) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "ARWORLD: must have minter role to mint");
        require(!withdrawnTokens[tokenId], "ChildMintableERC721: TOKEN_EXISTS_ON_ROOT_CHAIN");
        _mint(user, tokenId);
    }

    function mintBatch(address[] calldata users, uint256[] calldata tokenIds) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "ARWORLD: must have minter role to mint");
        uint256 length = tokenIds.length;
        require(length <= batchLimit, "ChildMintableERC721: EXCEEDS_BATCH_LIMIT");
        require(users.length == tokenIds.length,"ERROR: wrong length");
