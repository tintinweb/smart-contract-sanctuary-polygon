...
contract FaucetTokenFactory {
    ...
    function create(string memory name_, string memory symbol_, uint8 decimals_) public returns (address) {
        FaucetToken faucetToken = new FaucetToken();
        faucetToken.initialize(name_, symbol_, decimals_);
        faucetToken.mint(msg.sender, 1000 * 10 ** decimals_);
        tokens.push(address(faucetToken));
