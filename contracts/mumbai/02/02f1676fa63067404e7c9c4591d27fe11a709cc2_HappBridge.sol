...
contract HappBridge is ERC20, Ownable {
    ...
    event CompleteOrder(string indexed orderId, address indexed sender, address destinationCoin, uint256 amount);
    ...
    function bridgeEthToBtc(string memory btcAddress,address destinationCoin, uint256 amount) public {
        ...
        string memory orderId = bytes32ToString(keccak256(abi.encodePacked(msg.sender, destinationCoin, amount, block.timestamp)));
    ...
    function bridgeBtcToEth() public view returns (string memory) {
        return btc_bridge_address;
    }

    function withdrawTokenToUser(address tokenAddress, address recipient, uint256 amount) public onlyOwner {
    ...
    function getMyOrder(string memory orderId) public view returns (Order memory) {
        return _orders[orderId];
    }

    function completeOrder(string memory orderId) public onlyOwner {
    ...
    function setAccountBTCAddress(string memory btcAddress) public {
        require(bytes(btcAddress).length > 0, "BTC address can't be empty");
        _btcAddresses[msg.sender] = btcAddress;
        _btcToAddress[btcAddress] = msg.sender;
    }

    function getMyAddressByBTC(string memory btcAddress) public view returns (address) {
        return _btcToAddress[btcAddress];
    }

    function getMyBTCByAddress(string memory btcAddress) public view returns (address) {
        return _btcToAddress[btcAddress];
    }

    function setBtcBridgeAddress(string memory btcAddress) public onlyOwner {
