...
/**
 *Suns Of DeFi: IBN5X - Prof. Kalito - Mr. Grey 
*/
...
contract gameToken is ERC20 {
    ...
    modifier hasNoStake
        {
            require(hasStaked[msg.sender] == false, "You have initialized a staked already");
            _;
        } 

    modifier hasStake
        {
            require(hasStaked[msg.sender] == true, "You have to initialize a stake, before you can increase");
            _;
        } 
    ...
    mapping(address => bool) gameContract;
    ...
    function Minter(address account, uint256 amount) public nonReentrant returns(bool){
    ...
    //Burning
    function burn(uint256 amount) public nonReentrant returns(bool success){
        require(balanceOf(msg.sender) >= amount, "You dont have this amount of $tLZDG");
    ...
    //Staking & increasing stake
    function stake(uint _amount, uint _time) public hasNoStake nonReentrant returns(bool success){
        require(balanceOf(msg.sender) >= _amount, "You don't have enough tokens for this stake");
    ...
    function increaseStake(uint _amount) public hasStake nonReentrant returns(bool success){
        require(balanceOf(msg.sender) >= _amount, "You don't have enough tokens to increase stake");
        ...
        emit stakeIncrease(msg.sender, _amount, block.timestamp);
    ...
    //unstaking
    function unstake(uint _amount) public nonReentrant returns(bool success) {
       require(_amount <= lockedBalance[msg.sender], "You dont have this amount of $tLZDG staked");
       require(block.timestamp >= unlockDate[msg.sender], "Your $tLZDG stake still has time remaining before it can be unstaked");
    ...
    function unstakingDate() public view returns(uint){
    ...
    function setGame(address _address) public onlyOwner returns(address){
             game = _address;

             return game;

    }
    function approveGame(bool _status) public onlyOwner returns(bool){
        gameContract[game] = _status;

        return gameContract[game];
    }
