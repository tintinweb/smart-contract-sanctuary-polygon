...
contract P2PLending is Context, ReentrancyGuard, Pausable {
   ...
   //State Variables
    ...
    constructor(address _TokenLocker, address _WETH, address _Marketplace){
   ...
   //User functions 
    function openContract(address _collection, uint[] calldata _tokens, uint _amountToBorrow, uint8 _expiryInDays, uint16 _interestToPay) external whenNotPaused nonReentrant canOpenContract(_amountToBorrow, _expiryInDays, _interestToPay) {
        require(!Marketplace.isDistantCollection(_collection));
    ...
    function repayLoan(bytes32 _id) external nonReentrant canBeRepaid(_id) {
        ...
        uint repayableInterest = _updateRevenue(_loanContract.collection, interest, protocolFee);
    ...
    function _updateRevenue(address _collection, uint _amount, uint _tradeFee) private returns(uint) {
        (
            uint256 amount,
            uint256 marketplaceFee,
            uint256 collectionFee,
            uint256 tokenFee
        ) = Marketplace.calculateFee(_collection, 0, _amount, _tradeFee);
        uint256 totalFees = marketplaceFee + collectionFee + tokenFee;
        WETH.safeTransfer(address(Marketplace), totalFees);
    ...
    function liquidate(bytes32 _id) external nonReentrant canBeLiquidated(_id){
        ...
        uint liquidationRefund = _updateRevenue(_loanContract.collection, _securityFee, protocolFee);
        bytes32 _lockId = _loanContract.lockId;
   ...
   //Read functions 
    ...
    function getLoanData(bytes32 _id) external view returns(address,address,uint,uint,uint,bytes32){
        LoanContract storage _loanContract = borrowContract[_id];
   ...
   //Admin functions
    function updateFee(uint16 _securityFee, uint16 _protocolFee) external isAdmin {
        require(_securityFee >= 1 && _securityFee <= 10, "security fee params are out of bound");
        require(_protocolFee >= 1 && _protocolFee <= 30, "protocol fee params are out of bound");
    ...
    function updateInterestAndMinBorrowAmount(uint16 _minInterest, uint _minBorrowAmount) external isAdmin {
        require(_minInterest >= 1 && _minInterest <= aprDivider, "minimum interest params are out of bounds");
    ...
    function updateMarketplace(address _Marketplace) external isAdmin {
        require(_Marketplace != address(0), "Address 0 not allowed");
        Marketplace = IMarketplace(_Marketplace);
    }
