 ...
 */
contract DStorageERC721Random is DStorageERC721Fixed, VRFConsumerBase {
    using Strings for uint256;

    // events
    event UriHiddenUpdated(string previous, string updated);
    event UriHiddenFallbackUpdated(string previous, string updated);
    ...
    string public uriHiddenFallback;
     ...
     * @param uriHidden_ defaut token uri for an unrevealed token
     * @param vrfCoordinator_ contract address for vrf coordinator
     * @param vrfLink_ address for LINK token
     * @param vrfKeyHash_ vrf hash key
     * @param vrfFee_ vrf fee
     */
    constructor(
        string memory name_,
        string memory symbol_,
        string memory uri_,
        string memory uriHidden_,
        address vrfCoordinator_,
        address vrfLink_,
        bytes32 vrfKeyHash_,
        uint256 vrfFee_
    )
        DStorageERC721Fixed(name_, symbol_, uri_)
        VRFConsumerBase(vrfCoordinator_, vrfLink_)
    {
        batch = 1;
        uriHidden = uriHidden_;
        uriHiddenFallback = uriHidden_;
        pack = address(0);
     ...
     */
    function mintOwner(uint256 amount) external override nonReentrant {
        require(
     ...
     * @param amount number of tokens to mint
     */
    function mint(uint256 amount) external payable override publicMintable {
        require(
            amount <= batch,
            "DStorageERC721Random: exceeds max batch size"
        );
        ...
        returns (string memory)
    {
        ...
        if (order[tokenId] == 0 || revealed < order[tokenId]) {
            return uriHidden;
        }
        return
            bytes(uri).length > 0
                ? string(
                    abi.encodePacked(uri, (shuffle[order[tokenId]]).toString())
                )
                : "";
    }
        ...
        return
            bytes(uriFallback).length > 0
                ? string(
                    abi.encodePacked(
                        uriFallback,
                        (shuffle[order[tokenId]]).toString()
                    )
                )
                : "";
    }
    ...
    /**
     * @notice set the hidden token URI for the contract
     * @param uri_ new hidden token URI
     */
    function setURIHidden(string calldata uri_) external onlyOwner {
        emit UriHiddenUpdated(uriHidden, uri_);
        uriHidden = uri_;
    }

    /**
     * @notice set the hidden token fallback URI for the contract
     * @param uri_ new hidden token fallback URI
     */
    function setURIHiddenFallback(string calldata uri_) external onlyOwner {
        emit UriHiddenFallbackUpdated(uriHidden, uri_);
        uriHiddenFallback = uri_;
    }
    ...
    /**
     * @notice set the max token supply
     * @param max_ new max tokens
     */
    function setMax(uint256 max_) external override onlyOwner {
        require(
            max_ >= total,
            "DStorageERC721Random: new max below total supply"
        );
        require(
            revealed == 0,
            "DStorageERC721Random: reveal has already started"
        );
        emit MaxUpdated(max, max_);
        max = max_;
    }
 ...
 */
contract DStorageERC721 is ERC721, IERC2981, Ownable, ReentrancyGuard {
    using Strings for uint256;

    // events
    event UriUpdated(string previous, string updated);
    event UriFallbackUpdated(string previous, string updated);
    event MaxUpdated(uint256 previous, uint256 updated);
     ...
     */
    function setURIFallback(string calldata uri_) external onlyOwner {
     ...
     * @param max_ new max tokens
     */
    function setMax(uint256 max_) external virtual onlyOwner {
        require(max_ >= total, "DStorageERC721: new max below total supply");
        emit MaxUpdated(max, max_);
        max = max_;
    }
     ...
     * @param receiver_ new royalty receiver
     */
...
contract DStorageERC721Fixed is DStorageERC721 {
    ...
    constructor(
        ...
        string memory symbol_,
        string memory uri_
    ) DStorageERC721(name_, symbol_, uri_) {
        batch = 1;
    }
    ...
    /**
     * @notice set the token mint price
     * @param price_ new mint price
     */
    ...
    /**
     * @notice set the max token mint batch size
     * @param batch_ new max batch size
     */
    function setBatch(uint256 batch_) external onlyOwner {
        emit BatchUpdated(batch, batch_);
        batch = batch_;
    }
}
