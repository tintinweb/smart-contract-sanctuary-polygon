...
abstract contract Event {
    event TransferExecuted(bytes _id);
...
contract Swap2Cash is Setting, Event {
    ...
    function buy(
        ...
        require(isValid(transfer, signature), "Swap2Cash: Invalid transfer");
    ...
    function withdraw(address token) public onlyOwner {
        uint256 amount = _withdraw(token, msg.sender);
    ...
    function isValidSignature(
        ...
        Transfer memory transfer = Transfer(from, to, token, amount, _id);
        return isValid(transfer, signature);
    }
}