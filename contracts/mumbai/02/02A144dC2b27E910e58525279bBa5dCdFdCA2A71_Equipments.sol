...
library CraftingRecipes {
    ...
    function getMainMaterial(uint256 item_type) internal pure returns (uint256 material, uint256 amount){
        if(item_type == 0){material = 0; amount = 12;} //WEAPONS: PHANTOMSTEEL
        if(item_type == 1){material = 1; amount = 12;} //ARMORS: IRONFLEECE
        if(item_type == 2){material = 1; amount = 6;} //HELMS: IRONFLEECE
        if(item_type == 3){material = 2; amount = 3;} //ACCESSORIES: SILVERGLASS
        if(item_type == 4){material = 3; amount = 1;} //CONSUMABLES: AETHERLINK
    }

    function getIndirectMaterial(uint256 item_type) internal pure returns (uint256 material, uint256 amount){
        if(item_type == 0){material = 1; amount = 4;} //WEAPONS: IRONFLEECE
        if(item_type == 1){material = 0; amount = 4;} //ARMORS: PHANTOMSTEEL
        if(item_type == 2){material = 0; amount = 2;} //HELMS: PHANTOMSTEEL
        if(item_type == 3){material = 0; amount = 1;} //ACCESSORIES: PHANTOMSTEEL
        if(item_type == 4){material = 2; amount = 1;} //CONSUMABLES: SILVERGLASS
    }

    function getCatalyst(uint256 item_type) internal pure returns (uint256 catalyst, uint256 amount){
        if(item_type == 0){catalyst = 0; amount = 4;} //WEAPONS: BLACK SPARKSTONE
        if(item_type == 1){catalyst = 1; amount = 4;} //ARMORS: WHITE SPARKSTONE
        if(item_type == 2){catalyst = 1; amount = 2;} //HELMS: WHITE SPARKSTONE
        if(item_type == 3){catalyst = 2; amount = 1;} //ACCESSORIES: RED SPARKSTONE
        if(item_type == 4){catalyst = 3; amount = 1;} //CONSUMABLES: BLUE SPARKSTONE
    }
}
