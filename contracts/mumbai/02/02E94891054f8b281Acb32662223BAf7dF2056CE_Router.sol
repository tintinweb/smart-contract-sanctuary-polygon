    ...
    ISwapPoolPermissioned
{
        ...
        returns (uint256 _finalAmount, int256 _fee)
    {
        ...
        accumulatedSlippage = (accumulatedSlippage.toInt256() + _fee).toUint256();
        totalLiabilities -= _amountBeforeFees;
    ...
    function _depositFee(uint256 _depositAmount)
        ...
        returns (int256 _fee)
    {
        uint256 _balance = poolAsset.balanceOf(address(this));

        if (_balance >= totalLiabilities) {
            // Coverage ratio >= 100%
            // Keep in mind that deposits & withdrawals change the coverage ratio,
            // but they never cross the 100% mark to the other side
            _fee = 0;
        } else {
            // Coverage ratio < 100%
        ...
        returns (uint256 _effectiveAmount, int256 _slippage)
    {
        ...
        if (_balance - _amount >= totalLiabilities) {
            // Coverage ratio >= 100% before and after the swap
            _effectiveAmount = _amount;
            _slippage = 0;
        } else if (_balance > totalLiabilities) {
            uint256 _slippageFreeAmount = _balance - totalLiabilities;
            // Coverage ratio > 100% before the swap, but ends up < 100%
            _effectiveAmount = slippageCurve.effectiveSwapOut(
                totalLiabilities,
                totalLiabilities,
                0,
                _amount - _slippageFreeAmount
            );
            _slippage = (_amount - _slippageFreeAmount).toInt256() - _effectiveAmount.toInt256();
        } else {
            // Coverage ratio < 100%
            _effectiveAmount = slippageCurve.effectiveSwapOut(
                _balance,
...
abstract contract GenericPool is ERC20, IGenericPoolPermissioned {
        ...
        returns (uint256 _poolShares)
    {
        ...
        _processDeposit(_user, _amount, _poolShares);
    }
    ...
    IBackstopPool
{
        ...
        returns (uint256 _swapAmount)
    {
        ...
        address _swapToken = ISwapPool(_swapPool).asset();
        uint256 _backstopAmount = getTotalPoolWorth() * _shares / totalSupply();
    ...
    function coverage()
        ...
        returns (uint256 _reserves, uint256 _liabilities)
    {
        _liabilities = getTotalPoolWorth();
        _reserves = poolAsset.balanceOf(address(this));
    }
    ...
    function sharesTargetWorth(uint256 _shares)
        ...
        returns (uint256 _amount)
    {
        _amount = _shares * getTotalPoolWorth() / totalSupply();
    }
    ...
    function _depositFee(uint256 _depositAmount)
        ...
        returns (int256 _fee)
    {
        uint _reserves = poolAsset.balanceOf(address(this));
        uint _totalWorth = getTotalPoolWorth();

        if (_reserves < _totalWorth) {
            // Coverage ratio < 100%
            _fee = _depositAmount.toInt256() - slippageCurve.effectiveDeposit(
                _reserves,
                _totalWorth,
