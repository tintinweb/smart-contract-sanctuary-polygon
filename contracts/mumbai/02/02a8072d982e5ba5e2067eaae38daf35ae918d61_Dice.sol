...
contract Dice is ReentrancyGuard {
  ...
  event DiceRolled(uint256 indexed _betId, uint256 indexed _requestId, uint256 indexed _randomValue);
...
function placedBet(uint8 _sliderValue, bool _isRollOver) internal {
  ...
  */
  ...
  function _checkWinner(uint256 betId) internal {
    ...
    uint256 _randomNumber = ((uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,msg.sender,betId,_multiplier)))+ 1) % 100) + 1;
