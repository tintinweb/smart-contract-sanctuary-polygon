    ...
    UUPSUpgradeable
{
    ...
    mapping(uint8 => FeeCollector) transactionTypes;
    struct FeeCollector {
    ...
    ) internal override onlyOwnerAndAdmin {}
    ...
    function setAdvisorsAndPartnersWallet(
        address wallet
    ) external onlyOwnerAndAdmin {
        require(
            wallet != address(0),
    ...
    function setAllTeamWalletAddress(
        address _teamWallet,
        address _developmentWallet,
        address _acquisitionsWallet,
        address _advisorsAndPartnersWallet,
        address _preSeedAddress,
        ...
        require(
            _teamWallet != address(0) &&
                ...
                _acquisitionsWallet != address(0) &&
                _advisorsAndPartnersWallet != address(0) &&
                _preSeedAddress != address(0) &&
                _seedAddress != address(0) &&
                _privateSaleAddress != address(0) &&
                _publicSaleAddress != address(0) &&
    ...
    function setMaxTxAmountPercentage(
        uint256 _maxAmountTxPercentage
    ) external onlyOwner {
    ...
    function setFeeCollector(
        ...
        transactionTypes[transactionType] = FeeCollector(
            transactionName,
            taxPercentage,
    ...
    function processPayment(
        ...
        FeeCollector storage _feeCollector = transactionTypes[transactionType];
        uint256 taxAmount = 0;

        taxAmount = amount.mul(_feeCollector.feePercentage).div(100);
        if (taxAmount == 0) {
            transfer(to, amount);
        } else {
            uint256 balance = amount.sub(taxAmount);
            transfer(_feeCollector.feeWallet, taxAmount);
            transfer(to, balance);
        }

        emit ProcessPaymet(to, amount, transactionType, data);
    }
}