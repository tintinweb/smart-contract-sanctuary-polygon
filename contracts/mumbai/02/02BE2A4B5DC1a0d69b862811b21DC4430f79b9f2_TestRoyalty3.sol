...
contract TestRoyalty3 is ERC721Royalty, Ownable {
    ...
    uint256 public price = 1 ether; // 0.01 ETH

    uint96 public royaltyPer = 1500;

    bool public _saleStatus;
    
    mapping(address => uint) internal _maxnftcount;
    ...
    function mintNFT(uint numberOfTokens) public payable saleFlip {
        ...
        uint a =  _maxnftcount[msg.sender];
        require(a.add(numberOfTokens) <= 5, "Per User 5 NFT is allowed");
       _maxnftcount[msg.sender] = a + numberOfTokens;
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            if (totalSupply() <= MAX_QTY) {
                _safeMint(msg.sender, mintIndex+1);
                _setTokenRoyalty(mintIndex, msg.sender, royaltyPer);
            }
        }
    }
