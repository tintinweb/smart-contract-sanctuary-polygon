...
contract TestNFTContract is ViciERC721 {
    constructor() ViciERC721(
        "Test Sample NFT Contract",
        "tsNFT",
        0x100000000,
        IERC2981(address(0x4d0240b0018c401123b446590E139FCAE1b8A37b))
    ) {
    	setContractURI("ipfs://QmVSxGiiFL7pNUfzcmbgyZfs9nLwtzR5BT8trRGD9pDw9m");
    }
}
...
contract Enumerator is Ownable {
     ...
     * Calling conditions:
     *
     * - When `_fromAddress` and `_toAddress` are both non-zero, ``_fromAddress``'s `_tokenId` will be
     * transferred to `_toAddress`.
     * - When `_fromAddress` is zero, `_tokenId` will be minted for `_toAddress`.
     * - When `_toAddress` is zero, ``_fromAddress``'s `_tokenId` will be burned.
    ...
    ContextMixin
{
    ...
    IUriManager public uriManager = new UriManager();

    Enumerator internal enumerator = new Enumerator();
    ...
    /**
     * @notice Overwrites the default URI manager.
     * @param _uriManager address of the uri manager contract.
     *
     * Requirements:
     *
     * - Caller MUST be the contract owner
     * - `_uriManager` MUST  be a contract that supports the IUriManager interface
     * - This function SHOULD NOT be called more than once.
     */
    function setUriManager(UriManager _uriManager) public onlyOwner {
        ...
        returns (string memory)
    {
        ...
        return uriManager.getTokenURI(_tokenId);
    }
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index)
        ...
        returns (uint256)
    {
        return enumerator.tokenOfOwnerByIndex(_owner, _index);
    }
    ...
    function tokenByIndex(uint256 _index) public view returns (uint256) {
        return enumerator.tokenByIndex(_index);
    }
     ...
     */
    function balanceOf(address _owner) public view returns (uint256 balance) {
        return enumerator.balanceOf(_owner);
    }
    ...
    function setBaseURI(string memory _baseURI)
        ...
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
        uriManager.setBaseURI(_baseURI);
    }
    ...
    function setCustomURI(uint256 _tokenId, string memory _newURI)
        ...
        onlyOwnerOrRole(CREATOR_ROLE_NAME)
    {
        uriManager.setCustomURI(_tokenId, _newURI);
    }
     ...
     * @param _toAddress The account to receive the newly minted tokens.
    ...
    function _mint(address _toAddress, uint256 _tokenId) internal virtual {
        ...
        enumerator.beforeTokenTransfer(address(0), _toAddress, _tokenId);
    ...
    function _burn(uint256 _tokenId) internal virtual {
        ...
        enumerator.beforeTokenTransfer(token_owner, address(0), _tokenId);
    ...
    function _transfer(
        ...
        enumerator.beforeTokenTransfer(_fromAddress, _toAddress, _tokenId);
...
contract UriManager is IUriManager, Ownable {
    using Strings for string;
