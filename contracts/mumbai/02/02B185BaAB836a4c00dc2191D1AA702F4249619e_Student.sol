...
// File: contracts\EQ8dex\Student.sol

//
pragma solidity ^0.8.0;
contract Student is ERC721URIStorage {
    ...
    uint256 public Student_ID = 1;
    mapping(uint256 => StudentStruct) public studentInfo;
    mapping(uint256 => ParentStruct) public parentInfo;
    event SBT_mint(address minter, address receiver, uint256 SBT_ID);
    event SBT_burn(uint256 SBT_ID);

    constructor() ERC721("Student_Info", "Info") {
        admin = msg.sender;
    }


    function mint(address _to, StudentStruct memory _StudentStruct, ParentStruct memory _ParentStruct,string memory _URIofImage)
        external
    {
        _mint(_to, Student_ID);
        _setTokenURI(Student_ID, _URIofImage);
        _StudentStruct.StudentId = Student_ID;
        studentInfo[Student_ID] = _StudentStruct;
        parentInfo[Student_ID] = _ParentStruct;
        emit SBT_mint(admin, _to, Student_ID);
        Student_ID++;
    }
