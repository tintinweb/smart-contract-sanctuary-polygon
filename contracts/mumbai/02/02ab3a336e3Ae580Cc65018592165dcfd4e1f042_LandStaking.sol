...
contract LandStaking is Ownable, ReentrancyGuard {
    ...
    event UnstakeAllowed(bool value);
    ...
    modifier canUnstake() {
        require(unstakeAllowed, 'unstaking is not allowed');
        _;
    }
    ...
    function claim(uint256 id) external nonReentrant {
        ...
        require(_getBlockTime() >= stakingData.stakingStartTime + stakingData.duration, 'not now');

        landContract.transferFrom(address(this), sender, id);
        xpToken.transfer(sender, stakingData.reward);
        rewardLocked -= stakingData.reward;
     ...
     * @param id land Id
     **/
    function unstake(uint256 id) external nonReentrant canUnstake {
        address sender = _msgSender();
        StakingData memory stakingData = stakingDataMap[id];
        require(stakingData.status == 1, 'not staked');
        require(stakingData.staker == sender, 'not owner');
        require(
            _getBlockTime() < stakingData.stakingStartTime + stakingData.duration,
            'can claim'
        );

        landContract.transferFrom(address(this), sender, id);
        rewardLocked -= stakingData.reward;
        stakingDataMap[id].status = 2; // unstaked
    ...
    function claimBatch(uint256[] memory ids) external nonReentrant {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            landContract.transferFrom(address(this), sender, id);
            totalRewardTransfer += stakingData.reward;
            stakingDataMap[id].status = 3; // claimed
     ...
     * @param ids list land Ids
     **/
    function unstakeBatch(uint256[] memory ids) external nonReentrant canUnstake {
        address sender = _msgSender();
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            landContract.transferFrom(address(this), sender, id);
            totalRewardLocked += stakingData.reward;
            stakingDataMap[id].status = 2; // unstaked
        }
    ...
    function enableUnstake(bool value) external onlyAdmin {
        unstakeAllowed = value;
        emit UnstakeAllowed(value);
    }
