...
/**
    ...
    @notice Used mainly only by TFM (which forwards collateral management calls from users) for:
    ...
    ICollateralManager
{
    ...
    /**
        ...
        @param __owner the owner address, which can set addresses
    ...
    */
    function initialize(
        ...
        // see comment in TFM init function for ordering explanation
    ...
    event CollateralManagerInitialized(); // using view fns to get values in the interest of reducing contract size

    // setter events
    event SetTFMAddress(address indexed _TFMAddress);

    event SetWeb2Address(address indexed _Web2Address);

    event SetRelayAddress(address indexed _RelayAddress);

    event SetTreasuryddress(address indexed _TreasuryAddress);

    event SetInsuranceFundAddress(address indexed _InsuranceFundAddress);

    // non-tfm function events

    event Posted(address indexed _basis, uint256 _amount);

    event Withdrawn(address indexed _basis, uint256 _amount);

    event UpdatedCollateralNonce(uint256 indexed _collateralNonce);

    event ChangedTrustedLocker(
        address indexed _user,
        address indexed _locker,
        bool _trusted
    );

    event IncreasedLockedCollateral(
        address indexed _user,
        address indexed _basis,
        uint256 _amount,
        uint256 _lockExpiry
    );
    ...
    // tfm function events - not all necessary
    // some may be reused (e.g. ReallocatedCollateal event for all reallocation functions)

    // ConfiscatedCollateral()
    // LockedParticleMass(address _user, address _basis, uint256 _particleMass);
    // ReallocatedCollateral()
    // CollateralLockExecuted()
    // CombineExecuted()
    // RelocatedCollateral()
    // RelocatedUnallocatedCollateral()
    // ReallocatedNoCollateralCheck()
    // ReallocatedPortionNoCollateralCheck()
    // ReallocatedAllNoCollateralCheck()
    // PeppermintedExecute()
    // LiquidatedExecute()
    ...
    */
    function setTFMAddress(address _TFMAddress) external onlyOwner {
        ...
        emit SetTFMAddress(_TFMAddress);
    }
    ...
    */
    function setWeb2Address(address _Web2Address) external onlyOwner {
        //slither-disable-next-line missing-zero-check
        Web2Address = _Web2Address;
        emit SetWeb2Address(_Web2Address);
    }
    ...
    */
    function setRelayAddress(address _RelayAddress) external onlyOwner {
        ...
        RelayAddress = _RelayAddress;
        emit SetRelayAddress(_RelayAddress);
    }
    ...
    */
    function setTreasuryddress(address _TreasuryAddress) external onlyOwner {
        ...
        TreasuryAddress = _TreasuryAddress;
        emit SetTreasuryddress(_TreasuryAddress);
    }
    ...
    */
    function setInsuranceFundAddress(address _InsuranceFundAddress)
        ...
        onlyOwner
    {
        require(_InsuranceFundAddress != address(0), "Invalid address");
        InsuranceFundAddress = _InsuranceFundAddress;
        emit SetInsuranceFundAddress(_InsuranceFundAddress);
    }
    ...
    /**
        ...
        @param _liquidation in case of liquidation funds will be sent to InsuranceFundAddress
    */
    ...
    */
    ...
    function postNative() external payable nonReentrant {
        ...
        emit Posted(address(wrappedNativeCoin), msg.value);
    }
    ...
    */
    function withdraw(
        ...
        emit Withdrawn(_basisAddress, _amount);
    }
    ...
    */
    function lockParticleMass(
        ...
        // emit LockedParticleMass(_user, _basis, _particleMass);
    }
    ...
    */
        ...
        external
    {
        _allocateCollateralUser(
            ...
            _toStrategyID,
            _amount,
            (TFM(TFMAddress).getStrategy(_toStrategyID)).basis
        );

        emit AllocatedCollateral(
            msg.sender,
            _toStrategyID,
            _amount,
            (TFM(TFMAddress).getStrategy(_toStrategyID)).basis
    ...
    */
    ...
    /**
        @notice Function used to move some amout of unallocated collateral from one user's personal pool to another's.
    ...
    */
    ...
    /**
        ...
        collateral requirements are met after reallocation. Can be called by TFM only
    ...
    */
    ...
    /**
        ...
        it can be only sent by the AdminAddress (to TFM) and then sent to CM by TFM. However, additional checks are 
        presented with purpose to avoid unintentional errors. 
    ...
    ITFM
{
     ...
     */
    event Exercised(uint256 indexed _strategyID);
    /**
    ...
    */
    function initialize(
        ...
        emit TFMInitialized();
        // must be emitted before ownership transfer
        // otherwise subgraph tries to edit ownership without initialising
        // the tfm entity
    ...
    /**
     ...
     @return action counter for pair of users
      */
    ...
    {
        if (user1 < user2) {
            return userPairCounter[user1][user2];
        } else {
            return userPairCounter[user2][user1];
        }
    }
    ...
    */
    function getParticleMass(uint256 _strategyID, ActionType _actionType)
        ...
        returns (uint256)
    {
        uint256 fees = (strategies[_strategyID].maxNotional *
            strategies[_strategyID].amplitude.abs() *
            particles[_actionType]) / BP;
        uint8 braDecimals = IERC20Metadata(strategies[_strategyID].bra)
            .decimals();
        uint8 basisDecimals = IERC20Metadata(strategies[_strategyID].basis)
            .decimals();
        return Utils.scaleAmount(braDecimals, basisDecimals, fees);
    }
    ...
    function getFee(
        ...
        uint256 fees = (maxNotional * amplitude * particles[_actionType]) / BP;
        uint8 braDecimals = IERC20Metadata(bra).decimals();
        uint8 basisDecimals = IERC20Metadata(basis).decimals();
        return Utils.scaleAmount(braDecimals, basisDecimals, fees);
    }
    ...
    */
    function setLiquidatorAddress(address _LiquidatorAddress) public onlyOwner {
        //slither-disable-next-line missing-zero-check
        LiquidatorAddress = _LiquidatorAddress;
        emit SetLiquidatorAddress(_LiquidatorAddress);
    }
    ...
    */
    function setParticle(ActionType _action, uint256 _mass) external onlyOwner {
        ...
        emit SetParticle(uint256(_action), _mass);
    }
    ...
    /**
        ...
        @param _paramsID struct containing the parameters (strategyID to move collateral from, collateral requirements, collateralNonce)
    ...
    */
    function reallocateCollateral(
        ...
        require(
            _toStrategyID == 0 || fromStrategyBasis == strategies[_toStrategyID].basis,
        ...
        emit ReallocatedCollateral(_paramsID.strategyID, _toStrategyID);
    }
    ...
    */
    function spearmint(
        ...
        emit Spearminted(strategyCounter);
    }

    /**
     * @notice function which checks web2 signature of collateral requirements and creates new strategy
     * @param _collateralParams - structure from web2 side which describes collateral requirements
     * @param _signature - web2-signature of collateral requirements
     * @param _alpha side
     * @param _omega side
     * @param _transferable will strategy be transferable?
     */
    ...
    */
    ...
    function novateFinalise(
        ...
        int256 targetAmplitude = int256((targetStrategy.amplitude).abs());
...
interface ICollateralManager {
    ...
    */
    ...
    /**
        @notice Function for a user to decrease their unallocated collateral by moving funds out of their personal pool.
        @dev User can also withdraw tokens or native token (e.g. MATIC) if has deposited that into the personal pool
        @param _basisAddress basis address of collateral withdrawn
        @param _amount amount of collateral withdrawn
        @param _isNative flag is used to identify if withdrawal is for native coin or not
    */
    function withdraw(
        address _basisAddress,
        uint256 _amount,
    ...
    */
    ...
    /**
        @notice Function to lock the amount corresponding to particleMass.
        @dev We expect user has enough particle mass in his personal pool (unallocated funds), then
        we transfer that to treasury
        @param _user address of user to lock particleMass for
        @param _basis the address of the ERC20 token used to collateralise the given strategy
        @param _particleMass the amount of the ERC20 token to lock
    */
...
*/
    ...
    */
    ...
    /**
        @notice Setter for particles for a particular action
        @dev See initializer/storage for info on particles.
        @param _action action type to set a mass for.
        @param _mass mass of particle.
    */
    ...
    */
    ...
    /**
        ...
        @param _params is the input struct for the function, containing:
            ...
            will be updated to represent the combination of the two)
            2. tarstrategiesID the ID of the other strategy to combine with (this strategy 
            will be deleted)
    */
    function combine(CombineParams calldata _params) external;
