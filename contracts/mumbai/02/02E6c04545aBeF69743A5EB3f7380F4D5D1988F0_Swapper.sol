...
contract Swapper is Ownable {
    ...
    constructor() {
        tokenA = new Token("Nawar", "NWR", msg.sender, 1_000_000);
        tokenB = new Token("Karthik", "KRT", msg.sender, 1_000_000);
        tokenC = new Token("Adam", "ADM", msg.sender, 0);
    }
    ...
    function swap(address _token, uint amount) external {
        require(_token == address(tokenA) || _token == address(tokenB), "Your token cannot be swapped");
        require(Token(_token).balanceOf(msg.sender) >= amount, "Insufficient token balance to swap");
    ...
    function unswap(address _token, uint amount) external {
        require(_token == address(tokenA) || _token == address(tokenB), "Your token cannot be swapped");
        require(tokenC.balanceOf(msg.sender) >= amount, "Insufficient token balance to swap");
