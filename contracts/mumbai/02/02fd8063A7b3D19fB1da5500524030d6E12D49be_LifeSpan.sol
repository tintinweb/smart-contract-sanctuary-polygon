...
contract LifeSpan is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(string memory extUrl, string memory genImageUrl) initializer public {
        ...
        genders[0] = Gender('MALE', true);
        genders[1] = Gender('FEMALE', true);
    ...
    function safeMint(address to, string memory name, uint256 gender, uint256 birthdayDate) public onlyRole(MINTER_ROLE) {
        require(genders[gender].name.length != 0, "ERC721Metadata: Gender not exists");
        require(genders[gender].isActive, "ERC721Metadata: Gender not active");

        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        metaData[tokenId] = MetaData(stringToBytes32(name), gender, birthdayDate);
    }
    ...
    function baseSection(uint256 tokenId) internal view returns (bytes memory) {
        bytes memory base = abi.encodePacked(
            ...
            '"name": "', string(abi.encodePacked(metaData[tokenId].name)), '",',
    ...
    function attributesSection(uint256 tokenId) internal view returns (bytes memory) {
        bytes memory atr = abi.encodePacked(
            ...
            '},{',
            '"trait_type":"Gender","value":"', string(abi.encodePacked(genders[metaData[tokenId].gender].name)), '"',
            '}]',
            '}'
        );
    ...
    function addGender(uint256 id, string memory newGender) public onlyRole(OPERATOR_ROLE) {
        require(genders[id].name.length == 0, "ERC721Metadata: Gender already exists");

        genders[id].name = stringToBytes32(newGender);
        genders[id].isActive = true;
    }

    function setIsActiveGender(uint256 id, bool isActive) public onlyRole(OPERATOR_ROLE) {
        require(genders[id].name.length != 0, "ERC721Metadata: Gender not exists");
    ...
    function updateName(uint256 tokenId, string memory newName) public {
        ...
        metaData[tokenId].name = stringToBytes32(newName);
    }

    function updateGender(uint256 tokenId, uint256 newGender) public {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        require(ownerOf(tokenId) == msg.sender, "ERC721: you are not the owner of the token");
        require(genders[newGender].name.length != 0, "ERC721Metadata: Gender not exists");
