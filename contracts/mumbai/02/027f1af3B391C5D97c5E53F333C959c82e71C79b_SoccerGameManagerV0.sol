...
contract SoccerGameManagerV0 is Ownable {
    ...
    string [] public roles = ["goalkeeper", "defender", "middlefilder", "striker"];
    ...
    function removePlayer (address _collectionAddress, uint256 _id) public {
        require(playersContracts[_collectionAddress], "insert a valid address");
        require(activeTeam[msg.sender].length > 1);
    ...
    function removePlayer (address _collectionAddress, uint256 _id, address _coach) external {
        require(playersContracts[msg.sender]);
        require(activeTeam[_coach].length > 1);
    ...
    function isPlayerInTeam (address _collectionAddress, uint256 _id) public view returns(bool) {
        require(playersContracts[_collectionAddress], "insert a valid address");
    ...
    //MATCHMAKING
    function choosePlayer(string memory _key) public view returns(address){
        require(keccak256(abi.encodePacked(key)) == keccak256(abi.encodePacked(_key)));
