...
contract EthermonStakingData is EthermonStakingBasic {
    ...
    function getTokenData(uint256 _lockId)
        public
        view
        returns (TokenLocked memory, TokenMisc memory)
    {
        TokenData memory data = tokenIds[_lockId];
        return (data.lockedData, data.miscData);
    }
    ...
    function removeTokenData(uint64 _lockId) public onlyModerators {
        ...
        emit Withdraw(
            _data.lockedData.owner,
            _data.lockedData.monId,
            _data.lockedData.pfpId,
            _data.lockedData.emons,
            _data.miscData.endTime,
            _data.miscData.lastCalled,
            _lockId,
            _data.miscData.level,
            _data.miscData.validTeam,
            _data.miscData.teamPower,
            _data.miscData.badge,
            _data.miscData.balance,
            uint8(_data.miscData.duration)
        );
    }
