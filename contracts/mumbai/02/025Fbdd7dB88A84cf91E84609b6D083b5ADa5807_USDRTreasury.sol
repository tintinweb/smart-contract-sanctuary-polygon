...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    ...
    function purchaseReInitialSale(
        ...
        (address rePurchaseManager, address marketplace, address tracker) = abi
            .decode(
                addressProvider.getAddresses(
                    abi.encode(
        ...
        IERC20(paymentToken).approve(marketplace, ptAmount);
        //purchase ftnft
        ...
        //update tracker
        ITreasuryTracker(tracker).ftnftTreasuryPlaced(
            ftnft,
            lastReceivedTokenId,
            true
        );
    ...
    function withdrawTnft(
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (!ftnft) {
                tracker.tnftTreasuryPlaced(_nft, _tokenIds[i], false);
            } else {
                tracker.ftnftTreasuryPlaced(_nft, _tokenIds[i], false);
            }
        }
    }
