...
//        }
    ...
    function addTip(uint256 msgID) public payable nonReentrant whenNotPaused {
        ...
        uint256 sf = 0;
        if (cut > 0){
            sf = msg.value - (msg.value * (100 - cut));
    ...
    function addLike(uint256 msgID) public whenNotPaused nonReentrant {
        ...
        // Like a post
        Likes.addLike(msgID, msg.sender, costToLike);
    }
    ...
    function getMsgsByIDs(uint256[] calldata msgIDs) public view whenNotPaused returns (string memory) {
        ...
        string memory messages = MessageData.getMsgsByIDs(msgIDs);
...
interface ILikes {
    ...
    function addLike(uint256 msgID, address likedBy, uint256 tips) external;
}
