...
contract SwaprGL is BaseGovernanceUpgradeable, ListingHelper {
    ...
    function depositNFTs(bytes calldata data) external {
        ...
        (bytes memory message, bytes memory sig) = abi.decode(
            data,
            (bytes, bytes)
        );
    ...
    function createListing(
        ...
        require(
            swaprFee.getFeePaid(sender, paymentToken) >= fee,
            "LOWER_FEE_PAID"
        );
    ...
    function buyNowOrder(
        ...
        bytes memory data,
        uint split
    ) external payable onlyMarketplace(data) {
        (bytes memory listingData, ) = abi.decode(data, (bytes, bytes));
        ...
        require(
            buyerPurchasedAmount < order.maxBuyPerWallet,
            "PURCH_LMT_EXCEED"
        );
    ...
    function claim(bytes calldata data) external onlyMarketplace(data) {
        (bytes memory message, ) = abi.decode(data, (bytes, bytes));
        (bytes memory listingData, bytes memory lastBid) = abi.decode(
            message,
            (bytes, bytes)
        );
