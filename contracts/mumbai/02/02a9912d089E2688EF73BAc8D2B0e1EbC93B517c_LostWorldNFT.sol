    ...
    UUPSUpgradeable
{
    ...
    function editLand(
        ...
        if (enemyList.length > 0 && rewardList.length > 0) landData[landId].playable = true;
    ...
    function TESTING52() external pure returns (bool) {
        return true;
    }
}
...
interface ILostWorldData {
    function checkEnemyAvailable(uint256 enemyId) external returns (bool);
}
