...
*/
contract DrawCalculator is Manageable {
    ...
    event Deployed(
        DrawBufferInterface indexed drawBuffer
    );
    /* ============ Variables ============ */

    GaugeControllerInterface public gaugeController;

    /// @notice DrawBuffer address
    DrawBufferInterface public immutable drawBuffer;
    ...
    */
    constructor(
        GaugeControllerInterface _gaugeController,
        DrawBufferInterface _drawBuffer,
        address _owner
    ) Ownable(_owner) {
    ...
    */
    ...
    ) external returns (uint256[] memory prizesAwardable, bytes memory prizeCounts) {
    ...
    */
    ...
    function _calculatePrizesAwardable(
    ...
    ) internal returns (uint256[] memory prizesAwardable, bytes memory prizeCounts) {
        ...
        for (uint32 drawIndex = 0; drawIndex < draws.length; drawIndex++) {
            PrizeTier memory prizeTier = prizeTiers[history.binarySearch(_drawIds[drawIndex])];
            require(uint64(block.timestamp) < draws[drawIndex].timestamp + prizeTier.expiryDuration, "DrawCalc/draw-expired");
            uint64 startTimestamp = draws[drawIndex].timestamp - draws[drawIndex].beaconPeriodSeconds;
            uint64 endTimestamp = draws[drawIndex].timestamp - prizeTier.endTimestampOffset;
            uint64 totalUserPicks = _calculateUserPicks(
                _ticket, _user, startTimestamp, endTimestamp, prizeTier.poolStakeTotal, prizeTier.bitRangeSize, prizeTier.matchCardinality
            );

            (_prizesAwardable[drawIndex], _prizeCounts[drawIndex]) = _calculate(
                draws[drawIndex].winningRandomNumber,
                totalUserPicks,
                _userRandomNumber,
    ...
    function calculateUserPicks(
        ...
        IDrawBeacon.Draw[] memory draws = drawBuffer.getDraws(_drawIds);
        uint64[] memory picks = new uint64[](_drawIds.length);
        for (uint32 drawIndex = 0; drawIndex < draws.length; drawIndex++) {
            PrizeTier memory prizeTier = prizeTiers[history.binarySearch(_drawIds[drawIndex])];
            require(uint64(block.timestamp) < draws[drawIndex].timestamp + prizeTier.expiryDuration, "DrawCalc/draw-expired");
            uint64 startTimestamp = draws[drawIndex].timestamp - draws[drawIndex].beaconPeriodSeconds;
            uint64 endTimestamp = draws[drawIndex].timestamp - prizeTier.endTimestampOffset;
            picks[drawIndex] = _calculateUserPicks(
                _ticket, _user, startTimestamp, endTimestamp, prizeTier.poolStakeTotal, prizeTier.bitRangeSize, prizeTier.matchCardinality
            );
        }
        return picks;
    ...
    function _calculateUserPicks(
        ...
        uint256 _poolStakeTotal,
        uint8 _bitRangeSize,
        uint8 _matchCardinality
    ) internal view returns (uint64) {
        uint256 numberOfPicks = _getTotalPicks(_ticket, _startTimestamp, _endTimestamp, _poolStakeTotal, _bitRangeSize, _matchCardinality);
        uint256 normalizedBalance = _getNormalizedBalanceAt(_ticket, _user, _startTimestamp, _endTimestamp);
        return _calculateNumberOfUserPicks(
            numberOfPicks,
            normalizedBalance
        );
    }
     ...
     * @param _numberOfPicks number of picks
    ...
    function _calculateNumberOfUserPicks(
        uint256 _numberOfPicks,
        uint256 _normalizedUserBalance
    ) internal pure returns (uint64) {
        return uint64((_normalizedUserBalance * _numberOfPicks) / 1 ether);
    }

    function getDrawPicks(ITicket _ticket, uint32[] calldata _drawIds) external view returns (uint256[] memory) {
        IDrawBeacon.Draw[] memory draws = drawBuffer.getDraws(_drawIds);
        uint256[] memory drawPicks = new uint256[](_drawIds.length);
        for (uint32 drawIndex = 0; drawIndex < draws.length; drawIndex++) {
            PrizeTier memory prizeTier = prizeTiers[history.binarySearch(_drawIds[drawIndex])];
            uint64 startTimestamp = draws[drawIndex].timestamp - draws[drawIndex].beaconPeriodSeconds;
            uint64 endTimestamp = draws[drawIndex].timestamp - prizeTier.endTimestampOffset;
            drawPicks[drawIndex] = _getTotalPicks(_ticket, startTimestamp, endTimestamp, prizeTier.poolStakeTotal, prizeTier.bitRangeSize, prizeTier.matchCardinality);
        }
        return drawPicks;
    }

    function getTotalPicks(ITicket ticket, uint256 startTime, uint256 endTime, uint256 totalPoolStaked, uint8 bitRange, uint8 cardinality) external view returns (uint256) {
        return _getTotalPicks(ticket, startTime, endTime, totalPoolStaked, bitRange, cardinality);
    }

    function _getTotalPicks(ITicket ticket, uint256 startTime, uint256 endTime, uint256 totalPoolStaked, uint8 bitRange, uint8 cardinality) internal view returns (uint256) {
        uint256 totalChances = (2**bitRange)**cardinality;
        uint256 gaugeScaledAverage = gaugeController.getScaledAverageGaugeBetween(address(ticket), startTime, endTime);
        return (gaugeScaledAverage * totalChances) / totalPoolStaked;
    }
    ...
    function _getNormalizedBalanceAt(
        ...
        uint256[] memory balances = _ticket.getAverageBalancesBetween(
            _user,
            _timestampsWithStartCutoffTimes,
            _timestampsWithEndCutoffTimes
        );

        uint256[] memory totalSupplies = _ticket.getAverageTotalSuppliesBetween(
            _timestampsWithStartCutoffTimes,
            _timestampsWithEndCutoffTimes
        );
        ...
        if(totalSupplies[0] > 0){
            normalizedBalance = (balances[0] * 1 ether) / totalSupplies[0];
        }
     ...
     * @param _prizeTier           PrizeTier for that draw
    ...
    function _calculate(
        ...
        PrizeTier memory _prizeTier
    ) internal returns (uint256 prize, uint256[] memory prizeCounts) {

        // create bitmasks for the PrizeDistribution
        uint256[] memory masks = _createBitMasks(_prizeTier.matchCardinality, _prizeTier.bitRangeSize);
        uint32 picksLength = uint32(_picks.length);
        uint256[] memory _prizeCounts = new uint256[](_prizeTier.tiers.length);

        uint8 maxWinningTierIndex = 0;

        require(
            picksLength <= _prizeTier.maxPicksPerUser,
        ...
        prize = (prizeFraction * _prizeTier.prize) / 1e9;
    ...
    function _createBitMasks(uint8 _matchCardinality, uint8 _bitRangeSize)
        ...
        returns (uint256[] memory)
    {
        uint256[] memory masks = new uint256[](_matchCardinality);
        masks[0] =  (2**_bitRangeSize) - 1;

        for (uint8 maskIndex = 1; maskIndex < _matchCardinality; maskIndex++) {
            // shift mask bits to correct position and insert in result mask array
            masks[maskIndex] = masks[maskIndex - 1] << _bitRangeSize;
        }
    ...
    function getPrizeTier(uint32 drawId) external view returns (PrizeTier memory) {
        require(drawId > 0, "DrawCalculator/draw-id-not-zero");
    ...
    // @inheritdoc IDrawCalculator
    function popAndPush(PrizeTier calldata newPrizeTier) external onlyOwner returns (uint32) {
        uint length = history.length;
        require(length > 0, "DrawCalculator/history-empty");
        require(history[length - 1] == newPrizeTier.drawId, "DrawCalculator/invalid-draw-id");
    ...
    function _push(PrizeTier memory _prizeTier) internal {
        ...
        if (_length > 0) {
            ...
            require(
                ...
                "DrawCalculator/non-sequential-id"
            );
        }
    ...
    function _replace(PrizeTier calldata _prizeTier) internal {
        ...
        require(cardinality > 0, "DrawCalculator/no-prize-tiers");
        uint32 oldestDrawId = history[0];
        require(_prizeTier.drawId >= oldestDrawId, "DrawCalculator/draw-id-out-of-range");
        uint32 index = history.binarySearch(_prizeTier.drawId);
        require(history[index] == _prizeTier.drawId, "DrawCalculator/draw-id-must-match");
...
/**
    ...
    * @notice The PrizeDistributor contract holds Tickets (captured interest) and distributes tickets to users with winning draw claims.
              PrizeDistributor uses an external DrawCalculator to validate a users draw claim, before awarding payouts. To prevent users 
...
*/
contract PrizeDistributor is Manageable {
     ...
     */
    event DrawCalculatorSet(DrawCalculator indexed calculator);
    ...
    DrawCalculator internal drawCalculator;
    ...
    constructor(
        address _owner,
        IERC20 _token,
        DrawCalculator _drawCalculator
    ) Ownable(_owner) {
    ...
    function getDrawCalculator() external view returns (DrawCalculator) {
    ...
    function setDrawCalculator(DrawCalculator _newCalculator)
        external
        onlyManagerOrOwner
        returns (DrawCalculator)
    {
     ...
     */
    function _setDrawCalculator(DrawCalculator _newCalculator) internal {
