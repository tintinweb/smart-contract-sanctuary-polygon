...
contract HiveNFT is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable, VRFConsumerBase {
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        for(uint i = 0; i < 9999; i++) {
            uint256 randID = getInternalRandomNumber(randomness, 21, 40000, i);
            if (randID > 20 && _totalSupply[randID] == 0) {
                _totalSupply[randID] = 1;
                allocateTire(randID);
                isOverlapRewardClaimed[rateUpdatedTime[_tire[randID]]][randID] = true;
    ...
    // function buy(uint random) private {
	// 	for (uint256 i = 0; i < 9999; i++) {
    //         getRandomNumber();
	// 	}
	// 	revert();
    ...
    function getInternalRandomNumber(uint randomNum, uint from, uint to, uint salt) private pure returns(uint256) {
        return uint256(keccak256(abi.encode(randomNum, salt))).mod(to - from) + from;
    }
}