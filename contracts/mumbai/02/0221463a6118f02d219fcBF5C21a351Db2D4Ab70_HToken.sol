...
contract HToken is HTokenInternal {
  ...
  function depositUnderlying(uint256 _amount, address _to) external {
    ...
    depositUnderlyingInternal(_to, _amount);
  ...
  function repayBorrow(
    ...
    checkReentrant();
    if (_to == address(0)) _to = msg.sender;
    repayBorrowInternal(_to, _repayAmount, _collateralId);
    _status = _NOT_ENTERED;
  }
...
contract HTokenInternal is ERC1155, IERC721Receiver, AccessControl {
  ...
  event AccruedInterestToCoupons(uint256[] _ids, uint256 _earnings);
  ...
  event AddressUpdated(address indexed _oldReceiver, address indexed _newReceiver, uint256 _target);
   ...
   * @param   _amount   Quantity of underlying ERC-20 to transfer in
   */
  function depositUnderlyingInternal(address _to, uint256 _amount) internal {
    hivemind.depositAllowed(HTokenI(address(this)), _to, _amount);

    uint256 tokensToMint = (doUnderlyingTransferIn(msg.sender, _amount) * 1e18) / exchangeRateCurrent();
   ...
   * @return  Amount actually transferred, accounting for transfer tax
   */
  ...
  function borrowInternal(
    ...
    if (totalBorrows == 0) {
      totalShares = borrowIncurred;
      coupon.debtShares = borrowIncurred;
      borrowBalancePerUser[couponOwner] += borrowIncurred;
    } else {
      uint256 shares = (totalShares * borrowIncurred) / totalBorrows;
      totalShares += shares;
      coupon.debtShares += shares;
      borrowBalancePerUser[couponOwner] += shares;
    }
  ...
  function repayBorrowInternal(
    ...
    payment = doUnderlyingTransferIn(msg.sender, payment);
  ...
  function getCashPrior() public view returns (uint256) {
    uint256 totalFees = totalAdminFees + totalHiveFees + totalBorrowFees;
  ...
  {
    uint256 userDebt;
    if (totalShares > 0) userDebt = (borrowBalancePerUser[_account] * totalBorrows) / totalShares;
   ...
   * @param   _addAmount  Quantity of underlying ERC-20 token to add to the reserves
   */
  ...
  /**
   * @notice  Reduce reserves by transferring to admin
   * @dev     Accrues interest
   * @param   _reduceAmount   Quantity of underlying ERC-20 token to remove from the reserves
   */
  function _reduceReserves(uint256 _reduceAmount) external {
    ...
    if (getCashPrior() + totalReserves < _reduceAmount) {
  ...
  /**
   * @notice  Set new admin, hive, protocol and reserve factor mantissas
   * @dev     Accrues interest
  ...
  function _withdrawAdminFees(uint256 _amount) external {
    ...
    if (underlyingToken.balanceOf(address(this)) < _amount) {
      revert AdminError(Error.TOKEN_INSUFFICIENT_CASH);
    }
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i; i < len; ) {
      ...
      if (_ids[i] > 0) {
        ...
        borrowBalancePerUser[_from] -= _shares;
        borrowBalancePerUser[_to] += _shares;
        unchecked {
          ++lengthToModify;
        }
      }
