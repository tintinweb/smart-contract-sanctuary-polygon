...
contract RythumCoin is IERC20, Ownable {
    ...
    uint256 totalSupply_ = 10 * _decimalFactor;
   ...
   constructor() public {
    ...
    function buyRythumToken(uint256 numTokens) public payable returns(string memory,uint256) {
        uint256 totalAmount = numTokens*tokenPrice;
        balances[ownerAddress] = balances[ownerAddress].sub(numTokens);
        balances[msg.sender] = balances[msg.sender].add(numTokens);
        emit Transfer(msg.sender, ownerAddress, numTokens);
        payable(ownerAddress).transfer(totalAmount);
        return("Token Transfer Done",numTokens);
    }   
