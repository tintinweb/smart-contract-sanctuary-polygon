...
contract EliteForceS30 is ContractOwnership {
    address payable internal firstPartnerAddress;
    address payable internal secondPartnerAddress;
    ...
    constructor(address owner, address partner1, address partner2) {
        ownerAddress = payable(owner);
        firstPartnerAddress = payable(partner1);
        secondPartnerAddress = payable(partner2);

        // Fixed level price - in test, the price is 0.02 MATIC
        multiply = 10 ** 15;
        // Use on main-net 20 MATIC
        // multiply = 10 ** 18;
    ...
    function _splitPayout(uint256 price) internal
    {
        ...
        firstPartnerAddress.transfer(FIRST_PARTNER_PERCENT * price / 100);

        // Send to second partner
        secondPartnerAddress.transfer(SECOND_PARTNER_PERCENT * price / 100);
    }
}