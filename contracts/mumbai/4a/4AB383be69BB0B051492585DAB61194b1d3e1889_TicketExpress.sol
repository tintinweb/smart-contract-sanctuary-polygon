...
contract TicketExpress is ERC721, Ownable, PriceFeed {
    ...
    event HappeningCreated(
        uint256 HappeningId,
        string name,
        uint256 ticketPrice,
        uint256 totalTickets
    );
    event HappeningCancelled(uint256 happeningId);
    ...
    struct Happening {
        string happeningId; // this is the same as the name of the happening
        string name;
        uint256 ticketPrice;
        uint256 totalTickets;
        uint256 ticketsSold;
    ...
    Happening[] public happenings;
    mapping(string => bool) public happeningExists; // check if a happening exists with a given name
    ...
    function mintTicket(address to, uint256 HappeningId) public onlyOwner {
        require(HappeningId < happenings.length, "Event does not exist");
        require(!happenings[HappeningId].isCancelled, "Event is cancelled");
        require(
            happenings[HappeningId].ticketsSold <
                happenings[HappeningId].totalTickets,
    ...
    function createHappening(
        ...
        require(
            !happeningExists[_name],
            "Happening with this name already exists"
        );
        happenings.push(
            Happening({
                happeningId: _name,
        ...
        happeningExists[_name] = true;
        emit HappeningCreated(
            happenings.length - 1,
            _name,
            _ticketPrice,
            _totalTickets
    ...
    function cancelHappening(uint256 happeningId) public onlyOwner {
        require(happeningId < happenings.length, "Event does not exist");
        happenings[happeningId].isCancelled = true;
        emit HappeningCancelled(happeningId);
    }
    ...
    function buyNewTicket(uint256 happeningId) public payable {
        require(happeningId < happenings.length, "Event does not exist");
        require(!happenings[happeningId].isCancelled, "Event is cancelled");
        require(
            happenings[happeningId].ticketsSold <
        ...
        require(
            msg.value == happenings[happeningId].ticketPrice,
            "Incorrect price"
        );
        ticketId++;
