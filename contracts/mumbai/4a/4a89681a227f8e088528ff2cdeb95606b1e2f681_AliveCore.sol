...
address public aliveManagerWallet;
function changeManagerWallet(address _to) public onlyAdmin {
    aliveManagerWallet = _to;
}
...
address[] public imps; // TODO - upgradeable
    function initialize(address _aliveAdmin, address _aliveManagerWallet, address _aliveRoyaltyReceiver, address _bandImp, address[] calldata _imps) public initializer {
        require(_imps.length >= 3, "atleast 3 imps");
        imps = _imps;
...
//    proxy band creation
    function createBandProxy( address _bandAdmin, string memory _bandName) public onlyGovernor returns(address){
        ...
        bytes memory data = abi.encodeWithSelector(BandContract(address(0)).initialize.selector, id, _bandAdmin, _bandName, aliveManagerWallet, aliveRoyaltyReceiver, imps);
        BeaconProxy band = new BeaconProxy(address(bandBeacon), data);
        emit BandCreationConstructor(address(bandBeacon), data);
        Band memory bandNew;
        bandNew.bandId=id;
