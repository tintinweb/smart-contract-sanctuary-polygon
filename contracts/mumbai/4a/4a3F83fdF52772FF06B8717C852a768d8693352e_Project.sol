...
contract Project is IProject, ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable nonReentrant {
        ...
        if (isCreatedByAdmin) {
            ...
            _projectInput.saleStart = 0;
            _projectInput.saleEnd = 0;
        }
        ...
        if (isCreateNewToken) {
            ...
            token = (osbFactory.create(_projectInput.isSingle, _projectInput.uri, _projectInput.tokenName, _projectInput.tokenSymbol, _projectInput.royaltyReceiver, _projectInput.royaltyFeeNumerator)).token;
    ...
    /**
     * @dev To check whether Project is active
     * @param _projectId Project ID
     */
    function isActiveProject(uint256 _projectId) external view returns (bool) {
        return projects[_projectId].status == ProjectStatus.STARTED &&
            block.timestamp >= projects[_projectId].saleStart &&
            block.timestamp <= projects[_projectId].saleEnd;
    }
}