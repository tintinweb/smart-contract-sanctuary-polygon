...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256  public tokenPriceInitial_ = 0.000001 ether;
    ...
    function updateprice(uint PriceInitial_,uint PriceIncremental_) onlyOwner public {
        tokenPriceInitial_= PriceInitial_;
        tokenPriceIncremental_= PriceIncremental_; 
        emit Updateprice();
    }
   ...
   function updateTaxFee(uint TaxFee,uint marketingPer,uint autoBurnPer,uint RewardPer) onlyOwner public {
    ...
    function _takeFee(uint256 tFee,uint256 tAmount) private {
       uint256 MarketingShare=0;
        ...
        // BurningShare=(tAmount*100)/(100+_autoBurnPer);
    ...
    */
    ...
    function sell(uint256 _amountOfTokens) public {
        ...
        uint256 _taxedMatic = SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_matic, _marketing),_autoBurn), _reward),_tax);
     ...
     function purchaseTokens(uint256 _incomingMatic)internal returns(uint256)
    {
        ...
        // require(_amountOfTokens > 0 && (SafeMath.add(_amountOfTokens,_tTotal) > _tTotal),"Invalid No of Token To Be Purchased ?");
    ...
    function sellPrice() public view returns (uint256) {
       if (_tTotal == 60000000 * 10**18) {
        return tokenPriceInitial_.sub(tokenPriceIncremental_);
            ...
            uint256 _busd = tokensToMatic_(1e18);
            uint256 _marketing = calculateFee(_busd,_marketingPer);
            uint256 _reward = calculateFee(_busd,_RewardPer);
            uint256 _Burn = calculateFee(_busd,_autoBurnPer);
            uint256 _selltax = 0;//calculateFee(_matic, sellTax_);
            uint256 _taxedbusd = SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_busd, _marketing),_reward),_Burn),_selltax);
            return _taxedbusd;
    }
}
        ...
        returns(uint256)
    {
        ...
        if(_tTotal == 60000000 * 10**18){
            ...
            uint256 _Burn = calculateFee(_busd,_autoBurnPer);
            uint256 _tax = 0;
            uint256 _taxedbusd = SafeMath.add(SafeMath.add(SafeMath.add(SafeMath.add(_busd, _marketing),_reward),_Burn),_tax);
            return _taxedbusd;
        }
    ...
    function calculateSellToken(uint256 _tokensToSell) public view returns (uint256) {
        uint256 _busd = tokensToMatic_(_tokensToSell);
        uint256 _marketing = calculateFee(_busd,_marketingPer);
        uint256 _reward = calculateFee(_busd,_RewardPer);
        uint256 _Burn = calculateFee(_busd,_autoBurnPer);
        uint256 _tax = 0;
        uint256 _taxedMatic = SafeMath.sub(SafeMath.sub(SafeMath.sub(SafeMath.sub(_busd, _marketing),_reward),_Burn),_tax);
        return _taxedMatic;
    }
