...
interface ITakeOrders {
    function takeOrders(bytes32[] calldata tokenIds, address taker) external payable;
}
...
contract MultiCall is KeyEnumBase {
    ...
    function launchToChain(
        ...
        if (orders.length > 0) {
            ITakeOrders(MARKET).takeOrders{value: address(this).balance}(orders, msg.sender);
        }
    ...
    function checkOrders(bytes32[] memory orders) public view returns (uint256 state, uint256 accCost) {
        ...
        for (uint256 i = 0; i < orders.length; i++) {
            (market, taker, expire, price) = searchOrder(orders[i], false);
    ...
    function launchCheck(
        ...
        external
        view
        returns (uint256 state, bytes32 agentNode, uint256 ordersCost, uint256 registersCost, uint256 reservesCost)
    {
        agentNode = Parser.encodeNameToNode(agentSuffix, agentName);
        if (!IIsNodeActive(DB).isNodeActive(agentNode)) {
            return (ERROR, bytes32(0), 0, 0, 0);
            // Reason: agentNode is not existed
        }

        if (orders.length > 0) {
            (state, ordersCost) = checkOrders(orders);
            if (state != 0) {
                return (state, bytes32(0), 0, 0, 0);
            }
        }

        if (registers.length > 0) {
            (state, registersCost) = checkRegisters(registrarList[index], registers);
            if (state != 0) {
                return (state, bytes32(0), 0, 0, 0);
            }
        }
        ...
        if (reserves.length > 0) {
            (state, reservesCost) = checkReserves(registrarList[index], valueMiningList[index], reserves);
            if (state != 0) {
                return (state, bytes32(0), 0, 0, 0);
            }
        }
        ...
        returns (address market, address taker, uint64 expire, uint256 price)
    {
        ...
        return searchOrder(node, owner, buffer);
    }

    function searchOrder(bytes32 node, address owner, bool buffer)
        public
        view
        returns (address market, address taker, uint64 expire, uint256 price)
    {
        ...
        )
    {
        ...
        if (owner != address(0)) {
            uint64 expire;
            (, taker, expire, price) = searchOrder(targetNode, owner, false);
    ...
    function searchLikees(bytes32 liker)
        external
        view
        returns (
        ...
        )
    {
        ...
        uint256 length = reader.itemArrayLength(likerStatusKey);

        if (length > 0) {
            likees = new bytes32[](length);
            owners = new address[](length);
            timestamps = new uint256[](length);
            fullNames = new string[](length);
            bytes[] memory itemArray = reader.itemArraySlice(likerStatusKey, 0, length - 1);
            for (uint256 i = 0; i < length; i++) {
