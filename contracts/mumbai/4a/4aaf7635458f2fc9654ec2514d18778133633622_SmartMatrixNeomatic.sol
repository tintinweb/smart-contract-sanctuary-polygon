...
// File: contracts/Neomatic 3lvl.sol
...
contract SmartMatrixNeomatic is SmartMatrixNeomaticBasic {
    ...
    function init(address _ownerAddress, address _multisig, IERC20 _depositTokenAddress) public onlyContractOwner {
       ...
       BASIC_PRICE = 0;
        ...
        levelPrice[8] = 100e18;
        levelPrice[9] = 130e18;
        levelPrice[10] = 160e18;
        levelPrice[11] = 200e18;
        levelPrice[12] = 250e18;
        levelPrice[13] = 300e18;
        levelPrice[14] = 400e18;
        levelPrice[15] = 600e18;
        levelPrice[16] = 800e18;
        levelPrice[17] = 1200e18;
        levelPrice[18] = 2000e18;
        levelPrice[19] = 3000e18;
        levelPrice[20] = 4000e18;
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level) private {
        ...
        if (users[referrerAddress].x6Matrix[level].secondLevelReferrals.length < 2) {
            users[referrerAddress].x6Matrix[level].secondLevelReferrals.push(userAddress);
            emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].secondLevelReferrals.length));
            ...
            address ref = users[referrerAddress].x6Matrix[level].currentReferrer;            
            users[ref].x6Matrix[level].thirdLevelReferrals.push(userAddress); 
            
            uint len = users[ref].x6Matrix[level].secondLevelReferrals.length;
            
            if ((len == 2) && 
                (users[ref].x6Matrix[level].secondLevelReferrals[0] == referrerAddress) &&
                (users[ref].x6Matrix[level].secondLevelReferrals[1] == referrerAddress)) {
            ...
            } else if (len == 2 && users[ref].x6Matrix[level].secondLevelReferrals[1] == referrerAddress) {
                if (users[referrerAddress].x6Matrix[level].secondLevelReferrals.length == 1) {
                    emit NewUserPlace(userAddress, ref, 2, level, 5);
                } else {
                    emit NewUserPlace(userAddress, ref, 2, level, 6);
            ...
            return updateX6ReferrerThirdLevel(userAddress, ref, level);
        }
        
        users[referrerAddress].x6Matrix[level].thirdLevelReferrals.push(userAddress);

        if (users[referrerAddress].x6Matrix[level].closedPart != address(0)) {
            if ((users[referrerAddress].x6Matrix[level].secondLevelReferrals[0] == 
                users[referrerAddress].x6Matrix[level].secondLevelReferrals[1]) &&
                (users[referrerAddress].x6Matrix[level].secondLevelReferrals[0] ==
                users[referrerAddress].x6Matrix[level].closedPart)) {

                updateX6(userAddress, referrerAddress, level, true);
                return updateX6ReferrerThirdLevel(userAddress, referrerAddress, level);
            } else if (users[referrerAddress].x6Matrix[level].secondLevelReferrals[0] == 
                users[referrerAddress].x6Matrix[level].closedPart) {
                updateX6(userAddress, referrerAddress, level, true);
                return updateX6ReferrerThirdLevel(userAddress, referrerAddress, level);
            } else {
        ...
        if (users[referrerAddress].x6Matrix[level].secondLevelReferrals[1] == userAddress) {
            updateX6(userAddress, referrerAddress, level, false);
            return updateX6ReferrerThirdLevel(userAddress, referrerAddress, level);
        } else if (users[referrerAddress].x6Matrix[level].secondLevelReferrals[0] == userAddress) {
            updateX6(userAddress, referrerAddress, level, true);
            return updateX6ReferrerThirdLevel(userAddress, referrerAddress, level);
        }
        
        if (users[users[referrerAddress].x6Matrix[level].secondLevelReferrals[0]].x6Matrix[level].secondLevelReferrals.length <= 
            users[users[referrerAddress].x6Matrix[level].secondLevelReferrals[1]].x6Matrix[level].secondLevelReferrals.length) {
            updateX6(userAddress, referrerAddress, level, false);
        } else {
            updateX6(userAddress, referrerAddress, level, true);
        }
        
        updateX6ReferrerThirdLevel(userAddress, referrerAddress, level);
    }
    ...
    function updateX6ReferrerThirdLevel(address userAddress, address referrerAddress, uint8 level) private {
        if (users[referrerAddress].x6Matrix[level].thirdLevelReferrals.length < 4) {
            return sendETHDividends(referrerAddress, userAddress, 2, level);
        }
        
        address[] memory x6 = users[users[referrerAddress].x6Matrix[level].currentReferrer].x6Matrix[level].secondLevelReferrals;
        ...
        users[referrerAddress].x6Matrix[level].secondLevelReferrals = new address[](0);
        users[referrerAddress].x6Matrix[level].thirdLevelReferrals = new address[](0);
        users[referrerAddress].x6Matrix[level].closedPart = address(0);
