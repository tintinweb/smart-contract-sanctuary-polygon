...
contract NebulaMarketplaceContract is Ownable, ReentrancyGuard{
    ...
    function purchaseToken(uint256 listingId, uint256 amount) public payable nonReentrant {
        ...
        payable(address(this)).transfer((idToListing[listingId].price * amount/100) * platformFee); // Platform fee 20%
        payable(idToListing[listingId].seller).transfer((idToListing[listingId].price * amount/50) * sellerPart); // Transfering 80% to seller
    }
    ...
    function purchaseBatchTokens(uint256[] memory listingIds, uint256[] memory amounts) public payable nonReentrant {
        ...
        // uint256[] memory _tokenIds = new uint256[](listingIds.length) ;
        // uint256[] memory _amounts = new uint256[](listingIds.length) ;
        ...
        for (uint256 index = 0; index < listingIds.length; index++) 
        {
            ...
            bool _tokenSold = idToListing[_listingId].completed;
            ...
            uint256 _tokenCost = _tokenPrice * _amount;

            require(msg.sender != _tokenSeller, "Can't buy your onw tokens!");
            require(msg.value >=_tokenCost, "Insufficient funds!");
            require(token.balanceOf(_tokenSeller, _tokenId) >= _amount, "Seller doesn't have enough tokens!");
            require(_tokenSold == false, "Listing not available anymore!");
            require(_tokensAvailable >= _amount, "Not enough tokens left!");
            ...
            _tokensAvailable -= _amount;
            listingsArray[_listingId-1].tokensAvailable -= _amount;
            if(_tokenPrivateListing == false){
            ...
            token.safeTransferFrom(_tokenSeller, msg.sender, _tokenId, _amount, "");           
        }

        payable(address(this)).transfer((_itemsPrice * platformFee)/100); // Platform fee 20%
...
//              {
//                  contractWiseTokensListings.push(idToListing[index]);
//              }
            
//         }
//         return contractWiseTokensListings;
//     }
...
//         {  
//           if(idToListing[index].contractAddress == contractAddress && idToListing[index].completed == false)
//              {
//                  contractWiseTokensListings.push(listingsArray[index]);
//              }
