...
/**
 * @dev Extension that adds event's place
 *
 */
abstract contract EventPlace is OwnableUpgradeable {

	  string private _place;

		/**
     * @dev Sets the value of the `place`.
     */
    function __EventPlace_set(string memory place_) public onlyOwner {
    	__EventPlace_set_unchained(place_);
    }

    function __EventPlace_set_unchained(string memory place_) internal onlyOwner {
    	_place = place_;
    }

    /**
     * @dev Returns the value of the `place`.
     */
    function place() public view virtual returns (string memory) {
        return _place;
    }
...
/**
 * @dev Extension that adds `quote` address.
 *
 */
abstract contract EventPrice is OwnableUpgradeable {
     ...
     */
    function __EventPrice_set(uint price_) public onlyOwner {
    	__EventPrice_set_unchained(price_);
    }

    function __EventPrice_set_unchained(uint price_) internal onlyOwner {
    	_price = price_;
    }
 ...
 *
 */
abstract contract EventQuote is OwnableUpgradeable {
     ...
     * @dev Sets the value of the `starts`.
     */
    function __EventQuote_set(address quote_) public onlyOwner {
    	__EventQuote_set_unchained(quote_);
    }

    function __EventQuote_set_unchained(address quote_) internal onlyOwner {
    	_quote = quote_;
    }
...
ERC721BurnableUpgradeable, EventStarts, EventEnds, EventPlace, EventLatitude, EventLongtitude, EventCanceled, EventQuote,
EventPrice {
    ...
    function safeMint(address to, string memory uri)
				...
				whenNotPaused
		{
				...
				if (price() > 0) {
					uint balance = quote() == 0x0000000000000000000000000000000000000000 ? msg.sender.balance: IERC20(quote()).balanceOf(msg.sender);
					require(balance >= price(), "DrippsEvent: insufficient balance");
				}
