...
contract FundsWrapperCustody is Initializable, IFundsWrapperCustody {
  ...
  function downgradeAndWithdrawEth(uint256 amt)
    external
    override
    onlyFundsOwner
  {
  ...
  receive() external payable override {
    if (msg.sender != fundsOwner) {
      (bool sent, ) = address(fundsOwner).call{ value: msg.value }("");
      require(sent, "failed to redirect the funds");
    }
  }
}
...
abstract contract IFundsWrapperCustody {
  ...
  function downgradeAndWithdrawEth(uint256 amt) external virtual;
  ...
  function withdrawEth(uint256 amt) public onlyFundsOwner {
    ...
    emit TokenWithdrawed(address(0), msg.sender, amt);
  }
