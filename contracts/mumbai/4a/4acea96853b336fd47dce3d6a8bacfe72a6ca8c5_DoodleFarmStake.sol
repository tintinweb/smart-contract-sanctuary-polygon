...
contract DoodleFarmStake is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    modifier requireERC20Token() {
        require(address(rewardsToken) != address(0), "rewardsToken not set");
        _;
    }

    modifier requireParentToken() {
        require(address(parentNFT) != address(0), "parentNFT not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setERC20TokenAddress(address rewardsTokenAddr) public onlyOwner() {
      rewardsToken = IERC20Token(rewardsTokenAddr);
    }

    function setParentTokenAddress(address parentAddr) public onlyOwner() {
      parentNFT = IERC721(parentAddr);
    }
      ...
      returns (uint256[] memory)
    {
      ...
      uint256[] memory depositSetOtherContract = rewardsToken.depositsOf(account);
    ...
    {
      uint256[] memory tokensInOldContract = rewardsToken.depositsOf(account);
      ...
      returns (uint256[] memory rewards) 
    {
      ...
      for (uint256 i; i < tokenIds.length; i++) {
        ...
        uint256 lastBlock = arrayContains(tokensInOldContract, tokenId) ? 
          rewardsToken._depositBlocks(account, tokenId) 
    ...
    function claimRewards(uint256[] calldata tokenIds) public whenNotPaused requireERC20Token {
      uint256 reward; 
      uint256 blockCur = Math.min(block.number, expiration);
      uint256[] memory tokensInOldContract = rewardsToken.depositsOf(msg.sender);
      ...
      rewardsToken.claimRewards(tokensInOldContract);
    ...
    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireParentToken requireERC20Token {
        require(msg.sender != address(rewardsToken), "Invalid address");
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireParentToken requireERC20Token nonReentrant() {
        claimRewards(tokenIds);
