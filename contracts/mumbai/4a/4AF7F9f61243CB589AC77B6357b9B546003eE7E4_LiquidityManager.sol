...
contract LiquidityManager is AccessControl, Pausable {
    ...
    constructor(AddressProvider addressProvider_) {
        ...
        )
    {
        ...
        ITreasury(treasury).withdraw(underlying, underlyingAmount);
        (address token0, address token1, uint24 fee) = _getPoolInfo(
            nonfungiblePositionManager
        );
        _collectFees(nonfungiblePositionManager);
        _splitUnderlying(router, exchange, underlying, usdr, fee);
        _increaseLiquidity(
            nonfungiblePositionManager,
            underlying,
            usdr,
        ...
        )
    {
        if (token0 == underlying) {
            amount0 = IERC20(underlying).balanceOf(address(this));
            amount1 = IERC20(usdr).balanceOf(address(this));
            IERC20(underlying).approve(nonfungiblePositionManager, amount0);
            IERC20(usdr).approve(nonfungiblePositionManager, amount1);
        } else {
            amount0 = IERC20(usdr).balanceOf(address(this));
            amount1 = IERC20(underlying).balanceOf(address(this));
            IERC20(usdr).approve(nonfungiblePositionManager, amount0);
            IERC20(underlying).approve(nonfungiblePositionManager, amount1);
        }
    ...
    function _splitUnderlying(
        ...
        uint256 amountIn = (underlyingBalance -
            IExchange(exchange).scaleToUnderlying(usdrBalance)) >> 1;
        IERC20(underlying).approve(router, amountIn);
