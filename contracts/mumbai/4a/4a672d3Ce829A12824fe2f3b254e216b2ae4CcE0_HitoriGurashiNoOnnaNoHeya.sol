...
contract HitoriGurashiNoOnnaNoHeya is ERC721Enumerable, Ownable {
     ...
     * @dev 赤い薬と青い薬はなんですか？
     */
    function WhatsKusuri () external pure returns(string memory){
        return (unicode"青い薬を飲めば、お話は終わる。君はベッドで目を覚ます。好きなようにすればいい。赤い薬を飲めば、君は不思議の国にとどまり、私がウサギの穴の奥底を見せてあげよう");
    }

    /**
     * @dev なぜ一人暮らしの女の子の部屋なのに汚いのですか？
     */
    function NazeObeya () external pure returns(string memory){
        return (unicode"一体いつから、女の子の一人暮らしの部屋がキレイだと錯覚していた？");
    }


    /**
     * @dev ピンクの華やかな部屋が一人暮らしの女の子の部屋では？
     */
    function PinkNoHeya () external pure returns(string memory){
