...
contract WowTCommunityPoll is OwnableUpgradeable, AccessControlUpgradeable {
    struct Poll {
        string question;
        string[] options;  // Store the options for the poll
         mapping(string => uint256) optionToIndex;  // Mapping of option to index
         mapping(uint256 => uint256) votes; // Mapping of option index to vote count
  ...
  bytes32 public constant ADMIN_ROLE = keccak256('ADMIN_ROLE'); // Admin role for authorization
    ...
    function createPoll(string memory _communityName, string memory _question, string[] memory _options) public {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
        require(bytes(_question).length > 0, "Invalid question");
        CommunityDetails storage communityDetails = communityDetails[_communityName];
        uint256 pollId = communityDetails.pollCount;
        Poll storage newPoll = communityDetails.polls[pollId];
        newPoll.question = _question;
        newPoll.options = _options;
        // newPoll.optionCount = _options.length;  // Set the option count
        newPoll.timestamp = block.timestamp;
        for (uint256 i = 0; i < _options.length; i++) {
            // newPoll.votes[_options[i]] = 0; // Initialize vote count for each option to 0
            newPoll.optionToIndex[_options[i]] = i;
        }
        communityDetails.pollCount++;
    ...
    function vote(string memory _communityName, uint256 _pollId, uint256 _optionIndex, address _user) public {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
        CommunityDetails storage communityDetails = communityDetails[_communityName];
        require(_pollId < communityDetails.pollCount, "Invalid poll ID");
        Poll storage poll = communityDetails.polls[_pollId];
        require(_optionIndex < poll.options.length, "Invalid option index");
        require(!poll.hasVoted[_user], "Already voted");
        // Get the corresponding option from the provided index
        string memory selectedOption = poll.options[_optionIndex];

        // Increment the vote count for the selected option
        poll.votes[poll.optionToIndex[selectedOption]]++;
        poll.hasVoted[_user] = true;

    }

    function getPollCount(string memory _communityName) public view returns (uint256) {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );

        return communityDetails[_communityName].pollCount;
    }

    function getPoll(string memory _communityName, uint256 _pollId) public view returns (string memory, string[] memory, uint256[] memory, uint256) {
        require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
        ...
        return (poll.question, poll.options, voteCounts, poll.timestamp);
    }
}
...
contract WowTPoints is OwnableUpgradeable, AccessControlUpgradeable {
   ...
   */
  function setActiveUserPoints(uint16 newActiveUserPoints) external onlyOwner {
