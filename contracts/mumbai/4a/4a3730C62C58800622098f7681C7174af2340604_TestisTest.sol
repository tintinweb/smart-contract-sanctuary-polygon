...
contract TestisTest is Ownable, ERC20 {
    ...
    constructor() ERC20("TestisTest", "TestisTest") {
        uint256 totalSupply = 420690000000000 * 1e18;
        maxTransactionAmount = (totalSupply * 20) / 1000; // 2%
        maxWallet = (totalSupply * 20) / 1000; // 2%
        taxWallet = address(0xe3e6d9D7F39B326550d026e7e7518e018BDC3ABe);
        _mint(msg.sender, totalSupply);
    }

    function botList(address _address, bool _isDenylisting) external onlyOwner {
        bots[_address] = _isDenylisting;
    }
    ...
    function _transfer(
        ...
        require(!bots[recipient] && !bots[sender], "Denied");
        require(
        ...
        if (tradingEnabled && sender == uniswapV2Pair) {
            ...
            require(
                amount <= maxTransactionAmount,
                "Buy amount exceeds the maxTransactionAmount."
            );
        } else if (tradingEnabled && recipient == uniswapV2Pair) {
            require(
                amount <= maxTransactionAmount,
                "Sell amount exceeds the maxTransactionAmount."
            );
        }
