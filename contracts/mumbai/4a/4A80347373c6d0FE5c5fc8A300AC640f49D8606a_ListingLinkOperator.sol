...
contract ListingLinkOperator is Pausable {
  ...
  uint8 public constant CANCEL_BY_NO_MORE_OWNED_BY_THE_SELLER_CODE = 3;
  uint8 public constant CLOSED_BY_SUCCESS_CODE = 4;
  ...
  function _throwIfUnexpectedFailureAtErc721Owning(address nftAddress, uint256 tokenId) private view {
    require(_checkErc721Owning(nftAddress, tokenId, msg.sender), "ListingLinkOperator: Failure during NFT ownership control");
  }
  ...
  function _throwIfUnexpectedFailureAtErc1155Owning(address nftAddress, uint256 tokenId) private view {
    require(_checkErc1155Owning(nftAddress, tokenId, msg.sender), "ListingLinkOperator: Failure during NFT ownership control");
  }
  ...
  /**
   * @notice Check that the ownerAddress own at least 1 of the ERC1155 NFT currently
   * @param nftAddress The address of the NFT smartcontract to contact
   * @param tokenId The tokenId we want to check the owning
   * @param ownerAddress The address we expect to own at least one token
   * @return bool true if the ownerAddress have at least 1 of the specified tokenId on the specified nftAddress contract
   */
  function _checkErc1155Owning(address nftAddress, uint256 tokenId, address ownerAddress) private view returns (bool) {
    return 1 <= abi.decode(
      Address.functionStaticCall(
        nftAddress,
        abi.encodeWithSignature(
  ...
  /**
   * @notice Check that the ownerAddress is the owner of the ERC721 token (tokenId) at the nftAddress contract
   * @param nftAddress The address of the NFT smartcontract to contact
   * @param tokenId The tokenId we want to check the owning
   * @param ownerAddress The address we expect to be the owner of the token
   * @return bool true if the ownerAddress is the owner of the specified tokenId on the specified nftAddress contract
   */
  function _checkErc721Owning(address nftAddress, uint256 tokenId, address ownerAddress) private view returns (bool) {
    return ownerAddress == abi.decode(
      Address.functionStaticCall(
        nftAddress,
        abi.encodeWithSignature(
          "ownerOf(uint256)",
   ...
   * Also, if this contract isn't approved anymore to transfer the NFT, we automatically cancel the listing with the statusCode "CANCEL_BY_AUTHORIZATION_REMOVED_CODE"
   * if the seller doesn't own at least a balanceOf 1 of the ERC1155 or is not owner of the ERC721 token anymore, we automatically cancel the listing with the statusCode "CANCEL_BY_NO_MORE_OWNED_BY_THE_SELLER_CODE"
  ...
  function buy(uint256 listingLinkId) external whenNotPaused isExisting(listingLinkId) returns (bool) {
    ...
    // Check that the seller still own the NFT expect by the buyer
    if (! (listingLink.nftType == ERC1155_TYPE ?
      _checkErc1155Owning(listingLink.nftAddress, listingLink.nftId, listingLink.sellerAddress) :
      _checkErc721Owning(listingLink.nftAddress, listingLink.nftId, listingLink.sellerAddress)
    )) {
      _listingLinks[listingLinkId].isClosed = true;
      _listingLinks[listingLinkId].statusCode = CANCEL_BY_NO_MORE_OWNED_BY_THE_SELLER_CODE;
      emit ListingLinkClosed(listingLinkId, CANCEL_BY_NO_MORE_OWNED_BY_THE_SELLER_CODE);
      return false;
    }
