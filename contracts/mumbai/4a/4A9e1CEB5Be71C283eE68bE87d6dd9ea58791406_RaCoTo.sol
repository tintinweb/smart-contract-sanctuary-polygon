...
interface ITreasury {
  function addTreasury(address _userAddress, bytes32 _requestId) external;
}
...
contract RaCoTo is FunctionsClient {
  ...
  string[] public arguments;

  IERC20 public raCoTokenInstance;
  ITreasury public treasuryInstance;
  IRaCoToUserGov public userGovInstance;

  mapping(address => uint256) private tokensNeededToClaimByUser;
  mapping(address => uint256) private previousTokenClaimedByUser;
  ...
  constructor(
    ...
    treasuryInstance = ITreasury(_treasuryAddress);
  ...
  /**
   * @notice Executed when we need to push the request on the blockchain
   *
   * @param source - This will contain the JS source code which we are going to execute
   * @param secrets - This will contain the secret keys (encrypted)
   * @param subscriptionId - Subscription ID of the executer
   * @param gasLimit - Gas Limit (LINK)
   */

  // This function will be executed by user. Parameters need to be adjusted.
  ...
  function executeRequest(
    ...
    uint64 subscriptionId,
    uint32 gasLimit
  ) public payable onlyWhitelistedOwner(msg.sender) returns (bytes32) {
    ...
    string memory previousTokenAmount = uint256ToString(previousTokenClaimedByUser[raCoOwnerAddress]);
    arguments[0] = userGovInstance.getgeoJSON(msg.sender);
    arguments[1] = previousTokenAmount;
    req.addArgs(arguments);
  ...
  function claimToken() public onlyWhitelistedOwner(msg.sender) {
    ...
    // treasuryInstance.addTreasury(msg.sender, _requestId);
