...
contract Nexfi175 is Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    struct UserInfo {
        ...
        uint256 lastWithdrawnAt; // keep track of the last withdrawal time
    ...
    function initialize(
        ...
        PRECISION_FACTOR = uint256(10**(uint256(30) - IERC20(_rewardToken).decimals()));
        isInitialized = true;
    ...
    function updateReward() public onlyOwner {
        ...
        for (uint256 i = 0; i < nftContractAddressList.length; i++) {
            ...
            for (uint256 j = 0; j < _tokenIdList.length; j++) {
                if (
                    ...
                    extraBoosts[nftContractAddressList[i]][_tokenIdList[j]]
                        .value = 1 * (10**totalDecimals()); // nft price by 1$
                } else {
                    extraBoosts[nftContractAddressList[i]][_tokenIdList[j]]
                        .value +=
                        (extraBoosts[nftContractAddressList[i]][_tokenIdList[j]]
        ...
        nftRewardPerPeriod.rewardPerPeriod[_rewardStartTime] = amount / rewardingPeriod;
    ...
    function depositNft(
        ...
        require(
            canDepositSameNFT || _alreadyStakedSameNft == false,
        ...
        totalStakings = totalStakings.add(
            extraBoosts[_nftAddress][_nftTokenId].value * _amount
        );
    ...
    function totalDecimals() public view returns (uint256 _totalDecimals) {
    ...
    function claim() external {
        ...
        require(canHarvest(_msgSender(), pending), "cannot claim");
        ...
        for (uint256 i = 0; i < nftRewardPerPeriod.rewardStartTime.length; i++) {
            ...
            reward += nftRewardPerPeriod.rewardDebts[_msgSender()][key].lockupReward;
            nftRewardPerPeriod.rewardDebts[_msgSender()][key].lockupReward = 0;
        }
        require(pending == reward, "something went wrong");

        for (uint256 index = 0; index < rewardTokens.length; index++) {
            IERC20(rewardTokens[index]).transfer(_msgSender(), reward);
        }
    ...
    function updatePool() internal {
        ...
        for (
            ...
            nftRewardPerPeriod.accRewardPerShares[key] =
                nftRewardPerPeriod.accRewardPerShares[key] +
                ((rewardAccum * PRECISION_FACTOR * apr) / totalStakings);
        }
        lastRewardTime = block.timestamp;
    }
    ...
    function lockupReward() internal {
        ...
        for (
            ...
            nftRewardPerPeriod.rewardDebts[_msgSender()][key].lockupReward = pendingReward(
                _msgSender(),
                index
            );

            _reward += nftRewardPerPeriod.rewardDebts[_msgSender()][key].lockupReward;
        }
    ...
    function pendingReward(
        ...
        if (totalStakings > 0) {
            ...
            adjustedTokenPerShare +=
                (rewardAccum * PRECISION_FACTOR * apr) /
                totalStakings;
        }
