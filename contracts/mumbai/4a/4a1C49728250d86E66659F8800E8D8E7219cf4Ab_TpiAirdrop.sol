...
contract TpiAirdrop is Pausable, ITpiAirdrop {
    ...
    constructor(address gnftSegment_, bytes32 merkleRoot_) {
    ...
    function setMerkleRoot(bytes32 _merkleRoot) external {
        if (!GNFT_SEGMENT_MANAGEMENT.hasRole(MANAGER_ROLE, msg.sender))
            revert Auth();
        merkleRoot = _merkleRoot;
    }

    function setPause(bool newState) external {
        if (!GNFT_SEGMENT_MANAGEMENT.hasRole(PAUSER_ROLE, msg.sender))
            revert Auth();
