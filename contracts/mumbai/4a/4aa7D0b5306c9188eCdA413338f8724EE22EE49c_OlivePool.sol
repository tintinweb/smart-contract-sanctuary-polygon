...
contract OlivePool is ILiquidityPool, Initializable, ERC20, Ownable, Pausable, ReentrancyGuard {
    ...
    function deposit(uint256 amount) external override whenNotPaused onlyOwner {
        _deposit(msg.sender, msg.sender, amount);
    }

    function depositFor(address account, uint256 amount) external override whenNotPaused onlyOwner {
    ...
    function withdraw(uint256 requestedAmount) external override whenNotPaused nonReentrant onlyOwner {
        require(
        ...
        requestedWithdrawals[msg.sender].amount = requestedWithdrawals[msg.sender].amount - requestedAmount;
        ...
        withheldLiquidity = withheldLiquidity - requestedAmount;

        _burn(msg.sender, requestedAmount);
        underlyer.safeTransfer(msg.sender, requestedAmount);

        sendBalanceUpdate(msg.sender, requestedAmount, false);
    }
    ...
    function requestWithdrawal(uint256 amount) external override onlyOwner {
        ...
        uint256 currentCycleIndex = manager.getCurrentCycleIndex();
        if (manager.getRolloverStatus()) { // If manger is currently rolling over add two to min withdrawal cycle
            requestedWithdrawals[msg.sender].minCycle = currentCycleIndex + 2;
        } else { // If manager is not rolling over add one to minimum withdrawal cycle
            requestedWithdrawals[msg.sender].minCycle = currentCycleIndex + 1;
        }

        manager.requestWithdrawalEvent(msg.sender, address(this), amount, currentCycleIndex);
    }
    ...
    function preTransferAdjustWithheldLiquidity(address sender, uint256 amount) internal {
        if (requestedWithdrawals[sender].amount > 0) {
            ...
            uint256 newRequestedWithdrawl = requestedWithdrawals[sender].amount - Math.min(amount, requestedWithdrawals[sender].amount);

            //subtract from global withheld liquidity (reduce) by removing the delta of (requestedAmount - newRequestedAmount)
            withheldLiquidity = withheldLiquidity - (requestedWithdrawals[sender].amount - newRequestedWithdrawl);
    ...
    function approveManager(uint256 amount) public override onlyOwner {
        ...
        if (currentAllowance < amount) {
            uint256 delta = amount - currentAllowance;
            underlyer.safeIncreaseAllowance(address(manager), delta);
        } else {
            uint256 delta = currentAllowance - amount;
    ...
    function transferFrom(
        ...
        address recipient,
        uint256 amount
    ) public override whenNotPaused nonReentrant onlyOwner returns (bool) {
        require(sender != recipient, "Self transfer not allowed");
        preTransferAdjustWithheldLiquidity(sender, amount);
        (bool success) = super.transferFrom(sender, recipient, amount);

        sendBalanceUpdate(sender, amount, false);
        sendBalanceUpdate(recipient, amount, true);
    ...
    function _deposit(
        ...
        sendBalanceUpdate(toAccount, amount, true);
    }


    function sendDummyBalanceUpdate() external onlyOwner {
        manager.updateBalance(msg.sender, address(underlyer), 0, 0, true);
    }


    function sendBalanceUpdate(address _user, uint256 _exchangeAmount, bool _isPositive) private {
        uint256 userBalance = balanceOf(_user);
        manager.updateBalance(_user, address(underlyer), userBalance, _exchangeAmount, _isPositive);
    }
}
