...
contract vestor is Initializable,ReentrancyGuard{
    ...
    internal
    {
        uint256 totalamount = addforinvestors(amountperinvestors,vestingPeriod,cliffperiod);
         ...
         for (uint i = 0; i < investors.length; i++) {
               if(startperiod != 0){
                userclaimingstart[investors[i]][contractid.current()] = startperiod;
               }
               else{
   ...
   uint256 deduct = amountofinvestorsbyindex[ _address][_contractid] - gettotalamountunlocked(_address);
  ...
  function gettime (address _address)internal view returns(uint256){
    ...
    */
    ...
    function gettotalamountunlocked(address _address)public view returns(uint256){
                ...
                if(block.timestamp - userclaimingstart[ _address][0] >= c._vestingPeriod){
                    return amountofinvestorsbyindex[ _address][0];
                }
      uint256 total = timesclaimablebyinvestors[_address][0]*(gettime(_address)/c.cliffperiod);
      return total;
  }
