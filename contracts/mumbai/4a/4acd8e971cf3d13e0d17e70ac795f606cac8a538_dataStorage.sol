...
contract dataStorage {
    ...
    error InsufficientFunds(string message, uint256 requiredAmount, uint256 actualAmount);
    ...
    function getProfile(address userAddress) public view returns (userProfile memory, uint) {
        return (profiles[userAddress], levels[msg.sender]);
    }
    ...
    function setTheme(userTheme memory theme) public payable {
        if (levels[msg.sender] < 10) {
            ...
            // require(msg.value >= levelPrice, ("Insufficient payment for theme change", levelPrice, msg.value));
    ...
    function buyLevels(uint numLevels) public payable {
        ...
        require(levels[msg.sender] + numLevels <= maxLevel);
