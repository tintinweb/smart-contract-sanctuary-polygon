...
contract DogewoodPoly is PolyERC721 {
    ...
    address public quest; // quest contract address
    mapping(uint256 => uint88) public questCooldownLogs; // (dogeId => timestamp) - this is used for cooldown after quest
    /*///////////////////////////////////////////////////////////////
                    End of data
    //////////////////////////////////////////////////////////////*/
    ...
    function setQuest(address t_) external {
        require(msg.sender == admin);
        quest = t_;
    }
    ...
    */
    ...
    function doAction(uint256 id, Actions action_)
        ...
        noCheaters
    {
        if(action_ == Actions.FARMING) {
            require(
                block.timestamp >= questCooldownLogs[id] + 24 hours,
                "still in quest cool down"
            );
        }
        ...
        external
    {
        for (uint256 index = 0; index < ids.length; index++) {
            ...
            if(action_ == Actions.FARMING) {
                require(
                    block.timestamp >= questCooldownLogs[ids[index]] + 24 hours,
                    "still in quest cool down"
                );
            }
    ...
    function unstakeForQuest(address[] memory owners, uint256[] memory ids) external {
        require (msg.sender == quest, "not quest");
        require(owners.length == ids.length, "invalid input");
        for (uint256 i = 0; i < ids.length; i++) {
            if (activities[ids[i]].action != Actions.UNSTAKED) _doAction(ids[i], owners[i], Actions.UNSTAKED, owners[i]);
        }
    }

    function updateQuestCooldown(uint256[] memory dogeIds, uint88 timestamp) external {
        require (msg.sender == quest, "not quest");
        for (uint256 i = 0; i < dogeIds.length; i++) {
            questCooldownLogs[dogeIds[i]] = timestamp;
        }
    }
    ...
    function validateOwnerOfDoge(uint256 id, address who_) external view returns (bool) { 
        return (ownerOf[id] == who_ || activities[id].owner == who_);
    }
