...
contract OttopiaPortalCreatorV2 is OwnableUpgradeable, UUPSUpgradeable {
    enum SALE_STAGE {
        NOT_STARTED,
        PRE_SALE,
        PUBLIC_SALE
    ...
    struct DiscountConfig {
        ...
        uint256 percentageOff;
    }

    IOtto public OTTO;
    IERC20 public WETH;
    IBurnableERC20 public CLAM;
    IERC20 public MAI;
    ...
    // ========== V2 ==========
    uint256 public price; // in CLAM
    address public REWARD;
    DiscountConfig public discountConfig;
    uint256 public maticPrice; // in CLAM
    ...
    function priceInMatic(uint256 quantity_) public view returns (uint256) {
        if (
            block.timestamp >= discountConfig.startedAt &&
            block.timestamp <= discountConfig.endedAt &&
            quantity_ >= discountConfig.amountThreshold
        ) {
            return
                quantity_ *
                _calcPercentage(
                    maticPrice,
                    10000 - discountConfig.percentageOff
                );
        } else {
            return quantity_ * maticPrice;
        }
    }

    function mintWithMatic(address to_, uint256 quantity_)
        external
        payable
        callerIsUser
    {
        uint256 needed_ = priceInMatic(quantity_);
        require(msg.value == needed_, 'msg.value mismatch');
        _distributeMatic(needed_);
        OTTO.mint(to_, quantity_);
        emit OttoMinted(msg.sender, to_, quantity_);
    }

    function setMaticPrice(uint256 price_) external onlyOwner {
        maticPrice = price_;
    }
    ...
    function mint(
        ...
        require(payInCLAM, 'only supports pay in CLAM');
        uint256 needed = _pay(quantity_);
        OTTO.mint(to_, quantity_);
        _distribute(needed);
        emit OttoMinted(msg.sender, to_, quantity_);
    }
    ...
    // deprecated
    function _pay(uint256 quantity_) private returns (uint256 needed_) {
        needed_ = currentPrice(quantity_);
        // console.log('needed %s', needed_);
        CLAM.transferFrom(msg.sender, address(this), needed_);
    }

    function _distributeMatic(uint256 amount_) private {
        uint256 toDao = _calcPercentage(amount_, 5000); // 50% to DAO
        uint256 toReward = amount_ - toDao; // left to REWARD
        (bool success, ) = payable(dao).call{value: toDao}('');
        require(success, 'transfer to dao failed');
        payable(REWARD).call{value: toReward}('');
        require(success, 'transfer to reward failed');
    }

    // deprecated
    function _distribute(uint256 amount_) private {
        uint256 toBurn = _calcPercentage(amount_, 1000); // burn 10%
        uint256 toDao = _calcPercentage(amount_, 7000); // 70% to DAO
        uint256 rewards = amount_ - toBurn - toDao; // left to REWARD
        // console.log('amount %s', amount_);
        // console.log('burn %s, dao %s, rewards', toBurn, toDao, rewards);
        CLAM.burn(toBurn);
    ...
    // deprecated
    function currentPrice(uint256 quantity_) public view returns (uint256) {
        if (
            block.timestamp >= discountConfig.startedAt &&
            block.timestamp <= discountConfig.endedAt &&
            quantity_ >= discountConfig.amountThreshold
            ...
            return
                quantity_ *
                _calcPercentage(price, 10000 - discountConfig.percentageOff);
        } else {
    ...
    function setDiscount(DiscountConfig calldata discountConfig_)
        external
        onlyOwner
    {
        discountConfig = discountConfig_;
    }
    ...
    function _calcPercentage(uint256 value_, uint256 percentage_)
        private
        pure
        returns (uint256)
    {
        return (value_ * percentage_) / 10000;
    }
