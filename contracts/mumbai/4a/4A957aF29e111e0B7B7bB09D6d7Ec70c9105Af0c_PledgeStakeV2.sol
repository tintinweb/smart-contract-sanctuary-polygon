...
contract PledgeStakeV2 {
    ...
    uint256 private constant ROUND_DURATION = 240 seconds;
    ...
    function _updateRewards(address _user) private {
        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            ...
            if (!stake.unstaked || stake.lastCalculatedRound < TOTAL_ROUNDS) {
                ...
                if (roundNumber > stake.lastCalculatedRound && roundNumber < roundRewardPercentages.length) {
                    uint256 roundRewardPercentage = roundRewardPercentages[roundNumber];
                    ...
                    stake.lastCalculatedRound = roundNumber; // Increase the last calculated round
                }
            }
        }
    }
