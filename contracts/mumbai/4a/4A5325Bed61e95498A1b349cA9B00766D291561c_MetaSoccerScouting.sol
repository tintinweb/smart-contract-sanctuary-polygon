...
contract EntropyStorage is AccessControl {
  ...
  event EntropySetted(uint256 indexed tokenId, uint256 indexed index, uint256 value);
  ...
  function setEntropy(uint256 _tokenId, uint256 _index, uint256 _randomness) external onlyRole(SET_ENTROPY_ROLE) {
    ...
    emit EntropySetted(_tokenId, _index, _randomness);
  }
...
contract MetaSoccerPlayers is ERC721Enumerable, EntropyStorage, TokenWithdraw, ReentrancyGuard, Ownable {
  ...
  event AttributeSetted(uint256 indexed tokenId, string indexed attribute, string value);
  event DNASetted(uint256 indexed tokenId, string value);
  ...
  function setAttribute(uint256 _tokenId, string memory _attribute, string memory _value) external onlyRole(SET_ATTRIBUTES_ROLE) {
    ...
    emit AttributeSetted(_tokenId, _attribute, _value);
  }
...
contract MetaSoccerScouting is Context, Pausable, AccessControl, IERC721Receiver, ERC721Enumerable, ReentrancyGuard, EntropyStorage, EIP712
{
    ...
    uint8 public constant requiredPlayerSeeds = 4;
