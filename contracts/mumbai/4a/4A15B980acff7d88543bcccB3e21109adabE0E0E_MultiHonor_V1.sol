...
contract MultiHonor_V1 is Initializable, IMultiHonor, AccessControlUpgradeable {
    ...
    function __initVEExpire(bool veExpire) public {
        if (veExpire) {
            veExpireTime = 7257600; // 12 weeks
        }
    }
    ...
    // returns user's average VEPower for 12 weeks after checkpoint
    function VEPower(uint256 tokenId) override view external returns(uint64) {
        if (uint64(block.timestamp) - info[tokenId].VETimestamp > veExpireTime) {
            return 0;
        }
    ...
    // @dev set average VE power for 12 weeks
