...
contract Joey is ERC20Burnable, Ownable {
    /*
...
*/
    ...
    address public polymiceAddress;
    ...
    constructor() ERC20("Joey", "JOEY") {}

    function setPolymiceAddress(address _polymiceAddress) public onlyOwner {
        polymiceAddress = _polymiceAddress;
        return;
    }
    ...
    function stakeByIds(uint256[] memory tokenIds) public {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(
                IERC721(polymiceAddress).ownerOf(tokenIds[i]) == msg.sender &&
                    tokenIdToStaker[tokenIds[i]] == nullAddress,
                "Token must be stakable by you!"
            );

            IERC721(polymiceAddress).transferFrom(
                msg.sender,
                address(this),
                tokenIds[i]
            );
