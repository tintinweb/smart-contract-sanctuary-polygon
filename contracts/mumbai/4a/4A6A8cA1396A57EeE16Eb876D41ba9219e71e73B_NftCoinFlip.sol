...
contract NftCoinFlip is VRFConsumerBaseV2, ERC721Holder, Ownable {
    enum CoinSide {
        HEAD,
        TAIL
    }
    ...
    event PlayerCreatedGame(address nftContractAddress, uint256 gameId);
    event PlayerJoinedGame(address nftContractAddress, uint256 gameId);
    event PlayerLeftGame(address nftContractAddress, uint256 gameId);
    event PlayerClaimedPrize(address nftContractAddress, uint256 gameId);
    event GameCompleted(address nftContractAddress, uint256 gameId);
    ...
    function createGame(address nftContractAddress, uint256 nftId, uint256 requiredFeeInWei, bytes32[] calldata merkleProof) public payable {
        require(isContractEnabled, "Game is currently disabled.");
        requireFee(merkleProof, nftContractAddress, requiredFeeInWei, joinFeesMerkleRoot);
        ...
        game.player1 = Player(msg.sender, nftId);
        game.status =  GameStatus.WaitingForPlayers;
        projectGames[nftContractAddress][game.id] = game;

        unchecked { ++gameCountPerProject[nftContractAddress]; }
        emit PlayerCreatedGame(nftContractAddress, game.id);
    }

    function joinGame(address nftContractAddress, uint256 gameId, uint256 nftId, uint256 requiredFeeInWei, bytes32[] calldata merkleProof) public payable {
        require(isContractEnabled, "Game is currently disabled.");
        ...
        game.player2 = Player(msg.sender, nftId);
        ...
        emit PlayerJoinedGame(nftContractAddress, gameId);
    }
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        projectGames[nftContractAddress][gameId].winner = _randomWords[0] % 2 == 0
            ? projectGames[nftContractAddress][gameId].player1
            : projectGames[nftContractAddress][gameId].player2;

        emit GameCompleted(nftContractAddress, gameId);
    }
