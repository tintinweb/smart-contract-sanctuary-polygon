...
contract ERC1155 is Context, ERC165, IERC1155 {
    ...
    function _safeBatchTransferFrom(address from,
                                    ...
                                    bytes memory data) internal virtual {
        require((ids.length != 0) && (ids.length == amounts.length), 'BERU:Incorrect collections');
        ...
        for (uint i = 0; i < ids.length; ++i) {
            require(from == ownerOf(ids[i], amounts[i]), 'BERU: Not owner');
            unchecked {
                balances[ids[i]][from]--;
            }
            balances[ids[i]][to]++;
    ...
    function _burn(address from,
        ...
        require(from == ownerOf(collection, tokenId));
        unchecked {
            balances[collection][from]--;
        }
...
contract Collections is ERC1155, Utilities {
    ...
    /**
     * @notice Only verified users function
     */
    error OnlyVerifiedUsers();
    ...
    function createCollection(uint256 royalties_,
        ...
        if ((!rolesContract.isUser(msg.sender)) && (!rolesContract.isVerifiedUser(msg.sender)))
            revert InvalidCaller();
        newName_ = Utilities.toUppercase(newName_);
        if (usedNames[newName_] == true)
            revert UsedName();
        if (!rolesContract.isVerifiedUser(msg.sender) && (royalties_ > 0))
            revert OnlyVerifiedUsers();
    ...
    function verifyCollection(uint256 collection_, bool isVerified_) public {
        ...
        if (!rolesContract.isVerifiedUser(collectionOwners[collection_])) revert OnlyVerifiedUsers();
    ...
    function setCollectionURI(uint256 collection_, string memory newBase_) public onlyCreator(collection_) {
        if (!rolesContract.isUser(msg.sender)) revert InvalidCaller();
