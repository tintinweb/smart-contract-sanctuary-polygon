...
contract StakingContract is Ownable {
    ...
    // View function to see LP amount staked by a token.
    function getTokenStakedAmount(IERC20 _stakeToken, IERC20 _rewardToken, uint256 poolIndex, uint256 _token) external view returns (uint256) {
    ...
    function getPendingRewardsOfToken(IERC20 _stakeToken, IERC20 _rewardToken, uint256 poolIndex, uint256 _token) public view returns (uint256) {
    ...
    function stakeWithPool(IERC20 _stakeToken, IERC20 _rewardToken, uint256 _amount) external payable {
        ...
        if (token.amount > 0) {
            uint256 pendingAmount = getPendingRewardsOfToken(_stakeToken, _rewardToken, poolIndex, tokenId);
            if (pendingAmount > 0) {
