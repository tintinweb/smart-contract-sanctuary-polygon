...
contract CharityAuction is ERC721URIStorage,Pausable{
    ...
    event BidStart(uint256 tokenId,address bidder,uint256 bidvalue);
    ...
    function createAuction(uint256 tokenId,uint256 _duration)external {
        ...
        emit Auction(tokenId,_duration);
    }
    function bidStart(uint256 tokenId)external payable{
        ...
        require(_exists(tokenId),"TokenId does not exists");
         ...
         require(block.timestamp <=idtodonar[tokenId].auctionend,"Auction Time's UP");
         ...
         emit BidStart(tokenId,msg.sender,msg.value);
    }
    event EndAuction(uint256 tokenId,address winner,uint256 bidvalue);
    function endAuction(uint256 tokenId) external payable{
        ...
        emit EndAuction(tokenId,tempitem.highestbidder,tempitem.highestbid);
    }
    function releaseFund(uint256 tokenId)external payable{
        ...
        (bool success,)=idtodonar[tokenId].highestbidder.call{value:idtodonar[tokenId].highestbid}("");
        require(success,"Token Transfer Failed");
    ...
    event Withdraw(uint256 tokenId,address withdrawAddress,uint256 withdrawvalue);
    function withdraw(uint256 tokenId)external payable {
        ...
        for(uint256 i=0;i<bidnumber;i++){
            if(msg.sender==idtodonar[tokenId].allbidder[i]){
                idtodonar[tokenId].allbidder[i]=idtodonar[tokenId].allbidder[bidnumber-1];
                idtodonar[tokenId].allbidder.pop();
            }else{
               require(msg.sender!=idtodonar[tokenId].allbidder[i],"You are Not a Bidder");
            }
        }             
        emit Withdraw(tokenId,msg.sender,msg.value);
    }
    function bidwinner(uint256 tokenId)public view returns (address,uint256){
       return (idtodonar[tokenId].highestbidder,idtodonar[tokenId].highestbid);
    }
    function viewallbids(uint256 tokenId) public view returns(address[] memory ){
   
        return idtodonar[tokenId].allbidder;
    }
    function reset()external{
        require(msg.sender==Contractowner || msg.sender==arbiter);
        estate=State.Created;   
            
    }
    function burnitem(uint256 tokenId) external{
        require(msg.sender==Contractowner || msg.sender==arbiter,"Only Contract Owner or Arbiter can call this fucntion");
        _burn(tokenId);
    }
