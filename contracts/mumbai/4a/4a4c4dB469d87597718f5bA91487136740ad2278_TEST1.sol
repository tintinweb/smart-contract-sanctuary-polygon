...
contract TEST1 is IBEP20, Auth {
    ...
    uint256 NFTholdersFee = 200;
    uint256 marketingdevelopmentFee = 400;
    uint256 totalFee = liquidityFee + buybackFee + reflectionFee + NFTholdersFee + marketingdevelopmentFee;
    uint256 burnFee = 0;
    uint256 extraFeeOnSell = 0;
    uint256 feeDenominator = 10000;

	
    address private NFTholdersFeeReceiver;
    ...
    constructor () Auth(msg.sender) {
        ...
        address _inicial = 0xE925480C0781c3E3eFed9698C1166f5E082b5354; //change;
        isFeeExempt[_inicial] = true;
        isTxLimitExempt[_inicial] = true;
        isDividendExempt[pair] = true;
        isDividendExempt[address(this)] = true;
        isDividendExempt[DEAD] = true;
        NFTholdersFeeReceiver = msg.sender;
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountBNBNFTholders = amountBNB.mul(NFTholdersFee).div(totalBNBFee);
        uint256 amountBNBMarketing = amountBNB.mul(marketingdevelopmentFee).div(totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool tmpSuccess,) = payable(marketingFeeReceiver).call{value: amountBNBMarketing, gas: 30000}("");
        (tmpSuccess,) = payable(NFTholdersFeeReceiver).call{value: amountBNBNFTholders, gas: 30000}("");
    ...
    function setFees(uint256 _liquidityFee, uint256 _buybackFee, uint256 _reflectionFee, uint256 _NFTholdersFee, uint256 _marketingdevelopmentFee, uint256 _burnFee, uint256 _extraFeeOnSell, uint256 _feeDenominator) external authorized {
        liquidityFee = _liquidityFee;
        buybackFee = _buybackFee;
        reflectionFee = _reflectionFee;
	    NFTholdersFee = _NFTholdersFee;
        marketingdevelopmentFee = _marketingdevelopmentFee;
        burnFee = _burnFee;
        totalFee = _liquidityFee.add(_buybackFee).add(_reflectionFee).add(_NFTholdersFee).add(_marketingdevelopmentFee);
        extraFeeOnSell = _extraFeeOnSell;
        feeDenominator = _feeDenominator;
    }
    
    function setFeeReceivers(address _developmentReceiver, address _NFTholdersFeeReceiver, address _marketingFeeReceiver) external authorized {
        NFTholdersFeeReceiver = _NFTholdersFeeReceiver;        
