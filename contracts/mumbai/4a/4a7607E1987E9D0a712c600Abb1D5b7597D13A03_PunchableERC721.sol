...
contract PunchableERC721 is ERC721WhiteListMint{
    ...
    function punchACard(string memory eventId, uint256 _tID, uint8 increment) external {
        require( msg.sender == punchBook[eventId].puncherAddress, "Not the puncher Address");
        require(nftPunchBook[_tID][eventId] + increment <= punchBook[eventId].maxPunches, "Not enough punches left");
    ...
    function getNFTPunchesPerCard(string memory eventId, uint256 _tID) external view returns (uint){
        require(_exists(_tID), "ERC721Metadata: Punch query for nonexistent token");
 ...
 *****************************************************************************************/
 function mint(address _to, uint256 _tID, uint256 _eID) external override {
   require( owner() == _msgSender() || mintingAddress == _msgSender(), "Not allowed to mint");
 ...
 *****************************************************************************************/
 function mint(address _to, uint256 _tID, uint256 _eID, string memory _Uri) external override {
   require( owner() == _msgSender() || mintingAddress == _msgSender(), "Not allowed to mint");
   super._mint(_to, _tID);
   EID[_tID] = _eID;
...
contract SDKERC721 is ERC721, Ownable{
 ...
 //NFT Punching info
 // Goal is to have punches for an NFT, starts at 0 and a punching address can punch your NFT
 // Anyone can view the number of punches on an NFT
  address puncherAddress;
  mapping (uint256 => uint256) internal nftPunches;
 ...
 *****************************************************************************************/
 function setTokenEID(uint256 _tID, uint256 _eID) external onlyOwner {
   require(_exists(_tID), "Nonexistent token");
   EID[_tID] = _eID;
 }
 ...
 *****************************************************************************************/
 function tokenEID(uint256 _tID) public view returns (uint256) {
   require(_exists(_tID), "ERC721Metadata: URI query for nonexistent token");
   return EID[_tID];
 }
 ...
 *****************************************************************************************/
 ...
 /*****************************************************************************************
     setRoyalties: Sets the royalties of the NFTs

     @param _tokenId: the Token ID of the NFT

     @param _royaltiesReceipientAddress: the Token ID of the NFT

     @param _percentageBasisPoints: the Token ID of the NFT
 ...
 *****************************************************************************************/
 function totalExperiences() public view returns (uint256) {
  ...
  /*****************************************************************************************
     @dev Sets the puncher address

     @param _puncherAddress Address of the user that can "punch" the NFT
   *****************************************************************************************/
  function setPuncher(address _puncherAddress ) external onlyOwner {
      //changes the puncher address
      puncherAddress = _puncherAddress;
  }

  /*****************************************************************************************
     @dev Updates the punches in the NFT

     @param _puncherAddress Address of the user that can "punch" the NFT

     @param increment How much to increment the punch by. This allows for more feautres where you can have
     orthogonal punches, ie the puncher can punch in the 10s and in the 1s x times when x is less than 10
     or you can use to reset to 0
   *****************************************************************************************/
  function punchNFT(uint256 _tID, uint256 increment) external {
   require( msg.sender == puncherAddress,"Not the puncher Address");
   nftPunches[_tID] += increment;
 }


    /*****************************************************************************************
        nftPunches: Returns whatever the number of NFT punches is
    ...
    *****************************************************************************************/
function getNFTPunches(uint256 _tID) public view returns (uint256) {
    require(_exists(_tID), "ERC721Metadata: Punch query for nonexistent token");
    return nftPunches[_tID];
}
...
*****************************************************************************************/
  // function _seflDestruct(uint security_value) external onlyOwner{
