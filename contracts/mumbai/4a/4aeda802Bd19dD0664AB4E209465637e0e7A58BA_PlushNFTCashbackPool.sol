...
contract PlushNFTCashbackPool is Initializable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    mapping (address => uint256[]) public idsBalances;
    mapping (uint256 => Balance) public balanceInfo;
    ...
    {
        // require(token.balanceOf(address(this)) >= lockedTokens, "Pool is empty.");
    ...
    function withdraw(uint256 _amount) external
    {
       // require(token.balanceOf(address(this)) >= _amount, "Pool is empty.");
        ...
        for(uint256 i = 0; i < idsBalances[msg.sender].length; i++){
            if(balanceInfo[i].timeIsActive < block.timestamp){
