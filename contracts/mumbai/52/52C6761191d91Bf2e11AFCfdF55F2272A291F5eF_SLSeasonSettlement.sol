...
abstract contract Reward is Score {
    ...
    function claimSeasonReward(uint256 _seasonId) external {
        ...
        if (isClaimed(_seasonId, hunter)) revert AlreadyClaimed();
        ...
        if (seasonScore.total > 0) {
            ...
            seasonScoreCollection.mint(hunter, _seasonId, seasonScore.total);
        }
...
abstract contract Score is ISLSeasonSettlement, SeasonSettlementBase {
    ...
    function getCurrentSeasonScore(
        ...
        seasonScore = SeasonScore({
            ...
            collecting: collectingScore,
            total: questScore + activityScore + collectingScore
        });
    }
    ...
    function _getHunterQuestScore(
        ...
        uint256 questScore = seasonQuestContract.getHunterQuestScore(
            _seasonId,
            _hunter
        );

        return _calculateQuestScore(questScore);
    }
    ...
    function _getHunterQuestScoreBatch(
        ...
        uint256[] memory questScores = seasonQuestContract
            .getHunterQuestScoreBatch(_seasonId, _hunters);

        uint256[] memory results = new uint256[](hunterCount);

        for (uint256 i = 0; i < hunterCount; i = i.increment()) {
            results[i] = _calculateQuestScore(questScores[i]);
        }
    ...
    function _getActivityScore(
        ...
        uint256 activityScore = gateCount * scorePerGate;

        return _calculateActivityScore(activityScore);
    }
    ...
    function _getActivityScoreBatch(
        ...
        for (uint256 i = 0; i < hunterCount; i = i.increment()) {
            results[i] = _calculateActivityScore(gateCounts[i] * scorePerGate);
        }
    ...
    function _getLatestCollectingScore(
        ...
        return _calculateCollectingScore(collectingScore);
    }
    ...
    function _getLatestCollectingScoreBatch(
        ...
        for (uint256 i = 0; i < seasonCollectionIds.length; i = i.increment()) {
            ...
            for (uint256 j = 0; j < hunterCount; j = j.increment()) {
                results[j] += _calculateCollectingScore(collectingScores[j]);
            }
        }
