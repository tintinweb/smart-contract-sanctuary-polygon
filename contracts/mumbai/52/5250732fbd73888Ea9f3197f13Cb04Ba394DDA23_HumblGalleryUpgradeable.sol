    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getRoyalties(uint256 tokenId) public view returns (uint96[] memory, address[] memory) {
        ...
        HumblNFT storage nft = HumblNFTs[tokenId];
        uint96[] memory royalty = new uint96[](nft.royaltyReceiver.length);
        address[] memory receciver = new address[](nft.royaltyReceiver.length);
        royalty = nft.royalty;
        receciver = nft.royaltyReceiver;
        return (royalty, receciver);
    }
