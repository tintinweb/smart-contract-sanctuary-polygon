...
contract GUTradeMatcher {
    ...
    {
        TradeLib.checkFinalizeOfferPreconditions(manager.cancellations(), manager, data, GUCollections(data.nftData.nftContract));
            
        TradeLib.verifyFinalizeOfferSignatures(data);

        TradeLib.transferERC20(data.nftData, data.sale.tokenAddress, data.offer.initialOfferor, data.seller, manager.beneficiary(), data.offer.amount, data.auth.fee);
    ...
    function finalizeBatchOffers(TradeLib.FinalizeOffer[] calldata data) external
    {
        for(uint i; i < data.length; i++){
            TradeLib.checkFinalizeOfferPreconditions(manager.cancellations(), manager, data[i], GUCollections(data[i].nftData.nftContract));

            TradeLib.verifyFinalizeOfferSignatures(data[i]);

            TradeLib.transferERC20(data[i].nftData, data[i].sale.tokenAddress, data[i].offer.initialOfferor, data[i].seller, manager.beneficiary(), data[i].offer.amount,  data[i].auth.fee);
    ...
    {
        TradeLib.checkPurchasePreconditions(manager.cancellations(), manager, data, GUCollections(data.nftData.nftContract));
    ...
    function batchPurchase(TradeLib.FinalizePurchase[] calldata data) external
    {
        for (uint i; i<data.length; i++)
        {
            TradeLib.checkPurchasePreconditions(manager.cancellations(), manager, data[i], GUCollections(data[i].nftData.nftContract));
    ...
    {
        TradeLib.checkFinalizeAuctionPreconditions(manager.cancellations(), manager, data, GUCollections(data.nftData.nftContract));
    ...
    function finalizeBatchAuctions(TradeLib.FinalizeAuction[] calldata data) external
    {
        for (uint i; i<data.length; i++)
        {
            TradeLib.checkFinalizeAuctionPreconditions(manager.cancellations(), manager, data[i], GUCollections(data[i].nftData.nftContract));
...
library TradeLib {
    ...
    function checkFinalizeAuctionPreconditions(address cancellationsAddress, GUTradeManager manager, FinalizeAuction calldata data, GUCollections collections) internal {
    ...
    function checkPurchasePreconditions(address cancellationsAddress, GUTradeManager manager, FinalizePurchase calldata data, GUCollections collections) internal
    {
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
    ...
    function checkFinalizeOfferPreconditions(address cancellationsAddress, GUTradeManager manager, FinalizeOffer calldata data, GUCollections collections) internal
    {
...
contract GUCollections is ERC1155Supply, ERC1155URIStorage {
    ...
    function setFinalizedListings(uint listingId) public onlyTraders {
