...
contract VotingEscrowNaive is OwnableUpgradeable {
    ...
    struct PoolMemberDetails {
        address member;
        uint256 balance;
    }
    mapping(address /*user*/ => uint256) internal poolMemberAddressLocation;
    address[] internal poolMemberAddresses;
    ...
    function _create_lock(address user, uint256 amount, uint256 till) internal {
        ...
        uint poolMembersLength = poolMemberAddresses.length;
        poolMemberAddressLocation[user] = poolMembersLength;
        poolMemberAddresses.push(user);
    ...
    function withdraw() external {
        ...
        if(poolMemberAddresses.length > 1) {
            uint userPoolMemberLocation = poolMemberAddressLocation[msg.sender];
            poolMemberAddresses[userPoolMemberLocation] = poolMemberAddresses[poolMemberAddresses.length-1];
        }
        delete(poolMemberAddressLocation[msg.sender]);
        poolMemberAddresses.pop();
...
//                });
    ...
    function getPoolMembers() external view returns (address[] memory currentMembers){
        return poolMemberAddresses;
    }

    function getAllMemberCurrentWindowBalances() external view returns (PoolMemberDetails[] memory){
        uint256 _currentWindow = currentWindow();
        PoolMemberDetails[] memory _allMemberDetails = new PoolMemberDetails[](poolMemberAddresses.length);
        for (uint i = 0; i < poolMemberAddresses.length; i++){
            PoolMemberDetails memory _temp = PoolMemberDetails(poolMemberAddresses[i], userWindowBalance[poolMemberAddresses[i]][_currentWindow]);
            _allMemberDetails[i]= _temp;
        }
        return _allMemberDetails;
    }

    function getAllMemberBalancesAtWindow(uint256 _window) external view returns (PoolMemberDetails[] memory){
        PoolMemberDetails[] memory _allMemberDetails = new PoolMemberDetails[](poolMemberAddresses.length);
        for (uint i = 0; i < poolMemberAddresses.length; i++){
            PoolMemberDetails memory _temp = PoolMemberDetails(poolMemberAddresses[i], userWindowBalance[poolMemberAddresses[i]][_window]);
            _allMemberDetails[i]= _temp;
        }
        return _allMemberDetails;
    }
