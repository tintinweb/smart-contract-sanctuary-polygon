...
contract GenesisMint is GenesisStorage {
    ...
    constructor(string memory __contractURI, address operator)
        ERC721A("Centaurify_Collection_AAA", "CENT_AAA")
    {
        _setupRole(OPERATOR_ROLE, operator);
        _setupRole(ADMIN_ROLE, _msgSender());
        _setupRole(DEVELOPER_ROLE, dev);

        _contractURI = __contractURI;
        recipient = (payable(_msgSender()));
        setDefaultRoyalty(payable(_msgSender()), 750);
        ownerMint(_msgSender(), 500);
    }
    ...
    function ownerMint(address to, uint amount) public onlyRole(ADMIN_ROLE) {
...
abstract contract GenesisStorage is AccessControl, ERC721A, ERC2981 {
    enum Status {
        Phase1,
        Phase2,
        Phase3,
    ...
    address payable internal dev = payable(address(0x30824cB687E2768d239c84B69b242D4da9808D32));
    ...
    /// @dev Not authorized to execute the method.
    error NotAuthorized();
    ...
    /// @notice Modifier is used to check if the caller is the token owner.
    /// @param tokenId the Id of the token.
    modifier isTokenOwner(uint tokenId) {
        address _tokenOwner = ownerOf(tokenId);
        if (_msgSender() != _tokenOwner) revert NotAuthorized();
        _;
    }
...
/* ------------------------------------------------------------  ADMIN FUNCTIONS  ----------------------------------------------------------- */
    ...
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setMintPrice(uint _mintPrice) external onlyRole(ADMIN_ROLE) {
        mintPrice = _mintPrice;
    }

    /// @notice Adjust the current max items per transaction.
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setMaxItemsPerTx(uint _maxItemsPerTx) external onlyRole(ADMIN_ROLE) {
    ...
    /// @notice Adjust the startTimestamp for current phase.
    /// @param _startTimestamp The unix timestamp in seconds.
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setStartTimestamp(uint _startTimestamp) external onlyRole(ADMIN_ROLE) {
        startTimestamp = _startTimestamp;
    }

    /// @notice Adjust the endTimestamp for current phase.
    /// @param _endTimestamp The unix timestamp in seconds.
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function setEndTimestamp(uint _endTimestamp) external onlyRole(ADMIN_ROLE) {
    ...
    function removeStuckTokens(address _token, address _to) external onlyRole(DEVELOPER_ROLE) {
    ...
    /// @notice Function to adjust the uriSuffix.
    /// @param _suffix The new uriSuffix.
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    ...
    /// @dev Restricted to onlyRole(ADMIN_ROLE).
    function getMerkleRoot() external view onlyRole(ADMIN_ROLE) returns (bytes32) {
        return merkleRoot;
    }
