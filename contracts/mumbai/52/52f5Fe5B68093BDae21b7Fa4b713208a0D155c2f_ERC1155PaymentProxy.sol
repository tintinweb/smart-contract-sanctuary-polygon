...
interface ERC1155SelfMinterInterface {
    struct Tier {
        uint256 start;
        uint256 end;
        bytes32 merkleRoot;
...
contract ERC1155PaymentProxy is ERC1155, ReentrancyGuard, DefaultOperatorFilterer, Ownable {
    ...
    function tierMint(
        ...
        ERC1155SelfMinterInterface(originalContract).batchMint(mintAddress, amount);
    ...
    function _mintValidation(
        ...
        ERC1155SelfMinterInterface.Tier memory tier = ERC1155SelfMinterInterface(originalContract).tiers(tierNo);
        ...
        if(ERC1155SelfMinterInterface(originalContract).mintedPerAddress(mintAddress) + amount > tier.limitPerWalletPerTier) revert MintLimitReached();
    }
}
