...
contract GooFeeding is ERC721Full {
    ...
    int256 public constant growthPercent = 5;
    int256 public constant growthLog = 21189299000000000;
            ...
            if (egg.rarity == RARITY.COMMON && egg.parent == wbid) {
                require(
                    block.timestamp - feedTime >= 24 hours ||
                        egg.spawnTime - feedTime >= 24 hours,
        ...
        returns (uint256 totalFeed)
    {
        ...
        for (uint256 i = 0; i < mbids.length; i++) {
            require(
                msg.sender == buddyContract.ownerOf(mbids[i]) ||
                    msg.sender == buddyContract.getApproved(mbids[i]),
        ...
        for (uint256 i = 0; i < mbids.length; i++) {
            uint256 balance = goo.balanceOf(mbids[i]);

            if (balance < cost) {
                goo.burn(mbids[i], balance);
                cost -= balance;
            } else {
                goo.burn(mbids[i], cost);
                break;
            }
        }
    }
    ...
    function getCost(DNA _DNA) public pure returns (uint256 GOO) {
        if (uint256(_DNA) == 0) {
            GOO = 100000 * 1e18;
    ...
    function feed(uint256[] calldata mbids, uint256 wbid)
        ...
        spawnable(wbid)
    {
        if (block.timestamp - feedTime > 24 hours) {
            feedTime += 24 hours;
        }

        WaterBear memory bear = bears[wbid];
    ...
    function getBaseMaxLevel(DNA _DNA) public pure returns (uint256 GOO) {
        if (uint256(_DNA) == 0) {
            GOO = 30;
        } else if (uint256(_DNA) == 1) {
            GOO = 35;
    ...
    function feedEgg(
        ...
        uint256 maxGoo = getGOOToLevel(eggId, maxLevel) - egg.totalFeed;
        ...
        for (uint256 i = 0; i < mbids.length; i++) {
            ...
            } else if (totalFeed + balance > cost) {
                balance = maxGoo - cost;
            }
    ...
    function getGOOToLevel(uint256 eggId, uint256 level)
        public
        view
        returns (uint256 gooToLevel)
    {
        ...
        int256 base = int256(getCost(egg.dna)).div(100);

        int256 rate = (100 + growthPercent).powu(level);
        return uint256(base.mul(rate).div(int256(100).powu(level)));
    }

    function getLevel(uint256 eggId, uint256 GOO) public view returns (uint16) {
        Egg memory egg = _data[eggId];
        ...
        int256 currentLog = PRBMathSD59x18.ln(
            int256((egg.totalFeed + GOO) / (getCost(egg.dna) / 100))
        );

        return uint16(uint256(currentLog.div(growthLog)));
    }

    function getMaxLevel(uint256 eggId) public view returns (uint256 maxLevel) {
        Egg memory egg = _data[eggId];

        maxLevel = getBaseMaxLevel(egg.dna) + (50 * uint256(egg.rarity));
    }
    ...
    function getHatcheryCost(uint256 level) public pure returns (uint256 cost) {
        if (level == 0) {
            cost = 50000 * 1e18;
        } else if (level == 1) {
            cost = 250000 * 1e18;
...
contract ERC721Full is ERC721Enumerable, ERC721URIStorage, Pausable, Ownable {
    ...
    constructor() ERC721("GOO Baby Eggs - Water Bear", "GBE") {}
    ...
    function safeMint(address to) internal returns (uint256 tokenId) {
        ...
        _setTokenURI(
            current,
            string(
                abi.encodePacked("wbegg", Strings.toString(current), ".json")
            )
        );
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://testapi.microbuddies.io/";
    }
