...
contract ERC1155Upgradeable is Initializable, Context, ERC165Upgradeable, IERC1155Upgradeable, IERC1155MetadataURIUpgradeable {
    ...
    string private _name = "PelXP";
    string private _symbol = "PelxpNFT";
    ...
    function _mint(
        ...
        _beforeTokenTransfer(operator, address(0), to, _asSingletonArray(id), _asSingletonArray(amount), bytes(data));
...
contract fractionalNft is Ownable, Pausable, ERC1155Upgradeable {
    ...
    address public s_owner;
     ...
     mapping(uint=>string) private getURI;
     event tokenURI(uint ID, string uri);

    function init (address _marketPlaceAddress,uint _royaltyAmount) external  {
    ...
    function mintTokens(address _to, uint _tokenAmount,string[] memory _uri) external  {
        require (validUsers[msg.sender],'!Valid User Call');
        for (uint i =0;i<_tokenAmount;i++) {
            currentTokenId++;
            getURI[currentTokenId]=_uri[i];
            _mint(_to,currentTokenId,1,_uri[i]);
            emit tokenURI(currentTokenId, _uri[i]);
        }
    }
