...
// File contracts/IMintingModule.sol
    ...
    IMintingModule
{
        ...
        nonReentrant
    {
        ...
        require(
            msg.value == publicCost * quantity_,
            "Not enough cost or overpayment."
        );
        require(
            quantity_ <= mintableLimitPerTX,
            "Too many tokens to mint per transaction."
        );
    ...
    function preMint(uint256 quantity_) public payable override nonReentrant {
        ...
        require(
            ...
            "Not enough tokens in white list."
        );
        require(
            msg.value == publicCost * quantity_,
            "Not enough cost or overpayment."
    ...
    function setMaxMintableQuantityPerWL(uint256 maxMintableQuantityPerWL_)
        public
        onlyOwner
    {
        maxMintableQuantityPerWL = maxMintableQuantityPerWL_;
    ...
    function setMintableLimitPerTX(uint256 mintableLimitPerTX_)
        public
        onlyOwner
    {
        mintableLimitPerTX = mintableLimitPerTX_;
        ...
        returns (string memory)
    {
        ...
        return
            string(abi.encodePacked(base, tokenId_.toString(), baseExtension));
    }

    /// @notice Set new base token uri
    /// @dev If msg sender is not owner, reverts
    /// @param baseTokenURI_ The new base token uri
    function setBaseURI(string calldata baseTokenURI_)
        external
        override
        onlyOwner
    {
    ...
    function updateWL(address addr_, uint256 maxMint_)
        public
        override
        onlyOwner
    {
    ...
    /// @dev Only owner can call this function
    /// @param list_ The list of users that add to whitelist
    function applySpecificMaxMintToMultiWL(
    ...
    /// @dev Only owner can call this function
    /// @param list_ The list of users that add to whitelist
    function setMaxMintEachWL(address[] memory list_, uint256[] memory maxMint_)
        public
        onlyOwner
    {
        for (uint256 i = 0; i < list_.length; i++) {
    ...
    function getMaxMintableQuantityPerAddress(address owner)
        public
        view
        returns (uint256)
    {
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        return (
            ...
            (_salePrice * defaultPercentageBasisPoints) /
     ...
     */
    function setRoyaltiesRecipientAddress(
        address payable newRoyaltiesRecipientAddress
    ) external onlyOwner {
        require(
            newRoyaltiesRecipientAddress != address(0),
