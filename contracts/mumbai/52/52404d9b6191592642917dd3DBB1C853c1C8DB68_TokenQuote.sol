    ...
    TokenQuoteStorage
{
    ...
    /**
     * @dev Contract key for contract registry.
     */
    bytes4 private constant _SELF_KEY = bytes4(keccak256("TokenQuote"));
    ...
    /**
     * @dev Modifier to make sure the function is called by token quote signer role.
     */
    modifier onlyTokenQuoteManager() {
        _checkTokenQuoteSigner(_msgSender());
        _;
    }
    ...
    /**
     * @dev Reverts if the provided `account` doesn't have the token quote signer role.
     * @param account The account to check ownership for.
     */
    function _checkTokenQuoteSigner(address account) internal view {
        if (!_aclContract.hasRole(Roles.TOKEN_QUOTE_SIGNER, account)) {
            revert CallerIsNotTokenQuoteSigner();
        }
    }
     ...
     * _Available since v4.1._
     */
    function _decodeQuote(bytes memory encodedQuote) internal pure returns (TokenQuote memory quote) {
        (
            quote.listingId,
            quote.renter,
            quote.warperAddress,
