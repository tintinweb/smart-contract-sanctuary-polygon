...
contract Marketplace is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function buyItemWithVoucher(
        RequestVoucher calldata _voucher,
        address _payToken,
        uint256 _price,
        uint256 _royalty, //In 10^3 (1% = 10)
        address _owner,
        address _buyer
    )
        external
        ...
        nonReentrant
    {
        require(IWhitelist(addressRegistry.whitelistContract()).hasRoleAdmin(msg.sender),"Can only be called by an Admin");

        uint256 _requestID = IMintRequests(addressRegistry.mintRequest()).createRequestFromVoucher(_voucher);
        address _nftAddress = INFTFactory(addressRegistry.factory()).createNFTContractFromRequest(
            _requestID,
            _payToken
        );
        uint256 _tokenId = INFT(_nftAddress).mintFromRequest(_requestID, _payToken, _buyer);
        IMintRequests(addressRegistry.mintRequest()).setAsMinted(_requestID);

        uint256 royaltyFee = (_royalty.mul(_price)).div(1000);

        // Royalty Payments:
        IERC20Upgradeable(_payToken).safeTransferFrom(_buyer,ITreasuryManagement(addressRegistry.treasuryManagement()).verumTreasuryAddress() , (royaltyFee.mul(10)).div(100));
        IERC20Upgradeable(_payToken).safeTransferFrom(_buyer,ITreasuryManagement(addressRegistry.treasuryManagement()).veritoneTreasuryAddress() , (royaltyFee.mul(40)).div(100));
        IERC20Upgradeable(_payToken).safeTransferFrom(_buyer, INFT(_nftAddress).creators(_tokenId), (royaltyFee.mul(50)).div(100));
        IERC20Upgradeable(_payToken).safeTransferFrom(_buyer, _owner, _price.sub(royaltyFee));
    ...
    function createOffer(
        ...
        require(IERC20Upgradeable(_payToken).allowance(_msgSender(), address(this)) >= _pricePerItem * _quantity, "Not enough Allowance given");
...
interface INFTFactory {
  ...
  function createNFTContractFromRequest ( uint256 _requestId, address _token) external returns ( address );
...
interface IMintRequests {
    ...
    function requests(uint256) external view returns (Requests calldata _request);

    function requestsData(uint256) external view returns (RequestsData calldata _requestData);
...
interface INFT{
    function creators(uint256 _tokenId) external returns(address creator);
    function mintFromRequest(uint256 _requestId, address _ERC20token, address _sender) external returns (uint tokenId);
