...
contract Luner {
	...
	function moonPhaseA() public view returns (uint256) {
	...
	function moonPhaseB() public view returns (uint256) {
		uint256 timestamp = block.timestamp;
		uint256 lastNewMoon = 947163600 gwei; // january 6th 2000 -- gwei adds 9 zeros we will use as for decimals
		uint256 daysSince = ((timestamp * 1 gwei) - lastNewMoon) / 86400 gwei; // 86400 seconds in a day
		// emit DaysSince(daysSince);
		...
		uint256 phase = ((daysSince * 1 gwei)- 2451550.1 gwei) / 29.53058885 gwei;
    ...
    function moonPhaseC() public view returns (string memory) {
		uint256 timestamp = block.timestamp;
		uint256 lastNewMoon = 947163600 gwei; // january 6th 2000 -- gwei adds 9 zeros we will use as for decimals
		uint256 daysSince = ((timestamp * 1 gwei) - lastNewMoon) / 86400 gwei; // 86400 seconds in a day
		// emit DaysSince(daysSince);
