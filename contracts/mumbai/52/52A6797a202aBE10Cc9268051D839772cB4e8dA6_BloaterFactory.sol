 ...
 */
contract Bloater is IXReceiver, ERC20 {
  // The connext contract deployed on the same domain as this contract
  IConnext public immutable connext;
  // The target Bloater contract deployed on the target domain
  address public immutable target;
  // The destination domain ID, as defined in the Connext config
  uint32 public immutable destinationDomain;

  constructor(IConnext _connext, address _target, uint32 _destinationDomain, string memory _name, string memory _symbol) ERC20(_name, _symbol, 18) {
    connext = _connext;
    target = _target;
   ...
   * @param mintNumber The amount of bloating tokens to mint on the target chain.
   */
  function bloat(uint256 relayerFee, uint256 mintNumber) public payable {
    // Include the relayerFee so Bloater will use the same fee 
    // Include the address of this contract so Bloater knows where to send the "callback" to mint the tokens
    // Include the msg.sender so we know who to mint tokens to when we receive the callback
    bytes memory _callData = abi.encode(msg.sender, address(this), relayerFee, mintNumber);
   ...
   * @dev The "callback" function for this example. Will be triggered after the native Bloater xcalls back.
   */
  function xReceive(
    bytes32 _transferId,
    uint256 _amount,
    ...
    require(
      _origin == destinationDomain &&
        _originSender == target &&
        msg.sender == address(connext),
      "Expected source contract on origin domain called by Connext"
    );

    // Decode information coming from the target bloater through the Connext relayer
    (address _minter, uint256 _mintNumber) = abi.decode(_callData, (address, uint256));

    // Mint tokens to the address that sent the original xcall
    _mint(_minter, _mintNumber);
...
import {IConnext} from "@connext/smart-contracts/contracts/core/connext/interfaces/IConnext.sol";
import {Bloater} from "./Bloater.sol";

/**
 * @title Ping
 * @notice Ping side of a PingPong example.
 */
contract BloaterFactory {
  // Mapping associating a chain ID with a Bloater contract
  mapping (uint256 => Bloater) public bloaterAddresses;

  // Since the deployment of bloaters is not atomic, we should have an owner, so this factory can be trusted
  ...
  /** 
   * @notice Creates a new bloater token contract and stores its address.
   * @param _connext Address of the Connext contract in this destination domain.
   * @param _target Address of the Bloater contract on the destination domain.
   * @param _destinationDomain The destination domain ID.
   * @param _name The name of the token.
   * @param _symbol The symbol of the token.
   * @param _chainID The chain ID of the network the bloater will be deployed on.
   */
  function createBloater(
      IConnext _connext,
      address _target,
      uint32 _destinationDomain,
      string memory _name,
      ...
      uint256 _chainID
        ...
        public
    {
      // Make the creation function permissioned
      require(msg.sender == owner, "Only the owner can create new bloaters");

      // Create a new `Bloater` contract and return its address.
      Bloater bloaterAddress = new Bloater(_connext, _target, _destinationDomain, _name, _symbol);

      // Store the address of the new bloater contract
      bloaterAddresses[_chainID] = bloaterAddress;
    }
}
