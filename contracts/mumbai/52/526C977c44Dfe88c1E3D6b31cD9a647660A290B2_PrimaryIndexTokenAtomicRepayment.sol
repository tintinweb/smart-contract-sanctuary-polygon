...
contract PrimaryIndexTokenAtomicRepayment is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getCurrentDepositedRemaining(address account, address projectToken, address lendingToken) public view returns (uint256 depositedRemaining) {
        uint depositedAmount = primaryIndexToken.getDepositedAmount(account, projectToken);
        uint depositedAmountInUSD = primaryIndexToken.getTokenEvaluation(projectToken,depositedAmount);
        uint outstandingAmount = getTotalOutstanding(account, projectToken, lendingToken);
        uint outstandingInUSD = primaryIndexToken.getTokenEvaluation(lendingToken, outstandingAmount);
        depositedRemaining = depositedAmountInUSD > outstandingInUSD ? depositedAmountInUSD - outstandingInUSD : 0;
    }

    function getCurrent(address account, address projectToken, address lendingToken) public view returns (uint256, uint,uint,uint,uint ) {
        uint depositedAmount = primaryIndexToken.getDepositedAmount(account, projectToken);
        uint depositedAmountInUSD = primaryIndexToken.getTokenEvaluation(projectToken,depositedAmount);
        uint outstandingAmount = getTotalOutstanding(account, projectToken, lendingToken);
        uint outstandingInUSD = primaryIndexToken.getTokenEvaluation(lendingToken, outstandingAmount);
        uint256 depositedRemaining = depositedAmountInUSD > outstandingInUSD ? depositedAmountInUSD - outstandingInUSD : 0;
        return (depositedAmount, depositedAmountInUSD, outstandingAmount, outstandingInUSD, depositedRemaining);
    }
