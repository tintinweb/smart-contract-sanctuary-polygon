...
contract BoundlessAdapter is Context, ERC2771Context, AccessControlEnumerable {
            ...
            for (uint256 i = 0; i < contractCalls.length; i++) {
                ...
                if (i == 0) {
                    ...
                    if (approvalRemaining > 0) {
                        ...
                        token.approve(contractCalls[i].contractAddress, 0);
                    }
                }

                if (contractCalls[i].transactedToken != address(0x0)) {
                    uint256 transactedTokenBalanceAfter = IERC20(
                        contractCalls[i].transactedToken
                    ).balanceOf(address(this));
                    uint256 transactedTokenReceived = transactedTokenBalanceBefore -
                            transactedTokenBalanceAfter;
                    if (transactedTokenReceived > 0) {
                        IERC20(contractCalls[i].transactedToken).transfer(
                            fallbackRecipient,
    ...
    function mint(
        ...
        if (borrowAsset) {
            ...
            require(
                _msgSender() == recipient || hasRole(RELAYER, _msgSender()),
                "Bridge: only recipient or relayer can submit"
            );
        }
        require(fee <= ((amount * maxFeeBips) / 10000), "Bridge: invalid fee");
        ...
        token.transfer(recipient, amount - fee);
