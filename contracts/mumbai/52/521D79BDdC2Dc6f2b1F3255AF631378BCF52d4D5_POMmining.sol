...
contract POMmining is ReentrancyGuard, ERC1155Holder {
    //Store twee contracten (adoptie drop en token)
    DropERC1155 public immutable detectiveJames;
    DropERC1155 public immutable adoptionNftCollection;
    TokenERC20 public immutable rewardsToken;

    //Functie voor rewards en adoptie collectie addresses
    constructor(DropERC1155 jamesContractAddress, DropERC1155 adoptionContractAddress, TokenERC20 gemsContractAddress) {
        detectiveJames = jamesContractAddress;
        adoptionNftCollection = adoptionContractAddress;
    ...
    //Mapping van spelers addressen tot hun adoptie
    //Met default, speler geen adoptie. Zij zullen niet in de mapping zitten.
    //Mapping van address tot adoptie is niet gezet tot deze gestaked is.
    //In het voorbeeld, het tokenId of adoptie is vermeningvuldig voor rewards.
    mapping (address => James) public playerJames;
    mapping (address => MapValue) public cardAdoption;

    //Mapping van spelers address tot laaste keer claim 
    //Met default, speler heeft geen laatste tijd. Wordt niet gemapt.
    mapping (address => James) public playerLastUpdate;
    mapping (address => MapValue) public cardLastUpdate;

    function stake(uint256 _tokenId) external nonReentrant {
        require(detectiveJames.balanceOf(msg.sender, _tokenId) >= 1, "You must have at least 1 character!");
        require(adoptionNftCollection.balanceOf(msg.sender, _tokenId) >= 1, "You must have at least 1 AdoptionCard!");

        if (playerJames[msg.sender].isData) {
            detectiveJames.safeTransferFrom(address(this), msg.sender, playerJames[msg.sender].value, 1, "Return James.");
        }

        if (cardAdoption[msg.sender].isData) {
            adoptionNftCollection.safeTransferFrom(address(this), msg.sender, cardAdoption[msg.sender].value, 1, "Returning your card.");
        }

        //Bereken rewards voor staking en uitbetalen
        uint256 reward = calculateRewards(msg.sender);
        rewardsToken.transfer(msg.sender, reward);

        //Transfer card naar contract
        detectiveJames.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Stake James.");
        adoptionNftCollection.safeTransferFrom(msg.sender, address(this), _tokenId, 1, "Staking your Card.");

        playerJames[msg.sender].value = _tokenId;
        playerJames[msg.sender].isData = true;
        
        cardAdoption[msg.sender].value = _tokenId;
        cardAdoption[msg.sender].isData = true;


        //Update playerlastupdate mapping
        playerLastUpdate[msg.sender].isData = true;
        playerLastUpdate[msg.sender].value = block.timestamp;

        cardLastUpdate[msg.sender].isData = true;
        cardLastUpdate[msg.sender].value = block.timestamp;


    }

    function withdraw() external nonReentrant {
        require(playerJames[msg.sender].isData, "You do not have a James to withdraw.");
        require(cardAdoption[msg.sender].isData, "You do not have a card to withdraw.");

        //Berekening staking rewards
        uint256 reward = calculateRewards(msg.sender);
        rewardsToken.transfer(msg.sender, reward);

        //Stuur James of Card terug naar speler
        detectiveJames.safeTransferFrom(address(this), msg.sender, playerJames[msg.sender].value, 1, "Return James");
        adoptionNftCollection.safeTransferFrom(address(this), msg.sender, cardAdoption[msg.sender].value, 1, "Return your card");

        //update mapping
        playerJames[msg.sender].isData = false;
        cardAdoption[msg.sender].isData = false;
    ...
    function claim() external nonReentrant {
        //Bereken wat ze verdiend hebben en uitbetalen
        uint256 reward = calculateRewards(msg.sender);
        rewardsToken.transfer(msg.sender, reward);
    ...
    // Internal
    //Reken de rewards uit wanner voor laatst is uitbetaald
    //Rewards rate is 20.000.000 per block
    //Dit wordt gegenereerd door block.timestamp en playerLastUpdate
    //Als playerLastUpdate of playerAdoption niet is gezet dan geen rewards 
    function calculateRewards(address _player)
        public
        view
        returns (uint256 _rewards)
    {
        //Als playerLastUpdate of playerAdoption niet is gestaked dan geen rewards
        //James
        if (!playerLastUpdate[_player].isData || !playerJames[_player].isData) {
            return 0;
        } 

        //AdoptieCard
        if (!cardLastUpdate[_player].isData || !cardAdoption[_player].isData) {
            return 0;
        }

        //Bereken tijd verschil tussen nu en laatste keer staking/uitbetalen/claim
        uint256 timeDifference = block.timestamp - playerLastUpdate[_player].value;

        //Bereken rewards het verschuldigt is, 10trillion is gebasseerd op 18 decimals systeem
        uint256 rewards = timeDifference * 10_000_000_000_000 * (playerJames[_player].value + 1); (cardAdoption[_player].value + 1);
