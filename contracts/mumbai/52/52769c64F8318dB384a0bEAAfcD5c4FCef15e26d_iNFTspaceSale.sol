...
contract iNFTspaceSale is Ownable {
    ...
    struct SaleToken {
        ...
        address Address; // sale token contract address
        uint8   Type;    // sale token type: 0-721 1-1155
    ...
    PayCurrency [] public _supportCurrencies;
    ...
    event RegPayCurrency(address currency);
    event DisablePayCurrency(uint256 saleTokenId);
    event EnablePayCurrency(uint256 saleTokenId);

    event RegSaleToken(uint256 saleTokenId, address saleTokenAddr, uint8 saleTokenType);
    ...
    constructor(address  payable _platformRecipient, uint256 _platformFeeRatio){
        ...
        regPayCurrency(address(0)); // By default, platform currency is supported
    ...
    function list(
        ...
        require(_payCurrencyValidityById(_currencyId) == true, "iNFTspaceSale:currency must supported");
        ...
        require(_startTime >= block.timestamp, "iNFTspaceSale:saleToken sell should be the future");
        require(_stopTime > _startTime || _stopTime == 0, "iNFTspaceSale:saleToken sell time should be ok");
        if (_stopTime != 0) {
            require((_stopTime - _startTime) >=  maxSellingPeriod, "iNFTspaceSale:does not meet the maximum selling period");
        }

        uint256 assetId = _calculateAssetId(msg.sender, saleToken.Type, _tokenId, _nonce);

        if(saleToken.Type == SALE_TOKEN_ERC721) { // ERC721
            IERC721 iSaleToken = IERC721(saleToken.Address);
            ...
            IERC1155 iSaleToken = IERC1155(saleToken.Address);
            require(iSaleToken.balanceOf(msg.sender, _saleTokenId) >= 1, "iNFTspaceSale:self insufficient balance");
    ...
    function unList(uint256 _saleTokenId, uint256 _assetId) public{
        ...
        if(saleToken.Type == SALE_TOKEN_ERC721) { // ERC721
            IERC721 iSaleToken = IERC721(saleToken.Address);
            iSaleToken.safeTransferFrom(address(this), msg.sender, _sellingNFTs[_saleTokenId][_assetId].tokenId);
        } else {
            IERC1155 iSaleToken = IERC1155(saleToken.Address);
            iSaleToken.safeTransferFrom(address(this), msg.sender, _sellingNFTs[_saleTokenId][_assetId].tokenId, 1, "");
        }
    ...
    function buy(uint256 _saleTokenId, uint256 _assetId) public payable{
        ...
        SaleToken memory saleToken = getSaleTokenById(_saleTokenId);
        Selling memory s = _sellingNFTs[saleToken.id][_assetId];
        PayCurrency memory c = getPayCurrencyById(s.currencyId);
    ...
    function getSalerListNonce(address _saler) public view returns (uint256) {
        return _salerListNonce[_saler];
    }

    function validitySellingNFTsLength() public view returns (uint256) {
        return _sellingNFTsId.length;
    }

    function getSellingNFTs() public view returns (Selling[] memory) {
        uint256 len = _sellingNFTsId.length;

        Selling[] memory sellList = new Selling[](len);
        for(uint256 i = 0; i < len; i++){
            sellList[i] = _sellingNFTs[_sellingNFTsId[i].saleTokenId][_sellingNFTsId[i].assetId];
        }
        return sellList;
    }

    function iterValiditySellingNFTs(uint256 _start, uint256 _end) public view returns(Selling[] memory){
        uint256 len = _sellingNFTsId.length;
        if(len < _end ){
            _end = len;
        }
        Selling[] memory sellList = new Selling[](_end - _start);
        for(uint256 i = _start; i < _end; i++){
            sellList[i-_start] = _sellingNFTs[_sellingNFTsId[i].saleTokenId][_sellingNFTsId[i].assetId];
        }
        return sellList;
    }

    function getSaleTokenId(address _saleTokenAddress) public view returns (uint256) {
        for(uint i=0; i<_supportSaleTokens.length; i++){
            if(_supportSaleTokens[i].Address == _saleTokenAddress){
                return _supportSaleTokens[i].id;
            }
        }
        return 0;
    }
    ...
    function getSaleTokenById(uint256 _saleTokenId) public view returns (SaleToken memory) {
        ...
        return SaleToken({id: 0, Address: address(0), Type: 0, validity:false});
    }

    function getSaleTokenByAddress(address _saleToken) public view returns (SaleToken memory) {
        for(uint i=0; i<_supportSaleTokens.length; i++){
            if(_supportSaleTokens[i].Address == _saleToken){
                return _supportSaleTokens[i];
            }
        }
        return SaleToken({id: 0, Address: address(0), Type: 0, validity:false});
    ...
    function getSupportSaleTokens() public view returns (SaleToken [] memory) {
        return _supportSaleTokens;
    }

    function getPayCurrencyById(uint256 _currencyId) public view returns (PayCurrency memory) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].id == _currencyId){
                return _supportCurrencies[i];
            }
        }
        return PayCurrency({id: 0, currency: IERC20(address(0)), validity:false});
    }

    function getPayCurrencyByAddress(address _currency) public view returns (PayCurrency memory) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].currency == IERC20(_currency)){
                return _supportCurrencies[i];
            }
        }
        return PayCurrency({id: 0, currency: IERC20(address(0)), validity:false});
    ...
    function getPayCurrencies() public view returns (PayCurrency [] memory) {
    ...
    function regSaleToken(address _saleToken, uint8 _type) public onlyOperator{
        ...
        require(_type == SALE_TOKEN_ERC721 || _type == SALE_TOKEN_ERC1155, "iNFTSpaceSale:Invalid saleToken type");
        ...
        saleToken.Address = _saleToken;
        saleToken.Type = _type;
        saleToken.validity = true;

        _supportSaleTokens.push(saleToken);

        emit RegSaleToken(saleToken.id, saleToken.Address, saleToken.Type);
    }
    ...
    function regPayCurrency(address _currency) public onlyOperator{
        require(_payCurrencyByAddress(_currency) == false, "iNFTSpaceSale:currency already add yet");

        PayCurrency memory currency;
        if(_supportCurrencies.length == 0) {
            currency.id = 1;
            currency.currency = IERC20(_currency);
            currency.validity = true;
        }else{
            currency.id = _supportCurrencies[_supportCurrencies.length - 1].id + 1;
            currency.currency = IERC20(_currency);
            currency.validity = true;
        }

        _supportCurrencies.push(currency);

        emit RegPayCurrency(_currency);
    }

    function disablePayCurrency(uint256 _currencyId) public onlyOperator{
        require(_payCurrencyExistsById(_currencyId) == true, "iNFTSpaceSale:currency must add yet");

        for(uint i=0; i< _supportCurrencies.length; i++){
            if(_supportCurrencies[i].id == _currencyId){
                _supportCurrencies[i].validity = false;
            }
        }

        emit DisablePayCurrency(_currencyId);
    }

    function enablePayCurrency(uint256 _currencyId) public onlyOperator{
        require(_payCurrencyExistsById(_currencyId) == true, "iNFTSpaceSale:currency must add yet");

        for(uint i=0; i< _supportCurrencies.length; i++){
            if(_supportCurrencies[i].id == _currencyId){
                _supportCurrencies[i].validity = true;
            }
        }

        emit EnablePayCurrency(_currencyId);
    }

    function _payCurrencyByAddress(address _currency) internal view returns (bool) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].currency == IERC20(_currency)){
                return true;
            }
    ...
    function _payCurrencyExistsById(uint256 _currencyId) internal view returns (bool) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].id == _currencyId){
                return true;
            }
    ...
    function _payCurrencyValidityById(uint256 _currencyId) internal view returns (bool) {
        for(uint i=0; i<_supportCurrencies.length; i++){
            if(_supportCurrencies[i].id == _currencyId && _supportCurrencies[i].validity == true ){
                return true;
            }
        }
        return false;
    ...
    function _calculateAssetId(address _sealer, uint256 _assetType, uint256 _tokenId, uint256 _nonce) internal pure returns (uint256) {
        return uint256(keccak256(abi.encodePacked(_sealer, _assetType, _tokenId, _nonce)));
    }
