...
contract Trading is Ownable {
    ...
    function invest(uint256 _amount, string memory _password) external {
                require(!depositOff, "Deposit is Currently off");
    ...
    function weeklyWithdraw() external {
        ...
        require(
            userInvestment[msg.sender].invested,
    ...
    function toggleDepositOff(bool _val) external onlyOwner {
        depositOff = _val;
    }
    ...
    function updateMaxUserInvestment(uint256 _maxUserInvestment) external onlyOwner {
    ...
    function whitelistAddress(address _addr, bool _whitelist) external onlyOwner {
        isWhitelisted[_addr] = _whitelist;
    }
    function blacklistAddress(address _addr, bool _blacklist) external onlyOwner {
        isBlacklisted[_addr] = _blacklist;
    }
