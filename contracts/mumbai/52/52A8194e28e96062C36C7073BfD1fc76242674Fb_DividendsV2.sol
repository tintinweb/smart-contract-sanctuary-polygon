...
/*
 * This contract is used to distribute dividends to users that allocated xGrail here
 *
 ...
 * This distribution slot is the source of the dividends distribution to xGrail allocators during the current cycle
 *
 * This transfer from the pending slot to the distribution slot is based on cycleDividendsPercent and CYCLE_PERIOD_SECONDS
 *
 */
contract DividendsV2 is Ownable, ReentrancyGuard, IXGrailTokenUsage, IDividendsV2 {
  using SafeMath for uint256;
  ...
  address public immutable xGrailToken; // xGrailToken contract

  mapping(address => uint256) public usersAllocation; // User's xGrail allocation
  ...
  constructor(address xGrailToken_, uint256 startTime_) {
    require(xGrailToken_ != address(0), "zero address");
  ...
  modifier xGrailTokenOnly() {
    require(msg.sender == xGrailToken, "xGrailTokenOnly: caller should be XGrailToken");
    _;
  }
  ...
  /**
   * Allocates "userAddress" user's "amount" of xGrail to this dividends contract
   *
  ...
  /**
   * Deallocates "userAddress" user's "amount" of xGrail allocation from this dividends contract
   *
   * Can only be called by xGrailToken contract, which is trusted to verify amounts
   * "data" is only here for compatibility reasons (IxGrailTokenUsage)
   */
  ...
  */
  ...
  function _updateDividendsInfo(address token) internal {
    ...
    // if no xGrail is allocated or initial distribution has not started yet
  ...
  function _harvestDividends(address token) internal {
    ...
    uint256 userXGrailAllocation = usersAllocation[msg.sender];
    uint256 pending = user.pendingDividends.add(
      userXGrailAllocation.mul(accDividendsPerShare).div(1e18).sub(user.rewardDebt)
    );

    user.pendingDividends = 0;
    user.rewardDebt = userXGrailAllocation.mul(accDividendsPerShare).div(1e18);
