...
interface IERC20 {
   ...
   /** function blockedBalanceOf(address account) external view returns (uint256);
...
contract Pausable {
    event PausedTest(address indexed account);
    event UnpausedTest(address indexed account);
...
contract ERC20Destroyable {
    event DestroyedTest(address indexed caller, address indexed account, address indexed contractAddress);

    function destroy(address payable to) public {
        emit DestroyedTest(msg.sender, to, address(this));
        selfdestruct(to);
    }
}
...
contract TestToken is
    ...
    ERC20Destroyable
{
    event MintedTest(address indexed minter, address indexed account, uint256 value);
    event BurnedTest(address indexed burner, uint256 value);
    event BurnedForTest(address indexed burner, address indexed account, uint256 value);
    event ConfiscatedTest(address indexed account, uint256 amount, address indexed receiver);
    event NameUpdatedTest(address indexed account, string name);
    event SymbolUpdatedTest(address indexed account, string symbol);
    event ClassUpdatedTest(address indexed account, string class);
    event IssuerUpdatedTest(address indexed account, address indexed issuer);
    event CategoryUpdatedTest(address indexed account, bytes4 category);
    event TokenURIUpdatedTest(address indexed account, string tokenURI);
    ...
    function confiscate(
        ...
        emit ConfiscatedTest(confiscatee, amount, receiver);
    }
    ...
    function burn(uint256 _amount) public {
        ...
        emit BurnedTest(msg.sender, _amount);
    }
    ...
    function burnFor(address _account, uint256 _amount) public {
        ...
        emit BurnedForTest(msg.sender, _account, _amount);
    }
    ...
    function burnFrom(address _account, uint256 _amount) public {
        ...
        emit BurnedTest(_account, _amount);
    }
     ...
     */
    function testMint(address _account, uint256 _amount) public {
        super._mint(_account, _amount);
        emit MintedTest(msg.sender, _account, _amount);
    }
    ...
    function batchMint(address[] memory _accounts, uint256[] memory _amounts) public {
        ...
        require(_accounts.length <= BATCH_LIMIT, "SygnumToken: batch count is greater than BATCH_LIMIT.");
        for (uint256 i = 0; i < _accounts.length; i++) {
            testMint(_accounts[i], _amounts[i]);
        }
    }
    ...
    function updateName(string memory newName)  public {
        _name = newName;
        emit NameUpdatedTest(msg.sender, newName);
    } 
    ...
    function updateSymbol(string memory newSymbol) public {
        _symbol = newSymbol;
        emit SymbolUpdatedTest(msg.sender, newSymbol);
    }
    ...
    function updateClass(string memory newClass) public {
        _class = newClass;
        emit ClassUpdatedTest(msg.sender, newClass);
    }
    ...
    function updateIssuer(address newIssuer) public {
        _issuer = newIssuer;
        emit IssuerUpdatedTest(msg.sender, newIssuer);
    }
    ...
    function updateCategory(bytes4 newCategory) public {
        ...
        emit CategoryUpdatedTest(msg.sender, newCategory);
    }
    ...
    function updateTokenURI(string memory newTokenURI) public {
        ...
        emit TokenURIUpdatedTest(msg.sender, newTokenURI);
    }
