...
contract maticTest is ERC721Enumerable, Ownable, ERC721URIStorage {
    ...
    constructor() ERC721("TEST1 Domain Name Service", "TDNS") {
        priceFeed = AggregatorV3Interface(0x7bAC85A8a13A4BcD8abb3eB7d6b4d632c5a57676);
    }
    ...
    function domainPrice(string memory domainName, uint256 year) public view returns(uint256){
        ...
        if (lenCount <= 3){
            domainPriceValue = ((threeLetter * 10 ** 20) / getLatestPrice()).toUint256();
        }
        else if(lenCount == 4){
            domainPriceValue = ((fourLetter * 10 ** 20) / getLatestPrice()).toUint256();
        }
        else if(lenCount >= 5){
            domainPriceValue = ((fiveLetter * 10 ** 20) / getLatestPrice()).toUint256();
        }
