...
import "./interfaces/IWitsCore.sol";

contract Comet is Base {
    bytes32 private _merkleTreeRoot;
    mapping(address => uint256) internal _whitelistMinted;

    function initialize(
        Args memory args, 
        IERC721ABurnableUpgradeable _gBaby, 
        uint256[] memory ogs,
        IERC721ABurnableUpgradeable _quillAndInk
    ) public initializer {
        __Base_init(args);
        gBaby = _gBaby;
        quillAndInk = _quillAndInk;

        for(uint256 i; i < ogs.length; i++) {
            uint256 id = ogs[i];
            _isOg[id] = true;
        }
    }
    ...
    function whitelistMint(
        ...
        payable
    {
        require(stage == 1, "Whitelist sale not enabled");
        uint8 _stage = 1;
        require(MerkleProofUpgradeable.verify(
            proof_, _merkleTreeRoot, keccak256(abi.encodePacked(maxAmount, msg.sender))
        ), "Not whitelisted");
        uint256 minted = _whitelistMinted[msg.sender] + amount_;
        require(minted <= maxAmount, "Exceeds limit");
        _whitelistMinted[msg.sender] = minted;
        _callMint(msg.sender, amount_);
        _handlePayment(amount_ * price(_stage));
    }

    function currentClaimBalance(uint256, address account, uint256) public view returns (uint256) {
        return _whitelistMinted[account];
    }

    function remainingClaimBalance(uint256, address account, uint256 maxAmount_) public view returns (uint256) {
        uint256 minted = _whitelistMinted[account];
        if(minted == maxAmount_) return 0;
        else return maxAmount_ - minted;
    }

    function publicMint(uint256 amount_) external payable {
        require(stage == 2, "Public sale not enabled");
        uint8 _stage = 2;
    ...
    mapping (uint256 => bool) _isOg;
    IERC721ABurnableUpgradeable public gBaby;

    function isOg(uint256 id) public view returns (bool) {
        return _isOg[id];
    }

    function gBabyMintable(uint256[] memory ids) public view returns (uint256) {
        uint256 total;
    ...
    function gbabyMint(uint256[] memory ids) external {
        require(stage == 3, "Not Live");
        uint256 total;
        for(uint256 i; i < ids.length; i++) {
            uint256 id = ids[i];
            require(gBaby.ownerOf(id) == msg.sender, "Not Owner");
            if(_isOg[id]) {
                total += 2;
            } else {
                total++;
    ...
    function changeGBaby(IERC721ABurnableUpgradeable _gBaby) external onlyAdmin {
        gBaby = _gBaby;
    }

    function setOGs(uint256[] memory ogs) external onlyAdmin {
        for(uint256 i; i < ogs.length; i++) {
            uint256 id = ogs[i];
            _isOg[id] = !_isOg[id];
        }
    ...
    IERC721ABurnableUpgradeable public quillAndInk;

    function mint(uint256[] memory ids) external payable {
        require(stage == 3, "Not Live");     
        uint256 length = ids.length;
        require(length % 2 == 0, "Need to burn multiples of 2");
        uint256 total = length / 2;
        for(uint256 i; i < length; i++) {
            uint256 id = ids[i];
            require(quillAndInk.ownerOf(id) == msg.sender, "Not Owner");
            quillAndInk.burn(id);
        }
        _callMint(msg.sender, total);
        _handlePayment(total * price(1));
    }

    function mintable(uint256[] memory ids) public pure returns (uint256) {
        uint256 total = ids.length / 2;
        return total;
    }

    function changeQuillAndInk(IERC721ABurnableUpgradeable _quillAndInk) external onlyAdmin {
        quillAndInk = _quillAndInk;
    }

    bool public burnLive;
    IWitsCore public witsCore;

    function setBurnLive(bool live) external onlyAdmin {
        burnLive = live;
    }

    function setWitsCore(IWitsCore _witsCore) external onlyAdmin {
        witsCore = _witsCore;
    }

    function mintComet(uint256[] memory ids) external {
        require(burnLive, "not live");
        uint256 length = ids.length;
        for(uint256 i; i < length; i++) {
            uint256 id = ids[i];
            _burn(id, true);
        }        
        witsCore.mint(msg.sender, length);
    }

    function updateMerkleTreeRoot(bytes32 merkleTreeRoot_)
        public
        onlyAdmin
    {
