...
// File: Astralix/claim_astralix.sol
...
contract Claim {
    ...
    function getTokensClaimableByAddress(address wallet) public view returns (uint) {
        uint nftCount = IERC721(nftContractAddress).balanceOf(wallet);
        uint claimableTokens = 0;
        
        for (uint i = 0; i < nftCount; i++) {
            uint tokenId = IERC721Enumerable(nftContractAddress).tokenOfOwnerByIndex(wallet, i);
            if (!blockedTokens[tokenId]) {
