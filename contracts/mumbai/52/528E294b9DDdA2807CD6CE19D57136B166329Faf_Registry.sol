...
contract Registry {
    ...
    event LogC(address indexed sender, string name, address contractAddress, Coupon coupon);
    ...
    modifier onlyIfNotENSOwner(string memory name, Coupon memory coupon) {
        ...
        emit LogC(msg.sender, name, address(this), coupon);
        // if (!isVerifiedCoupon(digest, coupon)) {
        //     revert InvalidCoupon({ name: name, sender: msg.sender, contractAddress: address(this) });
        // }
        _;
    }
    ...
    //     require(initMap != address(0));
    //     map = initMap;
    //     require(initAdminSigner != address(0));
    //     adminSigner = initAdminSigner;
    //     claimed = 0;
    //     emit Hello();
    // }
    constructor(
    ...
    /// @dev set adminsigner
    function setAdminSigner(address verifierAdderss) external {
     ...
     */
    function changePhilandAddress(address phiMapAddress) external {
        require(phiMapAddress != address(0));
        map = phiMapAddress;
