    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(address => mapping(address => uint256)) public maxTotalShotAssetCount; //prj=>user=>uint256
    mapping(address => mapping(address => uint256)) public currentShotAssetCount;//prj=>user=>uint256
    ...
    function _leveragedBorrow(
...

        {
            ...
            currentShotAssetCount[borrower][lendingToken] += lendingTokenCount;
            maxTotalShotAssetCount[borrower][lendingToken] = currentShotAssetCount[borrower][
                lendingToken
            ];
    ...
    function reduceShotAsset(
        ...
        uint256 totalShotAssetCountOfLendingToken = currentShotAssetCount[account][
            lendingToken
        ];
        if (totalBorrowCount < totalShotAssetCountOfLendingToken) {
            currentShotAssetCount[account][lendingToken] = totalBorrowCount;
            PositionData[] storage _positionDatas = positionData[account];
            for (uint256 i = 0; i < _positionDatas.length; i++) {
                PositionData storage _positionData = _positionDatas[i];
    ...
    function addShotAsset(
        ...
        uint256 totalBorrowCount = borrowPosition.loanBody;
        uint256 currentShotAssetCountOfLendingToken = currentShotAssetCount[account][
            lendingToken
        ];
        uint256 maxShortAssetCountOfLendingToken = maxTotalShotAssetCount[account][
            lendingToken
        ];
        if (
