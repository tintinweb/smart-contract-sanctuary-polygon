...
contract ONFTBridge is NonblockingLzApp {
    bytes public constant PAYLOAD = "\x01\x02\x03\x04";

    uint256 public pricePerTokenTransfer = 0.0001 ether;
    address public commissionWallet = 0x86fc9DbcE9e909c7AB4D5D94F07e70742E2d144A;
    ...
    function _nonblockingLzReceive(uint16, bytes memory, uint64, bytes memory _payload) internal override {
        ...
        for(uint i=0;i<tokenIds.length;i++)
            IERC721(_dstToken).safeMint(toAddr, tokenIds[i]);
    }
    ...
    function traverse(uint16 _dstChainId, address _srcToken, address _dstToken, uint[] memory tokenIds) public payable {
        uint256 value = msg.value;
        require(pricePerTokenTransfer * tokenIds.length > value, "Insufficient value sent for gas fee and platform fee.");

        for(uint i=0;i<tokenIds.length;i++)
            IERC721(_srcToken).burn(tokenIds[i]);

        payable(commissionWallet).transfer(pricePerTokenTransfer * tokenIds.length);

        bytes memory payload = abi.encode(msg.sender, tokenIds, _dstToken);
        _lzSend(_dstChainId, payload, payable(msg.sender), address(0x0), bytes(""), msg.value - (pricePerTokenTransfer * tokenIds.length));
    }
...
interface IERC721 is IERC165 {
    ...
    function safeMint(address recAddr, uint256 tokenId) external;
}