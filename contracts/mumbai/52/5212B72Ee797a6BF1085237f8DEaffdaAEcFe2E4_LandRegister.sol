...
contract LandRegister is  ERC721URIStorage{
    ...
    function transferToken(address from, address to, uint256 tokenId, string memory tokenUri) public {
        ...
        require(msg.sender == from || msg.sender == contractDeployer, "You are not allowed to transfer this token");
    ...
    function getListedTokens() public view returns (Listing[] memory) {
        Listing[] memory listedTokens = new Listing[](_tokenIds.current());
        uint256 counter = 0;
        for (uint256 i = 1; i <= _tokenIds.current(); i++) {
            if (tokenIdToListing[i].tokenId != 0) {
                listedTokens[counter] = tokenIdToListing[i];
                counter++;
            }
        }
        return listedTokens;
    ...
    // get all tokens of an owner with name, description and imageurl boolean if they are listed,
    // if they are listed the price is also included


    
    function getTokensOfOwnerWithDetails() public view returns (Token[] memory) {
        OwnedToken[] memory tokens = getTokensOfOwner(msg.sender);
        Token[] memory tokensWithDetails = new Token[](tokens.length);
        for (uint256 i = 0; i < tokens.length; i++) {
            uint256 tokenId = tokens[i].tokenId;
            bool isListedForSale = isListed(tokenId);
            uint256 price = 0;
            if (isListedForSale) {
                price = tokenIdToListing[tokenId].price;
            }
            tokensWithDetails[i] = Token(tokenId, tokens[i].tokenUri, isListedForSale, price);
        }
