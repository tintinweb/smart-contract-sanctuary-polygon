    ...
    OperatorFilterer
{
    ...
    uint32 public constant MAX_MYTHIC_NFTS = 110; // Maximum allowed number of Mythic NFTs
    ...
    /// @param defaultOwner_ The address of the default owner
        ...
        OperatorFilterer(0x0000000000000000000000000000000000000000, false)
    {
        ...
        if (defaultOwner_ == address(0)) {
            revert Utils.OwnerIsZeroAddress();
        }
    ...
    /// @notice Determines if the specified NFTs are of Mythic type
    /// @param tokenIds_ An array of tokenIds to check if they are Mythic
    /// @return mythics A boolean array indicating if the corresponding tokenId is Mythic
    function isMythic(
        uint256[] calldata tokenIds_
    ) external view returns (bool[] memory) {
        bool[] memory mythics = new bool[](tokenIds_.length);
        for (uint256 i = 0; i < tokenIds_.length; ) {
            mythics[i] = _isMythic.get(tokenIds_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function devMint(
        ...
        _devMint(tos_, quantities_, isMythics_);
    }
    ...
    function _devMint(
        ...
        if (
            tos_.length != quantities_.length ||
            tos_.length != isMythics_.length
        ) {
            revert Utils.UnmatchedArrayLengths();
        }

        uint256 totalQuantity = 0;
        for (uint256 i = 0; i < quantities_.length; ) {
            unchecked {
                totalQuantity += quantities_[i];
                ++i;
            }
        }
        uint256[] memory tokenIds = new uint256[](totalQuantity);
        ...
        unchecked {
            for (uint256 i = 0; i < tos_.length; ++i) {
                for (uint256 j = 0; j < quantities_[i]; ++j) {
                    if (isMythics_[i]) {
                        tokenIds[index++] = _mintMythic(tos_[i]);
                    } else {
                        tokenIds[index++] = _mintNormal(tos_[i]);
                    }
                }
            }
        }
