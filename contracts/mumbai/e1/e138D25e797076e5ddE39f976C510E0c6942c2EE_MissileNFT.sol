    ...
    IMissileNFT
{
    ...
    function mintToken(address to, uint256[] memory ids) external override onlyGenesisMinter nonReentrant whenNotPaused {
        require(currentAmount + ids.length <= maxAmount,"exceed");
        for (uint256 i = 0; i < ids.length; i++) {
            require(ids[i] <= maxAmount, "invalid id");
            _safeMint(to, ids[i]);
        }
