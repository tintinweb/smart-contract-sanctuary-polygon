...
contract chain_scouts_ships_l2 is ERC721Upgradeable, OwnableUpgradeable, PausableUpgradeable, ILayerZeroReceiver {
    string public baseTokenURI;
    ILayerZeroEndpoint public migrationHandler;
    IERC721Upgradeable public chainScouts;
    uint public currentIndex;
    uint public gas;
    mapping (address => uint) public userMinted;

    function kickstarter(address _migrator) external initializer {
        __Ownable_init();
        __Pausable_init();
        currentIndex=1000;
        gas = 200000;
        migrationHandler = ILayerZeroEndpoint(_migrator);
//        chainScouts = IERC721Upgradeable(_chainScoutGenesisAddress);
        __ERC721_init('ChainScouts_Ships','CSS');
    }
    ...
    function mint(uint _amount) external {
        require (5 >= userMinted[msg.sender]+_amount,'Error: Not Genesis Holder');
        require (2000 >= currentIndex + _amount, 'Max Amount Minted');
    ...
    function transferTokensToL2(uint[] memory tokenIds, bytes memory l2Contract, uint16 destinationChainId, uint migrationType) external payable{
        burnTokens(tokenIds);
        bytes memory payload = abi.encode(msg.sender,migrationType,0,tokenIds);
        uint16 version = 1;
