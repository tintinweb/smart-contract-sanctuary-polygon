...
import "./interfaces/IIntellaUtil.sol";
    ...
    AccessControlEnumerable
{
    ...
    function registerToken(address token, address tokenPool)
        external
        onlyRole(OWNER_ROLE)
    {
        require(token != address(0), "token can not be zero address");
        require(tokenPool != address(0), "token pool can not be zero address");
        require(token != tokenPool, "token and token pool must be different");

        require(tokens.add(token), "token already registered");
        tokenPools[token] = tokenPool;

        emit TokenRegistered(token, tokenPool);
    }

    function unregisterToken(address token) external onlyRole(OWNER_ROLE) {
        require(tokens.remove(token), "token is not registered");
    ...
    function getTokenPool(address token) public view returns (address) {
        require(isRegisteredToken(token), "unregistered token");

        address tokenPool = tokenPools[token];
        require(tokenPool != address(0), "token pool is zero address");
     ...
     */
    function trasnferWithPermit(
        address to,
        address token,
        uint256 amount,
        string calldata memo,
        PermitData calldata permission
    ) external override {
        // permit for token to transfer
        _callSafePermit(token, permission);
        ...
        emit TransferedWithPermit(from, to, token, amount, memo);
    }
    ...
    function depositToken(
        ...
        address pool = getTokenPool(token);
    ...
    function withdrawToken(
        ...
        emit Withdrawn(pool, user, token, amount, memo);
    }
    ...
    function finalize() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit ContractClosed(_msgSender());
    }

    function _callSafePermit(address token, PermitData calldata data) internal {
...
interface IIntellaUtil {
    ...
    event ContractClosed(address indexed adminUser);
