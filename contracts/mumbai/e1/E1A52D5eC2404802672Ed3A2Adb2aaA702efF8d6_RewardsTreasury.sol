...
import "./FundsWrapperCustody.sol";
...
/**
  RewardsTreasury holds the funds for the project creators and handle the 
  reward streams to the verified NFT owners

  TODO:

  - The project owner should be able to set the flow rate of all streams
  - The contract should be able to support multiple ERC20 tokens
  - Create a function that can upgrade an array of flows
 */
contract RewardsTreasury is
  FundsWrapperCustody,
  UUPSUpgradeable,
  AccessControlUpgradeable
{
  ...
  event FlowDeleted(address indexed sender, address indexed receiver);

  bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

  // Warning: state variables must be added in an append-only fashion,
  // and do not rearrange the order of these variables
  ISuperToken public acceptedToken;

  CFAv1Library.InitData public cfa;

  // NOTE: double check if the flow rate for the smart contract is per second
  // the flowRate equals to 1 ether/month, calculated using https://docs.superfluid.finance/superfluid/protocol-developers/interactive-tutorials/money-streaming-1#flowrates
  int96 public maxRewardFlowRate;

  uint64 public maxNumberOfFlows;

  Counters.Counter public activeFlowCounts;

  // Excluding the contract creator, the contract can only have one operator at a time.
  ...
  function initialize(
    ...
    __FundsWrapperCustody_init(_fundsOwner, _ethProxy);

    maxRewardFlowRate = 385802469135;
    maxNumberOfFlows = 1000;
    _operator = _operatorAddress;
  ...
  function revokeAndSetOperator(address _operatorAddress)
    external
  ...
  function revokeAndSetDefaultAdmin(address _adminAddress)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    grantRole(OPERATOR_ROLE, _adminAddress);
  ...
  function createRewardFlow(address _receiver, int96 _flowRate)
    external
    onlyRole(OPERATOR_ROLE)
    onlyAllowedFlowRate(_flowRate)
  {
    require(
      activeFlowCounts.current() < maxNumberOfFlows,
      "max number of flows reached"
    );

    activeFlowCounts.increment();
    cfa.createFlow(_receiver, acceptedToken, _flowRate);
    emit FlowCreated(address(this), _receiver, _flowRate);
  }

  function updateRewardFlow(address _receiver, int96 _flowRate)
    external
    onlyRole(OPERATOR_ROLE)
    onlyAllowedFlowRate(_flowRate)
  {
    cfa.updateFlow(_receiver, acceptedToken, _flowRate);
    emit FlowUpdated(address(this), _receiver, _flowRate);
  }

  function deleteRewardFlow(address _receiver)
    external
    onlyRole(OPERATOR_ROLE)
  {
    activeFlowCounts.decrement();
    cfa.deleteFlow(address(this), _receiver, acceptedToken);
    emit FlowDeleted(address(this), _receiver);
  }

  function setMaxFlowRate(int96 _flowRate)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    maxRewardFlowRate = _flowRate;
  }

  function setMaxNumberOfFlows(uint64 numFlows)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    maxNumberOfFlows = numFlows;
  }
  ...
  modifier onlyAllowedFlowRate(int96 _flowRate) {
    require(_flowRate <= maxRewardFlowRate, "max flow rate exceeded");
    _;
  }
}
...
contract SETHProxy is ISETHCustom, CustomSuperTokenBase, UUPSProxy {
    ...
    function downgradeToETH(uint wad) external override {
        ...
        require(sent, "failed to send downgraded funds");
...
import "./interfaces/IFundsWrapperCustody.sol";

contract FundsWrapperCustody is Initializable, IFundsWrapperCustody {
  SETHProxy public ethProxy;

  function __FundsWrapperCustody_init(address _fundsOwner, SETHProxy _ethProxy)
    internal
    onlyInitializing
  {
  ...
  function depositEth() external payable override onlyFundsOwner {
    _depositEth(msg.value);
  }

  function withdrawEth(uint256 amt) external override onlyFundsOwner {
    ethProxy.downgradeToETH(amt);
    emit TokenWithdrawed(ethProxy.getUnderlyingToken(), msg.sender, amt);
  }

  function upgrade(ISuperToken token, uint256 amt)
    external
    override
    onlyFundsOwner
  {
    ERC20 underlyingToken = ERC20(token.getUnderlyingToken());
    require(
      underlyingToken.transferFrom(msg.sender, address(this), amt),
      "failed to deposit the token"
    );

    if (!_isApproved(underlyingToken, address(this), address(token), amt)) {
      require(
        underlyingToken.approve(address(token), amt),
    ...
    emit TokenDeposited(address(token), msg.sender, amt);
  }

  function downgrade(ISuperToken token, uint256 amt)
    external
    override
    onlyFundsOwner
  {
    token.downgrade(amt);
    withdrawToken(ERC20(token.getUnderlyingToken()), amt);
  }

  function _depositEth(uint256 amt) internal onlyFundsOwner {
    ethProxy.upgradeByETH{ value: amt }();
    emit TokenDeposited(ethProxy.getUnderlyingToken(), msg.sender, amt);
  }
...
/**
  Wrap the deposited funds and unwrap upon withdrawal

  TODO:
  Multi ERC-20 token plus native token support

  - depositEth, upgradeEth, possible to do depositAndUpgradeEth?
  - remove the accepted token input from the initializer and pass that in as a function argument instead
  - add the accepted token as an input to the events
 */

// contract FundsWrapperCustody is Initializable {
//   event ValueDeposited(address indexed _owner, uint256 _amt);
//   event ValueWithdrawed(address indexed _owner, uint256 _amt);
...
//   event FundTokenDeposited(address indexed _owner, uint256 _amt);
//   event FundTokenWithdrawed(address indexed _fundOwner, uint256 _amt);
//   event FundsOwnershipTransferred(
//     address indexed oldOwner,
//     address indexed newOwner
//   );

//   uint256 public constant APPROVAL_LIMIT = 2**256 - 1;

//   // Warning: state variables must be added in an append-only fashion,
//   // and do not rearrange the order of these variables
//   address private _fundsOwner;

//   ERC20 public underlyingToken;
//   ISuperToken public acceptedFundToken;

//   function __FundsWrapperCustody_init(
//     address fundsOwner,
//     ISuperToken _acceptedToken
//   ) internal onlyInitializing {
//     _fundsOwner = fundsOwner;
//     acceptedFundToken = _acceptedToken;

//     // Approves SuperToken to transfer the underlyingToken from the current contract.
//     // This enables the contract to upgrade ERC20 to SuperToken
//     underlyingToken = ERC20(acceptedFundToken.getUnderlyingToken());
//     underlyingToken.approve(address(_acceptedToken), APPROVAL_LIMIT);

//     // Approves the fundsOwner to withdraw the funds
//     underlyingToken.approve(fundsOwner, APPROVAL_LIMIT);
//   }

//   function upgradeByETH(SETHProxy _proxy) external payable {
//     _proxy.upgradeByETH{ value: msg.value }();
//   }

//   function deposit(uint256 _amt) external onlyFundsOwner {
//     require(_amt > 0, "deposit amount must be positive");
...
//       _amt
//     );
//     require(success, "failed to deposit the funds");
//     upgradeToken(_amt);

//     emit FundTokenDeposited(msg.sender, _amt);
//   }

//   function withdraw(uint256 _amt) external onlyFundsOwner {
//     require(
//       acceptedFundToken.balanceOf(address(this)) >= _amt,
//       "contract has insufficient funds"
//     );

//     acceptedFundToken.downgrade(_amt);
//     bool success = underlyingToken.transfer(msg.sender, _amt);
//     require(success, "failed to withdraw the funds");

//     emit FundTokenWithdrawed(msg.sender, _amt);
//   }

//   function balanceOf() external view returns (uint256) {
//     return acceptedFundToken.balanceOf(address(this));
//   }
...
//   function transferFundsOwnership(address _newOwner) public onlyFundsOwner {
//     require(_newOwner != address(0), "new owner is the zero address");
//     address oldOwner = _fundsOwner;

//     _fundsOwner = _newOwner;
//     _onFundsOwnershipTransferred(oldOwner, _newOwner);

//     emit FundsOwnershipTransferred(oldOwner, _newOwner);
//   }

//   function _onFundsOwnershipTransferred(address from, address to) internal {
//     require(underlyingToken.approve(from, 0), "failed to revoke allowances");

//     bool approved = underlyingToken.approve(to, APPROVAL_LIMIT);
//     require(approved, "failed to approve");
//   }

//   modifier onlyFundsOwner() {
//     require(msg.sender == _fundsOwner, "caller is not the funds owner");
//     _;
//   }

//   // Ideally, the functions below won't be called in production, but included here in case the funds owner sends the funds by accident
//   function approveToken(ERC20 _token, uint256 _amt)
//     external
//     onlyFundsOwner
//     returns (bool)
//   {
//     bool approved = _token.approve(msg.sender, _amt);
//     emit TokenApproval(address(_token), address(this), msg.sender, _amt);
//     return approved;
//   }

//   function withdrawToken(ERC20 _token, uint256 _amt)
//     external
//     onlyFundsOwner
//     returns (bool)
//   {
//     bool transferred = _token.transfer(msg.sender, _amt);
//     emit TokenTransfer(address(_token), address(this), msg.sender, _amt);
//     return transferred;
//   }

//   function withdrawValue(uint256 _amt) external onlyFundsOwner {
//     require(address(this).balance >= _amt, "insufficient funds to withdraw");
//     (bool sent, ) = msg.sender.call{ value: _amt }("");
//     require(sent, "failed to send the funds");

//     emit ValueWithdrawed(msg.sender, _amt);
//   }

//   function upgradeToken(uint256 amt) public onlyFundsOwner {
//     acceptedFundToken.upgrade(amt);
//   }

//   receive() external payable onlyFundsOwner {
...
abstract contract IFundsWrapperCustody {
  uint256 public constant APPROVAL_LIMIT = 2**256 - 1;

  address public fundsOwner;

  // NOTE: the token represents what the caller actually deposited or withdrawed
  ...
  function withdrawEth(uint256 amt) external virtual;

  /**
  @param token: the address of the super token the to upgrade to
   */
  function upgrade(ISuperToken token, uint256 amt) external virtual;

  function downgrade(ISuperToken token, uint256 amt) external virtual;

  // Allows the funds owner to withdraw if they transferred the token by accident
  function withdrawToken(ERC20 token, uint256 amt) public onlyFundsOwner {
    if (!_isApproved(token, address(this), msg.sender, amt)) {
      require(token.approve(msg.sender, amt), "failed to approve the tokens");
    }

    require(token.transfer(msg.sender, amt), "failed to transfer the tokens");

    emit TokenWithdrawed(address(token), msg.sender, amt);
  }
  ...
  function _isApproved(
    ...
    return token.allowance(owner, spender) >= amt;
  }

  // access control
  function transferFundsOwnership(address newOwner) external {
    require(fundsOwner != newOwner, "a new owner address is required");
    address oldOwner = fundsOwner;
    fundsOwner = newOwner;
    emit FundsOwnershipTransferred(oldOwner, newOwner);
  }

  receive() external payable virtual;

  modifier onlyFundsOwner() {
    require(fundsOwner == msg.sender);
    _;
  }
}
