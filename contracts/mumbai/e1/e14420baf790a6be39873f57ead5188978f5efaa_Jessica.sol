...
// File: contracts/DividendPayingTokenBNB.sol
...
// File: contracts/DividendTracker2.sol
...
// File: contracts/DividendTracker1.sol
...
contract DividendTracker1 is DividendPayingToken {
    ...
    constructor(address rewardToken) DividendPayingToken("DividendTracker1", "Tina_DividendTracker1", rewardToken) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**decimals()); //must hold 10000+ tokens
    }
...
contract Jessica is Context, ERC20, ERC20Burnable , Ownable {
    using SafeMath for uint256;
    using Address for address;
    
    bool private swapping;
    
    DividendTracker1 public dividendTrackerGALA; // For GALA
    ...
    // address public uniswapV2PairGALA;
    // address public uniswapV2PairBTCB;
    // address public uniswapV2PairBETH;
    ...
    constructor (address _router, address _beth, address _btcb, address _gala, DividendTracker1 _forGALA, DividendTracker1 _forBTCB, DividendTracker1 _forBETH) ERC20("JESSICA Token","JESSICA") {
        
        GALA = _gala;
        BTCB = _btcb;
        BETH = _beth;
        
        dividendTrackerGALA = _forGALA; // GALA
        ...
        // uniswapV2PairGALA = IUniswapV2Factory(IUniswapV2Router02(_router).factory()) // token and GALA
        //     .createPair(address(this), GALA);
    ...
    function getTransferAmounts(uint256 _amount) private returns (uint256, uint256) {
        ...
        forGALA = forGALA.add(forDividends.mul(GALAdivisor).div(100));
    ...
    function _transfer(
        ...
        require(from != address(0), "Jessica: transfer from the zero address");
        require(to != address(0), "Jessica: transfer to the zero address");
        require(canTransferMoreThanAllowed[from] || amount <= maxTxAmount,"Jessica: transfer more than allowed amount");
        if (amount == 0) {
        ...
        if(canSwap &&
            ...
            swapping = true;
                ...
                swapAndSendDividends(0, forBTCB);
                
              
                
                swapAndSendDividends(1, forBETH);
                
               
                swapAndSendDividends(2, forGALA);
        ...
        try dividendTrackerGALA.setBalance(payable(from), balanceOf(from)) {} catch {}
        try dividendTrackerGALA.setBalance(payable(to), balanceOf(to)) {} catch {}

        if (!swapping) {
            uint256 gas = gasForProcessing.div(3);

            try dividendTrackerGALA.process(gas) {} catch {}
            try dividendTrackerBETH.process(gas) {} catch {}
            try dividendTrackerBTCB.process(gas) {} catch {}
        }

        require(canHoldMoreThanAllowed[to] || balanceOf(to) <= maxBalance,"Jessica: Balance exceed max balance");
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        uint256 initialBalance = IERC20(GALA).balanceOf(address(this));

        swapTokenforGALA(half);

        uint256 swappedGALA = IERC20(GALA).balanceOf(address(this)).sub(initialBalance);
    ...
    function addLiquidity(uint256 tokenAmount, uint256 galaAmount) private {
        ...
        //transfer helper
        TransferHelper.safeApprove(GALA, uniswapV2Router, galaAmount);


        // add the liquidity
        IPancakeRouter02(uniswapV2Router).addLiquidity(
            address(this),
            GALA,
            tokenAmount,
            galaAmount,
    ...
    function swapAndSendDividends(uint256 _flag, uint256 tokens) private {
        if(_flag == 0){
            ...
            swapTokenforGALA(tokens);
            uint256 dividends = IERC20(GALA).balanceOf(address(this));

            bool success = IERC20(GALA).transfer(address(dividendTrackerGALA) ,dividends);
            
             if (success) {
                 dividendTrackerGALA.distributeRewardTokenDividends(dividends);
                //  forDividends = forDividends.sub(tokens);
                //  forBETH = 1;
             }
        }
    ...
    function swapTokenforBTCB(uint256 _amount) private {
        ...
        path[0] = address(this);
        path[1] = IPancakeRouter02(uniswapV2Router).WETH();
        path[2] = BTCB;
        ...
        IPancakeRouter02(uniswapV2Router).swapExactTokensForTokensSupportingFeeOnTransferTokens(
            _amount,
            0,
            path,
            address(this),
    ...
    function swapTokenforGALA(uint256 _amount) private {
        address[] memory path = new address[](3);
    ...
    function excludeFromDividendsGALA(address account) external onlyOwner {
        dividendTrackerGALA.excludeFromDividends(account);
    }
    ...
    function isExcludedFromDividendsGALA(address account) external view returns (bool){
        return dividendTrackerGALA.excludedFromDividends(account);
    }
    ...
    function updateDividendTrackerGALA(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerGALA), "Jessica: The dividend tracker already has that address");

        DividendTracker1 newDividendTracker = DividendTracker1(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "Jessica: The new dividend tracker must be owned by the HoneyPad token contract");

        newDividendTracker.excludeFromDividends(address(newDividendTracker));
        newDividendTracker.excludeFromDividends(address(this));
        newDividendTracker.excludeFromDividends(address(uniswapV2Router));

        dividendTrackerGALA = newDividendTracker;
    }
    
    function updateDividendTrackerBETH(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerBETH), "Jessica: The dividend tracker already has that address");
    ...
    function updateDividendTrackerBTCB(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTrackerBTCB), "Jessica: The dividend tracker already has that address");
    ...
    function updateClaimWaitGALA(uint256 claimWait) external onlyOwner {
        dividendTrackerGALA.updateClaimWait(claimWait);
    }
    ...
    function updateMinimumTokenBalanceForDividendsGALA(uint256 newTokenBalance) external onlyOwner {
        dividendTrackerGALA.updateMinimumTokenBalanceForDividends(newTokenBalance);
    }
    ...
    function getClaimWaitGALA() external view returns(uint256) {
        return dividendTrackerGALA.claimWait();
    }
    ...
    function getTotalDividendsDistributedGALA() external view returns (uint256) {
        return dividendTrackerGALA.totalDividendsDistributed();
    }
    ...
    function withdrawableDividendOfGALA(address account) public view returns(uint256) {
        return dividendTrackerGALA.withdrawableDividendOf(account);
    }
    ...
    function dividendTokenBalanceOfGALA(address account) public view returns (uint256) {
        return dividendTrackerGALA.balanceOf(account);
    }
    ...
    function getAccountDividendsInfoGALA(address account)
    external view returns (
        ...
        uint256,
        uint256) {
        return dividendTrackerGALA.getAccount(account);
    }
    ...
    function getAccountDividendsInfoAtIndexGALA(uint256 index)
    external view returns (
        address,
        int256,
        int256,
        ...
        uint256,
        uint256) {
        return dividendTrackerGALA.getAccountAtIndex(index);
    }
    ...
    function claimGALA() external {
        dividendTrackerGALA.processAccount(payable(msg.sender), false);
    }
    ...
    function getLastProcessedIndexGALA() external view returns(uint256) {
        return dividendTrackerGALA.getLastProcessedIndex();
    }
    ...
    function getNumberOfDividendTokenHoldersGALA() external view returns(uint256) {
        return dividendTrackerGALA.getNumberOfTokenHolders();
    }
   ...
   //Setter functions
    ...
    function setBTCBdivisor(uint256 _divisor) public onlyOwner {
        require(_divisor <= 100, "Jessica: % more than 100");
    ...
    function setCanHoldMoreThanAllowed(address _address) public onlyOwner {
        canHoldMoreThanAllowed[_address] = true;
    }

    function setCanTransferMoreThanAllowed(address _address) public onlyOwner {
        canTransferMoreThanAllowed[_address] = true;
    }
}