...
contract Token777 is ERC777 {
  ...
  ) payable ERC777(name, symbol, new address[](0)) {
    require(providerAllocation >= 0, "Provider allocation must be greater than or equal to 0");
    // require(_owner != address(0x00));
    // use default value of 18 for decimals from ERC20 standard.
    INITIAL_SUPPLY = initialSupply * (10**decimals());
    uint256 providerBalance = ((INITIAL_SUPPLY * providerAllocation) / 100); // service coins
    uint256 ownerBalance = INITIAL_SUPPLY - providerBalance; // owner's coins

    OWNER = payable(_owner);
    SERVICE_PROVIDER = payable(serviceProvider);

    if (providerAllocation > 0) {
      // allocate supply for the service
      _mint(SERVICE_PROVIDER, providerBalance, "", "");
    }

    _mint(_owner, ownerBalance, "", "");
  }
