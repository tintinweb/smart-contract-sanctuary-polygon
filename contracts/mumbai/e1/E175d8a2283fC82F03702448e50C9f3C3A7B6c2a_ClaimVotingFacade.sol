...
import "./interfaces/IClaimVotingFacade.sol";
...
contract ClaimVotingFacade is IClaimVotingFacade, Initializable, AbstractDependant {
    ...
    function __ClaimVotingFacade_init() external initializer {}
    ...
    function initializeVoting(
        ...
        require(
            ...
            "CVF: Not an allowed contract"
        );
        require(coverTokens > 0, "CVF: Claimer has no coverage");

        // this checks claim duplicate && appeal logic
        uint256 claimIndex =
            claimingRegistry.submitClaim(claimProvenance, evidenceURI, coverTokens, appeal);

        uint256 onePercentInBMIToLock =
            coverTokens.uncheckedDiv(100).mul(DECIMALS18).div(
                DecimalsConverter.convertTo18(bmiPriceInUSDT, 6)
            );

        bmiToken.transferFrom(
            claimProvenance.claimer,
            address(claimVoting),
            onePercentInBMIToLock
        ); // needed approval
        ...
        if (claimProvenance.provenance == IClaimingRegistry.Provenance.POLICY) {
            IPolicyBook.PolicyHolder memory policyHolder =
                IPolicyBook(msg.sender).userStats(claimProvenance.claimer);
            reinsuranceTokensAmount = policyHolder.reinsurancePrice;
            reinsuranceTokensAmount = Math.min(
                reinsuranceTokensAmount,
                coverTokens.uncheckedDiv(100)
            );
        } else if (claimProvenance.provenance == IClaimingRegistry.Provenance.DEMAND) {
            (, , , uint256 premiumAmount, ) =
                demandBook.getDemandAmountInfo(claimProvenance.demandIndex);

            reinsuranceTokensAmount = premiumAmount.mul(PROTOCOL_PERCENTAGE).uncheckedDiv(
                PERCENTAGE_100
            );
            reinsuranceTokensAmount = Math.min(
                reinsuranceTokensAmount,
    ...
    function anonymouslyVoteBatch(
        ...
        for (uint256 i = 0; i < claimIndexes.length; i = uncheckedInc(i)) {
            ...
            require(
                ...
                "CVF: Anonymous voting is over"
            );
            require(
                claimingRegistry.claimOwner(claimIndex) != msg.sender,
                "CVF: Voter is the claimer"
            );
            require(
                !claimingRegistry.isClaimAppeal(claimIndex) ||
                    reputationSystem.isTrustedVoter(msg.sender),
                "CVF: Not a trusted voter"
            );
            require(
                claimVoting.getVoteIndex(claimIndex, msg.sender) == 0,
                "CVF: Already voted for this claim"
            );

            claimVoting.addAnonymousVote(
                msg.sender,
                claimIndex,
                finalHashes[i],
                encryptedVotes[i],
    ...
    function exposeVoteBatch(
        ...
        for (uint256 i = 0; i < claimIndexes.length; i = uncheckedInc(i)) {
            uint256 claimIndex = claimIndexes[i];
            uint256 voteIndex = claimVoting.getVoteIndex(claimIndex, msg.sender);

            require(voteIndex != 0, "CVF: Vote doesn't exist");
            require(
                claimVoting.getVoteStatus(voteIndex) == IClaimVoting.VoteStatus.AWAITING_EXPOSURE,
                "CVF: Vote is not awaiting"
            );

            claimVoting.verifyExposedData(
                claimIndex,
                voteIndex,
    ...
    function calculateResult(uint256 claimIndex) external override {
        ...
        require(
            ...
            "CVF: Not allowed to calculate"
        );
        uint256 reward = claimVoting.sendRewardsForCalculationTo(claimIndex, msg.sender);

        emit RewardsForClaimCalculationSent(msg.sender, reward);
    ...
    function _resolveClaim(uint256 claimIndex) internal {
        ...
        claimVoting.updateVotedYesPercentage(claimIndex);
        (uint256 allVotedStakedStkBMI, uint256 votedYesPercentage, uint256 votedAverageAmount) =
            claimVoting.getVotedAmount(claimIndex);
        ...
        if (
            ...
            if (votedYesPercentage >= APPROVAL_PERCENTAGE) {
                ...
                claimingRegistry.acceptClaim(claimIndex, votedAverageAmount);
            } else {
                // reject & use locked BMI for rewards
                claimingRegistry.rejectClaim(claimIndex);
            }
    ...
    function receiveVoteResultBatch(uint256[] calldata claimIndexes) external override {
        ...
        uint256 stblReward = rewardAmount;
        uint256 bmiReward;
        uint256 bmiPenalty;
        ...
        for (uint256 i = 0; i < claimIndexes.length; i = uncheckedInc(i)) {
            uint256 claimIndex = claimIndexes[i];
            require(claimingRegistry.claimExists(claimIndex), "CVF: Claim doesn't exist");
            uint256 voteIndex = claimVoting.getVoteIndex(claimIndex, msg.sender);
            require(voteIndex != 0, "CVF: No vote on this claim");

            if (
                claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.EXPIRED
            ) {
                claimVoting.modifyReceivedVote(
                    claimIndex,
                    voteIndex,
                    msg.sender,
                    IClaimVoting.VoteStatus.CLAIM_EXPIRED
                );
            } else if (claimVoting.getVoteStatus(voteIndex) == IClaimVoting.VoteStatus.EXPIRED) {
                uint256 _bmiPenalty = claimVoting.calculateExpiredVote(claimIndex, msg.sender);

                bmiPenalty = bmiPenalty.add(_bmiPenalty);

                claimVoting.modifyReceivedVote(
                    claimIndex,
                    voteIndex,
                    msg.sender,
                    IClaimVoting.VoteStatus.EXPIRED
                );
            } else if (
                claimVoting.getVoteStatus(voteIndex) == IClaimVoting.VoteStatus.AWAITING_RECEPTION
            ) {
                (, uint256 votedYesPercentage, ) = claimVoting.getVotedAmount(claimIndex);
                uint256 suggestedAmount = claimVoting.getVoterSuggestedAmount(voteIndex);

                if (votedYesPercentage >= PERCENTAGE_50 && suggestedAmount > 0) {
                    (uint256 _stblReward, uint256 _bmiReward, uint256 newReputation) =
                        claimVoting.calculateMajorityYesVote(claimIndex, msg.sender, reputation);

                    stblReward = stblReward.add(_stblReward);
                    bmiReward = bmiReward.add(_bmiReward);
                    ...
                    claimVoting.modifyReceivedVote(
                        ...
                        voteIndex,
                        msg.sender,
                        IClaimVoting.VoteStatus.MAJORITY
                    );
                } else if (votedYesPercentage < PERCENTAGE_50 && suggestedAmount == 0) {
                    (uint256 _bmiReward, uint256 newReputation) =
                        claimVoting.calculateMajorityNoVote(claimIndex, msg.sender, reputation);
                    ...
                    (uint256 _bmiPenalty, uint256 newReputation) =
                        claimVoting.calculateMinorityVote(claimIndex, msg.sender, reputation);
                    ...
                    claimVoting.modifyReceivedVote(
                        ...
                        voteIndex,
                        msg.sender,
                        IClaimVoting.VoteStatus.MINORITY
                    );
                }
            }
        }
        if (stblReward > 0) {
            claimingRegistry.requestRewardWithdrawal(msg.sender, stblReward);
        }
        if (bmiReward > 0) {
            claimVoting.transferBMIReward(msg.sender, bmiReward);
        }
        if (bmiPenalty > 0) {
            stkBMIStaking.slashUserTokens(msg.sender, uint256(bmiPenalty));
        }
        reputationSystem.setNewReputation(msg.sender, reputation);
    }

    // function _getVoteRewards(
    //     uint256 claimIndex,
    //     uint256 voteIndex,
    //     address voter
    // )
    ...
    //         uint256 bmiPenalty,
    //         IClaimVoting.VoteStatus voteStatus
    //     )
    // {
    //     uint256 oldReputation = reputationSystem.reputation(voter);

    //     if (claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.EXPIRED) {
    //         voteStatus = IClaimVoting.VoteStatus.CLAIM_EXPIRED;
    //     } else if (claimVoting.getVoteStatus(voteIndex) == IClaimVoting.VoteStatus.EXPIRED) {
    //         bmiPenalty = claimVoting.calculateExpiredVote(claimIndex, voter);

    //         voteStatus = IClaimVoting.VoteStatus.EXPIRED;
    //     } else if (
    //         claimVoting.getVoteStatus(voteIndex) == IClaimVoting.VoteStatus.AWAITING_RECEPTION
    //     ) {
    //         (, uint256 votedYesPercentage, ) = claimVoting.getVotedAmount(claimIndex);
    ...
    //         if (votedYesPercentage >= PERCENTAGE_50 && suggestedAmount > 0) {
    //             (stblReward, bmiReward, newReputation) = claimVoting.calculateMajorityYesVote(
    //                 claimIndex,
    //                 voter,
    //                 oldReputation
    //             );

    //             voteStatus = IClaimVoting.VoteStatus.MAJORITY;
    //         } else if (votedYesPercentage < PERCENTAGE_50 && suggestedAmount == 0) {
    //             (bmiReward, newReputation) = claimVoting.calculateMajorityNoVote(
    //                 claimIndex,
    //                 voter,
    //                 oldReputation
    //             );
    ...
    //             voteStatus = IClaimVoting.VoteStatus.MAJORITY;
    //         } else {
    //             (bmiPenalty, newReputation) = claimVoting.calculateMinorityVote(
    //                 claimIndex,
    //                 voter,
    //                 oldReputation
    //             );

    //             voteStatus = IClaimVoting.VoteStatus.MINORITY;
    //         }
    //     }
    // }
}
...
interface IClaimVoting {
    ...
    struct PublicClaimInfo {
        uint256 claimIndex;
        IClaimingRegistry.ClaimProvenance claimProvenance;
        string evidenceURI;
        bool appeal;
        uint256 claimAmount;
        uint256 time;
    ...
    function getVoteStatus(uint256 index) external view returns (VoteStatus);

    function getVoteIndex(uint256 claimIndex, address user) external view returns (uint256);
    ...
    function verifyExposedData(
        uint256 claimIndex,
        uint256 voteIndex,
        uint256 suggestedClaimAmount,
        bytes32 hashedSignaturesOfClaim
    ) external view;
    ...
    function sendRewardsForCalculationTo(uint256 claimIndex, address calculator)
        external
        returns (uint256 reward);

    function updateVotedYesPercentage(uint256 claimIndex) external;

    function getVotedAmount(uint256 claimIndex)
        external
        view
        returns (
            uint256 allVotedStakedStkBMI,
            uint256 votedYesPercentage,
            uint256 votedAverageAmount
        );

    function isToReceive(uint256 claimIndex, address user) external view returns (bool);

    function getVoterSuggestedAmount(uint256 voteIndex)
        external
        view
        returns (uint256 suggestedAmount);

    function calculateMajorityYesVote(
        uint256 claimIndex,
        address voter,
    ...
    function calculateMajorityNoVote(
        uint256 claimIndex,
        address voter,
        uint256 oldReputation
    ) external view returns (uint256 _bmiReward, uint256 _newReputation);

    function calculateMinorityVote(
        uint256 claimIndex,
        address voter,
        uint256 oldReputation
    ) external view returns (uint256 _bmiPenalty, uint256 _newReputation);

    function calculateExpiredVote(uint256 claimIndex, address voter)
        external
        view
        returns (uint256 _bmiPenalty);

    function modifyReceivedVote(
        uint256 claimIndex,
        uint256 voteIndex,
        address voter,
    ...
    function transferBMILocked(uint256 claimIndex, address claimer) external;

    function transferBMIReward(address voter, uint256 bmiReward) external;
}
...
interface IDemandBook {
    struct DemandInfo {
        ...
        uint256 premiumAmount; // premiumAmount = rewardAmount + reinsurancePoolAmount
        bool isPremiumWithdrawn;
        uint256 epochAmount;
        uint256 creationTime;
        uint256 startTime;
        uint256 endTime;
        PublicDemandInfo publicDemandInfo; // only FE purpose (not accurate at solidity level as never updated)
    }
    ...
    function demandInfo(uint256 demandIndex)
        ...
        returns (
            ...
            uint256 startTime,
            uint256 endTime,
            PublicDemandInfo memory publicDemandInfo
        );
    ...
    function isDemandActive(uint256 demandIndex) external view returns (bool);
    ...
    function getListDemands(
        ...
        uint256 limit,
        ListOption listOption
    ) external view returns (DemandInfo[] memory demandPublicInfo);
    ...
    function getClaimApprovalAmount(uint256 demandIndex, uint256 bmiPriceInUSDT)
        external
        view
        returns (uint256);
    ...
    function commitClaim(
        uint256 demandIndex,
        uint256 _endTime,
        IClaimingRegistry.ClaimStatus _status
    ) external;
