...
/**
 * @title WETokenManager
 * @author Mihai Lazarut
 * @dev This contract allows users to participate in reveals where they can buy tickets
 * for a chance to win a token prize. The contract uses Chainlink VRF v2 for generating random
 * numbers to determine the winner.
 */
contract WETokenManager is
    AccessControl,
    ReentrancyGuard,
    VRFConsumerBaseV2,
    ConfirmedOwner
{
     ...
     * `sendTokensAndMATIC` function. It contains the following information:
     ...
     *
     * This function requests a random number from Chainlink VRF v2 and uses it to determine the
     * winner of the reveal.
     */
    function setWinner(
        uint256 _revealId
     ...
     * Finally, it calls the `sendTokensAndMATIC` function to send prizes
    ...
    function fulfillRandomWords(
        ...
        sendTokensAndMATIC(revealId, winnerTicketNumber);

        emit RequestFulfilled(_requestId, _randomWords);
    }

    /**
     * @dev Sends the prize tokens to the winner and MATIC to the supervisor
     * and completes the reveal
     * @param _revealId - Id of the reveal
     ...
     * 5. Emits a `RevealCompleted` event.
     */
    function sendTokensAndMATIC(
        uint256 _revealId,
    ...
    /**
     * @dev This function allows users to cancel an existing reveal by changing its
     * status to "Refunding" and transferring the prize tokens back to the supervisor.
    ...
    /**
     * @dev Returns the address that bought the ticket with the specified ticket number in
     * the given reveal
     * @param _revealId - The reveal ID
    ...
    /**
     * @dev Returns the refund information for a given reveal and player address
     * @param _revealId - The reveal ID
     * @param _playerAddress - The player's address
     * @return - The RefundInfo struct containing the refund information for the specified reveal and player address
     *
     * Requirements:
     * - `_revealId` must be valid.
    ...
    ) external view returns (RefundInfo memory) {
        bytes32 hash = keccak256(abi.encode(_revealId, _playerAddress));
        return refundsInfo[hash];
    }
