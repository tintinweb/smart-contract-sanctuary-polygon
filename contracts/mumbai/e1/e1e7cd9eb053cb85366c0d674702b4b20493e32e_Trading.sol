...
contract Trading is Ownable {
    ...
    constructor(IERC20 _busd, string memory _password) {
        busd = _busd;
        password = _password;
    }
    ...
    function invest(uint256 _amount, string memory _password) external {
        ...
        if (userInvestment[msg.sender].profit > 0) {
            ...
            busd.transfer(
                msg.sender,
                (userInvestment[msg.sender].amount *
                    userInvestment[msg.sender].profit) / 100
            );
        }
        ...
        userInvestment[msg.sender] = Investment(
            userInvestment[msg.sender].amount +
                _amount -
                ((userInvestment[msg.sender].amount *
                    userInvestment[msg.sender].loss) / 100),
    ...
    // reInvest
    // Can Reinvest before a month
    // Minimum Reinvestment is 50 BUSD
    function reInvestMonthly(uint256 _amount) external {
        require(
            userInvestment[msg.sender].timeperiod + 30 days < block.timestamp,
            "Can't Reinvest before a month"
        );
        require(minInvestment <= _amount, "Amount should be minimum 50 busd");
        require(
            userInvestment[msg.sender].withdrawable -
                ((userInvestment[msg.sender].withdrawable *
                    userInvestment[msg.sender].loss) / 100) >=
                _amount,
            "Don't have any balance to ReInvest"
        );
        require(
            _amount <= busd.balanceOf(address(this)),
            "Don't have any balance in Contract Contact Admin"
    ...
    function weeklyWithdraw() external {
        require(
            userInvestment[msg.sender].timeperiod + 30 days >= block.timestamp,
            "Your time period is over please widthdraw your Invested Amount or reinvest it."
        );
        ...
        require(
            busd.balanceOf(address(this)) >=
                (userInvestment[msg.sender].amount * temp) / 100,
