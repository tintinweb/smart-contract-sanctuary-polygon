...
abstract contract ERC721AOperator is IERC721AOperator, ERC721ARoyalty {
    ...
    uint96 internal _royaltyFeeBasis;

    uint256 public entriesCount; //starts from 1

    //nonce used to buy tickets
    ...
    function _saveTickets(uint256 currentIndex, uint256 amount, uint256 entryPos) internal {
        ...
        for (uint256 i; i < amount; i++) {
            ...
            _setTokenRoyalty(tokenId, _royaltyReceiver, _royaltyFeeBasis);
        }
    }
    ...
    function _buildData(
        ...
        return keccak256(abi.encodePacked(receiver, userId, entryPos, amount, nonce + 1));
    }

    /// @dev for signature
    function _getSigner(bytes32 data, bytes memory signature) internal pure returns (address) {
...
import '../nfts/ERC721EW.sol';

import '../OwnedByReserv.sol';


contract ERC721EWFactory is ERC721FactoryWide, IERC721EFactory, OwnedByReserv {
    ...
    function versionERC721Factory() override virtual external pure returns (string memory){
        return "1.0.0-beta.0+fob.rsv.iERC721EWFactory";
    }
    ...
    function deploy(
        ...
        ERC721EW erc721EW = new ERC721EW(
            name, 
            symbol, 
            venue,
            _ownerContract,
        ...
        address contractAddress = address(erc721EW);
...
interface IERC721EW is IERC721Wide {
    ...
    error ERC721EWWrongMerkleProof();
    error ERC721EWUnableToVerifyMerkleProof();
    ...
    /// @param merkleRoot if wallet is whitelisted, ticket price will be 0
    ...
    /// @param signature reserv managers signature
    ...
    function editMerkleRoot(uint256 entryPos, bytes32 newMerkleRoot) external;
...
import '../interfaces/IERC721EW.sol';
...
// @description this contract will use whitelist option for each ticket type, where a whitelisted user will get the ticket for free
contract ERC721EW is IERC721EW, ERC721AMarketplace, OwnedByReserv {
    ...
    mapping(uint256 => EntryType) internal _entryByPos;

    //in the form of userId => (entryPos => amountOfTickets)
    mapping(uint256 => mapping(uint256 => uint256)) internal _userTicketsBought;
    ...
    constructor(
        ...
        address reservOwner,
        bytes memory data
    ) ERC721A(name, symbol) OwnedByReserv(reservOwner) {
        venue = venue_;
        (
            string memory baseUri,
            address paymentToken_,
            address royaltyReceiver, //secondary sales, splitter
            bytes memory extraData
        ) = abi.decode(data, (string, address, address, bytes));

        _baseUri = baseUri;
        paymentToken = paymentToken_;

        (address treasury, uint96 feeBasis, uint96 royaltyFeeBasis) = IFeeManager(reservFeeManagerContract()).getFeeInfo();

        _royaltyReceiver = royaltyReceiver;
        _royaltyFeeBasis = royaltyFeeBasis;
        _setDefaultRoyalty(treasury, feeBasis);
    ...
    function versionERC721() external pure virtual override returns (string memory) {
        return '1.0.0-beta.0+fob.rsv.iERC721EW';
    }
    ...
    /// @inheritdoc IERC721EW
    function editMerkleRoot(uint256 entryPos, bytes32 newMerkleRoot) external virtual override onlyManagers {
        if (entryPos > entriesCount) revert ERC721WrongEntryPos();
        EntryType storage entry = _entryByPos[entryPos];
        entry.merkleRoot = newMerkleRoot;
    }
    ...
    function buyTickets(
        ...
        bytes32 hashData = _buildData(receiver, userId, entryPos, amount);
        address signer = _getSigner(hashData, signature);
        if (!isReserv(signer)) revert ERC721WrongSignature();
        nonce += 1;
        ...
        bytes32 merkleRoot = entryType.merkleRoot;

        //For merkle verification
        if (merkleRoot != '') {
            address wallet = _msgSender();
            if (isReservCardPayee(_msgSender())) wallet = receiver;
            if (!_verifyWhitelist(merkleRoot, merkleProof, wallet)) revert ERC721EWUnableToVerifyMerkleProof();
        }
        ...
        bytes memory data = abi.encode(userId);

        _safeMint(receiver, amount, data);
    ...
    function createEntry(
        ...
        _entryByPos[pos] = EntryType(name, price, maxSupply, maxBuy, 0, saleEnd, merkleRoot);
    ...
    function _verifyWhitelist(
        ...
        return (MerkleProof.verify(merkleProof, merkleRoot, keccak256(abi.encodePacked(addr))) == true);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721AMarketplace) returns (bool) {
        return
            interfaceId == type(IERC721EW).interfaceId ||
