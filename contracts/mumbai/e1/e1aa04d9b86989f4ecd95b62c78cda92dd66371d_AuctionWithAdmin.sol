...
contract AuctionWithAdmin is Ownable {
    ...
    mapping(address => mapping(uint256 => BidDetails)) private bidInfo;
    ...
    /// @param _tokenFee - Token fee.
    function setCancelBidFee(uint256 _tokenFee) public {
    ...
    function bid(uint256 _tokenId, uint256 _amount) public {      
        ...
        if(bidInfo[msg.sender][_tokenId].id != 0){
            bidInfo[bidInfo[msg.sender][_tokenId].prevBidder][_tokenId].nextBidder = bidInfo[msg.sender][_tokenId].nextBidder;
            bidInfo[bidInfo[msg.sender][_tokenId].nextBidder][_tokenId].prevBidder = bidInfo[msg.sender][_tokenId].prevBidder;
            delete bidInfo[msg.sender][_tokenId];
        }
        pending_claim_auction[msg.sender][_tokenId] = _amount;        
        BidDetails memory bid_info;
        curr_bid_id++;
        bid_info = BidDetails({
            id         : curr_bid_id,
        ...
        if(bidInfo[auction[_tokenId].highestBidder][_tokenId].nextBidder == address(0)){
            bidInfo[auction[_tokenId].highestBidder][_tokenId].nextBidder = msg.sender;
        }       
        bidInfo[msg.sender][_tokenId] = bid_info;
        pending_claim_auction[msg.sender][_tokenId] = _amount;
        auction[_tokenId].highestBidder = msg.sender;
        auction[_tokenId].highestBid = _amount;
        auction[_tokenId].totalBids++;
        emit Bid(auction[_tokenId].id, msg.sender, _tokenId, curr_bid_id, _amount, block.timestamp);
    }
    ...
    function cancelBid(uint256 _tokenId) public {
        ...
        if(auction[_tokenId].highestBidder == msg.sender){
            auction[_tokenId].highestBidder = bidInfo[msg.sender][_tokenId].prevBidder;
        }else{
            if(bidInfo[msg.sender][_tokenId].prevBidder == address(0)){
                bidInfo[bidInfo[msg.sender][_tokenId].nextBidder][_tokenId].prevBidder = address(0);
            }else{
                bidInfo[bidInfo[msg.sender][_tokenId].prevBidder][_tokenId].nextBidder = bidInfo[msg.sender][_tokenId].nextBidder;
                bidInfo[bidInfo[msg.sender][_tokenId].nextBidder][_tokenId].prevBidder = bidInfo[msg.sender][_tokenId].prevBidder;
            }
        ...
        delete bidInfo[msg.sender][_tokenId];
        emit BidCancelled(msg.sender, _tokenId, pending_claim_auction[msg.sender][_tokenId] - (pending_claim_auction[msg.sender][_tokenId] * (cancel_bid_fee / 100)), block.timestamp);
        token.transferFrom(address(this), msg.sender, pending_claim_auction[msg.sender][_tokenId] - (pending_claim_auction[msg.sender][_tokenId] * (cancel_bid_fee / 100)));       
    ...
    function auctionClaim(uint256 _tokenId) public {
        ...
        require(
            auction[_tokenId].highestBidder == msg.sender || msg.sender == EnumerableMap.get(auctionId, _tokenId),
    ...
    /// @dev Returns user bid details.
    function getBidInfo(address _user, uint256 _tokenId) public view returns(BidDetails memory userBidInfo){
        return bidInfo[_user][_tokenId];
    }
