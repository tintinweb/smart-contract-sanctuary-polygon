 ...
 */
contract DaddyUsername is ERC721, ReentrancyGuard, Ownable
{
	...
	mapping(uint256 => Username) public usernamesMapping;
	mapping(string => bool) public usernameExistsMapping;
	mapping(uint256 => mapping(address => Username)) public usernameDetails;

	uint256 public username_price = 100000000000000000;
	uint256 public total_usernames = 0;
	uint256 public max_username_supply = 10000;
	IERC20 token;


	
	constructor(IERC20 _token) ERC721("Daddy-Username", "Daddy-U") Ownable()
	{
		token = _token;
	}
	 ...
	 */
	function mintUsername(string memory _username, uint256 amount) external nonReentrant
	{
		require(validateContract(), "Invalid contract!");
		require(amount >= username_price, "Not enough ether!");
		require(validateUsername(_username), "Check username!");
	 ...
	 */
	function mintUsernameMulti(string[] memory _usernames, uint256[] memory amounts) external nonReentrant
	{
		require(validateContract(), "Invalid contract!");
		uint num_usernames = _usernames.length;
		require((num_usernames > 0), "Too many usernames!");


		// All good to go - now mint all the usernames.
		for (uint i=0; i<num_usernames; i++)
		{
			require(validateUsername(_usernames[i]), "Username not valid!");
            require(amounts[i] == username_price, "Not enough token!");
	...
	function validateUsername(string memory _username) internal view returns(bool)
	{
		...
		require(((bytes(_username).length >= 2) && (bytes(_username).length <= 50)), "Must be between 2 and 10 characters.");
	...
	function mintUsernameToken(string memory _username) internal
	{
		...
		uint256 new_token_id = getTokenIdfromString(_username);
		Username storage usernames = usernameDetails[new_token_id][msg.sender];

		uint nonce = 0;
		bytes6 _bg_color = getBackgroundColor(new_token_id, nonce++);
		bytes6 _text_color = getTextColor(new_token_id, nonce++);


		_safeMint(_msgSender(), new_token_id);

		usernames.username = _username;
		usernames.bg_color = _bg_color;
		usernames.text_color = _text_color;
		usernames.exists = true;
		usernamesMapping[new_token_id] = Username(_username, _bg_color, _text_color, true);
	...
	function tokenURI(uint256 _token_id) public view override returns(string memory)
	{
		...
		string memory json = string(
			abi.encodePacked(
				...
				'"attributes": [{"trait_type": "background color", "value": "', username.bg_color, '"},{"trait_type": "text color", "value": "', username.text_color, '"}],',
	...
	function getBackgroundColor(uint _token_id, uint _nonce) internal view returns(bytes6)
	{
		...
		if (rand <= 50) color = "000000"; 
		else if (rand <= 175) color = "000000"; 
		else if (rand <= 300) color = "000000"; 
		else if (rand <= 500) color = "000000"; 
