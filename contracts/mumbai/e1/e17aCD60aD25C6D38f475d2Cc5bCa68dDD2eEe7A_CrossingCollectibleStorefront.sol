...
contract CrossingCollectibleStorefront is ICrossingNFT, AccessControl {
  ...
  Counters.Counter private collectiblesForSaleCounter;
  ...
  function addCollectible(string memory uri, uint256 price)
    public
    onlyRole(CURATOR_ROLE)
    returns (uint256)
  {
    ...
    collectibles[collectibleId] = Collectible(true, uri, price);
    collectiblesForSaleCounter.increment();
    ...
    validCollectibleId(collectibleId)
  {
    ...
    collectiblesForSaleCounter.decrement();
  ...
  //TODO: Fix storage of for-sale collectibles to reduce gaps in collectibles mapping
  function getAllCollectible()
    public
    view
    returns(CollectibleForSale[] memory)
  {
    CollectibleForSale[] memory collectiblesForSale = new CollectibleForSale[](collectiblesForSaleCounter.current());
    uint256 collectibleIndex = 0;
    for(uint256 collectibleId = 0; collectibleId < collectibleCounter.current(); collectibleId++){
      if(collectibles[collectibleId].valid){
        collectiblesForSale[collectibleIndex] = CollectibleForSale(
            collectibleId,
            string(abi.encodePacked(baseURI, collectibles[collectibleId].collectibleURI)),
            collectibles[collectibleId].price
        );
        collectibleIndex += 1;
      }
    }
    return collectiblesForSale;
  }
  ...
  function mintCollectible(uint256 collectibleId)
    ...
    returns (uint256)
  {
    require(msg.value == collectibles[collectibleId].price, "Wrong price");

    string memory uri = collectibles[collectibleId].collectibleURI;
    delete collectibles[collectibleId];
    collectiblesForSaleCounter.decrement();
    uint256 tokenId = crossingNFTContract.safeMint(msg.sender, uri);
