...
contract MarketplaceFactory is AccessControl {
    // events
    ...
    // marketplaces fee collector address
    ...
    /**
     * @notice Constructor
     * @param _feeCollector Address that will receive generated fees in each marketplace (not royalties)
     */
    constructor (address _feeCollector, uint _marketplaceFee) {
    ...
    function deployMarketplace(address owner, address _nft, address _recoverAddress, uint _minBidPriceIncrease, 
        bool _shouldExtendEnd, bool _shouldStopBidding) external onlyRole(DEFAULT_ADMIN_ROLE) {

        Marketplace marketplace = new Marketplace(_nft, _recoverAddress, _minBidPriceIncrease, _shouldExtendEnd, 
            _shouldStopBidding, feeCollector, marketplaceFee);
     ...
     */
    function changeFeeCollector(address _newFeeCollector) external onlyRole(DEFAULT_ADMIN_ROLE) {
        feeCollector = _newFeeCollector;
    }

    /**
     * @notice Change marketplace fee 
     * @param _newFee New fee collector address
     */
    function changeMarketplaceFee(uint _newFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
...
contract Marketplace is Ownable {
     ...
     * @param _shouldStopBidding If true, if a bid price exceeds the direct sale price,
            ...
            the direct sale
     * @param _feeCollector Address that will receive marketplace fees (not royalties)
     */
    constructor (address _nft, address _recoverAddress, uint _minBidPriceIncrease, bool _shouldExtendEnd,
        bool _shouldStopBidding, address _feeCollector, uint _marketplaceFee) {
        nft = IERC721(_nft);
        ...
        marketplaceFeeCollector = _feeCollector;
        marketplaceFee = _marketplaceFee;
    ...
    /**
     * @notice Change marketplace fee collector address
     * @param _newFeeCollector New marketplace fee collector
     */
    function changeMarketplaceFeeCollector(address _newFeeCollector) external {
        require (msg.sender == marketplaceFeeCollector, "Marketplace: not fee collector");

        marketplaceFeeCollector = _newFeeCollector;
    }

    /**
     * @notice Change marketplace fee
     * @param _newFee New marketplace fee
     */
    function changeMarketplaceFee(uint _newFee) external {
        require (msg.sender == marketplaceFeeCollector, "Marketplace: not fee collector");

        // TODO: is 10% max marketplace fee okay?
        require (_newFee / feePrecision <= 10, "Marketplace: fee too high");

        marketplaceFee = _newFee;
    }

    function getSellOrder(uint _indexStart, uint _indexEnd) external returns (SellOrder[] memory) {
        uint quantityFetch = _indexEnd - _indexStart;
        SellOrder[] memory orders = new SellOrder[](quantityFetch);

        for(uint i = 0; i < quantityFetch; i++) {
            SellOrder memory sellOrder = sellOrders[_indexStart+i];
            orders[i] = sellOrder;
        }
    ...
    /*function getSellOrderUnsolded(uint _indexStart, uint _indexEnd) external returns (SellOrder[] memory) {
        uint quantityFetch = _indexEnd - _indexStart;
        SellOrder[] memory orders = new SellOrder[](quantityFetch);

        for(uint i = 0; i < quantityFetch; i++) {
            SellOrder memory sellOrder = sellOrders[_indexStart+i];
            if(!sellOrders.claimed) {
                orders[i] = sellOrder;
            }
        }
