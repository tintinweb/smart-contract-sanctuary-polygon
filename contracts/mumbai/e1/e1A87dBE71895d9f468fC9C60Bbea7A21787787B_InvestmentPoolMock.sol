...
contract InvestmentPool is IInitializableInvestmentPool, SuperAppBase, Context, Initializable {
    ...
    modifier onlyGovernancePoolOrGelato() {
        if (getGovernancePool() != _msgSender() && getGelatoSender() != _msgSender())
    ...
    modifier onlyGelatoSender() {
        if (msg.sender != getGelatoSender()) revert InvestmentPool__NotGelatoDedicatedSender();
        _;
    }
    ...
    function initialize(
        ...
        (gelatoSender, ) = IOpsProxyFactory(getGelatoOpsProxyFactory()).getProxyOf(address(this));
    ...
    function getGelatoSender() public view returns (address) {
        return gelatoSender;
    }
    ...
    function createTask() public payable {
        ...
        // Crafting ModuleData to create a task which utilise RESOLVER ,PROXY & SINGLE_EXEC Module.
...
interface IInvestmentPool is ISuperApp {
    ...
    function getGelatoSender() external view returns (address);
