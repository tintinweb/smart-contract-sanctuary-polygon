...
/// @title Withdraw To zkEVM for Polygon PoS -> zkEVM transfers
/// @author QEDK
/// @notice This contract enables withdrawals to the zkEVM chain from Polygon PoS to specific addresses using the
/// `MessageSent` event.
/// @custom:experimental This is an experimental contract.
contract WithdrawToZkEVM {
    ...
    constructor(IChildChainManager _childChainManager, IFxERC20ChildTunnel _fxERC20ChildTunnel) {
    ...
    /// @param _destination Destination on zkEVM to withdraw to
    function withdraw(IChildToken _childToken, uint256 _amount, address _destination) external {
        IERC20(address(_childToken)).safeTransferFrom(msg.sender, address(this), _amount);
        address _rootToken = childChainManager.childToRootToken(address(_childToken));
        if (_rootToken == address(0)) {
            _rootToken = fxERC20ChildTunnel.rootToChildToken(_rootToken);
            require(_rootToken != address(0), "NO_ROOT_TOKEN");

            fxERC20ChildTunnel.withdraw(address(_childToken), _amount);
        } else {
