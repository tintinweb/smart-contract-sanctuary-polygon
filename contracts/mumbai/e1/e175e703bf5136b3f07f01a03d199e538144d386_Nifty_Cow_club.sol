...
// File: contracts/Nifty_Cow_Club.sol
...
contract Nifty_Cow_club is ERC721A, Ownable, ReentrancyGuard{
    ...
    constructor() ERC721A("Nifty Cow Club", "NCC") {
        paused = true;
    }
    ...
    require(totalSupply() + _mintAmount <= maxSupply, "No more Cow");
    _;
    }

    function mint(uint256 _mintAmount) external payable mintCompliance(_mintAmount) {
        require(paused == false, "Must paused");
  ...
  function withdraw(address _ad1, uint256 _perc) external onlyOwner nonReentrant{
    (bool hs, ) = payable(_ad1).call{value: address(this).balance * _perc / 100}("");
    require(hs);
