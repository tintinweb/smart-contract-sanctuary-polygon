...
contract MetaMuscleLeague is
  ERC721Enumerable,
  Ownable,
  ReentrancyGuard,
  PaymentSplitter
{
  struct MetaMuscleAlien {
    uint256 dna;
  }

  mapping(uint256 => MetaMuscleAlien) internal aliens;
  ...
  uint8 private constant MAX_PER_ADDR = 10;
  uint8 private constant F_RES_AMT = 150;
  uint64 private constant MAX_PUBLIC_MM = 5350;
    ...
    returns (uint256, uint256)
  {
    ...
    require(_count > 0 && _count <= MAX_PER_ADDR, "Invalid count");
    require(_tokenIds.current() + _count <= MAX_PUBLIC_MM, "All tokens minted");
    require(
      allowListClaimed[msg.sender] + _count <= MAX_PER_ADDR,
      "Allowance exceeded"
    );
    require(_count * mintPrice == msg.value, "Not enough Matic sent");
  ...
  function founderMint(uint256 _count)
    ...
    returns (uint256, uint256)
  {
    require(_count > 0, "More than 0 needed");
    require(
      _reservedTokenIds.current() + _count <= F_RES_AMT,
      "Reserved aliens all minted"
    );
    require(
    ...
    uint256 firstMintedId = MAX_PUBLIC_MM + _tokenIds.current() + 1;

    for (uint256 i = 0; i < _count; i++) {
      _reservedTokenIds.increment();
      mint(MAX_PUBLIC_MM + _reservedTokenIds.current());
    }
