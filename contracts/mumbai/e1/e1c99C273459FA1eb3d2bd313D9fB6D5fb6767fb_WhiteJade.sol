...
contract WhiteJade is Ownable, ERC721A, ReentrancyGuard {
    using Strings for uint256;
  ...
  uint256 public  PRICE = 0.001 ether; //0.06 during pre-sale & 0.08 during public-sale
  ...
  uint public status = 0; //0 - sale pause, 1 - pre sale open & 2 - public sale open

  constructor() ERC721A("White Murim Jade Collection","White Jade", MaxMintPerBatch_, TotalCollectionSize_) {
    _uriBeforeRevel = "";
    _baseTokenURI="https://gateway.pinata.cloud/ipfs/QmcEFWecbY9L4DWWRGCaZkv4Hsw42vCzc7gS5VZCMuGVEG/";
  }
  ...
  modifier callerIsUser() {
    ...
    if(status == 1){
    //require(status == 1 , "Pre Sale is not Active");
    require(quantity == 1, "Only one mint in Pre Sale");
    require(numberMinted(msg.sender)< 1  , "Already minted in Pre Sale Round" );
    ...
    require(totalSupply() + quantity <= collectionSize, "This much quantity not available");
    require(numberMinted(msg.sender) + quantity <= MAX_PER_Address  , "Max can hold 5 NFTs" );
