...
contract operator {
    ...
    uint256 private cornMintingFee;

    mapping(uint256 => uint256) private emberMintingFee;

    uint256 private cupMintingFee;
    ...
    function assignNFT(Assign memory assign, Sign memory sign) public payable returns(bool) {
        ...
        if(assign.nftProjectType == projectType.corn) {
           mint721(assign.nftAddress, assign.from, assign.tokenURI);
        }
    ...
    function tradeAsset(Assign memory assign) public {
        for(uint256 i = 0; i < (assign.tokenIds).length; i++) {
            assign.nftAssetType == assetType.ERC1155 ?IERC1155(assign.nftAddress).safeTransferFrom(assign.from, assign.to, assign.tokenIds[i], assign.supply[i], "")
                                                     :IERC721(assign.nftAddress).safeTransferFrom(assign.from, assign.to, assign.tokenIds[i]);
        }
    }

    function LootTransfer(address nftAddress, assetType nftType, address from, address to, uint256 tokenId, uint256 supply) external onlyOwner returns(bool) {
        require(to != address(0),"reciever address is zero address");
        nftType == assetType.ERC1155 
            ?IERC1155(nftAddress).safeTransferFrom(from, to, tokenId, supply, "")
            :IERC721(nftAddress).safeTransferFrom(from, to, tokenId);
        return true;
    }

    function mint721(address nftAddress, address from, string memory tokenURI) internal {
