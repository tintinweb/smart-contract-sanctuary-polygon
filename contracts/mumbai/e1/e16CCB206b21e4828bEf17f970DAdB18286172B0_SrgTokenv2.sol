    ...
    ERC20Votes
{
    ...
    constructor(
        ...
        ColdStaking((MAX_SUPPLY * 5 ether) / 100)
        ERC20Permit("LumiShare LUMI")
    {
        transferFee = 0;
        srgTokenV1 = IERC20(_srgTokenV1);

        //Old SRG Token should be burned by Lumishare Multisig
        _mint(
            multiSigWallet,
            MAX_SUPPLY - 83333333 ether - (MAX_SUPPLY * 5) / 100
        );

        // _fundWallets()

        // Presale balance
        _mint(address(this), 83333333 ether);

        //Staking balance 5% of max supply
        _mint(address(this), (MAX_SUPPLY * 5 ether) / 100);
    ...
    function claimAirdrop() public {
        ...
        // Grant the contract the necessary allowance to transfer the caller's old tokens
        srgTokenV1.approve(address(this), srgTokenV1.balanceOf(msg.sender));
        ...
        require(
            srgTokenV1.transferFrom(msg.sender, address(0x0), oldBalance),
...
contract Illumipresale is Ownable, ReentrancyGuard {
    ...
    function claimTokensWithNative(address referralId, bool stableReward)
        ...
        nonReentrant
    {
        require(!pausedSell, "Presale is paused/ended");
        require(
            !pausedSCSell,
            "Can not buy with native currency at the moment"
        );
        require(
            goldList[_msgSender()] || openSell,
            "Caller is not in Gold list"
        );

        uint256 srgTokens = getAmountOfTokens(msg.value);

        require(
            presaleBalance - srgTokens >= 0,
            "Presale tokens have been sold"
        );

        if (referralActive) {
            _referralReward(referralId, stableReward, srgTokens, address(0));
        }
        ...
        emit TokensClaimed(srgTokens, referralId);
    }
    ...
    function claimTokensWithStable(
        ...
        require(!pausedSell, "Presale is paused/ended");
        require(!pausedSCSell, "Can not buy with StableCoins at the moment");
        require(
            goldList[_msgSender()] || openSell,
            "Caller is not in Gold list"
        );
        ...
        if (referralActive) {
            _referralReward(referralId, stableReward, srgTokens, erc20);
        } else {
            require(
                presaleBalance - srgTokens >= 0,
                "Presale tokens have been sold"
    ...
    function _referralReward(
        ...
        if (referralId == address(0)) return;
        ...
        if (erc20 == address(0)) {
            ...
            tokenForOwner.transferFrom(_msgSender(), owner(), amountForOwner);
            if (stableReward) {
                ...
                require(
                    presaleBalance - srgTokens - amountForReferral >= 0,
                    "Presale tokens have been sold"
                );
                tokenForReferral.transfer(referralId, amountForReferral);
                // Payment comes from here Lumi Launchpad+ presale
...
function unStake(uint256 stakeId) external {
    ...
    /**
     * @dev this function  withdraws balance to the coldstaking
     * @param amount, the amount of tokens that owner will withdraw

     */
    function withdrawBalanceToStaking(uint256 amount) public onlyOwner {
