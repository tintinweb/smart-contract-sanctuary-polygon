...
// File contracts/utils/CampaignERC721.sol
    ...
    IERC721MetadataUpgradeable
{
    ...
    function approve(address to, uint256 tokenId) public virtual override {
        address owner = CampaignERC721.ownerOf(tokenId);
    ...
    function _transfer(
        ...
        require(
            CampaignERC721.ownerOf(tokenId) == from,
    ...
    function _approve(address to, uint256 tokenId) internal virtual {
        ...
        emit Approval(CampaignERC721.ownerOf(tokenId), to, tokenId);
    }
...
// File contracts/utils/CampaignMetadata.sol

pragma solidity ^0.8.0;
/**
 * @notice A mixin to extend the OpenZeppelin metadata implementation.
 */
abstract contract CampaignMetadata is CampaignERC721 {
...
// File contracts/utils/CompareString.sol
...
// File contracts/interface/IAdCreation.sol

pragma solidity ^0.8.9;

interface IAdCreation {
...
interface ITreasury {
    function deposit(uint amount_, uint campaognID_) external payable;
    function withdrawFunds(address payable to, uint256 amount, uint campaignID_) external;
}


// File contracts/CreateCampaign.sol

// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.9;
contract Campaign is CampaignMetadata, CompareString {
    using AddressUpgradeable for address;
    using AddressUpgradeable for address payable;
    address private advertisementContract;
    ...
    struct CampignInfo {
        ...
        string dailyBudget;
        string CPI;
        string estimatedImpression;
        string imagePath;
        uint256 status;
    }

    mapping(uint256 => CampignInfo) public getInfo;
    ...
    error NotAValidCampaign(uint256 campaignID);
    ...
    /**
     * @dev Called once after the initial deployment to set the ad info
     */
    function updateAdvertisementContract(address advertisementContract_)
        external
        onlyOwner
    {
        //onlyInitializing
        require(
            advertisementContract_.isContract(),
            "Campaign: Address is not a contract"
        );
        advertisementContract = advertisementContract_;
    }

    /**
     * @notice Returns the address of the Foundation treasury.
     */
    function getAdvertisementContract() public view returns (address) {
        return advertisementContract;
    }

    function updateTreasuryContract(address payable treasuryContract_)
        external
        onlyOwner
    {
        require(
            treasuryContract_.isContract(),
            "Broadcasting: Address is not a contract"
        );
        treasuryContract = treasuryContract_;
    }
    ...
    // details_[0] = Campaign title;
    // details_[1] = Campaign campaignType;
    // details_[2] = Campaign startdate
    // details_[3] = Campaign endDate
    // details_[4] = Campaign baseCPI
    // details_[5] = Campaign dailyBudget
    // details_[6] = Campaign CPI
    // details_[7] = Campaign estimatedImpression
    // details_[8] = Resource IPFS Path
    function create(
        string memory tokenIPFSPath_,
        uint256[] memory advertisementIDs_,
        uint256 maxBudget_,
        string[9] memory details_ // treat everything as string
    ) external payable {
        uint256 mintedTokenID_ = _mintInternal(tokenIPFSPath_);

        require(
            msg.value == maxBudget_,
            "Value should be matched to Max Budget"
        );

        ITreasury(treasuryContract).deposit{value: msg.value}(
            msg.value,
            mintedTokenID_
        );

        // ITreasury(treasuryContract).send(msg.value);

        // ITreasury(treasuryContract).deposit(msg.value, mintedTokenID_);

        for (uint256 i = 0; i < advertisementIDs_.length; i++) {
            IAdCreation(getAdvertisementContract()).isValidAdvertisement(
                advertisementIDs_[i],
                msg.sender
            );
        }

        getInfo[mintedTokenID_] = CampignInfo(
            msg.sender,
            advertisementIDs_,
            mintedTokenID_,
            details_[0],
    ...
    function updateStatus(uint256 campaignID_, uint256 status_) external {
        isValidCampaign(campaignID_);
        require(
            msg.sender == getInfo[campaignID_].advertiser,
            "Only respective advertiser can update"
        );
        require(
            status_ == 0 || status_ == 1 || status_ == 2,
            "Incorrect status"
        );
        getInfo[campaignID_].status = status_;

        emit CampaignUpdated(campaignID_, msg.sender, status_);
    }

    function isValidCampaign(uint256 campaignID_) public view returns (bool) {
        if (
            _exists(campaignID_) &&
            (getInfo[campaignID_].status == 0 ||
                getInfo[campaignID_].status == 1 ||
                getInfo[campaignID_].status == 2)
        ) return true;
        else revert NotAValidCampaign(campaignID_);
    }

    function getCampaignCPI(uint256 campaignID_)
        public
        view
        returns (string memory)
    {
        return getInfo[campaignID_].baseCPI;
    }
}