...
// File: 02-01-23swapping/PreSale.sol
...
contract PreSale is Ownable{
    ...
    constructor(IERC20 _tokenAddress, uint _price) {
        tokenAddress = _tokenAddress;//IERC20(0x536E9E8971BB206f6fDa001cEbab7E933913d511);
        price = _price;
    ...
    function buy() public payable returns(bool) {
        require(_msgSender()!=address(0),"Null Addres can't buy token");
        // calculate token values aginst reciving accounts
        uint _tokenValue = msg.value.mul(price);

        require(_tokenValue <= tokenForSale()," Remaning Tokens Less than buying value");

        // transfer Eth to seller Address
        seller.transfer(address(this).balance);

        //transfer tokens to buyers address
        tokenAddress.transferFrom(seller, _msgSender(), _tokenValue);


        //update tokenId variable
    ...
    function updateTokenAdress(IERC20 _tokenAddress) public onlyOwner   {
...
// When you create a crypto token you need a presale or IDO smart contract