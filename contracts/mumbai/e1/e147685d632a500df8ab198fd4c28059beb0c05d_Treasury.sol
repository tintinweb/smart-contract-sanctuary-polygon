...
abstract contract CollateralManagement is AdminRole {
    ...
    /**
     * @notice Allows an admin to withdraw ERC-20 token funds.
    ...
    function withdrawNfts(address to, address nftContract, uint256 tokenId) 
    public
    onlyAdmin
    {
        require(address(this) == NFTContract(nftContract).ownerOf(tokenId),"Contract is not the Owner");
        NFTContract(nftContract).transferFrom(address(this),to,tokenId);
...
/**
 * @notice The Drop treasury which collects fees generated by the market.
