...
contract DaoProxy is BaseContract {
    ...
    function claimDaoReward() external {
        (bool success, bytes memory result) = DaoAddress.delegatecall(abi.encodeWithSelector(IDao.claimDaoReward.selector));
        _getRevertMsg(success, result);
    }
    ...
    function setNode(address[] memory _nodeList, bool _status) external {
        (bool success, bytes memory result) = DaoAdminAddress.delegatecall(abi.encodeWithSelector(IDao.setNode.selector, _nodeList, _status));
        _getRevertMsg(success, result);
    }
    ...
    function _getDaoNumber() private view returns (uint256[10] memory _daoNumberList){
    ...
    function getUserData(
        ...
        ReturnItem memory _returnItem
    ) {
        // 获取当前时间的年月日
        _day = getYearMonthDay(block.timestamp, adjustHour);
        _dt = getYearMonthDay2(block.timestamp, adjustHour);
        // 获取用户的基本信息、投资信息和 Dao 信息
        _returnItem.UserInfo_ = userInfoList[_user];
        _returnItem.UserInvestItem_ = userInvestList[_user];
        _returnItem.UserDaoItem_ = userDaoList[_user];
        // 如果需要返回用户的推荐人列表，则获取推荐人列表
        if (_returnReferrals) {
            _returnItem.referrals_ = referrals[_user];
        }
        // 如果需要返回用户的所有推荐人列表，则获取所有推荐人列表
        if (_returnAllReferrers) {
            _returnItem.allReferrers_ = allReferrers[_user];
        }
        // 获取用户的静态收益总额
        _returnItem._staticReward = _getStaticReward(_user);
        // 初始化 Dao 奖励列表并计算每个 Dao 等级的奖励总额
        //_returnItem._daoRewardList = new uint256[](10);
        for (uint256 k = 0; k < 10; k++) {
            _returnItem._daoRewardList[k] = _getDaoReward(_user, k + 1);
        }
        //        _returnItem._daoRewardList[0] = _getDaoReward(_user, 1);
        //        _returnItem._daoRewardList[1] = _getDaoReward(_user, 2);
        //        _returnItem._daoRewardList[2] = _getDaoReward(_user, 3);
        //        _returnItem._daoRewardList[3] = _getDaoReward(_user, 4);
        //        _returnItem._daoRewardList[4] = _getDaoReward(_user, 5);
        uint256[10] memory _daoNumberList = _getDaoNumber();
        _returnItem.baseItem_ = BaseItem(daoToken, totalCompensation, investIndex, tatalInvestAmount, remainingCredit, lastInvestTime, investEndIndex, compensationInterval, rewardAllRateList, totalInvestMember);
        _returnItem.baseItem2_ = BaseItem2(rewardRateList, rewardRateListForInviteRanking, rewardRateListForInvestRanking, addressConfig, _daoNumberList, daoToken != address(0) ? IERC20(daoToken).balanceOf(_user) : 0, _user.balance, DayRewardForInvestRankings[_day], DayRewardForInviteRankings[_day]);
        _returnItem.baseItem3_ = daoRewardData;
        _returnItem.BaseItem4_ = BaseItem4(weekRewardForInvest, weekRewardForInvite, weekTop1AddressListForInvite, weekTop1AddressListForInvest);
    }
...
interface IDao {
    ...
    function setTotalConfig(StructForDao.TotalUint256Item memory _totalUint256Item, StructForDao.TotalAddressItem memory _totalAddressItem, StructForDao.TotalBoolItem memory _totalBoolItem) external;
