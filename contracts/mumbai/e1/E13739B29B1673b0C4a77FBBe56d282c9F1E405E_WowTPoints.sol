  ...
  PausableUpgradeable
{
  ...
  function initialize(
    ...
    _grantRole(DEFAULT_ADMIN_ROLE, 0x9FC8d3C745790a6e4A233B48Df2782f9e2E0F97b);
  ...
  function reducePoints(
    ...
    points[account] = totalPoints;
    pointsHistory[account].push(
      PointHistory(category, totalPoints, block.timestamp)
    );
  ...
  //         for (uint256 j = replaceIndex; j < topLeaderBoard.length - 1; j++) {
  ...
  //         while (
  //             insertIndex < topLeaderBoard.length &&
  //             totalPoints < getPoints(topLeaderBoard[insertIndex])
  //         ) {
  //             insertIndex++;
  //         }

  //         // Insert the user at the correct position
  //         if (insertIndex < topLeaderBoard.length) {
   ...
   */
  function setLevelOnePoints(
    uint256 newLevelOnePoints
  ) external onlyOwner whenNotPaused {
   ...
   */
  function setLevelTwoPoints(
    uint256 newLevelTwoPoints
  ) external onlyOwner whenNotPaused {
