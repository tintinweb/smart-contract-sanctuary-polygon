...
// File contracts/NFTOasisV1.1.sol
...
abstract contract NFTOasisV1_1 is AccessControl, ERC721URIStorage, DefaultOperatorFilterer, Ownable {
    using Strings for uint256;
    ...
    /**
     * @dev Gas Optimization before : 31746 after: 9508
     */
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    function setApprovalForAll(address _spender, bool _approved)
        ...
        virtual
        override
        onlyAllowedOperatorApproval(_spender)
    {
        if (
    ...
    function isApprovedForAll(address __owner, address _spender)
        public
        view
        override
        returns (bool)
    {
        bool original = super.isApprovedForAll(__owner, _spender);
        if (address(approvalProxy) != address(0x0)) {
            return approvalProxy.isApprovedForAll(__owner, _spender, original);
        }
        return original;
    }
...
// File contracts/abegcs3.sol

// License-Identifier: MIT

pragma solidity ^0.8.0;

contract abegcs3 is NFTOasisV1_1 {
    constructor() NFTOasisV1_1("abegcs3", "abegcs3") {}
}