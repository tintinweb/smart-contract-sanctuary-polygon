...
//! --------------------------------------------------------------------------- GETTERS ---------------------------------------------------------------------------
  ...
  /*
  interface IPROXY { // ! AÑADIR INTERFACE proxy / collection
    ...
    constructor(address tokenSC_, address deployer_) { // ! AÑADIR AL CONSTRUCTOR
       
        proxy = IPROXY(tokenSC_);
    }

  function _sendRoyalitiesToBeru(uint256 value_) payable private returns (uint256 toBeru) {
    toBeru = value_ * 0.035;
    payable(beruWallet).transfer(toBeru); //! BeruWallet es la billetera a la que vamos a transferir los montos de beru
    return toBeru;
  }

  function _sendRoyalitiesToCreators(uint256[] memory collectionAddrees_, uint256 value_) payable private {
    uint memory aux = value_ / collectionAddrees_.length; // * Poruqe vamos a dividir el valor total de la oferta por la cantidad de tokens
    for(uint256 i=0 ; i<collectionAddrees_.length ; i++){
        proxy = IPROXY(collectionAddress_[i]);
        if(proxy.getRoyalities()){
            payable(proxy.owner()).transfer(aux * proxy.getRoyalities()); //! Como obtenemos el owner del la collection
        }
    }

    function _sendFoundsToSeller(address seller_, uint256 value_) payable private {
        payable(seller_).transfer(value_);
    }

    function sendFounds(uint256 offerId, uint256 value_) payable private {
        uint256 memory royalitiesToBeru = _sendRoyalitiesToBeru(value_);
        uint256 memory royalitiesToCreators = _sendRoyalitiesToCreators(offersList[offerId_].collectionAddrees_,value_);
        uint256 memory foundsToSeller = value_ - royalitiesToBeru - royalitiesToCreators;
        _sendFoundsToSeller(offersList[offerId_].collectionAddrees_.seller , foundsToSeller);
