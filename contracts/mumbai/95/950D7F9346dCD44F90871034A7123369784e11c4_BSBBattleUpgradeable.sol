...
contract BSBBattleUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    mapping(uint => uint) claimableLevelPercentage;
    ...
    function calculateReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        ...
        uint extraLevel_ = (claimableLevelPercentage[tokenId_] + fractionalClaimableLevelPercentage >= 100) ? 1 : 0;
    ...
    function _calculateSpentStamina(uint secondsLifetime_, bool applyBuff_, Stat memory stamina_) internal view returns(uint) {
        uint spentStamina = secondsLifetime_ * _calculateActualDecreaseRate(applyBuff_) / DAYS;
    ...
    function _calculateYieldReward(
        ...
        return secondsLifetime_ * _calculateDailyYieldWithBonus(yield_, level_, applyBuff_) / DAYS;
    }
    ...
    function _rewardBloodShard(
        ...
        if (!_pickPortionOutOfAmount(
            _getBloodShardWinChancePercentage(level_, applyBuff_), tokenId_, amount_
        )) {
            return;
        }
    ...
    function _pickWinnerFromElites(
        ...
        uint gen0ElitesWeight_ = EnumerableSetUpgradeable.length(elites[faction_][gen0Address]) * 3;
        uint elitesPoolSize_ = gen0ElitesWeight_ + EnumerableSetUpgradeable.length(elites[faction_][gen1Address]);
        ...
        if (winnerWeightedIndex_ >= gen0ElitesWeight_) {
            winnerTokenId_ = EnumerableSetUpgradeable.at(
                elites[faction_][gen1Address], winnerWeightedIndex_ - gen0ElitesWeight_
            );
        ...
        winnerTokenId_ = EnumerableSetUpgradeable.at(
            elites[faction_][gen0Address], winnerWeightedIndex_ / 3
        );
    ...
    function _claimRewards(CollectionItems memory collectionItems_, bool applyBuff_) internal returns (uint) {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            claimableLevelPercentage[collectionItems_.assets[i]] += fractionalClaimableLevelPercentage;

            if (claimableLevelPercentage[collectionItems_.assets[i]] >= 100) {
                levelReward_ += 1;
                claimableLevelPercentage[collectionItems_.assets[i]] -= 100;
            }
...
contract BSBBaseUpgradeable is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    uint256 constant public DAYS = 2 minutes;
