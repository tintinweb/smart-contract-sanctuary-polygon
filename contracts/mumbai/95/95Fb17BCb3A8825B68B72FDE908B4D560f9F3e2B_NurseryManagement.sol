...
contract NurseryManagement is Nursery {
    ...
    constructor(address _BZAI, uint256 _preMint,address _NFTreserve) Nursery(_preMint,_NFTreserve)  {
        BZAI = IERC20(_BZAI);
        for (uint256 i = 1 ; i <= _preMint ; ){
             eggsPrices[i] = ZaiStruct.EggsPrices(1000 * 1E18,5000 * 1E18,10000 * 1E18,100000 * 1E18);
            unchecked { ++ i; }
        }
    }
    ...
    function _getZaiPriceAvarage() internal view returns(uint256[4] memory){
...
contract Nursery is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _preMint, address _NFTreserve) ERC721("Nursery_NFT", "NURS") {
    ...
    function mintNursery(address _to, uint256 _bronzePrice, uint256 _silverPrice, uint256 _goldPrice, uint256 _platinumPrice)
        ...
        returns (uint256)
    {
        require(_bronzePrice >= minPrice,string(abi.encodePacked("Bronze egg can't be lower than",minPrice)));
