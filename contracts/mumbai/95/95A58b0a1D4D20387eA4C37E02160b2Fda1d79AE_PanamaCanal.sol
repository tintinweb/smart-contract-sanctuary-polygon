...
contract PanamaCanal is GameManager, VRFManager, ERC2771Context, AccessControlEnumerable {
        ...
        override
    {
        ...
        if(gameData[gameNumber].state == GameState.GETTING_INITIAL_VRF) {
            ...
            uint256[] memory dealtCards = dealCards(3, gameNumber, _randomWords);
            ...
            uint256[] memory dealtCards = dealCards(1, gameNumber, _randomWords);
            ...
            if(handValue >= 21){
                ...
                if (handValue > 21) {
                    ...
                    if(handPointer + 1 == gameData[gameNumber].numberOfPlayerHands){ // fix this
            ...
            uint256[] memory dealtCards = dealCards(2, gameNumber, _randomWords);
        ...
        else if (gameData[gameNumber].state == GameState.GETTING_VRF_DEALER){

            uint256 playerPayout;
           
            uint256[] memory dealtCards = dealCards(16, gameNumber, _randomWords);

            assignNextDealerCard(gameNumber, dealtCards[0]);

            // determine dealers hand
            (uint256 dealerHandValue, bool soft) = getHandValue(gameData[gameNumber].dealerCards);
    ...
    function hit() public {
        ...
        if(
            ...
            require(
                ...
                gameData[gameNumber].playerDealtCards[handPointer][1].number != 0,
        ...
        returns (uint256[] memory)
    {
        ...
        for (; g.drawIndex < startingIndex + _numberOfCards; ) {
            uint256 randomNum = _randomWords[g.drawIndex - startingIndex] %
                cardsRemaining;
...
contract VRFManager is VRFConsumerBaseV2 {
        ...
        VRFConsumerBaseV2(address(_vrfManagerConstructorArgs.VRFCoordinator))
    {
        ...
        uint256 i;
        for(; i < keyHash.length;){

            keyHash[i] = _vrfManagerConstructorArgs.keyHash;
    ...
    function updateKeyHash(bytes32 _keyHash, uint256 _index) public virtual {
        keyHash[_index] = _keyHash;
    }
}
