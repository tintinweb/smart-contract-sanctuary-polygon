...
// File contracts/pwspad/PWSDrop.sol


// PWS Contracts v1.0.0

pragma solidity ^0.8.15;
abstract contract PWSDrop is ERC721Enumerable, Ownable, ReentrancyGuard {
    // PWSPad
    address private immutable dappMaintenanceWallet;
    uint256 private immutable dappMaintenanceFee = 1;
    ...
    uint256 public mintRoyalties;

    bool public isPaused = true;
    uint256 public totalMintRewards = 0;

    mapping(uint256 => uint256) private tokensMatrix;
    uint256 private amountToReserve;
    ...
    constructor(
        ...
        address dappMaintenanceWallet_,
        uint256 maxSupply_,
        uint256 maxMint_,
        uint256 price_,
        uint256 whitelistPrice_,
        uint256 amountToReserve_,
        uint256 mintRoyalties_
    )
        ERC721(name_, symbol_)
    {
        baseTokenURI = baseTokenURI_;
        dappMaintenanceWallet = dappMaintenanceWallet_;
        projectWallet = projectWallet_;
        maxSupply = maxSupply_;
        maxMint = maxMint_;
        price = price_;
        whitelistPrice = whitelistPrice_;
        amountToReserve = amountToReserve_;
    ...
    function setPrices(uint256 price_, uint256 whitelistPrice_)
        external
        onlyOwner
    {
        price = price_;
    ...
    function mint(uint256 amount) external payable nonReentrant {
        ...
        uint256 price_ = isWhitelisted(_msgSender()) ? whitelistPrice : price;
        ...
        uint256 launchpadRoyalties = (msg.value * dappMaintenanceFee) / 100;
        ...
        totalMintRewards += msg.value - launchpadRoyalties - royalties;
        payout(msg.value - royalties);
...
// File contracts/BrainyBudz.sol


// Developer: https://twitter.com/Chief_Fredoo

pragma solidity ^0.8.15;
contract BrainyBudz is PWSDrop {
    constructor(address projectWallet_, address dappMaintenanceWallet_)
        PWSDrop(
            "BrainyBudz",
            "BB",
            "ipfs://brainybudzeieefy2tnflplvhgl2esazeggssbfxeotoaph5bhxlv2sqlorkfrmu/",
            projectWallet_,
            dappMaintenanceWallet_,
            8420,
            20,
            8.42 ether,
            6.42 ether,
