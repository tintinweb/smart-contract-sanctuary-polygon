...
contract SavageMintFeeRegistrar is Operator {
  ...
  function _payFee(address tokenIn, uint256 amountIn, uint256 paymentId, bool isNative) private returns (bool success) {
    ...
    if (tokenIn != intermediateStablecoin_) {
      stableAmount = _swap(_getInPath(tokenIn, intermediateStablecoin_), amountIn, this_);
    }
    success = stableAmount >= requiredStableAmount;
    require(success, "TokenIn amount not cover price");
    uint256 amountOut = amountIn;
    if (tokenIn != savageToken_) {
      amountOut = _swap(_getInPath(tokenIn, savageToken_), amountOut, this_);
    }
