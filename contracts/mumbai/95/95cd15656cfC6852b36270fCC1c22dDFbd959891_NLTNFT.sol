...
contract NLTNFT is ERC1155, Ownable {
    ...
    constructor() payable ERC1155('https://nlt.rocks/metadata/'){
        mintPrice = 1 ether;
        totalSupply = 0;
        maxSupply = 1000;
    ...
    function  mint(address account,uint256 id) payable public onlyOwner{
        ...
        if(id == 0){
            require(msg.value == 1 ether,'wrong mint price');
        }
        if(id == 1){
            require(msg.value == 2 ether,'wrong mint price');
        }
        if(id == 2){
            require(msg.value == 4 ether,'wrong mint price');
        }
        totalSupply ++;
        _mint(account, id, 1, '');
    }
