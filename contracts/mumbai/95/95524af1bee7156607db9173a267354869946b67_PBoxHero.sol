    ...
    UUPSUpgradeable
{
    ...
    PHero _hero;
    IERC20 _token;
    uint32 public constant MAX_BOX = 349600;
    uint256 public constant BOX_PRICE = 10 * 10**18; // 10 PPP
    ...
    function buyBox() public returns (uint256) {
        require(_tokenIdCounter.current() < MAX_BOX, "BOX_SALE_OUT");
        require(!_blackListAccounts[msg.sender], "ADDRESS_BLACKLIST");

        _token.transferFrom(msg.sender, address(this), BOX_PRICE);
