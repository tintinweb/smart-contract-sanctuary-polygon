...
/// @title Koia membership and also a PFP
contract Member is Ownable, ERC721A {
    using Strings for uint256;
    using SafeERC20 for IERC20;

    bytes32 public merkleRoot;
    uint256 public start; // Start time
    uint256 public immutable price; // Price of each token in USDC
    ...
    constructor(
        ...
        require(_maxSupply > 0, "Member: 0 max supply");
        require(_paymentToken != address(0), "Member: 0 payment token address");
        require(_price > 0, "Member: 0 price");
    ...
    function mint(bytes32[] calldata merkleProof) external {
        require(start != 0, "Member: sale not started");
        require(start <= block.timestamp, "Member: sale not started");
        require(!claimed[msg.sender], "Member: minted already");
        require(totalSupply() < maxSupply, "Member: exceed max supply");
    ...
    function setStart(uint256 _start) external onlyOwner {
        require(
            ...
            "Member: sale already started"
        );
        require(_start > block.timestamp, "Member: start time in the past");
        start = _start;
    }

    /// @param _baseTokenUri the collection's URL
    function setBaseTokenURI(string memory _baseTokenUri) external onlyOwner {
        require(bytes(_baseTokenUri).length > 0, "Member: empty URI");
    ...
    function withdraw(address to) external onlyOwner {
        require(to != address(0), "Member: 0 address");
