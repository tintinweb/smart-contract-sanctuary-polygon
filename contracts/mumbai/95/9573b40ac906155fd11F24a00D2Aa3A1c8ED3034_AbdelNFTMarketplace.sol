...
contract AbdelNFTMarketplace is ERC1155Holder, ReentrancyGuard, Ownable {
    ...
    function listItem(
        ...
        require(listData.nftAddress != address(0), "listItem: nft address is zero");
        ...
        require(listData.itemsAvailable != 0, "listItem: cannot list zero numner of items");
    ...
    function cancelListing(uint256 _itemId) external itemExists(_itemId) {
        ...
        if (item_.saleKind == SaleKind.Auction) {
            if (bid >= item_.reservePrice) {
                require(
                    block.timestamp < item_.expirationTime,
                    "cancelListing: winning bid been made, cannot cancel"
                );
    ...
    function testing(uint max, Item memory item) external {
