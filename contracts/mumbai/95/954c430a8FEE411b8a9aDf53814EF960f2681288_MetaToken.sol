...
import "../storage/MetaTokenStorage.sol";
...
contract MetaToken is ERC721, IMetaToken, MetaTokenInternal, MetaRestrictionsInternal, OwnableInternal {
    using Hooks for Hooks.Hook;
    using MetaTokenStorage for MetaTokenStorage.Layout;
    ...
    function registerBeforeTransferHook(address facet, bytes4 selector) external onlyOwner  {
        MetaTokenHooksStorage.addBeforeTokenTransferHook(MetaTokenHooksStorage.layout(), facet, selector);
    }
   
    function unregisterBeforeTransferHook(address facet, bytes4 selector) external onlyOwner  {
        MetaTokenHooksStorage.removeBeforeTokenTransferHook(MetaTokenHooksStorage.layout(), facet, selector);
    }
    ...
    function attach(uint256 fromPid, uint256 toPid, bytes32[] calldata categories) external {
        require(ownerOf(fromPid) == msg.sender && ownerOf(toPid) == msg.sender, "Only owner can attach");

        MetaGovernanceStorage.Layout storage l = MetaGovernanceStorage.layout();
        address propRegistry = l.getPropertyRegistry();

        for(uint256 c=0; c < categories.length; c++){
            (bytes32[] memory props,) = IMetaPropertyRegistry(propRegistry).getCategoryInfo(categories[c]);

            for(uint256 i=0; i < props.length; i++){
                _moveProperty(fromPid, toPid, props[i]);
            }
        }
    }

    function attach(uint256 fromPid, uint256 toPid, bytes32 category) external {
        require(ownerOf(fromPid) == msg.sender && ownerOf(toPid) == msg.sender, "Only owner can attach");

        MetaGovernanceStorage.Layout storage l = MetaGovernanceStorage.layout();
        address propRegistry = l.getPropertyRegistry();          

        (bytes32[] memory props,) = IMetaPropertyRegistry(propRegistry).getCategoryInfo(category);
    ...
    function detach(uint256 pid, bytes32[] calldata categories) external returns(uint256 newPid) {
        require(ownerOf(pid) == msg.sender, "Only owner can split");

        MetaGovernanceStorage.Layout storage l = MetaGovernanceStorage.layout();
        address propRegistry = l.getPropertyRegistry();          

        newPid = _mintInternal(msg.sender);

        for(uint256 c=0; c < categories.length; c++){
            (bytes32[] memory props, bool splitAllowed) = IMetaPropertyRegistry(propRegistry).getCategoryInfo(categories[c]);
            require(splitAllowed, "Category can not be splited");

            for(uint256 i=0; i < props.length; i++){
                _moveProperty(pid, newPid, props[i]);
            }
        }
    }
    ...
    function detach(uint256 pid, bytes32 category) external returns(uint256 newPid) {
        ...
        (bytes32[] memory props, bool splitAllowed) = IMetaPropertyRegistry(propRegistry).getCategoryInfo(category);
        require(splitAllowed, "Category can not be splited");
    ...
    function _moveProperty(uint256 fromPid, uint256 toPid, bytes32 prop) internal {
        MetaPropertiesStorage.Layout storage propStoragelayout = MetaPropertiesStorage.layout();
 
        if(!propStoragelayout.exist(fromPid, prop)) return;
        if(!propStoragelayout.exist(toPid, prop)){
            // No conflict
            MetaPropertyTransferHooksStorage.layout().executeBeforeTransferHook(prop, fromPid, toPid);
            moveAllRestrictionsInternal(fromPid, toPid, prop);
            propStoragelayout.move(fromPid, toPid, prop);
        } else {
            // Have conflist, let the PropertyManager to solve it
            MetaPropertyTransferHooksStorage.layout().executeOnTransferConflictHook(prop, fromPid, toPid);
        }
    }

    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        Hooks.Hook[] storage hooks = MetaTokenHooksStorage.layout().beforeTokenTransferHooks;
    ...
    function _isMinter(address sender) internal view returns(bool){
        return IMetaPropertyRegistry(MetaGovernanceStorage.layout().propertyRegistry).isMinter(sender);
    }

    function _mintInternal(address beneficiary) internal returns(uint256){
        uint256 newPid = MetaTokenStorage.layout().generateNextPid();
