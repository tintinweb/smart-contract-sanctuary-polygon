...
contract TestContract is BaseContract {
    ...
    //添加多个周地址，以满足触发条件
    function addWeekAddress(address[] memory _addressList) external onlyOwner {
        uint256 _num = _addressList.length;
        for (uint256 i = 0; i < _num; i++) {
            if (weekTop1AddressListForInvite.length < 7) {
                weekTop1AddressListForInvite.push(_addressList[i]);
            }
            if (weekTop1AddressListForInvest.length < 7) {
                weekTop1AddressListForInvest.push(_addressList[i]);
            }
        }
    }
    ...
    function addLast100(address[] memory _addressList) external onlyOwner {
        ...
        for (uint256 i = 0; i < _num; i++) {
            ...
            if (!Last100Addresses.contains(_user)) {
                Last100Addresses.add(_user);
            }
            //往dao1、dao2、dao3、dao4、dao5加地址
            for (uint256 k = 0; k < 5; k++) {
                if (!DaoSetList[k].contains(_user)) {
                    DaoSetList[k].add(_user);
                }
            }
        }
    }
