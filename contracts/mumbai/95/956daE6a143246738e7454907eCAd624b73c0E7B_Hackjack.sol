...
contract HackjackVRF is VRFConsumerBaseV2, BreakdownUint256, Destructible, RequestQueue { 
    ...
    mapping(uint256 => uint256) requestIdToRequestIndex;
    mapping(uint256 => uint256) public requestIndexToRandomWords;
    ...
    mapping(uint256 => requestInfo) public requestMap; 
    ...
    function requestRandomWords() public {
        ...
        requestInfo memory newRequest = requestInfo(false, false,false,0,0);

        requestIdToRequestIndex[requestId] = _requestCounter.current();
        requestMap[_requestCounter.current()] = newRequest;
        enqueue(_requestCounter.current());
    ...
    function fulfillRandomWords(
        ...
        requestId;
        // uint256 requestNumber = requestIdToRequestIndex[requestId];
        // uint256 randomWord = randomWords[0];
        // uint8[] memory randomUint8s = getUint256BrokenIntoUint8(randomWord);

        // requestIndexToRandomWords[requestNumber] = randomWords[0];
        // requestMap[requestNumber].isFulfilled = true; 
    }
}
...
contract RequestQueue {
    ...
    function enqueue(uint256 data) internal {
        _queue.enqueue(data);
        if (_queue.length() == 32) {
    ...
    function dequeue() internal returns (uint256 data) {
        data = _queue.dequeue();
        emit OperationResult(data);
    }

    function peek() internal view returns (uint256 data) {
        return _queue.peek();
    }

    function peekLast() internal view returns (uint256 data) {
