...
contract ReMintOrder is Owner {
    ...
    event Recovery(address indexed user, uint256 energy, uint256 indexed tokenId);

    constructor(ITemplar templar_, address slaveToken_, address pdToken_, address pdPair_, address ngcToken_) Owner() {
        templar = templar_;
    ...
    function orderMint(address tokenAddress, uint256 nftAmount) external checkContractCall checkPaused {
    ...
    function _getEnergy(address tokenAddress, uint256 tokenId) private view returns (uint256) {
    ...
    function getEnergy(address tokenAddress, uint256 tokenId) external view returns (uint256) {
        return _getEnergy(tokenAddress, tokenId);
    }

    function reMint(address tokenAddress, uint256[] memory tokenIds) external checkContractCall checkPaused {
        uint256 amount = 3;
    ...
    function recovery(address tokenAddress, uint256 tokenId, uint256 energy, address payToken) external checkContractCall checkPaused {
        require(tokenAddress == slaveToken, "character error");
        require(IERC721(tokenAddress).ownerOf(tokenId) == msg.sender, "not yours");

        uint256 eng = _getEnergy(tokenAddress, tokenId);

        require(eng + energy <= maxEnergy, "over flaw");
        uint fee = recoveryFee * energy;
