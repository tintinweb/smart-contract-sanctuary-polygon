...
contract RentalPool is AccessControl, IRentalPool {
    ...
    mapping(address => mapping(address => mapping(string => bool)))
        public ownerPendingMission;
    mapping(address => bool) public whitelistedOwners;
    EnumerableSet.Bytes32Set private usedUuid;

    bytes32 public constant MISSION_MANAGER = keccak256("MISSION_MANAGER");
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        _grantRole(MISSION_MANAGER, _missionManager);
    }
    ...
    function whitelistOwner(address _owner)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whitelistedOwners[_owner] = true;
    }

    function whitelistMultipleOwners(address[] calldata _owners)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function removeWhitelistedOwner(address _owner)
        external
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        whitelistedOwners[_owner] = false;
    }

    function createMission(NFTRental.Mission calldata newMission)
        external
        override
    {
        _createMission(newMission);
    }
    ...
    function createMultipleMissions(NFTRental.Mission[] calldata newMission)
        ...
        override
    {
        for (uint256 i; i < newMission.length; i++) {
            _createMission(newMission[i]);
        }
    }
    ...
    function sendStartingMissionNFT(
        ...
        delete ownerPendingMission[curMission.owner][curMission.tenant][
            curMission.dappId
        ];
    }

    function cancelPendingMission(string calldata _uuid) external override {
        NFTRental.Mission memory curMission = IMissionManager(missionManager)
            .getPendingMission(_uuid);
        ...
        delete ownerPendingMission[msg.sender][curMission.tenant][
            curMission.dappId
        ];
        IMissionManager(missionManager).cancelMission(_uuid);
    }

    function batchCancelPendingMission(string[] calldata _uuid)
        external
        override
    {
        for (uint32 i; i < _uuid.length; i++) {
            NFTRental.Mission memory curMission = IMissionManager(
                missionManager
            ).getPendingMission(_uuid[i]);
            require(msg.sender == curMission.owner, "Not mission owner");
            _transferAndUnstakeNFTs(
                curMission.owner,
                curMission.owner,
            ...
            IMissionManager(missionManager).cancelMission(_uuid[i]);
        }
    }
    ...
    function isUuidUsed(string calldata uuid)
        ...
        returns (bool isUsed)
    {
        return usedUuid.contains(keccak256(abi.encode(uuid)));
    }

    function _unstakeNFTs(
        address _owner,
    ...
    function _stakeNFT(
        ...
        for (uint32 i; i < _tokenIds.length; i++) {
            for (uint32 j; j < _tokenIds[i].length; j++) {
                ...
                stakedNFTs[msg.sender][_collections[i]].add(_tokenIds[i][j]);
                emit NFTStaked(_collections[i], msg.sender, _tokenIds[i][j]);
            }
        }
    }

    function _createMission(NFTRental.Mission calldata newMission) internal {
        require(whitelistedOwners[msg.sender], "Owner is not whitelisted");
        _verifyParam(
            newMission.dappId,
            newMission.collections,
            newMission.tokenIds
        );
        require(msg.sender == newMission.owner, "Sender is not mission owner");
        require(
            IMissionManager(missionManager).tenantHasOngoingMissionForDapp(
                newMission.tenant,
                newMission.dappId
            ) == false,
            "Tenant already have mission for dapp"
        );
        require(
            ownerPendingMission[msg.sender][newMission.tenant][
                newMission.dappId
            ] == false,
            "Owner already have pending mission for tenant and dapp"
        );
        require(
            !usedUuid.contains(keccak256(abi.encode(newMission.uuid))),
            "Uuid has already been used"
        );
        _stakeNFT(newMission.collections, newMission.tokenIds);
        IMissionManager(missionManager).setMission(newMission);
        usedUuid.add(keccak256(abi.encode(newMission.uuid)));
        ownerPendingMission[msg.sender][newMission.tenant][
            newMission.dappId
        ] = true;
    }
    ...
    function _verifyParam(
        ...
        require(
            ...
            "Incorrent lengths collections and tokenIds"
        );
    ...
    function _requireStakedNFTs(
        address[] calldata _collections,
        uint256[][] calldata _tokenIds
    ) internal view {
        for (uint32 i = 0; i < _tokenIds.length; i++) {
            for (uint32 j = 0; j < _tokenIds[i].length; j++) {
                require(
                    stakedNFTs[msg.sender][_collections[i]].contains(
                        _tokenIds[i][j]
                    ),
                    "NFT is not staked"
                );
