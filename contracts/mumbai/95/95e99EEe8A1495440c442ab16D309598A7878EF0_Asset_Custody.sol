...
interface IDelegator{
    ...
    function OnTransfer(address _contract, uint256 _tokenId, address from_addr, address to_addr) external;
...
contract Asset_Custody is Ownable, IERC721Receiver{
    ...
    function GetDelegator() private view returns (IDelegator){
        return delegator;
    }
    ...
    function nftMint(address _contract, string memory token_uri) public onlyOwner returns (uint256){
        GetDelegator().OnMint(_contract, msg.sender, token_uri);
    }
