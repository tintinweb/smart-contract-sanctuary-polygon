...
contract OSBStakeFactory is IOSBStakeFactory, ContextUpgradeable {
    ...
    function initialize(
        ...
        emit ContractDeployed(_settingAddress, _osbPassAddress, _libraryNFTStakeAddress, _libraryEditionStakeAddress, _libraryTokenStakeAddress);
    }
    ...
    modifier accessPass(AccessFunctionType _accessType) {
        require(osbPass.isAccessApproval(_accessType, _msgSender()), "Unauthorize");
        _;
    }
    ...
    function createTokenStake(
    ...
    ) external accessPass(AccessFunctionType.STAKING) returns (address) {
        lastId.increment();
        ...
        ContractInfo memory contractInfo = _createStake(ContractType.TOKEN_STAKE, address(_tokenStake), _defaultAdmin, _rewardToken, _stakingToken);
    ...
    function createNFTStake(
        ...
        ContractInfo memory contractInfo = _createStake(ContractType.NFT_STAKE, address(_nftStake), _defaultAdmin, _rewardToken, _stakingToken);
    ...
    function createEditionStake(
        ...
        ContractInfo memory contractInfo = _createStake(
            ContractType.EDITION_STAKE,
            address(_editionStake),
            _defaultAdmin,
            _rewardToken,
            _stakingToken
        );
