...
contract BSGBinance {
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
        ...
        RewardInfo storage userRewards = rewardInfo[_user];
        uint256 totalRewards = userRewards.directs.add(userRewardf.level2Released).add(userRewardf.level3Released).add(userRewardf.level4Released).add(userRewardf.level5Released);
