...
contract MultiSigWallet is Context {
    ...
    modifier onlyOwners() {
        require(isOwner[_msgSender()], "Caller is not owner");
        _;
    }

    modifier ownerExists(address owner) {
        require(isOwner[owner], "Owner not exists");
        _;
    }

    modifier ownerDoesNotExist(address owner) {
        require(!isOwner[owner], "Address is already owner");
        _;
    }
    ...
    modifier confirmed(bytes32 transactionId, address owner) {
        require(confirmations[transactionId][owner], "Owner already confirmed transaction");
        _;
    }

    modifier notConfirmed(bytes32 transactionId, address owner) {
        require(!confirmations[transactionId][owner], "Owner has not yet confirmed the transaction");
        _;
    }
    /**
     * @dev Returns the address of the owners.
     */
    function _owners() public view returns (address[] memory) {
        return owners;
    }
    ...
    function isConfirmed(bytes32 transactionId)
        ...
        returns (bool)
    {
        if(owners.length == 1 && isOwner[_msgSender()]){
            return  true;
        }
        uint count = 0;
    ...
    /// @dev Allows to get transaction id by function name.
    /// @param functionName String of multi sign transaction.
    function getTransactionIdForMultiSign(string memory functionName) pure public returns(bytes32){
        return keccak256(abi.encode(functionName));
    }
    ...
    function addOwner(address owner)
        ...
        onlyOwners
    {
        bytes32 transactionId = keccak256(abi.encode('addOwner'));
        require(isConfirmed(transactionId), "Permission of all owners required");
        isOwner[owner] = true;
        owners.push(owner);
        clearPermission(transactionId);
    ...
    function removeOwner(address owner)
        ...
        onlyOwners
    {
        bytes32 transactionId  = keccak256(abi.encode('removeOwner'));
        require(isConfirmed(transactionId), "Permission of all owners required");
        isOwner[owner] = false;
    ...
    function replaceOwner(address owner, address newOwner)
        ...
        onlyOwners
    {
        bytes32 transactionId  = keccak256(abi.encode('replaceOwner'));
        require(isConfirmed(transactionId), "Permission of all owners required");
        for (uint i=0; i < owners.length; i++)
            if (owners[i] == owner) {
                owners[i] = newOwner;
     ...
     * Public functions
     */
    /// @dev Contract constructor sets initial owners of confirmations.
    /// @param transactionId TransactionId of function name.
    function MultiSignTransaction(bytes32 transactionId)
        public
    ...
    /// @dev Contract constructor remove after transaction of owners confirmations.
    /// @param transactionId Transaction ID.
    function clearPermission(bytes32 transactionId) internal {
        for (uint i = 0; i < owners.length; i++)
              delete confirmations[transactionId][owners[i]];
    }
 ...
 */
contract EDVToken is MultiSigWallet, IERC20, IERC20Metadata {
    ...
    /*
     @dev function to receive external payable
    */
    ...
    */
    function mint(uint256 _amount) public onlyOwners {
        bytes32 transactionId  = keccak256(abi.encode('mint'));
        require(isConfirmed(transactionId), "Permission of all owners required");
    ...
    /*
     ...
     @param _amount uint256
    */
    function burn(uint256 _amount) public onlyOwners {
        bytes32 transactionId  = keccak256(abi.encode('burn'));
        require(isConfirmed(transactionId), "Permission of all owners required");
    ...
    */
    function withdraw(uint256 amount) external onlyOwners{
        bytes32 transactionId  = keccak256(abi.encode('withdraw'));
        require(isConfirmed(transactionId), "Permission of all owners required");
    ...
    /* 
     @dev function to transfer any ERC20 token
     @param tokenAddress token contract address
     @param tokens amount of tokens
     @return success boolean status
    */
    function transferAnyERC20Token(address recipient, address tokenAddress, uint tokens) public onlyOwners returns (bool success) {
        bytes32 transactionId  = keccak256(abi.encode('transferAnyERC20Token'));
        require(isConfirmed(transactionId), "Permission of all owners required");
        clearPermission(transactionId);
        return IERC20(tokenAddress).transfer(recipient, tokens);
    }
 ...
 */
abstract contract ERC1363 is EDVToken, IERC1363, ERC165 {
