...
contract Trading{
    ...
    event ChainlinkCallbackTimeout(uint orderId, StorageInterface.PendingMarketOrder order);
    ...
    function openTrade(
        ...
        NftRewardsInterface.OpenLimitOrderType _type,
        ...
        if(_type != NftRewardsInterface.OpenLimitOrderType.LEGACY){
            ...
            uint orderId = aggregator.getPrice(
                t.pairIndex, 
                AggregatorInterface.OrderType.MARKET_OPEN, 
    ...
    function closeTradeMarket(uint _pairIndex, uint _index) external notContract notDone{
        ...
        StorageInterface.Trade memory t = storageT.openTrades(msg.sender, _pairIndex, _index);
        StorageInterface.TradeInfo memory i = storageT.openTradesInfo(msg.sender, _pairIndex, _index);
        ...
        uint orderId = storageT.priceAggregator().getPrice(
            _pairIndex, 
            AggregatorInterface.OrderType.MARKET_CLOSE, 
            t.initialPosToken * t.leverage * i.tokenPriceDai / PRECISION
        );

        storageT.storePendingMarketOrder(
            StorageInterface.PendingMarketOrder(
                StorageInterface.Trade(msg.sender, _pairIndex, _index, 0, 0, 0, false, 0, 0, 0),
                0, 0, 0, 0, 0
            ), orderId, false
        );
    ...
    function updateOpenLimitOrder(
        ...
        StorageInterface.OpenLimitOrder memory o = storageT.getOpenLimitOrder(msg.sender, _pairIndex, _index);
    ...
    function updateSl(uint _pairIndex, uint _index, uint _newSl) external notContract notDone{
        ...
        if(_newSl == 0 || !aggregator.pairsStorage().guaranteedSlEnabled(_pairIndex)){
            ...
            uint orderId = aggregator.getPrice(
                _pairIndex,
                AggregatorInterface.OrderType.UPDATE_SL, 
                levPosDai / PRECISION
            );

            aggregator.storePendingSlOrder(
                orderId, 
                AggregatorInterface.PendingSl(msg.sender, _pairIndex, _index, t.openPrice, t.buy, _newSl)
            );
    ...
    function executeNftOrder(
        StorageInterface.LimitOrder _orderType, 
        address _trader, 
        uint _pairIndex, 
        uint _index,
        uint _nftId, 
        ...
        StorageInterface.Trade memory t = storageT.openTrades(_trader, _pairIndex, _index);
        ...
        require(_orderType == StorageInterface.LimitOrder.OPEN ? storageT.hasOpenLimitOrder(_trader, _pairIndex, _index)
            : t.leverage > 0, "NO_TRADE");
        require(_orderType != StorageInterface.LimitOrder.SL || t.sl > 0, "NO_SL");
        require(_orderType != StorageInterface.LimitOrder.LIQ || t.sl == 0, "HAS_SL");

        AggregatorInterface aggregator = storageT.priceAggregator();
        NftRewardsInterface nftIncentives = aggregator.nftRewards();

        NftRewardsInterface.TriggeredLimitId memory triggeredLimitId = NftRewardsInterface.TriggeredLimitId(
        ...
        if(!nftIncentives.triggered(triggeredLimitId) || nftIncentives.timedOut(triggeredLimitId)){
            ...
            if(_orderType == StorageInterface.LimitOrder.OPEN){
                StorageInterface.OpenLimitOrder memory l = storageT.getOpenLimitOrder(_trader, _pairIndex, _index);
                leveragedPosDai = l.positionSize * l.leverage;
            }else{
                StorageInterface.TradeInfo memory i = storageT.openTradesInfo(_trader, _pairIndex, _index);
            ...
            uint orderId = aggregator.getPrice(
                _pairIndex, 
                _orderType == StorageInterface.LimitOrder.OPEN ? 
                    AggregatorInterface.OrderType.LIMIT_OPEN : 
                    AggregatorInterface.OrderType.LIMIT_CLOSE,
                leveragedPosDai
            );

            storageT.storePendingNftOrder(
                StorageInterface.PendingNftOrder(
                    msg.sender,
                    _nftId,
                    _trader,
                    _pairIndex,
    ...
    function openTradeMarketTimeout(uint _order) external notContract notDone{
        ...
        StorageInterface.PendingMarketOrder memory o = storageT.reqID_pendingMarketOrder(_order);
