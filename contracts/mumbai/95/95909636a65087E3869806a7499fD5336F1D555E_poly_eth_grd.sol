...
contract poly_eth_grd is 
    ...
    constructor(address childChainManager) ERC721("Girls, Robots, Dragons", "GRD") {
    ...
    function setMinter(address user, bool state) external onlyRole(CONTRACT_ADMIN) {
        if (state) 
    ...
    function withdrawBatch(uint256[] calldata tokenIds) external {
        ...
        for (uint256 i; i < length; i++) {
            ...
            if (_msgSender() != ownerOf(tokenId))
                revert NotTokenOwner(_msgSender(), tokenId);
            _burn(tokenId);
        }
