...
// SPDX-License-Identifier: MIT
//** */ 1-Pragma (Style Guide)
pragma solidity ^0.8.8;

//** */ 2-Imports (Style Guide)
import "./PriceConverter.sol";
import "hardhat/console.sol";

// 849398 gas spended 828432 805577 748358 738769

//** */ 3-Errors (Style Guide)
// custom errorleri contract dısında tanımladıgımıza dikkat et.
error FundMe__NotOwner(); //ERROR'un basına hangi contracttan cıktıgını FundMe__ seklinde yazıyoruz bu bir convensiondur dah iiy anlamamızı saglar.
error FundMe__MinUsd();
error FundMe__SendFailed();

//** */ 4-Interfaces (Style Guide)
//** */ 5-Libraries (Style Guide)

//** */ 6-Contracts (Style Guide)

// Bu sekide contract hakkında bilgi verebilirsin buna natspec denir. bu natspecleri aynı zamanda solc'ile fs'e yazdıdıgmız documentasyona cevirebiliriz.
/**
 * @title "Some cool contract"
 * @author "Some cool author"
 * @notice "This is a demo contract"
 * @dev a note to developers
 */
contract FundMe {
    //** */ 1-Type declorations (Inside Contract Style Guide)
    ...
    //** */ 2-State variables (Inside Contract Style Guide)
    ...
    address private immutable i_owner; // contract owner. witraw fonksiyonunu sadece owner cagırabilir.

    // storage degerlerinin basına s_ koyduk. Storagelar cok gas harcarlar kullanırken dikkat et diye. constatlar ve immutalleler storagede yer kaplamadıkları icin storage degildirler.
    address[] private s_funders; // contract'a para gönderen hesapların array'ini tutuyrouz.
    mapping(address => uint256) public s_addressToAmountFunded; // her funder'ın ne kadar gönderdigini tutuyoruz.

    // chain'e göre degisebilecek bir price feed objesi olusturduk.
    address private s_priceFeed; //*her chain degistiginde sol dosyasına gelip manuel olarak address degistirmek yerine her parametre olarak yazmak yeterli.

    //** */ 3-Events (Inside Contract Style Guide)

    //** */ 4-Modifiers (Inside Contract Style Guide)

    modifier restricted() {
        //require(msg.sender == i_owner,"Only owner can call withraw");
        if (msg.sender != i_owner) {
            revert FundMe__NotOwner(); //require yerene kendi error'umuzu throw ediyoruz. Bu yöntem data gas efficent. Yani gas save etmemizi saglar.
            //revert() bir işlemi yarıda keser. require() gibi ancak icine bir condition almıyor cagırılması yeterli.
        }
        _;
    }

    //** */ 5-Functions (Inside Contract Style Guide)
    ...
    constructor(address _priceFeed) {
        ...
        s_priceFeed = _priceFeed; // price feed'i dısarıdan aldıgımız contractın adresine eşitliyoruz. Böylece network degisinde yeni adresi contructor yazmak yeterli olacak.
    }
    ...
    function fund() public payable {
        ...
        s_addressToAmountFunded[msg.sender] += msg.value; // COK ONEMLI += dedik çünkü birden fazla contribute yaparsa önceki miktarına eklesin istiyoruz. Bunu yapmazsan öncekini cotributionu sıfırlar sonuncuyu alır.
    }

    // Manager'ın fundlanan parayı cekmesini saglar.
    function withraw() public payable restricted {
        // storage degerleri her okundugunda yada yazıldıgında cok gas harcarlar bizde for loop'unda defalarca okuma yapıyoruz.
        // defalarca storage'dan okuma yapmak yerine storage degeri olan s_funders'ı memory'e kaydediyoruz ve bundan sonraki okumalarımızı storage'dan degil memoryden yapıyoruz.
        // eger s_funders bir mapping olsaydı bunu yapamazdık çünkü mappingsler memory'de olamazlar.
        address[] memory funders = s_funders;
        for (uint i = 0; i < funders.length; i++) {
            // console.log(funders[i], 66); //harhat sayesinde yaptıgımız bir console.log bunları consolda görebilirsin.
            // 1-) Resetting mapping value
            s_addressToAmountFunded[funders[i]] = 0; //s_addressToAmountFunded'ı memory'e cevirmedik cunku mappingler memoryde olamazlar.
            // 2-) Resetting array (memory array'i degil storage array'i sıfırlıyoruz)
            s_funders = new address[](0); //new ile bos bir array construct ediyor ve funders'a atıyoruz. new ile construct ettiklerinde lengt () icine gelir.
            ...
            (bool callSuccess, bytes memory dataReturned) = payable(i_owner)
                .call{value: address(this).balance}(""); //("") icine baska fonsiyonlar alabilecegi bir alan.
            ...
            if (!callSuccess) {
                revert FundMe__SendFailed();
            }
            // Durumdan duruma degissede genelde önerilen native token transfer yöntemi call()'dır.
        }
    }

    //** */ 6- View/Pure functions getterlar view fonsiyonlardır (Inside Contract Style Guide)
    //s_ ler bize yardımcı olur ancak contractla iletisime gecen userların s_ ları görmesi gerekmez onlara getterlarla daha güzel bir arayüz saglıyoruz.
