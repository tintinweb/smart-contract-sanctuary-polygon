    ...
    TunnelEnabled
{
    ...
    uint256 public baseGenomeChangePriceEth;
    uint256 public randomizeGenomePriceEth;

    AggregatorV3Interface private maticEthPriceFeed;
    address public baseDataFeedAddress;
    address public quoteDataFeedAddress;
    uint8 public priceDataDecimals;

    event BaseGenomeChangeEthPriceChanged(uint256 newGenomePriceChangeEth);
    event RandomizeGenomeEthPriceChanged(uint256 newRandomizeGenomePriceChangeEth);
    event BaseDataFeedAddressChanged(address newBaseDataFeedAddress);
    event QuoteDataFeedAddressChanged(address newQuoteDataFeedAddress);
    event PriceDataDecimalsChanged(uint8 newPriceDataDecimals);
    ...
    constructor(
        ...
        uint256 _maxSupply,
        uint256 _baseGenomeChangePriceEth,
        uint256 _randomizeGenomePriceEth,
        string memory _arweaveAssetsJSON,
        address _baseDataFeedAddress,
        address _quoteDataFeedAddress,
        uint8 _priceDataDecimals
    )
        SubjectWithGeneChanger(
            name,
            symbol,
        ...
        )
    {
        ...
        baseGenomeChangePriceEth = _baseGenomeChangePriceEth;
        randomizeGenomePriceEth = _randomizeGenomePriceEth;
        baseDataFeedAddress = _baseDataFeedAddress;
        quoteDataFeedAddress = _quoteDataFeedAddress;
        priceDataDecimals = _priceDataDecimals;
        arweaveAssetsJSON = _arweaveAssetsJSON;
        geneGenerator.random();
    }

    function getRandomizeGenomePrice()
        public
        view
        virtual
        override
        returns (uint256)
    {
        return ethToMatic(randomizeGenomePriceEth);
    }

    function getBaseGenomeChangePrice()
        public
        view
        virtual
        override
        returns (uint256)
    {
        return ethToMatic(baseGenomeChangePriceEth);
    }

    function ethToMatic(uint256 _ethAmount)
        private
        view
        returns (uint256 ethPrice)
    {
        int256 rate = getDerivedPrice();
        return ethPrice = (_ethAmount * 10**priceDataDecimals) / uint256(rate);
    }
    ...
    function getDerivedPrice()
        ...
        returns (int256)
    {
        require(priceDataDecimals > uint8(0) && priceDataDecimals <= uint8(18), "Invalid _decimals");
        int256 decimals = int256(10 ** uint256(priceDataDecimals));
        ( , int256 basePrice, , , ) = AggregatorV3Interface(baseDataFeedAddress).latestRoundData();
        uint8 baseDecimals = AggregatorV3Interface(baseDataFeedAddress).decimals();
        basePrice = scalePrice(basePrice, baseDecimals, priceDataDecimals);

        ( , int256 quotePrice, , , ) = AggregatorV3Interface(quoteDataFeedAddress).latestRoundData();
        uint8 quoteDecimals = AggregatorV3Interface(quoteDataFeedAddress).decimals();
        quotePrice = scalePrice(quotePrice, quoteDecimals, priceDataDecimals);
    ...
    function randomizeGenome(uint256 tokenId) public override payable {
        _randomizeGenome(tokenId, getRandomizeGenomePrice());
    }
    ...
    function changeBaseGenomeChangePriceEth(uint256 newGenomeChangePriceEth)
        public
        virtual
        override
        onlyDAO
    {
        baseGenomeChangePriceEth = newGenomeChangePriceEth;

        emit BaseGenomeChangeEthPriceChanged(newGenomeChangePriceEth);
    }

    function changeRandomizeGenomePriceEth(uint256 newRandomizeGenomePriceEth)
        public
        virtual
        override
        onlyDAO
    {
        randomizeGenomePriceEth = newRandomizeGenomePriceEth;

        emit RandomizeGenomeEthPriceChanged(newRandomizeGenomePriceEth);
    }

    function setBaseDataFeedAddress(address _baseDataFeedAddress)
        public
        virtual
        override
        onlyDAO
        isValidAddress(_baseDataFeedAddress)
    {
        baseDataFeedAddress = _baseDataFeedAddress;

        emit BaseDataFeedAddressChanged(_baseDataFeedAddress);
    }

    function setQuoteDataFeedAddress(address _quoteDataFeedAddress)
        public
        virtual
        override
        onlyDAO
        isValidAddress(_quoteDataFeedAddress)
    {
        quoteDataFeedAddress = _quoteDataFeedAddress;

        emit QuoteDataFeedAddressChanged(_quoteDataFeedAddress);
    }

    function setPriceDataDecimals(uint8 _priceDataDecimals)
        public
        virtual
        override
        onlyDAO
    {
        ...
        emit PriceDataDecimalsChanged(_priceDataDecimals);
    }
        ...
        returns (uint256 price)
    {
        ...
        return getBaseGenomeChangePrice().mul(1 << pastChanges);
    }
...
interface ISubjectChild is IERC721 {
    ...
    function getRandomizeGenomePrice()
        external
        view
        returns (uint256 randomizeGenomePrice);

    function getBaseGenomeChangePrice()
        external
        view
        returns (uint256 baseGenomeChangePrice);
    ...
    function changeBaseGenomeChangePriceEth(uint256 newGenomeChangePriceEth) external;

    function changeRandomizeGenomePriceEth(uint256 newRandomizeGenomePriceEth) external;

    function setBaseDataFeedAddress(address baseDataFeedAddress) external;

    function setQuoteDataFeedAddress(address quoteDataFeedAddress) external;

    function setPriceDataDecimals(uint8 priceDataDecimals) external;
