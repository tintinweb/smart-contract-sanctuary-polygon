...
contract ARC_Presale is Ownable, ReentrancyGuard{
    using SafeMath for uint256;
    

    uint256 public ARCpricePerUSD = 12000000000000000000000; // 12000 ARC per USD
    uint256 public ARC_Sold;
    uint256 public maxTokeninPresale= 2333000000*(1E18);
    uint256 public USDpricePerMatic= 2000000000000000000000; // 2000 USD per Matic
    ...
    function BuyARCWithMatic() public payable nonReentrant
    {
        require(ARC_Sold.add(getARCvalueperUSD(msg.value))<=maxTokeninPresale,"Hardcap Reached!");
        ...
        ARC_Sold =ARC_Sold.add(getARCvalueperUSD(msg.value)); 
    }


    function getARCvalueperUSD(uint256 value) public view returns(uint256)
    {
        uint256 usdprice=(USDpricePerMatic.mul(value)).div(1e18);
        return (ARCpricePerUSD.mul(usdprice)).div(1e18);
    }

    

    function setRewardARCPriceperUSD(uint256 _count) external onlyOwner {
        ARCpricePerUSD = _count;
    }

     function setUSDpricePerMatic(uint256 _count) external onlyOwner {
        USDpricePerMatic = _count;
    }
