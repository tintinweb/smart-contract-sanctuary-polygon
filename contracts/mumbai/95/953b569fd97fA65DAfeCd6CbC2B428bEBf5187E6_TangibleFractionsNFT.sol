    ...
    ITangibleFractionsNFT
{
    ...
    function claim(uint256 fractionId, uint256 amount) external {
        ...
        _claimTngbl(sender, fractionId, amount);
    }
        ...
        returns (uint256[] memory fractTokenIds)
    {
        ...
        hs.revenueShare_ = revenueShare.burnShare(
            _updateRevenueShare,
            address(this),
            fractionTokenId
        );
        hs.rentShare_ = rentShare.burnShare(
            _updateRentShare,
            address(this),
            fractionTokenId
        );
        ...
        for (uint256 i; i < length; i++) {
            ...
            uint256 newRevenueShare = (hs.revenueShare_ * shares[i]) /
                fullShare;
            uint256 newRentShare = (hs.rentShare_ * shares[i]) / fullShare;
            if (i != 0) {
                //mint others
