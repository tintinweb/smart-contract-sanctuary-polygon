    ...
    IDelegator
{
    ...
    function getCollateralRatioModel(address asset) external returns (address model) {
        ...
        emit GetCollateralRatioModel(model);
    }
    ...
    function getCurrentMaxLtvRatio(address asset) external returns (uint256 ratio) {
        ...
        emit GetCurrentMaxLtvRatio(ratio);
    }
    ...
    fallback() external {
        ...
        (bool success, bytes memory data) = delegateeAddress.delegatecall(msg.data);

        _safeRevert(success, data);
    }
}
