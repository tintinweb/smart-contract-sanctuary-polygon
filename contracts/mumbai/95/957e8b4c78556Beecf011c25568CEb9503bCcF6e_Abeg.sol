...
contract Abeg {
    ...
    address internal constant zeroAddress = 0x0000000000000000000000000000000000000000;
    ...
    modifier giveawayHasEnded(uint256 giveawayId) {
        require(giveaways[giveawayId].endAt >= block.timestamp, "Giveaway has not ended");
        _;
    }

    event GiveawayCreated(uint256 indexed giveaway_id, address indexed creator);
    event GiveawayParticipantAdded(uint256 indexed giveaway_id, address indexed participant);
    event GiveawayWinnersSelected(uint256 indexed giveaway_id, address[] indexed winners);
    ...
    constructor(
        ...
        require(
            _supportedERC20Tokens.length >= 1 && _supportedERC20Tokens.length <= tokensLimit,
        ...
        require(_randomNumSeed > 0 && _randomNumSeed < _randomNumModulo, "Seed number must be less than modulo");
        require(
    ...
    function createGiveaway(
        ...
        require(participantsCount > 0 && winnersCount > 0, "participants and winners can never be 0");
        require(participantsCount > winnersCount, "participants must always be greater than winners");

        // This is to limit gas during withdrawal
        require(winnersCount <= 10, "Giveaway can only have a max of 10 winners");

        require(endAt > block.timestamp, "giveaway cannot end before starting");
        require(endAt > block.timestamp + 15 minutes, "giveaway must have a duration of at least 15 minutes");
        require(bytes(_name).length != 0, "giveaway name cannot be empty");

        if (_tokenAddr == zeroAddress) {
            require(msg.value == _amount, "ether sent must be equal to the amount specified");
    ...
    function participateInGiveaway(uint256 giveawayId) external giveawayExists(giveawayId) returns (bool) {
        GiveawayDetail memory giveawayInfo = giveaways[giveawayId];

        require(giveawayInfo.creator != msg.sender, "Giveaway creator cannot participate");
        require(giveawayInfo.endAt < block.timestamp, "Giveaway has ended");
        // check if address is already a participants
        require(giveawaysParticipantsMapping[giveawayId][msg.sender] == true, "Already a participant for the giveaway");
        require(
            giveawaysParticipantsArray[giveawayId].length < giveawayInfo.maximumParticipants,
    ...
    function withdrawGiveawayPrize(
        ...
        require(giveawayWinners[giveawayId][msg.sender].isValid, "Not a winner");
        require(giveawayWinners[giveawayId][msg.sender].hasWithdrawnFunds, "A withdrawal has been issued alredy");
        ...
        if (giveawayInfo.tokenAddress == zeroAddress) {
            ...
            require(paymentToken.transfer(msg.sender, prizeMoney), "Failed to send prize money");
            giveawayWinners[giveawayId][msg.sender].hasWithdrawnFunds = true;
        }
        emit GiveawayWinnerPaid(giveawayId, msg.sender);
    }

    function pickWinners(uint256 giveawayId) internal giveawayExists(giveawayId) giveawayHasEnded(giveawayId) {
        GiveawayDetail memory giveawayInfo = giveaways[giveawayId];
        ...
        if (participants.length <= giveawayInfo.numberOfWinners) {
            for (uint256 i = 0; i < participants.length; i++) {
                ...
                giveawayWinners[giveawayId][winner] = GiveawayWinner({hasWithdrawnFunds: false, isValid: true});
            }
        } else {
            uint256[] memory randomNumbers = chooseRandomNumbers(giveawayInfo.numberOfWinners, participants.length);
        ...
        emit GiveawayWinnersSelected(giveawayId, giveawayWinnersArray[giveawayId]);
    }
    ...
    function pickGiveawayWinners(
        ...
        require(giveawayInfo.hasWinners != true, "Winners have selected already");
        pickWinners(giveawayId);
    }

    function addERC20Token(address _newSupportedToken) external isContractOwner(msg.sender) {
        require(_newSupportedToken != zeroAddress, "Invalid address provided");
        require(supportedERC20Tokens.length <= tokensLimit, "Only a maximum of ten ERC20 tokens are allowed");
    ...
    function getMyGiveaways(address _address) external view returns (uint256[] memory) {
    ...
    function chooseRandomNumbers(uint256 numsToGenerate, uint256 maxNumToChoose) internal returns (uint256[] memory) {
        uint256[] memory randNums = new uint256[](numsToGenerate);
        for (uint256 i = 0; i < numsToGenerate; i++) {
            uint256 randNum = (randomNumMultiplier * randomNumSeed + randomNumIncrement) % randomNumModulo;
