    ...
    UUPSUpgradeable
{
    ...
    function mintLand(
        ...
        require(x.length == y.length && y.length == uri.length, 'LWNFT: Array lengths mismatch');

        for (uint256 i = 0; i < uri.length; i++) {
            uint256 landId = _landId.current();
            lostLands[landId].exists = true;
            lostLands[landId].worldId = worldId;
            lostLands[landId].x = x[i];
            lostLands[landId].y = y[i];
            lostWorlds[worldId].landIds.push(landId);

            _safeMint(to, landId);
            _setTokenURI(landId, uri[i]);
            _landId.increment();
        }
    }

    function getLatestLand() public view returns (uint256) {
