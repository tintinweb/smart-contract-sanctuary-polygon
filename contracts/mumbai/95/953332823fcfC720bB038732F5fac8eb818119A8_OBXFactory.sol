...
contract OBXExchange is Ownable {
    ...
    address public KRSTMToken = 0xe11A86849d99F524cAC3E7A0Ec1241828e332C62;
    ...
    function distributeFees() external returns (bool) {
        if(tokenA == KRSTMToken){
            ...
            IERC20(tokenA).transfer(deployer, tokenAaccumulatedFee/2);
            IERC20(tokenA).transfer(synteticPool, tokenAaccumulatedFee/2);

        } else{
            IERC20(tokenA).transfer(deployer, tokenAaccumulatedFee);
        }

        if(tokenB == 0xfe4F5145f6e09952a5ba9e956ED0C25e3Fa4c7F1){
