...
contract KreatorhoodERC721 is ERC721Metadata, ERC721MetaTx, AccessControl, IERC6785, IERC2981 {
    ...
    function getUtilityUri(uint256 tokenId) external view override returns (string memory) {
        if (!_exists(tokenId)) revert NonExistentToken();
        uint256 last = utilities[tokenId].length - 1;
        return utilities[tokenId][last];
    }

    function getUtilityHistory(uint256 tokenId) external view override returns (string[] memory) {
        if (!_exists(tokenId)) revert NonExistentToken();
