...
contract ERC7254 is ERC20 {
     ...
     */
    function informationOf(address account) public view virtual returns (UserInformation memory) {
     ...
     */
    function tokenReward() public view virtual returns (address) {
        return _tokenReward;
    }
     ...
     */
    function updateReward(uint256 amount) public virtual returns(uint256) {
        address owner = _msgSender();
        if(totalSupply() != 0){
            IERC20(_tokenReward).transferFrom(owner, address(this), amount);
     ...
     */
    function viewReward(address account) public view virtual returns (uint256){
        UserInformation storage user = userInformation[account];
        uint256 reward = balanceOf(account) * rewardPerShare + user.inReward - user.withdraw - user.outReward;
        return reward / 1e18;
    }
    ...
    function getReward() public virtual returns(uint256){
        ...
        if(reward / 1e18 > 0){
            IERC20(_tokenReward).transfer(owner, reward / 1e18);
        }  
...
contract Mytoken is ERC7254  {
    constructor(string memory name, string memory symbol, address _tokenreward) ERC7254(name, symbol, _tokenreward) {
