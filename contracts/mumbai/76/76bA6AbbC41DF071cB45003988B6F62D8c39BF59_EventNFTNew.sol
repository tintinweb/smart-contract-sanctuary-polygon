...
contract EventNFTNew is Ownable, ERC721URIStorage {
    ...
    // }
		...
		event Airdrop(uint dayIndex, string[] ticketNameToBuy, uint[] ticketAmountToBuy, uint saleIndex);
    ...
    constructor(EventData memory eventData, string memory baseUri_) ERC721(eventData.eventName, eventData.eventLogo) {
        ...
        for (uint i = 0; i < eventData.dateEvents.length; i++) {
            ...
            for (uint j = 0; j < eventData.ticketDatas.length; j++) {
                ...
                newTokenId = (totalAmount - eventData.ticketDatas[j].amount) + 1;
                } else {
    ...
    function airdrop(uint dayIndex, string[] memory ticketNameToBuy, uint[] memory ticketAmountToBuy, uint saleIndex) public {
        ...
        require(block.timestamp < dailyEventData.dateEvent, "Event is Already Started");
				require(block.timestamp > dailyEventData.sales[saleIndex].date, string(abi.encodePacked(dailyEventData.sales[saleIndex].name, ": Ticket Sale is Not Opened")));
				if (saleIndex + 1 < dailyEventData.sales.length) {
						require(block.timestamp < dailyEventData.sales[saleIndex + 1].date, string(abi.encodePacked(dailyEventData.sales[saleIndex].name, ": This Type of Sale is Closed")));
				}
        ...
        require(ticketMaxPerWallet >= ticketTotal && userTicketAmount[msg.sender] <= ticketMaxPerWallet, string(abi.encodePacked("You Have Exceeded Max Amount of Ticket")));
        
        for (uint i = 0; i < ticketNameToBuy.length; i++) {
						require(ticketNameExist[ticketNameToBuy[i]], string(abi.encodePacked(ticketNameToBuy[i], ": This Ticket is Not Exist")));
            ...
            dailyEventData.ticketMinted += ticketAmountToBuy[i];
				...
				emit Airdrop(dayIndex, ticketNameToBuy, ticketAmountToBuy, saleIndex);
    }

    function _beforeTokenTransfer(address from, address to, uint tokenId, uint batchSize) internal view override {
        require(from == address(0) || ticketTransferable == true, "Ticket is Not Transferable");
    }
    ...
    function getContractDetail() public view returns (bool, uint, uint, string memory) {
        return (
            ticketTransferable,
            ticketMaxPerWallet,
            allTicketMinted,
    ...
    function getAllEventDetails() public view returns (Sale[] memory) {
        return (
					// dailyEvents.length,
					// tickets,
					sales
		...
		function getTicketDetails() public view returns (Ticket[] memory) {
			DailyEvent storage dailyEventData = dailyEvents[0];
			return dailyEventData.tickets;
		}

		function getSaleDetails() public view returns (Sale[] memory) {
			DailyEvent storage dailyEventData = dailyEvents[0];
			return dailyEventData.sales;
		}
