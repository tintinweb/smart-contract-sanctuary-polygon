...
contract PlutoV2forge is Ownable, Implementor {
    ...
    // REMOVE ON MAINNET onlyOwner
    function addRewardForAddressByAdmin(address _user, uint8[2] memory _arrayOfRewards)
        public
    {
        for (uint16 i = 0; i < 2; i++) {
            claimableRewards[_user][_arrayOfRewards[i]] += 1;
    ...
    function removeRewardForAddressByAdmin(address _user, uint8 _traitInput) public onlyOwner{
        claimableRewards[_user][_traitInput] -= 1;
    }
    ...
    function getHash(
        ...
        return keccak256(abi.encodePacked(md5hash, _traitInput,rowID, timeLimit));
    }
