...
// PRICE PERBACTH
    int256  public PriceWL = 80*10**15;
...
//ROLES
    bytes32 public constant PAUSEROLE = keccak256("PAUSEROLE");
    ...
    event ClaimUserFailIndex(address account, uint256[] indexed orders, uint256 value);
...
//Constructor, cria interface token, seta roles
    ...
    _grantRole(PAUSEROLE, msg.sender);
    ...
    function pause() public onlyRole(PAUSEROLE) {
        _pause();
    }

    function unpause() public onlyRole(PAUSEROLE) {
        _unpause();
    }

    function pauseWithdraw() public onlyRole(PAUSEROLE){
        isPauseWithdraw = true;
    }
    function pauseClaim() public onlyRole(PAUSEROLE){
        isPauseClaim = true;
    }

    modifier notPauseWithdraw(){
        require(isPauseWithdraw == false, "Withdraw is Pause");
        _;
    }

    modifier notPauseClaim(){
        require(isPauseClaim == false, "Claim is Pause");
        _;
    }
...
// MWP per MATIC dependendo do bacth
    ...
    //Value in Dolar for 1 Matic
        ...
        int256 dolWei = dol *10**18;
        if(batch == WL){
            return dolWei/PriceWL;
        }
        else if(batch == BATCH2){
            return dolWei/PriceBatch2;
        }
        else if(batch == BATCH3){
            return dolWei/PriceBatch3;
        }
        else{
            return dolWei/PricePO;
        }
    }

//pricefortokens ammount->matic 
...
//AQUI ELE CALCULA A QUANTIDADE DE TOKENS value->matic
//Escreve Sold per Batch, order,  
    function writeOrder(address _account, uint256 _tokens, bytes32 _method, bytes32 _batch) private{
...
//Verifica o supply da WL ou isP2
...
//Devolve qual bacth esta (nao leva em consideração WL)
    ...
    function BuyTokens() public payable {
        if(WTListOpen() && WhiteList[msg.sender] == true){
            uint _tokens = Tokens(msg.value, WL);
            BuyTokensWL(msg.sender, _tokens, MATIC);
        }
        else{
            bytes32 batch = Batch();
            uint _tokens = Tokens(msg.value, batch);
            if(batch == BATCH2){
                BuyTokensBATCH2(msg.sender, _tokens, MATIC);
            }
            else if(batch == BATCH3){
                BuyTokensBATCH3(msg.sender, _tokens, MATIC);
            }
            else if(batch == PO){
                writeOrder(msg.sender, _tokens, MATIC, PO);
            }
        }
    }

    function BuyTokensBATCH2(address _account, uint256 _tokens, bytes32 _method) private{
        uint remains = maxSupplyBatch2 - totalSoldBatch2;
        if(msg.value > remains){
            uint256 nextBatch = _tokens - remains;
            writeOrder(_account, remains, _method, BATCH2);
            BuyTokensBATCH3(_account, nextBatch, _method);         
        }
        else{
            writeOrder(_account, _tokens, _method, BATCH2);
        }
    }

    function BuyTokensBATCH3(address _account, uint256 _tokens, bytes32 _method) private{
        ...
        if(_tokens > remains){
            ...
            writeOrder(_account, _tokens, _method, BATCH3);
        }

    }
// Tanto para PIX e MATIC
    function BuyTokensWL(address _account, uint256 _tokens, bytes32 _method) private {
        ...
        if(_tokens > remains){
            ...
            writeOrder(_account, _tokens, _method, WL);
        }
    }

//FUNÇÃO CHAMADA PELO BACK
    function BuyPix(address _account, uint256 _tokens) public whenNotPaused onlyRole(BUYPIXROLE){

        if(WTListOpen() && WhiteList[_account]==true){
            BuyTokensWL(_account, _tokens, PIX);
        }
        else{
            bytes32 batch = Batch();
            if(batch == BATCH2){
                BuyTokensBATCH2(_account, _tokens, PIX);
            }
            else if(batch == BATCH3){
                BuyTokensBATCH3(_account, _tokens, PIX);
            }
            else if(batch == PO){
                writeOrder(_account, _tokens, MATIC, PIX);
            }
        }
    }
    ...
    function getTotalTokens(address _account) public view returns (uint256) {
    ...
    function getOrdem(uint _index) public view returns (address,uint256, string memory,string memory){
      ...
      uint256 ammount = Orders[_index].tokens;
    ...
    function claim() public notPauseClaim nonReentrant{
        ...
        if(sent == false){
            emit ClaimUserFailIndex(msg.sender, _orders, amountTokens);
        }
        require(sent, "Failed to transfer token to user");
        emit Claim(msg.sender, amountTokens);
    }
    function withdraw() public notPauseWithdraw onlyRole(WITHDRAWROLE){
...
contract MentoraWellPlayedToken is
    ERC20,
    ERC20Burnable,
    ERC20Permit,
    ERC20Votes,
    ...
    AccessControl
{
    ...
    constructor()
        ERC20("Mentora Well Played", "MWP")
        ERC20Capped(110_000_000_000_000_000_000_000_000)
        ERC20Permit("Mentora Well Played")
    {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
