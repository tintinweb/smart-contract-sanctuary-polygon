...
contract drop2 is Ownable, ERC721A, ReentrancyGuard, DefaultOperatorFilterer  {
    uint256 public MAX_SUPPLY = 500000;
    uint256 public PRICE = 10; // 0.00001 in USDC
    uint256 public PRESALE_PRICE = 10; // 0.00001 in USDC
    uint256 public maxPresale = 3000;
    uint256 public maxPublic = 3000;
    address public USDC = 0xFEca406dA9727A25E71e732F9961F680059eF1F9; //  USDC contract (mumbai)
    ...
    // getters
    function getERC20balance(address tokenAddress, address holder)
        public
        view
        returns (uint256)
    {
        return IERC20(tokenAddress).balanceOf(holder);
    }
        ...
        returns (string memory)
    {
        ...
        if(redeemed[tokenId]){
            return string(abi.encodePacked(baseURI, "redeemed/", Strings.toString(tokenId)));
        }
        else {
        ...
        nonReentrant
    {
        ...
        require(getERC20balance(USDC, msg.sender) >= PRESALE_PRICE * quantity, "Insufficient USDC");
        IERC20(USDC).transferFrom(msg.sender, address(this), PRESALE_PRICE * quantity);
        // check proof & mint
    ...
    function redeem(uint256 tokenId) external nonReentrant {
        ...
        require(redeemed[tokenId] == false, "token has already been redeemed");
        redeemed[tokenId] = true;
    }

    // public mint
    function publicSaleMint(uint8 quantity, address to)
        external
        payable
        nonReentrant
        callerIsUser
    {
        ...
        require(getERC20balance(USDC, msg.sender) >= PRICE * quantity, "Insufficient USDC");
        IERC20(USDC).transferFrom(msg.sender, address(this), PRICE * quantity);
        _safeMint(to, quantity);
    }
