...
contract CandidePaymaster is BasePaymaster {
    ...
    mapping(address => uint256) public gasBackBalances;
    uint256 public gasBackBasePoints = 100;
    //
    ...
    function changeGasBackBasePoints(uint256 _gasBackBasePoints) public {
        require(owner() == msg.sender, "CP00: only owner can change gas back base points");
        gasBackBasePoints = _gasBackBasePoints;
    }
    ...
    internal virtual override returns (bytes memory context, uint256 validationData){
        ...
        require(paymasterData.mode == SponsoringMode.GAS_BACK || paymasterData.signature.length == 64 || paymasterData.signature.length == 65, "CP01: invalid signature length in paymasterAndData");
        address account = userOp.getSender();
        if (paymasterData.mode == SponsoringMode.GAS_BACK){
			uint256 maxOpCost = maxCost + (COST_OF_POST * userOp.maxFeePerGas);
            if (gasBackBalances[account] < maxOpCost){
    ...
    function _postOp(PostOpMode mode, bytes calldata context, uint256 actualGasCost) internal override {
        ...
        if (sponsoringMode == SponsoringMode.FREE){
            emit UserOperationSponsored(account, address(token), 0);
            return;
        }
        ...
        //
        if (sponsoringMode == SponsoringMode.GAS_BACK){
            if (gasBackBalances[account] >= actualETHCost){
                gasBackBalances[account] -= actualETHCost;
            }
            emit UserOperationSponsored(account, address(token), 0);
            return;
        }
        ...
        if (sponsoringMode == SponsoringMode.GAS_AND_FEE){
            uint256 feeInETH = (fee * 1e18) / exchangeRate;
            gasBackBalances[account] += (feeInETH * gasBackBasePoints) / 100;
        }
