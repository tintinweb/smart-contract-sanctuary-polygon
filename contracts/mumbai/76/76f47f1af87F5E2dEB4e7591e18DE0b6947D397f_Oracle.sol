...
contract Oracle is IOracle {
    ...
    address public constant USDT = 0xC41F67f90BD92B4C8a5F892Ef14EEa9cc22231b6; // matic
    address public constant KOL = 0x303f5C384610A1750007C7910F28731d4A9f031c; // matic
    address public constant UNISWAP_V2_FACTORY = 0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32; // matic
    ...
    function setValues(address token) public {
        address pool = factoryInterface.getPair(USDT, token);
        if (pool != address(0)) {
            if (USDT < token) {
                (
    ...
    function fetch(address token) external override returns (uint256 price) {
        if (
            cummulativeAveragePrice[token] == 0 || (uint32(block.timestamp).sub(lastTokenTimestamp[token])) >= 3 minutes
        ) {
            setValues(token);
        }
        uint256 usdtPerKOL = _getAmounts(KOL);
        emit AssetValue(usdtPerKOL, block.timestamp);
        uint256 usdtPerToken = _getAmounts(token);
        emit AssetValue(usdtPerToken, block.timestamp);
        if (usdtPerToken == 0 || usdtPerKOL == 0) return 0;
        price = (usdtPerToken.mul(1e18)).div(usdtPerKOL);
        emit AssetValue(price, block.timestamp);
    }

    // KOLperUSDT
    function fetchKOLPrice() external override returns (uint256 price) {
        if (cummulativeAveragePrice[KOL] == 0 || (uint32(block.timestamp).sub(lastTokenTimestamp[KOL])) >= 3 minutes) {
            setValues(KOL);
        }
        uint32 timeElapsed = uint32((lastTokenTimestamp[KOL]).sub(tokenToTimestampLast[KOL]));
        price = _calculate(cummulativeEthPrice[KOL], cummulativeAveragePriceReserve[KOL], timeElapsed, USDT);
        emit AssetValue(price, block.timestamp);
    }

    function _getAmounts(address token) internal view returns (uint256 usdtPerToken) {
        address poolAddress = factoryInterface.getPair(USDT, token);
        if (poolAddress == address(0)) return 0;
        uint32 timeElapsed = uint32((lastTokenTimestamp[token]).sub(tokenToTimestampLast[token]));
        usdtPerToken = _calculate(
            cummulativeAveragePrice[token],
