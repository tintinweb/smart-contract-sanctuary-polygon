...
interface ILandManager {
    ...
    event LandMinted(uint256 id, uint256 rarity, uint256 floor);
    
    struct LandInfo {
        uint16 rarityType; // 0 = uncommon; 1 = common; 2 = rare; 3 = very rare
        uint16 floor; // 1 - 5
        uint16 level; // 1- 12
        uint16 richness;
    ...
    function updateBalance(uint256 id, uint256 newBalance) external;

    function updateResourceDeposit(uint256 landId) external;

    function addSymbol(uint256 landId, uint256 symbolId) external;

    function removeSymbol(uint256 landId, uint256 symbolId) external;

    function addItem(uint256 id, uint256 itemId) external;

    function removeItem(uint256 landId, uint256 itemId) external;

    function addDestroyedSymbols(uint256 landId, uint256 symbolId) external;

    function addDestroyedItems(uint256 landId, uint256 itemId) external;

    function addRemovedSymbols(uint256 landId, uint256 symbolId) external;
    ...
    IGame
{
    ...
    mapping (uint256 => mapping(uint256 => uint256)) public rarities; // level => loại symbol => rarity 4 loại symbols
    mapping (uint256 => RequestResult) public landRequests; // requestId => result
    ...
    function roll(uint256 landId) external nonReentrant whenNotPaused {
        ...
        landRequests[requestId].landId = landId;
    }
    ...
    // chọn symbol or deposit symbol or withdraw symbol; chọn items
    function choose() external nonReentrant whenNotPaused {

    }

    function depositSymbol(uint256 landId, uint256 resourceId) internal nonReentrant whenNotPaused {
        require(landId != 0 && lands[msg.sender].contains(landId), "caller is not owner");
        require(manager.landInfos(landId).resourceDeposit < manager.getLimit(landId).resourceDepositLimit, "exceed limit");
        address account = msg.sender;
        manager.updateResourceDeposit(landId);
        manager.addSymbol(landId, resourceId);
        resource.burn(account, resourceId, 1);
    }
    ...
    function withdrawToken(uint256 landId, address to, uint256 amount) external nonReentrant whenNotPaused {
        ...
        manager.updateBalance(landId, balance - amount);
    ...
    function setRarity(uint256 level, uint256[4] memory rate) external onlyOwner {
