...
contract PlushGetTree is Initializable, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function addTreeType(string memory _type, uint256 _price, uint256 _count) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!treeMap[_type].isValid, "This type of tree already exists");

        treeMap[_type] = Tree(true, _type, _price, _count);
    }

    function removeTreeType(string memory _type) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setTreeTypePrice(string memory _type, uint256 _price) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(isActive);
        require(treeMap[_type].isValid, "Not a valid tree type.");
        require(_price > 0);
    ...
    function setTreeTypeCount(string memory _type, uint256 _count) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(isActive);
        require(treeMap[_type].isValid, "Not a valid tree type.");
    ...
    function changeContractStatus() public onlyRole(DEFAULT_ADMIN_ROLE) {
        isActive = !isActive;
    }
}
