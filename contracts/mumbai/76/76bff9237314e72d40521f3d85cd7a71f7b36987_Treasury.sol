...
contract Treasury is ContractGuard {
    ...
    modifier checkEpoch {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getPegPrice() > XONEPriceCeiling) ? 0 : getPegCirculatingSupply().mul(maxSupplyContractionPercent).div(10000);
    }
    ...
    function getPegUpdatedPrice() public view returns (uint256 _XONEPrice) {
    ...
    function getBurnablePegLeft() public view returns (uint256 _burnablePegLeft) {
        uint256 _XONEPrice = getPegPrice();
        if (_XONEPrice <= XONEPriceOne) {
            uint256 _XONESupply = getPegCirculatingSupply();
            ...
            if (_bondMaxSupply > _bondSupply) {
                ...
                uint256 _maxBurnablePeg = _maxMintableBond.mul(_XONEPrice).div(1e18);
    ...
    function getRedeemableBonds() public view returns (uint256 _redeemableBonds) {
        ...
        if (_XONEPrice > XONEPriceCeiling) {
            uint256 _totalPeg = IERC20(XONE).balanceOf(address(this));
    ...
    function getBondDiscountRate() public view returns (uint256 _rate) {
        ...
        if (_XONEPrice <= XONEPriceOne) {
            if (discountPercent == 0) {
                ...
                uint256 _bondAmount = XONEPriceOne.mul(1e18).div(_XONEPrice); // to burn 1 PEG
    ...
    function initialize(
        ...
        maxSupplyExpansionPercent = 420; // Upto 4.2% supply for expansion
    ...
    function setPegPriceCeiling(uint256 _XONEPriceCeiling) external onlyOperator {
        require(_XONEPriceCeiling >= XONEPriceOne && _XONEPriceCeiling <= XONEPriceOne.mul(120).div(100), "out of range"); // [$1.0, $1.2]
    ...
    function setExtraFunds(
        ...
        address _devFund,
        uint256 _devFundSharedPercent) external onlyOperator 
     {
    ...
    function setPremiumThreshold(uint256 _premiumThreshold) external onlyOperator {
        require(_premiumThreshold >= XONEPriceCeiling, "_premiumThreshold exceeds pegPriceCeiling");
    ...
    function buyBonds(uint256 _XONEAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        emit BoughtBonds(msg.sender, _XONEAmount, _bondAmount);
    }
    ...
    function redeemBonds(uint256 _bondAmount, uint256 targetPrice) external onlyOneBlock checkCondition checkOperator {
        ...
        require(pegPrice == targetPrice, "Treasury: Peg price moved");
        require(
            pegPrice > XONEPriceCeiling, // price > $1.01
            "Treasury: pegPrice not eligible for bond purchase"
        );
        ...
        emit RedeemedBonds(msg.sender, _XONEAmount, _bondAmount);
    }

////
// Treasury mints the supply of XONE 
    ...
    function _sendToMasonry(uint256 _amount) internal {
        ...
        if (daoFundSharedPercent > 0) {
            ...
            IERC20(XONE).transfer(daoFund, _daoFundSharedAmount);
        ...
        IERC20(XONE).safeApprove(masonry, 0);
        IERC20(XONE).safeApprove(masonry, _amount);
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        uint256 pegSupply = getPegCirculatingSupply().sub(seigniorageSaved); //This interacts with _sendToMasonry to mint XONE
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _sendToMasonry(pegSupply.mul(bootstrapSupplyExpansionPercent).div(10000));
        } else 
            { if (previousEpochPegPrice > XONEPriceCeiling) {
                // Expansion ($Peg Price > 1 $Matic): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bond).totalSupply();
                uint256 _percentage = previousEpochPegPrice.sub(XONEPriceOne);
    ...
    function governanceRecoverUnsupported(
        ...
        require(address(_token) != address(xfactory), "xfactory");
