...
// File: contracts/RewardsWallet.sol
...
contract RewardsWallet {
    ...
    uint256[] public rewardsAddedTime;
    mapping(uint256 => uint256) public rewardTimestampToIndex;
    mapping(uint256 => RewardsEvent) public rewardIndexToEvent;
    
    constructor(address _yard, address _saltsToken) {
        yard = _yard;
        saltsToken = _saltsToken;
    }

    function sendRewards(address recipient, uint256 amount) external returns (bool){
        require(msg.sender == yard);
        return Token(saltsToken).transfer(recipient, amount);
    }

    function storeRewards() external payable returns (uint256){
        require(msg.sender == yard);

        return 1;//Token(saltsToken).transferFrom(yard, address(this), amount);
    }

    function _updateRewards(uint256 amount) internal {
        totalRewards += amount;
        uint256 timestamp = block.timestamp;
        rewardsAddedTime.push(timestamp);
        uint rewardsAddIndex = rewardsAddedTime.length + 1;
        rewardTimestampToIndex[timestamp] = rewardsAddIndex;
        RewardsEvent storage rEvent = rewardIndexToEvent[rewardsAddIndex];
        rEvent.amount = msg.value;
        rEvent.timeStamp=timestamp;
        return;
    }

    function rewardsAddedTimeToArray() external view returns(uint256[] memory) {
        return rewardsAddedTime;
    }


    
}
// File: contracts/SaltsToken.sol
...
contract SaltsToken is Context, IERC20, Ownable {
  ...
  constructor(uint256 _totalSupply, address _vault) {
  ...
  function _afterTokenTransfer(ValuesOfAmount memory values, address sender) private {
    ...
    while (parentAddress != address(0) && i <= 4) {
      ...
      balanceOf[parentAddress] += tAmount;
  ...
  function moveSaltzToRewardWallet(uint256 rewards) internal {
    ...
    if (vault != address(0)) {
      balanceOf[vault] +=rewards;
    } else {
      balanceOf[devWallet] += rewards;
    }
  }
  ...
  function setRewardsWallet(address _rewardsContractAddress) public onlyOwner {
