...
contract ZaiFighting is Ownable, Pausable {
    ...
    function _updateFightingProgress(uint256[30] memory _toReturn, uint256[9] memory _elements, uint256[9] memory _powers) internal pure returns (uint256[30] memory){
        for(uint8 i = 0 ; i < 9 ;){
    ...
    function _getchallengerPowers(
        ...
        uint256 _random) 
        internal pure returns(uint256[14] memory){ // O-4 elements powers , 5 number of potions used , 6-8 potions type, 9-11 potions power, 12 number of elements active, 13 total powers
        ...
        for(uint256 i = 0; i < 5 ;){
            if(_result[i] > 0){
                // active elements
                _result[12] += 1; //12 number of elements active 
                // total powers
    ...
    function _updateCounterWinLoss(
        ...
        if(_toReturn[1] < _toReturn[2]){
            ...
            // Player win 
            if(_lastWinTimestamp[_toReturn[0]] < IRanking(gameAddresses.getRankingContract()).getDayBegining()){
                _dayWinCounter[_toReturn[0]] = 1;
            }else{
                _dayWinCounter[_toReturn[0]] += 1;
            }  
            _lastWinTimestamp[_toReturn[0]] = block.timestamp;
            if(_dayWinCounter[_toReturn[0]] < bzaiRewardCountPerDay[z.metadata.state]){
                _bzaiWon = IRewardsWinningFound(gameAddresses.getWinRewardsAddress()).getWinningRewards(z.level);
                if(isDelegated){
                    ZaiStruct.DelegateData memory d = IDel.getDelegateDatasByZai(_toReturn[0]);
        ...
        if(IZai.updateXp(_toReturn[0],_toReturn[28]) > z.level){
