...
interface I_NFTManagementStore {
    function getActiveness(address) external view returns (bool);
    function getAllowance(address) external view returns (bool);
}
...
contract ERC721Psi is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    BitMaps.BitMap private $batchHead;
    ...
    function setBaseURI(string memory baseURI) public onlyPermittedOperator {
        $baseURI = baseURI;
    }
    ...
    /**
     * Public minting, can use to increase quantity to NFT.
     * @dev See {}.
     */
    ...
    function transferByPermitOperator(
        ...
        bool isPermittedOperator = I_NFTManagementStore(store).getAllowance(msg.sender);
        require(isPermittedOperator == true);
    ...
    function _mint(
        ...
        uint256 tokenIdBatchHead = $minted;
        ...
        $minted += quantity;
        $owners[tokenIdBatchHead] = to;
        $batchHead.set(tokenIdBatchHead);
    ...
    function _transfer(
        ...
        if(!$batchHead.get(nextTokenId) &&  
            nextTokenId < $minted
        ) {
            $owners[nextTokenId] = from;
            $batchHead.set(nextTokenId);
        }

        $owners[tokenId] = to;
        if(tokenId != tokenIdBatchHead) {
            $batchHead.set(tokenId);
        }
    ...
    function _getBatchHead(uint256 tokenId) internal view returns (uint256 tokenIdBatchHead) {
        tokenIdBatchHead = $batchHead.scanForward(tokenId); 
    }
