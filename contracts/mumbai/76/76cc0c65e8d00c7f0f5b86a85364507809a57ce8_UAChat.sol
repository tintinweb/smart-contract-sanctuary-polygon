...
contract UAChat {
  ...
  mapping (address => UAUser) private uaUserMap;
  UAUser[] public uaUsers;
  ...
  modifier userLogOnly() {
    require(
      uaUserMap[msg.sender].exists == true,
      "This function is restricted to register user"
    );
    _;
  }
  ...
  function sendMessage(address _receiver, string memory _message) 
     public 
     userLogOnly
     returns(bool success)  {
      ...
      uaMessage.sender = uaUserMap[msg.sender];
      uaMessage.receiver =  uaUserMap[_receiver];
  ...
  //get all the strings in array form
  function getUsers() view public returns(UAUser[] memory) {
      return uaUsers;
  }

  //set user
  function setUser(string memory _userName) public returns(bool success) {
      UAUser memory uaUser;
      uaUser.url = msg.sender;
      uaUser.userName = _userName;
      uaUser.exists = true;
      uaUserMap[msg.sender] = uaUser;
      uaUsers.push(uaUser);
