...
contract MummyClub is ERC721, Ownable, ReentrancyGuard {
    using SafeMath for uint256;

    uint256 public constant MAX_MMC_PURCHASE = 5; // max purchase per txn
    uint256 public constant MAX_MMC = 10000; // max of 10,000
    ...
    uint256 public mmcPrice = 400 ether; // 400 FTM
    uint256 public mmcPower = 10000; // 10000 power
    uint256 public esMMYBonus = 50e18; // 10000 power
    uint256 public totalVolume;
    uint256 public totalPower;

    uint256 public stepEsMMY = 9800; // 1.02
    uint256 public stepPrice = 10200; // 1.02
    uint256 public stepPower = 9800; // 0.98
    ...
    constructor(address _communityFund,address _esMMY, address _mmyVester) ERC721("MMC", "MummyClub NFT") {
    ...
    function getCurrentPP() public view returns (uint256 _mccPrice, uint256 _mccPower, uint256 _esMMYBonus) {
        ...
        uint256 modulus = totalSupply() % step;
        if (modulus == 0 && totalSupply() != 0) {
    ...
    function mintMummyClub(uint256 numberOfTokens) external payable nonReentrant {
        require(saleIsActive, 'Sale Is Not Active'); // Sale must be active
        require(numberOfTokens <= MAX_MMC_PURCHASE, 'Exceed Purchase'); // Max mint of 1
        require(totalSupply().add(numberOfTokens) <= MAX_MMC); // Max supply exceeded
        uint256 totalPrice = 0;
        for (uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() < MAX_MMC) {
                (mmcPrice, mmcPower, esMMYBonus) = this.getCurrentPP();
                totalPrice = totalPrice + mmcPrice;
                emit AssetMinted(msg.sender, totalSupply() + 1);
                nftDetailsById[totalSupply() + 1] = NFTDetails(mmcPower);
    ...
    function estimateAmount(uint256 numberOfTokens) external view returns (uint256) {
        uint256 totalPrice = 0;
        for (uint i = 0; i < numberOfTokens; i++) {
            if (totalSupply() + i < MAX_MMC) {
                uint256 _mmcPrice;
                (_mmcPrice,,) = this.getCurrentPP();
                totalPrice = totalPrice + mmcPrice;
            }
        }
        return totalPrice;
