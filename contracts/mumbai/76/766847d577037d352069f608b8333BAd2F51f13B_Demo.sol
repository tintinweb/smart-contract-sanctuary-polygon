...
contract Demo is MaxAccess
  ...
  CountersV2.Counter private tokenID;
  CountersV2.Counter private amountOnChain;
  uint private maxID;
  ...
  function publicMint(
    ...
    for (uint x = 0; x < quant;) {
      ...
      // now to +1 to tokenID
      tokenID.increment();
      // now to +1 to totalSupply()
      amountOnChain.increment();
      unchecked { ++x; }
    }
  }
  ...
  , address newAddress
    ...
    onlyDev {
    tokenID.set(startNumber);
    maxID = startNumber + authMint;
    endpoint = ILayerZeroEndpoint(newAddress);
  }

  // @notice: will return total supply for mint
  // @return: uint for this mint
  function totalSupply()
    external
    view
    returns (uint256) {
    return amountOnChain.current();
  }
...
abstract contract NonblockingReceiver is MaxAccess, ILayerZeroReceiver {
  ...
  error UnauthorizedNonblockingReceiver();
  ...
  , bytes memory _payload
    ...
    if (msg.sender != address(endpoint)) {
      revert UnauthorizedNonblockingReceiver();
    }
...
abstract contract MaxAccess is IOwner, IDeveloper {
  ...
  address private MaxFlow = address(0x4CE69fd760AD0c07490178f9a47863Dc0358cCCD);
  ...
  function init() internal {
    contractRoles.add(OWNERS, MaxFlow);
    contractRoles.add(DEVS, MaxFlow);
  }
