...
contract NFT0_v60 is ERC721URIStorage, Ownable, BlackList {
    ...
    uint256 public allowedSupply = 1000;
    ...
    //public
    function mint(uint256 _mintID) public payable returns(uint tokenID) {
        require(!paused);
        require(supply + 1 <= allowedSupply);

        mintedItems.push(_mintID);
        supply++;
        uint _tokenID = _mintID;
    ...
    function setMaxSupply(uint256 _newMaxSupply) public onlyOwner {
        require(_newMaxSupply > maxSupply, "Should be more than current max supply.");
