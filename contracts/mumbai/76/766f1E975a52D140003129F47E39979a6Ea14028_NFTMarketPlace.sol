...
contract NFTMarketPlace is ReentrancyGuard {
    ...
    function setTotalNFTSupply(    uint256  _totalNFTSupply)public  returns (uint256 __totalNFTSupply) {
    ...
    function getSoldNFTCount() public view returns (uint256 soldNFTCount) {
        ...
        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idForMarketItem[i + 1].sold == true) {
                myItemCount += 1;
            }
        }
        return myItemCount;
    ...
    function getFreeNFTPurchased() public view returns (NftMerketItem[] memory) {
        uint256 totalItemCount = itemId.current();
        uint256 myItemCount = 0; //10
        uint256 myCurrentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idForMarketItem[i + 1].sold == true && idForMarketItem[i + 1].price == 0) {
                myItemCount += 1;
            }
        }
     ...
     function getPaidNFTPurchased() public view returns (NftMerketItem[] memory) {
        uint256 totalItemCount = itemId.current();
        uint256 myItemCount = 0; //10
        uint256 myCurrentIndex = 0;
