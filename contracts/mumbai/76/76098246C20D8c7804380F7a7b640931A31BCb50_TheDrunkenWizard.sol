...
contract TheDrunkenWizard {
    ...
    function pickWinner(uint newCostPerPlayer,uint newTotalPlayerLimit, uint256[3] calldata purses, uint newSideBetCost, uint256[10] calldata seeds) public onlyOwner {
        ...
        uint seedNum = 0;
        uint winnerCount = 0;
        uint[10] memory _seeds = seeds;

        while (winnerCount < gamePurses.length) {
            uint256 randomWinnerID = random(players.length, _seeds[seedNum]);
