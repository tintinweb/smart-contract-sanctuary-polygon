...
contract mQuarkController is AccessControl {
  ...
  function setTemplateContractAddress(mQuarkTemplate template) external onlyRole(DEFAULT_ADMIN_ROLE) {
    s_template = template;
  }
...
contract mQuarkProject is ImQuarkProject, Ownable {
  ...
  /// @notice Prevents the same signature from being used twice
  // mapping(bytes => bool) private _inoperativeSignatures;
  event CollectionCreated(address instanceAddress, uint16 collectionId);
  ...
  function createCollection(
    ...
    // bytes[] calldata signatures,
    // string[] calldata uris,//extra- remove
    bytes32 merkelRoot
  ) public returns (address instance) {
    mQuarkController controller = s_registry.getContoller();
    mQuarkSubscriber subscriber = s_registry.getSubscriber();
    uint256 limitMintPrice = controller.getTemplateMintPrice(createParams.templateId);

    if (createParams.collectionURIs.length > 1 && isDynamicUri) revert("InvalidURIs();");
    ...
    if (createParams.mintPrice > 0) {
      if (createParams.collectionURIs.length > 1) {
    ...
    Collection memory m_collection = Collection({
      ...
      collectionURIs: isDynamicUri ? new string[](1) : createParams.collectionURIs,
      totalSupply: createParams.totalSupply,
      mintPrice: createParams.mintPrice,
      mintCount: 0,
      mintPerAccountLimit: createParams.mintPerAccountLimit,
      name: createParams.name,
      symbol: createParams.symbol,
      verifier: createParams.verifier,
      mintType: mintType,
      isWhitelisted: createParams.isWhitelisted,
      isFree: free
    });
    ...
    // );
    ImQuark721Implementation(instance).initilasiable(m_collection, msg.sender,merkelRoot);

    s_allCollections.push(instance);
    s_createdCollectionAddresses[m_collection.collectionId] = instance;
    emit NewCollection(instance, m_collection.collectionId);

    /// @dev should we not update the s_lastCollectionId after this?
    /// @dev we already update it everytime ++s_lastCollectionId.

    subscriber.setCollection(
      free,
    ...
    /// @dev we can add more parameters to the event.
    emit CollectionCreated(address(instance),m_collection.collectionId);
...
contract Registry is AccessControl, mQuarkProjectDeployer {
  ...
  function setImplementation(uint8 id, address implementation)  external onlyRole(DEFAULT_ADMIN_ROLE){
    s_implementations[id] = implementation;
  }
  ...
  function getContoller() external view returns (mQuarkController) {
    return (controller);
  }

  function getSubscriber() external view returns (mQuarkSubscriber) {
    return (subscriber);
  }
