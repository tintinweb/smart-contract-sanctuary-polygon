...
contract HErc20 is HToken {
  string public name = "HERC20 Test";
  string public symbol = "hTEST";
  ...
  /**
   * @notice Withdraws any amount of hTokens for the specified _amount of underlying ERC-20 asset
  ...
  function repayBorrow(uint256 _repayAmount, uint256 _collateralId) external {
    repayBorrowInternal(_repayAmount, _collateralId);
  }
  ...
  function repayBorrowBehalf(
    ...
    repayBorrowBehalfInternal(_borrower, _repayAmount, _collateralId);
  }
  ...
  /**
   * @notice A public function to sweep accidental ERC-20 transfers to this contract. Tokens are sent to dao
...
contract HToken is ERC1155, IERC721Receiver, ErrorReporter, AccessControlEnumerable {
   ...
   */
  event RepayBorrow(address indexed payer, uint256 repayAmount, uint256 accountBorrows, uint256 totalBorrows, uint256 _collateralId);
   ...
   */
  event NewHivemindAddr(HivemindI oldHivemind, HivemindI newHivemind);

  /**
   * @notice Event emitted when liquidator is changed
   */
  event NewLiquidator(address oldLiquidator, address newLiquidator);

  /**
   * @notice Event emitted when interestRateModel is changed
   */
  event NewMarketInterestRateModel(InterestRateModelI oldInterestRateModel, InterestRateModelI newInterestRateModel);
  ...
  /**
   * @notice dao address were sweeped tokens will be transmitted
   */
   ...
   */
  mapping(uint256 => Collateral) public borrowCouponsIdPerCollateral;
  ...
  constructor(
    ...
    borrowIndex = 1e18 / 10**(18 - _underlyingToken.decimals());

    // // Setup interest rate
    interestRateModel = _interestRateModel;

    // Set reserve factor
    _setReserveFactorFresh(_reserveFactorMantissa);

    // Set admin fee
    _setAdminFeeFresh(_adminFeeMantissa);

    // Set fuse fee
    _setFuseFeeFresh(_fuseFeeMantissa);

    // Check reserveFactorMantissa + newAdminFeeMantissa + newFuseFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (adminFeeMantissa + fuseFeeMantissa + reserveFactorMantissa > reserveFactorPlusFeesMaxMantissa) revert AdminError(Error.BAD_INPUT);
   ...
   * @param _newFuseFeeMantissa new fuse fee mantisa
   */
  function _setFuseFeeFresh(uint256 _newFuseFeeMantissa) internal {
    ...
    if (_newFuseFeeMantissa == type(uint256).max) _newFuseFeeMantissa = fuseFeeMantissa;

    // Check reserveFactorMantissa + newAdminFeeMantissa + newFuseFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (reserveFactorMantissa + adminFeeMantissa + _newFuseFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }

    // If setting fuse fee
    if (fuseFeeMantissa != _newFuseFeeMantissa) {
      // Set fuse fee
      uint256 oldFuseFeeMantissa = fuseFeeMantissa;
      fuseFeeMantissa = _newFuseFeeMantissa;

      // Emit event
      emit NewFuseFee(oldFuseFeeMantissa, _newFuseFeeMantissa);
   ...
   */
  function _setAdminFeeFresh(uint256 _newAdminFeeMantissa) internal {
    // Verify market's block number equals current block number
    if (accrualBlockNumber != getBlockNumber()) {
      revert AdminError(Error.MARKET_NOT_FRESH);
    }
    ...
    if (_newAdminFeeMantissa == type(uint256).max) _newAdminFeeMantissa = adminFeeMantissa;

    // Check reserveFactorMantissa + newAdminFeeMantissa + newFuseFeeMantissa ≤ reserveFactorPlusFeesMaxMantissa
    if (reserveFactorMantissa + _newAdminFeeMantissa + fuseFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }

    // If setting admin fee
    if (adminFeeMantissa != _newAdminFeeMantissa) {
      // Set admin fee
      uint256 oldAdminFeeMantissa = adminFeeMantissa;
      adminFeeMantissa = _newAdminFeeMantissa;

      // Emit event
      emit NewAdminFee(oldAdminFeeMantissa, _newAdminFeeMantissa);
    } else revert WrongParams();
  }
   ...
   */
  function _setReserveFactorFresh(uint256 _newReserveFactorMantissa) internal {
    // Verify market's block number equals current block number
    if (accrualBlockNumber != getBlockNumber()) {
      revert AdminError(Error.MARKET_NOT_FRESH);
    }
    ...
    if (_newReserveFactorMantissa + adminFeeMantissa + fuseFeeMantissa > reserveFactorPlusFeesMaxMantissa) {
      revert AdminError(Error.BAD_INPUT);
    }

    if (reserveFactorMantissa != _newReserveFactorMantissa) {
      uint256 oldReserveFactorMantissa = reserveFactorMantissa;
      reserveFactorMantissa = _newReserveFactorMantissa;

      emit NewReserveFactor(oldReserveFactorMantissa, _newReserveFactorMantissa);
    } else revert WrongParams();
  }
  ...
  */
  ...
  /**
   * @notice get borrowed amount from a coupon
   * @dev Accrue interest first to get fresh data
   * @param _collateralId index of the borrwed coupon
   * @return borrowed amount of underlying
   */
  function getBorrowFromCoupon(uint256 _collateralId) public view returns (uint256) {
    return borrowCoupons[_collateralId].borrowAmount;
  }
   ...
   */
  function getUserCoupons(address _user) public view returns (Coupon[] memory) {
    // unchecked {
    ...
    uint256 counter;
    for (; i < length; i++) {
      Collateral memory collateral = borrowCouponsIdPerCollateral[i];
  ...
  /**
   * @notice accruels interests to all coupons
   * @return all accrued interest
   */
  function accrueInterestToCoupons() public returns (uint256) {
    uint256 index;
    ...
    for (index = 1; index <= length; ) {
      Collateral memory collateral = borrowCouponsIdPerCollateral[index];
      unchecked {
        index++;
      }
      if (!collateral.active) continue;
      Coupon memory coupon = borrowCoupons[collateral.collateralId];
      if (coupon.active == 2) {
        runningTotal += accrueInterestToCoupon(coupon.collateralId);
      }
    }
  ...
  /**
   * @notice accruels interest to coupon
   * @param _collateralId borrow index
   * @return new total accrued
   */
  function accrueInterestToCoupon(uint256 _collateralId) public returns (uint256) {
    ...
    uint256 interestIncurred = (indexDelta * initialAmount) / 10**decimals;
    uint256 newTotal = initialAmount + interestIncurred;

    // Effects
    activeCoupon.borrowAmount = newTotal;
    activeCoupon.index = currentIndex;
    borrowBalancePerUser[activeCoupon.owner] += interestIncurred;
  ...
  /**
   * @notice minting hToken to `_to`
   * @param _to minting to
   * @param _amount the amount to mint
   */
  function _mintHToken(address _to, uint256 _amount) internal {
  ...
  function depositUnderlyingInternal(uint256 _amount) internal {
    ...
    uint256 deposited = doUnderlyingTransferIn(msg.sender, _amount);
  ...
  /**
   * @notice does transfer out, meaning will withdraw underlying token from the contract to `_to`
  ...
  /**
   * @notice does transfer in, meaning will deposit underlying token to the contract's balance
   *         `_from` needs to approve before hand
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    uint256 underlyingToRedeem = (_amount * exchangeRate) / 1e18;

    /* Fail if redeem not allowed */
    hivemind.redeemAllowed(HTokenI(address(this)), msg.sender, underlyingToRedeem);
  ...
  function withdrawInternal(uint256 _amount) internal {
    ...
    uint256 tokensToRedeem = (_amount * 1e18) / exchangeRate;
  ...
  /**
   * @notice used to deposit collateral
   * @param _tokenId to be deposited as collateral
   */
  ...
  /**
   * @notice minting a coupon when depositing collateral, this coupon will accrue borrows
  ...
  function _mintCoupon(
    ...
    borrowCouponsIdPerCollateral[currentId] = Collateral(_collateralId, true);
  ...
  /**
   * @notice borrows an amount and accrued it to the coupon
  ...
  function borrowInternal(uint256 _borrowAmount, uint256 _collateralId) internal {
    ...
    // Coupon memory coupon = searchUserCouponByNftId(msg.sender, _collateralId);
    Coupon memory coupon = borrowCoupons[_collateralId];

    // Can't be the null coupon
    if (coupon.active == 0) revert HTokenError(Error.COUPON_LOOKUP);
    ...
    hivemind.borrowAllowed(HTokenI(address(this)), msg.sender, address(collateralToken), _collateralId, _borrowAmount);
    ...
    borrowBalancePerUser[coupon.owner] += _borrowAmount;

    borrowCoupons[coupon.collateralId] = coupon;

    emit Borrow(msg.sender, _borrowAmount, _collateralId, coupon.borrowAmount);
    // Transfer the funds to borrower
    doUnderlyingTransferOut(msg.sender, _borrowAmount);
  }

  /**
   * @notice does collateral transfer out, transfer nft to the _to
  ...
  /**
   * @notice does collateral transfer in, transfer nft to the hToken
   * @param _from where to get the collateral
   * @param _collateralId what to transfer, the collateral id
   */
  function doCollateralTransferIn(address _from, uint256 _collateralId) internal {
    collateralToken.transferFrom(_from, address(this), _collateralId);
  }

  /**
   * @notice repais borrow for a collateral
   * @param _repayAmount the amount to repay
   * @param _collateralId to want collateral id to pay for
   */
  function repayBorrowInternal(uint256 _repayAmount, uint256 _collateralId) internal {
    hivemind.repayBorrowAllowed(HTokenI(address(this)), _repayAmount, address(collateralToken), _collateralId);
    ...
    // Set amount to repay
    uint256 payment;
    if (_repayAmount > debt) {
      payment = debt;
    } else {
      payment = _repayAmount;
    }
    ...
    emit RepayBorrow(msg.sender, payment, debt, totalBorrows, _collateralId);
  }

  /**
   * @notice repais borrow for a collateral for someone else
   * @param _borrower for whom to pay
   * @param _repayAmount the amount to repay
   * @param _collateralId to want collateral id to pay for
   */
  function repayBorrowBehalfInternal(
    ...
    // Sanity check to make sure the coupon was found correctly
    if (coupon.collateralId != _collateralId) revert HTokenError(Error.COUPON_NOT_FOUND);
    ...
    // Get outstanding debt
    if (coupon.borrowAmount == 0) revert HTokenError(Error.NO_DEBT);

    // Set amount to repay
    uint256 payment;
    if (_repayAmount > coupon.borrowAmount) {
      payment = coupon.borrowAmount;
    } else {
      payment = _repayAmount;
    }
  ...
  /**
   * @notice liquidate borrw once the health factor is over the collateral factor.
  ...
  /**
   * @notice check if liquidation is allowed for this coupon, meaning it's active
   * @param _collateralId what coupon to check for
   * @return true if it's allowed
   */
  function liquidationAllowedByCouponId(uint256 _collateralId) internal view returns (bool) {
    return borrowCoupons[_collateralId].active == 2;
  }

  /**
   * @notice check if liquidation is allowed for this nft, meaning it's active
  ...
  function liquidationAllowedByCollateralId(uint256 _collateralId) internal view returns (bool allowed) {
    unchecked {
      ...
      for (; i < length; i++) {
        ...
        if (!collateral.active) continue;
        Coupon memory coupon = borrowCoupons[collateral.collateralId];
        if (coupon.collateralId == _collateralId) return coupon.active == 2;
      }
    }
  }

  /**
   * @notice closing liquidation, meaning that you pay the borrow and burn the coupon
  ...
  function closeoutLiquidation(address _borrower, uint256 _collateralId) external {
    ...
    // Repay borrow
    repayBorrowBehalfInternal(_borrower, coupon.borrowAmount, _collateralId);

    // Burn the NFT coupon
    burnAndDelete(_borrower, coupon.collateralId, coupon.id); // TODO check this
  }

  /**
   * @notice checks if withdrawal are allowed for this token id
   * @param _user borrower who pays
   * @param _collateralId what to pay for
   * @return returns true if it's allowed
   */
  function withdrawalAllowed(address _user, uint256 _collateralId) public view returns (bool) {
    Coupon memory coupon = borrowCoupons[_collateralId];
    if (coupon.owner != _user) revert Unauthorized();
    return coupon.borrowAmount == 0;
  }

  /**
   * @notice burns a coupon and deletes it from the all coupons
   * @param _user user to whom to burn
  ...
  function burnAndDelete(
    ...
    borrowCoupons[_collateralId].active = 1;
    _burn(_user, _couponId, 1);
  }
  ...
  function withdrawCollateralInternal(uint256 _collateralId) internal {
    ...
    if (!withdrawalAllowed(msg.sender, activeCoupon.collateralId)) revert HTokenError(Error.WITHDRAW_NOT_ALLOWED);
  ...
  function exchangeRateStoredInternal() internal view returns (uint256) {
    ...
    if (_totalSupply == 0) {
      ...
      uint256 exchangeRate = (cashPlusBorrowsMinusReserves * 10**decimals) / _totalSupply;
  ...
  /**
   * @notice Function to simply retrieve block number
   ...
   */
  function getBalanceUnderlyingByUser(address _account) external view returns (uint256) {
    return (underlyingToken.balanceOf(_account) * exchangeRateStored()) / 1e18;
  }

  /**
   * @notice Get the number of coupons for a user
   * @dev this is gas intensive do not use it in any contract calls
   * @param _account the account we get the quantity for
   * @return The quantity of the coupons for _account
   */
  function getUserCouponsLength(address _account) external view returns (uint256) {
    return userToCoupons[_account];
  }
  ...
  /**
   * @notice Get underlying balance that is available to be withdrewn
   * @return The quantity of underlying that can be borrowed
   */
  function getAvailableUnderlyingToBorrow() external view returns (uint256) {
    return getCashPrior() - totalBorrows - totalAdminFees - totalFuseFees;
  }
  ...
  function accrueInterest() public {
    ...
    interestAccumulated = (simpleInterestFactor * totalBorrows) / 10**decimals;

    // TODO: Attribute borrows out into individual NFTs

    totalReserves += (reserveFactorMantissa * interestAccumulated) / 10**decimals;

    totalFuseFees += (fuseFeeMantissa * interestAccumulated) / 10**decimals;
    totalAdminFees += (adminFeeMantissa * interestAccumulated) / 10**decimals;

    borrowIndex += (simpleInterestFactor * borrowIndex) / 10**decimals;
    ...
    // Update borrows
    totalBorrows = accrueInterestToCoupons();
  ...
  /**
   * @notice adds reservers, accrues interests
  ...
  /**
   * @notice adds reservers, does not accrues interests
   * @param _addAmount what to add
   */
  ...
  {
    // Coupon[] memory userCoupons = getUserCoupons(_account);

    uint256 hTokenBalance = balanceOf(_account, 0);
    ...
    return (hTokenBalance, borrowBalancePerUser[_account], exchangeRateMantissa);
  }
   ...
   * @param _newFuseFeeMantissa new fee fuse mantisa
   */
  function _setFuseFee(uint256 _newFuseFeeMantissa) external {
    ...
    _setFuseFeeFresh(_newFuseFeeMantissa);
  }
  ...
  function _setAdminFee(uint256 _newAdminFeeMantissa) external {
    ...
    // _setAdminFeeFresh emits reserve-factor-specific logs on errors, so we don't need to.
    _setAdminFeeFresh(_newAdminFeeMantissa);
  }

  /**
   * @notice accrues interest and sets a new reserve factor for the protocol using _setReserveFactorFresh
   * @dev Admin function to accrue interest and set a new reserve factor
   * @param _newReserveFactorMantissa new reserver factor mantissa
   */
  ...
  /**
   * @notice sets liquidator address
   * @param _newLiquidator new liquidator
   */
  function setLiquidator(address _newLiquidator) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    address old = liquidator;
    liquidator = _newLiquidator;
    emit NewLiquidator(old, _newLiquidator);
  }

  /**
   * @notice sets hivemind address
   * @param _newHivemind new hivemind
   */
  function setHivemind(HivemindI _newHivemind) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) revert Unauthorized();
    HivemindI old = hivemind;
    hivemind = _newHivemind;
    emit NewHivemindAddr(old, _newHivemind);
  }
  ...
  function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC1155, AccessControlEnumerable) returns (bool) {
    _interfaceId == type(IERC1155).interfaceId || _interfaceId == type(AccessControlEnumerable).interfaceId;
  ...
  function _beforeTokenTransfer(
    ...
    for (j; j < _ids.length; j++) {
      ...
      if (_ids[j] > 0) {
        Collateral memory collateral = borrowCouponsIdPerCollateral[_ids[j]];
        if (!collateral.active) continue;
        borrowCoupons[collateral.collateralId].owner = _to;
        lengthToModify++;
      }
    }
