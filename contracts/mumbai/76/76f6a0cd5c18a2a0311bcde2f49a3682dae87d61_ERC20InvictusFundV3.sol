...
// File contracts/ERC20InvictusFundV3.sol
...
contract ERC20InvictusFundV3 is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable {
  ...
  event RejectedRedemption(uint256 withdrawAmount, string ticker, address participant);
   ...
   */
  function updateRequeueNonce(uint24 _requeueNonce) internal {
  ...
  function _handleRedemptionsDealing() private returns (bool) {
    ...
    for (uint256 i = redemptionsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(redemption.investor)) {
        redemptions.pop();
        transferFrom(DEAL_ADDRESS, withheldAddress, redemption.amount);
        emit RejectedRedemption(redemption.amount, redemption.ticker, redemption.investor);
        ...
        updateRequeueNonce(requeueNonce + 1);
      }
    }
