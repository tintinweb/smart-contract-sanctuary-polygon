...
contract MERXMarketplace is ReentrancyGuard, Pausable, AccessControl {
    ...
    function setIsWalletVerified(address wallet, bool status) public onlyRole(DEFAULT_ADMIN_ROLE) {
        verifieds[wallet] = status;
    }
    ...
    function createOrder (
        ...
        if(verifieds[msg.sender] && isUtility){
            // wallet that verified
            require(insuranceRate >= 10 && insuranceRate <= 90, "Insurance rate is incorrect");
        } else {
            require(insuranceRate == 0, "Insurance rate muse be zero");
        }
    ...
    function buyOrder( // terminate buy when it's auction
        ...
        bool isInsurance = verifieds[orderData.seller] && orderData.isUtility;
        uint256 toSeller = isInsurance ? price * (100 - orderData.insuranceRate) / 100 : price; // to seller
        uint256 toInsurance = isInsurance ? price * (orderData.insuranceRate) / 100 : 0;
        uint256 balance = IERC20(orderData.tokenAddress).balanceOf(msg.sender);
        uint256 fee = toSeller * _feeRate / 100;
        require(
            balance >= toSeller + toInsurance + fee,
        ...
        IERC20(orderData.tokenAddress).transferFrom(msg.sender, orderData.seller, toSeller); // to seller
        if(isInsurance) IERC20(orderData.tokenAddress).transferFrom(msg.sender, _poolWallet, toInsurance); // to pool insurance
        ...
        emit BoughtEvent(orderData.nftContract, msg.sender, orderData.tokenId, orderData.marketType, orderId, toSeller, toInsurance, fee, false);
        return orderData.orderId;
    }
    function bidAuction (
        uint256 orderId,
    ...
    function winnerAcceptBid(uint256 orderId) public nonReentrant returns(uint256) {
        ...
        bool isInsurance = verifieds[orderData.seller] && orderData.isUtility;
        uint256 toSeller = isInsurance ? orderData.currentPrice * (100 - orderData.insuranceRate) / 100 : orderData.currentPrice;
        uint256 toInsurance = verifieds[orderData.seller] ? orderData.currentPrice * (orderData.insuranceRate) / 100 : 0;
        uint256 fee = toSeller * _feeRate / 100;
        require(orderData.isActive, "Order isn't active");
        ...
        orders[orderId].isActive = false;
        orders[orderId].buyer = msg.sender;
        IERC20(orderData.tokenAddress).transferFrom(msg.sender, orders[orderId].seller, toSeller);
        IERC20(orderData.tokenAddress).transferFrom(msg.sender, _recipientWallet, fee);
        if(isInsurance) IERC20(orderData.tokenAddress).transferFrom(msg.sender, _poolWallet, toInsurance);
        IERC721(orderData.nftContract).setApprovalForAll(msg.sender, true);
