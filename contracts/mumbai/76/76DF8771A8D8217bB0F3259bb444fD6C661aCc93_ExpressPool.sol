...
contract ExpressPool is Ownable {
    ...
    function processDeposit(uint256 _depositAddressIndex, address _beneficiaryAddress) external onlyProcessor {
        address depositAddress = depositAddresses[_depositAddressIndex];
        require(depositAddress != address(0), "Cannot find address");
        address quoteTokenAddress = investmentPoolMaster.quoteTokenContractAddressList(quoteTokenIndex);
        uint256 usdBalance = IERC20(quoteTokenAddress).balanceOf(depositAddress);
        require(usdBalance > minDepositAmount, "Minimum deposit is not met");

        investmentPoolMaster.geniusRouter().processPayment(quoteTokenAddress, depositAddress, address(this), usdBalance);
        ...
        uint256 instantBuyAmount = usdBalance.mul(quickswapBuyRatio).div(BASIS_POINT);
        ...
        if (instantBuyAmount > 0 ) { 
            ...
            QuickswapInterface(exchangeProxyContract.getQuickswapRouterAddress()).swapExactTokensForTokens(
                instantBuyAmount,
                exchangeProxyContract.getAmountsOut(instantBuyAmount),
                paths,
                address(this),
                block.timestamp
            );
    ...
    function claimInvestmentRewards(uint256 _investmentIndex) external {
        ...
        investmentPoolMaster.geniusRouter().processPayment(baseTokenAddress, rewardsPayerAddress, investment.beneficiaryAddress, claimableRewards);
