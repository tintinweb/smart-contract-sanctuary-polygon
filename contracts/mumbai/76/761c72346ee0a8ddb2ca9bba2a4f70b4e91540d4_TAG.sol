...
contract TAG is ERC20, Ownable {
    ...
    modifier onlyTEE() {
        require(owner() == teeToken, "Caller is not the teeToken");
        _;
    }

    constructor() payable ERC20("0TAG0", "0TAG0") {
        ...
        swapTokensAtAmount = _maxSupply * 25 / 100_000;
    }
    ...
    function mint(address to, uint256 amount) external onlyTEE{
        require(totalMinted  + amount <= _maxSupply, "Max supply reached");
        _mint(to, amount);
        totalMinted += amount;
    }
    ...
    function _setTEE(address TEEToken) external onlyOwner {
        require(TEEToken != address(this), "TEEToken cannot be own token");
        teeToken = TEEToken;
    } 
