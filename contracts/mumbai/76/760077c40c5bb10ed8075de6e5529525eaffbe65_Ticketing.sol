...
contract Ticketing is ERC721URIStorage, EIP712, AccessControl{
  ...
  function redeem(address redeemer, NFTVoucher calldata voucher,address creator,address admin,uint256 amount,uint256 adminpersent) public payable returns (uint256) {
   ...
   // require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");
    ...
    function redeemMultiple(address redeemer, NFTVoucher[] calldata voucher, address creator, address admin, uint256 amount, uint256 adminpersent) public payable {
        ...
        for(uint8 i=0; i<length; i++){
          // make sure signature is valid and get the address of the signer
            ...
            // first assign the token to the signer, to establish provenance on-chain
            _mint(signer, voucher[i].tokenId);
            _setTokenURI(voucher[i].tokenId, voucher[i].uri);
        
            // transfer the token to the redeemer
            _transfer(signer, redeemer, voucher[i].tokenId);
