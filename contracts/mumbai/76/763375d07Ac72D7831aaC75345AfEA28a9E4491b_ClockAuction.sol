...
contract PlantBreeding is UIAMToken {
    ...
    function approveBreedingRights(address _addr, uint256 _tokenId)
        ...
        whenNotPaused
    {
        require(ownerOf(_tokenId) == msg.sender || _owns(msg.sender, _tokenId));
    ...
    function breedWithAuto(
        ...
        require(_isSiringPermitted(_sireId, _matronId, msg.sender));
...
contract AllelePlantEIP712 {
    ...
    function hash(Breed memory breed) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            BREED_TYPEHASH,
            breed.tokenA,
            breed.tokenB,
