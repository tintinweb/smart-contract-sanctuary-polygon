...
// File contracts/token/ERC1155/InventoryAccessControl.sol

pragma solidity >=0.6.6;

contract InventoryAccessControl is AccessControl {
    bytes32 public constant MINTER_ROLE = 0x241ecf16d79d0f8dbfb92cbc07fe17840425976cf0667f022fe9877caa831b08; // keccak256(abi.encodePacked("MINTER_ROLE"));
    ...
    bytes32 public constant MANAGER_ROLE = 0x241ecf16d79d0f8dbfb92cbc07fe17840425976cf0667f022fe9877caa831b08; // keccak256(abi.encodePacked("MINTER_ROLE"));
...
// File contracts/token/ERC1155/TowerInventory.sol
pragma solidity ^0.8.0;




contract TowerInventory is ERC1155, Pausable, InventoryAccessControl {
    ...
    event InternalMint(address indexed _sender, uint256[] _tokenIds, uint256[] _amounts);
    ...
    function uri(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(uri, tokenIdString, ".json"));
    }

    function setTokenTypes(uint256 _tokenTypeId, string memory _tokenType) external onlyAdmin onlyManager {
        _setTokenTypes(_tokenTypeId, _tokenType);
    }

    function _setTokenTypes(uint256 _tokenTypeId, string memory _tokenType) internal {
        tokenTypes[_tokenTypeId] = _tokenType;
    }

    function getNumberOfTokens(address _owner) external view returns (uint256) {
        return allTokens[_owner].length;
    }

    function getAllTokens(address _owner) external view returns (uint256[] memory) {
        return allTokens[_owner];
    }
    ...
    function batchMint(
        ...
        uint256[] memory _amounts,
        address _sender
    ) public onlyMinter onlyMinterAdmin {
        _mintBatch(_sender, _tokenIds, _amounts, "0x");
        emit InternalMint(_sender, _tokenIds, _amounts);
    }
    ...
    constructor(string memory _uri) Pausable() ERC1155(_uri) InventoryAccessControl() {
        signer = msg.sender;
