...
contract  Collection is ERC4907 {
    ...
    uint256 idCount;
    using Strings for uint256;
    mapping (uint256 => info) internal s_info;
    ...
    function mint(string memory newUri, address to) public returns (bool){
        idCount++;
        _safeMint(to, idCount);
        s_info[idCount].defaultURI = newUri;
        return true;
    }

    function changeURI(string memory newUri, uint256 tokenId) public returns (bool){
        s_info[tokenId].userURI = newUri;
        return true;
    }
  ...
  /*
    function rental(
        ...
        require(
            userOf(tokenId) == ownerOf(tokenId) &&
            userExpires(tokenId) < block.timestamp, "Currently on rental!"
        );
    ...
    function changeOwnerUri(uint256 tokenId, string memory newUri) public {
        require(msg.sender == ownerOf(tokenId), "Not owner");
        s_info[tokenId].defaultUri = newUri;
    }

    function changeUserUri(uint256 tokenId, string memory newUri) public {
        require(msg.sender == userOf(tokenId), "Not owner");
        s_info[tokenId].userUri = newUri;
    }
