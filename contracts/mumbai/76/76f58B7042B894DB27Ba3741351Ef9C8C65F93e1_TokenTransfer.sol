...
contract TokenTransfer is ERC2771Context {
    IERC20 _token;
    ...
    modifier checkAllowance(uint amount) {
        require(_token.allowance(_msgSender(), address(this)) >= amount, "Error");
        _;
    }
    
    //modifier only owner so that only owner who deployed the contract can call certain functions.
    modifier onlyOwner{
    ...
    //function param token must be the address of the token you wish to transfer from accountA to B 
    function setTokentoTransfer(address token) public {
        _token = IERC20(token);
    }

    //function to setTrustedForwarder:
