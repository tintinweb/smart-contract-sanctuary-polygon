...
contract WondCommunitySale is Ownable, ReentrancyGuard, PaymentSplitter {
    ...
    struct RoundInfo {
        ...
        uint256 publicPrice; /// public price without discount in 1e18 units
    ...
    /// usd allocation size to be in the tier 
    uint256 public firstTier = 10; // 10k$ -> first tier (public price / 2)
    uint256 public secondTier = 30; // 30k$ -> first tier (public price / 4)
    /// safety check
    ...
    event SetUpCommunitySale(uint256 alloc, uint256 price);
    ...
    nonReentrant {
        ...
        uint256 amountToPayUSD = allocation.fmul(round.publicPrice, FixedPointMathLib.WAD);
        /// we take MATIC/USD price from chainlink oracle
        uint256 maticPriceUSD = getLatestPriceMatic();
        /// calculate amount to send in MATIC
        uint256 amountToPayMATIC = checkPriceForAllocation(amountToPayUSD, maticPriceUSD);
    ...
    function checkPriceForAllocation(uint256 allocationUSD, uint256 maticPrice) internal view returns(uint256) {
        uint256 toPay = allocationUSD.fdiv(maticPrice, FixedPointMathLib.WAD);
        if (allocationUSD > firstTier && allocationUSD < secondTier) {
            toPay = toPay.fdiv(2, FixedPointMathLib.WAD);
        }
        else if (allocationUSD >= secondTier) {
            toPay = toPay.fdiv(4, FixedPointMathLib.WAD);
        }
    ...
    function setupCommunitySale (
        ...
        emit SetUpCommunitySale(_totalAllocation, _price);
    }
}