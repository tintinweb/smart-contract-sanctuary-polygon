...
contract HackjackVRF is VRFConsumerBaseV2, BreakdownUint256, Destructible, RequestQueue { 
    ...
    function fulfillRandomWords(
        ...
        // Fulfill requests until queue empty 
        while (!isEmpty()) {
        for (uint i=0; i< numWords; i++) {
            ...
            for (uint j=0; j<32; j++) {
                ...
                requestIndexToRequestInfo[requestIndex].randomNumber = randomUint8s[j];
            }
        }
    ...
    function testfulfillRandomWords() public {
        enqueue(234089);
        uint requestId = 123094871230987;
        uint256[2] memory randomWords = [uint(23580234),uint(23580234)];

        // Fulfill requests until queue empty 
        while (!isEmpty()) {
            uint256 word = dequeue();
