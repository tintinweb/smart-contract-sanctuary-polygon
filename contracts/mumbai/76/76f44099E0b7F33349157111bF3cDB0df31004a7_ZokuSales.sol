...
contract ZokuSales is Ownable, Initializable, ISales {
  ...
  struct Payment {
    uint256 basePrice;  // amount before fees applied
    uint256 afterFee; // amount after fees applied
  }
  ...
  mapping(address => mapping(address => Payment)) private _payments;
  ...
  function _addUser(
    ...
    if (item.user[_user].allocation > 0) {
      /// current user
  ...
  function _pay(uint256 price) internal {
    if (settings.isNativeToken()) {
      ...
      _payments[settings.nativeToken()][_msgSender()].basePrice += price;
      _payments[settings.nativeToken()][_msgSender()].afterFee += price - adminFees;
    } else {
      require(price <= IERC20(settings.paymentToken()).allowance(_msgSender(), address(this)), "Factory: Insufficent allowance");
      require(price <= IERC20(settings.paymentToken()).balanceOf(_msgSender()), "Factory: Insufficent Funds");
      ...
      _payments[settings.paymentToken()][_msgSender()].basePrice += price;
      _payments[settings.paymentToken()][_msgSender()].afterFee += price - adminFees;
    }
  }
  ...
  function mint(
    ...
    require(settings.token() != address(0), "Factory: Project not found");
    ...
    require(uint256(consumedQty + 1) <= uint256(qty), "Factory: All item minted");
    require(1 <= qty, "Factory: Exceeds max amount per mint");
    ...
    _mintedTokens[_msgSender()].push(tokenId);
    
    emit SalesTokenMinted(settings.token(), _stage, _item, 1, _msgSender(), tokenId, _tokenURI);
  }

  function paymentOf(address _user) public view returns(uint256 basePrice, uint256 afterFee) {
    basePrice = _payments[settings.paymentToken()][_user].basePrice;
    afterFee = _payments[settings.paymentToken()][_user].afterFee;
  }

  function refund() external {
    IERC721Standard tokenContract = IERC721Standard(settings.token());
    (uint256 basePrice, uint256 afterFee) = paymentOf(_msgSender());
    
    require(_mintedTokens[_msgSender()].length == tokenContract.balanceOf(_msgSender()), "Sales: Mismatch token balance");

    for (uint256 i = 0; i < _mintedTokens[_msgSender()].length; i++) {
      require(tokenContract.ownerOf(_mintedTokens[_msgSender()][i]) == _msgSender(), "Sales: One or more of token is not your owned");
    }

    require(settings.refundable(), "Sales: No refundable");
    require(afterFee > 0, "Sales: No refundable for you");
    
    uint256 amountToRefund = afterFee;

    if (settings.refundableIncludeFee()) {
      amountToRefund = basePrice;
    }

    if (settings.isNativeToken()) {
      require(address(this).balance >= amountToRefund, "Sales: Insufficent funds to refund");
      payable(_msgSender()).transfer(amountToRefund);
    } else {
      require(IERC20(settings.paymentToken()).balanceOf(address(this)) >= amountToRefund, "Sales: Insufficent funds to refund");
      SafeERC20.safeTransfer(IERC20(settings.paymentToken()), payable(_msgSender()), amountToRefund);
    }

    _payments[settings.paymentToken()][_msgSender()].afterFee = 0;

    if (settings.refundableIncludeFee()) {
      _payments[settings.paymentToken()][_msgSender()].basePrice = 0;
    }
    
    for (uint256 i = 0; i < _mintedTokens[_msgSender()].length; i++) {
      tokenContract.burn(_mintedTokens[_msgSender()][i]);
      emit Burned(settings.token(), _mintedTokens[_msgSender()][i], _msgSender());
    }
...
interface ISales {
  ...
  function transferOwnership(address newOwner) external;
  function mint(uint8 stage, uint8 item, string memory tokenURI) external payable returns(uint256 tokenId);
  function paymentOf(address user) external view returns(uint256 basePrice, uint256 afterFee);
  function refund() external;
}
...
interface ISettings {
  ...
  event RefundableUpdated(bool refundable, bool refundableIncludeFee, address sender);
  ...
  function refundableIncludeFee() external view returns(bool);
  ...
  function setRefundable(bool refundable, bool includeFee) external;
