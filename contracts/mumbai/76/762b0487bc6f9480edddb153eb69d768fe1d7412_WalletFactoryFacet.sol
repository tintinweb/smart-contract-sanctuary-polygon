...
contract WalletFactoryFacet {
	...
	function initialize(address _diamondAddress) external{
		if(LibDiamond.contractOwner()==msg.sender) revert WalletFactoryFacet__NotDiamondOwner();
		if(diamondAddress!=address(0)) revert WalletFactoryFacet__AlreadyInitialized();
	...
	if(address(this) != diamondAddress) revert WalletFactoryFacet__FunctionShouldBeCalledFromTheDiamond();
	_;
	}
	...
	function getWallet(address _owner) external view callingFromDiamond returns (address) {
