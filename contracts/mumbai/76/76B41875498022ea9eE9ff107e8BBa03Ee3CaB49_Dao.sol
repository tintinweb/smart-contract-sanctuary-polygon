...
contract Dao {
    ...
    function createProposal(string memory _description) public {
        ...
        emit proposalCreated(newProposal.id, _description, 0, msg.sender);
    ...
    function closeVotation() public {
        require(
            checkProposalEligibility(msg.sender),
            "Solo usuarios pertenecientes al NFT pueden cerrar la votacion"
        );

        uint256 votes = 0;
        Proposal memory winnerProposal;
        for (uint256 i = 0; i < proposals.length; i++) {
        ...
        for (uint256 i = 0; i < proposals.length; i++) {
            if (proposals[i].votes == votes) {
                proposalsWin.push(winnerProposal);
            }
        }
        delete proposals;
    }
    ...
    function voteOnProposal(uint256 _id, bool _vote) public {
        require(checkProposalExists(_id), "This Proposal does not exist");
        require(checkVoteEligibility(msg.sender), "You have already voted");
