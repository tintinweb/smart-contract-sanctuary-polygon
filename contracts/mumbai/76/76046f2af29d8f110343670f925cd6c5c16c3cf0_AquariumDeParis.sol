...
contract AquariumDeParis is ERC721A, Pausable, Ownable, ReentrancyGuard {
  //--------------------------------------------------
  // Constants
  //--------------------------------------------------
  uint256 public constant MAX_TOKENS = 1234;
  uint256 public constant MAX_TOKENS_PER_WALLET = 50;
  /// @notice 34 reserved, 50 early supporters
  uint256 public constant RESERVED_AMOUNT = 84;

  //--------------------------------------------------
  // Variables
  //--------------------------------------------------
  uint256 public maxTotalMintsThisStage = 1234;
  uint256 public maxMintBatchAmount = 50;
  uint256 public pricePerToken = 0.18 ether;
  ...
  //--------------------------------------------------
  constructor()
    ERC721A("Jellyfish - Aquarium de Paris x Metaseum", "METASEUM-JELLY")
  {}
  ...
  function mint(
    ...
    uint256 addressMintedMinter = addressMinted[minter];
    ...
    require(
      addressMintedMinter + amount <= MAX_TOKENS_PER_WALLET,
      "Address would exceed balance limit"
    );
    require(
      msg.value >= pricePerToken * amount,
      "Not enough ETH for transaction"
    );

    // The actual mint
    addressMintedMinter += amount;
    _mint(minter, amount);
    emit Mint(minter, amount);

    addressMinted[minter] = addressMintedMinter;
  }
  ...
  function setMaxMintBatchAmount(uint256 newMax) external onlyOwner {
    require(
      newMax <= MAX_TOKENS_PER_WALLET,
      "Batch max cannot exceed MAX_TOKENS"
    );
