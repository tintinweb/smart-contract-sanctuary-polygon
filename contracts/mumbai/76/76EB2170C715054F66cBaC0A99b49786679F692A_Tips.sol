...
contract Tips is ReentrancyGuard {
    ...
    constructor(address _kutils, uint256 _maxItemsReturn, uint256 _maxItemsPerBucket, uint256 _cut) {
    ...
    */
    function getTippersFromMsgID(uint256 msgID, uint256 startFrom) public view returns(string[2][] memory) {
    ...
    function addTipInt(uint256 msgID, address tippedBy, uint256 tips) internal {
        // Stringify the ID
        string memory msgIDStr = KUtils.toString(msgID);
    ...
    function addTaggedTips(address[] memory taggedAccounts, uint256 tipPerTag) public payable {
    ...
    function checkTips(address addr) public view returns (uint256){
        return tipsOwed[addr];
    }

    function claimTips(uint256 groupID) public nonReentrant {
    ...
    function addTipFromPost(uint256 msgID) public payable nonReentrant {
