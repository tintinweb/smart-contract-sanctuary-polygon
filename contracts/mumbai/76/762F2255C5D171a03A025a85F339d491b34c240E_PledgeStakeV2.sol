...
contract PledgeStakeV2 {
    ...
    function _calRoundRewards(address _user, uint256 _amount) internal view returns(uint256) {
        StakeInfo storage userStakeInfo = stakeInfo[_user];
        if (_amount == 0 || userStakeInfo.startTime == 0) {
        return 0;
        }
            uint256 round = ((block.timestamp - userStakeInfo.startTime) / roundDuration) + 1;
        if (round > userStakeInfo.lastRound && round <= maxNumOfRounds) {
            return _amount.mul(roundRewardPercentages[round]).div(100);
        }
    return 0;
    }
    ...
    function _updateRewards(address _user, uint256 _amount) private {
        ...
        while (upline != address(0) && userInfo[upline].totalReferrals >= 2 && upline != defaultReferrer) {
            uint256 level = userInfo[upline].level;
            uint256 levelReward = roundRewards.mul(levelPercentage[level - 1]).div(100);
    ...
    function withdraw() external {
        uint256 withdrawable = rewards[msg.sender].availableRewards;
        ...
        require(withdrawable <= rewardLimit, "Exceeded reward limit");
         rewards[msg.sender].withdrawnRewards = rewards[msg.sender].withdrawnRewards.add(withdrawable);
