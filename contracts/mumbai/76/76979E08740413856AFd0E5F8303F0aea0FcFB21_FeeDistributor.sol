...
interface FeeVaultInterface {
    function claim(address sender,uint256 amount,bool reSave) external; 
}
    ...
    Ownable
{
    ...
    uint256 public  WEEK = 7 * 600;
    uint256 public  TOKEN_CHECKPOINT_DEADLINE = 600;
    ...
    constructor(address _veDefedAddress,address _mainFeeToken,address _feeVault) public{
     ...
     function setTokenDeadline(uint256 _deadline) external onlyOwner{
        TOKEN_CHECKPOINT_DEADLINE = _deadline;
        WEEK = 7 * TOKEN_CHECKPOINT_DEADLINE;
    }


     function setVeDefedAddress(address _veDefedAddress) external onlyOwner{
        veDEFED = IVeDefed(_veDefedAddress);
    }
    ...
    function _checkpointDistribute() internal {
        uint256 toDistribute = IERC20(mainFeeToken).balanceOf(
            feeVault
        );
    ...
    */
     ...
     */
    function claim(bool reSave) external  nonReentrant returns (uint256) {
        ...
        if (amount != 0) {
            ...
            FeeVaultInterface(feeVault).claim(_sender,amount,reSave);
