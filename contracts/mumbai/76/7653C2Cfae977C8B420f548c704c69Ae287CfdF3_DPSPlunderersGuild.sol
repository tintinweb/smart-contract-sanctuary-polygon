...
contract DPSPlunderersGuild is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    function claimLockedRewards(CausalityParams memory causalityParams) external nonReentrant {
        ...
        if (userLockedChests[0].lockedBlock + gameSettings.getBlockJumps() + 1 != block.number) revert WrongState(2);
    ...
    function claimLockedBox(uint256 _amount, CausalityParams memory causalityParams) external nonReentrant {
        ...
        if ((lockedLockBox.lockedBlock + gameSettings.getBlockJumps()) + 1 != block.number) revert WrongState(2);
