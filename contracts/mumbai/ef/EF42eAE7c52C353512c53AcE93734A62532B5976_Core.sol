...
import "../proxy/ProxiableRoles.sol";
import "../utils/Multicall.sol";

abstract contract BaseModule is IModule, ProxiableRoles, Multicall {
    function multicall(
        bytes[] memory data
    ) public virtual override(IModule, Multicall) returns (bytes[] memory results) {
...
/// @title Core contract for the mini DAO
/// @author 0xR360
/// @dev Core smart contract for mini daos, creates, stores and executes votes.
/// based on openzeppelin GovernorUpgradeable.sol, GovernorCountingSimpleUpgradeable.sol, GovernorSettingsUpgradeable.sol
/// @notice Core stores main logic related to the mini daos: lifetime, ownable, voting, quorum, proposal threshold, etc.
contract Core is
    ICore,
    BaseModule,
    GovernorUpgradeable,
    ...
    GovernorVotesQuorumFractionUpgradeable
{
    ...
    /**
     * @dev IRegistry contract, it will store the asset recipient address
     */
    IRegistry public registry;

    /**
     * @dev IBaseWallet contract
     */
    IBaseWallet public baseWallet;

    /**
     * @dev limit lifetime of the dao
     */
    uint256 public lifetime;

    modifier aliveStatus(bool shouldBeAlive) {
        if ((shouldBeAlive && block.timestamp > lifetime)) revert LifetimeReached();
        if ((!shouldBeAlive && block.timestamp <= lifetime)) revert LifetimeNotReached();
        _;
    }
    ...
    function initialize(
        InitialSettings memory settings,
        IVotesUpgradeable initToken,
        IBaseWallet _baseWallet,
        IRegistry _registry
    ) public initializer {
        __ProxiableRoles();
        __GovernorCountingSimple_init();
        __GovernorVotes_init_unchained(initToken);
        __Governor_init(settings.name);
        __GovernorSettings_init(settings.votingDelay, settings.votingPeriod, settings.proposalThreshold);
        __GovernorVotesQuorumFraction_init_unchained(settings.quorum);
    ...
    /**
     * @dev Create a new proposal. Vote start (votingDelay) blocks after the proposal is created and ends
     * (votingPeriod) blocks after the voting starts. Allowers must approve before the proposal can be created
     * @param targets The list of target addresses for calls to be made.
     * @param values The list of values (i.e. msg.value) to be passed to the calls to be made.
     * @param calldatas The list of function signatures and arguments to be passed to the calls to be made.
     * @param description The description of the proposal. Used for record keeping purposes.
     *
     * Emits a {ProposalCreated} event.
     */
    function propose(
        ...
        bytes[] memory calldatas,
        string memory description
    ) public virtual override aliveStatus(true) returns (uint256) {
        return super.propose(targets, values, calldatas, description); //proposal validation
    }
    ...
    ) public virtual aliveStatus(true) returns (uint256[] memory) {
        uint256[] memory proposalIds = new uint256[](targets.length);
        for (uint256 i = 0; i < targets.length; i++) {
            proposalIds[i] = propose(targets[i], values[i], calldatas[i], descriptions[i]);
        }
        return proposalIds;
    }

    function returnBatchOnExpiry(
        bytes32[] memory assetTypes,
        address[] memory targets,
        address[] memory receivers,
        uint256[] memory ids
    ) external aliveStatus(false) onlyRole(MANAGER_ROLE) {
        if (
            assetTypes.length != targets.length ||
            assetTypes.length != receivers.length ||
            assetTypes.length != ids.length
        ) revert InvalidInput();

        bytes32 erc20Type = keccak256("ERC20");
        bytes32 erc721Type = keccak256("ERC721");
        bytes32 erc1155Type = keccak256("ERC1155");

        for (uint8 i = 0; i < assetTypes.length; i++) {
            if (assetTypes[i] == erc20Type) {
                returnERC20OnExpiry(IERC20(targets[i]), receivers[i]);
            } else if (assetTypes[i] == erc721Type) {
                returnERC721OnExpiry(IERC721(targets[i]), receivers[i], ids[i]);
            } else if (assetTypes[i] == erc1155Type) {
                returnERC1155OnExpiry(IERC1155(targets[i]), receivers[i], ids[i]);
            }
        }
    }

    function returnERC1155OnExpiry(
        IERC1155 token,
        address receiver,
        uint256 id
    ) public aliveStatus(false) onlyRole(MANAGER_ROLE) {
        uint256 balance = token.balanceOf(address(this), id);
        token.safeTransferFrom(address(this), receiver, id, balance, "");
    }

    function returnERC721OnExpiry(
        IERC721 token,
        address receiver,
        uint256 id
    ) public aliveStatus(false) onlyRole(MANAGER_ROLE) {
        token.safeTransferFrom(address(this), receiver, id);
    }

    function returnERC20OnExpiry(IERC20 token, address receiver) public aliveStatus(false) onlyRole(MANAGER_ROLE) {
        uint256 balance = token.balanceOf(address(this));
        token.safeTransfer(receiver, balance);
    }

    /**
     * @dev Update the voting delay. This operation can only be performed by the owner. Emits a VotingDelaySet event.
     * @param newVotingDelay New voting delay
     */
    function setVotingDelay(uint256 newVotingDelay) public virtual override onlyRole(MANAGER_ROLE) {
        _setVotingDelay(newVotingDelay);
    }

    /**
     * @dev Update the voting period. This operation can only be performed by the owner. Emits a VotingPeriodSet event.
     * @param newVotingPeriod New voting period
     */
    function setVotingPeriod(uint256 newVotingPeriod) public virtual override onlyRole(MANAGER_ROLE) {
        _setVotingPeriod(newVotingPeriod);
    }

    /**
     * @dev Update the proposal threshold. This operation can only be performed by the owner. Emits a {ProposalThresholdSet} event.
     * @param newProposalThreshold New proposal threshold
     */
    function setProposalThreshold(uint256 newProposalThreshold) public virtual override onlyRole(MANAGER_ROLE) {
        _setProposalThreshold(newProposalThreshold);
    }
     ...
     */
    function updateQuorumNumerator(uint256 newQuorumNumerator) external virtual override onlyRole(MANAGER_ROLE) {
        _updateQuorumNumerator(newQuorumNumerator);
    }
    ...
    function _execute(
        ...
        for (uint256 i = 0; i < targets.length; ++i) {
            bool success = baseWallet.execFromModule(
                targets[i],
                values[i],
                calldatas[i],
                IBaseWallet.Operation.Execute
            );
            if (!success) revert TransactionFailed(targets[i], values[i], calldatas[i]);
        }
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(AccessControlEnumerable, GovernorUpgradeable) returns (bool) {
        return super.supportsInterface(interfaceId);
    }
}
...
contract ProxiableRoles is Initializable, UUPSUpgradeable, AccessControlEnumerable {
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");

    function __ProxiableRoles() public initializer {
