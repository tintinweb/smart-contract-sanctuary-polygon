...
contract NftContract is Ownable, ERC721A, ReentrancyGuard {
  ...
  function PublicMint(uint256 _mintAmount) public payable callerIsUser {
    uint256 _saleStartTime = uint256(saleConfig.SaleStartTime);
    ...
    require(_mintAmount <= maxBatchSize, "Max NFT's Per Address exceeded");
    uint256 price = saleConfig.MintPrice * _mintAmount;
    _safeMint(msg.sender, _mintAmount);
    refundIfOver(price);

  }
  function WhitelistMint(uint256 _mintAmount) external payable callerIsUser {
    uint256 price = uint256(saleConfig.MintPrice);
  ...
  function setMintPrice(uint16 price) external onlyOwner {
