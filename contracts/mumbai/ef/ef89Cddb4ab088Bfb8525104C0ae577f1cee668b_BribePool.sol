...
contract BribePool is Initializable, ReentrancyGuard, BribePoolConf, IFeeDistributor {
    ...
    function initialize(
        ...
        potExpirationInDays = _potExpirationInDays;
    }
    ...
    function deposit(
        ...
        if (potState == PotState.NOT_CREATED) {
            ...
            pot.expiry = block.timestamp + potExpirationInDays * 1 days;
        }
    ...
    /// @param optionIndex index/number of the bribe pot for that proposal
    function cancelInvalidPot(bytes32 proposalId, uint256 optionIndex) external onlyOwner {
...
abstract contract BribePoolConf is IBribePool, OwnableUpgradeable {
    ...
    /// @dev pot expiry in days
    uint256 public potExpirationInDays;
    ...
    event SetPotExpiration(uint256 potExpirationInDays);
    ...
    function setPotExpiration(uint256 _potExpiration) external onlyOwner {
        potExpirationInDays = _potExpiration;
        emit SetPotExpiration(_potExpiration);
    }
