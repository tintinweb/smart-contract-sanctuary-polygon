...
contract RacinoCars is ERC721AEnumerable, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;

    uint256 public maxSalePlusOne = 10001;

    uint256 public tokenPriceUSD = 500000000; //USD price has 8 decimals (5$ = 500000000)
    ...
    uint256 public txLimitPlusOne = 10;
    uint256 public allowancePlusOne = 10;
    ...
    address public crossmintAddress = 0xDa30ee0788276c093e686780C25f6C9431027234; // Devnet: 0xDa30ee0788276c093e686780C25f6C9431027234 Mainnet: 0x12A80DAEaf8E7D646c4adfc4B107A2f1414E2002

    constructor() ERC721AEnumerable("Racino Cars", "RC") {
        priceFeed = AggregatorV3Interface(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada); // Devnet: 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada Mainnet: 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0

        // VEXT
        //erc20Payments[contractAddress] = ERC20Payment(contractAddress, 5 * IERC20Metadata(contractAddress).decimals(), true, block.timestamp);

        // 10% royalties
        _setRoyalties(owner(), 10000);
    }
    ...
    function getPriceEth() public view returns(uint256) {
          ...
          return tokenPriceUSD / uint256(price);
    }
     ...
     */
    function mintPublic(uint256 quantity, address to)
        external
        payable
        isContractState(ContractState.PUBLIC)
        withinMintLimit(quantity)
        correctValue(getPriceEth() * quantity)
    {
        require(_numberMinted(to) + quantity < allowancePlusOne, "Exceeds allowance");
    ...
    /**
     * Mint tokens when on the allowlist
     ...
     */
    function mintPrivate(uint256 quantity, address to, bytes32[] calldata proof, bytes32 leaf)
        external
        payable
        isContractState(ContractState.PRIVATE)
        withinMintLimit(quantity)
        correctValue(getPriceEth() * quantity)
    {
        require(_numberMinted(to) + quantity < allowancePlusOne, "Exceeds allowance");
    ...
    /**
     * Public mint using ERC20 Token
     * @param quantity Amount of tokens to mint.
     */
    function mintPublicERC20(uint256 quantity, address to, address contractAddress)
        external
        isContractState(ContractState.PUBLIC)
        withinMintLimit(quantity)
    {
        require(erc20Payments[contractAddress].approved, "ERC20 Token not approved!");
        require(_numberMinted(to) + quantity < allowancePlusOne, "Exceeds allowance");
        require(quantity < txLimitPlusOne, "Exceeds transaction limit");

        IERC20Metadata(contractAddress).transferFrom(msg.sender, address(this), erc20Payments[contractAddress].tokenPrice * quantity);
        _safeMint(to, quantity);
    }

    /**
     * Mint tokens when on the allowlist using ERC20 Token
     * @notice This function is only available to those on the allowlist.
     * @param quantity The number of tokens to mint.
     * @param proof The Merkle proof used to validate the leaf is in the root.
     */
    function mintPrivateERC20(uint256 quantity, address to, address contractAddress, bytes32[] calldata proof, bytes32 leaf)
        external
        isContractState(ContractState.PRIVATE)
        withinMintLimit(quantity)
    {
    ...
    /**
     * Mint function for crossmint
     * @param quantity Amount of tokens to mint.
     */
    function crossmint(uint256 quantity, address to)
        external
        payable
        withinMintLimit(quantity)
    {
        require(msg.sender == crossmintAddress, "Not Crossmint!");
        _safeMint(to, quantity);
    }
    ...
    function setContractState(uint256 contractState_) external onlyOwner {
        require(contractState_ < 3, "Invalid State!");
        
        if (contractState_ == 0) {
            contractState = ContractState.OFF;
        }
        else if (contractState_ == 1) {
            contractState = ContractState.PRIVATE;
        }
        else {
    ...
    /**
     * Create an ERC20 Payment
     */
    function addERC20Payment(address contractAddress, uint256 tokenPrice) public onlyOwner {
        require(!erc20Payments[contractAddress].approved, "ERC20 Payment already added!");
        require(tokenPrice > 0, "Invalid Price!");

        erc20Payments[contractAddress] = ERC20Payment(contractAddress, tokenPrice * IERC20Metadata(contractAddress).decimals(), true, block.timestamp);
    }

    /**
     * remove an ERC20 Payment
     */
    function removeERC20Payment(address contractAddress) public onlyOwner {
        require(erc20Payments[contractAddress].approved, "ERC20 Payment not added!");

        erc20Payments[contractAddress].approved = false;
        erc20Payments[contractAddress].lastUpdated = block.timestamp;
    }
    ...
    /**
     * Update token price USD.
     * @param tokenPrice_ The new token price
     */
    function setTokenPriceUSD(uint256 tokenPrice_) external onlyOwner {
     ...
     */
    function setAllowancePlusOne(uint256 newAllowance) external onlyOwner {
        allowancePlusOne = newAllowance + 1;
    }
    ...
    /**
     * Withdraw contract funds (ERC20) to a given address.
     * @param account The account to withdraw to.
     * @param amount The amount to withdraw.
     */
    function ownerWithdrawERC20(address contractAddress , address account, uint256 amount) external virtual onlyOwner {
        IERC20(contractAddress).transfer(account, amount * IERC20Metadata(contractAddress).decimals());
    }
