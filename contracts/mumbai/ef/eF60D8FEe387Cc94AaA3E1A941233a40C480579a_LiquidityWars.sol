...
error LiquidityVault__MsgSenderNotInGame();
...
contract LiquidityWars is AutomationCompatibleInterface, Ownable {
    ...
    function upgradeBuilding(Infrastructure _building) external onlyRunning {
        if (playerParams[msg.sender].numberOfLpTokens == 0) {
            revert LiquidityVault__MsgSenderNotInGame();
        }
        PlayerParams storage _player = playerParams[msg.sender];
    ...
    function getRewardsToClaim() external view returns (uint256) {
        return 0; // TODO
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        if (
            checkOption == 0 && // Upkeep data for resource distribution
            gameState == GameState.RUNNING &&
            ((block.timestamp - lastDistributed) > distributionInterval)
            // TODO if subscription has enough link
        ) {
            upkeepNeeded = true;
...
contract LiquidityVault is Ownable, AutomationCompatibleInterface {
    ...
    uint256 MINIMUM_LINK_AMOUNT = 10 * (10**18); //10 LINK
    ...
    /* Variables - TODO make all public variables private/internal and create getters */

    LiquidityWarsConfig configContract;
    LiquidityWars liquidityWarsContract;
    ...
    function depositLpToken(address _tokenAddress, uint256 _amount)
        ...
        onlyReady
    {
        uint256 expectedAmount = configContract.getUsdRequiredAmount();
        console.log(
    ...
    // TODO move functionality to liquidityWars
    // function _hash(address _playerAddress) private view returns (uint256) {
    //     //TODO: check if there is not repetition
    //     //for (uint256 i = 0; i < MAX_NUMBER_OF_PLAYERS; i++) {
    //     //uint256 trial = (i + _playerInfo.id) % MAX_NUMBER_OF_PLAYERS;
    //     return
    //         uint256(
    //             keccak256(abi.encodePacked(_playerAddress, block.timestamp))
    ...
    function claimRewardTokensFromProtocol()
        ...
        returns (uint256)
    {
        uint256 ratio = liquidityWarsContract.getRatioOfResources(
            address(this)
        ); // expected ((10**10 * vaultResources) / totalSupplyOfResources);
        uint256 rewardsToClaim = (totalSupplyOfRewards * ratio) / PRECISION; //10**10 precision
        ...
        rewardTokenToClaim.transfer(
            owner(), // Temporary solution (should be multisig vault)
            rewardsToClaim
        );
    ...
    function distributeRewards(address _token) public onlyContractOrOwner {
        address[] memory _players = players;
    ...
    function getPlayerAddress(uint256 _id)
        external
        view
        onlyRunning
        returns (address)
     ...
     * () The game should have at least 2 players and have some LP tokens - in order to start the game !!
     * 2. Our subscription should be funded with LINK
     * 3. The game should be in "RUNNING" state
     * II.The following should be true for ending the game:
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        // Prevents from calling by other actor
        if (
            (abi.decode(data, (uint256)) !=
                abi.decode(performData, (uint256))) || (!upkeepNeeded)
        ) {
    ...
    // function findPlayerInHashTable(address _playerAddress)
    //     public
    //     view
    //     returns (PlayerParams memory)
    // {
    //     return players[0]; // TODO: get player using available mapping
    // }
}
...
contract LiquidityWarsConfig is Ownable {
        ...
        returns (uint256) 
    {
        ...
        uint totalSupply = IERC20(_tokenAddress).totalSupply();
        ...
        uint256 reserve0USD = reserve0 * uint256(price0) * uint256(price1) / 1e26;
        uint256 reserve1USD = reserve1 * uint256(price1) / 1e18;
        ...
        // console.log("usdRequiredAmount: ", usdRequiredAmount);
        
        uint256 amountOfLpTokensRequired = 1e16*usdRequiredAmount/priceLP;
        // console.log("amountOfLpTokensRequired:", amountOfLpTokensRequired, "(8 dec)");
