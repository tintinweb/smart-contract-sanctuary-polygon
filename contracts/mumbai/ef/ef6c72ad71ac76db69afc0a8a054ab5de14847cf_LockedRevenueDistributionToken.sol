...
{
    uint256 public constant lockTime = 365 * 86400; // 1 year
    uint256 public earlyWithdrawalFee;

    mapping(address => WithdrawalRequest) public withdrawalRequests;
    mapping(address => bool) public earlyWithdrawalFeeExemptions;
    ...
    function createWithdrawalRequest() external override {
        WithdrawalRequest memory _request = WithdrawalRequest(
            ...
            balanceOfAssets(msg.sender)
        );
        withdrawalRequests[msg.sender] = _request;

        emit WithdrawalRequestCreated(msg.sender, _request);
    }

    function exerciseWithdrawalRequest() external override nonReentrant {
        WithdrawalRequest memory request = withdrawalRequests[msg.sender];
        require(
            block.timestamp >= request.unlockDate,
            "LRDT:WITHDRAWAL_NOT_UNLOCKED"
        );
        ...
        emit WithdrawalRequestExcercised(owner, request);
    }

    function exemptWithdrawalFeeFor(address addr) external override {
        require(msg.sender == owner, "LRDT:CALLER_NOT_OWNER");
        earlyWithdrawalFeeExemptions[addr] = true;

        emit WithdrawalFeeExempted(addr);
    }

    /// No more than possible output of assets on actual redeem
    ...
    {
        uint256 _earlyWithdrawalFee = earlyWithdrawalFeeExemptions[msg.sender]
            ? 0
            : earlyWithdrawalFee;
        _assets = super.previewRedeem(_shares);
        _assets = _divRoundUp(((100 - _earlyWithdrawalFee) * _assets), 100);
    }

    /// No fewer than possible input of shares on actual withdraw
        ...
        returns (uint256 _shares)
    {
        uint256 _earlyWithdrawalFee = earlyWithdrawalFeeExemptions[msg.sender]
            ? 0
            : earlyWithdrawalFee;
        /// @dev `_divRoundUp` not needed here as it's called in parent `previewWithdraw`
        _assets = (_assets * 100) / (100 - _earlyWithdrawalFee);
        _shares = super.previewWithdraw(_assets);
    }
}
...
interface ILockedRevenueDistributionToken {
    ...
    event WithdrawalRequestCreated(address user, WithdrawalRequest request);

    event WithdrawalRequestExcercised(address user, WithdrawalRequest request);

    event WithdrawalFeeExempted(address addr);

    function createWithdrawalRequest() external;

    function exerciseWithdrawalRequest() external;

    function exemptWithdrawalFeeFor(address addr) external;
}
