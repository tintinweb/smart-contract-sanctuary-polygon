...
*/
    ...
    function msend(
        ...
        if (cid == _dstChainId) this.mreceive(cid, params);
...
abstract contract MasterMessageHandler is IMaster, MasterModifiers, MasterEvents {
    ...
    function redeemAllowed(
        ...
        (, uint256 shortfall) = _getHypotheticalAccountLiquidity(
            ...
            params.pToken,
            params.amount,
            AccLiquidityOperation.WITHDRAW
        );
        //if approved, update the balance and fire off a return message
        // slither-disable-next-line incorrect-equality
        if (shortfall == 0) {
...
abstract contract IMaster is MasterStorage {
    ...
    function _getHypotheticalAccountLiquidity(
        ...
        uint256 amount,
        AccLiquidityOperation operation
    ) internal view virtual returns (uint256, uint256);

    function _getAccountCollateral(
        address account, 
        address pTokenModify, 
...
abstract contract MasterStorage {
    ...
    */
    ...
    enum AccLiquidityOperation{ DEPOSIT, WITHDRAW, BORROW, REPAY, LIQUIDATE, TOTAL, SIMDEPOSIT, SIMWITHDRAW, SIMBORROW, SIMREPAY }
}
...
interface ICRM {
    ...
    function setAbsMaxLtvRatios(uint256[] memory chainIds, address[] memory markets, uint256[] memory _maxLtvRatios) external;
    function setCollateralRatioModels(uint256[] memory chainIds, address[] memory markets, ICRM[] memory collateralRatioModels) external;
    function getCollateralRatioModel(uint256 chainId, address asset) external returns (address model);

    //TODO: put in separate interface
    function getCurrentMaxLtvRatios(uint256[] memory chainIds, address[] memory assets) external view returns (uint256[] memory ratios);
