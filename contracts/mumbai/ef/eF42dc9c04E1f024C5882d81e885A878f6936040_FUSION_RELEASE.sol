...
contract FUSION_RELEASE is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function doFusion(uint256 amount_) private returns (uint256 useAmount_,uint256 getQuantity_){
        ...
        uint256 seq = _seq;
        while(quantity > 0) {
            FusionSeq memory fusionItem = _fusionList[seq];
            if(fusionItem.balance > quantity){
                amount = amount.add(quantity.mul(fusionItem.price).div(_moffset));
            ...
            if(fusionItem.balance < quantity) {
                ...
                amount = amount.add(fusionItem.balance.mul(fusionItem.price).div(_moffset));
                quantity = quantity.sub(fusionItem.balance);
                _fusionList[seq].balance = 0;
                emit FusionSeqBalanceChangeEvent(seq, 0);
                seq = seq.add(1);
            }
        }
        require(useAmount_ ==  amount,"amount error");
        _seq = seq;
        ...
        _usdt.safeTransferFrom(_msgSender(), _collect, useAmount_);
    }
