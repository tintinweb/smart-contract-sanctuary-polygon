...
contract GiddyChef is Ownable {
    ...
    uint256 public immutable giddyPerSecond;
    ...
    function pendingGiddy(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 oxdReward = multiplier.mul(giddyPerSecond).mul(pool.allocPoint).div(totalAllocPoint);
            accGiddyPerShare = accGiddyPerShare.add(oxdReward.mul(1e12).div(lpSupply));
        }
    ...
    // Safe Giddy transfer function, just in case if rounding error causes pool to not have enough Giddy.
    function safeGiddyTransfer(address _to, uint256 _amount) internal {
        uint256 giddyBal = giddy.balanceOf(address(this));
        if (_amount > giddyBal) {
            giddy.transfer(_to, giddyBal);
        } else {
