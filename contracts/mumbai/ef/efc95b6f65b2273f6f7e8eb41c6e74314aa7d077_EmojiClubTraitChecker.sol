...
contract EmojiClubTraitChecker is Ownable, ITraitChecker {
    string public constant TRAIT_FAMILY = "Emoji Club";
    ...
    function getTrait(string calldata _username) external view returns (string memory, string memory) {
        ...
        for (uint i = 0; i < b.length;) {
            (, uint8 size) = UnicodeLib.readUnicodeDecimal(b, i);
        ...
        if(l == 1) return (TRAIT_FAMILY,"1 Emoji");
        if(l == 2) return (TRAIT_FAMILY,"2 Emoji");
        if(l == 3) return (TRAIT_FAMILY,"3 Emoji");
        if(l == 4) return (TRAIT_FAMILY,"4 Emoji");
        if(l == 5) return (TRAIT_FAMILY,"5 Emoji");
