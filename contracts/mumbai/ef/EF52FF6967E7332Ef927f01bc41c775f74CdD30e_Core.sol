...
error Core_InvalidAddress();
error Core__OnlyOwnerMethod();
error Core__TokenNotUsed();
error Core__AmountIsInvalid();
error Core__TransferFailed();
error Core__WithdrawAmtGreaterThanBalance();
error Core_InvalidOperator();
...
contract Core is ERC2771Recipient, ReentrancyGuard {
    /* State Variables */
    ...
    /* Events */
    event TransferCoreOwnership(address indexed oldOwner, address indexed newOwner);
    ...
    constructor(address owner) {
        s_coreOwner = owner;
        emit TransferCoreOwnership(address(0), s_coreOwner);
    }

    modifier onlyCoreOwner() {
        if (_msgSender() != s_coreOwner) {
    ...
    function transferCoreOwnership(address _newOwner) external onlyCoreOwner {
        _transferCoreOwnership(_newOwner);
    }

    function _transferCoreOwnership(address _newOwner) internal {
        if (_newOwner == address(0)) {
            revert Core_InvalidAddress();
        }
        address oldOwner = s_coreOwner;
        s_coreOwner = _newOwner;
        emit TransferCoreOwnership(oldOwner, _newOwner);
    }
    ...
    function handleBalance(
        ...
        uint256 amount,
        uint256 operator
    ) external onlyCoreOwner isTokenValid(token) {
        if (operator == 1) {
    ...
    function getBalancedStatus(address token) public view isTokenValid(token) returns (bool) {
        ...
        return contractBal == s_totalUserFunds[token] + s_totalStaked[token] + s_totalHL[token] + s_totalPayout[token];
    }
}
////