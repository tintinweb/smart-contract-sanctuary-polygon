...
import "./interfaces/INFTPussies.sol";
import "./IPrices.sol";
...
interface ITwapGetter {
    ...
    function getMaticPriceInUSDC() external view returns (uint256);
}

contract NFTPMysteryBox is ERC1155, ReentrancyGuard, AccessControl {
    ...
    */
    ...
    function _isPhotoCountSameOrHigher(
        uint256 id,
        uint16[] memory common,
        uint16[] memory rare,
        uint16[] memory legendary,
        ...
        require(commonM.length >= common.length && rareM.length >= rare.length && legendaryM.length >= legendary.length && pussM.length >= puss.length, "Cannot update collection with rarity array larger than it was before.");

        for(uint256 i = 0; i < common.length; i++) {
            result = result && (common[i] >= commonM[i] && rare[i] >= rareM[i] && legendary[i] >= legendaryM[i] && puss[i] >= pussM[i]);
        }
    ...
    function updateCollection(
        ...
        require(common.length == rare.length && rare.length == legendary.length && legendary.length == puss.length, "Array lenght mismatch");
        require(_isPhotoCountSameOrHigher(id, common, rare, legendary, puss), "New photo count lower than minted");
    ...
    */
    function setState(uint256 id, bool state) external onlyRole(DEFAULT_ADMIN_ROLE) {
        MysteryBox.Collection storage c = collections[id];
        c.setState(state);
    ...
    */
    function setSales(uint256[] memory amounts, uint256[] memory sales) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    * @param newuri new contract metadata uri */
    function setContractMetadataURI(string memory newuri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    function setNftp(address nftp_) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    */
    function setPaidWith(address token) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(token != address(0), "Cannot be 0");
        paidWith = IERC20(token);
    }
    ...
    * @param twapAddress_ prices contract address
    */
    function setTwapGetter(address twapAddress_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(twapAddress_ != address(0), "Cannot be 0");
        twapGetter = ITwapGetter(twapAddress_);
    }
    ...
    */
    function revealFulfill(uint256 counter, uint256 seed) external onlyRole(ADMIN_ROLE) {
    ...
    * @param amount Amount to be minted
    */
    function authorizedMint(address to, uint256 id, uint256 amount) external payable onlyRole(MINTER_ROLE) {
        MysteryBox.Collection storage c = collections[id];
        // mapping address -> (mapping collection -> count) += 1
        _mint(to, c, id, amount);
        emit BoxMint(id, amount, 0, to, false);
    }

    /**
    * @notice Allows admin to request a reveal for an address
    * @param _id collection id
    * @param _amount number of boxes to reveal
    * @param _addr address of a wallet for which is reveal intended
    */
    function authorizedReveal(uint256 _id, uint256 _amount, address _addr) external onlyRole(ADMIN_ROLE) {
        require(balanceOf(_addr, _id) >= _amount, "Not enough MysteryBoxes");
        require(_amount <= maxReveal, "Amount too high");
        MysteryBox.Collection storage c = collections[_id];
        require(c.revealable(), "Cannot be revealed yet");
        require(revealRequested[_addr] == 0, "Account already requested");

        revealRequests[requestCounterEnd] = RevealRequestInfo(_addr, _amount, _id, requestCounterEnd);
        revealRequested[_addr] = requestCounterEnd;
        requestCounterEnd++;
        emit RevealRequest(_addr, _id, _amount, requestCounterEnd, block.timestamp);
    }
    ...
    */
    function getPrice(uint256 id, uint256 amount) external view returns(uint256) {
        return _getPrice(id, amount);
    }

    /**
     * price for swipelux in MATIC
     * @dev (10**20) because 18 is for swipelux and 2 is precision from twapGetter
     */
    function getPriceBNB(uint256 id, uint256 amount) external view returns(uint256) {
        MysteryBox.Collection storage c = collections[id];
        uint256 priceMaticInUSD = twapGetter.getMaticPriceInUSDC();
        return ((c.price * amount) * (10 ** 20)) / priceMaticInUSD;
    }
    ...
    * @return uint256 cost of mint
    */
    function _getPrice(uint256 id, uint256 amount) internal view returns(uint256) {
        MysteryBox.Collection storage c = collections[id];
