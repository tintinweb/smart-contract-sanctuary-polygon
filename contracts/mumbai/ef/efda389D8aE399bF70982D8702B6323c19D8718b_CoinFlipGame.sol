...
contract CoinFlipGame is Ownable, ReentrancyGuard {
    ...
    uint256 private constant MAX_BET_AMOUNT = 10 * 1e18;
    uint256 private nonce;

    IERC20 private token;

    event CoinFlipResult(address indexed player, uint256 betAmount, uint8 guess, bool win);
    ...
    function random() private view returns (bool) {
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty))) % 10;
        return (randomNumber <= 2);
    }

    function placeBet(uint8 guess) external nonReentrant {
        require(guess == 0 || guess == 1, "Invalid guess, must be 0 (heads) or 1 (tails)");
        token.safeTransferFrom(msg.sender, address(this), MAX_BET_AMOUNT);

        bool flipResult = random();
        bool win = (flipResult == (guess == 1));
        ...
        if (win) {
            ...
            require(token.balanceOf(address(this)) >= burnAmount, "Not enough funds in the contract to burn tokens");
            ERC20Burnable(address(token)).burn(burnAmount);
        }

        emit CoinFlipResult(msg.sender, MAX_BET_AMOUNT, guess, win);
    }
