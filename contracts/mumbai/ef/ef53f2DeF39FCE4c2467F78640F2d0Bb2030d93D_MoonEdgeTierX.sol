...
contract MoonEdgeTierX is VRFConsumerBase, AccessControl {
    ...
    mapping(uint16 =>  mapping(uint16 => bool)) public _idoWinnersPreBoolByDrawId;  
    ...
    function finalizeRandomNess(uint16 idoIDToFinalize) public {
        ...
        for (uint16 i = 0; i < _idoWinnersToDraw[idoIDToFinalize]; i++) {
            ...
            _idoWinnersParticipantsIndex[idoIDToFinalize][i] = (_idoDrawValuesRaw[idoIDToFinalize][i] % _idoTotalNumberOfTickets[idoIDToFinalize] );
            ...
            _idoWinnersPreBoolByDrawId[idoIDToFinalize][i] = _idoWinnersBool[idoIDToFinalize][ currAddress ];

            // try to find a new winner
            if(_idoWinnersBool[idoIDToFinalize][ currAddress ]){
            //if(_idoWinnersPreBoolByDrawId[idoIDToFinalize][i]){
                for(uint16 e=0; e<100; e++){
                    ...
                    if(_idoWinnersParticipantsIndex[idoIDToFinalize][i] >= _idoTotalNumberOfTickets[idoIDToFinalize]){
