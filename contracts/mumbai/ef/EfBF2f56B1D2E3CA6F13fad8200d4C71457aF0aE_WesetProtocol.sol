...
import "./interface/IWesetDrop.sol";
import "./lib/MerkleProof.sol";

abstract contract WesetDrop is IWesetDrop {
    /*///////////////////////////////////////////////////////////////
                                Mappings
    //////////////////////////////////////////////////////////////*/
    ...
    /// @dev Mapping from tokenId => active price Array for the tokenId.
    mapping(uint256 => uint256[]) public priceArray;

    // /// @dev Mapping from tokenId => final price Array for the tokenId.
    // mapping(uint256 => uint256) public priceId;
    ...
    function verifyClaim(
        ...
        uint256 _supplyClaimedByWallet = supplyClaimedByWallet[
            conditionId[_tokenId]
        ][_claimer];

        if (_currency != claimCurrency ) {
            revert("!Currency");
        }
    ...
    /// @dev allows admin to reset price array for a given token
    function setPrice(uint256 _tokenId, uint256[] calldata _prices) internal {
        priceArray[_tokenId] = _prices;
    }
    ...
    /// @dev Returns specific price for NFT given tokenId, and priceId
    function getPriceOfNFT(uint256 _tokenId, uint256 _priceId)
        public
        view
        returns (uint256)
    {
        return priceArray[_tokenId][_priceId];
    }
    ...
    /// @dev Weset hook to grab price from price array 
    function _getPriceOfNFT(uint256 _tokenId, uint256 _priceId)
        internal
        virtual
        returns (uint256)
    {
...
/**
 *  Designed by LyghtCode for Weset.io
 */

contract WesetProtocol is
    ERC1155,
    Royalty,
    ContractMetadata,
    WesetDrop,
    ...
    DefaultOperatorFilterer
{
    ...
    function setApprovalForAll(
        address operator,
        bool approved
    ) public virtual override(ERC1155) onlyAllowedOperatorApproval(operator) {
    ...
    /// @dev Collects and distributes the primary sale value of NFTs being claimed.
    /// @dev Weset.io - Below we will override the pay amount below to match the tokenId
    /// and price array with priceId
    function _collectPriceOnClaim(
        address _primarySaleRecipient,
        uint256 _quantityToClaim,
        address _currency,
        ...
        /// @dev - force price to 0 when initializing so don't cancel the txn
        /// if (_pricePerToken == 0) {
        ///     return;
        /// }
        ...
        uint256 totalPrice = _getPriceOfNFT(_tokenId, _priceId);
        
        ///@dev - check price and stablecoin address
        if (
            _currency == CurrencyTransferLib.NATIVE_TOKEN ||
            _currency == CurrencyTransferLib.NATIVE_TEST ||
            _currency == CurrencyTransferLib.USDC_MUMBAI ||
            _currency == CurrencyTransferLib.USDC_TEST ||
            _currency == CurrencyTransferLib.USDC ||
            _currency == CurrencyTransferLib.USDT ||
            _currency == CurrencyTransferLib.DAI
        ) {
...
/**
 *  The interface `IClaimCondition` is adapted from thirdweb's 'Drop' contracts, which are distribution mechanisms for tokens.
 *
...
interface IClaimCondition {
     ...
     *
     *  @param currency                       sThe currency in which the `pricePerToken` must be paid.
     *
...
/**
 *  The interface `IWesetDrop` is written for Weset.io's smart contracts, which are fractionalized tokenized properties .
 *
 *  An authorized wallet can set a price array for the distribution of the contract's tokens.
 *  A claim condition defines criteria under which accounts can mint tokens. Claim conditions can be overwritten
 *  or added to by the contract admin. At any moment, there is only one active claim condition.
 */

interface IWesetDrop is IClaimCondition {
        /**
     ...
     *  @param priceId                        The priceId of the price array to use for the claim.
     ...
     *
     *  @param prices                   The price array for the distribution of the contract's tokens.
     */
    function setClaimConditions(
        uint256 tokenId,
    ...
    // function setPrice(uint256 tokenId, uint256[] memory prices) external;

    function getPriceOfNFT(uint256 tokenId, uint256 priceId) external view returns (uint256);
}
...
library CurrencyTransferLib {
    ...
    address public constant NATIVE_TEST = 0x0000000000000000000000000000000000001010;
    /// @dev The address of accepted Stable Tokens Polygon Mainnet.
    address public constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    address public constant DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    /// @dev The address of accepted Stable Tokens Polygon Mumbai.
    address public constant USDC_MUMBAI = 0x0FA8781a83E46826621b3BC094Ea2A0212e71B23;
    address public constant USDC_TEST = 0xE097d6B3100777DC31B34dC2c58fB524C2e76921;
