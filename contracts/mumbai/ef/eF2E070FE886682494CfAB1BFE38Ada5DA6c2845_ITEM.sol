...
interface IITEM {
    function batchMint(address recipient_, uint256 quantity_) external;
}
    ...
    ERC721URIStorageUpgradeable
{
    ...
    function batchMint(address recipient_, uint256 quantity_) external onlyRole(MINTER_ROLE) {
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, tokenId, quantity_);
    ...
    function getCallData(uint256 quantity) external pure returns (bytes memory) {
        bytes memory callData = abi.encodeCall(IITEM.batchMint, (address(0), quantity));
        return callData;
    }
