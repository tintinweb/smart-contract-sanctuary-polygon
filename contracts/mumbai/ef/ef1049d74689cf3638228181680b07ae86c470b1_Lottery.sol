...
contract Lottery {
    ...
    function startRound(uint8 numbers, uint8 max, uint256 duration) public onlyOwner {
        ...
        require(numbers == payouts.length - 1, "There must be 1 more payout than numbers.");
    ...
    function buyTickets(uint8[][] calldata guess) public payable {
    ...
    function calculatePayouts() internal {
        for(uint i; i < rounds[currentRound].tickets.length; i++) {
            ...
            unclaimedPrizes[rounds[currentRound].tickets[i].player] += rounds[currentRound].tickets[i].prize;
        }
...
*/
    ...
    function claimPrizes() public returns(uint256) {
        uint256 temp = unclaimedPrizes[msg.sender];
    ...
    function withdrawDev() public {
        require(msg.sender == devAddress, "Only the dev can withdraw their ETH");
        payable(msg.sender).transfer(devEth);
        devEth = 0;
    }
    ...
    function withdrawFees() public onlyOwner {
        ...
        payable(owner).transfer(vrfEth);
        payable(teamAddress).transfer(teamEth);
