...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  function borrow(
    ...
    if(amount > 0) borrowerList[asset].addUser(msg.sender);
  }
  ...
  function repay(
    ...
    if (userBorrowBalance == 0) borrowerList[asset].removeUser(msg.sender);
    return repayAmount;
  }
  ...
  function liquidationCall(
    ...
    borrowerList[asset].removeUser(user);
  }

  /// @inheritdoc IPool
  function checkAndLiquidate(address asset) public virtual override {
    address[] storage borrowers = borrowerList[asset].list;
  ...
  /// @inheritdoc IPool
  function splitReduxInterest(address asset) public virtual override {
    ParticularLogic.executeSplitReduxInterest(
      _reserves,
      supplierList[asset],
      DataTypes.ExecuteSplitReduxInterestParams({
        asset: asset,
        reduxInterval: _reduxInterval,
        oracle: ADDRESSES_PROVIDER.getPriceOracle()
      })
    );
  }
  ...
  function setReduxInterval(uint256 reduxInterval) external virtual override onlyPoolConfigurator {
    _reduxInterval = uint40(reduxInterval);
  }
    ...
    returns(uint256)
  {
    ...
    uint256 rate = _reserves[supplyingAsset].supplyRate * WadRayMath.RAY;
    ...
    returns(uint256)
  {
    ...
    address prxyTreasury = ADDRESSES_PROVIDER.getPrxyTreasury();
    address prxyAsset = IPrxyTreasury(prxyTreasury).getPrxyToken();
  ...
  function getBorrowInterestRate(address supplyingAsset) 
    ...
    returns(uint256)
  {
    uint256 rate = _reserves[supplyingAsset].borrowRate * WadRayMath.RAY;
    rate /= SECONDS_PER_YEAR * GeneralLogic.RATE_PRECISION;
...
contract PoolStorage {
  ...
  uint40 internal _reduxInterval = 1 days;
...
library PoolLogic {
    ...
    )
  {
    ...
    uint256 debtAmount = IPToken(reserve.pTokenAddress).borrowBalanceOf(params.user);
    totalCollateralBase = GeneralLogic.getUserBalanceInBaseCurrency(reserve.supplyingAsset, collateralAmount, params.oracle);
    totalDebtBase = GeneralLogic.getUserBalanceInBaseCurrency(reserve.borrowingAsset, debtAmount, params.oracle);
...
library BorrowLogic {
  ...
  function executeRepay(
    ...
    uint256 userBorrowBalanceUpdated = IPToken(reserve.pTokenAddress).borrowBalanceOf(msg.sender);
...
library ParticularLogic {
  ...
  event ReduxInterestSplit(
    ...
    uint256 reduxInterestAmount,
    uint256 supplierInterestSplitAmount,
    uint256 treasurySplitAmount
  );
  ...
  function executeSplitReduxInterest(
    ...
    DataTypes.ReserveData storage reserve = reservesData[params.asset];
    (uint256 reduxAmount, uint256 elapsedTime) = IPToken(reserve.pTokenAddress).getReduxInterest();
    require(elapsedTime >= params.reduxInterval, Errors.EARLY_REDUX_SPLIT);

    if (reduxAmount == 0) {
      reserve.supplyRate = 0;
      IPToken(reserve.pTokenAddress).reduxInterestToTreasury(0, 0);
      return;
    }
    
    uint256 maxLtv = reserve.configuration.getLiquidationThreshold();
    uint256[] memory uncollateralInBaseCurrency = new uint256[](suppliers.length);
    uint256 totalUncollateralInBaseCurrency = 0;

    for (uint256 i = 0; i < suppliers.length; i++) {
      uncollateralInBaseCurrency[i] = _calcUncollateralInBaseCurrency(reserve, suppliers[i], params.oracle, maxLtv);
      totalUncollateralInBaseCurrency += uncollateralInBaseCurrency[i];
    }

    uint256 reduxInBaseCurrency = GeneralLogic.getUserBalanceInBaseCurrency(reserve.supplyingAsset, reduxAmount, params.oracle);
    reserve.supplyRate = reserve.splitRate * reduxInBaseCurrency * MathUtils.SECONDS_PER_YEAR / (totalUncollateralInBaseCurrency * elapsedTime);

    uint256 splitAmount = reduxAmount * reserve.splitRate / GeneralLogic.RATE_PRECISION;
    uint256 suppliersSplitAmount = 0;

    for (uint256 i = 0; i < suppliers.length; i++) {
      uint256 userRewardAmount = splitAmount * uncollateralInBaseCurrency[i] / totalUncollateralInBaseCurrency;
      suppliersSplitAmount += userRewardAmount;      
      _supplyReward(reserve, suppliers[i], userRewardAmount);
    }

    IPToken(reserve.pTokenAddress).reduxInterestToTreasury(reduxAmount, reduxAmount - suppliersSplitAmount);
    emit ReduxInterestSplit(params.asset, reduxAmount, suppliersSplitAmount, reduxAmount - suppliersSplitAmount);
  }
  ...
  function _calcUncollateralInBaseCurrency (
    ...
    uint256 userBorrowBalance = IPToken(reserve.pTokenAddress).borrowBalanceOf(user);
    uint256 userSupplyInBaseCurrency = GeneralLogic.getUserBalanceInBaseCurrency(reserve.supplyingAsset, userSupplyBalance, oracle);
    uint256 userCollateralInBaseCurrency = GeneralLogic.getUserBalanceInBaseCurrency(reserve.borrowingAsset, userBorrowBalance, oracle) * 10000 / maxLtv;
    require(userSupplyInBaseCurrency >= userCollateralInBaseCurrency, Errors.INVALID_USER_BALANCE);
    return userSupplyInBaseCurrency - userCollateralInBaseCurrency;
  }
...
library LiquidationLogic {
  ...
  function executeLiquidationCall(
    ...
    ValidationLogic.validateLiquidationCall(reserve, params.user, params.oracle);
  ...
  function executeLiquidateWhenReached(
    ...
    bool liquidated = ValidationLogic.hasReachedLiquidation(reserve, params.user, params.oracle);
  ...
  //  **/
  // function isLiquidationPossibleCall(
  //   mapping(address => DataTypes.ReserveData) storage reservesData,
  //   DataTypes.ExecuteLiquidationCallParams memory params
  // ) external returns (bool) {
  //   DataTypes.ReserveData storage reserve = reservesData[params.asset];
  ...
  //   return ValidationLogic.hasReachedLiquidation(reserve, params.user, params.oracle);
  // }
}
...
interface IPToken is IERC20Detailed, IInitializablePToken {
   ...
   * @param amount The redux amount
   * @param treasurySplit The amount to transfer to treasury
   **/
  function reduxInterestToTreasury(uint256 amount, uint256 treasurySplit) external;

  /**
   * @notice Get the redux interest amount and elapsed time
   * @return The redux interest amount and elapsed time
   **/
  function getReduxInterest() external returns(uint256, uint256);
...
library ValidationLogic {
    ...
    view
  {
    ...
    uint256 userBorrowInBaseCurrency = GeneralLogic.getUserBalanceInBaseCurrency(reserve.borrowingAsset, borrowBalance, oracle);
   ...
   * @param oracle The address of price oracle
   */
  function validateLiquidationCall(DataTypes.ReserveData storage reserve, address user, address oracle)
    internal
    view
  {
    (bool isActive, bool isFrozen, , bool isPaused) = reserve.configuration.getFlags();
    ...
    uint256 borrowBalance = IPToken(reserve.pTokenAddress).borrowBalanceOf(user);
   ...
   * @return liquidation is possible or not
   */
  function hasReachedLiquidation(DataTypes.ReserveData storage reserve, address user, address oracle) internal view returns (bool) {
    (bool isActive, bool isFrozen, , bool isPaused) = reserve.configuration.getFlags();
