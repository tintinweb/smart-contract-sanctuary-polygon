...
interface IBoleroDeployer {
    ...
    function artistTokens(address artistAddress) external view returns(address);
...
contract BoleroManager is AccessControl {
    ...
    function grantTokens(
        ...
        require(hasRole(GRANTER_ROLE, msg.sender) || hasRole(MANAGER_ROLE, msg.sender));
    ...
    function setShares(
        ...
        IBoleroArtistICO(contractAddress).setShares(_shareForBolero, _shareForArtist);
    }

    function claimTreasure(address contractAddress) public onlyRole(MANAGER_ROLE) {
    ...
    function setArtistPayment(
        ...
        IBoleroArtist(contractAddress).setArtistPayment(_artistPayment, _isPaymentSplitter);
    }
    ...
    * */
    ...
    function getArtistContractAddress(address artistAddress) public view returns (address) {
        return boleroDeployer.artists(artistAddress);
    }

    function getArtistTokenAddress(address artistAddress) public view returns (address) {
        return boleroDeployer.artistTokens(artistAddress);
    }

    function getArtistICOAddress(address artistAddress) public returns (address) {
        boleroArtistToken = IBoleroArtistToken(getArtistTokenAddress(artistAddress));
        return boleroArtistToken.icoContract();
    }


    function getPaymentAddress(address artistAddress) public returns (bool, address) {
        boleroArtist = IBoleroArtist(getArtistContractAddress(artistAddress));
        if (boleroArtist.isWithPaymentSplitter()) {
            address paymentSplitterAddress = boleroArtist.artistPayment();
            return (true, paymentSplitterAddress);
        } else {
            address paymentAddress = boleroArtist.artistPayment();
