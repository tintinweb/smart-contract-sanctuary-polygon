...
contract BreedPlanet is BreedPlanetBase, ChainlinkRng {
    ...
    constructor(
        ...
        ChainlinkRng(_chainlinkSubscriptionId)
    {}
    ...
    //         breedStruct.miniBlackHoleNumber
    //     );
    // }
        ...
        override
    {
        ...
        // BreedStruct memory breedStruct = BreedStructMap[requestId];
        // _breed(
        //     address(0x466a1069Af84278Fa6d12Fe3551E6E2438b13002),
        //     1,
        //     2,
        //     true,
        //     2
...
contract BreedPlanetBase is ApeironPlanetGenerator, AccessProtected {
        ...
        if (miniBlackHoleNumber != 0) {
            require(
                userAddress == address(this) ||
                    miniBlackHole.isApprovedForAll(userAddress, address(this)),
                "Grant miniBlackHole approval for breeding contract"
            );
            require(
                miniBlackHole.balanceOf(userAddress, miniBlackHoleId) >=
                    miniBlackHoleNumber,
    ...
    function _breed(
        ...
        if (
            ...
            if (miniBlackHoleNumber != 0) {
                uint256 targetMiniBlackHoleNumber = miniBlackHoleNumbers[
                    _getPlanetData(planetAId).breedCount
                ] + miniBlackHoleNumbers[_getPlanetData(planetBId).breedCount];
                require(
                    miniBlackHoleNumber >= targetMiniBlackHoleNumber,
                    "blackHole is not enough to breed"
                );
                miniBlackHole.burn(
                    userAddress,
                    miniBlackHoleId,
                    targetMiniBlackHoleNumber
                );
            }
    ...
    // born function
    function _born(address userAddress, uint256 planetId) internal {
        IApeironPlanet.PlanetData memory planetData = _getPlanetData(planetId);
        // check can born
        require(
...
contract ChainlinkRng is VRFConsumerBaseV2, Random, Ownable {
    ...
    uint256 public r_status;
    address s_owner;

    event RandomWordsReturn(uint256 result);
    ...
    function getRandomNonce() external view returns (uint256) {
        return _getRandomNonce();
    }
