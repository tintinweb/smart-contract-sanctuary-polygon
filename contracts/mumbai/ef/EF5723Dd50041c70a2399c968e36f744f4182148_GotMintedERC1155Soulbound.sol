    ...
    ERC1155SupplyUpgradeable
{
    ...
    mapping(uint => address[]) internal allowList;
     ...
     * @dev Initializes the contract in unpaused state.
     */
    function Pausable_init() internal onlyRole(ADMIN_ROLE) onlyInitializing {
        __Pausable_init_unchained();
    }

    // Function sets Pause to "True"
    function pause() internal onlyRole(ADMIN_ROLE) whenNotPaused {
        _pause();
    }

    // Function sets Pause to "false"
    ...
    function createAllowList(uint tokenId, address [] calldata users)
        external
        onlyRole(ADMIN_ROLE)
    {
    ...
    function isOnAllowlist(uint tokenId, address user) public view returns (bool) {
    ...
    function getAllowList(uint tokenId) public view returns (address[] memory) {
        return allowList[tokenId];
    }

//***ADMIN LIST FUNCTIONALITY to be replaced with Access Control*/
    ...
    //access based on openzeppelin AccessControlUpgradeable.sol
    function assignRole(bytes32 role, address account) internal {
        grantRole(role, account);
    }

    //access based on openzeppelin AccessControlUpgradeable.sol
    function _hasRole(bytes32 role, address account) public view {
    ...
    {
        require(operator != msg.sender, "Cant give aprroval to self");
    ...
    function mint(
        ...
        //require(onlyAdmins == true, "Only Admins can mint tokens");
        require(amount > 0, "need to mint at least 1 NFT");
        uint256 supply = totalSupply(id);
        // checking against max supply, if the token's max supply has been set
        if (tokenIdtoTokenMaxSupply[id] > 0) {
            require(
                supply + amount <= tokenIdtoTokenMaxSupply[id],
                "max NFT limit exceeded"
            );
        }

        require(
            isOnAllowlist(id, msg.sender),
    ...
    //The following functions are required overrides from solidity
...
import "./GotMintedERC1155.sol";

contract GotMintedERC1155Soulbound is GotMintedERC1155 {
    function mint(
        uint256 id,
        uint256 amount,
        bytes memory data
    ...
    //Transfer Token -
    /**
     *@dev onlyOwner can transfer token
     */
    function transfer(
        address from,
        address to,
