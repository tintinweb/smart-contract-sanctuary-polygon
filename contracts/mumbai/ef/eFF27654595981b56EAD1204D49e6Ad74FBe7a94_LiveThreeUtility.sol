...
contract LiveThreeUtility {
    ...
    constructor(ISuperfluid _sfHost, address _gelatoOps) {
        ...
        cfaV1Lib = CFAv1Library.InitData(_sfHost, cfa);
        gelatoOps = _gelatoOps;
    }

    function deleteFlow(
        ISuperToken _superToken,
        address _sender, // should be EOA/AA SCW (sender of flow)
        address _receiver,
        address _app
    ) external {
        require(
            msg.sender == gelatoOps ||
                msg.sender == _sender ||
                msg.sender == _receiver,
            "Only Gelato Ops or Flow Participants"
        );

        cfaV1Lib.deleteFlowByOperator(_sender, _receiver, _superToken);
        cfaV1Lib.deleteFlowByOperator(_sender, _app, _superToken);
    }
}