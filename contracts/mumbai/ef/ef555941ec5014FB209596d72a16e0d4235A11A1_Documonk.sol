    ...
    OwnableUpgradeable
{
    ...
    CountersUpgradeable.Counter private _docItemIds; // Id for each individual item
    CountersUpgradeable.Counter private _instituteItemIds; // Id for each individual item
    ...
    function getInstituteDetails(string calldata UID)
        public
        view
        returns (InstituteDetails memory)
    {
        return instituteDetailsArray[UID];
    }

    // add wallet of UID in institute record
    function addWallet(string calldata UID, string calldata wallet)
        public
        onlyRole(RELAY_ROLE)
    {
        require(instituteDetailsArray[UID].timstamp > 0);
    ...
    // get wallets of UID
    function getWallet(string calldata UID)
        public
        view
    ...
    function getDocumentSHA256(string calldata hash)
        public
        view
        returns (InstituteDoc[] memory)
    {
        uint256 itemId = _docItemIds.current();
        InstituteDoc[] memory tempData = new InstituteDoc[](1);
        for (uint256 i = 0; i < itemId; i++) {
            if (
                keccak256(abi.encodePacked(documentDetailsArray[i].docHash)) ==
                ...
                tempData[0].instituteName = instituteDetailsArray[
                    documentDetailsArray[i].UID
                ].name;
                break;
            }
            if (
                keccak256(abi.encodePacked(documentDetailsArray[i].sha_256)) ==
                keccak256(abi.encodePacked(hash))
            ) {
                tempData[0].timstamp = documentDetailsArray[i].timstamp;
                tempData[0].UID = documentDetailsArray[i].UID;
    ...
    function getDocumentsCountByUID(string calldata UID)
        public
        view
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < itemId; i++) {
            if (
                keccak256(abi.encodePacked(documentDetailsArray[i].UID)) ==
