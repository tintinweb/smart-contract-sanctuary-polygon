...
contract Restaurants is ERC721 {
   ...
   */
  constructor(Token _token) ERC721("CUANBOSYA", "CNBS") public {
    token = _token;
  ...
  function donateETHToRestaurant(uint256 amount, uint _restaurantId, string memory _tokenURI) public returns (bool) {
    Restaurant memory _restaurant = restaurants[_restaurantId];

    require(_restaurant.donationNeeded >= amount);
    _restaurant.owner.transfer(amount);

    _restaurant.donationNeeded -= amount;
    ...
    nft[_tokenId] = NFT(_tokenId, _restaurant.name, red, green, blue, amount);

    // Give 3 token (FTR) to the donator
    token.transfer(msg.sender, 3000000000000000000);

    emit DonationForRestaurant(_restaurantId, amount, _restaurant.donationNeeded, now, msg.sender, _restaurant.owner);
  }
...
contract Token is ERC20 {
    ...
    constructor() ERC20("Fund CUANBOSYA", "CNBS") public {}

    function createTokens(address _recipient) external {
        require(isCreateToken == false);
        _mint(_recipient, 1000000000000000000000000000);
        isCreateToken = true;
    }
