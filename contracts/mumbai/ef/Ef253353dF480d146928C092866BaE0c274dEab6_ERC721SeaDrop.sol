...
import { IERC2981 } from "../lib/openzeppelinContracts/contracts/interfaces/IERC2981.sol";

import { IERC165 } from "../lib/openzeppelinContracts/contracts/utils/introspection/IERC165.sol";
...
contract ERC721ContractMetadata is ERC721A, TwoStepOwnable, ISeaDropTokenContractMetadata {
     ...
     */
    function emitBatchTokenURIUpdated(uint256 startTokenId, uint256 endTokenId) external {
        // Ensure the sender is only the owner or contract itself.
        _onlyOwnerOrSelf();
    ...
    DefaultOperatorFilterer
{
    ...
    // XP bridge contract
    address public xpBridge;

    // modifier to restrict access to the burner
    modifier onlyBurner() {
        require(msg.sender == burner, "ERC721SeaDrop: caller is not the burner");
        _;
    }
     ...
     */
    function mintSeaDrop(address minter, uint256 quantity) external payable virtual override nonReentrant {
     ...
     * Note: Only the owner can set the burner.
     */
    ...
    function setXpBridge(address _xpBridge) external onlyOwner {
        require(_xpBridge != address(0), "XP Bridge cannot be zero address");
        xpBridge = _xpBridge;
    }
     ...
     * Note: Only the burner can burn tokens.
     */
    function burn(uint256 tokenId) external onlyBurner {
        require(xpBridge != address(0), "XP Bridge not set");
        // check if the owner of the token is the xp bridge
        require(ownerOf(tokenId) == xpBridge, "Only NFTs owned by XP Bridge can be burned");
        _burn(tokenId);
    }
}
