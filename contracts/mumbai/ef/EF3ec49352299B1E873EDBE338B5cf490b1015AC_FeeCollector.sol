...
contract FeeCollector is IFeeCollector {
    ...
    constructor(address _WETH, address _Staking) {
        WETH_ADDRESS = _WETH;
        Staking_ADDRESS = _Staking;
    }

    fallback() external payable {}

    // TODO move non-staking reward first
    function updateReward() external override {
        uint256 native_balance = address(this).balance;
        WETH(payable(WETH_ADDRESS)).deposit{value: native_balance}();
        uint256 reward = WETH(payable(WETH_ADDRESS)).balanceOf(address(this));
        WETH(payable(WETH_ADDRESS)).transfer(Staking_ADDRESS, reward);
        TokenStaking(Staking_ADDRESS).updateRewards(reward);
    }

    function getBalances() external view override returns (uint256) {
        uint native_balance = address(this).balance;
        uint weth_balance = WETH(payable(WETH_ADDRESS)).balanceOf(address(this));
        return native_balance + weth_balance;
    }
...
contract TokenStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public immutable REWARD_DURATION_IN_BLOCKS = 6500; // 6500 ~ 1 day
    ...
    constructor(address _rewardToken, address _fanToken) {
    ...
    function updateRewards(uint256 reward) external {
        ...
        if (block.number >= periodEndBlock) {
            currentRewardPerBlock = reward / REWARD_DURATION_IN_BLOCKS;
        } else {
            currentRewardPerBlock =
                (reward +
                    ((periodEndBlock - block.number) * currentRewardPerBlock)) /
                REWARD_DURATION_IN_BLOCKS;
        }

        lastUpdateBlock = block.number;
        periodEndBlock = block.number + REWARD_DURATION_IN_BLOCKS;

        emit NewRewardPeriod(lastUpdateBlock, currentRewardPerBlock, reward);
    }

    /**
     * @notice Deposit staked tokens (and collect reward tokens if requested)
     * @param amount amount to deposit (in GLR)
     * @dev There is a limit of 1 GLR per deposit to prevent potential manipulation of current shares
     */
        ...
        updateReward(msg.sender)
    {
        require(
            ...
            "Deposit: Deposit amount must be >= 1"
        );

        // Record everybody last stake time
        ...
        // Transfer fan tokens to this address
        fanToken.safeTransferFrom(msg.sender, address(this), amount);
     ...
     * @param claimRewardToken whether to claim reward tokens
     */
    function _withdraw(uint256 shares, bool claimRewardToken)
        internal
        updateReward(msg.sender)
    {
        // Retrieve total amount staked and calculated current amount (in GLR)

        // Adjust internal shares
        userInfo[msg.sender].shares -= shares;
        totalShares -= shares;

        uint256 adjustedShares = shares;

        fanToken.safeTransfer(msg.sender, adjustedShares);
