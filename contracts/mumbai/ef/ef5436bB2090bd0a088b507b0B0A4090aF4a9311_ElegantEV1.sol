...
interface RulesInterface{
    ...
    function prizeCheck (uint _total) external view returns (uint[] calldata);
    function nutPrizeCheck (uint _total) external view returns (uint[] calldata);
    function getCoffer () external view returns (uint);
    function getBowl () external view returns (uint);
    function devCut () external view returns (uint);
    function refCut () external view returns (uint);
}
...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function signUp (uint[] calldata _racers, uint _event) public payable{
        ...
        uint bowel = 0;
        if (rulesByRace[_event].conCheck != address(0)){
            moolah = RulesInterface(rulesByRace[_event].conCheck).devCut();
            nards = RulesInterface(rulesByRace[_event].conCheck).refCut();
            coffee = RulesInterface(rulesByRace[_event].conCheck).getCoffer();
            bowel = RulesInterface(rulesByRace[_event].conCheck).getBowl();
        } else {
            moolah = money.mul(25).div(100);
            nards = nutso.mul(5).div(100);
            coffee = money.mul(25).div(100);
            bowel = nutso.mul(15).div(100);
        }
    ...
    function nextRace() public {
        ...
        if (currentRace < (raceNum-1)){
            ...
            uint[] memory prizes = RulesInterface(rulesByRace[_ev].conCheck).prizeCheck(coffers);
            uint[] memory nutses = RulesInterface(rulesByRace[_ev].conCheck).nutPrizeCheck(nutBowl);
            uint i = 0;
