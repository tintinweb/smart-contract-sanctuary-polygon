...
contract chain_scouts_ships_l2 is ERC721Upgradeable, OwnableUpgradeable, PausableUpgradeable, ILayerZeroReceiver {
    ...
    function mintToken (address _to, uint amountToMint) internal {
        for (uint i=0;i<amountToMint;i++) {
            currentIndex+=1;
            isMinted[currentIndex] = true;
    ...
    function mintTokens(address _to, uint[] memory tokenIds) internal {
        for (uint i=0; i< tokenIds.length; i++) {
            require(isMinted[tokenIds[i]],'Cannot migrate token which is not yet minted');
    ...
    function lzReceive(
        ...
        (address toAddress, uint migrationType, uint amount, uint256[] memory tokenIds) = abi.decode(
            _payload,
            (address, uint256, uint256, uint256[])
        );
        if (migrationType == 1)
            mintToken(toAddress, amount);
        else
