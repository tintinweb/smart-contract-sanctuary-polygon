...
contract SMRSkillsCertificate is  SMRCertificate {
    constructor() SMRCertificate("TEST - Skills Certificate", "SMRSKL", "Skills") {
...
abstract contract SMRCertificate is ERC165, ERC721, ISMRCertificate {
    ...
    string private constant _version = "v01";
   ...
   // mapping for all certificates
    mapping (uint256 => ISMRCertificate.Cert) certificate;
  ...
  function supportsInterface(bytes4 interfaceId) public view virtual override (ERC165, ERC721) returns (bool) {
    ...
    function attest(address to, string calldata uri, string calldata imprint, bytes calldata signature) external override onlyService returns (uint256) {
      uint256 _tokenId = _issueCertificate (to, uri, imprint, signature);
      emit Issue(certificate[_tokenId].issuer, certificate[_tokenId].user, _tokenId);
      //  _reserveCounter.increment(); is not needed because will happen in the end of the method
    ...
    function take(uint256 tokenId) external override  {
      ...
      require(certificate[tokenId].user != _user, "certificate is addressed for another user");

      certificate[tokenId].user = _user;
    ...
    function revoke(uint256 tokenId) external override {
      ...
      require(ownerOf(tokenId) ==  address(0), "certificate already issued and accepted");      
      address _user = certificate[tokenId].user;
      require(certificate[tokenId].issuer != msg.sender, "only issuer of the certificate can revoke it");
