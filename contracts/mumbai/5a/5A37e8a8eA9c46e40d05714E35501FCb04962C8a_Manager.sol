...
contract IQZoneNFT is ERC1155, AccessControl {
    ...
    bytes32 public BURNER_ROLE = keccak256('BURNER');
    bytes32 public INTERNAL_MINTER_ROLE = keccak256('INTERNAL_MINTER');
    ...
    function setBaseUri(
        ...
        string calldata baseUriSuffix_
    ...
    function setProperties(
        uint256 id,
        Property calldata props
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(idCounter > id, 'Bad id');
        require(
            props.singleton == false || properties[id].singleton == props.singleton,
            'singleton cant be changed'
        );
        require(
    ...
    // the "free" getter from public returns a tuple
    ...
    function create(Property memory property) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function _mint2(address to, uint256 id, uint256 amount) internal {
        require(
            properties[id].maxSupply == 0 ||
                properties[id].maxSupply >= supply[id] + amount,
            'max supply reached'
        );
    ...
    function mintBatch(MintBatch[] memory batch) external onlyRole(MINTER_ROLE) {
        ...
        for (uint256 i = 0; i < batch.length; ) {
            for (uint256 j = 0; j < batch[i].amounts.length; ) {
                require(
                    !properties[j].singleton ||
                        balanceOf(batch[i].to, j) + batch[i].amounts[j] <= 1,
    ...
    function balanceOfAllBatch(address user) external view returns (uint256[] memory) {
        address[] memory users = new address[](idCounter);
        uint256[] memory ids = new uint256[](idCounter);

        for (uint256 i = 0; i < idCounter; ) {
            users[i] = user;
            ids[i] = i;
            unchecked {
                ++i;
...
contract Manager is CredentialVerifier, IManager2, AccessControl {
    ...
    constructor(
        ...
        for (uint256 i = 0; i < stables_.length; ) {
        ...
        for (uint256 i = 0; i < dexes_.length; ) {
            dexes[dexes_[i]] = true;
            unchecked {
                ++i;
            }
        ...
        )
    {
        ...
        stableToken.safeTransferFrom(msg.sender, address(this), usd);
        console.log('safeTransferFrom');
        stableToken.safeIncreaseAllowance(lpad, usd);
        console.log('safeIncreaseAllowance');

        ILaunchpad(lpad).invest(msg.sender, usd, data, signature);
        console.log('lpad.invest');
    }

    function claim(uint256 round, address investor) public {
        require(rounds.length > round, 'bad round');
        address lpad = rounds[round];
    ...
    function changeVestingSchedule(
        uint256 round,
        IVesting.Unlock[] calldata schedule
    ) external onlyRole(LP_ADMIN_ROLE) {
        require(rounds.length > round, 'bad round');
        address lpad = rounds[round];
    ...
    function setCredentialVerifier(
        address credentialVerifier_
    ) external onlyRole(LP_ADMIN_ROLE) {
        credentialVerifier = credentialVerifier_;
    }
    ...
    function changeNftOracleSettings(
        NftOracleData calldata data
    ) external onlyRole(LP_ADMIN_ROLE) {
    ...
    function createRound(
        ...
        address lpad = address(new IQProxy(launchpadLogic, ''));
        rounds.push(lpad);
        ...
        address vesting = address(new IQProxy(vestingLogic, ''));
    ...
    function setAirdropContract(
        address airdropContract_
    ) external onlyRole(LP_ADMIN_ROLE) {
    ...
    function setRoundData(
        ...
        ILaunchpad(lpad).setRoundData(data);
    }
    ...
    function setStableToken(
        ...
        ILaunchpad(lpad).setStableToken(router);
    }
}
...
/**
 * @title IQProxy
 * @notice A simple non-upgradeable proxy contract for saving on gas if you need to deploy multiple instances of the same contract.
 * @author gotbit
 */

contract IQProxy is Proxy {
    address public immutable impl;

    constructor(address impl_, bytes memory data) {
        impl = impl_;
        if (data.length > 0) {
            (bool success, ) = impl_.delegatecall(data);
            if (!success) {
                assembly {
...
interface IAirdrop {
    ...
    function airdrop(
        address token,
        uint256 amount,
        IIqStaking.UserSharesOutput[] memory
    ) external;
}
...
interface IIqStaking {
    ...
    function userShares(
        uint256 offset,
        uint256 size
    ) external view returns (UserSharesOutput[] memory);
}
...
// the interface is separated due to https://github.com/ethereum/solidity/issues/11826
...
interface ILaunchpad2 {
    ...
    function setRoundData(Launch memory) external;

    function setDexRouter(address) external;

    function setStableToken(address) external;
}
...
interface IManager2 {
    ...
    function setRoundData(uint256 roundId, ILaunchpad.Launch memory data) external;

    function setDexRouter(uint256 roundId, address) external;

    function setStableToken(uint256 roundId, address stable) external;
    ...
    function setProjectParams(uint256 id, address token, address wallet) external;
}
