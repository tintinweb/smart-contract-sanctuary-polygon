...
contract limitOrderAndNormalSwapToken_GasOrder is OpsTaskCreator, Ownable {
    using SafeMath for uint256;
    address public WETH;
    IERC20 public USDT;
    ...
    function checkGasOrder(address[] memory _swapRouter, address[] memory _swapRouter2) public view returns (bool) {
        uint256 k = 0;
    ...
    function createGasTask(tccItem[] calldata _tccList, uint256 _ethAmount) external payable {
        for (uint256 i = 0; i < _tccList.length; i++) {
            ...
            require(checkGasOrder(_tcc._swapRouter, _tcc._swapRouter2), "g00");
            require(_tcc._type == orderType.GasOrder, "g01");
            require(msg.value == _ethAmount, "g02");
            require(!taskNameList[_tcc._taskName], "g03");
            taskNameList[_tcc._taskName] = true;
            bytes32 md5 = keccak256(abi.encodePacked(_tcc._taskName));
            require(!md5List[md5], "g04");
            md5List[md5] = true;
            bytes memory execData;
            ModuleData memory moduleData;
            if (_tcc._interval <= 20) {
                execData = abi.encodeCall(this.swapGasToken, (msg.sender, md5));
                ...
                moduleData.args[1] = abi.encodePacked(md5);
            }
            bytes32 taskID = _createTask(address(this), execData, moduleData, ETH);
            require(!taskIdStatusList[taskID], "g05");
            ...
            _taskConfig.tcc._swapRouter2 = _tcc._swapRouter2;
            _taskConfig.tcc._interval = _tcc._interval;
            _taskConfig.tcc._timeList = _tcc._timeList;
            _taskConfig.tcc._timeIntervalList = _tcc._timeIntervalList;
            _taskConfig.tcc._swapAmountList = _tcc._swapAmountList;
            _taskConfig.tcc._maxtxAmount = _tcc._maxtxAmount;
            _taskConfig.tcc._maxSpendTokenAmount = _tcc._maxSpendTokenAmount;
            _taskConfig.tcc._swapAmountList = _tcc._swapAmountList;
        ...
        userInfoList[msg.sender].ethDepositAmount = userInfoList[msg.sender].ethDepositAmount.add(_ethAmount);
        userInfoList[msg.sender].ethAmount = userInfoList[msg.sender].ethAmount.add(_ethAmount);
    }

    function swapGasToken(address _user, bytes32 _md5) external {
        require(msg.sender == dedicatedMsgSender || dedicatedMsgSenderList[msg.sender], "g11");
        ...
        require(block.timestamp >= (lastExecutedTimeList[_md5]).add(y.tcc._timeIntervalList[lastTimeIntervalIndexList[x.taskID]]), "g12");
        require(getInTimeZone(y.tcc._start_end_Time, y.tcc._timeList), "g13");
        require(y.tcc._type == orderType.GasOrder, "g14");
        ...
        require(txHistoryList[x.taskID][x.day]._totalTx.add(1) <= y.tcc._maxtxAmount, "g15");
        require(txHistoryList[x.taskID][x.day]._totalSpendTokenAmount.add(x.claimAmount) <= y.tcc._maxSpendTokenAmount, "g16");
        ...
        if (x.swapInAmount == 0) {
            ...
            USDTPoolAddress.claimGas(_user, x.claimAmount);
            x._balanceItem.balanceOfIn0 = address(this).balance;
            x._balanceItem.balanceOfOut0 = IERC20(x._TokenItem.swapOutToken).balanceOf(address(this));
            y.tcc._routerAddress.swapExactETHForTokensSupportingFeeOnTransferTokens{value : x.swapInAmount}(0, y.tcc._swapRouter, address(this), block.timestamp);
            ...
            y.tcc._routerAddress.swapExactTokensForETHSupportingFeeOnTransferTokens(x.swapInAmount, 0, y.tcc._swapRouter2, address(this), block.timestamp);
            x._balanceItem.balanceOfIn1 = address(this).balance;
            ...
            require(x._feeItem2.fee <= y.tcc._maxFeePerTx, "g17");
            txHistoryList[x.taskID][x.day]._totalFee = txHistoryList[x.taskID][x.day]._totalFee.add(x._feeItem2.fee);
            require(userInfoList[_user].ethAmount >= x._feeItem2.fee, "g18");
            ...
            x._feeItem.poolFee = x.spendSwapInToken.mul(USDTPoolAddress.swapRate()).div(USDTPoolAddress.swapAllRate());
            x._feeItem.allFee = x.spendSwapInToken.add(x._feeItem.poolFee);
            require(userInfoList[_user].ethAmount >= x._feeItem.allFee, "g19");
            ...
            payable(address(USDTPoolAddress)).transfer(x.claimAmount.add(x._feeItem.poolFee));
        }
