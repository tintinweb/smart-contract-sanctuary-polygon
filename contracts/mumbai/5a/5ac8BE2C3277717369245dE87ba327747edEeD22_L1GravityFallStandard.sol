...
interface IBridgeConfig {
  ...
  function addValidator(address _account) external;

  function removeValidator(address _account) external;
...
abstract contract Multisign {
  ...
  /** Modifier */
  modifier requireMultisign() {
    require(msg.sender == address(this), "Owner required");
    _;
  }
  ...
  /* Admins */
  function addMember(address _account) public virtual requireMultisign {
    members.add(_account);
  }

  function removeMember(address _account) public virtual requireMultisign {
  ...
  function submitTransaction(uint256 _value, bytes calldata _data) public requireOwner {
    _beforeAddTransaction(_value, _data);
  ...
  /* Internal */
  function _beforeAddTransaction(uint256 _value, bytes calldata _data) internal virtual;
}

// SPDX-License-Identifier: MIT
pragma solidity ^0.8.17;

interface IL1BridgeStandard {
  /* Events */
  event TokenDepositQueued(uint256 indexed orderId, address indexed l1Token, address indexed l2Token, address from, address to, uint256 amount);
  event TokenWithdrawalCompleted(uint256 indexed orderId, address indexed l1Token, address indexed l2Token, address from, address to, uint256 amount);
  ...
  function executeERC20Withdrawal(
    address[] memory _currentValidators,
    uint8[] memory _v,
    bytes32[] memory _r,
    bytes32[] memory _s,
...
import { Multisign, MemberSet } from "../common/Multisign.sol";
import { IBridgedToken } from "../common/IBridgedToken.sol";
import { IL1BridgeStandard } from "./IL1BridgeStandard.sol";
import { IBridgeConfig } from "../common/IConfig.sol";

contract L1GravityFallStandard is ReentrancyGuard, Multisign, IBridgeConfig, IL1BridgeStandard {
  ...
  mapping(uint256 => Order) public withdrawals;
  ...
  /* Admin */
  function updateKeeper(address _account) public requireMultisign {
    keeper = _account;
  }

  function updateConsensusPowerThreshold(uint256 _amount) public requireMultisign {
    consensusPowerThreshold = _amount;
  }

  function addValidator(address _account) public requireMultisign {
    validators.add(_account);
  }

  function removeValidator(address _account) public requireMultisign {
  ...
  function updateTokensGateway(
    ...
    address _l2Token,
    bool _status
  ) public requireMultisign {
    tokensGateway[_l1Token][_l2Token] = _status;
  }
  ...
  receive() external payable {
    _initiateETHDeposit(msg.sender, msg.sender, msg.value);
  }
  ...
  function depositETHTo(address _to) external payable {
    _initiateETHDeposit(msg.sender, _to, msg.value);
  }
  ...
  function _initiateETHDeposit(
    ...
    uint256 orderId = deposits.length;
    deposits[orderId].executed = true;
    deposits[orderId].from = _from;
    deposits[orderId].to = _to;
    deposits[orderId].amount = _amount;

    // TODO: process fee ?

    emit TokenDepositQueued(orderId, address(0), address(0), _from, _to, _amount);
  }
  ...
  function _initiateERC20Deposit(
    ...
    uint256 orderId = deposits.length;
    deposits[orderId].executed = true;
    deposits[orderId].l1Token = _l1Token;
    deposits[orderId].l2Token = _l2Token;
    deposits[orderId].from = _from;
    deposits[orderId].to = _to;
    deposits[orderId].amount = _amount;

    emit TokenDepositQueued(orderId, _l1Token, _l2Token, _from, _to, _amount);
  }
  ...
  function executeETHWithdrawal(
    ...
    require(!withdrawals[_orderId].executed, "Order already executed");
    require(
    ...
    _checkValidatorSignatures(
      ...
      keccak256(abi.encode(this.executeETHWithdrawal.selector, _orderId, _from, _to, _amount)),
    ...
    emit TokenWithdrawalCompleted(_orderId, address(0), address(0), _from, _to, _amount);
  }
  ...
  function executeERC20Withdrawal(
    ...
    _checkValidatorSignatures(
      ...
      _s,
      // Get hash of the transaction batch and checkpoint
      keccak256(abi.encode(this.executeERC20Withdrawal.selector, _orderId, _l1Token, _l2Token, _from, _to, _amount)),
      consensusPowerThreshold
    );

    IERC20(_l1Token).safeTransfer(_to, _amount);
    emit TokenWithdrawalCompleted(_orderId, _l1Token, _l2Token, _from, _to, _amount);
  }
  ...
  function _checkValidatorSignatures(
    ...
    for (uint256 i = 0; i < _currentValidators.length; i++) {
      address signer = _messageHash.toEthSignedMessageHash().recover(_v[i], _r[i], _s[i]);
  ...
  function _beforeAddTransaction(uint256, bytes calldata _data) internal pure override {
    bytes4 funcSelector = bytes4(_data[:4]);
    require(funcSelector == this.addMember.selector || funcSelector == this.removeMember.selector, "Invalid function selector");
  }
}
