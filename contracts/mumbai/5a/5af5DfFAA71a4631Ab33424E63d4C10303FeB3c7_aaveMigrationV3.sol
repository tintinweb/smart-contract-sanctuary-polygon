...
contract aaveMigrationV3 is IFlashLoanReceiver {
    ...
    function migrateWithNoDebt(
        address from,
        address to,
        address[] calldata suppliedAssets,
        uint256[] calldata suppliedAmounts
    ) external {
        for (uint256 i = 0; i < suppliedAssets.length; i++) {
            (address aToken, , ) = dataProvider.getReserveTokensAddresses(from);
            IERC20(aToken).transferFrom(
                from,
                address(this),
                suppliedAmounts[i]
            );
            IERC20(aToken).transfer(to, suppliedAmounts[i]);
        }
    }
    ...
    function migrate(
        address from,
        address to,
        bytes calldata suppliedParams,
        bytes calldata borrowedParams,
        bytes calldata borrowRatesParams
    ) external {
        ...
        _migrate(from, to, params);
        emit Migrated(from, to);
    }
    ...
    function executeOperation(
        ...
        address to;
        bytes memory param;
        (from, to, param) = abi.decode(params, (address, address, bytes));
        allowanceToPool(param);
        repayAllDebt(from, param);
        transferAllAtokens(from, param, to);
        borrowOnMigrated(to, param, premiums);
    ...
    function borrowOnMigrated(
        ...
        bool[] memory migratedBorrowRates;
        (borrowedAssets, borrowedAmounts) = getBorrowedParams(_params);
        (, migratedBorrowRates, ) = getBorrowRatesParams(_params);

        for (uint256 i = 0; i < borrowedAssets.length; i++) {
            uint256 rate = migratedBorrowRates[i] ? 1 : 0;
            POOL().borrow(
                borrowedAssets[i],
                borrowedAmounts[i] + _premiums[i],
                rate + 1,
                0,
                to
            );
    ...
    function transferAllAtokens(
        ...
        (suppliedAssets, suppliedAmounts) = getSuppliedParams(_params);
    ...
    function repayAllDebt(address user, bytes memory _params) internal {
        address[] memory borrowedAssets;
        bool[] memory borrowRates;

        (borrowedAssets, ) = getBorrowedParams(_params);
        (borrowRates, , ) = getBorrowRatesParams(_params);

        for (uint256 i = 0; i < borrowedAssets.length; i++) {
            (, address stableDebt, address variableDebt) = dataProvider
                .getReserveTokensAddresses(borrowedAssets[i]);

            uint256 amount = borrowRates[i]
                ? IERC20(variableDebt).balanceOf(user)
                : IERC20(stableDebt).balanceOf(user);

            uint256 rate = borrowRates[i] ? 1 : 0;

            (POOL()).repay(borrowedAssets[i], amount, rate + 1, user);
        }
    }

    function allowanceToPool(bytes memory _params) internal {
        address[] memory borrowedAssets;
        uint256[] memory borrowedAmounts;
        (borrowedAssets, borrowedAmounts) = getBorrowedParams(_params);

        for (uint256 i = 0; i < borrowedAssets.length; i++) {
            IERC20(borrowedAssets[i]).approve(
                provider.getPool(),
                borrowedAmounts[i]
            );
        }
    ...
    function _migrate(address from, address to, bytes memory params) internal {
        address[] memory borrowedAssets;
        uint256[] memory borrowedAmounts;
        uint256[] memory modes;
        (borrowedAssets, borrowedAmounts) = getBorrowedParams(params);
        ...
        POOL().flashLoan(
            ...
            modes,
            address(this),
            abi.encode(from, to, params),
            0
        );
    }

    function getSuppliedParams(
        bytes memory _params
    ) public pure returns (address[] memory, uint256[] memory) {
        bytes memory suppliedParams;
        address[] memory suppliedAssets;
        uint256[] memory suppliedAmounts;
        (suppliedParams, , ) = abi.decode(_params, (bytes, bytes, bytes));
        (suppliedAssets, suppliedAmounts) = abi.decode(
            suppliedParams,
            (address[], uint256[])
        );
        return (suppliedAssets, suppliedAmounts);
    }

    function getBorrowedParams(
        bytes memory _params
    ) public pure returns (address[] memory, uint256[] memory) {
        bytes memory borrowedParams;
        address[] memory borrowedAssets;
        uint256[] memory borrowedAmounts;
        (, borrowedParams, ) = abi.decode(_params, (bytes, bytes, bytes));
        (borrowedAssets, borrowedAmounts) = abi.decode(
            borrowedParams,
            (address[], uint256[])
        );
    ...
    function getBorrowRatesParams(
        bytes memory _params
    ) public pure returns (bool[] memory, bool[] memory, bool) {
        bytes memory borrowRatesParams;
        bool[] memory borrowRates;
        bool[] memory migratedBorrowRates;
        bool isEmode;
        (, , borrowRatesParams) = abi.decode(_params, (bytes, bytes, bytes));
        (borrowRates, migratedBorrowRates, isEmode) = abi.decode(
            borrowRatesParams,
            (bool[], bool[], bool)
        );
        return (borrowRates, migratedBorrowRates, isEmode);
    }
