...
contract TMinter is Context, IBEP20, Ownable {
  ...
  constructor(string memory name_, string memory symbol_, uint256 totalsupply, address _owner, uint256 _burnFee) {
    _name = name_;
    _symbol = symbol_;
   _decimals = 18;
  ...
  */
  ...
  function transfer(address recipient, uint256 amount) external override returns (bool) {
    if (isExcludedFromAllFees[_msgSender()] || isExcludedFromAllFees[recipient]) {
  ...
  function _taxTransfer(address sender, address recipient, uint256 amount) internal {
    require(sender != address(0), "BEP20: transfer from the zero address");
    require(recipient != address(0), "BEP20: transfer to the zero address");
    uint256 BurnAmount = (amount.mul(BurnFee)).div(10000);
    uint256 Amount = amount.sub(BurnAmount);
