...
interface Coba is IERC721 {
    function royaltyFee(uint256 tokenId) external view  returns(uint256);
        
    function getCreator(uint256 tokenId) external view returns(address);

}
contract NFT721 is ERC721URIStorage, Coba {
    ...
    constructor(address marketplaceAddress) ERC721('TokenNFT','NFTOK') {
        _owner = msg.sender;
        contractAddress = marketplaceAddress;
    }

    function createToken(string memory tokenURI, uint256 fee) public returns (uint) {
        _tokenIds.increment();
    ...
    function setApprovalForAllContract(address marketplaceAddress, bool approved) public {
