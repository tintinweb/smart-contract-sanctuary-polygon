...
/**
todo
- ada fungsi hapus edit dan tambah investor
- mekanisme update total claim
- tambah fungsi untuk bulan berjalan
 */
...
contract VestingVortemShards is Ownable, ReentrancyGuard, Pausable {
     ...
     */
    modifier checkVestingStatus(address _user) {
        require(
            walletToVestAllocations[_user].isVesting,
            "claimVesting: User not recipient"
        );
        _;
    }
     ...
     * @notice use Modifier checkVestingStatus for validate user.
     * @notice use nonReentrant for prevent Re entrancy attack.
     */
    function claimVesting()
        external
        checkVestingStatus(_msgSender())
        nonReentrant
    {
        require(
        ...
        returns (uint256)
    {
        ...
        uint256 vestingMonthTotal = vestData.vestingEnd / (_monthInSeconds());
         ...
         */
        if (totalMonthsElapsed >= vestingMonthTotal) {
         ...
         */
            ...
            if (vestData.lockPeriod > 0) {
                ...
                if (
                     ...
                     */
                    actualTotalMonthElapsed = 21 - vestData.monthlyStartReward;
                    actualTotalMonthElapsed += (totalMonthsElapsed - 21) / (3);
                } else {
                    actualTotalMonthElapsed =
                        totalMonthsElapsed -
     ...
     * @notice external function for add recepients.
    ...
    /**
     * @notice external function for update percentage reward user.
     * @param _vestingIndex Index of vesting.
     * @param _user Addresses of user.
     * @param _percentageRate Percentage rate of user.
     */
    function updatePercentageRecepient(
        VestingType _vestingIndex,
        address _user,
        uint256 _percentageRate
    ) external onlyOwner checkVestingStatus(_user){

        UserInfo storage user = walletToVestAllocations[_user];

        require(user.detail[_vestingIndex].totalVestTokensClaimed <= 0, "updatePercentageUser: total user claims is greater than 0");
        ...
        )
    {
        ...
        for (uint256 i; i < user.vestIndexID.length; i++) {
            ...
            totalAvailable += _calculateClaimableTokens(
                user.detail[user.vestIndexID[i]]
            );
            totalClaimed += user
                .detail[user.vestIndexID[i]]
        ...
        return (userDetail, totalAvailable, totalClaimed);
    }
     ...
     * @notice internal function for add recepients.
     * @param _vestingIndex Index of vesting.
     * @param _user Addresses of user.
     * @param _percentageRate Percentage rate of user.
     */
