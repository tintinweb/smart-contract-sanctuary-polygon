...
contract SceneContractV1 is SceneStorage {
    function initialize() public initializer {
        __ERC721_init("Scene", "SCENE");
        __Ownable_init();
        __ERC721Royalty_init();

        tokenCounter = 0;
        CollectionWalletAddress = _msgSender();
        _setDefaultRoyalty(CollectionWalletAddress, 10000);

        baseUri = "https://scene.com/";
        baseExtension = ".json";
        unRevealURI = "https://scene.com/unreveal.json";
    ...
    function sale(uint256 _mintAmount) external payable {
        require(
            ((saleStatus == 1 &&
                isWhitelistedAddress[msg.sender]) || saleStatus == 2) &&
                whitelistedAddressCount[msg.sender] + _mintAmount <=
                maxPerWallet,
            "SceneContractV1: Sale is closed or Exceeds limit amount by user"
        );
        require(
            (_mintAmount + tokenCounter) <= maxMintCount &&
                (_mintAmount * tokenPrice) == msg.value,
            "SceneContractV1: Exceeds limit"
        );
    ...
    function tokenURI(
        ...
        require(
            _exists(_tokenId),
            "SceneContractV1: URI query for nonexistent token"
        );
    ...
    function _beforeTokenTransfer(
        ...
        require(
            ...
            "SceneContractV1: Address is blacklisted"
        );
