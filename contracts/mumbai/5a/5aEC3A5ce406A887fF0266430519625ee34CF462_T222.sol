...
contract T222 is BaseContract {
    ...
    */
    ...
    function _distributeDayRewardForInvestRanking() private {
        ...
        //将第一名加入到周排行榜地址
        weekTop1AddressList.push(_list[0]);
        uint256 _totalAmount;
        for (uint256 i = 0; i < _num; i++) {
            address _user = _list[i];
    ...
    function _distributeWeekReward() private {
        ...
        if (_canDistribute) {
            // 对第一名进行奖励分配
            uint256 _shareAmount = weekReward * 50 / 100;
            userInfoList[firstAddress]._rankingRewardItem._updatedWeeKReward += _shareAmount;
            weekReward -= _shareAmount;
        }
        // 清空地址数组
    ...
    function distributeWeekReward() external {
    ...
    function addWeekAddress(address[] memory _addressList) external onlyOwner {
        ...
        for (uint256 i = 0; i < _num; i++) {
            if (weekTop1AddressList.length < 7) {
                weekTop1AddressList.push(_addressList[i]);
            }
        }
    }
