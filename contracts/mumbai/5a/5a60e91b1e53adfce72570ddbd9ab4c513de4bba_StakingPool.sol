...
contract StakingPool is Ownable {
    ...
    uint256 public constant TOKEN_DECIMAL_PRECISION = 1e18; // == 1 whole VLR token
    uint256 private constant _MAX_UINT256 = ~uint256(0);

    struct StakeStrategy {
        string label;
        uint256 lockup_period;
        uint256 bonus_rate;
        bool isActive;
    }

    mapping(uint256 => StakeStrategy) private strategies;
    uint256 public stakingPoolLegnth = 0;
    ...
    function addStakeStrategy(string memory _label, uint256 _lockup_period, uint16 _bonus_numerator) external onlyOwner {
        StakeStrategy memory stk;
        stk.label = _label;
        stk.lockup_period = _lockup_period;
        stk.bonus_rate = _bonus_numerator;
        stk.isActive = true;
        strategies[stakingPoolLegnth] = stk;
        stakingPoolLegnth++;
    }

    function removeStakeStrategy(uint16 index) external onlyOwner {
        for (uint i = index; i<stakingPoolLegnth-1; i++){
            strategies[i] = strategies[i+1];
        }
        delete strategies[stakingPoolLegnth-1];
        stakingPoolLegnth--;
    }
    ...
    constructor(
        ...
        stakingPoolLegnth = 0;
        StakeStrategy memory stk1;
        stk1.label = "Staking one";
        stk1.lockup_period = 10000;
        stk1.bonus_rate = 5;
        stk1.isActive = true;
        strategies[stakingPoolLegnth] = stk1;
        stakingPoolLegnth++;
        StakeStrategy memory stk2;
        stk2.label = "Staking two";
        stk2.lockup_period = 20000;
        stk2.bonus_rate = 10;
        stk2.isActive = true;
        strategies[stakingPoolLegnth] = stk2;
        stakingPoolLegnth++;
        StakeStrategy memory stk3;
        stk3.label = "Staking three";
        stk3.lockup_period = 30000;
        stk3.bonus_rate = 15;
        stk3.isActive = true;
        strategies[stakingPoolLegnth] = stk3;
        stakingPoolLegnth++;
        StakeStrategy memory stk4;
        stk4.label = "Staking four";
        stk4.lockup_period = 40000;
        stk4.bonus_rate = 20;
        stk4.isActive = true;
        strategies[stakingPoolLegnth] = stk4;
        stakingPoolLegnth++;
        
    }

    function getStakeStrategy(uint256 index) public view returns (StakeStrategy memory stk) {
        stk = strategies[index];
    }

    function updateStakeStrategy(uint256 index, string memory _label ,uint256 _period, uint16 _bonus_numerator, bool _isActive) external onlyOwner {
        strategies[index].label = _label;
        strategies[index].lockup_period = _period;
        strategies[index].bonus_rate = _bonus_numerator;
        strategies[index].isActive = _isActive;
    }
    ...
    function setFeatures(Features memory _f) external onlyOwner {
        _features = _f;
     ...
     */
    function stake(uint256 _amount, uint8 stakeIndex) external whenStakingEnabled {
        // Since the fractional part of the amount will be discarded make sure that the amount is
        // at least 1 VLR, otherwise there'll be literally nothing left to stake.
        require(_amount >= TOKEN_DECIMAL_PRECISION, "StakingPool: amount too small");
        require(stakeIndex < stakingPoolLegnth, "StakingPool: invalid lockup period");
        require(strategies[stakeIndex].isActive, "StakingPool: is not activated");
        ...
        _addStake(stakeholder.stakes, stakeAmount, stakeIndex);
     ...
     * @param _stakeStrategyIndex Period tokens should be restaked for.
    ...
    function restake(
        ...
        Stakeholder storage stakeholder = _stakeholderMap[msg.sender];
        require(_stakeStrategyIndex < stakingPoolLegnth, "StakingPool: invalid lockup period");
        require(strategies[_stakeStrategyIndex].isActive, "StakingPool: is not activated");
        require(
            (maxStakesPerAccount == 0) || (stakeholder.stakes.length < maxStakesPerAccount),
            "SP: account has too many stakes"
        );
        ...
        // drop the fractional part
        uint256 stakeAmount = _amount.div(TOKEN_DECIMAL_PRECISION).mul(TOKEN_DECIMAL_PRECISION);
        _addStake(stakeholder.stakes, stakeAmount, _stakeStrategyIndex);
    ...
    function amend(
        ...
        _amendStake(stakeholder.stakes, stakeAmount, _stakeStrategyIndex, _extend);
        ...
        returns (uint256 rewardsEarned, uint256 stakeUnlocked)
    {
        ...
        for (uint256 i = 0; i < stakeholder.stakes.length; i++) {
            ...
            uint256 bonusStart =
                Math.max(stakeholder.lastClaimedAt, stk.unlockOn.sub(strategies[stk.strategyIndex].lockup_period));
            uint256 bonusEnd = Math.min(_asAt, stk.unlockOn);
            require(bonusEnd >= bonusStart, "StakingPool: invalid bonus period");
            StakeStrategy memory stakeStrategy = strategies[stk.strategyIndex];
            uint256 rewardEarn = stk.amount.mul(stakeStrategy.bonus_rate)
                    .div(BONUS_DENOMINATOR)
                    .mul(annualBaseRewardsRate);
            rewardsEarned = rewardsEarned.add(
                    rewardEarn
                    .div(REWARDS_RATE_PRECISION)
                    .mul(bonusEnd.sub(bonusStart))
                    .div(365 days)
            );
        ...
        if (_asAt > stakeholder.lastClaimedAt) {
            rewardsEarned = rewardsEarned.add(
                stakeholder
                    ...
                    .mul(annualBaseRewardsRate)
                    .div(REWARDS_RATE_PRECISION)
                    .mul(_asAt.sub(stakeholder.lastClaimedAt))
                    .div(365 days)
            );
        }
    }
            ...
            for (int256 i = int256(lastIdx); i >= 0; i--) {
                ...
                uint256 bonusStart =
                    Math.max(
                        ...
                        stk.unlockOn.sub(strategies[stk.strategyIndex].lockup_period)
                    );
                uint256 bonusEnd = Math.min(block.timestamp, stk.unlockOn);
                require(bonusEnd >= bonusStart, "StakingPool: invalid bonus period");
                uint256 rewardEarn = stk.amount
                        .mul(strategies[stk.strategyIndex].bonus_rate)
                        .div(BONUS_DENOMINATOR);
                rewardsEarned = rewardsEarned.add(
                    rewardEarn
                        .mul(annualBaseRewardsRate)
                ...
                if (block.timestamp > stk.unlockOn) {
                    rewardsEarned = rewardsEarned.add(
                        stk
                            ...
                            .mul(annualBaseRewardsRate)
                            .div(REWARDS_RATE_PRECISION)
                            .mul(block.timestamp.sub(stk.unlockOn))
                            .div(365 days)
                    );
                }
                ...
                if (block.timestamp >= stk.unlockOn) {
                    ...
                    emit Unstaked(msg.sender, stk.amount, stk.strategyIndex, stk.unlockOn);
        ...
        if (block.timestamp > stakeholder.lastClaimedAt) {
            rewardsEarned = rewardsEarned.add(
                unlockedBalance
                    .mul(annualBaseRewardsRate)
                    .div(REWARDS_RATE_PRECISION)
                    .mul(block.timestamp.sub(stakeholder.lastClaimedAt))
                    .div(365 days)
            );
        }
    ...
    {
        stk.strategyIndex = _packedStake.strategyIndex;
    ...
    function _addStake(
        ...
        StakeStrategy memory strategy = strategies[_stratgyIndex];
        uint256 unlockOn = block.timestamp.add(strategy.lockup_period);
        Stake memory stk;
        stk.strategyIndex = _stratgyIndex;
        stk.unlockOn = unlockOn.toUint64();
        stk.amount = _amount.div(TOKEN_DECIMAL_PRECISION).toUint128(); // discard fractional part
        _stakes.push(stk);

        emit Staked(msg.sender, _amount, _stratgyIndex, unlockOn);
    }
    ...
    function _amendStake(
        ...
        Stake storage stk = _findMostRecentStake(_stakes, _stakeStrategyIndex);
        ...
        emit Unstaked(msg.sender, oldAmount, _stakeStrategyIndex, unlockOn);

        stk.amount = newAmount.div(TOKEN_DECIMAL_PRECISION).toUint128();
        if (_extend) {
            unlockOn = block.timestamp.add(strategies[stk.strategyIndex].lockup_period);
            stk.unlockOn = unlockOn.toUint64();
        }

        emit Staked(msg.sender, newAmount, _stakeStrategyIndex, unlockOn);
    }

    function _findMostRecentStake(Stake[] storage _stakes, uint8 _strategyIndex)
        private
        view
        returns (Stake storage)
    {
        ...
        for (uint256 i = 0; i < _stakes.length; i++) {
            if (
                (_stakes[i].strategyIndex == _strategyIndex) &&
    ...
    function batchImportAccounts(
        ...
        ExportedAccount[] memory _accounts
    ) external onlyOwner {
        require(
        ...
        for (uint256 i = 0; i < _stakeholders.length; i++) {
            ExportedAccount memory account = _accounts[i];
            ...
            for (uint256 j = 0; j < account.stakes.length; j++) {
                Stake memory stk;
                stk.strategyIndex = account.stakes[j].strategyIndex;
