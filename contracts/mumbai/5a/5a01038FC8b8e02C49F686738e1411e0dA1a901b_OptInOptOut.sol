...
contract OptInOptOut{
    ...
    */
    ...
    function userDataTransfer(address _userAd, bool _network, bool _location, ouraRingData memory _ouraData) external{
        if(_userAd == address(0)){ revert zeroAddressNotSupported();}
        if(adminAddress[msg.sender] != true){ revert notAdminAddress();}
        UserContract useC = UserContract(userContractAddress);
        (bool status,uint id) = useC.verifyUser(_userAd);
        ...
        if(!userOptins[_userAd].location){
            _location = false; //parameter value changed based on optIn status
        }
        if(!userOptins[_userAd].network){
            _network = false; //parameter value changed based on optIn status
        }
        optInsData memory data;
        data.location = _location;
        data.network = _network;
        if(userOptins[_userAd].ouraRing){ //If ouraRing data is set true in oura function, then only storing data.
    ...
    function userTransferredData(address _userAd, uint _index) external view returns(optInsData memory data){
        return  userDataTx[_userAd][_index];
    }
