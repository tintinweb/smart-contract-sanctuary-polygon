...
contract RACD {
    ...
    uint256 public summoningTime; // time RACD contract deployed
    ...
    struct Member {
        ...
        uint256 jailed; // set to the proposalIndex of a passing RACD kick proposal for this member. Prevents voting on and sponsoring proposals.
    }
    struct Proposal {
    ...
    modifier onlyMember {
        require(members[msg.sender].shares > 0, "Your are not a member of the RACD.");
        _;
    }
    ...
    function processProposal(uint256 proposalId) public onlyMember returns (bool) {
        ...
        require(proposals[proposalId].paymentRequested <= address(this).balance, "RACD balance too low to accept the proposal.");
    ...
    // Function to submit a vote to a proposal if you are a member of the RACD and you have not voted yet.
    // Voting period must be in session
    function submitVote(uint256 proposalId, uint8 uintVote) public onlyMember {
        require(members[msg.sender].exists, "Your are not a member of the RACD.");
        require(proposals[proposalId].exists, "This proposal does not exist.");
    ...
    // Deposit function to provide liquidity to RACD contract
