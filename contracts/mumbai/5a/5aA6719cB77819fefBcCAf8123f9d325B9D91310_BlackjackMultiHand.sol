...
contract BlackjackMultiHand is Claim, Bound {
    ...
    */
    ...
    mapping(uint256 => mapping(uint8 => uint8[])) public playerCards;
    ...
    */
    function play(uint8 hands, bool isEth, address tokenAddr, uint256 amount) external payable nonReentrant betLimit(isEth, amount, tokenAddr) {
        ...
        emit BetPlaced(currentBetId, block.timestamp, msg.sender, totalBet);
    ...
    function split(uint256 betId, uint8 handIndex) external payable nonReentrant playerChecks(betId, handIndex) {
        ...
        uint8 card1 = playerCards[betId][handIndex][0] % 13;
        uint8 card2 = playerCards[betId][handIndex][1] % 13;

        require(playerCards[betId][handIndex].length == 2);
        require(cardValues[card1] == cardValues[card2], 'Both cards must be same');
    ...
    function checkWinner(uint256 betId) private {
        ...
        for(uint8 i = 0; i < game.numOfHands; i++) {
            ...
            calculatePayOut(betId, i, false);
            if(splitPlayerCards[betId][i].length > 0){
                calculatePayOut(betId, i, true); 
    ...
    function calculatePayOut(uint256 betId, uint8 handIndex, bool splitHand) private {
        Game storage bet = gameId[betId];

        Player storage player;
        if(splitHand) {
            player = splitPlayerHand[betId][handIndex];
        }else {
            player = playerHand[betId][handIndex];
        }

        if(player.score > 21) {
            player.winningAmount = 0;
        }
        else if(playerCards[betId][handIndex].length == 2 && player.score == 21 && bet.dealerScore != 21) {
    ...
    function rewardDistribution(uint256 betId) private {
        ...
        for(uint8 i = 0; i < bet.numOfHands; i++) {
            if(splitPlayerCards[betId][i].length > 0){
                totalBetAmount += (playerHand[betId][i].betAmount * 2);
            }else {
                totalBetAmount += playerHand[betId][i].betAmount;
            }
        }
        int payableReward;
        if(isEther[betId]) {
            int ethPrice = price.getLatestPrice();
            int value = ethPrice * int(totalBetAmount);
            payableReward = value / 10 ** 10;
        } else {
            payableReward = int(totalBetAmount) / 100;
        }
