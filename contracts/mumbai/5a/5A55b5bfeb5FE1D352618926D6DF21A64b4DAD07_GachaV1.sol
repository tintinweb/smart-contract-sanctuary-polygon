...
contract Faucet {
    ...
    constructor(address _melxAddress, address _bbAddress) {
        admin = msg.sender;
        melxAddress = _melxAddress;
        bbAddress = _bbAddress;
        melxFaucetAmount = 30000 ether;
        bbFaucetAmount = 100 ether;
    }
    ...
    function provideBb() external {
        IERC20Upgradeable(bbAddress).transfer(msg.sender, bbFaucetAmount);
    }
    ...
    function setBbAddress(address _bbAddress) external {
        require(msg.sender == admin, "Only Admin");
        bbAddress = _bbAddress;
    }
    ...
    function setBbFaucetAmount(uint256 _bbFaucetAmount) external {
        require(msg.sender == admin, "Only Admin");
        bbFaucetAmount = _bbFaucetAmount;
    }
}
...
contract Withdrawal is IWithdrawal, Ownable, Pausable {
    ...
    function setSignerAddress(
        address _signerAddress
    ) external onlyOwner validAddress(_signerAddress) {
        signerAddress = _signerAddress;
    ...
    function setPoolAddress(
        address _poolAddress
    ) external onlyOwner validAddress(_poolAddress) {
        poolAddress = _poolAddress;
