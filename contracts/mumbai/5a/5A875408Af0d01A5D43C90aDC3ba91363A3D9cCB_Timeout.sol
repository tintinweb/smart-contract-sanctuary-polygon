    ...
    ReentrancyGuard
{
    ...
    bool public claim_holders_opened=false;
    bool public claim_game_opened=false;

    uint16 public constant maxSupplyToMint = 9472;

    uint8 private constant maxMintPerWallet_phase03 = 2;

    uint256 public price_by_egg=18 ether;
    uint8 public max_eggs_by_day=10;
    ...
    uint16[] private maxSupplyEvos = [0, 2368, 1184, 592, 296, 148, 74];
    ...
    uint16 private maxFusionsPerDay = 20;

    uint256 public maxTimeAfterTransfert = 172800;

    address private WhiteListSigner =
        0x99A7130dc775dB71E5252dE59F0f156DF1B96d89;
    ...
    mapping(uint256 => uint256) private transferts;
    ...
    mapping(bytes => uint16) private throttling;
    mapping(bytes => uint16) private throttling_buy;

    mapping(address => uint16) public bonuses_fusion;
    ...
    modifier throttleLimit() {
        if(getFusionCounter(msg.sender)<=0)
            revert("too many fusions for today");
        _;        
    }

    function getFusionCounter(address to) public view returns(uint16){
        uint currentDayPart = block.timestamp / 86400;
        bytes memory key = abi.encodePacked(to, currentDayPart);
        return maxFusionsPerDay + bonuses_fusion[to] - throttling[key];
    }

    function setBonusFusion(address to, uint16 quantity) external onlyOwner {
        bonuses_fusion[to] = quantity;
    }
    ...
    function getBuyCounter(address to) public view returns(uint16){
        uint currentDayPart = block.timestamp / 86400;
        bytes memory key = abi.encodePacked(to, currentDayPart);
        return maxFusionsPerDay - throttling_buy[key];
    }

    function setCurrentFusionsCount(address to,uint16 fusions) external onlyOwner{
        uint currentDayPart = block.timestamp / 86400;
        bytes memory key = abi.encodePacked(to, currentDayPart);
        throttling_buy[key]=fusions;
    }

    function setPriceOfEggs(uint256 newPrice) external onlyOwner(){
        price_by_egg=newPrice;
    }

    function setMaxEggsByDay(uint8 maxQuantity) external onlyOwner(){
        max_eggs_by_day=maxQuantity;
    }
    ...
    /**
     * @notice get the freeze timestamp of a token
     *
     */
    function getLastTransfert(uint256 tokenId) public view returns (uint256) {
        return transferts[tokenId];
    }
    /**
     * @notice set the freeze timestamp of a token
     *
     */
    function setLastTransfert(uint256 tokenId,uint256 timestamp) external onlyOwner(){
        transferts[tokenId]=timestamp;
    }


    function activeClaimHolders(bool active) external onlyOwner {
        claim_holders_opened=active;
    }

    function activeClaimGame(bool active) external onlyOwner {
        claim_game_opened=active;
    }
    ...
    function privatesalesmint(
        ...
        if (index + quantity > maxSupplyToMint)
    ...
    function whitelistsalesmint(
        ...
        if (index + quantityToMint > maxSupplyToMint) revert MaxSupplyReach();
    ...
    /**
     * @notice airdrop single egg
     **/
    function airdrop(address to, uint16 quantityToMint) external onlyOwner {
    ...
    /**
     * @notice airdrop full snapshot
     **/
     function airdropSpecial(address to, DataLibrarry.RestoreSnapshotItem memory itemsToRestore) external onlyOwner {
        DataLibrarry.Metadata memory metadata;
        metadata.evolution=itemsToRestore.evolution;
        metadata.types = itemsToRestore.types;
        _mint(to, itemsToRestore.quantity);
        unchecked {
            for(uint256 i=0;i<itemsToRestore.quantity;i++){
                metadatas[index+i]=metadata;
            }            
            index += itemsToRestore.quantity;
        }
    }
        ...
        throttleLimit
    {
        ...
        if (datas1.evolution != datas2.evolution)
            revert("fusion impossible car pas la meme evolution");
        if (datas1.types == datas2.types)
            revert("fusion impossible car de la meme couleur");
 
        if (
            transferts[tokenId0] > 0 && 
            transferts[tokenId0] + maxTimeAfterTransfert > block.timestamp
        ) revert("impossible de fusionner premier token gele");
        if (
            transferts[tokenId1] > 0 && 
            transferts[tokenId1] + maxTimeAfterTransfert > block.timestamp
        ) revert("impossible de fusionner second token gele");
        ...
        if (priceFusion == 0) revert("pas assez cher pour cette fusion");
        if (value < priceFusion) revert("pas assez cher pour cette fusion");
        DataLibrarry.Metadata memory metadata;
        if (nextEvo < 7) {
        ...
        _mint(msg.sender, 1);
        uint currentDayPart = block.timestamp / 86400;
        bytes memory key = abi.encodePacked(msg.sender, currentDayPart);
        if(throttling[key] > maxFusionsPerDay){
            bonuses_fusion[msg.sender]--;
        }else{
            throttling[key]++;
        }
        unchecked {
    ...
    function claimGame(uint256 tokenIdEvo7) external nonReentrant {
        if (!claim_game_opened) revert("Claim for game is not opened");
        if (Address.isContract(msg.sender)) revert SenderIsContract();
        ...
        uint256 gameWinnerPot = gameFunds/2;
        (bool success, ) = payable(address(msg.sender)).call{value: gameWinnerPot}(
            ""
        );
        if (!success) revert FailToTransferGameFunds();
        gameFunds = gameFunds - gameWinnerPot;
        _burn(tokenIdEvo7);
    ...
    function claim(
        ...
        if (!claim_holders_opened) revert("Claim for holders is not opened");
        if (Address.isContract(msg.sender)) revert SenderIsContract();
    ...
    function _afterTokenTransfers(
        ...
        if (from!=0x0000000000000000000000000000000000000000) {
            transferts[startTokenId] = block.timestamp;
        } else {
            if (transferts[startTokenId] > 0) {
                transferts[startTokenId] = 0;
            }
        }
    }
     ...
     * - Only owner of contract can call this function
     */
    function withdrawToWinnerOfFusions(address winner)
        external
        nonReentrant
        onlyOwner
    {
    ...
    /**
     * @notice emergency withdraw of all funds
     *
     * Requirements:
     *
     * - Only owner of contract can call this function
    ...
    function buy(uint8 quantity)
    external
    payable
    {
        if (Address.isContract(msg.sender)) revert SenderIsContract();
        uint256 value = msg.value;
        if(value < quantity*price_by_egg) revert("not enough money");
        if(getBuyCounter(msg.sender)<=0) revert("you allready bough too much today");
        _mint(msg.sender, quantity);
        uint currentDayPart = block.timestamp / 86400;
        bytes memory key = abi.encodePacked(msg.sender, currentDayPart);
        throttling_buy[key]++;
...
library FunctionLib {
  ...
  */
    ...
    returns(uint8)
  {
    ...
    //   delta = indexEvolutionBlue - indexEvolutionPink;
    // }else{
    //   delta = indexEvolutionPink - indexEvolutionBlue;
    // }
    // uint256 sqrt = Math.sqrt(indexEvolutionBlue+indexEvolutionPink);
    // if(sqrt>0){
    //   uint256 max_delta = indexEvolutionBlue+indexEvolutionPink / Math.sqrt(indexEvolutionBlue+indexEvolutionPink);
    //   if(delta<max_delta) return random;
    //   if(random == 0 ){
    //     return 1;
    //   }else{
    //     return 0;
