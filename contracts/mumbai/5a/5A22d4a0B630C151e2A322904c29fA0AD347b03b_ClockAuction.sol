...
contract ClockAuction is Pausable, Ownable, ClockAuctionBase {
    ...
    function isOnAuction(uint256 _tokenId) internal view returns (bool) {
        Auction storage auction = tokenIdToAuction[_tokenId];
...
contract ClockAuctionBase {
        ...
        returns (bool)
    {
        return (nonFungibleContract.ownerOf(_tokenId) == _claimant ||
            nonFungibleContract.owns(_claimant, _tokenId));
    }
...
contract PlantBreeding is UIAMToken {
    ...
    event Pregnant(uint256 parentId, uint256 sireId, uint256 cooldownEndBlock);

    event Birth(uint256 tokenId, uint16 generation, address[] owners);

    uint256 public autoBirthFee = 20000000000000;
    ...
    function approveBreedingRights(address _addr, uint256 _tokenId)
        ...
        whenNotPaused
    {
        require(ownerOf(_tokenId) == msg.sender || _owns(msg.sender, _tokenId), "You are not the owner");
    ...
    function isPregnant(uint256 _plantId) external view returns (bool) {
        require(_plantId > 0);
        // A Plant is pregnant if and only if this field is set
        return plants[_plantId].siringWith != 0;
    }

    function isReadyToBreed(uint256 _plantId) external view returns (bool) {
        require(_plantId > 0);
    ...
    function _breedWith(uint256 _matronId, uint256 _sireId) internal {
        ...
        // Clear breeding rights feature removed

        // Every time a Plant gets pregnant, counter is incremented.
        seeds++;

        // Emit the pregnancy event.
        emit Pregnant(_matronId, _sireId, matron.cooldownEndBlock);
    }
    ...
    function breedWithAuto(
        uint256 _matronId, uint256 _sireId, string memory _genesURI, string memory _tokenURI
        ...
        _breedWithAuto(_matronId, _sireId, _genesURI, _tokenURI, msg.sender);
    }
    ...
    function breedWithAutoByCFO(
        ...
        require(verifyBreedSignature(_matronId, _sireId, _tokenURI, _genesURI, nounce, _signer, signature), "Invalid signature");

        addNounce(_signer);

        // Breed
        _breedWithAuto(_matronId, _sireId, _genesURI, _tokenURI, _signer);
    ...
    function _breedWithAuto(
        ...
        seedsMetadata[_matronId] = Seed(_genes, _tokenURI);
        ...
        returns (uint256)
    {
        ...
        Plant storage matron = plants[_matronId];
        Seed storage seedMetadata = seedsMetadata[_matronId];
        ...
        uint256 plantId = _createPlant(
            ...
            seedMetadata.genes,
            address(this),
            seedMetadata.tokenURI,
            1
        );

        // add matrons owners and sire owners as new plants owner
        if (ownerOf(_matronId) != address(this)) {
            addOwner(plantId, ownerOf(_matronId));
        } else {
            address[] memory parentOwners = getOwners(_matronId);
            setOwners(plantId, parentOwners);
        }
        if (ownerOf(matron.siringWith) != address(this)) {
            addOwner(plantId, ownerOf(matron.siringWith));
        } else {
            address[] memory parentSireOwners = getOwners(matron.siringWith);
            for (uint256 i = 0; i < parentSireOwners.length; i++) {
                addOwner(plantId, parentSireOwners[i]);
            }
        }
        ...
        delete seedMetadata.tokenURI;
        delete seedMetadata.genes;
        ...
        IERC20 agt = IERC20(alleleGroupToken);
        uint balance = agt.balanceOf(address(this));

        if(balance >= autoBirthFee) {
            agt.transfer(
                msg.sender,
                autoBirthFee
            );
...
contract UIAMToken is AllelePlantBase, GeoLocation {
    uint256 public mintingFee = 100 * 1e18;
    uint256 public gen2CreateFee = 200 * 1e18;
    ...
    function mintUIAM(
        ...
        _mintUIAM( _tokenURI, _genes, lat, long, msg.sender);
    }

    function mintUIAMByCFO(string memory _tokenURI, string memory genesURI, int256 lat, int256 long, uint256 nounce, address signer, bytes memory signature) external onlyCfo() {
        require(verifyMintSignature(_tokenURI,genesURI,lat,long,nounce,signer,signature),"Invalid signature");
        addNounce(signer);
        _mintUIAM( _tokenURI, genesURI, lat, long, signer);
    }

    function _mintUIAM(string memory _tokenURI, string memory _genesURI, int256 lat, int256 long, address _signer) internal {
        uint256 tokenId = _createPlant(0, 0, _genesURI, _signer, _tokenURI, 1);
        // add location
        setLocation(tokenId, lat, long);
        addOwner(tokenId, _signer);

        emit Born(tokenId);
    }

    function createGen2(uint256 _parentId, uint256 total) external {
        require(
            alleleGroupToken != address(0),
            "AGT token address not set yet"
        );
    ...
    function CreateGen2ByCFO(uint256 _parentId, uint256 total, uint256 nounce, address signer, bytes memory signature) external onlyCfo() {
    ...
    function _createGen2(uint256 _parentId, uint256 total, address _signer) internal {
        ...
        require(
            ownerOf(_parentId) == _signer || _owns(_signer, _parentId),
            "UIAMToken : You are not the owner of the parent Plant"
        );

        Plant memory plant = plants[_parentId];

        address[] memory parentOwners = getOwners(_parentId);

        for (uint256 i = 0; i < total; i++) {
            uint256 tokenId = _createPlant(
                _parentId,
                plant.sireId,
                plant.genes,
                address(this),
            ...
            setOwners(tokenId, parentOwners);
            emit Born(tokenId);
        }
    }
    ...
    AllelePlantEIP712
{
    ...
    uint32 public cooldown = uint32(2 minutes);

    // An approximation of currently how many seconds are in between blocks.
    uint256 public secondsPerBlock = 5;

    mapping(uint256 => Plant) internal plants;
    mapping(uint256 => Seed) internal seedsMetadata;

    uint256 internal totalPlants;
    ...
    function transferFrom(
        ...
        if (ownerOf(_tokenId) == address(this)) {
            require(_tokenId != 0, "Can not transfer zero token");

            // Fractional Ownership changing
            // Check _owns
            require(_owns(_from, _tokenId), "You are not owner");
            // replace ownership from owners array
            _replaceOwner(_from, _to, _tokenId);
            // Keep breeding permission to old user
            addBreedingPermission(_tokenId, _from);

            // emit Fractional Transfer event
        } else {
...
contract GeoLocation {
    ...
    event LocationAdded(uint256 indexed id, int256 lat, int256 long);
    ...
    function setLocation(
        ...
        plantLocation[_tokenId] = Location({lat: lat, long: long});

        emit LocationAdded(_tokenId, lat, long);
    }
}
...
contract AllelePlantOwnership is Ownable {
    mapping(uint256 => address[]) private ownersToPlant;
    mapping(uint256 => address[]) private breedingPermission;
    address public breedingAuctionContract;

    event BreederAdded(uint256 indexed tokenId, address _addr);

    // set breeding auction contract
    function setBreedingAuction(address _contract) public onlyOwner {
        breedingAuctionContract = _contract;
    }

    // List all owners of a plant
    function getOwners(uint256 _tokenId)
        public
        view
        returns (address[] memory)
    {
        return ownersToPlant[_tokenId];
    }

    // Set new owners of a plant. It replace the previous owners
    function setOwners(uint256 _tokenId, address[] memory _owners) internal {
        ownersToPlant[_tokenId] = _owners;
    }

    // Push a new owners of a plant.
    function addOwner(uint256 _tokenId, address _owner) internal {
        ownersToPlant[_tokenId].push(_owner);
    }

    // Check if an address is owner of a token by tokenId
    function _owns(address _user, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
        address[] memory owners = ownersToPlant[_tokenId];
    ...
    // Check if an address is owner of a token by tokenId
    function owns(address _user, uint256 _tokenId) public view returns (bool) {
        return _owns(_user, _tokenId);
    }

    // Don't explicitly check owns , please do checking when calling it
    function _replaceOwner(
        address _from,
        address _to,
        uint256 _tokenId
    ) internal {
        require(_from != _to, "Can not transfer to the same user");
        address[] storage owners = ownersToPlant[_tokenId];
        uint256 totalOwners = owners.length;
    ...
    // get all the addresses that have breeding permission of a plant
    function getBreedingPermissions(uint256 _tokenId)
        public
        view
        returns (address[] memory)
    {
        return breedingPermission[_tokenId];
    }


    // set new breeding permission list to a plant
    function setBreedingPermissions(
        uint256 _tokenId,
        address[] memory _breeders
    ) internal {
        breedingPermission[_tokenId] = _breeders;
    }

    // add an address for breeding permission of a plant
    function addBreedingPermission(uint256 _tokenId, address _owner) internal {
        breedingPermission[_tokenId].push(_owner);
    ...
    function addBreedingRights(uint256 _tokenId, address _owner) external {
        require(
            _owns(msg.sender, _tokenId) || breedingAuctionContract == msg.sender
        );
    ...
    function _hasBreedingPermissions(address _user, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
        address[] memory permited = breedingPermission[_tokenId];
        uint256 totalOwners = permited.length;
        bool isPermitted;
    ...
    // Don't explicitly check owns , please do checking when calling it
    function _replaceBreedingPermission(
        address _from,
        address _to,
        uint256 _tokenId
    ) internal {
        require(_from != _to, "Can not transfer to the same user");
        address[] storage permited = breedingPermission[_tokenId];
        uint256 totalPermission = permited.length;

        for (uint256 i = 0; i < totalPermission; i++) {
            if (permited[i] == _from) {
                permited[i] = _to;
                break;
            }
...
contract AllelePlantEIP712 {
    ...
    bytes32 private constant MINT_TYPEHASH = keccak256("Mint(string tokenURI,string genesURI,int256 lat,int256 long,uint256 nounce)");
    bytes32 private constant BREED_TYPEHASH = keccak256("Breed(uint256 tokenA,uint256 tokenB,string tokenURI,string genesURI,uint256 nounce)");
    ...
    function hashBreed(Breed memory breed) internal pure returns (bytes32) {
        return keccak256(abi.encode(
            ...
            keccak256(bytes(breed.genesURI)),
            breed.nounce
        ));
    }
    ...
    function verifyMintSignature(string memory tokenURI, string memory genesURI, int256 lat, int256 long, uint256 nounce, address signer, bytes memory signature) public view returns (bool) {
    ...
    function verifyBreedSignature(uint256 tokenA, uint256 tokenB, string memory tokenURI, string memory genesURI, uint256 nounce, address signer, bytes memory signature) public view returns (bool) {
        if(nounces[signer] != nounce){
            return false;
        }
