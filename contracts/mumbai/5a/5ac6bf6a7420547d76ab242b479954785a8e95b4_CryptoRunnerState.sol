...
contract CryptoRunnerState is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function getClock(uint256 tokenId_) public view returns (uint256) {
        ...
        if(!cryptoRunner.initialized) {
            ...
            return
                (((block.timestamp - runsById[cryptoRunner.currentRunId].startTime) / 60) * clockRegen) + cryptoRunner.remainingClock;
        } else {
            return
                (((block.timestamp - runsById[cryptoRunner.finishedRuns[cryptoRunner.finishedRuns.length - 1]].endTime) / 60) * clockRegen) + cryptoRunner.remainingClock;
        }
    }
