...
/**
 * @title ERC-721 Non-CLOUDHEADZble Token Standard, optional metadata extension
...
/**
 * @dev Implementation of https://eips.ethereum.org/EIPS/eip-721[ERC721] Non-CLOUDHEADZble Token Standard, including
...
/**
 * @title ERC-721 Non-CLOUDHEADZble Token Standard, optional enumeration extension
 * @dev See https://eips.ethereum.org/EIPS/eip-721
 */
...
contract CLOUDHEADZ is ERC721Enumerable, Ownable, ReentrancyGuard {
    using Counters for Counters.Counter;
    using Strings for uint256;

    uint256 public constant CLOUDHEADZ_PUBLIC = 10000;
    uint256 public constant CLOUDHEADZ_MAX = CLOUDHEADZ_PUBLIC;
    ...
    Counters.Counter private _publicCLOUDHEADZ;

    constructor() ERC721("CLOUDHEADZ", "CH") {
    ...
    {
        require(
            _publicCLOUDHEADZ.current() < CLOUDHEADZ_PUBLIC,
            "Purchase would exceed CLOUDHEADZ_PUBLIC"
        );

        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 tokenId = _publicCLOUDHEADZ.current();

            if (_publicCLOUDHEADZ.current() < CLOUDHEADZ_PUBLIC) {
                _publicCLOUDHEADZ.increment();
                _safeMint(to, tokenId);
            }
        }
    }
