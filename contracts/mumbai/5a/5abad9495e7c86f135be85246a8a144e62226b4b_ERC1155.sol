...
contract ERC1155 is Ownable, IERC165, IERC1155, IERC1155MetadataURI {
    ...
    function createItems(string[] calldata _uris) external onlyOwner {
        for (uint256 i; i < _uris.length; i++) {
            _items[itemsLength] = Item(true, _uris[i], 0);
            emit URI(_uris[i], itemsLength);
            itemsLength++;
        }
    }
    ...
    function updateItem(uint256 id, string calldata _uri) external onlyOwner {
        if (!_items[id].exist) revert ItemNotExist();
        _items[id] = Item(true, _uri, _items[id].supply);
        emit URI(_uri, id);
    }
    ...
    function mintBatch(
        ...
        uint256[] calldata amounts,
        bool doSafeTransferAcceptanceCheck,
        bytes calldata data
    ) external onlyOwner {
        for (uint256 i; i < itemsIds.length; i++) {
            ...
            _balances[itemsIds[i]][to[i]] += amounts[i];

            if (doSafeTransferAcceptanceCheck)
