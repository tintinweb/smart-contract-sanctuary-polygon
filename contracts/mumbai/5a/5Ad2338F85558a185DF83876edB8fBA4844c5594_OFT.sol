...
abstract contract LzApp is Ownable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    // TODO (Must) Shouldn't just be uint16[] as result of existing remotes?
    function getTrustedRemotes(uint16[] memory _chainIds) external view returns (bytes[] memory) {
        bytes[] memory remotes;
        for (uint i = 0; i < _chainIds.length; i++) {
            remotes[i] = trustedRemoteLookup[_chainIds[i]];
        }
        return remotes;
    }
}
