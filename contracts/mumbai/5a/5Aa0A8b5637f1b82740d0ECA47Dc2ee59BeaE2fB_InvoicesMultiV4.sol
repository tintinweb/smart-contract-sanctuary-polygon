...
abstract contract BaseInvoicesToken is BaseProxy {
    ...
    function feeCalculation (uint256 _feeConst, uint8 _decimal) internal view virtual returns (uint256) {
        ...
        return _feeConst * 10 ** (_decimal - 18);
    }
}
...
abstract contract IERC20UpgradeableExtented is IERC20Upgradeable {
...
import "../../common/IERC20UpgradeableExtented.sol";
...
contract InvoicesMultiV4 is InvoicesMultiV3, BaseInvoicesToken {
    ...
    function _pay(
        ...
        Invoice memory _invoice = invoices[_invoice_id];
        IERC20Upgradeable _token = IERC20Upgradeable(address(usdt));
        IERC20UpgradeableExtented _tokenDec = IERC20UpgradeableExtented(address(usdt));
        if (invoiceToken[_invoice_id] != address(0)) {
            _token = IERC20Upgradeable(invoiceToken[_invoice_id]);
            _tokenDec = IERC20UpgradeableExtented(invoiceToken[_invoice_id]);
        }
        ...
        (uint256 weiAmount, uint256 feeAmount) = getweiAmount(
            ...
            fee,
            invoiceCreator[_invoice_id],
            feeCalculation(feeConst, _tokenDec.decimals())
        );
