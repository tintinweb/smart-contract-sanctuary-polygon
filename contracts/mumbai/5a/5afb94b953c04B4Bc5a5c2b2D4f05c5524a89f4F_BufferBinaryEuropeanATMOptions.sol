    ...
    Governable
{
    ...
    function transferFrom(
        ...
        uint256 nextAllowance = allowance(_sender, msg.sender).sub(
            _amount,
     ...
     */
    function setProjectOwner(address account) external {
    ...
    function _update_liquidity(address account) internal {
        uint256 len = liquidityPerUser[account].lockedAmounts.length;
        uint256 index = liquidityPerUser[account].lastUnlockedAmountIndex;
        for (uint256 n = index; n < len; n++) {
            if (
                ...
                liquidityPerUser[account].unlockedAmount += liquidityPerUser[
                    account
                ].lockedAmounts[n].amount;
                liquidityPerUser[account].lastUnlockedAmountIndex = n + 1;
            } else {
                break;
            }
        }
    ...
    function get_unlocked_liquidity(address account)
        external
        view
        returns (uint256 unlocked_amount)
    {
        uint256 len = liquidityPerUser[account].lockedAmounts.length;
        unlocked_amount = liquidityPerUser[account].unlockedAmount;
        uint256 index = liquidityPerUser[account].lastUnlockedAmountIndex;
        for (uint256 n = index; n < len; n++) {
            if (
                liquidityPerUser[account].lockedAmounts[n].timestamp +
                ...
                unlocked_amount += liquidityPerUser[account]
                    .lockedAmounts[n]
                    .amount;
            } else {
                break;
     ...
     * @param tokenXAmount  Amount of X to receive
     */
    function adminWithdraw(address user, uint256 tokenXAmount) external {
    ...
    function _beforeTokenTransfer(
        ...
        if (!isHandler[from] && !isHandler[to] && from != address(0)) {
            require(
                ...
                "Transfer of funds in lock in period is blocked"
            );
            liquidityPerUser[from].unlockedAmount -= value;
...
contract OptionConfigBinaryV2 is Ownable, IOptionsConfig {
    ...
    uint256 public referralDiscountPercentage = 0;
    ...
    /**
     * @notice Used for adjusting the referralDiscountPercentage
     * @param value New referralDiscountPercentage
     */
    function setReferralDiscountPercentage(uint256 value) external onlyOwner {
        referralDiscountPercentage = value;
        emit UpdateReferralDiscountPercentage(value);
    }
    ...
    function setSettlementFeePercentageForDown(uint256 value)
        ...
        onlyOwner
    {
        require(value < 20e2, "SettlementFeePercentageForDown is too high");
    ...
    function setSettlementFeePercentageForUp(uint256 value) external onlyOwner {
        require(value < 20e2, "SettlementFeePercentageForUp is too high");
    ...
    ERC721URIStorage
{
    ...
    mapping(TraderNFT.Tier => uint256) public traderNFTDiscount;

    mapping(uint256 => SlotDetail) public slotDetails;
    uint256 internal contractCreationTimestamp;
    mapping(uint256 => uint256) public settlementFeeDiscount;
    ...
    constructor(
        ...
        traderNFT = _traderNFT;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        traderNFTDiscount[TraderNFT.Tier.Diamond] = 75;
        traderNFTDiscount[TraderNFT.Tier.Diamond] = 50;
        traderNFTDiscount[TraderNFT.Tier.Diamond] = 35;
        traderNFTDiscount[TraderNFT.Tier.Diamond] = 25;
    }
    ...
    function getStrikeForATMOption(uint256 expectedStrike, uint256 slippage)
        ...
        returns (uint256 strike)
    {
        require(slippage <= 100, "Wrong slippage");
    ...
    function checkParams(
        ...
        returns (uint256 amount, uint256 revisedFee)
    {
        (uint256 unitFee, uint256 settlementFee, ) = fees(
            period,
            10**decimals(),
            strike,
            true,
        ...
        // TODO : check if the user actually has trader nft
        if (isTraderNFT) {
            unitFee =
                unitFee -
                ((settlementFee *
                    traderNFTDiscount[
                        traderNFT.tokenTierMappings(traderNFTID)
                    ]) / 100);
        }
        if (
            config.referralDiscountPercentage() > 0 &&
            referrer != user &&
            referrer != owner()
        ) {
            unitFee =
                unitFee -
                ((settlementFee * config.referralDiscountPercentage()) / 100);
        }
        amount = (totalFee * 10**decimals()) / unitFee;

        revisedFee = totalFee;
        if (amount > (pool.availableBalance() * 5) / 100) {
            require(allowPartialFill, "O23");
            amount = (pool.availableBalance() * 5) / 100;
            (revisedFee, , ) = fees(period, amount, strike, true, isAbove);
        }
        ...
        require(
            revisedFee * 365 days * 100 > amount * (period) * minimumYield,
            "O2"
        );
    }
    ...
    function createFromRouter(
        ...
        ISettlementFeeDisbursal(config.settlementFeeDisbursalContract())
            .distributeSettlementFee(settlementFee, referrer, user, owner());
