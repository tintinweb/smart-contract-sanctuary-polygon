  ...
  UUPSUpgradeable 
{
  ...
  address public _PAGETREE_MARKETPLACE;
    ...
    whenNotPaused
  {
    ...
    setApprovalForAll(_PAGETREE_MARKETPLACE, true);
  ...
  // Set new URI of a specific book (Only Page Tree can call it)
  ...
  function setMarketAddr(address marketAddr)
    ...
    onlyRole(OPERATOR_ROLE)
  {
    _PAGETREE_MARKETPLACE = marketAddr;
  }
  ...
  modifier notBlacklisted(address userAddress) {
    require(_BLACKLIST[userAddress] == false, "Account is blacklisted");
    _;
  }

  function addToBlacklist(address userAddress) 
    public 
  ...
  function removeFromBlacklist(address userAddress) 
    public 
    onlyRole(CENSOR_ROLE)  
  {
    _BLACKLIST[userAddress] = false;
    emit RemovedFromBlacklist(userAddress);
  }

  function isBlacklisted(address userAddress) 
    public view 
    returns(bool) 
  {
    return _BLACKLIST[userAddress];
  }
