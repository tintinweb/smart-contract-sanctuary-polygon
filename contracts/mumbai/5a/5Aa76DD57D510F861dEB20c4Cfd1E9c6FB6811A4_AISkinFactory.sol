    ...
    ReentrancyGuard
{
    ...
    function _publishItem(
        ...
        if (preMinted > 0) {
            _mintItemOnPublish(creator, collection, tokenId, preMinted);
        }
    }
    ...
    function _mintItemOnPublish(
        ...
        IBaseCollection.Item memory item = IBaseCollection(collection)
            .getItemInfo(tokenId);

        if (preMinted > _publishFreebiesAmount) {
            uint256 totalPrice = (preMinted - _publishFreebiesAmount) *
                item.priceInWei;
            uint256 preMintFee = (totalPrice * item.shareFee) / 10_000;
            if (item.tokenPayment != address(0)) {
                ...
                if (msg.value != preMintFee)
                    revert InsufficientPaymentBalance();
                Address.sendValue(payable(_treasury), preMintFee);
            }
        }
