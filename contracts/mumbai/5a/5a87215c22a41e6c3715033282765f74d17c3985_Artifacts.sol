...
contract Artifacts is ERC1155, Ownable, ERC1155Burnable, ERC1155Supply {
  ...
  function safeBatchTransferFrom(
    ...
    for (uint256 i = 0; i < length; ) {
      uint256 availableBalance = balanceOf(from, ids[i]) - lockedArtifacts[ids[i]][from];
  ...
  function burn(
    ...
    // Check available balance, then burn artifacts
    uint256 availableBalance = balanceOf(account, id) - lockedArtifacts[id][account];
  ...
  function burnBatch(
    ...
    // Check available balances for each id, then burn artifacts
    uint256 length = ids.length;
    for (uint256 i = 0; i < length; ) {
      uint256 availableBalance = balanceOf(account, ids[i]) - lockedArtifacts[ids[i]][account];
