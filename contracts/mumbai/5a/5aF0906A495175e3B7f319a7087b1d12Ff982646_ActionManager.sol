    ...
    ReentrancyGuardUpgradeable
{
     ...
     */
    event SmartPoolCreationSuccess(address indexed _msgSender);
    ...
    function smartPoolCreation(
        ...
        address crpoolAddress = address(ICRPFactory(crpFactoryAddress).newCrp(bFactoryAddress, _poolParams, _rights));

        // Create the CRPool xToken
        _deployXPoolToken(crpoolAddress, _poolParams.poolTokenName);

        // Approve tokens on the CRPoolProxy ?

        // Create the CRPool
        ICRPoolProxy(crpoolProxyAddress).createPool(crpoolAddress, _initialSupply);

        // Register on the BRegistry
        txOk = _addPoolPair(address(ICRPool(crpoolAddress).bPool()), _poolParams.constituentTokens);
        require(txOk, "addPoolPair failed");
        txOk = _sortPools(_poolParams.constituentTokens);
        require(txOk, "sortPools failed");
