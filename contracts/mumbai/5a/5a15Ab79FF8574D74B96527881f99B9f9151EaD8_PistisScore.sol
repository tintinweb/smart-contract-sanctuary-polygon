    ...
    IERC4906
{
    ...
    function updateTokenLoyaltyPointsByProvider(
        ...
        require(checkProviderTrustedAddress(_provider, msg.sender) == true, "Sender not in trusted addresses");
    ...
    function removeProviderTrustedAddress(uint256 _provider, address _trustedAddress) external onlyRole(DAO_ROLE) {
        require(providers[_provider].exists, "Provider doesn't exist");

        uint256 trustedAddressesLength = providers[_provider].trustedAddresses.length;
        ...
        for (uint256 i = 0; i < trustedAddressesLength; i++) {
            if (providers[_provider].trustedAddresses[i] == _trustedAddress) {
                if (i != trustedAddressesLength - 1) {
                    providers[_provider].trustedAddresses[i] = providers[_provider].trustedAddresses[trustedAddressesLength - 1];
                }
    ...
    function getProviderTrustedAddresses(uint256 _provider)
        public
        view
        returns (address[] memory)
    {
    ...
    function checkProviderTrustedAddress(uint256 _provider, address _wallet)
    public
    view
    returns (bool)
    {
    ...
    function getTokenProviderData(uint256 _tokenId, uint256 _provider)
        public
        view
    ...
    function baseSection(uint256 _tokenId) private view returns (bytes memory) {
        ...
        string memory baseName = bytes(name).length != 0 ? string(name) : string.concat("Pistis #", _tokenId.toString(), " token");
        string memory imageURI = string.concat(tokenImageRenderURL, _tokenId.toString());

        return abi.encodePacked(
            "{",
            '"description": "Pistis Score token",',
            '"external_url": "', externalURL, _tokenId.toString(), '",',
            '"name": "', baseName, '.pistis",',
            '"image": "', imageURI, '",'
        );
    }

    function attributesSection(uint256 _tokenId) private view returns (bytes memory) {
        return abi.encodePacked(
            '"attributes": [',
            '{ "trait_type": "Global rating", "value": ', tokens[_tokenId].globalRating.toString(), ' },',
            '{ "display_type": "date", "trait_type": "Last updated date", "value": ', tokens[_tokenId].ratingUpdatedDate.toString(), ' }',
            '] }'
        );
    }
