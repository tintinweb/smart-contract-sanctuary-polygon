...
contract BoredEyeIsERC721 is Initializable, ERC721Upgradeable, ERC721RoyaltyUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
  ...
  function safeMintNFT(uint96 _royaltyAmount, uint256 _sellPrice, string memory _uri) public returns(uint256) {
    address to = msg.sender;
    require(to == MARKETPLACE_ADDRESS, "Sorry, you cannot call this function.");

    uint256 tokenId = _tokenIdCounter.current();

    super._setTokenRoyalty(tokenId, to, _royaltyAmount);
    super.royaltyInfo(tokenId, _sellPrice);
