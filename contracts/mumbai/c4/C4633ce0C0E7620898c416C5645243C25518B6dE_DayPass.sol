...
contract DayPass is Ownable, ERC721Enumerable, ReentrancyGuard {
    ...
    constructor(address _verificationAdmin) public ERC721("DayPass", "DP") {
        verificationAdmin = _verificationAdmin;
    }

    //Modifier

    modifier checkVerificationAdmin(){
        require(msg.sender == verificationAdmin, "Authentication Failed");
        _;
    }
    ...
    function mintPassport(address mintTo)
        public
        checkVerificationAdmin
        nonReentrant
        returns (uint256)
    {
        require(!mintedByAddress[mintTo], "Minting limit reached for this address.");
        _tokenIds.increment();
        uint256 newItemId = _tokenIds.current();
        _safeMint(mintTo, newItemId);
        mintedByAddress[mintTo] = true;        
        return newItemId;
    }
