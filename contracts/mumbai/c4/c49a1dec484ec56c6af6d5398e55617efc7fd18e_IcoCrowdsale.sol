...
contract Crowdsale is ReentrancyGuard,Pausable {
     ...
     */
    constructor (uint256 rate, address payable wallet, IERC20 token,uint256 maxSupply) public {
        require(rate > 0);
    ...
    function _getTokenAmount(uint256 weiAmount) internal view returns (uint256) {
        uint256 amoutWithoutBonus = weiAmount.mul(_rate);
        return amoutWithoutBonus;
    }
...
contract IcoCrowdsale is MintedCrowdsale { 
    event CrowdsaleFinalized();
    ...
    public
    Crowdsale(rate, wallet, token,maxSupply)  
    {
