...
contract GW3SArtistSummit is ERC721Enumerable, Ownable {
    ...
    function getMetadatas(uint256 tokenId) public view returns(
        Svg memory svg,
        string memory type_,
        string memory title,
        string memory text) {
        svg.front = metadatas[tokenId].svg.front; 
        svg.back = metadatas[tokenId].svg.back;
        svg.left = metadatas[tokenId].svg.left;
        svg.right = metadatas[tokenId].svg.right;
        type_ = metadatas[tokenId].type_; 
        title = metadatas[tokenId].title; 
        text = metadatas[tokenId].text; 
    }
