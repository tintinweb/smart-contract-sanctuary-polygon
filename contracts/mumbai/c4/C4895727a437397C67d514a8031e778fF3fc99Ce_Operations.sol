...
function addReply(uint256 messageId, string memory response, Message memory messageOriginal, bool setRep, uint256 timestamp) private {
   ...
   function sendMessageToGroup(address[] memory receiver, string[] memory subject, string []memory message, string []memory cciMessages,bool isShareble, string[] memory fileHash, string memory emailGroup, address[] memory cciReceivers, uint256 timestamp) external {
        ...
        for(uint i = 0; i<receiver.length; i++){
            ...
            Message memory message = Message(messageCount, msg.sender, receiver[i], subject[i], message[i], messageTimestamp, false, isShareble,
                new address[](0),
                messageCount, fileHash[i], emailGroup,DeletionStatus.NotDeleted);
                rep[messageCount] = false;
        messages.push(message);
        messageCount++;
        }
        for(uint i = 0; i<cciReceivers.length; i++){
            ...
            Message memory message = Message(messageCount, msg.sender, cciReceivers[i], subject[i], cciMessages[i], messageTimestamp, false, isShareble,
                new address[](0),
                messageCount,fileHash[i], '',DeletionStatus.NotDeleted);
                rep[messageCount] = false;
        messages.push(message);
        messageCount++;
        }
...
contract Operations {
    ...
    constructor() {
        structures = Structures(0x6C330e24A6BDfDf8017994C134E20FE35C38D03A);
        chat = Chat(0xBDeb0Fb4F172153Df2c23F2e1e358027ff3b65ef);
    }
