 ...
 * @dev Factory to be used by operators to deploy arbitrary Sygnum NFT contracts.
 */
...
/**
 * @title NFTFactoryV4
 * @dev Contract for deploying and managing NFT collections
 */
contract NFTFactoryV4 is NFTFactoryV3 {
    event ImplementationTypeChanged(address indexed _newImplementation, uint256 _contractType);

    mapping(uint256 => address) contractType;

    /**
     * @dev Function deploying a new collection proxy, initializing it with the specified parameters,
     * recording the new deployed collection locally and emitting an event
     * @param data byte encoded parameters to deploy a collection
     * @param _baseOperators address of the base operator contract
     ...
     * @dev Function to change the implementation for future collections, callable only by owner
     * @param _newImplementation New implementation address
     * @param _contractType slot of the implementation to update
     */
    function changeTypeImplementation(address _newImplementation, uint256 _contractType) external onlyOperator {
        if (_newImplementation == address(0)) revert NFTFactoryNewImplementationIsZeroAddress();
        if (_newImplementation == implementation) revert NFTFactoryNewImplementationIsSameAddress();
        contractType[_contractType] = _newImplementation;

        emit ImplementationTypeChanged(_newImplementation, _contractType);
    }
     ...
     * @param data byte encoded parameters to deploy a collection
     */
    function newCollection(bytes calldata data, uint256 _contractType) public virtual onlyOperator returns (address) {
        return _newCollection(data, collectionBaseOperators, contractType[_contractType]);
    }
