...
contract SacredBeast is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    IERC20 immutable foodToken;
    IERC20 immutable evmToken;
    ...
    uint256 public constant BASE_EXP = 1e6;

    // week that rewards are streamed over
    ...
    mapping(uint256 => mapping(uint256 => uint256)) public unlocks;
    ...
    event Reclaim(uint256 tokenId, uint256 amount, uint256 week);
    event RewardPaid(address claimer, address rewardToken, uint256 reward);
    event Evole(uint256 tokenId, uint256 evolForm, uint256 race);
    event NewLockChoice(uint256 index, uint256 week, uint256 multiplier);
    event UpdateLockChoice(uint256 index, uint256 week, uint256 multiplier);
  ...
  |__________________________________*/
    ...
    function addLockChoice(uint256 _week, uint256 _multiplier)
        public
        onlyOwner
    {
        lockChoices.push(LockChoice({week: _week, multiplier: _multiplier}));
        emit NewLockChoice(lockChoices.length, _week, _multiplier);
    }
    ...
    function setLockChoice(
        ...
        lockChoice.week = _week;
        lockChoice.multiplier = _multiplier;
        emit UpdateLockChoice(_lockIndex, _week, _multiplier);
    }
  ...
  |__________________________________*/
    ...
    function mintBeast(uint256 _numberOfTokens) public {
        ...
        require(
            evmToken.transferFrom(msg.sender, seller, payAmount),
            "Wrong pay amount"
        );
    ...
    /**
        @notice Get the Token URI for each tokenId
     */
        ...
        returns (string memory)
    {
        ...
        string memory name = string(
            abi.encodePacked("SACRED BEAST #", _tokenId.toString())
        );
        string memory exp = nft[_tokenId].exp.toString();
        string memory race = nft[_tokenId].race.toString();
        string memory evolForm = nft[_tokenId].evolForm.toString();
    ...
    /**\
        @notice Deposit tokens into the contract to create a new lock for the nft.
        @param _tokenId tokenId to create a new lock for (does not have to be the caller)
        @param _amount Amount of tokens to lock. This balance transfered from the caller.
        @param _lockIndex The index of lockChoices. This will use for calculate unlock date and exp.
     */
    function feed(
        uint256 _tokenId,
        uint256 _amount,
        ...
        require(_amount > 0, "Cannot feed 0");
        ...
        uint256 statWeek = getWeek();
        uint256 unlockWeek = statWeek + lockChoice.week;
        unlocks[_tokenId][unlockWeek] += _amount;

        emit Feed(_tokenId, _amount, unlockWeek, gainExp);
    }
    ...
    function rewardPerToken(address _rewardsToken)
        ...
        returns (uint256)
    {
        return _rewardPerToken(_rewardsToken, lockedSupply);
    }
    ...
    function getRewardForDuration(address _rewardsToken)
        ...
        returns (uint256)
    {
        return rewardData[_rewardsToken].rewardRate * WEEK;
    }
    ...
    /**
        @notice Withdraw unlock token.
        @param _tokenId tokenId to reclaim unlock token.
        @param _weeks array of week to withdraw unlock token.
     */
    function reclaimLocks(uint256 _tokenId, uint256[] memory _weeks)
        public
        nonReentrant
        onlyExist(_tokenId)
        onlyNftOwner(_tokenId)
    {
        ...
        uint256 thisweek = getWeek();
        for (uint256 i; i <= _weeks.length; i++) {
            if (_weeks[i] < thisweek) {
                amount += unlocks[_tokenId][_weeks[i]];
                unlocks[_tokenId][_weeks[i]] = 0;
                emit Reclaim(_tokenId, amount, _weeks[i]);
            }
        }
    ...
    /**
        @notice Evole form of NFT.
        @dev The exp needed for each form is calculated by BaseExp * (5 ** EvolForm) * (Decimal 10 ** 18)
     */

    function evolutionNFT(uint256 _tokenId)
        public
        onlyExist(_tokenId)
        onlyNftOwner(_tokenId)
    {
        ...
        require(
            nft[_tokenId].exp >
                (BASE_EXP) * (5**(nft[_tokenId].evolForm)) * (10**18)
        );
        ...
        emit Evole(_tokenId, nft[_tokenId].evolForm, nft[_tokenId].race);
    }
  ...
  |__________________________________*/
    ...
    function _calConsumeEXP(uint256 _amount, LockChoice memory _choice)
        ...
        returns (uint256)
    {
        return _choice.week * _choice.multiplier * _amount;
    }
    ...
    function _notifyReward(address _rewardsToken, uint256 reward) internal {
        if (block.timestamp >= rewardData[_rewardsToken].periodFinish) {
            rewardData[_rewardsToken].rewardRate = reward / WEEK;
        } else {
            uint256 remaining = rewardData[_rewardsToken].periodFinish -
                block.timestamp;
            uint256 leftover = remaining * rewardData[_rewardsToken].rewardRate;
            rewardData[_rewardsToken].rewardRate = (reward + leftover) / (WEEK);
        }

        rewardData[_rewardsToken].lastUpdateTime = block.timestamp;
        rewardData[_rewardsToken].periodFinish = block.timestamp + WEEK;
    }

    /**
        @notice Added to support withdrawing ETH
     */
