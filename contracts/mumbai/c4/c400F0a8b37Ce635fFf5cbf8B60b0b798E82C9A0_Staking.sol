...
contract Staking is Ownable {
    ...
    */
    function calculateReward(uint256 id) public view returns (uint256) {
        require(!stakesOfUser[msg.sender][id].isWithdrawn, "This stake was withdrawn!");
        
        uint256 stakedTime = stakesOfUser[msg.sender][id].stakedTime;
        uint256 currentTime = block.timestamp;
        uint256 timeDiff = currentTime.sub(stakedTime);

        if(timeDiff <= 2592000) {
            return stakesOfUser[msg.sender][id].amount.mul(tpsRatios[0]).mul(timeDiff).div(1e18);
        } else if(timeDiff <= 7776000) {
            return stakesOfUser[msg.sender][id].amount.mul(tpsRatios[1]).mul(timeDiff).div(1e18);
        } else if(timeDiff <= 15552000) {
            return stakesOfUser[msg.sender][id].amount.mul(tpsRatios[2]).mul(timeDiff).div(1e18);
        } else {
            return stakesOfUser[msg.sender][id].amount.mul(tpsRatios[3]).mul(timeDiff).div(1e18);
        }
    }
    ...
    */
    function claimRewards(uint256 id) public {
        ...
        require(wholeMonthsPassed != 0, "You must wait at least a month before you can start claiming your rewards!");
        
        uint256 reward;

        if(wholeMonthsPassed <= 3) {
            reward = stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[0]).mul(wholeMonthsPassed * 2592000);
        } else if(wholeMonthsPassed <= 6) {
            reward = stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[1]).mul(wholeMonthsPassed * 2592000);
        } else if(wholeMonthsPassed <= 9) {
            reward = stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[2]).mul(wholeMonthsPassed * 2592000);
        } else {
            reward = stakesOfUser[msg.sender][id].amount.div(1e18).mul(tpsRatios[3]).mul(wholeMonthsPassed * 2592000);
        }
