...
contract AgentRoleUpgradeable is OwnableUpgradeable, AccessControl {
    ...
    function _setRoleToOwner(address _owner) internal {
        ...
        addAgent(_owner);
        bytes32[] memory issuerRoles = new bytes32[](5);
        issuerRoles[0] = Minter_Role;
        issuerRoles[1] = Burner_Role;
        issuerRoles[2] = Freezer_Role;
        issuerRoles[3] = UnFreezer_Role;
        issuerRoles[4] = ForceTransfer_Role;
        setRolesToAgent(_owner, issuerRoles);
        }

    function addIssuer(address _userAddress) external onlyOwner {
    ...
    function revokeIssuer(address _SubOwner) external onlyOwner {
        ...
        array[3] = UnFreezer_Role;
        array[4] = ForceTransfer_Role;
        revokeRolesOnAgent(_SubOwner, array);
    ...
    function setRolesToAgent(address _userAddress, bytes32[] memory roles) public onlyRole(Owner_Role){
        require(roles.length <= 5, "Maximum 3 roles are accepted for now");
    ...
    function revokeRolesOnAgent(address _userAddress, bytes32[] memory roles) public onlyRole(Owner_Role){
        for(uint i=0; i < roles.length; i++)
        {
