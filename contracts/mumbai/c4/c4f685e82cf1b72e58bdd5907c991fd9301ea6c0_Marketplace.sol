...
import {IRoyaltiesStorage} from "./Interfaces/IRoyaltiesStorage.sol";
import {IMarketplaceErrors} from "./IMarketplaceErrors.sol";
...
//                        (&&%&&&&&&&&&&&&&&%#
//                            ,&&&&&&&%&&/
//  __  __    _    ____  _  _______ _____ ____  _        _    ____ _____
    ...
    TransferProxy
{
    ...
    function batchExecuteOrders(
        ...
        if (orderDataLength == 0) {
            revert CannotExecuteZeroOrders();
        }
        if (orderDataLength != splitSignature.length) {
            revert IncorrectExecuteOrdersParams();
        }
    ...
    function _executeOrder(
        ...
        if (
            ...
            revert TransferExecutionFailed("BUYER");
        }
        ...
        if (
            ...
            revert TransferExecutionFailed("SELLER");
        }
    ...
    function executeTransfer(
        ...
        if (tokenAmount == 0) {
            revert CannotTransferZeroAmount();
        }
        ...
        if (hasFees) {
            ...
            (royaltyFeeAmounts_, royaltyRecipients_) = IRoyaltiesStorage(
                royaltiesStorage
    ...
    function transferETH(address payable user, uint256 amount) internal {
        ...
        if (!success) {
            revert ErrorWhileTransferringETH();
        }
    }
