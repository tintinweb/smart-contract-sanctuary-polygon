...
contract ReserveForWinRewards is Ownable { 
    ...
    // 50 000 000 in Reward winning found for incentize at beginning
    // 150 000 000 total reserve added to pool during 50 Months = 1521 Days => each hours (36 504Hours)
    // 150 000 000 / 36 504 = 4 109.14 BZAI 
    uint256 public hourlyAddOn = 410914 * 1E16; 
    ...
    modifier onlyGame(){
        require(msg.sender == gameAddresses.getWinRewardsAddress(),"only Rewards contract auth");
        _;
    }
    ...
    function getRewardFinished() external view returns(bool){
        return BZAI.balanceOf(address(this)) == 0;
    }

    function updateRewards() external onlyGame returns(bool){
        if(BZAI.balanceOf(address(this)) < hourlyAddOn){
            hourlyAddOn = BZAI.balanceOf(address(this));
        }
        return(BZAI.transfer(msg.sender,hourlyAddOn));
