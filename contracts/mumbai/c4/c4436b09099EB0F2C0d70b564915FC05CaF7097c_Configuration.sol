...
// File: contracts/MainContractBucket/Configuration.sol
...
contract Configuration is IConfig {
    ...
    mapping(uint => uint) public tokensPerStrike;

    mapping(uint => uint) public juryTokensSharePerStrike;


    function setTokensPerStrike(uint strike_,uint requiredPayment) external returns(bool) {
        tokensPerStrike[strike_] = requiredPayment;

        return true;
    }

    function setJuryTokensSharePerStrike(uint strike_,uint requiredShare_) external returns(bool) {
        juryTokensSharePerStrike[strike_] = requiredShare_;

        return true;
    }

    function getTokensPerStrike(uint strike_) external view override returns(uint) {
        return tokensPerStrike[strike_];
    }

    function getJuryTokensShare(uint strike_) external view override returns(uint) {

        return juryTokensSharePerStrike[strike_];
    }
    ...
    function setDisputeConfig(uint escrowAmount_,uint requirePaymentForJury_) external override returns(bool) {
    ...
    function getDisputeConfig() external view override returns(uint,uint) {
    ...
    event FeeDeductionConfigUpdated(uint,uint,uint,uint,uint,uint,uint,uint,uint,uint);
    ...
    function setFeeDeductionConfig(
        ...
        emit FeeDeductionConfigUpdated(platformFees_,after_full_swap_treasury_wallet_transfer_,after_full_swap_without_trend_setter_treasury_wallet_transfer_,dbeth_swap_amount_with_trend_setter_,dbeth_swap_amount_without_trend_setter_,bet_trend_setter_reward_,pool_distribution_amount_,burn_amount_,pool_distribution_amount_without_trendsetter_,burn_amount_without_trendsetter);
    ...
    function getFeeDeductionConfig() public override view returns(uint,uint,uint,uint,uint,uint,uint,uint,uint) {
        return(Platform_Fees,After_Full_Swap_Treasury_Wallet_Transfer,After_Full_Swap_Without_Trend_Setter_Treasury_Wallet_Transfer,DBETH_Swap_Amount_With_Trend_Setter,DBETH_Swap_Amount_WithOut_Trend_Setter,Bet_Trend_Setter_Reward,Pool_Distribution_Amount,Burn_Amount,Pool_Distribution_Amount_Without_TrendSetter);
    }
