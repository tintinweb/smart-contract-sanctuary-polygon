...
contract PhiMap is AccessControlUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    /* -------------------------------------------------------------------------- */
    /* ---------------------------------- Map ----------------------------------- */
    // Whether the map can be updated
    ...
    /* -------------------------------------------------------------------------- */
    /**
     * @notice Require that the map has not been locked.
     */
    ...
    modifier onlyIfNotReadyObject(string memory name, uint256 objectIndex) {
        address owner = ownerOfPhiland(name);
     ...
     */
    function create(string memory name, address caller) external onlyIfNotOnwer onlyIfNotLocked {
    ...
    /**
     * @notice Lock map edit.
     */
    function flipLockMap() external onlyIfNotOnwer {
     ...
     * @param name : Ens name
     */
    function withdrawWallPaper(string memory name) external onlyIfNotLocked onlyIfNotPhilandOwner(name) {
    ...
    function changeWallPaper(
        ...
        address contractAddress,
        uint256 tokenId
    ) public onlyIfNotLocked onlyIfNotPhilandOwner(name) {
        address lastWallPaperContractAddress = wallPaper[name].contractAddress;
        uint256 lastWallPaperTokenId = wallPaper[name].tokenId;
        if (lastWallPaperContractAddress != address(0)) {
            IObject _lastWallPaper = IObject(lastWallPaperContractAddress);
    ...
    function writeObjectToLand(
        ...
        Link memory link
    ) public onlyIfNotLocked onlyIfNotPhilandOwner(name) onlyIfNotDepositObject(name, objectData) {
     ...
     */
    function mapInitialization(string memory name) public onlyIfNotLocked onlyIfNotPhilandOwner(name) {
     ...
     * @notice Function for clear users map objects and links
     * @param name : Ens name
     * @param removeCheck : if removeCheck == 0 then remove is skipped
     * @param removeIndexArray : Array of Object index
     ...
     * @param changeWallCheck : if changeWallCheck ==  o then wallchange is skipped
    ...
    function save(
        ...
        address contractAddress,
        uint256 tokenId
    ) external onlyIfNotLocked onlyIfNotPhilandOwner(name) {
        batchRemoveAndWrite(name, removeIndexArray, objectDatas, links);
    ...
    function _removeUnUsedUserObject(string memory name) private {
        uint256 index = 0;
        bool check = false;
        ObjectInfo[] memory newUserObjects = new ObjectInfo[](userObject[name].length);
        for (uint256 i = 0; i < userObject[name].length; i++) {
            if (userObject[name][i].contractAddress == address(0)) {
                check = true;
                continue;
            }
            newUserObjects[index] = userObject[name][i];
            index = index + 1;
        }
        if (check) {
            for (uint256 i = 0; i < userObject[name].length; i++) {
    ...
    function writeLinkToObject(
        ...
        uint256 objectIndex,
        Link memory link
    ) public onlyIfNotLocked onlyIfNotPhilandOwner(name) onlyIfNotReadyObject(name, objectIndex) {
