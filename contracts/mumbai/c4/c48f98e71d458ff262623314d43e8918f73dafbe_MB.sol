...
contract MB {
	...
	uint constant public			INVEST_MIN_AMOUNT				= 0.0000001 ether;			// 0.5 MATIC
	uint[] public					REFERRAL_PERCENTS				= [0,0,0,0,0];	// 10% 3% 2% 1% 0.5%
	uint constant public			PROJECT_FEE						= 0;					// project fee 10% of deposit
	uint constant public			MAX_WITHDRAW_AMOUNT				= 100 ether;				// claim 100 MATIC max
	uint constant public			WITHDRAW_COOLDOWN				= 2 seconds;			// claim 24 times per day

	address payable public			INSURANCE_CONTRACT;
	mapping (uint => uint) public	INSURANCE_MAXBALANCE;
	uint constant public			INSURANCE_PERCENT				= 0;					// insurance fee 10% of claim
	uint constant public			INSURANCE_LOWBALANCE_PERCENT	= 0;					// protection kicks in at 25% or lower
