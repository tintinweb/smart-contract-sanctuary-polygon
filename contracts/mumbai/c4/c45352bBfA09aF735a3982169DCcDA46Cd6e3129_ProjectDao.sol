...
contract ProjectDao is ERC1155, AccessControlEnumerable, ERC1155Supply, Pausable {
  ...
  function setImgIpfsHash(string calldata _imgHash) external onlyRole(AUTHOR_ROLE) whenNotPaused {
    project.imgIpfsHash = _imgHash;
    emit ImgSet(_imgHash);
  }

  function setBlurbIpfsHash(string calldata _blurbHash) external onlyRole(AUTHOR_ROLE) whenNotPaused {
    project.blurbIpfsHash = _blurbHash;
    emit BlurbSet(_blurbHash);
  }

  function setGenre(string memory _genre) external onlyRole(AUTHOR_ROLE) whenNotPaused {
    project.genre = _genre;
    emit GenreSet(_genre);
  }

  function setSubtitle(string memory _subtitle) external onlyRole(AUTHOR_ROLE) whenNotPaused {
