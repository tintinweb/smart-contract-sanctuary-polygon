...
contract AccountsEscrow is Initializable, OwnableUpgradeable {
    ...
    function addToBalance(uint256 _amount, address _account) public {
        ...
        require(et.allowance(msg.sender, _account) >= _amount, "Insuficient Allowance");
        // require(et.transferFrom(msg.sender, _account, _amount), "token transfer failed");
        // et.transferFrom(msg.sender, _account, _amount);
        _appendAccount(_account);
        _addAccountBalance(_account, _amount);
        emit transferEvent(msg.sender, _account, _amount);
    }
