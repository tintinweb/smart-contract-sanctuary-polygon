...
// File: contracts/BoleroBatchSender.sol
...
contract BoleroBatchSender is ExternalStorage {
    ...
    * @param newOwner The address to transfer ownership to.
    */
    function sendTransferOwnership(address newOwner) public onlyOwner {
        require(newOwner != address(0));
    ...
    */
    ...
    event Multisended(uint256 total, address senderAddress, address tokenAddress);
   ...
   function multisendToken(address token, address[] memory _contributors, uint256[] memory _balances) public payable {
           ...
           emit Multisended(total, msg.sender,  token);
