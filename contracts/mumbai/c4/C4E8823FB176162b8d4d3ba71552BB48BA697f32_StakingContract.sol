...
//todo(): do final vulnerabilities checks and big(o) optimizations to lower gas costs and efficiency
...
contract StakingContract is ReentrancyGuard {
    ...
    // Staking parameters for each lock-up period
    struct StakingParams {
        ...
        uint256 referralCount; // number of referrals required to unlock this lock-up period
    }
    ...
    // Maps user addresses to their staked amount, lock-up period, and referral count
    ...
    constructor(IERC20 _token) {
        ...
        stakingParams[0].minStake = 30;
        stakingParams[0].maxWithdrawal = 50000;
        stakingParams[0].rewardPercentage = 15;
        stakingParams[0].referralCount = 0;

        stakingParams[1].minStake = 100; 
        stakingParams[1].maxWithdrawal = 80000;
        stakingParams[1].rewardPercentage = 25;
        stakingParams[1].referralCount = 2;

        stakingParams[2].minStake = 200; 
        stakingParams[2].maxWithdrawal = 100000;
        stakingParams[2].rewardPercentage = 35;
        stakingParams[2].referralCount = 5;

        stakingParams[3].minStake = 10000;
        stakingParams[3].maxWithdrawal = 300000;
        stakingParams[3].rewardPercentage = 40;
        stakingParams[3].referralCount = 10;

        stakingParams[4].minStake = 20000;
        stakingParams[4].maxWithdrawal = 800000;
    ...
    function stake(uint256 amount, address referralAddress) public noReentry {
        ...
        if( stakedAmounts[msg.sender].amount == 0){
            numberOfUsers++;
        }
        ...
        if (referralAddress != address(0) && referralAddress != msg.sender && stakedAmounts[msg.sender].referee == address(0)) {
            stakedAmounts[referralAddress].referralCount++;
            stakedAmounts[msg.sender].referee = referralAddress;
            stakedAmounts[referralAddress].amount += amount.mul(150).div(1000); // If null address rewards go to dead wallet
            stakedAmounts[stakedAmounts[referralAddress].referee].amount += amount.mul(7).div(1000); // If null address rewards go to dead wallet
            stakedAmounts[stakedAmounts[stakedAmounts[referralAddress].referee].referee].amount += amount.mul(3).div(1000); // If null address rewards go to dead wallet
        }
    ...
    // // Emergency withdraw function still working on
    // // todo(): Complete this
    // function withdraw(uint256 amount) public {
    //     require(amount <= stakedAmounts[msg.sender].amount, "Insufficient staked amount");
    //     require(amount <= stakingParams[stakedAmounts[msg.sender].lockUpPlan].maxWithdrawal, "Exceeded maximum withdrawal amount");

    //     // Transfer tokens back to the staker
    //     token.safeTransfer(msg.sender, amount);

    //     // Update staked amount for the user
    //     stakedAmounts[msg.sender].amount -= amount;
    // }
    ...
    function claimReward() public noReentry {
        ...
        token.safeTransfer(msg.sender, reward.div(100).mul(90));
        token.safeTransfer(marketingWallet, reward.div(100).mul(10));
