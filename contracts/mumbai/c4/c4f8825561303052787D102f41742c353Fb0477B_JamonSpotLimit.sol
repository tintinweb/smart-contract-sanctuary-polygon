    ...
    Ownable
{
    ...
    //---------- Contracts ----------//
    KeeperRegistryInterface private immutable keeperRegistery;
    //Address of JamonSwap Router
    IJamonRouter private immutable jamonRouter;
    ...
    modifier onlyKeeper() {
        require(
            ...
            "Only Kepper Registry"
        );
        _;
    }

    modifier onlyOnKeppersActive() {
        (address target, , , , , , ) = keeperRegistery.getUpkeep(kepperId);
        require(target == address(this), "Uninitialized kepper");
        _;
    }
        ...
        onlyOnKeppersActive
    {
        ...
        keeperRegistery.addFunds(kepperId, funds.toUint96());
    ...
    function createOrder(
        ...
        uint16 slippage,
        uint256 credits_
    ) external payable whenNotPaused nonReentrant onlyOnKeppersActive {
        //Checks
        require(slippage <= 10000, "Slippage out of bound");
        require(targetPrice > 0 && credits_ > 0, "Zero price");
        ...
        (bool success, uint256 price) = getPrice(amountIn, tokenIn, tokenOut);
        ...
        //Effects
        uint256 deadline = block.timestamp.add(7 days * credits_);
    ...
    function cancelOrder(bytes32 orderId) external nonReentrant {
        ...
        //Interactions
        if (order.tokenIn == address(jamonRouter.WETH())) {
    ...
    function setKepperId(uint256 id) external onlyOwner {
