...
contract MintAndRevealViaBurnFightMe is ERC721Base {
    event MintViaLootbox(
      ...
      bool public isLootboxContractSet = false;
      string public placeholderURIPrefix;

      mapping(uint256 => string) private overrideURIs;     
      mapping(uint256 => bool) private isNonRevealed; 
      uint256[] private nonRevealedTokenIDs;
      mapping(uint256 => uint256) private randomRolls;
      mapping(uint256 => uint256) private fighterLevels;
    ...
    modifier onlyHolderOfNFT(uint256 _tokenId) {
        require(isApprovedOrOwner(_msgSender(), _tokenId), "Caller is not owner nor approved");     
        _;
    }


    function setLootboxContract(address _lootboxContract) external onlyOwner {
        require(!isLootboxContractSet);
        lootboxContract = _lootboxContract;
        isLootboxContractSet = true;
    }

    function mintViaLootbox(uint256 _tokenId, uint256 _random, address _burnerAddress) external onlyLootboxContract {
        uint256 fighterTokenID = nextTokenIdToMint();
        string memory uri = string.concat(
            placeholderURIPrefix, 
            Strings.toString(_tokenId), 
            "?random=", 
            Strings.toString(_random),
            "&level=1");
        _setTokenURI(fighterTokenID, uri);
        _safeMint(_burnerAddress, 1, "");

        nonRevealedTokenIDs.push(fighterTokenID);
        overrideURIs[fighterTokenID] = uri;
        isNonRevealed[fighterTokenID] = true;
        randomRolls[_tokenId] = _random;
        fighterLevels[_tokenId] = 1;
        emit MintViaLootbox(_tokenId, _burnerAddress, msg.sender);
    }

    function levelUp(uint256 _tokenId) external onlyHolderOfNFT(_tokenId) {
        isNonRevealed[_tokenId] = true;
        nonRevealedTokenIDs.push(_tokenId);

        uint256 randomRoll = randomRolls[_tokenId];
        uint256 level = fighterLevels[_tokenId];

        overrideURIs[_tokenId] = string.concat(
            placeholderURIPrefix, 
            Strings.toString(_tokenId), 
            "?random=", 
            Strings.toString(randomRoll),
            "&level=",
            Strings.toString(level + 1));

        fighterLevels[_tokenId] = level + 1;        
    }

    function revealURI(uint256 _tokenId, string memory _uri) external onlyOwner {        
        if(isNonRevealed[_tokenId]) {
            delete isNonRevealed[_tokenId];
            overrideURIs[_tokenId] = _uri;
           
            uint256 index = 0;
            for(uint256 i = 0; i < nonRevealedTokenIDs.length; i++) {
                if(nonRevealedTokenIDs[i] == _tokenId) {
                    index = i;
                    break;
                }
            }
            nonRevealedTokenIDs[index] = nonRevealedTokenIDs[nonRevealedTokenIDs.length-1];
            nonRevealedTokenIDs.pop();      
        }
    }

    function getNonRevealedTokenIDs() external view returns(uint256[] memory) {
        return nonRevealedTokenIDs;
    }

    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        string memory overUriForToken = overrideURIs[_tokenId];
        if (bytes(overUriForToken).length > 0) {
            return overUriForToken;
        }
