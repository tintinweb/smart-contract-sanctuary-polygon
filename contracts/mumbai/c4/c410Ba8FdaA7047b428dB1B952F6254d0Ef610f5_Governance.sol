...
/**
    ...
    @dev This contract is being used as Governance of Vistaverse Land
       + Register address (Treasury) to receive Commission Fee 
       + Set up additional special roles - DEFAULT_ADMIN_ROLE, MANAGER_ROLE, MINTER_ROLE, and AUTHORIZER_ROLE
*/
contract Governance is AccessControlEnumerable {
    ...
    //     - EXECUTOR_ROLE
    //         + Has been granted a limited priviledge that allows to execute a specific task (i.e. airdrop)
    //     - AUTHORIZER_ROLE
    //         + Has been granted a limited priviledge (i.e. 3rd parties)
    ...
    /**
        @notice Register ERC-20 Token contract
        @dev  Caller must have MANAGER_ROLE
        @param _token         Address of ERC-20 Token contract (0x00 - Native Coin)
    */
    function addPayment(address _token) external onlyRole(MANAGER_ROLE) {
        if (paymentToken[_token]) revert Existed();

        paymentToken[_token] = true;
    }

    /**
        @notice Unregister ERC-20 Token contract
        @dev  Caller must have MANAGER_ROLE
        @param _token         Address of ERC-20 Token contract
    */
    function removePayment(address _token) external onlyRole(MANAGER_ROLE) {
        if (!paymentToken[_token]) revert NotFound();

        delete paymentToken[_token];
    }
    ...
    */
    function updateTreasury(address _newTreasury) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if (_newTreasury == address(0)) revert ZeroAddr();

        treasury = _newTreasury;
    }

    /**
        @notice Set `locked` state
        @dev  Caller must have MANAGER_ROLE
        @param _state        State boolean (true or false)
        Note: when `locked = true`, operation in other contracts will be halted
    */
    function setLock(bool _state) external onlyRole(MANAGER_ROLE) {
        locked = _state;
    }
}
