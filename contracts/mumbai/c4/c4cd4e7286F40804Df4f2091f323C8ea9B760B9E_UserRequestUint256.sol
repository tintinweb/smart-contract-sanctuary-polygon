...
contract UserRequestUint256 is UserRequest, ConfirmedOwner {
    ...
    uint256 expireDuration = 7 minutes;
    struct Response {
        address creator;
    ...
    function getResponseForUser(address requestCreator) public view returns(uint256 response){
        for(uint256 index; index < requestIds.length; index++){
            Response memory responseDetails = userResponses[requestIds[index]];
            if(responseDetails.creator == requestCreator){
                return responseDetails.responseData;
            }
        }
    }
    ...
    function runRequestQuery(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        ...
        newResponse.creator = msg.sender;
    }
    ...
    function saveQueryResponse(bytes32 requestId, uint256 data, string calldata errorMessage) external {
        ...
        newResponse.expireTime = block.timestamp + expireDuration;
    }
