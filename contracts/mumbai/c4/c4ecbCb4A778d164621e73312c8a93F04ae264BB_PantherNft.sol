...
contract PantherNft is ERC721, EIP712, Ownable {
    ...
    function mintWhitelist(uint256 val, bytes memory signature) external {
        ...
        if (val < 10000) {
            ...
            require(
                usdcToken.transferFrom(msg.sender, address(this), toPay),
                "Not engough usdc approved or available"
            );
        }
        if (val == 0) {
            require(
                usdcToken.transferFrom(msg.sender, address(this), mintCost),
                "Not engough eth"
            );
        }
        uint256 id = uint256(
    ...
    function withdrawUsd() external onlyOwner {
        address own = owner();
        require(
            usdcToken.transfer(own, usdcToken.balanceOf(address(this))),
            "failed"
        );
    }
}
