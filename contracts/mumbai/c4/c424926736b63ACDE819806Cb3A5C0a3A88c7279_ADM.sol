...
contract ADM {
    ...
    uint256[4] private level4Percents = [50, 100, 150, 50];
    ...
    struct RewardInfo{
        ...
        uint256[2] level3Released; //this contains level 2 and 3, to prevent stack to deep error
        uint256 level5Left;
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256) {
        ...
        uint256 totalRewards = userRewards.directs.add(userRewards.level3Released[0]).add(userRewards.level3Released[1]).add(userRewards.level4Released).add(userRewards.level5Released);
    ...
    function _calLevelNow(address _user) private view returns(uint256) {
        ...
        if(total >= 2000e6 && user.teamNum >= 200 && maxTeam >= 70000e6 && otherTeam >= 70000e6){
             levelNow = 5;
        }else if(user.teamNum >= 50 && maxTeam >= 20000e6 && otherTeam >= 20000e6){
             levelNow = 4;
        }else if(total >= 1000e6 && user.teamNum >= 20 && maxTeam >= 10000e6 && otherTeam >= 10000e6){
             levelNow = 3;
        }
        else if(total >= 1000e6 && user.teamNum >= 10 && maxTeam >= 7000e6 && otherTeam >= 7000e6){
           levelNow = 2;
        }else if(total >= 50e6){
            levelNow = 1;
        }
    ...
    function getRewardInfos(address _address, uint256 _type) public view returns (uint256, uint256, uint256, uint256, uint256, uint256){
     ...
     if(_type == 1){
         uint256 _release = rewardInfo[_address].level4Released.add(rewardInfo[_address].level3Released[0]);
         _release = _release.add(rewardInfo[_address].level3Released[1]);
            ...
            if(upline != address(0)){
                ...
                if(i > 4){
                     if(userInfo[upline].level >= 11){
                        reward = newAmount.mul(level4Percents[i - 5]).div(baseDivider);
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                    else if(userInfo[upline].level >= 7 && userInfo[upline].level < 11){
                        reward = newAmount.mul(level4Percents[i - 5]).div(baseDivider);
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                    else if(userInfo[upline].level > 4 && userInfo[upline].level < 7){
                        reward = newAmount.mul(level4Percents[i - 5]).div(baseDivider);
                        upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    }
                }else if(i > 0){
    ...
    function _releaseUpRewards(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < referDepth; i++){
            if(upline != address(0)){
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                if(i > 0 && i < 5 && userInfo[upline].level > 1){
                    if(upRewards.level4Freezed > 0){
                        uint256 level2Reward = newAmount.mul(level4Percents[i - 1]).div(baseDivider);
                        if(level2Reward > upRewards.level4Freezed){
                            level2Reward = upRewards.level4Freezed;
                        }
                        upRewards.level3Released[0] = upRewards.level3Released[0].add(level2Reward);
                        userInfo[upline].totalRevenue = userInfo[upline].totalRevenue.add(level2Reward);
                    }
                }
                if(i > 0 && i < 5 && userInfo[upline].level > 2){
                    //updating user commission based on its deposit instead
                    if(upRewards.level4Freezed > 0){
                        uint256 level3Reward = _amount.mul(100).div(baseDivider);
                        if(level3Reward > upRewards.level4Freezed){
                            level3Reward = upRewards.level4Freezed;
                        }
                        upRewards.level3Released[1] = upRewards.level3Released[1].add(level3Reward);
