...
contract RandomWinnerGame is VRFConsumerBase, Ownable {
	...
	//number of allowed players in a game
	uint8 maxPlayers;
	...
	/**
	*	Constrcutor inherits a VRFConsumerBase and initiates the values for the , fee, and gameStarted
	...
	*/	
	...
	function startGame(uint8 _maxPlayers, uint256 _entryFee) public onlyOwner{
		...
		delete players;
		//set the max players for the game
	...
	function joinGame() public payable{
		//check if there is a game actively running
		require(gameStarted, "Game has not been started yet");
		//check if the value sent by the user matches the entryFee
		require(msg.value == entryFee, "Value sent is not equal to entryFee");
		//check if there's still space to join the game
		require(players.length < maxPlayers, "Game is full");
		//add the sender to the playersList
		players.push(msg.sender);
		emit PlayerJoined(gameId, msg.sender);
		//if the list is full, start the wwinner selection process
	...
	/**
	*	fulfillRandomness: is called by VRFCoordinator when it ceives a valid VRF proof.
	...
	*	@param randomness - this is a random uint256 generated and returneed to us by the VRF Coordinator
	*/
	function fulfillRandomness(bytes32 requestId, uint256 randomness) internal virtual override {
		//We want our winnerIndex to be in the range of 0 to players.length - 1
	...
	function getRandomWinner() private returns (bytes32 requestId) {
		//LINK is an internal interface for Link Token found with in the VRFConsumerBase
		...
		//make request to the VRF coordinator
