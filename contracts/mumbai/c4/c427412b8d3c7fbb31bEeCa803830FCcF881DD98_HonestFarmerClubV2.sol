...
import "./interfaces/IWhitelistFarmer.sol";
import "./interfaces/IMigrationTractor.sol";
...
interface IWhitelistFarmer {
	/**
	 * Introspection
	 */
	function getAllocation(LibraryFarmer.MintType mintType, address _address)
		external
		view
		returns (uint256 allocation);

	function getAllocationBatch(
		LibraryFarmer.MintType mintType,
	 ...
	 * Delegates
	 */
	function numberOfDelegates() external view returns (uint256);
...
interface IHonestFarmerClubV1 is IERC1155 {}

interface IMigrationTractor is IERC1155Receiver {
	function PRE_MIGRATION_TOKEN_COUNT() external returns (uint256);

	function migrateFarmers(uint256[] memory ids) external;

	function emergencyMigrateFarmers(uint256[] memory ids, address to) external;
