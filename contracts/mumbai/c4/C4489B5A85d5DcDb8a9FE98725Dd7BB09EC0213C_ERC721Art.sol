    ...
    ERC2981Upgradeable
{
    ...
    ///@dev boolean that specifies if collection/creater has been corrupted
    bool public corrupted;

    ///@notice crowdfund contract settings
    address public crowdfund;
    bool public isCrowdFundSet;
    ...
    function initialize(
        ...
        price[Coin.USDT_TOKEN] = _priceInUSD;
        ...
        tokenContract[Coin.USDT_TOKEN] = IERC20(
    ...
    modifier crowdFundOnlyAuthorized() {
        ...
        notCorrupted
    {
        ...
        bool isAmountLowerThanPrice;
        if (_coin == Coin.ETH_COIN) {
            isAmountLowerThanPrice = msg.value < price[_coin];
        } else {
            isAmountLowerThanPrice =
                tokenContract[_coin].allowance(msg.sender, address(this)) <
                price[_coin];
        }
    ...
    function burn(
        uint256 _tokenId
    ) external override(IERC721Art) whenNotPaused nonReentrant onlyAuthorized {
        _burn(_tokenId);
    }
    ...
    function creatorsProSafeTransferFrom(
        ...
        notCorrupted
    {
        bool isAmountLowerThanTokenPrice;
        if (coin == Coin.ETH_COIN) {
            isAmountLowerThanTokenPrice = msg.value < tokenPrice[tokenId][coin];
        } else {
            isAmountLowerThanTokenPrice =
                tokenContract[coin].allowance(from, address(this)) <
                tokenPrice[tokenId][coin];
        }
        ...
        _doAllTransfers(coin, tokenPrice[tokenId][coin], tokenId, to, from);
    ...
    function setCorrupted(
        bool _corrupted
    ) external override(IERC721Art) nonReentrant {
        if (!management.managers(msg.sender) && msg.sender != crowdfund) {
            revert ERC721ArtNotManager();
        }
    ...
    function setCrowdfund(
        address _crowdfund
    ) external override(IERC721Art) whenNotPaused nonReentrant {
    ...
    /// @inheritdoc IERC721Art
    function pause() external override(IERC721Art) crowdFundOnlyAuthorized {
        _pause();
    }
    ...
    are allowed to execute this function. Uses _pause internal function from PausableUpgradeable. */
    /// @inheritdoc IERC721Art
    function unpause() external override(IERC721Art) crowdFundOnlyAuthorized {
        _unpause();
    }
    ...
    function _coinTransfer(
        ...
        if (_coin == Coin.ETH_COIN) {
            ...
            tokenContract[_coin].transferFrom(_from, _payee, _value);
        }
    }
    ...
    function _doAllTransfers(
        ...
        uint256 CreatorsProRoyalty = (_price * management.fee()) /
            _feeDenominator();
        _coinTransfer(CreatorsProRoyalty, _coin, management.multiSig(), _from);

        // collection creator transfer
        (, uint256 collectionCreatorRoyalty) = royaltyInfo(_tokenId, _price);
        _coinTransfer(collectionCreatorRoyalty, _coin, owner(), _from);
        ...
        uint256 ownerValue = _price -
            CreatorsProRoyalty -
            collectionCreatorRoyalty;
        _coinTransfer(ownerValue, _coin, _NFTOwner, _from);
    ...
    function _doAllTransfersMint(
        ...
        // collection creator transfer
        _coinTransfer(_price - CreatorsProRoyalty, _coin, owner(), msg.sender);
    ...
    function _transfer(
        ...
        if (block.timestamp - lastTransfer[tokenId] < 30 days) {
            revert ERC721ArtTrasnferDeadlineOngoing();
        }
