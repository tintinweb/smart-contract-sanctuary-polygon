...
contract TreasureHuntPirate is
    Initializable,
    ITreasureHuntTrait,
    ...
    OwnableUpgradeable
{
    ...
    modifier _isApprover() {
        require(
            msg.sender == treasureHuntFleet ||
                msg.sender == treasureHuntMarketPlace,
            "You are not Approver!"
        );
        _;
    }
    ...
    function massMint(bool presale, uint8 amt) public {
        ...
        require(
            !presale || (presale && presaleCnt + amt <= presaleMintMax),
            "Can't mint that amounts!"
        );

        uint256 price = presale ? mintPresalePrice : mintPrice;

        if (!teamlist[msg.sender]) {
            ITreasureHunt(rewardPool).transferCost(price * amt, false);
        }
    ...
    function getPirates(uint256[] memory tokenIDs)
        public
        view
        returns (Pirate[] memory)
    {
        Pirate[] memory pirates = new Pirate[](tokenIDs.length);
    ...
    function getAcitvePirates() public view returns (Pirate[] memory) {
        return getPirates(activePiratesByOwner[msg.sender]);
    }

    function inActivePirates(address acc)
        public
        view
    ...
    function getInAcitvePirates() public view returns (Pirate[] memory) {
        return getPirates(inActivePiratesByOwner[msg.sender]);
    }
    ...
    function unBoardPirates(uint256[] memory pirates) public _isFleet {
        for (uint256 i = 0; i < pirates.length; i++) {
            ...
            tokenIndex[pirates[i]] =
                inActivePiratesByOwner[tx.origin].length -
                1;
        }
    }
    ...
    function giveGift(
        ...
        require(
            amount > 0 && mysteryChestAmt[msg.sender] >= amount,
            "Can't gift chests!"
        );

        if (isDirect) {
            ITreasureHunt(rewardPool).transferCost(giftFee * amount, false);
        } else {
    ...
    function transferFrom(
        ...
        tokenIndex[
            activePiratesByOwner[from][activePiratesByOwner[from].length - 1]
        ] = tokenIndex[tokenID];
        activePiratesByOwner[from][tokenIndex[tokenID]] = activePiratesByOwner[
            from
        ][activePiratesByOwner[from].length - 1];
    ...
    function airdrop(address[] memory accounts, uint8[] memory cnts)
        public
        onlyOwner
    {
    ...
    function burnAllPirates() public {
        uint256 amount = inActivePiratesByOwner[msg.sender].length;
        for (uint256 i; i < amount; i++) {
            uint256 tokenID = inActivePiratesByOwner[msg.sender][i];
            _burn(tokenID);
            delete pirateInfo[tokenID];
            delete tokenIndex[inActivePiratesByOwner[msg.sender][i]];
        }
        delete inActivePiratesByOwner[msg.sender];
        ITreasureHunt(rewardPool).transferBurnReward(
            msg.sender,
            burnPrice * amount
        );
    }
    ...
    function burnPirates(uint256[] memory tokenIDs) public {
        for (uint256 i = 0; i < tokenIDs.length; i++) {
            ...
            _burn(tokenIDs[i]);
            if (isActivePirate[tokenIDs[i]]) {
                tokenIndex[
                    activePiratesByOwner[msg.sender][
                        activePiratesByOwner[msg.sender].length - 1
                    ]
                ] = tokenIndex[tokenIDs[i]];
                activePiratesByOwner[msg.sender][
                    tokenIndex[tokenIDs[i]]
                ] = activePiratesByOwner[msg.sender][
                    activePiratesByOwner[msg.sender].length - 1
                ];
                activePiratesByOwner[msg.sender].pop();
            } else {
                tokenIndex[
                    inActivePiratesByOwner[msg.sender][
                        inActivePiratesByOwner[msg.sender].length - 1
                    ]
                ] = tokenIndex[tokenIDs[i]];
                inActivePiratesByOwner[msg.sender][
                    tokenIndex[tokenIDs[i]]
