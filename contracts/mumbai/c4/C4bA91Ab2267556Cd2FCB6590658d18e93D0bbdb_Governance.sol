...
// File: @openzeppelin/contracts/governance/compatibility/IGovernorCompatibilityBravo.sol
...
// File: 2023/Bowo network/governance/Governance.sol
...
interface IFarming {
    ...
    function stakeHoldings(address account) external view returns (uint staked);
}

pragma solidity 0.8.18;

interface ITokenTimeLockFactory {
    function createTokenTimelock(address token, uint timelockId, address beneficiary, uint256 lockAmount, uint256 releaseTime) external;
}
...
contract Governance is Governor, GovernorCompatibilityBravo, GovernorVotes, GovernorVotesQuorumFraction, GovernorTimelockControl {
    ...
    IFarming public farm;
    ITokenTimeLockFactory public tokenTimeLockFactory;
    ...
    constructor(IVotes _shims, IFarming _farm, TimelockController _timelock, ITokenTimeLockFactory _tokenTimeLockFactory)
        Governor("Governance")
        GovernorVotes(_shims)
        GovernorVotesQuorumFraction(0)
        GovernorTimelockControl(_timelock)
    {
        regulation.requiredThreshold = 5000e18;
        regulation.votingDelay = 300;
        regulation.proposalThreshold = 1;
        tokenTimeLockFactory = _tokenTimeLockFactory;
        farm = _farm;
    }

    function setRequiredThreshold(uint requiredThreshold) public onlyGovernance {
        require(requiredThreshold != 0, "setRequiredThreshold: new requiredThreshold is zero");
        regulation.requiredThreshold = requiredThreshold;
    }

    function setProposalThreshold(uint newProposalThreshold) public onlyGovernance {
        require(newProposalThreshold != 0, "setProposalThreshold: new newProposalThreshold is zero");
        regulation.proposalThreshold = newProposalThreshold;
    }

    function setVotingDelay(uint newVotingDelay) public onlyGovernance {
        require(newVotingDelay != 0, "setVotingDelay: new voting delay is zero");
        regulation.votingDelay = newVotingDelay;
    }

    function setToken(IVotes newShims) public onlyGovernance {
        require(address(newShims) != address(0), "setToken: new token address is zero");
        token = newShims;
    }

    function setFarm(IFarming newFarm) public onlyGovernance {
        require(address(newFarm) != address(0), "setFarm: new farm address is zero");
        farm = newFarm;
    }
    ...
    function _propose(
        ...
        uint64 snapshot = block.number.toUint64() + votingDelay().toUint64();
        uint64 deadline = snapshot + votePeriod.toUint64();
    ...
    function propose(
        ...
        // _proposeValidator();
        // _storeProposal(_msgSender(), targets, values, signatures, calldatas, description);
        // return _propose(targets, values, _encodeCalldata(signatures, calldatas), description);
        revert("incorrect exec");
    }
    ...
    function propose(
        ...
        _proposeValidator();
        _storeProposal(_msgSender(), targets, values, new string[](calldatas.length), calldatas, description);
        return _propose(targets, values, calldatas, description, votePeriod);
    }
    ...
    {
        require(
            farm.stakeHoldings(_msgSender()) >= requiredProposalThreshold(),
            "propose: insufficient stake holding"
        );
    }
     ...
     * @dev See {IGovernor-castVote}.
     */
    function castVote(uint256 /*proposalId*/, uint8 /*support*/) public virtual override(Governor,IGovernor) returns (uint256) {
        revert("incorrect exec");
    }
    ...
    function _countVote(
        ...
        uint256 weight,
        bytes memory params
    ) internal virtual override(Governor, GovernorCompatibilityBravo) { 
        require(
            IERC20(address(token)).allowance(account, address(tokenTimeLockFactory)) >= weight,
            "_countVote: insufficient allowance to vote"
        );
        super._countVote(
            proposalId,
            account,
        ...
        tokenTimeLockFactory.createTokenTimelock(
            address(token), 
            proposalId, 
            account, 
            weight, 
    ...
    function getVotes(address account, uint256 ) public view virtual override(Governor,IGovernor) returns (uint256) {
        return farm.isStakeHolder(account) ? 1 : 0;
    }

    function requiredProposalThreshold() public view returns (uint256) {
        return regulation.requiredThreshold;
    }
    ...
    function proposalThreshold() public view override returns (uint256) {
        return regulation.proposalThreshold;
    }

    function votingDelay() public view override returns (uint256) {
        return regulation.votingDelay;
    }
