    ...
    OwnableUpgradeable
{
    ...
    struct RequestStatus {
        ...
        uint256[] randomWords;
        uint256 option; // loại function manager call: 0 = mintLand; 1 = roll
    }
    ...
    function initialize(address _link, address _vrfV2Wrapper) external initializer {
        __Pausable_init();
        __Ownable_init();
        LINK = LinkTokenInterface(_link);
        VRF_V2_WRAPPER = VRFV2WrapperInterface(_vrfV2Wrapper);
    ...
    function requestRandomWords(uint256 option_)
        external
        returns (uint256 requestId)
    {
        require((msg.sender == address(manager) && option_ == 0) || (msg.sender == address(game) && option_ == 1), "caller is not accepted");
        uint32 numWords;
        if (option_ == 0) {
            numWords = 1;
        } else {
    ...
    function fulfillRandomWords(
        ...
        if (s_requests[_requestId].option == 0) {
            // manager mint Land
            manager.executeMint(_randomWords[0], _requestId);
        } else {
            game.executeRoll(_randomWords, _requestId);
        }  
        // uint256 id = _randomWords[0] % 1000;
        // land.mintToken(s_requests[_requestId].user, id);
