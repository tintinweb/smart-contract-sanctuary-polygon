...
contract WeatherRiskNFT is 
    ...
    // address public constant PROVIDER_ADDRESS = 0x456789ccc3813e8797c4B5C5BAB846ee4A47b0BA;
    // address public constant STABLECOIN_ADDRESS = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    // address private constant LINK_ADDRESS = 0xb0897686c545045aFc77CF20eC7A532E3120E0F1;
    // uint256 private constant DECIMALS = 2;
    // uint256 private constant DISPUTE_PERIOD = 5 * 60;
    // uint256 private constant EVALUATION_BUFFER = 1 * 60; 
    address public constant PROVIDER_ADDRESS = 0xed0Db415764F60D5e02EFd5726E9aFb5B7257257;      // Provider (Mumbai)
    address public constant STABLECOIN_ADDRESS = 0x7385E2DDb07aB886Ea86F0F3F33b5E002A2E3943;    // Mumbai USDC0 test token address (redux)
    address private constant LINK_ADDRESS = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB;         // Mumbai Link token address
    uint256 private constant DECIMALS = 6;                                                      // Mumbai USDC0 decimals
    uint256 private constant DISPUTE_PERIOD = 5 * 60;                        // 5 minute test dispute period during which initially transferred payouts are held by smart contract before final settlement
    uint256 private constant EVALUATION_BUFFER = 1 * 60;                     // 1 minute test additional period following coverage end date before evaluation can be initiated
    ...
    // uint256 private constant DISPUTE_PERIOD = 1 * 24 * 60 * 60;              // 24 hour dispute period during which initially transferred payouts are held by smart contract before final settlement
    // uint256 private constant EVALUATION_BUFFER = 30 * 24 * 60 * 60;          // 30 day additional period following coverage end date before evaluation can be initiated
        ...
        external 
    {
        ...
        // chainlinkConfig = ChainlinkConfig(0x76dfA9a36db355F101B241b66e2fA97f7Ca09C24, "940708c3e44c40f1bd03d34c1b3edcf6", "6907c93f71df43669ac4661c3d74a3e8");       // Polygon
        ...
        returns (bool upkeepNeeded, bytes memory performData) 
    {
        ...
        uint[] memory awaiting;
        upkeepNeeded = false;
        for (uint i; i < supply; i++) {
            uint id = tokenByIndex(i);
            TokenState memory state = tokenStates[id];
            TokenUpdate memory update = tokenUpdates[id];
            if (state.endDate + EVALUATION_BUFFER < block.timestamp) {
                if (update.evalTimestamp == 0) {
                    upkeepNeeded = true;
                    awaiting[i] = id;
                } else {
                    if (update.disputed) {
        ...
        performData = abi.encode(awaiting);
    ...
    function performUpkeep(bytes calldata performData) 
        ...
        override 
    {
        (uint[] memory awaiting) = abi.decode(performData, (uint[]));
        uint numRequests = awaiting.length;
        for (uint i = 0; i < numRequests; i ++) {
            requestEvaluation(awaiting[i]);
        } 
    }
