...
contract Flipcoin {
    ...
    function createEmptyLobby(uint256 betAmount) external gasLimit(50000) {
        ...
        MiniLobby storage miniLobby = _miniLobbies[_lobbyCounter];
        miniLobby.creator = msg.sender;
        miniLobby.betAmount = betAmount;
        miniLobby.totalPot = betAmount * (10**_tokenDecimals);
    ...
    function placeBet(uint256 lobbyId) external gasLimit(200000) {
        require(lobbyId < _lobbyCounter, "Invalid lobby ID");
        require(!_miniLobbies[lobbyId].isClosed, "Lobby is closed");
        require(_amberToken.balanceOf(msg.sender) >= _miniLobbies[lobbyId].betAmount * (10**_tokenDecimals), "Insufficient AMBER token balance");

        MiniLobby storage miniLobby = _miniLobbies[lobbyId];
        require(miniLobby.bets[msg.sender] == 0, "Already placed a bet");

        uint256 tokenAmount = miniLobby.betAmount * (10**_tokenDecimals);
        require(_amberToken.transferFrom(msg.sender, address(this), tokenAmount), "Failed to transfer AMBER tokens");

        miniLobby.bets[msg.sender] = tokenAmount;
        miniLobby.totalPot = miniLobby.totalPot.add(tokenAmount);

        emit BetPlaced(msg.sender, miniLobby.betAmount, lobbyId);
