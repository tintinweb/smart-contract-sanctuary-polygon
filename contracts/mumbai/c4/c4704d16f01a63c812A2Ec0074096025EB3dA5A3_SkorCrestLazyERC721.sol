...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // Mapping from list of token owned
...
contract SkorCrestLazyERC721 is
    ERC721URIStorage,
    EIP712,
    AccessControl,
    Ownable
{
    ...
    string private constant SIGNING_DOMAIN = "Skorcrest";
    ...
    mapping(uint256=>uint256) public linkedLand;

    constructor(address payable minter, ILand _land)
        ERC721("SkorCrest", "Crest")
    ...
    function redeem(
        ...
        uint256 amount = _voucher.minPrice;
        require(msg.value >= amount, "INVALID_AMOUNT");
        // make sure signature is valid and get the address of the signer
        address signer = _verify(_voucher);
        require(signer == _wallet, "Signer is not the creator");
        _mint(_redeemer, tokenId);
        _setTokenURI(tokenId, _voucher.uri);
        linkedLand[tokenId] = tokenId;
        land.mintLand(_redeemer, landUri, tokenId);
    ...
    function mint(address user, string memory uri, string memory landUri) public onlyOwner {
        uint256 tokenId = CurrentTokenId + 1;
        CurrentTokenId = tokenId;
        _mint(user, tokenId);
        _setTokenURI(tokenId, uri);
        linkedLand[tokenId] = tokenId;
        land.mintLand(user, landUri, tokenId);
    }
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        ...
        _transfer(from, to, tokenId);
        land.transferFrom(from, to, tokenId);
    }
    ...
    function transferMultiple(address to, uint256[] calldata tokenId) public {
        uint8 length = uint8(tokenId.length);
        for (uint8 i = 0; i < length; i++) {
            _transfer(msg.sender, to, tokenId[i]);
            land.transferFrom(msg.sender, to, tokenId[i]);
        }
    }
