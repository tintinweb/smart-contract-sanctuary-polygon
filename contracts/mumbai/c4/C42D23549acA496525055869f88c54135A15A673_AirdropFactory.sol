...
contract AirdropFactory is UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    function deployMinimal(address _logic, string memory _name, string memory _ticker,
        string memory _contractURI, string memory _tokenURI) external returns (address) {
        address proxy = ClonesUpgradeable.clone(_logic);

        bytes memory _encodedFunction = abi.encodeWithSignature(
            "initialize(string,string,string,string)", _name, _ticker, _contractURI, _tokenURI
        );

        if (_encodedFunction.length > 0) {
            (bool success,) = proxy.call(_encodedFunction);
            require(success, "Proxy call failed");
        }
        emit AirdropAdDeployed(proxy);
