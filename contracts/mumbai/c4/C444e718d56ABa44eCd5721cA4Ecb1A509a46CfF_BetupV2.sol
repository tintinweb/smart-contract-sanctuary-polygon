...
//    mapping(bytes32 => PairsCRound) public _pairsCR;
    ...
    returns(RewardType memory reward)
    {
        ...
        if(_topTokenID > 0) {
            _maxLevel = IMembershipCard(membershipAddress).ownerOf(_topTokenID) == msg.sender ?
                IMembershipCard(membershipAddress).properties(_topTokenID).level :
                0;
        }
    ...
    */
    ...
    function _calcReward(PairsCRound memory _pcr, PlayerRound memory _player, uint256 _result, uint8 _maxLevel)
    ...
    function withdraw(
        ...
        if(_topTokenID > 0) {
            ...
            _maxLevel = imc.properties(_topTokenID).level;
        }
...
interface IMembershipCard {
    struct Property {
        uint8 body;
        uint8 level;
        // consistent of  `face` `accessory` `body` `clothing` `hat`
        // Could hold 2^120 (36/2) 18 parts
        uint120 dna;
    }
//    function calcDaoAmountPerUnit(address _user, uint8 _maxLevel) external view returns(uint256, uint256);
