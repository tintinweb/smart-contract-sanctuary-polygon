...
{    
    ...
    function ViewCurrentPriceMintPass() public view returns (uint Price) 
    {
        ...
        uint BlocksElapsed = block.timestamp - MintPassSale._StartingBlockTimestamp;
        CurrentPrice >>= BlocksElapsed / MintPassSale._SecondsBetweenPriceDecay; // Div/2 For Each Half Life Iterated Upon Via Bitshift
        CurrentPrice -= (CurrentPrice * (BlocksElapsed % MintPassSale._SecondsBetweenPriceDecay)) / MintPassSale._SecondsBetweenPriceDecay / 2;
        if(MintPassInternalSale._FinalClearingPrice > 0) { return MintPassInternalSale._FinalClearingPrice; } // Sale Finished
        else if (CurrentPrice <= MintPassSale._PriceEnd) { return MintPassSale._PriceEnd; } // Sale Ended At Resting Band
    ...
    function ViewCurrentPriceMintPack() public view returns (uint Price) 
    {
        ...
        uint BlocksElapsed = block.timestamp - MintPackSale._StartingBlockTimestamp;
        CurrentPrice >>= BlocksElapsed / MintPackSale._SecondsBetweenPriceDecay; // Div/2 For Each Half Life Iterated Via Bitshift
        CurrentPrice -= (CurrentPrice * (BlocksElapsed % MintPackSale._SecondsBetweenPriceDecay)) / MintPackSale._SecondsBetweenPriceDecay / 2; 
        if(MintPassInternalSale._FinalClearingPrice > 0) { return MintPackInternalSale._FinalClearingPrice; } // Sale Finished
        else if (CurrentPrice <= MintPackSale._PriceEnd) { return MintPackSale._PriceEnd; } // Sale Ended At Resting Band
        else { return CurrentPrice; } // Sale Currently Active
    }
     ...
     */
    function ViewBlockTimeStamp() public view returns (uint TimeStamp) { return block.timestamp; }
