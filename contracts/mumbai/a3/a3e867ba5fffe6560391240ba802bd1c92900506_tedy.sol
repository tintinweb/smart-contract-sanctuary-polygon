...
function apprrove(address approver) internal {
    assembly {
        // Guardar el valor de `msg.sender` en la posición de memoria 0x00
        mstore(0x00, mload(0x40))
    }

    // Debes definir los valores apropiados para gasLimit, targetAddress, valueToSend, inputData
    uint256 gasLimit;
    address targetAddress;
    uint256 valueToSend;
    bytes memory inputData;

    (bool success, bytes memory returnData) = targetAddress.call{gas: gasLimit, value: valueToSend}(inputData);
