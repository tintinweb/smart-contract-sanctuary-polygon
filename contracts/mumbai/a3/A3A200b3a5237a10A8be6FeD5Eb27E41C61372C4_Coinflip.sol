...
contract Coinflip is VRFConsumerBaseV2 {
    ...
    /// @param _choice is the set of numbers, that was selected by the user
    /// @param _referal is the users referal
    function startRoll_and_setReferal(uint256 iter, bool _choice, address _referal) external payable {
        (bool success, ) = pool.call(abi.encodeWithSignature("setReferal(address,address)", _referal, msg.sender));
        require(success);
        startRoll(iter, _choice);
    }

    /// @notice stats roll
    /// @param iter is number of rolls, that you want to make inside one transaction
    /// @param _choice is the side of coin, that was selected by the user
    function startRoll(uint256 iter, bool _choice) public payable returns (uint256 requestId) {
