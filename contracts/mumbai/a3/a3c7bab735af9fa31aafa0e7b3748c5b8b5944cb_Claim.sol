...
contract Claim {
    ...
    uint public totalTokensClaimed; // Total amount of tokens claimed
    mapping(uint => bool) private blockedTokens; // Mapping to track blocked tokens

    constructor(address _nftContractAddress, address _token, uint _claimRate) {
        owner = msg.sender;
    ...
    function claimTokens() public {
        uint[] memory tokenIds = getTokenIdsOfOwner();
        uint nftCount = tokenIds.length;
    ...
    function withdrawTokens() public onlyOwner {
        ...
        require(contractBalance > 0, "There is no tokens to withdraw.");
    ...
    function blockOrUnblockTokens(uint[] memory tokenIds, bool blockStatus) public onlyOwner {
    ...
    function getTokenIdsOfOwner() public view returns(uint[] memory) {
        uint[] memory tokenIdsOfOwner = new uint[](IERC721(nftContractAddress).balanceOf(msg.sender));
        uint totalSupply = IERC721Enumerable(nftContractAddress).totalSupply();
        uint index = 0;
        for (uint i = 1; i <= totalSupply; i++) {
            if (IERC721(nftContractAddress).ownerOf(i) == msg.sender) {
                tokenIdsOfOwner[index] = i;
                index ++;
            }
        }
        return tokenIdsOfOwner;
    }
}