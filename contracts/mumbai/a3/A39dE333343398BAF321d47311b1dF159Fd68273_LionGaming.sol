...
contract Sportsbook is AccessControlUpgradeable, EIP712Upgradeable {
        ...
        onlyRole(AGENT_ROLE)
    {
        ...
        uint256 toWin = calculateToWin(betSlip.amount, odds);
        ...
        for (uint256 i = 0; i < numFlags; i++) {
            ...
            if (permissions[i].allow == true) {
                bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(
                    ...
                    true,
                    betFlagNonces[permissions[i].user]
                )));
            ...
            betFlagNonces[permissions[i].user] += 1;
...
contract LionGaming is AccessControlUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
        ...
        returns (uint256, uint256, uint256)
    {
        ...
        uint256 lionGamingCut = truncate(amount * lionGamingCutBPS / (100**2), coinIndex);
...
contract HousePool {
    ...
    constructor(address lionGaming, address token) {
        stakedToken = IERC20(token);
        lionGamingContract = lionGaming;
    }

    function storeBets(BetInfo[] memory info) public
    {
        for (uint256 i = 0; i < info.length; i++) {
            bettingStakes += info[i].amount * 10**precisionDifference;

            betInfoWithId[info[i].Id].parley = info[i].parley;
    ...
    function setVOI(bytes memory signature, ValuesOfInterest calldata voi) external
    {
    }

    function settleBets(uint256[] memory _Id, uint256[] memory _result) public {
        for (uint256 i = 0; i < _Id.length; i++) {
            if (_result[i] == 1) {
                ...
                stakedToken.transfer(
                    lionGamingContract,
                    betInfoWithId[_Id[i]].amount.sub(betInfoWithId[_Id[i]].commission)
                );
                bettingStakes -=
                    betInfoWithId[_Id[i]].amount.sub(betInfoWithId[_Id[i]].commission) *
                    10**precisionDifference;
    ...
    function updateBets(
        ...
        for (uint256 i = 0; i < _Id.length; i++) {
            if (betInfoWithId[_Id[i]].parley == true)
