...
contract Refinery is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, Applications {
    ...
    */
    ...
    function _averagePrice(uint256 _tokenId) internal view returns (uint) {
        uint _totalPrice;
        uint _counter;
        ...
        uint _avgPrice = _totalPrice.div(_counter);
        return _avgPrice;
    }
    function averagePrice(uint256 _tokenId) external view returns (uint) {
        return _averagePrice(_tokenId);
    }

    function goldRefineryTokenTraits(uint256 _tokenId) external view returns (uint,uint) {
        return (averagePurity(_tokenId),_averagePrice(_tokenId));
    }   
    function compiledAttributes (uint256 _tokenId) internal view returns (string memory) {
        ...
        traits =  string(abi.encodePacked(
            ...
            attributeForTypeAndValue("Estimated Price (USD)", uint2str(_averagePrice(_tokenId).div(1**18))),',',
            attributeForTypeAndValue("Total Weight", uint2str(TokenDetails[_tokenId].weight)),',',
            attributeForTypeAndValue("Average Purity (%) *100", uint2str(averagePurity(_tokenId)))
        ));
