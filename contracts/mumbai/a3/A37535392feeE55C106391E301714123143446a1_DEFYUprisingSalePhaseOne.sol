...
contract DEFYUprisingSalePhaseOne is Pausable, AccessControl {
		...
		whenNotPaused()
	{
		...
		for (uint256 i = 0; i < tier1Invites.length; i++) 
		{
			...
			(bool success,) = inviteOriginalOwner.call{value : msg.value / tier1CommissionDivisor}('');
		...
		for (uint256 i = 0; i < tier2Invites.length; i++) 
		{
			...
			// Pay 10% commission to original owner of invite
			(bool success,) = inviteOriginalOwner.call{value : msg.value / tier2CommissionDivisor}('');
...
interface IDEFYUprisingMask {
  function mintMask(address to) external;
}
