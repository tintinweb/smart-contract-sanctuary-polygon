...
abstract contract ERC721Time is Context, ERC165, IERC721Time, IERC721Metadata {
     ...
     */
    function __ERC721_Init(string calldata sname, string calldata ssymbol) internal {
        _name = sname;
        _symbol = ssymbol;
    }
...
contract FollowNFT is LensNFTBase, IFollowNFT {
    ...
    function _moveDelegate(
        ...
        unchecked {
            ...
            if (!fromZero) {
                ...
                uint256 previous = _snapshots[from][fromSnapshotCount - 1]
                    .value;
                uint128 newValue = uint128(previous - amount);

                _writeSnapshot(from, newValue, fromSnapshotCount);
                emit Events.FollowNFTDelegatedPowerChanged(
                    from,
                    newValue,
                    block.timestamp
                );
            ...
            if (to != address(0)) {
                ...
                if (fromZero) {
                    ...
                    uint128 previousDelSupply = _delSupplySnapshots[
                        delSupplySnapshotCount - 1
                    ].value;
    ...
    function _writeSnapshot(
        ...
        unchecked {
            ...
            mapping(uint256 => Snapshot) storage ownerSnapshots = _snapshots[
                owner
            ];
            ...
            if (
                ...
                ownerSnapshots[ownerSnapshotCount - 1].blockNumber ==
                currentBlock
            ) {
                ownerSnapshots[ownerSnapshotCount - 1].value = newValue;
            } else {
                ownerSnapshots[ownerSnapshotCount] = Snapshot(
                    currentBlock,
                    newValue
                );
    ...
    function _writeSupplySnapshot(uint128 newValue, uint256 supplySnapshotCount)
        internal
    {
        unchecked {
            uint128 currentBlock = uint128(block.number);
            ...
            if (
                ...
                _delSupplySnapshots[supplySnapshotCount - 1].blockNumber ==
                currentBlock
            ) {
                _delSupplySnapshots[supplySnapshotCount - 1].value = newValue;
            } else {
                _delSupplySnapshots[supplySnapshotCount] = Snapshot(
                    currentBlock,
                    newValue
                );
...
interface ILensHub {
     ...
     */
    function follow(uint followType, uint256[] calldata profileIds, bytes[] calldata datas)
        external
     ...
     */
    function getFollowNFT(uint followType, uint256 profileId) external view returns (address);
