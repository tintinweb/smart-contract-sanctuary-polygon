...
contract EXPONA_RBCF {
    ...
    //   mapping(string => address) token; // Token Address Hold with name
      ...
      bool ownerPaid;
      // Events
     event SponsorIncome(address indexed _user, address indexed _referrer, uint _time, string tokenType);
     event LevelsIncome(address indexed _user, address indexed _referral, uint indexed _level, uint _time,string tokenType);
     event AutopoolIncome(string str1,address indexed sender, address indexed referrer, uint indexed height, uint time, string tokenType);
...
function Registration(uint _referrerID, uint256 _amount) public payable {
      ...
      require(token.allowance(msg.sender, address(this)) >= _amount, "NEED_TO_APPROVE_TOKEN");
       ...
       userList[currUserID]=msg.sender;
        ...
        emit SponsorIncome(msg.sender, userList[_referrerID], now,tokenAcceptType );
    }
...
function heightPayment(address _user,uint batch,uint id,uint h) internal{
    ...
    if((users[userList[id]].autopoolPayReciever != address(0)) && (userList[batch] != users[userList[id]].autopoolPayReciever) && (h <= height && h<=2 && id > 0 && ownerPaid!=true)) {
        ...
        sent = token.transferFrom(msg.sender,address(uint160(nextLevel)),Autopool_Level_Income);  
        ...
        if(sent){
            if(h==2){ 
                 ...
                 emit AutopoolIncome("Auto-Pool Payment Successful",_user,nextLevel,h,now,tokenAcceptType );
            }
            else{
                if(isTokenAcceptance){
                    tokenAcceptType = "EXPONA";
        ...
        heightPayment(_user,batch,id,h+1);
            ...
            if((h > 2 && h <= height) && users[userList[id]].referredUsers>=2 
            && (id > 0 && ownerPaid!=true)){
                address nextLevel = userList[id];
                if(!isTokenAcceptance){
                sent = address(uint160(nextLevel)).send(Autopool_Level_Income); 
...
function autoPool(address _user) internal {
    ...
    for(i = 1; i < currUserID; i++){
        if(users[userList[i]].batchPaid < batchSize){
           if(!isTokenAcceptance){
            sent = address(uint160(userList[i])).send(Autopool_Level_Income); 
           }
            else{
                sent = token.transferFrom(msg.sender,address(uint160(userList[i])),Autopool_Level_Income);  
            }    
            ...
            if(sent){
                ...
                emit AutopoolIncome("Auto-Pool Payment Successful",_user,userList[i],1,now,tokenAcceptType);
            }
...
function payReferral(uint _level, address _user) internal {
    ...
    referer = userList[users[_user].referrerID];
        ...
        if (sent) {
            ...
            emit LevelsIncome(referer, msg.sender, _level, now, tokenAcceptType);
