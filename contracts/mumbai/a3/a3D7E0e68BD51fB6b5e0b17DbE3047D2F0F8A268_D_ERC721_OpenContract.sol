 ...
 * @dev `D` in the `D_ERC721_OpenContract` contract means that token Metadata can be updated dynamically.
 */
contract D_ERC721_OpenContract is Ownable, AccessControl, ERC721 {
    struct Token {
        string uri;
    }
    ...
    /**
     * @dev Sets the name, symbol of ERC-721 collection, and contract Uri.
     * Sets `msg.sender` as a `DEFAULT_ADMIN_ROLE`. And sets counter to 1.
     *
     * Requirements:
     *
     * - `name_` cannot be empty.
     * - `symbol_` cannot be empty.
     * - `contractUri_` cannot be empty.
     *
     * @param name_ Name of NFT collection of ERC721 standard.
     * @param symbol_ Symbol of NFT collection of ERC721 standard.
     * @param contractUri_ Metadata contract base Uri.
     */
    constructor(
        string memory name_,
        ...
        require(
            ...
            "D_ERC721_OpenContract: Incorrect name"
        );
        require(
            bytes(symbol_).length != 0,
            "D_ERC721_OpenContract: Incorrect symbol"
        );
        require(
            bytes(contractUri_).length != 0,
            "D_ERC721_OpenContract: Incorrect contract Uri"
        );
     ...
     * - `backendTokenKey_` in `_backendTokenKeyToId` mapping required to not exist at the moment of minting.
     *
    ...
    function mint(
        ...
        require(
            _backendTokenKeyToId[backendTokenKey_] == 0,
            "D_ERC721_OpenContract: Mint need to be unique"
        );
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _tokens[tokenId].uri = string.concat(
            "ipfs://bafybeidwzt64jco62rorro2aedz37qiixvziud4f3owdqda67e4mtcmkki/",
            tokenId.toString()
        );
        _backendTokenKeyToId[backendTokenKey_] = tokenId;
        _keyFallback.push(backendTokenKey_);
        _safeMint(account_, tokenId);
        emit DynamicERC721Minted(account_, uri_, backendTokenKey_, tokenId);
    }
     ...
     *
     * - The caller must be an `DEFAULT_ADMIN_ROLE`.
     * - `backendTokenKey_` must be greater then 0, otherwise token doesnt exist in `_backendTokenKeyToId` mapping.
     ...
     */
    function changeNFT(string memory backendTokenKey_, string memory uri_)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        ...
        uint256 tokenId = _backendTokenKeyToId[backendTokenKey_];
        require(_exists(tokenId), "D_ERC721_OpenContract: Token nonexistent");
        _tokens[tokenId].uri = uri_;
     ...
     * `interfaceId_`.
     *
     * The ERC2981 royalty standard is not supported in this contract.
     * This function call must use less than 30 000 gas.
     */
     ...
     */
    function tokenOwner(string memory backendTokenKey_)
        external
        view
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (address)
     ...
     */
    function getTokenData(uint256 tokenId_)
        external
        view
     ...
     */
    function getTokenKeyId(string memory tokenKey_)
        external
        view
        returns (uint256)
    {
        return _backendTokenKeyToId[tokenKey_];
    }
