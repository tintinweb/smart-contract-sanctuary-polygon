...
contract RewardDistributor is Initializable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    mapping(address => bool) public isDirect;

    event SignerChanged(address newSigner);
    event UserClaimed(address user, address token, uint256 amount);
    event UserDirectClaimed(address user, address token, uint256 amount);
    ...
    function claimRewards(
        ...
        bytes32 _messageHash = getMessageHash(_amount, _msgSender(), _token, _isDirectClaim);
        require(
            _verifySignature(_messageHash, _signature),
            "RewardDistrobutor: invalid signature"
        );
        require(!(_isDirectClaim && isDirect[msg.sender]), "Invalid direct claim");
        ...
        if (_isDirectClaim) {
            isDirect[msg.sender] = true;
            emit UserDirectClaimed(_msgSender(), _token, claimAmount);
        } else {
    ...
    function getMessageHash(
        ...
        return keccak256(abi.encodePacked(_amount, _user, _token, _isDirectClaim));
    }
