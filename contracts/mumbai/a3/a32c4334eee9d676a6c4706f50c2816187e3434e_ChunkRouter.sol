...
* @author DeCommas team
    ...
    * Sends chunks of the user deposit with user metadata
    */
//contract ChunkRouter is ILayerZeroReceiver, Ownable {
contract ChunkRouter is ILayerZeroReceiver, UUPSUpgradeable, OwnableUpgradeable {

//    using EnumerableSet for EnumerableSet.AddressSet;

//    EnumerableSet.Bytes32Set private activeEvents;

    event LocalActionExecuted(address target);
    ...
    modifier onlyRouter {
        //FIXME implement guard
        _;
    }

    mapping(uint256 => mapping(address => uint256)) public pendingDeposits; // strategyId => user => amount
    mapping(uint256 => uint256) public totalStrategyPendingDeposits; // strategyId => amount

    /// StableCoin address of the current network  https://coinmarketcap.com/ru/currencies/usd-coin/
    address private _currentUSDCToken;

    event Deposited(address indexed user, uint256 indexed id, uint256 amount);
    event RequestedWithdraw(address indexed user, uint256 indexed id, uint256 amount);
    event Withdrawn(address indexed user, uint256 indexed id, uint256 amount);
    event CancelWithdrawn(address indexed user, uint256 indexed id, uint256 amount);

    ISgBridge public sgBridge;
    address public nativeLzEndpoint;

    function initialize(address sgBridge_,
                        address currentUSDC,
                        address nativeLzEndpoint_
                        ) external initializer {
        __Ownable_init();
        sgBridge = ISgBridge(sgBridge_);
        _currentUSDCToken = currentUSDC;
        nativeLzEndpoint = nativeLzEndpoint_;
//        actionPool = actionPool_;
    }

    function _authorizeUpgrade(address newImplementation) internal override {}

    function setSgBridge(address newBridge) external onlyOwner {
        sgBridge = ISgBridge(newBridge);
    }

     function deposit(uint256 strategyId, uint256 stableAmount) external {
         require(IERC20(_currentUSDCToken).transferFrom(_msgSender(), address(this), stableAmount), "Transfer failed");

         pendingDeposits[strategyId][msg.sender] += stableAmount;
         totalStrategyPendingDeposits[strategyId] += stableAmount;

         emit Deposited(_msgSender(), strategyId, stableAmount);
     }

    function bridgeInternal(uint256 amount,
        uint16 destinationChain,
        address destinationAddress,
        address destinationToken) public payable {
        sgBridge.bridge{value: msg.value}(
            _currentUSDCToken,
            amount,
            destinationChain,
            destinationAddress,
    ...
    function bridgeUsers(address[] calldata users, uint256 strategy) external onlyRouter {
    ...
    function bridge(bytes memory data) public onlyRouter {
    ...
    function localAction(bytes memory data) public onlyRouter {
        (address[] memory targets, bytes[] memory payloads) = abi.decode(data, (address[], bytes[]));
        require(targets.length == payloads.length, "Router/invalid-data");
        for (uint256 i = 0; i < targets.length; i++) {
            (bool success, bytes memory response) = targets[i].call(payloads[i]);
            if (!success) {
                revert(_getRevertMsg(response));
            }
            emit LocalActionExecuted(targets[i]);
        }
    }
...
//        require(_msgSender() == nativeLzEndpoint, "dcRouter:Sender isn't lzEndpoint");
//        require(
