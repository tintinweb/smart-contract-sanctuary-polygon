...
/******************************************************************************\
/// @title Portal - Exit Facet 
/// @dev This facet provides functions for user to exit their assets in expired deposit record or withdraw their balance based on the latest commit stage. 
/******************************************************************************/
...
contract ExitFacet is IOperations, ReentrancyGuard {
    ...
    /// @dev Event emitted when a pending deposit reuquest is successfully exited.
    event ExitedExpiredDeposit(
        address caller,
        uint128 requestId,
        address receiver,
        uint8 listingId,
    ...
    /// @dev Event emitted when a balance record is successfully exited.
    event ExitAsset(address caller, address defaultReceiver, uint8 listingId, uint88 _tokenId, uint128 _amount);

    /* ------ EXTERNAL FUNTIONS ------ */

    /// @notice Exit expired deposit record. Returns assets to depositor.
    /// @dev This function is guarded by the nonReentrant modifier.
    /// @dev Only available when MassExit mode is activated.
    /// @dev Pending deposit request must expired.
    /// @dev Receiver must not be blacklisted.
    /// @dev Hash of input data must match the stored hash of pending deposit request.
    ...
    function exitExpiredDeposit(
        ...
        require(!s.isBlacklisted[_receiver].status, "ExitFacet: Receiver is blacklisted");
        ...
        // Transfer assets to depositor and emit event.
        for (uint256 i = 0; i < dataLength; i++) {
            LibWithdrawBalance._releaseAsset(_receiver, _listingId, _tokenIds[i], _amounts[i], false);
        }

        emit ExitedExpiredDeposit(msg.sender, _requestId, _receiver, _listingId, _tokenIds, _amounts);
    }

    /// @notice Exit asset in user's balance based on latest commit commit stage root.
    /// @dev This function is guarded by the nonReentrant modifier.
    /// @dev Only available when MassExit mode is activated.
    /// @dev The input data must pass the verification check by re-creating the stage root of the latest commit.
    /// @param _defaultReceiver Receiver's address.
    /// @param _listingId Listing Id of the expired record.
    /// @param _tokenId TokenId of the expired record.
    /// @param _amount Amount of the expired record.
    /// @param _proofs Array of bytes32 hash nodes to regenerate the stage root.
    function exitAsset(
        ...
        // Generate exit key.
        bytes32 exitAssetKey = keccak256(abi.encode(_defaultReceiver, _listingId, s.currentBatchId, _tokenId, _amount));
        require(!s.isExited[exitAssetKey], "Exit: Asset already exited");

        // Invalidate the key and release asset.
        s.isExited[exitAssetKey] = true;

        LibWithdrawBalance._releaseAsset(_defaultReceiver, _listingId, _tokenId, _amount, false);

        emit ExitAsset(msg.sender, _defaultReceiver, _listingId, _tokenId, _amount);
    }
}
