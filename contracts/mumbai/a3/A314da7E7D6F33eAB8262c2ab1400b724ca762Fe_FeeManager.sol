...
contract FeeManager is Ownable {
    ...
    mapping(address => bool) public whitelistedReferralManager;
    ...
    modifier onlyReferralManager() {
        require(
            whitelistedReferralManager[msg.sender],
            "Only referral manager can call"
        );
        _;
    }
    ...
    function setReferralManager(address _referralManager, bool _status)
        external
        onlyOwner
    {
        require(
            _referralManager != address(0),
            "Beneficiary cant be zero address"
        );
        whitelistedReferralManager[_referralManager] = _status;
    }
    ...
    function retrieveReferralFees(
        ...
        if (_feeToken == address(0)) {
            TransferHelper.safeTransferETH(_referralSystem, _referralAmount);
        } else {
            // ERC20 token
