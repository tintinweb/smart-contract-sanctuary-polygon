...
contract AcceptingPostRules is IAcceptingPostRules, Context {
    ...
    constructor(address _registry) {
        ...
        require(soulBoundContract != address(0), "AcceptingPostRules: address can't be zero");
    ...
    function validate(address _communityId, address _user) external view override onlyPlugin returns(bool) {
        ...
        if (isActiveRule(_communityId, RulesList.ACCEPTING_FOR_COMMUNITY_MEMBERS_ONLY)) {
            require(IAccount(registry.account()).isCommunityUser(_communityId, _user), "AcceptingPostRules: wrong user");
            return true;
        }
        if (isActiveRule(_communityId, RulesList.ACCEPTING_AFTER_MODERATOR_APPROVED)) {
            require(IAccount(registry.account()).isModerator(_communityId, _user), "AcceptingPostRules: wrong moderator");
            return true;
        }
        if (isActiveRule(_communityId, RulesList.ACCEPTING_FOR_COMMUNITY_FOUNDERS_ONLY)) {
            require(ICommunityBlank(_communityId).creator() == _user, "AcceptingPostRules: wrong founder");
            return true;
        }
...
interface IAcceptingPostRules is IBaseRules {
