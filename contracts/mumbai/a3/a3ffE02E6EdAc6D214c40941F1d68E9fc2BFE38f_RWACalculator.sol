...
contract RWACalculator is AddressAccessor, IRWACalculator {
        ...
        )
    {
        ITreasuryTracker tracker = ITreasuryTracker(
            addressProvider.getAddress(TREASURY_TRACKER_ADDRESS)
        );

        (usdValueInTreasury, valueNotLatest) = tracker.getRwaUsdValue(
            IERC20Metadata(address(treasuryToken))
        );
        address vaultTracker = addressProvider.getAddress(
            VAULTS_TRACKER_ADDRESS
        );
