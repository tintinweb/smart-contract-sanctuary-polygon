...
// File contracts/interfaces/IIdentityVoting.sol

pragma solidity 0.8.17;

interface IIdentityVoting {
    struct ProposalInfo {
        uint256 proposalId;
        uint256 votedFor;
    ...
    event AddressRegistered(uint256 indexed userId, address indexed registeredAddr);
    event ProposalCreated(uint256 newProposalId, string proposalLink);
    event SuccessfullyVoted(uint256 indexed proposalId, uint256 userId, bool voteFor);

    function changeRegisteredAddress(address newAddr_) external;

    function createProposal(string calldata proposalLink_) external;

    function vote(uint256 proposalId_, bool voteFor_) external;
}
...
// File contracts/IdentityVoting.sol
...
contract IdentityVoting is IIdentityVoting, ZKPVerifier {
    uint64 public constant REGISTER_IDEN_REQUEST_ID = 1;
    ...
    // userId => proposalId => VotedInfo
    mapping(uint256 => mapping(uint256 => VotedInfo)) public alreadyVotedInfo;
    mapping(uint256 => ProposalInfo) public proposalsInfo;

    function changeRegisteredAddress(address newAddr_) external override {
        require(newAddr_ != address(0), "IdentityVoting: Invalid new registration address.");

        uint256 userId_ = addressToUserId[_msgSender()];

        require(
            registeredAddresses[userId_] == _msgSender(),
            "IdentityVoting: Address is not registered."
        );
        require(
            addressToUserId[newAddr_] == 0,
            "IdentityVoting: New address is already registered."
        );

        _registerAddr(userId_, newAddr_);

        delete addressToUserId[_msgSender()];
    }

    function createProposal(string calldata proposalLink_) external override {
        require(bytes(proposalLink_).length > 0, "IdentityVoting: Invalid proposal link.");

        uint256 currentPropId_ = nextProposalId++;

        proposalsInfo[currentPropId_] = ProposalInfo(currentPropId_, 0, 0, proposalLink_);

        emit ProposalCreated(currentPropId_, proposalLink_);
    }

    function vote(uint256 proposalId_, bool voteFor_) external override {
        ProposalInfo storage propInfo = proposalsInfo[proposalId_];

        require(
            bytes(propInfo.proposalLink).length != 0,
            "IdentityVoting: Proposal doesn't exist."
        );

        uint256 userId_ = addressToUserId[_msgSender()];

        require(userId_ != 0, "IdentityVoting: User is not registered.");
        require(
            alreadyVotedInfo[userId_][proposalId_].voterAddr == address(0),
            "IdentityVoting: User has already voted."
        );

        voteFor_ ? propInfo.votedFor++ : propInfo.votedAgainst++;

        alreadyVotedInfo[userId_][proposalId_] = VotedInfo(_msgSender(), voteFor_);

        emit SuccessfullyVoted(proposalId_, userId_, voteFor_);
    }

    function _beforeProofSubmit(
        uint64 requestId_,
        uint256[] memory inputs_,
        ICircuitValidator validator_
    ) internal view override {
        require(requestId_ == REGISTER_IDEN_REQUEST_ID, "IdentityVoting: Invalid request id");

        address addr_ = GenesisUtils.int256ToAddress(inputs_[validator_.getChallengeInputIndex()]);

        require(_msgSender() == addr_, "address in proof is not a sender address");

        require(
            registeredAddresses[_getIdentityId(inputs_, validator_)] == address(0) &&
                addressToUserId[addr_] == 0,
            "IdentityVoting: Identity is already registered"
        );
    }
    ...
    function _afterProofSubmit(
        ...
        _registerAddr(_getIdentityId(inputs_, validator_), _msgSender());
    }
    ...
    function _getIdentityId(
        ...
        return inputs_[validator_.getUserIdInputIndex()];
    }

    function _registerAddr(uint256 userId_, address addrToRegistation_) internal {
        registeredAddresses[userId_] = addrToRegistation_;
        addressToUserId[addrToRegistation_] = userId_;

        emit AddressRegistered(userId_, addrToRegistation_);
    }
}