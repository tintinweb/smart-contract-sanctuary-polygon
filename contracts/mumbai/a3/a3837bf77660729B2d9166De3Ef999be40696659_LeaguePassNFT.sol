...
contract LeaguePassNFT is ERC721Upgradeable, OwnableUpgradeable {
    ...
    /// @notice Fantasy League contract to add users to
    ...
    /// @notice Switch to limit amount of tokens per accout to 1
    ...
    function joinTheLeague(uint256 _nftId) external {
        ...
        require(msg.sender == ownerOf(_nftId), "That isn't your pass");
        _burn(_nftId);
        leagueParticipants[msg.sender] = true;
        fantasyLeague.addUser(msg.sender);
    ...
    function setPaymentToken(address _token, uint256 _NFTprice) public onlyOwner {
    ...
    function setDleagueToken(address _token) public onlyOwner {
    ...
    function setDleagueTokenAmount(uint256 _amount) public onlyOwner {
        dLeagAmount = _amount;
    ...
    function setSaleDeadline(uint256 _deadline) public onlyOwner {
    ...
    function setLeagueStartDate(uint256 _date) public onlyOwner {
    ...
    function setWhitelisteAddr(address _addr, bool _val) public onlyOwner {
        whitelist[_addr] = _val;
        emit WhitelistChanged(_addr, _val);
    }

    /// @notice Sets fantasy league contract
    /// @param _addr Fantasy league contract address
    function setFantasyLeagueContract(address _addr) external onlyOwner {
        fantasyLeague = IFantasyLeague(_addr);
        emit FantasyLeagueContractIsSet(_addr);
    }
    ...
    function setOneTokenPerAccount(bool _state) public onlyOwner {
