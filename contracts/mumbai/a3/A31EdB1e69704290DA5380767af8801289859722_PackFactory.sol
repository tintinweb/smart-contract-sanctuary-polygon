...
contract PackFactory is AccessControl, TinyImports, ReentrancyGuard { 
   ...
   */
    ...
    */   
    function DeployPack(string calldata name, string calldata symbol, string calldata contractURI, address admin, uint NFTsInPack, address royaltyRecipient, uint96 royaltyBasisPoints) external onlyRole(MINTER_ROLE) {
        // following line commented out, not sure what it does, looks like dupe deploy
        // packContract = new Packs(name, symbol, contractURI, admin, NFTsInPack, royaltyRecipient, royaltyBasisPoints);
        address newContract = address(new Packs(name, symbol, contractURI, admin, NFTsInPack, royaltyRecipient, royaltyBasisPoints));
...
contract Packs is SweetUnlimitedNFTMintingFunctions {
    ...
    */
    ...
    /*
    @Dev: These variables initialized during deployment.
    ...
    */
    ...
    bytes calldata signature) external onlyRole(MINTER_ROLE) {
        ...
        _burn(tokenId);
        require(externalContracts.length == momentsSerial.length);
    ...
    */
    ...
    function setMaxNFTsToBeExchangedForPacks(uint newMaxSweetNFTReturns) external onlyRole(MINTER_ROLE) {
...
library sweetAdminLib {
    // ganache testnet
    // address public constant COMMON_ADMIN = 0x7AF6136180f9F15F50217d1e6cCb1b442eF96Ed4;
    // production
