...
contract ProtectionProgram is IProtectionProgram, OwnableUpgradeable {
    ...
    {
        (bankersReward, rebelsReward,,) = _calculateRewards(_nums);
    }
    ...
    function _claim(uint256[] calldata _nums, bool _isWithdraw) private {
        ...
        for (uint256 i = 0; i < _nums.length; i++) {
            if (_isNumsRebel[i]) {
                require(msg.sender == nftToRebelStake[_nums[i]].owner, "PP: caller is not a token owner (1)");
                ...
                require(msg.sender == nftToBankerStake[_nums[i]].owner, "PP: caller is not a token owner (2)");

                if (_isWithdraw) {
                    require(
                        nftToBankerStake[_nums[i]].interactionTimestamp + params.withdrawLockupPeriod < block.timestamp,
    ...
    function _calculateRewards(uint256[] calldata _nums)
        private
        view
        returns (
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _nums.length; i++) {
            ...
            if (address(0) != nftToBankerStake[_nums[i]].owner) {
            ...
            } else if (address(0) != nftToRebelStake[_nums[i]].owner) {
