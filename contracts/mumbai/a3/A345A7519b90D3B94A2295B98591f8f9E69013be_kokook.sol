...
contract kokook is ERC721Full,Ownable, ReentrancyGuard {
    ...
    string  public name =  "asd";//vengeance
    string  public symbol = "^ence";
    string  public external_url = "www.vengeanceDAO.wtf";
   ...
   string private description = "Story goes that every generation has one. Some damned soul, cursed to ride the Multiverse, collecting on the Devil's deals. Many years ago, a CryptoRider was sent to the village of on-chain , to fetch a contract worth 1000 eth souls. But that contract was so powerful, he knew he could never let the Devil get his hands on it. So he did what no Rider has ever done before he outran the Devil himself. The thing about legends is... sometimes, they're true ";
   ...
   uint public YEND = BokkyPooBahsDateTimeLibrary.timestampFromDate(2004,1,1);
    ...
    constructor() ERC721Full("Doffie", symbol) public {
    ...
    function renderFromSeed(uint256 seed ,bool k) public view returns (string memory) {
        bytes32[] memory hashchain = Random.init(seed);
        ...
        Buffer.append(buffer, "<linearGradient id='a' x1='0' y1='0' x2='300' y2='500' gradientUnits='userSpaceOnUse'>"); 
        ...
        Buffer.append(buffer, ";'/><stop offset='100%' style='stop-color:#");
        stopColorBW = Random.uniform_uint(hashchain,16,255);
        Buffer.hexrgb(buffer, stopColorBW, stopColorBW, stopColorBW);
        Buffer.append(buffer, ";'/></linearGradient>");
        
        Buffer.append(buffer, '<rect x="0" y="0" width="640" height="640" fill="url(#a)" stroke="none" />');
        // Draw squiggles. Each squiggle is a pair of <radialGradient> and <path>.
        for(int i=0; i<squiggleCount && (k == false); i++) {
    ...
    returns (uint256[] memory)
  {
   ...
   function _mintLoop(address _receiver, uint256 _mintAmount) private   onlyOwner  {
    ...
    for (uint256 i = 0; i < _mintAmount; i++) {
       ...
       // uint256 seedSVG = uint256(getNumber(tokenId+1,1,99999999));
   ...
   // function createSVG() private nonReentrant {
    ...
    //    require(totalSupply() <= maxSupply, "Claimed. Sorry.");
...
function createChar(uint8 a,uint8 l ) public pure returns (string memory ){
...
function buildImage(uint256 seed) public view returns(string memory) {
     return Base64.encode(bytes(renderFromSeed(seed,false)));
  }
