...
contract MetaBoardsToken is Ownable, IERC20, IERC20Metadata, AccessControlEnumerable, ReentrancyGuard, Pausable {
    ...
    function _transfer(
        ...
        if (gasLimitActive && (isBuy || isSell)) {
        ...
        if(recipient == burnAddress){
            ...
            if(isSell){
                ...
                uint256 maxPercentAmount = totalSupply().mul(maxSellPercent).div(10000);
            ...
            if(isBuy){
                ...
                if(tradingActive) {
                    ...
                    if(!isToSystem && !isFromSystem && !_excludeTaxes.contains(recipient) && buyBurnTax > 0){
                        ...
                        postTaxAmount = amount.sub(toBurn);
...
//        isSwapping = false;
    ...
    function _mint(address account, uint256 amount) internal virtual {
        ...
        // add shares to the vault
        if(!_systemContracts.contains(account) && !_excludeTaxes.contains(account)){
            vault.giveShares(account, amount);
        }
