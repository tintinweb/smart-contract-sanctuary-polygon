...
contract HToken is ERC1155, IERC721Receiver, ErrorReporter, AccessControlEnumerable {
  ...
  /**
   * @notice withdraw fees events
   */
  event AdminFeesWithdrew(uint256 _amount);
  event FuseFeesWithdrew(uint256 _amount);
  ...
  /**
   * @notice keeping track of htokens per user
   */
  mapping(address => uint256) public hTokensPerUser;
  ...
  */
  ...
  function depositUnderlyingInternal(uint256 _amount) internal {
    ...
    hTokensPerUser[msg.sender] += tokensToMint;
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    hTokensPerUser[msg.sender] -= _amount;
  ...
  function withdrawInternal(uint256 _amount) internal {
    ...
    hTokensPerUser[msg.sender] -= tokensToRedeem;
   ...
   * @param _account borrower who pays
   * @param _collateralId what to pay for
   * @return returns true if it's allowed
   */
  function withdrawalAllowed(address _account, uint256 _collateralId) public view returns (bool) {
    Coupon memory coupon = borrowCoupons[_collateralId];
    if (coupon.owner != _account) revert Unauthorized();
  ...
  function burnAndDelete(
    ...
    borrowCouponsIdPerCollateral[_couponId].active = false;
  }
  ...
  function getActiveCoupons() external view returns (Coupon[] memory depositedCoupons) {
    unchecked {
      ...
      uint256 i;
      for (; i < length; i++) {
        if (borrowCouponsIdPerCollateral[i].active) {
          ++deposited;
        }
      }
      i = 0;
      ...
      for (; i < length; i++) {
        if (borrowCouponsIdPerCollateral[i].active) {
          depositedCoupons[j] = borrowCoupons[borrowCouponsIdPerCollateral[i].collateralId];
          j++;
        }
      }
    }
  ...
  function getAvailableUnderlyingForUser(address _account) external view returns (uint256) {
    return (hTokensPerUser[_account] * 1e18) / (exchangeRateStored());
  }
  ...
  /**
   * @notice Withdraw fuse fees
   * @param _amount amount to withdraw from the fuse fees
   */
  function _withdrawFuseFees(uint256 _amount) external {
    ...
    if (totalFuseFees < _amount) {
      revert AdminError(Error.AMOUNT_TOO_BIG);
    }

    if (getCashPrior() < totalFuseFees) {
      revert AdminError(Error.TOKEN_INSUFFICIENT_CASH);
    }

    totalFuseFees -= _amount;

    doUnderlyingTransferOut(dao, _amount);

    emit FuseFeesWithdrew(_amount);
  }
