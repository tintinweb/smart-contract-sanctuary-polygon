    ...
    OwnableUpgradeable 
{
    ...
    //==================
    uint public dailyAddFreeStor; //每日新增存储容量（字节数）  3G=30,000,000,000
    uint public dailyAddFreeFileCount; //每日新增免费文件数
    uint public dailyAddStorPrice; //每日新增存储容量费用（Mb/Ether）0.0001  1000000000000000
    uint public dailyAddPerFilePrice; //每日新增单个文件收费  0.0001 1000000000000000
    ...
    uint public perFilePric;       //单个文件收费 0.0001 1000000000000000
   ...
   // }
    ...
    // 检查账户余额
    //还需要对那些参数检查，是否正常？
    function saveFile(File calldata file)
        public
        //payable
        ...
        )
    {
        ...
        //uint256  totalStorage = userTotalStorage[msg.sender]; 
        //uint256  balance = userBalance[msg.sender];
        //int256 diff = totalStorage + params.size - freeStorage ;

        require(file.size > 0, 'Invalid file size');
        require(userFileUuid[file.owner][file.uuid] == 0, 'user file uuid has  been used');

        // if (totalStorage + file.size > freeStorage ) {  
        //     //检查余额足够一个月费用 
        //     require( 
        //         (balance + msg.value) > (totalStorage + file.size - freeStorage)*storagePrice ,
        ...
        // if (msg.value > 0 ){
        //     userBalance[msg.sender] = balance + msg.value;
        // }
        ...
        detail.totalCount = balanceOf(file.owner);
    ...
    function batchSaveFile(File[] calldata fileList) external{
    ...
    function removeFile(uint256 tokenId)
        public
    {
        ...
        detail.totalCount = balanceOf(file.owner) - 1;
    ...
    function batchRemoveFile(uint256[] memory tokenIds) public
    {
   ...
   function reFund(uint256 value)
    ...
    struct FeeRecord {
        ...
        uint  freeBandwidth;
        uint  dailyAddFreeStor; //每日新增存储容量（字节数）  3G=30,000,000,000
        uint  dailyAddFreeFileCount; //每日新增免费文件数
        uint  dailyAddStorPrice; //每日新增存储容量费用（Mb/Ether）0.0001
        uint  dailyAddPerFilePrice; //每日新增单个文件收费  0.0001 
        ...
        uint  perFilePric;       //单个文件收费 0.0001
    }
    ...
    function updateFeeRecord(uint256 timestamp) public onlyOwner {
        ...
        if (feeRecordList[uint16(year)][uint8(month)][uint8(day)].storagePrice == 0){
            ...
            _feeRecord.dailyAddFreeStor = dailyAddFreeStor;
            _feeRecord.dailyAddFreeFileCount = dailyAddFreeFileCount;
            _feeRecord.dailyAddStorPrice = dailyAddStorPrice;
            _feeRecord.dailyAddPerFilePrice = dailyAddPerFilePrice;
            _feeRecord.freeStorage = freeStorage;
            _feeRecord.freeFileCount = freeFileCount;
            _feeRecord.storagePrice = storagePrice;
            _feeRecord.perFilePric = perFilePric;
        }
    }


    //更新用户存储使用数据，时间戳所在日期的数据（0点后台执行，如果检测到没有昨天的数据，需要执行这个方法）
    function updateDayDetail(address user,uint256 timestamp) public {
    ...
    //获取用户每月历史数据
    function getUserMonthDetail(address user,uint year,uint month ) public view returns (DailyDetail[] memory, uint total) {
        DailyDetail[] memory list;
        total = 0 ;
        for(uint8 i = 1; i < 32; i++) {
           ...
           if (detail.day > 0 ){
               ...
               if(detail.dailycount > dailyAddFreeFileCount){
                    charge = charge + (detail.dailycount - _feeRecord.dailyAddFreeFileCount)*_feeRecord.dailyAddPerFilePrice;
                }
                if(detail.dailySummary > dailyAddFreeStor){
                    charge = charge + (detail.dailySummary - _feeRecord.dailyAddFreeStor)*_feeRecord.dailyAddStorPrice;
                }
                ...
                if(detail.totalCount > freeFileCount){
                    charge = charge + (detail.totalCount - _feeRecord.freeFileCount)*_feeRecord.perFilePric;
                }
    ...
    function getTodayDetail(address user) public view returns(DailyDetail memory){
        (uint year, uint month,uint day) = _daysToDate(block.timestamp);
        ...
        uint charge = 0;
        if(detail.dailycount > dailyAddFreeFileCount){
            charge = charge + (detail.dailycount - dailyAddFreeFileCount)*dailyAddPerFilePrice;
        }
        if(detail.dailySummary > dailyAddFreeStor){
            charge = charge + (detail.dailySummary - dailyAddFreeStor)*dailyAddStorPrice;
        }
        if(detail.totalSize > freeStorage){
            charge = charge + (detail.totalSize - freeStorage)*storagePrice;
        }
        if(detail.totalCount > freeFileCount){
            charge = charge + (detail.totalCount - freeFileCount)*perFilePric;
        }
        detail.charge = charge;
    ...
    // uint public dailyAddFreeStor; //每日新增存储容量（字节数）  3G=30,000,000,000
    // uint public dailyAddFreeFileCount; //每日新增免费文件数
    // uint public dailyAddStorPrice; //每日新增存储容量费用（byte/Ether）0.0000001
    // uint public dailyAddPayPerFile; //每日新增单个文件收费  0.0001 

    // uint public freeStorage;       //每个用户 免费空间， 单位 字节，比如免费 10G =10*1000*1000*1000 字节（按十进制）
    // uint public freeFileCount;     //每个用户 免费文件数
    // uint public storagePrice;      //存储价格：每个月每字节 多少etd ,单位: eth. 0.000000001
    // uint public perFilePric;       //单个文件收费 0.0001

    function setDailyAddFreeStor(uint byteCount) external onlyOwner{
      dailyAddFreeStor = byteCount;
    }

    function setDailyAddFreeFileCount(uint count) external onlyOwner{
      dailyAddFreeFileCount  = count;
    }

    function setDailyAddStorPrice(uint price) external onlyOwner{
      dailyAddStorPrice  = price;
    }

    function setDailyAddPerFilePrice(uint price) external onlyOwner{
      dailyAddPerFilePrice  = price;
    }
    ...
    function setPerFilePric(uint price) public onlyOwner {
        perFilePric = price;
    }
    ...
    function userBill(address user, uint year,uint month) public onlyOwner{
        ...
        (DailyDetail[] memory d, uint charge) = getUserMonthDetail(user,year,month);
        if (charge > 0) {
            require( userChargeFlag[user][year][month] == 0, "user has been charged!");
            uint256  balance = userBalance[user];
