...
// File: Mumbai20221108/Library/EnumerableSet.sol
...
// File: Mumbai20221108/Library/Counters.sol
...
// File: Mumbai20221108/Library/Strings.sol
...
// File: Mumbai20221108/Library/Context.sol
...
// File: Mumbai20221108/Library/Ownable.sol
...
// File: Mumbai20221108/Library/Address.sol
...
// File: Mumbai20221108/Library/IERC165.sol
...
// File: Mumbai20221108/Library/ERC165.sol
...
// File: Mumbai20221108/Library/IERC721.sol
...
// File: Mumbai20221108/Library/IERC721Metadata.sol
...
// File: Mumbai20221108/Library/Base64.sol
...
// File: Mumbai20221108/Library/IERC721Receiver.sol
...
// File: Mumbai20221108/Library/IChallenge.sol
...
// File: Mumbai20221108/Library/ERC721.sol
...
// File: Mumbai20221108/Library/ERC721Burnable.sol
...
// File: Mumbai20221108/Library/ERC721URIStorage.sol
...
// File: Mumbai20221108/ExerciseSupplementNFT.sol
...
contract ExerciseSupplementNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    constructor() ERC721("ExerciseSupplementNFT", "EXER") {
...
// File: Mumbai20221108/Library/SafeMath.sol
...
// File: Mumbai20221108/ChallengeDetail.sol
    ...
    */
    ...
    function giveUp() external canGiveUp notSelectGiveUp onTime available onlyStakeHolders {
        ...
        if (choiceAwardToSponsor) {
            ...
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                if(ERC20(erc20ListAddress[i]).balanceOf(address(this)) > 0) {
                    totalTokenRewardSubtractFee = listBalanceAllToken[i].mul(98).div(100);
                    ERC20(erc20ListAddress[i]).transfer(feeAddress, totalTokenRewardSubtractFee);
                }
            }
            ...
            tranferCoinNative(sponsor, amountToSponsor);
            ...
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                ...
                if(ERC20(erc20ListAddress[i]).balanceOf(address(this)) > 0) {
                    ...
                    ERC20(erc20ListAddress[i]).transfer(sponsor, amountNativeToSponsor);
            ...
            for (uint256 i = 0; i < index; i++) {
                ...
                tranferCoinNative(awardReceivers[i], amountTmp);

                for(uint256 j = 0; j < erc20ListAddress.length; j++) {
                    if(ERC20(erc20ListAddress[j]).balanceOf(address(this)) > 0) {
                        ...
                        ERC20(erc20ListAddress[j]).transfer(awardReceivers[i], amountTokenTmp);    
                    }
                }
            }
        }
    ...
    function transferToListReceiverSuccess() private {
        ...
        for (uint256 i = 0; i < index; i++) {
            ...
            for(uint256 j = 0; j < erc20ListAddress.length; j++) {
                if(ERC20(erc20ListAddress[j]).balanceOf(address(this)) > 0) {
                    ERC20(erc20ListAddress[j]).transfer(feeAddress, awardTokenReceivers[erc20ListAddress[j]][i]);
                }
            }
        ...
        for(uint256 j = 0; j < erc721Address.length; j++) {
            if(erc721Address[j] != address(0)) {
                if(allowGiveUp[2]) {
                    ExerciseSupplementNFT(erc721Address[j]).safeMint(
                        challenger,
                        encodeURI()
                    );
                }

                for(uint256 i = 0; i < ExerciseSupplementNFT(erc721Address[j]).getIndexToken(); i++) {
                    if(ExerciseSupplementNFT(erc721Address[j]).ownerOf(i) == address(this)) {
                        if(!ExerciseSupplementNFT(erc721Address[j]).isApprovedForAll(address(this), challenger)) {
                            ExerciseSupplementNFT(erc721Address[j]).setApprovalForAll(challenger, true);
                        }
                        ExerciseSupplementNFT(erc721Address[j]).safeTransferFrom(address(this), challenger, i);
                    }
                }
            }
        }
        //0x8f17d1351f53c847ed6a4024ae9674b3357e5504
    ...
    /**
     * @dev Private encode data after mint NFT
     */
    function encodeURI() public view returns (string memory) {
        return 
             string(
                abi.encodePacked(
                    /* solhint-disable */
     ...
     */
    function transferNFTWhenFailed() internal {
        for(uint256 j = 0; j < erc721Address.length; j++) {
            if(erc721Address[j] != address(0)) {
                for(uint256 i = 0; i < ExerciseSupplementNFT(erc721Address[j]).getIndexToken(); i++) {
                    address receiver = ExerciseSupplementNFT(erc721Address[j]).getHistoryNFT(i, address(this));
                    if(ExerciseSupplementNFT(erc721Address[j]).ownerOf(i) == address(this) && receiver != address(0)) {
                        if(!ExerciseSupplementNFT(erc721Address[j]).isApprovedForAll(address(this), receiver)) {
                            ExerciseSupplementNFT(erc721Address[j]).setApprovalForAll(receiver, true);
                        }
                        ExerciseSupplementNFT(erc721Address[j]).safeTransferFrom(address(this), receiver, i);
                    }
                }
            }
        }
    ...
    /**
     * @dev pdate balance and total rewards
     */
    function updateRewardSuccessAndfail() internal  {
        ...
        for(uint256 i = 0; i < erc20ListAddress.length; i++) {
            if(ERC20(erc20ListAddress[i]).balanceOf(address(this)) > 0) {
                ...
                ERC20(erc20ListAddress[i]).transfer(feeAddress, listBalanceAllToken[i].mul(2).div(100));
            }
        }
    }
    ...
    function getHistoryTokenAndCoinSendToContract() public view returns(
        uint256, 
        uint256, 
        uint256[] memory, 
        uint256[] memory
        ...
        if(allowGiveUp[1]) {
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                tokenBalanceAfter[i] = ERC20(erc20ListAddress[i]).balanceOf(address(this));
            }
            
            uint256 balanceContract;

            if(getContractBalance() > totalReward) {
                balanceContract = getContractBalance().sub(totalReward);
            }

            return(totalReward, balanceContract, tokenBalanceBefor, tokenBalanceAfter);
        } else {
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                if(i == 0) {
                    ...
                    if(tokenBalanceAfter[i] > totalReward) {
                        tokenBalanceAfter[i] = ERC20(erc20ListAddress[i]).balanceOf(address(this)).sub(totalReward);
                    }
                } else {
                    tokenBalanceAfter[i] = ERC20(erc20ListAddress[i]).balanceOf(address(this));
                }
            ...
            return(0, getContractBalance(), tokenBalanceBefor, tokenBalanceAfter);
        }
    }

    function tranferCoinNative(address payable from, uint256 value) internal {
...
// File: Mumbai20221108/CreateChallenge.sol
