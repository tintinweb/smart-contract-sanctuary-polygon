...
contract PresaleExchange is Initializable {

	address private _whitelistContract;
	address private _presaleContract;
	address private _reserveWETH;
	address private _reserveToken1;
	...
	function initialize(address[4] memory contracts, address _priceFeedAggregator, uint _supply, uint multiplicator) initializer public {
		...
		presaleToken = contracts[3];
		_canBuy = false;
		_canSell = false;
		_canSwap = false;
		_token1 = Akx(presaleToken);



	}

	mapping(address => mapping(address => uint256)) internal _rates;

	function _setRating(address  token1, address token2, uint256 rate) internal {
		_rates[token1][token2] = rate;
	}


	function buy() payable public {
		uint256 _amountToBuy = convertQtyToMatics(msg.value);
		uint256 _reserveBalance = _token1.balanceOf(address(this));
		require(msg.sender != address(0), "no zero address");
		require(_amountToBuy > 0, "you need to send some value");
		require(_amountToBuy <= _reserveBalance, "not enough reserve");
		_token1.transferFrom(address(this), msg.sender, _amountToBuy * 10 ** 18);
