...
contract LimitOrder is OrderBase {
    ...
    function createLimitTask(TccItem[] calldata _tccList) external nonReentrant payable {
    ...
    function limitOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        if (swapInToken != weth) {
            if (IERC20(swapInToken).allowance(address(this), multiDexRouterAddress) < y.tcc._limitItem._swapInAmount) {
                IERC20(swapInToken).approve(multiDexRouterAddress, approveAmount);
            }
        }
        if (swapInToken == weth) {
        ...
        uint256 _totalFee = _fee.add(_poolFee);
        require(_totalFee <= y.tcc._maxFeePerTx, "L11");
        require(userInfoList[_user].ethAmount >= _totalFee, "L12");
        _transfer(_fee, _feeToken);
        payable(usdtPoolAddress).transfer(_poolFee);
        userInfoList[_user].ethAmount = userInfoList[_user].ethAmount.sub(_totalFee);
        userInfoList[_user].ethUsedAmount = userInfoList[_user].ethUsedAmount.add(_totalFee);
        //平台总费用累计
        totalFee = totalFee.add(_totalFee);
        _cancelTask(_taskId);
        SwapEventItem memory t = SwapEventItem(swapInToken, swapOutToken, IERC20(swapInToken).decimals(), IERC20(swapOutToken).decimals(), 0, 0, 0, y.tcc._limitItem._swapInAmount, y.tcc._limitItem._minswapOutAmount, y.tcc._limitItem._swapOutAmount);
        emit OrderEvent(block.number, block.timestamp, y.tcc._type, _user, _taskId, tx.origin, _totalFee, t);
