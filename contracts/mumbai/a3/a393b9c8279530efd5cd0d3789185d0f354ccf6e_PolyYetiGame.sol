...
contract PolyYetiGame is OwnableUpgradeable, YetiSigner, ReentrancyGuardUpgradeable {
    ...
    function levelup(Rarity memory rarity, uint[] memory tokenIds) external onlyOwner{
        address account = rarity.userAddress;
    ...
    function addManyToPalace(Rarity memory rarity, uint[] memory tokenIds, uint8 activityId) external onlyOwner{
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            ...
            if (account != address(yeti)) {
                require(ownerOfToken[tokenIds[i]] == account, "AIN'T YO TOKEN");
    ...
    function ClaimSickYeti(Rarity memory rarity, uint tokenId) public onlyOwner{
        address account = rarity.userAddress;
        require(getSigner(rarity) == account, "Not Account");
    ...
    function Heal(Rarity memory rarity, uint tokenId) external onlyOwner{
        address account = rarity.userAddress;
        require(getSigner(rarity) == account, "Not Account");
