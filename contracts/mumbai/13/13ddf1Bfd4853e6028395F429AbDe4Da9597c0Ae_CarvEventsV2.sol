 ...
 */
contract CarvEventsV2 is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155URIStorageUpgradeable, UUPSUpgradeable {
    ...
    function nontransferrable(uint256 tokenId) external view returns (bool) {
        return(_nontransferrable[tokenId]);
    }

    function setNontransferrable(uint256 tokenId, bool newNontransferrable) external onlyOwner {
        _nontransferrable[tokenId] = newNontransferrable;
        emit NontransferrableSet(tokenId, newNontransferrable);
    }

    function synthetic(uint256 tokenId) external view returns (bool) {
        return(_synthetic[tokenId]);
    }

    function setSynthetic(uint256 tokenId, bool newIsSynthetic) external onlyOwner {
        _synthetic[tokenId] = newIsSynthetic;
        emit SyntheticSet(tokenId, newIsSynthetic);
    }

    function openToCarv(uint256 tokenId) external view returns (bool) {
        return(_openToCarv[tokenId]);
    }

    function setOpenToCarv(uint256 tokenId, bool newOpenToCarv) external onlyOwner {
        _openToCarv[tokenId] = newOpenToCarv;
        emit OpenToCarvSet(tokenId, newOpenToCarv);
    }

    function ingredientBadgeIds(uint256 tokenId) external view returns (uint256[] memory) {
        return(_ingredientBadgeIds[tokenId]);
    }

    function ingredientBadgeAmounts(uint256 tokenId) external view returns (uint256[] memory) {
        return(_ingredientBadgeAmounts[tokenId]);
    }

    // Set the ingredient badge(token) IDs and amounts required to carv the synthetic badge
    function setIngredientBadges(uint256 tokenId, uint256[] memory newIngredientBadgeIds, uint256[] memory newIngredientBadgeAmounts) external onlyOwner {
        require(_synthetic[tokenId], "CarvEvents: Token ID is not synthetic");
        ...
        _ingredientBadgeIds[tokenId] = newIngredientBadgeIds;
        _ingredientBadgeAmounts[tokenId] = newIngredientBadgeAmounts;
        emit IngredientBadgesSet(tokenId, newIngredientBadgeIds, newIngredientBadgeAmounts);
    }

    // Burn ingredient badges to carv synthetic badges
    function carvSynthetic(uint256 tokenId, uint256 amount) external {
        require(_synthetic[tokenId], "CarvEvents: Badge is not synthetic");
        require(_ingredientBadgeAmounts[tokenId].length > 0, "CarvEvents: Ingredient badges are not set");
        require(_openToCarv[tokenId], "CarvEvents: Badge is not open to carv");
        uint256[] memory burnBadgeAmounts = new uint256[](_ingredientBadgeAmounts[tokenId].length);
        for (uint256 i = 0; i < _ingredientBadgeAmounts[tokenId].length; i++) {
            burnBadgeAmounts[i] = _ingredientBadgeAmounts[tokenId][i] * amount;
        }
        _burnBatch(_msgSender(), _ingredientBadgeIds[tokenId], burnBadgeAmounts);
        _mint(_msgSender(), tokenId, amount, "");
        emit SyntheticCarved(_msgSender(), tokenId, amount);
    }

    function carv(address to, uint256 tokenId, uint256 amount) external onlyOwner {
        _mint(to, tokenId, amount, "");
        uint256[] memory tokenIds = _asSingletonArray(tokenId);
        uint256[] memory amounts = _asSingletonArray(amount);
        emit EventsCarved(to, tokenIds, amounts);
    }

    function carvBatch(address to, uint256[] memory tokenIds, uint256[] memory amounts) external onlyOwner {
