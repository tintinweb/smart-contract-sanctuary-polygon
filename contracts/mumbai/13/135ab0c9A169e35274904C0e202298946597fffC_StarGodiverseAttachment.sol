    ...
    IERC1155ReceiverUpgradeable
{
    ...
    /**
     * @notice attach/detach godiverse items, it will call _attachGodiverse/_detachGodiverse
     *
     * @param _starId - star Id
     * @param _slotArray - GodiverseSlot enum
     * @param _slotIndexArray - slot index
     * @param _godiverseIdArray - godiverse Id
     * @param _isAttachArray - true: _attachGodiverse/ false: _detachGodiverse
     */
    function attachAndDetachGodiverses(
        uint256 _starId,
        GodiverseSlot[] memory _slotArray,
        uint256[] memory _slotIndexArray,
        uint256[] memory _godiverseIdArray,
        bool[] memory _isAttachArray
        ...
        require(
            _slotArray.length == _slotIndexArray.length &&
                ...
                _slotArray.length == _isAttachArray.length,
            "Invalid argument"
        );
    ...
    function _detachGodiverse(
        ...
        for (uint256 i = 0; i < orbitalTrackData.length; i++) {
            if (
                orbitalTrackData[i].rentingState == OrbitalTrackState.Renting ||
                orbitalTrackData[i].rentingState == OrbitalTrackState.OwnRenting
            ) {
...
interface IApeironStarOrbitalTrackData {
    // Orbital track
    function planetOrbitalTrackCooldownMap(
        uint256 _planetId
    ...
    function addStarOrbitalTrackData(
        uint256 _starId,
        uint256 _otSlotCount
    ) external;

    function getStarOrbitalTrackData(
        uint256 _starId
    ...
    function checkOwnerAttachPlanet(
        uint256 _starId,
        uint256 _planetId
    ) external view returns (bool, uint256);

    function checkRentalAttachPlanet(
        uint256 _starId,
        uint256 _planetId,
        uint256 _inviteGroupId
    ) external view returns (bool, uint256);

    function checkPlanetAttachedIndex(
        uint256 _starId,
        uint256 _planetId
    ) external view returns (bool, uint256);
    ...
    function getStarOrbitalTrackRentedData(
        uint256 _starId
    ) external view returns (StarMeta.StarOrbitalTrackRentedData[] memory);
}
...
contract StarMeta {
    ...
    struct StarOrbitalTrackData {
        ...
        uint256 originalRentalStartTime;
