...
contract RangoMultichain is IRangoMultichain, BaseInterchainContract, IAnycallProxy {
    ...
    event MultichainBridge(
        MultichainBridgeType _actionType,
        address _fromToken,
    ...
    event AnySwapSwapStatusUpdated(
        address token, 
    ...
    function initialize(address _weth, address[] calldata _addresses) public initializer {
        ...
        addMultichainRoutersInternal(_addresses);
    ...
    function addMultichainRouters(address[] calldata _addresses) public onlyOwner {
        addMultichainRoutersInternal(_addresses);
    }
    ...
    function multichainBridge(
        ...
        require(multichainRouters[_request._multichainRouter], 'Requested router address not whitelisted');
        if (_request._actionType == MultichainBridgeType.OUT_NATIVE) {
            require(msg.value >= _inputAmount, 'Insufficient ETH sent for OUT_NATIVE action');
            require(_fromToken == ETH, 'Invalid _fromToken, it must be equal to null address');
        }

        if (_request._actionType != MultichainBridgeType.OUT_NATIVE) {
            SafeERC20Upgradeable.safeTransferFrom(IERC20Upgradeable(_fromToken), msg.sender, address(this), _inputAmount);
            approve(_fromToken, _request._multichainRouter, _inputAmount);
        }

        IMultichainRouter router = IMultichainRouter(_request._multichainRouter);

        if (_request._actionType == MultichainBridgeType.OUT) {
            router.anySwapOut(_request._underlyingToken, _request._receiverAddress, _inputAmount, _request._receiverChainID);
        } else if (_request._actionType == MultichainBridgeType.OUT_UNDERLYING) {
            router.anySwapOutUnderlying(_request._underlyingToken, _request._receiverAddress, _inputAmount, _request._receiverChainID);
        } else if (_request._actionType == MultichainBridgeType.OUT_NATIVE) {
            router.anySwapOutNative{value: msg.value}(_request._underlyingToken, _request._receiverAddress, _request._receiverChainID);
        } else if (_request._actionType == MultichainBridgeType.OUT_AND_CALL) {
            require(_request._underlyingToken ==_fromToken , 'Token mismatch');
            bytes memory payload = abi.encode(_request._payload);
            router.anySwapOutAndCall(
                _request._underlyingToken,
                toString(_request._receiverAddress), // fixme: convert to string
                _inputAmount,
                _request._receiverChainID,
                toString(_request._receiverContract),
                payload
            );
        } else {
            revert('Unknown multichain bridge type');
        }

        emit MultichainBridge(
            _request._actionType, 
            _fromToken, 
            _request._underlyingToken, 
            _inputAmount, 
            _request._multichainRouter, 
            _request._receiverAddress, 
            _request._receiverChainID
        );
    }

    // this function will be called from multichain executor
    function exec(
        address token,
        address receiver,
        uint256 amount,
        bytes calldata data
    ) override external returns (bool success, bytes memory result) {
        Interchain.RangoInterChainMessage memory message = abi.decode((data), (Interchain.RangoInterChainMessage));
        (address receivedToken, uint dstAmount, OperationStatus status) = handleDestinationMessage(token, amount, message);

        emit AnySwapSwapStatusUpdated(receivedToken, dstAmount, status, message.originalSender, message.recipient);
        return (true, abi.encode(token, amount));
    }

    // fixme: reuse satellite function
