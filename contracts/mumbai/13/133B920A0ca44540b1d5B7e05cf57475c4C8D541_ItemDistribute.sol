...
contract ItemDistribute is Ownable {
    ...
    function add_item(string memory name,uint32 amount,bool as_fallback) external onlyOwner{add_item_internal(name,amount,as_fallback);}
    function add_items(string[] memory names,uint32[] memory amounts) external onlyOwner{
        ...
        for(uint32 i = 0; i < names.length; i++)
            add_item_internal(names[i],amounts[i],false);
    }
    function clear_items() external onlyOwner {
        for(uint32 i = 0; i < items.length; i++)
            delete item_added[items[i].item_hash];
    ...
    function add_item_internal(string memory name,uint32 amount,bool as_fallback) internal {
        bytes32 item_hash = keccak256(abi.encodePacked(name));
        if(!item_added[item_hash]){
            items.push(Item({item : name,amount : amount,item_hash:item_hash,as_fallback :as_fallback }));
