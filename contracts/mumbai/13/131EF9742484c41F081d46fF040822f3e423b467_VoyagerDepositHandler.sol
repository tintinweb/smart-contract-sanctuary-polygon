    ...
    IApplication
{
    ...
    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) => if USDC is burnable and mintable
    mapping(bytes32 => bool) public _isUsdcBurnableMintable;

    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) => depositNonce => usdcNonce if usdc was burnt else 0
    ...
    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) + depositNonce => depositData
    mapping(bytes32 => mapping(uint64 => IDepositExecute.DepositData)) private _depositData;

    // keccak256(abi.encodePacked(destChainType, keccak256(abi.encode(destChainId)))) + depositNonce => Revert Executed?
    mapping(bytes32 => mapping(uint64 => bool)) private _executionRevertCompleted;

    /**---------------------EVENTS--------------------------- */
    // depositType =
    // 1 for token transfers where token is a reserve token
    // 2 for token transfer and execute where token is a reserve token
    // 3 for token transfer where token is a LP token
    // 4 for token transfer and execute where token is a LP token
    // 5 for token transfer where token is a non-reserve token
    // 6 for token transfer and execute where token is a non-reserve token
    event Deposit(
        uint8 depositType,
        uint64 indexed depositNonce,
        bytes32 indexed destChainIdBytes,
        uint256 indexed widgetId
    );

    event DepositRequestReverted(bytes32 indexed destChainIdBytes, uint64 indexed depositNonce);
    ...
    function _isContractWhitelisted(address _contract) private view {
        require(voyagerUtils.isContractWhitelisted(_contract) == true, "provided token not whitelisted");
    }
    ...
    /// @notice Function to set deposit data
    /// @param  _destChainIdBytes chainid bytes for dest chain
    /// @param  _depositNonce nonce for the transaction
    /// @param  _srcStableTokenAddress address of src stable token
    /// @param  _srcStableTokenAmount amount of src stable token deposited
    function setDepositData(
        bytes32 _destChainIdBytes,
        uint64 _depositNonce,
        address _srcStableTokenAddress,
        uint256 _srcStableTokenAmount
    ) internal {
        _depositData[_destChainIdBytes][_depositNonce] = IDepositExecute.DepositData(
            msg.sender,
            _srcStableTokenAddress,
            _srcStableTokenAmount
        );
    }
    ...
    /// @notice Function used to fetch deposit data
    function getDepositData(
        uint32 _destChainType,
        string memory _destChainId,
        uint64 _depositNonce
    ) public view returns (IDepositExecute.DepositData memory) {
        bytes32 destChainIdBytes = getChainIdBytes(_destChainType, _destChainId);
        return _depositData[destChainIdBytes][_depositNonce];
    }
        ...
        returns (IDepositExecute.ArbitraryInstruction memory)
    {
        ...
        ) = abi.decode(arbitraryData, (address, bytes4, bytes, address, uint256, uint256));
        ...
        returns (IDepositExecute.ReserveOrLPSwapInfo memory)
    {
        ...
        ) = abi.decode(swapData, (uint256, address, address, bytes32));
        ...
        returns (IDepositExecute.NonReserveSwapInfo memory)
    {
        ...
        (
            ...
            swapDetails.path,
            swapDetails.flags
        ) = abi.decode(swapData, (uint256, uint256, bytes32, address, address, bytes[], address[], uint256[]));
        return swapDetails;
    }
        ...
        returns (IDepositExecute.ExecuteSwapInfo memory)
    {
        ...
        ) = abi.decode(executeData, (uint256, address, bool, address, address, bytes[], address[], uint256[], uint256));
    ...
    function convertToWeth(uint256 amount) internal {
        ...
        require(IWETH(_WETH).transfer(msg.sender, amount));
    }
    ...
    function depositReserveToken(
        ...
        gatewayContract.requestToRouter(payload, routerBridge);

        emit Deposit(1, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }
    ...
    function depositReseveTokenAndExecute(
        ...
        bytes memory payload = abi.encode(2, swapDetails, executeDetails, arbitraryInstruction);

        gatewayContract.requestToRouter(payload, routerBridge);

        emit Deposit(2, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }

    function _depositReserveToken(IDepositExecute.ReserveOrLPSwapInfo memory swapDetails) internal returns (uint64) {
        ...
        uint64 _depositNonce = depositNonce[swapDetails.destChainIdBytes];
        _handleDepositForReserveToken(_depositNonce, swapDetails);
    ...
    function _handleDepositForReserveToken(uint64 _depositNonce, IDepositExecute.ReserveOrLPSwapInfo memory swapDetails)
        internal
    {
        // if (voyagerUtils.isBurnable(swapDetails.srcTokenAddress) == true) {
        // since in case of reserve tokens, srcTokenAmount = srcStableTokenAmount
        _reserve.burnERC20(swapDetails.srcTokenAddress, msg.sender, swapDetails.srcStableTokenAmount);
        // } else {
        //     _reserve.lockERC20(
        ...
        //         // Burn USDC
        //         handleUsdcBurn(swapDetails.destChainIdBytes, _depositNonce, swapDetails.srcStableTokenAmount);
        //     }
        // }
    ...
    /// @param executeData Execute data for the execution of transaction on the destination chain.
    function depositLPToken(bytes memory swapData, bytes memory executeData) external nonReentrant whenNotPaused {
        IDepositExecute.ReserveOrLPSwapInfo memory swapDetails = decodeReserveOrLpSwapData(swapData);
        IDepositExecute.ExecuteSwapInfo memory executeDetails = decodeExecuteData(executeData);
        swapDetails.depositor = msg.sender;

        uint64 nonce = _depositLPToken(swapDetails);
        executeDetails.depositNonce = nonce;

        bytes memory payload = abi.encode(0, swapDetails, executeDetails);
        gatewayContract.requestToRouter(payload, routerBridge);

        emit Deposit(3, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }
    ...
    function depositLPTokenAndExecute(
        ...
        emit Deposit(4, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }

    function _depositLPToken(IDepositExecute.ReserveOrLPSwapInfo memory swapDetails)
        internal
        isContractWhitelisted(swapDetails.srcTokenAddress)
        returns (uint64)
    {
        require(
        ...
        depositNonce[swapDetails.destChainIdBytes] += 1;
        _reserve.burnERC20(address(swapDetails.srcTokenAddress), msg.sender, swapDetails.srcStableTokenAmount);
    ...
    function depositNonReserveToken(
        ...
        uint64 nonce = _depositNonReserveToken(swapDetails);
        executeDetails.depositNonce = nonce;

        bytes memory payload = abi.encode(1, swapDetails, executeDetails);
        gatewayContract.requestToRouter(payload, routerBridge);

        emit Deposit(5, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }
    ...
    function depositNonReserveTokenAndExecute(
        ...
        bytes memory payload = abi.encode(3, swapDetails, executeDetails, arbitraryInstruction);
        gatewayContract.requestToRouter(payload, routerBridge);

        emit Deposit(6, nonce, swapDetails.destChainIdBytes, executeDetails.widgetID);
    }
        ...
        returns (uint64)
    {
        ...
        _reserve.lockERC20(swapDetails.srcTokenAddress, msg.sender, _oneSplitAddress, swapDetails.srcStableTokenAmount);
        ...
        if (voyagerUtils.isBurnable(swapDetails.srcStableTokenAddress)) {
            ...
            handleUsdcBurn(
                swapDetails.destChainIdBytes,
                depositNonce[swapDetails.destChainIdBytes],
                swapDetails.srcStableTokenAmount
            );
        }
        ...
        whenNotPaused
    {
        ...
        if (txType == 2) {
            (, bytes32 destChainIdBytes, uint64 _depositNonce) = abi.decode(payload, (uint8, bytes32, uint64));
            ...
            emit DepositRequestReverted(destChainIdBytes, _depositNonce);
        }
    }
    ...
    /// @param  _depositNonce deposit nonce for that chain type
    ...
    function handleUsdcBurn(
        ...
        uint64 usdcNonce = usdcBurnerContract.depositAndBurnUsdc(destChainIdBytes, amount);
        _usdcBurnData[destChainIdBytes][_depositNonce] = usdcNonce;
    }
}
...
interface IUsdcDepositAndBurn {
    ...
    function depositAndBurnUsdc(bytes32 destChainIdBytes, uint256 amount) external returns (uint64);
}
