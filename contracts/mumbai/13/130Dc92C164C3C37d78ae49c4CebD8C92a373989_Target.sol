...
contract Target is IXReceiver, Ownable {
    ...
    address public source = address(0x1b8b18D34d7B800461757929fd8f72DD17CA4D8A);
    ...
    function updatesource(address _source) public onlyOwner {
        source = _source;
    }
    ...
    function xReceive(
        ...
        uint256 depositAmount = abi.decode(_callData, (uint256));

        _depositFor(depositAmount);
    }

    function _depositFor(uint256 depositAmount) internal {
        underlyingToken.approve(address(eToken), depositAmount);
        ...
        sendPong(curr_balance - balance);

        balance = curr_balance;
    }

    function sendPong(uint256 eTokenAmount) public payable {
        bytes memory callData = abi.encode(eTokenAmount);
        ...
        connext.xcall{value: 0}(
            ...
            source, // _to: address of the target contract (Ping)
            address(underlyingToken), // _asset: address of the token contract
