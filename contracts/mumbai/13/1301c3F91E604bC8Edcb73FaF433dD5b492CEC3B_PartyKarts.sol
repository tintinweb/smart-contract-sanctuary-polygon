...
contract PartyKarts is ERC721Drop {
    ...
    function leaveRaceLobby(string memory _raceID) external {
        ...
        address[] storage readyPlayers = raceLobby.readyPlayers;
        ...
        for (uint i = 0; i < joinedPlayers.length; i++) {
            if (joinedPlayers[i] == msg.sender) {
                found = true;
                // If player was ready, unready them before leaving
                for (uint j = 0; j < readyPlayers.length; j++) {
                    if (readyPlayers[j] == msg.sender) {
                        readyPlayers[j] = readyPlayers[readyPlayers.length - 1];
                        readyPlayers.pop();
                        break;
                    }
                }
    ...
    function readyForRace(string memory _raceID) external {
        RaceLobby storage raceLobby = raceLobbies[_raceID];
        require(raceLobby.creator != address(0), "Race lobby not found");
        require(raceLobby.isOpen, "Race lobby is not open");
        require(!raceLobby.isStarted, "Race has already started");

        address[] storage joinedPlayers = raceLobby.joinedPlayers;
        uint256 maxRacers = raceLobby.maxRacers;
        require(joinedPlayers.length < maxRacers, "Race lobby is full");
        ...
        for (uint i = 0; i < readyPlayers.length; i++) {
            if (readyPlayers[i] == msg.sender) {
                alreadyReady = true;
                break;
            }
        }
        require(!alreadyReady, "You are already ready for this race");

        readyPlayers.push(msg.sender);
    }

    function unreadyForRace(string memory _raceID) external {
        RaceLobby storage raceLobby = raceLobbies[_raceID];
        require(raceLobby.creator != address(0), "Race lobby not found");
        require(raceLobby.isOpen, "Race lobby is not open");
        require(!raceLobby.isStarted, "Race has already started");
        ...
        for (uint i = 0; i < readyPlayers.length; i++) {
            if (readyPlayers[i] == msg.sender) {
                found = true;
                readyPlayers[i] = readyPlayers[readyPlayers.length - 1];
                readyPlayers.pop();
                break;
            }
        }
        require(found, "You are not ready for this race");
    }
