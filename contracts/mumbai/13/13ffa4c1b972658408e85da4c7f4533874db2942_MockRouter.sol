...
contract MockRouter {
    ...
    function goodPayment(uint256 id, address buyer) public payable {
        RoyaltyReceiver(token).onRoyaltyReceived{value: 1}(
            id,
            buyer,
            address(0),
            ''
    ...
    function badPayment(uint256 id, address buyer) public payable {
        RoyaltyReceiver(token).onRoyaltyReceived(id, buyer, address(0), '');
    }
}
