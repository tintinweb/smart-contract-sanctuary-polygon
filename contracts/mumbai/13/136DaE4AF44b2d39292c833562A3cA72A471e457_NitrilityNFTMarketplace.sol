...
contract NitrilityNFTMarketplace is ERC721URIStorage {
    ...
    EnumerableSet.UintSet _listedLicenseSet;
    Counters.Counter private _listedLicenseIds;
    Counters.Counter private _tokenIds;
    Counters.Counter private _soldLicenseIds;
    
    address payable marketOwner;
    address public company;
    address public nitrilityPool;
    ...
    mapping(string => mapping(uint256 => bool))
        private userIdToListedLicenseVisitedStatus;
    ...
    modifier onlyOwner() {
        require(msg.sender == marketOwner || msg.sender == company, "Only owners can mint");
        _;
    }
    ...
    function getListedLicensesCount() public view returns (uint256) {
        return _listedLicenseIds.current();
    }

    /* Creates a listedLicense */
    function createListedLicense(string memory _tokenURI) public payable {
        require(
            msg.value >= listingPrice,
            "Price must be equal to listing price"
        );
        _listedLicenseIds.increment();
        uint256 _newListedLicenseId = _listedLicenseIds.current();
        _listedLicenseSet.add(_newListedLicenseId);
        ...
        emit ListedLicenseCreated(_newListedLicenseId, _tokenURI, msg.sender, 0);
    }

    /* Returns all the listedLicenses available */
    function fetchListedLicenses() public view returns (ListedLicense[] memory) {
        uint256 itemCount = _listedLicenseSet.length();
        uint256 currentIndex = 0;

        ListedLicense[] memory items = new ListedLicense[](itemCount);

        for (uint256 index = 0; index < itemCount; index++) {
            uint256 currentId = _listedLicenseSet.at(index);
    ...
    function createToken(
        ...
        payable(nitrilityPool).transfer(msg.value.mul(25).div(1000));
        payable(company).transfer(msg.value.mul(25).div(1000));
        _tokenIds.increment();
        uint256 newTokenId = _tokenIds.current();
        _mint(msg.sender, newTokenId);
        _setTokenURI(newTokenId, _newTokenURI);
        createMarketSale(msg.sender, _listedLicenseId, newTokenId, _price, _newTokenURI, _usecase, _licenseName);
        approve(company, newTokenId);

        payable(idToListedLicense[_listedLicenseId].seller).transfer(msg.value.mul(95).div(100));
    ...
    function reCreateBurnedToken(
        ...
        _setTokenURI(newTokenId, _newTokenURI);
        createMarketSale(_ownerAddress, _listedLicenseId, newTokenId, _price, _newTokenURI, _usecase, _licenseName);
        approve(company, newTokenId);
    }
    ...
    function createMarketSale(
        ...
        ListedLicense storage _listedLicense = idToListedLicense[_listedLicenseId];
        uint256 salesCount = _listedLicense.salesCount;
        _listedLicense.salesCount = salesCount.add(1);
        address payable seller = _listedLicense.seller;
        ...
        _soldLicenseIds.increment();
        // payable(owner).transfer(listingPrice);
        emit SoldLicenseCreated(_tokenId, _newTokenURI, seller, msg.sender, _price, _usecase, _licenseName);
    }

    /* Returns all unsold market items */
    function fetchSoldLicenses() public view returns (SoldLicense[] memory) {
        uint256 itemCount = _tokenIds.current();
        uint256 unsoldItemCount = _tokenIds.current() - _soldLicenseIds.current();
        // uint unsoldItemCount = _tokenIds.current();
        uint256 currentIndex = 0;

        SoldLicense[] memory items = new SoldLicense[](unsoldItemCount);
        for (uint256 i = 0; i < itemCount; i++) {
            if (idToSoldLicense[i + 1].owner == address(this)) {
    ...
    function fetchMyNFTs() public view returns (SoldLicense[] memory) {
        uint256 totalItemCount = _tokenIds.current();
        uint256 itemCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalItemCount; i++) {
            if (idToSoldLicense[i + 1].owner == msg.sender) {
                itemCount += 1;
            }
        }
    ...
    function fetchCreatorNFTs(address _address)
        public
        view
    ...
    function fetchItemsListed() public view returns (ListedLicense[] memory) {
        uint256 totalListedLicenseCount = _listedLicenseIds.current();
        uint256 listedLicenseCount = 0;
        uint256 currentIndex = 0;

        for (uint256 i = 0; i < totalListedLicenseCount; i++) {
            if (idToListedLicense[i + 1].seller == msg.sender) {
    ...
    function burnSoldNFT(uint256 _tokenId) public {
        ...
        SoldLicense storage license = idToSoldLicense[_tokenId];
        license.owner = payable(0x0);
        _burn(_tokenId);
    ...
    function burnListedNFT(uint256 _listedLicenseId) public {
        ...
        _listedLicenseSet.remove(_listedLicenseId);
