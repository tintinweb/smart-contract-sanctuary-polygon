...
contract Crowdsale {
    ...
    constructor(uint256 _rate, address _wallet, ERC20 _token) {
        emit DebugUint256("Number/Rate", _rate, _rate > 0);
    ...
    function buyTokens(address _beneficiary) public payable {
        ...
        emit DebugUint256("Number/token.balanceOf", token.balanceOf(address(this)),token.balanceOf(address(this)) >= tokens);
        emit DebugUint256("Number/tokens", tokens, token.balanceOf(address(this)) >= tokens);
    ...
    function _preValidatePurchase(address _beneficiary, uint256 _weiAmount)
        ...
        virtual
    {
        emit DebugAddress("_beneficiary", _beneficiary, _beneficiary != address(0));
