...
contract IDOL_Token {
    ...
    uint256 private FirstRoundBonusTokens=2500000000000000000000;
    uint256 private SecondRoundBonusTokens=2500000000000000000000;
    uint256 private ThirdRoundBonusTokens=2500000000000000000000;
    ...
    //Events for Total Tranfer/sold and blance in Round1 Round2 round3
    event FirstsoldBalance(
        uint256 FirstRoundnumToken_initialToken,
        uint256 firstRoundBalance
    );
    event SecondsoldBalance(
        uint256 SecondRoundnumToken_initialToken,
        uint256 SecondRoundBalance
    );
    event ThirdsoldBalance(
        uint256 ThirdRoundnumToken_initialToken,
        uint256 ThirdRoundBalance
    );
    event FortholdBalance(
        uint256 ForthRoundnumToken_initialToken,
        uint256 forthRoundBalance
    );
    ...
    //forthRoundBalance
    uint256 public forthRoundBalance = 200000000000000000000000;
    ...
    //Tokens Transfer after Stacking periods
    uint256 private TokensAfterStack;
    ...
    //Events for FirstRoundStacked
    event StackedTokens (uint256 Value);
    ...
    constructor(
        ...
        require(_founders.length > 0, "Founders required");
        require(
        ...
        if(ThirdRoundnumToken_initialToken>0){
           uint256 _ThirdRBonusAmount = ThirdRoundBonusTokens/ThirdRoundnumToken_initialToken;
        ...
        returns (bool)
    {
        ...
        if (
            ...
            uint256 confirm_time = block.timestamp + 480;
        ...
        //ROUND 3 : Private VCs (31 Oct.- 06 Nov) Tokens 62,500
            ...
            uint256 confirm_time = block.timestamp + 400;
            locktime[_buyer] = confirm_time;
            ThirdRoundnumToken_initialToken += numOfTokens;
        ...
        //ROUND 4 : Public 07 October Tokens 200,000
            ...
            balances[Private_VCs] =
                ForthRoundnumToken -
                ForthRoundnumToken_initialToken;
            uint256 _forthRoundBalance = balances[Public];
            forthRoundBalance = _forthRoundBalance;
            if (!ownerAppended[_buyer]) {
                ownerAppended[_buyer] = true;
                owners.push(_buyer);
            }
            uint256 txIndex = transactions.length;
            emit EForthroundTranfer(txIndex, msg.sender, _buyer, numOfTokens);
            emit ForthRound_timeEvent(ForthroundBuy_Time);
            emit FortholdBalance(
                ForthRoundnumToken_initialToken,
                forthRoundBalance
            );
        }
    ...
    function TransferToWallets(address _user) public returns (bool) {
        ...
        if(block.timestamp>=locktime[_user]){
            uint256 _FirstRBonusAmount = FirstRoundBonusTokens/FirstRoundnumToken_initialToken;
            FirstRBonusAmount = _FirstRBonusAmount;
            _actualToWallet = numberOfTokens[_user]+(numberOfTokens[_user]*FirstRBonusAmount);
            balances[_user] += _actualToWallet;
            numberOfTokens[_user] = 0;
        }
        // else if(block.timestamp>=locktime[_user]){
        //     uint256 _SecondRBonusAmount = SecondRoundBonusTokens/SecondRoundnumToken_initialToken;
        //     SecondRBonusAmount = _SecondRBonusAmount;
        //     _actualToWallet = numberOfTokens[_user]*SecondRBonusAmount;
        //     balances[_user] += _actualToWallet;
        //     numberOfTokens[_user] = 0;
        // }
        // else if(block.timestamp>=locktime[_user]){
        //     uint256 _ThirdRBonusAmount = ThirdRoundBonusTokens/ThirdRoundnumToken_initialToken;
        //     ThirdRBonusAmount = _ThirdRBonusAmount;
        //     _actualToWallet = numberOfTokens[_user]*ThirdRBonusAmount;
        //     balances[_user] += _actualToWallet;
        //     numberOfTokens[_user] = 0;
        // }
        emit ETranferTowallet(_user, block.timestamp, _actualToWallet);
        return true;
    }
