...
contract OffchainAggregator is OwnerIsCreator, AggregatorV2V3Interface, TypeAndVersionInterface {
   ...
   * @return transmitter who has transmitted the latest report
   * @return nextTransmitter who is the next transmission reporter
   * @return index the index of the latest transmitter in the list
    ...
    )
  {
    ...
    if (s_transmitter.active){
      index = s_transmitter.index;
    }

    // Calculating the index of the next transmitter
    if(s_transmittersList.length > 0){
      if (s_transmittersList.length > index) {
        index++;
      } else {
        index = 0;
      }
      nextTransmitter = s_transmittersList[index];
    }
    ...
    external
  {
    ...
    require(msg.data.length == expectedMsgDataLength(_report), "transmit message length mismatch");
