...
contract EthermonStaking is EthermonStakingBasic {
    ...
    function withdrawDumpStake(
        uint256 _lockId,
        uint256 _index,
        address
    ) external {
        EthermonStakingDumpInterface stakingDump = EthermonStakingDumpInterface(
            stakingDumpContract
        );
        TokenData memory data = stakingDump.getDumpStakerByIndex(
            _lockId,
            _index
        );
        require(data.owner == msgSender(), "You are not the staker");
        require(data.endTime > now, "Time remaining to unstake");
        data.emons += data.balance;
        if (emon.balanceOf(address(this)) >= data.emons) {
            emon.safeTransfer(data.owner, data.emons);
            stakingDump.fetchStaker(data.lockId, data.owner);
            emit Withdraw(
                data.owner,
                address(this),
                data.monId,
