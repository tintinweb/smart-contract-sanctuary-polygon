...
contract DoodleStake is Ownable, IERC721Receiver, ReentrancyGuard, Pausable {
    ...
    modifier requireNFTToken() {
        require(address(nftToken) != address(0), "nftToken not set");
        _;
    }
...
/* STAKING MECHANICS */
    ...
    function setRewardTokenAddress(address rewardTokenAddr) public onlyOwner() {
      rewardToken = IRewardToken(rewardTokenAddr);
    }

    function setNFTTokenAddress(address nftAddr) public onlyOwner() {
      nftToken = IERC721(nftAddr);
    }
    ...
    //deposit function. 
    function deposit(uint256[] calldata tokenIds) external whenNotPaused requireNFTToken requireRewardToken {
    ...
    //withdrawal function.
    function withdraw(uint256[] calldata tokenIds) external whenNotPaused requireNFTToken requireRewardToken nonReentrant() {
        claimRewards(tokenIds);
