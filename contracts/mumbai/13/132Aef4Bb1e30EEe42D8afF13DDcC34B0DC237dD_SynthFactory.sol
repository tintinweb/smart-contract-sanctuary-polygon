...
import "../amm_pool/SynthERC20.sol";
...
interface ISynthAdapter {
    ...
    function chainIdFrom() external returns (uint64); // TODO what if token native in 2-3-4 chains? // []

    function chainSymbolFrom() external returns (string memory);

    function synthType() external returns (uint8);
}
