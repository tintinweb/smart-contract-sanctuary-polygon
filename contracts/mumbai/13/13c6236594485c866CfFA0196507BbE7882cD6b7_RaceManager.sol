...
contract RaceManager {
    ...
    mapping(uint256 => Race) raceInfo;
    mapping(uint256 => uint256) rocketInRace;
    mapping(uint256 => mapping(uint256 => RecruitmentEvent[])) raceEvents; // merge with states?
    mapping(uint256 => mapping(uint256 => RocketRaceState[])) raceRocketStates;
    ...
    function joinRace(uint256 raceId, uint256 rocketId) public {
        ...
        raceRocketStates[raceId][rocketId].push(
            RocketRaceState({
                valid: true,
                time: getRaceInfo(raceId).raceStartingTime, //now
                position: 0,
                velocity: 10e18,
        ...
        returns (uint256)
    {
        ...
        for (uint256 i = 0; i < tmpState.length; i++) {
            ...
            if (
                i + 1 < tmpState.length &&
                tmpState[i + 1].time != 0 &&
                block.timestamp > tmpState[i + 1].time
            ) {
                timeSlot = tmpState[i + 1].time;
            } else {
                timeSlot = block.timestamp;
            }

            uint256 elapsed = block.timestamp - tmpState[i].time;
