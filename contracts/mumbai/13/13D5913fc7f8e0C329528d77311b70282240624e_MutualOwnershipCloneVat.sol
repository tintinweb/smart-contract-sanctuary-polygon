...
import {MutualOwnershipProxy} from "./MutualOwnershipProxy.sol";
import {Manageable} from "../access/Manageable.sol";
import {IInitializer} from "../interfaces/IInitializer.sol";

contract MutualOwnershipCloneVat is Manageable {
    event Cloned(MutualOwnershipProxy clone, address implementation);
    ...
    function clone(bytes memory data)
        external
        returns (MutualOwnershipProxy instance)
    {
        require(data.length > 0, "Missing initialization data");

        // Replicate
        instance = new MutualOwnershipProxy(implementation);

        // Initialize clone
        IInitializer(address(instance)).init(data);
