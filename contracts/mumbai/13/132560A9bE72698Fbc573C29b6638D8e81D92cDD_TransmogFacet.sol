...
contract TransmogFacet is ReentrancyGuard {
    ...
    function mintERC1155ToPortal(uint8 _listingId, uint88 _tokenId, uint88 _amount) external {
    ...
    function mintBatchERC1155ToPortal(uint8 _listingId, uint88[] memory _tokenIds, uint88[] memory _amounts) external {
        IERC1155 tokenAddress = IERC1155(_validateMintable(_listingId, 3));
    ...
    /// @notice Mint a special ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenId Id of the token on its contract.
    /// @param _amount Amount to mint.
    /// @param _salt A random number convert to string.
    /// @param signature A signature from server.
    ...
    /// @notice Mint a batch of Spaceship ERC1155 NFTs into the portal.
    /// @param _listingId Token's listingId.
    /// @param _tokenIds Array of tokenIds.
    /// @param _amounts Array of amount to mint of each tokenId.
    /// @param _salt A random number convert to string.
