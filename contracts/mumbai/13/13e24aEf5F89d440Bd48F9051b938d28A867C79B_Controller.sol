...
contract Transition is TellorStorage, TellorVars {
    ...
    constructor(
        ...
        addresses[Feemanager] = _feemanager;
        addresses[Deployer] = _deployer;
    }
    ...
    function init() external {
        ...
        uints[_MINIMUM_DISPUTE_FEE] = 100e18;
        ...
        addresses[Feemanager] = _controller.addresses(
            Feemanager
        );
        // Mint to oracle, liquidity/hackathons/grants/bounty, and team operating grant contracts
        IController(TELLOR_ADDRESS).mint(
            addresses[_ORACLE_CONTRACT],
            210102e18
        );
        IController(TELLOR_ADDRESS).mint(
            0xcCB00E9c8fAcBB4661ef09Bb4EB5D86fFD989663,
            10005120e18
        );
        IController(TELLOR_ADDRESS).mint(
        ...
        returns (uint256)
    {
        if (_timestamp < uints[_SWITCH_TIME]) {
            return requestDetails[_requestId].finalValues[_timestamp];
             ...
             return IOracle(addresses[_ORACLE_CONTRACT]).getValueByTimestamp(
                bytes32(_requestId),
                _timestamp
            );
    }
