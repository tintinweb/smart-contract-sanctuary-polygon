...
contract Bridge is Ownable, Pausable, ReentrancyGuard {
    ...
    function sendToken(
        ...
        require(
            operator1 == msg.sender || operator2 == msg.sender,
            "Invalid opeartor"
        );
        ...
        if (out.value <= 0) {
            ...
            if (
                (msg.sender == operator1 && out.approveCount == 2) ||
                (msg.sender == operator2 && out.approveCount == 1)
            ) {
                require(
                    _value == out.value && out.token == _token,
                    "Out record mismatch"
                );
                out.approveCount = 3;
    ...
    function sendEther(
        ...
        if (out.value <= 0) {
            ...
            if (
                ...
                require(
                    _value == out.value && out.token == address(0),
                    "Out record mismatch"
                );
                out.approveCount = 3;
        ...
        nonReentrant
    {
        ...
        if (_value > 0) {
            ...
            require(
                (balance2 - balance1) == _value,
                "Transfer token in failure"
            );
            TokenIn storage inRec = ins[inLength];
            inRec.token = _token;
            inRec.from = msg.sender;
    ...
    function getOut(uint256 fromHash)
        public
        view
        returns (TokenOut memory result)
    {
