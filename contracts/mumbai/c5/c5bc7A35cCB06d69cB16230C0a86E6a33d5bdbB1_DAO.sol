...
contract DAO is Ownable, EIP712 {
    ...
    /**
     * @notice Mapping to store signature usage status.
     */
    mapping(bytes => bool) public used;

    /**
     * @notice Mapping to store user voted status for an event.
     */
    mapping(address => mapping(uint256 => bool)) public hasVoted;

    /**
     * @notice Mapping to store unique user vote count for proposal.
     */
   ...
   function vote(
        ...
        //     require(userOptionShare[msg.sender][_eventId][i] == 0, "Already voted for another option in this event");
        // }

        for (uint256 i = 0; i < _event.options.length; i++) {
        require(i == _option || userOptionShare[msg.sender][_eventId][i] == 0, "Already voted for another option in this event");
    }

        optionShare[_eventId][_option] += _amount;
        eventShare[_eventId] += _amount;
        userOptionShare[msg.sender][_eventId][_option] += _amount;
        if (!hasVoted[msg.sender][_eventId]) {
