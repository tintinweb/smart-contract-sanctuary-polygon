...
// File contracts/RiwaNFT.sol


pragma solidity ^0.8.9;
/// Not authorized to call this function.
error RiwaNFT__RoleNotGranted();

contract RiwaNFT is ERC721, Ownable, ReentrancyGuard {
    // variables
    address public contractOwner;
    string private currentBaseURI;
    uint256 public _tokenIdCounter;
    address public signerAddress =
        address(0xC671bfA8f33d3D8AB7252E598b936F06A8d74415);
    ...
    modifier onlyAdmin() {
        if (isOwnerShip[msg.sender] != true) {
            revert RiwaNFT__RoleNotGranted();
        }
        _;
    }
    ...
    constructor() ERC721("RiwaNFT", "RFT") {
    ...
    function verify(
        ...
        bytes32 msgHash = keccak256(
            abi.encodePacked(_message, _amount, _timeStamp, msg.sender)
        );
    ...
    function adminMint(address _to) external onlyAdmin {
        _safeMint(_to, _tokenIdCounter);
        emit NFTMinted(_to, _tokenIdCounter);
        _tokenIdCounter += 1;
    }
    ...
    function publicMint(
        ...
        require(
            verify(_message, _amount, _signature, _expires) == true,
            "signature not verified"
        );

        require(_expires >= block.timestamp, "signature expired");
        require(msg.value == _amount, "invalid payment amount");
        require(!isSignatureUsed[_signature], "This signature is already used");
