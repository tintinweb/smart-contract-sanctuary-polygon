...
contract Gelato1Balance is IGelato1Balance, Proxied {
    ...
    function requestWithdrawal(address _token, uint256 _withdrawalAmount)
        ...
        override
    {
        require(_withdrawalAmount > 0, "Invalid withdrawal amount");
    ...
    function cancelWithdrawalRequest(
        ...
        uint256 _totalFreedAmount,
        bytes32[] calldata _merkleProof
    ) external override {
        require(_cancelledAmount > 0, "Invalid amount");

        bytes32 leafHash = keccak256(
            abi.encode(chainId, msg.sender, _token, _totalFreedAmount)
        );
        ...
        require(
            newTotalWithdrawn <= _totalFreedAmount,
            "Sponsor cannot withdraw more than _totalFreedAmount"
        );
    ...
    function withdraw(
        ...
        require(
            newTotalWithdrawn <= _totalFreedAmount,
            "Cannot withdraw more than _totalFreedAmount"
        );
    ...
    function settle(bytes32 _root, bytes32 _newRoot)
        ...
        onlyManager
    {
        require(root == _root, "Current root does not match");
        root = _newRoot;
    ...
    function settleWithSignature(
        ...
        bytes32 digest = keccak256(
            abi.encode(chainId, root, _newRoot, managerNonce)
        );
        managerNonce++;
    ...
    function collectFeesWithSignature(
        ...
        bytes32 digest = keccak256(abi.encode(chainId, _fees, managerNonce));
        managerNonce++;
    ...
    function _collectFees(Fee[] calldata _fees) private {
        ...
        for (uint256 i; i < _fees.length; i++) {
            require(_fees[i].amount > 0, "Invalid amount");
            ...
            require(
                newTotalWithdrawn <= _fees[i].totalFreedAmount,
                "Cannot collect more than _totalFreedAmount"
            );
