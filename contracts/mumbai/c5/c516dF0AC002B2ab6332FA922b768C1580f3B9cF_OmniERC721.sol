...
contract ERC721A is IERC721A {
    ...
    /**
     * @dev Initial set of the first token ID to be minted.
     */
...
contract OmniERC721 is IOmniERC721, ERC721A, ReentrancyGuard {
     ...
     */
    function _validateMint(uint256 _quantity, address _owner, bytes32[] memory _merkleProof) internal {
        ...
        if (allowlist.isEnabled) {
            if (block.timestamp >= allowlist.publicFrom) {
                ...
                require(isAllowlisted(_merkleProof), "!allowlisted");
    ...
    function isAllowlisted(bytes32[] memory _merkleProof) public view returns (bool) {
...
interface IOmniERC721 {
    function mint(address owner, uint256 quantity, bytes32[] memory _merkleProof) external;
