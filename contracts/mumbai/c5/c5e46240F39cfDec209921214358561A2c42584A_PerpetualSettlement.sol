...
/*
    - If the maintainer deems the last observed settlement-price as correct, they can
    set the perpetual state from EMERGENCY to SETTLE (togglePerpEmergencyState).
    - If the maintainer thinks the last observed settlement-price is unfair (e.g., oracle issue),
    then governance can vote on the settlement price and adjust them via adjustSettlementPrice.
...
contract PerpetualSettlement is PerpetualRebalanceFunctions, IFunctionList, IPerpetualSettlement {
    ...
    /**
     * Governance can adjust the settlement price if the perpetual is in emergency state
     * @param _perpetualId id of the perpetual for which we adjust the settlement prices
     * @param _fSettlementS2 settlement price S2
     * @param _fSettlementS3 settlement price S3
     */
    function adjustSettlementPrice(
        ...
        require(perpetual.state == PerpetualState.EMERGENCY, "perp not in emergency state");
        perpetual.currentMarkPremiumRate.fPrice = 0;
        perpetual.fSettlementS2PriceData = _fSettlementS2;
        perpetual.fSettlementS3PriceData = _fSettlementS3;
    ...
    /**
     * The maintainer can activate the perpetual to be settled once in
     * Emergency state.
     * @param _perpetualId id of the perpetual in SETTLE or EMERGENCY state
     */
    function togglePerpEmergencyState(uint24 _perpetualId) external override onlyMaintainer {
        PerpetualData storage perpetual = _getPerpetual(_perpetualId);
        if (perpetual.state == PerpetualState.EMERGENCY) {
            perpetual.state = PerpetualState.SETTLE;
            emit SettleState(_perpetualId);
        } else if (perpetual.state == PerpetualState.SETTLE) {
            perpetual.state = PerpetualState.EMERGENCY;
            emit SetEmergencyState(
                _perpetualId,
                perpetual.currentMarkPremiumRate.fPrice,
                perpetual.fSettlementS2PriceData,
                perpetual.fSettlementS3PriceData
            );
        } else {
            revert("perp not in emrgcy/settle");
        }
    }
    ...
    function settleNextTraderInPool(uint8 _id) external override nonReentrant returns (bool) {
        ...
        int128 fTotalEmrgAMMMarginCashCC; // margin cash of all AMMs that are in emergency mode
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (perpetual.state == PerpetualState.SETTLE) {
                ...
                fTotalEmrgAMMMarginBalance = fTotalEmrgAMMMarginBalance.add(
                    _getMarginViewLogic().getMarginBalance(idx, address(this))
                );
                int128 fAMMCash = marginAccounts[idx][address(this)].fCashCC;
                fTotalEmrgAMMMarginCashCC = fTotalEmrgAMMMarginCashCC.add(fAMMCash);
        ...
        if (isAllCleared && iEmergencyCount > 0) {
            // Total capital of pools to be settled (=AMM margin) and the
            ...
            fTotalCapitalCC = fTotalCapitalCC.add(fTotalEmrgAMMMarginCashCC);
     ...
     * @param   _fTotalEmrgAMMMarginCashCC      Total AMM margin cash for all emergency-state perpetuals
     */
    function _prepareRedemption(
        LiquidityPoolData memory _liquidityPool,
        int128 _fTotalEmrgAMMMarginBalance,
        int128 _fTotalEmrgAMMMarginCashCC
    ) internal pure returns (int128, int128) {
        // correct the amm margin, default fund, liquidity provider pool,
        // so that no funds are locked.
        // 1) We add the AMM margin to the default fund, if anything left
        ...
        */
        ...
        // if amm balance < 0, we need to pay from the DF, just enough so that balance = 0
        // (accounting for cash already) in perp
        //
        int128 withdrawFromDF = fMgnBalanceToKeep.neg(); //.sub(_fTotalEmrgAMMMarginCashCC);
        ...
        if (withdrawFromDF > 0) {
            ...
            if (withdrawFromParticipationFund > _liquidityPool.fPnLparticipantsCashCC) {
                ...
                require(withdrawFromDF >= 0, "error in redemption rate");
            }
        }
    ...
    function _clearNextTraderInPerpetual(PerpetualData storage _perpetual)
        ...
        returns (bool)
    {
        require(_perpetual.state == PerpetualState.SETTLE, "state should be SETTLE");
        ...
        returns (bool)
    {
        ...
        require(activeAccounts[_perpetual.id].contains(_traderAddr), "account not found");
    ...
    function _setRedemptionRate(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            if (perpetual.state == PerpetualState.SETTLE) {
                ...
                // set AMM margin account to zero
    ...
    function settle(uint24 _perpetualID, address _traderAddr) external override nonReentrant {
        ...
        PerpetualData storage perpetual = _getPerpetual(_perpetualID);
        require(perpetual.state == PerpetualState.CLEARED, "state should be CLEARED");
        require(
            msg.sender == _traderAddr || msg.sender == maintainer(),
            "only maintainer or trader settle"
        );
        ...
        if (marginToReturn > 0) {
            _transferFromVaultToUser(
                _getLiquidityPoolFromPerpetual(perpetual.id),
                _traderAddr,
                marginToReturn
            );
        }
    ...
    /**
     * @notice Transfer any funds stored in this contract to the treasury.
     * @dev These are native token funds, not margin tokens.
     * All pools and perpetuals must be cleared for the transfer to take place. The storage is flushed as well.
     * Reverts if that is not the case, or if there are no funds to transfer.
     * @return success True if funds were found and transferred.
     */
    function transferValueToTreasury() external override nonReentrant returns (bool success) {
        bool allPoolsCleared = true;
        for (uint8 poolId = 1; poolId <= iPoolCount && allPoolsCleared; poolId++) {
            LiquidityPoolData storage liquidityPool = liquidityPools[poolId];
            uint256 length = liquidityPool.iPerpetualCount;
            bool allPerpsCleared = true;
            for (uint256 j = 0; j < length && allPerpsCleared; j++) {
                uint24 idx = perpetualIds[poolId][j];
                PerpetualData storage perpetual = perpetuals[poolId][idx];
                allPerpsCleared = allPerpsCleared && perpetual.state == PerpetualState.CLEARED;
            }
            allPoolsCleared = allPoolsCleared && allPerpsCleared;
        }
        require(allPoolsCleared, "not fully settled");
        if (address(this).balance > 0) {
            // any non-erc20 funds left are sent to the treasury
            // solhint-disable-next-line avoid-low-level-calls
            (success, ) = payable(treasuryAddress).call{ value: address(this).balance }("");
        }
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[2] = this.transferValueToTreasury.selector;
        functionList[3] = this.adjustSettlementPrice.selector;
        functionList[4] = this.togglePerpEmergencyState.selector;
...
contract PerpetualRebalanceFunctions is PerpetualBaseFunctions {
        ...
        internal
    {
        ...
        pool.fDefaultFundCashCC = pool.fDefaultFundCashCC.add(fAmmAmount);
        emit UpdateDefaultFundCash(pool.id, fAmmAmount, pool.fDefaultFundCashCC);
    }
    ...
    function _transferFromPoolToAMMMargin(
        ...
        if (fPoolFunds.add(_fMarginBalance) > 0) {
            ...
            if (fPnLPartFunds > pool.fTargetAMMFundSize) {
                // there are some LP funds allocated to the DF -> split what DF pays
                int128 fDFWeight = pool.fDefaultFundCashCC.div(
                    pool.fDefaultFundCashCC.add(fPnLPartFunds).sub(pool.fTargetAMMFundSize)
                );
        ...
        if (fDFAmount > pool.fDefaultFundCashCC) {
            fLPAmount = fLPAmount.add(fDFAmount.sub(pool.fDefaultFundCashCC));
            fDFAmount = pool.fDefaultFundCashCC;
        }
        // ensure LPs can cover total: otherwise stop the pool
        if (fLPAmount >= fPnLPartFunds) {
            // liquidity pool is depleted
            fLPAmount = fPnLPartFunds;
            _setLiqPoolEmergencyState(pool);
        }
        _decreaseDefaultFundCash(pool, fDFAmount);
        _decreasePoolCash(pool, fLPAmount);
        // update margin
        int128 fFeasibleMargin = fLPAmount.add(fDFAmount);
...
library ConverterDec18 {
    ...
    function fromDecN(int128 x, uint8 decimals) internal pure returns (int128) {
