...
contract Ttt is ERC721, Ownable {
    string private tokenURIPrefix = "ipfs://QmeaCMTcj1XankQ1Y9Koj7MxQHCC1SYXMTznj4UdugJK1e/";
    ...
    constructor(address _beneficiaryAddress) ERC721("ELS SPACE", "ELSS") {
    ...
    function batchMint(uint _count) public onlyOwner {
        for (; supply < _count;) {
            supply += 1;
            ...
            uint _tokenId = random();

            require(minted[_tokenId] != true, "Batch Mint: Token already minted!");
            
            _mint(beneficiaryAddress, _tokenId);
    ...
    function tokenURI(uint _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked(tokenURIPrefix, Strings.toString(_tokenId), ".json"));
    }
    ...
    function random() internal returns (uint) {
        uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, nonce))) % 3500;
        nonce++;
        return randomnumber;
    }
