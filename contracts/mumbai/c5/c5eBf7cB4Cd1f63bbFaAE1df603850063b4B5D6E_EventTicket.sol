...
/**
 * @title Slabs EventTicket
 * ERC1155Tradable - ERC1155 has create and mint functionality, and supports useful standards from OpenZeppelin,like _exists(), name(), symbol(), and totalSupply()
 */
contract EventTicket is ERC1155Supply, ERC2771Context, Authorizable, Whitelist {
    ...
    event SetLazyMint(address indexed _lazyMint);
    ...
    modifier onlyValidOwner() {
        require(lazyMint == _msgSender(), "caller is not valid for call");
        _;
    }
    ...
    function setRoyaltyRegistry(address _address) external onlyOwner {
        ...
        require(_address != royaltyRegistry, "same address already");
        royaltyRegistry = _address;
        emit SetRoyaltyRegistry(_address);
    }

    /**
     * @dev change the lazy mint contract address.
     * @param _address lazy mint address
     */
    function setLazyMint(address _address) external onlyOwner {
        require(_address != address(0), "invalid address");
        require(_address != lazyMint, "same address already");
        lazyMint = _address;
        emit SetLazyMint(_address);
    }

    /**
     * @dev Creates a new token type and assigns _initialSupply to an address
     * NOTE: remove onlyAuthorized if you want third parties to create new tokens on your contract (which may change your IDs)
    ...
    function create(
        ...
        require(!exists(_tokenId), "token is already in use");
        require(_amount > 0, "_amount must be greater than 0");
        _setCreator(_initialOwner, _tokenId);
    ...
    function mint(
        ...
        require(exists(_id), "token id not found");
        ...
        override
    {
        if (operator.isContract()) {
            require(
                ...
                "ticket: Only whitelist is allowed"
            );
        }
...
contract Authorizable is Ownable {
    ...
    function addAuthorized(address _toAdd) external onlyOwner {
        ...
        require(!authorized[_toAdd], "Authorizable: _toAdd is already added");
