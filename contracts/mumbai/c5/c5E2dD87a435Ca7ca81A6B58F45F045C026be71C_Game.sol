...
contract Game is Ownable, ReentrancyGuard {
  ...
  uint256 private _createGoalFee = 0.000001 ether;
  uint256 private _rewardsMultiplier = 5;
  Goal[] public goals;

  mapping(uint256 => address) public goalsToOwner;
  mapping(address => uint256) public ownerGoalsCount;
  ...
  constructor(IERC20 rewardsTokenContractAddress) {
    rewardsToken = rewardsTokenContractAddress;
  }

  function withdraw(uint256 _goalId) external nonReentrant ownerOf(_goalId) {
    require(goals[_goalId].achieved == false, "The goal has been already completed");
    goals[_goalId].achieved = true;
    uint256 rewards = _calculateRewards();
    rewardsToken.safeTransfer(msg.sender, rewards);
    payable(msg.sender).transfer(_createGoalFee);
  }

  function getCreateGoalFee() external view returns (uint256) {
    return _createGoalFee;
  }
  ...
  /// @param _name The name of new goal
  /// @param _expiredAt The timestamp when the goal will get expired
  function createGoal(string memory _name, uint256 _expiredAt) public payable {
    require(msg.value == _createGoalFee, "Goal fee not paid");

    _createGoal(_name, _expiredAt);
  }

  function setCreateGoalFee(uint256 _fee) public onlyOwner {
    _createGoalFee = _fee;
  }
  ...
  function getGoalsByOwner(address _owner) public view returns (uint256[] memory) {
  ...
  function _createGoal(string memory _name, uint256 _expiredAt) private {
  ...
  function _calculateRewards() private view returns (uint256) {
    return _createGoalFee * _rewardsMultiplier;
  }
}