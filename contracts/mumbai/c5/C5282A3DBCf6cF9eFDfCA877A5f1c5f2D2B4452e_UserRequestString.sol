...
interface ISxTRelayProxy is ISxTRelay {
    ...
    function implementationContract() external view returns (ISxTRelay);
...
contract UserRequestString is UserRequest, ConfirmedOwner {
    ...
    function runRequestQuery(string memory query, string memory resourceId, string memory jobId) external nonReentrant returns(bytes32 requestId){
        ...
        ISxTRelay sxtRelayImplementation = sxtRelayInstance.implementationContract();
        require(chainlinkToken.approve(address(sxtRelayImplementation), sxtRelayInstance.FEE()), "UserRequest: Insufficient allowance");
        requestId = sxtRelayInstance.requestQueryString(query, resourceId, address(this), this.saveQueryResponse.selector, jobId);
    }

    /**
     * @dev triggers the requestQueryString function of the SxTRelay contract
     * @param viewName - user viewName
     * @param jobId - Chainlink Job ID that SxT team has created on the Chainlink Oracle Node
     */
    function runRequestView(string memory viewName, string memory jobId) external nonReentrant returns(bytes32 requestId){
        ...
        ISxTRelay sxtRelayImplementation = sxtRelayInstance.implementationContract();
        require(chainlinkToken.approve(address(sxtRelayImplementation), sxtRelayInstance.FEE()), "UserRequest: Insufficient allowance");
        requestId = sxtRelayInstance.requestViewString(viewName, address(this), this.saveQueryResponse.selector, jobId);
    }
