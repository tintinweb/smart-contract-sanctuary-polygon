...
contract CellarStaking is ICellarStaking, Ownable {
    ...
    mapping(ERC20 => Reward) public rewardData;

    ERC20 public immutable override stakingToken;
    ERC20 public immutable baseDistributionToken;
    ERC20 public immutable secondaryDistributionToken;
    ERC20[] public distributionTokens;
     ...
     * @param _baseDistributionToken The base token the staking contract will distribute as rewards.
     * @param _secondaryDistributionToken The additional token the staking contract will distribute as rewards.
     * @param _baseEpochDuration The length of the base reward schedule.
     * @param _secondaryEpochDuration The length of the secondary reward schedule.
    ...
    constructor(
        address _owner,
        ERC20 _stakingToken,
        ERC20 _baseDistributionToken,
        ERC20 _secondaryDistributionToken,
        uint256 _baseEpochDuration,
        uint256 _secondaryEpochDuration,
        uint256 shortBoost,
        uint256 mediumBoost,
        uint256 longBoost,
        ...
        baseDistributionToken = _baseDistributionToken;
        secondaryDistributionToken = _secondaryDistributionToken;

        rewardData[baseDistributionToken].nextEpochDuration = _baseEpochDuration;
        rewardData[secondaryDistributionToken].nextEpochDuration = _secondaryEpochDuration;

        distributionTokens.push(baseDistributionToken);
        distributionTokens.push(secondaryDistributionToken);
    }
        ...
        updateRewards
    {
        ...
        if (amount < minimumDeposit)
            revert USR_MinimumDeposit(amount, minimumDeposit);

        for (uint256 i; i < distributionTokens.length; ++i) {
            ERC20 token = distributionTokens[i];
            Reward storage rData = rewardData[token];

            if (totalDeposits == 0 && rData.rewardsReady > 0) {
                _startProgram(rData.rewardsReady, token);

                rewardData[token].rewardsReady = 0;
                // Need to run updateRewards again
                _updateRewards(token);

            } else if (rData.endTimestamp != 0 && block.timestamp > rData.endTimestamp) {
                revert STATE_NoRewardsLeft();
            }
        }

        //@todo make endTimestamp global?
        ...
        uint256 depositId = stakes[msg.sender].length;

        stakes[msg.sender].push();

        UserStake storage newStake = stakes[msg.sender][depositId];

        newStake.amount = uint112(amount);
        newStake.amountWithBoost = uint112(amountWithBoost);
        newStake.unlockTimestamp = uint32(block.timestamp + lockDuration);
        newStake.lock = lock;

        for (uint256 i; i < distributionTokens.length; ++i) {
            ERC20 token = distributionTokens[i];

            if (!rewardData[token].started) continue;

            newStake.rewardDetails[token].rewardPerTokenPaid = uint112(rewardData[token].rewardPerTokenStored);
        }
    ...
    function unstake(
        uint256 depositId
    ) external override whenNotPaused updateRewards returns (uint256 reward) {
        ...
        updateRewards
    {
        ...
        for (uint256 i = 0; i < userStakes.length; i++) {
            ...
            if (s.amount != 0 && block.timestamp > s.unlockTimestamp) {
                _unstake(i);
            }
        }
    }
    ...
    function _updateRewardForStake(address user, uint256 depositId) internal {
        ...
        for (uint256 i; i < distributionTokens.length; ++i) {
            ...
            uint256 earned = _earned(s, token);

            s.rewardDetails[token].rewards += uint112(earned);
            s.rewardDetails[token].rewardPerTokenPaid = uint112(rewardData[token].rewardPerTokenStored);
        }
    }
    ...
    function _earned(
        ...
        uint256 rewardPerTokenAcc =
            rewardData[_distributionToken].rewardPerTokenStored -
            s.rewardDetails[_distributionToken].rewardPerTokenPaid;
     ...
     * @param depositId             The specified deposit to unstake from.
     */
    function _unstake(uint256 depositId) internal returns (uint256 r) {
        ...
        uint256 depositAmountWithBoost = s.amountWithBoost;

        if (depositAmount == 0) revert USR_NoDeposit(depositId);
        if (s.unlockTimestamp > block.timestamp)
            revert USR_StakeLocked(depositId);
        ...
        for (uint256 i; i < distributionTokens.length; ++i) {
            ...
            uint256 reward = s.rewardDetails[token].rewards;

            s.rewardDetails[token].rewards = 0;

            // Distribute reward
            token.safeTransfer(msg.sender, reward);

            emit Unstake(msg.sender, depositId, depositAmount, reward, token);
        }

        // Boosted amount same as deposit amount, since we have unbonded
        totalDeposits -= depositAmount;
        totalDepositsWithBoost -= depositAmountWithBoost;
    }
    ...
    function _claim(uint256 depositId) internal returns (uint256 reward) {
        ...
        for (uint256 i; i < distributionTokens.length; ++i) {
            ...
            reward = s.rewardDetails[token].rewards;
            ...
            if (reward > 0) {
                ...
                emit Claim(msg.sender, depositId, reward, token);
            }
        }
    ...
    function emergencyClaim() external override {
        ...
        for (uint256 i; i < distributionTokens.length; ++i) {
            ...
            for (uint256 j; j < userStakes.length; ++j) {
                _updateRewardForStake(msg.sender, j);

                UserStake storage s = userStakes[j];
            ...
            if (reward > 0) {
                ...
                emit EmergencyClaim(msg.sender, reward, token);
            }
        }
    }
     ...
     *
     * @param _reward                The amount of rewards to distribute per second.
     * @param _distributionToken     The distribution token.
     */
    function notifyRewardAmount(
        ...
        external
        override
        // @todo use access roles
        onlyOwner
    {
        //@todo test distribution token is allowed
        _updateRewards(_distributionToken);

        uint256 reward = _reward;

        Reward storage rData = rewardData[_distributionToken];

        if (block.timestamp < rData.endTimestamp) {
            uint256 remaining = rData.endTimestamp - block.timestamp;
            uint256 leftover = remaining * rData.rewardRate;
            reward += leftover;
        }

        if (reward < rData.nextEpochDuration) revert USR_ZeroRewardsPerEpoch();

        uint256 rewardBalance = _distributionToken.balanceOf(address(this));
        uint256 pendingRewards = reward + rData.rewardsReady;

        if (rewardBalance < pendingRewards)
            revert STATE_RewardsNotFunded(rewardBalance, pendingRewards);

        // prevent overflow when computing rewardPerToken
        uint256 proposedRewardRate = reward / rData.nextEpochDuration;
        if (
            proposedRewardRate >=
            ((type(uint256).max / ONE) / rData.nextEpochDuration)
        ) {
        ...
        if (totalDeposits == 0) {
            ...
            rData.rewardsReady += reward;
        } else {
            // Ready to start
            _startProgram(reward, _distributionToken);
        }

        rData.lastAccountingTimestamp = block.timestamp;
    }
    ...
    function setRewardsDuration(
        ...
        if (rewardData[_distributionToken].rewardsReady > 0) revert STATE_RewardsReady();

        rewardData[_distributionToken].nextEpochDuration = _epochDuration;
        emit EpochDurationChange(rewardData[_distributionToken].nextEpochDuration);
    }
    ...
    function emergencyStop(
        ...
        for (uint256 i; i < distributionTokens.length; i++) {
            ...
            Reward memory rData = rewardData[token];

            uint256 amountToReturn = token.balanceOf(address(this));
            ...
            if (makeRewardsClaimable) {
                ...
                uint256 remaining = rData.endTimestamp > block.timestamp
                    ? (rData.endTimestamp - block.timestamp) * rData.rewardRate
                    : 0;

                // Make sure any rewards except for remaining are kept for claims
                uint256 amountToKeep = rData.rewardRate *
                    rData.currentEpochDuration -
                    remaining;
     ...
     */
    function latestRewardsTimestamp(ERC20 _distributionToken) public view override returns (uint256) {
        return block.timestamp < rewardData[_distributionToken].endTimestamp
            ? block.timestamp
            : rewardData[_distributionToken].endTimestamp;
    }
    ...
    function rewardPerToken(
        ...
        view
        override
        returns (uint256 newRewardPerTokenStored, uint256 latestTimestamp)
    {
        require(
            _distributionToken == baseDistributionToken ||
            _distributionToken == secondaryDistributionToken,
            "Invalid distribution token"
        );

        latestTimestamp = latestRewardsTimestamp(_distributionToken);

        Reward storage rData = rewardData[_distributionToken];

        if (totalDeposits == 0 || !rData.started) return (rData.rewardPerTokenStored, latestTimestamp);

        //@todo normalize bases when tokens have different decimals

        uint256 timeElapsed = latestTimestamp - rData.lastAccountingTimestamp;
        uint256 rewardsForTime = timeElapsed * rData.rewardRate;
        uint256 newRewardsPerToken = (rewardsForTime * ONE) / totalDepositsWithBoost;

        newRewardPerTokenStored = rData.rewardPerTokenStored + newRewardsPerToken;
    }
    ...
    function getUserStakes(
        ...
        view
        override
        returns (SerializedStake[] memory)
    {
        SerializedStake[] memory stakeList = new SerializedStake[](stakes[user].length);

        for (uint256 i; i < stakeList.length; ++i) {
            UserStake storage userStake = stakes[user][i];

            stakeList[i] = SerializedStake({
                amount: userStake.amount,
                amountWithBoost: userStake.amountWithBoost,
                unlockTimestamp: userStake.unlockTimestamp,
                lock: userStake.lock,
                baseTokenRewards: userStake.rewardDetails[baseDistributionToken],
                secondaryTokenRewards: userStake.rewardDetails[secondaryDistributionToken]
            });
        }
    ...
    function getDistributionTokens() external view returns (ERC20[] memory) {
        return distributionTokens;
    }

    function rewardRate(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].rewardRate;
    }

    function rewardsReady(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].rewardsReady;
    }

    function rewardPerTokenStored(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].rewardPerTokenStored;
    }

    function nextEpochDuration(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].nextEpochDuration;
    }

    function endTimestamp(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].endTimestamp;
    }

    function currentEpochDuration(ERC20 _distributionToken) external view returns (uint256) {
        return rewardData[_distributionToken].currentEpochDuration;
    }
    ...
    modifier updateRewards() {
        for (uint256 i; i < distributionTokens.length; ++i) {
            if (rewardData[distributionTokens[i]].started) _updateRewards(distributionTokens[i]);
        }
        _;
    }
     ...
     */
    function _updateRewards(ERC20 _distributionToken) internal {
        (uint256 currentRewardPerTokenStored, uint256 lastUpdateTime) = rewardPerToken(_distributionToken);

        rewardData[_distributionToken].rewardPerTokenStored = currentRewardPerTokenStored;
        rewardData[_distributionToken].lastAccountingTimestamp = lastUpdateTime;
    }

    /**
     * @dev On initial deposit, start the rewards program.
     *
     * @param _reward                    The pending rewards to start distributing.
     * @param _distributionToken         The rewards token
     */
    function _startProgram(
        uint256 _reward,
        ERC20 _distributionToken
    ) internal {
        // Assumptions
        ...
        rData.rewardRate = _reward / rData.nextEpochDuration;
        rData.endTimestamp = block.timestamp + rData.nextEpochDuration;
        rData.currentEpochDuration = rData.nextEpochDuration;
        rData.started = true;

        emit Funding(_reward, rData.endTimestamp);
    }
    ...
    function _getBoost(
        Lock _lock
    ) internal view returns (uint256 boost, uint256 timelock) {
        if (_lock == Lock.short) {
...
interface ICellarStaking {
    ...
    event Unstake(address indexed user, uint256 depositId, uint256 amount, uint256 reward, ERC20 distributionToken);
    event Claim(address indexed user, uint256 depositId, uint256 amount, ERC20 distributionToken);
    ...
    event EmergencyClaim(address indexed user, uint256 amount, ERC20 distributionToken);
    ...
    struct UserStake {
        ...
        mapping(ERC20 => StakeRewardsDetails) rewardDetails;
        Lock lock;
    }

    struct SerializedStake {
        uint112 amount;
        uint112 amountWithBoost;
        uint32 unlockTimestamp;
        Lock lock;
        StakeRewardsDetails baseTokenRewards;
        StakeRewardsDetails secondaryTokenRewards;
    }
    ...
    struct Reward {
        ...
        uint256 nextEpochDuration;
        uint256 rewardRate;
        uint256 rewardPerTokenStored;
        uint256 rewardsReady;
        bool started;
    }
    ...
    function getDistributionTokens() external returns (ERC20[] memory);

    function currentEpochDuration(ERC20 _distributionToken) external returns (uint256);

    function nextEpochDuration(ERC20 _distributionToken) external returns (uint256);

    function rewardsReady(ERC20 _distributionToken) external returns (uint256);

    function minimumDeposit() external returns (uint256);

    function endTimestamp(ERC20 _distributionToken) external returns (uint256);

    function totalDeposits() external returns (uint256);

    function totalDepositsWithBoost() external returns (uint256);

    function rewardRate(ERC20 _distributionToken) external returns (uint256);

    function rewardPerTokenStored(ERC20 _distributionToken) external returns (uint256);
    ...
    function notifyRewardAmount(uint256 reward, ERC20 _distributionToken) external;

    function setRewardsDuration(uint256 _epochDuration, ERC20 _distributionToken) external;
    ...
    function latestRewardsTimestamp(ERC20 _distributionToken) external view returns (uint256);

    function rewardPerToken(ERC20 _distributionToken) external view returns (uint256, uint256);

    function getUserStakes(address user) external view returns (SerializedStake[] memory);
}
