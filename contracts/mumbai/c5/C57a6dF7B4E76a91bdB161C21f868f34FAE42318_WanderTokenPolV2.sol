 ...
 * functions. 
 */
contract WanderTokenPolV2 is Initializable, ERC20PresetMinterPauserUpgradeable {
    /**
    ...
    event SubmitRequest(address indexed owner, uint indexed reqIndex, address indexed to, uint value, uint method);
    event ExecuteRequest(address indexed executor, uint indexed reqIndex, uint method);
     ...
     * - 2: Transfer a quantity of $WANDER tokens owned by an adminsitrator 
        ...
        public 
    {
        ...
        /**
         * @dev If the adminsitrator wishes to mint more tokens that will
         * exceed the maximum total supply of $WANDER token, revert the
         * execution and return the adminsitrator with an error.
         */
        if ((_method == 3) && ((ERC20Upgradeable.totalSupply() + _value) > MAXSUPPLY)) {
            revert("ERC20Capped: cap exceeded");
        } else if (((_method == 2)) && _value < 1000000*10*18){
            /**
    ...
    function revokeConfirmRequest(uint _reqIndex) public {
        ...
        require((confirmIndex != 100), "req not confirmed");
        ...
        ) 
    {
     ...
     /**
     * @dev the new function that is not present in the previous
     * version of $WANDER token smart contract. Used for testing
     * purposes only.
     */
    function getNewVariableLength() public virtual view returns (uint256) {
        _onlyAdmin();
        return newVariable.length;
    }
     ...
     * the address is an adminsitrator
     */
