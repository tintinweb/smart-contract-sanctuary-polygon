...
contract Bubble is VRFConsumerBaseV2, Ownable {
    ...
    uint public minAmountBet = 0.0001 ether;
    ...
    function loginDouble(uint256 numberGame) external payable {
    ...
    function fulfillRandomWords(
        ...
        uint feeGame = s_games[s_requests[_requestId].numberGame].amountBet * 2 * 15 / 1000;
        s_games[s_requests[_requestId].numberGame].players[_randomWords[0] % 2].transfer(s_games[s_requests[_requestId].numberGame].amountBet * 2 - feeGame);
        amountFee += feeGame;
    }
    ...
    function getGameStatus(uint256 _gameNumber) public view returns (address payable[2] memory players, uint256 amountBet, bool isLive, bool isEnd, uint256 requestId) {
        require(_gameNumber < gameNumber, "number game not found");
        GameStatus memory gameStatus = s_games[_gameNumber];
        return (gameStatus.players, gameStatus.amountBet, gameStatus.isLive, gameStatus.isEnd, gameStatus.requestId);
    }

    function getWinnerNumber(uint256 _gameNumber) public view returns (uint256 winnerNumber) {
        require(_gameNumber < gameNumber, "number game not found");
        require(s_games[_gameNumber].isEnd, "game is not end");
        GameStatus memory gameStatus = s_games[_gameNumber];
    ...
    function withdraw() external onlyOwner {
        require(amountFee > 0.00001 ether, "Balance fee < 0.1 ether");
        address owner = owner();
