...
contract ChainBattles is ERC721URIStorage {
    ...
    uint256 private constant MAX_LEVEL_GROWTH = 3;
    ...
    function getAttack(uint256 tokenId) public view returns (string memory) {
        uint256 attack = tokenIdToCharacter[tokenId].attack;
    ...
    function train(uint256 tokenId) public {
        ...
        character.level += 1;
        character.hp += getLevelGrowth("hp");
        character.attack += getLevelGrowth("attack");
        character.defense += getLevelGrowth("defense");
    ...
    function getLevelGrowth(string memory attributeName) private view returns (uint256){
        uint256 randomInt = uint256(keccak256(abi.encodePacked(attributeName, block.number)));

        uint256 levelGrowth = 0;

        for (uint256 i=1; i<=MAX_LEVEL_GROWTH; i++)
        {
            if (randomInt % i == 0)
            {
                levelGrowth++;
