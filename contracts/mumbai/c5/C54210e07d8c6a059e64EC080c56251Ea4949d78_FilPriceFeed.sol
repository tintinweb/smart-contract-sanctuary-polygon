...
// File: https://github.com/tellor-io/usingtellor/blob/master/contracts/interface/IMappingContract.sol
...
// File: https://github.com/tellor-io/usingtellor/blob/master/contracts/interface/IERC2362.sol
...
// File: https://github.com/tellor-io/usingtellor/blob/master/contracts/interface/ITellor.sol
...
// File: https://github.com/tellor-io/usingtellor/blob/master/contracts/UsingTellor.sol
...
// File: contracts/PriceFeedPolygon.sol
...
contract FilPriceFeed is UsingTellor {
    bytes public queryDataFIL = abi.encode("SpotPrice", abi.encode("fil", "usd"));
    bytes32 public queryIdFIL = keccak256(queryDataFIL);
    uint256 public filPrice;

    bytes public queryDataMATIC = abi.encode("SpotPrice", abi.encode("matic", "usd"));
    bytes32 public queryIdMATIC = keccak256(queryDataMATIC);
        ...
        public
    {
        ...
        (bytes memory _value, uint256 _timestampRetrieved) =
            getDataBefore(queryIdFIL, block.timestamp - 15 minutes);
    ...
    function readMaticPrice()
        public
    {
        // Retrieve data at least 15 minutes old to allow time for disputes
        (bytes memory _value, uint256 _timestampRetrieved) =
            getDataBefore(queryIdMATIC, block.timestamp - 15 minutes);
        // If timestampRetrieved is 0, no data was found
        if(_timestampRetrieved > 0) {
            // Check that the data is not too old
            if(block.timestamp - _timestampRetrieved < 24 hours) {
                // Use the helper function _sliceUint to parse the bytes to uint256
                maticPrice = _sliceUint(_value);
            }
        }
    }
