...
contract distributorContract is ReentrancyGuardUpgradeable,OwnableUpgradeable {
        ...
        mapping(address=>uint) public getUserTotalVotes;
        // events
        ...
        function deposit(uint poolId, uint amount,string memory uri)external{
                logic(poolId, amount,true,0, uri);
        }

        function investBack(uint poolId, uint _nftId,uint desiredPool,uint _amount, string memory uri) external {
         uint amount= getRewardDetails(poolId, _nftId);// 32600000000000000000
          ...
          logic(desiredPool,_amount,false,poolId,uri);
          detailsPerNftId[_nftId][poolId].lastClaimTime=block.timestamp;
          remainningRewards[poolId][_nftId]=remainAmount;
        }

        function logic (uint poolId,uint tokenAmounts,bool status, uint actualOne, string memory uri) internal nonReentrant {
                ...
                getUserTotalVotes[msg.sender]+=tokenAmount;
                uint currentIndexOfPool = nftIndexPerPool[poolId]+1;
                uint weightageEquivalent=tokenAmount/100;
                ...
                if ((poolId== 1) || (actualOne==1)) {
                        ...
                        token.transferFrom(msg.sender, insuranceFundAddress, insuranceAmount);
                          ...
                          IPool(collectionPools[actualOne]).DistributePLXR(insuranceFundAddress,insuranceAmount);  
                        }
                } 
                ...
                if(status){
               token.transferFrom(msg.sender, collectionPools[pool], total);}
               else{
    
                    IPool(collectionPools[actualOne]).DistributePLXR(collectionPools[pool], total);
               }
               IGovernanceNFT(governanceTokenAddress[pool]).mintTokens(msg.sender, nftIndexPerPool[pool], uri);
               emit Deposit(msg.sender, collectionPools[pool],total, pool ,currentIndexOfPool);
        }
        ...
        function claimFunds (uint[] memory poolIds, uint[] memory _nftIds, uint[] memory _amount) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        if(poolIds[i]==1){
                            ...
                            amount-=insure;
                             IPool(collectionPools[poolIds[i]]).DistributePLXR(msg.sender, insure);
                        }else{
                        ...
                        IPool(collectionPools[poolIds[i]]).DistributePLXR(msg.sender, amount);
        ...
        function unstakeTokens (uint[] memory poolIds, uint[] memory _nftIds) external nonReentrant {
                ...
                for (uint i=0; i< poolIds.length; i++) {
                        ...
                        require(block.timestamp>unStakeConfirmedTime[msg.sender][_nftIds[i]][poolIds[i]]+watingTime,"Funds In Processing");
                        require (IGovernanceNFT(governanceTokenAddress[poolIds[i]]).ownerOf(_nftIds[i])==msg.sender,'Error: Caller Not Owner');
                        ...
                        if(poolIds[i]==1){
                           ...
                           IPool(collectionPools[poolId]).DistributePLXR(insuranceFundAddress,insurance); 
                        }
                        ...
                        IPool(collectionPools[poolId]).DistributePLXR(msg.sender,finalAmount);
        ...
        function setWaitTime(uint _time) external onlyOwner{
               watingTime=_time;
        }
        ...
        function calculateGovernanceToken(uint amount) external pure returns(uint, string memory ){
        ...
        function changeDetailsOfInvestors(address _to, address _nftAddress, uint _tokenId) external {
                ...
                uint newTermId = ++currentTermIdForUserPerPool[_to][poolId];
