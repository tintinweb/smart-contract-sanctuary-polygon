...
contract L2_SVG721 is SVG721, IL2_SVG721, ERC1155HolderUpgradeable {
	...
	function stake(
		...
		uint256 slotIndex = BaseEquipmentNFT(Equipment)
	...
	function unStake(uint256 tokenId, uint256 equipmentId) external {
		...
		for (uint256 i = 0; i < numFeatures; i++) {
			...
			uint256 equippedAt = equipped[tokenId][equipmentId].equippedAt;
			string memory equippedFor = equipped[tokenId][equipmentId]
				.equippedFor;

			if (currentTime > equippedAt)
				stakingDuration = equippedAt - currentTime;

			BaseEquipmentNFT.FeatureData memory featureData = BaseEquipmentNFT(
				Equipment
			).feature(equipmentId, featureNamesArr[i]);

			uint256 multiplier = stakingDuration.div(8 hours);
		...
		for (uint256 i = 0; i < numFeatures; i++) {
			...
			for (uint256 index = 0; index < equipmentIds.length; index++) {
				...
				IBaseEquipmentNFT.FeatureData
					memory featureData = BaseEquipmentNFT(Equipment).feature(
						equipmentIds[index],
						featureNamesArr[i]
					);
	...
	function setAllowedAddress(address addr, bool enabled) external {
		allowedTransferAddresses[addr] = enabled;
	}

	function setAllowTransfersOnL2(bool allow) external {
...
contract SVG721 is ISVG721, ERC721URIStorageUpgradeable, BaseNFT {
	...
	// STATE METHODS
	function mint(address to) public override onlyAdmin {
		tokenIds.increment();
