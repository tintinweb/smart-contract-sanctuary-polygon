...
contract TradableOdds is BaseRelayRecipient {
    ...
    event Debug(string label, uint data1, uint data2, uint data3);
    constructor(/*address trustedForwarder*/){
        owner = payable(_msgSender());
        _setTrustedForwarder(address(0x9399BB24DBB5C4b782C70c2969F58716Ebbd6a3b)); //Biconomy Matic Testnet
    }
    ...
    function doBuySellTxn(bytes32 key, Outcome yesNo, int256 units, uint256 worstPrice) public {
        ...
        if (units < 0){
            if (yesNo == Outcome.YES){
                require(playerPositions[key][_msgSender()].yes >= uint256(units * -1), 'Not enough Yes coins to sell');
            }
            else {
                require(playerPositions[key][_msgSender()].no >= uint256(units * -1), 'Not enough No coins to sell');
            }
        }
    ...
    function getUnitsWithSlipppage(bytes32 key, Outcome yesNo, int256 units, uint256 worstPrice) public view returns (int256 slippageUnits){
        ...
        if (yesNo == Outcome.YES){
            ...
            if (units > 0){//buy Yes
                require(worstPrice > yPrice, 'Buy Worst Price below Yes Price');
                cost = numerator *1e18 / (2*(1e18 - worstPriceSigned));
            }
            else {//sell Yes
                require(worstPrice < yPrice, 'Sell Worst Price above Yes Price');
            ...
            if (units > 0){//buy No
                require(worstPrice > nPrice, 'Buy Worst Price below No Price');
                cost = numerator*1e18 / (2*(1e18 - worstPriceSigned));
            }
            else {//sell No
                require(worstPrice < nPrice, 'Sell Worst Price above No Price');
                cost = numerator*1e18 / (2*(worstPriceSigned - 1e18));
            }
        }
    ...
    function _process(bytes32 key, Outcome yesNo, int256 units, int256 cost) internal {
        ...
        emit BuySellTxn(key, _msgSender(), block.timestamp, yesNo, units, int256(costWithCommission), yes, no);  
    }
    function _buyCommission(bytes32 key, int256 cost) internal returns (uint256 costWithCommission){
        ...
        require(balances[_msgSender()] >= uint256(costWithCommission), 'Not enough balance');
        balances[_msgSender()] -= uint256(costWithCommission);
    ...
    function _updatePool(bytes32 key, Outcome yesNo, int256 units, int256 cost) internal {
        if (cost > 0){
            if (yesNo == Outcome.YES){
                ...
                playerPositions[key][_msgSender()].yes += (uint256(units));
                ...
                playerPositions[key][_msgSender()].no += (uint256(units));
    ...
    function computeOpenClosePrice(bytes32 id, bool isOpen) public returns (uint){
        ...
        if (relevantRound > 0){
            if (roundPrice > 0){
                if (isOpen) {
                    financialMarkets[id].openingPrice = roundPrice;
                    financialMarkets[id].openingRound = relevantRound;
                } else {
                    financialMarkets[id].closingPrice = roundPrice;
                    financialMarkets[id].closingRound = relevantRound;
                }
            }
        }
        return roundPrice;
    ...
    function getRelevantRound(address oracle, uint timestampBoundary) public view returns (uint80 relevantRound, uint relevantRoundPrice){
        ...
        // emit Debug('RoundTimeStamp, TimeStampBoundary', roundTimestamp, timestampBoundary, roundId);
        while (roundTimestamp > timestampBoundary){
            (roundId,price,,roundTimestamp,) = AggregatorV3Interface(oracle).getRoundData(roundId - 1);
            //  emit Debug('LOOP', roundTimestamp, uint(price), roundId);
    ...
    function resolveFinancialMarket(bytes32 id, bool movePlayerClaims) public returns (uint closingPrice, uint80 closingRound){
        ...
        return (financialMarkets[id].closingPrice, financialMarkets[id].closingRound);
    }
    function resolveNonFinancialMarket(bytes32 id, Outcome outcome, bool movePlayerClaims) external {
        require(_msgSender() == predictionEvents[id].creator || _msgSender() == owner, 'Only Creator/Operator can resolve');
    ...
    function resolve(bytes32 key, Outcome outcome, bool updateWinnerBalances, bool updateCreatorBalance) internal {
        ...
        //require(_msgSender() == predictionEvents[key].creator || _msgSender() == owner, 'Only Creator/Operator can resolve');
    ...
    function claim(bytes32 key, bool withdrawAmounts, address player) external {
        ...
        if (withdrawAmounts) withdrawForPlayer(player);
        emit Redeem(key, player, current);
    }
    function withdrawForPlayer(address player) public{
        transferBalance(player);
    }
...
contract TradableOddsRolling is TradableOdds{
    ...
    constructor() {
        ...
        minLiquidityForDuration[BetDuration.OneWeek] = 10e18;
