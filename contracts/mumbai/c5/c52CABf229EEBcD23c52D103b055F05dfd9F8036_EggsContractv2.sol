...
contract EggsContractv2 is ERC1155LazyMint {
        
    mapping(address => mapping(uint256 => uint256)) public referralCount;
    mapping(uint256 => uint256) public buyPrices;
    mapping(uint256 => uint256) public sellValues;

    event Minted(address indexed user, uint256 indexed tokenId, address indexed referral);
    event Evolved(address indexed user, uint256 indexed tokenId);
    event Sold(address indexed user, uint256 indexed tokenId);
    
    // Initializing prices and sell values in the constructor
    constructor(
        string memory _name,
        string memory _symbol
    ...
    function buyNFT(uint256 _tokenId, address _ref) public payable nonReentrant {
    // Check that the tokenId is valid
    require(_tokenId >= 3 && _tokenId <= 5, "Invalid tokenId");
    uint256 nftPrice = buyPrices[_tokenId];

    // Check that the user has enough MATIC
    require(msg.value >= nftPrice, "Insufficient MATIC sent to mint NFT");

    // Other checks and logic remain the same
    require(_ref != msg.sender, "You cannot refer yourself");

    // Check if the user has already minted the NFTs up to the requested _tokenId
    require(this.balanceOf(msg.sender, _tokenId) == 0, "You have already minted this NFT");

    // If a referral address exists, increase their referral count
    if (_ref != address(0)) {
        referralCount[_ref][_tokenId]++;
    }

    // Mint the NFTs up to the _tokenId
    for(uint256 i = 3; i <= _tokenId; i++) {
        if(this.balanceOf(msg.sender, i) == 0)
        {
            _transferTokensOnClaim(msg.sender, i, 1);
            emit Minted(msg.sender, this.totalSupply(i), _ref);
        }
    }
    }


    function evolve() public nonReentrant
    {
        require(this.balanceOf(msg.sender, 3) == 1, "You have not minted this NFT yet");
        _burn(msg.sender, 3, 1);
        _mint(msg.sender, 2, 1, "evolved");
    }

    function sellNft(uint256 _tokenId) public nonReentrant
    {
        uint256 value = sellValues[_tokenId];
    ...
    {
        uint256 ref = referralCount[msg.sender][_tokenId];
        return ref;
    }

    function withdraw(uint256 amount) external nonReentrant onlyOwner {
    // Transfer the USDT from this contract to the creator
