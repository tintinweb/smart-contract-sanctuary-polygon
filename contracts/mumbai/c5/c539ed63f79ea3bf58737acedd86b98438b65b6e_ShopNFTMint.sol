  ...
  BaseRelayRecipient
{
    ...
    mintCompliance(mintAmount)
  {
    for (uint256 i = 0; i < mintAmount; i++) {
      NFTDetails memory newNft = NFTDetails({
        id: nextTokenId,
        tokenURI: string(abi.encodePacked(metaDataURI,nextTokenId,".json")),
        creator: creator,
        owner: msg.sender,
      ...
      emit Mint(nextTokenId, string(abi.encodePacked(metaDataURI,nextTokenId,".json")), msg.sender, block.timestamp);
    }
  }
    ...
    mintCompliance(ids.length)
  {
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      NFTDetails memory newNft = NFTDetails({
        id: ids[i],
        tokenURI: string(abi.encodePacked(metaDataURI,ids[i],".json")),
        creator: creator,
        owner: msg.sender,
        timeOfGenerate: block.timestamp
      });
      nftLogs[ids[i]] = newNft;
      emit Mint(ids[i], string(abi.encodePacked(metaDataURI,ids[i],".json")), msg.sender, block.timestamp);
      nextTokenId++;
      nftsInCirculation++;
    }
  }
