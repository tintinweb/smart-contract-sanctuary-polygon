 ...
 * Any changes/appends in storage variable needs to happen in TrufinThetaVaultStorage.
...
contract TrufinThetaVault is TrufinVault, TrufinThetaVaultStorage {
    ...
    function initialize(
        ...
        require(_period % 7 == 0, "invalid period");
    ...
    function setWhiteList(address _newWhiteList) external onlyOwner {
        require(_newWhiteList != address(0), "Whitelist can't be zero address");
    ...
    function completeWithdraw() external nonReentrant {
        ...
        // Failing here with both lastQueuedWithdrawAmount, withdrawAmount having same value
        console.log(lastQueuedWithdrawAmount, withdrawAmount);
    ...
    function commitAndClose() external nonReentrant {
        ...
        optionState.nextOptionReadyAt = uint32(block.timestamp);
    ...
    function _startAuction() private {
        ...
        auctionDetails.whitelist = WHITELIST;
...
library VaultLifecycle {
        ...
        )
    {
...

        {
            ...
            uint256 newSupply = params.currentShareSupply + mintShares;
    ...
    function getOrDeployOtoken(
        ...
        verifyOtoken(otoken, vaultParams, collateralAsset, closeParams.USDC);
    ...
    ERC20Upgradeable
{
    ...
    function baseInitialize(
        ...
        require(_wmatic != address(0), "WMATIC address is not set");
        require(_usdc != address(0), "USDC address is not set");
        require(
            _gnosisEasyAuction != address(0),
            "gnosisEasyAuction address is not set"
        );
        require(_whitelist != address(0), "Whitelist address is not set");
        require(
            _gammaController != address(0),
            "gammaController address is not set"
        );
        require(_marginPool != address(0), "marginPool address is not set");
    ...
    function setNewKeeper(address newKeeper) external onlyOwner {
        require(
            newKeeper != address(0),
            "newKeeper should not be a zero address"
        );
    ...
    function setFeeRecipient(address newFeeRecipient) external onlyOwner {
        require(
            ...
            "newFeeRecipient should not be a zero address"
        );
    ...
    function setCap(uint256 newCap) external onlyOwner {
        require(newCap > 0, "Cap should be greater than zero");
...
.
    ...
    function depositMATIC() external payable nonReentrant {
        require(
            ...
            "Only vault asset can be deposited"
        );
        ...
        nonReentrant
    {
        ...
        require(creditor != address(0), "Creditor address must be set");
        require(
            IMasterWhitelist(WHITELIST).isUserWhitelisted(msg.sender),
            "User is not whitelisted"
        );
    ...
    function _depositFor(uint256 amount, address creditor) private {
        require(
            IMasterWhitelist(WHITELIST).isUserWhitelisted(creditor),
            "User is not whitelisted"
        );

        uint256 currentRound = vaultState.round;
        if (!(IMasterWhitelist(WHITELIST).isVaultWhitelisted(creditor))) {
        ...
        internal
    {
        ...
        if (withdrawalIsSameRound) {
            ...
            withdrawals[initiateFor].round = uint16(currentRound);
        }

        ShareMath.assertUint128(withdrawalShares);
        withdrawals[initiateFor].shares = uint128(withdrawalShares);
    ...
    function initiateWithdraw(uint256 numShares) external nonReentrant {
        require(numShares > 0, "numShares should be greater than zero");
        require(
            IMasterWhitelist(WHITELIST).isUserWhitelisted(msg.sender),
            "User is not whitelisted"
        );
        ...
        nonReentrant
    {
        require(numShares > 0, "numShares should be greater than zero");
        require(
            IMasterWhitelist(WHITELIST).isLawyer(msg.sender),
            "Lawyer is not whitelisted"
        );
    ...
    function _redeem(
        ...
        Vault.DepositReceipt memory depositReceipt = depositReceipts[redeemFor];
        ...
        if (depositReceipt.round < currentRound) {
            depositReceipts[redeemFor].amount = 0;
        }

        ShareMath.assertUint128(numShares);
        depositReceipts[redeemFor].unredeemedShares = uint128(
        ...
        )
    {
        ...
        require(newOption != address(0), "nextOption should exist");
