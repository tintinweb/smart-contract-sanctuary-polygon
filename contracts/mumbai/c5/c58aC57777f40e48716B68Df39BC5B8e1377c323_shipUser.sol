...
contract shipUser is AccessControl  {
    bytes32 public constant USER_D = keccak256("USER_D");
    bytes32 public constant Bonus = keccak256("Bonus");
    using SafeMath for uint256;
    using Address for address;
    mapping (address => address) public user;
    mapping (address => uint256) public userD;
    mapping (uint256 => uint256) public rate;
    IERC20 payToken = IERC20(0xe1234Ca851C0c055c689BAB3957baa876CCef3D4);
    event Register(address indexed _a, address indexed _a1, uint256 _t);

    constructor() {
        _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _grantRole(USER_D, msg.sender);
        _grantRole(Bonus, msg.sender);
        rate[1] = 2;
        rate[2] = 2;
        rate[3] = 2;
        rate[4] = 2;
    ...
    function shareOutBonus(address _a, uint256 _a1) public payable onlyRole(Bonus){
        if(msg.value > 0){
            for (uint i = 1; i <= 15; i++) {
                uint256 bonus;
    ...
    function shareOutBonusToken(address _a, uint256 _a1) public onlyRole(Bonus){
        for (uint i = 1; i <= 15; i++) {
            uint256 bonus;
            if(user[_a] == address(0)){
                break;
            ...
            if(bonus > 0){
                payToken.transfer(user[_a], bonus);
            }
            _a = user[_a];
        }
    }
    function register(address refer) external {
        ...
        require(!checkRegister(msg.sender), "registered");
        user[msg.sender] = refer;
        emit Register(msg.sender,refer, block.timestamp);
    }
    ...
    function setUserD(address[] memory _a, uint256[] memory _t) external onlyRole(USER_D){
        for (uint i = 0; i < _a.length; i++){
            userD[_a[i]] = _t[i];
        }
    }
    function setRate(uint256 _a, uint256 _t) external onlyRole(DEFAULT_ADMIN_ROLE){
        rate[_a] = _t;
    }
    function withdraw(IERC20 _a, uint256 num, address _to) onlyRole(DEFAULT_ADMIN_ROLE) external {
        _a.transfer(_to, num);
    }
    function withdrawETH(uint256 num, address _to) onlyRole(DEFAULT_ADMIN_ROLE) external {
