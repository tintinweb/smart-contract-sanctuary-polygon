	...
	ReentrancyGuardUpgradeable
{
	...
	modifier isPolicy() {
		require(
			AR.isPolicy(NR.poolId(), msg.sender),
			"AdminRouter: You aren't Bpolicy."
		);
		_;
	}
	...
	modifier checkAsset(address _asset) {
		require(
			AR.isSupportAssets(NR.poolId(), _asset),
	...
	function allReferral() public view returns (address[] memory) {
		require(
			_allReferral.length > 0,
			"Referral: Not have any Referral Active."
		);
		uint256 countPass;
		uint256 countAdded;
	...
	function fundInFlow(
		...
		if (_referrer != address(0)) {
			require(
				isReferral(_referrer) == true,
				"You Refferal are not Refferer"
			);
	...
	function fundOutFlow(
		...
		require(
			...
			"Balance of Contract Not Enough."
		);
	...
	function isReferral(address _user) public view returns (bool) {
		...
		return
			_isReferral[_user] &&
			(_StakingData.balance >= _dataType.minUserStake);
	}
	...
	function withdraw(
		...
		IERC20 _ERC20 = IERC20(_asset);
		require(
			PDS.isBlacklistUser(msg.sender) != true,
			"Policy: The caller is Blacklist."
		);
		require(
			PDS.isBlacklistAssetsUser(msg.sender, _asset) != true,
			"Policy: The Assets of caller is Blacklist."
		);
		require(
			PDS.isBlacklistUser(_to) != true,
			"Policy: This destination is Blacklist."
		);
		require(
			PDS.isBlacklistAssetsUser(_to, _asset) != true,
			"Policy: This Assets of destination is Blacklist."
		);
		require(
			isReferral(msg.sender) == true,
			"Referral : You are not Refferer"
		);
		require(
			_ERC20.balanceOf(address(this)) >= _value,
			"Balance of Contract Not Enough."
