...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<style>.uid{fill:#2D42FF;font-family:serif;font-size:14px;font-weight:bold}</style>',
            ...
            '<text x="50%" y="60%" class="attrib" dominant-baseline="middle" text-anchor="middle">', "speed: ",getAttributes(tokenId, 2),'bps</text>',
            '<text x="50%" y="70%" class="attrib" dominant-baseline="middle" text-anchor="middle">', "attack: ",getAttributes(tokenId, 3),'bps</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
    ...
    function train(uint256 tokenId) public{
        ...
        tokenIdToAttributes[tokenId].level = currentLevel - 10 + randomness(1000,1);
        tokenIdToAttributes[tokenId].health = currentHealth - 10 + randomness(1000,1);
        tokenIdToAttributes[tokenId].speed = currentSpeed - 10 + randomness(1000,1);
        tokenIdToAttributes[tokenId].attack = currentAttack - 10 + randomness(1000,1);
