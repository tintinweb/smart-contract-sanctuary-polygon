...
contract ProspectingV2 is OwnableUpgradeable, ReentrancyGuardUpgradeable, IProspector, IVRFBatchHandler {
    ...
    function _mintBiomod(uint _orderId) internal {
        ...
        if(s_randomResult[_orderId] == 0)
            revert Prospector__RandNumNotFulfilled();
    ...
    function handleBatch(uint orderId, uint randomNumber) override external onlyOrderAutomation{
        s_randomResult[orderId] = randomNumber;
    }
