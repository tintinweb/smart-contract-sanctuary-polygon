...
// File: contracts/CentralizedSwattrex.sol
...
contract SmartContractTemplate is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    string public baseTokenURI = 
        "ipfs://QmRtSMN9Zg73E6rsdmcKddHvDUfZherJF5KRy9sbSWcZjk/";
    ...
    constructor() payable ERC721("Bass so Heavy", "BASSSOHEAVY") {
    ...
    function setTotalRaise(uint256 totalRaise_) external onlyOwner {
        totalRaise = totalRaise_;
        unitRaise = totalRaise / maxSupply;
    }
    ...
    function burnBatch(uint256[] memory nftIds) external virtual {
        //solhint-disable-next-line max-line-length
        for (uint256 i = 0; i < nftIds.length; ++i) {
            require(_isApprovedOrOwner(_msgSender(), nftIds[i]), "ERC721Burnable: caller is not owner nor approved");
            _burn(nftIds[i]);
            if (bytes(_tokenURIs[nftIds[i]]).length != 0) {
                delete _tokenURIs[nftIds[i]];
            }
        }
    }
