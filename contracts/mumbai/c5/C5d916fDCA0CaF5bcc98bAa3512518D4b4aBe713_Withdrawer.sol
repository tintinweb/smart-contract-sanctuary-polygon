...
contract Withdrawer is ReentrancyGuard, Ownable {
    ...
    event MarketplaceUpdated(address indexed oldAddress, address indexed _marketplace);
    ...
    receive() external payable { _handleIncomingBid(msg.sender, msg.value, false);}
    fallback() external payable { _handleIncomingBid(msg.sender, msg.value, false);}
    ...
    function updateMarketplace(address _marketplace) external onlyOwner {
        require(_marketplace != address(0), "Address Zero");
        require(_marketplace != address(marketplace), "Invalid Address");
    ...
    function claimNFTback(address tokenOwner, address tokenContract, uint256 tokenId) external  onlyWhitelisted returns(bool success) {

        // address tokenOwner = tokenOwner;
        require(address(this) == IERC721(tokenContract).ownerOf(tokenId), "Invalid Owner");

        IERC721(tokenContract).transferFrom(address(this), tokenOwner, tokenId);

        require(tokenOwner == IERC721(tokenContract).ownerOf(tokenId), "Invalid Owner");

        success = true;
    }

    function updateOutgoingToken(address recepient, uint _amount, bool usdt) external onlyWhitelisted returns(bool success) {

        require(recepient != address(0), "Invalid Address");
        require(_amount > 0, "Zero Amount");

        if (usdt) {
            balance[recepient] = BalanceInfo({
                    matic: balance[recepient].matic,
                    usdt: (balance[recepient].usdt - _amount)
                });
        } else {
            balance[recepient] = BalanceInfo({
                    matic: (balance[recepient].matic - _amount),
                    usdt: balance[recepient].usdt
                });
        }
    ...
    function updateIncomingToken(address recepient, uint _amount, bool usdt) external onlyWhitelisted returns(bool success) {
    ...
    function transferCurrency(address recepient, uint256 _amount, bool usdt, bool outgoing) external payable onlyWhitelisted {
    ...
    /**
    * @notice owner and treasury can claim amount after trading
    */
     ...
     */
    function _handleIncomingBid(address recepient, uint256 amount, bool currency) internal returns(bool) {
