...
abstract contract ExchangeMonthlyLimits is BasicCompliance {
    ...
    /// monthly Limit for exchange deposits
    uint256 private _exchangeMonthlyLimit;

    /// Mapping for monthly deposit Counters
    mapping(address => ExchangeTransferCounter) private _depositCounters;

    /// Mapping for wallets tagged as exchange wallets
    mapping(address => bool) private _exchangeIDs;

    /**
     *  this event is emitted whenever the Exchanges deposit Limit has been updated.
     *  the event is emitted by 'setExchangeMonthlyLimit'
     *  `_newExchangeMonthlyLimit` is the Limit amount of tokens to be transferred monthly to an exchange wallet.
     */
    ...
    function setExchangeMonthlyLimit(uint256 _newExchangeMonthlyLimit) external onlyOwner {
        _exchangeMonthlyLimit = _newExchangeMonthlyLimit;
    ...
    */
    ...
    /**
    *  @dev getter for `_depositCounters` variable on the counter parameter of the ExchangeTransferCounter struct
    *  @param _investorID ONCHAINID to be checked
    *  returns current monthly counter of `_investorID` deposits
    */
    function getMonthlyCounter(address _investorID) public view returns (uint256) {
        return (_depositCounters[_investorID]).monthlyCount;
    }

    /**
    *  @dev getter for `_depositCounters` variable on the timer parameter of the ExchangeTransferCounter struct
    *  @param _investorID ONCHAINID to be checked
    *  returns current timer of `_investorID` deposits
    */
    function getMonthlyTimer(address _investorID) public view returns (uint256) {
        return (_depositCounters[_investorID]).monthlyTimer;
    }

    /**
    *  @dev getter for `exchangeMonthlyLimit` variable
    *  returns the monthly limit set for exchange deposits
    */
    function getExchangeMonthlyLimit() public view returns (uint256) {
        return _exchangeMonthlyLimit;
    }
    ...
    *  transfers from exchange wallet to exchange wallet are not subject to any limitation
    *  Agents can bypass the rule
    ...
    (bool) {
        ...
        if (!isTokenAgent(_from) && _from != address(0)) {
            if (isExchangeID(receiverIdentity)) {
                if(_value > _exchangeMonthlyLimit) {
                    return false;
                }
                if (!_isDepositMonthFinished(senderIdentity)
                && ((getMonthlyCounter(senderIdentity) + _value > _exchangeMonthlyLimit))) {
                    return false;
                }
            }
        }
    ...
    */
    function _transferActionOnExchangeMonthlyLimits(address _from, address _to, uint256 _value) internal {
        address senderIdentity = _getIdentity(_from);
        address receiverIdentity = _getIdentity(_to);
        if(isExchangeID(receiverIdentity) && !isTokenAgent(_from) && !isExchangeID(senderIdentity)) {
            _increaseExchangeCounters(senderIdentity, _value);
        }
    }
    ...
    *  internal function, can be called only from the functions of the Compliance smart contract
    */
    function _increaseExchangeCounters(address _investorID, uint256 _value) internal {
        _resetExchangeMonthlyCooldown(_investorID);

        if ((getMonthlyCounter(_investorID) + _value) <= _exchangeMonthlyLimit) {
            (_depositCounters[_investorID]).monthlyCount += _value;
        }
    }
    ...
    *  internal function, can be called only from the functions of the Compliance smart contract
    */
    function _resetExchangeMonthlyCooldown(address _investorID) internal {
        if (_isDepositMonthFinished(_investorID)) {
            (_depositCounters[_investorID]).monthlyTimer = block.timestamp + 30 days;
            (_depositCounters[_investorID]).monthlyCount = 0;
        }
    }
    ...
    *  internal function, can be called only from the functions of the Compliance smart contract
    */
    function _isDepositMonthFinished(address _investorID) internal view returns (bool) {
        return (getMonthlyTimer(_investorID) <= block.timestamp);
    }
}
...
// File: contracts/compliance/legacy/deployed/EGXCompliance.sol
//
//                                             :+#####%%%%%%%%%%%%%%+
//                                         .-*@@@%+.:+%@@@@@%%#***%@@%=
//                                     :=*%@@@#=.      :#@@%       *@@@%=
...
contract EGXCompliance is ExchangeMonthlyLimits, DayMonthLimits, CountryRestrictions {
    /**
