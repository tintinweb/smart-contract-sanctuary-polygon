...
contract ChallengeDetail is IERC721Receiver, IChallenge {
    ...
    */
    address payable private challenger;
    ...
    */
    ...
    string private nftUrl = "https://gateway.pinata.cloud/ipfs/QmfLffCBPvLdDuZoZchPp24pKaaJnCmNDLn6XBKqy584dD";
    ...
    */
    ...
    function transferToListReceiverSuccess() private {      
        ...
        for(uint256 j = 0; j < erc721Address.length; j++) {
            if(erc721Address[j] != address(0)) {
                if(allowGiveUp[2]) {
                ...
                for(uint256 i = 0; i < getIndexToken(erc721Address[j]); i++) {
                    if(getOwnerOfNft(erc721Address[j], i) == address(this)) {
                        if(!checkApprovedForAll(erc721Address[j], address(this), challenger)) {
    ...
    function transferNFTWhenFailed() private {
        for(uint256 j = 0; j < erc721Address.length; j++) {
            if(erc721Address[j] != address(0)) {
                for(uint256 i = 0; i < getIndexToken(erc721Address[j]); i++) {
                    address receiver = ExerciseSupplementNFT(erc721Address[j]).getHistoryNFT(i, address(this));

                    if(getOwnerOfNft(erc721Address[j], i) == address(this) && receiver != address(0)) {
                        if(!checkApprovedForAll(erc721Address[j], address(this), receiver)) {
                            TransferHelper.safeApproveForAll(
                                erc721Address[j],
                                receiver,
                                true
...
contract HistoryBalanceChallenges{
    ...
    function getHistoryTokenAndCoinSendToContract(address payable _challenges) public view returns(
        uint256, 
        uint256, 
        uint256[] memory, 
        uint256[] memory,
        ...
        bool isCoinChallenges = ChallengeDetail(_challenges).allowGiveUp(1);
        uint256 totalReward = ChallengeDetail(_challenges).totalReward();
        address[] memory erc20ListAddress = ChallengeDetail(_challenges).allContractERC20();
        ...
        uint256 contractBalance = ChallengeDetail(_challenges).getContractBalance();
        if(isCoinChallenges) {
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                tokenBalanceAfter[i] = ERC20(erc20ListAddress[i]).balanceOf(_challenges);
            ...
            address payable challengeAddress = _challenges;
            for(uint256 i = 0; i < erc20ListAddress.length; i++) {
                listTokenSymbol[i] = ERC20(erc20ListAddress[i]).symbol();
