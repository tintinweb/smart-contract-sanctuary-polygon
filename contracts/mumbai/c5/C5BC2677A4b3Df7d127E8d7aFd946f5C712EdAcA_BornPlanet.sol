...
contract BreedPlanetBase is ApeironPlanetGenerator, Context, Ownable {
    ...
    BreedPlanetData public immutable breedPlanetDataContract;
    ERC721Burnable public immutable miniBlackHole;
    ...
    uint256 normalBreedBaseInterval = 14 * 3600 * 24;
    uint256 bornBaseInterval = 7 * 3600 * 24;
    uint256 additionBornBaseInterval = 14 * 3600 * 24;
    ...
    constructor(
        ...
        breedPlanetDataContract = BreedPlanetData(_breedAddress);
        miniBlackHole = ERC721Burnable(_miniBlackHoleAddress);
        ...
        returns (uint256, uint256)
    {
        uint256 parentAId = 0;
        uint256 parentBId = 0;
        PlanetMeta.PlanetData memory planetData = _getPlanetData(planetId);
    ...
    function checkCanBreed(
        ...
        // planet data
        PlanetMeta.PlanetData memory planetAData = _getPlanetData(planetAId);
        PlanetMeta.PlanetData memory planetBData = _getPlanetData(planetBId);
        ...
        if (isPrimeval) {
            ...
            require(blackHoleTokenId != 0, "Primeval require blackHoleTokenId");
        }
        if (blackHoleTokenId != 0) {
            require(
                miniBlackHole.ownerOf(blackHoleTokenId) == _msgSender(),
                "black hole ticket is not owned"
            );
        }
    ...
    function _breed(
        ...
        uint256 planetBId,
        bool isPrimeval,
        uint256 blackHoleTokenId // if 0 = not using black hole token
    ) internal {
        _checkAndAddBreedCountMax(planetAId);
        _checkAndAddBreedCountMax(planetBId);

        if (checkCanBreed(planetAId, planetBId, isPrimeval, blackHoleTokenId)) {
            // APRS and ANIMA Fees is require
            // PlanetMeta.PlanetData memory planetAData = _getPlanetData(
            //     planetAId
            // );
            // PlanetMeta.PlanetData memory planetBData = _getPlanetData(
            //     planetBId
            // );
            uint256 aprsPrice;
            uint256 animaPrice;
            if (isPrimeval || blackHoleTokenId != 0) {
                aprsPrice = aprsPrices[0];
                animaPrice = 0;
            } else {
                aprsPrice = aprsPrices[1];
                // animaPrice =
                //     animaPrices[planetAData.breedCount] +
                //     animaPrices[planetBData.breedCount];
                animaPrice =
            ...
            // todo mini black hole, use transferFrom or burn,
            if (blackHoleTokenId != 0) {
                // miniBlackHole.transferFrom(_msgSender(), address(this), blackHoleTokenId);
                miniBlackHole.burn(blackHoleTokenId);
            }
            ...
            //     breedInterval =
            //         normalBreedBaseInterval +
            //         additionBreedBaseInterval;
            // } else {
            //     breedInterval = normalBreedBaseInterval;
            // }
            ...
            // planetNextBornMap[currentPlanetId] = block.timestamp + bornInterval;
    ...
    function _getPlanetData(uint256 planetId)
        internal
        view
        returns (PlanetMeta.PlanetData memory)
    {
...
contract BreedPlanetData is Initializable, AccessProtectedUpgradable {
    ...
    function updatePlanetNextBornMap(uint256 planetId, uint256 nextBornTime) external onlyAdmin {
        planetNextBornMap[planetId] = nextBornTime;
    }
    
    function getPlanetNextBornTime(uint256 planetId) external returns(uint256) {
