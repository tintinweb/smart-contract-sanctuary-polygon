...
// File: Decentralized Storage/Box.sol
...
contract Box is Ownable {
    ...
    mapping(address => uint8) public requestCountWithNoResponce;
    // mapping(address => uint) public getAddressByIndex;
    ...
    function setAllowedUser(address user) public 
    // onlyOwner
     {
    ...
    function removeAllowedUser(address user) public 
    // onlyOwner
     {
    ...
    function requestAccessForBox() public {
        ...
        // getAddressByIndex[msg.sender] = requestedList.length-1;
    }

    // Function to accept Request
    function requestAccept(address user
    // , uint256 index
    ) public
    //  onlyOwner
      {
    ...
    function requestAutoAccept(address user
    // , uint256 index
    )
        public
    ...
    function requestReject(address user
    // , uint256 index
    )
        public
        // callbyFactoryContract
...
// File: Decentralized Storage/Subscription.sol
...
contract Subscription is Ownable {
    ...
    constructor() {
        packages.push(subscriptionPlans(1 * 1 ether, 10 * 1024));
        packages.push(subscriptionPlans(2 * 1 ether, 25 * 1024));
        packages.push(subscriptionPlans(3 * 1 ether, 50 * 1024));
    }
    ...
    function CurrentPrice(uint256 package) public view returns (uint256) {
        uint256 a = getEURtoUSDPrice() * 1 ether;
        uint256 b = getUSDtoETHPrice() * 1 ether;

        if (packages[package].price == 1) {
    ...
    // Functions To Calculate the Price in Fiat
