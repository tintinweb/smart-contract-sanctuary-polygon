...
contract Account is Ownable {
     ...
     * @param _email string user's email
     * @param _logoUrl string user's logo url
    ...
    function createUser(
        ...
        UserData memory userData = UserData(_email, _logoUrl, _nickname);
        addressToUserData[_address] = userData;
        usersCount++;
        emit UserCreated(_address, _email, _logoUrl, _nickname);
    }
     ...
     * @param _logoUrl string user's logo url
     */
    // replace email with wallet address
    function modifyLogo(address _address, string calldata _logoUrl)
        external
        onlyOwner
    {
        require(!isAddressUnique(_address), "Account: user doesn't exist");
        addressToUserData[_address].logoUrl = _logoUrl;
    }

    /**
     * @dev Modifies users logo
     * @param _address address of user's wallet
     * @param _nickname string user's image url
     */
    function modifyNickname(address _address, string calldata _nickname)
        external
        onlyOwner
    {
        require(!isAddressUnique(_address), "Account: user doesn't exist");
        require(bytes(_nickname).length != 0, "Account: nickname is empty");
        addressToUserData[_address].nickname = _nickname;
    }
    ...
    /**
     * @dev Checks is email unique
     * @param _address address user's of user's wallet
     * @return bool true if unique
     */
    function isAddressUnique(address _address) internal view returns (bool) {
        return (bytes(addressToUserData[_address].email).length == 0);
    }
}
