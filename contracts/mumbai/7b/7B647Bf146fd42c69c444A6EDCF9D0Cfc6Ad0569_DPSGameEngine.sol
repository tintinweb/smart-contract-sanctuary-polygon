...
contract DPSGameEngine is DPSGameEngineI, Ownable {
    ...
    DPSCrewForCoinIOld public crewForCoinOld;
    ...
    function checkIfViableClaimer(
        ...
        DPSCrewForCoinIOld.Asset memory assetDPSOld = crewForCoinOld.isDPSInMarket(_lockedVoyage.dpsId);
        DPSCrewForCoinIOld.Asset memory assetFlagshipOld = crewForCoinOld.isFlagshipInMarket(_lockedVoyage.flagshipId);

        if (assetDPS.borrower == address(0) && assetFlagship.borrower == address(0)) {
            // if it s not in the market on the new DPSCrewForCoin
            if (assetDPSOld.borrower == address(0) && assetFlagshipOld.borrower == address(0)) {
                // and if it s not in the market on the DPSCrewForCoinOld than claimer should be owner of the voyage
                return _claimer == _ownerOfVoyage;
            }
        }
        // if it's on the market on the old DPSCrewForCoin
        if (assetDPSOld.borrower != address(0) || assetFlagshipOld.borrower != address(0)) {
            return
                (assetDPSOld.endTime <= block.timestamp && assetDPSOld.lender == _claimer) ||
                assetDPSOld.borrower == _claimer ||
                (assetFlagshipOld.endTime <= block.timestamp && assetFlagshipOld.lender == _claimer) ||
                assetFlagshipOld.borrower == _claimer;
        }

        // else if it is on the new market
        return
    ...
    function setContract(address _contract, uint8 _target) external onlyOwner {
        ...
        else if (_target == 12) crewForCoinOld = DPSCrewForCoinIOld(_contract);
