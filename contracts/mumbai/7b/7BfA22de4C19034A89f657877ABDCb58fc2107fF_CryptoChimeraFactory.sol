...
contract CryptoChimeraFactory is Ownable {
    /*
    ...
    event newCryptoChimeraAddressEvent(address cryptoChimeraContractAddress);
    ...
    struct CurrencyCoinType {
        ...
        /*
         * Supported types:
         * 0 - price in the native currency (taskTypes[].nativeCrPrice). Not supported in the zkSync2.
    ...
    */
    ...
    function setCryptoChimeraContract(address cryptoChimeraContractAddress) external onlyOwner {
        ...
        emit newCryptoChimeraAddressEvent(cryptoChimeraContractAddress);
    }
    ...
    function addTask(uint8 taskTypeId, address currencyAddress, uint256 arg2, uint256 arg3) external payable {
        ...
        if (currencyAddress == address(0)) {
            currencyTypePayed = 0;
            // TODO: payable value ... require( ... taskTypes[].nativeCrPrice ... )
            // taskType.nativeCrPrice > 0 !
        } else {
            CurrencyCoinType storage choosedCurrency = taskPricesForCurrencies[currencyAddress];
    ...
    function unblockMissedTasks() external payable {
        // not workes in zkSync2, need repair
...
interface CryptoChimeraInterface is IERC721 {
     ...
     */
    function toggleMintState() external;
    ...
    function addTokenToFactoryDelivery(address recipient, uint256 tokenId, uint256 price) external;


    /*
     * Functions for Owner only (onlyOwner):
     */
    ...
    function withdrawBalanceWeis(uint256 amount) external;
    function withdrawAllBalanceWeis() external;
    function setDefaultRoyalty(address receiver, uint96 feeNumerator) external;
    function deleteDefaultRoyalty() external;
    function viewTokenAtFactoryDelivery(address recipient) external view returns (uint256 tokenId, uint256 price);
    function receiveFromCCHFactory(address recipient) external payable;
}
