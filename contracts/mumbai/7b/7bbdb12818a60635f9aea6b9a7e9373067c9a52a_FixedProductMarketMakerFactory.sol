...
contract FixedProductMarketMaker is Initializable, ERC20Upgradeable, ERC1155ReceiverUpgradeable, IMarketMakerV1b {
...
contract FixedProductMarketMakerFactory is Errors, IMarketFactoryEvents {
    ...
    function createMarket(
        ...
        //
        // The fee is a value less than 10^18 = 1 ether and is internally
        // used as a percentage and not an absolute value.
        // E.g. fee = 0.02 ether -> buy amound * (0.02 ether / 1 ether) = 2% of buy amount
    ...
    function createAndFundMarkets(
        ...
        for (uint256 i = 0; i < marketParamsArray.length; ++i) {
            fpmms[i] = _createAndFundMarketFor(
                receiver,
                conditionalTokens,
                collateralToken,
                oracle,
    ...
    /// @param params unique parameters for a single market
    /// @return the created and funded market
    function _createAndFundMarketFor(
        ...
        FixedProductMarketMaker fpmm = createMarket(
            conditionalTokens,
            collateralToken,
            oracle,
            params.questionId,
            params.distributionHint.length,
            params.haltTime,
            fee
        );
        ...
        if (balances.hasNonzeroEntries()) {
            ...
            collateralToken.safeTransfer(receiver, addedFunds);
        } else {
            // otherwise fund it
            collateralToken.safeApprove(address(fpmm), addedFunds);
            fpmm.addFundingFor(receiver, addedFunds, params.distributionHint);
        }
