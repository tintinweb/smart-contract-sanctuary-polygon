...
contract ExchangeGodMiner is IERC721Receiver {
    struct Info {
        uint256 price;
        address owner;
    }
    ...
    event  SellNFT(address indexed seller, uint256 indexed tokenID, uint256 price);
    event  CancelDeal(address indexed seller, uint256 indexed tokenID);
    event  BuyNFT(address indexed user, address indexed seller, uint256 indexed tokenID, uint256 price);

    uint private unlocked = 1;
    modifier lock() {
        require(unlocked == 1, 'ExchangeGodMiner: LOCKED');
        unlocked = 0;
        _;
        unlocked = 1;
    }
    ...
    function sellNFT(uint256 _tokenID, uint256 _NFTPrice) public {
        require(_NFTPrice > 0, "ExchangeGodMiner: NFT price must be positive.");
    ...
    function cancelDeal(uint256 _tokenID) lock public {
        require(sellInfo[_tokenID].owner == msg.sender, "ExchangeGodMiner: no authority.");
    ...
    function buyNFT(uint256 _tokenID, uint256 _NFTPrice) lock public payable {
        require(sellInfo[_tokenID].price == msg.value, "ExchangeGodMiner: price error.");
        require(_NFTPrice == msg.value, "ExchangeGodMiner: verify price error.");
