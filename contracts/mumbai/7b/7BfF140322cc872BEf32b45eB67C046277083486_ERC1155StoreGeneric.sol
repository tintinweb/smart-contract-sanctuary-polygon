...
error SG__CombinedDiscountTooLarge(uint256 _saleId, uint256 _price, uint256 _bulkDsc, uint256 _ownershipDsc);
 ...
 */
contract ERC1155StoreGeneric is IERC1155StoreGeneric, ERC1155Receiver, OwnableUpgradeable, ReentrancyGuardUpgradeable, EIP712Upgradeable {
    /*------------------- STATE VARIABLES -------------------*/
    ...
    ISaleBonuses s_saleBonuses;
    IPaperKeyManager paperKeyManager;

    bool public publicSale;
    address private s_signer;
    bytes32 private constant BUY_MESSAGE = keccak256("BuyMessage(uint256 id,address sender,uint256 nonce)");
    ...
    function createSale(
        ...
        bool _profitState,
        bool _adminSupplied
    ) external onlyGov returns (uint256 _saleId) {
        ...
        if (_adminSupplied) {
            IERC1155Upgradeable(_token).safeTransferFrom(s_tokenDonor, address(this), _tokenId, _totalUnitSupply * _unitSize, "");
        }

        emit SaleCreated(_token, _tokenId, _unitSize, _totalUnitSupply, _unitPrice, _unitsPerUser, _defaultCurrency, _profitState, _adminSupplied);
    }
    ...
    function modifySale(
        ...
        uint256 totalTokensBefore = s_sales[_saleId].unitSize * (s_sales[_saleId].totalUnitSupply - s_sold[_saleId]);
        ...
        if (s_sales[_saleId].adminSupplied) {
            if (totalTokensAfter > totalTokensBefore) {
                IERC1155Upgradeable(s_sales[_saleId].token).safeTransferFrom(s_tokenDonor, address(this), s_sales[_saleId].tokenId, (totalTokensAfter - totalTokensBefore), "");
            }
            if (totalTokensAfter < totalTokensBefore) {
                IERC1155Upgradeable(s_sales[_saleId].token).safeTransferFrom(address(this), s_tokenDonor, s_sales[_saleId].tokenId, (totalTokensBefore - totalTokensAfter), "");
            }
        }

        emit SaleModified(_saleId, _unitSize, _totalUnitSupply, _unitPrice, _unitsPerUser, _defaultCurrency, _profitState);
    }
    ...
    function deleteSale(uint256 _saleId) external onlyGov {
        ...
        uint256 tokensRemaining = s_sales[_saleId].unitSize * (s_sales[_saleId].totalUnitSupply - s_sold[_saleId]);
        ...
        if (adminSupplied) {
            IERC1155Upgradeable(token).safeTransferFrom(address(this), s_tokenDonor, tokenId, tokensRemaining, "");
        }
    ...
    /// @inheritdoc IERC1155StoreGeneric
    function whitelistCurrencies(uint256 _saleId, address[] calldata _currencyAddresses) external onlyGov {
    ...
    function buyTokens(
        ...
        BuyTokenInputs memory saleInputs = BuyTokenInputs({buyer: buyer, tokenAddress: tokenAddressRebate, tokenId: tokenIdRebate, numPurchases: numPurchases, saleId: saleId});
    ...
    function buyTokensWithSignature(
        ...
        bytes32 structHash = keccak256(abi.encode(BUY_MESSAGE, saleInputs.saleId, msg.sender, nonce));
        ...
        if (_tokenAddressRebate != address(0)) {
            ...
            for (uint256 i; i < discounts.length; ++i) {
                if (
                    ...
                    (discounts[i].tokenType == TokenType.ERC721 || (discounts[i].tokenType == TokenType.ERC1155 && discounts[i].tokenId == _tokenIdRebate))
                ) {
            ...
            if (discountAmount > 0) {
                ...
                if (tokenType == 1) {
                    if (IERC1155Upgradeable(_tokenAddressRebate).balanceOf(_buyer, _tokenIdRebate) == 0) {
    ...
    function checkPrice(
        ...
        Sale memory info = s_sales[_saleId];
        uint256 salePrice = info.unitPrice * _numPurchases;
        uint256 discountPrice;
        if (_tokenAddress != address(0)) {
            OwnershipDiscount[] memory ownershipDiscounts = s_ownershipDiscounts[_saleId];
            uint256 discount;
            for (uint256 i; i < ownershipDiscounts.length; ++i) {
                if (ownershipDiscounts[i].tokenAddress == _tokenAddress && (ownershipDiscounts[i].tokenType == TokenType.ERC721 || _tokenId == ownershipDiscounts[i].tokenId)) {
                    discount = ownershipDiscounts[i].basisPoints;
                }
            }
            if (discount > 0) {
                discountPrice = _calculateDiscountedPrice(discount, salePrice);
            } else {
    ...
    function _buyTokens(
        ...
        if (s_whitelistedCurrencies[saleInputs.saleId][_currency] == false) revert SG__CurrencyNotWhitelisted(_currency);
        if (s_perPlayerSold[saleInputs.saleId][saleInputs.buyer] + saleInputs.numPurchases > s_sales[saleInputs.saleId].unitsPerUser) revert SG__PurchaseExceedsPlayerMax();
        if (s_sold[saleInputs.saleId] + saleInputs.numPurchases > s_sales[saleInputs.saleId].totalUnitSupply) revert SG__PurchaseExceedsTotalMax();
        ...
        if (saleInputs.tokenAddress != address(0)) balance = _applyOwnershipDiscount(saleInputs.saleId, balance, saleInputs.tokenAddress, saleInputs.tokenId, saleInputs.buyer);
        ...
        if (_optInBonuses) {
            s_saleBonuses.claimBonusReward(saleInputs.saleId, uint32(saleInputs.numPurchases), _optInCategories, saleInputs.buyer);
        }

        if (!sale.adminSupplied) {
            ITrustedMintable(sale.token).trustedMint(saleInputs.buyer, sale.tokenId, sale.unitSize * saleInputs.numPurchases);
        } else {
            IERC1155Upgradeable(sale.token).safeTransferFrom(address(this), saleInputs.buyer, sale.tokenId, sale.unitSize * saleInputs.numPurchases, "");
        }

        emit TokenBought(saleInputs.saleId, saleInputs.numPurchases, saleInputs.tokenId, saleInputs.tokenAddress, _currency, _optInBonuses, _optInCategories, saleInputs.buyer);
    }

    function _applyBulkDiscount(uint256 _saleId, uint256 _numPurchases) internal view returns (uint256 _finalPrice) {
    ...
    function _applyOwnershipDiscount(
        ...
        for (uint256 i; i < discounts.length; ++i) {
            if (discounts[i].tokenAddress == _tokenAddress && (discounts[i].tokenType == TokenType.ERC721 || (discounts[i].tokenType == TokenType.ERC1155 && discounts[i].tokenId == _tokenId))) {
    ...
    function _erc20Payment(
        ...
        if (!IERC20(_currency).transferFrom(_recipient, address(this), erc20Price)) revert SG__TransferFailed();
    ...
    function _calculateDiscountedPrice(uint256 _bps, uint256 _salePrice) internal pure returns (uint256) {
    ...
    /// @inheritdoc IERC1155StoreGeneric
    function setFeeWalletsAndPercentages(address[] calldata _walletAddresses, uint256[] calldata _feeBps) external onlyGov {
        uint256 sum;
    ...
    function getSwapManager() external view returns (address) {
        return address(s_swapManager);
    }

    function getSaleBonus() external view returns (address) {
        return address(s_saleBonuses);
    }

    function getBeneficiaries() external view returns (Beneficiaries memory) {
        return s_beneficiaries;
    }

    function getSaleInfo(uint256 _id) external view returns (Sale memory) {
        return s_sales[_id];
    }
    ...
    function onERC1155Received(
        ...
        require(from == s_tokenDonor, "GENERIC STORE: Not Token Donor");
