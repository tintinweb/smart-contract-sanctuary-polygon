...
contract WinnerBelt is ERC1155, Ownable, AccessControl, VRFConsumerBase, SignedAllowance {
    ...
    address public constant draftPool = address(0xc0F5b93Bb77271d9CE898d08013f4958d1478534);
    ...
    /// @notice Available supply of draftable NFTs
    uint256 public availableSupply = 0;
    /// @notice Timestamp for opening raffle 1
    uint256 public raffleStartTime;

    /// @notice Track tokenId offset for tranches
    /// @dev Each tranche shuffles entry index to a random offset int which must be added to the tranche offset
    ...
    function mint(address to, uint256 tokenId) public payable onlyRole(MINTER_ROLE) {
        ...
        availableSupply += 1;
        // require(nftCount > ) can we ensure we dont mint more passes than NFTs available?
        _mint(to, tokenId, 1, "");
    }
    ...
    function enterRaffle(uint256 tokenId) external payable {
    ...
    /// @notice Clear a raffle tranche based on number of entries.
    function clearRaffle() external {
        ...
        // Run Fisher-Yates shuffle for entry count.
        // An address who enters the weekly raffle will be randomly assigned
        // an index regardless of when they entered. That index acts as an
        // offset for minting NFTs per tranche.
        for (uint256 i = 0; i < entryCount; ) {
            // Generate a random index to swap from
            uint256 randomIndex = i + (entropy % (entryCount - i));

            // Collect the value at that random index - this randomly selected
            // address is the interim "winner" of tokenId i
            address randomTmp = raffleEntries[randomIndex];

            // Stick the value at the current index to the random index -
            // we can't discard that entrant!
            raffleEntries[randomIndex] = raffleEntries[i];

            // Swap the value from the random index to the current index (i)
            // such that address at randomTmp is the interim winner of
            // tokenId i.
    ...
    /// @param raffle the accounts to allow
    /// @return signables the messages to sign
    function buildMessages(uint256 raffle) external view returns (Signable[] memory signables) {
        ...
        for (uint256 index = 0; index < count; index++) {
            ...
            signables[index] = Signable({
                message: createMessage(account, index + offset),
                nonce: index + offset,
                minter: account
            });
        }
