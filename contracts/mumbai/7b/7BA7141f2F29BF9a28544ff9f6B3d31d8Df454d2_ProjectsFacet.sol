...
contract ProjectsFacet {
    ...
    //  ==========  project logic    ==========
    function createProject(string memory _name, string memory _imageUrl, string memory _description) external {
        ...
        require(bytes(_imageUrl).length != 0 && bytes(_imageUrl).length <= 64);
        ...
        s.userProjectExists[operator][projectId] = true;

        s.projectUsers[projectId].push(operator);
        s.projectUserExists[projectId][operator] = true;

        s.operatorAdmins[projectId][operator] = true;
    ...
    function updateProjectById(
        ...
        // Not authorized.
        require(LibProjects.getAdminByProjectId(_projectId, LibERC2771Context.msgSender()));

        ProjectStruct memory currentProject = LibProjects.getProjectById(_projectId);
    ...
    function getProjectUsersById(
        uint256 _projectId
    ) external view returns (ProjectStruct memory project, ProjectUserStruct[] memory projectUsers) {
    ...
    function getUsersByProjectId(uint256 _projectId) public view returns (ProjectUserStruct[] memory) {
        ...
        for (uint256 i = 0; i < count; i++) {
            ...
            ProjectUserStruct memory projectUser = ProjectUserStruct({
                ...
                isAdmin: LibProjects.getAdminByProjectId(_projectId, walletAddress)
            });
    ...
    function getAdminByProjectId(uint256 _projectId, address _walletAddress) external view returns (bool) {
        return LibProjects.getAdminByProjectId(_projectId, _walletAddress);
    }
    ...
    function updateAdminByProjectId(uint256 _projectId, address _walletAddress, bool _isAdmin) external {
        ...
        require(s.projectUserExists[_projectId][_walletAddress]);

        if (LibProjects.getAdminByProjectId(_projectId, _walletAddress) != _isAdmin) {
            s.operatorAdmins[_projectId][_walletAddress] = _isAdmin;
        }
    }
}
...
library LibStrings {
    ...
    function compareToIgnoreCase(string memory _a, string memory _b) internal pure returns (bool) {
        ...
        for (uint i = 0; i < _aBytes.length; i++) {
...
library Regex {
    // ^\w{1,25}$
    function validateWordComponent(string memory input) internal pure returns (bool) {
