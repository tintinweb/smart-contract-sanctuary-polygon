...
// File: contracts/libs/WkmDateTime.sol
...
// File: contracts/authentication/Ownable.sol
...
abstract contract Ownable {
    ...
    modifier onlyPartnerOrWakam(bytes32 partnershipId) {
        require(
            ...
            "Ownable: caller is not a partner or wakam"
        );
        _;
    }
...
// File: contracts/Product020802.sol
...
contract Product020802 is Ownable {
    ...
    function computeCoverageStartDate(uint256 requestedCoverageStartDate, uint256 subscriptionDate)
        ...
        returns (uint256)
    {
        TemporalityOfDays requestedCoverageStartDateComparedToSubscriptionDate = checkTheTemporalityOfDays(
            subscriptionDate,
            requestedCoverageStartDate
        );
        require(
            requestedCoverageStartDateComparedToSubscriptionDate != TemporalityOfDays.PAST,
    ...
    function checkTheTemporalityOfDays(uint256 referenceDate, uint256 dateToBeCompare)
        private
        pure
        returns (TemporalityOfDays)
    {
        ...
        if (dateToBeCompareAtMidnight < referenceDateAtMidnight) {
            return TemporalityOfDays.PAST;
        } else if (dateToBeCompareAtMidnight > referenceDateAtMidnight) {
