...
contract NftTrio is Initializable, VRFConsumerBaseV2Upgradeable, ERC721HolderUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function createGame(address nftContractAddress, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        ...
        require(verifyNftPower(nftPowerMerkleProof, nftId, nftPower, nftPowerMerkleRoots[nftContractAddress]), "Invalid nft power merkle proof");
    ...
    function joinGame(address nftContractAddress, uint256 gameId, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        ...
        requireFee(feesMerkleProof, nftContractAddress, requiredFeeInWei, joinFeesMerkleRoot);
        require(verifyNftPower(nftPowerMerkleProof, nftId, nftPower, nftPowerMerkleRoots[nftContractAddress]), "Invalid nft power proof");
    ...
    function leaveGame(address nftContractAddress, uint256 gameId) public {
        require(projectGames[nftContractAddress][gameId].status == GameStatus.WaitingForPlayers, "Game with given id doesn't exist or is already in progress");
        require(projectGames[nftContractAddress][gameId].player1.playerAddress == msg.sender || projectGames[nftContractAddress][gameId].player2.playerAddress == msg.sender, "You are not participating in this game");

        IERC721 nftContract = IERC721(nftContractAddress);
        projectGames[nftContractAddress][gameId].timestamp = block.timestamp;

        uint256 nftId = 0;
        if (projectGames[nftContractAddress][gameId].player1.playerAddress == msg.sender) {
            if (projectGames[nftContractAddress][gameId].player2.playerAddress == address(0)) {
                nftId = projectGames[nftContractAddress][gameId].player1.nftId;
            ...
            nftId = projectGames[nftContractAddress][gameId].player2.nftId;
            projectGames[nftContractAddress][gameId].player2 = Player(address(0), 0, 0);
        }
    ...
    function requireFee(bytes32[] calldata merkleProof, address nftContractAddress, uint256 requiredFeeInWei, bytes32 merkleRoot) private view {
        require(verifyRequiredFee(merkleProof, nftContractAddress, requiredFeeInWei, merkleRoot), "Invalid fees merkle proof");
