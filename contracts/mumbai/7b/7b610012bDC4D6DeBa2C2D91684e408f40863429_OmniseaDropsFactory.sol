...
contract OmniseaDropsFactory is ReentrancyGuard {
    ...
    constructor(address _lzEndpoint, uint256 _minGasToTransfer) {
        require(_minGasToTransfer > 0 && _lzEndpoint != address(0));
    ...
    function setMinGasToTransfer(uint256 _minGasToTransfer) external {
        ...
        require(_minGasToTransfer > 0);
        minGasToTransfer = _minGasToTransfer;
    }

    function setLzEndpoint(address _lzEndpoint) external {
        require(msg.sender == owner && _lzEndpoint != address(0));
        lzEndpoint = _lzEndpoint;
    }
    ...
    function create(CreateParams calldata _params) external nonReentrant {
        require(_params.endTime > block.timestamp && _params.endTime <= block.timestamp + 30 days);
        require(bytes(_params.tokensURI).length > 0 && _params.maxSupply > 0);

        IOmniseaDropsRepository(repository).create(_params, msg.sender, lzEndpoint, minGasToTransfer);
    }
}
