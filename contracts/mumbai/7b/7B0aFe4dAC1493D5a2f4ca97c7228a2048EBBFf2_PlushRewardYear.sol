    ...
    Date
{
    ...
    uint256 public rewardEveryMonth;
    ...
    function initialize(
        ...
        lifeSpan = _lifeSpan;
        rewardEveryMonth = _rewardEveryMonth;
    ...
    /**
     * @notice Check unlock account balance
     * @param lifeSpanId requesting account
     * @return uint256 unlock balance in wei
     */
    function getUnlockBalance(uint256 lifeSpanId)
    external
    view
    returns (uint256)
    {
        uint16 birthdayYear = getYear(lifeSpan.getBirthdayDate(lifeSpanId));
        uint8 birthdayMonth = getMonth(lifeSpan.getBirthdayDate(lifeSpanId));
        uint8 birthdayDays = getDay(lifeSpan.getBirthdayDate(lifeSpanId));

        uint16 mintYear = getYear(lifeSpan.getDateOfMint(lifeSpanId));
        uint8 mintMonth = getMonth(lifeSpan.getDateOfMint(lifeSpanId));
        uint8 mintDays = getDay(lifeSpan.getDateOfMint(lifeSpanId));

        uint16 dateYear = getYear(block.timestamp);
        uint8 dateMonth = getMonth(block.timestamp);
        uint8 dateDays = getDay(block.timestamp);

        uint16 nextBirthdayYear = dateYear;
        uint8 nextBirthdayMonth = birthdayMonth;
        uint8 nextBirthdayDays = birthdayDays;

        if(toTimestamp(dateYear, dateMonth, dateDays, 0, 0, 0) > toTimestamp(dateYear, birthdayMonth, birthdayDays, 0, 0, 0)){
            nextBirthdayYear += 1;
        }

        uint256 nextBirthdayTimestamp = toTimestamp(nextBirthdayYear, nextBirthdayMonth, nextBirthdayDays, 0, 0, 0);
        uint256 mintTimestamp = toTimestamp(mintYear, mintMonth, mintDays, 0, 0, 0);
        uint256 dateNowTimestamp = toTimestamp(dateYear, dateMonth, dateDays, 0, 0, 0);
    ...
    /**
     * @notice Check lock account balance
     * @param lifeSpanId requesting account
     * @return uint256 lock balance in wei
     */
    function getLockBalance(uint256 lifeSpanId)
    external
    view
    returns (uint256)
    {
        return lifeSpanAccounts[lifeSpanId].balance;
    }
