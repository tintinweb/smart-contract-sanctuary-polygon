...
import "../../utils/ExecutionUtils.sol";
import "../../callback/DefaultCallbackHandler.sol";
...
*/
abstract contract CoreAccount is BaseAccount, DefaultCallbackHandler, Initializable, UUPSUpgradeable, OwnableUpgradeable, IERC1271 {
    ...
    */
    function isValidSignature(bytes32 hash, bytes memory signature) external virtual view returns (bytes4 magicValue);
    ...
    */
    ...
    /**
     * @dev Execute a sequence of batched transactions with zero native token.
     */
    function executeBatch(address[] calldata dest, bytes[] calldata func) external {
        _requireFromEntryPointOrOwner();
        require(dest.length == func.length, "wrong array lengths");
        for (uint256 i = 0; i < dest.length; i++) {
            ExecutionUtils.callAndRevert(dest[i], 0, func[i]);
        }
    }
...
*/
contract ECDSAAccount is CoreAccount {
    ...
    function _validateSignature(UserOperation calldata userOp, bytes32 userOpHash) internal override view returns (uint256 validationData) {
        // ECDSA
...
contract ECDSAAccountFactory {
    ...
    function createAccount(address _owner, bytes32 _salt) public returns (ECDSAAccount account) {
        ...
        account = ECDSAAccount(payable(new ERC1967Proxy{salt : _salt}(
                address(accountImplementation),
                abi.encodeCall(ECDSAAccount.initialize, (_owner)))));
