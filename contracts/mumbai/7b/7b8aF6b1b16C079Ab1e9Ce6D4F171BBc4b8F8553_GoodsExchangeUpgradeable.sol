...
import { GoodsPaymentUpgradeable } from "./internal-upgradeable/GoodsPaymentUpgradeable.sol";
    ...
    GoodsPaymentUpgradeable
{
    ...
    function initialize(address roleManager_, uint256 paymentPriceUsd_) external initializer {
        __BaseUpgradeable_init_unchained(roleManager_);
        __ChainLinkPriceOracle_init_unchained();
        __GoodsPayment_init_unchained(paymentPriceUsd_);
    }
...
import { PrimarySaleUpgradeable } from "./PrimarySaleUpgradeable.sol";
import { ChainLinkPriceOracleUpgradeable } from "./ChainLinkPriceOracleUpgradeable.sol";
...
abstract contract GoodsPaymentUpgradeable is
    BaseUpgradeable,
    BusinessAddressesUpgradeable,
    PrimarySaleUpgradeable,
    ChainLinkPriceOracleUpgradeable
{
    using BitMaps for BitMaps.BitMap;

    uint256 public paymentPriceUsd;
    mapping(address => BitMaps.BitMap) private _isCollectionUsed;

    function __GoodsPayment_init(uint256 paymentPriceUsd_) internal onlyInitializing {
        __GoodsPayment_init_unchained(paymentPriceUsd_);
    }

    function __GoodsPayment_init_unchained(uint256 paymentPriceUsd_) internal onlyInitializing {
        paymentPriceUsd = paymentPriceUsd_;
    }

    function calculatePaymentAmounts(
        address paymentTokens_,
        address collection_,
        ...
        uint256 paymentAmount = _getTokenAmountUp(paymentTokens_, paymentPriceUsd * total);

        return paymentAmount;
    }

    function setPaymentPriceUsd(uint256 amount_) external onlyRole(OPERATOR_ROLE) {
        paymentPriceUsd = amount_;
    }

    function pay(address paymentToken_, address collection_, uint256[] calldata tokenIds_) external {
    ...
    function _processPayment(address paymentToken_, address collection_, uint256[] calldata tokenIds_) internal {
        ...
        uint256 paymentAmount = calculatePaymentAmounts(paymentToken_, collection_, tokenIds_);

        if (paymentToken_ == address(0)) {
            SafeTransferLib.safeTransferETH(_recipient, paymentAmount);
        } else {
            SafeTransferLib.safeTransferFrom(paymentToken_, payer, _recipient, paymentAmount);
        }
    }
}
