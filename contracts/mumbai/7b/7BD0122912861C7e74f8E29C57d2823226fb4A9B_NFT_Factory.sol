...
import "./CentaurifyBase721.sol";

contract NFT_Factory {
    event Deployed(address addr, address owner);

    function deploy1155(uint _salt) external {
        MintableERC1155 _contract = new MintableERC1155{salt: bytes32(_salt)}(msg.sender);
        emit Deployed(address(_contract), address(msg.sender));
    }

    function deploy721(string memory _name, string memory _symbol, uint _max, address _artist, uint _salt) external {
        CentaurifyBase721 base721 = new CentaurifyBase721{salt: bytes32(_salt)}(_name, _symbol, _max, _artist);
    ...
    function getByteCode1155(address owner) public pure returns (bytes memory) {
        bytes memory bytecode = type(MintableERC1155).creationCode;
...
contract MintableERC1155 is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply {
    ...
    constructor(address _owner) ERC1155("https://xoovbqyg7wwx.usemoralis.com/metadata/erc1155/collections/0/{id}/{id}.json") {
...
contract CentaurifyBase721 is MintableERC721 {
    ...
    constructor(string memory _name, string memory _symbol, uint256 _max, address _artist)
