...
contract MBView is Ownable {
    ...
    function getPool(uint256 _poolIndex) external view
    ...
    function getItemsSupplies(uint256[] calldata _refs) public view returns(uint256[] memory) {
        ...
        for(uint256 i = 0; i < _len; i++){
            if(mbStorage.itemsRegistry(_refs[i]).expired) _supplies[i] = 0;
            else _supplies[i] = mbStorage.getItemSupply(_refs[i]);
        }
        return _supplies;
    }
    ...
    function userLastOpenedBox(address _user, uint256 _pool)
        public
        view
        returns (uint256)
    {
    ...
    function setMBStorage(address _newMBStorAddress) public onlyOwner {
