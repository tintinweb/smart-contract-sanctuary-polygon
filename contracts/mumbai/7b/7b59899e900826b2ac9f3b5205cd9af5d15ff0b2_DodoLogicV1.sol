...
pragma solidity >=0.4.22 <0.9.0;
 ...
 contract DodoLogicV1 is Initializable {
  ...
  function totalSupply() public view returns (uint256) {
    return IERC20(tokenContract).totalSupply();
  }
  ...
  function transfer(address recipient, uint256 amount) public returns (bool) {
    return IERC20(tokenContract).transfer(recipient, amount);
  }

  function allowance(address _owner, address spender) public view returns (uint256) {
    return IERC20(tokenContract).allowance(_owner, spender);
  }

  function approve(address spender, uint256 amount) public returns (bool) {
    return IERC20(tokenContract).approve(spender, amount);
  }

  function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
    return IERC20(tokenContract).transferFrom(sender, recipient, amount);
  }
  ...
  // 获取赚钱奖励基数
  function getIncomeBase() public pure returns (uint256) {
    return 1 ether; // 赚钱奖励基数
  }

  // 获取基础精度
  function getBaseDecimals() public pure returns (uint256) {
    return 10000; // BonusDecimals
  }

  // 获取Income等级提升增加的收益率
  function getIncomeLevelRate() public pure returns (uint256) {
    return 1000; // IncomeLevelRate 表示每级增加10%
  }

  // 获取bonnus等级提升增加的收益率
  function getBonusLevelRate() public pure returns (uint256) {
  ...
  function getIncome() public view returns (uint256) {
    PlayerData memory playerData = DodoStorageInterface(dataContract).getPlayerData(msg.sender);
    uint256 baseDecimals = getBaseDecimals();
    uint256 incomeLevelRate = getIncomeLevelRate();
    return getIncomeBase() * (playerData.incomeLevel * baseDecimals + incomeLevelRate) / baseDecimals; // incomeLevel 每级增加10%
  }
  ...
  function makeMoney() public payable {
    // 必须支付手续费
    require(msg.value == getFee(), "MakeMoneyGameLogic: fee error");
