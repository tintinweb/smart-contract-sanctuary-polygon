 ...
 */
contract CarvEvents is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, ERC1155URIStorageUpgradeable, UUPSUpgradeable, AccessControlUpgradeable, EIP712Upgradeable {
    ...
    bytes32 public constant PERMIT_TYPEHASH = keccak256("Permit(address to,uint256 tokenId,uint256 amount,uint256 nonce)");
    // Mapping nonces used in permits.
    ...
    */
    ...
    function carvWithPermit(Permit calldata _req, bytes calldata _signature) external {
        ...
        require(_req.nonce > _permitNonces[signer], "Invalid signer nonce");
        _permitNonces[signer] = _req.nonce;
    ...
    function recoverPermitSigner(Permit calldata _req, bytes calldata _signature) internal view returns (address) {
        return _hashTypedDataV4(keccak256(_encodePermit(_req))).recover(_signature);
    }
    ...
    function initDomainSeparatorParameters(string memory name, string memory version) external onlyRole(DEFAULT_ADMIN_ROLE) {
