    ...
    OwnableUpgradeable
{
    ...
    function initialize(address maticAddress, uint256 minimumStake)
        external
        initializer
    {
        __Pausable_init();
        ...
        __Ownable_init();
        maticToken = IChildToken(maticAddress);
        _minimumStake = minimumStake;
    }

    function stake(bool isRebasing) external payable override nonReentrant {
        uint256 realAmount = msg.value - _commission;
        // check min amount
        require(
            realAmount >= _minimumStake,
            "Value must be greater than min amount"
        );
        // This takes the token that was withdrawn and calls withdraw on the "native" ERC20.
        maticToken.withdraw{value: realAmount}(realAmount);
        emit Staked(msg.sender, realAmount, isRebasing);
    }

    function withdraw(uint256 threshold) external onlyOperator {
        // check min amount
        require(
            address(this).balance >= threshold,
            "commission less then input threshold"
        );
        uint256 toWithdraw = _commission;
        _commission = 0;
    ...
    function changeCommission(uint256 commission) external onlyOwner {
        _commission = commission;
    }

    function currentCommission() external view returns (uint256) {
        return _commission;
    }
}
