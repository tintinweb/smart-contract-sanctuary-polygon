...
contract GoldpayToken is Initializable, UUPSUpgradeable, ERC20Upgradeable, OwnableUpgradeable {
    ...
    function increaseSupply(address _destination, uint256 _amount) public onlySupplyController {
        _mint(_destination, _amount);
        emit IncreaseSupply(_destination, _amount);
    }

    //TODO: check
    function decreaseSupply(uint256 _amount) public onlySupplyController {
        _burn(owner(), _amount);
    ...
    function setSupplyController(address _newSupplyController) public onlyOwner {
        require(_newSupplyController != address(0), "cannot set supplyController to address zero");
        address _oldSupplyController = supplyController;
        supplyController = _newSupplyController;
        emit SupplyControllerSet(_oldSupplyController, supplyController);
    }

    modifier onlySupplyController() {
        require(msg.sender == supplyController, "only SupplyController");
        _;
    }
    ...
    function getFee(address _sender, address _recipient, uint256 _value) private view returns (uint256) {
        if (_sender == owner() || _recipient == owner() || _sender == supplyController || _recipient == supplyController || _sender == feeRecipient || _recipient == feeRecipient) {
            return 0;
        }
        return getFeeFor(_value);
    }
    ...
    function getTotalFeeCollected() public view returns (uint256) {
