...
abstract contract MasterInternals is IMaster, MasterEvents {
    ...
    event BorrowPlusEffects(uint256 borrowPlusEffects);

    event HypotheticalLiquidityArgs(
        address account,
        address pTokenModify,
        uint256 redeemTokens,
        uint256 borrowAmount
    ...
    function _getHypotheticalAccountLiquidity(
        ...
        emit HypotheticalLiquidityArgs(
            account,
            pTokenModify,
            redeemTokens,
            borrowAmount
        ...
        emit BorrowPlusEffects(vars.sumBorrowPlusEffects);
        ...
        for (uint256 i; i < assets.length; i++) {
            ...
            vars.tokensToDenom = ((((vars.collateralFactor *
                vars.exchangeRate) / 1e8) * vars.oraclePrice) /
                multiplier);
            ...
            emit AdditionalVars(
                ...
                multiplier,
                vars.tokensToDenom,
                (vars.tokensToDenom * vars.pTokenBalance) / multiplier,
                i
            );
