...
contract OrderManager is IUniswapV3MintCallback, IOrderManager, OddzOwnable {
    ...
    mapping(bytes32 => OrderInfo) public currentOrderMap;

    mapping(address => uint256) public totalCollateralForOrders;
    ...
    */
        ...
        )
    {
        ...
        openOrder.collateralForOrder =openOrder.collateralForOrder.add(params.collateralForOrder);
        totalCollateralForOrders[params.trader] =totalCollateralForOrders[params.trader].add(params.collateralForOrder);
        ...
        return (response.base, response.quote, response.liquidity,orderId);
    }
    ...
    */
        ...
        )
    {
        ...
        if (openOrder.liquidity == 0) {
            totalCollateralForOrders[params.trader] =totalCollateralForOrders[params.trader].sub(openOrder.collateralForOrder);
            bytes32[] storage orderIds = _currentOrderIdsMap[params.trader][
                params.baseToken
            ];
        ...
        return (response.base, response.quote, takerBase, takerQuote);
    }
    ...
    /**
     * @notice Calculates token value inside the pool for a trader
     * @param _trader Address of the trader
     * @param _baseToken Base token Address
     * @param _base  true means calculate base token value and false means quote token value
     * @return _totalOrdersValue returns all orders value for a particular token
     */
    function getTotalOrdersAmountInPool(
        address _trader,
        address _baseToken,
        bool _base
    ) public view override returns (uint256 _totalOrdersValue) {
     ...
     function getTotalCollateralForOrders(address trader)external override view returns(uint256){
         return totalCollateralForOrders[trader];
     }

      function getCurrentOrderMap(bytes32 orderId) external override view returns(OrderInfo memory){
