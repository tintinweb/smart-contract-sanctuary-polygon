...
import {ITestFactory} from "../interfaces/ITestFactory.sol";
import {ITestBaseCollection} from "../interfaces/ITestBaseCollection.sol";

abstract contract TestBaseCollection is
    ERC2981,
    ITestBaseCollection,
    ReentrancyGuard
{
    ...
    // Aiskin factory address
    address internal _aiskinFactory;

    // Token items
    mapping(uint256 => Item) internal _items;

    modifier onlyFactory() {
        if (aiskinFactory() != msg.sender) {
            revert CallerNotAiskinFactory();
        }
        _;
    }

    modifier onlyExistingItem(uint256 tokenId) {
        if (_items[tokenId].tokenId == 0) {
            revert NonExistingTokenId();
        }
        _;
    }

    modifier onlyMintableItem(uint256 tokenId, uint256 amount) {
        Item memory item = _items[tokenId];

        if (tokenId == 0 || item.tokenId == 0) {
            revert MintNonExistingItem();
        }
        ...
        if (item.minted + amount > item.cap) {
    ...
    function createItem(
        ...
        uint256 cap,
        bool limited
    ) external payable override onlyFactory nonReentrant {
        _beforeCreateItem(creator, tokenId);

        if (tokenId == 0) {
            revert UnableToSetTokenIdWithZeroValue();
        }
    ...
    function mintItem(
        ...
        uint256 tokenId,
        uint256 amount
    ) external payable override onlyMintableItem(tokenId, amount) nonReentrant {
        Item memory item = _items[tokenId];

        uint256 totalPrice = amount * item.priceInWei;

        if (item.tokenPayment == address(0) && totalPrice != msg.value) {
            revert InvalidMintAmount();
        }

        _distributeMintAmountReceived(
            item.creator,
            item.tokenPayment,
            item.shareFee,
            totalPrice
    ...
    function mintItemOnPublish(
        ...
        uint256 tokenId,
        uint256 preMinted
    ) external override onlyFactory onlyMintableItem(tokenId, preMinted) {
        _mint(creator, tokenId, preMinted);
    }

    function getItemInfo(
        uint256 tokenId
    ) public view virtual override returns (Item memory) {
    ...
    function royaltyInfo(
        ...
        (address receiver, uint96 royaltyFraction) = ITestFactory(
            aiskinFactory()
        ).getRoyaltyInfo();

        uint256 royaltyAmount = (_salePrice * royaltyFraction) /
            _feeDenominator();
    ...
    function _mint(address to, uint256 tokenId, uint256 amount) internal {
        Item memory item = _items[tokenId];
        _items[tokenId].minted = item.minted + amount;
    ...
    function _enableLimited(
        uint256 tokenId
    ) internal onlyExistingItem(tokenId) {
    ...
    function _distributeMintAmountReceived(
        address creator,
        address tokenPayment,
        uint256 shareFee,
        uint256 amount
    ) private {
        address treasury = ITestFactory(aiskinFactory()).getTreasury();
        (uint256 fee, uint256 createSales) = _calculateFeesAndCreatorSales(
            shareFee,
            amount
        );
        if (tokenPayment != address(0)) {
            require(
                IERC20(tokenPayment).transferFrom(msg.sender, treasury, fee)
            );
            require(
            ...
            Address.sendValue(payable(creator), createSales);
        }
    }

    /**
     *  Calculate fees and creator sales
     */
    function _calculateFeesAndCreatorSales(
        uint256 shareFee,
        uint256 value
    ) internal pure returns (uint256, uint256) {
        uint256 fee = (value * shareFee) / 10_000;
        return (fee, value - fee);
    }

    /**
     * @dev Implementer can override this function to add logic or some validation before creating item on this collection. See {IBaseCollection-createItem}
     *
     * @param creator The item owner
     * @param tokenId The {IBaseCollection-Item} tokenId
     */
    function _beforeCreateItem(
        address creator,
        uint256 tokenId
    ) internal virtual;

    /**
     * @dev Implementer should override this function to implement the mint functionality
     *
...
import {ITestBaseCollection} from "./ITestBaseCollection.sol";

interface ITestMasterCollection is ITestBaseCollection {
...
import {ITestBaseCollection} from "./interfaces/ITestBaseCollection.sol";
import {ITestMasterCollection} from "./interfaces/ITestMasterCollection.sol";
import {ITestFactory} from "./interfaces/ITestFactory.sol";

import {TestBaseCollection} from "./base/TestBaseCollection.sol";

contract TestMasterCollection is
    TestBaseCollection,
    ERC1155,
    AccessControl,
    ITestMasterCollection
{
    ...
    /// @dev tokenId -> drop collection address
    mapping(uint256 => address) private _dropReference;

    // creator -> tokenIds
    mapping(address => uint256[]) private _creatorItems;
    ...
    modifier onlyCreator(uint256 tokenId) {
        if (getItemInfo(tokenId).creator != _msgSender()) {
            revert CallerNotCreator();
        }
        _;
    }

    modifier onlyDropCollection() {
        // revert when caller is not on drop collections (whitelist)
        if (!ITestFactory(_aiskinFactory).isValidCollection(_msgSender())) {
            revert Unauthorized(keccak256("DROP_COLLECTION"));
        }
        _;
    }

    constructor(address aiskinFactory_) ERC1155("") {
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        __BaseCollection_init(aiskinFactory_, "Test Master Collection", "Test");
    }
    ...
    function registerDropItem(
        ...
        if (
            getItemInfo(tokenId).tokenId != 0 ||
            _dropReference[tokenId] != address(0)
        ) {
            revert TokenIdAlreadyExist();
        }
        _dropReference[tokenId] = collection;

        emit CreateItem(tokenId, collection, cap, priceInWei, limited);
    }
    ...
    function emitTransferSingle(
        ...
        uint256 id,
        uint256 amount
    ) external override onlyDropCollection {
        emit Locked(id);
        emit TransferSingle(operator, from, to, id, amount);
    }

    function emitLocked(uint256 tokenId) external {
        emit Locked(tokenId);
    }
    ...
    function enableLimited(
        uint256 tokenId
    ) external override onlyCreator(tokenId) {
        _enableLimited(tokenId);
    }
    ...
    function getItemInfo(
        ...
        view
        virtual
        override(ITestBaseCollection, TestBaseCollection)
        returns (Item memory)
    {
        if (_dropReference[tokenId] != address(0)) {
            return
                ITestBaseCollection(_dropReference[tokenId]).getItemInfo(
                    tokenId
                );
        }
    ...
    function balanceOf(
        ...
        address _dropAddress = _dropReference[tokenId];
        if (_dropAddress != address(0)) {
            return ERC1155(_dropAddress).balanceOf(account, tokenId);
        }
    ...
    function supportsInterface(
        ...
        public
        view
        override(TestBaseCollection, ERC1155, AccessControl)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
    ...
    function _beforeCreateItem(
        ...
        if (_dropReference[tokenId] != address(0)) {
            revert TokenIdExistOnDropCollection();
        }
        _creatorItems[creator].push(tokenId);
    }
    ...
    function _beforeTokenTransfer(
        ...
        // revert when ids is not on master collection
        if (ids.length == 1 && _dropReference[ids[0]] != address(0)) {
            revert();
        } else {
            uint256 idsSize = ids.length;
            for (uint256 n = 0; n < idsSize; ++n) {
                require(_dropReference[ids[n]] == address(0));
            }
        }
    }

    /**
     * MintItem internal call
     */
