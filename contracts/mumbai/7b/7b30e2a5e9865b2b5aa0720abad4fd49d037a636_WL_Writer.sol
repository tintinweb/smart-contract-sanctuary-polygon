...
// File: contracts/KMGInterface.sol
...
contract WL_Writer is Ownable, TeamsAccess {
    
    using Strings for uint256;
    uint256 public preSaleCount = 0;
    uint256 private commListCount = 0;

    uint256 public whiteListCount = 0;
    mapping(address => uint256) private whiteLists;

    function setWL(address[] memory list, uint256 maxMint) public virtual onlyOwner {
        for (uint256 i = 0; i < list.length; i++) {
            preSaleCount = preSaleCount - whiteLists[list[i]];
            whiteLists[list[i]]++;
            preSaleCount = preSaleCount + maxMint;
        }
    }
