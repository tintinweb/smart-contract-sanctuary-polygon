...
contract PlastikStaking is Ownable, ReentrancyGuard {
    ...
    uint256 public bonusEndBlock; // The block number when CAKE mining ends.
    uint256 public startBlock; // The block number when CAKE mining starts.
    ...
    uint256 public rewardPerBlock; // CAKE tokens created per block.
    ...
    event WithdrawLongTerm(uint256 _pid, uint256 amount);
    ...
    event AddPool(uint256 id, uint256 _minDeposit, uint256 _maxDeposit, uint256 _limitStaking, uint64 _APR, uint128 _period);
    event ClosePool(uint256 _pid);
    ...
    function addPool(uint256 _minDeposit, uint256 _maxDeposit, uint256 _limitStaking, uint64 _APR, uint128 _period) external onlySystemAdmin {
        ...
        emit AddPool(_id, _minDeposit, _maxDeposit, _limitStaking, _APR, _period);
    }
    ...
    function poolInformation(uint256 _pid, address _account) external view returns (uint256, uint128, uint64) {
        return (stakeAmout(_pid, _account), poolInfo[_pid].period, poolInfo[_pid].APR);
    }

    function stakeAmout(uint256 _pid, address _account) public view returns(uint256) {
        return userInfoLongTerm[_pid][_account].amount;
    }

    function poolStatus(uint256 _pid) external view returns (bool) {
        return poolInfo[_pid].status;
    }
    ...
    function flexibleUserStake(address _account) external view returns(uint256) {
        return userInfo[_account].amount;
    }

    function timeUserStakeLongTerm(uint256 _pid, address _account) external view returns(uint256) {
        return userInfoLongTerm[_pid][_account].amount;
    }
    ...
    function earned(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if ((pool.status == false) && (uint128(block.timestamp) < user.timeStake.add(pool.period))) {
            endUserStakedTime = uint128(pool.stopDay.sub(user.timeStake));
            return ((((endUserStakedTime).mul(user.amount)).mul(pool.APR)).div(SECONDS_IN_YEAR)).div(10000);
        }
    ...
    function depositLongTerm(uint256 _pid, uint256 _amount) external nonReentrant {
        ...
        require(pool.currentStaking.add(_amount) <= pool.limitStaking, "Invalid Amount");
        require(user.staked == false, "You're Staking");
    ...
    function withdrawLongTerm(uint256 _pid) external nonReentrant {
        ...
        emit WithdrawLongTerm(_pid, user.amount);
    }
    ...
    function pendingReward(address _user) external view returns (uint256) {
        ...
        //uint256 stakedTokenSupply = plastik.balanceOf(address(this));
