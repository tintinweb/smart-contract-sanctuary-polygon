...
contract LooksRareExchange is ILooksRareExchange, ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;//using the erc20

    using OrderTypes for OrderTypes.MakerOrder;//maker order
    using OrderTypes for OrderTypes.TakerOrder;//taker order
    ...
    constructor(
        ...
        currencyManager = ICurrencyManager(_currencyManager);//Initializing the currency Manager
        executionManager = IExecutionManager(_executionManager);//Initializing the Execution Manager
        royaltyFeeManager = IRoyaltyFeeManager(_royaltyFeeManager);//Royality Fee Manager
        WETH = _WETH;
        protocolFeeRecipient = _protocolFeeRecipient;//the protocol also have some fee which will transfer to the recepient
    }
    ...
    function cancelAllOrdersForSender(uint256 minNonce) external {
        require(minNonce > userMinOrderNonce[msg.sender], "Cancel: Order nonce lower than current");//Checking this because we need to cancel all the previous nonces
        require(minNonce < userMinOrderNonce[msg.sender] + 500000, "Cancel: Cannot cancel more orders");//we cannot milt more than 50000
    ...
    function cancelMultipleMakerOrders(uint256[] calldata orderNonces) external {
        ...
        for (uint256 i = 0; i < orderNonces.length; i++) {
            ...
            _isUserOrderNonceExecutedOrCancelled[msg.sender][orderNonces[i]] = true;//all the order nonces can be cancelled 
        }
    ...
    function matchAskWithTakerBidUsingETHAndWETH(
        ...
        if (takerBid.price > msg.value) {
            IERC20(WETH).safeTransferFrom(msg.sender, address(this), (takerBid.price - msg.value));//take the bid amount from the taker
        } else {
        ...
        _validateOrder(makerAsk, askHash);//check if the hash matches
        
        // Retrieve execution parameters
        (bool isExecutionValid, uint256 tokenId, uint256 amount) = IExecutionStrategy(makerAsk.strategy)
            .canExecuteTakerBid(takerBid, makerAsk);//send if the excution is called

        require(isExecutionValid, "Strategy: Execution invalid");//the execution is valid

        // Update maker ask order status to true (prevents replay)
        _isUserOrderNonceExecutedOrCancelled[makerAsk.signer][makerAsk.nonce] = true;//maker ask order is true

        // Execution part 1/2
        _transferFeesAndFundsWithWETH(
            makerAsk.strategy,
            makerAsk.collection,
            tokenId,//which token id to transfer
            makerAsk.signer,//whom we need transfer money 
            takerBid.price,
    ...
    function matchBidWithTakerAsk(OrderTypes.TakerOrder calldata takerAsk, OrderTypes.MakerOrder calldata makerBid)
        ...
        nonReentrant
    {
        require((!makerBid.isOrderAsk) && (takerAsk.isOrderAsk), "Order: Wrong sides");//taker ask
        require(msg.sender == takerAsk.taker, "Order: Taker must be the sender");//taker must be the sender 

        // Check the maker bid order
        bytes32 bidHash = makerBid.hash();
        _validateOrder(makerBid, bidHash);//verify the bid hash

        (bool isExecutionValid, uint256 tokenId, uint256 amount) = IExecutionStrategy(makerBid.strategy)
            .canExecuteTakerAsk(takerAsk, makerBid);//check the startergy using for this

        require(isExecutionValid, "Strategy: Execution invalid");//the startergy needs to true

        // Update maker bid order status to true (prevents replay)
        _isUserOrderNonceExecutedOrCancelled[makerBid.signer][makerBid.nonce] = true;//order is executed

        // Execution part 1/2
        _transferNonFungibleToken(makerBid.collection, msg.sender, makerBid.signer, tokenId, amount);//transfer nft to the maker 
    ...
    function _transferFeesAndFundsWithWETH(
        ...
        // Initialize the final amount that is transferred to seller
        uint256 finalSellerAmount = amount;//amount that needs to be transfered to seller
        ...
        // 1. Protocol fee
        {
            ...
            if ((protocolFeeRecipient != address(0)) && (protocolFeeAmount != 0)) {
                ...
                finalSellerAmount -= protocolFeeAmount;
            }//transfer the protocol fee for the protocol receipient
        }

        // 2. Royalty fee
        {
            (address royaltyFeeRecipient, uint256 royaltyFeeAmount) = royaltyFeeManager
                .calculateRoyaltyFeeAndGetRecipient(collection, tokenId, amount);//get the royality fee info from the royality manager contract and transfer the royality amount to it 
        ...
        require((finalSellerAmount * 10000) >= (minPercentageToAsk * amount), "Fees: Higher than expected");//remaining amount should be greater than or equal to the amount deducted from the royality or protocol fee

        // 3. Transfer final amount (post-fees) to seller
        {
            IERC20(WETH).safeTransfer(to, finalSellerAmount);////transfer the final amount to seller
        }
    }
     ...
     * @param tokenId tokenId
     * @param amount amount of tokens (1 f
     or ERC721, 1+ for ERC1155)
    ...
    function _transferNonFungibleToken(
        ...
        address transferManager = transferSelectorNFT.checkTransferManagerForToken(collection);//transfer manager for this collection

        // If no transfer manager found, it returns address(0)
        require(transferManager != address(0), "Transfer: No NFT transfer manager available");

        // If one is found, transfer the token
        ITransferManagerNFT(transferManager).transferNonFungibleToken(collection, from, to, tokenId, amount);//transfer the Nft to TransferManager
    }
    ...
    {
        //parameters------->Maker ask and ask hash
        // Verify whether order nonce has expired
        require((!_isUserOrderNonceExecutedOrCancelled[makerOrder.signer][makerOrder.nonce]) && (makerOrder.nonce >= userMinOrderNonce[makerOrder.signer]),"Order: Matching order expired" );//order should not be executed or cancelled and min order nonce should be less than the makerorder nonce

        // Verify the signer is not address(0)
        require(makerOrder.signer != address(0), "Order: Invalid signer");//signer should not equal to address zero 

        // Verify the amount is not 0
        require(makerOrder.amount > 0, "Order: Amount cannot be 0");//maker order amount should be greater than zero 
        ...
        require(currencyManager.isCurrencyWhitelisted(makerOrder.currency), "Currency: Not whitelisted");//the currency needs to be whitelisted

        // Verify whether strategy can be executed
        require(executionManager.isStrategyWhitelisted(makerOrder.strategy), "Strategy: Not whitelisted");//the statergy also needs to be whitelisted
    }
}
