...
contract AnimeTokenStakingPool is Ownable, ReentrancyGuard {
    ...
    uint256 public MINIMUM_CLAIMABLE_TIME = 5 minutes;
    ...
    function claim() public nonReentrant updateReward(msg.sender) {
        ...
        // require(_totalSupply + rewards[msg.sender] <= (IERC20(stakingToken).balanceOf(address(this))), "AnimeBankStakingPool: Cannot claim the staking token");
    ...
    function recover(uint256 tokenAmount) external onlyOwner {
        //require(_totalSupply + tokenAmount <= IERC20(stakingToken).balanceOf(address(this)), "AnimeBankStakingPool: Cannot withdraw the staking token");
        IERC20(stakingToken).transfer(owner(), tokenAmount);
