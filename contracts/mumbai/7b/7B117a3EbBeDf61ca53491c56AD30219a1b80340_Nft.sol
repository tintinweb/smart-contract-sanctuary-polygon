...
contract Nft is Initializable, ERC721AUpgradeable, OwnableUpgradeable {
    IPaperKeyManager paperKeyManager;

    bool private whitelistable;

    uint8 public mintLimit;
    uint8 public PLATFORM_FEE;

    uint64 private dropTime;

    uint256 public maxSupply;
    uint256 public mintingAmount;
    uint256 public whitelistMintingAmount;

    bytes32 private rootHash;

    address public PLATFORM_WALLET;

    mapping(address => uint8) public limits;
    mapping(address => uint256) public royalties;

    //check if the max limit has been met
    modifier checkMaxSupply() {
        require(totalSupply() < maxSupply, "max supply reached");
        _;
    }

    // checks the user has exceded the limit
    modifier checkWalletLimit(uint256 quantity, address sender) {
        require(limits[sender] + quantity <= mintLimit, "wallet limit reached");
        _;
    }

    modifier checkDropTime() {
        require(dropTime <= block.timestamp, "drop time has not been reached");
        _;
    }

    modifier checkWhitelist(bytes32[] memory proof, address sender) {
        if (whitelistable) {
            require(
                MerkleProof.verify(
                    proof,
    ...
    modifier checkMintingAmount() {
        if (whitelistable) {
            require(
                msg.value >= whitelistMintingAmount,
                "send some eth to mint"
            );
            _;
        } else {
            require(msg.value >= mintingAmount, "send some eth to mint");
            _;
        }
    }
    ...
    modifier checkWithdraw() {
        require(
            _msgSender() == PLATFORM_WALLET || _msgSender() == owner(),
            "only platform or owner can withdraw"
        );
        _;
    }
    ...
    //basically a contructor for Upgradable contracts
    ...
    function setData(
        ...
        bytes32 _rootHash,
        uint64 _timestamp,
        uint256 _whitelistMintingAmount,
        ...
        rootHash = _rootHash;
        dropTime = _timestamp;
        whitelistMintingAmount = _whitelistMintingAmount;
    ...
    function mint(uint8 _quantity, bytes32[] memory _proof)
        external
        payable
        checkMintingAmount
        checkMaxSupply
        checkDropTime
        checkWalletLimit(_quantity, _msgSender())
        checkWhitelist(_proof, _msgSender())
    {
        calculateRoyalties();
        _mint(_msgSender(), _quantity);
        limits[_msgSender()] += _quantity;
    }

    function airdrop(address _to)
        external
        checkMaxSupply
        checkWalletLimit(1, _to)
    {
        _mint(_to, 1);
        limits[_to] += 1;
    }

    function calculateRoyalties() private {
        uint256 platformFee = (PLATFORM_FEE * msg.value) / 100;
        royalties[PLATFORM_WALLET] = platformFee;
        royalties[owner()] = msg.value - platformFee;
    }

    function withdraw() external checkWithdraw {
        require(royalties[_msgSender()] > 0, "no royalties to withdraw");
        uint256 _amount = royalties[_msgSender()];
        royalties[_msgSender()] = 0;
        payable(_msgSender()).transfer(_amount);
    }

    function mintWithPaper(
        MintingInfo memory _mintingInfo,
        bytes32 _nonce,
        bytes memory _signature
    )
        external
        ...
        checkMaxSupply
        checkDropTime
        checkWalletLimit(_mintingInfo.quantity, _mintingInfo.to)
        checkWhitelist(_mintingInfo.proof, _mintingInfo.to)
        onlyPaper(
            keccak256(
                abi.encode(
                    _mintingInfo.quantity,
                    _mintingInfo.to,
                    _mintingInfo.proof
                )
            ),
    ...
    {
        calculateRoyalties();
        _mint(_mintingInfo.to, _mintingInfo.quantity);
        limits[_mintingInfo.to] += _mintingInfo.quantity;
    }
}
