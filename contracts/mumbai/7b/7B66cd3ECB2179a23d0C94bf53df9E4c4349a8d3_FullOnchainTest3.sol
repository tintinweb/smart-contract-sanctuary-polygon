...
contract  FullOnchainTest3 is ERC721Enumerable, ERC721Permit("FullOnchainTest3NFT:Token"), ERC721Mintable, ReentrancyGuard, Ownable {

    string[] private letters ;
    string[] private fontColors ;
    string[] private backgroundColors ;

    constructor() ERC721("FullOnchainTest3NFT:Token", "FULL TEST") Ownable() {}


    function setInfo(string memory _letter, string memory _fontColor, string memory _backgroundColor) public onlyOwner {
        letters[500] = _letter;
        fontColors[500] = _fontColor;
        backgroundColors[500] = _backgroundColor;
    }

    function getLetter(uint256 _tokenId) public view returns (string memory) {
        return letters[_tokenId];
    }

    function getFontColor(uint256 _tokenId) public view returns (string memory) {
        return fontColors[_tokenId];
    }

    function getBackgroundColor(uint256 _tokenId) public view returns (string memory) {
        return backgroundColors[_tokenId];
    }
    ...
    function tokenURI(uint256 _tokenId) public override view returns (string memory) {
        ...
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Sample test #', toString(_tokenId), '", "description": "This is test.", "image": "data:image/svg+xml;base64,', Base64.encode(bytes(output)), '"}'))));
