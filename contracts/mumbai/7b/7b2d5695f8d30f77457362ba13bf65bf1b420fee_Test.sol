...
contract Test{
    ...
    mapping(uint8 => address) public lastBoardUser;
    mapping(uint8 => uint256) public levelPrice;

    uint256[]  REFERRAL_PERCENTS = [100,50];
    uint256  communityPercent=10;
    uint256 public lastUserId = 2;
    uint256  liquidityFee = 5; 
    uint256  INTEREST_CYCLE = 1 days; 
    ...
    event BoardReward(address  _user,  uint256 reward, uint8 level, uint8 sublevel, uint8 autoBuy, uint256 currentRate, uint8 isSkip);
    ...
    function updateBoard(address user, uint8 level) private {
        ...
        if(users[user].boardMatrix[level].reinvestCount<1) { // when user's board breaking first time
            ...
            if(users[newBoardUser].partnersCount>=2 || globalCount[level]==currentGlobalCount[level]) 
            {            
            ...
            {
                newBoardUser=globalIndex[level][currentGlobalCount[level]+1];
                    //===== for first level of reinvested user
                users[user].boardMatrix[level].firstReferrals.push(allBoardUser[0]);
                users[allBoardUser[0]].boardMatrix[level].boardReferrer=user;
                users[user].boardMatrix[level].firstReferrals.push(allBoardUser[1]);
                users[allBoardUser[1]].boardMatrix[level].boardReferrer=user;

                //===== for first level of new user
                users[newBoardUser].boardMatrix[level].secondReferrals.push(allBoardUser[2]);
                users[allBoardUser[2]].boardMatrix[level].boardReferrer=newBoardUser;
                users[newBoardUser].boardMatrix[level].secondReferrals.push(allBoardUser[3]);
                users[allBoardUser[3]].boardMatrix[level].boardReferrer=newBoardUser;

                //===== for second level of reinvested user
                users[user].boardMatrix[level].secondReferrals.push(allBoardUser[4]);
                users[allBoardUser[4]].boardMatrix[level].boardReferrer=user;
                users[user].boardMatrix[level].secondReferrals.push(allBoardUser[5]);
                users[allBoardUser[5]].boardMatrix[level].boardReferrer=user;
            }
            //===== increasing reinvest
            ...
            emit BoardReward(user, reward, level, 1, 0, maticRate, isSkip);
        }
            ...
            if(level==10 || users[user].levelActive[level+1])
            {
                ...
                emit BoardReward(user, reward*2, level, 2, 0, maticRate, isSkip);
            }
            else
            {
                uint256 reward=(levelPrice[level].mul(1e18)).div(maticRate);
                autoUpgrade(user,(level+1));
                emit BoardReward(user, reward*2, level, 2, 1, maticRate, 1);
            }
    ...
    //======================== Admin funtionality start ==============================

    function adminRegistration(address userAddress, address referrerAddress, uint256 amount) public 
    {
        ...
        for(uint8 i=0; i<2; i++)
        {
            uint256 reward=(amount.mul(REFERRAL_PERCENTS[i])).div(1000);
            address(uint160(upline)).transfer(reward);
            ...
            if(upline==address(0))
            {
                devAddress.transfer((amount.mul(leftPer)).div(1000));
                break;
            }
        }
        ...
        for(uint8 j=1; j<=10; j++)
        {
            uint256 reward=(amount.mul(communityPercent)).div(1000);
            address(uint160(idToAddress[globalId])).transfer(reward);
            emit CommunityReward(idToAddress[globalId], userAddress, reward, 1, j, maticRate);        
            globalId--;
            leftCom=leftCom-communityPercent;
            if(globalId==0)
            {
                devAddress.transfer((amount.mul(leftCom)).div(1000));
                break;
            }           
        }
        lastUserId++;
        total_liquidity+=(amount.mul(liquidityFee)).div(100);
        
        emit Registration(userAddress, referrerAddress, users[userAddress].id, users[referrerAddress].id);
    }

    function adminBuyLevel(address userAddress, uint8 level, uint256 amount) public
    {
        require(userAddress==owner,"Only Owner!");
        require(isUserExists(userAddress), "user not exists!");
        require(level<11, "Max 10 Level!");  
        require(users[userAddress].levelActive[level-1],"Buy Previous Level First!");
        require(!users[userAddress].levelActive[level],"Level Already Activated!");
        
        users[userAddress].levelActive[level]=true;
        total_liquidity+=(levelPrice[level].mul(liquidityFee)).div(100);
        uint256 mintedToken=getMintedToken(userAddress);
        users[userAddress].genReward+=mintedToken;
        users[userAddress].currentLevel=level;
        users[userAddress].lastTokenWithdraw=block.timestamp;

        communityGlobalCount[level]+=1;
        communityGlobalIndex[level][communityGlobalCount[level]]=userAddress;
    
        address boardRef=globalIndex[level][currentGlobalCount[level]];

        if(users[boardRef].boardMatrix[level].firstReferrals.length<2) {
            users[userAddress].boardMatrix[level].boardReferrer=boardRef;            
            users[boardRef].boardMatrix[level].firstReferrals.push(userAddress); 
        }
                
        else {
           users[boardRef].boardMatrix[level].secondReferrals.push(userAddress);  
           users[userAddress].boardMatrix[level].boardReferrer=boardRef;
           if(users[boardRef].boardMatrix[level].secondReferrals.length==4) {
            updateBoard(boardRef,level);
           }
        ...
        for(uint8 i=0; i<2;)
        {
            if(users[upline].levelActive[level])
            {
                uint256 reward=(amount.mul(REFERRAL_PERCENTS[i])).div(1000);
                address(uint160(upline)).transfer(reward);
                emit ReferralReward(upline, userAddress, reward, level, i+1, maticRate);            
                leftPer=leftPer-REFERRAL_PERCENTS[i];
                i++;
            }
            upline=users[upline].referrer;
        ...
        while(j<11)
        {
            if(users[communityGlobalIndex[level][globalId]].levelActive[level])
            {
                uint256 reward=(amount.mul(communityPercent)).div(1000);   
                address(uint160(communityGlobalIndex[level][globalId])).transfer(reward);            
                emit CommunityReward(communityGlobalIndex[level][globalId], userAddress, reward, level, j, maticRate);
                leftCom=leftCom-communityPercent;
                j++;         
            }
            globalId--;
        ...
        address refer= communityGlobalIndex[level][communityGlobalCount[level]-1];
        emit BuyNewLevel(userAddress, users[userAddress].id, level, refer, users[refer].id);
    }  

    function adminAutoUpgrade(address user, uint8 level) private
    {
        users[user].levelActive[level]=true;
        total_liquidity+=(levelPrice[level].mul(liquidityFee)).div(100);
        uint256 mintedToken=getMintedToken(user);
