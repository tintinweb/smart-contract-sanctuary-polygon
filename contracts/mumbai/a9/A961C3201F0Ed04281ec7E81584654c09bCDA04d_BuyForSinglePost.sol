...
contract BuyForSinglePost is IExecutePlugin, BasePlugin {

    constructor(address _registry) {
        PLUGIN_VERSION = 1;
        PLUGIN_NAME = PluginsList.SUBSCRIPTION_BUY_FOR_SINGLE_POST;
    ...
    function execute(
        ...
        (address _user, uint256 _postId, uint256 _amount, uint256 _beforeStartDaysNumber) =
        abi.decode(_data,(address, uint256, uint256, uint256));
        uint256 startTime = block.timestamp + 1 days * _beforeStartDaysNumber;

        DataTypes.MinSimpleVars memory readVars;
        readVars.pluginName = PLUGIN_NAME;
        readVars.version = PLUGIN_VERSION;
        readVars.data = abi.encode(_postId);
        DataTypes.SinglePostInfo memory postInfo = ISinglePostData(registry.singlePostData()).readPost(readVars);

        uint256 paymentType = postInfo.paymentType;
        checkTrueType(DataTypes.PaymentType(paymentType));

        address postOwner = INFT(registry.nft()).ownerOf(_postId);

        require(postInfo.payAmount > 0, "Buy: wrong price for subscription");
        (uint256 daysCount, uint256 tokenAmount) = calcDays(DataTypes.PaymentType(paymentType), postInfo.payAmount, _amount);

        uint256 endTime = startTime + 1 days * daysCount;
        IToken pageToken = IToken(registry.token());
        require(pageToken.transferFrom(_sender, postOwner, tokenAmount), "Buy: wrong transfer");

        DataTypes.SimpleVars memory vars;
        vars.executedId = _executedId;
        vars.pluginName = PLUGIN_NAME;
        vars.version = PLUGIN_VERSION;
        vars.data = abi.encode(postInfo.creator, _user, paymentType, startTime, endTime);
        require(ISubscription(registry.subscription()).mint(vars) > 0, "Buy: wrong mint subscription");

        return true;
    }

    function checkTrueType(DataTypes.PaymentType _type) private pure {
        bool isTruePaymentType =
        ...
        require(isTruePaymentType, "Buy: wrong payment type");
    }

    function calcDays(
        DataTypes.PaymentType _type,
        uint256 _requiredAmount,
        uint256 _tokenAmount
    ) private pure returns(uint256 daysCount, uint256 necessaryAmount) {
        if (_type == DataTypes.PaymentType.FOR_SINGLE_POST_ONE_TIME) {
            require(_tokenAmount >= _requiredAmount, "Buy: wrong payment amount");
            necessaryAmount = _requiredAmount;
            daysCount = 100_000;
        }
        if (_type == DataTypes.PaymentType.FOR_SINGLE_POST_DAYS) {
            daysCount = _tokenAmount / _requiredAmount;
            require(daysCount > 0, "Buy: wrong payment amount");
            necessaryAmount = daysCount * _requiredAmount;
        }
        if (_type == DataTypes.PaymentType.FOR_SINGLE_POST_WEEKS) {
            uint256 weeksCount = _tokenAmount / _requiredAmount;
            require(weeksCount > 0, "Buy: wrong payment amount");
            daysCount = weeksCount * 7;
            necessaryAmount = weeksCount * _requiredAmount;
        }
        if (_type == DataTypes.PaymentType.FOR_SINGLE_POST_MONTHS) {
            uint256 monthsCount = _tokenAmount / _requiredAmount;
            require(monthsCount > 0, "Buy: wrong payment amount");
            daysCount = monthsCount * 30;
            necessaryAmount = monthsCount * _requiredAmount;
        }
    }
}
