...
contract NFT is ERC721URIStorage, EIP712, AccessControl {
    ...
    event deployContract(string _status);
    event checkRole(string _role);
    ...
    // constructor
    // constructor(string memory _name, string memory _symbol, address _minterAddress, address _validatorAddress)
    constructor()
        ERC721("KwikTrust", "KTX")
        EIP712(SIGNING_DOMAIN, SIGNATURE_VERSION)
    {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        grantRole(MINTER_ROLE, msg.sender);
        emit deployContract("Roles Granted");
    ...
    function mint(string memory _tokenURI, address marketplace) external {
        require(hasRole(MINTER_ROLE, msg.sender), "0x0m1");
        tokenId++;
    ...
    function bulkMint(string[] memory _tokenURI, address marketplace) external {
        require(_tokenURI.length > 150, "0x0m2");
        require(hasRole(MINTER_ROLE, msg.sender), "0x0m3");
        require(_tokenURI.length <= 150, "0x0m4");
    ...
    function lazyMint(
        ...
        require(hasRole(VALIDATOR_ROLE, verifyValidator), "0x0m5");
        require(msg.value >= voucher.price, "0X0m6");
    ...
    function hasRoles(bytes32 _role, address _account) public {
        hasRole(_role, _account);
        emit checkRole("Role Checked");
    }
}
