...
/// @dev fake WETH contract, testing purposes only
/// @notice this is a fake token, worthless, just for testing purposes.
contract FakeWrappedEther is ERC20, Ownable {
    /// @dev Amount of ether that can be claimed
    uint256 private claimLimit = 25 ether;

    /// @dev Already Claimed tokens
    error claimLimitReached(address);

    /**
     * @dev Fired in claimToken()
     *
     * @param _to receiver of the tokens
     * @param _amount received amount
     */
    event claimedTokens(address indexed _to, uint256 _amount);
    
    /**
     * @dev Fired in mint()
     *
     * @param _by the address that execute the function
     * @param _amount amount minted
     */
    event mintIncreased(address indexed _by, uint256 _amount);

    /// @dev to manage claimed amount so far
    mapping(address => uint256) private claims;

    constructor() ERC20("FakeWrappedEther", "fwETH") {
        // mint 100 ether to faucet
        _mint(address(this), 100 ether);
        // mint 10 ether to dev
        _mint(_msgSender(), 25 ether);
    }

    /// @dev only owner can mint more tokens for faucet
    function mint(uint256 _amount) public onlyOwner {
        uint256 _oldTotalSupply = totalSupply();
        _mint(address(this), _amount);
        emit mintIncreased(_msgSender(), _amount);
        // to validate that totalSupply is updated properly
        assert(totalSupply() == _oldTotalSupply + _amount);
    }

    /// @dev claim tokens by minting to _msgSender()
    function claimToken() public {
        if (claims[_msgSender()] > claimLimit)
            revert claimLimitReached(_msgSender());
        // update mapping
        claims[_msgSender()] += 1 ether;
        // transfer tokens, sender, amount
        _transfer(address(this), _msgSender(), 1 ether);
        emit claimedTokens(_msgSender(), 1 ether);
        // validate that _msgSender() receive their tokens
        assert(
 ...
 * @title PaticasSale
 *
 * @notice PaticasSale sales fixed amount of NFTs (tokens) for a fixed price;
 *
 *
 * @dev When buying a token from this smart contract, a token is minted to the recipient
 *
 ...
 *
 */
contract PawsSale is Ownable {
    /**
     * @dev Price for sales, "0.05" wETH in wei
     */
    uint256 public price = 50000000000000000;

    /**
     * @dev Last token ID to mint;
     * @dev once `totalSupply` exceeds `finalId` the sale pauses
     */
     ...
     */
    IMintableERC721 public immutable tokenContract;

    /**
     * @dev Address to transfer all sales payments
     */
    address public immutable paymentsAddress;

    /**
     * @dev WETH interface token contract
     */
    ...
    /// @dev contract is not set
    error AddressNotSet(address);
    /// @dev unexpected token contract type
    error UnexpectedTokenContractType(address);
    /// @dev incorrect batch amount
    error IncorrectBatchAmount(uint32);
    /// @dev mint limit reached
    error MintLimitReached(address, uint32);
    /// @dev not enough funds
    error NotEnoughFunds(address);
    /// @dev not enough funds from WETH allowance
    error NotEnoughAllowanceFunds(uint256);
    /// @dev not enough funds from WETH allowance
    error NotEnoughItemsAvailable(uint32);
    /// @dev WETH TransferFrom failed
    error TransferFromFailed();

    /// @dev calculate if 'account' have enough WETH for required '_amount'
    modifier cost(address account, uint256 _amount) {
        uint256 balance = wethToken.balanceOf(account);
        if (balance < _amount) revert AmountTooLow(balance);
        _;
    }

    /// @dev verify address can't be zero
    modifier notZeroAddress(address account) {
        if (account == address(0)) revert AddressNotSet(account);
        _;
    }

    /// @dev verify if account have not reached the mint limit
    modifier mintLimitAmount(address _account, uint32 _amount) {
        if (mintLimit != 0) {
            if (mints[_account] + _amount > mintLimit)
                revert MintLimitReached(_account, soldCounter);
        }

        _;
    }

    /// @dev verify if there are enough items available for requested amount
    modifier itemIsAvailable(uint32 _amount) {
        if (itemsAvailable() < _amount) revert NotEnoughItemsAvailable(itemsAvailable());
        _;
    }
     ...
     * @param _paymentsAddress all sales payments will be transfered to this address
     */
    constructor(
        ...
        if (_tokenContract == address(0)) revert AddressNotSet(_tokenContract);
        if (_paymentsAddress == address(0))
            revert AddressNotSet(_paymentsAddress);
        if (_wethToken == address(0)) revert AddressNotSet(_wethToken);
        ...
        if (
            ...
            !IERC721(_tokenContract).supportsInterface(
                type(IMintableERC721).interfaceId
            )
        ) revert UnexpectedTokenContractType(_tokenContract);
     ...
     *      1. `finalId` is not reached (`_totalSupply <= finalId`)
     *
     ...
     *      )
     *
     * @dev Requires next ID to be greater than zero (strict): `__totalSupply > 0`
     *
    ...
    function buyTo(
        ...
        public
        notZeroAddress(_to)
        cost(_msgSender(), (price * _amount))
        mintLimitAmount(_msgSender(), _amount)
        itemIsAvailable(_amount)
    {
        // verify inputs
        uint256 _approvedAmount = wethToken.allowance(
            _msgSender(),
            address(this)
        );
        if (_approvedAmount < _amount)
            revert NotEnoughAllowanceFunds(_approvedAmount);
        if (_amount < 1 || (batchLimit == 0 || _amount > batchLimit))
            revert IncorrectBatchAmount(_amount);
        // calculate the total price required
        uint256 totalPrice = price * _amount;
        // send totalPrice to payment Address
        if(! wethToken.transferFrom(_msgSender(), paymentsAddress, totalPrice) ) revert TransferFromFailed();

        // mint token to the recipient
        tokenContract.mintBatch(_to, _tokenId, _amount);
        ...
        // emit en event
        emit Bought(_msgSender(), _to, _amount, totalPrice);
    }
     ...
     *      Accepts WETH as payment and mints a token
     */
    function buySingle(uint256 _tokenId) public {
        // delegate to `buySingleTo` with the transaction sender set to be a recipient
        buySingleTo(_msgSender(), _tokenId);
    }
     ...
     */
    function buySingleTo(address _to, uint256 _tokenId)
        public
        notZeroAddress(_to)
        cost(_msgSender(), price)
        mintLimitAmount(_msgSender(), 1)
        itemIsAvailable(1)
    {
        // verify inputs
        uint256 _approvedAmount = wethToken.allowance(
        ...
        if (_approvedAmount < price)
            revert NotEnoughAllowanceFunds(_approvedAmount);
        // send totalPrice to payment Address
        if(! wethToken.transferFrom(_msgSender(), paymentsAddress, price) ) revert TransferFromFailed();
        
        // mint token to the recipient
        tokenContract.mint(_to, _tokenId);
    ...
    function withdraw() external onlyOwner {
        ...
        payable(paymentsAddress).transfer(_value);
    }

    /**
     * @dev Restricted access function to withdraw ERC20 tokens on the contract balance,
     *      sends tokens to the paymentsAddress
     */
    function stuckToken(address _tokenAddress) external onlyOwner {
        IERC20 _interface = IERC20(_tokenAddress);
        uint256 balanceOf = _interface.balanceOf(address(this));
        _interface.transfer(paymentsAddress, balanceOf);
    }
}
...
interface IMintableERC721 {
		/**
	 * @dev Returns actual total supply of tokens.
...
contract PawsInSand is ERC721, IMintableERC721, AccessControl, Ownable {
    ...
    /// @dev TokenId _exists is false
    error NonExistingToken(uint256);
    /// @dev Can't Mint more than MAX_SUPPLY
    error AboveMaxSupply(uint256);
    /// @dev Invalid onERC721Received response
    error InvalidOnERC721Received(address);
     ...
     */
    event BaseURIChanged(address indexed _by, string _oldVal, string _newVal);
    ...
    {
        string memory _oldURI = theBaseURI;
        // Update base uri
        theBaseURI = _newBaseURI;
        // Fire event
        emit BaseURIChanged(_msgSender(), _oldURI, _newBaseURI);
    }
    ...
    {
        if (_tokenId > MAX_SUPPLY || _totalSupply > MAX_SUPPLY)
            revert AboveMaxSupply(_tokenId);
    ...
    function mintBatch(
        ...
        if (_tokenId > MAX_SUPPLY && _n > MAX_SUPPLY)
            revert AboveMaxSupply(_tokenId);
        if (_totalSupply + _n > MAX_SUPPLY) revert AboveMaxSupply(_tokenId);
    ...
    function safeMint(
        ...
        if (_to.isContract()) {
            ...
            if (response != IERC721Receiver(_to).onERC721Received.selector)
                revert InvalidOnERC721Received(_to);
        }
    }
