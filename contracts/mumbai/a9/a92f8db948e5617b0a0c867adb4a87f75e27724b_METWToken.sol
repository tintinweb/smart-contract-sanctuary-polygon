...
// File: contracts/METW_token.sol

// contracts/METW_token.sol
...
contract METWToken is Ownable, ERC20 {

    using SafeMath for uint256;
    // using SafeERC20 for IERC20;

    address public feeDestination;
    ERC20 public priceToken;

    uint256 public createdTime;
    uint256 public lockupDuration;
    uint256 public penaltyFee;

    address public priceTokenAddress = address(0x0e6DE633CBC92e261a6B8c914FFDa8a2f8BD364d); // USDT
    uint256 public priceDecimals;

    uint256 public constant MAX_FEE = 100;
    uint256 public constant FEE_LIMIT = 10; // 10%

    constructor() ERC20("Math Token", "MT"){

        priceToken = ERC20(priceTokenAddress);
        priceDecimals = ERC20(priceTokenAddress).decimals();
    ...
    function generateToken(uint256 amount) external {

        if (penaltyFee == 0) {
            require(block.timestamp >= createdTime + lockupDuration, "You cannot buy yet!");
        }
        ...
        if (penaltyFee > 0 && block.timestamp < createdTime + lockupDuration) {
            feeAmount = amount.mul(penaltyFee).div(MAX_FEE);
        }

        uint256 priceDec = 10 ** priceDecimals;
        if (feeAmount > 0) {
            uint256 f_amount = feeAmount * priceDec; 
            priceToken.transfer(feeDestination, f_amount);
        }

        amount = amount.sub(feeAmount);

        uint256 p_amount = amount * priceDec;
        priceToken.transfer(feeDestination, p_amount);

        // Here, we sell 1000 token for 1 ETH. The _transfer method and the balanceOf method come from the ERC20 library
        uint256 token = amount.sub(penaltyFee) * 1000;

        assert (balanceOf(address(this)) >= token);

        uint256 dec = 10 ** decimals();
        uint256 price = token * dec;

        _transfer(address(this), _msgSender(), price);
    }

    function burnToken(address _user, uint256 amount) external onlyOwner {
        require(amount > 0, "!amount");

        assert (balanceOf(_user) >= amount);
        _burn(_user, amount);
    }
