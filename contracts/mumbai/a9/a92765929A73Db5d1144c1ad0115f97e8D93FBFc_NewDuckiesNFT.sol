...
// TODO:  1. Missing traits
//        2. Birthdate, longevity
    ...
    RandomUpgradeable
{
    ...
    // uniqueId plays role only when class == SuperLegendary
    CountersUpgradeable.Counter private nextUniqueId;

    mapping(uint256 => uint256) public tokenIdToGene;

    /*
     * Traits generation uses weighted random, which means specifying the probability for each trait value.
    ...
    function initialize(address duckiesAddress) public initializer {
        ...
        // Background
        traitWeights.push([1, 2, 3, 4]);
        // Element
        traitWeights.push([1, 2, 3, 4, 5]);
        // Eyes
        ...
        // UniquenessIndex is presented as a Counter, and makes sense only for SuperLegendary NFTs
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721Upgradeable) returns (string memory) {
        return
            bytes(apiBaseURL).length > 0 ? string(abi.encodePacked(apiBaseURL, tokenIdToGene[tokenId].toString())) : '';
    }
    ...
    function meld(uint256[5] calldata meldingTokenIds) external UseRandom returns (uint256) {
        ...
        tokenIdToGene[meldedTokenId] = meldedGene;
    ...
    function _mint() internal returns (uint256 tokenId, uint256 gene) {
        ...
        tokenIdToGene[tokenId] = gene;
        _safeMint(msg.sender, tokenId);
        nextNewTokenId.increment();

        if (gene.getTrait(Gene.Traits.Class) == uint8(Gene.Classes.SuperLegendary)) {
            nextUniqueId.increment();
        }
    }
    ...
    function _burnTokensAndGetGenes(uint256[5] memory tokenIds) internal returns (uint256[5] memory) {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            genes[i] = tokenIdToGene[tokenIds[i]];
            _burn(tokenIds[i]);
        }
