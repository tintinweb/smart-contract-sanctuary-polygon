...
contract NFTMarketplace is ReentrancyGuard, Ownable {
    ...
    function createMarketSale(address nftContract, uint256 itemId) public payable nonReentrant {
        ...
        // transfer the (item price - royalty  - feePercent) to the seller
        idToMarketItem[itemId].seller.transfer(msg.value - royaltyAmount - feePercent);

        IERC721(nftContract).transferFrom(address(this), msg.sender, tokenId);

        _itemsSold.increment();
        payable(feeAccount).transfer(listingPrice);
        payable(feeAccount).transfer(feePercent);
