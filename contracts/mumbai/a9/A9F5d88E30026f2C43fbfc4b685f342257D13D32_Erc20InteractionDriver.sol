...
contract Erc20CooperatableDriverContract is TokenCooperatableDriverContract {
    ...
    function fetch_token_contract_interface_id() internal virtual override returns(bytes4) {
       return type(IERC20TokenContract).interfaceId;
    }    
}
...
import "../base/Erc20CooperatableDriverContract.sol";
import "../Interfaces/IERC20InteractionDriver.sol";

contract Erc20InteractionDriver is Erc20CooperatableDriverContract,IERC20InteractionDriver {
    
    constructor(string memory name_,string memory desc_,address token_contract_) 
        Erc20CooperatableDriverContract(name_,desc_,token_contract_) {
    }
    function mintFor(address _addr,uint256 toMintAmount) external onlyOwnerAndOperatableTemplate{
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");
        
        IERC20TokenContract _tokenContract = IERC20TokenContract(_token_contract);
        _tokenContract.mintFor(_addr,toMintAmount);
    }
    function burnFor(address _addr,uint256 toBurnAmount) external onlyOwnerAndOperatableTemplate{
        address _token_contract = get_token_contract_address();
        require(_token_contract != address(0),"NO_TOKEN_CONTRACT");
        
        IERC20TokenContract _tokenContract = IERC20TokenContract(_token_contract);
        _tokenContract.burnFor(_addr,toBurnAmount);
    }
    function get_driver_token_contract() external view virtual override returns (address){return get_token_contract_address();}
    function supportsInterface(bytes4 interfaceId) public view virtual override returns (bool) {
        return
            interfaceId == type(IERC20InteractionDriver).interfaceId ||
            super.supportsInterface(interfaceId);
    }
