  ...
  function _distributeGapBonus(address _user,address ref)  internal returns(bool){
    ...
    function fetchUser() external onlyAdmin returns (bool){
       ...
       if(lastIDCount<dataContract(tokenAddress).lastIDCount()){
        lastIDCount++;
        address _usrAddress=dataContract(tokenAddress).userAddressByID(lastIDCount);
        _oldUserFetch(_usrAddress);
        userAddressByID[lastIDCount] = _usrAddress;
        _oldUserGainsFetch(_usrAddress);
       }
    ...
    function _oldUserFetch(address userAddress) internal {
       ...
       (oldUserinfo.joined,oldUserinfo.referral,oldUserinfo.id,oldUserinfo.activeDirect,oldUserinfo.teamCount,,oldUserinfo.strongTeam,,,,,,oldUserinfo.globalPoolCount,)=dataContract(tokenAddress).userInfos(userAddress);
        _royaltyQualify(userAddress);
        uint j =oldUserinfo.activeDirect-AR_DIRECTS;
        oldUserinfo.AR_VaildityIndex +=uint32 (j*sysInfos.AR_validity);
    ...
    function fetchPool() onlyAdmin external returns (bool){
     
        // skip revert call here 

        uint filter2xIndex = autoPool2xDataList.length==0?0:autoPool2xDataList.length-1;
         uint filter3xIndex = autoPool3xDataList.length==0?0:autoPool3xDataList.length-1;

        for(uint i ; i <=sysInfos.maxDepth; i++){
            
            if(filter2xIndex<=dataContract(tokenAddress).autoPool2xLastIndex()){
            

            autoPool2x memory mPool2x;
            (mPool2x.userID, mPool2x.autoPoolParent, mPool2x.mIndex)= dataContract(tokenAddress).autoPool2xDataList(filter2xIndex==0?0:filter2xIndex++);
           
            autoPool2xDataList.push(mPool2x);


            }
            if(filter3xIndex<=dataContract(tokenAddress).autoPool3xLastIndex()){
            
             autoPool3x memory mPool3x;
            ( mPool3x.userID, mPool3x.autoPoolParent, mPool3x.mIndex) = dataContract(tokenAddress).autoPool3xDataList(filter3xIndex==0?0:filter3xIndex++);
