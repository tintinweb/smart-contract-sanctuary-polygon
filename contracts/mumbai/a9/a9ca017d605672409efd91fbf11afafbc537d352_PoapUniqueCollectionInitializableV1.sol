    ...
    PausableUpgradeable
{
    ...
    modifier tokensLeft() {
        require(_hasTokensLeft(), "Purchase would exceed max tokens");
        _;
    }
    function _hasTokensLeft() internal virtual returns (bool hasTokensLeft) {
        hasTokensLeft = _tokenIdCounter.current() <= maxSupply;
    }
...
pragma solidity ^0.8.13;
contract PoapUniqueCollectionInitializableV1 is UniqueCollectionInitializableV1 {
    function initialize(
        UniqueCollectionInitializableV1.InitializeData calldata _init
    ) public override {
        super.initialize(_init);
    }
    function _hasTokensLeft()
        internal
        virtual
        override
        returns(bool)
