...
contract Kernal is ERC721 {
    ...
    constructor (string memory name, string memory symbol, address _creator, address _operator) ERC721 (name, symbol, _creator) {
        operator = _operator;
    }
    ...
    function createCollectible(address to, string memory tokenURI) external returns (uint256) {
    ...
    function changeOperator(address _operator) external onlyOwner returns(bool) {
        require(_operator != address(0), "operator address is the zero address");
