...
contract BPM is ERC721, Ownable, RandomlyAssigned {
  using Strings for uint256;
  uint256 public currentSupply = 0;
  
  string public baseURI = "ipfs://QmUtQqNCaPkDymJCpY244HbGcvL9N6mtGPhXpzPUCsv4FM/";

  constructor() 
    ERC721("Panda~Monium", "PANDAS")
    RandomlyAssigned(1500,1) // Max. 1500 NFTs available; Start counting from 1 (instead of 0)
    {
  ...
  function _baseURI() internal view virtual override returns (string memory) {
   ...
   function updateValue(uint256 supply) internal view returns (uint256 value){
       if(supply < 25) {
           return 0.005 ether;
       }
       if(supply <30 ) {
