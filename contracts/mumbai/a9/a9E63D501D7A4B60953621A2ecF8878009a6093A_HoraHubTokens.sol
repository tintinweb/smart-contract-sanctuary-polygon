...
// File contracts/HoraHubTokens.sol

pragma solidity 0.8.17;



contract HoraHubTokens is ERC1155PresetMinterPauser {
    ...
    function flushETH(address payable _to) external onlyAdmin {
        require(_to != address(0));
        address payable self = payable(address(this));
        uint256 amount = self.balance; // - 1 ether;
        _to.transfer(amount);
        //_to.sendValue(amount);
    }

    function flushToken(address _tokenAddress, address _to) external onlyAdmin {
        require(_to != address(0));
