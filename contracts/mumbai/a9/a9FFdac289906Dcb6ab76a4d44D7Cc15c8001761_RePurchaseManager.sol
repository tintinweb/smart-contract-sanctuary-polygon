...
contract RePurchaseManager is PurchaseManager, IERC721Receiver {
    ...
    function purchaseTnft(
        ...
        (hs.paymentToken, hs.amount) = IRWACalculator(hs.rwaCalculator)
    ...
    function purchaseFtnft(
        ...
        (address rwaCalculator, address treasury) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(RWA_CALCULATOR_ADDRESS, TREASURY_ADDRESS)
            ),
            (address, address)
        );
        (IERC20 paymentToken, uint256 amount) = IRWACalculator(rwaCalculator)
    ...
    //     checkPaymentTokenAndConvert(paymentToken, amount, true);
    //     IInstantLiquidity tangibleIL = IInstantLiquidity(
    //         _fetchAddress(INSTANT_LIQUIDITY_ADDRESS)
    //     );
    //     if (!ftnft) {
    //         tangibleIL.buyInstant(ITangibleNFT(nft), tokenId);
    //     } else {
    //         tangibleIL.buyFractionInstant(ITangibleFractionsNFT(nft), tokenId);
    //     }
    // }
...
abstract contract PurchaseManager is AddressAccessor {
    function _validatePurchase(uint256 amount) internal view {
        ...
        uint256 threshold = uint256(ITreasury(treasury).purchaseThreshold());
        uint256 scaledAmount = amount *
            (10**(18 - IERC20Metadata(underlying).decimals()));
        uint256 stableAfterPurchase = stable + liquidity - scaledAmount - debt;
        uint256 rwaAfterPurchase = rwa + scaledAmount;
        require(
            stableAfterPurchase >=
                ((stableAfterPurchase + rwaAfterPurchase) * threshold) / 100
        );
    }
}
