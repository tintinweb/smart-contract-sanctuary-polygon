...
contract galaxis_stickerbook_mark_two_transfer is ERC1155, Ownable {
    ...
    function newStickerBook(
        ...
        _newStickerBook(name, maxRedemptions, clearBadge, traitToClear);
    }
    ...
    function _newStickerBook(
        ...
        stickerBookData[bookPos] = stickerBookInfo(name,bookPos,false,maxRedemptions,0,"",0,_clearBadge,_traitToClear);
    }
    ...
    function addSticker(
        ...
        uint16 stickerBookID = getStickerBookID(sticker_name);
    ...
    function _addSticker(
        ...
        for (uint256 pos = 0; pos < thisStickerData.length; pos++){
            console.log("sticker entry ",pos," trait ",thisStickerData[pos].traitID);
    ...
    function convert(stickerInput memory si) internal returns (sticker memory sm) {
        ...
        if (address(_implementers[si.traitID]) == address(reg)) {  // boolean
            ...
            console.log(si.traitID,"BOOL av length : ",si.alternative_values.length);
    ...
    function claim(string calldata name, uint16[] calldata tokenIds) external {
        ...
        if (!_clearBadge) {
            ...
            require(reg.addressCanModifyTrait(address(this),_traitToClear),"Stickerbook cannot modify badge");
            for (uint16 pos = 0; pos < tokenIds.length; pos++) {
                require(nft.ownerOf(tokenIds[pos]) == msg.sender,"Claimant does not own these tokens" );
                if (!reg.hasTrait(_traitToClear,tokenIds[pos])) {
                    revert NoBadge(_traitToClear, tokenIds[pos]);
                }
                reg.setTrait(_traitToClear, tokenIds[pos],false);
            }
    ...
    function eligible(string memory name, uint16 tokenId) external view returns (uint256[] memory) {
       uint16 stickerBookID = getStickerBookID(name);
        ...
        for (uint16 position = 0; position < count; position++) {
    ...
    // use setRewardURI
    // function setLateURI(string calldata _newURI, uint16 tokenId) external onlyAllowed {
    //     lateURIS[tokenId] = _newURI;
    // }
    ...
    function addFullStickerBook(fullStickerBookData calldata fsbd) external onlyAllowed {
        console.log("name : ",fsbd.name,",max redemptions ",fsbd.maxRedemptions);
        _newStickerBook(fsbd.name,fsbd.maxRedemptions,fsbd.clearBadge, fsbd.traitToClear);
    ...
    function getStickerBookData(string calldata name) external view returns (fullStickerBookInfo memory fsbd) {
        fsbd.name = name;
        fsbd.stickerBookId = getStickerBookID(name);
        stickerBookInfo storage fsm = stickerBookData[fsbd.stickerBookId];
        fsbd.active = fsm.active;
        fsbd.maxRedemptions = fsm.maxRedemptions;
        fsbd.uri = fsm.uri;
        fsbd.numberRedeemed = fsm.numberRedeemed;
        fsbd.clearBadge = fsm.clearBadge;
        fsbd.traitToClear = fsm.traitToClear;
        fsbd.stickers = new sticker[][](fsm.stickerCount);
        for (uint16 j = 0; j < fsm.stickerCount; j++) {
            uint256 len = stickers[fsbd.stickerBookId][j].length;
            fsbd.stickers[j] = new sticker[](
                len
            );
            for (uint k = 0; k < len; k++) {
                fsbd.stickers[j][k] = stickers[fsbd.stickerBookId][j][k];
            }
        }
    }
...
interface ITraitValueImplementer {
    ...
    function getValues(uint16[] memory _tokenIds) external view returns (uint8[] memory result) ;
