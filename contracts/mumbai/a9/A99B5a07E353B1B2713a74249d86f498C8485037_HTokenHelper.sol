...
contract HTokenHelper is HTokenHelperI, Ownable {
  ...
  function getMaxBorrowableAmount(HTokenI _hToken, HivemindI _hivemind) external view returns (uint256) {
    uint256 floorPrice = getFloorPriceInUSD(_hToken);
    uint256 underlyingPriceInUSD = getUnderlyingPriceInUSD(_hToken);
    return (_hivemind.getCollateralFactor(_hToken) * floorPrice) / underlyingPriceInUSD;
  }
