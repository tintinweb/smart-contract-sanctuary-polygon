...
contract PaulAndSharkPhaseII is ERC1155, Ownable {
    ...
    function mintWithPaper(uint8 _id, uint16 _qty, bytes32 _nonce, bytes calldata _signature, address walletAddress) public payable checkFee
        checkTokenId(_id) checkLimit(_id, _qty)
        onlyPaper(keccak256(abi.encode(walletAddress, _id, _qty)), _nonce, _signature)
    {
        mintLimit[_id] = mintLimit[_id] - _qty;
        _mint(walletAddress, _id, _qty, "");
    }

    function mint(uint8 _id, uint16 _qty) public payable checkFee
        checkTokenId(_id) checkLimit(_id, _qty){
        mintLimit[_id] = mintLimit[_id] - _qty;
