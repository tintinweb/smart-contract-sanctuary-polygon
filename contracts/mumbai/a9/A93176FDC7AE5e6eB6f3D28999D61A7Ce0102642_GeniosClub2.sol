...
contract GeniosClub2 is IGeniosClub {
  ...
  function updateG3X7Referrer(
    ...
    if (ref_currentRef == Id0) {
      return
        sendG3X7TokenDividends(ref_currentRef, userAddr, level, address(0));
    }
    ...
    updateG3X7RefLastLevel(userAddr, curRefaddr, ref_currentRef, level);
  }
  ...
  function updateG3X7RefLastLevel(
    ...
    uint256 len = Users[lastRef].G3X7Matrix[level].SeventhLevelRefs.length;
    ...
    if (len < 2187) {
      ...
      Users[lastRef].G3X7MatrixRecycleAmount[level] += amount;
      return sendG3X7TokenDividends(curRefaddr, userAddr, level, lastRef);
    }

    Users[lastRef].G3X7MatrixRecycleAmount[level] = 0;
    sendG3X7TokenDividends(curRefaddr, userAddr, level, lastRef);

    // recycle
    Users[lastRef].G3X7Matrix[level].FirstLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].SecondLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].ThirdLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].FourthLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].FifthLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].SixthLevelRefs = new address[](0);
    Users[lastRef].G3X7Matrix[level].SeventhLevelRefs = new address[](0);

    if (!Users[lastRef].ActiveG3X7Levels[level + 1] && level != LAST_LEVEL) {
      Users[lastRef].G3X7Matrix[level].Blocked = true;
    }

    Users[lastRef].G3X7Matrix[level].ReinvestCount++;
    Users[lastRef].G3X7Matrix[level].ReinvestTime = block.timestamp;

    if (lastRef != Id0) {
      address freeRefAddr = findFreeG3X7Referrer(lastRef, level);
