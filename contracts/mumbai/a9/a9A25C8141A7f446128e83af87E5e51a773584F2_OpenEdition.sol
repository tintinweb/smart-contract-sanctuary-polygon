...
contract OpenEdition is ERC1155Base {
    ...
    mapping(address => mapping(uint256 => uint256)) public claimedAt;
    ...
    constructor(
        ...
        uint256 _timeToResell,
        address _royaltyRecipient,
        uint128 _royaltyBps
    ) ERC1155Base(_name, _symbol, _royaltyRecipient, _royaltyBps) {
        require(_dropStartTime < _dropEndTime, "invalid drop times");
        currencyAddress = _currencyAddress;
        dropStartTime = _dropStartTime;
        dropEndTime = _dropEndTime;
        maxPerWallet = _maxPerWallet;
        timeToResell = _timeToResell;
        totalTraits = 0;
    }

    /// @dev Set the base URI given token ID
    function setBaseURI(
        uint256 _tokenId,
        uint256 _price,
        string calldata _baseURI
    ) external onlyOwner {
        items[_tokenId] = Item({price: _price, baseURI: _baseURI});
        _setTokenURI(_tokenId, _baseURI);
        totalTraits++;
    }

    /// @dev Lets an authorized address mint NFTs to a recipient.
    ...
    function callMint(
        ...
        require(
            _tokenId != type(uint256).max && _tokenId < totalTraits,
            "invalid id"
        );
        require(
            block.timestamp >= dropStartTime && block.timestamp <= dropEndTime,
            "drop not active"
        );
        require(balanceOf[_to][_tokenId] + _amount <= maxPerWallet, "!Qty");
        require(
            IERC20(currencyAddress).transferFrom(
                msg.sender,
                address(this),
                items[_tokenId].price * _amount
            ),
            "!PriceOrCurrency"
        );
        // Store the timestamp when the token is minted
        claimedAt[_to][_tokenId] = block.timestamp;
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            // If the token is being transferred, check the timestamp logic
            if (from != address(0) && to != address(0)) {
                require(
                    block.timestamp <= claimedAt[from][ids[i]] + timeToResell,
                    "after timeToResell"
                );
            }
        }
