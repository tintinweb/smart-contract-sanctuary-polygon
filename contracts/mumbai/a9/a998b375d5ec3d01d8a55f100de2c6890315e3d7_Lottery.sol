...
contract Lottery is Ownable {
	...
	function getLotteryInfo(uint _round) public view returns (bool, uint[] memory, uint[] memory) {
		return (
			lotteryInfos[_round].lotteryFlag,
			lotteryInfos[_round].winningTails,
			lotteryInfos[_round].winRateBitValues
		);
	}
}