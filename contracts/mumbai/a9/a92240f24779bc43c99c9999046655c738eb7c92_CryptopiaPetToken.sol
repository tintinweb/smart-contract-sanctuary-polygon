...
/// @title Cryptopia Pet Token
/// @dev Non-fungible token (ERC721)
/// @author Frank Bonnet - <[email protected]>
contract CryptopiaPetToken is ICryptopiaPetToken, Initializable, ERC721EnumerableUpgradeable, ContextMixin, NativeMetaTransaction, TokenRetriever, OwnableUpgradeable {
    ...
    uint constant MIN_LEGENDARY_SCORE = 99_90; // 0.1%
    uint constant MIN_RARE_SCORE = 95_00; // 4.9%
    ...
    /// @dev pet name => Pet
    mapping(bytes32 => Pet) public pets;
    bytes32[] private petsIndex;

    /// @dev tokenId => pet name
    mapping (uint => bytes32) public tokenPets;

    /// @dev tokenId => score
    mapping (uint => uint) tokenScores;

    /// Refs
    IAuthenticator public proxyAuthenticator;


    /**
     * Modifiers
     */
    /// @dev Requires that a pet with `name` exists
    /// @param name Pet's name
    modifier onlyExistingPet(bytes32 name)
    {
        require(_petExists(name), "Non-existing pet");
        _;
    }
    ...
    /// @param _proxyAuthenticator Whiteliste for proxies
    ...
    function initialize(
        address _proxyAuthenticator, 
        string calldata _initialContractURI, 
        string calldata _initialBaseTokenURI) 
        public override initializer 
    {
        __Ownable_init();
        __EIP712_init("Cryptopia Pets");
        __ERC721_init("Cryptopia Pets", "PET");
        __ERC721Enumerable_init_unchained();
        
        proxyAuthenticator = IAuthenticator(_proxyAuthenticator);
    ...
    /// @return fileHash hash of the common 3d model
        ...
        )
    {
        ...
        fileHash = new bytes32[](take);
        species = new uint16[](take);
        modules = new uint24[](take);
        luck = new uint24[](take);
        charisma = new uint24[](take);
        speed = new uint24[](take);
        strength = new uint24[](take);
        special = new uint24[](take);

        uint index = skip;
        for (uint i = 0; i < take; i++)
        {
            name[i] = petsIndex[index];
            fileHash[i] = pets[name[i]].fileHash;
            species[i] = pets[name[i]].species;
            modules[i] = pets[name[i]].modules;
            luck[i] = pets[name[i]].luck;
            charisma[i] = pets[name[i]].charisma;
            speed[i] = pets[name[i]].speed;
            strength[i] = pets[name[i]].strength;
            special[i] = pets[name[i]].special;
            index++;
        }
    }
        ...
        )
    {
        ...
        modules = pets[name].modules;
        luck = pets[name].luck;
        charisma = pets[name].charisma;
        speed = pets[name].speed;
        strength = pets[name].strength;
        special = pets[name].special;
        fileHash = pets[name].fileHash;
    }
    ...
    function setPet(bytes32 _name, bytes32 _fileHash, uint16 _species, uint24[6] memory _attributes, uint32[3] memory _data) 
        ...
        onlyOwner 
    {
        ...
        if (!_petExists(_name))
        {
            petsIndex.push(_name);
        }

        // Set pet
        pets[_name].fileHash = _fileHash;
        pets[_name].species = _species;
        pets[_name].modules = _attributes[0];
        pets[_name].luck = _attributes[1];
        pets[_name].charisma = _attributes[2];
        pets[_name].speed = _attributes[3];
        pets[_name].strength = _attributes[4];
        pets[_name].special = _attributes[5];
        pets[_name].slot1 = _data[0];
        pets[_name].slot2 = _data[1];
        pets[_name].slot3 = _data[2];
    }
    ...
    function getTokenURI(uint _tokenId) override public view returns (string memory) {
        return string(abi.encodePacked(getBaseTokenURI(), tokenPets[_tokenId], "/", _getRarity(_tokenId)));
    }
    ...
    function isApprovedForAll(address _owner, address _operator) override public view returns (bool) {
        if (proxyAuthenticator.authenticate(_operator)) {
    ...
    function mintTo(address _to, bytes32 _pet, uint _minScore) 
        public override onlyOwner onlyExistingPet(_pet) {
        uint newTokenId = _getNextTokenId();
        _mint(_to, newTokenId);
        tokenPets[newTokenId] = _pet;
        _randomize(newTokenId, _minScore);
        _incrementTokenId();
    }
     ...
     * Private functions
     */
    /// @dev True if a pet with `name` exists
    /// @param name name of the pet
    function _petExists(bytes32 name) 
        internal view returns (bool) 
    {
        return pets[name].fileHash != 0;
    }
    ...
    /// @dev Get rarity as a string
    function _getRarity(uint _tokenId) private view returns (string memory)
    {
    ...
    /// @param _minScore that the token will receive max(_minScore, actualScore)
    function _randomize(uint _tokenId, uint _minScore) private {
        tokenScores[_tokenId] = MathUpgradeable.max(_randomAt(_random(), 0), _minScore);
    }
