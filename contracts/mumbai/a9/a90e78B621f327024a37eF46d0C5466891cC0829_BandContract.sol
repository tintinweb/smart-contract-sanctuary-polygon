...
// to update the split's implementation address for creating split wallets
    ...
    function initialize (uint _id, address _bandAdmin, string memory _bandName, address _aliveManager, address aliveRoyaltyReceiver) external initializer{
        bandId = _id;
...
contract AliveBonded is  ERC721A, ERC2981, AccessControl, Pausable, ReentrancyGuard
                         {
    bytes32  constant GOVERNANCE_ROLE = keccak256("GOVERNANCE_ROLE");
    bytes32  constant PAUSER_ROLE = keccak256("PAUSER_ROLE");
    ...
    constructor (
        ...
        // uint[3] memory configureBondedCurve // max total supply of dutch nft 
       
        )  ERC721A("AliveE1", "AE1") { 

        // require(configureBondedCurve[0] > 0, "max supply can't be 0");
        _pause();
        songId = _songID;
        uri= _songuri;
        _grantRole(DEFAULT_ADMIN_ROLE,_alivemanager);
 ...
 */
 ...
 // somew changes made - starts token id with 1, returns just the baseURI
