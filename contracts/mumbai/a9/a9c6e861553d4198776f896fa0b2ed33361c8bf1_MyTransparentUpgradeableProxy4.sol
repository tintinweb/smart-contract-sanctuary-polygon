...
// File contracts/shared/proxies/ERC1967UpgradeBeaconed.sol
...
// File contracts/temp/MyBeacon.sol

// 

pragma solidity ^0.8.12;
contract MyBeacon is Ownable {
    ...
    function getNonZeroImplementation(string memory _name) external view returns (address _implementation) {
        _implementation = implementations[_name];
        require(_implementation != address(0), "Ref not found.");
    ...
    function registerImplementation(string memory _name, address _address) external onlyOwner {
...
// File contracts/shared/proxies/interfaces/ITransparentUpgradeableBeaconedProxy.sol
...
// File contracts/temp/MyTempContract1155v4.sol

// 

pragma solidity 0.8.12;
abstract contract MyBeaconedProxy4 is Proxy, ERC1967UpgradeBeaconed {
    ...
    function _getImplementationFromBeacon(address _beacon) internal view override returns (address _impl) {
        return MyBeacon(_beacon).getNonZeroImplementation("MyTempContract1155v3.Implementation");
    }
}

contract MyTransparentUpgradeableProxy4 is MyBeaconedProxy4 {
    constructor(
        address _beaconAddress,
        address admin_,
        bytes memory _data
    ) payable MyBeaconedProxy4(_beaconAddress, _data) {
    ...
    function dispose() external ifAdmin {
        selfdestruct(payable(msg.sender));
    }
}