...
contract PrimaryIndexTokenAtomicRepaymentV3 is Initializable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    event AtomicRepayment(address indexed user, address collateral, address lendingAsset, uint256 amountSold, uint256 amountReceive);
    ...
    function getTotalOutstanding(address user, address lendingToken) public view returns(uint256 outstanding) {
        (uint256 loanBody, uint256 accrual) = primaryIndexToken.getEstimatedOutstanding(user, lendingToken);
    ...
    function getRemainingDeposit(address user, address projectToken) public view returns(uint256 remainingDeposit) {
        ...
        uint256 collateralRemaining = depositedRemaining * (projectTokenMultiplier) / primaryIndexToken.getTokenEvaluation(projectToken, projectTokenMultiplier);
        remainingDeposit = depositedProjectTokenAmount >= collateralRemaining ? collateralRemaining : depositedProjectTokenAmount;
   }
   ...
   /**
     ...
     */
    function repayAtomic(address lendingToken, address prjToken, uint256 collateralAmount, bytes memory buyCalldata, bool isRepayFully) public nonReentrant{
        _repayAtomic(msg.sender, lendingToken, prjToken, collateralAmount, buyCalldata, isRepayFully);
    }

    function repayAtomicFromRelatedContract(address user, address lendingToken, address prjToken, uint256 collateralAmount, bytes memory buyCalldata, bool isRepayFully) public nonReentrant{
        _repayAtomic(user, lendingToken, prjToken, collateralAmount, buyCalldata, isRepayFully);
    }

    function _repayAtomic(address user, address lendingToken, address prjToken, uint256 collateralAmount, bytes memory buyCalldata, bool isRepayFully) internal {
        ...
        (uint256 amountSold, uint256 amountReceive) = _swapAtomic(user, prjToken, lendingToken, collateralAmount, buyCalldata);
        if (isRepayFully) require(amountReceive >= getTotalOutstanding(user, lendingToken), "AtomicRepayment: amount receive not enough to repay fully");
        ...
        if (collateralAmount > amountSold) {
            ...
            primaryIndexToken.depositFromRelatedContracts(prjToken, collateralBalanceLeft, address(this), user);
        }
        ...
        primaryIndexToken.repayFromRelatedContract(lendingToken, amountReceive, address(this), user);
     ...
     */
    function _swapAtomic(address user, address prjToken, address lendingToken, uint256 collateralAmount, bytes memory buyCalldata) internal returns(uint256 amountSold, uint256 amountReceive) {
        primaryIndexToken.calcAndTransferDepositPosition(prjToken, collateralAmount, user, address(this));
        ...
        (amountSold, amountReceive) = _buyOnParaSwap(prjToken, lendingToken, augustusParaswap, buyCalldata);
    }
     ...
     */
    function _buyOnParaSwap(address tokenFrom, address tokenTo, address _target, bytes memory buyCalldata) internal returns (uint256 amountSold, uint256 amountReceive) {
