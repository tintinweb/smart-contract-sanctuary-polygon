...
contract CoinFlip {
    ...
    function flip(address _tokenAddress, uint256 _value, uint256 _choice) public {
        ...
        if (random % 2 == _choice) {
            ...
            emit CoinFlipResult(true);
            userWins[msg.sender]++;
            userAmount[msg.sender] += amount * 2;
        } else {
            // require(t.balanceOf(address(this)) >= amount, "The contract does not have enough tokens to send back.");
            // require(t.transfer(owner, amount), "Transfer of tokens failed.");
            emit CoinFlipResult(false);
        }
    ...
    function getPlayedCount(address _player) public view returns (uint) {
        return userFlips[_player];
    }

    function getWinCount(address _player) public view returns (uint) {
        return userWins[_player];
    }

    function getWinAmount(address _player) public view returns (uint) {
        return userAmount[_player];
    }
