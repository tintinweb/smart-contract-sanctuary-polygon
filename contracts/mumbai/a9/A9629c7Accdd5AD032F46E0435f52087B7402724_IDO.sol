...
contract IDO is Ownable{
    ...
    uint public saleTarget;
    uint public amountForSale;
    uint public price;
    uint constant public CONTRIBUTION_PERIOD = 172800; //2 days
    bool public openForClaims;
    ...
    address public projectPayout;
    address public gravityPayout;
    ...
    constructor(address _sale, address _purchase, uint _price, uint _start, uint _amountForSale, address _gravityPayout, address _projectPayout){
        ...
        saleTarget = amountForSale * price;
    ...
    function changeStart(uint _start) external{
        start = _start;
    }

    //only allowed during contribution period
    ...
    function processRefundAndAmountOwed(address _user) public{
        ...
        if(totalRaise < saleTarget){
            ...
            //over subscribed
            amountOwed[_user] = amountForSale * userContribution / totalRaise;
            uint amountToRefund = userContribution - (amountToSend*price);
            if(amountToRefund>0){
    ...
    function adjustClaims(bool _state) external onlyOwner{
        require(block.timestamp >= (start+CONTRIBUTION_PERIOD), "Contribution period is not over");
        openForClaims = _state;
    }


    //should be able to withdraw AFTER contribution period is over
    function withdraw() external onlyOwner{
        require(block.timestamp >= (start+CONTRIBUTION_PERIOD), "Contribution period is not over");
        require(!withdrawCalled, "Already called withdraw");
        uint fundsToDistribute = totalRaise > saleTarget ? saleTarget : totalRaise;
    ...
    function rescueFunds(address _token, uint _amount) external onlyOwner{
        require(block.timestamp >= (start+(CONTRIBUTION_PERIOD*7)), "Can not rescue funds until 2 weeks have passed since sale start");
        IERC20(_token).safeTransfer(msg.sender, _amount);
    }

    function saleInfo() external view returns(uint _start, uint _contributionEnd, uint _amountForSale, uint _price, bool _openForClaims){
        return(start, start+CONTRIBUTION_PERIOD, amountForSale, price, openForClaims);
    }

    function userInfo(address _user) external view returns(uint _contribution, uint _amountOwed){
        return(contribution[_user], amountOwed[_user]);
    }
}
