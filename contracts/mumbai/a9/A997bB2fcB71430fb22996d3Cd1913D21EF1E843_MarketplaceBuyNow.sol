...
contract MarketplaceBuyNow is Ownable, ReentrancyGuard {
    ...
    modifier notListed(
        ...
        modifier notListedBatch(
        address _nftAddress,
        uint256[] memory _tokenIdList,
        address _owner
    ) {
        for (uint i = 0; i < _tokenIdList.length; i++) {
            Listing memory listing = listings[_nftAddress][_tokenIdList[i]][_owner];
            require(listing.quantity == 0, "already listed");
        }
        _;
    }
    ...
    function listItemBatch(
        ...
        uint256 _pricePerItem,
        uint256 _startingTime
    ) external notListedBatch(_nftAddress, _tokenIdList, _msgSender()) {
        for (uint i = 0; i < _tokenIdList.length; i++) {
            listItem(_nftAddress, _tokenIdList[i], _quantity, _pricePerItem, _startingTime);
        }
    }
