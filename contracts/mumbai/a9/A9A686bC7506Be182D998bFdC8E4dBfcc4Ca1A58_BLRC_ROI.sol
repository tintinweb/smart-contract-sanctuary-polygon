...
contract BLRC_ROI {
    ...
    function checkNftClaimaible(address user,uint8 _type) public view returns(bool) {
        bool flag = false;
        if(_type==1 && userTeams[user].totalTeam >= 3 && users[user].refCount[1]>=1)//_type==1 && userTeams[user].totalTeam >= 10 && users[user].refCount[1]>=1
        {
            flag = true;
        }
        if(_type==2 && userTeams[user].totalTeam >= 4 && users[user].refCount[1]>=2)//_type==2 && userTeams[user].totalTeam >= 30 && users[user].refCount[1]>=3
        {
            flag = true;
        }
        if(_type==3 && userTeams[user].totalTeam >= 5 && users[user].refCount[1]>=3)//_type==3 && userTeams[user].totalTeam >= 60 && users[user].refCount[1]>=6
        {
            flag = true;
        }
        return flag;
