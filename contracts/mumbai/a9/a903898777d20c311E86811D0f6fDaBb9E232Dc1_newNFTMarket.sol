...
contract newNFTMarket is Initializable, ERC721Upgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, OwnableUpgradeable {
    ...
    //     __ERC721_init("NFTMonk Market", "MONKMKT");
...
/////////////////////////
 ...
 //Market List   
    ...
    function createMarketItem (
    ...
    ) public nonReentrant onlyRole(RELAY_ROLE) {
        INRPeggedMonk(inrAddress).relayTransfer (msg.sender, address(this),listingPrice);
        _itemIds.increment();
        ...
        IERC721Upgradeable(nftContract).transferFrom(seller, address(this), tokenId);
...
/// Market unlist
    ...
    function unlistMarketItem (
        uint256 itemId,
        address inrAddress,
        uint256 unlistingPrice

    ) public nonReentrant onlyRole(RELAY_ROLE) {
        address nftContract = idToMarketItem[itemId].nftContract;
        uint256 tokenId = idToMarketItem[itemId].tokenId;
        address seller = idToMarketItem[itemId].seller;
        INRPeggedMonk(inrAddress).relayTransfer (msg.sender, address(this),unlistingPrice);
        idToMarketItem[itemId].isSold = true;
        IERC721Upgradeable(nftContract).transferFrom(address(this), seller, tokenId);
...
/// Change Price
    ...
    function changePrice (
        uint256 itemId,
        address inrAddress,
        uint256 rateChangingPrice,
        uint256 newPrice

    ) public nonReentrant onlyRole(RELAY_ROLE) {
        address seller = idToMarketItem[itemId].seller;
        INRPeggedMonk(inrAddress).relayTransfer (msg.sender, address(this),rateChangingPrice);
 ...
 //Market Sale
    function createMarketSale(uint256 itemId, address buyer, address inrAddress, uint256 cutPrice, uint256 commissionPrice)
        public nonReentrant onlyRole(RELAY_ROLE)

    {
        ...
        address seller = idToMarketItem[itemId].seller;
        address nftContract = idToMarketItem[itemId].nftContract;
        // require(   |Use only if needed not required in relayed call.|
        //     msg.value == price,
        //     "Please make the price to be same as listing price"
        // );

        // idToMarketItem[itemId].seller.transfer(msg.value);
        INRPeggedMonk(inrAddress).relayTransfer (msg.sender, seller, cutPrice);
        INRPeggedMonk(inrAddress).relayTransfer (msg.sender, address(this), commissionPrice);
        IERC721Upgradeable(nftContract).transferFrom(address(this), buyer, tokenId);
        idToMarketItem[itemId].isSold = true;
