...
contract Auctions {
    ...
    function _registerAuction(Auction memory _auctionDetails) internal {
        ...
        _auctionDetails.toEOA,_auctionDetails.acceptedToken,_auctionDetails.startingPrice,
...
contract Orders {
    ...
    function _registerSellOrder(List memory _orderDetails) internal {
        ...
        activeListings[_orderDetails.seller].push(
            ...
            _orderDetails.toEOA,_orderDetails.maxTokenSell,_orderDetails.acceptedToken,_orderDetails.maxBuyPerWallet,
...
interface ISwapr{
    function makeUpfrontPayment(address acceptedToken, address seller, uint amount) external payable returns(bool paid);
    function depositNFT(address lock, uint nftId, address seller) external;
    function transferNFT(address lock, uint nftId, address sig) external;
    function claim(bytes memory data) external returns(bool);
}
...
interface ISwaprWallet{
    ...
    function depositERC(address depositor, address token) external;

    function lockNFT(bytes calldata sig, address lock, uint nftId, address owner) external;
    function lockNFTOC(address lock, uint nftId, address owner) external;
    function swapNFTOC(address lock, uint nftId, address owner) external;
    function releaseNFTOC(address lock, uint nftId, address receiver) external;

    function isNFTLocked(address lock, uint nftId) external view returns(bool);

    function getNFT(address lock, uint nftId) external view returns(bytes memory);
    ...
    function releaseFunds(address receiver, uint amount) external;
    function releaseFunds(address token, address receiver, uint amount) external;
    function releaseAll(address token, address receiver) external;//experimental
...
/// @title Manages on-chain listing records
/// @author swapr
/// @notice Allows only on-chain listings
/// @dev Can only be interacted from a recognised marketplace contract
contract Marketplace is BaseGovernanceUpgradeable, Orders, Auctions {
    ...
    /// @notice if the sale is in progress the function will freeze to prevent double spend
    ...
    /// @notice To update the swapr incase
    ...
    /// @notice Creates a new listing for Order type based on data provided
    /// @param data contains the encoded string as following:
    /**  
    *    bytes data: {
            // uint price,
            // address lock,
    ...
    */
    function createSellOrder(bytes calldata data) external {
        ...
        require(listing.maxTokenSell > 0 && listing.maxTokenSell <= EXP , "ERROR: INCORRECT_MAX_TOKEN_SELL");
        require(listing.maxBuyPerWallet > 0 && listing.maxBuyPerWallet <= EXP , "ERROR: INCORRECT_MAX_BUY_WALLET");
    ...
    /// @notice Allows users to buy NFT or Order type
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId to be purchased
    /// @param split part of NFT to be purchased in % as EXP
    /// @dev split should not receed 0 or exceed EXP
    function buyNowOrder(address lock, uint nftId, uint split) external inProgressGuard(lock, nftId) {
        ...
        require(split > 0 && split <= EXP && split <= order.remainingPart && split <= order.maxBuyPerWallet, "ERROR: INCORRECT_SPLIT_AMOUNT");
        ...
        uint price = (order.price * split) / EXP;

        Swapr.PayNow memory payOps = Swapr.PayNow(order.toEOA, order.acceptedToken, buyer, order.seller, swaprWallet.getBalance(buyer,order.acceptedToken), price);
        require(swaprContract.payNow(payOps), "ERROR: PAYMENT_FAILED");
    ...
    /// @notice Allows seller to cancel listed Order type
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    ...
    /// @notice Creates listing for Auction type
    /**  
    *    bytes data: {
            // address lock,
            // uint nftId,
    ...
    */
    ...
    /// @notice offers a bid on listed Auction type
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    /// @param offer price to be offered
    ...
    /// @notice Buys a listed Auction type by upfront payment
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    /// @dev inProgressGuard required to prevent reentrancy
    function buyNowAuction(address lock, uint nftId) external inProgressGuard(lock, nftId) {
        ...
        //MakePayment
        Swapr.PayNow memory payOps = Swapr.PayNow(auction.toEOA, auction.acceptedToken, buyer, auction.seller, swaprWallet.getBalance(buyer,auction.acceptedToken), auction.buyNowPrice);
        require(swaprContract.payNow(payOps), "ERROR: PAYMENT_FAILED");
    ...
    /// @notice to claim the rightful assets/funds as per Auction / Bid
    /// @dev works for both seller and buyer
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    function claim(address lock, uint nftId) external returns(bool claimed, string memory res) {

        (claimed, res) = isClaimable(lock, nftId);
        require(claimed, res);

        (claimed, res) = swaprContract.claim(_msgSender(), lock, nftId);
        require(claimed, res);
    ...
    /// @notice withdraws lockedin NFTs if deal ends
    function withdraw(address lock, uint nftId) external {
        require(isWithdrawable(lock, nftId), "ERROR: CAN_NOT_WITHDRAW");
        swaprContract.withdraw(_msgSender(), lock, nftId);
    }

    /// @notice withdraws unlocked ETH or ERC20
    function withdraw(address token) external {
        swaprContract.withdraw(_msgSender(), token);
    }

    /// @notice check if seller/bidder can claim assets
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    ...
    /// @notice check if owner can withdraw owned ERC721
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    function isWithdrawable(address lock, uint nftId) public view returns(bool withdrawable) {
        if(_msgSender() == swaprContract.ownerOf(lock, nftId)){
            if(!orderExists(lock, nftId)){
                withdrawable = true;
            }
            if(!auctionExists(lock, nftId)){
                withdrawable = true;
    ...
    /// @notice decodes Auction type info
...
/// @title Manages NFT listings and user funds to help marketplace interact with wallet
/// @author swapr
/// @notice Allows only on-chain listings
/// @dev Can only be interacted from a recognised marketplace contract
contract Swapr is BaseGovernanceUpgradeable {
    ...
    function attachNewMarketplace(address marketplaceAddress) external onlyAdmin {
        marketplaceContract = Marketplace(marketplaceAddress);
        _setupRole(MARKETPLACE_ROLE, marketplaceAddress);
    }

    /// @notice Triggers the payment for any valid purchase action on listing
    /// @dev Can handle Native or ERC payments of any kind
    /// @param payOps refer to type PayNow
    /// @return paid true if payment succeeds
    function payNow(PayNow memory payOps) public returns(bool paid) {
        if(payOps.fromBalance >= payOps.amount){
            if(payOps.acceptedToken == address(0)){
                swaprWallet.swapNative(payOps.from, payOps.receiver, payOps.amount);
                swaprWallet.unlockNative(payOps.from, payOps.amount);
                if(payOps.toEOA){swaprWallet.releaseNative(payOps.receiver, payOps.amount);}
            }else{
                swaprWallet.swapERC(payOps.acceptedToken, payOps.from, payOps.receiver, payOps.amount);
                swaprWallet.unlockERC(payOps.acceptedToken, payOps.from, payOps.amount);
                if(payOps.toEOA){swaprWallet.releaseERC(payOps.acceptedToken, payOps.receiver, payOps.amount);}
            }
            paid = true;
        }else{
            if(payOps.toEOA){swaprWallet.payNow(payOps.acceptedToken, payOps.from, payOps.receiver, payOps.amount);}
            else{
                if(payOps.acceptedToken == address(0)){
                    addFunds();
                    swaprWallet.swapNative(payOps.from, payOps.receiver, payOps.amount);
                }else{
                    _addFunds(payOps.acceptedToken, payOps.receiver);
                }
            }
            paid = true;
        }
    ...
    /// @notice External function for user/marketplace to deposit user funds
    ///         into swapr wallet 
    /// @dev Only deposits ERC20 tokens
    /// @param token address of token to be deposited
    function addFunds(address token) external {
        if(address(token) != address(0)){
            _addFunds(_msgSender(), token);
        }
    }
    
    /// @notice internal function to support custom depositor as msg.sender
    function _addFunds(address token, address receiver) internal {
        //Deposit ERC approved only into wallet
        swaprWallet.depositERC(receiver, token);
    }

    /// @notice Public function for user/marketplace to deposit user funds as ETH,BNB or MATIC etc
    function addFunds() public payable {
        //Deposit Native only into wallet
        require(msg.value > 0, "ERROR: LOW_VALUE_OBSERVED");
        payable(address(swaprWallet)).transfer(msg.value);
        swaprWallet.depositNative(_msgSender(), msg.value);
    }
    ...
    function ownerOf(address lock, uint nftId) public view returns(address owner) {
        owner = swaprWallet.getNFTOC(lock, nftId);
    }

    /// @notice helper for marketplace claim
    /// @dev directly interacts with swapr wallet for operations
    /// @param claimant the claiming party
    /// @param lock ERC721 lock proxy address
    /// @param nftId tokenId
    function claim(address claimant, address lock, uint nftId) external onlyMarketplace returns(bool claimed, string memory res) {
        ...
        bool toEOA = marketplaceContract.getSingleAuction(lock, nftId).toEOA;
        ...
        if(claimant == seller){
            if(marketplaceContract.bidExists(lock, nftId)){
                ...
                PayNow memory payOps = PayNow(toEOA, token, claimant, seller, swaprWallet.getLockedBalance(claimant,token), amount);
                claimed = payNow(payOps);
                if(claimed){
                    res = "";
                }
            }
    ...
    /// @notice for withdrawal only usable by marketplace
    function withdraw(address receiver, address lock, uint nftId) external onlyMarketplace {
        //TODO: check if NFT is already locked requires to claim first;
        _withdraw(receiver, lock, nftId);
    }
    /// @notice for withdrawal only usable by marketplace
    function withdraw(address receiver, address token) external onlyMarketplace {
        _withdraw(receiver, token);
    }
    ...
    function _withdraw(address claimant, address token) internal {
        ...
        if(amount > 0){
            if(token == address(0)){
                swaprWallet.releaseNative(claimant, amount);
            }else{
                swaprWallet.releaseERC(token, claimant, amount);
            }
        }
    }
...
contract SwaprGL is BaseGovernanceUpgradeable {

    /// @notice Struct type to encapsulate auction data
    /// @dev depositType is to detect the listing type
    /// @dev activeDepositType is to check if asset is already listed
    struct Auction {
        uint depositType;
    ...
    struct Bid{
        ...
        bytes listingSig;    //signature of the listing the bid was made on for cross verification
    }
    ...
    SwaprWallet private swaprWallet;
    address private theMarketplace;
    ...
    function depositNFTs(bytes calldata data) external {
        ...
        require(_verify(_msgSender(), data), "ERROR: INCORRECT_DEPOSIT_SIGNATURE");
        ...
        require(depositType == 3, "ERROR: INVALID_DEPOSIT_COMMAND");
        swaprWallet.lockNFT(sig, lock, nftId, _msgSender());

    }

    /// @notice Triggers the payment for any valid purchase auction on listing
    /// @dev Can handle Native or ERC payments of any kind
    /// @param payOps refer to type PayNow
    /// @return paid true if payment succeeds
    function payNow(PayNow memory payOps) internal returns(bool paid) {
        if(payOps.fromBalance >= payOps.amount){
            if(payOps.acceptedToken == address(0)){
                swaprWallet.swapNative(payOps.from, payOps.receiver, payOps.amount);
                swaprWallet.unlockNative(payOps.from, payOps.amount);
    ...
    */
    function createListing(uint listingType, bytes calldata data) external onlyMarketplace(data) {
        ...
        (bool isValid, address lock, uint nftId, bytes memory sig, string memory res) = _validateListing(listingType, listingData);
        require(isValid, res);
        swaprWallet.lockNFT(sig, lock, nftId, _msgSender());
    ...
    /// @param listingData must be provided with sellers signature as following:
    /**
    *       bytes listingData: {
    *           bytes message,
    *           bytes sellerSignature,
    ...
    */
    ...
    function isListable(bytes memory listingData) external view returns(bool isValid, string memory res) {
        
        Auction memory auction = _extractAuctionInfo(listingData);
        (isValid,,,,res) = _validateListing(auction.depositType, listingData);
    ...
    */
    ...
    function _validateListing(uint listingType, bytes memory listingData) internal view returns(bool isValid, address lock, uint nftId, bytes memory sig, string memory res) {

        if(_verify(_msgSender(), listingData)){
            ...
            if(!isValid){res = "INVALID_LISTING_TYPE";}
        }else{res = "INVALID_SELLER_SIGNATURE";}
    ...
    /// @param data must be provided with marketplace signature as following:
    /**
    *       bytes data: {
    *        bytes message: {
    *          bool isActiveBid,
    ...
    *          bytes activeBid: {
    *            bytes msg,
    *            bytes activeBidderSignature,
    *          },
    *          bytes proposedBid: {
    *            bytes msg,
    *            bytes proposedBidderSignature,
    *          }
    *        },
    *        bytes marketplaceSignature
    *       },
    */
    ...
    returns(bool isValid, string memory res, Bid memory validatedBid){
        ...
        res = "INCORRECT_PROPOSAL_TYPE";
        if(auction.depositType == 1){
            res = "BIDDER_IS_NOT_SIGNER";
            if(_verify(_msgSender(), proposedBidData)){
                if(!isActiveBid){
                    (isValid, proposedBid.listingEndTime, res) = _validateAsFirstBid(listingData, proposedBid);
                }else{
                    (isValid, proposedBid.listingEndTime, res) = _validateAsLastBid(listingData, activeBid, proposedBid);
                }
                uint bidderTotalBalance = swaprWallet.getTotalBalance(_msgSender(), auction.acceptedToken);
                res = "LOWER_BALANCE_THAN_BID_OFFER";
    ...
    /// @param data must be Order type provided with marketplace signature as following:
    /**  
    *    bytes data: {
    *       bytes listingData: {
    *           bytes message,
    ...
    */
    ...
    /// @return isSplit if true the following return Order type must be updated in DB
    /// @return newOrder new Order type data contains info regarding split for future reference
    function buyNowOrder(uint buyerPurchasedAmount, bytes memory data, uint split) external onlyMarketplace(data) returns(bool isSplit, Order memory newOrder) {
        ...
        address buyer = _msgSender();
        require(buyerPurchasedAmount < order.maxBuyPerWallet, "ERROR: PURCHASE_LIMIT_EXCEEDED");
        ...
        require(payNow(payOps), "ERROR: PAYMENT_FAILED");
        ...
        if(split < order.remainingPart){
            ...
            splitParts[1] = split;
            addresses[1] = buyer;
            uint[] memory newIDs = ILock(order.lock).split(order.nftId, splitParts, addresses);
            order.nftId = newIDs[0];
            order.remainingPart = splitParts[0];
            if(order.remainingPart > 0){
                isSplit = true;
    ...
    /// @dev automatically detects if buyer have wallet deposits or provoke for approval
    /// @dev you can check the deposits by getBalance() or else get approval for funds
    /// @param data must be Auction & Bid type provided with marketplace signature as following:
    /**  
    *    bytes data: {
    *       bytes message: {
    *           bytes listingData: {
    ...
    */
    function buyNowAuction(bytes memory data) external onlyMarketplace(data) {
        ...
        require(auction.depositType == 1, "INVALID_DEPOSIT_TYPE");
        require(block.timestamp >= bid.listingEndTime, "ERROR: AUCTION_ENDED");
        require(auction.buyNowPrice > bid.offerPrice, "ERROR: HIGHER_BID_PLACED");
    ...
    */
    ...
    /// @return res the reason of failure incase claim fails
    function claim(bytes calldata data) external onlyMarketplace(data) returns(bool success, string memory res) {
        ...
        address claimant = _msgSender();
        (success, res) = _validateClaim(claimant, listingData, lastBid);
        ...
        if(success){
            ...
            require(payNow(payOps), "ERROR: PAYMENT_FAILED");
                ...
                success = false;
                res = "CLAIM_DIRECTION_NOT_IDENTIFIED";
            }
        }
    ...
    *       }
    */
    /// @param lastBid must be provided with bidders signature as following:
    /**
    *       bytes lastBid: {
    *           bytes message,
    *           bytes bidderSignature,
    ...
    */
    /// @return isValid true if succeeds
    /// @return res reason of failure
    function isClaimable(bytes memory listingData, bytes memory lastBid) external view returns(bool isValid, string memory res) {
        (isValid, res) = _validateClaim(_msgSender(), listingData, lastBid);
    }
    ...
    */
    ...
    function _validateClaim(address claimant, bytes memory listingData, bytes memory bidData) internal view returns(bool isValid, string memory res) {
        ...
        res = "INVALID_LISTING_SIG";
        if(_verify(auction.seller, abi.encode(listingInfo, swaprWallet.getNFT(auction.lock, auction.nftId)))){
            res = "INVALID_BID_SIG";
            if(_verify(lastBid.bidder, bidData)){
                res = "INVALID_BID_REF_SIG";
                if(_verify(auction.seller, abi.encode(listingInfo, lastBid.listingSig))){
                    res = "AUCTION_NOT_ENDED";
                    if(block.timestamp >= lastBid.listingEndTime){
                        res = "";
                        if(_verify(claimant, listingData)){
                            //for seller
                            uint bidderTotalBalance = swaprWallet.getTotalBalance(lastBid.bidder, auction.acceptedToken);
                            if(bidderTotalBalance - lastBid.lockedBalance >= lastBid.offerPrice){
                                isValid = true;
                            }else{
                                res = "BIDDER_LOWER_BALANCE_THAN_OFR";
                            }
    ...
    *       }
    */
    function withdraw(bytes memory data) external onlyMarketplace(data) {
    ...
    /// @param token address of ERC20 token or address(0) incase of Native
    ...
    returns(
        ...
        if(swaprWallet.isNFTLocked(auction.lock, auction.nftId)){
            ...
            if(auction.depositType == 1){
                err = "SELLER_IS_ZERO_ADDR";
                if(auction.seller != address(0)){
                    err = "ZERO_STARTING_PRICE";
                    if(auction.startingPrice > 0){
                        err = "BUY_NOW_PRICE_IS_LOWER";
    ...
    returns(
        ...
        res = "INCORRECT_MAX_TOKEN_SELL_OR_MAX_BUY_WALLET";
        if(
    ...
    returns(
        ...
        if(_verify(auction.seller, abi.encode(listingInfo,bid.listingSig))){
            res = "AUCTION_CLOSED";
    ...
    returns(
        ...
        res = "SIG_CROSS_CHECK_FAILED";
        if(_verify(auction.seller, abi.encode(listingInfo,activeBid.listingSig))){
            ...
            if(_verify(activeBid.bidder, activeBidData)){
                ...
                if(activeBid.listingEndTime >= auction.endTime){
                    ...
                    if(activeBid.listingEndTime > block.timestamp){
                        if(activeBid.listingEndTime - block.timestamp <= 60){
                            proposedEndTime = activeBid.listingEndTime + timeOffset;
                        }else{
                            proposedEndTime = activeBid.listingEndTime;
                        }
                        res = "OFFER_LOWER_THAN_START_PRICE";
    ...
    function _verify(address signer, bytes memory _data) internal view returns(bool) {
        ...
        return SignatureCheckerUpgradeable.isValidSignatureNow(signer, _toEthSignedMessageHash(_toMessageHash(message)), signature);
    }
    function _toMessageHash(bytes memory _message) internal pure returns(bytes32) {
        return keccak256(abi.encodePacked(_message));
    }
    function _toEthSignedMessageHash(bytes32 _messageHash) internal pure returns(bytes32) {
        return keccak256(
            abi.encodePacked(
...
contract SwaprWallet is BaseGovernanceUpgradeable, ERC721HolderUpgradeable {
    ...
    mapping(address => mapping(uint => address)) private nftBank;//for on-chain nft deposits
    mapping(address => mapping(uint => bytes)) private lockedNFTs;// for sig based nft deposits
    mapping(address => mapping(uint => address)) private lockedNFTsOC;//locked on-chain nfts

    mapping(address => mapping(address => uint)) private ercBank;//erc deposits
    mapping(address => mapping(address => uint)) private ercLocked;//locked erc funds
    
    mapping(address => uint) private nativeBank;//native deposits
    mapping(address => uint) private nativeLocked;//locked native funds
    ...
    modifier onlySwapr() {
        require(hasRole(SWAPR_ROLE, _msgSender()) || hasRole(SWAPRGL_ROLE, _msgSender()), "ERROR: ONLY_SWAPR_ROLE");
        _;
    }
    ...
    function setSwaprGLRole(address swaprGLAddress) external onlyAdmin {
        //requires a decision in case
        _setupRole(SWAPRGL_ROLE, swaprGLAddress);
    }

    /// @notice performs an upfront payment from buyer to seller for Native/ERC20 tokens
    /// @dev can use if payment to externally owned accounts required by seller
    /// @param acceptedToken the token which seller want to accept payment with or address(0) in case of ETH
    /// @param from is the buyer who wants to pay
    /// @param to must be the seller address
    /// @param amount amount to be paid in 1e18 / or according to token specs
    /**
    *       bytes listingData: {
    *           bytes message,
    *           bytes sellerSignature,
    ...
    */
    ...
    /// @notice internal implementation for eth transfer from wallet contract
    function _payUpfront(address to, uint amount) internal returns(bool) {
        payable(to).transfer(amount);
        return true;
    }
    
    /// @notice internal implementation for ERC20 transfer from wallet contract's approved amount
    ...
    /// @notice locks the NFT within wallet for on-chain listing by owner address
    /// @dev if NFT not in wallet provokes to deposit
    /// @param lock lock proxy address
    /// @param nftId tokenId within the lock
    /// @param owner address of the owner to verify the ownership
    ...
    /// @notice swaps the NFT between two parties
    /// @dev swap only occurs within SwaprWallet
    /// @param lock lock proxy address
    /// @param nftId tokenId within the lock
    /// @param owner address of the owner to verify the ownership
    ...
    /// @notice removes the NFT record from within SwaprWallet
    /// @dev only for Swapr address based deposits
    /// @param lock lock proxy address
    /// @param nftId tokenId within the lock
    ...
    /// @notice deposits native ETH funds only within swapr wallet
    function depositNative(address depositor, uint amount) external onlySwapr {
    ...
    /// @notice locks native ETH funds only within swapr wallet
    ...
    /// @notice unlocks native ETH funds only within swapr wallet
    ...
    function releaseNative(address receiver, uint amount) external onlySwapr {
    ...
    /// @notice locks ERC20 funds only within swapr wallet
    function lockERC(address token, address owner, uint amount) external onlySwapr {
        require(ercBank[owner][token] - ercLocked[owner][token] >= amount, "ERROR: LOW_ERC_RESERVE");
        ercLocked[owner][token] += amount;
    }

    /// @notice unlocks ERC20 funds only within swapr wallet
    ...
    function releaseERC(address token, address receiver, uint amount) external onlySwapr {
    ...
    /// @notice get total balance ETH / ERC20
    /// @dev total balance includes the locked balance as well
    /// @param owner address of the holder
    /// @param token address(0) means ETH balance
    /// @return balance total funds in wallet
    ...
    function getBalance(address owner, address token) external view returns(uint balance) {
        if(token == address(0)){
            balance = _getNativeBalance(owner) - _getNativeLockedBalance(owner);
        }else{
            balance = _getErcBalance(owner, token) - _getErcLockedBalance(owner, token);
        }
    }

    /// @notice get locked balance only for ETH / ERC20
    /// @dev balance excludes the usable amount
    /// @param owner address of the holder
    /// @param token address(0) means ETH balance
    /// @return balance funds that are reserved for some transaction
    function getLockedBalance(address owner, address token) external view returns(uint balance) {
        if(token == address(0)){
