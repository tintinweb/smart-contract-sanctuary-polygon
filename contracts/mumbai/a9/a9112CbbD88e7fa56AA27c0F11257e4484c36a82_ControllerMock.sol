...
contract Controller is IController {
  ...
  event AddVault(uint256 vaultNumber, string vaultName);
  event AddProtocol(
    string name,
    uint256 vaultNumber,
    address provider,
  ...
  */
  ...
  function addVault(address _vault) external onlyDao {
    ...
    emit AddVault(IVault(_vault).vaultNumber(), IVault(_vault).name());
  }
...
contract ControllerMock is Controller {
  constructor(address _dao) Controller(_dao) {}

  function addVaultTest(uint256 _vaultNumber, string memory _name) external {
    emit AddVault(_vaultNumber, _name);
  }
