...
contract Randomness is VRFConsumerBase {
    ...
    //bonusRound => randomNumber
    //mapping (uint => uint) public randomNumber;
    //requestIds => bounsRound
        ...
        ) 
    {
        ...
        fee = 5 * 10 ** 14; // 0.0005 LINK
    }

    /** 
     * 得到随机数,不用自己提供种子，种子为hash生成
     */
    function getRandom() internal {
        //一轮只能获取一个随机数 => 测试可屏蔽
        //require(randomNumber[bonusRound] == 0, "Already-found-random");

        //将管理者的Link币转给这个合约,这个合约获取随机数
        //LINK.transferFrom(governanceContract.getAdmin(), address(this), fee);
        require(LINK.balanceOf(address(this)) > fee, "Not enough LINK - fill contract with faucet");
        
        //获取并记录requestId,并告知需要获取一个随机数 
