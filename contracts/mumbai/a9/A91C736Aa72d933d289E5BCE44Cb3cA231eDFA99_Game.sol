...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function arrangeMissile(uint256[12] memory positions, uint256 nftId) external payable override nonReentrant whenNotPaused {
        nonZero(positions);
        (uint256 price, uint256[] memory events, UseCache[] memory useCache, uint256[] memory amountAff, uint256 amountToMinimum, uint256[] memory amountNFT) = calPrice(positions, msg.sender, nftId);
        ...
        management.updateFreeMissile(nftId, amountNFT);
        if (msg.value > price) {
    ...
    function calPrice(
        ...
        address account,
        uint256 nftId
    ) public override view returns (uint256 price, uint256[] memory, UseCache[] memory, uint256[] memory, uint256, uint256[] memory) {
        ...
        uint256[] memory amountNFT = new uint256[](countTypeMissile + 1);
        uint256 value;
        for (uint256 i = 0; i < 12; i++) {
            ...
            if (!free && missileValues[p] != 0) {
                if (freeAffiliate[a][p] > amountAff[p]) {
                    amountAff[p] += 1;
                } else if (management.getOwnerOf(id) == a && id < management.freeMints() && management.freeInfos(0, p) > amountNFT[p] + management.freeUsed(id, p)) {
                    amountNFT[p] += 1;
                } else {
        ...
        if (price >= minimumAmount) {
            return (price, events, useCache, amountAff, 0, amountNFT);
        }    
        return (price, events, useCache, amountAff, minimumAmount - price, amountNFT);
    }
