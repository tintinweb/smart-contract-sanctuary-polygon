...
contract MustasheAPS {
    ...
    uint256 constant private MAX_RECENT_GAMES = 10;
    uint256 private recentGamesCount;
    uint256[MAX_RECENT_GAMES] private recentGameIds;
    uint256 private tokensSentToDead;
    ...
    IERC20 public token;
        ...
        function endGame(uint256 _gameId) private {
            ...
            recentGameIds[recentGamesCount % MAX_RECENT_GAMES] = _gameId;
            recentGamesCount++;
            tokensSentToDead += deadReward;
        }

        function getTokensSentToDead() external view returns (uint256) {
            return tokensSentToDead / 10**18;
        }
        ...
        function getRecentWinners(uint256 _count)
            external
            view
            returns (address[] memory, address[] memory)
        {
            uint256 endedGamesCount = 0;
            address[] memory winners = new address[](_count);
            address[] memory losers = new address[](_count);

            for (uint256 i = gameId; i >= 1 && endedGamesCount < _count; i--) {
                Game storage game = games[i];
                if (game.gameEnded || game.forcedEnd) {
                    address winner;
                    address loser;
                    ...
                    winners[endedGamesCount] = winner;
                    losers[endedGamesCount] = loser;
                    endedGamesCount++;
                }
            }
