...
contract SportsPrediction is Ownable, Pausable, ReentrancyGuard {
    ...
    function _safeEndMatch(
        ...
        require(
            block.timestamp <= matches[fixture].closeTimestamp + bufferSeconds,
            "Can only end match within bufferSeconds"
        );
        Match storage mm = matches[fixture];
        mm.matchResult = matchResult;
        mm.finished = finished;
        mm.closeTimestamp = block.timestamp;

        emit EndMatch(fixture, matchResult, finished);
    }
    ...
    function setFixtureResult(uint256 fixture, uint8 result, bool finished) public onlyOracle {
        _safeEndMatch(fixture, result, finished);
    }
