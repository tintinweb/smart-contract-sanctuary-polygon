...
contract Lottery is VRFConsumerBaseV2{
    ...
    constructor(uint _ticketPrice, address _lotteryToken, uint8 _winnersShare, uint8 _costAndDevShare, uint8 _burningShare, uint64 _s_subscriptionId)
    ...
    function changeConfig(uint _ticketPrice, address _lotteryToken, address _burningAddress, uint8 _winnersShare, uint8 _costAndDevShare, uint8 _burningShare) public{
        requireManager();
        require(lotteryState == state.Closed);
        require(_winnersShare + _costAndDevShare + _burningShare == 100, "Share sum must be 100");
        require(_ticketPrice < 100000000000000000000000001, "Ticket price can't be over 100m"); //to prevent overflow (10k tickets max. at once)
    ...
    function distributePrize(uint randomNumber) private {
        address winner;
    ...
    function changeVrfGasLimit(uint32 _callbackGasLimit) external {
        requireManager();
        callbackGasLimit = _callbackGasLimit;
    }

    function changeChainlinkSubscriptionId(uint64 _s_subscriptionId) external {
        requireManager();
        s_subscriptionId = _s_subscriptionId;
    }
}