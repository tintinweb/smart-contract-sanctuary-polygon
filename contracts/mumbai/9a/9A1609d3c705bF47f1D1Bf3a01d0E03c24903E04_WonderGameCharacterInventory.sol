...
contract WonderGameCharacterInventory is ERC721, WonderlandAccessControl {
    ...
    function _setIPFSBaseURI(string memory _ipfsBase)
    internal
    {
        ipfsBaseUri = _ipfsBase;
    }

    function setIPFSBaseUri(string memory _ipfsBase)
    public
    onlyOwner
    {
        emit IPFSBaseURIUpdated(
            ipfsBaseUri,
            _ipfsBase
        );
        _setIPFSBaseURI(_ipfsBase);
    }


    function ipfsTokenURI(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        string memory _baseURI_ = ipfsBaseUri;
        return bytes(_baseURI_).length > 0 ? string(abi.encodePacked(_baseURI_, tokenId.toString())) : "";
    }

    function burn(uint256 _tokenId) public whenBurnNotPaused {
        require(ownerOf(_tokenId) == msg.sender, "Only token owner can burn token");
        _burn(_tokenId);
        totalSupply = totalSupply - 1;
    }
...
abstract contract WonderlandAccessControl is AccessControl, Ownable {
    bytes32 internal constant MINTER_ROLE =
        0x9f2df0fed2c77648de5860a4cc508cd0818c85b8b8a1ab4ceeef8d981c8956a6; // keccak256(abi.encodePacked("MINTER_ROLE"));
    ...
    modifier whenBurnNotPaused() {
        require(!isBurnPaused, "Wonder Game Character Burn is paused");
        _;
    }
