...
contract PassiveIncomeNFTSwap is Ownable {
    ...
    function swap(uint256 tokenId, uint8 lockDurationInMonths, bool onlyLock) external {
        ...
        if (amount > 0) {
            ...
            passiveIncomeNFT.mint(msg.sender, amount, lockDurationInMonths, onlyLock);
        }
    }

    function swapGURU(uint256 amountIn, uint8 lockDurationInMonths, bool onlyLock) external {
...
contract TangibleTokenSwap is AccessControl {
    ...
    function swap(uint256 amount) external onlyRole(SWAPPER_ROLE) returns (uint256 amountOut) {
