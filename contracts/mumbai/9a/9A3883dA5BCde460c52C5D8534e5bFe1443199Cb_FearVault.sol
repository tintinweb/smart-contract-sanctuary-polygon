...
contract FearVault is AccessControl, Pausable, Withdrawable, ReentrancyGuard {
    ...
    struct Deposit {
        ...
        uint256 depositUnlockDate;  // date of deposit unlocking (deposit date + period)
        uint256 rewardUnlockDate;   // date of reward unlocking (deposit date + period + 6 months)
        bool editable;              // flag showing if this record was added and can be edited by admin (for gift deposits)
    }


    /**
     * @dev Fee percentage for instant withdrawing (in 1/100 of percent, default is 50%)
     */
    uint256 public instantWithdrawFeePercentage = 5000;
    ...
    uint256 constant private extraRewardLock = 180 days;
    ...
    constructor(address fearContractAddress) {
        ...
        depositSettings[1] = DepositSettings(1, 2000, 50000 * 10 ** 18, 1000000 * 10 ** 18);    // 1y - 20% apr, max 50k per wallet, total max 1M
        depositSettings[2] = DepositSettings(2, 3000, 40000 * 10 ** 18, 800000 * 10 ** 18);     // 2y - 30% apr, max 40k per wallet, total max 800k
        depositSettings[3] = DepositSettings(3, 4000, 30000 * 10 ** 18, 600000 * 10 ** 18);     // 3y - 40% apr, max 30k per wallet, total max 600k
        depositSettings[4] = DepositSettings(4, 5000, 20000 * 10 ** 18, 400000 * 10 ** 18);     // 4y - 50% apr, max 20k per wallet, total max 400k
    }
    ...
    function _getUserRewardAmount(address user) private view returns (uint256) {
        ...
        for (uint i = 0; i < userDeposits.length; i++)
            totalAmount = totalAmount.add(userDeposits[i].reward);
    ...
    function _getUserWithdrawableAmount(address user) private view returns (uint256) {
        ...
        for (uint i = 0; i < userDeposits.length; i++)
            if (userDeposits[i].depositUnlockDate <= block.timestamp)
                totalAmount = totalAmount.add(userDeposits[i].amount);
    ...
    function _getUserClaimableAmount(address user) private view returns (uint256) {
        ...
        for (uint i = 0; i < userDeposits.length; i++)
            if (userDeposits[i].rewardUnlockDate <= block.timestamp)
                totalAmount = totalAmount.add(userDeposits[i].reward);
    ...
    /** 
     * @dev Returns caller's reward amount for all deposited FEAR.
     */
    ...
    function deposit(uint256 period, uint256 amount) external whenNotPaused nonReentrant {
        ...
        require(_getUserDepositAmount(user).add(amount) <= settings.walletLimit, "Amount exceeds wallet limit for period");
        ...
        nextDepositId++;
        Deposit memory newDeposit = Deposit(nextDepositId, period, amount, reward, curTimestamp, depositUnlock, depositUnlock + extraRewardLock, false);
    ...
    function withdraw() external whenNotPaused nonReentrant {
        ...
        for (uint i = 0; i < deposits[user].length; i++) {
            ...
            if (iDeposit.amount > 0 && iDeposit.depositUnlockDate >= curTimestamp) {
     ...
     */
    function instantWithdraw(uint256 id) external whenNotPaused nonReentrant {
        address user = _msgSender();
    ...
    function claimReward() external whenNotPaused nonReentrant {
        ...
        for (uint i = 0; i < deposits[user].length; i++) {
            ...
            if (iDeposit.reward > 0 && iDeposit.rewardUnlockDate >= curTimestamp) {
                totalReward = totalReward.add(iDeposit.reward);
                iDeposit.reward = 0;
            }
        }
    ...
    function getFearDepositors() external view onlyRole(MANAGER_ROLE) returns (address[] memory) {
        address[] memory lockers = new address[](fearDepositors.length());

        for (uint i = 0; i < lockers.length; i++)
            lockers[i] = fearDepositors.at(i);
    ...
    function createGiftDeposit(address user, uint256 amount, uint256 unlockDate) external onlyRole(ADMIN_ROLE) {
        ...
        nextDepositId++;
        Deposit memory newDeposit = Deposit(nextDepositId, 0, amount, 0, curTimestamp, unlockDate, unlockDate, true);
    ...
    function editGiftDeposit(uint256 id, address user, uint256 amount, uint256 unlockDate) external onlyRole(ADMIN_ROLE) {
        address admin = _msgSender();
        ...
        uint256 curAmount = deposits[user][index].amount;
        ...
        deposits[user][index].amount = amount;
        deposits[user][index].depositUnlockDate = unlockDate;
    }
}