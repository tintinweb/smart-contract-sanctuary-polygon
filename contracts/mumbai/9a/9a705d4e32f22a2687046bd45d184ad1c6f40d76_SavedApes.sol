...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    bool public HiddenMetadata = true;
    ...
    */
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
      ...
      return bytes(currentBaseURI).length > 0 ? string(abi.encodePacked(currentBaseURI, tokenId.toString(), baseURISuffix)) : "";
    }
...
error MaxBuyTx();
error WrongAmount();
error FreeMintPaused();
error MaxSupplyReached();
error WithdrawalFailed();
error MintFromContractNotAllowed();

contract SavedApes is ERC721r, Ownable {

    bool public mintPaused = true;
    bool public freeMintPaused = true;

    uint256 private constant maxBuyTx = 5;
    uint256 private constant price = 0.01 ether;

    constructor () ERC721r("Saved Apes", "SAPS") { 
    }

    function NewBaseURI(string calldata _newbaseURI) external onlyOwner  {
    baseURI = _newbaseURI;
    }
    ...
    function setMetadataRevealed() external  onlyOwner {
    HiddenMetadata = !HiddenMetadata;
    }
 
    function isMinting() external onlyOwner  {
    mintPaused = !mintPaused;
    }

    function isFreeMint() external onlyOwner  {
    freeMintPaused = !freeMintPaused;
    }

    function mint(uint256 quantity) external payable  {
        if (mintPaused) revert MintingPaused();
        if (freeMintPaused) revert FreeMintPaused();
        if (quantity > maxBuyTx) revert MaxBuyTx();
        ...
        if (payForCount > 0) 
        {
        if (msg.value < payForCount * price)
        revert WrongAmount();
        }
    ...
    function reserve() external onlyOwner {
        ...
        for (uint i = 0; i < 50; i++) {
            _mintAtIndex(msg.sender, supply + i);
        }
    }
