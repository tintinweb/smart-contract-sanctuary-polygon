...
import "./lib/ActiniumMarketOrderStruct.sol";
import "./lib/ActiniumMarketManagement.sol";

contract ActiniumExchange is ActiniumMarketManagement {
    /*
        SetUp Parameter
    */
    ...
    */
    function _checkApproveCurrency(
        ...
        require(tokenContract != address(0), "TokenContract is zero");
        require(sender != address(0), "Sender is zero");

        uint256 allowanceTokenQuantity = IERC20(tokenContract).allowance(
            sender,
            address(this)
        );

        require(
            allowanceTokenQuantity >= requestTokenQuantity,
            "Transfer not approved by token owner"
        );
    }
    ...
    function _checkApproveItem(
        ...
        require(itemContract != address(0), "ItemContract is zero");
        require(userAddress != address(0), "UserAddress is zero");

        bool result = false;
        // check if itemContract supports ERC721 or ERC1155
        if (IERC165(itemContract).supportsInterface(IERC721_IID)) {
            // ERC721
            result = IERC721(itemContract).isApprovedForAll(
                userAddress,
                address(this)
            );
        } else if (IERC165(itemContract).supportsInterface(IERC1155_IID)) {
            // ERC1155
            result = IERC1155(itemContract).isApprovedForAll(
                userAddress,
                address(this)
            );
        } else {
        ...
        require(result, "Transfer not approved by Item owner");
    }

    function _checkBalanceToken(
        address tokenContract,
        address userAddress,
        uint256 requestTokenQuantity
    ) internal view {
        ...
        uint256 balanceTokenQuantity = IERC20(tokenContract).balanceOf(
            userAddress
        );

        require(
            balanceTokenQuantity >= requestTokenQuantity,
    ...
    function _checkOwnedItem(
        ...
        string memory errMsg = "Item balance is not enough";

        // check if itemContract supports ERC721 or ERC1155
        if (IERC165(itemContract).supportsInterface(IERC721_IID)) {
            // ERC721
            require(
                IERC721(itemContract).ownerOf(itemTokenId) == userAddress,
                errMsg
            );
        } else if (IERC165(itemContract).supportsInterface(IERC1155_IID)) {
            // ERC1155
            require(
                IERC1155(itemContract).balanceOf(userAddress, itemTokenId) >=
                quantity,
                errMsg
            );
        }
    ...
    function _transferItem(
        ...
        if (IERC165(itemContract).supportsInterface(IERC721_IID)) {
            // ERC721
            IERC721(itemContract).safeTransferFrom(sender, receiver, itemId);
        } else if (IERC165(itemContract).supportsInterface(IERC1155_IID)) {
            // ERC1155
            ...
            revert("Not supported item contract");
        }
    }
    ...
    function _transferMarketFee(
        ...
        require(currencyContract != address(0), "TokenContract is zero");
        require(sender != address(0), "Sender is zero");

        // Transfer tokens from sender to receiver.
        IERC20(currencyContract).safeTransferFrom(
            sender,
            address(this),
            requestQuantity
        );
    ...
    function _checkSellable(
        ...
        _checkOwnedItem(item, sellerAddress, itemId, quantity);
        _checkApproveItem(item, sellerAddress);
    }
    ...
    function _checkSpendable(
        ...
        _checkBalanceToken(currencyContract, buyerAddress, totalPrice);
    ...
    function _makeNewOrder(
        ...
        // TODO set Market Fee dynamic
        uint256 unitMarketFee = calculateMarketFee(unitPrice, 30);
    ...
    function _getExistingOrder(
        ...
        require(order.trader != address(0), "Order is not exist");
        return order;
    }
    ...
    function _checkOrderAcceptable(
        ...
        require(order.expirationTime > block.timestamp, "Order is expired");
        //        require(order.orderSide == OrderSide.SELL, "OrderSide is not SELL");
        require(sellerAddress != buyerAddress, "Trader is same as order maker");
        require(
            order.quantity >= requestOrderQuantity,
            "Available listed quantity is less than request quantity"
        );
        require(order.isFullfilled == false, "Order is fullfilled");
        require(order.isCanceled == false, "Order is canceled");
    }
    ...
    */
    function calculateMarketFee(
        ...
        // get feequantity using original fee quantity and matched quantity
        // e.g., feeRate 1 = 0.01% = 0.0001
        uint256 marketFee = (price * feeRate) / 10000;
        return marketFee;
    }

    function createListedOrder(
        uint256 expirationTime,
        address currencyContract,
        address item,
        uint256 itemId,
        uint256 unitPrice,
        uint256 quantity
    ) external checkSupportedCurrency(currencyContract) checkBlackList(msg.sender) checkSupportedItemContract(item) {
        address sellerAddress = msg.sender;

        // Check seller owned item, and transfer approved
        _checkSellable(sellerAddress, item, itemId, quantity);
        ...
        require(currencyContract != address(0), "Currency contract address is zero");
        require(item != address(0), "Item address is zero");
        require(itemId > 0, "Item id must be greater than zero");
    ...
    function createOfferOrder(
        ...
        uint256 unitPrice,
        uint256 quantity
    ) external checkSupportedCurrency(currencyContract) checkBlackList(msg.sender) checkSupportedItemContract(itemContract) {
        ...
        require(currencyContract != address(0), "Currency Contract address is zero");
        require(itemContract != address(0), "Item contract address is zero");
        require(itemId > 0, "Item id must be greater than zero");
        require(quantity > 0, "Quantity must be greater than zero");
    ...
    function acceptListedOrder(
        ...
        (
        uint256 currentOrderTotalPrice,
        uint256 currentMarketFee
        ) = _calculateTotalPriceAndFee(
            order.price,
            order.marketFee,
            requestOrderQuantity
        );
        ...
        _transferToken(
            ...
            buyerAddress,
            sellerAddress,
            currentOrderTotalPrice - currentMarketFee
        );
        // ERC20: buyer -> seller
    ...
    function acceptOfferOrder(
        ...
        require(order.orderSide == OrderSide.BUY, "OrderSide is not BUY");
        require(order.isCanceled == false, "Order is canceled");
    ...
    function updateListedOrder(
        address newCurrencyContract,
        uint256 targetOrderId,
        uint256 updateTotalQuantity,
        uint256 updateUnitPrice
    ) external checkSupportedCurrency(newCurrencyContract) checkBlackList(msg.sender) {
        require(
            updateTotalQuantity > 0 && updateUnitPrice > 0,
            "Update quantity and unit price is zero"
        );
        ...
        // Check Order Object
        require(
            sellerAddress == order.trader,
            "Only order trader can update order"
        );
        require(order.expirationTime > block.timestamp, "Order is expired");
        require(order.orderSide == OrderSide.SELL, "OrderSide is not SELL");
        require(order.isFullfilled == false, "Order is fullfilled");
        ...
        order.currencyContract = newCurrencyContract;
        order.quantity = updateTotalQuantity;
        order.price = updateUnitPrice;

        // Recalculate unit market fee
        // TODO: Market Fee Rate Policy, Current 0.3% = 30
        order.marketFee = calculateMarketFee(order.price, 30);
    ...
    function updateOfferOrder(
        ...
        _checkSpendable(buyerAddress, newCurrencyContract, updateTotalPrice);
    ...
    function cancelOrder(uint256 targetOrderId) external checkBlackList(msg.sender) {
        ...
        // Check Order Object
        require(
            traderAddress == order.trader || msg.sender == owner(),
            "Only the manager or the owner of the order can cancel"
        );
        require(order.expirationTime > block.timestamp, "Order is expired");
        require(order.isFullfilled == false, "Order is already fullfilled");
        require(order.isCanceled == false, "Order is already canceled");
    ...
    returns (uint256)
    {
        require(listOrderId.length == listQuantity.length, "Invalid input");

        uint256 totalPrice = 0;
        for (uint256 i = 0; i < listOrderId.length; i++) {
            MarketOrder memory order = _getExistingOrder(listOrderId[i]);
            require(order.currencyContract == currencyContract, "Invalid Currency Contract");
            totalPrice += order.price * listQuantity[i];
        }
        return totalPrice;
    }

    function acceptBulkListedOrder(
        address currencyContract,
        uint256[] memory listOrderId,
        uint256[] memory listQuantity
    ) external checkSupportedCurrency(currencyContract) checkBlackList(msg.sender){
        require(listOrderId.length == listQuantity.length, "Invalid input");
        ...
        for (uint256 i = 0; i < listOrderId.length; i++) {
            acceptListedOrder(listOrderId[i], listQuantity[i]);
        }
    }
}
...
import "./ActiniumMarketRole.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";
import "@openzeppelin/contracts/utils/introspection/IERC165.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/security/Pausable.sol";

contract ActiniumMarketManagement is ActiniumMarketRole {
    using SafeERC20 for IERC20;
    using EnumerableSet for EnumerableSet.AddressSet;
    ...
    modifier checkSupportedCurrency(address _token) {
        require(listCurrency.contains(_token), "Currency is not supported erc20.");
        _;
    }

    modifier checkSupportedItemContract(address _itemContract) {
        require(itemContracts.contains(_itemContract), "Item contract is not supported.");
        _;
    }

    modifier checkBlackList(address trader) {
        require(!blackLists.contains(trader), "Trader is in black list.");
        _;
    }
    ...
    function addSupportedCurrency(address _token) external onlyOwner {
        ...
        if (!listCurrency.contains(_token)) {
    ...
    function addSupportedItemContract(address _itemContract) external onlyOwner {
        ...
        if (!itemContracts.contains(_itemContract)) {
    ...
    function setThorium(address _thorium) external onlyOwner {
        thorium = _thorium;
    }
}
...
struct MarketOrder {
    ...
    // id fields
    uint256 orderId; 
    uint256 itemId; // item's specific ID
...
abstract contract ActiniumMarketRole is AccessControl,Ownable {
    ...
    modifier onlyMarket() {
        require(hasRole(MARKET_ROLE, msg.sender), "Access denied");
        _;
    }
}