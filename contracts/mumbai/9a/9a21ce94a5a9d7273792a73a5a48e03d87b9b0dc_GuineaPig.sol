    ...
    UUPSUpgradeable
{
    ...
    ////                   HELPER FUNCTIONS                    ////
    ///////////////////////////////////////////////////////////////
    function requestRandomNumber(address, uint256 _amount)
        external
        returns (uint256[] memory randomNumbers)
    {
        ++randNonce;
        randomNumbers = new uint256[](_amount);
        randomNumbers[0] = randomNumber(randNonce, 100);
        if (_amount == 2) {
            ++randNonce;
            randomNumbers[0] = randomNumber(randNonce, 2);
        }
    }

    function randomNumber(uint256 _entropy, uint256 _upperBound)
        internal
        pure
        returns (uint256)
    {
