...
contract PolicyDetails_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    mapping(string => bool) private _existPolicy;
    ...
    uint256[] internal percentWeight;
    ...
    event BlacklistAssetsOfUser(address[] CU, address[] user, bool[] boolean);
    event BlacklistOfUser(address[] user, bool[] boolean);
     ...
     *  
     *  Requirements: 
     *   - `msg.sender` Must be SuperManager.
    ...
    /**
     *  @notice Get the percentWeight of the redeemed.
     *  @param _asset  The asset address.
     *  @param _redeemId The redeemId.
     *  @return The array value of the number that is percentWeight.
     */
     ...
     *  @param _asset  The asset address.
     *  @param _redeemId The redeemId.
     *  @return The number value of the amountRequest.
     */
     ...
     */
    function isPolicyActive(string memory _policyId) public view returns(bool isActive) {
         if (
            isBlacklistOfUser(_policyIdData[_policyId].user) == true ||
            isBlacklistAssetsOfUser(_policyIdData[_policyId].user,_policyIdData[_policyId].asset) == true
        ){
            return false;
        } else if (
     ...
     *  @return The number value of the decimals.
     */
     ...
     *  @param _policyId The policyId.
     *  @return The boolean value that policy exist or not.
     */
    function exist(string memory _policyId) public view returns (bool) {
        return _existPolicy[_policyId];
    }
    ...
    function policyData(string memory _policyId)
        ...
        if (
            isBlacklistOfUser(_policyIdData[_policyId].user) == true ||
            isBlacklistAssetsOfUser(
                _policyIdData[_policyId].user,
                _policyIdData[_policyId].asset
            ) == true || _policyIdData[_policyId].isRedeemed == true
        ) {
            return (_policyIdData[_policyId], false, _policyIdData[_policyId].isRedeemed);
        }else if (
            _policyIdData[_policyId].untilDate >= block.timestamp &&
            block.timestamp >= _policyIdData[_policyId].startDate
        ) {
            return (_policyIdData[_policyId], true, _policyIdData[_policyId].isRedeemed);
        } else {
            return (_policyIdData[_policyId], false, _policyIdData[_policyId].isRedeemed);
        }
    }
    ...
    /**
     *  @dev Set the value of percentWeight (fundAllocationWeight).
     *  @param _percentWeight The array value of the number that is percentWeight. 
     * 
     *  Requirements:
     *   - `_percentWeight` Sum of the percentWeight must be 100.
     */
    function _setFundAllocationWeight(uint256[] memory _percentWeight) private {
        uint256 countpercent;

        for (uint256 i = 0; i < _percentWeight.length; i++) {
            countpercent += _percentWeight[i];
        }

        require(countpercent == 100, "Error Counting Percentage");
        percentWeight = _percentWeight;

        emit FundAllocationWeightChanged(_percentWeight);
    }

    /**
     *  @notice Set the value of percentWeight (fundAllocationWeight).
     *  @param _percentWeight The array value of the number that is percentWeight. 
     * 
     *  Requirements:
     *   - `msg.sender` Must be SuperManager.
     *   - `_percentWeight` Sum of the percentWeight must be 100.
     */
    function setFundAllocationWeight(uint256[] memory _percentWeight)
        public
        isSuperManager
    {
        require(_percentWeight.length == 7, "Percent should be have 7 arguments.");

        _setFundAllocationWeight(_percentWeight);
    }

    
    /**
     *  @notice Get percentWeight of the pool.
     *  @return The array value of the number that is percentWeight. 
     */
     ...
     *  @return The array value of the number that is percentWeight. 
     */
    function decodedIssuePolicy(bytes memory _data) internal pure returns(IssuePolicyParams memory) {
        (
     ...
     *   - `msg.sender` Must be Manager.
     *   - `msg.sender` Must approve from riskManager.
     *   - `msg.sender` Must not be blacklisted "user" and "asset".
     *   - `policyId` Must not exist in this pool.
     */
    function issuePolicy(bytes calldata _data) public isReady onlyPolicyManager {

        // _dPD = decodedPolicyData //
        IssuePolicyParams memory _dPD = decodedIssuePolicy(_data);
        
        require(RM.riskCoefficientOfUser(_dPD.user) > 0 && RM.riskCoefficientOfUser(_dPD.user) <= 10,"You didn't approve");
        require(isBlacklistOfUser(_dPD.user) != true,"Blacklist User");
        require(isBlacklistAssetsOfUser(_dPD.user, _dPD.asset) != true, "Blacklist assets of User");

        uint256[] memory mempercent = percentWeightIs();

        require(mempercent.length == 7,"Invaild Length of PercentWeight");
        require(exist(_dPD.policyId) != true, "This policyId exist");
        require(_dPD.pricing[2] == IERC20(_dPD.asset).decimals(), "Invalid Decimals of assets.");

        uint256 decimal = decimals();

        _existPolicy[_dPD.policyId] = true;
        ...
        allPoliciesCoverageValueBeforeCalculate += _dPD.pricing[1] * 10**decimal;
        ...
        if(_dPD.valueSub > 0) {
            ...
            SUB.claimSubsidized(_dPD.user,_dPD.asset,_dPD.pricing[0]*10**_dPD.pricing[2],poolId);

            emit PolicySubsidized(_dPD.orgSub,_dPD.orgSub,_dPD.valueSub,_dPD.percentSub);
        }
    }
     ...
     *   - `_percentRedeem` The number of _percentRedeem must greather than 0 and lessthan or equal to 100.
     *   - `_policyId` The owner of policyId must not be blacklisted.
     *   - `_policyId` The asset of policyId must not be blacklisted.
     *   - `_policyId` The percentWeight of policyId must have length 7.
     */
        ...
        returns (bool, string memory)
    {
        ...
        if (
            R.pendingClaimRequest(_policyId) == 0
        ) {
        ...
        if ((isBlacklistOfUser(_policyIdData[_policyId].user) == true)) {
        ...
        if (
            !((
                _policyIdData[_policyId].percent
            ).length == 7)
        ) {
        ...
        )
    {
        ...
        _amountRequest[_asset][redeemId] = (_policyIdData[_policyId].buyValue * _percentRedeem) / 100;

        _whoIsRequest[_asset][redeemId] = _user;
        _weightRequest[_asset][redeemId] = _policyIdData[_policyId].percent;
     ...
     *  @param _policyId The policyId.
     *  @return The number value of buyValue.
     */
     ...
     */
    function isUserRedeem(string memory _policyId)
        public
        view
        returns (bool)
    {
    ...
    function percentOfUserIs(string memory _policyId)
        ...
        returns (uint256[] memory)
    {
        return _policyIdData[_policyId].percent;
    }
        ...
        isSuperManager
    {
        ...
        emit BlacklistOfUser(_user, _boolean);
    }
    ...
    function blacklistAssetsOfUser(
        ...
        require(
            _user.length == _asset.length &&
                _user.length == _boolean.length &&
                _asset.length == _boolean.length,
            "Invaild Arrays"
        );

        for (uint256 i = 0; i < _user.length; i++) {
            require(_user[i] != address(0), "Invaild Address");
            require(_asset[i] != address(0), "Invaild Address");
            _blacklistUserAssets[_asset[i]][_user[i]] = _boolean[i];
        }

        emit BlacklistAssetsOfUser(_asset, _user, _boolean);
    }
    ...
    function activePoliciesUser(address _user) public view returns(uint256) {
        ...
        for(uint i=0;i<_policy.length;i++){
    ...
    function redeemIdLength() public view returns(uint) {
        return redeemId;
    }
}
...
interface IFactory {
    ...
    function allPairs(uint256 _policyId)
        external
        view
        returns (PairsData memory);
    ...
    function getPair(string memory _poolId)
        external
        view
        returns (address policy, address nameRegistry);
