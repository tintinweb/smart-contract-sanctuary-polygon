...
contract FactoryNFT {
    ...
    event NFTMinted(address owner, address tokenContract,uint tokenID); //emmited when ERC1155 token is minted
    ...
    function mintNFT(uint _indexOfContract,address to,uint tokenId) public {
        ...
        emit NFTMinted(tokens[_indexOfContract].owner(), address(tokens[_indexOfContract]),tokenId);
    }
    ...
    */
    ...
    function setTokenAllowedList(uint256 _index,uint tokenID,address[] memory _allowedList) public   {
        tokens[_index].setTokenAllowedList(tokenID,_allowedList);
    }
...
contract NFT is ERC1155, Ownable {
    ...
    // Create token 
    function createToken(string memory _name, bool _limited, uint128 _limit,bool _allowed, bool _allowedList,bool _allowTransfer,string memory _uri) external  onlyOwner returns(uint256) {
    ...
    function mintTokenItem(address to,uint tokenID) public onlyOwner checkMinting(tokenID) returns(uint256) {       
        _mint(to, tokenID, 1, '');
        tokenIdToToken[tokenID].tokenMinted += 1;
        return tokenID;
    }

    function mintTokenItemOther(address to,uint tokenID) public checkMinting(tokenID) returns(uint256){
        
       
        require(balanceOf(msg.sender,tokenID)==0,"Token can only be minted once");
       // Either the sender can be owner or belong to an allowed address for this token 
        require(tokenIdToToken[tokenID].isAllowed  || ( tokenIdToToken[tokenID].isAllowedList && allowedList[tokenID][msg.sender]) ,"User is not allowed to mint this token");
        _mint(to, tokenID, 1, '');
        tokenIdToToken[tokenID].tokenMinted += 1;
    ...
    function batchMintTokenItem(address[] memory to,uint tokenID) public onlyOwner checkMinting(tokenID)  {
   
        for (uint i = 0; i < to.length; i++) {
           mintTokenItem(to[i],tokenID);
        }
    }
    function batchMintTokenItemOther(address[] memory to,uint tokenID) public  checkMinting(tokenID) {
    ...
    function setTokenUri(string memory _uri,uint256 id) private onlyOwner {
        _tokenUris[id] = _uri; 
    }
    ...
    function setTokenAllowedList(address[] memory _allowedList,uint tokenID) public  onlyOwner {
        for (uint i = 0; i < _allowedList.length; i++) {
        allowedList[tokenID][_allowedList[i]] = true;
        }
    }
