...
contract MarketTieredLeverage is AccessControlledAndUpgradeable {
  ...
  */
  ...
  function getMiaAddress(PoolType poolType, uint256 index) external view returns (address) {
  ...
  function _rebalanceMarketPoolsCalculatePriceAndExecuteBatches(
    ...
    epochInfo.latestExecutedEpochIndex = epochToExecute.associatedEpochIndex;
  ...
  function _updateSystemStateInternal() internal virtual {
    ...
    for (
      ...
      // TODO: this should be handled by the oracle manager better.
      //       additionally, this might be buggy if the epochs don't allign properly after this.
      if (epochToExecute.oraclePrice < 1) {
        return;
      }
  ...
  function _mint(
    ...
    (uint32 epochIndex, bool isInEwtWithActionInPreviousEpoch) = _executePoolOutstandingUserMints(
      user,
      pool,
      poolType
    );
    ...
    // TODO URGENT: handle edge case where existing deposit exists in pool for users and it is currently the EWT: if (isInEwtWithActionInPreviousEpoch) {
  ...
  function _redeem(
    ...
    (uint32 epochIndex, bool isInEwtWithActionInPreviousEpoch) = _executePoolOutstandingUserRedeems(
      user,
      pool,
      poolType
    );
  ...
  function _executePoolOutstandingUserMints(
    ...
    uint256 pool,
    PoolType poolType
  ) internal virtual returns (uint32 epochIndex, bool isInEwtWithActionInPreviousEpoch) {
