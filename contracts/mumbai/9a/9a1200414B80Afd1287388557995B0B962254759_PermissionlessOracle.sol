...
contract PermissionlessOracle is PermissionlessOracleI, AccessControlEnumerable, Pausable {
  ...
  bool public testing;
  uint256 private _updateThreshold;
  ...
  function registerCollections(
    ...
    uint256[] calldata updateThresholds
    ...
    for (uint256 i; i < addresses.length; ++i) {
      collections[addresses[i]] = Collection(
        false,
        twapRanges[i],
        addresses[i],
        updateThresholds[i],
  ...
  function _setUpdateThreshold(uint256 _newThreshold, address _collection) external {
    ...
    emit ThresholdUpdated(_updateThreshold, _newThreshold, _collection);
