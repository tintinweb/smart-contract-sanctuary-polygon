...
contract MutualAccessFactory is BaseContractFactory, Ownable {
    address private _mutualAccessAntic;
    uint256 private _mutualAccessAnticPrimaryFeePercentage;
    uint256 private _mutualAccessAnticSecondaryFeePercentage;
    address[] private _mutualAccessManagers;
    address[] private _mutualAccessMarketplaces;

    struct MutualAccessArguments {
        MutualAccess.TicketTierInput[] tickets;
        MutualAccess.RoyaltyRecipient[] royaltyRecipients;
        MutualAccess.FeePercents royalties;
        uint256 expireAfterHours;
        uint256 ipfsUri;
    ...
    event MutualAccessDeployed(address contractAddress);

    constructor(
        address factoryOwner,
        address mutualAccessAntic,
        uint256 mutualAccessAnticPrimaryFeePercentage,
        uint256 mutualAccessAnticSecondaryFeePercentage,
        address[] memory mutualAccessManagers,
        address[] memory mutualAccessMarketplaces
    ) {
        transferOwnership(factoryOwner);

        _mutualAccessAntic = mutualAccessAntic;
        _mutualAccessAnticPrimaryFeePercentage = mutualAccessAnticPrimaryFeePercentage;
        _mutualAccessAnticSecondaryFeePercentage = mutualAccessAnticSecondaryFeePercentage;
        _mutualAccessManagers = mutualAccessManagers;
        _mutualAccessMarketplaces = mutualAccessMarketplaces;

        // Add this factory as a manager
        // This exist only for deployment, later the role is renounced
        _mutualAccessManagers.push(address(this));
    }
    ...
    {
        // Decode params
        MutualAccessArguments memory args = abi.decode(
            data,
            (MutualAccessArguments)
        );

        address[] memory marketplaces;

        // Allow sale on external marketplaces
        if (args.allowExternalSale) {
            marketplaces = _mutualAccessMarketplaces;
        }

        // Deploy MutualAccess
        MutualAccess mutualAccess = new MutualAccess(
            args.creatorAddress,
            _mutualAccessAntic,
            _mutualAccessAnticPrimaryFeePercentage,
            _mutualAccessAnticSecondaryFeePercentage,
            _mutualAccessManagers,
            _mutualAccessMarketplaces
        );

        // Create a new content
        mutualAccess.addNewContent(
            args.tickets,
            args.royaltyRecipients,
            args.royalties,
            args.expireAfterHours,
            args.ipfsUri,
            args.creatorAddress
        );

        // Renouncing the manager role
        mutualAccess.renounceRole(mutualAccess.MANAGER_ROLE(), address(this));

        emit MutualAccessDeployed(address(mutualAccess));

        return address(mutualAccess);
    }

    function setAnticParam(address antic) external onlyOwner {
        _mutualAccessAntic = antic;
    }

    function setAnticFeesParam(uint256 primary, uint256 secondary)
        external
        onlyOwner
    {
        _mutualAccessAnticPrimaryFeePercentage = primary;
        _mutualAccessAnticSecondaryFeePercentage = secondary;
    }

    function setManagersParam(address[] memory managers) external onlyOwner {
    ...
    function setMarketplacesParam(address[] memory marketplaces)
        ...
        onlyOwner
    {
        _mutualAccessMarketplaces = marketplaces;
    }

    function encode(MutualAccessArguments memory args)
        external
        pure
