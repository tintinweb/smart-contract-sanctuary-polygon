...
import {IListingMarket} from "./IListingMarket.sol";
import {IColleCollection} from "../collections/IColleCollection.sol";
import {IEscrow} from "../escrow/IEscrow.sol";

/// @title ListingMarketUpgradeable
/// @notice Contract that handles the listing and buying process of tokens on a marketplace.
/// @dev Inherits from BaseMarketUpgradeable. It introduces functions to list, delist and buy tokens, as well as their permit alternatives that use EIP-712 signatures.
contract ListingMarketUpgradeable is IListingMarket, BaseMarketUpgradeable {
    /// @dev Nested mapping from a collection address to token ID to ERC20 address to a price. It tracks the prices of listed tokens.
    /// @dev First address represents the collection, second the token id and the third is the address of the ERC20 token used as currency.
    mapping(address => mapping(uint256 => mapping(address => uint256))) internal listings;
    ...
    /**
     * @dev Allows the caller to list a token for sale.
    ...
    /**
     * @dev Allows a trusted contract to list a token for sale on behalf of a seller.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be listed for sale.
     * @param _seller The address of the seller.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
     * @param _price The price for which the token is listed.
     * @param _deadline The time at which the permission expires.
     * @param _signature The seller's signature, granting permission to list the token.
     */
    function permitList(
        address _collection,
        uint256 _tokenId,
    ...
    /**
     * @dev Allows the caller to delist a token.
     * @param _collection The address of the collection contract to which the token belongs.
    ...
    /**
     * @dev Allows a trusted contract to delist a token on behalf of a seller.
     * @param _seller The address of the seller.
     * @param _collection The address of the collection contract from which the token is to be delisted.
     * @param _tokenId The ID of the token to be delisted.
     * @param _erc20 The address of the ERC20 token that was to be used as the payment currency.
     * @param _deadline The time at which the permission expires.
     * @param _signature The seller's signature, granting permission to delist the token.
     */
    function permitDelist(
        address _seller,
        address _collection,
    ...
    /**
     * @dev Allows the caller to buy a token.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be bought.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
     * @param _payNow Specifies if payment will be made now or later.
     */
    function buy(address _collection, uint256 _tokenId, address _erc20, bool _payNow) public virtual {
        _buy(msg.sender, msg.sender, _collection, _tokenId, _erc20, _payNow);
    }

    /**
     * @dev Allows a trusted contract to buy a token on behalf of a buyer.
     ...
     * @param _payNow Specifies if payment will be made now or later.
     * @param _deadline The time at which the permission expires.
     * @param _signature The buyer's signature, granting permission to buy the token.
     */
    function permitBuy(
        address _buyer,
        address _collection,
    ...
    /**
     * @dev Allows a trusted contract to buy a token on behalf of a buyer using off-chain payment.
     * @param _buyer The address of the buyer.
     * @param _collection The address of the collection contract from which the token is to be bought.
     * @param _tokenId The ID of the token to be bought.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
     * @param _deadline The time at which the permission expires.
     * @param _signature The buyer's signature, granting permission to buy the token with off-chain payment.
     */
    function permitOffChainPaymentBuy(
        address _buyer,
        address _collection,
    ...
    function handleSaleClosed(uint256 _saleId) external virtual override onlyMarketHub {
        ...
        // Change of owners means old listings are no longer valid
        address[] memory erc20s = marketHub.getERC20s();
        for (uint i = 0; i < erc20s.length; ++i) {
            address erc20 = erc20s[i];
            if (listings[sale.erc721][sale.tokenId][sale.erc20] > 0) {
                listings[sale.erc721][sale.tokenId][sale.erc20] = 0;
                emit Delisted(sale.seller, sale.erc721, sale.tokenId, erc20);
            }
        }
    }
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override returns (bool) {
        return _interfaceId == type(IListingMarket).interfaceId || super.supportsInterface(_interfaceId);
    }

    /**
     * @dev Private function that lists a token for sale.
     * @param _seller The address of the seller.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be listed for sale.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
    ...
    /**
     * @dev Private function that delists a token.
     * @param _seller The address of the seller.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be delisted.
     * @param _erc20 The address of the ERC20 token that was to be used as the payment currency.
    ...
    function _delist(address _seller, address _collection, uint256 _tokenId, address _erc20) internal virtual {
        IColleCollection collection = marketHub.getCollection(_collection);
        require(_seller == collection.ownerOf(_tokenId), "Only the token owner can call this function");
        require(listings[_collection][_tokenId][_erc20] > 0, "Token is not listed");
        listings[_collection][_tokenId][_erc20] = 0;
        emit Delisted(_seller, address(collection), _tokenId, _erc20);
    }

    /**
     * @dev Private function that manages the buying of a token.
     * @param _spender The address of the spender (who actually sends the payment).
     * @param _buyer The address of the buyer (who will become the new owner).
     * @param _collection The address of the collection contract from which the token is to be bought.
     * @param _tokenId The ID of the token to be bought.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
     * @param _payNow Specifies if payment will be made now or later.
    ...
    function _buy(
        ...
        require(!marketHub.getEscrow().hasActiveSale(_collection, _tokenId), "Sale already in progress for this item");
        ...
        require(price > 0, "Token is not listed");
    ...
    /**
     * @dev Gets the listed price of a token.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token whose price to return.
     * @param _erc20 The address of the ERC20 token used as the payment currency.
     * @return Returns the price at which the token is listed.
     */
    ...
    /**
     * @dev Computes the EIP-712 hash that should be signed by the seller to allow listing a token.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be listed for sale.
     * @param _seller The address of the seller.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
    ...
    /**
     * @dev Computes the EIP-712 hash that should be signed by the seller to delist a token.
     * @param _seller The address of the seller.
     * @param _collection The address of the collection contract to which the token belongs.
     * @param _tokenId The ID of the token to be delisted.
     * @param _erc20 The address of the ERC20 token that was to be used as the payment currency.
    ...
    /**
     * @dev Computes the EIP-712 hash that should be signed by the buyer for off-chain payment to buy a token.
     * @param _buyer The address of the buyer.
     * @param _collection The address of the collection contract from which the token is to be bought.
     * @param _tokenId The ID of the token to be bought.
     * @param _erc20 The address of the ERC20 token to be used as the payment currency.
