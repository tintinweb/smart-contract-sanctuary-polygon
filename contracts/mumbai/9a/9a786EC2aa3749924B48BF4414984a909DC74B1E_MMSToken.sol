...
/**
 * @notice keccak256 signature values.
    ...
    Ownable
{
    ...
    bytes32 private _MINT_BATCH_TYPEHASH;

    string internal _baseUri;

    address public sigAdmin;
    ...
    constructor(
        ...
        EIP712("LiveCGIToken", "1")
        onlyValidRoyaltyFeeNumerator(_royaltyFeeNumerator)
    {
        _baseUri = _uri;
        ...
        _MINT_TYPEHASH = keccak256(
            "Mint(address to,uint256 id,uint256 amount,bytes data,uint256 nonce,uint256 deadline)"
        );
        _MINT_BATCH_TYPEHASH = keccak256(
            "MintBatch(address to,uint256[] ids,uint256[] amounts,bytes data,uint256 nonce,uint256 deadline)"
        );
    }
    ...
    modifier onlyValidRoyaltyFeeNumerator(uint96 _royaltyFeeNumerator) {
        require(
            _royaltyFeeNumerator <= _feeDenominator(),
            "LiveCGIToken: royalty fee will exceed sale price"
        );
        _;
    }
    ...
    function mintWithSignature(
        ...
        require(signer == sigAdmin, "LiveCGIToken: invalid signature creator");
    ...
    function setSigAdmin(address newSigAdmin) external onlyAdmin {
        sigAdmin = newSigAdmin;
    }

    function addBlacklist(address user) external onlyAdmin {
        isBlacklisted[user] = true;
    }

    function removeBlacklist(address user) external onlyAdmin {
    ...
    function setRoyaltyFeeNumerator(
        uint96 _royaltyFeeNumerator
    ) external onlyAdmin onlyValidRoyaltyFeeNumerator(_royaltyFeeNumerator) {
        royaltyFeeNumerator = _royaltyFeeNumerator;
    }
