 ...
 * @title Minter
 */
contract Minter is Ownable, ERC1155Holder {

    address public venlyContract;
    uint256 public mintingPrice = 0 ether;
    ...
    function getTokenTypeIds() public view returns (uint256[] memory) {
        uint n = tokenTypes.length;
        uint256[] memory ids = new uint[](n);
        for (uint i = 0; i < n; i++) {
            ids[i] = tokenTypes[i].id;
        }
        return ids;
    }

    function getMintPriceAndMaxMint() public view returns (uint256, uint256) {
        return (mintingPrice, maxMintPerAccount);
    }

    function getPausedMintPriceAndMaxMint() public view returns (bool, uint256, uint256) {
        return (paused, mintingPrice, maxMintPerAccount);
    }
    ...
    function setMintPriceAndMaxMint(uint256 price, uint256 maxMint) external onlyOwner {
        require(price > 0, "Minting price must be > 0");
        require(maxMint > 0, "Max mint per account must be > 0");
        mintingPrice = price;
