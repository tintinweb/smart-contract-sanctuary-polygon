...
contract Tickets is ERC20, ERC20Permit, Ownable, ITickets {
  ...
  address payable public multisigAddress = payable(0x42e7127f5b9FD0272e09714418337Ac2E171Fd23);
...
//   uint256 public _maxTxAmount = _totalSupply.div(10);
  ...
  function swapAndLiquify(uint256 tAmount) private lockTheSwap {
    ...
    uint256 tokensForLP = tAmount.sub(tokensForBurn).sub(tokensForTeam).div(2);
    uint256 tokensForSwap = tAmount.sub(tokensForBurn).sub(tokensForTeam).sub(tokensForLP);

    // _basicTransfer(address(this),deadAddress,tokensForBurn);
    if(tokensForBurn>0)_burn(address(this),tokensForBurn);
    if(tokensForTeam>0)_basicTransfer(address(this),multisigAddress,tokensForTeam);

    uint256 initialBalance = address(this).balance;
    swapTokensForEth(tokensForSwap);
    uint256 newBalance = address(this).balance.sub(initialBalance);

    if (newBalance > 0 && tokensForLP > 0)addLiquidity(tokensForLP, newBalance);
