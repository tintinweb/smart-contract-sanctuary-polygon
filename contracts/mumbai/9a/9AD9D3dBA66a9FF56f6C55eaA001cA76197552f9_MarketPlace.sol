...
contract MarketPlace is Ownable, AccessControl, IMarketPlace {
	...
	function buy(uint256 _tokenId, address _buyer) public payable override {
		...
		// if(block.timestamp >= TokenInfo.feeExpiryTime) _thirdpartyfee = cutPer10000(TokenInfo.thirdpartyfee, TokenInfo.minPrice);
		// else _thirdpartyfee = 0;
		// uint256 ownerfee = TokenInfo.minPrice.sub(_artistfee).sub(_platformfee).sub(_thirdpartyfee);
		...
		// if(_thirdpartyfee >0){(bool thirdpartytxsucess,) =TokenInfo.thirdParty.call{value:_thirdpartyfee}('');
	...
	function transferfees(uint256 _tokenId) internal {
		...
		uint256 ownerfee = TokenInfo.minPrice.sub(_artistfee).sub(_platformfee).sub(_galleryownerfee);
		TokenInfo.totalartistfee = addTotal(_artistfee, TokenInfo.totalartistfee);
		TokenInfo.totalplatformfee = addTotal(_platformfee, TokenInfo.totalplatformfee);
		TokenInfo.totalgalleryownerfee = addTotal(_galleryownerfee, TokenInfo.totalgalleryownerfee);
	...
	// 	uint256 _artistfee = cutPer10000(artistfee, sellAmount);
	// 	uint256 _platformfee = cutPer10000(platformfee, sellAmount);
	// 	uint256 _galleryownerfee = cutPer10000(galleryownerfee, sellAmount);

	// 	if (block.timestamp >= feeExpiryTime) {
	// 		_thirdpartyfee = cutPer10000(thirdpartyfee, sellAmount);
	// 	} else _thirdpartyfee = 0;
	// 	uint256 ownerfee = sellAmount.sub(_artistfee).sub(_platformfee).sub(_galleryownerfee);
