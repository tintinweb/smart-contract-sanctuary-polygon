...
contract Token is ERC721Base, ReentrancyGuard {
    ...
    ) ERC721Base(_maxSupply, _approvedOperator, _name, _symbol, _baseURI, _contractURI, _adminAddresses) {
        require(tokenPrices_.length == _maxSupply, "Incorrect number of token prices");
    ...
    /**
     * @notice Returns the price of token with the given ID
     ...
     */
    function setTokenPrice(uint256 _tokenId, uint256 _price) external onlyAdminAddress {
        _tokenPrices[_tokenId] = _price;
    }

    /**
     * @dev Updates receiver wallet address
     * @param _receiverWalletAddress New receiver wallet address to be set
     */
    function setReceiverWalletAddress(address _receiverWalletAddress) external onlyAdminAddress {
        receiverWalletAddress = _receiverWalletAddress;
    }
     ...
     */
    function adminBatchMint(address[] memory accounts) external nonReentrant onlyAdminAddress {
    ...
    function mint(address account) external payable nonReentrant whenNotPaused {
        require(totalSupply() < totalMaxSupply, "Max supply exceeded");
        ...
        (bool success, ) = receiverWalletAddress.call{value: price}("");
...
abstract contract ERC721Base is ERC721, Ownable, Pausable {
    ...
    /**
     * @notice Updates token total max supply
     * @param _maxSupply max supply to be set
     * @dev Can be done only by the contract owner
     */
    function setTotalMaxSupply(uint256 _maxSupply) external onlyOwner {
        totalMaxSupply = _maxSupply;
    }

    /**
     * @notice Updates approved operator address
