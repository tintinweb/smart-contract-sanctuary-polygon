...
import {ITradingFeesRegistry} from "./shared/interfaces/ITradingFeesRegistry.sol";
import {IAffiliateManager} from "./shared/interfaces/IAffiliateManager.sol";
import {IArtistManager} from "./shared/interfaces/IArtistManager.sol";
...
contract PixelParkExchange is IPixelParkExchange, ReentrancyGuard, Ownable {
    ...
    IArtistManager public artistManager;
    ...
    // event NewAffiliateManager(address indexed affiliateManager);
    // event NewArtistManager(address indexed artistManager);
    event NewArtistAffiliateManager(
        address indexed artistManager,
    ...
    constructor(
        ...
        WETH = _WETH;
        affiliateManager = IAffiliateManager(_affiliateManager);
        artistManager = IArtistManager(_artistManager);
    ...
    function matchAskWithTakerBid(
        ...
        //         if (takerBid.tokenId == tokenIds[i]) {
        //             found = true;
        //             break;
        //         }
        //     }

        //     require(found, "Invalid Token ID");
        // }
        _validateOrder(makerAsk, askHash);
    ...
    function matchBulkOrder(
        OrderTypes.TakerOrder[] calldata takerBid,
        OrderTypes.MakerOrder[] calldata makerAsk
    ) external {
        for (uint256 i = 0; i < takerBid.length; i++) {
            matchAskWithTakerBid(takerBid[i], makerAsk[i]);
        }
    }

    function matchBulkOrderWithETHandWETH(
        OrderTypes.TakerOrder[] calldata takerBid,
        OrderTypes.MakerOrder[] calldata makerAsk
    ) external payable {
        for (uint256 i = 0; i < takerBid.length; i++) {
            (bool success, ) = address(this).call{value: takerBid[i].price}(
                abi.encodeWithSelector(
                    this.matchAskWithTakerBidUsingETHAndWETH.selector,
                    takerBid[i],
                    makerAsk[i]
                )
            );
    ...
    /**
     * @notice Match a makerBid with a takerAsk (exception for English Auction)
     * @param takerAsk taker ask order
     * @param makerBid maker bid order
     */
    function matchMakerBidWithTakerAsk(
        OrderTypes.TakerOrder calldata takerAsk,
        OrderTypes.MakerOrder calldata makerBid
    ...
    function matchMakerBidWithMakerAsk(
        ...
        require(msg.sender == auctionAdmin, "not an auction admin");
        require(
     ...
     * @param _artistManager new currency manager address
     */
    function updateArtistAffiliateManager(
        address _artistManager,
        address _affiliateManager
    ) external onlyOwner {
        require(
            _artistManager != address(0) && _affiliateManager != address(0),
            "Owner: Cannot be null address"
        );
        artistManager = IArtistManager(_artistManager);
        affiliateManager = IAffiliateManager(_affiliateManager);
        emit NewArtistAffiliateManager(_artistManager, _affiliateManager);
        // emit NewAffiliateManager(_affiliateManager);
    }
    ...
    /**
     * @notice update the auction admin address
     * @param _auctionAdmin address of the auction admin
     */
    function updateAuctionAdmin(address _auctionAdmin) external onlyOwner {
        require(_auctionAdmin != address(0), "zero address");
    ...
    function _transferFeesAndFunds(
        ...
        address artist = artistManager.getArtist(collection, tokenId);
        ...
        // 1. Protocol fee
        {
            ...
            if (
                ...
                ITradingFeesRegistry(protocolFeeRecipient).notifyTradingFees(
                    artist,
                    // collection,
                    currency,
                    protocolFeeAmount
        ...
        //    and affiliate comission
        {
            ...
            if (
                ...
                if (affiliateManager.isAffiliateWhitelisted(affiliate)) {
                    uint256 commissionPercent = artistManager.commissionInfo(
                        artist
                    );

                    commission = (royaltyFeeAmount * commissionPercent) / 10000;
    ...
    function _transferFeesAndFundsWithWETH(
        ...
        //    and affiliate commission
        {
            ...
            if (
                ...
                if (
                    affiliate != address(0) &&
                    affiliateManager.isAffiliateWhitelisted(affiliate)
                ) {
                    uint256 commissionPercent = artistManager.commissionInfo(
                        artist
                    );
                    ...
                    IERC20(WETH).safeTransfer(affiliate, commission);
    ...
    function _validateOrder(
        ...
        require(
            !artistManager.collectionBlacklist(makerOrder.collection),
            "collection blocked"
        );

        require(
            !artistManager.nftBlacklist(
...
interface ITradingFeesRegistry {
...
interface IAffiliateManager {
    ...
    function isAffiliateWhitelisted(
        address affiliate
    ) external view returns (bool);

    function viewWhitelistedAffiliates(
        uint256 cursor,
        uint256 size
    ) external view returns (address[] memory, uint256);

    function viewCountWhitelistedAffiliates() external view returns (uint256);
}
...
interface IArtistManager {
    ...
    function addCollection(address _artist, address _collection) external;

    function setCommission(address _artist, uint256 _commissionRate) external;
    ...
    function viewWhitelistedArtist(
        uint256 cursor,
        uint256 size
    ) external view returns (address[] memory, uint256);

    function commissionInfo(address _artist) external view returns (uint256);

    function collectionBlacklist(
        address collection
    ) external view returns (bool);

    function nftBlacklist(
        address collection,
        uint256 tokenId
    ) external view returns (bool);
}
