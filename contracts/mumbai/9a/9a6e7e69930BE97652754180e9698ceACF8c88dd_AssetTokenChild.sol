...
import "./AccessManagerChild.sol";

/// @author Swarm Markets
/// @title AssetToken
/// @notice Main AST Token Contract
contract AssetTokenChild is ERC20, AccessManagerChild {
    ...
    /// @param _childChainManagerProxy manager proxy address
    ...
    constructor(
        ...
        _setChildChainManagerProxyAddress(_childChainManagerProxy);
    ...
    function _beforeTokenTransfer(
        ...
        /// @dev mint signature: ==> _beforeTokenTransfer(address(0), account, amount);
        /// @dev burn signature: ==> _beforeTokenTransfer(account, address(0), amount);
        /// @dev Burn is not allowed on freeze

        if (freezeState) {
            /// @dev FREEZED
            /// @dev Below it's a mint. Only the executor is allowed to receive
            if (_from == address(0)) {
                require(hasRole(EXECUTOR_ROLE, _to), "BTT: Mint _to not executor(freezed)");
            }

            /// @dev It is a transfer
            /// @dev only SFrole can send and authorize can receive
            if (_to != address(0) && _from != address(0)) {
                require(hasRole(SAFEGUARD_TRANSFER_ROLE, _from), "BTT: SAFEGUARD_TRANSFER_ROLE missing");
                require(mustBeAuthorizedHolder(_to), "BTT: From - not authorized f0");
                /// is this ok ?
            }
        } else {
            /// @dev NOT FREEZED
            /// @dev Below it's a mint. Only the executor and authorized can receive
            if (_from == address(0)) {
                if (!hasRole(EXECUTOR_ROLE, _to)) {
                    require(mustBeAuthorizedHolder(_to), "BTT: To - not authorized t1");
                }
            }

            /// @dev Below it's a burn. Only the executor and authorized can burn
            if (_to == address(0)) {
                if (!hasRole(EXECUTOR_ROLE, _from)) {
                    require(mustBeAuthorizedHolder(_from), "BTT: From - not authorized f1");
                }
            }

            /// @dev It is a transfer. Only the executor and authorized can transfer
            if (_to != address(0) && _from != address(0)) {
                /// @dev none of them are executors. Check both
                if (!hasRole(EXECUTOR_ROLE, _from) && !hasRole(EXECUTOR_ROLE, _to)) {
                    require(mustBeAuthorizedHolder(_from), "BTT: From - not authorized f2");
                    require(mustBeAuthorizedHolder(_to), "BTT: To - not authorized t2");
                } else {
                    /// @dev one of them is executor
                    if (!hasRole(EXECUTOR_ROLE, _from)) {
                        /// @dev _from is not executor
                        require(mustBeAuthorizedHolder(_from), "BTT: From - not authorized f3");
                    } else {
                        /// @dev _to is not executor
                        require(mustBeAuthorizedHolder(_to), "BTT: To - not authorized t3");
                    }
                }
            }
        }
    ...
    function setKya(string memory _kya) external onlyAdmin onlyUnfreezed {
    ...
    function setFreezeState(bool _freezeState) external onlyExecutor {
        if (freezeState) {
    ...
    function deposit(address _account, bytes calldata _depositData) external {
        require(_msgSender() == childChainManagerProxy, "DEP: not allowed");
        uint256 amount = abi.decode(_depositData, (uint256));
        _mint(_account, amount);
    }

    function withdraw(uint256 _amount) external onlyUnfreezed {
...
abstract contract AccessManagerChild is AccessControl {
    ...
    event ChildChainManagerProxyAddressSet(address indexed _newAddress);
    ...
    /// @notice Check if sender has the EXECUTOR_ROLE role
    modifier onlyExecutor() {
        require(hasRole(EXECUTOR_ROLE, _msgSender()), "not EXECUTOR");
        _;
    }
    ...
    modifier onlyAdminOrAgent() {
        require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()) || agents[_msgSender()], "not AGENT nor ADMIN");
        _;
    }
    ...
    /// @param _newAddress to replace the old one
    function _setChildChainManagerProxyAddress(address _newAddress) internal {
        require(_newAddress != address(0), "CCMP: Invalid address");
        emit ChildChainManagerProxyAddressSet(_newAddress);
        childChainManagerProxy = _newAddress;
    }
    ...
    function mustBeAuthorizedHolder(address _account) public view returns (bool) {
        ...
        if (hasRole(EXECUTOR_ROLE, _account) || agents[_account]) {
            return true;
        }
    ...
    /// @param _account to be granted the EXECUTOR_ROLE
    function grantExecutorRole(address _account) external onlyAdmin {
        grantRole(EXECUTOR_ROLE, _account);
    }

    /// @notice Sets childChainManagerProxy address
    /// @param _newAddress to replace the old one
    function setChildChainManagerProxyAddress(address _newAddress) external onlyAdmin {
        _setChildChainManagerProxyAddress(_newAddress);
    }

    /// @notice Adds an AGENT
    /// @param _newAgent to be added
    function addAgent(address _newAgent) external onlyAdmin {
    ...
    function transferAgentList(address _newAgent, address _oldAgent) external onlyAdminOrAgent {
    ...
    function addMemberToBlacklist(address _account) external onlyAdmin {
    ...
    function removeMemberFromBlacklist(address _account) external onlyAdmin {
