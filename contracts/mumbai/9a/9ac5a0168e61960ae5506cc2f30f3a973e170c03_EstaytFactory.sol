...
contract EstaytFactory is IEstaytFactory {
    ...
    function createEstayt(uint256 _proofOfLifeDuration, IBeneficiary.Beneficiary[] memory _beneficiaries) external {
        bytes memory encodedParams = abi.encode(msg.sender, _proofOfLifeDuration, _beneficiaries);
        bytes32 salt = keccak256(abi.encodePacked(block.number, encodedParams));

        address newEstaytAddress = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            address(this),
                            salt,
                            keccak256(abi.encodePacked(type(Estayt).creationCode, encodedParams))
                        )
                    )
                )
            )
        ...
        isEstayt[address(newEstaytAddress)] = true;
        estaytForAddress[msg.sender] = newEstaytAddress;

        require(
            address(new Estayt{salt: salt}(msg.sender, _proofOfLifeDuration, _beneficiaries)) == newEstaytAddress,
            "Prediction"
        );

        emit NewEstayt(newEstaytAddress, msg.sender);
    }
