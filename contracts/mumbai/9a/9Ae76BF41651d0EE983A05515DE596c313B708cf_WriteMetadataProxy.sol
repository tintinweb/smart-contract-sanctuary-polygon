...
contract WriteMetadataProxy is AccessControl {

    bytes32 public constant METADATA_ADDER_ROLE = keccak256("METADATA_ADDER_ROLE");

    constructor(address metadataAdder, address admin) { 
        _grantRole(DEFAULT_ADMIN_ROLE, admin);
        _grantRole(METADATA_ADDER_ROLE, metadataAdder);
    }

    function addUriArray(address[] memory tokenAddresses, string[] memory newuris) public {
        require(hasRole(METADATA_ADDER_ROLE, msg.sender), "Caller is not allowed to add metadata");
        require(newuris.length == tokenAddresses.length, "New URIs and token addresses must be same size");
        for (uint i = 0; i < newuris.length; i++) {
            ERC20PT_extendArr pt = ERC20PT_extendArr(tokenAddresses[i]);
            pt.addURI(newuris[i]);
        } 
    } 
