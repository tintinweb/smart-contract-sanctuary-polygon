    ...
    ICreatorsRegistry
{
    ...
    mapping(address => mapping(uint256 => LibPart.Part[])) public creators;

    /**
     * @dev Require _msgSender() to be the creator of the token id
     * this only succeeds when the tokens has only 1 creator, other wise use signing
     */
    modifier onlyCreator(address token, uint256 _id) {
        isCreator(token, _id);
        _;
    }
     ...
     */
    function isCreator(address token, uint256 _id)
        public
        view
        virtual
        override
    ...
    {
        require(
            IERC1155Tradeable(token).exists(_id),
            "CreatorsRegistry: Token does not exist"
        );
        require(
            creators[token][_id].length == 1,
            "CreatorsRegistry: There should be only 1 creator, use signing instead"
        );
        require(
            creators[token][_id][0].account == _msgSender(),
            "CreatorsRegistry: Only creator allowed"
        );
        return true;
    }
    ...
    function setCreatorBatch(
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            setCreator(token, _ids[i], _newCreators[i]);
        }
    }
    ...
    function setCreator(
        ...
        uint256 _id,
        address _newCreator
    ) public virtual override onlyCreator(token, _id) {
        creators[token][_id][0].account = payable(_newCreator);
        creators[token][_id][0].value = 10000;
    }
    ...
    {
        require(
            IERC1155Tradeable(token).exists(data.tokenId),
            "CreatorsRegistry: Token does not exist"
        );
        require(
            creators[token][data.tokenId].length ==
                data.previousSignatures.length,
            "CreatorsRegistry: Not enough previous signatures"
        );
        require(
            data.creators.length == data.signatures.length,
            "CreatorsRegistry: Not enough signatures"
        );

        bytes32 hash = LibERC1155Creators.hash(data);
        for (uint256 i = 0; i < creators[token][data.tokenId].length; i++) {
            address creator = creators[token][data.tokenId][i].account;
            if (creator != _msgSender()) {
                validate(creator, hash, data.previousSignatures[i]);
            }
        }

        validateCreators(token, _msgSender(), hash, data.creators, data.signatures);
        _saveCreators(token, data.tokenId, data.creators);
    }
    ...
    function validateCreators(
        ...
        if (
            ...
            (_creators[0].account == sender ||
                IERC1155Tradeable(token).proxiesRegistry().isProxy(
                    _creators[0].account,
                    sender
                ))
        ) return;
    ...
    function saveCreators(
        ...
        require(
            ...
            "CreatorsRegistry: Only possible by token contract"
        );
        _saveCreators(_msgSender(), tokenId, _creators);
    }

    function _saveCreators(
        address token,
        uint256 tokenId,
        LibPart.Part[] memory _creators
    ) internal virtual {
        delete creators[token][tokenId];
        uint256 total = 0;
        for (uint256 i = 0; i < _creators.length; i++) {
            require(
                _creators[i].account != address(0x0),
                "CreatorsRegistry: Account should be present"
            );
            require(
                _creators[i].value != 0,
                "CreatorsRegistry: Creator share should be positive"
            );
            total = total + _creators[i].value;
            creators[token][tokenId].push(_creators[i]);
        }

        require(
            total == 10000,
            "CreatorsRegistry: total amount of creators share should be 10000"
        );
    }
