...
contract Create is IExecutePlugin, Ownable, CloneFactory {
    ...
    function checkData(uint256 _version) private view {
        require(_version == PLUGIN_VERSION, "Create: wrong version");
        (bool enable, address pluginContract) = registry.getPlugin(PLUGIN_NAME, PLUGIN_VERSION);
        require(enable, "Create: wrong enable plugin");
        require(pluginContract == address(this), "Create: wrong plugin contract");
    }
}
