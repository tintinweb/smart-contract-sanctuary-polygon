...
*/
    ...
    function targetTo(uint256 _target, uint256 _price, IERC20 _currency, IERC20 _token) internal{
        uint8 cDcml = allowedCurrencies[_currency].decimals;
        uint8 tDcml = projectsByID[_token].tokenDecimals;
        uint256 trg = _target / 10 ** tDcml;
        uint256 trPrice = trg / _price;
        uint256 payTo = (toPolykick + toExchange) / 10 ** cDcml;
    ...
    function startILO(
        ...
        uint256 _duration = _days + block.timestamp; //(_days * 1 days) + block.timestamp;
        targetTo(_target, price, _currency, _token);
