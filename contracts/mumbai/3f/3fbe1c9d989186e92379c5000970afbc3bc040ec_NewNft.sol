...
interface IERC20 {
     ...
     * allowance mechanism. `amount` is t hen deducted from the caller's
     * allowance.
     *
     * Returns a boolean value indicating whether the operation succeeded.
     *
...
contract NewNft is Owned, ERC721URIStorage {
    ...
    function mintBatch(string[] memory ipfsmetadata, uint256 supply,  uint256[] memory id_) public {
    
        for (uint256 i = 0; i < supply; i++) {
            _safeMint(msg.sender, id_[i]);
            _setTokenURI(id_[i], ipfsmetadata[i]);
            _creator[id_[i]] = msg.sender;
        }
    }
}