...
contract BetsFuzzed is UltibetsCore, ReentrancyGuard {
        ...
        payable
    {
        require(
            eventList[_eventID].status == EventStatus.Open &&
                block.timestamp <= eventList[_eventID].startTime - noticeBetTime,
            "Non available bet."
        );
        ...
        BetData memory bet = BetData(
            msg.sender,
            _eventID,
            betDataByBettor[msg.sender][_eventID][_eventValue].betAmount +
                betAmount,
            0,
            _eventValue,
            block.timestamp
        );
        betDataByBettor[msg.sender][_eventID][_eventValue] = bet;

        dailyBetsByBettor[msg.sender][eventList[_eventID].eDate].push(bet);
        ...
        nonReentrant
    {
        ...
        uint256 betAmount = betDataByBettor[msg.sender][_eventID][_eventResult]
            .betAmount;
        ...
        nonReentrant
    {
        ...
        betDataByBettor[msg.sender][_eventID][_betValue].paidAmount = gain;
...
contract UltibetsCore is CustomAdmin {
    ...
    mapping(address => mapping(uint256 => BetData[])) dailyBetsByBettor; //bet list of a bettor
    mapping(address => mapping(uint256 => mapping(EventResult => BetData))) betDataByBettor; //betting history of a bettor   bettor => eventid => event result => betdata
    ...
    function dailyBetsOfBettor(
        ...
        for (uint256 i; i < betNum1; i++) {
            BetData memory bet = dailyBetsByBettor[_bettor][_date1][i];
        ...
        for (uint256 i; i < betNum2; i++) {
            BetData memory bet = dailyBetsByBettor[_bettor][_date2][i];
            EventInfo memory eventData = eventList[bet.eventID];
    ...
    function checkBetResult(
        ...
        BetData memory bet = betDataByBettor[_bettor][_eventID][_eventResult];
    ...
    function getWinAmount(
        ...
        uint256 betAmount = betDataByBettor[_bettor][_eventID][_betValue]
            .betAmount;
        ...
        returns (EventDetailPerBettor memory)
    {
        ...
        if (eType == EventType.Double) {
            ...
            sideBetAmounts[0] = betDataByBettor[_bettor][_eventID][
                EventResult.Home
            ].betAmount;
            sideBetAmounts[1] = betDataByBettor[_bettor][_eventID][
                EventResult.Away
            ].betAmount;
        } else {
            ...
            sideBetAmounts[2] = betDataByBettor[_bettor][_eventID][
                EventResult.Draw
            ].betAmount;
        }
