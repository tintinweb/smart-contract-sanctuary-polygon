...
contract Margin is ReentrancyGuard, MarginStorage, Ownable {
    ...
    function closePositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed, uint256 marginAmount, uint256 temp) public onlyPosition {
        Deposit storage depositInfo = depositDetails[user][assetAddress];
        
        depositInfo.usedMargin -= marginUsed;
        depositInfo.usedLoan -= loanUsed;
        depositInfo.marginAmount += marginAmount;
        depositInfo.marginAmount = depositInfo.marginAmount - temp;
        depositInfo.loanAmount += loanUsed;
    } 
    ...
    function liquidateMargin(address user, address assetAddress) external {
        ...
        if(marginAmount <= (interest + 949561471039225300)) { 
