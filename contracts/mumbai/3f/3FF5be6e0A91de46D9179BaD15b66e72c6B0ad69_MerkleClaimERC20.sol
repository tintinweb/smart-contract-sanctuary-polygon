...
contract MerkleClaimERC20 is ReentrancyGuard {
   ...
   /// setting the staked contract in constructor
    DankBankStaking private stakingContract;
    ...
    constructor(bytes32 _merkleRoot, address _memeTokenAddress, address _owner, address stakingAddress) {
        require(_memeTokenAddress != address(0), "memeTokenAddress cannot be 0");
        memeTokenAddress = _memeTokenAddress;
        owner = _owner;
        merkleRoot = _merkleRoot; 
        stakingContract = DankBankStaking(stakingAddress);
    }
    ...
    function claim(
        ...
        bool hasStaked = stakingContract.balanceOf(msg.sender) >= amount / 2;
        ...
        totalAmountClaimed[to] = totalAmountClaimed[to] + sendAmount;
    
        IERC20(memeTokenAddress).transfer(to, sendAmount);

        emit Claim(to, sendAmount);
    }

    function ownerWithdraw(uint256 amount) external onlyOwner {
        IERC20(memeTokenAddress).transfer(owner, amount);
    }
}
