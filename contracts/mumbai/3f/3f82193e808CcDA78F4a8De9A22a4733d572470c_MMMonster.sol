...
contract MMMonster is ERC721Enumerable, Ownable, AccessControl, Concatenate {
    ...
    function tokenURI(uint256 _tokenID) public view override virtual returns (string memory) {
        ...
        _result = concat(Strings.toString(_tokenID),"eggs");
        return _result;
    }
    ...
    function _mintSingleNFT(address _to) private {
        ...
        uint hp = randomNumber(randNonce, 7, 10);
        randNonce = randNonce.add(1);
        uint atk = randomNumber(randNonce, 7, 10);
        randNonce = randNonce.add(1);
        uint def = randomNumber(randNonce, 7, 10);
    }
