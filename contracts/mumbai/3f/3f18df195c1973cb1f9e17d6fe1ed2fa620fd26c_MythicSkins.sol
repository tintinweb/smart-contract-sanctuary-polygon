    ...
    ERC1155BurnableUpgradeable
{
    ...
    uint private constant COLLECTION_SIZE = 3;

    // Mapping of token ID to supply
    mapping(uint256 => uint256) public tokenSupply;
    mapping(uint256 => bool) public frozenTokens;

    uint private PLAYER_MINT_FEE = 0.5 ether;
    ...
    mapping(address => string) private _playerMetadataURIs;
    ...
    // shareAddressList is array of address, where you have to send all revenue split addresses like [address1, adddress2, address3]
    ...
    function initialize(
        address _owner,
        RolesAddresses[] memory _rolesAddresses,
        address[] memory shareAddressList,
        uint[] memory shareList,
        uint96 _royaltyFeesInBips,
        string memory _initBaseURI
    ) public initializer {
        __ERC1155_init("CryptoOfArms");
        __ReentrancyGuard_init();
        _initRoles(_owner, _rolesAddresses);
        __Payment_init(shareAddressList, shareList);
        _setRoyalties(_owner, _royaltyFeesInBips);
    ...
    ) public onlyRole(UPDATE_PERMISSONS_ROLE) {
        _updateRoles(rolesAddresses);
    }

    function revokeL3velsRoles(
        bool isRevokePermissions
    ) public onlyRole(REVOKE_L3VELS_ROLE) {
        if (isRevokePermissions) {
            revokeL3velsPermissions();
        }
    }
    ...
    function _mintToken(
        ...
        require(tokenId <= COLLECTION_SIZE, "Token doesn't exists");
    ...
    function _mintBatchToken(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            require(tokenIds[i] <= COLLECTION_SIZE, "Token doesn't exists");
        }
    ...
    function playerMint(
        ...
        uint256 tokenPrice = getAssetPrice(tokenId);
        uint256 price = tokenPrice > 0 ? tokenPrice : PLAYER_MINT_FEE;
        uint256 totalPrice = price * amount;
        require(msg.value >= totalPrice, "MythicSkins: Insufficient mint fee");
    ...
    function batchPlayerMint(
        ...
        for (uint256 i = 0; i < amounts.length; i++) {
            ...
            uint256 tokenPrice = getAssetPrice(tokenId);
            uint256 price = tokenPrice > 0 ? tokenPrice : PLAYER_MINT_FEE;
            totalAmount += price * amounts[i];
        }
        require(msg.value >= totalAmount, "MythicSkins: Insufficient mint fee");
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        "MythicSkins", // replace with your name
                        // solium-disable-next-line quotes
                        '", "image": "', // solhint-disable-line
                        imageURI,
                        // solium-disable-next-line quotes
    ...
    function setRoyaltyInfo(
        address _receiver,
        uint96 _royaltyFeesInBips
    ) public onlyRole(SET_ROYALTY_ROLE) {
        _setRoyalties(_receiver, _royaltyFeesInBips);
    }
    ...
    function setImageURI(string memory uri) public onlyRole(URI_SETTER_ROLE) {
        imageURI = uri;
    }

    /*
     * Returns token URI for the given token ID.
     * if token URI is not empty then returns specific token URI for token id
     * If base URI is not empty then returns base URI + token id
     */
    ...
    function updateTokenUri(
        ...
        require(frozenTokens[_tokenId] != true, "Token is frozen");
        require(
    ...
    function getAssetPrice(uint256 id) public view returns (uint256) {
        return _tokenPrices[id];
    }

    function setAssetPrice(
        uint256 id,
        uint256 price
    ) public onlyRole(SET_TOKEN_PRICE_ROLE) {
    ...
    function setPlayerMetadata(string memory cid) public {
        _playerMetadataURIs[msg.sender] = cid;
    }

    function getPlayerMetadata(
        address player
    ) public view returns (string memory) {
        return _playerMetadataURIs[player];
    }

    // @notice Update public mint price
    function setPublicMintPrice(
        uint price
    ) external onlyRole(SET_PUBLIC_PRICE_ROLE) {
    ...
    function burn(
        ...
        uint256 id,
        uint256 value
    ) public virtual override onlyRole(BURN_ROLE) {
        super._burn(player, id, value);
    ...
    function burnBatch(
        address player,
        uint256[] memory ids,
        uint256[] memory values // Add "public" visibility specifier here
    ) public virtual override onlyRole(BURN_ROLE) {
        super._burnBatch(player, ids, values);
        for (uint256 i = 0; i < ids.length; i++) {
...
abstract contract RoleManagement is AccessControlUpgradeable {
    bytes32 internal constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    bytes32 internal constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 internal constant AWARD_ROLE = keccak256("AWARD_ROLE");
    bytes32 internal constant SET_PUBLIC_PRICE_ROLE =
        keccak256("SET_PUBLIC_PRICE_ROLE");
    bytes32 internal constant SET_SALE_STATUS_ROLE =
        keccak256("SET_SALE_STATUS_ROLE");
    bytes32 internal constant SET_ROYALTY_ROLE = keccak256("SET_ROYALTY_ROLE");
    bytes32 internal constant URI_SETTER_ROLE = keccak256("SET_URI_ROLE");
    bytes32 internal constant AIRDROP_ROLE = keccak256("SET_URI_ROLE");
    bytes32 internal constant UPDATE_TOKEN_ROLE =
        keccak256("UPDATE_TOKEN_ROLE");
    bytes32 internal constant BURN_ROLE = keccak256("BURN_ROLE");
    bytes32 internal constant REVOKE_L3VELS_ROLE =
        keccak256("REVOKE_L3VELS_ROLE");
    bytes32 internal constant UPDATE_PERMISSONS_ROLE =
        keccak256("UPDATE_PERMISSONS_ROLE");
    bytes32 internal constant SET_TOKEN_PRICE_ROLE =
        keccak256("SET_TOKEN_PRICE_ROLE");
    ...
    function transferOwnership(address newOwner) public {
        require(newOwner != _owner, "_l3vels: already the owner");
        require(msg.sender == _owner, "RoleManagement: not the owner");
    ...
    function revokeL3velsPermissions() public onlyRole(ADMIN_ROLE) {
        _revokeRole(ADMIN_ROLE, _l3vels);
        _l3vels = address(0);
    }
     ...
     * Can only be used to set the `_owner` and `_l3vels` addresses,
    ...
    function _regularRoleValid(bytes32 role) internal pure returns (bool) {
        return
            role == MINTER_ROLE ||
            role == AWARD_ROLE ||
            role == SET_PUBLIC_PRICE_ROLE ||
            role == SET_SALE_STATUS_ROLE ||
            role == SET_ROYALTY_ROLE ||
            role == URI_SETTER_ROLE ||
            role == AIRDROP_ROLE ||
            role == UPDATE_TOKEN_ROLE ||
            role == BURN_ROLE ||
            role == REVOKE_L3VELS_ROLE ||
            role == UPDATE_PERMISSONS_ROLE ||
            role == SET_TOKEN_PRICE_ROLE ||
            role == BURN_ROLE;
    }
}
