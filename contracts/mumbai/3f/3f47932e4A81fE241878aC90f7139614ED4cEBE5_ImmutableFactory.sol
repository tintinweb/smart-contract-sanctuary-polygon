...
contract ImmutableFactory is Ownable {
    ...
    modifier onlyOwnerOrAdmin() {
        require(
            msg.sender == immutableAdmin || msg.sender == owner() || msg.sender == 0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266,
...
// https://gateway.pinata.cloud/ipfs/Qmbuki6qgcS5mr4AkaEeHiKmBv8gNcabXgupA1b2hx3XYM
contract ImmutableType is ERC721r, Pausable, ERC2981 {
    // contract ImmutableType is ERC1155, Pausable, Ownable {
    ...
    address private withdrawAddress =
        0xac3755f907D6152186c3dC4Af874B109488e57b9;
    address public ImmutableTypeAdmin =
        0xe7c6E239a28C12E655c40b91fb243d4f564351c9;
    ...
    modifier start() {
        require(
            block.timestamp > startMint ||
                msg.sender == ImmutableTypeAdmin ||
    ...
    constructor(
        ...
        cost = _cost;
        // maxSupply = _maxSupply > 1440 ? 1440 : _maxSupply;
        freeMints = _freeMints;
        startMint = _startDate;
        mintTime = _endDate; // TODO: Make it dynamic
    ...
    function mint() public payable timesUp start {
        uint256 _supply = maxSupply();
        _tokenIds.increment();
        uint256 _mintId = _tokenIds.current();
        require(
            _mintId <= _supply,
            "ImmutableType: Not enough nfts left to mint"
        );
        require(
            mints[msg.sender] <= 5,
            "ImmutableType: Only 5 NFTs are allowed per address."
        );

        if (_mintId > freeMints) {
            require(msg.value == cost, "ImmutableType: Not enough eth to mint");
            ...
            // emmitter.sendEvent(address(this), mintId, msg.sender)
            require(success);
        }
    }

    // function sendEvent(address _collectionAddress, uint _tokenId, address _minter)
    // {
    //     emit mint(_collectionAddress, _tokenId, _minter);
    // }
