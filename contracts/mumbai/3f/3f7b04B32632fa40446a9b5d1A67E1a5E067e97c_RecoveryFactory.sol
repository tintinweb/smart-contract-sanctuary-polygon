...
contract Recovery is IRecovery, YousovRoles {
    ...
    constructor(address _user,address _recoveryFactory, address _yousovAccessControl) {
    ...
    function contenderAgentsList() view external override returns (address[] memory ) {
        return contenderAgents;
    }
    function addNewAgentToRecovery() external override{
        require(YousovAccessControl(yousovAccessControl).hasRole(YOUSOV_USER_ROLE, user), "YOUSOV : Recovery user is not a yousov user");
        require(agentList.length < IUser(user).userChallenges().length * 2, "YOUSOV : Recovery selection is over");
    ...
    function getRecoveryStatus() external view override returns (RecoveryStatus currentRecoveryStatus) {
