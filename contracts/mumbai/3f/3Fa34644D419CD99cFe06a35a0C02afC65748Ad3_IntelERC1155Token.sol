...
contract IntelERC1155Token is IERC2981, ERC165Storage, Ownable, IERC1155MetadataURI, ERC1155, MinterRole {
    ...
    /// @dev mint flag mapping
    mapping(address => bool) minted;

    /// @dev whitelist minted flag mapping
    ...
    ///@dev events
    event MintCompleted(uint _dropPhase ,address _beneficiary);
    ...
    */
    constructor(string memory _name, string memory _symbol, string memory _tokenURIPrefix, address _royalty, bytes32 _root) ERC1155(_tokenURIPrefix) {
        name = _name;
        symbol = _symbol;
    ...
    */
    ...
    /**
     * @dev mint function which should be called by only minters.
     * @param _beneficiary address where mint to
     */
    function mint(bytes32[] calldata _merkleProof, address _beneficiary) public {
        require(dropPhase > 1, "not the drop period");
        require(totalsupply < maxsupply, "have already minted all NFTs");
        require(minted[_beneficiary] == false, "have already minted to this address");
        ...
        if(dropPhase == 2) {
            ...
            minted[_beneficiary] = true;
            whitelistMinted[_beneficiary] = true;
            totalsupply++;
        } else if(dropPhase == 3) {
            safeMint(_beneficiary, 1, 1);
            minted[_beneficiary] = true;
        ...
        emit MintCompleted(dropPhase, _beneficiary);
    }

    function getDropPhase() external view returns(uint8) {
        return dropPhase;
    }

    function setDropPhase(uint8 _dropPhase) external onlyOwner {
