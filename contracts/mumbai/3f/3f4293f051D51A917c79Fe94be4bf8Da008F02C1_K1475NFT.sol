...
contract K1475NFT is ERC721A, EIP2981PerTokenRoyalties, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _mintCount;
  uint256 private constant supply = 2000;
  ...
  constructor(string memory _baseUri, uint256 _newPrice) ERC721A("Kape1475", "K1475") {
    _admin = msg.sender;
  ...
  function remainingSupply() external view returns (uint256) {
    return supply - _mintCount.current();
  }
  ...
  function walletHoldsToken(address _wallet) public view returns (bool) {
    return IERC721A(address(this)).balanceOf(_wallet) > 0;
  }
  
  function mintToken() external payable returns (uint256) {
    bool isAMember = _teamAddress[msg.sender] == 1;
    uint256 newItemId = _mintCount.current();
    require(!walletHoldsToken(msg.sender), "This wallet holds a K1475 token already");
