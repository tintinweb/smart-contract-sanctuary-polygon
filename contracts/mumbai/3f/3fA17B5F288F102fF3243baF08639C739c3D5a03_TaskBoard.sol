...
contract TaskBoard is TaqAccessControl {
    ...
    /// @param tag1 The task first tag.
    /// @param tag2 The task second tag.
    ...
    function create(
        ...
        bytes32 tag1,
        bytes32 tag2
    ) external onlyTaqtiler returns (uint256 taskId) {
        ...
        emit Created(taskId, msg.sender);
    }
    ...
    /// @notice Get all tasks in this contract
    /// @return allTasks List of all tasks that were created
    function getAllTasks() external view returns (Task[] memory allTasks) {
        allTasks = new Task[](taskCounter);
