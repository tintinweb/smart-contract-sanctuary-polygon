...
contract Roulette is VRFConsumerBaseV2, Ownable {
  ...
  uint32 private constant CALL_BACK_GAS_LIMIT = 100_000;
  uint32 private constant MAX_CALL_BACK_GAS_LIMIT = 2500000;
  ...
  */
  ...
  event RouletteStarted(uint256 betId, uint256 time);
  ...
  */
  ...
  function bet(
    ...
    require(
      block.timestamp < betIdToBets[currentBetId].time + 6000000,
  ...
  */
  function checkWinner(uint256 number, uint256 _requestId) internal {
    uint256 betId = requestIdToBetId[_requestId];
    for (uint256 i = 0; i < players[betId].length; i++) {
      bool won = false;
  ...
  */
  ...
  function ReserveInToken(address ERC20Address) public view returns (uint256) {
    return IERC20(ERC20Address).balanceOf(address(this));
  }

  //Owner is allowed to withdraw the contract's matic balance.
  function MaticWithdraw(address _receiver, uint256 _amount)
    external
    onlyOwner
  {
    require(
      ReserveInMatic() >= _amount,
