...
contract FamosoStaking {
    ...
    uint256 public lockedTime = 600; // 14 * 24 * 3600; // 14days
    ...
    function getReward(address account) public view returns (uint256){
        ...
        uint256 accDebt = currentTimeStamp.sub(user.lastCalculatedTimeStamp).mul(rewardPerSec).mul(user.amount) / totalAmount;
        return(user.rewardDebt + accDebt);
    }
}
