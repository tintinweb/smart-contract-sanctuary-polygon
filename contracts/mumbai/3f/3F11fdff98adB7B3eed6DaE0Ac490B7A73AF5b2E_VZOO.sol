    ...
    Ownable
{
    ...
    function ownerMint(uint256 _amount) external onlyOwner {
        uint256 mintedBalance = addressMintedBalance[owner()];
        require(
            (totalSupply() + _amount) <= maxSupply(),
            "total supply limit reached"
        );
        require(
            (mintedBalance + _amount) <= maxMintTeam(),
        ...
        whenNotPaused
    {
        ...
        require(setNumItemsAllocated(_optionId), "failed to allocate items");
        uint256 allocatedItems = numItemsAllocated();
        require(canMint(allocatedItems), "total supply limit reached");
        require(canMintAndOwn(_toAddress, allocatedItems));
        require(checkValue(msg.value, allocatedItems));

        for (uint256 i = 0; i < numItemsAllocated(); i++) {
            mintTo(_toAddress);
        }
    }
    ...
    /// Returns the number of items allocated for minting
    /// @return number of items allocated
    function numItemsAllocated() internal view returns (uint256) {
        return _numItemsAllocated;
    }
    ...
    function canMint(uint256 _amount) public view returns (bool) {
        return totalSupply() <= (maxSupply() - _amount);
    }
    ...
    function canMintAndOwn(
        address _toAddress,
        uint256 requestedNumItems
    ) public view returns (bool) {
        uint256 mintedBalance = addressMintedBalance[_toAddress];
        if (_toAddress == owner()) {
            require(
                mintedBalance + requestedNumItems <= maxMintTeam(),
                "mint limit for team exceeded"
            );
            return true;
        }
        ...
        require(
            mintedBalance + requestedNumItems <= maxMintPerAddress(),
            "mint limit for address exceeded"
        );
        require(
            addressBalance + requestedNumItems <= maxNFTPerAddress(),
