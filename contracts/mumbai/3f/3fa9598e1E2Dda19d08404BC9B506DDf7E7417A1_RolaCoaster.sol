...
import "./AbstractRolaCoaster.sol";

contract RolaCoaster is AbstractRolaCoaster{
    ...
    uint8 constant ROLA = 0;
    uint8 constant ROLAGRAM_MINT_AMOUNT = 1;
    uint8 public rolaDecimals;
    ...
    modifier onlyMaintainer() {
        require(maintainer == _msgSender(), "RolaCoaster: caller is not the maintainer");
        _;
    }
    ...
    function uri(uint id) public view override returns (string memory) {
        require(exists(id), "RolaCoaster: URI nonexistent token");
        return tokenURI[id];
    }
    ...
    /// @param account The account address to mint ROLAGRAM and trasnfer ROLA form it 
    /// @param amountRola Amount of the ROlA tokens to transfer to treasury for minting the ROLAGRAM
    
    function mintRola(address account, uint256 amountRola) external override onlyMaintainer whenNotPaused nonReentrant whenNotBlackListedUser(account) {
        require(amountRola > 0, "RolaCoaster: ROLA amount should be greater than zero.");
        _mint(account, ROLA, amountRola*(10**rolaDecimals), '');
        emit RolaMinted(account, amountRola);
    }

    /// @dev This is the transferRolaAndMintRolaGram function. It is used for transferring the callers ROLA tokens and mint your ROLAGRAM NFT.
    /// @dev Only the maintainer or artist can call this function
    /// @param account The account address to mint ROLAGRAM and trasnfer ROLA form it 
    /// @param amountRola Amount of the ROlA tokens to transfer to treasury for minting the ROLAGRAM
    /// @param tokenUri Token Uri of the metadata ROLAGRAM minted

    function transferRolaAndMintRolaGram(address account, uint256 amountRola, string memory tokenUri) external override onlyMaintainer whenNotPaused nonReentrant whenNotBlackListedUser(account) {
        require(account != ZERO_ADDRESS, "RolaCoaster: Cannot send Rola from Zero Address.");
        require(amountRola > 0, "RolaCoaster: ROLA amount should be greater than zero.");
        safeTransferFrom(account, treasury, ROLA, amountRola*(10**rolaDecimals), '');
        nextTokenIndex.increment();
        uint256 tokenID = nextTokenIndex.current();
        tokenURI[tokenID] = tokenUri;
        _mint(account, tokenID, ROLAGRAM_MINT_AMOUNT, '');
        emit NewRolaGramMinted(account);
    }
    ...
    function airdropRola(address[] memory assigned, uint16[] memory quantity) external override onlyOwner whenNotPaused nonReentrant {
        ...
        for (uint8 index = 0; index < assigned.length; index++) {
            if(!_isBlackListUser(assigned[index])){
                _mint(assigned[index], ROLA, quantity[index], "");
            }
        }
    }
    ...
    function airdropRolaGram(address[] memory assigned, string[] memory tokenUri) external override onlyOwner whenNotPaused nonReentrant {
        require(assigned.length == tokenUri.length, "RolaCoaster: Incorrect parameter length");
        for (uint8 index = 0; index < assigned.length; index++) {
            if(!_isBlackListUser(assigned[index])){
                nextTokenIndex.increment();
                uint256 tokenID = nextTokenIndex.current();
                tokenURI[tokenID] = tokenUri[index];
                _mint(assigned[index], tokenID, ROLAGRAM_MINT_AMOUNT, "");
            }
        }
    }
    ...
    function updateTreasuryAddress(address newTreasuryAddress) external override onlyOwner whenNotPaused {
        require(maintainer != newTreasuryAddress,"RolaCoaster: The new treasury address must be different from the old one");
        maintainer = newTreasuryAddress;
    ...
    /// @dev This is the withdraw function. It is used by the owner to withdraw the MATIC amount to the owners address present in contract.
    /// @dev Only the owner or artist can call this function

    function withdraw() external override onlyOwner whenNotPaused nonReentrant {
        uint256 contractBalance = address(this).balance;
        require(contractBalance > 0, "RolaCoaster: Contract balance should be greater than zero.");
    ...
    /// @dev This function would be calledbefore each transfer token transaction in the contract
...
interface IRolaCoaster {
     ...
     */
    event NewRolaGramMinted(address account);

    /**
     * @dev Mint the ROLA tokens to the account
     */
    function mintRola(address account, uint256 amount) external;

    /**
     * @dev Transfer the ROLA tokens to treasury to mint a new ROLAGRAM
     */
    function transferRolaAndMintRolaGram(address account, uint256 amountRola, string memory tokenUri) external;

    /**
     * @dev Airdrop the ROLA tokens to a set of users
     */
    function airdropRola(address[] memory assigned, uint16[] memory quantity) external;
    ...
    /**
     * @dev Withdraw the MATIC present from the contract
     */
...
abstract contract AbstractRolaCoaster is ERC1155Supply, Ownable, Pausable, ReentrancyGuard, BlackList, IRolaCoaster {
