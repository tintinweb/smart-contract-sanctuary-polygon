...
contract Mid is Royalty, AccessControl, BaseRegistrarImplementation, Pausable, TokenURISetting {
    struct Properties{
        uint256 birthday;
        string rarity;
        uint256 seed;
    ...
    constructor(address usdt_, NNS nns_, address timelockController, address erc20TransferHelper_) BaseRegistrarImplementation("MetaID", "MID", nns_) Royalty(erc20TransferHelper_){
        ...
        _setupRole(META_ID_CONTRACT_ROLE, timelockController);
    }

    /**
     * @dev Initializes the contract with the given parameters.
     */
    function init(address micAddress_, address payable landAddress_, address payable midBank_, address payable midLandBank_, address payable alchemistBank_, address payable poolBank_) external {
        // require(hasRole(META_ID_ADMIN, msg.sender), "init only land admin");
        // require(_landAddress == address(0) && _landAddress != address(0), "init already initialized");
        _landAddress = landAddress_;
        _micToken =  IMIC(micAddress_);
        _poolBank = poolBank_;
        ROYALTY = 1*(10**18);
        super.init(midBank_, midLandBank_, alchemistBank_, micAddress_);
    }
    ...
    function mint(string memory name) nonReentrant external {
        require(IERC20(_usdt).allowance(_msgSender(), address(this)) >= currentPrice(), "Not enough approve tokens");
        require(_midBank != address(0) && _midLandBank != address(0) && _alchemistBank != address(0) && _poolBank != address(0), "Address cannot be 0");
        IERC20(_usdt).safeTransferFrom(msg.sender, _poolBank, currentPrice());
        mintRoute(name, msg.sender);
    }
    function mintRoute(string memory name, address owner_) public {
        require(name.lenOfChars() > 0 && name.lenOfChars() <= 8, "");
        ...
        _micToken.mint(_msgSender(), 300 ether);
        _micToken.mint(_midBank, 200 ether);
        ...
        _properties[tokenId] = Properties(block.timestamp, _rarity, seed);
    }
...
abstract contract Royalty is ReentrancyGuard, Context, IncomingTransferSupport{
     ...
     * @dev Initializes the contract with the given parameters.
     */
    function init(address payable midBank_, address payable midLandBank_, address payable alchemistBank_, address micAddress_) internal virtual{
        require(micAddress_ == address(0), "init already initialized");
        _midBank = midBank_;
    ...
    /**
     * @dev Returns the pool mid address.
     */
    function midBank() public view returns (address) {
        return _midBank;
    }
    ...
    function run(
        ...
        if(from != address(0) && to != address(0) && to != address(0x000000000000000000000000000000000000dEaD) && !to.isContract() && to != EOAOf[tokenId]){
            require(_midBank != address(0) && _midLandBank != address(0) && _alchemistBank != address(0), "Address cannot be 0");
            ...
            _MIDCoin.safeTransfer(_midBank, ROYALTY*2/10);
