...
contract VestingRouter {
    ...
    IERC20 private _musk;

    constructor (address[] memory vestingAddresses, uint[] memory vestingAmount, address muskToken) public {
        _vestingAddress = vestingAddresses;
        _vestingAmount = vestingAmount;
        _musk = IERC20(muskToken);
    }

    function allocateVestingFunds () public {
        for (uint i = lastAllocatedAddress; i < _vestingAddress.length; i++) {
            if (_musk.balanceOf(address(this)) < _vestingAmount[i] || gasleft() < 20000) {
                break;
            }
            lastAllocatedAddress++;
            _musk.transfer(_vestingAddress[i], _vestingAmount[i]);
        }
    }
...
contract Vesting is Ownable, ReentrancyGuard {
    ...
    uint public immutable NUMBER_OF_EPOCHS;
    ...
    constructor(address newOwner, address musk, uint startTime, uint totalBalance, uint nrOfEpochs) public {
        transferOwnership(newOwner);
        _musk = IERC20(musk);
        _startTime = startTime;
        totalDistributedBalance = totalBalance;
        NUMBER_OF_EPOCHS = nrOfEpochs;
    }
    ...
    function claimInternal (address to) internal {
        ...
        if (currentEpoch > NUMBER_OF_EPOCHS + 1) {
            ...
            _musk.transfer(to, _musk.balanceOf(address (this)));
            return;
        }
        ...
        if (balance > 0) {
            _musk.transfer(to, balance);
        }
    }

    function balance () public view returns (uint){
        return _musk.balanceOf(address (this));
    }
...
contract VestingTransferable is Vesting {
    ...
    constructor(address newClaimant, address newOwner, address muskToken, uint startTime, uint totalBalance, uint nrOfEpochs) Vesting(newOwner, muskToken, startTime, totalBalance, nrOfEpochs) public {
