...
contract AdAsset1155 is MultiNFTradeableUpgradeable {
    ...
    function __AdAsset_init(address proxyRegistryAddress_)
        public
        initializer
    {
        ...
        __MultiNFToken_init_unchained(
            "AdAsset1155",
            "AA11",
            "https://api.cros.io/ad-asset/v1/metadata/{id}/"
        );
        __MultiNFTradeable_init_unchained(proxyRegistryAddress_);
        __AdAsset_init_unchained();
    }

    /* solhint-disable-next-line  func-name-mixedcase, no-empty-blocks*/
    function __AdAsset_init_unchained() internal onlyInitializing {}

    /**
     * @dev contractURI to return the URI for storefront-level metadata for your contract
     */
    function contractURI() public pure returns (string memory) {
        return "https://adasset-api.cros.io/api/api/contract/adasset-erc1155/";
        /*
            return  "{"
...
import "./MultiNFTokenUpgradeable.sol";
 ...
 * functionality.
 */
abstract contract MultiNFTradeableUpgradeable is
    Initializable,
    MultiNFTokenUpgradeable,
    ERC2771ContextUpgradeable,
    OwnableUpgradeable
{
    using StringsUpgradeable for string;
    using SafeMathUpgradeable for uint256;
    using MultiNFTokenIdType for uint256;
    using MultiNFTokenIdType for uint128;

    address internal proxyRegistryAddress;
    mapping(uint128 => address) internal _tokenTypeCreators;

    /**
     * @dev Emitted when creator of tokens of token type `id` is changed from `from` to `to`.
     */
    event CreatorChange(address indexed from, address indexed to, uint128 id);
    ...
    /// @custom:oz-upgrades-unsafe-allow constructor
    // @todo Check for the modifiers on constructor to make it safe for upgradeable contracts
    constructor(address trustedForwarder)
        ERC2771ContextUpgradeable(trustedForwarder)
    {} // solhint-disable no-empty-blocks

    /* solhint-disable-next-line  func-name-mixedcase*/
    function __MultiNFTradeable_init(
        string memory name_,
        string memory symbol_,
        string memory uri_,
        address proxyRegistryAddress_
    ) internal onlyInitializing {
        __MultiNFToken_init_unchained(name_, symbol_, uri_);
        __Context_init_unchained();
        __ERC165_init_unchained();
        //__MinimalForwarder_init_unchained();
        //__EIP712_init_unchained(name_, "0.0.1");
    ...
    /* solhint-disable-next-line  func-name-mixedcase*/
    function __MultiNFTradeable_init_unchained(address proxyRegistryAddress_)
        internal
        onlyInitializing
    {
     ...
     */
    modifier creatorOnly(uint128 tokenType_) virtual {
        /* solhint-disable-next-line reason-string */
        require(
            _tokenTypeCreators[tokenType_] == _msgSender(),
            "MultiNFTradeableUpgradeable#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    ...
    modifier tokenOwnersOnly(uint256 tokenId_) virtual {
        ...
        require(
            ...
            "MultiNFTradeableUpgradeable#tokenOwnersOnly: ONLY_TOKENOWNERS_ALLOWED"
        );
        _;
    }
     ...
     **********************************************/
    /**
     * This is used instead of msg.sender as transactions won't be sent by the orignal token owner, but by OpenSea
     */
    function _msgSender()
        internal
        view
    ...
    /**
     * This is used instead of msg.data as transactions won't be sent by the orignal token owner, but by OpenSea
     */
    function _msgData()
        internal
        view
     ...
     */
    function setCreator(address to_, uint128[] memory ids_) public {
        /* solhint-disable-next-line reason-string */
        require(
            to_ != address(0),
            "MultiNFTradeableUpgradeable: INVALID_ADDRESS."
        );
    ...
    function createNonFungibleTokens(
        ...
        //console.log("**** DEBUG **** inside createNonFungibleTokens method");

        uint256 tt256 = tokenType_.toNonFungibleType256();
        //console.log("**** DEBUG **** createNonFungibleTokens - tokenType:%s", tt256);
        require(!exists(tt256), "token type already exists");
        _tokenTypeCreators[tokenType_] = _msgSender();
        
        //console.log("**** DEBUG **** calling _mintNonFungibleType:%s-%s-%s", initialOwner_, tt256, initialSupply_);

        _mintNonFungibleType(
            initialOwner_,
            tt256,
            initialSupply_,
            data_
    ...
    function mintNonFungibleToken(
        ...
        uint256 amount_,
        bytes memory data_
    ) public virtual creatorOnly(tokenType_) {
        _mintNonFungibleType(
            to_,
            tokenType_.toNonFungibleType256(),
            amount_,
            data_
        );
    }
    ...
    function batchMintNonFungibleTokens(
        address to_,
        uint128[] memory tokenTypes_,
        uint256[] memory amounts_,
        bytes memory data_
    ) public virtual {
        uint256[] memory tkidsArray = new uint256[](tokenTypes_.length);
        for (uint256 i = 0; i < tokenTypes_.length; i++) {
            /* solhint-disable-next-line reason-string */
            require(
                _tokenTypeCreators[tokenTypes_[i]] == _msgSender(),
                "MultiNFTradeableUpgradeable#batchMint: ONLY_CREATOR_ALLOWED"
            );
            tkidsArray[i] = tokenTypes_[i].toNonFungibleType256();
        }
        _mintNonFungibleTypeBatch(to_, tkidsArray, amounts_, data_);
    }
     ...
     */
    function _setCreator(address to_, uint128 tokenType_)
        internal
...
import "../libraries/MultiNFTokenIdType.sol";
import "hardhat/console.sol";

/**
 * @title MultiNFTokenUpgradeable
 * MultiNFTokenUpgradeable - Based on ERC1155 & ERC721 contract to support multiple non-fungible types and items.
 * The approach is to use common type information, same as when you create fungible id, to create non-fungible items
 * belonging to a type.
 */
abstract contract MultiNFTokenUpgradeable is
    Initializable,
    ContextUpgradeable,
    ERC165Upgradeable,
    IERC1155Upgradeable,
    ...
    IERC721MetadataUpgradeable
{
    ...
    /* solhint-disable-next-line  func-name-mixedcase, no-empty-blocks */
    function __MultiNFToken_init_unchained(
        string memory name_,
        string memory symbol_,
        string memory uri_
    ) internal onlyInitializing {
        ...
        returns (uint256)
    {
        ...
        if (id_.isNonFungibleItem()) {
            ...
            return _tokenTypeOwnerBalances[id_.getNonFungibleType()][account_];
        }
    }
    ...
    function approve(address, uint256) public virtual override {
        revert("IERC721: Only setApprovalForAll supported for ERCMultiToken");
    }
    ...
    function safeTransferFrom(
        ...
        require(
            tokenId_.isNonFungibleItem(),
            "IERC721: Transfer for non-fungible item only"
        );
        address operator = _msgSender();
        // specific instance (single) of the NFT item is transfered
        _transferTokenFrom(operator, from_, to_, tokenId_, 1, data_, true);
    }
    ...
    function transferFrom(
        ...
        address operator = _msgSender();
        // specific instance (single) of the NFT item is transfered
        _transferTokenFrom(operator, from_, to_, tokenId_, 1, "", false);
    }
    ...
    function safeTransferFrom(
        ...
        address operator = _msgSender();
        // specific instance (single) of the NFT item is transfered
        _transferTokenFrom(operator, from_, to_, id_, amount_, data_, true);
    }
    ...
    function totalSupply(uint256 id_) public view virtual returns (uint256) {
        if (id_.isNonFungibleItem()) {
            ...
            // if the id only has non-fungible type (lower128 are zero)
            // we return the token balances for the token-type
            uint128 tokenType = id_.getNonFungibleType();
     ...
     * - `id_` must refer to complete token-id of non-fungible type
     */
    function _burnNonFungibleItem(address from_, uint256 id_) internal virtual {
        require(from_ != address(0), "ERC1155: burn from the zero address");
        require(
            id_.isNonFungibleItem(),
            "ERC1155: burn single instance of non-fungible item"
        );
        uint8 amountToBurn = 1;

        address operator = _msgSender();
        uint256[] memory ids = _asSingletonArray(id_);
        uint256[] memory amounts = _asSingletonArray(amountToBurn);

        _beforeTokenTransfer(operator, from_, address(0), ids, amounts, "");

        /* burn logic */
        _burnMultiTokenType(from_, id_, amountToBurn);

        emit TransferSingle(operator, from_, address(0), id_, amountToBurn);
     ...
     * - `from_` cannot be the zero address
     */
    function _burnNonFungibleItemBatch(address from_, uint256[] memory ids_)
        internal
        virtual
    {
        require(from_ != address(0), "ERC1155: burn from the zero address");
        ...
        address operator = _msgSender();
        uint8 amountToBurn = 1;
        uint256[] memory amountsToBurnArray = _asSingletonArray(amountToBurn);
        ...
        for (uint256 i = 0; i < ids_.length; i++) {
            require(
                ids_[i].isNonFungibleItem(),
                "ERC1155: burn single instance of non-fungible item"
            );
            _burnMultiTokenType(from_, ids_[i], amountToBurn);
        }
    ...
    function _burnFromTokenType(
        ...
        require(
            id_.isNonFungibleType(),
            "ERC1155: burn amount tokens of fungible or non-fungible types"
        );
        ...
        /* burn logic */
        _burnMultiTokenType(from_, id_, amount_);

        emit TransferSingle(operator, from_, address(0), id_, amount_);
        _afterTokenTransfer(operator, from_, address(0), ids, amounts, "");
    }
     ...
     */
    function _burnFromTokenTypesBatch(
        address from_,
        uint256[] memory ids_,
        uint256[] memory amounts_
    ) internal virtual {
        ...
        for (uint256 i = 0; i < ids_.length; i++) {
            ...
            _burnMultiTokenType(from_, ids_[i], amounts_[i]);
        }

        emit TransferBatch(operator, from_, address(0), ids_, amounts_);
        _afterTokenTransfer(operator, from_, address(0), ids_, amounts_, "");
    }
    ...
    function _transferTokenFrom(
        ...
        if (safeTransfer_) {
            if (id_.isNonFungibleItem()) {
                require(
                    _checkOnERC721Received(operator, from_, to_, id_, data_),
    ...
    function _mintBaseTokenType(
         ...
         * mint logic
         ************************************/
        if (id_.isNonFungibleType()) {
            uint128 nfTokenType = id_.getNonFungibleType();
            ...
            for (
                ...
                uint256 tokid = nfTokenType.generateTokenIdFromTokenType128(i);
                _nfOwners[tokid] = to_;
            }
    ...
    function _burnMultiTokenType(
        ...
        if (id_.isNonFungibleItem()) {
            ...
            for (uint128 i = lastTokenIndex; j < amountToBurn && i > 0; i--) {
                uint256 tokid = tokenType.generateTokenIdFromTokenType256(i);
        ...
        _removeOwnerBalance(tokenType, from_, fromBalance.sub(amountToBurn));
        // update the token supply
        _removeTokenTypeSupply(tokenType, amountToBurn);
    }
    ...
    function _singleTransferFrom(
        ...
        if (id_.isNonFungibleItem()) {
            ...
            for (uint128 i = 0; i < lastTokenIndex && j < amount_; i++) {
                ...
                tempTokenId = nfTokenType.generateTokenIdFromTokenType128(
                    i + 1
                );
    ...
    function _allowedTokenTransfers(
        ...
        for (uint256 i = 0; i < ids_.length; i++) {
            if (ids_[i].isNonFungibleItem()) {
                allowedAmounts[i] = 1;
            } else if (ids_[i].isNonFungibleType()) {
