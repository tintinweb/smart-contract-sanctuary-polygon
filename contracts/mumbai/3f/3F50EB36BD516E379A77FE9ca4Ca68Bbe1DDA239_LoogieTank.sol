...
contract LoogieTank is ERC721Enumerable, Ownable, ERC1155Holder {
    ...
    mapping(address => string) public membershipByOwner;

    event ContainerMinted(uint256 id);
    event EntityReceived(uint256 id, uint256 value);

    constructor() ERC721("b.trax", "TRAX") {}
    ...
    function mintItem(string memory membershipId) public returns (uint256) {
        ...
        membershipByOwner[msg.sender] = membershipId;
    ...
    function tokenURI(uint256 tankId) public view override returns (string memory) {
        ...
        string memory _name = string(abi.encodePacked("b.trax member #", tankId.toString()));
        string memory description = string(abi.encodePacked("Ownership token for the marketplace that keeps track of your beats"));
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '", "tokenid": "', Strings.toString(tankId),
                                '", "attributes": [{',
                                '"trait_type": "Membership",',
                                '"value": "', membershipByOwner[msg.sender],
                                '", "}]}'
                            )
                        )
                    )
