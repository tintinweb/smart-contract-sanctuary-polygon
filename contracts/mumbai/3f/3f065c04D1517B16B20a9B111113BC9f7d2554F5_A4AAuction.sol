...
contract A4AAuction is Ownable, IERC721Receiver {
    ...
    address private a4A_NFT_address = 0xf0DFE1C98b0c8aC0c1f4D0fFb20D06cb8DA588B1;
...
/*
    function cancelBidForItem(uint256 itemID) external {
        require(block.timestamp < _auctionsEndDate[itemID]);
        require(_maxBidder[itemID] == _msgSender());

        (bool sentFee, ) = _maxBidder[itemID].call{value: _listedITemsMaxPrice[itemID]}("");
        require(sentFee, "Failed to send money back to previous highest bidder.");

        _listedITemsMaxPrice[itemID] = 0;
        _maxBidder[itemID] = address(0);
    }
*/
    //_maxBidderbyAuction[auctionId]
    function claimAuctionResult(uint256 itemID) external {
