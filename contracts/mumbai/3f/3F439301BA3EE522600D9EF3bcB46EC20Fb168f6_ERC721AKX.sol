...
contract ERC721AKX is ERC721Base {

    event CreateERC721AKX(address owner, string name, string symbol);

    function __ERC721AKX_init(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) external initializer {
        __ERC721AKX_init_unchained(_name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
        emit CreateERC721AKX(_msgSender(), _name, _symbol);
    }

    function __ERC721AKX_init_unchained(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address transferProxy, address lazyTransferProxy) internal {
        _setBaseURI(baseURI);
...
import "../LazyMint/IERC721LazyMint.sol";
import "../RoyaltiesUpgradeable/RoyaltiesV2Upgradeable.sol";
...
abstract contract HasContractURI is ERC165Upgradeable {
    ...
    function __HasContractURI_init_unchained(string memory _contractURI) internal initializer {
        ...
        supportsInterface(_INTERFACE_ID_CONTRACT_URI);
    }
...
abstract contract RoyaltiesV2Impl is AbstractRoyalties, RoyaltiesV2, IERC2981 {
    ...
    function getAKXV2Royalties(uint256 id) override external view returns (LibPart.Part[] memory) {
        return royalties[id];
    }
...
import "./LazyMint/LibERC721LazyMint.sol";
...
interface RoyaltiesV2 {
    ...
    function getAKXV2Royalties(uint256 id) external view returns (LibPart.Part[] memory);
}
...
import "@akxlab/libraries/LibSignature.sol";
