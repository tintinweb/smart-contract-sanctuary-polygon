...
contract VoxelVerse is ERC721, Ownable {
  using Strings for uint256;
  ...
  constructor() ERC721("SecondLand", "2LN") {
    setHiddenMetadataUri("ipfs://__CID__/hidden.json");  // 이부분을 채우려면 nft 발행을 한 url을 넣어줘야 함, How to create an NFT collection - Ultimate guide HashLips
    buildings.push(Building("ZERO", 0,0,0,0,0,0));
    buildings.push(Building("House", 1,3,2,2,0,3));
    buildings.push(Building("Kery House", 2,5,4,10,0,6));
    buildings.push(Building("Gallery", 1,6,2,10,0,0));
    buildings.push(Building("New House", 1,1,2,7,0,-7));
    buildings.push(Building("Cool House", 2,3,1,-9,0,-9));
    buildings.push(Building("Third House", 1,3,3,-4,0,2));
    buildings.push(Building("Ray House", 2,5,2,-2,0,5));
    buildings.push(Building("Forth House", 2,6,1,-10,0,4));
    ...
    buildings.push(Building("Fifth House", 3,2,2,-3,0,-2));
  }
