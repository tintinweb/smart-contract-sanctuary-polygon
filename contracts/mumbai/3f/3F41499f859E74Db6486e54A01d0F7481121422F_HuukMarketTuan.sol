    ...
    UUPSUpgradeable
{
    ...
    uint256 public xCreator; //15%
    uint256 public xPremium; // 2.5%
    ...
    // mapping(address => bool) public isHuukNFTs; // BỎ VÌ CÓ isHuukNFTPremiums ĐỦ RỒI
    ...
    mapping(address => mapping(bytes32 => uint256)) averageBuyCashInUSDT; // lastbuy price of NFT with id = keccak256(address, id) from user in USD
    mapping(address => mapping(bytes32 => uint256)) totalBuyQuantityInUSDT; // lastbuy price of NFT with id = keccak256(address, id) from user in USD
    ...
    function _createOrder(
        ...
        uint256 _price,
        address _paymentToken
    ) internal whenNotPaused returns (uint256 _orderId) {
        require(paymentMethod[_paymentToken], "token must be payment");
        // require(_quantity > 0, "quantity must > 0"); // VUTUAN: BỎ VÌ ĐÃ KHAI BÁO UINT
        bool isERC721 = IERC721Upgradeable(_tokenAddress).supportsInterface(
            _INTERFACE_ID_ERC721
        );
        uint256 balance;
        if (isERC721) {
            require(_quantity == 1, "Quantity must = 1");
            require(
                IERC721Upgradeable(_tokenAddress).ownerOf(_tokenId) == _owner,
                "Must be owner"
            );
            ...
            require(balance >= _quantity, "token-balance");
        ...
        bytes32 _id = keccak256(
            abi.encodePacked(_tokenAddress, _tokenId, _owner)
        );
    ...
    function _buy(
        ...
        // VUTUAN: TÍNH CHÍNH XÁC LƯỢNG BUYER PHẢI TRẢ
        uint256 exactPaymentAmount;
        if (_paymentToken != order.paymentToken) {
            orderAmount = estimateFromUSDT(_paymentToken, orderAmount);
        }

        if (isHuukNFTPremiums[order.tokenAddress]) {
            exactPaymentAmount =
                (orderAmount * (ZOOM_FEE + xPremium)) /
                ZOOM_FEE;
        } else {
        ...
        if (_paymentToken == huuk && discountForHuuk > 0) {
            exactPaymentAmount =
                (exactPaymentAmount * (ZOOM_FEE - discountForHuuk)) /
                ZOOM_FEE;
        }

        address payable sellerRef = getRefData(order.owner);
        address payable buyerRef = getRefData(msg.sender);

        if (discountForBuyer > 0 && buyerRef != address(0)) {
            // discount for buyer that have ref
            exactPaymentAmount =
                (exactPaymentAmount * (ZOOM_FEE - discountForBuyer)) /
                ZOOM_FEE;
        }
        ...
        // VUTUAN: TÍNH CHÍNH XÁC LƯỢNG BUYER PHẢI TRẢ - END
        ...
        return
            _match(
                ...
                _orderId,
                _quantity,
                estimateFromUSDT(_paymentToken, order.price),
                orderAmount,
                sellerRef,
                buyerRef
            ); // TRONG FN NÀY SẼ TÍNH TIỀN CHO REF, CHO CREATOR VÀ CHO SELLER
    }
    ...
    function _createBid(
        ...
        address _paymentToken, // payment method
        uint256 _expTime
    ) internal returns (uint256 _bidId) {
        require(paymentMethod[_paymentToken], "token must be payment");
        // require(_quantity > 0, "quantity must > 0"); // VUTUAN: BỎ VÌ ĐÃ KHAI BÁO UINT
        ...
        if (msg.value > 0) {
            require(msg.value >= _quantity * _price, "amount"); // VUTUAN: CÓ CHUYỂN MSG.VALUE NHƯNG SAO TOKEN LẠI KHÔNG THẤY CHUYỂN TOKEN ???
        } else {
    ...
    function _acceptBid(address _owner, uint256 _bidId)
        internal
        returns (bool)
    {
        ...
        uint256 exactPaymentAmount;
        bytes32 _id = keccak256(
            abi.encodePacked(bid.tokenAddress, bid.tokenId, _owner)
        );
        uint256 orderId = orderID[_id];
        Order memory order = orders[orderId];
        require(
            order.owner == _owner && order.isOnsale,
        ...
        uint256 orderAmount = bid.bidPrice * bid.quantity;
        if (isHuukNFTPremiums[order.tokenAddress]) {
            exactPaymentAmount =
                (orderAmount * (ZOOM_FEE + xPremium)) /
                ZOOM_FEE;
        ...
        return _updateBid(_bidId, bid.quantity);
    }
    ...
    function _paid(
        ...
        require(_to != address(0), "address must != 0");
        if (_token == address(0)) {
    ...
    function _updateOrder(
        ...
        // VUTUAN: ĐOẠN NÀY CẦN KIỂM TRA LẠI
        averageBuyCashInUSDT[_buyer][
            keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))
        ] =
            ((averageBuyCashInUSDT[_buyer][
                keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))
            ] *
                totalBuyQuantityInUSDT[_buyer][
                    keccak256(
                        abi.encodePacked(order.tokenAddress, order.tokenId)
                    )
                ] +
                estimateToUSDT(_paymentToken, _price) *
                _quantity) * ZOOM_FEE) /
            (totalBuyQuantityInUSDT[_buyer][
                keccak256(abi.encodePacked(order.tokenAddress, order.tokenId))
            ] + _quantity) /
        ...
        // VUTUAN: ĐOẠN NÀY CẦN KIỂM TRA LẠI - END

        // VUTUAN: CHUYỂN NFT CHO NGƯỜI MUA
        if (order.isERC721) {
        ...
        // VUTUAN: CHUYỂN NFT CHO NGƯỜI MUA - END
    ...
    function _match(
        ...
        if (yRefRate != 0) {
            if (buyerRef != address(0)) {
                _paid(
                    ...
                    (orderAmount *
                        (
                            isHuukNFTPremiums[order.tokenAddress]
                                ? xPremium
                                : xUser
                        ) *
                        yRefRate) / ZOOM_FEE
        ...
        // VUTUAN: TÍNH TIỀN REFERENCE - END

        // VUTUAN: TÍNH TIỀN TRẢ CHO CREATOR VÀ CHO SELLER
        amountToSeller =
            amountToSeller -
            ((orderAmount *
                (isHuukNFTPremiums[order.tokenAddress] ? xPremium : xUser)) /
                ZOOM_FEE);
        ...
        // VUTUAN: TÍNH TIỀN TRẢ CHO CREATOR VÀ CHO SELLER - END

        return _updateOrder(_buyer, _paymentToken, _orderId, _quantity, _price);
        // T
    }
    ...
    // VUTUAN: BỎ VÌ CÓ isHuukNFTPremiums ĐỦ RỒI
    ...
    function setPaymentMethod(address _token, bool _status)
        ...
        returns (bool)
    {
        require(_token != address(0), "Zero address is invalid");
        paymentMethod[_token] = _status;
        // IERC20Upgradeable(_token).approve(msg.sender, type(uint256).max);        // VUTUAN: BỎ VÌ KHÔNG CẦN APPROVE CHO CHÍNH MÌNH
    ...
    function lazyBuy(
        ...
        uint256 tokenId = _order.token.id > 0
            ? _order.token.id
            : _createToken(_order.token, _signature);
        ...
        returns (bool)
    {
        ...
        uint256 tokenId = _order.token.id > 0
            ? _order.token.id
            : _createToken(_order.token, _lazyBid.signature);
        ...
        return _acceptBid(_order.token.owner, bidId);
    }
    ...
    function _createToken(Token memory _token, bytes calldata _signature)
        ...
        returns (uint256)
    {
        require(_token.id > 0, "TokenId must = 0");
        if (_token.isERC721) {
            return
