...
contract Bank is IBank, ContextUpgradeable {
    ...
    event TestMint(bytes32 executedId, address indexed user, uint256 amount);
    ...
    function deposit(
        ...
        address _sender,
        uint256 _amount
    ) external override onlyTrustedPlugin(PluginsList.BANK_DEPOSIT, _version) returns(bool) {
    ...
    function withdraw(
        ...
        address _sender,
        uint256 _amount
    ) external override onlyTrustedPlugin(PluginsList.BANK_WITHDRAW, _version) returns(bool) {
        balances[_sender] -= _amount;
    ...
    function testMint(
        ...
        address _user,
        uint256 _amount
    ) external override onlyTrustedPlugin(PluginsList.FAUCET_TEST_MINT, _version) returns(bool) {
        require(_amount > 0, "PageBank: wrong amount");
        require(_user != address(0), "PageBank: wrong user");
        require(token.mint(_user, _amount), "PageBank: wrong mint of tokens");
        emit TestMint(_executedId, _user, _amount);
    ...
    function balanceOf(
        uint256 _version,
        address _user
    ) external view override onlyTrustedPlugin(PluginsList.BANK_BALANCE_OF, _version) returns (uint256) {
    ...
    function checkTrustedPlugin(bytes32 _checkedPluginName, uint256 _version) private view {
        require(
            ...
            "PageBank: caller is not the plugin"
        );
    }
}
