...
contract PerpetualUpdateLogic is PerpetualUpdateFunctions, IPerpetualUpdateLogic, IFunctionList {
    ...
    function setInitialFundAllocationWeight(uint24 _iPerpetualId) external override {
        require(msg.sender == address(this) || msg.sender == maintainer(), "onlythis/mntnr");
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        functionList[11] = this.setInitialFundAllocationWeight.selector;
        functionList[12] = this.updatePriceFeeds.selector;
