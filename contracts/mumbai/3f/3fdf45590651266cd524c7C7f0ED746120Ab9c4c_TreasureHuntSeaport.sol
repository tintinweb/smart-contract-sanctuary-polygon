...
contract TreasureHuntSeaport is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => address) public onSeaport;
    ...
    function join2Seaport(uint256 tokenID) public {
        ...
        require(seaportUsebility(tokenID, msg.sender), "Can't join now!");

        playersBySeaport[tokenID].add(msg.sender);
        seaportInfo[tokenID].Current++;
        onSeaport[msg.sender] = ownerOf(tokenID);
    }

    function seaportUsebility(uint256 tokenID, address account) public view returns(bool) {
    ...
    function isSeaportOwner(address add) public view returns(bool) {
        return  balanceOf(add) > 0;
    }

    function isOnSeaport(address add) public view returns(bool) {
        return onSeaport[add] != Zero || isSeaportOwner(add);
    }

    function getSeaportOwnerByPlayer(address add) public view returns(address) {
        return onSeaport[add];
    }

    function transferFrom(address from, address to, uint256 tokenId) public override(ERC721Upgradeable, IERC721Upgradeable) {
        require(isSeaportOwner(from) && !isSeaportOwner(to), "Can't transfer Seaport!"); 
