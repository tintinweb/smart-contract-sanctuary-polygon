...
// Fleato Contracts (last updated v1.0) (IPhysicalNFTSwap.sol)
...
interface IPhysicalNFTSwap {
    ...
    event NFTWithdrawn(bytes32 indexed chargeCode);
    event PaymentAndWithholdingRefunded(
        bytes32 indexed chargeCode,
        uint256 paymentCode
    );
    event NFTRefunded(bytes32 indexed chargeCode);
    ...
    event NFTScavenged(bytes32 indexed chargeCode);
}
...
contract PhysicalNFTSwap is ERC721Holder, ERC1155Holder, IPhysicalNFTSwap {
    ...
    modifier chargeExists(bytes32 _chargeCode) {
        require(hasCharge(_chargeCode), "chargeCode does not exist");
        _;
    }
    ...
    function _transferInNFT(bytes32 _chargeCode, address _from) internal {
    ...
    {
        ChargeContract storage c = charges[_chargeCode];
        //No need to check if this transfer is viable first, since it would just fail anyways
        if (c.nftContract != address(0) && c.nftInCustody == false) {
            _transferInNFT(_chargeCode, c.seller);
        }
