...
contract TestERC20mint is ERC721A, Ownable { // PaperVerification
    ...
    uint private constant MAX_MINT_AMOUNT = 50;

    mapping(address => uint) public amountNFTsperWallet;

    constructor(
        string memory _baseURI
    ) ERC721A("Dems", "DEMS") { //  PaperVerification(_paperKey)  address _paperKey, address _tokenAddress, 
        baseURI = _baseURI;
        // tokenAddress = IERC20(_tokenAddress);
    }

    function getClaimIneligibilityReason(address _userWallet, uint256 _quantity) external view returns (string memory) {
        if (amountNFTsperWallet[_userWallet] + _quantity > MAX_MINT_AMOUNT) {
            return "Invalid total mint amount!";
        } else if (totalSupply() + _quantity > MAX_SUPPLY) {
    ...
    modifier mintCompliance(uint256 _quantity) {
        require(amountNFTsperWallet[msg.sender] + _quantity <= MAX_MINT_AMOUNT, "You already claimed this NFT");
        require(_quantity == MAX_MINT_AMOUNT, "You can claim only 1 NFT");
        require(totalSupply() + _quantity <= MAX_SUPPLY, "Not enough NFTs left!");
        _;
    }

    // function paperMint(
    //     PaperMintData.MintData calldata _mintData, bytes memory data) external payable onlyPaper(_mintData) mintCompliance(_mintData.quantity) {
    //     amountNFTsperWallet[msg.sender] += _mintData.quantity;
    //     _safeMint(_mintData.recipient, _mintData.quantity, data);
    // }

    function claimTo(address _account, uint256 _quantity) external payable mintCompliance(_quantity) {
