 ...
 */
contract ContractsFactory is AccessControl {
    ...
    /*
    * @dev Internal function which deploys contract bytecode using assembly create2 command meaning address won't
    * be affected by `msg.sender` nonce, but by salt provided by user
    */
    function _deployContractInstance(
        ...
        assembly {
            addr := create2(0, add(creationBytecode, 32), mload(creationBytecode), salt)
        }

        // solhint-disable-next-line reason-string
        require(isContract(addr), "ContractsFactory: Contract was not been deployed. Check contract bytecode and contract params");

        return addr;
    }

    /*
    * @dev Deploys contract bytecode using assembly create2 command meaning address won't be affected by `msg.sender`
    * nonce, but by salt provided by user
    */
    function deployContractInstance(
        bytes memory bytecode,
        bytes memory constructorParams,
        bytes32 salt
    ) external virtual onlyRole(DEFAULT_ADMIN_ROLE) returns(address) {
        return _deployContractInstance(bytecode, constructorParams, salt);
    }

    /*
    * @dev Deploys contract bytecode using assembly create2 command meaning address won't be affected by `msg.sender`
    * nonce, but by salt provided by user and additionally calls contract using encoded function and params (useful
    * especially in cases of smart contracts with 'initialize' functionality)
    */
    function deployContractInstanceWithCall(
        bytes memory bytecode,
        bytes memory constructorParams,
        bytes32 salt,
        bytes memory callData
    ) external payable virtual onlyRole(DEFAULT_ADMIN_ROLE) returns(address, bytes memory) {
        address payable newContract = payable(_deployContractInstance(bytecode, constructorParams, salt));

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory data) = newContract.call{value: msg.value}(callData);
        require(success, "ContractsFactory: call failed");
...
contract PaypoolV1ERC20 is NonblockingLzAppUpgradeable, ERC20Upgradeable, PausableUpgradeable, IOFTUpgradeable {
    ...
    */
    function initialize(address _admin, ILayerZeroEndpoint _lzEndpoint, uint256 _globalSupply, uint256 _localSupply, string memory _tokenName, string memory _ticker) public initializer {
        __PaypoolV1ERC20_init(_admin, _lzEndpoint, _globalSupply, _localSupply, _tokenName, _ticker);
    }

    // solhint-disable-next-line func-name-mixedcase
    function __PaypoolV1ERC20_init(address _admin, ILayerZeroEndpoint _lzEndpoint, uint256 _globalSupply, uint256 _localSupply, string memory _tokenName, string memory _ticker) internal onlyInitializing {
        __PaypoolV1ERC20_init_unchained(_admin, _globalSupply, _localSupply);
    ...
    // solhint-disable-next-line func-name-mixedcase
    function __PaypoolV1ERC20_init_unchained(address _admin, uint256 _globalSupply, uint256 _localSupply) internal onlyInitializing {
