...
interface IPrematchCoreExtended is IPrematchCore {
    function azuroBet() external view returns (address);
...
contract X2OrNothing is VRFConsumer, OwnableUpgradeable, IX2OrNothing {
    ...
    IPrematchCoreExtended public core;
    ILPExtended public lp;

    address public payableToken;
    uint128 public lockedLiquidity;
    uint128 public minBet;
    uint64 public resultPeriod;
    uint64 public payoutMultiplier;
    uint64 public margin;
    ...
    function initialize(
        ...
        core = IPrematchCoreExtended(core_);
        lp = ILPExtended(core.lp());
    ...
    function betRedeem(uint256 tokenId) external {
        ...
        address azuroBet = core.azuroBet();

        IAzuroBet(azuroBet).transferFrom(msg.sender, address(this), tokenId);
        lp.withdrawPayout(address(core), tokenId, false);
    ...
    function fulfillRandomWords(
        ...
        emit GameResultFulfilled(requestId, game.amount, payout);
    ...
    function _putBet(address bettor, uint128 amount) internal {
        ...
        emit NewBet(bettor, requestId, amount);
    }
...
interface IX2OrNothing is IOwnable {
    ...
    event LiquidityWithdrawn(address user, uint128 amount);

    event NewBet(address player, uint256 requestId, uint128 amount);
    event BetPayout(address player, uint256 requestId, uint128 payout);
