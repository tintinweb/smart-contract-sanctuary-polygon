...
contract SavageStreetTester is ERC721r, Ownable, Pausable, ReentrancyGuard {
    ...
    //function to set only White Listed allowed to mint
    bool public onlyWhitelisted = true;

    function setOnlyWhitelisted(bool _state) public onlyOwner {
    onlyWhitelisted = _state;
    }
    /* end of whitelist section */
    ...
    returns (string memory)
  {
    ...
    //mint section
      ...
      //variables neccesary to build proper minting function
    ...
    function mintWL(uint quantity) external payable whenNotPaused{
        if(onlyWhitelisted = true){
            //require(balanceOf(msg.sender) + quantity <= maxMintForWhitelist, "Too many minted");
            require(iswhitelisted(msg.sender) == true, "Ah Ah Ah, you're not on the whitelist.");
            require(quantity <= mintThrottle, "Sorry the throttle is set to 2 per TX to be fair.");
            require(msg.value >= prePrice * quantity,"You didnt send enough to mint a NFT.");
           _mintRandom(msg.sender, quantity);
        } else {
          revert("sorry, closed. public minting is active");
          }
    }
    function mintPub(uint quantity) external payable whenNotPaused{
      if(onlyWhitelisted == false) {
        require(msg.value >= price * quantity,"Not enough matic sent... use the damn dapp wanna be hacker.");
        _mintRandom(msg.sender, quantity);
      } else {
        revert("sorry. only whitelist minting currently");
      }
    }
    ...
    function distributeShares() public onlyOwner nonReentrant {
     (bool payee, ) = payable(0xf53bE2592C16A92Ba2d3fEF398CF6D3008dF41e0).call{value: address(this).balance * 13 / 100}("");
     require(payee);
