 ...
 */
contract CelestialCastlePolyland is OwnableUpgradeable, UUPSUpgradeable, IERC721ReceiverUpgradeable, ICelestialCastle {
	bool public isTravelEnabled;
	/// @notice Celestial portal contract.
	PortalLike public portalPolyland;
	...
	modifier onlyPortal() {
		require(msg.sender == address(portalPolyland) || msg.sender == owner(), "CelestialCastle: sender is not the portal");
		_;
	}
	...
	function initialize(
		...
		portalPolyland = PortalLike(newPortal);
	...
	function travel(
		...
		if (freakIds.length > 0) {
			...
			calls[callsIndex] = abi.encode(CelestialPortalMessages.RETRIEVE_FREAKS, abi.encode(freakIds, freaks));
			callsIndex++;
		}
		...
		if (celestialIds.length > 0) {
			...
			calls[callsIndex] = abi.encode(CelestialPortalMessages.RETRIEVE_CELESTIALS, abi.encode(celestialIds, celestials));
			callsIndex++;
		}

		if (fbxAmount > 0) {
			freaksBucks.burn(msg.sender, fbxAmount);
			calls[callsIndex] = abi.encode(CelestialPortalMessages.RETRIEVE_FBX, abi.encode(fbxAmount));
		}
		portalPolyland.sendMessage(abi.encode(msg.sender, calls));
	}
