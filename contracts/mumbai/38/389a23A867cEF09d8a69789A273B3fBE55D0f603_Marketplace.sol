  ...
  ERC2771ContextUpgradeable
{
  ...
  function createOffer(uint256 tokenId, uint256 price) external {
    ...
    require(!offer.exist, "Offer already exist");
    offer.exist = true;
    offer.publisher = _msgSender();
    offer.price = price;
    mokensNFT.safeTransferFrom(_msgSender(), address(this), tokenId);
    emit NewOffer(offer.publisher, tokenId, offer.price);
  }

  function updateOfferPrice(uint256 tokenId, uint256 price) external {
    Offer storage offer = offers[tokenId];
    require(offer.exist, "Offer not exist");
    require(offer.publisher == _msgSender(), "Invalid sender");
    offer.price = price;
    emit UpdateOfferPrice(tokenId, offer.price);
  }
  ...
  function cancelOffer(uint256 tokenId) external {
    ...
    require(offer.publisher == _msgSender(), "Invalid sender");
    offer.exist = false;
    mokensNFT.safeTransferFrom(address(this), offer.publisher, tokenId);
    emit CancelOffer(tokenId);
  }

  function takeOffer(uint256 tokenId, address paymentToken) external payable {
    Offer storage offer = offers[tokenId];
    require(offer.exist, "Offer not exist");
    require(
    ...
    uint256 priceInPayToken = (offer.price * paymentTokenPrice) /
    ...
    if (paymentToken != address(0)) {
      IERC20Upgradeable paymentTokenContract = IERC20Upgradeable(paymentToken);
      require(
      ...
      require(
        paymentTokenContract.transferFrom(_msgSender(), vault, localCommission),
        "Can't transfer commission"
      );
    } else {
      // Native
      require(msg.value >= priceInPayToken, "Not enough funds transferred");
      payable(offer.publisher).transfer(priceInPayToken - localCommission);
    ...
    emit OfferTaken(tokenId, localCommission, paymentToken);
  }
...
interface IMokensNFT is IERC721Upgradeable {
