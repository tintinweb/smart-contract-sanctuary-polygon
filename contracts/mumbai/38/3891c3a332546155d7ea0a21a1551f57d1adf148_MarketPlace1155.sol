    ...
    IERC1155Receiver
{
    ...
    // State Variables
    // mapping of NFT contract addresses to token IDs that themselves point to AuctionListing data structs
    mapping(bytes32 => AuctionListing[]) private _listings;
    ...
    // event
    event TokenReceiptContractsUpdated(address tokenReceipt);
    ...
    /// @notice this event is emitted when a listing is updated by lister
    event ItemUpdated(
        address indexed nftAddress,
        uint256 indexed tokenId,
        address indexed seller,
    ...
    modifier isListSellerAndListed(
    ...
    {
        require(
            _platformFeePercent >= 0 && _platformFeePercent <= 10000,
            "PlatformFeePercent must between 0 and 10000"
        );
    ...
    /// @param _duration end time duration (in day)
    function listAuctionItem(
    ...
    ) external isOwner(_nftAddress, _nftId, _nftAmount, msg.sender) {
        bytes32 hash = _getListingHash(msg.sender, _nftAddress, _nftId);
        require(_startPrice > 0 && _endPrice > 0, "Price must be above Zero");
        require(
            approvedPurchaseTokenAddresses[_tokenAddress],
            "Token is not approved"
        );
        require(_nftAmount > 0, "Amount must be above Zero");
        require(
        ...
        for (
            listingIndex = 0;
            listingIndex < _listings[hash].length;
            listingIndex++
        ) {
            if (_listings[hash][listingIndex].startPrice == 0) {
                // set listing data
                _listings[hash][listingIndex] = AuctionListing(
                    msg.sender,
                    _nftAmount,
                    _tokenAddress,
                    _startPrice,
        ...
        if (listingIndex == _listings[hash].length) {
            // set listing data
            _listings[hash].push(
                AuctionListing(
                    msg.sender,
        ...
        _createReceipts(_nftAddress, _nftId, _nftAmount);
    ...
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    /// @param _listingIndex listing Index
    /// @param _tokenAddress token address
    /// @param _startPrice start price
    /// @param _endPrice end price
    /// @param _duration end time duration (in day)
    function updateAuctionItem(
        ...
        // check is there are this list or not
        uint256 listedAmount = _listings[hash][_listingIndex].amount;
        require(listedAmount > 0, "There are no this listing");

        _listings[hash][_listingIndex] = AuctionListing(
            msg.sender,
            listedAmount,
            _tokenAddress,
            _startPrice,
        ...
        )
    {
        bytes32 hash = _getListingHash(msg.sender, _nftAddress, _nftId);
        uint256 listedAmount = _listings[hash][_listingIndex].amount;
        delete (_listings[hash][_listingIndex]);
        ...
        _burnReceipts(msg.sender, _nftAddress, _nftId, listedAmount);

        emit ItemCanceled(_nftAddress, _nftId, msg.sender, _listingIndex);
    }
    ...
    function buyAuctionItem(
        ...
        AuctionListing memory listedItem = _listings[hash][_listingIndex];

        require(_listTime == listedItem.listTime, "list time is not correct");
        ...
        uint256 targetPrice = getAuctionItemPrice(
            _sellerAddress,
            _nftAddress,
            _nftId,
            _listingIndex
        ...
        uint256 payoutWalletReceive = (targetPrice * _nftAmount) -
            sellerReceive;
        IERC20Upgradeable(listedItem.purchaseTokenAddress).safeTransferFrom(
            msg.sender,
            listedItem.seller,
            sellerReceive
        );
        ...
        if ((listedItem.amount - _nftAmount) > 0) {
            _listings[hash][_listingIndex].amount -= _nftAmount;
        ...
        // burn seller stake token
        _burnReceipts(listedItem.seller, _nftAddress, _nftId, _nftAmount);
    ...
    function getAuctionListing(
        ...
        return
            _listings[_getListingHash(_sellerAddress, _nftAddress, _nftId)][
                _listingIndex
            ];
    }
    ...
    /// @notice create token receipts, to solve handle Stack too deep
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    /// @param _nftAmount nft amount
    function _createReceipts(
        address _nftAddress,
        uint256 _nftId,
        uint256 _nftAmount
    ) internal {
    ...
    /// @notice burn token receipts, to solve handle Stack too deep
    /// @param _nftOwnerAddress nft owner address
    /// @param _nftAddress nft address
    /// @param _nftId nft ID
    /// @param _nftAmount nft amount
    function _burnReceipts(
