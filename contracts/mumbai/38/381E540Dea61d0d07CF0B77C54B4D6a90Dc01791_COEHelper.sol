...
contract COEHelper {
  ...
  event TestEvent(uint256 price, uint256 convertedCost);
  ...
  function purchaseWithToken(
    ...
    if (_tokenAddress == aegAddress) {
      ...
      emit TestEvent(_totalCost, convertedCost);
    ...
    //emit an event to show price
    emit TestEvent(uint256(price), uint256(convertedCost / uint256(price)));
