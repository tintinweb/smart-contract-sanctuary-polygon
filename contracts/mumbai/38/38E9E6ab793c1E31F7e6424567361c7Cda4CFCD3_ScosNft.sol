...
contract ScosNft is ERC721, AccessControlEnumerable, ERC721Pausable {
    ...
    function tokenOf(address owner_addr) external view returns (uint256[] memory) {
        require(msg.sender  == owner_addr, "11111"); 
        require(hasRole(ADMIN_ROLE, msg.sender), "2222"); 

        require(_ownedTokens[owner_addr].length > 0, "no data!");
        return _ownedTokens[owner_addr];    
    }

    function _removeTokenFrom(address from, uint256 tokenId) private {
        if (from != address(0)) {
            uint256 _tokenIndex = _ownedTokenIndex[tokenId];
            uint256 _lastTokenIndex = _ownedTokens[from].length - 1;
            uint256 _lastTokenId = _ownedTokens[from][_lastTokenIndex];

            // Insert the last token into the position previously occupied by the removed token.
            _ownedTokens[from][_tokenIndex] = _lastTokenId;
