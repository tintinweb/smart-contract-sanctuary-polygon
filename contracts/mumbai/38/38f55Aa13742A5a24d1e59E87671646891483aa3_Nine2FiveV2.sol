...
interface INinetoFive {
    ...
    function userReward(address _userAddress) external view returns (uint256);
}

interface IDep {
    function teams(uint256 team)
        external
        view
        returns (
            address,
...
contract Nine2FiveV2 is UUPSUpgradeable, OwnableUpgradeable {
    uint256 public constant min = 30 ether;

    uint256 public tot_taken = 0;
    uint256 revenue = 0;
    uint256 tot_deposits = 0;
    bool ismigrationover;
    uint256 roi = 15;
    uint256 public n_users = 0;
    uint256 weekly_budget;
    address public teamwallet;
    address public multisig;
    address public mkt;
    uint256[] public teamIds;

    address public _NinetoFive;
    address public Departments = address(0);
    ...
    mapping(address => ref_system) public refSystem;
    ...
    function initialize(
        ...
        require(initialized == false, "already init");
        initialized = true;
        ...
        Nine = INinetoFive(0xAd3b8D3c88c423534dd0BF19f98074d246C06df9);
        Dep = IDep(0xd7A95605E4Ef6B587c53d7Ba8f9C76f8D0058D8f);
        BusdInterface = IERC20(0xf290334f4028B7a6657f41edb39C7044C85bc641);
    }

    modifier onlyTeam() {
        require(
            msg.sender == wallets[0].wallet,
            "Only the dep can call this function"
        );
        _;
    }

    function Sync(uint256 _team) public {
        require(
            investments[msg.sender].hasinvested == false,
            "User has already registered"
        );
        (, uint256 _current, , bool _hasinvested) = getInvestmentDetails(
            msg.sender
        );
        if (_hasinvested == true) {
            (, uint256 _pastClaim, uint256 _pastdeadline) = Nine.claimTime(
                msg.sender
            );
            (, uint256 _pastwith) = Nine.totalWithdraw(msg.sender);
            ...
            Dep.addUser(msg.sender, _current, _team);
            ...
            Dep.addUser(msg.sender, 0, _team);

        }
    }

function prova() public view returns (address) {
        (address manager, , , ) = Dep.teams(investments[msg.sender].dep);
        return manager;
    }
    ...
    function deposit(address _ref, uint256 _amount) public {
        ...
        uint256 userLastPoints = investments[msg.sender].points;
        ...
        //FIXME ref system has a problem
        (address manager, , , ) = Dep.teams(investments[msg.sender].dep);
        uint256 ref_fee_add = _amount / 100;
        if (_ref != address(0) && _ref != msg.sender) {
            BusdInterface.transfer(_ref, ref_fee_add);
            BusdInterface.transfer(manager, ref_fee_add / 2);
            BusdInterface.transfer(Departments, ref_fee_add / 2);
        } else {
            BusdInterface.transfer(Departments, ref_fee_add * 2); //2%
        }
        ...
        uint256 totalPoints = userLastPoints + userCurrentInvestment;

        tot_deposits += _amount;
        investments[msg.sender] = user_investment_details(
            msg.sender,
            totalInvestment,
            0,
            totalPoints,
            investments[msg.sender].dep,
            true
        );

        Dep.addUser(msg.sender, userCurrentInvestment, investments[msg.sender].dep);
    ...
    function compound() public {
        ...
        uint256 totalInvestment = userLastInvestment + userCurrentInvestment;
        uint256 userNewPoints = investments[msg.sender].points +
            (userCurrentInvestment / 2);
    ...
    function userReward(address _userAddress) public view returns (uint256) {
        ...
        (, uint256 begin_deadline, ) = Nine.claimTime(_userAddress);
        if (
            begin_deadline > claimTime[_userAddress].startTime &&
            !ismigrationover
        ) {
            return 0;
        }
    ...
    function Teamwithdraw(address _tosend, uint256 _amount) external {
        require(
            msg.sender == multisig || msg.sender == Departments,
            "sender must be multisig or Dep contract"
        );
        BusdInterface.transfer(_tosend, _amount);
        tot_taken += _amount;
        emit Ownerwithdraw(multisig, _amount);
    }

    //@devuseful to dispaly balance without accounting for withdrawals and revenues
    function depositrevenue(uint256 amount) external onlyTeam {
    ...
    function change_dep(address _newDep) external onlyTeam {
        Departments = _newDep;
    }
    ...
    //@dev Update wallet addresses
    function updateWDev(address newDev) external {
        require(
    ...
    function changeRoi(uint256 _roi) external onlyTeam {
        require(_roi >= 1 && _roi <= 30, "only 0.5 to 3% daily");
        roi = _roi;
    }

    function incrementWeektoCalc(uint256 _weeks) external onlyTeam {
        require(_weeks > 0 && _weeks <= 10, "max 10 weeks allowed");
        require(
            MONDAY_START + (7 days * _weeks) < block.timestamp,
            "cannot be in the future"
        );
        MONDAY_START += 7 days * _weeks;
    }

    function changeMigrationStatus() external {
        require(msg.sender == wallets[7].wallet, "sender must be dev");
    ...
    function TotDep() public view returns (uint256) {
        return tot_deposits;
    }
