...
abstract contract APPcore is  Ownable{
    ...
    string public baseURI_lock = "https://nft.aopanda.ainy-llc.com/app_lock/metadata/";
...
abstract contract APPadmin is APPcore,AccessControl,ERC721AQueryable,ERC2981{
    ...
    function setMinterContract(IMinter _tokenMinter) external onlyAdmin{
    ...
    function _setEndStake(uint256 _tokenId,uint256 _timestamp,CouponSet.Coupon memory _coupon) internal {
        require(stakeInfo[_tokenId].get128(0) == 1,"Not staked");
        require(_timestamp > block.timestamp,"Time stamp has expired.");
        require(_coupon._isVerifiedCoupon(_tokenId,_timestamp,adminSigner),"invalid coupon");

        _setEndStake_core(_tokenId);
    }

    function _setEndStake_core(uint256 _tokenId) internal {
        if(address(tokenMinter) != address(0)){
            // Supports token distribution by staking
            tokenMinter.claimToken(_tokenId);
        }
    ...
    function setStartStake_admin(uint256 _tokenId) external onlyAdmin{
        _setStartStake(_tokenId);
    }

    function setEndStake_admin(uint256 _tokenId) external onlyAdmin {
        _setEndStake_core(_tokenId);
    }
}
...
contract APP is APPadmin {
    ...
    constructor() ERC721A('Test Panda 1021', 'TST') {
        ...
        _safeMint(0x4D8b598c5FA79ec93a85fbc982b2d2577CA13D54, 10);
    ...
    function _beforeTokenTransfers(
        ...
        if(block.timestamp > 1666429200){   // for test 日本時間 10/22 18:00にロックアップ終了
            return; // Lock-up completed
        }
        address[6] memory rewardaddress = [0x4D8b598c5FA79ec93a85fbc982b2d2577CA13D54,
    ...
    function setEndStake(uint256 _tokenId,uint256 _timestamp,CouponSet.Coupon memory _coupon) public {  //public for array
        require(msg.sender == ownerOf(_tokenId),"Not the holder of tokenId");
        _setEndStake(_tokenId,_timestamp,_coupon);
    }
    ...
    function setEndStakeArray(uint256[] calldata _tokenId, uint256 _timestamp,CouponSet.Coupon[] memory _coupon)external {
        require(_tokenId.length > 0,"list length is error");

        for(uint256 i = 1; i < _tokenId.length;i++){
            setEndStake(_tokenId[i],_timestamp,_coupon[i]);
        } 
    }
...
library CouponSet {
    ...
    function _isVerifiedCoupon(
        ...
        bytes32 digest = _createDigest(_tokenId, _timestamp);
    ...
    function _createDigest(
        ...
        return
            keccak256(abi.encode(_tokenId, _timestamp, msg.sender));
    }
}
