...
contract TestContractV1 is ERC721A, Ownable {
    using Strings for uint256;
    ...
    uint256 public whitelistMintPrice = 1;
    ...
    constructor(string memory _baseUrl) ERC721A("TEST CONTRACT V1", "TEST") {
    ...
    // function to check if whitelist sale is active or not
    function isWhitelistActive() public view returns (bool) {
        return whitelistIsActive;
    } 

    function whitelistedMint(uint256 _amount) public payable {
        require(whitelistIsActive, "Whitelist sale not active");
        ...
        require(balanceOf(msg.sender) + _amount <= whitelistMintLimit, "Exceeds mint limit per address");
        require(msg.value >= whitelistMintPrice*_amount, "Insufficient funds");
    ...
    function airdrop(address[] memory _airdropAddresses, uint256[] memory _airdropAmounts) public onlyOwner {
        for (uint256 i = 0; i < _airdropAddresses.length; i++) {
            address to = _airdropAddresses[i];
            uint256 amount = _airdropAmounts[i];
            _mintLoop(to, amount);
        }
    }
