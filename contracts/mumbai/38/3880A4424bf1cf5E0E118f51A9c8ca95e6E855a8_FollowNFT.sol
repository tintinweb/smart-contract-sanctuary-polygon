...
contract FollowNFT is OxtripNFTBase, IFollowNFT {
    struct Snapshot {
        uint128 blockNumber;
        uint128 value;
    }
    ...
    function name() public view override returns (string memory) {
        string memory handle = IOxtripHub(HUB).getHandle(_profileId);
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return IOxtripHub(HUB).getFollowNFTURI(_profileId);
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        ...
        address followModule = IOxtripHub(HUB).getFollowModule(_profileId);

        _moveDelegate(fromDelegatee, toDelegatee, 1);

        super._beforeTokenTransfer(from, to, tokenId);
        IOxtripHub(HUB).emitFollowNFTTransferEvent(_profileId, tokenId, from, to);
