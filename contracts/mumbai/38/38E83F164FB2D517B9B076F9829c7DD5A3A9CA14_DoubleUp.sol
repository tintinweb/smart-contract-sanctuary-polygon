...
contract DoubleUp is Ownable, ReentrancyGuard {
    ...
    event Prize(address indexed prizeAddress, uint earnings, bool earnedBonus);
    event Eliminated(address indexed eliminatedAddress, uint collateral);
    event Damaged(address indexed target);
    event Entered(address indexed newPlayer, uint collateral, uint immunityLevel);
    event BalanceUpdate(uint balance);

    constructor(){
        entries[msg.sender] = Entry(address(0), maximumEntry, 0);
        entriesLength = 1;
        head = msg.sender;
        tail = msg.sender;
    }
    ...
    function entryState() view external returns(address[] memory addresses, uint[] memory immunities, uint[] memory amounts, uint256 balance){
        addresses = new address[](entriesLength);
        immunities = new uint[](entriesLength);
        amounts = new uint[](entriesLength);

        if(entriesLength == 0){
            return (addresses, immunities, amounts, accumulatedBalance());
        }
    ...
    function enter(uint256 entry, uint256 immunityLevel) payable external nonReentrant {
        ...
        require(entries[msg.sender].collateral == 0, "Already in line!");

        entry *= entryToCost;
        require(msg.value != entry, "Wrong entry fee!");

        uint256 immunityCost = (entry / immunityCostDivisor) * immunityLevel;
        uint256 devTakeAndBonusTake = entry / take;

        entry -= (devTakeAndBonusTake + immunityCost + devTakeAndBonusTake);
        bonusPrize += devTakeAndBonusTake;
        devFunds += (devTakeAndBonusTake + immunityCost);

        Entry storage current = entries[head];
        uint256 currentActionableBalance = address(this).balance - devFunds - bonusPrize;
        for(uint max = 0;max < 6 && head != address(0) && ((current.collateral * 2) >= currentActionableBalance);++max){
            uint256 amountWon = current.collateral * 2;
            uint256 totalAmountWon = amountWon;
            if(eliminationCounter >= eliminationsUntilBonus){
                totalAmountWon+=bonusPrize;
                eliminationCounter = 0;
                bonusPrize = 0;
                emit Prize(head, amountWon + bonusPrize, true);
            }else{
                emit Prize(head, amountWon, false);
            }
            prizesPaid+=totalAmountWon;
        ...
        emit BalanceUpdate(currentActionableBalance);
        
        if(entriesLength >= 4){
            address currentAddress = head;
            current = entries[currentAddress];
            Entry storage next = entries[current.next];
            for(uint max = 0;max < 255 && next.immunity != 0;++max){
                next.immunity--;
                emit Damaged(current.next);
            ...
            if(next.next == address(0)){
                current.next = msg.sender;
            }else{
                current.next = next.next; // Clip
                entries[current.next].next = msg.sender;
            }
        }else{
            entriesLength++;
            if(head == address(0)){
                ...
                entries[tail].next = msg.sender;
            }
        }

        tail = msg.sender;
        uint immunityLevelFinal = immunityLevel > 0 ? (immunityLevel * 2) + (entry / 2) : 0;
        entries[msg.sender] = Entry(address(0), entry, immunityLevelFinal);
        emit Entered(msg.sender, entry, immunityLevelFinal);
    }
