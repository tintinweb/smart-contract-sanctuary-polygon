...
import "../helpers/RewardsDistributionRecipient.sol";
import "../helpers/OwnableUpgradeable.sol";
import "../helpers/Pausable.sol";

/**
 * @title CrowdToken for the LPStakingRewards
 * @notice LPStaking mechanism and rewards calculation
 */
contract StakingLP is Initializable, UUPSUpgradeable, OwnableUpgradeable ,Pausable,ReentrancyGuardUpgradeable , RewardsDistributionRecipient  {
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    function stakeLP(uint256 _amount,address _account)
    external nonReentrant notPaused updateReward(_account)
    {
    ...
    ) external nonReentrant notPaused updateReward(_originAccount)
    {
        require(opportunityContract != address(0),"LPStaking:Opportunity Contract address does not set yet.");
        require(msg.sender == opportunityContract, "LPStaking: Request is not from opportunity Contract ");
    ...
    //how much reward ,this account has earned so far for this Lp amount
    function earnedWithLimitedLP(address _account,uint256 _lpAmount) public view returns (uint256)
    {
        return
        (_lpAmount * (rewardPerToken() - lpStakeholders[_account].userRewardPerTokenPaid) / 1e18 )
        + lpStakeholders[_account].lpRewards;
