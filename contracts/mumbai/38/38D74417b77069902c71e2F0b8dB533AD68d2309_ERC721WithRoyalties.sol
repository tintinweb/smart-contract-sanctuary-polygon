...
contract ERC721WithRoyalties is ERC721 , ERC2981PerTokenRoyalties, Ownable {
    uint256 nextTokenId;
    ...
    constructor(string memory name_, string memory symbol_, string memory _initBaseURI, string memory _notRevealedUri)
    ...
    function tokenURI(uint256 checktokenId)
    public
    view
    virtual
    override
    returns (string memory)
  {
    ...
    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, Strings.toString(checktokenId), baseExtension))
        : "";
  }
