    ...
    BaseCarbonTonneStorage
{
    ...
    function setFeeRedeemPercentage(uint256 _feeRedeemPercentageInBase)
        ...
        onlyOwner
    {
        if (_feeRedeemPercentageInBase >= feeRedeemDivider) revert InvalidFee();
    ...
    function setFeeRedeemReceiver(address _feeRedeemReceiver)
        ...
        onlyOwner
    {
        if (_feeRedeemReceiver == address(0)) revert EmptyAddress();
    ...
    function setFeeRedeemBurnPercentage(uint256 _feeRedeemBurnPercentageInBase)
        ...
        onlyOwner
    {
        if (_feeRedeemBurnPercentageInBase >= feeRedeemDivider)
            revert InvalidFee();
    ...
    function setFeeRedeemBurnAddress(address _feeRedeemBurnAddress)
        ...
        onlyOwner
    {
        if (_feeRedeemBurnAddress == address(0)) revert EmptyAddress();
    ...
    {
        if (tco2s.length == 0) revert EmptyArray();
        ...
        whenNotPaused
    {
        ...
        if (remainingSpace == 0) revert FullPool();
        ...
        returns (bool)
    {
        ...
        if (isToucanContract) {
            ...
            if (internalBlackList[erc20Addr]) revert Blacklisted();

            if (!checkAttributeMatching(erc20Addr)) revert AttributeMismatch();
        } else {
            /// @dev If not Toucan native contract, check if address is whitelisted
            if (!externalWhiteList[erc20Addr]) revert NonWhitelisted();
        }
    ...
    function calculateRedeemFees(
        ...
        if (tco2Length != amounts.length) revert LengthMismatch();
    ...
    function redeemAndBurn(address tco2, uint256 amount)
        ...
        whenNotPaused
    {
        if (!redeemFeeExemptedTCO2s[tco2]) revert NotExempted();
    ...
    function redeemAuto(uint256 amount) external virtual whenNotPaused {
        if (amount > totalSupply()) revert ExceedsTotalSupply();
        ...
        if (remainingAmount != 0) revert NonZeroRemaining();
    }
    ...
    function redeemSingle(address erc20, uint256 amount)
        ...
        whenNotPaused
    {
        if (tokenBalances[erc20] < amount) revert ExceedsTCO2Supply();
...
error StartTimeTooOld();
error RegionNotAccepted();
error StandardNotAccepted();
error MethodologyNotAccepted();
error InvalidFee();
error EmptyAddress();
error LengthMismatch();
error NotExempted();
error ExceedsTotalSupply();
error NonZeroRemaining();
error ExceedsTCO2Supply();
