    ...
    GovernorTimelockControl
{
    ...
    returns (IGovernor.ProposalState,uint256,uint256,uint256,uint256,uint256,uint256) {
        ...
        return (state(proposalID),getVotes(account,block.number-1),proposalSnapshot(proposalID),proposalDeadline(proposalID),votes_against,votes_for,votes_other);
    }
    ...
    /// @dev Get Aggregate Info.
    /// @param proposalID Proposal ID.
    function getProposalAggregate2(uint256 proposalID, address account)
    public
    view
    returns (string memory,uint8,address,bytes32,Governor.ProposalState,uint256,uint256,uint256) {
        require(proposalsMap[proposalID],"Proposal not exist");
        //uint256 snap = proposalSnapshot(proposalID);
        //uint256 dead = proposalDeadline(proposalID);
        //Governor.ProposalState state = state(proposalID);
        (uint256 votes_against,uint256 votes_for,uint256 votes_other) = proposalVotes(proposalID);
        (string memory hash, uint8 typ, address owner, bytes32 groups) = getSingleProposal(proposalID);
        
        
        //return (proposals[proposalsIndexMap[proposalID]].hash, proposals[proposalsIndexMap[proposalID]].typ,  proposals[proposalsIndexMap[proposalID]].owner,  proposals[proposalsIndexMap[proposalID]].groups, 
        //    state(proposalID),getVotes(account,block.number-1),proposalSnapshot(proposalID),proposalDeadline(proposalID),votes_against,votes_for,votes_other);
        return (hash, typ, owner, groups, state(proposalID),getVotes(account,block.number-1),proposalSnapshot(proposalID),proposalDeadline(proposalID));
    }*/
}
