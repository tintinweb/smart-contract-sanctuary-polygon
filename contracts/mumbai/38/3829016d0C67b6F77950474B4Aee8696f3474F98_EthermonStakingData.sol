...
contract EthermonStakingData is EthermonStakingBasic {
    ...
    function updateSumTeamPower(uint256 _sumTeamPower) public onlyModerators {
        SumTeamPower = _sumTeamPower;
        emit TeamPowerLog(SumTeamPower);
    }

    function updateTokenReward(bytes memory _data, uint256 _timeElapsed)
        public
        onlyModerators
    {
        TokenData memory data = abi.decode(_data, (TokenData));
