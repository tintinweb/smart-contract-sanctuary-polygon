    ...
    VRFConsumerBaseV2
{
    ...
    function totalSupply() public view virtual returns (uint256) {
        return _maxSupply - _numAvailableTokens - _totalBurn;
    }

    function _totalMinted() public view virtual returns (uint256) {
    ...
    function _mintRandom(address to, uint32 quantity) internal virtual {
        require(_msgSender() == tx.origin, "Contract not allow to mint");
        uint32 tempGas = callbackGasLimit * quantity;
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        override
    {
        address to = requestToAddress[requestId];
        require(to != address(0), "ERC721: mint to the zero address");
        require(
            randomWords.length > 0,
            "ERC721r: need to mint at least one token"
        );

        // TODO: Probably don't need this as it will underflow and revert automatically in this case
        require(
            _numAvailableTokens >= randomWords.length,
        ...
        _balances[to] += randomWords.length;
    }
