...
contract marketingkangroos is ERC721, ReentrancyGuard, Ownable {
    uint256 MAX_MINTS = 5;

    uint16 MAX_SUPPLY = 1030;
    ...
    constructor() ERC721("marketingkangroos", "mkg") {
    ...
    function mint(uint8 quantity) external payable nonReentrant{
        ...
        require(CURRENT_SUPPLY + quantity <= MAX_SUPPLY, "Not enough tokens left");
        require(msg.value >= (quantity * mintRate), "Not enough ether sent");
        _safeMint(msg.sender, quantity);
        CURRENT_SUPPLY = CURRENT_SUPPLY + quantity;
    }

    function freeMint(uint8 quantity) external payable nonReentrant{
        require(quantity > 0, "Invalid quantity");
        uint256 balance = balanceOf(msg.sender);
        require(freeMintEnabled, "Unabel to mint at the moment");
        require(quantity + balance <= 2, "Exceeded the limit");
        require(CURRENT_SUPPLY + quantity <= MAX_SUPPLY, "Not enough tokens left");
        _safeMint(msg.sender, quantity);
        CURRENT_SUPPLY = CURRENT_SUPPLY + quantity;
    }
