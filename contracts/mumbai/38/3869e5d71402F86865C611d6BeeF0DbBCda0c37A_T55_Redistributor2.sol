...
pragma solidity 0.8.17;
  ...
  contract T55_Redistributor2 is IERC777Recipient, ReentrancyGuard
  {
    ...
    event TransferReceived(address from, address to, uint256 amount);
    event Redistributed(address from, address lastSender, uint256 amount);

  constructor (address TRTAddress)
  {
    IERC1820Registry(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24).
    setInterfaceImplementer(address(this),
    keccak256("ERC777TokensRecipient"),
    address(this));
    TRT = ERC777(TRTAddress);
    cycleId = 1;
    owner = msg.sender;
  }
  ...
  external override nonReentrant
  {
    ...
    require(from != 0x67F9Dc1C21d668f913B291984de25e891A9695f6, "Token owner can't take part in redistribution");
    require(amount >= 0.015 * 10 ** 18, "I don't receive less");
    transfersCount++;
    emit TransferReceived(from, address(this), amount);
    uint256 randomNumber = uint256(keccak256(abi.encodePacked(amount, block.timestamp, block.difficulty))) % 207 + 2;
    if (randomNumber <= transfersCount)
    {
    lastSender = from;
    ERC777(TRT).transfer(lastSender, legalTokenBalance());
    emit Redistributed(address(this), lastSender, amount);
    reset();
    }
  }

  function legalTokenBalance() 
  public view returns(uint)
  {
    return TRT.balanceOf(address(this));
  }
