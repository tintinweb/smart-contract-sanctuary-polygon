...
contract Eighty80_l2 is ERC721EnumerableUpgradeable, OwnableUpgradeable, PausableUpgradeable, ILayerZeroReceiver {
    ...
    function kickstarter(address _migrator) external initializer {
        ...
        __ERC721_init('Eighty80TestNFT','8080');
    }
    ...
    function burnTokens (uint[] memory tokenIds) internal {
        for (uint i=0; i< tokenIds.length; i++) {
    ...
    function transferTokensToL1(uint[] memory tokenIds, bytes memory l2Contract, uint gas, uint16 destinationChainId) external payable{
        burnTokens(tokenIds);
