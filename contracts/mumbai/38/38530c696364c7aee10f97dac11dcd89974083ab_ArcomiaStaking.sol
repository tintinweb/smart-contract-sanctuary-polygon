...
abstract contract ArcomiaStakingUpgradeable is ReentrancyGuardUpgradeable, IStaking1155 {
     ...
     */
    function setDefaultTimeUnit(uint256 _collectionId, uint256 _defaultTimeUnit) external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }

        StakingCondition memory _defaultCondition = defaultCondition[_collectionId][nextDefaultConditionId[_collectionId] - 1];
        require(_defaultTimeUnit != _defaultCondition.timeUnit, "Default time-unit unchanged.");

        _setDefaultStakingCondition(_collectionId, _defaultTimeUnit, _defaultCondition.rewardsPerUnitTime);
     ...
     */
    function setDefaultRewardsPerUnitTime(uint256 _collectionId, uint256 _defaultRewardsPerUnitTime) external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }
        ...
        _setDefaultStakingCondition(_collectionId, _defaultCondition.timeUnit, _defaultRewardsPerUnitTime);
