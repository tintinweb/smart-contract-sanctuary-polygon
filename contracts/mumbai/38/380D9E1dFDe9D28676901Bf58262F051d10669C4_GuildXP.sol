...
contract GuildXP is Modifiers, Initializable {
    ...
    function claim(bytes calldata signature, uint256 xpAmount) external onlyMembers {
        ...
        GuildXPStorage.layout().balances[msg.sender] += xpAmount;
    }

    function mint(address to, uint256 amount) external onlyManagers {
        GuildXPStorage.layout().balances[to] += amount;
    }

    function burn(address from, uint256 amount) external onlyManagers {
        GuildXPStorage.layout().balances[from] -= amount;
    }
    ...
    function getBalance(address user) external view returns (uint256) {
        return GuildXPStorage.layout().balances[user];
    }
