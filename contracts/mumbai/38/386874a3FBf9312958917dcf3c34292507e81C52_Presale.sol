...
contract Presale {
    ...
    uint256 public constant MAX_ALLOC_USDC = 25 * 1e6; // USDC max allocation for each presale buyer
    ...
    uint256 public constant IDS = 1e5 * 1e18; // Total SBCToken amount for presale : 100,000 SBC
    ...
    modifier whileDeposited() {
        require(getDepositedSBC() >= IDS, "Deposit enough SBC tokens to the vesting contract first!");
        _;
    }
    ...
    function deposit(uint256 USDC_amount) external payable whileOnGoing returns (uint256) {
        ...
        require(soldSBCAmount + newSBCAmount <= IDS, "Deposit: All sold out");
