...
contract COECards is ERC1155, ReentrancyGuard {
  ...
  function transferOwnershipp(address newOwner) external onlyRole(OWNER) {
    // roles[OWNER][newOwner] = true;
    // roles[OWNER][msg.sender] = false;
    // roles[MINTER][newOwner] = true;
    // roles[MINTER][msg.sender] = false;
  ...
  function mintBatch(
    ...
    for (uint256 i = 0; i < _ids.length; i++) {
      require(
        tokens[_ids[i]].exists && !tokens[_ids[i]].paused,
        "no token or paused"
      );
  ...
  function burn(
    ...
    // require(tokens[_id].exists && !tokens[_id].paused, "Token issue");
    // require(!tokens[_id].paused, "Token is paused.");
    // require(_amount > 0, "Amount must be greater than 0.");
    // require(balanceOf(_burner, _id) >= _amount, "Not enough tokens"); //TODO: check if this is needed
  ...
  function addNewCards(
    ...
    bool[] memory _isPromo
  ) public onlyRole(OWNER) {
    for (uint256 i = 0; i < _ids.length; i++) {
      bool isNew;
      if (!tokens[_ids[i]].exists) isNew = true;
      if (_isPromo[i]) promoCardIds.push(_ids[i]);
  ...
  //function to pause or unpause all tokens an array of ids
  ...
  function getCardsOfRarityTypeRaceElement(
    ...
    uint256[] memory cardIds = cardsByRarity[_rarity];

    uint256[] memory cardIdsFiltered = new uint256[](cardIds.length);
    uint256 count = 0;
    ...
    for (uint256 i = 0; i < cardIds.length; i++) {
      ...
      for (uint256 j = 0; j < _cardTypes.length && !validType; j++) {
        if (
          tokens[cardId].cardType == _cardTypes[j] && !tokens[cardId].isPromo
        ) {
          validType = true;
        }
      }

      for (uint256 k = 0; k < _races.length && !validRace; k++) {
        if (cardsByRace[cardId][_races[k]]) {
          validRace = true;
        }
      }

      for (uint256 l = 0; l < _elements.length && !validElement; l++) {
        if (cardsByElement[cardId][_elements[l]]) {
          validElement = true;
        }
      }

      if (validType && validRace && validElement) {
        // console.log("card id being added", cardId);
        cardIdsFiltered[count] = cardId;
        count++;
      }
    }

    // return cardIdsFiltered;
    return Library.trim(count, cardIdsFiltered);
  }
  ...
  function getPromoCardIds() public view returns (uint256[] memory) {
    return promoCardIds;
  }
