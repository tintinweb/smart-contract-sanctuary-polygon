...
contract MockAAVELender is BaseStrategyRisky {
    ...
    IPoolAddressesProvider private immutable addressProvider;

    // AAVE pool token for target token, aToken.
    address public aTargetToken;
    ...
    // used for instant exits
    address public uniRouter;

    // instant exit route
    address[] public uniPath;

    constructor(
        address _targetTokenPriceFeed,
        address _uniRouter,
        address[] memory _uniPath,
        address _container,
    ...
    {
        // set price related things
        targetTokenPriceFeed = IAggregatorV3(_targetTokenPriceFeed);

        // set AAVE things
        addressProvider = protocolDataProvider.ADDRESSES_PROVIDER();
        lendingPool = IPool(addressProvider.getPool());
        (aTargetToken, , ) = protocolDataProvider.getReserveTokensAddresses(
            _targetToken
        );
        ...
        // set allowances
        _giveAllowancesRouter();
        _giveAllowancesLendingPool();

        workingToken = aTargetToken;
        workingTokenDecimals = IERC20Metadata(aTargetToken).decimals();
    }

    // SETTERS  /////////////////////////////////////////////////////

    function setUniRouter(address _uniRouter) external onlyManagers {
        require(_uniRouter != address(0), "0x0 address");

        _removeAllowancesRouter();
        uniRouter = _uniRouter;
        _giveAllowancesRouter();
    }

    // @notice Sets the uni router path
    // @dev This function only meant to be used on instant redeems in user interactions
    //      targetToken => wantToken is the only swap that can be executed in this contract.
    function setUniPath(address[] memory _uniPath) external onlyManagers {
        _checkSwapPath(_uniPath, targetToken, stableToken);
        uniPath = _uniPath;
    }
    ...
    function balanceOfWorkingPositions()
        ...
        returns (uint256)
    {
        // AAVE interest bearing tokens and the underlyings are always in same decimals
        // we can add them safely.
        return _balanceOfToken(aTargetToken) + _balanceOfToken(targetToken);
    }

    // total balance in terms of 'stableToken' that the strategy is managing.
    function estimatedTotalAssets() public view override returns (uint256) {
        uint256 price = getWorkingTokenPrice();
        return (balanceOfWorkingPositions() * price) / 1e18;
    }

    ////////////////////////////////////////////////////////////////////////////

    function _deposit() internal override {
      // put entire idle 'targetToken'
      lendingPool.supply(
          targetToken,
          _balanceOfToken(targetToken),
          address(this),
          0
      );
    }
    ...
    {
        address _aTargetToken = aTargetToken; // cache
        deltaPositions = _balanceOfToken(_aTargetToken);

        // user depositing target asset
        lendingPool.supply(targetToken, _amount, address(this), 0);
        deltaPositions = _balanceOfToken(_aTargetToken) - deltaPositions;
    }
    ...
    function _getOutPosition(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 freeTargetToken = _balanceOfToken(targetToken);

        if (freeTargetToken >= _amount) {
          return _amount;
        }
        uint256 neededTargetToken;

        unchecked {
          neededTargetToken = _amount - freeTargetToken;
        }

        neededTargetToken = Math.min(neededTargetToken, _balanceOfToken(aTargetToken));

        uint256 targetTokenFreed = lendingPool.withdraw(
            targetToken,
            neededTargetToken,
            address(this)
        );
        return targetTokenFreed;
    }
    ...
    function _giveAllowancesRouter() internal {
        IERC20(targetToken).approve(uniRouter, type(uint256).max);
    }

    function _giveAllowancesLendingPool() internal {
        IERC20(targetToken).approve(address(lendingPool), type(uint256).max);
    }

    function _removeAllowancesRouter() internal {
        IERC20(targetToken).approve(uniRouter, 0);
    }

    function _removeAllowancesLendingPool() internal {
        IERC20(targetToken).approve(address(lendingPool), 0);
    }
    ...
    function _prepareRetirement() internal override returns (uint256) {
        ...
        return _balanceOfToken(targetToken);
    }

    function _sellTargetToken(uint256 _amount) internal returns (uint256) {
        if (_amount > 0) {
            return
    ...
    function _stopStrategy(address _newStrategy) internal override {
      lendingPool.withdraw(
          targetToken,
          type(uint256).max,
          address(this)
      );
      _sellTargetToken(_balanceOfToken(targetToken));
      IERC20(stableToken).safeTransfer(_newStrategy, _balanceOfToken(stableToken));
    }
    ////////////////////////////////////////////////////////////////////////////
}
...
abstract contract BaseStrategyRisky is BaseStrategy {
    using SafeERC20 for IERC20;
    using PercentageHelper for uint256;

    // target token that this strategy can be exposed to (matic,eth,btc etc)
    address public immutable targetToken;
    uint256 public immutable targetTokenDecimals;
    uint256 public callFee;

    address public constant exchangeAddress =
        0xDef1C0ded9bec7F1a1670819833240f027b25EfF; // 0xswap exchange
    ...
    {
        require(
            address(IContainer(_container).targetToken()) == _targetToken,
            "!TARGET_TOKEN"
        );

        targetToken = _targetToken;
        targetTokenDecimals = IERC20Metadata(_targetToken).decimals();

        IERC20(_targetToken).approve(exchangeAddress, type(uint256).max);
        IERC20(_stableToken).approve(exchangeAddress, type(uint256).max);
    }
    ...
    function balanceOfWorkingPositionsPrecised() public view returns (uint256) {
        ...
        return IERC20(targetToken).balanceOf(address(this)) * 10**(18-targetTokenDecimals);
    }

    /////////************** OVERRIDE THESE METHODS *****************////////////

    // @notice Swap 'stableToken' to 'targetToken'. Deposit 'targetToken' to AAVE, receive aTokens.
    // @dev If swapData is zero bytes then function will deposit idle 'targetToken' to AAVE to receive aTokens without a need of swap.
    // if swapData provided, then the entire 'stableToken' amount needs to be swapped. No partial swaps allowed.
    // @param swapData 0x API data for swapping 'stableToken' tokens to target token
    function deposit(bytes calldata swapData) public {
        if (swapData.length != 0) {
            // swap entire idle 'stableToken' if any
            _exchange(
        ...
        // put entire 'targetToken' to position
        _deposit();
    }
    ...
    function _exchange(
        ...
        if (swapData[0] == 0x41) {
            // call `transformERC20()`
            (IERC20 input, IERC20 output, uint256 inputTokenAmount, , ) = abi
                .decode(
                    swapData[4:],
                    (IERC20, IERC20, uint256, uint256, Transformation[])
                );
            if (_possibility == 0)
                require(
                    address(input) == stableToken &&
                        address(output) == targetToken,
                    "P0"
                );
            else {
                require(
                    address(input) == targetToken &&
                        address(output) == stableToken,
                    "P1"
                );
            }
            // at least most 1% left that not swapped.
            require(
                inputTokenAmount >= _inputAmount.percentageSubstract(100),
                "Wrong swap data, INPUT_AMOUNT"
            );

            uint256 balanceBefore = output.balanceOf(address(this));
            // solhint-disable-next-line avoid-low-level-calls
            (bool success, ) = exchangeAddress.call(swapData);
            require(success, "SWAP_FAILED");

            uint256 balanceAfter = output.balanceOf(address(this));
            uint256 boughtAmount = balanceAfter - balanceBefore; // if before > after, tx will be reverted by underflow.

            if (callFee != 0 && _callFeeRecipient != address(0)) {
                uint256 _callFee = (boughtAmount * callFee) / 10_000; // 1%
                IERC20(output).safeTransfer(_callFeeRecipient, _callFee);
                return boughtAmount - _callFee;
            }
            return boughtAmount;
        } else {
            revert("INVALID_DATA");
    ...
    // for user withdrawal.
    // @param _amount Working token amount in 18 decimals to liquidate to 'targetToken'
    // @returns liquidated 'targetToken' amount
        ...
        returns (uint256)
    {
        ...
        uint256 targetTokenFreed = _getOutPosition(_amount);
        IERC20(targetToken).safeTransfer(msg.sender, targetTokenFreed);
        return targetTokenFreed;
    }
    ...
    // 'switchStrategy()' or someone stops the strategy by calling containers
    // 'reportStop()'.
    // @dev If the 'swapData' is '0x0' then it is a stop call. If not then it is a planned migration.
    // @param swapData 0x API data for swapping 'targetToken' to 'stableToken'
    // @param _newStrategy New strategy address to migrate
    // @param _callFeeRecipient Address to send call reward if triggered by 'reportStop()'
    // @return Returns the total 'stableToken' token transferred to '_newStrategy'
    function retire(
        bytes calldata swapData,
        address _newStrategy,
        address _callFeeRecipient
    ) public onlyContainer returns (uint256) {
        uint256 targetTokenAmount = _prepareRetirement();
        _exchange(targetTokenAmount, swapData, 1, _callFeeRecipient);
        uint256 stableTokenAmount = IERC20(stableToken).balanceOf(
            address(this)
        );
    ...
    // @notice Container calls this function if stopping conditions are met.
    function stopStrategy(address _newStrategy) external onlyContainer {
        _stopStrategy(_newStrategy);
    }

    //@dev Exit the strategy with all you can get via hardcoded routes.
    function _stopStrategy(address _newStrategy) internal virtual;
}
