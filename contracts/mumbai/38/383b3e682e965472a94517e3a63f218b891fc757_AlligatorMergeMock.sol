...
contract AlligatorMerge is Ownable, VRFConsumerBaseV2, IMergeGators {
    ...
    function merge(uint256 _1st, uint256 _2nd, uint256 _3rd) public payable {
        ...
        bytes memory skeleton = abi.encodeWithSignature("createSkeleton(uint8, uint8, uint8, uint8, uint8, uint8, uint8)",
            _1stType[0],
            _2ndType[0],
            _3rdType[0],
            _4thType[0],
        ...
        bytes memory levels = abi.encodeWithSignature("createLevels(uint8, uint8, uint8, uint8, uint8, uint8, uint8)",
            _1stType[1],
            _2ndType[1],
            _3rdType[1],
            _4thType[1],
        ...
        bytes memory mergeParams = abi.encodeWithSignature("merge(uint256, uint256, uint256, address)", _1st, _2nd, _3rd, msg.sender);
        ...
        (bool success, ) = alligatorsAddress.call(skeleton);
        (bool success0, ) = alligatorsAddress.call(levels);
        (bool success1, ) = alligatorsAddress.call(mergeParams);

        if (!approved || !success) revert Mergefailed();
        if (!success0 || !success1) revert Mergefailed();
