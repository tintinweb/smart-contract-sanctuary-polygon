...
contract LiftAMM is ERC20{ 
   ...
   modifier ensure(uint deadline) {
        require(deadline >= block.timestamp, "LiftAMM: EXPIRED");
        _;
    }
   ...
   constructor(string memory name_, string memory symbol_, address _tokenA, address _tokenB, uint256 _fee) ERC20(name_, symbol_) {
    ...
    function addLiquidity(uint256 amountA, uint256 amountB, uint deadline) external ensure(deadline) returns (uint256 liquidity) {       
        ...
        require(totalSupply() * amountA / balanceA == totalSupply() * amountB / balanceB, "LiftAMM: WRONG PROPORTION BETWEEN ASSET A AND B");
    ...
    function removeLiquidity(uint256 liquidity, uint deadline) external ensure(deadline) returns (uint256 amountA, uint256 amountB) {   
        require(balance[msg.sender] >= liquidity, "LiftAMM: NOT ENOUGH LIQUIDITY FOR THIS, AMOUNT TOO BIG"); 
        uint256 balanceA = IERC20(tokenA).balanceOf(address(this));
        uint256 balanceB = IERC20(tokenB).balanceOf(address(this));         
    ...
    function swap(address tokenIn, uint256 amountIn, uint amountOutMin, uint deadline) external ensure(deadline) returns (uint256 amountOut) {
        uint256 newBalance;
        uint256 amountInWithFee;

        require(tokenIn == tokenA || tokenIn == tokenB, "TOKENIN NOT IN POOL");  

        require(amountIn >= amountOutMin, "LiftAMM: INSUFFICIENT_OUTPUT_AMOUNT");      
        ...
        // Amount with Trade Fee
        amountInWithFee = amountIn + (amountIn * fee / 1000);
