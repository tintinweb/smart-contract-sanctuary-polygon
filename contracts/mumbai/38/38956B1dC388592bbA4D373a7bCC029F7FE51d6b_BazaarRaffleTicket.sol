...
contract Web3BazaarEscrow is Context, ERC1155Holder, ERC721Holder, MultiAccessControl
{
    ...
    function swapERC1155 (address from, address to, address tokenAddress, uint256 tokenId, uint256 amount) internal returns (bool)
    {
     ...
     /*
        swap token ERC20 from address to another. First call method verifyERC20 just to check if from address hold the amount required.
        And if he'd allowd web4bazaar contract to spender that amount
...
contract BazaarRaffleTicket is Context, ERC721, MultiAccessControl, ERC721Holder  {
    ...
    // transfer erc20
    function transferERC20(address tokenAddress, uint256 amount) public isOwner{
    ...
    //set price
    function setPriceByType(uint8 raffleType, uint256 newPrice) external returns (bool){
         if(raffleType == BRONZE){
    ...
    function setContractURI(string memory newuri)  external  isOwner() returns (bool)
    {
        _contractURI = newuri;
        return true;
    }
