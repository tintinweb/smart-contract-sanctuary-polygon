...
contract DalNFTest8 is ERC721A, Ownable, ReentrancyGuard {
  using Strings for uint256;

  bytes32 public merkleRoot;
  mapping(address => bool) public whitelistClaimed;
  mapping(uint256 => bool) public usedNFTIds; // Mapping to track used NFT IDs
  ...
  uint256 public maxMintAmountPerTx = 100; // Default maximum mint amount per transaction
  ...
  function useNFTId(uint256 _nftId) internal {
    usedNFTIds[_nftId] = true;
  }

  function isNFTIdUsed(uint256 _nftId) public view returns (bool) {
    return usedNFTIds[_nftId];
  }
}
