...
contract FixPrice {
    ...
    // Constructor
    address owner;
    constructor(address _nvg8_token, address _nvg8_factory) {
    ...
    function setMarketplace(address _nvg8_marketplace) public onlyOwner {
        require(_nvg8_marketplace != address(0), "Marketplace address cannot be 0");
        nvg8_marketplace = _nvg8_marketplace;
    }
    ...
    function buyToken(uint256 _dataToken, uint256 _amount, address _owner, address _buyer) public onlyMarketplace returns (bool _success) {
        ...
        require(IERC20Template(listings[_dataToken].erc20Token).balanceOf(_owner) > _amount * 10**18, "Not enough data tokens");
