...
contract HTokenHelper is Ownable {
  ...
  function getDebt(HTokenI _htoken) external view returns (uint256) {
    return
      ...
      ((_htoken.totalBorrows() * (_htoken.interestPerToken() - _htoken.interestPerTokenStored())) / 10**_htoken.decimals());
  }
  ...
  function getMarketOraclePrices(HTokenI _hToken) external view returns (uint256, uint256) {
    uint256 nftPriceInETH = priceOracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), _hToken.decimals());
    uint256 underlyingPriceInUSD = uint256(priceOracle.getUnderlyingPriceInUSD(_hToken.underlyingToken()));
    uint256 ethPrice = uint256(priceOracle.getEthPrice());

    return ((nftPriceInETH * ethPrice) / underlyingPriceInUSD, underlyingPriceInUSD);
  }

  function getNFTPriceInUSD(HTokenI _hToken) external view returns (uint256) {
    uint256 nftPriceInETH = priceOracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), _hToken.decimals());
    uint256 ethPrice = uint256(priceOracle.getEthPrice());

    return (nftPriceInETH * ethPrice);
  }

  function getUnderlyingPriceInUSD(HTokenI _hToken) external view returns (uint256) {
    return uint256(priceOracle.getUnderlyingPriceInUSD(_hToken.underlyingToken()));
  }
  ...
  function uri(uint256 _id, address _htokenAddress) external view returns (string memory) {
    ...
    if (_id > 0) {
      ...
      writer = writer.writeStringProperty("name", string(abi.encodePacked("Coupon ", _id.toString())));
      ...
      writer = writer.writeStringProperty("name", string(abi.encodePacked("HToken")));
  ...
  function setPriceOracle(PriceOracleI _newOracle) external onlyOwner {
...
interface PriceOracleI {
  ...
  function getUnderlyingPriceInUSD(IERC20 _erc20) external view returns (int256);
