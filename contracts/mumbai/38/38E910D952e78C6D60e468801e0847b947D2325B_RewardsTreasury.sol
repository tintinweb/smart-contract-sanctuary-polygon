  ...
  AccessControlUpgradeable
{
  ...
  // calculated using https://docs.superfluid.finance/superfluid/protocol-developers/interactive-tutorials/money-streaming-1#flowrates
  ...
  function initialize(
    ...
    initFundsWrapperCustody(_fundsOwner, _ethProxy);

    // 500 Tokens / Month
    maxRewardFlowRate = 192901234567901;
    maxNumberOfFlows = 5000;
  ...
  function createRewardFlow(
    address _receiver,
    ISuperToken acceptedToken,
    int96 _flowRate
  ) external onlyRole(OPERATOR_ROLE) onlyAllowedFlowRate(_flowRate) {
    require(
  ...
  function deleteRewardFlow(address _receiver, ISuperToken acceptedToken)
    external
...
contract FundsWrapperCustody is Initializable, IFundsWrapperCustody {
  ...
  function initFundsWrapperCustody(address _fundsOwner, SETHProxy _ethProxy)
    public
    initializer
  {
  ...
  function depositAndUpgradeEth() external payable override onlyFundsOwner {
    ethProxy.upgradeByETH{ value: msg.value }();
    emit TokenDeposited(ETH_ADDRESS, msg.sender, msg.value);
  }

  function downgradeToEth(uint256 amt) external override onlyFundsOwner {
    ethProxy.downgradeToETH(amt);
  }

  function transferAndUpgrade(ISuperToken token, uint256 amt)
    external
    override
    onlyFundsOwner
  {
    ...
    if (!_isApproved(underlyingToken, address(this), address(token), amt)) {
      require(
        underlyingToken.approve(address(token), APPROVAL_LIMIT),
  ...
  function downgradeAndWithdraw(ISuperToken token, uint256 amt)
    external
    override
    onlyFundsOwner
  {
...
abstract contract IFundsWrapperCustody {
  ...
  function depositAndUpgradeEth() external payable virtual;

  function downgradeToEth(uint256 amt) external virtual;

  /**
  @param token: the address of the super token the to upgrade to
   */
  function transferAndUpgrade(ISuperToken token, uint256 amt) external virtual;
  ...
  function withdrawEth(uint256 amt) public onlyFundsOwner {
    require(address(this).balance >= amt, "insufficent funds");
    (bool sent, ) = msg.sender.call{ value: amt }("");
    require(sent, "failed to withdraw the funds");

    emit TokenWithdrawed(ETH_ADDRESS, msg.sender, amt);
  }

  function balanceOfSuperToken(ISuperToken token)
    external
    view
    returns (uint256)
  {
  ...
  function withdrawToken(ERC20 token, uint256 amt) public onlyFundsOwner {
    if (!_isApproved(token, address(this), msg.sender, amt)) {
      require(
        token.approve(msg.sender, APPROVAL_LIMIT),
        "failed to approve the tokens"
      );
    }
  ...
  // access control
  function transferFundsOwnership(address newOwner) external onlyFundsOwner {
  ...
  modifier onlyFundsOwner() {
    require(fundsOwner == msg.sender, "caller is not the funds owner");
    _;
  }
