...
// File: contracts/Flipkart/NFT_V1.sol
...
contract ItemNFT_V1 is ERC721Enumerable, Ownable {
        ...
        returns (NFTDetails[] memory)
    {
         ...
         uint256 balanceOfUser = balanceOf(_address);
          NFTDetails[] memory _allNFTDetailsOfUser =  new NFTDetails[](balanceOfUser);
        for(uint8 i=0;i<balanceOfUser;i++){
            uint256 _tokenId = tokenOfOwnerByIndex(_address, i);
             Warranty memory w = getWarrentyInfo(_tokenId);
              _allNFTDetailsOfUser[i] = NFTDetails({tokenId: _tokenId, warrentyDetails: w});
        }
    ...
    function getTokenIdFromSerialNo(string memory _serialNo) public view returns(uint256){
