...
contract Factory {
    ...
    event VotingContractCreated(address contractAddress, string name);

    Voting[] public VotingArray;
    modifier onlyOwner(address user){
       require(user == owner);
       _;
    }

    function createNewVoting(IVoting.ElectionInfo memory _info, address _newContractOwner) public onlyOwner(msg.sender) returns(address){
        Voting voting = new Voting(_info, _newContractOwner);
        VotingArray.push(voting);
        emit VotingContractCreated(address(voting), _info.name);
