 ...
 * Has a function to create betting tournament.
 * Has functions to config betting tournament by adding more team or postpone betting period.
 * Has a function to place bet on tournament.
 * Has a function to set tournament winner.
 * Has functions for retriving user reward and claim user reward.
 * @notice Is pausable to prevent malicious behavior.
 */
contract Betting is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    event TournamentCreated(string tournamentId, uint256 endTime, string[] teamIds);
    event TournamentEntryAdded(string tournamentId, string[] teamIds);
    event TournamentBetPlaced(string tournamentId, string teamId, address indexed userAddress, uint256 betAmount);
    event TournamentWinnerUpdated(string tournamentId, string teamId);
    event TournamentRewardClaimed(string tournamentId, address indexed userAddress, uint256 claimAmount);
    event TournamentEndTimeUpdated(string tournamentId, uint256 newTime);
    ...
    */
    ...
    modifier tournamentCreated(string memory _tournamentId) {
        require(
            bytes(tournamentByTournamentId[_tournamentId].tournamentId).length > 0,
            "[Betting.tournamentCreated] Tournament not created yet"
        );
        _;
    }
    modifier tournamentEntryExist(string memory _tournamentId, string memory _teamId) {
        require(
            ...
            "[Betting.tournamentEntryExist] Tournament's entry not exist"
        );
        _;
    }
    modifier beforeTournamentBettingEnded(string memory _tournamentId) {
        require(
            block.timestamp < tournamentByTournamentId[_tournamentId].endTime,
            "[Betting.beforeTournamentBettingEnded] Tournament betting period ended"
        );
        _;
    }
    ...
    receive() external payable {
        revert("[Betting] Revert receive function.");
    }
    fallback() external payable {
        revert("[Betting] Revert fallback function.");
    }

    /** 
    * @dev Create betting tournament.
    * @param _tournamentId - tournament id generated from backend.
    * @param _endTime - betting period ending time.
    ...
    */
    function createBettingTournament(
        ...
        require(
            bytes(tournamentByTournamentId[_tournamentId].tournamentId).length == 0,
            "[Betting.createBettingTournament] Tournament already created"
        );
        require(
            block.timestamp < _endTime,
            "[Betting.createBettingTournament] Invalid tournament endtime"
        );
        require(
            _teamIds.length > 1,
            "[Betting.createBettingTournament] Invalid entiryIds length"
        );
        ...
        emit TournamentCreated(_tournamentId, _endTime, _teamIds);
    }
    ...
    * @param _teamIds - list of team id generated from backend.
    */
    function addBettingTournamentEntries(string memory _tournamentId, string[] memory _teamIds)
        public
        whenNotPaused
        tournamentCreated(_tournamentId)
        beforeTournamentBettingEnded(_tournamentId)
        onlyRole(WORKER)
    {
        require(
            _teamIds.length > 0,
            "[Betting.addBettingTournamentEntries] Invalid entiryIds length"
        );
        ...
        for (uint256 i = 0; i < _teamIds.length; i++) {
            ...
            require(
                ...
                "[Betting.addBettingTournamentEntry] Some team already exist in entry list"
            );
    ...
    */
        ...
        beforeTournamentBettingEnded(_tournamentId)
    {
        require(
            _betAmount > 0,
             "[Betting.placeTournamentBet] Invalid bet amount"
        );
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        Entry storage entry = tournament.entryByEntryId[tournament.entryIdByTeamId[_teamId]];
        ...
        emit TournamentBetPlaced(_tournamentId, _teamId, msg.sender, _betAmount);
    }
    ...
    * @param _tournamentId - tournament id generated from backend.
    */
    function claimTournamentReward(string memory _tournamentId) public nonReentrant {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        require(
            bytes(tournament.winner).length > 0,
            "[Betting.claimTournamentReward] Tournament winner not been updated yet"
        );

        uint256 reward = getUserTournamentClaimableReward(_tournamentId, msg.sender);
        require(
            reward > 0,
            "[Betting.claimTournamentReward] User not have claimable reward for this tournament"
        );

        tournament.tournamentClaimAmount += reward;
        tournament.userClaimAmount[msg.sender] += reward;
        safeUpoTransfer(msg.sender, reward);

        emit TournamentRewardClaimed(_tournamentId, msg.sender, reward);
    }
    ...
    */
    function calculateTournamentReward(string memory _tournamentId) internal {
        ...
        tournament.rewardPerBet = tournament.tournamentBetAmount != 0? tournament.tournamentBetAmount / entry.entryBetAmount: 0;
    }
    ...
    */
    function getUserTournamentClaimableReward(
        ...
        claimableReward = tournament.userClaimAmount[_userAddress] == 0? tournament.rewardPerBet * entry.userBetAmount[_userAddress]: 0;
    }
    ...
    */
    ...
    {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        UserEntryBetInfo[] memory list = new UserEntryBetInfo[](tournament.entryCount);
    ...
    /** 
    * @dev Postpone tournament end betting period.
    * @param _tournamentId - tournament id generated from backend.
    * @param _newEndTime - betting period ending time.
    */
    function setTournamentEndTime(string memory _tournamentId, uint256 _newEndTime)
        external
        whenNotPaused
        tournamentCreated(_tournamentId)
        beforeTournamentBettingEnded(_tournamentId)
    ...
    {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        tournament.endTime = _newEndTime;

        emit TournamentEndTimeUpdated(_tournamentId, _newEndTime);
    }
    ...
    */
    function setTournamentWinner(string memory _tournamentId, string memory _teamId)
        external
        whenNotPaused
        tournamentCreated(_tournamentId)
        tournamentEntryExist(_tournamentId, _teamId)
        onlyRole(WORKER)
    {
        require(
            block.timestamp >= tournamentByTournamentId[_tournamentId].endTime,
            "[Betting.setTournamentWinner] Tournament betting period not ended yet"
        );

        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        require(
            bytes(tournament.winner).length == 0,
            "[Betting.setTournamentWinner] Tournament already have winner"
        );

        tournament.winner = _teamId;
        calculateTournamentReward(_tournamentId);

        emit TournamentWinnerUpdated(_tournamentId, _teamId);
    }

    /**
     * @dev Set betting in to pause state (only claim reward function and getter is allowed).
     */
    ...
    /**
     * @dev Set betting in to normal state.
     */
