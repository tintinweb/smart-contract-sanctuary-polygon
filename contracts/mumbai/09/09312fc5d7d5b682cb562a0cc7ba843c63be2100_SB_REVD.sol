...
contract SB_REVD is ERC721, ERC721Enumerable, Ownable {
// contract Doodles_Working is ERC721, ERC721Enumerable, Ownable {
    ...
    bool public paused = true; // initial is true, need to set to false to allow mint by Public and Whitelist
    ...
    event MintedTokenId(address indexed _address, uint256 _tokenId, uint256 _price_per_token);
    event Minted(address indexed _address, uint256 quantity);
    event WhiteListMintedTokenId(address indexed _address, uint256 _tokenId, uint256 _price_per_token);
    event WhiteListMintedTokenIdList1(address indexed _address, uint256 _tokenId, uint256 _price_per_token);
    event WhiteListMinted(address indexed _address, uint256 quantity);
    ...
    function mintNFTAirDrop(uint256 numberOfNfts, uint256 _airDrop_id) public payable {
        ...
        if (_limitTotalSupplyIsActive){
            require(ts + numberOfNfts <= _limitTotalSupply, "Exceed total supply limit");
        }
        ...
        for (uint i = 0; i < numberOfNfts; i++) {
            ...
            if (isWhiteListActive) {
                emit WhiteListMintedTokenId(msg.sender, id, price_per_token);
            } else {
                emit MintedTokenId(msg.sender, id, price_per_token);
            }

        }

        if (isWhiteListActive) {
            emit WhiteListMinted(msg.sender, numberOfNfts);
        } else {
    ...
    constructor(
        ...
        max_public_mint = MAX_SUPPLY;
        max_whitelist_mint = MAX_SUPPLY;
        _limitTotalSupply = MAX_SUPPLY;
        //----------------------------------------------
    ...
    function setMaxWhiteListMint(uint16 _max_whitelist_mint) external {
        ...
        require(msg.sender == owner_address || msg.sender == admin_address,"Not Owner nor Admin");
        require(whiteList.length==0,"Previous whiteList not removed");
    ...
    bool public isWhiteListActiveList1 = false;
    ...
    function setWhiteListActiveStatusList1(bool _status) external {
        // replace onlyOwner with owner or admin
    ...
    // function addAndChangeWhiteListStatus(address _address) public onlyOwner {
        ...
        for (uint256 i = 0; i < _address.length; i++) {
            ...
            _whitelist_mint_max_list1[_address[i]]=max_whitelist_mint;
    ...
    function editSingleWhiteListUserWithLimitMintList1(address _address, uint256 _whitelist_limit_mint) public {
    // function addAndChangeWhiteListStatus(address _address) public onlyOwner {
        ...
        require(whitelistedAddressList1[_address] == true);
        // whitelistedAddress[_address] = true;

        _whitelist_mint_max_list1[_address]=_whitelist_limit_mint;

        emit SetWhiteListMintMaxSpecific(_address, _whitelist_limit_mint);
   
    }

    bool public _limitTotalSupplyIsActive=false; // false by default
    uint256 public _limitTotalSupply;

    function setMintLimitByTotalSupplyIsActive(bool _status) external {
        // replace onlyOwner with owner or admin
        
        require(msg.sender == owner_address || msg.sender == admin_address,"Not Owner nor Admin");

        _limitTotalSupplyIsActive = _status;
    }


    function setLimitTotalSupply(uint256 _totalSupply) external {
        // replace onlyOwner with owner or admin
        
        require(msg.sender == owner_address || msg.sender == admin_address,"Not Owner nor Admin");
        require(_totalSupply>totalSupply(),"Total Supply cannot be less than existing Total Supply");

        _limitTotalSupply = _totalSupply;
    }
    ...
    // function mint(uint numberOfTokens) public payable {
        ...
        if (isWhiteListActiveList1){
            require(whitelistedAddressList1[msg.sender], "Client is not on White list");
            // mint_limit=max_whitelist_mint;
            mint_limit=_whitelist_mint_max_list1[msg.sender];
        ...
        if (_limitTotalSupplyIsActive){
            require(ts + numberOfTokens <= _limitTotalSupply, "Exceed total supply limit");
        }
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (isWhiteListActive || isWhiteListActiveList1) {
                if (isWhiteListActive){
                    emit WhiteListMintedTokenId(msg.sender, id, price_per_token);
                } else if (isWhiteListActiveList1) {
                    emit WhiteListMintedTokenIdList1(msg.sender, id, price_per_token);
                }    
            } else {
                emit MintedTokenId(msg.sender, id, price_per_token);
            }
        ...
        if (isWhiteListActive|| isWhiteListActiveList1) {
            if (isWhiteListActive){
                emit WhiteListMinted(msg.sender, numberOfTokens);
            } else if (isWhiteListActiveList1) {
