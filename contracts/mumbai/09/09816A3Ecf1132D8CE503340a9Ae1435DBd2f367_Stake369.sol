...
contract Stake369 is ERC721, Ownable, RoyaltiesV2Impl {
    ...
    function getRemainingTokenNumber() view public onlyOwner returns (uint256){
       uint256 total = MAX_SUPPLY - mintedTokens;
       return total;
    }
    ...
    function revealNTF(uint256 tokenId,uint256 random) private onlyOwner returns (string memory) {
        require(reveal == true, "Not allowed to reveal yet");
        require(bytes(_uris[tokenId]).length == 0, "NFT already revealed");
        
        random = (random*4 + tokenId*3) % 9;
        random++;
        uint256 repeated = 12;
