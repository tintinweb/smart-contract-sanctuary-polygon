...
contract PaymasterV1 is BasePaymaster {
        ...
        )
    {
        ...
        uint256 _answer = int256ToUint256(answer);
        uint8 _decimal = oracle.decimals();

        return (maxCharge.mul(10**_decimal).div(_answer), _answer, _decimal);
    }
        ...
        returns (uint256 tokenActualCharge, uint256 ethActualCharge)
    {
        ...
        return (charge.mul(10**decimal).div(tokenPerEth), charge);
    }
        ...
        returns (bytes memory context, bool revertOnRecipientRevert)
    {
        ...
        return (abi.encode(payer, collector, tokenPerEth, decimal), false);
    }
    ...
    function postRelayedCall(
        ...
        (
            ...
            uint256 tokenPerEth,
            uint80 decimal
        ) = abi.decode(context, (address, address, uint256, uint80));
    ...
    function int256ToUint256(int256 val) internal pure returns (uint256) {
        if (val < 0) {
            return uint256(-val);
        } else {
            return uint256(val);
