...
contract SynthChan is ERC721Enumerable, Ownable, FlexAttributes, RoyaltiesV2Impl {
  ...
  // infos[0][4]   == image URL part1 (eg. 'https://...')
  // infos[0][5]   == image URL part2 (eg. '...storage.link')
  // infos[0][6]   == base64decoder/interactive/svg/js URL
  // infos[1][...] == NFTinteractiveURI params
  ...
  // }
  // NOTE: the JSON formatting is VERY strict, so make sure to quadruple check all [",'\n], etc for validity
  // must (double?)escape all entities, as appropriate. Verify with https://jsonformatter.curiousconcept.com or similar
  ...
  // image         == URL to an image; base64-encoded seems to not work...
  // image_data    == Raw SVG image data; very limited/incomplete in features supported. Definitely NOT all there.
  // external_url  == URL to external service where image is stored... can be the same string as `image`
  // animation_url == URL to a video/html/javascript/anything interactive that is linked with the NFT!
    ...
    return string(
      abi.encodePacked("data:application/json;base64,", Base64.encode(
        abi.encodePacked( // TODO: can try attributes(_tokenId, "Last Name").strValue save on space/gas??
          ...
          " \"image\":\"", imageURL(_tokenId), "\",",
          " \"animation_url\":\"", interactiveURI(_tokenId), "\",",
  ...
  function description(uint _tokenId) internal view returns (string memory) {
    ...
    string[3] memory filler = [
      string(abi.encodePacked(
        getAttribute(_tokenId, "Last Name").strValue," ",getAttribute(_tokenId, "First Name").strValue)
      ),
  ...
  function imageURL(uint256 _tokenId) internal view returns (bytes memory) {
    return abi.encodePacked(infos[0][4], getAttribute(_tokenId, "ipfs").strValue, infos[0][5]);
  }

  function interactiveURI(uint _tokenId) internal view returns (bytes memory) {
    bytes memory imgData;
    bytes1 comma = " ";
    for(uint i = 0; i < infos[1].length; i++) {
      imgData = abi.encodePacked(
        imgData,
        comma,
        "\"",infos[1][i],"\":\"",getAttribute(_tokenId, infos[1][i]).strValue,"\""
      );
      comma = ",";
    }
    return abi.encodePacked(infos[0][6], Base64.encode(imgData));
  }
  ...
  function jsonAttributes(uint _nftId) internal view returns (string memory json) {
    ...
    for (uint i=0; i<traitTypes.length; i++) {
      ...
      if(bytes(attr.strValue).length > 0) { // if we have something stored in strValue, assume that it is a STRING attr
        json = string(abi.encodePacked(json, comma, "{\"trait_type\":\"",traitTypes[i],"\",\"value\":\"",attr.strValue,"\"}"));
      } else if(attr.maxValue > 0) { // show value with maxValue
        json = string(abi.encodePacked(json, comma, "{\"display_type\":\"",displayTypes[uint(attr.dispType)],
