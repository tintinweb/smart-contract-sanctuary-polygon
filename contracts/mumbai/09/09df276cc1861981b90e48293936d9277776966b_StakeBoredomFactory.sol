...
// File: contracts/StakeBoredom.sol
...
contract StakeBoredom is ReentrancyGuard {
    ...
    mapping(bytes32 => uint256) private _referralStakes;

    bool public isInitialized;

    address public RewardPoolAdmin;
    ...
    function getRewardToken() external view returns (string memory) {
        return rewardToken.name();
    }

    function isExpired() public view returns (bool) {
        return block.timestamp >= _state.periodFinish;
    }
    ...
    function getDaysLeft() public view returns (uint256) {
        if (block.timestamp >= _state.periodFinish) {
            ...
            uint256 remainingTime = _state.periodFinish - block.timestamp;
            uint256 remainingDays = remainingTime / 1 days;
            return remainingDays;
        }
    }

    // call the Totalstaked with referral link
    function getTotalStakedReferral(string memory referral) external view returns (uint256) {
    bytes32 referralHash = keccak256(bytes(referral));
    return _referralStakes[referralHash];
}

    /* ========== MUTATIONS ========== */

    function initialize(address distributor, IBEP20 stakingToken_, IBEP20 rewardToken_, uint64 duration, uint256 _apy) external {
        require(!isInitialized, "Already initialized");
        require(msg.sender == RewardPoolAdmin, "Not An Admin");
        APY = _apy;
    ...
    function stake(uint256 amount, string memory referral) external payable nonReentrant updateReward(msg.sender)
    {
        ...
        // Add up the referral stake if a referral is provided
    if (bytes(referral).length > 0) {
        bytes32 referralHash = keccak256(bytes(referral));
        _referralStakes[referralHash] += amount;
    }
...
contract StakeBoredomFactory {
    mapping(uint256 => address) public deployedContracts;
    uint256 public contractCount;

    mapping(uint256 => address) public expiredContracts;
    uint256 public expiredContractCount;

    event NewStakeBoredomContract(address indexed stakeBoredom);
    event ContractExpired(address indexed stakeBoredom);

    uint256 private lastExpirationCheck;

    constructor() {
        lastExpirationCheck = block.timestamp;
    }
    ...
    function deployContract(
        ...
        bytes memory bytecode = type(StakeBoredom).creationCode;
        bytes32 salt = keccak256(
            abi.encodePacked(distributor, stakingToken, rewardToken, durationInDays, _apy, msg.sender)
        );
        address stakeBoredomAddress;

        assembly {
            stakeBoredomAddress := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }

        StakeBoredom(stakeBoredomAddress).initialize(
            distributor,
            stakingToken,
            rewardToken,
            durationInDays,
        ...
        deployedContracts[contractCount] = stakeBoredomAddress;
        contractCount++;

        emit NewStakeBoredomContract(stakeBoredomAddress);
    }

    function checkExpiredContracts() internal {
        uint256 contractIndex = 0;

        while (contractIndex < contractCount) {
            address contractAddress = deployedContracts[contractIndex];

            // Check if the contract is expired
            if (StakeBoredom(contractAddress).isExpired()) {
                // Move the expired contract to the expiredContracts mapping
                expiredContracts[expiredContractCount] = contractAddress;
                expiredContractCount++;

                // Emit an event to notify the expiration
                emit ContractExpired(contractAddress);

                // Remove the contract from the deployedContracts mapping
                deployedContracts[contractIndex] = deployedContracts[contractCount - 1];
                delete deployedContracts[contractCount - 1];
                contractCount--;
            } else {
                contractIndex++;
            }
    ...
    function callCheckExpiredContracts() external {
        // Check if 24 hours have passed since the last execution
        if (block.timestamp >= lastExpirationCheck + 1 days) {
            // Call the checkExpiredContracts function
