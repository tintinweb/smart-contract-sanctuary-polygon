 ...
 * @author test
 * @title Base
 * @notice ERC721 contract that allows users to trade assets, and has minting functionalities
 */
contract NFT is ERC721, Ownable {
    ...
    event SetSignerAddress(address indexed ownersClubNFTAddress);
    event Mint(address indexed beneficiary, uint256 tokenId);
    ...
    error VerificationFail(address _beneficiary, bytes _signature);
    error TransferForbidden();
    error AlreadyMinted(address _beneficiary);
        ...
        ERC721("NFT", "NFT")
    {
        ...
        // _totalSupply are initialized to 1, since starting at 0 leads to higher gas cost for the first minter
    ...
    function mint(uint256 _tokenId)
        ...
        returns (bool ret)
    {
        ret = mintTo(_msgSender(), _tokenId);
    }

    /**
     * @notice Mint token to the beneficiary after verifying tokenId and signature
     * @dev onlyOwner
     * @param _beneficiary - address eligible to get the token for tokenId
     * @param _tokenId - tokenId to be minted
     * @return bool
     */
    function mintTo(address _beneficiary, uint256 _tokenId)
        public
        payable
        onlyOwner
        returns (bool)
    {
        ...
        if (exists(_tokenId)) {
            revert TokenExists(_tokenId);
        }

        if (balanceOf(_beneficiary) > 0) {
            revert AlreadyMinted(_beneficiary);
        }

        _safeMint(_beneficiary, _tokenId);
        _totalSupply.increment();

        emit Mint(_beneficiary, _tokenId);
        return true;
    }

    /**
     * @notice See {ERC721-_beforeTokenTransfer}
     * @dev Override to forbit all trasfer function but pass throuh mint function
     * @param _from - address which want to transfer the token by tokenId
     */
    function _beforeTokenTransfer(
        address _from,
        address,
        ...
        // override to revert all trasfer function but pass through mint function
        if (_from != address(0)) {
