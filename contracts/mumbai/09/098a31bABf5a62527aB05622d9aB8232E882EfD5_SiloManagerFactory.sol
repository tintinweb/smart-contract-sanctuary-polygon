...
//TODO create a way for a keeper to migrate upkeeps to a new registry.
/*
So I'll need some way to iterate through all the existing upkeeps in the manager, but then the logic can check that maxValidBlockNumber. 
If it is equal to 2^64  -1 then the upkeep needs to be canceled. 
Else, if it is less than the current block number go ahead and withdraw the funds and add them to the new registry

Once an upkeep is canceled it should not be called
*/
contract SiloManagerFactory is Ownable, KeeperCompatibleInterface{
    ...
    uint32 public GAS_LIMIT = 3500000;
    ...
    //mumbai
    address constant public REGISTRAR_ADDRESS = 0xF43c9134Ae10f06efA914a8B1ca3B5d468130f47;
    ...
    //address constant public REGISTRAR_ADDRESS = 0x6179B349067af80D0c171f43E6d767E4A00775Cd;//Mainnet
    ...
    constructor(address _keeperRegistry, address _siloFactory) {
        keeperRegistry = _keeperRegistry;
        Registry = IKeepersRegistry(_keeperRegistry);
        siloFactory = _siloFactory;
    }

    function createFactoryUpkeep() external onlyOwner{
        require(!factoryUpkeepCreated, "Factory Upkeep already made!");
    ...
    function adjustKeepersRegistry(address _registry) external onlyOwner{
        keeperRegistry = _registry;
        Registry = IKeepersRegistry(keeperRegistry);
    }
    ...
    function createSiloManager(uint _amount) external{
        ...
        address manager = address(new SiloManager(address(this), msg.sender, _amount));
    ...
    {   uint registryCount = Registry.getUpkeepCount() - 1;
        if(registryCount > lastCheckedUpkeepId){
            uint batchSize; 
            ...
            for(uint i=0; i<batchSize; i++){
                ...
                if(isManager[potentialManager] && gas == GAS_LIMIT && check.length == 0 && admin == address(this)){//next upkeep id is a manager
                    performData = abi.encode(potentialManager, lastCheckedUpkeepId + i + 1);
                    upkeepNeeded = true;
                    break;
                }
            }
            if(!upkeepNeeded && batchSize == maxBatchSize){//only perform upkeep if we have reached the max batch size and an upkeep still isn't needed
                performData = abi.encode(address(0), lastCheckedUpkeepId+batchSize);
                upkeepNeeded = true;
            }
        }
    }
    ...
    function performUpkeep(bytes calldata performData) external override {
        require(msg.sender == keeperRegistry, "Caller must be keeper registry");
        (address manager, uint id) = abi.decode(performData, (address,uint));
...
contract SiloManager is KeeperCompatibleInterface{
    ...
    bytes4 private constant FUNC_SELECTOR = bytes4(keccak256("register(string,bytes,address,uint32,address,bytes,uint96,uint8)"));
    bool called;
    uint public addFundsThreshold = 100000000000000000;
    uint public upkeepId;
    uint96 public riskBuffer = 10000; //based off a number 10000 -> ∞
    ...
    constructor(address _mangerFactory, address _owner, uint _amount){
    ...
    function performUpkeep(bytes calldata performData) external override {
        if(customRegistry != address(0)){
            ...
            require(msg.sender == ManagerFactory.keeperRegistry(), "Caller must be keeper registry");
        }
        ...
        if(silo != address(this)){//trying to maintain a silo
            ...
            if(ERC677Link.balanceOf(address(this)) >= addFundsThreshold){//conditional checked in checkUpkeep to 
                ERC677Link.approve(ManagerFactory.keeperRegistry(), ERC677Link.balanceOf(address(this)));
                IKeepersRegistry(ManagerFactory.keeperRegistry()).addFunds(upkeepId, ERC677Link.balanceOf(address(this)));
            }
        }
    }
