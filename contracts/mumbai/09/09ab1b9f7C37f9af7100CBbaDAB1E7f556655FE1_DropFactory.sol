...
contract DropFactory is IDropsFactory {
    address private _dropOwner;
    address private _dropAntic;

    event DeployedDrop(address deployedContract);

    constructor(address dropOwner, address dropAntic) {
        _dropOwner = dropOwner;
        _dropAntic = dropAntic;
    }

    function deployDrop() external override returns (address) {
        RecipientsDrop dropContract = new RecipientsDrop(
            _dropOwner,
            _dropAntic
        );
        emit DeployedDrop(address(dropContract));
        return address(dropContract);
    }
}
