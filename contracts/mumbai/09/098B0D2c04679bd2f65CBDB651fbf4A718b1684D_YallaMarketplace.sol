...
contract YallaNFT is ERC1155 {
    ...
    mapping(uint256 => bool) public categoryTracker;
    mapping(uint256 => uint256) public tokenCategory;

    bool private allowCreatorMint;
    ...
    function setCustomUri(uint256 tokenId, string memory newUri) public {
        customUri[tokenId] = newUri;
    }

    function allowCreatorMinting(bool _creatorMint ) public onlyOwner {
        allowCreatorMint = _creatorMint;
    }
    ...
    function saveCategory(uint256 categoryId, string memory category) public onlyOwner {
        categories[ categoryId ] = category;
        categoryTracker[ categoryId ] = true;
    }

    //function changeTokenCategory() {}

    // function proxyMintAndTransfer(address initialOwner, uint256 tokenId, uint256 initialSupply, bytes memory data) public returns(uint256) {
    //     super._mint(initialOwner, tokenId, initialSupply, data);
    //     creators[ tokenId ] = initialOwner;
    //     super.setApprovalForAll(marketplaceAddress, true);
    //     return tokenId;
    // }


    function mint(uint256 tokenId, uint256 initialSupply, uint256 categoryId, string memory metadataUrl,  bytes memory data) public returns(uint256) { 

        // require(categoryTracker[ categoryId ], "invalid category id");
        require(bytes(categories[ categoryId ]).length > 0, "invalid category id");
        require(allowCreatorMint, "Creator miniting disabled"); 

        super._mint(msg.sender, tokenId, initialSupply, data);
        creators[ tokenId ] = msg.sender;
        tokenHolders[ tokenId ] = msg.sender; 
        tokenCategory[ tokenId ] = categoryId;
        customUri[tokenId] =metadataUrl;
     
        return tokenId;
    }


    function create(address initialOwner, uint256 tokenId, uint256 initialSupply, uint256 categoryId, string memory metadataUrl,  bytes memory data) public onlyOwner returns(uint256) { 

        require(categoryTracker[ categoryId ], "invalid category id");

        super._mint(initialOwner, tokenId, initialSupply, data);
        creators[ tokenId ] = initialOwner;
        tokenHolders[ tokenId ] = initialOwner; 
        tokenCategory[ tokenId ] = categoryId;
        customUri[tokenId] = metadataUrl;
        return tokenId;
    }

    function batchMint(address initialOwner, uint256[] memory tokenIds, uint256[] memory initialSupplies, uint256[] memory categoryIds, string[] memory metadataUrl, bytes memory data) public { //only owner
        require(allowCreatorMint, "Creator miniting disabled");
        
        for(uint i=0; i < tokenIds.length; i++) {
            if( categoryTracker[ categoryIds[i] ] ) {
                creators[ tokenIds[i] ] = initialOwner;
                tokenCategory[ tokenIds[i] ] = categoryIds[i];
                customUri[ tokenIds[i] ] = metadataUrl[i];
            }
            else 
                revert(" CategoryId doesn't exist");
        }
        super._mintBatch(initialOwner, tokenIds, initialSupplies, data);
    }
    
    // function burn(uint256 tokenId, uint256 quantity) public {
    //     super._burn(msg.sender, tokenId, quantity);
    //     delete creators[ tokenId ];
    // }

    // function batchBurn(uint256[] memory tokenIds, uint256[] memory quantities) public {
    //     for(uint i=0; i < tokenIds.length; i++) {
    //        delete creators[ tokenIds[i] ];
    //     }
    //     super._burnBatch(msg.sender, tokenIds, quantities);
    // }
    ...
    function getCategory(uint256 categoryId) external view returns ( string memory) {
        return categories[ categoryId ];
    }

    function isCategoryExist(uint256 categoryId) external view returns ( bool ) {
        return categoryTracker[ categoryId ];
    }
...
contract YallaMarketplace is ReentrancyGuard {
    ...
    */
    ...
    function secondCommitionDistribution(uint256 _value) internal pure returns(uint256,uint256,uint256){
        uint256 first = _value/100*20;
        uint256 second= _value/100*20;
        uint256 third = _value/100*60;
        return (first,second,third);
    }
    function firstCommitionDistribution(uint256 _value) internal pure returns(uint256,uint256){
        uint256 first = _value/100*40;
        uint256 second= _value/100*60;
        return (first,second);
    }
