...
contract UTDigitalRFactory is Ownable, BlackHolePrevention {
    ...
    uint256     public constant     version              = 20230515;
    ...
    struct inputTraitStruct {
        ...
        uint8   redeemMode;                 // 0 - Random redeam from the vault (with VRF), 1 - Nonrandom redeem from the vault (transfer first card from vault) (no VRF)
    }
...
contract UTDigitalRedeemStorage is UTGenericDroppableStorage {
    ...
    uint8       public immutable redeemMode;                    // 0 - Random redeam from the vault (with VRF), 1 - Nonrandom redeem from the vault (transfer first card from vault) (no VRF)
    ...
    constructor(
        ...
        uint8   _redeemMode,                        // 0 - Random redeam from the vault (with VRF), 1 - Nonrandom redeem from the vault (transfer first card from vault) (no VRF)
