...
contract Metagate is ERC721 {
    using Strings for uint256;

    address public shizo;
    ...
    mapping(uint256 => uint256) public totalViews;
    mapping(uint256 => uint256) public levels;
    mapping(uint256 => string) public names;
    mapping(uint256 => string) public descriptions;
    mapping(uint256 => string) public colors;
    mapping(uint256 => string) public videos;
    mapping(uint256 => string) public websites;

    event Purchase(address indexed seller, address indexed buyer, uint256 indexed tokenId, uint256 price);
    event LevelUp(address indexed owner, uint256 land, uint256 level);
    event NameChanged(address indexed owner, uint256 land, string name);
    event ColorChanged(address indexed owner, uint256 land, string color);
    event DescriptionChanged(address indexed owner, uint256 land, string description);
    event VideoChanged(address indexed owner, uint256 land, string video);
    event WebsiteChanged(address indexed owner, uint256 land, string website);

    mapping(uint256 => TokenOnMarketplace) public marketplace;

    constructor (address _shizo) ERC721("Metagate", "Metagate") {
        shizo = _shizo;
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(abi.encodePacked(baseURI, tokenId.toString()), "/metadata/")) : "";
    }
    ...
    function mint(address to, uint256 tokenId) public payable returns (uint256) {
        require(msg.value == PRICE, "Price is not correct");

        // LOGIC HERE, check signed msg and extract tokenId and type

        _safeMint(to, tokenId);
        types[tokenId] = 0;
        totalViews[tokenId] = 0;
        views[tokenId] = 0;
        levels[tokenId] = 1;
    ...
    function shizoRequiredForLvlup(uint256 tokenId) public view returns(uint256) {
        return levels[tokenId] * 10 * 10 ** ERC20(shizo).decimals(); // 10 SHZ * current level
    }

    function lvlup(uint tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "Only owner can do this");

        // LOGIC HERE

        uint256 requiredShz = shizoRequiredForLvlup(tokenId);

        require(IERC20(shizo).balanceOf(msg.sender) >= requiredShz, "Not enough SHZ");

        ERC20Burnable(shizo).burnFrom(msg.sender, requiredShz);
        levels[tokenId] += 1;

        emit LevelUp(ownerOf(tokenId), tokenId, levels[tokenId]);
    }
    ...
    function setName(uint tokenId, string memory name) external {
        ...
        names[tokenId] = name;
        emit NameChanged(ownerOf(tokenId), tokenId, name);
    }

    function setDescription(uint tokenId, string memory description) external {
        require(ownerOf(tokenId) == msg.sender, "Only owner can do this");

        // LOGIC HERE

        descriptions[tokenId] = description;
        emit DescriptionChanged(ownerOf(tokenId), tokenId, description);
    }

    function setColor(uint tokenId, string memory color) external {
        require(ownerOf(tokenId) == msg.sender, "Only owner can do this");
        require(isLand(tokenId), "This should be land");

        // LOGIC HERE

        colors[tokenId] = color;
        emit ColorChanged(ownerOf(tokenId), tokenId, color);
    }

    function setVideo(uint tokenId, string memory video) external {
        require(ownerOf(tokenId) == msg.sender, "Only owner can do this");
        require(isLand(tokenId), "This should be land");
        ...
        emit VideoChanged(ownerOf(tokenId), tokenId, video);
    }

    function setWebsite(uint tokenId, string memory website) external {
        require(ownerOf(tokenId) == msg.sender, "Only owner can do this");
        require(isLand(tokenId), "This should be land");

        // LOGIC HERE

        websites[tokenId] = website;
        emit WebsiteChanged(ownerOf(tokenId), tokenId, website);
    }
}
