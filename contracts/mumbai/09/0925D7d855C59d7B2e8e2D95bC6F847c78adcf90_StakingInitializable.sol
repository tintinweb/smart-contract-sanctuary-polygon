...
contract StakingInitializable is Ownable, ReentrancyGuard {
    ...
    event NewEndBlockAndRewardSupply(uint256 endBlock, uint256 rewardSupply);
     ...
     * @param _bonusEndBlock: end block (rewardSupply / rewardPerBlock)
     * @param _poolLimitPerUser: pool limit per user in stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @param _rewardSupply: total supply of reward
     */
    function initialize(
        IERC20 _stakedToken,
        IERC20 _rewardToken,
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user
                ...
                .sub(user.rewardDebt);
            if (pending > 0 || rewardSupply >= pending) {
    ...
    /**
     * @notice It allows the admin to update end blocks and reward supply
     * @dev This function is only callable by owner.
     * @param _rewardSupply: the new start block
     */
    function updateEndBlockAndRewardSupply(uint256 _rewardSupply)
        external
        onlyOwner
    {
        require(_rewardSupply > 0, "Reward supply must more than 0");
        ...
        rewardSupply = rewardSupply.add(_rewardSupply);

        bonusEndBlock = startBlock.add(rewardSupply.div(rewardPerBlock));

        // Set the lastRewardBlock as the startBlock
        lastRewardBlock = startBlock;

        // emit NewStartAndEndBlocks(_startBlock, _bonusEndBlock);
        emit NewEndBlockAndRewardSupply(bonusEndBlock, _rewardSupply);
    }
    ...
    function updateRewardPerBlock(uint256 _rewardPerBlock) external onlyOwner {
        require(_rewardPerBlock > 0, "Reward Per Block must more than 0");

        require(
            rewardPerBlock != _rewardPerBlock,
            "New rewardPerBlock must be different than current value"
        );
        ...
        bonusEndBlock = lastRewardBlock.add(rewardSupply.div(rewardPerBlock));
    ...
    function _updatePool() internal {
        ...
        if (stakedTokenSupply == 0 || rewardSupply == 0) {
...
contract StakingFactory is Ownable {
     ...
     * @param _poolLimitPerUser: pool limit per user in stakedToken (if any, else 0)
     * @param _admin: admin address with ownership
     * @param _rewardSupply: reward supply
    ...
    function deployPool(
        ...
        // require(_rewardSupply < 0, "Reward supply must more then 0");
        ...
        StakingInitializable(smartChefAddress).initialize(
            _stakedToken,
            _rewardToken,
            _rewardPerBlock,
            _startBlock,
