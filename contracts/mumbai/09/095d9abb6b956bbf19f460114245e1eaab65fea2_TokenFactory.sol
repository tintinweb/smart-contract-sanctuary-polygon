...
// File: HeftyVerse/heftiverse/contracts/VoucherLib.sol
...
interface INFTTemplate is IERC721Upgradeable
{
...
// File: HeftyVerse/heftiverse/contracts/Factory.sol
...
contract TokenFactory is Initializable, BasicMetaTransaction {
    ...
    function initialize(address _templateAddress)external initializer{
        templateAddress=_templateAddress;
    //    template=INFTTemplate(_templateAddress);

    }
    function create721Token(string memory name, string memory symbol,address _admin,address _superAdmin,address _token20,address _marketplace)external returns(address token)
    {
        token = Clones.clone(templateAddress);
        userNFTContracts[msg.sender]=token;
        template=INFTTemplate(token);
        template.initialize(
            name,
            symbol,
            _admin,
    ...
    //     INFTTemplate(address(_voucher.nftAddress)).redeem(_voucher, redeemer);
    // }
