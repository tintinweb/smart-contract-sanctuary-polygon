    ...
    ReentrancyGuardUpgradeable
{
    ...
    ISheeshaVesting public vesting;
    ...
    function initialize(
        ...
        tokenRewards = 100_000_000e18; // need to be change before deployment
        __Ownable_init();
    ...
    function setVesting(address _vesting) external onlyOwner {
        ...
        vesting = ISheeshaVesting(_vesting);
    }

    /**
     * @dev Sets auto compound Native contract
     * param _autoCompound address of auto compound Native contract
     */
     ...
     */
    function add(IERC20 _token) external onlyOwner {
        ...
        poolInfo.push(
            PoolInfo({
                token: _token,
                lastRewardBlock: lastRewardBlock, // update lastRewardBlock
                accSheeshaPerShare: 0 // update accSheeshaPerBlock
            })
        );
        started = true;
    }
    ...
    function addRewards(uint256 _amount) external {
        ...
        tokenRewards = tokenRewards + _amount;
    }
    ...
    /**
     * @notice migrates user staked amounts from previous vault contract
        ...
        nonReentrant
    {
        ...
        (uint256 aLeftover, uint256 unlockedAmount) = vesting
            .calculateAvailableAmountForStaking(_msgSender());
        uint256 userStakeAmount = user.amount - aLeftover;
        uint256 userAvailableAmount = userStakeAmount + unlockedAmount;
        ...
        if (_amount > 0) {
            if (userStakeAmount < _amount) {
                uint256 vestingAmount = _amount - userStakeAmount;
                vesting.withdrawFromStaking(_msgSender(), vestingAmount);
            }
            user.amount = user.amount - _amount;
            uint256 burnAmount = (_amount * 2) / 100;
    ...
    function emergencyWithdraw(uint256 _pid) external override nonReentrant {
        ...
        (uint256 aLeftover, uint256 unlockedAmount) = vesting
            .calculateAvailableAmountForStaking(_msgSender());
        uint256 userAvailableAmount = user.amount + unlockedAmount - aLeftover;
        if (unlockedAmount > 0) {
            vesting.withdrawFromStaking(_msgSender(), unlockedAmount);
        }
        if (locker != address(0)) {
            uint256 unlocked = ISheeshaVotesLocker(locker).unlockedSHOf(
                _msgSender()
        ...
        uint256 burnAmount = (userAvailableAmount * 2) / 100;
        sheesha.burn(burnAmount);
        IERC20Upgradeable(address(sheesha)).safeTransfer(
            _msgSender(),
            userAvailableAmount - burnAmount
        );
        user.rewardDebt = (user.amount * pool.accSheeshaPerShare) / 1e12;
        ...
        returns (uint256)
    {
        ...
        uint256 tokenSupply;
        if (sheesha.balanceOf(address(this)) >= tokenRewards) {
            tokenSupply = sheesha.balanceOf(address(this)) - tokenRewards;
        }
        if (block.number > pool.lastRewardBlock && tokenSupply != 0) {
            ...
            uint256 sheeshaReward = multiplier * sheeshaPerBlock;
            accSheeshaPerShare =
                accSheeshaPerShare +
                ((sheeshaReward * 1e12) / tokenSupply);
        }
    ...
    function token() external view override returns (address) {
        return address(ISheeshaToken(sheesha));
    }
    ...
    function staked() external view override returns (uint256) {
        uint256 balance = poolInfo[0].token.balanceOf(address(this));
        return balance > tokenRewards ? balance - tokenRewards : 0;
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        tokenRewards = tokenRewards - sheeshaReward;
