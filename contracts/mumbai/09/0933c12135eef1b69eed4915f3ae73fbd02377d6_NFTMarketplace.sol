...
contract NFTMarketplace is INFTMarketplace, Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function fetchListings() public view returns (uint256[] memory) {
        /* retrieves all current listings, or a range of listings. 
            This might be tricky due to how solidity handles returning dynamic arrays, 
            and might require additional off-chain processing to be useful. */
        // Return an array of all currently listed tokenIds
    }

    function fetchMyNFTs() public view returns (uint256[] memory) {
        /*( retrieves all NFTs owned by the user in the marketplace.) */
        // Return an array of all tokenIds owned by the sender
    }

    function fetchMyListings() public view returns (uint256[] memory) {
        /*(retrieves all NFTs listed by the user in the marketplace.) */
        // Return an array of all tokenIds listed by the sender
    }
