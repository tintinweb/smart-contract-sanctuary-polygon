...
contract Test{    
    ...
    function registration(address userAddress, address referrerAddress, uint256 id) private{
        ...
        emit test(1, ((mlevelPrice[1]*1e18)/maticRate));
    ...
    function updateX6Referrer(address userAddress, address referrerAddress, uint8 level, uint256 amount) private{
        ...
        {   
          emit test(3, users[referrerAddress].holdAmount[level]);      
    ...
    function autoUpgradeLevel(address _user, uint8 level) private{
        if(!users[_user].activeX3Levels[level])
        {
            ...
            uint256 maticPerDollor=(users[referrerAddress].holdAmount[level-1]/(mlevelPrice[level-1]*(level+3)));
            ...
            for(uint8 i=0;i<7;i++)
            {
                ...
                {
                    users[referrerAddress].withdrawable+=((levelUSD*(REFERRAL_PERCENTS[i+1]/1000))/1e20)*maticPerDollor;                
                    emit LevelBonus(referrerAddress, msg.sender,(levelUSD*REFERRAL_PERCENTS[i+1])/1000, ((levelUSD*(REFERRAL_PERCENTS[i+1]/1000))/1e20)*maticPerDollor, i+1);
