...
contract skyLottoNFTgameResult is VRFConsumerBaseV2 {
    ...
    uint256 public gameSpan = 10 minutes;
    ...
    function fulfillRandomWords(
        ...
        for (uint256 _index = 0; _index < 6; _index++) {
            gameResult[gameId].randomWords.push(
                (_randomWords[_index] % maxNo) + 1
            );
        }
        bool checkDuplicate;
        for (uint256 k = 0; k < gameResult[gameId].randomWords.length; k++) {
            for (
                uint256 j = k + 1;
                j < gameResult[gameId].randomWords.length;
                j++
            ) {
                if (
                    gameResult[gameId].randomWords[k] ==
                    gameResult[gameId].randomWords[j]
                ) {
                    checkDuplicate = true;
                }
            }
    ...
    function getResultDetails()
        ...
        returns (
            uint256 gameId,
            uint8 status,
            uint256 _resultDeclaredGameId
        )
    {
        return (
            lastActiveGameId,
            gameResult[lastActiveGameId].status,
            resultDeclaredGameId
        );
    }
