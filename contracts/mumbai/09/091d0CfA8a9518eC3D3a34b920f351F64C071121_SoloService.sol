...
contract SoloService is BaseTournamentService {
  ...
  // reviewed
  // pay fee for tournament
  function _payForSolo(
    address _currency,
    uint256 _buyIn,
    address _payer,
    uint256 _fixedFee
  ) internal {
    IERC20(_currency).transferFrom(_payer, address(this), _buyIn + _fixedFee);
  }
  ...
  function _refundByCancel(uint64 _serviceID, uint64 _tournamentID) internal virtual override {
    ...
    bool betNFT = false;
    if (tournament.configs.data.length > 0) {
      (betNFT) = abi.decode(tournament.configs.data, (bool));
    }
    ...
    for (uint256 i = 0; i < size; i++) {
      ...
      IERC20(tournament.configs.currency).transfer(receiver, tournament.configs.buy_in + fixedFeeCheckpoint[_tournamentID]);
    }
  }
  ...
  function joinTournament(bytes memory _signature, bytes memory _params) external virtual override onlyRoler("joinTournament") {
    ...
    (, uint64 tournamentID, address joiner, uint256 championID, uint16 stance) = abi.decode(_params, (uint64, uint64, address, uint256, uint16));
    ...
    require(tournament.configs.status == TournamentTypes.TournamentStatus.AVAILABLE, "TRV: Tournament Unavailable");
    require(IChampionUtils(championUtils).isOriginalOwnerOf(signer, championID), "TRV: Require owner"); // require owner of token
    require(!_isAlreadyJoin(tournament.warriors, championID), "TRV: Already joined"); // check if join or not
    require(tournament.configs.size > tournament.warriors.length, "TRV: Tournament filled"); // check tournament size

    require(_isInWhitelist(tournament.restrictions.whitelist, championID), "TRV: whitelist required"); // if WL, no need for BL
    ...
    _payForSolo(tournament.configs.currency, tournament.configs.buy_in, joiner, fixedFeeCheckpoint[tournamentID]);
  ...
  function createTournament(
    ...
    for (uint64 i = 0; i < size; i++) {
      ...
      ITournamentState(tournamentState).createTournament(serviceID, currentID, data, _restrictions[i]);
      fixedFeeCheckpoint[currentID] = fixedFee;
      currentID += 1;
    }
  ...
  function completeTournament(
    ...
    require(tournament.configs.status == TournamentTypes.TournamentStatus.READY, "Not allow"); // can only complete if ready
    bool betNFT = false;
    if (tournament.configs.data.length > 0) {
      (betNFT) = abi.decode(tournament.configs.data, (bool));
    }
    ...
    platformShare[tournament.configs.currency] += (fixedFeeCheckpoint[_tournamentID] * tournament.configs.size);

    if (prizePool > threshold) {
      platformShare[tournament.configs.currency] += ((prizePool * cutPercent) / 1000);
    }

    uint256 winnings;
    if (prizePool >= platformShare[tournament.configs.currency]) {
      winnings += prizePool - platformShare[tournament.configs.currency];
    }
    address winner;
...
contract BaseTournamentService is BaseService, ITournamentService {
  ...
  function completeTournament(
    ...
    uint256 prizePool = tournament.configs.buy_in * tournament.configs.size + tournament.configs.top_up;
    platformShare[tournament.configs.currency] += ((prizePool * tournament.configs.fee_percentage) / 10000);

    uint256 winnings = prizePool - platformShare[tournament.configs.currency];
    for (uint256 i = 0; i < _warriors.length; i++) {
      require(tournament.warriors[i].ID == _warriors[i].ID, "Champion ID mismatch");

      _warriors[i].account = tournament.warriors[i].account; // pay for joiner
...
interface ITournamentService {
  ...
  // create tournament
  function createTournament(TournamentTypes.TournamentConfigs[] memory configs, TournamentTypes.TournamentRestrictions[] memory _restrictions) external;
