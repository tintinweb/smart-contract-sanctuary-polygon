...
contract Subscription is OwnableUpgradeable, ERC721EnumerableUpgradeable, ISubscription {
    ...
    //nftId -> PaymentInfo
    mapping(uint256 => Metadata) private paymentInfo;

    event BuySubscription(bytes32 executedId, address owner, uint256 id);

    modifier onlyMultiPlugins(bytes32 _checkedPluginName, uint256 _version) {
        require(
            PluginsList.SUBSCRIPTION_BUY == _checkedPluginName
            || PluginsList.SUBSCRIPTION_BUY_FOR_SINGLE_POST == _checkedPluginName,
            "SinglePostData: wrong plugin");
    ...
    function initialize(
        ...
        __Ownable_init();
        __ERC721_init("Crypto.Page Subscription", "PAGE.Subscription");
    ...
    ) external override onlyMultiPlugins(vars.pluginName, vars.version) returns (uint256) {
        ( , address owner, , , ) =
        abi.decode(vars.data,(address, address, uint256, uint256, uint256));
        uint256 tokenId = idCounter.current();
        _mint(owner, tokenId);
        idCounter.increment();
        writeInfo(tokenId, vars.data);

        emit BuySubscription(vars.executedId, owner, tokenId);

        return tokenId;
    }

    function tokenURI(uint256 tokenId) public view virtual override(ERC721Upgradeable, ISubscription) returns (string memory) {
        _requireMinted(tokenId);
    ...
    function getPaymentInfo(uint256 _nftId) external override view returns(DataTypes.PaymentInfo memory info) {
        address currentOwner = super.ownerOf(_nftId);
        return convertMetadata(paymentInfo[_nftId], currentOwner);
    }
    ...
    function writeInfo(uint256 tokenId, bytes calldata data) private {
        (address communityId, , uint256 paymentType, uint256 startTime, uint256 endTime) =
        abi.decode(data,(address, address, uint256, uint256, uint256));
        paymentInfo[tokenId] = Metadata(startTime, endTime, communityId, DataTypes.PaymentType(paymentType));
    }

    function _requireMinted(uint256 tokenId) internal override view virtual {
        require(_exists(tokenId), "Subscription: invalid token ID");
    }

    function convertMetadata(Metadata storage _inData, address _tokenOwner) private view returns(DataTypes.PaymentInfo memory outData) {
        outData.startTime = _inData.startTime;
        outData.endTime = _inData.endTime;
        outData.communityId = _inData.communityId;
        outData.owner = _tokenOwner;
        outData.paymentType = _inData.paymentType;
    }
    ...
    function checkTrustedPlugin(bytes32 _checkedPluginName, uint256 _version) private view {
        require(
            ...
            "Subscription: caller is not the plugin"
        );
    }
}