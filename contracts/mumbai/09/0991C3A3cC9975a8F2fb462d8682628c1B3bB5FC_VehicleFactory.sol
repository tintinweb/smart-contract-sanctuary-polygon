   ...
   /**
    ...
    constructor(address _mgge_contract, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        _setPropValues(_type_id, "capacity", 10, 100);  
         ...
         //飞行器2（生命     
         _type_id =  _addItem(120002, 150, 320, "hp");  
        _setPropValues(_type_id, "potential", 1, 5);  
        ...
        _setPropValues(_type_id, "capacity", 10, 100);  
         ...
         //飞行器3（耐久 
         _type_id =  _addItem(120003, 100, 350, "duration");  
        _setPropValues(_type_id, "potential", 1, 5);  
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
        ...
        onlyAuthorizationContract //只允许主合约调用
    {
        ...
        props["defense"]   = 0; // 防御   , 总值     
        props["hp"]        = 0; // 生命   , 总值     
        props["duration"]  = 0;// 耐久度 , 总值     
        uint256 move;
        uint256 capacity;
        uint256 potential;
        ...
        if (settings[_type_id]["defense"].section > 0 ) {
             props["defense"] = settings[_type_id]["defense"].base + randomResult % settings[_type_id]["defense"].section;
        }

        if (settings[_type_id]["hp"].section > 0) {
            props["hp"]      = settings[_type_id]["hp"].base + randomResult % settings[_type_id]["hp"].section;
        }

        if (settings[_type_id]["duration"].section > 0) {
            props["duration"]      = settings[_type_id]["duration"].base + randomResult % settings[_type_id]["duration"].section;
        }

        uint margin = vehicles[_type_id].prop_sum - (props["duration"] + props["defense"] + props["hp"]);
        props[vehicles[_type_id].margin_name] = vehicles[_type_id].prop_sum - margin;

        if (settings[_type_id]["move"].section > 0) {
            move = settings[_type_id]["move"].base + randomResult % randomResult % settings[_type_id]["move"].section;
        }

        if (settings[_type_id]["capacity"].section > 0) {
            capacity = settings[_type_id]["capacity"].base + randomResult % randomResult % settings[_type_id]["capacity"].section;
        }

        if (settings[_type_id]["potential"].section > 0) {
            potential = settings[_type_id]["potential"].base + randomResult % randomResult % settings[_type_id]["potential"].section;
        }
        ...
        requiresUpgrade(_to, _id, _orderid, signature)
    {
        ...
        uint newValue = upgrade_base + randomResult % upgrade_max;
         ...
         if (rand == 0) {
            characters[_id].defense += newValue;
         } else if (rand == 1) {
            characters[_id].hp += newValue;
         } else {
            characters[_id].duration += newValue;
         }
    }
