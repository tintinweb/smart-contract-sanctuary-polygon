...
contract ERC1155m is ERC1155, Ownable, AccessControl {
    ...
    function mintBatch(
        ...
        require(accounts.length == ids.length, "ERC1155m: accounts and ids length mismatch");
        require(ids.length == amounts.length, "ERC1155m: ids and amounts length mismatch");
        for (uint256 i = 0; i < ids.length; i++) {
            if (!IMoleculeController(_moleculeMint).check(accounts[i])) {
                revert AccountNotAllowedToMint(accounts[i]);
            }
