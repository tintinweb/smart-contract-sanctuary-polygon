...
contract ArenaEscrowERC20 is Owned(msg.sender) {
    ...
    function createEvent(uint128 entryFee, ERC20 currency) public returns (uint256)  {
        if (entryFee == 0) revert("invalid entry fee");

        idCounter += 1;
        eventDetails memory ed = eventDetails(msg.sender, 0, EventState.INITIAL, entryFee, address(currency));
    ...
    function joinEvent(uint256 eventID) public payable {
        ...
        ERC20 token = ERC20(deets.currency);
        SafeTransferLib.safeTransferFrom(token, msg.sender, address(this), deets.entryFee);

        entries[eventID][msg.sender] = eventEntry(deets.entryFee, 0);
