...
contract NFTYDashPremium is ERC1155, Pausable, Ownable {
    ...
    string public baseURI = "https://bafybeibrox4nr4npwq2tirkaw7wuaiat5p6hysgiunfrk3wwm3bv5gbeym.ipfs.nftstorage.link/";

    event GovernanceMint(address to, uint256 amount);
    event PublicMint(address to, uint256 amount);
    event PublicFreeMint(address to, uint256 amount);

    constructor() ERC1155("") {}

    /** ----- Public ----- */

    function mintPublic(uint256 amount_) external payable whenNotPaused {
        ...
        require(totalSupply <= MAX_SUPPLY, "MAXED_COL");
        require(msg.value == amount_ * price, "!PRICE");
    ...
    function mintFreePublic(uint256 amount_, bytes32[] calldata merkleProof_) external whenNotPaused {
        require(mintedPerWallet[msg.sender] <= allowedPerWallet, "MAXED");
        ...
        emit PublicFreeMint(msg.sender, amount_);
    }
