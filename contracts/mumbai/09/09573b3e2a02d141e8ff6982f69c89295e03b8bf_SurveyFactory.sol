...
contract SurveyFactory {
    ...
    function deploySurvey(string memory ipfsAddress, address _currency, uint256 _bounty) external returns (address surveyAddress) {
         ...
         emit SurveyCreation(msg.sender, ipfsAddress, block.timestamp, _currency, _bounty);
         return surveyAddress;
    }

    event SurveyCreation(address indexed owner, string surveyAddress, uint256 creationDate, address currency, uint256 bounty);
}