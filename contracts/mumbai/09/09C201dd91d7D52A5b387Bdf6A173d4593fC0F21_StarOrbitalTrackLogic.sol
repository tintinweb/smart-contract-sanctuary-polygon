...
contract StarOrbitalTrackLogic is
    StarMeta,
    Initializable,
    UUPSUpgradeable,
    OwnableUpgradeable
{
    ...
    event ContractSettingUpdated(address indexed _starOrbitalTrackDataAddress);
    ...
    function updateContractSetting(address _starOrbitalTrackDataAddress)
        external
        onlyOwner
    {
        require(
        ...
        // event
        emit ContractSettingUpdated(_starOrbitalTrackDataAddress);
    }

    /**
     * @notice check planet owner Reach Max Count or not, only use when invite group ID != 0
     *
     * @param _renterAddress renter wallet address
        ...
        if (_inviteGroupId != 0) {
            ...
            for (uint256 i = 0; i < starOTDatas.length; i++) {
                ...
                if (
                    ...
                    _renterAddress == starOTDatas[i].planetOwnerAddress
                ) {
    ...
    /**
     * @notice StarOwnerDetachPlanet, only calc. the old rental balance
     * return (starOwnerReceive, planetOwnerReceive)
     *
     * @param _starId star ID
     * @param _rentedDataIndex Index
     * @param _earlyDetachFeePercent Early Detach Fee Percent
     * @param _percentMaxValue Percent Max Value
     */
    function _getStarOwnerDetachPlanetOldRentedBalance(
        uint256 _starId,
        uint256 _rentedDataIndex,
        uint256 _earlyDetachFeePercent,
        uint256 _percentMaxValue
    ) internal view returns (uint256 starOwnerReceive, uint256) {
        // renew handle
        uint256 rentalFeeAfterPlatform = starOrbitalTrackDataContract
        .getStarOrbitalTrackRentedData(_starId)[_rentedDataIndex]
        ...
        if (
            ...
            // if rent full old rental time, get all fee
            starOwnerReceive = rentalFeeAfterPlatform;
        } else {
            // if early detach, star owner need to pay earlyDetachFeePercent
            uint256 earlyDetachAdjustFee = _getEarlyDetachFee(
                rentalFeeAfterPlatform,
                _earlyDetachFeePercent,
                _percentMaxValue
            );
            // the old rented contract just give earlyDetachFeePercent to planet owner
            starOwnerReceive = rentalFeeAfterPlatform - earlyDetachAdjustFee;
        }

        return (starOwnerReceive, rentalFeeAfterPlatform - starOwnerReceive);
    }

    /**
     * @notice PlanetOwnerDetachPlanet, only calc. the old rental balance
     * return (starOwnerReceive), planetOwnerReceive will get 0
     *
     * @param _starId star ID
     * @param _rentedDataIndex Index
     */
    function _getPlanetOwnerDetachPlanetOldRentedBalance(
        uint256 _starId,
        uint256 _rentedDataIndex
    ...
    /**
     * @notice StarOwnerDetachPlanet, calc. the real rental balance
     * return (starOwnerReceive, planetOwnerReceive)
     *
     * @param _starId star ID
     * @param _planetId planet ID
     * @param _slotIndex SlotIndex
     * @param _earlyDetachFeePercent Early Detach Fee Percent
     * @param _percentMaxValue Percent Max Value
     */
    function getStarOwnerDetachPlanetBalance(
        ...
        StarOrbitalTrackData
            memory orbitalTrackData = starOrbitalTrackDataContract
                .getStarOrbitalTrackData(_starId)[_slotIndex];
        // rentalFeeAfterPlatform only count the new rental data
        uint256 rentalFeeAfterPlatform = orbitalTrackData
            .rentalFeeAfterPlatform;
        uint256 starOwnerReceive = 0;
        uint256 newStarOwnerReceive = 0;

        for (
            uint256 i = 0;
            i <
            starOrbitalTrackDataContract
                .getStarOrbitalTrackRentedData(_starId)
                .length;
            i++
        ) {
            if (
                starOrbitalTrackDataContract
                .getStarOrbitalTrackRentedData(_starId)[i].slotIndex ==
                _slotIndex &&
                starOrbitalTrackDataContract
                .getStarOrbitalTrackRentedData(_starId)[i].planetId ==
                _planetId &&
                starOrbitalTrackDataContract
                .getStarOrbitalTrackRentedData(_starId)[i].oldRentalEndTime !=
                orbitalTrackData.rentalEndTime
            ) {
                (
                    starOwnerReceive,
                    planetOwnerReceive
                ) = _getStarOwnerDetachPlanetOldRentedBalance(
                    _starId,
                    i,
                    _earlyDetachFeePercent,
                    _percentMaxValue
        ...
        // not renew handle, or already within new renew contract
        if (block.timestamp >= orbitalTrackData.rentalStartTime) {
            if (block.timestamp >= orbitalTrackData.rentalEndTime) {
                // if rent full time, get all fee
                newStarOwnerReceive = rentalFeeAfterPlatform;
            } else {
                // if early detach, star owner need to pay earlyDetachFeePercent
                uint256 earlyDetachAdjustFee = _getEarlyDetachFee(
                    rentalFeeAfterPlatform,
                ...
                // origin starOwnerReceive
                newStarOwnerReceive =
                    (rentalFeeAfterPlatform *
                        (block.timestamp - orbitalTrackData.rentalStartTime)) /
                    (orbitalTrackData.rentalEndTime -
                        orbitalTrackData.rentalStartTime);

                if (earlyDetachAdjustFee > newStarOwnerReceive) {
                    // if not pass 10% time, star owner get 0%
                    newStarOwnerReceive = 0;
                } else {
                    newStarOwnerReceive =
                        newStarOwnerReceive -
                        earlyDetachAdjustFee;
                }
            }
        }
        planetOwnerReceive += rentalFeeAfterPlatform - newStarOwnerReceive;

        return (starOwnerReceive + newStarOwnerReceive, planetOwnerReceive);
    }

    /**
     * @notice PlanetOwnerDetachPlanet, calc. the real rental balance
     * return (starOwnerReceive, planetOwnerReceive)
     *
     * @param _starId star ID
     * @param _planetId planet ID
    ...
    function getPlanetOwnerDetachPlanetBalance(
        ...
        for (
            ...
            if (
                ...
                // handle renew price
                starOwnerReceive = _getPlanetOwnerDetachPlanetOldRentedBalance(
                    _starId,
                    i
                );
                break;
        ...
        if (block.timestamp >= orbitalTrackData.rentalStartTime) {
            if (block.timestamp >= orbitalTrackData.rentalEndTime) {
                ...
                if (
                    rentalFeeAfterPlatform <
                    newStarOwnerReceive + earlyDetachAdjustFee
                ) {
                    // if already pass 90% time, star owner get 100%
                    newStarOwnerReceive = rentalFeeAfterPlatform;
                } else {
                    newStarOwnerReceive += earlyDetachAdjustFee;
                }
            ...
            // handle renewed rental and detach before the new start date
            uint256 earlyDetachAdjustFee = _getEarlyDetachFee(
                rentalFeeAfterPlatform,
                _earlyDetachFeePercent,
                _percentMaxValue
            );
            planetOwnerReceive += rentalFeeAfterPlatform - earlyDetachAdjustFee;
            starOwnerReceive += earlyDetachAdjustFee;
        }
    ...
    /**
     * @notice get Early Detach Fee
     *
     * @param _fee rentalFee
     * @param _earlyDetachFeePercent _earlyDetachFeePercent
     * @param _percentMaxValue _percentMaxValue
     */
    function _getEarlyDetachFee(
        uint256 _fee,
        uint256 _earlyDetachFeePercent,
        uint256 _percentMaxValue
    ) internal view returns (uint256) {
        return (_fee * _earlyDetachFeePercent) / _percentMaxValue;
    }
}
