...
interface NFTSTAKING{
    function addPoolBalance(uint256 _poolId) external payable;
}
...
contract BLRC_ROI {
    ...
    uint256 public minInvestment = 0.0003 ether;
    ...
    struct PlayerDailyRounds {
        uint256 investment; // total referrals user has in a particular rounds
    ...
    function setStakingContract(address _stakingAddress,address _lottoAddress) external {
    ...
    function invest(address payable referrer) public payable {
        ...
        refPayout(msg.sender, _amount, isNew);
        ...
        TOP_REFERRAL_BONUS = TOP_REFERRAL_BONUS.add(_amount.mul(3).div(100));
            
        plyrRnds_[user.referrer][accumulatRoundID].investment += _amount;
        emit ReferralDetails(msg.sender, user.referrer, accumulatRoundID);
            
        sendAccumulatedAmount();
        setTVLPercent();
        _highestReferrer(user.referrer);
        NFTSTAKING(stakingAddress).addPoolBalance{ value: _amount.mul(5).div(1000) }(1);
        NFTSTAKING(stakingAddress).addPoolBalance{ value: _amount.mul(1).div(100) }(2);
        NFTSTAKING(stakingAddress).addPoolBalance{ value: _amount.mul(15).div(1000) }(3);
        payable(lottoAddress).transfer(_amount.mul(1).div(100));
    ...
    function _highestReferrer(address _referrer) private {
        ...
        for (uint8 i = 0; i < 5; i++) {
            ...
            if (
                plyrRnds_[_referrer][accumulatRoundID].investment >
    ...
    function claimNft(uint8 _type) external {
        ...
        require(checkNftClaimaible(msg.sender,_type),"claimable condition not matching");
    ...
    function checkNftClaimaible(address user,uint8 _type) public view returns(bool) {
        bool flag = false;
        if(_type==1 && userTeams[user].totalTeam >= 3 && users[user].refCount[0]>=1)//_type==1 && userTeams[user].totalTeam >= 10 && users[user].refCount[0]>=1
        {
            flag = true;
        }
        if(_type==2 && userTeams[user].totalTeam >= 4 && users[user].refCount[0]>=2)//_type==2 && userTeams[user].totalTeam >= 30 && users[user].refCount[0]>=3
        {
            flag = true;
        }
        if(_type==3 && userTeams[user].totalTeam >= 5 && users[user].refCount[0]>=3)//_type==3 && userTeams[user].totalTeam >= 60 && users[user].refCount[0]>=6
        {
            flag = true;
        }
        return flag;
    ...
    function setTVLPercent() internal {
        ...
        if(balance/0.1 ether > currentRange)
        {
            TVLROI += 100;
        }
        else if(balance/0.1 ether < currentRange){
            TVLROI = 0;
        }
        currentRange = balance/0.1 ether;
    }
    ...
    function collect(address _user) private {
        ...
        User storage _upline = users[users[_user].referrer];
        EarningInfo storage _uplineE = earningInfo[users[_user].referrer];
    ...
    function getContractInfo() external view returns(uint256 _championBalance,uint256 _nextResultIn,uint256 _roundId) {
        return (TOP_REFERRAL_BONUS,nextResultIn,accumulatRoundID);
    }
    ...
    {
        EarningInfo memory earning = earningInfo[userAddress];
        ...
        for (uint256 i = 0; i < REFERRAL_PERCENTS.length; i++) {
            _referralearnings[i] = users[userAddress].refEarningsLevel[i];
            _referrals[i] = users[userAddress].refCount[i];
        }
        return (_referrals, _referralearnings, REFERRAL_PERCENTS,earning.refEarningOnDividend,
            earning.topRefBonus);
    }
        ...
        returns (address[] memory _players, uint256[] memory counts)
    {
        ...
        for (uint8 i = 0; i < 5; i++) {
            ...
            counts[i] = plyrRnds_[_players[i]][roundId].investment;
        }
