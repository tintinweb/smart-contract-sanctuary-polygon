...
contract ItemTransfer is IItemTransfer, GameBase, TokenAccess {
    constructor(address game)
    GameBase(game)
    TokenAccess(game) {
    }

    function transfer(TransferRecord calldata record) external virtual override onlyGameUser(record.worldId, record.tokenId) {
        ...
        ItemStorage itemStorage = getWorld(record.worldId).itemStorage();
        for (uint256 i; i < record.itemDefinitionIds.length; i++) {
            uint256 itemDefinitionId = record.itemDefinitionIds[i];
            int64 amount = record.amounts[i];

            itemStorage.addItem(record.tokenId, itemDefinitionId, amount * -1);
            itemStorage.addItem(record.targetTokenId, itemDefinitionId, amount);
        }
    ...
    function validateTransfer(TransferRecord calldata record) external view virtual override returns(TransferValidationResult memory) {
        ...
        require(address(world) != address(0), "wrong worldId");

        Character character = world.character();
        bool fromIsRevealed = character.isRevealeds(record.tokenId);
        bool toIsRevealed = character.isRevealeds(record.targetTokenId);
    ...
    function _validateAmounts(TransferRecord calldata record) private view returns(bool[] memory) {
        ...
        if (fromIsRevealed) {
            ItemStorage itemStorage = getWorld(record.worldId).itemStorage();
            int64[] memory items = itemStorage.getItems(record.tokenId, itemDefinitionIds);
            for (uint256 i; i < itemDefinitionIds.length; i++) {
