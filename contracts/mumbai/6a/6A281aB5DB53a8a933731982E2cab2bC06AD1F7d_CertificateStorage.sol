...
contract CertificateStorage  {
    ...
    modifier adminOperation {
        require(BQAdminModuleSC == msg.sender||BQManager == msg.sender, "unauthorized");
         _;
    }
    ...
    function signCertificate( uint certificateId, address signer) public   restrictedToIssuer returns(bool) {
        require(certificateContentbyId[certificateId].active, string(abi.encodePacked(toString(certificateId),"certificate does not exist or deleted")));
        require(signControl[certificateId][signer]==false,string(abi.encodePacked(toString(certificateId)," certificate already signed by user")));
    ...
    function getCertificateDetail(uint certificateId) public view returns( uint256 certId, bool isactive, string memory certificateDetailJson,uint256 creationDate, uint256 totalSignatures,uint8 docType,string memory courseInfo,address academicInstitutionIdentitySC, string memory academicInstitutionName)
    {
        if(isCertificateIssuedByInstitution(certificateId)) {
            ...
            return ( certificateId,cert.active,certificateDetailJson,cert.creationdDate,totalSignatures,cert.certificateType,courseInformation[cert.course],academicInstitutionIdentity,academicInstitutionName);
        }
    }
    ...
    function setBaseURL(string memory _baseURL) public  returns(bool) {
        require(msg.sender == academicInstitutionIdentity || msg.sender == BQManager, "Unauthorized");
        baseURL = _baseURL;

        return true;
    }
    function getCertificateURL(uint certificateId) public view returns(string memory) {
        return(string(abi.encodePacked(baseURL,toString(certificateId))));
    }
    function setCourse(uint32 courseId, string memory courseInfo) public returns(bool) {
        require(msg.sender == academicInstitutionIdentity || msg.sender == BQManager, "Unauthorized");
    ...
    function getStudies() public view returns(string[] memory allCourses) {
   ...
   //=========  Module migration functions  =================//
    ...
    //Auxiliar functions
    bytes16 private constant _caracters = "0123456789abcdef";
    ...
    function toString(uint256 value) internal pure returns (string memory) {
        unchecked {
            ...
            while (true) {
                ...
                /// @solidity memory-safe-assembly
                assembly {
                    mstore8(ptr, byte(mod(value, 10), _caracters))
                }
                value /= 10;
                if (value == 0) break;
            }
