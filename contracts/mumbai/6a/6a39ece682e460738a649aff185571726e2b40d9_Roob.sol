...
contract Roob is ERC20("ROOB", "ROOB"), Ownable {
    ...
    function claimReward(uint256 _slothRoobTokenId) public returns (uint256) {
        uint256 totalUnclaimedReward = 0;
        uint256 extraBouns = 0;
        
        require(stake[_slothRoobTokenId].isStaked == true, "Token is not stake yet");
        require(iSlothRoob.ownerOf(_slothRoobTokenId) == msg.sender, "Caller does not own the token being claimed for.");

        totalUnclaimedReward = computeUnclaimedReward(_slothRoobTokenId);

        uint256 daysPass = totalUnclaimedReward / stake[_slothRoobTokenId].baseRate;
        if(daysPass >=15 && daysPass < 30){
            extraBouns = ( 5 * totalUnclaimedReward ) / 100;
        }else if (daysPass >= 30 ){
    ...
    function claimRewardsForSloothRoob(uint256[] calldata _slothRoobTokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _slothRoobTokenIds.length; i++) {
            ...
            uint256 unclaimedReward = computeUnclaimedReward(_slothRoobTokenId);
    ...
    function getUnclaimedRewardAmountForSlothRoob(uint256 _tokenId) public view returns (uint256) {
