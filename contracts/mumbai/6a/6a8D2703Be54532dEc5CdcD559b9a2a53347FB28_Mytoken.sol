...
contract ERC7254 is ERC20 {
    ...
    function updateReward(address[] memory token, uint256[] memory amount) public virtual {
        ...
        if(totalSupply() != 0){
            for( uint256 i = 0; i < token.length; ++i){
                require(isTokenReward[token[i]], "ERC7254: token reward is not approve");
    ...
    */
    function _inReward(address user, uint256 amount) internal virtual {
        for (uint256 i = 0; i < _tokenReward.length; ++i){
            UserInformation memory userSender = informationOf(_tokenReward[i], user);
            userSender.inReward += amount * rewardPerShare[_tokenReward[i]];
        }
    }
    ...
    */
    function _outReward(address user, uint256 amount) internal virtual {
        for (uint i = 0; i < _tokenReward.length; ++i){
            UserInformation memory userTo = informationOf(_tokenReward[i], user);
    ...
    *
    */
    function _addTokenReward(address[] memory token) internal virtual {
...
contract Mytoken is ERC7254  {
    ...
    function addTokenReward(address[] memory tokenReward) external {
        _addTokenReward(tokenReward);
    }
}