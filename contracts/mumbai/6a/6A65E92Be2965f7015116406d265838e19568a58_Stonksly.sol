...
contract Stonksly is StonkslyInterface, Ownable {
    ...
    function finalizeSale(
        ...
        uint256 normalizedMaticPriceInUsd = uint256(price) * 1e10; //1000000000000000000
        uint256 maticPriceInCents = normalizedMaticPriceInUsd * 100; //100000000000000000000
        uint normalizedAssetPrice = _assetPrice * 1e18; //100000000000000000000

        uint256 sTokensValueInCents = (normalizedAssetPrice * request.amount) /
            1e18; // 100000000000000000000

        uint256 maticAmount = (sTokensValueInCents * 1e18) / maticPriceInCents; // 1000000000000000000

        // //1% fee
        uint256 maticToWithdraw = (maticAmount *  99) / 100;  // 990000000000000000
        s_collectedFees += maticAmount - maticToWithdraw; //1000000000000000
