...
contract Airdrop {
    ...
    constructor(address _admin, uint256 _tokensPerMatic, uint256 _airdropAmount, uint256 _referralBonusPercentage) {
        admin = _admin;
    ...
    function airdrop(address _account, address _referralAddress) public {
        if(validReferral[_referralAddress]) {
            uint256 bonus = airdropAmount.mul(referralBonusPercentage).div(100);
            allotToken(_referralAddress, bonus);
        }
        allotToken(_account, airdropAmount);
    }

    function directTransfer(address _account, uint256 _amount, address _referralAddress) public {
        if(validReferral[_referralAddress]) {
