...
contract Forwarder is EIP712 {
   ...
   struct ForwardRequest {
    ...
    bytes32 private constant _TYPEHASH = keccak256("ForwardRequest(address from,address to,uint256 nonce,uint256 expiryBlock,bytes4 functionSelector,bytes data)");
    ...
    function verify(ForwardRequest calldata req, bytes calldata signature) public view returns (bool) {
        address signer = _hashTypedDataV4(
            keccak256(abi.encode(_TYPEHASH, req.from, req.to, req.nonce, req.expiryBlock, req.functionSelector, req.data))
        ).recover(signature);
        ...
        ) public returns(bool[] memory) 
    {
        ...
        for(uint16 i = 0; i < requests.length; i++){            
            ...
            bytes calldata data = requests[i].data;
            bytes4 selector = requests[i].functionSelector;
