...
contract StaticMarket {
    ...
    function validateERC20DataFromCalls(
        ...
        bytes calldata addrs = calls[:96];
        bytes calldata _calls = calls[96:];
        for (uint256 i = 0; i < 3; ++i) {
            require(
                asset == abi.decode(addrs[(i * 32):32 * (i + 1)], (address)),
                ""
            );
            checkERC20Side(
                _calls[i * 100:100 * (i + 1)],
                taker,
                i == 0 ? maker : tokenGiveGet[i + 1],
                tokenIdAndNumeratorDenominator[i + 3]
            );
            sum += abi.decode(_calls[(i * 100) + 68:100 * (i + 1)], (uint256));
        }
    }
...
contract UnseenStatic is StaticMarket {
    ...
    constructor(address _atomicizer) {
        require(
            _atomicizer != address(0),
            "Unseen: Atomicizer address cannot be 0"
        );
