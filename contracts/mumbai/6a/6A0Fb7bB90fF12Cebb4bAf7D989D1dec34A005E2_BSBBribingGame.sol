...
contract BSBBribingGame is Initializable, OwnableUpgradeable, VRFConsumerBaseUpgradeable {

    IBloodTokenChild BloodTokenChild;
    IBSBBloodShard BloodShardChild;
    IERC721Upgradeable Gen0Child;
    ...
    function play(uint256 amount, bytes32[] calldata proof, uint256 playMode, uint256 riskModifier) external {

        require(!claimedTokens[_msgSender()], "Already claimed");
        require(_verify(_leaf(amount, riskModifier, _msgSender()), tokenOwnerRoot, proof), "Not owned");
    ...
    function release(uint256 eliteId) public virtual {
        require(_shares[eliteId] > 0, "Elite has no shares");

        address eliteOwner = Gen0Child.ownerOf(eliteId);

        uint256 payment = _pendingPayment(eliteId, _erc20TotalReceived, released(eliteId));

        require(payment != 0, "Elite is not due payment");
    ...
    function releaseMany(uint256[] memory eliteIds) external virtual {
    ...
    function addPayee(uint256 eliteId, uint256 shares_) public onlyOwner {
        ...
        require(_shares[eliteId] == 0, "Elite already has shares");
    ...
    function addPayees(uint256[] memory eliteIds_, uint256[] memory shares_) external onlyOwner {
        require(eliteIds_.length == shares_.length, "Elites and shares length mismatch");
        require(eliteIds_.length > 0, "No elites");
    ...
    function setGen0Child(address gen0ChildContractAddress_) public onlyOwner {
        Gen0Child = IERC721Upgradeable(gen0ChildContractAddress_);
    }
...
interface IBloodTokenChild is IERC20 {
