...
contract MutualAccess is ERC1155 {
    address payable private governance; // Address of contract admin
    uint256 private multeezFeesPercentage; // Global fee that Multeez collects
    uint256 public contentCount; // Number of created contents
    uint16 constant percentageDivider = 10000;
    ...
    // Describe a content
    struct Content {
        uint256 primaryTicketPrice;
        string contentUrl;
        uint startTime;
        uint endTime;
        bool primarySaleEnded;
        address contentOwnerAddress;
        uint256 secondarySaleFee; // 10000 -> 100.00%
        FeeRecipient[] feeRecipients;
    }

    // Maps content id to secondary ticket price
    mapping(uint256 => mapping(address => uint256)) private secondaryTicketPrices;
    // Maps content id to content
    mapping(uint256 => Content) internal content;
    ...
    constructor(address payable governance_, uint256 multeezFeesPercentage_) ERC1155("") {
        governance = governance_;
        contentCount = 0;
        multeezFeesPercentage = multeezFeesPercentage_;
    }
    ...
    function addNewContent(
        ...
        content[newContentId].primaryTicketPrice = primaryTicketPrice;
        ...
        if (expireAfterHours > 0){
            content[newContentId].endTime =  timeNow + (expireAfterHours * 1 hours);
        } else{
            //content never expires
            content[newContentId].endTime = MAX_INT;
        }
        
        for(uint256 i = 0; i < feeRecipients.length; i++){
            content[newContentId].feeRecipients.push(feeRecipients[i]);
        }
        ...
        content[newContentId].secondarySaleFee = secondarySaleFee;

        // Mint all the tickets to the creator
        _mint(content[newContentId].contentOwnerAddress, newContentId, initialSupply, "");
    }
    ...
    function setContentOwner(
        ...
        delete content[contentId].feeRecipients;
        for(uint256 i = 0; i < feeRecipients.length; i++){
            content[contentId].feeRecipients.push(feeRecipients[i]);
        }
    }

    // Returns the address of the current content owner
    function getContentOwnerAddress(
        uint256 contentId
    ) public view returns(address contentOwnerAddress) {
        contentOwnerAddress = content[contentId].contentOwnerAddress;
    }

    function setContentFeeRecipients(
        uint256 contentId, 
        FeeRecipient[] calldata feeRecipients
    ) external onlyContentOwner(contentId) {
        if (isFeeRecipientsValid(feeRecipients) == false)
    ...
    function getContentFeeRecipients(
        uint256 contentId
    ) public view returns(FeeRecipient[] memory feeRecipients) {
        feeRecipients = content[contentId].feeRecipients;
    }

    function getContentTimestamp(
        uint256 contentId
    ...
    ) public view returns(bool accessAllowed){
    ...
    // Sets the fee percentage that Multeez collects
    // Note: Can only be called by governance
    function setMulteezFees(uint256 feesPercentage) external onlyGovernance {
        multeezFeesPercentage = feesPercentage;
    }

    // Returns the fee percentage that Multeez collects
    // Note: Can only be called by governance
    function getMulteezFees() public onlyGovernance view returns(uint256 feesPercentage) {
        feesPercentage = multeezFeesPercentage;
    }
    ...
    function getSecondaryTicketPrice(
        uint256 contentId, 
        address sellerAddress
    ) public view returns(bool isForSale, uint256 ticketPrice){
        ticketPrice = secondaryTicketPrices[contentId][sellerAddress];
        isForSale = ticketPrice > 0;
    }

    function buyAccess(uint256 contentId, address payable ticketSeller) external payable{
        // If this is a secondary sale, 
        // check that primary sale is complete and use the appropriate price
        uint256 ticketPrice = content[contentId].primaryTicketPrice;
        uint256 secondarySaleFee = content[contentId].secondarySaleFee;  // 5678 ==> 0.5678%

        // Check if the seller tries to sell his ticket before
        // the primary sale has ended
        bool isSellerTriesToSellBeforePrimaryEnded = 
        ...
        if (content[contentId].primarySaleEnded == true){
            (bool isForSale, uint256 secondaryTicketPrice) = 
        ...
        uint256 multeezFees = calculateTicketFees(ticketPrice, multeezFeesPercentage);

        //For a secondary sale, collect fee
        if(content[contentId].primarySaleEnded == true && content[contentId].secondarySaleFee > 0){
            uint256 ticketFee = calculateTicketFees(ticketPrice, secondarySaleFee);
            ...
            for(uint256 i = 0; i < content[contentId].feeRecipients.length; i++){
                uint256 recipientFee = calculateTicketFees(
                    ticketFee, 
                    content[contentId].feeRecipients[i].feePercentage
                );
                payable(content[contentId].feeRecipients[i].recipient).transfer(recipientFee);
        ...
        // Collect Multeez fees
        governance.transfer(multeezFees);
        ticketPrice -= multeezFees;

        // Send ticket price to the seller. 
        ticketSeller.transfer(ticketPrice);

        // If the last primary sale ticket is sold, end primary sale
        content[contentId].primarySaleEnded = 
            ticketSeller == content[contentId].contentOwnerAddress &&
    ...
    function createMoreTickets(
        ...
        _mint(msg.sender, contentId, additionalSupply , "");
    }

    function isPrimarySaleComplete(uint256 contentId) public view returns(bool) {
    ...
    function calculateTicketFees(
        ...
        fee = (sellPrice * ticketFee) / percentageDivider;
    }

    // Returns true if all the fee percentages add to 100%, false otherwise
    function isFeeRecipientsValid(
        FeeRecipient[] calldata feeRecipients
        ...
        isValid = (feeSum == percentageDivider);
    }
}
