...
contract Trader is IHashflowQuote, Ownable, Sweepable, ReentrancyGuard {
	...
	function tradeXChain(
		...
		if (quote.baseToken != address(0)) {
			...
			(bool success,) = address(router).call{value: msg.value}(abi.encodeWithSignature("tradeXChain((uint16,uint16,address,bytes32,address,bytes32,address,address,address,uint256,uint256,uint256,uint256,bytes32,bytes),uint8)", quote));
			require(success, "ERC20 base token x-chain trade failed");
		} else {
			// this is a native token transfer
			// NOTE: for native base token trades, msg.value == xChainFeeEstimate + quote.baseTokenAmount + fee
			(bool success,) = address(router).call{value: msg.value - fee}(abi.encodeWithSignature("tradeXChain((uint16,uint16,address,bytes32,address,bytes32,address,address,address,uint256,uint256,uint256,uint256,bytes32,bytes),uint8)", quote));
			require(success, "ERC20 base token x-chain trade failed");
		}
