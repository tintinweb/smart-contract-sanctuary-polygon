...
import './NgageN721.sol';
import './Administrator.sol';
import './MintNFT.sol';

contract NgageN721Factory is AccessControl, ERC2771Context, ReentrancyGuardUpgradeable {
  bytes32 public constant CREATOR = keccak256('CREATOR');
  bytes32 public constant CONTRACT_CREATOR = keccak256('CONTRACT_CREATOR');
  bytes32 public constant ADMINISTRATOR_ROLE = keccak256('ADMINISTRATOR_ROLE');
  address public ngageN721;
  address public minimalForwarder;
  address public customForwarder;
  ...
  struct Factory {
    ...
    address _mintNFT;
    address _customForwarder;
    address _assetCreator;
  }

  mapping(string => bool) public isFactoryCreated;
  ...
  event FactoryCreated(
    ...
    address mintNFT,
    address customForwarder,
    address assetCreator
  );

  /**
   * @dev Constructor for NgageNFactory contract
   */
  constructor(address _minimalForwarder, address _customForwarder)
    ERC2771Context(_minimalForwarder)
  {
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setupRole(CONTRACT_CREATOR, _msgSender());
    ngageN721 = address(new NgageN721());
    minimalForwarder = _minimalForwarder;
    customForwarder = _customForwarder;
  }

  /**
     @dev modifier to check if the sender is the default admin of NgageNFactory contract
     * Revert if the sender is not the contract admin
     */
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'NNF: Only Admin');
    _;
  }
  ...
  /**
   * @dev Public function to create a new NgageN721 (proxy) contract
   * Reverts if the caller is not forwarder contract
   * Reverts if the caller does not CONTRACT_CREATOR role
   * Reverts if the factory name has already been used
   * Reverts if the caller is same as @param _owner, since it cannot be proxy admin
   * @param _storeId The unique id of store
   * @param _collectionId The unique id of collection
   * @param _name The name of the ERC721 contract
   * @param _symbol The symbol of the ERC721 contract
   * @param _isSBT To decide SBT contract or not
   * @param _uri The base URI to be passed as constructor parameter
   * @param _contractUri The contract metadata URI to be passed as constructor parameter
   * @param _owner The owner address which will have Default Admin rights to the factory
   * @param _assetCreator The address of the asset creator which has minitng rights
   * @param _royaltyRecipient The address which will receive royalty on nft sell
   * @param _royaltyPercentageShare The shares of royalty recipient. 0 to 10,000 shares
   */
  function createFactory(
    ...
    uint256 _royaltyPercentageShare
  ) public onlyTrustedForwarder nonReentrant {
    require(hasRole(CONTRACT_CREATOR, _msgSender()), 'NF:Not contract creator');
    require(!isFactoryCreated[_collectionId], 'NF:Factory exists');
    require(_owner != _msgSender(), 'NF:Contract owner cant be proxy admin');
    TransparentUpgradeableProxy _proxy = new TransparentUpgradeableProxy(
      ngageN721,
      _msgSender(),
      abi.encodeCall(
        NgageN721(address(0)).initialize,
        (_name, _symbol, _uri, _contractUri, _royaltyRecipient, _royaltyPercentageShare, _isSBT)
      )
    );
    Administrator _admn = new Administrator(
      INgageN721(address(_proxy)),
      minimalForwarder
    );
    MintNFT _mintNFT = new MintNFT(INgageN721(address(_proxy)), customForwarder);
    NgageN721(address(_proxy)).grantRole(DEFAULT_ADMIN_ROLE, _owner);
    NgageN721(address(_proxy)).grantRole(ADMINISTRATOR_ROLE, address(_admn));
    NgageN721(address(_proxy)).grantRole(ADMINISTRATOR_ROLE, address(_mintNFT));
    _admn.grantRole(DEFAULT_ADMIN_ROLE, _owner);
    _mintNFT.grantRole(DEFAULT_ADMIN_ROLE, _owner);
    _mintNFT.grantRole(CREATOR, _assetCreator);
    factories[_collectionId] = Factory(
      _storeId,
      _collectionId,
      _owner,
      address(_proxy),
    ...
    isFactoryCreated[_collectionId] = true;
    emit FactoryCreated(
      _storeId,
      _collectionId,
      _owner,
  ...
  /**
   * @dev Public function to set new ngageN721 contract address
   * Reverts if the caller is not an admin
   * @param _ngageN721 The address that will be set as new ngageN721 contract
   */
  function setNewNgageN(address _ngageN721) public onlyAdmin {
    require(_ngageN721 != address(0), 'NF: Invalid address');
    ngageN721 = _ngageN721;
  }

  /**
   * @dev Public function to set new custom forwarder contract address
   * Reverts if the caller is not an admin
   * @param _forwarder The address that will be set as new custom forwarder contract
   */
  function setNewCustomForwarder(address _forwarder) public onlyAdmin {
    require(_forwarder != address(0), 'NF: Invalid address');
    customForwarder = _forwarder;
  }
}
...
interface IERC2981Royalties {
  function royaltyInfo(uint256 _value)
    external
    view
  ...
  ReentrancyGuardUpgradeable
{
  ...
  function initialize(
    ...
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setRoyalties(_royaltyRecipient, _royaltyPercentage);
  }
  ...
  function _setRoyalties(address recipient, uint256 value) internal {
    ...
    require(
      recipient != address(0),
      'Royalty receiving address cannot be null'
    );
  ...
  // set new royalty recipient address
  function setReceiver(address _address) external onlyAdmin {
    require(_address != address(0), 'Royalty receiving address cannot be null');
    _royalties.recipient = _address;
  }

  // Set new royalty percentage in basis points <=10000
  function setRoyaltyPercentage(uint256 _royaltyPercentage) external onlyAdmin {
    require(_royaltyPercentage <= 10000, 'ERC2981Royalties: Too high');
    _royalties.amount = uint24(_royaltyPercentage);
  }
  ...
  /**
     ...
     */
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'NN: Only Admin');
    _;
  }

  /**
     @dev modifier to check if the sender is one of the whitelisted account aka administrator
     * Revert if the sender is not an administrator
     */
  modifier onlyAdministrator() {
    require(
      hasRole(ADMINISTRATOR_ROLE, _msgSender()),
      'NN: Only Administrator'
    );
    _;
  }
   ...
   * Reverts if the caller is not an administrator
  ...
  function setTokenURI(uint256 id, string memory _tokenUri)
    ...
    onlyAdministrator
  {
    require(bytes(_tokenUri).length != 0, 'NN: Invalid Token URI');
  ...
  function uri(uint256 id) public view returns (string memory) {
    _requireMinted(id);
    return string(abi.encodePacked(_baseURI(), tokenUris[id]));
  }
  ...
  function setContractURI(string memory _contractUri) public onlyAdmin {
    require(bytes(_contractUri).length != 0, 'NN: Invalid Contract URI');
    contractUri = _contractUri;
  }
   ...
   */
  function burnNFT(address from, uint256 id) external nonReentrant {
    require(
      _msgSender() == from || hasRole(ADMINISTRATOR_ROLE, _msgSender()),
      'NN: Only administrator or token holder can burn tokens'
    );
    require(ownerOf(id) == from, 'NN: Not owner');
    _burn(id);
  }
  ...
  //   for (uint256 i = 0; i < len; ++i) {
  //     uint256 _tokenId = ids[i];
  //     if (balanceOf(from, _tokenId) == 0) {
  //       tokenByOwner[from].remove(_tokenId);
  //     }
  //   }
  // }
  ...
  function safeTransferFrom(
    ...
    require(!isSBT, 'NN: SBT tokens can not be transferred');
    require(
      from == _msgSender() ||
   ...
   */
  function isApprovedForToken(address operator, uint256 id)
    public
    view
    returns (bool)
  {
    return getApproved(id) == operator;
  }
  ...
  function setApprovalForAll(
    ...
    require(
      _msgSender() == account || hasRole(ADMINISTRATOR_ROLE, _msgSender()),
      'NN: Only administrator or token holder'
    );
  ...
  //   for (uint256 i = 0; i < len; ++i) {
  //     require(
  //       balanceOf(account, ids[i]) != 0,
  //       'NN: approver must own the token'
  //     );
   ...
   * 4 mappings - 4 slots, 1 string - 1 slot, 1 bytes32 - 1 slot, 1 struct - 2 slots
   * total remaining space = 50 - 8 = 42
   */
...
import '@openzeppelin/contracts/access/AccessControl.sol';
import './INgageN721.sol';
import '@openzeppelin/contracts/metatx/ERC2771Context.sol';

contract MintNFT is AccessControl, ERC2771Context {
  bytes32 public constant CREATOR = keccak256('CREATOR');
  INgageN721 public ngagenAddress;

  /**
   * @dev Constructor for MintNFT contract
   * @param _ngagenAddress address of the ngageN contract
   * @param _forwarder MinForwarder instance address for ERC2771Context constructor
   */
  constructor(INgageN721 _ngagenAddress, address _forwarder)
  ...
  /**
     @dev modifier to check if the sender is the default admin of MintNFT contract
     * Revert if the sender is not the admin
     */
  modifier onlyAdmin() {
    require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), 'MintNFT: Only Admin');
    _;
  }
  ...
  /**
     ...
     */
  modifier onlyTrustedForwarder() {
    require(isTrustedForwarder(msg.sender), 'MintNFT: Only Trusted Forwarder');
    _;
  }
  ...
  function setNgageNAddress(address _ngagenAddress) public onlyAdmin {
    require(_ngagenAddress != address(0), 'MintNFT: Invalid address');
    ngagenAddress = INgageN721(_ngagenAddress);
  }

  /**
   * @dev Public function to mint a single new token and approve MintNFT contract
   ...
   * @param operator The address that will have approval to transfer the tokens
   */
  function mintAndApproveNFT(
    ...
    require(hasRole(CREATOR, to), 'MintNFT: Only CREATOR can mint asset');
    require(
      to == _msgSender(),
      'MintNFT: Only msg.sender with valid signature'
    );
    ngagenAddress.mintNFT(to, id, tokenUri);
    if (!ngagenAddress.isApprovedForToken(operator, id)) {
      ngagenAddress.setApprovalForToken(to, operator, id);
    }
  }

  // /**
  //  * @dev Public function to mint new tokens in batches and approve them to the MintNFT contract
  //  * Reverts if the caller is not trusted forwarder contract
  //  * Reverts if the length of ids and amounts is not equal
  //  * @param to The address to which the newly minted tokens will be assigned.
  //  * Requirements -
  ...
  //     'MintNFT: Only msg.sender with valid signature'
  //   );
  //   ngagenAddress.mintNFTBatch(to, ids, amounts, data, tokenUris);
  //   if (!ngagenAddress.isApprovedForTokens(to, operator, ids)) {
  //     ngagenAddress.setApprovalForTokens(to, operator, ids, true);
  //   }
  // }
}
...
interface INgageN721 is IAccessControlUpgradeable, IERC721EnumerableUpgradeable {
  function ADMINISTRATOR_ROLE() external pure returns (bytes32);
...
contract Administrator is AccessControl, ERC2771Context {
   ...
   * @param id The token ids of tokens to be burned
   */
  function burnNFT(address from, uint256 id) public onlyTrustedForwarder {
    require(
      from == _msgSender(),
      'ADMN: Only token owner with valid signature can burn his tokens'
    );
    require(ownerOf(id) == from, 'ADMN: Not owner of token');
    ngagenAddress.burnNFT(from, id);
  }
  ...
  function approveAndTransferNFT(
    ...
    if (!ngagenAddress.isApprovedForToken(address(this), id)) {
      ngagenAddress.setApprovalForToken(from, address(this), id);
    }
    ngagenAddress.safeTransferFrom(from, to, id);
  }
  ...
  function ownerOf(uint256 id) public view returns (address) {
    return ngagenAddress.ownerOf(id);
  }
  ...
  function isApprovedForToken(address operator, uint256 id)
    ...
    returns (bool)
  {
    return ngagenAddress.isApprovedForToken(operator, id);
  }
  ...
  function setApprovalForToken(
    ...
    ngagenAddress.setApprovalForToken(account, operator, id);
  }
