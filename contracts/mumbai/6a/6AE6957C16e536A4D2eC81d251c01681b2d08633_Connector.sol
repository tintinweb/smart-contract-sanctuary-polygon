...
contract NFT is ERC1155 {
    ...
    constructor(address account, uint256 _amount, string memory data, address _market) payable ERC1155("") {
        _data = data;
        creator = account;
        amount = _amount;
        market = _market;
    ...
    function mint(address account) private {
        for(uint i=0; i < amount; i++){
            _mint(account, i + 1, 1, "");
            // address[] memory newHistory;
            // newHistory[0] = account;
            Link memory newLink;
            newLink.id = i + 1;
            newLink.owner = account;
            // newLink.history = newHistory;
    ...
    function getHistory(uint256 id) public view returns (address[] memory history) {
        return (HyperLink[id].history);
    }

    function publishToken(uint256 price, uint256 id) public {
        require(msg.sender == market, "Only market can access");
    ...
    function removeTokenFromSale(uint256 id) public {
        require(msg.sender == market, "Only market can access");
        HyperLink[id].onSale = false;
        HyperLink[id].cost = 0;
    }
    function tokenOnSale(uint256 id) public view returns (bool) {
        return HyperLink[id].onSale;
    }
    
    function sendToken(uint256 id, address NFTReceiver) public {
        require(msg.sender == market, "Only market can access");
        HyperLink[id].owner = NFTReceiver; // передача nft
        HyperLink[id].history.push(NFTReceiver); // изменение истории
    }
    // function addChild(uint id, address sender, uint newNFTId) public {
    //     require(msg.sender == marketAddr);
    //     // some code
    // }
    // function addParents(address sender, uint256[] memory NFTids, uint256[] memory tokensIds) public {
    //     require(msg.sender == marketAddr);
    //     // some code
    // }
}
