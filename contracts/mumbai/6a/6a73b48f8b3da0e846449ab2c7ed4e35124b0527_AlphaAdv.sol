...
contract AlphaAdv is ERC721A, Ownable, ReentrancyGuard {
    enum Status {
        Pending,
        PreSale,
        PublicSale,
    ...
    uint256 public discountAmount = 0.005 ether;
    ...
    function presaleMint(uint256 amount, bytes32 _hashedMessage, uint8 _v, bytes32 _r, bytes32 _s) external nonReentrant payable {
        ...
        require(amount <= maxPerMint, "Max per mint amount per transaction exceeded.");
        require(totalSupply() + amount + reserveAmount - claimedReserved <= collectionSize, "Max supply exceeded.");
        uint256 totalCost = getPrice(0.015 ether, amount);
    ...
    function getPrice(uint256 price, uint256 _quantity) internal view returns (uint256){
        require(_quantity > 0, "Must be greater than zero");

        return (price * _quantity) - ((_quantity - 1) * discountAmount);
    }
