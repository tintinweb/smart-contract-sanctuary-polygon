...
contract CryptoLottery is ERC20 {
    ...
    constructor(
        ...
        uint256 fee
    ) ERC20("Crypto Lottery", "CL") payable {
        ...
        returns (Ticket[] memory tickets)
    {
        ...
        for (uint256 i = 0; i < _tickets_ref[user].length; i++) {
            ticket_array[i] = _tickets[_tickets_ref[user][i].round][
                _tickets_ref[user][i].number
            ];
        }
    ...
    function getLastTickets(uint256 count)
        ...
        returns (Ticket[] memory tickets)
    {
        Ticket[] memory ticket_array = new Ticket[](
            _tickets[_rounds.length - 1].length
        );
    ...
    function getLastRoundsPagination(uint256 cursor, uint256 howMany)
        external
        view
        returns (Round[] memory rounds, uint256 newCursor, uint256 total)
    {
        ...
        Round[] memory _rounds_array = new Round[](length);
        

        for (uint256 i = 0; i < length; i++) {
            _rounds_array[i] = _rounds[cursor + i];
        }

        return (_rounds_array, cursor + length, _total);
    }

    function getLastTicketsPagination(uint256 cursor, uint256 howMany)
        external
        view
    ...
    {   
        ...
        Ticket[] memory ticket_array = new Ticket[](length);

        for (
            uint256 i = 0; i < length;
            i++
        ) {
            ticket_array[i] = _tickets[_rounds.length - 1][cursor + i];
        }

        return (ticket_array, cursor + length, _total);
    }

    function getUserTicketsPagination(address user, uint256 cursor, uint256 howMany)
        external
        view
        returns (Ticket[] memory tickets, uint256 newCursor, uint256 total)
    {    
        ...
        for (uint256 i = 0; i < length; i++) {
            ticket_array[i] = _tickets[_tickets_ref[user][cursor + i].round][
                _tickets_ref[user][cursor + i].number
            ];
        }
