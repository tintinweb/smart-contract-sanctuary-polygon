...
contract UserRequestUint256 is UserRequest, ConfirmedOwner {
    ...
    struct Response {
        uint256 responseData;
        string errorMessage;
        uint256 expireTime;
    }
    mapping( address => bytes32) public userRequests;
    mapping( bytes32 => Response) public userResponses;
    ...
    function saveQueryResponse(bytes32 requestId, uint256 data, string calldata errorMessage) external {
        ...
        // Store response
        Response storage newResponse = userResponses[requestId];
        newResponse.errorMessage = errorMessage;
        newResponse.responseData = data;
        newResponse.expireTime = block.timestamp + 100;
    }

    function flushExpiredResponses() external {
        for(uint256 index; index < requestIds.length; index++){
            Response memory newResponse = userResponses[requestIds[index]];
            if(newResponse.expireTime > 0 && newResponse.expireTime < block.timestamp){
                delete userResponses[requestIds[index]];
