...
contract EnergyLevel is Multicall, HSystemChecker {
    ...
    bytes32 public constant CAT_TOKEN = keccak256('CAT');
    ...
    function setItemEnergyLevel(uint256 itemTokenId, uint16[] calldata energyLevels)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            energyLevels.length == CURRENT_MAX_ENERGY_COUNT,
            'EL 100 - Exceeded maximum allowable energy count'
        );
        // @dev Store the packed energy levels into the mapping
        _itemEnergy[itemTokenId] = _pack(energyLevels);
    ...
    function setTokenMaxEnergy(bytes32 tokenType, uint16[] calldata maxEnergy)
        external
        onlyRole(ADMIN_ROLE)
    {
        require(
            maxEnergy.length == CURRENT_MAX_ENERGY_COUNT,
            'EL 100 - Exceeded maximum allowable energy count'
        );

        _maxEnergy[tokenType] = _pack(maxEnergy);
    ...
    /// @notice Set the current max energy count
    /// @param newMaxEnergyCount - New max energy count to set
    function setTokenMaxEnergyCount(uint256 newMaxEnergyCount) external onlyRole(ADMIN_ROLE) {
        // current system only suppoers maximum of 4 paramameters
        require(newMaxEnergyCount < 5, 'EL 100 - Exceeded maximum allowable energy count');
    ...
    {
        return _unpack(_itemEnergy[itemTokenId]);
    }
    ...
    {
        return _unpack(_maxEnergy[tokenType]);
    }
    ...
    function _pack(uint16[] calldata energyLevelsArray)
        internal
        pure
        returns (uint64 packedEnergyLevels)
    {
    ...
    /// @return energyLevel3 - Energy Level 3 in uint16
    /// @return energyLevel4 - Energy Level 4 in uint16
    function _unpack(uint64 packedEnergyLevel)
        internal
        pure
        returns (
            uint16 energyLevel1,
