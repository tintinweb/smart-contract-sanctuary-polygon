    ...
    AccessControlUpgradeable
{
    ...
    function initialize(string memory _initialBaseURI) external initializer {
		__ERC721_init('CryptoPeople', 'CP');
    ...
    function setAdmin(address newAdminAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _grantRole(DEFAULT_ADMIN_ROLE, newAdminAddress);
    }

    function setMinter(address newMinterAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function setRoyaltyCollector(address royaltyCollector) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _royaltiesCollector = royaltyCollector;
    }
    ...
    function getNFTListedPrice(uint256 tokenId) public view returns (uint256) {
        require(_tokenIdCounter.current() <= tokenId, "Token ID is out of range");

        return _listings[tokenId].price;
    }

    function buyNFT(uint256 tokenId) public payable whenNotPaused {
        require(_listings[tokenId].isForSale, "NFT is not for sale");
        uint256 price = _listings[tokenId].price;
        require(msg.value >= price, "Insufficient payment");

        // Calculate the royalty
        uint256 royaltyAmount = (price * _royaltyFee) / 10000;

        // Transfer the payment
        payable(_listings[tokenId].seller).transfer(price - royaltyAmount);
        payable(address(this)).transfer(royaltyAmount);

        // Transfer the NFT
        _transfer(_listings[tokenId].seller, _msgSender(), tokenId);

        // Update the listing
        _listings[tokenId].isForSale = false;
        _listings[tokenId].seller = _msgSender();

        emit NFTSold(tokenId, _msgSender(), price);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC721Upgradeable, ERC721URIStorageUpgradeable, AccessControlUpgradeable, ERC721RoyaltyUpgradeable)
        returns (bool)
    {
        return super.supportsInterface(interfaceId);
    }
