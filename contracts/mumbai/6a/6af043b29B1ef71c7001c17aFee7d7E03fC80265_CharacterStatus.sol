...
contract CharacterStatus is ICharacterStatus, GameBase {
    constructor(address game)
    GameBase(game) {
    }

    // TODO: add access control modifier
    function setStatus(uint256 worldId, uint256 tokenId, uint256 statusSlotIndex, int64 value) external virtual override {
        Character character = getCharacter(worldId, tokenId);

        character.setStatus(statusSlotIndex, value);

        emit UpdateStatus(worldId, tokenId, statusSlotIndex, value);
    }

    function getStatuses(uint256 worldId, uint256 tokenId) external virtual override view returns(int64[] memory) {
        Character character = getCharacter(worldId, tokenId);

        return character.getStatuses();
    }

    function getStatusSlots(uint256 worldId, uint256 tokenId) external virtual override view returns(uint256[] memory) {
        Character character = getCharacter(worldId, tokenId);

        return character.characterDefinition().getStatusSlots(character.characterDefinitionId());
    }
}
