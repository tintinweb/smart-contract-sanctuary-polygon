...
contract NFT is ERC721A, ERC721AQueryable,
Ownable, PaymentSplitter {
    ...
    uint private constant MAX_GIFT = 4;
    uint private constant MAX_WHITELIST = 4;
    uint private constant MAX_PUBLIC = 16;
    ...
    uint public saleStartTime = 1683636319;
    ...
    address[] private _team = [0xe453ccF43aF6097850dD3F1E93BC0996Dd3142Dc,0xe71e1369e295964c9828C2aF9d6307b35448a9A1];
    uint[] private _teamShare= [70,30];
    ...
    uint private constant MAX_PER_ADDRESS_DURING_PUBLIC_MINT = 6;

    constructor(bytes32 _merkleRoot, string memory _baseURI)
    ERC721A("0xNFTEH","NFT0x")PaymentSplitter(_team, _teamShare){
    ...
    /**
    * @notice Modifier that runs the code if only the contract is not paused
    **/
    ...
    * @param _quantity Amount the user wants to mint
    * @param _proof The merkle proof
    **/
    function whitelistMint(address _account, uint _quantity, 
    bytes32[] calldata _proof) external payable whenNotPaused {
        require(getStep() == Step.WhitelistSale, "The whitelist Sale is not activated");
        require(isWhitelisted(_account, _proof), "Not whitelisted");
        require(amountNFTperWalletWhitelistSale[msg.sender] +
        _quantity <= MAX_PER_ADDRESS_DURING_WHITELIST_MINT, 
        "You can only mint 2 NFT during the Whitelist Sale");
        require(totalSupply() + _quantity <= MAX_WHITELIST, 
        "Max supply exceeded");
        require(msg.value >= PRICE_WHITELIST_MINT * _quantity, "Not enough funds");
    ...
    * @param _account Account which will have the NFTs
    * @param _quantity  Amount the user wants to mint
    **/
    function publicMint(address _account, uint _quantity) external payable whenNotPaused {
        require(getStep() == Step.PublicSale, "Public sale is not activated");
        require(amountNFTperWalletPublicSale[msg.sender] + 
        _quantity <= MAX_PER_ADDRESS_DURING_PUBLIC_MINT,
        "You can only mint 6 NFT during the public sale");
        require(totalSupply() + _quantity <= MAX_SUPPLY_MINUS_GIFT, 
        "Max supply exceeded");
    ...
    **/
    function gift(address _to, uint _quantity) external onlyOwner whenNotPaused {
        require(getStep() > Step.PublicSale,'Gift only after public sale');
        require(totalSupply() + _quantity <= MAX_SUPPLY, "Max supply exceeded");
        _safeMint(_to , _quantity);
    }

    /** 
    * @notice Allow to pause/unpaused the smart contract.
    * @param _isPaused true or false if we want to pause or unpaused the contract.
    **/
    ...
    /**
    * @notice Allows to change the starting time of the sale.
    ...
    **/
    ...
    /**
    * @notice Allow to change the base URI of the NFTs.
    ...
    **/
    ...
    /**
    * @notice Allows to change the merkle root.
    ...
    **/
    ...
    internal view returns(bool) {
        return MerkleProof.verify(_proof, merkleRoot, 
    ...
    **/
    ...
    (ERC721A, IERC721A) returns (string memory){
    ...
    **/
    ...
    //Not allow to receive ethers outside the minting function
