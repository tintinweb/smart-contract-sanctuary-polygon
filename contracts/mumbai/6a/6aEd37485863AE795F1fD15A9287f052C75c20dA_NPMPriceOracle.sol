...
contract NPMPriceOracle is IPriceOracle {
  ...
  function _consult(address token, uint256 amountIn) private view returns (uint256) {
    if (token == token0) {
  ...
  /**
   * @dev Gets the price of LP tokens in DAI
   * using the oracle and fair reserve as given in
   * https://blog.alphaventuredao.io/fair-lp-token-pricing
   *
   * @param r0 Provide the existing reserve0 value
   * @param r1 Provide the existing reserve1 value
   * @param p0 Provide the average price of NPM tokens
  ...
  function consultLP(uint256 amountIn) external view returns (uint256) {
    ...
    uint256 p0 = _consult(token0, 1 ether);
