...
contract NFT is ERC721Enumerable, Ownable {
  ...
  function mint(uint256 tokenQuantity) public payable {
    ...
    require(tokenQuantity > 0, "need to mint at least 1 NFT");
    require(tokenQuantity <= maxMint, "max mint amount per session exceeded");
    require(supply + tokenQuantity <= MAX_SUPPLY, "max NFT limit exceeded");

    if (msg.sender != owner()) {
        if(onlyWhitelisted == true) {
            require(isWhitelisted(msg.sender), "user is not whitelisted");
            uint256 ownerMintedCount = addressMintedBalance[msg.sender];
            require(ownerMintedCount + tokenQuantity <= nftPerAddressLimit, "max NFT per address exceeded");
        }
