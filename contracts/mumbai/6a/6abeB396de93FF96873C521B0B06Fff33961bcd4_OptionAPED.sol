...
contract OptionAPED is Ownable {
    ...
    uint256 private optionDuration;
    ...
    function getOptionInfo(uint256 id) public view returns (Option memory) {
        ...
        return Option(
            ...
            fee,
            optionStartTs,
            optionStartTs + depositPeriod,
            optionStartTs + depositPeriod + lockPeriod,
            options[id].startPrice > 0 ? options[id].startPrice : getPrice(optionStartTs + depositPeriod),
            options[id].closePrice > 0 ? options[id].closePrice : getPrice(optionStartTs + depositPeriod + lockPeriod),
            options[id].upBets,
            options[id].downBets,
    ...
    function getBidInfo(address client, uint256 id) public view returns (BidInfo[] memory _bidsInfo) {
        ...
        if (bids[client][id].betAmountDown > 0) {
            _bidsInfo[0] = BidInfo(
                ...
                getOptionStartTs(id) + depositPeriod,
                wonAmountDown,
    ...
    function claim(uint256 id) public {
        uint256 optionTimestamp = getOptionStartTs(id);
        if (!options[id].resolved && block.timestamp >= optionTimestamp + depositPeriod + lockPeriod) {
            _resolve(id, getPrice(optionTimestamp + depositPeriod), getPrice(optionTimestamp + depositPeriod + lockPeriod));
        }
    ...
    function getOptionStage(uint256 id) private view returns (OptionStage) {
        ...
        if (block.timestamp > optionStartTs + depositPeriod + lockPeriod) {
    ...
    function getOptionStartTs(uint256 id) private view returns (uint256) {
        if (id == 0) {
    ...
    function getPotentialEarningsUp(address user, uint256 optionIndex) internal view returns (uint256) {
        ...
        return (options[optionIndex].downBets * bids[user][optionIndex].betAmountUp / options[optionIndex].upBets) * (100 - fee) / 100;
    }
    ...
    function getPotentialEarningsDown(address user, uint256 optionIndex) internal view returns (uint256) {
        ...
        return (options[optionIndex].upBets * bids[user][optionIndex].betAmountDown / options[optionIndex].downBets) * (100 - fee) / 100;
    }
