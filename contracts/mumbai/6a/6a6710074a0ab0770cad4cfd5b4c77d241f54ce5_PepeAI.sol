...
// File: contracts/interfaces/IPepeAIBonusPool.sol


pragma solidity >=0.5.0;

interface IPepeAIBonusPool {
...
// File: contracts/token/PepeAI.sol
...
contract PepeAI is ERC20, Ownable {
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountPepeBurn = (taxAmount * burnFee) / (totalFee);
        uint256 amountPepeLp = (taxAmount * liquidityFee) / (totalFee);
        uint256 amountPepeBonus = (taxAmount * bonusFee) / (totalFee);
        taxAmount -= amountPepeBurn;
        taxAmount -= amountPepeLp;
        taxAmount -= amountPepeBonus;
        ...
        _transfer(address(this), DEAD, amountPepeBurn);
        _approve(address(this), address(bonusWallet), amountPepeBonus);
        IPepeAIBonusPool(bonusWallet).injectRewards(amountPepeBonus);
        ...
        emit SwapBack(amountPepeBurn, amountBackTokenGov1, amountBackTokenGov2, amountPepeLp, amountBackTokenJackpot, amountPepeBonus, amountBackTokenDev, block.timestamp);
    }
    ...
    function clearStuckEthBalance() external onlyOwner {
        ...
        require(success, 'PepeAI: ETH_TRANSFER_FAILED');
    }
    ...
    function addPair(address pair) public onlyOwner returns (bool) {
        require(pair != address(0), "PepeAI: pair is the zero address");
    ...
    function getPair(uint256 index) public view returns (address) {
        require(index <= _pairs.length() - 1, "PepeAI: index out of bounds");
