...
contract WowTCommunityVote is AccessControlUpgradeable, OwnableUpgradeable {
    mapping(string => uint8) public ratingsReceived;

    mapping(address => bool) public ratedAddress;
    ...
    struct CommunityDetails {
        ...
        // mapping(uint256 => uint256) votingsReceived;
    ...
    event movieVoted(string communityName, string catagory, uint256 movieId);
    ...
    function addMovie(
        ...
        newMovie.movieImgUrl = _movieImgUrl;
        newMovie.timestamp = block.timestamp;
        movieCount++;

        emit addMovieEvent(_communityName, "vote", newMovie.id);
    }
    ...
    function voteForMovie(
        ...
        Movie storage movie = communityDetail.movies[_movieId];
        require(
            communityDetail.votedAddress[_msgSender()] == false,
            "You can only vote once."
        );
        movie.totalVotes++;
        communityDetail.votedAddress[_msgSender()] = true;
        emit movieVoted(_communityName, "vote", _movieId);
    }
    ...
    // function getVoteCountforallMovies() public view returns (uint8[] memory) {
    //   uint8[] memory ratings = new uint8[](movieList.length);
    //   for (uint i = 0; i < movieList.length; i++) {
    //     ratings[i] = ratingsReceived[movieList[i]];
    //   }
    //   return ratings;
    // }
}
