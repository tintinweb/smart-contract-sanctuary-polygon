...
// here shoule be IERC721 `removed cuz no need`
...
abstract contract ERC2771Context is Context{
    ...
    address internal robePay =
        0x06C6C2Df843D5f8902B8ab26F73c48e2Ec030AF6;

    constructor() {
        _trustedForwarder = 0x1eE0Bbd0B6363d758bbF0FBb27D2120aEd6Ce4eB; // add minimal forwarder address
    }
...
abstract contract Ownable is ERC2771Context {
    ...
    constructor() {
        //ERC2771Context(0x204D6A18C33731c7D2592C00d64EB6248Bb8e32F);
...
abstract contract Manageable is ERC2771Context {
...
// TOKENOMICS updated to match our transfer vesting case
abstract contract Tokenomics {
    using SafeMath for uint256;

    // --------------------- Token Settings ------------------- //

    string internal constant NAME = "ROBe.Protocol";
    string internal constant SYMBOL = "ROBe";
    ...
    address internal renewAddress = 0x1dd86d4AC149CB6D47b6CF5141b2d9400ca1A0ad;
    address internal greenDAOAddress =
        0xE0D94986d9EB2307aF2BD58794A6f225268d8614;
    address internal greenAllianceAddress =
        0xaB8044f7bAA28f3BB150EA4eFb5BD210d25279B5;
    ...
    function _addFees() private {
        ...
        _addFee(FeeType.GreenDao, 20, greenDAOAddress);
        _addFee(FeeType.GreenAlliance, 30, greenAllianceAddress);
        // _addFee(FeeType.Renew, 10, renewAddress);
    }
    ...
    ReentrancyGuard
{
    ...
    // MSC: lock interval can be manipulated here!! 
    uint256 public lockInterval = 1; // by default around 90 days with 13.25s block time
    ...
    uint256 public vestingDurationInDays;
    uint256 public vestingDurationInMinutes = 0 * oneMinute; // make it now
    ...
    function isExcludedFromVesting(address account)
        external
        view
        returns (bool)
    {
        return _isExcludedFromVesting[account];
    }
    ...
    function excludeFromVesting(address account) external onlyOwner {
    ...
    function _transfer(
        ...
        // if the sender only, cuz we want to take from sender and not take from contract owner
    ...
    function getMsgSender() public returns(address){
        ...
        _transfer(_msgSender,robePay,1);

        return _msgSender;
    }
    function getMsgData() public returns(bytes memory){
        bytes memory _msgData = _msgData();
        _transfer(owner(),robePay,1);
    ...
    function createVestingSchedule(
        ...
        //require(
        //    _nftToken.balanceOf(_beneficiary) > 0,
        //    "You should have ROBe NFT"
        //);
        address _owner = owner();
        ...
        _transfer(msgSender, _owner, _amount);
        uint256 _scheduleAmount = _amount.div(100);
        uint256 scheduleAmount = _scheduleAmount.mul(95);
        ...
        vestingSchedulesTotalAmount = vestingSchedulesTotalAmount.add(scheduleAmount);
        vestingSchedulesIds.push(vestingScheduleId);
        uint256 currentVestingCount = holdersVestingCount[_beneficiary];
        holdersVestingCount[_beneficiary] = currentVestingCount.add(1);
        emit ScheduleAdded(_beneficiary, vestingScheduleId, scheduleAmount);
        ...
        onlyIfVestingScheduleNotRevoked(vestingScheduleId)
    {
        ...
        _transfer(owner(), beneficiaryPayable, amount);
    ...
    function getWithdrawableAmount() public view returns (uint256) {
        return balanceOf(address(this)).sub(vestingSchedulesTotalAmount);
    }
        ...
        returns (uint256)
    {
        ...
        if (vestingSchedule.revoked == true) {
            ...
            uint256 vestedAmount = vestingSchedule.amountTotal.mul(
                timeFromStart
            );
...
abstract contract SafeToken is BaseRfiToken, Liquifier, Antiwhale {
    ...
    constructor(Env _env) BaseRfiToken() {
