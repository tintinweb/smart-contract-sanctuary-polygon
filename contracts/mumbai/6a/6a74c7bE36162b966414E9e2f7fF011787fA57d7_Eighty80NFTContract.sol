...
contract Eighty80NFTContract is ERC1155Upgradeable, Ownable, Pausable {

    mapping (address => bool) public authorisedInteraction;
    mapping (uint => bool) public mintBlocker;
    mapping (uint => address) public ownerOf;
//    address public _stakingContract;

    function init(string memory uri)
    public
     //override(Ownable, Pausable)
    initializer
    {
        __ERC1155_init(uri);
        Pausableinitialize();
    ...
    function mintTokens(address _to, uint id) external {
        ...
        require (!mintBlocker[id],'Token Already Present');
        mintBlocker[id] = true;
        _mint(_to,id,1,"");
    }
    ...
    function burnAuthorisedToken(address _from, uint id) external {
        ...
        require (mintBlocker[id],'Token Not Present');
        mintBlocker[id] = false;
        _burn(_from, id, 1);
    }

    function burnToken(uint id) external {
        require (mintBlocker[id],'Token Not Present');
        mintBlocker[id] = false;
