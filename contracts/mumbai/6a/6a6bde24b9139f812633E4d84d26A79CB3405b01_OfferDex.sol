  ...
  function clearOffers(uint256 tokenId, address collection, uint256 index) private {
      ...
      for(uint256 i = 0; i < temp.count; i++){
        ...
        if(temp.offerIds[i] != index){
              token = ALIA;
           if(temp._offer[temp.offerIds[i]].currencyType == 1){
               price = price / 1000000000000;
               token = BUSD;
           }else if(temp._offer[temp.offerIds[i]].currencyType == 2) {
              token = ETH;
           }
         if(temp._offer[temp.offerIds[i]].currencyType == 3){
           chainTransfer(temp._offer[temp.offerIds[i]]._address, 1000, price);
         } else {
            token.transfer(temp._offer[temp.offerIds[i]]._address, price);
         }
        }
