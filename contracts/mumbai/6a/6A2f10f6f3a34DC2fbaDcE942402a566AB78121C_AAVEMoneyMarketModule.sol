...
contract AAVEMoneyMarketModule is WithStorage {
    ...
    function borrowAndSwapExactIn(uint256 interestRateMode, ExactInputWithLimitParams memory params) external returns (uint256 amountOut) {
        // borrow and send funds to this address for swaps
        IPool(aas().v3Pool).borrow(params.path.getFirstToken(), params.amountIn, interestRateMode, 0, msg.sender);
        // swap exact in with common router
        amountOut = IMinimalSwapRouter(us().swapRouter).exactInputWithLimit(params);
    }

    function borrowAndSwapExactInToETH(uint256 interestRateMode, StandaloneExactInputUniswapParams calldata params)
        external
    ...
    function swapAndRepayExactIn(uint256 interestRateMode, ExactInputMultiParams calldata params) external returns (uint256 amountOut) {
        IERC20(params.path.getFirstToken()).transferFrom(msg.sender, address(this), params.amountIn);
        // swap to self
        amountOut = IMinimalSwapRouter(us().swapRouter).exactInputToSelfWithLimit(params);
        // deposit received amount to aave on behalf of user
        amountOut = IPool(aas().v3Pool).repay(params.path.getLastToken(), amountOut, interestRateMode, msg.sender);
    }

    function swapETHAndRepayExactIn(uint256 interestRateMode, ExactInputMultiParams calldata params)
        external
        payable
        returns (uint256 amountOut)
    {
    ...
    function swapETHAndRepayExactOut(uint256 interestRateMode, ExactOutputMultiParams calldata params)
        external
        payable
        returns (uint256 amountIn)
    {
        ...
        // deposit received amount to the lending protocol on behalf of user
        IPool(aas().v3Pool).repay(params.path.getFirstToken(), params.amountOut, interestRateMode, msg.sender);
