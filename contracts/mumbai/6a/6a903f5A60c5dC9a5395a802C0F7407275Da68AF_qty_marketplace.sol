...
contract qty_marketplace is ERC721, Ownable {
    ...
    constructor() ERC721("QTYMarketplace", "QMP") {
        nextTokenId = 1;
        listingFee = 0.01 ether;
    }

    // NFT의 리스트를 확인
    function listNFT(uint256 tokenId, uint256 price, address collectionContractAddress) external {
        require(_exists(tokenId), "Token does not exist");
        require(!_isApprovedOrOwner(_msgSender(), tokenId), "Caller is not approved");

        // NFT 컬렉션의 컨트랙트 주소와 연동하여 소유자 확인
        require(IERC721(collectionContractAddress).ownerOf(tokenId) == _msgSender(), "Caller is not the owner");
    ...
    function removeFromMarketplace(uint256 tokenId) external {
        ...
        delete listings[tokenId];
        emit NFTListed(tokenId, listing.seller, listing.price);
    }

    // 마켓플레이스에 등록된 NFT의 정보를 가져오는 함수
