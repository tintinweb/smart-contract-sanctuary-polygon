...
contract DarkOperaPegToken is ERC20Burnable, Operator {
    using SafeMath for uint256;
    ...
    uint256 public taxRate;   // enabled for DUAH, to support Ukrainian affected by the war
    address public taxFund;   // Red-Cross Society wallet
    ...
    constructor(string memory name_, string memory symbol_, uint8 decimals_, uint256 _initSupply) public ERC20(name_, symbol_) {
        // "Dark Opera", "DARKO"
        // "Dark SpookyToken", "DBOO"
        // "Dark Ukrainian Hryvnia", "DUAH"
    ...
    function getBurnRate() public view returns (uint256) {
        ...
        if (_tokenPrice >= 1 ether) {
            return 0;
        }
        if (_tokenPrice >= 0.95 ether) {
            return burnRate.div(3);
        }
        if (_tokenPrice >= 0.90 ether) {
            return burnRate.mul(2).div(3);
        }
        return burnRate;
    }

    function setBurnRate(uint256 _burnRate) external onlyOwner {
        require(_burnRate <= 2000, "too high"); // <= 20%
        burnRate = _burnRate;
    }

    function setTaxRate(uint256 _taxRate) external onlyOwner {
        require(_taxRate <= 500, "too high"); // <= 5%
        taxRate = _taxRate;
    }
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        require(sender != address(0), "Darko: transfer to the zero address");
        require(recipient != address(0), "Darko: transfer to the zero address");
