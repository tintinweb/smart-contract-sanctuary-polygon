...
contract CrowdFund {
   ...
   // event CreateCrowdFund(address indexed _benef, uint indexed _targ, string indexed name, Category _cat);
    ...
    constructor(address _USDC, address _ownersAddress, uint _time, address _nft, uint _amountProposed, string memory _topic,   bytes  memory _description, Category _cat) {
    ...
    // function createCrowdFund(address _beneficiary, Category _cat) external onlyOwner {
    ...
    //     emit CreateCrowdFund(_beneficiary, targetAmount, name, _cat);
    // }
...
contract crowdFundFactory {
    ...
    function createCrowdfund( address _USDC, address _ownersAddress, uint256 _salt, uint256 deadline, address _nft, uint _amountProposed, string memory _topic,  bytes  memory _description, CrowdFund.Category _cat) public returns(address crowdFundAddr, address manager){
        CrowdFund crowdfund = (new CrowdFund){salt: bytes32(_salt)}(  _USDC,  _ownersAddress, deadline, _nft, _amountProposed, _topic, _description, _cat);
