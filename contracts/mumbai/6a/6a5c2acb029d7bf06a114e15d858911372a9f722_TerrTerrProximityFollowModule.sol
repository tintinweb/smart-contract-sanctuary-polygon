...
import {Errors} from "../lib/core/contracts/libraries/Errors.sol";
 ...
 */
contract TerrTerrProximityFollowModule is
    IFollowModule,
    FollowValidatorFollowModuleBase
{
    struct TerrTerrMapConfig {
        address mapAddress;
        address followingAddress;
    }

    error FollowNonTrasferable();
    error LocationsNotMatching();

    mapping(uint256 => TerrTerrMapConfig) public mapFollowByProfile;
        ...
        returns (bytes memory)
    {
        ...
        mapFollowByProfile[profileId] = TerrTerrMapConfig(
            mapAddress,
            followingAddress
        );
