...
contract Marketplace is Owner, ReentrancyGuard {
    ...
    constructor(uint256 _artistFeePercentage, uint256 _sellFeePercentage, address _walletReceivingSellfee, address _payTokenContract) {
    ...
    function newSellOrder(uint256 _token_id, uint256 _amount, uint256 _price) external returns (uint256) {
        ...
        emit OrderAdded(newOrderId, _token_id, msg.sender, _price);
        return newOrderId;
    }
    ...
    // quedaste aqui checar y ajustar todos los montos de ventas o compras en eventos y funciones y multiplicar los precios por los montos a la hora de la compra
