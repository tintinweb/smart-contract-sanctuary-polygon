...
contract YeildFarmingKingRival is ReentrancyGuard, AccessControl, Ownable, Pausable {
    ...
    uint256 public totalTokenInMonth; // reward prize in 1 month
     ...
     * @param _totalTokenInMonth: total prize in a month
     */
    function changeRewardPrizeIn1Second(uint256 _totalTokenInMonth) external onlyRole(MANAGERMENT_ROLE) {
        require(isEnable, "pool is closed");
        if (_totalTokenInMonth > 0){
            isEnable = true;
        } else {
            isEnable = false;
        ...
        rewardPrizeIn1Second = _totalTokenInMonth.div(2592000); //2592000 = 86400 * 30 day 
        totalTokenInMonth = _totalTokenInMonth;
        emit ChangeRewardPrizeIn1SecondEvent(msg.sender, totalTokenInMonth);
    }
     ...
     */
    function ludenaReward() external {
        uint256 amountStake = userInfor[msg.sender].amountStake;
        require(amountStake > 0, "user don't have token LP in pool");
        uint256 timeStake = userInfor[msg.sender].timeStake;
        uint256 timeNow;
