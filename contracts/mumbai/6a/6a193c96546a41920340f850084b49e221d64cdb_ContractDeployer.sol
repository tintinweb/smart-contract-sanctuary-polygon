...
contract TritonContract is Ownable {
    uint256 private hasSetOwner;

    /// @dev Initializes the owner of the contract.
    function tt_initializeOwner(address deployer) external {
...
contract ContractDeployer is IContractDeployer, ERC2771ContextUpgradeable, MulticallUpgradeable, AccessControlEnumerableUpgradeable {
    /*///////////////////////////////////////////////////////////////
                            State variables
    //////////////////////////////////////////////////////////////*/

    /// @dev The main triton registry.
    TTRegistry private immutable registry;
    ...
    constructor(
        ...
        registry = TTRegistry(_ttRegistry);
    ...
    function deployInstance(
        ...
        address computedContractAddress = Create2Upgradeable.computeAddress(salt, keccak256(contractBytecode), address(this));
        getContractDeployer[computedContractAddress] = caller;

        // deploy the contract
        deployedAddress = Create2Upgradeable.deploy(_value, salt, contractBytecode);

        // set the owner
        TritonContract(deployedAddress).tt_initializeOwner(caller);

        // register to metadata registry
        metadataRegistry.registerMetadata(deployedAddress, publishMetadataUri);

        // register to TTRegistry
    ...
    function deployInstanceProxy(
        ...
        // compute the address of the clone and save it
        address computedContractAddress = ClonesUpgradeable.predictDeterministicAddress(_implementation, salt, address(this));
        getContractDeployer[computedContractAddress] = caller;

        // deploy the clone
        deployedAddress = ClonesUpgradeable.cloneDeterministic(_implementation, salt);
        ...
        if (_initializeData.length > 0) {
            ...
            AddressUpgradeable.functionCallWithValue(deployedAddress, _initializeData, _value);
        }
