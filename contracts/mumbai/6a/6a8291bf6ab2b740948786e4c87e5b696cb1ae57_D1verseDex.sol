...
contract D1verseDex {
    ...
    bytes32 public HashEIP712Domain = keccak256("EIP712Domain(string name,string version,uint256 chainId,address verifyingContract)");
    bytes32 public HashOrderStruct = keccak256(
    ...
    constructor (string memory _name, string memory _version, address _feeRecipient, address _dexDAO) {
        dexDAO = _dexDAO;
        feeRecipient = _feeRecipient;
        allNftAllowed = false;
        allPaymentAllowed = false;
        allowedPayment[address(0)] = true;
        name = _name;
        version = _version;
    ...
    function setDexDAO(address _dexDAO) public onlyDAO {
        dexDAO = _dexDAO;
    }

    function setOperator(address addr, bool flag) public onlyDAO {
        if(!flag) {
    ...
    function setNameVersion(string memory _name, string memory _version) public onlyOperator {
        name = _name;
        version = _version;
        HashEIP712Name = keccak256(bytes(name));
        HashEIP712Version = keccak256(bytes(version));
    ...
    function setFeeRecipient(address _feeRecipient) public onlyOperator {
        require(_feeRecipient != address(0), "Fee recipient rate is address(0)");
        feeRecipient = _feeRecipient;
    }

    function setMaxRoyaltyRate(uint256 _maxRoyaltyRate) public onlyOperator {
        require(_maxRoyaltyRate <= feeDenominator, "Royalty rate is too high");
        maxRoyaltyRate = _maxRoyaltyRate;
    }

    function setAllNftAllowed(bool _allNftAllowed) public onlyOperator {
        allNftAllowed = _allNftAllowed;
    }

    function setAllPaymentAllowed(bool _allPaymentAllowed) public onlyOperator {
        allPaymentAllowed = _allPaymentAllowed;
    }

    function setNftAllowed(address _contract, bool flag) public onlyOperator {
        if (!flag) {
    ...
    function setPaymentAllowed(address _contract, bool flag) public onlyOperator {
        if (!flag) {
            delete allowedPayment[_contract];
        } else {
            allowedPayment[_contract] = true;
        }
    }

    function setD1verseNFT(address _contract, bool flag) public onlyOperator {
        if (!flag) {
            delete allowedNft[_contract];
    ...
    function exchangeFixedPrice(
        ...
        if(D1verseNFT[order.nft_contract]) {
            if(!ILazyMint(order.nft_contract).exists(order.token_id)) {
                ...
                ILazyMint(order.nft_contract).lazyMint(nft_buyer, order.token_id, nft_seller, order.royalty_rate);
            } else {
    ...
    function checkOrder(
        ...
        require(allPaymentAllowed || allowedPayment[order.payment_token], "Payment token contract is not supported");
    ...
    function isOrderFinalizedOrCanceled(FixedPriceOrder memory order) public view returns(bytes32, bytes memory, bool) {
        ...
        if(
            ...
            canceledOrder[order.maker][order_digest] ||
            canceledOrder[order.taker][order_digest] ||
            order.maker_nonce != userNonce[order.maker]
        ) {
            flag = true;
        }
        return (order_digest, order_bytes, flag);
    }
    ...
    function cancelOrder(bytes32 order_digest) public {
    ...
    // openzeppelin/contracts/utils/cryptography/draft-EIP712Upgradeable.sol
