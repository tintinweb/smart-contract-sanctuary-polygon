...
contract MLStaking is ERC20, Ownable, ReentrancyGuard {
	...
	**/
	modifier updateRewardLock(address account_, uint256 lockMonths_) {
		...
		uint256 rewardPerToken2Multiplier = rewardPerToken2Multiplier();
	...
	**/
	function rewardPerToken2Multiplier() public view returns (uint256) {
		if(_totalSumStaked == 0) {
			return 0;
		}
	...
	function _getRewardLock(
		...
		if(_accountsWithStakeLocked[msg.sender][lockMonths_].unlockDate < block.timestamp) {
			if(_rewardToken2 > 0) {
				...
				_mint(msg.sender, _rewardToken2);
			}

			if(
				_rewardToken1 > 0 &&
				STAKING_TOKEN.balanceOf(address(this)).sub(_totalSumStaked) >= _rewardToken1
			) {
				_accountsWithStakeLocked[msg.sender][lockMonths_].rewardToken1 = 0;
				_rewardTokenSum = _rewardTokenSum.sub(_rewardToken1);
