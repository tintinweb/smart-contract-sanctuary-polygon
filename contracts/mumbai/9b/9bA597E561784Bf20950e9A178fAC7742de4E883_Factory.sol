...
contract BasicMetaTransaction {
    ...
    function executeMetaTransaction(address userAddress, bytes memory functionSignature,
        bytes32 sigR, bytes32 sigS, uint8 sigV) public  returns(bytes memory) {
    ...
    AccessControlUpgradeable
{
    ...
    event ParticipantsBuy(uint256 _amount, uint _serialNo);
    ...
    event AuctionStakedAmountTransferred(
        address _oldLender,
        address _auctionWinner,
        address _borrower,
        uint _loanId,
    ...
    function setNftContract(
        address _nft
    ) external validation(_nft) onlyRole(DEFAULT_ADMIN_ROLE) {
        NFT = ILoanNFT(_nft);
    ...
    function setUsdcContract(
        address _usdc
    ) external validation(_usdc) onlyRole(DEFAULT_ADMIN_ROLE) {
        usdc = IERC20MetadataUpgradeable(_usdc);
    ...
    function setDefiWallet(
        address _defiWallet
    ) external validation(_defiWallet) onlyRole(DEFAULT_ADMIN_ROLE) {
        defiWallet = _defiWallet;
    ...
    function otherParticipantBuy(
        ...
        emit ParticipantsBuy(_pazaAmount, _serialNo);
    }
    ...
    function mintLoanNft(
        ...
        if (_details.isRefinance) {
            ...
            if (
                ...
                IPool(pools[_details.bank]).transferAuctionedStakedAmount(
                    _details.refinanceTokenID,
                    _details.bank
                );
            } else {
    ...
    /**
     * @notice used to transfer the staked Amount of auctioned contract
     * @param _oldLender addresses of oldLender
     * @param _auctionWinner address of auctionWinner
     * @param _borrower address of borrower
     * @param _loanId loanId to be auctioned
     * @param _serialNo Serial number to identify the transaction
     **/

    function auctionStakedAmountTransfer(
        address _oldLender,
        address _auctionWinner,
        address _borrower,
        uint _loanId,
        ...
        LoanLib.Loan memory temp = IPool(pools[_oldLender]).getUserDetails(
            _borrower,
            _loanId
        );
        IPool(pools[_oldLender]).transferStakedAmount(
            _borrower,
            pools[_oldLender],
            _loanId
        );
        IPool(pools[_auctionWinner]).updateAuctionedStakedAmount(
            _loanId,
            temp.borrowerShare
        );
        emit AuctionStakedAmountTransferred(
            _oldLender,
            _auctionWinner,
            _borrower,
            _loanId,
    ...
    function poolLoanTransfer(
        ...
        address _investor
    ) public validation(_investor) onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
    ...
    function prepaidPazaBuy(
        ...
        require(isPoolExist[_lender] == true, "Not Whitelisted");
        address receiver;
        uint pazaAmount;
        if (auctionreward) {
            receiver = pools[_lender];
            LoanLib.Loan memory temp = IPool(pools[_lender]).getUserDetails(
                _borrower,
                _loanId
            );
            pazaAmount = temp.discount - temp.borrowerShare;
        } else {
            receiver = _lender;
            pazaAmount = _amount;
        }
        if (
            usdc.balanceOf(prepaidUsdcWallet) >=
            (_amount * 10 ** usdc.decimals()) / (trade.getTokenPrice())
        ) {
            trade.buyWithApproval(prepaidUsdcWallet, receiver, pazaAmount);
            emit PazaBuyFromUsdcWallet(_serialNo, receiver, pazaAmount);
        } else {
            ...
            emit PazaBuyFromCoinbase(_serialNo, receiver, (pazaAmount));
        }
    }
    ...
    function updateBankDetails(
        ...
        require(isPoolExist[_bank] == true, "No Bank Pool");
        banks[_bank] = _info;
    ...
    function whitelistBank(
        ...
        address _bank,
        ControllerLib.bankInfo memory _info
    ) public validation(_owner) validation(_bank) onlyRole(DEFAULT_ADMIN_ROLE) {
        require(isPoolExist[_bank] == false, "Already Whitelisted");
        banks[_bank] = _info;
        _deployPool(_owner, _bank);
        isPoolExist[_bank] = true;
...
interface IFactory{
   // function isBankPoolBlacklisted(address _bank) external view returns(bool);
   function setpostpaidUsdcWallet(address _wallet) external;  
   function setCefiWallet(address _wallet) external;
   function poolLoanTransfer(address[] calldata _borrowers, uint256[] calldata _ids, address _investor) external; 
}
...
interface ILoanNFT is IERC721 {
    function loan(uint256 _tokenID) external returns (LoanLib.Loan memory);
    function borrower(address _borrower) external returns (uint256);
    function nftStatus(uint _id) external view returns(bool);
    function mint(
        address _to,
        uint256 _tokenId,
        string memory _tokenURI,
    ...
    function freezeNFT(uint _tokenId) external;
...
interface IPaza is IERC20Upgradeable, IAccessControl {
       function decimals() external returns(uint);
       function getTokenPrice() external returns(uint);
}
...
interface IPool {
    function initialize(address _owner, address _token, address _factory, address _bank) external;
    function updateUserDetail(LoanLib.Loan memory _details, uint _loanId) external;
    function stake(address _borrower, uint256 _amount, uint256 _loanId) external;
    function transferStakedAmount(address _borrower, address _bank, uint256 _loanId)external;
    function removeTokensOfBlacklistedBank(address _auctionWinner) external;
    function getUserDetails(address borrower, uint loanId) external view returns(LoanLib.Loan memory _details);
    function transferRewards(address _borrower, uint _loanId, address _auctionWinner) external;
    function updateAuctionedStakedAmount(uint _loanId, uint _amount)external;
    function transferAuctionedStakedAmount(uint _loanId, address _bank) external;
}
...
library LoanLib {
   struct Loan {
        ...
        uint128 loanAmount;
        uint128 discount;
        uint128 borrowerShare;
        uint128 ispzIncShare;
        uint128 disbursementInterval;
        uint128 refinanceTokenID;
        uint128 loanTenure;
    }   
