...
contract Land is Royalty, AccessControl, BaseRegistrarImplementation, Pausable, TokenURISetting, Initializable {
    ...
    constructor(NNS nns_, address timelockController, address erc20TransferHelper_) BaseRegistrarImplementation("Land", "LAND", nns_) Royalty(erc20TransferHelper_){
     ...
     */
    function init(address micAddress_, address midAddress_, address payable micBank_, address payable midLandBank_, address payable alchemistBank_) external initializer {
    ...
    function priceOracle(uint256 count) internal pure returns(uint256) {
        ...
        uint256 eplch = (count - 1) / 4; //100
        return 500 ether + eplch * 10 ether;
    }

    /**
     * @dev Returns the mid token address.
     */
    function midAddress() public view returns (address) {
        return _midAddress;
    }
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        string memory render = tokenURIEngine().render(tokenId);
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "', name,'", "description": "Land is.", ', render, '}'))));
        string memory output = string(abi.encodePacked('data:application/json;base64,', json));
        return output;
    }

    function mint(string memory name_) public nonReentrant {
        name_ = name_.toLowercase();
        (bytes32 name, bytes32 rootName) = _nameSplit(name_);
        ...
        if(_currentPrice > 0){
            ...
            _MIDCoin.safeTransfer(_midLandBank, _currentPrice*5/10);
            _MIDCoin.safeTransfer(_alchemistBank, _currentPrice*3/10);
        }
    }
    ...
    function _nameSplit(string memory input) internal pure returns (bytes32, bytes32) {
        ...
        require(rootName.keccak() == keccak256(bytes("land")), "Root name format error");
    ...
    function updateMember(uint256 tokenIdOfLand, uint256 tokenIdOfMid) public nonReentrant {
     ...
     */
    function countOfMembers(uint256 tokenId) public view returns (uint256) {
     ...
     */
    function members(uint256 tokenId) public view returns (uint256[] memory) {
...
abstract contract TokenURISetting {
    ...
    /// @dev set tokenURIEngine's address.
    function _setTokenURIEngine(address tokenURIEngine__) internal {
        _tokenURIEngine = tokenURIEngine__;
        emit CallSetTokenURIEngine(tokenURIEngine__);
    }
