...
contract RafflePanel {
    ...
    uint256 private serviceFeePercentage; // Service fee percentage deducted from each raffle pool
    ...
    constructor() {
        ...
        serviceFeePercentage = 5; // Set the service fee percentage to 5%
    }
    ...
    function selectWinner(uint256 raffleId) internal {
        ...
        raffle.winner = winner; // Set the winner's address
    ...
    function getRaffleDetails(uint256 raffleId) external view returns (uint256, uint256, uint256, uint256, bool, address, address, uint256) {
        Raffle storage raffle = raffles[raffleId];
        return (
            raffle.tokenId,
            raffle.ticketPrice,
    ...
    function removeRaffle(uint256 raffleId) external {
        require(raffleExists[raffleId], "Raffle does not exist.");
        Raffle storage raffle = raffles[raffleId];
        require(raffle.isActive == false, "Active raffles cannot be removed.");
    ...
    function withdrawFunds() external {
        require(!raffleExists[raffleIdCounter], "Cannot withdraw funds while an active raffle exists.");
