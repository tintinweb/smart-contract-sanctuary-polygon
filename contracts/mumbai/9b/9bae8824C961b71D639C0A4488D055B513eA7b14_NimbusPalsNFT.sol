...
contract NimbusPalsNFT is DefaultOperatorFilterer, ONFT721, ERC2981 {
 ...
 error NimbusPalsNFT__ClaimListFinalized();
 error NimbusPalsNFT__ClaimsListMustBeFinalized();
 error NimbusPalsNFT__FullyMinted();
 error NimbusPalsNFT__InvalidMintingChain();
 error NimbusPalsNFT__NoClaimAvailable();
 error NimbusPalsNFT__RescueNotActive();
 ...
 address _minter,
 uint256 _chainId
 ) ONFT721('NimbusPals', 'Pals', _minGas, _layerZeroEndpoint) {
 baseURI = __baseURI;
 ...
 if (_msgSender() != minter) revert NimbusPalsNFT__CallerNotMinter();
 if (totalSupply == MAX_MINT_ID) revert NimbusPalsNFT__FullyMinted();
 if (!claimsFinalized) revert NimbusPalsNFT__ClaimsListMustBeFinalized();
 if (!hasClaimAvailable[receiver])
 revert NimbusPalsNFT__NoClaimAvailable();

 if (_getChainId() != CHAIN_ID) {
 revert NimbusPalsNFT__InvalidMintingChain();
 }
 ...
 _requireOwner();
 if (claimsFinalized) revert NimbusPalsNFT__ClaimListFinalized();
 ...
 revert NimbusPalsNFT__RescueNotActive();
 if (_msgSender() != minter) revert NimbusPalsNFT__CallerNotMinter();
 if (totalSupply == MAX_MINT_ID) revert NimbusPalsNFT__FullyMinted();
 if (_getChainId() != CHAIN_ID) {
 revert NimbusPalsNFT__InvalidMintingChain();
