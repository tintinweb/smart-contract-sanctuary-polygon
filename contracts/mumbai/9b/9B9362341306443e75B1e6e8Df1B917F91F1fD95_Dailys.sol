...
contract Dailys is VRFConsumerBase, KeeperCompatibleInterface, Nft {
    using Strings for uint256;
...
contract Nft is ERC721Enumerable, Utils {
    struct Attr {
        string DayNumber;
        string name;
        uint256 edition;
    ...
    struct SunsetAttr {
        ...
        string gradientSequence1; // rand color sequence
        string randDuration1; // animation1 duration
        string timeScale1; // sec/min/hour
        string stopColor2; // start/end color
        string gradientSequence2; // rand color sequence
        string randDuration2; // animation1 duration
        string timeScale2; // sec/min/hour
        string stopColor3; // start/end color
        string gradientSequence3; // rand color sequence
        string randDuration3; // animation1 duration
        string timeScale3; // sec/min/hour
        string transformDuration1; // animation1 duration
        string transformScale1; // sec/min/hour
        string stopColor4; // start/end color
        string gradientSequence4; // rand color sequence
        string randDuration4; // animation1 duration
        string timeScale4; // sec/min/hour
        string stopColor5; // start/end color
        string opacity5; // start/end color
        string gradientSequence5; // rand color sequence
        string randDuration5; // animation1 duration
        string timeScale5; // sec/min/hour
        string transformDuration2; // animation1 duration
        string transformScale2; // sec/min/hour
    }

    mapping(uint256 => Attr) public attributes;
    mapping(uint256 => SunsetAttr) private sunsets;
    ...
    function _generateOnMint(uint256 _tokenID) internal {
        ...
        sunsets[_tokenID] = SunsetAttr(
            ///////////Colorstop1//////////////
            backgroundColors(stopColors[0]),
            buildColorStops(),
            Strings.toString(durationRand1),
            backgroundTimer(randMod(7)),
            ///////////Colorstop2//////////////
            backgroundColors(stopColors[1]),
            buildColorStops(),
            Strings.toString(durationRand1),
            backgroundTimer(randMod(7)),
            ///////////Colorstop3//////////////
            backgroundColors(stopColors[2]),
            buildColorStops(),
            Strings.toString(durationRand1),
            backgroundTimer(randMod(7)),
            ///////////transform1//////////////
            ...
            backgroundTimer(randMod(7)),
            ///////////Colorstop4//////////////
            backgroundColors(stopColors[3]),
            buildColorStops(),
            Strings.toString(durationRand1),
            backgroundTimer(randMod(7)),
            ///////////Colorstop5//////////////
            backgroundColors(stopColors[4]),
    ...
    function tokenURI(
        ...
        Attr storage attr = attributes[tokenId];
        bytes32 imageURI;

        if (attr.svg_True_False) {
            imageURI = bytes32(bytes(attr.SVG_Image));
        } else {
            imageURI = bytes32(bytes(attr.imageA));
            if (attr.switchOn && imageRand >= attr.switchPoint) {
                imageURI = bytes32(bytes(attr.imageB));
            }
        }
        ...
        bytes memory json = abi.encode(
            ...
            Strings.toString(attr.edition),
            "},",
            '{"trait_type":"masterNumber","value":',
            Strings.toString(attr.masterNumber),
            "},",
            '{"trait_type":"switchPoint","value":',
            Strings.toString(attr.switchPoint),
            "}",
            "]}"
        );
        return
...
contract Utils is Ownable {
    string[8] public colors = [
        ...
        "FEDD00",
        "FF8F1C"
    ]; // [red, purple, blue, green, gold, silver, yellow, orange]
    ...
    function stopOpacityPicker(
        ...
        return string(abi.encodePacked(index * 10 + 10, "%"));
    }

    function setColor(uint256 index, string memory newColor) public onlyOwner {
        require(index < colors.length, "Index out of range");
    ...
    function buildColorStops() internal view returns (string memory) {
        bytes memory result;
        for (uint i = 0; i < 8; i++) {
    ...
    function getSvg() public view returns (string memory) {
        ...
        string memory gradient1 = buildGradient(
            "a",
            stopColors[0],
            stopColors[1],
            stopColors[2],
        ...
        string memory gradient2 = buildGradient(
            "b",
            stopColors[3],
            stopColors[4],
            stopColors[5],
        ...
        string
            memory closer = '<rect fill="url(#a)" width="100%" height="100%"/><rect fill="url(#b)" width="100%" height="100%"/></svg>';

        return string(abi.encodePacked(header, gradient1, gradient2, closer));
    }
    ...
    function buildGradient(
        ...
        uint256 stopColor2,
        uint256 stopColor3,
        uint256 durationRand
    ) internal view returns (string memory) {
        string memory gradientStop1 = buildGradientStop(stopColor1, "0");
        string memory gradientStop2 = buildGradientStop(stopColor2, ".5");
        string memory gradientStop3 = buildGradientStop(stopColor3, "1");
        ...
        return
            string(
                abi.encodePacked(
                    '<linearGradient id="',
                    id,
                    '" gradientUnits="objectBoundingBox" x1="1" y1="0" x2="1" y2="1">',
                    gradientStop1,
                    gradientStop2,
                    gradientStop3,
                    transform,
