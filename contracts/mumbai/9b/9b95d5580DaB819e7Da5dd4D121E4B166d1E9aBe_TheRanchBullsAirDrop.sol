...
contract TheRanchBullsAirDrop is VRFConsumerBase, Ownable {
    ...
    uint256 public eligibleAmount = 0;

    uint public giveawayId = 1;
    
    event centennial_Air_Drop(
        address[] indexed nftOwnerAddresses,
        uint256[] indexed indexOfNfts,
    ...
    */
    ...
    function centennialAirDrop() public onlyOwner {
        ...
        require(eligibleAmount >= 0, "Not Enough Funds to Warrant calling the function to award the Bulls");

        IERC20 tokenContract = IERC20(rewardTokenContract);

        uint256 _balance = eligibleAmount;
        ...
        uint256[] memory winningIndexes = new uint256[](100);
    ...
    /**
     * When funding with USDC, only pass in a value of 250 for simplicity reasons 
    */
    function fund(uint256 _amount) public payable {}
        // uint256 amount = _amount * rewardDecimals;
        // require(address(rewardTokenContract) != address(0), "ERROR: Must set the reward token contract first");
        // IERC20(rewardTokenContract).transferFrom(msg.sender, address(this), amount);
        // if (eligibleAmount >= 0) {
    ...
    function updateEligableRewardAmount(uint256 _amount) external  onlyOwner {
...
contract TheRanchBullsMint is ERC721Enumerable, IERC2981, Ownable {
    ...
    uint256 public ORDER_QUANTITY_1 = 100;     //USD VALUE
    uint256 public ORDER_QUANTITY_2 = 90;
    uint256 public ORDER_QUANTITY_3 = 80;
    uint256 public ORDER_QUANTITY_5 = 60;
    uint256 public ORDER_QUANTITY_7 = 50;
    uint256 public ORDER_QUANTITY_11 = 40;
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        if (tokenQuantity == 1) {
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_1) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            ...
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_2) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            for(uint256 i = 0; i < tokenQuantity; i++) {
                addressPurchases[msg.sender]++;
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
            ...
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_3) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            for(uint256 i = 0; i < tokenQuantity; i++) {
                addressPurchases[msg.sender]++;
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
            ...
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_5) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            for(uint256 i = 0; i < tokenQuantity; i++) {
                addressPurchases[msg.sender]++;
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
            ...
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_7) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            for(uint256 i = 0; i < tokenQuantity; i++) {
                addressPurchases[msg.sender]++;
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
            ...
            require(msg.value >= (getMintPrice(ORDER_QUANTITY_11) * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (BULLS_MINT_PRICE * quantity)");
            for(uint256 i = 0; i < tokenQuantity; i++) {
                addressPurchases[msg.sender]++;
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
