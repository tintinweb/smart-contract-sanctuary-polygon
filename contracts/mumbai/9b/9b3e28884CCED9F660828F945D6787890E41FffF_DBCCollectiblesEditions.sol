...
import "@bitflips/common/contracts/token/ERC1155/extensions/ERC1155Burnable.sol";
import "@bitflips/common/contracts/token/ERC1155/extensions/ERC1155AuthorizedAgent.sol";

contract DBCCollectiblesEditions is ERC1155Burnable, ERC1155AuthorizedAgent {
    address private constant BITFLIPS = 0x5db929ec2f27cFb6988cF739cae05E95cbD0feCd;
    address private constant A7X = 0x5f64884dfDf8455180De72A561E85068097916ed;

    string public name = "Deathbats Club Collectibles Editions";
    string public symbol = "DBC-CE";

    constructor(string memory _initialBaseURI)
    ERC1155Core(_initialBaseURI,
    ...
    function airdrop(address [] calldata holders, uint256 id) external onlyContributor {
    ...
    function payoutContributors() external onlyContributor {
        ...
        _sendContributorPayment(author(), balance, 10);
        _sendContributorPayment(A7X, balance, 90);
    }

    function owner() public view override (ERC1155Core, ERC1155AuthorizedAgent) returns (address) {
        return ERC1155AuthorizedAgent.owner();
    }

    function isApprovedForAll(
        address ownerAddress,
        address operatorAddress
    ) public view override (ERC1155, ERC1155AuthorizedAgent) returns (bool isOperator) {
        return ERC1155AuthorizedAgent.isApprovedForAll(ownerAddress, operatorAddress);
    }
}
 ...
 *
 */
abstract contract ERC1155Burnable is ERC1155Core {
    function burn(
        address account,
        uint256 id,
        uint256 value
...
import "../ERC1155Core.sol";
import "../../../access/AuthorizedAgent.sol";


/**
 * @title  ERC1155AuthorizedAgent
 * @notice Extension adds AuthorizedAgent functionality to contracts extending ERC721AuthorizedAgent
 */
abstract contract ERC1155AuthorizedAgent is ERC1155Core, AuthorizedAgent {

    /**
     * @dev overrides owner to support agent approval for ERC1155
     */
    function owner() public view virtual override (Ownable, ERC1155Core) returns (address) {
        return ERC1155Core.owner();
    }

    /**
     * @dev overrides AuthorizedAgent to support agent approval for ERC1155
     */
    function isApprovedForAll(
        address ownerAddress,
        address operatorAddress
    ) public view virtual override (ERC1155) returns (bool isOperator) {
...
contract ERC1155Core is Ownable, Contributable, ERC1155, UpdatableOperatorFilterer, ERC2981Setter
{
    ...
    constructor(string memory _initialBaseURI, OperatorFiltererConfiguration memory operatorFilterConfig)
    ...
    /**
     * @dev overrides supportInterface to support both ERC1155 and ERC2981
     */
    ...
    /**
     * @dev overrides safeBatchTransferFrom for OperatorFilterer that only allow use by approved operators from
    ...
    function emitPermanent(string calldata value, uint256 tokenId) external onlyOwner {
        require(baseURIFinal, "Base URI must be finalized first");
        emit PermanentURI(value, tokenId);
    }
}
