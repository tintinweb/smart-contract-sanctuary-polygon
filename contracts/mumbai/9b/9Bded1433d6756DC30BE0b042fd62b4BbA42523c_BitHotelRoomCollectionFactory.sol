...
contract BitHotelRoomCollectionFactory is AccessControl {
    ...
    function deployAuction(
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            newCollection.mint(controller, i, uri, royaltyRecipient, royaltyValue);
            newCollection.approve(auctionContract, i);

            if (prices[1] == 0 || i == 1) {
                auction.setupAuction(controller, address(newCollection), i, prices[0], duration);
            } else {
                auction.setupAuction(controller, address(newCollection), i, prices[1], duration);
            }
        }

        emit DeployedTimeAuction(address(newCollection), msg.sender, name, symbol, replicas_, prices[0], duration);
    ...
    function deploy(
        ...
        for (uint256 i = 1; i <= replicas_; i++) {
            ...
            if (prices[1] == 0 || i == 1) {
                marketplace.addERC721Listing(controller, address(newCollection), i, prices[0]);
            } else {
                marketplace.addERC721Listing(controller, address(newCollection), i, prices[1]);
            }
        }
        ...
        emit Deployed(address(newCollection), msg.sender, name, symbol, replicas_, prices[0]);
