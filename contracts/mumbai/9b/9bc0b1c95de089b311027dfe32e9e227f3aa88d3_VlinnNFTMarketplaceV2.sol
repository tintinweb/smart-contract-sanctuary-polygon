...
/// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function listNft(
        ...
        uint256 _tokenId,
        uint256 _price
    ) external isVlinnNFT(_nft) whenNotPaused nonReentrant {
    ...
    function buyNFT(
        ...
        if(IERC165(listedNft.nft).supportsInterface(_INTERFACE_ID_ERC2981)){
            (address royaltyRecipient ,uint256 royaltyFee) = IERC2981(_nft).royaltyInfo(_tokenId, listedNft.price);
            if (royaltyFee > 0) {
           ...
           (uint96  royaltyFeePercent, address royaltyRecipient) = vlinnNFTFactory.getCustomRoyaltyInfo(_nft);
            if(royaltyFeePercent > 0){
                uint256 royaltyFee = calculateCustomFee(listedNft.price, royaltyFeePercent);
                payable(royaltyRecipient).transfer(royaltyFee);
                totalPrice -= royaltyFee;
            }
        }
        if(!vlinnNFTFactory.hasPromotion(_nft)){
