...
contract Ticket is ERC1155, AccessControl, Pausable, ERC1155Supply, PaymentSplitter {
    ...
    function setDiscountBuyer(address buyer, bool enabled) public onlyRole(DEFAULT_ADMIN_ROLE) {
        if (enabled) {
            _grantRole(DISCOUNT_BUYER_ROLE, buyer);
        } else {
            _revokeRole(DISCOUNT_BUYER_ROLE, buyer);
        }
    }
    ...
    function discountBuy(uint256 id, uint256 qty) public payable onlyRole(DISCOUNT_BUYER_ROLE) {
