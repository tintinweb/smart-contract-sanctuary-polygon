...
contract Marketplace721A is Initializable, ERC721A__IERC721ReceiverUpgradeable, OwnerOperator {
    ...
    event PaymentTransfer(string indexed paymentType, string unindexedPaymentType);
    ...
    IERC721AUpgradeable public ERC721ASell;
    ...
    modifier verifySignature(
        ...
        bytes32 hash = keccak256(abi.encodePacked(msg.sender, nonce, _tradeAddress, _attributes));
    ...
    function __Marketplace721A_init(address _ERC721ASell) public initializer {
        super.initialize();
        setERC721ASell(_ERC721ASell);
    }
    ...
    function sell(uint256 _tokenId, uint256 _orderPrice, address _currency) external {
        Order memory order = Order(address(ERC721ASell), _tokenId, msg.sender, _orderPrice, _currency);
        orders[_tokenId] = order;

        ERC721ASell.safeTransferFrom(msg.sender, address(this), _tokenId, '');

        emit CreateSellOrder(_tokenId, order.owner, order.tokenId, order.orderPrice, order.currency);
    }

    function buy(uint256 _tokenId, address _buyer, string memory paymentType) external payable operatorOrOwner {
        // check order status
        Order storage order = orders[_tokenId];
        require(order.owner != address(0), 'Order does not exist or is deleted');

        ERC721ASell.safeTransferFrom(address(this), _buyer, order.tokenId, '');

        if (order.currency == address(0)) {
            require(msg.value == order.orderPrice, 'Invalid price');
            TransferHelper.safeTransferETH(order.owner, order.orderPrice);
        } else {
            require(msg.value == 0, 'Native currency is not allowed');
            TransferHelper.safeTransferFrom(order.currency, msg.sender, order.owner, order.orderPrice);
        }

        emit Buy(_tokenId, _buyer, order.owner, order.tokenId, order.orderPrice, order.currency);
        emit PaymentTransfer(paymentType, paymentType);
    ...
    function paymentTransferFrom(
        ...
        ERC721ASell.safeTransferFrom(from, to, tokenId);
    ...
    function cancelSell(uint256 _orderId) external {
        ...
        ERC721ASell.safeTransferFrom(address(this), msg.sender, orders[_orderId].tokenId, '');
    ...
    function buyNFTNormal(
        ...
        Data memory tradeInfo = Data({ tradeAddress: _tradeAddress, attributes: _attributes });
        // check allowance of buyer
        require(
            IERC20Upgradeable(tradeInfo.tradeAddress[2]).allowance(msg.sender, address(this)) >=
        ...
        TransferHelper.safeTransferFrom(tradeInfo.tradeAddress[2], msg.sender, tradeInfo.tradeAddress[1], amount);

        // transfer NFT from seller to buyer
        ERC721ASell.safeTransferFrom(tradeInfo.tradeAddress[1], msg.sender, tradeInfo.attributes[1]);
        emit BuyNFTNormal(
    ...
    function buyNFTETH(
        ...
        uint256 nonce,
        bytes memory signature
    ) external payable verifySignature(nonce, _tradeAddress, _attributes, signature) {
        Data memory tradeInfo = Data({ tradeAddress: _tradeAddress, attributes: _attributes });
        (uint256 amount, uint256 feeOwner, uint256 feeAdmin) = checkFeeProductExits(
            tradeInfo.tradeAddress,
            tradeInfo.attributes
        ...
        if (msg.value > tradeInfo.attributes[0])
            TransferHelper.safeTransferETH(msg.sender, msg.value - tradeInfo.attributes[0]);
        emit BuyNFTETH(
            [msg.sender, tradeInfo.tradeAddress[1], tradeInfo.tradeAddress[2]],
            tradeInfo.attributes[1],
            tradeInfo.attributes[0]
    ...
    function acceptOfferNFT(
        ...
        require(
            ERC721ASell.getApproved(tradeInfo.attributes[1]) == address(this),
            'tokenId do not approve for contract'
        );
        // check allowance of buyer
        require(
            IERC20Upgradeable(tradeInfo.tradeAddress[2]).allowance(tradeInfo.tradeAddress[0], address(this)) >=
                tradeInfo.attributes[0],
            'token allowance too low'
        );
        ...
        // transfer token from buyer to seller
        TransferHelper.safeTransferFrom(tradeInfo.tradeAddress[2], tradeInfo.tradeAddress[0], msg.sender, amount);

        // transfer NFT from seller to buyer
        ERC721ASell.safeTransferFrom(tradeInfo.tradeAddress[1], tradeInfo.tradeAddress[0], tradeInfo.attributes[1]);
        emit AcceptOfferNFT(
            [msg.sender, tradeInfo.tradeAddress[1], tradeInfo.tradeAddress[2]],
            tradeInfo.attributes[1],
            tradeInfo.attributes[0]
    ...
    function setERC721ASell(address _ERC721ASell) public operatorOrOwner {
        ERC721ASell = IERC721AUpgradeable(_ERC721ASell);
    }
