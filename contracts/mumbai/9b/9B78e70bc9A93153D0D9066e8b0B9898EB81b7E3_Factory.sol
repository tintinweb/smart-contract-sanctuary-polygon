...
contract Factory is IFactory, IOwnable {
    ...
    function setMajorityTakeoverPercentage(uint256 _majorityPercentage) external onlyOwner{
        require(_majorityPercentage > 5000000, "INCP");
        majorityFractionShare = _majorityPercentage;
    }

    function setMajorityAboveMarketPercentage(uint256 _majorityMarketPercentage) external onlyOwner{
    ...
    function redeemToggle(RedeemVoucher calldata voucher)external override onlyOwner {
    ...
    function majorityShareTakeover(
        ...
        require(ftnft.fractionShares(holdingFraction) >= majorityFractionShare,"Not majority");
        
        ITangibleNFT tnft = ftnft.tnft();
        uint256 fingerprint = ftnft.tnftFingerprint();
        //take market price
        (uint256 weSellAt,,,,) = priceManager.getPriceOracleForCategory(tnft).usdcPrice(tnft, fingerprint, 0);
        ...
        for(uint256 i = 0; i < length; i++){
            ...
            uint256 usdcToTransfer = aboveMarketPrice * ftnft.fractionShares(otherFractions[i]) / ftnft.fullShare();
            //transfer the money
            USDC.safeTransferFrom(majorityOwner, ftnft.ownerOf(otherFractions[i]), usdcToTransfer);
            //take the ftnft
            ftnft.safeTransferFrom(ftnft.ownerOf(otherFractions[i]), majorityOwner, otherFractions[i]);
        }
    }
