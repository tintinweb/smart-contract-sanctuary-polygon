...
contract StakingAlgo1_matic is ReentrancyGuard {
    struct UserInfo {
    ...
    TOKEN public rewardToken = TOKEN(0x6c4544017B681CCFa5255143610385FEd3A0a445); // OMH token
    ...
    // MATIC/OMH pool contract
    constructor(
        uint256 _startDate,
        address _priceFeedAddr // Chainlink Aggregator : MATIC/USD
    ) {
        admin = msg.sender;
    ...
    /** external funtions */
    function deposit() external nonReentrant checkActive payable {
    ...
    function apyNotVip() external view returns (uint256) {
        ...
        if (totalDeposit > 0) {
            apy =
                (latestReward * APYDENOMINATOR * 365 * rewardPercent) / totalDeposit / rewardCycle / 100;
        }
    ...
    function apyOfVip1() external view returns (uint256) {
        ...
        if (totalDeposit > 0) {
            apy =
                (latestReward * APYDENOMINATOR * 365 * vip1RewardPercent) / totalDeposit / rewardCycle / 100;
        }
    ...
    function apyOfVip2() external view returns (uint256) {
        ...
        if (totalDeposit > 0) {
            apy =
                (latestReward * APYDENOMINATOR * 365 * vip2RewardPercent) / totalDeposit / rewardCycle / 100;
        }
    ...
    function depositTokenDecimals() public pure returns (uint8) {
        return 18;
    }
