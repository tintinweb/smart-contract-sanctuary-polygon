...
contract DPSPlunderersGuild is ERC1155Holder, Ownable, ReentrancyGuard {
    ...
    mapping(address => mapping(uint8 => LockedChest[])) private lockedChests;
    mapping(address => mapping(uint8 => LockedChest[])) private claimedChests;

    mapping(address => LockedLockBox) private lockedLockBoxes;
    mapping(address => LockedLockBox) private claimedLockBoxes;
    ...
    function lockToClaimRewards(uint256[] calldata _chestsTypes, uint256[] calldata _chestsAmounts) external nonReentrant {
        // require(gameSettings.isPaused(4) == 0, "Paused");
        if (gameSettings.isPaused(4) == 1) revert Paused();
        for (uint256 i; i < _chestsTypes.length; i++) {
            uint256 chestType = _chestsTypes[i];
            require(lockedChests[msg.sender][uint8(chestType)].length == 0, "lockedChest");
            uint256 amountOfChests = _chestsAmounts[i];
            if (chest.balanceOf(msg.sender, chestType) < amountOfChests) continue;
            lockedChests[msg.sender][uint8(chestType)].push(
                LockedChest(VOYAGE_TYPE(chestType), block.number, block.timestamp, 0, amountOfChests, msg.sender)
            );
            chest.safeTransferFrom(msg.sender, address(this), chestType, amountOfChests, "");
            emit LockingChest(msg.sender, chestType, amountOfChests);
        }
    }
    ...
    function claimLockedRewards(uint8 _type, CausalityParams memory causalityParams) external nonReentrant {
        LockedChest[] memory userLockedChests = lockedChests[msg.sender][_type];

        VOYAGE_TYPE voyageType = VOYAGE_TYPE(_type);
        ...
        for (uint256 i; i < userLockedChests.length; i++) {
            ...
            if (lockedChest.lockedBlock + gameSettings.getBlockJumps() > block.number) continue;
            uint256 result = causality.getRandom(
                msg.sender,
                causalityParams.blockNumber[i],
                causalityParams.hash1[i],
                causalityParams.hash2[i],
                causalityParams.timestamp[i],
                causalityParams.signature[i],
        ...
        delete lockedChests[msg.sender][_type];
    ...
    function claimLockedBox(uint256 _amount, CausalityParams memory causalityParams) external {
        LockedLockBox memory lockedLockBox = lockedLockBoxes[msg.sender];
        if (lockedLockBox.amount != _amount) revert WrongParams(2);
        // require(lockedLockBox.amount == _amount, "!lockedLockBox");
        if ((lockedLockBox.lockedBlock + gameSettings.getBlockJumps()) >= block.number) revert WrongState(1);
    ...
    function getLockedRewards(address _owner, uint8 _type) external view returns (LockedChest[] memory) {
        return lockedChests[_owner][_type];
    }
