...
import 'medallion/AbstractProductV1.sol';
...
contract ProductVariantsRandomV1 is AbstractProductV1 {
    using StringsUpgradeable for uint256;

    /// @notice list of all variants
    Variant[] _variants;

    /// @notice mapping of variant id to 1-indexed position in list of variants
    mapping(bytes32 => uint256) _variantsIndex;

    /// @notice mapping of token id to variant id
    mapping(uint256 => bytes32) _tokenVariants;

    /// @notice mapping of variant id to token ids
    mapping(bytes32 => uint256[]) _variantTokens;

    /// @notice mapping of token id to index in variant token list
    mapping(uint256 => uint256) _tokenVariantIndex;

    /// @notice Initializes the product implementation
    /// @dev Variants are expected to be detailed in contract Metadata
    ...
    /// @param variants_ The available product variants
    function __ProductVariantsRandomV1_init(
        ...
        Variant[8] calldata variants_
    ) public initializer {
        AbstractProductV1.__AbstractProductV1_init(
            name_,
            symbol_,
            contractURI_,
            tokenURI_,
    ...
    /// @dev replaces the static Product token URI with the usual dynamic one
    ...
    /// @notice Get the variant id by index
    /// @param index The variant index
    function variantByIndex(uint256 index) public view returns (bytes32) {
        require(index >= 0 && index < _variants.length, 'Product: out of bounds');

        return _variants[index].id;
    }

    /// @notice Get total supply for a variant
    ...
    /// @notice Return total available inventory
    /// @return int256 The number of available tokens
    function inventory() public view returns (uint256) {
        uint256 available = 0;
        for (uint256 i = 0; i < _variants.length; i++) {
            available += inventory(_variants[i].id);
        }

        return available;
    }

    /// @notice Return variant available inventory
    /// @param variantId The variant id to check for
    /// @return int256 The number of available tokens for the provided variant
    function inventory(bytes32 variantId) public view returns (uint256) {
        uint256 variantIndex = _variantsIndex[variantId] - 1;
        require(variantIndex >= 0, 'Product: variant not found');

        return _variants[variantIndex].initialInventory - _variantTokens[variantId].length;
    }

    /// @inheritdoc ERC721Upgradeable
    /// @dev Override to implement variant allocation
    function _beforeTokenTransfer(
        address from,
        address to,
        uint256 tokenId
    ) internal virtual override(AbstractProductV1) {
        super._beforeTokenTransfer(from, to, tokenId);

        if (from == address(0)) {
            require(isEligible(to), 'Product: recipient ineligible');
            _issueVariant(tokenId);
        }
    }

    /// @notice Function to check an issue recipient is eligible
    /// @dev Reverts if recipient is not eligible
    function isEligible(address recipient) internal view returns (bool) {
    ...
    /// @notice Assigns random variant to newly issued token id
    /// @dev This isn't intented to be securely random
    /// @param tokenId The token id
    function _issueVariant(uint256 tokenId) internal {
        uint256 numVariants = _variants.length;

        // single variant isn't random
        if (numVariants == 1) {
            return _issueVariant(tokenId, _variants[0].id);
        }

        // get available inventory and pick a random item
        uint256 available = inventory();
        uint256 random = uint256(keccak256(abi.encodePacked(block.difficulty, block.number, block.timestamp))) %
            available;

        // find variant for random item
        uint256 counter = 0;
        bytes32 chosenVariantID = 0x0;
        for (uint256 i = 0; i < numVariants; i++) {
            bytes32 variantId = _variants[i].id;
            counter += inventory(variantId);
            if (counter > random) {
                chosenVariantID = variantId;
                break;
            }
        }
        require(chosenVariantID != 0x0, 'Product: cannot assign variant');

        _issueVariant(tokenId, chosenVariantID);
    }

    /// @notice Assigns variant to newly issued token id
    /// @param tokenId The token id
    /// @param variantId The variant id
    function _issueVariant(uint256 tokenId, bytes32 variantId) internal {
        uint256 variantIndex = _variantsIndex[variantId] - 1;
        require(variantIndex >= 0, 'Product: variant not found');
...
import 'medallion/IProductHistoryV1.sol';
import 'medallion/IProductMetadataV1.sol';
import 'medallion/AbstractRescueFundsV1.sol';

abstract contract AbstractProductV1 is
    Initializable,
    AccessControlEnumerableUpgradeable,
    ERC721BurnableUpgradeable,
    ERC721EnumerableUpgradeable,
    UUPSUpgradeable,
    AbstractRescueFundsV1,
    IProductHistoryV1,
    IProductMetadataV1,
    IOwnable
{
    ...
    /// @param delegateAdmin_ The delegated admin address
    /// @param delegateIssuer_ The delegated issuer address
    function __AbstractProductV1_init(
        string calldata name_,
        string calldata symbol_,
        string calldata contractURI_,
        string calldata tokenURI_,
    ...
    /// @notice Issues a token to the recipient address
    ...
    function privateIssue(address recipient) public onlyProxy onlyRole(DELEGATE_ISSUER_ROLE) returns (uint256) {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(ownerOf(tokenId) != address(0), 'Product: invalid token ID');
    ...
    function transferredAt(uint256 tokenId, uint256 index) public view returns (uint256) {
        require(index < _transferredAt[tokenId].length, 'Product: out of bounds');
    ...
    /// @inheritdoc AbstractRescueFundsV1
    /// @dev Only callable by DELEGATE_ADMIN_ROLE
    function _authorizeRescue() internal virtual override onlyRole(DELEGATE_ADMIN_ROLE) {}
...
interface IProductHistoryV1 {
    ...
    /// @param index The transfer index to return history for
    /// @return uint256 When the token was issued
    function transferredAt(uint256 tokenId, uint256 index) external view returns (uint256);
...
interface IProductMetadataV1 {
    ...
    /// @dev Only callable by DELEGATE_ADMIN_ROLE
    /// @param newContractURI The new contract metadata URI
    function amendContractURI(string calldata newContractURI) external;

    /// @notice Amends the token URI shared by all tokens
    /// @dev Only callable by DELEGATE_ADMIN_ROLE
    /// @param newTokenURI The new token metadata URI
    function amendTokenURI(string calldata newTokenURI) external;
}
...
import 'medallion/IRescueFundsV1.sol';

abstract contract AbstractRescueFundsV1 is IRescueFundsV1 {
...
interface IRescueFundsV1 {
    ...
    /// @param recipientAddress The account to transfer to
    function withdrawNativeToken(uint256 amount, address recipientAddress) external;
}
