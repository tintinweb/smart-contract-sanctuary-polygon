...
import { PoseTypeUpgradeable } from "./internal-upgradeable/PoseTypeUpgradeable.sol";
import { PrimarySaleUpgradeable } from "./internal-upgradeable/PrimarySaleUpgradeable.sol";
...
import { IBase721Upgradeable } from "./interfaces/IBase721Upgradeable.sol";
    ...
    ERC721WithPermitUpgradable
{
    ...
    function initialize(
        ...
        __PoseType_init_unchained(types_, typeInfos_);
        __PrimarySale_init_unchained(recipient_);
        _idCounter = 1;
    }

    function buy(uint256 typeNFT_, uint256 quantity_, address recipient_) external payable override nonReentrant {
        TypeInfo memory typeInfo = _typeInfo[typeNFT_];
        uint256 total = typeInfo.price * quantity_;

        if (total == 0) revert NFT__InvalidType();

        _processPayment(typeInfo.paymentToken, total, _msgSender());
        ...
        _batchProcess(recipient_, tokenId, quantity_, typeInfo.executeOperation, typeInfo.operation);
    }

    function batchMint(uint256 typeNFT_, uint256 quantity_, address recipient_) external onlyRole(MINTER_ROLE) {
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_]);
        _batchProcess(
    ...
    function _processPayment(address paymentToken_, uint256 total_, address payer_) internal {
        uint256 refund;
        address recipient = _recipient;
        if (paymentToken_ == address(0)) {
            uint256 value = msg.value;
            refund = value - total_; // will throw underflow error if value < total -> skip refund
            if (refund > 0.1 ether) {
                SafeTransferLib.safeTransferETH(payer_, refund);
                SafeTransferLib.safeTransferETH(recipient, total_);
            } else {
                SafeTransferLib.safeTransferETH(recipient, value);
            }
        } else {
            SafeTransferLib.safeTransferFrom(paymentToken_, payer_, recipient, total_);
        }
    }
...
interface IBase721Upgradeable is IBaseUpgradeable {
    ...
    function buy(uint256 typeNFT_, uint256 quantity_, address recipient_) external payable;
}
...
interface IPoseTypeUpgradeable {
    ...
    error PoseTypeUpgradeable__ExceedLimit(uint256 typeNFT);
}
...
import { IERC721BurnableUpgradeable } from "../interfaces/IERC721BurnableUpgradeable.sol";
...
import { IPoseTypeUpgradeable } from "../interfaces/IPoseTypeUpgradeable.sol";
import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { OPERATOR_ROLE } from "../libraries/Constants.sol";

abstract contract PoseTypeUpgradeable is IPoseTypeUpgradeable, BaseUpgradeable {
    mapping(uint256 => TypeInfo) internal _typeInfo;
    mapping(uint256 => uint256) internal _sold;

    function __PoseType_init(uint256[] calldata types_, TypeInfo[] calldata typeInfos_) internal onlyInitializing {
        __PoseType_init_unchained(types_, typeInfos_);
    }

    function __PoseType_init_unchained(
        uint256[] calldata types_,
        TypeInfo[] calldata typeInfos_
    ) internal onlyInitializing {
    ...
    function getTypeNFT(uint256 typeNFT) external view returns (bool, address, uint256, uint256, Operation memory) {
        return (
            _typeInfo[typeNFT].executeOperation,
            _typeInfo[typeNFT].paymentToken,
            _typeInfo[typeNFT].price,
            _typeInfo[typeNFT].limit,
            _typeInfo[typeNFT].operation
        );
    }

    function setType(uint256 type_, TypeInfo calldata typeInfo_) external onlyRole(OPERATOR_ROLE) {
    ...
    function _setSold(uint256 typeNFT_, uint256 quantity_) internal {
        ...
        if (_sold[typeNFT_] > _typeInfo[typeNFT_].limit) revert PoseTypeUpgradeable__ExceedLimit(typeNFT_);
    }

    function _setType(uint256 type_, TypeInfo calldata typeInfo_) internal {
        if (_sold[type_] > typeInfo_.limit) revert PoseTypeUpgradeable__ExceedLimit(type_);
...
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { IPrimarySaleUpgradeable } from "../interfaces/IPrimarySaleUpgradeable.sol";
