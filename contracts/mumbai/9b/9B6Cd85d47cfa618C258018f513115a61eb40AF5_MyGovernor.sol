    ...
    Governor
{
    ...
    event Initializer(uint256 _snapshot, uint256 _deadline);
    ...
    function initialize(
        ...
        require(
            ...
            'Error: Cannot start before current week'
        );
        ...
        //set _votingEscrow
        _votingEscrow = getVotingEscrow();

        //set start of new week
        _weeklieproposalDetails[timestamp]._snapshot = timestamp;

        //set deadline of new week
        _weeklieproposalDetails[timestamp]._deadline =
            _weeklieproposalDetails[timestamp]._snapshot +
            VOTING_PERIOD;

        //set votingPower of new week to 0
        _weeklieproposalDetails[timestamp].votingPower = 0;

        //set forVotes of new week to 0
        _weeklieproposalDetails[timestamp].forVotes = 0;
        ...
        _weeklieproposalDetails[timestamp]._tie = true;
    ...
    function start_VOTING_PERIOD(
        ...
        require(
            started_time == true,
            'Error:Cannot start new voting period without setting a winner proposal'
        );
        ...
        require(
            currentWeek >= _weeklieproposalDetails[_startTime]._deadline,
            'Error:Cannot start before reach deadline'
        );

        //checking if there is a tie
        require(
            _weeklieproposalDetails[_startTime]._tie == false,
            "Error:Cannot start new voting period there's a tie, please extend_deadline"
        );

        //checking if the winner have been Executed or Canceled
        ProposalState status = state(
            _weeklieproposalDetails[_startTime].winnerId
        );
        ProposalState canceled = ProposalState.Canceled;
        ProposalState executed = ProposalState.Executed;
        require(
            status == canceled || status == executed,
            'Error:Execute or Cancel winner proposal first'
        );
    ...
    function set_Winner() public {
        ...
        require(
            started_time == false,
            'Winner cannot be setted: Finish current proposal first'
        );
        ...
        require(
            ...
            "Winner cannot be setted: Deadline hasn't been reached"
        );
        ...
        emit Winner(
            winnerProposal.winnerId,
            winnerProposal.votingPower,
            winnerProposal.forVotes,
            winnerProposal._snapshot,
            winnerProposal._deadline
        );
    }
    ...
    function getVotingPower(uint256 timestamp) external view returns (uint256) {
        return _weeklieproposalDetails[timestamp].votingPower;
    }

    /**
     * @notice Returns the beginning of the current week
     */
    function get_startTime() external view returns (uint256) {
        return _startTime;
    }
    ...
    function getWinnerId(uint256 timestamp) external view returns (uint256) {
        return _weeklieproposalDetails[timestamp].winnerId;
    }
     ...
     */
    function getSnapshot(uint256 week) public view returns (uint256) {
        return _weeklieproposalDetails[week]._snapshot;
    }

    /**
     * @notice Returns the current block
     */
     ...
     */
    function getDeadline(uint256 timestamp) public view returns (uint256) {
        return _weeklieproposalDetails[timestamp]._deadline;
    }

    /**
     * @notice Returns if the account has voted in the current week
     */
    function hasVoted_currentweek(
        uint256 timestamp,
        address account
    ...
    function _countVote(
        ...
        require(
            _startTime == timestamp,
            'GovernorCompatibilityBravo: timestamp is not from this week'
        );
        ...
        uint256 WinnerID = _weeklieproposalDetails[_startTime].winnerId;
        ...
        if (support == uint8(VoteType.For)) {
            //set receipt of the vote
            _weeklieproposalDetails[_startTime]
                .receipts[account]
                .hasVoted = true;
            ...
            if (WinnerID == 0) {
                ...
                //set the winner
                _weeklieproposalDetails[_startTime].forVotes = actual.forVotes;
                _weeklieproposalDetails[_startTime].votingPower =
                    _weeklieproposalDetails[_startTime].votingPower +
                    weight;
                _weeklieproposalDetails[_startTime].winnerId = proposalId;
                _weeklieproposalDetails[_startTime]._tie = false;
            } else {
                if (proposalId != WinnerID) {
                    //checking if actual and winner id are not the same

                    uint256 test = actual.forVotes + weight; // add weight actual proposal voting power
                    uint256 winner = _weeklieproposalDetails[_startTime]
                        .forVotes; //winner proposal voting power
                    if (winner == test) {
                        actual.forVotes = test; // add weight actual proposal voting power

                        //set the winner
                        _weeklieproposalDetails[_startTime].forVotes = actual
                            .forVotes;
                        _weeklieproposalDetails[_startTime].votingPower =
                            _weeklieproposalDetails[_startTime].votingPower +
                            weight; //overall winner proposal voting power
                        _weeklieproposalDetails[_startTime]
                            .winnerId = proposalId;
                        _weeklieproposalDetails[_startTime]._tie = true;
                    } else if (winner < test) {
                        actual.forVotes = test; // add weight actual proposal voting power
                ...
                if (
                    proposalId == WinnerID
                ) //checking if actual and winner id are the same
                {
                    actual.forVotes = actual.forVotes + weight; //add weight actual proposal voting power
    ...
    IERC1155Receiver
{
    ...
    function castVote(
        ...
        return _castVote(proposalId, timestamp, voter, support, '');
    }
    ...
    function castVoteWithReason(
        ...
        address voter = _msgSender();
        return _castVote(proposalId, timestamp, voter, support, reason);
    }
    ...
    function castVoteWithReasonAndParams(
        ...
        address voter = _msgSender();
        return _castVote(proposalId, timestamp, voter, support, reason, params);
    }
    ...
    function _castVote(
        ...
        _countVote(proposalId, timestamp, account, support, weight, params);
    ...
    GovernorTimelockControl
{
    ...
    constructor(
        ...
        Governor(name, votingEscrow, feeDistributor)
    ...
    /**
     * @dev See {IGovernorCompatibilityBravo-state}.
     */
    function state_ed(uint256 proposalId) public view returns (ProposalState) {
    ...
    function _propose(
        ...
        uint256 deadline = getDeadline(snapshot);
    ...
    /**
     * @dev See {IGovernor-executor}.
     */

    function executor_ed(
        uint256 snapshot
    ) internal view checkWeek(snapshot) returns (address) {
        return _executor();
    }
    ...
    modifier checkWeek(uint256 snapshot) {
        require(
            getSnapshot(snapshot) == snapshot,
            '_queued:snapshot is not from this week'
        );

        require(
            getDeadline(snapshot) >= block.timestamp,
            '_queued: deadline has been reached'
        );
        _;
    }
}
