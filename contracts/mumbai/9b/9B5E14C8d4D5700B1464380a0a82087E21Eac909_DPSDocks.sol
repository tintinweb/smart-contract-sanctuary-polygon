...
contract DPSDocks is ERC721Holder, ERC1155Holder, ReentrancyGuard, Ownable {
        ...
        nonReentrant
    {
        ...
        for (uint256 index; index < _lockedVoyages.length; index++) {
            ...
            // gameSettings.checkLockingItems(
            //     [lockedVoyage.voyageId, lockedVoyage.dpsId, lockedVoyage.flagshipId],
            //     msg.sender,
            //     [address(voyage), address(dps), address(flagship)]
            // );
            ...
            // console.log("totalSupportShips ", totalSupportShips);
            // console.log("lockedVoyage.totalSupportShips ", lockedVoyage.totalSupportShips);
            require(
    ...
    function computeVoyageState(
        ...
        for (uint256 i; i < _voyageConfig.sequence.length; i++) {
            ...
            claimingRewardsCache.entropy = string(abi.encodePacked("INTERACTION_RESULT_", i, "_", _lockedVoyage.voyageId));
    ...
    function awardRewards(
        ...
        if (_voyageResult.totalSupportShipsDestroyed > 0) {
            for (uint256 i; i < 9; i++) {
                if (_voyageResult.destroyedSupportShips[i] > 0) {
                    // console.log("Destroyed ship ", _voyageResult.destroyedSupportShips[i]);
                    supportShip.burn(address(this), SUPPORT_SHIP_TYPE(i), _voyageResult.destroyedSupportShips[i]);
