...
import { IERC721CrossChain } from "./IERC721CrossChain.sol";

contract CrossChainERC721 is AxelarExecutable, ERC721,ERC721URIStorage, IERC721CrossChain, Ownable{    
    using StringToAddress for string;
    using AddressToString for address;
    string public chainName; //To check if we are the source chain.
    string public minterChain; // To enable mints only from one chain to prevent duplicates
    ...
    constructor(
        ...
        string memory minterChain_,
        string memory chainName_
    ) AxelarExecutable(gateway_) ERC721(name_, symbol_) {
        gasReceiver = IAxelarGasService(gasReceiver_);
        minterChain = minterChain_;
        chainName = chainName_;
    }    

    function setChainName(string calldata chainName_) external onlyOwner{
        chainName = chainName_;
    }
    function setMinterChain(string calldata minterChain_) external onlyOwner{
        minterChain = minterChain_;
    }
