...
contract GPFlyer is Context, ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function getApproved(uint256 tokenId) public view virtual override returns (address) {
        ...
        if(approved == address(0) && !_tokenBifurcated[tokenId]) {
           approved = _target.getApproved(tokenId);
        }
        return approved;
    }
    ...
    function isApprovedForAll(address owner, address operator) public view virtual override returns (bool) {
        return (_operatorApprovals[owner][operator] || _target.isApprovedForAll(owner, operator));
    }
