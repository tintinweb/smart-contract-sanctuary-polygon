    ...
    VRFConsumerBaseV2Upgradeable
{
    ...
    function initialize(address vrfCoordinator, uint64 subscriptionId) public initializer {
        __ERC721URIStorage_init();
        __ERC721_init("TablelandVRF", "TVr");
        __Ownable_init();
        ...
        vrf.COORDINATOR = VRFCoordinatorV2Interface(vrfCoordinator);
        
        vrf.keyHash = 0x4b09e658ed251bcafeebbc69400383d49f344ace09b9576fe248bb02c003fe9f;
        vrf.callbackGasLimit = 100000;
        vrf.requestConfirmations = 3;
        vrf.numWords = 1;
        vrf.s_subscriptionId = subscriptionId;
    }
        ...
        returns (uint256 requestId)
    {
        ...
        requestId = vrf.COORDINATOR.requestRandomWords(
            vrf.keyHash,
            vrf.s_subscriptionId,
    ...
    function requests() public view returns (uint256[] memory ids) {
        return vrf.requestIds;
    }

    function requestCount() public view returns (uint256) {
        return vrf.requestIds.length;
    }

    function showAtIndex(uint256 index) public view returns (RequestStatus memory result) {
        return s_requests[vrf.requestIds[index]];
    }

    function show(uint256 requestId) public view returns (RequestStatus memory result) {
        return s_requests[requestId];
    }

    function showLatest() public view returns (RequestStatus memory result) {
        return s_requests[vrf.requestIds[vrf.requestIds.length-1]];
    }
     ...
     */
    function _initMetadata() external onlyOwner() returns (uint256 tokenId) {
    ...
    /**
     * @dev Add a new NFT to the table
     */
    function _insertNFT(
        uint256 tokenId,
        string memory name,
        ...
        stored._tableland.runSQL(
            ...
            string.concat(
                "INSERT INTO ",
                stored._table,
                "(mint, image, name, attributes) VALUES (",
                tokenIdString,
                ",'",
                image,
                "','",
    ...
    /**
     * @dev this method will be called by the owner to request the random number
     * used to issue tokenIds randomly across the collection. 
     */
    function preReveal() public onlyOwner() returns (uint256 tokenId) {
        require(vrf.requestIds.length == 0, "Random word already requested.");
        return requestRandomWords();
    }

    function shuffle() public {
        require(vrf.requestIds.length > 0, "No random numbers requested.");
        require(s_requests[0].fulfilled == true, "Still awaiting random number from oracle.");

        uint256 index = 0;
        for (uint256 i = vrf.requestIds.length - 1; i >= 0; i -=1) {
            if (s_requests[vrf.requestIds[i]].exists == true && s_requests[vrf.requestIds[i]].fulfilled == true) {
                index = i;
            }
        }

        // 3 is just our collection size
        string memory offset = StringsUpgradeable.toString(s_requests[vrf.requestIds[index]].randomWords[0] % 3);
        // example, if the offset = 2, we'll just shift the ids by that offset
        // we stored a "mint" order column to use for the shifting
        // UPDATE tablename SET id = CASE WHEN mint<2 THEN 2+mint ELSE mint-2 END;
        stored._tableland.runSQL(
            address(this),
            stored._tableId,
            string.concat(
        ...
        // Queue the next random word for fun shuffling
        requestRandomWords();
    }
    ...
    function buy(uint256 tokenId) external payable {
        // Note: lock it down so you can't buy other people's tokens. 
        // require(stored.available[tokenId] == 1, "Token not available");

        // Note: lock it down with a min price
        // require(msg.value >= stored.price, "not enough ETH sent.");
        // if (msg.value - stored.price > 0) {
        //     payable(msg.sender).transfer(msg.value - stored.price);
        // }

        // Note: in this demo, anyone can buy anyone elses token for free!
        super._safeTransfer(ownerOf(tokenId), msg.sender, tokenId, "");
    }

    function safeMintMany(string[] memory names, string[] memory images) public onlyOwner() {
        require(names.length == images.length, "insufficient attributes");
        for (uint256 i = 0; i<names.length; i+=1) {
            safeMint(names[i], images[i]);
        }
    }

    function safeMint(string memory name, string memory image) public onlyOwner() {
        uint256 tokenId = _tokenIdCounter.current();
        _insertNFT(tokenId, name, image);
    ...
    */
    function tokenURI(
        ...
        if (stored.reveal) {
            return string.concat(
                "https://testnet.tableland.network/query?unwrap=true&s=SELECT%20*%20FROM%20canvas_5_4%20WHERE%20id=",
                strTokenId
            );
        } 
        // Provide a pre-reveal metadata payload.
        return 'data:application/json,{"name":"SECRET","image":"https://download.vikidia.org/vikidia/fr/images/thumb/c/c9/CC0.svg/768px-CC0.svg.png"}';
    }
     ...
     * used to issue tokenIds randomly across the collection. 
     */
    function tableland() public view returns (uint256 tableId, string memory tableName) {
        return (stored._tableId, stored._table);
    }
