...
contract FutToken is ERC20, ERC20Burnable, Pausable, ERC20Permit, AccessControlled {
    ...
    constructor(uint256 _startingWeek, address _authority) ERC20("Fut Token", "FUT") ERC20Permit("Fut Token") AccessControlled(IAuthority(_authority)) {
    ...
    {        
        if (from != address(0)) {
            require(balanceOf(funds[PRIVATE_SALE].fundAddress) - amount >= getUnlockedBalance(PRIVATE_SALE, getCurrentWeek()), "Private Sale Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[PUBLIC_SALE].fundAddress) - amount >= getUnlockedBalance(PUBLIC_SALE, getCurrentWeek()), "Public Sale Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[STAKING_REWARDS].fundAddress) - amount >= getUnlockedBalance(STAKING_REWARDS, getCurrentWeek()), "Staking Rewards Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[GAMING_ISSUANCE].fundAddress) - amount >= getUnlockedBalance(GAMING_ISSUANCE, getCurrentWeek()), "Gaming Issuance Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[SOCCER_CLUBS].fundAddress) - amount >= getUnlockedBalance(SOCCER_CLUBS, getCurrentWeek()), "Soccer Clubs Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[FUTSTER_TEAM].fundAddress) - amount >= getUnlockedBalance(FUTSTER_TEAM, getCurrentWeek()), "Futster Team Funds: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[PRE_LAUNCH_FUND].fundAddress) - amount >= getUnlockedBalance(PRE_LAUNCH_FUND, getCurrentWeek()), "Pre-Launch Fund: Cannot transfer more than unlocked amount");
            require(balanceOf(funds[ADVISORS].fundAddress) - amount >= getUnlockedBalance(ADVISORS, getCurrentWeek()), "Advisors Funds: Cannot transfer more than unlocked amount");
        }
    ...
    function setStartingWeek(uint256 _startingWeek) public  { // TODO: onlyGovernor
        startingWeek = _startingWeek;
    }
    ...
    function distributionSetup() internal {
        ...
        funds[PRIVATE_SALE].fundAddress = 0x19944ba8212e4e21e3B827b44a186261a2C4725e;
        funds[PUBLIC_SALE].fundAddress = 0x2BbE8460102221e44D374f133125E3Ff14a1F5bc;
        funds[STAKING_REWARDS].fundAddress = 0x3F16DE6a0a8F42c7d175f20905207ff162fcf8Eb;
        funds[GAMING_ISSUANCE].fundAddress = 0x43f5004c179E9DAb29A9a4cbDFe6C8A13d7BB0aB;
        funds[SOCCER_CLUBS].fundAddress = 0x535178a607C42Cdc7D56718dcC5C03398956C427;
        funds[FUTSTER_TEAM].fundAddress = 0x620EB4E48f8F8E6BD156D6572BD11a4699d7c84a;
        funds[PRE_LAUNCH_FUND].fundAddress = 0x785e9cEa2bb97a95b0A576f9ce1C1b187ccF14C3;
        funds[ADVISORS].fundAddress = 0x8CA2A08dfeD293A6fEe915886C7262bC5fFe5b07;
