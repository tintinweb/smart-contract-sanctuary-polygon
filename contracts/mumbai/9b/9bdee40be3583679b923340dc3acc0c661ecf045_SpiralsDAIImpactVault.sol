...
/**
 * @title SpiralsDAIImpactVault
 * @author @douglasqian
 * @notice Implementation of ImpactVault on the Polygon DAI.
 *   DAI deposited is deposited in Aave V3's DAI lending pool as collateral.
 *   No loan positions are taken out to ensure a good health factor
 *   and minimizing chances of automatic liquidation. In return, this contract
 *   receives a rebasing token (aDAI) that it then deposits into a
 *   wrapped token vault to receieve a non-rebasing yield bearing token.
 */
contract SpiralsDAIImpactVault is ImpactVault {
    using SafeERC20Upgradeable for IERC20Upgradeable;

    event Receive(address indexed sender, uint256 indexed amount);
    event DependenciesUpdated(address indexed moolaLendingPool);

    IERC20Upgradeable public c_DAI;
    IAaveLendingPool public c_aaveLendingPool;
    IERC4626Upgradeable public c_waDAI;
    ...
    function initialize(
        ...
        setDependencies(_aaveLendingPoolAddress, _daiAddress);

        // Ensures that `_aaveLendingPoolAddress` has been sanitized.
        c_DAI = IERC20Upgradeable(_daiAddress);
        c_waDAI = IERC4626Upgradeable(_waDAIAddress);
        require(
            c_waDAI.asset() == address(getADAI()),
            "NON_MATCHING_aDAI_ADDRESS"
        );

        __ERC20_init("Green DAI", "gDAI");
        __ImpactVault_init(
            IERC20Upgradeable(_daiAddress),
            IERC20Upgradeable(c_waDAI),
            _impactVaultManagerAddress
        );
    }

    /**
     * @notice Sets dependencies on contract (Aave contract addresses).
     */
    function setDependencies(
        address _aaveLendingPoolAddress,
        address _daiAddress
    ) public onlyOwner {
        c_aaveLendingPool = IAaveLendingPool(_aaveLendingPoolAddress);
        require(
            c_aaveLendingPool.getReserveData(_daiAddress).aTokenAddress !=
                address(0),
            "INVALID_AAVE_LENDING_POOL"
        );
        emit DependenciesUpdated(_aaveLendingPoolAddress);
    }

    /**
     * @dev Deposit DAI into Aave lending pool and convert aDAI to waDAI.
     */
    function _stake(uint256 _amountDAI)
        internal
        virtual
        override
        returns (uint256)
    {
        ...
        asset.approve(address(c_aaveLendingPool), _amountDAI);

        uint256 aDAIBefore = getADAI().balanceOf(address(this));
        c_aaveLendingPool.deposit(address(asset), _amountDAI, address(this), 0);
        uint256 aDAIReceived = getADAI().balanceOf(address(this)) - aDAIBefore;

        // Convert aDAI to waDAI.
        getADAI().approve(address(c_waDAI), aDAIReceived);
        uint256 waDAIReceived = c_waDAI.deposit(aDAIReceived, address(this));
        return convertToAsset(waDAIReceived);
    }

    /**
     * @dev Withdraws DAI from waDAI and then converts it to aDAI from
     * Aave lending pool into DAI to send to receiver.
     */
    function _withdraw(address _receiver, uint256 _amount)
        ...
        override
    {
        // Withdraw aDAI assets to this vault.
        uint256 aDAIBefore = getADAI().balanceOf(address(this));
        c_waDAI.withdraw(_amount, address(this), address(this));
        uint256 aDAIReceived = getADAI().balanceOf(address(this)) - aDAIBefore;

        // Burn aDAI to withdraw DAI to receiver.
        // withdraw here returns _amount, return value not needed
        // slither-disable-next-line unused-return
        c_aaveLendingPool.withdraw(address(asset), aDAIReceived, _receiver);
    }
    ...
    {
        return c_waDAI.convertToAssets(_amountYieldAsset);
    }
    ...
    {
        return c_waDAI.convertToShares(_amountAsset);
    }

    /**
     * @dev Returns the Aave DAI AToken contract (aDAI).
     */
    function getADAI() internal view returns (IERC20Upgradeable) {
        return
            IERC20Upgradeable(
                c_aaveLendingPool.getReserveData(address(c_DAI)).aTokenAddress
            );
    }
}
