...
contract CollectionFactory is IOmniApp {
    ...
    function create(string memory _dstChainName, string memory collName, string memory collURI, string memory fileURI, uint256 price, uint256 from, uint256 to, string[] memory URIs, uint gas) public payable {
        require(bytes(collName).length >= 2);
