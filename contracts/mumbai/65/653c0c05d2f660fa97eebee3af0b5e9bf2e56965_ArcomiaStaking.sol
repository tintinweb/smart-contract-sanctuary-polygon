...
import { ArcomiaStakingUpgradeable } from "./lib/ArcomiaStakingUpgradeable.sol";
import "@thirdweb-dev/contracts/interfaces/staking/IEditionStake.sol";

contract ArcomiaStaking is
    Initializable,
    ContractMetadata,
    PermissionsEnumerable,
    ERC2771ContextUpgradeable,
    MulticallUpgradeable,
    ArcomiaStakingUpgradeable,
    ERC165Upgradeable,
    IERC1155ReceiverUpgradeable,
    IEditionStake
{
    bytes32 private constant MODULE_TYPE = bytes32("ArcomiaStaking");
    ...
    function initialize(
        ...
        __Staking1155_init(_stakingToken, 1);

        _setupContractURI(_contractURI);
        _setupRole(DEFAULT_ADMIN_ROLE, _defaultAdmin);
        _setDefaultStakingCondition(0, 1, 1);
    }
...
import "@thirdweb-dev/contracts/extension/interface/IStaking1155.sol";

abstract contract ArcomiaStakingUpgradeable is ReentrancyGuardUpgradeable, IStaking1155 {
    /*///////////////////////////////////////////////////////////////
    ...
    uint8 internal isStaking;

    ///@dev Next staking condition Id. Tracks number of conditon updates so far.
    mapping(uint256 => uint256) private nextDefaultConditionId;
    ...
    mapping(uint256 => mapping(uint256 => StakingCondition)) private defaultCondition;
    ...
    /// @dev Emitted when contract admin updates rewardsPerUnitTime.
    event UpdatedCollectionRewardsPerUnitTime(uint256 collectionId, uint256 oldRewardsPerUnitTime, uint256 newRewardsPerUnitTime);

    function __Staking1155_init(address _stakingToken, uint8 _isStaking) internal onlyInitializing {
    ...
    function setTimeUnit(uint256 _tokenId, uint256 _timeUnit) external virtual {
        ...
        StakingCondition memory condition = _nextConditionId == 0
            ? defaultCondition[_collectionId][nextDefaultConditionId[_collectionId] - 1]
     ...
     */
    function setCollectionRewardsPerUnitTime(uint256 _collectionId, uint256 _defaultTimeUnit, uint256 _defaultRewardsPerUnitTime) external virtual {
        if (!_canSetStakeConditions()) {
            revert("Not authorized");
        }

        _setDefaultStakingCondition(_collectionId, _defaultTimeUnit, _defaultRewardsPerUnitTime);

        emit UpdatedCollectionRewardsPerUnitTime(_collectionId, _defaultTimeUnit, _defaultRewardsPerUnitTime);
    }
    ...
    function getDefaultTimeUnit(uint _collectionId) public view returns (uint256 _timeUnit) {
        _timeUnit = defaultCondition[_collectionId][nextDefaultConditionId[_collectionId] - 1].timeUnit;
    }

    function getDefaultRewardsPerUnitTime(uint _collectionId) public view returns (uint256 _rewardsPerUnitTime) {
        _rewardsPerUnitTime = defaultCondition[_collectionId][nextDefaultConditionId[_collectionId] - 1].rewardsPerUnitTime;
    }
    ...
    function _stake(uint256 _tokenId, uint256 _amount) internal virtual {
        ...
        if (stakers[_tokenId][_stakeMsgSender()].amountStaked > 0) {
            ...
            stakers[_tokenId][_stakeMsgSender()].conditionIdOflastUpdate = _conditionId == 0
                ? nextDefaultConditionId[_collectionId] - 1
                : _conditionId - 1;
        }
    ...
    function _setStakingCondition(
        ...
        if (conditionId == 0) {
            uint256 _nextDefaultConditionId = nextDefaultConditionId[_collectionId];
            for (; conditionId < _nextDefaultConditionId; conditionId += 1) {
                StakingCondition memory _defaultCondition = defaultCondition[_collectionId][conditionId];
    ...
    function _setDefaultStakingCondition(uint256 _collectionId, uint256 _timeUnit, uint256 _rewardsPerUnitTime) internal virtual {
        require(_timeUnit != 0, "time-unit can't be 0");
        uint256 conditionId = nextDefaultConditionId[_collectionId];
        nextDefaultConditionId[_collectionId] += 1;

        defaultCondition[_collectionId][conditionId] = StakingCondition({
            timeUnit: _timeUnit,
            rewardsPerUnitTime: _rewardsPerUnitTime,
            startTimestamp: block.timestamp,
            endTimestamp: 0
        ...
        if (conditionId > 0) {
            defaultCondition[_collectionId][conditionId - 1].endTimestamp = block.timestamp;
        }
    }
    ...
    function _calculateRewards(uint256 _tokenId, address _staker) internal view virtual returns (uint256 _rewards) {
        ...
        if (_nextConditionId == 0) {
            _nextConditionId = nextDefaultConditionId[_collectionId];

            for (uint256 i = _stakerConditionId; i < _nextConditionId; i += 1) {
                StakingCondition memory condition = defaultCondition[_collectionId][i];
