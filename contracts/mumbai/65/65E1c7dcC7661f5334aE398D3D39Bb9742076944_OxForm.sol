...
contract OxForm is Ownable, ERC1155, AccessControl, ISismoStructs {
    ...
    function formContribution(
        ...
        require(form.escrowAmount >= form.submitionReward, "no rewards to take");

        // Verifying Claims
        if (formRequiredClaims[_formID].length > 0) {
            sismoVerifier.verifySismoProofs(proofs, formRequiredClaims[_formID]);
        }
    ...
    function changeFormStatus(
        uint256 _formID,
        bool isMintable
    ) external onlyRole(getFormAdminRole(_formID)) {
        formInfo[_formID].mintable = isMintable;
    }

    function changeTreasuryAddress(
        uint256 _formID,
        address _newTokenTreasuryAddress
    ) external onlyRole(getFormAdminRole(_formID)) {
        FormInfo storage form = formInfo[_formID];
