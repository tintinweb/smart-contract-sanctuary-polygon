...
contract Wunderland is ERC721Enumerable, Ownable {
  ...
  uint256 private costsPubSale = 20 ether;
  uint256 private costsPreSale = 20 ether;
  uint256 private maxSupplyPubSale = 777;
  uint256 private maxSupplyPreSale = 700;
  uint256 private nftPerSessionLimit = 5;
  uint256 private nftPerWalletLimitPubSale = 5;
  uint256 private nftPerWalletLimitPreSale = 2;
  ...
  function mint(uint256 _mintAmount) public payable {
    ...
    if(isPubSale()) {
      ...
      mintCosts = costsPubSale * _mintAmount;
    } else {
      mintCosts = costsPreSale * _mintAmount;
      require(supply + _mintAmount <= maxSupplyPreSale, "max NFT presale limit exceeded");
      require(isWhitelisted(msg.sender), "caller is not whitelisted");
      require(ownerMintedCount + _mintAmount <= nftPerWalletLimitPreSale, "max NFT per address exceeded");
    }
  ...
  function addWhitelist(address[] memory _beneficiaries) external onlyOwner {
    for (uint256 i = 0; i < _beneficiaries.length; i++) {
      whitelist[_beneficiaries[i]] = true;
    }
  }

  function removeWhitelist(address[] memory _beneficiaries) external onlyOwner {
    for (uint256 i = 0; i < _beneficiaries.length; i++) {
  ...
  function setNftPerWalletLimitPreSale(uint256 _newNftPerWalletLimitPreSale) public onlyOwner {
    nftPerWalletLimitPreSale = _newNftPerWalletLimitPreSale;
  }
  
  function setCostsPubSale(uint256 _newPubSaleCost) public onlyOwner {
    costsPubSale = _newPubSaleCost;
  }

  function setCostsPreSale(uint256 _newPreSaleCost) public onlyOwner {
    costsPreSale = _newPreSaleCost;
  }
  ...
  function setMaxSupplyPreSale(uint256 _maxSupplyPreSale) public onlyOwner ifNotLocked {
