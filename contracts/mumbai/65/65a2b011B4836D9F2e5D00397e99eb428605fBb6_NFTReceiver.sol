...
// File contracts/NFT-Receiver.sol

// The NFT Receiver is on the SENDING side of the bridge
contract NFTReceiver is NonblockingLzApp {
    address public NFT_CONTRACT_ADDRESS;
    uint16 public DESTINATION_LZ_CHAIN_ID;
    address public DESTINATION_CONTRACT_ADDRESS;

    constructor(
        address _endpoint,
        address _nftContractAddress,
        uint16 _destLzChainId,
        address _destinationContractAddress
    ) NonblockingLzApp(_endpoint) {
        NFT_CONTRACT_ADDRESS = _nftContractAddress;
        DESTINATION_LZ_CHAIN_ID = _destLzChainId;
        DESTINATION_CONTRACT_ADDRESS = _destinationContractAddress;
    }

    // a method named bridgeNFt that takes in a nft id, transfers the nft to the contract, and calls lzSend
    function bridgeNFT(uint256 _nftId) public payable {
        ...
        bytes memory payload = abi.encode(_nftId, msg.sender);
        _lzSend(
