...
contract WineOracleTangible is AdminAccess, IPriceOracle, PriceConverter {
    ...
    event GBPToUSDUpdated(uint256 indexed oldRatio, uint256 indexed newRatio);
    ...
    uint256 public GBPToUSD = 140; //1.40 conversion ratio
    ...
    function converGBPToUSD(uint256 priceInGBP)
        ...
        returns (uint256)
    {
        return (priceInGBP * GBPToUSD) / 100;
    }
        ...
        )
    {
        ...
        uint256 _lockedAmount = (nft.lockPercent(localFingerprint) *
    ...
    function setGBPToUSD(uint256 _gbpToUsd) external onlyAdmin {
        emit GBPToUSDUpdated(GBPToUSD, _gbpToUsd);
        GBPToUSD = _gbpToUsd;
    }
