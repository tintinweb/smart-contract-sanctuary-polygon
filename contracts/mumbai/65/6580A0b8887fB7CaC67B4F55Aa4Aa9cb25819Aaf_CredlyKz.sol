...
contract CredlyKz is ERC721, ERC721Enumerable, ERC721URIStorage, Ownable {
    ...
    function isIcanMintNft() public view returns (bool) {
        return _userCanMintNft[msg.sender];
    }

    // Функция для проверки разрешения на майнинг NFT для пользователя
    function isUserCanMintNft(address userAddress)
        public
        view
        onlyAuthorized
        returns (bool)
    {
        return _userCanMintNft[userAddress];
    }
    ...
    function getMyNftCount() public view returns (uint256) {
        return _userNftCount[msg.sender];
    }

    function getUserNftCount(address userAddress)
        public
        view
        onlyAuthorized
        returns (uint256)
    {
        return _userNftCount[userAddress];
    }
    ...
    function isMyCurrentNftCountInSubscription() public view returns (uint256) {
        return _userCurrentNftCountInSubscription[msg.sender];
    }
    ...
    // Получение максимального количество нфт в рамках подписки, для пользователя вызывающего контракт
    function getMyMaxNftCountInSubscription() public view returns (uint256) {
        return _userMaxNftCountInSubscription[msg.sender];
    }
    ...
    // Получение максимального количество нфт, по адресу, доступное только авторизованным
    function getUserMaxNftCountInSubscription(address userAddress)
        public
        view
        onlyAuthorized
        returns (uint256)
    {
        return _userMaxNftCountInSubscription[userAddress];
    }

    //--------------------------Майнинг------------_userMintExpirationDate

    //Получение даты истечения подписки пользователя который вызвал контракт
    function getMyMintExpirationDate() public view returns (uint256) {
        return _userMintExpirationDate[msg.sender];
    }

    //Получение даты истечения подписки по адресу
    function getUserMintExpirationDate(address userAddress)
        public
        view
        onlyAuthorized
        returns (uint256)
    {
        return _userMintExpirationDate[userAddress];
    }
    ...
    // Функция для проверки про авторизации пользователя
    function isAuthorizedProUser(address user) public view returns (bool) {
        return _authorizedProUsers[user];
    }
    ...
    function burnToken(uint256 tokenId) public onlyAuthorizedPro {
