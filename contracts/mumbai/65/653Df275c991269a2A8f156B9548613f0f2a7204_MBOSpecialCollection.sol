...
contract MBOSpecialCollection is ERC721Opensea {
    ...
    uint256 public MINT_PRICE = 0.0000123 ether;
    ...
    // =============== MINTING ===============
    function genesisClaimMint(
        uint256 genesisTokenId,
        MintRequest calldata _req,
        bytes calldata _signature
    ) external returns (uint256 tokenId, string memory) {
        // Check if the genesis token is owned by the caller
        require(
            MBOGenesisNFTContract.ownerOf(genesisTokenId) == msg.sender,
            "Genesis NFT must be owned by the sender"
        );

        // Check if the genesis token has already been claimed
        require(
            !checkIsGenesisClaimed(genesisTokenId),
        ...
        // TODO: Verify mint time limit

        // Verify the signature that it's from the contract owner
    }
    ...
    function getPhygitalClaimDataByAddress(
        ...
        for (uint256 i = 0; i < totalClaimed(); i++) {
            if (phygitalClaimData[i].claimerAddress == claimerAddress) {
                allPhygitalClaimData[counter] = phygitalClaimData[i];
                counter++;
            }
        }
    ...
    /**
     * @dev Changes the price of a regular claim
     * @param newPrice The new price of a regular claim
     */
    function changeMintPrice(uint256 newPrice) external onlyOwner {
        MINT_PRICE = newPrice * 10 ** 18;
    }
...
contract SignatureVerifier is EIP712 {
    ...
    string private constant DOMAIN_NAME = "SignatureVerifier";
    string private constant DOMAIN_VERSION = "1.0.0";
    uint256 private constant CHAIN_ID = 1; // replace with your chain id
    ...
    function verify(
        ...
        return mintRequest.to == recoveredAddress;
    }
    ...
    constructor() EIP712("SignatureVerifier", "1") {}
}
