...
contract GabrielV3 is Initializable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    function compoundArcha(uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(!(pool.compoundDisabled), "compounding is temporarily disabled");
        require(!(user.compounded), "you have already compounded your earnings");
        uint256 NORT = pool.NORT;
        ...
        if (pending > 0) {
            user.compounded = true;
            stakeFrom(msg.sender, _pid, pending, true);
        }
    }

    function disableCompounding(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        pool.compoundDisabled = true;
    }

    function disableUnstake(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unstakeDisabled = true;
    }

    function enableCompounding(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        pool.compoundDisabled = false;
    }

    function enableUnstake(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unstakeDisabled = false;
    }

    function enableUnstakingAndCompounding(uint256 _pid) external onlyOwner {
        PoolInfo storage pool = poolInfo[_pid];
        pool.unstakeDisabled = false;
        pool.compoundDisabled = false;
    }
    ...
    function harvest(uint256 _pid, bool leaveRewards) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        ...
        require(user.amount > 0 && !(user.harvested), "Harvest: no available rewards to claim");
        pool.harvestList.push(msg.sender);
        update(_pid);
        if (!(pool.unstakeDisabled)) {
            disableUnstake(_pid);
            disableCompounding(_pid);
        }
        uint256 NORT = pool.NORT;
        uint256 arrayLength = user.nonHarvestedRewards.length;
        user.harvested = true;
    ...
    //     PoolInfo storage pool = poolInfo[_pid];
    //     uint argArrayLength = harvestList.length;
    //     uint arrayLength = pool.harvestList.length;
    //     for (uint i; i < argArrayLength; i++) {
    //         UserInfo storage user = userInfo[_pid][harvestList[i]];
    //         user.harvested = false;
    //         user.compounded = false;
    //     }

    //     address lastArgAddr = harvestList[argArrayLength-1];
    ...
    function stake(uint256 _pid, uint256 _amount) external {
        stakeFrom(msg.sender, _pid, _amount, false);
    }
     ...
     * @param _amount amount of tokens to be deposited by user
     */
    function stakeFrom(address _from, uint256 _pid, uint256 _amount, bool compound) internal {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        (bool inAnotherPool, uint256 pid) = checkIfAlreadyInAPool(_from);
        if (inAnotherPool) {
            require(
                pid == _pid,
        ...
        bool alreadyInAPool = inPool[_from][_pid];
        if (!alreadyInAPool) {
            inPool[_from][_pid] = true;
        }
        if (!compound) { update(_pid); }
        if ((pool.unstakeDisabled)) {
            enableUnstake(_pid);
            enableCompounding(_pid);
        }
        pool.stakeList.push(_from);
        user.amount = user.amount.add(_amount);
        pool.staked = pool.staked.add(_amount);
        emit Stake(_pid, _from, _amount);
        if (!compound) {
    ...
    function unstake(uint256 _pid) external {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(!(pool.unstakeDisabled), "unstaking is temporarily disabled");
    ...
    function update(uint256 _pid) internal {
        PoolInfo storage pool = poolInfo[_pid];
