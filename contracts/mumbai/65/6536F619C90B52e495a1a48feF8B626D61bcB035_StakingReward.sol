...
contract StakingReward is OwnerPausable {
    ...
    uint256 public constant RELEASE_CYCLE_TIME = 1 hours;
    ...
    constructor(address owner, uint256 secondsPerBlock) OwnerPausable(owner) {
        ...
        BASE_REWARD_PER_BLOCK = secondsPerBlock*PRECISION_FACTOR/365 days;
    }
    ...
    function depositMock(uint256 index, uint256 amount) external {
        ...
        if (_userInfo[staker].amount > 0) {
            ...
            for (uint256 i; i < _orders[staker].length; i++) {
                uint256 multiplier = _getMultiplier(user.lastRewardBlock, block.number, 
                        _orders[staker][i].addedBlock+6*RELEASE_CYCLE_TIME);
                pendingRewards += multiplier*apy[user.level]*BASE_REWARD_PER_BLOCK/PRECISION_FACTOR;
            }
        ...
        OrderInfo[] storage orders_ = _orders[staker];
        orders_.push(OrderInfo(index, block.number, amount));
    }

    function calculatePendingRewards(address staker) external view returns(UserInfo memory) {
        ...
        if (_userInfo[staker].amount > 0) {
            ...
            for (uint256 i; i < _orders[staker].length; i++) {
                uint256 multiplier = _getMultiplier(user.lastRewardBlock, block.number, 
                        _orders[staker][i].addedBlock+6*RELEASE_CYCLE_TIME);
                pendingRewards += multiplier*apy[user.level]*BASE_REWARD_PER_BLOCK;
            }
            user.rewardDebt += pendingRewards;
        }
        return user;
     ...
     function calculatePendingRewards2(address staker) external view returns(uint256 pendingRewards) {
        UserInfo memory user = _userInfo[staker];
        if (_userInfo[staker].amount > 0) {
            for (uint256 i; i < _orders[staker].length; i++) {
                uint256 multiplier = _getMultiplier(user.lastRewardBlock, block.number, 
