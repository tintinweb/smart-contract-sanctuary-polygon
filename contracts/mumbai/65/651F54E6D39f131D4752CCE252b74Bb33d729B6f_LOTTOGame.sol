...
contract LOTTOGame is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function fulfillRandomWords(
        ...
        require(gameResult[gameId].status == 1, "Game Invalid");
        gameResult[gameId].status = 2;
        for (uint256 _index = 0; _index < 6; _index++) {
            gameResult[gameId].randomWords.push((_randomWords[_index] % 50) + 1);
        }
        ...
        for (uint256 k = 0; k < gameResult[gameId].randomWords.length; k++) {
            for (uint256 j = 1; j < gameResult[gameId].randomWords.length; j++) {
