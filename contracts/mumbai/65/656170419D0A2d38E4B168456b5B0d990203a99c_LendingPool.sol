...
contract LendingPool is Ownable, ReentrancyGuard {
    ...
    function getPools(
        ...
        PoolInfo[] memory _pools = new PoolInfo[](
            offset + limit > poolCount ? poolCount : limit
        );
        for (uint256 i = offset; i < poolCount && i < offset + limit; i++) {
            _pools[i] = pools[i];
        }
        return _pools;
    }
        ...
        )
    {
        ...
        RecordInfo[] memory _records = new RecordInfo[](
            offset + limit > _recordCount ? _recordCount : limit
        );
        uint256[] memory _withdrawableAmount = new uint256[](
            offset + limit > _recordCount ? _recordCount : limit
        );
        for (uint256 i = offset; i < _recordCount && i < offset + limit; i++) {
            RecordInfo memory _recordInfo = userRecords[userAddr].recordMap[i];
            _records[i] = _recordInfo;
            PoolInfo memory _poolInfo = pools[_recordInfo.poolId];
            uint256 _rateSum = rateSumList[i - offset];
            _withdrawableAmount[i] = _getWithdrawableAmountRaw(
                _poolInfo.depositToken,
                _poolInfo.rewardToken,
                _recordInfo.balance,
                _recordInfo.depositTime,
