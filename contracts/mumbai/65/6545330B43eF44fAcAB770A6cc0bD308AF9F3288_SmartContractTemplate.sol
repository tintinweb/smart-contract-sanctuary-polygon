...
// File: contracts/MyContractCentralized.sol
...
contract SmartContractTemplate is ERC721Enumerable, Ownable, ReentrancyGuard {
    function getLatestPrice() public view returns (uint256) {
        ...
        // Mainnet Polygon
        // ) = AggregatorV3Interface(0x7bac85a8a13a4bcd8abb3eb7d6b4d632c5a57676).latestRoundData();
    ...
    uint256 public totalRaise = 10000;
    ...
    address private constant _holderAddress = 0xA774427c1fA5B0Ab7cCa28b0d8045546C3e4193F;
    // ERC721URIStorage.sol
    // Optional mapping for token URIs
    mapping (uint256 => string) private _tokenURIs;
    constructor() payable ERC721("Baby Supe", "BABYSUPE") {
        // HAVE TO MANUALLY SET ROYALTY AFTER DEPLOYMENT
        // _setRoyalties(msg.sender, __ROYALTY__);
        // resetMintPrice();
    }
    ...
    function _mintHandler(uint256 nftId, address to) internal {
        mintedWallets[to]++;
        // totalSupply++;
    ...
    function mintBatch(
        ...
        require(
            (to == owner() || to == _holderAddress || mintedWallets[to] + number <= 5) && maxSupply > totalSupply(),
        ...
        require(
            _hashTransaction(to, number, nonce, typeId, nftIds) == hash && !_usedNonces[nonce],
        ...
        if (msg.value != 0) {
            payable(0xA774427c1fA5B0Ab7cCa28b0d8045546C3e4193F).transfer(msg.value);
        }
    ...
    /// @notice Mint to centralized wallet
    function centralizedWallet(
        uint256 number,
        string memory nonce,
        uint256[] memory nftIds
    ) external payable nonReentrant callerIsUser {
        for (uint256 i = 0; i < nftIds.length; ++i) {
            _mintHandler(nftIds[i], _holderAddress);
        }
    }
