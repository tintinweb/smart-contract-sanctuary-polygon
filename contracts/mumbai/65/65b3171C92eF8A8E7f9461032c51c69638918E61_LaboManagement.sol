...
contract LaboManagement is ERC721Holder, Ownable{
    ...
    uint256 public potionsStandardPrice = 100 * 1E18;
    ...
    modifier onlyLaboOwner(uint256 _laboId) {
        require(ILaboratory(gameAddresses.getLaboratoryNFTAddress()).ownerOf(_laboId)==msg.sender);
        _;
    }
    ...
    function setPotionStandardPrice(uint256 _price) external onlyOwner {
        potionsStandardPrice = _price;
    }
    ...
    function addWorkingSpotToLab(uint256 _laboId) external onlyLaboOwner(_laboId) returns(bool) {
        ...
        require(payWithRewardOrWallet(msg.sender, _paymentAddress,workingSpotPrice));
        IPayments(_paymentAddress).distributeFees(workingSpotPrice);

        return I.updateNumberOfWorkingSpots(_laboId);
    }

    function workInASpot(uint256 _zaiId, uint256 _laboId, uint256 _spotId) external {
        address _zaiAddress = gameAddresses.getZaiAddress();
        require(IERC721(_zaiAddress).ownerOf(_zaiId) == msg.sender,"Not your zai");
        require(IZaiNFT(_zaiAddress).isFree(_zaiId),"Not Free");
        require(ILaboratory(gameAddresses.getLaboratoryNFTAddress()).numberOfWorkingSpots(_laboId) >=_spotId, "Not enough spots");
        WorkInstance storage w = workingSpot[_laboId][_spotId];
        require(w.zaiId == 0 || block.timestamp > w.beginingAt + 1 days, "Spot not free");
        if(block.timestamp < w.beginingAt + 1 days + 600){
            require(
                IOracle(gameAddresses.getOracleAddress()).
                    getRandom(keccak256(abi.encodePacked(_zaiId,block.timestamp,msg.sender))) % 100
                <= 5,
                "Bot prevention only 5% chance in those 10 min");
        }

        _updateCredits(_laboId);

        if(w.zaiId != 0){
            _updateZai(w.zaiId,block.timestamp - w.beginingAt);
        }else{
            employees[_laboId] = employees[_laboId] + 1;
        }
        w.zaiId = _zaiId;
        w.beginingAt = block.timestamp;
        IZaiNFT(gameAddresses.getZaiAddress()).updateStatus(_zaiId,3,_laboId);
    }

    function stopWorking(uint256 _zaiId, uint256 _laboId, uint256 _spotId) external {
        require(IERC721(gameAddresses.getZaiAddress()).ownerOf(_zaiId) == msg.sender,"Not your zai");
        WorkInstance storage w = workingSpot[_laboId][_spotId];
        require(w.zaiId == _zaiId,"Not your zai");
        _updateZai(_zaiId,block.timestamp - w.beginingAt);
        w.beginingAt = 0;
        w.zaiId = 0;
        employees[_laboId] = employees[_laboId] - 1;
        _updateCredits(_laboId);
    ...
    function _updateZai(uint256 _zaiId, uint256 _xpAlchemy) internal {
        IZaiNFT I = IZaiNFT(gameAddresses.getZaiAddress());
        I.updateStatus(_zaiId,0,0);
        I.updateAlchemyXp(_zaiId,_xpAlchemy / 1000); //1pts = 1000 seconds
    }

    function buyStandardPotion(uint256 _potionType) public returns(uint256){
        require(_potionType == 5 || _potionType == 6, "Doesn't exist");
        address _paymentAddress = gameAddresses.getPaymentsAddress();
        
        require(payWithRewardOrWallet(msg.sender, _paymentAddress,potionsStandardPrice));
        IPayments(_paymentAddress).distributeFees(potionsStandardPrice);

        return IPotions(gameAddresses.getPotionAddress()).buyPotion(msg.sender,_potionType);
    }
    ...
    function _getCredit(uint256 _laboId) internal view returns (uint256) {
        uint256 _credits;
        uint256 _creditLastUpdate = ILaboratory(gameAddresses.getLaboratoryNFTAddress()).getCreditLastUpdate(_laboId);
        ...
        if(_creditLastUpdate == 0){
            ...
            if(employees[_laboId] > 0){
                _credits = _potionsCredits[_laboId] + 
                (_timePassed * employees[_laboId]);
                if(_credits > maxCredit){
                    _credits = maxCredit;
                }
            }else{
                _credits = _potionsCredits[_laboId] + (_timePassed / 4);
            }
    ...
    function createAndSellPotion(
        ...
        require(_type < 5,"not good potion");
        require(IOpenAndClose(gameAddresses.getOpenAndCloseAddress()).canLaboSell(_laboId), "You can't");
        _updateCredits(_laboId);
        require(_potionsCredits[_laboId] >= (_power * pointCreditCost), "Not enough credits");
        _potionsCredits[_laboId] = _potionsCredits[_laboId] - (_power * pointCreditCost);
        createdPotionsForLab[_laboId] = createdPotionsForLab[_laboId] + 1;

        uint256 potionId = IPotions(IAddresses(gameAddresses).getPotionAddress()).mintPotionForSale(_laboId,_price,_type,_power);
    ...
    function changePotionPrice(uint256 _potionId, uint256 _laboId, uint256 _price) external onlyLaboOwner(_laboId) returns(bool){
       return IPotions(gameAddresses.getPotionAddress()).changePotionPrice(_potionId, _laboId, _price);
    }

    function getUnsoldPotions(uint256 _laboId) external view returns(PotionStruct.Potion[] memory){
        uint256 unSoldNumber = EnumerableSet.length(laboUnsoldPotions[_laboId]);
        PotionStruct.Potion[] memory unSoldPotions = new PotionStruct.Potion[](unSoldNumber);
    ...
    function getLast10soldPotions(uint256 _laboId) external view returns(PotionStruct.Potion[] memory){
        uint256 soldNumber = EnumerableSet.length(laboSoldPotions[_laboId]);
        PotionStruct.Potion[] memory soldPotions = new PotionStruct.Potion[](10);
        IPotions potions = IPotions(gameAddresses.getPotionAddress());
        ...
        if(soldNumber == 0){
            ...
            for (uint256 i = 0 ; i < soldNumber ;){
                uint256 potionId = EnumerableSet.at(laboSoldPotions[_laboId],i);
            ...
            for (uint256 i = soldNumber - 10 ; i < soldNumber ; ){
                uint256 potionId = EnumerableSet.at(laboSoldPotions[_laboId],i);
                soldPotions[soldNumber - i - 1] = potions.getFullPotion(potionId); 
                unchecked{ ++i; }
            }
            return soldPotions;
        }
    ...
    function offerPotion(
        ...
        require(IOpenAndClose(gameAddresses.getOpenAndCloseAddress()).canLaboSell(_laboId), "You can't");
        _updateCredits(_laboId);
        require(_potionsCredits[_laboId] >= (_power * 2 * pointCreditCost), "Not enough credits");
        _potionsCredits[_laboId] = _potionsCredits[_laboId] - (_power * 2 * pointCreditCost);
        createdPotionsForLab[_laboId] = createdPotionsForLab[_laboId] + 1;

        return IPotions(IAddresses(gameAddresses).getPotionAddress()).offerPotion(_type,_power,_to);

    }

    function buyPotion(uint256 _potionId) public {
        address _potionAddress = gameAddresses.getPotionAddress();
        require(IERC721(_potionAddress).ownerOf(_potionId) == address(this), "Not in sale");
        (address _seller, uint256 _price, uint256 _fromLab) = IPotions(_potionAddress).getPotion(_potionId); 
        address _paymentAddress = gameAddresses.getPaymentsAddress();

        require(payWithRewardOrWallet(msg.sender, _paymentAddress,_price));
        IPayments(_paymentAddress).payOwner(_seller, _price);

        require(IPotions(_potionAddress).updatePotionSaleTimestamp(_potionId));

        laboratoryRevenues[_fromLab] = laboratoryRevenues[_fromLab] + _price;
        EnumerableSet.remove(laboUnsoldPotions[_fromLab],_potionId);
        EnumerableSet.add(laboSoldPotions[_fromLab],_potionId);

        IERC721(_potionAddress).transferFrom(address(this),msg.sender, _potionId);
    }
    ...
    function cleanSlotsBeforeClosing(uint256 _laboId) external returns(bool){
        ...
        if(employees[_laboId] == 0 ){
            ...
            uint256 numberOfSpots = ILaboratory(gameAddresses.getLaboratoryNFTAddress()).numberOfWorkingSpots(_laboId);
    ...
    function _updateCredits(uint256 _laboId) internal {
        ...
        require(ILaboratory(gameAddresses.getLaboratoryNFTAddress()).updateCreditLastUpdate(_laboId));
