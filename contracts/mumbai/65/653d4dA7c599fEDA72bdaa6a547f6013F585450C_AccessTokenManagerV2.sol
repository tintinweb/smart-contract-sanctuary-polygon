...
contract AccessTokenManagerV2 is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    {   
        ...
        Utils(utilsContract).validatePurchaseV2(
            Utils.PurchaseDataV2(msg.sender, msg.value ,channelId, accessTokenContract, accessToken)
        );
        ...
        whenNotPaused
    {
        ...
        Utils(utilsContract).validateInitAccessTokenV2(
            Utils.InitAccessTokenDataV2(
...
import "./AccessTokenManagerV2.sol";
...
contract Utils {
    ...
    struct InitAccessTokenDataV2{
        ...
        string uriMetadata;
        string communityId;
        AccessTokenManagerV2.AccessTokenItem item;
    }
    ...
    /// @param data InitAccessTokenData struct    
    /// @notice validation for init access token operation
    function validateInitAccessTokenV2(
        InitAccessTokenDataV2 memory data)
        external pure
    {
        require(bytes(data.channelId).length != 0, "ERR-1-ATM");
        require(data.creator != address(0), "ERR-8-ATM");
    ...
    /// @param data PurchaseData struct    
    /// @notice validation for purchase operation
    function validatePurchaseV2(
        PurchaseDataV2 memory data)
        external view
    {
