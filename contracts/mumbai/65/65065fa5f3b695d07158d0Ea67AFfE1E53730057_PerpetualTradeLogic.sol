...
contract PerpetualTradeLogic is PerpetualUpdateFunctions, IFunctionList, IPerpetualTradeLogic {
    ...
    function _preTrade(
        ...
        bool closePos = !_hasTheSameSign(fTraderPos, _fAmount) && fTraderPos != 0;
    ...
    /**
     * Get the price for a given trade amount
    ...
    {
        PerpetualData storage perpetual = _getPerpetual(_iPerpetualId);
        return _queryPriceFromAMM(perpetual, _fTradeAmountBC);
    }
        ...
        )
    {
        ...
        if (!_hasOpened) {
            ...
            penaltyTbps = _getAMMPerpLogic().holdingPeriodPenalty(
                block.number - mgn.iLastOpenBlock,
...
contract PerpetualUpdateFunctions is PerpetualBaseFunctions {
    ...
    int128 constant LAMBDA_SPREAD_CONVERGENCE = 14151956096107433984; // 0.7671790772159579 * 2^64
    ...
    function _getUpdatedTargetAMMFundSize(
        ...
        // account for AMM margin cash
        {
            int128 fMarginCashCC = marginAccounts[_perpetual.id][address(this)].fCashCC;
            fMStar = fMStar > fMarginCashCC ? fMStar.sub(fMarginCashCC) : int128(0);
        }
        return fMStar;
    }
    ...
    function _calculateVolatilitySpread(PerpetualData storage _perpetual)
        ...
        returns (int128)
    {
        uint256 numBlockSinceJump = block.number - _perpetual.iLastPriceJumpBlock;
        ...
        returns (bool isMarketClosed)
    {
        ...
        if (fPrice > 0) {
            ...
            if (fPrice != _perpetual.fSettlementS2PriceData) {
                ...
                if (_perpetual.iLastSettlementPriceUpdateBlock + 2 > block.number) {
                    int128 fJump = fPrice
                    ...
                    if (fJump > fCurrSpread.add(fCurrSpread)) {
                        ...
                        _perpetual.iLastPriceJumpBlock = uint64(block.number);
                    }
                }
                _perpetual.fSettlementS2PriceData = fPrice;
            }
            _perpetual.iLastSettlementPriceUpdateBlock = uint64(block.number);
        } else {
            isMarketClosed = true;
        }
