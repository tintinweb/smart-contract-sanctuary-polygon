 ...
 *         Initial Contract found at Vectorized/closedsea/src/example/ExampleERC721A.sol
 */

contract IWCmint is ERC721AQueryable, ERC721ABurnable, OperatorFilterer, Ownable, ERC2981 {
    bool public operatorFilteringEnabled;
    string internal baseTokenURI;
    uint256 internal maxSupply = 100;
    uint256 internal cost = 0.5 ether;
    bool internal paused = true;

    constructor() ERC721A("International Waters Collection", "IWC") {
    ...
    /// mint function
    function mint(uint256 _mintAmount, address destination) external payable {
        if (_mintAmount == 0) revert minNFTSRequired();
        if (paused) revert contractPaused();
        uint256 supply = totalSupply();
        if (supply + _mintAmount > maxSupply) revert maxSupplyReached();
        if (owner() != msg.sender) {
            if (msg.value != (cost * _mintAmount)) revert incorrectFunds();
        }

        _mint(destination, _mintAmount);
    }
