...
contract Collectible is Ownable, ERC1155 {
  using Strings for uint256;

  string public constant name = "Collectible";
  ...
  event RafflePurchased(address indexed account, uint256 indexed blockId, uint256 quantity);
  ...
  function addIssue(IssueFactory memory issue) public {
    ...
    require(issues[issue.tokenId].t == CollectibleType.Null && issues[issue.tokenId / 100].t == CollectibleType.Null, "Token already exists");
    require(issue.permissions.length < 5 && issue.permissions.length > 0, "Permissions must have up to 4 elements");

    if (issue.t == CollectibleType.Packdrop || issue.t == CollectibleType.Raffle) {
  ...
  function purchase(uint256 tokenId, uint256 amount) public payable {
    ...
    require(msg.value >= issue.cost * amount, "Not enough ETH");

    issue.currentSupply += amount;
    if (issue.t == CollectibleType.Packdrop) {
  ...
  function compress(uint256[] memory list) public pure returns (bytes32) {
  ...
  function uri(uint256 tokenId) public pure override returns (string memory) {
    return string(abi.encodePacked("https://octopus-app-du49r.ondigitalocean.app/metadata/collectibles/", tokenId.toString()));
  }
}
