...
// File: Template/ERC721A/TokyuEgg.sol
/**
...
contract TOKYUPLAZASHIBUYA is ERC721AUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable {

  // DANGER!! Don't Change the state variable orders when create upgrade smart contract
  ...
  // DANGER!! Initialize only once on the first smart contract deployments
  function initialize(string memory _tokenName, string memory _tokenSymbol, uint256 _maxSupply)  initializer  public {
    __ERC721A_init(
   ...
   function safeTransferFrom(
        ...
        require(!locked, "This NFT can't be transferred!");
  ...
  function bulkMint(address[] calldata _to, uint256[] calldata _id) public {
    require(hasRole(ADMIN, msg.sender), 'Only admin can use this function');
    require(!paused, 'The contract is paused!');
  ...
  function setLocked(bool _state) public  {
    require(hasRole(ADMIN, msg.sender), 'Only admin can use this function');
    locked = _state;
  }
