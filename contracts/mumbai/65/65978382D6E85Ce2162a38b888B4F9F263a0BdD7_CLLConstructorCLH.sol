...
contract CLLConstructorCLH is CLStorage {
	...
	function __CLHConstructor(
        ...
        external
    {
        require( _govRuleMaxUsers > 0, "Invalid user number" );
        govRuleApprovPercentage = 100;
        govRuleMaxUsers = _govRuleMaxUsers;
        govRuleMaxManagers = 1;
        houseName = _houseName;
        HOUSE_GOVERNANCE_MODEL = __GOV_DICTATORSHIP__;
        housePrivate = _housePrivate;
        houseOpen = _housePrivate ? false : _houseOpen;
        pxyCLF = ICLFactory( _CLC[ uint256( eCLC.pxyCLF ) ] );
        pxyApiCLH = CLHouseApi( _CLC[ uint256( eCLC.pxyApiCLH ) ] );
        pxyNFTManager = CLHNFT( _CLC[ uint256( eCLC.pxyNFTManager ) ] );
        pxyNFTMember = CLHNFT( _CLC[ uint256( eCLC.pxyNFTMember ) ] );
        pxyNFTInvitation = CLHNFT( _CLC[ uint256( eCLC.pxyNFTInvitation ) ] );
        ...
        arrUsers.push( address(0) );
        
        (bool successDGTCLL, bytes memory dataDLGTCLL) = pxyCLF.CLLUserManagement().delegatecall(
