...
contract MoaMarketplaceDonor is IMoaMarketplaceDonor, ReentrancyGuard, Ownable {
    ...
    mapping(address => mapping(address => mapping(uint256 => uint256))) public getItemIdFromAddress;
    mapping(address => bool) private nftContractWhitelist;
    ...
    modifier isUnqiueOnDonateItem(
        ...
        uint256 _itemId = getItemIdFromAddress[_nftAddress][spender][_tokenId];
        ...
        require(
            ...
            (_donateItem.donateItemId != 0 && _donateItem.complete != false) || // is already complete or revoked
            (_donateItem. donateItemId != 0 && _donateItem.revoke != false),
            "already exists"
        );
        _;
    }

    function _getDonationFinishTime(uint256 _donationDays) private view returns (uint64) {
        return uint64(block.timestamp + (_donationDays * 1 days));
    }
        ...
        returns (uint256)
    {
        ...
        require(nftContractWhitelist[nftContract] == true, "Not whitelisted NFT contract");

        // effect
        _donateItemIdCounter.increment(); // 1-based, so increment first
        uint256 _donateItemId = _donateItemIdCounter.current();
        uint64 _donationFinishTime = _getDonationFinishTime(donationDays);
        ...
        idToDonorItem[_donateItemId] = DonateItem({
            ...
            donationFinishTime: _donationFinishTime,
            complete: false,
            revoke: false
        });

        getItemIdFromAddress[nftContract][address(_msgSender())][tokenId] = _donateItemId;
        ...
        nonReentrant
    {
        ...
        bool revoke = idToDonorItem[itemId].revoke;
        uint64 donationFinishTime = idToDonorItem[itemId].donationFinishTime;
        require(complete != true && revoke != true, "Sale closed");
    ...
    {
        donateItemId = getItemIdFromAddress[_nftContract][IERC721(_nftContract).ownerOf(_tokenId)][_tokenId];
    }
    ...
    /// @dev set whitelisted NFT contract address 
    function setWhitlelistContract(address _nftContract, bool _approved) public onlyOwner {
        nftContractWhitelist[_nftContract] = _approved;
    }
}
...
interface IMoaCultivation is IMoaLendCultivation {
    ...
    event RestingEndTimeUpdate(uint256 indexed moaId, uint256 indexed restingCooldownEndTime);
