...
contract VwapOracle is Ownable, IVwapOracle {
    ...
    function _find(address _tokenAddr) internal view returns(uint256 i, bool exists) {
        i = 0;
        while (tokenToWatchPrices[i] != _tokenAddr) {
            i++;
        }

        if (i != tokenToWatchPrices.length) exists = true;
    }

    function _removeByIndex(uint256 _index) internal {
        require(_index < tokenToWatchPrices.length, "index out of bound");

        for (uint i = _index; i < tokenToWatchPrices.length - 1; i++) {
            tokenToWatchPrices[i] = tokenToWatchPrices[i + 1];
        }
        tokenToWatchPrices.pop();
    }

    function addToken(address _tokenAddr) external {
        (, bool exists) = _find(_tokenAddr);
        require(!exists, "VWAPcomponent: token already exists");
        tokenToWatchPrices.push(_tokenAddr);
    }

    function removeToken(address _tokenAddr) external onlyOwner {
        (uint256 i, bool exists) = _find(_tokenAddr);
        require(exists, "VWAPcomponent: token doesn't exists");
