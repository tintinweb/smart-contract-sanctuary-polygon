...
contract SheeshaVotes is ISheeshaVotes, ContextUpgradeable {
    ...
    function setVaults(bytes calldata data_) external override onlyDao {
        _setVaults(data_);
    }

    function _setVaults(bytes calldata data_) internal {
        require(data_.length == 0x40, "SHV: !data");
        (address SHVault_, address LPVault_) =
            abi.decode(data_, (address, address));
        __setVaults(SHVault_, LPVault_);
    }
        ...
        returns (uint256 amount)
    {
        ...
        if (unlocked_ > 0) {
            ...
            amount = ((10**_pairDecimals) * unlocked_ * 1e16) / lpPrice;
        }
    }
        ...
        returns (uint256 shPrice, uint256 lpPrice)
    {
        ...
        (uint256 reserveSH, uint256 reservePaired) =
            SHToken == pair.token0()
                ? _getReserves1(pair)
                : _getReserves2(pair);
        if (reserveSH > 0 && reservePaired > 0) {
    ...
    function __setVaults(address SHVault_, address LPVault_) private {
        SHVault = SHVault_;
        LPVault = LPVault_;
        SHToken = ISheeshaVaultInfo(SHVault_).token();
        LPToken = ISheeshaVaultInfo(LPVault_).token();
        IUniswapV2Pair pair = IUniswapV2Pair(LPToken);
        (_shDecimals, _stDecimals, _pairDecimals) = SHToken == pair.token0()
            ? _tokenDecimals1()
            : _tokenDecimals2();
        ...
        )
    {
        ...
        return (
            ...
            IERC20Meta(address(pair)).decimals()
        );
    }
    ...
    {
        (stDecimals, shDecimals, pairDecimals) = _tokenDecimals1();
    }
}
...
interface ISheeshaDao {
    ...
    event VotingCancelled(address canceller, address voting, string reason);
...
interface ISheeshaVoting is ISheeshaDaoInitializable {
    ...
    function cancel(address[] calldata members, address payable caller)
        external;
