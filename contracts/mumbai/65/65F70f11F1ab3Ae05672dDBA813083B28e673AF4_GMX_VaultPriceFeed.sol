...
import "./interfaces/GMX_IVaultPriceFeed.sol";
import "../oracle/interfaces/GMX_IPriceFeed.sol";
import "../oracle/interfaces/GMX_ISecondaryPriceFeed.sol";
import "../oracle/interfaces/GMX_IChainlinkFlags.sol";
import "../amm/interfaces/GMX_IPancakePair.sol";

pragma solidity 0.6.12;

contract GMX_VaultPriceFeed is GMX_IVaultPriceFeed {
    ...
    function getPrimaryPrice(address _token, bool _maximise) public override view returns (uint256) {
        ...
        if (chainlinkFlags != address(0)) {
            bool isRaised = GMX_IChainlinkFlags(chainlinkFlags).getFlag(FLAG_ARBITRUM_SEQ_OFFLINE);
            if (isRaised) {
        ...
        GMX_IPriceFeed priceFeed = GMX_IPriceFeed(priceFeedAddress);
    ...
    function getSecondaryPrice(address _token, uint256 _referencePrice, bool _maximise) public view returns (uint256) {
        ...
        return GMX_ISecondaryPriceFeed(secondaryPriceFeed).getPrice(_token, _referencePrice, _maximise);
    }
    ...
    function getPairPrice(address _pair, bool _divByReserve0) public view returns (uint256) {
        (uint256 reserve0, uint256 reserve1, ) = GMX_IPancakePair(_pair).getReserves();
        if (_divByReserve0) {
