...
contract PixelynNft is DefaultOperatorFilterer, ReentrancyGuard, Context, IERC165, IERC721, IERC721Metadata, IERC2981 {
    ...
    modifier isPixelynAdmin() {
        require(
            IPixelynNftDeployer(_pixelynNftDeployer).getAdminState(_msgSender()), 'PixelynNft: caller is not a admin'
        );
        _;
    }
    ...
    function setMintMaxSupply(uint256 newMintMaxSupply) external isPixelynAdmin {
        require(block.timestamp < _mintStartTimestamp, 'PixelynNft: Cannot update max supply after mint has started');
        require(totalSupply() == 0, 'PixelynNft: Cannot update max supply after tokens have been minted');
    ...
    function approve(address operator, uint256 tokenId) public virtual override onlyAllowedOperatorApproval(operator) {
        ...
        require(operator != owner, 'PixelynNft:approval to current owner');
    ...
    function reserve(address to, uint256 index) public isPixelynAdmin {
        ...
        require(_unusedTokensLeft > 0, 'PixelynNft: no tokens left for minting');
    ...
    function mintRandom(address to, uint256 numToMint) public payable nonReentrant {
        ...
        require(!_killSwitch, 'PixelynNft: minting is paused');
        require(to != address(0), 'PixelynNft: mint to the zero address');
        require(numToMint > 0, 'PixelynNft: need to mint at least one token');
        require(_unusedTokensLeft >= numToMint, 'PixelynNft: minting more tokens than supply');
        require(msg.value == _price * numToMint, 'PixelynNft: Did not send correct amount of funds');
    ...
    function airdrop(address[] calldata recipients) public isAirdropper {
        require(recipients.length > 0, 'PixelynNft: empty recipient list');
        require(_unusedTokensLeft >= recipients.length, 'PixelynNft: minting more tokens than supply');
    ...
    function _setBeneficiaries(SharedStructs.Beneficiary[] memory beneficiaries) internal {
        ...
        require(beneficiaries.length > 0 && beneficiaries.length < 4, 'PixelynNft: invalid number of beneficiaries');
        ...
        for (uint256 i; i < beneficiaries.length; i++) {
            SharedStructs.Beneficiary memory beneficiary = beneficiaries[i];
            require(beneficiary.percentage <= MAX_BPS, 'PixelynNft: beneficiary percentage is too high');
            _beneficiaries.push(beneficiary);
            totalPercentage += beneficiary.percentage;
        }

        // Ensure that the total percentage adds up to 100
        require(totalPercentage == MAX_BPS, 'PixelynNft: total percentage must be 100');
    }
    ...
    function _setGlobalRoyalties(address newRecipient, uint256 newPercentage) internal {
        require(newRecipient != address(0), 'PixelynNft: royalties recipient is the zero address');
        require(
            newPercentage <= MAX_BPS && newPercentage > 0,
            'PixelynNft: royalty must be less than or equal to 100% and greater than 0%'
        );
    ...
    function setTokenRoyalty(uint256 tokenId, address receiver, uint256 royaltyPercentage) external isPixelynAdmin {
        require(receiver != address(0), 'PixelynNft: royalties new recipient is the zero address');
        require(royaltyPercentage <= MAX_BPS, 'PixelynNft: roken royalty must be higher then 100%');
