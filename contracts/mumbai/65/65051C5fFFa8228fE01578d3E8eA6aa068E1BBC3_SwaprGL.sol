...
contract SwaprGL is BaseGovernanceWithUserUpgradable, ListingHelper {
    ...
    function _payFromWallet(PayNow memory _payOps, uint256 depositType) internal returns(bool) {
        ...
        if(_payOps.acceptedToken == address(0)){
            ...
            if(_payOps.toEOA){
                swaprWallet.releaseERC(_payOps.acceptedToken, _payOps.receiver, _payOps.receiver, _payOps.amount - finalListingFee);
            }
        }
        return true;
    }
    ...
    function buyNowOrder(uint buyerPurchasedAmount, bytes memory data, uint split) public payable onlyMarketplace(data) {
        ...
        require(split > 0 && split <= EXP && split <= order.remainingPart && split <= order.maxBuyPerWallet, "INC_SPLIT_AMT");
    ...
    function buyNowAuction(bytes memory data) public payable onlyMarketplace(data) {
...
contract ListingHelper {
    ...
    returns(
        ...
        if(swaprWallet.isNFTLocked(auction.lock, auction.nftId)){
            ...
            require(auction.startTime + timeOffset <= auction.endTime && auction.endTime >= block.timestamp + timeOffset, "INV_TIME");
            isValid = true;
        }
    ...
    returns(
        ...
        require(order.maxTokenSell > 0 && order.maxTokenSell <= EXP &&
            order.maxBuyPerWallet > 0 && order.maxBuyPerWallet <= EXP, "INV_DATA");
        if(swaprWallet.isNFTLocked(order.lock, order.nftId)){
            require(order.activeDepositType == 0 || order.activeDepositType == 3, "ALR_LISTED");
            isValid = true;
        }else{
            require(order.depositType == 2, "INV_DEP_TYPE");
    ...
    returns(
        ...
        if(
            !_verify(
                ...
                abi.encode(
                    listingInfo, 
                    swaprWallet.getNFT(bid.lock, bid.nftId)
                )
    ...
    returns(
        ...
        require(
            ...
            _verify(activeBid.bidder, activeBidData), "INV_SIG");
    ...
    function _validateClaim(address claimant, bytes memory listingData, bytes memory bidData) internal view returns(bool isValid, string memory res) {
        ...
        if(!_verify(auction.seller, listingData) || !_verify(lastBid.bidder, bidData) || (block.timestamp < lastBid.listingEndTime)) {
            res = "INV_DATA";
            return (false, res);
        }
        if(_verify(claimant, listingData)){
