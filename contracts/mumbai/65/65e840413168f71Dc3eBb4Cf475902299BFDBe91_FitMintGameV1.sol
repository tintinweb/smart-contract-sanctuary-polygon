...
contract FitMintGameV1 is Ownable, ReentrancyGuard {
    ...
    //allow admin to transfer tokens or change holdings of users
    bool public isAdminActive = false;
    bool public isReserveAdminActive = false;
    ...
    uint8 public stakingPenaltyPerc = 0;
    uint8 public stakingCooldownDays = 0;
    ...
    function setReserveAdminAddress(address _reserveAdminAddress) public onlyOwner {
        emit AdminAddressChanged(reserveAdminAddress, _reserveAdminAddress);
        reserveAdminAddress = _reserveAdminAddress;
    }

    function setAdminActive(bool _isAdminActive) public onlyOwner {
        isAdminActive = _isAdminActive;
    }

    function setReserveAdminActive(bool _isReserveAdminActive) public onlyOwner {
        isReserveAdminActive = _isReserveAdminActive;
    }
    ...
    function setStakingPenaltyPerc(uint8 _stakingPenaltyPerc) external onlyOwner {
        stakingPenaltyPerc = _stakingPenaltyPerc;
    }

    function setStakingCooldownDays(uint8 _stakingCooldownDays) external onlyOwner {
    ...
    function transferMatic(uint256 _amount, address _to) public nonReentrant{
        ...
        require(isAdminActive, "Functions for Admin Account is inactive");
    ...
    function reserveMatic(address _to, uint256 _amount) public nonReentrant{
        ...
        require(isReserveAdminActive, "Functions for Reserve Admin Account is inactive");
    ...
    function reserveTransferMatic(uint256 _amount, address _to) public nonReentrant{
        ...
        require(isAdminActive, "Functions for Reserve Admin Account is inactive");
        require(_to != address(0), "Cannot transfer Matic to the zero address");
        require(address(this).balance >= _amount, "Insufficient Matic in the Game Contract");
    ...
    function reserveStaking(address _to, uint256 _stakedAmount) public nonReentrant{
        require(msg.sender == reserveAdminAddress, "Only Admin Address can reserve staking balances");
        require(isReserveAdminActive, "Functions for Reserve Admin Account is inactive");
        require(IERC20(tokenAddress).balanceOf(_to) >= _stakedAmount, "Cannot Stake more than In Game Balance of the User");
