 ...
 */
contract FlexibleStaking is Ownable, ReentrancyGuard, Pausable {
    ...
    constructor(
        ...
        uint256 startDay = (startStakeTime - (startStakeTime % DAYEPOCH)) / DAYEPOCH;
        uint256 endDay = (endStakeTime - (endStakeTime % DAYEPOCH)) / DAYEPOCH;
    ...
    modifier beforeStakeStarted() {
        require(
            ...
            "[FlexibleStaking.beforeStakeStarted] Staking period started"
        );
        _;
    }
    ...
    modifier afterStakeStarted() {
        require(
            block.timestamp >= startStakeTime,
            "[FlexibleStaking.afterStakeStarted] Staking period not yet started"
        );
        _;
    }
    ...
    modifier afterStakeEnded() {
        require(
            ...
            "[FlexibleStaking.afterStakeEnded] Staking period not yet ended"
        );
        _;
    }
    ...
    receive() external payable {
        revert("[FlexibleStaking] Revert receive function.");
    }

    fallback() external payable {
        revert("[FlexibleStaking] Revert fallback function.");
    }
        ...
        nonReentrant
    {
        require(
            ...
            "[FlexibleStaking.stakeNaka] total stake amount should be less than user limit"
        );
        require(
            _nakaAmount + poolStakeTotal <= poolStakeLimit,
            "[FlexibleStaking.stakeNaka] total stake amount should be less than pool limit"
        );
        IERC20 token = IERC20(nakaTokenAddress);
        token.safeTransferFrom(msg.sender, address(this), _nakaAmount);
    ...
    function withdrawNaka() external afterStakeEnded nonReentrant {
        ...
        require(
            0 == unclaimAmount,
            "[FlexibleStaking.withdrawNaka] unclaim amount should be zero before user can withdraw"
        );
        ...
        whenNotPaused
    {
        ...
        require(
            ...
            "[FlexibleStaking.claimReward] claim amount should be less than user unclaim amount"
        );
    ...
    /**
     * @dev Function for retrieve current APR.
     */
    function getCurrentAPR()
        ...
        returns (uint256)
    {
        return (poolReward * 365 * 1000000) / poolStakeTotal / duration;
    }
        ...
        returns (uint256)
    {
        ...
        uint256 currentDay = (block.timestamp - (block.timestamp % DAYEPOCH)) /
            DAYEPOCH;
        uint256 startDay = (startStakeTime - (startStakeTime % DAYEPOCH)) / DAYEPOCH;
     ...
     */
    function increaseDay(uint256 _day) external {
        startStakeTime -= DAYEPOCH * _day;
        endStakeTime -= DAYEPOCH * _day;
    }
}
