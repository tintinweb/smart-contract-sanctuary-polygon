...
contract PublicGovSale is Initializable, IPublicGovSale, PausableUpgradeable, OwnableUpgradeable {
  ...
  uint128 public calculation; // cutoff time in unix
  ...
  uint256 public candidateTotalStaked;
  ...
  mapping(uint240 => mapping(address => uint256)) public userChainStaked; // user staked amount each chain
  ...
  mapping(uint240 => mapping(address => mapping(uint128 => uint256))) public chainUserReceivedPerRound; // user received amount each chain per round
  ...
  function totalRaised() external view virtual returns (uint256 totalRaisedAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      totalRaisedAmount += chainRaised[uint240(factory.allChainsStaked(i))];
    }
  }

  function totalRevenue() external view virtual returns (uint256 totalRevenueAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      totalRevenueAmount += chainRevenue[uint240(factory.allChainsStaked(i))];
    }
  }
   ...
   */
  function whitelistLength() external view virtual returns (uint256) {
  ...
  function chainUsersLength(uint240 _chainID) external view virtual returns (uint256 chainUsersLengthAmount) {
    chainUsersLengthAmount = chainUsers[_chainID].length;
  }

  /**
   * @dev Get all buyers/users length
   */
  function totalUsersLength() external view virtual returns (uint256 totalUsersLengthAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      totalUsersLengthAmount += chainUsers[uint240(factory.allChainsStaked(i))].length;
    }
  }
   ...
   */
  function remoteUsersLength() public view virtual returns (uint256 remoteUsersLengthAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      uint256 chainID = factory.allChainsStaked(i);
      if (chainID == block.chainid) continue;
      remoteUsersLengthAmount += chainUsers[uint240(chainID)].length;
    }
  }
   ...
   */
  function userHistoryLength(address _user) external view virtual returns (uint256 userHistoryLengthAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      userHistoryLengthAmount += chainInvoices[uint240(factory.allChainsStaked(i))][_user].length;
    }
  }
   ...
   */
  function userTotalStaked(address _user) public view virtual returns (uint256 userTotalStakedAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      userTotalStakedAmount += userChainStaked[uint240(factory.allChainsStaked(i))][_user];
    }
  }

  /**
   * @dev Get total purchase/received of a user
   * @param _user User address
   */
  function totalChainUserReceived(address _user) public view virtual returns (uint256 totalChainUserReceivedAmount) {
    uint256 chainStakedLength = factory.allChainsStakedLength();
    for (uint256 i = 0; i < chainStakedLength; ++i) {
      for (uint128 j = 1; j <= 4; ++j) {
        totalChainUserReceivedAmount += chainUserReceivedPerRound[uint240(factory.allChainsStaked(i))][_user][j];
      }
    }
  }
  ...
  function remoteUserData(
    uint240 _chainID,
    address _user
  ) external view virtual returns (uint256 index, uint256 bought, uint256 received, uint256 fee) {
  ...
  function calcUserAllocation(address _user, uint128 _boosterRunning) public view virtual returns (uint256 userAlloc) {
    ...
    uint256 userStakedToken = userTotalStaked(_user);
    ...
    if (_boosterRunning == 1) {
      ...
      if (uint128(block.timestamp) >= booster[2].start) {
        userAlloc = SaleLibrary.calcAllocFromKom(userStakedToken, totalStaked(), saleAmount - booster[1].tokenAchieved);
      }
  ...
  function _amountInCalc(
    ...
    uint256 purchaseThisRound = chainUserReceivedPerRound[_chainID][_user][_running];
   ...
   */
  function moveFund(uint128 _percent_d2, bool _devAddr, address _target) external virtual {
    _isEligible();
   ...
   */
  function buyToken(uint240 _chainID, uint256 _amountIn, address _user) external virtual whenNotPaused {
    ...
    if (running == 1) require(candidateTotalStaked > 0, '!voteStaked');

    if (sender == address(saleGateway) && running == 4 && paused()) {
    ...
    chainUserReceivedPerRound[_chainID][_user][running] += tokenReceivedFinal;
  ...
  function teamBuy(uint256 _tokenAmount) external virtual whenNotPaused {
    ...
    chainUserReceivedPerRound[uint240(block.chainid)][_msgSender()][running] += _tokenAmount;
  ...
  function _refund(address _from, address _to) internal virtual {
    ...
    uint256 amount = (uint256(totalChainUserReceived(_from)) * uint256(price) * uint256(_refund_d2)) / 1e22; // 1e18 (token decimal) + 1e4 (percent 2 decimal)
  ...
  function setUserChainStaked(
    ...
    for (uint256 i = 0; i < _user.length; ++i) {
      if (userChainStaked[_chainID][_user[i]] > 0) continue;

      userChainStaked[_chainID][_user[i]] = _stakedAmount[i];
      chainStaked[_chainID] += _stakedAmount[i];
    }
  }

  function resetUserChainStaked(uint240 _chainID, address[] calldata _user) external virtual onlyOwner {
    _isNotStarted();
    _isCalculated();
    ...
    for (uint256 i = 0; i < _user.length; ++i) {
      ...
      chainStaked[_chainID] -= userChainStaked[_chainID][_user[i]];
      delete userChainStaked[_chainID][_user[i]];
    }
  }
  ...
  function migrateCandidates(
    ...
    require(
      _users.length == _signatures.length && _users.length == _votedAt.length && block.timestamp > voteEnd,
      'bad'
    );
    address komV = factory.komV();

    uint256 candidateStaked = candidateTotalStaked;

    for (uint256 i = 0; i < _users.length; ++i) {
      if (
        isVoteValid[_users[i]] ||
        !verify(_users[i], _signatures[i]) ||
        (IERC20Upgradeable(komV).balanceOf(_users[i]) == 0 && userTotalStaked(_users[i]) == 0) ||
        _votedAt[i] < voteStart ||
        _votedAt[i] > voteEnd
      ) continue;

      candidateStaked += userTotalStaked(_users[i]);
