...
// event emmited by the contract
    function checkIn(uint256[] calldata sentinelIds, uint256[] calldata elderIds, uint256 artifactsAmount, uint256 renAmount, address _owner, uint256 chain) public returns (bool) {
        ...
        function transferTokensIn(uint256[] calldata tokenAmounts, bytes[] memory tokenSignatures, uint256[] calldata timestamps, address[] calldata owners, uint256[] calldata tokenIndex,  uint256 chain) public returns (bool) {
        ...
        chain == 1 ? isPlayer() : onlyOperator();
                ...
                for(uint i = 0; i < owners.length; i++){
                    ...
                    require(_isSignedByValidator(encodeTokenForSignature(tokenAmounts[i], owners[i], timestamps[i]),tokenSignatures[i]), "incorrect signature");
    ...
    function encodeTokenForSignature(uint256 tokenAmount, address owner, uint256 timestamp) public pure returns (bytes32) {
                return keccak256(
                        abi.encodePacked("\x19Ethereum Signed Message:\n32", 
                            keccak256(
                                    abi.encodePacked(tokenAmount, owner, timestamp))
                                    )
                                );
    }  
