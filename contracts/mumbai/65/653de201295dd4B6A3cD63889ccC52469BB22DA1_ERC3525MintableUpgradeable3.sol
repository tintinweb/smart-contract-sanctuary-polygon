...
contract ERC3525MintableUpgradeable3 is Initializable, ContextUpgradeable, ERC3525Upgradeable {

    string baseURI;
    uint public constant NORMAL = 1;
    uint public constant SPECIAL = 2;
    uint public constant UNIT = 10;

    uint mintTokenNum = 1001;
    ...
    // from slot1 to slot2
    function changefromNormalToSpecial(
        uint256 tokenId_,
        uint256 value_
    ) public{
        require(slotOf(tokenId_) == NORMAL, "slot ID is different");
        require(balanceOf(tokenId_) >= value_, "too short");
        require(ownerOf(tokenId_) == msg.sender, "this is not yours");
        uint modValue = value_ % UNIT;
        uint changeValue = value_ - modValue;
        _burnValue(tokenId_, changeValue);
        mint(msg.sender, mintTokenNum, SPECIAL, changeValue/UNIT);
        mintTokenNum++;
    }

    // from slot2 to slot1
    function changefromSpecialToNormal(
        uint256 fromTokenId_,
        uint256 toTokenId_,
        uint256 value_
    ) public{
        require(slotOf(fromTokenId_) == SPECIAL, "slot ID is different");
        require(slotOf(toTokenId_) == NORMAL, "slot ID is different");
        require(ownerOf(fromTokenId_) == msg.sender, "this is not yours");
        _burn(fromTokenId_);
        mintValue(toTokenId_, value_*UNIT);
    }
