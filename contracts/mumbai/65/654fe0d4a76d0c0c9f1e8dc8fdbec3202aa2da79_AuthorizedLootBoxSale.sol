...
interface ILootBoxNFT is IERC721Upgradeable {
    function mint(address _to, uint8 _tier) external;
}

contract AuthorizedLootBoxSale is
    Initializable,
    PausableUpgradeable,
    ...
    GenericTypedMessage
{
    ...
    bytes32 private constant _AUTHORIZED_ACCOUNT_TYPEHASH = keccak256("Authorization(address signer,address account,uint256 deadline,bytes32 authorization)");
    ...
    ILootBoxNFT public nft;
    IERC20Upgradeable public paymentToken;
    address public treasury;
    address public authority;
    Offer[] public offers;
    // buyer => authorized
    mapping(address => bool) public authorizedList;
    // buyer => qty
    mapping(address => uint256) public bought;
    uint256 public supply;
    uint256 public maxBoxesPerAccount;
        ...
        public
    {
        ...
        require(address(_paymentToken) != address(0), "Invalid payment token address");
        require(_operator != address(0), "Invalid operator address");
        require(_treasury != address(0), "Invalid treasury address");
        require(_authority != address(0), "Invalid authority address");
        require(_offers.length > 0, "Invalid tier list");
        ...
        treasury = _treasury;
        maxBoxesPerAccount = _offers.length;
    ...
    /**
     * @dev keeping the authorized accounts list in case we want
     * to enable direct buys
     */
    function batchAdd(
        address[] calldata _authorizedAccounts
    )
        external
        onlyRole(ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _authorizedAccounts.length; i++) {
            authorizedList[_authorizedAccounts[i]] = true;
        }
    }
    ...
    function updateMaxBoxesPerAccount(
        uint256 _newAmount
    )
        external
        onlyRole(ADMIN_ROLE)
    {
        maxBoxesPerAccount = _newAmount;
    }
    ...
    function buy(
        ...
        whenNotPaused
    {
        require(authorizedList[msg.sender], "Account not authorized");
        require(supply > 0, "Supply exhausted");
        require(bought[msg.sender] < maxBoxesPerAccount, "Max boxes per account reached");
        require(_verifySignature(_authorization), "Invalid authorization");
        ...
        paymentToken.transferFrom(msg.sender, treasury, offer.price);

        nft.mint(msg.sender, offer.boxTier);
    }
    ...
    function getOffers() external view returns (Offer[] memory) {
        Offer[] memory o = new Offer[](offers.length);
