...
contract CLFactory is ICLFactory, Initializable {
        ...
        public
    {
        ...
        if( _signerWallet != address(0) ) {
            ...
            ownerWallet = _signerWallet;
        }

        checkAccessNFT( ownerWallet );

        CLPBeacon pxyCLH = new CLPBeacon(
            address( beaconCLH ),
            abi.encodeWithSignature(
                "Init(address,string,string,bool,bool,uint256,address[7])",
                ownerWallet, 
                _ownerName,
                _houseName,
                _housePrivate,
        ...
        pxyNFTManager.forceMint( address( pxyCLH ), ownerWallet );
...
contract CLANFT is ERC721CL, Initializable {
    ...
    function holdersList() external view returns ( address[] memory arrHolders) {
        arrHolders = new address[] ( nftID+1 );
        for (uint256 idx = 1; idx <= nftID; idx++)
            arrHolders[ idx ] = __ownerOf[ idx ];
    }
}