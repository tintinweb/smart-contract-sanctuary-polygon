...
contract ChinaChicClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    function withdraw(address account, uint256 amount) public onlyOwner {
        ...
        ERC20(token).transferFrom(address(this), account, amount);
    }
    ...
    function hashMessage(address sender, string memory nonce)
        public
        view
        returns (bytes32)
    {
        bytes32 hash = keccak256(abi.encodePacked(sender, nonce, verifier));
        return hash;
    }
    ...
    function claim(
        ...
        require(matchSigner(hash, signature), "Address doesn't own nft");
        require(!nonces[nonce], "Hash reused");
        require(hashMessage(_msgSender(), nonce) == hash, "Hash failed");
        ...
        ERC20(token).transferFrom(address(this), msg.sender, remaining);
    }
}
