...
contract Router is Context {
    ...
    function token2ReceiveAmount(uint256 token1Amount)
        public
        view
        returns (uint256)
    {
    ...
    function token1ReceiveAmount(uint256 token2Amount)
        public
        view
        returns (uint256)
    {
    ...
    function swap(
        ...
        require(
            desc.srcToken == token1 || desc.srcToken == token2,
            "Unsupported fromToken"
        );
        require(
            desc.dstToken == token1 || desc.dstToken == token2,
            "Unsupported toToken"
        );
...
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import {Context} from "@openzeppelin/contracts/utils/Context.sol";
import {Router, SwapDescription} from "./Router.sol";

contract RouterAdapter is Context {
    ...
    function swap(
        ...
        desc.srcToken.approve(address(router), amount);
        return
