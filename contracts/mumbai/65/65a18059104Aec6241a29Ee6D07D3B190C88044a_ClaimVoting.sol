...
contract ClaimVoting is IClaimVoting, Initializable, AbstractDependant {
    ...
    IClaimVotingFacade public claimVotingFacade;

    // vote index -> results of calculation
    mapping(uint256 => VotesUpdatesInfo) public voteResults;
    ...
    modifier onlyClaimVotingFacade() {
        require(msg.sender == address(claimVotingFacade), "CV: Not ClaimVotingFacade");
        _;
    }
        ...
        onlyInjectorOrZero
    {
        ...
        claimVotingFacade = IClaimVotingFacade(_contractsRegistry.getClaimVotingFacadeContract());
    }

    function _isVoteAwaitingReception(uint256 voteIndex) internal view returns (bool) {
        uint256 claimIndex = _allVotesByIndexInst[voteIndex].claimIndex;

        return
            _allVotesByIndexInst[voteIndex].status == VoteStatus.EXPOSED_PENDING &&
            !claimingRegistry.isClaimPending(claimIndex);
    }

    function _isVoteAwaitingExposure(uint256 voteIndex) internal view returns (bool) {
        uint256 claimIndex = _allVotesByIndexInst[voteIndex].claimIndex;

        return (_allVotesByIndexInst[voteIndex].status == VoteStatus.ANONYMOUS_PENDING &&
            claimingRegistry.isClaimExposablyVotable(claimIndex));
    }

    function _isVoteExpired(uint256 voteIndex) internal view returns (bool) {
        uint256 claimIndex = _allVotesByIndexInst[voteIndex].claimIndex;
    ...
    function getVoteStatus(uint256 voteIndex) public view override returns (VoteStatus) {
        ...
        } else if (_isVoteAwaitingExposure(voteIndex)) {
            return VoteStatus.AWAITING_EXPOSURE;
        } else if (_isVoteExpired(voteIndex)) {
            return VoteStatus.EXPIRED;
        }

        return _allVotesByIndexInst[voteIndex].status;
    }

    function getVoteIndex(uint256 claimIndex, address user)
        external
        view
        override
        returns (uint256)
        ...
        returns (uint256 _claimsCount, PublicClaimInfo[] memory _votablesInfo)
    {
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            uint256 claimIndex = claimingRegistry.pendingClaimIndexAt(i);
            ...
            if (
                ...
                (!claimingRegistry.isClaimAppeal(claimIndex) || trustedVoter)
            ) {
                IClaimingRegistry.ClaimInfo memory claimInfo =
                    claimingRegistry.claimInfo(claimIndex);

                _votablesInfo[_claimsCount].claimIndex = claimIndex;
                _votablesInfo[_claimsCount].claimProvenance = claimInfo.claimProvenance;
    ...
    function listClaims(
        ...
        if (listOption == ListOption.ALL) {
            ...
            count = claimingRegistry.countBookClaimerClaims(msg.sender);
        }
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            uint256 claimIndex;
            if (listOption == ListOption.ALL) {
                claimIndex = claimingRegistry.claimIndexAt(i);
            } else if (listOption == ListOption.MINE) {
                claimIndex = claimingRegistry.claimOfOwnerIndexAt(msg.sender, i);
            }

            IClaimingRegistry.ClaimInfo memory claimInfo = claimingRegistry.claimInfo(claimIndex);

            uint256 newIndex = i.uncheckedSub(offset);
            _claimsInfo[newIndex].publicClaimInfo.claimIndex = claimIndex;
            _claimsInfo[newIndex].publicClaimInfo.claimProvenance = claimInfo.claimProvenance;
            ...
            if (_claimsInfo[newIndex].finalVerdict == IClaimingRegistry.ClaimStatus.ACCEPTED) {
                _claimsInfo[newIndex].finalClaimAmount = _votings[claimIndex]
                    .votedAverageWithdrawalAmount;
            }

            if (claimingRegistry.canClaimBeCalculatedByAnyone(claimIndex)) {
                _claimsInfo[newIndex].bmiCalculationReward = _getBMIRewardForCalculation(
                    claimIndex
                );
            }
        }
    ...
    /// @dev getter for FE to class votes on "my votes" page
    function getVotingPower(uint256 voteIndex) external view returns (uint256) {
        return
            _allVotesByIndexInst[voteIndex].voterReputation.mul(
        ...
        returns (MyVoteInfo[] memory _myVotesInfo)
    {
        ...
        for (uint256 i = offset; i < to; i = uncheckedInc(i)) {
            ...
            _myVotesInfo[newIndex].allClaimInfo.publicClaimInfo.claimProvenance = claimInfo
                .claimProvenance;
            ...
            _myVotesInfo[newIndex].suggestedAmount = _allVotesByIndexInst[voteIndex]
                .suggestedAmount;
            _myVotesInfo[newIndex].status = getVoteStatus(voteIndex);
            ...
            if (_myVotesInfo[newIndex].status == VoteStatus.ANONYMOUS_PENDING) {
                ...
                _myVotesInfo[newIndex].encryptedVote = _allVotesByIndexInst[voteIndex]
                    .encryptedVote;
                _myVotesInfo[newIndex].time = claimInfo
                    .dateSubmitted
        ...
        returns (VotesUpdatesInfo memory _myVotesUpdatesInfo)
    {
        ...
        if (_isVoteExpired(voteIndex)) {
            bmiPenaltyAmount = calculateExpiredVote(claimIndex, msg.sender);
            ...
            if (
                ...
                (stblAmount, bmiAmount, newReputation) = calculateMajorityYesVote(
                    claimIndex,
                    msg.sender,
                    oldReputation
                );
                ...
                (bmiAmount, newReputation) = calculateMajorityNoVote(
                    claimIndex,
                    msg.sender,
                    oldReputation
                );
                ...
                (bmiPenaltyAmount, newReputation) = calculateMinorityVote(
                    claimIndex,
                    msg.sender,
                    oldReputation
                );
    ...
    ) external override onlyClaimVotingFacade {
    ...
    ) external view override onlyClaimVotingFacade {
        bytes32 finalHash =
            keccak256(
                abi.encodePacked(
                    hashedSignaturesOfClaim,
    ...
    function calculateAverages(
        ...
        uint256 stakedStkBMI = _allVotesByIndexInst[voteIndex].stakedStkBMIAmount;
        uint256 reputationWithPrecision = _allVotesByIndexInst[voteIndex].voterReputation;

        if (votedFor) {
            uint256 votedPower = _votings[claimIndex].votedYesStakedStkBMIAmountWithReputation;
            ...
            _votings[claimIndex].votedYesStakedStkBMIAmountWithReputation = totalPower;
        } else {
            _votings[claimIndex].votedNoStakedStkBMIAmountWithReputation = _votings[claimIndex]
                .votedNoStakedStkBMIAmountWithReputation
                .add(stakedStkBMI.mul(reputationWithPrecision));
        }

        _votings[claimIndex].allVotedStakedStkBMIAmount = _votings[claimIndex]
            .allVotedStakedStkBMIAmount
            .add(stakedStkBMI);
    }
    ...
    function modifyExposedVote(
        ...
        _allVotesByIndexInst[voteIndex].finalHash = 0;
        delete _allVotesByIndexInst[voteIndex].encryptedVote;

        if (isConfirmed) {
            _allVotesByIndexInst[voteIndex].suggestedAmount = suggestedClaimAmount;
            _allVotesByIndexInst[voteIndex].accept = accept;

            _allVotesByIndexInst[voteIndex].status = VoteStatus.EXPOSED_PENDING;
        } else {
            _votings[claimIndex].voteIndexes.remove(voteIndex);
            _myNotReceivedVotes[voter].remove(claimIndex);
            _allVotesByIndexInst[voteIndex].status = VoteStatus.REJECTED;
        }
    }
    ...
    {
        reward = _getBMIRewardForCalculation(claimIndex);
    ...
    function updateVotedYesPercentage(uint256 claimIndex) external override onlyClaimVotingFacade {
    ...
    {
        allVotedStakedStkBMI = _votings[claimIndex].allVotedStakedStkBMIAmount;
        votedYesPercentage = _votings[claimIndex].votedYesPercentage;
        votedAverageAmount = _votings[claimIndex].votedAverageWithdrawalAmount;
    }
    ...
    {
        suggestedAmount = _allVotesByIndexInst[voteIndex].suggestedAmount;
    }
        ...
        )
    {
        ...
        if (claimingRegistry.claimStatus(claimIndex) == IClaimingRegistry.ClaimStatus.ACCEPTED) {
            ...
            _stblReward = info.reinsuranceTokensAmount.mul(voterRatio).uncheckedDiv(
                PERCENTAGE_100
            );
        } else {
            // calculate BMI reward tokens sent to the voter (from 1% locked)
            _bmiReward = info.lockedBMIAmount.mul(voterRatio).uncheckedDiv(PERCENTAGE_100);
        }
    ...
    function calculateMajorityNoVote(
        ...
        address voter,
        uint256 oldReputation
    ) public view override returns (uint256 _bmiReward, uint256 _newReputation) {
        VotingResult storage info = _votings[claimIndex];
    ...
    function calculateMinorityVote(
        ...
        address voter,
        uint256 oldReputation
    ) public view override returns (uint256 _bmiPenalty, uint256 _newReputation) {
        ...
        returns (uint256 _bmiPenalty)
    {
        ...
        _bmiPenalty = _allVotesByIndexInst[voteIndex].stakedStkBMIAmount;
    }
    ...
    function modifyReceivedVote(
        ...
        _allVotesByIndexInst[voteIndex].status = voteStatus;
        _myNotReceivedVotes[voter].remove(claimIndex);
    }

    function transferBMILocked(uint256 claimIndex, address claimer)
        external
        override
        onlyClaimingRegistry
    {
        ...
        _transferBMI(claimer, lockedAmount);
    }

    function transferBMIReward(address voter, uint256 bmiReward)
        external
        override
        onlyClaimVotingFacade
    {
        _transferBMI(voter, bmiReward);
    }

    function _transferBMI(address receiver, uint256 bmiAmount) internal {
        bmiToken.transfer(receiver, bmiAmount);
    }

    // TRY TO GROUP THOSE ONES into list getters
    function getVotingInfo(uint256 claimIndex)
        external
        view
        returns (uint256 countVoteOnClaim, uint256 lockedBMIAmount)
    {
