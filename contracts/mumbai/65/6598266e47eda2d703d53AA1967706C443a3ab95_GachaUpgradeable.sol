...
import { IGachaUpgradeable } from "./interfaces/IGachaUpgradeable.sol";
import { BaseUpgradeable } from "./internal-upgradeable/BaseUpgradeable.sol";
import { GachaTypeUpgradeable } from "./internal-upgradeable/GachaTypeUpgradeable.sol";
import { FeeSplitterUpgradeable } from "./internal-upgradeable/FeeSplitterUpgradeable.sol";
import { BaseUpgradeable } from "./internal-upgradeable/BaseUpgradeable.sol";
import { RandomSelectionUpgradeable } from "./internal-upgradeable/RandomSelectionUpgradeable.sol";
...
contract GachaUpgradeable is
    IGachaUpgradeable,
    BaseUpgradeable,
    ReentrancyGuardUpgradeable,
    GachaTypeUpgradeable,
    RandomSelectionUpgradeable,
    FeeSplitterUpgradeable
{
    ...
    function initialize(
        ...
        __BusinessAddressUpgradeable_init_unchained(operators_);
        __FeeSplitterUpgradeable_init_unchained(recipients_, percents_);
        __GachaType_init_unchained(types_, typeInfos_);
    }

    function buyTicket(uint256 type_, address recipient_) external payable override nonReentrant {
        TypeInfo memory typeInfo = _typeInfo[type_];

        if (typeInfo.price == 0) revert IGachaUpgradeable__InvalidType();

        uint256 value = msg.value;
        if (value < typeInfo.price) revert IGachaUpgradeable__InsufficientBalance();

        _slittingFee(typeInfo.paymentToken, value);

        _selectRandomElementsAndSend(typeInfo.asset, typeInfo.quantity, recipient_);

        emit BuyTicket(msg.sender, type_);
    }
}
...
interface IFeeSplitterUpgradeable {
...
interface IGachaTypeUpgradeable {
    event SetTypeNFT(uint256 tokenType, address asset, uint96 quantity, address paymentToken, uint256 price);
    ...
    function setType(uint256 type_, TypeInfo calldata typeInfo_) external;
}

// SPDX-License-Identifier: MIT
pragma solidity 0.8.19;

interface IGachaUpgradeable {
    error IGachaUpgradeable__InvalidType();
    error IGachaUpgradeable__InsufficientBalance();

    event BuyTicket(address user, uint256 gachaType);

    function buyTicket(uint256 type_, address recipient_) external payable;
}
...
abstract contract BusinessAddressesUpgradeable is BaseUpgradeable {
    ...
    function isBusiness(address businessAddress) external view returns (bool) {
...
abstract contract CollectionUpgradeable is BaseUpgradeable {
    using Helper for address;
    using BitMaps for BitMaps.BitMap;

    error Collection__NotAccepted();

    BitMaps.BitMap private _isAccepted;

    function __CollectionUpgradeable_init(address[] calldata collections_) internal onlyInitializing {
        __CollectionUpgradeable_init_unchained(collections_);
    }

    function __CollectionUpgradeable_init_unchained(address[] calldata collections_) internal onlyInitializing {
        uint256 length = collections_.length;
        for (uint256 i = 0; i < length; ) {
            _setCollectionStatus(collections_[i], true);
            unchecked {
                ++i;
            }
        }
    ...
    function isAccepted(address collection) external view returns (bool) {
        return _isAccepted.get(collection.toUint256());
    }

    function setCollectionStatus(address collection_, bool status_) external onlyRole(OPERATOR_ROLE) {
        _setCollectionStatus(collection_, status_);
    }

    function _setCollectionStatus(address collection_, bool status_) internal {
        uint256 collection = collection_.toUint256();
        _isAccepted.setTo(collection, status_);
    }

    function _onlyAccepted(address collection) internal view {
        if (!_isAccepted.get(collection.toUint256())) revert Collection__NotAccepted();
    }
...
import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { IFeeSplitterUpgradeable } from "../interfaces/IFeeSplitterUpgradeable.sol";
import { EnumerableSetUpgradeable } from "../libraries/EnumerableSetUpgradeable.sol";
import { SafeTransferLib } from "../libraries/SafeTransfer.sol";
import { OPERATOR_ROLE, HUNDER_PERCENT_IN_BPS } from "../libraries/Constants.sol";

contract FeeSplitterUpgradeable is IFeeSplitterUpgradeable, BaseUpgradeable {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;

    EnumerableSetUpgradeable.AddressSet internal _feeRecipients;
    ...
    function _slittingFee(address token_, uint256 amount_) internal {
        ...
        for (uint256 i = 0; i < length; ) {
            ...
            transferAmount = (amount_ * _percents[recipient]) / HUNDER_PERCENT_IN_BPS;
            if (token_ == address(0)) {
                SafeTransferLib.safeTransferETH(recipient, transferAmount);
            }
            // else {
            //     SafeTransferLib.safeTransferFrom(token_, sender, recipient, transferAmount);
            // }
            unchecked {
                ++i;
            }
    ...
    function viewFees() external view override returns (address[] memory, uint256[] memory fees) {
        uint256 length = _feeRecipients.length();
...
import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { IGachaTypeUpgradeable } from "../interfaces/IGachaTypeUpgradeable.sol";
import { OPERATOR_ROLE } from "../libraries/Constants.sol";

abstract contract GachaTypeUpgradeable is IGachaTypeUpgradeable, BaseUpgradeable {
    mapping(uint256 => TypeInfo) internal _typeInfo;

    function __GachaType_init(uint256[] calldata types_, TypeInfo[] calldata typesInfo_) internal onlyInitializing {
        __GachaType_init_unchained(types_, typesInfo_);
    }

    function __GachaType_init_unchained(
        uint256[] calldata types_,
        TypeInfo[] calldata typesInfo_
    ) internal onlyInitializing {
        uint256 length = types_.length;
        for (uint256 i = 0; i < length; ) {
            _setType(types_[i], typesInfo_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function setType(uint256 type_, TypeInfo calldata typeInfo_) external override onlyRole(OPERATOR_ROLE) {
        _setType(type_, typeInfo_);
    }

    function _setType(uint256 type_, TypeInfo calldata typeInfo_) internal {
        _typeInfo[type_] = TypeInfo(typeInfo_.asset, typeInfo_.quantity, typeInfo_.paymentToken, typeInfo_.price);
        emit SetTypeNFT(type_, typeInfo_.asset, typeInfo_.quantity, typeInfo_.paymentToken, typeInfo_.price);
    }

    function getTypeNFT(uint256 typeNFT) external view returns (address, uint96, address, uint96) {
        return (
            _typeInfo[typeNFT].asset,
            _typeInfo[typeNFT].quantity,
...
import { Initializable } from "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import { BusinessAddressesUpgradeable } from "./BusinessAddressesUpgradeable.sol";
import { CollectionUpgradeable } from "./CollectionUpgradeable.sol";
import { EnumerableSetUpgradeable } from "../libraries/EnumerableSetUpgradeable.sol";
import { SafeTransferLib } from "../libraries/SafeTransfer.sol";

contract RandomSelectionUpgradeable is Initializable, BusinessAddressesUpgradeable, CollectionUpgradeable {
    using EnumerableSetUpgradeable for *;

    EnumerableSetUpgradeable.AddressSet internal _assets;
    mapping(address => EnumerableSetUpgradeable.UintSet) internal _tokenIds;

    function _selectRandomElementsAndSend(address asset_, uint256 k_, address recipient_) internal {
        uint256 length;
        uint256 seed;
        uint256 tokenIndex;
        ...
        unchecked {
            seed = block.timestamp + block.basefee + block.gaslimit + block.prevrandao;
        }

        if (asset_ == address(0)) {
            address asset;
            for (uint256 i = 0; i < k_; ) {
                // random collection
                length = _assets.length();
                asset = _assets.at(_getRandomNumber(seed + i, length));

                // random tokenId
                length = _tokenIds[asset].length();
                if (length == 1) {
                    _assets.remove(asset);
                    tokenIndex = 0;
                } else {
                    tokenIndex = _getRandomNumber(seed, length);
                }

                _safeTransferFromIndex(asset, treasury, recipient_, tokenIndex);
            ...
            tokenIndex = _getRandomNumber(seed, _tokenIds[asset_].length());
            for (uint256 i = 0; i < k_; ) {
                _safeTransferFromIndex(asset_, treasury, recipient_, tokenIndex);
                unchecked {
                    ++i;
                }
            }
    ...
    function _safeTransferFromIndex(address asset_, address from_, address recipient_, uint256 tokenIndex_) internal {
        uint256 tokenId = _tokenIds[asset_].at(tokenIndex_);
        _tokenIds[asset_].remove(tokenId);
        SafeTransferLib.safeTransferFrom(asset_, from_, recipient_, tokenId);
    }

    function _getRandomNumber(uint256 seed_, uint256 upper_) private pure returns (uint256) {
        uint256 seed = uint256(keccak256(abi.encodePacked(seed_ + upper_)));
        return seed % upper_;
    }

    function deposit(address collection_, uint256[] calldata tokenIds) external {
        address operator = msg.sender;
        // _onlyBusiness(operator);
        // _onlyAccepted(collection_);

        uint256 length = tokenIds.length;

        for (uint i = 0; i < length; ) {
            SafeTransferLib.safeTransferFrom(collection_, operator, address(this), tokenIds[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function tokenLength(address asset) external view returns (uint256) {
        return _tokenIds[asset].length();
    }
