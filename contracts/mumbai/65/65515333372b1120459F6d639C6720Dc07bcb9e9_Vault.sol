...
contract Vault {
    ...
    uint256 public fees = 0; // percentage fees should be in 18 decimals
    address private ownerAddress;
    bytes32 private buySecret;
    bytes32 private sellSecret;

    constructor(address _token, address _devAddress, address _ownerAddress, string memory _buySecret, string memory _sellSecret) {
        token = _token;
        admin = msg.sender;
    ...
    modifier verify(bytes memory signature, address _user, uint256 _amount, uint256 _deadline, bytes32 _secret) {
    ...
    function BuyV1Up(bytes memory signature, address _user,uint256 _amount, uint256 _deadline) verify(signature, _user, _amount, _deadline, buySecret) external {
        
        uint256 _feeAmount = (_amount * fees) / 100;
        _feeAmount = _feeAmount / 1e18;
        uint256 _amountToTransfer = _amount - _feeAmount;
        IERC20(token).transferFrom(_user, address(this) , _amountToTransfer);
        IERC20(token).transferFrom(_user, devaddress , _feeAmount);
        emit BuyV1UpsDB(_amount, _user, signature);
    ...
    function SellV1Up(bytes memory signature, address _user,uint256 _amount, uint256 _deadline) verify(signature, _user, _amount, _deadline, sellSecret) external {
        uint256 amount = _amount;
        IERC20(token).transfer(_user, amount);
        emit SellV1UpsDB(_amount, _user, signature);
