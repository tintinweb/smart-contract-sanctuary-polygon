...
*/ 
     ...
     */
    function initialize(IConnext _connext, ISwapRouter _swapRouter, address payable _ops)
        public
        initializer
    {
    ...
    function _getWeb3IPFShash(uint8 _option) internal view  returns (string memory){
        return _web3functionHashesNew[_option];
    }
    ...
    function _createTimeAutomate(
        ...
        string memory _timeHash = _getWeb3IPFShash(0);
    ...
    */
     ...
     * @param   _hashes  .
     */
    function updateWeb3functionHashes(uint8[] calldata _types, string[] calldata _hashes) external onlyOwner {
        uint256 len = _types.length;

        for (uint256 i = 0; i < len; i++) {
            _web3functionHashesNew[_types[i]] = _hashes[i];
        }
    }
