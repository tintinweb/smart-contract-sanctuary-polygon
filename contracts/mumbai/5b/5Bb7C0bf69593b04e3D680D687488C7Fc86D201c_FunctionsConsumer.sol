...
/**
 * @title Functions Consumer contract for Uint256 type Response
...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
    ...
    * 8. Read the response after decoding to uint256 using the getIntResponse() function
    */
    ...
    /// @dev Javascript source code to be run by chainlink nodes
    string public sourceCode = "// This example shows how to fetch data from SxT Public Views API and parse the data to valid integer response\n\nconst validateInt = (responseObject) => {\n    const obj = responseObject.data[0];\n    const keys = Object.keys(obj);\n    if (keys.length > 0 && this.isNumberOrDecimal(obj[keys[0]].toString()) && Number.isInteger(parseInt(obj[keys[0]]))) {\n      return Math.round(parseFloat(obj[keys[0]]));\n    } else if (keys.length > 1 && this.isNumberOrDecimal(obj[keys[1]].toString()) && Number.isInteger(parseInt(obj[keys[1]]))) {\n      return Math.round(parseFloat(obj[keys[1]]));\n    }\n    return undefined;\n}\n\nconst viewId = args[0]\n\nconst response = await Functions.makeHttpRequest({\n    url: `https://gtw-alpha-int.spaceandtime.dev/v1/public/sql/views-by-id/${viewId}`,\n    method: \"POST\",\n    timeout: 9000,\n    headers: {\n        \"Content-Type\": \"application/json\"\n    },\n    data: null\n})\n\n// Use this in case if you are sure the view response would be single row and single column and the value is valid Integer\n// const result = Math.round(parseFloat(response.data[0][\"COLUMN_NAME\"]));\n\n// Use this in case if view response would have multiple columns and multiple rows and need to validate if 1st or 2nd column of 1st row is having an integer\n// const result = validateInt(response);\nif( !response || !response.data || !response.data.length > 0)\n    throw Error(\"Could not get response from API\")\nconst result = validateInt(response);\nif( !result)\n    throw Error(\"Invalid response\");\nreturn Functions.encodeUint256(result);";

    /**
     * @dev Event emitted when a request is fulfilled
     */
    event OCRResponse(bytes32 indexed requestId, bytes result, bytes err);

    /**
     * @dev Event emitted when the Javascript source code is updated by the owner 
     */
    event SourceCodeUpdated(string sourceCode);
     ...
     * Pass id of saved view inside the args[]
     ...
     * @param response Aggregated response from the user code
     * @param err Aggregated error from the user code or from the execution pipeline
     * Either response or error value will be set, but both the values will never be set
     */
    ...
    function updateOracleAddress(address newOracleAddress) public onlyOwner {
        require(newOracleAddress != ZERO_ADDRESS, "FunctionsConsumer: Cannot set to Zero Address");
        setOracle(newOracleAddress);
    }

    /**
     * @notice Provides decoded int value of latest response
     * @return decodedIntResponse decoded int value of latest response
     */
    function getIntResponse() public view returns(uint256 decodedIntResponse){
        return uint256(bytes32(latestResponse));
    } 

    /**
     * @notice Allows the Functions javascript source code to be updated
     * @param code New javascript source code
     */
    function updateSourceCode(string memory code) public onlyOwner {
        sourceCode = code;
        emit SourceCodeUpdated(sourceCode);
    }
