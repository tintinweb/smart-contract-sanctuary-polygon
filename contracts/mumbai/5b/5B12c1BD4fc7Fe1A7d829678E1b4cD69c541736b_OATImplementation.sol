    ...
    BaseRelayRecipient
{
    ...
    mapping(uint256 => uint256) private _idToPrice; 
    ...
    /**
    * To set the NFT price
    * 
    * @param _tokenId  -- Token id 
    * @param _price -- Price of NFT to set
    */
    function setPrice(
        uint256 _tokenId,
        uint256 _price
    )public returns (bool){
        require(_exists(_tokenId), "Artefy: set price query for nonexistent token");
        require(_isApprovedOrOwner(_msgSender(), _tokenId), "Artefy: Only owner or approved can set the price");
        _idToPrice[_tokenId] = _price;
        return true;
    }

    /**
    * To get the NFT price
    * 
    * @param _tokenId  -- Token id 
    */
    function getPrice(
    ...
    */
    function transferFromMatic(
        ...
        require(_idToPrice[tokenId] != 0, "Artefy: Amount of the NFT is not set.");
        require (msg.value == _idToPrice[tokenId], "Artefy: amount Tranffered is not enough to buy this painting.");
        address nftowner = ownerOf(tokenId);
        (bool sent,) = nftowner.call{value: msg.value}("");
        require(sent, "Artefy: Failed to send amount"); // Tranfering Matic value as well. 
        _transfer(nftowner, _msgSender(), tokenId);
        emit PaymentReceived(_msgSender(), nftowner, _idToPrice[tokenId]);
        return true;          
    }
    ...
    *
    * @param tokenId - token Id to tranfer it to buyer
    * @param amount - amount of NFT to transfer
    * @param receipient - receiver of the NFT
    */
    function transferNFT(
        ...
        require(_idToPrice[tokenId] != 0, "Artefy: Amount of the NFT is not set.");
        require (amount == _idToPrice[tokenId], "Artefy: amount Tranffered is not enough to buy this painting.");
        address nftowner = ownerOf(tokenId);
        require(_isApprovedOrOwner(_msgSender(), tokenId), "Artefy: Only owner or approved can transfer");
        _transfer(nftowner, receipient, tokenId);
        emit PaymentReceived(receipient, nftowner, _idToPrice[tokenId]);
        return true;          
    }
