...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner {
  ...
  mapping(string => uint256) adsWithBudget;
  struct PayoutDetails{
    uint256 earnings;
    string ON_ADS;
    address CONTENT_CREATOR;
  }
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    latestError = err;
    // // I want to convert this response to Struct or any other datatype.
    string memory sxt = string(response);
    strings.slice memory s = sxt.toSlice();
    ...
    address payable creator = payable(address(bytes20(bytes(splitResults[2]))));
    creator.transfer(payment);
    adsWithBudget[adsName] = adsWithBudget[adsName] - payment;

    emit w3adsPaid(creator,payment,adsName);
    //PayoutDetails memory payOutDetails;
    //(payOutDetails.earnings, payOutDetails.ON_ADS,payOutDetails.CONTENT_CREATOR) = abi.decode(response,(uint256,string,address));
