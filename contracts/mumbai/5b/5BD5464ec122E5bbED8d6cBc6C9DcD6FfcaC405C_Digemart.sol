...
contract Digemart is Converter{
    ...
    function convertMaticAmount(uint _amountInUSD) public view returns(uint) {
        address[] memory _path;
        _path = new address[](2);
        _path[0] = IUniswapRouter(router).WETH();
        _path[1] = stableCoin;
        uint _requiredAmount = IUniswapRouter(router).getAmountsIn(_amountInUSD, _path)[0];
        // Add 1% slippage
        return _requiredAmount * 101 / 100;
    }
