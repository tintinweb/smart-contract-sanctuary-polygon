...
contract Rewards is Initializable {
     ...
     */
    function distributionION(uint256 amount) public onlyStaking {
        if (amount != 0) {
    ...
    /**
    @dev Distributes Matic rewards
     */
    function distributionMatic() public onlyStaking {
        if (totalStakeMatic != 0 && valueNotDistribute != 0) {
            SMatic += ((valueNotDistribute) * 10**18) / totalStakeMatic;
            totalMatic += valueNotDistribute;
