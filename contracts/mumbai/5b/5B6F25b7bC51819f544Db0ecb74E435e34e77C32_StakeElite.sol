...
contract StakeElite is Ownable, Pausable {
    ...
    constructor (address _masterChef, address _wallet) {
        MasterChef = IMasterChef(_masterChef); // For testnet only
        
        wallet = payable(_wallet);

        planDetails[1] = PlanInfo({planID: 1, planDays: 30 * 86400, planReward: 2, planReferalReward: 1, planFee: 0 });
        planDetails[2] = PlanInfo({planID: 2, planDays: 182 * 86400, planReward: 4, planReferalReward: 2, planFee: 1 * 1e18 });
        planDetails[3] = PlanInfo({planID: 3, planDays: 365 * 86400, planReward: 10, planReferalReward: 5, planFee: 2 * 1e18 });
    }

    function Init( address _EliteNatiavePair, address _EliteUSDPair, address _rewardToken,uint256 _eliteNativePoolId, uint256 _eliteUSDPoolID ) external onlyOwner {
        require(!initilize,"already initilized");
        EliteNativePair = IERC20(_EliteNatiavePair);
        EliteUSDPair = IERC20(_EliteUSDPair);
        RewardToken = IERC20(_rewardToken);
        EliteNativeDepositID = _eliteNativePoolId;
        EliteUsdDepositID = _eliteUSDPoolID;
    ...
    function updateMasterChef(address _newMasterChef) external onlyOwner {
        require(_newMasterChef != address(0x0),"invalid address");
