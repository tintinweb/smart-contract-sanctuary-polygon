...
contract test is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    ...
    uint256 public DASD_SUPPLY = 4;
    
    bool public IS_DASD_ACTIVE = false;

    
    uint256 public DASD_TX_LIMIT = 4;

    
    uint256 public DASD_WALLET_LIMIT = 4;

    
    uint256 public DASD_PRICE = 0.001 ether;

    
    uint256 public DASD_COUNT = 0;

    

    
    mapping(address => uint) public DASD_Minted;

    bool public _revealed = false;

    string private baseURI = "";
    string private preRevealURI = "https://ipfs.w3bmint.xyz/ipfs/QmRb37CgFTW31sHKX8srtT8KjfANfySciKgY3xq26N1gD9";

    mapping(address => uint256) addressBlockBought;

    address public constant RL_ADDRESS = 0xc9b5553910bA47719e0202fF9F617B8BE06b3A09; 

    constructor() ERC721A("test", "ASDASD") {
    ...
    function DASDMint(uint256 numberOfTokens) external isSecured(DASD_PRICE * numberOfTokens) payable {
        validateMint(IS_DASD_ACTIVE, DASD_COUNT, DASD_SUPPLY, DASD_Minted[msg.sender], DASD_WALLET_LIMIT, DASD_TX_LIMIT, numberOfTokens, true);
        addressBlockBought[msg.sender] = block.timestamp;
        DASD_Minted[msg.sender] += numberOfTokens;
        DASD_COUNT += numberOfTokens;
    ...
    function toggleDASDMintStatus() external onlyOwner {
        IS_DASD_ACTIVE = !IS_DASD_ACTIVE;
    }

    // SUPPLY CHANGERS
    
    function setDASDSupply(uint256 _supply) external onlyOwner {
        DASD_SUPPLY = _supply;
    }
    ...
    function setDASDPrice(uint256 _price) external onlyOwner {
        DASD_PRICE = _price;
    }
