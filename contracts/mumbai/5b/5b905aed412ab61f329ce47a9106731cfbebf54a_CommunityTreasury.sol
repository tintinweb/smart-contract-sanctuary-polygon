...
/**
@title 2112 Community Treasury
@author 2112.run
@notice This contract holds community DATA
@dev Uses UUPS proxy for upgrades and requires roles for different admin functions
*/
contract CommunityTreasury is Treasury {
    /**
    @notice Initializer due to UUPSUpgradeable
    @dev Grants the roles to manage and upgrade the Treasury
    @param registry_ The Registry's address used to resolve addresses of the ecosystem
    @param nonce_ Prevent re-using signature
    @param signature_ Signature from deployer for Registry
    */
    function initialize(address registry_, uint256 nonce_, bytes memory signature_) public initializer {
        __Treasury_init(Constants.COMMUNITY_TREASURY, registry_, nonce_, signature_);
    }
    /**
...
error ExceedsCurrentBalance();
error OperatorCannotBeEoA();
error CannotTransferToZeroAddress();
error OnlyTransferrableToEoA();
error QuantityExceedsMaximumReward();
error OperatorNotApproved();

/**
@title 2112 Treasury
@author 2112.run
@notice This is the base treasury contract used to hold DATA
@dev Uses UUPS proxy for upgrades and requires roles for different admin functions
*/
abstract contract Treasury is RegisterableUpgradeableBase {
    using AddressUpgradeable for address;
    /**
    @notice Mapping of operators that can transfer rewards
    */
    ...
    @param approved Whether the address is an approved operator
    */
    event OperatorApproval(address indexed sender, address operator, bool approved);
    /**
    @notice Emitted when a reward is transferred
    @param sender The msg.sender
    @param beneficiary The address of the reward recipient
    @param amount The amount of DATA rewarded
    */
    event RewardTransferred(address indexed sender, address beneficiary, uint256 amount);
    /**
    @notice Emitted when the maximum DATA reward treshold is changed
    @param sender The msg.sender
    @param previousAmount The previous maximum reward
    @param newAmount The new maximum reward
    */
    event MaximumRewardChanged(address indexed sender, uint256 previousAmount, uint256 newAmount);
    /**
    @notice Initializer due to UUPSUpgradeable
    @dev Grants the roles to manage and upgrade the Treasury
    ...
    @param signature_ Signature from deployer for Registry
    */
    function __Treasury_init(bytes32 key_, address registry_, uint256 nonce_, bytes memory signature_) internal onlyInitializing {
        __RegisterableUpgradeableBase_init(key_, registry_, nonce_, signature_);
        address airdrops = registry.getEntry(Constants.AIRDROPS);
        operators[airdrops] = airdrops != address(0);
    }
    /**
    @notice Initializer due to UUPSUpgradeable
    @dev Grants the roles to manage and upgrade the Treasury
    ...
    @param signature_ Signature from deployer for Registry
    */
    function __Treasury_init_unchained(bytes32 key_, address registry_, uint256 nonce_, bytes memory signature_) internal onlyInitializing {
        __Treasury_init(key_, registry_, nonce_, signature_);
    }
    /**
    @notice Approve or Revoke an address as an operator
    @dev An operator cannot be an externally owned account, only contracts
    @param operator_ The address of the operator
    @param approved_ True to approve or False to revoke
    */
    function approve(address operator_, bool approved_) external onlyRole(DEFAULT_ADMIN_ROLE) {
        if(!operator_.isContract())
            revert OperatorCannotBeEoA();
        operators[operator_] = approved_;

        emit OperatorApproval(msg.sender, operator_, approved_);
    }
    /**
    @notice Check whether an address is an approved operator
    @param operator_ The address to check
    @return If the operator is approved
    */
    function isApproved(address operator_) external view returns (bool) {
        return operators[operator_];
    }
    /**
    @notice Set DATA if it does not exist yet
    @return IDATA
    */
    function _data() internal returns (IDATA) {
        if(address(data) == address(0)) {
            data = IDATA(registry.getEntry(Constants.DATA));
        }
        return data;
    }
    /**
    @notice Modifier that will only allow approved operators to
    */
    modifier onlyApproved() virtual {
...
*/
library Constants {
    // address internal constant MULTI_SIG           = 0x39191B3201Ec26b03A0c0D985c2BD41390439AAB;
    // address internal constant LIQUIDITY_PROVIDER  = 0xD1735b8d7178C4032aB642A301E5745FDCB4a890;
    // address internal constant SIGNER              = 0xB0F3B0aa2f7FbBBE4d309e60e1B07fD7513cE534;
    address internal constant MULTI_SIG           = 0xA1517b342cf4f9A4ca228A8FbC388F47Ebd0457B;
    address internal constant LIQUIDITY_PROVIDER  = 0x7d64e6983cF9dA0029fd52CFb847B4421bB9E69f;
    address internal constant SIGNER              = 0xa7F1400d46F34686Dd4ADB333B34D50ca34028F7;
