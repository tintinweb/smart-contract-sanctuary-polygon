...
contract Finde {
    ...
    // EVENTS
    event UriAdded(string indexed uri, address indexed provider, string[] indexed tags);
    ...
    // FUNCTIONS
    function addURI(string memory _uri, string memory _country, string[] memory _tags) public {
        ...
        if (_tags.length> 0){
            for (uint256 i = 0; i < _tags.length; i++) {
                ...
                tagToCountryToUris[_tags[i]][indexToProvider[addressToIndex[msg.sender]].country].push(_uri);
            }
        }
    ...
    function getPaginatedURIs(
        ...
        // possible cases:
        if (compareStringsbyBytes(_country, "") == true && compareStringsbyBytes(_tag, "") == true) {
        ...
        }else if(compareStringsbyBytes(_country, "") == true && compareStringsbyBytes(_tag, "") == false){
            // if `_country` is empty and `_tag` is not empty,
            if(tagToUris[_tag].length > 0 && tagToUris[_tag].length <= _limit){
                
                for (uint256 i = 0; i < tagToUris[_tag].length; i++) {
                    data[dataLength] = indexToProvider[uriToIndex[tagToUris[_tag][i]]];
                    dataLength++;
                }
            }else if(tagToUris[_tag].length > 0 && tagToUris[_tag].length > _limit){
                require(_offset+_limit <= tagToUris[_tag].length, "FINDE: Invalid offset and limit.");
                for (uint256 i = _offset; i < _offset+_limit; i++) {
                    data[dataLength] = indexToProvider[uriToIndex[tagToUris[_tag][i]]];
                    dataLength++;
                }
        ...
        }else if(compareStringsbyBytes(_country, "") == false && compareStringsbyBytes(_tag, "") == true){
            // if `_country` is not empty and `_tag` is empty,
            require(countryToIndexs[_country].length > 0, "FINDE: Invalid country.");
            require(_offset+_limit <= countryToIndexs[_country].length, "FINDE: Invalid offset and limit.");

            for (uint256 i = _offset; i < countryToIndexs[_country].length; i++) {
                data[dataLength] = indexToProvider[countryToIndexs[_country][i]];
                dataLength++;
            }
            // return all providers based on the offset, limit AND country
        }else if(compareStringsbyBytes(_country, "") == false && compareStringsbyBytes(_tag, "") == false){
            // if `_country` and `_tag` both are not empty,
            require(countryToIndexs[_country].length > 0, "FINDE: Invalid country.");
            require(tagToCountryToUris[_tag][_country].length <= _offset+_limit, "FINDE: Invalid offset and limit.");
           
            if(tagToCountryToUris[_tag][_country].length > 0 ){
                for (uint256 i = _offset; i < tagToCountryToUris[_tag][_country].length; i++) {
                    data[dataLength] = indexToProvider[uriToIndex[tagToCountryToUris[_tag][_country][i]]];
                    dataLength++;
                }
            }
