...
contract TomBird is ERC1155, Ownable {
    ...
    constructor(IERC20 USDC_address, string memory _uri, address _treasuryAddress) ERC1155("https://gateway.pinata.cloud/ipfs/QmSAsy34pvEGE8i29p2S7hDizReLYb4QPCKCLagzRAs9rW/1/") {
        setURI(_uri);
    ...
    */
    ...
    function mint(uint256 id, uint256 amount) public
    {
        ...
        // Must be approved before calling this function to contract

        USDC.transferFrom(msg.sender, treasuryAddress, 1000000);
    ...
    function mintBatch(uint256[] memory ids, uint256[] memory amounts, uint256 mintCount) public
    {
        if(mintCount == 15){
            ...
            require(ids.length == mintCount, "Unable to mint the ids over than mintCount");
            require(amounts.length == mintCount, "Unable to mint the amounts over than mintCount");
            // Must be approved before calling this function to contract

            USDC.transferFrom(msg.sender, treasuryAddress, 10000000);
            ...
            USDC.transferFrom(msg.sender, treasuryAddress, 50000000);
            _mintBatch(msg.sender, ids, amounts, "");
