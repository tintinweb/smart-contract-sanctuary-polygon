...
contract SafoLifeTimeInsuranceNftCollection is ERC721URIStorage, ERC721Enumerable {
    using Counters for Counters.Counter;
    Counters.Counter public _tokenIds;

    IERC20 public token = IERC20(0x7a5D4ed09Da0aDDe3eD2cDCF4Cc4d2e6271967C2);
    uint256 public tokenPrice = 10 ether;
    uint256 public ethPrice = 0.001 ether;
    address payable public owner =
        payable(0x2F56fDA413aE38E87a25b9d3e2C41B2d08090418);
    uint256 public refferalPercentage = 5;
    uint256 public userLimit = 21;
    uint256 public maxSupply = 500;
    uint256 public discountPercentage = 50;
    ...
    constructor() ERC721("SAFo Life Time Insurance", "SAFOLTI") {}
    ...
    function createNftToken(
        ...
        if (whitelist[msg.sender]) {
            ...
            require(
                _amount == ((totalNfts * tokenPrice) - discountAmount),
                "Invalid amount"
            );
        } else {
            require(_amount == totalNfts * tokenPrice, "Invalid amount");
        }
        require(
            _tokenIds.current() + totalNfts <= maxSupply,
            "Max supply exceeded"
        );
        uint256 _balance = balanceOf(msg.sender);
        require(_balance + totalNfts <= userLimit, "Nft mint limit exceeded");
        ...
        for (uint256 i = 0; i < totalNfts; i++) {
            ...
            _setTokenURI(newItemId, _tokenURI[i]);
        }
    }
    ...
    function createNftEth(
        ...
        if (whitelist[msg.sender]) {
            uint discountAmount = (totalNfts * ethPrice * discountPercentage) /
                100;
            require(
                msg.value == ((totalNfts * ethPrice) - discountAmount),
                "Invalid amount"
            );
        } else {
            require(msg.value == totalNfts * ethPrice, "Invalid amount");
        }
        require(
            _tokenIds.current() + totalNfts <= maxSupply,
            "Max supply exceeded"
    ...
    function setDiscountPercentage(
        ...
        require(_discountPercentage > 0, "Invalid percentage");
        discountPercentage = _discountPercentage;
    }

    function setUserLimit(uint256 _userLimit) public onlyOwner {
        require(_userLimit > 0, "Invalid limit");
    ...
    function setRefferalPercentage(
        ...
        require(_refferalPercentage > 0, "Invalid percentage");
