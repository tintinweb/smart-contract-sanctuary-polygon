...
contract MessagingApp {
    ...
    function sendMessage(address reciever, string calldata message, bool isShareable) external {
        require(
...
// function shareMessage(address receiver, string calldata message, address[] memory receivers, bool isShareable) external {
//     Message memory newMessage = Message(
//         msg.sender,
//         receiver,
//         message,
...
//     for (uint i = 0; i < receivers.length; i++) {
//         if (isShareable || receivers[i] == receiver) {
//             sendNotification(receivers[i], "You have received a shared message.");
//         }
//     }
// }

function MessageRecu(string memory email) external view returns (Message[] memory) {
        uint count = 0;
        for (uint i = 0; i < messages.length; i++) {
            if (messages[i].receiver == getAddress(email) && (!messages[i].read || messages[i].shares > 0)) {
                count++;
            }
        }
