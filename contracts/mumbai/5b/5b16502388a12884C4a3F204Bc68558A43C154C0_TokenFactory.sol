...
contract TokenFactory is AccessControl, OwnableUpgradeable, FactoryStorage{
    ...
    function _initIdentityRegistry(address identityRegistry) internal{
        ...
        success = false;
        (success,) = identityRegistryStorage.call(abi.encodeWithSignature("init()"));
        ...
        success = false;
        (success,) = claimTopicsRegistry.call(abi.encodeWithSignature("init()"));
        require(success, "claimTopicsRegistry init failure");
        success = false;
        (success,) = trustedIssuersRegistry.call(abi.encodeWithSignature("init()"));
    ...
    function deployAdminIdentity(uint256 executionBuffer, uint256 reqSig, address tokenizationAgent , address _issuer, address _token) public onlyRole(Operator_Role) returns (address) {
        ...
        (success,) = adminIdentityProxy.call(abi.encodeWithSelector(0xa4a2a9f6, address(this), executionBuffer, reqSig));
    ...
    function getAdminIdentityMinter(address _token) public view returns(address){
        return adminIdentityMinter[_token];
            ...
            // --------------------------Function for Trusted Issuers Registry-------------------------------------

    function getTrustedIssuersRegistry(address _token) public view returns(ITrustedIssuersRegistry) {
        return IIdentityRegistry(IToken(_token).identityRegistry()).issuersRegistry();
    }

    function getClaimTopicsRegistry(address _token) public view returns(IClaimTopicsRegistry) {
        return IIdentityRegistry(IToken(_token).identityRegistry()).topicsRegistry();
    }

     /**
     * @dev function to deploy claim issuer identity init and add claim issuer to the trusted issuer registry for a specific token.
     *      all claim issuer functionality for a token should be initiated by this function.
     * @param _token address of the token to add a trusted issuer for.
     * @param _claimIssuer the address to set as the initial claim issuer
     * @param _claimTopics uint256 calldata array holding claim topics the issuer will be allowed to issue
     */
    function deployAndAddTrustedIssuerForToken(address _token, address _claimIssuer, uint256[] calldata _claimTopics) public onlyRole(Operator_Role){
        require(tokenId[_token] != 0, "nonexistant token");
        IClaimIssuer issuerIdentity = _deployClaimIssuerIdentity(_claimIssuer);
        ITrustedIssuersRegistry trustedIssuersRegistry = getTrustedIssuersRegistry(_token);
        trustedIssuersRegistry.addTrustedIssuer(issuerIdentity, _claimTopics);   
    }

            // --------------------------Function for Claim Topics Registry-------------------------------------
    function addRequiredClaimTopicForToken(address _token, uint256 _claimTopic) public onlyRole(Operator_Role){
        require(tokenId[_token] != 0, "nonexistant token");
        IClaimTopicsRegistry claimTopicsRegistry = getClaimTopicsRegistry(_token);
        claimTopicsRegistry.addClaimTopic(_claimTopic);
    }

    function getRequiredClaimTopicsForToken(address _token) public view returns(uint256[] memory) {
        require(tokenId[_token] != 0, "nonexistant token");
        IClaimTopicsRegistry claimTopicsRegistry = getClaimTopicsRegistry(_token);
        return claimTopicsRegistry.getClaimTopics();
    }

    function removeRequiredClaimTopic(address _token, uint256 _claimTopic) public onlyRole(Operator_Role) {
        require(tokenId[_token] != 0, "nonexistant token");
        IClaimTopicsRegistry claimTopicsRegistry = getClaimTopicsRegistry(_token);
        claimTopicsRegistry.removeClaimTopic(_claimTopic);
    }

    function addClaimIssuerToClaimIssuerIdentity(address _issuerToAdd) public {
        require(claimIssuerIdentityId[msg.sender] != 0, "nonexistant claim issuer identity");
        IClaimIssuer issuerIdentity = IClaimIssuer(claimIssuerIdentities[claimIssuerIdentityId[msg.sender] - 1]);
        issuerIdentity.addKey(keccak256(abi.encode(_issuerToAdd)), 3, 1);
    }

    function getIssuerIdentity(address _issuer) public view returns(address) {
        return claimIssuerIdentities[claimIssuerIdentityId[_issuer] - 1];
    }

    function _deployClaimIssuerIdentity(address _issuer) internal returns(IClaimIssuer){
        address proxy = address(new ProxyV1());
        claimIssuerIdentities.push(proxy);
        claimIssuerIdentityId[_issuer] = claimIssuerIdentities.length;
        (bool success,) = proxy.call(abi.encodeWithSelector(0x3659cfe6, implAuthorityIssuerIdentity));
        require(success,"issuer identity upgrade failed");
        success = false;
        (success,) = proxy.call(abi.encodeWithSelector(0x19ab453c, address(this)));
        require(success, "Issuer Identity Intiatialization Failed");
        success = false;
        (success,) = proxy.call(abi.encodeWithSelector(0x1d381240, keccak256(abi.encode(_issuer)), 3, 1));
        require(success, "Issuer Identity addKey Failed");
        success = false;
        return IClaimIssuer(proxy);
    }

        // --------------------------CLAIM FUNCTIONALITY-------------------------------------
    /**
     * @dev add claim to a user identity on behalf of an approved claim issuer.
     * @param _user the address of the user (EOA) to add the claim for.
     * @param _claimIssuer the address of the approved claim issuer contract.
     * @param _topic the claim topic for the claim
     * @param _sig see IdentityInit.sol::addClaim()
     * @param _data see IdentityInit.sol::addClaim()
     * @param _uri see IdentityInit.sol::addClaim()
     */
    function addClaimOnUserIdentity(
        address _user, address _claimIssuer, 
        uint256 _topic, bytes memory _sig, 
        bytes memory _data, string memory _uri) public 
    {
        require(userIdentityId[_user] != 0, "nonexistant user");
        require(claimIssuerIdentityId[_claimIssuer] != 0, "nonexistant claim issuer identity");
        IIdentity userIdentity = IIdentity(userIdentities[userIdentityId[_user] - 1]);
        address issuerIdentity = claimIssuerIdentities[claimIssuerIdentityId[_claimIssuer] - 1];
        userIdentity.addClaim(_topic, 1, issuerIdentity, _sig, _data, _uri);
    }
