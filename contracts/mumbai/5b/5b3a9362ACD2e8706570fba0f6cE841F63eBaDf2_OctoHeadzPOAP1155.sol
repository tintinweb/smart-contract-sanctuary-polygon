...
contract OctoHeadzPOAP1155 is Ownable, ERC1155, Whitelistable {
    string public baseIPFS = "https://ipfs.io/ipfs/";
    ...
    constructor(string memory _URI, address _whitelistAdmin) 
    ...
    function setBaseIPS(string memory _newBaseIPFS) public onlyOwner {
        baseIPFS = _newBaseIPFS;
    }
    ...
    function mintPOAP(uint _eventID) public payable {
        _mint(msg.sender, _eventID, 1, "");
    }

    function airdropPOAP(address[] memory addresses, uint256 _numNFTz, uint _eventID) public onlyOwner {
        for (uint256 i = 0; i < addresses.length; i++) {
            _mint(addresses[i], _eventID, _numNFTz, "");
        }
    }
}
