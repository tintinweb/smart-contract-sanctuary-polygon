...
// File lib/ERC1155URIStorage.sol
...
contract EMPackageFactory {
    ...
    uint256 public packagesToBeRefundedCount;
    uint256 public totalPackagesCount;

    function createPackage(address _owner,
    uint256 _packageType,
    uint256 _tokenId,
    ...
    uint256 _price) internal {
        ownership[PackageType(_packageType)][_tokenId] = _owner;
        ...
        userPackagesPerType[_owner][PackageType(_packageType)].push(pack);
        packagesPerType[PackageType(_packageType)].push(pack);
        emit PackageCreated(_owner, _tokenId, PackageType(_packageType), _astronautId, _pfpId, _daoId, _refundable, time, currentTime(), _price);
    }
    ...
    function getUserPackagesCount(address _owner) external view returns(uint256){
        return userPackagesCount[_owner];
    }
    ...
    function ckeckIfPackageRefundale(uint256 _packageId) external view returns(bool) {
        return isPackageRefundale[_packageId];
    }
    ...
    function canBeRefuned(PackageType _packageType, uint256 _index, uint256 _refundPeriod) external view returns(bool) {
       return packagesPerType[_packageType][_index].refundStartTimestamp > 0 &&
        ...
        && block.timestamp <= packagesPerType[_packageType][_index].refundStartTimestamp + _refundPeriod;
    }

    function ownerOf(PackageType _packageType, uint256 _packageId) external view returns(address) {
        return ownership[_packageType][_packageId];
    }
    ...
    {
        return userPackagesPerType[_owner][_packageType][_packageId];
    }

    function packagesPerTypeCount(PackageType _packageType) external view returns(uint256) {
        return packagesPerType[PackageType(_packageType)].length;
    }
}


// File contracts/EMPackage.sol
...
contract EMPackage is ERC1155, EMPackageFactory, ERC1155URIStorage, Ownable {
    ...
    function mintPackageByGuildMember(
        ...
        require(_operators[_msgSender()], "EMPackage(mint): Not an operator");
        uint256 nextIndex = totalPackagesCount;
        _mint(to, id, 1, bytes(abi.encodePacked("Elon Must Package #", Strings.toString(id), Strings.toString(nextIndex))));
        createPackage(to, id, nextIndex, _astronautIdex, _pfpIndex, _daoIndex, false, 0);
    }
    ...
    uint256 _price
        ...
        for (uint256 index = 0; index < _quantity; index++) {
            uint256 nextIndex = totalPackagesCount;
            uint256 characterStartId = _astronautIdex + index;
            uint256 daoStartId = _daoIndex + index;
            uint256 pfpStartId = _pfpIndex + index;
            uint256 packeType = id;
            _mint(to, packeType, 1, bytes(abi.encodePacked("Elon Must Package #", Strings.toString(packeType), Strings.toString(nextIndex))));
            createPackage(to, id, nextIndex, characterStartId, pfpStartId, daoStartId, _refundable, _price);
        }
    }

    function setPackageRefund(address _owner, uint256 _packageType, uint256 _packageId) external {
        require(_operators[_msgSender()], "EMPackage(decresePackagesToBeRefundedCount): Not an operator");
        packagesToBeRefundedCount--;
        isPackageRefundale[_packageId] = false;
        userPackagesCount[_owner]--;
        packagesToBeRefundedCount--;
        delete ownership[PackageType(_packageType)][_packageId];
        delete userPackagesPerType[_owner][PackageType(_packageType)][_packageId];
    }
    ...
    function burnToken(
        ...
        require(_operators[_msgSender()], "EMPackage(burnToken): Not an operator");
