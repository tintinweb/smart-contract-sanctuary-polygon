...
contract BSBFactoryUpgradeable is Initializable, OwnableUpgradeable, IBSBTypes, BSBBaseUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    function setTreeHouseLevelCosts(
        ...
        require(levels_.length == bloodTokenCosts_.length && levels_.length == bloodShardCosts_.length, "BSB400");
    ...
    function mergeTreeHouses(uint[] calldata housesIds_) external {
        IERC721Child ERC721Caller =  IERC721Child(contracts["treeHouse"]);

        require(housesIds_.length == 2, "BSB400");
        require(ERC721Caller.ownerOf(housesIds_[0]) == _msgSender(), "BSB420");
        require(ERC721Caller.ownerOf(housesIds_[1]) == _msgSender(), "BSB420");
        ...
        require(resultingSize_ <= treeHousesStats_[treeHouseToBeUpgradedIndex_].size.max, "BSB400");
    ...
    function reRoll(uint toBeUpgradedTokenId_, uint[] calldata gen1Tokens_) external {
        address gen1Address_ = contracts["gen1"];

        require(IERC721Child(gen1Address_).ownerOf(toBeUpgradedTokenId_) == _msgSender(), "BSB420");

        for (uint i = 0; i < gen1Tokens_.length; ++i) {
            require(IERC721Child(gen1Address_).ownerOf(gen1Tokens_[i]) == _msgSender(), "BSB420");
        }
    ...
    function calculateReRollStats(
        ...
        uint average = levelSum / bearStats_.length;
        uint currentAdditionalSuccessPercentage_ = _getLevelImpact(average).reRollSuccessBoostPercentage;
