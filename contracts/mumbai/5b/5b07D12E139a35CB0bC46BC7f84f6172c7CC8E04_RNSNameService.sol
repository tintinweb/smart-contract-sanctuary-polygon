...
import "./RNSSemanticBaseStruct.sol";
import {RNSSemanticSBTLogicUpgradeable} from "../libraries/RNSSemanticSBTLogicUpgradeable.sol";

contract RNSSemanticSBTUpgradeable is Initializable, OwnableUpgradeable, ERC165Upgradeable, ERC721Upgradeable, IERC721EnumerableUpgradeable, ISemanticSBT, ISemanticSBTSchema, IERC5192 {
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;
    ...
    mapping(uint256 => mapping(bytes32 => uint256)) internal _subjectIndex;
    ...
    function initialize(
        ...
        RNSSemanticSBTLogicUpgradeable.addClass(classes_, _classNames, _classIndex);
        RNSSemanticSBTLogicUpgradeable.addPredicate(predicates_, _predicates, _predicateIndex);
    ...
    function subjectIndex(string memory subjectValue, string memory className_) public view returns (uint256){
        uint256 sIndex = _subjectIndex[_classIndex[className_]][bytes32(bytes(subjectValue))];
    ...
    function subject(uint256 index) public view returns (string memory subjectValue, string memory className_){
        ...
        subjectValue = string(abi.encodePacked(_subjects[index].value));
    ...
    function rdfOf(uint256 tokenId) public view override returns (string memory) {
        ...
        return string(RNSSemanticSBTLogicUpgradeable.buildRDF(_tokens[tokenId], _classNames, _predicates, _subjects));
    }
    ...
    function _mint(uint256 tokenId, address account, SubjectPO[] memory subjectPOList) internal {
        ...
        RNSSemanticSBTLogicUpgradeable.addSubjectPO(pIndex, oIndex, subjectPOList, _predicates, _subjects);
...
import "../core/RNSSemanticBaseStruct.sol";
...
library RNSSemanticSBTLogicUpgradeable {
    using AddressUpgradeable for address;
    using StringsUpgradeable for uint256;
    using StringsUpgradeable for uint160;
    using StringsUpgradeable for address;
...
//    string  constant BLANK_SPACE = " ";
    ...
    //";"
    bytes4  constant TURTLE_LINE_SUFFIX = 0x3b000000;
    //" . "
    bytes4  constant TURTLE_END_SUFFIX = 0x202e2000;
    //"Soul"
    bytes4  constant SOUL_CLASS_NAME = 0x536f756c;

    //:
    bytes4  constant public ENTITY_PREFIX = 0x3a000000;
    //p:
    bytes4  constant public PROPERTY_PREFIX = 0x703a0000;
    //"_"
    bytes4  constant CONCATENATION_CHARACTER = 0x5f000000;
    //"["
    bytes4  constant BLANK_NODE_START_CHARACTER = 0x5b000000;
    //"]"
    bytes4  constant BLANK_NODE_END_CHARACTER = 0x5d000000;
    //" "
    bytes4  constant BLANK_SPACE = 0x20000000;
...
//    }
    ...
    function addSubjectPO(uint256[] storage pIndex, uint256[] storage oIndex, SubjectPO[] memory subjectPOList, Predicate[] storage _predicates, Subject[] storage _subjects) external {
    ...
    function buildRDF(SPO storage spo, string[] storage _classNames, Predicate[] storage _predicates, Subject[] storage _subjects) external view returns (bytes memory _rdf){
        ...
        for (uint256 i; i < len;) {
            ...
            _rdf = bytes.concat(_rdf, buildSubjectRDF(_classNames[_subjects[oIndex].cIndex], _predicates[pIndex].name, _subjects[oIndex].value));

            bytes4 suffix = i == len - 1 ? TURTLE_END_SUFFIX : TURTLE_LINE_SUFFIX;
            _rdf = bytes.concat(_rdf, suffix);
            unchecked{
                ++i;
            }
        }
    ...
    function buildS(SPO storage spo, string[] storage _classNames, Subject[] storage _subjects) public view returns (bytes memory){
        uint256 sIndex = spo.sIndex;
        bytes memory _className = sIndex == 0 ? abi.encodePacked(SOUL_CLASS_NAME) : abi.encodePacked(_classNames[_subjects[sIndex].cIndex]);
        bytes memory subjectValue = sIndex == 0 ? abi.encodePacked(address(spo.owner)) : abi.encodePacked(_subjects[sIndex].value);
        return abi.encodePacked(ENTITY_PREFIX, _className, CONCATENATION_CHARACTER, subjectValue, BLANK_SPACE);
    }



    function buildSubjectRDF(string storage _className, string storage name, bytes32 value) internal view returns (bytes memory){
        bytes  memory p = abi.encodePacked(PROPERTY_PREFIX, name);
        bytes memory o = abi.encodePacked(ENTITY_PREFIX, _className, CONCATENATION_CHARACTER, value);
        return abi.encodePacked(p, BLANK_SPACE, o);
    }


    function buildStringRDFCustom(string calldata class, string calldata entityValue, string calldata predicate, string calldata o) external pure returns (bytes memory){
        bytes memory s = abi.encodePacked(ENTITY_PREFIX, class, CONCATENATION_CHARACTER, entityValue, BLANK_SPACE);
        bytes memory p = abi.encodePacked(PROPERTY_PREFIX, predicate, BLANK_SPACE);
        return abi.encodePacked(s, p, o, TURTLE_END_SUFFIX);
    }
    ...
    function _addSubject(bytes32 value, uint256 cIndex,
        Subject[] storage _subjects,
        mapping(uint256 => mapping(bytes32 => uint256)) storage _subjectIndex) public returns (uint256 sIndex){
...
import "../core/RNSSemanticSBTUpgradeable.sol";
//import "../interfaces/social/INameService.sol";
import {RNSSemanticSBTLogicUpgradeable} from "../libraries/RNSSemanticSBTLogicUpgradeable.sol";
import {RNSNameServiceLogic} from "../libraries/RNSNameServiceLogic.sol";


contract RNSNameService is RNSSemanticSBTUpgradeable {
    using StringsUpgradeable for uint256;
    using StringsUpgradeable for address;
    ...
    function initialize(
        ...
        suffix = suffix_;
        _suffix = bytes32(bytes(suffix));
    }


    function register(address owner, bytes32 name, bool resolve) external virtual  returns (uint tokenId) {
        return _register(owner, name, resolve);
    }



    function setNameForAddr(address addr_, string calldata _name) external  {
        ...
        RNSNameServiceLogic.setNameForAddr(addr_, sIndex,
            _ownedResolvedName,
            _ownerOfResolvedName);
        RNSNameServiceLogic.updatePIndexOfToken(addr_, spo);
    ...
    function setProfileURI(string calldata profileURI_) external {
        ...
        string memory rdf = string(RNSSemanticSBTLogicUpgradeable.buildStringRDFCustom(SOUL_CLASS_NAME, msg.sender.toHexString(), _predicates[PROFILE_URI_PREDICATE_INDEX].name, string.concat('"', profileURI_, '"')));
    ...
    function addr(string calldata name) virtual  external view returns (address){
        uint256 sIndex = _subjectIndex[NAME_CLASS_INDEX][bytes32(bytes(name))];
    ...
    function nameOf(address addr_) external view returns (string memory){
        ...
        return string(abi.encodePacked(_subjects[sIndex].value));
    }

    function nameOfTokenId(uint256 tokenId) external view returns (string memory){
        return string(abi.encodePacked(_subjects[tokenId].value));
    }
    ...
    virtual
    view
    override(RNSSemanticSBTUpgradeable)
    returns (string memory)
    {
        require(
            _exists(tokenId),
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(RNSSemanticSBTUpgradeable) returns (bool) {
//        return interfaceId == type(INameService).interfaceId ||
        return super.supportsInterface(interfaceId);
    }


    function _register(address owner, bytes32 name, bool resolve) internal returns (uint tokenId) {
        bytes32 fullName = bytes32(bytes.concat(name, _suffix));
        require(_subjectIndex[NAME_CLASS_INDEX][fullName] == 0, "NameService: already added");
        tokenId = _addEmptyToken(owner, 0);
        uint256 sIndex = RNSSemanticSBTLogicUpgradeable._addSubject(fullName, NAME_CLASS_INDEX, _subjects, _subjectIndex);
        SubjectPO[] memory subjectPOList = RNSNameServiceLogic.register(msg.sender, owner, sIndex, resolve,
