...
import './interfaces/IAntbTreasury.sol';
import './interfaces/IAntbStaking.sol';
import './interfaces/IAntbBondingCalculator.sol';
...
contract AntbBondDepository is Ownable {
    ...
    /* ======== STATE VARIABLES ======== */
    address public immutable ANTB; // token given as payment for bond
    address public immutable principle; // token used to create bond
    address public immutable treasury; // mints Antb when receives principle
    ...
    struct Bond {
        uint256 payout; // ANTB remaining to be paid
    ...
    function deposit(
        ...
        console.log('before decayDebt');
        decayDebt();
        require(totalDebt <= terms.maxDebt, 'Max capacity reached');
        console.log('after decayDebt');

        console.log('before bondPriceInUSD');
        uint256 priceInUSD = bondPriceInUSD(); // Stored in bond info
        console.log('after bondPriceInUSD');
        console.log('before _bondPrice');
        uint256 nativePrice = _bondPrice();
        console.log('after _bondPrice');
        ...
        console.log('before IAntbTreasury(treasury).valueOfToken');
        uint256 value = IAntbTreasury(treasury).valueOfToken(
            principle,
            _amount
        );
        console.log('after IAntbTreasury(treasury).valueOfToken');
        console.log('before payoutFor');
        uint256 payout = payoutFor(value); // payout to bonder is computed
        console.log('after payoutFor');

        require(payout >= 10000000, 'Bond too small'); // must be > 0.01 ANTB ( underflow protection )
        console.log('before maxPayout');
        require(payout <= maxPayout(), 'Bond too large'); // size protection because there is no slippage
        console.log('after maxPayout');
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) ANTB
         */
        IERC20(principle).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(principle).approve(address(treasury), _amount);
        IAntbTreasury(treasury).deposit(_amount, principle, profit);

        if (fee != 0) {
            // fee is transferred to dao
            IERC20(ANTB).safeTransfer(DAO, fee);
        }
        ...
        if (percentVested >= 10000) {
            ...
            bondInfo[_recipient] = Bond({
                ...
                vesting: info.vesting.sub(
                    block.timestamp.sub(info.lastTimestamp)
                ),
                lastTimestamp: block.timestamp,
    ...
    function stakeOrSend(
        ...
        if (!_stake) {
            ...
            IERC20(ANTB).transfer(_recipient, _amount); // send payout
        } else {
            // if user wants to stake
            if (useHelper) {
                // use if staking warmup is 0
                IERC20(ANTB).approve(stakingHelper, _amount);
    ...
    function adjust() internal {
        uint256 blockCanAdjust = adjustment.lastTimestamp.add(
            adjustment.buffer
        );
    ...
    function maxPayout() public view returns (uint256) {
        return IERC20(ANTB).totalSupply().mul(terms.maxPayout).div(100000);
    }
    ...
    function bondPriceInUSD() public view returns (uint256 price_) {
        if (isLiquidityBond) {
            price_ = bondPrice()
                .mul(IAntbBondingCalculator(bondCalculator).markdown(principle))
                .div(100);
        } else {
    ...
    /**
     *  @notice calculate current ratio of debt to ANTB supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns (uint256 debtRatio_) {
        uint256 supply = IERC20(ANTB).totalSupply();
    ...
    function standardizedDebtRatio() external view returns (uint256) {
        if (isLiquidityBond) {
            return
                debtRatio()
                    .mul(
                        IAntbBondingCalculator(bondCalculator).markdown(
                            principle
                        )
                    )
                    .div(1e9);
    ...
    /**
     *  @notice calculate amount of ANTB available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or ANTB) to the DAO
     *  @return bool
     */
...
interface IAntbBondingCalculator {
