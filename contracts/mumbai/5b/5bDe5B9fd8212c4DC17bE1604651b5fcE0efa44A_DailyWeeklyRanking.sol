...
contract DailyWeeklyRanking is Ownable{
    ...
    mapping(uint256 => uint256) public zaiWins;
    mapping(uint256 => uint256) public zaiDefeats;

    event DailyChallengeResult(ChallengeDatas[10] ranking, uint256 indexed timestamp, uint256 dayNumber);
    event WeeklyChallengeResult(ChallengeDatas[10] ranking, uint256 indexed timestamp, uint256 weekNumber);
    ...
    function _updateDayAndWeekBeginning() internal returns(bool){
        if(block.timestamp >= dayBeginingTimestamp + 1 days){
            ...
            _dayNumber +=1;
            emit DailyChallengeResult(_dailyChallenge, block.timestamp, _dayNumber);
            _payDailyWinners();
        }
        if(block.timestamp >= weekBeginingTimestamp + 7 days){
            ...
            _weekNumber +=1;
            emit WeeklyChallengeResult(_weeklyChallenge, block.timestamp, _weekNumber);
            _payWeeklyWinners();
        }
        return true;
    }
    ...
    function _payDailyWinners() internal {
        if(_dailyChallenge[0].user == address(0x0)){
            return;
        }
        ...
        uint256 _rewards = IRewardsRankingFound(_inGameRewardAddress).getDailyRewards(paymentAddress);
        ...
        for(uint256 i = 0 ; i< 10 ;){
            if(_dailyChallenge[i].user != address(0x0)){
                ...
                require(IPayments(paymentAddress).rewardPlayer(_user,_toSend));
                unchecked{ ++i; }
            }
        }
    }
    ...
    function _payWeeklyWinners() internal {
        if(_weeklyChallenge[0].user == address(0x0)){
            return;
        }
        address _inGameRewardAddress = gameAddresses.getChallengeRewardsAddress();
        address paymentAddress = gameAddresses.getPaymentsAddress();        
        uint256 _rewards = IRewardsRankingFound(_inGameRewardAddress).getWeeklyRewards(paymentAddress);
