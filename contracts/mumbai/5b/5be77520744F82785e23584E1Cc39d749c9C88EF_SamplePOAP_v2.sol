 ...
 */
abstract contract ERC2771RecipientUpgradeable is IERC2771RecipientUpgradeable {
 ...
 */
abstract contract IERC2771RecipientUpgradeable {
...
import "./SamplePOAP.sol";

contract SamplePOAP_v2 is SamplePOAP {
    ...
    function _burnSinglePoap(address tokenOwner, uint256 tokenId) internal virtual {
        if (ownerOf(tokenId) != tokenOwner) revert("Invalid owner.");

        // Get POAP ID from the `tokenId`
...
import "../ERC2771RecipientUpgradeable.sol";
import "../Version.sol";
    ...
    Version
{
    ...
    /// Emitted when POAP is burned from owner
    ...
    /// Emitted when trusted forwarder contract is updated
    event TrustedForwarderUpdated (address indexed trustedForwarder);
    ...
    function setTrustedForwarder (address trustedForwarder) external virtual onlyRole(ADMIN_ROLE) whenNotPaused {
        ERC2771RecipientUpgradeable._setTrustedForwarder(trustedForwarder);
    ...
    function getPoapURI(uint256 poapId) public view virtual returns (string memory) {
        if (bytes(_poapUris[poapId]).length == 0) revert("The POAP ID does not exist.");
    ...
    function _mintSinglePoap(address to, uint256 poapId) internal virtual {
        if (bytes(_poapUris[poapId]).length == 0) revert("URI does not exist.");

        // Cache next token ID to be minted as `tokenId`
        uint256 tokenId = _nextTokenId();

        // Map `poapId` to `tokenId`
    ...
    /// Overrides required by {ERC2771RecipientUpgradeable}
    function _msgSender() internal view override(ContextUpgradeable, ERC2771RecipientUpgradeable) returns (address) {
        return ERC2771RecipientUpgradeable._msgSender();
    }

    /// Overrides required by {ERC2771RecipientUpgradeable}
    function _msgData() internal view override(ContextUpgradeable, ERC2771RecipientUpgradeable) returns (bytes calldata) {
...
abstract contract Version is Initializable {
    uint8 internal _version;

    ///
    function __Version_init(uint8 version) internal onlyInitializing {
        _version = version;
    }

    ///
    function __Version_init_unchained() internal onlyInitializing {
    }
