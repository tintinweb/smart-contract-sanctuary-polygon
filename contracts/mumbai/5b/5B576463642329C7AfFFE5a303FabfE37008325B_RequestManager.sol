    ...
    function processAaveDistributionForDraw(address registry_) public payable returns(bool) {
        if(tokenAddress == address(0)) {
            if(address(this).balance > totalAvailableLiquidity) {
                ...
                emit PostDrawDistribution(address(this),betCreator, betTaker, IConfig(registry_).getAdmin(), _bal/2, _bal/2, 0);
            }
        } else {
            ...
            if(_currentBal > totalAvailableLiquidity) {
                ...
                emit PostDrawDistribution(address(this),betCreator, betTaker, IConfig(registry_).getAdmin(), _currentBal/2, _currentBal/2, 0);
            }
        }
