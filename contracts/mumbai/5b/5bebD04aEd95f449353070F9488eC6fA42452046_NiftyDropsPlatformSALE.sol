...
contract NiftyDropsPlatformSALE is NinfyDropsInterface, ERC721, Ownable {
    ...
    modifier onlyWithdrawalWallet() {
        ...
        require(address(withdrawalWallet) == _msgSender(), 
            ...
            "NiftyDropPlatformSale: caller is not the withdrawal wallet");
    ...
    function setTicketAddress(address _ticketAddress) external onlyOwner override {
        ...
        require(_ticketAddress != address(0), "NiftyDropPlatform::setTicketAddress: Invalid address.");
    ...
    function presaleBuy(uint256 _ticketId, uint _amount) external payable override {
        ...
        require(presaleActive, "NiftyDropPlatform::presaleBuy: Presale is not Active.");

        require(ERC721(ticketAddress).ownerOf(_ticketId) == _msgSender(), "NiftyDropPlatform::presaleBuy: invalid ticket.");

        require(!usedTickets[_ticketId], "NiftyDropPlatform::presaleBuy: Ticket already used.");

        require(_amount <= mintLimit, "NiftyDropPlatform::presaleBuy: Too many tokens for one transaction.");

        require(msg.value >= mintPrice.mul(_amount), "NiftyDropPlatform::presaleBuy: Insufficient payment.");
    ...
    function whitelistBuy(uint256 _amount) external payable override {
        ...
        require(whitelistSaleActive, "NiftyDropPlatformSale::whitelistBuy: Whitelist Buy is not Active.");

        require(_amount <= mintLimit, "NiftyDropPlatform::presaleBuy: Too many tokens for one transaction.");

        require(msg.value >= mintPrice.mul(_amount), "Dropspace::presaleBuy: Insufficient payment.");

        require(whitelist[_msgSender()], "NiftyDropPlatform::whitelistBuy: You are not whitelisted."); 
    ...
    function clearTicket(uint256 _ticketId) external override onlyOwner{
        ...
        require(usedTickets[_ticketId], "NiftyDropPlatform::clearTicket: Ticket is not used");
