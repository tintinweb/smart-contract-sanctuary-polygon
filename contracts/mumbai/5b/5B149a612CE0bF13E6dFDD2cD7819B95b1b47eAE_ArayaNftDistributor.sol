...
contract ArayaNftDistributor is AccessControl, Pausable, Withdrawable, ReentrancyGuard, NativeMetaTransaction, ContextMixin {
    ...
    */
    event TokenClaimed(address indexed holderAccount, address indexed account, uint256 id, uint256 amount);
    ...
    constructor(string memory domainName, address arayaNftContractAddress, address fearContractAddress) {
    ...
    */
    ...
    function claimTokens(address holderAddress, address fearWalletAddress, uint256 id, bytes memory signature) external {
        uint256 amount = claimableTokens[holderAddress][id];
        ...
        bool validSignature = SignatureChecker.isValidSignatureNow(holderAddress, messageHash, signature);
        require(validSignature, "Signature check failure");
        
        claimableTokens[holderAddress][id] = 0;
        arayaNftContract.initialMint(fearWalletAddress, id, amount, "");
        emit TokenClaimed(holderAddress, fearWalletAddress, id, amount);
    }
