...
contract DetonatedToonzV0 is Erc721V1, Erc721aMigrationToolsV1 {
    error MigrationNotCompleteYet();
    error BitMapValuesMustHaveSameLength();
    ...
    function initialize(
        ...
        DetonatedToonzStorageV1.layout()._degenToonzCollection = _degenToonzCollection;
        DetonatedToonzStorageV1.layout()._gmGnDynamiteCollection = _gmGnDynamiteCollection;
    }
    ...
    function importDetonationStatuses(uint256[] memory _normalBitMapValues, uint256[] memory _radioactiveBitMapValues) public onlyOwner {
        if (_normalBitMapValues.length != _radioactiveBitMapValues.length) {
            revert BitMapValuesMustHaveSameLength();
        }

        uint256 i = 0;
        for (;;) {
            DetonatedToonzStorageV1.layout()._detonationStatusesBitMap[1][i] = _normalBitMapValues[i];
            DetonatedToonzStorageV1.layout()._detonationStatusesBitMap[2][i] = _radioactiveBitMapValues[i];

            if (_normalBitMapValues.length == ++i) break;
        }
    }
