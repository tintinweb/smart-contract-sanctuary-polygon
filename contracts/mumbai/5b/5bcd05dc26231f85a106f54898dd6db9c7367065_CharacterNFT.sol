...
contract CharacterNFT is Ownable, ERC721Enumerable, Pausable, AccessControl {
    ...
    mapping(string => uint256) public groupToQuantity;

    mapping(string => mapping(address => uint256)) public groupToPrice;

    mapping(string => uint256[2]) public groupToTime;
    // end whitelisted mapping

    // event free
    mapping(string => uint256) public freeCharacterQ;

    mapping(string => uint256[2]) public freeGroupToTime;

    mapping(string => uint256) public groupToLockUp;
    //
    ...
    event getCharacterEvent(address buyer, uint256[] tokenIds);
    event settingEvent(string eventType, string groupId, uint256[2] price, uint256[2] duration, uint256 quantity, uint8 pharse);
    ...
    function setPresale(string memory _group, address[2] memory tokenAddress,uint256[2] memory _price, uint256[2] memory duration, uint256 _quantity, uint8 pharse) external onlyRole(MANAGERMENT_ROLE){
        ...
        if(presaleTime[0] == 0 || duration[0] < presaleTime[0]){
            presaleTime[0] = duration[0];
        }
        if(presaleTime[1] < duration[1]){
            presaleTime[1] = duration[1];
        }

        emit settingEvent("presale", _group, _price, presaleTime, _quantity, pharse);
    }

    function setPublicSale(address[2] memory tokenAddress, uint256[2] memory _price, uint256[2] memory duration, uint256 _quantity, uint8 pharse) external onlyRole(MANAGERMENT_ROLE){
        ...
        emit settingEvent("publicSale", "", _price, publicSaleTime, _quantity, pharse);
    }

    function setEvent(string memory _group, uint256[2] memory duration, uint256 _quantity, uint256 lockUpTime, uint8 pharse) external onlyRole(MANAGERMENT_ROLE){
        freeCharacterQ[_group] = _quantity;
        freeGroupToTime[_group] = [duration[0], duration[1]];
        if(eventTime[0] == 0 || duration[0] < eventTime[0]){
            eventTime[0] = duration[0];
        }
        if(eventTime[1] < duration[1]){
            eventTime[1] = duration[1];
        }
        groupToLockUp[_group] = lockUpTime;
        emit settingEvent("freeEvent", _group, [uint256(0), uint256(0)], eventTime, _quantity, pharse);
    }
    ...
    function getFreeCharacter() external isWhitelisted{
        ...
        uint256[] memory listToken = new uint256[](8);
        // mint one by one
        for (uint256 i = 0; i < 8; i++) {
            uint256 tokenId = _tokenIdCount.current();
            _mint(msg.sender, tokenId);
            availableTime[tokenId] = block.timestamp + groupToLockUp[groupWhitelist[msg.sender]];
            isFreeNFT[tokenId] = true;
            listToken[i] = tokenId;
            _tokenIdCount.increment();
        }
        emit getCharacterEvent(msg.sender, listToken);
    }
