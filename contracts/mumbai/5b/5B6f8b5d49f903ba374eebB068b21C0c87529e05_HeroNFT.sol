...
contract HeroNFT is ERC721URIStorage, Ownable {
    ...
    /// @dev An array containing the Herosß struct for all Heros in existence. The ID
    ///  of each hero is actually an index into this array. Note that ID 0 is a negahero,

    Hero[] public heros;

    /// @dev A mapping from hero IDs to the address that owns them. All heros have
    ...
    /// @dev A mapping from Hero IDs to an address that has been approved to call
    ...
    // Events
    event NewHero(address indexed owner, uint256 id); 
    ...
    {
        require(
            fee <= IERC20(tokenAddress).balanceOf(msg.sender),
            "You token balance is too low"
        );
