...
contract SmartPayrollByTime is AutomationCompatible{
  ...
  constructor(contractParams memory _contractParams, address _factory,address _DAOAddress) {
    ...
    i_registry = IAutomationRegistryInterface(_contractParams._registry);

    upkeeperState = State.active;
  ...
  function checkUpkeep(
    bytes calldata /* checkData */
      ...
      if(upkeeperState == State.active) {
        upkeepNeeded = (block.timestamp - lastTimeStamp) > i_interval ;
      }else if(upkeeperState == State.cancel){
          upkeepNeeded = block.number- lastBlockNumber>50;
      }else{
        upkeepNeeded = false;
      }
  ...
  function performUpkeep(bytes calldata /* performData */) external override {
    ...
    if(upkeeperState == State.active &&  counter <i_round){
      if (i_ERC20Token == address(0)) {
        require(address(this).balance > i_amount, "ETH not enough");
        payable(i_receiver).transfer(i_amount);
      } else {
        require(IERC20(i_ERC20Token).balanceOf(address(this)) >= i_amount, "Not enough token");
        IERC20(i_ERC20Token).transfer(i_receiver, i_amount);
      }
      i_factory.creditMint(i_credit,i_sender,i_receiver);
      lastTimeStamp = block.timestamp;
      counter = counter + 1;
    }else if(upkeeperState == State.active &&  counter >=i_round){
      upKeepID = i_factory.getUpkeeperID(address(this));
      i_registry.cancelUpkeep(upKeepID);
      lastBlockNumber = block.number;
      upkeeperState = State.cancel;
    }else{
      upKeepID = i_factory.getUpkeeperID(address(this));
      i_registry.withdrawFunds(upKeepID,i_sender);
    }
  ...
  function contractDestruct() external {
      ...
      if (i_ERC20Token == address(0)) {
        ...
        require(token.balanceOf(address(this)) >= i_amount, "Not enough token");
...
contract SmartPayrollFactory {
  ...
  function createUpkeepContract(
    ...
    SmartPayrollByTime smartPayrollByTime = new SmartPayrollByTime(_upkeepContractParams,address(this),address(daoVault));
