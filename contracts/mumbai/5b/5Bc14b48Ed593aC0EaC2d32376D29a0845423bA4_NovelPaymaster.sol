...
contract NovelPaymaster is BasePaymaster {
    ...
    function enableContract(address target) external onlyOwner {
        emit ContractWhitelisted(target);
...
contract NovelCollection is Ownable, IERC721, ERC721Enumerable, ERC2981, BaseRelayRecipient, ContextMixin, NativeMetaTransaction {
    ...
    mapping(address => uint32) whitelistPerAddress;
    mapping(address => uint32) mintPerAddress;
    mapping(string => uint32) whitelistedClaimIds;
    mapping(string => bool) mintedClaimIds;
    ...
    constructor(
        // Name of collection (ex: A Novel Collection)
        string memory _name,
        // Max amount of that token (ex: 10000)
        uint32 _supplyCap,
        // proves that metadata is valid (ex: like an MD5 hash of the metadata json)
        string memory _metadataProofHash,
        // URI for collection metadata (ex: https://novel.shop/metadata/x102129)
        string memory __cURI,
        // GSN Forwarder address for gasless txns
        address _forwarder,
        // Paymaster address to cover gas
        address _paymasterContractAddress,
        ...
        ERC721(_name, "")
    {
        ...
        // whitelist this contract for paymaster
        address(_paymasterContractAddress).delegatecall(
    ...
    function whitelistClaimAmount(
        ...
        require(
            ...
            "NovelCollection: claim ID has already been whitelisted"
        );

        _whitelistedTotal = _whitelistedTotal + amount;
        whitelistPerAddress[customer] += amount;
    ...
    function hasClaimIdBeenMinted(string memory claimId)
        public
        view
        returns (bool)
    {
    ...
    function whitelistedAmount(address customer) public view returns (uint32) {
        return whitelistPerAddress[customer];
    }

    function mintedAmount(address customer) public view returns (uint32) {
        return mintPerAddress[customer];
    }
    ...
    function mint(uint32 amount, string memory claimId) external {
        ...
        require(
            whitelistedClaimIds[claimId] != 0,
            "NovelCollection: claim ID has not been whitelisted"
        );
        ...
        require(
            ...
            string(
                abi.encodePacked(
                    "NovelCollection: incorrect amount for claim ID, received ",
                    amount,
                    " but expected ",
                    whitelistedClaimIds[claimId]
                )
            )
        );

        require(
            !mintedClaimIds[claimId],
            "NovelCollection: claim ID has already been minted"
        );

        mintedClaimIds[claimId] = true;
        for (uint256 i = 0; i < amount; i++) {
            _nextTokenId++; // this begins token IDs at 1
            mintPerAddress[customer]++;
