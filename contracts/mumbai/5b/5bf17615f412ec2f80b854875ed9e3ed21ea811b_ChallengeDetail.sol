...
contract ChallengeDetail is IERC721Receiver {
    ...
    */
    ...
    modifier verifySignature(
        ...
        require(
            securityAddress == verifyString(message, v, r, s) &&
    ...
    /**
     * @dev Allows users to withdraw ERC20, ERC721, and ERC1155 tokens after the completion of a project
     * @param _listTokenErc20 An array of ERC20 token addresses
     * @param _listToken721 An array of ERC721 token addresses
     * @param _listToken1155 An array of ERC1155 token addresses
     * @param _listIndexNFT721 An array of arrays containing the indices of ERC721 tokens to be withdrawn
     * @param _listIndexNFT1155 An array of arrays containing the indices of ERC1155 tokens to be withdrawn
     */
    function withdrawTokensOnCompletion(
        address[] memory _listTokenErc20,
        address[] memory _listToken721,
        address[] memory _listToken1155,
        uint256[][] memory _listIndexNFT721,
        uint256[][] memory _listIndexNFT1155
    ) external {
        ...
        require(
            _listToken721.length == _listIndexNFT721.length,
            "List token erc721 not correct"
        );
        for (uint256 i = 0; i < _listToken721.length; i++) {
            address token721 = _listToken721[i];
            uint256[] memory indexNFT721 = _listIndexNFT721[i];

            for (uint256 j = 0; j < indexNFT721.length; j++) {
        ...
        require(
            _listToken1155.length == _listIndexNFT1155.length,
            "List token erc1155 not correct"
        );
        for (uint256 i = 0; i < _listToken1155.length; i++) {
            address token1155 = _listToken1155[i];
            uint256[] memory indexNFT1155 = _listIndexNFT1155[i];
            IERC1155 erc1155 = IERC1155(token1155);

            for (uint256 j = 0; j < indexNFT1155.length; j++) {
                uint256 balanceERC1155 = erc1155.balanceOf(
                    address(this),
                    indexNFT1155[j]
                );
    ...
    function transferToListReceiverSuccess() private {
        ...
        address[] memory authorizedListAddress = authorizedERC721Contracts;
        for (uint256 j = 0; j < authorizedListAddress.length; j++) {
            if (
                IExerciseSupplementNFT(erc721Address[0]).typeNfts(
                    authorizedListAddress[j]
                )
            ) {
                for (
                    uint256 i = 0;
                    i < getIndexToken(authorizedListAddress[j]);
                    i++
                ) {
                    if (
                        getOwnerOfNft(authorizedListAddress[j], i) ==
                        address(this)
                    ) {
                        TransferHelper.safeTransferFrom(
                            authorizedListAddress[j],
                ...
                for (
                    ...
                    uint256 balanceERC1155 = IERC1155(authorizedListAddress[j])
                        .balanceOf(address(this), i);
                    if (balanceERC1155 > 0) {
                        ...
                        // Withdraw ERC1155 tokens by transferring the specified balances to the challenger
                        TransferHelper.safeTransferNFT1155(
                            authorizedListAddress[j],
                            address(this),
                            challenger,
    ...
    function getIndexToken(
        ...
        return IExerciseSupplementNFT(_erc721Address).nextTokenIdToMint();
    }
    ...
    function transferNFTWhenFailed(address _erc721Address) private {
        ...
        for (uint256 j = 0; j < authorizedERC721Contracts.length; j++) {
            // Check if the NFT contract is an ERC721 or ERC1155 contract
            if (
                IExerciseSupplementNFT(_erc721Address).typeNfts(
                    authorizedERC721Contracts[j]
                )
            ) {
                // Check if the contract has any NFTs
                if (
                    IExerciseSupplementNFT(authorizedERC721Contracts[j])
                    ...
                    if (
                        ...
                        for (
                            uint256 i = 0;
                            i < getIndexToken(authorizedERC721Contracts[j]);
                            i++
                        ) {
                            // Check if the current contract address is the current owner of the NFT
                            address receiver = IExerciseSupplementNFT(
                                authorizedERC721Contracts[j]
                            ).getHistoryNFT(i, address(this));
                            if (receiver == address(0)) {
                                receiver = returnedNFTWallet;
                            }
                            if (
                                getOwnerOfNft(
                                    authorizedERC721Contracts[j],
                ...
                for (
                    ...
                    uint256 balanceTokenERC1155 = IERC1155(
                        authorizedERC721Contracts[j]
                    ).balanceOf(address(this), i);
