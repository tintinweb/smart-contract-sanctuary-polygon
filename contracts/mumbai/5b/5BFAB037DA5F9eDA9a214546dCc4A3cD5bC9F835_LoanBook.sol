...
contract LoanBook is ILoanBook, SecuritizeConstants, UUPSUpgradeable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    modifier availableLoan(bytes32 loanId) {
    ...
    function loanDetail(bytes32 loanId) external view availableLoan(loanId) returns (DataTypes.Loan memory) {
        require(loans[loanId].available, "loan not found");
        return loans[loanId];
    }
    ...
    function borrowLoan(
        ...
        _borrowLoan(loanId, amount, legalContractHash, investorWallet);
    }
    ...
    function executePreApprovedTransaction(
        ...
        noncePerInvestor[investorId] = noncePerInvestor[investorId] + 1;
    ...
    function _borrowLoan(
        ...
        if (loan.isDsTokenLoan) {
            ...
            revert('Stable coin borrowings are not supported');
        }
    ...
    function repay(bytes32 borrowingId) external whenNotPaused onlyBorrower(borrowingId) {
        ...
        uint256 annualInterest = borrowing.collateralAmount * loan.apy / PERCENTAGE_FACTOR;
        // Borrowing time duration in seconds
        uint256 borrowingTime = block.timestamp - borrowing.creationTime;
        if (borrowingTime > loan.period) {
            borrowingTime = loan.period;
        }

        uint256 borrowingInterest = borrowingTime * annualInterest / SECONDS_IN_ONE_YEAR;
        uint256 paybackAmount = borrowing.collateralAmount - borrowingInterest;
    ...
    function _addLoan(
        ...
        require(isDsTokenLoan, 'feature not implemented');
        ...
        bool okTransfer = dsToken.transferFrom(msg.sender, address(this), amount);
        require(okTransfer, 'error transfer tokens to loan-book');
    ...
    function _removeLoan(bytes32 loanId) internal {
        ...
        emit RemoveLoan(loan.lender, loanId, loan.amount);
    }
