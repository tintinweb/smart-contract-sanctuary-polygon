...
contract NebulaMarketplaceContract is Ownable, ERC1155Holder, ReentrancyGuard {
    ...
    uint256 _nonExistingIndex = 1000000; /* non-existing token...*/
    ...
    modifier onlyItemOrMarketplaceOwner(address _contractAddress, uint256 _id) {
        ...
        require(
            isAllowed == true,
            "Only product or contract owner can do this operation"
        );
        _;
    }
    ...
    function listSingleToken(
        ...
        NebulaDropsCollectible tokenContract = NebulaDropsCollectible(
            _contractAddress
        );
        ...
        require(
            tokenContract.balanceOf(msg.sender, _tokenId) >= _amount,
        ...
        require(
            tokenIdToListing[_contractAddress][_tokenId].contractAddress !=
                _contractAddress,
            "Token is already listed for this Smart Contract!"
        );
        ...
        uint256 _oldEntryIndex = _nonExistingIndex; /* non-existing token...*/
        for (uint256 i = 0; i < listingsArray.length; i++) {
            if (
                listingsArray[i].contractAddress == _contractAddress &&
                listingsArray[i].tokenId == _tokenId
            ) {
                _oldEntryIndex = i;
            }
        }

        if (_oldEntryIndex != _nonExistingIndex) {
            listingsArray[_oldEntryIndex] = tokenIdToListing[_contractAddress][
                _tokenId
            ];
        } else {
    ...
    /* Creates the sale of a marketplace item */
    /* Transfers ownership of the items between parties */
    function listTokensRange(
        address _contractAddress,
        ...
        for (uint256 i = _firstTokenId; i <= _lastTokenId; i++) {
            ...
            require(
                tokenContract.balanceOf(msg.sender, tokenId) >= _amount,
                "Caller must own given token!"
            );
            require(
                tokenIdToListing[_contractAddress][tokenId].contractAddress !=
                    _contractAddress,
                "Token is already listed for this Smart Contract!"
            );
    ...
    function buyToken(
        ...
        if (
            tokenIdToListing[_contractAddress][_tokenId].privateListing == true
        ) {
            bool whitelisted = false;
            for (
                uint256 i = 0;
                i < tokenIdToListing[_contractAddress][_tokenId].buyers.length;
                i++
            ) {
                if (
                    tokenIdToListing[_contractAddress][_tokenId].buyers[i] ==
                    msg.sender
                ) {
                    whitelisted = true;
                }
        ...
        require(
            msg.value >=
                (tokenIdToListing[_contractAddress][_tokenId].price * _amount) *
                    (10**18),
            "Insufficient funds!"
        );
        require(
            tokenContract.balanceOf(
                address(this),
                tokenIdToListing[_contractAddress][_tokenId].tokenId
            ) >= _amount,
            "Seller doesn't have enough tokens!"
        );
        require(
            tokenIdToListing[_contractAddress][_tokenId].sold == false,
            "NFTListing not available anymore!"
        );
        require(
            tokenIdToListing[_contractAddress][_tokenId].tokensAvailable >=
                _amount,
            "Not enough tokens left!"
        );

        uint256 netAmount = (tokenIdToListing[_contractAddress][_tokenId]
            .price * _amount) * (10**18);

        // Get the creator share
        (address creatorAddress, uint256 creatorShare) = getCreatorShare(
            tokenIdToListing[_contractAddress][_tokenId].contractAddress,
            _tokenId,
            netAmount
        );
        ...
        require(
            ...
            "Do not have valid Owner detail."
        );
        ...
        if (
            tokenIdToListing[_contractAddress][_tokenId].privateListing == false
        ) {
            tokenIdToListing[_contractAddress][_tokenId].buyers.push(
                msg.sender
            );
        }
        ...
        tokenIdToListing[_contractAddress][_tokenId].owner = payable(
            msg.sender
        );
        ...
        require(
            _listIndex != _nonExistingIndex, /* non-existing token...*/
            "Invalid Token detail!"
        );
        listingsArray[_listIndex] = tokenIdToListing[_contractAddress][
            _tokenId
        ];
        ...
        (bool successPlatform, ) = (owner()).call{
            value: (netAmount * platformFee) / 100
        }(""); //  Deducting Platform fee for this transaction.
        require(successPlatform, "Failed to deduct Platform fees!");

        // Deducting Artist share for this transaction.
        (bool successArtistShare, ) = _seller.call{
            value: (netAmount * sellerShare) / 100
        }(""); // Transfering seller's share to the Owner.
    ...
    function buyTokens(
        address _contractAddress,
        uint256 _totalTokensToBuy,
        uint256 _amount
    )
        public
        payable
        nonReentrant //uint256[] memory _tokenIds, uint256[] memory _amounts
    {
        NebulaDropsCollectible tokenContract = NebulaDropsCollectible(
            _contractAddress
        );
        ...
        uint256 _totalPrice = _list[0].price *
            _amount *
            _totalTokensToBuy *
            (10**18);
        ...
        for (
            uint256 idxValidation = 0;
            idxValidation < _totalTokensToBuy;
            idxValidation++
        ) {
            ...
            if (
                tokenIdToListing[_contractAddress][_tokenId].privateListing ==
                true
            ) {
                bool whitelisted = false;
                for (
                    uint256 i = 0;
                    i <
                    tokenIdToListing[_contractAddress][_tokenId].buyers.length;
                    i++
                ) {
                    if (
                        tokenIdToListing[_contractAddress][_tokenId].buyers[
                            i
                        ] == msg.sender
                    ) {
                        whitelisted = true;
        ...
        // Deducting Platform fee for this transaction.
        (bool successPlatform, ) = (owner()).call{
            value: (_totalPrice * platformFee) / 100
        }(""); //  Deducting Platform fee for this transaction.
        require(successPlatform, "Failed to deduct Platform fees!");

        // Deducting Artist Share for this transaction.
        (bool successArtistShare, ) = _seller.call{
            value: (_totalPrice * sellerShare) / 100
        }(""); // Transfering seller's share to the Owner.
        require(successArtistShare, "Failed to deduct Artist Share!");
        ...
        for (uint256 index = 0; index < _totalTokensToBuy; index++) {
            ...
            tokenIdToListing[_contractAddress][_tokenId]
            ...
            uint256 netAmount = (listingsArray[_listIndex].price *
                _tokenAmount) * (10**18); //* _totalTokensToBuy

            //require(1==2, Strings.toString(_list[index].tokenId));
            // Get the creator share
            (address creatorAddress, uint256 creatorShare) = getCreatorShare(
                _list[index].contractAddress,
                _list[index].tokenId,
                netAmount
            );
            ...
            require(
                creatorShare > 0,
                "Creator share should be greater than 0."
            );

            // Deducting Royalty fee for this transaction.
            (bool successRoyalty, ) = creatorAddress.call{value: creatorShare}(
                ""
            ); // Deducting Minter's Royalty fee.
            require(successRoyalty, "Failed to deduct Royalty fees!");
            tokenContract.safeTransferFrom(
                address(this),
                msg.sender,
...
/* Resell Token */
        ...
        returns (uint256)
    {
        ...
        ) /* New Entry...*/
        {
            ...
            tokenIdToListing[_contractAddress][_tokenId].sold = false;
            tokenIdToListing[_contractAddress][_tokenId].deleted = false;
        }

        /* Find listArrayIndex for the tokenId..*/
        uint256 _listIndex = listArrayIndexByTokenId(_tokenId);

        if (_listIndex == _nonExistingIndex) /* non-existing token...*/
        {
            listingsArray.push(tokenIdToListing[_contractAddress][_tokenId]);
        } else {
            listingsArray[_listIndex] = tokenIdToListing[_contractAddress][
    ...
    /* Delete Listed Item */
    function deleteListedItem(address _contractAddress, uint256 _tokenId)
        public
        onlyItemOrMarketplaceOwner(_contractAddress, _tokenId)
    {
        ...
        require(
            tokenContract.balanceOf(address(this), _tokenId) >= 1,
            "Caller must own given token!"
        );
        require(
            tokenIdToListing[_contractAddress][_tokenId].sold == false,
        ...
        tokenContract.safeTransferFrom(
            address(this),
            tokenIdToListing[_contractAddress][_tokenId].owner,
            _tokenId,
            tokenIdToListing[_contractAddress][_tokenId].tokensAvailable,
            ""
        );
        ...
        //tokenIdToListing[_contractAddress][_tokenId].sold = false;
        tokenIdToListing[_contractAddress][_tokenId].tokensAvailable = 0;
        tokenIdToListing[_contractAddress][_tokenId].deleted = true;
        ...
        //listingsArray[_listIndex].sold = false;
        listingsArray[_listIndex].tokensAvailable = 0;
        listingsArray[_listIndex].deleted = true;
    ...
    function deleteAllListedItems(address _contractAddress) public onlyOwner {
        // require(msg.sender == tokenIdToListing[_tokenId].owner, "Not caller's listing!");
    ...
    function getCreatorShare(
        ...
        if (
            ...
            ) = NebulaDropsCollectible(contractAddress).royaltyInfo(
                    tokenId,
                    tokenPrice
                );
    ...
    function fetchStoredTokenDetail(address _contractAddress, uint256 _id)
        public
        view
    ...
    function fetchAllMarketItems(address _contractAddress)
        public
        view
        onlyOwner
        returns (NFTListing[] memory)
    ...
    function fetchAvailableMarketItems(address _contractAddress)
        public
        view
        returns (NFTListing[] memory)
    {
        ...
        for (uint256 i = 0; i < listingsArray.length; i++) {
            if (
                listingsArray[i].contractAddress == _contractAddress &&
                listingsArray[i].sold == false &&
                listingsArray[i].deleted == false &&
                listingsArray[i].tokensAvailable > 0
            ) {
                itemCount += 1;
            }
        }
        ...
        for (uint256 index = 0; index < listingsArray.length; index++) {
            if (
                listingsArray[index].contractAddress == _contractAddress &&
                listingsArray[index].sold == false &&
                listingsArray[index].deleted == false &&
                listingsArray[index].tokensAvailable > 0
            ) {
                NFTListing storage currentItem = listingsArray[index];
                items[currentIndex] = currentItem;
                currentIndex += 1;
    ...
    function listArrayIndexByTokenId(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        return _nonExistingIndex; /* non-existing token...*/
    }
