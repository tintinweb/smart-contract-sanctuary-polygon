...
contract DNAMigrationFacet{
    ...
    function registerNewLandType(
        ...
        if(!_overwriteExistingData) {
            require(!ds.registeredLandTypes[_landTypeId], "DNAMigrationFacet: This land type is already registered");
        }
        ds.rarityByLandType[_landTypeId] = _rarity;
        ds.classByLandType[_landTypeId] = _class;
        ds.classGroupByLandType[_landTypeId] = _classGroup;
        ds.limitedEditionByLandType[_landTypeId] = _limitedEdition;
        ds.registeredLandTypes[_landTypeId] = true;
    }

    function debugGetRegisteredLandType(uint256 _landTypeId) external view returns (bool) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        return ds.registeredLandTypes[_landTypeId];
    }

    function associateLandTypesWithClasses(
        uint256[] memory _landTypes,
        uint256[] memory _classes
    ) external {
        LibDiamond.enforceIsContractOwner();
        uint len = _landTypes.length;
        require(len == _classes.length, "DNAMigrationFacet: Mismatched landType and class array lengths");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 i = 0; i < len; ++i) {
            ds.classByLandType[_landTypes[i]] = _classes[i];
        }
    }

    function associateLandTypesWithClassGroups(
        uint256[] memory _landTypes,
        uint256[] memory _classGroups
    ) external {
        LibDiamond.enforceIsContractOwner();
        uint len = _landTypes.length;
        require(len == _classGroups.length, "DNAMigrationFacet: Mismatched landType and classGroup array lengths");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 i = 0; i < len; ++i) {
            ds.classGroupByLandType[_landTypes[i]] = _classGroups[i];
        }
    }

    function associateLandTypesWithRarity(
        uint256[] memory _landTypes,
        uint256[] memory _rarities
    ) external {
        LibDiamond.enforceIsContractOwner();
        uint len = _landTypes.length;
        require(len == _rarities.length, "DNAMigrationFacet: Mismatched landType and rarity array lengths");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 i = 0; i < len; ++i) {
            ds.rarityByLandType[_landTypes[i]] = _rarities[i];
        }
    }

    function migrateLandsToV1DNA(
        uint256[] memory _tokenIds,
        uint16[] memory _firstNames,
        uint16[] memory _middleNames,
        ...
        require(len == _middleNames.length, "DNAMigrationFacet: Mismatched tokenId and middletName array lengths");
        require(len == _lastNames.length, "DNAMigrationFacet: Mismatched tokenId and lastName array lengths");
        require(LibLandDNA.DNA_VERSION == 1, "DNAMigrationFacet: DNA has upgraded beyond this migration method");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        for(uint256 i = 0; i < len; ++i) {
            uint256 dna = generateDNAForOriginToken(_tokenIds[i]);

            bytes memory firstName = bytes(ds.firstNamesList[_firstNames[i]]);
            require(firstName.length > 0, "DNAMigrationFacet: Unknown first name");

            bytes memory middleName = bytes(ds.middleNamesList[_middleNames[i]]);
            require(middleName.length > 0, "DNAMigrationFacet: Unknown middle name");

            bytes memory lastName = bytes(ds.lastNamesList[_lastNames[i]]);
            require(lastName.length > 0, "DNAMigrationFacet: Unknown last name");

            dna = LibLandDNA._setFirstNameIndex(dna, _firstNames[i]);
            dna = LibLandDNA._setMiddleNameIndex(dna, _middleNames[i]);
            dna = LibLandDNA._setLastNameIndex(dna, _lastNames[i]);

            if(_bypassDNAEvents) {
                //  direct assignment - bypassees safeguards and reporting
                ds.land_dna[_tokenIds[i]] = dna;
            } else {
                //  preferred, but slower - an event is fired for each id
                LibLandDNA._setDNA(_tokenIds[i], dna);
            }
        }
    }
    ...
    function migrateLandsToV1DNAByIDRange(
        ...
        require(_lastTokenId <= 30000, "DNAMigrationFacet: Token ID out of range");
        ...
        for(uint256 tokenId = _firstTokenId; tokenId <= _lastTokenId; ++tokenId) {
            ...
            uint256 dna = generateDNAForOriginToken(tokenId);
    ...
    //  Map tokenId properties from the first 30,000 tokens into v1 DNA
    function generateDNAForOriginToken(uint256 _tokenId) internal returns (uint256) {
        require(_tokenId > 0, "DNAMigrationFacet: TokenId cannot be 0");
        require(_tokenId <= 30000, "DNAMigrationFacet: Cannot migrate tokens above id 30,000");
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256 dna = ds.land_dna[_tokenId];
        uint256 landType = uint256(ds.landTypeByTokenId[_tokenId]);

        if(landType == 0) {
            //  This is a hack for Mumbai...
            //  Since we didn't mint all 30,000 lands, we need to spoof the
            //  landType by using the order of the first 3 presales...
            if(_tokenId <= 1000) {
                landType = 1;
                ...
                landType = 10;
            } else if (_tokenId <= 11000) {
                landType = 1;
            } else if (_tokenId <= 12000) {
                landType = 2;
            } else if (_tokenId <= 13000) {
                landType = 3;
            } else if (_tokenId <= 14000) {
                landType = 4;
            } else if (_tokenId <= 15000) {
                landType = 5;
            } else if (_tokenId <= 16000) {
                landType = 6;
            } else if (_tokenId <= 17000) {
                landType = 7;
            } else if (_tokenId <= 18000) {
                landType = 8;
            } else if (_tokenId <= 19000) {
                landType = 9;
            } else if (_tokenId <= 20000) {
                landType = 10;
            } else if (_tokenId <= 21000) {
                landType = 1;
            } else if (_tokenId <= 22000) {
                landType = 2;
            } else if (_tokenId <= 23000) {
                landType = 3;
            } else if (_tokenId <= 24000) {
                landType = 4;
            } else if (_tokenId <= 25000) {
                landType = 5;
            } else if (_tokenId <= 26000) {
                landType = 6;
            } else if (_tokenId <= 27000) {
                landType = 7;
            } else if (_tokenId <= 28000) {
                landType = 8;
            } else if (_tokenId <= 29000) {
                landType = 9;
        ...
        require(ds.registeredLandTypes[landType], "DNAMigrationFacet: TokenId points to an unregistered landType");

        dna = LibLandDNA._setVersion(dna, LibLandDNA.DNA_VERSION);
        dna = LibLandDNA._setLandType(dna, landType);

        if(_tokenId >= 1 && _tokenId <= 10000) {
            dna = LibLandDNA._setOrigin(dna, 1);    // first presale
        } else if (_tokenId >= 10001 && _tokenId <= 20000) {
            dna = LibLandDNA._setOrigin(dna, 2);    // second presale
        } else if (_tokenId >= 20001 && _tokenId <= 30000) {
            dna = LibLandDNA._setOrigin(dna, 3);    // third presale
        }

        if(landType == 1) {
            dna = LibLandDNA._setRarity(dna, LibLandDNA.RARITY_MYTHIC);    // mythic
        } else if (landType >= 2 && landType <= 4) {
            dna = LibLandDNA._setRarity(dna, LibLandDNA.RARITY_RARE);    // rare
        } else if (landType >= 5 && landType <= 10){
            dna = LibLandDNA._setRarity(dna, LibLandDNA.RARITY_COMMON);    // common
        }

        if(LibLandDNA._getLevel(dna) == 0) {
