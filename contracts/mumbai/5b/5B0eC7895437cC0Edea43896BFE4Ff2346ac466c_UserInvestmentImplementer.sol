...
contract UserInvestmentImplementer {
    ...
    // function setValue(uint16 _tokenId, uint32 _projectId, bool _isCrypto, uint256 _value) internal {
    ...
    // function setValues(uint16[] memory _tokenIds, uint32[] memory _projectIds, bool _isCrypto, uint256[] memory _values) public onlyAllowed {
    //     require(_tokenIds.length == _projectIds.length && _tokenIds.length == _values.length, "UserInvestmentImplementer: Array lengths do not match");
    //     for (uint16 i = 0; i < _tokenIds.length; i++) {
    //         setValue(_tokenIds[i], _projectIds[i], _isCrypto, _values[i]);
    //     }
    // }
