...
contract PepemonMatchmaker is ERC1155Holder, ERC721Holder, Ownable {
    ...
    function imDone() public onlyOwner {
    ...
    function doBattle(uint256 player1deckId, uint256 player2deckId) internal returns (address, address, uint256) {
        ...
        //console.log("doBattle:217");
        return (msg.sender, deckOwner[player2deckId], 1);
    }
}
...
contract PepemonBattle is AdminRole {
    ...
    function createAFkTonOfEvents(uint256 count, address p1Addr, address p2Addr) external {
        //uint256 gas = gasleft();
        for (uint256 i = 0; i< count; ++i) {
            emit BattleCreated(p1Addr, p2Addr, ++_nextBattleId);
        }
        //console.log("createAFkTonOfEvents: used gas:", gas - gasleft());
    }
