...
contract pNewLand is Ownable, Pausable, ERC721Holder, ERC1155Holder {
    ...
    function _claimFromMiner(uint16 _tokenId, bool _unstake) internal returns (uint owed) {
        ...
        if (!_unstake) {
            ...
            bool stolen = false;
            address luckyPirate;
            uint16 randomNumber = _getSomeRandomNumber(_tokenId, 100);

            if (randomNumber <= 5 && _tokenId >= 10000 && !bypassPirateSteal) {
                luckyPirate = _randomPirateOwner(randomNumber);
            ...
            if (randomNumber <= 50 && !bypassMinerUnstakePenalty) {
                _payTax(owed);
                owed = 0;
            }
    ...
    function _randomPirateOwner(uint16 _seed) internal view returns (address) {
        if (totalPirateStaked != 0) {
            return pirateHolders[_getSomeRandomNumber(totalPirateStaked + _seed, pirateHolders.length)]; 
        } else {
            return address(0x0);
        }
    }
