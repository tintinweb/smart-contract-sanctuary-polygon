...
contract RewardPoolRegistryV1 is IRewardPoolRegistryV1 {
    ...
    function createRewardPool(
        ...
        if (shouldCreatePaymaster && _ownerToPaymaster[owner] == address(0)) {
    ...
    function createPaymaster(address pool) external {
        address owner = msg.sender;
        address paymaster = _createAndSetPaymaster(owner);
    ...
    function setPaymaster(address paymaster) external {
        address owner = msg.sender;
        _setPaymaster(owner, paymaster);

        emit PaymasterSet(owner, paymaster);
    }
    ...
    function _setPaymaster(address owner, address paymaster) private {
...
interface IRewardPoolRegistryV1 {
    ...
    /* @dev Emitted when new `paymaster` is set by the `owner` */
    event PaymasterSet(address indexed owner, address paymaster);
