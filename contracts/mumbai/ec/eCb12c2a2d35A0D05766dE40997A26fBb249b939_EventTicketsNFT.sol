...
contract EventTicketsNFT is ERC1155Base, PrimarySale, SignatureMintERC1155 {
    ...
    modifier onlyOrgAdminOrContributorMembers() {
        IOazizOrganization orgSc = IOazizOrganization(organizationOwner);
        require(
            orgSc.hasRole(orgSc.ADMIN_ROLE(), msg.sender)
            || orgSc.hasRole(orgSc.CONTRIBUTOR(), msg.sender),
            "Not enough permission for make a gift"
        );
        _;
    }
    ...
    function makeGift(MintRequest calldata _req) external onlyOrgAdminOrContributorMembers {
     ...
     *  @param _amount  The amount of the NFT to burn.
     */
    function cancelTokenByOrg(
        address _owner,
        uint256 _tokenId,
        uint256 _amount
    ) external onlyOrgAdminOrContributorMembers {
        _burn(_owner, _tokenId, _amount);
    }
    ...
    //
    //   #################### New/overridden internal functions
    //
    ...
    returns (uint256 tokenIdToMint) {
        ...
        if (tokenIdToMint == 0 && _req.tokenId == type(uint256).max) {
            ...
            if (tokenIdToMint == 0) {
                require(_req.tokenId < nextIdToMint, "invalid token id for force mint");
                tokenIdToMint = _req.tokenId;
            }
            else {
                require(tokenIdToMint == _req.tokenId, "Try to force mint with exist externalId to tokenId mapping");
                tokenIdToMint = _req.tokenId;
            }
        }
        require(tokenIdToMint != 0, "TokenId should be chosen");
    }
