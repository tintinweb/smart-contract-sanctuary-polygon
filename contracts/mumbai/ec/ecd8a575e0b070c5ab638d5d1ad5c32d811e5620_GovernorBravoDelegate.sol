  ...
  GovernorBravoEvents
{
  ...
  uint256 public constant MIN_PROPOSAL_THRESHOLD = 50000e18; // 50,000 Cult

  /// @notice The maximum setable proposal threshold
  uint256 public constant MAX_PROPOSAL_THRESHOLD = 6000000000000e18; //6000000000000 Cult
  ...
  uint256 public constant quorumVotes = 1000000000e18; // 1 Billion
  ...
  function initialize(
    ...
    require(
      dCult_ != address(0),
      "GovernorBravo::initialize: invalid dCult address"
    );
    require(
    ...
    require(
      treasury_ != address(0),
      "GovernorBravo::initialize: invalid treasury address"
    );
  ...
  function propose(
    ...
    require(
      dCult.checkHighestStaker(0, msg.sender),
  ...
  function castVoteInternal(
    ...
    require(
      !dCult.checkHighestStaker(0, msg.sender),
      "GovernorBravo::castVoteInternal: Top staker cannot vote"
    );
    require(
  ...
  function _setInvesteeDetails(address _investee) external {
    require(
      msg.sender == admin,
      "GovernorBravo::_setInvesteeDetails: admin only"
    );
    require(
      _investee != address(0),
      "GovernorBravo::_setInvesteeDetails: zero address"
    );
  ...
  function _fundInvestee() external returns (address) {
    require(
      msg.sender == treasury,
      "GovernorBravo::_fundInvestee: treasury only"
    );
    require(
      nextInvesteeFund <= nextInvestee,
      "GovernorBravo::_fundInvestee: No new investee"
    );

    nextInvesteeFund = add256(nextInvesteeFund, 1);
    emit InvesteeFunded(
      investeeDetails[sub256(nextInvesteeFund, 1)],
      sub256(nextInvesteeFund, 1)
    );
  ...
  function _setWhitelistGuardian(address account) external {
    ...
    require(
      account != address(0),
      "GovernorBravo:_setWhitelistGuardian: zero address"
    );
    require(
      msg.sender == admin,
