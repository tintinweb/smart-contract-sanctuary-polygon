    ...
    StavmeSaEventProviderFixedResults
{
    ...
    function initialize(
        ...
        __StavmeSaEventProviderFixedResults_init(initializeDto.results);
    }
    ...
    function cancelEvent(uint256 eventId, bytes calldata data) external payable onlyOwner(msg.sender) {
        _cancelEvent(eventId, data);
    }
    
    function settleEvent(uint256 eventId, EventResult result, bytes calldata data) external payable onlyOwner(msg.sender) {
        _settleEvent(eventId, result, data);
    }
}
    ...
    StavmeSaERC721
{
    ...
    mapping(uint256 => EventResult) internal __eventResults;
    ...
    function _createEvent(
        ...
        emit EventCreated(eventId, owner, startTime, endTime, data);
    }
    ...
    function _cancelEvent(
        ...
        uint48 startTime = __eventsById[eventId].startTime;
        if (block.timestamp > startTime) {
            revert MustNotHaveStarted(eventId, startTime);
        }

        _burn(eventId);

        emit EventCancelled(eventId, data);
    }
    ...
    function _settleEvent(
        ...
        uint48 endTime = __eventsById[eventId].endTime;
        if (block.timestamp < endTime) {
            revert MustHaveEnded(eventId, endTime);
        }
        ...
        __eventResults[eventId] = result;

        emit EventSettled(eventId, result, data);
    }
    ...
    function getEventStartAndEndTime(
        uint256 eventId
    )
        external
        view
        override
        onlyIfExists(eventId)
        returns (uint48 startTime, uint48 endTime)
    {
        EventDefinition memory eventDefinition = __eventsById[eventId];
    ...
    function getEventResult(
        ...
        return _getEventResult(eventId);
    }
    ...
    function _getEventResult(
        uint256 eventId
    ) internal view returns (EventResult result) {
        return __eventResults[eventId];
    }
    ...
    modifier onlyIfOpen(uint256 eventId) {
        ...
        if (EventResult.unwrap(_getEventResult(eventId)).length != 0) {
