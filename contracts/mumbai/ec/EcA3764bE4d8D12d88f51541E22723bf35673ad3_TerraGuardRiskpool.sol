...
contract TerraGuardRiskpool is 
    BasicRiskpool
{
    struct BundleInfo {
        uint256 bundleId;
    ...
    uint256 public constant SUM_OF_SUM_INSURED_CAP = 10 * 10**6 * 10**6;

    uint256 public constant MIN_BUNDLE_LIFETIME = 90 * 24 * 3600;
    uint256 public constant MAX_BUNDLE_LIFETIME = 365 * 24 * 3600;
    uint256 public constant MIN_POLICY_DURATION = 0;
    uint256 public constant MAX_POLICY_DURATION = 366 * 24 * 3600;
    uint256 public constant MIN_POLICY_COVERAGE = 0; // as usdt amount
    uint256 public constant MAX_POLICY_COVERAGE = 10 ** 5 * 10 ** 6; // as usdt amount
    ...
    modifier onlyAllowedAccount {
        require(isAllowed(_msgSender()), "ERROR:TGR-001:ACCOUNT_NOT_ALLOWED_FOR_BUNDLE_CREATION");
        _;
    }
    ...
    constructor(
        ...
        BasicRiskpool(name, collateralizationLevel, SUM_OF_SUM_INSURED_CAP, erc20Token, wallet, registry)
    {
        uint256 sumInsuredPercentage = 100;
        require(
            sumInsuredPercentage > 0 && sumInsuredPercentage <= 100,
            "ERROR:TGR-005:SUM_INSURED_PERCENTAGE_INVALID");
        ...
        _riskpoolCapitalCap = SUM_OF_SUM_INSURED_CAP;
    ...
    function setCapitalCaps(
        ...
        onlyOwner
    {
        require(poolCapitalCap <= getSumOfSumInsuredCap(), "ERROR:TGR-011:POOL_CAPITAL_CAP_TOO_LARGE");
        require(bundleCapitalCap < poolCapitalCap, "ERROR:TGR-012:BUNDLE_CAPITAL_CAP_TOO_LARGE");
        require(bundleCapitalCap > 0, "ERROR:TGR-013:BUNDLE_CAPITAL_CAP_ZERO");
        ...
        returns(uint256 bundleId)
    {
        require(
            ...
            "ERROR:TGR-020:NAME_NOT_UNIQUE");
        require(
            lifetime >= MIN_BUNDLE_LIFETIME
            && lifetime <= MAX_BUNDLE_LIFETIME, 
            "ERROR:TGR-021:LIFETIME_INVALID");
    ...
    //         "ERROR:TGR-100:FUNDING_EXCEEDS_BUNDLE_CAPITAL_CAP");

    //     require(
    //         getCapital() <= _riskpoolCapitalCap, 
    //         "ERROR:TGR-101:FUNDING_EXCEEDS_RISKPOOL_CAPITAL_CAP");
    // }
