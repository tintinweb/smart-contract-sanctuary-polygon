...
contract NFTMasterPiece is ERC2771Context, ERC721Enumerable, ERC721Royalty, Ownable, INFTMasterPiece {
    IMasterPiece masterPieceContract;
    PriceConverter priceConverter;

    mapping(bytes32 => uint256) private _masterPiecesRafflingCount;
    mapping(uint256 => nftMasterPieceStruct) private _nftMasterPieces;  // tokenId => nftMasterPieceStruct
    // mapping (address => bool) public frozenAccount;//address account => isFreeze

    address private marketPlaceAddress;
    address private addressRroseContract;
    address private museumContract;
    address payable royaltyReceiver;

    constructor(RroseForwarder forwarder, PriceConverter _priceConverter)
    ERC2771Context(address(forwarder))
    ERC721("EverRose MasterPiece", "ERMP") {
        priceConverter = _priceConverter;
        royaltyReceiver = payable(_msgSender()); // Set to the contract owner to collect fees and redispatch to artist off-chain
        _setDefaultRoyalty(royaltyReceiver, 1000);  // Set 10% in basis point as default commission fees
    }
    ...
    // TODO reintroduce this function?
    ...
    function setStatus(Status new_status, uint256 tokenId) external validTokenId(tokenId) {
        require(isContract(_msgSender()) && _msgSender() == marketPlaceAddress, "bad contract");
        _nftMasterPieces[tokenId].status = Status(new_status);
    }
    ...
    function setAddressRroseContract(address _addressRroseContract) public onlyOwner {
        addressRroseContract = _addressRroseContract;
    }

    function setMuseumContract(address _addressMuseumContract) public onlyOwner {
        museumContract = _addressMuseumContract;
    }

    function setMasterPieceContract(address _addressMasterPieceContract) public onlyOwner {
        masterPieceContract = IMasterPiece(_addressMasterPieceContract);
    }
    ...
    function getToken(uint256 _tokenId) public view returns (nftMasterPieceStruct memory nftMasterPiece) {
        return _nftMasterPieces[_tokenId];
    }

    function getMasterPieceCurrentSupply(bytes32 key) public view returns (uint256 supply) {
        return _masterPiecesRafflingCount[key];
    }

    function isMasterPieceMaxSupplyReached(bytes32 key) private view returns (bool) {
        return masterPieceContract.get(key).maxSupply <= _masterPiecesRafflingCount[key];
    }

    function getFloorPriceForToken(uint256 _tokenId) public view returns (uint256) {
        return masterPieceContract.get(_nftMasterPieces[_tokenId].masterPiece.key).floorPrice;
    }

    function getFeesBasisPointsForToken(uint256 _tokenId) public view returns (uint256) {
        return masterPieceContract.get(_nftMasterPieces[_tokenId].masterPiece.key).feesBasisPoints;
    }

    // TODO reactivate this function?
    /*    function _baseURI() internal pure override returns (string memory) {
            return "ipfs://medias.rrose.io";
        }*/

    function getRandomKey(uint256 seed, bytes32 keyMuseum) private view returns (bytes32) {
        // 1 retrieve all masterpieces for a museum
        IMuseum.MasterPieceRelationship[] memory arraysMasterPieceRelationship = IMuseum(museumContract).getAvailableMasterPieces(keyMuseum);

        // 2 get a random number range 0...arraysMasterPieceRelationship.length
        // randomIdx is commented to optimize contract size
        // uint256 randomIdx = Randomness.RNG(seed, arraysMasterPieceRelationship.length);

        // console.log("MasterPieces: %s", arraysMasterPieceRelationship.length);
        // console.logBytes32(arraysMasterPieceRelationship[randomIdx].key);

        // 3 get the randomKey
        // return arraysMasterPieceRelationship[randomIdx].key;
        return arraysMasterPieceRelationship[Randomness.RNG(seed, arraysMasterPieceRelationship.length)].key;
    }

    function raffling(uint256 seed, bytes32 keyMuseum, uint256 _tokenIdBox) private returns (nftMasterPieceStruct memory nftMasterPiece) {
        // Get a random MasterPiece key
        bytes32 randomMasterpieceKey = getRandomKey(seed, keyMuseum);
        // Check that random key is a real index OR maxSupply is not reached
        // /!\
        // /!\ can be gaz consuming and subject to infinite loop depending on the masterpieces/masterboxes max raffling settings
        // /!\
        while (!masterPieceContract.exist(randomMasterpieceKey) || isMasterPieceMaxSupplyReached(randomMasterpieceKey)) {
            randomMasterpieceKey = getRandomKey(seed++, keyMuseum);
        }
        // Update the raffling count for this MasterPiece
        _masterPiecesRafflingCount[randomMasterpieceKey]++;

        return nftMasterPieceStruct({
            boxTokenId: _tokenIdBox,
            collectionId: _masterPiecesRafflingCount[randomMasterpieceKey],
            masterPiece: masterPieceContract.get(randomMasterpieceKey), // Get the MasterPiece template raffled
            status: Status.UNLISTED
        });
    }

    function randomMint(address _toAddress, uint256 seed, bytes32 keyMuseum, uint256 _tokenIdBox) external returns (uint256 _tokenId) {
        require(addressRroseContract != address(0), "bad contract");
        require(isContract(_msgSender()) && _msgSender() == addressRroseContract, "bad contract");

        // New formatted tokenId : MasterPieceKey + _ + collectionId counter
        // 1  Raffling the mp and link it with the detail struct of masterpiece with the new tokenID
        INFTMasterPiece.nftMasterPieceStruct memory nftMpStruct = raffling(seed, keyMuseum, _tokenIdBox);

        // 2  Generating the tokenId based on MasterPiece key and collectionId counter
        uint256 tokenId = uint256(keccak256(abi.encodePacked(nftMpStruct.masterPiece.key, "_", nftMpStruct.collectionId)));

        // 3 mint the mp
        _safeMint(_toAddress, tokenId);
        _nftMasterPieces[tokenId] = nftMpStruct;

        // 4  We check the MasterPiece supply
        bytes32 masterPieceKey = nftMpStruct.masterPiece.key;
        if(isMasterPieceMaxSupplyReached(masterPieceKey)) {
            IMuseum(museumContract).updateStatus(keyMuseum, masterPieceKey);
            emit MasterPieceMaxSupplyReached(masterPieceKey);
        }

        // 5 We set the Token royalty according to the template parameters
        _setTokenRoyalty(tokenId, royaltyReceiver, uint96(nftMpStruct.masterPiece.feesBasisPoints));

        emit masterPieceRaffled(_toAddress, tokenId, nftMpStruct);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override(ERC721, ERC721Enumerable) {
        // require( !frozenAccount[_msgSender()] && !frozenAccount[from] && !frozenAccount[to],"One of account is frozen");
        super._beforeTokenTransfer(from, to, tokenId);
        /*if(from == marketPlaceAddress || to == marketPlaceAddress || from == address(0) || to == address(0)) {
            return;
        }
        // Handle all external transfer not related to the EverRose MarketPlace
        else {
            //console.log("External _beforeTokenTransfer: from: %s - to: %s - tokenId: %s", from, to, tokenId);
            if(IMarketPlace(marketPlaceAddress).getFloorPriceMode()) {
                uint256 floorPrice  = _nftMasterPieces[tokenId].masterPiece.floorPrice;
                // console.log("Floor price mode: %s", priceConverter.getMaticInWeiFromEurCents(_nftMasterPieces[tokenId].masterPiece.floorPrice));
                require(msg.value >= floorPrice, "Need more money");
            }
    ...
    function tokenURI(uint256 tokenId) public view override(ERC721) validTokenId(tokenId) returns (string memory) {
        return _nftMasterPieces[tokenId].masterPiece.uri;
    }
