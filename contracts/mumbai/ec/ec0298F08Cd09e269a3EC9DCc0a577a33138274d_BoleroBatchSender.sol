...
contract BoleroBatchSender is ExternalStorage {
    ...
    */
    ...
    function multisendToken(BatchTransfer[] memory batchData) public payable {
            require(batchData.length <= arrayLimit());
            uint8 i = 0;
            for (i; i < batchData.length; i++) {
                ERC20 erc20token = ERC20(batchData[i].tokenAddress);
                uint256 total = 0;
                uint8 j = 0;
                for(j; j < batchData[i].operations.length; j++){
                    erc20token.transferFrom(msg.sender, batchData[i].operations[j].reciever , batchData[i].operations[j].balance);
                    total += batchData[i].operations[j].balance;
                }
                emit Multisended(total, msg.sender, batchData[i].operations[j].reciever);
            }
