...
contract Manager is Ownable, Pausable {
	...
	// mapping category => fee
	mapping(uint256 => uint256) public commissions;
	mapping(uint256 => bool) public isCommissionSets;
	...
	function setSystemFee(
		...
		_setSystemFee(_xUser, _yRefRate, _zProfitToCreator);
	}
	...
	function setCommissions(uint256[] memory _categoryIds, uint256[] memory _commissions) public onlyOwner {
		require(_categoryIds.length > 0 && _categoryIds.length == _commissions.length, "Invalid-input");
		for (uint256 i = 0; i < _categoryIds.length; i++) {
			setCommission(_categoryIds[i], _commissions[i]);
		}
	}

	function setCommission(uint256 _categoryId, uint256 _commission) public onlyOwner {
		require(_categoryId > 0, "Invalid-categoryId");
		commissions[_categoryId] = _commission;
		isCommissionSets[_categoryId] = true;
	}

	function getCommission(uint256 _categoryId) public view returns(uint256) {
		if (isCommissionSets[_categoryId]) {
			return commissions[_categoryId];
		}
		return xUser;
	}
...
contract DigicapMarket is Manager, ERC721Holder, ReentrancyGuard {
	...
	event OrderCreated(uint256 indexed _orderId, address _tokenAddress, uint256 indexed _tokenId, uint256 _price, address _paymentToken);
	...
	event BidCreated(uint256 indexed _bidId, address _tokenAddress, uint256 indexed _tokenId, uint256 _price, address _paymentToken);
	...
	function _match(
		...
		uint256 _categoryId = callGetCategoryToken(order.tokenAddress, order.tokenId);
		uint256 _commission = getCommission(_categoryId);
        uint256 amountToSeller = _price - (_price * _commission / ZOOM_FEE);
	...
	function callGetCategoryToken(address _token, uint256 _tokenId) private returns (uint256) {
        (bool success,  bytes memory data) = _token.call{gas: 5000}(abi.encodeWithSignature("getCategoryToken(uint256)", _tokenId));

		uint256 number;
        for (uint i = 0; i < data.length; i++){
            number = number + uint(uint8(data[i]))*(2**(8*(data.length-(i+1))));
        }
    	return number;
    }
	...
	function createOrder(
		...
		emit OrderCreated(_orderId, _tokenAddress, _tokenId, _price, _paymentToken);
		return _orderId;
	}
	...
	function buy(
		...
		require(_paymentToken == order.paymentToken, 'Payment-method-does-not-support');
		...
		if (_paymentToken == address(0) && msg.value > 0) {
			...
			IERC20(_paymentToken).safeTransferFrom(msg.sender, address(this), orderAmount);
		}

		emit Buy(_orderId, _paymentToken, orderAmount);
		return _match(msg.sender, _paymentToken, _orderId, orderAmount);
	}
	...
	function createBid(
		...
		_bidId = totalBids;
		totalBids++;
		emit BidCreated(_bidId, _tokenAddress, _tokenId, _price, _paymentToken);
		return _bidId;
	}
	...
	function acceptBid(uint256 _bidId) external whenNotPaused() returns (bool) {
        ...
        bid.status = false;
        bids[_bidId] = bid;
        return _match(bid.bidder, bid.paymentToken, _orderId, bid.bidPrice);
	}
	...
	function cancelBid(uint256 _bidId) external whenNotPaused() nonReentrant() {
		...
		if (payBackAmount > 0) {
			if (bid.paymentToken != address(0)) {
				IERC20(bid.paymentToken).safeTransfer(bid.bidder, payBackAmount);
			} else {
	...
	function updateBid(
		...
		Bid memory bid = bids[_bidId];
		require(bid.bidder == msg.sender, 'Invalid-bidder');
		require(bid.bidPrice != _bidPrice, 'Invalid-bidPrice');
		require(bid.status, 'Bid-cancelled-or-accepted');
		bool isExcess = bid.bidPrice > _bidPrice;
		uint256 amount = isExcess ? bid.bidPrice - _bidPrice : _bidPrice - bid.bidPrice;

		if (bid.paymentToken != address(0)) {
			if (isExcess) {
				IERC20(bid.paymentToken).safeTransfer(bid.bidder, amount);
			} else {
				IERC20(bid.paymentToken).safeTransferFrom(bid.bidder, address(this), amount);
			}
		} else {
			if (isExcess) {
