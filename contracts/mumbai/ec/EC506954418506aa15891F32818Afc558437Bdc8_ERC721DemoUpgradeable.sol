...
contract ERC721DemoProxy is ERC1967Proxy {
...
contract ERC721DemoUpgradeable is Initializable, UUPSUpgradeable, ERC721EnumerableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC2981Upgradeable, DefaultOperatorFiltererUpgradeable, IERC4906Upgradeable {
    using SafeMathUpgradeable for uint256; //計算ライブラリを使えるようにする
    ...
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers(); // 実装コントラクト自体の再初期化を防ぐ
    }

    function initialize() public initializer {
        // 継承コントラクトの初期化
        __ERC721_init("ERCD", "ERCD");
        ...
        // 継承先のさらに継承先のコントラクトの初期化
        __ERC165_init();
        __Context_init();
    }

    // アップグレード メカニズムへのアクセス制限をするには、_authorizeUpgrade()をOverrideする必要がある
    function _authorizeUpgrade(address) internal override onlyOwner {}


    function batchMetadataUpdateForAll() public onlyOwner {
        emit BatchMetadataUpdate(0, type(uint256).max);
    }

    function batchMetadataUpdate(uint256 fromTokenId, uint256 toTokenId) public onlyOwner {
        emit BatchMetadataUpdate(fromTokenId, toTokenId);
    }

    /**
     * 外部コントラクトから呼び出されるMint関数
     */
    function externalMint(address toAddr, uint256 amount) external onlyOwner nonReentrant {
    ...
    function supportsInterface(bytes4 interfaceId) public view override(IERC165Upgradeable, ERC721EnumerableUpgradeable, ERC2981Upgradeable) returns (bool) {
