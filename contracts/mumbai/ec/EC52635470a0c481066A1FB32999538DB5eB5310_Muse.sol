 ...
 */
contract ERC4610 is Context, ERC165, IERC4610 {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return
            interfaceId == type(IERC4610).interfaceId ||  
    ...
    // function setDelegators(uint tokenId) public payable{
    //     address owner = ownerOf(tokenId);
    //     require(msg.sender != owner, "ERC4610: setDelegator to current owner");
    //     require(msg.value == 0.1 ether);
    //     _setDelegator(msg.sender, tokenId);
     ...
     */
    function delegatorOf(uint256 tokenId) public view virtual override returns (address[] memory) {
        require(_exists(tokenId), "ERC4610: delegated query for nonexistent token");
        address[] memory delegators = _delegators[tokenId];
        return delegators;
    }

    function checkDelegates(address delegate, uint tokenId) internal view returns(bool){
        address[] memory delegators = _delegators[tokenId];
        for (uint256 i = 0; i < delegators.length; i++) {
            if (delegators[i] == delegate)
                return true;
        }
        return false;
    }
    ...
    function _setDelegator(address delegator, uint256 tokenId) internal virtual {
        _delegators[tokenId].push(delegator);
...
interface IERC4610 is IERC721 {
     ...
     * - `tokenId` must exist.
     */
    function delegatorOf(uint256 tokenId) external view returns (address[] memory);
