    ...
    UUPSUpgradeable
{
    ...
    function trait(uint256 tokenId) external view returns (uint256) {
        return state.tokenTraits[tokenId];
    }
    ...
    function safeMint(
        ...
        state.setTokenTrait(newTokenId);
...
library App {
    ...
    event TokenTraitSet(uint256 tokenId, uint256 trait);
    ...
    function initialize(AppType.State storage state) public {
        ...
        state.config.uints[AppType.UintConfig.RAND_REFRESH_PERIOD] = 100;
        if (state.config.uints[AppType.UintConfig.CHAIN_ID] == 0x89) {
            ...
            state
                .rand
                .linkVRFAddress = 0x8C7382F9D8f56b33781fE506E897a4F1e2d17255;
            state
                .rand
                .linkVRFKeyHash = 0x6e75b569a01ef56d18cab6a8e71e6600d6ce853834d4a5748b720d06f878b3a4;
            state.config.uints[AppType.UintConfig.RAND_LINK_FEE] =
                0.0001 *
                10**18; // 0.0001 LINK
            ...
            // state
            //     .rand
            //     .linkVRFAddress = 0x6A2AAd07396B36Fe02a22b33cf443582f682c82f;
            // state
            //     .rand
            //     .linkVRFKeyHash = 0xd4bb89654db74673a187bd804519e65e3f71a52bc55f11da7601a13dcf505314;
            // state.config.uints[AppType.UintConfig.RAND_LINK_FEE] =
            //     0.005 *
            //     10**18; // 0.0001 LINK
    ...
    function setTokenTrait(AppType.State storage state, uint256 tokenId)
        external
    {
        state.tokenTraits[tokenId] = uint256(
            keccak256(
                abi.encode(
                    block.timestamp,
                    state.rand.latest,
                    uint256(blockhash(block.number - 1)),
                    uint256(8881618152015201520080513151514888),
                    tokenId,
                    msg.sender,
                    block.number
                )
        ...
        emit TokenTraitSet(tokenId, state.tokenTraits[tokenId]);
    }
}
