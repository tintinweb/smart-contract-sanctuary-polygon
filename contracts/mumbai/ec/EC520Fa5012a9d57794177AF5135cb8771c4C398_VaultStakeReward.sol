...
contract VaultStakeReward is OwnerGovernableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, ERC20Upgradeable, ERC4626Upgradeable {
    ...
    enum StakeType {Stake, Redeem, Reinvest}

    IKiloConfig public kiloConfig;
    ...
    event SetPerpTrade(address perpTrade);
    ...
    event VaultCapAndPeriodUpdated(
        uint256 cap,
    ...
    function deposit(uint256 amount, address user) public override nonReentrant returns (uint256) {
        ...
        require(IERC20Upgradeable(token).balanceOf(address(this)) + amount <= cap, "!cap");
        //更新下当前用户stake奖励
        _updateReward(user);
        ...
        //用户资产transfer
        uint256 shares = previewDeposit(amount * tokenBase / BASE);
        _deposit(user, user, amount * tokenBase / BASE, shares);
        emit StakedRedeemItem(user, msg.sender == address(this) ? StakeType.Reinvest : StakeType.Stake, amount, shares, 0);
    ...
    function redeem(
        ...
        require(IPerpTrade(perpTrade).totalOpenInterest() <= uint256(IERC20Upgradeable(token).balanceOf(address(this))) * kiloConfig.getKiloConfig().utilizationMultiplier / (10 ** 4), "!utilized");
        uint256 assets = previewRedeem(shares);
        _withdraw(_msgSender(), receiver, owner, assets, shares);
        emit StakedRedeemItem(user, StakeType.Redeem, 0, shares, assets);
    ...
    function updateVaultCapAndPeriod(uint256 _cap, uint256 _stakingPeriod) external onlyOwner {
        ...
        emit VaultCapAndPeriodUpdated(_cap, _stakingPeriod);
    }
...
interface IKiloConfig {
    ...
    function feeCalculatorAddr() external returns(address);
    function fundingManagerAddr() external returns(address);
    function perpTradeAddr() external returns(address);
    function kiloPriceFeedAddr() external returns(address);
    function orderBookAddr() external returns(address);
    function positionManagerAddr() external returns(address);
    function stakeRewardAddr() external returns(address);
    function productManagerAddr() external returns(address);
