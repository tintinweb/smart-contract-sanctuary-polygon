...
contract Market is Context, ERC721Holder, Ownable {
    ...
    mapping(uint256 => uint256[]) private offerListKeys;
    ...
    modifier isOfferExpired(uint256 _tokenId, uint256 _offerId) {
        require(_isOfferExpired(_tokenId, _offerId), "offer already expired");
        _;
    }
    ...
    function _isOfferExpired(uint256 _tokenId, uint256 _offerId)
        internal
        view
        returns (bool)
    {
        Offer storage offer = offers[_tokenId][_offerId];
    ...
    function _makeListing(
        ...
        listing.beneficiary = payable(_msgSender());
    ...
    function makeListing(
        ...
        if (nft.ownerOf(_tokenId) == address(msg.sender)) {
    ...
    function makeOffer(
        ...
        uint256 endTime,
        uint256 reservePrice
    ) public payable offerRequirements(reservePrice) returns (uint256) {
        ...
        offer.price = reservePrice;

        offerListKeys[_tokenID].push(_offerID);
    ...
    function approveOffer(uint256 _tokenID, uint256 _offerID)
        public
        isOwner(_tokenID)
        isOfferExpired(_tokenID, _offerID)
    {
        Offer storage offer = offers[_tokenID][_offerID];

        nft.safeTransferFrom(_msgSender(), offer.buyer, offer.tokenId);
        token.safeTransferFrom(offer.buyer, _msgSender(), offer.price);

        for (uint256 i = 0; i < offerListKeys[offer.tokenId].length; i++) {
            delete offers[_tokenID][offerListKeys[offer.tokenId][i]];
        }
