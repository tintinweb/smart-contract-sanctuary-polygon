...
contract UNAPod is Ownable {
    ...
    function _getDataCollection(address addr) internal view returns(Data[] memory) {
        return _pod[addr];
    }
    ...
    function _copyData(address addr, address to) internal {
        for (uint i = 0; i < _pod[addr].length; i++) {
            _pod[to][i] = _pod[addr][i];
        }
    }

    function _removeDataCollection(address addr) internal {
        delete _pod[addr];
    }
    ...
    function copyData(address addr, address to) public {
        require(isAuthorized(addr, "WRITE", ""), "This tuple address/key is not authorized to access the data.");
        _copyData(addr, to);
    }

    function removeDataCollection(address addr) public {
        /* Only admin and user */
        require(msg.sender == owner() || msg.sender == addr, "You are not authorized to remove this data collection.");
        _removeDataCollection(addr);
    }
    ...
    function isAuthorized(address addr, string memory accessType, string memory _key) internal view returns(bool boolean) {
        ...
        if (msg.sender == owner() || msg.sender == addr) {
            return true;
        } else if (challengeDelegations(accessType, addr, _key)) {
            /* Delegated access */
            return true;
        } else {
            return false;
