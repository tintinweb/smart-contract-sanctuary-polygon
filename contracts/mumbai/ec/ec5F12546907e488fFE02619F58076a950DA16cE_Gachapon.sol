...
contract Gachapon is PrizePoolManager, StateManager, IGachapon, VRFConsumerBase {
  ...
  /// @notice The address of the native token wrapper
  address public immutable wrappedNativeTokenAddress;

  constructor(address _deployer, address _wrappedNativeTokenAddress) 
  ...
  function initialize(string memory _gachaponTitle, address _currency) external {
    require(msg.sender == address(factory), "The gachapon can only be initialized by the factory, after its creation");
  ...
  function play(uint8 _times) public payable override {
  ...
  /// @inheritdoc IGachapon
  function play(uint8 _times, uint256 _maxAmountForAddress, uint256 _price, bytes32[] calldata _merkleProof) public payable override {
  ...
  /// @dev Transfer tokens from the player to this contract
  function _transferTokensToGachapon(uint256 _amount) private {
    if (currency == wrappedNativeTokenAddress) {
      INativeTokenWrapper(wrappedNativeTokenAddress).deposit{value: msg.value}();
      if (_amount > msg.value) {
        IERC20(wrappedNativeTokenAddress).transferFrom(msg.sender, address(this), (_amount - msg.value));
      } else {
        require(_amount == msg.value, "Wrong amount of token received");
      }
    } else {
      if (msg.value > 0) {
        revert("This gachapon does not accept this token");
      }
  ...
  function _handlePayment(uint256 _amount) private {
    _transferTokensToGachapon(_amount);

    uint256 _ownerShare = _amount * ownerRate / 1000;
    uint256 _teamTreasuryShare = _amount - _ownerShare;
    ...
    if (_teamTreasuryShare > 0) {
      IERC20(currency).transfer(ITreasuryManager(deployer).teamTreasuryAddress(), _teamTreasuryShare);
    }
  }

  /// @dev Will send their share of the profits to every addresses in 'ownerProfitsAddresses' and return the amount of dust
  function _distributeTokensToOwnerProfitsAddresses(uint256 _tokenAmounts) private returns(uint256) {
    ...
    if (ownerProfitsAddresses.length == 0) {
      IERC20(currency).transfer(owner(), _tokenAmounts);
      return 0;
    }
    ...
    for(uint8 _i = 0; _i < ownerProfitsAddresses.length; _i++) {
      ...
      IERC20(currency).transfer(ownerProfitsAddresses[_i], _addressShare);
...
interface IGachapon {
  ...
  function play(uint8 _times) external payable;
  ...
  function play(uint8 _times, uint256 _maxAmountForAddress, uint256 _price, bytes32[] calldata _merkleProof) external payable;
