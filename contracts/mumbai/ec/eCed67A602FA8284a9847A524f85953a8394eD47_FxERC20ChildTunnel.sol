...
contract FxERC20ChildTunnel is FxBaseChildTunnel, Create2 {
    ...
    function _mapToken(bytes memory syncData) internal returns (address) {
        ...
        require(childToken != address(0x0), "Not the zeroth address");

        // get root to child token
        address oldChildToken = rootToChildToken[rootToken];

        // check if it's already mapped
        require(oldChildToken == address(0x0), "FxERC20ChildTunnel: ALREADY_MAPPED");
    ...
    function _syncDeposit(bytes memory syncData) internal {
        ...
        require(childToken != address(0), "Open Token cannot be zero address");
        // deposit tokens
