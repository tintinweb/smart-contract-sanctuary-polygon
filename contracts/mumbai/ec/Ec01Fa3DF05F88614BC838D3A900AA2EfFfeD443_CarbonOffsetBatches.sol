...
import './interfaces/IBuckMintContractRegistry.sol';
import './interfaces/ICarbonOffsetBatches.sol';
import './BuckMintCarbonOffsetsFactory.sol';
    ...
    CarbonOffsetBatchesStorage
{
    ...
    event BatchLinkedWithVintage(uint256 tokenId, uint256 projectVintageTokenId);
    event BatchComment(uint256 tokenId, uint256 commentId, address sender, string comment);
    ...
    function initialize(address _contractRegistry) external virtual initializer {
        __Context_init_unchained();
        __ERC721_init_unchained('BuckMint Protocol: Carbon Offset Batches', 'BUCKMINT-COB');
        __Ownable_init_unchained();
        __Pausable_init_unchained();
        contractRegistry = _contractRegistry;
    ...
    function confirmRetirement(uint256 tokenId) public virtual onlyVerifier whenNotPaused {
        require(_exists(tokenId), 'ERC721: approved query for nonexistent token');
        require(nftList[tokenId].status != RetirementStatus.Confirmed, 'Batch retirement is already confirmed');
        require(nftList[tokenId].projectVintageTokenId != 0, 'Cannot retire batch without project vintage');
    ...
    function rejectRetirement(uint256 tokenId) public virtual onlyVerifier whenNotPaused {
    ...
    function rejectWithComment(uint256 tokenId, string memory comment) external virtual onlyVerifier whenNotPaused {
    ...
    function rejectApprovedWithComment(uint256 tokenId, string memory comment) external virtual onlyOwner whenNotPaused {
        require(nftList[tokenId].status == RetirementStatus.Confirmed, 'Batch must be in confirmed state to be rejected');
        require(IBuckMintContractRegistry(contractRegistry).checkERC20(ownerOf(tokenId)) == false, 'Batch has already been fractionalized');
        updateStatus(tokenId, RetirementStatus.Rejected);
        addComment(tokenId, comment);
    }
    ...
    function setToPending(uint256 tokenId) external virtual onlyVerifier whenNotPaused {
    ...
    function linkWithVintage(uint256 tokenId, uint256 projectVintageTokenId) public virtual onlyVerifier whenNotPaused {
        checkProjectVintageTokenExists(contractRegistry, projectVintageTokenId);
    ...
    function confirmRetirementWithVintage(uint256 tokenId, uint256 projectVintageTokenId) external virtual onlyVerifier whenNotPaused {
        require(nftList[tokenId].status != RetirementStatus.Confirmed, 'Batch retirement is already confirmed');
        ...
        // batch.
        require(nftList[tokenId].projectVintageTokenId == 0, 'Vintage is already set and cannot be changed; use confirmRetirement instead');
    ...
    function unsetSerialNumber(string memory serialNumber) external onlyVerifier {
    ...
    /// @param quantity quantity in bCO2e
    /// @param uri optional tokenURI with additional information
    function updateBatchWithData(uint256 tokenId, string memory serialNumber, uint256 quantity, string memory uri) external virtual whenNotPaused {
    ...
    function setSerialandQuantity(uint256 tokenId, string memory newSerialNumber, uint256 newQuantity) external virtual whenNotPaused {
        require(ownerOf(tokenId) == _msgSender() || hasRole(VERIFIER_ROLE, _msgSender()), 'Error: update only by owner or verifier');
    ...
    /// @dev Used in BCO2 contract's receive hook `onERC721Received`
    function getBatchNFTData(uint256 tokenId) external view virtual override returns (uint256, uint256, RetirementStatus) {
        return (nftList[tokenId].projectVintageTokenId, nftList[tokenId].quantity, nftList[tokenId].status);
    }
    ...
    function getComments(uint256 tokenId) external view virtual returns (string[] memory, address[] memory) {
    ...
    /// @notice Function that automatically converts Batch-NFT to BCO2 (ERC20)
    /// @dev Queries the factory to find the corresponding BCO2 contract
    /// Fractionalization happens via receive hook on `safeTransferFrom`
    function fractionalize(uint256 tokenId) external virtual {
        require(_isApprovedOrOwner(_msgSender(), tokenId), 'ERC721: transfer caller is not owner nor approved');
        require(nftList[tokenId].status == RetirementStatus.Confirmed, 'Error: cannot fractionalize before confirmation');

        address ERC20Factory = IBuckMintContractRegistry(contractRegistry).buckMintCarbonOffsetsFactoryAddress();
        uint256 pvId = nftList[tokenId].projectVintageTokenId;
        address pvERC20 = BuckMintCarbonOffsetsFactory(ERC20Factory).pvIdtoERC20(pvId);
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(AccessControlUpgradeable, ERC721EnumerableUpgradeable) returns (bool) {
    ...
    function addComment(uint256 tokenId, string memory comment) public virtual {
        require(
            hasRole(VERIFIER_ROLE, _msgSender()) || _msgSender() == ownerOf(tokenId) || _msgSender() == owner(),
        ...
        emit BatchComment(tokenId, nftList[tokenId].comments.length, _msgSender(), comment);
    }
