...
contract SleepSwapDca is Ownable {
  ...
  uint128 public constant MILLI_SECONDS_IN_A_DAY = 86400000;
  ...
  event OrderCancelled(uint256 orderId, uint256 executedTrades, uint256 remainingAmount);
  ...
  function withdrawByOrderId(uint256 _orderId) public {
    ...
    emit OrderCancelled(_order.orderId, _order.executedTrades, _order.remainingAmount);
  }
  ...
  function _executeOrder(uint256 orderId) internal onlyManager {
    ...
    require(selected_order.isOpen, "Order removed!");
    require(
      selected_order.lastExecutionTime +  selected_order.frequency.mul(MILLI_SECONDS_IN_A_DAY) <= block.timestamp,
    ...
    require(
      selected_order.isOpen == true, "Order closed!"
    );

    poolBalance -= selected_order.tradeAmount; // deduct usdt from pool on order executed
    selected_order.remainingAmount -= selected_order.tradeAmount; // deduct usdt from order on order executed
