...
contract LiquidityGeneratorToken is Context, IERC20, Ownable {
    ...
    function setTaxFeePercent(uint256 taxFee_) external onlyOwner() {
        require(taxFee_ >= 0 && taxFee_ <= maxTaxFee, "taxFee out of range");
        taxFee = taxFee_;
    }

    function setLiquidityFeePercent(uint256 liquidityFee_) external onlyOwner() {
        require(liquidityFee_ >= 0 && liquidityFee_ <= maxLiqFee, "liquidityFee out of range");
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount, bool takeFee) private {
        if (!takeFee)//FIXME expensive operation
            removeAllFee();
