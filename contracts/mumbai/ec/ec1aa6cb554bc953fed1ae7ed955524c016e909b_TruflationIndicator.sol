...
contract TruflationIndicator is ChainlinkClient, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(Wagies nft) external initializer {
        ...
        _jobID = "8b459447262a4ccf8863962e073576d9";
...
contract Wagies is ERC721AUpgradeable, ERC721AQueryableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    /* Events */
    event TruflationIndicatorEnable(bool resetMints);
    event ETHPriceIndicatorEnable(bool resetMints);
    ...
    function truflationIndicatorEnable() external onlyIndicator(truflationIndicator) {
        ...
        bool resetMints = (block.timestamp + PERIOD_RESET_TIME) >= periodEnd;

        _enableMintForAmount(MINTS_PER_PERIOD, PERIOD_LENGTH, false, resetMints);
        emit TruflationIndicatorEnable(resetMints);
    }
    ...
    function ethPriceIndicatorEnable() external onlyIndicator(ethPriceIndicator) {
        ...
        _enableMintForAmount(MINTS_PER_PERIOD, PERIOD_LENGTH, false, resetMints);
        emit ETHPriceIndicatorEnable(resetMints);
    }
    ...
    function remainingMints(address user) external view returns (uint256) {
        if(block.timestamp > mintSettings._periodEnd) return 0;
        ...
        uint256 totalRemainingMints = _remainingMints(_totalMinted());

        return min(MAX_MINTS_PER_PERIOD - userAmountMinted, totalRemainingMints);
    }
    ...
    function _enableMintForAmount(
        ...
        if (allowLowering || newMaxMintId > mintSettings._maxMintId) {
            // Casting is safe due to the `min` above, assuming MAX_ID <= type(uint136).max
            mintSettings._maxMintId = uint136(newMaxMintId);
        }
    }
