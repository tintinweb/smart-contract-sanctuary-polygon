...
contract EggsNFT is ERC721, Ownable {
    ...
    event MaturitiesUpdated(uint256 indexed tokenId, uint256 oldMaturity, uint256 newMaturity);
    ...
    function payForClaimZai(uint256 _tokenId,string memory _zaiName) external returns (uint256) {
        // UPDATE AUDIT : prevent bot action
        require(tx.origin == msg.sender, "contract not allowed");
        require(msg.sender == ownerOf(_tokenId), "Not yours");
        ...
        if(_toBurn != 0){
            IPayments IPay = IPayments(gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS));
    ...
    function _generateRandomZai(
        ...
        uint256 zaiId = IZaiNFT(gameAddresses.getAddressOf(AddressesInit.Addresses.ZAI_NFT)).mintZai(
            _user,
            _name,
            _state
        );
    ...
    function coverEggWithChicken(uint256 _tokenId,uint256 _chickenId) external {
        require(IERC721(gameAddresses.getAddressOf(AddressesInit.Addresses.CHICKEN)).ownerOf(_chickenId) == msg.sender, "not your chicken");
        ...
        require(_originMaturity > _newMaturity, "Doesn't need to cover this egg");
