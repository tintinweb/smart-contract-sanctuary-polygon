...
contract BuyBox is SecurityBase   {
    ...
    uint constant public ROUND = 6;
    uint constant public ROUND_SIZE = 2;
    uint constant public LUCKY_VALUE = 50;
    uint constant public TOTAL_SUPPLY = 100;
    ...
    constructor(uint price){
        ...
        _cursor = 0;
        _price = price;
        _headers = new string[](ROUND_SIZE);

        string memory classify = "#A1#";

        // Pre-generated metadata header
        for (uint i=0; i<ROUND_SIZE; i++) {
            uint n = (ROUND-1)*ROUND_SIZE + i;
            _headers[i] = _stringJoin(_stringJoin("A", _itos(n)), classify);
        }
    ...
    function registerToken(address newValue) external onlyMinter {
        // decimals???
    ...
    function finish() external onlyMinter {
        if (!paused()) {
            _pause();
        }
    }
    ...
    function pause() external override onlyMinter {
        require(false, "Function has been removed");
    }

    function unpause() external override onlyMinter {
        require(false, "Function has been removed");
    }
    ...
    function buy(address spender, address token, uint amount, string memory nonce) external onlyMinter {
        require(amount >= _price, "wrong amount");
        ...
        for (uint i=0; i<cnt; i++) {
            string memory head = _headers[_cursor / LUCKY_VALUE];
            string memory newSeq = _stringPadding(_itos(_cursor), "0", 3);
            string memory desc = _stringJoin(head, newSeq);
            _cursor++;

            lstMetadatas[i] = desc;
        }

        uint usedAmount = cnt * _price;
...
contract SecurityBase is AccessControlEnumerable, Pausable {
    ...
    function pause() external virtual onlyMinter {
        _pause();
    }

    function unpause() external virtual onlyMinter {
        _unpause();
    }
