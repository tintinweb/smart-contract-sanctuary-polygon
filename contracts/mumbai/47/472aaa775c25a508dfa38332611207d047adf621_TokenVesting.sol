...
contract TokenVesting is ReentrancyGuard, Ownable {
    ...
    modifier onlyIfVestingScheduleNotRevoked(address _beneficiary) {
        require(vestingSchedules[_beneficiary].initialized, "Vesting schedule not initialized");
        require(!vestingSchedules[_beneficiary].revoked, "Vesting schedule revoked");
        _;
    }
    ...
    function createVestingSchedule(
        ...
        require(_amount <= getUnallocatedFundsAmount(), "Insufficient funds available");
        require(!vestingSchedules[_beneficiary].initialized, "Vesting schedule for this beneficiary already exists");
    ...
    function claimVestedToken(address _beneficiary, uint256 _amount) public nonReentrant onlyIfVestingScheduleNotRevoked(_beneficiary) {
        VestingSchedule storage vestingSchedule = vestingSchedules[_beneficiary];
        uint256 tgeAmount = _getTgeAmount(vestingSchedule);
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule);
        uint256 releaseAmount = tgeAmount.add(vestedAmount);
        
        require(releaseAmount >= _amount, "TokenVesting: cannot release tokens, insufficient vested tokens");
        ...
        require(token.transfer(_beneficiary, _amount), "Token withdrawal failed.");
    }
    ...
    function _computeReleasableAmount(VestingSchedule memory vestingSchedule) internal view returns (uint256) {
        ...
        if (currentTime < vestingSchedule.cliff || vestingSchedule.revoked) {
            ...
            uint256 vestedAmountPerSlice = _calculateReleasableAmount(vestingSchedule);
            uint256 vestedAmount = vestedAmountPerSlice.mul(vestedSlicePeriods);
            return vestedAmount.sub(vestingSchedule.released);
        }
    }

    function _calculateReleasableAmount(VestingSchedule memory vestingSchedule) internal pure returns (uint256) {
