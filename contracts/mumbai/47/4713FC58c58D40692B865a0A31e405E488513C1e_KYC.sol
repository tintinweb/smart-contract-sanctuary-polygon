...
/**
@title A Know Your Customer Contract (KYCC) facilitating royalty management via KYC tokens as described below.
@author Jason Aubrey (GigaStar)
@notice Copyright 2023, GigaStar Technologies LLC, All Rights Reserved, https://gigastar.io
@dev Features provided by this contract:
* KYC: A token is created for a user that has passed a KYC (Know Your Customer) check. 
* Attributes: Each token has a user id that can be associated with off-chain data. A date and level are on-chain.
* Same User Check: Allows a user id to associate with multiple wallets, allowing a contract to distinguish between
*     a trade (i.e. a transfer betwen different users) vs a same-user transfer (e.g. hot to cold storage).
* Security: Via OpenZeppelin contracts and best practices
* Upgradeabilty: Via OpenZeppelin
@dev Conventions: Underscore prefixes are used for both 'private' and 'internal' members. In some cases 'internal'
    facilitate tests accessing state that would otherwise be private.
*/
contract KYC is IKYC, ERC721Upgradeable, PausableUpgradeable, Access {
    ...
    mapping(uint256 => Token) private _tokens; // key = token id
    mapping(address => uint256) private _tokenIds; // key = owner
    uint256 internal _tokenIdSeed; // ensures uniqueness
    // UPGRADABILITY: All new variables should be immediately above this line and decrement the gap array
    
    // See https://docs.openzeppelin.com/contracts/4.x/upgradeable#storage_gaps
    uint256[46] private __gap; // Array size + the sum of all storage slots used (except this member) should be 50
    ...
    */
    ...
    {
        return interfaceId == type(IKYC).interfaceId || super.supportsInterface(interfaceId);
    }
    ...
    {
        _requireNotPaused();
        return tokenOwner == address(0) ? 1 : super.balanceOf(tokenOwner); // super call reverts if not found
    }

    function ownerOf(uint256 /*tokenId*/)
        public pure override(ERC721Upgradeable, IERC721Upgradeable) returns(address)
    {
        revert DisabledFeature(Tag.ownerOf); // Discourage caller from getting all state
    }

    // solhint-disable-next-line no-empty-blocks
    function _beforeTokenTransfer(address, address, uint256, uint256) internal view override onlyManager { }

    function isApprovedForAll(address tokenOwner, address operator)
        public view override(ERC721Upgradeable, IERC721Upgradeable) returns(bool)
    {
        if(hasRole(ROLE_MANAGER, _msgSender()) || tokenOwner == operator)
            return true;
    ...
    function _baseURI() internal view override returns(string memory) {
        return _uriBase; // See {ERC721Upgradeable-tokenURI}
    }

    // Implement IKYC
    function areSameUser(address owner1, address owner2) external view override returns(bool) {
        _requireNotPaused();
        return owner1 == owner2 ? true : _getUserId(owner1) == _getUserId(owner2);
    }

    function _getUserId(address owner_) private view returns(uint256) {
        if(owner_ == address(0))  // is mint or burn
            return 0;
        _checkNonZero(Tag.unknownOwner, getCheckLevelByOwner(owner_));
        uint256 tokenId = _tokenIds[owner_];
        return _tokens[tokenId].userId;
    }

    function getCheckLevelByTokenId(uint256 tokenId) external view override returns(uint256) {
        _requireNotPaused();
        return _tokens[tokenId].level; // 0 if tokenId not found
    }

    function getCheckLevelByOwner(address owner_) public view override returns(uint256) {
        _requireNotPaused();
        uint256 tokenId = _tokenIds[owner_];
        return tokenId == 0 ? 0 : _tokens[tokenId].level; // 0 if tokenId not found
    }
    ...
    function createToken(address to, uint256 userId, uint256 issuedDate, uint256 level_)
        external override onlyManager returns(uint256)
    {
        _createToken(to, userId, issuedDate, level_);
        return _tokenIdSeed;
    }
    ...
    function createTokenBatch(
        ...
        external override onlyManager returns(uint256)
    {
        _checkNonZero(Tag.ownersLength, owners.length);
        _checkArrayLength(Tag.owners, owners.length, userIds.length);
        _checkArrayLength(Tag.owners, owners.length, issuedDates.length);
        unchecked {
            ...
            for(uint256 i; i < owners.length; ++i) // TOKEN_COUNT: Maybe many, caller must page
                _createToken(owners[i], userIds[i], issuedDates[i], levels[i]);
            return _tokenIdSeed - owners.length + 1; // return first id of range created: [result, result + tokenCount)
        }
    }

    function _createToken(address to, uint256 userId, uint256 issuedDate, uint256 level_) private {
        _checkNonZero(Tag.userIdReserved, userId);
        _checkDate(Tag.issuedDate, issuedDate);
        uint256 tokenId = ++_tokenIdSeed;
        Token storage token = _tokens[tokenId];
        token.userId = userId;
        token.owner = to;
        token.issuedDate = issuedDate;
        token.level = level_;
        _checkZero(Tag.existingOwner, _tokenIds[to]);
        _tokenIds[to] = tokenId;
        _safeMint(to, tokenId); // emit Transfer
    }

    function getToken(uint256 tokenId) external override view onlyManager returns(uint256, address, uint256, uint256) {
        super.ownerOf(tokenId); // if not found, reverts with: ERC721: invalid token ID
        Token storage token = _tokens[tokenId];
        return (token.userId, token.owner, token.issuedDate, token.level);
    }

    function destroyToken(uint256 tokenId) external override onlyManager {
        _destroyToken(tokenId);
    }

    function destroyTokens(uint256[] calldata tokenIds) external override onlyManager {
        unchecked {
            // slither-disable-next-line uninitialized-local // value zero init is default behavior
            for(uint256 i; i < tokenIds.length; ++i) // See TOKEN_COUNT
                _destroyToken(tokenIds[i]);
        }
    }

    function _destroyToken(uint256 tokenId) private {
        super.ownerOf(tokenId); // if not found, reverts with: ERC721: invalid token ID
        Token storage token = _tokens[tokenId];
        delete _tokenIds[token.owner];
