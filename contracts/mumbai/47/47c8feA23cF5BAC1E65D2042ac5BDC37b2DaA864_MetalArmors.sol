...
contract MetalArmors is Ownable, ERC1155, ArmorTokensMetadata {
    ...
    /// @notice Declare all components for Armor. Performs safety checks
    ...
    {
        return exchangeArmorCount[chubbyApeId];
    }
    ...
    function CreateArmorAndMetalApe(uint256 chubbyApeId)
        public
        returns (uint256 metalApeID, uint256[] memory ids)
    {
        ...
        // Mint Ape
        metalApeID = IMetalApe(metalApe).mintMetalApeFromArmors(msg.sender);

        // Mint Armors
        ids = _declare(address(IMetalApe(metalApe)));
        ...
        // IMetalApe(metalApe).equipFromArmorsContract(metalApeID, eqpArray, ids);
        IMetalApe(metalApe).createEquipRecord(metalApeID, eqpArray, ids);

        // send equp
        return (metalApeID, ids);
    }
