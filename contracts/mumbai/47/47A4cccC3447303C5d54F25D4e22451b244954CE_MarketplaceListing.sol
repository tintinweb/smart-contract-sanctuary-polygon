...
contract MarketplaceListing is Ownable {
    ...
    // USDC Address
    address public erc20Address = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    ...
    mapping(uint256 => Listing) private hashToActiveListing;

    uint256[] private _openListings;
    uint256 private _marketplaceFee;
    address private _marketplaceFeeRecipient;
    /**
     * @dev Emitted when new listing is created by the owner of the contract. 
     */
    ...
    function onERC721Received() public virtual returns (bytes4) {
    ...
    function getListing(uint256 listingId) public view virtual returns (Listing memory){
    ...
    function getOpenListingsForAddr(address _addr) public view virtual returns (uint256[] memory){
        require(_addr !=  address(0), "Please use a valid address");
        uint256[] memory _listingsForUser;
        uint256 count = 0;

        for (uint i=0; i<_openListings.length; i++) {
         Listing memory listing = _listings[i];
         if(listing.seller == _addr){
            _listingsForUser[count] = listing.listingId;
            count++;
         }

        }
        return _listingsForUser;
    }
    ...
    function createListing(
        ...
        uint256 index = _getHash(string(abi.encodePacked(nftAddress, '/',tokenId)));

        // check if exists & not active
        if(hashToActiveListing[index].listingId != 0){
            require(hashToActiveListing[index].state != State.INITIATED, "Active listing exists!");
        }
        ...
        hashToActiveListing[index] = listing;
        _listings[_listingId.current()] = listing;
        _openListings.push(_listingId.current());
    ...
    function buyAssetFromListing(
        ...
        if (listing.state != State.INITIATED) {
            ...
            // if (
            //     ERC721(listing.nftAddress).getApproved(listing.tokenId) !=
            //     address(this)
            // ) {
            //     revert(
        ...
        // ERC20 token = ERC20(listing.erc20Address);

        IERC20 usdcContract = getERC20Contract();
        IERC721 tokenContract = getERCContract(listing.nftAddress);
        // if (listing.price + fee > token.allowance(buyer, address(this))) {
        //     if (msg.value > 0) {
        //         // require(erc20Contract.balanceOf(msg.sender) >= cost,"Insufficient ERC20 allowance balance for paying for the asset.")  ;
        //         // erc20Contract.transferFrom(msg.sender, recipient, cost);
        ...
        // tokenContract.setApprovalForAll(listing.nftAddress, true);
    ...
    function getERCContract(address erc721Address) internal view returns (IERC721) {
        return IERC721(erc721Address);
    }

    function getERC20Contract() internal view returns (IERC20) {
        return IERC20(erc20Address);
    }

    function checkActiveListing() internal view returns (bool) {
        return true;
    }


    function _getHash(string memory key) 
        internal view returns (uint) 
    {
        ...
        uint random = 
             uint(keccak256(abi.encodePacked(key)));
     ...
     */
    function cancelListing(uint256 listingId) public virtual {
        Listing memory listing = _listings[listingId];
        ...
        uint256 index = _getHash(string(abi.encodePacked(listing.nftAddress, '/',listing.tokenId)));
