...
contract Reader is Governable {
    ...
    function getMaxAmountIn(IVault _vault, address _tokenIn, address _tokenOut) public view returns (uint256) {
        ...
        uint256 maxUsdaAmount = _vault.maxUsdaAmounts(_tokenIn);

        if (maxUsdaAmount != 0) {
            if (maxUsdaAmount < _vault.usdaAmounts(_tokenIn)) {
                return 0;
            }

            uint256 maxAmountIn = maxUsdaAmount.sub(_vault.usdaAmounts(_tokenIn));
            maxAmountIn = maxAmountIn.mul(10 ** tokenInDecimals).div(10 ** USDA_DECIMALS);
    ...
    function getAmountOut(IVault _vault, address _tokenIn, address _tokenOut, uint256 _amountIn) public view returns (uint256, uint256) {
        ...
        {
            uint256 usdaAmount = _amountIn.mul(priceIn).div(PRICE_PRECISION);
            usdaAmount = usdaAmount.mul(10 ** USDA_DECIMALS).div(10 ** tokenInDecimals);
            ...
            uint256 feesBasisPoints0 = _vault.getFeeBasisPoints(_tokenIn, usdaAmount, baseBps, taxBps, true);
            uint256 feesBasisPoints1 = _vault.getFeeBasisPoints(_tokenOut, usdaAmount, baseBps, taxBps, false);
    ...
    function getFundingRates(address _vault, address _weth, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            if (vault.cumulativeBorrowRates(token) > 0) {
                uint256 nextRate = vault.getNextBorrowRate(token);
                uint256 baseRate = vault.cumulativeBorrowRates(token);
    ...
    function getVaultTokenInfo(address _vault, address _weth, uint256 _usdaAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 2] = vault.usdaAmounts(token);
            amounts[i * propsLength + 3] = vault.getRedemptionAmount(token, _usdaAmount);
    ...
    function getFullVaultTokenInfo(address _vault, address _weth, uint256 _usdaAmount, address[] memory _tokens) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            amounts[i * propsLength + 6] = vault.maxUsdaAmounts(token);
    ...
    function getVaultTokenInfoV2(address _vault, address _weth, uint256 _usdaAmount, address[] memory _tokens) public view returns (uint256[] memory) {
    ...
    function getPositions(address _vault, address _account, address[] memory _collateralTokens, address[] memory _indexTokens, bool[] memory _isLong) public view returns(uint256[] memory) {
        ...
        for (uint256 i = 0; i < _collateralTokens.length; i++) {
            ...
            if (averagePrice > 0) {
                (bool hasProfit, uint256 delta) = IVault(_vault).getDelta(_indexTokens[i], size, averagePrice, _isLong[i], lastIncreasedTime, false);
