...
error Minting_WhitelistMintseNotLive();
error Minting_AddressNotWhitelistedOrAlreadyDone();
...
contract TheRanchBTCBullsCommunity is 
    ...
    bool public allowWhitelistedMints;
    ...
    */
    ...
    event whitelistBullsEnteringRanch(
        address indexed NewBullOwner,
        uint256  BullGiven
    );
    ...
    */
    function mint(uint256 _tokenQuantity) public payable {
        ...
        if(_tokenQuantity > 3 && partnershipMintLevelForAddress[msg.sender] == 2) {
            if (_tokenSupply.current() + (_tokenQuantity + 2 ) > maxSupply) {revert Minting_ExceedsTotalBulls();}
        } else {
            if (_tokenSupply.current() + (_tokenQuantity + partnershipMintLevelForAddress[msg.sender]) > maxSupply) {revert Minting_ExceedsTotalBulls();}
        }
        ...
        if(partnershipMintLevelForAddress[msg.sender] == 1) {
            for(uint256 i = 0; i < (_tokenQuantity + 1); i++) {
                _tokenSupply.increment();
                _safeMint(msg.sender, _tokenSupply.current());
            }
            partnershipMintLevelForAddress[msg.sender] = 0;

        } else if (partnershipMintLevelForAddress[msg.sender] == 2){
            if(_tokenQuantity < 4){
                for(uint256 i = 0; i < (_tokenQuantity + 1); i++) {
                    _tokenSupply.increment();
                    _safeMint(msg.sender, _tokenSupply.current());
                ...
                for(uint256 i = 0; i < (_tokenQuantity + 2); i++) {
                    _tokenSupply.increment();
                    _safeMint(msg.sender, _tokenSupply.current());
                } 
            }
        ...
        if (getUserAlreadyInDailyGiveawayStatus(msg.sender) == false){
            mintingGiveawayPlayers.push(payable(msg.sender));
            userInMintingGiveaway[msg.sender] = true; 
        }
    ...
    function paperMint(address _address, uint256 _tokenQuantity) public payable {
        ...
        uint256 hostingSafeAmt = totalTransactionCost * 15 / 100;
        uint256 marketingSafeAmt = totalTransactionCost * 1 / 100;
        uint256 btcMinersSafeAmt = totalTransactionCost - (referralFundAmt + marketingSafeAmt + hostingSafeAmt); 
        btcMinersSafeBalance += btcMinersSafeAmt;
        hostingSafeBalance += hostingSafeAmt; 
        marketingSafeBalance += marketingSafeAmt; 
        USDCRewardsBalance += referralFundAmt;
    ...
    */
    ...
    mapping(address => uint256) public myParnterNetworkTeamCount;  // Is the person already in the minting Giveaway?

    address public topNetworker;
    ...
    function setPartnerAddress(address _newPartner)  public {
        ...
        if (currentPartner == address(0)){
            ...
            if (myParnterNetworkTeamCount[_newPartner] > myParnterNetworkTeamCount[topNetworker]){
                topNetworker = _newPartner;
            }
   ...
   // METADATA
    ...
    function setPublicPaperSaleLiveStatus(bool _active) external onlyOwner{
        publicPaperSaleLive = _active ;
    }
    ...
    function setallowWhitelistedMintsState(bool _state) external onlyOwner {
        allowWhitelistedMints = _state;
    }

    function addOrRemoveAddressesWhitelist(address[] calldata _users, bool _bool) external onlyOwner {
        for (uint256 i=0; i< _users.length ; i++){
            addressHasWhitelistMintLeft[_users[i]] = _bool;
        }
    }


    mapping(address => bool) public addressHasWhitelistMintLeft;
    

    function whitelistMint() public nonReentrant{
        if (paused) { revert Contract_CurrentlyPaused_CheckSocials();}
        if (!allowWhitelistedMints) { revert Minting_WhitelistMintseNotLive();}
        if (_tokenSupply.current() > maxSupply) {revert Minting_ExceedsTotalBulls();}
        if (addressHasWhitelistMintLeft[msg.sender] == false){ revert Minting_AddressNotWhitelistedOrAlreadyDone();}
        ...
        for(uint256 i = 0; i < 1; i++) {
            _tokenSupply.increment();
            _safeMint(msg.sender, _tokenSupply.current());
            addressHasWhitelistMintLeft[msg.sender] = false;
        }

        emit whitelistBullsEnteringRanch(msg.sender, btcBullIndex);
    }



    function setPartnershipAddressesLevel(address[] calldata _users, uint256 _level) external onlyOwner {
        for (uint256 i=0; i< _users.length ; i++){
            partnershipMintLevelForAddress[_users[i]] = _level;
        }
    }


    mapping(address => uint256) public partnershipMintLevelForAddress;
