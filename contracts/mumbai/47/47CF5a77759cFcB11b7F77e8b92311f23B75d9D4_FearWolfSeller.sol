...
contract FearWolfSeller is MuonClient, AccessControl, Pausable, Withdrawable {
    ...
    function _verifyTSS(string memory data, bytes memory reqId, SchnorrSign memory signature) private {
    ...
    function buyWolves(
        ...
        address owner = address(uint160(decodedData[0]));
        address nonce = address(uint160(decodedData[1]));
        
        SchnorrSign memory signature = SchnorrSign(decodedData[4], owner, nonce);

        // FIXME: uncomment and form correct message that was signed
        //_verifyTSS(fearPrice.toString(), _toBytes(decodedData[3]), signature);
