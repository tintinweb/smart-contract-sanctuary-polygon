...
contract claimToken is Context, Ownable {
    ...
    function getUserClaim(address _tokenContract, uint256 user_id) view public returns (UserClaim memory) {
    ...
    function setUserClaim(address _tokenContract, uint user_id, uint amount, string memory claimId) public onlyOwner {
        uint tokenBalance = getAmountToken(_tokenContract);
        if(!_userClaims[_tokenContract][user_id].exists){
        ...
        for (uint i = 0; i < _userClaims[_tokenContract][user_id].claimIds.length - 1; i++) {
            if(keccak256(bytes(_userClaims[_tokenContract][user_id].claimIds[i])) == keccak256(bytes(claimId))){
                return;
            }
        }
    ...
    function setUserClaimMulti(address _tokenContract, ClaimRequest[] memory claimRequests) public onlyOwner {
        ...
        for (uint i = 0; i < claimRequests.length - 1; i++) {
            setUserClaim(_tokenContract, claimRequests[i].user_id, claimRequests[i].amount, claimRequests[i].claimId);
        }
    }
    
    // claim

    function claim(address _tokenContract, uint user_id) public {
