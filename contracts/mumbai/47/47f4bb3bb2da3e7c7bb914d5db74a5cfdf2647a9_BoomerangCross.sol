...
// ... mumbay: 0x47F4bb3bb2Da3e7c7bB914D5DB74a5cFDF2647a9
...
contract BoomerangCross is ERC721, ERC721URIStorage, ERC721Burnable, ERC2981, Ownable, ReentrancyGuard {
    ...
    function msgMintBulk(address[] memory _to, string memory _uri/*, uint256 _when*/, bytes32 messageHash, uint8 _v, bytes32 _r, bytes32 _s) 
    public /*theUser(messageHash, signature)*/ returns (uint256 _tokenId, /*uint256 _destroy,*/ address _sender) {
        require(verifySignature(messageHash, _v, _r, _s) == msg.sender, "only signer");
    ...
    function pfpMint(string memory _uri, bytes32 messageHash, uint8 _v, bytes32 _r, bytes32 _s) 
    ...
    function msgMint(address _to, string memory _uri/*, uint256 _when*/, bytes32 messageHash, uint8 _v, bytes32 _r, bytes32 _s) 
    public /*theUser(messageHash, signature)*/ returns (uint256 _tokenId, /*uint256 _destroy,*/ address _sender) {
        require(verifySignature(messageHash, _v, _r, _s) == msg.sender, "only signer");
        uint256 _id = _tokenIdCounter.current();
