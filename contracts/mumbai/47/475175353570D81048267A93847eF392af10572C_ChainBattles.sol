...
contract ChainBattles is ERC721URIStorage {
    ...
    function generateCharacter(uint256 tokenId) public view returns(string memory){
        ...
        bytes memory svg = abi.encodePacked(
            ...
            '<style>.uid{fill:#2D42FF;font-family:serif;font-size:12px;font-weight:bold}</style>',
            ...
            '<text x="50%" y="70%" class="attrib" dominant-baseline="middle" text-anchor="middle">', "Yield: ",getAttributes(tokenId, 3),'bps</text>',
            '</svg>'
        );
        return string(
            abi.encodePacked(
