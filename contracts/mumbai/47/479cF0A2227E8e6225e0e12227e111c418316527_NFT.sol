...
contract NFT is ERC721Psi, Ownable {
    ...
    event TokenMinted(uint256 indexed tokenId, uint256 quantity, string recipient);

    /// @notice Mints `quantity` of tokens if available
    function mint(address recipient, uint256 quantity)
        public onlyOwner
    {
        require(quantity + totalSupply() < maxTokens, "Max supply minted");

        _safeMint(recipient, quantity);

        // emit TokenMinted(tokenId, quantity, recipient);
    }
    ...
    function mintMax(address recipient) 
        external onlyOwner 
    {
        require(totalSupply() < maxTokens, "Max supply minted");

        mint(recipient, maxTokens - totalSupply());
    }
