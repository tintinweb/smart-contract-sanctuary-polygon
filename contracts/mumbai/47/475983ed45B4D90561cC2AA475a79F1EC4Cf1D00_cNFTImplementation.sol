...
contract cNFTImplementation is CompoundableNFT {
...
abstract contract CompoundableNFT is ERC721EnumerableUpgradeable, OwnableUpgradeable, RevokableDefaultOperatorFiltererUpgradeable {
    ...
    */
    ...
    // parentId => tailAddress => tailIds
    mapping(uint256 => mapping (address => mapping (uint256 => uint256))) private tailIndex;

    // this mapping keeps track of the index of a tail within the tails array of a particular parent
    // parentId => tailAddress => tailId => index
    mapping(uint256 => mapping (address => uint256[])) private tailsOf;

    // this mapping keeps track of the tails addresses of a particular parent
    // parentId => tailAddress[]
    mapping(uint256 => address[]) private tailsAddresses;

    // this mapping keeps track of the tail address index
    // parentId => tailAddress => index
    mapping (uint256 => mapping (address => uint256)) private tailAddressIndex;
    ...
    */
    ...
    /**
        ...
        * @dev represent one attachment triplet. Such a triplet comprises of the tokenIds of the parent and the tail tokens,
    ...
    */
        ...
        for (uint256 i=0; i<tailAddresses.length; i++) {
            ...
            parentOf[tailAddresses[i]][tailIds[i]] = parentIds[i];
    
            if (tailsOf[parentIds[i]][tailAddresses[i]].length == 0){
                tailAddressIndex[parentIds[i]][tailAddresses[i]] = tailsAddresses[parentIds[i]].length;
                tailsAddresses[parentIds[i]].push(tailAddresses[i]);
            }
            tailIndex[parentIds[i]][tailAddresses[i]][tailIds[i]] = tailsOf[parentIds[i]][tailAddresses[i]].length;
            tailsOf[parentIds[i]][tailAddresses[i]].push(tailIds[i]);
        ...
        emit Attach(tailAddresses, tailIds, parentIds, msg.sender);
    }
    ...
    /**
        ...
        * @dev `topOwnerOf` function.
        * @dev The tuple formed by elements from the three input arrays corresponding to a particular index
        * @dev represent one detachment triplet. Such a triplet comprises of the tokenIds of the parent and the tail tokens,
        * @dev and the contract address of the tail token.
        * @param tailAddresses Array of addresses of tail contracts
        * @param tailIds Array of tail tokenIds
        * @param parentIds Array of parent tokenIds
    */
    function detach(
        ...
        for (uint256 i=0; i<tailAddresses.length; i++) {
            ...
            require(parentOf[tailAddresses[i]][tailIds[i]] == parentIds[i], "Detach: Tail not attached to the parent");
            if (IERC1155Upgradeable(tailAddresses[i]).supportsInterface(0xd9b67a26) == true) {
            ...
            delete parentOf[tailAddresses[i]][tailIds[i]];
            popToken(tailAddresses[i], parentIds[i], tailIds[i]);
            if (tailsOf[parentIds[i]][tailAddresses[i]].length == 0) {
                popTailAddress(parentIds[i], tailAddresses[i]);
            }
        }
        emit Detach(tailAddresses, tailIds, parentIds, msg.sender);
    }

    /**
        * @notice Given the id and the address of the tail, this function returns it's parent.
    ...
    */
    function getParentOf(address tailAddress, uint256 tailId) public view returns (uint256) {
        return parentOf[tailAddress][tailId];
    }
    ...
    */
    ...
    /**
        * @notice This function returns all the tails of a parent belonging to a particular contract
        * @param parentId TokenId of the parent
        * @param tailAddress Address of the tails
        * @return Array of the ids of the tails
    */
    function getTailsOf(uint256 parentId,address tailAddress) public view returns (uint256[] memory) {
        return tailsOf[parentId][tailAddress];
    }
    ...
    */
    function getTailsAddresses(uint256 parentTokenId) public view returns (address[] memory) {
        return tailsAddresses[parentTokenId];
    }
    ...
    /**
        ...
        * @param parentTokenId The parent token id
        * @param tailTokenId The tail token id
    */
    function popToken(
        address tailAddress,
        ...
        uint256 lastTokenIndex = tailsOf[parentTokenId][tailAddress].length - 1;
        uint256 lastToken = tailsOf[parentTokenId][tailAddress][lastTokenIndex];
        uint256 tokenIndex = tailIndex[parentTokenId][tailAddress][tailTokenId];
        tailsOf[parentTokenId][tailAddress][tokenIndex] = lastToken;
        tailsOf[parentTokenId][tailAddress].pop();
        tailIndex[parentTokenId][tailAddress][lastToken] = tokenIndex;
        delete tailIndex[parentTokenId][tailAddress][tailTokenId];
    }
    ...
    */
    function popTailAddress(
        ...
        uint256 lastAddressIndex = tailsAddresses[parentTokenId].length - 1;
        address lastAddress = tailsAddresses[parentTokenId][lastAddressIndex];
        uint256 addressIndex = tailAddressIndex[parentTokenId][tailAddress];
        tailsAddresses[parentTokenId][addressIndex] = lastAddress;
        tailsAddresses[parentTokenId].pop();
        tailAddressIndex[parentTokenId][lastAddress] = addressIndex;
        delete tailAddressIndex[parentTokenId][tailAddress];
    }
