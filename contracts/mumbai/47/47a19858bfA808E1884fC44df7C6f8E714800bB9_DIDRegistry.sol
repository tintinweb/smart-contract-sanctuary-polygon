...
contract DIDRegistry is AbstractBase, IDIDRegistry, IERC165 {
   ...
   */
  function register(string calldata _method, address _id) external override whileActive {
   ...
   * @param _id ID address
   */
  function deactivate(string calldata _method, address _id) external override {
    if (_msgSender() != _id) {
  ...
  function isActive(string calldata _method, address _id) public view override whileActive returns (bool status) {
    bytes32 hash = keccak256(abi.encodePacked('did:', _method, ':', _id));
    uint168 record = registry[hash];
    address recordAddress = address(uint160(record >> 8));
    RegistrationState recordState = RegistrationState(uint8(record));
  ...
  function isActive(bytes32 _did) public view override whileActive returns (bool status) {
  ...
  function supportsInterface(bytes4 interfaceId) public pure override returns (bool) {
    return interfaceId == type(IDIDRegistry).interfaceId;
  }
}
