...
contract TournamentManager is VRFConsumerBaseV2 {
  ...
  function createTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint256[10] memory gotchiFormation, string memory teamName) public {
  ...
  function wrapToTeam(uint256[5] memory gotchis, uint256 gotchiLeader, uint256[10] memory gotchiFormation, address owner, string memory teamName) internal pure returns (Team memory) {
    Team memory newTeam;
  ...
  function modifyTeamBeforeBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint256[10] memory gotchiFormation, string memory teamName) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
    require(tournament_state == TOURNAMENT_STATE.INSCRIPTION_OPEN, "Tournament not open");
  ...
  function modiftyTeamAfterBattle(uint256 teamIndex, uint256[5] memory gotchis, uint256 gotchiLeader, uint256[10] memory gotchiFormation) external {
    require(gotchis.length == 5, "Invalid number of Gotchi IDs provided.");
  ...
  function getTeamFormation(uint256 teamId) public view returns (uint256[10] memory){
  ...
  function checkGotchiFormationValidity(uint256[10] memory gotchiFormation) public view returns (bool) {
    ...
    for (uint256 i = 0; i < 10; i += 2) {
        ...
        for (uint256 j = i + 2; j < 10; j += 2) {
            if (gotchiFormation[i] == gotchiFormation[j] && gotchiFormation[i + 1] == gotchiFormation[j + 1]) {
                return false;
            }
        }
    }
