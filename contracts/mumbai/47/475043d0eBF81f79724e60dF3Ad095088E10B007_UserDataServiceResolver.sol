 ...
 */
    ...
    function _createNewAccount(address accountOwner) internal returns (uint256) {
        ...
        bytes32 _ident = sha256(abi.encodePacked(tokenId, accountOwner));
...
abstract contract BaseUserRegistry is ERC721, Ownable {
    ...
    function _register(address _owner, bool update) internal  returns (uint256) {
        ...
        registrationQueue[index][index] = true;

        super._safeMint(_owner, index);

        registrationQueue[index][index] = false;
