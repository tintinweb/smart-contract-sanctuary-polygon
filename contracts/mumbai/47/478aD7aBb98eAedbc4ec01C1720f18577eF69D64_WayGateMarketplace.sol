...
contract WayGateMarketplace is WayGateRoyalties {
    ...
    struct NFTDetails {
        ...
        NftTokenType nftTokenTypeStatus; // select the AirdropNft status
    }
    ...
    event AdminSettingsUpdated(uint256 serviceFee, uint256 royaltyFee);
    ...
    function listNFTForTimedAuction(
        ...
        require(
            ...
            "WAYGATE MARKETPLACE: Timed Auction does not support ERC1155"
        );
        ...
        nftListingIdHashMapping[_nftListingIdHash].tokenId = _tokenId;
        nftListingIdHashMapping[_nftListingIdHash].price = _price;
        nftListingIdHashMapping[_nftListingIdHash].copies = 1;
        nftListingIdHashMapping[_nftListingIdHash].saleStatus = saleTypeChoice(
            1
        );
    ...
    function buyNFTForFixedPrice(
        ...
        uint256 uintPrice = msg.value / copies;
        require(
            uintPrice == nftListingIdHashMapping[_nftListingIdHash].price,
            "Insufficient Amount"
        );
    ...
    function _transferNftAndFeeHandler(
        ...
        if (
            nftListingIdHashMapping[_nftListingIdHash].nftTokenTypeStatus ==
        ...
        if (
            ...
            require(
                _copies <= nftListingIdHashMapping[_nftListingIdHash].copies,
                "ERC1155: The Number Of Copies Has Been Decreased"
            );
            require(
                _price ==
                    nftListingIdHashMapping[_nftListingIdHash].price * _copies,
                "ERC1155: The Price Must Be A Multiple Of The Number Of Copies"
            );
            nftContract1155.safeTransferFrom(
                address(this),
                _buyerAddress,
            ...
            uint256 remainingCopies = nftListingIdHashMapping[_nftListingIdHash]
                .copies - _copies;
    ...
    function setAdminSettings(
        ...
        uint256 _servicePercentage,
        uint256 _specialNftDiscountPercentage,
        uint256 _maxRoyaltyPercentage,
        address _hostContract
    ) external onlyOwner {
        require(
            _specialNftDiscountPercentage <= _servicePercentage,
            "Invalid Discount Fee"
        );
        airdropFeePercentage = _airdropPercentage;
        serviceFeePercentage = _servicePercentage;
        specialNftDiscountPercentage =
            _servicePercentage -
    ...
    function setPlatformTokenContractAddress(
        ...
        require(
            ...
            "Way-Gate Platform Token: Invalid Address"
        );
        require(
...
contract WayGateRoyalties is Initializable, OwnableUpgradeable {
    uint256 airdropHolderReceivingPercentage;
    uint256 partnersReceivingPrecentage;

    address airdropFeeDistributorAddress;
    address platformFeeReceiver;

    mapping(address => bool) exists;

    address[] wayGatePartners;
    ...
    function setWayGateDistrubutorAddress(
        address _airdropFeeDistributorAddress
    ) external {
        airdropFeeDistributorAddress = _airdropFeeDistributorAddress;
    }

    function setAirdropHolderReceivingPercentage(
        uint256 _airdropHolderReceivingPercentage
    ) external onlyOwner {
        airdropHolderReceivingPercentage = _airdropHolderReceivingPercentage;
    }

    function setPartnersReceivingPrecentage(
        uint256 _partnersReceivingPrecentage
    ) external onlyOwner {
        partnersReceivingPrecentage = _partnersReceivingPrecentage;
    }

    function setWayGatePartners(address[] calldata entries) external onlyOwner {
        for (uint256 i = 0; i < entries.length; i++) {
            address entry = entries[i];
            require(
                entry != address(0),
                "WayGateFeeDistributor: Cannot add zero address"
            );
            require(
                !exists[entry],
                "WayGateFeeDistributor: Cannot add duplicate address"
            );
            exists[entry] = true;
            wayGatePartners.push(entry);
        }
    }
    ...
    function _royaltyAndWayGateNFTFee(
        ...
        _transferAmountToMinter(royaltyAmount, minterAddress);
        _totalNftPrice = _totalNftPrice - (_WayGateFee + royaltyAmount);
        _transferAmountToSeller(_totalNftPrice, nftSeller); // Send Amount to NFT Seller after Tax deduction
        emit RoyaltiesTransfer(_WayGateFee, royaltyAmount, _totalNftPrice);
    }
    ...
    function _calculateWayGateFee(
        ...
        platformFeeDistribution(WayGateAmount);
    ...
    function _transferAmountToMinter(
        ...
        _minterAddress.transfer(_royaltyAmount);
    }

    function platformFeeDistribution(uint totalAmount) internal {
        uint256 airdropFeeAmount = (totalAmount *
            airdropHolderReceivingPercentage) / 1000;
        _transferAmountToAirdropHolders(airdropFeeAmount);
        uint256 partnersFeeAmount = (totalAmount *
            partnersReceivingPrecentage) / 1000;
        _transferAmountToPartners(partnersFeeAmount);
        totalAmount = totalAmount - (airdropFeeAmount + partnersFeeAmount);
        payable(platformFeeReceiver).transfer(totalAmount);
    }

    function _transferAmountToPartners(uint256 _partnersFee) internal {
        uint256 totalPartners = wayGatePartners.length;
        uint256 feePerInvsetor = _partnersFee / totalPartners;
        for (uint256 i; i < wayGatePartners.length; i++) {
            payable(wayGatePartners[i]).transfer(feePerInvsetor);
        }
        emit PartnersFeeTransferred(
            totalPartners,
            feePerInvsetor,
            _partnersFee
        );
    ...
    function _transferAmountToAirdropHolders(uint airdropAmount) internal {
        require(
            airdropFeeDistributorAddress != address(0),
            "WayGate Royalties: Set Airdrop Fee Distributor Address"
        );
        (bool success, ) = payable(airdropFeeDistributorAddress).call{
            value: airdropAmount
        }("");
    }
}