...
contract CrazyBearsStaking is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    function _stake(address _from, uint256 _tokenId) internal {
        require((msg.sender == stakableContractAddress),"Contract not recognised!");
    ...
    function stakeAll() public {
        ...
        require(stakeableContract.isApprovedForAll(msg.sender, address(this)), "You need to allow the contract access to your Bunny Hole tokens.");
        for(uint i=0; i < _tNum; i++){
            stakeableContract.safeTransferFrom(msg.sender, address(this), _tokens[i]);
            _stake(msg.sender, _tokens[i]);
        }
    }
