...
contract MarketplaceListing is Ownable, MarketplaceListingHelper {
    ...
    // MAPPINGS
    // User address to active listings
    mapping(address => EnumerableSet.UintSet) private addrToActiveListings;
    mapping(address => mapping(uint256 => uint256)) internal nftToLatestListingId;
    ...
    function getActiveListings() external view returns (uint256[] memory) {
     ...
     function getActiveOffers() external view returns (uint256[] memory) {
        return openOffers.values();
    }

    function getListingsByUser(address userAddress) external view returns (uint256[] memory){
        ...
        Listing memory listing = tokenIdToListing[_tokenId];
        return listing;
    }

    // Use listing ID and return offers array
    function getOfferIdsByTokenId(uint256 _tokenId)
        external
        view
        returns (uint256[] memory){
        return tokenIdToActiveOffers[_tokenId].values();
    }

    function getOfferByOfferId(uint256 offerId)
        external
        view
    ...
    // Use user addr and return all offers
    function getOfferIdsByUser(address _addr) external view returns (uint256[] memory){
         return addrToActiveOffers[_addr].values();
    }
    ...
    function createListing(uint256 tokenId, uint256 price, uint256 duration) public { 
        ...
        require(
            listingIdToListing[nftToLatestListingId[nftAddress][tokenId]].state != State.OPEN,
    ...
    function buyFromListing(uint256 listingId) public {
        ...
        require(IERC721(nftAddress).ownerOf(listing.tokenId) == listing.seller, "You must own the token");

        require(IERC721(nftAddress).isApprovedForAll(listing.seller, address(this)),"Must approve this contract for transfers.");
        ...
        IERC20(usdcAddress).transferFrom(
            ...
            listingIdToListing[listingId].price
        );
    ...
    function _removeListingStorage(uint256 listingId) internal {
        ...
        // Nft markets
        // nftToLatestListingId[nftAddress][listingIdToListing[listingId].tokenId].remove(listingId);

        // Map token id to listing
    ...
    function _addListingStorage(Listing memory listing) internal {
        ...
        // Nft listings
        // nftToLatestListingId[nftAddress][listing.tokenId].add(listing.listingId);

        // Map token id to listing
        tokenIdToListing[listing.tokenId] = listing;
    ...
    function _removeOfferStorage(uint256 offerId) internal {        
        ...
        // Remove from addr's active offer set
        addrToActiveOffers[msg.sender].remove(offerId);

        // Remove offer from listing's active offer set
    ...
    function _addOfferStorage(Offer memory offer) internal {
        ...
        // Add to offer id mapping 
        offerIdToOffer[offer.offerId] = offer;

        // Add to addr's active offer set
        addrToActiveOffers[msg.sender].add(offer.offerId);

        // Add offer to listing's active offer set
