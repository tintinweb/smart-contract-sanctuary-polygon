...
contract NFT is ERC721, ERC721Enumerable, ERC2981, Ownable, PullPayment, ReentrancyGuard, ContextMixin {
   ...
   function withdraw() external payable onlyOwner nonReentrant {
      ...
      // safeTransfer(address(this), msg.sender, address(this).balance);
   }
   ...
   function devMint(uint256 n) external nonReentrant onlyOwner {
      ...
      uint256 devSupply = devMinted + n;
      require(devSupply <= devReserve, 'dev mint limit reached');
      require(devSupply + supply <= maxSupply, 'max supply reached');
   ...
   function mint(uint256 quantity_) external payable nonReentrant {
	...
	require(ts.add(quantity_) <= maxSupply, 'Sold out');
     ...
     uint256 totalCost = quantity_.mul(mintPrice);
      ...
      require(walletMints[msg.sender] + quantity_ <= maxPerWallet, 'The maximum you can mint per address is 25 NFTs.');
