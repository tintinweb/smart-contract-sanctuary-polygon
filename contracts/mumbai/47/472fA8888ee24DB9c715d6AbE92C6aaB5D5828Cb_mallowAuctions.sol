...
contract mallowAuctions is Ownable{
    ...
    uint256 _startPrice, string memory _imgSrcLink, string memory _discordLink, string memory _twitterLink, uint256 _maxWhiteLists) external onlyApprovedAddresses{
        require(!auctionExists[_projectName],"The name is already defined");
        require(!auctionSettings[totalAuctions].exists,"This auction already exists");
        require(_startTime > 0 ,"Incorret start time value");
        require(_closeTime > 0 ,"Incorret close time value");
    ...
    bytes32 _r, bytes32 _s, address _sender) external onlyApprovedAddresses{
        ...
        require(!isWhiteListed(_sender,_Auction_ID),"Already whiteListed");
        ...
        whitelists[_Auction_ID].push(_sender);
        emit auctionEntry(_sender,_Auction_ID, auctionPrice, block.timestamp);
    }
    function updateAuction(uint256 _auctionId, string memory _projectName, uint256 _startTime, uint256 _closeTime, uint _dutchRate,uint256 _timeRate,
    uint256 _startPrice,uint256 _minPrice, string memory _imgSrcLink, string memory _discordLink, string memory _twitterLink, uint256 _maxWhiteLists) external onlyApprovedAddresses{
    ...
    function getWhitelists(uint256 _auctionId) public view returns (address[] memory){
        return whitelists[_auctionId];
    }
}