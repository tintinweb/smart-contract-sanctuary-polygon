...
contract NFTFactory is Ownable {
  ...
  modifier onlyValid(address nftContract) {
    require(isValid[nftContract], "NFTFactory: Invalid NFT contract");
    _;
  }
  ...
  function createERC721A(
    ...
    ERC721ATemplate newNFTcontract = new ERC721ATemplate(
      tokenName_,
      tokenSymbol_,
      maxItems_,
      mintPrice_,
    ...
    isValid[address(newNFTcontract)] = true;
    addresses.push(address(newNFTcontract));
    emit NFTContractCreated(address(newNFTcontract), NFTContractType.ERC721A);
  }

  function withdraw(address nftContract)
    external
    onlyOwner
    onlyValid(nftContract)
  {
    INFTContractTemplate(nftContract).withdraw(treasuryAddress);
  }

  function pauseMint(address nftContract)
    external
    onlyOwner
    onlyValid(nftContract)
  {
  ...
  function unpauseMint(address nftContract)
    external
    onlyOwner
    onlyValid(nftContract)
  {
  ...
  function setDevCut(address nftContract, uint devCut_)
    external
    onlyOwner
    onlyValid(nftContract)
  {
