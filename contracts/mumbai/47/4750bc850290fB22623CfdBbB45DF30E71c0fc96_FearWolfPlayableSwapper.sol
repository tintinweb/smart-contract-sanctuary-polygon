...
import "./common/access/IRoleContainerDistributor.sol";
import "./common/ERC20/IERC20.sol";
import "./common/ERC721/IERC721.sol";
...
interface IFearWolf is IERC721, IAccessControl, IRoleContainerDistributor {
    function getTokenParams(uint256 tokenId) external view returns (WolfParams memory);
    function setInitialOwner(address initialOwner, uint256 wolvesCount) external;
    function tokensOfOwner(address owner) external view returns(uint256[] memory);
}

contract FearWolfPlayableSwapper is AccessControl, Pausable, Withdrawable, ReentrancyGuard {

    IFearWolf public fearWolfContract;
    address public nonPlayableWolvesBuffer;
    ...
    constructor(address fearWolfContractAddress, address bufferAddress) {
        fearWolfContract = IFearWolf(fearWolfContractAddress);
        nonPlayableWolvesBuffer = bufferAddress;
    }

    function _beforeUnpause() internal view virtual override {
        require(fearWolfContract.hasRole(fearWolfContract.DISTRIBUTOR_ROLE(), address(this)), "Not a distributor");
    }

    function singleTrySuccessChance() public view returns (uint256) {
        return clan.length * percentageDivider / 40;
    }

    function multipleTriesSuccessChance(uint256 triesCount) external view returns (uint256) {
        uint256 singleChance = singleTrySuccessChance();
        uint256 singleFailChance = percentageDivider - singleChance;
        uint256 result = singleFailChance;

        for (uint i = 1; i < triesCount; i++) {
            result *= singleFailChance;
            result /= percentageDivider;
        }

        return percentageDivider - result;
    }

    function isPlayable(uint256 tokenId) public view returns(bool) {
        WolfParams memory wolfParams = fearWolfContract.getTokenParams(tokenId);

        for (uint i = 0; i < clan.length; i++) {
            if (wolfParams.clan == clan[i] && wolfParams.breed == breed[i])
                return true;
        }
    ...
    function exchangeToPlayableWolf(uint256 tokenId, uint256 triesCount) external whenNotPaused {
        address caller = _msgSender();
        address contract_ = address(this);

        require(fearWolfContract.ownerOf(tokenId) == caller, "Must be the owner");
        require(!isPlayable(tokenId), "Already playable");
        require(fearWolfContract.isApprovedForAll(caller, contract_), "Contract must be approved");

        for (uint i = 0; i < triesCount; i++) {
            fearWolfContract.setInitialOwner(contract_, 1);
            uint256[] memory contractTokens = fearWolfContract.tokensOfOwner(contract_);

            require(contractTokens.length == 1, "Something went wrong");
            uint256 newTokenId = contractTokens[0];
            if (isPlayable(newTokenId)) {
                fearWolfContract.safeTransferFrom(caller, contract_, tokenId, '');
                fearWolfContract.safeTransferFrom(contract_, caller, newTokenId, '');
                break;
            }
            else {
                fearWolfContract.safeTransferFrom(contract_, nonPlayableWolvesBuffer, newTokenId, '');
            }
        }
    }

    function addPlayableWolf(uint8 clan_, uint8 breed_) external onlyRole(ADMIN_ROLE) {
        clan.push(clan_);
        breed.push(breed_);
    }
}
...
/**
 * @dev Interface of a contract containing identifier for Distributor role.
 */
interface IRoleContainerDistributor {
    /**
    * @dev Returns Distributor role identifier.
    */
