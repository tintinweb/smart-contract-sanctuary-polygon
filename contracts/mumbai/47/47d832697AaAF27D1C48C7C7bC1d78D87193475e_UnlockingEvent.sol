...
/**
    @author The Calystral Team
    @title A contract for the Crate Unlocking Event
*/
contract UnlockingEvent is
    ...
    CommonConstants
{
    ...
    ==============================*/
    /**
        @dev MUST emit an assigned position for each crate that was received.
        The `serran` argument MUST be the user's address who sent the crate.
        The `randomPosition` argument MUST be the random position.
        The `challengeNumber` MUST be a uint256 challenging number (for cases where the random position is not unique).
    */
    event OnRandomPositionAssigned(
        address indexed serran,
    ...
    /// @dev A divisor used to create a smaller number space between 0 and 399,999.
    uint256 public constant NUMBER_SPACE_DIVISOR = type(uint256).max / 400000;
    ...
    */
    ...
    {
        require(
            _ids.length == 1 && _ids[0] == TRANSCENDENT_SET_CRATE_TYPEID,
            "The sent assets are invalid"
        );
        require(_values[0] % DECIMALS == 0, "Keine halben Sachen");

        _assetsContract().safeTransferFrom(
            address(this),
            getContractAddress(1005),
            TRANSCENDENT_SET_CRATE_TYPEID,
            _values[0],
            _data
        );

        uint256 crateAmount = _values[0] / DECIMALS;
        for (uint256 i = 0; i < crateAmount; i++) {
            // 	event thrown with random uint256 number
            uint256 randomNumber = _getRandomUnit256();
            emit OnRandomPositionAssigned(
                _from,
                randomNumber / NUMBER_SPACE_DIVISOR,
                randomNumber
            );
        }
        _randomnessNonce++;
