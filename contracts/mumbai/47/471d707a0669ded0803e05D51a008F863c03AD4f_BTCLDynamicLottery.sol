...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomness) internal override {
        // Determine the number of rounds to process
        uint256 numberOfRoundsToProcess = 0;
        uint256[] memory roundsToProcess = new uint256[](maxRoundsToDraw);

        for (uint256 i = nextRound; i < nextRound + maxRoundsToDraw; i++) {
            if (rounds[i].status.requestId == requestId && rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
                numberOfRoundsToProcess++;
                roundsToProcess[numberOfRoundsToProcess - 1] = i;
            }
        }

        // Check if we received the correct number of random numbers
        uint256 numWords = totalWinners * numberOfRoundsToProcess;
        require(randomness.length == numWords, "Invalid number of random words");

        // Loop through all rounds that need to be drawn and set the randomness
        uint256 startIndex = 0;

        for (uint256 i = 0; i < numberOfRoundsToProcess; i++) {
            ...
            startIndex += totalWinners;
        }

        // Increment the next round if all rounds have been drawn
        if (roundsToProcess[numberOfRoundsToProcess - 1] + 1 == nextRound + maxRoundsToDraw) {
            nextRound += maxRoundsToDraw;
            round++;
            rounds[round].status.roundStatus = BTCLCore.Status.Open;
