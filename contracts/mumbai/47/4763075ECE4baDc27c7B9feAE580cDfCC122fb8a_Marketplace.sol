...
contract Marketplace is ERC721Holder, Ownable {
  ...
  /**
   * @notice Marketplace fee that goes to Game reserve
   *
   * @dev 3 decimal precision - e.g. 50000 = 50%
   */
  uint256 private reserveFee;
   ...
   */
  constructor(address _card, address payable _daoAddress, uint256 _reserveFee, uint256 _daoFee) {
    card = CardNFT(_card);
   ...
   * @param oldReserveFee old value of reserve fee before change
   * @param newReserveFee new value of reserve fee after change
   * @param oldDaoFee old value of reserve fee before change
   * @param newDaoFee new value of reserve fee after change
   */
  event FeesChanged(
    address by,
    uint256 oldReserveFee,
   ...
   * @dev 3 decimal precision - e.g. 50000 = 50%
   *
   * @param _reserveFee new reserve fee value
   * @param _daoFee new DAO fee value
   */
  function changeFees(uint256 _reserveFee, uint256 _daoFee) external onlyOwner {
    // Emit event
    emit FeesChanged(msg.sender, reserveFee, _reserveFee, daoFee, _daoFee);
   ...
   * @param _collectionDaoAddress the address of collection dao to get marketplace fee
   * @return amount with fees deducted
   */
  function takeFeesFrom(uint256 _amount, address payable _collectionDaoAddress) internal returns (uint256) {
    // Calculate DAO fee portion
    uint256 daoFeePortion = _amount * daoFee / 100_000;

    // Send fee portion to DAO
    (bool daoSuccess,) = daoAddress.call{value: daoFeePortion}("");
    require(daoSuccess, "could not transfer to dao");

    // Calculate DAO fee portion
    uint256 reserveFeePortion = _amount * reserveFee / 100_000;

    // Send fee portion to Game reserve
    (bool reserveSuccess,) = daoAddress.call{value: reserveFeePortion}("");
    require(reserveSuccess, "could not transfer to reserve");

    // Get marketplaceRoyalty using smart contract address
    uint32 marketplaceRoyalty = CollectionDao(_collectionDaoAddress).marketplaceRoyalty();
    uint256 marketplaceRoyaltyFeePortion = _amount * marketplaceRoyalty / 100_000;

    if (marketplaceRoyaltyFeePortion > 0) {
      //Send % to collection dao
      _collectionDaoAddress.sendValue(marketplaceRoyaltyFeePortion);
    }

    return _amount - daoFeePortion - reserveFeePortion - marketplaceRoyaltyFeePortion;
  }
   ...
   *         one and guaranteed for at least 48 hours
  ...
  function placeOfferOnSellOrder(
    ...
    // is at least 48 hours in the future
    require(_offerGuarantee >= block.timestamp + 48 hours, "offer guarantee must be at least 48 hours in the future");
    ...
    if(sellOrder.highestOfferor != address(0) && !sellOrder.highestOfferor.isContract()) {
      ...
      payable(sellOrder.highestOfferor).transfer(sellOrder.highestOffer);
    }
  ...
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    address payable _collectionDaoAddress;
    ...
    for(uint256 i = 0; i < sellOrder.assetIds.length; i++) {
      if (i == 0) {
        ...
        (,uint16 cardCollection,,) = card.cardInfo(sellOrder.assetIds[i]);
        (,,,,_collectionDaoAddress,) = card.cardCollectionInfo(cardCollection);
      }
    ...
    if(!sellOrder.owner.isContract()) {
      ...
      payable(sellOrder.owner).transfer(takeFeesFrom(sellOrder.highestOffer, _collectionDaoAddress));
    }
   ...
   */
  function fulfilSellOrder(bytes32 _orderId) external payable onlyIfSenderNotContract sellOrderExists(_orderId) {
    // Copy sell order in memory
    SellOrder memory sellOrder = sellOrders[_orderId];
  ...
  function placeOfferOnBuyOrder(
    ...
    (uint256 cardType,,,) = card.cardInfo(_cardId);
  ...
  function acceptBuyOrderLowestOffer(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    if(!buyOrder.lowestOfferor.isContract()) {
      (,uint16 cardCollection,,) = card.cardInfo(buyOrder.lowestOfferCardId);
      (,,,,address payable _collectionDaoAddress,) = card.cardCollectionInfo(cardCollection);

      // Transfer Ether amount without fees to sell
      // (fees are paid to DAO and reserve in the takeFeesFrom function)
      payable(buyOrder.lowestOfferor).transfer(takeFeesFrom(buyOrder.lowestOffer, _collectionDaoAddress));
    }
  ...
  function fulfilBuyOrder(bytes32 _orderId, uint256 _cardId) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    ...
    // Ensure a card of correct type was sent
    (uint256 cardType,uint16 cardCollection,,) = card.cardInfo(_cardId);
    require(cardType == buyOrder.cardType, "invalid card type");
    ...
    // (fees are paid to DAO and reserve in the takeFeesFrom function)
    payable(msg.sender).transfer(takeFeesFrom(buyOrder.bid, _collectionDaoAddress));
    ...
    if(buyOrder.lowestOfferor != address(0) && !buyOrder.lowestOfferor.isContract()) {
      // Refund previous offeror
      payable(buyOrder.lowestOfferor).transfer(buyOrder.lowestOffer);
    }
 ...
 * @dev Represents a collection
  ...
  struct Collection {
    uint256 colMinted;
    uint256 colBurned;
    uint256 totalSupply; // for limited collections total count of the cards
    uint256 byDate; // for limited collections timestamp of the date until minting is allowed
    address payable daoAddress; // the dao address of collection
    uint8 isPrivate; // private collections can be minted only in full deck mint
  }
 ...
 * @dev Fired in addCollection()
   ...
   * @param daoAddress dao address of collection
   */
 ...
 * @notice Add new collection to cardCollectionInfo mapping
   ...
   * @param _daoAddress dao address of collection
   * @param _totalSupply total supplied amount of cards of collection
   * @param _byDate mint allowed by date of collection
   * @param _isPrivate is collection private
   */
  function addCollectionDaoAddress(uint16 _collection, address payable _daoAddress, uint256 _totalSupply, uint256 _byDate, uint8 _isPrivate) external onlyOwner {
    // verify dao address is set
    require(_daoAddress != address(0), "dao address is not set");

    require(_isPrivate <= 1, "private param can be 0 or 1");

    if (cardCollectionInfo[_collection].daoAddress != address(0)) {
      if (cardCollectionInfo[_collection].daoAddress != _daoAddress) {
        emit DaoAddressChanged(
          _collection,
          _daoAddress
        );
      }

      cardCollectionInfo[_collection].daoAddress = _daoAddress;
      cardCollectionInfo[_collection].totalSupply = _totalSupply;
      cardCollectionInfo[_collection].byDate = _byDate;
      cardCollectionInfo[_collection].isPrivate = _isPrivate;
    }
    else {
