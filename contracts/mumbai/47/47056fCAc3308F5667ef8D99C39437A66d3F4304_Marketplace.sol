...
contract Marketplace is AdminAndTangibleAccess, IMarketplace, IERC721Receiver {
    ...
    function buyUnminted(
        ...
        (uint256 tokenPrice, uint256 stock, , , ) = factory
            .priceManager()
            .getPriceOracleForCategory(nft)
            .usdcPrice(nft, _fingerprint, 0);

        require((tokenPrice > 0) && (stock > 0), "tnft !exist or 0 stock");
    ...
    {
        require(
            (fingerprints.length > 0) && (fingerprints.length < 101),
            "incorrect length"
        );
        ...
        for (uint256 i = 0; i < fingerprints.length; i++) {
            ...
            ) = factory.priceManager().getPriceOracleForCategory(nft).usdcPrice(
                    nft,
                    fingerprints[i],
                    0
                );
            weSellAt[i] = pricesOracle._weSellAt;
            weSellAtStock[i] = pricesOracle._weSellAtStock;
            weBuyAt[i] = pricesOracle._weBuyAt;
            weBuyAtStock[i] = pricesOracle._weBuyAtStock;
            lockedAmount[i] = pricesOracle._lockedAmount;
        }
    ...
    {
        require(tokenIds.length > 0, "incorrect length tokenids");
        uint256 length = tokenIds.length;
        PricesOracle memory pricesOracle;
    ...
    function latestTimeStampFromOracle(ITangibleNFT nft, uint256 fingerprint)
        external
        view
        override
        returns (uint256)
