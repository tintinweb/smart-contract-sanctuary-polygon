...
contract FastPay is Initializable, OwnableUpgradeable{
    ...
    function deposition(address token, uint256 amt) internal {
    ...
    function cashSweepAndMultiSend(
        ...
        require(sendAmt == withdraw.merchantAmt);

        if(checkBalance(withdraw)) {

            multiSendFromBalance(withdraw, payees, amounts);
    ...
    function multiSendFromBalance(
        ...
        TransferHelper.safeApprove(withdraw.token, nSender, withdraw.merchantAmt);
