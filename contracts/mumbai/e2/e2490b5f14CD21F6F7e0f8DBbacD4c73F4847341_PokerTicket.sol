...
contract PokerTicket is ERC721A, Ownable, ReentrancyGuard {
    ...
    event PriceConfigChanged(PriceConfig config);
    ...
    uint64 public constant MAX_FREE_TOKEN = 2000;
    uint64 public constant MAX_TOKEN_PER_WHITELIST_MINT = 2;
    ...
    constructor(string memory baseURI_, PriceConfig memory priceConfig_, MerkleRootConfig memory merkleRootConfig_) ERC721A("PokerTogether Pass Ticket", "PPT") {
        baseURI = baseURI_;
    ...
    function alphaSale(bytes32[] calldata signature_) external callerIsUser nonReentrant {
        require(status == Status.Alpha, "alpha sale has not enabled");
        require(isAlphaAddress(_msgSender(), signature_), "caller is not in alpha or invalid signature");
        uint256 alphaSaleMinted = _numberMinted(_msgSender()) + 1;
        require(alphaSaleMinted <= 1, "max mint amount per wallet exceeded");
    ...
    function whitelistSale(uint64 numberOfTokens_, bytes32[] calldata signature_) external payable callerIsUser nonReentrant {
        require(status == Status.WhitelistSale, "whitelist sale has not enabled");
        ...
        require(whitelistMinted <= MAX_TOKEN_PER_WHITELIST_MINT, "max mint amount per wallet exceeded");
    ...
    function publicSale(uint64 numberOfTokens_) external payable callerIsUser nonReentrant {
        require(status == Status.PublicSale, "public sale has not enabled");
    ...
    function getWhitelistSalePrice() public view returns (uint256) {
        return priceConfig.whitelistPrice;
    }
    ...
    /**
     * @notice setPriceConfig is used to set the configuration related to price.
     * This process is under the supervision of the community.
     * @param priceConfig_ price config
     */
    function setPriceConfig(PriceConfig calldata priceConfig_) external onlyOwner {
        require(priceConfig_.price > 0, "public sale price must greater than zero");
        require(priceConfig_.whitelistPrice > 0, "whitelist sale price must greater than zero");
        priceConfig = priceConfig_;
        emit PriceConfigChanged(priceConfig_);
    }
    
    /**
     * @notice setMerkleRootConfig is used to set the configuration related to merkle tree root.
     * This process is under the supervision of the community.
     * @param merkleRootConfig_ config
     */
