...
import "./TokenHoldings.sol";
import "../interfaces/IFACTORYHOLDINGS.sol";

contract FactoryERC20 is IFACTORYHOLDINGS, Ownable { 
    //////////////////////////////////////////////////////////////////////////////////////////////////
    // State
    //////////////////////////////////////////////////////////////////////////////////////////////////

    address private s_contractAddressDividends;
    ...
    function setDividends(address p_contractAddressDividends) public onlyOwner override {
        s_contractAddressDividends = p_contractAddressDividends;
    }
    ...
    function create(
        ...
        return address(
            new TokenHoldings(
                ...
                p_symbol, 
                msg.sender,
                p_contractAddressSeller, 
                s_contractAddressDividends,
                p_amountToSell 
            )
        );
    }
...
interface IMANAGERWL {
    function whiteList() external view returns(address);
}

contract TokenHoldings is ITOKENHOLDINGS, ERC20, Pausable, Ownable {
    //////////////////////////////////////////////////////////////////////////////////////////////////
    // State
    //////////////////////////////////////////////////////////////////////////////////////////////////

    address private s_manager;
    address private s_contractSeller;
    address private s_contractDividends;
    ...
    constructor(
        ...
        address p_contractDividends,
        uint256 p_amount
    ) ERC20(p_name, p_symbol) {
        s_manager = p_manager;
        s_contractSeller = p_contractSeller;
        s_contractDividends = p_contractDividends;

        _mint(p_contractSeller, p_amount);
    ...
    function forcedTransferStocks(address p_from, address p_to) public onlyOwner override returns(bool) {
        _transfer(
            p_from,
            p_to,
            balanceOf(p_from)
        ...
        emit ForcedTransferStocks(p_from, p_to);
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount)
        ...
        override
    {
        ISELLER sellerContract = ISELLER(s_contractSeller);
        
        if (from != address(0)) {
            require(
                sellerContract.canTransferHoldings(),
                "Contract seller reverte the transfer"
            );
        }

        IWHITELIST.Status statusToWL = IWHITELIST(IMANAGERWL(s_manager).whiteList()).status(to); 

        require(
            statusToWL == IWHITELIST.Status.Active ||
            from == address(0) ||
            to == s_contractSeller || 
            to == address(0), 
            "To account not active"
        );

        bool statusFrom = sellerContract.status(from);
        bool statusTo = sellerContract.status(to);
        
        if (statusFrom) {
            if (!statusTo) {
                require(sellerContract.setThroughCompany(amount, false), "Setting error");
            }
        }

        if (!statusFrom) {
            if (statusTo) {
                require(sellerContract.setThroughCompany(amount, true), "Setting error");
            }
        }

        uint256 amountStock = IDIVIDENDS(s_contractDividends).amountStock(address(this));

        if (from != address(0) && from != s_contractSeller && amountStock > 0 && amount > 0) {
            if (!statusFrom) {
                IDIVIDENDS(s_contractDividends).addToClaim(from, amount * amountStock); 
            }

            if (to != address(0) && to != s_contractSeller && !statusTo) {
                IDIVIDENDS(s_contractDividends).addClaimedDividends(to, amount * amountStock);
            }
        }
