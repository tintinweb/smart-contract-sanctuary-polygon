...
contract lockICO is TokenTimelockUpgradeable {
    function initialize(
...
contract IcoGeneric is ReentrancyGuard, Ownable {
    ...
    Sale[] public salesList;

    address[] public allowedUsdAddress;
    ...
    constructor(
        ...
        token = IERC20MetadataUpgradeable(_tokenAddress);
    ...
    // The amount must be sent with the same DECIMALS that the USDT (6), DAI (18)
        ...
        isAllowedUsdAddress(usdAddress)
    {
        ...
        require(tokenAmountInWei > 0, 'amount cannot be 0');

        require(tokenBalance() > 0 && tokenBalance() >= tokenAmountInWei, 'The ICO has no tokens available to sell anymore.');

        uint256 allowanceInWei = usd.allowance(msg.sender, address(this));

        // We first convert it to WEI
        uint256 usdAmountInWei = convertUsdToWei(amountInUsd, usd);

        require(allowanceInWei >= usdAmountInWei, 'You must allow this contract to send USDT or increase the amount.');

        // From WEI we convert it back to USD so we are sure it has the same decimals as requested
    ...
    // It will check the decimals of the STABLE COIN
    // For DAI we have 18 decimals WEI
    // For USDT we have 6 decimals
    // First we convert it back to ETHER, then to WEI
    ...
    // The amount to sell must follow the TOKEN decimals
    // Usually 18, but we never know
    ...
    function transferTokenToLockedAddresses(
        ...
        for (uint256 i = 1; i <= term; i++) {
            ...
            lockICO timeLockContract = new lockICO();
    ...
    function setAllowedUsdAddress(address usdAddress) external onlyOwner {
        allowedUsdAddress.push(usdAddress);
    }

    function listSales() external view returns (Sale[] memory) {
        Sale[] memory sales_ = new Sale[](salesList.length);
        for (uint256 i = 0; i < salesList.length; i++) {
            sales_[i] = salesList[i];
        }
        return sales_;
    }

    function deleteAllowedUsdAddress(address usdAddress) external onlyOwner {
        for (uint16 i = 0; i < allowedUsdAddress.length; i++) {
            if (allowedUsdAddress[i] == usdAddress) {
                delete allowedUsdAddress[i];
                break;
            }
        }
    }
    ...
    modifier isAllowedUsdAddress(address usdAddress) {
        ...
        require(isAllowed, 'This USD address is not in our white list.');
        _;
    }
    ...
    modifier notInvesting() {
        require(!isAlreadyInvesting(), "You can't participate of the ICO more than one time.");
        _;
    }
}
