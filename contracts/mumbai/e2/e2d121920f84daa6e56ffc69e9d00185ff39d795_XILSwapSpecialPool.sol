...
contract XILSwapSpecialPool is Ownable {
    ...
    constructor(address _token, uint256 miniMumStake, uint256 referral) {
    ...
    // Safe XILSwap withdraw function by admin
    function safeXILSwapWithdraw(address _to, uint256 _amount)  external onlyOwner {
        uint256 XILSwapBalalance = IBEP20(TOKEN).balanceOf(address(this));
        if (_amount > XILSwapBalalance) {
            IBEP20(TOKEN).transfer(_to, XILSwapBalalance);
        } else {
