...
contract TestNFTHoHo is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_GIFT = 333;
    uint private constant MAX_WHITELIST = 2000;
    uint private constant MAX_PUBLIC = 1000;
    ...
    uint private constant maxPerAddressDuringPublicMint = 5;

    bool public isPaused;

    uint private teamLength;

    address[] private _team = [0x92B8B880525e7913ded64ac09be099A085A25e19];

    uint[] private _teamShares = [100];

    constructor(bytes32 _merkleRoot, string memory _baseURI)
    ERC721A("TestNFTHoHo", "Hoho") 
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser{
        ...
        require(amountNFTperWalletWhitelistSale[msg.sender] + _quantity <= maxPerAddressDuringWhitelistMint, "You can only get 5 NFT on the Whitelist Sale");
    ...
    function publicMint(address _account, uint _quantity) external payable callerIsUser {
        ...
        require(amountNFTperWalletPublicSale[msg.sender] + _quantity <= maxPerAddressDuringPublicMint, "You can only get 5 NFTs on the Whitelist Sale");
