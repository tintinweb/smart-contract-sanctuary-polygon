...
contract NFTMarket is ReentrancyGuard {
    ...
    function createMarketItem(
        ...
        uint256 tokenId,
        address seller,
        uint256 price) public payable nonReentrant returns(uint){
         ...
         idMarketItem[itemId] = MarketItem(
             ...
             nftContract,
             tokenId,
             payable(seller), //address of the seller putting the nft up for sale
        ...
        function saleItems(
            address nftContract,
            uint256[] memory itemIdList
            ) public payable nonReentrant {
                uint totalPrice = 0;
                require(itemIdList.length > 0, "Item list must not be empty!");
                for (uint256 i = 0; i < itemIdList.length; i++) {
                    totalPrice += idMarketItem[itemIdList[i]].price;
                }
                require(msg.value == totalPrice, "Please submit the asking price in order to complete purchase");
                for (uint256 i = 0; i < itemIdList.length; i++) {
                    uint tokenId = idMarketItem[itemIdList[i]].tokenId;
                    //pay the seller the amount
                    idMarketItem[itemIdList[i]].seller.transfer(msg.value - _getListingPrice(idMarketItem[itemIdList[i]].price));
                    //transfer ownership of the nft from the owner to the buyer
                    IERC721(nftContract).transferFrom(idMarketItem[itemIdList[i]].seller, msg.sender, tokenId);
                    idMarketItem[itemIdList[i]].owner = payable(msg.sender); //mark buyer as new owner
                    idMarketItem[itemIdList[i]].sold = true; //mark that it has been sold
                    _itemsSold.increment(); //increment the total number of Items sold by 1
                    payable(owner).transfer(_getListingPrice(idMarketItem[itemIdList[i]].price)); //pay owner of contract the listing price
                }
        ...
        function _getListingPrice(uint256 price) private view returns(uint) {
            return price / 100 * listingPricePercent;
        }
