...
contract Chat {
    Structures structures;
     constructor() {
    structures = Structures(0xF62048bBd3AF9aAE8BF6F2d4DF3dcdA7c2505106);
}
    enum DeletionStatus {
        NotDeleted,
        DeletedBySender,
...
function addReply(uint256 messageId, string memory response, Message memory messageOriginal, bool setRep, uint256 timestamp) private {
   ...
   function sendMessageToGroup(address[] memory receiver, string memory subject, string []memory message, string []memory cciMessages,bool isShareble, string memory fileHash, string memory emailGroup, address[] memory cciReceivers, uint256 timestamp) external {
        require(
            structures.checkUserExists(msg.sender) == true,
            "You must have an account"
        );
...
contract Operations {
    ...
    constructor() {
        structures = Structures(0xF62048bBd3AF9aAE8BF6F2d4DF3dcdA7c2505106);
        chat = Chat(0x98c8691FF00E564b4d4e5E61e89C3eA510817E8c);
    }
...
contract Structures {
    ...
    function deleteUser(address walletAddress) public onlyAdmin {
        ...
        require(
            checkUserExists(walletAddress) == true,
            "User with given address does not exist"
        );
        ...
        for (uint i = 0; i < userAddresses.length; i++) {
            if (userAddresses[i] == walletAddress) {
                ...
                userAddresses.pop();
                userEmails[i] = userEmails[userEmails.length - 1];
                userEmails.pop();
                break;
            }
        }
