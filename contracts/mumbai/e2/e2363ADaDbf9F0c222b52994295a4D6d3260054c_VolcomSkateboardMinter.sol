...
interface ExistingWTCContract{
    ...
    function punchACard(string memory eventId, uint256 _tID, uint increment) external;
...
contract VolcomSkateboardMinter is Ownable{
    ...
    function getRandomEID() internal view returns (uint){
        uint x = uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp, block.number)));
    ...
    function mintWhitelist() external earlyMint {
    ...
    function mint() external payable regularMint{
