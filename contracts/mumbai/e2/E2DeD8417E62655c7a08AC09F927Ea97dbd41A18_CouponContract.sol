...
contract CouponContract is ERC721, Ownable{
    ...
    */
    ...
    uint public CouponCreated_Code = 201;
    uint public CouponAccepted_Code = 202;
     ...
     */
    function createCoupon(uint _id, string memory _ipfsUrl, uint _start, uint _end) public onlyAdmin{
        if(idCheck[_id]){
        ...
        tokenIds.push(_id);
        idCheck[_id] = true;
        emit CouponCreated(_id, _ipfsUrl, CouponCreated_Code);
    }
    ...
    function redeemCoupon(address _driverAddress, uint _id) external onlyAdmin{
        ...
        emit Redeemed(_driverAddress, _id, CouponAccepted_Code);
    }
    ...
    function viewCouponValidityToUser(address _ad, uint _couponId) external view returns(
        ...
        if(linkConfirmation[_ad][_couponId] && viewCouponStatus(_couponId) == false){
            return(
            couponInformation[_couponId]._tokenId,
            couponInformation[_couponId]._startDate,
            couponInformation[_couponId]._endDate,
...
contract RegistrationContract is Ownable {
    ...
    // type - Event
    event DriverRegistered(address indexed _driverAddress);
    ...
    function registerDriver(address _newDriver) public onlyAdmin {
        ...
        emit DriverRegistered(_newDriver);
    }
