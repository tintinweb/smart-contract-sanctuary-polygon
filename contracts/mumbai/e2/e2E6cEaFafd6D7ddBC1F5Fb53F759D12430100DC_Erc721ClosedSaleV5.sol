...
abstract contract Erc721SaleV5 is AccessControlV5, ERC721, ERC721Royalty, ERC721Pausable {
  ...
  event SignatureVerifierUpdated(address oldSignatureVerifier, address newSignatureVerifier);
  ...
  /**
        @dev This method can be called to update the signatureVerifier address, if a new SignatureVerifier is deployed.
        @param signatureVerifier_ The new address to be used.
    */
  function updateSignatureVerifier(address signatureVerifier_) external onlyRole(CONTRACT_ADMIN_ROLE) {
    require(signatureVerifier_ != address(0) && signatureVerifier_ != signatureVerifier, "36");
    address oldSignatureVerifier = signatureVerifier;
    signatureVerifier = signatureVerifier_;
    emit SignatureVerifierUpdated(oldSignatureVerifier, signatureVerifier);
  }
