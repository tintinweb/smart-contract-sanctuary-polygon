...
import "../Interfaces/IEarlyAccessNFT.sol";
 ...
 */
contract SwapPage is Initializable{
 ...
 */
     ...
     */
    function purchaseBYSL(address givenToken,uint amount) external {
        require(amount > 0, 'amount is greater than zero.');
        require(startTime <= block.timestamp,"purchase yet to start");
        if(startTime + (IEarlyAccessNFT(Admin.EarlyAccess()).top() * 1 hours) >= block.timestamp){
            require((startTime + (IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) * 1 hours) >= block.timestamp)
            && (IEarlyAccessNFT(Admin.EarlyAccess()).Won(msg.sender) != 0),"no access to purchase");
        }
        if(givenToken == Admin.BUSD()){
            IERC20(Admin.BUSD()).safeTransferFrom(msg.sender,address(this),amount);
        }else{
            IERC20(givenToken).safeTransferFrom(msg.sender,address(this), amount);
            IERC20(givenToken).approve(Admin.ApeswapRouter(), amount);
            address[] memory path = new address[](2);
            path[0] = givenToken;
            path[1] = Admin.BUSD();
            amount = IUniswapV2Router02(Admin.ApeswapRouter()).swapExactTokensForTokens(
                amount,
                0,
                path,
                address(this),
        ...
        (uint BUSDAmount, uint leftAmount) = IReferralNFT(Admin.Refferal()).rewardDistribution(msg.sender, (amount *1/10));
        if(amount != 0){
            console.log("Admin.Refferal()",Admin.Refferal());
            IERC20(Admin.BUSD()).transfer(Admin.Refferal(),BUSDAmount);
        }
        if(leftAmount != 0){
            IERC20(Admin.BUSD()).transfer(Admin.Treasury(),leftAmount/2);
            IERC20(Admin.BUSD()).transfer(Admin.TeamAddress(),leftAmount/2);
        }
        IERC20(Admin.BUSD()).safeTransfer(Admin.TeamAddress(),(amount *9/20));
        IERC20(Admin.BUSD()).safeTransfer(Admin.Treasury(),(amount *9/20));
        uint ProtocolPricebYSL = (amount * 9) / 10 * (IBYSL(bYSL).protocolPrice()) / 10 **18; //Divide by 10**18 for testing 
        // uint currentBackedPrice = IBYSL(Admin.bYSL()).backedPrice();
        IBYSL(Admin.bYSL()).mint(msg.sender, ProtocolPricebYSL);
    }

    function setStartTime(uint _startTime) external{
        require(msg.sender == Admin.EarlyAccess() || msg.sender == Admin.operator());
     ...
     */
    function sellBYSLviaSWAP(address wantToken, uint256 amount) external { 
        require(amount > 0, "Amount cannot be zero");
        uint256 BUSDAmount;
        IProtocolOwnedLiquidity(Admin.POL()).sellBYSL(msg.sender,amount);
        if(wantToken == Admin.BUSD()){
        BUSDAmount = IERC20(Admin.BUSD()).balanceOf(address(this));
        IERC20(Admin.BUSD()).safeTransfer(msg.sender,BUSDAmount);
        }
        else{
            address[] memory path = new address[](2);
            path[0] = Admin.BUSD();
    ...
    Note allocation point should be equal to tax
        ...
        only admin can call 
     */
    function setbYSLAndAllocationTax(uint ExitFees, uint[] memory allocationExitFees) external _isAdmin {
        require(ExitFees > 0, "bYSL: Tax Should be greater than Zero");
        require(allocationExitFees.length > 0, "bYSL: AllocationTax should not be empty");
        uint total;
        for(uint i; i < allocationExitFees.length; i++) {
            total += allocationExitFees[i];
        }
        require(total == ExitFees, "bYSL: Tax not equal to sum of allocation tax");
        exit_Fees = ExitFees;
...
interface IReferralNFT{
    function rewardDistribution(address user, uint _reward) external returns(uint amount, uint leftAmount);
}
