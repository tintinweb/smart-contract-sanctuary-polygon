...
contract Gratie is AccessControlUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function initialize(
        ...
        address _businessNFTs,
        address _serviceProviderNFTs,
        address _defaultAdminAddress,
        address _usdcContractAddress,
        address[] calldata _gratiePlatformAdmins,
        BusinessNftTier[] calldata _businessNftTiers
        ...
        businessNFTs = IERC721(_businessNFTs);
        serviceProviderNFTs = IERC1155(_serviceProviderNFTs);
        usdc = IERC20Upgradeable(_usdcContractAddress);

        for(uint256 i; i<_gratiePlatformAdmins.length;) {
            _grantRole(GRATIE_PLATFORM_ADMIN, _gratiePlatformAdmins[i]);
            unchecked { ++i; }
        }
        _addBusinessNftTiers(_businessNftTiers);
    }
    ...
    function registerBusiness(
        ...
        // Transfer USDC from msg.sender to platform fee receiver.
        usdc.transferFrom(msg.sender, platformFeeReceiver, tierData.usdcPrice);
        
        // Mint ERC-721 NFT
        businessNFTs.mint(msg.sender, totalBusinesses, _nftMetadataURI);
    ...
    function generateRewardTokens(
        ...
        if (rewardToken == address(0)) {
            rewardToken = address(new RewardToken(_tokenName, _tokenSymbol, _tokenIconURL, address(this)));
