...
contract RaffleMarketplace is Ownable, VRFConsumerBaseV2 {
    /*
        1. Create raffle
    ...
    struct Raffle {
        ...
        uint256 durationofRaffle; // how long will the raffle go on once starts
    ...
    address public linkTokenAddress = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB; // token address for mumbai
    ...
    bytes4 private constant FUNC_SELECTOR =
        bytes4(keccak256("register(string,bytes,address,uint32,address,bytes,uint96,uint8)"));
        uint256 MINIMUM_FUNDING_AMOUNT = 1 ether;
    ...
    function verifyRaffle(uint256 id) external invalidTickerId(id) doesRaffleExists(id) onlyOwner {
        ...
        RaffleContract raffle = new RaffleContract(
            id,
            block.timestamp + raffleStruct.durationofRaffle,
    ...
    */
    ...
    function modifyLinkTokenAddress(address tokenAddress) external onlyOwner{
        linkTokenAddress=tokenAddress;
        LINKTOKEN=LinkTokenInterface(tokenAddress);
    }

    function modifyMinimumFundingAmount(uint256 amount) external onlyOwner{
        MINIMUM_FUNDING_AMOUNT = amount;
    }
...
/*
    TODO:
    add subscription to the deployed raffle contract
