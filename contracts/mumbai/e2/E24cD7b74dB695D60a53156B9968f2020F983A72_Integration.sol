    ...
    GovernorTimelockControl
{
    ...
    /// @dev Get Aggregate Info.
    /// @param proposalID Proposal ID.
    function getProposalAggregateAll(uint256 proposalID, address account)
    public
    view
    returns (ProposalAggregate[] memory) {
        ...
        for(uint256 i=0;i<size;i++){
            ...
            proposal_aggregate[i].state = state(proposalID);
            (proposal_aggregate[i].votes_for,proposal_aggregate[i].votes_against,) = proposalVotes(proposalID);
            proposal_aggregate[i].proposalDeadline =  proposalDeadline(proposalID);
            proposal_aggregate[i].proposalSnapshot = proposalSnapshot(proposalID);
            proposal_aggregate[i].user_voted = getVotes(msg.sender,block.number-1);
            proposal_aggregate[i].quorum = quorumPerGroup(proposalID);
        }
