...
contract RaceFactory is IRaceFactory {
    ...
    /// @param raceID_ as string.
    /// @param title_ as string.
    /// @param uri_ as string.
    /// @param raceStartTime_ as uint256.
    /// @param raceAccess_ as uint256 index in RaceAccess type.
    /// @return Race contract.
    function newRace(
...
interface INitroLeague {
    ...
    function createRaceEvent(string calldata raceEventID, uint256 raceEventType, string memory title, string memory uri) external returns(address);
    // Races.
...
contract Race is IRace, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    function endRace(address payable[] memory results_) external override {
        ...
        if(raceAccess == RaceAccess.OPEN && feeAmount > 0 && players.length > 0 && feeToken.balanceOf(address(this)) > 0){
            feeToken.transfer(treasuryWallet, feeAmount * players.length);
