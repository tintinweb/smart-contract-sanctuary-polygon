...
contract BaseV2Voter is IveListener, ContractGuard, ReentrancyGuard, PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(address __ve) external initializer {
        ...
        ve = __ve;
        poolLength = 3; // [0, 1, 2] = [Protocol owned liquidity, Treasury, Profit share]

        poolWeightMin = [3000, 1000, 1000];
        poolWeightMax = [7000, 5000, 5000];
    }

    function setEpochReward(uint256 _epochReward) external onlyOwner {
        epochReward = _epochReward;
        if (totalWeight > 0){
            IveCssRoom(boardroom).onRewardUpdated(epochReward, totalWeight, weights[2]);
        }
    }
    ...
    function getStakerOfNft(uint256 _tokenId) public view returns (address) {
        ...
        return _boardroom == address(0) ? address(0) : IveCssRoom(_boardroom).stakerOfNFT(_tokenId);
    }
