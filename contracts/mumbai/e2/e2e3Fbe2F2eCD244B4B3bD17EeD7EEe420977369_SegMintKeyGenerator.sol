...
contract SegMintKeyGenerator is SegMintExchangeStructs {
    ...
    event setAdminAddressEvent(
    address indexed AdminAddress,
    address previousAdminAddress,
    ...
    modifier onlyAdmin() {
        require(
            msg.sender == _admin,
            "SegMint Exchange: Sender is not the Admin address!"
        );
        _;
    }
    ...
    // Set Admin Address
    function setAdminAddress(address admin_)
        public
        onlyAdmin
        notNullAddress(admin_, "Admin Address")
    {
        // Store previous admin address
        address previousAdminAddress = _admin;

        // Update admin address
        _admin = admin_;

        // Emit event
        emit setAdminAddressEvent(
            msg.sender,
    ...
    //             NFTOwnerAddress: NFTCreatorAddress_,
    ...
    onlyAdmin
  {
    require(NFTOwnerAddress_ != address(0), "SegMint Exchange: Cant Fractionalize burnt NFT");
    // get locking and fractionalization index
    uint256 lockAndFractionalizationIndex = _SegMintExchangeDB
      .getLockingAndFractionalizingIndex(NFTContractAddress_, NFTTokenID_);
    ...
    _SegMintExchangeDB.createLockingAndFractionalizationInfo(
      lockingAndFractionalizingNFTInfo({
        lockAndFractionalizationIndex: _SegMintExchangeDB
          .getLockingAndFractionalizingIndex(NFTContractAddress_, NFTTokenID_),
        NFTCreatorAddress: NFTOwnerAddress_,
        NFTOwnerAddress: NFTOwnerAddress_,
        NFTContractAddress: NFTContractAddress_,
        NFTTokenID: NFTTokenID_,
        isSegMintNFT: true,
        lockingContractAddress: address(0),
        lockerAddress: address(this),
        isLocked: true,
        lockingTimestamp: block.timestamp,
        unlockTimestamp: 0,
...
contract SegMintExchange is SegMintExchangeStructs {
    ...
    // purchase fractions from multiple listings at once
    function BulkPurchaseFraction(bulkPurchase[] memory data) public payable onlyKYCAuthorized {
        uint256 purchaseCost = 0;
        for (uint256 i=0; i<data.length; i++) {
            // fetch the listing from the _listingsInfo
            ListingInfo memory ListingINFO = _SegMintExchangeDB
            .getListingInfoByListingIndex(data[i].listingIndex);
             purchaseCost += ListingINFO.pricePerFraction * data[i].amount;
            _purchaseFractions(data[i].amount, data[i].listingIndex);
        }
        require(msg.value == purchaseCost, "Please send proper msg value according to the purchase quantity");
    }

    // user purchase fractions
    function _purchaseFractions(uint256 amount_, uint256 listingIndex_)
        internal
        onlyExistingListingIndex(listingIndex_)
    {
        // purchase amount should be greater than zero
        ...
        // transfer payment to nft owner
        payable(ListingINFO.ERC1155TokenOwnerAddress).transfer(ListingINFO.pricePerFraction * amount_);
