...
contract WowTCommunity is OwnableUpgradeable, AccessControlUpgradeable {
    ...
    function addMembers(
        ...
        if (!(defaultCommunity[_communityName])) {
            ...
            points.reducePoints(_communityParticipant, communityEntryPoints);
        }
        communityMap[_communityName].members[_communityParticipant] = true;
        communityMap[_communityName].userName[
...
contract WowTCommunityVotings is OwnableUpgradeable, AccessControlUpgradeable {
    struct Movie {
        string title;
        uint256 voteCount;
        string imageURL;
    ...
    mapping(string => Category) public communities;
    mapping(uint256 => Movie) public movies;
    mapping(string => uint256) public categoryCount;
    mapping(string => uint256) public movieCount;

    address public communityContract;
    WowTCommunity private community;

    // constructor(address _communityContract) {
    //     wowTCommunity = WowTCommunity(_communityContract);
    // }

    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE"); // Admin role for authorization

    event CatagoryCreated(
        string communityName,
        string catagory,
        uint256 catagoryId
    );
    event MovieVoted(string communityName, string catagory, uint256 catagoryId);

    function initialize(address _communityContract) public initializer {
        __Ownable_init();
        _grantRole(ADMIN_ROLE, _msgSender());
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
    ...
    function addCategory(
        ...
        string[] memory _movieTitles,
        string[] memory _movieImageURLs
    ) public {
        require(
        ...
        // );
        require(
            _movieTitles.length == _movieImageURLs.length,
            "Mismatched titles and images"
        );

        uint256 categoryId = categoryCount[_communityName];
        categoryCount[_communityName] = categoryId;

        Category storage category = communities[_communityName];
        category.name = _categoryName;
        category.movieIds = new uint256[](_movieTitles.length);

        for (uint256 i = 0; i < _movieTitles.length; i++) {
            uint256 movieId = movieCount[_communityName];
            movieCount[_communityName] = movieId;

            movies[movieId] = Movie({
                title: _movieTitles[i],
                voteCount: 0,
                imageURL: _movieImageURLs[i]
            });
            category.movieIds[i] = movieId;
            movieCount[_communityName]++;
        }

        categoryCount[_communityName]++;

        emit CatagoryCreated(_communityName, "vote", categoryId);
    }
    ...
    function vote(
        ...
        // );
        require(
            _categoryId <= categoryCount[_communityName],
            "Invalid category ID"
        );
        require(_movieId <= movieCount[_communityName], "Invalid movie ID");

        Category storage category = communities[_communityName];
        require(
            !category.voted[_msgSender()],
            "Already voted in this category"
        );

        category.voted[_msgSender()] = true;
        movies[_movieId].voteCount++;
        emit MovieVoted(_communityName, "vote", _categoryId);
    }
    ...
    function getMovie(
        ...
        return (movie.title, movie.voteCount, movie.imageURL);
    }
    ...
    function getCategory(
        ...
        Category storage category = communities[_communityName];
        return (category.name, category.movieIds);
    }
    ...
    function getCategoryByName(
        ...
        Category storage category = communities[_communityName];
        for (uint256 i = 0; i < categoryCount[_communityName]; i++) {
            uint256 categoryId = i;
            if (
                keccak256(bytes(category.name)) ==
...
contract WowTPoints is OwnableUpgradeable, AccessControlUpgradeable {
    ...
    function addActiveUserPoints(
        ...
        addPoints(account, activeUserPoints, _catagory);
    }
