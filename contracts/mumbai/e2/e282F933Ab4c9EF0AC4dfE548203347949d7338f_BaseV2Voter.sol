    ...
    OwnableUpgradeable
{
    ...
    address public boardroom; // VeFbaRoom
    ...
    function reset(uint256 _tokenId) external onlyOneBlock whenNotPaused {
        require(
            Ive(_ve).isApprovedOrOwner(msg.sender, _tokenId) ||
                getStakerOfNft(_tokenId) == msg.sender,
            "!owned"
        );
        _reset(_tokenId);
    ...
    function vote(uint256 _tokenId, uint256[] calldata _weights)
        external
        onlyOneBlock
        whenNotPaused
    {
        ...
        for (i = 0; i < _poolCnt; i++) {
            ...
            require(
                _w >= poolWeightMin[i] && _w <= poolWeightMax[i],
                "out of range"
            );
            _totalVoteWeight += _w;
        }
    ...
    function addPool(uint256 _weightMin, uint256 _weightMax)
        external
        onlyOwner
    {
        require(
            _weightMin >= 0 && _weightMin <= _weightMax && _weightMax <= 10000,
            "out of range"
        );
        poolLength++;
    ...
    function getStakerOfNft(uint256 _tokenId) public view returns (address) {
        ...
        return
            _boardroom == address(0)
                ? address(0)
                : IveFbaRoom(_boardroom).stakerOfNFT(_tokenId);
    }
