...
// NOTE: using solidity 0.6.6 to match imports 0xD71B6AeAaD532D139A6F0067dF7aDC69d6F5C869
...
contract UniswapV2Twap {
    ...
    // NOTE: IUniswapV2Pair
    constructor(IUniswapV2Pair _pair, address _KOL) public {
        pair = _pair;
        token0 = _pair.token0();
        token1 = _pair.token1();
    ...
    function fetchKOLPrice(uint256 amountIn) external returns (uint256 amountOut) {
        ...
        if (blockTimestamp - blockTimestampLast <= PERIOD) {
            amountOut = consult(KOL, amountIn);
        } else {
            update();
            amountOut = consult(KOL, amountIn);
        }

        emit AssetValue(amountOut, block.timestamp);
    }
    ...
    function fetchUSDTPrice(address token, uint256 amountIn) external returns (uint256 amountOut) {
        (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) = UniswapV2OracleLibrary
            .currentCumulativePrices(address(pair));
        if (blockTimestamp - blockTimestampLast <= PERIOD) {
            amountOut = consult(token, amountIn);
        } else {
            update();
            amountOut = consult(token, amountIn);
        }
