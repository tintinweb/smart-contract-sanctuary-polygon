...
contract LootBoxRedemer is Ownable {
    ...
    // mappping of user => collectionAddress => collectionId => amount
    mapping(address => mapping(address => mapping(uint256 => uint256)))
        public productClaimedDetails;
    ...
    function changeWhiteListStatus(address publicAddress, bool status)
        external
        onlyOwner
    {
    ...
    function _validateSigner(
        ...
        bytes32 CLAIM_PRODUCT = keccak256(
            "ClaimProduct(address collectionAddress,uint256 collectionId,uint256 claimAmount,uint256 totalBought,address erc20TokenAddress,uint256 erc20Amount,address redeemer)"
        );
    ...
    /**
     *
     * @param collectionAddress address of claiming token
     * @param collectionId collectionId of erc1155 // Currently we are giving lootbox which will be utlized to equip on nft
     * @param claimAmount claiming amount
     * @param totalBought totalAmount user bought
     * @param erc20TokenAddress tokenAddress of erc20 need to get paid for minting as nft
     * @param erc20Amount tokenAmount while converting to nft
     * @param signature  signature signed by masterAddress
     */
    function claimProduct(
        ...
        require(signer == masterAddress, "Invalid Signer");
        uint256 previouslyClaimed = productClaimedDetails[msg.sender][
            collectionAddress
        ][collectionId];
        require(
            previouslyClaimed + claimAmount <= totalBought,
            "Claim amount exceeds total bought"
        );
        if (erc20Amount > 0) {
        ...
        productClaimedDetails[msg.sender][collectionAddress][
            collectionId
        ] += claimAmount;
        _transferERC1155(collectionAddress, collectionId, claimAmount);
    }
    ...
    function updateClaimedAssets(
        ...
        productClaimedDetails[claimer][collectionAddress][
            collectionId
        ] += claimAmount;
    }
}
