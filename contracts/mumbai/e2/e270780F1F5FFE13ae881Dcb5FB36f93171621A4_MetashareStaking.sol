...
contract MetashareStaking is IMetasharesStaking, OwnableUpgradeable, ERC1155HolderUpgradeable, ReentrancyGuardUpgradeable, IXReceiver {
    ...
    function claimOnEthereum(uint256 relayerFee, uint256 returnRelayerFee) external nonReentrant whenNotPaused {
        bytes memory _callData = abi.encode(msg.sender, returnRelayerFee);
        connext.xcall{value: relayerFee}(
            1, // _destination: Domain ID of the destination chain
            ethereumStaking, // _to: address of the target contract (Pong)
    ...
    function xReceive(
        ...
        require(_originSender == ethereumStaking, "MetashareStaking: INVALID_ORIGIN");
        require(_origin == 1, "MetashareStaking: INVALID_ORIGIN");
