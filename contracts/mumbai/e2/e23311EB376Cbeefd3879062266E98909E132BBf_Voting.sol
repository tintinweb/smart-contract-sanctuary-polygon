...
contract Voting is GSNRecipient {
    ...
    function castVote(Voter calldata newVoter, bool support) external onlyWhitelisted {

        Voter memory voter = _voters[_msgSender()];
        ...
        _voters[_msgSender()] = newVoter;
        _voters[_msgSender()].index = index;
        _voters[_msgSender()].walletAddress = _msgSender();

        counterArray[index]++;
    }

    function getVoter(address wallet) external view onlyOwner returns (Voter memory) {
