...
contract BTCLDynamicLottery is AutomationCompatible, AutomatedRandomness, ReentrancyGuard {
    ...
    */
    ...
    function detectLastFullRound() public view returns (uint lastFullRound) {
        lastFullRound= round;
        for (uint i = round; i < nextRound; i++) {
            if(rounds[lastFullRound].status.totalBets == maxPlayers && rounds[lastFullRound].status.roundStatus == BTCLCore.Status.Open || rounds[lastFullRound].status.roundStatus == BTCLCore.Status.Drawing) {
                lastFullRound++;
                break;
            } else {
                lastFullRound++;
    ...
    // function detectLastFullRound() public view returns (uint lastFullRound) {
    //     lastFullRound = 0;
    //     for (uint i = 1; i < nextRound; i++) {
    //         if(rounds[i].status.totalBets == maxPlayers && rounds[i].status.roundStatus == BTCLCore.Status.Open || rounds[i].status.roundStatus == BTCLCore.Status.Drawing) {
    //             break;
    //         } else {
    //             lastFullRound = i;
    //         }
