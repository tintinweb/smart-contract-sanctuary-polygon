    ...
    UUPSUpgradeable
{
    ...
    CountersUpgradeable.Counter private _arenaIds;
    GoldFeverNativeGold ngl;
    GoldFeverItem gfi;
    address nftContract;
    uint256 private nglFromSellingHour;
    uint256 private miningSpeed;
    uint256 public arenaHourPrice;

    // mapping
    mapping(uint256 => bool) public isArenaAttacked;
    ...
    function initialize(
        ...
        __GoldFeverMiningClaim_init(admin, gfiContract_, nglContract_);
    }

    function __GoldFeverMiningClaim_init(
        address admin,
        address gfiContract_,
        address nglContract_
    ) internal onlyInitializing {
        __ReentrancyGuard_init();
        __GoldFeverMiningClaim_init_unchained(admin, gfiContract_, nglContract_);
        __UUPSUpgradeable_init();
    }

    function __GoldFeverMiningClaim_init_unchained(
        address admin,
        address gfiContract_,
        address nglContract_
    ) internal onlyInitializing {
        _setupContractId("GoldFeverMiningClaim");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
        ...
        miningSpeed = 100 * (10**decimals);
        nglFromSellingHour = 0;
    }

    event ForwarderContractUpdated(address forwarder_);
    event ArenaAttackedSet(uint256 arenaId, bool isAttacked);
    ...
    function setArenaAttacked(uint256 arenaId, bool isAttacked)
        external
        only(GAME_SERVER_ROLE)
    {
        isArenaAttacked[arenaId] = isAttacked;
        emit ArenaAttackedSet(arenaId, isAttacked);
    }
    ...
    event ArenaHourPriceUpdated(uint256 _oldPrice, uint256 _newPrice);
    event MiningSpeedUpdated(uint256 _oldMiningSpeed, uint256 _newMiningSpeed);
    ...
    function createMiningClaim(
        ...
        uint256 arenaHour,
        uint256 maxMiners
    ) external nonReentrant only(MINTER_ROLE) {
        require(
            gfi.ownerOf(miningClaimId) != address(0),
            "GoldFeverMiningClaim: Mining claim id does not exist"
        );
        require(
            idToMiningClaim[miningClaimId].status != MINING_CLAIM_CREATED,
            "GoldFeverMiningClaim: Mining claim already created"
        );
        ngl.transferFrom(_msgSender(), address(this), nglAmount);
    ...
    function supply(uint256 miningClaimId, uint256 nglAmount)
        external
        only(DEFAULT_ADMIN_ROLE)
    {
        require(
    ...
    function getArenaHour(uint256 miningClaimId)
        external
        view
        returns (uint256)
    {
    ...
    function setArenaHourPrice(uint256 _arenaHourPrice)
        ...
        only(DEFAULT_ADMIN_ROLE)
    {
        emit ArenaHourPriceUpdated(arenaHourPrice, _arenaHourPrice);
        arenaHourPrice = _arenaHourPrice;
    }

    function getArenaHourPrice() external view returns (uint256) {
        uint256 decimals = ngl.decimals();
        ...
        nonReentrant
    {
        ...
        require(
            gfi.ownerOf(miningClaimId) == msgSender,
            "GoldFeverMiningClaim: Only owner of mining claim can buy arena hour"
        );
        uint256 price = arenaHour * arenaHourPrice;
        nglFromSellingHour += price;
        ngl.transferFrom(msgSender, address(this), price);
        idToMiningClaim[miningClaimId].arenaHour += arenaHour;
    ...
    function setMiningSpeed(uint256 _miningSpeed)
        external
        only(DEFAULT_ADMIN_ROLE)
    {
        emit MiningSpeedUpdated(miningSpeed, _miningSpeed);
    ...
    function startArena(
        ...
        require(
            gfi.ownerOf(miningClaimId) == msgSender,
            "GoldFeverMiningClaim: Only owner of mining claim can start arena"
        );
        require(
            duration <= idToMiningClaim[miningClaimId].arenaHour,
            "GoldFeverMiningClaim: Arena open duration must be less than or equal to arena total hour"
        );
        require(
            duration > 0,
            "GoldFeverMiningClaim: Arena open duration must be greater than 0"
        );
        ...
        idToMiningClaim[miningClaimId].currentArenaId = arenaId;
        gfi.safeTransferFrom(msgSender, address(this), miningClaimId);
        ...
        only(DEFAULT_ADMIN_ROLE)
    {
        require(
            ...
            "GoldFeverMiningClaim: Arena is not finished"
        );
        uint256 miningClaimId = idToArena[arenaId].miningClaimId;
        address miningClaimOwner = idToArena[arenaId].owner;
        uint256 holdingFee = idToArena[arenaId].holdingFee;
        ...
        idToMiningClaim[miningClaimId].currentArenaId = 0;
        idToArena[arenaId].numMinersInArena = 0;
        ngl.transfer(miningClaimOwner, holdingFee);
        emit ArenaClosed(arenaId);
    }

    function registerAtArena(uint256 arenaId) external nonReentrant {
        require(
            idToArena[arenaId].status == ARENA_STARTED,
            "GoldFeverMiningClaim: Arena is not started"
        );
        uint256 upfrontFee = idToArena[arenaId].upfrontFee;
        address msgSender = _msgSender();
        require(
            isMinerRegistered(arenaId, msgSender) == false,
            "GoldFeverMiningClaim: Miner already registered"
        );
        idToMinerByArena[arenaId][msgSender] = Miner(
            arenaId,
            msgSender,
            MINER_REGISTERED
        );

        ngl.transferFrom(msgSender, address(this), upfrontFee);
        idToArena[arenaId].holdingFee += upfrontFee;
        emit MinerRegistered(arenaId, msgSender, MINER_REGISTERED);
    }

    function cancelArenaRegistration(uint256 arenaId) external nonReentrant {
        address msgSender = _msgSender();
        require(
            idToArena[arenaId].status != ARENA_CLOSED,
            "GoldFeverMiningClaim: Arena is closed"
        );
        require(
            idToMinerByArena[arenaId][msgSender].status == MINER_REGISTERED ||
                idToMinerByArena[arenaId][msgSender].status == MINER_LEFT,
            "GoldFeverMiningClaim: Miner not registered"
        );
        uint256 upfrontFee = idToArena[arenaId].upfrontFee;

        if (idToMinerByArena[arenaId][msgSender].status == MINER_REGISTERED) {
            ngl.transfer(msgSender, upfrontFee);
            idToArena[arenaId].holdingFee -= upfrontFee;
        }
        delete idToMinerByArena[arenaId][msgSender];
        emit MinerCanceledRegistration(arenaId, msgSender, MINER_UNREGISTERED);
    }
    ...
    {
        require(
            idToMinerByArena[arenaId][minerAddress].status ==
                MINER_REGISTERED ||
                idToMinerByArena[arenaId][minerAddress].status == MINER_LEFT,
            "GoldFeverMiningClaim: Miner not registered"
        );
        uint256 miningClaimId = idToArena[arenaId].miningClaimId;
        require(
        ...
        require(
            ...
            "GoldFeverMiningClaim: Arena is full"
        );
        uint256 upfrontFee = idToArena[arenaId].upfrontFee;
        ...
        only(GAME_SERVER_ROLE)
    {
        require(
            idToMinerByArena[arenaId][minerAddress].status == MINER_ENTERED,
            "GoldFeverMiningClaim: Miner not entered arena"
        );
        idToMinerByArena[arenaId][minerAddress].status = MINER_LEFT;
        idToArena[arenaId].numMinersInArena--;
    ...
    function bankWithdraw(
        uint256 arenaId,
        address minerAddress,
        uint256 nglWithdrawAmount,
        address govAddress,
        uint256 govTax,
        address guildOwnerAddress,
        uint256 guildFee
    ) external nonReentrant only(BANK_ROLE) {
        uint256 miningClaimId = idToArena[arenaId].miningClaimId;
        uint256 maxNglAmountCanWithdraw = idToArena[arenaId].duration *
            idToMiningClaim[miningClaimId].maxMiners *
            miningSpeed;
        require(
            nglWithdrawAmount > 0,
            "GoldFeverMiningClaim: Amount must be greater than 0"
        );
        require(
            nglWithdrawAmount <= maxNglAmountCanWithdraw,
            "GoldFeverMiningClaim: Amount must be less than or equal to max amount"
        );
        require(
            nglWithdrawAmount <= idToMiningClaim[miningClaimId].nglAmount,
            "GoldFeverMiningClaim: Amount must be less than or equal to total amount"
        );

        uint256 nglAmountAfterTax = nglWithdrawAmount -
            (nglWithdrawAmount * govTax) /
            10**uint256(feeDecimals()) /
            100;
        uint256 ownerEarn = (nglAmountAfterTax *
            idToArena[arenaId].commissionRate) /
            10**uint256(feeDecimals()) /
            100;
        ngl.transfer(
            govAddress,
            (nglWithdrawAmount * govTax) / 10**uint256(feeDecimals()) / 100
        );
        ngl.transfer(idToArena[arenaId].owner, ownerEarn);
        uint256 minerEarnBeforeFee = nglAmountAfterTax - ownerEarn;
        uint256 guildEarn = (minerEarnBeforeFee * guildFee) /
            10**uint256(feeDecimals()) /
            100;
        ngl.transfer(guildOwnerAddress, guildEarn);
        ngl.transfer(minerAddress, minerEarnBeforeFee - guildEarn);

        idToMiningClaim[miningClaimId].nglAmount -= nglWithdrawAmount;
        emit MinerWithdrawn(
            arenaId,
            miningClaimId,
            minerAddress,
    ...
    function withdrawNglFromSellingHour()
        external
        nonReentrant
        only(COMPANY_ROLE)
    {
        ngl.transfer(_msgSender(), nglFromSellingHour);
        nglFromSellingHour = 0;
    }

    function getArenaIdByMiningClaimId(uint256 miningClaimId)
        external
        view
        returns (uint256)
    {
        return idToMiningClaim[miningClaimId].currentArenaId;
    }

    function isMinerRegistered(uint256 arenaId, address minerAddress)
        public
        view
        returns (bool)
    {
