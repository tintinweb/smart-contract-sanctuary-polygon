...
contract NFT is ERC721 {
	...
	struct TokenInfo {
		uint8[PIXELS_AMOUNT][PIXELS_AMOUNT] pixels;
		uint256 tokenId;
		address owner;
	}
	...
	function getTokensInfo(uint256[] calldata tokenIds) external view returns (TokenInfo[] memory) {
		TokenInfo[] memory infos;

		for (uint256 i = 0; i < tokenIds.length; i++) {
			uint256 tokenId = tokenIds[i];
			infos[i] = (TokenInfo(_pixelsOf[tokenId], tokenId, ownerOf(tokenId)));
		}
