...
abstract contract ERC20PausableCrowdsale is ERC20Crowdsale {

    bool private _paused;

    modifier whenPaused {
        require(_paused, "ERC20PausableCrowdsale: not paused");
        _;
    }

    modifier whenNotPaused {
        require(!_paused, "ERC20PausableCrowdsale: paused");
        _;
    }

    event Paused(address indexed pauser);
    event Resumed(address indexed resumer);
...
abstract contract ERC20PerTransactionCappedCrowdsale is ERC20Crowdsale {

    uint256 private immutable _perTransactionCap;

    constructor(uint256 perTransactionCap_) {
        require(perTransactionCap_ > 0, "ERC20PerTransactionCappedCrowdsale: per-transaction cap must be positive");

        _perTransactionCap = perTransactionCap_;
    }

    function perTransactionCap() public view returns (uint256) {
        return _perTransactionCap;
    }

    function _preValidatePurchase(address beneficiary, uint256 weiAmount, uint256 nativeToQuotingTokenRate) internal view virtual override {
        uint256 weiInQuotingTokens = _getWeiRaisedInQuotingTokens(weiAmount, nativeToQuotingTokenRate);
        require(weiInQuotingTokens <= _perTransactionCap, "ERC20PerTransactionCappedCrowdsale: per-transaction cap exceeded");
...
import "@opengtl/crowdsale/contracts/ERC20/ERC20PausableCrowdsale.sol";
import "@opengtl/crowdsale/contracts/ERC20/ERC20PerTransactionCappedCrowdsale.sol";
import "../token/ERC20Mintable.sol";
import "../vault/MultiVestingVault.sol";

/**
 * @title PrimeGamesTokenCrowdsale
 * 
 * @dev The PGT Token crowdsale contract. The crowdsale is finalizable and time-bound, it also has a hardcap and minimum threshold.
 * Both hardcap and minimum threshold is quoted in USDC.
 *
 * The crowdsale receives MATIC.
 * 
 * This crowdsale uses the dynamically issued tokens so that:
 * 1. A token being sold should implement the ERC20Mintable interface.
 * 2. The crowdsale contract has to have access to token minting (Token owner, ROLE_MINTER, etc)
 * 
 * After purchase, 10% of purchased tokens are transferred to the buyer and 90% are locked into the vesting vault.
 * They will be released after a certain amount of time.
 */
contract PrimeGamesTokenCrowdsale is TimedERC20Crowdsale, ERC20CappedCrowdsale, ERC20MinimumThresholdCrowdsale,
    ERC20PausableCrowdsale, ERC20PerTransactionCappedCrowdsale {
    ...
    /**
     * @dev Is crowdsale finished?
     */
    bool private _finished = false;

    /**
     * @dev Execute the code if crowdsale is not finished
     */
    ...
    /**
     * @dev Crowdsale Finished event
     * @param timestamp when a crowdsale was finished
     * @param finisher who finished a crowdsale
     */
    ...
    constructor(uint256 rate, address payable wallet, ERC20Mintable token_, PriceFeedClient priceFeedClient,
                ...
                uint256 perTransactionCap_) 
                TimedERC20Crowdsale(openingTime, type(uint256).max) ERC20Crowdsale(rate, wallet, token_, priceFeedClient)
                ERC20CappedCrowdsale(cap, delta) ERC20MinimumThresholdCrowdsale(minimumThreshold_) ERC20PausableCrowdsale()
                ERC20PerTransactionCappedCrowdsale(perTransactionCap_) {
        _vault = vault_;
    }
    ...
    /**
     * @dev Can the finish method be called
     */
    ...
    /**
     * @dev Finish crowdsale and clean up. Can be called only once and only by owner.
     * This is the place where a crowdsale should remove all ownerships / roles it has.
     */
    function finish() public onlyOwner crowdsaleNotFinished {
        require(canFinish(), "PrimeGamesTokenCrowdsale: crowdsale finish conditions are not met");

        _finished = true;
        ERC20Mintable(address(token())).renounceMinter();
        _vault.renounceOwnership();
    ...
    function _deliverTokens(address beneficiary, uint256 tokenAmount) internal virtual override {
        uint256 tokensForUser = tokenAmount / 10;
    ...
    function _preValidatePurchase(address beneficiary, uint256 weiAmount, uint256 nativeToQuotingTokenRate)
        internal virtual override(TimedERC20Crowdsale, ERC20CappedCrowdsale, ERC20MinimumThresholdCrowdsale,
            ERC20PausableCrowdsale, ERC20PerTransactionCappedCrowdsale) crowdsaleNotFinished view {
        super._preValidatePurchase(beneficiary, weiAmount, nativeToQuotingTokenRate);
    }
