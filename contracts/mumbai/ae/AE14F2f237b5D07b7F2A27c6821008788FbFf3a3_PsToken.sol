    ...
    ReentrancyGuard
{
    ...
    uint256 public constant maxPerMint = 4;
    ...
    constructor(
        ...
        baseURI = _uri;
        //Set counter to start with 1
        _tokenIds.increment();
        //Add owner to whitelist
        whitelist[msg.sender] = true;
    }

    function mintItems(uint256 _count) external nonReentrant{
        uint256 totalMinted = totalSupply();

        require(mintable, "Sale has not started yet");
        require(!isSoldOut, "This collection is sold out!");
        require(whitelist[msg.sender], "You are not in the whitelist.");
        require(totalMinted.add(_count) <= maxSupply, "Your number of minting is exceeded the max supply");
        require(_count >0 && _count <= maxPerMint, "You have received the maximum amount of NFTs allowed.");
    ...
    function _mintItem() private {
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        _checkSoldOut(newItemId);
        _tokenIds.increment();
    }

    function _checkSoldOut(uint256 _tokenId) private {
    ...
    function isAlreadySoldOut() external view returns(bool) {
        return isSoldOut;
    }

    function checkWhitelist(address _addr) external view returns(bool) {
        bool userIsWhitelisted = whitelist[_addr];
