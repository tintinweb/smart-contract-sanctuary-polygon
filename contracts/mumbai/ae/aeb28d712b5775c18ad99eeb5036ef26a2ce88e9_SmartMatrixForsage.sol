...
constructor(address ownerAddress, address secondOwnerAddress, address thirdOwnerAddress, address forthOwnerAddress) public {
    ...
    // Adding forth owner
    User memory forthUser = User({
        id: 4,
        referrer: secondOwnerAddress,
        partnersCount: uint(0)
    });
    users[forthOwnerAddress] = forthUser;
    idToAddress[4] = forthOwnerAddress;

    for (uint8 i = 1; i <= 5; i++) {
        users[forthOwnerAddress].activeLevels[i] = true;
    }

    userIds[4] = forthOwnerAddress;
}
...
function buyNewLevel(uint8 level) external {
    ...
    require(daiCoin.balanceOf(msg.sender) <= levelPrice[level], "not enough Balance");
    ...
    function registration(address userAddress, address referrerAddress) private {
        require(msg.value == 1 ether, "registration cost 0.05");
