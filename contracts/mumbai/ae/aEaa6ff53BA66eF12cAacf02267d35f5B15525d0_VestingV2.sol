...
contract VestingV2 is IVestingV2, OwnableUpgradeable, DateTime {
    struct Info {
        bool isPartOfOtherStrats;
        uint256 totalClaimable;
        uint256 totalClaimed;
        uint256 totalVested;
        uint256 lowestStartTime;
        uint256 highestEndTime;
    }
    ...
    function claimDistribution(uint256 _option, address _wallet) public returns (bool) {
        ...
        if (releaseAmount > 0) {
            ...
            IERC20Upgradeable(token).safeTransfer(_wallet, releaseAmount);
    ...
    function claimAll() external {
        uint256 len = vestingPools.length;
        for (uint256 i = 0; i < len; i++) {
            if (vestingPools[i].hasWhitelist[msg.sender].active) {
                claimDistribution(i, msg.sender);
            }
        }
    }
    ...
    function getInfo(address _user) public view returns (Info memory) {
        uint256 len = vestingPools.length;
        Info memory info;
        info.lowestStartTime = vestingPools[0].start;
        info.highestEndTime = vestingPools[0].cliff + vestingPools[0].duration;
        for (uint256 i = 0; i < len; i++) {
            if (vestingPools[i].hasWhitelist[_user].active) {
                if (i > 0) info.isPartOfOtherStrats = true;
                info.lowestStartTime = vestingPools[i].start < info.lowestStartTime
                    ? vestingPools[i].start
                    : info.lowestStartTime;
                info.highestEndTime = vestingPools[i].cliff + vestingPools[i].duration > info.highestEndTime
                    ? vestingPools[i].cliff + vestingPools[i].duration
                    : info.highestEndTime;
                info.totalClaimable += calculateReleasableAmount(i, _user);
                info.totalClaimed += vestingPools[i]
                    .whitelistPool[vestingPools[i].hasWhitelist[_user].arrIdx]
                    .distributedAmount;
                info.totalVested += vestingPools[i].whitelistPool[vestingPools[i].hasWhitelist[_user].arrIdx].amount;
            }
        }
