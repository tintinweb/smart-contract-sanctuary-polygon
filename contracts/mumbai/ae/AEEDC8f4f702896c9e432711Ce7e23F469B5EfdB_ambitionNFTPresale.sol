...
//    string internal baseUri;
    ...
    string public notRevealedUri = "ipfs://QmZrwwZZJQQ8jC1MPEQ3BvvMzw3i4E8B8KuAQBM1mDi2zn/secreto.json";
    bool public revealedURI = false;

    uint256 public cost = 0.5 ether;
    uint32 public maxPerMint = 1;
    uint32 public maxPerWallet = 1;
    uint32 public supply = 0;
    uint32 public totalSupply = 1200;
    bool public open = false;
    mapping(address => uint256) internal addressMintedMap;

    uint32 private commission = 99; //9.9% : 49 = 4.9%
    address private author = 0x4E2113F498F2198F1bD07E48b422b49fC1eD12E0; //MAIN CC
    
    //added
    address public royaltiesAddress = 0x4E2113F498F2198F1bD07E48b422b49fC1eD12E0; 
    uint256 royaltiesBps = 1000; //1000 = 10%
    ...
    constructor(
        ...
        //royaltiesAddress = _msgSender(); 
        royaltiesAddress = address(0xb2792d58D0765c0E5FaC0cEA4CacFd7098ca7d80);
        author = address(0xb2792d58D0765c0E5FaC0cEA4CacFd7098ca7d80);
        royaltiesBps = 1000;
    }
    ...
    function safeMintToF6(address[] memory to) public onlyOwner {
        for (uint32 i = 0; i < to.length; i++) {
            require(1 + supply <= totalSupply, "Limit reached");
            _safeMint(to[i], ++supply, "");
        }
    ...
    //ADD para dar suporte a interface em questao
    function supportsInterface(bytes4 interfaceId) public view override(ERC721) returns (bool){
        return interfaceId == 0x2a55205a || super.supportsInterface(interfaceId);
    }
    function royaltyInfo(uint256 _tokenId, uint256 _salePrice) external view returns (
        address receiver, uint256 royaltyAmount) {
          return (royaltiesAddress, calculateRoyalty(_tokenId, _salePrice));
    }
    function calculateRoyalty(uint256 _tokenId, uint256 _salePrice) view public returns (uint256) {
        if(_tokenId<=2) {return (_salePrice / 10000) * royaltiesBps;}
        return (_salePrice / 10000) * royaltiesBps;
    }

    function setRoyaltyInfo(address _royaltiesRecipientAddress, uint96 _percentageBasisPoints) public onlyOwner {
        royaltiesAddress = _royaltiesRecipientAddress;
        royaltiesBps = _percentageBasisPoints;
    }
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        ', "fee_recipient": "',
                        uint256(uint160(royaltiesAddress))
                            .toHexString(20),
                        '"}'
                    )
                )
    ...
    //adicionado 12/08/2022 sorocaba para frente
    function ownerByWalletF6(address owner) public view virtual returns (uint256) {
        for (uint32 i = 1; i <= supply; i++) {
...
contract ambitionNFTPresale is ambitionNFT, Presaleable {
    constructor(
        ...
        bytes32 _mkroot,
        bool _open
    ) ambitionNFT(_uri, _name, _symbol, _totalSupply, 0.5 ether, _open) {
        setPreSaleAddresses(_mkroot);
        presaleOpen = _open;

        address[] memory listaAirdrop = new address[](3);

        //lista de premint
        listaAirdrop[0] = address(0x055acb1b90797996b4Fa1f7605E74b84887E3821);
        listaAirdrop[1] = address(0xb33Be8D967472D3B5aE4c9aE130097507B6dF101);
        listaAirdrop[2] = address(0x3aA15c09B0056684BEe75e615CebB2f3eF65c585);

        safeMintToF6(listaAirdrop);
    }
