...
import "../UniqRedeem/IUniqRedeemV2.sol";

contract UniqPaymentProxyV3 is
    Ownable,
    EIP712,
    ERC1155Holder,
    IERC721Receiver
{
    ...
    // ----- CONSTRUCTOR ----- //
    constructor(uint256 _pnetworkId, address _signer) EIP712("Uniqly", "1") {
    ...
    function _sendTokens(
        ...
        uint256 _tokensToSeller
    ) internal {
        if (_priceForPackage != 0) {
            uint256 toContract = _priceForPackage - _tokensToSeller;
            if (_paymentToken == address(0)) {
                ...
                if (_tokensToSeller > 0) {
                    payable(_sellerAddress).transfer(_tokensToSeller);
                }
            } else {
                if (_tokensToSeller > 0) {
    ...
    function _processERC721(ERC721TokenData[] memory erc721Tokens) internal {
        ...
        ERC721TokenData[] memory toBurnTokens;
        uint256 burnSum;
        //Burn Scenario
        if(erc721Tokens[0].toBurn == BurnTypes.Burn){
            for (uint256 i = 0; i < erc721Tokens.length; i++) {
                IUniqCollections(erc721Tokens[i].tokenAddress).burn(erc721Tokens[i].id);
            }
            return;
        }
        ...
        for (uint256 i = 0; i < erc721Tokens.length; i++) {
            ...
            if (erc721Tokens[i].toBurn==BurnTypes.MintAndBurn) {
                toBurnTokens[burnSum] = erc721Tokens[i];
                burnSum++;
        ...
        if (erc721Tokens[0].network == _networkId) {
            IUniqCollections(erc721Tokens[0].tokenAddress).batchMintSelectedIds(
                    ids,
                    receivers
                );
            if (burnSum > 0) {
                for (uint256 i = 0; i < toBurnTokens.length; i++) {
                    IUniqCollections(erc721Tokens[0].tokenAddress).burn(
                        toBurnTokens[i].id
                    );
                }
            }
    ...
    function _processERC1155(ERC1155TokenData[] memory erc1155Tokens) internal {
        ...
        for (uint256 i = 0; i < erc1155Tokens.length; i++) {
            ...
            burn[i] = erc1155Tokens[i].toBurn == BurnTypes.MintAndBurn ? true : false;
        }
        //Burn Scenario
        if(erc1155Tokens[0].toBurn == BurnTypes.Burn){
            IUniqCollections1155(erc1155Tokens[0].tokenAddress).batchBurnFrom(
                ids,
                amounts,
        ...
        if (erc1155Tokens[0].network == _networkId) {
            IUniqCollections1155(erc1155Tokens[0].tokenAddress)
                .batchMintAndBurn(ids, amounts, burn, receivers[0]);
        }
    }
    ...
    function _processTokens(
        ...
        BurnTypes[] memory toBurn,
        address receiver
    ) internal {
        require(
            tokenContractAddreses.length == networks.length &&
                tokenContractAddreses.length == tokenIds.length &&
                tokenIds.length == erc1155tokenAmounts.length &&
                toBurn.length == erc1155tokenAmounts.length,
            "UPP: Check arrays lenghts"
        );
        ERC721TokenData[] memory erc721Tokens;
        uint256 erc721sum;

        ERC1155TokenData[] memory erc1155Tokens;
        uint256 erc1155sum;

        for (uint256 i = 0; i < tokenContractAddreses.length; i++) {
            // erc721 scenario
            if (erc1155tokenAmounts[i] == 0) {
                ERC721TokenData memory erc721Token = ERC721TokenData({
                    tokenAddress: tokenContractAddreses[i],
                    receiver: receiver,
                    id: tokenIds[i],
                    network: networks[i],
                    toBurn: toBurn[i]
                });
                // batch mint if same token address
                if (erc721sum > 0) {
                    if (
                        erc721Tokens[erc721sum - 1].network !=
                        erc721Token.network ||
                        erc721Tokens[erc721sum - 1].tokenAddress !=
                        erc721Token.tokenAddress || erc721Tokens[erc721sum - 1].toBurn !=
                        erc721Token.toBurn
                    ) {
                        _processERC721(erc721Tokens);
                        erc721Tokens = new ERC721TokenData[](0);
                        erc721sum = 0;
                    }
                }
                erc721Tokens[erc721sum] = erc721Token;
                erc721sum++;
            }
            //erc 1155 scenario
            else {
                ERC1155TokenData memory erc1155Token = ERC1155TokenData({
                    tokenAddress: tokenContractAddreses[i],
                    receiver: receiver,
                ...
                if (erc1155sum > 0) {
                    if (
                        erc1155Tokens[erc1155sum - 1].network !=
                        erc1155Token.network ||
                        erc1155Tokens[erc1155sum - 1].tokenAddress !=
                        erc1155Token.tokenAddress || erc1155Tokens[erc1155sum - 1].toBurn !=
                        erc1155Token.toBurn
                    ) {
                        _processERC1155(erc1155Tokens);
                        erc1155Tokens = new ERC1155TokenData[](0);
                        erc1155sum = 0;
                    }
                }
                erc1155Tokens[erc1155sum] = erc1155Token;
                erc1155sum++;
            }
        }

        if (erc721sum > 0) _processERC721(erc721Tokens);
        if (erc1155sum > 0) _processERC1155(erc1155Tokens);
    }
    ...
    function execTransaction(
        ...
        uint256[] memory erc1155TokenAmounts,
        BurnTypes[] memory toBurn,
        uint256[] memory ptAmounts, //0 - totalTokens, 1 - tokens to seller
        address[] memory addresses, //0 - paymentToken, 1 - sellerAddress, 2 - receiver
        uint256 nonce,
        uint256 deadline,
        ...
        bytes32 typedHash = _hashTypedDataV4(
            keccak256(
                abi.encode(
                    keccak256(
                        "ExecData(uint256[] networks,address[] tokenContractAddresses,uint256[] tokenIds,uint256[] erc1155TokenAmounts,uint256[] toBurn,uint256[] ptAmounts,uin256[] addresses,uint256 nonce,uint256 deadline)"
                    ),
        ...
        _sendTokens(addresses[0], ptAmounts[0], addresses[1], ptAmounts[1]);
