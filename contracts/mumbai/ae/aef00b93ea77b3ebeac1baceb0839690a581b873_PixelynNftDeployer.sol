    ...
    IERC4906
{
    ...
    /**
     * @dev update metadata for a token id and emit an event
     * this allows marketplaces to reindex it
     * @param _tokenId The token id to update metadata for
     */
    function metadataUpdate(uint256 _tokenId) external isAirdropper {
        emit MetadataUpdate(_tokenId);
    }

    /**
     * @dev update metadata for a range of token ids and emit an event
     * this allows marketplaces to reindex it
     * @param _fromTokenId The token id to start updating metadata for
     * @param _toTokenId The token id to stop updating metadata for
     */
    function batchMetadataUpdate(uint256 _fromTokenId, uint256 _toTokenId) external isAirdropper {
    ...
    /**
     * @dev update metadata for all token ids and emit an event
     * this allows marketplaces to reindex it
     * we use the full range of max mint supply as it random mints on-chain
     */
    function refreshAllMetadata() external isAirdropper {
        _refreshAllMetadata();
    }
     ...
     */
    function reserve(address to, uint256 index) external isPixelynAdmin {
     ...
     *
     */
    function mintRandom(address to, uint256 numToMint) external payable nonReentrant {
    ...
    /**
     * @dev mints an NFT to an address - used for marketing assets and giveaways.
     * This still uses mint random to keep it fair.
     *
     * Requirements:
     *
     ...
     *
     */
    function mintFor(address to, uint256 numToMint) external isAirdropper {
        require(numToMint > 0, 'mint at least one token');
        require(_unusedTokensLeft >= numToMint, 'minting more tokens than supply');
...
contract PixelynNftDeployer is IPixelynNftDeployer, Context, Ownable {
     ...
     */
    modifier isAdmin() {
        require(_admins[_msgSender()], 'not a admin');
        _;
    }
