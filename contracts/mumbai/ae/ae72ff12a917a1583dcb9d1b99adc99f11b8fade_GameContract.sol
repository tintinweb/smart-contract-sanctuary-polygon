...
//  Admin puts the players in the game. 
    function enterGame(address player1, address player2) public onlyOwner{
        if(palyersInAMatch()==2){
            game_state = GAME_STATE.CLOSED;
            revert MaximumPlayersLimitReached();
        }
        require(game_state == GAME_STATE.OPEN, "There's an ongoing game now!");
        require(getWneoBalance(player1) >= fee, "Not enough WNEO in player1 account!");
        require(getWneoBalance(player2) >= fee, "Not enough WNEO in player2 account!");
        players.push(payable(player1));
        players.push(payable(player2));
        IERC20(wneoAddress).transferFrom(player1,address(this),fee);
        IERC20(wneoAddress).transferFrom(player2,address(this),fee);
    }
...
//  Function called by expireGame to draw the match if conditions are met.
    function drawGame() internal {
        ...
        game_state = GAME_STATE.CALCULATING_WINNER;
        game_state= GAME_STATE.CLOSED;
        IERC20(wneoAddress).transfer(players[0],fee);
        IERC20(wneoAddress).transfer(players[1],fee);
        emit GameDrawn();
    }
...
//  Callable by admin, updates the fee if needed.
