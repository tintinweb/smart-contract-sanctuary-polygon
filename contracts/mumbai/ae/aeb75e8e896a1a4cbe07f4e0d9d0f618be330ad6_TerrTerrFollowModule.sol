...
import {ModuleBase} from "../lib/core/contracts/core/modules/ModuleBase.sol";
import {FollowValidatorFollowModuleBase} from "../lib/core/contracts/core/modules/follow/FollowValidatorFollowModuleBase.sol";
import {IFollowModule} from "../lib/core/contracts/interfaces/IFollowModule.sol";
import {Errors} from "./libs/Errors.sol";
import "./libs/Structs.sol";
import "@openzeppelin/contracts/token/ERC721/IERC721.sol";

interface ITerrTerrMap {
    function getUserLocation(address account) external view returns (uint256);
}

/**
 * @title TerrTerrFollowModule
 *
 * ████████╗███████╗██████╗ ██████╗     ████████╗███████╗██████╗ ██████╗
 * ╚══██╔══╝██╔════╝██╔══██╗██╔══██╗    ╚══██╔══╝██╔════╝██╔══██╗██╔══██╗
 *    ██║   █████╗  ██████╔╝██████╔╝       ██║   █████╗  ██████╔╝██████╔╝
 ...
 * their location on the TerrTerr map.
 */
contract TerrTerrFollowModule is
    IFollowModule,
    FollowValidatorFollowModuleBase
{
    mapping(uint256 => TerrTerrMapFollowConfig) public mapFollowByProfile;
        ...
        returns (bytes memory)
    {
        ...
        mapFollowByProfile[profileId] = TerrTerrMapFollowConfig(
            mapAddress,
            followingAddress
        );
    ...
    function updateMap(uint256 profileId, address _mapAddress) external {
        if (IERC721(HUB).ownerOf(profileId) != msg.sender)
            revert Errors.notProfileOwner();

        mapFollowByProfile[profileId].mapAddress = _mapAddress;
    }
    ...
    function _checkProximity(address _user, uint256 _profileId) private view {
        if (mapFollowByProfile[_profileId].mapAddress != address(0)) {
            if (
                ITerrTerrMap(mapFollowByProfile[_profileId].mapAddress)
                    .getUserLocation(_user) !=
                ITerrTerrMap(mapFollowByProfile[_profileId].mapAddress)
                    .getUserLocation(
