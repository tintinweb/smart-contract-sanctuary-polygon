 ...
 */
contract GenericERC20 is Context, IERC20 {
...
// The contract of "Staked RUBY", "xRUBY"
contract StakedRUBY is GenericERC20("Staked RUBY", "xRUBY") {
    using SafeMath for uint256;
    IERC20 public ruby;
    ...
    function enter(uint256 _amount) public {
        ...
        // Calculate and mint the amount of xRUBY the RUBY is worth. The ratio will change overtime, as xRUBY is burned/minted and RUBY deposited.
        else {
    ...
    // returns the total amount of RUBY an address has in the contract including fees earned
    function RUBYBalance(address _account) external view returns (uint256 rubyAmount_) {
        uint256 xRUBYAmount = balanceOf(_account);
        uint256 totalxRUBY = totalSupply();
        rubyAmount_ = xRUBYAmount.mul(ruby.balanceOf(address(this))).div(totalxRUBY);
    }

    // returns how much RUBY someone gets for redeeming xRUBY
    function xRUBYForRUBY(uint256 _xRUBYAmount) external view returns (uint256 rubyAmount_) {
        uint256 totalxRUBY = totalSupply();
        rubyAmount_ = _xRUBYAmount.mul(ruby.balanceOf(address(this))).div(totalxRUBY);
    }

    // returns how much xRUBY someone gets for depositing RUBY
    function RUBYForxRUBY(uint256 _rubyAmount) external view returns (uint256 xRUBYAmount_) {
        uint256 totalRUBY = ruby.balanceOf(address(this));
        uint256 totalxRUBY = totalSupply();
        if (totalxRUBY == 0 || totalRUBY == 0) {
            xRUBYAmount_ = _rubyAmount;
        }
        else {
            xRUBYAmount_ = _rubyAmount.mul(totalxRUBY).div(totalRUBY);
        }
    }
    ...
    */
    ...
    function delegateBySig(address delegatee, uint nonce, uint expiry, uint8 v, bytes32 r, bytes32 s ) external {
        ...
        require(block.timestamp <= expiry, "RUBY::delegateBySig: signature expired");
