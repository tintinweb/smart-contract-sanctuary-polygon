...
contract limitOrderAndNormalSwapToken_Admin is OpsTaskCreator, Ownable {
    ...
    function setFeeRates(uint256 _feeRate, uint256 _feeAllRate) public onlyOwner {
        feeRate = _feeRate;
    ...
    function deposit(address _token, uint256 _tokenAmount, uint256 _devAmount) external payable {
        ...
        if (_devAmount > 0 && address(devToken) != address(0) && devFee > 0) {
            devToken.transferFrom(_user, address(this), _devAmount);
    ...
    function withdraw(address _token, uint256 _tokenAmount, uint256 _ethAmount, uint256 _devAmount) external {
        ...
        if (_devAmount > 0 && address(devToken) != address(0) && devFee > 0) {
            devToken.transfer(_user, _devAmount);
    ...
    function withdrawAll() external {
        require(userInfoList[msg.sender].usdtAmount > 0 || userInfoList[msg.sender].ethAmount > 0 || userInfoList[msg.sender].devAmount > 0, "A12");
