...
contract ItemTransfer is IItemTransfer, GameBase {
    ...
    function transfer(uint256 worldId, uint256 tokenId, uint256 targetTokenId, uint256 itemId, int64 amount) external virtual override {
        Character from = getCharacter(worldId, tokenId);
        Character to = getCharacter(worldId, targetTokenId);

        require(address(from) != address(0), "wrong tokenId");
        require(address(to) != address(0), "wrong targetTokenId");
        require(amount > 0, "wrong amount");
        require(this.canTransfer(worldId, tokenId, targetTokenId, itemId, amount), "Cannot transfer");

        from.addItem(itemId, amount * -1);
        to.addItem(itemId, amount);

        emit TransferItem(worldId, tokenId, targetTokenId, itemId, amount);
    }

    function canTransfer(uint256 worldId, uint256 tokenId, uint256 targetTokenId, uint256 itemId, int64 amount) external view virtual override returns(bool) {
        Character from = getCharacter(worldId, tokenId);
        Character to = getCharacter(worldId, targetTokenId);
        ...
        itemIds[0] = itemId;
        int64[] memory items = from.getItems(itemIds);
