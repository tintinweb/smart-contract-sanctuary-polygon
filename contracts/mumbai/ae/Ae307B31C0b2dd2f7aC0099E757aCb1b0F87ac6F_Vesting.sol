...
contract Vesting is Roles {
    ...
    function getPreviousBlock(
        ...
        for (uint256 i = _phaseId - 1; i >= 0; i--) {
            if (phaseStorage[i].isActive) {
                return (true, phaseStorage[i].toBlock);
            }
        }
        return (false, 0);
    }

    function getLaterBlock(
        uint256 _phaseId
    ) public view returns (bool, uint256) {
    ...
    function addNewPhase(
        ...
        require(toBlock > block.number, "start block not correct");

        (bool hasPre, uint256 blockNumber) = getPreviousBlock(
            phaseStorage.length
        );
        if (hasPre) {
            require(
        ...
        emit AddNewPhase(phaseStorage.length, phaseName, toBlock, amountToken);
    ...
    function deletePhase(uint256 _phaseId) public onlyRole(GUARDIAN_ROLE) {
        ...
        require(
            currentPhase.toBlock > block.number,
    ...
    function editPhase(
        ...
        require(_toBlock > block.number, "start block not correct");
        ...
        if (hasPre) {
            require(_toBlock > blockNumber, "Distributed time not correct");
        }

        (bool hasLater, uint256 blockNumber2) = getLaterBlock(
            phaseStorage.length
        );

        if (hasLater) {
            require(_toBlock < blockNumber2, "Distributed time not correct");
        }
        ...
        emit EditPhase(_phaseId, _name, _toBlock, _amountToken);
        remainingReward += currentPhase.amountToken;
        remainingReward -= _amountToken;

        currentPhase.phaseName = _name;
        currentPhase.toBlock = _toBlock;
    ...
    function pushMerkleTree(
        ...
        require(block.number > phaseStorage[phaseId].toBlock, "Not end yet");
