...
contract MarketTieredLeverage is AccessControlledAndUpgradeable {
  ...
  */
  ...
  function _rebalanceMarketPoolsCalculatePriceAndExecuteBatches(
    ...
    for (uint8 poolType = uint8(PoolType.SHORT); poolType <= uint8(PoolType.LONG); poolType++) {
      ...
      for (uint8 poolIndex = 0; poolIndex < maxPoolIndex; poolIndex++) {
        ...
        syntheticToken_priceSnapshot[epochToExecute.associatedEpochIndex][PoolType(poolType)][
          poolIndex
        ] = price;
    ...
    // TODO: could save gas here by only updating this storage once at the end.
  ...
  function _updateSystemStateInternal() internal virtual {
    ...
    // TODO : the following two value updates should be scrutinized and updated correctly (bugs!) 
    ...
    for (
      ...
      // TODO if on the 3rd or more iteration of this loop, don't need to execute below, because pending actions will only exist for first 2 missed epochs.
      // Other pending actions would have triggered upkeep
