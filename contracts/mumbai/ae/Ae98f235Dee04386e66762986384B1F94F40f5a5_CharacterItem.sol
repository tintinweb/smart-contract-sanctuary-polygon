...
contract CharacterItem is ICharacterItem, GameBase {
    ...
    function addItem(uint256 worldId, uint256 tokenId, uint256 itemDefinitionId, int64 value) external virtual override {
        World world = getWorld(worldId);
        Character character = getCharacter(worldId, tokenId);
        ItemDefinition itemDefinition = world.itemDefinition();

        require(itemDefinition.itemTypes(itemDefinitionId) != ItemDefinition.ItemType.Invalid, "wrong item definition id");

        character.addItem(itemDefinitionId, value);

        emit AddItem(worldId, tokenId, itemDefinitionId);
    }
    ...
    function getItems(uint256 worldId, uint256 tokenId, uint256[] memory itemDefinitionIds) external view virtual override returns(int64[] memory) {
        ...
        return character.getItems(itemDefinitionIds);
    }
}
