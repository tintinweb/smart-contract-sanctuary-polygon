...
contract VoteOlive is Initializable, IVoteOlive, Ownable, Pausable {
    ...
    //                 placement.percent = (placement.amount * 1e4) / maxAvailableVotes;

    //                 //Update user aggregation
    //                 userVoteItems[account][placement.reactorKey] = placement.amount;
    //                 userVotePercent[account][placement.reactorKey] = placement.percent;
    ...
    //             totalUsedPercent: (total * 1e4) / maxAvailableVotes
    //         });
    ...
    // System aggregation will be 0 always because not updated anywhere (since using percent instead of amount)
    ...
    function getMaxVoteBalance(address account) public view override returns (uint256) {
        address[] memory votingTokens = new address[](1);
        votingTokens[0] = veolive;
    ...
    function setVeOlive(address _veolive) external override onlyOwner {
        require(_veolive != address(0), "Cannot be zero address");
        require(veolive == address(0), "Already Initialized");
        veolive = _veolive;
    }
    ...
    function _vote(UserVotePayload memory userVotePayload) internal whenNotPaused {
        ...
        for (uint256 i = 0; i < userVotePayload.allocations.length; i++) {
            ...
            if (userVotePercent[account][reactorKey] > 0) {
                ...
                if (currentPercent > percent) {
                    // systemAggregations[reactorKey] = systemAggregations[reactorKey] - (currentPercent - percent);
                    totalUsedPercent = totalUsedPercent - (currentPercent - percent);
                } else if (currentPercent < percent) {
                    // systemAggregations[reactorKey] = systemAggregations[reactorKey] + (percent - currentPercent);
        ...
        require(totalUsedPercent <= 10000, "Max 100%");
    ...
    function _getVotingPower(TokenBalance[] memory balances) private pure returns (uint256 votes) {
        for (uint256 i = 0; i < balances.length; i++) {
            votes = votes + ((balances[i].amount) / ONE_WITH_EIGHTEEN_PRECISION);
        }
    }
}