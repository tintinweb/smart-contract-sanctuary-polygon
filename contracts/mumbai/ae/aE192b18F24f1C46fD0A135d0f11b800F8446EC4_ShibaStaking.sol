...
contract ShibaStaking is Ownable, ReentrancyGuard {
    ...
    mapping(address => uint) public claimInfo;
    ...
    function withdrawCapital(uint _amount) external nonReentrant {
        ...
        depositInfo[msg.sender].totalDeposit -= withdrawAmount;
    ...
    function getClaimableAmount(address _investor, uint256 nftBalance) public view returns (uint){
        if (depositInfo[_investor].amount > 0) {
            ...
            if (nftBalance > 0) {
                uint256 claimableAmount = (RoiTime * depositInfo[_investor].amount * apr1).div(percentRate * rewardPeriod);

                return claimableAmount;
            } else {
                uint256 claimableAmount = (RoiTime * depositInfo[_investor].amount * apr0).div(percentRate * rewardPeriod);
