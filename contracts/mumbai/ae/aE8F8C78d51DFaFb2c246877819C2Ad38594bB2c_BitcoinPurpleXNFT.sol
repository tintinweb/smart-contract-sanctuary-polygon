...
contract BitcoinPurpleXNFT is ERC721, Ownable {
    using Strings for uint256;
    ...
    constructor() ERC721("BitcoinPurple", "XNFTs") {
        maxSupply = 20000;
    ...
    function convertBitcoinToMatic() public view returns (uint256) {
        (, int256 btcPrice, , , ) = BTCpriceFeed.latestRoundData();
        (, int256 maticPrice, , , ) = MATICpriceFeed.latestRoundData();
        uint256 btcValue;
        uint256 bitcoinPurpleValue;
        uint256 bitcoinPurpleValueInMATIC;
        uint256 maticValue;
        uint256 finalMaticValue;
        // Chainlink provides price data with 8 decimals
        uint256 btcDecimals = 10**uint256(BTCpriceFeed.decimals());
        uint256 maticDecimals = 10**uint256(MATICpriceFeed.decimals());

        btcValue = uint256(btcPrice) / btcDecimals; // BTC value in USD
        bitcoinPurpleValue = btcValue / 2; // 1 Bitcoin Purple value in USD
        maticValue = ((10**18) * uint256(maticPrice)) / maticDecimals; // ETH value in USD
        finalMaticValue = (((10**18) * btcValue) / maticValue) * (1 ether);
        bitcoinPurpleValueInMATIC = (finalMaticValue / 10**18) / 2; // 1 Bitcoin Purple value in MATIC
        return bitcoinPurpleValueInMATIC;
    }
}