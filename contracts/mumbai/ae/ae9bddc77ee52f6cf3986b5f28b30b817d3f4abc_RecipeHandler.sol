...
contract RecipeHandler is Ownable, StudioSignature {
  ...
  event ApplyArtifactSuccess(address indexed account, uint256[] artifactIds, uint256[] amounts);
  ...
  function openChest(RecipeSignature calldata packet) external {
    ...
    emit ChestOpened(msg.sender, hunterRarities, outHunters, outSizes[0], outSizes[1], outSizes[2]);
  ...
  function applyArtifact(RecipeSignature calldata packet) external {
    ...
    (uint256 hunterId, uint256[] memory artifactIds, uint256[] memory amounts) =
      abi.decode(packet.payload, (uint256, uint256[], uint256[]));

    emit ApplyArtifactSuccess(msg.sender, artifactIds, amounts);

    require(hunters.ownerOf(hunterId) == msg.sender, "Not the owner of hunter");

    // OUT - Burn artifacts
    perks.safeBatchTransferFrom(msg.sender, address(0x0), artifactIds, amounts, "");
  }
}
