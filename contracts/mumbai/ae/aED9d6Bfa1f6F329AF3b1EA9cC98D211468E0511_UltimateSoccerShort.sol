...
contract UltimateSoccerShort is Pausable{
  ...
  struct Player {
    ...
    function transferFrom(
        ...
        if (from != address(0)) {
            ...
            delete playerIndexToApproved[tokenId];
        }
    ...
    function addPlayer(uint256 tokenId) public {
        players.push(Player(tokenId, tokenURI(tokenId)));
    }
    ...
    function delPlayer(uint256 tokenId) public {
        delete players[tokenId];
    }
    ...
    function mint(address to, uint256 tokenId) public onlyCOO {
        mint(to, tokenId);
        addPlayer(tokenId);
    }

    function setRoyaltyInfo(address _receiver, uint96 _royaltyFees) public onlyCFO {
        royaltyAddress = _receiver;
        royaltyFees = _royaltyFees;
    }

    function royaltyMint(address to, uint256 tokenId, uint96 _royaltyFees, address _royaltyAddress) public onlyCOO {
        mint(to, tokenId);
        setRoyaltyInfo(_royaltyAddress,_royaltyFees);
        addPlayer(tokenId);
    }
  ...
  function _beforeTokenTransfer(address from, address to, uint256 tokenId)
    ...
    // ERC-165 Compatibility (https://github.com/ethereum/EIPs/issues/165)
     function supportsInterface(bytes4 interfaceId)
            ...
            returns (bool)
    {
        return interfaceId == 0x2a55205a || supportsInterface(interfaceId);
    }
