...
contract Marketplace is ERC721Holder {
    ...
    mapping(address => bool) public acceptedTokens; // Добавленный маппинг для хранения разрешенных токенов
    ...
    function buy(address nftContract, uint256 itemId, address tokenContract) external {
        Item storage item = items[itemId];
        require(item.isForSale, "Item is not for sale");
        require(item.seller != address(0), "Item does not exist");

        // Проверка, что указанный токен разрешен
        require(acceptedTokens[tokenContract], "Token not accepted");

        // Получение интерфейса для токена
        IERC20 token = IERC20(tokenContract);
        uint256 allowance = token.allowance(msg.sender, address(this));
        require(allowance >= item.price, "Insufficient token allowance");
    ...
    // Добавление разрешенного токена
    function addAcceptedToken(address tokenContract) external onlyMarketplaceOwner {
        acceptedTokens[tokenContract] = true;
    }

    // Удаление разрешенного токена
    function removeAcceptedToken(address tokenContract) external onlyMarketplaceOwner {
        delete acceptedTokens[tokenContract];
    }
}