...
contract DistrictPower is ReentrancyGuard, Bucket {
    ...
    struct PositionInfo { //盲盒信息
        ...
        uint256 stopLossTime;//爆仓时间
        uint256 lossAmount;//损失金额
        bool lossAmountExist;//true奖励存在
        uint256 claimableDays;//索赔天数
        uint256 claimableAmount;//索赔数量
    }
    ...
    function setPosition ( //开盲盒  入金
        ...
        // construct position info 构造位置信息
        {
            ...
            if(ledgerType == 3 && userGlobalInfo.rewardAmount > 0 && userGlobalInfo.rewardTime + TIME_UNIT > block.timestamp){
                uint256 claimableDays = 30 + _getRandomIndex(currentEpochs[ledgerType],10);
    ...
    function getRewardsEligibility(
        uint256 epoch,
        uint256 ledgerType,
        uint256 positionIndex
        ...
        positionInfo.lossAmountExist = false;
        return (positionInfo.stopLossTime, positionInfo.lossAmount);
    }
