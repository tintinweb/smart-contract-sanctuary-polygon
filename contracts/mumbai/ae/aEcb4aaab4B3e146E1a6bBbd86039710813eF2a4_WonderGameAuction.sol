...
contract WonderGameAuction is Ownable, INFTMintInitiator {
    ...
    constructor(
        ...
        //auctionRegistry[0].startTime=_starttime;
        //auctionRegistry[0].endTime=_starttime+bidtime;
        uint256 requestId = wonderGameMinter.mint(address(this), 1);
        requestStartTime[requestId] = startTime;
    }
    ...
    function acknowledgeMint(
        ...
        uint256 _startTime = requestStartTime[_requestId];
        _start(_tokenIds[0], _startTime);
        requestIds[_requestId] = true;
    }

    // function startNewAuction() external {
    //     _startNewAuction(startTime);
    // }

    // function _startNewAuction(uint256 _startTime) internal {
    ...
    function _bid(uint256 _bidAmount) internal {
        ...
        require(auctionRegistry[auctionId].started, "Auction not started");
        require(user != address(0), "Caller shouldnt be zero address");
        require(currenttimestamp <= auctionRegistry[auctionId].endTime, "Auction not ended");
    ...
    function claim(uint256 _auctionId) external {
        address user = msg.sender;
        require(auctionRegistry[_auctionId].ended, "Auction not ended");
    ...
    function _end() internal returns (uint256) {
        require(auctionRegistry[auctionId].started, "Auction not started!");
        uint256 tempEnd = auctionRegistry[auctionId].endTime;
        require(block.timestamp >= auctionRegistry[auctionId].endTime, "Auction time is not over!");
        require(!auctionRegistry[auctionId].ended, "Auction is already Ended!");
