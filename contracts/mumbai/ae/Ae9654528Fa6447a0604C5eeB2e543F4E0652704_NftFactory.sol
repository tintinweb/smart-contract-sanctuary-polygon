...
contract NftFactory is Ownable, ERC721, ProxyTarget {
	...
	mapping(uint256 => uint8) public tokenLevel; // ID to level
	//COUNTS
	...
	function tokenURI(uint256 tokenId) public view override(ERC721) returns (string memory) {
		...
		return
			bytes(currentBaseURI).length > 0
				? string(abi.encodePacked(currentBaseURI, uint256(tokenLevel[tokenId]).toString(), baseExtension))
				: "";
	}
	...
	function mintBatch(
        ...
        for (uint256 j = 0; j < receiver.length; j++) {
            for (uint256 i = 0; i < tokenAmount[j]; i++) {
                ...
                tokenLevel[tokenId_.current()] = 1;
                tokenOwner[tokenId_.current()] = receiver[j];
            }
        }
	}

	// game call
	function restrictedChangeNft(uint256 tokenID, uint8 level) external {
