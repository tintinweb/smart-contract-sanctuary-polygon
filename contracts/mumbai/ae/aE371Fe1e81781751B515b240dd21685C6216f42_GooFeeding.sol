...
contract GooFeeding is ERC721Full {
    ...
    uint256 public constant growthPercent = 5;
    ...
    function getBaseMaxLevel(DNA _DNA) public pure returns (uint16 maxLevel) {
        if (uint256(_DNA) == 0) {
            maxLevel = 30;
        } else if (uint256(_DNA) == 1) {
            maxLevel = 35;
        ...
        returns (uint16 level)
    {
        ...
        int256 totalGOO = int256(egg.totalFeed + GOO + 20000 ether);
        int256 step = PRBMathSD59x18.div(totalGOO, 20000 ether);
