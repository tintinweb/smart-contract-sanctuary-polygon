...
// File: wagmi staking/wagmi_staking.sol_flattened.sol
...
contract WagmigamesLock is Ownable, ReentrancyGuard {
	...
	event Claimed(address indexed user, uint256 amount,  uint256 claimedTime);
    event Compounded(address indexed user, uint256 amount, uint256 compoundedTime);
    event Withdrawn(address indexed user,  uint256 amount, uint256 withdrawnTime);
	event NewDeposit(address indexed user, uint256 planIdx, uint256 amount);
    ...
    function lockup(uint256 _amount, uint8 _planIdx) external nonReentrant {
        require(!emergencyPaused, "Project has been paused");
    ...
    /**
    * Withdraw just locked amounts in case of project emergency
    */

    function emergencyWithdraw(uint256 depIdx) public nonReentrant {
        require(emergencyPaused, "Project has been paused");

        TUser storage user = users[msg.sender];

        require(!user.deposits[depIdx].isDeceased, "deposit is deceased");

        user.deposits[depIdx].checkpoint = block.timestamp;
		user.deposits[depIdx].isDeceased = true;

         uint256 withdrawableAmount = user.deposits[depIdx].amount;
        ...
        stakingToken.safeTransfer(msg.sender, withdrawableAmount);

        emit Withdrawn(msg.sender, withdrawableAmount,  block.timestamp);
    }
    ...
    function unlock (uint256 depIdx) private {
		...
		emit Claimed(msg.sender, claimAmount_, block.timestamp );
        emit Withdrawn(msg.sender, withdrawableAmount,  block.timestamp);
    ...
    function claim(uint256 depIdx) public nonReentrant{
        ...
        if(block.timestamp >= lockupPeriod){
            ...
            stakingToken.safeTransfer(msg.sender, claimAmount);
    ...
    function compoundDividends(uint256 depIdx) external nonReentrant{
        ...
        user.deposits[depIdx].compoundedRewards += claimAmount_; // update the compounded rewards
    ...
    public view returns (uint256 o_amount, uint256 checkPoint) {
		...
		uint256 timeB = block.timestamp >= deposit.timeEnd ? deposit.timeEnd : block.timestamp;

		 // Let's calculate the maximum amount which can be earned per annum
        (uint256 interestEarnedPerAnnum) = _getResult(deposit.planIdx, deposit.amount);
        uint256 dayCounts = plans[deposit.planIdx].durationDays * TIME_STEP;

        // Let's calculate how many wei are earned per second
        uint256 weiPerSecond = interestEarnedPerAnnum.div(dayCounts);
        
        if (timeA < timeB) {
            uint256 secondsRemaining = timeB.sub(timeA);
            // There are 31536000 seconds per annum, so let's calculate the interest for this remaining time period
            o_amount = weiPerSecond.mul(secondsRemaining);
    ...
    function setEnergencyPause(bool value) external onlyOwner(){
