...
contract EthermonAvatarDistributor is AccessControl {
    ...
    bool public onlyMintingByTokenHoldersAllowed = true;
    uint256 public minTokenBalanceRequired = 1000000000000000000000; //1000 EMON to mint
    ...
    function mint(uint256 _num) public payable returns (bool) {
        ...
        uint256 evavBalance = eavaToken.balanceOf(msg.sender);

        require(
            (whitelist1Enabled &&
                whitelist1[msg.sender] &&
                (evavBalance >= MAX_MINT_LIMIT)) ||
                (
                    (whitelist2Enabled &&
        ...
        require(
            (!whitelist2Enabled && (evavBalance >= MAX_MINT_LIMIT)) ||
    ...
    bool public _mintingUsingEMONPaused = false;
    uint256 public tokenPriceEMON = uint256(2000 * 10**18); // = 2000 EMON paid to mint

    function setMaxMintLimit(uint8 _limit) external {
        require(
            _limit < 0 && hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
    ...
    function getUserMintCount(address _addr) public view returns (uint8) {
        return userMintCount[_addr];
    }
