...
contract BWBMARKETM is Initializable , IERC721ReceiverUpgradeable , ERC1155HolderUpgradeable ,  OwnableUpgradeable {

    using AddressUpgradeable for address payable;
    using CountersUpgradeable for CountersUpgradeable.Counter;

    address payable private feeAddr;
    CountersUpgradeable.Counter private auctionIds;

    function __ADFMARKET_init(address payable feeAddress) internal onlyInitializing {
        feeAddr = feeAddress;
        auctionEndTime = 0;
    }

    function initialize() initializer public {
        __ADFMARKET_init(payable(msg.sender));
        __Ownable_init(); 
    }
    ...
    modifier isStart(address _NFTAddress , uint256 _TokenID , uint256 _startPrice) {
        ...
        require(msg.sender == IERC721Upgradeable(_NFTAddress).ownerOf(_TokenID) , "MSG.SENDER is not the owner");
    ...
    {   
        ...
        auction memory _auction = auction({
            ...
            _endTime        : block.timestamp + 5 minutes,
            _Started        : true,
            _Ended          : false
        });
        ...
        IERC721Upgradeable(_NFTAddress).safeTransferFrom(msg.sender, address(this), _TokenID);

        emit Start(_NFTAddress , _TokenID , _auctionInfo[_NFTAddress][_TokenID]);
    }

    function getAuctionEndTime (uint256 _auctionEndTime) public onlyOwner{
        ...
        idEnd(_NFTAddress , _TokenID)
    {
        ...
        if (c._HighestBidder == address(0)){ 
            ...
            IERC721Upgradeable(_NFTAddress).safeTransferFrom(address(this), c._Seller , _TokenID);

        } else {
            
            IERC721Upgradeable(_NFTAddress).safeTransferFrom(address(this), c._HighestBidder , _TokenID);
        ...
        emit End(c._HighestBidder , c._HighestBid);
    }
    ...
    mapping ( address => mapping ( uint256 => NFTSellInfo )) _DepositNFT1155;
    ...
    event buyNFTs(address _NFTAddress , uint256 _tokenId , address seller ,address buyer);
    ...
    modifier isNFTSale (address _NFTAddress , uint256 _TokenId , uint256 _Price ) {
        ...
        require (_DepositNFT[_NFTAddress][_TokenId]._Selled == false , "NFT ");
        require (_Price > 0 , "Price must be greater than 0 ");
        _;
    }

    modifier isOwnerOf(address _NFTAddress , uint256 _TokenId , address account) {
        require (account == IERC721Upgradeable(_NFTAddress).ownerOf(_TokenId) , "...");
        _;
    }
    ...
    function depositNFT1155 (address _NFTAddress , uint256 _TokenID, uint256 _Price , bytes memory data) public {
        ...
        _DepositNFT1155[_NFTAddress][_TokenID] = _info;

        IERC1155Upgradeable(_NFTAddress).safeTransferFrom(msg.sender, address(this), _TokenID, 1, data);

        emit DepositNFT(_NFTAddress, _TokenID, _DepositNFT[_NFTAddress][_TokenID]);
    }

    function cancelAtNFT1155 (address _NFTAddress , uint256 _TokenID,  bytes memory data) public {
        
        IERC1155Upgradeable(_NFTAddress).safeTransferFrom(address(this), msg.sender, _TokenID, 1, data);
        ...
        onlyNFTSeller(_NFTAddress , _TokenID ) 
    {
        ...
        IERC721Upgradeable(_NFTAddress).safeTransferFrom(address(this) , msg.sender , _TokenID);
    ...
    {   
        ...
        emit buyNFTs(_NFTAddress , _TokenID , c._Seller ,c._Buyer);
    }
