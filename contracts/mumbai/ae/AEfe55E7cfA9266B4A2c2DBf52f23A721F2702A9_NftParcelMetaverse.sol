...
contract NftParcelMetaverse is ERC721URIStorage {
    ...
    function getmetadataAttribute(uint256 _tokenId) public view returns (string memory) {
        return string(abi.encodePacked(
            ...
            string(abi.encodePacked('{"trait_type":"Name_Map_World","value":"', Attribute[_tokenId].map, '"}'))
        ));
    }

    function formatTokenURI(string memory _imageURI, uint256 _tokenId) public view returns (string memory) {
        string memory attribute = getmetadataAttribute(_tokenId);
        return string(
                abi.encodePacked( 
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                ' "attributes":"[', attribute,']", "image":"',_imageURI,'"}'
                            )
                        )
                    )
                )
