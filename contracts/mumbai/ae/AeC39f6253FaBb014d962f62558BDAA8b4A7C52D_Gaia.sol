...
GAIA - a 0xBossyBabe(TM) token, in partnership with "Grandmother Grove" tree-planting charity and "Charged Particles" NFT protocol.
https://twitter.com/0xBossyBabe
https://grandmothergrove.org/
https://charged.fi/
*/

/// @title The GAIA Token
/// @author 0xBossyBabe
/// @notice The GAIA Token is the social token for 0xBossyBabe's Gaia project.
/// @dev Mintable. Burnable. Snapshot. Access control. Taxable. Unstuckable.
...
contract Gaia is ReentrancyGuard, ERC20, ERC20Burnable, ERC20Snapshot, IERC721Receiver, IERC1155Receiver, AccessControl, Pausable, Taxable, Releasable {
    ...
    function allowToken(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _allowToken(token);
    }

    function disallowToken(address token) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _disallowToken(token);
    }

    function lockAllowToken() public onlyRole(DEFAULT_ADMIN_ROLE) {
        _lockAllowToken();
    }

    function lockDisallowToken() public onlyRole(DEFAULT_ADMIN_ROLE) {
        _lockDisallowToken();
    }
    
    function releaseAllETH(address payable account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseAllETH(account);
    }
    
    function releaseETH(address payable account, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseETH(account, amount);
    }

    function releaseAllERC20(IERC20 token, address account) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseAllERC20(token, account);
    }
    
    function releaseERC20(IERC20 token, address account, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseERC20(token, account, amount);
    }

    function releaseERC721(IERC721 token, address account, uint256 tokenId) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseERC721(token, account, tokenId);
    }

    function releaseAllERC1155(IERC1155 token, address account, uint256 tokenId) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseAllERC1155(token, account, tokenId);
    }
    
    function releaseERC1155(IERC1155 token, address account, uint256 tokenId, uint256 amount) public onlyRole(DEFAULT_ADMIN_ROLE) {
        _releaseERC1155(token, account, tokenId, amount);
    }
    ...
    function burnAll(address account) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(hasRole(BLACKLISTED, account), "Error: address not blacklisted");
        ...
        override(ERC20, ERC20Snapshot)
    {
        if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            require(!paused(), "Error: transfers are paused");
        }
   ...
   function _transfer(address from, address to, uint256 amount)
        ...
        nonReentrant
    {
        if (hasRole(DEFAULT_ADMIN_ROLE, msg.sender)) {
            ...
            require(!hasRole(BLACKLISTED, to), "Error: recipient blacklisted");
            require(!hasRole(BLACKLISTED, from), "Error: sender blacklisted");
...
/// @author People's Treasury
/// @notice Allows contract to accept and release native, ERC20, ERC721, and ERC1155 tokens.
...
/// @dev Name and type of constants defined.
    ...
    bool private _allowUnlocked;
    bool private _disallowUnlocked;

/// @dev Constructor includes payable modifier.

    constructor(address[] memory __disallowed) payable {
        _allowUnlocked = true;
        _disallowUnlocked = true;
        for (uint256 i = 0; i < __disallowed.length; i++) {
            _disallowToken(__disallowed[i]);
        }
    }
...
/// @dev Internal functions to require token be allowed or disallowed.
    ...
    function _requireTokenAllowed(address token) internal view virtual {
        require(_disallowed[token] != true, "Releasable: token must not be disallowed");
    }

    function _requireTokenDisallowed(address token) internal view virtual {
        require(_disallowed[token] == true, "Releasable: token must not be allowed");
    }

    function _requireAllowUnlocked() internal view virtual {
        require(_allowUnlocked == true, "Releasable: disallow list locked from allowing new tokens");
    }
    
    function _requireDisallowUnlocked() internal view virtual {
        require(_disallowUnlocked == true, "Releasable: disallow list locked from disallowing new tokens");
    }

/// @dev Internal functions allow external calls to NFT's for transfering.

    function _callOptionalReturnERC721(IERC721 token, bytes memory data) private {
        bytes memory returndata = address(token).functionCall(data, "Unstuckable: low-level call failed");
        if (returndata.length > 0) {
            require(abi.decode(returndata, (bool)), "Unstuckable: ERC721 operation did not succeed");
        }
    }
    
    function _callOptionalReturnERC1155(IERC1155 token, bytes memory data) private {
        bytes memory returndata = address(token).functionCall(data, "Unstuckable: low-level call failed");
        if (returndata.length > 0) {
            require(abi.decode(returndata, (bool)), "Unstuckable: ERC1155 operation did not succeed");
        }
    }

/// @dev Internal virtual functions update the disallowed list of tokens.

    function _disallowToken(address token) internal virtual {
        _requireDisallowUnlocked();
        _requireTokenAllowed(token);
        _disallowed[token] = true;
        emit DisallowToken(token);
    }

    function _allowToken(address token) internal virtual {
        _requireAllowUnlocked();
        _requireTokenDisallowed(token);
        _disallowed[token] = false;
        emit AllowToken(token);
    }

    function _lockAllowToken() internal virtual {
        _allowUnlocked = false;
    }
    
    function _lockDisallowToken() internal virtual {
        _disallowUnlocked = false;
    }
...
/// @dev Internal virtual functions perform the requested contract updates and emit the events to the blockchain.
    ...
    function _releaseAllETH(address payable account) internal virtual {
        ...
        _releaseETH(account, payment);
    }

    function _releaseETH(address payable account, uint256 payment) internal virtual {
		uint256 totalBalance = address(this).balance;
        require(totalBalance != 0, "Unstuckable: no network tokens to release");
        require(totalBalance >= payment, "Unstuckable: not enough network tokens to release");
        Address.sendValue(account, payment);
        emit ReleasedETH(account, payment);
    }

    function _releaseAllERC20(IERC20 token, address account) internal virtual {
		uint256 payment = token.balanceOf(address(this));
        _releaseERC20(token, account, payment);
    }
    
    function _releaseERC20(IERC20 token, address account, uint256 payment) internal virtual {
        _requireTokenAllowed(address(token));
		uint256 totalBalance = token.balanceOf(address(this));
        require(totalBalance != 0, "Unstuckable: no ERC20 tokens to release");
        require(totalBalance >= payment, "Unstuckable: not enough ERC20 tokens to release");
        SafeERC20.safeTransfer(token, account, payment);
        emit ReleasedERC20(token, account, payment);
    }

    function _releaseERC721(IERC721 token, address account, uint256 tokenId) internal virtual {
        _requireTokenAllowed(address(token));
        address owner = token.ownerOf(tokenId);
        require(owner == address(this), "Unstuckable: contract is not owner of ERC721");
        _callOptionalReturnERC721(token, abi.encodeWithSelector(token.transferFrom.selector, address(this), account, tokenId));
    ...
    function _releaseAllERC1155(IERC1155 token, address account, uint256 tokenId) internal virtual {
        uint256 payment = token.balanceOf(address(this), tokenId);
        _releaseERC1155(token, account, tokenId, payment);
    }

    function _releaseERC1155(IERC1155 token, address account, uint256 tokenId, uint256 payment) internal virtual {
        _requireTokenAllowed(address(token));
        uint256 totalBalance = token.balanceOf(address(this), tokenId);
        require(totalBalance != 0, "Unstuckable: no ERC1155 tokens to release");
        require(totalBalance >= payment, "Unstuckable: not enough ERC1155 tokens to release");
        _callOptionalReturnERC1155(token, abi.encodeWithSelector(token.safeTransferFrom.selector, address(this), account, tokenId, payment));
        emit ReleasedERC1155(token, account, tokenId, payment);
    }
