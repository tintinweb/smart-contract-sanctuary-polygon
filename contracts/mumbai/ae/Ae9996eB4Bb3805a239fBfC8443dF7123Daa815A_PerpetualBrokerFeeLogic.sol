...
{
    /**
     * What does the exchange charge for this order? (including broker fee)
    ...
    /**
     * Splits the given (relative) fee into the portion that stays with the AMM
     * and that which is distributed to liquidity providers
     * @param _fee Relative fee to split, in tbps
     * @return Fee sent to the treasury
     * @return Fee sent to the PnL participation fund (liquidity providers)
     */
    ...
    /**
     * Set the oracle factory address to convert the traded volume into
     * USD
     * @param _iPerpetualId id of perpetual
     * @param _oracleFactoryAddr address for oracle factory
     */
    function setOracleFactoryForPerpetual(uint24 _iPerpetualId, address _oracleFactoryAddr)
        external
        override
        onlyMaintainer
    {
        _setOracleForPerpetual(_iPerpetualId, _oracleFactoryAddr);
    }

    /**
     * Set utility token (own more tokens pay less fees)
     * @param _tokenAddr D8X token
     */
    function setUtilityTokenAddr(address _tokenAddr) external override onlyMaintainer {
        require(_tokenAddr != address(0), "invalid _tokenAddr");
        utilityTokenAddress = _tokenAddr;
        emit SetUtilityToken(_tokenAddr);
    }

    /**
     * Set fees for broker due to purchased lots ('designation')
     * @param _designations array of USD-volumes
     * @param _feesTbps relative fee array in 1/10 of a basis point (=1e-5)
     */
    function setFeesForDesignation(uint32[] calldata _designations, uint16[] calldata _feesTbps)
        external
        onlyMaintainer
    {
        _setTiers(_designations, _feesTbps, brokerDesignations, designationFeesTbps);
        emit SetBrokerDesignations(_designations, _feesTbps);
    }

    /**
     * Set fees for broker utility token ownership
     * There are n tiers and n+1 fees
     * @param _tiers array of token ownership tiers
     * @param _feesTbps relative fee array in 1/10 of a basis point (=1e-5)
     */
    function setBrokerTiers(uint256[] calldata _tiers, uint16[] calldata _feesTbps)
        external
        onlyMaintainer
    {
        _setTiers(_tiers, _feesTbps, brokerTiers, brokerFeesTbps);
        emit SetBrokerTiers(_tiers, _feesTbps);
    }

    /**
     * Set fees for trader utility token ownership
     * There are n tiers and n+1 fees
     * @param _tiers array of trader ownership tiers
     * @param _feesTbps relative fee array in 1/10 of a basis point (=1e-5)
     */
    function setTraderTiers(uint256[] calldata _tiers, uint16[] calldata _feesTbps)
        external
        onlyMaintainer
    {
        _setTiers(_tiers, _feesTbps, traderTiers, traderFeesTbps);
        emit SetTraderTiers(_tiers, _feesTbps);
    }

    /**
     * Set the volume thresholds and the corresponding fees for traders.
     * There are n tiers and n+1 fees
     * @param _tiers boundaries for average volume traded
     * @param _feesTbps fee in 1/10 of a basis point
     */
    function setTraderVolumeTiers(uint256[] calldata _tiers, uint16[] calldata _feesTbps)
        external
        onlyMaintainer
    {
        _setTiers(_tiers, _feesTbps, traderVolumeTiers, traderVolumeFeesTbps);
        emit SetTraderVolumeTiers(_tiers, _feesTbps);
    }

    /**
     * Set the volume thresholds and the corresponding fees for brokers.
     * There are n tiers and n+1 fees
     * @param _tiers boundaries for average volume traded
     * @param _feesTbps fee in 1/10 of a basis point
     */
    function setBrokerVolumeTiers(uint256[] calldata _tiers, uint16[] calldata _feesTbps)
        external
        onlyMaintainer
    {
        _setTiers(_tiers, _feesTbps, brokerVolumeTiers, brokerVolumeFeesTbps);
        emit SetBrokerVolumeTiers(_tiers, _feesTbps);
    }

    /**
     * Convert base-currency to USD using the last recorded price.
     * It is acceptable if this price is slightly outdated.
     * @param _iPerpetualId id of the perpetual for which we want the conversion
     */
    ...
    /**
     * Get the broker-fee that a trader is charged with the given
     * broker
     * @param _poolId pool
     * @param _brokerAddr broker address
     */
    ...
    /**
     * Get relative fee in 1/10 basis points for the given number of
     * purchased broker-lots
     * @param _brokerDesignation number of lots
     */
    ...
    /**
     * Get number of lots purchased by the broker
     * (=designation)
     * @param _poolId designation is per liq pool
     * @param _brokerAddr address of the broker
     */
    ...
    /**
     * Get the fee for the broker resulting from the amount of utility
     * tokens that the broker owns (broker stake)
     * @param _brokerAddr address of the broker
     */
    ...
    /**
     * Get the fee for the trader resulting from the amount of utility
     * tokens that the trader owns (trader stake)
    ...
    /**
     * Get the fee for the trader resulting from their trading volume
    ...
    /**
     * Get the traded volume measure for the trader in the given
     * pool
     * @param _poolId pool id
     * @param _traderAddr address of the trader
     */
    function getCurrentTraderVolume(uint8 _poolId, address _traderAddr)
        ...
        returns (int128)
    {
        return _getCurrentVolumeEMA(traderVolumeEMA[_poolId][_traderAddr]);
    }

    /**
     * Get the broker fee charged to the trader resulting from the broker
     * trading volume
     * @param _poolId pool id
     * @param _brokerAddr address of the broker
     */
    ...
    /**
     * Get the traded volume measure for the broker in the given
     * pool
     * @param _poolId pool id
     * @param _brokerAddr address of the broker
     */
    function getCurrentBrokerVolume(uint8 _poolId, address _brokerAddr)
        ...
        returns (int128)
    {
        return _getCurrentVolumeEMA(brokerVolumeEMA[_poolId][_brokerAddr]);
    }

    /**
     * Transfer some lots of broker address (broker lots will be re-assigned)
     * @param _poolId           id of the pool
     * @param _transferToAddr   transfer ownership from msg.sender to this address
     * @param _numberLots       number of lots to transfer. If > owned lots, reduced to owned lots
     */
    function transferBrokerLots(
        ...
        _transferBrokerLots(_poolId, _transferToAddr, _numberLots);
    }

    /**
     * Transfer all the broker volume and lots to another address
     * @param _poolId           id of the pool
     * @param _transferToAddr   transfer ownership from msg.sender to this address
     */
    function transferBrokerOwnership(uint8 _poolId, address _transferToAddr) external override {
        //4294967295=max uint 32
        _transferBrokerLots(_poolId, _transferToAddr, 4294967295);
        if (brokerVolumeEMA[_poolId][msg.sender].fTradingVolumeEMAusd > 0) {
            _transferBrokerVolume(_poolId, _transferToAddr);
        }
    }

    /**
     * This function is called by the perpetuals contracts when a broker
     * joins the pool. It sets the initial volume for the broker artificially
     * such that the fee resulting from the trading volume corresponds to the invested lots.
     * @param _poolId id of the pool
     * @param _brokerAddr broker address
     * @param _feeTbps fee in 1/10 of a basis point (=1e-5)
     */
    function setInitialVolumeForFee(
        ...
        require(msg.sender == address(this), "onlythis");
        require(brokerMap[_poolId][_brokerAddr] == 0, "not first deposit");
        require(brokerVolumeFeesTbps.length > 0, "broker volume tiers not set");
        int128 volumeTier = ConverterDec18.fromDec18(
            int256(_searchTier(_feeTbps, brokerVolumeTiers, brokerVolumeFeesTbps))
        );
        VolumeEMA storage vol = brokerVolumeEMA[_poolId][_brokerAddr];
        vol.fTradingVolumeEMAusd = volumeTier;
        vol.timestamp = uint64(block.timestamp);
    }
    ...
    function getFunctionList() external pure virtual override returns (bytes4[] memory, bytes32) {
        ...
        bytes4[] memory functionList = new bytes4[](24);
        ...
        functionList[9] = this.splitProtocolFee.selector;
        functionList[10] = this.queryExchangeFee.selector;
        functionList[11] = this.updateVolumeEMAOnNewTrade.selector;
        functionList[12] = this.setOracleFactoryForPerpetual.selector;
        functionList[13] = this.getLastPerpetualBaseToUSDConversion.selector;
        functionList[14] = this.setTraderVolumeTiers.selector;
        functionList[15] = this.setBrokerVolumeTiers.selector;
        functionList[16] = this.getFeeForBrokerVolume.selector;
        functionList[17] = this.getFeeForTraderVolume.selector;
        functionList[18] = this.transferBrokerLots.selector;
        functionList[19] = this.transferBrokerOwnership.selector;
        functionList[20] = this.getCurrentBrokerVolume.selector;
        functionList[21] = this.getCurrentTraderVolume.selector;
        functionList[22] = this.setInitialVolumeForFee.selector;
        functionList[23] = this.getBrokerDesignation.selector;
...
contract PerpetualVolumeTierFunctions is PerpetualBrokerFeeFunctions {
    ...
    // EMA lambda:
    // E(EMA) = 1/(1 - L) * E(trade size) =  N * E(trade size)  -> L = 1 - 1/N
    // 1 - 1 / (30 days in seconds) = round((1 - 1/(30*24*60*60))*(2**64)) = 18446736956910141440 = 0xfffff986fdb2fe48
    int128 internal constant VOLUME_EMA_LAMBDA = 0xfffff986fdb2fe48;
    uint64 internal constant BASE_TO_USD_UPDATE_TIME = 3600; //how frequent (seconds) do we update the price info
    uint64 public lastBaseToUSDUpdateTs;

    /**
     * @dev Link a perpetual to an oracle factory.
     * @param _iPerpetualId Perpetual id, unique across liquidity pools
     * @param _oracleFactoryAddr Address of the oracle factory to set
     */
    ...
    /**
     * @dev Determine exchange fee based on a pool, a trader, and a broker
     * @param _poolId Liquidity pool id
     * @param _traderAddr Trader address
     * @param _brokerAddr Broker address
     * @return fFeeTbps Trading fee charged by protocol
     */
    ...
    /**
     * @dev Determine the exchange fee based solely on a given trader's trading volume
     */
    ...
    /**
     * @dev Determine the exchange fee based solely on a given broker's volume
     */
    ...
    /**
     * Get time-weighted volume EMA given its last observed value.
     */
    function _getCurrentVolumeEMA(VolumeEMA storage vol) internal view returns (int128) {
        uint256 iDeltaTime = uint256(uint64(block.timestamp) - vol.timestamp);
        int128 fLAdj = iDeltaTime > 18000000 ? int128(0) : VOLUME_EMA_LAMBDA.pow(iDeltaTime);
        return vol.fTradingVolumeEMAusd.mul(fLAdj);
    }
    ...
    /**
     * @dev Computes an EMA that considers observations that could be irregularly made over time
     */
    function _ema(VolumeEMA storage vol, int128 _absTradeAmountUSD) internal {
        ...
        if (iDeltaTime > 18000000) {
            ...
            fLAdj = VOLUME_EMA_LAMBDA.pow(iDeltaTime);
        }
        vol.timestamp = ts;
    ...
    /**
     * @dev Determine the exchange fee of all perps in a liquidity poo based soleley on a broker's tier.
     */
    function _getBrokerInducedFee(uint8 _poolId, address _brokerAddr)
        ...
        returns (uint16)
    {
        uint32 iBrokerDesignation = brokerMap[_poolId][_brokerAddr];
    ...
    /**
     * @dev Determine the exchange fee of all perps in a liquidity poo based soleley on a trader's tier.
     */
    ...
    /**
     * Transfer lots of broker address to a new address (broker lots will be re-assigned)
     * New address will contain old lots plus new ones
     * @param _poolId           id of the pool
     * @param _transferToAddr   transfer ownership from msg.sender to this address
     * @param _numLots          number of lots to transfer
     */
    function _transferBrokerLots(
        uint8 _poolId,
        address _transferToAddr,
        uint32 _numLots
    ) internal {
        require(brokerMap[_poolId][msg.sender] != 0, "sender not broker");
        uint32 numLotsAvailable = brokerMap[_poolId][msg.sender];
        if (numLotsAvailable <= _numLots) {
            delete brokerMap[_poolId][msg.sender];
            _numLots = numLotsAvailable;
        } else {
            brokerMap[_poolId][msg.sender] -= _numLots;
        }
        brokerMap[_poolId][_transferToAddr] += _numLots;
        emit BrokerLotsTransferred(_poolId, msg.sender, _transferToAddr, _numLots);
    }

    /**
     * Transfer broker volume to another address
     * @param _poolId           id of the liquidity pool
     * @param _transferToAddr   new broker address
     */
    function _transferBrokerVolume(uint8 _poolId, address _transferToAddr) internal {
        //mapping(uint8 => mapping(address => VolumeEMA)) public brokerVolumeEMA;
        require(
            brokerVolumeEMA[_poolId][msg.sender].fTradingVolumeEMAusd > 0,
            "no vol to transfer"
        );
        VolumeEMA storage tradingVolume = brokerVolumeEMA[_poolId][msg.sender];
        VolumeEMA storage otherTradingVolume = brokerVolumeEMA[_poolId][_transferToAddr];
        // add to trading volume
        otherTradingVolume.fTradingVolumeEMAusd += tradingVolume.fTradingVolumeEMAusd;
        // we leave the timestamp unchanged unless inexistent
        if (otherTradingVolume.timestamp == 0) {
            otherTradingVolume.timestamp = tradingVolume.timestamp;
        }
        delete brokerVolumeEMA[_poolId][msg.sender];
        emit BrokerVolumeTransferred(
            _poolId,
            msg.sender,
            _transferToAddr,
            tradingVolume.fTradingVolumeEMAusd
        );
    }
}
...
contract PerpetualBrokerFeeFunctions is PerpetualBaseFunctions, PerpetualHashFunctions {
    ...
    uint32[] public brokerDesignations;
    uint16[] public traderFeesTbps;
    uint16[] public brokerFeesTbps;
    uint16[] public designationFeesTbps;

    address public utilityTokenAddress;
    int128 public constant PNL_PART_FEE_RATIO = 0x4000000000000000; //ABDK(0x4000000000000000)=0.25

    /**
     * @dev Computes the fee corresponding to a given broker designation, in tbps
     * Reverts if fee structure is not set.
     * @param _brokerDesignation Number of lots
     */
    function _getFeeForBrokerDesignation(uint32 _brokerDesignation)
        internal
        view
        returns (uint16)
    {
        require(designationFeesTbps.length > 0, "no designation fee set");
        if (designationFeesTbps.length == 1) {
            return designationFeesTbps[0];
        }
        return _searchFee(_brokerDesignation, brokerDesignations, designationFeesTbps);
    }

    /**
     * @dev Splits the given (relative) fee into the portion that stays with the AMM
     * and that which is distributed to liquidity providers
     * @param _fee Relative fee to split, in tbps
     * @return Fee sent to the treasury
     * @return Fee sent to the PnL participation fund (liquidity providers)
     */
    function _splitProtocolFee(uint16 _fee) internal pure returns (int128, int128) {
        int128 treasuryfee = (ConverterDec18.TBPSTOABDK * int128(uint128(_fee)));
    ...
    /**
     * @dev Utility function to set tiers and fees. Tiers are token amounts, hence uint256.
     */
    function _setTiers(
        uint256[] calldata _tiers,
        uint16[] calldata _feesTbps,
    ...
    /**
     * @dev Utility function to set tiers and fees. Tiers are lot amounts, hence uint32.
     */
    function _setTiers(
        uint32[] calldata _tiers,
        uint16[] calldata _feesTbps,
        uint32[] storage _tiersTarget,
        uint16[] storage _feesTbpsTarget
    ) internal {
        require(_tiers.length + 1 == _feesTbps.length, "fees must fit tier");
        uint256 k;
    ...
    /**
     * @dev Find the fee that corresponds to the tier implied by the amount. Amount is uin256 (tokens)
    ...
    /**
     * @dev Find the fee that corresponds to the tier implied by the amount. Amount is uint32 (lots)
     * @param _amount   amount of lots owned
     * @param _tiers    array with tier limits
     * @param _fees     array with fees in tenth of basis points
     */
    function _searchFee(
        uint32 _amount,
        uint32[] storage _tiers,
        uint16[] storage _fees
    ) internal view returns (uint16) {
        uint256 k;
        while (k < _tiers.length && _amount > _tiers[k]) {
    ...
    /**
     * @dev Find the amount that corresponds to the tier implied by the fee. Fee is in tbps.
     * @param _fee target fee
     * @param _tiers array with tier limits
     * @param _fees array with fees in tenth of basis points
     */
    function _searchTier(
        uint16 _fee,
        uint256[] storage _tiers,
        uint16[] storage _fees
    ) internal view returns (uint256) {
        // if target fee can't be achieved, return highest tier (with the lowest fee)
        if (_tiers.length < 1) {
            // no tiers, default to 0
            return 0;
        } else if (_fee <= _fees[_fees.length - 1]) {
            // highest tier
            return _tiers[_tiers.length - 1];
        }
        // target tier is somewhere in between
        uint256 k = _tiers.length - 1;
        while (k > 0 && _fee > _fees[k]) {
            k = k - 1;
        }
        return _tiers[k];
    }
