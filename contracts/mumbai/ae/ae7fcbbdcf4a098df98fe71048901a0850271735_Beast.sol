...
import "./FreyjaRelic.sol";
import "./SharedLib.sol";

contract Beast is BaseERC721Upgradeable {
    event Mint(uint256 tokenId, address to, uint8 faction);
    event ChangeClass(uint256 tokenId, uint8 class);
    ...
    event ExperienceUpdated(uint256 tokenId, uint256 experience);
    ...
    ERC20Burnable private _skull;
    ERC20Burnable private _tear;
    IROItem private _item;
    FreyjaRelic private _freyjaRelic;
    uint256 private _tokenIds;
    mapping(uint256 => BeastStruct) private _beasts;
    mapping(address => bool) private _trainers;
    mapping(uint256 => uint64[ITEM_SLOTS]) private _beastItems;
    mapping(uint8 => SharedLib.Stats) private _factionStats;
    ...
    function _burnSpellRequirements() internal {
        ...
        _freyjaRelic.burnBatch(spellRelics, spellRelicsAmounts);
    }
    ...
    function _mintBeast(uint8 faction) internal returns (uint256) {
        ...
        _beasts[newBeastId] = BeastStruct({
            level: 1,
            experience: 0,
            faction: faction,
            class: 0
        ...
        emit Mint(newBeastId, msg.sender, faction);
        return newBeastId;
    }
        ...
        noContracts
    {
        ...
        BeastStruct storage beast = _beasts[beastId];
        require(beast.class == 0, "only base class");
        require(beast.level > 19, "not allowed");
        _tear.burnFrom(msg.sender, 10000 ether);

        beast.class = class;
        emit ChangeClass(beastId, class);
    }

    function giveExperience(uint256 beastId, uint64 experience)
        external
        onlyTrainers
    {
        BeastStruct storage beast = _beasts[beastId];
        ...
        emit ExperienceUpdated(beastId, beast.experience);

        uint64 nextLevelExperience = beast.level == 1
            ? 500
            : 500 * (beast.level**2) + 500 * ((beast.level - 1)**2);

        if (beast.experience > nextLevelExperience && beast.level < 99) {
            beast.level++;
            emit LevelUp(beastId, beast.level);
        }
    }
    ...
    function assignItem(
        ...
        uint8 slot,
        uint256 beastId
    ) external onlyBeastOwner(beastId) noContracts {
        _validateItemRequirements(beastId, itemId, slot);
        _removeItem(beastId, slot);

        _item.transferFrom(msg.sender, address(this), itemId);
        _beastItems[beastId][slot] = itemId;

        emit ItemAssign(beastId, itemId, slot);
    }

    function removeItem(uint256 beastId, uint8 slot)
        external
        onlyBeastOwner(beastId)
        noContracts
    {
    ...
    function _validateItemRequirements(
        uint256 beastId,
        uint64 itemId,
        uint8 slot
    ) internal view {
        SharedLib.ItemType memory itemType = _item.getItemType(itemId);
        bool validSlot = false;
        for (uint256 index = 0; index < itemType.slots.length; index++) {
            if (itemType.slots[index] == slot) {
                validSlot = true;
                break;
            }
        }
        require(validSlot, "Invalid item slot");

        BeastStruct memory beast = _beasts[beastId];

        require(beast.level >= itemType.levelRequirement, "not enough lvl");

        uint16 classNumberRepresentation = SharedLib
            .classToNumberRepresentation(beast.faction, beast.class);

        uint16[12] memory itemClasses = itemType.classes;
        bool validClass = false;

        for (uint256 index = 0; index < itemClasses.length; index++) {
            if (itemClasses[index] == classNumberRepresentation) {
                validClass = true;
                break;
            }
        }
        require(validClass, "Beast class can't equip item");
    }

    function _removeItem(uint256 beastId, uint8 slot)
        internal
        onlyBeastOwner(beastId)
    {
        uint256 previousItem = _beastItems[beastId][slot];
        if (previousItem > 0) {
            _item.transferFrom(address(this), msg.sender, previousItem);
            _beastItems[beastId][slot] = 0;
            emit ItemRemove(beastId, slot);
        }
    }
    ...
    function setAddresses(
        ...
        _skull = ERC20Burnable(_skullAddress);
        _item = IROItem(_itemAddress);
        _freyjaRelic = FreyjaRelic(_freyjaRelicAddress);
        _tear = ERC20Burnable(_tearAddress);
    }

    function updateFactionStats(
        uint8 faction,
        SharedLib.Stats memory factionStats
    ) external onlyOwner {
        _factionStats[faction] = factionStats;
    }

    function addTrainer(address trainer) external onlyOwner {
        _trainers[trainer] = true;
    }

    function removeTrainer(address trainer) external onlyOwner {
        _trainers[trainer] = false;
    }
    ...
    function getBeast(uint256 beastId)
        external
        view
        returns (BeastStruct memory, uint64[ITEM_SLOTS] memory)
    {
        BeastStruct memory beast = _beasts[beastId];

        uint64[ITEM_SLOTS] memory itemSlots = _beastItems[beastId];

        return (beast, itemSlots);
    }
        ...
        returns (BeastStruct[] memory, uint64[ITEM_SLOTS][] memory)
    {
        ...
        BeastStruct[] memory beasts = new BeastStruct[](tokenIds.length);
        uint64[ITEM_SLOTS][] memory items = new uint64[ITEM_SLOTS][](
            tokenIds.length
        );
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            beasts[index] = _beasts[beastId];
            items[index] = _beastItems[beastId];
        }
        return (beasts, items);
    }

    function getBeastItems(uint256 beastId)
        external
        view
        returns (uint64[ITEM_SLOTS] memory)
    {
        return _beastItems[beastId];
    }

    function getBeastLevel(uint256 beastId) external view returns (uint8) {
        return _beasts[beastId].level;
    }

    function getBeastClass(uint256 beastId) external view returns (uint8) {
        return _beasts[beastId].class;
    }

    function getBeastFaction(uint256 beastId) external view returns (uint8) {
        return _beasts[beastId].faction;
    }
        ...
        returns (uint16)
    {
        return
            SharedLib.classToNumberRepresentation(
                _beasts[beastId].faction,
                _beasts[beastId].class
            );
    }
    ...
    function _getBeastStats(uint256 beastId)
        ...
        returns (SharedLib.Stats memory)
    {
        uint8 beastFaction = _beasts[beastId].faction;
        SharedLib.Stats memory stats = _factionStats[beastFaction];

        for (uint8 slot = 0; slot < ITEM_SLOTS; slot++) {
            if (_beastItems[beastId][slot] > 0) {
                SharedLib.Stats memory itemModifiers = _item.getItemStats(
                    _beastItems[beastId][slot]
                );
                stats.hp += itemModifiers.hp;
                stats.mana += itemModifiers.mana;
                stats.con += itemModifiers.con;
                stats.str += itemModifiers.str;
                stats.dex += itemModifiers.dex;
                stats.wis += itemModifiers.wis;
                stats.intell += itemModifiers.intell;
            }
        }
    ...
    modifier onlyTrainers() {
        require(_trainers[msg.sender], "only trainers");
        _;
    }
    ...
    UUPSUpgradeable
{
    ...
    function airdropRelics(uint256 id, address[] memory wallets)
        external
        onlyOwner
    {
        require(
            _relicMintCounts[id] + wallets.length <= _relicMintLimits[id],
            "E02: Supply limit exceeded"
        );
