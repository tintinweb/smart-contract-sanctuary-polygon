...
contract Book is Utils{
    ...
    constructor(address _bookAddr, uint256 _index){
        book = MyBook(_bookAddr);
        index = _index;
    }
    uint256 public sumPercent = 10**6;
}
...
contract Valve is Book, ERC1155Holder {
    ...
    function Split(address _token) public returns(uint256){
        Percent[] memory percents = book.returnPercents(index);
        ...
        if (branchCount > 0){            
            ...
            for (uint i = 0; i < branchCount; i++) {
                bool success = ERC20(_token).transfer(percents[i].addr, amount * percents[i].percent / sumPercent);
            }        
