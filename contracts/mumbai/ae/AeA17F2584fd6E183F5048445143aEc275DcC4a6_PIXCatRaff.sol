...
contract PIXCatRaff is IPIXCatRaff, VRFConsumerBaseV2, Ownable, Pausable {
    ...
    uint256 public constant SECONDS_PER_WEEK = 1 hours;
    ...
    function claimPrize(uint256 _week) external whenNotPaused {
        for (uint8 place = 0; place < 5; place++) {
            if (!prizeClaimed[_week][place] && place < weeklyWinners[_week].length) {
                address winner = weeklyWinners[_week][place];
                if (msg.sender == winner) {
                    prizeClaimed[_week][place] = true;

                    WeeklyTicketInfo storage info = weeklyTickets[_week];
                    uint256 prize = ((info.prize + weeklyPrize) * POOL_PERCENT[place]) / 100;

                    pixToken.safeTransferFrom(treasury, msg.sender, prize);

                    emit PrizeClaimed(_week, place, msg.sender, prize);
                }
            }
        }
    }
