...
contract BoomerangCross is ERC721, ERC721URIStorage, ERC721Burnable, ERC2981, Ownable, ReentrancyGuard {
    ...
    uint256 price = 0.03 ether;                 // 30000000000000000 wei, not cost for users, only newcommers pay for mint
    ...
    function _pfpMint(string memory _uri) internal returns (uint256 _tokenId) {
        ...
        _setMinter(msg.sender, _tokenId, _uri);
    }

    // _when = x hours {[14400 = 4H], [28800 = 8H], [43200 = 12H], [86400 = 24H]}
    function _msgMint(address _to, string memory _uri, uint256 _when) internal returns (uint256 _tokenId, uint256 _destroy, address _sender) {
