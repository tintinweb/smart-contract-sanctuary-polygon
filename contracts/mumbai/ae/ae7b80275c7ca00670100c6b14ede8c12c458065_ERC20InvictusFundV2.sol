...
// File contracts/ERC20InvictusFundV2.sol
...
contract ERC20InvictusFundV2 is OwnableUpgradeable, ERC20PresetMinterPauserUpgradeable {
  ...
  event RequeueRedemptionEvent(uint256 withdrawAmount, string ticker, address participant);
  ...
  /**
   * @dev Requeue redemtion in a supported ticker to be processed on the next dealing.
   * @param withdrawAmount The amount of fund tokens to redeem.
   * @param ticker The ticker you would like to recieved your redemption in.
   * @param participant The user to receive the redemption
   */
  function requeueRedemption(
    uint256 withdrawAmount,
    string memory ticker,
    address participant
  ) external onlyInvestmentOracle {
    redemptions.push(Redemption(withdrawAmount, block.timestamp, ticker, participant));
    emit RedemptionRequest(withdrawAmount, block.timestamp, ticker, participant);
  }
  ...
  function queueInvestment(
    ...
    investments.push(Investment(amount, block.timestamp, chainId, index, investmentTicker, transactionHash, investor));
  ...
  /**
   * @dev Updates the whitelist address.
   * @param newWhitelistAddress The new address where whitelist is checked.
   */
  function updateWhitelistAddress(address newWhitelistAddress) public onlyOwner {
    require(newWhitelistAddress != address(0), "Invalid address");
    whitelistContract = newWhitelistAddress;
  }
  ...
  function updateCutOfftime(uint32 time) public onlyPricingOracle {
    require(time > 0, "Invalid cut off time");
    cutOffTime = time;
  }
  ...
  function _handleRedemptionsDealing() private returns (bool) {
    ...
    for (uint256 i = redemptionsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(redemption.investor)) {
        ...
        redemptions.pop();
        emit RequeueRedemptionEvent(redemption.amount, redemption.ticker, redemption.investor);
      }
    }
  ...
  function _handleInvestmentsDealing() private returns (bool) {
    ...
    for (uint256 i = investmentsLength(); i > endingIndex; i--) {
      ...
      if (!InvictusWhitelist(whitelistContract).isWhitelisted(investment.investor)) {
        ...
        investments.pop();
        investmentTxIndex[investment.transactionHash][investment.index] = false;
      }
    }
