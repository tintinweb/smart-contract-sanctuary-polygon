...
/* @title OpenScienceNFT
 * @notice Smart contract for minting unique SCI NFTs
 */
contract OpenScienceNFT is ERC721, Ownable {
     ...
     *
     * _baseURI value is immutable: it can only be set once during construction.
     */
    constructor(
        string memory _baseURI,
        address _admin,
        address contractOwner
    ) ERC721("OpenScienceNFT", "SCINFTS") {
        //solhint-disable-next-line reason-string
        require(
            contractOwner != address(0),
            "Contractowner cannot be zero addr"
        );
        require(_admin != address(0), "Admin cannot be zero addr");
        admin = _admin;
    ...
    /**
     * @dev To update contract admin
     *
     * Requirements:
     * - invocation can be done, only by the contract owner.
     */
    function updateAdmin(address newAdmin) external onlyOwner {
        require(newAdmin != address(0), "New admin cannot be zero addr");
        address oldAdmin = admin;
        admin = newAdmin;
        emit AdminChanged(oldAdmin, newAdmin);
    }

    function updateBaseURI(string memory _baseURI) external onlyOwnerOrAdmin {
    ...
    /**
     * @dev To mint OpenScienceNFT (OpenScienceNFT)
     *
     * Requirements:
     * - invocation can be done, only by the contract owner or Admin.
     */
    function mint(address[] memory _receivers, uint256[] memory mintIds)
        external
        onlyOwnerOrAdmin
    {
    ...
    /**
     * @dev To get all mined nfts
     */
    function totalSCINFTS() external view returns (uint256) {
        return totalSupply();
    }
     ...
     * @dev To get all mined nfts
     */
    function sciNFTsOF(address user) external view returns (uint256[] memory) {
    ...
    function _transfer(
        ...
        // CHECK FOR 1111111... if it exists in `to` then put the tokenId in that slot
