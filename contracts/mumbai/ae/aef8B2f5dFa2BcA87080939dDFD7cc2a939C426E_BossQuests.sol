...
contract BossQuests is Multicall, HSystemChecker {
    ...
    function deleteBossQuest(uint16 bossQuestId)
        external
        onlyRole(ADMIN_ROLE)
    {
        _deleteBossQuest(bossQuestId);
    ...
    function _pack16_128(uint16[] memory things) internal pure returns (uint128 result) {
    ...
    function _unpack16_128(uint128 packedThings) internal pure returns (uint16[] memory result) {
