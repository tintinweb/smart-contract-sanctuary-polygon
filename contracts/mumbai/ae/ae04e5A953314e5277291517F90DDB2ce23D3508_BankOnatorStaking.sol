...
contract BankOnatorStaking is Ownable, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    uint256 public DIVIDER;
    uint256 public DECIMAL;
    uint256 public APYBASE; // in seconds = 365 days
    ...
    function initalize(address token) external initializer {
        addMembership(1 * 10 ** DECIMAL, 20);
        addMembership(500000 * 10 ** DECIMAL, 40);
        addMembership(1000000 * 10 ** DECIMAL, 60);
        addMembership(1500000 * 10 ** DECIMAL, 80);
        setToken(token);
    ...
    function setLocker(address _locker) external onlyOwner {
        require(_locker != address(0), "Locker address cannot be 0");
        locker = _locker;
    ...
    function setLastRewardTime(uint256 _newLastRewardTime) external {
        lastRewardTime = _newLastRewardTime;
    }

    function stake(uint256 tokens) external nonReentrant whenNotPaused {
        require(tokens > 0, "You need to stake token greater than 0");

        uint256 currentEpochTime = block.timestamp;
        require(
            currentEpochTime <= lastRewardTime,
            "Error: Current Epoch Time is greater than the Last Reward Time"
        );

        Stake storage staker = _stakes[msg.sender];
        uint256 newStakedAmount = staker.stakedAmount + tokens;
        require(
            membershipLevels[0].threshold <= newStakedAmount,
            "Insufficient tokens for staking."
        );
        _token.transferFrom(msg.sender, address(this), tokens);

        if (_stakes[msg.sender].stakedAmount != 0) {
            uint256 totalStaked = _stakes[msg.sender].stakedAmount;
            uint256 newRewardAmount = calculateReward(
                getAPY(totalStaked),
                _stakes[msg.sender].lastStakedTime,
                totalStaked
            );
            staker.totalEarned += newRewardAmount;
        }
        staker.lastStakedTime = currentEpochTime;
        staker.stakedAmount = newStakedAmount;
    ...
    function unstake(uint256 unstakeAmount) external nonReentrant {
        ...
        Stake storage staker = _stakes[msg.sender];
        require(
            unstakeAmount <= staker.stakedAmount,
            "Unstake amount exceeds total staked amount"
        );

        uint256 resultingStakeAmt = _stakes[msg.sender].stakedAmount -
            unstakeAmount;

        require(
            resultingStakeAmt == 0 ||
                resultingStakeAmt >= membershipLevels[0].threshold,
        ...
        if (resultingStakeAmt != 0) {
            uint256 newRewardAmount = calculateReward(
                getAPY(staker.stakedAmount),
                staker.lastStakedTime,
                staker.stakedAmount
            );
            _stakes[msg.sender].totalEarned += newRewardAmount;
        }

        _token.transfer(msg.sender, unstakeAmount);

        if (resultingStakeAmt == 0) {
            delete _stakes[msg.sender];
        } else {
            staker.stakedAmount = resultingStakeAmt;
    ...
    function claim() external whenNotPaused {
        Stake storage stakeDetails = _stakes[msg.sender];

        uint256 newRewardAmount = calculateReward(
            getAPY(stakeDetails.stakedAmount),
            stakeDetails.lastStakedTime,
            stakeDetails.stakedAmount
        );

        stakeDetails.totalEarned += newRewardAmount;

        uint256 claimableAmount = stakeDetails.totalEarned -
            stakeDetails.claimed;

        require(
            claimableAmount > 0,
            "Error: No claimable amount available. The claimable amount must be greater than zero to proceed with the claim"
        );

        _token.transferFrom(locker, msg.sender, claimableAmount);

        stakeDetails.lastStakedTime = block.timestamp;
        stakeDetails.claimed += claimableAmount;
    ...
    function getTotalRewards(address user) public view returns (uint256) {
        ...
        uint256 lastStakedTime = userStake.lastStakedTime;
        uint256 totalStaked = userStake.stakedAmount;
        uint256 totalEarned = userStake.totalEarned;
        uint256 claimed = userStake.claimed;
        uint256 APY = getAPY(totalStaked);

        return
            calculateReward(APY, lastStakedTime, totalStaked) +
            totalEarned -
            claimed;
    }
    ...
    function calculateReward(
        ...
        if (lastStakedTime == 0 || lastStakedTime >= lastRewardTime) {
            return 0;
        }

        uint256 currentTimeStamp = block.timestamp;

        uint256 timeElapsed = currentTimeStamp <= lastRewardTime
            ? currentTimeStamp
            : lastRewardTime - lastStakedTime;
        uint256 numerator = tokens.mul(APY).mul(timeElapsed);
        uint256 denominator = DIVIDER.mul(APYBASE);

        return numerator.div(denominator);
    }

    function changeMembershipAPY(
        uint256 index,
        uint256 newAPY
    ) external onlyOwner {
    ...
    function emergencyExit() external nonReentrant {
        Stake storage staker = _stakes[msg.sender];
        uint256 unstakeAmount = staker.stakedAmount;
