    ...
    ReentrancyGuard
{
    ...
    // 10 = 10% of booster pack price distributed to each card creator (besides minter), so 40% total in a 5-card pack.
    ...
    constructor(
        ...
        setCreatorPercentage(_creatorPercentage);
    }
    ...
    // 
    // Mints boosterPackSize random cards from the existing collection.
    // The difference between _mintBoosterPackAfterSupplyLimitReached and
    // _mintBoosterPack is that the latter mints a new card and then chooses 4 random ones.
    function _mintBoosterPackAfterSupplyLimitReached(
        address account,
        bytes memory data
    ) private {
        ...
        for (uint256 i = 0; i < boosterPackSize; i++) {
            ...
            // Royalties: Send 10% of mint price to the 4 other card creators in this booster pack
            (bool success,) = payable(_addressesById[rand]).call{value:msg.value * (creatorPercentage / 100)}("");
    ...
    function mintBoosterAfterSupplyLimitReached(bytes memory data)
        public
        payable
        mintBoosterPackPriceCompliance()
    {
        // require(!allowlistMintEnabled, 'The public mint has not started.');
        _mintBoosterPackAfterSupplyLimitReached(_msgSender(), data);
    }

    // Mints 1 new card and boosterPackSize-1 random cards from the collection.
