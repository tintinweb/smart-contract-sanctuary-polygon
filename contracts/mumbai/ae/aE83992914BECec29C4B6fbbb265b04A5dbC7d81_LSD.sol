...
import "hardhat/console.sol"; // TODO remove on mainnet deployment
import "./abstracts/BaseContract.sol";
import "./interfaces/IRankingNFT.sol";
...
contract LSD is BaseContract {
  ...
  event STMinted(address indexed miner, uint amount, uint expiredAt, uint timestamp);
  ...
  function mint(uint _tokenAmount, uint _duration) external {
    ...
    if (_duration >= 60 && _tokenAmount >= config.minAmountForTaxDiscount) {
      if (users[msg.sender].qualifyForDiscountUntil > block.timestamp) {
        users[msg.sender].qualifyForDiscountUntil += _duration * config.secondsInADay;
      } else {
    ...
    emit STMinted(msg.sender, _tokenAmount, expiredAt, block.timestamp);
  }
...
interface IRankingNFT is IERC721Upgradeable {
