...
contract AlchemistDAO {
    ...
    address private immutable _alchemistBank;
    constructor(address mic_, address alchemist_, address alchemistBank_){
        _mic = mic_;
    ...
    function withdraw (uint256 tokenId) public returns(uint256){
        ...
        IERC20 mic = IERC20(_mic);
        uint256 totalAmount = mic.balanceOf(_alchemistBank) + totalWithdrawal;
        if ((totalAmount / 3000) > withdrawn[tokenId]) {
            uint256 amount = (totalAmount / 3000) - withdrawn[tokenId];
            withdrawn[tokenId] += amount;
            IBank(_alchemistBank).handleOutgoingTransfer(msg.sender, amount, _mic);
            totalWithdrawal += amount;
            require(mic.balanceOf(_alchemistBank) + totalWithdrawal == totalAmount, 'Withdrawal failed');
            return amount;
        }
        return uint256(0);
    }
