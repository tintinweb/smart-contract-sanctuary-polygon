...
contract Daf is ERC20, Ownable, Pausable {
    ...
    */
    ...
    function transferToSales(address _dafSalesAddress) external onlyOwner {
        require(
            _dafSalesAddress != address(0),
            "Daf: daf sales address should not be zero address"
        );
        _transfer(address(this), _dafSalesAddress, maxSupply);
    }
    ...
    override
    {
        if (from != address(0) && balanceOf(from) < dafNft.userHedgingBalance(from)) {
            dafNft.stopUserHedges(from, balanceOf(from));
        }
        super._afterTokenTransfer(from, to, amount);
    }
