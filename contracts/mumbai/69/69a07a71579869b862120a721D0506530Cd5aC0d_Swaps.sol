...
contract Swaps is ERC721Enumerable, Ownable, SuperAppBase{
    ...
    function newSwap(address _receiver, address _payer, int96 _requiredFlowRate, uint _amountUnderlying) external{
        cfaV1.createFlowByOperator(_payer, address(this), token, _requiredFlowRate);
        ...
        returns (bytes memory newCtx)
    {
        ...
        try IStrategy(a.strategy).closeSwap(a.amountUnderlyingExposed){
        }catch(bytes memory e){
            emit Error(string(e));
        }

        // payer index is always +1 receiver
        try ISwapReceiver(receiver).settle(settlement, a.lockedCollateral, ownerOf(receiverIndex+1), a.strategy){
