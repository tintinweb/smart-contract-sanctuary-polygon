...
contract ERC20Factory is Ownable, CloneBase {
    ...
    event ERC20Created(address newERC20Address, uint256 _identifier);
    ...
    function addImplementation(address _implementation) external onlyOwner {
        ...
        implementationIdVsImplementation[nextId] = _implementation;
        nextId = nextId.add(1);

        emit ImplementationLaunched(nextId, _implementation);
    }

    function updateImplementation(uint256 _id, address _implementation)
        external
        onlyOwner
    {
        require(
            implementationIdVsImplementation[_id] != address(0),
            "Implementation not present"
        );
        implementationIdVsImplementation[_id] = _implementation;

        emit ImplementationUpdated(_id, _implementation);
    }
    ...
    function _createERC20(
        ...
        ERC20Lib(clone).init(_implementationData);

        OwnableUpgradeable(clone).transferOwnership(owner);

        emit ERC20Created(clone, _id);
    }
    ...
    function createERC20(
        ...
        _handleFeeManager();
        _createERC20(_id, _owner, _implementationData);
    }
...
contract ERC20Lib is ERC20Upgradeable, OwnableUpgradeable {
    event TokenURLUpdated(string url);
    ...
    function updateTokenURL(string memory _tokenURL) external onlyOwner {
        tokenURL = _tokenURL;
        emit TokenURLUpdated(_tokenURL);
    }
}
...
abstract contract Initializable {
    ...
    modifier initializer() {
        console.log('_initializing ',_initializing);
        console.log('_initialized ',_initialized);
        console.log('_isConstructor() ',_isConstructor());
