...
contract GamespadLand is ERC721Psi, AccessProtected, IERC721Receiver {
    ...
    uint256 private constant PLOT_SIZE = 300;
    uint256 private constant TOTAL_PLOTS = PLOT_SIZE * PLOT_SIZE;
    ...
    constructor() ERC721Psi("GAMESPAD LAND", "LAND") {
        baseURI = "";
    }
     ...
     */
    function mintLands(uint256 _quantity) external onlyAdmin {
        require(
            _quantity <= TOTAL_PLOTS,
            "INVALID OPERATION: _quantity param of mintLands cannot be greater than the amount of TOTAL_PLOTS"
        );
     ...
     * @dev - callable only by admin
     *
     * @param _quantity - number of PLOTs to mint
     */
    function setPlots(uint256 _quantity) external onlyAdmin {
        require(
            _quantity <= TOTAL_PLOTS,
            "INVALID OPERATION: _quantity param of setPlots cannot be greater than the amount of TOTAL_PLOTS"
        );
        for (uint256 i = 0; i < _quantity; i++) {
            uint256 newNftId = _nftIds.current();
            require(newNftId < TOTAL_PLOTS, "INVALID OPERATION: tokenId exceeds the TOTAL_PLOTS amount");
            uint256 x = newNftId % PLOT_SIZE;
            uint256 y = newNftId / PLOT_SIZE;
            uint256 coords = formatCoordinates(x, y);
            require(_coordsToId[coords] == 0, "NFT for {x,y} already minted");
            _coordsToId[coords] = newNftId;
            _idToCoords[newNftId] = coords;
            _nftIds.increment();
        }
    }
    ...
    /**
     * @notice - get Coordinates by Token ID
     *
