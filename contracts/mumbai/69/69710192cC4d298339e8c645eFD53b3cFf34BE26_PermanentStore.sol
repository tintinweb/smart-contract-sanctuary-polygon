...
import "../core/CozyItem.sol";

contract PermanentStore is AccessControl {
  ...
  struct PermanentItemView {
    uint id;
    CozyItem.Item cozyItem;
    uint price;
    bool locked;
    bool exists;
  }

  mapping(uint => PermanentItem) private permanentItems;
  uint public numItems;

  CozyCoin private coin;
  CozyItem private item;

  error DoesNotExist(uint id);
  error AlreadyExists(uint id);
  error Locked(uint id);
  error TransferFailed(uint id);

  event Purchased(uint indexed id, address indexed owner, uint indexed cozyItemId, uint supply);

  constructor(address _coin, address _item) {
    coin = CozyCoin(_coin);
    item = CozyItem(_item);
  ...
  function getPermanentItem(uint id) external view returns (PermanentItemView memory) {
    PermanentItem memory permanentItem = permanentItems[id];
    return
      PermanentItemView({
        id: permanentItem.id,
        cozyItem: item.getItem(permanentItem.cozyItemId),
        price: permanentItem.price,
        locked: permanentItem.locked,
        exists: permanentItem.exists
      });
  }

  function getPermanentItems(uint[] calldata ids)
    external
    view
    returns (PermanentItemView[] memory)
  {
    PermanentItemView[] memory result = new PermanentItemView[](ids.length);

    for (uint i = 0; i < ids.length; i++) {
      result[i] = this.getPermanentItem(ids[i]);
    }
  ...
  function mint(uint permanentItemId, uint amount) external notLocked(permanentItemId) {
    PermanentItem memory _permanentItem = permanentItems[permanentItemId];
    CozyItem.Item memory _item = item.getItem(_permanentItem.cozyItemId);

    if (!_item.exists) {
      revert DoesNotExist(_item.id);
    }

    bool sent = coin.transferFrom(msg.sender, address(this), _permanentItem.price * amount);

    if (!sent) {
      revert TransferFailed(permanentItemId);
    }

    item.mint(msg.sender, _item.id, amount);

    emit Purchased(permanentItemId, msg.sender, _item.id, _item.supply);
  }

  // ----------- Admin --------

  function createPermanentItem(uint cozyItemId, uint price) external onlyRole(DEFAULT_ADMIN_ROLE) {
    permanentItems[numItems] = PermanentItem({
      id: numItems,
      cozyItemId: cozyItemId,
      exists: true,
      locked: false,
      price: price
    });
  ...
  function updatePermanentItemPrice(uint id, uint price)
    external
    onlyRole(DEFAULT_ADMIN_ROLE)
    notLocked(id)
    exists(id)
  {
    permanentItems[id].price = price;
  }

  function lockPermanentItem(uint id) external onlyRole(DEFAULT_ADMIN_ROLE) exists(id) {
    permanentItems[id].locked = true;
  }

  // --------- Modifiers -----------

  modifier notLocked(uint id) {
    if (permanentItems[id].locked) {
      revert Locked(id);
    }
    _;
  }
...
contract CozyItem is AccessControl, ERC1155, ERC1155Burnable {
  ...
  event Created(Item item);

  error AlreadyExists(uint id);
  error Invalid(uint id);
  ...
  function getItems(uint[] memory ids) external view returns (Item[] memory) {
  ...
  function setUri(uint id, string memory _uri) external onlyRole(DEFAULT_ADMIN_ROLE) {
    Item storage item = items[id];
    if (item.uriLocked) {
      revert IsLocked(id);
    }
  ...
  function lockUri(uint id) external onlyRole(DEFAULT_ADMIN_ROLE) {
    items[id].uriLocked = true;
  }

  function lockCap(uint id) external onlyRole(DEFAULT_ADMIN_ROLE) {
    items[id].capLocked = true;
  }

  function lockMint(uint id) external onlyRole(DEFAULT_ADMIN_ROLE) {
    items[id].mintLocked = true;
  }
  ...
  function _beforeTokenTransfer(
    ...
    for (uint i = 0; i < ids.length; i++) {
      ...
      if (isMinting && !item.exists) {
        revert Invalid(id);
      }

      if (isMinting && item.mintLocked) {
        revert IsLocked(id);
      }

      if (isMinting && item.supply + amounts[i] > item.cap) {
        revert IsCapped(id);
      }

      if (isMinting) {
        item.supply += amounts[i];
      } else {
        item.supply -= amounts[i];
      }
    }
