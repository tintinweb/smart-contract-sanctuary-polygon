...
interface EthermonAdventureTransformInterface {
    function transform(uint64 _monObj) external;
}
...
contract EtheremonAdventureHandler is BasicAccessControl, EthermonEnum {
    ...
    address public ethermonTransformAdventure;
    ...
    function handleSingleItem(
        ...
        if (_classId == expItemClass) {
            uint8 currentLevel = getLevel(obj.exp) - 1;
            ...
            EthermonAdventureTransformInterface adventureTransform = EthermonAdventureTransformInterface(
                    ethermonTransformAdventure
                );
            adventureTransform.transform(obj.monsterId);
    ...
    function handleMultipleItems(
        ...
        require(
            (currentValue - STAT_MAX_VALUE) < maxValue,
            "Already reached max stat"
        );
    ...
    function getMonsterDataInfoByIndex(address _trainer, uint256 _index)
        public
        view
        returns (MonsterObjWithStats memory)
    {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        EtheremonMonsterNFTInterface monsterTokenContract = EtheremonMonsterNFTInterface(
                monsterNFT
