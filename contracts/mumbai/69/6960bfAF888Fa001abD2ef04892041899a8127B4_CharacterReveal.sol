...
contract CharacterReveal is ICharacterReveal, GameBase, Random {
    constructor(address game)
    GameBase(game) {
    }

    // TODO: add access control modifier
    function reveal(uint256 worldId, uint256 tokenId) external override {
        ...
        require(address(character) == address(0), "already exists!");
    ...
    function getRevealState(uint256 worldId, uint256 tokenId) external view override returns(RevealStateRecord memory)  {
        ...
        if (address(character) == address(0)) {
            return RevealStateRecord(0, false);
        }

        return RevealStateRecord(
            character.isRevealed() ? character.characterDefinitionId() : 0,
    ...
    function _characterReveal(uint256 worldId, uint256 tokenId) private {
        ...
        world.addCharacter(tokenId, record.characterDefinitionIds[index]);

        _itemPackReveal(worldId, tokenId, record.itemPackDefinitionIds[index]);

        Character character = getCharacter(worldId, tokenId);
        character.setIsRevealed(true);

        emit Reveal(worldId, tokenId, character.characterDefinitionId());
    }

    function _itemPackReveal(uint256 worldId, uint256 tokenId, uint256 itemPackDefinitionId) private {
        Character character = getCharacter(worldId, tokenId);
        ...
        for (uint256 i; i < mintedItemDefinitionIds.length; i++) {
            if (mintedItemDefinitionIds[i] > 0 && mintedAmounts[i] > 0) {
                character.addItem(mintedItemDefinitionIds[i], mintedAmounts[i]);
            }
        }

        _equipDefault(worldId, tokenId, mintedItemDefinitionIds);
    }

    function _equipDefault(uint256 worldId, uint256 tokenId, uint256[] memory itemDefinitionIds) private {
        World world = getWorld(worldId);
...
contract ItemPackReveal is IItemPackReveal, GameBase, Random {
    constructor(address game)
    GameBase(game) {
    }

    // TODO: add access control modifier
    function reveal(uint256 worldId, uint256 tokenId, uint256 itemPackId) external override {
        World world = getWorld(worldId);
        Character character = getCharacter(worldId, tokenId);
    ...
    function getItemPacks(uint256 worldId, address playerWallet) external view virtual override returns(uint256[] memory, uint256[] memory, bool[] memory) {
        World world = getWorld(worldId);
        ItemPack itemPack = world.itemPack();

        uint256[] memory itemPackIds = itemPack.getItemPackIds(playerWallet);
        uint256[] memory itemPackDefinitionIds = new uint256[](itemPackIds.length);
        bool[] memory isRevealeds = new bool[](itemPackIds.length);

        for (uint256 i; i < itemPackIds.length; i++) {
            uint256 itemPackId = itemPackIds[i];
            itemPackDefinitionIds[i] = itemPack.itemPackDefinitionId(itemPackId);
            isRevealeds[i] = itemPack.isRevealed(itemPackId);
        }

        return (itemPackIds, itemPackDefinitionIds, isRevealeds);
    }
