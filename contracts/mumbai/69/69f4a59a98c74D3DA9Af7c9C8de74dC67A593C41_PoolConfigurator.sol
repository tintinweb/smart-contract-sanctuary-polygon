...
contract PoolConfigurator is VersionedInitializable, IPoolConfigurator {
  ...
  // /// @inheritdoc IPoolConfigurator
  // function setReduxTransferInterval(uint256 interval) external override onlyPoolAdmin {
  //   _pool.SetReduxTConfiguration(asset, currentConfig);
  // }
  ...
  /// @inheritdoc IPoolConfigurator
  function setReserveRates(address asset, uint256 supplyRate, uint256 borrowRate, uint256 prxyRate, uint256 splitRate) public override onlyPoolAdmin {
    _pool.setReserveRates(asset, supplyRate, borrowRate, prxyRate, splitRate);
  }
...
library DataTypes {
  struct ReserveData {
    ...
    // the split rate.
    uint256 splitRate;
    // the dynamic supply rate
    uint256 dynamicSupplyRate;
    // the dynamic split rate
    uint256 dynamicSplitRate;
...
interface IPoolConfigurator {
  ...
  /**
   * @notice Sets the reserve interest rates and split rate
   * @param asset The address of the underlying asset of the reserve
   * @param supplyRate The supply rate 
   * @param borrowRate The borrow rate
   * @param prxyRate The prxy rate
   * @param splitRate THe split rate
   */
  function setReserveRates(address asset, uint256 supplyRate, uint256 borrowRate, uint256 prxyRate, uint256 splitRate) external;
...
interface IPool {
  ...
  /**
   * @notice Sets the interest rates and split rate of the reserve
   * @dev Only callable by the PoolConfigurator contract
   * @param asset The address of the underlying asset of the reserve
   * @param supplyRate The supply rate 
   * @param borrowRate The borrow rate
...
interface IPoolDataProvider {
   ...
   * @return dynamicSupplyRate The dynamic supply rate rate of the reserve
