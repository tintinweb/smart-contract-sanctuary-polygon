...
contract NFT is ERC1155URIStorage {
    ...
    event CustomTransferSingle(address operator, address indexed from, address indexed to, uint256 id, uint256 value, bytes indexed assetId);
    function mint(uint256 _id, uint256 _amount, bytes memory _data, string memory _uri)public {
    ...
    function _afterTokenTransfer(
        ...
        if(data.length != 0){
            emit CustomTransferSingle(operator, from, to, ids[0], amounts[0], data);
        }
    }
}