...
contract HToken is ERC1155, IERC721Receiver, ExponentialNoError, TokenErrorReporter, AccessControlEnumerable {
  ...
  function redeemInternal(uint256 _amount) internal {
    ...
    uint256 allowed = hivemind.redeemAllowed(HTokenI(address(this)), msg.sender, underlyingToRedeem);
    if (allowed != 0) {
      revert HivemindError(ComptrollerErrorReporter.Error.COMPTROLLER_REJECTION);
    }
  ...
  function repayBorrowInternal(uint256 _repayAmount, uint256 _tokenId) internal {
    uint256 allowed = hivemind.repayBorrowAllowed(HTokenI(address(this)), msg.sender, _tokenId, _repayAmount);
