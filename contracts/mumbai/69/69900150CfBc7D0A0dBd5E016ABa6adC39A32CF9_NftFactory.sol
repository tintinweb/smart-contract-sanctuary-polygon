...
contract NftFactory is Ownable, ERC721, ProxyTarget {
    ...
    function initialize(address _gameAddress, address _tokenAddress,address _whitelistChecker,address _metadata,address _vrf) external {
    ...
    function setContract(address _gameAddress, address _tokenAddress, address _whitelistChecker, address _metadata,address _vrf) external onlyOwner {
        game = IGameEngine(_gameAddress);
        SUP = IERC20(_tokenAddress);
        whitelistChecker = IWhitelist(_whitelistChecker);
        metadataHandler = IMetadata(_metadata);
    ...
    function presaleBuyAndStake(bool stake,uint8 tokenType, uint tokenAmount,bytes memory signature) external payable {
    ...
    // available at this link: https://hungerbrainz.com/terms 
        ...
        require (salePrice*tokenAmount <= msg.value, "INSUFFICIENT_ETH");
    ...
    function buyAndStake(bool stake,uint8 tokenType, uint tokenAmount) external payable {
    //  By calling this function, you agreed that you have read and accepted the terms & conditions
    // available at this link: https://hungerbrainz.com/terms 
        require(tokenType < 2,"Invalid type");
        require(tokenId_.current() <= SUP_THRESHOLD,"Buy using SUP");
