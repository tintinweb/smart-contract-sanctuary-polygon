...
contract ERC721A is IERC721A {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (tokenId < 10)
            return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI,"00",_toString(tokenId),".json")) : '';
        if (tokenId < 100)
             return bytes(baseURI).length != 0 ? string(abi.encodePacked(baseURI,"0",_toString(tokenId),".json")) : '';
        else 
...
contract GalaxyWarriorsRevolution is DefaultOperatorFilterer, ERC721A {
    ...
    uint256 public constant MAXIMUM_SUPPLY = 9229;
    uint256 public constant PUBLIC_MINT_LIMIT_PER_WALLET = 5;
    uint256 public constant PRICE_PER_MINT = 0.05 ether; // PLACEHOLDER VALUE. REPLACE AS NEEDED
