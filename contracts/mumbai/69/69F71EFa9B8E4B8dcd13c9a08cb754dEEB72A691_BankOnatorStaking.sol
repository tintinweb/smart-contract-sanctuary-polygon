...
contract BankOnatorStaking is Ownable, ReentrancyGuard {
    ...
    constructor(address token) {
        addMembership(1 * 10 ** DECIMAL, 200);
        addMembership(500000 * 10 ** DECIMAL, 400);
        addMembership(1000000 * 10 ** DECIMAL, 600);
        addMembership(1500000 * 10 ** DECIMAL, 800);
        setToken(token);
    }
    ...
    function claim() external {
        ...
        uint256 previousClaimed = _stakes[msg.sender].claimed;
        uint256 totalEarned = _stakes[msg.sender].totalEarned;

        _token.transferFrom(locker, msg.sender, totalEarned - previousClaimed);
        _stakes[msg.sender].lastStakedTime = block.timestamp;
        stakeDetails.claimed = totalEarned;

        emit Claimed(msg.sender, totalEarned - previousClaimed);
    }
