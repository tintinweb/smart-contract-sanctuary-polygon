  ...
  IERC721Enumerable
{
  ...
  function addTokenIdMinted(uint quantity) internal{ 
    for(uint i=0; i<quantity;i++){
      if(_exists(currentIndex)){
        currentIndex++;
      }
      tokenIdMinted[currentIndex]=true;

    }
  }
  function initializeCurrentIndex() internal{
    currentIndex = 1; 
  }
...
contract NatashatiniNFT is Ownable, ERC721A, ReentrancyGuard {
  ...
  constructor() ERC721A("Natashatini NFT Test","Natashanatini_T", MaxMintPerBatch_, TotalCollectionSize_) {
    setURIbeforeReveal("https://gateway.pinata.cloud/ipfs/Qma1QXa6GkWRGpYe9PKVpXYSrTbErFPq2pn3AA9veom8Ch");
    setBaseURI("https://gateway.pinata.cloud/ipfs/QmRPDM5vBq5raekngxRKxemkyMkjCMHJCmCMfocx3rgDHn/");
  }
