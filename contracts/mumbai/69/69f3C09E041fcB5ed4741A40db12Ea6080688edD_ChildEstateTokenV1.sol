...
contract EstateBaseToken is ImmutableERC721, Initializable, WithMinter, TheSandbox712 {
    ...
    function _verifyAdjacencyCreate(EstateCRUDData memory creation, bytes memory signature) internal view {
        ...
        require(signer == backAddress, "INVALID SIGNATURE");
    }
    ...
    function _addLandsGamesAssociation(
        ...
        uint256[] memory gamesToAdd = new uint256[](numUniqueGames);
        uint256 index = 0;
        for (uint256 i = 0; i < uiniqueGameIds.length; i++) {
            if (i == 0 && gameIds[i] != 0) {
                gamesToAdd[index] = gameIds[0];
                index++;
            } else {
                if (uiniqueGameIds[i] != 0) {
                    gamesToAdd[index] = gameIds[uiniqueGameIds[i]];
                    index++;
                }
            }
        }
    ...
    function _removeGamesOfLands(
        ...
        if (gameIdsToRemove.length > 0) {
            ...
            /* console.log("number of id uniques");
            console.log(numberOfUnique);
            console.log("number of games to remove");
