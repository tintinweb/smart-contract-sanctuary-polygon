...
contract MerkleDistributor is AccessControl {
    ...
    struct Project {
        ...
        bytes32 vipMerkleRoot; // merkle root level 4-5
        BitMaps.BitMap bitmap; // distribute status of index
        BitMaps.BitMap vipBitmap; // distribute status of index
    ...
    function launchpad(
        ...
        project.vipMerkleRoot = _vipMerkleRoot;
    ...
    function freeBatchMint(
        ...
        for (uint256 j = 0; j < claimdata.length; j++) {
            if (project.vipBitmap.get(claimdata[j].index))
            ...
            project.vipBitmap.set(claimdata[j].index);
        }
    }
    ...
    function freeClaim(
        ...
        if (
            !MerkleProof.verify(
                merkleProof,
                free ? project.vipMerkleRoot : project.merkleRoot,
                node
            )
        ) revert InvalidProof();
        ...
        if (free) {
            ...
            emit freeClaimed(roundID, msg.sender, index, num);
        } else {
            // Receipt token && Refund token
            uint256 total = project.price * num;
            if (project.payment == address(0)) {
    ...
    // Returns true if the index has been marked claimed by this round.
    function isClaimedVip(
        uint256 roundID,
        uint256 index
    ) external view returns (bool) {
        Project storage project = round[roundID];
        return project.vipBitmap.get(index);
    }
}