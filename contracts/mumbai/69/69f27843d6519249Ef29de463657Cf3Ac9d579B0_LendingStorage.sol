...
contract LendingStorage is AccessControl, ReentrancyGuard, Pausable {
  ...
  function setPause(bool _isPause) external onlyAdmin onlyOperator {
  ...
  function setMarket(address _market) external onlyAdmin onlyOperator {
    _setupRole(MARKET, _market);
  }

  function setMaxWaitingTime(uint32 _minutes) external onlyAdmin onlyOperator {
    require(_minutes > 0 && _minutes <= 60, "VALUE_INVALID");
    _maxWaitingTime = _minutes * 60;
  }

  function setPercentDecimals(uint32 _decimals) external onlyAdmin onlyOperator {
    require(_decimals > 0 && _decimals <= 10000, "VALUE_INVALID");
    _percentDecimals = _decimals;
  }

  function setTransactionWallet(address _address) external onlyAdmin onlyOperator {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _transactionWallet = _address;
  }

  function setCommissionWallet(address _address) external onlyAdmin onlyOperator {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _commissionWallet = _address;
  }

  function setCurrencyToken(address _token) external onlyAdmin onlyOperator {
  ...
  function setAddressVerify(address _address) external onlyAdmin onlyOperator {
    require(_address != address(0), "ADDRESS_CAN_NOT_IS_ZERO");
    _verifier = _address;
  }

  function addNFTContractAddress(address[] memory _nftContractAddresses) external onlyAdmin onlyOperator {
    for(uint256 i = 0; i < _nftContractAddresses.length; i++) {
      _contractAddresses.push(_nftContractAddresses[i]);
    }
  }

  function reloadNFTContractAddress(address[] memory _nftContractAddresses) external onlyAdmin onlyOperator {
