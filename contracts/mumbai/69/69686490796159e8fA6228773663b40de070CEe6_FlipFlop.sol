...
contract FlipFlop is OptionsVault {
    ...
    function initialize(
        ...
        swap = Swap(0, 0, 0, 0, 0, address(0), 0, 0);
        ...
        n_T = 10**18;
        // emit Initialized(address(swapVault), address(whiteList));
    }
    ...
    function pause() external onlyOwner {
        vaultState = FlipFlopStates.ContractIsPaused;
    }

    function unpause(FlipFlopStates newContractState) external onlyOwner {
...
contract OptionsVault is Withdraw, ITrufinVault {
    ...
    function internalSwapComputationPostSwap() external onlyOwner {
    ...
    function startEpoch() external nonReentrant onlyOwner {
        require(
            vaultState == FlipFlopStates.FundsToBeSendToFundamentalVaults,
            "e39"
        );
    ...
    function _calculateS_E_S_U(UserDoubleLinkedList.User memory user) internal {
        ...
        if (
            //slither-disable-next-line calls-loop
            whiteList.isUserBlacklisted(user.user) ||
            //slither-disable-next-line calls-loop
    ...
    function _assignUserShares(UserDoubleLinkedList.User memory user) internal {
        ...
        {
            uint256 S2xe1 = swap.S2.wadMul(e1);

            uint256 InSwaXS2 = Compute.mulPrice(
                internalSwapAsset1,
                decimalsOfAsset1,
                swap.S2,
                18,
            ...
            n = (10**18 *
                (u1 +
                    Compute.scaleDecimals(
                        ...
                        decimalsOfAsset1,
                        decimalsOfAsset2
                    ))).wadDiv(internalSwapAsset2 + InSwaXS2);
        }
    ...
    function withdrawFromFundamentalVaults() external onlyOwner {
        ...
        e_o = fundamentalVault1.accountVaultBalance(address(this));
        // Amount of funds received at the end of epoch for asset 2
        u_o = fundamentalVault2.accountVaultBalance(address(this));
        ...
        totalAmountOfAsset1 += e_o;
        totalAmountOfAsset2 += u_o;
    ...
    function _redeemUserShares(UserDoubleLinkedList.User memory user) internal {
        ...
        uint256 ratio = n.wadDiv(n_T);

        // n/n_T * E
        user.amountOfAsset1 += Compute.mulPrice(
            e_o,
            decimalsOfAsset1,
            ratio,
            18,
        ...
        // n/n_T * U
        user.amountOfAsset2 += Compute.mulPrice(
            u_o,
            decimalsOfAsset2,
            ratio,
            18,
        ...
        //slither-disable-next-line costly-loop
        totalLockedAmountOfAsset1 += lock1;
        //slither-disable-next-line costly-loop
        totalLockedAmountOfAsset2 += lock2;
        putUser(user);
    }
    ...
    function sendToSwapVault() external onlyOwner nonReentrant {
    ...
    function receiveSwappedFunds(
        ...
        totalAmountOfAsset1 += amountAsset1;
        totalAmountOfAsset2 += amountAsset2;
    ...
    /// @param targetAddress The address to send the fees to.
    function chargePerformanceFee(address targetAddress)
        external
        onlyOwner
        nonReentrant
    {
        ...
        e_V -= asset1PerformanceFee;
        u_V -= asset2PerformanceFee;

        if (asset1PerformanceFee > 0) {
            //slither-disable-next-line reentrancy-no-eth
            IERC20(asset1).safeTransfer(targetAddress, asset1PerformanceFee);
        }

        if (asset2PerformanceFee > 0) {
            //slither-disable-next-line reentrancy-no-eth
            IERC20(asset2).safeTransfer(targetAddress, asset2PerformanceFee);
        }
...
contract Deposit is UserDoubleLinkedList {
    ...
    function setCap(
        ...
        FlipFlopLib.assertUint104(_maxCapOfAsset1);
        FlipFlopLib.assertUint104(_maxCapOfAsset2);
        FlipFlopLib.assertUint104(_minDepositOfAsset1);
        FlipFlopLib.assertUint104(_minDepositOfAsset2);
    ...
    function calcLockedAmount(
        ...
        if (
            (_queuedAmountOfAsset1ToWithdraw *
                ...
                0) &&
            (_queuedAmountOfAsset1ToWithdraw + _queuedAmountOfAsset2ToWithdraw >
                0)
        ) {
    ...
    function _computeUserValues(
        ...
        if (temp.asset == assetDetails.asset1) {
            ...
            if (Compute.wadMul(e, temp.S) > u) {
                ...
                {
                    uint256 SmulS_E = temp.S.wadMul(assetDetails.S_E - temp.A);
                    stack = stack.wadDiv(assetDetails.S_E);
                    stack = (SmulS_E).wadDiv(assetDetails.S_E) + stack;
                }

                uint256 S_EMinusA = assetDetails.S_E - temp.A + temp.A_S;
            ...
            if (temp.S.wadMul(e) > u) {
...

                {
                    ...
                    stack = (assetDetails.S_U - temp.A).wadDiv(divValue);
...
library LinkedListLib {
    ...
    function _addressHeadCheck(address _address) public pure {
