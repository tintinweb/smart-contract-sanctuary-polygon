...
contract ReflectionsNoirRevised is ERC721AQueryable, Ownable {
    using Strings for uint256;
    ...
    modifier mintChecks(uint8 quantity) {
    ...
    // ===== Private Mint =====
    function privateMint(bytes32[] memory proof, uint8 quantity) external payable mintChecks(quantity) supplyCheck(quantity)  {
    ...
    // ===== Mint =====
    function mint(uint8 quantity) external payable mintChecks(quantity) supplyCheck(quantity)  {
        ...
        if(totalSupply() == maxSupply) revealedState = true;
    }
    ...
    // ===== Split Funds =====
    function sendFunds(uint256 _totalMsgValue) private {
        uint amount1 = (_totalMsgValue * 10) / 100;
        (bool s1,) = BGAddress.call{value: amount1}("");
        (bool s2,) = RSAddress.call{value: _totalMsgValue - amount1}("");
