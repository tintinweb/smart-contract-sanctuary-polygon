...
contract FanTokenV1 is FanTokenVariableV1{
    ...
    */
    ...
    function lockUp(address _to, uint256 _amount, uint256 _duration) external onlyOwner returns(uint256){
        require(msg.sender != _to,"FanTokenV1: Owner cannot lock tokens for itself");
        ...
        emit lockCreated(_to, _amount, _duration);  
        return lockCounter;   
    }
    ...
    */
    ...
    function modifylock(uint256 _lockId, uint256 _duration) external lockExistsOrActive(_lockId) onlyOwner returns(bool) {    
    ...
    */
    ...
    function getlockedAmount(address _user) public view returns (uint256 LockedAmount){
        ...
        for(uint256 i=0; i <= userLocks[_user].length; i++){
            ...
            if(lockDetails[i].lockDuration > block.timestamp){
                LockedAmount += lockDetails[i].lockAmount;
            }  
        }       
    }
    ...
    */
    ...
    function blacklistAccount(address _target, bool _isBlacklisted) external onlyOwner returns(bool) {
    ...
    */  
    ...
    /**
     * @dev overrides the internal transfer function where  user cannot perform any transfer for locked amount and blacklisted user cannot transfer tokens.
    */  
    ...
    function _transfer(
        ...
        require(!isblacklistedAccount[from],"FanTokenV1: Sender is blacklisted");                 // Check if sender is not blacklisted
    ...
    */  
    ...
    function _approve(
        ...
        require(!isblacklistedAccount[owner],"FanTokenV1: approver is blacklisted");                 // Check if sender is not blacklisted
...
contract FanTokenVariableV1 is ERC20Upgradeable, OwnableUpgradeable{
    ...
    */
    event lockCreated(address to, uint256 quantity, uint256 duration);
