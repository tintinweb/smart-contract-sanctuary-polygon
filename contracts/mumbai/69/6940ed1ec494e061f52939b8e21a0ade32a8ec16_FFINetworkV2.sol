...
contract FFINetworkV2 {
    ...
    **/
    ...
    function update_StakingPackage(uint _index,uint256 _price) external {
      require(primaryAdmin==msg.sender, 'Admin what?');
    ...
    Check Weather User Exists Or Not
    **/
    function staking(uint8 package,uint256 _amountOfTokens)
        onlybelievers()
        public
        returns(bool)
    {
        require(package >= 0 && package <= 6, "Invalid Package ?"); 
        require(maticToTokens_(_amountOfTokens) == stakingPackages[package] , "Invalid Package Price ?");
        uint256 tokensMaticValue=maticToTokens_(_amountOfTokens);
        ...
        stakingManagement(_amountOfTokens);
     ...
     * Remember, there's a 10% fee here as well.
    */
    function stakingManagement(uint256 _amountOfTokens)
        onlybelievers ()
        internal
        returns(bool)
    {
        ...
        uint256 _referraldistribution = calculateFee(_staking,30);
        _staking -= _referraldistribution;
        uint256 _taxedTokens = SafeMath.sub(_amountOfTokens,_staking);

        _staking = maticToTokens_(_staking);

        uint256 _tokenFee=_staking;
