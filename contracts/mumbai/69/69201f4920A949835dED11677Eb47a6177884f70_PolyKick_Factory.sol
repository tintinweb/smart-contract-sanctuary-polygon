...
*/
    ...
    function withdrawTokens(address _buyer) external nonReentrant {
    ...
    function returnFunds(address _buyer) external nonReentrant {
        require(isBuyer[_buyer] == true, "Not a Buyer");
        require(
    ...
    function distributeTokensBatch(uint256 startIndex, uint256 endIndex)
        external nonReentrant
    {
        require(success == true, "ILO Failed");
        require(startIndex < endIndex, "Invalid start and end index");
        require(endIndex <= buyersList.length, "End index out of range");

        for (uint256 i = startIndex; i < endIndex; i++) {
            address currentBuyer = buyersList[i];
            if (isBuyer[currentBuyer]) {
                if (allowMintNFT == true) {
                    nftContract.mint(currentBuyer);
                }
                uint256 buyerAmount = buyer[currentBuyer].tokenAmount;
                emit tokenWithdraw(currentBuyer, buyerAmount);
                token.transfer(currentBuyer, buyerAmount);
                buyer[currentBuyer].tokenAmount -= buyerAmount;
                isBuyer[currentBuyer] = false;
            }
        }
    }
