...
contract Gallery is Ownable, ReentrancyGuard {
    ...
    event ListingBid(uint256 indexed id, address indexed bidder, uint256 amount);
    ...
    constructor(address bitflix_, uint256 gracePeriod_) Ownable() {
        bitflix = bitflix_;
    ...
    function _bid(uint256 listingId, uint256 amount) private {
        ...
        emit ListingBid(listingId, msg.sender, amount);
    }
...
contract Collection is ERC721URIStorage, Ownable {
    ...
    event TokenMinted(uint256 indexed tokenId, address artist, address celeb);
    ...
    function _mint(
        ...
        _lastTokenId++;
        _mint(address(gallery), _lastTokenId);
        ...
        emit TokenMinted(_lastTokenId, artist, celeb);
    ...
    function setApprovalForAll(address operator, bool approved) public virtual override {
        require(!gallery.isBlacklisted(operator), "Impossible to approve blacklisted address");
        super.setApprovalForAll(operator, approved);
    }

    function _approve(address to, uint256 tokenId) internal virtual override {
        require(!gallery.isBlacklisted(to), "Impossible to approve blacklisted address");
        super._approve(to, tokenId);
    }

    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        require(!gallery.isBlacklisted(to), "Impossible to transfer to blacklisted address");
        require(isGalleryTransfer(from, to, address(gallery))
                || isGalleryTransfer(from, to, galleryBridge), "Transfer is allowed only over gallery");
        super._transfer(from, to, tokenId);
    }

    function isGalleryTransfer(address _from, address _to, address _gallery) private pure returns (bool) {
