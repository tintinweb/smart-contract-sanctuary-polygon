  ...
  ERC1155BurnableUpgradeable
{
  ...
  function createPack(
    ...
    require(_tokens.length > 0, "Empty");
    ...
    uint256 firstRemaining = _tokens[0].remaining;
    for (uint256 i = 0; i < _tokens.length; i++) {
      if (_packType == PackType.BUNDLE) {
        require(_tokens[i].remaining == firstRemaining, "ITR");
      }
      PackItem memory _item = _tokens[i];
      info.tokens.push(_item);
      info.tokensRemaining += _item.remaining;
      IERC1155Upgradeable(_tokens[i].tokenAddress).safeTransferFrom(
        msg.sender,
        address(this),
        _item.tokenId,
        _item.remaining,
    ...
    if (_packType == PackType.BUNDLE) {
      require(firstRemaining == _maxSupply, "IMS");
    } else {
      require(info.tokensRemaining == _maxSupply, "IMS");
    }
  ...
  function updatePack(
    ...
    _packs[_packId].nonTradeable = _nonTradeable;
    ...
    returns (uint256[] memory tokenIndexes)
  {
    if (pack.packType == PackType.BUNDLE) {
      ...
      for (uint256 i = 0; i < pack.tokens.length; i++) {
        pack.tokens[i].remaining--;
        _tokenIndexes[i] = i;
      }
      return _tokenIndexes;
    } else {
      uint256 offset;
      uint256 _rand = uint256(keccak256(abi.encode(rand())));
      uint256 index = _rand % pack.tokensRemaining;
      uint256[] memory _tokenIndexes = new uint256[](1);

      for (uint256 i = 0; i < pack.tokens.length; i += 1) {
        PackItem memory _token = pack.tokens[i];
        if (_token.remaining > 0 && index <= offset + _token.remaining) {
          pack.tokens[i].remaining--;
          _tokenIndexes[0] = i;
          break;
        } else {
          offset = offset + _token.remaining;
        }
      }
      return _tokenIndexes;
    }
  ...
  function openPack(uint256 _packId) external {
    ...
    if (pack.packType == PackType.LOCKBOX) {
      _burnToken(pack.tokenKey);
    }

    uint256[] memory tokenIndexes = genRewardTokens(pack);
    pack.tokensRemaining = pack.tokensRemaining - tokenIndexes.length;
  ...
  function rand() private view returns (uint256 random) {
    random = uint256(
      keccak256(
        abi.encodePacked(
          msg.sender,
  ...
  function _burnToken(PackItem memory _token) internal {
    ERC1155BurnableUpgradeable _tokenContract = ERC1155BurnableUpgradeable(
      _token.tokenAddress
    );
    require(
      _tokenContract.balanceOf(msg.sender, _token.tokenId) >= 1,
      "No key"
    );
    require(_tokenContract.isApprovedForAll(msg.sender, address(this)), "NA");
    _tokenContract.burn(msg.sender, _token.tokenId, 1);
  }
}
  ...
  SignatureMintableUpgradeable
{
  ...
  modifier beforeMint(MintTokenSignature calldata mintToken) {
    require(mintToken.to == msg.sender, "Not self");

    if (mintToken.currency == address(this)) {
      require(msg.value >= mintToken.cost, "Invalid value");
    }

    for (uint256 i = 0; i < mintToken.tokenIds.length; i++) {
      require(
        _packs[mintToken.tokenIds[i]].minted + mintToken.counts[i] <=
          _packs[mintToken.tokenIds[i]].maxSupply,
        "Sold out"
      );
    }
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      if (from != address(0)) {
        require(!_packs[ids[i]].nonTradeable, "NT");
      }
