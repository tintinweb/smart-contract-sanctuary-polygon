...
contract MetalArmorLogic is Ownable {
    ...
    uint8 public funcCreateArmorAndMetalApe = 15;
    uint8 public funcCombineMetalApe = 16;
    uint8 public funcChangeArmorOnMetalApe = 17;
    ...
    function ChubbyFreeMintArmor(
        ...
        _ChubbyFreeMintSetting.Minted = _ChubbyFreeMintSetting.Minted + _amount;
    ...
    function PayMintArmorByMATIC(
        ...
        _PayMintSetting.Minted = _PayMintSetting.Minted + _amount;
        requestIDs[_requestID] = true;
    ...
    ///#region Mint Metal Ape & Armor
    //Mint Metal Ape & Armor
    function CreateArmorAndMetalApe(
        address[] memory _equipItemAddresses,
        uint256[] memory _tokenList,
        uint256 _amount,
        uint256 _value,
        ...
        uint256 _requestID,
        bytes memory _signature
    ) public returns (uint256[][] memory idss, uint256[] memory metalApeIDs) {
        require(_amount != 0, "Amount count wrong");
        require(_funcType == funcCreateArmorAndMetalApe, "funcType wrong");
        ...
        for (uint16 i = 0; i < _amount; i++) {
            ...
            require(
                _ChubbyFreeMintSetting.exchangeApeCount[_tokenList[i]] <
                    exchangeApeLimit,
            ...
            _ChubbyFreeMintSetting.mintedApeCount[_tokenList[i]] =
                _ChubbyFreeMintSetting.mintedApeCount[_tokenList[i]] +
                1;
        ...
        // mint
        idss = new uint256[][](_amount);
        metalApeIDs = new uint256[](_amount);

        for (uint16 i = 0; i < _amount; i++) {
            // Mint Armor to
            uint256[] memory _itemIds = _ArmorMint(msg.sender);
            idss[i] = _itemIds;

            // Mint Ape
            uint256 metalapeID = _MetalApeMint(msg.sender);
            metalApeIDs[i] = metalapeID;

            // equip

            IERC721MetalAPE(MetalAPE).equipCompleteSetEquipment(
                metalapeID,
                _equipItemAddresses,
                _itemIds
            );
        ...
        return (idss, metalApeIDs);
    }

    ///#endregion Mint Metal Ape & Armor

    ///#region CombineMetalApe
    function CombineMetalApe(
        uint256[] memory _tokenList,
        address[] memory _equipItemAddresses,
        uint256[] memory _itemIds,
        uint256 _amount,
        ...
        uint256 _requestID,
        bytes memory _signature
    ) public returns (uint256[] memory metalApeIDs) {
        require(_amount == 1, "Amount count wrong");
        require(_funcType == funcCreateArmorAndMetalApe, "funcType wrong");
        ...
        // Update exchange info
        for (uint16 i = 0; i < _amount; i++) {
            // Check _itemIds (armors owner)
            require(
                IERC1155MetalARMOR(MetalARMOR).getTokenBalance(
                    _itemIds[i],
                    msg.sender
                ) != 0,
                "Must own Armor token"
            );
    ...
    ///#endregion CombineMetalApe

    ///#region ChangeArmorOnMetalApe
    function ChangeArmorOnMetalApe(
        address[] memory _equipItemAddresses,
        uint256[] memory _equipItemIds,
        string[] memory _unequipItemTypes,
        uint256 _MetalAepID,
        ...
        require(_amount == 1, "Amount count wrong");
        require(_funcType == funcChangeArmorOnMetalApe, "funcType wrong");
        require(!requestIDs[_requestID], "RequestID check failed"); //RequestID check
        require(
            verifySignData(
                keccak256(
        ...
        require(
            IERC721MetalAPE(MetalAPE).ownerOf(_MetalAepID) == msg.sender,
            "Must own Armor token"
        );

        // EQUIPPING/UNEQUIPPING //
        IERC721MetalAPE(MetalAPE).batchChanges(
            _MetalAepID,
            _equipItemAddresses,
            _equipItemIds,
            _unequipItemTypes
    ...
    ///#endregion ChangeArmorOnMetalApe
