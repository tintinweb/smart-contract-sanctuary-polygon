...
contract FactoryERC1155 {
    ...
    */
    function mintERC1155(uint _index, string memory _name, uint256 amount) public {
     ...
     function getIdOwnedbyAccount(uint _index, address account) public view returns (uint id){ 
    	
    	    	return (tokens[_index].getIdOwnedbyAccount(account));
...
contract ERC1155Token is ERC1155, Ownable {
    ...
    */
    function createMapping() private {
     ...
     function getIdOwnedbyAccount(address account) public view returns (uint id) {
    	
    	for (uint i = 0; i < ids.length; i++) { 
    	 if(idToMinter[ids[i]] == account) {
   ...
   return (false);  
       ...
       //for each id
        ...
        // check if the balance of the owner for that Id is 0 meaning the nft of that id 
        // hasn´t been minted
        
        if(balanceOf(owner, ids[id]) == 0) {
        
        	//return the id that's ready to be minted
    	...
    	// if it didn´t find any, return 0 meaning all collection is minted
    	 return (0);
