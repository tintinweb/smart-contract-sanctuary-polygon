...
contract Jewellery is IShop, ERC165{
    ...
    // frontend need to show item media, such as pic/3d/music/video/etc...
	function itemMedia(uint256 id) external view virtual returns (string memory)
    {
        if(id > 0 && id <= _totalItem) {
            return _jewels[id].cid;
        }
        else {
            revert("not found result");
        }
	...
	// function items() external view virtual returns (Product memory)
    // {
    //     return jewels;
    // }
