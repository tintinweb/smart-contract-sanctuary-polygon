...
contract TreasuryHuntCommon is VRFConsumerBaseV2, ReentrancyGuard {
    ...
    event HuntResult(Hunting indexed hunt);
        ...
        VRFConsumerBaseV2(vrfCoordinator)
    {
        ...
        CharacterMinter = ICharacterMinter(
            _CharacterMinter
        );
    ...
    function launchTreasuryHunt(uint256 characterId, uint32 numberOfHunts)
        external
        nonReentrant
        onlyHuman
    {
        ...
        uint64 totalStats = uint64(
            _checkBrothelJob(
                characterId,
                (characterInfos.boarding +
                    characterInfos.sailing +
                    characterInfos.charisma)
            )
        );
    ...
    function ClaimTreasuryHunt(uint256 characterId)
        external
        nonReentrant
        onlyHuman
    {
    ...
    function _bonusFLAG(uint256 randomWord, Hunting memory huntings)
        internal
        returns (bool)
    {
    ...
    function _checkBrothelJob(uint256 characterId, uint256 totalStats)
        internal
        returns (uint256)
    {
        uint256 result;
    ...
    function getToggleMint(address user, uint256 tokenId)
        external
        view
        returns (bool)
    {
