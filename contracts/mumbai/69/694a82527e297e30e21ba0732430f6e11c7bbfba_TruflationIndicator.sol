...
contract TruflationIndicator is ChainlinkClient, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize(Wagies nft) external initializer {
        ...
        _requiredDifference = 0.5 ether; // 0.5%
...
contract Wagies is ERC721AUpgradeable, ERC721AQueryableUpgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function truflationIndicatorEnable(uint256 difference) external onlyIndicator(truflationIndicator) {
    ...
    function ethPriceIndicatorEnable(uint256 difference) external onlyIndicator(ethPriceIndicator) {
        uint256 periodEnd = mintSettings._periodEnd;
        bool resetMints = (block.timestamp + PERIOD_RESET_TIME) >= periodEnd;
    ...
    function _mintNoChecks(uint256 amount) external {
        _mint(msg.sender, amount);
    }
...
contract ETHPriceIndicator is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _checkDifference(uint256 currentPrice) internal {
        ...
        if (difference >= _requiredDifference) {
            _nft.ethPriceIndicatorEnable(difference);
            maxRecordedPrice = currentPrice;
        }
    }
