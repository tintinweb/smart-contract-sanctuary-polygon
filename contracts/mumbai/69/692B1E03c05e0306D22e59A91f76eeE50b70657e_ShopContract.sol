...
contract ShopContract is Ownable, KeeperCompatibleInterface {
    ...
    function settlePayment(uint256 paymentEntryId) payable public{
        ...
        require(DAI.approve(address(uniswapV2Router), 10000), "approve failed");

        address[] memory path = new address[](2);
        path[0] = uniswapV2Router.WETH();
        path[1] = address(DAI);

        uniswapV2Router.swapExactETHForTokens{value: 10000}(0, path, msg.sender, block.timestamp+10000);

        emit paymentSettled(freeSettledPaymentId - 1);
        emit test(uniswapV2Router.WETH());
    }
