...
contract DeerClubExclusivePass is Ownable, ERC721Enumerable, BridgeNFT, MinterRole {
    ...
    function mint(address to, uint256 id, bytes calldata) override external onlyMinter {
        ...
        tokenMintInfo[id] = tokenMintInfos.length - 1;
    }
     ...
     */
    function setTierInfo(uint16[] memory maxSupplies, uint16[] memory startIndexes) external onlyOwner {
    ...
    /**
     * @dev Set each tokenId's metadata uri's index.
     */
    function setMetaDataIndex(uint16[] memory tokenIds, uint16[] memory uriIndexs) external onlyOwner {
        require(tokenIds.length == uriIndexs.length, "Wrong values");
    ...
    function tokenURI(uint256 tokenId) 
        ...
        return bytes(baseUri).length != 0 ? string(abi.encodePacked(baseUri, metadataUri[tokenId].toString(), BASE_EXTENSION)) : '';
    }
    ...
    */
    function getTokenMintInfo(uint tokenId) public view returns (TokenMintInfo memory) {
        require(_exists(tokenId), "Mint Info query for nonexistent token");
    ...
    */
    function getTiersOfTokenIds(uint[] memory tokenIds) public view returns (uint[] memory) {
        ...
        for (uint i = 0; i < tokenLength; i++) {
            tokenId = tokenIds[i];
            require(_exists(tokenId), "Tier Info query for nonexistent token");

            if (tiers[1].startingIndex <= metadataUri[tokenId] && 
                tiers[2].startingIndex > metadataUri[tokenId]) {
                tiersOfTokenIds[i] = 1;
            }
            else if (tiers[2].startingIndex <= metadataUri[tokenId] && 
                     tiers[3].startingIndex > metadataUri[tokenId]) {
                tiersOfTokenIds[i] = 2;
            }
            else if (tiers[3].startingIndex <= metadataUri[tokenId]) {
                tiersOfTokenIds[i] = 3;
            }
        }
