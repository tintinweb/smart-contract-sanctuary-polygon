...
contract LostWorldData is CustomERC2771ContextUpgradeable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getChargeAmount(uint256 landId) public view returns (uint8) {
        require(rechargeAmount[landId].initialized, 'LWD: Land is not initialized');
        return rechargeAmount[landId].chargeAmount;
    }
    ...
    function getEnemyRarity(uint256 enemyId) external view returns (uint8, string memory) {
        ...
        string memory enemyRarity;

        uint8 rarityIndex = 0;
        if (enemy.cleaveChance <= 2) {
            rarityIndex = 1; // Common
            enemyRarity = 'common';
        } else if (enemy.cleaveChance > 3 && enemy.cleaveChance <= 5) {
            rarityIndex = 2; // Rare
            enemyRarity = 'rare';
        } else if (enemy.cleaveChance > 6 && enemy.cleaveChance <= 8) {
            rarityIndex = 3; // Epic
            enemyRarity = 'epic';
        } else if (enemy.cleaveChance > 8) {
            rarityIndex = 4;
            enemyRarity = 'mythical';
        } else revert('LWD: Enemy unknown rarity');

        return (rarityIndex, enemyRarity);
    }
