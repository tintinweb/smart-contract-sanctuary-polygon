...
contract RagnarokConsumable is
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    uint256 public constant EXPEDITION_CHEST = 1;
    uint256 public constant COLLAB_CHEST = 2;
    uint256 public constant OG_CHEST = 3;
    uint256 public constant BOSS_CHEST = 4;
    uint256 public constant ELITE_CHEST = 5;
    uint256 public constant EXP_CHEST_1 = 101;
    uint256 public constant EXP_CHEST_2 = 102;
    uint256 public constant EXP_CHEST_3 = 103;
    uint256 public constant EXP_CHEST_4 = 104;

    mapping(address => bool) private _auth;
    // V2
    address public randomOracleAddress;
    address public skullAddress;
    address public freyjaTearAddress;
    address public freyjaRelicAddress;
    address public itemsAddress;
    mapping(address => BlessingEvent) private _blessingEvents;
    uint16[16] public chestDropProbabilities;
    mapping(uint256 => uint16[16]) private _gearDropProbabilitesByChestId;
    address public beastAddress;

    function initialize() public initializer {
        __ERC1155_init("https://api.ragnarokonchain.io/api/consumables/");
    ...
    function mint(
        ...
        require(_auth[msg.sender], "not allowed");
        _mint(to, id, amount, "");
    }
    ...
    function useExperienceCrates(
        uint16 id,
        uint16 amount,
        uint256 beastId
    ) external {
        require(id > 100 && id < 200, "only exp crates");
        require(balanceOf(msg.sender, id) >= amount, "invalid amount");
        _burn(msg.sender, id, amount);
        IBeast(beastAddress).giveExperience(beastId, ((id - 100)**2) * amount * uint32(10000));
    }

    function blessChests(uint16 id, uint16 amount) external {
        require(id < 100, "only drop chests");
        require(_blessingEvents[msg.sender].id == 0, "already blessing");
        require(balanceOf(msg.sender, id) >= amount, "not enough chests");
        uint256 requiredTears = amount * 10;
        IRagnarokERC20(freyjaTearAddress).burnFrom(msg.sender, requiredTears * 1 ether);
        bytes32 requestId = IRandomOracle(randomOracleAddress).requestRandomNumber();
        _blessingEvents[msg.sender] = BlessingEvent({
            id: id,
            amount: amount,
            requestId: requestId,
            revealBlock: block.number + 60
        });
    }

    function openChests() external {
        BlessingEvent memory blessingEvent = _blessingEvents[msg.sender];
        uint256 randomSeed = IRandomOracle(randomOracleAddress).getRandomNumber(blessingEvent.requestId);
        require(blessingEvent.revealBlock < block.number, "not blessed");
        require(randomSeed > 0, "random seed not ready");
        delete _blessingEvents[msg.sender];
        _burn(msg.sender, blessingEvent.id, blessingEvent.amount);
        _mintChestsDrops(blessingEvent.amount, randomSeed, blessingEvent.id);
    }

    function previewChests() external view returns (uint8[] memory, uint256[] memory) {
        BlessingEvent memory blessingEvent = _blessingEvents[msg.sender];
        uint256 randomSeed = IRandomOracle(randomOracleAddress).getRandomNumber(blessingEvent.requestId);
        require(blessingEvent.revealBlock < block.number, "not blessed");
        require(randomSeed > 0, "random seed not ready");
        return _getChestsDrops(blessingEvent.amount, randomSeed, blessingEvent.id);
    }

    function _getChestsDrops(
        uint16 amount,
        uint256 randomSeed,
        uint256 id
    ) internal view returns (uint8[] memory, uint256[] memory) {
        uint8[] memory drops = new uint8[](amount);
        uint256[] memory rewards = new uint256[](amount);
        for (uint256 index = 0; index < amount; index++) {
            uint256 randomNumber = _getRandomNumber(randomSeed, index, 100);
            uint8 drop = id == ELITE_CHEST ? 0 : _rollRarity(randomNumber, chestDropProbabilities);
            drops[index] = drop;
            if (drop == 0) {
                // mintGear
                rewards[index] = _rollItemId(id, randomSeed, index);
            } else if (drop == 1) {
                // mintTears
                rewards[index] = 60 ether;
            } else if (drop == 2) {
                // mint eye
                rewards[index] = 4;
            } else if (drop == 3) {
    ...
    function _mintChestsDrops(
        ...
        for (uint256 index = 0; index < amount; index++) {
            ...
            if (drop == 0) {
                // mintGear
                IRagnarokItem(itemsAddress).mint(msg.sender, _rollItemId(id, randomSeed, index));
            } else if (drop == 1) {
                // mintTears
                IRagnarokERC20(freyjaTearAddress).mint(msg.sender, 60 ether);
            } else if (drop == 2) {
                // mint eye
                IFreyjaRelic(freyjaRelicAddress).mint(msg.sender, 4, 1);
            } else if (drop == 3) {
                // mint blood
                IFreyjaRelic(freyjaRelicAddress).mint(msg.sender, 5, 1);
            } else if (drop == 4) {
                // mint skull
                IRagnarokERC20(skullAddress).mint(msg.sender, 30 ether);
            } else {
                revert("unknown drop");
            }
        }
    ...
    function _rollItemId(
        uint256 id,
        uint256 randomSeed,
        uint256 nonce
    ) internal view returns (uint16) {
        // SWAP nonce and randomSeed to generate a different number from chest drop
        uint256 randomNumber = _getRandomNumber(nonce, randomSeed, 100);
        uint8 rarity = _rollRarity(randomNumber, _gearDropProbabilitesByChestId[id]) + 1;
        uint16[] memory items = IRagnarokItem(itemsAddress).getItemsByRarity(rarity);
        uint256 index = _getRandomNumber(nonce + 1, randomSeed, items.length);
        return items[index];
    }

    function _rollRarity(uint256 randomNumber, uint16[16] memory probabilties) internal pure returns (uint8) {
        uint256 accPercentage = 0;
        for (uint8 index = 0; index < probabilties.length; index++) {
            uint256 currentPercentage = probabilties[index];
            if (randomNumber >= accPercentage && randomNumber < accPercentage + currentPercentage) {
                return index;
            }
            accPercentage += currentPercentage;
        }
        revert("rarity roll failed");
    }
    ...
    // GETTER FUNCTIONS
    function getBlessingEvent(address owner) external view returns (BlessingEvent memory) {
        return _blessingEvents[owner];
    }

    function uri(uint256 id) public view override returns (string memory) {
        return string(abi.encodePacked(super.uri(id), StringsUpgradeable.toString(id)));
    }

    function getGearDropProbabilities(uint256 id) external view returns (uint16[16] memory) {
        return _gearDropProbabilitesByChestId[id];
    }
    ...
    function setAuth(address _address, bool value) external onlyOwner {
        _auth[_address] = value;
    }

    function setAddresses(
        address _randomOracleAddress,
        address _skullAddress,
        address _freyjaTearAddress,
        address _freyjaRelicAddress,
        address _itemsAddress
    ) external onlyOwner {
        randomOracleAddress = _randomOracleAddress;
        skullAddress = _skullAddress;
        freyjaTearAddress = _freyjaTearAddress;
        freyjaRelicAddress = _freyjaRelicAddress;
        itemsAddress = _itemsAddress;
    }

    function setBeastAddress(address _beastAddress) external onlyOwner {
        beastAddress = _beastAddress;
    }

    function setChestDropProbabilities(uint16[16] memory drops) external onlyOwner {
        chestDropProbabilities = drops;
    }

    function setGearDropProbabilities(uint256 id, uint16[16] memory drops) external onlyOwner {
        _gearDropProbabilitesByChestId[id] = drops;
    }
