...
contract Co2Certificates is ERC721Enumerable, Ownable {
//   using Strings for uint256;
   using Strings for *;
  ...
  function buildImage(uint256 _tokenId) public view returns(string memory) {
      ...
      return Base64.encode(bytes(
          abi.encodePacked(
              ...
              '<text x="50%" y="50%" dominant-baseline="middle" fill="hsl(',currentWord.textHue,', 100%, 80%)" text-anchor="middle" font-size="15">',string.concat("Current Token id:", Strings.toString(_tokenId)),'</text>',
              '<text x="50%" y="25%" dominant-baseline="bottom" fill="hsl(',currentWord.textHue,', 100%, 80%)" text-anchor="middle" font-size="15">',Strings.toHexString(uint160(msg.sender), 20),'</text>',
              '</svg>'
          )
      ));
  }

  function buildImage2() pure public  returns (string memory){
    string memory  test= string.concat("Reward for token id ", Strings.toString(100), " is not available");
    return test;
  }
  function buildMetadata2(uint256 tokenId)public view returns(string memory) {
  ...
  function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
      ...
      return buildMetadata2(_tokenId);
  }
