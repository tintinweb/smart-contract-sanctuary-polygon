...
/**
 * @title AssetPool - A contract to hold available Resources and GameItems in the PV Gaming Ecosystem
 * 
 * @author Jack Chuma
 */
contract AssetPool is IAssetPool, AccessControl, ERC1155Holder {

    bytes32 constant USER_ROLE = keccak256("USER_ROLE");

    address crafting;
    uint256 powCoefficient;
    ...
    constructor(address _pow, uint256 _powCoefficient, address _adminWallet) {
        if (
            _pow == address(0) || 
     ...
     */
    function withdrawPOW(
        address _to, 
        uint256 _amount
    ) external onlyRole(USER_ROLE) {
        pow.transfer(_to, _amount);
    }
    ...
    function calcResourcesOutFromPOWIn(
        ...
        if (
            _resourceIds.length != _powSplitUp.length ||
            _resourceIds.length != _minAmountsOut.length
        ) revert LengthMismatch();

        uint256 _totalPOWUsed;
        uint256 _currentPowBal = pow.balanceOf(address(this));
        uint256[] memory _amounts = new uint256[](_resourceIds.length);
        uint256[] memory _idList = new uint256[](_resourceIds.length);

        for (uint256 i = 0; i < _resourceIds.length; ) {
            ...
            _amounts[i] = Balancer.outGivenIn(
                ...
                powCoefficient + _currentPowBal + _totalPOWUsed, //balanceI
                _amountI
            );
    ...
    function calcPOWOutFromResourcesIn(
        ...
        if (_resourceIds.length != _amountsIn.length) revert LengthMismatch();
        uint256[] memory _idList = new uint256[](_resourceIds.length);
        uint256 _currentPowBal = pow.balanceOf(address(this));
        ...
        for (uint256 i = 0; i < _resourceIds.length; ) {
            ...
            uint256 _amountO = Balancer.outGivenIn(
                _currentPowBal + powCoefficient - _amountPOW, // balanceO
    ...
    function calcPOWInFromResourcesOut(
        ...
        if (_resourceIds.length != _amountsOut.length) revert LengthMismatch();
        uint256[] memory _idList = new uint256[](_resourceIds.length);
        uint256 _currentPowBal = pow.balanceOf(address(this));
        ...
        for (uint256 i = 0; i < _resourceIds.length; ) {
            ...
            uint256 _amountI = Balancer.inGivenOut(
                _currentPowBal + powCoefficient + _powIn, // balanceI
                resources.balanceOf(address(this), _id), // balanceO
        ...
        returns (bool) 
    {
        return interfaceId == type(IERC1155Receiver).interfaceId || 
               ...
               interfaceId == type(IAssetPool).interfaceId;
    }
...
library Balancer {
    ...
    **********************************************************************************/
    function outGivenIn(
        ...
        uint256 balanceI,
        uint256 amountI
    ) external pure returns (uint256 amountO) {
