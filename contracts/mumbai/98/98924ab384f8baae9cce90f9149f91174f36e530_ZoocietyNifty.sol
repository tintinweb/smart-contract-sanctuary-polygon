    ...
    IERC4907
{
    ...
    constructor() ERC721("Zoociety Nifty", "NIFTY") {
        newAccessPass(0, "NIFTILIATE", 0, 365);
        newAccessPass(1, "AIMAGE", 10, 30);
        newAccessPass(2, "TOOLKAITS", 10, 30);
        newAccessPass(3, "SUPERPROMPTS", 10, 365);
    ...
    function mintMultiAccessPass(
        ...
        if (isAffiliate[_affiliate]) {
            uint256 reward = msg.value.mul(percentRewards[_affiliate]).div(100);

            if (invitedBy[_affiliate] == address(0)) {
                _affiliate.transfer(reward);
            } else {
                uint256 rreward = reward.mul(percentRewards[_affiliate]).div(
                    100
                );
