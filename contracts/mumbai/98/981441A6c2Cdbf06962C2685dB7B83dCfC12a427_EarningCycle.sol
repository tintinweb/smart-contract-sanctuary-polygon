...
contract EarningCycle is Ownable, ReentrancyGuard {
    ...
    address public uplineAddr = 0x1aE0EA34a72D944a8C7603FfB3eC30a6669E454C;
    address public distributeAddr = 0x4B0897b0513fdC7C541B6d9D7E929C4e5364D2dB;
    address public devAddr = 0xdD870fA1b7C4700F2BD7f44238821C26f7392148;
    ...
    constructor() {
        amount.depositAmt = 2000 * 1e6;
        amount.upgradeAmt2 = 4000 * 1e6;
        amount.upgradeAmt3 = 8000 * 1e6;
        amount.upgradeAmt4 = 16000 * 1e6;
        amount.upgradeAmt5 = 32000 * 1e6;
    ...
    function deposit(address refAddr) public nonReentrant {
        ...
        totalDeposits = totalDeposits.add(amount.depositAmt);
    ...
    function upgrade2() private {
        ...
        totalDeposits = totalDeposits.add(amount.upgradeAmt2);
    ...
    function upgrade3() private {
        ...
        totalDeposits = totalDeposits.add(amount.upgradeAmt3);
    ...
    function upgrade4() private {
        ...
        totalDeposits = totalDeposits.add(amount.upgradeAmt4);
    ...
    function upgrade5() private {
        ...
        totalDeposits = totalDeposits.add(amount.upgradeAmt5);
    ...
    */
    function limitLevels() public onlyOwner {
        for (uint i = 0; i < userAddrs.length; i++) {
            if (users[userAddrs[i]].levelStatus.upgraded4 && block.timestamp.sub(users[userAddrs[i]].lastTime) >= 160 days) {
                users[userAddrs[i]].levelStatus.upgraded4 = false;
            } else if (users[userAddrs[i]].levelStatus.upgraded3 && block.timestamp.sub(users[userAddrs[i]].lastTime) >= 120 days) {
                users[userAddrs[i]].levelStatus.upgraded3 = false;
            } else if (users[userAddrs[i]].levelStatus.upgraded2 && block.timestamp.sub(users[userAddrs[i]].lastTime) >= 90 days) {
                users[userAddrs[i]].levelStatus.upgraded2 = false;
            } else if (users[userAddrs[i]].levelStatus.deposited && block.timestamp.sub(users[userAddrs[i]].lastTime) >= 60 days) {
                users[userAddrs[i]].levelStatus.deposited = false;
            }
        }
    }
