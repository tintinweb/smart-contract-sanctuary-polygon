...
contract ShibaReferralsVault is IShibaReferralsVault, ReentrancyGuard, Pausable, Ownable {
    //---------- Libraries ----------//
    using SafeMath for uint256;
    using Counters for Counters.Counter;

    //---------- Contracts ----------//
    IERC20 private CHIP; // SWC token contract.
    address private ReferralsHub;

    //---------- Variables ----------//
    Counters.Counter public totalHolders; // Total wallets in pool.
    uint256 public totalPoints; // Total points in pool.
    ...
    //---------- Events -----------//
    event Deposit(uint256 amount, uint256 totalPoints);
    ...
    //---------- Modifiers ------------//
    modifier onlyReferralsHub() {
        require(_msgSender() == ReferralsHub);
        _;
    }

    //----------- Internal Functions -----------//
    /**
     * @dev Distribute CHIPs.
     * @param amount_ amount of CHIPs to distribute.
     */
    function _disburse(uint256 amount_) internal {
        if (totalPoints > 0 && amount_ >= 1000000) {
            pointsXtoken += (amount_.mul(10e18)).div(totalPoints);
            unclaimedTokens += amount_;
            emit Deposit(amount_, totalPoints);
        }
    }

    /**
     * @dev Calculates the CHIPs pending distribution and distributes them if there is an undistributed amount.
     */
    ...
    function _getRewards(address wallet_) internal view returns (uint256) {
        ...
        return
            (stakeHolders[wallet_].stakedPoints.mul(newTokenPoints)).div(10e18);
    }
    ...
    function _initStake(address wallet_, uint256 amount_) internal virtual {
        ...
        stakeHolders[wallet_].stakedPoints = amount_;
        totalPoints += amount_;
    ...
    function _getPendingBal(address wallet_) internal view returns (uint256) {
        ...
        return
            (stakeHolders[wallet_].stakedPoints.mul(newTokenPoints))
                .div(10e18)
                .add(pending);
    }
    ...
    function isInPool(address wallet_) public view returns (bool) {
        return stakeHolders[wallet_].stakedPoints > 0;
    }
            ...
            uint256 pending = balance.sub(processed);
            {
                if (pending > 1000000) {
                    ...
                    uint256 oldTokenPoints = pointsXtoken.add(
                        (pending.mul(10e18)).div(totalPoints)
                    );
                    ...
                    uint256 unprocessedBal = (
                        stakeHolders[holder].stakedPoints.mul(newTokenPoints)
                    ).div(10e18);
    ...
    {
        Wallet storage w = stakeHolders[wallet_];
        return (w.stakedPoints, w.startTime);
    }

    /**
     * @notice Add points to receive rewards.
     * @param _points Amount of points to add.
     * @param _acount Address of referent.
     */
    function addPoints(address _acount, uint256 _points)
        external
        override
        onlyReferralsHub
    {
        require(_points > 0, "Vault: zero points");
        require(_acount != address(0), "Vault: zero address");
        if (isInPool(_acount)) {
            _addStake(_acount, _points);
        } else {
            _initStake(_acount, _points);
        }
        emit AddedPoints(_acount, _points);
    }

    /**
     * @notice Change wallet to new wallet.
     * @param _oldAcount Address of old referent.
     * @param _newAcount Address of new referent.
     */
    function migrateWallet(address _oldAcount, address _newAcount)
        external
        override
        onlyReferralsHub
    {
        require(isInPool(_oldAcount), "Vault: Not in pool");
        require(_newAcount != address(0), "Vault: zero address");
        _recalculateBalances();
        _harvest(_oldAcount);
        Wallet memory w = stakeHolders[_oldAcount];
        delete stakeHolders[_oldAcount];
        stakeHolders[_newAcount] = w;
        emit WalletMigrated(_oldAcount, _newAcount);
    }
    ...
    /**
     * @notice Remove wallet from pool.
     */
    function removePoints(address _acount) external onlyOwner {
        require(isInPool(_acount), "Not in pool");
        _harvest(_acount);
        uint256 stakedBal = stakeHolders[_acount].stakedPoints;
        totalPoints -= stakedBal;
        delete stakeHolders[_acount];
        totalHolders.decrement();
        emit RemovedPoints(_acount, stakedBal);
    }
    ...
    function getInvalidTokens(address to_, address token_) external onlyOwner {
        require(to_ != address(0x0), "Zero address");
        require(token_ != address(CHIP), "Invalid token");
        uint256 balance = IERC20(token_).balanceOf(address(this));
        IERC20(token_).transfer(to_, balance);
    }

    /**
     * @notice Update referral hub.
     */
    function setReferralHub(address _referralHub) external onlyOwner {
        require(_referralHub != address(0), "zero address");
        ReferralsHub = _referralHub;
    }
