  ...
  Ownable
{
  ...
  constructor() ERC721('ExampleNFT', 'SFT') {}
  ...
  function getAllExistingTokenIds() external view returns (uint256[] memory) {
    uint256 existingIdCounter = 0;

    for (uint256 i = 0; i < totalSupply(); i++) {
      if (_exists(tokenByIndex(i))) {
        existingIdCounter++;
      }
    }

    uint256[] memory result = new uint256[](existingIdCounter);

    uint256 nextResultPosition = 0;
    for (uint256 i = 0; i < totalSupply(); i++) {
      if (_exists(tokenByIndex(i))) {
        result[nextResultPosition] = tokenByIndex(i);
        nextResultPosition++;
      }
    }
  ...
  function getTokenIdsForUser(address from)
    external
    view
    returns (uint256[] memory)
  {
    uint256 balance = ERC721.balanceOf(from);
