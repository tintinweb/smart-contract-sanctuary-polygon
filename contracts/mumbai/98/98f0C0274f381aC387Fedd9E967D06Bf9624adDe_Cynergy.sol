...
contract Cynergy is ERC20Burnable {
  ...
  constructor() ERC20('Cynergy', 'Cynergy') {
    owner = msg.sender;
    _mint(msg.sender, 1000 * 10**super.decimals());
  }

  function claim(uint256 amount, bytes memory signature) external {
    bytes32 hashed = abi.encodePacked('cynergy:claim:', amount.toString()).toEthSignedMessageHash();
