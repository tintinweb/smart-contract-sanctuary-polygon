...
contract RevenueShare is AccessControl, ReentrancyGuard{
  ...
  mapping(uint256=>Request) public receiveIdToRequest;
  struct Request {
      bool isWETH;
      uint256 tokenId;
      uint256 amount;
   ...
   * Event
   */
  event ReceivedETH(uint256 receiveId, uint256 amount);
  event ReceivedWETH(uint256 receiveId, uint256 amount);
  event RequestAdded(uint256 receiveId, bool isWETH, uint tokenId, uint256 amount, address[] m);
  ...
  function addRequest(uint256 _receiveId, uint tokenId, address[] memory m) external onlyRole(DEFAULT_ADMIN_ROLE) {
    _updateReceivedWETH();
    Payment memory payment = receiveIdToPayments[_receiveId];
    bool isWETH = payment.isWETH;
    if(payment.amount == 0) revert ZeroAmountRequest();
    //amount is given by receivedIdToPayments
    Request memory request =  Request(isWETH, tokenId, payment.amount, m);
    receiveIdToRequest[_receiveId] = request;
    ...
    emit RequestAdded(_receiveId, isWETH, tokenId, payment.amount, m);
  }
  ...
  function batchWithdraw() external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    ...
    // To get the wallet list: receiveIdToRequest[receiveId(itterable)].members
    for(uint i=withdrawnId; i<=receiveId; i++) {
      address[] memory m = receiveIdToRequest[i].members;
  ...
  function withdrawETH(address account, uint amount) external payable nonReentrant{
  ...
  function withdrawWETH(address account, uint amount) external payable nonReentrant{
