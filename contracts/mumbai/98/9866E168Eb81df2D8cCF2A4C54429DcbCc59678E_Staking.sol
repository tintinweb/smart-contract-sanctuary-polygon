...
contract Staking {
    ...
    function setLiquidityAddress(address token) public whenPermited {
    ...
    function setAdelantarTiempo(uint value) public whenPermited {
        adelantarTiempo=value;
    }
    ...
    function unstake(uint posicion) public {
        ...
        require((block.timestamp-(staked.time-adelantarTiempo)) > (seconds_month*staked.tipo),"Error: Could not unstake yet");
        ...
        if(posicion==last){
            delete wallet_staked_[msg.sender][last];
        }else{
    ...
    function stake(uint amount,uint tipo) public whenNotPaused {
        ...
        require(viability>5000000,"Error: Forbidden stake");
