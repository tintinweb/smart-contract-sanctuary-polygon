...
contract nftSharing {
    ...
    function depositNft(address _nftAddress,uint _nftId) public payable returns(address){
        ...
        //deploy contract
        address nftShareTokenContractAddress = deployNftShareToken(_nftAddress,_nftId);

        //receive Nft
        IERC721 nftContract = IERC721(_nftAddress);
        nftContract.transferFrom(msg.sender,nftShareTokenContractAddress,_nftId);
        

        //add 10% of token to liquidity in swap 
        nftShareToken nftShareTokenContract = nftShareToken(nftShareTokenContractAddress);
        nftShareTokenContract.approve(routerContractAddress,initLiquidityTokenAmount);
        IRouter routerContract = IRouter(routerContractAddress);
        routerContract.addLiquidityETH{value:msg.value}(
            nftShareTokenContractAddress,
        ...
        nftShareTokenContract.transfer(feeTo,feeTokenAmount);
        nftShareTokenContract.transfer(msg.sender,nftShareTokenContract.balanceOf(address(this)));

        // get lptoken contract address
        address factoryContractAddress = routerContract.factory();
        address WETH = routerContract.WETH();
        IFactory factoryContract = IFactory(factoryContractAddress);
        address pairContractAddress = factoryContract.getPair(WETH,nftShareTokenContractAddress);

        //set lptoken address to nftShareTokenContract
        nftShareTokenContract.setPairContractAddress(pairContractAddress);
        
        //change nft contract owner
        nftShareTokenContract.setOwner(owner);

        return nftShareTokenContractAddress;
    }
