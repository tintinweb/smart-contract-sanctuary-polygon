...
contract StakingContract is Initializable, ReentrancyGuardUpgradeable, UUPSUpgradeable , OwnableUpgradeable{
    ...
    // ) 
    // initializer public {
    //      durations = [7 minutes, 30 minutes, 60 minutes , 120 minutes];
...
event StakeRecord(uint256 stakingCount , uint256 stakeAmount ,uint256 stakingPlan ,uint256 withDrawTime , uint256 rewardCalculations, address stakerAddress);
...
function staking(uint256 amount, uint256 plan ) external nonReentrant  {
        ...
        user.stakerecord[user.stakeCount].bonus , msg.sender);

}

    function withdraw(uint count) public{
    User storage user = users[msg.sender];
    ...
    if(block.timestamp >= user.stakerecord[count].withdrawTime){
        stakingToken.transferFrom(ownerAddress,msg.sender,user.stakerecord[count].amount);
        rewardsToken.transferFrom(ownerAddress,msg.sender,user.stakerecord[count].bonus);
...
function rewardCalculate(uint256 plan) public view returns(uint256 mm){
    User storage user = users[msg.sender];
        if (plan == 0){
            return calculateReward(user.stakerecord[user.stakeCount].amount,475);
        }else if (plan == 1){
            return calculateReward(user.stakerecord[user.stakeCount].amount,675);
        }else if(plan == 2){
            return calculateReward(user.stakerecord[user.stakeCount].amount,875);
        }
        else if(plan == 3){
            return calculateReward(user.stakerecord[user.stakeCount].amount,975);
        }
   }
   
  function calculateReward(uint256 _amount,uint256 percentage) public pure returns(uint256){
