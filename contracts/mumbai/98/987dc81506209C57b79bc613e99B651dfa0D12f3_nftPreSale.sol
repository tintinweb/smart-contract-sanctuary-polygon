...
contract nftPreSale is Ownable {
    ...
    function blindReferer(address _referer) external {
        ...
        if (!allRefererSet.contains(_referer)) {
            allRefererSet.add(_referer);
        }
        if (directReferralUsersList[_referer].length() >= 10 && !allShareRefererSet.contains(_referer)) {
    ...
    function buyNft(uint256 _amount) external {
        ...
        //不能超过总量
        require(config.idoAmount.add(_amount) <= config.totalAmount, "t005");
        //每个人不能超过10个
    ...
    //获取用户的直推客户的数量和列表
    function getDirectReferralUsersList(address _user) external view returns (uint256 _num, address[] memory _addressList) {
        _num = directReferralUsersList[_user].length();
        _addressList = directReferralUsersList[_user].values();
    }

    //获取用户的直推客户的数量和列表
    function getDirectReferralUsersListByIndexList(address _user, uint256[] memory _indexList) external view returns (uint256 _num, address[] memory _addressList) {
        _num = _indexList.length;
        _addressList = new address[](_num);
        for (uint256 i = 0; i < _num; i++) {
            uint256 _index = _indexList[i];
    ...
    function getAllInfo(address _user) external view returns (Item.returnStruct memory _returnList) {
        //ido配置信息
