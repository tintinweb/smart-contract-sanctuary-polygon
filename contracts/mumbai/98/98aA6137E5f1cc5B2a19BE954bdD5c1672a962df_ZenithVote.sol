...
contract ZenithVote is Context, ReentrancyGuard{
    ...
    address immutable public zta;
    address immutable public ztb;
    ...
    constructor(
        ...
        require(
            assetToken.isERC20() && voteToken.isERC20(),
            "ZenithVote : assetToken or/and voteToken address is not ERC20!"
        );
        zta = assetToken;
        ztb = voteToken;
    }
    ...
    function claimVote(
        ...
        require(
            getVoteData(voteId).end < block.timestamp,
            "ZenithVote : Please wait until vote period ended!"
        );
        require(
            getVoteData(voteId).owner == _msgSender(),
            "ZenithVote : Only vote owner allowed!"
        );
        require(
            getVoteData(voteId).claimed == false,
            "ZenithVote : Owner already claim!"
        );
        ...
        if(
            ...
            unchecked{
                ...
                for(uint256 a; a < votedData.length; a++){
                    totalToken += votedData[a] * getVoteData(
                        voteId
                    ).amountVote;
                }

                IERC20(ztb).safeTransfer(
                    _msgSender(),
                    totalToken
                );
            }
    ...
    function userWantVote(
        ...
        require(
            getVoteData(voteId).claimed == false,
            "ZenithVote : This vote already claimed!"
        );
        require(
            getVoteData(voteId).end >= block.timestamp,
        ...
        if(getVoteData(voteId).amountHold > 0){
            require(
                IERC20(zta).balanceOf(_msgSender()) >= getVoteData(voteId).amountHold,
                "ZenithVote : Minimum holding is required!"
            );
        }

        if(getVoteData(voteId).amountVote > 0){
            IERC20(ztb).safeTransferFrom(
                _msgSender(),
                address(this),
                getVoteData(voteId).amountVote
            );
        }
        ...
        totalUserVote[voteId][userChoose] += 1;
    ...
    function getTotalUserVote(
        ...
        uint256 getLength = getVoteData(voteId).choice.length;
