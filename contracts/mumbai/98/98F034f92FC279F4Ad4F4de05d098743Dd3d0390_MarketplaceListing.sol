...
contract MarketplaceListing is Ownable, MarketplaceListingHelper {
    ...
    function getActiveListings() external view returns (uint256[] memory) {
     ...
     function getActiveOfferss() external view returns (uint256[] memory) {
    ...
    function getOfferByOfferId(uint256 _offerId)
        external
        view
        returns (Offer memory){
        return offerIdToOffer[_offerId];
    }
    ...
    //     //TODO
    //     require(IERC721(listing.nftAddress).ownerOf(listing.tokenId) == listing.seller, "You must own the token");

    //     require(IERC721(listing.nftAddress).isApprovedForAll(listing.seller, address(this)),"Must approve this contract for transfers.");
