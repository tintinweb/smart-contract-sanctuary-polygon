...
contract FinanceClaim is IFinanceClaim, Ownable {
    ...
    function addPoolRewards(uint256[] calldata poolsIdxs, address[] calldata rewards) external onlyFinOps {
        ...
        for(uint i = 0; i < poolsIdxs.length; i++) {
    ...
    function setFundsClaimable(uint256 poolIndex, address[] calldata users, uint256[] calldata amounts) external onlyOwner isValidPoolIndex(poolIndex) {
        ...
        for(uint256 i = 0; i < users.length; i++) {
            ...
            pools[poolIndex].validUserClaims[users[i]] = keccak256(abi.encodePacked(users[i], poolIndex, pools[poolIndex].latestWithdrawalTs));
        }
    ...
    /// @notice Claims all the left funds in a pool. This is to be called by FinOps in case users don't claim their funds on a long period of time
    /// @dev Resets the valid claim timestamp of the pool, so all users who have not claimed their funds until then, will not be able to claim them anymore
    /// @dev only triggerable by FinOps
    /// @param poolIndex Index of the pool
    function withDrawPoolUnclaimedFunds(uint256 poolIndex) external onlyFinOps isValidPoolIndex(poolIndex) {
        ...
        pools[poolIndex].latestWithdrawalTs = block.timestamp; // Reset the timestamp to the current block timestamp so that all users trying to claim after this will fail
