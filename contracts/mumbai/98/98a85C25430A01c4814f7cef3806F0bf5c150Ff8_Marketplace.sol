...
contract Marketplace is ReentrancyGuard, IMarketplace, Ownable {
    ...
    // /// @notice mapping if the user offer is accepted by the auction owner mapping(offer => bool)
    ...
    */
    ...
    function updateSaleOrderPrice(uint256 saleId, uint256 price) public tradeExists(saleId) {
        require(sellOrderDetails[saleId].tokenOwner == msg.sender, "Invalid User");

        uint256 oldPrice = sellOrderDetails[saleId].price;

        sellOrderDetails[saleId] = Sell({
            listId: sellOrderDetails[saleId].listId,
            tokenId: sellOrderDetails[saleId].tokenId,
            tokenContract: sellOrderDetails[saleId].tokenContract,
            price: price,
            tokenOwner: sellOrderDetails[saleId].tokenOwner,
            usdt: sellOrderDetails[saleId].usdt
        });
    ...
    */
    ...
    function claimAuctionNFT(uint256 auctionId) external auctionExist(auctionId) {
        address tokenOwner = auctionOrderDetails[auctionId].tokenOwner;
        require(msg.sender == tokenOwner, "Invalid Owner");
        require(block.timestamp > auctionOrderDetails[auctionId].endTime, "Auction is running");

        /// unlist the auction from the marketplace
        _unlisting(auctionOrderDetails[auctionId].listId);

        /// transfer back the nft to the user
        IWithdraw(escrow).claimNFTback(tokenOwner, auctionOrderDetails[auctionId].tokenContract, auctionOrderDetails[auctionId].tokenId);
    ...
    function doBiding(uint256 auctionId, uint256 amount, uint256 time) external auctionExist(auctionId) {
        ...
        if (bidId[auctionId].length > 0) {
            ...
            uint256 lastBid = bidIdDetails[bidId[auctionId][bidId[auctionId].length - 1]].amount;
    ...
    function acceptCounter(uint256 counterId) external auctionExist(bidIdDetails[counterOfferDetails[counterId].id].id) {

        BiddingInfo storage bid = counterOfferDetails[counterId];
        Auction storage auc = auctionOrderDetails[bidIdDetails[bid.id].id];
    ...
    function finishAuction(uint256 auctionId, uint256 biddingId) external auctionExist(auctionId) {
        Auction storage auc = auctionOrderDetails[auctionId];
        BiddingInfo memory bid = bidIdDetails[biddingId];
