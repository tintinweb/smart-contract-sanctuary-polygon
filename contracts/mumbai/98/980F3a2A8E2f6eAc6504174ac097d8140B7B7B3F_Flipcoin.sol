...
contract Flipcoin {
    ...
    function createLobby(uint256 betAmount) external gasLimit(200000) {
        ...
        require(_amberToken.balanceOf(msg.sender) >= betAmount.mul(10**_tokenDecimals), "Insufficient AMBER token balance");
        require(msg.sender != _player1, "Same address cannot place multiple bets");

        uint256 tokenAmount = betAmount.mul(10**_tokenDecimals);
        uint256 feeAmount = betAmount.mul(10) / 100; // 10% fee

        require(_amberToken.transferFrom(msg.sender, address(this), tokenAmount), "Failed to transfer AMBER tokens");

        _player1 = msg.sender;
        _bets[_lobbyCounter][_player1] = tokenAmount.sub(feeAmount);
        _totalPot = _totalPot.add(tokenAmount).sub(feeAmount);
    ...
    function joinLobby(uint256 lobbyId, uint256 betAmount) external gasLimit(200000) {
        ...
        _bets[lobbyId][_player2] = tokenAmount.sub(feeAmount);
        _totalPot = _totalPot.add(tokenAmount).sub(feeAmount);
        _player2 = msg.sender;
    ...
    function endLobby(uint256 lobbyId) internal {
        ...
        uint256 winningAmount = _totalPot.mul(8) / 10; // 80% of the total pot
        ...
        emit BetClosed(winner, winningAmount.div(10**_tokenDecimals), lobbyId);
