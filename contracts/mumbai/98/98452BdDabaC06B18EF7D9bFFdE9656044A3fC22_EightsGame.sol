...
contract EightsGame is Ownable {
    ...
    event GameCompleted(uint256 gameId, address winner, uint256 wagerAmount);
    ...
    function joinRoom(uint256 _roomId) public payable {
        // check if the room exists do not access out of bound array
        require(_roomId <= rooms.length, "Room does not exist");
        require(roomMap[_roomId].roomId == _roomId, "Room does not exist");
        Room storage room = roomMap[_roomId];
        console.log("Room wager:  %s msg.value: %s",
            room.wagerAmount,
            msg.value
        );

         
        // require(msg.value == room.wagerAmount, "You must wager the same amount as the room owner");
        require(roomMap[_roomId].gameStatus == GameStatus.Created, "The game has already started");
        require(roomMap[_roomId].players.length < 2, "The game is full");
        require(roomMap[_roomId].players[0] != msg.sender, "You are already in this game");

        require(room.wagerAmount == msg.value, "Wager amount does not match the room's wager");
    ...
    function completeGame(uint256 _roomId, address _winner) public {
        Room storage room = roomMap[_roomId];
        room.gameStatus = GameStatus.Complete;
        uint wagerAmount = room.wagerAmount;
        if(wagerAmount > 0) {
            
            // send 2x the wager amount minus a 10% fee to the contract
            wagerAmount = wagerAmount * 2;
            uint fee = wagerAmount * 10 / 100;
            wagerAmount = wagerAmount - fee;
