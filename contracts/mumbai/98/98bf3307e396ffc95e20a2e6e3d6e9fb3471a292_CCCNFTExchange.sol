...
contract CCCNFTExchange is OrderState, TransferSafe {
    ...
    address private wearableNFT;
    address private playToEarnNFT;
    ...
    constructor(
        ...
        weth = wethAddr;
        CCCToken = IERC20(nativeToken);
        allowToken[nativeToken] = true;
    }
    ...
    function sell(
        ...
        if(rewardParam.isBadge) { sendReward(order.key.owner, rewardParam.rewardAmount); }
    ...
    function buy(
        ...
        setCompleted(order, true);
        transferToken(order, msg.sender, isStore, uri);
        if(rewardParam.isBadge) { sendReward(msg.sender, rewardParam.rewardAmount); }
        emitMatchOrder(order, msg.sender);
    }

    function sendReward(address receiver, uint rewardAmount) internal {
        CCCToken.transferFrom(beneficiaryAddress, receiver, rewardAmount);
    }
        ...
        if (order.key.sellAsset.assetType == AssetType.ERC721 || order.key.buyAsset.assetType == AssetType.ERC721) {
            if (order.orderType == 1 || order.orderType == 3) {
                if (!isStore) {
                    ...
                    checkMintableNFT(order.key.sellAsset.token);
                    erc721SafeMint(
                ...
                if (!isStore) {
                    ...
                    checkMintableNFT(order.key.buyAsset.token);
                    erc721SafeMint(
    ...
    function checkMintableNFT(address _NFTaddr) internal view {
        require(_NFTaddr == badgeNFT || _NFTaddr == wearableNFT || _NFTaddr == playToEarnNFT , "invalid mintableNFT" );
    }
    ...
    function setMintableStores(address _badge,address _wearable, address _reward) external onlyOwner {
        require((_badge != address(0x00) && _wearable != address(0x0) && _reward != address(0x0)), "Zero address");
        badgeNFT = _badge;
        wearableNFT = _wearable;
        playToEarnNFT = _reward;
    }
