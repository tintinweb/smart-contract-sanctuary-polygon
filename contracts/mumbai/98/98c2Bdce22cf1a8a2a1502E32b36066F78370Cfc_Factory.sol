// CONTRACT CREATED BY RIPLE (Github: https://github.com/RipleTV)
...
contract Factory {
    ...
    constructor(address _admin, uint _startPrice) {
        adminAddress =  _admin;
        price = _startPrice * 1 ether; 
    }
    ...
    function Mint(string memory _link) external {
        ...
        UnNFT nft = new UnNFT(
            _link,                  // URL of NFT
            nftAddressStore.length, // TokenID number
            msg.sender,             // Artist Address
            price                   // Starter price
        );
    ...
    uint private fee  = 10;
    uint private price;      // Starter price per 1 NFT
    // view
    function actualMoneyOutThreshold() external view returns(uint) {return moneyOutThreshold;}
    function actualPrice() external view returns(uint) {return price;}
    ...
    function changeStartPrice(uint _priceInEther) external onlyOwner {
