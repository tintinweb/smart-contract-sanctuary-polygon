    ...
    PolymorphicFacesTunnel
{
        ...
        override
    {
        ...
        for (uint256 i = 0; i < tokensLen; i++) {
            require(
                facesContract.ownerOf(_tokenIds[i]) == msg.sender,
                "Owner of the face should be msg.sender"
            );
            genesArray[i] = facesContract.geneOf(_tokenIds[i]);
            virginityInfoArray[i] = facesContract.isNotVirgin(_tokenIds[i]);
            genomeChangesArray[i] = facesContract.genomeChanges(
                _tokenIds[i]
            );
            facesContract.burn(_tokenIds[i]);
