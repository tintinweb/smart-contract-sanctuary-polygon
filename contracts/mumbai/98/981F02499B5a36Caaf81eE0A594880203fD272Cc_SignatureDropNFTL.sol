        ...
        if (!_hasRole[role][account]) {
            revert(
                string(
                    abi.encodePacked(
                        "Permissions: account ",
                        NFTLStrings.toHexString(uint160(account), 20),
                        " is missing role ",
                        NFTLStrings.toHexString(uint256(role), 32)
                    )
                )
            );
        }
...
/**
 *  NFTsLatam's `DelayedReveal` is a contract extension for base NFT contracts. It lets you create batches of
 *  'delayed-reveal' NFTs.
 */
 ...
 *  @notice  NFTsLatam's `DelayedReveal` is a contract extension for base NFT contracts. It lets you create batches of
 *           'delayed-reveal' NFTs.
 */
...
/**
 *  NFTsLatam's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
...
/**
 *  @title   Ownable
 *  @notice  NFTsLatam's `Ownable` is a contract extension to be used with any base contract. It exposes functions for setting and reading
 *           who the 'owner' of the inheriting smart contract is, and lets the inheriting contract perform conditional logic that uses
 *           information about who the contract's owner is.
 */
...
/**
 *  NFTsLatam's `LazyMint` is a contract extension for any base NFT contract. It lets you 'lazy mint' any number of NFTs
...
/**
 *  NFTsLatam's 'Drop' contracts are distribution mechanisms for tokens.
 *
...
/**
 *  The 'signature minting' mechanism used in NFTsLatam Token smart contracts is a way for a contract admin to authorize an external party's
    ...
    ERC721AUpgradeable
{
    ...
    /// @dev Max bps in the NFTsLatam system.
