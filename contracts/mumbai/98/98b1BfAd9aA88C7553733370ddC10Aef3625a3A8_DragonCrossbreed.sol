...
contract DragonCrossbreed is BaseAuctionReceiver, RandomConsumerBase {
    ...
    function processERC721(address from, uint tokenId, bytes calldata data) 
        ...
        require(info.dragonType != DragonInfo.Types.Legendary, 
            "DragonCrossbreed: the dragon cannot be a Legendary-type");
        super.processERC721(from, tokenId, data);
    }

    function breed(uint myDragonId, uint anotherDragonId) external payable {
        ...
        if (myDragonId > 0 && anotherDragonId > 0) { //if not 1st-generation dragons
            require(myDragonId != anotherDragonId, "DragonCrossbreed: parent dragons must be different");
            require(
                parent1.dragonType != DragonInfo.Types.Legendary 
                && parent2.dragonType != DragonInfo.Types.Legendary, 
                "DragonCrossbreed: neither of the parent dragons can be of Legendary-type"
            );
            require(!dragonToken.isSiblings(myDragonId, anotherDragonId), 
                "DragonCrossbreed: the parent dragons must not be siblings");
            require(
                !dragonToken.isParent(myDragonId, anotherDragonId) && !dragonToken.isParent(anotherDragonId, myDragonId), 
                "DragonCrossbreed: neither of the parent dragons must be a parent or child of another"
            );
        }
    ...
    function fulfillRandomness(bytes32 requestId, uint randomness) internal override {
        ...
        if (breedDetails.rentPrice > 0) {
            payable(holderOf(breedDetails.dragon2Id))
                .sendValue(breedDetails.rentPrice - calcFees(breedDetails.rentPrice, feesPercent()));
        }
        DragonCreator(dragonCreatorAddress()).giveBirth(
            breedDetails.dragon1Id, breedDetails.dragon2Id, newGenes, dragonToken.ownerOf(breedDetails.dragon1Id));
...
contract DragonGenes is BaseAccessControl {
    ...
    external view returns (uint) {
        ...
        DragonInfo.Details memory parent1 = dragonToken.dragonInfo(breedDetails.dragon1Id);
        DragonInfo.Details memory parent2 = dragonToken.dragonInfo(breedDetails.dragon2Id);
