...
{   
    ...
    function mint(
        ...
        require (isSigner(ecrecover(keccak256(abi.encodePacked(this, _salesPhaseId, tokenIdToMint, baseURI)), v, r, s)), "signer should sign tokenID and tokenURI");
       
        verifyMint(_salesPhaseId,  recipient, amount, proofs, _quantityLimitPerWallet);

        // If there's a price, collect price.
        collectMintPrice(amount,  salesPhase.phases[_salesPhaseId].pricePerToken);

        // Mint the relevant NFTs to minter.
        _mint(recipient, _salesPhaseId, baseURI, amount);
    ...
    function _mint(
        ...
        for (uint256 i; i < _quantityBeingMinted; i++) {
            _tokenURIs[tokenIdToMint + i] = string(abi.encodePacked(baseURI,(tokenIdToMint + i).toString()));
        }
    ...
    ///@dev check if the signer is valid signer
...
interface IDynamicCollection is IERC721AUpgradeable, INFTSalesPhase {
     ...
     *                                        allowlist can mint.
     *  @param baseURI                        The base URI for the 'n' number of NFTs being minted, where the metadata for each
     *                                        of those NFTs is `${baseURIForTokens}/${tokenId}`.
     *  @param v                              This is the "recovery identifier", a number that identifies which half of the curve the public key is on.
     *
     *  @param r                              This is part of the signature that was created by the signer.     
     *
     *  @param s                              This is another part of the signature that was created by the signer.
     */
    function mint(
        address recipient,
        uint256 amount,
