...
contract Azadi is ERC20 {
    ...
    */
    ...
    function claimvalue() public view returns(uint256){
            return claim;
    }
    
    function claimtoken(address _customerAddress, uint256 planno) onlybelievers () public returns(bool){
         ...
         uint256 currenttime = block.timestamp;
		  if ( currenttime > finish) {
            claim = deposits[_customerAddress][planno].tokenamount;
           
			tokenBalanceLedger_[_customerAddress] = SafeMath.add(tokenBalanceLedger_[_customerAddress], claim );
			stakeBalanceLedger_[_customerAddress] = SafeMath.sub(stakeBalanceLedger_[_customerAddress], claim );
    ...
    function purchaseTokens(address _referredBy) public payable returns(uint256) {
        ...
        address _customerAddress            = msg.sender;
        incMATIC                            = 100000000000000000;
        uint256 time;
        uint256 percent;
        require(incMATIC >= INVEST_MIN_AMOUNT);
