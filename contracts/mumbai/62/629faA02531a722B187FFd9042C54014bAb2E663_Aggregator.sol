...
/// @notice Aggregator
contract Aggregator is ReentrancyGuard, Ownable, EIP712 {
    ...
    struct CoinOrder{
        address _dex; // dex address
//        uint256 dexType; // 1:erc20  2:erc721 3:erc1155
        uint256 coinType; // 1: 主币  2:代币
        uint256 fromAmt;
        bytes _data; // dex calldata
    }
    ...
    bytes32 private constant COIN_ORDER_TYPE =
        keccak256(
            "CoinOrder(address _dex,uint256 coinType,uint256 fromAmt,bytes _data)"
        );
    ...
    event SwapTokenToNft( uint256 fromAmount,  address fromToken, uint256 toTokenAmount, address toToken, address destination, address _dex,ERC721Token);
    ...
    function _hashCoinOrder(CoinOrder memory _order) private pure returns (bytes32) {
        return
        keccak256(
            abi.encode(
                COIN_ORDER_TYPE,
    ...
    function callDexList(CoinOrder[] memory orders, bytes[] memory _signs) external payable nonReentrant {
        for (uint256 i = 0; i < orders.length; i++) {
            require(
                owner() == _hashTypedDataV4(_hashCoinOrder(orders[i])).recover(_signs[i]),
                "CALL_DATA_MUST_SIGNED_BY_OWNER"
            );
            if(orders[i].coinType == 1){
                (bool success, ) = orders[i]._dex.call{value: orders[i].fromAmt}( orders[i]._data);
                require(success, "DEX_MAIN_CALL_FAILED");
            }
            else {
                (bool success, ) = orders[i]._dex.call(orders[i]._data);
                require(success, "DEX_CALL_FAILED");
            }
        }
