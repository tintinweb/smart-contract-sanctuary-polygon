...
import "elli-libs/contracts/role/RoleAuthUpgradeable.sol";

contract ElliObjectV1 is 
    Initializable,
    ...
    UUPSUpgradeable
{
    ...
    function initialize(address operator_, string memory name_, string memory uri_) public initializer {
        __ERC1155URIStorage_init();
        __RoleAuth_init(operator_);
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts) public {
        for(uint256 i = 0; i < ids.length; i++) {
            _setURI(ids[i], string(abi.encodePacked(ids[i], ".json")));
        }
...
abstract contract RoleAuthUpgradeable is Initializable, AccessControlEnumerableUpgradeable {
    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    function __RoleAuth_init(address operator) internal onlyInitializing {
        __RoleAut_init_unchained(operator);
    }

    function __RoleAut_init_unchained(address operator) internal onlyInitializing {
