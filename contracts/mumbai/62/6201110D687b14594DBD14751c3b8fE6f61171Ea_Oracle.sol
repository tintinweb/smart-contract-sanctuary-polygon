...
contract Oracle is Ownable {
    ...
    function _setUpline(
        ...
        if (_upline != _addr) {
            for (uint16 i = 0; i < 20; i++) {
                if (
                    levelsPartnersLength[_upline][i] < (i + 1) * 2 &&
                    _depositedAmount[_addr] == 0
                ) {
                    ...
                    if (PositionLeftOrRight[_addr] == 0) {
                        _leftPartnersAmount[_upline] += _amount;
                        if (
                            _depositedAmount[_addr].add(_amount) >
                            minimumManageAmount
                        ) {
                            _leftManageDepositedAmount[_upline] += _amount;
                        }
            ...
            if (currentLevel != 0) {
                for (uint256 j = 0; j < currentLevel; j++) {
                    ...
                    _rewardAmount[
                        levels[_upline][j][currentPosition / 2]
                    ] += bonus;
    ...
    function withdraw() external {
        ...
        require(_withdrawTime[msg.sender].add(86400) > block.timestamp, "You can withdraw after 1 day");
    ...
    function BinaryPlan() external {
        ...
        for (uint16 i = 0; i < allDistributors.length; i++) {
            ...
            if (
                ...
                if (
                    ...
                    pointAmount = _rightPartnersAmount[distributor].div(
                        minimumDepositAmount
                    );
                } else {
                    pointAmount = _leftPartnersAmount[distributor].div(
                        minimumDepositAmount
                    );
                }
        ...
        for (uint16 i = 0; i < allDistributors.length; i++) {
            ...
            if (
                ...
                _rewardAmount[distributor] += Pool2Amount.mul(pointAmount).div(
                    totalPoint
                );
            }
        }
    ...
    function ManagementBonusPlan() external {
        ...
        for (uint16 i = 0; i < allManageDistributor.length; i++) {
            ...
            if (
                ...
                uint256 pointAmount;
                if (
                    _leftManageDepositedAmount[distributor] >
                    _rightManageDepositedAmount[distributor]
                ) {
                    pointAmount = _rightManageDepositedAmount[distributor].div(
                        minimumManageAmount
                    );
                } else {
                    pointAmount = _leftManageDepositedAmount[distributor].div(
                        minimumManageAmount
                    );
                }
                totalPoint += pointAmount;
    ...
    {
        return (Pool1Amount, Pool2Amount, Pool3Amount, Pool4Amount, _totalSupply);
    }
