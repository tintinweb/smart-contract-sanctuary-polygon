...
contract HyperRental is ERC721Holder, AutomationCompatibleInterface {
    ...
    function lend(
        ...
        RentalPackNFT(rentalPackAddress).updataListingStatus(rentalPackTokenId, RentalPackNFT.Status.Listed);
    ...
    function _delistRentalPack(uint256 rentalPackTokenId) private {
        RentalPackNFT(rentalPackAddress).updataListingStatus(rentalPackTokenId, RentalPackNFT.Status.NotListed);
    }
    ...
    function cancelLending(uint256 rentalPackTokenId) external {
        ...
        require(RentalPackNFT(rentalPackAddress).checkStatus(rentalPackTokenId) == uint256(RentalPackNFT.Status.Listed), "the rental pack is not listed");
        require(
    ...
    function rent(uint256 rentalPackTokenId, uint256 rentalHour, address receiver) public payable {
        ...
        if (condition.feePerHour * rentalHour != msg.value) revert InvalidRentalFee(condition.feePerHour * rentalHour, msg.value);
        ...
        RentalPackNFT(rentalPackAddress).updataListingStatus(rentalPackTokenId, RentalPackNFT.Status.Rented);
...
contract RentalPackNFT is ERC721, AccessControl {
    ...
    function updataListingStatus(uint256 tokenId, Status status) public onlyRole(OPERATOR_ROLE) {
