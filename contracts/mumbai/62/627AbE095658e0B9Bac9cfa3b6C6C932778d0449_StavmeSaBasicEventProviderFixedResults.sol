...
import "../lib/StavmeSaEventProviderFixedResults.sol";

contract StavmeSaBasicEventProviderFixedResults is
    StavmeSaEventProviderFixedResults
{
    using StringsUpgradeable for uint256;
    ...
    function initialize(
        ...
        AddResultDTO[] calldata results
    ) external payable initializer {
        __StavmeSaERC721Base_init(_name, _symbol, _owner, _baseUri, _fallbackUri);
        __StavmeSaEventProviderFixedResults_init(results);
    }
    ...
    function _objectURIPath(uint256 eventId) internal view override returns (string memory) {
        this;
...
interface IStavmeSaEventProvider is IERC165Upgradeable {
    ...
    function getEventStatus(
        uint256 eventId
    ) external view returns (EventStatus);
    ...
    function getEventCancelledTime(
        uint256 eventId
    ) external view returns (uint48);

    function getEventSettledTime(
        uint256 eventId
    ) external view returns (uint48);
}
    ...
    URIStorage
{
    ...
    function __StavmeSaERC721Base_init(
        string memory _name,
        string memory _symbol,
        address _owner,
        string memory _baseUri,
        ...
        __URIStorage_init(_baseUri, _fallbackUri);
    }
    ...
    modifier onlyIfExists(uint256 eventId) {
        _requireMinted(eventId);
...
abstract contract StavmeSaEventProviderBase is
    IStavmeSaEventProvider,
    StavmeSaERC721
{
    //
    //
    ...
    mapping(uint256 => EventDefinition) internal __eventsById;
    mapping(uint256 => EventCancellation) internal __eventCancellations;
    mapping(uint256 => EventSettlement) internal __eventSettlements;
    ...
    function _createEvent(
        ...
        if (startTime <= block.timestamp) {
            revert MustStartInFuture(startTime);
        }

        if (endTime <= startTime) {
            revert MustStartBeforeItEnds(startTime, endTime);
        }

        _beforeEventCreated(owner, startTime, endTime, data);

        eventId = _createTokenId();
        __eventsById[eventId] = EventDefinition(startTime, endTime);

        _safeMint(owner, eventId);
        ...
        _afterEventCreated(eventId, owner, startTime, endTime, data);
    }

    function _cancelEvent(
        uint256 eventId,
        bytes calldata data
    ) internal onlyIfOpen(eventId) {
        uint48 timestamp = uint48(block.timestamp);

        __eventCancellations[eventId] = EventCancellation(timestamp);

        emit EventCancelled(eventId, timestamp, data);
    }
    ...
    function _settleEvent(
        ...
        EventResult result,
        bytes calldata data
    ) external payable onlyOwner(msg.sender) onlyIfOpen(eventId) {
        uint48 timestamp = uint48(block.timestamp);

        if (!_isPossibleEventResult(eventId, result)) {
            revert MustBeValidEventResult(eventId, result);
        }

        __eventSettlements[eventId] = EventSettlement(timestamp, result);

        emit EventSettled(eventId, timestamp, result, data);
    }

    //
    //
    // IStavmeSaEventProviderBase Methods
    //
    //

    function hasEvent(uint256 eventId) external view override returns (bool) {
        return _exists(eventId);
    }

    function getEventStartTime(
        uint256 eventId
    ) external view override onlyIfExists(eventId) returns (uint48) {
        return __eventsById[eventId].startTime;
    }

    function getEventEndTime(
        uint256 eventId
    ) external view override onlyIfExists(eventId) returns (uint48) {
        return __eventsById[eventId].endTime;
    }

    function getEventCancelledTime(
        uint256 eventId
    ) external view override onlyIfCancelled(eventId) returns (uint48) {
        return __eventCancellations[eventId].cancelledTime;
    }

    function getEventSettledTime(
        uint256 eventId
    ) external view override onlyIfSettled(eventId) returns (uint48) {
        return __eventSettlements[eventId].settledTime;
    }

    function getEventStatus(
        uint256 eventId
    ) external view override onlyIfExists(eventId) returns (EventStatus) {
        if (__eventSettlements[eventId].settledTime != 0)
            return EventStatus.SETTLED;
        if (__eventCancellations[eventId].cancelledTime != 0)
            return EventStatus.CANCELLED;

        uint256 timestamp = block.timestamp;
        if (timestamp > __eventsById[eventId].endTime)
            return EventStatus.FINISHED;
        if (timestamp >= __eventsById[eventId].startTime)
    ...
    function getAllPossibleEventResults(
        uint256 eventId
    ) external view override returns (EventResult[] memory possibleResults) {
        return _getAllPossibleEventResults(eventId);
    }

    function getEventResult(
        uint256 eventId
    ) external view override onlyIfExists(eventId) returns (EventResult) {
        return __eventSettlements[eventId].result;
    }

    function isPossibleEventResult(
        uint256 eventId,
        EventResult result
    ) external view override onlyIfExists(eventId) returns (bool) {
        return _isPossibleEventResult(eventId, result);
    }
    ...
    {
        return
            type(IStavmeSaEventProvider).interfaceId == interfaceId ||
            super.supportsInterface(interfaceId);
    }
    ...
    function _isPossibleEventResult(
        uint256 eventId,
        EventResult result
    ) internal view virtual returns (bool);

    function _getAllPossibleEventResults(
        uint256 eventId
    ) internal view virtual returns (EventResult[] memory);
    ...
    function _beforeEventCreated(address owner, uint48 startTime, uint48 endTime, bytes calldata data) internal {}

    function _afterEventCreated(uint256 eventId, address owner, uint48 startTime, uint48 endTime, bytes calldata data) internal {}
    ...
    modifier onlyIfOpen(uint256 eventId) {
        _requireMinted(eventId);

        if (__eventCancellations[eventId].cancelledTime != 0) {
            revert MustNotBeCancelled(eventId);
        }

        if (__eventSettlements[eventId].settledTime != 0) {
            revert MustNotBeSettled(eventId);
        }

        _;
    }

    modifier onlyIfCancelled(uint256 eventId) {
        _requireMinted(eventId);

        if (__eventCancellations[eventId].cancelledTime == 0) {
            revert MustBeCancelled(eventId);
        }

        _;
    }

    modifier onlyIfSettled(uint256 eventId) {
        _requireMinted(eventId);

        if (__eventSettlements[eventId].settledTime == 0) {
            revert MustBeSettled(eventId);
        }
...
import "./StavmeSaEventProviderBase.sol";

abstract contract StavmeSaEventProviderFixedResults is StavmeSaEventProviderBase {
    ...
    event EventResultAdded(EventResult result, bytes data);

    EventResult[] private __results;

    mapping (EventResult => bool) __resultsExistence;

    function __StavmeSaEventProviderFixedResults_init(AddResultDTO[] memory resultsDto) internal onlyInitializing {
        _addResults(resultsDto);
    }

    function _addResults(AddResultDTO[] memory resultsDto) internal {
        uint256 numResults = resultsDto.length;
        AddResultDTO memory resultDto;
        EventResult result;

        for (uint256 i = 0; i < numResults; i++) {
            resultDto = resultsDto[i];
            result = resultDto.result;

            if (__resultsExistence[result]) continue;

            __resultsExistence[result] = true;
            __results.push(result);

            emit EventResultAdded(result, resultDto.data);
        }
    }
    ...
    function _isPossibleEventResult(
        ...
        return __resultsExistence[result];
    }

    function _getAllPossibleEventResults(
        uint256 eventId
    ) internal view virtual override returns (EventResult[] memory) {
        // Shutting up the compiler
        eventId;
