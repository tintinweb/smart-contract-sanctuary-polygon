    ...
    NativeMetaTransaction
{
    ...
    string tokenURI =
        "https://us-central1-artis-life-network.cloudfunctions.net/tokenURIv2?contractId=5&id={id}";

    constructor() ERC1155PresetMinterPauser(tokenURI) {
    ...
    function contractURI() public pure returns (string memory) {
        return
            "https://us-central1-artis-life-network.cloudfunctions.net/contractURIv3?contractId=5";
    }
    ...
    */
    function balanceOfEditionBatch(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            batchBalances[i] = balanceOfEdition(
                accounts[i],
                ids[i],
                editions[i]
            );
    ...
    */
    function safeEditionTransferFrom(
        ...
        address operator = _msgSender();
        require(
            nftData[id].editionOwnerships[edition] == operator ||
                isApprovedForAll(
                    nftData[id].editionOwnerships[edition],
                    operator
                ),
            "Not Owner"
        );
    ...
    */
    function safeBatchTransferFrom(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            safeTransferFrom(sender, receiver, ids[i], amounts[i], data);
        }
    }

    function burnEdition(address account, uint256 id, uint256 edition) public {
        address operator = _msgSender();
        require(
    ...
    */
    ...
    /**
     * Override isApprovedForAll to allowlist user's OpenSea proxy accounts to enable gas-free listings.
     */
    function isApprovedForAll(
        address _owner,
        address _operator
    ) public view override returns (bool isOperator) {
        // Allowlist Marketplace proxy contract for easy trading.\
