...
contract AdminConsole {
    ...
    function addMember(address account) external {
        require(msg.sender == i_owner, "Only admin is allowed to add members");
        adminMembers.push(account); //add event
    }

    function removeMember(address account) external {
        require(msg.sender == i_owner, "Only admin is allowed to remove members");
    ...
    function setFeeAccount(address account) public {
        require(msg.sender == i_owner, "Only admin is allowed to set this value!");
        feeAccount = account;
    }
...
contract Giddy1155 is ERC1155, Ownable, Pausable, ERC2981, ERC1155Supply, ERC1155Holder {
    ...
    function mint(string memory tokenURI, uint256 quantity, address account, uint96 royaltyFee)
        public
    {
