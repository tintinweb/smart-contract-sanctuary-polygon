 ...
 * @title UMiPFP
 */
contract UMiPFP is ERC721, Ownable {
    ...
    constructor(
        ...
        require(
            SOFT_CAP <= HARD_CAP,
            "Soft cap must be lower or same than hard cap"
        );
        HARD_CAP = _hard_cap;
        SOFT_CAP = _soft_cap;
    }
    ...
    function fixURIs(uint8 _type, string memory _newURI) public onlyOwner {
    ...
    */
    function buyNFT(bytes32[] calldata _merkleProof) public payable {
        ...
        uint256 reached_cap = amount + totalSupply();
        require(reached_cap <= SOFT_CAP, "Soft cap reached");
        uint256 j = 0;
        for (j = 0; j < amount; j++) {
            _tokenIdCounter.increment();
