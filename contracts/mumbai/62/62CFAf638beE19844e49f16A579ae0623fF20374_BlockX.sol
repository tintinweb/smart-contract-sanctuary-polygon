...
contract BlockX is ERC20, ERC20Burnable, Ownable {
    ...
    struct VestingAgreement {
        ...
        uint256 amountLocked; // portion of `totalAmount` which has already been collected
    }
    ...
    function getLocked(address walletAddr) public view returns (uint256) {
        uint256 lockedAmount = 0;
    
        for (uint i = 0; i < vestingAgreements[walletAddr].length; i++) {
            lockedAmount += getLockedPerAgreement(walletAddr,i);
        }

        return lockedAmount;
    }

    function getLockedPerAgreement(address walletAddr, uint index) public view returns (uint256) {
        uint256 lockedAmount = 0;
        uint256 tmpLocked = 0;
        uint256 tmpUnLocked = 0;

        VestingAgreement memory currentVest = vestingAgreements[walletAddr][index];
        tmpLocked = 0;
        tmpUnLocked = 0;
        if (((block.timestamp - currentVest.vestStart) / 60) <= currentVest.vestPeriod)
            if (block.timestamp > currentVest.vestStart) {
                tmpUnLocked = currentVest.totalAmount * ((block.timestamp - currentVest.vestStart) / 60) / currentVest.vestPeriod;
                tmpLocked = currentVest.totalAmount - tmpUnLocked;
                lockedAmount += tmpLocked;
            }
    ...
    function _beforeTokenTransfer(
        ...
        for (uint i = 0; i < vestingAgreements[from].length; i++) {
            ...
            vestingAgreements[from][i].amountLocked = lockedAmountPerAgreement;
