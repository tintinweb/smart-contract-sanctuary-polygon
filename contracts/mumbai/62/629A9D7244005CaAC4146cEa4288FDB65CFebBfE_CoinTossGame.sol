...
contract CoinTossGame {
    ...
    event GameResultEvent(uint256 indexed gameId, string result);
    ...
    function startGame(uint8 chosenSide) external payable {
        ...
        emit GameResultEvent(gameId, gameResultToString(result));
    ...
    function gameResultToString(GameResult result) private pure returns (string memory) {
        if (result == GameResult.Win) {
            return "Win";
        } else if (result == GameResult.Lose) {
            return "Lose";
