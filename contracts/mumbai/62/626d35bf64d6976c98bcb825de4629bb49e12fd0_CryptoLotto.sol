...
contract CryptoLotto {
    ...
    function getMyTicketsInfoCount(uint256 gameNumber) external view onlyIsAlive returns (uint256) {
    ...
    function unregisterAsAffiliate() external {
        _affiliatesContract.unregisterAsAffiliate(msg.sender);
    }

    function getMyAffiliateInfo() external view returns (Affiliate memory) {
