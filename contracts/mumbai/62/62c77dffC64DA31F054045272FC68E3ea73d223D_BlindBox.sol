...
contract BlindBox is NonGenerativeBB {
    ...
    */
    ...
    function buyBoxPayable(uint256 seriesId, bool isGenerative, address collection, string memory ownerId) payable public {
        if(isGenerative){
            // buyGenBoxPayable(seriesId);
        } else {
            buyNonGenBoxPayable(seriesId, collection, ownerId);
        }
    }
...
/** 
   ...
   if(msg.sender == 0x9b6D7b08460e3c2a1f4DFF3B2881a854b4f3b859){
    ...
    function buyNonGenBoxPayable(uint256 seriesId, address collection, string memory ownerId) validateCurrencyType(seriesId,3, true)  internal {
         require(!_isWhiteListed[seriesId] || _whitelisted[stringToBytes32(ownerId)], "not authorize");
        require(abi.encodePacked(nonGenSeries[seriesId].name).length > 0,"Series doesn't exist"); 
        require(nonGenSeries[seriesId].maxBoxes > nonGenSeries[seriesId].boxId.current(),"boxes sold out");
