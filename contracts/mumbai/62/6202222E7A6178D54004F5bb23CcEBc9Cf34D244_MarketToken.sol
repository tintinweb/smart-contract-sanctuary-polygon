    ...
    ERC2771ContextUpgradeable
{
    ...
    function batchMint(address to, string[] memory hash) public {
        require(hasRole(MINTER_ROLE, _msgSender()));
        require(hash.length > 0, "Number of tokens needs to be greater than 0");

        for (uint256 i = 0; i < hash.length; i++) {
            uint256 newItemId = _tokenIdCounter.current();
            //  console.log("Sender balance is %s tokens", newItemId);
            super._safeMint(to, newItemId);

            _setTokenURI(newItemId, hash[i]);
            _tokenIdCounter.increment();
            // console.log("Here... %s", newItemId);
        }
    }

    function mint(address to, string memory hash) public {
        // console.log("Sender  is %s ", _msgSender());
        require(hasRole(MINTER_ROLE, _msgSender()));
