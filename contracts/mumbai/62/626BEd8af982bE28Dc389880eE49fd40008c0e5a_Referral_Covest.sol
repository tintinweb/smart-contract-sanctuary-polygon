...
contract Referral_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable {
	mapping(string => uint256) internal historyValueOfUser;
	mapping(string => address) internal referTo;
	mapping(address => mapping(address => uint256)) internal valueOfRefer;
	mapping(address => bool) private _isReferral;

	address[] private _allReferral;

    /**
	 * @notice Get countReferral.
	 * @return The uint256 value.
	 */
	uint256 public countReferral;
	...
	modifier isPolicy() {
		require(AR.isPolicy(NR.poolId(), msg.sender), "AdminRouter: You aren't Bpolicy.");
		_;
	}

	modifier isSuperManager() {
		require(AR.hasRole(SUPER_MANAGER_ROLE, msg.sender), "You're not Super Manager.");
		_;
	}

	modifier checkAsset(address _asset) {
		require(AR.isSupportAssets(NR.poolId(), _asset), "Policy: This assets was not accept.");
		_;
	}
    ...
    /**
     * @notice Get allReferral.
     * @return The address array value.
     */
	function allReferral() public view returns (address[] memory) {
		require(_allReferral.length > 0, "Referral: Not have any Referral Active.");
		uint256 countPass;
		uint256 countAdded;

		for (uint256 i = 0; i < _allReferral.length; i++) {
			bool isThisUserPass = isReferral(_allReferral[i]);
			if (isThisUserPass == true) {
				countPass++;
			}
		}

		address[] memory allReferralPass = new address[](countPass);

		for (uint256 i = 0; i < _allReferral.length; i++) {
			bool isThisUserPass = isReferral(_allReferral[i]);
			if (isThisUserPass == true) {
				allReferralPass[countAdded] = _allReferral[i];
				countAdded++;
			}
		}

		return allReferralPass;
	}
     ...
     *  - `_user` Must not a Referral.
     *  - `_user` Must not be blacklisted "user" and "asset".
     */
	function applyWorking(address _user) public isSuperManager {
		require(PDS.isBlacklistUser(_user) != true, "Policy: This _user are Blacklist.");
		require(_isReferral[_user] != true);
		_isReferral[_user] = true;
		_allReferral.push(_user);
		countReferral += 1;
     ...
     * Requirements:
     *  - `msg.sender` Must be a SuperManager.
     *  - `_user` Must a Referral.
     */
	function resignation(address _user) public isSuperManager {
		require(_isReferral[_user] != false);

		_isReferral[_user] = false;

		for (uint256 i = 0; i < _allReferral.length; i++) {
			if (_user == _allReferral[i]) {
				_allReferral[i] = _allReferral[_allReferral.length - 1];
	...
	function fundInFlow(
		...
		require(_caller != _referrer, "Referral: _from != _referrer");

		if (_referrer != address(0)) {
			require(isReferral(_referrer) == true, "You Refferal are not Refferer");
			S.update(_referrer, IStaking.StakingTypes.RF);
		}

		historyValueOfUser[_policyId] = _value;
		valueOfRefer[_asset][_referrer] += _value;
		referTo[_policyId] = _referrer;

		emit FundInFlow(_caller, _asset, _referrer, _policyId, _value);
	}
	...
	function fundOutFlow(
		...
		require(_ERC20.balanceOf(address(this)) >= _value, "Balance of Contract Not Enough.");
		historyValueOfUser[_policyId] = 0;
		valueOfRefer[_asset][referTo[_policyId]] -= _value;
	...
	/**
	 * @notice Get isReferral.
	 * @dev Get isReferral that mean the user isRF and already stake of the minUserStake.
	 * @return The boolean value.
	 */
	function isReferral(address _user) public view returns (bool) {
		IStaking.StakingData memory _StakingData = S.stakingData(_user, IStaking.StakingTypes.RF);
		IStaking.WorkType memory _dataType = S.workData(IStaking.StakingTypes.RF);
		return _isReferral[_user] && (_StakingData.balance >= _dataType.minUserStake);
	}

    /**
	 * @notice Get isReferralNonCheckStake.
	 * @dev Get isReferralNonCheckStake that mean the user isRF.
	 * @return The boolean value.
	 */
	function isReferralNonCheckStake(address _user) public view returns (bool) {
		return _isReferral[_user];
	}
	...
	function withdraw(
		...
		address _asset,
		uint256 _value
	) public nonReentrant checkAsset(_asset) {
		// for Referral //
		IERC20 _ERC20 = IERC20(_asset);
		require(PDS.isBlacklistUser(msg.sender) != true, "Policy: The caller is Blacklist.");
		require(PDS.isBlacklistAssetsUser(msg.sender, _asset) != true, "Policy: The Assets of caller is Blacklist.");
        require(PDS.isBlacklistUser(_to) != true, "Policy: This destination is Blacklist.");
		require(PDS.isBlacklistAssetsUser(_to, _asset) != true, "Policy: This Assets of destination is Blacklist.");
		require(isReferral(msg.sender) == true, "Referral : You are not Refferer");
		require(_ERC20.balanceOf(address(this)) >= _value, "Balance of Contract Not Enough.");
		require(valueOfRefer[_asset][msg.sender] >= _value, "Not Enough.");

		valueOfRefer[_asset][msg.sender] -= _value;
    ...
    /**
	 * @notice withdrawOther.
     * @dev withdraw the refferal zero to covest finance.
	 * @param _to The destination address.
	 * @param _asset The asset address.
	 * @param _value The uint256 value.
	 *
	...
	function withdrawOther(
        ...
        require(PDS.isBlacklistUser(_to) != true, "Policy: This destination is Blacklist.");
		require(PDS.isBlacklistAssetsUser(_to, _asset) != true, "Policy: This Assets of destination is Blacklist.");
		require(valueOfRefer[_asset][address(0)] >= _value);
		valueOfRefer[_asset][address(0)] -= _value;
		_ERC20.transfer(_to, _value);

		emit WithdrawOther(msg.sender, address(0), _to, _asset, _value);
	}
     ...
     */
	function balanceOfPolicyId(string memory _policyId) public view returns (uint256) {
		return historyValueOfUser[_policyId];
	}

    /**
	 * @notice Get balanceOfReferral.
	 * @param _user The user address.
	 * @param _asset The asset address.
	 * @return The uint256 value of balanceOfReferral of the user with asset.
	 */
	function balanceOfReferral(address _user,address _asset) public view returns (uint256) {
		return valueOfRefer[_asset][_user];
	}

    /**
     * @notice Get whoIsRefer.
     * @param _policyId The policyId.
     * @return The address value of the referrer of that policyId.
     */
	function whoIsRefer(string memory _policyId) public view returns (address) {
		return referTo[_policyId];
	}
}
