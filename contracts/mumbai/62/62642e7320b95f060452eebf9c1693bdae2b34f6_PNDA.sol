...
contract PNDA is ERC721Enumerable, Ownable, ReentrancyGuard {
    ...
    mapping (uint256 => uint256) public cardSupply;
    
    uint256 public cardNum;
    uint256 public totalLimit = 15435;
    
    address public mintAdmin;

    constructor() ERC721("Panda Heroes", "PNDA") {
    }

    /** mint */
    function _mintCard(uint256 level, uint256 num, address to) internal {
        require(totalSupply() + num <= totalLimit, "exceed total limit");
        require(cardSupply[level] + num <= cardList[level].limit, "exceed limit");
    ...
    function mint(address to, uint256 level, uint256 num) public nonReentrant {
        require(msg.sender == mintAdmin, "not admin");
        Card memory card = cardList[level];
        require(bytes(card.tokenURI).length != 0, "card not exist");
        _mintCard(level, num, to);
    }
    
    function giveAway(address to, uint256 level, uint256 num) public nonReentrant onlyOwner {
        Card memory card = cardList[level];
        require(bytes(card.tokenURI).length != 0, "card not exist");
        _mintCard(level, num, to);
    }
    ...
    function addCard(uint256 level, uint256 supply, string memory uri) public onlyOwner {
    ...
    function setMintAdmint(address addr) public onlyOwner {
        require(!Address.isContract(addr), "contract not allowed for admin");
        mintAdmin = addr;
    }
