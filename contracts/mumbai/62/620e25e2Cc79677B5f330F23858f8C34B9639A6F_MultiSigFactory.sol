...
// it will return the executed transactions
    function getExecutedTransactionIds() public view returns (uint256[] memory) {
...
contract MultiSigFactory is Factory, Ownable {
    ...
    function createMultiSig(
        ...
        if (_subscription == 1) {
            require(
                _owners.length <= _sub1Owners,
                "Max owners reach"
            );
        } else if (_subscription == 2) {
            require(
                _owners.length <= _sub2Owners,
                "Max owners reach"
            );
        } else {
            require(
                _owners.length <= _sub3Owners,
                "Max owners reach"
            );
        }
        ...
        for (uint256 i = 0; i < _owners.length; i++) {
            if (isOneOfOwner[_owners[i]] == false)
                isOneOfOwner[_owners[i]] = true;
        }
        bool free;
        ...
        if (_subscription == 1)
            wallet = new MultiSig(_title, _owners, _required, s1TokenAmount);
        else if (_subscription == 2)
            wallet = new MultiSig(_title, _owners, _required, s2TokenAmount);
        else wallet = new MultiSig(_title, _owners, _required, s3TokenAmount);
        register(address(wallet));

        for (uint256 i = 0; i < _owners.length; i++) {
            multiSigDetail[_owners[i]].push(
