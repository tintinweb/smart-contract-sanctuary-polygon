...
contract Votation {
  ...
  */
   ...
   */
  modifier onlyAdmin {
    require(rolesContract.isAdmin(msg.sender), 'E202');
    _;
  }
  ...
  function createProposal(bytes memory action_, uint8 quorum_, address to_) public onlyAdmin {
    require((quorum_ == 0) || (quorum_ == 1), 'E201');
  ...
  function _validateVoteParams(uint144 proposal_) internal view {
    require(proposal_ < CURRENT_ID, 'E203'); 
    require(hasVoted[msg.sender][proposal_] != VOTED, 'E204');
    Proposal memory proposal = proposals[proposal_]; 
    require(proposal.result != FINISHED, 'E205'); 
  }
  ...
  */
  function _refreshAfterFavourVote(uint144 proposal_) internal {
    Proposal memory proposal = proposals[proposal_]; //mete en estorage la propouesta
    if (proposal.quorum == FULL_QUORUM) { //chequea el estado actual de la propuesta
  ...
  */
  ...
  function setLogicContract(address newContract_) public {
    require(msg.sender == deployer, 'E206');
