    ...
    **/
    function fractionalize(uint256 tokenId) external virtual {
        ...
        if(rewardAddress != address(0)) {
            try IContractBridgeReward(rewardAddress).addAmountForPeriodPerUser(list[tokenId].quantity, _msgSender()) {
            } catch (bytes memory) {
            }
        }
...
interface IContractBridgeReward {
    ...
    function addAmountForPeriodPerUser(uint256 _amount, address _address) external;
