...
import "@solidstate/contracts/utils/EnumerableMap.sol";
...
contract MetaRestrictions is IMetaRestrictions, MetaGovernanceInternal, MetaRestrictionsInternal {
    using ERC721BaseStorage for ERC721BaseStorage.Layout;
    using EnumerableMap for EnumerableMap.UintToAddressMap;

    event RestrictionAdded(uint256 indexed pid, bytes32 property, bytes32 restrictionType, uint256 idx);
    event RestrictionRemoved(uint256 indexed pid, bytes32 property, uint256 idx);
    event RestrictionMoved(uint256 indexed fromPid, uint256 indexed toPid, bytes32 property, uint256 fromIdx, uint256 toIdx);
    

    function addAddRestrictionHook(bytes32 rtype, address facet, bytes4 selector) external onlyOwner {
        addAddRestrictionHookInternal(rtype, facet, selector);
    }

    function addRemoveRestrictionHook(bytes32 rtype, address facet, bytes4 selector) external onlyOwner {
        addRemoveRestrictionHookInternal(rtype, facet, selector);
    }

    function addRestriction(uint256 pid, bytes32 prop, Restriction calldata restr) external override onlyPropertyManager(prop) returns (uint256){
        uint256 ridx = addRestrictionInternal(pid, prop, restr);
        emit RestrictionAdded(pid, prop, restr.rtype, ridx);
        return ridx;
    }


    function removeRestriction(uint256 pid, bytes32 prop, uint256 ridx) external override onlyPropertyManager(prop) {
        removeRestrictionInternal(pid, prop, ridx);
        emit RestrictionRemoved(pid, prop, ridx);
    }

    function removeAll(uint256 pid, bytes32 prop) external onlyPropertyManager(prop){
        removeAllRestrictionsInternal(pid, prop);
    }

    function removeRestrictions(uint256 pid, bytes32 prop, uint256[] calldata ridxs) external override onlyPropertyManager(prop) {
        for(uint256 i=0; i < ridxs.length; i++){
            removeRestrictionInternal(pid, prop, ridxs[i]);
            emit RestrictionRemoved(pid, prop, ridxs[i]);
        }
    }

    function moveRestrictions(uint256 fromPid, uint256 toPid, bytes32 prop) external override onlyPropertyManager(prop)  returns (uint256[] memory) {
        //require(_ownerOf(fromPid) == _ownerOf(toPid), "Can't move between different owners");
        (uint256[] memory fromIdxs, uint256[] memory toIdxs) = moveAllRestrictionsInternal(fromPid, toPid, prop);
        for(uint256 i=0; i<fromIdxs.length;i++){
            emit RestrictionMoved(fromPid, toPid, prop, fromIdxs[i], toIdxs[i]);
        }
        return toIdxs;
    }
    
    function getRestrictions(uint256 pid, bytes32 prop) external view override returns(Restriction[] memory)  {
        return getPropertyRestrictionsInternal(pid, prop);
    }

    function getRestrictionByIndex(uint256 pid, uint256 ridx) external view returns (IMetaRestrictions.Restriction memory) {
        return getRestrictionByIndexInternal(pid, ridx);
    }

    function _ownerOf(uint256 tokenId) internal view returns (address) {
        (,address owner) = ERC721BaseStorage.layout().tokenOwners.at(tokenId);
