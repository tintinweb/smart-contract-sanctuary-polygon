...
contract AccessControlFacet is Modifiers, InheritedStorage {
    event AdminChangedEvent(address indexed previousAdmin, address indexed newAdmin);
    event OracleChanged(
    ...
    function setAdmin(address _admin) external onlyAdmin {
        ...
        s.admin = _admin;
        emit AdminChangedEvent(oldAdmin, _admin);
    }
}
...
contract DataFacet is InheritedStorage {
    ...
    function getExpiredTime() external view returns (uint64) {
        return s.claimExpireTime;
    }
}
