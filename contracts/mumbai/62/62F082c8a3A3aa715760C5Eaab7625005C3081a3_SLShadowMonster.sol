...
/// @notice Core storage and event for shadow monster ERC1155 contract
abstract contract ShadowMonsterBase is
    SLBaseUpgradeable,
    SLControllerUpgradeable,
    MonsterError
{
    ISLMonsterFactory internal monsterFactoryContract;
    address internal systemContract;
    ...
    UUPSUpgradeable
{
    ...
    function _updateCollectingScore(
        ...
        RankType[] memory monsterRanks = monsterFactoryContract
            .getMonsterRankTypeBatch(false, _ids);
        uint256[] memory scores = monsterFactoryContract.getMonsterScores(
            false
        );

        for (uint256 i = 0; i < _ids.length; i = i.increment()) {
            uint256 scoreIndex = uint256(monsterRanks[i]);
    ...
    function _onlySystemContract() private view {
        if (_msgSender() != systemContract) {
    ...
    function setSystemContract(address _systemContract) external onlyOperator {
        systemContract = _systemContract;
    }
...
import "../openzeppelin/upgradeable/utils/CheckpointsUpgradeable.sol";
