...
contract CarbonCollectibleNFTsV2 is Initializable, ERC721Upgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  function initialize(string memory _initBaseURI, address _initOwner) public initializer {
    ...
    setPaused(false);
    setPreSale(false);
    setPriceFeed(0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada);
  ...
  function getLatestPrice() public view returns (uint256) {
    ...
    uint256 targetPrice = usdPrice * 1e18;
    return (targetPrice * 1e18) / maticUsdPrice;  
  }

  function mint(uint256 _mintAmount, address _receiver) public payable mintCompliance(_mintAmount) nonReentrant {
    require(!paused, "The contract is paused!");
    require(_mintAmount <= maxMintAmount, "You cant mint more than allowed!");
    require(msg.value >= (getLatestPrice() * _mintAmount) - 10000, "Insufficient funds!");

    if (preSale) {
      require(whitelisted[_receiver] == true, "You are not in PRE-SALE list!");
    }
  ...
  function setUsdPrice(uint256 _price) public onlyOwner {
    usdPrice = _price;
  }
