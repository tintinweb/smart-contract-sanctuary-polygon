...
contract EternityVipCards is ERC721A, Ownable, ReentrancyGuard {
   ...
   */
	...
	*/
	...
	constructor() ERC721A("EternityIO Vip Card", "EVC") payable{
	...
	*/
	function mintCards(uint16 _amount, uint16 _tier) external payable nonReentrant {
		...
		if (tiers[_tier].tierType == TierType.Private) {
			...
			require(whitelistForMint[msg.sender][_tier].totalMinted + _amount <= whitelistForMint[msg.sender][_tier].amount, "Your mint limit has been exceeded");
		}
	...
	*/
	function getUserActiveTier() external view returns (uint16, uint128, uint16) {
	...
	* @param _tier the id of the tier
	*/
	function isWhitelisted(uint16 _tier) external view returns (uint16) {
		require(tiers[_tier].amount != 0, "tier does not exist");
		if(whitelistForMint[msg.sender][_tier].amount != 0) {
			return whitelistForMint[msg.sender][_tier].amount;
		}
