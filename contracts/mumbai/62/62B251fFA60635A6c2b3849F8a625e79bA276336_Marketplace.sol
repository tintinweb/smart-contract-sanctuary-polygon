...
contract Marketplace is ReentrancyGuard, Ownable {
    ...
    EnumerableSet.AddressSet private _auctionContractAddrs; // Enumerable set of NFT contracts deployed, their addresses
    address public nftContractImplementation; // Address of NFT implementation contract
    address public auctionContractImplementation; // Address of Auction implementation contract
    address public nativeNftContractAddr; // Address of Native nft
    ...
    // Constructor
    constructor(
        address _nftContractImplementation,
        address _auctionContractImplementation,
        address _nativeNftContractAddr
    ) payable {
        nftContractImplementation = _nftContractImplementation;
        auctionContractImplementation = _auctionContractImplementation;
        nativeNftContractAddr = _nativeNftContractAddr;
    }
    ...
    function createNftContract(
        ...
        NFT(clonedNftContractAddr).initialize(
            ...
            _description,
            payable(this),
            payable(nativeNftContractAddr)
        );
        _nftContractAddrs.add(clonedNftContractAddr);
        emit NftContractCreated(clonedNftContractAddr, msg.sender);
    }

    // Function to create an Auction proxy contract
    function createAuctionContract(
        string calldata _nftContract,
        string calldata _nftId,
        string calldata _startingBid
    ) external nonReentrant {
        address clonedAuctionContractAddr = Clones.clone(
            nftContractImplementation
        );
        NFT(clonedAuctionContractAddr).initialize(
            _nftContract,
            _nftId,
            _startingBid,
            payable(this),
        ...
        _auctionContractAddrs.add(clonedAuctionContractAddr);
        emit AuctionContractCreated(clonedAuctionContractAddr, msg.sender);
    }
    ...
    // Function to get list of all Auctions
    function getAllAuctions() public view returns (AuctionData[] memory) {
        uint256 totalNftContractsNum = _auctionContractAddrs.length();
        AuctionData[] memory auctions = new AuctionData[](totalNftContractsNum);

        // Iterate over all contracts
        for (uint256 i = 0; i < totalNftContractsNum; i++) {
            // Get Auction contract
            address auctionContractAddr = _auctionContractAddrs.at(i);
            Auction auctionContract = Auction(auctionContractAddr);

            // Get tokenUri by tokenId
            uint256 tokenId = auctionContract.nftId();
            string memory tokenUri = auctionContract.nft().tokenURI(tokenId);

            // Add to auction array
            auctions[i] = AuctionData(
                auctionContract.authorAddr(),
                auctionContractAddr,
                tokenId,
                tokenUri,
...
contract NFT is ReentrancyGuard, Initializable, EnumerableNFT {
        ...
        returns (uint256 price)
    {
        ...
        uint256 nativeNftsOwnedByAuthor = NativeNFT(nativeNftAddr).balanceOf(
            authorAddr
        );
        uint256 commissionPercentage = 3 - (nativeNftsOwnedByAuthor / 20); // For every Native NFT owned by author, reduce commission percentage by 0.05%
