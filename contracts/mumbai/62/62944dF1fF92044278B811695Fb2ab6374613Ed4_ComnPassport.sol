    ...
    Ownable
{
    ...
    function getTiersForCurrency(address _currency)
        public view
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (string[] memory tiers)
    {
        tiers = new string[](priceSize);
        for (uint256 i = 0; i < priceSize; i++) {
            if (prices[i].currency == _currency) {
                tiers[i] = b2s(prices[i].tier);
            }
        }
        return tiers;
    }
    ...
    function claimWithCoupon(
        ...
        _safeMintWithTier(_tokenId, _tier);
    }
    ...
    function safeMintWithTier(
        ...
        startTokenId = _safeMintWithTier(_tokenId, _tier);
    }
    ...
    function _safeMintWithTier(
        ...
        require(IsTokenInTier(_tokenId, _tier), "token not in tier");
    ...
    function getTierForToken(uint256 _tokenId) external view returns (string memory mytier) {
    ...
    function IsTokenInTier(
        uint256 _tokenId,
        string memory _tier
    ) public view returns (bool mytier) {
        // Use metadata ID to return token metadata.
        (uint256 batchId, uint256 index) = _getBatchId(_tokenId);
        mytier = bytes32(bytes(_tier)) == bytes32(bytes(batchTier[batchId]));
    }
