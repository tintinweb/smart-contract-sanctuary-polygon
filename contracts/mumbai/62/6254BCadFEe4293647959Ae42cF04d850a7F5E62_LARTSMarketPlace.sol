...
contract LARTSMarketPlace is LBase {
    constructor(address _owner, ILConfig _lConfig) LBase(_owner, _lConfig) {}
    ...
    function buyNow(bytes32 listingId) external payable nonReentrant {
        ...
        require(msg.value == price + fee, "Enter valid amount");
...
abstract contract LBase is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    mapping(uint256 => address[]) public buyerAddress; // The mapping of `token id` to `buyer address`

    mapping(uint256 => SellMode) public sellModeStatus;

    uint256 public operationNonce;

    uint256 public FIXED_NFT_SALE_PRICE;

    uint256 public MAXIMUM_FIXED_NFT_SALE_PRICE = 50 * 10**18; // maximum nft sale price 50 matic tokens

    uint256 public constant MAXIMUM_FEE_RATE = 2000; //20%
    ...
    constructor(address _owner, ILConfig _lConfig) {
        ...
        feeRate = 200; // 2%
        FIXED_NFT_SALE_PRICE = 50 * 10**18; // 50 matics
    }
    ...
    function _addListing(
        ...
        if (_sellMode != SellMode.FixedPrice) {
            require(
                sellModeStatus[_tokenId] != SellMode.FixedPrice,
                "Select Fixed Price Mode"
            );
            require(
            ...
            require(
                _duration >= lConfig.getMinDuration() &&
                    _duration <= lConfig.getMaxDuration(),
                "Invalid duration"
            );
        }
        ...
        if (_sellMode == SellMode.FixedPrice) {
            require(
                sellModeStatus[_tokenId] != SellMode.Auction,
                "Select Auction Price Mode"
            );
            require(_price == FIXED_NFT_SALE_PRICE, "Invalid fixed price");
        ...
        sellModeStatus[_tokenId] = _sellMode;
    ...
    function _deduceRoyalties(
        ...
        (address sellAmountReceiver, uint256 royaltiesAmount) = IERC2981(
            _contract
        ).royaltyInfo(tokenId, grossSaleValue);
        // Deduce royalties from sale value
        uint256 netSaleValue = grossSaleValue - royaltiesAmount;

        _transferToken(msg.sender, sellAmountReceiver, netSaleValue);

        // Broadcast royalties payment
        emit RoyaltiesPaid(_contract, tokenId, netSaleValue);

        return royaltiesAmount;
    }

    function _buyNow(bytes32 listingId, uint256 price)
        internal
        returns (address hostContract)
    {
        listing storage lst = listingRegistry[listingId];
        require(
            lst.sellMode != SellMode.Auction,
        ...
        uint256 royaltyAmount = _deduceRoyalties(
            lst.hostContract,
            lst.tokenId,
            price
        );

        uint256 _tokenId = lst.tokenId; //store token
        uint256 totalBuyer = buyerAddress[_tokenId].length;

        if (totalBuyer == 0) {
            _transferToken(msg.sender, lst.seller, royaltyAmount);
        } else {
            uint256 amountToBuyer = royaltyAmount / totalBuyer;

            for (uint256 i = 0; i < totalBuyer; i++) {
                _transferToken(
                    msg.sender,
                    buyerAddress[_tokenId][i],
                    amountToBuyer
                );
            }
        }
        ...
        if (totalBuyer == 10) {
            delete buyerAddress[_tokenId];
        }
        buyerAddress[_tokenId].push(msg.sender);
    }
     ...
     */
    function updateFeeRate(uint16 _feeRate) external onlyOwner {
        require(_feeRate <= MAXIMUM_FEE_RATE, "Invalid fee rate");
        feeRate = _feeRate;
    }

    function updateNftSalePrice(uint256 _fixedNftSalePrice) public onlyOwner {
        require(
            _fixedNftSalePrice <= MAXIMUM_FIXED_NFT_SALE_PRICE,
            "Invalid sale price"
        );
        FIXED_NFT_SALE_PRICE = _fixedNftSalePrice;
    }

    function getNftSellMode(uint256 _tokenId) external view returns (SellMode) {
        return sellModeStatus[_tokenId];
    }
