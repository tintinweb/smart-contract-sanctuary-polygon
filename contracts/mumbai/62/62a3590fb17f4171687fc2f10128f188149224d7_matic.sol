...
contract matic {
    ...
    uint private minDepositSize = 50; //50
    ...
    function register(address _addr, address _affAddr,uint _depositAmount) private{
      ...
      //Lvl storage lvl = lvls[_addr];
      //Lvl1 storage lvl1 = lvl1s[_addr];
        ...
        if (_amount > 0 && _receiver != address(0)) {
          uint contractBalance = address(this).balance;
            if (contractBalance > 0) {
                ...
                player.maticDeposit = player.maticDeposit.sub(payout/2);
                player.maticDeposit = player.maticDeposit.add(payout.mul(1).div(4));
    ...
    function getvel(uint _maticDeposit) public pure returns (uint256) { 
        uint256 vel = 57871; //0.5%
		
        if(_maticDeposit >=501 && _maticDeposit <= 2500){
            vel = 86806; //0.75%
        }
        if(_maticDeposit >= 2501){
            vel = 116000; //1%
        }
