...
contract AssetTokenIssuer is AccessControlUpgradeable {
    ...
    function setInterestRate(uint256 _interestRate, bool _positiveInterest) external requireAdmin {
        _getAssetTokenData().setInterestRate(assetTokenAddress, _interestRate, _positiveInterest);
    }

    function transferIssuer(address newIssuer) public requireAdmin {
    ...
    function _getAssetTokenData() private view returns (IAssetTokenData) {
        return IAssetTokenData(IAssetToken(assetTokenAddress).assetTokenDataAddress());
    }
}
