...
contract PaybNftMarketplace is IPaybNftMarketplace, ERC721, IERC721Pausable{
    ...
    function getAddressPool() public view returns(address){
        return PaybPool;
    }
    ...
    function mintFragment(address _to,uint256 _rootTokenId) public override onlyAdmin() onlyLaunchPadContract(){
    ...
    function buy(uint256 _tokenId) public payable override tokenNotFound(_tokenId) isUnlock(_tokenId) isRootToken(_tokenId){
        ...
        if(tokenAddress[_tokenId] == address(0)) {
            ...
            if(_subOwnerFee > 0){
                IPaybNftPool(PaybPool).subOwnerFeeBalance{value: _subOwnerFee}();
            }
            ...
            if(_fragmentTokens[_tokenId]){
                ...
                uint256 _amountEarn = _subOwnerFee.div(_subOwners[_rootId].length-1 > 0 ? _subOwners[_rootId].length-1 : 1);
                for(uint256 i=0; i< _subOwners[_rootId].length; ++i){
                    if(_subOwners[_rootId][i] != _owner){
                        amountEarn[_subOwners[_rootId][i]][tokenAddress[tokenId]] += _amountEarn;
                    }
    ...
    function sendTo(uint256 _tokenId, address payable _to) public onlyAdmin() tokenNotFound(_tokenId) payable{
        ...
        if(_token == address(0))
            require(_price == msg.value, "Value != price!");
    ...
    function setDefaultAmountEarn(address _user, address _tokenAddress) external override{
        require(msg.sender == getAddressPool(), "Isn`t Pool");
...
contract ERC721 is Context, ERC165, IERC721, IERC721Enumerable, IERC721Metadata, ILockable, ERC721Receiver {
    ...
    modifier onlyOwner() {
     ...
     modifier onlyAdmin() {
        require(_admins[msg.sender] || owner == msg.sender, "Only admin or owner");
        _;
    }

    modifier onlyLaunchPadContract(){
        require(_launchPad == msg.sender, "Only launchpad contract");
        _;
    }
