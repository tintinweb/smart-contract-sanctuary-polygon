...
// File: contracts/IAssetPack.sol


pragma solidity ^0.8.7;

interface IAssetPack {
  enum PackType {
    BOX,
    LOCKBOX,
    BUNDLE
  ...
  event PackCreated(uint256 indexed packId);
  event PackOpened(
...
// File: contracts/utils/abstracts/WithPacksUpgradeable.sol
...
- NT = No Tokens
- II = Invalid Item
- ITR = Invalid Tokens Remaining
- IMS = Invalid Max Supply
- DNE = Does Not Exist
- EOA = Must be externally owned account
*/

abstract contract WithPacksUpgradeable is
  IAssetPack,
  TeamMembersUpgradeable,
  ERC1155BurnableUpgradeable
{
  mapping(uint256 => Pack) internal _packs;
  uint256 internal _nextTokenIndex;

  function createPack(
    PackItem[] calldata _tokens,
    PackType _packType,
    string calldata _uri,
    bool _closed,
    uint256 _maxSupply,
    uint256 _maxPerWallet,
    PackItem calldata _tokenKey
  ) external onlyTeamOrOwner {
    require(_tokens.length > 0, "NT");

    Pack storage info = _packs[_nextTokenIndex];
    info.packType = _packType;
    info.closed = _closed;
    info.uri = _uri;
    info.maxPerWallet = _maxPerWallet;
    info.maxSupply = _maxSupply;
    info.tokenKey = _tokenKey;

    PackItem memory firstToken = _tokens[0];
    uint256 _supplyNeeded;
    for (uint256 i; i < _tokens.length; i++) {
      PackItem memory _item = _tokens[i];
      if (_packType == PackType.BUNDLE) {
        require(
          (_item.remaining / _item.amount) ==
            (firstToken.remaining / firstToken.amount),
          "II"
        );
      }
      require(_item.remaining % _item.amount == 0, "ITR");
      if (_item.itemType == ItemType.ERC1155) {
        IERC1155Upgradeable(_item.tokenAddress).safeTransferFrom(
          msg.sender,
          address(this),
          _item.tokenId,
          _item.remaining / 1 ether,
          ""
        );
      } else {
        IERC20(_item.tokenAddress).transferFrom(
          msg.sender,
          address(this),
          _item.remaining
        );
      }
      info.tokens.push(_item);
      info.tokensRemaining += _item.remaining;
      _supplyNeeded += _item.remaining / _item.amount;
    }
    if (_packType == PackType.BUNDLE) {
      require((firstToken.remaining / firstToken.amount) == _maxSupply, "IMS");
    } else {
      require(_supplyNeeded == _maxSupply, "IMS");
    }

    emit PackCreated(_nextTokenIndex);
    _nextTokenIndex += 1;
  }
  ...
  function updatePack(
    ...
    require(exists(_packId), "DNE");
    _packs[_packId].uri = _uri;
    _packs[_packId].closed = _closed;
    _packs[_packId].maxPerWallet = _maxPerWallet;
  }

  function genRewardTokens(Pack storage pack)
    internal
    returns (uint256[] memory tokenIndexes)
  {
    if (pack.packType == PackType.BUNDLE) {
      uint256[] memory _tokenIndexes = new uint256[](pack.tokens.length);
      for (uint256 i; i < pack.tokens.length; i += 1) {
        pack.tokens[i].remaining -= pack.tokens[i].amount;
        pack.tokensRemaining -= pack.tokens[i].amount;
        _tokenIndexes[i] = i;
      }
      ...
      uint256 target = _rand % pack.tokensRemaining;
      uint256[] memory _tokenIndexes = new uint256[](1);
      for (uint256 i; i < pack.tokens.length; i += 1) {
        PackItem memory _token = pack.tokens[i];
        if (_token.remaining > 0 && target <= offset + _token.remaining) {
          pack.tokens[i].remaining -= _token.amount;
          pack.tokensRemaining -= _token.amount;
          _tokenIndexes[0] = i;
          break;
        } else {
          offset += _token.remaining;
        }
      }
      return _tokenIndexes;
    }
  ...
  function openPack(uint256 _packId) external {
    ...
    Pack storage pack = _packs[_packId];

    _burn(msg.sender, _packId, 1);
    if (pack.packType == PackType.LOCKBOX) {
      ERC1155BurnableUpgradeable(pack.tokenKey.tokenAddress).burn(
        msg.sender,
        pack.tokenKey.tokenId,
        pack.tokenKey.amount / 1 ether
      );
    }

    uint256[] memory tokenIndexes = genRewardTokens(pack);

    _transferTokenBatch(address(this), msg.sender, pack.tokens, tokenIndexes);
    emit PackOpened(_packId, msg.sender, 1, tokenIndexes);
  }

  function getPack(uint256 _tokenId) public view returns (Pack memory) {
    require(exists(_tokenId), "DNE");
    return _packs[_tokenId];
  }

  function getPackCount() public view returns (uint256) {
  ...
  function _transferTokenBatch(
    address _from,
    address _to,
    PackItem[] memory _tokens,
    uint256[] memory _indexes
  ) internal {
    for (uint256 i; i < _indexes.length; i += 1) {
      PackItem memory _token = _tokens[_indexes[i]];
      if (_token.itemType == ItemType.ERC1155) {
        IERC1155Upgradeable(_token.tokenAddress).safeTransferFrom(
          _from,
          _to,
          _token.tokenId,
          _token.amount / 1 ether,
          ""
        );
      } else {
        IERC20(_token.tokenAddress).transfer(_to, _token.amount);
      }
    }
  }
}
...
// File: contracts/ERC1155AssetsPackUpgradeable.sol
...
contract ERC1155AssetsPackUpgradeable is
  ERC1155HolderUpgradeable,
  IERC2981Upgradeable,
  WithPacksUpgradeable,
  WithdrawableUpgradeable,
  WithRoyaltyUpgradeable,
  SignatureMintableUpgradeable
{
  mapping(address => mapping(uint256 => uint256)) private minted;

  function __ERC1155AssetsPackUpgradeable_init(
  ...
  function uri(uint256 _tokenId) public view override returns (string memory) {
    return _packs[_tokenId].uri;
  }

  modifier beforeMint(MintTokenSignature calldata mintToken) {
    require(mintToken.to == msg.sender, "NS");

    if (mintToken.currency == address(this)) {
      require(msg.value >= mintToken.cost, "IC");
    } else {
      IERC20(mintToken.currency).transferFrom(
        msg.sender,
        address(this),
        mintToken.cost
      );
    ...
    for (uint256 i = 0; i < mintToken.tokenIds.length; i++) {
      Pack storage _pack = _packs[mintToken.tokenIds[i]];
      require(_pack.minted + mintToken.counts[i] <= _pack.maxSupply, "SO");
    }
  ...
  function supportsInterface(bytes4 interfaceId)
    public
    view
    override(ERC1155Upgradeable, IERC165Upgradeable, ERC1155ReceiverUpgradeable)
    returns (bool)
  {
    return
  ...
  function _beforeTokenTransfer(
    ...
    for (uint256 i = 0; i < ids.length; i++) {
      if (from == address(0)) {
        require(!_packs[ids[i]].closed, "Closed");

        minted[to][ids[i]] = minted[to][ids[i]] + amounts[i];
        require(minted[to][ids[i]] <= _packs[ids[i]].maxPerWallet, "MPW");
        _packs[ids[i]].minted = _packs[ids[i]].minted + amounts[i];
      }
    }
  ...
  {
    require(exists(0), "NP");
    uint256[] memory balances = new uint256[](getPackCount());
    for (uint256 i = 0; i < getPackCount(); i++) {
...
// File: contracts/ERC1155AssetsPackImpl.sol




contract ERC1155AssetsPackImpl is
  ERC1155AssetsPackUpgradeable,
  UUPSUpgradeable
{
  function initialize(address _signerAddress, uint256 _commission)
    ...
    initializer
  {
    __ERC1155AssetsPackUpgradeable_init(_signerAddress, _commission);
  }
