...
contract FootBallERC721 is ERC721 , Ownable{
    ...
    function mint(address _to, uint256 _mintAmount) public payable {
        ...
        require(_mintAmount <= maxPurchase,string(abi.encodePacked("You can only mint maximum of ", maxPurchase, "NFTs!")));
        require(supply + _mintAmount <= maxSupply, "Purchase would exceed max supply!");
        
        // Can be removed?? 
        require(balanceOf(msg.sender) + _mintAmount <= maxPurchase, string(abi.encodePacked("You can only mint maximum of ", maxPurchase, "NFTs!")));

        if (msg.sender != owner()) {
            require(saleIsActive, "Minting is not active now!");
            require(msg.value >= cost * _mintAmount, "Ether value sent is not correct");
            if (!whitelistedAddresses[msg.sender]) {
                require(publicSaleIsActive, "Public minting is not active now!");
            }
        }
    ...
    function setTokenURI(uint256 _tokenId, string memory _tokenURI) public onlyOwner {
        //add freezing option for fans don't want to update.
        require(_exists(_tokenId), "ERC721: URI set of nonexistent token");
        tokenURIs[_tokenId] = _tokenURI;
    }

 
    function batchSetTokenURI(uint256[] memory _tokenId, string[] memory _tokenURI) public onlyOwner {
        //loop on _setTokenURI(_tokenId, _tokenURI);
    }
