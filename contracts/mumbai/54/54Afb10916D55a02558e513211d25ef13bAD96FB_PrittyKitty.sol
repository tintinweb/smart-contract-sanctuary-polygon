...
contract PrittyKitty is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    // public
    function mint(uint256 _mintAmount, uint256 _level) external {
        ...
        if (msg.sender != owner()) {
            ...
            uint256 price = prices[_level];

            //check if already minted the required level
            require(checkQualifiedLevel(msg.sender,_level) == true, "Not Qualified for Level");
            ...
            if (allowFreeMint == true && isFreeMinted(msg.sender) == true) {
                ...
                uint256 cost = price * _mintAmount;
                require(wethContract.allowance(msg.sender,address(this)) >= cost, "Incorrect allowance");
                require(balance >= cost, "Insuficient ETH balance");
                wethContract.transferFrom(msg.sender, address(this), cost);
            }
        }
    ...
    function levelOfOwner(address _owner) public view returns (uint256[] memory){
    ...
    function checkQualifiedLevel(address _owner, uint256 _level) public view returns (bool)
    {
        if(_level == 1){
            return true;
        } else {
            ...
            for (uint256 i; i < ownerTokenCount; i++) {
                if(tokenLevel[tokenOfOwnerByIndex(_owner, i)] >= _level - 1 && tokenLevel[tokenOfOwnerByIndex(_owner, i)] <= _level){
                    isQualified = true;
                }
            }
            return isQualified;
    ...
    //only owner
    function addPrice(uint256 _level, uint256 _price) public onlyOwner {
        prices[_level] = _price;
    }
    function deletePrice(uint256 _level) public onlyOwner {
        delete prices[_level];
    }

    function setReveal(bool _value) public onlyOwner {
        revealed = _value;
    }
