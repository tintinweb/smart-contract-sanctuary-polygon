...
interface ICraftLogic {
    function craft(uint256 recipeId) external;

    function craftByRelayer(uint256 recipeId) external;
}
...
interface IPhiDaily {
    ...
    function claimMaterialObjectByRelayer(uint32 eventid, uint16 logicid, Coupon memory coupon) external;
...
contract PhiGelatoRelay is ReentrancyGuard, ERC2771Context {
    ...
    constructor(address trustedForwarder, address _phidaily, address _craftLogic) ERC2771Context(trustedForwarder) {
        if (_phidaily == address(0)) {
            revert PhiDailyNotSet();
        }
        if (_craftLogic == address(0)) {
            revert craftLogicNotSet();
        }
        phiDaily = _phidaily;
    ...
    function claimMaterialObjectByRelayer(
        ...
        IPhiDaily(phiDaily).claimMaterialObjectByRelayer(eventid, logicid, coupon);
    ...
    function batchClaimMaterialObjectByRelayer(
        ...
        IPhiDaily(phiDaily).batchClaimMaterialObjectByRelayer(eventids, logicids, coupons);
        emit LogBatchClaimMaterialObjects(eventids, logicids, _msgSender());
    }

    /// @dev Allows user to craft objects via a relayer.
    function craftByRelayer(uint256 recipeId) external nonReentrant {
        ICraftLogic(craftLogic).craftByRelayer(recipeId);
