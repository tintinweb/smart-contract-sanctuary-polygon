...
contract CalvariaMysteryBox is ERC721Enumerable, Ownable {
    uint public mintPrice = 0 ether;
    uint public maxItems = 1000;
    ...
    constructor(uint startTime) ERC721("Calvaria Mystery Box", "CALVARIABOX") {
    ...
    function publicMint(uint amount) external payable {
        ...
        if(mintPrice > 0) require(msg.value / amount == mintPrice, "The ETH amount does not match the desired tokens amount");

        mintWithoutValidation(_msgSender(), amount);
    }

    function giveawayMint(address to, uint amount) external onlyOwner {
        mintWithoutValidation(to, amount);
    }

    function mintWithoutValidation(address to, uint amount) internal {
        require(totalSupply() + amount <= maxItems, "Over the maximum allowed total supply");
    ...
    function setMaxItems(uint _maxItems) external onlyOwner {
        require(maxItems != _maxItems, "Attempt to set the same value");
        maxItems = _maxItems;
    }
