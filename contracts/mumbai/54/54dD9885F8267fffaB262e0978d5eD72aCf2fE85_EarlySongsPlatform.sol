...
contract EarlySongsPlatform is ReentrancyGuard {
    ...
    function mintNewItem(uint256 itemId, string memory tokenURI, uint256 mintPrice) public payable nonReentrant returns (uint256) {
        require(mintPrice == idToPlatformItem[itemId].mintPrice, "Price must be equal to item's mint price");
        ...
        uint256 newTokenId = nftContract.mint(msg.sender, tokenURI);
...
contract ShareableERC721 is ERC721URIStorage, Ownable, IERC5023 {
    ...
    function mint(address account, string memory tokenURI) external onlyPlatform onlyLowerThanMaxSupply returns (uint256) {
