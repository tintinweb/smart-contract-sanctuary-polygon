...
contract TestMinter is Ownable {
    ...
    // The root hash of the Merkle Tree previously generated for the whitelist.
    bytes32 public merkleRoot;

    // Mapping variable to mark whitelist addresses as having claimed.
    mapping(address => uint256) public whitelistCountClaimed;
    ...
    modifier mintCompliance(uint256 _mintAmount) {
        ...
        require(
            token.totalSupply() + _mintAmount <= maxSupply,
            "Max supply exceeded!"
        );
        _;
    }
    ...
    function mint(
        ...
        require(
            whitelistCountClaimed[msg.sender] + _mintAmount <= _whitelistAmount,
            "Address has already claimed."
        );

        // Verify the provided _merkleProof, given to us through the API call on our website.
        bytes32 leaf = keccak256(
            abi.encodePacked(msg.sender, _whitelistAmount, _hasFreeMint)
        );
        ...
        // Add amount to address that claimed their token.
        whitelistCountClaimed[msg.sender] += _mintAmount;

        // Free mint
        if (_hasFreeMint && !freeMintClaimed[msg.sender]) {
            _mintAmount += 1;
