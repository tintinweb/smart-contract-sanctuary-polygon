    ...
    UUPSUpgradeable
{
    ...
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LCT: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LCT: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LCT: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCT: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
        vault = vaultAddress;
        initializeERC2771(address(forwarder));

        __ERC20_init('Lost Chain Token', 'LCT');
        __ERC20Burnable_init();
        __UUPSUpgradeable_init();
        _mint(vault, 1000000000 * 10**decimals());
    }
    ...
    function setLCGD(address gameDataAddress) public onlyRole('DEFAULT_ADMIN_ROLE', msg.sender) {
        require(gameDataAddress != address(0x0), 'LCT: Game data is zero address');
    ...
    function gameDataTokenTransfer(
        ...
        require(_msgSender() == LCGD, 'LCT: caller is not LCGD');
