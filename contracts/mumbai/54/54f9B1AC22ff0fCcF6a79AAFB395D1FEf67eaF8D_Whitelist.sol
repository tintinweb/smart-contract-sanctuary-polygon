...
contract Whitelist is Ownable {
    ...
    function removeFromList(address addr) external onlyOwnerOrOperator {
    ...
    function batchAddWhitelist(address[] memory addrs) external onlyOwner {
    ...
    function batchRemoveWhitelist(address[] memory addrs) external onlyOwner {
        for (uint256 i = 0; i < addrs.length; i++) {
            if (whitelist[addrs[i]]) {
                removeWhitelist(addrs[i]);
            }
        }
    }
}
