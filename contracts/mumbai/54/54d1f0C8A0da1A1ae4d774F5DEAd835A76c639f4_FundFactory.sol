...
contract FundFactory {
    ...
    constructor(address _fundLogicAddress, address _usdcToken, address _zeroExchange) {
    ...
    function withdrawFees() external onlyOwner {
        ERC20 usdcToken = ERC20(USDC_TOKEN);
        uint256 balance = usdcToken.balanceOf(address(this));
        require(balance >= 0, "balance < 0");
        
        usdcToken.safeTransfer(PROTOCOL_OWNER, balance);
    }
    ...
    function addTokenToWhiteList(uint256 _whitelistIndex, string memory _name, string memory _symbol, address _address) onlyWhitelistOwner(_whitelistIndex) external {
    ...
    function createFund(
        ...
        require(bytes(_tokenName).length > 0, "The name is mandatory");
        require(bytes(_tokenSymbol).length > 0, "The symbole is mandatory");
        if (!_useWhitelist) {
            _whitelistId = 0;
        }

        FundProxy proxy = new FundProxy(
            FUND_LOGIC,
            abi.encodeWithSelector(
                FundLogic.initialize.selector,
                msg.sender,
                _fundPaymentFees,
                _fundManagmentFees,
                USDC_TOKEN,
        ...
        emit FundProxyDeployed(msg.sender, address(proxy), _tokenName, _tokenSymbol);

        fundsList.push(address(proxy));
    ...
    function getFactoryInformations() public view returns (address, address, address, address, uint256, uint256) {
        return (address(this), FUND_LOGIC, USDC_TOKEN, PROTOCOL_OWNER, getFundCount(), ERC20(USDC_TOKEN).balanceOf(address(this)));
    }
...
contract FundLogic is Initializable, TokenLogic, AccessControlEnumerableUpgradeable {
    ...
    uint256 private constant feeBpsForProtocol = 60; // 0,6%
    uint256 private constant feeDepositForProtocol = 50; // 0,5%
    uint256 private feeBpsForFund;
    uint256 private feeDepositForFund;
    uint256 private fundLastPaid;
    uint256 private whitelistId;
    ...
    event FeesPaidForMinting(uint256 mintingFeeForProtocol, uint256 mintingFeeForOwner, uint256 secondsPaid);
    event FeesPaidForManagment(uint256 managmentFeeForProtocol, uint256 managmentFeeForFundOwner, uint256 secondsPaid);
    event SharesMinted(uint256 sharesAmount, uint256 investmentAmount, address indexed owner);
    event InvestmentRedeemed(address indexed owner, uint256 amount);
    ...
    function investInFund(uint256 _investmentAmount) external returns (uint256) {
        ...
        // pay fees
        bool feesPayed = __payFees(_investmentAmount);
        ...
        // Mint share for user
        uint256 shareToMint = __sharesToMint(_investmentAmount);
        // Investment
        usdcToken.transferFrom(msg.sender, address(this), _investmentAmount);
        _mint(msg.sender, shareToMint);

        emit SharesMinted(shareToMint, _investmentAmount, msg.sender);
        
        return shareToMint;
    }

    /**
     * @notice function to redeem shares of the fund
     * @param _shares The number of shares that the user want to redeem
     */
    function redeemInvestment(uint256 _shares) external returns (bool) {
        require(_shares > 0, "_shares need to be >0");
        require(balanceOf(msg.sender) >= _shares, "user shares < shares wanted");
        ...
        ERC20 usdcToken = ERC20(USDC_TOKEN);
        uint256 tokenToRedeem = __tokenToRedeem(_shares);
        require(getAssetBalance() >= tokenToRedeem, "fund balance to low for redeem");

        usdcToken.safeTransfer(msg.sender, tokenToRedeem);
        _burn(msg.sender, _shares);

        emit InvestmentRedeemed(msg.sender, _shares);
    ...
    function fillQuote(
    ...
    ) external onlyRole(MANAGER) payable returns (bool) {
        ...
        if (useWhitelist) {
            ...
            require(fundFactory.doesTokenExist(whitelistId, address(_sellToken)), "sellToken not approved");
            // if its the case, check if _sellToken is inside
            require(fundFactory.doesTokenExist(whitelistId, address(_buyToken)), "buyToken not approved");
        }
        ...
        require(__approveAssetMaxAsNeeded(_spender, address(_sellToken)), "error with allowance");
        /*
        address _asset,
        address _target,
     ...
     */
    function __payFees(uint256 _investmentAmount) private returns(bool) {
        ERC20 usdcToken = ERC20(USDC_TOKEN);
        uint256 lastPaid = getFundLastPaid();
        uint256 secondsDue = block.timestamp.sub(lastPaid);
        uint256 assetBalance = getAssetBalance();

        // get protocole fee due for gestion from fund
        uint256 managmentFeeForProtocol = __calcFeeDueForProtocol(lastPaid, secondsDue, assetBalance);

        // get fund fee due for gestion from fund
        uint256 managmentFeeForFundOwner = __calcFeeDueForFund(lastPaid, secondsDue, assetBalance);

        // make all transfert for diffrent fee (to fund and to protocol)
        // Managment fees
        require(assetBalance >= managmentFeeForProtocol.add(managmentFeeForFundOwner), "Insufficient funds to pay fees");
        usdcToken.transfer(FUND_FACTORY, managmentFeeForProtocol);
        usdcToken.transfer(OWNER, managmentFeeForFundOwner);

        emit FeesPaidForManagment(managmentFeeForProtocol, managmentFeeForFundOwner, secondsDue);

        // Minting fees
        if (_investmentAmount > 0) {
            // get fund fee due for investment from user
            uint256 mintingFeeForOwner = _investmentAmount.div(10000).mul(getFeeDepositForFund());

            // get protocole fee due for investment from user
            uint256 mintingFeeForProtocol = _investmentAmount.div(10000).mul(getFeeDepositForProtocol());

            // check that user has enough usdc for fee + investment
            uint256 total = _investmentAmount.add(mintingFeeForOwner).add(mintingFeeForProtocol);
            uint balance = usdcToken.balanceOf(msg.sender);
            require(balance >= total, "Insufficient funds for transfer");

            // check the allowance
            uint256 allowed = usdcToken.allowance(msg.sender, address(this));
            require(allowed >= total, "Allowance to low for transfer");

            // Minting fees
            usdcToken.transferFrom(msg.sender, FUND_FACTORY, mintingFeeForProtocol);
            usdcToken.transferFrom(msg.sender, OWNER, mintingFeeForOwner);

            emit FeesPaidForMinting(mintingFeeForProtocol, mintingFeeForOwner, secondsDue);
        }
    ...
    function __calcFeeDueForFund(uint256 _lastPaid, uint256 _secondsDue, uint256 _assetBalance) private view returns (uint256 feeDue_) {
        uint256 rawTokensDue = 0;
        if (_lastPaid < block.timestamp) {
            rawTokensDue = _assetBalance
                .mul(getFeeBpsFund())
                .mul(_secondsDue)
                .div(SECONDS_IN_YEAR)
                .div(MAX_BPS);
        ...
        uint256 supplyNetRawTokensDue = _assetBalance.sub(rawTokensDue);
        feeDue_ = 0;
        if (supplyNetRawTokensDue != 0) {
            feeDue_ = rawTokensDue.mul(_assetBalance).div(supplyNetRawTokensDue);
        }
        return feeDue_;
    }
    
    function __calcFeeDueForProtocol(uint256 _lastPaid, uint256 _secondsDue, uint256 _assetBalance) private view returns (uint256 feeDue_) {
        uint256 rawTokensDue  = 0;
        if (_lastPaid < block.timestamp) {
        rawTokensDue = _assetBalance
    ...
    function __sharesToMint(uint256 _amountToConvert) private view returns (uint256 sharesToMint_) {
        ...
        uint256 denominationAssetBalance = getAssetBalance();
        uint256 shareDecimal = decimals();
        uint256 denominationAssetDecimal = ERC20(USDC_TOKEN).decimals();
        uint256 sharesUnit = 10 ** uint256(shareDecimal);

        if (sharesSupply == 0) {
            // sharesToMint_ = 10**uint256(ERC20(USDC_TOKEN).decimals());
        ...
        denominationAssetBalance = denominationAssetBalance.mul(10**uint256(decimalDifference));

        uint256 sharePrice = denominationAssetBalance.div(sharesSupply);
        sharesToMint_ = _amountToConvert.mul(10**uint256(decimalDifference)).div(sharePrice);

        return sharesToMint_;
    }

    function __tokenToRedeem(uint256 _sharesToRedeem) private view returns (uint256 tokenToRedeem_) {
        uint256 sharesSupply = totalSupply();
        uint256 denominationAssetBalance = getAssetBalance();

        tokenToRedeem_ = denominationAssetBalance.mul(_sharesToRedeem).div(sharesSupply);
    ...
    function __removeTrackedAsset(address _asset) private notShares(_asset) {
        if (isTrackedAsset(_asset)) {
            ...
            address lastElementAddress = trackedAssets[trackedAssets.length - 1];
     ...
     */
     function __isAssetNeedToBeTracked(address _asset) private view notShares(_asset) returns (bool) {
    ...
    function __approveAssetMaxAsNeeded(
        ...
        uint256 tokenAllowance = ERC20(_asset).allowance(address(this), _spender);
        // if (tokenAllowance < _neededAmount) {
            if (tokenAllowance > 0) {
                ERC20(_asset).safeApprove(_spender, 0);
            }
            ERC20(_asset).safeApprove(_spender, type(uint256).max);
        // }
        return true;
    }
    ...
    /**
     * @dev function to the get the contract's balance of assets
     */
    function getAssetBalance() public view returns (uint256 balance_) {
        return ERC20(USDC_TOKEN).balanceOf(address(this));
    }
     ...
     */
    function getWhitelistId() public view returns (uint256) {
        return whitelistId;
    }
    ...
    */
    function getFundInformations() public view returns(string memory, string memory, uint256, address, uint256, uint256, uint256, uint256, uint256, address[] memory, bool, uint256) {
        return (name(), symbol(), totalSupply(), OWNER, feeBpsForFund, feeDepositForFund, feeBpsForProtocol, feeDepositForProtocol, getAssetBalance(), getTrackedAssets(), useWhitelist, getWhitelistId());
    }
