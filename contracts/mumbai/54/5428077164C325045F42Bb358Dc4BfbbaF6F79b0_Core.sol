...
contract Core is Initializable, Proxied, Governed, AccessControl, ReentrancyGuard, ICoreContract {
    ...
    function nextWorkflowStage() external override onlyGovernor {
        ...
        if (currentWorkflow == Workflow.SaleOpen) {
            ...
            IMetaPayment(registry.getMetaPayment()).setFreezeStatus(registry.getMFS(), false);
        }
    ...
    function giveMFSFromPool(uint256 userId, uint256 amount) external override onlyMetaForceContractRole {
        ...
        address metaPool = tempRegistry.getMetaPool();
        IMFS mfs = IMFS(tempRegistry.getMFS());
        IMetaPayment payment = IMetaPayment(tempRegistry.getMetaPayment());
    ...
    function distibuteMFSEmission() external override nonReentrant {
        IMFS mfsToken = IMFS(registry.getMFS());

        if (mfsToken.emissionCommitted()) {
            revert MetaForceSpaceCoreEmissionCommitted();
        }
        IMetaPayment(registry.getMetaPayment()).setFreezeStatus(address(mfsToken), true);
        ...
        uint256 sLiquidity = cachedCap.mul(SHARE_OF_LIQUIDITY_POOL);
        mfsToken.mint(registry.getLiquidityPool(), sLiquidity);
        tempCap -= sLiquidity;

        uint256 sParticipants = cachedCap.mul(SHARE_OF_FORSAGE_PARTICIPANTS);
        mfsToken.mint(registry.getForsageParticipants(), sParticipants);
        tempCap -= sParticipants;
