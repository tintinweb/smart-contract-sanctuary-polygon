...
interface ICompliance {
    ...
    function addSubOwnerOnCompliance(address _userAddress) external;
...
import 'contracts/interface/IIdentityRegistryStorage.sol';
...
function addSubOwnerOnIR(address _userAddress) external;
...
contract AgentRoleUpgradeable is OwnableUpgradeable, AccessControl {
    ...
    // bytes32 public constant Sub_Admin = keccak256("Sub_Admin");
    ...
    address[] private representatives;
    mapping(address => address[]) public totalRepresentatives;
    mapping(address => bool) public agentBlockStatus;

    event AgentAdded(address indexed _agent);
    event AgentRemoved(address indexed _agent);
    event TokenizationAgentAdded(address indexed _agent);
    event TokenizationAgentRemoved(address indexed _agent);


    Roles.Role private _agents;
    Roles.Role private _tokenizationAgents;

    modifier Role(bytes32 role) {
        require(hasRole(role, msg.sender),"Not an Agent or has any Role ");
        _;
    }

    modifier onlyAdmins{
        require(hasRole(SubAdmin_Role, msg.sender) || hasRole(Owner_Role, msg.sender), 'You Dont Have Admin Role');
        _;
    }
    ...
    function _setAdminRole(address _owner) internal {
        _setupRole(SubAdmin_Role, _owner);    }

    function addSubAdmin(address _userAddress) public onlyAdmins {
        _setupRole(SubAdmin_Role, _userAddress);
    }

    function revokeSubAdmin(address _SubOwner) public onlyAdmins {
        require(isSubAdmin(_SubOwner), "Doesn't have Owner Role");
        _revokeRole(SubAdmin_Role, _SubOwner);
    }

    function isSubAdmin (address _userAddress) public view returns (bool){
        return hasRole(SubAdmin_Role, _userAddress);
    }

    function setRolesToAgent(address _userAddress, bytes32[] memory roles) public onlyAdmins{
        require(roles.length <= 5, "Maximum 5 roles are accepted for now");
        for(uint i = 0; i < roles.length; i++)
        {
            _setupRole(roles[i], _userAddress);
        }
    }

    function revokeRolesOnAgent(address _userAddress, bytes32[] memory roles) public onlyAdmins{
        for(uint i=0; i < roles.length; i++)
        {
            if(hasRole(roles[i], _userAddress))
            {
                _revokeRole(roles[i], _userAddress);
            }
        }
    }
    ...
    function addAgent(address _agent) public onlyAdmins {
        _agents.add(_agent);
        emit AgentAdded(_agent);
    }

    function removeAgent(address _agent) public onlyAdmins {
        _agents.remove(_agent);
        emit AgentRemoved(_agent);
    }

     function addIssuer(address _userAddress) public onlyRole(Owner_Role) {
        ...
        setRolesToAgent(_userAddress, array);
        _setupRole(Owner_Role, _userAddress);
    }

    function revokeIssuer(address _issuerAddress) public onlyRole(Owner_Role) {
        ...
        array[3] = UnFreezer_Role;
        array[4] = ForceTransfer_Role;
        revokeRolesOnAgent(_issuerAddress, array);
        _revokeRole(Owner_Role, _issuerAddress);
    }

    function isIssuer (address _userAddress) public view returns (bool){
        return hasRole(Owner_Role, _userAddress);
    }

    

    function isTransferAgent(address _agent) public view returns (bool) {
        return _agents.has(_agent);
    }

    function addTransferAgent(address _agent) public onlyRole(Owner_Role) {
        _agents.add(_agent);
        bytes32[] memory array = new bytes32[](3);
        array[0] = Freezer_Role;
        array[1] = UnFreezer_Role;
        array[2] = ForceTransfer_Role;
        setRolesToAgent(_agent, array);
        emit AgentAdded(_agent);
    }

    function removeTransferAgent(address _agent) public onlyRole(Owner_Role) {
        _agents.remove(_agent);
        bytes32[] memory array = new bytes32[](2);
        array[0] = Freezer_Role;
        array[1] = UnFreezer_Role;
        array[2] = ForceTransfer_Role;
        revokeRolesOnAgent(_agent, array);
        emit AgentRemoved(_agent);
    }

    function isTokenizationAgent(address _agent) public view returns (bool) {
        return _tokenizationAgents.has(_agent);
    }

    function addTokenizationAgent(address _agent) public onlyRole(Owner_Role) {
        _tokenizationAgents.add(_agent);
        bytes32[] memory array = new bytes32[](2);
        array[0] = Minter_Role;
        array[1] = Burner_Role;
        setRolesToAgent(_agent, array);
        emit TokenizationAgentAdded(_agent);
    }

    function removeTokenizationAgent(address _agent) public onlyRole(Owner_Role) {
        _tokenizationAgents.remove(_agent);
        bytes32[] memory array = new bytes32[](2);
        array[0] = Minter_Role;
        array[1] = Burner_Role;
        revokeRolesOnAgent(_agent, array);
        emit TokenizationAgentRemoved(_agent);
    }

    // function isAgentBlocked(address _agentAddress) view public returns(bool){
    //     return agentBlockStatus[_agentAddress];
    // }

    // function blockAgents(address _tokenizationAgent,bytes32[] memory _roles) public onlyRole(Owner_Role){
    //     require(!agentBlockStatus[_tokenizationAgent],"Agent already Blocked");
    //     revokeRolesOnAgent(_tokenizationAgent, _roles);
    //     for(uint i =0 ; i <= totalRepresentatives[_tokenizationAgent].length; i++){
    //         revokeRolesOnAgent(totalRepresentatives[_tokenizationAgent][i], _roles);
    //     }
    //     agentBlockStatus[_tokenizationAgent] = true;
    // }

    // function unBlockAgent(address _tokenizationAgent, bytes32[] memory _roles) public onlyRole(Owner_Role){
    //     require(agentBlockStatus[_tokenizationAgent],"Agent is not Blocked");
    //     setRolesToAgent(_tokenizationAgent, _roles);
    //     for(uint i =0 ; i <= totalRepresentatives[_tokenizationAgent].length; i++){
    //         setRolesToAgent(totalRepresentatives[_tokenizationAgent][i], _roles);
    //     }
    //     agentBlockStatus[_tokenizationAgent] = false;
    // }

    // function addRepresentative(address _agent, address representative) public onlyRole(Owner_Role){
    //         require(isTokenizationAgent(_agent) || isTransferAgent(_agent),"Not the requried agent");
    //         if(isTokenizationAgent(_agent)){
    //             totalRepresentatives[_agent].push(representative);
    //             bytes32[] memory array = new bytes32[](2);
    //             array[0] = Minter_Role;
    //             array[1] = Burner_Role;
    //             setRolesToAgent(representative, array);
    //         }
    //         else if(isTransferAgent(_agent)){
    //             totalRepresentatives[_agent].push(representative);
    //             bytes32[] memory array = new bytes32[](3);
    //             array[0] = Freezer_Role;
    //             array[1] = UnFreezer_Role;
    ...
    // function removeRepresentative(address _agent, address representative) public onlyRole(Owner_Role){
...
contract TokenStorage {
    ...
    // uint internal transferLimit;
    // uint [] internal _blacklistCountries;
...
contract TokenST is IToken, AgentRoleUpgradeable, TokenStorage {
    ...
    function init(
        ...
        emit UpdatedTokenInformation(tokenName, tokenSymbol, tokenDecimals, tokenOnchainID, maxSupply);
        __Ownable_init_unchained();
    }
    ...
    /// @dev Modifier to make a function callable only when the agent got the Role.
    modifier gotRole(bytes32 role) {
        require(hasRole(role, msg.sender),"Doesn't have Role to this ");
        _;
    }
     ...
     */
    function setOnchainID(address _onchainID) external override onlyAdmins {
        tokenOnchainID = _onchainID;
        emit UpdatedTokenInformation(tokenName, tokenSymbol, tokenDecimals, tokenOnchainID, maxSupply);
    }
    ...
    function forcedTransfer(
        ...
        address _to,
        uint256 _amount
    ) public override gotRole(ForceTransfer_Role) returns (bool) {
     ...
     *  @dev See {IToken-mint}.
     */
    function mint(address _to, uint256 _amount) public override gotRole(Minter_Role){
     ...
     *  @dev See {IToken-burn}.
     */
    function burn(address _userAddress, uint256 _amount) public override gotRole(Burner_Role) {
     ...
     */
    function freezePartialTokens(address _userAddress, uint256 _amount) public override gotRole(Freezer_Role) {
     ...
     */
    function unfreezePartialTokens(address _userAddress, uint256 _amount) public override gotRole(UnFreezer_Role) {
