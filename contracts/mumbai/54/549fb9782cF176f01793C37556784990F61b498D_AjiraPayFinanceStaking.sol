...
// File contracts/AjiraPayFinanceStaking.sol
...
contract AjiraPayFinanceStaking is Ownable, AccessControl, Pausable, ReentrancyGuard{
    ...
    mapping(address => mapping(address => uint256)) public totalStakedForByHolder;
    ...
    struct NewStake{
        ...
        uint256 stakeDuration; //days
        uint256 deadline; //block.timestamp + stakeDurationInDays
        uint256 apr;
    ...
    struct StakeTier{
        uint256 id;
        uint256 apr;
        uint256 deadlineInDays; //days
        uint256 creationTimestamp;
        address creatorAccount;
        bool isPaused;
    }
    
    StakeTier public tier;
    ...
    event PauseStakingTier(
        address indexed caller,
        uint256 indexed tierId,
        uint256 indexed timestamp
    );

    event UnPauseStakingTier(
        address indexed caller,
        uint256 indexed tierId,
        uint256 indexed timestamp
    );
    ...
    event SetMinStakingPeriod(
        address indexed caller,
        uint256 indexed minDurationInDays,
        uint256 indexed timestamp
    );

    event SetMaxStakingPeriod(
        address indexed caller,
    ...
    function stake(uint256 _amount, uint256 _stakingTierId) external whenNotPaused nonReentrant {
        ...
        uint256 totalInvestorHoldings = _amount.add(holderStakedBalance);

        require(totalInvestorHoldings >= minStake && totalInvestorHoldings <= maxStake,"AJP Staking: AJP Staking: Stake Limit Reached");
        ...
        totalStakedTokens = totalStakedTokens.add(_amount); //_amount * 10 ** decimals
        ...
        _transferStakedTokens(msg.sender, _amount); //_amount * 10 ** decimals
    ...
    function withdrawSingleStake(uint256 _stakingId) external canClaimRewards(msg.sender) hasNotClaimedStakeId(msg.sender, _stakingId) whenNotPaused nonReentrant{
        ...
        _withdraw(msg.sender,totalAccumulatedReward);
    ...
    function pauseStakingTier(uint256 _tier) external onlyRole(MANAGER_ROLE){
        StakeTier storage _stakeTier = stakingTierById[_tier];
        _stakeTier.isPaused = true;
        emit PauseStakingTier(msg.sender, _stakeTier.id, block.timestamp);
    }

    function unpauseStakingTier(uint256 _tier) external onlyRole(MANAGER_ROLE){
        StakeTier storage _stakeTier = stakingTierById[_tier];
        _stakeTier.isPaused = false;
        emit UnPauseStakingTier(msg.sender, _stakeTier.id, block.timestamp);
    }

    function setMinStakeAmount(uint256 _amount) external onlyRole(MANAGER_ROLE) nonReentrant{
        require(_amount > 0,"AJP Staking: Min Stake Cannot be zero");
        minStake = _amount * 1e18;
        emit SetMinStake(msg.sender, _amount, block.timestamp);
    }

    function setMaxStakeAmount(uint256 _amount) external onlyRole(MANAGER_ROLE) nonReentrant{
        require(_amount > 0,"AJP Staking: Max Stake Cannot be zero");
        maxStake = _amount * 1e18;
        emit SetMaxStake(msg.sender, _amount, block.timestamp);
    }

    function setMinStakingPeriodInDays(uint256 _minStakingDays) external onlyRole(MANAGER_ROLE) nonReentrant{
        ...
        emit SetMinStakingPeriod(msg.sender, minStakingPeriod, block.timestamp);
    }

    function setMaxStakingPeriodInDays(uint256 _maxStakingDays) external onlyRole(MANAGER_ROLE) nonReentrant{
        ...
        emit SetMaxStakingPeriod(msg.sender, maxStakingPeriod, block.timestamp);
    }
    ...
    receive() external payable{
        revert("Cannot accept native token");
    }
