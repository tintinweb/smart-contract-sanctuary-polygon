  ...
  DefaultOperatorFilterer
{
  ...
  constructor() ERC721("Aether Games Ethernals", "AGETH") {
    _royaltyAmount = 500;
  ...
  {
    require(
      _typeIds.length == _uris.length && _typeIds.length == _rarities.length
      // _typeIds.length == _sbs.length &&
      // _typeIds.length == _paused.length
    );
  ...
  function _setType(
    ...
    // uint256 _price,
    string memory _uri,
    Library.Rarity _rarity // bool _sb, // bool _paused
  ) private {
  ...
  modifier onlyRole(bytes32 role) {
    require(roles[role][msg.sender], "Not authorized to ethernals.");
    _;
  }
