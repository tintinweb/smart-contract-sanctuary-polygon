...
contract Coin is Ownable{
    ...
    function preSale(address inviter, uint256 amount) external {
        ...
        uint256 tokenNum = amount.mul(priceDenominator).div(priceNumerator);
    ...
    function withdrawToken() external {
        ...
        require(block.timestamp > endTime, "Cannot take");
    ...
    function poolLeft() public view returns(uint256) {
        ...
        if (totalSupply > unableToken){
            return totalSupply.sub(unableToken);
        }
        return 0;
    }

    function getIdoInfos() external view returns(uint256[] memory, uint256[] memory, ERC20){
        ...
        array[6] = startTime;
        array[7] = endTime;
        return (array, generationFees, mToken);
    }
    ...
    function getUserInfos(address account) external view returns(uint256[] memory){
        ...
        array[1] = sales[account];
        array[2] = uToken.balanceOf(account);
        array[3] = uToken.allowance(account, address(this));
        return array;
    }
}