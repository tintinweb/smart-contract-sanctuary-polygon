...
contract WethReceiver is EIP712MetaTransaction {
  ...
  event ReceiveWethUserBehalf(address _from, string _productId, uint256 _amount);
  ...
  modifier onlyDbilia() {
    require(
      ...
      "WethReceiver#onlyDbilia: caller is not one of dbilia accounts"
    );
    _;
  }
 ...
 // Protect public function with passcode
  modifier verifyPasscode(bytes32 _passcode) {
    require(
      ...
      "WethReceiver#verifyPasscode: invalid passcode"
    );
    _;
  }
  ...
  {
    require(bytes(_productId).length > 0, "WethReceiver#receiveWeth: invalid _productId");
    require(_amount > 0, "WethReceiver#receiveWeth: invalid _amount");

    weth.safeTransferFrom(msgSender(), dbiliaToken.dbiliaFee(), _amount);

    emit ReceiveWeth(msgSender(), _productId, _amount);
  }

  function receiveWethUserBehalf(
    string memory _productId, 
    address _sender,
    uint256 _amount
  )
    ...
    onlyDbilia
  {
    require(bytes(_productId).length > 0, "WethReceiver#receiveWethUserBehalf: invalid _productId");
    require(_sender != address(0), "WethReceiver#receiveWethUserBehalf: _sender is empty");
    require(_amount > 0, "WethReceiver#receiveWethUserBehalf: invalid _amount");

    weth.safeTransferFrom(_sender, dbiliaToken.dbiliaFee(), _amount);
    
    emit ReceiveWethUserBehalf(_sender, _productId, _amount);
  }

  /**
   * Set passcode for wethReceiver contract
   *
