...
import {BasicOrderParameters} from "contracts/lib/seaport/contracts/lib/ConsiderationStructs.sol";
...
contract DelegationManager is Ownable, ReentrancyGuard {
    ...
    function createERC721Wrapper(IERC721Metadata _contract) external returns (address) {
        return _createERC721Wrapper(_contract);
    }
    ...
    function _getERC721WrapperAddress(IERC721Metadata _contract, uint256 _version) internal view returns (address) {
        require(
            _version < wrapperContracts[address(_contract)].length, 
            "DelegationManager: Wrapper version doesn't exist"
        );
    ...
    function getERC721WrapperAddress(IERC721Metadata _contract, uint256 _version) external view returns (address) {
        return _getERC721WrapperAddress(_contract, _version);
    }

    function addSubscriber(IERC721Metadata _contract, uint256 _version, uint256 tokenId, address subscriber) internal returns (bool) {
        address wrapperAddress = _getERC721WrapperAddress(_contract, _version);
        return IERC721Wrapper(wrapperAddress).setSubscriber(subscriber, tokenId);
    }

    function getSubscriber(IERC721Metadata _contract, uint256 _version, uint256 tokenId) external view returns (address){
        address wrapperAddress = _getERC721WrapperAddress(_contract, _version);
        return IERC721Wrapper(wrapperAddress).getSubscriber(tokenId);
    }
    ...
    function checkSignature(string memory message, bytes memory signature, address signer) external pure {
        _checkSignature(message, signature, signer);
    }
    ...
    function subscribeToMultiplePayments(uint256 tokenId, IERC721Metadata contractAddress, string memory scoringMessage, bytes memory scoringSig, address scoringAddress, address subscriberAddress, uint256 version, BasicOrderParameters calldata basicComponents) external onlyOwner {
        // check scoring API signature 
        _checkSignature(scoringMessage, scoringSig, scoringAddress);

        // buy NFT on marketplace (seaport for opensea)
        SETTLEMENTMANAGER.executeBuy(basicComponents);

        // get or create ERC721wrapper contract for NFT
        address wrapperAddress;
        wrapperAddress = _getERC721WrapperAddress(contractAddress, 0);
        if(wrapperAddress == address(0)) {
            wrapperAddress = _createERC721Wrapper(contractAddress);
        } 

        // add subscriber to mapping
        addSubscriber(contractAddress, version, tokenId, subscriberAddress);

        // transfer NFT to wrapperNFT corresponding
        

    }

    function payCreditStep(uint256 creditId, uint256 stepId) external payable {

    }

    function revokeSubcription () external view returns (uint256) {
    ...
    ReentrancyGuard
{
    ...
    /// @notice tokenId to subscriber
    mapping(uint256 => address) public subscribers;

    constructor(
        IERC721Metadata _contract,
        address _factory
    )
        ERC721(
    ...
    function _wrap(uint256 _tokenId) internal nonReentrant {
        ...
        if (_ownerOf(_tokenId) == address(0)) {
            _safeMint(subscribers[_tokenId], _tokenId);
        }

        emit Wrapped(_tokenId, subscribers[_tokenId]);
    }
    ...
    function setSubscriber(
        ...
        subscribers[tokenId] = subcriber;
        return true;
    }

    function getSubscriber(uint256 tokenId) external view returns (address) {
        return subscribers[tokenId];
    }
...
interface IERC721Wrapper {
    ...
    /// @notice this function return a test value 
...
interface ISettlementManager {
    ...
    function executeBuy(BasicOrderParameters calldata basicComponents) external;
...
import { ConsiderationInterface as SeaportInterface } from "contracts/lib/seaport/contracts/interfaces/ConsiderationInterface.sol";
...
contract SeaportSettlementManager {
    ...
    event Decoded(BasicOrderParameters params, uint256 orderType, SeaportInterface seaport);


    /// encode basicOrder
    function encode( BasicOrderParameters calldata _basicOrderParams ) external pure returns (bytes memory) {
    ...
    */
    function fulfillBasicOrder(bytes memory order) external payable {
        BasicOrderParameters memory params = abi.decode(order, (BasicOrderParameters));
        ...
        // if trade is ETH <=> NFT
            ...
            emit Decoded(params, order_type, seaport);
