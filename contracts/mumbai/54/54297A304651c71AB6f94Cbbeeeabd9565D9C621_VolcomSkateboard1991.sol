...
import "./PunchableERC721.sol";


contract VolcomSkateboard1991 is PunchableERC721{

    //uint16 maxSkateboards = 1991;
    uint16 constant maxSkateboards = 19;
    uint16 startingIndex;
    uint public reveal_time;
    mapping(uint16 => string) private _uris;
    ...
    constructor(string[] memory __uris,
        ...
        PunchableERC721(_reservedIDs, name, symbol){
            price = _price;
            discount = _discount;
            for( uint i=0; i < _whitelist.length; i++ ){
                whitelist[_whitelist[i]]=true;
            }
            for (uint16 i=0; i < maxSkateboards; i++){
                _uris[i] = __uris[i];
            }
            ////console.log("_uris[1]", _uris[1]);
            //console.log("_uris[18]", _uris[18]);
    }
    ...
    function setUri(uint16 position, string memory _uri) external onlyOwner {
        _uris[position] = _uri;
    }
    ...
    function setRevealTime(uint8 numberOfDays) external onlyOwner {
        ...
        //console.log("reveal_time: ",reveal_time );
    ...
    function setStartingIndex() external onlyOwner {
        ...
        while( startingIndex == 0) {
            startingIndex = uint16(uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % maxSkateboards);
        }
    ...
    function mint() external payable {
        require(supply < maxSkateboards, "103");
        if (whitelist[_msgSender()] ){
            require(msg.value >= (price*(10000 - discount))/10000 , "Not enough Matic");
        }else{
    ...
    *****************************************************************************************/
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!_globalUriEmpty()){
            //console.log("from global metadata");
            return globalMetadata;
        }
        else if (reveal_time <= block.timestamp && startingIndex != 0){
            //console.log("from actual uri. reveal_time: ",reveal_time );
            //console.log("startingIndex: ", startingIndex);
            uint16 uri = uint16((tokenId + startingIndex - 1) % maxSkateboards);
            //console.log("uri: ", uri);
            //console.log("_uris[uri]: ", _uris[uri]);
            return _uris[uri];
...
import "./WTCERC_721.sol";


contract PunchableERC721 is WTCERC721{
    ...
    constructor(uint _reservedIDs, string memory name, string memory symbol) WTCERC721(_reservedIDs, name, symbol){}

    /***********************************************************************
    *   @dev create a punch card with a name and an amount of punches
    *   @param  eventId The name of the event. Must match the event ID one in the minter contract.
    ...
    *   @param  eventId The name of the event to burn. 
    ***********************************************************************/
    ...
    *   @param  eventId The name of the event. 
    *   @param  _tID the NFT ID that holds the punch card to punch.
    ...
    *   @param  eventId The name of the event. 
    *   @param  _tID the NFT ID that holds the punch card to query.
    ***********************************************************************/
...
contract WTCERC721 is ERC721, Ownable{

   //Global metadata
  string internal globalMetadata;
 ...
 *****************************************************************************************/
    ...
    while(gap < maxGap){
      if (_exists(_j)){
        ...
        if(ownerOf(_j) == owner){
         ...
         message = appendUintToString( message, _j);
         message = string(abi.encodePacked(message, ":"));
         message = appendUintToString( message, tokenExperience(_j));
 ...
 function tokenExperience(uint tokenId) public virtual view returns (uint) {
    return EID[tokenId];
 }
