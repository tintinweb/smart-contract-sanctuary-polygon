...
contract ERC1155 is Context, ERC165, IERC1155 {
    ...
    mapping(uint8 => uint256) private mintingFees;
    ...
    constructor () {
        ...
        _royaltyFee[3] = 50;
        _royaltyFee[4] = 50;
        mintingFees[1] = 0.3 * 10 ** 18;
        mintingFees[2] = 0.3 * 10 ** 18;
        mintingFees[3] = 0.3 * 10 ** 18;
        mintingFees[4] = 0.3 * 10 ** 18;
    ...
    */
    ...
    function mint(address to, uint8 _tokenId, uint256 supply, Sign calldata sign) external payable {
        require(msg.value == mintingFees[_tokenId], "Mint: invalid Fees");
        require(supply == 1, "Minting limit exceeds");
        require(payable(owner).send(msg.value), "failed on transfer");
        require(!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        verifySign(_tokenId, supply, msg.sender, sign);
        _mint(to, _tokenId, supply);
    }

    function setFees(uint8 _tokenId, uint256 _fee) external {
    ...
    */
    ...
    function _mint(address to, uint256 tokenId, uint256 _supply) internal {
        ...
        require(tokenId <= 4, "tokenId should be less than 4");
        creators[tokenId] = to;
