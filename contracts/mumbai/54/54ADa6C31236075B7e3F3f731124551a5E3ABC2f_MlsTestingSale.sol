...
import './MlsTesting.sol';
import "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

contract MlsTestingSale {
    address payable public admin;
    // address payable private ethFunds = payable(0xD35c657500a074A03E367d8EC2f939fada73f839);
    address payable private ethFunds = payable(0x712Fcf279610616DDEDb71eA7692396f51146CBB);

    MlsTesting public token;
    ...
    function MlsTestingPriceInETH() public view returns(int) {
        int ethPrice = getETHPrice();
        return tokenPriceUSD / ethPrice;
    }

    function buyToken(uint256 _amount) public payable {
        int MlsTestingPriceETH = MlsTestingPriceInETH();
        // Check that the buyer sends the enough ETH
        require(int(msg.value) >= MlsTestingPriceETH * int(_amount));
...
contract MlsTesting is ERC20, ERC20Burnable, Pausable, Ownable, ERC20FlashMint {
    constructor(string memory name_, string memory symbol_, uint initialSupply_) ERC20(name_, symbol_) {
        // _mint(msg.sender, initialSupply_); //open this one and comment the next line for testing
