...
contract PhiMap is MultiOwner, ERC1155Receiver {
    ...
    function batchDeposit(
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            ...
            depositInfo[msg.sender][_contractAddresses[i]][_tokenIds[i]] = Deposit(currentAmount+_amounts[i], block.timestamp);
        ...
        returns (ObjectLinkInfo[] memory)
    {
     ...
     function viewLinks(string calldata name)
        external
        view
        returns (Links[] memory)
    {
        Links[] memory links = new Links[](userObject[name].length);
        for (uint256 i = 0; i < userObject[name].length; i++) {
            if(userObjectLink[name][i].length != 0 ){

                Links memory objectLink = Links(
                i,
                userObjectLink[name][i][0].title,
                userObjectLink[name][i][0].url
                );
                links[i]=objectLink;
            }
        }
