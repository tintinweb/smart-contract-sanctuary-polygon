...
contract Sale is AccessControlEnumerable {
    ...
    bytes32 public constant LAND_SALE_TYPEHASH =
        keccak256(
            "LandSale(address userAddress,uint256 amount,uint256 deadline,uint256[] coordinates)"
        );
    bytes32 public DOMAIN_SEPARATOR;

    event Buy(LandSale landSale);
    ...
    function _verifySignature(LandSale memory landSale) internal {
        ...
        require(
            !boughtOrCanceledHash[digest],
            "Signature already used or canceled"
        );
