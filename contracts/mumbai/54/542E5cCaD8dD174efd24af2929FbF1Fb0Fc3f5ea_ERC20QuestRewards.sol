...
contract ERC20QuestRewards is Initializable, OwnableUpgradeable, PausableUpgradeable, EIP712Upgradeable, ReentrancyGuardUpgradeable {
    using Address for address;

    /* ============ TypeHashes and Constants ============ */
    
    bytes32 public constant ERC20_ACTIVATE_CAMPAIGN_TYPEHASH = keccak256(
        "ERC20LaunchCampaign(uint96 cid,address admin,uint256 startTime,uint256 endTime,uint256 claimTime,CampaignReward[] campaignRewards)CampaignReward(uint96 rewardId,address tokenAddress,uint256 numRewards)"
    );
    ...
    event ERC20SignerUpdate(address _oldSigner, address _newSigner);
    ...
    function launchCampaign(
        ...
        require(
            _verify(
                _hashLaunchCampaign(_cid, msg.sender, _startTime, _endTime, _claimTime, _campaignRewards),
                _signature
            ),
        ...
        emit ERC20LaunchCampaign(_cid, msg.sender, _startTime, _endTime, _claimTime, _campaignRewards);
    }
    ...
    function claim(
        ...
        require(!hasClaimed[_rewardId][msg.sender], "ERC20QuestRewards: Already claimed");
        require(!hasClaimedUserId[_rewardId][_userId], "ERC20QuestRewards: Already claimed");

        require(
            _verify(
                _hashClaim(_rewardId, _userId, msg.sender, _amountToClaim),
                _signature
            ),
            "ERC20QuestRewards: Invalid signature"
        );
        ...
        hasClaimed[_rewardId][msg.sender] = true;
        ...
        if (rewardConfig.tokenAddress == CurrencyTransferLib.NATIVE_TOKEN) {
            (bool success, ) = payable(msg.sender).call{value: _amountToClaim}(new bytes(0));
            require(success, "ERC20QuestRewards: Transfer failed");
        } else {
            bool success = IERC20(rewardConfig.tokenAddress).transfer(msg.sender, _amountToClaim);
            require(success, "ERC20QuestRewards: Transfer failed");
        }

        emit ERC20Claim(_rewardId, _userId, msg.sender, _amountToClaim);
    }

    function withdraw(uint96 _cid) external whenNotPaused nonReentrant {
