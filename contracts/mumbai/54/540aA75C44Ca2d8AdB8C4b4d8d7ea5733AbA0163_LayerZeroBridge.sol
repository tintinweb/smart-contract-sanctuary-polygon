...
contract ERC721Mintable is ERC721EnumerableUpgradeable, ITokenBridgeable, OwnableUpgradeable, IERC721MintableInitializable {
    ...
    function initialize_ERC721Mintable(bytes calldata initializerEncoded_) initializer override public {
    (string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) = this.decodeInitializer_ERC721Mintable(initializerEncoded_);
        _initialize_ERC721Mintable(name_, symbol_, contractURI_, owner_, system_);
    }

    function _initialize_ERC721Mintable(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) onlyInitializing internal {
        _initializeUnchainedInternal(name_, symbol_, contractURI_, owner_, system_);
    }

    function decodeInitializer_ERC721Mintable(bytes calldata initializerEncoded_) override external pure returns (string memory, string memory, string memory, address, address){
        return abi.decode(initializerEncoded_[4:], (string, string, string, address, address));
    }
    ...
    function encodeInitializer(string memory name_, string memory symbol_, string memory contractURI_, address owner_, address system_) override public pure returns (bytes memory){
        return abi.encodeWithSelector(
            IERC721MintableInitializable.initialize_ERC721Mintable.selector,
            name_,
            symbol_,
            contractURI_,
            owner_,
...
contract ContractFactory {
     ...
     */
    function registerNewBlueprintVersion(address blueprintAddress, string memory blueprintName, uint16 version) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        ...
        beacons[blueprintName].upgradeTo(blueprintAddress);
     ...
     */
    function deregisterLatestBlueprint(string memory name) external onlyRole(systemContext.getAccessControlList().CONTROL_LIST_ADMIN_ROLE()) {
        ...
        if (latestBlueprintVersion[name] > 1) {
            ...
            beacons[name].upgradeTo(blueprints[name][latestBlueprintVersion[name]]);
        }
        else {
...
interface INonNativeWrapperInitializable {
    ...
    function initialize_NonNativeWrapper(bytes calldata initializerEncoded_) external;
