...
/**
 * @title The registry contract for the RareMint Slab Series NFTs based off the Courtyard Series Contract Source.
 *
 * Minting RareMint NFT cannot be achieved by interacting with this contract. Instead, minter contracts are granted a
 * {MINTER_ROLE} role, which allows them to mint new tokens on behalf of collectors when new assets are released.
 *
 * @dev This system acts as a gate keeper to ensure the security around minting operations, as well as gives RareMint
 * the opportunity to implement various minting experiences depending on the kind of assets to which a token is minted.
 */
contract RareMintRegistry is Initializable, UUPSUpgradeable, ITokenRegistryUpgradeable, AccessControlEnumerableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    function initialize(address admin) public initializer {
        ERC721Upgradeable.__ERC721_init("RareMint NFT", "RAREMINT");
     ...
     */
    function _proofOfIntegrityAsHexString(uint256 tokenId) private pure returns (string memory) {
    ...
    function getProofOfIntegrity(uint256 tokenId) public view returns (bytes32) {
        require(_exists(tokenId), "RareMintRegistry: Nonexistent token.");
     ...
     *      - the token must exist.
     */
    function getProofOfIntegrityAsHexString(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "RareMintRegistry: Nonexistent token.");
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return "https://content.raremint.com/v0/api/content/rmt";
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        require(_exists(tokenId), "RareMintRegistry: Nonexistent token.");
        return string(abi.encodePacked(_baseURI(), "/", _proofOfIntegrityAsHexString(tokenId)));
    }
     ...
     */
    function mintToken(address _to, bytes32 _proof) external override onlyMinter returns (uint256) {
        uint256 tokenId = _tokenId(_proof);
        require(!_exists(tokenId), "RareMintRegistry: Token already exists.");
