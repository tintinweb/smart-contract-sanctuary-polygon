...
contract Points is Ownable {
    ...
    mapping(bytes32 => OperationsHistory) private operations;

    uint256 private lastOperation;
    ...
    function addPerson(bytes32 _id) internal onlyOwner {
    ...
    function adjustPersonPoints(bytes32 _id, uint256 _amount) external onlyOwner {
        ...
        addOperation("adjusted", _id, _amount);
    ...
    function checkIfPersonExists(bytes32 _id) internal view returns (bool) {
    ...
    function addOperation(string memory _operation, bytes32 _id, uint256 _amount) internal onlyOwner {
        lastOperation += 1;
        operations[bytes32(lastOperation)] = OperationsHistory({
            operationID : bytes32(lastOperation),
            id : _id,
    ...
    function seeHistory(bytes32 _id) external view returns (OperationsHistory[] memory) {
        uint32 foundMatches;

        OperationsHistory[] memory res = new OperationsHistory[] (lastOperation);

        //initiaze the array / struct / structure

        for (uint256 i = 0; i <= lastOperation; i++){
            
            if(operations[bytes32(i)].id == _id){

                res[foundMatches] = OperationsHistory(
                operations[bytes32(i)].operationID,
                operations[bytes32(i)].id,
                operations[bytes32(i)].date,
                operations[bytes32(i)].operation,
                operations[bytes32(i)].amount
