...
// File: contracts/WarLogic/IWarLogic.sol
...
interface IWarLogic {
	...
	function increaseCitizenLevel(uint256 citizenId) external;
	function getCastleWorkEffort(uint256 castleId) external view returns (uint256);
}
...
// File: contracts/GameDesign/IWarDesign.sol
...
// File: contracts/WarCitizenDetails.sol
...
// File: contracts/WarCitizen/WarCitizenToken.sol
...
contract WarCitizenToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, ContextMixin, ERC721BurnableUpgradeable {
	...
	function burnBatch(uint256[] memory ids) external onlyRole(BURNER_ROLE) {
		for (uint256 i = 0; i < ids.length; ++i) {
			...
			_burn(ids[i]);
			_ownedTokensCount[msg.sender].decrement();
			delete tokenIds[msg.sender][ids[i]];
		}
	}
	...
	function upgrade(uint256 baseId, uint256 burnId) external {
		...
		// Increase +1 in work effort of the allocated castle of citizen, if citizen is allocated
		logic.increaseCitizenLevel(baseId);
