...
import "./IVCArtNftERC1155.sol";

contract VCArtNftERC1155 is IVCArtNftERC1155, ERC2981, AccessControl, ERC1155Supply {
    ...
    /// @notice Only accounts with this role can approve transfers for anyone
    ...
    function _onlyAdmin() internal view {
        if (msg.sender != admin) {
            revert ArtNftOnlyAdminAllowedError();
        }
    }
    ...
    constructor(uint256 _maxRoyaltyBps, uint256 _maxBatchSize, address _admin) ERC1155("https://ipfs.io/ipfs/") {
    ...
    function setAdmin(address _admin) external {
        _onlyAdmin();
        if (_admin == address(this) || _admin == address(0) || _admin == admin) {
            revert ArtNftUnexpectedAdminAddressError();
        }
        admin = _admin;
    }

    function setMinterRole(address _address, bool grant) external {
        _onlyAdmin();
        if (grant) {
    ...
    function setApproverRole(address _approver, bool grant) external {
        _onlyAdmin();
        if (grant) {
    ...
    function setLazyMinterRole(address _lazyMinter, bool grant) external {
        _onlyAdmin();
        if (grant) {
            _grantRole(LAZY_MINTER_ROLE, _lazyMinter);
        } else {
            _revokeRole(LAZY_MINTER_ROLE, _lazyMinter);
        }
    }
    ...
    function setApprovalForAllCustom(address caller, address operator, bool approved) external onlyRole(APPROVER_ROLE) {
        _setApprovalForAll(caller, operator, approved);
    }

    function lazyMintBatchOnBehalf(LazyMintBatchArgs memory _args) external onlyRole(LAZY_MINTER_ROLE) {
        _lazyMintBatch(_args);
    }

    function lazyMintBatch(LazyMintBatchNoCreatorArgs memory _args) external {
        LazyMintBatchArgs memory args = LazyMintBatchArgs(
            msg.sender,
            _args.cid,
            _args.name,
            _args.totalSupplies,
    ...
    function requireCanRequestMint(address _by, uint256 _tokenId, uint256 _amount) public view {
        if (exists(_tokenId)) {
            revert ArtAlreadyMintedError();
        }
        if (_by != creatorOf(_tokenId)) {
            revert ArtOnlyCreatorCanRequestError();
        }
        if (_amount > lazyTotalSupply(_tokenId)) {
            revert ArtExceededTotalSupplyError();
        }
    }
    ...
    function lazyTotalSupply(uint256 _tokenId) public view returns (uint256) {
        ...
        if (batchId == 0) {
            revert ArtTokenNotYetCreatedError();
        }
    ...
    function mintTo(uint256 _tokenId, address _to, uint256 _amount) external onlyRole(MINTER_ROLE) {
        uint256 batchId = _tokenIdToBatchId[_tokenId];
        if (batchId == 0) {
            revert ArtTokenNotYetCreatedError();
        }
        if (exists(_tokenId)) {
            revert ArtTokenAlreadyMintedError();
        }
        ...
        // posiblemente borrar esta variable
     ...
     * @dev Indicates whether any token exist with a given id, or not.
     */
    function exists(uint256 id) public view override(IVCArtNftERC1155, ERC1155Supply) returns (bool) {
        return ERC1155Supply.totalSupply(id) > 0;
    }
    ...
    function setTokenRoyaltyReceiver(uint256 _tokenId, address _receiver) external {
        _onlyTokenCreator(_tokenId);

        if (_receiver == address(0)) {
            revert ArtRoyaltyBeneficaryZeroAddressError();
        }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(IVCArtNftERC1155, ERC2981, AccessControl, ERC1155) returns (bool) {
        return
    ...
    function _lazyMintBatch(LazyMintBatchArgs memory args) internal {
        uint256 batchSize = args.totalSupplies.length;

        if (0 == batchSize || batchSize > maxBatchSize) {
            revert ArtBatchSizeError();
        }
        if (args.royaltyFeeBps > maxRoyaltyBps) {
            revert ArtExceededMaxRoyaltyError();
        }
        if (args.receiver == address(0)) {
            revert ArtRoyaltyBeneficaryZeroAddressError();
        }
        ...
        Batch memory newBatch = Batch(args.cid, args.name, args.creator, firstTokenId, args.totalSupplies);

        tokenBatch.push(newBatch);

        for (uint256 i = 0; i < batchSize; i++) {
            if (args.totalSupplies[i] == 0) {
                revert ArtTotalSupplyZeroError();
            }
            uint256 tokenId = firstTokenId + i;
            _tokenIdToBatchId[tokenId] = currentBatchId;
            _setTokenRoyalty(tokenId, args.receiver, args.royaltyFeeBps);
        }

        emit BatchCreated(args.creator, currentBatchId, newBatch, args.royaltyFeeBps);
    }
}