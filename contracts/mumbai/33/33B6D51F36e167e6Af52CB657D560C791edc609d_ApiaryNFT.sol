...
contract ApiaryNFT is
    CreatorsHubOpenBase("https://apiary.creators-hub-cluster.license.rocks/")
{
    constructor() {}

    using SafeMath for uint256;

    mapping(address => bool) private paymentGatewayAddrs;
    mapping(address => bool) private whitelistedSellers;

    address public kycRegistryAddr;

    modifier onlyOperator {
      require(_rootOperators[_msgSender()] == true, "Must be owner or operator");
      _;
    }

    function setKycRegistryAddr(address _addr) public onlyOperator {
      kycRegistryAddr = _addr;
    } 

    function whitelistGateway(address _gateway, bool _flag) public onlyOperator {
      paymentGatewayAddrs[_gateway] = _flag;
    } 

    function whitelistSeller(address _gateway, bool _flag) public onlyOperator {
      whitelistedSellers[_gateway] = _flag;
    }     
    ...
    function withdrawErc20(address _tokenContract, uint256 _amount) public onlyOperator{
    ...
    function buyToken(
      ...
      require(paymentGatewayAddrs[_msgSender()], "caller not allowed");
      require(whitelistedSellers[_from], "seller not whitelisted");

      _balances[_tokenId][_from] = _balances[_tokenId][_from].sub(
          _amount,
          "ERC1155: insufficient balance for transfer"
      );

      _balances[_tokenId][_to] = _balances[_tokenId][_to].add(_amount);

      emit TransferSingle(_msgSender(), _from, _to, _tokenId, _amount);
    }

    function checkClaimEligibility(address _from, address _to, uint256 _tokenId, uint256 _quantity) external view returns (string memory) {
      if (kycRegistryAddr != address(0) && !KycRegistryInterface(kycRegistryAddr).check(_to)) {
        return "Buyer is not after KYC";
      } else if (_quantity > _balances[_tokenId][_from]) {
