...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    mapping(uint256 => string) internal TokenCID;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        if (from == address(0)) {
            ...
            link[to][tokenId] = TokenCID[tokenId];
...
contract DIGITAL is ERC721Enumerable, Ownable {
    using Strings for uint256;
    ...
    mapping(uint256 => uint256) internal tokenRoyalty;

    address public  minter;
    bool public use_minter_roll;

    event onMint(uint256 indexed tokenid,address indexed creator,string cid,uint256 roylty);
    ...
    function mint(uint256 royalty, string memory cid) public {
        uint256 supply = totalSupply();
        if(use_minter_roll){
            require(whitelisted[msg.sender],"not minter");
        }
        require(royalty <= 2500,"notavliable");

        uint256 d = supply+1;
        _safeMint(msg.sender,d);
        creator[d] = msg.sender;
        TokenCID[d] = string(abi.encodePacked("ipfs://", cid));
        string memory tokneURL = TokenCID[d];
        tokenRoyalty[d] = royalty;
        emit onMint(d,msg.sender,tokneURL,royalty);
    }
    ...
    function isMinter(address _add) external view returns(bool){
        if(whitelisted[_add]){
            return true;
        }else{
            return false;
    ...
    function changeRoll(bool _use_minter_roll) public onlyOwner returns(bool){
        use_minter_roll = _use_minter_roll;
        return true;
    }
