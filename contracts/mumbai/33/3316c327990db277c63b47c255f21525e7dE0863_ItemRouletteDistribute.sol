...
abstract contract RouletteDistributeBaseContract is CooperatableBase {
    ...
    function update_interval_quota(uint256 id_,uint32 quota) external onlyOwner {
        require(defined[id_],"NO_INTERVAL");
    ...
    function _roulette(address _addr,uint32 rand_value) internal returns (bool,uint256,bool){
        ...
        return (overlap_count >= 1,found_interval_id,is_ssr_guarantee_hit);
    }
...
contract ItemRouletteDistribute is RouletteDistributeBaseContract {
    ...
    event OnRoulette(address indexed _addr,RewardType reward_type,address indexed contract_address,uint256 indexed token_id,uint256 amount,bool is_ssr_guarantee_hit,bool is_default_reward);
    ...
    function roulette(address _addr,uint32 rand_value) external onlyOwnerOrOperator returns (RewardType,address,uint256,uint256){
       
        (bool found,uint256 interval_id,bool is_ssr_guarantee_hit) = _roulette(_addr, rand_value);
        ...
        emit OnRoulette(_addr,_reward.reward_type,_reward.contract_address,_reward.token_id,amount,is_ssr_guarantee_hit,!found || (interval_id == 0));
    ...
    function _form_erc721_reward(address token_contract,uint256 min_amount,uint256 max_amount)  internal view returns (Reward memory){
        require(token_contract != address(0),"NULL_ADDRESS");
        require(token_contract.supportsInterface(type(IERC721TokenContract).interfaceId),"NOT_ERC721_CONTRACT");
        require(min_amount <= max_amount,"INVALID_RANGE");
