...
contract TrainingCenter is ERC721Enumerable, Ownable {
    ...
    constructor(uint256 _preMint, address _NFTreserve) ERC721("Training_Center_NFT", "TNFT") {
        _preMintNumber = _preMint;
        for(uint256 i = 0; i < _preMint ; ){
            _mintTrainingCenter(_NFTreserve);
            unchecked{ ++i; }
        }
    }
    ...
    function addTrainingSpots(uint256 _tokenId, uint256 _amount) external onlyAuth returns(bool){
        require(_numberOfTrainingSpots[_tokenId] + _amount <= 10, "Maximum 10 training spot by training center");
