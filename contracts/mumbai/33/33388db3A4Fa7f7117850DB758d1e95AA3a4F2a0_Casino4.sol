...
contract Casino4 {
    ...
    function startCoinTossGame(uint8 chosenSide) external payable {
        ...
        if (chosenSide == 1 && trueResult) {
            ...
            coinTossGameResults[gameId] = "You Win! The Coin Flipped Heads";
        } else if (chosenSide == 2 && !trueResult) {
            result = GameResult.Win;
            coinTossGameResults[gameId] = "You Win! The Coin Flipped Tails";
        } else if (chosenSide == 1 && !trueResult) {
            result = GameResult.Lose;
            coinTossGameResults[gameId] = "You Lost! The Coin Flipped Tails";
        } else if (chosenSide == 2 && trueResult) {
            result = GameResult.Lose;
            coinTossGameResults[gameId] = "You Lost! The Coin Flipped Heads";
        }
    ...
    GameResult result;
    uint8 rolledNumber = getRandomDiceRollResult(chosenNumber);

    if (rolledNumber == chosenNumber) {
        result = GameResult.Win;
        diceRollGameResults[gameId] = string(
            abi.encodePacked("Congratulations! You won the dice roll game! The dice rolled ", uintToString(rolledNumber))
        );
    } else {
        result = GameResult.Lose;
        diceRollGameResults[gameId] = string(
            abi.encodePacked("Oops! You lost the dice roll game. The dice rolled ", uintToString(rolledNumber), ". Better luck next time!")
        );
    }
    ...
    function getRandomDiceRollResult(uint8) private view returns (uint8) {
    bytes32 blockHash = blockhash(block.number - 1);
    uint256 randomNumber = uint256(blockHash);
