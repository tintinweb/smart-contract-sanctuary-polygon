...
contract BanaCat is ERC721Enumerable, Ownable {
    ...
    constructor(string memory _initBaseURI) ERC721("BanaCat", "BNC") {
    ...
    // 拿到tokenURI ipfs://QmTwLDV6CqG1Q2BKsADpGmC1b82ns6wKeCnge5ivPBx6wm/2.json
    function tokenURI(
        uint256 tokenId
    ...
    // 设置单次最大的mint数量
    function setmaxMintAmountOneTime(
    ...
    // 查看某个索引所对应的暗号属性
    function showSecretattributes(
        uint256 _ID
    ) public view onlyOwner returns (Signal memory secret) {
        require(_ID >= 0 && _ID < Signals.length, "Error: Illegal ID");
        return Signals[_ID];
    }

    // 输入secret索引，获取剩余还没被mint的token数量
    function getRemainingtokenAmount(
        uint256 _ID
    ) public view onlyOwner returns (uint256 amount) {
    ...
    function checkSecret(
        string memory userInput
    ) public view returns (uint256 _ID) {
        uint256 index = 0;
    ...
    function setMaxTokenAmountForEachAddress(uint _MaxAmount) public onlyOwner {
