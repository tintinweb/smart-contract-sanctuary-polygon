...
import { HUNDER_PERCENT_IN_BPS, UPGRADER_ROLE } from "./libraries/Constants.sol";
    ...
    ClaimGoodUpgradeable
{
    ...
    modifier onlyRole(bytes32 role) {
        if (!_checkRole(role)) revert IGoodsExchange__NotAuthorized();
        _;
    }
    ...
    event Test(address instance, address from, address collection, uint256[] tokenIds, uint8 v, bytes32 r, bytes32 s);
    ...
    function onERC721Received(
        ...
        claim_.instance = address(this);
        claim_.recipient = from_;
        claim_.collection = _msgSender();
        claim_.tokenIds = new uint256[](1);
        claim_.tokenIds[0] = tokenId_;

        Types.Signature[] memory signatures_ = abi.decode(data_, (Types.Signature[]));
        ...
        // IERC721Upgradeable(claim_.collection).burn(tokenId_);
    ...
    function _processClaim(Types.Claim memory claim_, Types.Signature[] memory signatures_) internal {
        ...
        _claimGoods(claim_.recipient, claim_.collection, claim_.tokenIds);
    }
...
abstract contract ClaimGoodUpgradeable is UniqueCheckingUpgradeable {
    ...
    function isClaimed(address collection, uint256 tokenId) external view returns (bool claimed, address recipient) {
    ...
    function _claimGoods(address recipient_, address collection_, uint256[] memory tokenIds_) internal {
        ...
        for (uint256 i; i < length; ) {
            ...
            if (recipient_ != collection.ownerOf(tokenId)) revert ClaimGoodUpgradeable__NotOwner();
        ...
        emit Claimed(msg.sender, collection_, tokenIds_);
    }
}
...
library Types {
    // keccak256("Claim(address instance,address recipient,address collection,uint256[] tokenIds)")
    bytes32 internal constant CLAIM_HASH = 0xdf8d435a6a5fc3b814b5aaa2380c42b5bbc62f6714c4e76d63b98a6c9260301a;
