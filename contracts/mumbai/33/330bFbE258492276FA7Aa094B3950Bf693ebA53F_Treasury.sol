...
contract Treasury is AccessControl {
	...
	constructor (address _rewardController) public {
	...
	function setTokenController(address _tokenController) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not a operator");
        TokenController = _tokenController;
        
	}

	function setRewardController(address _rewardController) public {
		require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "Caller is not a operator");
        RewardController = _rewardController;
	...
	function withdrawToEthereum(address ETHToken,uint256 amount,address ETHUser) public {
		require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
		(address token,) = ITokenController(TokenController).getToken(ETHToken);
		ILendingPool(LendingPOOL).withdraw(token,amount,address(this));
		ITokenController(TokenController).burn(token,address(this),amount);
		emit WithdrawToEthereum(ETHToken,amount,ETHUser);
	}
