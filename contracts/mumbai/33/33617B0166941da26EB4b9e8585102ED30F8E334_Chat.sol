  ...
  //--------------------------------------------------------------------------------------   
   ...
   require(walletAddress != address(0), "User with given address does not exist.");
    ...
    function sendMessage(address receiver, string calldata subject, string memory message, string memory fileHash, string memory receiverGroup) external {
        ...
        Message memory message = Message(messageCount, msg.sender, receiver, subject, message, false, block.timestamp, false, fileHash, receiverGroup, DeletionStatus.NotDeleted);
        messages.push(message);
        //emit MessageSent(msg.sender, receiver, messageHash);
        messageCount++;
        }        

        function sendMessageToGroup(address[] memory receiver, string calldata subject, string []memory message,bool cci, string memory fileHash, string memory emailGroup, string memory cciReceivers) external {
        require(
            checkUserExists(msg.sender) == true,
            "You must have an account"
        );
        for(uint i = 0; i<receiver.length; i++){
            ...
            Message memory message = Message(messageCount, msg.sender, receiver[i], subject, message[i], cci, block.timestamp, false, fileHash, emailGroup,DeletionStatus.NotDeleted);
        messages.push(message);
        messageCount++;
        }
