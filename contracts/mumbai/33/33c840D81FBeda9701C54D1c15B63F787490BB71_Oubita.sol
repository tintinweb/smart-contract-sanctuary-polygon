...
contract Oubita is ERC721, Ownable {
   ...
   //uint256 public NFTperAddressLimit= 20;
    ...
    uint public costETH_whitelist = 20000000000000000;
    uint public costETH_cost = 150000000000000000;
    uint public costETH_costNormal = 200000000000000000;

    //cost in TOKEN

    uint public cost = 590000000000000;
    uint public costWhitelist= 4500000000000000;
    uint public costNormal= 5900000000000000;
    //Badse URI
    string baseURI;
    ...
    function multiTokensTransfer(address _to, uint _type, uint _amount, uint _counter) public payable{
        if(_type == 1){
            if(onlyWhitelisted == true){
                ...
                require(msg.value >= costWhitelist * _amount, "You dont have enough balance");
            }else if(_counter < specialSale){
                
                require(msg.value >= cost * _amount, "You dont have enough balance");
            }else if(_counter >= specialSale){
                
                require(msg.value >= costNormal * _amount, "You dont have enough balance");
            }
            ...
            if(onlyWhitelisted == true){
                ...
                uint precio = costETH_cost * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter >= specialSale){
                uint precio = costETH_costNormal * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }
        }
    } 
