...
contract PositionSwapRouter {
    ...
    //maker? => underlyingOrderID
    mapping(bool => mapping(uint256 => PositionSwap[]))
        public positionsSwapHistory;
    uint256 askID;
    uint256 bidID;
    ...
    event BidCreated(PositionBid bid);
    ...
    event BidTaken(
        ...
        address collateral,
        uint256 bidAmount,
        uint256 positionTransferAmount,
        uint256 bidID
    );
    event AskTaken(
        address user,
    ...
    constructor(
        ...
        drvs_vault = Derivswap(_vaultAddress);
        askID = 0;
        bidID = 0;
    }
    ...
    function create_position_ask(
        ...
        uint256 _collateralAskAmt
    ) public {
        PositionAsk memory ask;
        if (_underliningOrderType == 0) {
            DRVSStructs.MakerOrder memory maker = fetch_orderbook_maker_order(
                _underlyingOrderID
            );
            ...
            DRVSStructs.TakerOrder memory taker = fetch_orderbook_taker_order(
                _underlyingOrderID
            );
            DRVSStructs.MakerOrder memory maker = fetch_orderbook_maker_order(
                taker.makerOrder_ID
    ...
    function create_position_bid(
        ...
        uint256 _underlyingOrderID,
        uint256 _alternateCollateralAmt
    ) public payable {
        PositionBid memory bid;
        DRVSStructs.MakerOrder memory maker;
        if (_underlyingOrderType == 0) {
            maker = fetch_orderbook_maker_order(_underlyingOrderID);
            ...
            bid = PositionBid(
                ...
                maker.order_ID,
                maker.collateral,
                maker.collateral==address(0)?msg.value:_alternateCollateralAmt,
                block.timestamp
            );
            ...
            maker = fetch_orderbook_maker_order(taker.makerOrder_ID);
            require(
        ...
        if (maker.collateral == address(0)) {
            ...
            require(_alternateCollateralAmt != 0);
            IERC20 collateral_token = IERC20(maker.collateral);
            uint256 users_collateral_balance = collateral_token.balanceOf(
                msg.sender
            );
            require(
                users_collateral_balance >= _alternateCollateralAmt,
                "ERC20 balance insufficent for bid."
            );
            collateral_token.transferFrom(
                msg.sender,
                address(this),
                _alternateCollateralAmt
            );
        }
    ...
    function accept_position_bid(uint256 _bidID) public {
        ...
        if (bid.underlyingIsMaker == true) {
            ...
            if (success == true) {
                ...
                PositionSwap memory swap = PositionSwap(
                    msg.sender,
                    bid.user,
                    maker.collateral,
                    bid.collateralAmount,
                ...
                positionsSwapHistory[true][maker.order_ID].push(swap);
            }
            ...
            if (success == true) {
                ...
                positionsSwapHistory[false][taker.order_ID].push(swap);
            }
        }
        if (success == true) {
            ...
            if(bid.collateral==address(0)){
                payable(msg.sender).transfer((((bid.collateralAmount) * 997) / 1000));
            }
            else{
                IERC20 collateral_token = IERC20(bid.collateral);
                collateral_token.transfer(msg.sender,(((bid.collateralAmount) * 997) / 1000));
            }
        }
    }
    ...
    function accept_position_ask(uint256 _askID) public payable {
        ...
        if(ask.collateral==address(0)){
            require(
                msg.value == ask.collateralAmount,
                "Insufficent or incorrect amount required to assume this position."
            );
        }
        else{
            collateral_token = IERC20(ask.collateral);
            require(collateral_token.balanceOf(msg.sender) >= ask.collateralAmount, "ERC20 balance insufficient to accept position's ask.");
            collateral_token.transferFrom(msg.sender, address(this), ask.collateralAmount);
        }
        ...
        if (success == true) {
            ...
            if(ask.collateral == address(0)){
                ...
                collateral_token.transfer(seller,(((ask.collateralAmount) * 997) / 1000));
            }
        }
    }
        ...
        for (uint256 ix = 0; ix < makers_bids_length; ix++) {
            ...
            if (bidCanceled[bid.positionBidID] == false) {
                if (bidAccepted[bid.positionBidID] == false) {
                    ...
                    if(bid.collateral==address(0)){
                        payable(bid.user).transfer(bid.collateralAmount);
                    }
                    else{
                        IERC20 collateral_token = IERC20(bid.collateral);
                        collateral_token.transfer(bid.user, bid.collateralAmount);
                    }
    ...
    function taker_order_cleanup(uint256 _underlyingOrderID) internal {
        ...
        uint256 takers_bids_length = bid_lengths(_underlyingOrderID, 1);
    ...
    function fetch_orderbook_maker_order(
        uint256 _makerOrderID
    ) internal view returns (DRVSStructs.MakerOrder memory maker) {
        (
            uint256 order_ID,
            address user,
            address collateral,
    ...
    function fetch_orderbook_taker_order(
        uint256 _takerOrderID
    ) internal view returns (DRVSStructs.TakerOrder memory taker) {
        (
            uint256 order_ID,
            address user,
            uint256 ethPosted,
...
contract Derivswap {
    ...
    mapping(address => uint256) public earn_box_fees_injected_multi;
    ...
    function protocol_claim_quarter_of_excess_profit() public {
        ...
        protocol_earnings_native -= int256(((uint256(protocol_earnings_native) -
            ...
            earn_box_fees_injected_multi[address(0)]) * 25) / 100);
    ...
    function settleOrder(
        ...
        if (maker.collateral == address(0)) {
            ...
            protocol_earnings_by_asset[maker.collateral] += payout.protocol_profit;
        }
        ...
        if (maker.collateral == address(0)) {
            ...
            if (initialLiquidityProvided == true) {
                ...
                earnBox.inject_profit_multi{value: protocol_profit / 4}(address(0),0);
                earn_box_fees_injected_multi[address(0)] += (protocol_profit/4);
            }
            if (uint256(protocol_earnings_native) >= initialLiqThreshold) {
            ...
            //Profit injection alt-collateral
            if(initialLiquidityProvided==true){
                collateral_token.transfer(earnBoxAddress,(protocol_profit/4));
                earnBox.inject_profit_multi{value: 0}(maker.collateral,protocol_profit/4);
                earn_box_fees_injected_multi[maker.collateral] += (protocol_profit/4);
            }
