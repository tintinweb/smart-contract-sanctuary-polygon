...
contract BridgeAssist is AccessControl, Pausable, EIP712 {
    ...
    bytes32 public constant TRANSACTION_TYPE_HASH =
        keccak256(
    ...
    function send(
        ...
        _receiveTokens(msg.sender, amount * exchangeRate);

        uint256 currentFee = (amount / FEE_DENOMINATOR) * feeSend;
        if (currentFee != 0) _dispenseTokens(feeWallet, currentFee * exchangeRate);

        string memory fromUser = Strings.toHexString(uint256(uint160(msg.sender)), 20);
        transactions[fromUser].push(
            Transaction({
                fromUser: fromUser,
                toUser: toUser,
        ...
        whenNotPaused
    {
        ...
        uint256 currentFee = (amount / FEE_DENOMINATOR) * feeFulfill;
    ...
    /// @param exchangeRatesFrom exchange rates for `chains`
    function addChains(string[] calldata chains, uint256[] calldata exchangeRatesFrom)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i; i < chains.length; ) {
            ...
            exchangeRateFrom[bytes32(bytes(chains[i]))] = exchangeRatesFrom[i];
            unchecked {
                ++i;
            }
        }
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i; i < chains.length; ) {
            ...
            exchangeRateFrom[bytes32(bytes(chains[i]))] = 0;
            unchecked {
                ++i;
            }
        }
    ...
    /// @dev returns a list of bridge transactions sent by `user` that
    /// have been sent from or fulfilled on the current chain
    /// @param user sender address, can be e.g. an eth address or a solana address
    /// @return list of transactions
    function getUserTransactions(string calldata user)
        external
        view
    ...
    /// @dev returns the amount of bridge transactions sent by `user` that
    /// have been sent from or fulfilled on the current chain
    /// @param user user
    /// @return amount of transactions
    function getUserTransactionsAmount(string calldata user)
        external
        view
        returns (uint256)
    {
    ...
    /// @dev hashes `Transaction` structure with EIP-712 standart
