...
contract TaxTerminator {
    ...
    function _terminate(uint256 _tokenId) private {
        bytes memory payload = abi.encodeWithSignature("settleTax(uint256)", _tokenId);
    ...
    function trollerTerminator_1000(address _owner, uint _limit) external {
        bytes memory payload = abi.encodeWithSignature("getTokensByOwner(address,uint256,uint256)", _owner, _limit, 0);
        (bool success, bytes memory data) = theSpace.call(payload);
