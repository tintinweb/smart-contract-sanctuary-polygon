...
contract MATICMining {
    ...
    uint[] public dailyROI = [40,44,48,52,56,60,65];
   ...
   address[] public ai_pool_top;
    ...
    function BNBToUSD(uint bnbAmount) public view returns(uint)
    {
        ...
        //return bnbAmount * 1700536360000000000 * (10 ** (decimals-8)) / (10 ** (decimals));
    }
    function USDToBNB(uint busdAmount) public view returns(uint)
    {
        uint256  bnbpreice = IEACAggregatorProxy(EACAggregatorProxyAddress).latestAnswer();
        return busdAmount  / bnbpreice * (10 ** (decimals-10));
 //       return busdAmount  / 1700536360000000000 * (10 ** (decimals-10));
    }
    ...
    function checklimit(address _addr, uint256 amount, bool isROI, uint stakeIndex) public view returns(uint)
    {
      ...
      {
        if(_addr != terminal){
          uint256 dailyROILimit = stakeInfo[_addr][stakeIndex].amount;
          if((isROI && claimeddailyROIGain[_addr].add(amount) > dailyROILimit ))
          {          
...
//          uint usedDays = (tim2 - tim) / oD;
          ...
          uint256 checkedamt= checklimit(_user, amt, true, stakeIndex);
          return checkedamt ;
    ...
    function buy(address _referredBy,uint256 USDAmount) payable external returns(uint256)
    {
      ...
      require(USDAmount <= BNBToUSD(msg.value),"Invalid MATIC");
      ...
      if(totaldaily>0)
      {
        ...
        for(uint i =1 ; i <= 20;i++)
        {
          ...
          if(ref == address(0)) ref = terminal;
            ...
            uint256 checkedamt= checklimit(ref,amt,false,0);
            if(checkedamt==amt){
      ...
      require(!isContract(msg.sender),  'No contract address allowed to withdraw');
       ...
       // uint tim = stakeInfo[msg.sender][stakeIndex].stakeTime;
        uint tim2 = stakeInfo[msg.sender][stakeIndex].lastWithdrawTime;
          uint daysPassed = (block.timestamp - tim2 ) / oneDay;
          uint256 amt = (amount * extraROI(amount*2) / 10000) * daysPassed ;
          uint256 checkedamt= checklimit(msg.sender,amt,true,stakeIndex);
            if(checkedamt < amt){
              uint remamt = amt - checkedamt;
    ...
    function _AIpollDeposits(address _addr, uint256 _totalbus) private {
      ...
      function _drawPool() private {
          ...
          for(uint8 i = 0; i < pool_bonuses.length; i++) {
              ...
              uint256 checkedamt= checklimit(pool_top[i], win,false,0);
              if(checkedamt==win){
    ...
    function purchaseTokens(address _customerAddress, uint256 _amountOfTokens, address _referredBy, uint256 mainamount) internal returns(uint256)
    {
        ...
        uint256 checkedamt= checklimit(_referredBy, directincome,false,0);
    ...
    function distributeRewards(uint256 _amount, address _idToDistribute)  internal
    {
        ...
        for(uint i=0; i<9; i++)
        {
            ...
            uint256 checkedamt= checklimit(_idToDistribute,amt, false,0);
            if(checkedamt==amt){
    ...
    {
        if(_grv >= 1 * (10** decimals) && _grv <= 500 * (10** decimals))
        {
            return dailyROI[0];
        }
        ...
        else if(_grv > 10000 * (10** decimals) && _grv <= 25000 * (10** decimals) )
        {
            return dailyROI[4];
        }
        else if(_grv > 25000 * (10** decimals) && _grv <= 50000 * (10** decimals) )
        {
            return dailyROI[5];
        }
        else if(_grv > 50000 * (10** decimals) )
        {
            return dailyROI[6];
        }
        return 0;
