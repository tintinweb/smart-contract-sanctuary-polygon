...
contract OwensCricket is ERC721URIStorage,Ownable{
    ...
    function addNewTemplate(Schema memory _newTemplate) external onlyOwner{
        ...
        templateData[templateId] = _newTemplate;
    ...
    function mint(address _to,uint256 _templateId) public onlyOwner returns(uint256){
        ...
        metaData[tokenId].matches = templateData[_templateId].matches;
    ...
    function getMetaData(uint256 _tokenId) external view returns(Schema memory){
        ...
        temp = templateData[(_tokenId/BASE_TEMPLATE_ID)*BASE_TEMPLATE_ID];

        temp.runs = metaData[_tokenId].runs;
        temp.matches = metaData[_tokenId].matches;
