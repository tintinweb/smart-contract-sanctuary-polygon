    ...
    Ownable
{
    ...
    bytes32 private magicCouponAdmin;
    ...
    ) ERC721(_symbol, _name)
    {
        ...
        bytes32 _magicCouponAdmin = keccak256("MAGIC_COUPON_ADMIN_ROLE");

        _setupContractURI(_contractURI);
        _setupRole(DEFAULT_ADMIN_ROLE, _defaultAdmin);
        _setupRole(_minterRole, _defaultAdmin);
        _setupRole(_magicCouponAdmin, _defaultAdmin);
        ...
        magicCouponAdmin = _magicCouponAdmin;
    ...
    function claimWithCoupon(
        ...
        (uint256 discountedPrice, string memory nonce) = _calcPrice(coupon, _currency, _tier);
        coupon.usedCount += 1;
    ...
    function _safeMintWithTier(
        ...
        // for (uint i; i < ownerTokens[_msgSender()].length; i++)
        // {
        //     if (strcmp(getTierForToken(ownerTokens[_msgSender()][i]), _tier))
        //         revert TierAlreadyOwned();
        // }

        // totalRemainingInTier[_tier] -= 1;
        // mintedInTier[_tier] += 1;
        // ownerTokens[_msgSender()].push(_tokenId);
   ...
   function getCouponIdx(string calldata _coupon)
    ...
    function _calcPrice(
        ...
        address _currency,
        string calldata _tier
    ) internal view returns (uint256 discountedPrice, string memory nonce) {
        if (_currency == address(0))
        {
            // extract the nonce
            nonce = substr(_coupon.id, 0, 11);
            if (strcmp(nonces[_msgSender()], nonce))
                revert CouponExhausted();

            // This recreates the message hash that was signed on the client.
            bytes32 hash = keccak256(abi.encodePacked(_msgSender(), "+", _tier, nonce)).toEthSignedMessageHash();
            bytes memory signature = b64decode(substr(_coupon.id, 12, bytes(_coupon.id).length));
            address signer = hash.recover(bytes(signature));

            // Verify that the message's signer has the fiat role
            if (!hasRole(magicCouponAdmin, signer))
