        ...
        for (uint256 i = 0; i < stakeIds[_user].length; i++) {
            StakeInfo storage stake = stakes[_user][stakeIds[_user][i]];
             if (!stake.unstaked && stake.roundNumber < TOTAL_ROUNDS) {
                uint256 roundEndTime = stake.startTime.add(ROUND_DURATION.mul(stake.roundNumber + 1));
                
                 if (block.timestamp >= roundEndTime) {
                    uint256 roundRewardPercentage = roundRewardPercentages[stake.roundNumber];
                    ...
                    stake.roundNumber = stake.roundNumber.add(1);
                    roundEndTime = stake.startTime.add(ROUND_DURATION.mul(stake.roundNumber + 1));
                }
            }
        }
    }
