    ...
    function registerOnEvent(uint256 _eventID) external payable {
        ...
        require(
            eventData[_eventID].state == EventState.RegisterStart &&
                block.timestamp <= eventData[_eventID].registDeadline,
...
contract SquidBetsCore is ISBCNFTType, CustomAdmin {
    ...
    event VoteEvent(uint256 eventID, VotingResult voteResult);

    event RoundAdded(uint256 eventID, uint8 roundLevel);
    ...
    function addRound(
        ...
        emit RoundAdded(_eventID, newLevel);
    }
    ...
    function reportResult(
        ...
        if (currentLevel == eventData[_eventID].totalRound) {
            if (winnersNumber == 1) {
                ...
                eventData[_eventID].state = EventState.OnPrizing;
    ...
    function vote(VotingResult _playerVote, uint256 _eventID) external {
        ...
        emit VoteEvent(_eventID, _playerVote);
    }
    ...
    function resultVote(uint256 _eventID) external onlyAdmin {
        if (eventVote[_eventID].splitPoint > eventVote[_eventID].soloPoint) {
            ...
            eventData[_eventID].state = EventState.OnPrizing;
            winnersNumberOfEvent[_eventID] = uint16(winnersOfFinalRound[_eventID]
                .length());
        } else {
            eventData[_eventID].state = EventState.OnPickWinner;
            eventData[_eventID].vResult = VotingResult.Solo;
        }
