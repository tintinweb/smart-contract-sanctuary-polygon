...
import "../registry/PartnerRegistry.sol";
import "../registry/INfcIdRegistry.sol";
...
contract Flex2Earn is OperatorAccessControlUpgradeable {
    mapping(address => uint256) public partnerBalances;
    mapping(uint256 => uint256) public paidRewardsByEvent;

    PartnerRegistry partnerRegistry;
    INfcIdRegistry nfcIdRegistry;
    IERC20 an1Token;
    Poap poap;
    ...
    modifier onlyPartner() {
        require(partnerRegistry.isPartner(msg.sender), "Flex2Earn::onlyPartner: caller is not a registered partner");
        _;
    }

    modifier onlyPartnerOrOperator() {
        require(
            partnerRegistry.isPartner(msg.sender) || partnerRegistry.isOperator(msg.sender),
            "Flex2Earn::onlyPartnerOrOperator: caller is not a registered partner nor operator");
        _;
    }

    modifier onlyClerk() {
        require(partnerRegistry.isClerk(msg.sender), "Flex2Earn::onlyClerk: caller is not a registered clerk");
        _;
    }

    function __Flex2Earn_initialize(address _partnerRegistry, address _nfcIdRegistry, address _an1Token, address _poap) public initializer onlyInitializing {
        __OperatorAccessControl_init(_msgSender());
        __Flex2Earn_initialize_unchained(_partnerRegistry, _nfcIdRegistry, _an1Token, _poap);
    }

    function __Flex2Earn_initialize_unchained(address _partnerRegistry, address _nfcIdRegistry, address _an1Token, address _poap) public onlyInitializing {
        partnerRegistry = PartnerRegistry(_partnerRegistry);
        nfcIdRegistry = INfcIdRegistry(_nfcIdRegistry);
        an1Token = IERC20(_an1Token);
        poap = Poap(_poap);
    }

    function depositReward(uint256 _amount) external onlyPartner {
        require(_amount > 0, "Flex2Earn::depositReward: the deposit must be greater than zero");
        require(
            an1Token.allowance(msg.sender, address(this)) >= _amount,
            "Flex2Earn::depositReward: Flex2Earn contract not allowed to move deposit"
        );
        require(an1Token.transferFrom(msg.sender, address(this), _amount), "Flex2Earn::depositReward: deposit transfer failed");
        partnerBalances[msg.sender] += _amount;

        emit TokensDeposited(msg.sender, _amount);
    }

    function awardFlexer(uint256 _flexedNfcId) external onlyClerk {
        require(_flexedNfcId > 0, "Flex2Earn::awardFlexer: NFC ID can't be zero");
        (address collection, uint tokenId) = nfcIdRegistry.getTokenForId(_flexedNfcId);
        require(collection != address(0), "Flex2Earn::awardFlexer: could not resolve NFC ID to collection");
        IPhygitalItems phygitalCollection = IPhygitalItems(collection);
        require(
            phygitalCollection.stateOf(tokenId) == IPhygitalItems.ItemStatus.REDEEMED,
            "Flex2Earn::awardFlexer: the NFT is not coupled with the scanned item"
        );
        address partner = partnerRegistry.partnerOfClerk(msg.sender);
        uint256 eventId = partnerRegistry.getClerkEventId(msg.sender);
        uint256 balance = partnerBalances[partner];
        uint256 reward = partnerRegistry.getRewardPerFlexer(eventId);

        require(partnerRegistry.isCollectionAllowed(eventId, collection), "Flex2Earn::awardFlexer: collection is not allowed");

        require(
            balance > reward,
            "Flex2Earn::awardFlexer: deposited reward balance insufficient"
        );

        uint256 tokenPool = partnerRegistry.getTokenPool(eventId);

        require(
            paidRewardsByEvent[eventId] < tokenPool,
            "Flex2Earn::awardFlexer: rewards exceeded token pool"
        );

        (uint256 startTime, uint256 endTime) = partnerRegistry.getStartAndEndTime(eventId);

        require(
            block.timestamp >= startTime &&
            block.timestamp <= endTime,
            "Flex2Earn::awardFlexer: cannot award flexer berfore startTime or after endTime"
        );

        partnerBalances[partner] -= reward;
        paidRewardsByEvent[eventId] += reward;
        address flexer = IERC721(collection).ownerOf(tokenId);
        an1Token.transfer(flexer, reward);

        if(partnerRegistry.isPoapAssignedToEvent(eventId)) {
            // poapId is the same like eventId
            poap.mint(flexer, eventId);
        }

        emit TokensAwarded(msg.sender, flexer, reward, eventId);
    }

    function withdraw(address partner) external onlyPartnerOrOperator {
        require(partnerRegistry.isPartner(partner), "Flex2Earn::withdraw: cannot withdraw into non registered partner");

        uint256 amount = partnerBalances[partner];
        an1Token.transfer(partner, amount);
        delete partnerBalances[partner];

        emit TokensWithdrawn(partner, amount);
    }
}
...
contract PartnerRegistry is OperatorAccessControlUpgradeable {
    ...
    function createEvent(uint256 startTime, uint256 endTime, uint256 rewardPerFlexer, uint256 another1TokenRewardPool, bool isPoapAssigned) public onlyPartner(msg.sender) {
        ...
        events.push(Event(msg.sender, startTime, endTime, rewardPerFlexer, another1TokenRewardPool, isPoapAssigned));
...
/**
 * @title AN1 INfcIdRegistry interface
 *
 * Contains methods to add NFC ID to registry and to query the registry for NFC IDs
 *
 * @dev See https://github.com/an1official/an1-contracts/
 */
interface INfcIdRegistry {
    ...
    /**
     * @dev Allows Operators to register NFC IDs with NFTs
     *
     * Requirements:
     * - Same number of _tokenIds, _collections and _nfcIds
     * - Only Operator may call this function.
     * - Each Collection must be a registered collection.
     * - Token with the given ID must be already minted
     * - Revert if the NFC ID is already assigned.
     * - NFC ID must not be zero.
     *
     * This method can only be called by the contract operator.
     *
     * Emits a {IdRegistered} event for every new id added to the registry.
     */
    function registerIds(uint256[] memory _nfcIds, address[] memory _collections, uint256[] memory _tokenIds) external;

    /**
     * @dev Returns the collection and tokenId for a given NFC ID
     *
     * Requirements:
     *
     * - Returns `0x0` for unset ids.
     * - Returns the collection address and the tokenId for a given nfcId.
     *
     */
    function getTokenForId(uint256 _nfcId) external view returns (address, uint256);

    /**
     * @dev Returns the NFC ID for a given collection and tokenId
     *
     * Requirements:
     *
     * - Returns `0x0` for unset tokens and collections.
     * - Returns the NFC ID for a given collection and tokenId
     *
     */
    function getNfcIdForToken(address _collection, uint256 _tokenId) external view returns (uint256);
}
...
contract Poap is ERC1155, Ownable {
    address private immutable flex2Earn;
    mapping(address => uint256[]) poapIdsByAddress;

    event PoapMinted(uint256 indexed popaId, address to);

    constructor(string memory uri, address _flex2Earn) ERC1155(uri) {
        require(_flex2Earn != address(0), "Poap::contrcutor: flex2Earn cannot be 0 address");
        flex2Earn = _flex2Earn;
    }

    modifier onlyOwnerOrFlex2Earn {
        require(msg.sender == owner() || msg.sender == flex2Earn, "Poap::onlyOwnerOrFlex2Earn: caller is not an owner neither Flex2Earn");
        _;
    }

    function mint(address to, uint256 poapId) public onlyOwnerOrFlex2Earn {
        if(!isPoapOwner(poapId, to)) {
            _mint(to, poapId, 1, "");
            poapIdsByAddress[to].push(poapId);
            emit PoapMinted(poapId, to);
        }
    }

    function safeTransferFrom(address, address, uint256, uint256, bytes memory) public pure override {
        revert("Poap::safeTransferFrom: Poap is not transferable");
    }

    function safeBatchTransferFrom(address, address, uint256[] memory, uint256[] memory, bytes memory) public pure override {
        revert("Poap::safeBatchTransferFrom: Poap is not transferable");
    }

    // public getters

    function isPoapOwner(uint256 poapId, address owner) public view returns(bool) {
        return balanceOf(owner, poapId) > 0;
    }

    function getOwnedPoaps(address owner) public view returns(uint256[] memory) {
        return poapIdsByAddress[owner];
    }
}
