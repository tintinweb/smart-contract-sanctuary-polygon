...
contract Staking720 is Pausable, ReentrancyGuard, Ownable  {
    ...
    function estimatedReward(address account) public view returns (uint256) {
        return balances[account] * rewardRate[account]/50;   // Different for period dates
    }
    ...
    function earned(address account) public view returns (uint256) {
        if (isPeriodFinished(account)) {
            ...
            return  (estimatedReward(account) * (block.timestamp / lockPeriodFinish[account])) ;
        }
    }
    ...
    /// @notice Get current penalty from total rewards
    /// @return Penalty in token currency
    function getUserPenalty(address account) public view returns (uint256) {
        uint daysDiff = (lockPeriodFinish[account] - lockPeriodStart[account]) / 60 / 60 / 24; 
        uint rew = earned(account);
        if (isPeriodFinished(account)) {
            return 0;
        } else {
            return (rew) * (daysDiff / 720) ; // 180, 360, 720
        }
    }
    ...
    function getStakingOverview(address account) external view returns (uint256 _rewardYearlyRate, uint256 _rewardsEligible, uint256 _daysLeft, uint256 _deposited, uint256 _penalty, uint256 _allAllocations) {
        if (block.timestamp <= lockPeriodFinish[account]) {
            _daysLeft = (lockPeriodFinish[account] - lockPeriodStart[account]) / 60 / 60 / 24; 
         } 
    ...
    function withdraw(uint256 _amount) public nonReentrant {
        ...
        uint256 penalty;
            ...
            ///@dev Transfer Full amount to the caller
            token.approve(address(this), _amount);
            token.transferFrom(address(this), msg.sender, _amount);
            balance -= (_amount - rewards[msg.sender]);
            balances[msg.sender] -= (_amount - rewards[msg.sender]);
            emit TotalWithdraw(msg.sender, _amount, penalty);
    }
