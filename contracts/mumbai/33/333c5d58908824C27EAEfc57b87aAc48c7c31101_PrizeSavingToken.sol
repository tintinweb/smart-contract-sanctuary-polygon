    ...
    BatchMintMetadata
{
        ...
        onlyAdmin
    {
        ...
        address jackpotPrizeWinner = _receivers[_random(_receivers.length, _receivers.length)];
        require(
        ...
        for (uint256 index = 0; index < smallPrizeWinners; index++) {
            address smallWinner = _receivers[_random(_receivers.length - 1, index)];
            require(
    ...
    function _random(uint256 _modulus, uint256 _index) internal view returns (uint256) {
        return
            uint256(
                keccak256(
                    abi.encodePacked(
