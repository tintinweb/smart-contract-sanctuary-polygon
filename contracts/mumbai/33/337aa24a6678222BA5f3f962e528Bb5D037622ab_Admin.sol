...
contract Admin is Initializable, AccessControl, BasicMetaTransaction, ISafe {
    ...
    function tradingBuy(
        ...
        uint256 tokenValue = _oraclePrice(_amount);
        if (tokenValue > 0) {
            // usdc.approve(tradingContract,_amount);
            usdc.transferFrom(_owner, roobaWallet, uint256(tokenValue));
        }
        emit BuyTrading(_amount, _owner, tradingContract, tnxId);
    }

    function _oraclePrice(uint256 _amount) public returns (uint256) {
    ...
    function addAgentOnIdentityRegistry(address _agent, address _token)
        external
    {
        IToken token = IToken(_token);
    ...
    function whitelistUser(
        ...
        IFactory(tokenFactory).deployAndWhitelist(
            _token,
            _userAddress,
            _countryCode
        );
    ...
    function batchWhitelistUser(
        ...
        IFactory(tokenFactory).batchDeployAndWhitelist(
            _token,
            _userAddress,
            _countrycodes
        );
    ...
    // function registerIdentityContract(
    //     address _userAddress,
    //     address _identity,
    //     uint16 _country,
    //     address _token
