...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner, ERC1155 {
    ...
    event DebugLog(string description, address requester);
        ...
        override
    {
        ...
        if (_error.length == 0 && requestIdToRequester[id] != address(0)) {
          ...
          string memory prompt = ""; // Retrieve the prompt associated with the request
          string memory completion = string(_response);
          emit DebugLog("Requester's address: ", requester);
          mintNFT(prompt, completion, requester);
        }
    }
    ...
    function mintNFT(string memory prompt, string memory completion, address requester) internal {
      require(requester != address(0), "Invalid requester address");
      uint256 newTokenId = tokenCounter;
      // Update the _mint() function call with the required arguments
      _mint(requester, newTokenId, 1, "");
