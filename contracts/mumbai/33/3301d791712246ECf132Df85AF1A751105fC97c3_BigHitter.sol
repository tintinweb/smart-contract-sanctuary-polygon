...
import "stl-contracts/attestation/VerifyAttestation.sol";
import "stl-contracts/ERC/ERC5169.sol";
import "stl-contracts/access/UriChanger.sol";
import "./ERC721EnumMetadata.sol";
 ...
 * {ERC721Enumerable}.
 */
contract BigHitter is Ownable, UriChanger, ERC721EnumMetadata, VerifyAttestation, ERC5169 {
    // using Address for address;
    ...
    string constant _contractName = "Catch Max";
    string constant _contractSymbol = "CATCHMAX";
    
    string public conferenceID = "55";
    address public attestorKey = 0x538080305560986811c3c1A2c5BCb4F37670EF7e;
    address public issuerKey = 0x09B6cC1cB661d523Db1F8E854cfbfE2A03100d95;

    address public beneficiary;

    event AttestationDataUpdated(address attestor, address issuer, string conferenceId);
    event BeneficiaryUpdate(address indexed prevAddress, address indexed newAddress);
    ...
    function _authorizeUpdateBaseURI(string memory newBaseURI) internal override onlyUriChanger {}
    
    /**
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
     */
    constructor () ERC721EnumMetadata(_contractName, _contractSymbol, _metadataURI) UriChanger(_msgSender()) {
        _setBeneficiary(_msgSender());
    }

    function setBeneficiary(address newAddress) public onlyOwner {
        _setBeneficiary(newAddress);
    }

    function _setBeneficiary(address newAddress) internal {
        emit BeneficiaryUpdate(beneficiary, newAddress);
        beneficiary = newAddress;
    }

    function updateAttestationData(address attestor, address issuer, string memory conferenceId) public onlyOwner {
        attestorKey = attestor;
        issuerKey = issuer;
        conferenceID = conferenceId;
        emit AttestationDataUpdated(attestor, issuer, conferenceId);
    }
    ...
    function mintUsingAttestation(bytes memory attestation) public returns (uint256 tokenId) {
        ...
        (subject, tokenBytes, conferenceBytes, timeStampValid) = verifyTicketAttestation(attestation, attestorKey, issuerKey);
    ...
    function mintDirect(address _erc20contract, uint price, uint tokenId, uint ttl, bytes calldata signature) public payable {

        bytes memory toSign = abi.encodePacked(address(this), msg.sender, block.chainid, _erc20contract, price, tokenId, ttl );

        (address recovered,) = keccak256(toSign).toEthSignedMessageHash().tryRecover(signature);

        require(recovered == uriChanger(), "Wrong metadata signer");
        require(ttl > block.timestamp, "Request expired");

        if (_erc20contract == address(0)){
            // collect MATIC tokens under this contract
            require(msg.value == price, "Wrong token amount attached");
        } else {
            // send ERC20 funds immediately to save lot of GAS
            IERC20(_erc20contract).safeTransferFrom(_msgSender() ,beneficiary, price);
        }
    ...
    /**
    anyone can run withdraw(), but beneficiary will get funds
     */
    ...
    function mintByOperator(address to, uint tokenId) public onlyUriChanger {
    ...
    function burn(uint256 tokenId) public {
        require(_exists(tokenId), "BH: invalid token ID");
        require(_isApprovedOrOwner(_msgSender(), tokenId), "BH: caller is not token owner nor approved");
        _burn(tokenId);
    }
...
abstract contract ERC721EnumMetadata is ERC721Enumerable {
    ...
    constructor( string memory _contractName, string memory _contractSymbol, string memory __metadataURI ) ERC721(_contractName, _contractSymbol) {
        metadataURI = __metadataURI;
    }

    function contractURI() public view returns(string memory) {
        return string(abi.encodePacked(metadataURI, "/contract/", symbol(), JSON_FILE));
    }
        ...
        if (bytes(base).length > 0) {
            ...
            return string(abi.encodePacked(metadataURI, "/", block.chainid.toString(), "/", contractAddress(), "/", tokenId.toString()));
        }
    }
    ...
    function setBaseURI(string memory baseURI_) public {
         _authorizeUpdateBaseURI(baseURI_);
    ...
    function _authorizeUpdateBaseURI(string memory newBaseURI) internal virtual;
