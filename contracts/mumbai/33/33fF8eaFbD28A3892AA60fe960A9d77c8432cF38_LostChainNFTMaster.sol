...
contract LostChainNFTMaster is
    ...
    UUPSUpgradeable
{
    ...
    function setRBALinker(address rbaLinkerAddress) public onlyRole(LCRBALinker.getRole('DEFAULT_ADMIN_ROLE')) {
        require(rbaLinkerAddress != address(0x0), 'LCNFTM: RBA linker is zero address');
    ...
    function initialize(LostChainForwarder forwarder, address rbaLinkerAddress) public initializer {
        require(address(forwarder) != address(0x0), 'LCNFTM: forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LCNFTM: RBA linker is zero address');
        LCRBALinker = ILostChainRBALinker(rbaLinkerAddress);
    ...
    function mint(
        ...
        bytes memory data
    ) public onlyRole(LCRBALinker.getRole('GAME_MANAGER')) {
    ...
    function TESTING20() external pure returns (bool) {
        return true;
    }
}
