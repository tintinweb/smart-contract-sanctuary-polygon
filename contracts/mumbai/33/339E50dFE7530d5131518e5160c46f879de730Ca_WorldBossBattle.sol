...
interface BattleOps {
    ...
    function restake(uint256 _tokenId) external;
    function restakeTokens(uint256[] memory _tokenIds) external;
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/v4.5.0/contracts/utils/cryptography/ECDSA.sol
...
contract WorldBattleSession is Ownable, GameAccessControls {
    ...
    mapping(address => uint) public syncedTime;
...
contract WorldBossBattle is Context, WorldBattleSession, BattleOps {
    using ECDSA for bytes32;
    ...
    // Structure of the boss in the game
    struct Boss {
        uint256 bossId;
        string name;
        uint256 maxHp;
    ...
    function stake(uint256 _tokenId) override external {
        internalStake(_msgSender(), _tokenId);
    }

    function stakeAll() override external {
        internalStakeAll(_msgSender());
    }

    function stakeTokens(uint256[] memory tokenIds) override external {
        address player = _msgSender();
        for(uint i = 0; i < tokenIds.length; i++){
            internalStake(player, tokenIds[i]);
        }
    }
    ...
    function unstake(uint256 _tokenId) override external {
        require(tokenOwner[_tokenId] == _msgSender(), "NFTStaking: Unauthorized.");
        internalStake(_msgSender(), _tokenId);
    }

    function unstakeAll() override external {
        internalUnstakeAll(_msgSender());
    }

    function unstakeTokens(uint256[] memory tokenIds) override external {
        address player = _msgSender();
        for(uint i = 0; i < tokenIds.length; i++){
            internalUnstake(player, tokenIds[i]);
        }
    }
    ...
    function internalStake(address _player, uint256 _tokenId) internal {
        ...
        if(checkReqiredTokenBalance(existingStakedTokensCount + 1, towerBalance)){
            ...
            emit NFTStaked(_player, _tokenId);
        } else {
    ...
    function internalStakeAll(address _player) internal {
        uint256[] memory allTokens = cdhNFT.getAllTokens(_player);
        uint256 nftBalance = allTokens.length;
        if (nftBalance > 0 ){
            for (uint i = 0; i < nftBalance; i++){
                internalStake(_player, allTokens[i]);
            }
        }
    }

    function internalStakeBatch(address _player, uint256[] memory _tokenIds) internal {
        for (uint i = 0 ; i< _tokenIds.length; i++){
            internalStake(_player, _tokenIds[i]);
        }
    }

    function internalUnstake(address _player, uint256 _tokenId) internal {
        Staker storage staker = stakers[_player];
        ...
        emit NFTUnstaked(_player, _tokenId);
    }

    function internalUnstakeAll(address _player) internal {
         uint256[] memory allTokens = cdhNFT.getAllTokens(_player);
        uint256 nftCounts = allTokens.length;
        if (nftCounts > 0 ){
            for (uint i = 0; i < nftCounts; i++){
                internalUnstake(_player, allTokens[i]);
            }
        }
    }

    function internalUnstakeBatch(address _player, uint256[] memory _tokenIds) internal {
        for (uint i = 0; i < _tokenIds.length; i++) {
            if (tokenOwner[_tokenIds[i]] == msg.sender) {
                internalUnstake(_player, _tokenIds[i]);
            }
        }
    }

    function restake(uint256 _tokenId) override external {
        // restake existing staked tokenId
        // check if tokenId is of owner
        // check if tokenId is staked
        // check if tokenId is valid
    }

    function restakeTokens(uint256[] memory _tokenIds) override external {

    }

    function restakeAll() override external{
