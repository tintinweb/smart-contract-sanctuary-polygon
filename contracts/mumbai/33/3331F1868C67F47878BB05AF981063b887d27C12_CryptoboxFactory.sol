...
contract Cryptobox is ICryptobox, EternalOwnable {
    ...
    function dispenseMany(
        ...
        require(candidates.length <= _capacity - _totalParticipated);
        _requireIsActive();
    ...
    function _requireIsActive() internal view {
        if (!_active) revert CryptoboxIsFinished();
    }
    ...
    function _reward(Participation.Participant memory candidate) private {
        if (_participated(candidate)) revert AlreadyParticipated();
    ...
    function _reward(Participation.Participant[] memory candidates) private {
        ...
        for (uint i = 0; i < candidates.length; i++) {
            if (_participated(candidates[i])) revert AlreadyParticipated();
            _participatedAddresses[candidates[i].addr] = true;
            _participatedNames[candidates[i].name] = true;
        }
        for (uint i = 0; i < candidates.length; i++) {
