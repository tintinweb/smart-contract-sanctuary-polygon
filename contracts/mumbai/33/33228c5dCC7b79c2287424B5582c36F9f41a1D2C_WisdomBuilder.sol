...
// File: WisdomBuilder.sol

contract WisdomBuilder is AbstractDynamicURI, BaseViciContract {
    ...
    function setExternalURL(string memory _externalURL) 
        public
        onlyOwnerOrRole(URI_MANAGER)
    {
    ...
    function setImageData(string memory _imageData)
        public
        onlyOwnerOrRole(URI_MANAGER)
    {
    ...
    function getImageUri() public view returns (string memory) {
        return string.concat("data:image/svg+xml;base64,", imageData);
    }

    function tokenURI(uint256 tokenId) public view returns (string memory) {
        string memory tokenName;
        if (tokens.hasName(tokenId)) {
            tokenName = tokens.getName(tokenId);
        } else {
