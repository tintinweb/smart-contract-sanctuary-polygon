...
import { ABI_MULTISIG_REVEAL_SALE, ABI_REVEAL_SALE, ABI_VOUCHER_CLOSED_SALE_MINT, ABI_CLOSED_SALE_MINT, ABI_RESERVE_TOKENS, ABI_MULTISIG_RESERVE_TOKENS} from "./FunctionAbiV5.sol";
...
contract Erc721ClosedSaleV5 is Erc721SaleV5 {
  ...
  function reserveTokens(uint256 tokenCount_, MultiSigParams calldata params_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
    ...
    _reserveTokensTo(to_, params_);
  }

  function reserveTokensTo(address[] memory to_, MultiSigParams calldata params_) external onlyRole(CONTRACT_ADMIN_ROLE) whenNotPaused {
    _reserveTokensTo(to_, params_);
  }

  function _reserveTokensTo(address[] memory to_, MultiSigParams calldata params_) internal {
    require(totalSupply + to_.length <= maxSupply, "17");
    require(!msg.sender.isContract(), "16");
    require(to_.length > 0, "20");
    _verifyMultisig(
      abi.encode(ABI_MULTISIG_RESERVE_TOKENS, to_.length, params_.nonce, ABI_RESERVE_TOKENS),
      params_,
      CONTRACT_ADMIN_ROLE
    );
    for (uint256 i = 1; i <= to_.length; i++) {
      _safeMint(to_[i-1], _tokenIdCounter.current());
...
bytes32 constant ABI_MULTISIG_RESERVE_TOKENS = keccak256("MultiSig(string baseURI,uint256 nonce,string functionABI)");
