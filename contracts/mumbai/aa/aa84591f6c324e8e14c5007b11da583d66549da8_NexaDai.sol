...
/*
   ...
   ---------------------------------------------------------------------------------
   Copyright (c) 2023 onwards NexaDai crowdfunding Community. ( https://nexadai.io )
   ---------------------------------------------------------------------------------                                                           
...
// Replace below address with main token token
    ...
    address constant oldContract= 0x8b6a7f448e9a32443c5F852806857cb868B18aEA;// old contract for data fetching.
   ...
   struct userInfo {
    ...
    function regUser( address _referral ) public returns(bool){
     ...
     require(_referral!=zeroAddress,"EC-1");
     require(userInfos[msgSender].referral==zeroAddress,"EC-2");
    ...
    function buyTopup() public returns (bool) {
        ...
        address msgSender=msg.sender;
        require(userInfos[msgSender].referral!= zeroAddress || msgSender==defaultAddress,"EC-3");
       
        uint128 payAmot=JOIN_PRICE;
        // internal buy mode.
        if (userInfos[msgSender].joined==false){
           payAmot=(JOIN_PRICE-POOL_PRICE);
            _buyMode(msgSender,payAmot);
        }   
        else _buyMode(msgSender,payAmot);
        userGains[msgSender].withdrawLimit += JOIN_PRICE*3;
       
        _activateUser(msgSender);
        ERC20In(tokenAddress).transfer(defaultAddress,(payAmot*sysInfos.withdrawFee/100)); //System fee.
    ...
    function _activateUser(address msgSender) internal {
        ...
        uint128 payAmount=(_payCulculate(ref,1,(SPONSER_PAY+poolPay)));

        if(payAmount>0){
            if(ref==zeroAddress) ref=defaultAddress;
            userGains[ref].totalSponserGains+=payAmount;
            emit directIncomeEv(msgSender,ref,SPONSER_PAY);
            if(poolPay>0)emit poolSponsorBonusEv(userInfos[msgSender].id,userInfos[ref].id,POOL_UPLINE_BONUS);
    ...
    function buyPool_2X() external  returns(bool) {
        ...
        if(tokenAddress!=zeroAddress)_buyMode(msgSender,POOL_PRICE);
    ...
    function transferCredit(address _to, uint128 _amount) external  returns(bool) {
      ...
      // Deposit credits fund .
        ...
        require(userInfos[msgSender].referral!=zeroAddress,"EC3");
        userGains[msgSender].creditFund+=_amount;
...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
   ...
   function _updateTeamNum(address ref) internal  {
        ...
        for(uint i; i < TEAM_DEPTH; i++){
            ...
            if(usr != zeroAddress)userInfos[usr].teamCount++;
            if(ref == zeroAddress) break;
    ...
    function _defaultUser(address _user) internal {
        ...
        _autoPool3xPosition(_user,false);
         ...
         emit regUserEv(_user,zeroAddress,lastIDCount);
  ...
  function _distributeGapBonus(address _user,address ref)  internal returns(bool){
    ...
    function _distributeUnilevelIncome(address _user,address _referral) internal {
        ...
        for (uint i=1 ; i <= TEAM_DEPTH; i++)
        {
            ...
            if(usr == zeroAddress || userInfos[usr].activeDirect<2) usr = defaultAddress;
    ...
    function _closingRoyality() internal {
        ...
        uint current = now;
        if(nextRoyalty<=current || (!sysInfos.poolSpan && defaultAddress==msg.sender) ){
            //global royalty data
    ...
    function _transferTo(address user,uint128 amot)internal {
        if(!dataFetchDone)return;
          ...
          if(tokenAddress!=zeroAddress) ERC20In(tokenAddress).transfer(user,(amot-(amot*sysInfos.withdrawFee/100))); // zero balance call    
    ...
    function syncIndex() internal {
        ...
        if (nextMemberDownlineFill==0){
            ...
            if(recMindex==0 && defID!=1){

                while (recMindex==0 && defID!=1){
    ...
    require(msg.sender==defaultAddress,"EC-4");
        for (uint i ; i< _dataWriteLimit;i++){
            ...
            (,,,uint128 totalGlobalRoyalityGains,uint128 totalActiveRoyalityGains,,,,,uint128 withdrawLimit,,,)=dataContract(oldContract).userGains(user);
            if(totalActiveRoyalityGains>0){
              userInfos[user].AR_index=royalty_Index;
              userGains[user].totalActiveRoyalityGains=totalActiveRoyalityGains;
            } 
            if(totalGlobalRoyalityGains>0){
                userInfos[user].GR_index=royalty_Index;
                userGains[user].totalGlobalRoyalityGains=totalGlobalRoyalityGains;
            } 
            //write the storage.
            uint tot=totalGains_(user);
    ...
    function fetchPools(uint _dataWriteLimit) external  {
        ...
        // validate parameters
             ...
             for (uint i ; i< _dataWriteLimit;i++){
                ...
                (uint poolIn2xuserID,,uint poolIn2xmIndex)= dataContract(oldContract).autoPool2xDataList(now2xIndex);
                (uint poolIn3xuserID,,uint poolIn3xmIndex) = dataContract(oldContract).autoPool3xDataList(now3xIndex);
            
                address _nowAddress = dataContract(oldContract).userAddressByID(poolIn2xuserID);

                if(poolIn2xuserID==poolIn3xuserID && m2In==poolIn2xmIndex && last3xIndex!=now3xIndex && (m3In==poolIn3xmIndex || poolIn3xmIndex==0)){   
                    //topup
                    _activateUser(_nowAddress);  
                }
                 
                else if ((poolIn2xuserID!=poolIn3xuserID || last3xIndex==now3xIndex) && m2In==poolIn2xmIndex){
                    //Pool entry
                    _autoPool2xPosition(_nowAddress,true);
                    userInfos[_nowAddress].globalPoolCount++;
                    address ref=userInfos[_nowAddress].referral;
                    if(ref==zeroAddress)ref=defaultAddress;
                    userGains[ref].totalSponserGains+=POOL_UPLINE_BONUS;
                    emit poolSponsorBonusEv(userInfos[_nowAddress].id,userInfos[ref].id,POOL_UPLINE_BONUS);
