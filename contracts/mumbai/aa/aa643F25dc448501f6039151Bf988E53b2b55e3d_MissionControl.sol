...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    */
    event RadiusSet(uint newRadius);
    ...
    mapping(address => mapping(int => mapping(int => TileElement[2]))) tiles;
    mapping(address => uint) stakeNonces;
    mapping(uint => bool) raidSpent;
    ...
    // Constants
    uint constant WASTE_ID = uint(IAssetManager.AssetIds.Waste);
    bytes32 private constant RAID_MESSAGE =
    keccak256(
        "RaidMessage(address _defender,uint256 _timestamp,uint256 _raidId)"
    );
    ...
    */
    modifier validCoordinates(int _x, int _y, int _z){
        int sum = _x + _y + _z;
        uint rad = (abs(_x) + abs(_y) + abs(_z)) / 2;
    ...
    */
    modifier onWhitelist(address _tokenAddress){
        require(whiteList[_tokenAddress], "MISSION_CONTROL: Not whitelisted");
        _;
    }
    /**
    ...
    */
    function initialize(uint _radius, address _mint) external initializer {
    ...
    function placeNFT(int _x, int _y, int _z, uint _tokenId, address _tokenAddress) override external {
        __placeNFT(_x, _y, _z, _tokenId, _tokenAddress);
    }

    /// @inheritdoc IMissionControl
    function placeNFTs(PlaceOrder[] memory _placeOrders) override external {
        for (uint i; i < _placeOrders.length; i++) {
            __placeNFT(_placeOrders[i].x, _placeOrders[i].y, _placeOrders[i].z, _placeOrders[i].tokenId, _placeOrders[i].tokenAddress);
        }
    }

    /// @inheritdoc IMissionControl
    function removeNFT(int _x, int _y, int _z) external override validCoordinates(_x, _y, _z) {
    ...
    /// @inheritdoc IMissionControl
    function checkTile(address _user, int _x, int _y, int _z) external view validCoordinates(_x, _y, _z) override returns (uint, uint){
    ...
    */
    function collectFromTiles(CollectOrder[] memory _orders) external override {
    ...
    external override {
        ...
        address signer = ECDSA.recover(_hashTypedDataV4(keccak256(
                abi.encode(RAID_MESSAGE, _defender, _timestamp, _raidId)
            )), _v, _r, _s);
        require(signers[signer], "MISSION_CONTROL: INVALID_SIGNATURE");
        require(int(block.timestamp) - int(_timestamp) < raidTimeout, "MISSION_CONTROL: Signature has timed out");
    ...
    /// @inheritdoc IMissionControl
    function checkStakedOnTile(address _user, int _x, int _y, int _z)
    external validCoordinates(_x, _y, _z) override view returns (TileElement memory _base, TileElement memory _top){
    ...
    */
    function abs(int _value) private pure returns (uint){
    ...
    */
    function _handleBasePlacement(int _x, int _y, int _z, uint _tokenId, address _stakeable, uint _nonce) private {
    ...
    * @param _stakeable Interface towards the token contract
    */
    function _handleTopPlacement(int _x, int _y, int _z, uint _tokenId, address _stakeable, uint _nonce) private {
        require(
            tiles[msg.sender][_x][_y][0].nonce != 0
            && tiles[msg.sender][_x][_y][1].nonce == 0,
            "MISSION_CONTROL: Invalid placement");
        IMissionControlStakeable(_stakeable).onStaked(msg.sender, _tokenId, _nonce, tiles[msg.sender][_x][_y][0].stakeable, tiles[msg.sender][_x][_y][0].tokenId);
    ...
    */
    function __placeNFT(int _x, int _y, int _z, uint _tokenId, address _tokenAddress) private validCoordinates(_x, _y, _z) onWhitelist(_tokenAddress) {
        // If the sender is not the owner of the token, the transaction should revert here
        if (IMissionControlStakeable(_tokenAddress).isBase(_tokenId)) {
            _handleBasePlacement(_x, _y, _z, _tokenId, _tokenAddress, ++stakeNonces[msg.sender]);
        } else {
            _handleTopPlacement(_x, _y, _z, _tokenId, _tokenAddress, ++stakeNonces[msg.sender]);
        }

        emit NFTPlaced(msg.sender,
            _tokenAddress,
            _tokenId,
            _x,
            _y,
    ...
    * @param _z Z-coordinate of tile
    */
    function __removeNFT(int _x, int _y, int _z) private validCoordinates(_x, _y, _z) {
        TileElement[2] storage tileElements = tiles[msg.sender][_x][_y];
        uint zeroOrOne = __zeroOrOne(_x, _y, msg.sender);
    ...
    */
    function __zeroOrOne(int _x, int _y, address _sender) internal view returns (uint zeroOrOne){
    ...
    function _handle_collect(int _x, int _y, int _z) private validCoordinates(_x, _y, _z) returns (uint, uint){
        TileElement memory tileElement = tiles[msg.sender][_x][_y][__zeroOrOne(_x, _y, msg.sender)];
        return IMissionControlStakeable(tileElement.stakeable).onCollect(msg.sender, tileElement.nonce);
    }

    function _handle_raid_collect(
        address _defender, int _x, int _y
    ) private validCoordinates(_x, _y, - (_x + _y)) returns (uint, uint){
    ...
    */
    function setRadius(uint _radius) external onlyOwner {
        require(_radius > 0, "Radius can't be zero");
        allowedRadius = _radius;
        emit RadiusSet(_radius);
    }

    function setRaidParams(int _timeout) external onlyOwner {
...
interface IMissionControl {
   ...
   * @return uint Token identifier for token to be collected
    ...
    * @dev Any downside to using structs as an argument?
    */
    function collectFromTiles(CollectOrder[] calldata _orders) external;
