  ...
  IERC2981
{
  ...
  function safeBatchMint(address to, BatchInfo[] memory tokenIds_) public {
    require(msg.sender == allowedMinter, "Only allowed minter can mint");

    uint256 arraySize = tokenIds_.length;
    ...
    unchecked {
      for (uint256 i = 0; i < arraySize; i++) {
        ...
        _safeMint(to, tokenId);
        _setTokenURI(tokenId, tokenIds_[i].polygonTokenURI);
        _tokenIdCounter.increment();
        setRoyalty(tokenId, collectionCreator, royaltyPercentage);

        emit BindNFT(tokenId, tokenIds_[i].terraTokenId, to, tokenIds_[i].polygonTokenURI);
      }
    }
  }
