...
// File: contracts/oracle/EnumerableSet.sol
...
contract ETHPriceOracle is AccessControl {
    ...
    address public chainlinkUsdEthPrice;
    bool public useChainlink;

    event AssetPriceUpdated(address _asset, uint256 _price, uint256 timestamp);
    event EthPriceUpdated(uint256 _price, uint256 timestamp);

    constructor(address _chainlinkUsdEthPrice) public {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(OPERATORROLE, msg.sender);
        chainlinkUsdEthPrice = _chainlinkUsdEthPrice;
        useChainlink = true;
    }
    ...
    function getEthUsdPrice() external view returns (uint256) {
        if (useChainlink) {
            return IChainlinkAggregator(chainlinkUsdEthPrice).latestAnswer();
        } else {
            return ethPriceUsd;
        }
    }

    function setEthUsdPrice(uint256 _price, bool _useChainlink) external {
        require(hasRole(OPERATORROLE, msg.sender), "Caller is not a operator");
        ethPriceUsd = _price;
