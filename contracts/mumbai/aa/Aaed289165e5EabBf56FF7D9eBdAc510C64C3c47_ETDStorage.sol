...
//         }
    ...
    function getUserAvailableBalance(string memory user) public view returns(int){
        ...
        for (uint j = 1; j < 7; j++){
            uint _chargeT = 0;
            t = t-dayx*24*60*60;
            (uint _year, uint _month,uint _day) = _daysToDate(t);
            dayx=_day;            
            if (userChargeFlag[user][_year][_month] == 0){
                 (, _chargeT) = getUserMonthDetail(user,_year,_month);
             }
            _charge = _charge + _chargeT;
        }
    ...
    function setStorageToken(address _token) external onlyOwner {				
        StorageToken = IERC20Burnable(_token);				
    }

    function setStorageNFT(address _token) external onlyOwner {				
    ...
    // �����ã�
    function setDetail(string memory owner, uint16 year,uint8 month,uint8 day) external {
        DailyDetail memory detail = storageDailyRec[owner][uint16(year)][uint8(month)][uint8(day-1)];
        detail.day = day;
        detail.totalSize = 80000000000 ;
        detail.dailySummary = 2000;
        detail.dailycount = 3;
        detail.totalCount = 300;

        storageDailyRec[owner][uint16(year)][uint8(month)][uint8(day)] = detail; 
    }
