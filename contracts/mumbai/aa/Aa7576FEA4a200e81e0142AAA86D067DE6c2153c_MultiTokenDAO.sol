...
abstract contract GovernorVotesQuorumFraction is GovernorVotes {
    ...
    function quorum(uint256 blockNumber) public view virtual override returns (uint256) {
        uint256 pastSupplyOfVotingTokens;
        for(uint8 i; i<votingTokens.length; i++) {
            pastSupplyOfVotingTokens += votingTokens[i].getPastTotalSupply(blockNumber);
        }
        return (pastSupplyOfVotingTokens * quorumNumerator()) / quorumDenominator();
    }
...
abstract contract GovernorPreventLateQuorum is Governor {
    ...
    function _castVote(
        ...
        uint256 result = super._castVote(tokenIndex, proposalId, account, support, reason, params);
...
abstract contract Governor is Context, ERC165, EIP712, IGovernor, IERC721Receiver, IERC1155Receiver {
    ...
    bytes32 public constant BALLOT_TYPEHASH = keccak256("Ballot(uint8 tokenIndex,uint256 proposalId,uint8 support)");
    bytes32 public constant EXTENDED_BALLOT_TYPEHASH =
        keccak256("ExtendedBallot(uint8 tokenIndex,uint256 proposalId,uint8 support,string reason,bytes params)");
    ...
    function propose(
        ...
        require(
            getAllVotes(_msgSender(), block.number - 1) >= proposalThreshold(),
     ...
     */
    function getVotes(uint8 tokenIndex, address account, uint256 blockNumber) public view virtual override returns (uint256) {
        return _getVotes(tokenIndex, account, blockNumber, _defaultParams());
    }

    function getAllVotes(address account, uint256 blockNumber) public view virtual override returns (uint256) {
        return _getAllVotes(account, blockNumber, _defaultParams());
    }
    ...
    function getVotesWithParams(
        ...
        return _getVotes(tokenIndex, account, blockNumber, params);
    }

    /**
     * @dev See {IGovernor-castVote}.
     */
    function castVote(uint8 tokenIndex, uint256 proposalId, uint8 support) public virtual override returns (uint256) {
        address voter = _msgSender();
        return _castVote(tokenIndex, proposalId, voter, support, "");
    }
    ...
    function castVoteWithReason(
        ...
        address voter = _msgSender();
        return _castVote(tokenIndex, proposalId, voter, support, reason);
    }
    ...
    function castVoteWithReasonAndParams(
        ...
        address voter = _msgSender();
        return _castVote(tokenIndex, proposalId, voter, support, reason, params);
    }
    ...
    function castVoteBySig(
        ...
        address voter = ECDSA.recover(
            _hashTypedDataV4(keccak256(abi.encode(BALLOT_TYPEHASH, tokenIndex, proposalId, support))),
            v,
            r,
            s
        );
    ...
    function _castVote(
        ...
        return _castVote(tokenIndex, proposalId, account, support, reason, _defaultParams());
    }
    ...
    function _castVote(
        ...
        uint256 weight = _getVotes(tokenIndex, account, proposal.voteStart.getDeadline(), params);
        _countVote(proposalId, account, support, weight, params);

        if (params.length == 0) {
            emit VoteCast(account, proposalId, support, tokenIndex, weight, reason);
        } else {
            emit VoteCastWithParams(account, proposalId, support, tokenIndex, weight, reason, params);
        }
...
abstract contract IGovernor is IERC165 {
     ...
     */
    event VoteCast(address indexed voter, uint256 proposalId, uint8 support, uint8 tokenIndex, uint256 weight, string reason);
     ...
     */
    function getVotes(uint8 tokenIndex, address account, uint256 blockNumber) public view virtual returns (uint256);

    function getAllVotes(address account, uint256 blockNumber) public view virtual returns (uint256);
     ...
     */
    function castVote(uint8 tokenIndex, uint256 proposalId, uint8 support) public virtual returns (uint256 balance);
