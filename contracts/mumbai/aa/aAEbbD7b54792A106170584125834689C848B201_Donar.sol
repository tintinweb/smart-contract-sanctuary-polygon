...
contract Donar is ERC20 {
     ...
     * @author Okhamena Azeez and Daniel
    ...
    address[] private s_NgoTrackingList;
    address[] private s_donorsTrackingList;
 ...
 */
    ...
    */
    function getReturnSameAddress() public  view returns (uint256) {
        for (uint256 i = 0; i < s_donorsList.length; i++) {
            if (s_donorsList[i] == msg.sender) {
                return i;
            }
        }
    }
    ...
    */
    function donate(address _toNgo) public payable {
        ...
        for (uint256 i = 0; i < s_donorsList.length; i++) {
            if (s_donorsTrackingList[i] == _toNgo) {
        ...
        s_NgoList.push(_toNgo);
        s_donorsTrackingList.push(msg.sender);
        s_NgoTrackingList.push(_toNgo);
    ...
    */
    function HowManyDonorExist() public view onlyOwner returns (uint256) {
        return s_donorsTrackingList.length;
    }
    /**
    ...
    */
    function HowManyNgoExist() public view onlyOwner returns (uint256) {
        return s_NgoTrackingList.length;
    }
    ...
    */
    ...
    function NgoSendToHelpNeeded(
        ...
        for (uint256 i = 0; i < s_donorsList.length; i++) {
            if (
                s_donorsTrackingList[i] == _ngo ||
                s_donorsTrackingList[i] == _to
            ) {
                revert Donar__InvalidAddress();
            }
        }
    ...
    * @notice - This function can only be performed by the deployer of the contract and it is  used for testing
    * @return - address at a particular position in the Donor array
    */
    function getDonorArray(uint256 _i) public view onlyOwner returns (address) {
    ...
    */
    function allowDonationToNgo(address _ngo) public onlyOwner view returns (bool) {
        for (uint256 i = 0; i < s_NgoTrackingList.length; i++) {
            if (s_NgoTrackingList[i] == _ngo) {
                return true;
            }
        }
    } 
    ...
    */
    function getNgoArray(uint256 _i)  public onlyOwner view returns (address) {
        return s_NgoList[_i];
    }
    ...
    */
    function getDonorPositionInTheArray(uint256 _i) public view  returns (address) {
        return s_donorsTrackingList[_i];
    }
