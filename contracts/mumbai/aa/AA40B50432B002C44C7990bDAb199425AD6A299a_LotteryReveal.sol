...
contract LotteryReveal is VRFConsumerBaseV2, Ownable {
    ...
    mapping(uint256 => bytes32) internal revealedType;
    ...
    function revealCollection(
        ...
        bytes32[] calldata tokenLabels
    ) external isOpen onlyOwner {
        requestRandomWords();
        require(randomSeed != 0, "RandomSeed not called");
        tokenIds = shuffle(tokenIds, randomSeed);
        for (uint256 i; i < tokenIds.length; i++) {
            revealedType[tokenIds[i]] = tokenLabels[i];
        }
        lottery_state = LOTTERY_STATE.CLOSED;
    }

    function getRevealToken(uint256 tokenID) external view returns (bytes32) {
        return revealedType[tokenID];
    }
