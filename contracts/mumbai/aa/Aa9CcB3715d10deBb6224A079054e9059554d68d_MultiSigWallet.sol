...
contract MultiSigWallet {
    ...
    mapping(uint256 => uint256) proposalLockTime;
    uint256 public constant DELAY_TIME = 2 days;
        ...
        onlyOwner
    {
        ...
        proposalLockTime[txIndex] = block.timestamp;
        transactions.push(
            Transaction({
        ...
        notExecuted(_txIndex)
    {
        ...
        require(
            block.timestamp >= (proposalLockTime[_txIndex] + DELAY_TIME),
            "Can not execute before delay time"
        );
        ...
        notExecuted(_txIndex)
    {
        ...
        require(
            block.timestamp > (proposalLockTime[_txIndex] + DELAY_TIME),
            "Can not revoke after delay time"
        );

        transaction.numConfirmations -= 1;
        isConfirmed[_txIndex][msg.sender] = false;
        delete proposalLockTime[_txIndex];
