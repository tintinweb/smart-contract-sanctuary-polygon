...
interface EthermonStakingInterface {
    ...
    function updateNewSumTeamPower(uint256 _sumTeamPower, uint256 _totalStaked)
        external;
    ...
    function increaseSumTeamPower(uint256 _teamPower, uint256 _emons) external;

    function decreaseSumTeamPower(uint256 _teamPower, uint256 _emons) external;
...
contract EthermonStaking is EthermonStakingBasic {
    ...
    uint16[] public daysToStake = [1, 7, 14, 30, 60, 120];
    uint16[] public daysAdvantage = [10, 11, 12, 13, 17, 25];
        ...
        onlyModerators
    {
        ...
        if (_daysToStake.length < daysToStake.length) {
            for (uint256 index = 0; index < daysToStake.length; index++) {
                if (index >= _daysToStake.length - 1) {
                    daysToStake.pop();
                    continue;
                }
        ...
        onlyModerators
    {
        ...
        if (_daysAdvantage.length < daysAdvantage.length) {
            for (uint256 index = 0; index < daysAdvantage.length; index++) {
                if (index >= _daysAdvantage.length) {
                    daysAdvantage.pop();
                    continue;
                }
    ...
    function depositeTokens(
        ...
        stakingData.increaseSumTeamPower(data.teamPower, data.emons);
    ...
    function updateTokens(
        ...
        uint256 emonPerPeriod = stakingData.EmonPerPeriod();
        ...
        if (timeElapsed > 0) {
            if (currentTime >= data.endTime) {
                ...
                stakingData.decreaseSumTeamPower(data.teamPower, data.emons);
    ...
    function updateStakingData(
        ...
        uint256 totalStaked = stakingData.totalStaked();
        ...
        if (_depositeToken._day > data.duration) {
            ...
            require(
                maxStakingTime > endTime &&
                    ((maxStakingTime - endTime) / 1 minutes) >= daysToStake[0],
                "Date is too high for staking"
            );
        ...
        stakingData.updateNewSumTeamPower(sumTeamPower, totalStaked);
