...
contract ERC721A is IERC721A {
    ...
    event Deposit(address tokenAddress, uint256 amount, uint256 lockTime);
    ...
    mapping(uint256 => uint256) private _lockedBalances;
    ...
    function tokenURI(
        ...
        return
            bytes(uri).length != 0
                ? string(abi.encodePacked(uri, _toString(tokenId)))
                : "";
    }
    ...
    function deposit(
        ...
        _tokenDeposits[tokenId] = TokenDeposit({
            ...
            lockTime: lockTime,
            withdrawn: false,
            lockedBalance: amount // Set the locked balance equal to the deposited amount
        });

        emit Deposit(tokenAddress, amount, lockTime);
    }

    function getSavingsBalance(uint256 tokenId) public view returns (uint256) {
