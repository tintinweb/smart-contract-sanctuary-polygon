...
contract LevelStorage is Ownable, ERC721Holder {
    ...
    function addFighter(uint256 _level, uint256 _zaiId) external onlyAuth returns(bool) {
       return EnumerableSet.add(levelFighters[_level], _zaiId);
    }

    function removeFighter(uint256 _level, uint256 _zaiId) external onlyAuth returns (bool){
       return EnumerableSet.remove(levelFighters[_level], _zaiId);
    }

    function getLevelLength(uint256 _level) external view returns(uint256) {
        return EnumerableSet.length(levelFighters[_level]);
    }

    function getRandomZaiFromLevel(uint256 _level, uint256 _idForbiden) external onlyAuth returns(uint256){
        if(_level > 50){ // all level 50 + got same elements points so they can fight together
            _level = 50;
        }
        uint256 index = IOracle(gameAddresses.getOracleAddress()).getRandom(keccak256(abi.encodePacked(_level,EnumerableSet.length(levelFighters[_level]), block.timestamp)));
        index = index % (EnumerableSet.length(levelFighters[_level]));
        
        if(EnumerableSet.at(levelFighters[_level],index) == _idForbiden){
            if(index == 0){
                index = 1;
            }else{
                index -= 1;
        ...
        return (EnumerableSet.at(levelFighters[_level],index));
    }
