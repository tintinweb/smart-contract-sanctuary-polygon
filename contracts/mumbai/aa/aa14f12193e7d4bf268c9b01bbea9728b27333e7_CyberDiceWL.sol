...
contract CyberDiceWL {
   ...
   */
    ...
    event BetPlaced(uint256 _id, address _user, uint256 _cap, BetRisk risk);
   ...
   */
    ...
    uint256[] public odds = [6, 4, 2]; //6, 4, 2
    ...
    function getCurrentPrice(BetRisk riskLevel)
        public
        view
    ...
    function placeBet(BetRisk riskLevel) public payable {
        require(
            msg.value >= getCurrentPrice(riskLevel),
    ...
    function roll(uint256 id) public returns (uint256) {
        ...
        require(
            block.number <= bets[id].minimumBlockNumberForRoll + 255,
            "TOO LATE"
        );
