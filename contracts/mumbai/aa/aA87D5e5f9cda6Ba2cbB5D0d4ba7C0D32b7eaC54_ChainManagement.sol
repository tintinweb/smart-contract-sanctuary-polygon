...
contract AdminControl is Initializable, OwnableUpgradeable {
    ...
    modifier onlyChainManager() {
        require(hasChainManagement(), "You do not have access to Chain management");
        _;
    }
    ...
    function grantInvestorManagement(address _address) external onlyOwner {
    ...
    function revokeInvestorManagement(address _address) external onlyOwner {
        require(adminPermissions[_address].role == Role.ADMIN, "Address is not an admin");
        adminPermissions[_address].investorManagement = false;
    }

    function grantChainManagement(address _address) external onlyOwner {
        require(adminPermissions[_address].role == Role.ADMIN, "Address is not an admin");
        adminPermissions[_address].chainManagement = true;
    }

    function revokeChainManagement(address _address) external onlyOwner {
        require(adminPermissions[_address].role == Role.ADMIN, "Address is not an admin");
        adminPermissions[_address].chainManagement = false;
    }

    function getAdmin(address _address) external view returns (Permissions memory) {
        return adminPermissions[_address];
    }

    function hasInvestorManagement() external view returns (bool) {
...
contract ChainManagement is Initializable , AdminControl{
    address adminControlAddress_;
    
    function initialize(address adminControlAddress) public initializer {
        adminControlAddress_ = adminControlAddress;
