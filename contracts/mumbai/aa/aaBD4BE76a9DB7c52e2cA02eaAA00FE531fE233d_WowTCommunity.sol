...
contract WowTCommunity is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  function addMembers(
    ...
    communityMap[_communityName].userName[_communityParticipant] = _userName;
  ...
  function getUserName(
    ...
    return communityMap[_communityName].userName[_userAddress];
  }
  ...
  //   function deleteQuizElements(
  //     string memory _communityName,
  //     uint256 count
  //   ) public {
  //     require(
  //       communityMap[_communityName].quizesforEntry.length >= count,
  //       'Array length is less'
  //     );

  //     for (uint i = 0; i < count; i++) {
  //       delete communityMap[_communityName].quizesforEntry[i];
  //     }
  //   }

  // function removeFirstElement(string memory _communityName) external {
  //     require(
  //         communityMap[_communityName].quizesforEntry.length > 0,
  //         "Array is empty."
  //     );
  ...
  //     for (
  //         uint i = 0;
  //         i < communityMap[_communityName].quizesforEntry.length / 2;
  //         i++
  //     ) {
  //         string memory temp = communityMap[_communityName].quizesforEntry[i];
  //         communityMap[_communityName].quizesforEntry[i] = communityMap[
  //             _communityName
  //         ].quizesforEntry[
  //                 communityMap[_communityName].quizesforEntry.length - 1 - i
  //             ];
  //         communityMap[_communityName].quizesforEntry[
  //             communityMap[_communityName].quizesforEntry.length - 1 - i
  //         ] = temp;
  ...
  //     // Remove the last element (previously the first element)
  //     communityMap[_communityName].quizesforEntry.pop();
  ...
  //     // emit ArrayUpdated(myArray);
  // }

  // function getQuizLength(
  //     string memory _communityName
  // ) public view returns (uint256) {
  //     return communityMap[_communityName].quizesforEntry.length;
  // }
  ...
  // function getCommunityPostImage(
  //     string memory _communityName,
  //     uint _index
  // ) public view returns (string memory) {
  ...
  // function setCommunityPostImage(
  //     string memory _communityName,
  //     uint _index,
...
contract WowTPoints is OwnableUpgradeable, AccessControlUpgradeable {
  ...
  uint256[] public pointValues;

  // address[10] public topLeaderBoardAddress; // Array to store top 10 leaderboard positions
  uint256 public topLeaderboardSize; // Dynamic leaderboard size
  address[] public topLeaderBoard;
  mapping(address => bool) private isLeaderBoardEntry;
  ...
  function initialize(
    ...
    topLeaderboardSize = _topLeaderboardSize;
  }
  ...
  function addActiveUserPoints(
    ...
    addPoints(account, pointValues[2], _catagory);
  }
  ...
  function reducePoints(
    ...
    pointsHistory[account].push(
      PointHistory(category, uint256(_points), block.timestamp)
    );
    updateLeaderBoard(account, points[account]); // Update leaderboard with new points
  ...
  function updateLeaderBoard(address account, uint256 totalPoints) private {
    ...
    uint256 lowestPoints;

    if (topLeaderBoard.length < topLeaderboardSize) {
      isNewLeader = true;
      lowestPoints = 0;
    } else {
      lowestPoints = getPoints(topLeaderBoard[topLeaderboardSize - 1]);
    ...
    if (isNewLeader) {
      if (isLeaderBoardEntry[account]) {
        removeLeaderBoardEntry(account);
      }

      uint256 insertIndex = topLeaderBoard.length;
      for (uint256 i = 0; i < topLeaderBoard.length; i++) {
        if (totalPoints > getPoints(topLeaderBoard[i])) {
          insertIndex = i;
          break;
        }
      }

      topLeaderBoard.push(address(0));
      for (uint256 i = topLeaderBoard.length - 1; i > insertIndex; i--) {
        topLeaderBoard[i] = topLeaderBoard[i - 1];
      }

      topLeaderBoard[insertIndex] = account;
      isLeaderBoardEntry[account] = true;

      if (topLeaderBoard.length > topLeaderboardSize) {
        removeLeaderBoardEntry(topLeaderBoard[topLeaderBoard.length - 1]);
      }
    }
  }

  function removeLeaderBoardEntry(address account) private {
    for (uint256 i = 0; i < topLeaderBoard.length; i++) {
      if (topLeaderBoard[i] == account) {
        for (uint256 j = i; j < topLeaderBoard.length - 1; j++) {
    ...
    isLeaderBoardEntry[account] = false;
  }
  ...
  function getPointsDetails(
    ...
    for (uint256 i = 0; i < pointsHistory[account].length; i++) {
      if (
        ...
        categoriesInRange[currentIndex] = pointsHistory[account][i].category;
        timestampsInRange[currentIndex] = pointsHistory[account][i].timestamp;
        currentIndex++;
      }
    }
  ...
  function getPointValue(uint256 index) public view returns (uint256) {
    require(index < 13, 'Invalid index'); // Ensure the index is within the valid range
  ...
  function setLevelOnePoints(uint256 newLevelOnePoints) external onlyOwner {
    pointValues[0] = newLevelOnePoints;
  }
  ...
  function setLevelTwoPoints(uint256 newLevelTwoPoints) external onlyOwner {
    pointValues[1] = newLevelTwoPoints;
  }
   ...
   */
  function setActiveUserPoints(uint256 newActiveUserPoints) external onlyOwner {
    pointValues[2] = newActiveUserPoints;
  }
  ...
  function setMinimumPointsForConvertion(
    ...
    pointValues[3] = newMinimumPointsForConvertion;
  }
}