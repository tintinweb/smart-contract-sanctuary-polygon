...
contract TestCryptoBullGuild is ERC721URIStorage, Ownable {
    ...
    bool public isTokenURIRevealed = false;
    uint256 public maxSupply = 7777;
    uint256 public giveawaySupply = 177;
    uint256 public presaleSupply = 1000;
    uint256 public presalePrice = 0.007 ether;
    uint256 public maxPerPresaleTx = 5;
    ...
    address[] private royaltyAddresses = [
        0xC89d4Fb3F63cc2e7323558EDD36157F2cF485e31,
        0x5dC72858EF40E0C56B164bc0Dd48182b070b34D8
    ];
    mapping(address => uint256) private royaltyShares;
    ...
    constructor(
        ...
        royaltyShares[royaltyAddresses[0]] = 50;
        royaltyShares[royaltyAddresses[1]] = 50;
    }
    ...
    function presaleMint(uint256 quantity) external payable {
        ...
        require(totalSupply() + quantity <= maxSupply - giveawaySupply, "Exceeded max supply of tokens!");
    ...
    function publicSaleMint(uint256 quantity) external payable {
        ...
        require(supply.current() + quantity <= maxSupply, "Max supply exceeded!");
    ...
    function data() public view returns (Data memory){
        return Data({
            ...
            isTokenURIRevealed: isTokenURIRevealed,
            maxSupply: maxSupply,
            giveawaySupply: giveawaySupply,
    ...
    function tokenURI(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        if (!isTokenURIRevealed||(isTokenURIRevealed&&bytes(base).length==0)) {
            return hiddenTokenURI;
        }
        return string(abi.encodePacked(base, Strings.toString(_tokenId), tokenURISuffix));
    }
    ...
    function giveawayMint(uint256 quantity, address to) external onlyOwner {
    ...
    function setIsPresaleOpen(bool _isPresaleOpen) external onlyOwner {
        isPresaleOpen = _isPresaleOpen;
    }

    function setIsPublicSaleOpen(bool _isPublicSaleOpen) external onlyOwner {
        isPublicSaleOpen = _isPublicSaleOpen;
    }

    function setIsTokenURIRevealed(bool _isTokenURIRevealed, string memory _tokenURIPrefix) external onlyOwner {
        isTokenURIRevealed = _isTokenURIRevealed;
        tokenURIPrefix = isTokenURIRevealed?_tokenURIPrefix:"";
    }
    ...
    function setGiveawaySupply(uint256 _giveawaySupply) external onlyOwner {
    ...
    function withdraw() external onlyOwner {
        ...
        for (uint256 i = 0; i < royaltyAddresses.length; i++) {
            payable(royaltyAddresses[i]).transfer(
                balance.div(100).mul(royaltyShares[royaltyAddresses[i]])
            );
        }
    }
}
