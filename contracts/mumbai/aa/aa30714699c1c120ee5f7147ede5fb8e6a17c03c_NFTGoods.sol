...
contract NFTGoods is INFTGoods, Initializable, Proxied, ERC721ForProxied, AccessControl {
    ...
    mapping(uint256 => Status) public override getStatus;
    mapping(uint256 => string) private _tokenURI;

    constructor() {}

    function upgradeStatus(uint256 tokenId) external override {
        if (msg.sender != ownerOf(tokenId)) {
            revert NFTGoodsNotAnOwnerOfNFT();
        }
        Status currentStatus = getStatus[tokenId];
        if (currentStatus == Status.Unpacked) {
            revert NFTGoodsMaxStatusReached();
        }
    ...
    function changeTokenURI(uint256 tokenId, string memory newURI) external override onlyRole(MINTER_ROLE) {
        _tokenURI[tokenId] = newURI;
        emit TokenURIChanged(tokenId, newURI);
    }

    function mint(address to, uint256 tokenId, string memory uri) external override onlyRole(MINTER_ROLE) {
    ...
    function _beforeTokenTransfer(
        ...
        if (
            ...
            revert NFTGoodsTransferNotAllowed();
        }
    ...
    function _afterTokenTransfer(
        ...
        if (getStatus[firstTokenId] == Status.Packed && from != address(0)) {
...
interface IControl {
    ...
    function setCountTensOfThousands(address nftGoods, uint256 count) external;
