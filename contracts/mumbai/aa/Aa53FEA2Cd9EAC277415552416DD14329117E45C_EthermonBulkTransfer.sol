...
contract EthermonBulkTransfer is BasicAccessControl {
    ...
    function bulkTransfer(
        ...
        uint64[] calldata _tokenId
    ) external onlyModerators {
        EtheremonDataBase data = EtheremonDataBase(dataContract);
        require(_tokenId.length != _to.length && _from != address(0));

        for (uint256 i = 0; i < _to.length; i++) {
            if (_to[i] != address(0)) {
                data.removeMonsterIdMapping(_from, _tokenId[i]);
                data.addMonsterIdMapping(_to[i], _tokenId[i]);
                emit Transfer(_from, _to[i], _tokenId[i]);
            }
        }
    }
}