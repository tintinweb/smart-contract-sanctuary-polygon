    ...
    IHDNSRegistry
{
    ...
    modifier onlyApprovedOrOwnerDeadman(uint256 tokenId, string calldata key) {
        if(
            ...
            require(_msgSender() == _mintingManager, 'HDNSRegistry: CALLER_IS_NOT_ALLOWED_TO_SET_EXPIRATION');
        }
        _;
    }
    ...
    function repoDeadDomain(uint256 tokenId) external override onlyMintingManager {
        uint256 keyHash = uint256(keccak256(abi.encodePacked('expiration')));
        string memory domainExpString = _records[_presetOf(tokenId)][keyHash];
        uint256 domainExp = domainExpString.st2Num();
        require(block.timestamp >= domainExp + 90 days, 'HDNS Registry: DOMAIN_IS_NOT_DEAD');
        _burn(tokenId);
    }
...
abstract contract RecordStorage is 
    ...
    function _get(string memory key, uint256 tokenId) internal view returns (string memory) {
    ...
    function _get(uint256 keyHash, uint256 tokenId) private view returns (string memory) {
        if (
            _isReadRestricted(tokenId) || 
            (bytes(_records[_presetOf(tokenId)][keyHash]).length > 0 && _isDead(tokenId))) {
            return '';
        }
    ...
    function _set(
        ...
        require(!_isDead(tokenId), 'HDNSRegistry: DOMAIN_IS_EXPIRED');
    ...
    function _isDead(uint256 tokenId) internal view returns (bool) {
        uint256 keyHash = uint256(keccak256(abi.encodePacked('expiration')));
        string memory domainExpString = _records[_presetOf(tokenId)][keyHash];
        if(bytes(domainExpString).length > 0) {
            uint256 domainExp = domainExpString.st2Num();
            return block.timestamp >= domainExp;
        }
        return false;
    }
...
// @author Unstoppable Domains, Daosourced  
// @date june 17 2023
