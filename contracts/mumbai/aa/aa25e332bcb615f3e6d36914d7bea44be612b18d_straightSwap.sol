...
contract straightSwap {
    ...
    uint256 public straightShooter;
    ...
    function init721Trade(address _giveNFT, uint256 _giveId, address _wantNFT, uint256 _wantID, uint256 _endtime) public payable trustworthy truHolder(_giveNFT)  canTransferNFT(_giveNFT, _giveId) returns(bool){
            ...
            if(trustScore[msg.sender] <= straightShooter || waved[_giveNFT] != true){
    ...
    function getTrustScore() public view returns(uint256){
        return trustScore[msg.sender];
    }

    function isTrustWorthy() public view returns(bool){
        return dontTrust[msg.sender];
    }
    ...
    function setThreshold(uint256 _number) public onlyOwner returns(uint256){
        straightShooter = _number;
