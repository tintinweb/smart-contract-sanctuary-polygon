...
contract ChainBattles is ERC721URIStorage {
    ...
    // Initializing ERC721 contract which carries params like name and any symbol you wish to give
    ...
    function train(uint256 tokenId) public {
        ...
        require(msg.sender == ownerOf(tokenId), "Only owner can train their token");
        uint256 currentLevel = tokenIdToLevels[tokenId];
        tokenIdToLevels[tokenId] = currentLevel + 1;
        _setTokenURI(tokenId, getTokenURI(tokenId));
    }

    function detrain(uint256 tokenId) public {
        require(_exists(tokenId), "Please use an existing token id");
        require(msg.sender == ownerOf(tokenId), "Only owner can train their token");
        uint256 currentLevel = tokenIdToLevels[tokenId];
        tokenIdToLevels[tokenId] = currentLevel - 1;
