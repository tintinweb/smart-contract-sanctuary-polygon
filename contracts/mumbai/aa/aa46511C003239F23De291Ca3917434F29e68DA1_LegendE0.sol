 ...
 * @notice This is the contract of the presale of the Legend NFT
 */
contract LegendE0 is ERC721, ERC721Enumerable, Pausable, AccessControl, ERC721Burnable, ReentrancyGuard {
    // Token counter
    ...
    // URI of the tokens
    string public baseURI = "http://vegabuild.es/mcg_tokens/3.json";
    ...
    // Events
    event LegendMinted(uint256 indexed tokenId, address to);
     ...
     */
    constructor(address selectedStablecoin, uint256 newPrice) ERC721("LegendEd0", "LGE0") {
    ...
    function setNewStableCoin(address newStablecoin) external onlyRole(ADMIN_ROLE) {
        require(newStablecoin != address(0), "LGE0: StableCoin address shouldn't be zero address");
    ...
    function setNewPrice(uint256 newPrice) external onlyRole(ADMIN_ROLE) {
        require(newPrice > 0, "LGE0: Price must be greater than 0 wei");
        price = newPrice;
    } 
    ...
    function setChildContractAddress(address contractAddress) external onlyRole(ADMIN_ROLE) {
        require(contractAddress != address(0), "LGE0: Zero address shouldn't be child contract");
    ...
    function setCounterBreeding(uint256 id, address owner) external {
        require(msg.sender == _child, "LGE0: Caller isnt the child contract");
        require(owner == ownerOf(id), "LGE0: The sender isnt the owner of the token");
        _breedingCounter[id]++;
    }
    ...
    /**
     * @notice Set a new URI for Legend ED0 NFT
    ...
    /**
     * @notice Inherit function which returns the URI of Legend ED0 NFT
     * @dev ALL the tokens of Legend ED0 have ONE SINGLE URI
     * @param tokenId The token ID
     * @return baseURI the baseURI of Legend ED0
     */
    ...
    function setNewPrivateSell(address beneficiary, uint8 quantity) external onlyRole(ADMIN_ROLE) {
        require(_tokenIdCounter.current() + (_soldPrivately + quantity) <= MAX_SUPPLY, "LGE0: Max supply reached");
        require(beneficiary != address(0), "LGE0: Address shouldn't be zero");
    ...
    function doPrivateSell() external whenNotPaused nonReentrant {
        ...
        require(_privateSell[msg.sender] > 0, "LGE0: Beneficiary isn't into private sell");
        require(_endOfPS, "LGE0: Presale is not over yet");

        for(uint8 i = 0; i < _privateSell[msg.sender]; i++){
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            emit LegendMinted(tokenId, msg.sender);
    ...
    /**
     * @notice This function mints one Legend NFT from ed0
     * @param to The address of minter
     */
    function mintLegend(address to) external nonReentrant whenNotPaused {
        require(to != address(0), "LGE0: Address shouldn't be zero");
