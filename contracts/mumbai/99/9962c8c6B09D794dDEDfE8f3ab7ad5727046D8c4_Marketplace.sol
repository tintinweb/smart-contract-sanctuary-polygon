    ...
    IERC1155ReceiverUpgradeable
{
    ...
    /// @dev Moment collection address already burnt
    address public momentCollection;

    /// @dev burn ticket count per card
    uint256 burnTicketPerCard = 100;
    ...
    /// @dev Mapping from wallet => how many tickets burnt
    mapping(address => uint256) public burntTickets;
    ...
    function createListing(ListingParameters memory _params) external override onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function buy(
        ...
        uint256 _burnTickets = IMoment(momentCollection).burnTickets(payer);
        ...
        require(_burnTickets - burntTickets[payer] >= burnTicketPerCard, "Not enough tickets");

        burntTickets[payer] += burnTicketPerCard;
    ...
    /// @dev set collection address
    function setBurnTicketPerCard(uint256 _burnTicketPerCard) external onlyRole(DEFAULT_ADMIN_ROLE) {
        burnTicketPerCard = _burnTicketPerCard;
    }

    /// @dev set collection address
    function setCollection(address _momentCollection) external onlyRole(DEFAULT_ADMIN_ROLE) {
        momentCollection = _momentCollection;
    }
...
interface IMoment {
    function burnTickets(address addr) external view returns (uint256);
}
