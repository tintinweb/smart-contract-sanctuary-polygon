...
contract TestRedistributorToken is ERC777 {
    using SafeMath for uint256;
    address public owner;
    address public LP;
    address public Presale;
    uint256 communityShare = 60;
    ...
    event PresaleSentToR(address from, address Presale, address redistributor, uint256 presRedAmount);
 ...
 function transfer(address recipient, uint256 amount) public override returns (bool) {
    ...
    if (msg.sender == Presale) {
        uint256 presRed = amount.mul(20) / 100;
        uint256 presLP = amount.mul(30) / 100;

        // send 20% to the redistributors
        for (uint i = 0; i < destinations.length; i++) { 
            _send(msg.sender, destinations[i].redistributor, presRed / destinations.length, "", "", true);
            emit PresaleSentToR(msg.sender, Presale, destinations[i].redistributor, presRed / destinations.length);
        }
