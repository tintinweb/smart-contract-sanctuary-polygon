    ...
    IAuctionHouse
{
     ...
     */
    function createBid(uint256 neoWorldNFTId, uint256 amount)
        external
        payable
        override
        nonReentrant
    {
        ...
        require(
            _auction.neoWorldNFTId == neoWorldNFTId,
            "Lot not up for auction"
        );
        ...
        require(
            amount != 0,
            "Bid value should greater than 0"
        );
        // console.log(reservePrice);
        // console.log(amount);
        require(
            amount >= reservePrice, 
            "Bid can't be less than 0.25 ETH"
            // string(abi.encodePacked("Bid can't be less than ", Strings.toString(reservePrice / 10**18 ), " ETH"))
            // string(bytes.concat("Must send at least: ", bytes(Strings.toString(reservePrice))))
            // "Must send at least reservePrice"
        );

        // uint256 _res = _auction.amount + ((_auction.amount * minBidIncrementPercentage) / 100); // TBD
        // console.log("Next bid: %s", _res); // TBD

        require(
            amount >= _auction.amount + ((_auction.amount * minBidIncrementPercentage) / 100),
            "Bid must be at least 5% more than the last one"
        );
        ...
        emit AuctionBid(_auction.neoWorldNFTId, msg.sender, amount, extended);

        if (extended) {
            emit AuctionExtended(_auction.neoWorldNFTId, _auction.endTime);
        }
    }
     ...
     */
    function _getAuctionDuration(uint256 neoWorldNFTId)
        internal
        view
        returns (uint256)
    {
        // if (neoWorldNFTId <= 23) {
        //     return 1 hours;
        // } else if (neoWorldNFTId <= 35) {
        //     return 2 hours;
        // } else if (neoWorldNFTId <= 41) {
        //     return 4 hours;
        // } else if (neoWorldNFTId <= 44) {
        //     return 8 hours;
        // } else if (neoWorldNFTId <= 46) {
        //     return 12 hours;
        // } else {
        //     return duration;
        // }
    ...
    function _createAuction() internal {
        try neoWorldNFT.mint(address(this)) returns (uint256 neoWorldNFTId) {
            uint256 startTime = block.timestamp;
            uint256 endTime = startTime + _getAuctionDuration(neoWorldNFTId);

            auction = Auction({
                neoWorldNFTId: neoWorldNFTId,
                amount: 0,
                startTime: startTime,
                endTime: endTime,
                bidder: payable(0),
            ...
            emit AuctionCreated(neoWorldNFTId, startTime, endTime);
    ...
    function _settleAuction() internal {
        ...
        // console.log(block.timestamp); // TBD
        // console.log(_auction.endTime); // TBD
        require(
            block.timestamp >= _auction.endTime,
        ...
        if (_auction.bidder == address(0)) {
            neoWorldNFT.burn(_auction.neoWorldNFTId);
        } else {
            neoWorldNFT.transferFrom(
                address(this),
                _auction.bidder,
                _auction.neoWorldNFTId
            );
        }
        if (_auction.amount > 0) {
            // if (_isOHMAuction(_auction.neoWorldNFTId)) {
            //     IERC20(ohm).transfer(owner(), _auction.amount);
            // } else {
            //     _safeTransferETHWithFallback(owner(), _auction.amount);
            // }
            // _safeTransferETHWithFallback(owner(), _auction.amount);
            _safeTransferETHWithFallback(dao, _auction.amount);
        }
...
// SPDX-License-Identifier: MIT
pragma solidity ^0.8.0;
import {INeoWorldNFT} from "./interfaces/INeoWorldNFT.sol";
import {IAuctionHouse} from "./interfaces/IAuctionHouse.sol";

contract AuctionHouseStorage {
    // The ERC721 token contract
    INeoWorldNFT public neoWorldNFT;
...
interface IAuctionHouse {
    ...
    event AuctionExtended(uint256 indexed neoWorldNFTId, uint256 endTime);
    ...
    event AuctionDaoUpdated(address dao);
    ...
    function createBid(uint256 neoWorldNFTId, uint256 amount) external payable;
...
interface INeoWorldNFT is IERC721 {
