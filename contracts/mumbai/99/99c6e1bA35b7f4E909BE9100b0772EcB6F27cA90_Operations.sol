...
contract Operations is VRFConsumerBaseV2, ERC2771Recipient {
    ...
    function handleLoss(bytes32 betkey, uint8 betOrder, uint256 stake, uint256 payout, uint256 hl, bytes32 odds, bool[3] calldata controls, uint256[2] calldata operator) public onlySettleSigner {
        ...
        if (controls[1]) i_core.handleHL(betSlip.provider, betSlip.token, hl, operator[0]);
        if (controls[2]) i_core.handlePayout(betSlip.provider, betSlip.token, payout, operator[1]);
    ...
    function handleSuspension(bytes32 betkey, uint8 betOrder, uint256 stake, uint256 payout, bytes32 odds, bool[2] calldata controls) public onlySettleSigner {
        BetSlip storage betSlip = s_keyToBetSlip[betkey];
        if (betkey != betSlip.betkey || betkey == 0) revert("Operations__InvalidBetId");
        if (isSettled[betkey][betOrder] == true) revert("Already settled");
        if (stake > i_core.getUserStaked(betSlip.bettor, betSlip.token)) revert("Operations__InsufficentStakes");
