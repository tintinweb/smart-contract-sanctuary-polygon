...
contract LotteryContract is Ownable {
    ...
    function startLottery(uint256 _endTime, uint256 _startTime, uint256 _price, address _token, uint256[7] memory _reward) public onlyOwner {
    ...
    function adminGiveFreeTickets(uint256 _lotteryId, uint256[] memory _ticketNumber, address _receipent) public onlyOwner{
        ...
        for (uint256 i = 0; i < _ticketNumber.length; i++) {
            ...
            numberTicketsPerLotteryId[_lotteryId][1 + (thisTicketNumber / 100000)]++;
            numberTicketsPerLotteryId[_lotteryId][11 + (thisTicketNumber / 10000)]++;
            numberTicketsPerLotteryId[_lotteryId][111 + (thisTicketNumber / 1000)]++;
            numberTicketsPerLotteryId[_lotteryId][1111 + (thisTicketNumber / 100)]++;
            numberTicketsPerLotteryId[_lotteryId][11111 + (thisTicketNumber / 10)]++;
            numberTicketsPerLotteryId[_lotteryId][111111 + (thisTicketNumber / 1)]++;
    ...
    function getFinalNo(uint256 randomNo) internal pure returns(uint256){
        uint256 finalNo =0;
        for(uint256 i=0; i<6; i++){
            uint256 lastDigit = randomNo % 10;
            finalNo = finalNo * 10 + lastDigit;
            randomNo /= 10;
        }
        return finalNo;
    }
    ...
    function drawnLottery(uint256 _lotteryId) public onlyOwner {
        ...
        lotteries[_lotteryId].finalNumber = getFinalNo(uint(keccak256(abi.encodePacked(_lotteryId , blockhash(lotteries[_lotteryId].winningTicketBlock))))) ;      
    ...
    function viewTicketIdRewards(uint256 _ticketId, uint256 _lotteryId) public view returns(uint256, uint256){
        ...
        for (uint256 i = 6; i >= 1; i--) {
            if(((lotteryTicket[_ticketId].number)/(10**(i-1))) == (winningNumber/(10**(i-1)))) {
                ++j;
            }
        }
        if(j == 0){
            ...
            (uint256[] memory winner,) = viewCountWinnersInLottery(_lotteryId); 
            uint256 totalUser = winner[j-1];
