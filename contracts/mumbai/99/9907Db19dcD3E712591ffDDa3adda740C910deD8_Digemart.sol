...
contract Digemart is Converter{
    ...
    //CONSTRUCTOR
    constructor(address _router, address _stableCoin, address _priceFeedAddress) {
        owner = msg.sender;
        router = _router;
        ...
        notEmptyString(_name)
    {
        ...
        uint convertedAmount = getConversionRate(msg.value, priceFeed);
        require(
            convertedAmount >= signUpFee,
     ...
     */
    function buyProduct(uint256[] memory _productIds)
        public
        payable
        onlyUser
    {
        ...
        _path = new address[](2);
        _path[0] = IUniswapRouter(router).WETH();
        _path[1] = stableCoin;
        uint _requiredAmount = IUniswapRouter(router).getAmountsIn(_totalPrice, _path)[0];
        require(msg.value >= _requiredAmount, "Insufficient amount");
        IUniswapRouter(router).swapExactETHForTokens{value: msg.value}(_totalPrice, _path, address(this), block.timestamp);

        emit newOrder(purchaseId, msg.sender, _totalPrice, _productIds);
    }

    function convertPrice(uint amountINUSD) public view returns(uint) {
        uint maticToUsd = getConversionRate(1 ether, priceFeed);
        uint convertedPrice = (1 ether / maticToUsd) * amountINUSD;
        ...
        orderNotCompleted(_purchaseId)
    {
        ...
        // Pay merchant
        TransferHelper.safeTransfer(stableCoin, _merchant, _price);
