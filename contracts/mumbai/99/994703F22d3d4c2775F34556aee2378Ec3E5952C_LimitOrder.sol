...
contract LimitOrder is OrderBase {
    ...
    function createLimitTask(TccItemV2[] calldata _tccList, LimitItem[] calldata _limitItemList) external nonReentrant payable {
        ...
        for (uint256 i = 0; i < _num; i++) {
            ...
            LimitItem memory _limitItem = _limitItemList[i];
            ...
            if (_y.swapInToken != weth) {
                ...
                IERC20(_y.swapInToken).transferFrom(_user, address(this), _limitItem._swapInAmount);
                uint256 _amountA = IERC20(_y.swapInToken).balanceOf(address(this));
                _maxSpendTokenAmount = _amountA.sub(_amountB);
            } else {
                IWETH(weth).deposit{value: _limitItem._swapInAmount}();
                _maxSpendTokenAmount = _limitItem._swapInAmount;
                _ethAmount = _ethAmount.add(_limitItem._swapInAmount);
            }

            _y._taskConfig._limitItem._swapInAmount = _maxSpendTokenAmount;
            _y._taskConfig._limitItem._swapInAmountOld = _limitItem._swapInAmount;
            ...
            emit OrderIdMd5TaskIdEvent(taskAmount, _y.md5, _y.taskId);
    ...
    function limitOrder(address _user, bytes32 _md5) external nonReentrant {
        ...
        y._limitItem._swapOutAmount = x.swapOutAmount;
        require(y._limitItem._swapOutAmount >= y._limitItem._minswapOutAmount, "L17");
        //验证成交价格是否合理
