...
contract LockSetting is Ownable {
    ...
    event UpdateDiscountPercent(uint256 oldValue, uint256 newValue);
    ...
    constructor() {
        SETTING.BASE_FEE = 0.01 ether;
        SETTING.TOKEN_FEE = 2;
        SETTING.ADDRESS_FEE = payable(msg.sender);
        WHITELIST_FEE_ADDRESS.add(msg.sender);
    }
    ...
    function setDiscountPercent(uint256 _discountPercent) external onlyOwner {
        require(_discountPercent <= 1000, "INVALID DISCOUNT PERCENT");
        uint256 oldValue = discountPercent;
        discountPercent = _discountPercent;
        emit UpdateDiscountPercent(oldValue, discountPercent);
    }
