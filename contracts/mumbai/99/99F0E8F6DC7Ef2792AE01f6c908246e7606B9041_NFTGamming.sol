...
contract NFTGamming is ERC721, Ownable {
  ...
  ///  GameToken struct to store player token info
  struct GameToken {
    string name; 
    uint256 id; 
  ...
  struct Battle {
    ...
    bytes32 battleHash; 
    address player1NftAddress;
    uint256 player1NftId;
    address player2NftAddress;
    uint256 player2NftId;
    string name; 
    address[2] players; 
    uint8[2] moves; 
    address winner; 
  ...
  constructor() ERC721("NFT Gaming","NG") {
  ...
  modifier onlyRegisteredPlayer() {
    require(isPlayer(msg.sender), "Please register as a player first");
    _;
  }
  ...
  function initialize() private {
    ...
    battles.push(Battle(BattleStatus.PENDING, bytes32(0),address(0), 0, address(0), 0, "", [address(0), address(0)], [0, 0], address(0)));
  }
  ...
  /// internal function to generate random number; used for Battle Card Attack and Defense Strength
  function _createRandomNum(uint256 _max, address _sender) internal view returns (uint256 randomValue) {
    uint256 randomNum = uint256(keccak256(abi.encodePacked(block.timestamp, _sender)));
  ...
  ///  internal function to create a new Battle Card
  ...
  function createBattle(string memory _name, address _player1nftaddress, uint256 _player1nftid) external returns (Battle memory) {
    require(isPlayer(msg.sender), "Please Register Player First"); 
    ...
    IERC721(_player1nftaddress).transferFrom(
      msg.sender,
      address(this),
      _player1nftid
    );
  ...
  // Player joins battle
  function joinBattle(string memory _name, address _player2nftaddress, uint256 _player2nftid) external returns (Battle memory) {
    ...
    _battle.player2NftAddress = _player2nftaddress;
    _battle.player2NftId = _player2nftid;
    ...
    IERC721(_player2nftaddress).transferFrom(
      msg.sender,
      address(this),
      _player2nftid
    );
  ...
  ///  internal function to end the battle
  function _endBattle(address battleEnder, Battle memory _battle) internal returns (Battle memory) {
    ...
    address battleLoser = battleEnder == _battle.players[0] ? _battle.players[1] : _battle.players[0];

    emit BattleEnded(_battle.name, battleEnder, battleLoser); // Emits BattleEnded event

    // Transfer both NFTs to the battleEnder's address
    IERC721(_battle.player1NftAddress).transferFrom(
        address(this),
        battleEnder,
        _battle.player1NftId
    );
    IERC721(_battle.player2NftAddress).transferFrom(
        address(this),
        battleEnder,
        _battle.player2NftId
    );
