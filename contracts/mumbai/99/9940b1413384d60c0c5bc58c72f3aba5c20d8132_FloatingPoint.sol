...
contract AKX is Initializable, UUPSUpgradeable, AccessControlEnumerableUpgradeable,  States, ReentrancyGuardUpgradeable {
  ...
  address public sbts;
    ...
    function initialize(address _priceOracle, address _labzToken,address vipNftManagerContract) public initializer {
        __AKX_init(_priceOracle, _labzToken,  vipNftManagerContract);
    }

    function __AKX_init(address _priceOracle, address _labzToken,  address vipNftManagerContract) public onlyInitializing {
        VERSION = 1;
        NAME = "AKX CORE";
        __Context_init();
    ...
    Ownable
{
    ...
    function TokenMaticRate() public view returns (uint256 r) {
        r = PriceMath.calculateRate(tokenPerMatic, 1);
    }

    function TokenUSDRate() public view returns (uint256 r) {
        r = PriceMath.calculateTokenUSDPrice(
            TokenMaticRate(),
            AKXPriceOracle(_priceOracle).maticUSD()
        );
    }
    ...
    function _calcProtocolFee(uint256 value) internal view returns (uint256 r) {
        ...
        r = PriceMath.calculateRate(_val, percent) * 1e18;
    }
...
library PriceMath {
    ...
    function calculateRate(uint256 _price1, uint256 _price2) public pure returns(uint256 result) {
        if(_price1 > _price2) {
            FloatingPoint.Float memory _floating = FloatingPoint.divide(18, _price1, _price2);
            result = _floating.remainder;
        } else if(_price2 > _price1){
            FloatingPoint.Float memory  _floating = FloatingPoint.divide(18, _price2, _price1);
            result = _floating.remainder;
        }
        result = 1;
    }
...
contract VIPWallet is Lock {
    ...
    function _calculateRewards(uint256 value) internal  returns(uint256) {

       
            uint256 amt = (((((block.timestamp - lastUpdateTime) * value)) * rewardsPerHour) / 3600);
            unclaimed += amt;
    ...
    function stake(uint256 value) public  {
        ...
        uint256 r = _calculateRewards(value); 
    ...
    function calcLocked() public  view returns(uint256) {
...
contract VIPWalletFactory is Ownable {
    ...
    function getAddress(uint256 tokenId) public view returns(address) {
          bytes32 salt = keccak256(abi.encode(msg.sender, tokenId));
         address addr = Clones.predictDeterministicAddress(vipWalletImpl,salt);
         return addr;
    }
