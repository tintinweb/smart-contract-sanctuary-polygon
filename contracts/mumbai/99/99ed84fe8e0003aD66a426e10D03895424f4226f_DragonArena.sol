...
contract DragonArena is LockableReceiver, ReentrancyGuard { 

    string public constant BAD_ADDRESS_ERROR = "DragonArena: bad address";
    string public constant BAD_AMOUNT_ERROR = "DragonArena: bad amount";
    string public constant LOCK_DRAGONS_ERROR = "DragonArena: both the dragons must be locked";
    string public constant HOLDER_ERROR = "DragonArena: both the dragons belong to the same account";
    string public constant WITHDRAW_DEPOSIT_ERROR = "DragonArena: cannot withdraw this amount";
    string public constant NOT_ENOUGH_DEPOSIT_ERROR = "DragonArena: not enough deposit amount";
    string public constant BET_AMOUNT_ERROR = "DragonArena: the bet amount is too small";

    enum FightTypes { Regular, Money }
    ...
    uint private _fightFees;
    uint _minBetAmount; //applied, if the fight type is "Money"

    event PriceChanged(address indexed operator, uint previousValue, uint newValue);
    event FeesChanged(address indexed operator, uint previousValue, uint newValue);
    event MinBetAmountChanged(address indexed operator, uint previousValue, uint newValue);
    event Deposit(address indexed caller, address indexed to, uint amount);
    event DepositWithdrawn(address indexed caller, address indexed to, uint amount);
    event Fight(address indexed operator, 
        uint winnerId, 
        uint loserId, 
        FightTypes fightType, 
        uint platformPrice,
        uint betAmount);
    event TrophyTransfer(address indexed winnerAccount, uint tokenId, uint betAmount);
    ...
    function setPrice(uint newPrice) external onlyRole(CFO_ROLE) {
        ...
        emit PriceChanged(_msgSender(), previousPrice, newPrice);
    }
    ...
    function setFees(uint newFees) external onlyRole(CFO_ROLE) {
        uint previousPrice = _fightFees;
        _fightFees = newFees;
        emit FeesChanged(_msgSender(), previousPrice, newFees);
    }

    function minBetAmount() public view returns(uint) {
        return _minBetAmount;
    }

    function setMinBetAmount(uint newAmount) external onlyRole(CFO_ROLE) {
        uint previousAmount = _minBetAmount;
        _minBetAmount = newAmount;
        emit MinBetAmountChanged(_msgSender(), previousAmount, newAmount);
    }

    function depositOf(address account) public view returns (uint) {
        return deposits[account];
    }

    function startFight(uint dragon1Id, uint dragon2Id, FightTypes fightType, uint betAmount, uint salt)
    external onlyRole(COO_ROLE) {
        require(isLocked(dragon1Id) && isLocked(dragon2Id), LOCK_DRAGONS_ERROR);
        address dragon1Holder = holderOf(dragon1Id);
        address dragon2Holder = holderOf(dragon2Id);
        uint totalAmount = price();
        if (fightType == FightTypes.Money) {
            require(betAmount >= minBetAmount(), BET_AMOUNT_ERROR);
            totalAmount += betAmount;
        }
        require(dragon1Holder != dragon2Holder, HOLDER_ERROR);
        require(
            depositOf(dragon1Holder) >= totalAmount && depositOf(dragon2Holder) >= totalAmount,
            NOT_ENOUGH_DEPOSIT_ERROR
        );
        
        deposits[dragon1Holder] -= totalAmount;
        deposits[dragon2Holder] -= totalAmount;

        _doFight(dragon1Id, dragon2Id, fightType, betAmount, salt);
    }

    function _doFight(uint dragon1Id, uint dragon2Id, FightTypes fightType, uint betAmount, uint salt) internal {
        DragonToken dragonToken = DragonToken(tokenContract());

        //init the strengths before fighting
        dragonToken.setStrength(dragon1Id);
        dragonToken.setStrength(dragon2Id);

        DragonInfo.Details memory dd1 = dragonToken.dragonInfo(dragon1Id);
        DragonInfo.Details memory dd2 = dragonToken.dragonInfo(dragon2Id);
        ...
        emit Fight(_msgSender(), winnerId, loserId, fightType, price(), betAmount);
        _transferTrophy(winnerId, loserId, fightType, betAmount);
    }

    function deposit(address to) payable external  {
        require(!Address.isContract(to), BAD_ADDRESS_ERROR);
        require(msg.value > 0, BAD_AMOUNT_ERROR);
        
        deposits[to] += msg.value;
        emit Deposit(_msgSender(), to, msg.value);
    }

    function withdrawDeposit(address payable to, uint amount) external nonReentrant {
        require(!to.isContract(), BAD_ADDRESS_ERROR);
        require(depositOf(_msgSender()) >= amount, WITHDRAW_DEPOSIT_ERROR);
        
        deposits[_msgSender()] -= amount;
        to.sendValue(amount);
        
        emit DepositWithdrawn(_msgSender(), to, amount);
    }

    function _transferTrophy(uint winnerId, uint loserId, FightTypes fightType, uint betAmount) internal {
        address winnerAccount = holderOf(winnerId);
        if (fightType == FightTypes.Regular) {
            _lock(loserId, winnerAccount);
            _transferTokenToHolder(loserId);
            emit TrophyTransfer(winnerAccount, loserId, 0);
        }
        else {
            uint prize = betAmount - _calcFees(betAmount, fees());
            deposits[winnerAccount] += (betAmount + prize);
            emit TrophyTransfer(winnerAccount, 0, prize);
        }
    }

    function _calcFees(uint weiAmount, uint fees100) internal virtual pure returns (uint) {
        return weiAmount.div(1e4).mul(fees100);
    }

    function _randomFightResult(
        uint dragon1Id, uint dragon1Strength, uint dragon2Id, uint dragon2Strength, uint salt)
    internal view returns (uint, uint) {

        uint totalStrength = dragon1Strength + dragon2Strength;
        uint result = Random.rand(salt).mod(totalStrength);
