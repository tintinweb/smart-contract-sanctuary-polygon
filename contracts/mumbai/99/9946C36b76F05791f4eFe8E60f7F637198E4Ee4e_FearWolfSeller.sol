...
contract SchnorrSECP256K1Verifier {
  ...
  /** **************************************************************************
      ...
      **************************************************************************
      ...
      @dev 5. Compute e=uint256(keccak256(PKx as a 32-byte big-endian
                                        ΓÇû PKyp as a single byte
                                        ΓÇû msgHash
                                        ΓÇû nonceTimesGeneratorAddress))
              This value e is called "msgChallenge" in verifySignature's source
              code below. Here "ΓÇû" means concatenation of the listed byte
              arrays.
  ...
  function verifySignature(
    ...
    // Avoid signature malleability from multiple representations for Γäñ/QΓäñ elts
    ...
    // ecrecover(-s*r,v,r,e*r) is (rΓü╗┬╣ mod Q)*(e*r*R-(-s)*r*g)=e*R+s*g, where R
...
contract FearWolfSeller is MuonClient, AccessControl, Pausable, Withdrawable {
    ...
    constructor(
        address fearContractAddress, 
        address fearWolfContractAddress,
        uint256 _muonAppId,
        PublicKey memory _muonPublicKey
    ...
    function _verifyTSS(uint256 fearPrice, uint256 ethPrice, uint256 timestamp, bytes memory reqId, SchnorrSign memory signature) private {
        ...
        bool verified = muonVerify(reqId, uint256(hash), signature, muonPublicKey);
        require(verified, "TSS not verified");
        require(block.timestamp - timestamp < acceptableMuonDelay, "Expired signature");
    }

    function getFearAmount(uint256 wolvesNumber, uint256 fearPrice, uint256 ethPrice) public view returns(uint256) {
        uint256 interPrice = fearWolfPrice.mul(ethPrice);
    ...
    function buyWolves(
        ...
        uint256 fearPrice,
        uint256 ethPrice,
        bytes calldata packedMuonData
    ) public whenNotPaused {
        address caller = _msgSender();

        (uint256[5] memory decodedData) = abi.decode(packedMuonData, (uint256[5]));
        
        SchnorrSign memory sign = SchnorrSign(decodedData[0], address(uint160(decodedData[1])), address(uint160(decodedData[2])));
        _verifyTSS(fearPrice, ethPrice, decodedData[4], _toBytes(decodedData[3]), sign);
