...
contract TWICOIN is ITWICOIN, VERIFY, TIP{
  ...
  uint256 public verifyCoolDays;
  uint256 public verifyBalanceLimit;
  bool public verifyAccessible;
  //userName
  ...
  // ---------------------------------------------  //
    ...
    constructor (){
        verifyCoolDays = 0;
        verifyBalanceLimit = 0;
        verifyAccessible = true;
    ...
    modifier _verifyAssessible(){
        ...
        require(balance >= verifyBalanceLimit,"204");

        uint256 bindDate = addressBindTime[_msgSender()];
        if(bindDate != 0){
          uint256 coolDownTime = bindDate.add(verifyCoolDays.mul(SEC_PER_DAY));
...
abstract contract VERIFY is Context, ChainlinkClient, ConfirmedOwner {
    ...
    function requestVerify(
        ...
        return sendChainlinkRequestTo(oracleAddress, req, defaultFee);
     ...
     */
    function fulfill(bytes32 _requestId, bool verifyRes_) public recordChainlinkFulfillment(_requestId)
    {
