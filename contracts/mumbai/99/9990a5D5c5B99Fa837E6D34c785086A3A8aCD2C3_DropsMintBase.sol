...
contract DropsMintBase is ERC721, ReentrancyGuard, Ownable {
...
/*----------------------------------------------------------User methods----------------------------------------------------------- */
  ...
  function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, '0')) : "";
    }
  ...
  function price() public view returns (uint256){
    ...
    return (priceInMatic / (uint256(multiplier)/ 10**8)) * 10**17;
  }
