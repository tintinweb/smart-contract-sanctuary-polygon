...
//   mapping(address => uint256[]) public userPosts; // user address => post IDs
  ...
  event postUpVoted(string communityName, string catagory, uint256 postId, uint256 upVotes);
  event postDownVoted(string communityName, string catagory, uint256 postId, uint256 downVotes);
  event postCommented(string communityName, string catagory, uint256 postId, uint256 commentVotes);
  ...
  function createPost(
    ...
    Post storage newPost = communityDetail.posts[communityDetail.postCount];
    // posts[_communityName][postCount] = Post(postCount, _content, 0, 0, 0, creator);
    newPost.id = communityDetail.postCount;
  ...
  function UpVotePost(string calldata _communityName, uint256 _postId) public {
    ...
    CommunityDetails storage communityDetail = communityDetails[_communityName];
    require(_postId <= communityDetail.postCount, 'Invalid post ID');
  ...
  function downVotePost(string calldata _communityName, uint256 _postId) public {
    //require(_postId > 0 && _postId <= postCount, "Invalid post ID");
    require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
  ...
  function commentPost(string calldata _communityName, uint256 _postId) public {
    // require(_postId > 0 && _postId <= postCount, "Invalid post ID");
    require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
  ...
  function tipPost(string calldata _communityName, uint256 _postId, uint256 _points) public {
    require(
            community.checkCommunityExists(_communityName),
            "Community doesn't exist"
        );
  ...
  function getPost(string calldata _communityName, uint256 _postId) public view returns (string memory, string memory, uint256, uint256, uint256, uint256, address) {
        require(community.checkCommunityExists(_communityName), "Community doesn't exist");
        CommunityDetails storage communityDetail = communityDetails[_communityName];
        require(_postId < communityDetail.postCount, "Invalid post ID");
        Post storage post = communityDetail.posts[_postId];
        return (post.content, post.imageUrl, post.upVotes, post.downVotes, post.comments, post.timestamp, post.creator);
    }

     function getPostCount(string calldata _communityName) public view returns (uint256) {
        require(community.checkCommunityExists(_communityName), "Community doesn't exist");
        return communityDetails[_communityName].postCount;
    }
