...
*/
    ...
    function createOrder(
        ...
        //require(totalOrders < maxOrders,"Order book is full");    
    ...
    function deleteOrder() internal{
        //require(p2p[msg.sender].sB == msg.sender, "Index error");
        //uint256 _orderNumber = p2p[msg.sender].orderNumber;
        //uint256 _orderIndex = userOrder[_orderNumber];
        ...
        //uint256 _oR = _orderIndex+1;
        //delete userOrder[_orderNumber];
        delete p2p[msg.sender];
        //fixOrders(_orderIndex);//, _oR);
        canCreateOrder[msg.sender] = true;
    ...
    function fixOrders(uint256 _orderIndex) internal { //, uint256 _oR) internal{
        //uint256 _oR = _orderIndex + 2;
        //uint j = _oR;
        /*for(uint i = _orderIndex; i < OrderDetails.length-1 ; i++){
            OrderDetails[_orderIndex] = OrderDetails[i+1];
            //counts[_orderIndex] = counts[i+1];
            //userOrder[j++] -= 1;
    ...
    function takeOrder(address _orderOwner, uint256 _amount) external nonReentrant{
        ...
        require(_amount.mul(ISLAMIdcml) <= p2p[_orderOwner].remainAmount, "Seller has less ISLAMI than order");
            if(_amount.mul(ISLAMIdcml) < p2p[_orderOwner].remainAmount){
                p2p[_orderOwner].remainAmount -= _amount.mul(ISLAMIdcml);
            ...
            else if(_amount.mul(ISLAMIdcml) == p2p[_orderOwner].remainAmount){
                p2p[_orderOwner].remainAmount = 0;
                //require approve from currency(USDT, USDC) contract
                _currency.transferFrom(msg.sender, seller, toPay);
                ISLAMI.transfer(msg.sender, _amount.mul(ISLAMIdcml));
                ISLAMIinOrder -= _amount.mul(ISLAMIdcml);
        ...
        require(_amount.mul(USDdcml) <= p2p[_orderOwner].remainCurrency, "Seller has less USD than order");
            if(_amount.mul(USDdcml) < p2p[_orderOwner].remainCurrency){
                p2p[_orderOwner].remainCurrency -= _amount.mul(USDdcml);
                //require approve from ISLAMICOIN contract
            ...
            else if(_amount.mul(USDdcml) == p2p[_orderOwner].remainCurrency){
                p2p[_orderOwner].remainCurrency -= _amount.mul(USDdcml);
                //require approve from ISLAMICOIN contract
                ISLAMI.transferFrom(msg.sender, seller, toReceive);
                _currency.transfer(msg.sender, _amount.mul(USDdcml));
