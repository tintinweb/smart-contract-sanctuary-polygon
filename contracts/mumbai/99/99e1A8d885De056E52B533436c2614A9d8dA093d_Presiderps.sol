...
contract Presiderps is ERC721A, MultisigOwnable, AccessControl {
    ...
    uint public constant COLLECTION_SIZE = 17760;
    uint public constant AIRDROP_LIMIT = 2000;
    uint public price;
    ...
    function setSalePrice(uint _price) external onlyRole(MANAGER_ROLE) {
        price = _price;
    }

    /// @notice the initial 100 tokens will be minted to the team vault for use in giveaways and collaborations.
    function airdrop(address to, uint quantity) external onlyRole(MANAGER_ROLE) {
    ...
    function mint() external {
        ...
        require(totalSupply() + 1 <= COLLECTION_SIZE, "EXCEEDS_COLLECTION_SIZE");
        uint cost;
        cost = price;
    ...
    /// @notice may mint up to 5 tokens per transaction at the public sale price.
    function mintTo(address _address) external payable {
        require(saleActive, "PUBLIC_SALE_INACTIVE");
        require(totalSupply() + 1 <= COLLECTION_SIZE, "EXCEEDS_COLLECTION_SIZE");
        // uint cost;
        // cost = price;
