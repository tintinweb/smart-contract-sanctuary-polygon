...
contract DalContr is ERC721A, Ownable, ReentrancyGuard {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] calldata _merkleProof) public payable mintCompliance(_mintAmount) mintPriceCompliance(_mintAmount) {
    ...
    // Subtract count of used NFTs
    uint256[] memory tokensOwned = bxCol1NFT.tokensOfOwner(_msgSender());
    uint256 usedNftCount = 0;
    for (uint256 i = 0; i < tokensOwned.length; i++) {
      if (usedNFTs[tokensOwned[i]]) {
        usedNftCount++;
      }
    }
    bxCol1NftCount -= usedNftCount;
