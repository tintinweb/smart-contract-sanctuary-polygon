    ...
    NativeMetaTransaction
{
    ...
    function addAncestors(
        ...
        pro.classId = _classId;
        if(value1 > 0) {
            pro.ancestors.push(value1);
        }
        if(value2 > 0) {
            pro.ancestors.push(value2);
        }
        if(value3 > 0) {
            pro.ancestors.push(value3);
        }
    }
        ...
        onlyModerators
    {
        ...
        if (_type == PropertyType.ANCESTOR) {
            ...
            uint256 ancestorsSize = pro.ancestors.length;
            for (uint32 i = 0; i < ancestorsSize; i++) pro.ancestors.pop();
        } else if (_type == PropertyType.XFACTOR) {
            require(pro.xfactors.length > 0);
            uint256 xfactorsSize = pro.xfactors.length;
            for (uint32 i = 0; i < xfactorsSize; i++) pro.xfactors.pop();
        }
    }
...
contract EthermonAdventureTransform is BasicAccessControl {
    ...
    function transform(uint64 _objId) external {
        ...
        if (objInfo.classId == 0 || objInfo.owner != msgSender()) revert();
        ...
        emit EventTransform(msgSender(), _objId, newObjId);
    }
}