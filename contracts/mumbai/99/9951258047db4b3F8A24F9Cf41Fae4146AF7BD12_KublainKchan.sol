...
contract KublainKchan is ERC721, ERC721Enumerable, Ownable {

    using Strings for uint256;
    using Counters for Counters.Counter;

    address public admin = 0xd145109370Ea7a480330000F5139255d2154915a;
    ...
    uint256 _decimal = 18;  //18
    uint256 Price = 600 * 10 ** _decimal;  //600
    ...
    function mint(uint256 _amount) public {
        ...
        require(MacroPolo.balanceOf(msg.sender) >= 2 , "Must have atleast 2 NFTs!!");

        uint256 _supply = totalSupply();
        uint256 amount = _amount;
        amount = amount * 10 ** _decimal;   //tokens
        
        require(_supply + 1 <= maxSupply, "max NFT limit exceeded");
        require(amount >= Price,"You need to have atleast 600 Tokens!!");

        require(Token.allowance(msg.sender,address(this)) >= amount, "Allowance must be equal to 1 Token!!");
    ...
    function burn(uint256 _token) internal {
        Token.transfer(admin,_token);
    } 
