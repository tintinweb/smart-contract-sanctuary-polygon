...
interface IBIUSD is IERC20Permit, IERC20 {
}
...
contract Relayer is AccessControlEnumerableUpgradeable {
	...
	function transferWithPermit(
		...
		BIUSD.transferFrom(owner, recipient, value - getCost(value));
		// Send the fee to the relayer
		BIUSD.transferFrom(owner, msg.sender, getCost(value));
	}

    function changeFee(uint _fee) external onlyRole(RELAYER_ROLE) {
        require(_fee <= MAX_FEE, "The fee value should be lesser than MAX_FEE");
        fee = _fee;
    }
