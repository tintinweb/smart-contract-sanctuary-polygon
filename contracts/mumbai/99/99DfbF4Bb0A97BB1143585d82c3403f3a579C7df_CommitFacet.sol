...
/******************************************************************************\
/// @title Sipher Portal - Commit Facet
/// @author Ather Labs
/// @dev This facet provides function for the Committer to upload a new commit.
/******************************************************************************/
...
contract CommitFacet is IOperations {
    ...
    /* ------ EVENTS ------ */
    /// @dev The data struct emits when a new commit is successfully stored.
    event NewCommit(bytes32 stateRoot, bytes32 pendingOperationsHash, uint256 batchId, uint256 timestamp);

    /* ------ EXTERNAL FUNTIONS ------ */
    /// @notice Process a new commit, each commit represents the changes on layer II between 2 consecutive commits.
    /// @dev Only Committer can call this function.
    /// @dev This function can only be called when Operation is enabled.
    /// @param _stateRoot The bytes32 Merkle tree hash root of all account balances in layer II.
    /// @param _pendingOperationHash The bytes32 hash result of all pending transaction data on layer II.
    function commitBatch(bytes32 _stateRoot, bytes32 _pendingOperationHash) external {
        ...
        emit NewCommit(newCommit.stateRoot, newCommit.pendingOperationsHash, newCommit.batchId, newCommit.timestamp);
    }
}
