...
contract BlueprintNFT is ERC721Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, IBlueprintNFT {
  ...
  function supportsInterface(bytes4 _interfaceId)
    ...
    returns (bool)
  {
    return _interfaceId == type(IBlueprintNFT).interfaceId || super.supportsInterface(_interfaceId);
  }
   ...
   * @dev Only callable by owner
   * @param _startLock Start time for lock transfer
   * @param _endLock End time for lock transfer
   */
  function setLockTransfer(uint256 _startLock, uint256 _endLock) external onlyOwner {
    startLock = _startLock;
    endLock = _endLock;
    emit SetLockTransfer(_startLock, _endLock);
  }

  /**
   * @notice Set Max Mint Id
   * @dev Only callable by owner
   * @param _maxMintId Maximum mint id
   */
  function setMaxMintId(uint256 _maxMintId) external onlyOwner {
    maxMintId = _maxMintId;
    emit SetMaxMintId(_maxMintId);
  }
  ...
  function bulkMint(uint256 _numberOfNft, address _to) external override onlyRole(MINTER_ROLE) {
    require(mintId + _numberOfNft - 1 <= maxMintId, "0x1");
