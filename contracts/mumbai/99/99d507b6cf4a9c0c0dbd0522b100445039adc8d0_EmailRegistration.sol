...
contract EmailRegistration {
    struct Email {
        ...
        uint256 registrationTimestamp; // Added registration timestamp
    ...
    constructor() {
        ...
        emailRegistrationDuration = 1 days; // Default registration duration
        emailExpiryDuration = 1 days; // Default expiry duration
    }
   ...
   function registerEmail(
    ...
    userEmails[msg.sender] = Email({
        ...
        registrationTimestamp: block.timestamp, // Store the registration timestamp as the current block timestamp
...
function getEmailExpiry(address walletAddress) public view returns (uint256, string memory, string memory, string memory, uint256, uint256) {
    require(bytes(userEmails[walletAddress].emailName).length != 0, "Email not registered");

    uint256 registrationTimestamp = userEmails[walletAddress].registrationTimestamp;
    ...
    return (daysRemaining, registrationDate, expiryDate, expiryTime, registrationTimestamp, expiryTimestamp);
}
