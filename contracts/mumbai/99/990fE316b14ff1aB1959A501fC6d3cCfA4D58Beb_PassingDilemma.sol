...
contract PassingDilemma is Game {
  ...
  // Array of rewards for a given decision, based on the opponent's decision
  ...
  constructor(address _bootcampPlayer, address _reddies) Game(_bootcampPlayer, _reddies) {
    ...
    userRewards[1] = [50,50,0,0];
    userRewards[2] = [0,0,0,0];
    userRewards[3] = [100,100,100,0];

    safePoolRewards[0] = [0,0,0,0];
    safePoolRewards[1] = [0,0,0,0];
    safePoolRewards[2] = [25,25,25,0];
    safePoolRewards[3] = [0,0,0,25];
  ...
  // Lifecycle
  Phase public phase;
  uint256 public playDeadline; // Deadline after which choices will be revealed
  uint256 public watershedLength; // All signups must take place before the deadline minus the watershed
  uint256 public allocationLive; // When are auto-allocations live?

  // Safepool rewards friendly players
  ...
  // Probailities for the risky ones
  ...
  function traitBoost(uint256 tokenId) public view returns (uint32) {
      ...
      return metadata.pet || metadata.quirks == 18 ? 6554 : 0;
  }
  ...
  // There are more timestamps in this one
  ...
  function signUp(uint256[] calldata tokenIds) public originCheck {
    require(block.timestamp < (playDeadline - watershedLength), "SIGNUPS CLOSED");
    _signUp(tokenIds);
  }

  // Function to add some more signups to match up the stragglers
  function helperSignUp(uint256[] calldata tokenIds) public {
    require(helpers[msg.sender], "HELPERS ONLY");
    _signUp(tokenIds);
  }

  function _signUp(uint256[] calldata tokenIds) internal duringGame whenNotPaused phaseCheck(Phase.Play) {
  ...
  function allocate(uint256 count) public whenNotPaused phaseCheck(Phase.Play) {
  ...
  function updateChoices(uint256[] calldata tokenIds, bool[] calldata choices, address signer, bytes calldata signature) internal {
    ...
    for (uint256 i = 0; i < tokenIds.length; i++) {
        ...
        require(player.userLookup == userId, "NOT SIGNERS PLAYER");
  ...
  function batchUpdateChoices(UserChoice[] calldata choices) public onlyOwner phaseCheck(Phase.Play) {
  ...
  function processMatchUps(uint256 count) public whenNotPaused phaseCheck(Phase.Results) {
    uint256 i;
  ...
  function processRefunds(uint8 queue, uint256 count) public whenNotPaused phaseCheck(Phase.Results) {
  ...
  function payUser(uint256 userId) internal {
    ...
    if(safeCount > 0) {
      payout += uint256(user.safeTokens.length) * (safePool / safeCount); // divide rounds down to the nearest REDDIE
    }
  ...
  function processUsers(uint256 count) public whenNotPaused phaseCheck(Phase.Payout) {
