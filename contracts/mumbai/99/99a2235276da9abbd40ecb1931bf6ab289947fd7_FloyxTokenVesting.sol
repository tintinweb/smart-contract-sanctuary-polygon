...
contract FloyxTokenVesting is ReentrancyGuard, Ownable {
     ...
     * @param _token Address of the Floyx token contract
     */
     ...
     * @param _slicePeriodSeconds The duration of a slice period for vesting in seconds.
     * @param _amount The total amount of tokens to be released at the end of the vesting.
     * @param _releasedPercentInMultipleOf10 The released percentage for each slice period (in multiples of 10).
     * @param _tgeAmount The amount of tokens to be released at the start.
     */
    ...
    function createVestingSchedule(
        ...
        emit addVesting(
            ...
            _slicePeriodSeconds,
            _amount,
            _releasedPercentInMultipleOf10,
            _tgeAmount
        );
    }

    /**
     * @dev Modifies an existing vesting schedule for a beneficiary.
     * @param _beneficiary The address of the beneficiary.
     * @param _duration The new duration of the vesting schedule.
     * @param _slicePeriodSeconds The new duration of a slice period for vesting.
     * @param _amount The new total amount of tokens to be released at the end of the vesting.
     * @param _releasedPercentInMultipleOf10 The new released percentage for each slice period (in multiples of 10).
     */
    ...
    function modifiyVesting(
        ...
        uint256 _amount,
        uint8 _releasedPercentInMultipleOf10
    ) external onlyOwner onlyIfVestingScheduleNotRevoked(_beneficiary) {
        ...
        vestingSchedule.releasedPercent = _releasedPercentInMultipleOf10;
        emit modifyVesting(
            _beneficiary,
            _duration,
            _slicePeriodSeconds,
    ...
    /**
     * @dev Revokes a vesting schedule for a beneficiary.
     * @param _beneficiary The address of the beneficiary.
     */
    ...
    function revoke(
        ...
        uint256 releaseAmount = vestedAmount.add(tgeAmount);

        if (releaseAmount > 0) {
            claimVestedToken(_beneficiary, releaseAmount);
        }

        uint256 unreleased = vestingSchedule.amountTotal.sub(
            vestingSchedule.released
        );
        totalVestedAmount = totalVestedAmount.sub(unreleased);
    ...
    function deleteVesting(address _beneficiary) external onlyOwner {
        VestingSchedule storage vestingSchedule = vestingSchedules[
            _beneficiary
        ];
        require(vestingSchedule.revoked, "first revoke the vesting");
        delete vestingSchedules[_beneficiary];
    }

    /**
     * @dev Returns the claimable amount of tokens for a beneficiary.
     * @param _beneficiary The address of the beneficiary.
     * @return The claimable amount of tokens.
     */
    ...
    /**
     * @dev Returns the total vested amount of tokens.
     ...
     * @return The total released amount of tokens.
     */
    ...
    /**
     * @dev Returns the amount of unallocated funds in the contract.
    ...
    /**
     * @dev Returns the available funds in the contract.
    ...
    /**
     * @dev Claims vested tokens for a beneficiary. Transfers the specified amount of tokens to the beneficiary.
     * @param _beneficiary The address of the beneficiary.
     * @param _amount The amount of tokens to claim.
     * Requirements:
     * - The vesting schedule for the beneficiary must not be revoked.
     * - The release amount (tgeAmount + vestedAmount) must be greater than or equal to the specified amount.
     */

    function claimVestedToken(
        address _beneficiary,
        uint256 _amount
    ) public nonReentrant onlyIfVestingScheduleNotRevoked(_beneficiary) {
        VestingSchedule storage vestingSchedule = vestingSchedules[
            _beneficiary
        ];
        uint256 tgeAmount = _getTgeAmount(vestingSchedule);
        uint256 vestedAmount = _computeReleasableAmount(vestingSchedule);
        uint256 releaseAmount = tgeAmount.add(vestedAmount);

        require(
            releaseAmount >= _amount,
            "TokenVesting: cannot release tokens, insufficient vested tokens"
        );

        vestingSchedule.released = vestingSchedule.released.add(_amount);
        totalVestedAmount = totalVestedAmount.sub(_amount);
        totalReleasedAmount = totalReleasedAmount.add(_amount);
    ...
    /**
     * @dev Withdraws unallocated funds to the specified receiver address.
     * @param _receiver The address of the receiver.
     * @param _amount The amount of unallocated funds to withdraw.
     * Requirements:
     * - The receiver address must not be the zero address.
     * - The specified amount must be greater than zero and not exceed the available unallocated funds amount.
     */
    ...
    /**
     * @dev Calculates the releasable amount of tokens per slice period for a vesting schedule.
     * @param vestingSchedule The vesting schedule.
     * @return The releasable amount of tokens per slice period.
     */
    ...
    function _calculateReleasableAmount(
        ...
        return totalAmount.mul(releasedPercent).div(1000);
    }

    /**
     * @dev Gets the TGE (Token Generation Event) amount for a vesting schedule.
     * @param vestingSchedule The vesting schedule.
     * @return The TGE amount.
     */
    ...
    /**
     * @dev Gets the current time in seconds since the Unix epoch.
