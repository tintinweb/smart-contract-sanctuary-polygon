...
contract ThreeEarsBunny is AccessControl, ERC721ALockable, DefaultOperatorFilterer, ERC2981, Ownable {
    ...
    function hasMinted() external view returns (bool) {
        ...
        if (currentPhase == Phase.GuaranteeWhiteList) {
            return guaranteeMinted[msg.sender];
        } else if (currentPhase == Phase.NonGuaranteeWhiteList) {
            return nonGuaranteeMinted[msg.sender];
        }
        return false;
    }
