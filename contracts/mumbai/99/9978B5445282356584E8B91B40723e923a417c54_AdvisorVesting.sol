...
contract NWTVestingHub is Ownable, ReentrancyGuard, Pausable {
  using SafeMath for uint256;
  using SafeERC20 for IERC20;

  IERC20 public token = IERC20(0x1FeC2ED74b5aa55aaD9e0C3e472239A5927aAa62);
  ...
  function claim(address benificery) public whenNotPaused nonReentrant {
    uint256 claimableTokens = getClaimableTokens(benificery);
    require(claimableTokens > 0, "NWTVestingHub: No claimable tokens");

    uint256 elapsedTime = block.timestamp - start;

    if (elapsedTime >= duration == false) {
      require(
        block.timestamp >= buyers[benificery].lastClaimTime.add(claimCooldown),
        "NWTVestingHub: The claim cool down period has not passed yet"
      );
    }

    _claimedTokens[benificery] += claimableTokens;

    token.safeTransfer(benificery, claimableTokens);
    buyers[_msgSender()].claimedAmount = buyers[_msgSender()].claimedAmount.add(
      claimableTokens
    );
    buyers[_msgSender()].lastClaimTime = block.timestamp;

    emit TokensClaimed(benificery, claimableTokens);
  }
  ...
  function getReleasedTokensAtTimestamp(
    ...
    uint256 vestingStartTime = start.add(vestingCliff);
    ...
    uint256 initialRelease = (_allocatedTokens[beneficiary] *
      initialReleasePercentage) / 100;
    uint256 remainingTokensAfterInitialRelease = _allocatedTokens[beneficiary] -
      initialRelease;
  ...
  function allocateTokens(
    address[] memory benificiaries,
    uint256[] memory amounts
  ) external onlyOwner {
    _allocateTokens(benificiaries, amounts);
  }
  ...
  function _allocateTokens(
    ...
    for (uint256 i = 0; i < beneficiaries.length; i++) {
      ...
      _allocatedTokens[beneficiaries[i]] =
        _allocatedTokens[beneficiaries[i]] +
        amounts[i];
  ...
  function withdrawUnsoldTokens() public onlyOwner {
    require(
      ...
      "NWTVestingHub: Cannot withdraw tokens before end"
    );
...
import { NWTVestingHub } from "../base/NWTVestingHub.sol";

contract AdvisorVesting is NWTVestingHub {
  uint256 private START_DATE = block.timestamp;
  uint256 private DURATION = 690 days;
  uint256 private INITIAL_REALEASE_PERCENTAGE = 0;
  uint256 private CLAIM_COOL_DOWN = 30 days;
  uint256 private VESTING_CLIFF = 90 days;
