...
import {INFight} from "../Interfaces/NFight/INFight.sol";
import {VRFConsumerBase} from "../Libraries/UpgradeableVRFConsumerBase/UpgradeableVRFConsumerBase.sol";
import {Base64} from "../Libraries/Base64.sol";

import {ASSPLayerChild} from "../ASSPLayer/ASSPLayerChild.sol";

contract NFightChildUpgradeable is ASSPLayerChild, VRFConsumerBase, PullPaymentUpgradeable, ReentrancyGuardUpgradeable {
    using Strings for uint256;
    ...
    uint256 public constant attributeBaseCost = 1e16; // 0.01 MATIC
    ...
    mapping(address => mapping(uint256 => INFight.Fighter)) public nFighters;
    mapping(address => address) internal contractOwners;
    mapping(address => Player) public players;

    mapping(uint256 => uint256) internal randomnessForToken;
    mapping(bytes32 => MainnetNFT) internal tokenForRequest;
    ...
    function initialize(
        ...
        NFTXP = ERC20(wrassToken);
        __PullPayment_init_unchained();
        VRFConsumerBase.unchainedInitialize(vRFCoordinator, linkToken);
        ASSPLayerChild.initialize(trustedForwarder, _fxChild);
        keyHash = _keyhash;
        fee = 0.0001 * 10**18;
        wETH = ERC20(_wETH);
    }

    function _msgData() internal view override(ASSPLayerChild) returns (bytes calldata) {
        return super._msgData();
    }

    function _msgSender() internal view override(ASSPLayerChild) returns (address sender) {
        return super._msgSender();
    }

    /* solhint-disable quotes */

    function jsonDataForToken(address mainnetContract, uint256 tokenId) public view override returns (string memory) {
        INFight.Fighter memory fighter = nFighters[mainnetContract][tokenId];

        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
                    Base64.encode(bytes(abi.encodePacked('{"attributes":', encodedAttributes(fighter), '"}}')))
                )
            );
    }

    function encodedAttributes(INFight.Fighter memory fighter) internal pure returns (bytes memory) {
        return
            abi.encodePacked(
                '{"aggression":"',
                fighter.aggression.toString(),
                '", "awareness":"',
                fighter.awareness.toString(),
                '", "determination":"',
                fighter.determination.toString(),
                '", "power":"',
                fighter.power.toString(),
                '", "resilience":"',
                fighter.resilience.toString(),
                '", "speed":"',
                fighter.speed.toString()
            );
    }
    ...
    function _beforeProjectRegistration(
        ...
        // bool success = wETH.transferFrom(owner, address(this), 5 ether);
        // TODO: this payment should be transferred to a Vesting contract
        // require(success, "payment err");
    }
    ...
    function _onProjectRegistration(
        ...
        contractOwners[nftContract] = contractOwner;
        emit ProjectRegistered(nftContract, chainId);
        _sendMessageToRoot(abi.encode(nftContract));
    }

    function setMatchAddress(address matchAddress) public onlyRole(DEFAULT_ADMIN_ROLE) {
        grantRole(REFEREE_ROLE, matchAddress);
    }

    struct NFighterToken {
        MainnetNFT nft;
        INFight.Fighter fighter;
    }

    function setStartingAttributes(NFighterToken[] calldata fighters) external onlyRole(DEFAULT_ADMIN_ROLE) {
        uint256 length = fighters.length;
        for (uint256 index = 0; index < length; ) {
            NFighterToken memory fighterToken = fighters[index];
            INFight.Fighter storage fighter = nFighters[fighterToken.nft.contractAddress][fighterToken.nft.tokenId];

            fighter.aggression = fighterToken.fighter.aggression;
            fighter.awareness = fighterToken.fighter.awareness;
            fighter.determination = fighterToken.fighter.determination;
            fighter.power = fighterToken.fighter.power;
            fighter.resilience = fighterToken.fighter.resilience;
            fighter.speed = fighterToken.fighter.speed;
    ...
    function reportWin(address player) public onlyRole(REFEREE_ROLE) {
        players[player].wins += 1;
    }

    function reportLoss(address player) public onlyRole(REFEREE_ROLE) {
        players[player].losses += 1;
    }

    function registerPolygonProject(address contractAddress, address owner) external {
        _registerProject(contractAddress, owner, 137);
    }
    ...
    function registerPolygonNFT(
        ...
        _registerToken(contractAddress, tokenId, owner);
    }
    ...
    function _onRegistration(
        ...
        if (fighter.power == 0) {
            nFighters[mainnetContract][tokenId] = INFight.Fighter({
                power: 0,
                speed: 0,
                aggression: 0,
                determination: 0,
            ...
            tokenForRequest[requestId] = MainnetNFT({contractAddress: mainnetContract, tokenId: tokenId});
        } else {
            emit FighterUpdated(
                mainnetContract,
                tokenId,
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        MainnetNFT storage token = tokenForRequest[requestId];
        INFight.Fighter storage fighter = nFighters[token.contractAddress][token.tokenId];
        uint256[] memory attributes = calculateAttributes(randomness, 6);

        fighter.aggression = attributes[0];
        fighter.awareness = attributes[1];
        fighter.determination = attributes[2];
        fighter.power = attributes[3];
        fighter.resilience = attributes[4];
        fighter.speed = attributes[5];

        emit FighterUpdated(
            token.contractAddress,
            token.tokenId,
            ownerOf(token.contractAddress, token.tokenId),
            fighter.aggression,
            fighter.awareness,
            fighter.determination,
            fighter.power,
    ...
    function calculateAttributes(uint256 randomValue, uint256 n)
        internal
        view
        returns (uint256[] memory expandedValues)
    {
        ...
        for (uint256 i = 0; i < n; i++) {
            uint256 raw = uint256(keccak256(abi.encode(randomValue, i, block.timestamp)));
            expandedValues[i] = (raw % 400) + 350;
        }
    ...
    function upgradeWrassler(
        address mainnetContract,
        uint256 tokenId,
        INFight.Fighter memory upgradeParams
    ) public nonReentrant {
        INFight.Fighter storage fighter = nFighters[mainnetContract][tokenId];

        uint256 total = calculateUpgrades(fighter, upgradeParams);
        bool success = NFTXP.transferFrom(_msgSender(), address(this), total);

        require(success, "buy failed");

        fighter.power += upgradeParams.power;
        fighter.speed += upgradeParams.speed;
        fighter.aggression += upgradeParams.aggression;
        fighter.determination += upgradeParams.determination;
        fighter.resilience += upgradeParams.resilience;
        fighter.awareness += upgradeParams.awareness;

        _asyncTransfer(contractOwners[mainnetContract], total / 10);

        emit FighterUpdated(
            mainnetContract,
            tokenId,
            ownerOf(mainnetContract, tokenId),
            fighter.aggression,
            fighter.awareness,
            fighter.determination,
            fighter.power,
    ...
    function calculateUpgrades(INFight.Fighter storage fighter, INFight.Fighter memory upgradeParams)
        internal
        view
        returns (uint256)
    {
        return
            calculateUpgradeCost(fighter.power, upgradeParams.power) +
            calculateUpgradeCost(fighter.speed, upgradeParams.speed) +
            calculateUpgradeCost(fighter.aggression, upgradeParams.aggression) +
            calculateUpgradeCost(fighter.determination, upgradeParams.determination) +
            calculateUpgradeCost(fighter.resilience, upgradeParams.resilience) +
            calculateUpgradeCost(fighter.awareness, upgradeParams.awareness);
    }

    function calculateUpgradeCost(uint256 startPoints, uint256 toAdd) public pure returns (uint256) {
        return attributeBaseCost * ((startPoints + toAdd)**3 - startPoints**3);
    }
    ...
    function getFighter(address mainnetContract, uint256 _tokenId)
        external
        view
        returns (
            uint256 power,
    ...
    {
        INFight.Fighter memory fighter = nFighters[mainnetContract][_tokenId];
 ...
 * @dev until it calls responds to a request.
 */
abstract contract VRFConsumerBase is VRFRequestIDBase, Initializable {
    /**
     * @notice fulfillRandomness handles the VRF response. Your contract must
     * @notice implement it. See "SECURITY CONSIDERATIONS" above for important
     * @notice principles to keep in mind when implementing your fulfillRandomness
    ...
    function unchainedInitialize(address _vrfCoordinator, address _link) internal initializer {
        vrfCoordinator = _vrfCoordinator;
        LINK = LinkTokenInterface(_link);
    }
...
import {FxBaseChildTunnelUpgradeable} from "../Libraries/tunnel/FxBaseChildTunnelUpgradeable.sol";
import {EssentialERC2771ContextUpgradeable} from "../Libraries/fwd/EssentialERC2771ContextUpgradeable.sol";
import "../Libraries/fwd/IForwardRequest.sol";

import "./ASSPLayer.sol";
import "./ERC721ContractEnumerable.sol";

contract ASSPLayerChild is
    ASSPLayer,
    FxBaseChildTunnelUpgradeable,
    AccessControlUpgradeable,
    ERC721ContractEnumerable,
    EssentialERC2771ContextUpgradeable
{
    ...
    /// @notice Restricted to meta-tx forwarder
    modifier onlyForwarder() {
        require(isTrustedForwarder(msg.sender), "ASSPLayer:429");
        _;
    }

    /// @notice Restricted to approved operators
    modifier onlyApproved(address contractAddress, uint256 tokenId) {
        address owner = ownerOf(contractAddress, tokenId);
        require(isApprovedForAll(owner, _msgSender()), "ASSPLayer:429");
        _;
    }

    /// -----------------------------------------------------------------------
    /// Initiliazer - Protocol is Upgradeable Proxy
    /// -----------------------------------------------------------------------
    function initialize(address trustedForwarder, address _fxChild) internal {
        _setupRole(DEFAULT_ADMIN_ROLE, address(msg.sender));
        EssentialERC2771ContextUpgradeable.unchainedInitialize(address(trustedForwarder));
        FxBaseChildTunnelUpgradeable.initialize(_fxChild);
    }
    ...
    /// -----------------------------------------------------------------------
    /// Meta-transacation public functions
    /// -----------------------------------------------------------------------

    function registerProject(address contractAddress) external payable override onlyForwarder {
        _registerProject(contractAddress, _msgSender(), 1);
    }

    /// @dev first attempt at using trusted NFT from calldata instead of args
    function registerToken() external onlyForwarder {
        IForwardRequest.NFT memory nft = _msgNFT();

        _registerToken(nft.contractAddress, nft.tokenId, _msgSender());
    }

    /// -----------------------------------------------------------------------
    /// FxPortal functions for L1 bytes messages
    /// -----------------------------------------------------------------------
    function _processMessageFromRoot(
        ...
        (address contractAddress, address contractOwner, uint256 tokenId, address tokenOwner) = abi.decode(
            data,
            (address, address, uint256, address)
        );

        if (contractOwner != address(0)) {
            _registerProject(contractAddress, contractOwner, 1);
            return;
        }

        if (tokenId != 0) {
            _registerToken(contractAddress, tokenId, tokenOwner);
        }
    }
    ...
    function _registerProject(
        ...
        _beforeProjectRegistration(contractAddress, owner, chainId);
        registeredContract[contractAddress] = true;
        _onProjectRegistration(contractAddress, owner, chainId);
    }
    ...
    function _registerToken(
        ...
        require(registeredContract[mainnetContract] == true, "Project not registered");
        address existingOwner = _owners[mainnetContract][tokenId];

        if (existingOwner != address(0)) {
            _removeTokenFromOwnerEnumeration(existingOwner, mainnetContract, tokenId);
        }

        _addTokenToOwnerEnumeration(owner, mainnetContract, tokenId);
        emit TokenRegistered(mainnetContract, tokenId, owner);

        _onRegistration(mainnetContract, tokenId, owner);
    }
    ...
    function _msgSender()
        ...
        view
        virtual
        override(EssentialERC2771ContextUpgradeable, ContextUpgradeable)
    ...
    function _msgData()
        ...
        view
        virtual
        override(ContextUpgradeable, EssentialERC2771ContextUpgradeable)
        returns (bytes calldata)
    {
        return EssentialERC2771ContextUpgradeable._msgData();
    }
}
 ...
 */
abstract contract FxBaseChildTunnelUpgradeable is IFxMessageProcessor {
    ...
    function initialize(address _fxChild) internal {
        fxChild = _fxChild;
    }
    ...
    function processMessageFromRoot(
        ...
        require(msg.sender == fxChild, "FxBaseChildTunnel: INVALID_SENDER"); // polygon contract
 ...
 * @dev Context variant with ERC2771 support.
 */
abstract contract EssentialERC2771ContextUpgradeable is Context, Initializable {
    address private _trustedForwarder;
    address public owner;
    ...
    function unchainedInitialize(address trustedForwarder) internal initializer {
        _trustedForwarder = trustedForwarder;
    }
...
contract ERC721ContractEnumerable {
    struct OwnedToken {
        address contractAddress;
        uint256 tokenId;
    }
    ...
    // Mapping from contract to token ID to owner address
    mapping(address => mapping(uint256 => address)) internal _owners;

    // Mapping owner address to token count
    mapping(address => uint256) internal _balances;

    // Mapping from owner to list of OwnedToken structs
    mapping(address => mapping(uint256 => OwnedToken)) internal _ownedTokens;
     ...
     * @dev See {IERC721-ownerOf}.
     */
    function ownerOf(address contractAddress, uint256 tokenId) public view virtual returns (address) {
        address owner = _owners[contractAddress][tokenId];
        // require(owner != address(0), "ERC721: owner query for nonexistent token");
        return owner;
    }
     ...
     * @dev See {IERC721Enumerable-tokenOfOwnerByIndex}.
     */
    function tokenOfOwnerByIndex(address owner, uint256 index) public view virtual returns (OwnedToken memory) {
    ...
    function _addTokenToOwnerEnumeration(
        ...
        _ownedTokens[to][length] = OwnedToken({contractAddress: contractAddress, tokenId: tokenId});
        _ownedTokensIndex[contractAddress][tokenId] = length;
        _owners[contractAddress][tokenId] = to;
        _balances[to] += 1;
    }
    ...
    function _removeTokenFromOwnerEnumeration(
        ...
        uint256 tokenIndex = _ownedTokensIndex[contractAddress][tokenId];

        // When the token to delete is the last token, the swap operation is unnecessary
        if (tokenIndex != lastTokenIndex) {
            OwnedToken memory lastToken = _ownedTokens[from][lastTokenIndex];

            _ownedTokens[from][tokenIndex] = lastToken; // Move the last token to the slot of the to-delete token
            _ownedTokensIndex[contractAddress][tokenId] = tokenIndex; // Update the moved token's index
        }

        // This also deletes the contents at the last position of the array
        delete _ownedTokensIndex[contractAddress][tokenId];
        delete _ownedTokens[from][lastTokenIndex];
        _balances[from] -= 1;
    }
...
import {INFight} from "../Interfaces/NFight/INFight.sol";
import {EssentialERC2771Context} from "../Libraries/fwd/EssentialERC2771Context.sol";

import {WrasslingMatch} from "../WrasslingMatch/WrasslingMatch.sol";

contract TrainingMatch is WrasslingMatch, AccessControl, EssentialERC2771Context, VRFConsumerBase {
    uint256 public constant STAT_BASE = 100;
    ...
    mapping(uint256 => WrasslingMove) internal moves;
    Counters.Counter private movesCount;
    Counters.Counter private _matchId;

    mapping(bytes32 => address) internal requestIdPlayer;
    mapping(address => Match) public matches;

    INFight public nFight;

    mapping(address => uint256) public completedTraining;
    ...
    constructor(
        ...
        bytes32 _keyhash,
        address _nFight
    ) VRFConsumerBase(vRFCoordinator, linkToken) EssentialERC2771Context(trustedForwarder) {
        _setupRole(DEFAULT_ADMIN_ROLE, address(0xc0F5b93Bb77271d9CE898d08013f4958d1478534));
        _setupRole(ADMIN_ROLE, address(0xc0F5b93Bb77271d9CE898d08013f4958d1478534));
    ...
    function _msgData() internal view override(Context, EssentialERC2771Context) returns (bytes calldata) {
        return EssentialERC2771Context._msgData();
    }

    function _msgSender() internal view override(Context, EssentialERC2771Context) returns (address sender) {
        return EssentialERC2771Context._msgSender();
    }

    function setNFight(address nFightAddress) public onlyRole(ADMIN_ROLE) {
        nFight = INFight(nFightAddress);
    }

    function setMoves(WrasslingMove[] memory _moves) public onlyRole(ADMIN_ROLE) {
        uint256 startIndex = movesCount._value;
        for (uint256 index = 0; index < _moves.length; index++) {
            moves[index + startIndex] = _moves[index];
            movesCount.increment();
        }
    }

    function resetMoves() public onlyRole(ADMIN_ROLE) {
        movesCount.reset();
    }

    function challengeCpu(
        ...
        require(matches[player].movesCount == 0, "Training match in progress");

        bytes32 requestId = requestRandomness(keyHash, fee);
        requestIdPlayer[requestId] = player;
    ...
    function calculateMove(
        WrasslingMove memory move,
        INFight.Fighter memory performer,
        INFight.Fighter memory defender,
        uint256 luckSeed
    ) internal pure returns (uint256 hpHit) {
        hpHit = move.impact.add(100);

        hpHit = hpHit.add(move.aggressionMultiplier.mul(performer.aggression).div(400));
        hpHit = hpHit.add(move.powerMultiplier.mul(performer.power).div(400));
        hpHit = hpHit.add(move.speedMultiplier.mul(performer.speed).div(400));
        hpHit = hpHit.add(move.resilienceMultiplier.mul(performer.resilience).div(400));
        hpHit = hpHit.add(move.determinationMultiplier.mul(performer.determination).div(400));

        uint256 hpSub = 0;
        hpSub = hpSub.add(move.awarenessMultiplier.mul(defender.awareness).div(400));
        hpSub = hpSub.add(move.powerDefenseMultiplier.mul(defender.power).div(400));
        hpSub = hpSub.add(move.speedDefenseMultiplier.mul(defender.speed).div(400));
        hpSub = hpSub.add(move.resilienceDefenseMultiplier.mul(defender.resilience).div(400));
        hpSub = hpSub.add(move.determinationDefenseMultiplier.mul(defender.determination).div(400));

        hpHit = hpHit > hpSub ? hpHit.sub(hpSub) : 0;

        uint256 luckAdjust = luckSeed.mod(30);

        if (luckAdjust >= 15) {
            hpHit = hpHit.add(luckAdjust.sub(15));
        }

        if (luckAdjust < 15) {
            hpHit = hpHit > luckAdjust ? hpHit.sub(luckAdjust) : 0;
        }

        return hpHit;
    }

    function allGt(uint256[] memory elements, uint256 limit) internal pure returns (bool) {
        for (uint256 index = 0; index < elements.length; index++) {
            if (elements[index] <= limit) {
                return false;
            }
        }
    ...
    function performSinglePlayerTurn(uint8 moveIndex) external onlyForwarder {
        Match storage currentMatch = matches[_msgSender()];

        uint256[] memory moveIdxs = expandIndex(currentMatch.randomness, 4);
        uint256[] memory moveSeeds = expand(currentMatch.randomness, 2);

        INFight.Fighter memory player = makeWrassler(currentMatch.challengerContract, currentMatch.challengerTokenId);
        INFight.Fighter memory cpu = makeWrassler(currentMatch.targetContract, currentMatch.targetTokenId);

        uint8[3] memory moveIndexes = abi.decode(currentMatch.nextMoves, (uint8[3]));

        WrasslingMove memory attack = moves[moveIndexes[moveIndex]];
        uint256 hpHit = calculateMove(attack, player, cpu, moveSeeds[0]);
        // emit MovePerformed(keccak256(abi.encodePacked(_msgSender())), hpHit, attack.id);
        if (currentMatch.targetHp <= hpHit) {
            currentMatch.targetHp = 0;
            nFight.reportWin(_msgSender(), currentMatch.targetContract, currentMatch.targetTokenId);
            currentMatch.movesCount += 1;
            completedTraining[_msgSender()] = currentMatch.challengerHp;
            return;
        }

        currentMatch.targetHp -= uint16(hpHit);

        WrasslingMove memory cpuAttack = moves[moveIdxs[0]];
        uint256 playerHpHit = calculateMove(cpuAttack, cpu, player, moveSeeds[1]);
        // emit MovePerformed(keccak256(abi.encodePacked(_msgSender())), playerHpHit, cpuAttack.id);

        if (currentMatch.challengerHp <= playerHpHit) {
            currentMatch.challengerHp = 0;
            nFight.reportLoss(_msgSender());
            currentMatch.movesCount += 2;
            return;
        }

        currentMatch.challengerHp -= uint16(playerHpHit);
        currentMatch.nextMoves = abi.encode(uint8(moveIdxs[1]), uint8(moveIdxs[2]), uint8(moveIdxs[3]));
        currentMatch.movesCount += 2;
    }
     ...
     */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        Match storage currentMatch = matches[requestIdPlayer[requestId]];
        currentMatch.randomness = randomness;

        uint256[] memory nextMoves = expandIndex(currentMatch.randomness, 3);
        currentMatch.nextMoves = abi.encode(uint8(nextMoves[0]), uint8(nextMoves[1]), uint8(nextMoves[2]));
        emit MatchReady(currentMatch.id);
    }

    function expand(uint256 randomValue, uint256 n) internal view returns (uint256[] memory expandedValues) {
        expandedValues = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomValue, i, block.timestamp)));
        }
        return expandedValues;
    }

    function expandIndex(uint256 randomValue, uint256 n) internal view returns (uint256[] memory expandedValues) {
        expandedValues = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomValue, i, block.timestamp))) % movesCount._value;
        }
        return expandedValues;
    }

    function makeWrassler(address, uint256) internal pure returns (INFight.Fighter memory) {
        INFight.Fighter memory wrassler = INFight.Fighter({
            power: STAT_BASE,
            speed: STAT_BASE,
            aggression: STAT_BASE,
            determination: STAT_BASE,
            resilience: STAT_BASE,
            awareness: STAT_BASE
        });
    ...
    function withdrawToken(address _tokenContract, uint256 _amount) public onlyRole(ADMIN_ROLE) {
...
abstract contract WrasslingMatch {
    event ChallengeMade(uint256 matchId);
    event MatchReady(uint256 matchId);
    event MovePerformed(bytes32 matchId, uint256 hpHit, string moveId);
...
contract SinglePlayerMatch is Initializable, AccessControlUpgradeable, ERC2771ContextUpgradeable, VRFConsumerBase {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    event RequestedRandomness(bytes32 requestId);
    ...
    event MatchReady(uint256 matchId);
    event MovePerformed(uint256 matchId, uint256 hpHit, string moveId);
    ...
    mapping(bytes32 => uint256) internal matchForRandomRequest;
    mapping(uint256 => Match) public matches;

    INFight public nFight;

    modifier onlyForwarder() {
        require(isTrustedForwarder(msg.sender), "WA:429");
        _;
    }
    ...
    function initialize(
        ...
        ERC2771ContextUpgradeable.__ERC2771Context_init_unchained(address(trustedForwarder));
        VRFConsumerBase.unchainedInitialize(vRFCoordinator, linkToken);
        keyHash = _keyhash;
        fee = 0.0001 * 10**18;
    ...
    function challengeCpu(
        ...
        bytes32 requestId = requestRandomness(keyHash, fee);
        _matchId.increment();
        uint256 matchId = _matchId.current();

        matchForRandomRequest[requestId] = matchId;
    ...
    function performSinglePlayerTurn(uint8 moveIndex, uint256 matchId) external onlyForwarder {
        ...
        WrasslingMove memory attack = moves[moveIndexes[moveIndex]];
        uint256 hpHit = calculateMove(attack, player, cpu, moveSeeds[0]);
        emit MovePerformed(matchId, hpHit, attack.id);
        if (currentMatch.targetHp <= hpHit) {
            currentMatch.targetHp = 0;
            nFight.reportWin(_msgSender(), currentMatch.targetContract, currentMatch.targetTokenId);
            currentMatch.movesCount += 1;
        ...
        WrasslingMove memory cpuAttack = moves[moveIdxs[0]];
        uint256 playerHpHit = calculateMove(cpuAttack, cpu, player, moveSeeds[1]);
        emit MovePerformed(matchId, playerHpHit, cpuAttack.id);
     ...
     */
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        Match storage currentMatch = matches[matchForRandomRequest[requestId]];
        currentMatch.randomness = randomness;
    ...
    function makeWrassler(address contractAddress, uint256 _tokenId) internal view returns (INFight.Fighter memory) {
        (
            uint256 aggression,
            uint256 awareness,
            uint256 determination,
...
contract CPUWrasslers is ERC721PresetMinterPauserAutoId, Ownable {
    using Strings for uint256;
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    mapping(uint256 => INFight.Fighter) public cpuWrasslers;
    mapping(uint256 => string) internal metadata;

    constructor() ERC721PresetMinterPauserAutoId("CPUWrasslers", "WRASSNPC", "ipfs://") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ADMIN_ROLE, msg.sender);
    }

    function devMint() public {
        _tokenIds.increment();
    ...
    function attributesURI(uint256 tokenId) public view returns (string memory) {
        require(_exists(tokenId), "attributesURI query for nonexistent token");

        INFight.Fighter storage wrassler = cpuWrasslers[tokenId];

        return
            string(
                abi.encodePacked(
                    "data:application/json;base64,",
                    Base64.encode(bytes(abi.encodePacked('{"attributes":', encodedAttributes(wrassler), '"}}')))
                )
            );
    }

    function encodedAttributes(INFight.Fighter memory wrassler) internal pure returns (bytes memory) {
        return
            abi.encodePacked(
                '{"aggression":"',
                wrassler.aggression.toString(),
                '", "awareness":"',
                wrassler.awareness.toString(),
                '", "determination":"',
                wrassler.determination.toString(),
                '", "power":"',
                wrassler.power.toString(),
                '", "resilience":"',
                wrassler.resilience.toString(),
                '", "speed":"',
                wrassler.speed.toString()
            );
    }
    /* solhint-enable quotes */
}
...
contract FirstEditionWrasslers is ERC721, ERC721EnumerableEssential, Ownable, AccessControl, SignedAllowance {
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");
    ...
    constructor(address _nFight) ERC721("First Edition Wrasslers", "FEWRASS") {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ADMIN_ROLE, msg.sender);
    ...
    function claim(uint256 nonce, bytes memory signature) external {
        ...
        _transfer(draftVault, msg.sender, nonce);
    }

    function mintToPool(uint256 count) external onlyOwner {
        for (uint256 index = 0; index < count; ) {
            _mint(draftVault, ++lastTokenId);
            nFight.registerToken(address(this), lastTokenId);
            ++index;
        }
    }
    ...
    function setVault(address _draftVault) external onlyOwner {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721EnumerableEssential) {
        super._beforeTokenTransfer(from, to, tokenId);
    }
}
...
import {VestingWalletClone} from "./VestingWalletClone.sol";
import {ClonesWithImmutableArgs} from "../clones/ClonesWithImmutableArgs.sol";

contract VestingWalletCloneFactory {
    using ClonesWithImmutableArgs for address;

    VestingWalletClone public implementation;

    constructor(VestingWalletClone implementation_) {
        implementation = implementation_;
    }

    function createClone(address beneficiary) external returns (VestingWalletClone clone) {
        bytes memory data = abi.encodePacked(beneficiary, block.timestamp);
        clone = VestingWalletClone(payable(address(implementation).clone(data)));
    }
}
