...
contract PixelNFTFactory is AccessControl {
    ...
    function createRaffle(
        ...
        // PixelNFT(collection).grantRole(PixelNFT(collection).MINTER_ROLE(), minter);
    ...
    // function getRaffles(address user) public view returns(Raffle[] memory userRaffles){
    //     userRaffles = new Raffle[](raffles[user].length);
    //     for(uint256 i = 0; i < userRaffles.length; i++) {
    //         userRaffles[i] = raffles[user][i];
    //     }
    // }
...
contract PixelNFTMinter is AccessControl {
    ...
    constructor(
        ...
        publicStartTime = _preSaleStartTime + 30 days;
        emit StatusUpdated(status);
    }

    modifier isMintValid(uint256 _count) {
        require(_count <= maxPerUser, "exceed maxPerUser");
    ...
    ) public payable isMintValid(_count) {
        require(!paused, 'minting is paused');
        require(status == Status.PreSale || status == Status.Public, 'minting is unavailable');
        if (status == Status.PreSale) {
            require(preSaleStartTime <= block.timestamp, 'preSale is not started');
            require(MerkleProof.verify(_proof, merkleRoot, keccak256(abi.encodePacked(_to))), "invalid proof");
        }
    ...
    function awardToWinner(uint256 _requestId, uint256 _randomWord) public returns(bool) {
        require(msg.sender == address(vrf), 'Only VRF can call');
        ...
        require(award(winnerAddress), 'Unable to award');
...
contract PixelNFT is ERC721, ERC721Enumerable, ERC721URIStorage, AccessControl, Ownable, ERC721Royalty {
    ...
    function mint(address to, uint256 id) external onlyRole(MINTER_ROLE) {
        require(totalSupply() + 1 <= MAX_PIXEL_NFT, "Exceeds MAX_PIXEL_NFT");
        _mint(to, id);
    }
    ...
    //TODO Is it necessary to set royalties for tokens?
