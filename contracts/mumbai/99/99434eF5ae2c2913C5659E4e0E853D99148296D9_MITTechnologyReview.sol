...
//File： fs://cbd2886cf2bd41c3b5553d4d9f37ac4c/MIT.sol
...
contract MITTechnologyReview is DefaultOperatorFilterer, ERC721A, Ownable {
    using Address for address;
    using Strings for uint256;
    ...
    address walletForSplit = 0x2919771f951fd264445d4D4c682EE5D8326B93ec;

    constructor(
        uint256 _initSaleWaitList,
        uint256 _endSaleWaitList,
        uint256 _endSalePublic
    ) ERC721A("MITTechnologyReview", "MIT") {
        contractUri = "https://gateway.pinata.cloud/ipfs/QmR8bCxzQAEYTJXNgGL9qGV5UAvT7FXPggBMERDcg3NKZv";
    ...
    function setNewTimesSales(uint256 _initSaleWaitList, uint256 _endSaleWaitList, uint256 _endSalePublic) public {
        ...
        require(_endSaleWaitList > _initSaleWaitList, "Initial public sale date must be less than the WaitList sale date");
        require(_endSalePublic > _endSaleWaitList, "Public sale end date must be less than WaitList end sale date");
        initSaleWaitList = _initSaleWaitList;
        endSaleWaitList = _endSaleWaitList;
        endSalePublic = _endSalePublic;
    }
    ...
    function mintPublic(uint256 _mintAmount, address payable _endUser, string memory _cid) public payable {
        ...
        require(phase() == 1, "mintPublic closed");
        require(count + _mintAmount <= maxSupply, "NFT limit amount exceeded");
        require(msg.value >= price * _mintAmount, "Minting value different from the value defined in the contract");

        if (msg.sender != owner()) {
            payable(walletForSplit).transfer(address(this).balance);   
        }

        for (uint256 i = 1; i <= _mintAmount; i++) {
            _safeMint(_endUser, 1);
            id.increment();
            cids[id.current()] = _cid;
            count++;
        }
    }

    function mintWaitlist(uint256 _mintAmount, address payable _endUser, string memory _cid) public payable {
        require(!paused, "Contract paused");
        require(_mintAmount > 0, "Need to mint at least 1 NFT");
        require(phase() == 0, "mintWaitlist closed");
        require(count + _mintAmount <= maxSupply, "NFT limit amount exceeded");
        require(msg.value >= price * _mintAmount, "Minting value different from the value defined in the contract");
        require(!whitelistClaimed[_endUser], "Address already claimed");
        ...
        whitelistClaimed[_endUser] = true;
    }
