...
contract AggregateBribe {
    ...
    */
    ...
    function notifyRewardAmount(address token, uint amount) external lock {
        require(amount > 0);
        require(IBribeAssetRegistry(bribeAssetRegistry).isWhitelisted(token), "bribe tokens must be whitelisted");
...
interface IBribeFactory {
    function createBribe(address _bribeAssetRegistry) external returns (address);    
}
...
// vote for DToken for receiving KZA emission on BaseRewardPool
contract Voter is Ownable {

    uint internal constant DURATION = 7 days; // rewards are released over 7 days
    address public bribefactory; // deploy external bribe
    ...
    event Voted(address indexed voter, address pool, uint256 weight, uint256 epoch);
    event MarketBribeCreated(address market, address bribe);
    event Abstained(address voter, uint256 weight, uint256 epoch);
    event SetDelegation(address voter, address delegatee);
    ...
    function _vote(address _account, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        usedWeights[_account] = _totalWeight;
    }
    ...
    /// @param _account the account that has claimable
    /// @param _to the recipient
    function claimBribesExternal(address[] memory _bribes, address[][] memory _tokens, address _account, address _to) external {
        require(_bribes.length > 0 && _tokens.length > 0);
        address bribe;
        for (uint i = 0; i < _bribes.length; i++) {
            bribe = _bribes[i];
            require(bribe != address(0));
            IBribe(bribe).getRewardForOwner(_tokens[i], _account, _to);
        }
    }
    /// @notice in internal bribe each bribe address comes with 1 token and there is no need to specify
    /// @param _bribes each address of the bribe deployment
    /// @param _account the account that has claimable
    /// @param _to the recipient
    function claimBribesInternal(address[] memory _bribes, address _account, address _to) external {
        require(_bribes.length > 0);
        address bribe;
        for (uint i = 0; i < _bribes.length; i++) {
            bribe = _bribes[i];
            require(bribe != address(0));
            IBribe(bribe).getRewardForOwner(_account, _to);
        }
    }

    function _createBribe(address owner, address registry) internal returns(address bribe) {
        bribe = address(new AggregateBribe(owner, registry));
    }
}
