...
contract HolderPassERC721 is ERC721, AccessControl, Ownable{
    ...
    function mintPass(uint256 mitamaTokenId, address to) external onlyRole(MINTER_ROLE) {
        if(mitamaTokenId > 10000) revert InvalidTokenId();
    ...
    /**
     * Setter functin of token metadata
     */
    ...
    function setPassFile(string memory _description) public onlyRole(SETTER_ROLE){
        description = _description;
    }

    function setPassName(string memory _description) public onlyRole(SETTER_ROLE){
        description = _description;
    }
