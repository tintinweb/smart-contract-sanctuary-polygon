...
contract Whitelist is AccessControl {
     ...
     */
    function addAddress(address _address) public onlyRole(SUPER_ADMIN) {
        require(
    ...
    function removeAddress(address _address) public onlyRole(SUPER_ADMIN) {
        require(
    ...
    Whitelist
{
    ...
    event TokenOwnerShipTransffered(address _from,address _to,uint tokenId);
    event TrustedForwarderChanged(
        address indexed newForwarderAddress,
    ...
    constructor(
        ...
        _owner = _msgSender();
        ERC20Forwarder = trustedForwarder;
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _setupRole(SUPER_ADMIN, _msgSender());
     ...
     */
    function resetTokenRoyalty(uint256 _tokenId) public onlyRole(SUPER_ADMIN) {
    ...
    /**
        Modifier-->Check if the address is not equal to zero address
        CheckZero
     */
     modifier CheckZero(address _inp){
         require(_inp !=address(0));
         _;
     }
     /**
        modifier--->caller needs to be ERC20 Forwarder
      */
      modifier CheckForwarder()
      {
          require(msg.sender == ERC20Forwarder);
          _;
      }
    /**
     * @dev Deletes the default royalty.
     */
    function deleteDefaultRoyalty() public onlyRole(SUPER_ADMIN) {
    ...
    function safeTransferFrom(
     ...
     /*
        Change the Owner and admin address
    **/
    function transferOwnership(address _to)public onlyRole(SUPER_ADMIN) CheckZero(_to)
    {
        _owner = _to;
        _setupRole(SUPER_ADMIN, _to);//it is not transfer Ownership ,it is just adding the superadmin 
        emit TransferOwnership(_msgSender(),_to);
    }
    /**
       Change Default Ownership
     */
     function ChangeDefaultAdminRole(address _to)public onlyRole(DEFAULT_ADMIN_ROLE) CheckZero(_to)
     {
         _setupRole(DEFAULT_ADMIN_ROLE, _to);

     }
    /**
        Change the ERC20Forwarder
    */
    function ChangeERC20Forwarder(address _forwarder)public onlyRole(SUPER_ADMIN)CheckZero(_forwarder)
    {
        ERC20Forwarder=_forwarder;
    }
    /**
     * @dev Transfer token Ownership.
     *only admin can call the function
     */
    function transferTokenOwnership(uint256 tokenId, address newOwner)
        external
        onlyRole(SUPER_ADMIN)
    {
        _holderTokens[_msgSender()].remove(tokenId);
        _holderTokens[newOwner].add(tokenId);
        super._transfer(_msgSender(), newOwner, tokenId);
        emit TokenOwnerShipTransffered(_msgSender(), newOwner, tokenId);
    }

    // /**
    //  * @dev get token Ownership with sign.anyone can call ,and who ever calling should need to sign the transaction 
    //  */
    // function getTokenOwnership(
    //     uint256 tokenId,
    //     address to
    // ) external CheckForwarder {
       
    //     _holderTokens[_owner].remove(tokenId);
    //     _holderTokens[to].add(tokenId);
    //     super._transfer(_owner, to, tokenId);
    //     emit TokenOwnerShipTransffered(_owner, to, tokenId);
    // }
