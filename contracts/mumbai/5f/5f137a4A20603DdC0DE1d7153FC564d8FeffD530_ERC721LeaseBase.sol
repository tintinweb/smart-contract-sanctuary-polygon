...
constructor() ERC721 ("Lease", "LEASE") {
  ...
  event AssetCreated(
    ...
    mapping(address => mapping (uint => uint[])) public leasesByAddress;
    ...
    function getAssets(uint _tokenId) public view returns (string memory) {
...
function isLeaseAvailable(uint256 _tokenId, uint256 _start,
        uint256 _end, uint _now) public view returns (bool) {
        if(_now == 0) {
            _now = block.timestamp;
        }
        require(_end > _start);
        require(_end.sub(_start).mul(86400) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start).mul(86400) >= MIN_LEASE_DURATION, "Lease duration is too short");
        uint256 currentContractDate = (_now.sub(TIME_START)).div(86400);
        require( _end >= _start, "End date should be later than start");
        require (_start >= currentContractDate, "Start date cannot be in the past");

        if(_start > lastEndTimeByToken[_tokenId]) {
            return true;
        }

        for(uint j=_start; j<=_end; j++) {
    ...
    function lease(uint256 _tokenId, uint256 _start, uint256 _end) public {
        require(_end > _start);
        require(_end.sub(_start).mul(86400) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start).mul(86400) >= MIN_LEASE_DURATION, "Lease duration is too short");
        require(_tokenId != 0);
        ...
        if (terms.length != 0) {
            leaseAvailable = isLeaseAvailable(_tokenId, _start, _end, block.timestamp);
            require(leaseAvailable, "Lease not available");
        }

        uint startDate = _start.mul(1 days).add(TIME_START);
        uint endDate = _end.mul(1 days).add(TIME_START);
        ...
        if (leaseAvailable) {
          ...
          leasesByToken[_tokenId].push(Term(msg.sender, _tokenId, startDate, endDate));
        ...
        if (lastEndTimeByToken[_tokenId] < _end) {
          ...
          function leaseOnTransfer(uint256 _tokenId, uint256 _start, uint256 _end, address _addressTo, uint _oldStart, uint _oldEnd) internal {
        require(_end > _start);
        require(_end.sub(_start).mul(86400) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start).mul(86400) >= MIN_LEASE_DURATION, "Lease duration is too short");
        require(_tokenId != 0);
        ...
        if (terms.length != 0) {
            leaseGood = isLeaseAvailable(_tokenId, _start, _end, block.timestamp);
        }

        uint startDate = _start.mul(1 days).add(TIME_START);
        uint endDate = _end.mul(1 days).add(TIME_START);

        uint oldStart = _oldStart.sub(TIME_START).div(86400);
        uint oldEnd = _oldEnd.sub(TIME_START).div(86400);
        ...
        if (leaseGood) {
          ...
          leasesByToken[_tokenId].push(Term(_addressTo, _tokenId, startDate, endDate));
          leasesByAddress[_addressTo][_tokenId].push(terms.length);

           uint newStart = _start.sub(1);
          uint newEnd = _end.add(1);
   ...
   function transferLease(uint256 _tokenId, uint256 _start, uint256 _end, address _addressTo) external payable {
          ...
          Term memory term = getLease(_tokenId, _start.mul(86400).add(TIME_START));

            require(term.lessee == msg.sender, "Address does not have rights to this lease");
          require(term.endTime >= _end, "Lessee does not lease past end date");

          uint oldStart = term.startTime;
          uint oldEnd = term.endTime;

          uint oldStartDays = term.startTime.sub(TIME_START).div(86400);

          unlease(_tokenId, oldStartDays);
          leaseOnTransfer(_tokenId, _start, _end, _addressTo, oldStart, oldEnd);
        }

    function unlease(uint256 _tokenId, uint _dayInLease) public payable {
        require(leasesByToken[_tokenId].length > 0, "No terms exist for this lease");

        address lessee = lesseeOf(_tokenId, _dayInLease.mul(86400).add(TIME_START));
        require(lessee == msg.sender, "Address does not have rights to this lease");
          
          uint[] memory leases = leasesByAddress[msg.sender][_tokenId];

        uint tempStart = getLeaseStart(_tokenId, _dayInLease);
        uint tempEnd = getLeaseEnd(_tokenId, _dayInLease);
        ...
        for (uint i=tempStart; i<=tempEnd; i++) {
...
function mintAsset(string memory _name, string memory _description, string memory _image) external payable {
    _tokenIds.increment();
