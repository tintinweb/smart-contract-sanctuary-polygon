...
/// @dev Custom errors For Verified Address feature.
...
/// @dev Custom errors for Stake & Reward distribution.
/// STAKE
error FailedToTransfer();
error AmountCannotBeZero();
error TokenIsNotInTheList();
error TokenAlreadyInTheList();
error AmountIsSameAsPrevious();
error TokenAddressCannotBeZero();
error TokenIsNotInTheStakingTokenList();
error InvestmentAmountShouldNotBeZero();
error InvestmentAmountShouldBeMultipleOf100();

/// Distribute Reward
error NoActiveStakeToReward();
error RewardPercentCannotBeZero();
error SameRewardPercentAsPrevious();
error DenominatorShouldBeMultipleOf100();
error YouHaveToWaitTillSevenDaysToDistributeRewardsAgain();
    ...
    ReentrancyGuardUpgradeable
{
     ...
     * @param investor: The investor address who staked into this contract.
     * @param amount: The amount investor staked into this contract.
     */
    event Staked(address investor, uint256 amount);
    ...
    function stake(
        ...
        if (!isTokenAlreadyInList[_tokenAddress])
            revert TokenIsNotInTheStakingTokenList();

        /// @dev Getting the amount of USDT and Tokens to stake
        (
            uint256 amountOfUSDT,
            uint256 amountOfTokens
        ) = _getAmountOfBothStakingTokens(_tokenAddress, _amount);
        ...
        if (amountOfUSDT > 0 && amountOfTokens > 0) {
            ...
            bool success2 = IERC20(_tokenAddress).transferFrom(
                msg.sender,
                address(this),
                amountOfTokens
            );
            if (!success2) revert FailedToTransfer();
        }
    ...
    function distribute(uint256 batchSize) external onlyOwner nonReentrant {
        /// @dev 7 Days checking.
        if (block.timestamp < (lastRewardDistributedOn + 7 days))
            revert YouHaveToWaitTillSevenDaysToDistributeRewardsAgain();
        ...
        if (
            totalActiveStakes == 0 ||
            lastCompletedStake == (totalActiveStakes - 1)
        ) revert NoActiveStakeToReward();
        ...
        lastProcessedBatchTill = (index == totalActiveStakes) ? 0 : index;
    ...
    function updateRewardPercentage(
        ...
        if (_newRewardPercent == 0) revert RewardPercentCannotBeZero();
        if (!(_denominator % 100 == 0))
            revert DenominatorShouldBeMultipleOf100();
        ...
        ) revert SameRewardPercentAsPrevious();
    ...
    function addToken(
        ...
        if (isTokenAlreadyInList[_tokenAddress]) revert TokenAlreadyInTheList();
        if (_usdPrice == 0) revert AmountCannotBeZero();
    ...
    function removeToken(
        ...
        if (!isTokenAlreadyInList[_tokenAddress]) revert TokenIsNotInTheList();
    ...
    function setPrice(
        ...
        if (_newPrice == 0) revert AmountCannotBeZero();
        if (!isTokenAlreadyInList[_tokenAddress]) revert TokenIsNotInTheList();

        /// @dev checking if the token price is same as before.
        if (tokenToPrice[_tokenAddress] == _newPrice)
            revert AmountIsSameAsPrevious();
    ...
    function _validateAmountIsMultipleOf100(uint256 _amount) private pure {
        /// @dev Parameter checking.
        if (_amount == 0) revert InvestmentAmountShouldNotBeZero();
        /// @dev Amount checking ig it is a multiple of 100 VYNC or not.
        if (!((_amount % 100) == 0))
            revert InvestmentAmountShouldBeMultipleOf100();
    }
