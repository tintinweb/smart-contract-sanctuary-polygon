...
// File contracts/interfaces/IWrapperToNative.sol
...
// File contracts/IFOUpgradeable.sol
...
contract IFOUpgradeable is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
	...
	function updateDetails(
		IfoDetails memory detail
	) external notInitialized {
		depositToken = detail._depositToken;
		offeringToken = detail._offeringToken;
		startTime = detail._startTime;
		endTime = detail._endTime;

		offeringAmount = detail._offeringAmount;
		raisingAmount = detail._raisingAmount;
		devFees = detail._devFees;
		protocolFees = detail._protocolFees;
		escrowRatio = detail._escrowRatio;

		devFeesAddress = detail._devFeesAddress;
		protocolFeesAddress = detail._protocolFeesAddress;
		escrowAddress = detail._escrowAddress;
		receiverAddress = detail._receiverAddress;

		lastRewardTime = detail._endTime;

		escrowDuration = detail._escrowDuration;
		vestingDuration = detail._vestingDuration;
	...
	function timelyWithdraw() external {
		...
		if (!sentToEscrow) {
			...
			if (distributionAmount < offeringAmount && offeringToken.balanceOf(address(this)) > 0) {
				offeringToken.safeTransfer(receiverAddress, offeringAmount.sub(distributionAmount));
			}
...
contract IFOFactory {
   IFOUpgradeable[] public IFOArray;

   function CreateIfo(IWMATIC _WMATIC, IWrapperToNative _nativeConvertor) public {
     IFOUpgradeable ifo = new IFOUpgradeable();
     ifo.initialize(_WMATIC,_nativeConvertor);
     IFOArray.push(ifo);
   }

   function IfoSetter(uint256 index,IFOUpgradeable.IfoDetails memory detail) public {
     IFOUpgradeable(address(IFOArray[index])).updateDetails(detail);
   }
