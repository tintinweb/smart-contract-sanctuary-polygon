...
contract mQuark is ImQuark, ERC721, IERC2981, DefaultOperatorFilterer, Ownable {
  ...
  function mintWithPreURI(
    ...
    _onlymQuarkControl();
    if (templateId == 0) revert InvalidTemplateId();
    if (!_verifyCollectionURIEditSignature(signature, signer, projectId, templateId, collectionId, uri))
...
contract mQuarkControl is ImQuarkControl, ReentrancyGuard, PullPayment, AccessControl {
   ...
   * @param projectId  Slot's project's id
   */
  function addURISlotToNFT(address tokenContract, uint256 tokenId, uint256 projectId) external payable nonReentrant {
    if (_registeredProjects[projectId].id == 0) revert GivenProjectIdNotExist();
  ...
  function createCollections(
    ...
    for (uint256 i = 0; i < _templatesLength; ) {
      if ((createParams.collectionPrices[i] < _templateMintPrices[createParams.templateIds[i]]) && (createParams.collectionPrices[i] != 0))
      ...
      _collectionPrices[projectId][createParams.templateIds[i]][createParams.collectionIds[i]] = createParams.collectionPrices[i];
  ...
  function registerExternalCollection(
    address externalCollectionContract,
    uint256 templateId
