...
contract BSGBinance {
    ...
    uint256 private constant timeStep =15*30*60;

    uint256 private constant dayPerCycle = 15*30*60; 
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount){
                ...
                uint256 staticReward = order.amount.mul(dayRPercents).mul(dayPerCycle).div(30).div(baseDivider);
                if(isFreezeReward){
    ...
    function checkGwei(address _gwei) external {
