...
uint256 constant BOSS_BATTLE_COOLDOWN_ID = uint256(
    keccak256("boss_battle_cooldown_id")
);
...
uint256 constant BOSS_COMBAT_MAX_AMOUNT_MOVES = 10;
uint256 constant BOSS_COMBAT_ACCURACY_DICE_ROLL = 200;

// Time limit for Boss battles : set to 12 hours
...
/// @notice Input param for _validateEndBattlePartial
struct ValidateEndBattlPartialParams {
    uint256[] moves;
...
contract BossBattleSystem is CoreBattleSystem {
    ...
    /// @notice Mapping to store wallet address > battleEntity
    mapping(address => uint256) private _walletToBattleEntity;
    ...
    /// @notice Wallet is still in cooldown
    error WalletStillInCooldown();
    ...
    /**
     * @dev Returns cooldown timestamp for Wallet
     * @return timestamp epoch time in seconds when cooldown is refreshed
     */
    function getWalletCooldown(address wallet) external view returns (uint32) {
        return
            ICooldownSystem(_getSystem(COOLDOWN_SYSTEM_ID)).getCooldown(
                EntityLibrary.addressToEntity(wallet),
    ...
    /**
     * @dev Returns an active battle if one exists
     * @param wallet address to look up an active battle by
     * @return battle a Battle struct containing data for a battle
     * @return isActive a boolean that is true if the battle is still active
     */
    function getActiveBattleByWallet(
        address wallet
    ) external view returns (Battle memory, bool isActive) {
        Battle memory battle = _getBattle(_walletToBattleEntity[wallet]);
        // BattleEntity cooldown == current time is still before battle expires
        bool beforeCooldownTime = ICooldownSystem(
            _getSystem(COOLDOWN_SYSTEM_ID)
        ).isInCooldown(battle.battleEntity, BOSS_BATTLE_COOLDOWN_ID);
    ...
    /**
     * @dev Create a battle if it doesnt exist for this wallet
    ...
    function startBattle(
        ...
        _deleteBattle(_walletToBattleEntity[account]);
        ...
        _walletToBattleEntity[account] = battleEntity;
        return battleEntity;
    }
     ...
     */
    function endBattle(EndBattleParams calldata params) external nonReentrant {
        // Check account is executing their own battle || battle entity != 0
        address wallet = _getPlayerAccount(_msgSender());
        if (
            _walletToBattleEntity[wallet] != params.battleEntity ||
        ...
        if (
            params.moves.length > BOSS_COMBAT_MAX_AMOUNT_MOVES ||
        ...
        bool isKillShot;
        // If bossStartingHealth != 0 && totalDamageDealt >= bossStartingHealth then killshot has been dealt
        if (
            bossStartingHealth != 0 &&
            params.totalDamageDealt >= bossStartingHealth
        ) {
            isKillShot = true;
            // Record finalblow data
        ...
        _validateEndBattlePartial(
            ValidateEndBattlPartialParams(
                params.moves,
                battle.attackerEntity,
                battle.defenderEntity,
        ...
        // Clear battle record
        _clearBattleEntity(wallet);
    }

    /** INTERNAL **/

    /**
     * @dev Simple threshold check
     * @param params Damage the ship did to the boss
     */
    function _validateEndBattlePartial(
        ValidateEndBattlPartialParams memory params
    ) internal view {
        // Get TokenTemplate system
        ITokenTemplateSystem tokenTemplateSystem = ITokenTemplateSystem(
            _getSystem(TOKEN_TEMPLATE_SYSTEM_ID)
        );
        // Get boss affinity from TokenTemplate system
        (address tokenContract, uint256 tokenId) = EntityLibrary.entityToToken(
            params.defenderEntity
        );
        uint256 bossAffinity = tokenTemplateSystem.getTraitUint256(
            tokenContract,
            tokenId,
        ...
        // get affinity for pirate vs boss, cast to int256
        int256 affinityModifier = SafeCast.toInt256(
            AffinitySystem(_getSystem(AFFINITY_SYSTEM_ID)).getDamageModifier(
        ...
        // get ship base damage from TokenTemplate
        (tokenContract, tokenId) = EntityLibrary.entityToToken(
            params.attackerEntity
        );
        int256 shipBaseDamage = tokenTemplateSystem.getTraitInt256(
            tokenContract,
            tokenId,
            DAMAGE_TRAIT_ID
        );

        CoreMoveSystem moveSystem = CoreMoveSystem(
        ...
        for (uint i = 0; i < params.moves.length; ++i) {
            moveMods = moveSystem.getCombatModifiers(params.moves[i]);
            totalDamageCalculated +=
                ((shipBaseDamage + moveMods[0]) * affinityModifier) /
                10000;
        }
        // Ship cannot do this much damage, revert
    ...
    /**
     * @dev Put a cooldown on Wallet, Ship, and BattleEntity (id)
     */
    function _requireValidCooldowns(
        ...
        // Apply cooldown on wallet, revert if still in cooldown
        if (
            cooldown.updateAndCheckCooldown(
                EntityLibrary.addressToEntity(account),
                BOSS_BATTLE_COOLDOWN_ID,
                BOSS_BATTLE_COOLDOWN_TIME
            )
        ) {
            revert WalletStillInCooldown();
        }
    ...
    function _getBattleEntity(address wallet) internal view returns (uint256) {
        return _walletToBattleEntity[wallet];
    }

    function _clearBattleEntity(address wallet) internal {
        _deleteBattle(_walletToBattleEntity[wallet]);
        delete (_walletToBattleEntity[wallet]);
    }
...
abstract contract CoreBattleSystem is GameRegistryConsumerUpgradeable {
    ...
    function _createBattle(
        ...
        // Initialize battle
        Battle storage battle = _battles[battleEntity];
        battle.battleEntity = battleEntity;
        battle.attackerEntity = attackerEntity;
        battle.defenderEntity = defenderEntity;
        battle.attackerOverloads = attackerOverloads;
        battle.defenderOverloads = defenderOverloads;
        battle.attackerCombatable = attackerCombatable;
        battle.defenderCombatable = defenderCombatable;

        // Instead of VRF callback, store battleSeed from client.
        // This is less secure because seed is unverified, but a better UX.
        battle.battleSeed = battleSeed;

        // Request VRF randomness for battle
        // uint256 requestId = _requestRandomWords(1);
        // _requestToBattleEntity[requestId] = battle.battleEntity;
...
enum MoveTypes {
    ...
    POWER_STRIKE,
    QUICK_SHOT,
    length // This must remain as last member in enum
}
...
contract CoreMoveSystem is GameRegistryConsumerUpgradeable, IMoveSystem {
    ...
    /**
     * Takes a moveId and returns stat modifiers in CombatStats format
     * @param moveId move identifier to lookup modifiers for
     * @return int256[] stat modifiers for provided moveId
     */
    function getCombatModifiers(uint256 moveId)
        external
        view
        override
        returns (int256[] memory)
    {
        ...
        IGameGlobals gameGlobals = IGameGlobals(_getSystem(GAME_GLOBALS_ID));
        return gameGlobals.getInt256Array(_moveIdToGlobal[moveId]);
    }
}
...
import {ID as PIRATE_NFT_ID} from "../tokens/PirateNFT.sol";
import {ITokenTemplateSystem, ID as TOKEN_TEMPLATE_SYSTEM_ID} from "../tokens/ITokenTemplateSystem.sol";
...
contract ShipCombatable is Combatable {
    ...
    function getCombatStats(
        ...
        // TODO: insert ShipSlotSystem.getCombatStats(entityId)

        // Retrieves combat stat modifiers from move system
    ...
    function canAttack(
        ...
        if (caller != IGameNFT(contractAddress).ownerOf(tokenId)) {
...
contract BossBattleSystemMock is BossBattleSystem {
    ...
    function rewindWalletCooldown(uint32 rewindTime) external {
        ICooldownSystem cooldown = ICooldownSystem(
            _getSystem(COOLDOWN_SYSTEM_ID)
        );
    ...
    /**
     * @dev Simple validation end-battle call
     * @param moves Set of move ids the ship made in order
     * @param totalDamageTaken Damage the ship sustained aka amount of damage boss did
     * @param totalDamageDealt Damage the ship did to the boss
     */
    function endBattleMock(
        ...
        address wallet = _getPlayerAccount(_msgSender());
        if (_getBattleEntity(wallet) != battleEntity || battleEntity == 0) {
            revert InvalidCallToEndBattle();
        }
        // Check if call to end-battle still within battle time limit
        if (
        ...
        if (moves.length > BOSS_COMBAT_MAX_AMOUNT_MOVES || moves.length == 0) {
            revert InvalidEndBattleParams();
        }
        ...
        bool isKillShot;
        // If bossStartingHealth != 0 && totalDamageDealt >= bossStartingHealth then killshot has been dealt
        if (bossStartingHealth != 0 && totalDamageDealt >= bossStartingHealth) {
            isKillShot = true;
            // Record finalblow data
            bossEntityToFinalBlow[battle.defenderEntity] = FinalBlow(
                battle.attackerEntity,
        ...
        // if (useFullValidationFlag) {
        //     if (bossStartingHealth > 0) {
        //         // Calculate full damage taken / damage dealt result.
        //         // Can ignore if boss already dead & final attacks coming through
        ...
        //             ValidateFullParams({
        //                 moves: moves,
        //                 seed: battle.battleSeed,
        //                 totalDamageTaken: totalDamageTaken,
        //                 totalDamageDealt: totalDamageDealt,
        //                 shipHealth: shipStartingHealth,
        ...
        //     }
        // } else {
        //     Simple threshold check, no calculations
        //     _validateEndBattlePartial(
        //         ValidateEndBattlPartialParams(
        //             moves,
        //             battle.attackerEntity,
        ...
        // _updateBossBattleCount(wallet, battle.defenderEntity, totalDamageDealt);
