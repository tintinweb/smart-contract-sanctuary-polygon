...
contract CrookedSnouts is VRFConsumerBaseV2, ERC721URIStorage {
  ...
  function fulfillRandomWords(uint256 _requestId, uint256[] memory randomNumbers) internal override {
    ...
    uint256 bg = (randomNumbers[0] % 7) + 1;
    uint256 face = (randomNumbers[1] % 8) + 1;
    uint256 eyes = (randomNumbers[2] % 11) + 1;
    uint256 head = (randomNumbers[3] % 11) + 1;
    uint256 mouth = (randomNumbers[4] % 11) + 1;
    uint256 nose = (randomNumbers[5] % 7) + 1;
      

    create(bg, face, eyes, head, mouth, nose);
  }
