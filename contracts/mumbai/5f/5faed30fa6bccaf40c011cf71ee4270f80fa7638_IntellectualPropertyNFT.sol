...
interface EndorsedDocs {
    ...
    function mint(address _to, bytes32 _documentHash) external;
...
contract IntellectualPropertyNFT is EndorsedDocs, SilentSigner, ERC5554, EIP5453, ERC5289Library {
    ...
    function mint(address _to, bytes32 _documentHash) public {
    ...
    function tokenURI(bytes32 _tokenId) public view returns (string memory) {
        return string(abi.encodePacked(_ownerOf[_tokenId], "/", string(abi.encodePacked(_tokenId))));
    }
