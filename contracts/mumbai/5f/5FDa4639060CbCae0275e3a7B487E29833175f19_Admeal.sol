...
contract Admeal is ERC721Enumerable, Ownable {
    ...
    bytes32 public partnerWhitelistedMerkleRoot;

    constructor() ERC721("Admeal", "ADMEAL") {}
    ...
    function mint(uint256 numberOfTokens, bytes32[] calldata merkleProof) external payable {
        ...
        require(msg.value == numberOfTokens * MINT_PRICE, "Incorrect Ether value");
    ...
    // Mint a single NFT for whitelisted partners
    function mintPartner(bytes32[] calldata merkleProof) external {
        require(balanceOf(msg.sender) == 0, "Unable to mint over 1");
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            MerkleProof.verify(merkleProof, partnerWhitelistedMerkleRoot, leaf),
            "Invalid Merkle proof"
        );
    ...
    // Set the Merkle root for partner whitelisted addresses
    function setPartnerWhitelistedMerkleRoot(bytes32 root) external onlyOwner {
        partnerWhitelistedMerkleRoot = root;
    }
}
