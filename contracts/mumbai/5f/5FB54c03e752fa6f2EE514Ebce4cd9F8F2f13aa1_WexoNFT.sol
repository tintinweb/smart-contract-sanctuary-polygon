...
contract WexoNFT is ERC1155, Ownable {
   ...
   */
    modifier creatorOnly(uint256 _id) {
        require(creators[_id] == msg.sender, "WexoNFT#creatorOnly: ONLY_CREATOR_ALLOWED");
        _;
    }
   ...
   */
    modifier ownersOnly(uint256 _id) {
        require(balanceOf(msg.sender, _id) > 0, "WexoNFT#ownersOnly: ONLY_OWNERS_ALLOWED");
        _;
    }
    ...
    * @param _metadataUri Optional metadata URI for this token type (either URI or JSON must be set)
    ...
    */
    function create(
        ...
        _metadataUris[_id] = _metadataUri;
    ...
    */
    function setCreator(
        ...
        require(_to != address(0), "WexoNFT#setCreator: INVALID_ADDRESS.");
    ...
    */
    ...
    /**
     * @dev Returns metadata JSON if URI is encoded as base64, otherwise returns original URI
    */
    function metadata(uint256 tokenId) public view returns (string memory) {
        string memory _uri = uri(tokenId);
        string memory _prefix = "data:application/json;base64,";
        if (bytes(_uri).length > bytes(_prefix).length) {
            string memory _uriPrefix = _substring(_uri, 0, bytes(_prefix).length);
            if (_compareStrings(_prefix, _uriPrefix)) {
                string memory _base64 = _substring(_uri, bytes(_prefix).length, bytes(_uri).length);
                return Base64.decode(_base64);
            }
        }
        return _uri;
    }
