...
contract HappBridge is ERC20, Ownable {
    ...
    function bridgeEthToBtc(string memory btcAddress,address destinationCoin, uint256 amount) public {
        ...
        uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, block.basefee, block.coinbase)));
        string memory orderId = string(abi.encodePacked(block.timestamp, "-", randomNumber));
