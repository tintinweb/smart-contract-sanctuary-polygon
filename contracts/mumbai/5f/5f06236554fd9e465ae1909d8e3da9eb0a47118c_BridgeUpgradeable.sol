...
contract BridgeUpgradeable is Initializable, OwnableUpgradeable, RouterCrossTalkUpgradeable {
    ...
    function crossChainTransferOut(
        ...
        uint256 transferIndex = currentTransferIndex[_tokenTicker][_msgSender()][_toChainId];
        ++currentTransferIndex[_tokenTicker][_msgSender()][_toChainId];
    ...
    function createCrossChainTransferMapping(
        ...
        require(_transferIndex == currentTransferIndex[_tokenTicker][_userAddress][chainId], "INVALID_TRANSFER_INDEX");
        ...
        transferMapping[_tokenTicker][_userAddress][chainId][currentTransferIndex[_tokenTicker][_userAddress][chainId]++] = _noOfTokens;

        emit BridgeTransactionEnd(_transferIndex, chainId, _userAddress, _tokenTicker);
    }
    ...
    function crossChainTransferIn(
        ...
        require(transferMapping[_tokenTicker][_msgSender()][chainId][_index] > 0, "TRANSFER_MAPPING_NOT_EXISTS");
        require(_noOfTokens <= transferMapping[_tokenTicker][_msgSender()][chainId][_index], "EXCESS_TOKENS_REQUESTED");
        ...
        // }
        transferMapping[_tokenTicker][_msgSender()][chainId][_index] -= _noOfTokens;
        if(transferMapping[_tokenTicker][_msgSender()][chainId][_index] == 0) {
            delete transferMapping[_tokenTicker][_msgSender()][chainId][_index];
        }

        transferOut(_noOfTokens, _tokenTicker);

        emit BridgeTransactionEnd(_index, chainId, _msgSender(), _tokenTicker);
    }
...
contract FeePoolUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function getLPsFee(
        ...
        if((epochIndex == 1 && epochsLength == 0) || epochIndex == epochsLength + 1) {
            noOfDepositors = tokenBridgeRegistryUpgradeable.getNoOfDepositors(tokenTicker);
        } else {
            noOfDepositors = bridgeUpgradeable.getEpochTotalDepositors(tokenTicker, epochIndex);
        }
    ...
    function getAdminTokenFees(
        ...
        for (uint256 epochIndex = adminClaimedTillEpoch[_tokenTicker] + 1; epochIndex <= epochsLength; epochIndex++) {
            uint256 noOfDepositors = bridgeUpgradeable.getEpochTotalDepositors(_tokenTicker, epochIndex);
            if(noOfDepositors == 0) {
                continue;
            }
