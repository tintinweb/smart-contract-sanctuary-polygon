...
contract LOTTOGame is VRFConsumerBaseV2, ConfirmedOwner {
    ...
    function claimJackpot(uint256 gameId,uint256 _nftId) external
    {
        ...
        emit EarningEvent(amount, msg.sender, address(0),0, block.timestamp);
             emit JackpotClaimed(
        msg.sender,
        amount,
        gameId,
        ...
        if(ref!=address(0)){
            ...
            IERC20(busd).transfer(ref, refamount);
            emit EarningEvent(refamount, ref, msg.sender,0, block.timestamp);
        }
