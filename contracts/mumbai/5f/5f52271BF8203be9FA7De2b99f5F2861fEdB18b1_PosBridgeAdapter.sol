...
contract Worker is Ownable {
	...
	function pushTransfer() external
	{
		require(!msg.sender.isContract(), "Can be called only by external account");
		require(isSent, "sendAssets called before");
		...
		{
			IChildErc20(_asset).withdraw(_amount);
		}
	}
}
