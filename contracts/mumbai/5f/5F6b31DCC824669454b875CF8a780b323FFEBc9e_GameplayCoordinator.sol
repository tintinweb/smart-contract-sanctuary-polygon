...
contract GameplayCoordinator is IGameplayCoordinator, Controllable {
    IPxlbot pxlbot;
    mapping(uint256 => address) busyBots;
    
    constructor(IPxlbot _pxlbot) Controllable() {
        pxlbot = _pxlbot;
    }

    function isBotBusy(uint256 botId) public override view returns(bool) {
        return busyBots[botId] != address(0);
    }

    function makeBotBusy(uint256 botId) public override onlyController inGameOnly(botId) {
        require(!isBotBusy(botId), 'GameplayCoordinator: bot is busy');
        busyBots[botId] = _msgSender();
    }

    function makeBotUnbusy(uint256 botId) public override onlyController {
        require(_msgSender() == busyBots[botId], 'GameplayCoordinator: bot is busy with different address');
        delete busyBots[botId];
    }

    function isBotInGame(uint256 botId) public override view returns(bool) {
        return pxlbot.isTokenInPlay(botId);
    }

    modifier inGameOnly(uint256 botId) {
        require(isBotInGame(botId), 'GameplayCoordinator: bot is not in game');
        _;
    }
}
