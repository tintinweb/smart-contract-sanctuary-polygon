...
contract SubTokenDescriptor is ISubTokenDescriptor {
    ...
    function tokenURI(address merchantToken, address planManager, address subInfoManager, uint256 tokenId) external view override returns (string memory) {
        ...
        uint8 paymentTokenDecimals = IERC20Metadata(plan.paymentToken).decimals();
        ...
        price : DecimalToString.decimalString(plan.pricePerBillingPeriod, paymentTokenDecimals, false)
        });
        ...
        json = string(abi.encodePacked(json, '{"trait_type": "subscription_valid", "value": "', booleanToString(subInfo.nextBillingTime > block.timestamp), '"}'));
