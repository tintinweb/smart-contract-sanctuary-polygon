...
contract ThalloBridgeEvents is UUPSUpgradeable, OwnableUpgradeable {
    ...
    */
    ...
    event TokensIssued(address tcc, address to, uint256 quantity, uint256 eventIndex, uint256 custodyIndex);
    event TokensRetired(address tcc, address to, uint256 quantity, uint256 eventIndex, uint256 retirementId);
    event TokensUncustodied(address tcc, address requestor, uint256 quantity, uint256 eventIndex);
    event ReturnUncusdody(address tcc, address requestor);
    event EventEdited(uint24 projectId, uint16 vintage, uint256 eventIndex, EventEdit edit, uint256[] custodyIndices);
    event CustodianAdded(address custodian);
    event CustodianRemoved(address who);
    ...
    function issueVintageTokens(
        ...
        require(tccAddress != address(0), "TBE:TCC does not exist");
        require(_registryBlock.blockType == RegistryBlockType.CUSTODIED, "TBE:Not custody block");
    ...
    function retireVintageTokens(
        ...
        bytes32 vintageKey = keccak256(abi.encodePacked(_projectId, _vintage));
        address tccAddress = tccInfo[vintageKey].tccAddress;
        require(msg.sender == tccAddress, "TBE:Only TCC");
    ...
    function finalizeRetirement(
        ...
        bytes32 vintageKey = keccak256(abi.encodePacked(_projectId, _vintage));
        require(!bridgeEvents[vintageKey][_eventIndex].finalized, "TBE:Already finalized");
        //slither-disable-next-line reentrancy-no-eth
    ...
    /**
     *@notice Returns carbon credits on chain to
    ...
    function returnUncustody(address _tcc, address _requestor) external onlyCustodian {
        ...
        emit ReturnUncusdody(_tcc, _requestor);
    }
    ...
    function setVintageAddress(bytes32 _vintageKey, address _tccAddress) external {
        require(msg.sender == thalloBridgeRegistry, "TBE:Only registry");
