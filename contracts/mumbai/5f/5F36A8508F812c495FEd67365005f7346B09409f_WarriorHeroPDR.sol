...
contract WarriorHeroPDR is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, AccessControl, ERC721Burnable {
    ...
    constructor(address owner) ERC721("My Warriro Hero For Testing", "MWHFT") {
    ...
    function mint(address to, uint256 rarity)  public onlyRole(MINTER_ROLE) {
        ...
        emit EMintWarriorHero(_tokenId, uri, to, _rarity);
    }
    ...
    function countOwnHero(address owner) public view returns(uint256) {
        return _ownerHeros[owner].current();
    }

    function countHeroDetail() public view returns(uint256, uint256, uint256, uint256) {
