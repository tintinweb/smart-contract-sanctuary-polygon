    ...
    AccessControl
{
    ...
    // events
    event LogVcmMsaIdNotarized(address employer, string msaId, address caseManager);
    ...
    event LogVcmPolicyCreated(bytes32 processId, string msaId, address caseManager);
    ...
    function notarizeMsaId(address employer, string memory msaId)
        external
        onlyRole(CASE_MANAGER_ROLE)
    {
        _msaId[employer] = msaId;

        emit LogVcmMsaIdNotarized(employer, msaId, msg.sender);        
    }


    function hasNotarizedMsaId(address employer) public view returns(bool hasMsaId) {
        return length(_msaId[employer]) > 0;
    }


    function getNotarizedMsaId(address employer) public view returns(string memory msaId) {
        ...
        returns (bytes32 processId) 
    {
        ...
        bytes memory applicationData = encodeApplicationData(employer, caseSubject, patient);
        ...
        onlyRole(CASE_MANAGER_ROLE)
    {
        ...
        caseData.sowDocumentHash = sowDocumentHash;
        ...
        onlyRole(CASE_MANAGER_ROLE)
    {
        ...
        // need to have valid msaid at time of underwriting
        (address employer,,) = decodeApplicationData(application.data);
        ...
        emit LogVcmPolicyCreated(processId, msaId, msg.sender);

        // adjust sum insured to scheduled payouts
        ...
        if(payoutAmount < application.premiumAmount) {
            uint256 premiumAmount = payoutAmount;
            uint256 sumInsuredAmount = premiumAmount + 1;
            _adjustPremiumSumInsured(processId, premiumAmount, sumInsuredAmount);
        }
    ...
    function getClaimAmount(bytes32 processId)
        public
        view 
    ...
    function calculateClaimAmount(ServiceProviderPayment[
    ...
    {
        return abi.decode(data, (address, string, address));
    }
    ...
    {
        return abi.encode(employer, caseSubject, patient);
    }
        ...
        )
    {
        ...
        sowDocumentHash = caseData.sowDocumentHash;
