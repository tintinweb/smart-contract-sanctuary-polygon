...
// to update the split's implementation address for creating split wallets
    ...
    // }
     ...
     function createSongDutch( address _royaltyReceiverSecondary, address royaltyReceiverMint, uint96 _offchainRoyalty, string memory _uri ,string memory _contractURI, uint96 _royaltyFeesInBips, address _artist, uint[5] calldata _configureDutchAuction) public onlyGovernor returns(address) {
...
function isContract(address _addr) view public returns (bool isContractorNot){
  uint32 size;
  assembly {
  ...
  return (size > 0);
    ...
    uint32[] calldata percentAllocations) public onlyGovernor returns(address){
       ...
       address predictedSplit = ISplitMain(splits).predictImmutableSplitAddress(accounts, percentAllocations, 0);
       ...
       if(!isContract(predictedSplit)){
         ...
         emit splitWalletCreated(predictedSplit, splitAddToId[predictedSplit]);
