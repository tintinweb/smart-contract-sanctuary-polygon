...
contract BSBForestUpgradeable is Initializable, BSBBaseGameModeUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    uint public gen1HouseHoldBonusPercentage;
    uint public fullHouseHoldBonusPercentage;
    uint public tokenGeneratorWithBuffBonusPercentage;
    uint public houseHoldBasePercentage;
    uint public houseHoldBaseRisk;
    uint public houseHoldRiskMintPassReduction;
    uint public baseRisk;
    uint public yieldDaysRate;
    uint public treeHouseEnrollYieldBuff;
    uint public mintPassYieldBuff;
    uint public baseStaminaYield;
    uint public mintPassStaminaBuff;
    uint public staminaDaysRate;
    ...
    function __BSBForest_init(
        ...
        tokenGeneratorWithBuffBonusPercentage = 100;
    ...
    function setGen1HouseHoldBonusPercentage(uint value_) external onlyOwner {
        gen1HouseHoldBonusPercentage = value_;
    }

    function setFullHouseHoldBonusPercentage(uint value_) external onlyOwner {
        fullHouseHoldBonusPercentage = value_;
    }

    function setTokenGeneratorWithBuffBonusPercentage(uint value_) external onlyOwner {
        tokenGeneratorWithBuffBonusPercentage = value_;
    }

    function setHouseHoldBasePercentage(uint value_) external onlyOwner {
        houseHoldBasePercentage = value_;
    }

    function setHouseHoldBaseRisk(uint value_) external onlyOwner {
        houseHoldBaseRisk = value_;
    }

    function setHouseHoldRiskMintPassReduction(uint value_) external onlyOwner {
    ...
    function setYieldDaysRate(uint value_) external onlyOwner {
        yieldDaysRate = value_;
    }

    function setTreeHouseEnrollYieldBuff(uint value_) external onlyOwner {
        treeHouseEnrollYieldBuff = value_;
    }

    function setMintPassYieldBuff(uint value_) external onlyOwner {
        mintPassYieldBuff = value_;
    }

    function setBaseStaminaYield(uint value_) external onlyOwner {
        baseStaminaYield = value_;
    }

    function setMintPassStaminaBuff(uint value_) external onlyOwner {
        mintPassStaminaBuff = value_;
    }

    function setStaminaDaysRate(uint value_) external onlyOwner {
    ...
    function stackInHouse(CollectionItems[] calldata collectionItems_, uint256 houseId_) external returns(uint) {
        ...
        for (uint i = 0; i < collectionItems_.length; ++i) {
            ...
            for (uint j = 0; j < collectionItems_[i].assets.length; ++j) {
                _stackToken(collectionItems_[i].collection, collectionItems_[i].assets[j], houseId_);
            }
        }
    ...
    function unStackFromHouse(CollectionItems[] calldata collectionItems_) external returns(uint){
        bool applyBuff_ = BSBMintPassBuffCaller.getBuffBalance(_msgSender()) != 0;
        ...
        for (uint i = 0; i < collectionItems_.length; ++i) {
            ...
            for (uint j = 0; j < collectionItems_[i].assets.length; ++j) {
                _unstackToken(collectionItems_[i].collection, collectionItems_[i].assets[j]);
            }
        }
        BSBBloodTokenCaller.mint(_msgSender(), accumulatedYield_);
    ...
    function _addToElitesIfNecessary(CollectionItems calldata collectionItem_) internal {
    ...
    function _removeFromElitesAndUnstackIfNecessary(CollectionItems calldata collectionItem_) internal {
        BearStats[] memory bearsStats = BSBStorageCaller.getBearsStats(
            collectionItem_.collection, collectionItem_.assets
        );
        for (uint i = 0; i < collectionItem_.assets.length; ++i) {
            bool shouldRemoveEliteFromPool = (collectionItem_.collection == contracts["gen0"] || collectionItem_.collection == contracts["gen1"]) &&
            bearsStats[i].isElite;

            if (shouldRemoveEliteFromPool) {
                _removeEliteFromPool(collectionItem_.collection, collectionItem_.assets[i], bearsStats[i].faction);
            }
            delete shouldRemoveEliteFromPool;

            if (collectionItem_.collection == contracts["treeHouse"]) {
                require(getTreeHouseOccupancy(collectionItem_.assets[i]) == 0, "BSB450");
            } else if (tokenHouseAppartenance[collectionItem_.collection][collectionItem_.assets[i]]) {
    ...
    function _stackToken(address collection_, uint tokenId_, uint houseId_) internal {
        require(!tokenHouseAppartenance[collection_][tokenId_], "BSB400");
    ...
    function _calculateTokenGeneratorYield(
        ...
        if (applyBuff_) {
            yieldBonus_ = tokenGeneratorWithBuffBonusPercentage;
        } else {
    ...
    function _claimBearRewards(CollectionItems calldata collectionItems_, bool applyBuff_) internal returns(uint) {
        ...
        for (uint i = 0; i < collectionItems_.assets.length; ++i) {
            ...
            bearStats[i].stamina.value += _calculateBearAccumulatedStamina(
                collectionItems_.collection, collectionItems_.assets[i], applyBuff_, bearStats[i].stamina
            );
    ...
    function calculateBearReward(address collection_, uint tokenId_) public view returns (uint, uint) {
        ...
        return (
            ...
            _calculateBearAccumulatedStamina(collection_, tokenId_, applyBuff_, bearStats.stamina)
        );
    }
    ...
    function _calculateYieldForRate(
        ...
        return (block.timestamp - stateChangeDates[collection_][tokenId_]) * yield_ / (yieldDaysRate * DAYS);
    }
}
