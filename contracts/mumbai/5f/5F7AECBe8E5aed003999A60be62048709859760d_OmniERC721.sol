...
contract OmniERC721 is IOmniERC721, ERC721A, ReentrancyGuard {
    ...
    bool public isAllowlisted;
    mapping(address => uint256) public allowlistedMints;
...
//        }
    ...
    function _validateMint(uint256 _quantity, address _owner) internal view {
        ...
        if (isAllowlisted) require(allowlistedMints[_owner] >= _quantity);
    }
    ...
    function allowlist(address[] calldata addresses, uint256 maxMints) external {
        require(msg.sender == owner);

        for (uint i = 0; i < addresses.length; i++) {
            allowlistedMints[addresses[i]] = maxMints;
        }
        isAllowlisted = true;
    }
}
