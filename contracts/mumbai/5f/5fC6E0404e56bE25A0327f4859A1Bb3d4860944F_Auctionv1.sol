...
contract Auctionv1 is Initializable, OwnableUpgradeable {
    ...
    */
    ...
    function setWithdrawalAddress(address newWithdrawalAddress) external {
        require(msg.sender == withdrawAddress, "You're not allowed");
    ...
    function getTokenIds(uint256 auctionId) view external returns(uint256[] memory tokenIds) {
    ...
    function currentAuctionId() view external returns(uint256) {
    ...
    function getActiveAuctionsId() external view returns (uint256[] memory, string[] memory, bool[] memory) {
        uint256 size = auctionsDetails.length;
        uint256[] memory auctionIds = new uint256[](size);
        string[] memory auctionName = new string[](size);
        bool[] memory isEnded     = new bool[](size);

        for (uint256 i = 0; i < auctionsDetails.length; i++) {
            auctionIds[i]  = i;
            auctionName[i] = auctionsDetails[i].name;
            isEnded[i]     = auctionsDetails[i].endTime > block.timestamp;
        }
        return (auctionIds, auctionName, isEnded);
    }
}
