...
contract Dijnee is ERC721Enumerable, Ownable {
  using Strings for uint256;
 ...
 mapping(uint256 => _bidDetail) public bidDetail;
  ...
  function walletOfOwner(address _owner) public view returns (uint256[] memory){
    ...
    function multiMint(address user, uint256[] memory tokenId, uint256 royalty) external {       
        ...
        for(uint i = 0; i < tokenId.length; i++){
            _safeMint(user, tokenId[i]);        
            tokenDetails[tokenId[i]] = _tokenDetails(msg.sender,user,royalty,0,SellType.FIXED_PRICE,true,true);
        }
    }

    function Mint(address user, uint256 tokenId, uint256 royalty) external returns(uint256){       
        require(!paused, "the contract is paused");        
        _safeMint(user, tokenId);
        if(!isApprovedForAll(msg.sender, address(this))){
            setApprovalForAll(address(this), true);
        }        
        tokenDetails[tokenId] = _tokenDetails(msg.sender,user,royalty,0,SellType.FIXED_PRICE,true,true);
        return tokenId;
    }

    function putOnSale(uint256 tokenId,uint256 salePrice,SellType sellType) public {
    ...
    function buy(uint256 tokenId) payable public {
        ...
        payable(tokenDetails[tokenId].owner).transfer(tokenDetails[tokenId].salePrice.sub(royaltyAmt));
  ...
  function cancelBid(uint256 tokenId) external {
    require(bidDetail[tokenId].isActive, "Bid not available");
    require(bidDetail[tokenId].bidder == msg.sender, "Bid not available");
    
    payable(bidDetail[tokenId].bidder).transfer(bidDetail[tokenId].amount);
    bidDetail[tokenId].bidder = address(0);
    bidDetail[tokenId].isActive = false;
}
  ...
  function acceptBid(uint256 tokenId) external {
    ...
    payable(tokenDetails[tokenId].owner).transfer(bidDetail[tokenId].amount.sub(royaltyAmt));
