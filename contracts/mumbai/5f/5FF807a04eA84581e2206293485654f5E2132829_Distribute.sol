...
contract Distribute is Ownable {
    ...
    function getBalance(address _address) public view returns(uint256, uint256) {
        ...
        if(genesisWETHBalance[_address] > 0){ WETH += foundersWETHBalance[_address]; }
        if(foundersGGBalance[_address] > 0){ GG += foundersWETHBalance[_address]; }
        if(genesisGGBalance[_address] > 0){ GG += foundersWETHBalance[_address]; }
        return (WETH, GG);
    }
