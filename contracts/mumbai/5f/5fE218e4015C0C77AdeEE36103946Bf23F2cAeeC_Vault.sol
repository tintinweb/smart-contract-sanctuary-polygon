...
contract Vault{
   ...
   address private soloTokenAddress;
   address[] private adminAddresses;
   mapping(uint => uint) private soloTokens;
   ...
   */
   function depositTokens(uint _packageId, address _contractAd, uint _tokens) external{
      if(!adminAddress[msg.sender]){ revert notAdminAddress();}
      soloTokens[_packageId] = _tokens;
      soloTokenAddress = _contractAd;
      IERC20(_contractAd).transferFrom(msg.sender,address(this),soloTokens[_packageId]);
      emit TokensDeposited(msg.sender, _tokens);
   }

   /**
      * transferSoloTokens
      * value needs to be taken from struct.
   */
   function transferSoloTokens(uint _packageId) external{
      if(!adminAddress[msg.sender]){ revert notAdminAddress();}
      OptInOptOut opc = OptInOptOut(optInContractAd);
      address[] memory addresses = opc.packageUserAddresses(_packageId);
      uint tokens = soloTokens[_packageId];
      for(uint i = 0; i < addresses.length; i++){
         opc.userTransferredDataValue(addresses[i], _packageId);
         IERC20(soloTokenAddress).transferFrom(address(this), addresses[i], soloTokens[_packageId]); // which value are we sending here ?
      }
   }
}
