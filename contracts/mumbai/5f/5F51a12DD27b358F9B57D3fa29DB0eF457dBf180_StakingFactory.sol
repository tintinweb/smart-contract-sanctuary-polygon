  ...
  AdminProxyManager
{
  ...
  function allStakingsLength() external view virtual returns (uint256) {
    return allStakings.length;
  }

  function owner() public view override(IFactory, OwnableUpgradeable) returns(address){
    return super.owner();
  }
  ...
  function createStaking(
    ...
    address _owner
  ) external virtual onlyOwner whenNotPaused returns (address staking) {
    bytes memory data = abi.encodeWithSelector(
      IStaking.init.selector,
      _token,
      _minStaking,
      _lockPeriodInDays,
      _apy_d2,
    ...
    staking = address(new BeaconProxy(beacon, data));
    allStakings.push(staking);

    emit StakingCreated(
      _token,
      allStakings.length - 1,
      block.timestamp,
      staking
    );
  }
