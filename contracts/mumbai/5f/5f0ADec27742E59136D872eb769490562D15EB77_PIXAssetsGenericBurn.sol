 ...
 */
contract PIXAssetsGenericBurn is IGenericBurn, OwnableUpgradeable {
    ...
    function setContents(
        ...
        if (!s_tokenCategoryActive[_tokenId][_categoryId]) revert GB__InvalidCategoryId(_categoryId);
        if (_amounts.length != _weights.length || _amounts.length != _tokens.length || _amounts.length != _tokenIds.length) revert GB__ArraysNotSameLength();
    ...
    function _burnTokens(uint256 _tokenId, uint256 _amount) internal {
        IAssetManager(pixAssets).trustedBurn(msg.sender, _tokenId, _amount);
    }
    ...
    function _mintReward(
        ...
        ITrustedMintable(_category.tokens[_index]).trustedMint(_user, _category.tokenIds[_index], _category.tokenAmounts[_index]);
