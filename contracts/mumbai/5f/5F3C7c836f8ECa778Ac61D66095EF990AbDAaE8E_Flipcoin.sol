...
contract Flipcoin {
    ...
    function joinLobby(uint256 lobbyId, uint256 betAmount) external gasLimit(200000) {
        ...
        require(betAmount >= _bets[lobbyId][_player1], "Invalid bet amount");
        require(_amberToken.balanceOf(msg.sender) >= betAmount * (10**_tokenDecimals), "Insufficient AMBER token balance");
    ...
    function closeLobby(uint256 lobbyId) external onlyOwner gasLimit(200000) {
    ...
    function _lobbyEnded(uint256 lobbyId) internal {
    ...
    modifier gasLimit(uint256 limit) {
        require(gasleft() >= limit, "Not enough gas");
        _;
    }
}