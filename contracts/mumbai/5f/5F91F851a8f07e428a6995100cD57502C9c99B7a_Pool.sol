...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  /// @inheritdoc IPool
  function setReserveRates(address asset, uint256 supplyRate, uint256 borrowRate, uint256 prxyRate, uint256 splitRate) external virtual override onlyPoolConfigurator {
    ...
    returns(uint256)
  {
    ...
    uint256 rate = _reserves[supplyingAsset].dynamicSupplyRate * WadRayMath.RAY;
...
library ParticularLogic {
  ...
  function executeSplitReduxInterest(
    ...
    if (reduxAmount == 0) {
      reserve.dynamicSupplyRate = reserve.supplyRate;
    ...
    uint256 dynamicSupplyRate = reserve.supplyRate;
    uint256 dynamicSplitRate = reserve.supplyRate * totalUncollateralInBaseCurrency * elapsedTime  / (reduxInBaseCurrency * MathUtils.SECONDS_PER_YEAR);

    if (dynamicSplitRate > reserve.splitRate) {
      dynamicSplitRate = reserve.splitRate;
      dynamicSupplyRate = reserve.splitRate * reduxInBaseCurrency * MathUtils.SECONDS_PER_YEAR / (totalUncollateralInBaseCurrency * elapsedTime);
    }
    reserve.dynamicSupplyRate = dynamicSupplyRate;

    uint256 splitAmount = reduxAmount * dynamicSplitRate / GeneralLogic.RATE_PRECISION;
...
library ReserveLogic {
  ...
  function init(
    ...
    reserve.dynamicSupplyRate = supplyRate;
    reserve.dynamicSplitRate = splitRate;
  }
}
