...
contract dice {
    ...
    event SettlementEnd(string indexed orderIdIdx, string orderId, uint256 randomNum, uint256[] results,BetRegion[] winRegions, address[] accounts, BetRegion[] betRegions, uint256[] betAmounts, uint256[] payouts); //结算完成
    ...
    function doSettlement(uint256 randomNum, uint256[] memory results) private {
        ...
        emit SettlementEnd(orderId, orderId, randomNum, results, winRegions, accounts, betRegions, betAmounts, payouts);
    ...
    function ownerWithdraw(uint256 amount) public isOwner {
        ...
        require(amount <= address(this).balance, "withdraw amount need ge the contract balance.");
