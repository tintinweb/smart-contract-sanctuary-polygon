...
contract lbtestev2 is VRFV2WrapperConsumerBase,ConfirmedOwner,ReentrancyGuard {
	using SafeMath for uint256;
    ...
    uint256 internal TICKET_VAL = 5 * 10**stableDecimals; //minimum amount (in wei) for getting registered on list
        ...
        VRFV2WrapperConsumerBase(linkAddress, wrapperAddress) 
    {
        ...
        lottoWeekly = IluckblocksWeekly(0xa7200AD163B8D99797C67A5e8d59b756a9DFe905);
        weeklyContract = 0xa7200AD163B8D99797C67A5e8d59b756a9DFe905;
    ...
    function getJackpotinUSD() public view returns (uint256) {
       ...
       uint256 result = (prizeValueJackpot * tprice) / 10**18; 
    ...
    function withdrawExcessBalance () public onlyOwner() {
        ...
        eth.safeTransfer(msg.sender,contractBalance);
    ...
    function fillJackpot(uint256 amount) external {
        ...
        uint256 amountInToken = (50 * 10**18 * 10**18) / tprice;  // 50 usd in token

        minimumPrize = amountInToken;

        require(amount >= minimumPrize,"not enough amount - below 50 usd");
        require(eth.balanceOf(msg.sender) >= amount,"not enough balance");
        
        eth.safeTransferFrom(msg.sender,address(this),amount);
    ...
    function accumulateJackpot(uint256 amount) internal {
        ...
        if(amount > minimumPrize){
         ...
         eth.safeTransfer(weeklyContract,amount.mul(15).div(100));
    ...
    function swapTokensToUSD(uint256 tokenAmount, address user, uint typeSwap) private {
        ...
        eth.approve(address(uniswapRouter), (amountInToken+10000000000000000000));

        // make the swap
        uniswapRouter.swapTokensForExactTokens(
            tokenAmount,
            amountInToken+10000000000000000000, // to garantee the trade
            path,
            user, // receiving user
            block.timestamp + 300
        );
        ...
        if(typeSwap == 1){
         ...
         prizeValueJackpot -= amountInToken+50000000000000000;
    ...
    function autoSpin(address _caller, uint8[] calldata _nodes) public nonReentrant {
        ...
        if(lbNodes.getUserActivation(_caller,0) == false){
            lbNodes.activateNodes(_caller,_nodes,2,0);
        }else{
            ...
            if(getJackpotinUSD() >= minimumPot && registeredsAccts.length >= 20){ 


                    if(block.timestamp > lastAutoSpin + 900 && reset == false){    // if no node activates in 5 minutes period queue resets
                        lbNodes.resetQueue(2,0);
                        reset = true;
                    } else if(lbNodes.updateNodeInfo(_caller,botReward,0,2) == true){
    ...
    function getRandomNumber() internal returns (uint256 requestId) {
        ...
        // minimum 10 usd to nftnodes if sufficient balance

        if(contractBalance >= minimumPrize*3){
                         
            swapTokensToUSD((10 * 10**stableDecimals),address(this),0);
    ...
    */
    function fulfillRandomWords(uint256 requestId, uint256[] memory _randomWords) internal override {
        ...
        if (idInPosition < 10) {
            ...
            krstm.safeTransfer(userWallet, prizeValueJackpot);
            prizeValueJackpot = 0;
    ...
    function BuyTicket(address player,uint quantity) public nonReentrant {
        ...
        // update balances
         ...
         prizeValueJackpot = (eth.balanceOf(address(this)) - contractBalance); // ((dailyFeed * 10**12) / tprice) / 10**10;
    ...
    function ForfeitTicket(uint256 index) public nonReentrant {
        ...
        require(getJackpotinUSD() >= 5 * 10**18, "Contract need to have atleast 5 USD in value.");
