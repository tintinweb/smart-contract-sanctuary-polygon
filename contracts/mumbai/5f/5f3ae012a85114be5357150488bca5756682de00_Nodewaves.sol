...
contract TransferContract is ReEntrancyGuard{
...
/*        Read contract          */
    ...
    function depositTenReferral(address _f1, address _f2, address _f3, address _f4, address _f5, address _f6, address _f7, address _f8, address _f9, address _f10) public payable{
        ...
        timestamp[msg.sender] = block.timestamp;
        totalUser +=1;
        uint total_percentages = f_percentages[0]+f_percentages[1]+f_percentages[2]+f_percentages[3]+f_percentages[4]+f_percentages[5]+f_percentages[6]+f_percentages[7]+f_percentages[8]+f_percentages[9];
    ...
    function companyWithdraw(uint _amount) public {
        require(msg.sender == company, "You are not othorised");

        company.transfer(_amount);
    }

    function companyWithdrawToken(uint _amount) public {
        require(msg.sender == company, "You are not othorised");
...
contract Nodewaves is ERC20, ERC20Burnable, Ownable, ReEntrancyGuard {
    ...
    function buyNode(address[10] memory _friend) public payable{
        for (uint i = 0; i<10;i++){
            if(_friend[i] != address(0)){
                referral_balances[_friend[i]] +=(msg.value*deposit_one_percentage)/100;
            }
        }
        isUser[msg.sender] = true;
        timestamp[msg.sender] = block.timestamp;
    ...
    function buyMembership(address[10] memory _friends) public payable{
