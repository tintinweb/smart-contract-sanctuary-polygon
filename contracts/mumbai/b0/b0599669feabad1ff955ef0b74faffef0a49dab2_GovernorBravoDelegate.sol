...
contract GovernorBravoDelegate is Initializable,UUPSUpgradeable,GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    uint public constant MIN_PROPOSAL_THRESHOLD = 50000e18; // 50,000 Revolt

    /// @notice The maximum setable proposal threshold
    uint public constant MAX_PROPOSAL_THRESHOLD = 6000000000000e18; //6000000000000 Revolt
    ...
    /**
      ...
      */
    function initialize(address timelock_, address uRvlt_, uint votingPeriod_, uint proposalThreshold_, address treasury_, uint proposalPeriod_, uint rvltPriceProposal_) public initializer{
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(proposals[latestProposalIds[msg.sender]].startTime > proposalPeriod, "GovernorBravo::propose: Only one proposal can create in one batch");
        ...
        newProposal.startBlockNumber = block.number;
    ...
    function castVoteInternal(address voter, uint proposalId, uint8 support) internal returns (uint256) {
        require(!uRvlt.iCultMandator(voter),"GovernorBravo::castVoteInternal: Non mandorts cannot vote");
        require(state(proposalId) == ProposalState.Active, "GovernorBravo::castVoteInternal: voting is closed");
        require(support <= 2, "GovernorBravo::castVoteInternal: invalid vote type");
        Proposal storage proposal = proposals[proposalId];
        Receipt storage receipt = proposal.receipts[voter];
        require(receipt.hasVoted == false, "GovernorBravo::castVoteInternal: voter already voted");
        uint256 votes = uRvlt.getPastVotes(voter, proposal.startBlockNumber);
    ...
    function _setInvesteeDetails(address investee_, uint256 fundAmount_) external {
        ...
        InversteeDetails memory investee = investeeDetails[nextInvestee];
