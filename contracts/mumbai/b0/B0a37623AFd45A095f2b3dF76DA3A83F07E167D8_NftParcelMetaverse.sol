...
contract NftParcelMetaverse is ERC721URIStorage {
    ...
    function create(
        ...
        setAttributeMetadata(tokenCounter, _nameWorld, _size, _type, _map);
    ...
    private {
            Attr memory newMetadata = Attr({
                nameWorld : _nameMetaverse,
                size : _metaverseSize,
                _type : _metaverseType,
                map : _metaverseMap
            });
            Attribute[_tokenId] = newMetadata;
        }

    function getmetadataAttribute(uint256 _tokenId) public view returns (string memory) {
        string[4] memory i;
        i[0] = string.concat('[{"trait_type": "Name World","value": "', getName(_tokenId), '"},');
        i[1] = string.concat('{"trait_type": "Size World","value": "', getSize(_tokenId), '"},');
        i[2] = string.concat('{"trait_type": "Type Map","value": "', getType(_tokenId), '"},');
        i[3] = string.concat('{"trait_type": "Name Map World","value": "', getMap(_tokenId), '"}]');
        
        string memory output = string.concat(i[0], i[1], i[2], i[3]);
        return output;
    }
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                ...
                                '", "description":"An NFT based on SVG!", "attributes":"', getmetadataAttribute(_tokenId),'", "image":"',_imageURI,'"}'
                            )
                        )
                    )
                )
