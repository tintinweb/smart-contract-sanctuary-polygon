...
contract TestXS {
    ...
    function getRandomID(address _owner, uint256 randomNumber) public view returns(uint256){
        ...
        if(number <= 9600){
            ...
            plantID +=  _randomSkill(_owner, randomNumber) * 10**6 + _randomSkillZone(_owner, randomNumber) * 10**4 + _randomSupport(_owner, randomNumber + 532323 + 532323) * 10**2 + _randomSabotage(_owner, randomNumber + 532323);
            return plantID;
