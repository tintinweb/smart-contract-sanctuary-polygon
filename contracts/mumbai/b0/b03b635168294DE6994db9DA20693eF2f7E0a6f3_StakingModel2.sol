...
contract StakingModel2 is StakingCalculations{
    ...
    struct StakeState {
        ...
        uint256 poolSize; // Limit on no of tokens that can be staked
    ...
    struct StakeRewardState {
        uint256 rewardsClaimed; //Total rewards that are claimed by all accounts
        uint256 rewardsTotal; //Total rewards in contract
    ...
    constructor (
        ...
        //set parameters for staking model
        stakeState.poolSize = 1000000000;
        daysOpenForStake = 10; //10 days
        daysOfMandatoryLock = 1; //1 day
        daysOfMaturity = 10; //10 days
        rewardState.rewardsTotal = 40000000; //4% of pool size
        maturityRewardAPY = 40; //40%
        earlyRewardAPY = 30; //30%
        
        //set time for stake and early reward withdraw
        stakingStarts = block.timestamp; //Time at which contract is deployed
        stakingEnds = block.timestamp.add(daysOpenForStake*86400); //(10*24*60*60) = 10 days
        earlyWithdrawStarts = block.timestamp.add(daysOfMandatoryLock*86400); //(10*24*60*60) = 10 days
        earlyWithdrawEnds = earlyWithdrawStarts.add(777600);//(9*24*60*60) = 9 days
    ...
    function stake(uint256 _amount) _positive(_amount) _after(stakingStarts) _before(stakingEnds) public returns (bool _success) {
        ...
        // calculating new maturity time by adding days of maturity = 10 days to current time
    ...
    function withdrawAfterMaturitySeparate(uint256 _id) public returns(bool){
        ...
        rewardState.rewardsClaimed = (rewardState.rewardsClaimed).add(rewardBalance);
    ...
    function withdrawAfterMaturityCombined() public returns(bool){
        ...
        stakeState.stakedBalance = (stakeState.stakedBalance).sub(calcStakes);
        rewardState.rewardsClaimed = (rewardState.rewardsClaimed).add(maturityReward);
    ...
    function withdrawEarlySeparate(uint256 _id) _after(earlyWithdrawStarts) _before(earlyWithdrawEnds) public returns(bool){
        ...
        rewardState.rewardsClaimed = (rewardState.rewardsClaimed).add(earlyReward);
        stakeState.stakedBalance = (stakeState.stakedBalance).sub(calcStakes);
    ...
    function getTotalStakeAmount(address staker) public view returns(uint256){
        return details[staker].totalStakeAmount;
    }
