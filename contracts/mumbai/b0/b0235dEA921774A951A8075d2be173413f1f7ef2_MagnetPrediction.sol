...
interface IMagnetNft is IERC1155 {
    function deposit(uint256 id) external payable;

    function withdrawPredictionBalance(
        address _address,
        uint256 id,
        uint256 amount
    ) external;
...
import "./interfaces/IMagnetNft.sol";
 ...
 */
contract MagnetPrediction is Ownable, Pausable, ReentrancyGuard {
    using SafeERC20 for IERC20;

    AggregatorV3Interface public oracle;
    IMagnetNft public nftContract;

    address public betOperatorAddress;
    uint public ownerBetGas;
    ...
    mapping(address => uint) public betBalances;
    ...
    constructor(
        ...
        nftContract = IMagnetNft(_nftContract);
        betOperatorAddress = _betOperatorAddress;
    }
    ...
    function _betBear(
        ...
        address _address,
        uint betAmount
    ) private whenNotPaused nonReentrant notContract {
        ...
        require(
            ledger[epoch][_address].amount == 0,
        ...
        // Update user data
        BetInfo storage betInfo = ledger[epoch][_address];
        betInfo.position = Position.Bear;
        betInfo.amount = betAmount;
        userRounds[_address].push(epoch);

        emit BetBear(_address, epoch, betAmount);
    }
    ...
    function _betBull(
        ...
        emit BetBull(_address, epoch, betAmount);
    }

    function betBear(
        uint256 epoch
    ) external payable whenNotPaused nonReentrant notContract {
        _betBear(epoch, msg.sender, msg.value);
    }

    function betBull(
        uint256 epoch
    ) external payable whenNotPaused nonReentrant notContract {
        _betBull(epoch, msg.sender, msg.value);
    }

    function ownerBetBull(
        uint256 epoch,
        address _address,
        uint betAmount
    ) external whenNotPaused nonReentrant notContract {
        require(
            betBalances[msg.sender] >= betAmount,
            "Insufficient bet balance"
        );
        betBalances[msg.sender] -= betAmount;
        _betBull(epoch, _address, betAmount);
    }

    function ownerBetBear(
        uint256 epoch,
        address _address,
        uint betAmount
    ) external whenNotPaused nonReentrant notContract {
        ...
        betBalances[msg.sender] -= betAmount;
        _betBear(epoch, _address, betAmount);
    }

    function setOwnerBetGas(uint newGas) external {
        ownerBetGas = newGas;
    }
    ...
    function depositBalance(
        uint id,
        uint amount,
        uint32 futureBetsCount
    ) external {
        require(
            nftContract.balanceOf(msg.sender, id) > 0,
            "Caller does not own the token"
        );
        uint gasFee = futureBetsCount * ownerBetGas;
        nftContract.withdrawPredictionBalance(
            address(this),
            id,
            amount + gasFee
        );
        betBalances[msg.sender] += amount;
        payable(betOperatorAddress).transfer(gasFee);
    }
