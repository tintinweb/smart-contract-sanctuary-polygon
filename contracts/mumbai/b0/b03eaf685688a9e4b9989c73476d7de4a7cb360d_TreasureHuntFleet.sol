    ...
    OwnableUpgradeable
{
    ...
    function buildFleet(
        ...
        uint256 power = getHaki(pirates);
        require(power >= 100, "Too low haki Power!");
        if (!teamlist[msg.sender]) {
            ITreasureHunt(rewardPool).transferCost(fleetMintPrice, false);
        }

        ITreasureHunt(treasureHuntPirate).boardPirates(pirates);
        ITreasureHunt(treasureHuntShip).joinShips(ships);
        mintCnt++;
        uint256 tokenID = mintCnt;
        uint8 rank = getRank(ships);
        uint8 life = getFleetLifeCycle(power);
    ...
    function transferFrom(
        ...
        tokenIndex[
            fleetsByOwner[from][fleetsByOwner[from].length - 1]
        ] = tokenIndex[tokenID];
        fleetsByOwner[from][tokenIndex[tokenID]] = fleetsByOwner[from][
            fleetsByOwner[from].length - 1
        ];
    ...
    function transferFleet(uint256 tokenID) public _isMarket {
        require(
            fleetInfo[tokenID].LifeCycle == (30 + (fleetInfo[tokenID].Power - 100) / 1000) * 5 &&
                ITreasureHunt(rewardPool).getRepairCost(tokenID) == 0,
            "Can't sell this fleet!"
        );
    ...
    function setRepairCost(uint256 tokenID, uint256 amount)
        public
        onlyRewardPool
    {
        fleetInfo[tokenID].RepairCost += amount;
    }

    function resetRepairCost(uint256 tokenID) public onlyRewardPool {
        fleetInfo[tokenID].RepairCost = 0;
    }
    ...
    function getFleetRaidLeftTime(uint256 tokenID) public view returns(uint256) {
        if (block.timestamp <= fleetInfo[tokenID].RaidClock) {
            return fleetInfo[tokenID].RaidClock - block.timestamp;
        } else {
            return 0;
        }
    ...
    function setFleetDurability(uint256 tokenID, bool dur)
        public
        onlyRewardPool
    {
    ...
    function addContract(
        ...
        uint8 _contractDay,
        bool isDirect
    ) public fleetOwner(tokenID) {
        if (!teamlist[msg.sender]) {
            uint256 fund = _contractDay * fleetInfo[tokenID].pirates.length;
            if (isDirect) {
                ITreasureHunt(rewardPool).transferCost(fund, false);
            } else {
                ITreasureHunt(rewardPool).payUsingReward(
                    msg.sender,
                    fund,
                    false
                );
    ...
    function addFund(
        ...
        uint8 decimal = ITreasureHunt(rewardPool).getDecimal();

        if (isFuel) {
            fleetInfo[tokenID].Fuel += fund * (10**decimal);
        } else {
            fleetInfo[tokenID].Energy += fund * (10**decimal);
        }
    }

    function updateFleetFund(
        uint256 _tokenID,
        uint256 _fuel,
        uint256 _energy
        ...
        fleetInfo[_tokenID].LifeCycle--;
    }

    function reduceDoubleOrNothingLifeCycle(uint256 _tokenID, address acc)
        public
        onlyRewardPool
    {
        require(ownerOf(_tokenID) == acc, "You are not token Owner!");
        fleetInfo[_tokenID].LifeCycle--;
    }

    function dismantleFleet(uint256 tokenID) public fleetOwner(tokenID) {
        require(
            fleetInfo[tokenID].Durability,
            "Should repair before dismantle!"
        );
        ITreasureHunt(treasureHuntPirate).unBoardPirates(
            fleetInfo[tokenID].pirates
        );
        ITreasureHunt(treasureHuntShip).disJoinShips(fleetInfo[tokenID].ships);
        ITreasureHunt(rewardPool).updateExperience(
            msg.sender,
            fleetInfo[tokenID].Power
        );
        _burn(tokenID);

        tokenIndex[
            fleetsByOwner[msg.sender][fleetsByOwner[msg.sender].length - 1]
        ] = tokenIndex[tokenID];
        fleetsByOwner[msg.sender][tokenIndex[tokenID]] = fleetsByOwner[
            msg.sender
        ][fleetsByOwner[msg.sender].length - 1];
        fleetsByOwner[msg.sender].pop();
        delete tokenIndex[tokenID];
        delete fleetInfo[tokenID];
    }

    function getFleetLifeCycle(uint256 power) internal pure returns (uint8) {
        uint8 life = uint8(30 + ((power - 100) / 1000) * 5);
        return life;
    }
    ...
    function getFleetsByOwner(address acc)
        public
        view
    ...
    function getFleetsData(uint256[] memory tokenIDs)
        public
        view
        returns (Fleet[] memory)
    {
        Fleet[] memory fleets = new Fleet[](tokenIDs.length);
        for (uint8 i = 0; i < tokenIDs.length; i++) {
            fleets[i] = fleetInfo[tokenIDs[i]];
        }
        return fleets;
    }

    function getFleetsDataByOwner() public view returns (Fleet[] memory) {
        return getFleetsData(fleetsByOwner[msg.sender]);
    }
    ...
    function setTreasureHuntPirate(address _treasureHuntPirate)
        public
        onlyOwner
    {
        treasureHuntPirate = _treasureHuntPirate;
