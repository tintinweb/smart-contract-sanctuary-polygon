...
contract NiioERC721SingleV1 is Initializable, ERC721Upgradeable, PausableUpgradeable, AccessControlUpgradeable, ERC721BurnableUpgradeable, UUPSUpgradeable {

    using CountersUpgradeable for CountersUpgradeable.Counter;

    /**
     * Tracking the nextTokenId (instead of the currentTokenId) to save users on gas costs.
    ...
    function initialize(address admin, address superAdmin) initializer public {
        __ERC721_init("NiioNFT", "NIIO");
        __Pausable_init();
    ...
    function mintNft(address creator)
    external
    onlyRole(ADMIN_ROLE)
    returns (uint256) {
    ...
    function mintAndTransfer(address creator, address buyer)
    external
    onlyRole(ADMIN_ROLE)
    returns (uint256) {
        uint256 newNftTokenId = _nextTokenId.current();
    ...
    function mintAndTransferByGallery(address creator, address gallery, address buyer)
    external
    onlyRole(ADMIN_ROLE)
    returns (uint256) {
        uint256 newNftTokenId = _nextTokenId.current();
