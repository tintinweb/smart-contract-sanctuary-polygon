...
contract WelfareFactory is AccessControl {
    ...
    function newLotteryWithoutRNG(uint8 _length, address payable _lottery)
        external
        onlyRole(FACTORY_ROLE)
        returns (address)
    {
        ...
        lottery.setRandomNumberGenerator(
            Lottery(_lottery).randomNumberGenerator()
        );
        phases[phase] = address(lottery);
        lottery.transferOwnership(msg.sender);
...
contract Lottery is IRandomConsumer, Ownable, AccessControl {
    ...
    ) external onlyRole(LOTTERY_ROLE) isState(LotteryState.Open) {
        require(
