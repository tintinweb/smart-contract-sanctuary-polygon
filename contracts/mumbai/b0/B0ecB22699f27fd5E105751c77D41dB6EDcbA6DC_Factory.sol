    ...
    AccessControl
{
    ...
    function mintLoanNft(
        ...
        IPool(pools[_details.bank]).stake(
            _details.borrower,
            (((_details.borrowerShare) * 975) / 1000),
            _details.tokenID
        );
 ...
 console.log("mintloanNFT 1");
    ...
    function _prepaidPostpaid(LoanLib.Loan memory _details) internal {
        uint256 defiShare = (((_details.foundationShare) * 25) / 1000) +
            (((_details.borrowerShare) * 25) / 1000);

        if (_details.isPrepaid) {
            Paza.transferFrom(_details.bank, defiWallet, defiShare);

            Paza.transferFrom(
                _details.bank,
                foundationWallet,
                ((_details.foundationShare) * 975) / 1000
            );
            ...
            Paza.safeTransfer(defiWallet, defiShare);
            Paza.safeTransfer(
                pools[_details.bank],
                (((_details.borrowerShare) * 975) / 1000)
            );
