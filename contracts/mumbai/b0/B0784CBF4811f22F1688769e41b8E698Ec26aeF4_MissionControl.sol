...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    function placeNFT(
        ...
        // Should only allow radius 1 tiles to be rented
    ...
    function removeNFT(
        ...
        require((abs(_x) + abs(_y) + abs(_z)) / 2 == 1, "MISSION_CONTROL: RADIUS 1 ONLY");
    ...
    function createRentTiles(
        ...
        int96 tilesRented;
        // todo: change for gas efficiency
        uint256 i;
        uint256 length = _placeOrders.length;
        for (; i < length; i++) {
            // get placeOrder item once
            ...
            if (tile.nonce != 0 && tile.pausedAt != 0) {
                ...
                // Should only allow radius 2 tiles to be rented through superfluid
                require(
        ...
        require(tilesRented * RENT_TILE_FLOW_RATE == _flowRate);
    }
    ...
    function updateRentTiles(
        ...
        for (i; i < removeTileLength; i++) {
            ...
            __removeNFT(removeTileItem.x, removeTileItem.y, removeTileItem.z);
        }
    ...
    function deleteRentTiles(address _supertoken, address _renter) external onlyStreamer {
        ...
        for (i; i < length; i++) {
            ...
            // If the tile element was a top, we need to pause the bottom as well
    ...
    function removeNFTs(PlaceOrder[] memory _placeOrders) external override {
        for (uint256 i; i < _placeOrders.length; i++) {
            __removeNFT(_placeOrders[i].order.x, _placeOrders[i].order.y, _placeOrders[i].order.z);
        }
    }
    ...
    function checkTile(
        ...
        return IMissionControlStakeable(tile.stakeable).checkTile(_user, tile.nonce, tile.pausedAt);
    }
    ...
    function checkStakedOnTile(
        ...
        int256 _y,
        int256 _z
    ) external view override validCoordinates(_x, _y, _z) returns (TileElement memory _base, TileElement memory _top) {
    ...
    function __placeNFT(
        ...
        if (IMissionControlStakeable(tokenAddress).isBase(_tokenId)) {
            _handleBasePlacement(order.x, order.y, order.z, _tokenId, tokenAddress, ++stakeNonces[_staker]);
        } else {
            _handleTopPlacement(order.x, order.y, order.z, _tokenId, tokenAddress, ++stakeNonces[_staker]);
        }
    ...
    function __removeNFT(
        ...
        if (zeroOrOne == 1) {
            IMissionControlStakeable(tileElements[0].stakeable).reset(msg.sender, tileElements[0].nonce);
        }
        emit NFTRemoved(
            msg.sender,
    ...
    function _handle_raid_collect(
        ...
        return iStakeable.onCollect(msg.sender, tileElement.nonce, tileElement.pausedAt);
    }
