...
contract RecordLabel is FunctionsClient, ConfirmedOwner {
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    if (nilErr) {
      ...
      // uint256 amountDue = // TODO (increase in listener count / 1000 * Rate Per 1000)

      // Update Artist Mapping.
      string memory listenerCountStr = Strings.toString(latestListenerCount); // convert to string to store in mapping.abi
      console.log("\nLatest listener count : %s", listenerCountStr);
      string memory artistId = latestArtistRequestedId;
      artistData[artistId].lastListenerCount = listenerCountStr;
  ...
  function setArtistData(
    ...
    string memory name,
    string memory lastListenerCount,
    string memory email,
    address walletAddress
  ) public onlyOwner {
