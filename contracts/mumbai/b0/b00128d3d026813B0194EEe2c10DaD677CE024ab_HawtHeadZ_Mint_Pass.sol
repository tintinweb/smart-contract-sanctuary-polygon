...
contract HawtHeadZ_Mint_Pass  is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
	...
	function mint(uint256 mintAmount) external payable {
        uint256 costToPay = getCost(mintAmount);
		require(!pause, "The contract is paused");
        require(_totalMinted() + mintAmount <= maxSupply,"Exceeds max supply");
		require(balance[msg.sender] + mintAmount <= maxPerWallet, "Max per wallet exceeded");
        require(msg.value >= costToPay, "insufficient funds");

		_safeMint(msg.sender, mintAmount);
		balance[msg.sender] = balance[msg.sender] + mintAmount;
	}
    ...
    function getCost(uint256 amt) public view returns(uint256){
        if(wlSupply > totalSupply()){
            uint256 wlRemaining = wlSupply - totalSupply();

            if(amt > wlRemaining) return ((amt - wlRemaining) * cost) + (wlRemaining * preCost);
            else return wlRemaining * preCost;
        }
        else return cost * amt;
    }
	...
	function setMaxPerWallet(uint256 _newMax) public onlyOwner{
