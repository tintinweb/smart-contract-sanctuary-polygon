...
interface IMargin {
    ...
    function getDepositAmounts(address user, address assetAddress) external returns(uint256, uint256, uint256, uint256);

    function ltmRatio(address user, address assetAddress, uint256 source) external view returns (uint256);

    function setPositionAmount(address user, address assetAddress, uint256 marginUsed, uint256 loanUsed, uint256 source) external;
...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function createPositionAndTrade(address tokenAddress, address targetToken, uint256 amount) external{
       ...
       (uint256 availableMargin,
        ...
        uint256 ltmRatio = IMargin(IAdminFunctions(adminContract).getMarginContract()).ltmRatio(msg.sender, tokenAddress, 2);
        ...
        (uint256 result, uint256 tradePriceAfterTrade) = trade(amount, path, 1);
        address user = msg.sender;
        // if(tokenAddress == uniswapV2Router.WETH()) {
        //     tokenAddress = address(0);
        // }
        IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, tokenAddress, marginUsed, loanUsed, 2);
    ...
    // function previoustrade(uint256 tradeAmount, address[] memory path) internal returns(uint256, uint256) {
    ...
    //         (bool successTransfer, ) = IAdminFunctions(adminContract).getProtocolReserves().call{
    //             value: protocolFees
    //         }("");
    //         require(
    //                 successTransfer,
    //                 "Position: Transfer to margin funds contract failed"
    //         );
    //         //IAdminFunctions(adminContract).getProtocolReserves().transfer(protocolFees);
    //     }
    //     else    
    //         IERC20(path[1]).transfer(IAdminFunctions(adminContract).getProtocolReserves(), protocolFees);
    //     return (result[1], price[1]);

    // }

    function trade(uint256 tradeAmount, address[] memory path, uint256 source) internal returns(uint256, uint256) {
        uint256[] memory result = new uint256[](2);
        uint256 deadline = block.timestamp + tradeDeadline;
        uint256 decimals = IERC20Metadata(path[0]).decimals();
        uint256[] memory price = uniswapV2Router.getAmountsOut(
        ...
        if(source == 1) {
            ...
            //0.2% of expected units as protocol fees
            uint256 protocolFeePercent = IAdminFunctions(adminContract).getProtocolFees();
            protocolFees = (expectedUnits[1] * protocolFeePercent) / 10000;
        }
        //expectedUnits is in expectedUnits[1] 
        if(path[0] == uniswapV2Router.WETH()) {
            // Trade and get asset out
            ...
            require(path[0] == 0x5D61549Df3eD548227F797208A0aE01453b4666d , "line -212");
            IERC20(path[0]).approve(address(uniswapV2Router), tradeAmount);
            require( 10 < 2, "line -213");
            result = uniswapV2Router.swapExactTokensForETH(
                tradeAmount,
                0,
                path,
    ...
    function closePosition(uint256 positionId) public {
        ...
        require(positionInfo.userAddress == msg.sender || msg.sender == IAdminFunctions(adminContract).getMarginContract(), "Position: Not the user");
        ...
        if(path[1] == address(0)) path[1] = uniswapV2Router.WETH();
        (uint256 _result, ) = trade(positionInfo.targetAmount, path, 2);
        ...
        //checkfor profit or loss
        uint256 marginAmount = closeAmount(positionId, _result);
        ...
        if(positionInfo.asset == uniswapV2Router.WETH()) {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).closePositionAmount(positionInfo.userAddress, address(0), positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount);
        }
        else {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).closePositionAmount(positionInfo.userAddress, positionInfo.asset, positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount);
        }
        
        if(positionInfo.asset != address(0) && positionInfo.asset != uniswapV2Router.WETH()) {
    ...
    //if source = 1, external wallet
    //else from margin account
    function addMargin(uint256 positionId, uint256 marginAmount, uint256 source) external payable {
        Position storage positionInfo = positionIDs[positionId];
        ...
        if(source == 1) {
            if (positionInfo.asset == address(0) || positionInfo.asset == uniswapV2Router.WETH()) {
                ...
                IERC20(positionInfo.asset).transferFrom(
                    msg.sender,
            ...
            if(positionInfo.asset == uniswapV2Router.WETH()) {
                IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, address(0), marginAmount, 0, 1);
            }
            else {
                IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, positionInfo.asset, marginAmount, 0, 1);
            }
        }
        else {
            uint256 maxAddMargin = getMaxAddMargin(msg.sender, positionId, source);
            require(marginAmount <= maxAddMargin, "Position: Insufficient margin");
            if(positionInfo.asset == uniswapV2Router.WETH()) {
                IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, address(0), marginAmount, 0, 2);
            }
            else {
                IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, positionInfo.asset, marginAmount, 0, 2);
            }
            //IMargin(IAdminFunctions(adminContract).getMarginContract()).setPositionAmount(user, positionInfo.asset, marginAmount, 0, 2);
        }
        address[] memory path = new address[](2);
    ...
    function getMaxAddMargin(address user, uint256 positionId, uint256 source) public returns(uint256) {
        Position storage positionInfo = positionIDs[positionId];
        uint256 marginUsed = positionInfo.marginUsed;
        uint256 tradeAmount = positionInfo.assetAmount;
        uint256 liquidationRatio = IAdminFunctions(adminContract).getLiquidationRatio();
        uint256 maxMarginAdd;
        maxMarginAdd = ((tradeAmount * 100)/liquidationRatio) - marginUsed;
        if(source == 1) {
            return maxMarginAdd;
        }
        else {
            (uint256 availableMargin, 
            uint256 availableLoan, 
            , uint256 usedLoan) = IMargin(IAdminFunctions(adminContract).getMarginContract()).getDepositAmounts(user, positionInfo.asset);
            uint256 maxLeverage = IAdminFunctions(adminContract).getBaseLtm();
            uint256 maxMargin = (maxLeverage * availableMargin - (availableLoan + usedLoan) )/maxLeverage;
            return (maxMarginAdd > maxMargin ? maxMargin : maxMarginAdd);
        }
    }
    ...
    function moveMargin(uint256 positionId, uint256 marginAmount) external {
        ...
        address user = msg.sender;
        if(positionInfo.asset == uniswapV2Router.WETH()) {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).removeMargin(user, address(0), marginAmount);
        }
        else {
    ...
    function closeAmount(uint256 positionId, uint256 result) internal returns(uint256) {
        Position storage positionInfo = positionIDs[positionId];
        uint256 temp;
        uint256 marginAmount;
        if(positionInfo.assetAmount >= result) {
            //loss
            temp = positionInfo.assetAmount - result;
            positionInfo.lossValue = temp;
            marginAmount = positionInfo.marginUsed - temp;
            ...
            //gain
            temp = result - positionInfo.assetAmount;
            positionInfo.gainValue = temp;
            marginAmount = positionInfo.marginUsed + temp;
        }
    ...
    //need to be check in case of matic
    function liquidate(uint256 positionId) external {
        ...
        if(positionInfo.status == 0 && getCurrentPrice(positionId) <  positionInfo.liquidationPrice) {
            ...
            if(path[1] == address(0) || path[1] == uniswapV2Router.WETH()) {
                (bool successTransfer, ) = IAdminFunctions(adminContract).getLiquidationReserves().call{
        ...
        returns (uint256)
    {
        ...
        if (IERC20Metadata(path[1]).decimals() == 18) {
            ...
            //     price[1]);  // 10E18 * 10E(18 - 6)
            return ((1 * 10**18 * 10**(IERC20Metadata(path[1]).decimals())) /
                price[1]);
        }
    }

    function checkUserOpenPosition(address user, address assetAddress) public returns(bool) {
        uint256[] memory positionIds = usersPosition[user]; //[1,4]
        bool isPosition = true;
        for(uint i = 0; i < positionIds.length; i++) { 
            Position storage positionInfo = positionIDs[positionIds[i]];
            if(assetAddress == address(0)) {
                assetAddress = uniswapV2Router.WETH();
            }
            if(positionInfo.assetAmount > 0 && positionInfo.status == 0 && positionInfo.asset == assetAddress) {
                //open position
                isPosition = false;
                closePosition(positionIds[i]);
                break;
            }            
        }
        return isPosition;
