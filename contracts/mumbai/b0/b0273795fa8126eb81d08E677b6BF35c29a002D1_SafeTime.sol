...
contract SafeTime {
	...
	function _initTime(uint256 _externalTimeStampTrusted)  private  {
		...
		_t = Time.time(0, _externalTimeStampTrusted, block.number, 0);
		_timeIsInit = true;
		emit TimeInitialized(_externalTimeStampTrusted, block.number, _t);
	}

	function updateTime(Time.time memory t) internal  {
		_currentTime = _t._setTime(block.number);
	}
