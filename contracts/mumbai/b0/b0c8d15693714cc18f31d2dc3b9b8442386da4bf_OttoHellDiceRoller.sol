...
interface IStore is IOttopiaStore {
    function payWithMatic(string calldata key_, uint256 amount_)
        external
        payable;

    function payment(string calldata key_) external view returns (uint256);
}

contract OttoHellDiceRoller is
    BaseContractV2,
    IOttoItemEffect,
    IOttoItemFactoryV2,
    IERC1155ReceiverUpgradeable
{
    string public constant PAYMENT_KEY = 'dice';
    event RollDice(
    ...
    IStore public STORE;
    uint256 public DICE_PRODUCT_ID;
    uint256 public DICE_ITEM_ID;
    ...
    modifier summonedOtto(uint256 ottoId_) {
        require(
    ...
    function initialize(
        ...
        DICE_PRODUCT_ID = diceProductId_;
        DICE_ITEM_ID = diceItemId_;
    }

    function rollWithMatic(uint256 ottoId_, uint256 amount_)
        external
        payable
        onlyOttoOwner(ottoId_)
        summonedOtto(ottoId_)
    {
        require(amount_ == 1, 'amount must be 1');
        STORE.payWithMatic{value: msg.value}(PAYMENT_KEY, amount_);
        emit RollDice(msg.sender, ottoId_, 1);
    }

    function roll(uint256 ottoId_, uint256 amount_)
        external
        onlyOttoOwner(ottoId_)
        summonedOtto(ottoId_)
    {
        require(amount_ == 1, 'amount must be 1');
        uint256 totalPrice = STORE.discountPriceOf(DICE_PRODUCT_ID) * amount_;
        CLAM.transferFrom(_msgSender(), address(this), totalPrice);
        CLAM.approve(address(STORE), totalPrice);
        STORE.buy(address(this), DICE_PRODUCT_ID, amount_);
        for (uint256 i = 0; i < amount_; i++) {
            ITEM.transferToParent(
                address(this),
                address(OTTO),
                ottoId_,
                DICE_ITEM_ID,
    ...
    function shipNoChainlink(
        ...
        require(
            _msgSender() == address(STORE),
            'only STORE can call this function'
        );
        ITEM.mint(to_, DICE_ITEM_ID, productAmount_, '');
        emit ShipOrderNoChainLink(from_, fromProductId_, to_, productAmount_);
    }
    ...
    function applyTo(
        ...
        require(
            ...
            "Otto can't use this item"
        );
        emit RollDice(address(uint160(uint256(bytes32(data_)))), ottoId_, 1);
    }
