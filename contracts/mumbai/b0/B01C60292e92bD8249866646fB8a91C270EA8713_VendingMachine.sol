    ...
    EIP712MetaTransaction("VendingMachine", "1")
{
    ...
    uint256 public burntSeed = 1200;
    uint256 public breedeeFeeSeed = 500;
    uint256 public daoSeed = 300;
    address public daoAddress = 0x1DFdbb74d24A8B239F48891Ac883ef3859924979;

    ERC20 internal seedContract =
        ERC20(0x1DFdbb74d24A8B239F48891Ac883ef3859924979);

    constructor() {
        // Grant other privileges to the contract creator
        _setupRole(DEFAULT_ADMIN_ROLE, msgSender());
        grantRole(PRIV_MANAGE, msgSender());
        grantRole(PRIV_WITHDRAW, msgSender());
    }
    ...
    function _deductInventoryItem(
        ...
        require(inventoryEntry.quantity != UINT32_MAX, "VM: Infinite quantity");
    ...
    function breedSeed(
        ...
        uint32 quantity = inventoryEntries[_itemId].quantity;
        require(quantity != 0);

        // Split the SEED 3 ways to burn, breedee, and FlowerpatchDAO
        seedContract.burn(msgSender(), burntSeed);
        ...
        emit BreedSeed(
            msgSender(),
            _breedee,
            burntSeed + breedeeFeeSeed + daoSeed,
            _varData
        );
    }
    ...
    function setBurntSeed(uint256 _fee) external onlyRole(PRIV_MANAGE) {
        burntSeed = _fee;
    }
