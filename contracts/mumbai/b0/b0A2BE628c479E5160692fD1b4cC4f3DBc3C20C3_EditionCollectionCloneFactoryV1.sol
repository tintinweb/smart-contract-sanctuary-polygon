...
import "./EditionCollectionInitializableV1.sol";

/**
 * @title A Factory contract that can create new clones of `EditionCollectionInitializableV1`
 ...
 * that allows to create new clones of the `EditionCollectionInitializableV1` contract.
 *
 * On calling the {createEditionCollection} new clones of `EditionCollectionInitializableV1`
 * are created on behalve of a creator. All parameters are forwared to the initializer
 * of `EditionCollectionInitializableV1`
 * The clones are not upgradeable because the implementation contracts address is hardcoded
 * in the bytecode of the clone.
 */
contract EditionCollectionCloneFactoryV1 is AccessControl{
    EditionCollectionInitializableV1[] private editionCollections;

    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");

    address public immutable editionCollectionImplementation;

    /**
     * @dev Emitted when a `manager` created a new `EditionCollectionInitializableV1` through
     * a call to {createEditionCollection}.
     * `editionCollection` is the address of the new `EditionCollectionInitializableV1`.
     * been created.
     */
    event EditionCollectionCreated (
        address indexed editionCollection
    );


    /**
     * @notice Constructs the `EditionCollectionCloneFactoryV1`.
     * @dev This contract should only be deployed once.
     ...
     * Can create new `EditionCollectionInitializableV1` through {createEditionCollection}
     */
    constructor(
        address _roleAdmin,
        address _manager
        ...
        editionCollectionImplementation = address(new EditionCollectionInitializableV1());
    }

    // External functions

    /**
     * @notice Create a new clone of `EditionCollectionInitializableV1`
     * @dev clones of `EditionCollectionInitializableV1` are created by the `manager`
    ...
    function createEditionCollection(
        EditionCollectionInitializableV1.InitializeData calldata _init
    ) external onlyRole(MANAGER_ROLE) returns (address) {
        address clone = Clones.clone(editionCollectionImplementation);
        EditionCollectionInitializableV1(clone).initialize(_init);
        editionCollections.push(EditionCollectionInitializableV1(clone));
        emit EditionCollectionCreated(clone);
        return clone;
    }
    ...
    /**
     * @notice Get address of previously created `EditionCollectionInitializableV1` clones
     * @dev this function is used for the fuel-blockchain-listner
     * event handler get all created `EditionCollectionInitializableV1` in order to
     * register event handlers.
     * @param _i index in the collection
     * @return the address of EditionCollectionInitializableV1
     */
    function getCollection(uint256 _i) external view returns(address) {
        return address(editionCollections[_i]);
    }

    /**
     * @notice Get how many `EditionCollectionInitializableV1` clones have previously been created.
     * @dev this function is used for the fuel-blockchain-listner
     * event handler to get all created `EditionCollectionInitializableV1` in order to
     * register event handlers.
     * @return length of the `editionCollections` array
     */
    function collectionsLength() external view returns(uint256) {
        return editionCollections.length;
    }
}
...
import "./EditionCollectionRoyaltiesV1.sol";

/**
 * @title Fuel Initializable implementation of ERC1155
 * @author https://www.onfuel.io
 * @dev this contract will not be deployed directly.
 * New clones shall be created through `EditionCollectionCloneFactoryV1`.
 ...
 *  [x] ERC-1155 contract with proxy setup, as it will be reusable in the future
 *  Collection to have the following attributes
 *  - [x] Sale Start / End time … end can be set unlimited
 *  - [x] Royalty fee
 *  - [x] Allowlist merkle root
 *  NFTs (editions) to have the following attributes
 *  - [x] Unique metadata per each NFT
 *  - [x] Supply (fixed or unlimited)
 *  - [x] Price
 *  - [x] Ability to keep adding NFTs to the contract, even after publishing
 *  - [x] Manager mint
 *  - [x] Public mint
 *  - [ ] Ensure contract is OpenSea compatible and it shows up on OpenSea
 *  - [x] Withdrawal function
 *
 *  Ability to update the following fields anytime
 *  - [x] Sale start and end date
 *  - [x] Allowlist merkle root
 *
 *
 * Accounts:
 ...
 * - can call {setPublicMintEndBlockHeight}
 * - can call {setSaleStateAndBlockHeights}
 * - can call {setRoyaltyRecipient}
 * - can call {decreaseRoyaltyPercentage}
 * - can call {setBaseURI}
 * - can call {setBaseURIBatch}
 * - can call {addEdititionBatch}
 * - can call {addEditition}
 * - can call {setAllowListMerkleRoot}
 * - can call {managerMint}
 * - can call {pause}
 */
// ERC1155Upgradeable,
contract EditionCollectionInitializableV1 is
    Initializable,
    ERC1155SupplyUpgradeable,
    EditionCollectionRoyaltiesV1,
    ...
    PausableUpgradeable
{
    ...
    */
    ...
    /**
    * @dev the block.number uintil the {publicMint} can be called
    * to buy tokens with ETH/MATIC.
    *
    * N == 0            -> {publicMint} is unlimited (open forever)
    * N >  block.number -> {publicMint} is open
    * N <= block.number -> {publicMint} is over (disabled)
    */
    uint128 public publicMintEndBlockHeight;
    ...
    /**
     * @dev baseURIs for token metadata
     * Can be changed after initialized through {setBaseURI} {setBaseURIBatch}
     */
    ...
    /**
    * @dev Emitted when a `manager` changes the publicMintEndBlockHeight
    * through a call to {toggleSaleState} or {setPublicMintEndBlockHeight}.
     */
    event PublicMintEndBlockHeightChanged(
        uint128 publicMintEndBlockHeight
    );
    ...
    /**
     * @notice Struct for initializing the `EditionCollectionInitializableV1`.
     * @param _roleAdmin address of the account that will have `DEFAULT_ADMIN_ROLE`.
     * Can grant and revoke roles to other accounts on the new `EditionCollectionInitializableV1`.
     * Can NOT mint, pause, ect. on the new `EditionCollectionInitializableV1`.
     * @param _manager account that will have `MANAGER_ROLE` on the new EditionCollectionV`.
    ...
    /**
     * @notice initialize the `EditionCollectionInitializableV1`.
     * @dev clones of this contract are created by the `manager`
     * account of fuel-core through it's `EditionCollectionCloneFactoryV1` contract
    ...
    function initialize(
       InitializeData calldata _init
        ...
        require(
            _init.maxSupply.length == _init.mintPrice.length
            &&
            _init.maxSupply.length == _init.baseURI.length,
            "Array lengh mismatch"
        );

        // DONT __ERC1155_init(); , its just a waste of storage space.
        ...
        string[] memory _metadataUris = _init.baseURI;
        baseURIs = _metadataUris;
    ...
    modifier tokensLeft(uint256 tokenId, uint256 ammountToMint) {
        require(maxSupply.length > tokenId, "Edition invalid");
        require(
            // maxSupply[tokenId] == 0 ||
            // totalSupply(tokenId) <= maxSupply[tokenId],
            supplyLeft(tokenId) >= ammountToMint,
            "Purchase would exceed max tokens");
        _;
    }


    modifier canStillMint(address to, uint256 tokenId, uint256 amount) {
        if (maxMintPerAddress > 0) {
            require(
                minted[to][tokenId] + amount <= maxMintPerAddress,
    ...
    function supplyLeft(uint256 _tokenId) public view returns (uint256) {
        uint256 _maxSupply = maxSupply[_tokenId];
        if (_maxSupply == 0) {
            // MAX_UINT - minted
            uint256 MAX_INT = 0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff;
            return MAX_INT - totalSupply(_tokenId);
        }
        return _maxSupply - totalSupply(_tokenId);
    }

    // External functions

    /**
     * @notice publish the `EditionCollectionInitializableV1` as active for sale.
    ...
    /**
     * @notice see {_setPublicMintEndBlockHeight}
     */
    function setPublicMintEndBlockHeight(
        uint128 _publicMintEndBlockHeight
    ) external onlyManager {
        _setPublicMintEndBlockHeight(_publicMintEndBlockHeight);
    }
     ...
     */
    function setBaseURI(
        uint256 _indexOfUriToUpdate,
        string calldata _newBaseURI
    ) external onlyManager {
        require(_indexOfUriToUpdate < baseURIs.length);
        baseURIs[_indexOfUriToUpdate] = _newBaseURI;
        emit URI(_newBaseURI, _indexOfUriToUpdate);
    }

    function setBaseURIBatch(
        uint256[] calldata _indicesOfUrisToUpdate,
        string[] calldata _urisToUpdate
    ) public onlyManager {
        require(_indicesOfUrisToUpdate.length > 0, "empty indices");
        require(_indicesOfUrisToUpdate.length == _urisToUpdate.length, "array length mismatch");
        uint metadataUrisLength = baseURIs.length;
        require(metadataUrisLength > 0);
        for(uint i = 0; i < _indicesOfUrisToUpdate.length; i++) {
            uint index = _indicesOfUrisToUpdate[i];
            require(index < metadataUrisLength, "invalid tokenId");
            string memory newBaseUri = _urisToUpdate[i];
            baseURIs[index] = newBaseUri;
            emit URI(newBaseUri, index);
        }
    }

    function addEdititionBatch(
        uint64[] calldata _maxSupplies,
        uint112[] calldata _mintPrice,
        string[] calldata _tokenUris
    ) public onlyManager {
        require(_maxSupplies.length == _mintPrice.length, "array length mismatch");
        require(_maxSupplies.length == _tokenUris.length, "array length mismatch");
        for(uint i = 0; i < _maxSupplies.length; i++) {
            maxSupply.push(_maxSupplies[i]);
            mintPrice.push(_mintPrice[i]);
            string memory newBaseUri = _tokenUris[i];
            baseURIs.push(newBaseUri);
            emit URI(newBaseUri, maxSupply.length - 1);
        }
    }

    function addEditition(
        uint64 _maxSupply,
        uint112 _mintPrice,
        ...
        baseURIs.push(_tokenUri);
        uint256 tokenId = maxSupply.length-1;
        emit EditionCreated(tokenId);
    ...
    /**
     * @notice update the root hash of the merkletree for allowlist
     * @param _allowListMerkleRoot the merkle root hash
     */
    function setAllowListMerkleRoot(bytes32 _allowListMerkleRoot) external onlyManager {
     ...
     * through use of the _mint internal function.
     * @param to address of the receiver of the newly minted token
     */
    function managerMint(address to, uint256 tokenId, uint256 amount)
        external
        isSaleActive
        tokensLeft(tokenId, amount)
        canStillMint(to, tokenId, amount)
        onlyManager
    {
    ...
    function allowListMint(
        ...
        isSaleActive
        tokensLeft(tokenId, amount)
        canStillMint(_msgSender(), tokenId, amount)
    {
        require(
        ...
        if(maxAllowListMintPerAddress > 0) {
            require(
                minted[_msgSender()][tokenId] < maxAllowListMintPerAddress,
                "already minted"
            );
        }
        require(msg.value >= mintPrice[tokenId]*amount, "price too low");
        minted[_msgSender()][tokenId]+=amount;
        ...
        canStillMint(_msgSender(), tokenId, amount)
    {
        ...
        require(
            publicMintEndBlockHeight == 0 || block.number < publicMintEndBlockHeight,
            "public sale over"
        );
        require(msg.value >= mintPrice[tokenId]*amount, "price too low");
        minted[_msgSender()][tokenId]+=amount;
    ...
    function getEditionData(uint256 tokenId) public view returns (
        uint256 _currentSupply,
        uint256 _maxSupply,
        uint256 _mintPrice,
        string memory _baseURI
    ) {
        require(mintPrice.length > tokenId, "Edition invalid");
        _currentSupply = totalSupply(tokenId);
        _maxSupply = maxSupply[tokenId];
        _mintPrice = mintPrice[tokenId];
        _baseURI = baseURIs[tokenId];
    }
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        require(tokenId < baseURIs.length, "Edition invalid");
    ...
    function supportsInterface(bytes4 interfaceId)
        public
        view
        override(ERC1155Upgradeable, RoyaltiesV1, AccessControlUpgradeable)
        returns (bool)
    {
        return
    ...
    function _setPublicMintStartBlockHeight(
        ...
        require(
            publicMintEndBlockHeight == 0
            ||
            publicMintEndBlockHeight > _publicMintStartBlockHeight,
            "blockheight invalid"
        );
    ...
    /**
     * @notice setter for publicMintStartBlockHeight
     * @param _publicMintEndBlockHeight the block.number at which
     * {publicMint} will become enabled. 0 means disabled.
     */
    function _setPublicMintEndBlockHeight(
        uint128 _publicMintEndBlockHeight
    ) internal {
        require(
            _publicMintEndBlockHeight > publicMintStartBlockHeight,
            "blockheight invalid"
        );
        publicMintEndBlockHeight = _publicMintEndBlockHeight;
        emit PublicMintEndBlockHeightChanged(_publicMintEndBlockHeight);
    }
     ...
     * @param ids array of IDs of the tokens to be sent
     */
...
contract EditionCollectionRoyaltiesV1 is RoyaltiesV1 {
