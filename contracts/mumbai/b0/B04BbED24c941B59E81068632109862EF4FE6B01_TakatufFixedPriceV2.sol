    ...
    ReentrancyGuardUpgradeable
{
    ...
    ) public nonReentrant returns (uint256 offerId_) {
        (
     ...
     *@param offerId_ OfferId of NFT
     */
    function cancelSale(uint256 offerId_) external nonReentrant{
        if (_marketplaceManager.isPaused()) revert GeneralError("TKF:128");
        if ((_marketplaceManager.isBlocked(msg.sender) == true))
            revert GeneralError("TKF:126");
        if (
     ...
     */
    function buyNft(uint256 offerId_, uint256 quantity_, address _buyer) external payable nonReentrant {
        if (_marketplaceManager.isPaused()) revert GeneralError("TKF:128");
        if ((_marketplaceManager.isBlocked(_buyer) == true))
            revert GeneralError("TKF:126");
        TakatufIMarketplace.Offer memory offer = TakatufIMarketplace(
            _marketplace
        ).getOfferStatus(offerId_);
        ...
        Payout memory payoutData = _buyNft(
            BuyNFT(offerId_, _buyer, quantity_, msg.value)
        );
    ...
    ) internal nonReentrant returns (Payout memory payout_) {
