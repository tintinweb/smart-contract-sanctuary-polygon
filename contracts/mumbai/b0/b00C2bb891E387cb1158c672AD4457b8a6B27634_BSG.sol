...
contract BSG {
    ...
    uint256 private constant maxAddFreeze = 3 minutes;
    ...
    function getCurReward(address _user) public view returns(uint256){
        (uint256 staticReward ,) = _calCurStaticRewards(_user);
        (uint256 dynamicReward , ) = _calCurDynamicRewards(_user);
        return rewardInfo[_user].capitals.add(staticReward.add(dynamicReward));
    }
