...
import "./ICrowdfundingControl.sol";
...
contract Calcs is AccessControl {
    ...
    ICrowdfundingControl public CrowdControl; 

    /// @dev Constructor to set the Roles and the Lending Control Contract address.
    /// @param _crowdControl the address of the Lending Control Contract.
    constructor(address _crowdControl){
        ...
        CrowdControl = ICrowdfundingControl(_crowdControl);
    }
    ...
    function calcInterestAccumulated(address _wallet,uint256 _idLending) public view returns(uint256 interesToPay, uint256 timeOfCalc){
        (uint256 amount, , uint96 lastCalcTime, uint256 payPerSecond, , , bool isFullyPaid) = CrowdControl.getLenderInfo(_wallet, _idLending);
    ...
    /// @param _amountDeposit - the amount to withdraw.
    /// @return totalInterest - the penalization amount with 18 decimals.
    function calcTotalPayInterest(uint256 _amountDeposit, uint16 _interes) public pure returns(uint256 totalInterest){
        PRBMath.UD60x18 memory amount = PRBMath.UD60x18({value: _amountDeposit * 1e12});
        PRBMath.UD60x18 memory AmountxPercentage = amount.mul(PRBMath.UD60x18({value: _interes}));
        PRBMath.UD60x18 memory annualPay = AmountxPercentage.div(PRBMath.UD60x18({value: 10000}));
        PRBMath.UD60x18 memory totalPay = annualPay.mul(PRBMath.UD60x18({value: 3e18 })); //3 years

        totalInterest = totalPay.value;
    }
    ...
    function updateCrowdfundingControl(address _LendingControl) external onlyDev {
        CrowdControl = ICrowdfundingControl(_LendingControl);
    }
...
interface ICrowdfundingControl is IFounders {

    /// @dev Function to register a new user and his deposit in Lending/Earn.
    function addRegistry(address wallet, uint256 _amount, uint256 _payPerSecond, uint256 _totalToPaid, uint32 _totalNFTsBuyed) external returns(uint256 _id);
    ...
    function getLenderInfo(address wallet, uint256 id) external view returns (uint256 , uint256 , uint96 , uint256 , uint96 , bool , bool );
    ...
    function getRewardsClaimed(address wallet, uint256 id) external view returns (uint256 rewards, uint96 claimTime, uint96 lastTimeClaim,  uint256 totalToPaid);
    ...
    function updatePayPerSecond(address _wallet, uint256 _idCrowd ,uint256 _newPayPerSecond) external;

    /// @dev Function to update the flag that validate if all the interest has been paid to the user.
    function setAllInteresPaid(address _wallet, uint256 _idCrow, bool _state) external;

    /// @dev Function to update the last time the interest to be paid to the user was calculated.
    function updateLastTimeClaim(address wallet, uint256 _idCrowd, uint256 _timeClaim) external;

    /// @dev Function to get the pending interests to be paid to the user.
    function getPendingRewards(address _wallet, uint256 _idCrowd) external view returns(uint256);
    ...
    function updatePendingRewards(address wallet, uint256 _idCrowd, uint256 _timeClaim) external;

    /// @dev Function to get the date of the last time a user set a request to withdraw the generated interests.
    function getLastSetClaimRewards(address _wallet,uint256 _idCrowd) external view returns(uint96 lastSetClaimRewards);
