...
//可暂停武器铸造合约
contract WeaponFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    constructor(address _mgge, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        //初始化
        //近战主武器1（攻击） 
        uint _type_id = _addItem(100001,200,400,"attack");
        _setPropValues(_type_id, "attack",250,300);
        ...
        //近战主武器2（防御） 
         _type_id = _addItem(100002,150,410,"defense");
        _setPropValues(_type_id, "attack",150,200);
        _setPropValues(_type_id, "defense",70,100);
        _setPropValues(_type_id, "hp",20,50);
        _setPropValues(_type_id, "duration",10,50);
        _setPropValues(_type_id, "potential",1,5);
        //近战主武器3（均衡） 
         _type_id = _addItem(100003,100,420,"duration");
        _setPropValues(_type_id, "attack",100,190);
        _setPropValues(_type_id, "defense",10,30);
        _setPropValues(_type_id, "hp",10,30);
        _setPropValues(_type_id, "duration",10,50);
        _setPropValues(_type_id, "potential",1,5);
        //远程主武器1（攻击） 
         _type_id = _addItem(100501,200,401,"attack");
        ...
        _setPropValues(_type_id, "potential",1,5);
        //远程主武器2（生命） 
         _type_id = _addItem(100502,150,411,"hp");
        ...
        _setPropValues(_type_id, "potential",1,5);
        //远程主武器3（均衡） 
         _type_id = _addItem(100503,100,421,"duration");
    ...
    function _addItem(
        ...
        weapons[_type_id] = Weapon (
            _item_config_id,      //物品id 
            _weight,              //权重
            _prop_sum,            //综合值
            0,                    //计数器
            _hash_margin_name          //余量
        );
        return _type_id;
    }
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    // 合约操作员设置upgrade_base
    function setUpgradeMin(uint256 _min) external nonReentrant onlyOperator returns(bool){
        upgrade_base = _min;
        return true;
    }

    // 合约操作员设置upgrade_max
    function setUpgradeMax(uint256 _max) external nonReentrant onlyOperator returns(bool){
        upgrade_max = _max;
        return true;
    }
    ...
    function setWeight(uint256 _type_id, uint256 _weight) external nonReentrant onlyOperator returns(bool){
        weapons[_type_id].weight = _weight;
        return true;
    }
        ...
        returns(uint[] memory)
    {
        ...
        for (uint i = 0; i < _typeCount; i++) {
             if (weapons[i].count < weapons[i].prop_sum) {
                 _count ++;
             }
        }
        if (_count == 0) {
        ...
        require(
            weapons[_type_id].margin_name == hash_attack ||  
            weapons[_type_id].margin_name == hash_defense ||  
            weapons[_type_id].margin_name == hash_hp ||
            weapons[_type_id].margin_name == hash_duration, 
            "wrong margin_name"
        );
        ...
        uint256 potential;      //潜力 1-5

        Weapon storage weapon = weapons[_type_id];
        weapon.count +=1;
        uint new_rand;
        ...
        uint margin = weapons[_type_id].prop_sum - (props[hash_attack] +props[hash_duration] + props[hash_defense] +props[hash_hp]);
        props[weapons[_type_id].margin_name] +=  margin;
