...
contract SnookState is ISnookState, Initializable {
  event DescriptorSet(uint tokenId, uint stars, uint traitCount, uint lives); // DEBUG ONLY: defining an event
  ...
  function _setDescriptor(uint tokenId, Descriptor calldata descriptor) internal {
    emit DescriptorSet(tokenId, descriptor.stars, descriptor.traitCount, descriptor.lives); // DEBUG ONLY: emitting event
