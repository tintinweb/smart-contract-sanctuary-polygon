    ...
    ERC2771ContextUpgradeable
{
    ...
    ClaimCondition internal _claimCondition;
    address payable public beneficiary;

    /// prevent implementation-contract init() from getting called
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(Forwarder forwarder) ERC2771ContextUpgradeable(address(forwarder)) {
    ...
    function initialize(
        ...
        // set default claim condtion and beneficiary
        _claimCondition = claimCondition;
        beneficiary = payable(beneficiary_ == address(0) ? owner : beneficiary_);
    ...
    function give(
        ...
        require(owner() == _msgSender(), "give: unauthorized issuer");
        require(_msgSender() != to, "cannot give to self");
    ...
    function take(
        ...
        _requireClaimable();
        require(owner() == from, "take: unauthorized issuer");
    ...
    /// @notice claim a SBT by markle tree whitelist
    /// @param proof markle proof
    function claim(bytes32[] calldata proof) external payable virtual {
        _requireClaimable();

        // validate merkle proof
        if (_claimCondition.merkleRoot != bytes32(0)) {
            bytes32 merkleLeaf = keccak256(abi.encodePacked(_msgSender()));
            require(MerkleProofUpgradeable.verify(proof, _claimCondition.merkleRoot, merkleLeaf), "Invalid proof");
        }

        // validate fee
        uint256 fee = _claimCondition.pricePerToken * 1;
        if (fee > 0) {
            // native token
            if (_claimCondition.tokenAddress == NATIVE_TOKEN) {
                require(msg.value == fee, "Invalid price");
                // native token is transferred automatically due to payable method
            } else {
                // ERC20 token
                require(
                    IERC20(_claimCondition.tokenAddress).allowance(_msgSender(), address(this)) >= fee,
                    "Token not allowed"
                );

                // transfer fee
                IERC20(_claimCondition.tokenAddress).transferFrom(_msgSender(), address(this), fee);
            }
        }

        // validate period time
        if (_claimCondition.startTime > 0) require(_claimCondition.startTime <= block.timestamp, "Invalid period");
        if (_claimCondition.endTime > 0) require(_claimCondition.endTime >= block.timestamp, "Invalid period");
    ...
    /// @notice set claim condition
    /// @param condition a claim condition includes merkleRoot, pricePerToken and tokenAddress
    function setClaimCondition(ClaimCondition calldata condition) external override onlyOwner {
        _claimCondition = condition;

        emit ClaimConditionUpdated(_claimCondition);
    }

    /// @notice withdraw claim fee
    /// @param tokenAddress native token address or ERC20 address
    function withdraw(address tokenAddress) external override onlyOwner {
        ...
        if (tokenAddress == NATIVE_TOKEN) {
            ...
            // in case of withdraw, it needs to execute `approve()` on the token contract to allow itself the transfer
            tokenContract.approve(address(this), balance);
            tokenContract.transferFrom(address(this), beneficiary, balance);
        }
    }
     ...
     * @param beneficiary_ The beneficiary address to be set
     */
    function setBeneficiary(address beneficiary_) public virtual onlyOwner {
        require(beneficiary_ != address(0x0), "Invalid beneficiary");
...
interface IClaimCondition {
    ...
    event Withdrawal(address indexed to, address tokenAddress, uint256 amount);

    event ClaimConditionUpdated(ClaimCondition condition);

    function setClaimCondition(ClaimCondition calldata condition) external;
