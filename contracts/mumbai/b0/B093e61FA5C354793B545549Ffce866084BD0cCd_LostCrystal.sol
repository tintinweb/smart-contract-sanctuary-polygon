...
contract LostCrystal is CustomERC2771ContextUpgradeable, ERC20Upgradeable, ERC20BurnableUpgradeable, UUPSUpgradeable {
    // Lost Chain RBA Linker
    ILostChainRBA private LCRBALinker;
    modifier onlyRole(string memory name, address account) {
        require(LCRBALinker.checkAccountRole(name, account), 'LC: Unauthorized');
        _;
    }

    function setRBALinker(address rbaLinkerAddress) public onlyRole('DEFAULT_ADMIN_ROLE', _msgSender()) {
        require(rbaLinkerAddress != address(0x0), 'LC: RBA linker is zero address');
    ...
    function initialize(
        ...
        require(address(forwarder) != address(0x0), 'LC: Forwarder is zero address');
        require(rbaLinkerAddress != address(0x0), 'LC: RBA linker is zero address');
        LCRBALinker = ILostChainRBA(rbaLinkerAddress);
        vault = vaultAddress;
        initializeERC2771(address(forwarder));

        __ERC20_init('Lost Crystal', 'LC');
        __ERC20Burnable_init();
        __UUPSUpgradeable_init();
        _mint(vault, 100000000000 * 10 ** decimals());
    }
    ...
    function getUserBalance(address playerAddreess) external view returns (uint256) {
        return balanceOf(playerAddreess);
    }
    ...
    function gameDataTokenTransfer(
        ...
        require(_msgSender() == LCGD, 'LC: caller is not LCGD');
        _transfer(from, to, amount);
    }

    // TEST FUNCTION TO ALTER BYTECODE (RE-VERIFY)
    function TESTING60() external pure returns (bool) {
        return true;
    }
}
