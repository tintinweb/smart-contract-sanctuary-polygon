...
library ERC721AStorage {
    ...
    struct Layout {
        ...
        // token progress
        mapping (uint256 => uint256) _progress;
    ...
    bytes32 internal constant STORAGE_SLOT = keccak256('ERC721A.contracts.storage');
...
import {LibDiamond} from "src/libraries/LibDiamond.sol";
import {ERC721AUpgradeable} from "../ERC721AUpgradeable/ERC721AUpgradeable.sol";



contract profileNftFacet is ERC721AUpgradeable {
   error ProfileNFT__isAddress0();
    error ProfileNFT__isAlreadyOwner();
    error ProfileNFT__isNotOwner();
    error ProfileNFT__tokenIdNotExists();
    error ProfileNFT__notCalledFromDiamond();
    error ProfileNFT__alreadyInitialized();

    event ProfileNFTInitialized();

    function s() internal pure returns (ERC721AStorage.Layout storage) {
    ...
    modifier onlyOwnerCan() {
		if(LibDiamond.contractOwner() != msg.sender) revert ProfileNFT__isNotOwner();
		_;
	}
    modifier callingFromDiamond() {
        if(address(this) != s()._diamondAddress) revert ProfileNFT__notCalledFromDiamond();
        _;
    }
    modifier idDoExists(uint256 id) {
        if(_ownerOf(id) == address(0)) revert ProfileNFT__tokenIdNotExists();
        _;
    }
    


    function init() external onlyOwnerCan {
        if (s()._isInitialized == true) revert ProfileNFT__alreadyInitialized();
        
        s()._name = "Metadev3 Le Club - Profile NFT";
        s()._symbol = "M3CA";
        s()._currentIndex = 0;
        s()._burnCounter = 0;
        s()._diamondAddress = address(this);
        ...
        emit ProfileNFTInitialized();
    }

    function awardNFT(address recipient, string memory tokenURI) public callingFromDiamond returns (uint256) {
		uint256 currentIndex = s()._currentIndex;
		_safeMint(recipient, 1);
		_setTokenURI(currentIndex, tokenURI);

		return currentIndex;
	}

    function burn (uint256 tokenId) public onlyOwnerCan idDoExists(tokenId) {
        _burn(tokenId);
    }

    function setTokenURI(uint256 tokenId, string memory _tokenURI)  
        public onlyOwnerCan idDoExists(tokenId) 
    {
    ...
    function tokenProgress(uint256 tokenId)  public view virtual returns (uint256) {
        return s()._progress[tokenId];
    }

    function setTokenProgress(uint256 tokenId, uint256 progress)  
        public onlyOwnerCan idDoExists(tokenId) callingFromDiamond
    {
        s()._progress[tokenId] = progress;
    }

    function diamondAddress() external view returns (address) {
        return s()._diamondAddress;
    }
