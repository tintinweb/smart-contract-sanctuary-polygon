...
import "./NonFungibleCollection.sol";
...
contract NFTFactory is Initializable, OwnableUpgradeable {
    address private nonFungibleBeacon;
    ...
    mapping(address => CollectionData) public collectionDetails;
    ...
    function initialize(
        address _nonFungibleBeacon,
        address _semiFungibleBeacon
    ) external initializer {
        nonFungibleBeacon = _nonFungibleBeacon;
    ...
    function createCollection(
        ...
        if (_collectionType == CollectionType.NonFungible) {
            address newCollection = address(
                new BeaconProxy(
                    nonFungibleBeacon,
                    abi.encodeWithSelector(
                        NonFungibleCollection.initialize.selector,
                        _name,
                        _symbol
                    )
                )
            );
            NonFungibleCollection(newCollection).transferOwnership(msg.sender);
            collectionDetails[newCollection] = CollectionData(
                msg.sender,
                _collectionType
            );
        ...
        } else if (_collectionType == CollectionType.SemiFungible) {
            address newCollection = address(
                new BeaconProxy(
                    semiFungibleBeacon,
                    abi.encodeWithSelector(
...
contract NonFungibleCollection is
    Initializable,
    ERC721Upgradeable,
