...
contract chainlordsStaking is  Ownable {
    ...
    // address tempUser = 0xd9145CCE52D386f254917e481eB44e9943F39138;
    ...
    function unstake(address tokenAddress, uint256 amount) public whenOpenForStaking{
        ...
        users[msg.sender].amountPerToken[tokenAddress].sub(amount);
        users[msg.sender]._depositBlocks[tokenAddress] = block.number;
    ...
    function getUser(address userAddress) public returns ( bool ) {
        if(users[userAddress].exists ==false){
            return true;
        }else{
          return  false;
        }
    ...
    function calculateUserTotalRewards2(address userAddress) public view returns(uint256){
        // users[userAddress].rewardAmount = users[userAddress].rewardAmount.add(getUserNewRewards(userAddress));
        address[] memory tokenList = getListOfTokens();
        for(uint256 i=0; i<tokenList.length;i++){
            address tokenAddress = tokenList[i];
    ...
    function getUserNewRewards(address userAddress) public view returns(uint256){
        ...
        for(uint256 i=0; i<numberOfTokens;i++){
            address tokenAddress = tokenList[i];
            if(users[userAddress].amountPerToken[tokenAddress]>0){
                uint256 currentAmmount = getAmountPerToken(userAddress,tokenAddress);
                rewards = rewards.add(calculateRewards(getBlock(userAddress, tokenAddress), tokenAddress, currentAmmount));
            }
        }
        return rewards;
    }
    ...
    function getAmountPerToken(address user, address tokenAddress) public view returns(uint256){
        return users[user].amountPerToken[tokenAddress];
    }

    function getBlock(address user, address tokenAddress) public view returns(uint256){
    ...
    function setRewardToken(address rewardTokenAddress, uint256 rewardTokenAPY) public onlyOwner returns(bool){
