...
interface IFractions is IERC1155 {
	...
	function setLiquidationStatus(uint256 tokenId) external;
...
import "./TestTokenSplitter.sol";
import "./interfaces/IFractionsSale.sol";

contract TestTimedTokenSplitter is TestTokenSplitter {
	...
	constructor(
		...
		address seller_
	) TestTokenSplitter(redemptionToken_, token_, saleId_, tokenPrice_) {
	...
	function releaseSeller() public onlyWhileSaleClosed onlySuccessfulSale {
	...
	function release(address[] calldata users) public override(TestTokenSplitter) onlyWhileSaleClosed onlyFailedSale {
		super.release(users);
	}
}
