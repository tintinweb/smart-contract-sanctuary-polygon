...
// File: contracts/DeployOld/EIP712Base.sol
...
// File: contracts/DeployOld/NativeOwnerApproval.sol

pragma solidity ^0.8.18;



contract NativeOwnerApproval is EIP712Base, OwnableUpgradeable {
    ...
    function checkOwnerApproval(bytes memory userData, bytes32 sigR, bytes32 sigS, uint8 sigV) internal {
    ...
    function hashOwnerApproval(OwnerApproval memory ownerApprovalTx) internal pure returns (bytes32) {
        return
            keccak256(
                abi.encode(
                    OWNER_APPROVAL_TYPEHASH,
                    ownerApprovalTx.nonce,
                    ownerApprovalTx.from,
                    keccak256(ownerApprovalTx.userData)
                )
            );
    }
...
// File: contracts/DeployOld/PurchaseBNZ.sol
...
// Purchase BNZ
contract PurchaseBNZ is NativeOwnerApproval, UUPSUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /* ========== EVENTS ========== */
    event WithdrawnBNZ(address indexed ownerAddress, uint256 amount);
    event WithdrawnEther(address indexed ownerAddress, uint256 amount);
    event BoughtBNZ(address indexed buyer, uint256 amount);

    function initialize(address _bnz) public virtual initializer {
        _initializeEIP712('PurchaseBNZ', '1');
    ...
    /** Buy BNZ From the Contract
     * @param amountOfBNZ - Amount of BNZ To Buy
     * @param deadline - DeadLine to complete the transaction
     * @param sigR - Owner Signature R
     * @param sigS - Owner Signature S
     * @param sigV - Owner Signature V
     */
    function buyBNZ(
        uint256 amountOfBNZ,
        uint256 deadline,
        ...
        checkOwnerApproval(abi.encode(msg.sender, amountOfBNZ, deadline), sigR, sigS, sigV);
        //uint256 amountOfBNZ = (msg.value / tokenPriceInWei) * 1e18;
        bnz.safeTransfer(msg.sender, amountOfBNZ);
        emit BoughtBNZ(msg.sender, amountOfBNZ);
    }

    /** Withdraw BNZ From the Contract
     * @param amount - Amount of BNZ To Withdraw
     */
    function withdrawBNZ(uint256 amount) external onlyOwner nonReentrant {
        bnz.safeTransfer(msg.sender, amount);
        emit WithdrawnBNZ(msg.sender, amount);
    }
    ...
    function withdrawEther(uint256 amount) external onlyOwner nonReentrant {
        ...
        emit WithdrawnEther(msg.sender, amount);
    }
