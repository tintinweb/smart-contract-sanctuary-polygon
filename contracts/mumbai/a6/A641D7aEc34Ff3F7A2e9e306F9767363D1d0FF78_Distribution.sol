...
contract Distribution is Ownable {
	...
	function distribute(address[] memory aTokens) public  onlyOwner {
		for(uint256 i=0; i<aTokens.length; i++){
			uint256 aTokenBalance = IAToken(aTokens[i]).balanceOf(address(this));
			uint256 treasuryAmount = aTokenBalance.mul(platformRatio).div(100);
			uint256 feeValutAmount = aTokenBalance.mul(bonusRatio).div(100);
			IERC20(aTokens[i]).transfer(treasury,treasuryAmount);
			IERC20(aTokens[i]).transfer(feeVault,feeValutAmount);
		}
	...
	function setRatio(uint256 _platformRatio, uint256 _bonusRatio) external onlyOwner {
		require(_platformRatio + _bonusRatio == 100,"ratio error");
