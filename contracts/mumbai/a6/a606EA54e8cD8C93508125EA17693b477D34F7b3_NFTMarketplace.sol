...
contract NFTMarketplace is ERC721URIStorage {
    ...
    function createMarketItem(
      ...
      idToMarketItem[tokenId] =  MarketItem(
        tokenId,payable(msg.sender),payable(address(this)),payable(address(this)),payable(royalty),
        price,tokenURI,_creator,isAuction,block.timestamp + _time,block.timestamp,
        isMarket,0,rarity);

      _transfer(msg.sender, address(this), tokenId);
      emit MarketTrx(
       tokenId,msg.sender,address(0),price,
       "Create NFT Listed on Market Place",block.timestamp);
      emit MarketItemCreated(
        tokenId,msg.sender,address(this),price,isAuction,isMarket
      );
    }
    ...
    function resellToken(uint256 [] calldata tokenIds, uint256 price) public {
        ...
        for(uint i =0; i < tokenIds.length; i ++)
        {
            ...
            if(idToMarketItem[tokenId].owner == msg.sender){
              if(idToMarketItem[tokenId].isAuction == false){
                if(idToMarketItem[tokenId].isMarket == false){
                  ...
                  _transfer(msg.sender, address(this), tokenId);
                  emit MarketTrx(tokenId,msg.sender,address(0),price,
                                  "Lists NFT on market place",
                                  block.timestamp
                  );
                }
              }
    ...
    function createMarketSale(
      uint256 [] calldata tokenIds
        ...
        require(msg.value > 0, "Buying price should be higher than Zero");
        ...
        for(uint i =0; i < tokenIds.length; i ++)
        {
            ...
            if(seller != msg.sender){
              if(idToMarketItem[tokenId].isMarket == true){
                if(pricefrom >= price){
                  ...
                  if(msg.sender.balance >= price){
                    ...
                    emit MarketTrx(tokenId,msg.sender,msg.sender,price,
                                  "Bought NFT from market place",
                                  block.timestamp
                    );
                  }
                }
    ...
    function fetchMarketItems() public view returns (MarketItem[] memory) {
        ...
        if (idToMarketItem[i + 1].owner == address(this) && idToMarketItem[i + 1].isMarket && (idToMarketItem[i + 1].nonce != 0)) {
            itemCount += 1;
        }
        }
    ...
    function fetchAuctionItems() public view returns (MarketItem[] memory) {
        ...
        for (uint i = 0; i < totalItemCount; i++) {
        if (idToMarketItem[i + 1].owner == address(this) && idToMarketItem[i + 1].isAuction && (idToMarketItem[i + 1].nonce != 0)) {
            itemCount += 1;
        }
        }
