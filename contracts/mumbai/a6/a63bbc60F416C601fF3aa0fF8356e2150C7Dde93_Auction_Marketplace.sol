...
contract Auction_Marketplace is INft_Marketplace, IERC721Receiver {
    ...
    mapping(address => mapping(uint256 => mapping(address => Bid))) public bidByOrderId;
    mapping(address => mapping(uint256 => Bid)) public highestByAssetId;
    mapping(address => mapping(uint256 => address[])) public bidderList;
    ...
    function getBidUser(address _nftAddress, uint256 _assetId, address bidder_addr)
        public
        view
        returns (
            bytes32 id,
    ...
    {
        Bid memory bid = bidByOrderId[_nftAddress][_assetId][bidder_addr];
        id = bid.id;
        bidder = bid.bidder;
        price = bid.price;
    }

    function getBidList(address _nftAddress, uint256 _assetId)
        public
        view
        returns (
            address[] memory
    ...
    {
        return bidderList[_nftAddress][_assetId];
    }
    ...
    function safePlaceBid(
        ...
        Bid memory bid = bidByOrderId[_nftAddress][_assetId][msg.sender];
        if( bid.id != 0 ) _updateBid(_nftAddress, _assetId, _price);
     ...
     */
    function acceptBid(address _nftAddress, uint256 _assetId, address buyer) public {
        Bid memory bid = bidByOrderId[_nftAddress][_assetId][buyer];

        // remove bid
        delete bidByOrderId[_nftAddress][_assetId][buyer];
    ...
    function _executeOrder(
        ...
        address[] memory bidders = bidderList[_nftAddress][_assetId];
        for(uint i = 0 ; i < bidders.length ; i ++){
            Bid memory bid = bidByOrderId[_nftAddress][_assetId][bidders[i]];
            if(bid.bidder != _buyer){
                payable(bidders[i]).transfer(
                    bid.price
                );
                delete bidByOrderId[_nftAddress][_assetId][bidders[i]];
            }
        }
        bidderList[_nftAddress][_assetId] = new address[](0);
        // Notify ..
    ...
    function _createBid(
        ...
        Bid memory highestBid = highestByAssetId[_nftAddress][_assetId];
        
        // if theres no previous bid, just check price > 0
        if (highestBid.id != 0) {
            require(
                _price >= highestBid.price,
        ...
        // Save highest Bid for this nft
        highestByAssetId[_nftAddress][_assetId] = Bid({
            id: bidId,
            bidder: msg.sender,
            price: _price
        });
        ...
        bidByOrderId[_nftAddress][_assetId][msg.sender] = Bid({
            id: bidId,
            bidder: msg.sender,
            price: _price
        });
        address[] storage arr = bidderList[_nftAddress][_assetId];
        arr.push(msg.sender);
        bidderList[_nftAddress][_assetId] = arr;
        emit BidCreated(
            bidId,
            _nftAddress,
            _assetId,
    ...
    function _updateBid(
        ...
        // Checks order validity
        Bid memory curBid = bidByOrderId[_nftAddress][_assetId][msg.sender];
        ...
        // Save Bid for this order
        bidByOrderId[_nftAddress][_assetId][msg.sender] = newBid;

        // Save highest Bid for this nft
        highestByAssetId[_nftAddress][_assetId] = newBid;
    ...
    function _cancelBid(address _nftAddress, uint256 _assetId) internal {
        Bid memory bid = bidByOrderId[_nftAddress][_assetId][msg.sender];
        delete bidByOrderId[_nftAddress][_assetId][msg.sender];
        Bid memory topBid = highestByAssetId[_nftAddress][_assetId];
        topBid.price = 0;
        address[] memory bidders = bidderList[_nftAddress][_assetId];
        address[] memory newArr = new address[](bidders.length - 1);
        bool checked = false;
        for(uint i = 0 ; i < bidders.length ; i ++){
            if(checked == true){
                newArr[i - 1] = bidders[i];
            }else{
            ...
            if(bidders[i] != msg.sender){
                Bid memory tmpBid = bidByOrderId[_nftAddress][_assetId][bidders[i]];
                if(tmpBid.price >= topBid.price)topBid = tmpBid;
            }
        }
        payable(bid.bidder).transfer(bid.price);
        highestByAssetId[_nftAddress][_assetId] = topBid;
        bidderList[_nftAddress][_assetId] = newArr;
