...
/**
 * @notice Represents a balance of token held in PiggyBank.
 * @member token Address of the token.
 * @member balance Balance of the token held in the PiggyBank.
 */
struct TokenBalance {
    IERC20 token;
    uint256 balance;
...
struct ProposalMilestone {
    ...
    string milestoneResearchCid; // link to the IPFS files
    string pictureCid; // link to the IPFS files
}
 ...
 * @dev Contract uses SafeERC20 for research funcing.
 */
contract DeSciPlatform {
    ...
    /**
     * @notice Event emitted when PiggyBank is broken.
     * @dev Emitted when `breakPiggyBank` is called.
     * @param breaker Who broke the PiggyBank.
     */
    event PiggyBankBroken(address indexed breaker);

    /* ========== CONSTANTS ========== */

    /** @notice Maximum capacity of the PiggyBank. */
    uint256 public constant CAPACITY = 1000;
    ...
    /** @notice Current amount of tokens deposited in the PiggyBank. */
    uint256 public filledCapacity = 0;

    /** @notice Is the PiggyBank broken. */
    bool public broken = false;

    /** @notice List of all different tokens deposited in the PiggyBank. */
    IERC20[] public tokensDeposited;

    /** @notice Balance of tokens deposited in the PiggyBank. */
    mapping(IERC20 => uint256) public balance;
    ...
    function getProposals() external view returns(ResearchProposalView[] memory) {
        ResearchProposalView[] memory researchProposals = new ResearchProposalView[](proposals.length);
    ...
    function getProposal(uint256 proposalId) public view returns(ResearchProposalView memory) {
        ...
        ProposalMilestone[] storage proposalMilestonesStorage = proposalResearchMilestones[
            proposalId
        ];

        uint256 milestonesCount = proposalMilestonesStorage.length;

        ProposalMilestone[] memory proposalMilestones = new ProposalMilestone[](milestonesCount);

        for (uint256 i = 0; i < milestonesCount; i++) {
            proposalMilestones[i] = proposalMilestones[i];
        }
        ...
        )
    {
        ...
        require(milestones.length > 0, "DeSciPlatform::createResearchProposal: Proposal must have at least one milestone");
...

        {
            ...
            for (uint256 i = 0; i < milestones.length; i++) {
                ...
                require(milestones[i].percentage > lastPercentage, "DeSciPlatform::createResearchProposal: Milestone percentage must be more than previous");
            ...
            require(lastPercentage == 100, "DeSciPlatform::createResearchProposal: Milestone last percentage mustbe 100");
        }
...

        {
            ...
            emit ResearchProposalCreated(proposal, proposalId);
        }
    ...
    function confirmMilestoneCompleted(uint256 proposalId, uint256 milestoneIndex, string calldata nftMetadataCid)
        external
        onlyFunder(proposalId)
        verifyProposalMilestoneIndexState(
            proposalId,
