...
interface IConsentRegistry {
    ...
    function getConsents() external view returns (string[] memory);
...
contract ConsentRegistry is IConsentRegistry, Ownable {
    ...
    function getConsents() external view override returns (string[] memory) {
        return _getConsents();
    }
    ...
    function _getConsents() internal view returns (string[] memory) {
        string[] memory tmp = new string[](_consentsCount);
        uint32 count = 0;

        for (uint32 i = 1; i <= _consentsCount; i++) {
            tmp[count] = consents[i].name;
            count++;
        }
    ...
    function _getActiveConsents() internal view returns (string[] memory) {
        string[] memory filtered = new string[](_consentsCount);
        uint32 count = 0;

        for (uint32 i = 1; i <= _consentsCount; i++) {
            if (consents[i].active) {
                filtered[count] = consents[i].name;
                count++;
            }
        }
