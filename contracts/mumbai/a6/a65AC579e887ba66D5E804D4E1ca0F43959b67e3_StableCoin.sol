...
abstract contract StableCoinRebaseToken  is Ownable, ERC20 {
    ...
    modifier limiter(address atAddress) {
        require(!inCooldown(atAddress),"Limit reached");
        _;
    }

    function iterate(address atAddress) private {
        if(cooldownEnd[atAddress]<block.timestamp){
            count[atAddress] = count[atAddress].add(1);
    ...
    function _transfer(
        ...
        address to,
        uint256 amount
    ) internal override virtual limiter(tx.origin) {
    ...
    function setWhitelisted(address atAddress, bool toValue) external onlyOwner {
...
contract StableCoin is StableCoinRebaseToken{
    using SafeMath for uint256;

    string private constant _name   = "Stable Coin";
