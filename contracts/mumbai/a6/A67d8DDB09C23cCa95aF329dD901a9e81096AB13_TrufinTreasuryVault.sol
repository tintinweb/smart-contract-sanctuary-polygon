...
import {TrufinTreasuryVaultStorage} from "../../../storage/TrufinTreasuryVaultStorage.sol";
    ...
    TrufinTreasuryVaultStorage
{
    ...
    function _depositFor(uint256 amount, address creditor) private {
        ...
        uint256 totalUserDeposit = accountVaultBalance(msg.sender)
            .add(depositReceipt.amount)
            .add(amount);
    ...
    function withdrawInstantly(uint256 amount) external nonReentrant {
        ...
        uint256 userBalance = accountVaultBalance(msg.sender).add(
            receiptAmount
        );
    ...
    function _closeShort(address oldOption) private {
        ...
        if (oldOption != address(0)) {
            uint256 withdrawAmount = VaultLifecycleTreasury.settleShort(
                GAMMA_CONTROLLER
            );
    ...
    function burnRemainingOTokens() external onlyKeeper nonReentrant {
        uint256 unlockedAssetAmount = VaultLifecycleTreasury.burnOtokens(
            GAMMA_CONTROLLER,
    ...
    function _chargePerformanceFee(IERC20 token, uint256 amount) internal {
        address recipient = feeRecipient;
        uint256 transferAmount = amount.mul(performanceFee).div(
