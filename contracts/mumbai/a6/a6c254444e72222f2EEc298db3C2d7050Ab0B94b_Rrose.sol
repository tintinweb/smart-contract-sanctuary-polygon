...
interface IMasterPiece {
    ...
    struct masterPieceStruct {
        ...
        string artistBirth;         // date de naissance de l’artiste (year)
        string artistDeath;         // date de décès de l’artiste (year)
...
interface INFTMasterBox {
    ...
    event boxMinted(address receiver, uint tokenId, bytes32 key);
...
contract MasterPiece is Ownable, IMasterPiece {
    ...
    function add(bytes32 key, masterPieceStruct memory masterPiece) public onlyOwner {
        ...
        m.artistBirth = masterPiece.artistBirth;
        m.artistDeath = masterPiece.artistDeath;
    ...
    function update(bytes32 key, masterPieceStruct memory masterpiece) public onlyOwner {
        ...
        m.artistBirth = (bytes(masterpiece.artistBirth).length > 0) ? masterpiece.artistBirth : m.artistBirth;
        m.artistDeath = (bytes(masterpiece.artistDeath).length > 0) ? masterpiece.artistDeath : m.artistDeath;
