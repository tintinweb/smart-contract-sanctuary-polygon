    ...
    PausableUpgradeable
{
    ...
    /**
     * @dev called to retrieve length of a users flexible stake deposits
     *
     * @return _depositLength number of locked stake deposits for the user
     */
    function getLockedUserDeposits(address _lockedUser)
        public
        view
        returns (uint256 _depositLength)
    {
        _depositLength = lockedStakingUsers[_lockedUser].deposits.length;
    }

    /**
     * @dev called to retrieve length of a users locked stake deposits
     *
     * @return _depositLength number of locked stake deposits for the user
     */
    function getFlexibleUserDeposits(address _flexibleUser)
        public
        view
        returns (uint256 _depositLength)
    {
        _depositLength = flexibleStakingUsers[_flexibleUser].deposits.length;
    }

    /**
     * @dev called to retrieve deposit details of a certain flexible stake deposit
     *
     * @return _tokenAmount token amount of deposit
     * @return _stakeWeight weight of the deposited stake
     */
    function getFlexibleDeposit(address _staker, uint256 _depositIndex)
        public
        view
        returns (uint256 _tokenAmount, uint256 _stakeWeight)
    {
        _tokenAmount = flexibleStakingUsers[_staker]
            .deposits[_depositIndex]
            .tokenAmount;
        _stakeWeight = flexibleStakingUsers[_staker]
            .deposits[_depositIndex]
            .weight;
    }

    /**
     * @dev called to retrieve deposit details of a certain locked stake deposit
     *
     * @return _tokenAmount token amount of deposit
     * @return _stakeWeight weight of the deposited stake
     * @return _lockedFrom deposit was locked on this time
     * @return _lockedUntil deposit shall remain locked until this time
     * @return _lootBoxesClaimed number of loot boxes claimed on this deposit
     */
    function getLockedDeposit(address _staker, uint256 _depositIndex)
        public
        view
        returns (
            ...
            uint256 _lootBoxesClaimed
        )
    {
        _tokenAmount = lockedStakingUsers[_staker]
            .deposits[_depositIndex]
            .tokenAmount;
        _stakeWeight = lockedStakingUsers[_staker]
            .deposits[_depositIndex]
            .weight;
        _lockedFrom = lockedStakingUsers[_staker]
            .deposits[_depositIndex]
            .lockedFrom;
        _lockedUntil = lockedStakingUsers[_staker]
            .deposits[_depositIndex]
            .lockedUntil;
        _lootBoxesClaimed = lockedStakingUsers[_staker]
            .deposits[_depositIndex]
            .lootBoxesClaimed;
    }
    ...
    // Added to support recovering LP tokens from contract
    function recoverStakingTokens(uint256 tokenAmount)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(tokenAmount <= rewardTokens, "not enough rewards available");
        stakingToken.transfer(address(msg.sender), tokenAmount);
    }
