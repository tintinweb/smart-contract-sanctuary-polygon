...
contract Betting is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    event TournamentAdminRewardWithdrawed(string tournamentId, address indexed userAddress, uint256 claimAmount);
    ...
    */
    ...
    modifier afterTournamentWinnerSetted(string memory _tournamentId) {
        require(
            bytes(tournamentByTournamentId[_tournamentId].winner).length > 0,
            "[Betting.afterTournamentWinnerSetted] Tournament winner not been setted yet"
        );
        _;
    }
    ...
    */
    function claimTournamentReward(string memory _tournamentId)
        ...
        nonReentrant
        tournamentCreated(_tournamentId)
        afterTournamentWinnerSetted(_tournamentId)
    {
        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
    ...
    * @param _tournamentId - tournament id generated from backend.
    */
    function adminWithdraw(string memory _tournamentId)
        external
        nonReentrant
        tournamentCreated(_tournamentId)
        afterTournamentWinnerSetted(_tournamentId)
        onlyRole(WORKER)
    {
        ...
        require(
            ...
            "[Betting.adminWithdraw] Admin cannot withdraw when there is bet on winner"
        );
        require(
            tournament.tournamentClaimAmount == 0,
            "[Betting.adminWithdraw] Admin cannot withdraw more than once"
        );

        uint256 reward = tournament.tournamentBetAmount;
        require(
            reward > 0,
            "[Betting.adminWithdraw] Tournament have no reward to be withdrawed"
        );
        tournament.tournamentClaimAmount += reward;
        safeUpoTransfer(msg.sender, reward);
    ...
    */
    function calculateTournamentReward(string memory _tournamentId) internal {
        ...
        tournament.rewardPerBet = entry.entryBetAmount != 0? tournament.tournamentBetAmount / entry.entryBetAmount: 0;
    }
