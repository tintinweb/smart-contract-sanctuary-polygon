...
contract XSeedStake is Ownable {
    ...
    uint256 public totalRemainingPayouts;
	...
	function _invest(address payable referrer, uint8 _plan, address payable sender, uint256 value) private {
		...
		// require(user.activeInvestments < MAX_ACTIVE_STAKES, "Cannot have more than 6 active investments!");
        ...
        require(xseed.balanceOf(address(this)) >= profit-value, "Contract does not have enough balance");
        ...
        user.totalActiveStaked = user.totalActiveStaked.add(value);
        ...
        user.expectedEarnings = user.expectedEarnings.add(profit);
        totalRemainingPayouts = totalRemainingPayouts.add(profit);
	...
	function withdraw(uint256 depositIndex) public {
        ...
        users[msg.sender].totalActiveStaked = users[msg.sender].totalActiveStaked.sub(withdrawamount);
		users[msg.sender].withdrawn = users[msg.sender].withdrawn.add(withdrawamount);
        users[msg.sender].expectedEarnings = users[msg.sender].expectedEarnings.sub(withdrawamount);
        totalRemainingPayouts = totalRemainingPayouts.sub(withdrawamount);
    ...
    function withdrawExcessFunds() public onlyOwner {
        uint256 balance = xseed.balanceOf(address(this));
        uint256 excessFunds = balance - totalRemainingPayouts;
        require(excessFunds>0, "There are no excess funds in the smart contract");
        xseed.transfer(owner(), excessFunds);
    }
    ...
    function getUserActiveStaked(address userAddress) public view returns(uint256) {
        return users[userAddress].totalActiveStaked;
    }
