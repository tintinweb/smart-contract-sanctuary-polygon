...
// File: contracts/WarCitizen/IWarCitizenToken.sol
...
// File: contracts/WarCastle/IWarCastleToken.sol
...
// File: contracts/WarCastleDetails.sol
...
// File: contracts/WarLogic/WarLogic.sol
...
contract WarLogic is AccessControlUpgradeable, UUPSUpgradeable, IWarLogic {
    ...
    mapping(uint256 => uint256) private castle_work_effort;

    // New maps with counters
    mapping (address => Counters.Counter) private _citizens_allocation;
    ...
    function getCastleWorkEffort(uint256 castleId) external view returns (uint256) {
        return castle_work_effort[castleId];
    }
    ...
    function allocateCitizen(uint256 castleId, uint256 citizenId) public {
        ...
        castle_work_effort[castleId] += (citizen_details.work_effort + citizen_details.level);
    ...
    function deallocateAllCitizens(uint256 castleId) public {
        ...
        castle_work_effort[castleId] = 0;
        delete castle_citizens[castleId];
    }

    function increaseCitizenLevel(uint256 citizenId) public {
        require(msg.sender == address(contract_castle) || msg.sender == address(contract_citizen), "Not permitted");
        (, uint256 castleId) = citizens_allocation[msg.sender].tryGet(citizenId);
        // If it's a allocated citizen, increase +1 in the work effort of castle
        if (castleId != 0) {
            castle_work_effort[castleId] +=1;
        }
    }
