...
contract DeceaLink is Ownable {
    ...
    function updateBot(
        ...
        require(botIndexs[id] >= 0, "Bot does not exist");
    ...
    function showBot(uint8 id) external view returns (botData memory) {
        require(botExists[id], "Bot does not exist");
        require(botIndexs[id] >= 0, "Bot does not exist");
        return listBotData[botIndexs[id]];
    }
    ...
    function payLicense(
        ...
        require(botIndexs[idBot] >= 0, "Bot does not exist");
        ...
        if (msg.value >= bot.botLicensePay) {
            ...
            if (isLicenseExist == true) {
                ...
                licId = Strings.toString(hashedWord);
                totalLicense++;
    ...
    function payToERC20(
        ...
        //token.transferFrom(address(this), ownerMe, amount);
        return true;
    }
    ...
    function addPaymentToken(
        ...
        require(msg.sender == ownerAddress);
        require(
            allowedERC20TokenExist[addressERC20Token] == false,
    ...
    function addPaymentTokenBotPrice(
        ...
        // only the customer can use this function
        require(msg.sender == ownerAddress);
        require(botExists[botId], "Bot does not exist");
        require(
            allowedERC20TokenExist[addressERC20Token],
            "Token Address Not exist"
        );
        require(
            botMinPaymentERC20TokenExist[addressERC20Token][botId] == false,
            "Bot Payment for Token Address Already exist"
        );
        require(payMinimumERC20TokenPrice > 0, "botLicensePay cannot be zero");
    ...
    function updatePaymentTokenBotPrice(
        ...
        require(
            botMinPaymentERC20TokenExist[addressERC20Token][botId],
            "Bot Payment for Token Address Not exist"
        );
        require(payMinimumERC20TokenPrice > 0, "botLicensePay cannot be zero");
        uint8 idAddressContract = allowedERC20TokenIndex[addressERC20Token];
    ...
    function changeDefaultValueStableCoinPaymentToken(
        ...
        require(msg.sender == ownerAddress);
        require(
            defaultValueStableCoinInDollar > 0,
            "defaultValueStableCoin Cant Zero"
        );
        ...
        returns (uint256 nativeMinPayment, ERC20TokenDataMinPaymentBot[] memory)
    {
        ...
        if (addressERC20Token == address(0)) {
            uint8 indexercpay = 0;
    ...
    function listBotPayment(
        ...
        returns (uint256 nativeMinPayment, ERC20TokenDataMinPaymentBot[] memory)
    {
        return _listBotPayment(botId, addressERC20Token);
    }
    ...
    function approvePaymentTokens(
        ...
        require(addressERC20Token != address(0), "addressERC20Token empty");
        require(
            allowedERC20TokenExist[addressERC20Token],
            "Token Address Not exist"
        );
        require(_tokenamount > 0, "_tokenamount cannot be zero");
        IERC20 token = IERC20(addressERC20Token);
    ...
    function getAllowancePaymentTokens(
        ...
        require(
            isContract(checkAddress) == false,
            "addressERC20Token is smart contract"
        );
        IERC20 token = IERC20(addressERC20Token);
        address chkaddrs;
    ...
    function payLicenseWithToken(
        ...
        ) = _listBotPayment(botId, addressERC20Token);
