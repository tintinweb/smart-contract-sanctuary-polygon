...
//** Author ishanshahzad :  Marketplace) Contract 2022.11 */
...
contract OOMMarketPlace is OOMContractBase {
    ...
    function buyNow(bytes32 listingId) external payable nonReentrant {
        ...
        _buyNow(msg.sender,listingId, price);
    }  
    function buyNowGL(address _fromAddress, bytes32 listingId) external payable nonReentrant onlyOwner{ // only opererator
        ...
        _buyNow(_fromAddress,listingId, price);
    } 
...
abstract contract OOMContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function _placeBid(address _fromAddress, bytes32 listingId, uint256 price) internal {
    ...
    function _buyNow(address _fromAddress,bytes32 listingId, uint256 price) internal returns (address hostContract) {
        listing storage lst = listingRegistry[listingId];
     ...
     */
    function removeListing(address _fromAddress, bytes32 listingId) external nonReentrant onlyOwner {
        listing storage lst = listingRegistry[listingId];
