...
contract Miami_Token is Context, IBEP20, Ownable {
    using SafeMath for uint256;
    using Address for address;
...
// MasterChef is the master of Mia. He can make Mia and he is a fair guy.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once Mia is sufficiently
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Miaes
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accMiaPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accMiaPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint;       // How many allocation points assigned to this pool. Miaes to distribute per block.
        uint256 lastRewardBlock;  // Last block number that Miaes distribution occurs.
        uint256 accMiaPerShare;   // Accumulated Miaes per share, times 1e18. See below.
        uint16 depositFeeBP;      // Deposit fee in basis points
        uint256 lpSupply;
    }

    // The Mia TOKEN!
    Miami_Token public Mia;
    address public devAddress;
    address public feeAddress;
    // address public vaultAddress;

    // Mia tokens created per block.
    uint256 public MiaPerBlock = 0.01 ether;
    
    uint256 constant MAX_MIA_PERBLOCK= 2 ether;
    ...
    // The block number when Mia mining starts.
    uint256 public startBlock;

    // Mia referral contract address.
    ...
    event UpdateEmissionRate(address indexed user, uint256 MiaPerBlock);
    ...
    // Update the given pool's Mia allocation point and deposit fee. Can only be called by the owner.
    ...
    // View function to see pending Miaes on frontend.
    function pendingMia(uint256 _pid, address _user) external view returns (uint256) {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];
        uint256 accMiaPerShare = pool.accMiaPerShare;
        uint256 lpSupply = pool.lpToken.balanceOf(address(this));
        if (block.number > pool.lastRewardBlock && lpSupply != 0 && totalAllocPoint>0) {
            uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
            uint256 MiaReward = multiplier.mul(MiaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accMiaPerShare = accMiaPerShare.add(MiaReward.mul(1e18).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 MiaReward = multiplier.mul(MiaPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Mia.mint(devAddress, MiaReward.div(10));
        Mia.mint(address(this), MiaReward);
        pool.accMiaPerShare = pool.accMiaPerShare.add(MiaReward.mul(1e18).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to MasterChef for Mia allocation.
    function deposit(uint256 _pid, uint256 _amount, address _referrer) public nonReentrant {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accMiaPerShare).div(1e18).sub(user.rewardDebt);
            if (pending > 0) {
                safeMiaTransfer(msg.sender, pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accMiaPerShare).div(1e18);
    ...
    // Safe Mia transfer function, just in case if rounding error causes pool to not have enough FOXs.
    function safeMiaTransfer(address _to, uint256 _amount) internal {
        uint256 MiaBal = Mia.balanceOf(address(this));
        bool transferSuccess = false;
        if (_amount > MiaBal) {
            transferSuccess = Mia.transfer(_to, MiaBal);
        } else {
            transferSuccess = Mia.transfer(_to, _amount);
        }
        require(transferSuccess, "safeMiaTransfer: Transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _MiaPerBlock) external onlyOwner {
        require(_MiaPerBlock<=MAX_MIA_PERBLOCK,"Too high");
        massUpdatePools();
        MiaPerBlock = _MiaPerBlock;
        emit UpdateEmissionRate(msg.sender, _MiaPerBlock);
    }
    ...
    function payReferralCommission(address _user, uint256 _pending) internal {
        if (address(referral) != address(0) && referralCommissionRate > 0) {
            ...
            if (referrer != address(0) && commissionAmount > 0) {
                Mia.mint(referrer, commissionAmount);
