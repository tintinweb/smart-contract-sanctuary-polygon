...
contract Staking is Ownable {
    ...
    */
    function claimRewards(uint256 id) public {
        ...
        uint256 wholeMonthsPassed = (currentTime.sub(stakesOfUser[msg.sender][id].stakedTime)).div(60);
        ...
        if(wholeMonthsPassed <= 3) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[0]).mul(wholeMonthsPassed * 120).div(1e18);
        } else if(wholeMonthsPassed <= 6) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[1]).mul(wholeMonthsPassed * 120).div(1e18);
        } else if(wholeMonthsPassed <= 9) {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[2]).mul(wholeMonthsPassed * 120).div(1e18);
        } else {
            reward = stakesOfUser[msg.sender][id].amount.mul(tpsRatios[3]).mul(wholeMonthsPassed * 120).div(1e18);
        }
