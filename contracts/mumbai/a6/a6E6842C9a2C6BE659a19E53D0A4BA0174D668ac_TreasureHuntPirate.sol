...
contract TreasureHuntPirate is Initializable, ITreasureHuntTrait, ERC721EnumerableUpgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable {
    ...
    mapping(address => uint256[]) public activePiratesByOwner;
    mapping(address => uint256[]) public inActivePiratesByOwner;
    ...
    function massMint(bool presale, uint8 amt) public {
        ...
        ITreasureHunt(rewardPool).transferCost(price * amt, true);
    ...
    function opneBox(uint16 amt) public {
    ...
    function mint(address to, uint256 tokenID) internal {
        ...
        ITreasureHunt(treasureHuntPicker).randomPirate(tokenID);
        activePiratesByOwner[to].push(tokenID);
        tokenIndex[tokenID] = activePiratesByOwner[to].length - 1;
        isActivePirate[tokenID] = true;
    }

    function setPirate(uint256 tokenID, uint8 star, uint16 power) public {
        require(msg.sender == treasureHuntPicker, "You are not picker!");
        pirateInfo[tokenID] = Pirate(tokenID, star, power);
        string memory uri = string(abi.encodePacked(pirateBaseURI, StringsUpgradeable.toString(star)));
    ...
    function getPirateInfo(uint256 tokenID) public view returns (uint8, uint256) {
    ...
    function setGiftFee(uint256 _burnPrice) public onlyOwner {
        burnPrice = _burnPrice;
    }

    function setApprovalForAll_(address operator) public _isFleet {
    ...
    function boardPirates(uint256[] memory tokenIDs) public _isFleet {
        for(uint256 i = 0; i < tokenIDs.length; i++) {
            ...
            safeTransferFrom(tx.origin, treasureHuntFleet, tokenIDs[i]);
        }
    }
    ...
    function unBoardPirates(uint256[] memory pirates) public _isFleet {
        for (uint256 i = 0; i < pirates.length; i++) {
            ...
            safeTransferFrom(treasureHuntFleet, tx.origin, pirates[i]);
    ...
    function burnPirates(uint256[] memory tokenIDs) public {
        ...
        ITreasureHunt(treasureHuntPicker).transferBurnReward(msg.sender, burnPrice * tokenIDs.length);
    }
...
interface ITreasureHunt is ITreasureHuntTrait {
    ...
    //picker
    function randomPirate(uint256 tokenID) external returns(bool);
    function randomShip(uint256 tokenID) external returns(bool);
    function random() external returns(uint256);
    //pirate
    function boardPirates(uint256[] memory) external;
    function getPirate(uint256) external returns(Pirate memory);
    function getPirates(uint256[] memory) external returns(Pirate[] memory);
    function setPirate(uint256, uint8, uint16) external;
    function unBoardPirates(uint256[] memory) external;
    //ship
    function disJoinShips(uint256[] memory) external;
    function getShip(uint256) external returns(Ship memory);
    function getShips(uint256[] memory) external returns(Ship[] memory);
    function joinShips(uint256[] memory) external;
    function setShip(uint256, uint8) external;
    //fleet
    function getFleetNumByOwner(address) external returns(uint256);
    function getFleetInfo(uint256) external returns(Fleet memory);
    function updateFleetFund(uint256, uint256, uint256, bool) external;
    function getFleetRaidTime(uint256) external returns(uint256);
    function setFleetRaidTime(uint256) external;
    function getTotalHakiByOwner(address) external returns(uint256);
    ...
    //seaport
    function isOnSeaport(address) external returns(bool);
    function isSeaportOwner(address) external returns(bool);
    function getSeaportOwnerByPlayer(address) external returns(address);
    //random generator
