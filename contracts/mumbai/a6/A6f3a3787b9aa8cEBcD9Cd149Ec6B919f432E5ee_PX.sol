...
contract PX is ERC721URIStorage, Ownable {
    ...
    struct Storage {
        ...
        mapping(bytes32 => App) appMetadata;
    }
    struct FreePackage {
    ...
    function setAppMetadata(uint256 tokenId, string memory appId, string memory input) public {
        require(_exists(tokenId), "Incorrect tokenId");
        require(_isApprovedOrOwner(_msgSender(), tokenId),"Not NFT owner");
        App memory app = App(1, input); // 1 active, 0 not active
        pxStorage[tokenId].appMetadata[keccak256(abi.encodePacked(appId))] = app;
    }
}