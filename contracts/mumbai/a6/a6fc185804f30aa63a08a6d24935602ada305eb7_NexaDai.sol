...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
    ...
    function setPrams(address newTokenAddress, uint8 withdrwalFee, uint64 setPoolTime, uint8 setPoolLimit,uint8 _maxCycle, uint8 _gapMaxDepth,uint64 _royaltyClosingTime, uint8 _AR_validity) external returns(bool){
   ...
   //Incomming transaction
    ...
    function payUserPosition(uint _mIndex, uint _from ,uint _to, uint cycle) internal {
        ...
        userGains[usr].totalAutopool2xGains+=POOL_2X_PAY;// Pool pay
  ...
  function _distributeGapBonus(address _user,address ref)  internal returns(bool){
    ...
    function fetchUser() external onlyAdmin returns (bool){
     
      
       if(lastIDCount<dataContract(tokenAddress).lastIDCount()){
        lastIDCount++;
        _oldUserFetch(dataContract(tokenAddress).userAddressByID(lastIDCount));
        _oldUserGainsFetch(dataContract(tokenAddress).userAddressByID(lastIDCount));
       }
    ...
    function _oldUserFetch(address userAddress) internal {

       userInfo memory oldUserinfo;
       (oldUserinfo.joined,oldUserinfo.referral,oldUserinfo.id,oldUserinfo.activeDirect,oldUserinfo.teamCount,,oldUserinfo.strongTeam,,,,,,oldUserinfo.globalPoolCount,)=dataContract(tokenAddress).userInfos(userAddress);
        userInfos[userAddress]=oldUserinfo;//write the storage. 
    }

    function _oldUserGainsFetch( address userAddress) internal {

        userIncome memory oldUserGain;
        (,,,,,,,,,oldUserGain.totalWithdrawn,oldUserGain.withdrawLimit,oldUserGain.creditFund,oldUserGain.topup_Count)=dataContract(tokenAddress).userGains(userAddress);
        oldUserGain.transferGains=dataContract(tokenAddress).totalGains_();
        for (uint i ; i<=oldUserGain.topup_Count; i++)_royaltyCollection();
        
        userGains[userAddress]=oldUserGain;//write the storage.
    }
    ...
    function testAllEvent() external {


        // call all event from here 

        emit autopool3xPayEv (0,0, 1,5000);
        emit regUserEv(msg.sender,msg.sender,2);
        emit activeRoyaltyQualify(msg.sender,1,1,true);
        emit unilevelEv(msg.sender,msg.sender,1,5000);
        emit autopool2xPosition(1,1,1,1);
        emit poolRoyaltyBonusEv(1,0,1,2);
        emit autopool3xPosition(0,1,1,1);
        emit gapGenerationBonus_Ev(msg.sender,msg.sender,1,50000);
        emit royaltyPay_Ev(msg.sender,600,300);
