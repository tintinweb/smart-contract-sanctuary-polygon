...
/**
 *
 * SkullCoin Contract (The native token of HashSkuls)
 * @dev Extends standard ERC20 contract
 */
contract SkullCoin is Context, IERC20 , ReentrancyGuard, Ownable {
    ...
    // Public variables ; set during deployment of contract and can't change after 
    uint256 public emissionStart;

    uint256 public emissionEnd; 

    uint256 public emissionPerDay = 10 * (10 ** 18);
    //ERC20 inbuilt variables
    ...
    constructor(string memory name_, string memory symbol_,uint256  EmissionStartTimestamp) {
        ...
        _decimals = 18;
        //uint256  EmissionStartTimestamp = 1647700000;
    ...
    /**
     * @dev When accumulated NCTs have last been claimed for a HashSkull index
     */
    function lastClaim(uint256 tokenIndex) public view returns (uint256) {
        require(ISkulls(_nftContractAddress).ownerOf(tokenIndex) != address(0), "Owner cannot be 0 address");
        require(tokenIndex < ISkulls(_nftContractAddress).totalSupply(), "NFT at index has not been minted yet");
    ...
    /**
     * @dev Accumulated SKULL Coins for a HashSkull token index.
     */
    function accumulated(uint256 tokenIndex) public view returns (uint256) {
        ...
        totalAccumulated = accumulationPeriod.sub(lastClaimed).mul(emissionPerDay).div(SECONDS_IN_A_DAY);
        ...
        if (lastClaimed == emissionStart) {
            uint256 initialAllotment = ISkulls(_nftContractAddress).isMintedBeforeReveal(tokenIndex) == true ? INITIAL_ALLOTMENT.mul(PRE_REVEAL_MULTIPLIER) : INITIAL_ALLOTMENT;
    ...
    function setNFTContractAddress(address nftContractAddress) public onlyOwner {
        //require(_nftContractAddress == address(0), "Already set");
        
        _nftContractAddress = nftContractAddress;
    }

    /**
     * @dev Claim mints SKULLs for all the NFTs owned by the Owner
     */
    function claimAll() public nonReentrant returns (uint256) {
        require(block.timestamp > emissionStart, "Emission has not started yet");
        uint256[] memory tokenIndices = ISkulls(_nftContractAddress).tokensOfOwner(msg.sender);
        uint256 totalClaimQty = 0;
        for (uint i = 0; i < tokenIndices.length; i++) {
            // Sanity check for non-minted index
            require(tokenIndices[i] < ISkulls(_nftContractAddress).totalSupply(), "NFT at index has not been minted yet");

            uint tokenIndex = tokenIndices[i];
            require(ISkulls(_nftContractAddress).ownerOf(tokenIndex) == msg.sender, "Sender is not the owner");
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        // Approval check is skipped if the caller of transferFrom is the HashSkulls contract. For better UX.
