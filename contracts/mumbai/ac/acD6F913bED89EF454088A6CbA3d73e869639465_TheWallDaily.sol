...
contract TheWallDaily is Context, IERC721Receiver, ReentrancyGuard
{
    ...
    event ContractDeployed(uint256 constLiquidity, uint256 maximumAreasInPool, uint256 rentDurationSeconds);
    ...
    event Rented(address indexed tenant, uint256 indexed areaId, uint256 finishTime);
    ...
    uint256 public constant rentDurationSeconds = 48 hours;
    ...
    constructor(address payable thewall, address thewallcore)
    {
        ...
        emit ContractDeployed(constLiquidity, maximumAreasInPool, rentDurationSeconds);
    }
    ...
    {
        return (area.finishTime > block.timestamp) ? area.tenant : address(0);
    }
    ...
    function setContent(uint256 areaId, bytes memory content) public payable
    {
        ...
        area.finishTime = block.timestamp + rentDurationSeconds;
        _thewall.setContent(areaId, content);
        emit Rented(area.tenant, areaId, area.finishTime);
    }
}