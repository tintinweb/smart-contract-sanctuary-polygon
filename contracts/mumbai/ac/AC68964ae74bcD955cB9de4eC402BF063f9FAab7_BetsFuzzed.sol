...
contract UltibetsCore is CustomAdmin {
    ...
    function betsOfBettor(
        ...
        uint256 betNum = totalBetsOfBetter(_bettor);
    ...
    function addCategory(
        ...
        for (uint8 i; i < _subcategories.length; i++) {
            subcategories[uint8(categoryList.length)][i] = _subcategories[i];
        }
    ...
    function totalBetsOfBetter(address _bettor) public view returns (uint256) {
    ...
    function getEventDetailByBetter(uint256 _eventID)
        external
        view
        returns (EventDetailPerBetter memory)
    {
        uint256[] memory sidePoolVolumes;
        uint256[] memory sideBetAmounts;
        uint8 categoryId = eventList[_eventID].category;
        EventType eType = categoryList[categoryId - 1].eType;
        if (eType == EventType.Double) {
            ...
            sideBetAmounts[0] = betDataByBettor[msg.sender][_eventID][
                EventResult.Home
            ].betAmount;
            sideBetAmounts[1] = betDataByBettor[msg.sender][_eventID][
                EventResult.Away
            ].betAmount;
        } else {
        ...
        return
            EventDetailPerBetter(
                _eventID,
                eventList[_eventID].description,
                categoryList[categoryId].name,
                subcategories[categoryId][eventList[_eventID].subcategory],
