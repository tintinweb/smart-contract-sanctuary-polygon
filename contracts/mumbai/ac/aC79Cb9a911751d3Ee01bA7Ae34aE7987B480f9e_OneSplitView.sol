...
abstract contract IOneSplit is IOneSplitConsts {
    ...
    function setExecuteHandlerAddress(address _executeHandlerAddress) external virtual returns (bool);

    function setReserveAddress(address _reserveAddress) external virtual returns (bool);

    function setVoyagerUtils(address _voyagerUtils) external virtual returns (bool);
...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    address public executeHandlerAddress;
    IHandlerReserve public reserveInstance;
    ILiquidityPool public voyagerUtilsInstance;
    ...
    function initialize(
        ...
        address _executeHandlerAddress,
        address _reserveAddress,
        address _voyagerUtilsAddress,
        IEthHandler ethHandler
    ) public initializer {
        __AccessControl_init();
        _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
        oneSplitView = _oneSplitView;
        executeHandlerAddress = _executeHandlerAddress;
        reserveInstance = IHandlerReserve(_reserveAddress);
        voyagerUtilsInstance = ILiquidityPool(_voyagerUtilsAddress);
        _ethHandler = ethHandler;
    }

    modifier onlyHandler() {
        require(msg.sender == executeHandlerAddress, "sender must be handler contract");
        _;
    }
    ...
    function setExecuteHandlerAddress(address _executeHandlerAddress)
        public
        override
        onlyRole(DEFAULT_ADMIN_ROLE)
        returns (bool)
    {
        require(_executeHandlerAddress != address(0), "Recipient can't be null");
        executeHandlerAddress = _executeHandlerAddress;
        return true;
    }
    ...
    function setVoyagerUtils(address _voyagerUtils) public override onlyRole(DEFAULT_ADMIN_ROLE) returns (bool) {
        require(_voyagerUtils != address(0), "Address can't be null");
        voyagerUtilsInstance = ILiquidityPool(_voyagerUtils);
        return true;
    }
    ...
    function _swap(
        ...
        if (
            ...
            voyagerUtilsInstance.unstake(address(destToken), amount, false);
            return amount;
        }
        ...
        if (reserveInstance._lpToContract(address(destToken)) == address(fromToken)) {
            ...
            voyagerUtilsInstance.stake(address(fromToken), amount);
            return amount;
        }
