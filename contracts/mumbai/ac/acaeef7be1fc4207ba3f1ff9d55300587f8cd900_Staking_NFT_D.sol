...
// File: contracts/AsicNFT_Staking2.sol
...
contract Staking_NFT_D is Ownable, ReentrancyGuard, Pausable {
    ...
    IERC20Burnable public kWattToken;
    IERC721 public immutable nftCollection;

    uint256 constant SECONDS_IN_HOUR = 3600;
    uint256 constant KILOWATTS_TO_SECONDS = 1200;

    struct Staker {
        uint256[] stakedTokenIds;
        uint256 timeOfLastUpdate;
        uint256 unclaimedRewards;
        uint256 kWattBalance;
    }

    uint256 private rewardsPerHour = 694444444444444;
    ...
    constructor(IERC721 _nftCollection, IERC20 _rewardsToken, IERC20Burnable _kWattToken) {
        nftCollection = _nftCollection;
        rewardsToken = _rewardsToken;
        kWattToken = _kWattToken;
    }

    function stake(uint256[] calldata _tokenIds, uint256 _kWattAmount) external whenNotPaused {
        Staker storage staker = stakers[msg.sender];
        require(_kWattAmount >= _tokenIds.length * KILOWATTS_TO_SECONDS * 1 hours, "Not enough kWatt tokens for 1 hour of staking for each NFT");
        ...
        kWattToken.transferFrom(msg.sender, address(this), _kWattAmount);
        staker.kWattBalance += _kWattAmount;
    ...
    function calculateRewards(address _stakerAddress) private returns (uint256) {
        Staker storage staker = stakers[_stakerAddress];
        uint256 timeSinceLastUpdate = block.timestamp - staker.timeOfLastUpdate;
        uint256 hoursElapsed = timeSinceLastUpdate / SECONDS_IN_HOUR;
        uint256 stakedTokenIdsCount = staker.stakedTokenIds.length;

        if (staker.kWattBalance < stakedTokenIdsCount * KILOWATTS_TO_SECONDS * hoursElapsed) {
            uint256 kWattBurned = staker.kWattBalance;
            staker.kWattBalance = 0;
            kWattToken.burn(kWattBurned);
            hoursElapsed = kWattBurned / (KILOWATTS_TO_SECONDS * stakedTokenIdsCount);
        } else {
            uint256 kWattBurned = stakedTokenIdsCount * KILOWATTS_TO_SECONDS * hoursElapsed;
            staker.kWattBalance -= kWattBurned;
            kWattToken.burn(kWattBurned);
        }

        return hoursElapsed * stakedTokenIdsCount * rewardsPerHour;
    }
    ...
    function availableRewards(address _user) internal view returns (uint256 _rewards) {
        ...
        return staker.unclaimedRewards + (hoursElapsed * staker.stakedTokenIds.length * rewardsPerHour);
    }
