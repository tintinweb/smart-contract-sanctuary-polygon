...
contract Ownable  {
    ...
    modifier onlySuperOwner(){
        require(owner() == msg.sender, "Ownable: caller is not the super owner");
        _;
    }

    modifier onlyOwner() virtual {
        require(owners[msg.sender] == true, "Ownable: caller is not the owner");
        _;
    }
...
contract TraderSpiritsFree_Mint is Ownable, ERC721, ERC721Holder{ 
    ...
    uint256 public publicMintLimit = 950;         //  Public mint limit
    uint256 public giftMintLimit = 50;              //  Gift mint limit
    uint256 public giftMintId = 951;
    uint256 public publicMintId = 1;
    uint256 public mintPrice = 0;        // minting price 0.1 ETH
    uint256 public perTxQuantity = 1;
    uint256 public perWalletQuantity = 1;
    ...
    function setMintPrice(uint256 newPrice) external onlyOwner {
        require(newPrice != mintPrice, wrongValuePassed);
        mintPrice = newPrice;
    }
    ...
    function mintByUser(uint256 quantity) external payable isPause isHalt {
        ...
        require(
            ...
            msgValue >= mintPrice * quantity,
            "invalid transaction."
        );

        if(msgValue > 0){
            (bool success,) = payable(fundReceiver).call{value: msg.value}("");
            require(success == true, "Payment sending failed!.");
        }
