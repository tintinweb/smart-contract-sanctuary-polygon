...
contract Forwarder is IForwarder, ERC165, Ownable {
    ...
    // Mapping of valid relayers to call this contract
    mapping(address => bool) public relayers;

    // bytes4(keccak256(bytes("invokeSpend(address,uint256,uint256,uint256,bytes)")));
    // address _handlerAddress,uint256 _id,uint256 _primeValue,uint256 _timestamp,bytes memory _data
    bytes4 public invokeTokenSignature = 0xb51919a8;
    // bytes4(keccak256(bytes("invokeSpend(address,uint256,uint256,uint256,uint256,bytes)")));
    // address _handlerAddress,uint256 _id,uint256 _tokenId,uint256 _amount,uint256 _timestamp,bytes memory _data
    bytes4 public invokeNFTSignature = 0x183cbd08;
    ...
    function execute(
        ...
        require(relayers[msg.sender], "Relayer must be whitelisted address");
        require(
        ...
        bytes4 signature = bytes4(
            abi.encodePacked(req.data[0], req.data[1], req.data[2], req.data[3])
        );
        require(
            signature == invokeTokenSignature ||
                signature == invokeNFTSignature,
            "FWD: invalid function signature"
        );
        ...
        // If MATIC involved, transfer out equivalent wrapped MATIC
        if (req.value > 0) {
            wMATIC.safeTransferFrom(req.from, msg.sender, req.value);
        }
    ...
    function setRelayerAddress(address _relayer, bool _value) public onlyOwner {
        relayers[_relayer] = _value;
    }

    function setWrappedMatic(address _wMATIC) public onlyOwner {
