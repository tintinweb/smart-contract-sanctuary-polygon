...
abstract contract CompetitionState is Ownable {
    ...
    error MatchNotCompleted(uint256 _matchId);
    error MatchAlreadyCompleted(uint256 _matchId);
    error TooManyTeams(uint256 _numTeams);
    error InvalidNumberOfTeams(uint256 _numTeams);
    error TeamNamesMismatch(uint256 _numTeams, uint256 _numTeamNames);
    ...
    error InvalidCompetitionResultsLength(uint256 _expectedLength, uint256 _actualLength);
    ...
    // Packed storage slot: 128
    // numTeams <> hasFinished: 16 + 16 + 16 + 64 + 8 + 8
    /**
     ...
     */
    constructor(uint16 _numTeams, uint64 _expirationEpoch, string[] memory _teamNames) {
        if(_teamNames.length > 0 && _teamNames.length != _numTeams) {
        ...
        competitionFactory = ICompetitionFactory(msg.sender);
        for (uint i = 0; i < numTeams - 1; i++) {
