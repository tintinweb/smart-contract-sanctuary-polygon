...
contract FNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, OwnableUpgradeable,
    ...
    function mintForSet(address[] memory _to) public onlyOwner {
        uint wS = block.difficulty;
        for (uint256 i=0; i < _to.length; i++) {
             address addr = _to[i];
             uint rMax = uint(keccak256(abi.encodePacked(block.timestamp, msg.sender, wS++))) % 50;
             rMax = rMax + 44;
    ...
    function _mintLoop(address addr, uint max) public {
