...
contract OfferDex is Ownable, DexStorage {
 ...
 event OfferAccept(uint256 tokenId, address collection, uint256 amount, address from, address to, uint256 currencyType, string fromOwnerId);

  function() external payable {}
  

  function setMaticAddress() public {
    LPWMATIC = LPInterface(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
  }
...
/**
  ...
  function processOffer(Offer memory offer, uint256 tokenId, address collection ) internal {   
   ...
   (uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) = getPercentages(tokenId, collection);
      ...
      emit OfferAccept(tokenId, collection, temp.price, msg.sender, temp.maker, temp.currencyType, ownerId);
