...
//     :       :   : :  : :: : :  :::  :: : :       :      @@        @@     @@@    ::   :::  @@@@@@@   :: ::::  ::: ::::  @@@@@@@   :   : :  ::         ::  @@@@@@@     \\
    ...
    string private  M3TADAO_ORGANIZATION_PREFIX;
    string private  ORGANIZATION_SCHEMA;


    string  private _UserTable;
    uint256 private _UserTableId;
    string private  M3TADAO_PROFILE_PREFIX;
    string private  PROFILE_SCHEMA;

    string  private _ProposalTable;
    uint256 private _proposalTableID;
    string private  PROPOSAL_PREFIX;
    string private  PROPOSAL_SCHEMA;


    string  private _VotingTable;
    uint256 private _votingTableID;
    string private  VOTES_PREFIX;
    string private  VOTES_SCHEMA;

        // Tableland Hiring table variables

    string  private _hireTable;
    uint256 private _hiringTableId;
    string private  HIRE_PREFIX;
    string private  HIRE_SCHEMA;
...
// 0xD504d012D78B81fA27288628f3fC89B0e2f56e24
    ...
    {
        M3TADAO_PROFILE_PREFIX = "M3taProfile";
        PROPOSAL_PREFIX = "M3taProposal";
        VOTES_PREFIX = "M3taVote";
        HIRE_PREFIX = "M3taHire";
        M3TADAO_ORGANIZATION_PREFIX = "M3taOrganization";
        HIRE_SCHEMA = " (hireID text, profAddress text, accountid text, hireTitle text, hireDescription text);";
        // VOTES_SCHEMA = " (proposalid text, accountid text, respondent text, vote text, unique(proposalid, respondent));";
        VOTES_SCHEMA = " (proposalid text, accountid text, respondent text, vote text);";

        PROPOSAL_SCHEMA = " (proposalid text, accountid text, proposer text, body text);";
        ORGANIZATION_SCHEMA = " (founderAddress text, identifier text, OrganizationID text, groupID text, OrganizationHex text, OrganizationName text, imageURI text, description text);";
        PROFILE_SCHEMA = " (userName text, userAddress text, identifier text, userDID text, userName text, imageURI text, description text);";
        ...
        _OrganizationTable = toNameFromId(M3TADAO_ORGANIZATION_PREFIX, _OrganizationTableId);
        ...
        // _UserTable = toNameFromId(M3TADAO_PROFILE_PREFIX, _UserTableId);
        ...
        // _ProposalTable = toNameFromId(PROPOSAL_PREFIX, _proposalTableID);
        ...
        // _VotingTable = toNameFromId(PROPOSAL_PREFIX, _votingTableID);
        ...
        // _hireTable = toNameFromId(HIRE_PREFIX, _hiringTableId);
    ...
    function indexProfile(string memory profiledid , string memory userName , string memory imageURI , string memory description) public {
        ...
        toInsert(
                ...
                string.concat(
                    quote(Strings.toHexString(msg.sender)),
                    ",",
                    quote(Strings.toString(profileID.current())),
                    ",",
                    quote(profiledid),
                    ",",
                    quote(userName),
        ...
        public  
    {
        ...
        toInsert(
                ...
                string.concat(
                    quote(Strings.toHexString(msg.sender)),
                    ",",
                    quote(Strings.toString(ValistID.current())),
                    ",",
                    quote(Strings.toString(OrganizationID)),
                    ",",
                    quote(groupID),
                    ",",
                    quote(OrganizationHex),
                    ",",
    ...
    function Proposal(
            ...
            string memory statement =
                toInsert(
                    ...
                    string.concat(
                        quote(Strings.toString(proposalID.current())),
                        ",",
                        quote(Strings.toString(accountID)),
                        ",",
                        quote(Strings.toHexString(msg.sender)),
                        ",",
                        quote(proposal_CID)
    ...
    function Vote(
        ...
        string memory statement =
            toInsert(
                ...
                string.concat(
                    quote(Strings.toString(qid)),
                    ",",
                    quote(Strings.toString(accountID)),
                    ",",
                    quote(Strings.toHexString(msg.sender)),
                    ",",
                    vote ? quote("1") : quote("0")
                )
            );
            runSQL(_votingTableID,statement);      
     ...
     function createHiringRequest(uint accountID , string memory hireTitle , string memory hireDescription) public  {
        ...
        string memory statement =
            toInsert(
                ...
                string.concat(
                    quote(Strings.toString(hireID.current())),
                    ",",
                    quote(Strings.toHexString(msg.sender)),
                    ",",
                    quote(Strings.toString(accountID)),
    ...
    function rejectHiringRequest( uint256 hireId) public {
        ...
        string memory statement = toDelete(HIRE_PREFIX, _hiringTableId, filter);
