...
// File: DoubleOne/IERC5006.sol

// https://github.com/ethereum/EIPs/blob/master/assets/eip-5006/contracts/IERC5006.sol
...
// File: DoubleOne/TowerInventory.sol
...
contract CDHAccessControl is AccessControl {
    ...
    bytes32 public constant MINTER_ROLE_ADMIN =
        0x11c2020b6b4f4e00c2410234e0c72636b4739cf7cda4d8e24ef6b881350e6704; //keccak256(abi.encodePacked("MINTER_ROLE_ADMIN"));
...
abstract contract CardPool is Ownable, Pausable {
    ...
    function identifier() public pure virtual returns (bytes1) {
        return IDENTIFIER;
    }
    ...
    function numberOfCardsByRarity(bytes1 rarity)
        external
        view
        returns (uint256)
    {
    ...
    function getCardIdByRarity(bytes1 rarity, uint256 index)
        external
        view
        returns (uint256)
    {
    ...
    function getCardNameByType(bytes16 _cardType)
        public
        view
        returns (string memory)
    {
    ...
    function getCardTypeByName(string memory _name)
        external
        view
        returns (bytes16)
    {
    ...
    EIP712MetaTransaction("Crazy Defence Heroes Inventory", "1.0")
{
    ...
    string public baseUrl =
        "https://gateway.pinata.cloud/ipfs/QmXQMCos8RXSvUwFzob5P42B9XcgQ9TxEVr9quCFEBDGAj/"; // temporary
    ...
    function getAllTokens(address user)
        external
        view
        returns (uint256[] memory)
    {
    ...
    function setPool(bytes1 poolId, address poolAddress)
        external
    ...
    function _generateTokenId(uint256 _cardId) internal returns (uint256) {
        ...
        uris[uint256(tokenId)] = string(
            abi.encodePacked(baseUrl, uint2str(uint256(tokenId)), ".json")
        );
    ...
    /// @param tokenIdcardId Card Id number to generate Token Id
    /// @param amount Amount of tokens to mint
    /// @param newuri Data of minting token
    function mint(
        address account,
        uint256 tokenIdcardId,
        uint256 amount,
        string memory newuri
    ) external onlyMinter {
        counter[tokenIdcardId]++;
    
        require(blockNumber[uint256(tokenIdcardId)] == 0, "Token already minted");

        uris[uint256(tokenIdcardId)] = string(
            abi.encodePacked(newuri, uint2str(uint256(tokenIdcardId)), ".json")
        );
        blockNumber[uint256(tokenIdcardId)] = block.number;

        _mint(account, tokenIdcardId, amount, "0x00");
    }
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            for (uint256 j; j < senderTokenList.length; j++) {
                if (senderTokenList[j] == ids[i]) {
                    senderTokenList[j] = senderTokenList[
                        senderTokenList.length - 1
                    ];
                    senderTokenList.pop();
                }
            }
...
// File: DoubleOne/WrappedTowerInventory.sol

pragma solidity 0.7.6;

contract WrappedTowerInventory is CDHInventory, IERC5006 {
    ...
    mapping(uint256 => mapping(address => EnumSet.UintSet))
        private _userRecordIds;
    uint256 _curRecordId;
    uint256 recordLimit;

    constructor(string memory uri_, uint256 recordLimit_) CDHInventory(uri_) {
    ...
    function createUserRecord(
        ...
        safeTransferFrom(owner, address(this), tokenId, amount, "");
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        returns (bool)
    {
        return interfaceId == type(IERC5006).interfaceId;
    }
