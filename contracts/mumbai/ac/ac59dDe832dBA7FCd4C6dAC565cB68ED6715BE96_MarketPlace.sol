...
contract MarketPlace is ERC165 {
     ...
     * @return bool returns true on successful transaction
     */
    function buy(uint256 _orderNonce, uint256 amount)
        external
        payable
        returns (bool)
    {
        ...
        if (_order.paymentToken == address(0)) {
            ...
            require(
                msg.value == 0,
                "Paying Native token for non-native token payment"
            );
        }
    ...
    function sendValue(
        ...
        if (_token != address(0)) {
            ...
            require(success, "Native token payment failed");
        }
    }

    bool paymentInProgress;
    modifier paying() {
        require(!paymentInProgress, "Payment in progress");
