...
contract Dividends is IDIVIDENDS, Pausable, Ownable {
    ...
    function amountSnapshotsAccount(address p_contractHoldings, address p_account, uint256 p_idSnapshot) public view override returns(uint256, bool) { 
        uint256 balanceAccount = ERC20CUSTOM(p_contractHoldings).balanceOfAt(p_account, p_idSnapshot); 
        uint256 totalSupply = ERC20CUSTOM(p_contractHoldings).totalSupplyAt(p_idSnapshot);
        
        if (balanceAccount == 0) { return (0, true); }

        uint256 amountDividendsAccount = ((balanceAccount * 1 ether) / totalSupply) * s_amountSnapshots[p_contractHoldings][p_idSnapshot];
        amountDividendsAccount = amountDividendsAccount / 1 ether;

        return (amountDividendsAccount, ITOKENHOLDINGS(p_contractHoldings).snapshotUsed(p_account, p_idSnapshot));
    }
