...
interface IExecutable {
    ...
    function changeVotingDuration(uint32 _days) external;
}

contract Xxraphlygon is Ownable {
    ...
    function changeVotingDuration(
      ...
      uint32 _days
        ...
        bytes32 txHash = keccak256(abi.encodePacked(nonce, _days));
        require(
            !confirmedTransactions[msg.sender][txHash],
            "Transaction already confirmed"
        );
        confirmedTransactions[msg.sender][txHash] = true;
        if (isConfirmed(txHash)) {
            executableInstance.changeVotingDuration(_days);
            delete confirmedTransactions[msg.sender][txHash];
        }   
    }
