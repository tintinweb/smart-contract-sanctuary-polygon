...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function returnRace(uint _num) public view returns(uint[] memory){
        ...
        uint[] memory _steps;
        bool[] memory done;
        uint _size = rulesByRace[currentEvent].gridSize;
        uint _rem = 0;
        uint fHundo = 500;
        for (uint i=0; i<_size; i++){
            _rem = fHundo.mod(perfByRacer[_racers[i]].firstLeg);
            if (_rem > 0){_rem = 1;}
                
            _steps[i] = fHundo.div(perfByRacer[_racers[i]].firstLeg) + _rem;
            _rem = fHundo.mod(perfByRacer[_racers[i]].secondLeg);
            if (_rem > 0){_rem = 1;}
                
            _steps[i] = _steps[i] + fHundo.div(perfByRacer[_racers[i]].secondLeg) + _rem;
        }
        /*
