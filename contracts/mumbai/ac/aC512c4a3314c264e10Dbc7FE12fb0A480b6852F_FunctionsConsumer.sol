...
contract FunctionsConsumer is FunctionsClient, ConfirmedOwner, ERC721URIStorage {
  ...
  string public pokeName;
  uint256 public pokeId;
   ...
   */
  constructor(address oracle) FunctionsClient(oracle) ConfirmedOwner(msg.sender) ERC721("The OffChain Poke's", "TOP") {}
  ...
  function executeRequest(
    ...
    if (args.length > 0) pokeName = args[0];
    Functions.Request memory req;
  ...
  function fulfillRequest(bytes32 requestId, bytes memory response, bytes memory err) internal override {
    ...
    uint256 _id = abi.decode(response, (uint256));
    pokeId = _id;
    emit Pokemon(pokeName, _id);
  }
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            pokeName,
            '",',
            '"description": "A Functions Demo Monster", '
            '"image": "https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/',
            string(Strings.toString(pokeId)),
            '.png"',
            "}"
          )
        )
