...
contract DaddyUsername is ERC721, ReentrancyGuard, Ownable
{
	...
	function validateUsername(string memory _username) internal view returns(bool)
	{
		...
		require(((bytes(_username).length >= 2) && (bytes(_username).length <= 50)), "Must be between 2 and 50 characters.");
	...
	function mintUsernameToken(string memory _username) internal
	{
		...
		usernamesMapping[new_token_id] = Username(_username, true);
	...
	function buildSVGFromToken(Username memory username) internal pure returns(string memory)
	{
		...
		if (keccak256(abi.encodePacked()) == keccak256(abi.encodePacked("glitch")))
		{
