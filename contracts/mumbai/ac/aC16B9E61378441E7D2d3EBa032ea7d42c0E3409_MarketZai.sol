...
contract MarketZai is ERC721, Ownable {
    ...
    uint256 public blockPerDay = 28800;//bscTestnet //testnet polygon = 17280 ::: mainNet 43200
    ...
    event InterfacesUpdated(address zaiNFT, address levelStorage, address ipfsStorage, address payments,address claimAddress, uint256 nurseryPreMintNumber,address nurseryManagement, address oracle);
    event ZaiBorned(uint256 indexed state, uint256 zaiId, address owner);
    event UpdateBlockPerDay(uint256 oldData, uint256 newData);
    event MultiplicatorsSetted(uint256[4] oldMultiplicators,uint256[4] newMultiplicators);
    ...
    constructor() ERC721("Ticket_ZAI_NFT", "TICKET") {
        ...
        ticketCreationDate = TicketCreationDate(block.timestamp,block.timestamp,block.timestamp,block.timestamp);
    }
    ...
    function updateInterfaces() external {
        ...
        levelStorage = gameAddresses.getAddressOf(AddressesInit.Addresses.LEVEL_STORAGE);
        Iipfs = IipfsIdStorage(gameAddresses.getAddressOf(AddressesInit.Addresses.IPFS_STORAGE));
        IPay = IPayments(gameAddresses.getAddressOf(AddressesInit.Addresses.PAYMENTS));
        claimNFTsAddress = gameAddresses.getAddressOf(AddressesInit.Addresses.CLAIM_NFTS);
        nurseriesPremintNumber = INurseryNFT(gameAddresses.getAddressOf(AddressesInit.Addresses.NURSERY_NFT)).getPreMintNumber();
        INursMan = INurseryManagement(gameAddresses.getAddressOf(AddressesInit.Addresses.NURSERY_MANAGEMENT));
        Oracle = IOracle(gameAddresses.getAddressOf(AddressesInit.Addresses.ORACLE));
        emit InterfacesUpdated(address(IZai),levelStorage, address(Iipfs),address(IPay) ,address(claimNFTsAddress), nurseriesPremintNumber,address(INursMan), address(Oracle));
    }
    ...
    function createTickets(uint256 _quantity) external onlyOwner {
        uint256 _toAdd = (block.timestamp - ticketCreationDate.randomDate) / 1 days;
    ...
    function _mintSilverTicket(address _claimNFTsAddress,uint256 _currentSeason, uint256 _dateCreation) internal{
        uint256 _silverToMint = (block.timestamp - ticketCreationDate.silverDate) / 5 days; 
        if(_silverToMint != 0){
            ticketCreationDate.silverDate = block.timestamp;
            for (uint256 i ; i < _silverToMint; ) {
                _mintTicket(_claimNFTsAddress, _currentSeason,_dateCreation,1);
                unchecked {
                    ++i;
                }
            }
    ...
    function _mintGoldTicket(address _claimNFTsAddress,uint256 _currentSeason, uint256 _dateCreation) internal{
        uint256 _goldToMint = (block.timestamp - ticketCreationDate.goldDate) / 15 days; 
        if(_goldToMint != 0){
            ticketCreationDate.goldDate = block.timestamp;
            for (uint256 i ; i < _goldToMint; ) {
                _mintTicket(_claimNFTsAddress, _currentSeason,_dateCreation,2);
                unchecked {
                    ++i;
                }
            }
    ...
    function _mintPlatinumTicket(address _claimNFTsAddress,uint256 _currentSeason, uint256 _dateCreation) internal{
        uint256 _platinumToMint = (block.timestamp - ticketCreationDate.platinumDate) / 30 days; 
        if(_platinumToMint != 0){
            ticketCreationDate.platinumDate = block.timestamp;
            for (uint256 i ; i < _platinumToMint; ) {
                _mintTicket(_claimNFTsAddress, _currentSeason,_dateCreation,3);
                unchecked {
                    ++i;
                }
            }
    ...
    function _mintTicket(address _address,uint256 _currentSeason, uint256 _dateCreation, uint256 _ticketType)
        internal
        returns (uint256)
    {
        _tokenIds.increment();
        ...
        ticketDatas[_newItemId] = TicketDatas(_ticketType,_currentSeason,_dateCreation);
        ...
        returns (uint256)
    {
        ...
        if (_ticket.ticketType == 0) {
            ...
            uint256 zaiId = _generateRandomZai(_name, msg.sender, _ticket.ticketType);
    ...
    function _randomMint(string memory _name) internal returns (uint256 zaiId) {
        ...
        if (_random < 150) {
            //1,5%
            _state = 3;
        }else if (_random >= 150 && _random < 800) {
            // 800 - 150 = 650 => 6,5%
            _state = 2;
        }else if (_random >= 800 && _random < 3000) {
            // 3000 - 800 = 2200 => 22%
            _state = 1;
        }else if (_random >= 3000) {
            // 70%
            _state = 0;
        }
    ...
    function getZaiPrice() external view returns (uint256[4] memory) {
        return [_getZaiPrice(0),_getZaiPrice(1),_getZaiPrice(2),_getZaiPrice(3)];
    }
        ...
        returns (uint256)
    {
        ...
        _price -=
            (_pastBlockInCurrentDay * _toReducePerBlock);
