...
interface IERC20 is IERC20Metadata {
    ...
    function transfer(address to, uint256 usdwWei) external returns (bool);

    function approve(address spender, uint256 usdwWei) external returns (bool);
...
interface ISWAP {
    ...
    function swapOut(address toToken, uint256 usdwWei) external returns (bool);
}
...
interface IOFCHAIN is ISWAP {
    ...
    function withdraw(uint256 usdwWei) external payable returns (bool);
...
interface ILOCK {
    ...
    function unLockFrom(address from, uint256 usdwWei) external returns (bool);

    function ofLock(address wallet) external returns (uint256);

    function lock(uint256 usdwWei, uint256 month) external returns (bool);
...
interface IOWNER is ILOCK {
    ...
    function mint(address to, uint256 usdwWei) external returns (bool);

    function burnFrom(address from, uint256 usdwWei) external returns (bool);
...
interface IUSDW is IERC20, ISTAKEUSDW, IBASE {
...
contract USDW is IERC20, IUSDW, IOWNER {
    using Address for address;
    ...
    constructor(
        ...
        _initContract(NAME, SYMBOL, SYMBOL_OF);
    }
    ...
    function mint(address to, uint256 usdwWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_isWallet(to) && _isOwner()) {
            return _mint(to, usdwWei);
        }
        return false;
    }

    function burnFrom(address from, uint256 usdwWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_hasWei(from, usdwWei) && _isOwner()) {
            return _burn(from, usdwWei);
        }
        return false;
    }
    ...
    function lockFrom(
        ...
        if (_hasWei(from, usdwWei) && day > 0 && _isOwner()) {
            return _lock(from, usdwWei, day);
        }
        return false;
    }

    function unLockFrom(address from, uint256 usdwWei)
        public
        virtual
        override
        returns (bool)
    {
        if (_lockOf(from) >= usdwWei && _isOwner()) {
            _decLock(from, usdwWei);
            return true;
        }
        return false;
    }
    ...
    function burnBy(uint256 usdwWei) external virtual override returns (bool) {
        if (_isBlack(_wallet()) || !_isAnd()) {
            return false;
        }
        if (_hasWei(_wallet(), usdwWei)) {
            return _burn(_wallet(), usdwWei);
        }
        return false;
    }

    function mintBy(uint256 usdwWei) external virtual override returns (bool) {
        if (_isBlack(_wallet()) || !_isAnd()) {
            return false;
        }
        IERC20 _token = IERC20(address(BTC));
        uint256 btcOf = _token.balanceOf(_wallet());
        uint256 btcWei = _fromUsdw(usdwWei, _symbolOf, 18);
        if (btcOf < btcWei) {
            return false;
        }

        return _mint(_wallet(), usdwWei);
    }
    ...
    function stake(
        ...
        if (_hasWei(staker, usdwWei)) {
            _transfer(staker, _contract, usdwWei);
        }
        _stake(pool, staker, usdwWei, stake_Month);
        return true;
    }

    function unStake() public virtual override returns (bool) {
        uint256 usdwStaking = _staking(_wallet());
        bool isTime = block.timestamp > _unStakeAt(_wallet());

        if (usdwStaking > 0 && isTime && _isAnd()) {
            _fromContract(_wallet(), usdwStaking);
            _decStake(_wallet(), usdwStaking);
            return true;
        }
        return false;
    }
    ...
    function donate(
        ...
        if (isMonth && _hasWei(_sender(), usdwWei)) {
            ...
            _transfer(_sender(), to, usdwWei);
            _transfer(to, _contract, usdwWei);
            _incStake(to, usdwWei);
            return true;
        }
    ...
    function send(address to) public payable virtual override returns (bool) {
        uint256 usdwWei = _toUsdw(_chainOf, msg.value, _decimal);

        if (1 > usdwWei || 1 > msg.value) {
            return false;
        }
    ...
    function deposit() public payable virtual override returns (bool) {
        ...
        bool isValue = msg.value > 0 && usdwWei > 0;
        // bool canBuy = _maxCap >= _totalSupply() + usdwWei && isValue;
        ...
        _fromContract(_sender(), usdwWei);

        return called_;
    }

    function withdraw(uint256 usdwWei)
        public
        payable
        virtual
        override
    ...
    function swapOut(address toToken, uint256 usdwWei)
        public
        virtual
        override
        returns (bool)
    {
        ...
        return _swapOut(toToken, usdwWei);
    }

    ////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    function lock(uint256 usdwWei, uint256 day)
        external
        virtual
        override
        returns (bool)
    {
        if (day > 0 && _hasWei(_sender(), usdwWei)) {
            return _lock(_sender(), usdwWei, day);
        }
        return false;
    }
    ...
    function approve(address spender, uint256 usdwWei)
        external
        virtual
        override
        returns (bool)
    {
        ...
        return _approve(_sender(), spender, usdwWei);
    }
    ...
    function transfer(address to, uint256 usdwWei)
        public
        virtual
        override
        returns (bool)
    ...
    function transferFrom(
        ...
        _transfer(from, to, usdwWei);
        _approve(from, _sender(), _allowed[from][_sender()] - usdwWei);
        return true;
    }
    ...
    function _lock(
        ...
        _incLock(from, usdwWei);

        return true;
    }

    function _incTotalExchange(uint256 usdwWei) internal {
        _totalExchange += usdwWei;
    }

    function _incTotalSupply(uint256 usdwWei) internal {
        _totalBalance += usdwWei;
    }

    function _decTotalSupply(uint256 usdwWei) internal {
        _totalBalance -= usdwWei;
    }

    function _incStake(address staker_, uint256 usdwWei) internal {
        if (usdwWei > 0) {
            _stakingOf[staker_] += usdwWei;
            _totalStaking += usdwWei;
            _totalExchange += usdwWei;
        }

        emit Staking(staker_, usdwWei, _stakeAt(staker_));
    }

    function _decStake(address staker_, uint256 usdwWei) internal {
        _stakingOf[staker_] -= usdwWei;
        _totalStaking -= usdwWei;
        _totalExchange += usdwWei;
    ...
    function _incLock(address locker_, uint256 usdwWei) internal {
        _totalLocking += usdwWei;
        _locksOf[locker_] += usdwWei;
        _transfer(locker_, _contract, usdwWei);
        emit Locking(locker_, usdwWei, _unLockAt(locker_));
    }

    function _decLock(address locker_, uint256 usdwWei) internal {
        _totalLocking -= usdwWei;
        _locksOf[locker_] -= usdwWei;
        _fromContract(locker_, usdwWei);
    ...
    function _emitTransfer(
        ...
        emit Transfer(from, to, usdwWei);
        _afterTransfer(from, to, usdwWei);
    }

    function _incBalance(address wallet, uint256 usdwWei) internal {
        _balances[wallet] += usdwWei;
    }

    function _decBalance(address wallet, uint256 usdwWei) internal {
        _balances[wallet] -= usdwWei;
    }
    ...
    function _burn(address from, uint256 usdwWei) internal returns (bool) {
        _beforeTransfer(from, address(0), usdwWei);

        _incTotalExchange(usdwWei);
        _decTotalSupply(usdwWei);
        _decBalance(from, usdwWei);

        _emitTransfer(from, address(0), usdwWei);
        return true;
    }

    function _fromContract(address to, uint256 usdwWei) internal {
        if (_hasWei(_contract, usdwWei)) {
            _transfer(_contract, to, usdwWei);
        } else {
            _mint(to, usdwWei);
        }
    }

    function _mint(address to, uint256 usdwWei_) internal returns (bool) {
        uint256 usdwWei = _ParseWei(usdwWei_);

        if (!_isWallet(to) || usdwWei < 1) {
            return false;
        }
        _incTotalExchange(usdwWei);
        _incTotalSupply(usdwWei);
        _incBalance(to, usdwWei);

        _emitTransfer(_contract, to, usdwWei);
        return true;
    }
    ...
    function _approve(
        ...
        require(_isWallet(owner_), "USDW: approve from the zero address");
        require(_isWallet(spender_), "USDW: approve to the zero address");

        _allowed[owner_][spender_] = usdwWei;
        emit Approval(owner_, spender_, usdwWei);
        return true;
    }
    ...
    function _swapFrom(
        ...
        uint256 usdwWei = _toUsdw(
            _SYMBOLOF(token),
            tokenWei,
            _decimalOf(token)
        );
        bool isTokenWei = _isTokenList(token) && usdwWei > 0 && tokenWei > 0;

        if (isTokenWei) {
            // token to sender
            // redeem token to usdw
            // transfer token to contract this, then
            require(_token.transferFrom(fr_, ctr_, tokenWei) == true, "Error");
            _fromContract(fr_, usdwWei);
            return true;
        }
    ...
    function _swapOut(address toToken, uint256 usdwWei)
        internal
        returns (bool)
    {
        IERC20 _token = IERC20(toToken);

        uint256 tokenWei = _fromUsdw(
            usdwWei,
            _SYMBOLOF(toToken),
            _decimalOf(toToken)
        );

        uint256 fee_swap_ = _fee_swap(usdwWei);
        uint256 usdwWei_tx = usdwWei + fee_swap_;

        bool isusdwWei = _hasWei(_sender(), usdwWei_tx) &&
            _token.balanceOf(_contract) >= tokenWei;
        bool isTokenWallet = _isTokenList(toToken) && toToken != _contract;
        if (isusdwWei && isTokenWallet) {
            ...
            return _transfer(_sender(), _contract, usdwWei_tx);
        }
        return false;
    }

    function _fee_swap(uint256 usdwWei) internal view returns (uint256) {
        uint256 mulOf_ = usdwWei * _oracle(_FEE_SWAP, "USD");
        return _ParseWei(mulOf_ / _10Pow(_decimal));
    }

    function _ParseWei(uint256 usdwWei_) internal pure returns (uint256) {
        uint256 usdwWei6 = usdwWei_ / _10Pow(12);
        return usdwWei6 * _10Pow(12);
    }
    ...
    function _transfer(
        ...
        _beforeTransfer(from, to, usdwWei);
    ...
    function _fromUsdw(
        ...
        uint256 mulOf_ = usdwWei * _oracle(_symbol, CURRENCY);
