...
contract YexleAppian is ERC721Burnable, Ownable {
    ...
    */
    ...
    mapping(address => bool) private voteRecord;
    mapping(uint => mapping(address => bool)) private approverDecision;
    mapping(uint => mapping(address => bool)) private L1approverDecision;
    mapping(uint => mapping(address => bool)) private L2approverDecision;
    ...
    */
    ...
    function approveAndTransferLand1(approverData memory _data) external{
        ...
        if(voteRecord[msg.sender]){ revert alreadyApproved();}
    ...
    function landDocumentViewRequest(address _requester, uint tokenId) external{
    ...
    //Read -> View Land Document: (Who ever got access by L1approver can view the doc).
    function viewDocumentByRequester(address _requester, uint tokenId) external view returns(string memory DocumentURI){
        if(viewAccessGranted[_requester][tokenId] && !saleStatus[tokenId]){
            if (!_exists(tokenId)) { revert URIQueryForNonexistentToken(); }
            return string(abi.encodePacked(metadataUri, holdUri[tokenId]));
        }else{
    ...
    function ownerDecisionToBuyLand(address _requester, uint _tokenId, bool _status) external{
        require(msg.sender == _owners[_tokenId],"you are not the owner of nft");
        if(viewAccessGranted[_requester][_tokenId] && landRequest[_tokenId][msg.sender]){
    ...
    // Write -> Registration Submission by the requester
    ...
    function approveByL1(approverData memory _data) external{
        if(msg.sender != L1Approver){ revert notL1Approver();}
        if(voteRecord[msg.sender]){ revert alreadyApproved();}
        if(registrationDocumentStatus[_data._tokenId] == true && _data.status == true){
            L1approverDecision[_data._tokenId][msg.sender] = _data.status;
            voteRecord[msg.sender] = true;
            approvecount[_data._tokenId] += 1;
        }
    }
    ...
    function approveByL2(approverData memory _data) external{
        ...
        if(L1approverDecision[_data._tokenId][msg.sender] != true){ revert L1Rejected();}
        if(voteRecord[msg.sender]){ revert alreadyApproved();}
        if(L1approverDecision[_data._tokenId][L1Approver] == true && _data.status == true){
            L2approverDecision[_data._tokenId][msg.sender] = _data.status;
            voteRecord[msg.sender] = true;
            approvecount[_data._tokenId] += 1;
        }
        if(approvecount[_data._tokenId] == 2){
    ...
    function viewDocumentAfterSale(address _newOwner, uint _tokenId) external view returns(string memory DocumentUri){
        if(_newOwner == _owners[_tokenId]){
            return string(abi.encodePacked(metadataUri, holdUri[_tokenId]));
        }else{
            return "address is not owner of the nft";
        }
    }
    /**
