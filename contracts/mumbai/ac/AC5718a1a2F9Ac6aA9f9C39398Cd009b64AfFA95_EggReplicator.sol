...
contract EggReplicator is BaseAccessControl {
    
    address private _tokenContractAddress;

    event ReplicaCreated(address indexed to, uint tokenId, uint eggValue);
    ...
    function replicate(
        address to, uint tokenId, uint value, string memory cid, string memory hatchCid) external onlyRole(COO_ROLE) {
        EggToken(tokenContract()).mintReplica(to, tokenId, value, cid, hatchCid);
