    ...
    AbstractSlasher
{
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; i = uncheckedInc(i)) {
            if (amounts[i] != 1) {
                // not an NFT
                continue;
            }
        ...
        nonReentrant
    {
        ...
        rewardsGenerator.aggregate(policyBookAddress, tokenIds, _nftMintId.uncheckedSub(1)); // nftMintId is changed, so -1
    }
    ...
    function _stakeBMIX(
        ...
        rewardsGenerator.stake(policyBookAddress, _nftMintId.uncheckedSub(1), stblAmount); // nftMintId is changed, so -1
    }
    ...
    function _transferProfit(uint256 tokenId, bool onlyProfit) internal {
        ...
        if (allowStakeProfit) {
            ...
            totalProfit = totalProfit.uncheckedSub(bmiStakingProfit);
