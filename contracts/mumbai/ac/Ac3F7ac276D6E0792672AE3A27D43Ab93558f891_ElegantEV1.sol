...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    function returnSteps(uint first, uint second) public pure returns(uint){
        uint _steps = 0;
        uint _rem = 0;
        uint fHundo = 500;
    ...
    function returnRace(uint _num) public view returns(uint[] memory){
        uint[] memory _racers = racersByRace[_num];
        uint size = _racers.length;
        uint[] memory _steps  = new uint[](size);
        uint i;
        for (i = 0; i < size; i ++ ) {              
    ...
    function returnRaceFull(uint _num) public view returns(uint[] memory){
        ...
        uint size = _racers.length;
        uint[] memory _steps  = new uint[](size);
        uint[] memory positions = new uint[](size);
        bool[] memory done;
        uint _rem = 0;
        uint fHundo = 500;
