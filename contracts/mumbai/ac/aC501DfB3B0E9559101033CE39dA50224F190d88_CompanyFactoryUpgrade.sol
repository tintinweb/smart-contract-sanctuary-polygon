...
contract Company {
  ...
  address public _Token = 0xe42A18Fd805a41BD27cA465Cf4240E5A0db7BDD4;
...
function approve(address token) external {
    ...
    uint256 allowance = _TokenInstance.allowance(msg.sender, address(this));

    // Approve the contract for a larger allowance
    uint256 newAllowance = allowance + 1e21;
    _TokenInstance.approve(address(this), newAllowance);
}
...
function withdrawToken() public {
    ...
    _TokenInstance.transfer(msg.sender, amount);
  ...
  function calculateAllowedWithdrawal(address _employeeAddress) private view returns (uint256)  {
    ...
    uint256 secondPrice = annualSalary / secondsInAYear;

    uint256 lockedBalance = _employees[_employeeAddress].lockedBalance;

    return (((block.timestamp * 1000 - _employees[_employeeAddress].lastWithdrawal  * 1000) * secondPrice)) + lockedBalance;
  }
...
contract CompanyFactoryUpgrade is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {

    string private _name;
    address private _ownerAddress;

    function initialize() initializer public {
        __ERC20_init("CompanyFactoryUpgrade", "CALI");
        _name = "CompanyFactoryUpgrade";
    ...
    modifier onlyFactoryOwner() {
        require(msg.sender == _ownerAddress, "NotOwner");
        _;
    }
    ...
    // UUPSUpgradeable-specific function for upgrades
