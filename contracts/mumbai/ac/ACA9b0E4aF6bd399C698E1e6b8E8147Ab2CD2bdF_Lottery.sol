...
contract Lottery {
    ...
    uint256 public constant maxTickets = 100000000000000; // maximum tickets per lottery
    uint256 public constant ticketCommission = 0.10 ether; // commition per ticket
    ...
    address public titosAddress = 0x7aBF39F2a1B1A445d6D5c5C9eFd9A657d0880E00; // Replace with the TITOS token contract address
    ...
    function BuyTickets(uint256 _amount) public {
        ...
        IERC20(titosAddress).transferFrom(msg.sender, address(this), _amount);
        require(_amount % ticketPrice == 0, string.concat("the value must be multiple of ", Strings.toString(ticketPrice), " TITO's tokens"));

        uint256 numOfTicketsToBuy = _amount / ticketPrice;
    ...
    function AllTickets() public view returns (uint256) {
