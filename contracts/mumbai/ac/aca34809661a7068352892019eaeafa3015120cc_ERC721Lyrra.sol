    ...
    function hash(SkuInfo memory data) internal /*pure*/ returns (bytes32) {
       ...
       bytes32[] memory royaltiesBytes = new bytes32[](data.royalties.length);
        for (uint i = 0; i < data.royalties.length; i++) {
            royaltiesBytes[i] = LibPart.hashData(data.royalties[i]);
        }
        bytes32[] memory creatorsBytes = new bytes32[](data.creators.length);
        for (uint i = 0; i < data.creators.length; i++) {
            creatorsBytes[i] = LibPart.hashData(data.creators[i]);
        }
        /*
...
abstract contract ERC721Lazy is OwnableUpgradeable,IERC721LazyMint, ERC721Upgradeable, Mint721Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl,SignerRole {
   ...
   function setFansFee(address payable recipient,uint32 value) public onlyOwner{
    ...
    event payMintAndTransferHash(bytes message,address signer );
    // pay mint
    function payMintAndTransfer(uint256 tokenId, address to,string memory tokenURI,Sku.SkuInfo memory skuInfo, uint8 v, bytes32 r, bytes32 s) public payable override { 
          ...
          require(skuInfo.start > 0 ,"start wrong");
          require(skuInfo.end > 0 ,"end wrong");
          ...
          emit payMintAndTransferHash(message,signer);

          address minter = address(tokenId >> 96);
          require(minter == skuInfo.creators[0].account, "token id incorrect");
          //require(minter == ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", Sku.hash(skuInfo))), skuInfo.v, skuInfo.r, skuInfo.s), "owner should sign skuInfo");
