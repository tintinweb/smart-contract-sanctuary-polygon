    ...
    OwnableUpgradeable
{
    ...
    mapping(uint256 => uint256) fleetRaidClock;
    ...
    function buildFleet(
        ...
        ITreasureHunt(rewardPool).transferCost(fleetMintPrice, true);
    ...
    function getFleetRaidTime(uint256 tokenID) public view returns(uint256) {
        return fleetRaidClock[tokenID];
    }

    function readFleetRaidTime(uint256 tokenID) public view returns(uint256) {
        if (fleetRaidClock[tokenID] >= block.timestamp) {
            return fleetRaidClock[tokenID] - block.timestamp;
        } else {
            return 0;
        }
    }

    function setFleetRaidTime(uint256 tokenID) public {
        require(msg.sender == rewardPool, "You can't set the time!");
        fleetRaidClock[tokenID] = block.timestamp + fleetRaidInterval;
    }
    ...
    function addContract(uint256 tokenID, uint8 _contractDay) public fleetOwner(tokenID) {
        ITreasureHunt(rewardPool).transferCost(_contractDay * fleetInfo[tokenID].pirates.length, false);
    ...
    function getRank(uint256[] memory ships) public returns (uint8) {
        uint8[] memory starShips;
    ...
    function getHaki(uint256[] memory pirates) internal returns (uint256) {
        uint256 haki;
