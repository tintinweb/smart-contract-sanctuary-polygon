...
contract GameItems is Ownable, ERC2981, Formatter, ERC721URIStorage {
    
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    uint256 immutable public maxSupply = 50;
    ...
    string[] private faces = ["guruguru", "hosome", "hukigen", "kirakira", "kotti", "kowai", "nemui", "normal", "unique", "uwame"];    
    string[] private styles = ["bear", "bunny", "cat", "cow", "deer", "dog", "dragon", "fox", "horse", "mouse", "normal", "unique"];
    ...
    constructor() ERC721 ("GameItems", "GITS") {
        address receiver = 0x2420a79B525734AeA50F3E02ad933Ab795D8523C;
        uint96 feeNumerator = 1000; // the fee rate is 10%
    ...
    function safeMint(address to, uint256 _colorIndex, uint256 _faceIndex, uint256 _styleIndex) public onlyOwner {
        ...
        require(_styleIndex < 13, "_styleIndex must be from 0 to 12");
        
        string memory _color = colors[_colorIndex];
        string memory _face = faces[_faceIndex];
        string memory _style = styles[_styleIndex];

        ampontants[_tokenIds.current()] = Ampontant({color: _color, face: _face, style: _style}); 
    ...
    function tokenURI(uint256 tokenId) public override view returns (string memory output) {
        string memory img0 = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 246.45 226.94"><polygon points="0.92 116 134.71 0.8 245.39 225.92 0.92 116" fill="#ff0" stroke="#000" stroke-miterlimit="10"/></svg>';
        ...
        output = formatTokenURI(imageURI);
        return output;
    }
}
...
contract Formatter {
    function svgToImageURI(string memory _svg) internal virtual pure returns (string memory){
    ...
    function formatTokenURI(string memory _imageURI) internal virtual pure returns (string memory){
