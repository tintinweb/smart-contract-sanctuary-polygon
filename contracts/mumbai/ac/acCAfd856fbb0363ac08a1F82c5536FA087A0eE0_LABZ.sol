    ...
    Ownable
{
    ...
    constructor(
        ...
        maxLabzPerAccounts = 1000000 * 10**18;
        setPriceOracle(priceOracle);
        setSupply(max, 75000000 * 10**18);
        setMultiSignatureWallet(multi);
        startSale(_unlockTime);
    }
    ...
    function setSupply(uint256 maxSupply_, uint256 vipSupply_)
        internal
    {
        maxSupply = maxSupply_;
        vipSaleSupply = vipSupply_;
    }
    ...
    function TokenMaticRate() public view returns (uint256 r) {
        r = 20;
    }

    function TokenUSDRate() public view returns (uint256 r) {
        r = AKXPriceOracle(_priceOracle).AkxUSD() / 1e18;
    }

    function TokenTotalValue() public view returns (uint256 r) {
        r = totalMinted * TokenUSDRate();
    }
    ...
    function _beforeTokenTransfer(
        ...
        require(
            from.isContract() != true,
            "can only transfer to non contract addresses"
        );
    ...
    function _mint(address account, uint256 amount)
        ...
        override(ERC20)
    {
        if (saleStarted == true && totalMinted + amount >= vipSaleSupply) {
            saleEnded = true;
            saleStarted = false;
            saleEndTime = block.timestamp;
            revert("all vip tokens are sold");
        }
        super._mint(account, amount);
        if (enableProtocolFee) {
            uint256 _fee = _calcProtocolFee(amount / 1e18);
            super._mint(feeWallet, _fee);
            emit ProtocolFeeMinted(feeWallet, _fee);
        }
        totalMinted += amount;
    }
    ...
    function transfer(address to, uint256 amount)
        ...
        returns (bool)
    {
        require(to != address(this), "cannot transfer between contracts");
        require(
            (block.timestamp > unlockTime) || (msg.sender == owner()),
            "transfers locked"
        );
    ...
    modifier onlyWhenUnlock() {
        require(unlockTime < block.timestamp, "cannot do this when locked");
        _;
    }
...
abstract contract FloatingPoint {
    ...
    function calculateExactTokensForMatics(uint256 tokenPrice, uint256 maticPrice, uint256 valueSentInMatics) public view returns(uint256 result) {
