...
contract JamAirdropCampaign is Ownable, ReentrancyGuard {
    enum AssetType {
        ERC20,
        ERC721,
        ERC1155
    ...
    mapping(uint256 => AirdropCampaign) private _allCampaigns;
    ...
    constructor(uint256 maxBatchSize, uint256 feePerBatch) Ownable() {
        require(
            maxBatchSize > 0,
            "JamAirdropCampaign: batch size must be greater than zero"
        );
    ...
    modifier onlyOperators() {
        require(
            ...
            "JamAirdropCampaign: caller is not operator"
        );
        _;
    }
        ...
        onlyOwner
    {
        require(
            ...
            "JamAirdropCampaign: lengths mismatch"
        );
    ...
    function createAirdropCampaign(
        ...
        require(
            ...
            "JamAirdropCampaign: campaign already created"
        );
        ...
        require(
            ...
            "JamAirdropCampaign: insufficient airdrop fee"
        );
        if (msg.value > airdropFee) {
            (bool success, ) = payable(msg.sender).call{
                value: msg.value - airdropFee
            }("");
            require(success, "JamAirdropCampaign: failed to return excess");
        }

        // Validate data
        require(
            block.timestamp < startingTime,
            "JamAirdropCampaign: starting time too low"
        );
        for (uint256 i = 0; i < assets.length; i++) {
            Asset memory asset = assets[i];
            require(
                uint256(asset.assetType) <= 3,
                "JamAirdropCampaign: invalid asset type"
            );
            if (asset.assetType == AssetType.ERC20)
                require(
                    asset.assetId == 0,
                    "JamAirdropCampaign: invalid ERC20 asset ID"
                );
            else if (asset.assetType == AssetType.ERC721)
                require(
                    asset.availableAmount == 1,
                    "JamAirdropCampaign: invalid ERC721 amount"
                );
        }
    ...
    function updateCampaign(
        ...
        require(
            campaign.creator == msg.sender,
            "JamAirdropCampaign: caller is not campaign owner"
        );

        // Make sure that this campaign has not started yet
        require(
            block.timestamp < campaign.startingTime,
            "JamAirdropCampaign: campaign started, cannot update assets"
        );
    ...
    function airdrop(
        ...
        require(
            ...
            "JamAirdropCampaign: campaign does not exist"
        );
        AirdropCampaign storage campaign = _campaignById[campaignId];
        require(
            block.timestamp > campaign.startingTime,
            "JamAirdropCampaign: campaign not start yet"
        );
        require(
        ...
        require(
            ...
            "JamAirdropCampaign: too many assets airdropped"
        );
        ...
        for (uint256 i = 0; i < assetIndexes.length; i++) {
            ...
            if (asset.assetType == AssetType.ERC20) {
                ...
                require(
                    success,
                    "JamAirdropCampaign: failed to send ERC20 assets"
                );
    ...
    function withdrawAirdropFee(address recipient) external onlyOwner {
        ...
        require(success, "JamAirdropCampaign: failed to withdraw airdrop fee");
    }
}
