...
contract Nightwalkers is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(whitelistEndTime_ > whitelistStartTime_, "Not valid time");
        ...
        _setDefaultRoyalty(treasuryAddress, numerator);
    }

    function mintWhitelist(uint256 quantity, bytes32[] memory proof) external payable nonReentrant {
        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(proof, whitelistRoot, leaf), "Not whitelisted");
        require(block.timestamp >= whitelistStartTime, "Whitelist not started");
        require(block.timestamp <= whitelistEndTime, "Whitelist ended");
    ...
    function mintPublic(uint256 quantity) external payable nonReentrant {
        require(block.timestamp >= publicStartTime, "Public not started");
