...
import { StructsLibrary } from "../will/StructsLibrary.sol";
 ...
 */
contract WillAccountFactory {
    // SimpleAccount public immutable accountImplementation;
    WillAccount public immutable willAccountImplementation;

    constructor(IEntryPoint _entryPoint) {
        willAccountImplementation = new WillAccount(_entryPoint);
    }
     ...
     */
    function createAccount(address owner,uint256 salt) public returns (WillAccount ret) {
        address addr = getAddress(owner, salt);
        uint codeSize = addr.code.length;
        if (codeSize > 0) {
            return WillAccount(payable(addr));
        }
        ret = WillAccount(payable(new ERC1967Proxy{salt : bytes32(salt)}(
                address(willAccountImplementation),
                abi.encodeCall(WillAccount.initialize, (owner))
            )));
    }
