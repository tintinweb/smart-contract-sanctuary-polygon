...
import "./EversePausableUpgradeable.sol";
 ...
 */
contract Everse1155 is
    ERC1155Upgradeable,
    EversePausableUpgradeable
{
    ...
    uint256 constant LOWER_TOKEN_ID_MASK_COMPOSE = uint256(2 ** 128);
    ...
    */ 
     ...
     */
    //onlyOwner - REMOVED 
    function createTier(uint128 tierID, uint256 maxSupply) external whenNotPaused {
        require(!this.tierExists(tierID), "Tier already exists");
        require(tierID != 0 && maxSupply >= 1, "Invalid tier definition");
        require(_tierCurrentSupply[tierID] == 0 && tierMaxSupply[tierID] == 0, "Tier exists");
    ...
    //onlyOwner - IS REMOVED, AS THE UPLOADER CAN MINT ON IT'S BEHALF.
    function mintLDAByAdmin(address owner, uint256 ldaID, bytes calldata data) public onlyOwner whenNotPaused {
    ...
    //       LDAs from different tiers can be minted together. 
    //onlyOwner - IS REMOVED, AS THE UPLOADER CAN MINT ON IT'S BEHALF.
    function mintLDAByOwner(uint256 ldaID) public whenNotPaused {
        require(_owners[ldaID] == address(0), "LDA already minted");
        (uint128 tierID,) = decomposeLDA_ID(ldaID);
        // NOTE: This check also implicitly checks that the tier exists as mintable()
        //       is a stricter requirement than exists(). 
    ...
    function decomposeLDA_ID(
        uint256 ldaID
    ) public pure virtual returns (uint128 tierID, uint128 tokenID) {
    ...
    function testCompose(
        uint128 tierID
        , uint128 tokenID
    ) public view virtual returns (uint128 tierID1, uint128 tokenID1) {
       return decomposeLDA_ID(composeLDA_ID(tierID, tokenID));
    }

    function composeLDA_ID(
        uint128 tierID
        , uint128 tokenID
    ) public view virtual returns (uint256 ldaID) {
        //console.log('solidity compose ldaId', (tierID * LOWER_TOKEN_ID_MASK_COMPOSE) | tokenID);
        return (tierID * (LOWER_TOKEN_ID_MASK_COMPOSE)) | tokenID;
    }
...
contract EversePausableUpgradeable is PausableUpgradeable, OwnableUpgradeable {
