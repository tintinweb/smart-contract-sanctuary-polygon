...
interface ICampWhitelistInterface {
    function isWhitelisted(address _address) external view returns (bool);
}

contract CampNFT721 is ERC721 {
    // ICampWhitelistInterface address
    address private CampWhitelistAddress;
    ICampWhitelistInterface campWhitelist;

    address public owner;
    uint256 public tokenCounter;
    uint256 public transhuman_total_supply = 70;
    uint256 public posthuman_total_supply = 70;
    uint256 public support_total_supply = 70;
    ...
    constructor(
        ...
        tokenCounter = 0;
        owner = msg.sender;
        campWhitelist = ICampWhitelistInterface(whitelistAddress);
    ...
    modifier isWhitelisted(address _accountAddress) {
        require(
            campWhitelist.isWhitelisted(_accountAddress),
            "Only whitelisted user can mint!"
        );
        _;
    }
    ...
    function verifySign(
        ...
        bytes32 hash = keccak256(abi.encodePacked(this, price, tokenURI));
        require(
            owner ==
                ecrecover(
                    keccak256(
    ...
    function createCollectible(
        ...
        uint256 price,
        Sign memory sign
    ) external isWhitelisted(msg.sender) returns (uint256) {
        // (!usedNonce[sign.nonce], "Nonce : Invalid Nonce");
        uint256 currentTime = block.timestamp;
        require(
            (currentTime - deployTime) / 86400 < saleEndDate,
            "Sale period is invalid"
        );
        ...
        verifySign(tokenURI, price, sign);

        if (price == 500000000000000000000) {
            require(
                transhuman_total_supply > 0,
                "insufficient transhuman total supply"
            );

            transhuman_total_supply = transhuman_total_supply - 1;
        }

        if (price == 2000000000000000000000) {
            require(
                posthuman_total_supply > 0,
                "insufficient posthuman total supply"
            );
            posthuman_total_supply = posthuman_total_supply - 1;
        }

        if (price == 0) {
            require(
                support_total_supply > 0,
                "insufficient support total supply"
            );
            support_total_supply = support_total_supply - 1;
        }
    ...
    function resetSupply(
        ...
        transhuman_total_supply = tts;
        posthuman_total_supply = pts;
        support_total_supply = sts;
    }

    function endSale() external onlyOwner {
        uint256 currentTime = block.timestamp;

        require(currentTime - deployTime > ONE_MONTH, "Date is invalid.");
        transhuman_total_supply = 0;
        posthuman_total_supply = 0;
        support_total_supply = 0;
    }
