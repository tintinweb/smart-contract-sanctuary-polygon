...
import "../../collections/child/interface/IERC721Burnable.sol";
...
contract BSBFactoryUpgradeable is Initializable, OwnableUpgradeable, IBSBTypes, BSBBaseUpgradeable, VRFConsumerBaseUpgradeable {
    ...
    uint public treeHouseMergeStartingPrice;
    ...
    function __BSBFactory_init(
        ...
        treeHouseMergeStartingPrice = 10000;
    ...
    function setTreeHouseMergeStartingPrice(uint amount_) external onlyOwner {
        treeHouseMergeStartingPrice = amount_;
    }
    ...
    function mergeTreeHouses(uint[] calldata housesIds_) external {
        IERC721Upgradeable ERC721Caller =  IERC721Upgradeable(contracts["treeHouse"]);
        ...
        BSBBloodTokenCaller.spend(treeHouseMergeStartingPrice * ETHER, _msgSender());

        treeHousesStats_[treeHouseToBeUpgradedIndex_].size.value += 1;
    ...
    function _burnTreeHouse(uint id_) internal {
        ...
        idsToBurn[0] = id_;
        IERC721Burnable(contracts["treeHouse"]).burn(idsToBurn, _msgSender());
        delete idsToBurn;
    }
    ...
    function reRoll(uint toBeUpgradedTokenId_, uint[] calldata gen1Tokens_) external {
        ...
        require(IERC721Upgradeable(gen1Address_).ownerOf(toBeUpgradedTokenId_) == _msgSender(), "BSB420");

        for (uint i = 0; i < gen1Tokens_.length; ++i) {
            require(IERC721Upgradeable(gen1Address_).ownerOf(gen1Tokens_[i]) == _msgSender(), "BSB420");
        }
    ...
    function _burnGen1Tokens(uint[] memory genTokens_) internal {
        IERC721Burnable(contracts["gen1"]).burn(genTokens_, _msgSender());
    }
