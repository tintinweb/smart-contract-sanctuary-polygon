...
contract Vesting is Ownable {
  ...
  event AddUsersRewards(address indexed owner, uint256 amount, address indexed user);
  

  constructor(address mgbToken_, uint256 periodFinish_) {
    mgbToken = IERC20(mgbToken_);
  ...
  function addUsersRewards(Reward[] memory rewards_) external onlyOwner {
    for (uint i = 0; i < rewards_.length; i++) {
      ...
      emit AddUsersRewards(msg.sender, r.amount, r.user); 
    }
  }
  ...
  /**
   * @dev calculates total amounts must be rewarded and transfers MGB to the address
   */
  function getReward() public {
    ...
    if (reward > 0) {
      ...
      mgbToken.safeTransfer(msg.sender, reward);
...
//periodfinish -- wallet addr 
//mgbtoken adrr -- token adrr