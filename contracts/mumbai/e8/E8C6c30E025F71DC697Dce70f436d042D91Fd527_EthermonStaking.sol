...
contract EthermonStaking is EthermonStakingBasic, EthermonEnum {
    ...
    function depositeTokens(
        ...
        require(
            !ethermonTadeData.isOnBorrow(_monId) && obj.trainer == msgSender(),
            "Either you mon is already on stake or borrowed by someone else."
        );

        uint256 balance = emon.balanceOf(msgSender()) * 10**decimal;
        ...
        uint256 hourlyEmon = (data.teamPower / stakingData.SumTeamPower()) *
            600 *
            timeElapsed; //Nice to have not essential
        data.balance += hourlyEmon; //This would be 0 as lastCalled is 0

        ethermonTadeData.addBorrowingItem(
            obj.trainer,
            obj.monsterId,
            0,
            address(0x4aFEfaf8Fa2F1e18ba0D2633052A4cE6f4743A61),
            true,
            dayTime,
            currentTime
        );
        ...
        emit Deposite(msgSender(), address(this), data.emons);
    }
    ...
    function withdrawTokens(uint256 _lockId) internal {
        EthermonStakingInterface stakingData = EthermonStakingInterface(
            stakingDataContract
        );
        ...
        // require(
        //     data.emons >= minDepositeValue,
        //     "Amount is < minimum deposite value"
        // );
        // require(data.owner == msgSender(), "You are not the owner of this PFP");

        uint256 emonBalance = data.emons + data.balance;
        emon.safeTransfer(data.owner, emonBalance);
    ...
    function updateTokens(uint256 _lockId) external {
        EthermonStakingInterface stakingData = EthermonStakingInterface(
            stakingDataContract
        );
        uint256 currentTime = now;
        ...
        if (timeElapsed > 0 && currentTime > data.endTime) {
            ...
            data.balance +=
                (data.teamPower / stakingData.SumTeamPower()) *
                600 *
                timeElapsed;

            data.emons += data.balance;
            emit Withdraw(address(this), data.owner, data.emons);
            emit Deposite(data.owner, address(this), timeElapsed);
        ...
        //bytes memory output = abi.encode(data);
        stakingData.updateTokenData(data.balance, data.lastCalled, _lockId);
        emit Deposite(data.owner, address(this), timeElapsed);
    }
}