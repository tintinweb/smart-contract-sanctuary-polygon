...
contract RouteProxy is FlashLoanReceiverBaseV2, Withdrawable {
  ...
  constructor(address approveProxy, address _addressProvider) FlashLoanReceiverBaseV2(_addressProvider) {
  ...
  function _flashloan(
    ...
    for (uint256 i; i < assets.length; i++) {
      _assets[i] = assets[i] == address(0) ? _WMATIC_ADDRESS_ : assets[i];
    }
  ...
  function _multiHopSingleSwap(PathInfo[] memory pathInfos) internal returns (uint256[] memory outputs) {
    ...
    if (pathInfos[pathInfoNum - 1].fromToken == address(0)) {
      _checkDeposit(pathInfos[pathInfoNum - 1].adapter, pathInfos[pathInfoNum - 1].pool, outputs[pathInfoNum]);
    }
    ...
    _checkWithdraw(pathInfos[pathInfoNum - 1].adapter, pathInfos[pathInfoNum - 1].toToken, outputs[pathInfoNum]);
  }
  ...
  function _calcMultiHopSingleSwap(PathInfo[] memory pathInfos) internal returns (uint256[] memory outputs) {
    uint256 pathInfoNum = pathInfos.length;
    ...
    require(pathInfos[pathInfoNum - 1].fromToken != pathInfos[pathInfoNum - 1].toToken, "from to are the same");
  ...
  function _singleHopMultiSwap(WeightPathInfo memory weightPathInfo) internal returns (uint256 output) {
    require(
  ...
  function _calcSingleHopMultiSwap(WeightPathInfo memory weightPathInfo) internal returns (uint256 output) {
    require(
      weightPathInfo.weights.length == weightPathInfo.adapters.length &&
        weightPathInfo.weights.length == weightPathInfo.pools.length,
      "Invalid input length"
  ...
  function _multiHopMultiSwap(WeightPathInfo[] memory weightPathInfos) internal returns (uint256[] memory outputs) {
  ...
  function _calcMultiHopMultiSwap(WeightPathInfo[] memory weightPathInfos) internal returns (uint256[] memory outputs) {
    outputs = new uint256[](weightPathInfos.length + 1);
    outputs[0] = weightPathInfos[0].amountIn;
    for (uint256 i = 1; i < weightPathInfos.length; i++) {
      require(weightPathInfos[i - 1].toToken == weightPathInfos[i].fromToken, "Not valid multihop Path");
  ...
  {
    require(linearWeightPathInfo.weights.length == linearWeightPathInfo.weightPathInfos.length, "Invalid input length");
    uint256 totalWeight;
    ...
    for (uint256 i; i < splitNum; i++) {
      ...
      require(
        linearWeightPathInfo.weightPathInfos[i][hopNum - 1].toToken == linearWeightPathInfo.toToken,
        "Not valid linear toToken"
      );
      require(
        linearWeightPathInfo.weightPathInfos[i][0].fromToken == linearWeightPathInfo.fromToken,
    ...
    returns (uint256 output)
  {
    ...
    for (uint256 i; i < splitNum; i++) {
      ...
      rest = rest.sub(partAmountIn);
      linearWeightPathInfo.weightPathInfos[i][0].amountIn = partAmountIn;
      uint256[] memory outputs = _calcMultiHopMultiSwap(linearWeightPathInfo.weightPathInfos[i]);
      output += outputs[outputs.length - 1];
    }
  }
  ...
  function shieldSwap(
    ...
    for (uint256 i; i < flashDes.length; i++) {
      require(flashDes[i].pathInfos[0].amountIn == flashDes[i].amountIn, "flashloan amountIn not match");
      require(
        flashDes[i].pathInfos[0].fromToken == flashDes[i].asset &&
          flashDes[i].asset == flashDes[i].pathInfos[flashDes[i].pathInfos.length - 1].toToken,
      ...
      if (
        ...
        _flashloan(flashDes[i].asset, flashDes[i].amountIn, abi.encode(flashDes[i].pathInfos));

        if (toToken == flashDes[i].asset) {
          output += IERC20(flashDes[i].asset).uniBalanceOf(address(this));
        }
        IERC20(flashDes[i].asset).uniTransfer(msg.sender, IERC20(flashDes[i].asset).uniBalanceOf(address(this)));
      }
    }
  ...
  function _checkDeposit(
    ...
    address wrapped = IRouterAdapter(adapter).factoryToWMatic(IRouterAdapter(adapter).factory(pool));
