...
contract hrdwrk is Ownable, ERC721A, ReentrancyGuard {

    string private reavealURL = "ipfs://QmdTaeG7dACdzmxjFih8yyXNhsdkpAMubyTEMMechBc7Qx";
    ...
    function mint(uint256 quantity) external payable callerIsUser {
        ...
        require(publicMints[msg.sender] + quantity <= maxPerUser, "MAX_LIMIT_YOU_CAN_MINT_REACHED");
        require(totalSupply() + quantity <= maxToken, "NOT_ENOUGH_SUPPLY_TO_MINT_DESIRED_AMOUNT");
        if (publicMints[msg.sender] == 0) {
            require(msg.value >= price * (quantity-1) || msg.sender == owner(), "Value below price");
        } else {
            require(msg.value >= price * quantity || msg.sender == owner(), "Value below price");
        }
        publicMints[msg.sender] = publicMints[msg.sender] + quantity;
        _safeMint(msg.sender, quantity);
    }

    function allowListMint(uint256 quantity) external payable callerIsUser {
        require(allowList[msg.sender] || msg.sender == owner(), "YOU_ARE_NOT_IN_ALLOWLIST");
        require(quantity > 0, "INVALID_QUANTITY");
        require(wlMints[msg.sender] + quantity <= maxPerUser, "MAX_LIMIT_YOU_CAN_MINT_REACHED");
        require(totalSupply() + quantity <= maxToken, "NOT_ENOUGH_SUPPLY_TO_MINT_DESIRED_AMOUNT");
        if (wlMints[msg.sender] == 0) {
            require(msg.value >= price * (quantity-1) || msg.sender == owner(), "Value below price");
        } else {
            require(msg.value >= price * quantity || msg.sender == owner(), "Value below price");
        }
        wlMints[msg.sender] = wlMints[msg.sender] + quantity;
        _safeMint(msg.sender, quantity);
    }
    ...
    function goPublic(uint256 _MaxClaim) external onlyOwner {
        publicSale = !publicSale;
        maxPerUser = _MaxClaim;
    }
