...
contract blog {
  ...
  function join(string memory anonname) public returns(string memory, address, uint8){
      // uint256 randNUm =  uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
      //   msg.sender))) % 20;
    ...
    //string memory anonname = string(bytes.concat(bytes(name), bytes(randNUm)));
    BD.anonName = anonname;

    return(anonname, msg.sender, uint8(block.timestamp));
  }
  ...
  function createBlog(string memory topic, string memory blogdetails, string memory _imageUrl) public returns(address, string memory, uint8){
    bloggersDetails storage BD = bloggersdetails[msg.sender];
    ...
    return(msg.sender, topic, uint8(block.timestamp));
  }
  ...
  function upVote(address blogOwner, uint8 blogId) public blogExist(blogOwner, blogId) returns(address, uint8){
    ...
    return (msg.sender, uint8(block.timestamp));
  }
  ...
  function downVote(address blogOwner, uint8 blogId) public blogExist(blogOwner, blogId) returns(address, uint8){
  ...
  function tip(address blogOwner, uint8 blogId, uint8 amount) public blogExist(blogOwner, blogId) returns(address, address, uint256){
    bloggersDetails storage BD = bloggersdetails[blogOwner];
    address to = BD.paymentaddress;
    IERC20(token).transferFrom(msg.sender, to, amount *1e18);
    BD.eachBlogs[blogId].totalTip = amount;  

    return(blogOwner, msg.sender, amount);
  }
  ...
  /// @dev A view function to get all blogs created by a person
  /// @dev you call the totalBloggersBlog function and pass it as your array lenght when calling this function in your
  // for loop
  function getBlog(address blogOwner, uint8 blogId) public view returns(blogs memory){
  bloggersDetails storage BD = bloggersdetails[blogOwner];
    require(BD.eachBlogs[blogId].timecreated != 0, "Blog doesn't exist");
    return (BD.eachBlogs[blogId]);
  }
