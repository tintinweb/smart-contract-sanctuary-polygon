...
// hevm: flattened sources of src/ChildDeployer.sol
// SPDX-License-Identifier: GNU-3 AND AGPL-3.0-or-later AND GPL-3.0-or-later
pragma solidity =0.6.12 >0.4.13 >=0.4.0 >=0.4.23 >=0.6.12;

////// lib/ds-guard.git/lib/ds-auth/src/auth.sol
...
////// lib/ds-guard.git/src/guard.sol
...
////// lib/ds-token.git/lib/ds-math/src/math.sol
...
////// lib/ds-token.git/src/token.sol
...
/* import "ds-token.git/token.sol"; */
/* import "ds-guard.git/guard.sol"; */

contract ChildToken is DSToken, DSGuard {

  constructor (string memory symbol_, string memory name_, uint cap_) public DSAuth() DSToken(symbol_) DSGuard() {
    name = name_;
    cap = cap_;
    // implement its own authority functions
    setAuthority(this);
  }
  ...
  /*
    ...
    */
  function deposit(address guy, bytes calldata depositData) external {
    uint256 wad = abi.decode(depositData, (uint256));
    mint(guy, wad);
  }
...
/* import "dss-vest/DssVest.sol"; */

contract Vester is DssVestMintable {
  constructor (address _gem) public DssVestMintable(_gem) {}
}

////// src/ChildDeployer.sol
/* pragma solidity >=0.4.0; */

/* import "./ChildToken.sol"; */
/* import "./Vester.sol"; */

contract ChildDeployer {

  ChildToken public immutable token;
  Vester public immutable vester;
  address public immutable proxy;
  address public immutable sender;
  uint public immutable blocknumber;

  constructor(string memory symbol, string memory name, uint cap, address proxy_) public {
    // Deploy child token and vester
    ChildToken token_ = new ChildToken(symbol,name,cap);
    Vester vester_ = new Vester(address(token_));

    // Let vester mint
    token_.permit(address(vester_),address(token_),token_.mint.selector);

    // Let polygon proxy mint (called 'deposit' on child chain)
    token_.permit(proxy_,address(token_),token_.deposit.selector);

    // Remove owner of token
    token_.setOwner(address(0));

    // Remove limits on maximum per-second vesting rate
    vester_.file("cap",type(uint).max);

    // Set msg.sender as only owner of vester
    vester_.rely(msg.sender);
    vester_.deny(address(this));
