...
contract ShibAnonMixerV2 is Ownable {
    ...
    function depositToken(address tokenAddress, uint decimals, uint256 tokenAmount, bool holdNFT) public {
        ...
        IERC20(tokenAddress).transferFrom(msg.sender, treasuryWallet, fee * decimals);
        IERC20(tokenAddress).transferFrom(msg.sender, address(this), amount * decimals);
    ...
    function withdrawToken(address tokenAddress, uint decimals, address[] memory addresses, bool[] memory nftHoldings, uint256 _ringIndex) external onlyRelayer {
        ...
        for (uint256 i = 0; i < addresses.length; i++) {
            if (nftHoldings[i]) {
                IERC20(tokenAddress).transfer(addresses[i], allowedTokenAmounts[_ringIndex - 1] * decimals);
            } else {
                IERC20(tokenAddress).transfer(addresses[i], withdrawableToken * decimals);
            }
        }
