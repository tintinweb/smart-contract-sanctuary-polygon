...
contract Liquidity is Ownable, Initializable {
    ...
    function initialize(address tokenAdd, uint allocation, uint subsequentClaims, uint timestamp) external initializer {
        require(timestamp > 0, "Liquidity : timestamp > 0");
        require(subsequentClaims > 0,"Liquidity : subsequentClaims > 0");
    ...
    function addDistributionWallet( address[] memory account, uint[] memory amount, uint[] memory startTime) external onlyOwner {
        require(account.length < 30,"Liquidity : length < 30");
        require((account.length == amount.length) && (startTime.length == amount.length),"Liquidity : length mismatch");
        uint currentTime = block.timestamp;

        for(uint i=0; i< account.length; i++) {
            require((total[0] + amount[i]) <= token.balanceOf(address(this)), "Liquidity : insufficient balance to allocate");
    ...
    function revokeDistributionWallet(address[] memory account) external onlyOwner {
        ...
        for(uint i=0; i< account.length; i++) {  
            ...
            require(user_.totalClaimed < user_.balance, "Liquidity : user claimed all funds or user may not be added");
            uint amountToRevoke = user_.balance - user_.totalClaimed;
            delete user[account[i]];

            if(amountToRevoke > 0) {
                require(token.balanceOf(address(this)) >= amountToRevoke, "Liquidity : insufficient balance to revoke");
                require(token.transfer(owner(),amountToRevoke),"Liquidity : revoke transfer failed");
            }
        }
    }
    ...
    function claim() external {
        ...
        require(user_.balance > 0, "Liquidity : user not exist");
        require(user_.totalClaimed < user_.balance, "Liquidity : total claim < total balance");
        require((user_.lastClaim > 0) && ((user_.lastClaim + claimPeriod) <= block.timestamp), "Liquidity : lastClaim < block.timestamp");
    ...
    function failcase( address tokenAdd, uint amount) external onlyOwner{
        ...
        if(tokenAdd == address(0)) {
            require(self.balance >= amount, "Liquidity : insufficient balance");
            require(payable(owner()).send(amount), "Liquidity : transfer failed");
        } else {
            require(IBEP20(tokenAdd).balanceOf(self) >= amount, "Liquidity : insufficient balance");
            ...
            require(IBEP20(tokenAdd).transfer(owner(),amount), "Liquidity : transfer failed");
        }
    }
}