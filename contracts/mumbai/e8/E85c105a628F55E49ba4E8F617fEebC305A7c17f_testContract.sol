...
// Deployed to Polygon Mumbai 0xCC415067B84f9b73c9897Cb0096fe2a8497AA580
...
contract testContract is Ownable, KeeperCompatibleInterface {
    ...
    mapping(address => uint) UserVoted2;
    ...
    function CreateNewProgram(
        ...
        fundGoal = fundGoal * (10 ^ 18 wei); // Error when fundgoal < 1  value
    ...
    function Accept(bytes32 programId) external {
        ...
        // emit error(UserVoted[programId][msg.sender]);

        // require(
        //     UserVoted2[msg.sender] == 0,
        //     "You have already Voted for this program"
        // );
        require(
            UserVoted[programId][msg.sender] == 0,
            "You have already Voted for this program"
        );

        // require(user.HasVoted = false , "You have already Voted for this program");
    ...
    function Donate(bytes32 programId) external payable {
        ...
        require(
            ...
            "Program not in fund raising state"
        );
    ...
    function getProgramVotes(bytes32 programId)
        public
        returns (
            uint,
            uint,
    ...
    function HasUserVoted(bytes32 programId) external view returns (uint) {
        return UserVoted[programId][msg.sender];
    }
