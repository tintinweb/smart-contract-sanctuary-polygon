...
contract BinaryOptionManager is
    AccessControl,
    ILiquidityPool
{
    ...
    constructor(ERC20 _tokenX, address _vault) {
        tokenX = _tokenX;
        owner = msg.sender;
        vault = _vault;
    ...
    function lock(
        ...
        tokenX.safeTransferFrom(msg.sender, vault, premium);
        // check todo:暂时注释
//        require(IVault(vault).whitelistedTokens(address(tokenX)), "Pool: tokenX is not whitelisted");
//        require(IVault(vault).stableTokens(address(tokenX)),"Pool: tokenX is not stable");
        IVault(vault).optionPoolDeposit(address(tokenX));
    ...
    function send(
        ...
        // vault transfer tokenX to user account todo:暂时注释
//        require(IVault(vault).whitelistedTokens(address(tokenX)), "Pool: tokenX is not whitelisted");
//        require(IVault(vault).stableTokens(address(tokenX)),"Pool: tokenX is not stable");
        IVault(vault).optionPoolWithdraw(address(tokenX), to, transferTokenXAmount);
        ...
        if (transferTokenXAmount <= ll.premium)
            ...
            // Platform loss
            emit Loss(id, transferTokenXAmount - ll.premium);
    }
    ...
    {
        uint256 tokenxBalance = IVault(vault).poolAmounts(address(tokenX));
        return tokenxBalance - lockedPremium;
    }
