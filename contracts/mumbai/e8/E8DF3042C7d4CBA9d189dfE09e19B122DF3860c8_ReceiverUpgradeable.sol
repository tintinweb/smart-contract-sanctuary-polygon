 ...
 * @author gotbit
 * @notice Contract responsible for receiving tokens from swaps to make our
 * bots slightly less noticeable.
 */

contract ReceiverUpgradeable is Initializable, Roles {
    using SafeERC20 for IERC20;

    address public manager;
    uint256 public receiverId;

    function init(address manager_, uint256 receiverId_) external initializer {
        manager = manager_;
    ...
    function withdraw(address token, address to, uint256 amount) external {
        ...
        require(
            IOfficerUpgradeable(officer).hasRole(ADMIN_ROLE, msg.sender) ||
                msg.sender == manager ||
                msg.sender == officer ||
