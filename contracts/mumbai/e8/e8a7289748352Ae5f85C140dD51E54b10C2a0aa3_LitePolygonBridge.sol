...
contract LitePolygonBridge is AdminModule {
    ...
    function _processMessageFromRoot(
        ...
        bridgeNonceToData[id_] = StateData(
            0,
            key_,
            encodedRootData_
        );
        ...
        //     ExchangePriceData memory exchangePriceData_ = abi.decode(encodedRootData_, (ExchangePriceData));
        //     IiTokenVaultPolygon(exchangePriceData_.childVault).updateExchangePrice(exchangePriceData_.exchangePrice);
        //     emit LogUpdatedExchangePrice(id_, exchangePriceData_.childVault, exchangePriceData_.exchangePrice);
        // }
    }

    function processUpdateExchangePrice(uint256[] memory ids) public {
        for (uint256 i = 0; i < ids.length; i++) {
            StateData memory stateData_ = bridgeNonceToData[ids[i]];
            ...
            if(stateData_.key == UPDATE_EXCHANGE_PRICE) {
                ...
                IiTokenVaultPolygon(exchangePriceData_.childVault).updateExchangePrice(exchangePriceData_.exchangePrice);
                emit LogUpdatedExchangePrice(ids[i], exchangePriceData_.childVault, exchangePriceData_.exchangePrice);
            } else if (false) {
                ...
                for (uint256 j = 0; j < length_; j++) {
                    ...
                    emit LogUpdatedExchangePrice(ids[i], exchangePriceDatas_[j].childVault, exchangePriceDatas_[j].exchangePrice);
                }
            }
            bridgeNonceToData[ids[i]].isExecuted = 1;
        }
    }
