...
contract ChainBattles is ERC721URIStorage{
    ...
    function createRandom(uint number) public view returns(uint){
        return uint(blockhash(block.number-number));
    }

    // Second method to generate random number
    function createRandomPartTwo(uint number) pure public returns(uint){
        return uint(keccak256(abi.encodePacked(number*3)));
    }

    // Third method to generate random number
    function createRandomPartThree(uint number) public view returns(uint){
        return number * uint(keccak256(abi.encodePacked(block.timestamp,block.difficulty,  
        msg.sender)));
    }
    ...
    function mint() public{
        ...
        console.log("Value of newItemId is ",newItemId);
