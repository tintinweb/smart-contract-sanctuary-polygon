...
contract PrivatePlacementMarket is OwnablePausable, ReentrancyGuardUpgradeable{
    ...
    mapping(uint256 => uint256) private _stockNftIdValue; //nftid => usdtValue
    ...
    function initBag() private {
        ...
        _stockNftIdValue[0] = _bags[0].stockValue;
        _stockNftIdValue[1] = _bags[1].stockValue;
        _stockNftIdValue[2] = _bags[2].stockValue;
        _stockNftIdValue[3] = _bags[3].stockValue;
        _stockNftIdValue[4] = _bags[4].stockValue;
    }
    ...
    function buy(uint256 index) external whenNotPaused {
        ...
        if (_bags[index].stockValue > 0) {
            ...
            _stockERC1155.mintStockNft(trader, index, 1);
        }
    ...
    function withdrawStockMargin() external whenNotPaused nonReentrant{
        ...
        for(uint256 index; index < bagSize; index++) {
            uint256 amount = _stockERC1155.balanceOf(msg.sender, index);
            if (amount > 0) {
                _stockERC1155.burn(msg.sender, index, amount);
            }
        }
    ...
    function addBag(uint256 index, uint256[] calldata bag_) external onlyOwnerOrOperator {
        require(index == bagSize, "index error");
        bagSize += 1;
        _bags[index] = Bag(bag_[0], bag_[1], bag_[2], bag_[3]);
        _stockNftIdValue[index] = bag_[2];
    }
    ...
    function stockNftIdValues() 
        external view returns(
            uint256[] memory ids, 
            uint256[] memory values
    ) {
        for (uint index; index < bagSize; index++) {
            ids[index] = index;
            values[index] = _bags[index].stockValue;
        }
    }
    ...
    function listStockNft(address user) external view returns(
        ...
        address[] memory accounts = new address[](bagSize);
        for (uint256 index; index < bagSize; index++) {
            accounts[index] = user;
            ids[index] = index;
            values[index] = stockNftValue(index);
        }
        amounts = _stockERC1155.balanceOfBatch(accounts, ids);
    }

    function stockNftValue(uint256 id) public view returns(uint256) {
        return _bags[id].stockValue;
    }
    ...
    function getAddresses() external view returns (
        ...
        return (address(_stockERC1155), _ttAddress, address(_stockMargin), _stockMargin.marginDepositAddress());
    }
...
contract StakingReward is OwnablePausable, ReentrancyGuardUpgradeable {
    ...
    mapping(address => OrderInfo[]) private _finishedOrders;
    ...
    function depositMock(uint256 amount, uint256 loopCounter) external onlyOwnerOrOperator {
        address staker = msg.sender;
        UserInfo storage user = _userInfo[staker];
        OrderInfo[] storage userOrders = _orders[staker]; 
        user.stakedAmount += loopCounter*amount;
        for (uint256 i; i<loopCounter; i++) {
            userOrders.push(OrderInfo(block.timestamp, amount, amount, block.timestamp));
        }
    ...
    function withdrawMock(uint256 amount) external onlyOwnerOrOperator {
    ...
    function finishedOrders(address staker) external view returns(OrderInfo[] memory) {
        return _finishedOrders[staker];
    }
...
contract StockERC1155 is OwnablePausable, ERC1155SupplyUpgradeable {
    ...
    function stockNftValue(uint256 id) external view returns(uint256) {
        return IPPM(_ppmAddress).stockNftValue(id);
    }
