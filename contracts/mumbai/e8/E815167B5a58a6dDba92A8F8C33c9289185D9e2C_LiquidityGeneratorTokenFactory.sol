...
import "../liquidity-generator/LiquidityGeneratorToken.sol";
...
contract LiquidityGeneratorTokenFactory {
    ...
    function createContract(
        ...
        uint256 totalSupply,
        address[] memory addressInfo,
        uint16[] memory feeInfo
    ) external payable {
        string memory name = tokenInfo[0];
        string memory symbol = tokenInfo[1];
        address router = addressInfo[0];
        address charityAddress = addressInfo[1];
        uint16 taxFeeBps = feeInfo[0];
        uint16 liquidityFeeBps = feeInfo[1];
        uint16 charityFeeBps = feeInfo[2];
        LiquidityGeneratorToken liquidityToken = new LiquidityGeneratorToken(name, symbol, totalSupply, router, charityAddress, taxFeeBps, liquidityFeeBps, charityFeeBps);
        liquidityToken.transfer(msg.sender, liquidityToken.totalSupply());
        liquidityToken.transferOwnership(msg.sender);
        feeReceiver.transfer(msg.value);
        emit TokenCreated(address(liquidityToken));
    }
}
