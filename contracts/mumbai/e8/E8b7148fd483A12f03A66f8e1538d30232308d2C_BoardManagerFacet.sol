...
contract BoardManagerFacet is Modifiers{

    function createBoard(string memory _gameId, uint256 _boardFee, address _feeToken, address _airDropToken, uint256 _airDropAmt) external onlyHost returns (address) {
        return BigBoardManagerLib.createBoard(_gameId, _boardFee, _feeToken, _airDropToken, _airDropAmt);
    }

    function cancelBoard(address _board) external founderOrHostOnly {
        //Board boardToBeCanceled = Board(_board);
        if(BigBoardManagerLib.getFactory().founderz[msg.sender]){
            Board(_board).cancelBoard();
        } else { // if msg.sender is not factory founder
            require(msg.sender == Board(_board).host(), "Only 1 host can cancel this board.");
            Board(_board).cancelBoard();
        }
        
    }

     function withdrawBBH() external onlyFounder {
        require(BigBoardManagerLib.getFactory().hostToken.transfer(msg.sender, BigBoardManagerLib.getFactory().hostToken.balanceOf(address(this))), "Unable to transfer");
    }
}
...
contract Modifiers {
    ...
    modifier onlyHost {
        require(BigBoardManagerLib.getFactory().hostToken.balanceOf(BigBoardManagerLib.msgSender()) > 0);
        _;
    }

    modifier onlyFounder{
        require(BigBoardManagerLib.getFactory().founderz[BigBoardManagerLib.msgSender()]);
        _;
    }

    modifier founderOrHostOnly{
        require(BigBoardManagerLib.getFactory().founderz[BigBoardManagerLib.msgSender()] || BigBoardManagerLib.getFactory().hostToken.balanceOf(msg.sender) > 0);
        _;
    }
