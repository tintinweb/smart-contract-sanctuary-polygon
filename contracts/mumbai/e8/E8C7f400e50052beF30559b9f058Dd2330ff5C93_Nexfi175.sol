...
contract Nexfi175 is Ownable, ReentrancyGuard, IERC1155Receiver {
    ...
    // update user's reward based on amount staked of users
    mapping(address => uint256) userRewardLockedUps;
    ...
    mapping(address => mapping(address => uint256)) private rewardLockups;
    ...
    struct RewardInfo {
        address rewardToken;
        uint256 startTime; // initial rewardEndTime
        uint256 endTime; // initial rewardEndTime
        uint256 apr;
        uint256 threshold;
    }
    ...
    function updateReward() external onlyOwner {
        ...
        for (uint256 i = 0; i < nftContractAddressList.length; i++) {
            ...
            for (uint256 j = 0; j < _tokenIdList.length; j++) {
                if (extraBoosts[nftContractAddressList[i]][_tokenIdList[j]].value == 0) {
                    extraBoosts[nftContractAddressList[i]][_tokenIdList[j]].value = 1000; // nft price by 1$
                } else {
                    extraBoosts[nftContractAddressList[i]][_tokenIdList[j]].value += 
                        extraBoosts[nftContractAddressList[i]][_tokenIdList[j]].value * apr / percentage;
                }

                amount = extraBoosts[nftContractAddressList[i]][_tokenIdList[j]].value;
            }
        }
        ...
        nftRewardPerPeriod.indexs[_rewardStartTime] = nftRewardPerPeriod.rewardStartTime.length + 1;
    ...
    function updatePool() internal {
        ...
        for (uint256 i=0; i<nftRewardPerPeriod.rewardStartTime.length; i++) {
            ...
            uint256 rewardAccum = nftRewardPerPeriod.rewardPerPeriod[key] * multipilier;

            nftRewardPerPeriod.accRewardPerShares[key] = nftRewardPerPeriod.accRewardPerShares[key] + (
    ...
    //     uint256 pending = 0;
    //     uint256 totalRewardLock = 0;

    //     if (rewardTokens.length > 0) {
    //         for (uint8 index = 0; index < rewardTokens.length; index++) {
    //             pending = pendingReward(_msgSender(), rewardTokens[index]);

    //             totalRewards = totalRewards.add(pending).sub(
    //                 rewardLockups[_msgSender()][rewardTokens[index]]
    //             );

    //             rewardLockups[_msgSender()][rewardTokens[index]] = pending;
    //             totalRewardLock = rewardLockups[_msgSender()][
    //                 rewardTokens[index]
    //             ];
    //         }
    //         emit RewardLocked(_msgSender(), totalRewardLock, block.timestamp);
    //     }
    // }
        ...
        returns (uint256 pending)
    {
        ...
        for (
            ...
            pending += pendingReward(account, nftRewardPerPeriod.rewardStartTime[index]);
        }
    }
    ...
    function pendingReward(
        ...
        uint256 adjustedTokenPerShare = nftRewardPerPeriod.accRewardPerShares[key];
        if (totalStakings > 0) {
            uint256 rewardAccum = nftRewardPerPeriod.rewardPerPeriod[key] * multipilier;
        ...
        uint etmAmount = adjustedTokenPerShare * user.amount;
        ...
        //     .div(10000)
        //     .div(PRECISION_FACTOR)
        //     .add(rewardLockups[_msgSender()][BUSD_BSC]);
    }
