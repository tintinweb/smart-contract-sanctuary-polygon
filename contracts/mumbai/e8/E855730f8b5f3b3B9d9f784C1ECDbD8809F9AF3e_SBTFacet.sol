...
contract SBTFacet is IERC1155MetadataURI {
    LibAppStorage s;
    ...
    function mintBatch(address[] memory _tos, uint256 tokenId_) external {
        // _tos must be longer than 1.
        require(_tos.length > 0);

        uint256 projectId = s.tokenMetadata[tokenId_].projectId;

        // Not authorized.
        require(LibProjects.getAdminByProjectId(projectId, LibERC2771Context.msgSender()));
        ...
        for (uint256 i = 0; i < _tos.length; i++) {
            ...
            require(s.balances[tokenId_][to] == 0);

            s.balances[tokenId_][to]++;
    ...
    function burn(address _from, uint256 tokenId_) external {
        ...
        uint256 fromBalance = s.balances[tokenId_][_from];

        // Don't own this token.
        require(fromBalance >= 1);
        unchecked {
            s.balances[tokenId_][_from] = fromBalance - 1;
        }
    ...
    function uri(uint256 _id) public view returns (string memory) {
        ...
        require(bytes(s.tokenMetadata[_id].name).length > 0);
        ...
        string memory output = string(abi.encodePacked(s.tokenUri, json));
        return output;
    }
}
...
struct LibAppStorage {
    ...
    mapping(address => mapping(uint256 => bool)) userProjectExists;
    // Mapping from project id to user address
    mapping(uint256 => address[]) projectUsers;
    mapping(uint256 => mapping(address => bool)) projectUserExists;
    // Mapping from project id to user admin
    mapping(uint256 => mapping(address => bool)) operatorAdmins;
}
...
library LibProjects {
    ...
    function getProjectById(uint256 _projectId) internal view returns (ProjectStruct memory) {
        return appStorage().projects[_projectId];
    }

    function getAdminByProjectId(uint256 _projectId, address _walletAddress) internal view returns (bool) {
        return appStorage().operatorAdmins[_projectId][_walletAddress];
    }

    function addUsersToProject(uint256 _projectId, address[] memory _addressList) internal {
        LibAppStorage storage s = appStorage();

        // Not authorized.
        require(s.operatorAdmins[_projectId][LibERC2771Context.msgSender()]);

        uint256 count = _addressList.length;
        for (uint256 i = 0; i < count; i++) {
            address walletAddress = _addressList[i];

            if (!s.projectUserExists[_projectId][walletAddress]) {
                s.projectUsers[_projectId].push(walletAddress);
                s.projectUserExists[_projectId][walletAddress] = true;
                s.operatorAdmins[_projectId][walletAddress] = false;
            }

            if (!s.userProjectExists[walletAddress][_projectId]) {
                s.userProjects[walletAddress].push(_projectId);
                s.userProjectExists[walletAddress][_projectId] = true;
            }
        }
    }
}