...
/* 
 source from https://github.com/balajiboggaram/tokenswap/blob/main/TokenSwap.sol
*/ 
...
contract TokenSwap {
    ...
    constructor(address _tokenA, address _owner1Address, address _tokenB, address _owner2Address) {
        tokenA = IERC20(_tokenA);
        tokenB = IERC20(_tokenB);
        owner1 = _owner1Address;
     ...
     *
     * Requirements : 
     * - 'msg.sender' should be either owner1 or owner2.
     * - amount1 should be approved by owner1.
     * - amount2 should be approved by owner2.
    */ 
    function swap(uint amount1, uint amount2) public returns (bool) {

        // Has to be called by owner1 or owner2 only
        require(msg.sender == owner1 || msg.sender == owner2, "401 Unauthorized Caller");
        
        // owner1 has to authorize the required amount to swap contract
        require(tokenA.allowance(owner1, address(this)) >= amount1, "owner1 has not authorized the required balance");
        
        // owner2 has to authorize the required amount to swap contract
        require(tokenB.allowance(owner2, address(this)) >= amount2, "owner2 has not authorized the requied balance");

        // Actual transfer 
        transferAmount(tokenA, owner1, owner2, amount1);
        transferAmount(tokenB, owner2, owner1, amount2);
...
contract i3WPFT is ERC20, ERC20Burnable, Ownable {
    
    mapping(address => bool) public _isBlacklisted;

    constructor() ERC20("i3 Waste PowerTech Token", "i3WPFT") {
    	mint(_msgSender(), 21000000 * 10 ** decimals());
    }
