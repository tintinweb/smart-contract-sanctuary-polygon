...
contract NINEMATIC is ERC20 {
    ...
    */
    constructor(address payable admin,address payable developeradd,address payable marketingadd,address tokenAddr) public {
        // add administrators here
        administrators 	= admin;
		developer 		= developeradd;
		marketing 		= marketingadd;
		
		require(!isContract(admin) && isContract(tokenAddr));
    ...
    function purchaseTokens(address _referredBy,uint256 value, uint256 totamt) public payable returns(uint256) {
        ...
        uint256 time;
        uint256 percent;
        require(totamt >= INVEST_MIN_AMOUNT);
		...
		for (uint256 i = 0; i < 30; i++) {
				if (referr != address(0)) {
					teambusiness[referr] = teambusiness[referr].add(totamt);
        ...
        {
            address sponser = _referredBy;
            if(boosterCount_[sponser] == 2 ){
                 boostertime_2[sponser] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponser].length; i++) {
                    
                    if(deposits[sponser][i].percent == 50) deposits[sponser][i].time    = 134;
                    if(deposits[sponser][i].percent == 75) deposits[sponser][i].time    = 115;
                    if(deposits[sponser][i].percent == 100) deposits[sponser][i].time   = 100;
                   
                }
            }

            if(boosterCount_[sponser] == 4 ){
                 boostertime_4[sponser] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponser].length; i++) {
                    
                    if(deposits[sponser][i].percent == 50) deposits[sponser][i].time    = 80;
                    if(deposits[sponser][i].percent == 75) deposits[sponser][i].time    = 73;
                    if(deposits[sponser][i].percent == 100) deposits[sponser][i].time   = 67;

                }
            }

            if(boosterCount_[sponser] == 6 ){
                 boostertime_6[sponser] = block.timestamp;
                for (uint256 i = 0; i < deposits[sponser].length; i++) {
                    
                    if(deposits[sponser][i].percent == 50) deposits[sponser][i].time    = 57;
                    if(deposits[sponser][i].percent == 75) deposits[sponser][i].time    = 54;
                    if(deposits[sponser][i].percent == 100) deposits[sponser][i].time   = 50;
                }
            }

            if(boosterCount_[sponser] >= 7 ){
                if(boostertime_7[sponser] == 0){
					boostertime_7[sponser] = block.timestamp;
					tokenBalanceLedger_[sponser] = SafeMath.add(tokenBalanceLedger_[sponser], stakeBalanceLedger_[sponser]);
				}
            }
