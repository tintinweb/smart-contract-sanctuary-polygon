...
interface ERC721 {
  ...
  /**
   * @dev Set or reaffirm the approved address for an Bundle of NFT.
   * @notice The zero address indicates there is no approved address. Throws unless `msg.sender` is
   * the current NFT owner, or an authorized operator of the current owner.
   * @param _approved The new approved NFT controller.
   * @param _tokenId[]  The NFTs to approve.
   */
  function approveBatch(address _approved, uint256[] calldata _tokenId) external;
...
contract NFToken is ERC721, SupportsInterface {
  ...
  /**
  $ @dev Stored Creator Permenatly for Sending Royalty
   */
  mapping(uint256 => address payable) public creator;
  ...
  /**
   * @dev Guarantees that the msg.sender is an owner or operator of the given NFT.
   * @param _tokenId [] ID of the NFTs to validate.
   */
  modifier canOperateBatch(uint256[] memory _tokenId) {
    for (uint256 i = 0; i < _tokenId.length; i++) {
      address tokenOwner = idToOwner[_tokenId[i]];
      require(tokenOwner == msg.sender || ownerToOperators[tokenOwner][msg.sender], NOT_OWNER_OR_OPERATOR);
      _;
    }
  }
  ...
  modifier canTransfer(uint256 _tokenId) {
    address tokenOwner = idToOwner[_tokenId];
    require(
      tokenOwner == msg.sender || idToApproval[_tokenId] == msg.sender || ownerToOperators[tokenOwner][msg.sender],
      NOT_OWNER_APPROWED_OR_OPERATOR
    );
    _;
  }

  modifier canTransferWithOwner(uint256 _tokenId) {
    address tokenOwner = idToOwner[_tokenId];
    require(
  ...
  /**
   * @dev Guarantees that _tokenId is a valid Token.
   * @param _tokenId [] IDs of the NFT to validate.
   */
  modifier validNFTokenBatch(uint256[] memory _tokenId) {
    for (uint256 i = 0; i < _tokenId.length; i++) {
      require(idToOwner[_tokenId[i]] != address(0), NOT_VALID_NFT);
      _;
    }
  }
   ...
   * @param _tokenId ID of the token to be approved.
   */
  function approveBatch(address _approved, uint256[] calldata _tokenId)
    external
    override
    canOperateBatch(_tokenId)
    validNFTokenBatch(_tokenId)
  {
    for (uint256 i = 0; i < _tokenId.length; i++) {
      address tokenOwner = idToOwner[_tokenId[i]];
      require(_approved != tokenOwner, IS_OWNER);

      idToApproval[_tokenId[i]] = _approved;
      emit Approval(tokenOwner, _approved, _tokenId[i]);
    }
  }
...
abstract contract NFTokenEnumerableMetadata is NFToken, ERC721Metadata, ERC721Enumerable {
  ...
  function _setTokenPayload(uint256 _tokenId, string memory _uri) internal validNFToken(_tokenId) {
  ...
  function _mint(address _to, uint256 _tokenId) internal virtual override {
    // console.log("To is ", _to,"Caller ", msg.sender);
    super._mint(_to, _tokenId);
    tokens.push(_tokenId);
    creator[_tokenId] = payable(_to);
 ...
 */
contract VorbitNFT is NFTokenEnumerableMetadata, Ownable {
  ...
  function randMod(uint256 _modulus, uint256 _randNonce) internal returns (uint256) {
  ...
  function updateTokenUri(uint256 _tokenId, string memory _uri) public validNFToken(_tokenId) onlyOwner {
    idToUri[_tokenId] = _uri;
  }
  ...
  ) public validNFToken(_tokenId) canTransfer(_tokenId) {
    super._setTokenUri(_tokenId, _uri);
    super._setTokenPayload(_tokenId, _payload);
  }
}