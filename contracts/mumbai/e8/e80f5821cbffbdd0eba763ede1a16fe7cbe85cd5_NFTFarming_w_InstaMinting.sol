...
contract NFTFarming_w_InstaMinting is IERC777Recipient {
    using SafeMath for uint256;
    ERC777 public artisToken;
    uint256 public TotalValueLocked;
    uint256 public feesCollected;
    uint256 public creationTime = block.timestamp;
    uint256 public constant _WITHDRAWTIMELOCK = 7 minutes;
    ...
    mapping(bytes => nft_object) public allNFTs;
    bytes[] public allNFTPoolIDs;
    uint256 public totalNFTIDs;

    constructor() payable {
        artisToken = ERC777(0xFF1e159e7C67C52cb37B744e5F2e66FeAEfea57E);
    ...
    */
    modifier onlyOwner() {
        require(adminAddress == msg.sender, "Caller not owner.");
        _;
    }
    ...
    */
    function nftRemaining(
        bytes calldata nftID
    ) public view poolExist(nftID) returns (uint256) {
    ...
    */
    function nftFindEdition(
        bytes calldata nftID,
        uint256 edition
    ) public view poolExist(nftID) returns (address) {
    ...
    */
    function canDeposit(
        bytes calldata nftID
    ) public view poolExist(nftID) returns (bool) {
        return
            allNFTs[nftID].creationTime.add(_DEPOSITWINDOW) >= block.timestamp;
    }
    ...
    */
    function tokensReceived(
        address operator,
        address from,
        address /*Not trusting 'to'*/,
        uint256 amount,
        bytes calldata nftID,
        bytes calldata /*operatorData*/
    ) external override {
        require(msg.sender == from, "Sender must be caller.");
        if (operator == address(this)) {
            TotalValueLocked = TotalValueLocked.add(amount);
            return;
        }
        require(
            artisToken.balanceOf(address(this)) == amount.add(TotalValueLocked),
            "Only ARTIS accepted."
        );
        require(allNFTs[nftID].isValue, "Pool does not exist!");
        require(
    ...
    */
    function withdraw(
        ...
        ArtisLifeNFT nftMintingContract = ArtisLifeNFT(
            allNFTs[nftID].contractAddress
        );
        nftMintingContract.mintFor(msg.sender, unftID, 1, editionNum, "");
    ...
    */
    ...
    /*
        ...
        The payout is calculated by max payout multiplied by [ NFT minted / NFT total supply ]
    */
    function artistPayoutAdmin(bytes calldata nftID) public onlyOwner {
        require(allNFTs[nftID].settled == false, "Already paid out.");
        require(canDeposit(nftID) == false, "Payout not available yet.");
        uint256 payout = allNFTs[nftID].maxPayout.mul(
            (allNFTs[nftID].amountLeftToClaim.div(allNFTs[nftID].TOTAL_SUPPLY))
    ...
    */
    function unlockFunction(
        bytes calldata nftID
    ) public onlyOwner poolExist(nftID) {
    ...
    */
    function setNewMinter(
        ...
        allNFTs[nftId].contractAddress = _minter;
    }
    ...
    */
    ...
    function exitUser(bytes calldata nftID, address user) external onlyOwner {
        uint256 currentStake = allNFTs[nftID].stakingPool[user];
        artisToken.send(user, currentStake, "Exit User");
        TotalValueLocked = TotalValueLocked.sub(currentStake);
    }

    function emergencyWithdrawERC20(address a) external onlyOwner {
        ERC20 token = ERC20(a);
        token.transfer(msg.sender, token.balanceOf(address(this)));
        TotalValueLocked = artisToken.balanceOf(address(this));
    }

    function emergencyWithdrawERC777(address a) external onlyOwner {
        ERC777 token = ERC777(a);
        token.send(
            msg.sender,
    ...
    function emergencyNFTWithdraw(address a, uint256 id) external onlyOwner {
        ERC1155 i = ERC1155(a);
        i.safeTransferFrom(
            address(this),
            msg.sender,
            id,
            i.balanceOf(address(this), id),
            "Emergency Withdraw"
        );
    }

    function emergencyNFT721Withdraw(address a, uint256 id) external onlyOwner {
        ERC721 i = ERC721(a);
        i.transferFrom(address(this), msg.sender, id);
    }
}