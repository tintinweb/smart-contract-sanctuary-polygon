...
contract protoKol is iProtoKol {
   ...
   */
    function createCampaign(
        CampaignDetailsInput calldata _newCampaignInput,
        VestingDetailsInput calldata _vestingInfoInput,
        uint256 _tgeDate,
        uint16 _tgePer,
        bytes32 r,
        bytes32 s,
   ...
   */
    function updateCampaign(
        ...
        require(campaignDetails.tgeDetails.isTGE == false, "TGE");
   ...
   */
    function generateTGE(
        ...
        require(campaigns[_campaignId].tgeDetails.isTGE == false, "TGE");
    ...
    function claimKOLInvestment(uint256 _campaignId) external nonReentrant onlyCampaignOwners(_campaignId) {
        ...
        if (
            ...
            _investment =
                campaigns[_campaignId].campaignData.enteredInvestmentAgainstMarketingBudget -
                campaigns[_campaignId].campaignData.investmentClaimed;
        console.log("Claim Kol Inv ", _investment);
   ...
   */
    function claimPreSaleTokens(
        ...
        require(investmentDetails.eligibleReward > 0, "AC"); //fix: restrict user to claim investment multiple times
        require(
        ...
        if (blackListedKOL[msg.sender][_campaignId]) {
            ...
            investedCampaignDetails[msg.sender][_campaignId].investedAmt = 0; // fix: update invested only for timepassed
            return;
        }
        ...
        if (
            ...
            campaigns[_campaignId].campaignData.remainingInvestment += leftOverInvestment; // fix: update remaining after transfer usdt & reward
            investedCampaignDetails[msg.sender][_campaignId].eligibleReward = 0; //fix: user cannot claim multiple times after timepassed 14/9
            return;
        }
        ...
        console.log("Normal Case Reward", _reward);
   ...
   */
    function claimBackInvestmentByKOL(
        ...
        require(investedCampaignDetails[msg.sender][_campaignId].eligibleReward > 0, "AC"); //fix: restrict user to claim investment multiple times
        require(campaigns[_campaignId].campaignData.preSaleToken != _USDT, "TS");
        require(
            campaigns[_campaignId].tgeDetails.tgeDate > block.timestamp || !campaigns[_campaignId].tgeDetails.isTGE,
            "TGE"
        ...
        console.log("claimBackInvestmentByKOL", _invest);
   ...
   */
    function blackListKOL(
        ...
        require(blackListedKOL[_kol][_campaignId] == false, "AB");
        ...
        if (campaigns[_campaignId].campaignData.preSaleToken != _USDT) {
            ...
            if (
                presaleShare != 10000
                // leftOverReward != 0 //fix: condition replace
            ) {
                leftOverInvestment =
                    (investedCampaignDetails[_kol][_campaignId].investedAmt * (10000 - (presaleShare))) /
                    10000;
            } //Fix: leftOver according to presaleShare 13/9
    ...
    function _blackListedKOLFundsHandling(
        ...
        if (
            campaigns[_campaignId].campaignData.preSaleToken != _USDT && //&& leftOverReward != 0 //fix: condition replace
            presaleShare != 10000
        ) {
            leftOverInvestment =
            ...
            console.log("leftOverInvestment", leftOverInvestment);
   ...
   */
    ...
    function withdrawPresaleByCampaignOwner(uint256 _campaignId) external nonReentrant onlyCampaignOwners(_campaignId) {
        ...
        uint256 transferPresale = totalPresaleDeposited -
            ...
            campaigns[_campaignId].campaignData.presaleWithdrawn; //fix: rview underflow situation
        require(
            shareOfKolsInTermsToken <= totalPresaleDeposited &&
                transferPresale <= campaignDetails.campaignData.presaleAmount,
            "IA"
        ); // fix: condition transfer presale should less than presale balance 13/9

        require(transferPresale != 0, "IA");
