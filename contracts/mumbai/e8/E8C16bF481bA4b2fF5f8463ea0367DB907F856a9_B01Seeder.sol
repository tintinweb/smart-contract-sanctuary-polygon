...
/// @title The B01Token pseudo-random seed generator
...
contract B01Seeder is IB01Seeder {
    /**
     * @notice Generate a pseudo-random B01 seed using the previous blockhash and b01 ID.
     */
    function generateSeed(uint256 b01Id, IB01Descriptor descriptor) external view override returns (Seed memory) {
        uint256 pr = uint256(keccak256(abi.encodePacked(blockhash(block.number - 1), b01Id)));

        IB01Descriptor.AttributeRanges memory ranges = descriptor.getAttributeRanges();
        IB01Seeder.Seed memory seed;
        ...
        seed.orientation = randomUint256InRange(pr >> (16 * 9), ranges.orientationRange[0], ranges.orientationRange[1]);
