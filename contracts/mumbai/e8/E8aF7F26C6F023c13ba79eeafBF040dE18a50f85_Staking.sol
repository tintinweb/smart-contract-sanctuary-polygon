...
contract Staking is Ownable, IERC721Receiver, ReentrancyGuard {
    ...
    uint256 public MAX_STAKE_PER_TXN = 10;
    uint256 public MAX_RELEASE_PER_TXN = 50;
    ...
    event MaxReleasePerTransactionChanged(uint256 maxRelease);
    event MaxStakePerTransactionChanged(uint256 maxStake);
    ...
    function unstake(StakeArgs[] calldata args) external nonReentrant {
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            releaseQueue[args[i].contractAddress].push(qItem);
    ...
    function releaseToOwner(address contractAddress) external nonReentrant {
        for (uint256 i = 0; i < MAX_RELEASE_PER_TXN; i++) {
            if (!isReadyForRelease(contractAddress)) {
                break;
            }
            ...
            nftContracts[contractAddress].safeTransferFrom(
                address(this),
                qItem.tokenOwner,
                qItem.tokenId,
                "0x00"
    ...
    function setMaxStakePerTransaction(
    ...
    function isReadyForRelease(
        ...
        return releaseQueue[contractAddress].isReadyForRelease();
    }
