...
contract CrazyBearsStaking is ERC1155Holder, IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    event Staked(address indexed user, uint256 tokenId, uint timestamp);
    event StakedAll(address indexed user, uint256[] tokenIds, uint timestamp);
    event Unstaked(address indexed user, uint256 tokenId, uint timestamp);
    event UnstakedAll(address indexed user, uint256[] tokenIds, uint timestamp);

    using SafeMath for uint256;
    address public rewardContractAddress;
    IERC1155 rewardContract = IERC1155(rewardContractAddress);
    ...
    mapping(address => uint[]) tokenByUser;

    // constructor(address _stakableContractAddress, address _rewardContractAddress) {
    constructor() {
        // rewardContractAddress = _rewardContractAddress;
        rewardContractAddress = 0x4c281b02bdAfe722f95E184257C5706ED394FD75;
        rewardContract = IERC1155(rewardContractAddress);
    ...
    function setReward(address _contractAddress, uint _rewardPerDay) public onlyOwner {
        rewardPerDay = _rewardPerDay;
        rewardContractAddress = _contractAddress;
        rewardContract = IERC1155(rewardContractAddress);
    }
    ...
    function _stake(address _from, uint256 _tokenId) internal returns (stakedToken memory) {
        ...
        tokenByUser[_from].push(_tokenId);
        return staking;
    }
    ...
    function stake(uint256 _tokenId) public {
        ...
        stakedToken memory staked = _stake(msg.sender, _tokenId);
        emit Staked(msg.sender, _tokenId, staked.since);
    }
    ...
    function stakeAll() public {
        ...
        for(uint i=0; i < _tNum; i++){
            staked = _stake(msg.sender, _tokens[i]);
        }
        emit StakedAll(msg.sender, _tokens, staked.since);
    }
    ...
    function getTokensOfOwner(address _owner) public view returns ( uint [] memory){
        ...
        ownersTokens = tokenByUser[_owner];      
        return ownersTokens;
    }
    ...
    function checkReward(uint256 _tokenID) public view returns (uint){
        ...
        uint passedDays = passedSec.div(30);
        uint tokenEarned = passedDays.mul(rewardPerDay);
        return tokenEarned;
        // return 10;
    }
    ...
    function receiveReward(uint256 _tokenID) nonReentrant public {
        ...
        if (rewardContract.balanceOf(address(this), rewardId) <= earned) {
            rewardInterface(rewardContractAddress).mint(address(this), rewardId, (earned + 100));
        }
    ...
    function unstake(uint256 _tokenID) public {
        ...
        uint256[] storage stakedNFTs = tokenByUser[user];
        uint index;
    ...
    function receiveRewardAll() public {
        for (uint i = 0; i < tokenByUser[msg.sender].length; i ++){
            uint tokenID = tokenByUser[msg.sender][i];
    ...
    function unstakeAll() public {
        receiveRewardAll();
        sendBackAllTokens(msg.sender);
    }

    function mintReward(uint256 rewardId, uint256 amount) public onlyOwner {      
        rewardInterface(rewardContractAddress).mint(address(this), rewardId, amount);
    }

    function emergencyReturn(address _owner) public onlyOwner {      
        sendBackAllTokens(_owner);
    }

    function sendBackAllTokens(address _owner) internal {
        uint256[] storage stakedNFTs = tokenByUser[_owner];
