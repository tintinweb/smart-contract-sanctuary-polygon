...
contract Pizza is Initializable,  UUPSUpgradeable, OwnableUpgradeable{
    ...
    ///@dev no constructor in upgradeable contract insted we have initialize
    ///@param _sliceCount initial number of slice
    function initialize(uint256 _sliceCount) public initializer{
        slices = _sliceCount;
        // @dev as there is no constructor , we need to initialize the OwnableUpgradeable explicitly
        __Ownable_init();
    }
    ...
    function add_slice(uint256 addslice)external{
        slices += addslice;
    }
}
