...
contract MusixContract is ERC721Enumerable,Ownable {
    ...
    uint256 public NFT_PRICE = 0.0369 ether;
    ...
    function mintNft(uint _quantity) external payable{
        ...
        for (uint256 i = 0; i < _quantity; i++) {
            ...
            tokenIdToRarity[_tokenIds.current()] = randomize();
            _tokenIds.increment();
        }
        
    }

    function randomize() private view returns (string memory) {
        uint random = (uint(keccak256(abi.encodePacked(block.difficulty, block.timestamp))) % 10000) + 1;
        if(random > 0 && random <= 5){
            return "gold";
        }else if(random > 5 && random <= 100){
            return "silver";
        }else if(random > 100 && random <= 500){
            return "copper";
        }else if(random > 500 && random <= 2000){
            return "green";
        }else{
            return "blue";
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return string(abi.encodePacked(_baseURI(), tokenId.toString(), tokenIdToRarity[tokenId], ".json"));
    }
