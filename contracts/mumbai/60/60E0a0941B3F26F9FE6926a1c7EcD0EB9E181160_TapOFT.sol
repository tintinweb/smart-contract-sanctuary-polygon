...
contract TapOFT is PausableOFT {
    ...
    function getVotingPower(uint256 _amount, uint256 _time) external payable {
        ...
        bytes memory payload = abi.encode(abi.encodePacked(msg.sender), _amount, true, _time);
    ...
    function _nonblockingLzReceive(
        ...
        (bytes memory toAddressBytes, uint256 amount, bool lock, uint256 time) = abi.decode(_payload, (bytes, uint256, bool, uint256));
        address toAddress;
        assembly {
            toAddress := mload(add(toAddressBytes, 20))
        }

        //credit on current chain
        if (lock && time > 0) {
            //credit here so we can create_lock_for; TODO: approve veTap & lock
