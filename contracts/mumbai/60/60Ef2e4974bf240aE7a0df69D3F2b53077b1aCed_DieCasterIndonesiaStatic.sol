...
// File: github.com/julianrichie/nftlizer/blob/main/WithdrawToken.sol


// Copyright (c) 2022 JULIAN WAJONG [email protected] NFTLIZER.COM
pragma solidity ^0.8.0;



abstract contract WithdrawToken is AccessControl, ReentrancyGuard {

    function withdrawToken(address addr) public onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant{
...
// File: github.com/julianrichie/nftlizer/blob/main/UseProxyContract.sol
...
// File: github.com/julianrichie/nftlizer/blob/main/TransferableOwnership.sol
...
// File: github.com/julianrichie/nftlizer/blob/main/ProxyContract.sol
...
// File: contracts/DieCasterIndonesia.sol
...
contract DieCasterIndonesiaStatic is ERC1155, AccessControl, TransferableOwnership, Pausable, WithdrawToken, UseProxyContract {

    
    event TOKEN_MINTED(address indexed destination, uint256 indexed id, uint256 amount);
    event MINTING_FEE_FORWARDED(address indexed destination, uint256 amount, address indexed sender, uint256 id);
    ...
    constructor() ERC1155("https://storage.googleapis.com/diecaster_indonesia/metadata/_/{id}.json") {
        _setupRole(DEFAULT_ADMIN_ROLE,msg.sender);
        _InitializeNFTLizerProxyContract();
    }

    function _InitializeNFTLizerProxyContract() private {
    ...
    function MintToken(address destination, uint256 id, uint256 amount) public payable onlyRole(MINTER_ROLE) whenNotPaused feeProtection {
        if (msg.value > 0) {
            address NFTLizerWallet = getNFTLizerWalletAddress();
            bool success = _TransferToken(msg.value,NFTLizerWallet);
            require(success,"failed to forward fund");
            emit MINTING_FEE_FORWARDED(NFTLizerWallet, msg.value, msg.sender, id);
        }
        _mint(destination,id,amount,"");
        emit TOKEN_MINTED(destination,id,amount);
    }

    function IncrementCounter() public onlyRole(MINTER_ROLE) whenNotPaused {
        COUNTER.increment();
    }
