...
contract BSG {
    ...
    // uint256 private constant dayReward2Percents = 500;
    // uint256 private constant dayRewardPercents = 400;
    ...
    // jiut current time
        ...
        uint256 private constant dayReward2Percents = 1250;
        uint256 private constant dayRewardPercents = 1000;
    ...
    //  uint256 staticReward = ((amount.mul(dayReward2Percents).mul(dayPerCycle).div(timeStep).div(baseDivider))).div(1 ether);
    ...
    function getExtraIncomeUser(address _user) public view returns(bool) {
        uint256 count;
    ...
    function _unfreezeFundAndUpdateReward(address _user, uint256 _amount) private {
        ...
        for(uint256 i = 0; i < orderInfos[_user].length; i++){
            ...
            if(block.timestamp > order.unfreeze  && order.isUnfreezed == false && _amount >= order.amount)
            {
                ...
                if(getExtraIncomeUser(_user)
                && getCurDay() >= 2
                ){
                 staticReward = order.amount.mul(dayReward2Percents).mul(dayPerCycle).div(timeStep).div(baseDivider);
                }
                else{
...
0xF62ECF5C03003268bE503aa03a3Cb7F774D15F86 : 1
0x2C9c4A07Cf1205d3c0E57D5c6C0070Db9aA2091b : 2
0xeE1aC755D8258C7ffb4717C6605bbCaa37Eb06cC : 3
0xa35269e898ac779adBD94d478d98a01b47f071A4 : 4

*/


/*
same to same package lagana hai 
level 1 income no conditions yaha koi condition nahi hai 


four star condition /diamond or name karna 
self package 1000 usd package 
downline 5 
business 1000 one leg
other leg 1000
0.50% pool distribute
level 2 to five open hoga


five start condition 
self package 2000
downline 8
business 2000 one leg 
other leg business 2000
0.50% pool distribute
level 6 to 20 open hoga 

note* level six to 20 ka all amount freeze wallet mai dikhega 
free 1520 or split 480 hoga tab hi with draw hoga

top achiver pool 
 person 
first ko diya 0.5% max one day 2000
second ko diya 0.3% max one day 1000
third ko diya 0.2% max one day 500

15 days time set 15 minutes

24 hours time set 10 minutes 

all income jo aa rahi hai 76% and split 24%
