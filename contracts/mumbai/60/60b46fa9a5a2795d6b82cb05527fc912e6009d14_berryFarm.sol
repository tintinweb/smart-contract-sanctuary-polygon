...
contract berryFarm is Ownable {   
 ...
 function reinvest() external  {
    require(investors[msg.sender].roiAmountTill<block.timestamp,"OneDay");
	  uint amount = profit();
    require(amount >= 1 );
  ...
  function withdrawable(address user) public view returns (uint amount) {
   ...
   function singleDeposit(address user,uint index) public view returns (uint,uint,uint,uint,uint,uint) {
    Investor storage investor = investors[user];
    uint tillRoiTime = investors[user].roiAmountTill;
      ...
      return (tariff.time,tillRoiTime,appendTime,finish,since,till);
