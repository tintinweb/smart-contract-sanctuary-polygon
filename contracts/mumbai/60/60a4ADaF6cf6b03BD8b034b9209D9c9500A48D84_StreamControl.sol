...
contract StreamControl is IStreamControl {
    ...
    function flowCreateAndScheduleDelete(
        ...
        if (!LibStream._storageStreamControl().superTokens[_superToken])
            revert InvalidSuperToken();
        if (_flowRateReceiver <= 0 || _flowRateApp <= 0)
            revert InvalidFlowRate(); // TODO: revert if _flowRateApp is less than %take of total flow, (need add % variable)
        if (_flowLifespan <= 0) revert InvalidFlowLifespan();

        LibStream._storageStreamControl()._cfaLib.createFlowByOperator(
            msg.sender,
            _receiver,
            _superToken,
            _flowRateReceiver
        ...
        // LibStream._storageStreamControl().taskIds[msg.sender].push(
        //     taskId
    ...
    function deleteFlow(
        ...
        LibStream._storageStreamControl()._cfaLib.deleteFlowByOperator(
            _sender,
            _receiver,
            _superToken
        );
