...
contract NFTCore is ERC1155, Pausable {
    ...
    function exchange(uint256 series, uint256 pic) external {
        ...
        address capsule_address = capsuleContracts[series];
        INFTCapsule capsule = INFTCapsule(capsule_address);
        //(uint256 tokenId, bool verified) = checkFragment(msg.sender, series, pic,capsule.fragmentAmount());
    ...
    function _removeOwnedTokens(
        ...
        if (ownedTokens[from][NFTInstanceIndex].count - amount == 0) {
            //delte tokenIdToBool
    ...
    // ) internal view returns (uint256 _tokenId, bool _verified) {

    //     //require(_seriesId == seriesId, 'Wrong Series');
    //     //require(_picId <= picAmount, 'Exceed pic amount');

    //     _tokenId = _seriesId * 1e12 + _picId * 1e6;
    //     for (uint256 tokenId = _tokenId + 1; tokenId <= _tokenId + _fragmentAmount; tokenId++) {
    //         //require(fragmentMintAmount[tokenId] > 0, 'Wrong picture ID');
    //         require(balanceOf(_address, tokenId) >0, "Don't have enough fragments");
    //     }
