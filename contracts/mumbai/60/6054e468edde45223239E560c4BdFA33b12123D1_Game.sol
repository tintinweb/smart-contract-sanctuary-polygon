...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    uint256 public override countTypeMissile;
    ...
    function arrangeMissile(uint256[12] calldata positions, uint256 nftId, address management) external payable override nonReentrant whenNotPaused {
        nonZero(positions);
        (uint256 price, uint256[] memory events, UseCache[] memory useCache, AmountCache[] memory amount, uint256 amountToMinimum) = calPrice(positions, msg.sender, nftId, management);
        ...
        for (uint256 i = 1; i < countTypeMissile + 1; i++) {
            if (amount[i].amountAff > 0) {
                freeAffiliate[msg.sender][i] -= amount[i].amountAff;
            }
            if (amount[i].amountNFT > 0) {
                INFTManagement(management).updateFreeMissile(nftId, i, amount[i].amountNFT);
            }
        }
        if (msg.value > price) {
    ...
    function initUseCache(uint256[] memory events, address account) internal view returns (UseCache[] memory) {
        UseCache[] memory useCache = new UseCache[](events.length);
        for (uint256 i = 0; i < events.length; i++) {
            FreeEvent memory f = freeEventInfos[events[i]];
            useCache[i].amountUserUsed = freeMissileUsed[account][uint256(f.freeEventId)];
            useCache[i].amountUsed = uint256(f.amountFreeUsed);
        }
        return useCache;
    }

    function calPrice(
        uint256[12] calldata positions, 
        address account,
        uint256 nftId,
        address management
    ) public override view returns (uint256 price, uint256[] memory, UseCache[] memory, AmountCache[] memory, uint256) {
        (uint256[] memory events,,) = getEventPerAccount(account);
        UseCache[] memory useCache = initUseCache(events, account);
        AmountCache[] memory amount = new AmountCache[](countTypeMissile + 1);
        uint256 value;
        for (uint256 i = 0; i < 12; i++) {
            ...
            for (uint256 j = 0; j < events.length; j++) {
                FreeEvent memory f = freeEventInfos[events[j]];
                if (p == f.typeMissileFree && 
                    useCache[j].amountUserUsed < f.amountFree && 
                    useCache[j].amountUsed < f.totalFree
                ) {
            ...
            if (!free && missileValues[p] != 0) {
                if (freeAffiliate[a][p] > amount[p].amountAff) {
                    amount[p].amountAff += 1;
                } else if (checkManagement(management, a, id, amount[p].amountNFT, p)) {
                    amount[p].amountNFT += 1;
                } else {
                    value += missileValues[p];
                }
            }
        }
        price = value * cost;
        if (price >= minimumAmount) {
            return (price, events, useCache, amount, 0);
        }    
        return (price, events, useCache, amount, minimumAmount - price);
    }
    ...
    function checkManagement(
        ...
        uint256 amount, 
        uint256 typeMissile
    ) internal view returns (bool condition) {
        uint256 typeNft = INFTManagement(management).freeTypes(nftId);
        condition = nftId != 0 &&
            INFTManagement(management).getOwnerOf(nftId) == account && 
            INFTManagement(management).freeInfos(typeNft, typeMissile) > amount + INFTManagement(management).freeUsed(nftId, typeMissile);
    }
...
interface INFTManagement {
    ...
    event SetTypeForNNT(uint256[] nftIds, uint256[] types);
