...
contract NINEMATIC is ERC20 {
    ...
    uint256 constant internal tokenPriceInitial_            = 167 finney;
    ...
    */
    ...
    function withdraw() onlyhodler() public {
        ...
        require((totaldeposit.mul(10)) <= (payoutsTo_[_customerAddress].add(withdrawamt)));
    ...
    function poolprofit(address _customerAddress) public view returns(uint256){
        ...
        for	(uint256 y=1; y<= referralCount_[_customerAddress]; y++)
		{
            ...
            if(levels[_customerAddress][0] == 6 ){
                ...
                if(directbusiness[_customerAddress] >= 10000*10**18){
                        if(level <= 20){
                        for (uint256 i = 0; i < deposits[downline].length; i++) {
                            uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                            if (stakingTime_[downline] < finish) {
                ...
                if(directbusiness[_customerAddress] >= 50000*10**18){
                     if(level <= 27){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
                ...
                if(directbusiness[_customerAddress] >= 65000*10**18){
                     if(level <= 28){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
                 ...
                 if(directbusiness[_customerAddress] >= 80000*10**18){
                     if(level <= 29){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
                ...
                if(directbusiness[_customerAddress] >= 100000*10**18){
                     if(level <= 30){
                    for (uint256 i = 0; i < deposits[downline].length; i++) {
                        uint256 finish = deposits[downline][i].start.add(deposits[downline][i].time).mul(1 days);
                        if (stakingTime_[downline] < finish) {
    ...
    function distributeRefBonus(uint256 _incomingmatic, address _referredBy, address _sender, bool _newReferral) internal {
        ...
        _referralBonus 		            = _incomingmatic;
        
        bonusLv1                        = SafeMath.percent(_referralBonus,50,100,18);
        bonusLv2                        = SafeMath.percent(_referralBonus,30,100,18);
        bonusLv3                        = SafeMath.percent(_referralBonus,20,100,18);
    ...
    function purchaseTokens(address _referredBy) public payable returns(uint256) {
        ...
        if(incMATIC >= 50*10**18 && incMATIC <= 500*10**18) {
            time = 400;
            percent = 50;
        }
        ...
        if(dayscount <= 10)
        {
            if(levels[_referredBy][0] == 2 && directbusiness[_referredBy] >= 1000*10**18){
            ...
            if(levels[_referredBy][0] == 4 && directbusiness[_referredBy] >= 20000*10**18){
            ...
            if(levels[_referredBy][0] == 6 && directbusiness[_referredBy] >= 30000*10**18){
            ...
            if(levels[_referredBy][0] == 7 && directbusiness[_referredBy] >= 40000*10**18){
