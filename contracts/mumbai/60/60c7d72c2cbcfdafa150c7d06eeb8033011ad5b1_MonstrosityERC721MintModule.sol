...
import "../interfaces/IERC721TransferEvent.sol";
import "../lib/LibMonstrosity.sol";
import "../lib/LibMonstrosityGameData.sol";
import "../lib/LibMonstrosityERC721.sol";
import "./MonstrosityGameModule.sol";
import "./MonstrosityWalletModule.sol";

contract MonstrosityERC721MintModule is MonstrosityBase, IERC721TransferEvent {
  using LibMonstrosityGameData for GameData;
  using LibMonstrosityERC721 for ERC721Data;

  function setMerkleRoot(bytes32 root, uint256 listId) external onlyOwner {
    _nftData.merkleRoot[listId] = root;
  }
  ...
  function startMint() external onlyOwner {
    ...
    _nftData.baseUri = "https://be.monstrosity.io/monsters/";

    // mint the princess nft and lock in the contract
    address _contract = address(this);

    _nftData.balances[_contract] += 1;
    _nftData.owners[1] = _contract;
    _nftData.supply += 1;
  }
  ...
  function _mint(
    address to,
    bytes32 monsterName,
    uint64[4] memory traits
  ) internal {
    uint256 tokenId = _nftData.supply + 1;
    require(to != address(0) && !_nftData._exists(tokenId));
    // validate trait points
    uint128 totalTraitPoints;
    uint128 i;

    while (i < 4) {
      totalTraitPoints += traits[i];
      i += 1;
    }

    require(totalTraitPoints == 40);

    _nftData.balances[to] += 1;
    _nftData.owners[tokenId] = to;

    emit Transfer(address(0), to, tokenId, monsterName);
    require(LibMonstrosityERC721._checkOnERC721Received(address(0), to, tokenId, ""));

    _nftData.supply += 1;

    // monsters minted after the final campaign has started won't be participating in this game
    if (_gameData._stage() > GameStage.Recovery) {
      return;
    }

    (bool success, ) = _ds[MonstrosityGameModule.recruitMonster.selector].delegatecall(
      abi.encodeWithSelector(MonstrosityGameModule.recruitMonster.selector, tokenId, traits)
    );
    require(success, "ERR_RECRUIT");
  }

  /// @dev Public mint method
  /// @param monsterName Monster name
  /// @param traits Trait points
  function mint(bytes32 monsterName, uint64[4] calldata traits) external payable nonReentrant {
    ...
    uint256 price = stage > GameStage.Recruitment && stage < GameStage.FinalCampaign ? MINT_PRICE * 2 : MINT_PRICE;

    require(_gameData.startTime > 0 && _nftData.supply + 1 <= 10000 && msg.value == price);

    _mint(msg.sender, monsterName, traits);
    _gameData._updatePools(price, true, MINT_PERCENT_PRIZE);
  }
  ...
  /// @param listId Whitelist ID
  function whitelistMint(
    bytes32 monsterName,
    uint64[4] calldata traits,
    bytes32[] calldata proof,
    uint256 listId
  ) external payable nonReentrant {
    require(
      _gameData.startTime > 0 &&
        _nftData.supply + 1 <= 10000 &&
        _gameData._stage() < GameStage.InitialCampaign &&
        _nftData.whitelistMints[listId][msg.sender] < 2 &&
        (listId == 2 || msg.value == (MINT_PRICE * MINT_PRICE_DISCOUNT) / 100) &&
        LibMonstrosity._verifyProof(msg.sender, proof, _nftData.merkleRoot[listId])
    );

    _mint(msg.sender, monsterName, traits);
    _nftData.whitelistMints[listId][msg.sender] += 1;

    if (listId == 1) {
      _gameData._updatePools(msg.value, true, MINT_PERCENT_PRIZE);
    }
  }
}
