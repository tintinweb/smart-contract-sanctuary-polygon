...
contract AllelePlantCore is PlantBreeding {
        ...
        )
    {
        ...
        isReady = (plant.cooldownTimestamp <= block.timestamp);
        nextActionAt = plant.cooldownTimestamp;
        siringWithId = plant.siringWith;
    ...
    function withdrawAgt(uint _amount) external onlyOwner {
        ...
        require(_amount < amount && _amount < amount - subtractFees);
...
contract PlantBreeding is UIAMToken {
    ...
    event Pregnant(uint64 parentId, uint64 sireId, uint64 cooldownTimestamp);

    uint256 public autoBirthFee = 20000000000000;

    uint64 public seeds;
    uint64 public cooldownTime = 5 minutes;
    ...
    function _isReadyToBreed(Plant memory _plant) internal view returns (bool) {
        return
            (_plant.siringWith == 0) &&
            (_plant.cooldownTimestamp <= uint64(block.timestamp));
    }
        ...
        returns (bool)
    {
        return
            ...
            (_matron.cooldownTimestamp <= uint64(block.timestamp));
    }
    ...
    function _triggerCooldown(Plant storage _plant) internal {
        _plant.cooldownTimestamp = uint64(
            block.timestamp + cooldownTime
        );
    }

    function isPregnant(uint64 _plantId) external view returns (bool) {
        require(_plantId > 0 && _exists(_plantId), "PlantBreeding: Plant not exists");
    ...
    function _breedWith(uint64 _matronId, uint64 _sireId) internal {
        ...
        emit Pregnant(_matronId, _sireId, matron.cooldownTimestamp);
    }
    ...
    function _breedWithAuto(
        ...
        seedsMetadata[_matronId] =  _tokenURI;
    ...
    function giveBirth(uint64 _matronId)
        external
        whenNotPaused
        returns (uint64)
    {
        ...
        require(_isReadyToGiveBirth(matron), "PlantBreeding: Not ready to give Birth");
        ...
        uint64 plantId = _createPlant(
            ...
            matron.siringWith,
            address(this),
            seedsMetadata[_matronId]
        );

        // add matrons owners and sire owners as new plants owner
        
        _setOwner(_matronId, matron.siringWith, plantId);

        // Clear the reference to sire from the matron (REQUIRED! Having siringWith
    ...
    AllelePlantEIP712
{
    ...
    mapping(uint64 => string) internal seedsMetadata;
    ...
    function transferFrom(
        ...
        if (ownerOf(_tokenId) == address(this)) {
            // check spender is owner or 
            address spender = msg.sender;
            
            // Check _owns
            require(_owns(spender, tokenID) || spender == actionContract, "You are not owner nor approved");
            // replace ownership from owners array
            if(spender == actionContract){
                _replaceOwner(_from, _to, tokenID);
            }else{
                _replaceOwner(spender, _to, tokenID);
            }
        } else {
        ...
        addBreedingPermission(tokenID, _from);
...
contract AllelePlantOwnership is Ownable {
    ...
    mapping(uint64 => mapping (address => uint64)) private ownersToPlantIndex;
    mapping(uint64 => address[]) private ownersToPlantArray;
    mapping(uint64 => mapping (address => bool)) private breedingPermission;
    ...
    function addOwner(uint64 _tokenId, address _owner) internal {
        if(ownersToPlantIndex[_tokenId][_owner] == 0){
            ownersToPlantIndex[_tokenId][_owner] = uint64(ownersToPlantArray[_tokenId].length + 1);
            ownersToPlantArray[_tokenId].push(_owner);
        }
    }

    // Check if an address is owner of a token by tokenId
    function _owns(address _user, uint64 _tokenId)
        internal
        view
        returns (bool)
    {
        return ownersToPlantIndex[_tokenId][_user] > 0;
    }

    // Check if an address is owner of a token by tokenId
    function owns(address _user, uint64 _tokenId) public view returns (bool) {
    ...
    function _replaceOwner(
        ...
        uint64 index = ownersToPlantIndex[_tokenId][_from];
        ownersToPlantIndex[_tokenId][_from] = 0;
        ownersToPlantIndex[_tokenId][_to] = index;
        ownersToPlantArray[_tokenId][index - 1] = _to;
    }

    // Merge two tokens ownership addresses to a new plant (unique addresses)
    // @warning : It cost huge gass. If plant ownership is very deep , It can fail.
    function _setOwner(uint64 _tokenA, uint64 _tokenB, uint64 _newTokenID) internal {
        
        address[] memory tokenAOwners = ownersToPlantArray[_tokenA];
        address[] memory tokenBOwners = ownersToPlantArray[_tokenB];
        
        for(uint64 i = 0; i < tokenAOwners.length; i++){
             ownersToPlantIndex[_newTokenID][tokenAOwners[i]] = i+1 ;
        }
        ownersToPlantArray[_newTokenID] = tokenAOwners;
        for(uint64 i=0; i< tokenBOwners.length; i++) {
            address _ownerB = tokenBOwners[i];
            if(ownersToPlantIndex[_newTokenID][_ownerB] == 0){
                addOwner(_newTokenID, _ownerB);
            }
        }
    }
    ...
    function addBreedingPermission(uint64 _tokenId, address _owner) internal {
        breedingPermission[_tokenId][_owner] = true; 
    ...
    function _hasBreedingPermissions(address _user, uint64 _tokenId)
        ...
        returns (bool)
    {
        return breedingPermission[_tokenId][_user];
    }
    ...
    function _replaceBreedingPermission(
        ...
        require(_from != _to, "Can not transfer to the same user");
        breedingPermission[_tokenId][_from] = false;
        breedingPermission[_tokenId][_to] = true;
    }

    function getOwners(uint64 _tokenId) external view returns(address[] memory){
        return ownersToPlantArray[_tokenId];
    }
}
