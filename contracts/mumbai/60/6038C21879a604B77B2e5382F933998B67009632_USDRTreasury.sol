...
contract USDRTreasury is AddressAccessor, ITreasury, IERC721Receiver {
    ...
    uint16 public purchaseTreshold; //100% = 10000 2 decimals
    ...
    function setPurchaseThreshold(uint16 _purchaseThreshold)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        purchaseTreshold = _purchaseThreshold;
    }
...
bytes32 constant TRACKER_ROLE = bytes32(keccak256("TRACKER_ROLE"));
bytes32 constant ROUTER_POLICY_ROLE = bytes32(keccak256("ROUTER_POLICY_ROLE"));
bytes32 constant TREASURY_POLICY_ROLE = bytes32(
    keccak256("TREASURY_POLICY_ROLE")
);
bytes32 constant TREASURY_WITHDRAW_ROLE = bytes32(
    keccak256("TREASURY_WITHDRAW_ROLE")
);
//for handling managers
bytes32 constant TREASURY_ROLE = bytes32(keccak256("TREASURY_ROLE"));
