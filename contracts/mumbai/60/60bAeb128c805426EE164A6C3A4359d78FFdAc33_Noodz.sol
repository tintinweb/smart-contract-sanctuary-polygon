...
// import "../interfaces/INoodzNames.sol";

contract Noodz is ERC721Enumerable, AccessControl {
    bytes32 public constant BREEDING_ROLE = bytes32("BREEDING_ROLE");

    // INoodzNames public noodzNames;
    uint public mintPrice;
    uint public maxToMint;
    uint public maxMintSupply;
    bool public saleIsActive;
    bool public whitelistSaleIsActive;
    mapping(address => uint) public whitelistNoodlesAmount;
    IERC20 public paymentsToken;
    string public baseURI;

    // event NoodleNamesChanged(INoodzNames noodzNames);
    event NoodleNamesChanged(uint tokenName);
    ...
    constructor(string memory name, string memory symbol, IERC20 _paymentsToken) ERC721(name, symbol) {
        paymentsToken = _paymentsToken;
        // noodzNames = _noodzNames;
        maxMintSupply = 100;
        mintPrice = 6e16;
        maxToMint = 4;
    ...
    function setWhitelistNoodlesAmount(address[] calldata users, uint[] calldata noodzAmount) external onlyOwner {
        require(users.length == noodzAmount.length, "incorrect arrays");

        for (uint i; i < users.length; i++) {
            whitelistNoodlesAmount[users[i]] = noodzAmount[i];
        }
    }
    ...
    // function setNoodleNames(INoodzNames _noodzNames) external onlyOwner {
    //     noodzNames = _noodzNames;
    //     emit NoodleNamesChanged(noodzNames);
    // }
    ...
    function reserveNoodles(address _to, uint _numberOfTokens) external onlyOwner {
        require(_to != address(0), "Invalid address to reserve");
        uint supply = totalSupply();

        for (uint i; i < _numberOfTokens; i++) {
            _safeMint(_to, supply + i);
            // noodzNames.setBasicNameOnMint(supply + i);
        }
    }

    function whitelistMintNoodles(uint numberOfTokens) external {
        require(whitelistSaleIsActive, "The sale must be active to mint");
        require(numberOfTokens <= whitelistNoodlesAmount[msg.sender], "You are not allowed to mint this amount");
        ...
        for (uint i; i < numberOfTokens; i++) {
            ...
            // noodzNames.setBasicNameOnMint(mintIndex);
        }
    }

    function mintNoodz(uint numberOfTokens) external {
