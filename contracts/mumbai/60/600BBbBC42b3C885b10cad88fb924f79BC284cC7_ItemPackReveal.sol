...
contract ItemPackReveal is IItemPackReveal, GameBase {
    ...
    function reveal(uint256 worldId, uint256 tokenId, uint256 itemPackId) external override {
        ...
        itemPack.updateItemPack(itemPackId, itemPackDefinitionId, true);
        _mintItems(worldId, tokenId, itemPackDefinitionId);

        emit RevealItemPack(worldId, tokenId, itemPackId);
    }
    ...
    function _mintItems(uint256 worldId, uint256 tokenId, uint256 itemPackDefinitionId) private {
        Character character = getCharacter(worldId, tokenId);
        World world = getWorld(worldId);
        ItemPackDefinition itemPackDefinition = world.itemPackDefinition();
        (, uint256[] memory itemDefinitionIds, int64[] memory amounts) = itemPackDefinition.getItemPackDefinition(itemPackDefinitionId);

        for (uint256 i; i < itemDefinitionIds.length; i++) {
            character.addItem(itemDefinitionIds[i], amounts[i]);
        }
    }
}
