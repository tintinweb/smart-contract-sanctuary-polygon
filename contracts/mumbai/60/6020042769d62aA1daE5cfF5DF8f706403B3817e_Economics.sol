...
contract Economics is IEconomics, AuthModifiers, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function _reserveFuelForRate(
        ...
        uint256 _min = uint256(_minFee) * 1e15;
        uint256 _max = uint256(_maxFee) * 1e15;
        ...
        for (uint256 i = 0; i < _ticketActions.length; i++) {
            uint256 _fuelUsd = uint256(_ticketActions[i].basePrice) * _rate * 1e9;
    ...
    function reserveFuelPrimarySale(
        ...
        IEventImplementation.TicketAction[] calldata _ticketActions
    ) external onlyEvent onlyConfigured(_relayerAddress) returns (uint256) {
        ...
        IntegratorData storage _integrator = integratorData[_integratorIndex];
        uint256 _fuel = _reserveFuelForRate(
            _integrator,
            _ticketActions,
            _integratorRates.minFeePrimary,
            _integratorRates.maxFeePrimary,
            _integratorRates.primaryRate
        );

        _integrator.activeTicketCount += uint32(_ticketActions.length);

        emit FuelReservedPrimary(_integratorIndex, uint32(_ticketActions.length), _fuel);
        return _fuel;
    }
    ...
    function reserveFuelSecondarySale(
        ...
        uint256 _fuel = _reserveFuelForRate(
            _integrator,
            _ticketActions,
            _integratorRates.minFeeSecondary,
            _integratorRates.maxFeeSecondary,
            _integratorRates.secondaryRate
        );

        emit FuelReservedSecondary(_integratorIndex, uint32(_ticketActions.length), _fuel);
        return _fuel;
    }
    ...
    */
    ...
    function collectSpentFuel() public nonReentrant {
    ...
    @param _newAvailableFuel the correct/intended balance of the silo in GET 
    @param _newReservedFuel the correct/intended balance of the silo in GET 
    ...
    function correctAccountBalance(
        ...
        if (_newAvailableFuel > _oldAvailableFuel) {
            ...
            spentFuel = spentFuel + (_oldAvailableFuel - _newAvailableFuel);
        }

        integrator.availableFuel = _newAvailableFuel;
        integrator.reservedFuel = _newReservedFuel;

        emit AccountBalanceCorrected(integrator.index, _oldAvailableFuel, _newAvailableFuel, _newReservedFuel);
    }
    ...
    */
     ...
     */
    function updateDynamicRates(uint32 _integratorIndex, DynamicRates calldata _dynamicRates)
        external
        onlyIntegratorAdmin
    {
    ...
    function setIntegratorAvailableFuel(uint32 _integratorIndex, uint256 _availableFuel) external onlyIntegratorAdmin {
        integratorData[_integratorIndex].availableFuel = _availableFuel;
        emit UpdateIntegratorAvailableFuel(_integratorIndex, _availableFuel);
    }

    function setIntegratorReservedFuel(uint32 _integratorIndex, uint256 _reservedFuel) external onlyIntegratorAdmin {
        integratorData[_integratorIndex].reservedFuel = _reservedFuel;
        emit UpdateIntegratorReservedFuel(_integratorIndex, _reservedFuel);
    }

    function setIntegratorPrice(uint32 _integratorIndex, uint256 _price) external onlyIntegratorAdmin {
    ...
    function viewIntegratorReservedFuel(uint32 _integratorIndex) external view returns (uint256) {
        return integratorData[_integratorIndex].reservedFuel;
    }
    ...
    function viewBillingStatus(uint32 _integratorIndex) external view returns (bool) {
        return integratorData[_integratorIndex].isBillingEnabled;
    }

    function viewConfigurationStatus(uint32 _integratorIndex) external view returns (bool) {
        return integratorData[_integratorIndex].isConfigured;
    }
...
interface IEconomics {
    ...
    event UpdateIntegratorAvailableFuel(uint32 integratorIndex, uint256 availableFuel);
    event UpdateIntegratorReservedFuel(uint32 integratorIndex, uint256 reservedFuel);
    ...
    function setIntegratorAvailableFuel(uint32 _integratorIndex, uint256 _availableFuel) external;

    function setIntegratorReservedFuel(uint32 _integratorIndex, uint256 _reservedFuel) external;
