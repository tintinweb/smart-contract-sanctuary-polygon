...
contract DTPBST is Context, AccessControlEnumerable, ERC20Burnable, ERC20Pausable, ReentrancyGuard, BlackList {
    ...
    constructor() ERC20("DividendTokenPammBotSuperTrend", "DTPBST") {
        ...
        _token = ERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
    }

    function getTokenDividend() external view returns(string memory) {
        return _token.symbol();
    }

    function setTokenDividend(address token_address) external onlyRole(ROLE_ADMIN) {
        _token = ERC20(token_address);
    }
    ...
    function depositDividends(uint256 _amount) external nonReentrant {
        require(totalSupply() != 0, "Token: No tokens minted");
        _token.approve(address(this), _amount);
        require(_token.allowance(_msgSender(), address(this)) >= _amount, "Token: amount token allowance");
