...
contract AuctionWithAdmin is Ownable {
	...
	function initialize(address _nftToken, address _token) public onlyOwner virtual returns(bool){
	...
	function bid(uint _tokenId, uint _amount) public {
        ...
        require(_amount >= auction[_tokenId].price, "Bid must be at least the reserve price");
	...
	function makeOffer(uint _tokenId, uint _amount) public {
	    ...
	    require(_amount > offer[_tokenId].price, "Offer amount less then already offerred");
		token.transferFrom(msg.sender, address(this), _amount);
	    pending_claim[msg.sender][_tokenId] += _amount;
	...
	function reciveOffer(uint _tokenId) public {
		require(msg.sender == token_seller[_tokenId], "You are not owner");
		 ...
		 token.transfer(token_seller[_tokenId], (offer[_tokenId].price * (100 - sell_token_fee) / 100));
	...
	function withdraw(uint _tokenId) public {
	    ...
	    if(msg.sender == token_seller[_tokenId]){
			token.transfer(beneficiary, (auction[_tokenId].highestBid * auction_token_fee / 100));
		 	token.transfer(msg.sender, (auction[_tokenId].highestBid * (100 - auction_token_fee) / 100));
	...
	function buy(uint _tokenId, uint _amount) public {
		 require(token_seller[_tokenId]!= address(0) && token_price[_tokenId] > 0, "Token not for sell");
	     require(_amount >= token_price[_tokenId], "Your amount is less");
	     nft_token.transferFrom(address(this), msg.sender, _tokenId);
		 token.transfer(beneficiary, (_amount * sell_token_fee / 100));
		 token.transfer(token_seller[_tokenId], (_amount * (100 - sell_token_fee) / 100));
		 delete token_seller[_tokenId];
	     delete token_price[_tokenId];
	     emit Buy(msg.sender, _tokenId, token_seller[_tokenId], _amount, block.timestamp);
    }
