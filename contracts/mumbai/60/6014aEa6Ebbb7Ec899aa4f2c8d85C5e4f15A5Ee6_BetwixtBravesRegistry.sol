...
contract BetwixtBravesRegistry is Ownable {
    ...
    function save_burn_data(uint16 _tokenId, string memory _burnData) external onlyAdmin {
    ...
    function reset_burn_data(uint16 _tokenId) external onlyAdmin {
        require(token_supply>=_tokenId && _tokenId>=1, "Invalid tokenId");
        burnMapping[_tokenId] = "";
        emit BurnDataChanged(_tokenId,"");
    }

    function get_burn_data_of_tokenId(uint16 _tokenId) public view returns(string memory){
        return burnMapping[_tokenId];
    }
    ...
    function set_admin(address _newAdminAddress, bool state) external onlyAdmin{
