...
contract DSMath {
    function sadd(uint x, uint y) internal pure returns (uint z) {
        require((z = x + y) >= x, "ds-math-sadd-overflow");
    }
    function ssub(uint x, uint y) internal pure returns (uint z) {
        require((z = x - y) <= x, "ds-math-ssub-underflow");
    }
    function smul(uint x, uint y) internal pure returns (uint z) {
        require(y == 0 || (z = x * y) / y == x, "ds-math-smul-overflow");
    }
    ...
    function wmul(uint x, uint y) internal pure returns (uint z) {
        z = sadd(smul(x, y), WAD / 2) / WAD;
    }
    function rmul(uint x, uint y) internal pure returns (uint z) {
        z = sadd(smul(x, y), RAY / 2) / RAY;
    }
    function wdiv(uint x, uint y) internal pure returns (uint z) {
        z = sadd(smul(x, WAD), y / 2) / y;
    }
    function rdiv(uint x, uint y) internal pure returns (uint z) {
        z = sadd(smul(x, RAY), y / 2) / y;
    }
...
// File: contracts/simple_market.sol
...
contract SimpleMarket is EventfulMarket, DSMath {
    ...
    {
        OfferInfo memory offer = offers[id];
        uint spend = smul(quantity, offer.buy_amt) / offer.pay_amt;
        ...
        offers[id].pay_amt = ssub(offer.pay_amt, quantity);
        offers[id].buy_amt = ssub(offer.buy_amt, spend);
...
// File: contracts/matching_market.sol
...
contract MatchingMarket is MatchingEvents, SimpleMarket {
        ...
        returns (uint fill_amt)
    {
        ...
        while (pay_amt > 0) {                           //while there is amount to sell
            ...
            if (pay_amt >= offers[offerId].buy_amt) {                       //If amount to sell is higher or equal than current offer amount to buy
                fill_amt = sadd(fill_amt, offers[offerId].pay_amt);          //Add amount bought to acumulator
                pay_amt = ssub(pay_amt, offers[offerId].buy_amt);            //Decrease amount to sell
                take(bytes32(offerId), uint128(offers[offerId].pay_amt));   //We take the whole offer
            } else { // if lower
                uint256 baux = rmul(pay_amt * 10 ** 9, rdiv(offers[offerId].pay_amt, offers[offerId].buy_amt)) / 10 ** 9;
                fill_amt = sadd(fill_amt, baux);         //Add amount bought to acumulator
        ...
        returns (uint fill_amt)
    {
        ...
        while (buy_amt > 0) {                           //Meanwhile there is amount to buy
            ...
            if (buy_amt >= offers[offerId].pay_amt) {                       //If amount to buy is higher or equal than current offer amount to sell
                fill_amt = sadd(fill_amt, offers[offerId].buy_amt);          //Add amount sold to acumulator
                buy_amt = ssub(buy_amt, offers[offerId].pay_amt);            //Decrease amount to buy
                take(bytes32(offerId), uint128(offers[offerId].pay_amt));   //We take the whole offer
            } else {                                                        //if lower
                fill_amt = sadd(fill_amt, rmul(buy_amt * 10 ** 9, rdiv(offers[offerId].buy_amt, offers[offerId].pay_amt)) / 10 ** 9); //Add amount sold to acumulator
    ...
    function getBuyAmount(ERC20 buy_gem, ERC20 pay_gem, uint pay_amt) public view returns (uint fill_amt) {
        ...
        while (pay_amt > offers[offerId].buy_amt) {
            fill_amt = sadd(fill_amt, offers[offerId].pay_amt);  //Add amount to buy accumulator
            pay_amt = ssub(pay_amt, offers[offerId].buy_amt);    //Decrease amount to pay
            if (pay_amt > 0) {                                  //If we still need more offers
                offerId = getWorseOffer(offerId);               //We look for the next best offer
                require(offerId != 0);                          //Fails if there are not enough offers to complete
            }
        }
        fill_amt = sadd(fill_amt, rmul(pay_amt * 10 ** 9, rdiv(offers[offerId].pay_amt, offers[offerId].buy_amt)) / 10 ** 9); //Add proportional amount of last offer to buy accumulator
    }

    function getPayAmount(ERC20 pay_gem, ERC20 buy_gem, uint buy_amt) public view returns (uint fill_amt) {
        uint256 offerId = getBestOffer(buy_gem, pay_gem);           //Get best offer for the token pair
        while (buy_amt > offers[offerId].pay_amt) {
            fill_amt = sadd(fill_amt, offers[offerId].buy_amt);  //Add amount to pay accumulator
            buy_amt = ssub(buy_amt, offers[offerId].pay_amt);    //Decrease amount to buy
        ...
        fill_amt = sadd(fill_amt, rmul(buy_amt * 10 ** 9, rdiv(offers[offerId].buy_amt, offers[offerId].pay_amt)) / 10 ** 9); //Add proportional amount of last offer to pay accumulator
    }
    ...
    function _isPricedLtOrEq(
        ...
        returns (bool)
    {
        return smul(offers[low].buy_amt, offers[high].pay_amt)
          >= smul(offers[high].buy_amt, offers[low].pay_amt);
    }
        ...
        returns (uint id)
    {
        ...
        while (_best[address(t_buy_gem)][address(t_pay_gem)] > 0) {
            ...
            if (smul(m_buy_amt, t_buy_amt) > smul(t_pay_amt, m_pay_amt) +
            ...
            t_buy_amt = ssub(t_buy_amt, min(m_pay_amt, t_buy_amt));
            t_pay_amt = smul(t_buy_amt, t_pay_amt) / t_buy_amt_old;
