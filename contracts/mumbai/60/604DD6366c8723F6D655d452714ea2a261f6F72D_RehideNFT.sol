...
interface IRehideNFT {
    ...
    event AddToReadBlocklist(uint256 indexed tokenId, address[] toAddAddresses);
...
abstract contract RehideBase is ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, IRehideBase {
     ...
     * @dev Payment
     */
    address payable public _platformWallet = payable(0xF766b4699590FA04f98098FcD14E7fEf83517110);
    uint256 public _minEthMintPrice = 1000000000000000; // wei = 0.001 ETH
    uint256 public _readPlatformPercentage = 10;
...
contract RehideNFT is IRehideNFT, RehideBase {
    ...
    mapping(uint256 => address[]) public _noteReadBlocklistAddresses;

    /**
     * @dev Notes Shared with Address
     */
    mapping(address => uint256[]) internal _notesSharedWithAddress;
    mapping(address => uint256[]) internal _notesBlockedForAddress;
    ...
    function mintNFT(
        ...
        // Don't enforce min price to owner and whitelisted addresses
    ...
    function readSharedNote(uint256 tokenId) public payable returns (uint256 readsAvailable) {
        ...
        // Check blocklist
        address[] memory noteReadBlocklistAddresses = _noteReadBlocklistAddresses[tokenId];
        if (noteReadBlocklistAddresses.length > 0) {
            bool isBlocked = false;
            for (uint256 i = 0; i < noteReadBlocklistAddresses.length; i++) {
                if (_msgSender() == noteReadBlocklistAddresses[i]) {
                    isBlocked = true;
                    break;
                }
            }
            require(isBlocked, "RehideNFT: Address is in blocklist");
        }
    ...
    function getNoteReads(uint256 tokenId)
        external
        view
        returns (NoteRead[] memory tokenNoteReads)
    {
        tokenNoteReads = _noteReads[tokenId];
    }

    function getWalletNoteReads(address account)
        external
        view
        returns (uint256[] memory walletNoteReads)
    {
    ...
    function addToReadBlocklist(uint256 tokenId, address[] memory toAddAddresses) public {
        require(_exists(tokenId), "RehideNFT: Allowlist set of nonexistent token");
        require(ownerOf(tokenId) == _msgSender(), "RehideNFT: Trying to set another wallet's token allowlist");
        require(toAddAddresses.length > 0, "RehideNFT: No addresses to add");

        for (uint256 i = 0; i < toAddAddresses.length; i++) {
            _noteReadBlocklistAddresses[tokenId].push(toAddAddresses[i]); // tokenId => address
            _notesBlockedForAddress[toAddAddresses[i]].push(tokenId); // address => tokenId
        }
        
        emit AddToReadBlocklist(tokenId, toAddAddresses);
    }
    ...
    function getNoteReadAllowlist(uint256 tokenId) external view
        returns (address[] memory addresses)
    {
        require(_exists(tokenId), "RehideNFT: Nonexistent token");
        // require(_notesMapping[tokenId].allowlistOnly > 0, "RehideNFT: Not allowlist token");

        addresses = _noteReadAllowlistAddresses[tokenId];
    }

    function getNoteReadBlocklist(uint256 tokenId) external view
        returns (address[] memory addresses)
    {
        require(_exists(tokenId), "RehideNFT: Nonexistent token");

        addresses = _noteReadBlocklistAddresses[tokenId];
    }

    function getWalletNotesCount(address account)
        external
        view
        returns (uint256 notesCount)
    {
        notesCount = _tokenIdsForAddress[account].length;
    }

    function getWalletNotes(address account)
        external
        view
        returns (Note[] memory)
    {
        uint256[] memory tokenIds = _tokenIdsForAddress[account];
        uint256 notesCount = tokenIds.length;
        Note[] memory result = new Note[](notesCount);
        for (uint256 i = 0; i < notesCount; i++) {
            result[i] = _notesMapping[tokenIds[i]];
        }
        return result;
    }

    /**
    * @dev Get Notes shared with Wallet
    */
   function getNotesBlockedForAddressCount(address account)
        external
        view
        returns (uint256 notesCount)
    {
        notesCount = _notesBlockedForAddress[account].length;
    }

    function getNotesBlockedForAddress(address account)
        external
        view
        returns (Note[] memory)
    {
        uint256[] memory accountNoteIds = _notesBlockedForAddress[account];
        uint256 notesCount = accountNoteIds.length;
        Note[] memory result = new Note[](notesCount);
        for (uint256 i = 0; i < notesCount; i++) {
            result[i] = _notesMapping[accountNoteIds[i]];
        }
        return result;
    }

    /**
    * @dev Get Notes blocked for Wallet
    */
   function getNotesSharedWithAddressCount(address account)
        external
        view
        returns (uint256 notesCount)
    {
    ...
    function getNotesSharedWithAddress(address account)
        external
        view
        returns (Note[] memory)
    {
        uint256[] memory accountNoteIds = _notesSharedWithAddress[account];
        uint256 notesCount = accountNoteIds.length;
        Note[] memory result = new Note[](notesCount);
        for (uint256 i = 0; i < notesCount; i++) {
            result[i] = _notesMapping[accountNoteIds[i]];
    ...
    function distributeReferrerRewards(address payable referrer) private {
        ...
        if (mintFee > 0) {
            ...
            _platformWallet.transfer(mintFee - totalTxReferrerRewards);
            emit RewardsTransferred(_platformWallet, mintFee - totalTxReferrerRewards);
        }
    }
    ...
    function setTokenPackage(uint256 tokenId, string memory tokenPackage) private {
        ...
        // Note storage note = _notesMapping[tokenId];
        // require(note.ttl > block.timestamp, "RehideNFT: Trying to set package of expired note");

        // require(ownerOf(tokenId) == _msgSender(), "RehideNFT: Trying to set another wallet's token package");
        // Commenting above because this private function only gets called at mint (owner could be diff. wallet-recipient)
