...
contract SwapPoolV2 is OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
  ...
  INativeERC20 public middleware;
  ...
  function setMiddleware(address newMiddleware) external virtual onlyOwner {
    require(address(middleware) != address(0), "alreadyd set");
    middleware = INativeERC20(newMiddleware);
    nativeToken.approve(newMiddleware, type(uint256).max);
  }
