...
interface IBase721 {
    ...
    function createCollectible(address from, string memory tokenURI, address[] memory royalty, uint256[] memory royaltyFee) external returns(uint256);

}

interface IBase1155 {

    function mint(address from, string memory uri, uint256 supply, address[] memory royaltyAddress, uint256[] memory royaltyFee) external;
...
contract TransferProxy {
    ...
    function erc721mint(IBase721 token, address from, string memory tokenURI, address[] memory royalty, uint256[] memory royaltyFee) external {
        token.createCollectible(from, tokenURI, royalty, royaltyFee);
    }

    function erc1155mint(IBase1155 token, address from, string memory tokenURI, address[] memory royalty, uint256[] memory royaltyFee, uint256 supply) external {
...
contract Trade {
    ...
    function mint(address nftAddress, BuyingAssetType nftType, string memory tokenURI, uint256 supply, address[] memory recipient, uint256[] memory royaltyFee ) external onlyMinter returns(bool) {
        if(nftType == BuyingAssetType.ERC721) {
            transferProxy.erc721mint(IBase721(nftAddress), msg.sender, tokenURI, recipient, royaltyFee);
        }
        else if(nftType == BuyingAssetType.ERC1155) {
            transferProxy.erc1155mint(IBase1155(nftAddress), msg.sender, tokenURI, recipient, royaltyFee, supply);
        }
        return true;
    }
