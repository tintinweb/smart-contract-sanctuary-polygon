...
contract ProtectionProgram is IProtectionProgram, OwnableUpgradeable {
    ...
    function getNftsByOwner(address _address, uint256 _from, uint256 _amount) external view override returns(uint256[] memory, CreatureType[] memory) {
        ...
        CreatureType[] memory _creatureTypes = new CreatureType[](_amount);
        uint256 _k = _from;
        for (uint256 i = 0; i < _amount; i++) {
            uint256 _num = nftOwnerToNftNumber[_address].at(_k);
            _nfts[i] = _num;
            _creatureTypes[i] = _creatureContract.getCreatureType(_num);
            _k++;
        }

        return (_nfts, _creatureTypes);
    }
...
interface IProtectionProgram is ICreatureType {
    ...
    function getNftsByOwner(address _address, uint256 _from, uint256 _amount) external view returns(uint256[] memory, CreatureType[] memory);
}
