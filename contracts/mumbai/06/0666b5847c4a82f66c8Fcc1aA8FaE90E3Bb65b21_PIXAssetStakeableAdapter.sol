...
contract PIXAssetStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    ...
    event FacilityStakeRequirementsUpdated(
        uint _tokenId,
        uint _pixRarity,
        bool _allowed);
    ...
    event TokenInfoUpdated(bool isStakeable,
        bool isBase,
        bool isRaidable,
        Distribution class,
        uint resourceEV,
    ...
    mapping(address => mapping(uint => uint)) seeds;
    mapping(address => mapping(uint => uint)) tokenIds;
    mapping(address => mapping(uint => uint)) facilityOutput;
    mapping(address => mapping(uint => uint)) facilityLeadTime;
    mapping(uint => TokenInfo) tokenInfo;
    mapping(uint => mapping(uint => bool)) facilityStakeReq;
    ...
    modifier onlyStakedTokens(address _userAddress, uint _nonce){
    ...
    function setTokenInfo(uint _tokenId,
        bool _isStakeable,
        bool _isBase,
        bool _isRaidable,
        Distribution _class,
        uint _resourceEv,
        uint _resourceId,
        uint _resourceCap) external onlyOwner {
    ...
    function setFacilityStats(uint[] memory _lead, uint[] memory _amt) external onlyOwner {
        leadTimes = _lead;
        prodAmounts = _amt;
    }

    function __update_cutoff(uint _id, uint _ev, uint _cap) private {
        cutoffs[_id] = CUTOFF_COEF * (_cap / _ev) + CUTOFF_INTERCEPT;
    }

    function setFacilityStakeRequirements(uint _tokenId, uint _pixRarity, bool _allowed) external onlyOwner {
    ...
    function setERC1155(address _erc1155) onlyOwner external {
    ...
    /// @inheritdoc IMissionControlStakeable
    function checkTile(address _userAddress, uint _nonce) external view override returns (uint _amount, uint _retTokenId){
    ...
    function onCollect(address _userAddress, uint _nonce) external override onlyMissionControl returns (uint, uint){
        (uint available, uint id) = __check_tile(_userAddress, _nonce);
        seeds[_userAddress][_nonce] = block.timestamp;
        return (available, id);
    }
    /// @inheritdoc IMissionControlStakeable
    function isBase(uint _tokenId) view external override returns (bool _isBase){
    ...
    /// @inheritdoc IMissionControlStakeable
    function onStaked(address _currentOwner, uint _tokenId, uint _nonce, address _underlyingToken, uint _underlyingTokenId) override external onlyMissionControl {
    ...
    /// @inheritdoc IMissionControlStakeable
    function onUnstaked(address _newOwner, uint _nonce) override external onlyMissionControl onlyStakedTokens(_newOwner, _nonce) {
    ...
    function __handleFacilitySetup(uint _tokenId, address _owner, address _underlyingToken, uint _underlyingTokenId, uint _nonce) private returns (bool) {
        IPIX.PIXInfo memory info = pix.getInfo(_underlyingTokenId);
        facilityLeadTime[_owner][_nonce] = leadTimes[uint(info.category)];
        facilityOutput[_owner][_nonce] = prodAmounts[uint(info.size)];
        return facilityStakeReq[_tokenId][uint(info.category)] && _underlyingToken == pixAddress;
    }

    function __check_tile(address _userAddress, uint _nonce) private view returns (uint, uint){
    ...
    function __checkTile_facility(address _userAddress, uint tokenId, uint _nonce) internal virtual view returns (uint, uint){
        uint localSeed = seeds[_userAddress][_nonce];
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint ret = (facilityOutput[_userAddress][_nonce] * ((block.timestamp - localSeed) / facilityLeadTime[_userAddress][_nonce]));
    ...
    function __checkTile_deterministic_linear(address _userAddress, uint tokenId, uint _nonce) internal virtual view returns (uint, uint){
        uint localSeed = seeds[_userAddress][_nonce];
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint ret = (tokenInfo[tokenId].resourceEV * (block.timestamp - localSeed)) / 86000;
        return (ret, tokenInfo[tokenId].resourceId);
    }

    function __checkTile_Poisson(address _player_address, uint tokenId, uint _nonce) internal virtual view returns (uint, uint){
        int localSeed = int(seeds[_player_address][_nonce]);
        require(localSeed != 0, "MCS1155_ADAPTER: No such token staked");
        uint ret = 0;
        ...
        if (uint(blockTime - localSeed) > cutoffs[tokenId]) {
            return (tokenInfo[tokenId].resourceCap, tokenInfo[tokenId].resourceId);
        }
        uint localEV = tokenInfo[tokenId].resourceEV;
        uint localCap = tokenInfo[tokenId].resourceCap;
        int T = _getT(_player_address, tokenId, localSeed, localEV);
    ...
    */
    ...
    function _getT(address _playerAddress, uint tokenId, int seed, uint ev) view internal returns (int T){
        require(ev > 0, "PIX: Expected value is zero");
        T = _max_div(86400 * MAX_SCALE,
            int(ev) * MAX_SCALE) * _exponential_2(_playerAddress, tokenId, seed) / (MAX_SCALE * MAX_SCALE);
    }

    function _uniform(address _playerAddress, uint tokenId, int seed) pure internal returns (uint uniform_sample){
        uniform_sample = uint(keccak256(abi.encode(_playerAddress, tokenId, seed))) % PROBA_RES;
    }

    function _exponential_2(address _playerAddress, uint tokenId, int seed) pure internal returns (int256 exponential_sample){
        exponential_sample = - _max_ln(
            MAX_SCALE - _max_div(
            MAX_SCALE * int(_uniform(_playerAddress, tokenId, seed)),
