...
contract CryptoRunnerState is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public initialClock; // 100
    uint256 public clockRegen; // 100% regen in 24 hours / block
    uint256 public maxClock; // 100

    DATA public data;

    mapping(uint256 => bool) private usedNonces;
    mapping(string => bool) private usedRunIds;
    mapping(string => bool) private finishedRuns;
    mapping(uint256 => string[]) private runsByRunner;
    ...
    event RunStarted(address indexed sender, uint256 indexed tokenId, string indexed runId);
    event RunEnded(address indexed to, uint256 indexed tokenId, string indexed runId);
    ...
    function startRun(bytes calldata data_) external returns (CryptoRunner memory) {
        ...
        if (!cryptoRunner.initialized) {
            ...
            uint256 leftOverClock = _hasEnoughClock(runArgs[0], runArgs[1]);
            cryptoRunner.remainingClock = leftOverClock > maxClock ? maxClock : leftOverClock;
        }
        cryptoRunner.lastRunTimestamp = block.timestamp;
        cryptoRunner.isRunning = true;
        cryptoRunner.lastRunId = runId;
        cryptoRunner.tokenId = runArgs[0];
        cryptoRunner.initialized = true;
        cryptoRunners[runArgs[0]] = cryptoRunner;
        runsByRunner[runArgs[0]].push(runId);
    ...
    function _startRunArgs(bytes memory data_) internal returns (uint256[] memory, string memory) {
        ...
        require(!usedRunIds[runId], "E005");
        usedRunIds[runId] = true;
        usedNonces[args[2]] = true;
        runIds.push(runId);
        nonces.push(args[2]);
        return (args, runId);
    }
    ...
    function userRuns(uint256 tokenId_) public view returns (string[] memory) {
        return runsByRunner[tokenId_];
    }
