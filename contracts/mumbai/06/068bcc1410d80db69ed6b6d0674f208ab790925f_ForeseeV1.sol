...
/**
 * @title ForeseeV1
 * @notice Based on the PancakeswapPredictionV2 contract
 */
contract ForeseeV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    event NewAggregator(address aggregator);
    ...
    constructor(
        ...
        aggregator = AggregatorInterface(_aggregatorAddress);
    ...
    function claim(uint256[] calldata epochs) external nonReentrant notContract {
        ...
        for (uint256 i = 0; i < epochs.length; i++) {
            ...
            //
            require(aggregator.latestTimestamp() > rounds[epochs[i]].closeTimestamp, "Round has not ended");
     ...
     * @dev Callable by admin
     */
    function setAggregator(address _aggregator) external whenPaused onlyAdmin {
        require(_aggregator != address(0), "Cannot be zero address");
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            !round.oracleCalled &&
            !betInfo.claimed &&
            aggregator.latestTimestamp() > round.closeTimestamp + bufferSeconds &&
            betInfo.amount != 0;
    }
    ...
    function _safeEndRound(
        ...
        require(
            aggregator.latestTimestamp() >= rounds[epoch].closeTimestamp,
            "Can only end round after closeTimestamp"
        );
        require(
            aggregator.latestTimestamp() <= rounds[epoch].closeTimestamp + bufferSeconds,
    ...
    function _safeLockRound(
        ...
        require(aggregator.latestTimestamp() >= rounds[epoch].lockTimestamp, "Can only lock round after lockTimestamp");
        require(
            aggregator.latestTimestamp() <= rounds[epoch].lockTimestamp + bufferSeconds,
            "Can only lock round within bufferSeconds"
        );
        Round storage round = rounds[epoch];
        round.closeTimestamp = aggregator.latestTimestamp() + intervalSeconds;
    ...
    function _safeStartRound(uint256 epoch) internal {
        ...
        require(
            aggregator.latestTimestamp() >= rounds[epoch - 2].closeTimestamp,
    ...
    function _startRound(uint256 epoch) internal {
        Round storage round = rounds[epoch];
        round.startTimestamp = aggregator.latestTimestamp();
        round.lockTimestamp = aggregator.latestTimestamp() + intervalSeconds;
        round.closeTimestamp = aggregator.latestTimestamp() + (2 * intervalSeconds);
        round.epoch = epoch;
        round.totalAmount = 0;
    ...
    function _bettable(uint256 epoch) internal view returns (bool) {
        return
            ...
            aggregator.latestTimestamp() > rounds[epoch].startTimestamp &&
            aggregator.latestTimestamp() < rounds[epoch].lockTimestamp;
    }
    ...
    function _getPriceFromOracle() internal view returns (uint80, int256) {
        uint256 leastAllowedTimestamp = aggregator.latestTimestamp() + oracleUpdateAllowance;
