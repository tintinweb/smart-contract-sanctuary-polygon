...
/// @custom:security-contact [email protected]ev
contract AgroSuranceLand is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
  ...
  constructor(string memory _description, string memory _defaultCropImage) ERC721("AgroSuranceLand", "ASL") {
    description = _description;
    cropDetails[0].cropId = 0;
  ...
  function addCrop(string calldata name, string calldata image) external onlyOwner returns (uint256 cropId) {
    ...
    crop.name = name;
    crop.image = image;
    emit CropAdded(cropId, name, image);
  }

  function updateCrop(uint256 cropId, string calldata name, string calldata image) external onlyOwner {
    if (bytes(name).length == 0) revert EmptyString();
    if (!cropExists(cropId)) revert InvalidCropId();
    Crop storage crop = cropDetails[cropId];
    crop.name = name;
    crop.image = image;
    emit CropUpdated(cropId, name, image);
  }

  function addLand(string calldata name, int32 lat, int32 long, uint256 area) external returns (uint256 landId) {
    if (bytes(name).length == 0) revert EmptyString();
  ...
  function addCurrentCycle(
    ...
    landHistory[landId].push(
      LandHistory({landId: landId, historyId: historyId, cropId: cycleCropId, from: cycleFrom, to: cycleTo})
    );
  ...
  function tokenURI(uint256 tokenId) public view override returns (string memory) {
    ...
    Crop memory crop = cropDetails[land.currentCycleTo > block.timestamp ? land.currentCycleCropId : 0];
...
contract InsuranceManager is FunctionsClient, ConfirmedOwner {
  ...
  function checkInsuranceStatus(bytes32 requestId) external onlyRequestOwner(requestId) {
    ...
    requestId = _executeRequest(checkInsuranceStatusCode, args);
    requestTypes[requestId] = RequestType.CHECK_CLAIM;
    quoteRequests[requestId].insuranceStatusRequestTime = block.timestamp;
    emit CheckClaimRequestMade(msg.sender, landId, requestId, cropId, request.insuranceFrom, insuranceTo);
  }
