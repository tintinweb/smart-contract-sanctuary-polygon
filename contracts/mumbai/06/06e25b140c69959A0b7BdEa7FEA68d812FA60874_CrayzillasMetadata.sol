...
abstract contract ColorableSectionMap is IColorableSectionMap {
    ...
    */
    mapping(uint256 => mapping(uint256 => bool)) internal _colorableAreas;
    mapping(uint256 => uint256) internal _numColorableAreas; // traitIds -> # colorable areas

    function colorableAreas(uint256 traitId, uint256 layerId) public view override returns (bool isColorable) {
        return _colorableAreas[traitId][layerId];
    }

    function numColorableAreas(uint256 traitId) public view override returns (uint256 numColorableAreasOfTrait) {
    ...
    function _setColorableAreas(
        ...
        for (uint256 i = 0; i < _loopThrough; i++) {
            _colorableAreas[_traitIds[i]][_colorableLayerIds[i]] = true;
            _numColorableAreas[_traitIds[i]]++;
        }
    }
    ...
    function verifyColorMap(
        ...
        uint256[] memory _layerIdsToColor
