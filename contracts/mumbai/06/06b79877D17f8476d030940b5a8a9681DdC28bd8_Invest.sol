...
interface IPayableToken is IERC20, IERC20Permit, IERC20Metadata {}
interface IReleaseToken is IERC20, IERC20Metadata {}
...
contract Invest is Ownable, ReentrancyGuard {
    ...
    IPayableToken public payableToken;

    constructor(IPayableToken _payableToken) {
        payableToken = _payableToken;
    }

    modifier onlyOwnerOrSigner() {
        require(owner() == _msgSender() || signers[_msgSender()] == true, "InvestSplitter: caller must be owner or signer");
        _;
    }
    ...
    function getInvestorsCount(string memory _key) public view onlyProjectOwner(_key) returns(uint256) {
        return projects[_key].investors.length;
    }

    function getInvestors(string memory _key, uint256 startIndex, uint256 count) public view onlyProjectOwner(_key) returns (Investor[] memory) {
    ...
    function setProjectToken(string memory _key, IReleaseToken _token) public onlyProjectOwner(_key) {
        require(_token != IReleaseToken(address(0)), "Token address cannot be zero address");
    ...
    function createProject(string memory _key, uint256 _price, uint256 _decimals, uint256 _limit) public {
        ...
        Project memory newProject = Project({
            ...
            releaseToken: IReleaseToken(address(0)),
            investors: investors,
            funds: projectFunds
        });
    ...
    function invest(address _user, string memory _key, uint256 _amount) internal {
        require(IPayableToken(payableToken).transferFrom(_user, projects[_key].beneficiary, _amount));
        ...
        uint256 tokens = (investment * 10 ** projects[_key].decimals)/ (projects[_key].price * 10 ** payableToken.decimals());
    ...
    function investWithPermit(bool isFund, address _user, string memory _key, uint256 _amount, uint8 v, bytes32 r, bytes32 s, uint256 _expiry) public {
        IPayableToken(payableToken).permit(_user, address(this), _amount, _expiry,  v, r, s);
    ...
    function sendReleasableTokens(string memory _key, address _investor, uint256 _amount) internal {
        require(
            IReleaseToken(projects[_key].releaseToken).transferFrom(
                _msgSender(), _investor, _amount * 10 ** projects[_key].releaseToken.decimals()
            )
        );
    ...
    function releaseToken(string memory _key, uint256 _amount) public nonReentrant {
        ...
        require(projects[_key].releaseToken != IReleaseToken(address(0)), "Release token is not set");
