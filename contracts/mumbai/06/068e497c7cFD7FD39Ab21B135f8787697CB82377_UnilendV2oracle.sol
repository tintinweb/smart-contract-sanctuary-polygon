...
import "../lib/lib/utils/Context.sol";
import "../lib/lib/utils/math/SafeMath.sol";
import "../lib/lib/access/Ownable.sol";
...
contract UnilendV2oracle is Ownable {
    ...
    function getChainLinkAssetPrice(
        address asset
    ...
    function setTokenPrice(address token0, address token1, uint price) public {
        ...
        aPrice[token1][token0] = uint(1).div(price);
    }

    function setTokenPriceB(address token0, address token1, uint price) public {
        aPrice[token0][token1] = price;
    }
}