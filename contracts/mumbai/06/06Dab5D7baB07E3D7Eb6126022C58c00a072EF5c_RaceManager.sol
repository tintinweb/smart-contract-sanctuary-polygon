...
contract RaceManager is AccessControl, System, ERC2771Context_Upgradeable {
    ...
    mapping(string => address[]) public racers;
    ...
    event RaceCreated(string raceId);
    event RaceWon(string raceId, address winner, uint256 prize);
    event CommissionWithdrawn(uint256 amount);

    constructor(IERC20 _entryToken, uint256 _entryFee, uint256 _commission, address _forwarder, bytes32 _systemId) ERC2771Context_Upgradeable(_forwarder) System(_systemId){
    ...
    function createOrJoinRace(string memory raceId) external {
        require(entryToken.balanceOf(msg.sender) >= entryFee, "Insufficient token balance");

        if (races[raceId].entryFee == 0) {
            races[raceId] = Race(entryToken, entryFee, commission, false);
            emit RaceCreated(raceId);
        }

        entryToken.transferFrom(msg.sender, address(this), entryFee);
        racers[raceId].push(msg.sender);
    }

    function endRace(string memory raceId, address winner) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(!races[raceId].raceEnded, "Race already ended");

        uint256 totalPrize = entryToken.balanceOf(address(this)) * (100 - races[raceId].commission) / 100;
        entryToken.transfer(winner, totalPrize);
        races[raceId].raceEnded = true;
        totalCommission += entryToken.balanceOf(address(this));

        emit RaceWon(raceId, winner, totalPrize);
    }

    function withdrawCommission() external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function getRacers(string memory raceId) external view returns (address[] memory) {
        return racers[raceId];
    }

    function setEntryFee(uint256 _entryFee) external onlyRole(DEFAULT_ADMIN_ROLE) {
        entryFee = _entryFee;
    }
