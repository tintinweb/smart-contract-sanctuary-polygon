...
contract NFTFactory is Initializable, Ownable {
    ...
    mapping(uint256 => address) public deployedCollections;
    uint256 public deployedCollectionsLength;

    event NewCollection(uint256 _id, address _proxyAddress);
    event ProxyAdminChanged(address _newProxyAdmin);
    event ImplementationChanged(address _newImplementation);

    function initialize(address _impl, address _admin) public initializer {
    ...
    function newProxy(
        ...
        TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
            ...
            abi.encodeWithSignature(
                "initialize(bytes,address,address,string,uint256,uint256)",
        ...
        deployedCollections[deployedCollectionsLength] = address(proxy);
        emit NewCollection(deployedCollectionsLength, address(proxy));
        ++deployedCollectionsLength;

        return address(proxy);
    }

    function changeProxyAdmin(address _newProxyAdmin) public onlyOwner {
...
import "@rari-capital/src/tokens/ERC1155.sol";
import "@sygnum/solidity-base-contracts/contracts/helpers/Initializable.sol";
...
contract SygnumERC1155 is Initializable, Operatorable, Pausable, ERC1155 {
    ...
    function initialize(
        ...
        maxUniqueTokens = _encodedMaxTokenSupplies.length;
    ...
    {
        return string(abi.encodePacked(baseUri, bytes32(id), ".json"));
    }

    function maxTokenSupply(uint256 tokenId)
        public
        view
        virtual
        returns (uint256 res)
    {
        res = uint8(encodedMaxTokenSupplies[tokenId]);
    }
    ...
    function _mint(
        ...
        if (mintDuration > 0) {
            require(block.timestamp >= startDate, "Minting has not started");
            require(
                block.timestamp < startDate + mintDuration,
                "Minting has ended"
            );
        }

        require(
            tokenSupply[id] + amount <= maxTokenSupply(id),
            "Amount of tokens to mint too high"
        );
        require(id < maxUniqueTokens, "Invalid ID");
