...
/// @dev Clones for this contract will be deployed by registered MembershipsFactory contract.
    ...
    IMembershipsV1
{
    ...
    /// @notice Emitted when trying to set `factory` to zero address
    error InvalidFactory();

    /// @notice Emitted when trying to initialize with `cap` <= 0
    error InvalidCap();
    ...
    /// @notice Factory contract that deploys new clones
    ...
    /// @notice Sets `_factory`
    /// @dev Reverts if `_factory` param is address(0)
    /// @param _factory address of allowed MembershipsFactory contract
    ...
    /// @param baseURI_ contractURI for Membership
    ...
    function initialize(
        ...
        if (_membership.cap <= 0) revert InvalidCap();
    ...
    /// @dev Reverts if purchases are paused.
    ...
    /// @dev Reverts if renewals are paused.
    ///      Reverts if `msg.value` is not set to `price`.
    ...
    function withdraw() external onlyOwner nonReentrant {
        uint256 fee = (address(this).balance * IMembershipsFactory(factory).feeBPS()) / 10_000;

        address payable feeTreasury = IMembershipsFactory(factory).feeTreasury();
        if (fee > 0) {
            _sendEther(feeTreasury, fee);
        }

        emit Withdrawal(address(this).balance, treasury, fee, feeTreasury);

        _sendEther(treasury, address(this).balance);
    }
    ...
    /// @dev Utility method to handle ether transfers.
