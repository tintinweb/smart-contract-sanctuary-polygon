...
contract CryptoBuggy is Ownable {
    ...
    mapping(address => uint256) public partialUsers;
    ...
    constructor(
        ...
        donateNFT = DonateNFT(_donateNft);
    ...
    function addFundPartially(string memory _signature) public payable {
        ...
        if (partialUsers[msg.sender] == 0) {
            (partialUsers[msg.sender]) = donateNFT.mintToken(
                msg.sender,
                msg.value
            );
        } else {
            donateNFT.addValue(partialUsers[msg.sender], msg.value);
        }
        uniqUsers++;
    ...
    function setNewBuggyPrice(uint256 newBuggyPrice) public onlyOwner {
        price = newBuggyPrice;
    }
}
