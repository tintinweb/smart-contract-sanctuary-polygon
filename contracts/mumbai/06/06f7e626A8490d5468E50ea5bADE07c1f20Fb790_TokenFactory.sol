...
contract TokenFactory is AccessControl {
    ...
    function createToken(
        ...
        address _transferAgent, address _tokenizationAgent) external onlyRole(Owner_Role){
        ...
        _initCompliance(identityRegistry, _name, _symbol, _decimals, _cap, mappingValue, _holdReleaseTimestamp, _tokenLimitPerUser,_countries,_transferAgent, _tokenizationAgent);
    }
    function _initCompliance(address identityRegistry,string memory _name,string memory _symbol,uint8 _decimals,uint _cap, string memory mappingValue, 
                uint256 _holdReleaseTimestamp, uint256 _tokenLimitPerUser, uint16[] calldata _countries, address _transferAgent, address _tokenizationAgent) internal{
        ...
        require(success, "Adding Trnasfer Agent Failed");
        success = false;
        ( success,) = proxy.call(abi.encodeWithSelector(0xadccbc3c, _tokenizationAgent));
    ...
    function whitelistUser(address _token, address _userAddress, uint16 _countryCode) public onlyRole(Owner_Role) {
        IIdentityRegistry iR = IToken(_token).identityRegistry();
        address userIdentity = deployIdentity(_userAddress,false);
        IIdentityRegistry(iR).registerIdentity(_userAddress, IIdentity(userIdentity), _countryCode);
        emit whiteListed(_userAddress);
    }

    function batchWhitelistUser(address _token, address[] calldata _userAddress, uint16[] calldata _countrycodes)external onlyRole(Owner_Role){
