...
contract Manager is IMANAGER, AccessControl {
    ...
    function mintNft(
        ...
        uint256 nftID = INFTSERC721(s_nftsERC721).mint(p_uri);

        address sellerAddress = IFACTORYSELLER(s_factorySeller).create(p_stableCoinAddress, p_beneficiaryAddress, p_aggregatorStableCoinDollar);

        s_hodings[nftID] = IFACTORYHOLDINGS(s_factoryHoldings).create(sellerAddress, p_amountToSell, p_name, p_symbol);

        ISELLER(sellerAddress).setHoldingsAddress(s_hodings[nftID]);

        s_sellers[s_hodings[nftID]] = sellerAddress;

        IDIVIDENDS(s_dividends).addHoldingsAddress(s_hodings[nftID]); 
    ...
    function buyThroughCompany(
        ...
        ISELLER(p_addressContractSeller).buyThroughCompany(p_amountToken, p_buyer);
    }
    ...
    function addDividends(address p_addressOrigin, address p_addressContractHoldings, uint256 p_amountIncrementDividends) public override {
        require(
            hasRole(ADMIN_ROLE, msg.sender), 
            "Without permission"
        );

        IDIVIDENDS(s_dividends).addDividends(p_addressOrigin, p_addressContractHoldings, p_amountIncrementDividends);
    }
    ...
    function setSignerDividends(address p_newSigner) public override { 
        require(
            hasRole(ADMIN_ROLE, msg.sender), 
            "Without permission"
        );

        IDIVIDENDS(s_dividends).setSigner(p_newSigner);
    }
    ...
    function setBaseUriNFTs(string memory p_baseURI) public override {
        require(
            hasRole(ADMIN_ROLE, msg.sender), 
            "Without permission"
        );
        
        INFTSERC721(s_nftsERC721).setBaseURI(p_baseURI);
    }
}
...
interface IMANAGER {   
    ...
    // PUBLIC FUNCTIONS
        ...
        function addDividends(address p_addressOrigin, address p_addressContractHoldings, uint256 p_amountIncrementDividends) external;
        function setStatusWhiteList(
            address p_address, 
            IWHITELIST.Status p_status
        ) external;
