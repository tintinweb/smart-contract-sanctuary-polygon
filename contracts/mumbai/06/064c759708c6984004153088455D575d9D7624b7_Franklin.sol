    ...
    UUPSUpgradeable
{
    ...
    event BulkAddedWorkers(address[] _workerAddresses);
    ...
    event StreamDeposit(uint256 _amount, address _token);
    /// @notice Emits amount and token address withdrawn from nonstreaming treasury
    event StaticTreasuryWithdrawal(uint256 _amount, address _token);
    // @notice Emits amount and token address withdrawn from streaming treasury
    event StreamingTreasuryWithdrawal(uint256 _amount, address _token);
    ...
    **/
    ...
    /// @notice Emits details of streams created in bulk
    event StreamsBulkCreated(
        address[] _workerAddresses,
        address[] _tokenAddresses,
        uint256[] _startTimes,
        uint256[] _endTimes,
    ...
    /// @dev Array of workers allowing us to loop through streams for getter f()
    address[] workerArray;
    ...
    modifier onlyAdmin() {
        require(admins[_msgSender()], "Must be an admin");
        _;
    }
    ...
    modifier onlyApprovedTokens(address _token) {
        require(registeredToken[_token], "Token is not approved");
        _;
    }
    ...
    /// @param firstToken The first _token approved for Payroll
    function initialize(address firstToken, address forwarder)
        public
        initializer
    {
        __Ownable_init();
    ...
    function getTotalTreasury(address _token)
        ...
        returns (uint256)
    {
        uint256 streamBalance = getTreasuryStreamBalance(_token);
        uint256 staticBalance = getStaticTreasuryBalance(_token);

        uint256 totalTreasury = streamBalance + staticBalance;
    ...
    function getStaticTreasuryBalance(address _token)
        public
        view
        onlyAdminOrOwner
        onlyApprovedTokens(_token)
        returns (uint256)
    {
        ...
        uint256 staticBalance = treasury.staticBalance;
        return (staticBalance);
    }

    /// @notice Provides the quantity of funds used for streaming payroll
    /// @param _token The address of the ERC20 _token contract
    /// @return The quantity of funds in the streaming treasury
    function getTreasuryStreamBalance(address _token)
        public
        view
        onlyAdminOrOwner
        onlyApprovedTokens(_token)
    ...
    {
        Treasury storage treasury = tokenTreasury[_token];
        uint256 settledBalance = treasury.settledStreamingBalance;
        uint256 streamed = 0;

        /* For each worker, sum the value streamed and add to streamed
           which will be subtracted from the settled amount to get the balance*/
        for (uint256 i = 0; i < workerArray.length; i++) {
            Balance storage b = tokenBalance[workerArray[i]][_token];
            Stream storage current = b.current;
            Stream storage next = b.next;
            // check that current stream started
        ...
        return (settledBalance - streamed);
    }
    ...
    function getTreasuryStreamRate(address _token)
        public
        view
        onlyAdminOrOwner
        onlyApprovedTokens(_token)
    ...
    function getPayrollAsWorker(address _token)
        ...
        returns (uint256)
    {
        uint256 staticBalance = getSettledBalance(_msgSender(), _token);
        uint256 streamBalance = getStreamBalance(_msgSender(), _token);
        return (staticBalance + streamBalance);
    }
    ...
    function getStreamBalance(address _worker, address _token)
        ...
        returns (uint256)
    {
        uint256 currentBalance = getStreamCurrentBalance(_worker, _token);
        // nextBalance > 0 only if current balance has expired
        uint256 nextBalance = getStreamNextBalance(_worker, _token);
    ...
    function getStreamCurrentBalance(address _worker, address _token)
        internal
        view
        returns (uint256)
    {
        ...
        if (current.start <= block.timestamp) {
            ...
            if (current.end != 0 && block.timestamp > current.end) {
                balance =
    ...
    /// @param _token The address of the _oken being looked up
    /// @return Value of balance.current stream for defined worker/token
    function getStreamNextBalance(address _worker, address _token)
        internal
        view
        returns (uint256)
    {
    ...
    function getNextStreamParams(address _worker, address _token)
        external
        view
        onlyAdminOrOwner
        onlyExistingWorker(_worker)
    ...
    function getPayrollAsAdminOrOwner(address _worker, address _token)
        ...
        returns (uint256)
    {
        uint256 staticBalance = getSettledBalance(_worker, _token);
        uint256 streamBalance = getStreamBalance(_worker, _token);
    ...
    function removeAdmin(address _admin) external whenNotPaused onlyOwner {
        require(admins[_admin] == true, "Admin does not exist");
    ...
    function addApprovedToken(address _token) public whenNotPaused onlyOwner {
    ...
    function addWorker(address _worker) external whenNotPaused onlyAdmin {
        require(
            _worker != address(this),
            "Cannot add the address of this contract"
        );
        require(!workers[_worker], "Worker already exists");

        workers[_worker] = true;
        workerArray.push(_worker);
    ...
    function bulkAddWorkers(address[] calldata _workerAddresses)
        ...
        onlyAdmin
    {
        for (uint256 i = 0; i < _workerAddresses.length; i++) {
            require(
                !workers[_workerAddresses[i]],
                "Worker address already exists"
            );

            workers[_workerAddresses[i]] = true;
            workerArray.push(_workerAddresses[i]);
        }

        emit BulkAddedWorkers(_workerAddresses);
    }

    /** @notice
        Updates an workers wallet address and transfers payroll from the
        previous wallet address to the new wallet address */
    /// @param _oldAddress The previous address of the worker
    /// @param _newAddress The new address used by the worker
    function updateWorkerAddress(address _oldAddress, address _newAddress)
        external
        whenNotPaused
        onlyAdmin
    {
        require(workers[_oldAddress], "Old address does not exist");
        require(!workers[_newAddress], "New address already exists");
        ...
        for (uint256 i = 0; i < approvedTokens.length; i++) {
            ...
            delete (tokenBalance[_oldAddress][approvedTokens[i]]);
            tokenBalance[_newAddress][approvedTokens[i]] = oldAddressBalance;
        }

        // replace old address with new address in array
        for (uint256 i = 0; i < workerArray.length; i++) {
            if (workerArray[i] == _oldAddress) {
                workerArray[i] = _newAddress;
            }
        }

        delete (workers[_oldAddress]);
        workers[_newAddress] = true;

        emit WorkerAddressUpdated(_oldAddress, _newAddress);
    }
    ...
    function bulkPayWorkers(
        ...
        require(
            _workerAddresses.length <= 500,
            "Batch payment max size is 500"
        );

        for (uint256 i = 0; i < _workerAddresses.length; i++) {
            require(
                workers[_workerAddresses[i]],
                "Worker address does not exist"
            );
            require(_amounts[i] > 0, "Must pay a non-zero amount");
            require(
                registeredToken[_tokenAddresses[i]],
            ...
            tokenBalance[_workerAddresses[i]][_tokenAddresses[i]]
                .settled += _amounts[i];
        }
    ...
    function directTransfer(
        ...
        address _token,
        address _worker
    ) public onlyAdmin onlyApprovedTokens(_token) onlyExistingWorker(_worker) {
        require(
            _amount <= getStaticTreasuryBalance(_token),
            "Insufficient treasury funds"
        );
    ...
    function transferToWorkerBalance(
        ...
        require(workers[_worker], "_worker address not found");
    ...
    function createStream(
        ...
        require(_start < _end, "Start time must be less than end time");
        require(_rate > 0, "Must specify a non-zero rate");
        require(_start > block.timestamp, "_start must not be in the past");
        require(_end > block.timestamp, "_end must not be in the past");
    ...
    function BulkCreateStreams(
        ...
        require(
            _workerAddresses.length <= 500,
            "Bulk stream creation max size is 500"
        );
    ...
    /// @notice terminates a payroll stream
    /// @param _worker The address of the worker receiving the stream
    /// @param _token The address of the token being streamed
    /// @param _end When the stream ends (in seconds past the epoch)
    function endStream(
        ...
        onlyExistingWorker(_worker)
    {
        require(_end != 0, "Must specify a non-zero end time");
        require(_end > block.timestamp, "_end must not be in the past");
    ...
    function deleteStream(address _worker, address _token)
        ...
        onlyExistingWorker(_worker)
    {
        uint256 streamValue = getStreamCurrentBalance(_worker, _token);
        Treasury storage t = tokenTreasury[_token];
        Balance storage b = tokenBalance[_worker][_token];
        if (b.current.withdrawn > 0) {
            t.settledStreamingBalance -= b.current.withdrawn;
        }

        delete (b.current); // this returns value to the treasury balance if no value has been withdrawn
    ...
    function updateBalance(address _worker, address _token) internal {
        Balance storage b = tokenBalance[_worker][_token];
        Treasury storage t = tokenTreasury[_token];
        ...
        if (b.current.end <= block.timestamp && b.current.end != 0) {
            ...
            b.settled += getStreamCurrentBalance(_worker, _token);
    ...
    /// @param _amount The amount of funds to be deposited
    /// @param _token The token to be deposited
    function depositStreamFunds(uint256 _amount, address _token)
        public
        onlyAdminOrOwner
        onlyApprovedTokens(_token)
    {
        ...
        emit StreamDeposit(_amount, _token);
    }

    /** @notice Withdraw funds from the treasury. Pulls from the static treasury
        first before pulling from the streaming treasury. Front end will
        surface a warning if a withdrawal pulls from the streaming treasury */
    /// @param _token The token to withdraw
    /// @param _amount The amount being withdrawn from the treasury
    function withdrawFromStaticTreasury(address _token, uint256 _amount)
        external
        onlyOwner
        onlyApprovedTokens(_token)
    {
        ...
        require(_amount <= staticBalance, "Insufficient treasury funds");
        ...
        emit StaticTreasuryWithdrawal(_amount, _token);
    }

    function withdrawFromStreamingTreasury(address _token, uint256 _amount)
        external
        onlyOwner
        onlyApprovedTokens(_token)
    {
        require(_amount != 0, "Must withdraw a non-zero amount");
        uint256 streamingBalance = getTreasuryStreamBalance(_token);

        require(_amount <= streamingBalance, "Insufficient treasury funds");

        Treasury storage treasury = tokenTreasury[_token];
        treasury.settledStreamingBalance -= _amount;
        IERC20Upgradeable(_token).safeTransfer(_msgSender(), _amount);

        emit StreamingTreasuryWithdrawal(_amount, _token);
    }
