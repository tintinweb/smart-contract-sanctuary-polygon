...
interface IDropClaimCondition {
    /**
     *  The criteria that make up a claim condition.
     *
    ...
    /**
     *  @notice The criteria that sets up the allocation for specific wallets
     *  By Default the wallet can claim upto quantityLimitPerWallet as specified in ClaimCondition
     *  unless there is a special allocation for the wallet
     *
     *  @param claimer                  The wallet address of the claimer
     *
     *  @param allocation               The total number NFT allocated to this wallet address.
     */
...
// File: contracts/interface/drop/IDropERC721.sol
interface IDropERC721 is IERC721, IDropClaimCondition {
    /// Emitted when tokens are claimed.
    event TokensClaimed(
        uint256 indexed claimConditionID,
        address indexed claimedBy,
        address indexed _receivedBy,
    ...
    ERC721Burnable
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("TheOrientalist-ERC721");
    uint256 private constant VERSION = 1;

    /// Only transfers to or from TRANSFER_ROLE holders are valid, when transfers are restricted.
    bytes32 private constant TRANSFER_ROLE = keccak256("TRANSFER_ROLE");
    /// Only MINTER_ROLE holders can lazy mint NFTs.
    bytes32 private constant MINTER_ROLE = keccak256("MINTER_ROLE");

    /// Max bps in the thirdweb system.
    ...
    /// The address that receives all platform fees from all sales.
    address private platformFeeRecipient;

    /// The % of primary sales collected as platform fees.
    uint16 private platformFeeBps;

    /// The (default) address that receives all royalty value.
    address private royaltyRecipient;

    /// The (default) % of a sale to take as royalty (in basis points).
    ...
    /// Mapping from address => total number of NFTs a wallet has allocated.
    mapping(address => uint256) public walletAllocationCount;
    ...
    function verifyClaimMerkleProof(
        ...
        address _claimer,
        bytes32[] calldata _proofs
    ) public view returns (bool validMerkleProof) {
        ClaimCondition memory currentClaimPhase = claimCondition.phases[_conditionId];

        if (currentClaimPhase.merkleRoot != bytes32(0)) {
            validMerkleProof = MerkleProof.verify(
                _proofs,
                currentClaimPhase.merkleRoot,
                keccak256(bytes.concat(keccak256(abi.encode(_claimer))))
            );
    ...
    function verifyClaim(
        ...
        // check if the selected quantity is within the allocated qty or max qty limit per wallet during this claim phase
        require(
            _quantity > 0 &&
                ((walletAllocationCount[_claimer] == 0 && _quantity <= currentClaimPhase.quantityLimitPerWallet) || _quantity <= walletAllocationCount[_claimer]),
            "invalid quantity."
        );
        // check if the qty of already claimed + the selected qty is still within the max supply for this phase
        require(
        ...
        if (walletAllocationCount[_claimer] > 0)
            require(
                walletClaimCount[_claimer] + _quantity <= walletAllocationCount[_claimer],
    ...
    function transferClaimedTokens(
        ...
        claimCondition.limitLastClaimTimestamp[_conditionId][_to] = block.timestamp;
        walletClaimCount[_to] += _quantityBeingClaimed;
    ...
    function setClaimAllocations(WalletAllocation[] calldata _allocations)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        for (uint256 i = 0; i < _allocations.length; i++) {
            walletAllocationCount[_allocations[i].claimer] = _allocations[i].allocation;
        }
    }

    /**
     *  Lets an account with `MINTER_ROLE` lazy mint 'n' NFTs.
