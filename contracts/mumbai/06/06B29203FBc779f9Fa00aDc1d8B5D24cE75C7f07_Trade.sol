...
contract Trade {
    ...
    event TradingFee(uint8 TradingFee);
    ...
    uint8 private platformFeePermille;
    ...
    constructor (uint8 _platformFee, uint8 _tradingFee, uint8 _swappingFee, TransferProxy _transferProxy) {
        platformFeePermille = _platformFee;
    ...
    function buyerPlatformFee() external view virtual returns (uint8) {
        return platformFeePermille;
    }

    function sellerPlatformFee() external view virtual returns (uint8) {
        return platformFeePermille;
    }

    function setPlatformFee(uint8 _platformFee) external onlyOwner returns(bool) {
        platformFeePermille = _platformFee;
        emit PlatformFee(platformFeePermille);
        return true;
    }
    ...
    function setTradingFee(uint8 _tradingFee) external onlyOwner returns(bool) {
        tradingFeePermille = _tradingFee;
        emit TradingFee(tradingFeePermille);
        return true;
    }
    ...
    function getFees(Order memory order) internal view returns(Fee memory){
        ...
        uint assetFee;
        uint priceValue = order.amount * 1000 / (1000 + platformFeePermille);
        platformFee = order.amount - priceValue;
        uint price = priceValue * 1000 / (1000 + tradingFeePermille);
        tradingFee = priceValue - price;
        ...
        uint sellerServiceFee = platformFee + tradingFee;
        assetFee = price - fee - sellerServiceFee;
