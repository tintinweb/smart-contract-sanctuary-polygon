...
contract OddzVault is ReentrancyGuardUpgradeable, OddzPausable {
    ...
    modifier onlyBalanceManager() {
        require(
            _msgSender() == balanceManager,
            "OddzVault: Only balance manager allowed"
        );
        _;
    }
    ...
    /**
     * @notice Used to update Balance Manager contract address
     * @param _orderManager Address of the balance manager ontract
     */
        ...
        nonReentrant
    {
        ...
        require(availableCollateral>= _amount.toInt256(),"OddzVault:Not enough available collateral");

        //update balance of the trader
        balance[traderAddress][_token] = balance[traderAddress][_token].sub(
            _amount.toInt256()
        );
        ...
        returns (int256 _balance)
    {
     ...
     /**
     * @notice return available collateral that is free/available  (totalDepositedCollateral - totalCollateralInPositions - totalCollateralInLiquidityPositions)
     * @param _trader The Address of the trader
     * @return _balance of the trader for that token
     */
        ...
        returns (int256 _balance)
    {
        ...
        uint256 totalUsedCollateral = IBalanceManager(balanceManager)
            .totalUsedCollateralInPositions(_trader);
    ...
    function getPositionCollateralByRatio(
        ...
        /* int256 accountValue = balance[_trader][settlementToken];
          uint256 totalUsedCollateral =IBalanceManager(balanceManager).totalUsedCollateralInPositions(_trader);
          uint256 totalUsedCollateralForOrders=IOrderManager(orderManager).getTotalCollateralForOrders(_trader);
          totalUsedCollateral=totalUsedCollateral.add(totalUsedCollateralForOrders);
          require(accountValue >= totalUsedCollateral.toInt256(),"OddzVault: Not enough collateral"); */
        require(
   ...
   /**
    ...
    /* *
     * @notice Returns how much margin is required for added liquidity
     * @param _trader The Address of the trader
     * @param _ratio initial margin ratio
     */
   /*  function _getTotalMarginRequirement(address _trader, uint24 _ratio)
        internal
        view
        returns (uint256)
    {
        uint256 totalPoolAmountValue = IBalanceManager(balanceManager)
            .getTotalAmountValue(_trader);
        return totalPoolAmountValue.mulRatio(_ratio);
    } */
}
