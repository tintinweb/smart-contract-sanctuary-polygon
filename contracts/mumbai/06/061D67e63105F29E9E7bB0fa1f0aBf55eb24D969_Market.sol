...
interface IERC721 is IERC165 {
    ...
    //PunkBidWithdrawn(tokenId, punkBids[tokenId].value, _msgSender());
}
...
contract Market is ERC721, Ownable {
    ...
    function enterBidForItem(uint256 tokenId) public payable {
        ...
        if (existing.value > 0) {
            ...
            // require(success , 'Refund the failing bid filed');
        }
    ...
    function acceptBidForPunk(uint256 tokenId, address _contract) public onlyOwnerItem(_msgSender(), tokenId,_contract){
        IERC721 buyOfContract =  CollectionsContract[_contract]; 
        require(marketPaused == false, 'Market Paused');
        ...
        emit BoughtBid(tokenId, bid.value, _msgSender(), bid.bidder , _contract);
    }
