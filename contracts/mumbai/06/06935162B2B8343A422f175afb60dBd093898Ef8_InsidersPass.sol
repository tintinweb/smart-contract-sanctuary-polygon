...
contract InsidersPass is InsidersPassAuction  {
    ...
    //[[1,1000000000000000000,[1670439600] ,100,"http://URI",0,false, 2]]
        ...
        eventIsNotPaused(ticketTypeDetails[ticketID].eventIDforTicket)
    {
        ...
        if(ticketTypeDetails[ticketID].mintable){
                 if(noOfCopies > lazyMintedTickets[account][ticketID] && lazyMintedTickets[account][ticketID] > 0)
                {   
                    require((ticketTypeDetails[ticketID].copiesSold + (noOfCopies - lazyMintedTickets[account][ticketID])) <= ticketTypeDetails[ticketID].totalCopies,"Not enough Copies available");
                     if (_msgSender() != owner())
                        require(msg.value ==(ticketTypeDetails[ticketID].ticketFixPrice) * (noOfCopies - lazyMintedTickets[account][ticketID]),"Not enough amount provided");
                    mint(account, ticketID, (lazyMintedTickets[account][ticketID] + (noOfCopies - lazyMintedTickets[account][ticketID])), ""); 
                    ticketTypeDetails[ticketID].copiesSold = ticketTypeDetails[ticketID].copiesSold + (noOfCopies - lazyMintedTickets[account][ticketID]);
                    lazyMintedTickets[account][ticketID] = 0;
                }
                 else if(lazyMintedTickets[account][ticketID] == 0){
    ...
    InsidersPassRoyalties //is SpaceERC20
{
    ...
    mapping(address => mapping(uint => uint32)) lazyMintedTickets;
    ...
    function buyPackageForFixedPrice(
        ...
        for (uint256 i = 0;i < ticketsPackage[packageID].ticketIDs.length;i++) {
            require((balanceOf(ticketsPackage[packageID].packageOwner,ticketsPackage[packageID].ticketIDs[i]) + lazyMintedTickets[ticketsPackage[packageID].packageOwner][ticketsPackage[packageID].ticketIDs[i]]) >=
    ...
    function placePackageForTimedAuction(
        ...
        require(eventDetails[_eventID].eventEndDate > block.timestamp && eventDetails[_eventID].eventEndDate > _auctionEndTime, "This event has passed... Cannot create Package or Auction End time should be before Event End time");
        for (uint256 i = 0; i < _ticketIDs.length; i++) {
            ...
            require(Ticket[_msgSender()][_ticketIDs[i]].Exists,
                "Ticket not exist"
            );
          require(
                (super.balanceOf(msg.sender, _ticketIDs[i]) -
                    CopiesOnBidOrSale[msg.sender][_ticketIDs[i]]) +  lazyMintedTickets[msg.sender][_ticketIDs[i]] >=
    ...
    function placePackageForFixedPrice(
        ...
        require(eventDetails[_eventID].eventEndDate > block.timestamp, "This event has passed... Cannot create Package");
        for (uint256 i = 0; i < _ticketIDs.length; i++) {
            // require(Ticket[owner][_ticketIDs[i]].Exists && (balanceOf(msg.sender, _ticketIDs[i]) - CopiesOnBidOrSale[msg.sender][_ticketIDs[i]]) <= ticketCopiesCount[i] &&   _eventID ==  ticketTypeDetails[_ticketIDs[i]].eventIDforTicket, "Error: Cannot Place package for Fixed Price");            
            require(
                (super.balanceOf(msg.sender, _ticketIDs[i]) -
    ...
    function addAuctionBid(
        uint256 packageID,
        uint256 _bidAmount
    ) public payable PackageExists(packageID) onAuction(ticketsPackage[packageID].packageOwner, packageID){
        require(eventDetails[ticketTypeDetails[ticketsPackage[packageID].ticketIDs[0]].eventIDforTicket].eventEndDate > block.timestamp, "This event has passed... Cannot purchase Package");
