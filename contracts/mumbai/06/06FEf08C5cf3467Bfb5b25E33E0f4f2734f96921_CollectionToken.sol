...
import "./interfaces/ICollectionToken.sol";

contract CollectionToken is
    ICollectionToken,
    IERC721Metadata,
    ERC721,
    ERC2981,
    ...
    VRFConsumerBaseV2
{
    ...
    // Chainlink parameters to request a random number.
    ...
    // The gas lane, specifies the maximum gas price to bump to.
    ...
    mapping(address => mapping(uint256 => uint256)) private _mintedBy;
    // Chainlink vrf requests (requestID => nameHash).
    mapping(uint256 => MintRequest) private _vrfRequests;
    ...
    // Random values received from Chainlink to shift on.
    mapping(uint256 => uint256) private _randomOffsets;
    ...
    event LotteryFinished(string subcollectionName);
    event MaxMintPerAddressChanged(string subcollectionName, uint16 indexed count);
    event MerkleRootChanged(string subcollectionName, bytes32 root);
    event MetadataUpdate(uint256 indexed _tokenId);
    event MintPriceChanged(string subcollectionName, uint256 indexed mintPrice);
     ...
     * @param contractMetadataURI The URI of the contract metadata.
     * @param coordinator The address of the VRF Coordinator contract.
     * @param subscriptionId The ID of the VRF subscription used by the contract.
     * @param keyHash The gas lane key hash used to generate the random number.
     */
    constructor(
        string memory name,
        ...
        bytes32 keyHash
    ) ERC721(name, symbol) VRFConsumerBaseV2(coordinator) {
    ...
    function addSubcollection(
        ...
        require(bytes(params.name).length != 0, "Missing name");
        require(bytes(params.baseURI).length != 0, "Missing base URI");
        require(bytes(params.defaultTokenURI).length != 0, "Missing default token URI");
        require(bytes(params.upgradedTokenURI).length != 0, "Missing default token URI");
        require(params.mainSupply != 0, "Invalid main supply");
        require(params.mintPrice > 0, "Invalid mint price");
        require(params.maxMintPerAddress > 0, "Invalid max mint per address");

        bytes32 subcollectionNameHash = keccak256(abi.encodePacked(params.name));
        if (subcollections.length != 0) {
            require(
                keccak256(abi.encodePacked(
                    subcollections[_subIdByNameHash[subcollectionNameHash]]
                    .params.name
                )) != subcollectionNameHash, "Duplicate name"
            );
        }
     ...
     * @param proof Proof containing elements hashed with the recipient's address,
     * to check if it is on the whitelist.
     */
    function mint(
        ...
        require(bytes(subcollection.params.name).length != 0, "Invalid collection ID");
        require(!subcollection.isRevealed, "Already revealed");
        if (!hasRole(CLAIMER_ROLE, _msgSender())) {
            require(msg.value == subcollection.params.mintPrice, "Invalid msg value");
            require(
                _mintedBy[_msgSender()][subcollectionIdx] <=
                    subcollection.params.maxMintPerAddress,
                "Max supply per address"
            );
            if (subcollection.whitelistMintEnabled) {
                require(
                    _verifyProof(
                        subcollectionName,
                        keccak256(
        ...
        require(
            subcollection.totalMinted < subcollection.params.mainSupply,
            "Subcollection is sold"
        );
        ...
        _vrfRequests[vrfRequestId] = MintRequest({
        ...
        // if (subcollection.totalMinted == subcollection.params.mainSupply) {
        //     emit SubcollectionSold(subcollectionName);
        // }
        // return tokenId;
    }
     ...
     * @param goldenTicketNumber The ID assigned to the golden token being created.
     * @param to The address that will be assigned ownership of the golden token.
     */ 
    function mintGoldenToken(
        string memory subcollectionName,
        uint256 goldenTicketNumber,
        address to
    ) external onlyRole(LOTTERY_ROLE) {
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        require(isLotteryOver(subcollectionName), "Lottery is not over");
        require(
            goldenTicketNumber <= subcollections[subcollectionIdx].params.goldSupply &&
                goldenTicketNumber > 0,
            "Invalid token ID"
        );
        uint256 endIndex = subcollections[subcollectionIdx].endIndex;
        uint256 tokenId = endIndex + goldenTicketNumber;
        _safeMint(to, tokenId);
        _subIdByTokenId[tokenId] = subcollectionIdx;
    }

    /**
     * @dev Upgrade function that sets a new tokenURI for a given NFT token.
     * @param subcollectionName subcollection name.
     * @param tokenId uint256 ID of the token to upgrade.
     * @param signature bytes signature in Hex format recovered from signed message.
     */
    function upgradeToken(
        ...
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        require(
            subcollectionIdx == getSubcollectionIdxByTokenId(tokenId),
            "Invalid subcollection"
        );
        string memory upgradedTokenURI = subcollections[subcollectionIdx].params.upgradedTokenURI;
        require(isSubcollectionRevealed(subcollectionName), "Subcollection must be revealed");
        require(bytes(_tokenURIs[tokenId]).length == 0, "Token is already upraded");
        _requireMinted(tokenId);
        ...
        require(hasRole(TRUSTED_SIGNER_ROLE, signer), "Invalid signature");
        uint256 shiftedId = _shiftId(tokenId);
        _setTokenURI(tokenId, string(abi.encodePacked(upgradedTokenURI, shiftedId.toString(), ".json")));
    ...
    /**
     * @dev Set a new mint price for the contract.
     * @param subcollectionName subcollection name.
    ...
    function setMintPrice(
        ...
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        subcollections[subcollectionIdx].params.mintPrice = newMintPrice;
        emit MintPriceChanged(subcollectionName, newMintPrice);
    }
     ...
     * @param contractMetadataURI The URI to set as the contract metadata.
     */
    function setContractURI(
        string memory contractMetadataURI
    ) external onlyRole(TRUSTED_SIGNER_ROLE) {
        _contractMetadataURI = contractMetadataURI;
        emit ContractMetadataUpdated(contractMetadataURI);
    }
    ...
    /**
     * @dev A function used to finish a lottery of a subcollection with the specified name.
     * @param subcollectionName The name of the subcollection to finish its lottery.
     */
    function finishLottery(
        string memory subcollectionName
    ) external onlyRole(LOTTERY_ROLE) {
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        subcollections[subcollectionIdx].isLotteryOver = true;
        emit LotteryFinished(subcollectionName);
    }

    /**
     * @dev Returns `_contractMetadataURI`.
     */
    ...
    function getSubcollectionIdxByName(
        ...
        require(subcollections.length != 0, "No nested collections");
        require(
            keccak256(
                abi.encodePacked(subcollections[subcollectionIdx].params.name)
            ) == subcollectionNameHash, "Invalid collection"
        );
        return _subIdByNameHash[subcollectionNameHash];
    }

    /**
     * @dev Returns true if collection had been revealed.
     * @param subcollectionName subcollection name.
     */
    function isSubcollectionRevealed(
        string memory subcollectionName
    ) public view returns (bool) {
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        return subcollections[subcollectionIdx].isRevealed;
    }

    /**
     * @dev Check if the lottery of a subcollection with the specified name is over or not.
     * @param subcollectionName The name of the subcollection to check if its lottery is over.
     * @return bool value indicating if the lottery of the specified subcollection is over or not.
     */
    function isLotteryOver(
        string memory subcollectionName
    ) public view returns (bool) {
        uint256 subcollectionIdx = getSubcollectionIdxByName(subcollectionName);
        return subcollections[subcollectionIdx].isLotteryOver;
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view override(ICollectionToken, AccessControl, ERC721, ERC2981, IERC165) returns (bool) {
        return
     ...
     * If the token URI was not set, it will return a default base token URI.
     * If the collection has not been revealed yet, it will return a default token URI.
    ...
    function tokenURI(
        uint256 tokenId
    ) public view override(ICollectionToken, IERC721Metadata, ERC721) returns (string memory) {
        uint256 subcollectionIdx = getSubcollectionIdxByTokenId(tokenId);
        Subcollection storage collection = subcollections[subcollectionIdx];
        _requireMinted(tokenId);
        string memory baseURI = collection.params.baseURI;
        string memory _tokenURI = _tokenURIs[tokenId];
        if (!collection.isRevealed) {
            return collection.params.defaultTokenURI;
        }
        if (bytes(_tokenURI).length > 0) {
            return _tokenURI;
        }
        if(tokenId <= collection.endIndex){
            tokenId = _shiftId(tokenId);
        }
        return
            bytes(collection.params.baseURI).length > 0
                ? string(abi.encodePacked(baseURI, tokenId.toString(), ".json"))
                : collection.params.defaultTokenURI;
    }
    ...
    function fulfillRandomWords(
        ...
        _mint(request.subcollectionName, request.mintTo, randomWords[0]);
    }
    ...
    /**
     * @dev Reveal the random shift value for the collection
     * @param subcollectionNameHash bytes32 subcollection name hash.
     * @param trueRandom The value returned from VRF randomness request
     * @notice This function is restricted to coordinator role only and can only be
     * executed once it sets the randomShiftValue by taking the modulo of trueRandom and
     * sets isRevealed to true. An event Revealed is emitted.
     */
    function _reveal(
        bytes32 subcollectionNameHash,
        uint256 trueRandom
    ) private {
        uint256 subcollectionIdx = _subIdByNameHash[subcollectionNameHash];
        require(!subcollections[subcollectionIdx].isRevealed, "Already revealed");
        _randomOffsets[subcollectionIdx] = trueRandom % (type(uint256).max - totalSupply);
        subcollections[subcollectionIdx].isRevealed = true;
        emit BatchMetadataUpdate(
            subcollections[subcollectionIdx].startIndex,
            subcollections[subcollectionIdx].endIndex
        );
    }

    /**
     * @dev Sets the token URI for a given `tokenId`.
     * Reverts if the token does not exist. This is useful to avoid wasting gas
     * on non-existent tokens.
     * @param tokenId uint256 ID of the token to set its URI.
     * @param _tokenURI string metadata URI to assign.
     */
    ...
    function _nextRandomId(
        ...
        uint256 random = trueRandom % range;
    ...
    /**
     * @dev Shifts the tokenId by a random value mod the range of the collection.
     * @param tokenId uint256 ID of the token to shift.
     * @return The shifted token ID.
     */
    function _shiftId(uint256 tokenId) private view returns (uint256) {
        uint256 subcollectionIdx = getSubcollectionIdxByTokenId(tokenId);
        uint256 startIndex = subcollections[subcollectionIdx].startIndex;
        uint256 endIndex = subcollections[subcollectionIdx].endIndex;
        uint256 range = endIndex - startIndex + 1;
        uint256 shiftedId = startIndex + (tokenId + _randomOffsets[subcollectionIdx]) % range;
        return shiftedId;
    }
...
interface ICollectionToken  {
    ...
    function addSubcollection(SubcollectionParams memory params) external;

    function mint(string memory subcollectionName, address to, bytes32[] memory proof) external payable;

    function setMerkleRoot(string memory subcollectionName, bytes32 root) external;

    function setMintPrice(string memory subcollectionName, uint256 newMintPrice) external;

    function setMaxMintPerAddress(string memory subcollectionName, uint16 maxMintPerAddress) external;

    function setTrustedForwarder(address forwarder) external;

    function setWhitelistMintEnabled(string memory subcollectionName, bool whitelistMintEnabled) external;

    function setBlacklist(address adrs, bool ban) external;

    function setBurnEnabled(string memory subcollectionName, bool burnEnabled) external;
    ...
    function isSubcollectionRevealed(string memory subcollectionName) external view returns (bool);

    function isLotteryOver(string memory subcollectionName) external view returns (bool);
    ...
    function finishLottery(string memory subcollectionName) external;

    function mintGoldenToken(string memory subcollectionName, uint256 goldenTicketNumber, address to) external;
