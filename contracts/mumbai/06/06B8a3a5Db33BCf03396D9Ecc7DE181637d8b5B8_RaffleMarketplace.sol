...
contract RaffleMarketplace is Ownable {
    /*
        1. Create raffle
    ...
    event RaffleCreated(uint256 indexed raffleTicker, address indexed hoster,Raffle  raffle, Stage.RaffleStage[] stages,RafflePrize[] prizes);
    ...
    event RaffleStageTicketPriceUpdated(uint256 indexed raffleTicker,Stage.StageType indexed stageType, uint256 indexed price);
    event RaffleStageTicketAvailabilityUpdated(uint256 indexed raffleTicker,Stage.StageType indexed stageType, uint256 indexed availability);
    ...
    function createRaffle(
        ...
        emit RaffleCreated(raffleTicker,msg.sender,_raffles[raffleTicker],_raffleToRaffleStages[raffleTicker],_raffleToRafflePrizes[raffleTicker]);
         ...
         RaffleContract raffle = new RaffleContract(
          ...
          block.timestamp+  raffleStruct.durationofRaffle,
           ...
           uint32(_raffleToRafflePrizes[id].length),
            _raffleToRaffleStages[id],address(0),0
    ...
    */
    function addStage(
        ...
        uint256 ticketPrice
    ) external invalidTickerId(raffleId) onlyRaffleHoster(raffleId) isRaffleNotVerified(raffleId) raffleStageNotExists(raffleId,stageType) {
        // gets the next stage id
        
        Stage.RaffleStage memory stage = Stage.RaffleStage(stageType, ticketsAvailable, ticketPrice,0);
    ...
    ) external invalidTickerId(raffleId) onlyRaffleHoster(raffleId) isRaffleNotVerified(raffleId) doesRaffleExists(raffleId) raffleStageExists(raffleId, stageType)  {
    ...
    function modifyStageTickets(
        ...
        Stage.StageType stageType,
        uint256 ticketsAvailable
    ) external invalidTickerId(raffleId) onlyRaffleHoster(raffleId) doesRaffleExists(raffleId) isRaffleNotVerified(raffleId) raffleStageExists(raffleId, stageType)  {
      Stage.RaffleStage[]  storage stages = _raffleToRaffleStages[raffleId];
        for(uint256 i=0;i<stages.length;i++){
            if(stages[i].stageType == stageType){
    ...
    function _addStageInStorage(Stage.RaffleStage[] memory _stages) internal {
         for (uint256 i = 0; i < _stages.length; i++) {
            ...
            Stage.RaffleStage(_stages[i].stageType, _stages[i].ticketsAvailable, _stages[i].ticketPrice, 0)
            );
        }
    }
    ...
    function updateWinners(uint256 id,address payable[] memory winners) external onlyRaffleContract(id) {
        _raffles[id].winners=winners;
        emit RaffleWinnersPicked(id,winners);    
    }

    function updateRaffleState(uint256 id, Stage.RaffleState  state) external onlyRaffleContract(id) {
    ...
    function getRaffleHosterById(uint256 id) external view invalidTickerId(id) doesRaffleExists(id) returns (address) {
    ...
    function getParticularRaffleStage(uint256 id,Stage.StageType stageType) external  view invalidTickerId(id) raffleStageExists(id,stageType) returns(Stage.RaffleStage memory){
    ...
    function getRaffleVerificationInfo(uint256 id) external view invalidTickerId(id) doesRaffleExists(id) returns(bool){
    ...
    modifier onlyRaffleContract(uint256 id){
        require((msg.sender==_raffles[id].raffleAddress) &&(_raffles[id].raffleAddress!=address(0)));
        _;
    }

    // same function to see if stage exists or not
    function doesRaffleStageExists(uint256 raffleId,Stage.StageType stageType) public view returns(bool) {
    ...
    function doesRafflePrizeExists(uint256 raffleId, uint256 prizeId) public  view invalidTickerId(raffleId) doesRaffleExists(raffleId) returns(bool) {
