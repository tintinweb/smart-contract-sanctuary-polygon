...
contract Mining is Ownable{
    uint private rewardPerHour;
    uint private withdrawLimit;
    address hashTokenAddress=0xD9E62cc1aE72A3C3AB4f84C2B9bB54c4a0b99e7e;

    mapping(address=>uint) public unClaimedReward;
    mapping(address=>uint) public reward;
    event Withdraw(
        address indexed withdrawingAccount,
        uint amount
    ); 
    
    event StopMining(
        address indexed stoppingAccount,
        uint amount
    ); 
    // transfer to user account
    function withdrawToAccount() public payable {
        uint _amount=unClaimedReward[msg.sender];
        require(_amount>=withdrawLimit,"Kindly mine more coins, Withdraw limit not reached.");
        unClaimedReward[msg.sender]=0;
        reward[msg.sender] += _amount;
        IndexxHash(hashTokenAddress).mint(msg.sender, _amount);
        emit Withdraw(msg.sender,_amount);
    }

    function stopMining(uint _amount) public{
        unClaimedReward[msg.sender] += _amount;
        emit StopMining(msg.sender,_amount);
    }
    // getter
    ...
    function getHashTokenAddress() public view returns(address){
        return hashTokenAddress;
    }

    // setter
    function setRewardPerHour(uint _rewardPerHour) public onlyOwner {
        rewardPerHour=_rewardPerHour;
    }

    function setWitdrawLimit(uint _withdrawLimit) public onlyOwner{
        withdrawLimit=_withdrawLimit;
    }
    function setHashTokenAddress(address _hashTokenAddress) public onlyOwner{
