...
contract RabbitNFT is ERC721Enumerable, Ownable {
  ...
  function tokenURI(uint256 tokenId) 
    ...
    returns (string memory)
    {
        ...
        string memory special = _tokenURIs[tokenId];
        ...
        if (bytes(special).length > 0) {
            return special;
        }
        // If there is no tokenURI, show baseURI instead.
        else return string(abi.encodePacked(base, tokenId.toString(), metadataFormat));
    }
  ...
  function setMaxMintPerTX(uint256 _newAmount) public onlyOwner {
    maxMintPerTX = _newAmount;
  }
  ...
  function setMetadataFormat(string memory _newFormat) public onlyOwner {
    metadataFormat = _newFormat;
  }
