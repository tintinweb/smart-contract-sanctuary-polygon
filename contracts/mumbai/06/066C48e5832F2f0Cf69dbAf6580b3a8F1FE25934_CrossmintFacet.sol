    ...
    modifier validQuantity(uint256 _id, uint256 _amount) {
        ...
        if (maxSupply > 0)
            if (
                ...
                ERC1155Storage.layout().tokenData[_id].maxSupply
...
contract CrossmintFacet is ERC1155 {
    ...
    function mint(
        ...
        if (mintType == MintType.SINGLE) {
            ERC1155Facet(address(this)).mint(account, id, amount);
        } else if (mintType == MintType.PACK) {
            ERC1155Facet(address(this)).packMint(account, id, amount);
        } else if (mintType == MintType.CROSSMINT_USDC_SINGLE) {
