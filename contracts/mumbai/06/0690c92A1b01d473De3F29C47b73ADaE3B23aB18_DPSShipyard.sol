...
contract DPSShipyard is Ownable, ReentrancyGuard {
    ...
    function claimFlagship(uint16 _dpsId) external nonReentrant {
        ...
        // if (flagship.exists(_dpsId)) revert WrongParams(2);
    ...
    function upgradeFlagship(
        ...
        // require(flagship.ownerOf(_flagshipId) == msg.sender, "Not owner");
        if (flagship.ownerOf(_flagshipId) != msg.sender) revert WrongParams(3);

        // require(_parts.length == _levels.length, "Incorrect data");
        if (_parts.length != _levels.length) revert WrongParams(3);

        uint256 doubloonPerUpgrade = gameSettings.getDoubloonPerUpgradePart();
        ...
        // require(amountOfDoubloons > 0 && doubloon.balanceOf(msg.sender) >= amountOfDoubloons, "!doubloons");
    ...
    function buySupportShips(SUPPORT_SHIP_TYPE _type, uint256 _quantity) external nonReentrant {
        ...
        supportShip.mint(msg.sender, _type, _quantity);
