...
contract Defi11Staking is AccessControl, ReentrancyGuard {
    ...
    /*
     * @notice
      rewards is 1000 because it is used to represent 0.001, since we only use integer numbers
      This will give users 0.1% reward for each staked token
     */
    ...
    mapping(uint256 => bool) perFunctionStatus; // 0-stake,1-withdraw,2-getrewards,3-withdrawRewardToken
    ...
    modifier onlyOwner() {
        require(
            owner == msg.sender && hasRole(REWARDS_ROLE, msg.sender),
    ...
    modifier pausedStatus(uint256 _index) {
        require(!perFunctionStatus[_index], "Function is paused");
        _;
    }

    modifier rewardStatus() {
        require(!rewardSystemStatus, "Rewards system is paused");
        _;
    }
    ...
    constructor(
        ...
        require(
            _token != address(0) && _owner != address(0),
            "Invalid address"
        );
        require(
            _rewardRate > 0 && _decimals > 0 && _rewardsDuration > 0,
            "Invalid parameters"
        );
        ...
        _setupRole(REWARDS_ROLE, _owner);
    }
        ...
        updateReward(msg.sender)
    {
        ...
        require(u.amount >= _amount && _amount != 0, "Invalid amount");
        u.amount -= _amount;
    ...
    function exit() external nonReentrant whenNotPaused pausedStatus(3) {
    ...
    function pausedFunction(uint256 _index, bool _status)
        external
        nonReentrant
        onlyOwner
    {
    ...
    function rewardSystemPaused(bool _status) external nonReentrant onlyOwner {
        require(rewardSystemStatus != _status, "different value required");

        if (lastUpdatedDuration == 0) {
            require(_status, "for first time");
        }
    ...
    function earned(
        ...
        uint256 timeDiff = _updateTime - _sinceTime;
        uint256 totalAmount = (userInfo[_account].amount *
            rewardRate *
            timeDiff);

        uint256 currentReward = ((timeDiff * totalAmount) /
            (decimals * rewardsDuration));
    ...
    function updatedEarned(address _account) public view returns (uint256) {
        ...
        uint256 since = userInfo[_account].since;

        if (!rewardSystemStatus && since > currentUpdatedDuration) {
            claimRewards = earned(_account, block.timestamp, since);
        } else if (rewardSystemStatus) {
            claimRewards = earned(_account, lastUpdatedDuration, since);
        } else {
            if (since > lastUpdatedDuration) {
                claimRewards =
                    ((block.timestamp - currentUpdatedDuration) *
                        rewardRate *
                        userInfo[_account].amount) /
                    (rewardsDuration * decimals);
            } else {
                claimRewards =
                    earned(_account, lastUpdatedDuration, since) +
                    ((block.timestamp - currentUpdatedDuration) *
                        rewardRate *
                        userInfo[_account].amount) /
                    (rewardsDuration * decimals);
