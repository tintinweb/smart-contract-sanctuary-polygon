...
contract PaymentContract is Ownable2Step, EIP712, AccessControl, ReentrancyGuard{
    ...
    function cancelPayment(PaymentVoucher calldata voucher) external nonReentrant{
        ...
        require(payments[voucher.paymentId].amount > 0, "Payment does not exist");
        require(!payments[voucher.paymentId].isConfirmed, "Payment is already confirmed");
        uint256 amount = payments[voucher.paymentId].amount;
        refunds[msg.sender] += amount;
        payments[voucher.paymentId].amount = 0;
        usedSignatures[messageHash] = true;
        emit CancelEvent(voucher.receiver, msg.sender, voucher.paymentId, amount);
    }    
    ...
    function confirmPayment(PaymentVoucher calldata voucher) external nonReentrant{
        ...
        usedSignatures[messageHash] = true;
        emit ConfirmEvent( payments[voucher.paymentId].receiver, msg.sender, voucher.paymentId);
    }
    ...
    function _hash(PaymentVoucher calldata voucher) internal view returns (bytes32) {
        ...
        keccak256("PaymentVoucher(address receiver,address caller,uint256 amount,uint256 paymentId,uint256 nonce)"),
        voucher.receiver,
        voucher.caller,
        voucher.amount,
        voucher.paymentId,
    ...
    function setPaySigner(address _addr) external onlyOwner{
