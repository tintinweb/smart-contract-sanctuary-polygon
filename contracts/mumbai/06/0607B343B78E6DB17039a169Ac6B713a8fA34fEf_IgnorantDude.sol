...
contract IgnorantDude is ERC721Enumerable, Ownable, SalesActivation, ReentrancyGuard {
    ...
    function presale(uint256 _ignorantNumber) external payable isPresalesActive nonReentrant {
        ...
        for (uint i = 0; i < _ignorantNumber; i++) {
    ...
    function mint(uint256 _ignorantNumber) external payable isPublicSalesActive nonReentrant {
        uint256 supply = totalSupply();
        uint256 tokenId = 1;
        require(supply + _ignorantNumber <= max_sales_ignorant, "Exceeds maximum ignorant dude supply");
        require(tx.origin == msg.sender, "Contracts are not allowed to mint");
        require(_ignorantNumber > 0, "You cannont mint 0 ignorant dude");
        require(_ignorantNumber <= 3, "You are not allowed to buy many ignorant dude at once");
        require(msg.value >= price * _ignorantNumber, "You don't send enough eth to purchase the ignorant dude");
