...
/// -----------------------------------------------------------------------
    ...
    function createDiscountFromPrice(creationStruct memory _params) public returns(uint256 discountID){
        ...
        if(!success){
            ...
            _params.fullPrice = _bytesToUint(price);
        }else{
    ...
    function createDiscount(creationStruct memory _params) public returns(uint256 discountID){
        ...
        if(!success){
            ...
            if(msg.sender == _params.materialOwner)
                ...
                require(_params.recPayout < 10001, "Core SFT : Rec cannot take more than 100% of the discount");
            require(_params.userDiscount + _params.recPayout >= _params.recPayout, "Core SFT : Cannot give payouts if there is no discount"); //MUDAR PARA _params.userDiscount + _params.recPayout >= _params.userDiscount
            _params.fullPrice = _bytesToUint(price);
        }else{
            revert("Core SFT : Price on the material is not set yet");
        }
...
/// -----------------------------------------------------------------------
    ...
    function redeem(uint256 _tokenId,bool _bag, bytes calldata _callerParams) public  payable returns(bool){
        _checkEOA();
        ...
        return _aux.redeemJoint ? 
        _redeemFromBagJoint(_tokenId,_bag,_callerParams) :
        _redeemFromBagSeparately(_tokenId,_bag,_callerParams);
    ...
    function _redeemFromBagSeparately(uint256 _tokenId,bool _bag, bytes calldata _callerParams) internal returns(bool){
        // _checkEOA();
        // require(!_bag || bag[_tokenId] > 0, "Core SFT : There is no more bag discounts for that token");
        creationStruct memory _aux = publicSearch[_tokenId];
    ...
    function _redeemFromBagJoint(uint256 _tokenId,bool _bag, bytes calldata _callerParams) internal returns(bool){
        // _checkEOA();
        // require(!_bag || bag[_tokenId] > 0, "Core SFT : There is no more bag discounts for that token");
        creationStruct memory _aux = publicSearch[_tokenId];
...
/// -----------------------------------------------------------------------
    ...
    function setAcceptedToken(address _token) public onlyOwner {
        ...
        acceptedTokensDecimals[_token] = _bytesToUint(_data);
    }
    ...
    function setProtocolFee(uint256 _protocolFee) public onlyOwner {
        require(_protocolFee != protocolFee, "Core SFT : The protocol Fee is already in use");
...
/// -----------------------------------------------------------------------
    ...
    function withdrawProtocol(address _token) public onlyOwner{
        if(_token != address(0)){
            require(IERC20(_token).balanceOf(address(this))>0, "Core SFT : There is no withdraw balance");
...
/// -----------------------------------------------------------------------
    ...
    function _paymentHandler(creationStruct memory _aux) internal virtual returns (uint256 _initialUserDiscount){
        ...
        uint256 _protocolPayout = (_aux.fullPrice * protocolFee)/10000;
        uint256 _price = _initialUserDiscount + _creatorPayout + _protocolPayout;
    ...
    function _calculateDiscount(creationStruct memory _aux) internal view returns (uint256){
    ...
    function _redeemId(creationStruct memory _aux,bytes calldata _callerParams) internal returns (bool){
        if(_aux.funcSelector != bytes4(0)){
            ...
            uint256 _token = _bytesToUint(_callerParams);
    ...
    function _redeemIdJoint(creationStruct memory _aux,bytes calldata _callerParams) internal returns (bool){
        ...
        if(_aux.funcSelector != bytes4(0)){
            if(_aux.acceptedPayment == address(0)){
                ...
                IERC20(_aux.acceptedPayment).transferFrom(msg.sender, address(this), _price);
                require(_func(_callerParams) == _aux.funcSelector, "Core SFT : _callerParams not matching encodeWithSignature");
                IERC20(_aux.acceptedPayment).approve(_aux.material, _initialUserDiscount);
                (bool success,) = _aux.material.call(_callerParams);
                require(success,"Core SFT : Something went wrong with the redeem, please check inputs");
                IERC20(_aux.acceptedPayment).transfer(_aux.creator, _creatorPayout);
            }
...
///@dev Create a disicount with neither price nor maxDiscount only by the material owner
