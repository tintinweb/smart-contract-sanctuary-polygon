...
import './interfaces/ILaunchpadExchangeV2Router01.sol';
import './interfaces/IERC20.sol';
import './interfaces/IWETH.sol';

contract LaunchpadExchangeV2Router01 is ILaunchpadExchangeV2Router01 {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = LaunchpadExchangeV2Library.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = LaunchpadExchangeV2Library.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'LaunchpadExchangeV2Router: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = LaunchpadExchangeV2Library.quote(amountBDesired, reserveB, reserveA);
                assert(amountAOptimal <= amountADesired);
                require(amountAOptimal >= amountAMin, 'LaunchpadExchangeV2Router: INSUFFICIENT_A_AMOUNT');
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = ILaunchpadExchangeV2Pair(pair).burn(to);
    ...
    function removeLiquidityWithPermit(
        ...
        ILaunchpadExchangeV2Pair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) private {
        for (uint i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2 ? LaunchpadExchangeV2Library.pairFor(factory, output, path[i + 2]) : _to;
            ILaunchpadExchangeV2Pair(LaunchpadExchangeV2Library.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to, new bytes(0));
        }
    }
    function swapExactTokensForTokens(
        uint amountIn,
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'LaunchpadExchangeV2Router: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, LaunchpadExchangeV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
    ...
    function swapTokensForExactTokens(
        ...
        require(amounts[0] <= amountInMax, 'LaunchpadExchangeV2Router: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(path[0], msg.sender, LaunchpadExchangeV2Library.pairFor(factory, path[0], path[1]), amounts[0]);
        _swap(amounts, path, to);
    }
    function swapExactETHForTokens(uint amountOutMin, address[] calldata path, address to, uint deadline)
        ...
        returns (uint[] memory amounts)
    {
        ...
        assert(IWETH(WETH).transfer(LaunchpadExchangeV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    function swapTokensForExactETH(uint amountOut, uint amountInMax, address[] calldata path, address to, uint deadline)
        external
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) public pure override returns (uint amountIn) {
        return LaunchpadExchangeV2Library.getAmountOut(amountOut, reserveIn, reserveOut);
    }
