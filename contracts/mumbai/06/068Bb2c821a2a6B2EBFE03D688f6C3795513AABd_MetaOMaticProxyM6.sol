...
interface IPOLYCONTRACTM6PROXYA {
    function _PlaceInM610Tree(address user,address referrer) external returns (bool);
    function _PlaceInM620Tree(address user,address referrer) external returns (bool);
}

interface IPOLYCONTRACTM6PROXYB {
    function _PlaceInM640Tree(address user,address referrer) external returns (bool);
    function _PlaceInM680Tree(address user,address referrer) external returns (bool);
}

interface IPOLYCONTRACTM6PROXYC {
    function _PlaceInM6160Tree(address user,address referrer) external returns (bool);
    function _PlaceInM6320Tree(address user,address referrer) external returns (bool);
}

contract MetaOMaticProxyM6 is IPOLYCONTRACTM6 {
    ...
    uint256[6] public packagePrice = [10 ether,20 ether,40 ether,80 ether,160 ether,320 ether];

    struct SystemM6Details {
        uint256 totalM610Bonus;
        uint256 totalM620Bonus;
        uint256 totalM640Bonus;
        uint256 totalM680Bonus;
        uint256 totalM6160Bonus;
        uint256 totalM6320Bonus;
    }
    ...
    constructor() {
      contractOwner=0xDf8dDF6AD357cafFCfA07d8405277952d050b050;
    }
    ...
    function placeInM6(address user,address referrer,uint package) public override returns (bool) {
      if(package==0){
        m6proxya._PlaceInM610Tree(user,referrer);
      }
      else if(package==1){
        m6proxya._PlaceInM620Tree(user,referrer);
      }
      else if(package==2){
        m6proxyb._PlaceInM640Tree(user,referrer);
      }
      else if(package==3){
        m6proxyb._PlaceInM680Tree(user,referrer);
      }
      else if(package==4){
        m6proxyc._PlaceInM6160Tree(user,referrer);
      }
      else if(package==5){
        m6proxyc._PlaceInM6320Tree(user,referrer);
      }
      return true;
    }
    ...
    function creditM6Income(address user,uint package,uint level,uint IsFreshId) public override returns (bool) {
      if(IsFreshId==1){
        ...
        if(package==0){
          _UserM6BonusDetails[user].totalM610Bonus += amount;
          _SystemM6Details[0].totalM610Bonus += amount;
        }
        else if(package==1){
          _UserM6BonusDetails[user].totalM620Bonus += amount;
          _SystemM6Details[0].totalM620Bonus += amount;
        }
        else if(package==2){
          _UserM6BonusDetails[user].totalM640Bonus += amount;
          _SystemM6Details[0].totalM640Bonus += amount;
        }
        else if(package==3){
           _UserM6BonusDetails[user].totalM680Bonus += amount;
          _SystemM6Details[0].totalM680Bonus += amount;
        }
        else if(package==4){
          _UserM6BonusDetails[user].totalM6160Bonus += amount;
          _SystemM6Details[0].totalM6160Bonus += amount;
        }
        else if(package==5){
          _UserM6BonusDetails[user].totalM6320Bonus += amount;
          _SystemM6Details[0].totalM6320Bonus += amount;
        }
