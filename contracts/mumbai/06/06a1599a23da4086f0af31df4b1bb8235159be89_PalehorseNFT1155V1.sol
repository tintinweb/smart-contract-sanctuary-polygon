...
contract PalehorseNFT1155V1 is ERC1155 {
    ...
    modifier whitelistedOnly(){
        require(whitelistedUsers[msg.sender], "PalehorseNFT721V1: Whitelisted users only.");
        _;
    }

    function whitelistUsers(address[] memory users) onlyOwner external{
        for (uint256 index = 0; index < users.length; index++) {
            whitelistedUsers[users[index]] = true;
        }
    }

    function removeWhitelistUsers(address[] memory users) onlyOwner external{
        for (uint256 index = 0; index < users.length; index++) {
    ...
    */
    ...
    function mint(string memory uri, uint256 supply, uint256 fee, Sign memory sign)  whitelistedOnly public {
        verifySign(uri, sign);
