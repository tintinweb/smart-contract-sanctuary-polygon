...
//E1155002 : Functionality is paused
//E1155003 : Must be > 0
...
contract ERC1155NFT is ERC1155Supply, ContextMixin, Whitelist {
  ...
  /**
   * @notice enables whitelisting
   *
   * @param value true/false which determines if whitelisting is enabled or not
   */
  function whiteList(bool value) public onlyAdmin {
    super.enableWhitelisting(value);
  }
  ...
  /**
   * @dev Throws if called when specific contract functionalities are paused by the admin.
   */
  modifier whenNotPaused() {
    require(!paused || isAdmin(_msgSender()), 'E1155002');
    _;
  }
  ...
  /**
   * @notice function to check if whitelisting is active & if the user is whitelisted
   */
  function checkwhiteListing(address _sender) internal view returns (bool) {
    return isWhitelistingActive() && isWhitelisted(_sender);
  }

  /**
   * @notice function to pause specific functionalities within the contract
   */
  function pauseContractMethods(bool _value) external onlyAdmin {
    paused = _value;
  }
  ...
  function buy(uint256 _tokenId, uint256 _amount) public payable whenNotPaused {
    require(checkwhiteListing(msg.sender) || selfMint, 'E1155008');
    require(
      (maxPerUser > 0 &&
  ...
  /**
   * @notice function to mint tokens in batches
   *
   * @param _to wallet to which the tokens will be minted
   * @param _tokenIds array of id's assigned to each minted token
   * @param _amounts number of tokens to be minted for each id
   */
  function batchMint(
    address _to,
    uint256[] calldata _tokenIds,
  ...
  /**
   * @notice function to update multiple tokens with its corresponding metadata in batches
   * @dev can be used to update metadata for multiple tokens with single metadata
   *
   * @param _tokenIds array of id's whose metadata will be updated
   * @param _metadata array of metadata which will be assigned to the respective tokens
   */
  function batchSetMetadata(
    uint256[] calldata _tokenIds,
    string[] memory _metadata
  ) public onlyAdmin {
    require(_tokenIds.length > 0 && _metadata.length > 0, 'E1155009');
    if (_metadata.length == _tokenIds.length) {
      for (uint256 i = 0; i < _tokenIds.length; i++) {
        setTokenMetadata(_tokenIds[i], _metadata[i]);
      }
    }
    if (_tokenIds.length > 1 && _metadata.length == 1) {
      for (uint256 i = 0; i < _tokenIds.length; i++) {
        setTokenMetadata(_tokenIds[i], _metadata[0]);
      }
    }
  }
  ...
  /**
   * @notice function to transfer tokens in batches
   *
   * @param _from wallet from which tokens will be transferred/debited
   * @param _to wallet to which tokens will be credited
   * @param _tokenIds array of id's which will be transferred
   * @param _amounts number of tokens to be transfered for each id
   */
  function batchTransfer(
    ...
    uint256[] calldata _tokenIds,
    uint256[] calldata _amounts
  ) external virtual whenNotPaused isTransferable {
    require(
      _from == _msgSender() ||
        _from == msg.sender ||
        isApprovedForAll(msg.sender, _msgSender()),
  ...
  function isApprovedForAll(
    address _owner,
    address _operator
  ) public view override whenNotPaused returns (bool isOperator) {
