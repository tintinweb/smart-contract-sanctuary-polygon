...
// File: contracts/TrashPandazClubSteakingV3.sol


/* TrashPandazClub Steaking V3 🥩 */
...
contract TrashPandazClubSteakingV3 is IERC721Receiver, ReentrancyGuard, Ownable, Pausable {
    ...
    bool public trashlordsOnly = true;
    bool public rewardDividedByPool = true;
    bool public stackMultipliers = false;
    uint256 public rewardRate = 100; //Trash / Day
    uint256 public maxWithdrawPercentageOfContract = 50; // 50 = 0.5%, 500 = 5%, 2000 = 20%, etc. PER NFT 
    ...
    function stake(uint256[] calldata tokenIds) external nonReentrant whenNotPaused{
        uint256 tokenId;
        if(trashlordsOnly){
            if(walletHoldsTrashLord(msg.sender)){
    ...
    function rewardMath(address account, uint256 stakedAt) internal view returns(uint256){
        if (rewardDividedByPool == true){
            return (( rewardRate * getMultiplier(account) * 1 ether / 100 * (block.timestamp - stakedAt) / 86400 ) ) / totalStaked; // why divide by 100?
        }
        else {
            return ( rewardRate * getMultiplier(account) * 1 ether / 100 * (block.timestamp - stakedAt) / 86400 ) ; 
        }
    ...
    function getMultiplier(address _account) public view returns (uint256){ 
        ...
        if( stackMultipliers == true ){
            for (uint i = 0; i < SM_Contracts.length; i++) {
                //get number of tokens in wallet for each contract
    ...
    function walletHoldsTrashLord(address _account) internal view returns (bool) {
        address _c_addr = 0x2953399124F0cBB46d2CbACD8A89cF0599974963;
        uint256 _t_id = 2028626844729816297473413048497949099433804772995310846604280455040849674265;
        IERC1155 token1155 = IERC1155(_c_addr);
        return token1155.balanceOf(_account, _t_id) > 0; 
    }
    ...
    function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal nonReentrant{
        ...
        uint256 maxWithdraw = bal * maxWithdrawPercentageOfContract/10000 ; // set max withdraw to % of contract.
    ...
    function viewTrashLordsOnly() public view returns(bool){
        return trashlordsOnly;
    }

    function viewRewardDividedByPool() public view returns(bool){
        return rewardDividedByPool;
    }
    ...
    function removeSM(uint _index) external onlyOwner{
        ...
        for (uint i = _index; i < SM_Contracts.length - 1; i++) {
    ...
    function setSM(address _contract, uint256 _tokenId, uint256 _stakingMultiplier) external onlyOwner{
        SM_Contracts.push(_contract);
        SM_TokenIds.push(_tokenId);
        SM_Multipliers.push(_stakingMultiplier);
    }

    function setTrashLordsOnly(bool _bool) external onlyOwner{
        trashlordsOnly = _bool;
    }

    function setDividedByPool(bool _bool) external onlyOwner{
        rewardDividedByPool = _bool;
    }
...
// allow multipliers / check to make sure nothing bad happens when multiplying nothing in the array (probably not necessary for this check)?

// maxpercentage per nft instead of entire earnings so people cant withdraw max multiple times.