...
library LibDataTypes {
     ...
     * @param POLYWRAP Use off-chain data as condition & input data for execution. {See PolywrapModule.sol}
     */
    enum Module {
        RESOLVER,
        TIME,
...
contract PolywrapModule is TaskModuleBase {
    /**
     * @notice Helper function to encode arguments for PolywrapModule.
     *
     * @param _polywrapCid Ipfs hash of polywrap resolver.
     * @param _userArgsHex User arguments for polywrap resolver in hexadecimal form.
     */
    function encodeModuleArg(
        string memory _polywrapCid,
        bytes calldata _userArgsHex
    ) external pure returns (bytes memory) {
        return abi.encode(_polywrapCid, _userArgsHex);
    }
}
