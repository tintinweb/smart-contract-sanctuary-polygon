    ...
    BaseRelayRecipient
{
    ...
    mapping(uint256 => KeyDetails) NFTLogs;

    mapping(address => bool) public claimedWallets;

    event Mint(uint256 tokenId, address tokenOwner, KeyDetails newNFT);

    event MintBatch(uint256 tokenId, address tokenOwner, KeyDetails newNFT);
    ...
    // Mint Methods
    function mintKey() public virtual mintCompliance(_msgSender()) {
        _mint(_msgSender(), nextTokenId, 1, "");

        KeyDetails memory newNft = KeyDetails({
            id: nextTokenId,
            creator: _msgSender(),
            tokenUri: uri(nextTokenId),
    ...
    function setMaxSupply(uint256 supply) public virtual {
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender),
            "KeyDetails: Must have admin role to change"
        );
        maxSupply = supply;
    }
    ...
    // func to get tokens in circulation
    function checkWalletEligibility(address _address)
        public
        view
        returns (bool)
    {
