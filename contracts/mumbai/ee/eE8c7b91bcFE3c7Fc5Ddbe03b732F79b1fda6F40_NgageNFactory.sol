...
contract NgageNFactory is AccessControl, ERC2771Context {
  ...
  mapping(string => bool) public factoryCreated;
  mapping(string => address) public factoryOwner;
  ...
  /**
   * @dev Public function to create a new NgageN (proxy) contract
   ...
   * @param _factoryName The unique name of factory
  ...
  function createFactory(
    ...
    require(!factoryCreated[_factoryName], 'NF:Factory exists');
    require(_owner != _msgSender(), 'NF:Contract owner cant be proxy admin');
    {
      TransparentUpgradeableProxy proxy = new TransparentUpgradeableProxy(
        ...
        abi.encodeWithSelector(
          NgageN(address(0)).initialize.selector,
          _uri,
          _contractUri,
          _royaltyRecipient,
      ...
      Administrator admn = new Administrator(
        INgageN(address(proxy)),
        minimalForwarder
      );
      MintNFT mintNFT = new MintNFT(INgageN(address(proxy)), customForwarder);
      NgageN(address(proxy)).grantRole(DEFAULT_ADMIN_ROLE, _owner);
      admn.grantRole(DEFAULT_ADMIN_ROLE, _owner);
      mintNFT.grantRole(DEFAULT_ADMIN_ROLE, _owner);
      mintNFT.grantRole(CREATOR, _assetCreator);
      factories[_factoryName] = Factory(
        _factoryName,
        _owner,
        address(proxy),
        address(admn),
      ...
      factoryOwner[_factoryName] = _owner;
      factoryCreated[_factoryName] = true;
    }
  }

  function getFactoryData(string memory _factoryName)
    public
    view
    returns (
      address,
  ...
  {
    return (
      factories[_factoryName]._ngageN,
      factories[_factoryName]._administrator,
      factories[_factoryName]._mintNFT,
      customForwarder,
      factories[_factoryName]._assetCreator
    );
  }

  /**
   * @dev Public function to set new ngageN contract address
   * Reverts if the caller is not an admin
   * @param _ngageN The address that will be set as new ngageN contract
   */
  function setNewNgageN(address _ngageN) public onlyAdmin {
    require(_ngageN != address(0), 'NF: Invalid address');
    ngageN = _ngageN;
  }
...
contract NgageN is AccessControlUpgradeable, ERC1155Upgradeable, ERC2981Base {
  ...
  function uri(uint256 id) public view override returns (string memory) {
    return string(abi.encodePacked(super.uri(id), tokenUris[id]));
  }
  ...
  function setApprovalForToken(
    ...
    require(balanceOf(account, id) > 0, 'NN: approver must own the token');
...
contract MintNFT is AccessControl, ERC2771Context {
  ...
  function mintAndApproveNFT(
    ...
    require(
      ...
      'MintNFT: Only CREATOR with valid signature'
    );
    ngagenAddress.mintNFT(to, id, amount, data, tokenUri);
    if (!ngagenAddress.isApprovedForToken(to, operator, id)) {
      ngagenAddress.setApprovalForToken(to, operator, id, true);
    }
  }
