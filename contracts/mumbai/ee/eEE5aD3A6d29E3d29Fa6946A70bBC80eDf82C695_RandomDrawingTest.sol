    ...
    IERC1155Receiver
{
    ...
    function _removeContestant(address _contestant) internal {
    ...
    function _selectNextWinner(uint256 _randomNumber) internal {
...
// File: MockRandomDrawing.sol

contract MockRandomDrawing is RandomDrawing {
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableUint256Set for EnumerableUint256Set.Uint256Set;
    ...
    constructor(
        ...
        uint256 _fee,
        bytes32 _keyhash
    ) RandomDrawing(_vrfCoordinator, _link, _fee, _keyhash) {}

    function getEligibleContestantAt(uint256 _index) public view returns (address) {
        return eligibleContestants.at(_index);
    }

    function getEligibleNftAt(uint256 _index) public view returns (uint256) {
        return nftDrawingPool.at(_index);
    }

    function selectNextWinner(uint256 _randomNumber) public {
        require(factoryState == STATE.COMPLETE, "Must 'drawNumber' first");
        _selectNextWinner(_randomNumber);
    }
}
// File: RandomDrawingTest.sol

contract RandomDrawingTest is MockRandomDrawing {
	IERC721Enumerable public constant HODLERS = IERC721Enumerable(
		address(0xB464e7Ad9aEfAeB0b60a3A5978B9369a66FFBA8C)
	);

	constructor() MockRandomDrawing(
        address(0x8C7382F9D8f56b33781fE506E897a4F1e2d17255), // vrfCoordinator
        address(0x326C977E6efc84E512bB9C30f76E30c160eD06FB), // LINK token address
        100000000000000, // fee (0.0001 LINK)
        0x6e75b569a01ef56d18cab6a8e71e6600d6ce853834d4a5748b720d06f878b3a4 // keyhash
    ) {
    	// employees and partners are not eligible
    	setIneligible(address(0xCba12e5624f16f2c2cE4fFE8c92Dd7f15b6bB21d));
    	setIneligible(address(0x09C1602a9D0de03C393d027a15A31b5e9d88ea63));
    }
    
    function loadTestContestants() public onlyOwnerOrRole(OOMPA_LOOMPA) {
    	loadContestants(HODLERS);
    }
}