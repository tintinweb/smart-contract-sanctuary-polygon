...
import "./XSeedToken.sol";
import "hardhat/console.sol";

contract TokenVesting is Ownable {
    uint256 public totalVestings;
    XSeedToken public ERC20Interface;
    ...
    constructor(address _tokenAddress) {
        ...
        ERC20Interface = XSeedToken(_tokenAddress);
    ...
    function createVesting(
        ...
        ERC20Interface.contractMint(_mintAddress, _amount);
        return true;
    }
        ...
        returns (bool)
    {
        ...
        require(msg.sender == vestingDetail.receiver, "You are not the Receiver");
        require(!vestingDetail.expired, "ID expired");
        vestingID[id].expired = true;
        ERC20Interface.approve(vestingDetail.mintAddress, vestingDetail.amount);
        ERC20Interface.transferFrom(
            vestingDetail.mintAddress,
            vestingDetail.receiver,
            vestingDetail.amount
    ...
    function createMultipleVesting(
        ...
        require(
            _receivers.length == _amounts.length &&
                _amounts.length == _releases.length && _mintAddresses.length == _releases.length,
            "Invalid data"
        );
    ...
    {
        VestingDetails[] memory res = new VestingDetails[](receiverIDs[user].length);
        for (uint256 i = 0; i < receiverIDs[user].length; i++) {
            uint256 index = receiverIDs[user][i];
            res[i] = VestingDetails(
                vestingID[index].receiver,
                vestingID[index].mintAddress,
                vestingID[index].release,
                vestingID[index].amount,
                vestingID[index].expired
            );
        }
        return res;
    }
...
contract XSeedToken is ERC20Capped, ERC20Mintable, ERC20Burnable {
    string constant t_name = "MetaXSeed";
    string constant t_symbol = "XSEED";
    uint256 constant t_cap = 200000000000000000000000000;
    uint256 constant t_initialBalance = 200000000000000000000000000;
    uint8 constant t_decimals = 18;

    address public vestingContract = address(0);
    mapping (address => bool) public holders;
    uint8 public holderCount;
    ...
    function setVestingContractAddress(address account) public onlyOwner {
        if(vestingContract == address(0))
            vestingContract = account;
    }

    function contractMint(address account, uint256 amount) public {
        require(msg.sender == vestingContract, "This is not vesting contract!");
        super._mint(account, amount);
    }
    ...
    function transferFrom(address sender, address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        if(address(msg.sender) == vestingContract) {
            _approve(sender, vestingContract, amount);
        }
    ...
    function transfer(address recipient, uint256 amount) public virtual override returns (bool) {
        ...
        addHolders(recipient);
        addHolders(address(msg.sender));
        return true;
    }

    function addHolders(address account) internal {
