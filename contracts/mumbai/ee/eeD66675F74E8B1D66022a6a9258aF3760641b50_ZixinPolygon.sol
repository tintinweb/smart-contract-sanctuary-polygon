...
contract ZixinPolygon is ERC721, ERC721URIStorage, IDapp, FunctionsClient, ConfirmedOwner {
  ...
  function claimZixin(
    ...
    bytes32 requestId = _executeRequest(zixinToSourceCode[zixinId], secrets, args, subscriptionId, gasLimit);
  ...
  function setContractOnChain(string calldata chainId, string calldata contractAddress) external onlyOwner {
  ...
  function transferCrossChain(
    ...
    require(
      keccak256(abi.encodePacked(ourContractOnChains[destChainId])) != keccak256(abi.encodePacked("")),
      "contract on dest not set"
    );

    for (uint256 i = 0; i < transferParams.zixinIds.length; i++) {
      require(transferParams.zixinIds[i] < _zixinIds.current(), "Zixin does not exist");
      require(zixinToTokenId[transferParams.zixinIds[i]][msg.sender] != 0, "Zixin unavailable");
      transferParams.metadataUrls[i] = bytes(tokenURI(zixinToTokenId[transferParams.zixinIds[i]][msg.sender]));
    }
