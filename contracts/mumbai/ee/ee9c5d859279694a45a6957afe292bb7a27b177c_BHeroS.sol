...
contract BHeroS is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  event UpgradeShieldLevel(address indexed owner, uint256 idHeroS, uint256 numRock);
  ...
  function upgradeShieldLevel(uint256 idHeroS, uint256 numRock) external {
    address owner = msg.sender;
    uint256 rarity;
    uint256 level;
    uint256 detailHeroS;
    ...
    emit UpgradeShieldLevel(owner, idHeroS, numRock);
  }
...
contract BHeroToken is ERC721Upgradeable, AccessControlUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function upgrade() public pure returns(uint) {
    return 1;
  }
}
