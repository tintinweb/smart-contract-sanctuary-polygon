...
contract CRPoolProxy is Ownable, ERC1155Holder {
    ...
    function getUpdateWeightCost(
        ...
        return
            SmartPoolManager.getUpdateWeightCost(
                IConfigurableRightsPool(address(_pool)),
                _pool.bPool(),
                _token,
                _newWeight
            );
    ...
    function getUpdateWeightEarnings(
        ...
        return
            SmartPoolManager.getUpdateWeightEarnings(
                IConfigurableRightsPool(address(_pool)),
                _pool.bPool(),
                _token,
                _newWeight
    ...
    function updateWeight(
        ...
        uint256 cost = getUpdateWeightCost(ICRPool(_pool), _token, _newWeight);

        // In order to keep the pool stable
        // - Provide xLPT to the pool if reducing weight
        // - provide xToken if increasing weight
        if (_newWeight < currentWeight) {
            _pullXLptToPool(ICRPool(_pool), cost);
        } else {
            _pullXTokenToPool(ICRPool(_pool), IXToken(_token), cost);
        }
     ...
     */
    function getRemoveTokenCost(ICRPool _pool, address _token) public view returns (uint256) {
        return SmartPoolManager.getRemoveTokenCost(IConfigurableRightsPool(address(_pool)), _pool.bPool(), _token);
    }

    /**
     * @notice Computes the earnings from removing a token from the pool
     */
    function getRemoveTokenEarnings(ICRPool _pool, address _token) external view returns (uint256) {
        return SmartPoolManager.getRemoveTokenEarnings(IConfigurableRightsPool(address(_pool)), _pool.bPool(), _token);
    }

    function removeToken(address _pool, address _token) external {
        require(ICRPool(_pool).owner() == _msgSender(), "ONLY_POOL_OWNER");

        // Pull the xLPT cost from the sender
        uint256 xLPTCost = getRemoveTokenCost(ICRPool(_pool), _token);
        _pullXLptToPool(ICRPool(_pool), xLPTCost);
     ...
     * @param _minAmountsOut Minimum amounts out.
     */
    function exitPool(
        address _pool,
     ...
     * @param _minPoolAmountOut Minimum amount of pool shares to get.
     */
     ...
     * @param _maxAmountIn Minimum amount of tokenIn to pay.
     */
     ...
     * @param _minAmountOut Minimum amount of tokenIn to get.
     */
    ...
    function _pullXTokenToPool(
        ...
        _transferFromSenderToContract(_xToken, _amount);

        // Approve the pool to spend the xToken
        _xToken.approve(address(_pool), _amount);
    }
    ...
    function _pushXTokenToSender(IXToken _xToken) internal {
        // Send all the XToken to the sender
        _transferFromContractToSender(_xToken, _getContractBalance(_xToken));
    }
