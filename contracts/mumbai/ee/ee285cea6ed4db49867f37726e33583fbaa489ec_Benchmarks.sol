...
contract Benchmarks is IBenchmarks {
    ...
    /// @notice The validity period increases by one second every VALIDITY_PERIOD_SCALE
    /// @notice after the request timestamp.
    uint256 public constant VALIDITY_PERIOD_SCALE = 1 minutes;

    /// @notice The maximum allowed difference between the request timestamp and the price data publish time
    uint256 public constant VALIDITY_PERIOD_MAX = 5 seconds;

    /// @notice Prices are scaled to DECIMALS decimals before being recorded
    uint256 public constant DECIMALS = 18;

    /// @notice Pyth contract
    IPyth public immutable pyth;

    /// @notice Recorded prices
    ...
    /// @return benchmarkId - Benchmark ID
    ...
    /// @notice Returns the fee required to verify and decode benchmark price data
    ...
    /// @notice Records a benchmark price, first validating the signature of
    /// @notice the price data on the Pyth contract.
    /// @notice Benchmark prices are scaled to 18 decimals. There may be loss
    /// @notice of precision if the price data is using more than 18 decimals.
    /// @param _priceId   - Pyth price ID
    /// @param _timestamp - Request timestamp
    ...
    /// @notice Submits price data to the Pyth contract for signature
    /// @notice verification and returns the decoded data.
    /// @param _priceId     - Pyth price ID
    /// @param _timestamp   - request timestamp
    /// @param _priceData   - the signed price data
    ...
    /// @notice Computes the maximum allowed value of the publish time of price data,
    /// @notice given the request timestamp and the current block timestamp.
    /// @notice The maximum publish time increases linearly as a function of block.timestamp.
    /// @notice It starts at the request timestamp and increases by one second every
    /// @notice VALIDITY_PERIOD_SCALE until it hits a maximum of VALIDITY_PERIOD_MAX.
    /// @param _timestamp      - Request timestamp
    /// @return maxPublishTime - Maximum value of the publish time
    function _computeMaxPublishTime(uint256 _timestamp) internal view returns (uint64) {
        if (block.timestamp < _timestamp) {
            return uint64(_timestamp);
        }

        // the validity period increases by 1 second every VALIDITY_PERIOD_SCALE
        // does not exceed VALIDITY_PERIOD_MAX
        uint64 validityPeriod = uint64(
            _min((block.timestamp - _timestamp) / VALIDITY_PERIOD_SCALE, VALIDITY_PERIOD_MAX)
        );

        // add validity period to request timestamp
    ...
    /// @return minimum - Minimum of _a and _b
    ...
    /// @param _decimals - Decimal precision of the original price
    /// @return rescaledPrice    - Price rescaled to internal decimal precision
