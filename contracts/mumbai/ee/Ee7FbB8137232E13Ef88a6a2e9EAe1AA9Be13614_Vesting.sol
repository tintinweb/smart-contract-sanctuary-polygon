    ...
    PausableUpgradeable
{
    ...
    struct Scheme {
        ...
        /// @custom:oz-upgrades-renamed-from vestingPeriod
    ...
    uint256 public secondsOfDay; //@dev epoch time and in seconds
    uint256 public tge;
    ...
    mapping(address => bool) private _wallets; // Unused
    mapping(address => uint256[]) private _totalSubscriptionsByWallet;

    address public emergencyWallet;
    address public upgradeWallet;

    function initialize(
        address gameStateToken,
        address upgradeWallet_,
        address emergencyWallet_,
    ...
    function addScheme(
        ...
        scheme.vestingPeriod = vestingFrequency * secondsOfDay;
        ...
        subcriptionExist(subscriptionId)
    {
        ...
        uint256 _completedSub = _claim(subscription, subscriptionId, block.timestamp);
    ...
    function isWalletOnVesting(address wallet) public view returns (bool onActiveVesting) {
        if(_totalSubscriptionsByWallet[wallet].length > 0) {
            Subscription memory activeSub;
            for(uint256 i = 0; i < _totalSubscriptionsByWallet[wallet].length; i++) {

                //get subscription by subscriptionId from subscription list
                activeSub = _subscriptions[_totalSubscriptionsByWallet[wallet][i]];
                if(activeSub.isActive) {
                    onActiveVesting = true;
                    break;
                }
            }
        }
        ...
        )
    {
        ...
        vestingFrequency = scheme.vestingPeriod / secondsOfDay;
    ...
    function setEmergencyWallet(address _emergencyWallet) public onlyOwner {
        require(_emergencyWallet != address(0), "Setting Emergency wallet to the zero-address");
        emergencyWallet = _emergencyWallet;
    }

    function setUpgradeWallet(address wallet)
        public
        onlyOwner
    {
        require(wallet != address(0), "Setting upgrade wallet to the zero-address");
    ...
    function _claim(Subscription storage subscription, uint256 subscriptionId, uint256 blockTime)
        internal
    ...
    function _onlyUpgradeWalletOrOwner() private view {
        require(
            _msgSender() == upgradeWallet || owner() == _msgSender(),
    ...
    function _subscriptionExist(uint256 subscriptionId) private view {
        require(
            ...
            "Subscription does not exist or is already finished"
        );
    }
