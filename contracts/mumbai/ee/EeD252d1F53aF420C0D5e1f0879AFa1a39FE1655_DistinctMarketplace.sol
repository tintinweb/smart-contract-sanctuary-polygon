    ...
    Whitelist
{
    ...
    function _transferTokenBackToSeller(uint256 saleId) internal {
        ...
        uint256[] memory tokenIds = drop.tokenIds;
        uint256 tokenIdsArraylength = tokenIds.length;
        uint256[] memory amounts = new uint256[](tokenIdsArraylength);

        for (uint256 i = 0; i < tokenIdsArraylength; i++) {
            amounts[i] = drop.dropItems[tokenIds[i]].amount;
            drop.dropItems[tokenIds[i]].amount = 0;
        }
...
contract Whitelist is Initializable, ContextUpgradeable {
    ...
    /**
     * @notice returns if givent user is whitelisted for given sale id.
     */
...
import "./DistinctNftNode.sol";
import "./DistinctMarketFee.sol";
import "./DistinctTreasuryNode.sol";

abstract contract AdminAuth is
    DistinctNftNode,
    DistinctRole,
    DistinctMarketFee,
    DistinctTreasuryNode
{
    ...
    function _updateProperties(Functionalities functionality, bool value)
        internal
    {
        isAllowed[functionality] = value;
    }

    function getFunctionalities() external view returns (bool updateSale) {
        return (isAllowed[Functionalities.UpdateSale]);
    }
    ...
    function adminUpdateFunctionalityPermission(
...
abstract contract DistinctRole is ContextUpgradeable, DistinctNftNode {
    ...
    function _isDistinctOperator() internal view returns (bool) {
        return IDistinctRoleManager(getDistinctNft()).isCreator(_msgSender());
    }
}
