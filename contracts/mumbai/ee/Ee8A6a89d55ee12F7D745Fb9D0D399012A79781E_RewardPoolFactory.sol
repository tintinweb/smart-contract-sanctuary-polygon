...
contract RewardPoolFactory {
    ...
    function createRewardPool(uint256 _rewardPoolId) public payable {
        require(msg.value > 0, "Value must be greater than 0");
        RewardPool newPool = (new RewardPool){value: msg.value}(_rewardPoolId);
        emit RewardPoolCreated(address(newPool));
        rewardPools.push(address(newPool));
    }
}

contract RewardPool is ChainlinkClient {
    uint256 public rewardPoolId; // Added rewardPoolId as a read-only property

    receive() external payable {}

    constructor(uint256 _rewardPoolId) payable {
        // Added rewardPoolId as a constructor parameter
