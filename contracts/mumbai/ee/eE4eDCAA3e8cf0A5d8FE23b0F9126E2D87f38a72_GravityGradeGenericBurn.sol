...
contract GravityGradeGenericBurn is IGravityGradeGenericBurn, OwnableUpgradeable {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords)
        ...
        onlyOracle
    {
        RequestInputs memory requestInputs = RequestInputs({
        ...
        uint256[] memory expandedValues = new uint256[](requestInputs.randWordsCount);
        for (uint256 i = 0; i < requestInputs.randWordsCount; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(randomWords[0], i)));
        }

        ContentCategory[] memory categories = s_tokenCategories[requestInputs.tokenId];

        for (uint256 i; i < categories.length; i++) {
            bool categoryExcluded;
            for (uint256 z; z < excludedIds.length; z++) {
                if (categories[i].id == excludedIds[z]) {
                    categoryExcluded = true;
                    break;
                }
            }
            if (categoryExcluded) continue;
            for (uint256 j; j < requestInputs.openings; j++) {
                ...
                for (uint256 k; k < draws; k++) {
                    ...
                    _mintReward(requestInputs.user, categories[i], indexContent);
                }
            }
        }
