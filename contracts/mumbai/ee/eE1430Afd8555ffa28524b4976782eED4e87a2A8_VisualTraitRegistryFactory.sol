...
import "../VisualTraitRegistry/VisualTraitRegistry.sol";
import "../extras/recovery/BlackHolePrevention.sol";

contract VisualTraitRegistryFactory is Ownable, BlackHolePrevention {
    using Strings  for uint32; 

    uint256     public constant     version              = 20230621;
    uint8       public constant     traitType            = 104;            // Visual Traits
    ...
    function getMyCommunityRegistry(uint16 _communityId) internal view returns ( CommunityRegistry ) {
        ...
        require(
            crAddr != address(0),
            "VisualTraitRegistryFactory: Invalid community ID"
        );
    ...
    function addTrait(
        ...
        CommunityRegistry thisCommunityRegistry = getMyCommunityRegistry(_inputTrait.communityId);

        // Get trait registry for selected token
        ECRegistryV3c traitRegistry = ECRegistryV3c(
            thisCommunityRegistry.getRegistryAddress(
                string(
                    abi.encodePacked(
                        "TRAIT_REGISTRY_", _inputTrait.tokenNum.toString()
                    )
                )
            )
        );
        ...
        require(
            ...
            "VisualTraitRegistryFactory: Trait Registry not installed"
        );

        // Check if caller is TRAIT_REGISTRY_ADMIN
        require(
            traitRegistry.isAllowed(TRAIT_REGISTRY_ADMIN, msg.sender),
            "VisualTraitRegistryFactory: Unauthorized"
        );

        // Get next available ID for the new trait
        traitId = traitRegistry.traitCount();

        // check for TOKEN_{id} AND CHILD_TOKEN_{id}
        address ERC721 = thisCommunityRegistry.getRegistryAddress(
            string(
                abi.encodePacked(
                    "TOKEN_", _inputTrait.tokenNum.toString()
                )
            )
        );

        address ERC721_CHILD = thisCommunityRegistry.getRegistryAddress(
            string(
                abi.encodePacked(
                    "TOKEN_CHILD_", _inputTrait.tokenNum.toString()
                )
            )
        );
        ...
        // NFT must exist!
        require(
            (ERC721 != address(0) || ERC721_CHILD != address(0)),
            "VisualTraitRegistryFactory: Token not installed"
        );

        // check if we have a visual trait registry already deployed
        ECRegistryV3c.traitStruct[] memory existingTraits = traitRegistry.getTraits();
        bool hasVisualTraitRegistry = false;
        for(uint16 i = 0; i < existingTraits.length; i++) {
            if(existingTraits[i].traitType == traitType) {
                hasVisualTraitRegistry = true;
            }
        }

        require(!hasVisualTraitRegistry, "VisualTraitRegistryFactory: Visual Trait Registry already exists!");

        // Deploy new VisualTraitRegistry
        VisualTraitRegistry newImplementer = new VisualTraitRegistry();
        newImplementer.init(address(traitRegistry), traitId);

        ECRegistryV3c.traitStruct[] memory traits = new ECRegistryV3c.traitStruct[](1);
        traits[0] = ECRegistryV3c.traitStruct(
            traitId, 
            traitType, 
            _inputTrait.start, 
            _inputTrait.end, 
            _inputTrait.enabled, 
            address(newImplementer), 
            _inputTrait.ipfsHash, 
            _inputTrait.name
        );

        // Give this factory access to write into the Trait registry if it's missing
        ...
        // Add it into the Trait Registry
        traitRegistry.addTrait(traits);

        traitRegistry.setDefaultTraitControllerType(
            address(newImplementer),
            traitType
        );
    }
