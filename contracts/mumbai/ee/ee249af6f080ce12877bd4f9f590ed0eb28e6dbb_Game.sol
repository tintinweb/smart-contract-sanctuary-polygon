...
contract Game {
    ...
    function mapPlayer(string memory _name, int _str, int _agi, int _wis) internal returns (uint) {
        addHero(_name, 100, _str, _agi, _wis);
        return players.length-1; // return index of newly mapped player
    }
    
    function battle(string memory _name, int _str, int _agi, int _wis) public {
        // initialize player
        mapPlayer(_name, _str, _agi, _wis);
