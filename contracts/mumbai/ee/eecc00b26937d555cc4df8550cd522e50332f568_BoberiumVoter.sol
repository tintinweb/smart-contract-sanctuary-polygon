...
contract BoberiumVotingStorage is IBoberiumVotingStorage, Ownable {
    ...
    mapping(address => mapping(uint256 => uint256)) public votingUnits;
    ...
    function changeVotingNominator(address _nftAddress, uint256 _tokenId, uint256 _votingUnits) public onlyOwner {
        votingUnits[_nftAddress][_tokenId] = _votingUnits;
    }

    function changeVotingNominatorBatch(address _nftAddress, uint256[] calldata _tokenIds, uint256[] calldata _votingUnits) public onlyOwner {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            votingUnits[_nftAddress][_tokenIds[i]] = _votingUnits[i];
        }
    }

    function getVotingUnits(address nftAddress, uint256 tokenId) external onlyWhitelisted(nftAddress) view returns (uint256) {
        uint256 votingUnit = votingUnits[nftAddress][tokenId];
        if (votingUnit != 0) {
            return votingUnit;
        }
        return 1;
...
interface IBoberiumVotingStorage {
    function getVotingUnits(address nftAddress, uint256 tokenId) external returns (uint256);
...
contract BoberiumVoter is AccessControl {
    ...
    function vote(bytes32 proposalId, uint256 tokenId, uint256 choice) public onlyActiveVoting(proposalId) onlyValidVote(proposalId, tokenId, choice) {
        Proposal storage proposal = _proposals[proposalId];

        uint256 votingUnits = votingStorage.getVotingUnits(address(proposal.votingToken), tokenId);
        if (votingUnits == 0)
