...
contract Vault is AccessControl {
    ...
    bytes4 public constant DEPOSIT_RESERVE_AND_EXECUTE_SELECTOR = bytes4(keccak256("depositReserveTokenAndExecute(bool,bool,bytes,bytes,bytes,bytes)"));
    // depositNonReserveTokenAndExecute(bool,bool,bytes,bytes,bytes)
    bytes4 public constant DEPOSIT_NON_RESERVE_AND_EXECUTE_SELECTOR = bytes4(keccak256("depositNonReserveTokenAndExecute(bool,bool,bytes,bytes,bytes,bytes)"));
    // depositLPTokenAndExecute(bool,bytes,bytes,bytes)
    bytes4 public constant DEPOSIT_LP_AND_EXECUTE_SELECTOR = bytes4(keccak256("depositLPTokenAndExecute(bool,bytes,bytes,bytes,bytes)"));
    ...
    function setContractsOnChain(bytes32 chainIdBytes, bytes calldata contractAddr) external onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function stakeCrossChain(
        ...
        bytes memory arbitraryData = abi.encode(
            ourContractsOnChain[destChainIdBytes],
            data,
            2000000, // gas limit
            50000000000 // gas price
        );
        ...
        if (selector == DEPOSIT_RESERVE_AND_EXECUTE_SELECTOR || selector == DEPOSIT_NON_RESERVE_AND_EXECUTE_SELECTOR) {
            (success, ) = voyagerDepositHandler.call{ value: msg.value }(
                abi.encodeWithSelector(selector, false, false, swapData, executeData, arbitraryData, requestMetadata)
            );
        } else {
            (success, ) = voyagerDepositHandler.call{ value: msg.value }(
                abi.encodeWithSelector(selector, false, swapData, executeData, arbitraryData, requestMetadata)
            );
        }
    ...
    function voyagerReceive(
        ...
        require(sourceSenderAddress == toAddress(ourContractsOnChain[srcChainIdBytes]), "not our contract");
            ...
            IERC20(settlementToken).safeIncreaseAllowance(address(stakingContract), settlementAmount );
            // decoding the data we sent from the source chain
            address user = abi.decode(data, (address));
            // calling the stake function
            stakingContract.stake(user, settlementToken, settlementAmount);
    }
