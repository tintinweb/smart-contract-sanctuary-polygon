...
contract LittlelamboNFTs is ERC721Enumerable, Ownable,Whitelist {
  using Strings for uint256;
  ...
  uint256 public publicsalePerAddressLimit = 5;
  uint256 public remainingNftForSale = 1500;
  ...
  mapping(address => uint256) public addressMintedPresale;
  ...
  function _baseURI() internal view virtual override returns (string memory) {
    ...
    function mint(uint _mintAmount) public payable {
        ...
        if(currentPhase == 1){
            ...
            for (uint256 i = 1; i <= _mintAmount; i++) {
                remainingNftForSale--;
                addressMintedPresale[msg.sender]++;
                _safeMint(msg.sender, _mintedSupply + i);
                totalMinted++;
            }
        }else{
    ...
    function mintByLittleLambo(address user, uint _mintAmount) public onlyOwner {
        require(!paused, "the contract is paused");
        require(isSaleStarted,"sale is not started");
        require(remainingNftForSale >= _mintAmount, "Exceeds Remaining NFT for sale");
        uint256 supply = totalSupply();
        ...
        uint256 _mintedSupply = totalMinted;
        if(currentPhase == 1){
            require(whitelist[user], "User is not whitelisted");
            for (uint256 i = 1; i <= _mintAmount; i++) {
                remainingNftForSale--;
                addressMintedPresale[user]++;
                _safeMint(user, _mintedSupply + i);
                totalMinted++;
            }
        }else{
            uint256 ownerMintedCount = addressMintedPublicsale[user];
            require(ownerMintedCount + _mintAmount <= publicsalePerAddressLimit, "Max NFTs per address exceeded.");
            for (uint256 i = 1; i <= _mintAmount; i++) {
                remainingNftForSale--;
                addressMintedPublicsale[user]++;
                _safeMint(user, _mintedSupply + i);
                totalMinted++;
            }
        }
  ...
  function setRemainingNftForSale(uint _remainingNftForSale, uint256 _currentPhase) public onlyOwner {
