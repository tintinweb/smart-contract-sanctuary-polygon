 ...
 */
contract SafeManagerFacet is RebaseOpt, Common, ReentrancyGuard, Initializable {
    ...
    function adjustSafeBal(
        ...
        if (_add == true) {
            ...
            require(
                safe[_owner][_index].bal >= _amount,
                "SafeManager: Safe cannot have negative balance"
            );
            safe[_owner][_index].bal -= _amount;
        }
    }
    ...
    function adjustSafeDebt(
        ...
        require(
            ...
            "SafeManager: debtToken mismatch"
        );
        require(
            safe[_owner][_index].status == Status(1),
            "SafeManager: Safe not active"
        ...
        if (_add == false) {
            require(
                _amount <= safe[_owner][_index].debt,
                "SafeManager: Safe cannot have negative debt"
            );
        ...
        )
    {
        ...
        CR = (assets * activeTokenPrice)
            .divPrecisely(debt * debtTokenPrice)
            .mulTruncate(10_000);
    ...
    function getMaxBorrow(
        ...
        uint _index,
        address _debtToken
    ) public view returns (uint maxBorrow) {
        address activeToken = safe[_owner][_index].activeToken;
        address debtToken = safe[_owner][_index].debtToken;
    ...
    function setActiveToDebtTokenMCR(
        address _activeToken,
        address _debtToken,
        uint _MCR
    ) external {
    ...
    function setUnactiveCounterpart(
        address _activeToken,
    ...
    function setActivePool(address _token, address _activePool) external {
...
contract Common {
    ...
    modifier onlySafeOps() {
        require(
            msg.sender == safeOperations,
            "SafeManager: Only SafeOps can call"
        );
        _;
    }

    function setSafeOps(address _safeOperations) external {
