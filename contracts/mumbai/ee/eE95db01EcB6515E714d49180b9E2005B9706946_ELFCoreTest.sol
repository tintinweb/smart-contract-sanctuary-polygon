...
contract ELFBase is Pausable, IERC721TokenReceiver, IERC165, IERC721Metadata {
    ...
    function supportsInterface(bytes4 _interfaceID) external pure override returns (bool) {
        return ((_interfaceID == InterfaceSignature_ERC165) ||
            ...
            (_interfaceID == InterfaceSignature_ERC721TokenReceiver) ||
    ...
    function gainELF(uint256 _tokenId)
        external
        view
        returns (
            uint256,
...
contract ELFTransfer is ELFBase, IERC721 {
    ...
    /// @dev Required for ERC-721 compliance.
    function ownerOf(uint256 _tokenId) public view override returns (address res) {
...
contract ELFCore is ELFTransfer {
    ...
    constructor() {
        ELF memory _ELF = ELF({label: 777, dad: 0, mom: 0, gene: 0, bornAt: block.timestamp});
        ELFs.push(_ELF);
    ...
    function capsuleMachine(bool triple) external payable whenNotPaused {
        ...
        for (uint256 i = 0; i < count; i++) {
            ELF memory _ELF = ELF({label: label, dad: 0, mom: 0, gene: genes[i], bornAt: block.timestamp + 60});
    ...
    function spawnEgg(uint256 momTokenId, uint256 dadTokenId) external whenNotPaused returns (uint256 tokenId) {
        ...
        ELF memory _ELF = ELF({label: 0, dad: dadTokenId, mom: momTokenId, gene: gene, bornAt: seed + 60});
        tokenId = ELFs.length;
        ELFs.push(_ELF);
...
contract ELFCoreTest is ELFCore {

    function assignGene(uint256 gene, address _to) external onlyAdmin {
        uint256 id=ELFs.length;
        ELF memory _ELF = ELF({label: 0, dad: 0, mom: 0, gene: gene, bornAt: block.timestamp + 60});
