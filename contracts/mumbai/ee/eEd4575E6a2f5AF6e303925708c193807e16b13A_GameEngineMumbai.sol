...
interface INFTFactory is IERC721{
    ...
    function getTokenInfo(uint256 _tokenId) external view returns(uint8, uint8, address);
}
...
contract GameEngine is Ownable, ReentrancyGuard, ProxyTarget {
	...
	mapping(uint8 => mapping(uint8 => uint256[])) public pool; //0 bad 1 good (1-5) levels
	...
	function initialize(address _randomEngineAddress, address _nftAddress) external {
	...
	function getPool(uint8 i, uint8 j) public view returns (uint256[] memory) {
		return pool[i][j];
	}
	...
	function moveToLast(uint256 _tokenId) internal {
		(uint8 tokenType, , ) = nftToken.getTokenInfo(_tokenId);
		uint8 level = uint8(levelOfToken[_tokenId]);
		uint256 position = tokenToArrayPosition[_tokenId];
		uint256[] storage currentPool = pool[tokenType][level];
	...
	function determineAndPush(uint256 tokenId) internal {
		...
		(uint8 tokenType, , ) = nftToken.getTokenInfo(tokenId);
	...
	function steal(uint8 tokenType, uint256 nonce) internal returns (uint256) {
		...
		uint256 tokenToGet = randomNumber % pool[tokenType][level].length;
		return pool[tokenType][level][tokenToGet];
	}

	function whichLevelToChoose(uint8 tokenType, uint256 randomNumber) internal view returns (uint8) {
	...
	function howManyTokensCanSteal(uint8 tokenType) internal view returns (uint256) {
		uint256[2] memory totalStaked;
	...
	function onSuccess(uint256 tokenId) internal virtual returns (bool) {
		(uint8 nftType, , ) = nftToken.getTokenInfo(tokenId);
	...
	function onCriticalSuccess(uint256 tokenId) internal virtual returns (bool) {
		...
		lastClaim[tokenId] = block.timestamp;
		if (
			uint256(keccak256(abi.encodePacked(randomNumberGenerator.getRandom("onCriticalSuccess", _nonce++), "1"))) %
				100 <
			40 &&
		...
		uint256 value = howManyTokensCanSteal(nftType);
		...
		for (uint256 i = 0; i < value; i++) {
			...
			if (stolenTokenId != 0) {
				...
				(, , address tokenOwner) = nftToken.getTokenInfo(stolenTokenId);
				emit Steal(msg.sender, tokenOwner, stolenTokenId);
	...
	function onCriticalFail(uint256 tokenId) internal returns (bool) {
		(, , address tokenOwner) = nftToken.getTokenInfo(tokenId);
		emit Die(tokenOwner, tokenId);
	...
	function claimStake(uint256 tokenId) internal returns (bool) {
		...
		(, uint8 level, ) = nftToken.getTokenInfo(tokenId);
	...
	function claimAndUnstake(bool claim, uint256[] memory tokenAmount) external nonReentrant {
		for (uint256 i = 0; i < tokenAmount.length; i++) {
			(, , address tokenOwner) = nftToken.getTokenInfo(tokenAmount[i]);
			require(tokenOwner == msg.sender, "Caller not the owner");
	...
	function totalStakedOfType(uint8 tokenType) public view returns (uint256) {
		uint256 totalStaked;
		for (uint8 j = 1; j < 6; j++) {
	 ...
	 *
	 * returns (tokenType, level, owner, isStaked, stakeTime, lastClaimTime, tokenToRandomHourInStake)
	 */
	...
	{
		(uint8 tokenType_, uint8 level_, address tokenOwner) = nftToken.getTokenInfo(tokenId);
		return TokenMeta(
			tokenType_,
			level_,
			tokenOwner,
	...
	struct TokenMeta {
		...
		uint256 tokenToRandomHourInStake;
	}
}
