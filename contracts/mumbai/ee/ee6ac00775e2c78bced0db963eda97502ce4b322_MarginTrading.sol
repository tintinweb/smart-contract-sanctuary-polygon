...
contract MarginTrading is Ownable{
    ...
    function deposit(address tokenAddress, uint256 amount, uint256 leverage) external payable {
        ...
        availableLiquidity[tokenAddress] = availableLiquidity[tokenAddress].sub(depositInfo.loanAmount);
    ...
    function createPositionandTrade(address tokenAddress, address targetToken, uint256 amount) external {
        ...
        uint256 allowedDeposit = depositInfo.assetAmount.add(depositInfo.loanAmount).sub(depositInfo.usedMargin.add(depositInfo.usedLoan));
        ...
        depositInfo.usedLoan = depositInfo.usedLoan.add(amount.mul(80).div(100));
        // Used margin is 20% of given amount
        depositInfo.usedMargin =  depositInfo.usedMargin.add(amount.mul(20).div(100));
    ...
    function trade(uint256 tradeAmount, address[] memory path) internal returns(uint256, uint256) {
        ...
        uint[] memory price = uniswapV2Router.getAmountsOut(1*10**decimals, path);
    ...
    function closePosition(uint256 _positionID) public payable {
        ...
        require(positionInfo.userAddress == msg.sender, "Close Position : Not the user");
        ...
        uint decimals = IERC20(path[1]).decimals();
        address[] memory path_ = new address[](2);
        path_[0] = path[1]; path_[1] = path[0];
        // Get Price
        uint[] memory price = uniswapV2Router.getAmountsOut(1*10**decimals, path_);
        // Store Traded values
        ...
        depositInfo.usedMargin =  depositInfo.usedMargin.sub(positionInfo.assetAmount.mul(20).div(100));
    ...
    function withdraw(address tokenAddress, uint256 amount) external payable {
        ...
        uint256 actualAsset = depositInfo.assetAmount+depositInfo.lossValue-depositInfo.gainValue;
        ...
        depositInfo.timestamp = block.number;  
        availableLiquidity[tokenAddress] = availableLiquidity[tokenAddress].add(amount*leverage); 
    } 

    function withdrawTest(address tokenAddress, uint256 amount) external payable {
        Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
        require(depositInfo.assetAmount.sub(depositInfo.usedMargin) > 0, "Withdraw call : Insufficient Deposit");    
        
        uint256 actualAsset = depositInfo.assetAmount+depositInfo.lossValue-depositInfo.gainValue+depositInfo.exitAmount;
        // given 350(actualAsset)
        // Loan 550(loanAmount)
        // for x amount => (loanAmount*x)/actualAsset
        uint256 temp;
        if(depositInfo.assetAmount.add(depositInfo.usedMargin) == 0) 
            temp = depositInfo.loanAmount.mul(amount+depositInfo.lossValue-depositInfo.gainValue);
        else
            temp = depositInfo.loanAmount.mul(amount);
        ...
        require(amount > _interest, "Withdraw call : Amount too less for paying interest");
        ...
        if(depositInfo.assetAmount.add(depositInfo.usedMargin) == 0) {
            ...
            depositInfo.lossValue = 0; depositInfo.gainValue = 0;
            // allocatedLoan = amount+depositInfo.lossValue-depositInfo.gainValue;
        }
        else // have exit amount for calculate actual deposit 
            depositInfo.exitAmount = depositInfo.exitAmount.add(amount);
        ...
        availableLiquidity[tokenAddress] = availableLiquidity[tokenAddress].add(allocatedLoan);
        // Reduce paid interest
        depositInfo.interestAccumulated = depositInfo.interestAccumulated.sub(_interest);
    ...
    function accureInterest(address tokenAddress, uint256 loanAmount) internal {
        Deposit storage depositInfo = deposits[msg.sender][tokenAddress];
        depositInfo.interestAccumulated = depositInfo.interestAccumulated.add(checkInterestAccumulated1(msg.sender, tokenAddress, loanAmount));
    }
    ...
    function checkInterestAccumulated1(address userAddress, address tokenAddress, uint256 loanAmount) public view returns(uint256) {
        Deposit memory depositInfo = deposits[userAddress][tokenAddress];
        // uint256 loanAmount = depositInfo.loanAmount;
        uint256 interval = (block.number.sub(depositInfo.timestamp)).div(20);
        uint interest = depositInfo.interestAccumulated.add(loanAmount.mul(interval.mul(interestPerBlock).div(1000)).div(1E18));
    ...
    function addAsset(address tokenAddress, uint256 amount) external payable {
        ...
        availableLiquidity[tokenAddress] = availableLiquidity[tokenAddress].add(amount);
    }
    ...
    function removeAsset(address tokenAddress) external onlyOwner {
        if(tokenAddress == address(0)) {
            ...
            IERC20(tokenAddress).transfer(msg.sender, availableLiquidity[tokenAddress]);
        }
