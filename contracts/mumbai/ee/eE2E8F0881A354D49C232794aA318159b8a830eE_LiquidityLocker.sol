...
contract LiquidityLocker is ReentrancyGuard, Ownable {
    ...
    event URLUpdated(uint256 _tokenIdentifier, string _tokenUrl);
    ...
    string public token0URL;
    string public token1URL;
     ...
     */
    function init(bytes memory _encodedData) public {
        ...
        (, , , , , , , token0URL, token1URL) = abi.decode(
            _encodedData,
            (
                IERC20,
                address,
    ...
    function updateToken0URL(string memory _tokenURL) external onlyOwner {
        token0URL = _tokenURL;
        emit URLUpdated(0, token0URL);
    }

    function updateToken1URL(string memory _tokenURL) external onlyOwner {
        token1URL = _tokenURL;
        emit URLUpdated(1, token1URL);
    }
