...
contract MultichainManager is ZetaInteractor, ZetaReceiver {
    error InvalidMessage();

    bytes32 public constant CROSS_CHAIN_DELEGATE_VOTE =
        keccak256("CROSS_CHAIN_DELEGATE_VOTE");
    bytes32 public constant CROSS_CHAIN_TRANSFER_MESSAGE =
        keccak256("CROSS_CHAIN_TRANSFER_MESSAGE");
    bytes32 public constant CROSS_CHAIN_CAST_VOTE =
        keccak256("CROSS_CHAIN_CAST_VOTE");

    IERC20 internal immutable _zetaToken;

    ZetaTokenConsumer private immutable _zetaConsumer;
    IDAOToken private immutable _daoToken;
    ...
    constructor(
        ...
        _daoToken = IDAOToken(DAOTokenAddress);
    }
    ...
    function onZetaMessage(
        ...
        if (messageType == CROSS_CHAIN_DELEGATE_VOTE) {
            _executeCrossChainDelegateVote(sender, args);
        }
    }
    ...
    function _executeCrossChainDelegateVote(
        address _sender,
        bytes memory args
    ) internal {
        address _to = abi.decode(args, (address));
        _daoToken.crossChainDelegateVote(_sender, _to);
    }
}
