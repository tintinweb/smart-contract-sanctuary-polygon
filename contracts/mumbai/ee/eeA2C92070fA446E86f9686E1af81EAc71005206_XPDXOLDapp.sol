...
// File contracts/XPDXOLVerifyAsset.sol


pragma solidity ^0.8.4;

library XPDXOLVerifyAsset {
...
// File contracts/IXPDXOLToken.sol
pragma solidity ^0.8.4;
/**
 * @dev XPDXOL token Interface
 */
interface IXPDXOLToken is IERC20 {
...
// File contracts/XPDXOLDapp.sol


pragma solidity ^0.8.4;

contract XPDXOLDapp is Ownable, IXPDXOLAsset {
	bool public XPDXOL_IS_AWESOME = true;
	...
	IXPDXOLToken public _XPDXOLToken;
	IXPDXOLNFT public _XPDXOLNFT;
	...
	address public rewardsWallet = 0x24554c414E440000000000000000000000000002; // $XPDXOL Internal Rewards Wallet
	address public WMATICAddress = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;

	string private _tokenBaseURI = "https://api.XPDXOL.io/metadata/";
	...
	constructor(
		...
		_XPDXOLToken = IXPDXOLToken(XPDXOLToken);
		...
		uniswapV2Pair = IUniswapV2Factory(_uniswapV2Router.factory()).getPair(
			address(_XPDXOLToken),
			address(WMATICAddress) //_uniswapV2Router.WETH()
		);

		liquidityWallet = address(_XPDXOLToken);
		_XPDXOLNFT = IXPDXOLNFT(XPDXOLNFT);
	...
	function mint(
		...
		uint256 parentId,
		uint256 population,
		uint256 XPDXOLReward,
		address referral,
		bytes memory signature
	) external payable whenNotPaused {
		require(_XPDXOLNFT.exists(tokenId) == false, "TOKEN_EXISTS");
		...
		require(
			XPDXOLVerifyAsset.verify2(signerAddress, hash, signature) == true,
			"INVALIDSIG"
		); // Invalid signature
		...
		if (payType == 1) // BNB
		{
			...
			_XPDXOLNFT.mint(tokenId, parentId, population, 0);
			standardTransfer(tokenId, msg.sender, address(0), referral);
		}
		// $XPDXOL
		else {
			_XPDXOLToken.transferFrom(msg.sender, address(this), price);
			_XPDXOLNFT.mint(tokenId, parentId, population, 0);
		...
		if (XPDXOLReward > 0) {
			_XPDXOLToken.addAirdropReward(msg.sender, XPDXOLReward);
			emit AirdropReward(msg.sender, XPDXOLReward);
		}
	}
	...
	{
		require(payType == 1 || payType == 2, "INVALID_PAYTYPE");
		require(_XPDXOLNFT.exists(tokenId), "NOT_MINTED");

		Asset memory asset = _XPDXOLNFT.getTokenDetail(tokenId);
		address previousOwner = _XPDXOLNFT.getTokenOwner(tokenId);
	...
	function unlock(
		...
		// $XPDXOL
		if (payType == 2) {
			_XPDXOLToken.transferFrom(msg.sender, marketingWallet, amount);
	...
	function airdrop(
		...
		_XPDXOLNFT.airdrop(tokenId, parentId, population, assetType);
	}
	...
	function setTokenMetadata(
		...
		require(_XPDXOLNFT.exists(tokenId), "NOT_FOUND");
		address tokenOwner = _XPDXOLNFT.getTokenOwner(tokenId);
		require(_msgSender() == tokenOwner, "NOT_OWNER");
		require(taxRate >= 0, "TAXRATE_TOO_LOW");
		require(taxRate <= 75, "TAXRATE_TOO_HIGH");
		require(block.timestamp <= validTo, "EXPIRED");
		require(
			XPDXOLVerifyAsset.verifyMeta(
				signerAddress,
				tokenId,
				validTo,
				taxRate,
	...
	function setTokenPrice(
		...
		address tokenOwner = _XPDXOLNFT.getTokenOwner(tokenId);
		require(_msgSender() == tokenOwner, "NOT_OWNER");
		_XPDXOLNFT.setTokenPrice(tokenId, price, priceBNB);
	}
	...
	function notifyTokenTransfer(
		...
		_XPDXOLToken.setRewardsFactor(seller, sellerRewardsFactor);
		_XPDXOLToken.setRewardsFactor(buyer, buyerRewardsFactor);
	}
	...
	function standardTransfer(
		...
		if (beforeSwapEthBalance > 0) {
			...
			if (refferalFee > 0) {
				...
				_XPDXOLToken.transfer(rewardsWallet, conv);
				_XPDXOLToken.addStaticReward(referral, conv);
	...
	function updateRewards(address owner) internal {
		if (owner != address(0)) {
			uint256 rewardsFactor = _XPDXOLNFT.getTotalPopulationByOwner(owner);
			_XPDXOLToken.setRewardsFactor(owner, rewardsFactor);
		}
	}
	...
	function standardERC20Transfer(
		...
		if (address(previousOwner) == address(0)) {
			...
			if (refferalFee > 0) {
				totalRefferalPayout = totalRefferalPayout.add(refferalFee);
				_XPDXOLToken.transfer(rewardsWallet, refferalFee);
				_XPDXOLToken.addStaticReward(referral, refferalFee);
		...
		/// @dev Transfer WORLDTAX_FEE to liquidity wallet
		if (tax.WORLDTAX_FEE > 0) {
			_XPDXOLToken.transfer(liquidityWallet, tax.WORLDTAX_FEE);
		}

		if (tax.MARKETING_FEE > 0) {
			_XPDXOLToken.transfer(marketingWallet, tax.MARKETING_FEE);
		}
		...
		if (previousOwner != address(0) && sellerBalance > 0) {
			_XPDXOLToken.transfer(previousOwner, sellerBalance);
		}
		...
		returns (uint256)
	{
		...
		Asset memory token = _XPDXOLNFT.getTokenDetail(tokenId);
		...
		address payee = _XPDXOLNFT.getTokenOwner(tokenId);

		_XPDXOLToken.transfer(rewardsWallet, balance);
		_XPDXOLToken.addStaticReward(payee, balance);

		_XPDXOLNFT.increaseTokenTotalTax(tokenId, balance);
	...
	function getReferralFee(
		...
		if (
			...
			if (_XPDXOLToken.balanceOf(referer) >= REFERRALFEE2_MIN_TOKENS) {
		...
		returns (uint256)
	{
		...
		path[0] = address(WMATICAddress); //uniswapV2Router.WETH(); 
		path[1] = address(_XPDXOLToken);
	...
	/// @dev Withdraw XPDXOL that gets stuck in contract by accident
	function emergencyWithdrawXPDXOL() external onlyOwner {
		_XPDXOLToken.transfer(owner(), _XPDXOLToken.balanceOf(address(this)));
	}
	...
	event XPDXOLContractAllowChange(
