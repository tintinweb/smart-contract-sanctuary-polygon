...
contract MyERC20 is ERC20{
    ...
    modifier checkAddress(address from){
        require(from != address(0), "ERC20: burn from the zero address");
        _;
    }

    function burn(address from, uint256 amount) external checkAddress(from){
    ...
    function burnWithPercent(address from, uint256 percent) external checkAddress(from){
        require(percent > 0 || percent <= 100, "Invalid Percentage");
        uint256 percentage = (_balances[from] / 100) * percent;
        require(_balances[from] >= percentage, "ERC20: burn amount exceeds balance");
        _balances[from] -= percentage;
