...
abstract contract GovernorCompatibilityBravo is IGovernorTimelock, IGovernorCompatibilityBravo, Governor {
    ...
    function initialize() public initializer returns (uint256) {
       ...
       // _feeDistributor = getFeeDistributor();
        ...
        winnerProposal._deadline = block.timestamp + 7 days;
    ...
    function set_Winner() public returns (uint256) {
        ...
        uint256 currentWeek = block.timestamp;
        console.log("currentWeel: %s, deadline: %s", currentWeek, winnerProposal._deadline);
        require(currentWeek >= winnerProposal._deadline,'Deadline has not been reached');

        if (winnerProposal._tie == true) {
            
            if ( winnerProposal.votingPower == 0) {
                started_time = true;
                return 0;
        }else 
            if( winnerProposal.votingPower > 0){
        ...
        ProposalDetails storage actual = _proposalDetails[winnerProposal.winnerId];
        actual.Winner= true;
    ...
    function getUserVotingPower(address account, uint256 timestamp) external view returns (uint256) {
        WeeklieProposalsDetails storage winnerProposal = _weeklieproposalDetails[timestamp];
        return winnerProposal.receipts[account].votes;
    }
    ...
    function propose(
        ...
        _storeProposal(_msgSender(), targets, values, new string[](calldatas.length), calldatas, description, token,proposalId);
        
        return super.propose(targets, values, calldatas, description, token,snapshot , deadline, proposalId );
    }
    ...
    function propose(
        ...
        require(started_time == false, 'Finish current proposal first');
        _storeProposal(_msgSender(), targets, values, signatures, calldatas, description, token,proposalId);
        return propose(targets, values, _encodeCalldata(signatures, calldatas), description, token,snapshot , deadline, proposalId);
    }
    /**
    ...
    function queue(uint256 proposalId) public virtual override {
        ...
        queue(details.targets, details.values, _encodeCalldata(details.signatures, details.calldatas), details.descriptionHash,proposalId);
    }
    ...
    function execute(uint256 proposalId) public payable virtual override {
        ...
        require (proposalId== winnerProposal.winnerId, 'Error: just the winner can be executed');

        super.execute(details.targets, details.values, _encodeCalldata(details.signatures, details.calldatas), details.descriptionHash,proposalId);
    ...
    function _countVote(
        ...
        require(actual.Winner == false, "Error: you can not vote for winner");
        // setting winner proposal
        WeeklieProposalsDetails storage winnerProposal = _weeklieproposalDetails[_startTime];
...
abstract contract GovernorTimelockControl is IGovernorTimelock, Governor {
    ...
    mapping(uint256 => uint256) private _timelockIds;
    ...
    function state(uint256 proposalId) public view virtual override(IGovernor, Governor) returns (ProposalState) {
        ...
        uint256 queueid = _timelockIds[proposalId];
        if (queueid == uint256(0)) {
            return status;
    ...
    function queue(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash,uint256 proposalId) public virtual override returns (uint256) {
        //uint256 proposalId = hashProposal(targets, values, calldatas, descriptionHash);

        require(state(proposalId) == ProposalState.Succeeded, 'Governor: proposal not successful');

        uint256 delay = _timelock.getMinDelay();
        _timelockIds[proposalId] = proposalId;//_timelock.hashOperationBatch(targets, values, calldatas, 0, descriptionHash)
        _timelock.scheduleBatch(targets, values, calldatas, 0, descriptionHash, delay,proposalId);
     ...
     */
    function _execute(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash, uint256 id) internal virtual {
        _timelock.executeBatch{value: msg.value}(targets, values, calldatas, 0, descriptionHash, id );
    }
    ...
    function _cancel(uint256 proposalId) internal virtual override returns (uint256) {
         proposalId = super._cancel(proposalId);
...
abstract contract IGovernorTimelock is IGovernor {
    ...
    function queue(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash,uint256 proposalId) public virtual returns (uint256 );
}
...
abstract contract Governor is Initializable, Context, ERC165, EIP712, IGovernor, IERC721Receiver, IERC1155Receiver {
     ...
     */
    function execute(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash,uint256 proposalId) public payable virtual override returns (uint256) {
        //uint256 proposalId = hashProposal(targets, values, calldatas, descriptionHash);
    ...
    function _execute(address[] memory targets, uint256[] memory values, bytes[] memory calldatas) internal virtual {
        ...
        (bool success, bytes memory returndata) = targets[0].call{value: values[0]}(calldatas[0]);
        Address.verifyCallResult(success, returndata, errorMessage);
...
*/
     ...
     */
    function execute(address[] memory targets, uint256[] memory values, bytes[] memory calldatas, bytes32 descriptionHash,uint256 proposalId) public payable virtual returns (uint256 );
...
contract MyGovernor is Governor, GovernorCompatibilityBravo, GovernorVotes, GovernorTimelockControl, Ownable {
    ...
    ) GovernorVotes(_token) Governor(name, votingEscrow) GovernorTimelockControl(_timelock) {
     ...
     */
     function propose(
        ...
        return propose(targets, values, calldatas, description, token, 0, 0,0);
    }
    ...
    function propose(
        ...
        address[] memory new_targets = new address[](targets.length + 1);
        uint256[] memory new_values = new uint256[](values.length + 1);
        bytes[] memory new_calldatas = new bytes[](calldatas.length + 1);
        

        for (uint256 i = 0; i < targets.length ; ++i) {
            new_targets[i] = targets[i ];
            new_values[i] = values[i ];
            new_calldatas[i] = calldatas[i ];
        }
        ...
        return super.propose(new_targets, new_values, new_calldatas, description, token, 0, 0, counter++);
    }
    ...
    function queue(
        ...
        return super.queue(targets, values, calldatas, descriptionHash, proposalId);
    }
...
contract TimelockController is AccessControl, IERC721Receiver, IERC1155Receiver {
     ...
     */
    event CallScheduled(uint256 indexed id, uint256 indexed index, address target, uint256 value, bytes data, bytes32 predecessor, uint256 delay);

    /**
     * @dev Emitted when a call is performed as part of operation `id`.
     */
    event CallExecuted(uint256 indexed id, uint256 indexed index, address target, uint256 value, bytes data);
     ...
     */
    function isOperation(uint256 id) public view virtual returns (bool registered) {
     ...
     */
    function isOperationPending(uint256 id) public view virtual returns (bool pending) {
     ...
     */
    function isOperationReady(uint256 id) public view virtual returns (bool ready) {
     ...
     */
    function isOperationDone(uint256 id) public view virtual returns (bool done) {
        return getTimestamp(id) == _DONE_TIMESTAMP;
    }
     ...
     */
    function getTimestamp(uint256 id) public view virtual returns (uint256 timestamp) {
     ...
     */
    function schedule(address target, uint256 value, bytes calldata data, bytes32 predecessor, bytes32 salt, uint256 delay, uint256 id) public virtual onlyRole(PROPOSER_ROLE) {
     ...
     */
    function _schedule(uint256 id, uint256 delay) private {
     ...
     */
    function cancel(uint256 id) public virtual onlyRole(CANCELLER_ROLE) {
    ...
    function execute(address target, uint256 value, bytes calldata payload, uint256 predecessor, bytes32 salt, uint256 id) public payable virtual onlyRoleOrOpenRole(EXECUTOR_ROLE) {
     ...
     */
    function _beforeCall(uint256 id, uint256 predecessor) private view {
        require(isOperationReady(id), 'TimelockController: operation is not ready');
        require(predecessor == uint256(0) || isOperationDone(predecessor), 'TimelockController: missing dependency');
    }
