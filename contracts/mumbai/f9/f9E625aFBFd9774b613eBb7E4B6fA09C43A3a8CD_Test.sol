...
contract Test{
    ...
    function registration(address userAddress, address payable referrerAddress, uint256 id) private{
        ...
        for(uint8 i=0;i<7;i++)
        {
            ...
            {
                referrerAddress.transfer((((levelUSD*REFERRAL_PERCENTS[i])/1000)/maticRate)*1e18);
                emit LevelBonus(referrerAddress, msg.sender, (((levelUSD*REFERRAL_PERCENTS[i])/1000)/maticRate)*1e18, i+1);
