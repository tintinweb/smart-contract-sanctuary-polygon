...
contract SoulBasedToken is ERC721URIStorage {
  ...
  constructor() ERC721("SBT","SBT"){
   ...
   function mint(address _to, SBT memory _sbt, string memory URI) external  {
     require(msg.sender == owner, "Not owner");
      _mint(owner, SBT_ID);
      _setTokenURI(SBT_ID, URI);
      _transfer(owner, _to, SBT_ID);
      uint length = userInfo[_to].length;
      userInfo[_to][length] = SBT_ID;
      info[SBT_ID] = _sbt;
      SBT_ID++;
    ...
    function burn(uint _sbtId) external {
      require(msg.sender == owner, "Not Owner");
      _burn(_sbtId);
    }
