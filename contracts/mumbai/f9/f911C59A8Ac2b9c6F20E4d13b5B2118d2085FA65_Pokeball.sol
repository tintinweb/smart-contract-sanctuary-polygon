...
contract Pokeball is ERC1155, Ownable {
    ...
    function buyPokeball(address to, uint256 id, uint256 amount) external payable {
        require(id > 0 && id < 5, "Invalid TokenID");
        require(msg.value == tokenIdToTokenPrice[id], "Not enough value");
        _mint(to, id, amount, "");
    }
    ...
    function updateTokenUri(uint256 id, string memory tokenUri) external onlyOwner {
        tokenIdToTokenURI[id] = tokenUri;
    }

    function updateTokenPrice(uint256 id, uint256 tokenPrice) external onlyOwner {
