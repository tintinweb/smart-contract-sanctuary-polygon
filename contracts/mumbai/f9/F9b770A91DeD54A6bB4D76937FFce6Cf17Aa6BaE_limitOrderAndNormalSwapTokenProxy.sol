...
interface limitOrderAndNormalSwapToken {
    ...
    function checkTcc(address[] memory _swapRouter, address[] memory _swapRouter2) external view returns (bool);

    function checkTcc2(address[] memory _swapRouter, address[] memory _swapRouter2) external view returns (bool);

    function getInTimeZone(uint256[] memory _start_end_Time, uint256[] memory _timeList) external view returns (bool _inTimeZone);
...
contract limitOrderAndNormalSwapTokenProxy is OpsTaskCreator, Ownable {
    ...
    function checkTcc(address[] memory _swapRouter, address[] memory _swapRouter2) public view returns (bool) {
        return limitOrderAndNormalSwapToken(USDTOrderAddress).checkTcc(_swapRouter, _swapRouter2);
    }

    function checkTcc2(address[] memory _swapRouter, address[] memory _swapRouter2) public view returns (bool) {
        return limitOrderAndNormalSwapToken(GasOrderAddress).checkTcc2(_swapRouter, _swapRouter2);
    }

    function getInTimeZone(uint256[] memory _start_end_Time, uint256[] memory _timeList) public view returns (bool _inTimeZone) {
        return limitOrderAndNormalSwapToken(USDTOrderAddress).getInTimeZone(_start_end_Time, _timeList);
    }
