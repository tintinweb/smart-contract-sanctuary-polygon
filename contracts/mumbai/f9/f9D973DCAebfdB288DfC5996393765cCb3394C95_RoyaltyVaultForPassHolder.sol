 ...
 *|  _  // _ \| | | |/ _` | | __| | | \ \/ / _ \| | __|
 *| | \ \ (_) | |_| | (_| | | |_| |_| |\  / (_) | | |_ 
 *|_|  \_\___/ \__, |\__,_|_|\__|\__, | \/ \___/|_|\__|
 *              __/ |             __/ |                
 *             |___/             |___/                 
 *
...
contract RoyaltyVaultForPassHolder is AccessControl, ReentrancyGuard{
  ...
  function batchWithdraw(uint256 counter) external onlyRole(WITHDRAWER_ROLE) payable nonReentrant{
    if(batchWithdrawnId + counter > receiveId) revert NoWithdrawableRequest();
  ...
  function withdraw() external payable nonReentrant{
    uint256 claimableETH = claimablePerAddress[msg.sender].amountETH;
    uint256 claimableWETH = claimablePerAddress[msg.sender].amountWETH;
    if(claimableETH == 0 && claimableWETH == 0) revert AcountHasNoClaimableAmount();
    if(claimableETH > 0) {
      claimablePerAddress[msg.sender].amountETH -= claimableETH;
      withdrawnPerAddress[msg.sender].amountETH += claimableETH;
      ETHbal -= claimableETH;
      _transfer(msg.sender, claimableETH);
    }
    // update the claimable and withdrawn of WETH
    if(claimableWETH > 0){
      claimablePerAddress[msg.sender].amountWETH -= claimableWETH;
      withdrawnPerAddress[msg.sender].amountWETH += claimableWETH;
      WETHbal -= claimableWETH;
  ...
  function sweep(address[] memory addressList) external payable onlyRole(WITHDRAWER_ROLE) nonReentrant{
    ...
    for(uint256 i; i < addressList.length; i++){
      uint256 claimableETH = claimablePerAddress[addressList[i]].amountETH;
      uint256 claimableWETH = claimablePerAddress[addressList[i]].amountWETH;
      if(claimableETH > 0) {
        claimablePerAddress[addressList[i]].amountETH -= claimableETH;
        withdrawnPerAddress[addressList[i]].amountETH += claimableETH;
        ETHbal -= claimableETH;
        _transfer(TEAM_WALLET, claimableETH);
      }
      if(claimableWETH > 0) {
        claimablePerAddress[addressList[i]].amountWETH -= claimableWETH;
        withdrawnPerAddress[addressList[i]].amountWETH += claimableWETH;
        WETHbal -= claimableWETH;
