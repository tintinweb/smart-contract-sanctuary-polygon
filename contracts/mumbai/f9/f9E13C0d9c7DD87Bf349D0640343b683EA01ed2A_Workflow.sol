...
contract Workflow is KeeperCompatibleInterface {
    ...
    function getLatestWMaticPrice()
        public
        view
        returns (uint256)
    {
        AggregatorV3Interface tokenAFeed = AggregatorV3Interface(WMATIC);
        uint256 price = _getLatestTokenPrice(tokenAFeed);
    ...
    function performUpkeep(bytes calldata /* performData */) external override {
        if (block.timestamp > interval + lastTimeStamp) {
            ...
            for (uint256 i = 0; i < limitOrderWorkflows.length; i++) {
                ...
                AggregatorV3Interface tokenAFeed = AggregatorV3Interface(details.tokenA);
                uint256 lastTokenAPrice = _getLatestTokenPrice(tokenAFeed);

                uint256 deltaA = details.tokenAReferencePrice - lastTokenAPrice;
