...
contract PolyDAO is ReentrancyGuard, AccessControl {
    ...
    event Payment(address indexed stakeholder, address projectAddress, uint256 amount);
    ...
    function createProposal(string calldata title, string calldata description, address receiverAddress, uint256 amount, string calldata imageId) public payable onlyMember("Only Members can create new proposal.") {
    ...
    function getStakeholderBalance() public view onlyStakeholder("Only Stakeholder can call this function") returns (uint256) {
        return stakeholders[msg.sender];
    }

    function getMemberBalance() public view onlyMember("Only Members can call this function") returns (uint256) {
    ...
    function vote(uint256 proposalId, bool inFavour) public onlyStakeholder("Only Stakeholders can vote on a proposal") {
        Proposal storage proposal = proposals[proposalId];
    ...
    function provideFunds(uint256 proposalId, uint256 fundAmount) public payable onlyStakeholder("Only Stakeholders can make payments") {
        Proposal storage proposal = proposals[proposalId];

        if (proposal.isPaid) revert("Required funds are provided");
        if (proposal.voteInFavor <= proposal.voteAgainst) revert("This proposal is not selected for funding");
    ...
    function releaseFunding(uint256 proposalId) public payable onlyStakeholder("Only Stakeholders are allowed to release funds") {
        Proposal storage proposal = proposals[proposalId];
