...
import "../../contracts/contracts-upgradeable/ODFDaoUpgradeable.sol";

contract $ODFDAOUpgradeable is ODFDAOUpgradeable {
    bytes32 public __hh_exposed_bytecode_marker = "hardhat-exposed";
...
import "../../../contracts/contracts/interfaces/IODFDao.sol";
...
contract ODFDAOUpgradeable is Initializable, OwnableUpgradeable, IODFDao {
     ...
     */
    IODFToken public votingToken;

    /**
     * @notice Save the address of the token to use as reward.
     */
    IODFToken public rewardToken;

    /**
     * @notice Tokens given as reward per vote.
     */
    ...
    function initialize(
        ...
        __Ownable_init(receiver_);
        votingToken = IODFToken(votingToken_);
        rewardToken = IODFToken(rewardToken_);
        rewardPerVote = 2 ether;
    }
    ...
    function vote(uint256 id_, bool approval_) external notFrozen {
        votingToken.transferFrom(msg.sender, address(this), 1 ether);
        ...
        uint256 _rewardPerVote = rewardPerVote;

        if (_rewardPerVote > 0)
            rewardToken.transfer(msg.sender, _rewardPerVote);
    ...
    /**
     * @notice Set the reward per vote.
     * @param rewardPerVote_ Amount of reward per vote.
     * @dev 0 to disable rewards.
     * @dev 1 ether = 1 token.
     */
    function setRewardPerVote(uint256 rewardPerVote_) external onlyOwner {
    ...
    function withdrawBalance() external onlyOwner {
        ...
        if (balance > 0) votingToken.transfer(owner(), balance);
        balance = rewardToken.balanceOf(address(this));
        if (balance > 0) rewardToken.transfer(owner(), balance);
    }
