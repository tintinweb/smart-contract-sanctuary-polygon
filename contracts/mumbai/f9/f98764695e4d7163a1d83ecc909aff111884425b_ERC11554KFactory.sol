...
/**
 * @dev {ERC11554KFactory}
 */
contract ERC11554KFactory is Initializable, OwnableUpgradeable {
     ...
     */
    function linkCollection(address collection) external virtual {
        require(
            _msgSender() == IERC11554K(collection).owner(),
            "ERC11554KFactory: caller is not a collection's owner"
        );
        require(
            !isLinkedCollection[collection],
            "ERC11554KFactory: collection already linked"
        );
        ...
        virtual
    {
        ...
        require(
            isLinkedCollection[collection],
            "ERC11554KFactory: not a linked collection"
        );
...
interface IERC11554K {
    ...
    function originators(uint256 id) external view returns (address);

    function whereItemStored(uint256 id) external view returns (address);
