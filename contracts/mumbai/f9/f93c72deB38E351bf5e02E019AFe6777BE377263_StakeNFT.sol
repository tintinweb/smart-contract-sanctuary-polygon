...
contract StakeNFT is Pausable, Ownable, ReentrancyGuard {
    ...
    mapping(uint256 => uint256) public repurchaseLimit;
    mapping(address => uint256) public totalRepurchased;
    ...
    function claimReward(uint256[] calldata _ids) public nonReentrant {
        ...
        for (uint256 i = 0; i < _ids.length; i++) {
            ...
            // for repurchase function 
            repurchaseLimit[_ids[i]] += reward;
    ...
    function repurchase(uint256 tokenId) public {
        uint256 nft_cost;
        if (tokenId >= 1 && tokenId <= 10) {
            ...
            nft_cost = 10000 ether;
        } else if (tokenId >= 71 && tokenId <= 80) {
            nft_cost = 25000 ether;
        } else if (tokenId >= 81 && tokenId <= 90) {
            nft_cost = 50000 ether;
        }
        totalRepurchased[msg.sender] += nft_cost;
        Token(token0).transferFrom(msg.sender, address(nftToken), nft_cost);
        repurchaseLimit[tokenId] = 0;
    }
    ...
    //View
    function getAmountout(uint256 totalEVC) public view returns(uint256) {
        address[] memory path = new address[](2);
        path[0] = token1;
        path[1] = token0;
        uint256[] memory evcValue = IPancakeRouter(pancakeRouterAdress).getAmountsOut(totalEVC, path);
        return evcValue[1];
    }
    ...
    function getUnClaimedReward(uint256 tokenId) public view returns(uint256) {
        ...
        anualReward = (plans[tokenInfos[tokenId].planId].perNFTPrice * apr) / 10000;
        ...
        if (repurchaseLimit[tokenId] > 0) {
            uint256 total_evc = getAmountout(repurchaseLimit[tokenId]);
            require(total_evc < (nft_cost * 4), "you need to repurchase the nft first");
        }
    ...
    function setRepurchaseamount(uint256 tokenid, uint256 amount) public onlyOwner {
        repurchaseLimit[tokenid] = amount;
    }
