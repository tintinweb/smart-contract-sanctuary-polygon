...
contract DexilonTest_v11 {
    ...
    */
    function btcToUsdcMarketConvert(int assetAmount) public view returns (int) {
        ...
        return (signedBtcToUsdPrice*assetAmount/signedUsdcToUsdPrice)/int(10**(DECIMALS_BTC - DECIMALS_USDC));
    }
        ...
        returns (uint256 , uint256 , userAssetParameters[] memory ) 
    {
        ...
        uint256 lockedBalance = calculateTotalLocked(user) / uint(ROUNDING_FACTOR);

        userAssetParameters[] memory userAssets = new userAssetParameters[](assetsNames.length);

        for (uint256 i=0; i < assetsNames.length; i++) {
            userAssets[i] = _setOutputDecimalsForAsset(usersAssetsData[user][assetsNames[i]]);
        }
    ...
    **/
    function updateUserBalances(
        ...
        int leverage = int(uint256(usersAssetsData[user][asset].assetLeverage)) * BTC_UNIT_FACTOR;
        int availableBalance = int(usersAvailableBalances[user]) + tradeFee * ROUNDING_FACTOR;

        if(bytes(usersAssetsData[user][asset].assetName).length == 0){
        ...
        if (((assetAmount > 0 && assetBalance < 0) || 
            ...
            availableBalance = availableBalance  
                ...
                - assetBalance*(assetPrice - assetRate)/(BTC_UNIT_FACTOR);
        ...
        if (assetBalance == 0  
            ...
            availableBalance = availableBalance 
                ...
                + assetAmount*(assetPrice - assetRate)/(BTC_UNIT_FACTOR);
    ...
    function batchFundingRateFees(FeesArray[] memory batchFees) public onlyOwner {
        ...
        for (uint256 i=0; i < max; i++) {
            if (batchFees[i].feeAmount < 0 ) {
                ...
                require(usersAvailableBalances[batchFees[i].userAddress] >= uint(-batchFees[i].feeAmount), 
                        "Not enough balance!");
