...
contract FootballBets is Ownable {
    ...
    function buyTicket(uint256 _matchId, uint8 _betTypeId, uint8 _betDoor, uint32 _betOdd, uint256 _betAmount) public checkBlackList  returns (uint256 _ticketIndex) {
        uint256 _betAmountWithFee = _betAmount.mul(10**2-tokenAddress.getBuyTicketTax()).div(10 ** 2);
        require(_betAmountWithFee >= standardPrice, "_betAmount less than standard price");

        uint256 _maxBetAmount = maxBetAmount;
        require(_betAmountWithFee <= _maxBetAmount, "_betAmount exceeds _maxBetAmount");
        ...
        totalBetAmount = totalBetAmount.add(_betAmountWithFee);
        playerStats[_player].totalBet = playerStats[_player].totalBet.add(_betAmountWithFee);
        ticketsOf[_player].push(_ticketIndex);

        emit NewTicket(_player, _ticketIndex, _matchId, _betTypeId, _betAmountWithFee, _betDoor, _betOdd, betType.goalRate, block.timestamp);
    }
