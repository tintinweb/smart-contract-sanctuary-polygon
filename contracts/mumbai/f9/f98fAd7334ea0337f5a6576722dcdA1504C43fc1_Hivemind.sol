 ...
 * @notice Hivemind can be interpreted as the brain of the Honey Protocol, is the one who decides who can borrow and how much can borrow
 * @author mario
 */
contract Hivemind is HivemindV1Storage, HivemindI, ErrorReporter, AccessControlEnumerable, ReentrancyGuard {
  ...
  uint256 private updateThreshold = 86400;
  ...
  function exitMarket(HTokenI _hToken) external override nonReentrant {
    ...
    HTokenI[] memory userAssetList = accountMarkets[msg.sender];
    ...
    HTokenI[] storage storedList = accountMarkets[msg.sender];
  ...
  function getCollateralPriceInUnderlyingPrice(HTokenI _hToken) internal view returns (uint256) {
    (uint128 nftPriceInETH, uint128 lastUpdated) = oracle.getUnderlyingFloorNFTPrice(
  ...
  function getHypotheticalAccountLiquidityRedeemInternal(
    ...
    HTokenI[] memory assets = accountMarkets[_account];
    uint256 i;
  ...
  function _setCollateralFactor(HTokenI _hToken, uint256 _newCollateralFactorMantissa) external {
    ...
    (uint128 oraclePrice, ) = oracle.getUnderlyingFloorNFTPrice(address(_hToken.collateralToken()), _hToken.decimals());
    // If collateral factor != 0, fail if price == 0
    if (
      _newCollateralFactorMantissa != 0 &&
      (oracle.getUnderlyingPriceInUSD(_hToken.underlyingToken(), _hToken.decimals()) == 0 || oraclePrice == 0)
    ) {
      revert MarketError(Error.MARKET_NOT_LISTED);
    }
