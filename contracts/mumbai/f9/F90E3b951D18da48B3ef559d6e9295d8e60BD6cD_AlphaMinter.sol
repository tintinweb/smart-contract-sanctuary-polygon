...
import '../utils/PausableContract.sol';
import '../tokens/IPxlbot.sol';
import '../minting/ScionCoordinator/IScionCoordinator.sol';
import './IMinter.sol';

contract AlphaMinter is Ownable, IERC721Receiver, Controllable, PausableContract, IMinter {
  IPxlbot pxlbot;
  ...
  bool public public_mint_enabled = false;
  ...
  mapping(uint256 => bool) public claimed_playable_tokens;
  ...
  function mintScion() public payable whenNotPaused {
    require(
      canMintBot(1) == true,
      'unable to mint; either wrong caller, minting dsiabled or bot allowance exceeded'
    );
    (
      uint256 parent_id,
  ...
  function mintWithToken() external payable override whenNotPaused {
    require(
  ...
  function canMintBot(uint8 num_bots) public view returns (bool) {
    return
      isController(_msgSender()) || (bot_counter + num_bots <= bot_allowance) && public_mint_enabled;
  }
  ...
  function setPublicMintEnabled(bool _val) external onlyController {
    public_mint_enabled = _val;
  }
...
abstract contract PausableContract is Pausable, Controllable {
