...
contract TestRandom999 is ERC721, Ownable, RandomlyAssigned { // change NFT on your contract name
  using Strings for uint256;
  // uint256 public requested;
  uint256 public currentSupply = 0;
  
  string public baseURI = "https://ipfs.io/ipfs/QmcoHHT82SEhF7UXPpKfz4DPsmx9oBrq5zC5e2gc2PXK4R/";

  constructor() 
    ERC721("TEST RANDOM", "rndm") // input the name and tiker for token. the name can be with few words. its okay
    RandomlyAssigned(15,1) // set max limit acorting to your collection. Max. 10000 NFTs available; Start counting from 1 (instead of 0)
    
       { 
            mint(); // you can left only{mint();} and in such case owner get 1 nft after deploy
        }
      ...
      payable
  {
      ...
      if (msg.sender != owner()) {  
        require( msg.value >= 0.0003 ether); // set the price per a NFT
       require( balanceOf(msg.sender) <= 1); // might be a limit or what?????
      }
