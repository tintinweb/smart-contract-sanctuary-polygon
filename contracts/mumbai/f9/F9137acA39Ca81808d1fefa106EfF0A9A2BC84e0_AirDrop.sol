...
import "./interfaces/ISRPManagement.sol";

/**
   @title AirDrop contract
   @dev This contract (version 1) is being used to support Air Drop event
...
contract AirDrop is Context {
	ISRPManagement public gov;

	bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
	bytes32 public constant ADMIN_ROLE = 0x00;
    bytes32 public constant VERSION = keccak256("AIRDROP_v1");
	...
	constructor(address _gov) {
		gov = ISRPManagement(_gov);
	}

	/**
       	@notice Update Address of SRPManagement contract
       	@dev  Caller must have ADMIN_ROLE
		@param	_gov				Address of SRPManagement contract (or address(0))
    ...
    */
	function claim(
        ...
        require(_nftType == NFT721 || _nftType == NFT1155, "Type not supported");
        if(_nftType == NFT721)
		...
		bytes32 _leaf = keccak256(
			abi.encodePacked(
                _user, _tokenID, _eventID, _nftType, _token, _amount, _distributor
            )
		);
		require(
			MerkleProof.verify(_proof, _root, _leaf), "Invalid claiming request"
		);

        if (_nftType == NFT721)
		    IERC721(_token).safeTransferFrom(_distributor, _user, _tokenID);
        else 
            IERC1155(_token).safeTransferFrom(_distributor, _user, _tokenID, _amount, "");
