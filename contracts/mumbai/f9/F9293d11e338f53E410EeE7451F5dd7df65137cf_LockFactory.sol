...
abstract contract DeployFee is Context {
    ...
    IERC20Metadata public discountToken;
    ...
    AggregatorV3Interface internal discountTokenPriceFeed;
        ...
        returns (uint256 fixedTokenAmount, uint256 percentageTokenAmount)
    {
        ...
        if(paymentToken == discountToken) {
            fixedTokenAmount -= fixedTokenAmount / ( EXP_VALUE / discountAmount );
            percentageTokenAmount -= percentageTokenAmount / ( EXP_VALUE / discountAmount );
        }
    }
    ...
    function setDiscountTokenInternal(
        ...
        address _discountPriceFeed,
        uint256 _discountAmount
    ) internal {
        require(_discountToken != address(0) && _discountPriceFeed != address(0), "Cant set address 0");
        require(discountAmount < EXP_VALUE, "Discount cant be more than 100%");
        if(address(discountToken) != address(0)) {
            isPaymentToken[address(discountToken)] = false;
        }
        discountToken = IERC20Metadata(_discountToken);
        discountTokenPriceFeed = AggregatorV3Interface(_discountPriceFeed);
        discountAmount = _discountAmount;
        isPaymentToken[_discountToken] = true;
    }
...
contract LockFactory is AccessControlEnumerable, DeployFee {
    ...
    function setDiscountToken(
        ...
        setDiscountTokenInternal(_discountToken, _discountPriceFeed, _discountAmount);
    }
