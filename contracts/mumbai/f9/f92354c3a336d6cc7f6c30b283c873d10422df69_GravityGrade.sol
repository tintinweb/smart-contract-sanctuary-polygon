 ...
 */
contract GravityGrade is IGravityGrade, OwnableUpgradeable, ERC1155Upgradeable {
    /* Type Declarations */
    struct Sale {
        uint256 saleId;
        uint256 tokenId;
    ...
    function setModerator(address _moderatorAddress) external onlyOwner {
        if (_moderatorAddress == address(0)) revert GravityGrade__ZeroAddress();
        ...
        onlyGov
    {
        ...
        if (sum + _info.basisPoints >= 10000)
            revert GravityGrade__ValueTooLarge(sum + _info.basisPoints, 10000);
    ...
    /**
     * @notice adaptor to allow purchases via transakOne
    ...
    function _buyPacks(
        ...
        if (_currency == ADDRESS_ZERO) {
            ethPayment(_recipient, info.defaultCurrency, _currency, _numPurchases, info.salePrice);
        } else {
            erc20Payment(
                info,
                _saleId,
    ...
    function ethPayment(
        ...
        uint256 ethPrice = s_oracle.getAmountOut(
            _defaultCurrency,
            _currency,
            _numPurchases * _salePrice
        );
    ...
    function erc20Payment(
        ...
        uint256 balance = _apply_bulk_discount(
            _saleId,
            _info.salePrice * _numPurchases,
            _numPurchases
        );
        if (_currency != _info.defaultCurrency) {
            balance = s_oracle.getAmountOut(
                _info.defaultCurrency,
                _currency,
                _numPurchases * _info.salePrice
            );
        }
