...
{
    event Withdrawal(string _subscriptionId, uint256 _fee, uint256 _balance);
    event Renewal(string _subscriberId, string _subscriptionId, address walletAddress, uint256 expiration, uint256 _amount);
    ...
    event Subscribed(string _subscriberId, string _subscriptionId, uint256 _amount);
    event Cancelled(string _subscriberId);
    ...
    function _subscribe(string memory _subscriptionId, string memory _subscriberId, Subscriber memory _subscriber) internal virtual {
        ...
        emit Subscribed(_subscriberId, _subscriptionId, subscription.price);
    }

    function _cancel(string memory subscriberId) internal virtual {
        delete subscribers[subscriberId];
        emit Cancelled(subscriberId);
    }
    ...
    function _withdrawBalance(string memory _subscriptionId, Subscription storage _subscription) internal virtual {
        ...
        emit Withdrawal(_subscriptionId, fee, _balance);
    }
    ...
    function _renewSubscription(string memory _subscriberId, string memory _subscriptionId, Subscriber storage _subscriber) internal virtual {
        ...
        emit Renewal(_subscriberId, _subscriptionId, _msgSender(), expiration, _subscription.price);
    }
 ...
 */
abstract contract ERC2771ContextUpgradeableModified is Initializable, ContextUpgradeable {
    mapping(address => bool) internal _trustedForwarder;
