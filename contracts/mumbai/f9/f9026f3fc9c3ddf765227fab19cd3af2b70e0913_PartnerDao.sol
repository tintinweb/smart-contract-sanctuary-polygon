 ...
 *         Eternity Deck game
 */
contract PartnerDao is Ownable {
  /**
  ...
  /**
   * @dev Fired in addEternityAddress function
   *
   * @param eternityAddress one of the eternity infrastructure addresses
   */
  event EternityAddressAdded(address eternityAddress);

  /**
   * @dev Fired in removeEternityAddress function
   *
   * @param eternityAddress one of the eternity infrastructure addresses
   */
  event EternityAddressRemoved(address eternityAddress);
   ...
   */
  function withdraw(uint256 _amount) external onlyIfShareholderHasBalance(msg.sender, _amount) {
   ...
   */
  function forceWithdraw(address _addr, uint256 _amount) external onlyOwner onlyIfShareholderHasBalance(_addr, _amount) {
   ...
   */
  function dangerousForceWithdraw(uint256 _amount) external onlyOwner {
    (bool success, ) = payable(msg.sender).call{value: _amount}("");
    require(success, "can not withdraw");
