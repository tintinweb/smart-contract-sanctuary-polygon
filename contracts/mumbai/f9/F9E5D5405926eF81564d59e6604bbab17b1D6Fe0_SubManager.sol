...
import {BokkyPooBahsDateTimeLibrary as TimeLib} from "./DateTimeLibrary.sol";
...
library Period {
        ...
        returns (uint ts)
    {
        if (period == PeriodType.DAY) {
            ...
            ts = TimeLib.addDays(curTimestamp, 7);
    ...
    function getPeriodTimestamp(
        ...
        if (period == PeriodType.DAY) {
            ...
            ts = TimeLib.addDays(curTimestamp, count * 7);
        } else if (period == PeriodType.MONTH) {
            ts = TimeLib.addMonths(curTimestamp, count);
        } else if (period == PeriodType.QUARTER) {
            ts = TimeLib.addMonths(curTimestamp, count * 3);
        } else if (period == PeriodType.YEAR) {
