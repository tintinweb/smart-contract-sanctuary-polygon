...
contract StakingReward is OwnerPausable {
    ...
    uint256 private immutable BASE_REWARD_PER_BLOCK; //1%的apy 1个区块的奖励
    ...
    function calculatePendingRewards() external view returns(UserInfo memory) {
        address staker = msg.sender;
    ...
    function userInfo() external view returns(UserInfo memory) {
        return _userInfo[msg.sender];
    }

    function orders() external view returns(OrderInfo[] memory) {
