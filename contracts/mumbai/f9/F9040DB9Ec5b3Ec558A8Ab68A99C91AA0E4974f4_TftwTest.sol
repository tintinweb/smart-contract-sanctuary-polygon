...
contract TftwTest is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    ...
    constructor() ERC721("TftwTest", "TftwT") {}
    ...
    function safeMint(address to) public payable {
        if (_tokenIds.current() < 4) {
            require(msg.sender == owner(), "Reserved token.");
    ...
    function safeBigPackMint(address to) public payable {
        ...
        bigPackSale++;
        for (uint256 i = 0; i < bigPackQuantity; i++) {
            _tokenIds.increment();
            _safeMint(to, _tokenIds.current());
        }
    }
    ...
    function safeMediumBigPackMint(address to) public payable {
        ...
        mediumPackSale++;
        for (uint256 i = 0; i < mediumPackQuantity; i++) {
            _tokenIds.increment();
            _safeMint(to, _tokenIds.current());
        }
    }
    ...
    function safeSmallBigPackMint(address to) public payable {
        ...
        smallPackSale++;
        for (uint256 i = 0; i < smallPackQuantity; i++) {
            _tokenIds.increment();
            _safeMint(to, _tokenIds.current());
        }
    }
