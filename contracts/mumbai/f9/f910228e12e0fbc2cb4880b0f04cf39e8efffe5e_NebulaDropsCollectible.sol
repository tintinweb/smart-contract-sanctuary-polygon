...
contract NebulaDropsCollectible is ERC1155, Ownable {
    ...
    public onlyOwner
    {
        allowArtistToMintTokens[_allowedminter] =  ApprovedArtists4Minting (
            // _dropid,
            _allowedminter,
            _tokensowner,
            _totaltokens
    ...
    function rejectApprovedDrops(address  _allowedminter) public onlyOwner{
        delete allowArtistToMintTokens[_allowedminter];  
    }

    /* this will return a list of all whitelisted Marketplaces..*/
    function fetchApprovedDropsList(address _allowedminter) public view returns(ApprovedArtists4Minting memory) {
      return allowArtistToMintTokens[_allowedminter];  
    }


    function mint(uint256 _id, uint256 _amount, address _nebulaMarketplace) 
        public  payable  
     {
       ...
       // if(mintedTokensByArtists[_dropid].tokensMinted > 0){
        ...
        address _tokensOwner = allowArtistToMintTokens[msg.sender].tokensOwner;
        ...
        setApprovalForAll(_nebulaMarketplace,true);
        ...
        emit TokenMinted(_tokensOwner, _id);
    }


    function mintBatch(uint256[] memory _ids, uint256[] memory _amounts, address _nebulaMarketplace)
        public payable
    {
        // string memory _name, string memory _newuri, 
        ApprovedArtists4Minting memory _approvedArtist =   allowArtistToMintTokens[msg.sender];
