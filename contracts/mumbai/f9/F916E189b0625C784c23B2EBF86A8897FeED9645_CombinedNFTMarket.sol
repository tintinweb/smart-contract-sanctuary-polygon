...
contract CombinedNFTMarket is ReentrancyGuard, Ownable {
    ...
    function pauseBids(bool _state) public onlyOwner {
        bidsPaused = _state;
    }

    function setCurrentItemId(uint256 itemId_) public onlyOwner {
        require(itemId_>= _itemIds, "New item id must be greater or equal to current item id");
        _itemIds = itemId_;
    }

    function setCurrentBidItemId(uint256 bidItemId_) public onlyOwner {
        require(bidItemId_>= _bidItemIds, "New bid item id must be greater or equal to current item id");
    ...
    function migrateMarketItem(address itemOwner, uint256 itemId, bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        ...
        if (isErc721) {
            amount = 1;
            require(IERC721(nftContract).ownerOf(tokenId) == itemOwner, "ERC721 token does not belong to the author.");
            if (!IERC721(nftContract).isApprovedForAll(address(itemOwner), address(this))) {
                revert("Missing IERC721 approval.");
            }
        } else {
            require(amount > 0, "ERC1155 token amount must be > 0");
            require(IERC1155(nftContract).balanceOf(itemOwner, tokenId) >= amount, "ERC1155 token balance is not sufficient for the seller..");
            if (!IERC1155(nftContract).isApprovedForAll(address(itemOwner), address(this))) {
                revert("Missing IERC1155 approval.");
            }
        }
    ...
    function createMarketItem(bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        ...
        if (isErc721) {
            amount = 1;
            require(IERC721(nftContract).ownerOf(tokenId) == msg.sender, "ERC721 token does not belong to the author.");
        } else {
            require(amount > 0, "ERC1155 token amount must be > 0");
            require(IERC1155(nftContract).balanceOf(msg.sender, tokenId) >= amount, "ERC1155 token balance is not sufficient for the seller..");
        }

        //TODO: have to check duplicates isErc721, State.INITIATED, nftContract,tokenId, seller
    ...
    function updateMarketItem(uint256 itemId, uint256 price) public payable nonReentrant {
        require(!paused, "Contract is paused");
        MarketItem memory item = idToMarketItem[itemId];
        require(item.state == State.INITIATED, "Listing is not in INITIATED state. Aborting.");
        require(item.seller == msg.sender || msg.sender == owner(), "Listing can't be edited from other than seller or owner. Aborting.");
        require(item.price > price, "New price must be less than exiting price");

        if (item.isErc721) {
            require(IERC721(item.nftContract).ownerOf(item.tokenId) == msg.sender, "ERC721 token does not belong to the author.");
        } else {
            require(IERC1155(item.nftContract).balanceOf(msg.sender, item.tokenId) >= item.amount, "ERC1155 token balance is not sufficient for the seller..");
        }
    ...
    function cancelMarketItem(uint256 itemId) public virtual {
        MarketItem memory item = idToMarketItem[itemId];
        require(item.state == State.INITIATED, "Listing is not in INITIATED state. Aborting.");
        require(item.seller == msg.sender || msg.sender == owner(), "Listing can't be cancelled from other than seller or owner. Aborting.");
    ...
    function createMarketSale(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        require(!paused, "Contract is paused");
        MarketItem memory item = idToMarketItem[itemId];
        require(item.state == State.INITIATED, "MarketItem is in wrong state");
        require(item.seller != msg.sender, "Listing can't be sold to the same buyer. Aborting.");
        require(amount > 0, "Token amount must be > 0");
        require(amount <= item.amount, "Token amount must less or equal to market item amount");
        require(item.erc20Address == erc20Address, "ERC20 token address as a payer method should be the same as in the listing. Either listing, or method call has wrong ERC20 address.");


        if (item.isErc721) {
            require(amount == 1, "Token amount must be equal to 1");
            ...
            if (IERC721(item.nftContract).ownerOf(item.tokenId) != item.seller) {
                revert("Insufficient balance of the asset in this listing. Probably was already sold.");
            }
        } else {
        ...
        if (item.erc20Address == address(0)) {
            require(msg.sender.balance >= priceTotal.add(fee), "Insufficient balance to pay for the asset.");
            if (msg.value < priceTotal.add(fee)) {
            ...
            if (msg.value.sub(priceTotal).sub(fee) > 0) {
                Address.sendValue(payable(msg.sender), msg.value.sub(priceTotal).sub(fee));
            }
        } else {
            IERC20 token = IERC20(item.erc20Address);
            require(token.balanceOf(msg.sender) >= priceTotal.add(fee), "Insufficient ERC20 balance for paying for the asset.");
            if (priceTotal.add(fee) > token.allowance(msg.sender, address(this))) {
    ...
    function migrateMarketBidItem(address itemOwner, uint256 itemId, bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        require(_managers[msg.sender], "!manager");
        require(erc20Address != address(0), "Native token bids not supported");
        ...
        require(token.balanceOf(itemOwner) >= priceTotal, "Insufficient ERC20 balance for paying for the asset.");
    ...
    function createMarketBidItem(bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        require(!bidsPaused, "Contract is paused");
        require(erc20Address != address(0), "Native token bids not supported");
        //check that bidder has the funds
        require(price > 0, "Price must be at least 1 wei");
        if (isErc721) {
        ...
        IERC20 token = IERC20(erc20Address);
        require(token.balanceOf(msg.sender) >= priceTotal, "Insufficient ERC20 balance for paying for the asset.");
    ...
    function updateMarketBidItem(uint256 itemId, uint256 price) public payable nonReentrant {
        ...
        require(price > 0, "Price must be at least 1 wei");
        MarketBidItem memory item = idToMarketBidItem[itemId];
        require(item.state == BidState.INITIATED, "Listing is not in INITIATED state. Aborting.");
        require(item.bidder == msg.sender || msg.sender == owner(), "Listing can't be edited from other than bidder or owner. Aborting.");
        require(item.price < price, "New price must be more than exiting price");
    ...
    function cancelMarketBidItem(uint256 itemId) public virtual {
        MarketBidItem memory item = idToMarketBidItem[itemId];
        require(item.state == BidState.INITIATED, "Listing is not in INITIATED state. Aborting.");
        require(item.bidder == msg.sender || msg.sender == owner(), "Listing can't be cancelled from other than bidder or owner. Aborting.");
    ...
    function createMarketAccept(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        ...
        MarketBidItem memory item = idToMarketBidItem[itemId];
        require(item.state == BidState.INITIATED, "MarketBidItem is in wrong state");
        require(item.bidder != msg.sender, "Listing can't be accepted to the same owner. Aborting.");
        require(amount > 0, "Token amount must be > 0");
        require(amount <= item.amount, "Token amount must less or equal to market item amount");
        require(item.erc20Address == erc20Address, "ERC20 token address as a payer method should be the same as in the listing. Either listing, or method call has wrong ERC20 address.");
        ...
        if (item.isErc721) {
            ...
            if (IERC721(item.nftContract).ownerOf(item.tokenId) != msg.sender) {
                revert("Insufficient balance of the asset in this listing. Probably was already accepted.");
            }
        } else {
        ...
        // msg.sender will receive price.sub(fee)
        uint256 priceTotal = item.price.mul(amount);
        uint256 fee = priceTotal.mul(_marketplaceFee).div(10000);
        uint256 remainAmount = item.amount.sub(amount);
        ...
        IERC20 token = IERC20(item.erc20Address);
        require(token.balanceOf(item.bidder) >= priceTotal.add(fee), "Insufficient bidder ERC20 balance for paying for the asset.");
        if (priceTotal.add(fee) > token.allowance(item.bidder, address(this))) {
            revert("Insufficient bidder ERC20 allowance balance for paying for the asset.");
        }
