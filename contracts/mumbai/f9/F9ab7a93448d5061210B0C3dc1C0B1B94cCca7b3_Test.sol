...
contract Test{    
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external payable {
        ...
        if (matrix == 1) 
        {
            require((msg.value*(maticRate/1e18)) >= levelPrice[level], "invalid price!");            
        ...
        else 
        {
            ...
            require((msg.value*(maticRate/1e18)) >= mlevelPrice[level], "invalid price!");            
    ...
    function registration(address userAddress, address referrerAddress, uint256 id) private{
        ...
        //=========== start mt3 matrix ==========
            ...
            if (users[userAddress].x3Matrix[1].referrals.length == 2) {
                currentMT3Id++;
            }
        //=========== end mt3 matrix ==========
       ...
       //       address(uint160(owner)).transferToken((mlevelPrice[level]*5)/100,tokenId);
        //        uint256 ded=(mlevelPrice[level]*5)/100+(mlevelPrice[1]);
            ...
            if(users[referrerAddress].x6Matrix[level].referrals.length < level+3) 
            {
                users[referrerAddress].x6Matrix[level].referrals.push(userAddress);
                emit NewUserPlace(userAddress, referrerAddress, 2, level, uint8(users[referrerAddress].x6Matrix[level].referrals.length));
                emit MatrixBonus(referrerAddress, amount, (level-1),(amount*1e18)/maticRate);
    ...
    function autoUpgrade(address _user, uint8 level) private{
            ...
            uint256 maticPerDollor=(users[_user].holdAmount[level-1]*1e18)/totalMatrixValue;      
             ...
             users[_user].x6Matrix[level].referrals =new address[](0);
             users[_user].activeX6Levels[level]=false;
    ...
    function withdrawLevelIncome() public{
        ...
        for(i=0;i<10;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payUpId], msg.sender, rest/100, (rest/100)/maticRate, i+1,1);
        }
        ...
        for(i=0;i<10;i++)
        {
           ...
           emit CommunityBonus(idToAddress2[payDownId], msg.sender, rest/100, (rest/100)/maticRate, i+1,2);
        }
