...
contract Cubit is ERC20, ERC20Burnable, ERC20Snapshot, Ownable, Pausable, ERC20Permit, ERC20Votes, ERC20Capped {
    
    
    constructor() ERC20("Cubit", "CBIT") ERC20Permit("Cubit") ERC20Capped(21000000 ether) {}
...
// File: contracts/ERC2771Context_Upgradeable.sol
...
contract ERC20MFab is ERC2771Context_Upgradeable, System, Ownable {
  Cubit private _token;

  constructor(address _forwarder, bytes32 _systemId) ERC2771Context_Upgradeable(_forwarder) System(_systemId) {
    _token = Cubit(0x3dde32b69652fB3a2A5Aed8833792E9fD9e6eb76); _paused = false;// Replace with the actual address of the Cubits contract
  }
  ...
  function getCubitTotalSupply() public view returns (uint256) {
    return _token.totalSupply();
  }

  function transferFrom(address sender, address recipient, uint256 amount) public returns (bool) {
    require(_token.approve(_msgSender(), amount), "Approval failed"); // Approve the transfer of tokens from `sender` to this contract

    require(_token.transferFrom(sender, recipient, amount), "Transfer failed"); // Call the transferFrom function on the Cubit contract
  ...
  function transfer(address recipient, uint256 amount) public returns (bool) {
    require(Cubit(address(_token)).balanceOf(_msgSender()) >= amount, "Insufficient balance in Cubit");

    require(Cubit(address(_token)).transfer(recipient, amount), "Transfer failed");

    emit Transfer(address(Cubit(address(_token))), recipient, amount);
  ...
  function balanceOf(address account) public view returns (uint256) {
    return Cubit(address(_token)).balanceOf(account);
  }
  ...
  //Pause and Unpause Contract Features.
  event Paused(address account);
