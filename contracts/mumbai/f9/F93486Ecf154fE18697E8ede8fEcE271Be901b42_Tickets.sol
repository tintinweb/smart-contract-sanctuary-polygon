    ...
    ERC721BurnableUpgradeable
{
    ...
    function mintTicket(uint256 ticketId, address to)
        public
        whenNotPaused
        onlyRole(MINTER_ROLE)
        returns (bool)
    {
        return _mintTicket(ticketId, to);
    }

    function mintTicketsToManyUsers(uint256 ticketId, address[] memory to)
        public
        whenNotPaused
        onlyRole(MINTER_ROLE)
        returns (bool)
    {
        for (uint256 i = 0; i < to.length; ++i) {
            _mintTicket(ticketId, to[i]);
        }
        return true;
    }

    function burn(uint256 token) public override {
        require(_isApprovedOrOwner(msg.sender, token));
        _burn(token);
    }
    ...
    function _baseURI() internal pure override returns (string memory) {
        return "https://us-central1-welook-siwe.cloudfunctions.net/ipfs/v2/t/";
    }
    ...
    function _mintTicket(uint256 ticketId, address to) internal returns (bool) {
    ...
    {
        uint ticketId = _token[tokenId];
        return
            string.concat(
                _baseURI(),
                StringsUpgradeable.toString(ticketId),
                "/",
