...
contract Target is IXReceiver {
    ...
    IERC20 public farmInputToken = IERC20(0xeDb95D8037f769B72AAab41deeC92903A98C9E16);
    IFarm public farm = IFarm(0xeE9dc3d0a64202F29827AE3763908C6Dbc493c4A);

    // The connext contract deployed on the same domain as this contract
    IConnext public connext = IConnext(0x173d82FF0294d4bb83A3AAF30Be958Cbc6D809f7);
    ...
    function xReceive(
        ...
        (address userAddress, uint256 depositAmount) = abi.decode(_callData, (address, uint256));

        _depositFor(userAddress, depositAmount);
    }

    function _depositFor(address userAddress, uint256 depositAmount) internal {
        farmInputToken.approve(address(farm), depositAmount);
        farm.depositFor(depositAmount, userAddress);
        // The nested xcall
        sendPong();
    }

    function sendPong() public payable {
        farmInputToken.approve(address(connext), 10 * 1e18);
        connext.xcall{value: 0}(
            1735353714, // _destination: Domain ID of the destination chain
            0xf7A4C1cCFa41aE8B875A1802D014deF201eb34ac, // _to: address of the target contract (Ping)
            address(farmInputToken), // _asset: address of the token contract
            msg.sender, // _delegate: address that can revert or forceLocal on destination
            10 * 1e18, // _amount: amount of tokens to transfer
