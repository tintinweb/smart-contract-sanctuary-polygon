...
contract CLLConstructorCLH is CLStorage {

	function __CLHConstructor(
		address _owner, 
        ...
        external
    {
        if( __GOV_DICTATORSHIP__ == _govModel ){
            govRuleApprovPercentage = 100;
            govRuleMaxUsers = _govRules[ uint256( gRule.maxUsers ) ];
            govRuleMaxManagers = 1;
        }
        else if( __GOV_COMMITTEE__ == _govModel ){
            govRuleApprovPercentage = _govRules[ uint256( gRule.approvPercentage ) ];
            govRuleMaxUsers = _govRules[ uint256( gRule.maxUsers ) ];
            govRuleMaxManagers = _govRules[ uint256( gRule.maxManagers ) ];
        }
        else if( __GOV_SIMPLE_MAJORITY__ == _govModel ){
            govRuleApprovPercentage = _govRules[ uint256( gRule.approvPercentage ) ];
            govRuleMaxUsers = _govRules[ uint256( gRule.maxUsers ) ];
            govRuleMaxManagers = _govRules[ uint256( gRule.maxUsers ) ];
        }
        else
            revert InvalidGovernanceType( _govModel );

        HOUSE_NAME = _houseName;
        HOUSE_GOVERNANCE_MODEL = _govModel;
        housePrivate = _housePrivate;
        houseOpen = _housePrivate ? false : _houseOpen;
        CCLFACTORY = ICLFactory( _CLC[ uint256( eCLC.CLFACTORY ) ] );
        CLHAPI = _CLC[ uint256( eCLC.CLHAPI ) ];
        CLHSAFE = _CLC[ uint256( eCLC.CLHSAFE ) ];
        whiteListNFT = _CLC[ uint256( eCLC.whiteListNFT ) ];

        nftAdmin = ( new CLHNFT )( {
            _name: string.concat("Admin NFT for ", _houseName),
            _symbol: "CLHADM"
        } );

        nftMember = ( new CLHNFT )( {
            _name: string.concat("User NFT for ", _houseName),
            _symbol: "CLHMMR"
        } );

        nftInvitation = ( new CLHNFT )( {
            _name: string.concat("Invitation NFT for ", _houseName),
            _symbol: "CLHIVT"
        } );
        ...
        if( 
            ...
            &&
            ( __GOV_COMMITTEE__ == _govModel || __GOV_SIMPLE_MAJORITY__ == _govModel )
        ) {
            for( uint8 wid = 0 ; wid < _ManagerWallets.length ; wid++ ){
                if( address( 0 ) != _ManagerWallets[ wid ] ) {
                    ( successDGTCLL, ) = CLLUserManagement.delegatecall(
                        abi.encodeWithSignature( 
        ...
        vaultCLH = ( new CLVault )( { _CLH: address( this ) } );
    }
}
