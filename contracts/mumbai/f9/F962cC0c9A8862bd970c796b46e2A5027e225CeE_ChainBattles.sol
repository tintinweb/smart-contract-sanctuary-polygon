...
contract ChainBattles is ERC721URIStorage  {
    ...
    function mint() public {
      ...
      tokenIdToInfo[newItemId] = Info(0, 1, 1, 1);
      _setTokenURI(newItemId, getTokenURI(newItemId));
    }

    function train(uint256 tokenId, uint addPoint) public {
      require(_exists(tokenId));
      require(ownerOf(tokenId) == msg.sender, "You must own this NFT to train it!");
      Info memory currentInfo = tokenIdToInfo[tokenId];
      require(addPoint == 1 || addPoint == 2 || addPoint == 3, "You can only train with 1, 2 or 3!"); 
      Info memory newInfo = Info(
          currentInfo.level + 1, 
          currentInfo.speed, 
          currentInfo.strength,
          currentInfo.life
      );
      if(addPoint == 1){
        newInfo.speed += 1;
  ...
  function getSpeed(uint256 tokenId) public view returns (string memory) {
    Info memory info = tokenIdToInfo[tokenId];
    return info.speed.toString();
  }

  function getStrength(uint256 tokenId) public view returns (string memory) {
    Info memory info = tokenIdToInfo[tokenId];
    return info.strength.toString();
  }

  function getLife(uint256 tokenId) public view returns (string memory) {
    Info memory info = tokenIdToInfo[tokenId];
    return info.life.toString();
  }
