...
contract GoBots is ERC721, IERC721Enumerable, Ownable {
    ...
    constructor(address _feeReceiver, uint16 initialTotalMintLimit, uint16 initialOwnershipMintLimit, uint256 initialMintPrice, string memory initialMetadataBaseURI, string memory initialDefaultContentBaseURI) ERC721("Go!Bots", "\u22A1") Ownable() {
    ...
    function mintToken(uint256 tokenId) public payable {
        ...
        require(mintedCount() + 1 <= totalMintLimit, "reached current minting limit");
    ...
    function _mint(uint256 tokenId) internal {
        require(tokenId > 0 && tokenId <= SUPPLY_LIMIT, "invalid tokenId");
        _ownedTokensIds[msg.sender].push(tokenId);
        super._safeMint(msg.sender, tokenId);
    }

    function getAllIdsByOwned(address _account) external view returns(uint256[] memory) {
        return _ownedTokensIds[_account];
    }
    ...
    function tokenByIndex(uint256 index) public pure override returns (uint256) {
        require(index >= 0 && index < SUPPLY_LIMIT, "invalid index");
        return index + 1;
    }
