...
contract swapHelperDiyTask2 is OpsTaskCreator, Ownable {
    ...
    function editTask(bytes32 _taskID, uint256 _swapAmount, uint256 _interval, uint256[] memory _start_end_Time, uint256[] memory _timeList) external {
        require((_start_end_Time.length == 2) && (_start_end_Time[1] > _start_end_Time[0]) && (_timeList.length % 2 == 0), "e001");
        taskConfig storage y = taskConfigList[_taskID];
        require(msg.sender == owner() || msg.sender == y._owner, "e002");
