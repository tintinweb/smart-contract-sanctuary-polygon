...
contract Instruments is ERC721Base {
    ...
    function removeCertToken(uint256 tokenId, uint256 certTokenId) public {
        require(_exists(tokenId), "Not exist");
        require(_existsCertToken(certTokenId), "Token not exist");

        uint256 certTokenIndex;
        ...
        // require(IERC721(certToken.contractAddress).ownerOf(certTokenId) == address(this),
        //     "Cert token not owned");
    ...
    function _existsCertToken(uint256 tokenId) public view returns (bool) {
        Token[] memory certTokens = _certTokens[tokenId];

        for (uint i = 0; i < certTokens.length; i++) {
            if (certTokens[i].tokenId == tokenId)  {
                return true;
            }
        }
