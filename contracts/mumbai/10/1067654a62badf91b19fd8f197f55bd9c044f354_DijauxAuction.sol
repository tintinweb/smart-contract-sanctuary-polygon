...
contract DijauxAuction is DijauxFunctions, ERC721HolderUpgradeable {
    ...
    event SetAuction(address seller, uint256 tokenId);
    event Bid(address bidder, uint256 price);
    event EndAuction(address owner, uint256 tokenId, uint256 finalprice);
    ...
    function CreateAuction(
        ...
        emit SetAuction(msg.sender, _tokenId);
    }
    ...
    function FinalizedAuction(uint256 _tokenId, uint256 _charge) public payable {
        ...
        if(auction.auction_highestbidder != address(0)) {
            ...
            emit EndAuction(msg.sender, _tokenId, auction.auction_highestbid);
        }else {
...
import "./DijauxData.sol";

contract DijauxFunctions is DijauxData {

    event BuyNFT(address buyer, uint256 tokenId, uint256 _charge);
    event ChangePrice(uint256 tokenId, uint256 _newPrice);
    event ToggleSale(uint256 tokenId, bool status);

    function BuyNFTToken(uint256 _tokenId, uint256 _charge) public payable {
        address tokenOwner = ownerOf(_tokenId);
        require(tokenOwner != msg.sender, "must not owner");
        DijauxPiece memory dijauxpiecemem = allDijauxPieces[_tokenId];
        require(dijauxpiecemem.price == msg.value, "price not match");
        require(dijauxpiecemem.forsale, "not sale");
        _transfer(ownerOf(_tokenId), msg.sender, _tokenId);

        uint256 ChargeGoesToCreator = ((msg.value - ((msg.value * _charge) / 100)) * dijauxpiecemem.royalties) / 100;
        uint256 finalValuePrice = (msg.value - ((msg.value * _charge) / 100)) - ChargeGoesToCreator;

        payable(0x01a9223C008F5e86a54776c00BAB58141dD8D008).transfer(((msg.value * _charge) / 100) / 2);
        payable(0xb81593939decf7F053c9Dc9bc69BFB8E2A90E9eB).transfer(((msg.value * _charge) / 100) / 2);

        payable(dijauxpiecemem.creator).transfer(ChargeGoesToCreator);
        payable(tokenOwner).transfer(finalValuePrice);
        dijauxpiecemem.forsale = false;
        allDijauxPieces[_tokenId] = dijauxpiecemem;

        emit BuyNFT(msg.sender, _tokenId, _charge);
    }

    function changeTokenPrice(uint256 _tokenId, uint256 _newPrice) public {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
        DijauxPiece memory dijauxpiecemem = allDijauxPieces[_tokenId];
        dijauxpiecemem.price = _newPrice;
        allDijauxPieces[_tokenId] = dijauxpiecemem;

        emit ChangePrice(_tokenId, _newPrice);
    }

    function toggleForSale(uint256 _tokenId) public {
        require(ownerOf(_tokenId) == msg.sender, "not owner");
        DijauxPiece memory dijauxpiecemem = allDijauxPieces[_tokenId];
        if(dijauxpiecemem.forsale) {
            dijauxpiecemem.forsale = false;
            emit ToggleSale(_tokenId, false);
        } else {
            dijauxpiecemem.forsale = true;
            emit ToggleSale(_tokenId, true);
        }
        allDijauxPieces[_tokenId] = dijauxpiecemem;
    }
}
...
contract DijauxData is ERC721Upgradeable, ERC721URIStorageUpgradeable, OwnableUpgradeable, EIP712Upgradeable, AccessControlUpgradeable {

    uint256 public dijauxNFTCounter;
    ...
    mapping(uint256 => DijauxPiece) public allDijauxPieces;
    ...
    function initialize(
        ...
        __ERC721_init("Dijaux", "DUX");
        __ERC721_init_unchained("LazyMint", "MTK");
        __EIP712_init_unchained(SIGNING_DOMAIN, SIGNING_VERSION);
        _setupRole(MINTER_ROLE, minter);
    }

    function lazyMintNFT(address _to, LazyMintVoucher calldata _voucher) public payable {
        address signer = _verify(_voucher);
        require(hasRole(MINTER_ROLE, signer), "wrong role.");
        require(msg.value >= _voucher.price / (10 ** 18), "price is not enough");
        _safeMint(_to, _voucher.tokenId);
        _setTokenURI(_voucher.tokenId, _voucher.uri);

        tokenURIExists[_voucher.uri] = true;
        DijauxPiece memory newDijauxPiece = DijauxPiece(
        signer,
        _voucher.price,
        _voucher.royalties,
        _voucher.forsale);
        allDijauxPieces[_voucher.tokenId] = newDijauxPiece;
        emit Mint(_to, _voucher.tokenId);
    }

    function _hash(LazyMintVoucher calldata voucher) internal view returns(bytes32){
        return _hashTypedDataV4(keccak256(abi.encode(
            //function selector
            keccak256("LazyMintVoucher(uint256 tokenId,uint256 price,string uri,uint royalties,bool forsale)"),
            voucher.tokenId,
            voucher.price,
    ...
    function _verify(LazyMintVoucher calldata voucher) public view returns(address){
        bytes32 digest = _hash(voucher);
        //returns signer
        return ECDSAUpgradeable.recover(digest, voucher.signature);
    }

    function MintNFT(string memory _tokenURI, uint256 _price, uint _royalties, bool _saleVal) public payable {
        dijauxNFTCounter ++;
        super._mint(msg.sender, dijauxNFTCounter);
        _setTokenURI(dijauxNFTCounter, _tokenURI);
        ...
        _royalties,
        _saleVal);
        allDijauxPieces[dijauxNFTCounter] = newDijauxPiece;

        emit Mint(msg.sender, dijauxNFTCounter);
    }
