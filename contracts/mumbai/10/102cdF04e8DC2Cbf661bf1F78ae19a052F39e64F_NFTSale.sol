...
contract NFTSale is Ownable, IERC721Receiver, ReentrancyGuard {
     ...
     */
    function putNFTForSale(uint256 nftId, uint256 price) external nonReentrant {
        ...
        require(!_nftSales[saleId].isActive, "NFT Sale is active");
        Sale memory sale = Sale(saleId, nftId, price, _msgSender(), true);
        _nftSales[saleId] = sale;
        emit SaleAdded(saleId, nftId, price, _msgSender());
    }
     ...
     */
    function cancelSale(uint256 saleId) external onlyOwner nonReentrant {
        require(_nftSales[saleId].isActive, "NFT is not up for sale");
        _nftSales[saleId].isActive = false;
        emit SaleCancelled(
            saleId,
            _nftSales[saleId].nftId,
            _nftSales[saleId].price,
            _nftSales[saleId].owner
        );
    }
    ...
    function purchaseNFT(uint256 saleId) external payable nonReentrant {
        require(
            msg.value == _nftSales[saleId].price,
            "value not equal to price of nft"
        );
        require(_nftSales[saleId].isActive, "NFT is not for sale");
        address seller = _nftSales[saleId].owner;
        address buyer = _msgSender();

        _nftSales[saleId].owner = buyer;
        _nftSales[saleId].isActive = false;
