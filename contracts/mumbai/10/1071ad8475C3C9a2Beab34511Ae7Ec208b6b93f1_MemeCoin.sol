...
//open zeppelin code is audited, so you don't need to worry about contract security. The code is safe.
//otherwise, we would need to review all imports and the code below. This way - only the code below.
...
contract MemeCoin is ERC20, Ownable, ERC20Burnable {
    // Defines the MemeCoin contract as a subclass of ERC20, Ownable, and ERC20Burnable - whatever they are, memecoin is too (inheritance)
    event tokensBurned(address indexed owner, uint256 amount, string message); //unsigned integers, only positive
    // Event (we define events beforehand, when they happen with their properties, something below happens) emitted when tokens are burned
    ...
    //msg - global variable, there is also state - contract, and local - function
    //so below the function can refer to anyone on the blockchain (sender, higher owner)

    constructor() ERC20("PaxRomanaCoin", "PRC") {
    ...
    function mint(address to, uint256 amount) public onlyOwner {
        ...
        _mint(to, amount); //the code to this is in the ERC-20 import 
    ...
    //there is a function named burn in one of the imports. You need to override.
    //onlyOwner means that we want to allow just the owner to burn/mint token
