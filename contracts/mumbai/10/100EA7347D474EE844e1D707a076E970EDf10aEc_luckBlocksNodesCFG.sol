...
contract luckBlocksNodesCFG is Ownable,ReentrancyGuard {
  ...
  function resetQueue(uint lottery, uint weekly) external {
      ...
      if(weekly == 1){
        require(block.timestamp > lbcontract.autoSpinTimestamp() + 5100,"autoSpin parameters not met"); // on mainnet 605100
   ...
   function updateNodeInfo(address caller, uint256 reward, uint weekly,uint lottery) external returns(bool success) {
      ...
      if(weekly == 1){
       require(block.timestamp > lbcontract.autoSpinTimestamp() + 4800,"autoSpin parameters not met"); // on mainnet 604800
