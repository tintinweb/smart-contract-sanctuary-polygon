...
contract CryptoCowboysTest is ERC721Enumerable, ERC721Royalty, Ownable {
  ...
  uint public minCost = 18;
  bool public paused = false;
  bool public revealed = false;

  address private _txFeeAddress;

  mapping(uint256 => uint) public cost;
  ...
  constructor(
    ...
    cost[0] = 10000000000000000000;
    cost[1] = 12000000000000000000;
    cost[2] = 14000000000000000000;
    cost[3] = 16000000000000000000;
    ...
    returns (string memory)
  {
    ...
    string memory tokenPart = revealed ? tokenId.toString() : "Hidden";

    return bytes(currentBaseURI).length > 0
        ? string(abi.encodePacked(currentBaseURI, tokenPart, baseExtension))
        : "";
  }
  ...
  function _isWhitelisted(address from) private view returns (bool) {
    return from == owner() || whitelisted[from] == true;
  }

  //only owner
  function setRoyality(uint96 _feeNumerator) public onlyOwner {
    _setDefaultRoyalty(owner(), _feeNumerator);
  }

  function setCost(uint256 _index, uint _newCost) public onlyOwner {
    cost[_index] = _newCost * 1000000000000000000;
  }
