...
contract MultiTask {
    ...
    uint256 public performanceBondPercent;
     ...
     * @param _performanceBondPercent The guarantee for project performance, paid after all payments.
     * @param _performanceBondDuration the time after all tasks completed to pay performance bond.
    ...
    constructor(
        ...
        uint256 _performanceBondPercent,
        uint256 _performanceBondDuration,
        User memory _owner,
        ...
        performanceBondPercent = _performanceBondPercent;

        uint256 performanceBondAmount;
        ...
        for (uint256 i = 0; i < _taskNames.length; i++) {
            ...
            performanceBondAmount += _wages[i] * _performanceBondPercent/100;
        }
        performanceBond = Task(
            "Performance Bond", 
            performanceBondAmount, 
            checkPoint + _performanceBondDuration * 1 days, 
            false, 
            false
        );
    }
    ...
    function releaseFund(uint256 _taskId) external {
        ...
        totalAmount *= (100 - performanceBondPercent)/100;
    ...
    function tasks() public view returns (Task[] memory temp) {
        temp = new Task[](_tasks.length + 1);

        for(uint256 i; i < _tasks.length; i++) {
            temp[i] = _tasks[i];
        }

        temp[temp.length - 1] = performanceBond;
    }
