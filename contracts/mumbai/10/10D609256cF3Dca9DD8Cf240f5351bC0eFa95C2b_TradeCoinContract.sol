    ...
    ITradeCoinContract
{
    ...
    bytes32 private constant _MINT_TYPEHASH =
        keccak256(
            "Mint(string product,uint256 amount,bytes32 unit,string state,uint256 CO2Emissions,string transformation,address owner,address tokenizer,uint256 paymentInWei,bytes32[] docHashes,bytes32[] docTypes,bytes32 rootHash,uint256 nonce,uint256 deadline)"
        );
    ...
    function mintProduct(
        ...
        {
            string[] memory trans = new string[](1);
            trans[0] = productInit.transformation;

            _tradeCoin[tokenId] = TradeCoin(
                productInit.product,
                productInit.amount,
                productInit.unit,
                productInit.state,
                ...
                currentHandler,
                trans,
                productInit.documents.rootHash
            );
        }
    ...
    {
        int256 intValue = int256(_tradeCoin[tokenId].amount);
        ...
        _tradeCoin[tokenId].transformations.push(transformationCode);
        int256 newAmount = intValue + weightDifference;
        _tradeCoin[tokenId].amount = uint256(newAmount);
        _tradeCoin[tokenId].rootHash = documents.rootHash;
        ...
        onlyEnoughDocuments(documents)
    {
        ...
        for (uint256 tokenId; tokenId < tokenIds.length; tokenId++) {
            _tradeCoin[tokenIds[tokenId]].rootHash = rootHash[tokenId];
        ...
        onlyEnoughDocuments(documents)
    {
        ...
        require(
            _tradeCoin[tokenId].amount == sumPartitions,
    ...
    function batchProduct(
        ...
        for (uint256 index; index < cachedTokenIdsLength; ) {
            ...
            //Note: to prevent stack too deep error
            {
                TradeCoin storage tradeCoinPointer = _tradeCoin[
                    tokenIds[index]
                ];
            ...
            cummulativeAmount += _tradeCoin[tokenIds[index]].amount;
            cummulativeCO2Emissions += _tradeCoin[tokenIds[index]].CO2Emissions;
            burn(tokenIds[index], documents, geoLocation);
            unchecked {
                ++index;
            }
        }
        ...
        )
    {
        ...
        product = _tradeCoin[tokenId].product;
        amount = _tradeCoin[tokenId].amount;
        unit = _tradeCoin[tokenId].unit;
        state = _tradeCoin[tokenId].state;
        CO2Emissions = _tradeCoin[tokenId].CO2Emissions;
        currentHandler = _tradeCoin[tokenId].currentHandler;
        transformations = _tradeCoin[tokenId].transformations;
        rootHash = _tradeCoin[tokenId].rootHash;
    }
    ...
    function mintAfterSplitOrBatch(
        ...
        _tradeCoin[id].rootHash = rootHash;
