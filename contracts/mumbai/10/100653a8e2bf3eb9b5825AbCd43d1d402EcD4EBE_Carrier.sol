...
contract Carrier is ERC1155, ERC1155Supply, Ownable {
    uint256 public constant ULTRA = 0;
    uint256 public constant RARE = 1;
    uint256 public constant CLASSIC = 2;

    constructor(string memory _uri) ERC1155(_uri) {
        _mint(msg.sender, ULTRA, 30, "");
        _mint(msg.sender, RARE, 2970, "");
        _mint(msg.sender, CLASSIC, 7000, "");
    }
        ...
        onlyOwner
    {
        address from = owner();
        require(
            balanceOf(from, _dropNumber) >= _list.length,
            "Not enough balance"
        );
        for (uint256 i = 0; i < _list.length; i++) {
            safeTransferFrom(from, _list[i], _dropNumber, 1, "");
        }
    }
