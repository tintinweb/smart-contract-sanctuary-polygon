...
contract StakingNft is Ownable, ERC721Holder {
    ...
    function updateReward(address _user) public {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if(staker.tokenStakingCoolDown[ids[i]] > int(block.timestamp + stakingTime) && staker.tokenStakingCoolDown[ids[i]] >= 0){
                ...
                console.log(staker.balance, partialTime,stakedDays);
    ...
    function getTest ( address _user, uint256 _id) view public returns(bool, int, uint, uint){
            Staker storage staker = stakers[_user];
        return (staker.tokenStakingCoolDown[_id] > int(block.timestamp + stakingTime),staker.tokenStakingCoolDown[_id], block.timestamp, stakingTime );
    }
}