...
contract ApyFlow is ERC20("Yielder", "YLDR"), Ownable {
    ...
    function totalAssets() public view returns (uint256 total) {
        ...
        return pricesOracle.convertFromUSD(valueInUSD, address(asset));
    }
    ...
    function deposit(
        ...
        for (uint256 i = 0; i < vaults.length(); i++) {
            ...
            deposited += pricesOracle.convert(
                vaultAsset,
                address(asset),
                _deposited
            );
    ...
    function redeem(
        ...
        uint256 newPricePerToken = (totalAssetsBefore - totalAssets()) * (10 ** decimals()) / shares;
        uint256 fee;
        if (newPricePerToken > lastPricePerToken) {
            uint256 adjustedPricePerToken = lastPricePerToken +
                (newPricePerToken - lastPricePerToken) *
                (1000 - feeInPpm) / 1000;
            fee = assets - assets * adjustedPricePerToken / newPricePerToken;
            if (fee > 0) {
                assets -= fee;
                asset.safeTransfer(feeTreasury, fee);
            }
        }
        if (assets > 0) {
    ...
    function computeScoreDeviationInPpm(
        ...
        uint256 assetsAtVault = pricesOracle.convert(
            vault.asset(),
            address(asset),
            vault.convertToAssets(shares)
        );
        return
            int256((1000 * assetsAtVault) / totalAssets()) -
