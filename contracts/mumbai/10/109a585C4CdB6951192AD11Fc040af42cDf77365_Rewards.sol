...
contract Rewards is Ownable {
     ...
     * @param _rewardId ID of reward claim
     * @param _user Wallet address of game user
     * @param _powIn POW value of reward
     * @param _resourceIds Array of Resource IDs to be included in reward
     * @param _powSplitUp Array of POW values to be allocated to each specific resource (must add up to powIn)
     * @param _minAmountsOut Array of minimum expected amounts out to protect against slippage
     */
    function claimResourceReward(
        ...
        IERC20(pow).transfer(address(assetPool), _powIn);
        ...
        emit ResourceReward(_rewardId, _user, _powIn, _resourceIds, _amounts);
    }
     ...
     * @param _resourceIds Array of Resource IDs to be included in reward
     * @param _powSplitUp Array of POW values to be allocated to each specific resource (must add up to powIn)
     * @param _minResourceAmountsOut Array of minimum expected amounts out to protect against slippage
    ...
    function claimGameItemReward(
        ...
        uint256[] calldata _resourceIds,
        uint256[] calldata _powSplitUp,
        uint256[] memory _minResourceAmountsOut,
        uint256 _itemId
    ) external onlyOwner {
        uint256[] memory _amounts = assetPool.calcResourcesOutFromPOWIn(
            _powIn, 
...
contract AssetPool is Ownable, ERC1155Holder {
     ...
     * @param _minAmountsOut Array of minimum amounts expected out for each Resource to protect against slippage
     */
    function calcResourcesOutFromPOWIn(
        ...
        if (
            len != _powSplitUp.length || 
            len != _minAmountsOut.length
        ...
        for (uint256 i = 0; i < len; ) {
            ...
            if (amounts[i] < _minAmountsOut[i]) revert SlippageExceeded();
...
contract Crafting is Ownable {
     ...
     * @dev Locks resources and sends game item to user
     * @param _subscriptId ID of subscript transaction
     * @param _requestor Address of user of game looking to subscript game item
     * @param _resourceIds Array of resource IDs to lock in game item
     * @param _amounts Array of amounts of each resource to lock in game item
     * @param _itemId ID of game item to send to user
     */
    function subscript(
        uint256 _subscriptId,
        address _requestor,
        uint256[] calldata _resourceIds,
        uint256[] calldata _amounts,
        uint256 _itemId
        ...
        emit Subscript(_subscriptId, _requestor, _resourceIds, _amounts, _itemId);
    }
     ...
     * @param _destroyId ID of destroy transaction
     * @param _destroyer Game ecosystem user that destroyed another user's Game Item
     * @param _prey Game ecosystem user that was ATTACKED
     * @param _to Address to send Resources to
     * @param _itemId ID of Game Item to be destroyed
     */
    function destroyForResources(
        ...
        GameItem memory item = lockedValue[_itemId];

        //Burn Game Item
        gameItems.burn(_prey, _itemId);

        // Send locked resources to _destroyer
        resources.safeBatchTransferFrom(
            lockedResources, 
            _to, 
            item.resourceIds, 
    ...
    function destroyForPow(
        ...
        assetPool.transfer(_to, item.pow);
