...
contract TdexPriceFeed is OwnablePausable {
    ...
    function initialize(
        ...
        chainlinkPriceFeed = AggregatorV3Interface(usdtUsdPriceFeed);
    }
    
    function getPrice(address tokenContract) external view returns(uint256){
        return _priceFeed[tokenContract];
    }

    function setPrice(address tokenContract, uint256 price) external onlyOwnerOrOperator {
        _priceFeed[tokenContract] = price;
    }

    function getUsdtUsdPrice() external view returns(uint256) {
        (,int price,,,) = chainlinkPriceFeed.latestRoundData();
