...
contract moderntsthaiku is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public moderntsthaiku_PRESALE = 1; //presale
    uint public moderntsthaiku_WALLET_PRESALE_LIMIT = 10; //moderntsthaikuperwalletInWholeDuration
    uint public moderntsthaiku_WALLET_PUBLICSALE_LIMIT = 20; //moderntsthaikuperwalletInWholeDuration
    uint public moderntsthaiku_MAX_COUNT = 1400; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public moderntsthaiku_MAINSALE_PRICE = 0.5 ether; //priceInMainSale
    uint public moderntsthaiku_PRESALE_PRICE = 0.3 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmUtzoC5kJoZRkCiBnQE31GY95T2pcfQNXVJioo9VN9U4D/";
    ...
    constructor() ERC721("moderntst Haiku", "moderntsthaiku") {}

    function setmoderntsthaiku_MAX_COUNT(uint _maxCount) external onlyOwner{
        moderntsthaiku_MAX_COUNT = _maxCount;
    }

    function setmoderntsthaiku_PRESALE(uint _presale) external onlyOwner{
        moderntsthaiku_PRESALE = _presale;
    }

    function setmoderntsthaiku_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        moderntsthaiku_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setmoderntsthaiku_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        moderntsthaiku_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setmoderntsthaiku_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        moderntsthaiku_MAINSALE_PRICE = _mainSalePrice;
    }

    function setmoderntsthaiku_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        moderntsthaiku_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= moderntsthaiku_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= moderntsthaiku_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= moderntsthaiku_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (moderntsthaiku_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (moderntsthaiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= moderntsthaiku_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//moderntsthaiku_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= moderntsthaiku_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= moderntsthaiku_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(moderntsthaiku_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
