...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base, Pausable {
    ...
    uint256 public maxLoanPeriod = 2592000;

    uint256 public minLoanPeriod = 86400;

    uint256 public season = 0;
    mapping(uint256 => mapping(address => uint256)) public seasonRewardsPercentageTracker;
    mapping(address => uint256) public addressIsGivingAwayNFTRewardsTo;
    mapping(uint256 => bool) public areNFTsClaimed;

    mapping(address => mapping(uint256 => bool)) public nftOnLoan;
    ...
    event LoanIssued(address loanee, uint256 loanId, uint256 startingTime, uint256 endTime);

    event LoanCanceled(address owner, address[] nftAddresses, uint256[] nftIds, uint256 loanId);

    event WithdrawNfts(address indexed admin, address indexed to, address[] nftAddresses, uint256[] nftIds);

    event LoanPeriodEnded(uint256 indexed loanId, uint256 timestamp, address owner, address loanee);
    ...
    function updatetreasuryPercentage(uint256 _percent) external onlyAdmin {
        require(_percent <= 10000, "Percentage cannot be greater than 100%");
    ...
    function updateMaxTimePeriod(uint256 _timePeriod) external onlyAdmin {
        ...
        require(_timePeriod > minLoanPeriod, "time period lower than minLoanPeriod");
    ...
    function isBundled(address _nftAddress, uint256 _nftId) external view returns (bool) {
    ...
    function hasAccessToNFT(
        ...
        return (_loanee == loanItems[loanId].loanee);
    }
    ...
    function createLoanableItem(
        ...
        if (_upfrontFee != 0) {
            require(
                _timePeriod >= minLoanPeriod && _timePeriod <= maxLoanPeriod,
        ...
        loanItems[newLoanId].bundleUUID = bundleUUID;
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            ...
            _nftToBundleId[nftAddress][nftId] = newLoanId;
            loanItems[newLoanId].nftAddresses.push(nftAddress);
            loanItems[newLoanId].tokenIds.push(nftId);

            nftOnLoan[nftAddress][nftId] = true;
    ...
    /**
     * Loaner can loan an Item to a loanee from offer
     *
    ...
    function issueLoanFromOffer(
        ...
        require(owner != offer.loanee, "loaner cannot be loanee");
        ...
        if (loanItems[_loanId].claimer == NFTRewardsClaimer.loaner) {
            require(addressIsGivingAwayNFTRewardsTo[offer.loanee] == 0, "Loanee is activly giving away rewards");
        }
        checkLoanRewardPercentageLimit(offer.loanee, offer.percentageRewards);
        ...
        loanItems[_loanId].endTime = block.timestamp + loanItems[_loanId].timePeriod;

        NFTRewardsClaimer _claimer = offer.claimer ? NFTRewardsClaimer.loaner : NFTRewardsClaimer.loanee;
        loanItems[_loanId].claimer = _claimer;

        if (_claimer == NFTRewardsClaimer.loaner) {
            addressIsGivingAwayNFTRewardsTo[offer.loanee] = _loanId;
        }
        setLoanAsActive(offer.loanee, _loanId);
    }
    ...
    function loanItem(uint256 _loanId) external nonReentrant whenNotPaused {
        address sender = _msgSender();
        LoanableItem memory loanItem = loanItems[_loanId];
        require(loanItem.owner != address(0), "Loanable Item Not Found");
        require(sender != loanItem.owner, "loaner cannot be loanee");
        require(loanItem.loanee == address(0), "Loan Item is already loaned");
        require(!areNFTsClaimed[_loanId], "NFTs already claimed, cannot issue loan");
        if (loanItem.reservedTo != address(0)) {
            require(loanItem.reservedTo == sender, "Private loan can only be issued to reserved user");
        }
        if (loanItem.claimer == NFTRewardsClaimer.loaner) {
            require(addressIsGivingAwayNFTRewardsTo[sender] == 0, "Loanee is activly giving away rewards");
        }
        checkLoanRewardPercentageLimit(sender, loanItem.percentageRewards);

        NFTRewardsClaimer _claimer = loanItem.claimer == NFTRewardsClaimer.loaner
            ? NFTRewardsClaimer.loaner
            : NFTRewardsClaimer.loanee;
        if (_claimer == NFTRewardsClaimer.loaner) {
            addressIsGivingAwayNFTRewardsTo[sender] = _loanId;
        }
        ...
        setLoanAsActive(sender, _loanId);
    }

    function setLoanAsActive(address sender, uint256 _loanId) internal {
        seasonRewardsPercentageTracker[season][sender] =
            seasonRewardsPercentageTracker[season][sender] +
            loanItems[_loanId].percentageRewards;
        ...
        if (loanItems[_loanId].upfrontFee != 0) {
            if (treasury != address(0) && treasuryPercentage != 0) {
                ...
                token.safeTransferFrom(sender, treasury, contractFee);
                token.safeTransferFrom(sender, loanItems[_loanId].owner, loanItems[_loanId].upfrontFee - contractFee);
            } else {
                token.safeTransferFrom(sender, loanItems[_loanId].owner, loanItems[_loanId].upfrontFee);
            }
        }
        emit LoanIssued(sender, _loanId, loanItems[_loanId].startingTime, loanItems[_loanId].endTime);
    }

    function checkLoanRewardPercentageLimit(address _address, uint8 percentageIncrease) internal {
        uint256 calculation = seasonRewardsPercentageTracker[season][_address] + percentageIncrease;
        if (calculation > 100) {
            revert("Loaner cannot give more than 100% rewards");
        }
    }
    ...
    /**
     * Loaner can cancel loan
     *
     * @param _loanId - Id of the loaned item
     *
     */
    function cancelLoan(uint256 _loanId) external nonReentrant whenNotPaused {
        require(!loanItems[_loanId].canceled, "Loan already canceled");
        require(
            _msgSender() == loanItems[_loanId].owner || _msgSender() == loanItems[_loanId].loanee,
            "Sender is not the owner, or loanee"
        );
        if (loanItems[_loanId].upfrontFee != 0) {
            require(
                _msgSender() == loanItems[_loanId].loanee,
                "only loanee can cancel loan when fee is greater than 0"
            );
        }
        loanItems[_loanId].canceled = true;
        endLoan(_loanId);
    ...
    /**
     * Loaner can end loan for lonnee if cost was 0
     * Loanee can end loan whenever they want
     *
     * @param _loanId - Id of the loaned item
     *
     */
    function endLoanPeriod(uint256 _loanId) external nonReentrant whenNotPaused {
        LoanableItem memory _loan = loanItems[_loanId];
        address _sender = _msgSender();
        require(_sender == _loan.owner || _sender == _loan.loanee, "Only loaner or loanee can end loan period");
        if (_sender == _loan.owner) {
            require(_loan.upfrontFee == 0, "Loaner can only end loan period when up front fee is 0");
        }

        endLoan(_loanId);

        emit LoanPeriodEnded(_loanId, block.timestamp, _loan.owner, _loan.loanee);
    }

    function endLoan(uint256 _loanId) internal {
        if (loanItems[_loanId].claimer == NFTRewardsClaimer.loaner) {
            addressIsGivingAwayNFTRewardsTo[loanItems[_loanId].loanee] = 0;
        }

        if (loanItems[_loanId].loanee != address(0)) {
            seasonRewardsPercentageTracker[season][loanItems[_loanId].loanee] =
                seasonRewardsPercentageTracker[season][loanItems[_loanId].loanee] -
                loanItems[_loanId].percentageRewards;
        }
        areNFTsClaimed[_loanId] = true;

        loanItems[_loanId].endTime = block.timestamp;
    ...
    function withdrawTokenToOwners(uint256[] memory loanIds) external onlyAdmin {
        for (uint256 _loanIdIndex = 0; _loanIdIndex < loanIds.length; _loanIdIndex++) {
            uint256 _loanId = loanIds[_loanIdIndex];
            endLoan(_loanId);
        }
    }

    function setSeason(uint256 _season) external onlyAdmin {
        season = _season;
    }
