...
contract    RuskoffKill     is ReentrancyGuard
{
    ...
    event   AddPlayerToRoom( bool wasGamePlayed, uint roomId, uint playerCount, uint entryPrice, uint perWinnerGain, uint roomFee, bool hasWon, TRoom room); 
    ...
    function    addPlayerToRoom(uint roomId) external payable   nonReentrant
    {
        ...
        emit AddPlayerToRoom(false, roomId, room.playerWalletList.length, room.entryPrice,
                             room.gainPerWinner, 0,
                             false,
    ...
    function    startGame(TRoom storage room, uint roomId) internal
    {
        ...
        emit AddPlayerToRoom(true,
                             roomId, room.playerWalletList.length, room.entryPrice,
    ...
    function    getLatestDraws(uint count) external view returns(TRoom[] memory)
    {
        ...
        for (uint i; i<rooms.length; i++)
        {
            ...
            if (room.winnerWalletList.length==0)    continue;   // pas encore de joueurs gagnants, on saute
