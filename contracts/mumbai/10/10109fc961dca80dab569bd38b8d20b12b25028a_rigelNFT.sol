...
contract rigelNFT is ERC1155, Ownable {
  ...
  function mintBatch( address _to, uint256[] memory _tokenId, uint256[] memory _amount, string memory _uri)  external onlyOwner returns (bool) {      
    _mintBatch(_to, _tokenId, _amount, bytes(_uri));
    _setURI(_uri);
    return true;
  }
