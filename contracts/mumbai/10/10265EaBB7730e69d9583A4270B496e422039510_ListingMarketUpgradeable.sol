...
contract ListingMarketUpgradeable is BaseMarketUpgradeable {
    // collection => tokenId => erc20 => price
    ...
    function list(address _collection, uint256 _tokenId, address _erc20, uint256 _price) public {
        _list(msg.sender, _collection, _tokenId, _erc20, _price);
    }
    ...
    function permitList(
        ...
        isValidSignature(
            _seller,
            _getPermitListHash(_collection, _tokenId, _seller, _erc20, _price, _deadline),
            _deadline,
            _signature
        )
    {
        _list(_seller, _collection, _tokenId, _erc20, _price);
    }

    function delist(address _collection, uint256 _tokenId, address _erc20) public {
        _delist(msg.sender, _collection, _tokenId, _erc20);
    }
    ...
    function permitDelist(
        ...
        isValidSignature(
            _seller,
            _getPermitDelistHash(_seller, _collection, _tokenId, _erc20, _deadline),
            _deadline,
            _signature
        )
    {
        _delist(_seller, _collection, _tokenId, _erc20);
    }

    function buy(address _collection, uint256 _tokenId, address _erc20, bool _payNow) public {
        _buy(msg.sender, msg.sender, _collection, _tokenId, _erc20, _payNow);
    }
    ...
    function permitBuy(
        ...
        isValidSignature(
            _buyer,
            _getPermitBuyHash(_buyer, _collection, _tokenId, _erc20, _payNow, _deadline),
            _deadline,
            _signature
        )
    {
        _buy(_buyer, _buyer, _collection, _tokenId, _erc20, _payNow);
    }
    ...
    function permitOffChainPaymentBuy(
        ...
        isValidSignature(
            _buyer,
            _getPermitOffChainPaymentBuyHash(_buyer, _collection, _tokenId, _erc20, _deadline),
            _deadline,
            _signature
        )
    {
        _buy(msg.sender, _buyer, _collection, _tokenId, _erc20, false);
    }
    ...
    function handleTokenIdNoLongerAvailable(
        ...
        for (uint i = 0; i < erc20s.length; ++i) {
            ...
            if (listings[_erc721][_tokenId][erc20] > 0) {
                listings[_erc721][_tokenId][erc20] = 0;
                emit Delisted(_seller, _erc721, _tokenId, erc20);
            }
        }
    }

    // Private Functions
    function _list(address _seller, address _collection, uint256 _tokenId, address _erc20, uint256 _price) private {
        ...
        listings[_collection][_tokenId][_erc20] = _price;
        emit Listed(_seller, address(collection), _tokenId, _erc20, _price);
    }

    function _delist(address _seller, address _collection, uint256 _tokenId, address _erc20) private {
        IColleCollection collection = marketHub.getCollection(_collection);
        require(_seller == collection.ownerOf(_tokenId), "Marketplace: Only the token owner can call this function.");
        require(listings[_collection][_tokenId][_erc20] > 0, "Marketplace: Token is not listed.");
        listings[_collection][_tokenId][_erc20] = 0;
        emit Delisted(_seller, address(collection), _tokenId, _erc20);
    }
    ...
    function _buy(
        ...
        require(_spender != address(0), "Marketplace: Spender cannot be zero address.");

        uint256 price = listings[_collection][_tokenId][_erc20];
        require(price > 0, "Marketplace: Token is not listed.");

        IColleCollection collection = marketHub.getCollection(_collection);

        address seller = collection.ownerOf(_tokenId);
        _createSale(seller, _buyer, _spender, _collection, _tokenId, _erc20, price, _payNow);
    }

    // Views
    function getListedPrice(address _collection, uint256 _tokenId, address _erc20) public view returns (uint256) {
        return listings[_collection][_tokenId][_erc20];
    }
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "List(address collection,uint256 tokenId,address seller,address erc20,uint256 price,uint256 deadline,uint256 nonce)"
                            ),
                            _collection,
                            _tokenId,
                            _seller,
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "Delist(address collection,uint256 tokenId,address seller,address erc20,uint256 deadline,uint256 nonce)"
                            ),
                            _collection,
                            _tokenId,
                            _seller,
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "Buy(address collection,uint256 tokenId,address buyer,address erc20,bool payNow,uint256 deadline,uint256 nonce)"
                            ),
                            _collection,
                            _tokenId,
                            _buyer,
    ...
    function _getPermitOffChainPaymentBuyHash(
        address _buyer,
        address _collection,
        uint256 _tokenId,
        address _erc20,
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "OffChainPaymentBuy(address collection,uint256 tokenId,address buyer,address erc20,uint256 deadline,uint256 nonce)"
                            ),
                            _collection,
                            _tokenId,
                            _buyer,
