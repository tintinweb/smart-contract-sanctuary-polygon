...
contract PartnerRegistry is OperatorAccessControlUpgradeable {
    ...
    mapping(uint256 => bytes32) public merkleRootByEventId;
    mapping(address => bool) public partnerLocked;
    ...
    modifier onlyUnlockedPartner() {
        require(isPartner(msg.sender) && !partnerLocked[msg.sender], "PartnerRegistry::onlyUnlockedPartner: address is not an unlocked partner");
        _;
    }

    modifier onlyUnlockedEventOwnerOrOperator(uint256 eventId) {
        require(
            (events[eventId].partner == msg.sender && !partnerLocked[msg.sender]) ||
            isOperator(msg.sender),
            "PartnerRegistry::onlyUnlockedEventOwnerOrOperator: sender is neither unlocked Event owner nor Operator");
        _;
    }
    ...
    function createEvent(uint256 startTime, uint256 endTime, uint256 rewardPerFlexer, uint256 another1TokenRewardPool, bool isPoapAssigned) public onlyUnlockedPartner {
    ...
    function modifyEvent(uint256 eventId, uint256 startTime, uint256 endTime, uint256 rewardPerFlexer, uint256 another1TokenRewardPool, bool isPoapAssigned) public onlyUnlockedEventOwnerOrOperator(eventId) {
    ...
    function addClerksToEvent(uint256 eventId, address[] memory _clerks) public onlyUnlockedEventOwnerOrOperator(eventId) {
        for(uint256 i = 0; i < _clerks.length; ++i) {
            _addClerkToEvent(eventId, _clerks[i]);
        }
    }

    function _removeClerkFromEvent(address _clerk) private onlyUnlockedEventOwnerOrOperator(getClerkEventId(_clerk)) {
    ...
    function addAllowedCollections(uint256 eventId, address[] memory colectionAddresses) public onlyUnlockedEventOwnerOrOperator(eventId) {
    ...
    function removeAllowedCollections(uint256 eventId, address[] memory colectionAddresses) public onlyUnlockedEventOwnerOrOperator(eventId) {
        for(uint256 i = 0; i < colectionAddresses.length; ++i) {
            if(allowedCollectionsByEventId[eventId][colectionAddresses[i]]) {
                allowedCollectionsCounterByEventId[eventId]--;
            }
    ...
    function setEventCancellation(uint256 eventId, bool isCancelled) public onlyUnlockedEventOwnerOrOperator(eventId) {
        events[eventId].isCancelled = isCancelled;
        emit EventCancellation(eventId, isCancelled);
    }

    function setMerkleRoot(uint256 eventId, bytes32 merkleRoot) public onlyUnlockedEventOwnerOrOperator(eventId) {
        merkleRootByEventId[eventId] = merkleRoot;
        emit MerkleRootChanged(eventId, merkleRoot);
    }

    function changePartnerLock(address partner, bool isLocked) public onlyOperator onlyPartner(partner) {
        partnerLocked[partner] = isLocked;
        emit PartnerLockChanged(partner, isLocked);
    }
