    ...
    ERC721Royalty
{
    ...
    bytes32 public constant PURCHASED = keccak256("PURCHASED");
    ...
    constructor(
        ...
        string memory baseURI_
    ) ERC721("Gold Fever Mask", "GFM") {
        baseURI = baseURI_;
        _setupContractId("GoldFeverMask");
        _setupRole(DEFAULT_ADMIN_ROLE, admin);
    ...
    event ForwarderContractUpdated(address forwarder_);
    event ForgeMaskFeeUpdated(uint256 _oldFee, uint256 _newFee);
    event UnforgeMaskFeeUpdated(uint256 _oldFee, uint256 _newFee);
    event PurchaseMaskCostUpdated(uint256 _oldCost, uint256 _newCost);
    event CollectedFeeWithdrawn(uint256 _collectedFee);
    ...
    mapping(address => bool) public addressToFirstFreeForge;
    ...
    function forgeMask(
        ...
        require(maskShape > 0, "GoldFeverMask: Need at least one shape");
        require(maskMaterial > 0, "GoldFeverMask: Need at least one material");
        ...
        require(
            IGoldFeverItemType(itemTypeContract).getItemType(maskShape) >=
                ...
                IGoldFeverItemType(itemTypeContract).getItemType(maskShape) <=
                231024,
            "GoldFeverMask: Invalid mask shape"
        );
        ...
        require(
            IGoldFeverItemType(itemTypeContract).getItemType(maskMaterial) >=
                ...
                231124,
            "GoldFeverMask: Invalid mask material"
        );
        ...
        require(
            (IGoldFeverItemType(itemTypeContract).getItemType(topElement) >=
                ...
                IGoldFeverItemType(itemTypeContract).getItemType(topElement) ==
                0,
            "GoldFeverMask: Invalid top element"
        );
        ...
        require(
            (IGoldFeverItemType(itemTypeContract).getItemType(frontElement) >=
                ...
                0,
            "GoldFeverMask: Invalid front element"
        );
        ...
        require(
            (IGoldFeverItemType(itemTypeContract).getItemType(scratches) >=
                ...
                IGoldFeverItemType(itemTypeContract).getItemType(scratches) ==
                0,
            "GoldFeverMask: Invalid scratches"
        );
        ...
        require(
            (IGoldFeverItemType(itemTypeContract).getItemType(paintOver) >=
                ...
                IGoldFeverItemType(itemTypeContract).getItemType(paintOver) ==
                0,
            "GoldFeverMask: Invalid paintOver"
        );
        ...
        require(
            ...
            "GoldFeverMask: This mask type is already forged by other user"
        );

        address msgSender = _msgSender();
        gfi.safeTransferFrom(msgSender, address(this), maskShape);
        gfi.safeTransferFrom(msgSender, address(this), maskMaterial);
        if (IGoldFeverItemType(itemTypeContract).getItemType(topElement) != 0) {
            gfi.safeTransferFrom(msgSender, address(this), topElement);
        }
        if (
            IGoldFeverItemType(itemTypeContract).getItemType(frontElement) != 0
        ) {
            gfi.safeTransferFrom(msgSender, address(this), frontElement);
        }
        if (IGoldFeverItemType(itemTypeContract).getItemType(scratches) != 0) {
            gfi.safeTransferFrom(msgSender, address(this), scratches);
        }
        if (IGoldFeverItemType(itemTypeContract).getItemType(paintOver) != 0) {
            gfi.safeTransferFrom(msgSender, address(this), paintOver);
        }

        _mint(msgSender, maskId);
        ...
        if (addressToFirstFreeForge[msgSender]) {
            ngl.transferFrom(msgSender, address(this), adminEarn);
            ngl.burnFrom(msgSender, forgeMaskFee - adminEarn);
            nglFromCollectedFee += adminEarn;
        } else {
            addressToFirstFreeForge[msgSender] = true;
        }
    ...
    function unforgeMask(uint256 maskId) external nonReentrant {
        require(
            idToMask[maskId].status == FORGED ||
                idToMask[maskId].status == PURCHASED,
            "GoldFeverMask: Mask is not forged"
        );
        ...
        require(
            msgSender == owner,
            "GoldFeverMask: onlyRole owner can unforge"
        );
        ...
        ngl.transferFrom(msgSender, address(this), adminEarn);
        ngl.burnFrom(msgSender, unforgeMaskFee - adminEarn);
        nglFromCollectedFee += adminEarn;
    ...
    function updateForgeMaskFee(
        uint256 _fee
    ) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_fee > 0, "GoldFeverMask: Fee must be greater than 0");
        emit ForgeMaskFeeUpdated(forgeMaskFee, _fee);
        forgeMaskFee = _fee;
    }

    function updateUnforgeMaskFee(
        uint256 _fee
    ) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_fee > 0, "GoldFeverMask: Fee must be greater than 0");
        emit UnforgeMaskFeeUpdated(unforgeMaskFee, _fee);
        unforgeMaskFee = _fee;
    }

    function updatePurchaseMaskCost(
        uint256 _cost
    ) external nonReentrant onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _cost > 0,
            "GoldFeverMask: Purchase cost must be greater than 0"
        );
        emit PurchaseMaskCostUpdated(purchaseMaskCost, _cost);
        ...
        onlyRole(COMPANY_ROLE)
    {
        ...
        emit CollectedFeeWithdrawn(nglFromCollectedFee);
        nglFromCollectedFee = 0;
    }

    function getForgeMaskFee() external view returns (uint256) {
        return forgeMaskFee;
    }

    function getUnforgeMaskFee() external view returns (uint256) {
        return unforgeMaskFee;
    }

    function getPurchaseMaskCost() external view returns (uint256) {
        return purchaseMaskCost;
    }

    function purchaseMask(uint256 maskId) external nonReentrant {
        address owner = ownerOf(maskId);
        require(
            _msgSender() == owner,
            "GoldFeverMask: onlyRole owner can purchase"
        );
        require(
            idToMask[maskId].status == FORGED,
            "GoldFeverMask: Mask is not forged or already purchased"
        );
    ...
    function setCommissionRate(
        ...
        require(
            _rate >= 0 && _rate <= 100,
            "GoldFeverMask: Commission rate must be between 0 and 100"
        );
