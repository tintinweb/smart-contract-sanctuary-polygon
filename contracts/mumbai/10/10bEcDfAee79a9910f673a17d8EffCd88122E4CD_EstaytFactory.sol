...
contract EstaytFactory is IEstaytFactory {
    ...
    function createEstayt(uint256 _proofOfLifeDuration, IBeneficiary.Beneficiary[] memory _beneficiaries) external {
                ...
                uint256(
                    keccak256(
                        abi.encodePacked(
                            ...
                            keccak256(
                                abi.encodePacked(
                                    type(Estayt).creationCode, abi.encode(msg.sender, _proofOfLifeDuration, _beneficiaries)
                                )
                            )
                        )
                    )
        ...
        Estayt estayt = new Estayt{salt: salt}(msg.sender, _proofOfLifeDuration, _beneficiaries);

        require(address(estayt) == newEstaytAddress, "Prediction");
