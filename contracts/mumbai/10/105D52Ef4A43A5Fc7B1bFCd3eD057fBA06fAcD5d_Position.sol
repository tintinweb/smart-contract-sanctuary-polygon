...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function closePosition(uint256 positionId) public {
        ...
        if(positionInfo.asset == uniswapV2Router.WETH()) {
            IMargin(IAdminFunctions(adminContract).getMarginContract()).closePositionAmount(user, address(0), positionInfo.marginUsed ,positionInfo.loanUsed, marginAmount);
        }
        else {
        ...
        returns (uint256)
    {
        ...
        address[] memory path = new address[](2);
        path[0] = positionInfo.asset; //matic 
        path[1] = positionInfo.targetAsset; //eth
