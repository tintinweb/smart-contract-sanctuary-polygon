...
contract FFINetworkV2 {
    ...
    address constant internal stakingWalletAddress=0x58eFce14AD83D3a08276fB67BBcD51aAFcFCbaaa;
    address constant internal rewardWalletAddress=0x99b99CE0A428AfCA0e11C77918c2D5862724b95b;
    ...
    uint256 public totalCompletedStakers;
    uint public noofDaysForROI=540;
    address[] public stakingQualifier;
    uint256 public _24rewardPool;

	uint256 public totalStakingCollected;
    ...
    */
    constructor() public {
        ...
        administrators[0x7CeCbee1241EbD6A06BCEB61d190C9BE3683E747] = true;
        primaryAdmin = 0x7CeCbee1241EbD6A06BCEB61d190C9BE3683E747;
    }
    ...
    */
    ...
    //Execute ROI Payout
    function _executeROI(uint8 fromQualifier,uint8 toQualifier) 
    onlyAdministrator()
    public {    
       require(primaryAdmin==msg.sender, 'Admin what?');
       uint256 _reward = calculateFee(_24rewardPool,30);
       uint256 _staking = calculateFee(_24rewardPool,70);
       ...
       stakingBalance_[stakingWalletAddress] = SafeMath.sub(stakingBalance_[stakingWalletAddress], _staking);  
       _24rewardPool -=_reward; 
       if(stakingQualifier.length > 0) {
        for(uint8 i = fromQualifier; i < toQualifier; i++) {
         ...
         uint noofqualifier=(activeStakerCount[currentpackageid]-completeStakerCount[currentpackageid]);
         uint256 eachPackageShare=((_staking*roiPer)/100)/1e18;
         uint256 eachPersonShare=(eachPackageShare/noofqualifier);

         userdetails.roiBonus +=eachPersonShare;
         ...
         _24rewardPool -= eachPersonShare;
         
         uint256 principleToBeRelease=0;
         if(user.paidDays==1 || user.paidDays==2){
            //if(user.paidDays==180 || user.paidDays==360){
            user.apyPer=(user.apyPer/2); 
            if(user.paidDays==1){
                principleToBeRelease=(user.selfCurrentPackagePurchase/2);
            }
            else{
                principleToBeRelease=(user.selfCurrentPackagePurchase/4);
            }         
         }
         //else if(user.paidDays==540){   
         else if(user.paidDays==3){  
             principleToBeRelease=user.selfCurrentPackagePurchase/4;           
             user.apyPer=0;   
             user.paidDays=0; 
             user.selfCurrentPackagePurchase=0;
             completeStakerCount[currentpackageid]+=1;
             totalActiveStakers-=1;
             totalCompletedStakers+=1;
         }
         userdetails.principleReleased+=principleToBeRelease;
         //Exchange Tokens
    ...
    **/
        ...
        returns(bool)
    {
	    ...
	    if(user.userId == 0) {
            totalUniqueStakers = SafeMath.add(totalUniqueStakers,1);
        ...
        user.selfTotalPackagePurchase += tokensMaticValue;
        user.selfCurrentPackagePurchase += tokensMaticValue;
        user.activeStakingPackage[package]=true;
        user.currentpackageid=package;
        activeStakerCount[package]+=1;
        totalStakers = SafeMath.add(totalStakers,1);
        totalActiveStakers=SafeMath.add(totalActiveStakers,1);
        user.apyPer=stakingAPY[package];
        user.lastUpdateTime=block.timestamp;
        totalStakingCollected = SafeMath.add(totalStakingCollected,tokensMaticValue); 
