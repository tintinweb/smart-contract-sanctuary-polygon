...
contract N3XCloneFactory is Initializable, AccessControlUpgradeable, EIP712MetaTransactionUpgradeable {
    ...
    mapping(address => address) public proxyToDeployer;
    ...
    function initialize() initializer public {
        __EIP712MetaTransactionUpgradeable_init("N3X Clone Factory", "1");
        __AccessControl_init();

        _grantRole(FACTORY_ROLE, _msgSender());
    }

    function createProxyClone(
        ...
        returns (address) 
    {
        require(approvedImplementations[implementation_], "Implementation not approved");

        bytes32 salthash = keccak256(abi.encodePacked(_msgSender(), salt));
        address proxyClone = ClonesUpgradeable.cloneDeterministic(implementation_, salthash);

        proxyToDeployer[proxyClone] = _msgSender();

        emit ProxyDeployed(implementation_, proxyClone, _msgSender());

        if (data.length > 0) {
            // slither-disable-next-line unused-return
            AddressUpgradeable.functionCall(proxyClone, data);
        }
    ...
    function addImplementation(address implementation_) external {
        require(hasRole(FACTORY_ROLE, _msgSender()), "Not in role");

        IN3XClone module = IN3XClone(implementation_);

        bytes32 ctype = module.contractType();
        require(ctype.length > 0, "Invalid contract");

        uint256 version = module.contractVersion();
        uint256 currentVersionOfType = currentVersion[ctype];
        require(version >= currentVersionOfType, "Invalid contract version");

        currentVersion[ctype] = version;
        implementation[ctype][version] = implementation_;
        approvedImplementations[implementation_] = true;

        emit ImplementationAdded(implementation_, ctype, version);
    }

    /// @dev Lets a contract admin approve a specific contract for deployment.
    function approveImplementation(address implementation_, bool isApproved) external {
        require(hasRole(FACTORY_ROLE, _msgSender()), "Not in role");

        approvedImplementations[implementation_] = isApproved;

        emit ImplementationApproved(implementation_, isApproved);
    }

    /// @dev Returns the implementation given a contract type and version.
    function getImplementation(bytes32 type_, uint256 version_) external view returns (address) {
        return implementation[type_][version_];
    }

    /// @dev Returns the latest implementation given a contract type.
    function getLatestImplementation(bytes32 type_) external view returns (address) {
        return implementation[type_][currentVersion[type_]];
    }
}
