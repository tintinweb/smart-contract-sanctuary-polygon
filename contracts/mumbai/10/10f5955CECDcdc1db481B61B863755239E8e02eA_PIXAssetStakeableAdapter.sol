...
contract PIXAssetStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC1155HolderUpgradeable {
    ...
    event TokenInfoUpdated(bool isStakeable, bool isBase, bool isRaidable, Distribution class, uint256 resourceEV, uint256 resourceId, uint256 resourceCap);
    ...
    function onStaked(
        ...
        if (tokenInfo[_tokenId][_x][_y].class == Distribution.Facility) {
            require(__handleFacilitySetup(_tokenId, _currentOwner, _underlyingToken, _underlyingTokenId, _nonce), "MCS1155_ADAPTER: Facility must be staked on correct tier");
        }
    ...
    /// @inheritdoc IMissionControlStakeable
    function onUnstaked(address _newOwner, uint256 _nonce) external override onlyMissionControl onlyStakedTokens(_newOwner, _nonce) {
    ...
    function __timeLeft_deterministic_linear(
        ...
        uint256 completionTime = (DAY_IN_SEC / tokenInfo[tokenId][_x][_y].resourceEV) * tokenInfo[tokenId][_x][_y].resourceCap + localSeed;
    ...
    function __timeLeft_facility(
        ...
        uint256 output = (facilityOutput[_userAddress][_nonce] * structureBonuses[_x][_y]) / STRUCTURE_BONUS_DENOMINATOR;

        uint256 completionTime = (tokenInfo[tokenId][_x][_y].resourceCap * facilityLeadTime[_userAddress][_nonce]) / output + localSeed;
        int256 timeRemaining = int256(completionTime) - int256(block.timestamp);
        return timeRemaining > 0 ? timeRemaining : int256(0);
    }
    ...
    function __checkTile_facility(
        ...
        uint256 ret = (facilityOutput[_userAddress][_nonce] * ((untilTimestamp - localSeed) / facilityLeadTime[_userAddress][_nonce]));
    ...
    */
    ...
    function _getT(
        ...
        T = (_max_div(int256(DAY_IN_SEC) * MAX_SCALE, int256(ev) * MAX_SCALE) * _exponential_2(_playerAddress, tokenId, seed)) / (MAX_SCALE * MAX_SCALE);
    }
    ...
    function _exponential_2(
        ...
        exponential_sample = -_max_ln(MAX_SCALE - _max_div(MAX_SCALE * int256(_uniform(_playerAddress, tokenId, seed)), MAX_SCALE * int256(PROBA_RES)));
    }
