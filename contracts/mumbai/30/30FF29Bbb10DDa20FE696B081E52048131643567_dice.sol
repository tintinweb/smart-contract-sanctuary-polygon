...
contract dice {
    ...
    // event BetEnd(uint256 randomNum, uint256[] results); //结束投注通知&披露随机数
    event BetSuccess(string indexed orderIdIdx, string orderId, address betMan, BetRegion betRegion, uint256 betAmount); //下注成功通知
    event SettlementEnd(string indexed orderIdIdx, string orderId, uint256 randomNum, uint256[] results, BetRecord[] bets); //结算完成
    ...
    function doSettlement(uint256 randomNum, uint256[] memory results) private {
        ...
        emit SettlementEnd(orderId, orderId, randomNum, results, bets);
