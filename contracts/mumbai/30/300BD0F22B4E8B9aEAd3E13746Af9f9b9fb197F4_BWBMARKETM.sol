...
contract BWBMARKETM is Initializable , IERC721ReceiverUpgradeable , ERC1155HolderUpgradeable ,  OwnableUpgradeable {
    ...
    modifier onlyNFTSeller (address _NFTAddress , uint256 _TokenId ) {
        ...
        if (_DepositNFT[_NFTAddress][_TokenId]._Seller  != msg.sender) {
            revert("MSG.SENDER is not NFT Seller..");
        }

        if (_DepositNFT[_NFTAddress][_TokenId]._Deposited != true) {
            revert("NFT not Deposited");
        }
    ...
    function depositNFT1155 (address _NFTAddress , uint256 _TokenID, uint256 _Price , bytes memory data) public onlyOwner{
    ...
    function cancelAtNFT1155 (address _NFTAddress , uint256 _TokenID,  bytes memory data) public onlyOwner{
