...
error InvalidSocialsHash(bytes32 socialsHash);
...
contract PhaverIssuer is Pausable, Ownable {
  ...
  uint8 public constant TOKEN_ID = 1; // force id to be 1

  mapping(address => bool) public _revokedRelayer; // relayer rights can be revoked by user
  mapping(bytes32 => bool) private _usedTwitterIdHash; // for deduplication
  mapping(bytes32 => bool) private _usedDiscordIdHash; // for deduplication
  mapping(address => bytes32) private _socialsHash; // check if sbt is updated with the same socials hashes
  ...
  /// @param ABTAddress The address of the ABT contract.
  /// @param relayer The address of the relayer.
  constructor(address ABTAddress, address relayer) {
  ...
  /// @notice Revokes the relayer rights for a given address.
  function revokeRelayer() external {
    _revokedRelayer[msg.sender] = true;
  }

  /// @notice Issues a Phaver ABT to the provided receiver address.
  ...
  function issueCredential(
    ...
    _verifySigner(msg.sender, receiver);
    ...
    bytes32 socialsHashes = keccak256(
      abi.encodePacked(receiver, twitterIdHash, discordIdHash)
    );
    // link address to socials hashes
    _socialsHash[receiver] = socialsHashes;

    ABT.issue(receiver, TOKEN_ID, credentialURL);
  }

  /// @notice Updates the credential URL of the provided receiver's ABT.
  /// @param receiver The address to receive the Phaver SBT.
  /// @param credentialURL URL of the caller's updated credentials.
  /// @param proof Proof the caller own's the updated credentials.
  /// @param socialsHash Hash of the caller's address, twitter and discord hashes.
  function updateCredential(
    ...
    // check if provided socials hashes are the same the ones the user minted the SBT with
    if (_socialsHash[receiver] != socialsHash)
      revert InvalidSocialsHash(socialsHash);

    ABT.update(receiver, TOKEN_ID, credentialURL);
  }
  ...
  function _deduplicateCredential(bytes32 twitterIdHash, bytes32 discordIdHash)
    internal
  {
    if (_usedTwitterIdHash[twitterIdHash])
      revert HashAlreadyUsed(twitterIdHash);
    if (_usedDiscordIdHash[discordIdHash])
      revert HashAlreadyUsed(discordIdHash);

    _usedTwitterIdHash[twitterIdHash] = true;
    _usedDiscordIdHash[discordIdHash] = true;
  }

  /// @notice Verifies the signer is authorized to issue or update the SBT.
  /// @param signer The address of the signer.
  /// @param receiver The address to receive the SBT.
  function _verifySigner(address signer, address receiver) internal view {
    // revert if the receiver has revoked the relayer and the signer is not the receiver.
    if (_revokedRelayer[receiver] && signer != receiver)
      revert InvalidSigner(signer, receiver);

    // revert if the signer is not the relayer or receiver.
    if (signer != _relayer && signer != receiver)
      revert InvalidSigner(signer, _relayer);
  }
