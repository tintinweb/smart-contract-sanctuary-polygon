...
contract EpillioVesting is OwnableUpgradeable,ReentrancyGuardUpgradeable{
    ...
    uint[] public intermediateAmountReleased=[0,0,0,0,0,0,0,0];
    uint[] public linearVestingAmountReleased=[0,1000,900,900,980,970,1000,0]; // stores percentage
    uint[] public intermediateVestingTimePeriod=[0,0,0,0,0,0,0,0];
    uint[] public linearVestingTimePeriod=[0,120 minutes,100 minutes,130 minutes,270 minutes,270 minutes ,150 minutes,0];
    ...
    function setAllDates(uint[] memory  saleType, uint[] memory  initialDate, uint[] memory  intermediate,uint[] memory linearDate) external  onlyOwner{
        for(uint i=0; i < saleType.length; ++i){
        initialDates[saleType[i]]=initialDate[i];
        intermediateDates[saleType[i]]=intermediate[i];
        LinearDates[saleType[i]]=linearDate[i];
        }
      
        initialVestingAmountWithdrawThresholdTime = [0,initialDates[1],initialDates[2],initialDates[3],initialDates[4],initialDates[5],initialDates[6],initialDates[7]];
        intermediateVestingAmountWithdrawThresholdTime = [0,0,0,0,0,0,0,0];
        linearVestingAmountWithdrawThresholdTime= [0,LinearDates[1] ,LinearDates[2],LinearDates[3],LinearDates[4]  ,LinearDates[5],LinearDates[6],0 ];

    }

    function setSingleDate(uint saleType,uint initialDate,uint intermediate,uint linearDate)external onlyOwner{
