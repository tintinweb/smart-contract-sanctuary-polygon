...
contract PNounsProvider3 is IAssetProviderEx, Ownable, IERC165 {
  ...
  function generateSVGPart(uint256 _assetId) public view override returns (string memory svgPart, string memory tag) {
    ...
    stack.noun553X = stack.noun553Y % 100;
    stack.noun553Position = stack.noun553X % 3;
    stack.noun553Rotate = (stack.noun553X + stack.noun553Y) % 360;

    if (stack.noun553Position % 2 == 0) {
      stack.noun553X += 700;
    }
    if (stack.noun553Position == 1) {
      stack.noun553Y += 500;
    }

    // background
    stack.background = SVG.rect().fill((_assetId % 2 == 0 ? '#CCFFCC' : '#FFCCD8'));
