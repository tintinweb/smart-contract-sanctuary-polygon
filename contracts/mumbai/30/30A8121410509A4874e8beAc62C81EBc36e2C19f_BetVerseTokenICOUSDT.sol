...
contract BetVerseTokenICOUSDT is ERC20, ReentrancyGuard, Ownable {
    ...
    {
        uint256 tokenAmount = getTokenAmountToSell(usdtAmount);
        ...
        usdt.transferFrom(msg.sender, walletTo, usdtAmount);
    ...
    function getTokenAmountToSell(uint256 usdtAmount) public view returns (uint256) {
        return usdtAmount.div(price) * 10**usdtDecimals;
    }

    function hasEnoughTokensToSell(uint256 usdtAmount)
        public
        view
        returns (
            bool,
    ...
    {
        return (tokenBalance() >= getTokenAmountToSell(usdtAmount), tokenBalance(), getTokenAmountToSell(usdtAmount));
    }
