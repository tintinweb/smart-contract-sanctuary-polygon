...
contract Pot {
    ...
    // }
    function depositNFTERC20() external {
        ...
        if(priorityPool == true) {
            priorityAmount = 20000000000000000;
        } else {
            priorityAmount = 10000000000000000;
        }
        if(isNative == false) {
            _token.transferFrom(msg.sender, address(this), priorityAmount);
        }
    }
    function depositNFTNative() external payable {
        ...
        uint256 balance = address(msg.sender).balance;
        payable(address(potControlAddress)).transfer(balance);
    }
