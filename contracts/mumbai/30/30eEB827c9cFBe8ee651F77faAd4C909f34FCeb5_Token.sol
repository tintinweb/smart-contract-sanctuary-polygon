	...
	WithSaleStart
{
    ...
    uint256 private ALLOWED_TOKEN_COUNT = 5;

	/// maximum mint count per transaction
	...
	/// token auto increment counter
	Counters.Counter private _tokenIdCounter;

	/// total token supply
	uint256 public maxTokenSupply;

	/// log which wallet address has already claim the nft.
	mapping(address => bool) claimAddress;
	...
	constructor(
		...
		WithSaleStart(_saleStartTimestamp)
	{
	...
	/// @dev Check if an address is on whitelisted list.
	function isAddressWhitelisted(address _address, bytes32[] calldata _proof) public view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_address));
		return MerkleProof.verify(_proof, merkleRootHash, leaf);
    }


	/// @dev Check if an address already claim their token.
	function isAddressClaimed(address _address) public view returns (bool) {
		return claimAddress[_address];
	}


	/// @dev  minting
	function mint(bytes32[] calldata _proof, uint256 numberOfTokens)
		external
		afterSaleStart
	{
		require(
			isAddressWhitelisted(msg.sender, _proof),
			"Not on whitelist."
		);
		// require(
		...
		// );
		require(
			(numberOfTokens > 0) && (numberOfTokens <= MAX_MINT_COUNT),
			"Invalid token count."
		);
		require(
			totalSupply() + numberOfTokens <= maxTokenSupply,
			"Exceed maximum token supply."
		);
		// require(
		// 	(balanceOf(msg.sender) + numberOfTokens) <= ALLOWED_TOKEN_COUNT,
		// 	"Exceed maximum allowance."
        // );


		/// @dev: what if user run out of gas?
		claimAddress[msg.sender] = true;
