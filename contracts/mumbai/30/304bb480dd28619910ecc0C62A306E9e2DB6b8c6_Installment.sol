...
contract Installment is Ownable, AccessControl, ReentrancyGuard {
    ...
    */
    ...
    /** 
    * @dev Set SetVaultBalanceAddress `vaultAddress` for emit vaultBalance
    * Set SetVaultItemAddress `vaultAddress` for emit vaultItem
    * BuyItem `userAddress`,`itemId,`,`itemAmount`,`nakaAmount`for emit BuyItem
    */ 
    ...
    */
    modifier whenShopNotPaused() {
        require(!_paused, "[Shop.whenShopNotPaused] Not Paused");
        _;
    }
    ...
    */
    modifier whenShopPaused() {
        require(_paused, "[Shop.whenShopPaused] Paused");
        _;
    }
    ...
    function pruchase(address _sellerAccount, bytes32 _orderId , uint256 _period) public  {
        ...
        bytes32 _billId = keccak256(
         ...
         Bill memory bill = billByBillId[msg.sender][_billId] = Bill({
            billId : _billId,
            buyer : msg.sender,
    ...
    function payBill(address _buyerAccount, bytes32 _billId , uint256 _period,uint256 _nakaAmount ) public {
        Bill memory bill = billByBillId[_buyerAccount][_billId];
        vaultBalance.decreaseBalance(_buyerAccount, _nakaAmount * _period);
        vaultBalance.increaseBalance(bill.seller, _nakaAmount * _period);
        ...
        if (bill.periodBalance == 0){
            ...
            landNFT._beforeTokenTransfer(bill.seller,bill.buyer,bill.tokenId);
        }
     
      emit PayBill(_billId, _buyerAccount ,bill.seller,bill.periodBalance,bill.billBalance,_nakaAmount);
    }
    function cancelOrder(address _sellerAccount ,bytes32 _orderId) public   {
        Order memory order = orderByOrderId[_sellerAccount][_orderId];
    ...
    function cancelBill(address _sellerAccount ,bytes32 _billId) public  {
        Bill memory bill = billByBillId[_sellerAccount][_billId];
        require(bill.billId != 0, "Asset not published");
        require(bill.seller == msg.sender, "Unauthorized user");
        vaultBalance.increaseBalance(bill.buyer,bill.pledge);

        delete billByBillId[bill.seller][_billId];
    ...
    function Blacklist(address _sellerAccount ,bytes32 _billId) public {
        Bill memory bill = billByBillId[_sellerAccount][_billId];
        require(bill.billId != 0, "Asset not published");
        require(bill.seller == msg.sender, "Unauthorized user");
        vaultBalance.increaseBalance(bill.seller,bill.pledge);
    ...
    */
    function pauseShop() external onlyOwner whenShopNotPaused {
        _paused = true;
        emit ShopPaused();
    }
    ...
    * can only be called by the creator of contract.
    */
    function unpauseShop() external onlyOwner whenShopPaused {
        _paused = false;
        emit ShopUnpaused();
    }
...
interface ILandNFT{
    ...
    function _beforeTokenTransfer( address _from, address _to, uint256 _tokenId)external;
