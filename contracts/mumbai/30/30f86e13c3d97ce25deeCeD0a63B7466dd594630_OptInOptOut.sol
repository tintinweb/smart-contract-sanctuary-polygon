...
contract OptInOptOut is Initializable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    */
    function whitelistAdmin(address _admin) external onlyOwner isInitialized{
...
contract UserContract is Initializable, OwnableUpgradeable, UUPSUpgradeable{
    ...
    */
    function addUser(address _ad, uint _type) external isInitialized{
        if(!adminAddress[msg.sender]){ revert notAdminAddress();}
    ...
    */
    function addUserBulk(userBulkData[] memory userData) external isInitialized{
        if(!adminAddress[msg.sender]){ revert notAdminAddress();}
