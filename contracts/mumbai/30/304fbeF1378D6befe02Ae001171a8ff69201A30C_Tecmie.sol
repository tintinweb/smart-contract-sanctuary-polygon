...
contract Tecmie {
    ...
    TecmieContract tecmieContract;

    constructor(){
        owner = msg.sender;
        nextProposal = 1;
        tecmieContract = TecmieContract(0x2953399124F0cBB46d2CbACD8A89cF0599974963);
        validTokens = [10231361513290462284284883755725626377251653755052925501953286090101884977153];
    }
    ...
    function checkProposalEligibility(address _proposalist) private view returns (
        ...
        for(uint i = 0; i < validTokens.length; i++){
            if(tecmieContract.balanceOf(_proposalist, validTokens[i]) >= 1){
                return true;
            }
        }
        return false;
