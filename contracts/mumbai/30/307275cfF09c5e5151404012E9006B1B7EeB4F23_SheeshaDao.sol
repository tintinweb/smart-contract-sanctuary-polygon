...
import "./SheeshaFactory.sol";
import "./SheeshaVotes.sol";
import "./interfaces/ISheeshaDao.sol";
import "./interfaces/ISheeshaVoting.sol";

contract SheeshaDao is
    ISheeshaDao,
    ContextUpgradeable,
    ...
    bytes32 public constant SHEESHADAO_ADMIN_ROLE = keccak256("SHEESHADAO_ADMIN_ROLE");

    address public override votes;
    address[] public votings;

    mapping(address => mapping(address => bool)) public override delegates;
     ...
     */
    modifier onlyAdmin() {
        require(hasRole(SHEESHADAO_ADMIN_ROLE, _msgSender()), "SHD: admin only");
        _;
    }
     ...
     */
    modifier onlyMember() {
        require(ISheeshaVotes(votes).totalOf(_msgSender()) > 0, "SHD: member only");
        _;
    }
    ...
    modifier onlyLatestCompletedVoting() {
        ...
        require(_msgSender() == voting_ 
            && ISheeshaVoting(voting_).state() == ISheeshaVoting.State.STATE_COMPLETED,
            "SHD: completed and non-executed voting only");
        _;
    }

    function initialize(address voting, bytes calldata vaults) external initializer {
     ...
     */
    function __SheeshaDao_init(address voting, bytes calldata vaults) internal onlyInitializing {
     ...
     * @dev Initialize voting & vaults.
     */
    function __SheeshaDao_init_unchained(address voting, bytes calldata vaults) internal onlyInitializing {
    ...
    function activeVoting() public view override returns (address) {
        ...
        return ISheeshaVoting(voting_).state() == ISheeshaVoting.State.STATE_ACTIVE ? voting_ : address(0);
    }

    function createVoting(address implementation, bytes calldata data) external onlyAdmin {
        require(activeVoting() == address(0), "SHD: has active voting");
        address voting = SheeshaFactory.create(address(this), implementation, data);
        votings.push(voting);
        emit VotingCreated(_msgSender(), voting);
    }

    function delegate(address whom) external onlyMember {
        require(!delegates[_msgSender()][whom], "SHD: already delegated");
        delegates[_msgSender()][whom] = true;
        emit VotingDelegated(_msgSender(), whom);
    }

    function undelegate(address whom) external onlyMember {
        require(delegates[_msgSender()][whom], "SHD: already undelegated");
        delete delegates[_msgSender()][whom];
        emit VotingUndelegated(_msgSender(), whom);
    }

    function execute(address target, uint256 value, bytes calldata data) external override onlyLatestCompletedVoting {
        if (data.length == 0) {
...
import "./interfaces/ISheeshaDaoInitializable.sol";

library SheeshaFactory {
    function create(address dao, address implementation, bytes calldata data) internal returns (address instance) {
