...
contract shimadamasafumi_Artwork_Onchain_Datebase_v3 {
    ...
    require(_start <= _end, "Invalid slice range");
    require(_end <= _bytes.length, "Invalid slice range");
    ...
    function bulkImportWorks(string memory csvData) external onlyOwner {
    bytes memory csvBytes = bytes(csvData);
    require(csvBytes.length > 0, "CSV data is empty");

    uint256 currentIndex = 0;
    uint256 totalLength = csvBytes.length;
    string memory inventoryNumber;
    string memory title_en;
    string memory title_ja;
    ...
    while (currentIndex < totalLength) {
        // Parse each line from the CSV data
        string memory line;
        uint256 lineEndIndex = findLineEnd(csvBytes, currentIndex);
        if (lineEndIndex >= currentIndex) {
            line = string(slice(csvBytes, currentIndex, lineEndIndex));
        ...
        string[] memory fields = splitCSVLine(line);
        ...
        // Add the work
        addWorkInternal(inventoryNumber, title_en, title_ja, material, edition, year, size, imageUrl, status);
    }
}
    ...
    function addWork(
        ...
        addWorkInternal(_inventoryNumber, _title_en, _title_ja, _material, _edition, _year, _size, _imageUrl, _status);
    }

    function addWorkInternal(
        string memory _inventoryNumber,
        string memory _title_en,
        string memory _title_ja,
        string memory _material,
...
// Helper function to find the end index of a line in CSV data
function findLineEnd(bytes memory data, uint256 startIndex) internal pure returns (uint256) {
...
function splitCSVLine(string memory line) internal pure returns (string[] memory) {
    bytes memory lineBytes = bytes(line);
    uint256 fieldCount = 1;
    ...
    uint256 fieldStartIndex = 0;
    bool inQuotes = false;
    for (uint256 i = 0; i < lineBytes.length; i++) {
        if (lineBytes[i] == '"') {
            inQuotes = !inQuotes;
        } else if (lineBytes[i] == "," && !inQuotes) {
            fields[fieldIndex] = substring(line, fieldStartIndex, i);
            fieldIndex++;
            fieldStartIndex = i + 1;
        }
    }
    fields[fieldIndex] = substring(line, fieldStartIndex, lineBytes.length);
    return fields;
}
...
function substring(string memory str, uint256 startIndex, uint256 endIndex) internal pure returns (string memory) {
    ...
    require(endIndex <= strBytes.length, "Invalid substring range");
