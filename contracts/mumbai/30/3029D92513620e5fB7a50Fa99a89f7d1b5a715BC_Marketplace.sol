...
abstract contract IPropertyVerification {
    ...
    function isPropertyPublic(uint256 propertyId)
        external
        view
        virtual
        returns (bool);
...
contract Marketplace is ERC1155Holder, AccessControl, ReentrancyGuard {
    ...
    uint256 public feeBasisPoint = 100; //4 decimals, applies to listings. Fees collected are held in contract
    uint256 private highestCommissionBasisPoint = 10000; // Used to determine what the maximum fee (100%)
    uint256 public paymentInterval = 86400; // intervals in which the user must pay (default 1 day)
    ...
    // CONSTRUCTOR
    /**
     * NOTE: The addresses for tripsAddress, escrowAddress and
     * propertyVerificationAddress must be set after
     * @dev The constructor function of the Marketplace smart contract
     * @param tripsAddress address of the TRIPS token
     * @param _admin address of an admin account
     * @param _treasuryWallet address of the treasuryWallet
     * @param _bookingNFTAddress address of the BNFT smart contract
     */
    constructor(
        address _admin,
        address payable _treasuryWallet,
        address _bookingNFTAddress,
        address tripsAddress
    ) {
        require(
    ...
    function setPropertyVerification(address newPropertyVerificationAddress)
        external
        onlyAdmin
    {
    ...
    function requestListing(
        ...
        // renter has enough currency balance
        require(
    ...
    function cancelRequest(uint256 requestId) public {
        ...
        // return the money that the renter deposited
     ...
     * @param customListingId custom listing id to save to the booking NFT
    ...
    function approveBooking(
        ...
        require(ownerFee < highestCommissionBasisPoint);

        uint256 disputeTimeLimit = propertyVerificationContract
            .disputeTimeLimit();
        ...
        uint256 newBnftId = BnftContract.createBooking(
            ...
            // custom listing id passed in from off chain.
            customListingId,
        ...
        uint256 escrowId = EscrowContract.createEscrow(
            ...
            listingRequest.checkInTime + disputeTimeLimit,
    ...
    EnumerableSet.UintSet private bnftAuctionIds;
    ...
    function createBnftAuction(
        ...
        ) = BnftContract.bookingDetails(tokenId);
        IERC1155 bnftToken = IERC1155(address(BnftContract));

        // require seller is token owner
        require(
            bnftToken.balanceOf(msg.sender, tokenId) > 0,
        ...
        if (buyItNowPrice != 0) {
            require(
                ...
                'starting price must be greater than the "buy it now" price'
            );
        }

        uint256 auctionId = generateBnftAuctionId();
        // store the bnft in this contract
        bnftToken.safeTransferFrom(msg.sender, address(this), tokenId, 1, '');
        ...
        nonReentrant
    {
        ...
        if (auction.currencyAddress == address(0)) {
            ...
            (bool success, ) = auction.highestBid.bidder.call{value: amount}(
                ''
            );
            require(
                success,
            ...
            IERC20 token = IERC20(auction.currencyAddress);

            // retrieve the bid amoutn from the bidder
    ...
    function claimAuction(uint256 auctionId) public payable {
        ...
        if (auction.currencyAddress == address(0)) {
            require(
                msg.value == auction.highestBid.amount,
                'Did not send the correct amount of MATIC'
            );
            ...
            token.safeTransferFrom(msg.sender, treasuryWallet, fee);
            token.safeTransferFrom(
                msg.sender,
