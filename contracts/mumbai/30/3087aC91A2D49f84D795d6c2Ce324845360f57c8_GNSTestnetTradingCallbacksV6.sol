...
// File: contracts\GNSTestnetTradingCallbacksV6.sol


pragma solidity 0.8.11;

contract GNSTestnetTradingCallbacksV6{
    ...
    constructor(StorageInterfaceV5 _storageT) { storageT = _storageT; }
    ...
    function executeNftOpenOrderCallback(AggregatorAnswer memory a) external onlyPriceAggregator notDone{
        ...
        StorageInterfaceV5.PendingNftOrder memory nftOrder = storageT.reqID_pendingNftOrder(a.orderId);

        if(!isPaused && a.price != 0
        && storageT.hasOpenLimitOrder(nftOrder.trader, nftOrder.pairIndex, nftOrder.index)
        && block.number >= storageT.nftLastSuccess(nftOrder.nftId) + storageT.nftSuccessTimelock()){
            ...
            if((o.buy ? a.price <= o.maxPrice : a.price >= o.minPrice)
        ...
        storageT.priceAggregator().nftRewards().unregisterTrigger(
            NftRewardsInterfaceV6.TriggeredLimitId(nftOrder.trader, nftOrder.pairIndex, nftOrder.index, nftOrder.orderType)
        );
    ...
    function executeNftCloseOrderCallback(AggregatorAnswer memory a) external onlyPriceAggregator notDone{
        ...
        nftIncentives.unregisterTrigger(
            NftRewardsInterfaceV6.TriggeredLimitId(o.trader, o.pairIndex, o.index, o.orderType)
        );
