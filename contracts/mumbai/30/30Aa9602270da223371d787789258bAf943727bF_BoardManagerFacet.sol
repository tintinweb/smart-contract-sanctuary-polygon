...
contract BoardManagerFacet {
    ...
    //hostOnly
    modifier onlyHost {
        BigBoardManagerLib.onlyHost();
        _;
    }

    modifier founderOrHostOnly {
        BigBoardManagerLib.founderOrHostOnly();
        _;
    }
    ...
    function cancelBoard(address _board) external founderOrHostOnly {
        ...
        if(f.founderz[msg.sender]){
            boardToBeCanceled.cancelBoard();
        } else { // if msg.sender is not factory founder
            require(msg.sender == boardToBeCanceled.host(), "Only 1 host can cancel this board.");
            boardToBeCanceled.cancelBoard();
        }
     ...
     function withdrawBBH() external onlyFounder {
        ...
        require(f.hostToken.transfer(msg.sender, f.hostToken.balanceOf(address(this))), "Unable to transfer");
    }
}
