...
contract StakingV2 is Ownable, ReentrancyGuard {
  ...
  function unstake() external nonReentrant {
    ...
    token.safeTransfer(msg.sender, stakeAmount.add(reward.sub(userInfo[msg.sender].rewardDebt)));
   ...
   */
  function updatePool(uint256 _totalWeightedScore) external onlyGovernment {
    require(block.timestamp >= lastRewardBlockTime, "invalid call");

    uint256 last = lastRewardBlockTime.sub(startBlockTime).div(ONE_WEEK);
  ...
  function apy() external view returns (uint256) {
    ...
    if (totalStaked == 0) {
      ...
      _apy = rewardDrop[current].mul(WEEKS_OF_ONE_YEAR).mul(MAX_BPS).div(totalStaked);
    }
  ...
  function getScore(uint256 stakingId, uint256 weekNumber) internal view returns (uint256) {
    uint256 score = 0;
    uint256 diff = weekNumber > WEEKS_OF_ONE_YEAR ? WEEKS_OF_ONE_YEAR : weekNumber;
