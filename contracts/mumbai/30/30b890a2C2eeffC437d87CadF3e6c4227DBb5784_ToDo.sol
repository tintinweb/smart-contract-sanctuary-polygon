...
contract ToDo is BaseRelayRecipient {
  ...
  function addToDo(string calldata _todo) public {
    _todoList[msg.sender].push(Memo(false, _todo));
  }

  function toggleToDoCompleted(uint8 index) public {
    _todoList[msg.sender][index].completed = !_todoList[msg.sender][index].completed;
  }

  function deleteToDo(uint8 index) public {
    uint256 end = _todoList[msg.sender].length - 1;
    for (uint8 i = index; i < end; i++) {
      _todoList[msg.sender][i] = _todoList[msg.sender][i + 1];
    }
    _todoList[msg.sender].pop();
  }

  function todoList() public view returns (Memo[] memory) {
    return _todoList[msg.sender];
  }
