...
contract NFTDOTCManager is ERC1155Holder, INFTdOTC, ReentrancyGuard, AdminFunctions {
    ...
    function setNftEscrowLinker() external returns (bool status) {
        require(hasRole(NFT_ESCROW_MANAGER_ROLE, _msgSender()), "Not allowed");
    ...
    function setNftEscrowAddress(address _address) public returns (bool status) {
        require(hasRole(NFT_ESCROW_MANAGER_ROLE, _msgSender()), "Not allowed");
        escrow = _address;
        return true;
    }
    ...
    */
    ...
    modifier isSpecialNftOffer(uint256 offerId) {
        if (allNftOffers[offerId].specialAddress != address(0)) {
            require(allNftOffers[offerId].specialAddress == msg.sender, "CAN'T_TAKE_OFFER");
        }
        _;
    }
