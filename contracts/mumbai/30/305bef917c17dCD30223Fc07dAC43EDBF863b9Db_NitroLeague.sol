...
interface INitroLeague {
    ...
    function createRaceEvent(string calldata raceEventID, uint256 raceEventType) external returns(address);
...
contract Race is IRace, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    enum RaceState { UNSCHEDULED, SCHEDULED, ACTIVE, COMPLETE }
...
contract RaceEvent is IRaceEvent, Context, Ownable, RewardManager, TokenWithdrawer {
    ...
    enum RaceEventType { DAILY, WEEKLY, MONTHLY, TOURNAMENT }
    ...
    constructor(address nitroLeague_, address raceFactory_, uint256 raceEventType_) {
        // Access.
        nitroLeague = INitroLeague(nitroLeague_);
...
contract NitroLeague is INitroLeague, Context, Ownable, TokenWithdrawer {
    ...
    function createRaceEvent(string calldata raceEventID, uint256 raceEventType) external override returns(address) {
        ...
        IRaceEvent raceEvent = IRaceEvent(raceEventFactory.newRaceEvent(
            address(this), address(raceFactory), raceEventType
        ));
