...
contract Parts is ERC721Enumerable, AccessControl {
    ...
    // internal
    function validateHolderMinting(
    ...
    function mintPartsByApeNFTOwner(uint256 blockNumberLimit, uint8 nftType, uint256[] calldata tokenIds, bytes calldata signature) external {
        require(block.number <= blockNumberLimit, "transaction has expired");
        require(validateHolderMinting(blockNumberLimit, nftType, _msgSender(), tokenIds, signature), "Invalid signature");
