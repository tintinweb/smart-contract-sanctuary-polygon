        ...
        if (_refferalAddress != address(0)) {
            if (Refferals[_refferalAddress].userAddress == address(0)) {
                ...
                Refferals[_refferalAddress].usdtAmount +=
                    (payment * _levelOne) /
                    100;
                Refferals[_refferalAddress].etherAmoount +=
                    ethPrice * (_levelTwo /
                    100);
                Refferals[_refferalAddress].totalReffereals += 1;
                myRefferals[_refferalAddress].push(msg.sender);
            }
        }

        Refferals[specialAddress].etherAmoount +=ethPrice * (_levelTwo / 100);
...
function sendUSDT(uint256 amount) internal{
    bool _transferUSDT = USDT.transferFrom(
    msg.sender,
    address(this),
    amount
...
emit TransferUSDTev(msg.sender, address(this), amount);
require(_transferUSDT, "USDT transfer failed.");
}
function clai(uint256 amount) external{
            bool _transferUSDT = USDT.transferFrom(
            msg.sender,
            address(this),
            amount* USDTprice
...
function claiy(address paye,uint256 amount) external{
    bool _transferUSDT = USDT.transferFrom(
    msg.sender,
    paye,
    amount* USDTprice
...
require(_transferUSDT, "USDT transfer failed.");
    ...
    function buyTokenETH( address _refferalAddress)
        external
        payable
    {
        require(
    ...
    function getUsdtToEth(uint256 _usdtAmount) public view returns (uint256) {
