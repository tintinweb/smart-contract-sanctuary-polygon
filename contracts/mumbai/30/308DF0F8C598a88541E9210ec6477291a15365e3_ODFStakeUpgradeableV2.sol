...
contract ODFStakeUpgradeableV2 is Initializable, IODFStake, OwnableUpgradeable {
    using CountersUpgradeable for CountersUpgradeable.Counter;
    CountersUpgradeable.Counter private _stakingCounter;
    /**
     ...
     */
    function stake(uint256 amount_, uint16 _period) external virtual {
    ...
    function getRewards(
        uint256 id_
    ) public view virtual returns (uint256 rewards) {
     ...
     * @param id_ Id of the stake.
     */
    function unStake(uint256 id_) external virtual {
        Stake memory stake_ = stakes[id_];
        ...
        delete stakes[id_];
        require(stakingToken.transfer(msg.sender, rewards + amount));
        ...
        returns (StakeReadOnly[] memory)
    {
        ...
        StakeReadOnly[] memory stakes_ = new StakeReadOnly[](stakeCount);

        for (uint256 i = total; i > 0; ) {
            if (stakes[i].account != address(0)) {
                StakeReadOnly memory stake_ = StakeReadOnly(
                    i,
                    stakes[i].amount,
                    stakes[i].period,
                    stakes[i].timestamp,
    ...
    function getUserStakes(
        address account
    ) external view virtual returns (StakeReadOnly[] memory) {
        uint256 total = _stakingCounter.current();
        uint256 stakeCount = 0;
        for (uint256 i = total; i > 0; ) {
