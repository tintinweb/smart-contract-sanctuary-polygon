...
abstract contract ERC1155Collection is ERC1155Proxy, IERC1155Collection {
        ...
        returns (string memory)
    {
        ...
        if (bytes(baseCollectionURI).length == 0 || bytes(_collectionMetadataURIs[_id]).length > 0)
        ...
        returns (string memory)
    {
        ...
        if (bytes(_tokenMetadataURIs[_id]).length > 0) return _tokenMetadataURIs[_id];
...
abstract contract ERC1155MetadataURI is ERC1155, IERC1155MetadataURIUpgradeable {
    ...
    function uri(uint256 _id)
        ...
        returns (string memory)
    {
        if (bytes(baseURI).length == 0 || bytes(_tokenMetadataURIs[_id]).length > 0) 
            return _tokenMetadataURIs[_id];
        return string(abi.encodePacked(baseURI, Strings.uint2str(_id), ".json"));
    }
...
library LibERC1155Lazy {
    ...
    bytes32 public constant MINT_SINGLE_TYPEHASH =
        keccak256(
            "MintSingleData(uint160 minter,uint32 collectionId,uint64 tokenId,uint256 supply,string collectionUri,string baseUri,string tokenUri,Part[] creators,Part[] royalties)Part(address account,uint256 value)"
        );
        ...
        return
            keccak256(
                abi.encode(
                    ...
                    keccak256(abi.encodePacked(data.tokenUri)),
                    keccak256(abi.encodePacked(creatorsBytes)),
...
import "@passive-income/naas-erc-1155/contracts/abstracts/ERC1155/presets/ERC1155Tradeable.sol";
import "@passive-income/naas-erc-1155/contracts/abstracts/ERC1155/extensions/ERC1155Proxy.sol";
import "@passive-income/naas-erc-1155/contracts/abstracts/ERC1155/extensions/ERC1155Collection.sol";
import "@passive-income/naas-erc-1155/contracts/interfaces/ICreatorsRegistry.sol";
import "@passive-income/naas-erc-1155/contracts/libraries/LibTokenIdentifier.sol";
...
contract NaaS is ERC1155Tradeable {
    ...
    event MinterAdded(address indexed addedBy, address indexed minter);
    event MinterRevoked(address indexed revokedBy, address indexed minter);

    function __ERC1155NaaS_init(
        string memory _name,
        string memory _symbol,
        IProxiesRegistry _proxiesRegistry,
        ICreatorsRegistry _creatorsRegistry,
    ...
    function setMinter(address minter, bool value) external onlyOwner {
        require(isMinter[minter] == value, "ALREADY_VALUE");
        isMinter[minter] = value;
        if (value) emit MinterAdded(_msgSender(), minter);
        else emit MinterRevoked(_msgSender(), minter);
    }
...
contract TagPlaceAsset is NaaS {
    function initialize(
        IProxiesRegistry _proxiesRegistry,
        ICreatorsRegistry _creatorsRegistry,
        INaaSRoyaltiesProvider _royaltiesProvider,
