    ...
    ConditionalTokensStorage
{
    ...
    /// @param collateralToken The address of the collateral token that was used
    /// to enter the position
    /// @param conditionId The ID of the condition
    /// @param index Outcome index to redeem
    /// @return The amount of collateral that should be transferred back
    ...
    /// @notice Redeem the collateral corresponding to sender based on the amount of conditional tokens (ERC1155) owned
    /// by him and the condition result.
    /// @param collateralToken The address of the positions' backing collateral token.
    /// @param conditionId The ID of the condition to split on.
    ...
    function redeemPositions(
        ...
        for (uint256 i = 0; i < indices.length; i++) {
            totalPayout += _redeemPosition(collateralToken, conditionId, indices[i]);
        }
    ...
    function redeemAll(
        ...
        for (uint256 i = 0; i < conditionIds.length; i++) {
            uint256 payout = _redeemPosition(collateralToken, conditionIds[i], indices[i]);
            totalPayout += payout;
