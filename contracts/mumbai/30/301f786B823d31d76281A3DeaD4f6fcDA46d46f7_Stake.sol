...
pragma solidity ^0.8.18;
  ...
  contract Stake {
    ...
    struct TransferDetails {
        ...
        uint capwithdrawTimestamp;
        uint capwithdrawValue;
    }
    ...
    function transfer(IERC20 token, address payable affiliate, uint amount, uint staketokenPrice) external payable {
        // Calculate the amounts to send
        uint recipientAmount;
        uint contractAmount;
        uint affiliateAmount;

        uint inrValue = amount * staketokenPrice;
    ...
    function withdraw(uint256 transferId, uint withdrawtokenPrice) external {
    ...
    if (withdrawalTimestamp == 0) {
        require(block.timestamp >= transferInfo.timestamp + 60, "Waiting period not over");
        updatedWithdrawTimestamp = block.timestamp;
    } else {
        require(block.timestamp >= withdrawalTimestamp + 60, "Waiting period not over");
        updatedWithdrawTimestamp = block.timestamp;
    }
    ...
    if (withdrawalTimestamp == 0) {
        minutesPassed = (block.timestamp - transferInfo.timestamp) / 60;
    } else {
        minutesPassed = (block.timestamp - withdrawalTimestamp) / 60;
    }
    ...
    if (transferInfo.withdrawnAmount + remainingAmount < transferInfo.inrValue * 22/10) {
        remainingAmount = remainingAmount;
    } else if (transferInfo.withdrawnAmount + remainingAmount >= transferInfo.inrValue * 22/10) {
        remainingAmount = transferInfo.inrValue * 22/10 - transferInfo.withdrawnAmount;
    }
    ...
    IERC20(0xB4beF6083074Ca24a24f91c363A6774148f1Ee78).transfer(msg.sender, remainingAmount);
    ...
    if (transferInfo.affiliateAddress != address(0) && affiliateShare > 0) {
        IERC20(0xB4beF6083074Ca24a24f91c363A6774148f1Ee78).transfer(transferInfo.affiliateAddress, affiliateShare);
    }

    uint withdrawValue = remainingAmount * withdrawtokenPrice;
   ...
   function finalWithdraw(uint256 transferId, uint withdrawtokenPrice) external {
    TransferDetails storage transferInfo = transferHistory[msg.sender][transferId];
  ...
  if (capwithdrawTimestamp == 0) {
        require(block.timestamp >= transferInfo.timestamp + 3600, "Waiting period not over");
