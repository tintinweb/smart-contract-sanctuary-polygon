...
contract Factory_Covest is Initializable, UUPSUpgradeable, ReentrancyGuardUpgradeable  {
    ...
    mapping(string => mapping(address => mapping(address => bool))) private _dataUserPair;
    ...
    event PairCreated(string PolicyName,string poolId,address policy,address nameRegistry,uint256 Id);

    function hasRole(bytes32 role, address account) public view returns (bool) {
        return AR.hasRole(role,account);
    }

    function allPairs(uint256 _policyId) public view returns(PairsData memory) {
        return _dataPairing[_policyId];
    }

    function getPair(string memory _poolId) public view returns(address policy,address nameRegistry) {
        if(_dataPairingBool[_poolId] == true){
            return (_dataPairing[_dataStringToParingId[_poolId]].policy,_dataPairing[_dataStringToParingId[_poolId]].nameRegistry);
        }else{
    ...
    function createPair(address _policyDetails,address _nameRegistry) public isSuperManager  {
        require(_policyDetails != address(0) && _nameRegistry != address(0));
        uint Id = pairsarray.length;
        IPolicyDetails _PDS_ = IPolicyDetails(_policyDetails);
        string memory poolName = _PDS_.poolName();
        string memory poolId = _PDS_.poolId();
        _dataPairing[Id] = PairsData(poolName,poolId,_policyDetails,_nameRegistry);
