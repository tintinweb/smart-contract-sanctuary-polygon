    ...
    ERC1155Supply
{
    ...
    function mint(
        ...
        if (address(token) != address(0)) {
            ...
            require(!tokensUsed[address(token)][id][tokenId], "Id already used");
            require(
    ...
    function mintMultiple(
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (token == rayc) {
                totalValue += tokenIdToConfig[id].priceRayc;
            } else if (token == zayc) {
                totalValue += tokenIdToConfig[id].priceZayc;
            } else {
                require(
                    tokenIdToConfig[id].openToPublic,
                    "Not open to the public"
                );
                totalValue += tokenIdToConfig[id].pricePublic;
            }
        }
