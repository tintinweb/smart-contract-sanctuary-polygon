...
contract LuckyMe is Ownable {
  ...
  ********************************************************/
  ...
  function register(
    ...
    registerUplineMemberRef(_plan, _ref, _amount);
  ...
  function renew(uint256 _id) public noReentrant {
    ...
    renewUplineMemberRef(_user.Ref, _amount);
  ...
  function upgradePlan(uint256 _id) public noReentrant {
    ...
    upgradePlanUplineMemberRef(_user.Ref, _amount);
  ...
  function enterGame(
    ...
    uplinePicksRef(_user.Ref, _amount);
  ...
  function withdrawPrizes(uint8 _game, uint256 _gameId) public noReentrant {
    ...
    for (uint8 i = 0; i < 3; i++) {
      ...
      uplinePrizesRef(_User.Ref, _game, i);
  ...
  ********************************************************/
  ...
  function upgradePlanUplineMemberRef(uint256 _ref, uint256 _amount) private {
    uint8 j;
    while (j < RefLevels) {
      user memory _user = User[_ref];
      if (
  ...
  function renewUplineMemberRef(uint256 _ref, uint256 _amount) private {
    uint8 j;
    while (j < RefLevels) {
      user memory _user = User[_ref];
      if (
  ...
  function uplinePicksRef(uint256 _ref, uint256 _amount) private {
    uint8 j;
    while (j < PurLevels) {
      user memory _user = User[_ref];
  ...
  function uplinePrizesRef(uint256 _ref, uint8 _game, uint8 _i) private {
    uint8 j;
    while (j < PriLevels) {
      user memory _user = User[_ref];
