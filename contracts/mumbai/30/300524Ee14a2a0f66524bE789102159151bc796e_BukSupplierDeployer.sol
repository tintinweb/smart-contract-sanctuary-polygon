...
* @title Buk Supplier Deployer Contract
...
contract BukSupplierDeployer is Context {
    ...
    */
    function updateFactory(address _factory_contract) public onlyAdmin(_msgSender()) {
        factory_contract = _factory_contract;
...
contract SupplierContract is AccessControl, ERC1155 {
    ...
    */
    mapping(uint256 => string) bookingTickets; //tokenID -> uri
    /**
    * @dev Mapping for token transferability status
    */
    mapping(uint256 => bool) transferable; //tokenID -> uri
    ...
    */
    event UpdateSupplierDetails(address indexed utility_contract);
    /**
    * @dev Event to set new contract URI
    */
    event SetURI(string indexed contract_uri);
    /**
    ...
    */
    function updateSupplierDetails(string memory _name) external onlyRole(UPDATE_CONTRACT_ROLE) {
        ...
        emit UpdateSupplierDetails(utility_contract);
    }
    ...
    */
    function safeBatchTransferFrom(address from, address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) public virtual override {
        for(uint i=0; i<ids.length; i++) {
            require(transferable[ids[i]]==true, "One of these NFT is non-transferable");
        }
        require(from == _msgSender() || isApprovedForAll(from, _msgSender()), "ERC1155: caller is not token owner or approved");
        _safeBatchTransferFrom(from, to, ids, amounts, data);
    }
