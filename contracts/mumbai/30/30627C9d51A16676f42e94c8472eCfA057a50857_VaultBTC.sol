...
contract VaultBTC is Ownable {
    using SafeMath for uint256;
    using Address for address;

    bool private active;
    address private eBTC;
    address private BTC;
    IERC20 private isa;
    ...
    AggregatorV2V3Interface public btcOracle;
    ...
    constructor(
        ...
        btcOracle = AggregatorV2V3Interface(_btcOracle);
    }

    event PriceUpdate(uint256 BTCPrice, uint256 eBTCPrice, bool updated);
    event TokenBuy(
        address account,
        address token,
    ...
    function getLatestPrices() public view returns (uint256, uint256) {
        ...
        int256 price1 = btcOracle.latestAnswer();
    ...
    function geteBTCToken() public view returns (address) {
        return (eBTC);
    }

    function getBTCToken() public view returns (address) {
        return (BTC);
    }
    ...
    function setTokens(address eTYAddress, address eITYAddress)
        ...
        onlyOwner
    {
        require(BTC == address(0) || eBTC == address(0));
        (eBTC, BTC) = (eITYAddress, eTYAddress);
        price[BTC] = initialPrice;
        price[eBTC] = initialPrice;
    }
