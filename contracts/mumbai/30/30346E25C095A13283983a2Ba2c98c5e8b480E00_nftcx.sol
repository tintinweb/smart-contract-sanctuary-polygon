...
abstract contract ReentrancyGuard {
    ...
    enum typeSet {AllSet, GoldSet, SilverSet, BronzeSet, taskUserSet, allGoldUserSet, allSilverUserSet, allBronzeUserSet, GoldLastIndexSet}
interface IERC721 {
    function transferFrom(
        address from,
        address to,
...
contract nftcx is Ownable {
    ...
    event BindReferer(uint256 _time, address _user, address _referer, address _node);
    ...
    function addNodeList(address[] memory _nodeList) external onlyOwner {
        for (uint256 i = 0; i < _nodeList.length; i++) {
            ...
            if (!AllSet.contains(_node)) {
                ...
                emit BindReferer(block.timestamp, _node, address(0), address(1));
            }
        }
    }
    ...
    function buyNFT(address _referer) external {
        ...
        if (nodeList[_referer] == address(1)) {
            ...
            emit BindReferer(block.timestamp, _user, _referer, _node);
        } else {
            //推荐人是铜卡
            ...
            if (taskAmountList[_referer] == 2) {
                ...
                emit BindReferer(block.timestamp, _referer, refererList[_referer], address(2));
            }
        }
        address _sonAddress = _user;
        for (uint256 k = 0; k < 1000000; k++) {
            _sonAddress = refererList[_sonAddress];
            if (_sonAddress == address(0)) {
                break;
            }
            sonAmountList[_sonAddress] = sonAmountList[_sonAddress].add(1);
        }
    }
    ...
    function upgradeToGold(uint256 _swapOutAmount) external {
        ...
        emit BindReferer(block.timestamp, _user, refererList[_user], address(1));
    }
