...
contract NftAuction is IERC721Receiver, ReentrancyGuard, AccessProtected, BaseRelayRecipient {
    ...
    event BidPlacedInEnglishAuction(uint256 indexed auctionId, uint256 indexed bidPrice, address indexed bidder,bool paymentTypeMatic);
    ...
    function transferWithTreasury(address _to, uint256 _amount) internal {
        uint256 treasuryFee = _amount.mul(treasuryPercentagehaka).div(100).div(100);
        erc20.safeTransfer(treasuryAddress, treasuryFee);
        erc20.safeTransfer(_to, _amount - treasuryFee);
    }
    ...
    function startDutchAuction(
        ...
        return openAuction(choice, _nftIds, _startPrice, _endBid, _duration,_paymentTypeMatic);
    }
    ...
    function startEnglishAuction(
        ...
        return openAuction(choice, _nftIds, _startPrice, 0, _duration,_paymentTypeMatic);
    }
    ...
    function placeBidInEnglishAuction(
        ...
        if(!auctions[_auctionId].paymentType){
           erc20.safeTransferFrom(_msgSender(), address(this), toLock);
        }
        ...
        emit BidPlacedInEnglishAuction(_auctionId, auctions[_auctionId].highestBid, auctions[_auctionId].highestBidder,auctions[_auctionId].paymentType);
    }
    ...
    function buyNftFromDutchAuction(
        ...
        auctions[_auctionId].highestBid = currentPrice;
        auctions[_auctionId].highestBidder = _msgSender();
        auctions[_auctionId].isSold = true;
        uint256 fees=currentPrice*treasuryPercentageMatic/10000;
        uint256 Maticpay=currentPrice-fees;
    ...
    function claimFundsFromEnglishAuction(uint256 _auctionId) external nonReentrant {
        ...
        if(!auctions[_auctionId].paymentType){
        erc20.safeTransferFrom(address(this), sender, claimable);
        }
        else{
