...
contract TravelMission is AirdroppedLocationMission, IInventoryEntityContract {
  IInventoryItemContract rewardItem;
  Entity thisEntity = Entity({ _contract: this, id: 2 });
  ...
  function setRewardItem(address _rewardItem) external onlyController {
    rewardItem = IInventoryItemContract(_rewardItem);
  }
    ...
    returns (Reward memory)
  {
    ...
    //choose the right contract for getting the reward, pxl or loot (NFT)
    IMissionBudget rewardContract = pxl_budget;
    if (mission.loot_index > -1) {
      rewardContract = lootCoordinator;
    }
    return
      rewardContract.getReward(
        ...
        score,
        totalPossible,
        mission.reward, //this is a bounty
        mission.loot_index
      );
  }
