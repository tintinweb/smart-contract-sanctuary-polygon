...
//-------------------------------ADMIN CALLER FUNCTION -----------------------------------
    ...
    function setPrams(uint32 withdrwalFee, uint64 setPoolTime, uint32 setPoolLimit,uint32 _maxCycle,uint64 _royaltyClosingTime, uint32 _AR_validity, bool _poolSpan)  external  returns(bool){
   ...
   function _updateTeamNum(address ref) internal  {
    ...
    function _poolPosition()internal{
        ...
        //Pool time reset
        if (userInfos[msgSender].poolTime < now){
            userInfos[msgSender].poolTime = uint64(sysInfos.pool2Deadline+now);
  ...
  function _distributeGapBonus(address _user,address ref)  internal returns(bool){
    ...
    function _closingRoyality() internal {
        ...
        if(nextRoyalty<=now || defaultAddress==msg.sender ){
            ...
            nextRoyalty=uint64 (now + sysInfos.royaltyValidity);
            //Reset collection
            delete GR_collection;
            delete AR_collection;
    ...
    function _payCulculate (address to_usr,uint32 typ,uint128 amot)internal  returns (uint128){
