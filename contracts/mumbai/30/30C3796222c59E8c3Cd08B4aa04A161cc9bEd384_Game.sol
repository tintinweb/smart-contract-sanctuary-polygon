...
contract Game is IGame, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable{
    ...
    function canShoot(address account) public override view returns (bool) {
        return !requestIdUsed[userRequests[account].requestId] && userRequests[account].requestId != 0;
    }
    ...
    function arrangeMissile(uint256[12] memory positions) external payable override nonZero(positions) nonReentrant whenNotPaused {
        (uint256 price, uint256[] memory events, UseCache[] memory useCache, uint256 amountAff, uint256 amountToMinimum) = calPrice(positions, msg.sender);
        ...
        userRequests[msg.sender].requestId = requestId;
        userRequests[msg.sender].price = price;
    ...
    function shoot() external override nonReentrant whenNotPaused {
        ...
        require(canShoot(msg.sender), "can not shoot");
        ...
        (uint256 finalReward,) = rewardMul(userReward, msg.sender);
        playNumbers += 1;
    ...
    function calPrice(
        ...
        address account
    ) public override view returns (uint256 price, uint256[] memory, UseCache[] memory, uint256 amountAff, uint256) {
        uint256 value;
    ...
    function getFreeMissilePerAccount(address account) external override view returns (uint256[] memory) {
        ...
        if (freeAffiliate[account].typeMissile != 0) {
    ...
    function setGoldRate(uint256 newGoldRate) external onlyOwner {
        require(newGoldRate < 1000000 && newGoldRate != 0, "invalid");
        goldRate = newGoldRate;
    }

    function setGhostRate(uint256 newGhostRate) external onlyOwner {
        require(newGhostRate < 1000000 && newGhostRate != 0, "invalid");
    ...
    function addToWhitelist(address account_) external onlyOwner {
        require(whitelistIds[account_] == 0 && account_ != address(0), "existed");
    ...
    function addToWhitelistFree(address account_, uint256 freeEventId_) external whitelistValidateById(freeEventId_) onlyOwner {
        require(whitelistFreeIds[account_][freeEventId_] == 0 && account_ != address(0), "existed");
...
pragma solidity ^0.8.0; 
 ...
 interface IGame {
    ...
    function calPrice(uint256[12] memory positions, address account) external view returns (uint256, uint256[] memory, UseCache[] memory, uint256 amountAff, uint256);
