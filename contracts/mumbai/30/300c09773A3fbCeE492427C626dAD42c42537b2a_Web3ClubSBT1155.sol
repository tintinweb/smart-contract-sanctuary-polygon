...
// File contracts/Web3Club1155.sol




contract Web3ClubSBT1155 is ERC1155URIStorage,ISoul{
    string public name;
    string public symbol;
    address public scoreContract;
    address private owner;
    mapping(uint256 => string) soulName;
    mapping(uint256 => bool) isCreated;

    modifier isOwner{
        require(msg.sender == owner,"you are not owner");
        _;
    }
    modifier isScoreContract{
        require(msg.sender == scoreContract,"only scoreContract could");
        _;
    }
    ...
    //设置做题记录合约
    function setScoreContract(address _scoreContract) external isOwner{
        scoreContract = _scoreContract;
    }
    //铸造sbt
    function mint(address to, uint256 id) external isScoreContract{
        _mint(to,id,1,"");
        isCreated[id] = true;
    }
    //设置sbt的URI
    function setURI(uint256 id, string memory tokenURI) external isOwner{
        _setURI(id, tokenURI);
    }
    //设置sbt的soulName
    function setSoulName(uint256 id, string memory sbtName) external isOwner{
        soulName[id] = sbtName;
    }
    //实现ISoul
    //得到sbt的soulName
    function getSoulName(uint256 sbtId) external view returns(string memory){
        require(isCreated[sbtId],"sbt dosen't exist");
        return soulName[sbtId];
    }
    //判断该sbt是否被铸造
    function isSoulbound(uint256 sbtId) external view returns (bool){
        return isCreated[sbtId];
    }
    //得到sbt的uri
    function soulURI(uint256 sbtId) external view returns(string memory){
        require(isCreated[sbtId],"sbt dosen't exist");
