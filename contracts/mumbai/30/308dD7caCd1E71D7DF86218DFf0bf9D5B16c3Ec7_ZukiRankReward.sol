...
contract ZukiRankReward is Ownable, Pausable, ReentrancyGuard {
    ...
    address public ZK_TOKEN = 0xD7023b58e27F2aEa2e43BBd77d47C4f20fb83d84;
        ...
        whenNotPaused
    {
        ...
        uint256 gpAmount = campaigns[campaignId]
            ...
            .mul(ranks[whiteList[_msgSender()]])
            .div(PERCENTS_DIVIDER);
        uint256 zkAmount = campaigns[campaignId]
            .totalPoolZK
            .mul(ranks[whiteList[_msgSender()]])
            .div(PERCENTS_DIVIDER);
        IERC20(ZP_TOKEN).transfer(_msgSender(), zpAmount);
        IERC20(GP_TOKEN).transfer(_msgSender(), gpAmount);
        IERC20(ZK_TOKEN).transfer(_msgSender(), zkAmount);
        emit Claim(_msgSender(), gpAmount, zpAmount, zkAmount);
        userInfo.campaigns.push(campaigns[campaignId]);
        campaigns[campaignId].totalClaimed = campaigns[campaignId]
            .totalClaimed
            .add(1);
    }

    function getReward(address userAddress, uint256 campaignId)
        public
        view
        returns (
            uint256 zpAmount,
    ...
    {
        require(whiteList[userAddress] > 0, "not in whiteList");
        require(
            campaigns[campaignId].startTime <= block.timestamp,
            "campaign not start"
        );
        ...
        require(!claimed, "already claimed");
        zpAmount = campaigns[campaignId]
            .totalPoolZP
            .mul(ranks[whiteList[userAddress]])
            .div(PERCENTS_DIVIDER);
        gpAmount = campaigns[campaignId]
            .totalPoolGP
            .mul(ranks[whiteList[userAddress]])
            .div(PERCENTS_DIVIDER);
        zkAmount = campaigns[campaignId]
            .totalPoolZK
            .mul(ranks[whiteList[userAddress]])
            .div(PERCENTS_DIVIDER);
    }
