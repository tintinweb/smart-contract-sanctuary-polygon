...
contract FeeCollectHook {
        ...
        onlyHookManager
    {
        ...
        if (collectControl.feeToken == address(0)) {
            require(
                msg.value >= collectControl.collectFee,
                "CollectFee insufficient"
            );
            _transfer(
                collectControl.feeToken,
                collectControl.feeReceiver,
                collectControl.collectFee
            );
            uint256 refund = msg.value - collectControl.collectFee;
            if (refund > 0) {
                _transfer(collectControl.feeToken, collector, refund);
            }
        } else {
            IERC20(collectControl.feeToken).safeTransferFrom(
                collector,
                collectControl.feeReceiver,
                collectControl.collectFee
            );
