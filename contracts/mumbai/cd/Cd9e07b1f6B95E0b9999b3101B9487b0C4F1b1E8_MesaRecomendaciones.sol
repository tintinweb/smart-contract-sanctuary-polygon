...
contract MesaRecomendaciones {
    ...
    function verRecomendacion(uint256 recomendacionID) public view returns(string memory, string memory, string memory, uint) {
        Recomendacion memory rec = recomendaciones[recomendacionID];
        return(rec.recomendacion, rec.activo, rec.cliente, rec.fecha);
    }
}