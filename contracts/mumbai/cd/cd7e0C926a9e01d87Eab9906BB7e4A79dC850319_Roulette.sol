  ...
  function spinWheel(uint256 betId, bool isEth, address tokenAddr) private  {
    ...
    for(uint i = 0; i < bet.betType.length; i++){
      ...
      if(won){
          ...
          if (isEth) {
              withdrawEth(msg.sender, bet.winningAmount);
          }
          else if (!isEth) {
              withdrawToken(msg.sender, tokenAddr, bet.winningAmount);
          }          
      }
    }
  ...
  */
  function spinWheelForRoom(uint256 roomId) external onlyAdmin {
    ...
    for(uint j = 0; j < participants[roomId].length; j++) {
      ...
      if (isEther[roomId]) {
          withdrawEth(b.bettorAddress, b.winningAmount);
      }
      else if (!isEther[roomId]) {
        withdrawToken(b.bettorAddress, tokenAddress[roomId], b.winningAmount);
      }    
      }
     } 
