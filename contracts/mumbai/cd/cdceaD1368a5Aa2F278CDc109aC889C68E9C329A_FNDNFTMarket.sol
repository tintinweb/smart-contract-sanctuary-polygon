    ...
    NFTMarketReserveAuction
{
    ...
    function adminUpdateConfig(
        ...
        uint256 primaryFanFeeBasisPoints,
        uint256 secondaryFanFeeBasisPoints,
        uint256 ownersFeeBasicPoints
        ...
        _updateMarketFees(
            ...
            primaryFanFeeBasisPoints,
            secondaryFanFeeBasisPoints,
            ownersFeeBasicPoints
        );
    }
    ...
    NFTMarketAuction
{
    ...
    mapping(address => mapping(uint256 => mapping( uint256 => ReserveAuction))) public bidderInfoToAuctionId;
    mapping(address => mapping(uint256 => uint256)) private nftContractToTokenIdToAuctionId;
    mapping(uint256 => mapping(uint256 => address)) public biddersListToAuctionId;
    mapping(address => mapping(uint256 => ReserveAuction)) public bannedAuction;
    mapping(address => mapping(uint256 => bool)) public bannedTokens;
    mapping(uint256 => ReserveAuction) private auctionIdToAuction;
    mapping(uint256 => uint256) public bidderBuyerPoolCounter;
    ...
    modifier onlyValidAuctionConfig(uint256 reservePrice) {
        require(reservePrice >= 10000000000000, "NFTMarketReserveAuction: Reserve price must be at least 1 wei");
        _;
    }
     ...
     */
    function _getSellerFor(address nftContract, uint256 tokenId) internal view virtual override returns (address payable) {
    ...
    {   
        ...
        auctionIdToAuction[auctionId] = ReserveAuction(
            ...
            reservePrice,
            BidderInfo(payable(address(0)), 0, 0)
        );
    ...
    function placeBid(uint256 auctionId) public payable nonReentrant {
        ...
        require(bidderBuyerPoolCounter[auctionId] < 200,"To many bids");
        ...
        if (auction.endTime == 0) {
            ...
            biddersListToAuctionId[bidderBuyerPoolCounter[auctionId]][auctionId] = auction.bidder;
            bidderInfoToAuctionId[auction.bidder][auctionId][bidderBuyerPoolCounter[auctionId]].bidderInfo.bidderAddress = auction.bidder;
            bidderInfoToAuctionId[auction.bidder][auctionId][bidderBuyerPoolCounter[auctionId]].bidderInfo.bidAmount = msg.value - auction.amount;

            auction.amount = msg.value;
             auction.bidderInfo.bidderAddress = auction.bidder;                                                    
             auction.bidderInfo.bidAmount = bidderInfoToAuctionId[auction.bidder][auctionId][bidderBuyerPoolCounter[auctionId]].bidderInfo.bidAmount;

            // On the first bid, the endTime is now + duration
            bidderBuyerPoolCounter[auctionId]++;
    ...
    function finalizeReserveAuction(uint256 auctionId) public nonReentrant { 
        ReserveAuction memory auction = auctionIdToAuction[auctionId];
        require(auction.endTime > 0,"NFTMarketReserveAuction: Auction was already settled or  was not started");
        require(auction.endTime < block.timestamp,"NFTMarketReserveAuction: Auction still in progress");

        uint256 counter = bidderBuyerPoolCounter[auctionId];
        BidderInfo[] memory bidderInformation = new BidderInfo[](counter); 
        (,uint256 primaryFanFeeBasisPoints, uint256 secondaryFanfeePoints,) = getFeeConfig();

        for (uint256 i = 0; i < counter; i++) {
            address bidders = biddersListToAuctionId[i][auctionId];

            bidderInformation[i].bidderAddress = bidderInfoToAuctionId[bidders][auctionId][i].bidderInfo.bidderAddress;
            bidderInformation[i].bidAmount = bidderInfoToAuctionId[bidders][auctionId][i].bidderInfo.bidAmount;

            if (getIsPrimary(auction.nftContract, auction.tokenId)) {
                //calculate fee amount primary sale
                bidderInformation[i].feeAmount =((bidderInfoToAuctionId[bidders][auctionId][i].bidderInfo.bidAmount * 100000000000) / auction.amount)
                * (((auction.amount * primaryFanFeeBasisPoints) /BASIS_POINTS) / 100000000000); //primary primaryFanFeeBasisPoints = 5%
            } else {
                //calculate fee amount secondary sale
                bidderInformation[i].feeAmount = ((bidderInfoToAuctionId[bidders][auctionId][i].bidderInfo.bidAmount * 100000000000) / auction.amount)
                * (((auction.amount * secondaryFanfeePoints) / BASIS_POINTS) / 100000000000);  //primary secondaryFanfeePoints = 1%
            }
        }

        ownersPoolCounter[auction.nftContract][auction.tokenId]++;
        ownersListToAuctionId[auction.nftContract][auction.tokenId][ownersPoolCounter[auction.nftContract][auction.tokenId]] = auction.bidder; //add owner to Owners Pool 
        ...
        GetFees memory FeesInfo = _distributeFunds(
            auction.nftContract,
            auction.tokenId,
            auction.seller,
            auction.amount,
        ...
        emit ReserveAuctionFinalized(
            ...
            auction.seller,
            auction.bidder,
            FeesInfo.foundationFee,
            FeesInfo.creatorSecondaryFee,
            FeesInfo.ownerRev,
            bidderInformation
        );
    }
    ...
    function adminUnbanToken(address nftContract, uint256 tokenId)public onlyFoundationAdmin{
    ...
    SendValueWithFallbackWithdraw
{
    ...
    uint256 private _primaryFanFeeBasisPoints;
    uint256 private _secondaryFanFeeBasisPoints;
    uint256 private _ownersFeeBasicPoints;
    ...
    mapping(address => mapping(uint256 => mapping(uint256 => address payable))) public ownersListToAuctionId; 
    mapping(address => mapping(uint256 => uint256)) public ownersPoolCounter;
        ...
        )
    {
        return (       
            ...
            _primaryFanFeeBasisPoints,
            _secondaryFanFeeBasisPoints,
            _ownersFeeBasicPoints
        );
    }
     ...
     */
    function getFees( address nftContract, uint256 tokenId, uint256 price ) public view returns (GetFees memory FeesInfo) {
        (FeesInfo, , ) = _getFees( nftContract, tokenId, _getSellerFor(nftContract, tokenId), price);
    }
    ...
    function _getFees( address nftContract, uint256 tokenId, address payable seller, uint256 price) private view returns (
            GetFees memory FeesInfo,
            address payable creatorSecondaryFeeTo,
            address payable ownerRevTo
        )
    {
        // The tokenCreatorPaymentAddress replaces the creator as the fee recipient.
        (address payable creator, address payable tokenCreatorPaymentAddress ) = _getCreatorAndPaymentAddress(nftContract, tokenId);

        // uint256 foundationFeeBasisPoints;
        uint256 FanFeeBasisPoints;
        uint256 ownersFeeBasicPoints;
        ...
        if (_getIsPrimary(nftContract, tokenId, creator, seller)) {
            ...
            FanFeeBasisPoints = _primaryFanFeeBasisPoints; 

            // On a primary sale, the previous owner == 0.
            ownersFeeBasicPoints = 0;
        } else {
           
            FanFeeBasisPoints = _secondaryFanFeeBasisPoints;
            ownersFeeBasicPoints = _ownersFeeBasicPoints;
            ...
            if (tokenCreatorPaymentAddress != address(0)) {
                ...
                FeesInfo.creatorSecondaryFee =price.mul(_secondaryCreatorFeeBasisPoints) /BASIS_POINTS;
        ...
        // SafeMath is not required when dividing by a constant value > 0.
        FeesInfo.fanFee = price.mul(FanFeeBasisPoints) / BASIS_POINTS;
        FeesInfo.ownersFee = price.mul(ownersFeeBasicPoints) / BASIS_POINTS;
        FeesInfo.ownerRev = price.sub(FeesInfo.creatorSecondaryFee).sub(FeesInfo.fanFee).sub(FeesInfo.ownersFee);
    }
    ...
    function _distributeFunds(
        ...
        address payable seller,
        uint256 price,
        BidderInfo[] memory bidderInformation,
        uint256 counter
    ) public returns (GetFees memory FeesInfo) {
       
        address payable creatorFeeTo;
        address payable ownerRevTo;
        (FeesInfo, creatorFeeTo, ownerRevTo) = _getFees(nftContract, tokenId, seller, price);
        ...
        for (uint256 i = 0; i < ownersPoolCounter[nftContract][tokenId]; i++) { // send fee to every Owners of token after primary sale
            _sendValueWithFallbackWithdrawWithMediumGasLimit(
                ownersListToAuctionId[nftContract][tokenId][i],
                (FeesInfo.ownersFee.div(ownersPoolCounter[nftContract][tokenId]))
            );
        }

        for (uint256 i = 0; i < counter; i++) { // send fee to every fan that made bit
            _sendValueWithFallbackWithdrawWithMediumGasLimit(
                bidderInformation[i].bidderAddress,
                bidderInformation[i].feeAmount
            );
        }
        _sendValueWithFallbackWithdrawWithMediumGasLimit(
            creatorFeeTo,
     ...
     */
     function _updateCreatorFees( uint256 secondaryCreatorFeeBasisPoints) internal{
        require(
            secondaryCreatorFeeBasisPoints <= 1500,
            "NFTMarketFees: Creator Fees > 15%"
        );
    ...
    function _updateMarketFees(
        ...
        require(
            ownersFeeBasicPoints.add(secondaryFanFeeBasisPoints.add(primaryFanFeeBasisPoints.add(secondaryCreatorFeeBasisPoints))) < BASIS_POINTS,
            "NFTMarketFees: Fees > 100%"
        );

        _secondaryCreatorFeeBasisPoints = secondaryCreatorFeeBasisPoints;
        _primaryFanFeeBasisPoints = primaryFanFeeBasisPoints;
        _secondaryFanFeeBasisPoints = secondaryFanFeeBasisPoints;
        _ownersFeeBasicPoints = ownersFeeBasicPoints;
