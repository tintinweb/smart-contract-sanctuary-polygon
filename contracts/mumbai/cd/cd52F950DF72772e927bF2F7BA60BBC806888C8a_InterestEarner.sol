...
contract InterestEarner {
    ...
    function unstakeAllTokensAndWithdrawInterestEarned(IERC20 token)
        public
        timePeriodIsSet
        percentageIsSet
        noReentrant
    {
        ...
        uint256 hundredPercent = initialStakingTimestamp[msg.sender].add(
            timePeriod
        );
        emit hundredPercentTime(hundredPercent);
        uint initialTimeStamp = initialStakingTimestamp[msg.sender];
        ...
        // }
         if (calcTime < _OneWeakTime) {
          ...
          emit intrestValue(interestToPayOut);
           ...
           interestToPayOut = expectedInterest[msg.sender].mul(3).div(4);
