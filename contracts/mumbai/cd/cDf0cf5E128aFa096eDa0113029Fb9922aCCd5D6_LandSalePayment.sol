...
contract LandSalePayment is AccessControl, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        uint256[] memory _maxCategorySupply,
        uint256[3][13] memory _slot,
        uint256[5][13] memory _slotSupply
    ) {
        ...
        for (uint256 index = 0; index < _category.length; index++) {
            addNewLandCategory(
                _category[index],
                _slotDependency[index],
                _priceInUSD[index],
                _maxCategorySupply[index]
            );
        }
    ...
    function buyLandWithTVK(
        ...
        require(
            TVK.allowance(msg.sender, address(this)) >=
                getlandpriceInTVK(_category),
    ...
    function increaseLandCategorySupply(
        string memory _category,
        ...
        landCategory[_category].maxCategorySupply = landCategory[_category]
            .maxCategorySupply
            .add(_additionalSupply);
        maxSupply = maxSupply.add(_additionalSupply);

        for (uint256 index = 1; index <= slotCount; index++) {
            slot[index].slotSupply[_category].maxSlotCategorySupply = slot[
                index
            ].slotSupply[_category].maxSlotCategorySupply.add(
                    _additionalSupply
                );
        }
    ...
    function decreaseLandCategorySupply(
        string memory _category,
        uint256 _additionalSupply
    ) public onlyOwner {
        require(landCategory[_category].status, "Landsale: Invalid caetgory!");
        require(_additionalSupply > 0, "Landsale: Invalid value!");
        require(
            landCategory[_category].maxCategorySupply.sub(
                landCategory[_category].mintedCategorySupply
            ) >= _additionalSupply,
            "Landsale: Decreasing supply impacts the minted supply!"
        );

        landCategory[_category].maxCategorySupply = landCategory[_category]
            .maxCategorySupply
            .sub(_additionalSupply);
        maxSupply = maxSupply.sub(_additionalSupply);

        for (uint256 index = 1; index <= slotCount; index++) {
            slot[index].slotSupply[_category].maxSlotCategorySupply = slot[
                index
            ].slotSupply[_category].maxSlotCategorySupply.sub(
                    _additionalSupply
                );
        }
    ...
    function updateTVKInUSDPrice(uint256 _TVKinUSDprice) public {
        require(
            hasRole(admin_role, _msgSender()),
            "Landsale: Must have admin role to mint."
        );
    ...
    function updateETHInUSDPrice(uint256 _ETHinUSDprice) public {
        require(
            hasRole(admin_role, _msgSender()),
            "Landsale: Must have admin role to mint."
        );
    ...
    {
        _price = (landCategory[_category].priceInUSD.mul(TVKinUSDprice)).div(
            1 ether
        );
    }
    ...
    function getlandPriceInETH(string memory _category)
        ...
        returns (uint256 _price)
    {
        _price = (landCategory[_category].priceInUSD.mul(ETHinUSDprice)).div(
            1 ether
        );
    }
