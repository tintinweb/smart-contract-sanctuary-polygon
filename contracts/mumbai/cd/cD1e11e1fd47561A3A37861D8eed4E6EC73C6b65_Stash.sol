...
interface ICoGResources {
    ...
    function resourcePrices(uint8) external view returns (int256);
    ...
    OwnableUpgradeable
{
    ...
    mapping (address => EnumerableMapUpgradeable.UintToUintMap) private resources;
    mapping (address => EnumerableMapUpgradeable.UintToUintMap) private enriches;
    ...
    function buyEnrich(uint256[] memory ids, uint256[] memory amounts) external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            enrichCapsule.mint(account, ids[i], amounts[i]);
        }
    ...
    function depositEnrich(uint256[] memory enrichId, uint256[] memory amount) external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < enrichId.length; i++) {
            if (
                ...
                (, uint256 current) = enriches[account].tryGet(enrichId[i]);
                enriches[account].set(enrichId[i], current + amount[i]);
                enrichCapsule.burn(account, enrichId[i], amount[i]);
            }
        }
    }

    function withdrawEnrich(uint256[] memory enrichId, uint256[] memory amount) external nonReentrant whenNotPaused {
        require(enrichId.length == amount.length, "invalid length");
        uint256 maxId = enrichCapsule.maxId();
        address account = msg.sender;
        for (uint256 i = 0; i < enrichId.length; i++) {
            ...
            if (
                ...
                if (current == amount[i]) {
                    enriches[account].remove(enrichId[i]);
                } else {
                    enriches[account].set(enrichId[i], current - amount[i]);
                }
                enrichCapsule.mint(account, enrichId[i], amount[i]);
            }
        }
    }
    ...
    function depositResource(uint256[] memory resourceId, uint256[] memory amount) external nonReentrant whenNotPaused {
        ...
        for (uint256 i = 0; i < resourceId.length; i++) {
            if (
                ...
                (, uint256 current) = resources[account].tryGet(resourceId[i]);
                resources[account].set(resourceId[i], current + amount[i]);
                resource.burn(account, resourceId[i], amount[i]);
            }
        }
    }

    function withdrawResource(uint256[] memory resourceId, uint256[] memory amount) external nonReentrant whenNotPaused {
        require(resourceId.length == amount.length, "invalid length");
        address account = msg.sender;
        for (uint256 i = 0; i < resourceId.length; i++) {
            (, uint256 current) = resources[account].tryGet(resourceId[i]);
            if (
                ...
                if (current == amount[i]) {
                    resources[account].remove(resourceId[i]);
                } else {
                    resources[account].set(resourceId[i], current - amount[i]);
                }
                resource.mint(account, resourceId[i], amount[i]);
            }
        }
    }

    function importEnrichToLand(uint256 landId, uint256 enrichId) external nonReentrant whenNotPaused {
        address account = msg.sender;
        (, uint256 current) = enriches[account].tryGet(enrichId);
        (address land, address game) = manager.getAddress();
        require(
            (account == ILand(land).ownerOf(landId) || IGame(game).ownerOfLand(account, landId)) && 
            current > 0, 
            "invalid"
        );
        uint256 newRichness = calEnrichAmount(landId, enrichId);
        manager.enrich(landId, newRichness);
        if (current == 1) {
            enriches[account].remove(enrichId);
        } else {
            enriches[account].set(enrichId, current - 1);
        }    
    }

    function importResourceToLand(uint256 landId, uint256[] memory resourceIds) external nonReentrant whenNotPaused {
        address account = msg.sender;
        (, address game) = manager.getAddress();
        require (IGame(game).ownerOfLand(account, landId), "caller is not owner");
        require(
            manager.landInfos(landId).resourceDeposit + resourceIds.length <= manager.getLimit(landId).resourceDepositLimit,
            "exceed limit"
        );
        uint256 count;
        for (uint256 i = 0; i < resourceIds.length; i++) {
            (, uint256 current) = resources[account].tryGet(resourceIds[i]);
            if (current > 0) {
                address cog = IGame(game).cogResources();
                int256 payout = ICoGResources(cog).resourcesInitPayout(uint8(resourceIds[i]));
                manager.addResource(landId, resourceIds[i], 1, payout, 0);
                if (current == 1) {
                    resources[account].remove(resourceIds[i]);
                } else {
                    resources[account].set(resourceIds[i], current - 1);
                }
                count += 1;
            }
        }
        manager.updateResourceDeposit(landId, count);
    }

    function calEnrichAmount(uint256 landId, uint256 enrichId) public view returns (uint256) {
        ILandManager.LandInfo memory info = manager.landInfos(landId);
        uint256 max = manager.richnesses(info.rarityType, info.floor);
        uint256 raw_incre = max * enrichCapsule.values(enrichId);
        uint256 incre = raw_incre >= 50 + (raw_incre / 100) * 100 ? 
            1 + raw_incre / 100 :
            raw_incre / 100;
        return info.richness + incre > max ? max : info.richness + incre;
    }
    ...
    function getResources(address account) external view returns (uint256[] memory sym, uint256[] memory qt) {
        sym = resources[account].keys();
        qt = new uint256[](sym.length);
        for (uint256 i = 0; i < sym.length; i++) {
            (, uint256 quantity) = resources[account].tryGet(sym[i]);
            qt[i] = quantity;
        }
    }

    function getEnriches(address account) external view returns (uint256[] memory sym, uint256[] memory qt) {
        sym = enriches[account].keys();
        qt = new uint256[](sym.length);
        for (uint256 i = 0; i < sym.length; i++) {
            (, uint256 quantity) = enriches[account].tryGet(sym[i]);
            qt[i] = quantity;
        }
    }
