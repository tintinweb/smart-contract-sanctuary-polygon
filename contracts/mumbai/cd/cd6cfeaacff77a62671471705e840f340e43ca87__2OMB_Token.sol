...
contract _2OMB_Token is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 20000 ether;
    // Initial distribution for the day 2-5 2OMB-WETH LP -> 2OMB pool
    uint256 public constant INITIAL_2OMB_POOL_DISTRIBUTION = 140000 ether;
    ...
    /**
     * @notice Constructs the 2OMB ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("2omb Token", "2OMB") {
        // Mints 1 2OMB to contract creator for initial pool setup
    ...
    function _get2ombPrice() internal view returns (uint256 _2ombPrice) {
        try IOracle(two_ombOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("2omb: failed to fetch 2OMB price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _2ombPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_2ombPrice >= taxTiersTwaps[tierId]) {
    ...
    function set2ombOracle(address _2ombOracle) public onlyOperatorOrTaxOffice {
        require(_2ombOracle != address(0), "oracle address cannot be 0 address");
        two_ombOracle = _2ombOracle;
    }
    ...
    /**
     * @notice Operator mints 2OMB to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of 2OMB to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 current2ombPrice = _get2ombPrice();
            currentTaxRate = _updateTaxRate(current2ombPrice);
            if (current2ombPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_2ombPool != address(0), "!_2ombPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_2ombPool, INITIAL_2OMB_POOL_DISTRIBUTION);
    }
