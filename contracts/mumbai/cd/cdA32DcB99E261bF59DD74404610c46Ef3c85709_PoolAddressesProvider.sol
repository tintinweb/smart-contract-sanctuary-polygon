...
contract PoolAddressesProvider is IPoolAddressesProvider {
  ...
  bytes32 private constant PRXY_TREASURY = 'PRXY_TREASURY';
  bytes32 private constant TRADING_WALLET = 'TRADING_WALLET';
  bytes32 private constant DAO_WALLET = 'DAO_WALLET';
  ...
  /// @inheritdoc IPoolAddressesProvider
  function getPrxyTreasury() external view override returns (address) {
    return getAddress(PRXY_TREASURY);
  }
  

  /// @inheritdoc IPoolAddressesProvider
  function setPrxyTreasury(address newPrxyTreasury) external override onlyOwner {
    address oldPrxyTreasury = _addresses[PRXY_TREASURY];
    _addresses[PRXY_TREASURY] = newPrxyTreasury;
    emit PrxyTreasuryUpdated(oldPrxyTreasury, newPrxyTreasury);
  }

  /// @inheritdoc IPoolAddressesProvider
  function getTradingWallet() external view override returns (address) {
    return getAddress(TRADING_WALLET);
  }
  

  /// @inheritdoc IPoolAddressesProvider
  function setTradingWallet(address newTradingWallet) external override onlyOwner {
    address oldTradingWallet = _addresses[TRADING_WALLET];
    _addresses[TRADING_WALLET] = newTradingWallet;
    emit TradingWalletUpdated(oldTradingWallet, newTradingWallet);
  }

  /// @inheritdoc IPoolAddressesProvider
  function getDaoWallet() external view override returns (address) {
    return getAddress(DAO_WALLET);
  }
  

  /// @inheritdoc IPoolAddressesProvider
  function setDaoWallet(address newDaoWallet) external override onlyOwner {
    address oldDaoWallet = _addresses[DAO_WALLET];
    _addresses[DAO_WALLET] = newDaoWallet;
    emit TradingWalletUpdated(oldDaoWallet, newDaoWallet);
  }
