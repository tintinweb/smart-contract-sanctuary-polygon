  ...
  PausableUpgradeable
{
  ...
  modifier onSelling(uint256 _nftId) {
    require(selling[_nftId].from == OVRLand.ownerOf(_nftId), "Not for sale");
    _;
  }

  modifier onSellingContainer(uint256 _containerId) {
    require(
      sellingContainer[_containerId].from == OVRContainer.ownerOf(_containerId),
      "Not for sale"
    );
    _;
  }
   ...
   * @return _offer Offer
   */
  function lastOffer(uint256 _nftId) public view returns (Offer memory _offer) {
    ...
    onSellingContainer(_containerId)
  {
    ...
    emit ContainerBought(_containerId, value, _msgSender(), currentTimestamp);
  }
    ...
    isContainerOwner(_containerId)
  {
    ...
    containerOnSelling[_containerId] = false;
    delete sellingContainer[_containerId];
    emit SellContainerCancelled(_containerId, _msgSender(), currentTimestamp);
  }
