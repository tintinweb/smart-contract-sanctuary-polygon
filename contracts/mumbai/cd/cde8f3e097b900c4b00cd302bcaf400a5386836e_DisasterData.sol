...
contract DisasterData {
    ...
    function getAccumulatedSeverity(string memory district) public view returns (uint) {
     ...
     string[] dists = ["Ahmednagar", "Akola", "Amravati", "Aurangabad", "Beed", "Bhandara", "Buldhana", "Chandrapur", "Dhule", "Gadchiroli", "Gondia", "Hingoli", "Jalgaon", "Jalna", "Kolhapur", "Latur", "Mumbai City", "Mumbai Suburban", "Nagpur", "Nanded", "Nandurbar", "Nashik", "Osmanabad", "Palghar", "Parbhani", "Pune", "Raigad", "Ratnagiri", "Sangli", "Satara", "Sindhudurg", "Solapur", "Thane", "Wardha", "Washim", "Yavatmal"];

    function cumulSeverity() public view returns(uint){
        uint allsevsum;
        for(uint i=0;i<36;i++){
            if(getAccumulatedSeverity(dists[i])>=0&&getAccumulatedSeverity(dists[i])<100){
                allsevsum+=getAccumulatedSeverity(dists[i]);
            }
        }
