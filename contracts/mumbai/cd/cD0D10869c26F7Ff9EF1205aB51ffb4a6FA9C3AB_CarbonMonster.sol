...
contract CarbonMonster is Ownable, ERC721Enumerable {
    ...
    function getParams(uint256 tokenId) public view returns (
            string memory svg,
            bool isAlive,
            uint256 life,
            uint256 age,
    ...
    function _getSvg(uint256 tokenId) private view returns (string memory) {
        Monster storage m = monsters[tokenId];
        return m.isAlive() ? _getAliveSvg(_totalFeededAmount[tokenId] / 10**18) : _getDeadSvg(tokenId, m.getAge());
    }

    function _getFlower(uint256 tokenId) private view returns (string memory) {
        uint256 rand = uint256(keccak256(abi.encodePacked(block.timestamp, tokenId))) % 4;
        return [unicode"💐", unicode"🌼", unicode"🌻", unicode"🌹"][rand];
    }

    function _lerpWithClamp(
        int256 x0,
        int256 y0,
        int256 x1,
        int256 y1,
        ...
        int256 y = y0 + ((y1 - y0) * (x - x0)) / (x1 - x0);
        if (y > y1) return y1;
        if (y < y0) return y0;
        return y;
    }

    function _getDeadSvg(
        uint256 tokenId,
        uint256 ageAtDeath
    ) private view returns (string memory) {
        uint256 scaleX10 = uint256(_lerpWithClamp(100, 10, 720, 24, int256(ageAtDeath))); // {age:100,scale:10} to {age:720,scale:24}
        bytes memory scale = abi.encodePacked((scaleX10 / 10).toString(), ".", (scaleX10 % 10).toString());
        string memory flower = _getFlower(tokenId);
        return
            string(
                abi.encodePacked(
                    abi.encodePacked(
                        '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="-200 -200 400 400" style="overflow:visible">',
                        '<style>text{stroke:none;text-anchor:middle;dominant-baseline:central}</style><path fill="#444" stroke="#000" d="M-200-200h400v400h-400z"/>',
                        '<g transform="scale(',
                        scale,
                        ')"><path d="M-60-60Q0-100,60-60V80H-60Z" fill="#ccc" stroke="#000" stroke-width="3"/><text x="0" y="-50" font-size="12">REST IN PEACE</text></g>'
                    ),
                    abi.encodePacked(
                        '<text y="10" font-size="20">AGED</text><text y="30" font-size="20">',
                        ageAtDeath.toString(),
                        '</text><text y="50" font-size="20">HOURS</text><text y="120" font-size="50">',
                        flower,
                        '</text></svg>'
                    )
                )
    ...
    function _getAliveSvg(uint256 totalFeededAmount) private pure returns (string memory) {
        return string(
            abi.encodePacked(
                ...
                totalFeededAmount.toString(),
                '"/>',
                '</svg>'
            )
        );
        // uint256 size = uint256(_lerpWithClamp(0, 30, 720, 330, int256(age))); // {age:0,size:30} to {age:720,size:330}
        // uint256 lifeGaugeWidth = uint256(_lerpWithClamp(0, 0, 100, 320, int256(life))); // {life:0,width:0} to {life:100,width:320}
        // string memory lifeGaugeColor = MonsterLib.isLifeLessThanHalf(life) ? "#ff0" : "#0f0";
        // return
        //     string(
        //         abi.encodePacked(
        //             abi.encodePacked(
        //                 '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="-200 -200 400 400">',
        //                 '<style>text{paint-order:stroke;stroke:wheat;stroke-width:3px;text-anchor:middle;dominant-baseline:central}</style><path fill="wheat" stroke="#000" d="M-200-200h400v400h-400z"/>',
        //                 '<clipPath id="r"><path d="M-200-200h400v400h-400z" /></clipPath>',
        //                 '<g clip-path="url(#r)"><text font-size="',
        //                 size.toString(),
        //                 unicode'">👻</text></g><text y="-160" font-size="30" font-family="fantasy">Carbon Monster'
        //             ),
        //             abi.encodePacked(
        //                 '</text><text y="-120" font-size="20">',
        //                 age.toString(),
        //                 ' hours old</text><path fill="red" stroke="#000" d="M-160 120h320v20h-320z"/><path fill="',
        //                 lifeGaugeColor,
