...
/**
  ...
  */
  function placeOfferOnSellOrder(
    ...
    uint256 offerValue = _value;

    require(offerValue > 0, "offer value must grater than 0");

    (, uint256 marketplaceFeeValue,)  = _getBuyerFeeWithDiscount(offerValue, sellOrder.collectionId, _coupon, _discount);
    
    offerValue  = _value - marketplaceFeeValue;

   
    // If the sell order is not open to offers without an ask price
    if(sellOrder.ask != 0) {
      // Ensure the offer value is less than the order ask
      require(offerValue < sellOrder.ask, "offer value must be less than order ask");
    }
    if(sellOrder.ask == 0 && sellOrder.offerMinPrice > 0){
      require(offerValue >= sellOrder.offerMinPrice, "offer value must be higher than offer min price");
    }

    // Ensure the value of the new offer is higher than the current
    require(offerValue > sellOrder.highestOffer, "offer value must be higher than current");
    ...
    sellOrder.highestOffer = offerValue;
  ...
  */
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    require(success, "Transfer to sell order owner failed.");
     _transferFees(marketplaceFeeValue, collectionAddress, partnerDaoAddress);
