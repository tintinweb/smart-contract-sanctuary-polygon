...
contract BavePayV1Factory is IBavePayV1Factory {
    ...
    modifier onlyPayment(uint256 paymentId) {
    ...
    function createPayment(
        ...
        uint256 rate
    ) external onlyBaveOper returns (address payment) {
        uint256 paymentId = allPayments.length;
        payment = address(
            new BavePayV1Payment(
                clientAcct,
                clientOper,
                tokens,
                rate,
        ...
        emit PaymentCreated(paymentId, payment);
        emit ClientChanged(paymentId, 0, address(0), clientAcct);
        emit ClientChanged(paymentId, 1, address(0), clientOper);
        for (uint256 i = 0; i < tokens.length; i++) {
            emit TokenChanged(paymentId, tokens[i], true);
        }
    }

    function setBaveAcct(address _baveAcct) external onlyBaveOper {
        require(_baveAcct != address(0));
        emit BaveChanged(0, baveAcct, _baveAcct);
        baveAcct = _baveAcct;
    }

    function setBaveOper(address _baveOper) external onlyBaveOper {
    ...
    function ledger(
        ...
        address to,
        uint256 amount
    ) external onlyPayment(paymentId) {
    ...
    function tokenChanged(
        ...
        emit TokenChanged(paymentId, token, boolean);
    }
}
...
contract BavePayV1Payment is IBavePayV1Payment {
    ...
    function paymentToken(
        ...
        _safeTransferFrom(tokenAddr, address(this), baveAcct(), commission);
    }
