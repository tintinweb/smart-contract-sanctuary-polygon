...
// File: contracts/nfc/FwenNFC.sol
...
contract FwenNFC is ERC721, IERC2981, Ownable {
    // ======== Royalties ==========
    ...
    mapping(uint256 => bytes32) public nfc;
    // mapping(uint256 => uint256) public price;
    // mapping(uint256 => bytes32) public remark;

    event NFCUpdated(uint256 indexed tokenId, bytes32 indexed nfc);

    /**
     * @dev Initializes the contract by setting royalty info.
     */
    constructor(string memory name_, string memory symbol_, address royaltyAddress, uint256 royaltyPercent) ERC721(name_, symbol_)  {
        _royaltyAddress = royaltyAddress;
        _royaltyPercent = royaltyPercent;
    }

    modifier onlyOperator() {
        if(!operators[msg.sender]) { revert NonOperator(); }
        _;
    }
    ...
    function updateOperator(address operator, bool valid) external virtual onlyOwner {
        operators[operator] = valid;
    }

    function updateNFC(uint256 tokenId, bytes32 nfc_) external onlyOperator {
        if (!_exists(tokenId)) { revert NonExistentToken(); }
        nfc[tokenId] = nfc_;
        emit NFCUpdated(tokenId, nfc_);
    }
     ...
     */
    function batchMint(address[] memory tos, bytes32[] memory nfcs) external onlyOperator {
