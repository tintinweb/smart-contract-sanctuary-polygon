...
contract UniswapV2 {
    ...
    constructor(string memory name, address factory, address router) {
        _factory = IUniswapV2Factory(factory);
        _router = IUniswapV2Router(router);
        _name = name;

        _baseTokens = [
            0x4B703C9b462b11bcA2dF855efA5920e739454FB9,
    ...
    {
        (reserve0, reserve1, blockTimestampLast) = IUniswapV2Pair(getPair(token0, token1)).getReserves();
    }
    ...
    function swap(
        ...
        if (inputToken != address(0) && outputToken != address(0)) {
            if (amountIn > 0) {
                ...
                require(amounts[0] <= slippageAmount, "UniswapV2Router-: EXCESSIVE_INPUT_AMOUNT");
            ...
            if (inputToken == address(0) && amountIn > 0) {
            ...
            } else if (inputToken == address(0) && amountIn == 0) {
                ...
                if (address(this).balance > 0) { // refund dust eth, if any
                    IERC20(address(0)).uniTransfer(payable(msg.sender), address(this).balance);
                }

            } else if (outputToken == address(0) && amountIn > 0) {
    ...
    function _transferIn(address inputToken, uint256 amountIn) internal {
        ...
        require(IERC20(inputToken).approve(address(_router), amountIn), "approve failed.");
    }

    receive() external payable {}
}

contract UniswapV2Aggergator {

    string[] public _names;
    mapping (string => UniswapV2) _routers;

    constructor() {

    }

    function createRouter(string memory name, address factory, address router) external {
        require(address(_routers[name]) == address(0), "name existed");
        _names.push(name);
        _routers[name] = new UniswapV2(name, factory, router);
    }

    function getBestPath(address token0, address token1, uint256 amountIn, uint256 amountOut) 
        external
        view
        returns (string memory bestName, address[] memory bestPath, uint256 bestResult)
    {

        for (uint256 i; i < _names.length; i++) {
            (address[] memory path, uint256 result) = _routers[_names[i]].getBestPath(token0, token1, amountIn, amountOut);
            if (result == 0) {
                continue;
            }
            if (amountIn > 0) {
