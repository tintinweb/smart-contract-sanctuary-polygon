...
//可暂停采集器铸造合约
contract CollectorFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    //记录有多少种物品
	Counters.Counter private _typeTracker;
    ...
    string internal nftNameCollector = "MetaGlobe Collector NFT";
    
    struct Character {
        uint256 duration;            //耐久度
        uint256 oec_rate;            //源晶采集效率
        uint256 uru_rate;            //钨矿采集效率
        uint256 scan;                //扫描 
    ...
    constructor(address _mgge, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        DOMAIN_SEPARATOR = keccak256(
                abi.encode(
                    ...
                    keccak256(bytes("CollectorFactory")),
                    keccak256(bytes("1")),
                    block.chainid,
                    address(this)
                )
        ...
        //初始化
        //采集器1（耐久增益)
        uint _type_id =  _addItem(130001, 200, 450, "duration");  
        _setPropValues(_type_id, "duration", 100, 200); 
        _setPropValues(_type_id, "oec_rate", 10, 75); 
        _setPropValues(_type_id, "uru_rate", 10, 75); 
        _setPropValues(_type_id, "scan", 30, 50);  
        _setPropValues(_type_id, "potential", 1, 5);  

        //采集器2（源晶增益)
        _type_id =  _addItem(130002, 150, 420, "oec_rate");  
        _setPropValues(_type_id, "duration", 80, 100); 
        _setPropValues(_type_id, "oec_rate", 100, 150); 
        _setPropValues(_type_id, "uru_rate", 80, 100); 
        _setPropValues(_type_id, "scan", 30, 50);  
         _setPropValues(_type_id, "potential", 1, 5);  

        //采集器3（钨卢增益)
        _type_id =  _addItem(130003, 100, 400, "uru_rate");  
        _setPropValues(_type_id, "duration", 80, 100); 
        _setPropValues(_type_id, "oec_rate", 80, 100); 
        _setPropValues(_type_id, "uru_rate", 100, 150); 
        _setPropValues(_type_id, "scan", 30, 50); 
        _setPropValues(_type_id, "potential", 1, 5);   
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    function setWeight(uint256 _type_id, uint256 _weight) external nonReentrant onlyOperator returns(bool){
        require(_type_id<=5);
        collectors[_type_id].weight = _weight;
        return true;
    }
    ...
    /**
     * @dev Require _owner to own more than 0 of the token id
     */
    modifier ownersOnly(address _owner, uint256 _id) {
        require(mgge.balanceOf(_owner, _id) > 0, "HARVESTER_ONLY_OWNERS_ALLOWED");
        _;
    }
        ...
        // onlyAuthorizationContract // TODO 上线后需要打开 只允许主合约调用
    {
        ...
        for (uint i = 0; i <= _typeCount; i++) {
             if (collectors[i].count < collectors[i].prop_sum) {
                 _count ++;
             }
        }
        if (_count == 0) {
            for (uint k = 0; k <= _typeCount; k++) {
                collectors[k].count =0;
            }
            _type_id =0;
        ...
        Collector storage collector = collectors[_type_id];
        collector.count +=1;
        uint new_rand;
        ...
        if (settings[_type_id]["oec_rate"].section > 0) {
            new_rand = randomResult.sub(randomResult%100).div(100);
            props["oec_rate"]  = settings[_type_id]["oec_rate"].base + new_rand % settings[_type_id]["oec_rate"].section;
        }
        
        if (settings[_type_id]["uru_rate"].section > 0) {
            new_rand = randomResult.sub(randomResult%1000).div(1000);
            props["uru_rate"]  = settings[_type_id]["uru_rate"].base + new_rand % settings[_type_id]["uru_rate"].section;
        }

        if (settings[_type_id]["scan"].section > 0) {
            new_rand = randomResult.sub(randomResult%10000).div(10000);
            props["scan"]  = settings[_type_id]["scan"].base + new_rand % settings[_type_id]["scan"].section;
        }
        
        //余量
        uint margin = collectors[_type_id].prop_sum - (props["duration"] + props["oec_rate"] + props["uru_rate"] + props["scan"]);
        props[collectors[_type_id].margin_name] = collectors[_type_id].prop_sum - margin;
       ...
       if (settings[_type_id]["potential"].section > 0) {
        ...
        characters[_id] = Character(
                props["duration"],    //耐久度
                props["oec_rate"],    //源晶采集效率
                props["uru_rate"],    //钨矿采集效率
                props["scan"] ,       //扫描 
                potential             //潜力
        );
        ...
        requiresUpgrade(_to, _id, _orderid, signature)
    {
        ...
        //先随机选出哪一个是强化的属性
        uint rand = randomResult % 4; // 0,1,2,3
        ...
        if (rand == 0) {
            ...
            ch.oec_rate += newValue;
        } else if (rand == 2) {
            ch.uru_rate += newValue;
        } else {
            ch.scan += newValue;
        }
        ...
        requiresRevert(_to, _id, _orderid, signature)
    {
        ...
        ch.oec_rate  = revertcharacters[_id].oec_rate;  //源晶采集效率
        ch.uru_rate  = revertcharacters[_id].uru_rate;  //钨矿采集效率
    ...
    returns (Collector memory)
    {
        ...
        for (uint i = 0; i < _typeCount; i++) {
             if (collectors[i].item_id == item_id) {
