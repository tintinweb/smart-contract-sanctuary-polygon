...
contract MetropolitanNFT is Initializable, ERC721Upgradeable, PausableUpgradeable, 
   ...
   /// @custom:oz-upgrades-unsafe-allow constructor
    ...
    function obtenerNFTs(address dueno) public onlyRole(DEFAULT_ADMIN_ROLE) view returns (uint256[] memory) {        
        return NFTsMinteados[dueno]; 
    }

    function liberarNFTs(address liberador, uint256 idNFT) public onlyRole(DEFAULT_ADMIN_ROLE){
        // Validar el address
        require(liberador != address(0), "Metropolitan NFT: Address a mintear es invalido");
        // Validar el id
        require((idNFT > 0) && (idNFT < (maxIdNFT+1)), "Metropolitan NFT: El id esta fuera del rango permitido");
        // Validar dueno
        require(vendidoNFT[idNFT] == liberador, "Metropolitan NFT: El address dueno es otro distinto");

        delete vendidoNFT[idNFT];
        for(uint256 i = 0; i < NFTsMinteados[liberador].length; i++){
            if(NFTsMinteados[liberador][i] == idNFT){
                delete NFTsMinteados[liberador][i];
                break;
            }
        }
