...
contract ApyFlow is ERC20("Apyflow", "APFLW"), Ownable {
    ...
    function recomputePricePerTokenAndHarvestFee() public {
        ...
        if (newPricePerToken > lastPricePerToken) {
            uint256 oldTotalAssets = (_totalSupply * lastPricePerToken) /
                10 ** decimals();
    ...
    function totalAssets() public view returns (uint256 total) {
        for (uint256 i = 0; i < vaults.length(); i++) {
            ...
            total +=
                (vault.convertToAssets(balanceAtVault) * (10 ** decimals())) /
    ...
    function _convertToAssets(
        ...
        return
            ((totalSupply_ == 0) || (totalAssets_ == 0))
                ? shares
                : (shares * totalAssets_) / totalSupply_;
    }
    ...
    function _convertToShares(
        ...
        return
            ((totalSupply_ == 0) || (totalAssets_ == 0))
                ? assets
                : (assets * totalSupply_) / totalAssets_;
    }
    ...
    function deposit(
        ...
        require(allowedTokens[asset], "asset is not allowed");
        recomputePricePerTokenAndHarvestFee();
        ...
        for (uint256 i = 0; i < vaults.length(); i++) {
            SingleAssetVault vault = SingleAssetVault(vaults.at(i));
            uint256 amount = assetConverter.safeSwap(
                asset,
                vault.asset(),
                (assets * vault.totalPortfolioScore()) / totalScore
            );
            if (amount == 0) continue;
            uint256 _deposited = vault.convertToAssets(
                vault.deposit(amount, address(this))
            );
            deposited +=
                (_deposited * (10 ** decimals())) /
                (10 ** vault.decimals());
        }

        shares = _convertToShares(
            deposited,
            totalAssets() - deposited,
            totalSupply()
        );
        _mint(receiver, shares);
    }

    function _redeem(address asset, uint256 shares) internal returns(uint256 assets) {
        for (uint256 i = 0; i < vaults.length(); i++) {
            SingleAssetVault vault = SingleAssetVault(vaults.at(i));
            ...
            assets += assetConverter.safeSwap(vault.asset(), asset, amount);
        }
    }
    ...
    function redeem(
        ...
        require(allowedTokens[asset], "asset is not allowed");
        assets = _redeem(asset, shares);
        _burn(msg.sender, shares);

        uint256 totalSupplyBefore = totalSupply();
        recomputePricePerTokenAndHarvestFee();

        uint256 fee = assets - (totalSupplyBefore * assets) / totalSupply();
        assets -= fee;
    ...
    function computeScoreDeviationInPpm(
    ...
    function rebalance(
        ...
        require(scoreDeviation1 >= 0);
        require(
            (scoreDeviation2 * int256(1000 - allowedDeviationInPpm)) / 1000 <= 0
        );
    }

    function previewRedeemHelper(address asset, uint256 shares) external {
        require(msg.sender == address(this));
        uint256 assets = _redeem(asset, shares);
        assembly {
            let ptr := mload(0x40)
            mstore(ptr, assets)
            revert(ptr, 32)
    ...
    function previewRedeem(address asset, uint256 shares) external returns (uint256 assets) {
        require(allowedTokens[asset], "asset is not allowed");
        try this.previewRedeemHelper(asset, shares) {}
