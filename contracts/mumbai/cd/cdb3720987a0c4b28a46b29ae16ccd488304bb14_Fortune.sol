...
//  onlyOwner
 ...
 validTokenId(_id)
  ...
  function uri(uint _id) public override view returns (string memory) {
    ...
    // function addToken(uint256 supply, uint256 rate) public 
    //     onlyOwner        
    //     {
    //     supplies.push(supply);
    //     minted.push(0);
    //     rates.push(rate);
    //     WhitelistCount.push(0);   
    //     emit Log("New Token added", msg.sender, supplies.length, 0);       
...
function random(uint number) public view returns(uint){
    ...
    modifier onlyWhitelistAddress() {
        ...
        require(whitelist[msg.sender] !=0, "Address not whitelisted. Cant mint.");
        _;
    }
