...
contract ProofOfBrand is ERC721, ERC721Holder, Ownable {
    ...
    // Mapping between buyer and tokenId
    // This is use in the policy to get the tokenId for a specific buyer
    mapping(address => uint256) public tokenForBuyer;
    ...
    constructor(
        ...
        address registry
    ) ERC721(brandName, brandSymbol) {
        /*
         * The Tableland address on your current chain
         */
        _tableland = ITablelandTables(registry);

        _tablePrefix = brandName;
        ...
        */
        _metadataTableId = _tableland.createTable(
            ...
            SQLHelpers.toCreateFromSchema(
                "id int, customer text, points int, category text, perks int, pfp_image_url text, favorite_product int, vibe text, guild text",
                _tablePrefix
            )
        );
    }

    function mint(address buyer) external onlyOwner {
        require(balanceOf(buyer) == 0, "Buyer already has a token");
        _tokenIds.increment();
        uint256 tokenId = _tokenIds.current();
        _safeMint(buyer, tokenId);
        _insertSQLToken(tokenId, buyer);
        tokenForBuyer[buyer] = tokenId;
    }

    function _insertSQLToken(uint256 tokenId, address buyer) private {
        _tableland.runSQL(
            address(this),
            _metadataTableId,
            SQLHelpers.toInsert(
                ...
                // column names
                "id, customer, points, category, perks, pfp_image_url, favorite_product, vibe, guild",
                 // values
                string.concat(
    ...
    function setController(address controller) public onlyOwner {
        _tableland.setController(address(this), _metadataTableId, controller);
    }
}