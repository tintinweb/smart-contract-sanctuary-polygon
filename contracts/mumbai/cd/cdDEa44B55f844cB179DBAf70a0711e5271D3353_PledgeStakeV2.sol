...
contract PledgeStakeV2 {
    ...
    uint256[12] private roundRewardPercentages = [
        3,
        4,
        5,
        6,
    ...
    function register(address _referrer) external {
        ...
        require(
            userInfo[_referrer].totalStake > 0 || _referrer == defaultReferrer,
            "Invalid referrer"
        );
    ...
    function _stake(address _user, uint256 _amount) private {
        ...
        if (referrer != address(0)) {
            ...
            rewardInfo[referrer].directRewards = rewardInfo[referrer]
                .directRewards
                .add(directReward);
            rewardInfo[referrer].totalRewards = rewardInfo[referrer]
                .totalRewards
                .add(directReward);
            rewardInfo[referrer].availableRewards = rewardInfo[referrer]
                .availableRewards
                .add(directReward);
        }
    ...
    function getRoundReward(
        ...
        uint256 roundNumber = (block.timestamp - stake.startTime) /
            ROUND_DURATION;
    ...
    // Calculate Level reward for Upline using round reward
    function getLevelReward(address _user, uint256 _index
    ) public view returns (uint256 llevelReward) {
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        for (uint256 i = 0; i < LEVEL_COUNT; i++) {
            if (upline != address(0)) {
                uint256 roundReward = getRoundReward(_user, _index);
                if (upline == defaultReferrer || stakes[upline].length > 0) {
                    uint256 levelRewardPercentage = levelPercentage[i];
                    uint256 levelReward = roundReward
                        .mul(levelRewardPercentage)
                        .div(100);
                    return levelReward;
                }
            }
    ...
    // Get total level rewards
    function getTotalLevelRewrds(address _user) public view returns (uint256) {
        UserInfo storage user = userInfo[_user];
        address upline = user.referrer;
        ...
        for (uint256 i = 0; i < stakes[_user].length; i++) {
            ...
            if (upline == defaultReferrer || stakes[_user].length > 0) {
                if (!stake.unstaked) {
                    uint256 roundNumber = (block.timestamp - stake.startTime) /
                        ROUND_DURATION;
                    for (uint256 j = 0; j < roundNumber; j++) {
                        uint256 roundRewardPercentage = roundRewardPercentages[
                            j
                        ];
                        uint256 roundReward = stake
                            .amount
                            .mul(roundRewardPercentage)
                            .div(100);
                        for (uint256 k = 0; k < LEVEL_COUNT; k++) {
                            ...
                            uint256 levelReward = roundReward
                                .mul(levelRewardPercentage)
                                .div(100);
                            totalLevelRewards = totalLevelRewards.add(
                                levelReward
                            );
                        }
                    }
    ...
    function withdraw() external {
        ...
        uint256 totalRewards = directRewards
            .add(roundRewards)
            .add(levelRewards)
            .add(UnstakeAmount);
        ...
        rewardInfo[msg.sender].withdrawnRewards = withdrawnRewards.add(
            withdrawable
        );
        rewardInfo[msg.sender].availableRewards = rewardInfo[msg.sender]
            .availableRewards
            .sub(withdrawable);
    ...
    function _getLevelNow(address _user) private view returns (uint256) {
        ...
        return
            totalReferrals == 0
                ? 0
                : (
                    totalReferrals >= 2 && totalReferrals <= 10
                        ? totalReferrals - 1
                        : (totalReferrals == 12 ? 10 : userInfo[_user].level)
                );
    }
    ...
    function unstake(uint256 _index) external {
        ...
        uint256 roundNumber = (block.timestamp -
            stakes[msg.sender][_index].startTime) / ROUND_DURATION;
        require(roundNumber >= 1, "Stake has not completed round 1");
        // Update rewards
