...
interface IERC1155SelfMinter {
    ...
    function mintedPerAddress(address wallet) external view returns (uint256);
}
...
contract ERC1155PaymentProxy is ERC1155, ReentrancyGuard, DefaultOperatorFilterer, Ownable {
    ...
    function _mintValidation(
        ...
        if (tier.isPublic == false && !MerkleProof.verify(_merkleProof, tier.merkleRoot, keccak256(abi.encodePacked(mintAddress, tierNo)))) {
            // require (1 == 2, "invalid proof");
            revert InvalidProof();
        }

        if (mintedPerAddress[mintAddress] + amount > tiers[tierNo].limitPerWalletPerTier) {
            revert MintLimitReached();
        }

        uint256 userBalance = ERC20(tokenAddress).balanceOf(mintAddress);

        require(
            userBalance >= minimumUserBalance,
