...
contract AuthentiaComposableNFT is ERC998ERC721TopDown, Ownable {
    ...
    event Burn(address indexed from, uint indexed tokenid);
    ...
    /**
     * @dev This function returns token url for each tokenid. 
     * This function is protected from opensea by using onlyOwner modifier
     */
    function tokenURI(uint256 tokenId) public view virtual override onlyOwner() returns (string memory) {
     ...
     */
    function mint(address _to, uint256 _nftId, bytes32 ipfsAddress, bytes32[] calldata attributes) external virtual onlyOwner {
    ...
    /**
     * @dev burn requested nft id and send all children to owner wallet.
     */
    function burn(address _from, uint256 _nftId) external virtual onlyOwner {
        require(_from == ownerOf(_nftId), 'AuthentiaNFT: NFT not belongs to from address');

        uint256 numChildContracts = tokenData[_nftId].childContracts.length();
        _approve(_msgSender(), _nftId);
        if(numChildContracts > 0) {
            for(uint256 i = 0; i < numChildContracts; i++) {
                address contractAddr = tokenData[_nftId].childContracts.at(i);
                for(uint256 j = 0; j < tokenData[_nftId].childsForChildContract[contractAddr].length(); i++) {
                    safeTransferChild(_nftId, _from, contractAddr, tokenData[_nftId].childsForChildContract[contractAddr].at(j), '');
                }
            }
            delete tokenData[_nftId];
        }
        _burn(_nftId);
        emit Burn(_from, _nftId);
    }
    /**
    ...
    function addNewProductAttributes(bytes32[] memory attributes) external virtual onlyOwner {
        for(uint16 i = 0; i < attributes.length; i++) {
            productAttaributes.push(attributes[i]);
        }
    }
...
contract ERC721A is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        uint256 index = _tokenOwnership[tokenId].index;
        if(_ownersData[owner][index] == tokenId) {
            _ownersData[owner][index] = _ownersData[owner][_ownersData[owner].length - 1];
            _tokenOwnership[_ownersData[owner][index]].index = index;
            _ownersData[owner].pop();
        }

        delete _tokenOwnership[tokenId];
...
contract ERC998ERC721TopDown is ERC721A, IERC998ERC721TopDown, IERC998ERC721TopDownEnumerable {
    ...
    function totalChildContracts(uint256 _tokenId) external view virtual override returns (uint256) {
        require(_exists(_tokenId), 'ERC998: token does not exists');
    ...
    function _ownerOf(uint256 tokenId) internal view virtual override returns (address)  {
        bytes memory val = abi.encodePacked(rootOwnerOf(tokenId));
        address addr;
        assembly {
           addr := mload(add(val,20))
        }
        return addr;
    }
