    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice Modifies the start block number for execute proposal.
    /// @notice Only callable by an address that currently has the admin role.
    /// @param blockNum Value {startBlockNumber} will be changed to.
    function adminSetStartBlockNumber(uint256 blockNum) public virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        require(blockNum > 0, "BridgeUpgradeable: start block number cannot be 0");
        startBlockNumber = blockNum;
    }
    ...
    function voteProposal(
        ...
        require(
            startBlockNumber > 0 && block.number > startBlockNumber,
            "BridgeUpgradeable: start block num not set or start block number is greater than current block"
        );

        require(!_isNonceExecuted[chainID][depositNonce], "BridgeUpgradeable: nonce already executed");
    ...
    function executeProposal(
        ...
        require(
            startBlockNumber > 0 && block.number > startBlockNumber,
            "BridgeUpgradeable: start block num not set or > current block"
        );
