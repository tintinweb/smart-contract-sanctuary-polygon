...
contract FundU is UserWallet {
    ...
    constructor(
        ...
        address _USDC,
        address _USDT
    ) UserWallet(_CNET, _USDC, _USDT, _protocolManager, _feeManager) {
        s_streamId = 0;
    }
    ...
    function _newStream(
        ...
        if (_tokenAddress == i_CNET) {
            _depositPlusFees = _amountToDeposit;
            require(
    ...
    function amountNeeded(uint256 toDeposit, address tokenAddress) public view returns (uint256) {
        uint256 needed;
        if (tokenAddress == i_CNET) {
            needed = toDeposit * 10 ** 18;
        } else {
...
contract UserWallet is ReentrancyGuard, FundUtils {
    ...
    function setNewFeeManagerAddress(address newManager) external onlyProtocolManager {
        ...
        // Resetting to zero
        CNET.safeApprove(address(feeManager), 0);
        USDC.safeApprove(address(feeManager), 0);
        USDT.safeApprove(address(feeManager), 0);
