...
contract FreechainFactory is FreechainArtist {
    mapping(address => address[]) AllArtistContracts;
    ...
    function deployERC1155(
        ...
        ERC1155Token t = new ERC1155Token(_contractHash, _hashOfNFTS, quantities, msg.sender);
        AllArtistContracts[msg.sender].push( address(t) );
        tokens.push(t);
...
contract ERC1155Token is ERC1155, ERC1155Supply, Ownable  {
    ...
    */
    constructor(
        ...
        require(_hashOfNFTS.length == quantities.length);
        uint length = _hashOfNFTS.length;
        for(uint i = 0; i < length;) {
            _tokenID.increment();
            _tokenURIs[_tokenID.current()] = _hashOfNFTS[i];
            _mint(owner, _tokenID.current(), quantities[i], "");
            unchecked {
                ++i;
            }
        }
