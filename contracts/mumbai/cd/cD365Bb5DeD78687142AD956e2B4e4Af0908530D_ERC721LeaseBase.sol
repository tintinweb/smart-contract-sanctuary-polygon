...
function isLeaseAvailable(uint256 _tokenId, uint256 _start,
    ...
    function lease(uint256 _tokenId, uint256 _start, uint256 _end) external payable {
        require(_end > _start);
        require(_end.sub(_start).mul(86400) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start).mul(86400) >= MIN_LEASE_DURATION, "Lease duration is too short");
        require(_tokenId != 0);
        ...
        uint endDate = startDate.add(_end.mul(1 days));
        ...
        if (lastEndTimeByToken[_tokenId] < _end) {
          ...
          function leaseOnTransfer(uint256 _tokenId, uint256 _start, uint256 _end, address _addressTo) internal {
        require(_end > _start);
        require(_end.sub(_start).mul(86400) <= MAX_LEASE_DURATION, "Lease duration is too long");
        require(_end.sub(_start).mul(86400) >= MIN_LEASE_DURATION, "Lease duration is too short");
        require(_tokenId != 0);
   ...
   function transferLease(uint256 _tokenId, uint256 _start, uint256 _end, address _addressTo) external payable {
          ...
          unlease(_tokenId, _start, _end);
          leaseOnTransfer(_tokenId, _start, _end, _addressTo);
        }

    function unlease(uint256 _tokenId, uint _start, uint _end) public payable {
