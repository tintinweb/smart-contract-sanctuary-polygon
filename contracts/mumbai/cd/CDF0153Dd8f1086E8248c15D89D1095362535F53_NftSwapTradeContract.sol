...
/**
 * @dev Contract implementing base trade contract functions for Global Fear Exchange.
 */
abstract contract BaseTradeContract is AccessControl, Pausable, Withdrawable, ITradeContract {
    using SafeMath for uint256;

    string public name;
    
    TradePosition[] public tradePositions;
    mapping(string => TradePosition) public tradePositionsByName;

    mapping(address => mapping(string => uint256)) internal walletTradesAmount;
    ...
    function _preTrade(address buyer, string memory positionName, uint256 amount) internal virtual {
        TradePosition storage position = tradePositionsByName[positionName];

        require(position.maxPerTrade == 0 || amount <= position.maxPerTrade, "Maximum per trade limit is exceeded");
        require(position.maxPerWallet == 0 || walletTradesAmount[buyer][positionName] + amount <= position.maxPerWallet, "Maximum per wallet limit is exceeded");
    }

    function _postTrade(address buyer, string memory positionName, uint256 amount) internal virtual {
        walletTradesAmount[buyer][positionName] += amount;
    }

    function trade(address buyer, string memory positionName, uint256 amount) external {
        _preTrade(buyer, positionName, amount);
        _transferInAssets(positionName, buyer, address(this), amount);
        _transferOutAssets(positionName, address(this), buyer, amount);
        _postTrade(buyer, positionName, amount);
    }
    ...
    function addTradePosition(string memory positionName, uint256 maxPerTrade, uint256 maxPerWallet) external onlyRole(MANAGER_ROLE) {
        TradePosition storage newPosition = tradePositions.push();
        newPosition.positionName = positionName;
        newPosition.maxPerTrade = maxPerTrade;
        newPosition.maxPerWallet = maxPerWallet;
        tradePositionsByName[positionName] = newPosition;
    }

    function editTradePosition(uint256 positionIndex, string memory positionName, uint256 maxPerTrade, uint256 maxPerWallet) external onlyRole(MANAGER_ROLE) {
        TradePosition storage newPosition = tradePositions[positionIndex];
        newPosition.positionName = positionName;
        newPosition.maxPerTrade = maxPerTrade;
        newPosition.maxPerWallet = maxPerWallet;
    }

    function removeTradePosition(uint256 positionIndex) public onlyRole(MANAGER_ROLE) {
        string memory positionName = tradePositions[positionIndex].positionName;
        tradePositions[positionIndex] = tradePositions[tradePositions.length - 1];
        tradePositions.pop();
        delete tradePositionsByName[positionName];
    }

    function removeTradePositionByName(string memory positionName) external onlyRole(MANAGER_ROLE) {
        bytes32 hashedName = keccak256(bytes(positionName));

        for (uint i = 0; i < tradePositions.length; i++) {
            if (keccak256(bytes(tradePositions[i].positionName)) == hashedName) {
                removeTradePosition(i);
                break;
            }
        }
    }

    function editTradePositionAddInAsset(uint256 positionIndex, string memory assetName, AssetType assetType, address assetAddress, uint256 decimals, uint256 amount, uint256 id) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
        AssetInfo storage asset = position.inAssets.push();
        _editAsset(asset, assetName, assetType, assetAddress, decimals, amount, id);
    }

    function editTradePositionEditInAsset(uint256 positionIndex, uint256 assetIndex, string memory assetName, AssetType assetType, address assetAddress, uint256 decimals, uint256 amount, uint256 id) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
        AssetInfo storage asset = position.inAssets[assetIndex];
        _editAsset(asset, assetName, assetType, assetAddress, decimals, amount, id);
    }

    function editTradePositionRemoveInAsset(uint256 positionIndex, uint256 assetIndex) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
        position.inAssets[assetIndex] = position.inAssets[position.inAssets.length - 1];
        position.inAssets.pop();
    }

    function editTradePositionAddOutAsset(uint256 positionIndex, string memory assetName, AssetType assetType, address assetAddress, uint256 decimals, uint256 amount, uint256 id) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
        AssetInfo storage asset = position.outAssets.push();
        _editAsset(asset, assetName, assetType, assetAddress, decimals, amount, id);
    }

    function editTradePositionEditOutAsset(uint256 positionIndex, uint256 assetIndex, string memory assetName, AssetType assetType, address assetAddress, uint256 decimals, uint256 amount, uint256 id) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
        AssetInfo storage asset = position.outAssets[assetIndex];
        _editAsset(asset, assetName, assetType, assetAddress, decimals, amount, id);
    }

    function editTradePositionRemoveOutAsset(uint256 positionIndex, uint256 assetIndex) external onlyRole(MANAGER_ROLE) {
        TradePosition storage position = tradePositions[positionIndex];
