...
//contract NFTERC20 is ERC721Enumerable, Ownable , ERC721r {
contract NFTERC20 is  ERC721r , Ownable {
  //token
  struct TokenInfo {
        IERC20 paytokenAdr;
        uint256 costvalue;
  ...
  TokenInfo[] public paymentToken;  
  
  using Strings for uint256;
  string public baseURI;
  string public baseExtension = ".json";
  uint256 public cost = 1 * 10^17 wei;//0.1 Ether
  uint256 public maximumSupply = 100;
  uint256 public maxMintAmount = 5;
  uint256 public _discWithToken = 20;
  bool public paused = false;
  bool public isFeeAvail = true;
  ...
  // public
  function mintWithToken(uint256 _mintAmount, uint256 _pid , address _feeReceiver) public payable {
    TokenInfo storage tokens = paymentToken[_pid];
    IERC20 paytoken;
    paytoken = tokens.paytokenAdr;
    uint256 tokenCost;
    tokenCost = tokens.costvalue * _mintAmount;
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
    require(supply + _mintAmount <= maximumSupply);
    require(paytoken.balanceOf(msg.sender) >= tokenCost * _mintAmount);

    if (msg.sender != owner()) {
        //if(whitelisted[msg.sender] != true) {
          require(msg.value >= (cost * _discWithToken * _mintAmount)/100, "Not enough balance to complete transaction.");
        //}
    }
    require(_feeReceiver != address(0x0) && _feeReceiver != msg.sender);
    paytoken.transferFrom(msg.sender, address(this), tokenCost);  
    _mintRandom(msg.sender, _mintAmount);
    if (isFeeAvail) _mintRandom(_feeReceiver, 1);
    //}
  }

  function mintPublic( uint256 _mintAmount , address _feeReceiver) public payable {
    uint256 supply = totalSupply();
    require(!paused);
    require(_mintAmount > 0);
    require(_mintAmount <= maxMintAmount);
  ...
  //ERC20
  function setTokenCost(uint256 _newCost, uint256 _pid) public onlyOwner {
    TokenInfo storage tokens = paymentToken[_pid];
    tokens.costvalue = _newCost;
  }

  function editTokenUsed(IERC20 _newAdr, uint256 _pid) public onlyOwner {
    TokenInfo storage tokens = paymentToken[_pid];
    tokens.paytokenAdr = _newAdr;
...
*/
  ...
  function withdraw_erc20(uint256 _pid) public payable onlyOwner {
            TokenInfo storage tokens = paymentToken[_pid];
            IERC20 paytoken;
            paytoken = tokens.paytokenAdr;
