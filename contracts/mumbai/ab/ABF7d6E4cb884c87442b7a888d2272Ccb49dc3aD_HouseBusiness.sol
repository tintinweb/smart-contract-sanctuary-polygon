...
contract HouseBusiness is ERC721, ERC721URIStorage {
    ...
    function addHistory(
        ...
        // uint256 _percent;
        // if (_contractId > 0) _percent = labelPercent.connectContract;
        // if (bytes(_houseImg).length > 0) _percent += labelPercent.image;
        // if (bytes(_houseBrand).length > 0) _percent += labelPercent.brand;
        // if (bytes(_desc).length > 0) _percent += labelPercent.desc;
        // if (bytes(_brandType).length > 0) _percent += labelPercent.brandType;
        // if (_yearField > 0) _percent += labelPercent.year;

        // allHouses[_houseId].price += (historyTypes[_historyTypeId].value * _percent) / 100;
    ...
    function test(uint256 _houseId) public view returns (uint256) {
