...
import "@openzeppelin/contracts/utils/math/SafeMath.sol";
 contract PledgeStakeV2 {
    ...
    function getLevelReward(address _user) public view returns (uint256) {
        ...
        UserInfo storage user = userInfo[_user];
        uint256 _level = user.level;
        
        address upline = user.referrer;
        
        while (upline != address(0) && userInfo[upline].totalReferrals >= 2) {
            for (uint256 i = 0; i < stakes[upline].length; i++) {
                uint256 roundRewards = getRoundReward(upline, i);
                uint256 levelOfPercentage = levelPercentage[_level - 1]; // Get the level percentage from the array
                levelReward += (roundRewards * levelOfPercentage) / 100;
            }
    ...
    function getTotalLevelRewards(address _user) public view returns (uint256) {
        ...
        // Iterate through each level and calculate the level rewards
    ...
    function _updateRewards(address _user) private {
        rewards[_user].roundRewards = getTotalRoundRewards(_user);
        rewards[_user].levelRewards = getTotalLevelRewards(_user);
        rewards[_user].totalRewards = rewards[_user].directRewards.add(rewards[_user].roundRewards).add(rewards[_user].levelRewards);
        rewards[_user].availableRewards = rewards[_user].totalRewards.sub(rewards[_user].withdrawnRewards);
    }
