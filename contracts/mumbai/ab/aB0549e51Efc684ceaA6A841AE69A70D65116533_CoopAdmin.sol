...
contract CoopAdmin is Ownable {
    ...
    event Activated(uint16 tokenId, address owner ,uint8 activated, uint256 stake, uint256 timestamp);
    ...
    function transferFromOwner(uint16 tokenId, address to, uint256 stake, uint8 activated, bool fullMembership) public onlyOwner {
        ...
        emit Activated(tokenId, to, activated, stake, block.timestamp);
    }

    function mint(address to, uint256 stake, uint8 activated, bool fullMembership ) public onlyOwner {
        ...
        emit Activated(uint16(tokenId), to, activated, stake, block.timestamp);
    }

    function setActive(uint16 tokenId, uint8 activated) public onlyOwner {
        activatedImplementer.setValue(tokenId, activated);
        emit Activated(tokenId, address(0), activated, stakeImplementer.getValue(tokenId), block.timestamp);
    }
}