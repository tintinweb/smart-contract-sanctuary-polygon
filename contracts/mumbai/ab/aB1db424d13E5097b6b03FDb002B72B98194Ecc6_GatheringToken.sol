...
contract GatheringToken is Initializable, ERC20Upgradeable {
  uint256 DOC_AWARD;
  uint256 VOTE_AWARD;
  uint256 PASS_THRESHOLD;
  uint256 VOTE_DOCS_TO_CHECK;
  ...
  event DocAdded(uint256 docId);
  event DocApproved(uint256 docId);

  function addDoc(string memory _docUid, uint8 _docType) external returns (uint256) {
  ...
  function voteOnDoc(uint256 _docId, uint8 _vote) external returns (bool approved) {
    require(balanceOf(msg.sender) > 0, 'Not part of this gathering.');
    require(_docId < docCount, "Doc doesn't exist.");

    bool votedAlready = false;
    Doc storage doc = docs[_docId];

    // Check if the user has voted already
    for (uint256 j; j < doc.totalVotesCount; j++) {
    ...
    // _mint(msg.sender, voteAward * 10**uint256(decimals()));
    ...
    if (_vote == 1) {
      ...
      uint256 threshold = totalSupply() * (PASS_THRESHOLD / 100);
      if (doc.totalApproveVotesWeight > threshold) {
        doc.approved = true;

        // Reward submitter with award tokens
        _mint(doc.submitter, DOC_AWARD * 10**uint256(decimals()));
        // Caculate allowance value
        uint256 approvedUSDC = IERC20(paymentTokenAddress).allowance(doc.submitter, address(this)); // calcluate approved USDC balance of submitter
        // Transfer USDC token
        transferPaymentToken(doc.submitter, address(this), approvedUSDC); // transfer x-amount USDC to token contract

        emit DocApproved(_docId);
        return true;
      }
      return false;
    } else if (_vote == 2) {
  ...
  function docsToVoteOn() public view returns (uint256[] memory) {
    ...
    if (docCount < VOTE_DOCS_TO_CHECK) {
      toCheck = docCount;
    } else {
      toCheck = VOTE_DOCS_TO_CHECK;
    }

    uint256[] memory docsToVoteOnArr = new uint256[](toCheck);
    uint256 toVoteCount = 0;
    for (uint256 i = 1; i <= toCheck; i++) {
    ...
    for (uint256 i = 0; i < toVoteCount; i++) {
  ...
  function transferPaymentToken(
    ...
    // If you test this function, you must change this funtion as public function not private function
  ...
  function tokenTransfer() external {
    ...
    uint256 usdcBalance = IERC20(tokenAddress).balanceOf(address(this)); // get ERC20 token balance of gatheringToken contract
    IERC20(tokenAddress).transfer(msg.sender, usdcBalance); // transfer ERC20 token balance of gatheringToken contract to gBGT holder.
  }
  */
