...
contract Alligators is
    ...
    */
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        internalFulfill(_requestId);
        emit RequestFulfilled(_requestId, _randomWords);
    }

    function internalFulfill(uint256 _requestId) internal {
        uint256[] memory randomWords_ = vrf_requests[_requestId].randomWords;
        address minter = vrf_requests[_requestId].sender;
        uint256 perThisTx = vrf_requests[_requestId].quantity;
        uint256 moddedRng = randomWords_[0] % MAX_CHANCE_VALUE;
        if (saleStatus == SaleStatus.PUBLIC || saleStatus == SaleStatus.PRESALE) {
            if (moddedRng < MIN_CHANCE_VALUE) {
                alligatorMint(minter, perThisTx, _requestId);
            } else {
                jokerMint(minter, _requestId);
            }
        }
    ...
    function alligatorMint(address to,uint256 _quantity, uint256 _requestId) internal {
        ...
        uint256[] memory randomWords_ = vrf_requests[_requestId].randomWords;
            for (uint i = 1; i <= _quantity; i++) {
                Trait1 type1 = Trait1(randomWords_[1+i] % 3);
                Trait2 type2 = Trait2(randomWords_[2+i] % 3);
                Trait3 type3 = Trait3(randomWords_[3+i] % 3);
                Trait4 type4 = Trait4(randomWords_[4+i] % 3);
                Trait5 type5 = Trait5(randomWords_[5+i] % 3);
                Trait6 type6 = Trait6(randomWords_[6+i] % 3);
                Trait7 type7 = Trait7(randomWords_[7+i] % 3);
                _anatomy[_currentIndex] = NFT_Anatomy(
                    {
                        trait1: type1, 
                        trait2: type2,
