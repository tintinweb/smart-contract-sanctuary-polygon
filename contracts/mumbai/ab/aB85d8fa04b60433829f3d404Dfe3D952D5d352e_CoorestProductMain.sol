...
import "CoorestProductCo2.sol";
import "CoorestProductUsdc.sol";

contract CoorestProductMain is 
    Ownable
{
    ...
    struct CombinedPolicy {
        ...
        uint256 nftreeId;
        uint256 duration;
        uint256 sumInsured; // used for usdc sub-polciy
        bytes32 processIdCco2;
        bytes32 processIdUsdc;
        uint256 createdAt;
    }

    event LogCoorestMainProductsSet(address productCco2, address productUsdc);
    event LogCoorestMainPolicyCreated(bytes32 policyId, bytes32 processIdCoo2, bytes32 processIdUsdc);

    mapping(address /* policyHolder */ => bytes32 [] /* processIds */) private _processIdsForHolder;
    mapping(bytes32 /* policy id */ => CombinedPolicy) private _policies;
    bytes32 [] private _policyIds;

    CoorestProductCo2 private _productCco2;
    CoorestProductUsdc private _productUsdc;
    ...
    function setProducts(
        ...
        onlyOwner
    {
        require(productCco2 != address(0), "ERROR:COM-001:CCO2_PRODUCT_ZERO");
        require(productUsdc != address(0), "ERROR:COM-002:USDC_PRODUCT_ZERO");
        // TODO add more checks (eg products have tokens as expected)

        _productCco2 = CoorestProductCo2(productCco2);
        _productUsdc = CoorestProductUsdc(productUsdc);

        emit LogCoorestMainProductsSet(productCco2, productUsdc);
    }
    ...
    function newPolicy(
        ...
        external
        returns(bytes32 combinedPolicyId)
    {
        require(productsAreSet(), "ERROR:COM-010:PRODUCTS_NOT_SET");
        require(cco2 || usdc, "ERROR:COM-011:EMPTY_POLICY_INVALID");
        ...
        if(cco2) {
            (processIdCco2,) = _productCco2.newCo2Policy(
                policyHolder,
                nftreeHolder,
                nftreeId,
                duration);
        ...
        if(usdc) {
            (processIdUsdc,) = _productUsdc.newUsdcPolicy(
                policyHolder,
                nftreeHolder,
                nftreeId,
                duration,
        ...
        combinedPolicyId = getPolicyId(processIdCco2, processIdUsdc);
        _processIdsForHolder[policyHolder].push(combinedPolicyId);

        _policyIds.push(combinedPolicyId);
        _policies[combinedPolicyId] = CombinedPolicy(
            combinedPolicyId,
            nftreeHolder,
            nftreeId,
            duration,
            sumInsured, // used for usdc sub-polciy
            processIdCco2,
            processIdUsdc,
            block.timestamp);
        
        emit LogCoorestMainPolicyCreated(combinedPolicyId, processIdCco2, processIdUsdc);
    }
    ...
    {
        require(_processIdsForHolder[policyHolder].length > 0, "ERROR:COM-020:NO_POLICIES_FOR_HOLDER");
        require(idx < _processIdsForHolder[policyHolder].length, "ERROR:COM-021:POLICY_INDEX_TOO_LARGE");
        return _processIdsForHolder[policyHolder][idx];
    }


    function processIds() external view returns(uint256 numberOfCombinedPolicies) {
        return _policyIds.length;
    }


    function getProcessId(uint256 idx) external view returns(bytes32 policyId) {
        require(idx < _policyIds.length, "ERROR:COM-020:INDEX_TOO_LARGE");
        return _policyIds[idx];
    }


    function getPolicy(bytes32 policyId) external view returns(CombinedPolicy memory policy) {
        require(_policies[policyId].createdAt > 0, "ERROR:COM-030:POLICY_ID_UNKNOWN");
        return _policies[policyId];
    }


    function getCco2Product() external view returns(CoorestProductCo2) {
        return _productCco2;
    }


    function getUsdcProduct() external view returns(CoorestProductUsdc) {
        return _productUsdc;
    }


    function productsAreSet() public view returns(bool) {
        return address(_productCco2) != address(0) && address(_productUsdc) != address(0);
    }


    function getPolicyId(bytes32 processIdCco2, bytes32 processIdUsdc) public pure returns(bytes32 policyId) {
        return keccak256(abi.encode(processIdCco2, processIdUsdc));
    }
}
