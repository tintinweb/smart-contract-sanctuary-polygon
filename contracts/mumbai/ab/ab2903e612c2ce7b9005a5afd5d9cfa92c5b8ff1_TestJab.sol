...
contract TestJab is FTM20, Ownable {
    ...
    constructor(address payable _creatorWallet, address payable _marketingWallet) FTM20("TESTJab", "JBL") {
    ...
    function buy(address referrer) external payable {
        ...
        uint256 tokenBrought = ftmToTokens_(ftmAfterTax);
        ...
        if (user.referrer != address(0)) {
            address upline = user.referrer;
            uint256 totalInvest = users[upline].totalInvested;
        ...
        uint256 _updatedPayouts;
        if(_fee > (profitPerShare_ * tokenBrought)){
            _updatedPayouts = (uint256) ((profitPerShare_ * tokenBrought) - _fee);
        }else{
            _updatedPayouts = (uint256) ((profitPerShare_ * tokenBrought));
        }
    ...
    function sell(uint256 _amount) external {
        ...
        uint256 ftmAmount = tokensToFtm_(_amount);
        ...
        if(user.payoutTo > _updatedPayouts){
            user.payoutTo -= _updatedPayouts;
        }else{
            user.payoutTo = 0;
        }
        ...
        (creatorWallet).transfer(deductionAmount/2);
        payable(msg.sender).transfer(ftmRemaining);
