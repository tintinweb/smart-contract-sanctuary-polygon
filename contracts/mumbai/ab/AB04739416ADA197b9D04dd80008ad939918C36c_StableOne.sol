...
contract StableOne is ReentrancyGuard, Ownable {
    ...
    uint256 private constant PLAN_TERM = 30 minutes;
    uint256 private constant TIME_STEP_DRAW = 1 minutes;
    ...
    uint256 private constant AVG_SCALE = 1000;
    ...
    struct Investor {
        ...
        uint256 previousAvg;
        uint256 previousAvgCalculationDate;
        uint256 totalAvg;
   ...
   mapping(address => Investor) public investors;
	...
	event OnWithdraw(address investor, uint256 amount); // complte it
    ...
    function calculateWeightedAverage(
        uint256 _previousAvg,
        uint256 _prevInvestment,
        uint256 _totalInvestment,
        uint256 _numOfDays
    ) private pure returns(uint256){
        if(_totalInvestment == 0 || _prevInvestment == 0){
            return 0;
        }
        uint256 firstEquation = (_previousAvg * _prevInvestment) / _totalInvestment;
        uint256 secondEquation = (_prevInvestment * _numOfDays * 1000) / _totalInvestment;
        uint256 weightedAvg = firstEquation + secondEquation;
        return weightedAvg;
    }
    ...
    function _invest(
        ...
        uint256 _prevInvestment = _investor.totalInvestment;
        if (_investor.addr == address(0)) {
            _addInvestor(_addr, _ref, _amount);
            investors[_addr].previousAvgCalculationDate = block.timestamp;
        }

        _investor.totalDeposit = _investor.totalDeposit.add(
            _amount
        );
        
        totalInvested = totalInvested.add(_amount);
        activeInvested = activeInvested.add(_amount);
        investors[_addr].totalInvestment = investors[_addr].totalInvestment.add(_amount);

        uint256 numOfDays = block.timestamp.sub(_investor.previousAvgCalculationDate).div(TIME_STEP_DRAW);
        if(numOfDays >= 1){
        uint256 weightedAvg = calculateWeightedAverage(
            _investor.previousAvg,
            _prevInvestment,
            _investor.totalInvestment,
            numOfDays
        );

        _investor.previousAvg = weightedAvg;
        _investor.totalAvg = calculateWeightedAverage(
            _investor.totalAvg,
            _prevInvestment,
            _investor.totalInvestment,
            numOfDays
        ...
        _investor.previousAvgCalculationDate = block.timestamp;
    ...
    function _reinvest(address _addr,uint256 _amount) private returns(bool){
        ...
        _investor.totalReinvest = _investor.totalReinvest.add(
            _amount
        );
    ...
    function _addInvestor(
        ...
        investors[_addr].startedDate = block.timestamp;
		investors[_addr].lastWithdrawDate = block.timestamp;
    }
    ...
    function _sendRewardOnInvestment(uint256 _amount) private {
        ...
        uint256 rewardForSecondaryBenificiary = _amount
            ...
            .div(1000);
        uint256 totalReward = rewardForPrimaryBenificiary + rewardForSecondaryBenificiary;
        primaryBenificiary.transfer(totalReward);
    }
    ...
    function _sendRewardOnReinvestment(uint256 _amount) private {
        ...
        uint256 totalReward = rewardForPrimaryBenificiary.add(rewardForSecondaryBenificiary);
        primaryBenificiary.transfer(totalReward);
    }
    ...
    function getDividends(address addr) public view returns (uint256,uint256) {
        Investor storage investor = investors[addr];

        uint256 dividendAmount = 0;
        console.log("dividendAmount",dividendAmount);
        uint256 currentAvg = investor.previousAvg;
        console.log("currentAvg",currentAvg);
        uint256 numOfDays = block.timestamp.sub(investor.previousAvgCalculationDate).div(TIME_STEP_DRAW);
        console.log("numOfDays",numOfDays);
        if(numOfDays>=1){
            currentAvg = calculateWeightedAverage(
                investor.previousAvg,
        ...
        dividendAmount = (investor.totalInvestment * 70 * currentAvg)/(1000*1000);
        console.log("dividendAmount",dividendAmount);

        return (dividendAmount,currentAvg);
    }
    ...
    function withdraw() public nonReentrant {
		require(investors[msg.sender].lastWithdrawDate+TIME_STEP_DRAW<=block.timestamp,"Withdrawal limit is 1 withdrawal in 24 hours");
        uint256 _amountToReinvest=0;
		uint256 _reinvestAmount=0;
		uint256 totalToReinvest=0;
        uint256 dividendAmount;
        uint256 currentAvg;
        (dividendAmount,currentAvg) = getDividends(msg.sender);
        ...
        if(investors[msg.sender].totalAvg>=30000){
            investors[msg.sender].totalInvestment = 0;
        }
        ...
        investors[msg.sender].totalWithdraw = investors[msg.sender].totalWithdraw.add(remainingAmount);
        investors[msg.sender].previousAvg = 0;
