...
contract gameescroe is Ownable {
    using Address for address payable;
    HRAC public _HRAC;
    Coin public coin;
    event Deposited(uint256 amount);
    event Withdrawn();
    uint256 coinsamount = 300;
    uint256 tokenleft;
    constructor (HRAC HRAC_) Ownable() {
        _HRAC = HRAC_;      
    }
    function deposit(uint256 amount) public virtual onlyOwner {
        require (amount > 0, "amount should be greater than zero");
        _HRAC.transferFrom(msg.sender,address(this),amount);
        tokenleft = amount;
    }
    function withdraw() external payable {
        require(msg.value == coinsamount, "value of coin is equal tp 300");
        uint256 tokenamouunt = 1;
        require(tokenleft >= tokenamouunt);
        tokenleft = tokenleft - tokenamouunt;
        _HRAC.transferFrom(address(this),msg.sender,tokenamouunt);
        coin.transferFrom(msg.sender,owner(),msg.value);
        emit Withdrawn();
