...
contract Auctionv1 is Initializable, OwnableUpgradeable {
    ...
    */
    ...
    modifier checkForBidAuction(Auctions storage _auc, uint256 _tokenId) {
        require(
            _auc.startTime < block.timestamp && _auc.endTime +  _auc.tokenBid[_tokenId].length * fifteenMinute > block.timestamp && checkTokenIdExist(_auc, _tokenId)
        );
        _;
    }
        ...
        checkForBidAuction(auctionsDetails[auctionId], tokenId) 
    {
        ...
        emit onBuyOrBid(auctionId, _auc.imxNftAddress, _auc.price, block.timestamp,_auc.endTime + _auc.largestBidLength * fifteenMinute, tokenId, _auc.auctionType);
    }
    ...
    function endAuction(uint256 auctionId) public onlyOwner {
        Auctions storage _auc = auctionsDetails[auctionId];
        require(
            !_auc.isEnded && _auc.endTime + _auc.largestBidLength * fifteenMinute < block.timestamp,
    ...
    {     
        ...
        for (uint256 i = 0; i < auctionsDetails.length; i++) {
            if (auctionsDetails[i].endTime <= block.timestamp && !auctionsDetails[i].isEnded)  {
                size++;
            }
        }
        ...
        )
    {
     ...
     function checkTokenIdExist(Auctions storage _auc, uint256 tokenId) internal returns (bool) {
        bool flag = false;
        for(uint256 i = 0; i < _auc.tokenIds.length; i++) {
    ...
    function withdrawAnonymousToken(IERC20Upgradeable token, address receiver, uint256 amount) external onlyOwner {
