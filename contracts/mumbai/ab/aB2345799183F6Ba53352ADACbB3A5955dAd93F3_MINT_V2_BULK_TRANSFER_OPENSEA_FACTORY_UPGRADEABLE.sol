...
import './MINT_V2_BULK_TRANSFER_OPENSEA_UPGRADEABLE.sol';
import '@openzeppelin/contracts/proxy/transparent/TransparentUpgradeableProxy.sol';
import '@openzeppelin/contracts/proxy/transparent/ProxyAdmin.sol';

/**
 * MINT_V2_BULK_TRANSFER_OPENSEA_FACTORY_UPGRADEABLE
 *
 * Client deploys from the Mint console so
 * Using this contract, Mint system can tie it to the Mint system by storing the address of the deployed contract.
 */
contract MINT_V2_BULK_TRANSFER_OPENSEA_FACTORY_UPGRADEABLE is IMINT_CONTRACT {
  bytes32 private constant CONTRACT_TYPE = bytes32('MINT_BULK_OPENSEA_FACTORY_UP');
  ...
  /**
   * @dev Deploy ERC721_MINT_V2_BULK_TRANSFER_OPENSEA and set up settings that is needed for Mint
   *
   * royaltyFeeNumerator's max is 1000 so 10 === 1%
   */
  function createProxyContract(
    ...
    bool isForbiddenSale,
    address[] memory minterRoleAddress
    ...
    TransparentUpgradeableProxy newProxy = new TransparentUpgradeableProxy(
      ...
      address(PROXY_ADMIN),
      abi.encodeWithSelector(MINT_V2_BULK_TRANSFER_OPENSEA_UPGRADEABLE(address(0)).initialize.selector, name, symbol)
    );

    MINT_V2_BULK_TRANSFER_OPENSEA_UPGRADEABLE proxyLogic = MINT_V2_BULK_TRANSFER_OPENSEA_UPGRADEABLE(address(newProxy));

    // Setting Royalty
    proxyLogic.setDefaultRoyalty(royaltyReceiver, royaltyFeeNumerator);

    // Setting Role
    // Because Factory becomes Owner by default.
    proxyLogic.transferOwnership(msg.sender);
    proxyLogic.grantRole(proxyLogic.DEFAULT_ADMIN_ROLE(), msg.sender);
    proxyLogic.grantRole(proxyLogic.PAUSER_ROLE(), msg.sender);
    proxyLogic.grantRole(proxyLogic.ROYALTY_ROLE(), msg.sender);
    for (uint i = 0; i < minterRoleAddress.length; i++) {
      proxyLogic.grantRole(proxyLogic.MINTER_ROLE(), minterRoleAddress[i]);
    }
    ...
    // revoke role after init the contract
    proxyLogic.renounceRole(proxyLogic.DEFAULT_ADMIN_ROLE(), address(this));
    proxyLogic.renounceRole(proxyLogic.PAUSER_ROLE(), address(this));
    proxyLogic.renounceRole(proxyLogic.ROYALTY_ROLE(), address(this));
    proxyLogic.renounceRole(proxyLogic.MINTER_ROLE(), address(this));

    emit ContractDeployed(msg.sender, address(proxyLogic));
    return address(proxyLogic);
  }
}
...
import 'operator-filter-registry1.4.1/src/upgradeable/DefaultOperatorFiltererUpgradeable.sol';

import '../IMINT_CONTRACT.sol';

/**
 * ERC_721_MINT_V2_BULK_TRANSFER_OPENSEA
 * - Support Royalty(https://docs.openzeppelin.com/contracts/4.x/api/token/erc721#ERC721Royalty)
 * - AccessControl
 * - Ownable(For only OpenSea)
 * - ERC721URIStorage
 * - Burnable
 * - Forbid secondary sales
 *
 * NOTE: Inintialized by https://wizard.openzeppelin.com/#erc721
 */
contract MINT_V2_BULK_TRANSFER_OPENSEA_UPGRADEABLE is
  Initializable,
  ERC721Upgradeable,
  ...
  IMINT_CONTRACT
{
  bytes32 private constant CONTRACT_TYPE = bytes32('MINT_BULK_OPENSEA_UPGRADEABLE');
  ...
  function transferFrom(
    ...
    address to,
    uint256 tokenId
  ) public virtual override(ERC721Upgradeable, IERC721Upgradeable) whenNotPaused onlyAllowedOperator(from) {
  ...
  function bulkMint(
    ...
    require(length == tokenURIs.length, 'tokenIds Length and tokenURIs length are not matched');
    require(length == toAddresses.length, 'TokenIds Length and toAddresses length are not matched');
    for (uint8 i; i < length; i++) {
      mint(tokenIds[i], toAddresses[i], tokenURIs[i]);
    }
  }
  ...
  function bulkTransfer(
    ...
    address from,
    address[] memory toAddresses
  ) public virtual whenNotPaused onlyAllowedOperator(from) {
    uint256 length = tokenIds.length;
    require(length == toAddresses.length, 'TokenIds Length and toAddresses length are not matched');
    for (uint8 i; i < length; i++) {
