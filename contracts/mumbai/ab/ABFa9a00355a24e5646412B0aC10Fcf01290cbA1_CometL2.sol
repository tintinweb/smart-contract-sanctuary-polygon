...
// File contracts/CometL2.sol

pragma solidity ^0.8.9;


contract CometL2 is AxelarExecutable {
    ...
    error Unauthorized();
    error InvalidCallingChain();
    event Executed(address indexed _from, uint8 _value);

    constructor(address _pauseGuardian, address _governor, address gateway_) AxelarExecutable(gateway_) {
    ...
    function _execute(
        ...
        // Make sure the call is coming from ETH mainnet
        if (keccak256(abi.encodePacked(_sourceChain)) != keccak256(abi.encodePacked("ethereum-2"))) revert InvalidCallingChain();
        //if (msg.sender != governor && msg.sender != pauseGuardian) revert Unauthorized();

        pauseFlags = abi.decode(payload, (uint8));
        
        emit Executed(msg.sender, pauseFlags);
        //emit PauseAction(supplyPaused, transferPaused, withdrawPaused, absorbPaused, buyPaused);
    }
