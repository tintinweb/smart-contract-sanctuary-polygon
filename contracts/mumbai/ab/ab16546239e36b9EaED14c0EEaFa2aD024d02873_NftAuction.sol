...
contract NftAuction is IERC721Receiver, ReentrancyGuard, AccessProtected, BaseRelayRecipient, Pausable {
    ...
    function claimNftFromEnglishAuction(uint256 _auctionId) external nonReentrant whenNotPaused {
        require(auctions[_auctionId].isActive == true, "Not active auction");
        require(auctions[_auctionId].closingTime <= block.timestamp, "Auction is not closed");
        require(auctions[_auctionId].highestBidder == _msgSender(), "You are not winner of this Auction");
    ...
    function setTrustedForwarder(address _trustedForwarder) external onlyAdmin whenNotPaused {
