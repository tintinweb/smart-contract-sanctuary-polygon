...
//shareholders
    ...
    mapping(uint16 => ShareData) public share;
    ...
    function revenueToBalance(uint8 range) public{
        uint amount = ((revenue*1)/100)-ranges[range].paid;

        if(amount < 9)
            return;

        for(uint8 i = (range-9); i <= range; i++)
            share[i].balance += amount;
    ...
    function getDividend(uint16 start, uint16 max) public {
        ...
        {
            uint16 k = shareholders[msg.sender].shares[i];
            amount += share[k].balance;
            share[k].balance = 0;
        }
