...
contract MarketMakerLite is IMarketMaker {
    ...
    function approveAll() public {
        IERC20(token0).approve(address(NFTManager), ~uint256(0));
        IERC20(token1).approve(address(NFTManager), ~uint256(0));
    }
    ...
    function getBalance() public returns (uint256 amount0, uint256 amount1) {
        amount0 = IERC20(token0).balanceOf(collector);
        amount1 = IERC20(token1).balanceOf(collector);
    }
