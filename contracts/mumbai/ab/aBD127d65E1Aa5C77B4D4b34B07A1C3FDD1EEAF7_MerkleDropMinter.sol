...
import { MerkleProofLib } from "../helpers/MerkleProofLib.sol";
import { IERC165 } from "@openzeppelin/contracts/utils/introspection/IERC165.sol";
import { IMelonFeeRegistry } from "../interfaces/IMelonFeeRegistry.sol";
import { BaseMinter } from "./BaseMinter.sol";
import { IMerkleDropMinter, DropMintData, MintInfo } from "../interfaces/IMerkleDropMinter.sol";
 ...
 * @dev Module for minting Melon drops using a merkle tree of approved accounts.
 */
contract MerkleDropMinter is IMerkleDropMinter, BaseMinter {
    ...
    function mint(
        ...
        _mint(drop, mintId, requestedQuantity, affiliate);
    ...
    function setMaxMintable(
        ...
        _dropMintData[drop][mintId].maxMintable = maxMintable;
        emit MaxMintableSet(drop, mintId, maxMintable);
    }
    ...
    function setMerkleRootHash(
        ...
        _dropMintData[drop][mintId].merkleRootHash = merkleRootHash;
        emit MerkleRootHashSet(drop, mintId, merkleRootHash);
    }
...
/**
 * @dev All the information about a merkle drop mint (combines DropMintData with BaseData).
 */
struct MintInfo {
    uint32 startTime;
    uint32 endTime;
...
interface IMerkleDropMinter is IMinterModule {
    ...
    /**
     * @dev Emitted when the `maxMintable` is changed for (`drop`, `mintId`).
     * @param drop               Address of the song drop contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintable The maximum number of tokens that can be minted on this schedule.
     */
    event MaxMintableSet(address indexed drop, uint128 indexed mintId, uint32 maxMintable);
    ...
    /**
     * @dev Emitted when the `merkleRootHash` is changed for (`drop`, `mintId`).
     * @param drop        Address of the song drop contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
    event MerkleRootHashSet(address indexed drop, uint128 indexed mintId, bytes32 merkleRootHash);
    ...
    /*
     * @dev Sets the `maxMintable` for (`drop`, `mintId`).
     * @param drop               Address of the song drop contract we are minting for.
     * @param mintId                The mint ID.
     * @param maxMintable The maximum number of tokens that can be minted on this schedule.
     */
    ...
    /*
     * @dev Sets the `merkleRootHash` for (`drop`, `mintId`).
     * @param drop        Address of the song drop contract we are minting for.
     * @param mintId         The mint ID.
     * @param merkleRootHash The merkle root hash of the entries.
     */
