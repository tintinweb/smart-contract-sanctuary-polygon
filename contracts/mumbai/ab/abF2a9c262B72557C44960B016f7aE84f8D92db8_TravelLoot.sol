...
contract TravelLoot is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        ...
        parts[0] = '<svg xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 350 350"><style>.base { fill: white; font-family: serif; font-size: 14px; }</style><rect width="100%" height="100%" fill="black" /><text x="45%" y="40" class="base">';

        parts[1] = toString(tokenId);

        parts[2] = '</text><text x="40%" y="80" class="base">';

        parts[3] = getVehicles(tokenId);

        parts[4] = '</text><text x="40%" y="100" class="base">';

        parts[5] = getRarity(tokenId);

        parts[6] = '</text><text x="40%" y="120" class="base">';

        parts[7] = getSpeed(tokenId);

        parts[8] = '</text><text x="40%" y="140" class="base">';

        parts[9] = getSpecialPowers(tokenId);

        parts[10] = '</text><text x="40%" y="160" class="base">';
        ...
        parts[14] = '</text><text x="40%" y="40" class="base">';
