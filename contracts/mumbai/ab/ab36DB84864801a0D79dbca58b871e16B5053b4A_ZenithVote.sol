...
contract ZenithVote is Ownable, ReentrancyGuard{
    ...
    function userWantVote(
        ...
        if(getVoteData(voteId, voteId)[0].amountHold > 0){
            require(
                IERC20(ztv).balanceOf(_msgSender()) >= getVoteData(voteId, voteId)[0].amountHold,
                "ZenithVote : Minimum holding is required!"
            );
        }

        uint256 amountVote = getVoteData(voteId, voteId)[0].amountVote;
    ...
    function getLogVoteData(
        ...
        if (maxSize == 1) {
            ...
            if(maxSize <= latestId){
                for (uint256 i = 1; i <= maxSize; i++) {
                    log[i] = voteLog[voteId][choiceId][latestId - i];
                }
            }else{
    ...
    function getVoteData(
        ...
        if (voteFromId == voteToId) {
            ...
            for (uint256 i = voteFromId; i <= voteToId; i++) {
                proposals[i-(voteFromId)] = userVote[i];
            }
        }
    }
