...
contract EpillioVesting is OwnableUpgradeable,ReentrancyGuardUpgradeable{
    
    using SafeERC20Upgradeable for IERC20Upgradeable;
    // epillo token
       IERC20Upgradeable public epillioToken;
    // usdt Token
    ...
    //mapping initial dates
    mapping(uint=>uint)public initialDates;
    // intermediate dates
    mapping(uint=>uint) public intermediateDates;
    // linear Dates
    mapping(uint=>uint) public LinearDates;
    ...
    uint[] public initialAmountReleased=[0,0,100,100,20,30,0,0,1000];
    uint[] public intermediateAmountReleased=[0,0,0,0,0,0,0,1000,0];
    uint[] public linearVestingAmountReleased=[0,1000,900,900,980,970,0,0,0]; // stores percentage
    uint[] public intermediateVestingTimePeriod=[0,0,0,0,0,0,0,0,0];
    uint[] public linearVestingTimePeriod=[0,120 minutes,100 minutes,130 minutes,270 minutes,270 minutes ,300 minutes ,0,0];

    receive()external payable{}


    function initialize(address _epillioToken,address _usdt, uint _usdtRate,uint tokenRate)external initializer{
        usdtRate=_usdtRate;
        tokenRateMatic=tokenRate;
        epillioToken=IERC20Upgradeable(_epillioToken);
    ...
    function buyEpillio(uint amount, bool isMatic)external payable nonReentrant{
            require(block.timestamp<=ICOEndDate,"Check ICO Dates");
            require (!isBlackListed[msg.sender],'User BlackListed');
            require(msg.value>0,"check balance");
            uint __amount;
            if(isMatic){
                  __amount=msg.value*tokenRateMatic;
                  AvailableMatic+=msg.value;}
            else{
                  __amount=amount*usdtRate;
            ...
            vesting.intermediateToBeClaimed = (intermediateAmountReleased[3] * finalAmount)/ 1000;
    ...
    function getContractTokenBalance() external view returns(uint){
        return epillioToken.balanceOf(address(this));
    }

    function settokenRate(uint tokerate)external onlyOwner{
            tokenRateMatic=tokerate;
    }


    function withdraw() external nonReentrant{
        require(initialDates[InvestorDetails[msg.sender].investorType]>0,"Vesting Not Started For this SaleType");
        ...
        if (returnType == 1) {
            ...
            epillioToken.safeTransfer(msg.sender, amount);
    ...
    function setRewardTokenAddress (address _tokenAddress) public onlyOwner {
        ...
        epillioToken = IERC20Upgradeable(_tokenAddress);
        emit SetRewardToken(msg.sender, _tokenAddress);
    }
    
    function setDates (uint  saleType,uint   initialDate,uint  intermediate, uint  linearDate) external onlyOwner {
        initialDates[saleType]=initialDate;
        intermediateDates[saleType]=intermediate;
        LinearDates[saleType]=linearDate;
        initialVestingAmountWithdrawThresholdTime = [0,initialDates[1],initialDates[2],initialDates[3],initialDates[4],initialDates[5],initialDates[6],initialDates[7],initialDates[8]];
        intermediateVestingAmountWithdrawThresholdTime = [0,0,0,0,0,0,0,intermediateDates[7],0];
        linearVestingAmountWithdrawThresholdTime= [0,LinearDates[1] ,LinearDates[2],LinearDates[3],LinearDates[4]  ,LinearDates[5],LinearDates[6],0,0 ];
    ...
    function intermediateVestStatus(address _userAddress) public view returns (uint,uint) {
        ...
        uint timeDifference;
        // uint[11] memory intermediateArray = [0,0,intermediateDates[2],intermediateDates[3],0,0,0,intermediateDates[7],intermediateDates[8],0,0];// give Intermediate time
        
        if (block.timestamp <= intermediateVestingTimePeriod[InvestorDetails[_userAddress].investorType]+intermediateDates[InvestorDetails[_userAddress].investorType])
        {
            if(!isIntermediateClaimed[_userAddress]){
             timeDifference=block.timestamp-intermediateDates[InvestorDetails[_userAddress].investorType];
            }else{
    ...
    function linearVestingDetails(address _userAddress) public view returns (uint) {
        uint lastClaimTime = InvestorDetails[_userAddress].lastClaimTime;
        uint timeDifference;
        // uint[11] memory linearTime =[0,globalStartTime+ 60 minutes ,globalStartTime + 1 minutes ,globalStartTime + 1 minutes,globalStartTime +  1 minutes ,globalStartTime + 180 minutes,globalStartTime + 365 minutes,globalStartTime + 30 minutes, globalStartTime + 90 minutes,globalStartTime + 1 minutes,0];
        if (block.timestamp <= linearVestingTimePeriod[InvestorDetails[_userAddress].investorType]+LinearDates[InvestorDetails[_userAddress].investorType]) {
             if(!isLinearClaimed[_userAddress]){

                    timeDifference=block.timestamp-LinearDates[InvestorDetails[_userAddress].investorType];
             }else{
                 timeDifference = block.timestamp - lastClaimTime;
             }
        }
    ...
    function getLinearVestEndTime(uint256 saletype) public view returns(uint){
        uint256 endminutes=linearVestingAmountWithdrawThresholdTime[saletype]+linearVestingTimePeriod[saletype];
        return endminutes;  
    }
    ...
    function extractMatic(uint amount) public onlyOwner {
     ...
     function transferToken(address ERC20Address, uint256 value) public onlyOwner {
        ...
        if(epillioToken==IERC20Upgradeable(ERC20Address)){
    ...
    function removeSingleUser(address _userAddress)public onlyOwner{
        ...
        require(initialDates[InvestorDetails[_userAddress].investorType]==0,"Vesting Time Started");
    ...
    function depositEpillio(uint amount)external onlyOwner{
         totalDepositedTokens+=amount;
         AvailableTokens+=amount;
         epillioToken.safeTransferFrom(msg.sender,address(this), amount);
    }
