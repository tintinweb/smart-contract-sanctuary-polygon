...
contract FortuneCookie is ERC721Enumerable, Ownable {
    ...
    function numStandardMessages() external view returns(uint){
    ...
    function generateSVG() public view returns(string memory){
        bytes memory svg = abi.encodePacked(cookieSVG);
        return string(
            abi.encodePacked(
    ...
    function tokenURI(uint256 token) public override view returns (string memory){
        ...
        bytes memory dataURI = abi.encodePacked(
            '{\n',
                ...
                '\t"description": "','A crisp fortune cookie, with a secret message inside.','",\n',
                '\t"image_data": "', string(generateSVG()), '",\n',
                '\t"attributes": [ \n',
                string.concat('\t\t{ "trait_type": "# of Times Read", "value": ', Strings.toString(nft.reads),', "display_type": "number" },\n',
    ...
    function _setToken(uint256 tokenID, NFT memory token) private {
        ...
        tokens[tokenID].read = token.read;
    }
	...
	{
        require(!pauseMint, 'minting is currently paused');
    ...
    function open(uint256 tokenID) external returns (string memory){
        ...
        require(tokens[tokenID].read == false, 'This fortune cookie has already been read');
        tokens[tokenID].read = true;
        tokens[tokenID].reads++;
        reads++;
        return tokenSecretMessages[tokenID];
    }
    function read(uint256 tokenID) external returns (string memory){
        require(tokens[tokenID].minted == true, 'This token does not exist');
        require(ownerOf(tokenID) == msg.sender, 'You must be the owner this token to read the message');
        require(tokens[tokenID].read == true, 'You must crack open the fortune cookie before you can read the message');
        tokens[tokenID].reads++;
        reads++;
        return tokenSecretMessages[tokenID];
    }
