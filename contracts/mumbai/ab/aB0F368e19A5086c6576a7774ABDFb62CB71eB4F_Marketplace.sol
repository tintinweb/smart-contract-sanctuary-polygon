...
abstract contract Protected is Ownable, EIP712 {
    ...
    bytes32 internal constant _MINT_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyWithMintParams(address seller,uint256 listingId,uint256 tokenId,uint256 totalEditions,string meta,address currency,uint96 feeRate,uint256 price,uint256 startTime,uint256 endTime,address royaltyReceiver,uint96 royalty)"
        );
    bytes32 internal constant _BUY_TYPEHASH =
        // prettier-ignore
        keccak256(
            "BuyParams(address seller,address tokenAddress,uint256 listingId,uint256 tokenId,uint256 totalEditions,address currency,uint96 feeRate,uint256 price,uint256 startTime,uint256 endTime)"
        );
    bytes32 internal constant _MINT_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellWithMintParams(address buyer,uint256 offerId,uint256 tokenId,uint256 totalEditions,string meta,address currency,uint96 feeRate,uint256 price,uint256 endTime,address royaltyReceiver,uint96 royalty)"
        );
    bytes32 internal constant _BUY_OFFER_TYPEHASH =
        // prettier-ignore
        keccak256(
            "SellParams(address buyer,address tokenAddress,uint256 offerId,uint256 tokenId,uint256 totalEditions,address currency,uint96 feeRate,uint256 price,uint256 endTime)"
        );

    bytes32 internal constant _PLATFORM_TYPEHASH =
        keccak256(
            "PlatformParams(address receiver,uint256 editions,bytes sellerSignature)"
        );
    bytes32 internal constant _PLATFORM_OFFER_TYPEHASH =
        keccak256(
            "PlatformOfferParams(address seller,uint256 editions,bytes buyerSignature)"
        );
...
contract Marketplace is Protected, Pausable {
    ...
    function buyWithMint(
        ...
        uint256 payment = mintData.price * platformData.editions;
        _checkMoney(_msgSender(), mintData.currency, payment);
        console.log(platformData.receiver);
    ...
    function buy(
        ...
        uint256 payment = buyData.price * platformData.editions;
    ...
    function sell(
        ...
        external
        // onlySeller(platformData.seller)
        whenNotPaused
    {
        if (isSeller(platformData)) revert NotASeller();
        ...
        if (
            ...
            ICounter(counter).initOffer(buyData.offerId, buyData.totalEditions);
        }
        ...
        ICounter(counter).decreaseOffer(buyData.offerId, platformData.editions);
    ...
    function isSeller(
        ...
        return _msgSender() == platformData.seller;
    }
    ...
    function _acceptBidWithMint(
        ...
        if (mintData.totalEditions != 1) revert TokenIsNotUnique();
    ...
    function _acceptBid(
        ...
        if (buyData.totalEditions != 1) revert TokenIsNotUnique();
