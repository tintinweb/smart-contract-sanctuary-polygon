...
// File: project/contracts/ZipGovernance.sol


pragma solidity 0.8.17;


contract ZipGovernance is Ownable {
...
// File: project/contracts/ZipDAO.sol
...
contract ZipDAO is ZipGovernance, ReentrancyGuard {
    ...
    uint256 public releaseStep = 60 seconds;  // seconds, minutes, hours, days, weeks, years
    uint256 public scheduleStartTime;

    // CategoryID => releaseSchedules Array
    mapping(uint256 => uint256[24]) public releaseSchedules;
    ...
    address[] public beneficiaryAdrs;
    ...
    //-----------------------------------------------
    /*
    |- 1. Each beneficiary to be able to withdraw released tokens
    ...
    |
    |- 2. Owner to be able to call Emergency_Shutdown
    |
    */
    // votes[Voter][Beneficiary][Slot] -> Vote
    mapping(address => mapping(address => mapping (uint256 => Vote))) internal votes;
    
    // slotConfirms[beneficiary] -> Weights[]
    mapping(address => uint256[24]) internal slotConfirms;
    ...
    function addCategory(uint8 catId, string memory catTitle, uint16 _share) external onlyOwner {
        require(catId > 0, "Category ID has to be greater than zero");
        require(_share > 0, "Share has to be greater than zero");
        
        require(keccak256(abi.encode(catTitle)) != keccak256(abi.encode("")), "Category title has to be nonempty");

        catIds.push(catId);
        categories[catId] = Category({title: catTitle, shareOfTotalSup: _share});
    }


    function addSchedule(uint8 catId, uint8[24] memory _schedule) external onlyOwner {
        require(catId > 0, "Category ID has to be greater than zero");
        releaseSchedules[catId] = _schedule;
    }
    ...
    function start(address _zicTokenAdr) external onlyOwner {
        console.log("_zicTokenAdr: ", _zicTokenAdr);
    ...
    function emergencyShutdown() external onlyOwner nonReentrant {
        zic.transfer(msg.sender, zic.balanceOf(address(this)));
        selfdestruct(payable(msg.sender));
    }


    /**
     * @dev Allows the contract owner (deployer) to withdraw the ETH from the contract
     */
    ...
    /**
     * @dev Beneficiary call this function to withdraw released tokens.
     */
    ...
    /**
     * @dev Get the amount of tokens that beneficiary can withdraw now, 
     * base on release schedule of the category.
     */
...
//------------------------------------------------------------
// TODO : Edit beneficiary wallet (only for team members)
//------------------------------------------------------------
// a- Beneficiary send request to change the wallet address
//      1- Beneficiary request
//      2- Admin accept and verify (ecrecover)
// b- consider a recovery wallet and activate it by admin, whenever beneficiary request
//

//------------------------------------------------------------
// TODO : Admin can Pause functions `register` and `withdraw`
//------------------------------------------------------------