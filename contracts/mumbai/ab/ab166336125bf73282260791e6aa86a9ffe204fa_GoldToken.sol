...
contract GoldToken is ERC721Enumerable, Ownable, ERC721Burnable, ERC721Pausable, Applications {
    ...
    function mintPrice(uint256 _count,uint256 _collectionId) public view returns(uint256[] memory) {
        ...
        uint256[] memory _maintPrices = calculateMaintenance(_count,1); // 15 yers = 5475
    ...
    function compiledAttributes (uint256 _tokenId) internal view returns (string memory) {
        return string(abi.encodePacked(
            '"CreationDate":"',uint2str(GoldTokenDetails[_tokenId].creationDate),'",',
            '"Weight":"',uint2str(GoldTokenDetails[_tokenId].weight),'",',
            '"CollectionId":"',uint2str(GoldTokenDetails[_tokenId].collectionId),'",',
            '"TokenActive":"',GoldTokenDetails[_tokenId].active ? "TRUE" : "FALSE",'",',
            '"EntireCollection":"',isEntireCollection(GoldTokenDetails[_tokenId].weight, GoldCollections[GoldTokenDetails[_tokenId].collectionId].weight),'",',
            '"Purity":"',uint2str(GoldCollections[GoldTokenDetails[_tokenId].collectionId].purity),'",'
        ));
    }
