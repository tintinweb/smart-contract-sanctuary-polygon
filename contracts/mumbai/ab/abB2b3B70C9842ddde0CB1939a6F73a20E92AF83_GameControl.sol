...
contract GameControl is Ownable, ReentrancyGuard {
    ...
    mapping(uint => uint) rewards;// or banishment || when user reach level out, will be reward, vs banishment
    ...
    // Game hooks ///////////////////////////////////////////////////////////////////////////////
    function entryFeeDeposit(uint amount) external {
    ...
    function levelReward(uint levelNo, bool levelStatus) external {
        require(!paused, "game is paused");
        require(entryStatus[msg.sender], "You didn't pay entry fee yet!");
        require(playerInfo[msg.sender].levelNo + 1 >= levelNo, "Game level is not correct");
        require(playerInfo[msg.sender].claimAble, "Already claimed");
        uint rewardAmount = rewards[levelNo];
        uint contractBalance = IERC20(TokenContract).balanceOf(address(this));
        if (levelStatus) {
            require(contractBalance >= rewardAmount, "reward amount exceed");
            ...
            uint devTax = (rewardAmount * devFee) / percentRate;
            uint burnTax = (rewardAmount * burnFee) / percentRate;
            IERC20(TokenContract).safeTransferFrom(msg.sender, DevWallet, devTax);
            IERC20(TokenContract).safeTransferFrom(msg.sender, BurnAddress, burnTax);
            playerInfo[msg.sender] = levelInfo(1, true, false, false, false);
        }
    ...
    function revival(uint levelNo) external {
        require(!paused, "game is paused");
        require(entryStatus[msg.sender], "You didn't pay entry fee yet!");
        uint revivalTax = rewards[levelNo] * revivalFee;
