...
contract MarketplaceV1 is  Initializable, ERC721Holder, OwnableUpgradeable {
   ...
   */
  function initialize (address _card, address payable _daoAddress, uint256 _daoFee, uint8 _minOfferGuaranteeHours) public initializer{
...
/**
  ...
  event MinOfferGuaranteeHoursChanged(
    address by,
    uint256 oldMinOfferGuaranteeHours,
    uint256 newMinOfferGuaranteeHours
  );
...
* @dev Fired in cancelOfferOnBuyOrder()
  ...
  function changeFees(uint256 _daoFee) external onlyOwner {
    require( _daoFee >= 0 && _daoFee <= 100_000, "Invalid fee value");
   ...
   */
  function _takeFeesFrom(uint256 _amount, address _collectionAddress, address payable _collectionPartnerDaoAddress) internal returns (uint256) {
    // Get marketplaceFee using smart contract address
    uint marketplaceFee = ICollection(_collectionAddress).marketplaceFeePercent();
    uint256 marketplaceFeeValueWithDiscount = 0;
    if (marketplaceFee > 0) {
      //Send % to dao
        uint256 marketplaceFeeValue = _amount * marketplaceFee / 100_000;
        //marketplaceFeeValueWithDiscount = ICollection(_collectionAddress).checkDiscount(marketplaceFeeValue);
        marketplaceFeeValueWithDiscount = marketplaceFeeValue;
  ...
  */
  function cancelSellOrder(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(sellOrder.highestOfferor != address(0) && !sellOrder.highestOfferor.isContract()) {
      // Refund offeror
       (bool success, ) = payable(sellOrder.highestOfferor).call{value: sellOrder.highestOffer}("");
  ...
  */
  function acceptSellOrderHighestOffer(bytes32 _orderId) external sellOrderExists(_orderId) senderOwnsSellOrder(_orderId) {
    ...
    if(!sellOrder.owner.isContract() && sellOrder.highestOffer > 0) {
      ...
      // (fees are paid to DAO and reserve in the _takeFeesFrom function)
     (bool success, ) =  payable(sellOrder.owner).call{value: _takeFeesFrom(sellOrder.highestOffer, collectionAddress, partnerDaoAddress)}("");
  ...
  function fulfillSellOrder(bytes32 _orderId) external payable onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    if(!sellOrder.owner.isContract()) {
      // Transfer Ether amount without fees to sell order owner
      // (fees are paid to DAO and Partner in the _takeFeesFrom function)
      (bool success, ) = payable(sellOrder.owner).call{value: _takeFeesFrom(sellOrder.highestOffer, collectionAddress, partnerDaoAddress)}("");
      require(success, "Transfer to sell order owner failed.");
    }
...
function buySellOrder(bytes32 _orderId) external payable onlyIfSenderNotContract sellOrderExists(_orderId) {
    ...
    if(!sellOrder.owner.isContract()) {
      // Transfer Ether amount without fees to sell order owner
      // (fees are paid to DAO and Partner in the _takeFeesFrom function)
      (bool success, ) = payable(sellOrder.owner).call{value: _takeFeesFrom(msg.value, collectionAddress, partnerDaoAddress)}("");
      require(success, "Transfer to sell order owner failed.");
    }
 ...
 * @notice Cancelles an offer on a buy order
  ...
  function acceptBuyOrderLowestOffer(bytes32 _orderId) external buyOrderExists(_orderId) senderOwnsBuyOrder(_orderId) {
    ...
    if(!buyOrder.lowestOfferor.isContract()) {
      ...
      // (fees are paid to DAO and Partner in the _takeFeesFrom function)
      (bool success, ) = payable(buyOrder.lowestOfferor).call{value: _takeFeesFrom(buyOrder.lowestOffer, collectionAddress, partnerDaoAddress)}("");
  ...
  function fulfilBuyOrder(bytes32 _orderId, uint256 _cardId) external onlyIfSenderNotContract buyOrderExists(_orderId) {
    ...
    // Transfer Ether amount without fees to seller
    // (fees are paid to DAO and reserve in the _takeFeesFrom function)
     (bool success, ) =  payable(msg.sender).call{value: _takeFeesFrom(buyOrder.bid, collectionAddress, partnerDaoAddress)}("");
    require(success, "Transfer to sell order owner failed.");
    ...
    if(buyOrder.lowestOfferor != address(0) && !buyOrder.lowestOfferor.isContract()) {
      // Refund previous offeror
      (bool success, ) = payable(buyOrder.lowestOfferor).call{value: buyOrder.lowestOffer}("");
      require(success, "Refund previous offeror failed.");
    }
