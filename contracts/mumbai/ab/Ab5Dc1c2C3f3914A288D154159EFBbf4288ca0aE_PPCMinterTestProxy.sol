...
contract PPCMinterTestProxy is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    function initialize(address _nftAdmin) external initializer {
    ...
    function trade(uint256 _noofnfts) public payable nonReentrant {
        ...
        uint256 nftidend = nftidstart + _noofnfts;
        assert(nftidend <= redeemendrange);
        claimIndex += _noofnfts;

        for (uint256 i = nftidstart; i < nftidend; i++) {
            nft.safeTransferFrom(nftAdmin, msg.sender, i);
            emit Claim(msg.sender, i);
        }
    }

    function remainigContribution(address _owner)
        public
        view
        returns (uint256)
    {
    ...
    function changeMaxInvestment(uint256 _newmax) external onlyOwner {
    ...
    function checkSpecialNFT(address _user) public view returns (bool) {
        return (IERC721Upgradeable(specialNftadd1).balanceOf(_user) > 0 ||
            IERC721Upgradeable(specialNftadd2).balanceOf(_user) > 0);
    }
    ...
    function initializeICO(
        uint256 _price,
        address _nftaddress,
        address _specialnftadd1,
        address _specialnftadd2,
        uint256 _icotarget,
        uint256 _maxinvestment,
        uint256 _nftstartingrange,
        uint256 _nftendrange
    ) public onlyOwner {
