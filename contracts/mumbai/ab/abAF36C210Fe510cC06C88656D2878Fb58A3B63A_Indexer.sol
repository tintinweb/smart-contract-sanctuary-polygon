...
contract Indexer is Initializable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    function getCourse(uint256 institutionId, string memory courseId) public view returns (Institution.Course memory) {
        return getInstitutionContract(institutionId).getCourse(courseId);
    }

    function getStudentId(uint256 institutionId, string memory courseId, address student) public view returns (uint256) {
        return getInstitutionContract(institutionId).getStudentId(courseId, student);
    }

    function getStudentURI(uint256 institutionId, string memory courseId, address student) public view returns (string memory) {
...
contract Institution is Initializable, PausableUpgradeable, AccessControlUpgradeable {
    ...
    mapping(string => mapping(address => Student)) public students;
    mapping(string => mapping(uint256 => address)) public studentsIndexToAddress;
    ...
    // Courses management
    function registerCourse(string memory courseId, string memory courseBaseURI) public onlyRole(MINTER_ROLE) {
        require(studentsERC721 != address(0), "StudentsERC721 not defined");
        require(!courses[courseId].exists, "Course id must be unique");
    ...
    function unregisterCourse(string memory courseId) public onlyRole(MINTER_ROLE) onlyActiveCourse(courseId) {
        courses[courseId].isActive = false;
        activeCourseCounter.decrement();

        emit CourseUnregistered(courseId);
    }

    function getCourse(string memory courseId) public view returns (Course memory) {
        return courses[courseId];
    }
    
    // Student management
    function registerStudent(string memory courseId, address student) 
    public 
    ...
    nonZero(student) {
        ...
        studentsIndexToAddress[courseId][courses[courseId].studentsBalance] = student;
    ...
    function unregisterStudent(string memory courseId, address student) public onlyRole(MINTER_ROLE) onlyActiveStudent(courseId, student) {
    ...
    function getStudentId(string memory courseId, address student) public view onlyActiveStudent(courseId, student) returns (uint256) {
        return students[courseId][student].id;
    }

    function getStudentURI(string memory courseId, address student) public view onlyActiveStudent(courseId, student) returns (string memory) {
    ...
    function getAllStudentIds(address student) public view returns (uint256[] memory) {
        ...
        for (uint i = 0; i < coursesIds.length; i++) {
            string memory courseId = coursesIds[i];
            if (students[courseId][student].isActive) {
                tokensIds[tokenIndex] = students[courseId][student].id;
                tokenIndex += 1;
            }
        }
        return tokensIds;
    ...
    modifier onlyActiveCourse(string memory courseId) {
        require(courses[courseId].isActive, "Course inactive");
        _;
    }

    modifier onlyActiveStudent(string memory courseId, address studentId) {
        require(students[courseId][studentId].isActive, "Student not registered");
        _;
    }

    modifier onlyInactiveStudent(string memory courseId, address studentId) {
