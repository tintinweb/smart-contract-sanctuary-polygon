...
import "./TrustedController.sol";


/**
 * @dev A registrar controller for registering and renewing names at fixed cost.
 */
contract RealRegistrarEntry is Ownable, IERC165 {

    uint64 private constant MAX_EXPIRY = type(uint64).max;
    TrustedController immutable base;
    mapping(address => bool) public operators;

    event SetOperator(address operator, bool isOperator);

    constructor(TrustedController _base, address _operator) {
        base = _base;
    ...
    function register (
        ...
        base.register(name, owner, MAX_EXPIRY, resolver);
    }
    ...
    function batchRegister(
        ...
        require(names.length == owners.length, "size not match");
        uint256 len = names.length;
        for (uint256 i; i < len; i++) {
            base.register(names[i], owners[i], MAX_EXPIRY, resolver);
        }
    }
...
contract TrustedController is ERC721Enumerable, IBaseRegistrar, ReverseClaimer, Ownable {
    ...
    ) ERC721(_name, _symbol) ReverseClaimer(_ens, msg.sender) {
        ens = _ens;
        baseNode = _baseNode;
        baseName = _baseName;
        reverseRegistrar = _reverseRegistrar;
    ...
    function transferFrom(
        ...
        _transfer(from, to, tokenId);
        _configResolver(from, to, tokenId);
    }

    function _configResolver(address from, address to, uint256 tokenId) internal {
        ...
        address resolver = ens.resolver(nodehash);
        Resolver(resolver).setAddr(nodehash, to);
        reverseRegistrar.setNameForAddr(to, to, resolver, string.concat(names[tokenId], baseName));
    ...
    function register (
        ...
        _mint(owner, id);
        ens.setSubnodeOwner(baseNode, bytes32(id), address(this));
        bytes32 nodehash = keccak256(abi.encodePacked(baseNode, label));
        ens.setResolver(nodehash, resolver);
        Resolver(resolver).setAddr(nodehash, owner);
        reverseRegistrar.setNameForAddr(owner, owner, resolver, string.concat(name, baseName));
