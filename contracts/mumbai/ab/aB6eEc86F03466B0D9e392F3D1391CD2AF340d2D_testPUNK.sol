...
contract testPUNK is ERC721r {
    ...
    function _baseURI() override internal view virtual returns (string memory)  {
        return "https://gateway.pinata.cloud/ipfs/QmU75pGEju2ezPvkRjrUrNxzvU5MUrmkn9YE8GcaAQV7CW/";
    }
}
...
contract ERC721r is Context, ERC165, IERC721, IERC721Metadata {
    ...
    // BaseURI Extension for .json metadata
     ...
     */
    constructor(string memory name_, string memory symbol_, uint _maxSupplies) {
        _name = name_;
        _symbol = symbol_;
        _maxSupply = _maxSupplies;
        _numAvailableTokens = _maxSupplies;
    }
     ...
     * token will be the concatenation of the `baseURI` and the `_tokenId`. Empty
    ...
    function approve(address to, uint256 _tokenId) public virtual override {
        address owner = ERC721r.ownerOf(_tokenId);
    ...
    /**
     * @dev Safely transfers `_tokenId` token from `from` to `to`, checking first that contract recipients
    ...
    /**
     * @dev Returns whether `spender` is allowed to manage `_tokenId`.
     *
     * Requirements:
     *
     * - `_tokenId` must exist.
    ...
    function _mintIdWithoutBalanceUpdate(address to, uint256 _tokenId) private {
    ...
    function _mintRandom(address to, uint _numToMint) internal virtual {
        ...
        for (uint256 i; i < _numToMint; ++i) { // Do this ++ unchecked?
            uint256 _tokenId = getRandomAvailable_tokenId(to, updatedNumAvailableTokens);
            
            _mintIdWithoutBalanceUpdate(to, _tokenId);
    ...
    function getRandomAvailable_tokenId(address to, uint updatedNumAvailableTokens)
        internal
        returns (uint256)
    {
    ...
    // Not as good as minting a specific _tokenId, but will behave the same at the start
    ...
    function _mintAtIndex(address to, uint index) internal virtual {
        ...
        uint _tokenId = getAvailableTokenAtIndex(index, _numAvailableTokens);
        --_numAvailableTokens;
    ...
    /**
     * @dev Transfers `_tokenId` from `from` to `to`.
    ...
    function _transfer(
        ...
        require(ERC721r.ownerOf(_tokenId) == from, "ERC721: transfer from incorrect owner");
    ...
    function _approve(address to, uint256 _tokenId) internal virtual {
        ...
        emit Approval(ERC721r.ownerOf(_tokenId), to, _tokenId);
    }
     ...
     * Calling conditions:
     *
     * - When `from` and `to` are both non-zero, ``from``'s `_tokenId` will be
     * transferred to `to`.
     * - When `from` is zero, `_tokenId` will be minted for `to`.
     * - When `to` is zero, ``from``'s `_tokenId` will be burned.
