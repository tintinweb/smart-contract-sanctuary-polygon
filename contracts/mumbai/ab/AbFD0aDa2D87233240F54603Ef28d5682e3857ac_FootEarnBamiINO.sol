...
contract FootEarnBamiINO is Ownable, ReentrancyGuard, IERC721Receiver {
    ...
    function claimNft(uint256 _nftType) public isRun validClaimTime nonReentrant{
        require(inWhiteList[msg.sender] == true, "You are ot in whitelist");
        require(isClaimed[_nftType][msg.sender] == false, "You have claimed already");

        address nft = nftTypeMap[_nftType].nftAddress;
        require(IERC721(nft).balanceOf(address(this)) >= userBoughtNft[msg.sender][_nftType], "Not enough NFT");

        for(uint256 jndex = 0; jndex < userBoughtNft[msg.sender][_nftType]; jndex++){
            uint256 tokenId = ERC721Enumerable(nft).tokenOfOwnerByIndex(address(this), 0);

            IERC721(nft).transferFrom(address(this), msg.sender, tokenId);

            emit TransferComplete(msg.sender, _nftType, tokenId);
        }

        isClaimed[_nftType][msg.sender] = true;
