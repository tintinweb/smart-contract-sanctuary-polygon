...
contract PancakeZapV1 is Ownable, ReentrancyGuard {
     ...
     */
    function zapInBNB(address _lpToken, uint256 _tokenAmountOutMin, bool isFeeEnabled) external payable nonReentrant {
        WBNB.deposit{value: msg.value}();

        // Call zap function
        uint256 lpTokenAmountTransferred = _zapIn(WBNBAddress, msg.value, _lpToken, _tokenAmountOutMin, isFeeEnabled);
     ...
     */
    function zapInTokenRebalancing(zapTokenRebalancing calldata _zapTokenRebalancing) external nonReentrant {
        // Transfer tokens to this contract
        IERC20(_zapTokenRebalancing._token0ToZap).safeTransferFrom(
            msg.sender,
            address(this),
            _zapTokenRebalancing._token0AmountIn
        );
        IERC20(_zapTokenRebalancing._token1ToZap).safeTransferFrom(
            msg.sender,
            address(this),
            _zapTokenRebalancing._token1AmountIn
        );

        // Call zapIn function
        uint256 lpTokenAmountTransferred = _zapInRebalancing(_zapTokenRebalancing);

        // Emit event
        emit ZapInRebalancing(
            _zapTokenRebalancing._token0ToZap,
            _zapTokenRebalancing._token1ToZap,
            _zapTokenRebalancing._lpToken,
            _zapTokenRebalancing._token0AmountIn,
            _zapTokenRebalancing._token1AmountIn,
            lpTokenAmountTransferred,
            msg.sender
    ...
    function zapInBNBRebalancing(
        ...
        zapTokenRebalancing memory _zapTokenRebalancing = zapTokenRebalancing(
            WBNBAddress,
            _token1ToZap,
            msg.value,
            _token1AmountIn,
    ...
    function estimateZapInSwap(
        ...
        if (token0 == _tokenToZap) {
            swapTokenOut = token1;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveA, reserveB, isFeeEnabled);
            swapAmountOut = pancakeRouter.getAmountOut(swapAmountIn, reserveA, reserveB, isFeeEnabled);
        } else {
            swapTokenOut = token0;
            swapAmountIn = _calculateAmountToSwap(_tokenAmountIn, reserveB, reserveA, isFeeEnabled);
            swapAmountOut = pancakeRouter.getAmountOut(swapAmountIn, reserveB, reserveA, isFeeEnabled);
        }
    ...
    function estimateZapInRebalancingSwap(
        ...
        address _lpToken,
        bool isFeeEnabled
    ) external view returns (uint256 swapAmountIn, uint256 swapAmountOut, bool sellToken0) {
        require(
    ...
    function _zapInRebalancing(
        zapTokenRebalancing memory _zapTokenRebalancing
    ) internal returns (uint256 lpTokenReceived) {
        require(
            _zapTokenRebalancing._token0ToZap == IPancakePair(_zapTokenRebalancing._lpToken).token0() ||
                _zapTokenRebalancing._token0ToZap == IPancakePair(_zapTokenRebalancing._lpToken).token1(),
            'Zap: Wrong token0'
        );
        require(
            _zapTokenRebalancing._token1ToZap == IPancakePair(_zapTokenRebalancing._lpToken).token0() ||
                _zapTokenRebalancing._token1ToZap == IPancakePair(_zapTokenRebalancing._lpToken).token1(),
            'Zap: Wrong token1'
        );

        require(_zapTokenRebalancing._token0ToZap != _zapTokenRebalancing._token1ToZap, 'Zap: Same tokens');
        ...
        {
            // Convert to uint256 (from uint112)
            (uint256 reserveA, uint256 reserveB, ) = IPancakePair(_zapTokenRebalancing._lpToken).getReserves();

            require((reserveA >= MINIMUM_AMOUNT) && (reserveB >= MINIMUM_AMOUNT), 'Zap: Reserves too low');

            if (_zapTokenRebalancing._token0ToZap == IPancakePair(_zapTokenRebalancing._lpToken).token0()) {
                swapAmountIn = _calculateAmountToSwapForRebalancing(
                    _zapTokenRebalancing._token0AmountIn,
                    _zapTokenRebalancing._token1AmountIn,
                    reserveA,
                    reserveB,
                    _zapTokenRebalancing._isToken0Sold,
                    _zapTokenRebalancing.isFeeEnabled
                );
                require(reserveA / swapAmountIn >= maxZapReverseRatio, 'Zap: Quantity higher than limit');
            } else {
                swapAmountIn = _calculateAmountToSwapForRebalancing(
        ...
        require(swapAmountIn <= _zapTokenRebalancing._tokenAmountInMax, 'Zap: Amount to swap too high');

        address[] memory path = new address[](2);

        // Define path for swapping and check whether to approve token to sell in intermediary swap
        if (_zapTokenRebalancing._isToken0Sold) {
            path[0] = _zapTokenRebalancing._token0ToZap;
            path[1] = _zapTokenRebalancing._token1ToZap;
            _approveTokenIfNeeded(_zapTokenRebalancing._token0ToZap, swapAmountIn);
        } else {
            path[0] = _zapTokenRebalancing._token1ToZap;
            path[1] = _zapTokenRebalancing._token0ToZap;
            _approveTokenIfNeeded(_zapTokenRebalancing._token1ToZap, swapAmountIn);
        }

        // Execute the swap and retrieve quantity received
        uint256[] memory swapedAmounts = pancakeRouter.swapExactTokensForTokens(
            swapAmountIn,
            _zapTokenRebalancing._tokenAmountOutMin,
            path,
            address(this),
            block.timestamp,
            _zapTokenRebalancing.isFeeEnabled
        ...
        if (_zapTokenRebalancing._isToken0Sold) {
            ...
            (, , lpTokenReceived) = pancakeRouter.addLiquidity(
                path[0],
                path[1],
                (_zapTokenRebalancing._token0AmountIn - swapedAmounts[0]),
                (_zapTokenRebalancing._token1AmountIn + swapedAmounts[1]),
                1,
                1,
                msg.sender,
                block.timestamp
            ...
            (, , lpTokenReceived) = pancakeRouter.addLiquidity(
                path[0],
                path[1],
                (_zapTokenRebalancing._token1AmountIn - swapedAmounts[0]),
                (_zapTokenRebalancing._token0AmountIn + swapedAmounts[1]),
                1,
                1,
                msg.sender,
                block.timestamp
    ...
    function _calculateAmountToSwap(
        ...
        uint256 nominator = pancakeRouter.getAmountOut(halfToken0Amount, _reserve0, _reserve1, isFeeEnabled);
    ...
    function _calculateAmountToSwapForRebalancing(
        ...
        if (sellToken0) {
            ...
            uint256 nominator = pancakeRouter.getAmountOut(token0AmountToSell, _reserve0, _reserve1, isFeeEnabled);
            uint256 denominator = pancakeRouter.quote(
                token0AmountToSell,
            ...
            uint256 nominator = pancakeRouter.getAmountOut(token1AmountToSell, _reserve1, _reserve0, isFeeEnabled);
