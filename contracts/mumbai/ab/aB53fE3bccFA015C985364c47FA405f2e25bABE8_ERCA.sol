...
*/
        ...
        contract ERCA is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
            ...
            constructor(uint256 _price, uint256 __maxSupply, string memory _initBaseURI, uint256 _maxMintAmountPerTx, uint256 _maxMintAmountPerWallet, string memory _initNotRevealedUri) ERC721A("ERCA", "ERCA") {
        ...
        modifier whitelistMintPriceCompliance(bytes32[] calldata _merkleProof, uint256 _mintAmount) {
          ...
          function updateHashRootForAll(bytes32[] memory hashRootArray, uint256[] memory PriceArray, uint256[] memory  MaxMintArray)  public { 
            require(hashRootArray.length <= 0, "Too many inputs on the hash root!");
            require(PriceArray.length <= 0, "Too many inputs on the Price!");
            require(MaxMintArray.length <= 0, "Too many inputs on the Max mint amount!");
