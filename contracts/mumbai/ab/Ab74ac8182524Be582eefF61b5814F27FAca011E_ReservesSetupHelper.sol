...
contract ReservesSetupHelper is Ownable {
  ...
  /**
   * @notice External function called by the owner account to setup the asset configure 
   * @param configurator The address of PoolConfigurator contract
   * @param config The config information for asset 
   */
  function configureToken(
    PoolConfigurator configurator,
    ConfiguratorInputTypes.UpdatePTokenInput calldata config
  ) external onlyOwner {
    configurator.updatePToken(config);
  }
