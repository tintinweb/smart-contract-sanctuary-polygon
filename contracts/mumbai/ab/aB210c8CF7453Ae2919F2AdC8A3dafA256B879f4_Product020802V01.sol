...
contract Product020802V01 is OwnableV01 {
    ...
    uint256 private totalPolicies;
    mapping(string => Product020802Lib.Policy) private allPolicies;
    mapping(string => bool) private isPolicyExists;
    ...
    event NewPolicyCreated(
        ...
        string indexed onChainPolicyId // To verify
    );
    ...
    function endorsePolicy(
        ...
        Product020802Lib.Policy storage _policy = allPolicies[endorsementParams.onChainPolicyId];// can we remove the policy storage in the end??

        Product020802Lib.endorsePolicy(
            endorsementParams,
            _policy,
            Product020802Lib.EndorsePolicyLibParams(TIME_ZONE_OFFSET, updateSelectedOptions, updateCoverages)
        );

        _policy.operationType = endorsementParams.operationType;
        _policy.operationEffectiveDate = block.timestamp;
    }
...
library Product020802Lib {
    ...
    function endorsePolicy(
        ...
        EndorsePolicyLibParams memory endorsePolicyLibParams
    ) external {
        checkEndorsePolicyBusinessRules(endorsementParams, policy.operationType);

        if (endorsementParams.requestedCoverageStartDate != 0) {
            uint256 coverageStartDate = computeCoverageStartDate(
                endorsementParams.requestedCoverageStartDate,
                policy.subscriptionDate,
                endorsePolicyLibParams.timezoneOffset
            );
            policy.coverageStartDate = coverageStartDate;
            policy.expirationDate = coverageStartDate.addYears(1, endorsePolicyLibParams.timezoneOffset);
        }
        ...
        if (endorsePolicyLibParams.updateSelectedOptions) {
            policy.selectedOptions = endorsementParams.selectedOptions;
        }
        if (endorsePolicyLibParams.updateCoverages) {
