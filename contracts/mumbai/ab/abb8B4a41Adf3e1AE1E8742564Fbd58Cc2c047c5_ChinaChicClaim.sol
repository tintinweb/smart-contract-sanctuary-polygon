...
contract ChinaChicClaim is Ownable, Pausable, ReentrancyGuard {
    ...
    function claim(
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            uint256 claimedDeposit = claimed[tokenIds[i]];
            if (claimedDeposit < totalDeposit) {
                uint256 eachRemaining = totalDeposit - claimedDeposit;
                remaining += eachRemaining;
                claimed[tokenIds[i]] = totalDeposit;
            }
        }
