...
contract CrossmintContract is ERC721A, Ownable, Pausable, PaymentSplitter {
  ...
  mapping(address => uint256) private _crossmintRelays;
  ...
  function earlyPurchaseHelper(address to, uint256 count) internal {
  ...
  modifier onlyCrossmint() {
    require(_crossmintRelays[msg.sender] == 1, "Invalid Crossmint relay");
    _;
  }

  function crossmintPurchase(address to, uint256 count) external payable whenNotPaused onlyCrossmint {
    purchaseHelper(to, count);
  }

  function crossmintEarlyPurchase(address to, uint256 count) external payable whenNotPaused onlyCrossmint {
    require(isPreSaleActive(), "BASE_COLLECTION/CANNOT_MINT_PRESALE");

    earlyPurchaseHelper(to, count);
  }

  function addCrossmintRelay(address relay) external onlyOwner {
    _crossmintRelays[relay] = 1;
  }

  function removeCrossmintRelay(address relay) external onlyOwner {
    _crossmintRelays[relay] = 0;
  }
}
