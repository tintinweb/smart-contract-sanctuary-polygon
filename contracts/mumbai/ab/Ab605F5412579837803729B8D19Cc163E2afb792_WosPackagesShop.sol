...
contract Tax is CollateralWrapper {
    ...
    function setGlobalFeePercent(uint256 newTax) external onlyOwner {
    ...
    function setTaxStakingPercent(
        ...
        require(
            _totalTaxPercentage() <= 100,
            "Total tax percentage cannot exceed 100"
        );
        taxStakingPercent = newTaxStakingPercent;
    }

    function setTaxDGPercent(uint256 newTaxDGPercent) external onlyOwner {
        require(
    ...
    function setTaxJSPercent(uint256 newTaxJSPercent) external onlyOwner {
        require(
    ...
    function setTaxAggPercent(uint256 newTaxAggPercent) external onlyOwner {
        require(
    ...
    function setTaxOpPercent(uint256 newTaxOpPercent) external onlyOwner {
        require(
    ...
    function setTaxSlPercent(uint256 newTaxSlPercent) external onlyOwner {
        require(
    ...
    function _totalTaxPercentage() private view returns (uint256) {
        return
            taxStakingPercent +
            taxDGPercent +
            taxJSPercent +
...
/**
 * @title Wos Packages Shop
 * @author RGonzalez
 * @notice This contract was created to expand the functionality of the already deployed "WosPackages" contract.
 */
contract WosPackagesShop is ReentrancyGuard, Tax {
    using ECDSALib for bytes;
    using SafeERC20 for IERC20;
    ...
    event WosSold(address indexed buyer, uint256 totalWos, uint256 totalPrice);
    ...
    constructor(
        ...
        IECDSASignature2 _signature,
        IUAC _uac
    ) Tax(address(0), _collateral, _aggAddress, _signature, _uac) {
        require(
    ...
    function createWosPackage(
        ...
        uint256 timestamp,
        bytes[] memory signatures
    ) external status returns (uint256) {
        uac.verifyGameStatus(1);
        signature.verifyMessage(
            abi.encodePacked(price, wosAmount, amount).hash(),
            nonce,
            timestamp,
            signatures
        );
    ...
    function buyWosPackage(
        uint256 itemId,
        uint256 amount
    ) external checkPackage(itemId) nonReentrant status {
    ...
    function buyWOS(uint256 wosAmount) external nonReentrant status {
        uac.verifyAll(msg.sender, 1);
        require(
            wosAmount >= raise(10, 3) && wosAmount <= raise(9000, 3),
            "amount has to be between these values: 10 - 9000 wos/usdt"
        );
        uint256 totalPrice = wosAmount;
        _ctSign("BW_TF_STEP_1");
        _collateralTransferFrom(msg.sender, address(this), totalPrice);
        _ctSign("BW_T_STEP_2");
        _taxDistributionPreSales(totalPrice);
        IOperable(address(wosContract)).mint(wosAmount);
        wosContract.safeTransfer(msg.sender, wosAmount);
        emit WosSold(msg.sender, wosAmount, totalPrice);
    }
    ...
    function addPackages(
        ...
        uint256 timestamp,
        bytes[] memory signatures
    ) external checkPackage(itemId) status {
        uac.verifyAll(msg.sender, 1);
        signature.verifyMessage(
            abi.encodePacked(itemId, amount).hash(),
            nonce,
            timestamp,
            signatures
        );
    ...
    function getPackageData(
        uint256 itemId
    ) external view returns (WosPackage memory) {
    ...
    function switchStatus(
        ...
        if (turnOff)
            emit StatusChanged(
                "All wos purchase operations by this means have been suspended",
                turnOff
            );
        if (!turnOff)
            emit StatusChanged(
                "All wos purchase operations by this means have been resumed",
                turnOff
            );
    }
    ...
    modifier checkPackage(uint256 itemId) {
        require(itemId > 0, "itemId cannot be 0");
        require(
            wosPackageId[itemId].itemId == itemId,
    ...
    modifier status() {
        require(
            !turnOff,
            "All wos and package purchase operations are suspended."
        );
        _;
    }
}
