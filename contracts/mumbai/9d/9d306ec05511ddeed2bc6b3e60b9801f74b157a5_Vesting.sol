...
contract Vesting {
    ...
    constructor(
        ...
        CLIFF_PERIOD = CLIFF_PERIOD_ * 1 hours;
    ...
    returns (uint256 _amountVested)
    {
        ...
        vestedPercent = ((block.timestamp - firstVestingPoint) / 1 hours).mul(
            UNLOCK_UNIT
        );

        if (
            block.timestamp > firstVestingPoint.add((VESTING_PERIOD) * 1 hours)
        ) {
            vestedPercent = 100;
        }
