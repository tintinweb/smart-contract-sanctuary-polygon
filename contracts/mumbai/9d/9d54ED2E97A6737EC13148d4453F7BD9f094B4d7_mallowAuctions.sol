...
contract mallowAuctions is Ownable{
    ...
    function enterAuction(uint256 _auctionId, address _sender, string memory _type, uint256 _price,
    bytes calldata _signature, uint256 _messageLength) external{
        require(ECDSA.recover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", _messageLength.toString(),
        "Auction ID: ", _auctionId.toString(), "\nAuction Type: ", _type,"\nEntry Price: ", _price.toString(),
        "\nNonce: ", (_nonces[_sender]++).toString())), _signature) == _sender, "INVALID_SIGNATURE");
    ...
    function verify(uint256 _auctionId, address _sender, string memory _type, uint256 _price,
    bytes calldata _signature, uint256 _messageLength) public view returns (bool){
        return ECDSA.recover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n", _messageLength.toString(),
        "Auction ID: ", _auctionId.toString(), "\nAuction Type: ", _type,"\nEntry Price: ", _price.toString(),
        "\nNonce: ", (_nonces[_sender]).toString())), _signature) == _sender ? true: false;
    }
}