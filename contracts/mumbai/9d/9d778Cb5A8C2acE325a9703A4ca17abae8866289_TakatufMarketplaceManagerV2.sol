 ...
 */
contract TakatufMarketplaceManagerV2 is
    EIP712Upgradeable,
    ...
    PausableUpgradeable
{
    ...
    TakatufIERC1155V2 private _takatufErc1155V2;
    ...
    modifier isTakatufMarketplace() {
        if (
            msg.sender != address(_takatufErc1155V2) &&
            msg.sender != address(_takatufFixedPrice) &&
            msg.sender != _marketplace
        ) revert GeneralError("TKF:106");
        _;
    }
     ...
     */
    function initialize(string memory name_, string memory version_)
        external
        initializer
    {
    ...
    function setConfiguration(
        ...
        if (!isAdmin(msg.sender)) revert GeneralError("TKF:101");
        if (blockList[msg.sender] == true) revert GeneralError("TKF:126");
     ...
     */
    function blockListUpdate(address _address, bool _value) public onlyRole(PAUSER_ROLE){
     ...
     *@param takatufErc1155V2_ address of takatufErc1155V2.
     */
    function setContractAddress(
        address takatufMarketplace_,
        address takatufErc1155V2_,
        address takatufFixedPrice_
    ) external {
        if (!isAdmin(msg.sender)) revert GeneralError("TKF:101");
        if (blockList[msg.sender] == true) revert GeneralError("TKF:126");
        if (takatufMarketplace_ != address(0))
            _marketplace = takatufMarketplace_;
        if (takatufErc1155V2_ != address(0))
            _takatufErc1155V2 = TakatufIERC1155V2(takatufErc1155V2_);
        if (takatufFixedPrice_ != address(0))
            _takatufFixedPrice = TakatufIFixedPrice(takatufFixedPrice_);
    }
    ...
    function addCryptoToken(
        ...
        if (!isAdmin(msg.sender)) revert GeneralError("TKF:101");
        if (blockList[msg.sender] == true) revert GeneralError("TKF:126");
        if (tokenExist(tokenName_)) revert GeneralError("TKF:111");
        if (!_is20(address_)) revert GeneralError("TKF:113");
        if (
            keccak256(abi.encodePacked(tokenName_)) ==
            keccak256(abi.encodePacked(""))
        ) revert GeneralError("TKF:114");
        if (tokenValue_ == 0) revert GeneralError("TKF:115");
        ...
        _cryptoTokenCount = _cryptoTokenCount + 1;
    }
    ...
    function editCryptoToken(
        ...
        if (!isAdmin(msg.sender)) revert GeneralError("TKF:101");
        if (blockList[msg.sender] == true) revert GeneralError("TKF:126");
        if (!tokenExist(tokenName_)) revert GeneralError("TKF:112");
        if (!_is20(address_)) revert GeneralError("TKF:113");
        if (
            keccak256(abi.encodePacked(tokenName_)) ==
            keccak256(abi.encodePacked(""))
        ...
        whenNotPaused
    {
        ...
        if (enable_ && _cryptoTokenList[tokenName_].isEnabled)
            revert GeneralError("TKF:116");
        if (!enable_ && !_cryptoTokenList[tokenName_].isEnabled)
            revert GeneralError("TKF:117");
        ...
        )
    {
        ...
        takatufMarketplace_ = _marketplace;
        takatufErc1155V2_ = address(_takatufErc1155V2);
    }
     ...
     *@return contractType_ type of contract such as TAKATUF_V2,COLLECTOR,EXTERNAL,UNSUPPORTED.
     *@return isERC1155_ checks if contract is ERC1155.
     */
    function getContractDetails(address contractAddress_)
        public
        isTakatufMarketplace
        returns (ContractType contractType_, bool isERC1155_)
    {
        if (contractAddress_ == address(_takatufErc1155V2))
            return (ContractType.TAKATUF_V2, true);
        else {
            bytes memory payload = abi.encodeWithSignature(
                "isCollectionContract()"
            );
            (bool success, ) = contractAddress_.call(payload);
            if (success) return (ContractType.COLLECTOR, false);
            else {
                if (_is1155(contractAddress_))
                    return (ContractType.EXTERNAL, true);
                else return (ContractType.UNSUPPORTED, true);
            }
        }
    }
     ...
     *@return isERC1155_ checks if it is a ERC1155 contract.
     *@return isOwner_ checks the owner of contract.
     *@return quantity_ amount of NFTs.
     */
    ...
    {
        (contractType_, isERC1155_) = getContractDetails(contractAddress_);
        ...
        )
    {
        ...
        if (contractType == ContractType.TAKATUF_V2 && isERC1155) {
            if (calculatePayout_.quantity > quantity)
                revert GeneralError("TKF:119");
            TakatufIERC1155V2.NftData memory nftData = TakatufIERC1155V2(
                calculatePayout_.contractAddress
            ).getNftInfo(calculatePayout_.tokenId);
            uint256 nftTotalSupply = TakatufIERC1155V2(
                calculatePayout_.contractAddress
            ).totalSupply(calculatePayout_.tokenId);
            (recepientAddresses_, paymentAmount_) = _payoutTakatufV2Erc1155(
                CalculatePayout1155(
    ...
    /**
     *@notice verifying the signature for lazymintingV1 in fixed price.
     *@param lazyData_ contains tokenAddress,uri,seller address,creater addresses, royalties percentage, investors addresses, revenue percentage
    ...
    function _payoutTakatufV2Erc1155(
        ...
        )
    {
        ...
        if (
            ...
            (calculatePayout1155_.seller == calculatePayout1155_.minter)
        ) {
            CalculatePayout1155
    ...
    function _isNftTransferApproved(address seller_, address nftContract_)
        ...
        returns (bool)
    {
        if (nftContract_ == address(_takatufErc1155V2)) return true;
        return IERC1155(nftContract_).isApprovedForAll(seller_, _marketplace);
    }
