...
contract GarbageUniversity is ERC721, ERC721URIStorage, Ownable {
    ...
    //Mapping to check whether an address has minted a token or not
     ...
     * @dev Initializes the contract by setting a `name` and a `symbol` to the token collection.
    */
    constructor() ERC721("Garbage University", "GU") {}
    ...
    function mint(string memory uri) external {
        require(!hasSoul[msg.sender], "SoulboundNFT: SBT already minted");
     ...
     */
    function setBatchTokenURI(uint256[] memory tokenIds, string[] memory _tokenURIs) public onlyOwner {
        uint256 tokenIdLength = tokenIds.length;
        require(tokenIdLength <= 100, "SoulboundNFT: Max length of token ids is 100");
    ...
    function _beforeTokenTransfer(address from, address to, uint256) pure override internal {
        require(from == address(0) || to == address(0), "SoulboundNFT: Not allowed to transfer token");
    }
    ...
    function approve(address, uint256) public override {
        revert("SoulboundNFT: Approval not allowed");
    }
