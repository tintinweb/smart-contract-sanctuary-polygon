...
contract TimeVaultLock is ERC20, ReentrancyGuard {

    mapping(address => uint256) public depositTime;

    constructor() ERC20("BankToken", "BTK") {}

    function deposit() public payable {
        if (depositTime[msg.sender] != 0) revert Deposited();
        if (msg.value != 1) revert InvalidMsgValue();
        depositTime[msg.sender] = block.timestamp;
    }

    function withdraw() public nonReentrant {
        if (depositTime[msg.sender] == 0) revert NotDeposit();
