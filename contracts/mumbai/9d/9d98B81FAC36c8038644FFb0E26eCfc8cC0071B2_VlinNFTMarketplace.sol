...
// File: contracts/VlinMarketplace.sol
...
interface IVlinNFTFactory {
    ...
    function isVlinNFT(address _nft) external view returns (bool);
}
...
/* VLIN NFT Marketplace
    ...
    Royalty fees
*/
contract VlinNFTMarketplace is Ownable, ReentrancyGuard {
    IVlinNFTFactory private immutable vlinNFTFactory;
    ...
    constructor(
        ...
        IVlinNFTFactory _vlinNFTFactory
    ) {
        require(_platformFee <= 10000, "can't more than 10 percent");
        platformFee = _platformFee;
        feeRecipient = _feeRecipient;
        vlinNFTFactory = _vlinNFTFactory;
    }

    modifier isVlinNFT(address _nft) {
        require(vlinNFTFactory.isVlinNFT(_nft), "not a Vlin NFT");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isVlinNFT(_nft) isPayableToken(_payToken) {
    ...
    function buyNFT(
        ...
        IVlinNFT nft = IVlinNFT(listedNft.nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        IVlinNFT nft = IVlinNFT(offer.nft);
        address royaltyRecipient = nft.getRoyaltyRecipient();
        uint256 royaltyFee = nft.getRoyaltyFee();
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        IVlinNFT vlinNft = IVlinNFT(_nft);
        address royaltyRecipient = vlinNft.getRoyaltyRecipient();
        uint256 royaltyFee = vlinNft.getRoyaltyFee();
