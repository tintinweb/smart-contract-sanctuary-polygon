...
contract NFDT is AgentStore, IERC721, IERC721Metadata, IERC721Enumerable, ERC165, AutomationCompatible {
    ...
    function safeTransferFrom(address _from, address _to, uint256 _tokenId) public {
        ...
        if (isCovenantLegallyEnded(_tokenId)) {
            if (!isCovenantAdministrativelyEnded(_tokenId)) {
                endExpired(_tokenId);
            }
            //revert("Without data only sending to the owner is supported");
            begin(_tokenId, File(0, _to));
        } else {
    ...
    function transferFrom(address _from, address _to, uint256 _tokenId) external {
        if (isCovenantLegallyEnded(_tokenId)) {
            require(_from == owner(), "Ended covenants can be only sent from the owner");
    ...
    function tokenOfOwnerByIndex(address _owner, uint256 _index) external view returns (uint256) {
       // todo optimize
       for (uint256 i = 0; i < COUNT; i++) {
           if (durationOf(i) != 0 && ownerOf(i) == _owner) {
               if (_index == 0) {
                   return i;
               }
    ...
    function _transferOwnership(address newOwner) internal virtual override {
        ...
        for (uint256 i = 0; i < COUNT; i++) {
            if (durationOf(i) != 0 && isCovenantAdministrativelyEnded(i)) {
                emit Transfer(previousOwner, newOwner, i);
            }
        }
    ...
    function checkUpkeep(bytes calldata checkData) external view returns (bool upkeepNeeded, bytes memory performData) {
        uint256[COUNT] memory tokens;
        uint tokenCount = 0;
        
        for (uint256 i = 0; i < COUNT; i++) {
            if (durationOf(i) != 0 && isCovenantLegallyEnded(i) && !isCovenantAdministrativelyEnded(i)) {
                tokens[tokenCount++] = i;
            }
        }
        ...
        return (tokenCount != 0, abi.encode(result));
    }

    function performUpkeep(bytes calldata performData) external {
        (uint256[] memory tokens) = abi.decode(performData, (uint256[]));
...
contract AgentStore is Ownable {
    ...
    //mapping(uint => Queue) private expirationQueuesByDuration;

    function durationOf(uint256 _tokenId) public pure returns (uint) {
        return _tokenId == 0 ? 60 * 10
            : 1 <= _tokenId && _tokenId <= 100 ? 3600 * 24
            : 100 < _tokenId && _tokenId <= 200 ? 3600 * 24 * 7
            : 1000 < _tokenId && _tokenId <= 1100 ? DURATION_1M
            : 3000 < _tokenId && _tokenId <= 3100 ? DURATION_3M
            : 6000 < _tokenId && _tokenId <= 6100 ? DURATION_6M
            : 8000 < _tokenId && _tokenId <= 8500 ? DURATION_1Y
            : 0;
    }
    ...
    constructor() {
        //uint lastDuration = 0;
        //for (uint256 i = 0; i < COUNT; i++) {
        //    if (durationOf(i) != lastDuration) {
        //        expirationQueuesByDuration[durationOf(i)].initialize();
        //        lastDuration = durationOf(i);
        //    }
        //}
    }

    function begin(uint256 _tokenId, File memory _file) public {
        uint _expiration = block.timestamp + durationOf(_tokenId);
    ...
    function afterEnd(uint256 _tokenId) internal {
        //expirationQueuesByDuration[durationOf(_tokenId)].unlink(_tokenId);
    ...
    function afterBegin(uint256 _tokenId) internal {
        //expirationQueuesByDuration[durationOf(_tokenId)].push(_tokenId);
...
uint256 constant COUNT = 10000;

uint constant DURATION_1Y = 31556952; // https://qr.ae/pyPsqx
uint constant DURATION_1M = DURATION_1Y / 12;
uint constant DURATION_3M = DURATION_1M * 3;
uint constant DURATION_6M = DURATION_1M * 6;
