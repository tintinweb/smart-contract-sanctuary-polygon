...
contract CoM is Initializable, ERC20Upgradeable, ERC20BasicApproveExtension, WithOwnerUpgradeable, WithSuperOperators {
...
contract WithSuperOperators is ContextUpgradeable, WithOwnerUpgradeable {
    ...
    function setSuperOperator(address superOperator, bool enabled) external onlyOwner {
        ...
        if(enabled) {
            if(!isSuperOperator(superOperator)) {
                __superOperators.push(superOperator);
            }
        }
        else {
            if(isSuperOperator(superOperator)) {
                __superOperators.push(superOperator);
                for(uint i; i< __superOperators.length;i++) {
                    if(__superOperators[i] == superOperator) {
                        delete __superOperators[i];
                        break;
                    }
                }
            }
    ...
    function superOperators() public view returns (address[] memory) {
        return __superOperators;
    }
}
