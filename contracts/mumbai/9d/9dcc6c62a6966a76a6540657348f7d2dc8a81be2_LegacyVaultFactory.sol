...
contract LegacyVault is ILegacyVault, AccessControl, Pausable {
    ...
    event BackupWalletAdded(string userId, address indexed owner, uint8 index, address indexed backupwallet);
    ...
    function getBackupWallet(address _owner) external view returns (address[2] memory) {
    ...
    function addBackupWallet(string memory userId, uint8 index, address _backupWallet) external onlyListedUser(_msgSender()) {
        require(
    ...
    function switchBackupWallet(string memory userId, address owner) external {
        require(
            _msgSender() == backupWallets[owner][0] || _msgSender() == backupWallets[owner][1],
        ...
        for (uint i = 0; i < ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc1155Assets.length; i++) {
            IERC1155 _contract = IERC1155(ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc1155Assets[i]._contract);
        ...
        for (uint i = 0; i < ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc721Assets.length; i++) {
            IERC721 _contract = IERC721(ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc721Assets[i]._contract);
        ...
        for (uint i = 0; i < ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc20Assets.length; i++) {
            IERC20 _contract = IERC20(ILegacyAssetManager(_legacyAssetManager).getUserAssets(owner).erc20Assets[i]._contract);
...
contract LegacyVaultFactory is ILegacyVaultFactory, AccessControl, Pausable {
    ...
    function createVault(string memory userId, address _memberAddress) external override onlyRole(ADMIN_ROLE) whenNotPaused {
