...
contract UmbrellaFund is Ownable, ReentrancyGuard {
    ...
    function claimReward(uint256 id) public nonReentrant {
        require(
            ...
            "Only the investor of this id can claim reward"
        );
    ...
    function claimAllReward() public nonReentrant {
        require(ownedDeposits[msg.sender].length > 0, "You must deposit once at least");
        ...
        // check if user has claimable reward > 0
        require(allClaimableReward > 0, "Your reward is zero");
    ...
    function withdrawCapital(uint256 id) public nonReentrant {
        ...
        require(
            ...
            "Withdrawal lock time is not finished yet"
        );
        require(depositState[id].state, "You already withdrew capital");
    ...
    function depositFunds() external payable onlyOwner returns(bool) {
        require(msg.value > 0, "You have to deposit more than 0 matic");
        return true;
    }
