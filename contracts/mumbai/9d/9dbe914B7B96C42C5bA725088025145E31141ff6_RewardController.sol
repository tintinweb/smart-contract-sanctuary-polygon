...
import { IRewardController } from "../common/interface/IRewardController.sol";
/* incentive controller */
import { IncentiveController } from "../common/controller/IncentiveController.sol";

contract RewardController is
	Initializable,
	UUPSUpgradeable,
	IRewardController,
	IncentiveController
{
	...
	function initialize(
		...
		uint256 totalDistributionPerSecond, //{6659056316590563}0.00665905 shared per seconds daily(575.342)
		uint256 distributionEnd //{315360000000000000000000000}+315360000 seconds
	) external initializer {
		_controller = coreController;

		// reward settings
		//solhint-disable-next-line not-rely-on-time
		_distributionEnd = block.timestamp + distributionEnd;
		_distributionPerSecond = totalDistributionPerSecond;
	}
	...
	function accrueRewards(
		...
		uint256 accruedRewards = _updateAccountDistributionIndex(
			account,
			msg.sender,
			balance,
			totalSupply
		...
		if (accruedRewards != 0) {
			_unclaimedRewards[account] += accruedRewards;
			emit RewardsAccrued(account, accruedRewards);
		}
	}

	//solhint-disable-next-line no-empty-blocks
	// function refreshDistributionSpeed(address[] calldata assets) external {
	/* @todo automatically refresh the emission distribution among assets through monetary-weight calcs */
	// }

	function configureRewardEmission(address partner, uint88 emissionPerSecond)
		external
		override
		onlyConfigurator
	{
		_configureRewardEmission(partner, emissionPerSecond);
	}
	...
	function configureRewardEmissions(
		...
		if (partners.length != emissionsPerSecond.length)
			revert InconsistentParameterLength();

		for (uint256 i = 0; i < partners.length; i++) {
			_configureRewardEmission(partners[i], emissionsPerSecond[i]);
		}
	}
	...
	function claimRewards(
		...
		uint256 amount,
		address to //TODO allow ID as recipient
	...
	function claimRewards(address[] calldata assets, uint256 amount)
		...
		returns (uint256)
	{
		return _claimRewards(assets, amount, msg.sender, msg.sender);
	}
	...
	function getRewardIndex(address account, address partner)
		...
		returns (uint256)
	{
		return rewards[partner].recipientIndex[account];
	}
	...
	{
		return (
			rewards[partner].index,
			rewards[partner].emissionPerSecond,
			rewards[partner].lastUpdateTimestamp
		);
	}
	...
	{
		unclaimedRewards = _unclaimedRewards[account]; //get previously recorded unclaimed reward

		for (uint256 i = 0; i < partners.length; i++) {
			unclaimedRewards += _getRewardBalance(account, partners[i]); // then calculate unrecorded data per asset
		}
	}

	/* getters zone */
	function isRewarded(address partner) external view override returns (bool) {
		return rewards[partner].emissionPerSecond > 0;
	}

	function getRewardDistributionEnd() external view returns (uint256) {
		return _distributionEnd;
	}

	function getRewardDistributionPerSecond() external view returns (uint256) {
		return _distributionPerSecond;
	}
	...
	function setRewardToken(address rewardToken)
		...
		onlyConfigurator
	{
		_reward = IERC20Upgradeable(rewardToken);
	}
	...
	function setDistributionEnd(uint256 distributionEnd)
		...
		onlyConfigurator
	{
		if (_distributionEnd < block.timestamp)
			_distributionEnd = distributionEnd;
	}

	function setDistributionPerSecond(uint256 distributionPerSecond)
		external
		onlyConfigurator
	{
		_distributionPerSecond = distributionPerSecond;
	}
...
/* interfaces */
import { IScaledToken } from "../interface/IScaledToken.sol";

abstract contract IncentiveController {
	/* using deps */
	...
	/* Emission and Index-related events */
	event RewardEmissionUpdated(address indexed partner, uint256 emission);

	event RewardIndexUpdated(address indexed partner, uint256 index);

	event AccountIndexUpdated(
		address indexed account,
		address indexed partner,
		uint256 index
	);

	/* Reward Distribution related events */
	...
	/* var decls */
	uint256 internal _distributionEnd; // Reward distribution end timestamp
	uint256 internal _distributionPerSecond; // Total Reward distribution per second
	IERC20Upgradeable internal _reward;

	/* internal functions below */

	/**
	 * @dev Configures the emission of rewards to a particular partner
	 * @param partner The partner being configured, usually vault or credit tokens
	 * @param emissionPerSecond The emmission per second
	 **/
	function _configureRewardEmission(
		address partner,
		uint128 emissionPerSecond
	) internal {
		RewardData storage rewardData = rewards[partner];

		_updateDistributionIndex(
			partner,
			rewardData,
			IScaledToken(partner).scaledTotalSupply()
		);

		rewardData.emissionPerSecond = emissionPerSecond;

		emit RewardEmissionUpdated(partner, emissionPerSecond);
	}

	function _getRewardBalance(address account, address partner)
		internal
		view
		returns (uint256 unclaimedRewards)
	{
		(uint256 accountLiquidity, uint256 totalLiquidity) = IScaledToken(
			partner
		).getScaledAccountBalanceAndSupply(account);

		unclaimedRewards = _getUnclaimedReward(
			account,
			partner,
			accountLiquidity,
			totalLiquidity
	...
	/**
	 * @dev Claims reward on all the `partners`, accumulating the pending rewards.
	...
	function _claimRewards(
		...
		uint256 unclaimedRewards = _unclaimedRewards[account];
		uint256 accruedRewards = 0;

		for (uint256 i = 0; i < partners.length; i++) {
			address partner = partners[i]; //cache partner

			(uint256 accountLiquidity, uint256 totalLiquidity) = IScaledToken(
				partner
			).getScaledAccountBalanceAndSupply(account); //get liquidity info

			accruedRewards += _updateAccountDistributionIndex(
				account,
				partner,
				accountLiquidity,
				totalLiquidity
		...
		_unclaimedRewards[account] = unclaimedRewards - amountToClaim; // Safe due to the previous line

		// transfer rewards
		_reward.safeTransfer(to, amountToClaim);
		emit RewardsClaimed(account, to, amountToClaim);

		return amountToClaim;
	}

	/**
	 * @dev Updates the state of a partner distribution, mainly rewards index and timestamp
	 * @param partner The partner distribution being updated, e.g vaultTokens or CreditTokens
	 * @param rewardData Storage pointer to the distribution's config
	 * @param totalLiquidity Total liquidity provided to this `partner` at this moment of distribution
	...
	function _updateDistributionIndex(
		...
		uint128 lastUpdateTimestamp = rewardData.lastUpdateTimestamp;
		...
		if (newIndex != oldIndex) {
			...
			emit RewardIndexUpdated(partner, newIndex);
		}

		//solhint-disable-next-line not-rely-on-time
		rewardData.lastUpdateTimestamp = uint128(block.timestamp);

		return newIndex;
	}

	/**
	 * @dev Updates account's reward state in a distribution
	 * @param account The rewarded account
	 * @param partner The partner rewarding `account`
	 * @param accountLiquidity The liquidity provided by the account in the distribution at the moment
	 * @param totalLiquidity Current liquidity in the distribution
	 * @return accruedRewards The accrued rewards for the account until the moment
	 **/
	function _updateAccountDistributionIndex(
		address account,
		address partner,
		...
		RewardData storage rewardData = rewards[partner];
		uint256 accountIndex = rewardData.recipientIndex[account];

		uint256 newIndex = _updateDistributionIndex(
			partner,
			rewardData,
			totalLiquidity
		);

		if (accountIndex != newIndex) {
			...
			rewardData.recipientIndex[account] = newIndex;
			emit AccountIndexUpdated(account, partner, newIndex);
		}
	}

	/**
	 * @dev Return the accrued rewards for an account on a particular partner
	 * @param account The rewarded account
	 * @param partner The partner's reward being claimed
	 * @param accountLiquidity The liquidity provided by the account in the distribution at the moment
	 * @param totalLiquidity The total liquidity provided
	 * @return The accrued rewards for the account until the moment
	 **/
	function _getUnclaimedReward(
		address account,
		address partner,
		uint256 accountLiquidity,
		uint256 totalLiquidity
		...
		RewardData storage rewardData = rewards[partner];
		uint256 rewardIndex = _getDistributionIndex(
			rewardData.index,
			rewardData.emissionPerSecond,
		...
		return
			_getRewards(
				accountLiquidity,
				rewardIndex,
				rewardData.recipientIndex[account]
			);
	}

	/**
	 * @dev Internal function for the calculation of account's rewards on a distribution
	 * @param principalBalance Liquidity provided by account on a distribution
	 * @param vaultIndex Current index of the distribution
	...
	function _getRewards(
		...
		return (principalBalance * (vaultIndex - accountIndex)) / 10**18;
	}

	/**
	 * @dev Calculates the next value of specific distribution index, with validations
	 * @param currentIndex Current index of the distribution
	 * @param emissionPerSecond Representing the total rewards distributed per second per rewardToken unit
	...
	function _getDistributionIndex(
		...
		uint256 endOfDistribution = _distributionEnd;
		//solhint-disable-next-line not-rely-on-time
		...
		if (
			...
			lastUpdateTimestamp >= endOfDistribution
		) {
			return currentIndex;
		}

		uint256 currentTimestamp = blockTimeStamp > endOfDistribution
			? endOfDistribution
			: blockTimeStamp;
		uint256 timeDelta = currentTimestamp - lastUpdateTimestamp;
		return
			((emissionPerSecond * timeDelta * (10**18)) / totalBalance) +
			currentIndex;
	}
}
