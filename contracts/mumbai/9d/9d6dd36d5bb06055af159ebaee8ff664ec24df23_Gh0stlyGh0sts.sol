...
contract Gh0stlyGh0sts is Ownable, ERC721, NonblockingReceiver {
    ...
    address public _owner; //Owner del contrato
    string private baseURI; //Base URI
    uint256 nextTokenId = 0; //Token ID (Identificador del NFT)
    uint256 MAX_MINT_ETHEREUM = 3084; //Max supply 

    uint gasForDestinationLzReceive = 350000; //**Cantidad de gas para que la transacción pase en la chain de destino

    constructor(string memory baseURI_, address _layerZeroEndpoint) ERC721("Gh0stlyGh0sts", "gg") { 
        _owner = msg.sender; //Define el owner
        endpoint = ILayerZeroEndpoint(_layerZeroEndpoint); //Crea una instancia del ILayerZeroEndpoint y la vincula a la dirección que se pase como argumento
        baseURI = baseURI_; //Setea el BASE URI
    }

    // mint function
    // you can choose to mint 1 or 2
    // mint is free, but payments are accepted
    function mint(uint8 numTokens) external payable { //Recibe la cantida de tokens que se desea mintear, MAX 2
        require(numTokens < 3, "GG: Max 2 NFTs per transaction");
        require(nextTokenId + numTokens <= MAX_MINT_ETHEREUM, "GG: Mint exceeds supply");
        _safeMint(msg.sender, ++nextTokenId); //Le pasa el nuevo owner de los NFTs a mintear y aumenta el valor del tokenID +1 
        if (numTokens == 2) {
            _safeMint(msg.sender, ++nextTokenId); //Parece ser  la misma mierda de la función anterior
        }
    }
    ...
    function traverseChains(uint16 _chainId, uint tokenId) public payable {
        require(msg.sender == ownerOf(tokenId), "You must own the token to traverse"); //Exije que el que llame esta función sea el owner 
        require(trustedRemoteLookup[_chainId].length > 0, "This chain is currently unavailable for travel"); //Exige que haya un valor en el trustedRemoteLookup en la chain de destino, parece ser un mecanismo que permite ver si una chain está disponible o no.

        // burn NFT, eliminating it from circulation on src chain
        _burn(tokenId); //Quema el token

        // abi.encode() the payload with the values to send
        bytes memory payload = abi.encode(msg.sender, tokenId); //Codifica los valores a enviar y se los asigna a la variable payload

        // encode adapterParams to specify more gas for the destination
        uint16 version = 1; //Setea la versión a utilizar como Default
        bytes memory adapterParams = abi.encodePacked(version, gasForDestinationLzReceive); //Codifica los parametros y asigna los valores a pasar la variable adapterParams

        // get the fees we need to pay to LayerZero + Relayer to cover message delivery
        // you will be refunded for extra gas paid
        (uint messageFee, ) = endpoint.estimateFees(_chainId, address(this), payload, false, adapterParams); //Llama a la función estimateFees pasandole los parametros recopilados y codificados hasta ahora. Y almacena el valor en la variable messageFee
        
        require(msg.value >= messageFee, "GG: msg.value not enough to cover messageFee. Send gas for message fees"); //Exige que el valor a pagar por GasFees sea mayor al estimado recibido de la función estimateFees
    ...
    function setBaseURI(string memory URI) external onlyOwner {
        baseURI = URI; //Setea el BASE URI
    }

    function donate() external payable {
        // thank you //Acepta donaciones... si se lo juro, crealas. 
    }

    // This allows the devs to receive kind donations
    function withdraw(uint amt) external onlyOwner {
        (bool sent, ) = payable(_owner).call{value: amt}(""); //Permite retirar X (amt) cantidad de Ether del contrato
    ...
    function setGasForDestinationLzReceive(uint newVal) external onlyOwner {
        gasForDestinationLzReceive = newVal; //Permite modificar la variable gasForDestinationLzReceive para permitir que se adapte a posibles  cambios  futuros
    }

    // ------------------
    // Internal Functions
    // ------------------
        //Esta función permite crear los NFTs en la dirección de destino (no se quien la llama)
