...
/**
 * @title Partial ERC20 compatible wrapper for an EnvoyStaking contract.
 * @author Kasper De Blieck ([email protected])
 * This contract implements a number of ERC20 functions.
 * Only read functions are present, allowing view integration with ERC20
 * compatible applications.
 */
contract EnvoyStakingERC20 is Ownable{
    ...
    function balanceOf(address stakeholderAddress) public view returns (uint256 balance){
        (, StakeholderLib.Stakeholder memory stakeholder) = 
            stakingContract.calculateRewards(
                stakeholderAddress,
    ...
    /**
     * Transfer funds locked in the contract to the real staking contract.
     */
    function transferToStaking() public {
        stakingContract.stakingToken().transfer(
            address(stakingContract),
            stakingContract.stakingToken().balanceOf(address(this))
        );
    }

    function updateStakingAddress(address stakingContractAddress) public onlyOwner{
        stakingContract = EnvoyStaking(stakingContractAddress);
    }
...
library Constants {
    ...
    uint constant public rewardPeriodDuration = 600;//86400;
}
