...
contract NFT is ERC1155Supply, Ownable {
    ...
    mapping(address => uint8) mintMapping;

    event PermanetEvent(string _value, uint256 indexed _id);
    ...
    function setMeta(
        ...
        require(meta[tokenId].maxSupply == 0, "token already added");
        meta[tokenId] = Meta(_name, _uri, amount, _maxSupply, 0);
    }

    function mint(uint256 tokenId, uint256 amount) external {
        require(block.timestamp > start, "mint not start");
        require(meta[tokenId].maxSupply > 0, "unknown tokenId");
        require(meta[tokenId].supply < meta[tokenId].maxSupply, "reach maximum supply");
        require(mintMapping[_msgSender()] == 0, "already minted");
        ...
        mintMapping[_msgSender()] = 1;

        emit PermanetEvent(meta[tokenId].uri, tokenId);
    }
    ...
    function minted() public view returns(bool) {
        return mintMapping[_msgSender()] == 1;
    }
