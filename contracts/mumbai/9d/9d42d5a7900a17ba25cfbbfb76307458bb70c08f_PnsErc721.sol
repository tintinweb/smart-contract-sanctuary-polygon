...
abstract contract ERC721Enumerable is ERC721, IERC721Enumerable {
    ...
    function allTokensOfOwner(address owner, uint256 startIndex, uint256 lastIndex) public view returns (uint256[] memory, string[] memory, uint256[] memory) {
        uint256 supply = getNextTokenId();
        require(startIndex != 0 && lastIndex != 0 && startIndex <= supply && lastIndex <= supply && startIndex < lastIndex, "Request is out of bounds.");
        ...
        for(uint i=startIndex; i <= lastIndex; i++){
            if(owner == getOwnerOf(i)){
                tokenIDs[count] = i;
                names[count] = getName(i);
                expiration[count] = getExpiration(i);
                count++;
            }
        }
