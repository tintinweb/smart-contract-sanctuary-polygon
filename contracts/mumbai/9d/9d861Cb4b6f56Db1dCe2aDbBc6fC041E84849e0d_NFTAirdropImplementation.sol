...
import "../Interface/ICNTNFT.sol";

/**
 * @dev {NFTAirdropImplementation} contract, including:
 *
 *  - ability to mint NFTs using merkle proofs
 *  - allows to update params by `owner`
 */
contract NFTAirdropImplementation is Ownable, Initializable {
    using SafeMath for uint256;

    event Redeemed(address _user, uint256 _nftId);

    event MerkleRootUpdated(bytes32 _newMerkleRoot);

    event MaxMintingAllowedUpdated(uint256 _maxMintingUpdated);

    event UserLimitUpdated(uint256 _newUserLimit);

    event ERC721Updated(address _newERC721);
    ...
    /**
     * Initializes the state of the contract.
     *
     * @param _encodedData Bytes encoded contract state to be used to define
     *                     initial state of the contract.
     */
    function init(bytes memory _encodedData) external initializer {
        (userLimit, maxMintingAllowed, root, erc721, owner) = abi.decode(
            _encodedData,
            (uint256, uint256, bytes32, ICNTNFT, address)
        );
    }
        ...
        onlyOwner
    {
        ...
        emit MaxMintingAllowedUpdated(_maxMintingAllowed);
    }
    ...
    function updateUserLimit(uint256 _userLimit) external onlyOwner {
        ...
        emit UserLimitUpdated(_userLimit);
    }
    ...
    function updateERC721(ICNTNFT _erc721) external onlyOwner {
        erc721 = _erc721;
        emit ERC721Updated(address(_erc721));
    }
     ...
     */
    function redeem(address _account, bytes32[] calldata _proof) external {
        require(
        ...
        mintedNFTs[_account] = mintedNFTs[_account].add(uint256(1));
        totalNFTsMinted = totalNFTsMinted.add(uint256(1));
    ...
    /**
     * @notice Verifies if it's a valid leaf.
     *
