...
contract ERC721 is Context, ERC165, IERC721 {
    ...
    /**
     * @dev 주어진 address에서 보유중인 NFT의 개수를 반환
    ...
    /**
     * @dev 해당 NFT의 토큰아이디를 가지고 있는 소유자의 address를 반환
    ...
    function _burn(address owner, uint256 tokenId) internal virtual {
        require(ERC721.ownerOf(tokenId) == owner, "ERC721: burn of token that is not own");
    ...
    function _burn(uint256 tokenId) internal virtual {
        _burn(ERC721.ownerOf(tokenId), tokenId);
    }
...
//contract ERC721Token is Context, ERC165, ERC721, IERC721Metadata, MinterRole {
...
contract CretaNFT is ERC721Metadata {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    address payable payableOwner;
    address owner;
    uint256 listingPrice = 30*1000000;
    ...
    constructor() ERC721Metadata("Creta NFT", "CNFT") {
        payableOwner = payable(msg.sender);
        owner = msg.sender;
    }
    
    // function createToken(uint tokenId, address contractAddress, string memory tokenURI) public returns (uint256) {
    //     _tokenIds.increment();
    ...
    function sellMintAndCoinTransfer(uint256 tokenId, string memory tokenURI) public payable {
        ...
        require(address(msg.sender).balance >= msg.value, "Address: insufficient balance");
        payable(payableOwner).transfer(msg.value);
    ...
    function sellMintAndTokenTransfer(uint256 tokenId, string memory tokenURI, uint256 count, address cretaTokenAddress) public payable {
        require(count == tokenPrice, "Please submit the asking price in order to complete the purchase");
        ...
        IERC20(cretaTokenAddress).transferFrom(msg.sender, owner, count);
