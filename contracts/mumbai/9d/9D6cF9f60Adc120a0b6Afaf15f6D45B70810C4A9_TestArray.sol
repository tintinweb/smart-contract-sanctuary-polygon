...
contract TestArray {
    ...
    function decode(bytes memory encodedData) public pure returns (address[] memory) {
        return abi.decode(encodedData, (address[]));
    }

    function encodeArray(address[] memory addrArray) public pure returns (bytes memory) {
        return abi.encode(addrArray);
    }

    function decodeArray(bytes memory encodedArrayData) public pure returns (address[] memory) {
