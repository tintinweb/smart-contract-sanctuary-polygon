...
contract ScoreContract is IERC20, Ownable {
    ...
    string private _name = 'MMM SCORE';
    ...
    function mint(address to, uint256 amount) external onlyCaller returns (bool) {
        _mint(to, amount);
        return true;
    }

    function burn (address from, uint256 amount) external onlyCaller returns (bool) {
        _burn(from, amount);
        return true;
    }
}