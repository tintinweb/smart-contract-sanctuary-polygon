...
contract AssassinBabies is ERC721Enumerable, Ownable {
    using MerkleProof for bytes32[];

    mapping(address => uint8) public alreadyMinted;

    uint256 public babyPrice = 50000000000000000; // .05 MATIC
    ...
    uint256 public constant maxMint = 6050;
    ...
    function mint(
        ...
        require(alreadyMinted[msg.sender] < 5, "Address already used");
        require(NFTsId <= maxMint, "Mint limit reached");
        require(amount <= 5, "Can not mint more than 5 tokens at once");
        require(msg.value == (babyPrice * amount), "The price is invalid");

        for (uint8 i = 1; i <= amount; i++) _safeMint(msg.sender, NFTsId++);
