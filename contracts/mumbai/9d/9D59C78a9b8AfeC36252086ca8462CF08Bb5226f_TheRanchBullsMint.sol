...
contract TheRanchBullsMint is ERC721Enumerable, IERC2981, Ownable {
    ...
    uint256 public NODEBULLS_MINT_PRICE = 1;
    ...
    function mint(uint256 tokenQuantity) external payable {
        ...
        if (presaleLive) {
            if (whiteList[msg.sender]) {
                ...
                require(msg.value >= ((getMintPrice() * tokenQuantity) - getMintPrice()), "ERROR: PRICE EXCEEDS THE AMT PROVIDED ((NODEBULLS_MINT_PRICE * quantity)) - NODEBULLS_MINT_PRICE)");
                ...
                require(addressPurchases[msg.sender] + tokenQuantity <= 5, "EXCEEDS PRESALE AMOUNT");
                require(msg.value >= (getMintPrice() * tokenQuantity), "ERROR: PRICE EXCEEDS THE AMT PROVIDED (NODEBULLS_MINT_PRICE * quantity)");
                for (uint256 i = 0; i < tokenQuantity; i++) {
                    addressPurchases[msg.sender]++;
                    _tokenSupply.increment();
                    _safeMint(msg.sender, _tokenSupply.current());
    ...
    //     uint256 costToMint = ((NODEBULLS_MINT_PRICE * 10**8) * 10**8) / adjustedPrice;
    //     return costToMint;
    // }
    ...
    function getMintPrice() public view returns (uint256) {
        ...
        // 50/2,000
        // 50 * 100000 / 2000
        uint256 cost = ((NODEBULLS_MINT_PRICE * 10**18) * 10**18) / adjustedPrice;
        return cost;
    }
