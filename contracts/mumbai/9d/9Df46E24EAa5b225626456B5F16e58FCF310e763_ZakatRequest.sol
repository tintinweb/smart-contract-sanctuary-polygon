...
contract ZakatRequest is BasicAccessControl, ZakatBasic {
        ...
        returns (address)
    {
        require(
            ...
            "Invalid request id from approval"
        );
        ...
        onlyModerators
    {
        ...
        uint256 reqId = zktApprove.requestId - 1;

        if (zktApprove.requestId != _requestId) {
            address anotherRequester = requestIds[reqId];
            requestIds[reqId] = requester;
        ...
        requestIds.pop();
        emit Approve(requester, zktApprove.doner, _approve, reqId);
    }
}