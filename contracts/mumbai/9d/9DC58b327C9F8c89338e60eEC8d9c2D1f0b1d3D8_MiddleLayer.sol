...
contract MiddleLayer is IMiddleLayer, MiddleLayerAdmin, CommonModifiers {
    ...
    function msend(
        ...
        IRoute(_route).msend{value:msg.value}(
            _dstChainId,
            params,
            _refundAddress
        );
...
abstract contract MiddleLayerAdmin is MiddleLayerStorage, MiddleLayerModifiers {
    ...
    function setLoanAgent(address _loanAgent) public onlyOwner() {
        loanAgent = ILoanAgent(_loanAgent);
    }

    function setPUSD(address _pusd) public onlyOwner() {
        pusd = PUSDMessageHandler(_pusd);
    }

    function addRoute(IRoute _newroute) public onlyOwner() {
        routes.push(_newroute);
        authRoutes[address(_newroute)] = true;
    }

    function removeRoute(IRoute _routeToRemove) public onlyOwner() {
        for (uint i; i < routes.length; i++) {
            if (routes[i] == _routeToRemove) {
                // swap the route to remove with the last item
                routes[i] = routes[routes.length-1];
                // pop the last item
                routes.pop();

                authRoutes[address(_routeToRemove)] = false;
                return;
            }
        }
