...
abstract contract ERC721DropWithURIOverride is ERC721Drop {
    function tokenURI(
        uint256 tokenId
...
import "contracts/ERC721DropWithURIOverride.sol";
import "@thirdweb-dev/contracts/extension/PermissionsEnumerable.sol";

contract yieldones is ERC721DropWithURIOverride, PermissionsEnumerable {
    ...
    mapping(uint256 => string) private updatedURIs;
        ...
        )
    {
        ...
        grantRole(LEVEL_ROLE, msg.sender);
        grantRole(URI_ROLE, msg.sender);
        //probably setup here another wallet to have uri and level access
    }

    event TokenLevelChanged(uint256 indexed tokenId, uint256 level);
    ...
    function setTokenLevel(
        uint256 tokenId,
        uint256 level
    ) public onlyRole(LEVEL_ROLE) {
        tokenLevel[tokenId] = level;
        emit TokenLevelChanged(tokenId, level);
    }

    function tokenURI(
        uint256 tokenId
    ) public view virtual override returns (string memory) {
        string memory uri = updatedURIs[tokenId];
    ...
    function setTokenURI(
        ...
        updatedURIs[tokenId] = newURI;
        emit TokenURIChanged(tokenId, oldURI, newURI);
    }
}