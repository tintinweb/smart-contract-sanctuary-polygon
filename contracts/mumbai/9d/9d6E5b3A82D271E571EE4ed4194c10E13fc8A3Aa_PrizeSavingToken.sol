    ...
    BatchMintMetadata
{
    ...
    // Token Id => true/false
    mapping(uint256 => bool) public isDrawed;
    uint256 private nextTokenIdToMint_;

    uint256 public currentRaffleId = 0;
    mapping(uint256 => uint256) public totalSupply;

    address public payoutTokenAddress; // MMK Token
    ...
    function setPayoutTokenAddress(address _payoutTokenAddress)
        external
        onlyAdmin
    {
        payoutTokenAddress = _payoutTokenAddress;
        originalBalance = IERC20(payoutTokenAddress).balanceOf(address(this));
    }
    ...
    function batchTransferTickets(
        ...
        require(
            ...
            "Invalid leng receivers and amounts"
        );
        ...
        isDrawed[_tokenId] = false;
    }

    function drawRaffle(uint256 _tokenId, address[] memory _receivers)
        external
        onlyAdmin
    {
        require(isDrawed[_tokenId] == false, "The round already drawed");

        require(
            payoutTokenAddress != address(0),
            "Payout token address is not set"
        );
        // 1. get a address for  the jackpot prize winner
        address jackpotPrizeWinner = _receivers[
            _random(_receivers.length, _receivers.length)
        ];
        require(
            balanceOf[address(jackpotPrizeWinner)][_tokenId] > 0,
            "No tickets issued for this draw"
        );

        uint256 mainPrizeAmount = balanceOf[address(jackpotPrizeWinner)][
            _tokenId
        ] *
            1250 *
            10**18;
        // Transfer for main prize for winner
        IERC20(payoutTokenAddress).transfer(
            jackpotPrizeWinner,
            mainPrizeAmount
        );

        // 2. get 20% of addresses for small consolation prize
        uint256 smallPrizeWinners = (_receivers.length - 1) / 5;
        for (uint256 index = 0; index < smallPrizeWinners; index++) {
            address smallWinner = _receivers[
                _random(_receivers.length - 1, index)
            ];
            require(
                balanceOf[address(smallWinner)][_tokenId] > 0,
                "No tickets issued for this draw"
            );
            IERC20(payoutTokenAddress).transfer(smallWinner, 1000 * 10**18);
        }
        isDrawed[_tokenId] = true;
    }
    ...
    function _random(uint256 _modulus, uint256 _index)
        internal
        view
        returns (uint256)
    {
