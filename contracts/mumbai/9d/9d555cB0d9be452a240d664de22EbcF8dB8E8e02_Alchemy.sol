...
contract Alchemy is Ownable {
    ...
    function useAlchemy(uint256 _zaiId, uint256[] memory _usedPotions, uint256 _sacrifiedZai) external {
        ...
        if(_usedPotions.length == 6){
            require(z.alchemyXp >= 4000, 'Your Zai has not enough alchemyXp');
            require(_sacrifiedZai == 0, "You can't sacrify a ZAI and 6 potions");
        }
        if(_usedPotions.length == 5){
            require(z.alchemyXp >= 3000, 'Your Zai has not enough alchemyXp');
        }
        if(_usedPotions.length == 4){
            require(z.alchemyXp >= 2000, 'Your Zai has not enough alchemyXp');
            require(_sacrifiedZai == 0, "You can't sacrify a ZAI and 4 potions");
        }
        if(_usedPotions.length == 3){
            require(z.alchemyXp >= 1000, 'Your Zai has not enough alchemyXp');
        ...
        if(_usedPotions.length > 0){
            for(uint256 i = 0 ; i < _usedPotions.length ; i++){
                ...
                if(p.powers.water > 0){
                    totalPowers[0] += p.powers.water; 
                }
                if(p.powers.fire > 0){
                    totalPowers[1] += p.powers.fire; 
                }
                if(p.powers.metal > 0){
                    totalPowers[2] += p.powers.metal; 
                }
                if(p.powers.air > 0){
                    totalPowers[3] += p.powers.air; 
                }
                if(p.powers.stone > 0){
                    totalPowers[4] += p.powers.stone; 
                }
                if(p.powers.rest > 0){
                    totalPowers[6] += p.powers.rest; // rest power is added to xp
                }
                if(p.powers.xp > 0){
                    totalPowers[6] += p.powers.xp;  
                }
        ...
        uint256 random = _getRandom(msg.sender, z.alchemyXp, _zaiId);
