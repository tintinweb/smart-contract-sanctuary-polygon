...
contract MiRapid {
    ...
    function _distributelevelIncome(address _user, uint256 _amount,uint8 level,bool isFromRegistration) private {
        ...
        for(; i <= LAST_LEVEL; i++){
            if(upline != address(0)){
                ...
                emit Transaction(upline,_user,reward,level,(i+1));
    ...
    function manageReward(address _user) private {
        ...
        if(users[_user].partnersCount>=teamCount[nextrank])
        {
    ...
    function getOrderLength(uint256 _dayNow) external view returns(uint256) {
        dayDirect5Users[_dayNow].length;
    }    
