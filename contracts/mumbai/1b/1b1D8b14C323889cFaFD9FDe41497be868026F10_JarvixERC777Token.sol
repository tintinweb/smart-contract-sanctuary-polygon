...
/** Cannot have a map cap of zero, otherwise no token would ever exist */
error JVX_ERC777_CapIsZero();
/** Cannot have a max cap lower than initial supply */
error JVX_ERC777_CapIsLowerThanSupply();
/** Cannot mint more than max cap */
error JVX_ERC777_CapExceeded();
/** Cannot use more than amount available for message sender */
error JVX_ERC777_AmountExceeded();

/**
 * @title This is the Jarvix ERC777 token contract.
 ...
 * @author tazous
 */
contract JarvixERC777Token is ERC777, PausableImpl, Ownable {
    ...
    /**
     * @dev Jarvix ERC777 token contract constructor
    ...
    ERC777(name, symbol, new address[](0)) {
        if(cap_ == 0) revert JVX_ERC777_CapIsZero();
        if(cap_ < initialSupply_) revert JVX_ERC777_CapIsLowerThanSupply();
        _cap = cap_;
    ...
    function burnFrom(address from, uint256 amount) public onlyRole(BURNER_ROLE) {
        ...
        if(currentAllowance < amount) revert JVX_ERC777_AmountExceeded();
    ...
    internal virtual override {
        if(totalSupply() + burnedSupply() + amount > cap()) revert JVX_ERC777_CapExceeded();
