...
contract uRevolt is Initializable, UUPSUpgradeable, ERC20Upgradeable, ERC20PermitUpgradeable, ERC20VotesUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    struct UserInfo {
        ...
        uint256 rewardRVLTDebt; // Reward debt in RVLT.
        //
    ...
    function initialize(        
        ...
        __ERC20_init_unchained("dRVLT", "dRVLT");
        __Pausable_init_unchained();
        __ReentrancyGuard_init();
        ERC20PermitUpgradeable.__ERC20Permit_init("dRVLT");
    ...
    function pendingRVLT(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            uint256 rewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalRVLTStaked.sub(totalRevoltUsedForPurchase)).sub(totalRewardMandators);
            if(_pid != 0){
        ...
        return user.amount.mul(accRVLTPerShare).div(1e12).sub(user.rewardRVLTDebt);
    }
    ...
    function updatePool(uint256 _pid) public randomGenerationCompleted{
        PoolInfo storage pool = poolInfo[_pid];
    ...
    function _deposit(uint256 _pid,address _userAddress, uint256 _amount) internal {
        ...
        if (user.amount > 0) {
            revoltReward = user.amount.mul(pool.accRVLTPerShare).div(1e12).sub(user.rewardRVLTDebt);
            pool.lpToken.safeTransfer(_userAddress, revoltReward);
        }
        user.amount = user.amount.add(_amount);
        user.rewardRVLTDebt = user.amount.mul(pool.accRVLTPerShare).div(1e12);
        if(_pid == 0){
            pool.lastRVLTRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalRVLTStaked.sub(totalRevoltUsedForPurchase));
    ...
    function _withdraw(uint256 _pid,address _userAddress, uint256 _amount) internal {
        ...
        uint256 revoltReward = user.amount.mul(pool.accRVLTPerShare).div(1e12).sub(user.rewardRVLTDebt);
        pool.lpToken.safeTransfer(_userAddress, revoltReward);
    ...
    function claimRVLT(uint256 _pid) public randomGenerationCompleted nonReentrant{
        ...
        if(_pid == 0){
            pool.lastRVLTRewardBalance = pool.lpToken.balanceOf(address(this)).sub(totalRVLTStaked.sub(totalRevoltUsedForPurchase)).sub(totalRewardMandators);
        }else{
            totalRewardMandators = totalRewardMandators.sub(revoltReward);
            totalRVLTRewardMandators[_pid] = totalRVLTRewardMandators[_pid].sub(revoltReward);
            pool.lastRVLTRewardBalance = totalRVLTRewardMandators[_pid];
    ...
    // Safe RVLT transfer function to admin.
    function accessRVLTTokens(uint256 _pid, address _to, uint256 _amount) public randomGenerationCompleted{
        require(msg.sender == adminAddress, "sender must be admin address");
        require(totalRVLTStaked.sub(totalRevoltUsedForPurchase) >= _amount, "Amount must be less than staked RVLT amount");
        PoolInfo storage pool = poolInfo[_pid];
        uint256 RevoltBal = pool.lpToken.balanceOf(address(this));
        if (_amount > RevoltBal) {
            pool.lpToken.transfer(_to, RevoltBal);
            totalRevoltUsedForPurchase = totalRevoltUsedForPurchase.add(RevoltBal);
            emit EmergencyWithdraw(_to, _pid, RevoltBal);
        } else {
            pool.lpToken.transfer(_to, _amount);
            totalRevoltUsedForPurchase = totalRevoltUsedForPurchase.add(_amount);
    ...
    function _randomSelectUsers(
        ...
        uint256 value = two;
        while (i < selectedIndexes.length.add(numberOfUsers) && selectedIndexes.length < randomThreshold) {
            value =
                uint256(keccak256(abi.encodePacked(one, value))) %
                _totalUsers;
            if (
                !is_mandator[lastFulfilledId][stakerId[value]] &&
                stakerId[value] != address(0)
            ) {
                is_mandator[lastFulfilledId][stakerId[value]] = true;
                selectedIndexes.push(value.add(1));
                UserInfo storage user = userInfo[0][stakerId[value]];
                _deposit(lastFulfilledId,stakerId[value], user.amount);
                i += 1;
            }
        }
