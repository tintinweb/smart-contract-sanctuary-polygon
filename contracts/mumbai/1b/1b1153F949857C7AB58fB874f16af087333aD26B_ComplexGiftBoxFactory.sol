...
// @title ComplexGiftBoxFactory
// @notice Used to create gift boxes with complex redemption options
contract ComplexGiftBoxFactory {

  IGiftRegistry internal registry;

  // @name Setup Factory
  constructor(address _giftRegistry) {
    registry = IGiftRegistry(_giftRegistry);
  }

  // @name Create Complex Gift Box
  // @notice Allows configuring a starting conservator, target redemption date, required redemption balance, and/or target ERC20 token balances
  function createComplexGiftBox(
    address _recipient,
    address _conservator,
    uint256 _targetDate,
    uint256 _targetBalance,
    address _targetToken,
    uint256 _targetTokenBalance
  ) external returns (address) {
    ComplexGiftBox box = new ComplexGiftBox(address(registry),
      _recipient,
      msg.sender,
      _conservator,
      _targetDate,
    ...
    // Register details with registry
    registry.registerGiftBox(_recipient, address(box));
    if(_conservator != address(0)){
      registry.registerConservator(address(box), _conservator);
    }
...
interface IGiftBox is IERC777Recipient, IERC1155Receiver {
  ...
  // @name Get Number of Allowed ERC-20 tokens
  // @dev can be used to loop through and find all enabled tokens
  function getNumAllowedERC20() external view returns(uint256);
  ...
  // @name Check if ERC20 token is Enabled
  function isERC20Enabled(address _token) external view returns(bool);

  // @name Get Type
  // @notice Used to determine the specific kind of GiftBox
  function getType() external pure returns(string memory);

  // @name Get Recipient
  // @notice Retrieves the address for the gift recipient
  function recipient() external returns(address);

  // @name Get Gift Giver
  // @notice Retrieves the address that created the gift box
...
// @title Complex Gift Box
// @notice Gift box with configurable, combinable requirements for the recipient
contract ComplexGiftBox is IGiftBox, ERC721Holder, ERC165 {
  ...
  mapping(address => bool) private enabledErc20Tokens;
  ...
  // @name Reduce Target Balance
  // @dev Used with Conservator + TargetBalance functionality
  ...
  // @name Reduce Target Token Balance
  // @dev Used with Conservator + TargetTokenBalance functionality
  function reduceTargetTokenBalance(uint256 _targetTokenBalance) external {
    require(_targetTokenBalance < targetTokenBalance, "Can only reduce target");
    require(isConservator[msg.sender], "Only conservator can reduce");
    targetTokenBalance = _targetTokenBalance;
  }

  // @name Reduce Target Date
  // @dev Used with Conservator + TargetDate functionality
  function reduceTargetDate(uint256 _targetDate) external {
    require(_targetDate < targetDate, "Can only reduce target");
    require(isConservator[msg.sender], "Only conservator can reduce");
    targetDate = _targetDate;
  }
  ...
  function allowERC20(address _token) external {
    ...
    enabledErc20Tokens[_token] = true;
  }
  ...
  // @name Check if ERC20 token is Enabled
  function isERC20Enabled(address _token) external view returns(bool){
    return enabledErc20Tokens[_token];
  }
