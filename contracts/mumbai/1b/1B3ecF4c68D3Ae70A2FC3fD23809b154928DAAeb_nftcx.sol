...
contract nftcx is Ownable {
    ...
    struct userInfoItem {
        ...
        address[] allUserList; //金卡下所有地址
    }
    using SafeMath for uint256;
    using EnumerableSet for EnumerableSet.AddressSet;
    using EnumerableSet for EnumerableSet.UintSet;
    enum setType{AllSet, GoldSet, SilverSet, BronzeSet, taskUserSet, allGoldUserSet, allSilverUserSet, allBronzeUserSet}
    //所有用户集
    ...
    function buyNFT(address _referer) external {
        ...
        require(!AllSet.contains(_user), "e001");
        //用户不能已存在
        require(AllSet.contains(_referer), "e002");
        //推荐人必须存在
        require(nodeList[_referer] != address(2), "e003");
        //不能是银卡
        AllSet.add(_user);
    ...
    function upgradeToGold() external {
        ...
        //必须是银卡用户
        require(SilverSet.contains(_user), "e004");
        require(taskAmountList[_user] == 2, "e005");
    ...
    function getSet(uint256 _setType, address _user) external view returns (address[] memory _addressList, uint256 _num) {
        if (_setType == 0) {
