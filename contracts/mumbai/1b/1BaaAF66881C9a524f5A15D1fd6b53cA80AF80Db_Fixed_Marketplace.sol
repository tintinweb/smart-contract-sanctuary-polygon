...
contract Fixed_Marketplace is INft_Marketplace, FeeManager, IERC721Receiver {
        ...
        payable
    {
        ...
        if (IERC165(_nftAddress).supportsInterface(_INTERFACE_ID_ROYALTY)) {
            ...
            // Remove the following block 2021-08-30
            // else 
            // {
        ...
        if (FeeManager.cutPerMillion > 0) {
            ...
            if (compareStrings(order.tokenSymbol, cifi_Symbol)) {
                ...
                acceptedToken.transferFrom(
                    msg.sender, //buyer
                    owner(),
                    saleShareAmount // saleShareAmount.div(3) changed_at 2021-08-30
                );
        ...
        if (compareStrings(order.tokenSymbol, cifi_Symbol)) {
            ...
            uint256 amount =
                order.price.sub(
                    saleShareAmount // .div(3).mul(2)).sub(royaltyFeeAmount changed_at 2021-08-30
                );
            acceptedToken.transferFrom(
                msg.sender, // buyer
                order.seller, // seller
