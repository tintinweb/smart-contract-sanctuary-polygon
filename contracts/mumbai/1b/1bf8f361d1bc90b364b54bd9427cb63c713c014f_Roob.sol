...
contract Roob is ERC20("ROOB", "ROOB"), Ownable {
    ...
    mapping(address => uint256[]) public userStakedTokenCollection2;
    mapping(address => uint256[]) public userStakedTokenCollection3;
    mapping(address => uint256[]) public userStakedTokenCollection4;
    mapping(address => uint256[]) public userStakedTokenCollection5;
    mapping(address => uint256[]) public userStakedTokenCollection6;
    ...
    constructor(address _slothRoobAddress) {
        ...
        //testing
        userStakedToken[msg.sender].push(1);

        TokenData memory newTokenData;
        newTokenData.baseRate = basicReward;
        newTokenData.isStaked = true;
        newTokenData.user = 0xF1d3217f5D8368248E9AfBAd25e5396b5a93599b;

        stake[1] = newTokenData;
        //

        bytes32 lastClaimKey = keccak256(abi.encode(1));
        slothRoobLastClaim[lastClaimKey] = block.timestamp;      
    }

    function setRewardAmount(uint256 _numberOfTokensForReward) public onlyOwner{
    ...
    function claimRewardsForSloothRoob(uint256[] calldata _slothRoobTokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _slothRoobTokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stake[_slothRoobTokenId].baseRate;

            if(daysPass >=15 && daysPass < 30){
            extraBouns = ( 5 * unclaimedReward ) / 100;
            }else if (daysPass >= 30 ){
                extraBouns = ( 15 * totalUnclaimedReward ) / 100;
            }

            totalUnclaimedReward = totalUnclaimedReward + unclaimedReward + extraBouns;
        ...
        _mint(msg.sender, totalUnclaimedReward * 10 ** 18);
    ...
    function claimRewardsForCollection2(uint256[] calldata _collection2TokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _collection2TokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stakeCollection2[_slothRoobTokenId].baseRate;
    ...
    function claimRewardsForCollection3(uint256[] calldata _collection3TokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _collection3TokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stakeCollection3[_slothRoobTokenId].baseRate;
    ...
    function claimRewardsForCollection4(uint256[] calldata _collection4TokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _collection4TokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stakeCollection4[_slothRoobTokenId].baseRate;
    ...
    function claimRewardsForCollection5(uint256[] calldata _collection5TokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _collection5TokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stakeCollection5[_slothRoobTokenId].baseRate;
    ...
    function claimRewardsForCollection6(uint256[] calldata _collection6TokenIds) public returns (uint256) {
        ...
        for(uint i = 0; i < _collection6TokenIds.length; i++) {
            ...
            uint256 daysPass = unclaimedReward / stakeCollection6[_slothRoobTokenId].baseRate;
    ...
    // function getTokenStakeTime(uint256 _tokenId) public view returns (uint256) {
    ...
    function computeAccumulatedReward(uint256 _lastClaimDate, uint256 currentTime) internal pure returns (uint256) {
        ...
        uint256 accumulatedReward = 10 ** 18 * secondsElapsed / 1 days;
    ...
    function computeUnclaimedRewardSlothRoob(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stake[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function computeUnclaimedRewardCollection2(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stakeCollection2[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function computeUnclaimedRewardCollection3(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stakeCollection3[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function computeUnclaimedRewardCollection4(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stakeCollection4[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function computeUnclaimedRewardCollection5(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stakeCollection5[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function computeUnclaimedRewardCollection6(uint256 _tokenId) internal view returns (uint256) {
        ...
        uint256 daysPass = computeAccumulatedReward(lastClaimDate, block.timestamp);
            // return (daysPass * stake[_tokenId].baseRate) / 10 ** 18;
        uint256 result = daysPass * stakeCollection6[_tokenId].baseRate;   
        return (result / 10 ** 18);
        }else{
            return 0;
        }
    ...
    function setSloothRoobAddress(address _slothRoobAddress) public onlyOwner(){
	    SlothRoobContractAddress = _slothRoobAddress;
        iSlothRoob =  ISlothRoob(_slothRoobAddress);
	}

    function setCollection2Address(address _collection2Address) public onlyOwner(){
        iCollection2 =  ISlothRoob(_collection2Address);
	}

    function setCollection3Address(address _collection3Address) public onlyOwner(){
        iCollection3 =  ISlothRoob(_collection3Address);
	}

    function setCollection4Address(address _collection4Address) public onlyOwner(){
        iCollection4 =  ISlothRoob(_collection4Address);
	}

    function setCollection5Address(address _collection5Address) public onlyOwner(){
        iCollection5 =  ISlothRoob(_collection5Address);
	}

    function setCollection6Address(address _collection6Address) public onlyOwner(){
