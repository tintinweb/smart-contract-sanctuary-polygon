  ...
  ERC20Safe
{
  ...
  */
  function deposit(bytes32 resourceID, address depositer, bytes memory data) external override returns (bytes memory) {
    _onlyBridge();

    // Data decoder begin
    string memory sender; // cVa
    address recipient; // recipient
    uint8 srcTokenDecimals; // src token decimal
    uint256 amount;

    (sender, recipient, srcTokenDecimals, amount) = abi.decode(data, (string, address, uint8, uint256));
    // Data decoder end
  ...
  */
  function executeProposal(bytes32 resourceID, bytes calldata data) external override {
    ...
    (recipient, srcTokenDecimals, amount, isWrapped) = abi.decode(data, (string, uint8, uint256, bool));
    // Data decoder end

    address tokenAddress = _resourceIDToTokenContractAddress[resourceID];
    require(bytes(recipient).length == 20 && _contractWhitelist[tokenAddress], 'bad');

    // parse recipient to address type
    ...
    if (tokenAddress == address(_WETH) && !isWrapped) {
      ...
      releaseERC20(tokenAddress, recipientAddr, amount);
    }
  }
