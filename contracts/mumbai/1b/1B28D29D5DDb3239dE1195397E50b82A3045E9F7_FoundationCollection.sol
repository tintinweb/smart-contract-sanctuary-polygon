 ...
 */
contract FoundationCollection is Ownable {
  ...
  uint256 public prize = 1000000000000; // 1 ETH  should be 1000000000000000000
...
/**
  ...
  *
  */
uint256 public mintBasePrice = 9758000; //should be 9758000000000, now less for testing; 
...
/**
  ...
  *
  */
uint public marketplaceFee = 3_000;
...
address payable public gameTreasury;
  ...
  */
  uint256 public totalPrizesClaimed = 0;
...
/**
   ...
   * @param tokenIds tokenIds that were burned
...
function mintCardsCost(uint256 _amount) public view returns (uint256 cost, uint256 collectionMintFee, uint256 partnerFee) {
  ...
  uint256 collectionMintFeeWithDiscount = checkDiscount(collectionMintFee);

  partnerFee = costWithoutFees * partnerFeePercent / 100_000;

  cost =  costWithoutFees + collectionMintFeeWithDiscount + partnerFee;

  return (cost, collectionMintFeeWithDiscount, partnerFee);
}

function mintCards (uint256 _amount) public payable {
    ...
    (cost, collectionMintFee, partnerFee) = mintCardsCost(_amount);
    ...
    uint256 treasury = cost - collectionMintFee - partnerFee;
    if(treasury > 0){
    ...
    for(uint i = 0; i < _amount; i++) {
      ...
      uint256 randomNumber = uint256(rng(4294967296, i));


      // Generate a pseudorandom card token id
      uint256 tokenId = uint256(keccak256(abi.encodePacked(_amount, block.timestamp, randomNumber, msg.sender, i)));
...
function upgradeCardCost(uint256[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId) public view validCardType(_targetCardType) returns (uint256 upgradeFeeCostWithDiscount, uint256 cardsTypeValueDiffCost) {
    uint256 feeValue;
    ...
    if(_collectionId == collectionId){
      ...
      // 15482  --- 6 of Diamonds
      feeValue = prize * totalValueGivenCards * upgradeFee / 100_000 / 2;
      upgradeFeeCost = feeValue / 10**8;

     
      
      //ask Armen if prize can be 0.5
    } else{
      (,uint256 _collectionPrize,,) = card.cardCollectionInfo(_collectionId);
      uint256 collectionsRatio = _collectionPrize * 10**8 / prize;
      
      feeValue = prize * cardValue[_targetCardType] * upgradeFee / 100_000 / 2;
      upgradeFeeCost = feeValue / 10**8;
      //236 * 10 / 1000000000
    ...
    // chech for the discount
    upgradeFeeCostWithDiscount = checkDiscount(upgradeFeeCost);
    
    console.log("cardsTypeValueDiff");
    console.log(cardsTypeValueDiff);

    cardsTypeValueDiffCost = (cardsTypeValueDiff * prize / 2) / 10**8;
    ...
    return (upgradeFeeCostWithDiscount, cardsTypeValueDiffCost);
}
...
/**
   ...
   */
  function upgradeCards(uint256[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId) public payable  validCardType(_targetCardType) {
   ...
   uint32 highestCardValue = 0;
    ...
    require(_collectionPrize >= prize, "Target collection prize should be grater");
    ...
    console.log("TotalValue before");
    console.log(totalValue);
    console.log("collectionsRatio");

    console.log(collectionsRatio / 10**8);
    ...
    console.log(cardValue[_targetCardType] * collectionsRatio / 10**8);
    ...
    (uint256 upgradeFeeCost, uint256 cardsTypeValueDiffCost)= upgradeCardCost(_tokenIds, _targetCardType, _collectionId);
    ...
    // If user sent more funds than required, refund excess
    if(msg.value > upgradeFeeCost) {
      payable(msg.sender).sendValue(msg.value - upgradeFeeCost);
    }
    {
      uint256 upgradeCommission = cardsTypeValueDiffCost;
      //cache back to player
      if(cardsTypeValueDiffCost > 0){
        if(gametreasury.balance() < cardsTypeValueDiffCost){
            uint256 cardsTypeValueDiffCostFromTreasuryReserve = cardsTypeValueDiffCost - gametreasury.balance();
            treasuryreserve.giveUpgradeCacheBack(msg.sender, cardsTypeValueDiffCostFromTreasuryReserve);
            cardsTypeValueDiffCost -= cardsTypeValueDiffCostFromTreasuryReserve;
        }
        if(cardsTypeValueDiffCost > 0){
          gametreasury.giveUpgradeCacheBack(msg.sender, cardsTypeValueDiffCost);
        }
    ...
    uint256 targetTokenId = uint256(keccak256(abi.encodePacked(block.timestamp, _tokenIds, msg.sender)));
...
function calculateBuyback(uint256[] calldata _tokenIds) public view returns (uint256 floorValue, uint256 bonusEarned, uint256 buyback) {
  ...
  for(uint8 i = 1; i < payoutDeck.length; i++) {
    ...
    for(uint8 i=0;i<13;i++){
     ...
     // comboValue = comboValue;
      ...
      console.log(win_value);
      buyback = (win_value * (prize / 10**18 ) * 10**18 ) / 10**8;
  ...
  function claimBuyBack (uint256[] calldata _tokenIds) public {
   ...
   */
  function checkDiscount (uint256 _fee) internal view returns (uint256) {
      return _fee;
  }
/**
   ...
   */
  function rng(uint256 _max, uint256 _nonce) private returns (uint256) {
