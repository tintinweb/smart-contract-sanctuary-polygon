...
contract CommandCenter is IERC721ReceiverUpgradeable, Initializable, AccessControlUpgradeable, ERC721Upgradeable {
    ...
    function conquer(uint256 operatorId, uint256 weaponId) external {
        ...
        if (res.attackerXpGain > 0) {
            operators.gainXp(operatorId, res.attackerXpGain < 65535 ? res.attackerXpGain : 65535);
        }
        if (res.defenderXpGain > 0) {
            operators.gainXp(defenderOperatorId, res.defenderXpGain < 65535 ? res.defenderXpGain : 65535);
        }
    ...
    function fightOutcome(
        ...
        uint16 attackerXpGain = uint16(
            ABDKMath64x64.divu(defenderFightData.power, attackerFightData.power).mulu(baseXpGain)
        );
        uint16 defenderXpGain = uint16(
            ABDKMath64x64.divu(attackerFightData.power, defenderFightData.power).mulu(baseXpGain)
        );
        return
            FightResult(
                conquered,
