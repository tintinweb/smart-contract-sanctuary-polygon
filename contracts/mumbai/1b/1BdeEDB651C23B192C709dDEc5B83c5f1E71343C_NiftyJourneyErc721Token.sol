    ...
    IHostileMarketplaceRegistryImplementer
{
    ...
    constructor(address marketplaceRegistryAddress) {
    ...
    function toggleMarketplaceBlockList(bool updatedRegistry) public {
        _requireOnlyValidSender();
        useRegistry = updatedRegistry;
    }
    ...
    function mintMore(uint88 amount) external notFinalized {
        ...
        require(
            amount > 0,
            "Amount of nifties to add is zero; needs to be more."
        );
    ...
    function burnUnusedTokens(uint88 startTokenId) public notFinalized {
        ...
        require(
            startTokenId <= collectionStatus.amountCreated,
            "Start Token is greater than existing tokens"
        );

        require(
            maxTouchedTokenId < startTokenId,
            "You are trying to burn tokens which have already been used."
        );

        // Get editable reference to nifty type
        NiftyType storage niftyTypeDetail = niftyTypes[
    ...
    function transferFrom(
        ...
        if (_msgSender() != owner) {
            // This would block marketplaces that were added to the registry after the user called approve() or setApprovalForAll(),
            // at the expense of some gas on each transfer.  However, we should never prevent a transfer by the token owner
    ...
    function _transfer(
        ...
        // Block transfers to blocked custodial marketplaces or brokers.
