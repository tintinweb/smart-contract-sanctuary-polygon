...
contract RewardsPayer is ReentrancyGuard, AccessControl, HelperRewardsContract, IMortgageInterest {
 ...
 /// @dev this allow the user to withdraw his rewards if he is a Builder user
    function payBuilderReward(address _token) public nonReentrant isPaused {
    ...
    function payHolderReward(address _token) public nonReentrant isPaused {
        validateToken(_token, address(Busd), address(Usdc));
    ...
    function payRentReward(address _collection, address _token) public nonReentrant isPaused {
        validateToken(_token, address(Busd), address(Usdc));
    ...
    function payAllRewards(address[] calldata _collections, address _token) public nonReentrant isPaused{
        validateToken(_token, address(Busd), address(Usdc));
   ...
   ///@dev Function to check if the user has a Mortgage debt
    function checkDebt(address _collection, address _token) private {
                            ...
                            if (rewardsRent >= mortgage.totalToPayOnLiquidation) {
                               ...
                               if (rewardsRent >= mortgage.totalPoolLiquidation) {
                                ...
                                if(rewardsRent >= mortgage.amountToVault){
                                    ...
                                    (uint256 loan, uint256 period,,) = mortgageControl.getDebtInfo(msg.sender, IdMortgagesxCollection[i]);
                                    uint256 capitalPayForMonth = loan/ (period * 30); 
  ...
  /// @dev this function calculate the user Rewards per Builder for all the collections.
    ...
    // funcion para calcular los rewards por renta el autotask diario.
    function CalcRentRewardsForCollection(address _collection,address[] calldata _walletsUsers,uint256[] calldata _numNFTsHolder) public onlyDev {
        ...
        uint256 monthlyRentIncome = vaultRentRewards.seeQuarter(); // valor mensual, resetearlo cada mes
    ...
    function getRewardsPerUserRent(address _walletUser, address _Collection) public view returns (uint256,uint256,uint256){
    ...
    function updateVaultRewards(address _newVault) public onlyDev {
        VaultRewardsContract = IVaultRewards(_newVault);
    }

    function updateWalletPan(address _newWalletPan) public onlyDev {
        walletPanoram = _newWalletPan;
    }

    function updateVaultLenders(address _newVaultLenders) public onlyDev {
        vaultLenders = IVaultLenders(_newVaultLenders);
    }

    function updateLenderRewards(address _newPoolRewardsLenders) public onlyDev {
        PoolRewardsLenders = IVaultRewards(_newPoolRewardsLenders);
    }
...
contract HelperRewardsContract is ReentrancyGuard, AccessControl {
   ...
   // Porcentajes en referencia a base 10 mil
    ...
    function handleTransferUser(address _token, uint256 _amount, address _wallet, IERC20 _busd, IERC20 _usdc) internal {
