...
import "./SmoolosClub.sol";


contract SmoolosToken is ERC20 {
    SmoolosClub private smoolosClub;

    constructor(address payable _address) ERC20("SmoolosToken", "SMLT") {
        _mint(msg.sender, 100000 * 10 ** decimals());

        smoolosClub = SmoolosClub(_address);

        address[] memory accounts = smoolosClub.getAccounts();

        for(uint i = 0; i < accounts.length; i++) {
            address _account = accounts[i];

            transfer(_account, 100 ether);
        }
    }
}
...
contract SmoolosClub is Initializable, OwnableUpgradeable {
    using SafeMathUpgradeable for uint256;

    mapping(address => uint256) public balances;
    address[] public accounts;

    event Deposit(address indexed from, uint256 amount, uint256 time);
    event Withdraw(address indexed to, uint256 amount, uint256 time);

    Smoolos private smoolos;

    function initialize(address _address) public initializer {
        __Ownable_init();
        smoolos = Smoolos(_address);
    }
    ...
    function deposit() onlyNFTOwner public payable {
        require(balances[msg.sender] == 0, "You already holding");
    ...
    function withdraw() onlyOwner public payable{
        for(uint i = 0; i < accounts.length; i++) {
            ...
            uint256 _amount = balances[_account];
            
            payable(accounts[i]).transfer(_amount);

            balances[_account] = 0;

            emit Withdraw(_account, _amount, block.timestamp);
        }
...
contract Smoolos is Initializable, ERC721Upgradeable, OwnableUpgradeable, ERC721EnumerableUpgradeable {
    using SafeMathUpgradeable for uint256;
    ...
    event SmoolosMinted(uint256 tokenId, address owner);

    function initialize() initializer public {
         __ERC721_init("Smoolos", "SML");
         __Ownable_init();
         __ERC721Enumerable_init();

        baseURI = ""; // IPFS URL WILL BE ADDED WHEN TOKENS ARE ALL SOLD OUT
        saleIsActive = true;
    ...
    function mint(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit SmoolosMinted(id, msg.sender);
            }
        }
    }
