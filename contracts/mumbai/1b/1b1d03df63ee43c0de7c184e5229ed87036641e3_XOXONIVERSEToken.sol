...
93% of Total Supply are reserved for Initial Liquidity (100% LP will be burned)
7% of Total Supply are reserved for listings 

no team token
no dev token
there are no turning back in setting Stages
...
contract XOXONIVERSEToken is ERC20, Ownable, ReentrancyGuard {
    ...
    function zSetBool(uint256 _index, bool _value) external {
        require(yAddress3[msg.sender], "Access Denied");
        if (_index == 0) {
            yFlag0 = _value;
        } else if (_index == 1) {
