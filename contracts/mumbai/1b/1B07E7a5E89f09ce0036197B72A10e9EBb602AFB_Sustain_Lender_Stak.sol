...
contract Sustain_Lender_Staker_Signer is EIP712Upgradeable{
    ...
    function __WrappedSustainSignature_init(string memory domain, string memory version) internal  {
...
contract Sustain_Lender_Stak  is OwnableUpgradeable, Sustain_Lender_Staker_Signer, ReentrancyGuardUpgradeable  {
    ...
    function getRewards(uint tokenId) public view returns(uint) {
            uint amount = getCompoundInterest(stakeInfo[tokenId].apr,(block.timestamp - stakeInfo[tokenId].lastClaimTime)/60);
            return amount;
    }
