...
import "../interfaces/IVvsFarmingPool.sol";
import "../interfaces/IProfitDistributor.sol";

contract StrategyVvsLpCroVvs is Ownable, ReentrancyGuard, Pausable, IStrategy {
    ...
    address public router = address(0xC60aE14F2568b102F8Ca6266e8799112846DD088); // VVS Router
    ...
    uint256 public autoEarnLimit = 100 ether; // $500
    uint256 public autoEarnDelaySeconds = 24 hours;
    ...
    function getName() public pure returns (string memory) {
        return "V5S.Finance:StrategyVvsLpCroVvs";
    }
    ...
    function _farm() internal {
        ...
        IVvsFarmingPool(farmingPool).deposit(farmingPoolId, _wantBal);
    }

    function _withdrawSome(uint _amount) internal {
        IVvsFarmingPool(farmingPool).withdraw(farmingPoolId, _amount);
    }

    function _exit() internal {
        IVvsFarmingPool(farmingPool).withdraw(farmingPoolId, inFarmBalance());
    }

    function _emergencyExit() internal {
        IVvsFarmingPool(farmingPool).emergencyWithdraw(farmingPoolId);
    }

    function inFarmBalance() public view returns (uint256 _amount) {
        (_amount, ) = IVvsFarmingPool(farmingPool).userInfo(farmingPoolId, address(this));
    }

    function _harvest() internal {
        return IVvsFarmingPool(farmingPool).withdraw(farmingPoolId, 0);
    }

    function pendingHarvest() public view returns (uint256) {
        return IVvsFarmingPool(farmingPool).pendingVVS(farmingPoolId, address(this));
    }
    ...
    function setProfitDistributor(address _profitDistributor) external onlyOwner {
        require(_profitDistributor != address(0), "invalidAddress");
    ...
    function executeTransaction(
        ...
        require(success, "StrategyVvsLpCroVvs::executeTransaction: Transaction execution reverted.");
