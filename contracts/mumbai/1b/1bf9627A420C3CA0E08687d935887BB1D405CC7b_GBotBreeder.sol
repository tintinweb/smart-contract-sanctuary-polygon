...
uint256 internal constant BABY_LOCK_TIME = 48 hours;
...
interface IGBotCalculations {
    ...
    function determineEvolutionOmpCost(uint256 rarity, uint256 currentCop) external pure returns (uint256 ompCost);
    ...
    function determineBreedingOmpCost(uint256[] memory parentsMetadatas) external pure returns (uint256 ompCost);
}
...
contract GBotBreeder is ERC721Receiver, Ownable, Pausable, GBotBabyLock, BreedingProperties {
  ...
  event BreedingStarted(address indexed owner, uint256[] parents, uint256 tokenId, uint256 lockTimestamp);
    ...
    function breed(address sender, bytes calldata sig, uint256[] memory tokenIds) public virtual whenNotPaused {
        ...
        uint256 cost = GBotCalculations.determineBreedingGmeeCost(parentsEnergyCores);
        ...
        // Lock Baby GBot
          _lockToken(BABY_LOCK_TIME, babyBotMetadata);
        // Save Baby GBot tokenId to user
        addRightfulOwner(_msgSender(), babyBotMetadata);

        counter++;
        nonces[_msgSender()] ++;
        emit BreedingStarted(_msgSender(), tokenIds, babyBotMetadata, block.timestamp + BABY_LOCK_TIME);
    }
