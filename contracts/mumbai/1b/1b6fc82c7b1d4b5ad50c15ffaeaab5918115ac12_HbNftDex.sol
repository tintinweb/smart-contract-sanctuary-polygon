...
contract HbNftDex {
    ...
    uint256 public maxRoyaltyRate = 1000;
    ...
    function setAllNftAllowed(bool _allNftAllowed) public onlyDAO {
        allNftAllowed = _allNftAllowed;
    }

    function addAllowedNft(address _contract) public onlyDAO {
        if(!allowedNft[_contract]) {
            allowedNft[_contract] = true;
        }
    }

    function removeAllowedNft(address _contract) public onlyDAO {
        if(allowedNft[_contract]) {
            delete allowedNft[_contract];
        }
    }

    function addAllowedPayment(address _contract) public onlyDAO {
        if(!allowedPayment[_contract]) {
            allowedPayment[_contract] = true;
        }
    }

    function removeAllowedPayment(address _contract) public onlyDAO {
        if(allowedPayment[_contract]) {
    ...
    function exchangeFixedPrice(
        ...
        (bytes32 order_digest, bytes memory order_bytes) = checkSignature(taker, order, maker_sig, taker_sig);
        ...
        swapToken(order, nft_seller, nft_buyer);
    ...
    function checkSignature(
        ...
        require(
            ...
            !canceledOrder[maker][order_digest] &&
            !canceledOrder[taker][order_digest],
    ...
    function swapToken(FixedPriceOrder memory order, address nft_seller, address nft_buyer) private {
        require(allowedPayment[order.payment_token], "Payment token contract is not supported");
    ...
    function cancelOrder(bytes32 order_hash) public {
        canceledOrder[msg.sender][order_hash] = true;
