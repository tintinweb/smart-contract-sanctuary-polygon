...
contract Mint_Scavenger_Airdrop_ is
    Initializable,
    ...
    OwnableUpgradeable
{
    ...
    mapping(uint256 => uint256) private counters;
    mapping(uint256 => uint256) private counters_max;

    function initialize(
        address _server_address,
        uint256[] memory _min_arr,
        uint256[] memory _max_arr,
        string memory token_name_symbol
    ) public initializer {
        server_address = _server_address;

        for (uint256 i = 1; i <= _min_arr.length; i++) {
            counters[i] = _min_arr[i - 1];
            counters_max[i] = _max_arr[i - 1];
        }
    ...
    function mint(
        address addr,
        string memory url,
        uint256 counters_key
    ) public {
        // require(msg.sender == server_address, "Address mismatch");
        require(counters[counters_key] > 0, "Out of bounds key");
        require(
            counters[counters_key] <= counters_max[counters_key],
            "Exceeded max"
        );
        _safeMint(addr, counters[counters_key]);
        _setTokenURI(counters[counters_key], url);
