...
contract KwikTrustFactory is AccessControlUpgradeable {
  // Roles role new contract deployment
  bytes32 public constant DEPLOYER_ROLE = keccak256("DEPLOYER_ROLE");

  // mapping to store contract address against contract name
  mapping(string => KwikTrustNFT) public nftContractAddress;
  ...
  function __KwikTrustFactory_init() external initializer {
   ...
   * @param _validatorRole lazyminting validator role
   */

  function deployMintingContract(
    string memory _name,
    string memory _symbol,
    address _validatorRole
  ) public {
    require(hasRole(DEPLOYER_ROLE, msg.sender), "Unauthorized admin");
    KwikTrustNFT nftContract = new KwikTrustNFT(_name, _symbol, msg.sender, _validatorRole);
...
contract KwikTrustNFT is ERC721URIStorage, EIP712, AccessControl {
    ...
    event HandleLazy(address _minter);
    event HandleBulkMint(address _minter, uint256 id);
     ...
     * @param _name Name of Smart Contract
     * @param _symbol Symbol
     * @param _minter Admin role
     * @param _validator Lazy minting validator Role
     */
    ...
    constructor(
        ...
        _setupRole(DEFAULT_ADMIN_ROLE, _minter);
        ...
        returns (uint256)
    {
        ...
        emit HandleBulkMint(msg.sender, tokenId);
        return tokenId;
    }
     ...
     * @param _signature signature of lazy minted NFT
     * @param _token ERC20: token address
     * @param _marketplace NFT Marketplace contract address
    ...
    function lazyMint(
        ...
        emit HandleLazy(verifyValidator);
        require(hasRole(VALIDATOR_ROLE, verifyValidator), "KwikTrust: Invalid signature");
        tokenId++;
        _safeMint(_signer, tokenId);
        _setTokenURI(tokenId, voucher.uri);
        _transfer(_signer, msg.sender, tokenId);
        IERC20(voucher.token).transferFrom(msg.sender, _signer, voucher.price);
        setApprovalForAll(_marketplace, true);
        emit HandleLazyMint(_signer, msg.sender, voucher.uri, tokenId);
        return tokenId;
    }
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "NFTVoucher(string uri,uint256 price,address token)"
                        ),
