...
// File: contracts/N7PaymentSplitter.sol



    // OpenZeppelin Contracts v4.4.1 (finance/PaymentSplitter.sol) -- Modified 
    ...
    */
    ...
    contract N7PaymentSplitter is Context {
...
// File: contracts/N7Collection.sol
...
contract N7Collection is ERC721Enumerable, N7PaymentSplitter, Ownable {
    ...
    uint256 public constant BUY_LIMIT_PER_TX = 1000000;

    uint256 private constant MAX_NFT = 1000;



    uint256 public NFTPrice = 10000000000000000; //0.01 ether;   
    ...
    int256 public tn_lastSold = -1;



    address[] private contributors = [0xC599FCb8f9A3BaC613ab076c8965c601bB86Fcc9,
                                      0xE977503E70e34164dF50291A5742Cb5C633bd142,
                                      0x0Fb64d905263d10c876eBFb9419028af713671B0
                                     ];

    uint256[] private sharesContributors = [5000, 4500, 500];
    ...
    */
    ...
    //    require((totalSupply() + _numOfTokens) - giveawayCount <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
	 require((_tn_distributed() + _numOfTokens)   <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
    ...
    /*
    * Function to set NFT price per unit 
    */ 
    ...
    onlyOwner 
  {
    ...
    /*
    * Function to get NFT price per unit 
    */ 
    ...
    view returns (uint256)
  {
    ...
    function _tn_distributed()
    internal view virtual returns (uint256)
    {
        //return totalSupply()-giveawayCount;
        return totalSupply();
    }

    function  _tn_mint( address to)
        internal virtual
    {
        
        // recherche un "non distribué" dans la table de distribution
        // l'allocation directe par le vendeur creant des discontinuité de table
        uint256 tokenId = 0;
        if (tn_lastSold>-1)     tokenId = uint256(tn_lastSold)+1;

        for(; tokenId <= MAX_NFT; tokenId++){
            ...
            if(!_exists(tokenId))
            {
                ...
                tn_lastSold= int256(tokenId);	// dernier vendu
                return;
            }
        }
    ...
    */ 
...

    {
        ...
        require(isActive, "Nefty : Contract is not active");
        require(_numOfTokens <= BUY_LIMIT_PER_TX, "Nefty : Cannot mint above limit");
        require(_numOfTokens > 0, "Nefty : Cannot mint 0");
        require(nftClaimed[msg.sender] <= BUY_LIMIT_PER_TX, "Nefty : Cannot mint above limit for one address");
        //require((totalSupply() + _numOfTokens) - giveawayCount <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
        require((_tn_distributed() + _numOfTokens)   <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
        require(msg.value >= (NFTPrice * _numOfTokens), "Nefty : Price highter than offer");
    ...
    /*
    * Function to mint all NFTs for giveaway and partnership
    */
    ...
    * Function to mint all NFTs for giveaway and partnerships
...

    {
        ...
        require((_tn_distributed() + _tokenId.length)   <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
    ...
    */
    ...
    /*
    * Function to mint all NFTs for set to owner
    */
    ...
    {
        require((_tn_distributed() + (_tokenIdTo-_tokenIdFrom+1) )   <= MAX_NFT, "Nefty : Purchase would exceed max public supply of NFTs");
	
 
        for(uint256 tokenId = _tokenIdFrom; tokenId <=_tokenIdTo; tokenId++){
	
			require(!_exists(tokenId), "Nefty: token already minted");
		}

 
        for(uint256 tokenId = _tokenIdFrom; tokenId <=_tokenIdTo; tokenId++){

            require(tokenId < MAX_NFT, "Nefty : Tokens number to mint cannot exceed number of MAX tokens");
