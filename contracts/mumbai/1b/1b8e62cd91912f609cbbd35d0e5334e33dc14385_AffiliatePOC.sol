...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        // hardcode this for simplicity of demo
        return "https://res.cloudinary.com/dispatchxyz/image/upload/v1675795024/SDK%20Docs/White_Tshirt_ohaq4f.png";
    }
...
contract AffiliatePOC is ERC721, ReentrancyGuard, Signable, AccessControl {
  bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
  event SaleEvent(string indexed rId, uint256);
  ...
  IArtOfXMintPass private _artOfXMintPass;
  ...
  function ownerMint(address to, uint256 amount) external onlyOwner nonReentrant {
    if (_nextTokenCount + amount - 1 > maxSupply) revert Errors.SupplyLimitReached();
  ...
  function preSaleMint(
    ...
    string memory rId
  ) external payable costs(amount) phaseRequired(Phase.PRE_SALE) nonReentrant {
    if (!_verify(signer(), _hash(msg.sender, maxAmount), signature)) revert Errors.InvalidSignature();

    if (minted[msg.sender] + amount > maxAmount) revert Errors.AccountAlreadyMintedMax();

    _mintLogic(amount, rId);
    // comment out for simplicity
    // _mintArtOfXLogic(msg.sender, amount);
  }

  // This function was modified to showcase the PoC
  function mint(uint256 amount, string memory rId)
    external
    payable
    costs(amount)
    ...
    nonReentrant
  {
    if (minted[msg.sender] + amount > mintsPerAccountOnPublicSale) revert Errors.AccountAlreadyMintedMax();
  ...
  function reduceMaxSupply(uint256 newMaxSupply) external onlyOwner {
    if (newMaxSupply < _nextTokenCount - 1) revert Errors.MaxSupplyTooSmall();

    if (newMaxSupply > orginalMaxSupply) revert Errors.CanNotIncreaseMaxSupply();
  ...
  function setContractURI(string calldata baseContractURI_) external onlyOwner {
    _baseContractURI = baseContractURI_;
  }
  ...
  function setArtOfXMintPass(address artOfXMintPass_) external onlyOwner {
    _artOfXMintPass = IArtOfXMintPass(artOfXMintPass_);
  }
  ...
  function _mintLogic(uint256 amount, string memory rId) private {
    if (_nextTokenCount + amount - 1 > maxSupply) revert Errors.SupplyLimitReached();
    ...
    // assign none when empty
    if (bytes(rId).length == 0) {
      rId = "none";
    }
    // emit an event which contains the referral/affiliateId and the amount
    emit SaleEvent(rId, amount);
  }
  ...
  function _mintArtOfXLogic(address to, uint256 amount) private {
    if (address(_artOfXMintPass) == address(0)) revert Errors.IArtOfXMintPass();

    _artOfXMintPass.mint(to, amount);
  }
}