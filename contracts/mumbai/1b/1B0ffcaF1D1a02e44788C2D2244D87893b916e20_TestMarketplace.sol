    ...
    ERC721HolderUpgradeable
{
    ...
    function createAsk(
        ...
        require(IERC721Upgradeable(_nftAddress).ownerOf(_tokenId) == msg.sender, "YOU DO NOT HAVE THIS NFT");
        require(
            IERC721Upgradeable(_nftAddress).isApprovedForAll(msg.sender, address(this)) ||
                IERC721Upgradeable(_nftAddress).getApproved(_tokenId) == address(this),
            "NO APPROVEMENT FOR THIS TOKEN"
        );

        IERC721Upgradeable(_nftAddress).safeTransferFrom(msg.sender, address(this), _tokenId);

        asks[_nftAddress][_tokenId] = Ask({ seller: msg.sender, askPrice: _askPrice });
        tokenIdsPerNft[_nftAddress].add(_tokenId);

        emit NewAskCreated(_nftAddress, _tokenId, msg.sender, _askPrice);
    }

    function buyNft(address _nftAddress, uint256 _tokenId) external payable whenNotPaused nonReentrant {
        require(tokenIdsPerNft[_nftAddress].contains(_tokenId), "THIS TOKEN IS NOT FOR SALE");
        ...
        (address receiver, uint256 royaltyAmount) = IERC2981(_nftAddress).royaltyInfo(_tokenId, _ask.askPrice);
    ...
    function updateTransactableNft(address _nftAddress, bool _isTransactable) external onlyOwner {
