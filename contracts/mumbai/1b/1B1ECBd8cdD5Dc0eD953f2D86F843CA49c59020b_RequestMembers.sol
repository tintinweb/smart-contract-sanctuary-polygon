...
/// @notice Contract stored user requests.
/// @dev 
contract RequestMembers {
    ...
    /// @notice Array of user requests.
    /// @dev Array of user requests.
    address[] public requestedAccounts;

    /// @notice Map user request address to statu.
    /// @dev Map user request address to statu.
    mapping(address => bool) requestedAccountsMap;

    /// @notice Event emmited when new request from user is sended.
    /// @dev Event emmited when new request from user is sended.
    /// @return status True/False - status of excution.
    /// @return message Information with Error or Successfull execution.
    /// @return account Account sended request.
    event RequestAdd(bool status,string message, address account);

    /// @notice Event emmited when new request is approved by Admin.
    /// @dev Event emmited when new request is approved by Admin.
    /// @return status True/False - status of excution.
    /// @return message Information with Error or Successfull execution.
    /// @return account Approved Account.
    event RequestApprove(bool status,string message, address account);

    /// @notice Event emmited when new request is deleted by Admin.
    /// @dev Event emmited when new request is deleted by Admin.
    /// @return status True/False - status of excution.
    /// @return message Information with Error or Successfull execution.
    /// @return account Deleted account.
    event RequestDelete(bool status,string message, address account);

    /// @notice Constructor.
    /// @dev  Constructor.
    /// @param roleContract Address UsersGroups Contract.
    constructor (address roleContract) public  {
    ...
    /// @notice Get all request.
    /// @dev Get all request.
    /// @return address_array Array of user's address requests.
    ...
    /// @notice Add user Request.
    /// @dev Add user Request.
    /// @param _address Account Address.
    /// @return status True/False of execution.
    function addToQueueRequestedSingle(address _address)
    public
    returns (bool)
    {
        if(!roles.isMember(msg.sender) && !ifRequestedAccounts(_address)){
            requestedAccounts.push(_address);
            requestedAccountsMap[_address] = true;
            emit RequestAdd(true,"User added",_address);
            return true;
        }
        else
        {
            emit RequestAdd(false,"User is already memeber or is in request queue.",_address);
            return false;
        }
    }

    /// @notice Accept User request.Set user permissions and add to Member Group.
    /// @dev Accept User request. Set user permissions and add to Member Group.
    /// @param _address Account Address.
    /// @return status True/False of execution.
    ...
    public
    returns (bool) {
        if(!roles.isMember(_address) && ifRequestedAccounts(_address)){
                for(uint i=0;i < requestedAccounts.length; i++){
                    if(requestedAccounts[i] == _address){
                        ...
                        roles.addUser(_address);
                        emit RequestApprove(true,"User added",_address);
                        return true;
                    }
        ...
        {
            emit RequestApprove(false,"User is Member or is not in queue.",_address);
            return false;
        }
    }

    /// @notice Validate if user request exist.
    /// @dev Validate if user request exist.
    /// @param _address Account Address
    /// @return status True/False of exist
    ...
    /// @notice Delete user request.
    /// @dev Delete user request.
    /// @param _address Account Address.
    /// @return status True/False of status execution.
    function deleteRequestedAccounts(address _address)
    onlyAdmin
    public
    returns (bool) {
        if(ifRequestedAccounts(_address)){
                for(uint i=0;i < requestedAccounts.length; i++){
                    if(requestedAccounts[i] == _address){
                            for(uint ii = i; ii < requestedAccounts.length-1; ii++){
                                requestedAccounts[ii] = requestedAccounts[ii+1];      
                        ...
                        delete requestedAccountsMap[_address];
                        requestedAccounts.pop();
                        emit RequestDelete(true,"User was deleted from request queue..",_address);
                        return true;
                    }
        ...
        {
            emit RequestDelete(false,"User is not in request queue..",_address);
            return false;
        }
    }
