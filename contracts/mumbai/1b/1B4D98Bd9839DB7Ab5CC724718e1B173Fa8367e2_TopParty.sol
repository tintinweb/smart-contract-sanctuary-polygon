    ...
    UUPSUpgradeable
{
    ...
    function getDeposits(address _contractAddress)
        external
        view
        contractNotPaused(_contractAddress)
        returns (Deposit[] memory _items)
    {
        uint256 _itemsCount = depositPools[_contractAddress].length - currentPoolStartAt[_contractAddress];
        _items = new Deposit[](_itemsCount);
        uint32 _itemsIdx = 0;
        for (uint32 _poolIdx = currentPoolStartAt[_contractAddress]; _poolIdx < depositPools[_contractAddress].length; _poolIdx ++) {
    ...
    function getWithdrawable()
        external
        view
        returns (WithdrawableItem[10] memory _items)
    {
        // capped at 10 items
        ...
        for (
            ...
            if (withdrawable > 0) {
                _items[_itemIdx] = WithdrawableItem({
                    contractAddress: _contractAddress,
                    amount: withdrawable
                });
                _itemIdx += 1;
