 ...
 */
contract Biggytoken is Ownable, Stakeable {
    /**
    ...
    function _mint(address account, uint256 amount) internal {
        require(
            account != address(0),
            "biggyToken: cannot mint to zero address"
        );
    ...
    function _burn(address account, uint256 amount) internal {
        require(
            account != address(0),
            "biggytoken: cannot burn from zero address"
        );
        require(
            _balances[account] >= amount,
            "biggytoken: Cannot burn more than the account owns"
        );
    ...
    function _transfer(
        ...
        require(sender != address(0), "biggytoken: transfer from zero address");
        require(
            recipient != address(0),
            "biggytoken: transfer to zero address"
        );
        require(
            _balances[sender] >= amount,
            "biggytoken: cant transfer more than your account holds"
        );
    ...
    function _approve(
        ...
        require(
            owner != address(0),
            "biggytoken: approve cannot be done from zero address"
        );
        require(
            spender != address(0),
            "biggytoken: approve cannot be to zero address"
        );
    ...
    function transferFrom(
        ...
        require(
            ...
            "biggytoken: You cannot spend that much on this account"
        );
        // Transfer first
    ...
    function stake(uint256 _amount) public {
        ...
        require(
            ...
            "biggytoken: Cannot stake more than you own"
        );
    ...
    function withdrawStake(uint256 amount, uint256 stake_index) public {
        require(
            block.timestamp > (end + 7 days),
            "you can only withdraw after a week boss"
        );
