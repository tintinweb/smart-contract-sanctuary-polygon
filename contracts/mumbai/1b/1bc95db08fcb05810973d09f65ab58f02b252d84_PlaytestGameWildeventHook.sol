...
contract PlaytestGameWildeventHook is IWildeventHook {
    event PlaytestGameComplete(
        uint32[] wildfileIds, uint64 playtestWildeventId, uint32 winnerWildfileId, uint32 loserWildfileId
    );
    ...
    function onWildevent(uint32[] calldata wildfileIds, bytes calldata data) external override {
        ...
        (uint64 playtestWildeventId, uint32 winnerWildfileId, uint32 loserWildfileId) = decode(data);
        emit PlaytestGameComplete(wildfileIds, playtestWildeventId, winnerWildfileId, loserWildfileId);
    }

    function encode(uint64 playtestWildeventId, uint32 winnerWildfileId, uint32 loserWildfileId)
        public
        pure
        returns (bytes memory data)
    {
        data = bytes.concat(data, LibEncode.encodeUint64(playtestWildeventId));
        data = bytes.concat(data, LibEncode.encodeUint32(winnerWildfileId));
        data = bytes.concat(data, LibEncode.encodeUint32(loserWildfileId));
        return data;
    }

    function decode(bytes memory data) public pure returns (uint64, uint32, uint32) {
        uint40 offsetBytes = 0;
        uint64 playtestWildeventId;
        uint32 winnerWildfileId;
        uint32 loserWildfileId;
        (playtestWildeventId, offsetBytes) = LibEncode.decodeUint64(data, offsetBytes);
        (winnerWildfileId, offsetBytes) = LibEncode.decodeUint32(data, offsetBytes);
        (loserWildfileId, offsetBytes) = LibEncode.decodeUint32(data, offsetBytes);

        return (playtestWildeventId, winnerWildfileId, loserWildfileId);
    }
}
