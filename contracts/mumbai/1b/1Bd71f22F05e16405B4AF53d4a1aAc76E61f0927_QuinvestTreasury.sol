...
contract QuinvestTreasury is Pausable, Ownable, ReentrancyGuard {
    ...
    // reward cycle for production a week
    uint256 public rewardCycle = 2 * 60;
    ...
    function stakeToken(
        ...
        require(
            stableToken.balanceOf(_msgSender()) >=
                (stakeAmount * 10 ** usdtDecimal) / (10 ** qnvDecimal),
    ...
    function redeemUSDT() external whenNotPaused {
        ...
        stableToken.transfer(
            _msgSender(),
            (rQNVToken.balanceOf(_msgSender()) * 10 ** usdtDecimal) /
                (10 ** qnvDecimal)
        );
        rQNVToken._burn(_msgSender(), rQNVToken.balanceOf(_msgSender()));
    ...
    function withdrawFunds(uint256 amount) external onlyOwner {
        require(
            stableToken.balanceOf(_msgSender()) >
                ((amount * 10 ** usdtDecimal) / (10 ** qnvDecimal)),
            "Inficiant funds"
        );
        stableToken.transfer(
            _msgSender(),
