...
contract DaoContract is BaseContract {
    ...
    */
    ...
    function _distributeWeekReward(string memory _type) private {
        ...
        if (_canDistribute) {
            lastWeekDataList[_type].rewardAmount = _amount;
            // 对第一名进行奖励分配
            userInfoList[firstAddress]._userDataItem._updatedWeeKReward += _amount;
            weekReWardList[_type] -= _amount;
            lastWeekDataList[_type].remainingRewardAmount = weekReWardList[_type];
        } else {
            //不分配是不是留到下一次或者截留
        }
        uint256 _gas1 = gasleft();
        ...
        lastWeekDataList[_type].weekAddressList = weekAddressList[_type];
        lastWeekDataList[_type].weekDayList = weekDayList[_type];
        lastWeekDataList[_type].weekDataList = weekDataList[_type];
