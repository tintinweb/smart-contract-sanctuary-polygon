...
contract Pool is VersionedInitializable, PoolStorage, IPool {
  ...
  function repay(
    ...
    (uint256 repay, uint256 userBorrowBalance) = BorrowLogic.executeRepay(
        _reserves,
    ...
    if (userBorrowBalance == 0) _removeBorrower(msg.sender);
    return repay;
  }
  ...
  function checkAndLiquidate(address asset) public virtual override {
    uint256 len = _borrowersList.length;
    for (uint256 i = 0; i < len; i++) {
      address user = _borrowersList[i];
      ...
      if (isLiquidated) _removeBorrower(user);
    }
  }
  ...
  function _addBorrower(address userAddress) internal {
    if (_borrowerIds[userAddress] == 0) {
      _borrowersList.push(userAddress);
      _borrowerIds[userAddress] = _borrowersList.length;
    }
  }

  function _removeBorrower(address userAddress) internal {
    uint256 id = _borrowerIds[userAddress];
    if (id > 0) {
      _borrowerIds[userAddress] = 0;
      if(id != _borrowersList.length) {
        address last = _borrowersList[_borrowersList.length - 1];
        _borrowersList[id - 1] = last;
        _borrowerIds[last] = id;
      }
      _borrowersList.pop();
    }
  }
