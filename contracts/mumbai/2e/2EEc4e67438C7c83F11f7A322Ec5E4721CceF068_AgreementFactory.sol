...
import "./AgreementsPayMaster.sol";
import "@openzeppelin/contracts/utils/structs/EnumerableSet.sol";

contract AgreementFactory is BaseRelayRecipient  {
    using EnumerableSet for EnumerableSet.AddressSet;

    // Vars:
    AgreementsPayMaster private paymaster;

    // Storage:
    EnumerableSet.AddressSet private _newAgreements; // Mapping of agreements deployed:
    mapping (address => string) public baseURIs; // Mapping of the baseURI of the agreements deployed:
    ...
    event AgreementCreated(
        ...
        uint8 maxTokens, 
        uint256 endTime, 
        bool isPrivate); // Event emitted when an agreement is created.

    // Constructor:
    constructor(address _forwarder, address payable _paymaster) {
        _setTrustedForwarder(_forwarder);
        paymaster = AgreementsPayMaster(_paymaster);
    }

    function getAgreementAddress(
        string memory name,
    ...
    function createAgreement(
        ...
        Agreement agreement = new Agreement{salt: _salt}(
            name,
            contractURI,
            maxTokens,
            endTime,
        ...
        _newAgreements.add(address(agreement));
        baseURIs[address(agreement)] = string(bytes.concat(bytes('ipfs://'),bytes(contractURI)));
        paymaster.addNewAgreement(address(agreement));
        emit AgreementCreated(agreement, name, contractURI, maxTokens, endTime, isPrivate);
    }

    function getAgreements() public view returns(address[] memory) {
        return _newAgreements.values();
    }
...
contract Agreement is ERC721A, Pausable, AccessControl, ERC721ABurnable, BaseRelayRecipient {
    //Roles:
    ...
    // Vars:
    string public _contractURI = ""; // URI of the contract.
    uint256 public immutable _maxTokens; // Maximum number of tokens that can be created.
    uint256 public immutable _endTime; // Timestamp when the agreement ends.
    bool public immutable visibility; // Visibility of the agreement.
    ...
    // Errors:
    error EndTimeMustBeGreaterThanToday();
    error ContractURINotEmpty();
    error AgreementHasEnded();


    constructor(
        string memory name,
        string memory contractIPFSHash, // This must be hash of IPFS without schema 'ipfs://'
        uint256 maxTokens,
        uint256 endTime,
        bool _visibility,
        address owner,
        ...
        if (endTime > 0 && endTime <= block.timestamp) revert EndTimeMustBeGreaterThanToday();
        if (bytes(contractIPFSHash).length <= 0) revert ContractURINotEmpty();
        _grantRole(DEFAULT_ADMIN_ROLE, owner); // Grant admin role to the deployer.
        _grantRole(PAUSER_ROLE, owner); // Grant the pauser role to the deployer.
        _grantRole(MINTER_ROLE, owner); // Grant the minter role to the deployer.
        _contractURI = string(bytes.concat(bytes('ipfs://'),bytes(contractIPFSHash))); // Setting the contract URI.
        _maxTokens = maxTokens; // Setting the maximum number of tokens that can be created.
        _endTime = endTime; // Setting the end time.
        visibility = _visibility; // Setting the visibility.
        _safeMint(owner, _maxTokens);
    }
    ...
    /**
     * @dev This overrides to start with tokenID 1.
     */
    ...
    /**
     * @dev In this version we want that all tokens returns contractURI.
     */
    ...
    function pause() public onlyRole(PAUSER_ROLE) {
        _pause();
        emit ContractPaused(_msgSender()); // Emitting the pause event.

    }

    function unpause() public onlyRole(PAUSER_ROLE) {
        _unpause();
        emit ContractUnpaused(_msgSender()); // Emitting the unpause event.
    ...
    function _beforeTokenTransfers(address from, address to, uint256 startTokenId, uint256 quantity)
        ...
        override (ERC721A)
    {
        // If agreement has expired transfers are't allowed
        if (_endTime > 0 && block.timestamp > _endTime) revert AgreementHasEnded();
     ...
     */
    function _msgSender() internal override(BaseRelayRecipient, Context) view returns (address ret) {
     ...
     */
    function _msgData() internal override(BaseRelayRecipient, Context) view returns (bytes calldata ret) {
        if (msg.data.length >= 20 && isTrustedForwarder(msg.sender)) {
...
contract AgreementsPayMaster is BasePaymaster {
  using EnumerableSet for EnumerableSet.AddressSet;

  address public agreementFactory;
  EnumerableSet.AddressSet private agreements;

  event AgreementFactorySet(address target);
  event AgreementAdded(address agreement);

  error OnlyAgreementFactory();

	function setTarget(address target) external onlyOwner {
		agreementFactory = target;
		emit AgreementFactorySet(target);
	}
  ...
  function preRelayedCall(
      ...
      uint256 maxPossibleGas
        ...
        require(validRequest(relayRequest.request.to));
  ...
  function validRequest(address to) internal view returns(bool) {
    if(to == agreementFactory) return true;
    return agreements.contains(to);
  }

  function addNewAgreement(address _agreement) public {
    if (msg.sender != agreementFactory) revert OnlyAgreementFactory();
    agreements.add(_agreement);
    emit AgreementAdded(_agreement);
  }
}
...
contract ERC721A is IERC721A {
    ...
    /**
     * @dev Returns true if this contract implements the interface defind by
