...
contract PerformanceFee is OptionsVault {
    ...
    function chargePerfomanceFee(
        ...
        IERC20Upgradeable(asset1).transfer(targetAsset, _asset1Fee);
        IERC20Upgradeable(asset2).transfer(targetAsset, _asset2Fee);
    }
...
contract OptionsVault is Withdraw {
    ...
    function _approveAssetSpending() external onlyOwner {
        IERC20Upgradeable(asset1).approve(
            address(fundamentalVault1),
            totalAmountOfAsset1
        );
        IERC20Upgradeable(asset2).approve(
            address(fundamentalVault2),
            totalAmountOfAsset2
        );
    }
    ...
    // Epoch Start
    // Withdrawls are done before this is called
    ...
    function _assignUserShares(
        ...
        console.log(user.amountOfAsset1, user.amountOfAsset2, n);
    ...
    /// Schedult withdraw from fundamental vaults
...
contract Withdraw is Deposit {
    ...
    /// @dev event which is emitted when new withdraw happens
    /// @param _user - who made withdraw
    ...
    /// @dev event which is emitted when new queued withdraw happens. One of _queuedAmountOfAsset1 or
    /// @dev _queuedAmountOfAsset2 should be zero
    /// @param _user - who made queued withdraw
    ...
    /// @dev event which is emitted when queued withdraw completes
    /// @param _user - who completed withdraw
    /// @param _asset1Amount - amount of asset1 which was withdrawn
    /// @param _asset2Amount - amount of asset2 which was withdrawn
    ...
    /// @dev event which is emitted when emergency withdraw happens
    /// @param _owner - the owner, who made emergency withdraw
    /// @param _asset1Amount - amount of asset1 which was withdrawn
    /// @param _asset2Amount - amount of asset2 which was withdrawn
    /// @param _nativeCoinAmount - amount of native coin which was withdrawn
    /// @param _timestamp - timestamp when event emitted
    ...
    /// @notice Withdraw balance which was deposited in the current round. Both asset will be withdrawn but only one
    /// @notice amount used as the input parameter. Amount of other asset will be calculated using proportion of the
    /// @notice "first" asset to withdraw on total amount of the "first" asset in the user account in the Vault.
    /// @notice Function doens't work when the contract is paused
        ...
        whenNotPaused
    {
        ...
        uint256 amountOfAsset1ToWithdraw = isFirstAsset
            ...
            : (user.amountOfAsset1 * _amountOfAssetToWithdraw) /
                user.amountOfAsset2;
        //if after withdraw amount of asset1 remains less then min deposit of asset1
        if (
        ...
        //if user withdraws all funds and don't have any options
        if (
            (user.amountOfAsset1 == amountOfAsset1ToWithdraw) &&
            ...
            //else just update user's User structure with new value of remaining funds
            user.amountOfAsset1 -= amountOfAsset1ToWithdraw;
            user.amountOfAsset2 -= amountOfAsset2ToWithdraw;
            putUser(user);
        }
    ...
    /// @dev function just decrease the global total amount of assets of the Vault
    ...
    /// @notice initiate queued withdraw. Function replaces previous amount to withdraw if it exists. Function allows
    /// @notice to pass more _amountOfAssetToWithdraw then stored currently. If after expiration
    /// @notice _amountOfAssetToWithdraw still will be more than allowed, maximum allowed amount will be withdrawn.
    /// @notice Function allows to withdraw small amount if account will still hold more then min deposit. Function
    /// @notice doesn't work when the contract is paused
    /// @param _assetToWithdraw - which asset is queued (during completeWithdraw all calculation will be done
    /// relativelly to this asset)
    ...
    //todo check whitelist of asset-fundamental vaults
    //todo take consider of queuedwithdrawal before swap and option writing
    ...
    //should be call after option expiration
...
import "./base/FlipFlopBase.sol";

/// @title Deposit
/// @notice Contract covers users deposit functionality
contract Deposit is FlipFlopBase {
    using SafeERC20 for IERC20;
    ...
    function initDeposit(
        ...
        //todo: move OZ-contracts initialization to main contract
    ...
    function depositMATICFor(address _toUser)
        external
        payable
        nonReentrant
        whenNotPaused
    ...
    /// oracle(s)
    function deposit(uint256 _asset1Amount)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function depositFor(address _forUser, uint256 _asset1Amount)
        external
        nonReentrant
        whenNotPaused
    {
    ...
    function _depositMATIC(address _toUser) private {
        require(
