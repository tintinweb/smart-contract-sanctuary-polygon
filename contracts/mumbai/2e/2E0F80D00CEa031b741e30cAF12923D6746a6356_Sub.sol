...
contract Sub {
    ...
    function userAcceptSub(uint256 subId) public userRefersToSub(msg.sender,subId) {
        subscriptions[subId].active = true;
    }

    function userRemoveSub(uint256 subId) public userRefersToSub(msg.sender,subId){
        subscriptions[subId].active = false;
    }

    function orgCreateSub(uint256 amount)
        public
        returns (uint64)
    {
        subscriptionCounter++;
    ...
    function canAccess(uint256 subsId)
        public
        view
        returns (bool)
    {
