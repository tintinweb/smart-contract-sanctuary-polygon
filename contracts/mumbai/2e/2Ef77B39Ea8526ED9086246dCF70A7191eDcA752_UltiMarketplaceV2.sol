    ...
    IUltiMarketplaceV2
{
    ...
    function buyFromOffer(
        ...
        tokensSoldFromOffer[offer.id] += tokenQuantity;
    ...
    function cancelOffer(LibOffer.Offer calldata offer, bytes calldata signature) external nonReentrant {
        ...
        require(tokensSold < offer.quantity, "Insufficient number of tokens available in offer");
    ...
    function _buyFromOffer(
        ...
        if (offer.lazy) {
            ...
            uint256 sellerRest = token.supply - offer.quantity;
            if (sellerRest > 0) {
                _transferFrom(
                    address(this),
                    offer.seller,
    ...
    function _validateBid(
        ...
        bytes4 currencyType = _availableCurrencies[auction.currency];
        require(
            currencyType == LibAsset.ASSET_TYPE_ERC20 ||
                currencyType == LibAsset.ASSET_TYPE_NATIVE ||
                currencyType == LibAsset.ASSET_TYPE_ERC721 ||
                currencyType == LibAsset.ASSET_TYPE_ERC1155,
    ...
    function _validateBuyFromOffer(
        ...
        bytes4 currencyType = _availableCurrencies[offer.currency];
        require(
            currencyType == LibAsset.ASSET_TYPE_ERC20 ||
                currencyType == LibAsset.ASSET_TYPE_NATIVE ||
                currencyType == LibAsset.ASSET_TYPE_ERC721 ||
