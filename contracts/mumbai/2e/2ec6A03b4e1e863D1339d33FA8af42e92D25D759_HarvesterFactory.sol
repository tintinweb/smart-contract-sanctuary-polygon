...
//可暂停带黑名单机制的采集器铸造合约
contract HarvesterFactory is Ownable, ReentrancyGuard, AccessControlEnumerable, IFactory, Pausable {
    ...
    bytes32 internal constant UPGRADE_HASH = keccak256("Upgrade(address to,uint256 id,uint256 _max,uint256 orderid)");
    bytes32 internal constant REVERT_HASH = keccak256("Upgrade(address to,uint256 id,uint256[] _attributes,uint256 orderid)");
    ...
    constructor(address _mgge, address _serverSide, uint256 _salt) ReentrancyGuard() {
        ...
        DOMAIN_SEPARATOR = keccak256(
                abi.encode(
                    ...
                    keccak256(bytes("HarvesterFactory")),
                    keccak256(bytes("1")),
                    block.chainid,
                    address(this)
                )
  ...
  // MANAGING Authorization Contract Address for Call 
   ...
   // 合约部署者增加操作员
    ...
    //升级验签 
    modifier requiresUpgrade(address to, uint256 id,uint _max, uint orderid, bytes calldata signature) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(UPGRADE_HASH,to,id,_max,orderid))
            )
        );
    ...
    //恢复元数据验签 
    modifier requiresRevert(address to, uint256 id, uint256[] memory _attributes, uint orderid, bytes calldata signature) {
        ...
        bytes32 digest = keccak256(
            abi.encodePacked(
                "\x19\x01",
                DOMAIN_SEPARATOR,
                keccak256(abi.encode(REVERT_HASH,to,id,_attributes,orderid))
            )
        );
    ...
    function geneMetaData(
        ...
        external
        override 
        onlyAuthorizationContract //只允许主合约调用
        ownersOnly(_to, _id)
    {
        ...
        uint rand = uint256(keccak256(abi.encode(randomResult, 1))) % 3; // 0,1,2
         ...
         if (rand == 0) {
            uru_efficiency = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_durability = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            energy_efficiency  = 300 - uru_efficiency - max_durability;
        } else if (rand == 1) {
            energy_efficiency = uint256(keccak256(abi.encode(randomResult, 2))) % 30 + 70; 
            max_durability = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            uru_efficiency = 300 - energy_efficiency - max_durability;
        } else {
            energy_efficiency = uint256(keccak256(abi.encode(randomResult, 2))) %  30 + 70; 
            uru_efficiency = uint256(keccak256(abi.encode(randomResult, 3))) % 30 + 70; 
            max_durability = 300 - energy_efficiency - uru_efficiency;
        } 
        
        uint256 potential = 1 + uint256(keccak256(abi.encode(randomResult, 4))) % 100; //潜力1-100
        ...
        emit OnGeneMetaData(_to, _id, characters[_id]);
    ...
    function upgradeMetaData(
        ...
        requiresUpgrade(_to, _id, _max, _orderid, signature)
    {
        ...
        if (rand == 0) {
            characters[_id].energy_efficiency += uint256(keccak256(abi.encode(randomResult, 2))) % _max; 
        } else if (rand == 1) {
            characters[_id].uru_efficiency += uint256(keccak256(abi.encode(randomResult, 3))) % _max; 
        } else {
            characters[_id].max_durability += uint256(keccak256(abi.encode(randomResult, 4))) % _max; 
        }
    ...
    function revertMetaData(
        ...
        override 
        onlyAuthorizationContract
        requiresRevert(_to, _id, _attributes, _orderid, signature)
    {
        require(_attributes.length == 3, "length must be 3");

        characters[_id].energy_efficiency = _attributes[0];
        characters[_id].uru_efficiency = _attributes[1];
        characters[_id].max_durability = _attributes[2];
    }
