...
// SPDX-License-Identifier: MIT
// https://gateway.pinata.cloud/ipfs/QmcYkyfafk8ATECzW4B98Q6uyHtSzmmTgqhpuiqeMrPR7Y/
/**
...
contract ERC721Simple is ERC721, Ownable {
    ...
    event mintNFT(address _addr, uint256 _id);

    // Initializes the contract by setting a `name` and a `symbol`
    constructor(string memory _initBaseURI) ERC721("SLEEPYDOG", "DOG") {
        _supply.increment();
        setBaseURI(_initBaseURI);
    }

    // Mint an NFT
    function mint(address _addr) public returns (string memory) {
        // require(saleActive, "Sale is closed at the moment.");
        //  require(msg.value >= MINT_PRICE, "Not enough ETH sent; check price!");

        _safeMint(_addr, _supply.current());
        _supply.increment();
        emit mintNFT(_addr, _supply.current());
        return "sdfklw";
    }
