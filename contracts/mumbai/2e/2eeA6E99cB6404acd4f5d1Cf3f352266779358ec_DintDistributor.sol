...
contract DintDistributor is Ownable {
    ...
    function sendDint(address _recipient, uint256 _amount) external {
      require(user[_msgSender()].isRegistered, "sender not registered");
      ...
      IERC20(dintToken).transferFrom(_msgSender(), _recipient, (_amount * forUser / 10000));
      IERC20(dintToken).transferFrom(_msgSender(), feeCollector, (_amount * forDintApp / 10000));

      for(uint8 i; i < count; i++) {
        IERC20(dintToken).transferFrom(_msgSender(), referrer[i], (_amount * forReferral[i] / 10000));
      }
      
      emit tipSent(_msgSender(), _recipient, _amount);
    }
