...
*/
        ...
        contract PolygonCollection is ERC721A, Ownable, ReentrancyGuard, DefaultOperatorFilterer {
            ...
            bool public whitelistFeature = true;
                  ...
                  bytes32 public MerkleGroupRootOG = 0x3496a6f0ef28483485f5646cc525b448cf4c4aec0b2b75fe92ad8416e32cee75;
                  uint256 public MerklePriceOG = 0.02 ether;
                  uint256 public MerkleMaxWalletOG = 2;
                  
                  bytes32 public MerkleGroupRootPresale = 0x2f1ef3dd969422ead102c53249c128ac7e8c6b0615251523a8256f82db04be8e;
                  uint256 public MerklePricePresale = 0.03 ether;
                  uint256 public MerkleMaxWalletPresale = 5;
            ...
            constructor(uint256 _price, uint256 __maxSupply, string memory _initBaseURI, uint256 _maxMintAmountPerTx, uint256 _maxMintAmountPerWallet) ERC721A("Polygon Collection", "PC") {
        ...
        modifier whitelistMintPriceCompliance(bytes32[] calldata _merkleProof, uint256 _mintAmount) {
          ...
          _;
            ...
            function checkProofPerUser(bytes32[] memory _merkleProof, bytes32 leaf) view public returns (bool){
              ...
              if(MerkleProof.verify(_merkleProof, MerkleGroupRootOG, leaf)) return true ;
           if(MerkleProof.verify(_merkleProof, MerkleGroupRootPresale, leaf)) return true ;
            ...
            function checkPricePerUser(bytes32[] memory _merkleProof, bytes32 leaf) view public returns (uint256){
                ...
                if(MerkleProof.verify(_merkleProof, MerkleGroupRootOG, leaf)) return MerklePriceOG ;
             if(MerkleProof.verify(_merkleProof, MerkleGroupRootPresale, leaf)) return MerklePricePresale ;
            ...
            function checkMaxMintPerUser(bytes32[] memory _merkleProof, bytes32 leaf, uint256 amount) view public returns (bool){
              ...
              if(MerkleProof.verify(_merkleProof, MerkleGroupRootOG, leaf)) {
                  if(balance <= MerkleMaxWalletOG) return true;
                  else return false;
                }
                 if(MerkleProof.verify(_merkleProof, MerkleGroupRootPresale, leaf)) {
                  if(balance <= MerkleMaxWalletPresale) return true;
                  else return false;
                }
          ...
          function updateHashRootForAll(bytes32[] memory hashRootArray, uint256[] memory PriceArray, uint256[] memory  MaxMintArray)  public { 
            require(hashRootArray.length <= 2, "Too many inputs on the hash root!");
            require(PriceArray.length <= 2, "Too many inputs on the Price!");
            require(MaxMintArray.length <= 2, "Too many inputs on the Max mint amount!");

            
                MerkleGroupRootOG = hashRootArray[0];
                MerklePriceOG = PriceArray[0];
                MerkleMaxWalletOG = MaxMintArray[0];
                
                MerkleGroupRootPresale = hashRootArray[1];
                MerklePricePresale = PriceArray[1];
                MerkleMaxWalletPresale = MaxMintArray[1];
