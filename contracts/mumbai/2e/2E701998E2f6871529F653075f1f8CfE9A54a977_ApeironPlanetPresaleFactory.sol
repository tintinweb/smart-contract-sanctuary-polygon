...
contract ApeironPlanetGenerator is Random {
    ...
    event GenerateGeneId(Bloodline bloodline, ElementType[] elementOrders, uint256[] attributes, uint256 geneId);
    ...
    constructor() {
        ...
        //28 tags for Tri
        planetTagsPerBloodline[Bloodline.Tri].push(PlanetTag(19, 35, 35, 0, 35)); //Rainforest
        ...
        planetTagsPerBloodline[Bloodline.Tri].push(PlanetTag(26, 0, 35, 35, 35)); //Tundra
        ...
        planetTagsPerBloodline[Bloodline.Tri].push(PlanetTag(33, 35, 0, 35, 35)); //Wasteland
        ...
        planetTagsPerBloodline[Bloodline.Tri].push(PlanetTag(40, 35, 35, 35, 0)); //Salt Marsh
        ...
        //16 tags for Mix
        planetTagsPerBloodline[Bloodline.Mix].push(PlanetTag(47, 30, 30, 30, 30)); //Utopia
        planetTagsPerBloodline[Bloodline.Mix].push(PlanetTag(48, 25, 25, 25, 25)); //Garden
    ...
    function _getBloodline(CoreType coreType, uint256 valueFrom1To100)
        internal view returns (Bloodline)
    {
        require(valueFrom1To100 >= 1 && valueFrom1To100 <= 100, 'invalid valueFrom1To100');
        ...
        uint256 baseValue = 0;
        for (uint256 idx=uint256(Bloodline.Pure); idx<=uint256(Bloodline.Mix); idx++) {
    ...
    function _getPlanetTag(CoreType coreType, Bloodline bloodline, uint256 valueFrom1To100, uint256 valueFrom1To8064) //8064 = 18 * 28 * 16
        internal view returns (PlanetTag memory)
    {
        require(valueFrom1To100 >= 1 && valueFrom1To100 <= 100, 'invalid valueFrom1To100');
        require(valueFrom1To8064 >= 1 && valueFrom1To8064 <= 8064, 'invalid valueFrom1To8064');
        ...
        if (bloodline != Bloodline.Pure
            ...
            planetTag = planetTagsPerBloodline[bloodline][valueFrom1To8064 % planetTagsPerBloodline[bloodline].length];
        }
        return planetTag;
    }

    function _getElementOrders(Bloodline bloodline, PlanetTag memory planetTag, uint256 valueFrom1To144) //144 = 24 (1 * 2 * 3 * 4: for random remaining elements) x 6 (1 * 2 * 3: for random tag)
        internal pure returns (ElementType[] memory)
    {
        require(valueFrom1To144 >= 1 && valueFrom1To144 <= 144, 'invalid valueFrom1To144');

        ElementType[] memory orders = new ElementType[](4);
        ElementType[] memory results = new ElementType[](1 + uint256(bloodline));
        uint256 pickedIndex;
        ...
        //have any tag
            ...
            pickedIndex = uint256(orders[valueFrom1To144 % possibleElementSize]);
            //reduce the random factor
    ...
    function _getMaxBetweenValueAndPlanetTag(uint256 value, ElementType elementType, PlanetTag memory planetTag)
    ...
    function _getElementValues(Bloodline bloodline, PlanetTag memory planetTag, ElementType[] memory elementOrders, uint256[3] memory randomBaseValues)
        internal pure returns (uint256[4] memory)
    {
        require(elementOrders.length == uint256(bloodline) + 1, 'invalid elementOrders');
        ...
        if (bloodline == Bloodline.Pure) {
            ...
            values[uint256(elementOrders[0])] = _getMaxBetweenValueAndPlanetTag(_randomRangeByBaseValue(randomBaseValues[0], 50, 59), elementOrders[0], planetTag);
            values[uint256(elementOrders[1])] = 100 - values[uint256(elementOrders[0])];
        }
        else if (bloodline == Bloodline.Tri) {
            values[uint256(elementOrders[0])] = _getMaxBetweenValueAndPlanetTag(_randomRangeByBaseValue(randomBaseValues[0], 33, 43), elementOrders[0], planetTag);
            values[uint256(elementOrders[1])] = _randomRangeByBaseValue(randomBaseValues[1], 23, Math.min(43, 95 - values[uint256(elementOrders[0])]));
            values[uint256(elementOrders[2])] = 100 - values[uint256(elementOrders[0])] - values[uint256(elementOrders[1])];
        }
        else if (bloodline == Bloodline.Mix) {
            values[uint256(elementOrders[0])] = _getMaxBetweenValueAndPlanetTag(_randomRangeByBaseValue(randomBaseValues[0], 25, 35), elementOrders[0], planetTag);
            values[uint256(elementOrders[1])] = _randomRangeByBaseValue(randomBaseValues[1], 20, 34);
            values[uint256(elementOrders[2])] = _randomRangeByBaseValue(randomBaseValues[2], 20, Math.min(34, 95 - values[uint256(elementOrders[0])] - values[uint256(elementOrders[1])]));
            values[uint256(elementOrders[3])] = 100 - values[uint256(elementOrders[0])] - values[uint256(elementOrders[1])] - values[uint256(elementOrders[2])];
        }

        return values;
    }

    function _generateGeneId(CoreType coreType)
        internal returns (uint256)
    {
        Bloodline bloodline = _getBloodline(coreType, _randomRange(1, 100));
        PlanetTag memory planetTag = _getPlanetTag(coreType, bloodline, _randomRange(1, 100), _randomRange(1, 8064));
        ElementType[] memory elementOrders = _getElementOrders(bloodline, planetTag, _randomRange(1, 144));
        uint256[4] memory elementValues = _getElementValues(bloodline, planetTag, elementOrders, [ _getRandomBaseValue(), _getRandomBaseValue() , _getRandomBaseValue() ]);
        ...
        uint geneId = _convertToGeneId(attributes);
