...
contract USDRExchange is AddressAccessor, IExchange, Pausable {
    ...
    function avgTNGBLMintPrice() external view returns (uint) {
    ...
    function swapFromTNGBL(
        ...
        uint256 multiplier;
        {
            ...
            (
                ...
                uint256 liquidity,
                uint256 debt
            ) = ITreasury(treasury).getTreasuryValue(true);
            ...
            require(
                (usdrMarketCap * 12 * multiplier) / 10000 - tngbl_ >= amountIn,
        ...
        IERC20(tngbl).safeTransferFrom(msg.sender, treasury, amountIn);
        {
            ...
            uint256 quote = IPriceOracle(tngblOracle).quote(amountIn);
        ...
        _updateTNGBLMintingStats(int256((amountIn * 100) / multiplier), int256(amountOut));
    }
    ...
    {
        assembly {
            if iszero(iszero(usdrMarketCapE18)) {
                let p := div(mul(stableBacking, 100), usdrMarketCapE18)
                switch lt(p, 130)
                case false {
                    multiplier := 200//100
                }
    ...
    function _checkTNGBLToUSDRMarketCapRatio() private view {
        ...
        require(
            stable + rwa + liquidity - debt >=
                IStaking(staking).usdrMarketCap() * 88e7,
        ...
        )
    {
        ...
        require(tngblVal >= amountOut, "insufficient backing");
    }
        ...
        returns (address promissory, uint256 amountOut)
    {
        ...
        (
            ...
            uint256 liquidity,
            uint256 debt
        ) = ITreasury(treasury).getTreasuryValue(false);
        require(
            stable + rwa + liquidity - debt >=
    ...
    function afterBurnTNGBL(uint256 amount) external {
        (
            address treasury,
            address oracle
        ) = abi.decode(
        ...
        uint256 tngblPrice = IPriceOracle(oracle).quote(1e18);
        uint256 usdrAmount = (amount * tngblPrice) / 1e27;
        _updateTNGBLMintingStats(-int256(amount), -int256(usdrAmount));
    }

    function _updateTNGBLMintingStats(
        int256 tngblIn,
        int256 usdrOut
    ) private {
        TNGBLMintingStats memory stats = _tngblMintingStats;
        ...
        if (stats.tngblValue == 0 || usdrOut < 0 && stats.usdrMinted < uint256(-usdrOut)) {
            stats.usdrMinted = 0;
        } else {
