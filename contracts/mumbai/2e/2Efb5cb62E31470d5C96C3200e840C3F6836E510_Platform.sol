...
contract Platform is Roles {
    ...
    uint256 public constant minWeightedDollar = 300 * (10 ** (6 + 18));
    ...
    function setDefaultSuperNode(address superNode) external onlyOwner {
        require(superNode != address(0) && selfNodeOf[superNode] == address(0), "Args error");
        defaultSuperNode = superNode;
    ...
    function upgradeAndBind(address superNode) external {
        require(
            level2nd[msg.sender] >= 2 && level3th[msg.sender] >= 4
                && weightedDollar[msg.sender] >= minWeightedDollar,
            "Can not upgrade"
        );
    ...
    function setSuperNode(address superNode, address selfNode) external onlyOwner {
        require(superNode != selfNode && superNode != address(0) && selfNode != address(0), "Error1");
        require(channelOf[superNode] == address(0) && channelOf[selfNode] == address(0), "Error2");
        require(selfNodeOf[superNode] == address(0) && selfNodeOf[selfNode] == address(0), "Error3");
        require(agentOf[superNode] == address(0) && level2nd[superNode] == 0, "Error4");
    ...
    function play(
        ...
        require(agent != player && selfNodeOf[agent] == address(0), "Agent is a super node");
        ...
        if (agent != address(0)) {
            weightedDollar[agent] += (dollar * weight[mainType]);
            buildRelation(player, agent, _agent);
        }

        // Step2 calculate share
        ICalculateShare calc = ICalculateShare(shareCalc[mainType]);
        uint256 shareAmount = calc.calculateShare(mainType, subType, player, dollar);
    ...
    function setWeight(uint64 mainType, uint256 _weight) external onlyOwner {
