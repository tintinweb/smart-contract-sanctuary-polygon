...
contract PoolStakingFixed is Pausable, Ownable, ReentrancyGuard {
    ...
    // user => total amount staked user
    mapping(address => uint256) public amountStakedOfUser;
    // orderId => Order
    ...
    function viewStoreTokenIds(uint256 _pageIndex, uint256 _pageSize)
        external
        view
        returns (uint256[] memory data, uint256 total)
    {
        total = storeTokenIds.length();
        ...
        for (uint256 i = startIndex; i < endIndex; i++) {
            data[i - startIndex] = storeTokenIds.at(i);
        }
        return (data, total);
    }
        ...
        verifyActiveTime
    {
        ...
        amountStakedOfUser[msg.sender] += price;
    ...
    function withdraw(uint256 _orderId) external whenNotPaused nonReentrant {
        ...
        amountStakedOfUser[msg.sender] -= order.amount;
