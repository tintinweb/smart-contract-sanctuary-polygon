  ...
  ReentrancyGuardUpgradeable
{
  ...
  bytes32 public constant BOX_RANDOM_MINTER = keccak256("BOX_MINTER");
  ...
  struct Token {
    Rarity rare;
    // TODO implements token stat
    bool freezing;
    uint256 createTimestamp;
  }

  address[] public boxAddress;
  mapping(address => uint256) private boxIndex;
  mapping(address => bool) private isBoxAddress;
  ...
  event Unbox(address indexed owner, address indexed boxAddress, uint256 indexed nftId);

  function initialize(string memory baseURI) public initializer {
    __ERC721_init("Halo NFT", "NFT");
  ...
  function getInfoForStaking(uint256 tokenId) external view returns (address tokenOwner) {
  ...
  function addBlackListNFT(address[] calldata addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 i = 0; i < addresses.length; i++) {
      userBlacklist[addresses[i]] = true;
    }
  }

  function delBlackListNFT(address[] calldata addresses) public onlyRole(DEFAULT_ADMIN_ROLE) {
    for (uint256 i = 0; i < addresses.length; i++) {
  ...
  function approve(address to, uint256 tokenId) public override {
    // if (_nft[tokenId].freezing == true) {
    //   revert("ERC721: Token frozen");
    // }
    super.approve(to, tokenId);
  }

  function boxRandomMint(address to, uint8 rarity)
    public
    onlyRole(BOX_RANDOM_MINTER)
    nonReentrant
    returns (uint256 id)
  {
    require(to != address(0), "Address can not be zero");
    require(rarity < 5, "Internal contract error");
    _lastTokenId += 1;
    uint256 tokenId = _lastTokenId;
    _safeMint(to, tokenId);
    _nft[tokenId] = Token({rare: Rarity(rarity), freezing: true, createTimestamp: block.timestamp});
    emit Unbox(to, msg.sender, _lastTokenId);
    return _lastTokenId;
  }
  ...
  function grantRole(bytes32 role, address account) public override onlyRole(getRoleAdmin(role)) {
    if (role == BOX_RANDOM_MINTER) {
      boxIndex[account] = boxAddress.length;
      boxAddress.push(account);
    }
    _grantRole(role, account);
  }

  function revokeRole(bytes32 role, address account) public override onlyRole(getRoleAdmin(role)) {
    if (hasRole(role, account) && role == BOX_RANDOM_MINTER) {
      address lastBoxAddress = boxAddress[boxAddress.length - 1];
      boxAddress[boxIndex[account]] = lastBoxAddress;
      boxAddress.pop();
      boxIndex[lastBoxAddress] = boxIndex[account];
      delete boxIndex[account];
    }
    _revokeRole(role, account);
  }

  function getBoxLength() external view returns (uint256) {
    return boxAddress.length;
  }
  ...
  function _deleteNftOwnAtIndex(uint256 index, address owner) internal {
    uint256[] storage array = nftOwned[owner];
