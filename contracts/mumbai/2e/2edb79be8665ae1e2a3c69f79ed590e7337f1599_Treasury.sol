...
contract Sale is Ownable {
    ...
    /// System fee = (payment * _fee)/_DIVISOR
    uint32 private constant _DIVISOR = 100_000;

    uint32 private _fee;

    // The addresses of the price setting accounts
    mapping(address account => bool isApproved) public approvedAccountAddresses; 

    /// The number of sales that have been created - 1
    uint128 public saleIdCounter;

    modifier onlyApproved() {
        require(approvedAccountAddresses[msg.sender], "Not Approved Account");
        _;
    }

    /// The sale info of a particular sale
    mapping(uint256 saleId => SaleInfo saleInfo) saleIdToSaleInfo;

    /*//////////////////////////////////////////////////////////////
                             CONSTRUCTOR
    //////////////////////////////////////////////////////////////*/

    constructor(address token, address factory, address treasury, uint256 fee) {
        _TOKEN = IERC20(token);
        _FACTORY = ICollectionFactory(factory);
        _TREASURY = ITreasury(treasury);
        _fee = uint32(fee);
        approvedAccountAddresses[owner()] = true;
    }
     ...
     * @param nftAmount The amount of nfts in to sell
    ...
    function createSale(
        ...
        saleIdToSaleInfo[saleId].nftAmount = uint64(nftAmount);
    ...
    function cancelSale(uint256 saleId) external {
        ...
        /// check if the sale is not nftSold out
        uint256 available;
        uint256 nftSold = saleIdToSaleInfo_.nftSold;
        unchecked {
            available = saleIdToSaleInfo_.nftAmount - nftSold;
        }
        if (available == 0) revert SaleIsSoldOut();

        /// Update the nftAmount to sell
        saleIdToSaleInfo_.nftAmount = uint64(nftSold);

        /// Transfer unsold fractionalized nfts from the Treasury back to the seller
        _TREASURY.tranferFractionalizedNFTs(
     ...
     */
    function buy(uint256 saleId, uint256 nftAmount) external {
        if (saleId > saleIdCounter) revert SaleDoesNotExsit();

        SaleInfo storage saleIdToSaleInfo_ = saleIdToSaleInfo[saleId];
        if (saleIdToSaleInfo_.isForBullion) {
            if (msg.sender != _BULLIONWALLET) revert OnlyBullionCanBuy();
        }

        uint256 available;
        unchecked {
            available = saleIdToSaleInfo_.nftAmount - saleIdToSaleInfo_.nftSold;
        }
        ...
        uint256 systemFee = (payment * _fee) / _DIVISOR;

        /// Update the amount of nfts nftSold
        unchecked {
            saleIdToSaleInfo_.nftSold += uint64(nftAmount);
        }
     ...
     * @param nftPrice The price for each fractionalized nft
     */
    function setPrice(uint256 saleId, uint256 nftPrice) external onlyApproved {
    ...
    /** @notice Gives input address approvedContract access
        @dev Only the contract owner can call this function
        @param _toApprove The address to give admin access to
    */
    function addApprovedAccount(address _toApprove) external onlyOwner {
      approvedAccountAddresses[_toApprove] = true;
    }

    /** @notice Removes approvedContract access from input address
        @dev Only the contract owner can call this function
        @param _toRemove The address to give admin access to
    */
    function removeApprovedAccount(address _toRemove) external onlyOwner {
        approvedAccountAddresses[_toRemove] = false;
    }
}
...
contract Treasury is Ownable2Step, ERC1155Holder {
     ...
     */
    function withdraw(uint256 _amountUSDC) external onlyOwner {
        if (_amountUSDC == 0) revert AmountMustBeGreaterThan0();
        if (USDC.balanceOf(address(this)) < _amountUSDC)
            revert InsufficientBalance(address(this).balance);

        USDC.safeTransfer(msg.sender, _amountUSDC);
     ...
     * @dev Only Owner can call this function.
     * @param saleContractAddress_, The address of the Sale contract.
     */
    function setSaleContractAddress(
        address saleContractAddress_
    ) external onlyOwner {
        saleContractAddress = saleContractAddress_;
    }
     ...
     */
    function tranferFractionalizedNFTs(
