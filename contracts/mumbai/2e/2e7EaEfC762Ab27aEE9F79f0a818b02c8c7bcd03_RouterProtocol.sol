    ...
    UUPSUpgradeable
{
    ...
    uint64 public rChainType;
    string public rChainId;
    string public tokenOnRouterEVMAddress;
    string public tokenOnRouterRouterAddress;

    mapping(uint64 => mapping(string => bytes)) public remoteContracts;
    ...
    function initialize(
        ...
        string memory _tokenOnRouterRouterAddress,
        string memory _tokenOnRouterEVMAddress
    ) public initializer {
        ...
        tokenOnRouterRouterAddress = _tokenOnRouterRouterAddress;
        tokenOnRouterEVMAddress = _tokenOnRouterEVMAddress;
    ...
    function setContractOnChain(
        ...
        remoteContracts[chainType][chainId] = toBytes(contractAddress);
    }

    function setDappMetadata(string memory feePayerAddress) external payable {
        gateway.setDappMetadata{value: msg.value}(feePayerAddress);
    }
    ...
    function sendRPTokenCrossChain(
        ...
        if (
            ...
            gateway.requestToRouter{value: msg.value}(
                ...
                "", //routeRecipient
                payload,
                tokenOnRouterEVMAddress,
                destGasLimit,
                "" // empty asm
            );
        } else {
            if (expiryTimestamp == 0) expiryTimestamp = type(uint64).max;
            bytes[] memory addresses = new bytes[](1);
            addresses[0] = remoteContracts[_dstChainType][_dstChainId];
            bytes[] memory payloads = new bytes[](1);
            payloads[0] = payload;

            gateway.requestToDest(
                Utils.RequestArgs(expiryTimestamp, false),
                Utils.AckType(Utils.AckType.ACK_ON_BOTH),
    ...
    function handleRequestFromSource(
        ...
        require(
            keccak256(srcContractAddress) ==
                keccak256(remoteContracts[srcChainType][srcChainId]),
            "Invalid src chain"
        );
        ...
        _mint(to, amount);
        return abi.encode(amount, from); //empty for both cases
    }

    function handleRequestFromRouter(
        string memory sender,
        bytes memory payload
    ) external isGateway {} // empty for now
