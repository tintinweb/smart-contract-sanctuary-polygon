    ...
    ReentrancyGuardUpgradeable
{
    ...
    function initialize(uint256 _feePercent, uint256 _royalityPercent)
        external
        initializer
    {
        __Ownable_init();
    ...
    function getRoyalyPercentage() public view virtual returns (uint256) {
        return _royalityFee;
    }
    ...
    function setRoyalityFee(uint256 fee) public virtual onlyOwner {
        _royalityFee = fee;
    }
    ...
    function createListing(
        ...
        for (uint256 i; i < listingIds.length; i++) {
            if (
                ...
                _listings[listingIds[i]].seller == msg.sender
            ) {
                require(
                    ...
                    IERC721Upgradeable(_listings[listingIds[i]].nftAddress)
                        .safeTransferFrom(
                            ...
                            abi.encodePacked(
                                "SafeTransferFrom",
                                "'''###'''",
                                _uint2str(_listings[listingIds[i]].price)
                            )
                        );
                } else {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            _creators[i] = NftContractInterFace(nftAddress).getCreator(
                tokenIds[i]
            );
        }
    ...
    function buyAssetFromListing(
        ...
        uint256 royalityFee = ((listing.price * _royalityFee) /
