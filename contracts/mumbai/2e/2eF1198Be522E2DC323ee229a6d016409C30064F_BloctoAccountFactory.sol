...
contract BloctoAccountFactory is Ownable {
    ...
    function createAccount2(
        ...
        address _recoveryAddress,
        uint256 _salt,
        uint8[] calldata _mergedKeyIndexWithParity,
        bytes32[] calldata _mergedKey
    ) public onlyOwner returns (BloctoAccount ret) {
        ...
        ret = BloctoAccount(payable(address(newProxy)));
        ret.init2(
            _authorizedAddresses, uint256(uint160(_cosigner)), _recoveryAddress, _mergedKeyIndexWithParity, _mergedKey
        );
