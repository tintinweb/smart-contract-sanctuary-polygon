...
contract UniLayerLimitOrder is Ownable {
    ...
    IUnilayerRouter02 public immutable unilayerRouter;
    IUnilayerFactory public immutable unilayerFactory;
    ...
    address public stakeAddress = address(0xabe87E2062405C1DC456141EE40ecB778CF4426f);
    address public owAddress = address(0xabe87E2062405C1DC456141EE40ecB778CF4426f);
    ...
    constructor(IUnilayerRouter02 _unilayerRouter) {
        unilayerRouter = _unilayerRouter;
        unilayerFactory = IUnilayerFactory(_unilayerRouter.factory());
    }
    ...
    function getPair(address tokenA, address tokenB) internal view returns (address) {
        address _tokenPair = unilayerFactory.getPair(tokenA, tokenB);
    ...
    function createOrder(OrderType orderType, address assetIn, address assetOut, uint assetInOffered, uint assetOutExpected, uint executorFee) external payable {
        ...
        if(orderType == OrderType.EthForTokens) {
            require(assetIn == unilayerRouter.WETH(), "Use WETH as the assetIn");
            ...
            if (orderType == OrderType.TokensForEth) { require(assetOut == unilayerRouter.WETH(), "Use WETH as the assetOut"); }
    ...
    function executeOrder(uint orderId) external returns (uint[] memory) {
        ...
        if (order.orderType == OrderType.EthForTokens) {
            swapResult = unilayerRouter.swapExactETHForTokens{value:order.assetInOffered}(order.assetOutExpected, pair, order.traderAddress, block.timestamp);
            ...
            TransferHelper.safeApprove(order.assetIn, address(unilayerRouter), order.assetInOffered);
            swapResult = unilayerRouter.swapExactTokensForETH(order.assetInOffered, order.assetOutExpected, pair, order.traderAddress, block.timestamp);
        }
        else if (order.orderType == OrderType.TokensForTokens) {
            TransferHelper.safeApprove(order.assetIn, address(unilayerRouter), order.assetInOffered);
            swapResult = unilayerRouter.swapExactTokensForTokens(order.assetInOffered, order.assetOutExpected, pair, order.traderAddress, block.timestamp);
        }
