...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
  ...
  // mapping for token creator
    ...
    uint256 internal _totalSupply = 500;
    ...
    */
    ...
    function createCollectible(address to, string memory _tokenURI) external returns (uint256) {
        require(msg.sender == _operator,"Operator: caller is not a operator");
        uint256 newItemId = tokenCounter;
        _safeMint(to, newItemId, rFee);
...
contract KERNEL721 is ERC721 {
    ...
    function setRoyalty(uint256 _fee) external onlyOwner {
        rFee = _fee;
