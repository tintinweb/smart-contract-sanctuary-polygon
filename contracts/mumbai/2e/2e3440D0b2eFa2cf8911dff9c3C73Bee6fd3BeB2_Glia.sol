...
contract ArtworkAccessControl {
    ...
    modifier onlyCEO() {
        require(msg.sender == ceoAddress,"You must be contract CEO");
        _;
    }

    /// @dev Access modifier for CFO-only functionality
    modifier onlyCFO() {
        require(msg.sender == cfoAddress, "You must be contract CFO");
        _;
    }

    /// @dev Access modifier for COO-only functionality
    modifier onlyCOO() {
        require(msg.sender == cooAddress, "You must be contract COO");
        _;
    }
    ...
    modifier onlyCLevel() {
        require(
            ...
            msg.sender == cfoAddress, "Must be C-LEVEL"
        );
        _;
    }
    ...
    function setCEO(address _newCEO) external payable onlyCEO {
        require(_newCEO != address(0),"CEO Address must not be null");
    ...
    function setCFO(address _newCFO) external onlyCEO {
        require(_newCFO != address(0),"CFO can not be null");
    ...
    ///  derived contracts.
    function unpause() public virtual payable onlyCEO whenPaused {
        // can't unpause if contract was upgraded
        paused = false;
    }
}

contract ArtworkMinting is ArtworkAccessControl {
    ...
    uint256 public autoCreationFee = 0;
...
contract ArtworkOwnership is ArtworkMinting {
    
    event TokenTransfer(address from, address to, uint256 tokenId);
    ...
    function _assignLeftpiece(uint256 _topId, uint256 _leftId) internal {
        ...
        emit CoupledArt(artworkIndexToOwner[_topId], _topId, _leftId);
    }
    ...
    function _assignRightpiece(uint256 _topId, uint256 _rightId) internal {
        ...
        // Emit the coupling event.
        emit CoupledArtRightpiece(artworkIndexToOwner[_topId], _topId, _rightId);
    }
}

contract ArtworkCore is ArtworkOwnership {
    mapping (address => mapping (address => bool)) private _operatorApprovals;
    ...
    function assignControlPiece(uint256 _tokenId, uint256 _controlId) external payable whenNotPaused  {
        require(_owns(msg.sender, _tokenId),"Must own token you are trying to assign from");
    ...
    function assignLeftpiece(uint256 _topId, uint256 _leftId) external payable whenNotPaused  {
        require(_owns(msg.sender, _topId),"Must own token you are trying to assign from");
        require(_isCouplingPermitted(_topId, _leftId),"Coupling is not permitted between these tokens");
       _assignLeftpiece(_topId,_leftId);
    }

    function assignRightpiece(uint256 _topId, uint256 _rightId) external payable whenNotPaused  {
        require(_owns(msg.sender, _topId),"Must own token you are trying to assign from");
        require(_isCouplingPermitted(_topId, _rightId),"Coupling is not permitted between these tokens");
    ...
    function grantRightpiece(uint256 _masterId,uint256 _controlTargetId, uint256 _receiverId, uint256 _tokenToGrant, address _controlOwner) external payable whenNotPaused {
        require(artworkIndexToControlPiece[_masterId] != 0, "Master Token can not have null control token");
        require(_owns(msg.sender, _masterId),"You must own the master token");  
        require(_ownsOrControls(msg.sender, _tokenToGrant, _masterId),"You must own or control the tokens");
        require(_owns(_controlOwner, _receiverId),"Control owner must own receiver token");
        require(_owns(_controlOwner, _controlTargetId),"Control owner must own target token");
        require(_shareControl(_masterId,_controlTargetId),"Master token must share control with controlTarget");
        require(_shareControl(_masterId,_receiverId),"Master token must share control with receiver ID");

        _assignRightpiece(_receiverId,_tokenToGrant);
        emit Grant(_receiverId, _tokenToGrant);
    }
    ...
    function grantControlpiece(uint256 _masterId,uint256 _tokenToGrantControlOf, uint256 _tokenToGrantControlTo)  external payable whenNotPaused {
        require(artworkIndexToControlPiece[_masterId] != 0, "Master Token can not have null control token");
        require(_owns(msg.sender, _masterId),"You must own the master token");  
        require(_ownsOrControls(msg.sender, _tokenToGrantControlOf, _masterId),"You must own or control the tokens");
        require(_shareControl(_masterId,_tokenToGrantControlOf),"Master token must share control with controlTarget");
        artworkIndexToControlPiece[_tokenToGrantControlTo];
        emit GrantControl(_tokenToGrantControlOf, _tokenToGrantControlTo);
    }
    
    function pointRightToControlledArtwork(uint256 _masterId, uint256 _tokenToPoint, uint256 _topToken) external payable whenNotPaused {
        require(artworkIndexToControlPiece[_masterId] != 0, "Master Token can not have null control token");
        require(_owns(msg.sender, _masterId),"You must own the master token");  
        require(_ownsOrControls(msg.sender, _topToken, _masterId),"Must own or control both tokens");
        require(_shareControl(_masterId,_tokenToPoint),"Master token must share control with Target");
        _assignRightpiece(_topToken,_tokenToPoint);
        emit Grant(_topToken, _tokenToPoint);
    }

    function pointLeftToControlledArtwork(uint256 _masterId, uint256 _tokenToPoint, uint256 _topToken) external payable whenNotPaused {
        require(artworkIndexToControlPiece[_masterId] != 0, "Master Token can not have null control token");
        require(_owns(msg.sender, _masterId),"You must own the master token");  
        require(_ownsOrControls(msg.sender, _topToken, _masterId),"Must own or control both tokens");
        require(_shareControl(_masterId,_tokenToPoint),"Master token must share control with Target");
    ...
    function removeRightpiece(uint256 _topId) external payable whenNotPaused {
        require(_owns(msg.sender, _topId),"Must own token");
...
contract Glia is ERC721Enumerable, Ownable, ArtworkCore {
    using Strings for uint256;
	string _baseTokenURI;


    // Truth.　
    constructor(string memory baseURI) ERC721("glia.icu", "GLIA")  {
        setBaseURI(baseURI);
        // Starts paused.
        paused = true;
    ...
    // Removed function walletOfOwner - this may have some value to add back... TBD
    
    function _exists(uint256 tokenId) internal view virtual override returns (bool) {
        return artworkIndexToOwner[tokenId] != address(0);
    }
    ...
    function _createArtwork(
        ...
        require(isTaken[_artworkContent] == false,"Artwork is not unique");
        ...
        // per ERC721 draft
        _safeMint(_owner, newArtId);
        _transfer(address(0), _owner, newArtId);
        return newArtId;
    }
    
    // GLIA.icu custom contract functions
    ...
    function createPromoArtwork(bytes calldata _artworkContent) external payable onlyCOO {
        address artOwner = msg.sender;
        if (artOwner == address(0)) {
             artOwner = cooAddress;
        }
        require(promoCreatedCount < PROMO_CREATION_LIMIT,"Too many promos created");
        promoCreatedCount++;
    ...
    // Same as above but accessible by non-COO requires additional payment equal to autoCreation Fee
     function createArtwork(bytes calldata _artworkContent) external payable whenNotPaused {
        address artOwner = msg.sender;
        if (artOwner == address(0)) {
             artOwner = cooAddress;
        }
        require(msg.value >= autoCreationFee,"Message value is not enough - must include autoCreationFee");
        _createArtwork(_artworkContent, artOwner,artOwner);
    }
     ...
     function approveCoupling(address _addr, uint256 _partnerId)
        external payable
        whenNotPaused  {
        require(_owns(msg.sender, _partnerId),"You do not own this token");
        ...
        returns (address owner)
    {
        ...
        require(owner != address(0),"Token doesnt exist or is burned");
    }
