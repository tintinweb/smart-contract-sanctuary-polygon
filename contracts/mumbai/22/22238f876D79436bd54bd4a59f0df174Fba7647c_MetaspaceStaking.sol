...
contract MetaspaceStaking is Ownable {
    /* ========== STATE VARIABLES ========== */
    ...
    function calculateReward(uint256 _amount, uint256 _stakeDurationInSec)
        public
        view
        returns (uint256)
    {
    ...
    function stake(uint256 _mldAmount, uint256 _stakeDurationInSec) external {
        ...
        stakeInfo.metalordStaked += _mldAmount;
        stakeInfo.lordStaked += _requiredLord;
    ...
    function unstake(uint256 _index) external withdrawCheck(_index) {
        ...
        uint256 returnAmount = userDepositInfo.mldStaked +
            userDepositInfo.reward;

        userDepositInfo.claimed = true;
        durationwiseStake[userDepositInfo.durationStaked]
            .metalordStaked -= userDepositInfo.mldStaked;
        uint256 _lordStaked = userDepositInfo.lordStaked;
        durationwiseStake[userDepositInfo.durationStaked]
            .lordStaked -= _lordStaked;

        mldToken.transfer(msg.sender, returnAmount);
        uint256 lordFees = (userDepositInfo.lordStaked *
            unstakeLordFeesPercentage) / 100;
    ...
    function getUserUnclaimInfo(address _user)
        public
        view
        returns (
            DepositInfo[] memory unclaimedMaturedDeposits,
            DepositInfo[] memory unclaimedPreMaturedDeposits,
        ...
        for (uint256 i = 0; i < userDeposits[_user].length; i++) {
            ...
            if (!userInfo.claimed) {
                if (block.timestamp >= userInfo.maturity)
                    unclaimedMaturedDeposits[
                        unclaimedMaturedDeposits.length
                    ] = userInfo;
                else {
                    unclaimedPreMaturedDeposits[
                        unclaimedPreMaturedDeposits.length
                    ] = userInfo;
                }
                pendingClaimableRewards =
                    pendingClaimableRewards +
                    userInfo.reward;
            }
        }
    ...
    function userTotalDeposits(address _user)
        public
        view
        returns (uint256 totalMLDStaked)
    {
        for (uint256 i = 0; i < userDeposits[_user].length; i++) {
            DepositInfo memory userInfo = userDeposits[msg.sender][i];
            if (!userInfo.claimed)
                totalMLDStaked = totalMLDStaked + userInfo.mldStaked;
        }
    }

    function userNextUnlock(address _of)
        public
        view
    ...
    function addPeriod(uint256 _stakeDurationInDays, uint256 _multiplier)
        external
        onlyOwner
    {
    ...
    function invalidatePeriod(uint256 _stakeDurationInSec) external onlyOwner {
        ...
        require(stakeInfo.valid, "Invalid Staking Duration");
        durationwiseStake[_stakeDurationInSec].valid = false;
    }

    function updatePreMaturePenalty(uint256 _penaltyPercentage)
        external
        onlyOwner
        incorrectPercentage(_penaltyPercentage)
    {
        preMatureMLDPenalty = _penaltyPercentage;
    }

    function updateLordUnstakingFees(uint256 _unstakingfees)
        external
        onlyOwner
        incorrectPercentage(_unstakingfees)
    {
