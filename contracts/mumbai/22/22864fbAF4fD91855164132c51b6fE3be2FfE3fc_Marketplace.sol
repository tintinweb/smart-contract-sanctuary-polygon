...
contract Marketplace {
    ...
    struct MarketItem {
        ...
        uint256 tokenId; //idnetifier within the contract
    ...
    function buyMarketItem(uint256 _itemId) public payable {
        require(marketItems[_itemId].isPresent == true, "Item is not present");
        require(marketItems[_itemId].isSold == false, "Item is already sold");
        require(
        ...
        //console is working
        // console.log("NFT with ItemId: ", _itemId, ", is sold to: ", msg.sender);
    }

    function getMarketItem(
        uint256 _tokenId
    ) public view returns (MarketItem memory marketItem) {
        marketItem = marketItems[_tokenId];
    }

    function changeListingPrice(uint256 _newPrice) public {
        require(_newPrice > 0, "Listing price must be greater than 0");
        require(
            msg.sender == owner,
            "Only owner is allowed to change the listing price"
        );
