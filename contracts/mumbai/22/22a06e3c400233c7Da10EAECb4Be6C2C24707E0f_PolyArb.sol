...
contract PolyArb is FlashLoanReceiverBase, IUniswapV2Callee {
    ...
    function multiCyclicFLParams(
        ...
        // _token param structure is: [   [ [a,b,c] ],[ [c,d,a] ]   ]
        uint256 j = 0;
        uint256 len = _targets.length;
    ...
    */
    ...
    function flashloanUniswap(address borrowedTokenAddress, uint256 amountToBorrow, bytes memory _params) external permitRole(USER) {
        ...
        if (borrowedTokenAddress == address(WMATIC) || borrowedTokenAddress == address(SAND)) {
            // WMATIC-SAND
            pair = address(0xf3963A1f3a5f2a484a430A8aA9420B1a2369c2be);
        } else {
    ...
    function multiSwapParams(
        uint256 _cycleType, // (activates in flashloan)  0: multiSingleSwapFLParams OR 1: multiCyclicFLParams
        uint256 _amountToCoinbase, 
        address[] memory _targets, // Market Addresses
        bytes[] memory _payloads, // Swap Action
        address[] memory _tokens, // Path of tokens
        address[] memory _dexRouterContractAddress, // Address of DEX router contracts
        address[][] memory _routerTokens // Activates in multiCyclicFLParams [[dex0inputToken, dex0path]] eg. [ [a, [a,b,c] ], [c, [c,d,a] ] ]
        ...
        return abi.encode(
            ...
            _payloads,
            _tokens,
            _dexRouterContractAddress,
            _routerTokens
        );
    }

    function swapParams(
        uint256 amount0Out,  // (activates in flashloan)  0: multiSingleSwapFLParams OR 1: multiCyclicFLParams
        uint256 amount1Out, 
        address to,  // Market Addresses
        bytes calldata /* data */  // leave this blank
    ) external pure returns (bytes memory params) {
        return bytes(abi.encodeWithSignature(
