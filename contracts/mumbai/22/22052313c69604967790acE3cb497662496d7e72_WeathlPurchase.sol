...
contract WeathlPurchase is ERC721, ERC721Enumerable, ERC721Burnable, AccessControl {
    ...
    function safeMint(address to, uint256 amount, string memory token) public onlyRole(MINTER_ROLE) returns (uint256) {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
        purchases[tokenId].push(Purchase(to, amount, token));
        return tokenId;
    }
    ...
    function getPurchase(uint256 tokenId) public view onlyRole(MINTER_ROLE) returns (address, uint256, string memory) {
        return (purchases[tokenId][0].owner, purchases[tokenId][0].amount, purchases[tokenId][0].token);
