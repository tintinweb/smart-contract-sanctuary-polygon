 ...
 *
 */
contract Forwarder is ERC165, IForwarder, Pausable {
  ...
  function onlyWithdrawer() internal view {
    require(IForwarderFactory(factoryAddress).hasRole(WITHDRAWER_ROLE, _msgSender()), "withdrawer only");
  }

  function onlyValid() internal view {
    require(IForwarderFactory(factoryAddress).isValid(), "!valid");
  }

  function supportsInterface(bytes4 _interfaceID) public view override returns (bool) {
    return
      _interfaceID == type(IForwarder).interfaceId ||
  ...
  function deposit(
    ...
    if(IHandler(IForwarderFactory(factoryAddress).handlerAddress())._resourceIDToTokenContractAddress(_resourceID) == wethAddress) {
  ...
  function decodeDepositData(bytes memory _depositData) internal pure returns(
    address sender,
  ...
  receive() external payable {
    ...
    IForwarderFactory(factoryAddress).emitNativeReceived(
      _msgSender(),
      address(this),
      msg.value
    );
