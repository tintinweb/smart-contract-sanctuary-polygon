...
contract TangibleRevenueShare is AccessControl {
    ...
    function canClaim() external view returns (bool) {
        ...
        if (_distributed > 0 && !_claimed[_snapshot][who]) {
            uint256 numTokens = nft.balanceOf(who);
            for (uint256 i = 0; i < numTokens; i++) {
                uint256 tokenId = nft.tokenOfOwnerByIndex(who, i);
                uint256 share = nft.getShareAt(tokenId, _snapshot);
    ...
    function claim() external {
        ...
        uint256 balance = nft.balanceOf(who);
        uint256 amount;
...
contract PassiveIncomeNFT is ERC721Enumerable, AccessControl {
    ...
    function burn(uint256 tokenId) external {
        ...
        (uint256 amount, ) = claimableIncome(tokenId);
        if (amount > 0) {
            totalClaimed += amount;
        }
    ...
    {
        Lock memory lock = locks[tokenId];
        /*uint256 fullAmount = (lock.lockedAmount * lock.multiplier) /
            1e18 -
            lock.claimed;
        console.log("full: ", fullAmount);
    ...
    function _snapshot(uint256 tokenId) private {
        _snapshotOfAt[0][_nextSnapshot] = _currentMaxPayout;
        _snapshotOfAt[tokenId][_nextSnapshot] = locks[tokenId].maxPayout;
    }

    function setNextSnapshot(uint256 snapshot) external onlyRole(SNAPPER_ROLE) {
        _previousSnapshot[snapshot] = _nextSnapshot;
