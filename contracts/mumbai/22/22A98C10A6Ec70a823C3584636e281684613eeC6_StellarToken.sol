...
contract StellarToken is IERC20 {
    ...
    INonfungiblePositionManager private constant positionManager = INonfungiblePositionManager(0xC36442b4a4522E871399CD717aBDD847Ab11FE88);
    ...
    event TokensSwapped(address indexed fromToken, address indexed toToken, address indexed sender, uint256 amountIn, uint256 amountOut, uint256 fees);
    ...
    constructor() {
        ...
        MATIC_ADDRESS = 0x9c3C9283D3e44854697Cd22D3Faa240Cfb032889;
    ...
    function createPair() external onlyDev {
        ...
        address liquidityToken = positionManager.createAndInitializePoolIfNecessary(
            address(this),
            MATIC_ADDRESS,
            3000,
            0
        ...
        uniswapPair = liquidityToken;
    }

    function addLiquidity(uint256 amountIn, uint256 amountOutMin, uint160 sqrtPriceLimitX96) external onlyDev {
        // Approve the swapRouter to spend the input token (MATIC)
        IERC20 token = IERC20(MATIC_ADDRESS);
        token.approve(address(swapRouter), amountIn);

        // Set the parameters for the exact input single swap
        ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams({
            tokenIn: MATIC_ADDRESS,
            tokenOut: address(this),
            fee: 3000, // Fee amount for Uniswap V3 (0.3%)
        ...
        // Perform the exact input single swap and get the output amount
        uint256 amountOut = swapRouter.exactInputSingle(params);

        // Mint a new position in the Uniswap V3 pool and get the position token ID
        (uint256 tokenId, , ,) = positionManager.mint{value: 0}(
            INonfungiblePositionManager.MintParams({
                token0: MATIC_ADDRESS,
                token1: address(this),
                fee: 3000,
                tickLower: -887220,
                tickUpper: 887220,
    ...
    function removeLiquidity(uint256 liquidity, uint256 amountOutMin) external onlyDev {
        uint128 liquidityAmount = uint128(liquidity);
        IERC20 liquidityToken = IERC20(uniswapPair);

        liquidityToken.transferFrom(msg.sender, address(this), liquidityAmount);
        liquidityToken.approve(address(positionManager), liquidityAmount);
        ...
        emit TokensSwapped(address(this), MATIC_ADDRESS, msg.sender, liquidityAmount, amount0, 0);
    }
