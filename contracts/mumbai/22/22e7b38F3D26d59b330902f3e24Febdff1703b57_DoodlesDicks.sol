...
error DoodlesDickNotOwnedLongEnough();
error InvalidChunk();

interface DoodlesDick {
...
contract DoodlesDicks is ERC721A, MultisigOwnable, VRFConsumerBaseV2 {
  ...
  DoodlesDick public immutable doodlesdick;

  // Keys are doodles dick token ids
  mapping(uint256 => bool) public doodlesDickCanClaim;
  ...
  constructor(
    address _doodlesDickAddress,
    uint256 _maxSupply,
    address _vrfCoordinator,
    address _linkToken,
    string memory initialName,
    string memory initialSymbol
  ) ERC721A('DoodlesDicks', 'DOODLESDICK') VRFConsumerBaseV2(_vrfCoordinator) {
    doodlesdick = DoodlesDick(_doodlesDickAddress);
  ...
  function airdrop(
    ...
    uint256[] calldata numDoodlesDickTokens,
    uint256 chunkNum
  ) external onlyRealOwner {
    if (processedChunksForAirdrop[chunkNum] || balanceOf(receivers[0]) > 0) {
      revert ChunkHasBeenAirdropped();
    }
    if (receivers.length != numDoodlesDickTokens.length) {
      revert InvalidChunk();
    }
    for (uint256 i; i < receivers.length; ++i) {
      _mintWrapper(receivers[i], numDoodlesDickTokens[i]);
    }
    processedChunksForAirdrop[chunkNum] = true;
  }

  // Used to claim unclaimed tokens after airdrop/claim phase
  function devClaim(uint256 numDoodlesDickTokens) external onlyRealOwner {
    _mintWrapper(msg.sender, numDoodlesDickTokens);
  }

  function claim(uint256[] calldata doodlesDickTokenIds) external {
    ...
    for (uint256 i; i < doodlesDickTokenIds.length; ++i) {
      uint256 doodlesDickId = doodlesDickTokenIds[i];

      DoodlesDick.TokenOwnership memory tokenOwnership = doodlesdick.getOwnershipData(
        doodlesDickId
      );
      address tokenOwner = tokenOwnership.addr;
      if (tokenOwner != msg.sender) revert MismatchedTokenOwner();
      if (!doodlesDickCanClaim[doodlesDickId]) revert BeanCannotBeClaimed();
      ...
      if (block.timestamp - ownershipStart < MIN_OWNERSHIP_TIME_FOR_CLAIM) {
        revert DoodlesDickNotOwnedLongEnough();
      }
      doodlesDickCanClaim[doodlesDickId] = false;
    }
    _mintWrapper(msg.sender, doodlesDickTokenIds.length);
  }

  function _mintWrapper(address to, uint256 numDoodlesDickTokens) internal {
    uint256 numToMint = numDoodlesDickTokens * 2;
  ...
  function setCanClaim(uint256[] calldata doodlesDickIds) external onlyRealOwner {
    for (uint256 i; i < doodlesDickIds.length; ++i) {
      doodlesDickCanClaim[doodlesDickIds[i]] = true;
    }
  }
