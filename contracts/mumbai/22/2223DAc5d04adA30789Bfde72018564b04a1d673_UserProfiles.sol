...
interface IRewards {
    ...
    // External Contract to call with message details
    function checkRewards(uint256 msgID, uint256 postCount) external;
}
    ...
    */
    ...
    function recordPost(address posterAddress, uint256 tipPerTag, address[] calldata tipReceivers, uint256 isCommentOf, address tipContract, uint256 erc20Tips, uint256 msgID) public onlyAdmins {
        ...
        if (tx.origin == posterAddress){
            Rewards.checkRewards(msgID, usrProfileMap[posterAddress].userStats.postCount);
        }
    }
...
//    function setNFTAsAvatar(address _nftContract, uint256 tokenId, uint256 groupID) public whenNotPaused nonReentrant {
    ...
    */
//    function setContractHook(address contractAddress) public whenNotPaused nonReentrant {
    function setContractHook(address contractAddress) public whenNotPaused {
        ...
        if (contractAddress != address(0)){
            ...
            address[] memory testAddArray;
            uint256[] memory testNumArray;

            newMsgCH.msgID = 0;
            newMsgCH.postedBy = [msg.sender, address(0)];
            newMsgCH.message = "Test";
            newMsgCH.paid = 0;
            newMsgCH.hashtags = testArray;
            newMsgCH.taggedAccounts = testAddArray;
            newMsgCH.asGroup = 0;
            newMsgCH.inGroups = testNumArray;
        ...
        usrProfileMap[msg.sender].contractHook = contractAddress;
    }
