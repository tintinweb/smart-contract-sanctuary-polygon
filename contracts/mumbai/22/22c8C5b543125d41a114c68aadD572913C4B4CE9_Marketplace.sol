...
// Using ERC2771 to extracting original signer in case of meta transactions
// always using _msgSender()
    ...
    IERC1155ReceiverUpgradeable
{
    ...
    bytes32 private constant MODULE_TYPE = bytes32("NFTMarketplace");
    ...
    /// @dev The address of native token wrapper contract
    ...
    address private platformFeeRecipient = address(this);

    /// @dev The max bps of the contract
    uint64 private constant MAX_BPS = 10_000;

    /// @dev The % of primary sales collected as platform fees
    uint64 private platformFeeBps = 3;

    /**
     * @dev The amount of item added to an auction's endTime, if a bid is made within 'timeBuffer'
    ...
    /// @dev Mapping from uid of a direct listing => offeror address => offer made to the direct listing
    ...
    /// @dev Lets the contract receives native tokens from  `nativeTokenWrapepr` withdraw
    ...
    function createListing(ListingParameters memory _params) external override {
        ...
        // Check if token is approved for market, correct owner, and valid balance
        ...
        // Token listed for sale in an auction are escrowed in Marketplace
    ...
    /// @dev Let's a listing creator edit the listing's parameters
    function updateListing(
        uint256 _listingId,
        uint256 _quantityToList,
        uint256 _reservePricePerToken,
    ...
    function executeSale(
        ...
        // token owner transfer token to receiver
        transferListingToken(
            _targetListing.tokenOwner,
            _receiver,
            _listingTokenAmountToTransfer,
    ...
    function offer(
        ...
        if (targetListing.listingType == ListingType.Auction) {
            // A bid to an auction must be in the auction's desired currency
            require(
                newOffer.currency == targetListing.currency,
                "must be approved currency to bid"
            );
    ...
    function handleBid(Listing memory _targetListing, Offer memory _incomingBid)
        internal
    {
        Offer memory currencyWinningBid = winningBid[_targetListing.listingId];
        uint256 currentOfferAmount = currencyWinningBid.pricePerToken *
            currencyWinningBid.quantityWanted;
        uint256 incomingOfferAmount = _incomingBid.pricePerToken *
            _incomingBid.quantityWanted;
        address _nativeTokenWrapper = nativeTokenWrapper;

        // Close auction and executes sale if there's a buyout price and incoming offer amount is buyout price
        if (
        ...
        if (
            currencyWinningBid.offeror != address(0) && currentOfferAmount > 0
        ) {
            CurrencyTransferLib.transferCurrencyWithWrapper(
                _targetListing.currency,
                address(this),
                currencyWinningBid.offeror,
                currentOfferAmount,
                _nativeTokenWrapper
            );
        }
    ...
    /// @dev Checks whether an incoming bids is the new current highest bid
    ...
    /// @dev Lets an account close an auction for either (1) winning bidder, or auction creator
        ...
        onlyExistingListing(_listingId)
    {
        ...
        if (toCancel) {
            // let listing creator cancel auction
    ...
    /*///////////////////////////////////////////////////////////////
            Share (direct+auction listings) internal functions
    //////////////////////////////////////////////////////////////*/
    ...
    function payout(
        ...
        // pay platformfee for marketplace
        CurrencyTransferLib.transferCurrencyWithWrapper(
            _currencyToUse,
            _payer,
    ...
    /// @dev Validate that `_addrToCheck` owns and has approved marketplace to transfer the appropriate amount of currency
    ...
    /// @dev Check if token is approved for market, correct owner and valid balance
    ...
    function validateDireciListingSale(
        Listing memory _listing,
        address _payer,
        uint256 _quantityToBuy,
        address _currency,
        uint256 _settledTotalPrice
    ) internal {
        require(
        ...
        // Check whether a valid quantity of listed token is being bought
        require(
            _listing.quantity > 0 &&
                _quantityToBuy > 0 &&
        ...
        if (_currency == CurrencyTransferLib.NATIVE_TOKEN) {
            require(msg.value == _settledTotalPrice, "msg.value != price");
        } else {
            validateERC20BalAndAllowance(_payer, _currency, _settledTotalPrice);
        }

        // Check whether token owner owns and has approved `quantityToBuy` amount of listing tokens from the listings
        validateOwnershipAndApproval(
            _listing.tokenOwner,
    ...
    /// @dev get the quantity of the token (for ERC721 it always is 1)
    ...
    /// @dev get token type of the asset contract
    ...
    /// @dev Lets a contract admin update platform fee receipient and bps
    ...
    /// @dev Lets a contract admin set auction buffer
    function setAunctionBuffers(uint256 _timeBuffer, uint256 _bidBufferBps)
        external
