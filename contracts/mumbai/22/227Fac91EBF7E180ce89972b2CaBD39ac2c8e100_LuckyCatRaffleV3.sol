...
interface ICatRaffle_3_0 {
     ...
     * Emits RaffleClaimed
     */
    function claimRewards(uint256 _raffleId, uint256 _prizeId) external;
    ...
    PausableUpgradeable
{
    ...
    function enterRaffle(uint256 _raffleId, uint256 _numTickets) external {
        ...
        for (uint256 i = raffleEntries[_raffleId]; i < raffleEntries[_raffleId] + _numTickets; i++)
            ticketToUser[_raffleId][i] = msg.sender;
     ...
     */
    function claimRewards(uint256 _raffleId, uint256 _prizeId) external {
        if (claimed[_raffleId][_prizeId]) revert ICatRaffle_3_0__AlreadyClaimed();
        if (!raffleDrawn[_raffleId]) revert ICatRaffle_3_0__RaffleNotDrawn();

        address winningAddress = getWinningAddress(_raffleId, _prizeId);

        _givePrize(winningAddress, _raffleId, _prizeId);
        claimed[_raffleId][_prizeId] = true;
    ...
    function getWinningAddress(uint256 _raffleId, uint256 _prizeId) public view returns (address) {
        ...
        return ticketToUser[_raffleId][winningTicket];
    }
