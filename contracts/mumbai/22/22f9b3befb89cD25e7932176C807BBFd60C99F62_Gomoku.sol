...
contract Gomoku  is Initializable, UUPSUpgradeable, OwnableUpgradeable{
    ...
    event entryPlayerEvent(uint256 gamekey,address playerAddr);
    event startGameEvent(uint256 gamekey,address playerAddr);
    event standEndEvent(uint256 gamekey,address playerAddr,uint8 columns,uint8 raws);
    event turnEndEvent(uint256 gamekey,address playerAddr);
   ...
   function _authorizeUpgrade(address) internal override onlyOwner {}
    ...
    {
        //msg.senderが0x0じゃないかをチェック
        require(msg.sender!=address(0x0),"Don't use 0 addr");
        //セットされたステータスは正常値か
    ...
    function entryGame(uint256 gamekey) public {
        ...
        //アドレスが0x0の場合はリバート
        require(address(0x0) != msg.sender,"Don't use 0 addr");
...
library GameKeyHash 
{
    function createGameHash(uint256 nonce,address ownerAddr) internal pure returns(uint256 hash)
    {
