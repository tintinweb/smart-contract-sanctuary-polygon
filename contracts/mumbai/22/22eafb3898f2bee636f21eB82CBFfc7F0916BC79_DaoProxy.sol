...
contract DaoProxy is BaseContract {
    ...
    function setDaoAmount(uint256 _dao1Amount, uint256 _dao2Amount, uint256 _dao3Amount, uint256 _dao4Amount) external {
        (bool success, bytes memory result) = DaoAdminAddress.delegatecall(abi.encodeWithSelector(IDao.setDaoAmount.selector, _dao1Amount, _dao2Amount, _dao3Amount, _dao4Amount));
        _getRevertMsg(success, result);
    }
    ...
    function getUserData(
        ...
        _returnItem._daoRewardList = new uint256[](4);
        ...
        _returnItem.baseItem_ = BaseItem(daoToken, totalCompensation, investIndex, tatalInvestAmount, remainingCredit, lastInvestTime, investEndIndex, compensationInterval, rewardAllRateList, weekReward, totalInvestMember);
        _returnItem.baseItem2_ = BaseItem2(weekTop1AddressList, rewardRateList, rewardRateListForInviteRanking, rewardRateListForInvestRanking, addressConfig, Dao1Set.length(), Dao2Set.length(), Dao3Set.length(), Dao4Set.length(), daoToken != address(0) ? IERC20(daoToken).balanceOf(_user) : 0, _user.balance, DayRewardForInvestRankings[_day], DayRewardForInviteRankings[_day]);
    }
    ...
    function getBigNetAmountList(address[] memory _addressList) external view returns (DaoInfoItem[] memory DaoInfoItem_) {
        ...
        for (uint256 i = 0; i < _num; i++) {
            ...
            uint256 _otherAmounts = userInfoList[_user]._totalNetInvestment - userInfoList[_user]._depositAmount - _bigAmount;
            DaoInfoItem_[i] = DaoInfoItem(_user, _bigAmount, _otherAmounts);
        }
    }
