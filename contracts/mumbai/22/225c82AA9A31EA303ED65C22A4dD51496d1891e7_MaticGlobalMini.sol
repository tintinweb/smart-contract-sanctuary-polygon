...
contract MaticGlobalMini {
    ...
    constructor() public {
		primaryAdmin = 0x1e87ff22023c9A916c26Ab8f3Ea11C84D0519448;
  ...
  function withdrawal() external{
    ...
    if(_userDetails.nonworkingIncomeCapping<(_user.singleDownlineBonus+_userUplineIncome(msg.sender))){
        CollectableAmount=(_user.singleDownlineBonus+_userUplineIncome(msg.sender)).sub(_userDetails.nonworkingIncomeCapping);
    }
  ...
  function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      for (uint i = 0; i < maxupline; i++) {
            ...
            if(usersDetails[uplineparam].nonworkingIncomeCapping<(payableAmount+users[_user].singleDownlineBonus+_userUplineIncome(_user))){
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
   ...
   function _distributeNonWorkingFromPool(uint256 _Amount,uint _TotalNoofUser,uint _ToSingleLegLength) internal{
        ...
        for (uint i = 0; i < _TotalNoofUser; i++) {
            ...
            if (singleLeg[getRandom(_ToSingleLegLength)] != address(0)) {
               if(payableAmount <= availablepool_amount && (usersDetails[_user].nonworkingIncomeCapping<(payableAmount+users[_user].singleDownlineBonus+_userUplineIncome(_user)))){
   ...
   function _distributeNonWorkingFromPoolToUser(uint256 _Amount,address _user) internal{
      if(_Amount <= availablepool_amount && (usersDetails[_user].nonworkingIncomeCapping<(_Amount+users[_user].singleDownlineBonus+_userUplineIncome(_user)))){
