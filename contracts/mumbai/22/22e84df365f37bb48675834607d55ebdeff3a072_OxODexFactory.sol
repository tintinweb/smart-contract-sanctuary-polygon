...
contract OxODexFactory is Initializable, PausableUpgradeable  {
    ...
    event FeeChanged(uint256 fee, FeeType feeType); 
    event TokenChanged(address indexed token);
    event TreasurerChanged(address indexed treasurer);
    ...
    uint256 public tokenFeeDiscountPercent; // 0.1% of total supply  
    ...
    function initialize(
        ...
        if(_relayerAddress == address(0)) revert ZeroAddress();
        ...
        // init values
        tokenFeeDiscountPercent = 100;
        fee = 90; // 0.9% fee
        tokenFee = 45; // 0.45% fee
        relayerFee = 30; // 0.3% fee
    ...
    function setRelayer(address payable _relayerAddress) external onlyManager {
        if(_relayerAddress == address(0)) revert ZeroAddress();
        relayerAddress = _relayerAddress;

        emit RelayerChanged(_relayerAddress);
    }
...
contract OxODexTokenPool {
    ...
    modifier chargeForGas(uint256 relayerFee) {
        address _factory = factory;
        require(relayerFee <= IOxODexFactory(_factory).maxRelayerGasCharge(address(this)) , "RELAYER_FEE_TOO_HIGH");
        _;
        if(relayerFee > 0) {
            address payable relayerAddress = IOxODexFactory(_factory).relayerAddress();
            ERC20(token).transfer(relayerAddress, relayerFee);
        }
    }
    ...
    function _sendFundsWithRelayerFee(uint256 _amount, address _token, address _recipient) private returns(uint256 relayerFee){
        relayerFee = getRelayerFeeForAmount(_amount);
        address payable relayerAddress = IOxODexFactory(factory).relayerAddress();
        IERC20(_token).transfer(relayerAddress, relayerFee);
