...
contract BattleSetOperator is Context, IRentingTypes, ReentrancyGuard, Pausable, Ownable {
    ...
    constructor(address storageContractAddress, address myListingsStorageAddress,  address xoiliumMarketplaceAddress) {
    ...
    function cancelBattleSetListing(uint256 landId) nonReentrant external returns (bool) {
        ...
        if (status == TokenRentingStatus.AVAILABLE) {
            ...
            removeFromMyListingStorage(listing.owner, landId, listing.battleSet.botsIds);
    ...
    function listBattleSet(uint256 landId, uint256[] memory botIds, Coin coin, uint256 price,
        ...
        addToMyListingStorage(_msgSender(), landId, botIds);
    ...
    function addToMyListingStorage(address owner, uint256 landId, uint256[] memory botIds) private {
        myListingsStorage.add(TradedNft.RBXL, owner, landId);
        myListingsStorage.addAll(TradedNft.RBFB, owner, botIds);
    }

    function removeFromMyListingStorage(address owner, uint256 landId, uint256[] memory botIds) private {
        myListingsStorage.remove(TradedNft.RBXL, owner, landId);
