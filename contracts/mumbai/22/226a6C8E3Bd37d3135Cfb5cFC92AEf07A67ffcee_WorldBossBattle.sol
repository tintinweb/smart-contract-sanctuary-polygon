...
/// @dev all the funtion present here might be overriden 
interface BattleOps {
...
contract BattleData is GameAccessControls {
    ...
    event CreateBattle(address indexed adminAddress, string BattleId, string[] BossId, uint256 startedTime, uint256 endTime);
    event UpdateBattle(address indexed adminAddress, string BattleId, string[] BossId, uint256 startedTime, uint256 endTime);
    event DisableBattle(string BattleId, bool status);
    event EnableBattle(string BattleId, bool status);

    /// @notice makes a normal address admin
    /// @param normal_address address
    function makeAdmin(address normal_address) public {
        grantRole(DEFAULT_ADMIN_ROLE, normal_address);
    }

    /// @notice check whether address have admin role or not
    /// @param input_address address
    /// @return True if address has an admin role otherwise false
    function checkadmin(address input_address) public view returns (bool) {
        return hasRole(DEFAULT_ADMIN_ROLE, input_address);
    }

    /// @notice gives the end time of an battle in unix
    /// @param _battleId which is in string format
    function getBattleEndTime(string memory _battleId) public view returns (uint256) {
        return battles[_battleId].endTime;
    }

    /// @notice function to create battle requires battleId and bossId, starting time and ending time
    /** @param _battleId which is a string and identitfy battles,
     *  @param _bossId which is an array of string,
     * @param _startedTime and
     * @param _endTime are unix timestamp
     *  @return index in battleList
     */
    function createBattle(string memory _battleId, string[] memory _bossId, uint256 _startedTime, uint256 _endTime) public returns (uint index) {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to create battle.");
        require(_endTime > _startedTime, "NFTBattle: start time is bigger");
        for (uint i = 0; i < _bossId.length; i++) {
            require(checksBossStatus(_bossId[i]) != 0, "NFTStaking: boss not created");
        }
        address adminAddress = msg.sender;
        Battle storage battleInstance = battles[_battleId];
        battleInstance.battleId = _battleId;
        battleInstance.startedTime = _startedTime;
        ...
        emit CreateBattle(adminAddress, _battleId, _bossId, _startedTime, _endTime);
        return battleList.length - 1;
    }
     ...
     * @param _endTime are unix timestamp
     */
    function updateBattle(string memory _battleId, string[] memory _bossId, uint256 _startedTime, uint256 _endTime, bool status) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to update battle.");
        address adminAddress = msg.sender;
        Battle storage battleInstance = battles[_battleId];
        battleInstance.battleId = _battleId;
        battleInstance.startedTime = _startedTime;
        battleInstance.endTime = _endTime;
        battleInstance.status = status;
        for (uint i = 0; i < _bossId.length; i++) {
            battleInstance.bossId.push(_bossId[i]);
        }
        emit UpdateBattle(adminAddress, _battleId, _bossId, _startedTime, _endTime);
        battleList.push(_battleId);
    }
    ...
    /// @param _battleId its a string
    function checkBattleHasEnded(string memory _battleId) public view returns (uint256) {
        if (block.timestamp > battles[_battleId].endTime) {
            return 1;
        }
        return 0;
    }

    /// @notice function to change true staus of battle to false i.e make inactive
    /// @param _battleId its a string
    /// @dev only admin are allowed to disable if battle is enabled
    function disableBattle(string memory _battleId) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to update battle.");
        require(battles[_battleId].status = true, "disableBattle: battle status is already disabled or battle hasnt been created");
        battles[_battleId].status = false;
        emit DisableBattle(_battleId, battles[_battleId].status);
    }

    /// @notice function to change false staus of battle to true i.e make active
    /// @param _battleId its a string
    /// @dev only admin are allowed to enable if battle is disabled
    function enableBattle(string memory _battleId) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to update battle.");
        require(checksBattleStatus(_battleId) == false, "enableBattle: battle hasnt disabled");
        battles[_battleId].status = true;
        emit EnableBattle(_battleId, battles[_battleId].status);
    }

    /// @notice function to get information of battle by its Id
    /// @param _battleId its a string
    ...
    /// @notice to see all the battle that has been created
    ...
    event CreateBoss(address indexed adminAddress, string bossId, string name, uint256 maxHp, string uri);
    event UpdateBoss(address indexed adminAddress, string bossId, string name, uint256 maxHp, string uri);

    /// @notice function to create a new boss
    /** @param bossId which is a string and identitfy boss,
    ...
    function newBoss(string memory bossId, string memory name, uint256 maxHp, string memory uri) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to create boss.");
        address adminAddress = msg.sender;
        ...
        bossInstance.BossIndex = bossList.length - 1;
        emit CreateBoss(adminAddress, bossId, name, maxHp, uri);
    }
    ...
    function updateBoss(string memory bossId, string memory name, uint256 maxHp, string memory uri) public {
        require(checkadmin(msg.sender) != false, "NFTStaking: Unauthorized to update boss.");
        address adminAddress = msg.sender;
        Boss storage bossInstance = bosses[bossId];
        bossInstance.bossId = bossId;
        bossInstance.name = name;
        ...
        bossInstance.status = true;
        bossList.push(bossId);
        emit UpdateBoss(adminAddress, bossId, name, maxHp, uri);
    }

    function deleteBoss(address adminAddress) public {}

    /// @notice function to see all the boss that has been created
    /// @return array of string of bossid
    ...
    function checksBossStatus(string memory _bossId) public view returns (uint256) {
        if (bosses[_bossId].status == true) {
            return 1;
        }
        return 0;
    }
...
contract GameAccessControls is AccessControlUpgradeable {
    ...
    /// @notice this is a intialier funciton since constructor doesnt work in upgradable function
    ...
    /// @notice function to Revoke an address Admin role 
    ...
    /// @notice function to give an address minter role 
    /// @param _address address that will be getting role
    ...
    /// @notice function to Revoke an address minter role 
    /// @param _address address that will be revoked
...
contract WorldBattleSession is OwnableUpgradeable {
...
contract WorldBossBattle is Initializable, ContextUpgradeable, WorldBattleSession, BattleOps, BattleData {
    using ECDSA for bytes32;
    ...
    /// @notice Staker struct mapping
    mapping(address => Staker) public stakers;

    /// @notice tokenId is mapped to address
    ...
    */
    function initialize(ICDHNFTInventory _cdhNft, ITOWER _towerToken, address _nftHolder) public initializer {
    ...
    /// @notice modifier to regulate address should be admin
    modifier onlyAdminRole() {
        require(checkadmin(_msgSender()), "NFTStaking: Unauthorized.");
        _;
    }

    /// @notice modifier to regulate battleshould be created
    modifier checkStatusOfBattle(string memory _battleId) {
        require(checksBattleStatus(_battleId) == true, "NFTStaking: battle not created");
        _;
    }
    ...
    function setTowerToken(address _towerToken) external onlyAdminRole {
    ...
    ///@dev only admin can set that
    function setCDHNFTContractAddress(address _nftContract) external onlyAdminRole {
    ...
    ///@dev only admin can set that
    function setNFTHolderAddress(address _holder) external onlyAdminRole {
        nftHolder = _holder;
    }
    ...
    function checkReqiredTokenBalance(uint256 _nftCount, uint256 _balance) public pure returns (bool) {
    ...
    /// @param time in unix
    /// @return time
    function minStakePeriod(uint256 time) external onlyAdminRole returns (uint256) {
        cooldownPeriod = time;
    ...
    function internalStake(address _player, uint256 _tokenId, string memory _battleId) internal {
        require(checksBattleStatus(_battleId) == true, "NFTStaking: battle not created");
        require(checkBattleHasEnded(_battleId) == 0, "NFTStaking: battle ended");
        ...
        if (checkReqiredTokenBalance(existingStakedTokensCount + 1, towerBalance)) {
            ...
            emit NFTStaked(_player, _tokenId, _battleId, staker.stakedTokenTime[_tokenId]);
    ...
    function internalUnstake(address _player, uint256 _tokenId, string memory _battleId) internal checkStatusOfBattle(_battleId) {
        Staker storage staker = stakers[_player];

        require(block.timestamp > (staker.stakedTokenTime[_tokenId] + cooldownPeriod) || block.timestamp > getBattleEndTime(_battleId), "NFTUnStaking: cooldown not over");
    ...
    function internalRestake(address _player, uint256 _tokenId, string memory _battleId) internal checkStatusOfBattle(_battleId) {
