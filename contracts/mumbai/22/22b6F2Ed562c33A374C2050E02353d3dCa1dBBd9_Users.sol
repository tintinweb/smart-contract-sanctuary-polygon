...
contract Users is AccessControl, ERC2771Context {
  ...
  function createUser(string memory _email, string memory _password, address _wallet) external onlyTrustedForwarder{
    require(hasRole(DEFAULT_ADMIN_ROLE, _msgSender()), 'US: Not Admin ');
    require(!userExists[_email], 'US: user already exists');
    UsersData[_email] = User(
      _email,
      _wallet,
      _password
    );
  ...
  function getUser(string memory _email) external onlyAdmin  view returns ( string memory email, string memory password, address wallet) {
    require(!userExists[_email], 'US: user already exists');
