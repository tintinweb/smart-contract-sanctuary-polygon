    ...
    EssentialERC2771Context
{
    ...
    uint256 public constant TOTAL_SUPPLY = 2100;

    /// ============ Mutable storage ============
    /// @notice Number of assigned tokens
    uint256 public shuffledCount;
    /// @notice Number of Wrasslers per minted Packs
    uint256 public mintedSupply;

    mapping(bytes32 => Token) public openers;

    /// @notice Global array of shuffled tokenIds.
    ...
    constructor(
        ...
        ERC1155("ipfs://QmPKfR2SfABB3n7P71sx6iU6efGr6N3e7tBKrDnZJVS8KP/{id}.json")
        VRFConsumerBase(_LINK_VRF_COORDINATOR_ADDRESS, _LINK_ADDRESS)
        EssentialERC2771Context(_trustedForwarder)
    {
    ...
    /// @notice Claim selection of 1, 3 and 5 packs per airdrop
    /// @param packs selection of packs by tokenId
    /// @param signature signature created offchain and provided to user
    function claimPacks(uint256[] calldata packs, bytes calldata signature) external {
        uint256 total;
        uint256[] memory amounts;
        for (uint i = 0; i < packs.length; i++) {
            require(packs[i] == 1 || packs[i] == 3 || packs[i] == 5, "Invalid pack ID");
            total += packs[i];
            amounts[i] = 1;
        }

        require(validateSignature(_msgSender(), total, signature), "Invalid claim");

        _mintBatch(_msgSender(), packs, amounts, "");

        mintedSupply += total;

        require(mintedSupply <= TOTAL_SUPPLY, "Invalid mint");
    }

    /// @notice Burn card pack NFT to receive randomized NFTs
    /// @param tokenId corresponds to count of NFTs to randomly assign
    function openPack(uint64 tokenId) external {
        _burn(_msgSender(), tokenId, 1);

        // request random entropy for pack
        bytes32 requestId = requestRandomness(KEY_HASH, LINK_FEE);

        // store minter and pack count for randomness request
        openers[requestId] = Token({minter: _msgSender(), id: tokenId});

        // event mostly used for testing
    ...
    function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        Token memory pass = openers[requestId];
        randomizePack(pass.id, randomness, pass.minter);
    }

    function randomizePack(uint256 numShuffles, uint256 entropy, address minter) internal {
        uint256 nextId = shuffledCount + numShuffles;

        for (uint256 i = shuffledCount; i < nextId; ) {
            // Generate a random index to select from
            uint256 randomIndex = i + (entropy % (TOTAL_SUPPLY - i));

            // Collect the Token at that random index - this has been warmed or swapped to
            Token memory assignedToken = tokens[randomIndex];

            // Move the less random Token from the current index to randomIndex
            tokens[randomIndex] = tokens[i];

            // Set minter on assigned Token
            assignedToken.minter = minter;

            // Write assigned Token to storage at current Index
            tokens[i] = assignedToken;

            // Emit event used for creating offchain signatures
            emit Assigned(minter, assignedToken.id);
        ...
        shuffledCount = nextId;
    }

    /// ============ Admin Functions ============

    function devMintMultiple(Token[] calldata addresses) external onlyOwner {
        uint256 count = addresses.length;
        require(LINK_TOKEN.balanceOf(address(this)) >= LINK_FEE * count, "Not enough LINK");

        for (uint256 index = 0; index < count; ) {
            Token calldata pass = addresses[index];
            require(mintedSupply + pass.id <= TOTAL_SUPPLY, "Mint pass exceeds token supply");
            _mint(pass.minter, pass.id, 1, "");
            unchecked {
                ++index;
                mintedSupply += pass.id;
            }
        }
    }

    /// @notice Warm the Fisher Yates shuffle mapping
    /// @param ids array of randomized tokenIds
    /// @dev warming the tokens array with offchain randomness helps remove some bias in Fisher Yates
    /// while also saving some gas on chainlink callback and simplifying the shuffle function
    function warm(uint64[] calldata ids, uint256 offset) external onlyOwner {
        require(tokens[offset].id == 0, "Already warmed");
        uint256 nextOffset = offset + ids.length;
        for (uint256 i = offset; i < nextOffset; ) {
            uint64 id = ids[i - offset];
            tokens[i] = Token({id: id, minter: address(0)});
            unchecked {
                ++i;
            }
        }
    ...
    /// @param start the raffle tranche
    /// @param count the raffle tranche
    ...
    /// to claim their assigned tokens.
    function buildMessages(uint256 start, uint256 count) external view returns (Signable[] memory signables) {
        signables = new Signable[](count);
        uint256 max = count + 1;

        for (uint256 index = start; index < max; index++) {
            Token memory token = tokens[index];
            address account = token.minter;
            uint256 tokenId = token.id;
            signables[index] = Signable({message: createMessage(account, tokenId), nonce: tokenId, minter: account});
        }
    }

    function setSigner(address _signer) external {
        _setAllowancesSigner(_signer);
    }
...
abstract contract Assigner {
    event Assigned(address indexed minter, uint64 indexed tokenId);
}
...
contract SignedAllowance {
    ...
    function validateSignature(address account, uint256 nonce, bytes memory signature) public returns (bool) {
    ...
    function _validateSignature(
        ...
        if (message.recover(signature) != signer) return false;
        if (usedAllowances[message] == true) return false;
    ...
    function _useAllowance(address account, uint256 nonce, bytes memory signature) internal returns (bool) {
        return validateSignature(account, nonce, signature);
    }
