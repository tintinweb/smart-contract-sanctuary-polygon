...
interface IERC20Zetadata is IERC20 {
    function name() external view returns (string memory);
    function symbol() external view returns (string memory);
    function decimals() external view returns (uint8);
}

contract ERC20 is Context, IERC20, IERC20Zetadata {
...
contract ZetaLocker {
    ...
    function lockSend(string calldata receiver, uint amount, uint wanted, string calldata chainid, bytes calldata message) external {
        bool success = zeta.transferFrom(msg.sender, address(this), amount); 
        require(success == true, "lockSend: transfer fails"); 
        emit LockSend(msg.sender, receiver, amount, wanted, chainid, message); 
    }

    function unlock(address receiver, uint256 amount, bytes32 sendHash) external {
        require(msg.sender == TSSAddress, "need unlockAdmin permission"); 
        uint lockedAmount = getLockedAmount();
        // supply: the total supply of M on all chains except Ethereum
        // locked: the total M locked in this contract
        (uint supply,) = oracle.getSupplyAndLocked();
        // make sure that mint does not inflate supply of M:
        // unlock() must be after a commensurate burn on other chains. 
        // Because supply oracle is not real time, we allow 2% flexibility. 
        require(supply + amount <= lockedAmount * flexibility / 100, "total supply exceeded");
        bool success = zeta.transfer(receiver, amount);
        require(success == true, "unlock: transfer failed"); 
        emit Unlock(receiver, amount, sendHash);
    }
}

contract TestSupplyOracle {
    uint supply;
    uint locked;  
    address owner; 
