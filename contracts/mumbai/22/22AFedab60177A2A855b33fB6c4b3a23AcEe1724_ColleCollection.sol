...
contract ColleCollection is IColleCollection, SignatureValidator, ERC721URIStorage, MarketHubRegistrar {
    ...
    function permitSetApprovalForAll(
        ...
        isValidSignature(
            _owner,
            _getPermitSetApprovalForAllHash(_owner, _operator, _approve, _deadline),
            _deadline,
            _signature
        )
    {
        _setApprovalForAll(_owner, _operator, _approve);
    }
     ...
     * @param _deadline The deadline for the permit.
     */
    function _getPermitSetApprovalForAllHash(
        address _owner,
        address _operator,
        bool _approve,
        uint256 _deadline
            ...
            keccak256(
                abi.encodePacked(
                    ...
                    keccak256(
                        abi.encode(
                            keccak256(
                                "SetApprovalForAll(address owner,address operator,bool approve,uint256 deadline,uint256 nonce)"
                            ),
                            _owner,
                            _operator,
                            _approve,
