...
contract ERC20Upgradeable is Initializable, ContextUpgradeable, IERC20Upgradeable, IERC20MetadataUpgradeable {
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(_lock[account].lockupList.length == 0,"burn is not allowed because it is locked");
        require(account != address(0), "The 'burn' function is not available when the lock is set");
    ...
    /*==================================================================
                            전송하고 잠금설정  
    ==================================================================*/
    function transferAndLockup(address to, uint32 lockupTime, uint128 lockupAmount) external { 
        _ProjectFunctions();
    ...
    function lockupPlanSet(address account, uint32 lockupTime, uint128[] calldata lockupPlanTime, uint128[] calldata lockupPlanAmount) external returns (bool) {
        ...
        for (uint32 i=0; i<lockupPlanTime.length;){
            uint128 i_lockupPlanTime = lockupPlanTime[i];
            uint128 i_lockupPlanAmount = lockupPlanAmount[i];
            require((_initialLock[account][lockupTime]._lockupPlan[i_lockupPlanTime].lockupPlanAmount == 0) && (i_lockupPlanTime >= lockupTime) && (i_lockupPlanTime >= block.timestamp) ,"Incorrect lockupPlanTime");
            require(i_lockupPlanAmount != 0,"Incorrect lockupPlanAmount");
            unchecked{addAmount += i_lockupPlanAmount;}
            _initialLock[account][lockupTime]._lockupPlan[i_lockupPlanTime] = lockupPlanInfo(uint32(_initialLock[account][lockupTime].lockupPlanList.length),i_lockupPlanAmount);
            _initialLock[account][lockupTime].lockupPlanList.push(i_lockupPlanTime);
            unchecked{ i++; }
        }
        uint128 lockTotalAmount = _initialLock[account][lockupTime].lockupPlanTotalAmount+addAmount;
        require(lockup_Amount >= lockTotalAmount ,"lockupAmount exceeded");
        _initialLock[account][lockupTime].lockupPlanTotalAmount = lockTotalAmount;
        return true;
    }
    /*==================================================================
                            잠금플랜 삭제 
    ...
    modifier FreezeStatus(address from,address to) {
        require(!_freezingStatus[from], "Frozen caller address");
        require(!_freezingStatus[to], "Frozen recipient address");
        _;
    }
