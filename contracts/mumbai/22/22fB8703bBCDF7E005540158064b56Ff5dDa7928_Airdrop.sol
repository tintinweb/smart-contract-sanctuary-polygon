...
contract Airdrop is AccessControl, EIP712 {
    ...
    MetarunCollection public collectionToken;
     ...
     */
    constructor(address _token) EIP712("MetarunAirdrop", "0.1") {
        require(_token != address(0), "Empty collection token address");
        collectionToken = MetarunCollection(_token);
        _setupRole(ALLOCATOR_ROLE, msg.sender);
    }

    function claim(bytes memory signature) public {
        bytes32 allocationDigest = _getTypedAllocationDigest(msg.sender);
        ...
        require(!claimed[msg.sender], "This address claimed yet");

        uint256 idx = _getRandomTokenId();
        collectionToken.mint(msg.sender, idx, 1);
        owners[idx] = msg.sender;
    ...
    function _getAllocationStructHash(address rewardee) internal pure returns (bytes32) {
        bytes32 allocationTypeHash = keccak256("Allocation(address rewardee)");
        return (keccak256(abi.encode(allocationTypeHash, rewardee)));
    }

    function _getTypedAllocationDigest(address rewardee) internal view returns (bytes32) {
        return (_hashTypedDataV4(_getAllocationStructHash(rewardee)));
    }

    function _getRandomTokenId() internal view returns (uint256) {
        uint256 tokenKind;
        uint256 tokenId;
        uint256 randomKindInt = uint256(keccak256(abi.encodePacked(block.timestamp, gasleft(), blockhash(block.number - 1)))) % 100;
        if (randomKindInt < 75) {
            tokenKind = 0x0600;
        } else if (randomKindInt < 95) {
            tokenKind = 0x0601;
        } else {
            tokenKind = 0x0602;
        }

        uint256 randomIdInt = uint256(keccak256(abi.encodePacked(block.timestamp, gasleft(), blockhash(block.number - 1)))) % 1000;

        tokenId = ((tokenKind << 16) | (randomIdInt));
        // console.log("Random id is ", tokenId);
        while (owners[tokenId] != address(0)) {
            // console.log("Id %s is Already taken", tokenId);
            tokenId += 1;
        }
        // console.log("Id %s is available", tokenId);
        return tokenId;
    }
}
...
contract MetarunCollection is ERC1155Upgradeable, AccessControlUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable {
    ...
    uint256 public constant COMMON_2D_KIND = 0x0600;
    uint256 public constant RARE_2D_KIND = 0x0601;
    uint256 public constant MYTHICAL_2D_KIND = 0x0602;
