    ...
    AccessControlUpgradeable
{
    ...
    function firstSale(address to) external onlyRole(MINTER_ROLE) {
        require(_mintStage == 0, "First sale is already done");

        _mint(to, _firstSupply);
        _mintStage++;
    }

    function secondSale(address to) external onlyRole(MINTER_ROLE) {
        require(_mintStage == 1, "Not second step");

        _mint(to, _secondSupply);
        _mintStage++;
    }

    function thirdSale(address to) external onlyRole(MINTER_ROLE) {
        require(_mintStage == 2, "Not third step");
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        require(_mintStage == 3, "Private sale is not ended");
