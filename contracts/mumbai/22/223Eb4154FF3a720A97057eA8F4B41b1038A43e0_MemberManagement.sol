...
/// @title Member Management smart contract for CryptoMintr Protocol
/// @author Naman R. Bhardwaj, Lokesh Nalot
/// @notice This smart contract manages members and networks for the CryptoMintr protocol
/// @dev For using on multiple chains, the numbering distinction may have to be changed, or maybe simply managed in database. Also commission percentages are denoted as 1000 for 100% throughout
...
contract MemberManagement is ReentrancyGuard {
    ...
    uint256 public lowerCappingCommisions = 0; //0%
    uint256 public upperCappingCommisions = 100; //10%
    // Member_id to Member
    ...
    function changeLowerCapping(uint256 _new) public onlyOwner {
        require(
            _new < upperCappingCommisions,
            "Lower capping cannot be same or greater than upper capping"
        );
    ...
    function changeUpperCapping(uint256 _new) public onlyOwner {
        require(
            _new > lowerCappingCommisions,
            "Upper capping cannot be same or lesser than lower capping"
        );
    ...
    //change network's lister commission
    ...
    //change network's seller commission
    ...
    //change network's protocol commission
    ...
    //change independent protocol fee for a member
    ...
    //change independent fee for a member
