    ...
    NativeMetaTransaction
{
     ...
     */
    function batchMint(address[] memory _recipients, string[] memory _tokenURIs)
        external
        onlyRole(MINTER_ROLE)
    {
        require(_tokenURIs.length != 0, "Error: Token URIs are empty!");
        require(_recipients.length != 0, "Error: Recipinets are empty!");
        require(
            _recipients.length == _tokenURIs.length,
            "Error: Recipinets are empty!"
        );
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(bytes(_tokenURIs[i]).length != 0, "Error: Token URI is empty!");
            uint256 currentTokenId = _nextTokenId.current();
            _nextTokenId.increment();
            _safeMint(_recipients[i], currentTokenId);
