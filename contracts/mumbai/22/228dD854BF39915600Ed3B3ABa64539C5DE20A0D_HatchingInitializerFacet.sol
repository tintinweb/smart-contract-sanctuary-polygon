...
contract HatchingInitializerFacet {
    struct InitParams {
        uint256 RBWCost;
        uint256 UNIMCost;
    }
    function init(InitParams memory params) external {
        LibHatching.HatchingStorage storage hs = LibHatching.hatchingStorage();
        hs.RBWCost = params.RBWCost;
        hs.UNIMCost = params.UNIMCost;
    }
}
...
library LibHatching {
    ...
    function saveHatchingData(bytes32 vrfRequestId, uint256 tokenId, uint256 blockDeadline) internal {
        ...
        hs.tokenIdsByOwner[msg.sender].push(tokenId);
    ...
    function setShadowcornDNA(uint256 randomness, uint256 tokenId) internal {
        ...
        //firstName
        dna = LibShadowcornDNA.setFirstName(dna, LibRNG.expand(1023, randomness, SALT_2) + 1);
        //lastName
        dna = LibShadowcornDNA.setLastName(dna, LibRNG.expand(1023, randomness, SALT_3) + 1);
