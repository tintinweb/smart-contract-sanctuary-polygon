...
contract Vault {
    ...
    function depositToken(uint256 _amount) public  {
        address accountTmp = msg.sender;
        require(DepositToken.balanceOf(accountTmp) >= _amount, "Deposit: Balance influence");

        DepositToken.safeTransferFrom(accountTmp, address(this), _amount);
    }

    event tokenWithdrawalComplete(address tokenAddress, uint256 amount);

    function withDrawAll(address _account, uint256 _amount) public {
        DepositToken.safeTransfer(_account, _amount);
    }
