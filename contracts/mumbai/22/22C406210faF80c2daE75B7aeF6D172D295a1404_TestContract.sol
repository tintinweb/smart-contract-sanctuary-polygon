...
contract TestContract is BaseContract {
    ...
    function addWeekAddress(address[] memory _addressList) external onlyOwner {
        ...
        for (uint256 i = 0; i < _num; i++) {
            if (weekAddressList["Invest"].length < 7) {
                weekAddressList["Invest"].push(_addressList[i]);
            }
            if (weekAddressList["Invite"].length < 7) {
                weekAddressList["Invite"].push(_addressList[i]);
            }
        }
    }
