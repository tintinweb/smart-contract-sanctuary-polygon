...
contract SwapNFT is AccessControl, ERC2771Context {

    /**
     * @dev Constructor for SwapNFT contract
     * @param _forwarder CustomForwarder instance address for ERC2771Context constructor
     */
    constructor(CustomForwarder _forwarder)
    ...
    modifier onlyTrustedForwarder() {
        require(
            ...
            "Swap: Only Trusted Forwarder"
        );
        _;
    }

    /**
     * @dev Emitted when `userA` swaps his tokens with tokens of `userB` successfully,
     * where all tokens are from same contract.
     */
    event SwapNFTBatch(
        address userA,
        INgageN nftAddressA,
    ...
    /**
     * @dev Emitted when `userA` swaps his tokens with tokens of `userB` successfully,
     * where tokens are from different contract.
     */
    event HybridSwapNFTBatch(
        address userA,
        INgageN[] nftAddressA,
        uint256[] idsA,
        uint256[] amountsA,
    ...
    /**
     @dev Function to swap NFTs between two users with the same NFT contract
     * @param userA address of the first user
     * @param nftAddressA address of the NFT contract of the first user
     * @param idsA array of NFT ids of the first user
     * @param amountsA array of amounts of NFTs to swap of the first user
     * @param userB address of the second user
     * @param nftAddressB address of the NFT contract of the second user
     * @param idsB array of NFT ids of the second user
     * @param amountsB array of amounts of NFTs to swap of the second user
     */
    function swapNFTs(
        address userA,
        INgageN nftAddressA,
        ...
        require(
            userA != userB,
            "Swap: User A and User B cannot be the same"
        );
        require(userA == _msgSender(), "Swap: Only lister can swap");
        // Verify signature of user B proposal

        nftAddressA.safeBatchTransferFrom(userA, userB, idsA, amountsA, dataA);
        nftAddressB.safeBatchTransferFrom(userB, userA, idsB, amountsB, dataB);
    ...
    /**
     @dev Function to swap NFTs between two users with the different NFT contract
     * @param userA address of the first user
     * @param nftAddressA address of the NFT contracts of the first user
     * @param idsA array of NFT ids of the first user
     * @param amountsA array of amounts of NFTs to swap of the first user
     * @param userB address of the second user
     * @param nftAddressB address of the NFT contracts of the second user
     * @param idsB array of NFT ids of the second user
     * @param amountsB array of amounts of NFTs to swap of the second user
     */
    function hybridSwapNFTs(
        address userA,
        INgageN[] memory nftAddressA,
        uint256[] memory idsA,
        uint256[] memory amountsA,
        bytes memory dataA,
        address userB,
        INgageN[] memory nftAddressB,
        uint256[] memory idsB,
        uint256[] memory amountsB,
        bytes memory dataB
    ) public onlyTrustedForwarder {
        ...
        for (uint256 i = 0; i < nftAddressA.length; i++) {
            nftAddressA[i].safeTransferFrom(userA, userB, idsA[i], amountsA[i], dataA);
        }
        for (uint256 j = 0; j < nftAddressB.length; j++) {
            nftAddressB[j].safeTransferFrom(userB, userA, idsB[j], amountsB[j], dataB);
        }

        emit HybridSwapNFTBatch(
            userA,
            nftAddressA,
            idsA,
            amountsA,
