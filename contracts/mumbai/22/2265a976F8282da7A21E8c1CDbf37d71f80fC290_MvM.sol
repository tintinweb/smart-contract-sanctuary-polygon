    ...
    ReentrancyGuard
{
     ...
     */
    function claim(address _receiver, uint256 _quantity, uint256 _obycTokenId,uint256 _mvmTokenId,
        uint256 _obycLabsTokenId, uint256 _level) public payable nonReentrant {
        
        // require(_currentIndex <= nextTokenIdToLazyMint, "Not enough lazy minted tokens.");
        
        // Add your claim verification logic by overriding this function.
        verifyClaim(msg.sender, _quantity, _obycTokenId,_obycLabsTokenId, _mvmTokenId,_level); 
     ...
     */
    function verifyClaim(address _claimer,
        uint256 _quantity,
        uint256 _obycTokenId,
        uint256 _obycLabTokenId,
        uint256 _mvmTokenId,
        uint256 _level) public payable virtual {}
...
interface IClaimableERC721 {
     ...
     */
    function claim(address _receiver, uint256 _quantity, uint256 _obycTokenId,uint256 _mvmTokenId,
        uint256 _obycLabsTokenId, uint256 _level) external payable;
    ...
    function verifyClaim(
       address _claimer,
        ...
        uint256 _obycLabTokenId,
        uint256 _mvmTokenId,
        uint256 _level) external payable;
}
...
contract MvM is ERC721A, Ownable {
    ...
    bool public isPausedMintL1 = true;
    bool public isPausedMintL2 = true;
        ...
        returns (bool)
    {
        for (
            uint256 index = 0;
            index < mvmL1Tokens.length;
            index++
        ) {
            if (
                transformInfoLevelOneByTokenId[mvmL1Tokens[index]].obycTokenId ==
                _obycTokenId
            ) {
                return true;
            }
    ...
    // event ConsoleLog(uint256 level,uint256 tokenid,uint256 status,string types);
        ...
        returns (uint256[2] memory)
    {
       ...
       if(level==1){
        ...
        for (
            ...
            if (
                transformInfoLevelTwoByTokenId[mvmL2Tokens[index]].obycTokenId ==
                _obycTokenId
            ) {
    ...
    function isCorrectLabTokenForL2Mint(
        ...
        if (
            transformInfoLevelOneByTokenId[_mvmTokenId]
                .obycLabsTokenId ==
            0 &&
            _obycLabTokenId == 2
        ) {
    ...
    function verifyClaim(
        ...
        require(_quantity == 1, "You Can Only Mint One");
        if (_level == 1) {
            require(isPausedMintL1, "Minting for L1 is Paused");
            ...
            transformInfoLevelOneByTokenId[_currentIndex]=transformInfoLevelOne;
            mvmL1Tokens.push(_currentIndex);
            tokenIdStatus[_obycTokenId] = 1;
        } else if (_level == 2) {
            require(isPausedMintL2, "Minting for L2 is Paused");
            address tokenIdAdd = ownerOf(_mvmTokenId);
            address sender = address(msg.sender);
            require(sender == tokenIdAdd, "You Dont Own the MvM NFT");
            require(
                !isAlreadyMintedMvML2(_mvmTokenId),
            ...
            require(
                isCorrectLabTokenForL2Mint(_obycLabTokenId, _mvmTokenId),
            ...
            TransformInfoLevelTwo
                memory transformInfoLevelTwo = TransformInfoLevelTwo(
                    msg.sender,
                    _currentIndex,
                    transformInfoLevelOneByTokenId[_mvmTokenId].obycTokenId,
                    _obycLabTokenId
                );
            transformInfoLevelTwoByTokenId[_currentIndex]=transformInfoLevelTwo;
            mvmL2Tokens.push(_currentIndex);
            tokenIdStatus[transformInfoLevelOneByTokenId[_mvmTokenId].obycTokenId] = 2;
        } 
    }
    ...
    function setCanMintL2(bool _isPausedMintL2) public onlyOwner {
        isPausedMintL2 = _isPausedMintL2;
    }
