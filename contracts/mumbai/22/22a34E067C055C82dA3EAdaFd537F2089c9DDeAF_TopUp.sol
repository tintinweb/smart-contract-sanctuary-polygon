...
contract TopUp is ITopUp, OwnableUpgradeable, PausableUpgradeable, AuthModifiers, UUPSUpgradeable {
    ...
    function _swapFromPair(uint256 _amountIn, uint256 _amountOutMin) internal whenNotPaused returns (uint256, uint256) {
        ...
        uint256 _swapPrice = (_amounts[0] * 1e18) / _amounts[2];
    ...
    function topUpCustodial(
        ...
        emit ToppedUp(_relayerAddress, msg.sender, _availableFuel, _amountFuel, _swapPrice);
    }

    function topUpNonCustodial(address _relayerAddress, uint256 _amountFuel) external whenNotPaused {
        uint256 _price = registry.priceOracle().price();
        uint256 _availableFuel = registry.economics().topUpIntegrator(_relayerAddress, msg.sender, _amountFuel, _price);
        emit ToppedUp(_relayerAddress, msg.sender, _availableFuel, _amountFuel, _price);
    }
...
interface ITopUp {
    ...
    function topUpNonCustodial(address _relayerAddress, uint256 _amountFuel) external;
