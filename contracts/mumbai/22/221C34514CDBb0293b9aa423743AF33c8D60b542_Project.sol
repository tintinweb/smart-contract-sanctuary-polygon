...
interface IProject {
    ...
    function getTotalBuyersWaitingDistribution(
        uint256 _projectId
    ) external view returns (uint256);

    function addTotalBuyersWaitingDistribution(uint256 _projectId) external;

    function setSoldQuantityToProject(
        uint256 _projectId,
        uint256 _quantity
    ) external;
...
interface ISale {
    function getSalesProject(
        uint256 projectId
    ) external view returns (SaleInfo[] memory);

    function getSaleIdsOfProject(
        uint256 _projectId
    ...
    function close(
        uint256 closeLimit,
        ProjectInfo memory _project,
        SaleInfo memory _sale,
        uint256 _totalBuyersWaitingClose,
        bool _isGive
    ) external returns (uint256);

    function createSale(
        address _caller,
        bool _isCreateNewToken,
        bool _isSetRoyalty,
        ProjectInfo memory _project,
        SaleInput memory _saleInput
    ) external returns (uint256);
    ...
    function getSaleNotCloseLength(
        uint256 _projectId
    ) external view returns (uint256);

    function getSaleIdNotCloseByIndex(
        uint256 _projectId,
        uint256 _index
    ) external view returns (uint256);
}
...
contract Project is IProject, ContextUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 public minShareProfitPercent; /// Profit sharing on each product sale batch of the end-users
    ...
    /// @dev Emit an event when minShareProfitPercent is updated
    event SetMinShareProfitPercent(uint256 indexed oldValue, uint256 indexed newValue);
    ...
    event SetTotalBuyersWaitingDistribution(
     ...
     * @param _input.minShareProfitPercent -> Profit sharing on each product sale batch of the end-users
    ...
    function initialize(InitializeInput memory _input) external initializer {
        ...
        minShareProfitPercent = _input.minShareProfitPercent;
    ...
    /**
     * @notice Set the new minShareProfitPercent
     * @param _minimum New minimum value
     */
    function setProfitShareMinimum(uint256 _minimum) external onlySuperAdmin {
        uint256 oldValue = minShareProfitPercent;
        minShareProfitPercent = _minimum;
        emit SetMinShareProfitPercent(oldValue, _minimum);
    }
    ...
    function closeProject(
        uint256 _projectId,
        uint256[] memory _saleIds,
        bool _isGive
    ) external nonReentrant onlyManager(_projectId) {
        ...
        for (uint256 i = 0; i < _saleIds.length; ++i) {
            ...
            if (saleInfo.amount > 0) {
                ...
                _project.isSingle
                    ...
                    : IOSB1155(_project.token).safeTransferFrom(
                        address(sale),
                        _project.manager,
                        saleInfo.tokenId,
                        saleInfo.amount,
            ...
            totalBuyersWaitingDistribution = sale.close(
                _closeLimit - _count,
                _project,
                saleInfo,
                totalBuyersWaitingDistribution,
                _isGive
            );
        }
    ...
    function setSoldQuantityToProject(
        uint256 _projectId,
        uint256 _quantity
    ) external projectIsValid(_projectId) onlySale {
    ...
    function isManager(uint256 _projectId, address _account) public view returns (bool) {
        if (projects[_projectId].isCreatedByAdmin) {
            return
                setting.isSuperAdmin(_account) || setting.isAdmin(_account) || _account == projects[_projectId].manager;
        } else {
    ...
    function publish(ProjectInput memory _projectInput, SaleInput[] memory _saleInputs) external payable nonReentrant {
        ...
        require(
            isCreatedByAdmin ? _projectInput.profitShare == 0 : _projectInput.profitShare >= minShareProfitPercent,
            "Invalid profitShare"
        );
        require(
            _projectInput.saleStart >= block.timestamp && _projectInput.saleStart < _projectInput.saleEnd,
            "Invalid sale time"
        );
    ...
    ) external nonReentrant projectIsValid(_projectId) onlyManager(_projectId) {
