...
contract DuelNFTStaking is Ownable, ERC721Holder {

    IERC20Token public rewardsToken;
    IERC721 public nft;
    address public feeRecipient;
    uint public feeAmount = 1; // 1%

    uint256 public stakedTotal;
    uint256 public stakingStartTime;
    uint256 constant stakingTime = 180 seconds;
    uint256 public token = 42e14;
    ...
    constructor(IERC721 _nft, IERC20Token _rewardsToken, address _feeRecipient) {
        nft = _nft;
    ...
    function initStaking() public onlyOwner {
        ...
        require(!initialised, "DuelNFTStaking: Already initialised");
    ...
    function setFeeAmount(uint _feeAmount) public onlyOwner {
        require(_feeAmount<100, "Fee amount must be under 100");
    ...
    function setTokenReward(uint256 _reward) public onlyOwner {
        //needs access control
        token = _reward;
    }
    ...
    function checkReward(address _user)
        public view
        returns (uint256 balance)
    {   
        
        Staker storage staker = stakers[_user];
        uint256 currentBalance = staker.balance;
        uint256[] storage ids = staker.tokenIds;
        for (uint256 i = 0; i < ids.length; i++) {

            if ( staker.tokenStakingCoolDown[ids[i]] < block.timestamp + stakingTime && staker.tokenStakingCoolDown[ids[i]] > 0) {

                // @get stakedDays
                uint256 stakedDays = ((block.timestamp - uint(staker.tokenStakingCoolDown[ids[i]]))) / stakingTime;

                if (stakedDays > 0) {
                    currentBalance +=  token * stakedDays;
                }
    ...
    /// @notice getting current blocktime
    function getBlockTime()
        public view
        returns (uint blockTime)
    {   
    ...
    function _stake(address _user, uint256 _tokenId) internal {
        require(initialised, "DuelNFTStaking: the staking has not started yet");
        require(
            nft.ownerOf(_tokenId) == _user,
            "DuelNFTStaking: User must be the owner of the token"
        );
        Staker storage staker = stakers[_user];
    ...
    function emergencyUnstake(uint256 _tokenId) public {
        require(
            ...
            "DuelNFTStaking: Sender must have staked tokenID"
        );
        _unstake(msg.sender, _tokenId);
    ...
    function _unstake(address _user, uint256 _tokenId) internal {
        ...
        require(
            tokenOwner[_tokenId] == _user,
            "DuelNFTStaking: user must be the owner of the staked nft"
        );
        Staker storage staker = stakers[_user];
    ...
    function claimReward(address _user) public {
        ...
        require(tokensClaimable == true, "DuelNFTStaking: Token claiming not started yet");
        require(stakers[_user].tokenIds.length > 0 , "DuelNFTStaking: No staked token");
        ...
        if (stakers[_user].balance > 0) {

            //rewardsToken.mint(_user, stakers[_user].balance);
            rewardsToken.mint(address(this), stakers[_user].balance);

            stakers[_user].rewardsReleased += stakers[_user].balance;
            uint256 reward = stakers[_user].balance;
            stakers[_user].balance = 0;

            rewardsToken.transferFrom(address(this),_user, reward-((reward*feeAmount)/100));
            rewardsToken.transferFrom(address(this),feeRecipient,(reward*feeAmount)/100);
       
            //emit RewardPaid(_user, stakers[_user].balance-(stakers[_user].balance*feeAmount/100));
            //emit RewardPaid(feeRecipient, (stakers[_user].balance*feeAmount)/100);
        }
