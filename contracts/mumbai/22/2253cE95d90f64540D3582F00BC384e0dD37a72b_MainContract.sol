...
import "../abstract/VRFConsumerBaseV2Upgradable.sol";

contract MainContract is Initializable, OwnableUpgradeable, UUPSUpgradeable, Modifiers, VRFConsumerBaseV2Upgradable {
  ...
  function initialize(address _vrfCoordinator, bytes32 _keyHash) public initializer {
    __Ownable_init();
    __UUPSUpgradeable_init();
    initCoordinator(_vrfCoordinator);
  ...
  function updateChainlinkSubscriptionVRF(uint64 _subscriptionId) public onlyOwner {
  ...
  function fulfillRandomWords(
    ...
    for (uint _i = 0; _i < _randomWords.length; ++_i) {
      randomWords.push(_randomWords[_i]);
    }
  }

  function getRandomVRF(uint _max, uint8 _shift) public returns (uint) {
    uint _random;
    if (randomWords.length > 0) {
      uint _randomWord = randomWords[0];
      _random = _randomWord % _max;

      randomWords[0] = randomWords[randomWords.length - 1];
      randomWords.pop();
    } else {
      // no random words fallback
      _random = uint(keccak256(abi.encodePacked(_shift, msg.sender, block.difficulty, block.timestamp, uint(1)))) % _max;
    }
  ...
  function checkRandomVRF(uint _max) public returns (uint, uint) {
    uint _randomWord = randomWords[0];
    uint _random = _randomWord % _max;
    return (_randomWord, _random);
  }
}
...
abstract contract VRFConsumerBaseV2Upgradable {
   ...
   */
    ...
    function initCoordinator(address _vrfCoordinator) internal {
