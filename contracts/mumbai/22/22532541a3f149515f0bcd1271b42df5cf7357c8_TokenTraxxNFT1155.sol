 ...
 * account.
 */
abstract contract ERC1155EnumerableUpgradeable is Initializable, ERC1155Upgradeable, ErrorCodes {
    function __ERC1155EnumerableUpgradeable_init() internal onlyInitializing {
    }

    function __ERC1155EnumerableUpgradeable_init_unchained() internal onlyInitializing {
    }
     ...
     */
    function _removeTokenFromOwnerEnumeration(address from, uint256 tokenId, uint256 amount) internal {
        if(balanceOf(from, tokenId) == amount){
            uint256 _tokenIndex = _ownedTokensIndex(from, tokenId);
            _ownedTokens[from][_tokenIndex] = _ownedTokens[from][_ownedTokens[from].length-1];
            _ownedTokens[from].pop();
        }
    }    

    function _ownedTokensIndex(address owner, uint256 tokenId) internal virtual returns (uint256 index) {
        uint256 length = _ownedTokens[owner].length;
        bool found;
        for(index = 0; index < length; index ++) {
            if(_ownedTokens[owner][index] == tokenId) { found = true; return index; }
        }
        require(
            found,
...
interface IERC1155Custom{
    ...
    ) external view returns (uint256 maxQuantityAllowed);
    ...
    IERC1155Custom
{
    ...
    event CollectionCreated(Collection indexed collectionData);
    event CollectionUpdated(Collection indexed collectionData);
    ...
    function createCollection(
        ...
        require(bytes(_collection.title).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(bytes(_collection.description).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(bytes(_collection.thumbnail).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(bytes(_collection.bgImage).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(bytes(_collection.bgColor).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        //require(_tokenIds.length > 0, "tokenIds is required");
        require(_collection.owner != address(0), throwError(ZERO_ADDRESS));
        require(
            _collection.userPurchaseLimit > 0 || _collection.userPurchaseLimit == -1,
            throwError(INVALID_COLLECTION_DETAIL)
        );
        require(
            _collection.allowedPurchaseLimit > 0 || _collection.allowedPurchaseLimit == -1,
            throwError(INVALID_COLLECTION_DETAIL)
        );
        ...
        _collections[id].allowedPurchaseLimit = _collection.allowedPurchaseLimit;
        _collections[id].primarySoldCount = 0;
        uint256 wlParamsLength = _collection.whitelistParams.length;
        
        for(uint256 i =0; i<wlParamsLength; i++){
            _collections[id].whitelistParams.push(_collection.whitelistParams[i]);
        }

        _collectionIdTracker.increment();

        emit CollectionCreated(_collections[id]);
    ...
    function addTokensToCollection(
        ...
        require(_collectionId >= 0, throwError(COLLECTION_ID_CANNOT_BE_LESS_THAN_ZERO));
        require(
        ...
        emit CollectionUpdated(_collections[_collectionId]);
    ...
    */
    function updateCollectionAfterPrimarySale(
        ...
        require(msg.sender == market, throwError(SENDER_SHOULD_BE_MARKETPLACE_CONTRACT));
    ...
    function updateCollection(
        ...
        require(msg.sender == _collections[_id].owner, throwError(INVALID_OWNER));
        ...
        require(bytes(_collection.bgImage).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(bytes(_collection.bgColor).length > 0, throwError(INVALID_COLLECTION_DETAIL));
        require(_collection.tokenIds.length > 0,throwError(INVALID_COLLECTION_DETAIL));
        require(
            _collection.allowedPurchaseLimit > 0 || _collection.allowedPurchaseLimit == -1,
            throwError(INVALID_COLLECTION_DETAIL)
        );
       // require(_whitelistParams.length > 0, "whitelistParams is required"); // should we remove this and add a separate setter for whitelist params?
        ...
        _collections[_id].allowedPurchaseLimit = _collection.allowedPurchaseLimit;
        _collections[_id].updatedAt = block.timestamp;
        uint256 wlParamsLength = _collection.whitelistParams.length;
        
        for(uint256 i =0; i<wlParamsLength; i++){
            _collections[_id].whitelistParams.push(_collection.whitelistParams[i]);
        } 
        
        
        // _collections[_id].whitelistParams = _collection.whitelistParams;

        emit CollectionUpdated(_collections[_id]);
    ...
    function addWhitelistToCollection(
        ...
        WhitelistParams[] memory _whitelistParams
            ...
            uint256 wlParamsLength = _whitelistParams.length;
            require(wlParamsLength > 0, throwError(INVALID_WHITELIST_PARAMS));
            
            for(uint256 i =0; i<wlParamsLength; i++){
                _collections[_id].whitelistParams.push(_whitelistParams[i]);
            } 
            
           // _collections[_id].whitelistParams = _whitelistParams;
        ...
        )
    {
         for(uint256 i = 0; i < ids.length; ++i){
            ...
            if (to == address(0)) {//burn
                ...
                require(
                    balanceOf(from, tokenId) >= amount,
                    throwError(INSUFFICIENT_BALANCE_FOR_TRANSFER_OR_ID_NOT_FOUND) 
                );
            }        
        }
    ...
    function addQuantity(
        ...
        require(
            hasRole(DEFAULT_ADMIN_ROLE, msg.sender) || firstOwner[id] == msg.sender,
    ...
    function batchMintTraxx(
        ...
        require(collectionIds.length == ids.length, throwError(INVALID_FUNCTION_PARAMS));
    ...
    function uri(uint256 tokenId) public view virtual override (ERC1155Upgradeable, ERC1155URIStorageUpgradeable) returns (string memory) {
        ...
        // If token URI is set, concatenate base URI and tokenURI (via abi.encodePacked).
        return bytes(tokenURI).length > 0 ? string(abi.encodePacked(_baseURI, tokenURI)) : "";
    }
    ...
    {
        require(
            tokensOfOwner(_owner).length>0, 
            throwError(NO_IDS_FOUND)
        );
        ...
        for (uint256 i =0; i<ids.length; i++){
            TokenBalance memory token = TokenBalance(ids[i], balanceOf(_owner, ids[i]));
