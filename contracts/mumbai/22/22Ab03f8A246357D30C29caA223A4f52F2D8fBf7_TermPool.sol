...
interface ITermPool {
  ...
  error TermCanceledOrRepayed(uint256 termId);
  error NotListed();
  error ZeroAmount();
...
contract TermPool is ITermPool, Initializable, TermUtils {
  ...
  function lock(
    ...
    if (term.status == TermStatus.Cancelled) {
      revert TermCanceledOrRepayed(_termId);
    }
  ...
  function unlock(uint256 _termId) external override existingTerm(_termId) {
    ...
    uint256 balance = IERC20MetadataUpgradeable(term.tpToken).balanceOf(msg.sender);

    if (balance == 0) revert ZeroAmount();
    // TODO: modify tpToken to unlock with reward or not. Move this logic to tpToken
    ITpToken(term.tpToken).burn(msg.sender, balance);

    if (term.status == TermStatus.Cancelled) {
      emit LiquidityRedeemed(msg.sender, _termId, balance);
    } else {
      uint256 reward = rewardOf(_termId, msg.sender);
      if (reward > term.availableReward) revert NotEnoughReward();
      unchecked {
    ...
    IERC20Upgradeable(cpToken).safeTransfer(msg.sender, balance);
  }

  /// @notice Topups the reward in the contract
  /// @dev Only borrower can call this function
  /// @param _amount Amount of tokens to repay
  function topupReward(uint256 _termId, uint256 _amount) external override onlyBorrower {
    Term storage term = terms[_termId];
  ...
  function withdrawReward(uint256 _termId) external override onlyFactoryOwner {
    Term storage term = terms[_termId];
