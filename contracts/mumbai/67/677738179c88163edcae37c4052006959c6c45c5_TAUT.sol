...
contract TAUT is Initializable, ERC20Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
  ...
  /**
   * @dev Check if the amount is multiple of a 1e21.
   */
  modifier onlyMultipleOfThousand(uint256 amount) {
   ...
   */
  function burn(bytes16 id, uint256 amount) external onlyMultipleOfThousand(amount) {
   ...
   * @param amount The amount of tokens to burn.
   */
  function burnFrom(address account, uint256 amount) external onlyOwner onlyMultipleOfThousand(amount) {
    _burn(account, amount);
  }

  /**
   * @notice Calculates compound decay for which the rate is set for `FEE_PERIOD_DAYS`.
   * @param amount The initial amount to decay.
  ...
  function calculateCompoundDecay(
    ...
    return _calculateCompoundDecay(amount, decayRateBps, elapsedTimeDays);
  }
  ...
  function _calculateCompoundDecay(
    ...
    for (uint256 i; i < elapsedTimeDays; i = _unsafeIncrement(i)) {
      unchecked {
        amount = prevAmount - (prevAmount * decayRateBps) / DECAY_RATIO_DENOMINATOR;
      }
      prevAmount = amount;
    }
