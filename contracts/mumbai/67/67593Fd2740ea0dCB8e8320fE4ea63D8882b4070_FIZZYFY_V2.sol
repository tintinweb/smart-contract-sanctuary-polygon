...
contract FIZZYFY_V2 
{
    ...
    uint256[5] private tokenBonusPercents = [500, 1500, 2000, 2500, 3500];
	...
	function depositBySplit(uint256 _amount) external {
        ...
        uint256 baseBal = _amount.div(2);
        require(rewardInfo[msg.sender].split >= baseBal, "insufficient split");
        require(userInfo[msg.sender].totalFund >= baseBal, "insufficient fund balance");
        rewardInfo[msg.sender].split = rewardInfo[msg.sender].split.sub(baseBal);
        userInfo[msg.sender].totalFund = userInfo[msg.sender].totalFund.sub(baseBal);
        _deposit(msg.sender, _amount);
	...
	function _updateTeamNum(address _user, uint256 _amount) private {
		...
		for(uint256 i = 0; i < 1; i++)
		{
            if(upline != address(0))
			{
                ...
                {
                    if(userInfo[upline].directnum < 5 )
                    {
                        uint256 amount = 0;
                        ...
                        upRewards.tokenBonus = amount.mul(tokenBonusPercents[userInfo[upline].directnum]).div(baseDivider);
                    }
                }
    ...
    function withdraw() external {
        ...
        require(user.directnum > 0 || (user.directnum == 0 && user.totalRevenue <= maxRevenue), "max revenue reached with no direct");
