...
contract EarlyAccessToken is ERC721URIStorage, AccessControl, ReentrancyGuard, IEvents{
    ...
    function endTheAuction() external onlyRole(ADMIN_ROLE) {
        ...
        for (uint i = 1; i <= top24.length ; i++) {
            if(RankRegistered[top24[top24.length - i]] != true){
                ...
                IERC20(IAdmin(admin).BUSD()).transfer(IAdmin(admin).Treasury(), Bidding[top24[top24.length - i]]/2);
                IERC20(IAdmin(admin).BUSD()).transfer(IAdmin(admin).TeamAddress(),Bidding[top24[top24.length - i]]/2);
