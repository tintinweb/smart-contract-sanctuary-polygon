...
contract StakingV4 is Ownable, ERC721Holder, ReentrancyGuard {
    ...
    function payReward(UserInfo storage _user) internal {
        ...
        if (pending > 0) {
            ...
            // if (_user.bonus > 0) {
            //     normalizeReward = normalizeReward.add(
            //         _user.bonus.mul(rewardPerAmount)
            //     );
            // }
            //  console.log("normalizeReward2",normalizeReward);
            if (_user.amount > newPopularity) {
                console.log("Horse expire");
