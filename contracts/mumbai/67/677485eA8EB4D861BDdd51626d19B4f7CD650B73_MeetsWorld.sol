...
contract MeetsWorld is Ownable, ERC721 {
    ...
    // tokenId => rarityLevel
    mapping (uint256 => uint8) assignedCategory; // 0 = Basic, 1 = Normal, 2 = Rare, 3 = Epic ,4=Exclusive
    ...
    function assignCategory(uint256 _rad) internal returns (uint8) {
    ...
    function otherOption() internal returns (uint8) {
    ...
    function explicitAssignCategory() public onlyOwner{

       for(uint32 i=0;i<_tokenIds.current();i++){

        uint256 rad = random();
        uint8 assignCat = assignCategory(rad);

        require(assignCat < 5, "Minting Finish");

        assignedCategory[i]=assignCat;
