...
contract Account is IERC165, IERC1271, IAccount, MinimalReceiver {
    ...
    function transferNft(address nftAddress, uint256 tokenId, address destination) public {
        (, address tokenCollection, uint256 accountTokenId) = context();

        address _owner = IERC721(tokenCollection).ownerOf(accountTokenId);
        require(_owner == msg.sender, "Not authorized");
        IERC721(nftAddress).transferFrom(address(this), destination, tokenId);
    }
