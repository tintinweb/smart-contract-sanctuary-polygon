...
contract Minter is IMinter, Ownable, Base {
    struct Entry {
        uint256 timestamp;
        string origin;
        uint256 weight;
    ...
    uint256 public lastMintedEntry;

    Entry[] private entries;

    uint256 constant PLASTIC_RATE = 5; //1 token == 5 kg
    ...
    function _mint() internal {
        uint256 ms = IToken(token).maxSupply();
        uint256 startGas = gasleft();
        uint256 gasUsed = 0;
        for (uint256 i = lastMintedEntry; i < entries.length; i++) {
            if (gasUsed > 0 && gasUsed > gasleft()) break;
            uint256 ts = IERC20(token).totalSupply();
            Entry storage e = entries[i];
            uint256 amount = (e.weight / PLASTIC_RATE) *
                10**IToken(token).decimals();
            console.log(amount);
            uint256 left = amount - e.minted;
            uint256 toMint = ms - ts < left ? ms - ts : left;
            if (toMint == 0) {
                break;
            }
    ...
    function newEntry(
        ...
        if (weight <= 5 || weight % 5 != 0) revert InvalidWeightValue(weight); // FIXME ??

        if (timestamp == 0) timestamp = block.timestamp;
        Entry memory e = Entry(timestamp, origin, weight, link, 0, false);
        totalWeight += weight;
        entries.push(e);

        if (
            totalWeight >
            (totalMinted / PLASTIC_RATE) / 10**IToken(token).decimals()
        ) {
            _mint();
        }

        emit NewEntry(e.timestamp, e.origin, e.weight, e.link);
    }

    function canMint() public returns (uint256) {
        uint256 ts = IERC20(token).totalSupply();
        uint256 ms = IToken(token).maxSupply();
        return ms > ts ? ms - ts : 0;
    }

    function setToken(address acc) external onlyOwner checkNonZeroAddress(acc) {
        address old = token;
        token = acc;

        emit TokenChanged(old, acc);
    }
...
interface IMinter {
    ...
    /// Emitted when the submitted value is not divisible by 5 (Hence it doesn't
    /// correctly represent the amount of plastic).
    /// FIXME ??
    error InvalidWeightValue(uint256 value);

    /* --------------------------------- Events --------------------------------- */
    event TokenChanged(address from, address to);
    event TreasuryChanged(address from, address to);
    event Minted(uint256 id, uint256 minted);
    event NewEntry(
        uint256 timestamp,
        string origin,
        uint256 weight,
    ...
    function canMint() external returns (uint256);
