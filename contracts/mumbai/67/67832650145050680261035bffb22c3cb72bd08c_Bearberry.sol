...
contract Bearberry is ERC20 {
    ...
    function claimTokens(address _nftContract, uint256[] memory tokenIds) external {
        ...
        for (uint256 i = 0; i < tokenIds.length; i++) {
            ...
            uint256 tokensToClaim = calculateTokensToClaim(_nftContract, tokenId);
            totalTokensToClaim += tokensToClaim;
            _lastClaim[_nftContract][tokenId] = block.timestamp;
        }

        require(totalTokensToClaim > 0, "No tokens to claim");
        _mint(msg.sender, totalTokensToClaim);
    }

    function calculateTokensToClaim(address _nftContract, uint256 tokenId) public view returns (uint256) {
        uint256 lastClaimed;
