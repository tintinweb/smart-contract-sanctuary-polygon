...
import "../lib/lib/utils/Counters.sol";
...
contract UnilendV2Pool is UnilendV2library, UnilendV2transfer {
    ...
    function userHealthFactorLtv(
        uint _nftID
    ) public view returns (uint256 _healthFactor0, uint256 _healthFactor1) {
        (uint _lendBalance0, uint _borrowBalance0) = userBalanceOftoken0(
            _nftID
        );
        (uint _lendBalance1, uint _borrowBalance1) = userBalanceOftoken1(
            _nftID
        );
        ...
        if (_borrowBalance0 == 0) {
            ...
            uint collateralBalance = IUnilendV2Core(core).getOraclePrice(
                token1,
                token0,
                _lendBalance1
            );
    ...
    function userHealthFactor(
        ...
        if (_borrowBalance0 == 0) {
            ...
            _healthFactor0 = (
                collateralBalance.mul(uint(100).sub(lb)).mul(1e18).div(100)
            ).div(_borrowBalance0);
        }
    ...
    function userBalanceOftoken1(
        uint _nftID
    ) public view returns (uint _lendBalance1, uint _borrowBalance1) {
        pM memory _positionMt = positionData[_nftID];
        tM memory _tm1 = token1Data;

        uint _totalBorrow = _tm1.totalBorrow;
        if (block.number > lastUpdated) {
            uint interestRate1 = getInterestRate(
                _tm1.totalBorrow,
    ...
    function userSharesOftoken0(
        uint _nftID
    ) public view returns (uint _lendShare0, uint _borrowShare0) {
        pM memory _positionMt = positionData[_nftID];
    ...
    function userSharesOftoken1(
        uint _nftID
    ) public view returns (uint _lendShare1, uint _borrowShare1) {
        pM memory _positionMt = positionData[_nftID];
    ...
    function lend(
        uint _nftID,
        int amount
    ) external onlyCore returns (int mintedTokens) {
        uint ntokens0;
        uint ntokens1;
        ...
        if (amount < 0) {
            ...
            ntokens0 = calculateShare(
                _tm0.totalLendShare,
                _totTokenBalance0.sub(uint(-amount)),
                uint(-amount)
            );
        ...
        if (amount > 0) {
            ...
            ntokens1 = calculateShare(
                _tm1.totalLendShare,
                _totTokenBalance1.sub(uint(amount)),
                uint(amount)
            );
    ...
    function redeem(
        ...
        int tok_amount,
        address _receiver
    ) external onlyCore returns (int _amount) {
        accrueInterest();
        ...
        if (tok_amount < 0) {
            require(
                _positionMt.token0lendShare >= uint(-tok_amount),
        ...
        if (tok_amount > 0) {
            require(
                _positionMt.token1lendShare >= uint(tok_amount),
                "Balance Exceeds Requested"
            );
    ...
    function redeemUnderlying(
        ...
        int _amount,
        address _receiver
    ) external onlyCore returns (int rtAmount) {
        accrueInterest();
        ...
        if (_amount < 0) {
            ...
            uint tokenBalance0 = IERC20(token0).balanceOf(address(this));
            uint _totTokenBalance0 = tokenBalance0.add(_tm0.totalBorrow);
            uint tok_amount0 = getShareByValue(
                _totTokenBalance0,
                _tm0.totalLendShare,
                uint(-_amount)
            );
        ...
        if (_amount > 0) {
            ...
            uint tokenBalance1 = IERC20(token1).balanceOf(address(this));
            uint _totTokenBalance1 = tokenBalance1.add(_tm1.totalBorrow);
            uint tok_amount1 = getShareByValue(
                _totTokenBalance1,
                _tm1.totalLendShare,
                uint(_amount)
            );
    ...
    function repay(
        ...
        int amount,
        address _payer
    ) external onlyCore returns (int _rAmount) {
        accrueInterest();
        ...
        if (amount < 0) {
            ...
            uint _totalBorrow = _tm0.totalBorrow;
            uint _totalLiability = getShareValue(
                _totalBorrow,
                _tm0.totalBorrowShare,
                _positionMt.token0borrowShare
            );
            ...
            if (uint(-amount) > _totalLiability) {
                ...
                uint amountToShare = getShareByValue(
                    _totalBorrow,
                    _tm0.totalBorrowShare,
                    uint(-amount)
                );
    ...
    function liquidateInternal(
        ...
        int amount,
        uint _toNftID
    ) internal returns (int liquidatedAmount, int totReceiveAmount) {
        accrueInterest();
        ...
        if (amount < 0) {
            ...
            if (_healthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
                ...
                uint amountToShare0 = getShareByValue(
                    _tm0.totalBorrow,
                    _tm0.totalBorrowShare,
                    procAmountIN
                );
                ...
                uint _totTokenBalance1 = IERC20(token1)
                    .balanceOf(address(this))
                    .add(_tm1.totalBorrow);
                uint amountToShare1 = getShareByValue(
                    _totTokenBalance1,
                    _tm1.totalLendShare,
                    recAmountIN
                );
        ...
        if (amount > 0) {
            ...
            if (_healthFactor < HEALTH_FACTOR_LIQUIDATION_THRESHOLD) {
                ...
                uint _totTokenBalance0 = IERC20(token0)
                    .balanceOf(address(this))
    ...
    function liquidate(
        ...
        address _receiver,
        uint _toNftID
    ) external onlyCore returns (int liquidatedAmount) {
        accrueInterest();
    ...
    function liquidateMulti(
        ...
        int[] calldata amounts,
        address _receiver,
        uint _toNftID
    ) external onlyCore returns (int liquidatedAmountTotal) {
        accrueInterest();
