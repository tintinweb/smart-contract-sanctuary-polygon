...
contract WrasslersAuctionClaim is ERC721, Ownable, GBMInitiator {
    ...
    function addAuctionRound(uint256 start, uint256 exclusiveEnd) external onlyOwner {
        setStartTime(block.timestamp);
        setEndTime(block.timestamp + 1 days);
        _GBM(auctionContract).massRegistrerERC721Each(address(this), address(this), start, exclusiveEnd);
    }
