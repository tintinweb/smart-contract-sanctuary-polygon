...
contract USDRTreasury is ITreasury, AccessControl, IERC721Receiver {
    ...
    function deposit(address stableToken, uint256 amount) external override {
        require(
            approvedStableReserves[stableToken] ||
                stableToken == addressProvider.getAddress(TNGBL_ADDRESS),
            "not approved"
        );
        //update books
    ...
    {
        (address underlying, address liquidityManager) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(UNDERLYING_ADDRESS, LIQUIDITY_MANAGER_ADDRESS)
            ),
            (address, address)
        );
        IERC20(underlying).approve(liquidityManager, underlyingAmount);
        (liquidity, amount0, amount1) = ILiquidityManager(liquidityManager)
            .increaseLiquidity(underlyingAmount);
    }

    function checkPaymentTokenAndConvert(
        IERC20 paymentToken,
        uint256 amount,
        ...
        if (paymentToken.balanceOf(address(this)) < amount) {
            uint256 exactIn = ITokenSwap(tokenSwap).quoteIn(
                underlying,
                address(paymentToken),
                amount
            );
            IERC20(underlying).approve(tokenSwap, exactIn);
            ITokenSwap(tokenSwap).exchange(
                underlying,
                address(paymentToken),
                exactIn,
        ...
        if (!instantLiquidity_) {
            paymentToken.approve(marketplace, amount);
        } else {
            paymentToken.approve(instantLiquidity, amount);
        }
    }
    ...
    function purchaseTnft(
        ...
        address rwaCalculator = abi.decode(
            addressProvider.getAddresses(abi.encode(RWA_CALCULATOR_ADDRESS)),
            (address)
        );
        ...
        if (tokenId == 0) {
            ...
            marketplace.buy(ITangibleNFT(tnft), tokenId, _years);
        }
        // update records
        updateTrackerTnft(tnft, latestReceivedToken, true);
    }
    ...
    function purchaseFtnft(
        ...
        ITangibleMarketplace(
            addressProvider.getAddress(TANGIBLE_MARKETPLACE_ADDRESS)
        ).buyFraction(ITangibleFractionsNFT(ftnft), fractTokenId, share);
        //update records
        updateTrackerFtnft(ftnft, latestReceivedToken, true);
    }
    ...
    function sellTnft(
        ...
        for (uint256 i = 0; i < length; i++) {
            IERC721(tnft).approve(marketplace, tokenIds[i]);
            updateTrackerTnft(tnft, tokenIds[i], false);
        }
    ...
    function sellFtnft(
        ...
        updateTrackerTnft(ftnft, tokenId, false);
    }
    ...
    function sellFtnftInitial(
        ...
        IERC721(tnft).approve(marketplace, tokenId);
        ...
        if (keepShare != 0) {
            updateTrackerFtnft(latestReceivedNFT, latestReceivedToken, true);
        }
    }
        ...
        onlyRole(TREASURY_POLICY_ROLE)
    {
        ITangibleMarketplace(
            addressProvider.getAddress(TANGIBLE_MARKETPLACE_ADDRESS)
        ).stopFractSale(ITangibleFractionsNFT(ftnft), tokenId);
        //update records
        updateTrackerFtnft(ftnft, tokenId, true);
    }

    function stopSellTnft(address tnft, uint256[] calldata tokenIds)
        external
        onlyRole(TREASURY_POLICY_ROLE)
    {
        uint256 length = tokenIds.length;
        ITangibleMarketplace(
            addressProvider.getAddress(TANGIBLE_MARKETPLACE_ADDRESS)
        ).stopBatchSale(ITangibleNFT(tnft), tokenIds);
        //update records
        for (uint256 i = 0; i < length; i++) {
            updateTrackerTnft(tnft, tokenIds[i], true);
        }
    }
    ...
    function defractionalize(
        ...
        ITreasuryTracker(addressProvider.getAddress(TREASURY_TRACKER_ADDRESS))
    ...
    function purchaseTnftIL(
        ...
        IInstantLiquidity tangibleIL = IInstantLiquidity(
            addressProvider.getAddress(INSTANT_LIQUIDITY_ADDRESS)
        );
        if (!ftnft) {
    ...
    function withdrawTnft(address _nft, uint256 _tokenId)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        require(emergencyStop, "Not emergency");
    ...
    function withdrawFTnft(address _ftnft, uint256 _tokenId)
        external
        onlyRole(TREASURY_WITHDRAW_ROLE)
    {
        require(emergencyStop, "Not emergency");
        require(
            IERC721(_ftnft).ownerOf(_tokenId) == address(this),
            "Treasury not fowner"
        );
        IERC721(_ftnft).safeTransferFrom(address(this), msg.sender, _tokenId);
        updateTrackerFtnft(_ftnft, _tokenId, false);
    }

    function burnTNGBL() external onlyRole(CONTROLLER_ROLE) {
        (address staking, address tngbl) = abi.decode(
            addressProvider.getAddresses(
                abi.encode(STAKING_ADDRESS, TNGBL_ADDRESS)
            ),
            (address, address)
        );
        (uint256 stable, uint256 rwa, ) = getTreasuryValue();
        uint256 tngblBalance = IERC20(tngbl).balanceOf(address(this));
        uint256 usdrMarketCap = IStaking(staking).usdrMarketCap();
        uint256 threshold = usdrMarketCap * uint256(tngblBurnThreshold) * 1e7;
        require(stable + rwa > threshold, "insufficient backing");
