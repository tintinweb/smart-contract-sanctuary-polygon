...
contract SBTFacet is IERC1155MetadataURI, Modifiers {
    ...
    function mintBatch(address[] calldata _tos, uint256 cardId_) external onlyTrustedForwarder {
    ...
    function burn(address _from, uint256 cardId_) external onlyTrustedForwarder {
    ...
    function uri(uint256 _id) external view returns (string memory) {
        require(bytes(s.cardMetadata[_id].name).length != 0, "SBTFacet: URI query for nonexistent card");

        MetadataStruct memory cardMetadata = s.cardMetadata[_id];
        uint256 projectId = cardMetadata.projectId;
            ...
            bytes(
                string(
                    abi.encodePacked(
                        ...
                        bytes(cardMetadata.imageCID).length > 0 ? cardMetadata.imageCID : s.cardImageCID,
                        '",',
                        ...
                        '"value": "',
                        Strings.toHexString(cardMetadata.createdBy),
                        '"',
                        "}",
                        "]",
                        "}"
