...
interface IVoting {
    function addVoteReward(uint256 nftId, uint256 amount) external;
}
...
contract Market is Context, ERC721Holder, Ownable {
    ...
    uint128 private bidPeriodSec = 60 * 60 * 24 * 7;
    ...
    uint16 private fundListingUpPrice = 2000; // is 20 percent

    uint16 private profitToRewardPool = 7500; // margin profit to reward pool 75%

    uint16 private voteReward = 5000; // vote reward 50% from profit after distribute reward pool
    ...
    EnumerableSet.UintSet private listOpenAuction;

    uint256 private totalFundSale;

    uint256 private totalFundProfit;
    ...
    function getFundListingUpPrice() public view returns (uint16) {
        return fundListingUpPrice;
    }

    function setFundListingUpPrice(uint16 newUpPrice) public onlyOwner {
        fundListingUpPrice = newUpPrice;
    }

    function getTotalFundSale() public view returns (uint256) {
        return totalFundSale;
    }

    function getTotalFundProfit() public view returns (uint256) {
    ...
    function createPrivateBidList(
        ...
        listOpenAuction.add(_auctionID);
    ...
    function setVoteAddr(address _voteAddr) public onlyOwner {
        voting = IVoting(_voteAddr);
    }

    function getTreasureAddr() public view returns (address) {
        return treasureWallet;
    }

    function setTreasureAddr(address _treasureWallet) public onlyOwner {
        treasureWallet = _treasureWallet;
    }

    function setBidPriodSec(uint128 _bidPeriodSec) public onlyOwner {
        bidPeriodSec = _bidPeriodSec;
    }
        ...
        onlyApplicableBuyer(_auctionID)
    {
        ...
        address previousBidder = act.bidder;
    ...
    function endBid(uint256 _auctionID) public {
        ReserveAuction storage act = auctionIdToReserveAuction[_auctionID];
        require(act.auctionEnd == false, "this auction ended");
        require(act.endTime <= block.timestamp, "this auction not ended");
        ...
        if (act.bidder != address(0)) {
            ...
            uint256 profit = act.amount - act.startPrice;
            _sendProfit(profit, act.tokenId);
            totalFundSale = totalFundSale + act.amount;
            totalFundProfit = totalFundProfit + profit;
        } else {
            isWon = false;
            _makeListing(act.tokenId, block.timestamp, 0, act.startPrice, true);
        }

        act.auctionEnd = true;
        listOpenAuction.remove(_auctionID);
        emit PrivateBidClosed(
            act.auctionId,
            act.tokenId,
            isWon,
            act.bidder,
    ...
    function _sendProfit(uint256 profit, uint256 tokenId) private {
        uint256 rewardPool = (profit * profitToRewardPool) / 10000;
        uint256 remainingReward = profit - rewardPool;
        uint256 toVoteReward = (remainingReward * voteReward) / 10000;
        uint256 toSpacialReward = remainingReward - toVoteReward;

        token.safeTransfer(address(staking), rewardPool);
        token.approve(address(voting), toVoteReward);
        voting.addVoteReward(tokenId, toVoteReward);
        token.safeTransfer(treasureWallet, toSpacialReward);
    }
    ...
    function _makeListing(
        ...
        if (isFund) {
            listing.isFund = true;
            uint256 salePrice = (reservePrice * fundListingUpPrice) / 10000; // 20 percent
    ...
    function makeListing(
        ...
        return _makeListing(_tokenId, startTime, endTime, reservePrice, false);
    }
        ...
        returns (Listing memory)
    {
        ...
        if (lis.seller != nft.ownerOf(_tokenId)) {
            return mList;
        }
        ...
        if (_includeExpired) {
            ...
            if (lis.endTime > block.timestamp || lis.isFund) {
                mList = lis;
            }
        }
        ...
        listingOnGoing(_listingID)
    {
        ...
        if (listing.isFund) {
            ...
            uint256 profit = listing.price - listing.stakingFund;
            _sendProfit(profit, listing.tokenId);
            totalFundSale = totalFundSale + listing.price;
            totalFundProfit = totalFundProfit + profit;
        } else {
        ...
        isOfferExpired(_offerID)
    {
        ...
        token.safeTransferFrom(offer.buyer, address(this), offer.price);
        token.safeTransfer(_msgSender(), offer.price - fee);
        // TODO transfer fee to treasure address
    ...
    function closeAllEndBid() public {
        uint256[] memory ids = listOpenAuction.values();
        for (uint256 i = 0; i < ids.length; i++) {
            ReserveAuction storage act = auctionIdToReserveAuction[ids[i]];
            if (act.auctionEnd == false && act.endTime <= block.timestamp) {
                endBid(ids[i]);
            }
        }
    }

    function hasEndBid() public view returns (bool) {
        uint256[] memory ids = listOpenAuction.values();
        for (uint256 i = 0; i < ids.length; i++) {
            if (auctionIdToReserveAuction[ids[i]].endTime <= block.timestamp) {
                return true;
            }
        }
        return false;
