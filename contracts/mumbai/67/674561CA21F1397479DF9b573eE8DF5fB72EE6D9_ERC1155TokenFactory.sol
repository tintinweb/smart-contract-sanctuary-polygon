...
abstract contract CustomERC2771Context is Context, Ownable {
...
import "./Nonces.sol";
import "./CustomERC2771Context.sol";

contract ERC1155Token is CustomERC2771Context, ERC1155Supply, ERC1155Burnable, Pausable, Nonces {
    ...
    constructor(string memory contractName, string memory uri, address trustedForwarder, address authSigner) ERC1155(uri) CustomERC2771Context(trustedForwarder) {
        name=contractName;
        _authoritySigner=authSigner;
    }

    function _msgSender() internal view virtual override(Context, CustomERC2771Context) returns (address) {
        return CustomERC2771Context._msgSender();
    }

    function _msgData() internal view virtual override(Context, CustomERC2771Context) returns (bytes memory) {
    ...
    function mint(address to, uint256 id, uint256 amount, bytes memory data, bytes memory signature) public whenNotPaused {
        ...
        _validateAuthorizedMintIfNotOwnerOrAuthority(sender,to,id,amount,data,_useNonce(sender),block.chainid,address(this),signature);
    ...
    function batchMint(address to, uint256[] memory  ids,  uint256[] memory amounts, bytes memory data, bytes memory signature) public whenNotPaused{  
        address sender = _msgSender();
        _validateAuthorizedBatchMintIfNotOwnerOrAuthority(sender,to,ids,amounts,data,_useNonce(sender),block.chainid,address(this),signature);
    ...
    function _validateAuthorizedMintIfNotOwnerOrAuthority(address minter, address to, uint256 id, uint256 amount, 
        bytes memory data, uint256 nonce, uint256 chainId, address verifyingContract, bytes memory signature) internal view {
        if(_isOwnerOrAuthority())
            return;
        bytes32 contentHash = keccak256(abi.encode(minter,to,id,amount,keccak256(data),nonce,chainId,verifyingContract));
        address signer = signatureWallet(contentHash, signature);
        require(signer == _authoritySigner, "Not authorized to mint");
    }

    function _validateAuthorizedBatchMintIfNotOwnerOrAuthority(address minter, address to, uint256[] memory ids, uint256[] memory amounts,
         bytes memory data, uint256 nonce, uint256 chainId, address verifyingContract, bytes memory signature) internal view {
        if(_isOwnerOrAuthority())
            return;
        bytes32 contentHash = keccak256(abi.encode(minter,to,ids,amounts,keccak256(data),nonce,chainId,verifyingContract));
        address signer = signatureWallet(contentHash, signature);
        require(signer == _authoritySigner, "Not authorized to mint");
    }

    function _isOwnerOrAuthority() private view returns (bool){
        return owner()==_msgSender() || _authoritySigner==_msgSender();
    }

    function _validateMint(address to, uint256 id,uint256 amount) internal view {
        address sender = _msgSender();
        require(sender!=owner() && to==sender || sender==owner() || sender==_authoritySigner, "Only owner or authoritySigner can mint to other addres");
