...
contract test002Proxy is OpsTaskCreator, Ownable {
    ...
    address public XD = 0xd9fc82020513b92737B1ceB8dC66de9D3941BE45;
    ...
    function setDefaultSwapInfo(uint256 _approveAmount) public onlyOwner {
        XD.delegatecall(abi.encodeWithSelector(test002K.setDefaultSwapInfo.selector, _approveAmount));
    }

    function setUSDTPoolAddress(USDTPool _USDTPoolAddress) public onlyOwner {
        XD.delegatecall(abi.encodeWithSelector(test002K.setUSDTPoolAddress.selector, _USDTPoolAddress));
    }
    ...
    function createTask(
        ...
        payable(XD).delegatecall(abi.encodeWithSelector(test002K.createTask.selector, _tcc));
    }

    function editTaskSwapAmountList(bytes32 _taskID, uint256[] memory _swapAmountList) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.editTaskSwapAmountList.selector, _taskID, _swapAmountList));
    }

    function editTaskStartEndTime(bytes32 _taskID, uint256[] memory _start_end_Time) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.editTaskStartEndTime.selector, _taskID, _start_end_Time));
    }

    function editTaskTimeList(bytes32 _taskID, uint256[] memory _timeList) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.editTaskTimeList.selector, _taskID, _timeList));
    }

    function editTaskTimeIntervalList(bytes32 _taskID, uint256[] memory _timeIntervalList) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.editTaskTimeIntervalList.selector, _taskID, _timeIntervalList));
    }

    function editTaskLimit(bytes32 _taskID, uint256 _maxtxAmount, uint256 _maxSpendTokenAmount, uint256 _maxFeePerTx) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.editTaskLimit.selector, _taskID, _maxtxAmount, _maxSpendTokenAmount, _maxFeePerTx));
    }

    function swapToken(address _user, bytes32 _md5) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.swapToken.selector, _user, _md5));
    }

    function deposit(uint256 _usdtAmount) external payable {
        payable(XD).delegatecall(abi.encodeWithSelector(test002K.deposit.selector, _usdtAmount));
    }

    function withdraw(uint256 _usdtAmount, uint256 _ethAmount) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.withdraw.selector, _usdtAmount, _ethAmount));
    }

    function claimToken(IERC20 _token, uint256 _amount) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.claimToken.selector, _token, _amount));
    }

    function claimEth(uint256 _amount) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.claimEth.selector, _amount));
    }

    function cancelTask(bytes32 _taskID) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.cancelTask.selector, _taskID));
    }

    function restartTask(bytes32 _taskID) external {
        XD.delegatecall(abi.encodeWithSelector(test002K.restartTask.selector, _taskID));
    }
