...
contract AaveProtocolDataProvider is IPoolDataProvider {
  ...
  function getAllReservesTokens() external view returns (TokenData[] memory) {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      reservesTokens[i] = TokenData({
        symbol: IPToken(reserves[i]).symbol(),
  ...
  function getAllPTokens() external view returns (TokenData[] memory) {
    ...
    for (uint256 i = 0; i < reserves.length; i++) {
      ...
      pTokens[i] = TokenData({
        symbol: IPToken(reserveData.pTokenAddress).symbol(),
   ...
   * @return totalPTokenBalance The total PToken balance of the reserve
   * @return totalBorrowBalances The total borrow balances of the reserve
   * @return totalPrxyBalance The total Prxy balance of the reserve
   * @return totalCollateralBalance The total collateral balance of the reserve
   * @return totalUncollateralBalance The total uncollateral balance of the reserve
   ...
   * @return utilizationRate The utilization rate of the reserve
  ...
  function getReserveData(address asset)
    ...
    view
    returns (
      uint256 totalPTokenBalance,
      uint256[] memory totalBorrowBalances,
      uint256 totalPrxyBalance,
      uint256 totalCollateralBalance,
      uint256 totalUncollateralBalance,
      uint256 supplyRate,
      uint256 borrowRate,
      uint256 prxyRate,
      uint256 splitRate,
  ...
  {
    address supplyingAsset = asset;
    IPool POOL = IPool(ADDRESSES_PROVIDER.getPool());
    DataTypes.ReserveData memory reserve = POOL.getReserveData(supplyingAsset);
    (totalBorrowBalances, totalCollateralBalance) = _calcCollateralBalance(reserve, supplyingAsset);

    uint256 totalSupply = IPToken(reserve.pTokenAddress).totalSupply(); 
    totalPTokenBalance = totalSupply;
    totalPrxyBalance = IPToken(reserve.pTokenAddress).totalPrxy();
    totalUncollateralBalance = totalSupply - totalCollateralBalance;
    supplyRate = reserve.supplyRate;
    borrowRate = reserve.borrowRate;
    prxyRate = reserve.prxyRate;
    splitRate = reserve.splitRate;
    utilizationRate = (totalCollateralBalance * reserve.configuration.getLtv()) / totalSupply;
    lastUpdateTimestamp = IPToken(reserve.pTokenAddress).lastUpdateTimestamp();
  }

  function _calcCollateralBalance(DataTypes.ReserveData memory reserve, address asset) internal view returns(
    uint256[] memory totalBorrowBalances,
    uint256 totalCollateralBalance
  ) {
    IPool POOL = IPool(ADDRESSES_PROVIDER.getPool());
    address[] memory reserveList = POOL.getReservesList();
    totalBorrowBalances = new uint256[](reserveList.length);
    totalCollateralBalance = 0;

    for(uint256 i = 0; i < reserveList.length; i++) {
      totalBorrowBalances[i] = IPToken(reserve.pTokenAddress).totalBorrow(reserveList[i]);
      totalCollateralBalance += totalBorrowBalances[i].rayMul(POOL.getCollateralRate(asset, reserveList[i]));  
    }
  }

  /**
   * @notice Returns the total supply of pTokens for a given asset
   * @param asset The address of the underlying asset of the reserve
   * @return The total supply of the pToken
   **/
  function getTotalSupply(address asset) external view override returns (uint256) {
    ...
    return IPToken(reserve.pTokenAddress).totalSupply();
  }
   ...
   * @return currentBorrowBalances The current borrow balances of the user
   * @return currentPrxyBalance The current Prxy balance of the user
   * @return currentCollateralBalance The current collateral balance of the user
   * @return currentUncollateralBalance The current uncollateral balance of the user
   * @return supplyRate The liquidity rate of the reserve
   * @return borrowRate The borrow rate of the reserve
   * @return prxyRate The prxy rate of the reserve
   * @return lastUpdateTimestamp The timestamp of the last update of the user
   * @return supplyMode The supply mode of the user
   **/
  function getUserReserveData(address asset, address user)
    ...
    returns (
      ...
      uint256[] memory currentBorrowBalances,
      uint256 currentPrxyBalance,
      uint256 currentCollateralBalance,
      uint256 currentUncollateralBalance,
      uint256 supplyRate,
      uint256 borrowRate,
      uint256 prxyRate,
      uint40 lastUpdateTimestamp,
    ...
    )
  {
    ...
    currentBorrowBalances = new uint256[](reserveList.length);

    currentPTokenBalance = IPToken(reserve.pTokenAddress).balanceOf(user);
    for(uint256 i = 0; i < reserveList.length; i++) {
      currentBorrowBalances[i] = IPToken(reserve.pTokenAddress).borrowBalanceOf(user, reserveList[i]);
    }

    currentPrxyBalance = IPToken(reserve.pTokenAddress).prxyBalanceOf(user);
    currentCollateralBalance = IPToken(reserve.pTokenAddress).collateralBalanceOf(user);
    currentUncollateralBalance = currentPTokenBalance - currentCollateralBalance;
    supplyRate = reserve.supplyRate;
    borrowRate = reserve.borrowRate;
    prxyRate = reserve.prxyRate;
    lastUpdateTimestamp = IPToken(reserve.pTokenAddress).lastUpdateTimestampOf(user);
    supplyMode = IPToken(reserve.pTokenAddress).supplyMode(user);
  }

  /**
   * @notice Returns the token addresses of the reserve
   * @param asset The address of the underlying asset of the reserve
   * @return pTokenAddress The PToken address of the reserve
   */
  function getReserveToken(address asset)
    external
    view
    returns (
