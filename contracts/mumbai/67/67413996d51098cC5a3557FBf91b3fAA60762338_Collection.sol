...
contract Collection is Ownable, ERC721Consecutive {

    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;

    /// @dev Contract URI used as metadata info in NFT collection.
    ...
    event StatusUpdated(uint256 indexed tokenId, string indexed status);
    ...
    constructor(
        ...
        string[] memory _nftDesc,
        string[] memory _nftImage,
        string[] memory _nftLocations
        // address _splitterAddr,
        // uint96 _feeTaken
    ) ERC721(_name, _symbol) {
        require(_nftNames.length == _nftLocations.length);
        contractURI = _contractURI;
        uint96 _batchSize = uint96(_nftNames.length);
        _mintConsecutive(msg.sender, _batchSize);
        ...
        // _setDefaultRoyalty(_splitterAddr, _feeTaken);
    }
        ...
        returns (string memory)
    {
        string memory json = Base64.encode(
            abi.encodePacked(
                ...
                nfts[_tokenId].location,
                '"}, {"trait_type": "Status", "value": "',
                nfts[_tokenId].status,
                '"} ]}'
            )
        );
    ...
    function updateStatus(uint256 _tokenId, string memory _status) external onlyOwner {
        nfts[_tokenId].status = _status;
        emit StatusUpdated(_tokenId, _status);
    }
