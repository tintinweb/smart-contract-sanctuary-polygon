...
contract BitConeAI is ERC721URIStorage, Ownable {
    ...
    mapping(uint256 => uint256) private isNFTExist;
    ...
    function generateImage(uint256 _id) public {       
        ...
        require(isNFTExist[_id] < freeLimit ,"Free Limit Exceeded");
        require(IERC721(NFT).ownerOf(_id) == msg.sender, "You are not the owner of this NFT");
        if(isNFTExist[_id] == 0){
            nftIds.push(_id);
        }       
        isNFTExist[_id] = isNFTExist[_id] + 1;
    ...
    // uint256 num;
    // function enterNftIds(uint256 _num) public {
    //     for(uint256 _id=1; _id < num; _id++){
    //     nftIds.push(_id);
    //     }
    //     num += _num;
    // }

    function checkNFTExist(uint256 id) public view returns(uint256){
