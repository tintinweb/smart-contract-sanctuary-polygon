...
contract Whitelist is Ownable {
     ...
     */
    function addCollectionAddress(address _address) public onlyOwner() {
        require(
    ...
    function removeCollectionAddress(address _address) public onlyOwner() {
        require(
 ...
 */
contract MarketPlace is ERC2771Context,IAuctionHouse, ReentrancyGuard,Whitelist,ERC721Holder{
    ...
    uint public plateformFeePercentage;
    ...
    // set all plateform fee to this address
     ...
     * Constructor
     */   
    constructor(address _LNQAddress,address trustedForwarder) ERC2771Context(trustedForwarder) {
    ...
    function createSaleOrder(
        ...
        address tokenContract,
        uint256 reservePrice
    ) public nonReentrant isWhitelisted(tokenContract) returns (uint256) {
        require(
            IERC165(tokenContract).supportsInterface(interfaceId),
            "tokenContract does not support ERC721 interface"
        ); 
        address tokenOwner = IERC721(tokenContract).ownerOf(tokenId);
        require(_msgSender() == IERC721(tokenContract).getApproved(tokenId) || _msgSender() == tokenOwner, "Caller must be approved or owner for token id");
        ...
        emit OrderPlaced(orderId,tokenId,tokenContract,reservePrice,tokenOwner);
        return orderId;
    }
    ...
    function cancelOrder(uint256 orderId) external nonReentrant orderExists(orderId) {
        ...
        emit OrderCanceled(orderId,saleOrder[orderId].tokenId,saleOrder[orderId].tokenContract,saleOrder[orderId].tokenOwner);
    }

    /**
     * @notice change reserve price.
     * @dev change saling price 
     */
    function setOrderReservePrice(uint256 orderId, uint256 reservePrice) external orderExists(orderId) {
        require(reservePrice != saleOrder[orderId].reservePrice,"price must be difference");
        require(_msgSender() == saleOrder[orderId].tokenOwner, "Must be sale order owner can change");
        saleOrder[orderId].reservePrice = reservePrice;
        emit OrderReservePriceUpdated(orderId, saleOrder[orderId].tokenId, saleOrder[orderId].tokenContract, reservePrice);
    }
     ...
     */
    function buyOrder(uint256 orderId,uint amount) external orderExists(orderId) nonReentrant {

        uint256 plateformFee = 0;
        uint256 royaltyFee = 0;
        uint256 tokenOwnerProfit = saleOrder[orderId].reservePrice;
        require(amount == tokenOwnerProfit,"amount must be equal to sale order price");

        _handleIncomingBid(tokenOwnerProfit,LNQTokenAddress);

        IERC721(saleOrder[orderId].tokenContract).safeTransferFrom(address(this), _msgSender(), saleOrder[orderId].tokenId);

        if(treasury != address(0)) {
            plateformFee = (tokenOwnerProfit*plateformFeePercentage)/(10000);
        ...
        _handleOutgoingBid(saleOrder[orderId].tokenOwner, tokenOwnerProfit, LNQTokenAddress);
        emit OrderSaleEnded(orderId,saleOrder[orderId].tokenId,saleOrder[orderId].tokenContract,saleOrder[orderId].tokenOwner,_msgSender(),tokenOwnerProfit);
    ...
    function createAuction(
        ...
        emit AuctionCreated(auctionId, tokenId, tokenContract, duration, reservePrice, tokenOwner);
        return auctionId;
    }

    function setAuctionReservePrice(uint256 auctionId, uint256 reservePrice) external override auctionExists(auctionId) {
        require(reservePrice != auctions[auctionId].reservePrice,"price must be difference");
        require(_msgSender() == auctions[auctionId].tokenOwner, "Must be auction token owner");
    ...
    nonReentrant
    {
        ...
        if(auctions[auctionId].firstBidTime == 0) {
            ...
            _handleOutgoingBid(lastBidder, auctions[auctionId].amount, LNQTokenAddress);
        }
        ...
        if (
            auctions[auctionId].firstBidTime+(auctions[auctionId].duration)-(
                block.timestamp
            ) < timeBuffer//if the timegap is less than 15 mins 
        ) {
    ...
    function endAuction(uint256 auctionId) external override auctionExists(auctionId) nonReentrant {
        ...
        try IERC721(auctions[auctionId].tokenContract).safeTransferFrom(address(this), auctions[auctionId].bidder, auctions[auctionId].tokenId) {} catch {
            _handleOutgoingBid(auctions[auctionId].bidder, auctions[auctionId].amount, LNQTokenAddress);
        ...
        _handleOutgoingBid(auctions[auctionId].tokenOwner, tokenOwnerProfit,LNQTokenAddress);
    ...
    /**
      This is for transfering the native token
     */
    ...
    /**
        Cancel the Auction by the given auctionId
     */
    ...
    /**
        Cancel the Order by the the orderId
     */
   ...
   /**
        Check if the Auction existed or not
    */
