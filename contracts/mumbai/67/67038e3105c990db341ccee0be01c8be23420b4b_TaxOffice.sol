...
interface ITaxable {
    ...
    function setSharkOracle(address _SharkOracle) external;
...
contract TaxOffice is Operator {
    ...
    constructor(address _Shark) public {
        require(_Shark != address(0), "Shark address cannot be 0");
        Shark = _Shark;
    }
    ...
    function enableAutoCalculateTax() public onlyOperator {
        ITaxable(Shark).enableAutoCalculateTax();
    }

    function disableAutoCalculateTax() public onlyOperator {
        ITaxable(Shark).disableAutoCalculateTax();
    }

    function setTaxRate(uint256 _taxRate) public onlyOperator {
        ITaxable(Shark).setTaxRate(_taxRate);
    }

    function setBurnThreshold(uint256 _burnThreshold) public onlyOperator {
        ITaxable(Shark).setBurnThreshold(_burnThreshold);
    }

    function setTaxCollectorAddress(address _taxCollectorAddress) public onlyOperator {
        ITaxable(Shark).setTaxCollectorAddress(_taxCollectorAddress);
    }
    ...
    function setTaxableSharkOracle(address _SharkOracle) external onlyOperator {
        ITaxable(Shark).setSharkOracle(_SharkOracle);
    }

    function transferTaxOffice(address _newTaxOffice) external onlyOperator {
        ITaxable(Shark).setTaxOffice(_newTaxOffice);
    }
}