...
contract MyToken is ERC721, ERC721Enumerable, ERC721URIStorage, Pausable, Ownable, ERC721Burnable, EIP712, ERC721Votes {
    ...
    constructor() ERC721("testtoken4", "tt4") EIP712("testtoken4", "1") {}
    ...
    function bulkMint(address to, string memory uri, uint256 amount) public onlyOwner {
        if(i < amount){
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            _safeMint(to, tokenId);
            string memory token = Strings.toString(tokenId);
            _setTokenURI(tokenId, string.concat(uri,token,".json"));
            i++;
            bulkMint(to,uri,amount);
        } else {
            i = 0;
        }
    }
