...
contract NFTAuction is ERC721 {
    ...
    event AuctionCreated(uint256 indexed tokenId, uint256 auctionEndTime);
    ...
    function createAuction(uint256 tokenId, uint256 duration) external {
        require(ownerOf(tokenId) == msg.sender, "You must own the NFT to create an auction");
        require(auctions[tokenId].seller == address(0), "NFT is already in an auction");

        _tokenIds.increment();
        uint256 auctionId = _tokenIds.current();
        ...
        emit AuctionCreated(tokenId, auctionEndTime);
    }
    ...
    function endAuction(uint256 auctionId) external {
        ...
        safeTransferFrom(address(this), auction.highestBidder, auctionId);
    }

    function makeTransfer(address collection, address to, uint256 tokenId) public {
