...
contract MedicalRecord is ERC721URIStorage {
    ...
    function mint(string memory base64SVGString) public {
        require(bytes(base64SVGString).length != 0, "Please provide the svg base64 qr");
        uint256 newItemId = _tokenIds.current();
        _safeMint(msg.sender, newItemId);
        tokenIdtoPersonAddress[newItemId] = msg.sender;
        _setTokenURI(newItemId, getTokenURI(newItemId, base64SVGString));

        _tokenIds.increment();
    }

    function updateQr(uint256 tokenId, string memory base64SVGString) public returns(bool) {
