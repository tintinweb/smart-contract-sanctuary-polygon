...
contract BrickStaking{
    ...
    uint256 constant REWARD_PERIOD = 1 minutes; //should be 1/2 year
    uint256 constant UPGRADE_REWARD_PERIOD = 1095 days;
    uint256 constant PIXEL_VALUE = 10; //should be 10000
    ...
    event test(uint256 stakedAmount, uint256 percent);
    ...
    function getNewPixelsAmount(uint256 stakeValue, address staker) public view returns (uint256 amount){
        return (stakeValue + PIXEL_VALUE / 2 - bricksToFinishPixel[staker]) / PIXEL_VALUE;
    }
    ...
    function getRewardAmount(address staker) public returns (uint256){
        uint256 reward = 0;
        ...
        for(uint256 i = 0; i < stakings[staker].length; i++){
            ...
            uint256 rewardLevel = getRewardLevel(msg.sender, stakings[staker][i].stakingTime);
            reward += ((stakings[staker][i].amount * REWARD_PERCENT[rewardLevel]) / 100) * stakingPeriods;
            emit test(reward, stakingPeriods);
        }
        return reward;
    }
