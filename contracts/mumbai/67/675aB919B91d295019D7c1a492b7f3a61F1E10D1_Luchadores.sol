...
contract Luchadores is ERC721Enumerable, IChildToken, AccessControl, NativeMetaTransaction, ContextMixin {
  ...
  event WithdrawnBatch(address indexed user, uint256[] tokenIds);
	...
	bytes32 public constant DNA_MANAGER_ROLE = keccak256("DNA_MANAGER_ROLE");
	...
	constructor(address _luchaNames, address childChainManager) ERC721("Luchadores", "LUCHADORES") {
		...
		_setupRole(DNA_MANAGER_ROLE, _msgSender());
	...
	function setDNA(uint256[] calldata ids, uint256[] calldata dnas) external onlyRole(DNA_MANAGER_ROLE) {
	...
	function setCustomNamesEnabled(bool val) external onlyRole(DNA_MANAGER_ROLE) {
