...
/// This smart contract is built on the ERC721A smart contract from ChiruLabs for cheaper batch minting and better fee optimisation.   
...
contract GenesisMint is ERC721A, ERC2981, Ownable {
    enum Status { Phase1, Phase2, Phase3, Public }
    ...
    /// @notice The current presale mint values.
    uint public presaleMintPrice;

    /// @notice Premint phase1 data. 
    uint public premintPhase1Price = 0.1 ether;
    uint public phase1StartTimestamp;
    uint public phase1MaxMint = 5;

    /// @notice Premint phase2 data.
    uint public premintPhase2Price = 0.12 ether;
    uint public phase2StartTimestamp;
    uint public phase2MaxMint = 4;
    
    /// @notice Premint phase3 data.
    uint public premintPhase3Price = 0.15 ether;
    uint public phase3StartTimestamp;
    uint public phase3MaxMint = 3;

    /// @notice Public minting data.
    uint public publicMintPrice = 0.2 ether;
    uint public publicStartTimestamp;
    uint public publicMaxMint = 3;

    uint96 public defaultRoyalty = 750;
    ...
    error LowAmount(uint Minimum, uint Amount);
    ...
    modifier phaseOneIsOpen() {
        ...
        require(phase1StartTimestamp != 0, "Start timestamp not set");
        require(block.timestamp >= phase1StartTimestamp, "Not open yet");
        _;
    }
    ...
    modifier phaseTwoIsOpen() {
        ...
        require(phase2StartTimestamp != 0, "Start timestamp not set");
        require(block.timestamp >= phase2StartTimestamp, "Not open yet");
        _;
    }
    ...
    modifier phaseThreeIsOpen() {
        ...
        require(phase3StartTimestamp != 0, "Start timestamp not set");
        require(block.timestamp >= phase3StartTimestamp, "Not open yet");
        _;
    }
    ...
    modifier publicMintingIsOpen() {
        ...
        require(publicStartTimestamp != 0, "Start timestamp not set");
        require(block.timestamp >= publicStartTimestamp, "Not open yet");
        _;
    }

    event Minted(address indexed Owner, uint Amount, uint indexed LastTokenId, Status indexed MintPhase);

    constructor(address payable _owner) ERC721A("Centaurify_GoldenTicket", "CENT_GT", 100) {
        require(_owner != address(0x0), "Set owner");
        transferOwnership(_owner);
        royaltyReceiver = _owner;

        _setDefaultRoyalty(royaltyReceiver, defaultRoyalty);
        ownerMint(_owner, 100);
    }

    function contractURI() public pure returns (string memory) {
        return "https://CONTRACT_METADATA_URI_HERE";
    }
    ...
    /// @dev Require phase1IsOpen
    function whitelistPhase1Mint(uint amount, bytes32 leaf, bytes32[] memory proof) external payable phaseOneIsOpen {
        ...
        if (!whitelistPhase1Used[msg.sender]) {
            ...
            require(keccak256(abi.encodePacked(msg.sender, phase1MaxMint)) == leaf, "Sender and amount don't match Merkle leaf");
            ...
            whitelistPhase1Remaining[msg.sender] = phase1MaxMint;
        }

        // Require nonzero amount
        if (amount <= 0) revert LowAmount(1, amount);

        // Check proper amount sent
        require(msg.value == amount * premintPhase1Price, "Send proper ETH amount");

        require(whitelistPhase1Remaining[msg.sender] >= amount, "Can't mint more than remaining allocation");
    ...
    /// @param leaf the leaf node of the three.
    /// @param proof the proof from the merkletree.
    /// @dev Require phase2IsOpen.
    function whitelistPhase2Mint(uint amount, bytes32 leaf, bytes32[] memory proof) external payable phaseTwoIsOpen {
        // Create storage element tracking user mints if this is the first mint for them
        if (!whitelistPhase2Used[msg.sender]) {
            // Verify that (msg.sender, amount) correspond to Merkle leaf
            require(keccak256(abi.encodePacked(msg.sender, phase2MaxMint)) == leaf, "Sender and amount don't match Merkle leaf");

            // Verify that (leaf, proof) matches the Merkle root
            require(verify(merkleRoot, leaf, proof), "Not a valid leaf in the Merkle tree");

            whitelistPhase2Used[msg.sender] = true;
            whitelistPhase2Remaining[msg.sender] = phase2MaxMint;
        }
        ...
        // Check proper amount sent
        require(msg.value == amount * premintPhase2Price, "Send proper ETH amount");

        require(whitelistPhase2Remaining[msg.sender] >= amount, "Can't mint more than remaining allocation");
    ...
    /// @param leaf the leaf node of the three.
    /// @param proof the proof from the merkletree.
    /// @dev Require phase3IsOpen
    function whitelistPhase3Mint(uint amount, bytes32 leaf, bytes32[] memory proof) external payable phaseThreeIsOpen {
        // Create storage element tracking user mints if this is the first mint for them
        if (!whitelistPhase3Used[msg.sender]) {
            // Verify that (msg.sender, amount) correspond to Merkle leaf
            require(keccak256(abi.encodePacked(msg.sender, phase3MaxMint)) == leaf, "Sender and amount don't match Merkle leaf");

            // Verify that (leaf, proof) matches the Merkle root
            require(verify(merkleRoot, leaf, proof), "Not a valid leaf in the Merkle tree");

            whitelistPhase3Used[msg.sender] = true;
            whitelistPhase3Remaining[msg.sender] = phase3MaxMint;
        }
        ...
        // Check proper amount sent
        require(msg.value == amount * premintPhase3Price, "Send proper ETH amount");

        require(whitelistPhase3Remaining[msg.sender] >= amount, "Can't mint more than remaining allocation");
    ...
    function publicMint(uint amount) external payable publicMintingIsOpen {
        // Require nonzero amount
        if (amount <= 0) revert LowAmount(1, amount);

        // Check proper amount sent
        require(msg.value == amount * publicMintPrice, "Send proper ETH amount");
    ...
    function _mintWithoutValidation(address to, uint amount, bool skipMaxItems) internal {
        ...
        _safeMint(to, amount);
        emit Minted(to, amount, currentIndex, status);
    }
    ...
    function setPublicMintPrice(uint _presaleMintPrice) external onlyOwner {
        publicMintPrice = _presaleMintPrice;
    }
    ...
    function setDefaultRoyaltyInfo(address payable _royaltyReceiver, uint96 _royaltyBIPS) external onlyOwner {
        defaultRoyalty = _royaltyBIPS;
    ...
    function setDefaultRoyaltyBIPS(uint96 _royaltyBIPS) external onlyOwner {
        defaultRoyalty = _royaltyBIPS;
    }

    /* ------------------------------------------------------------  ADMIN (PREMINT & PUBLICMINT) FUNCTIONS  ----------------------------------------------------------- */

    /// @notice Used to set the Status to Premint Phase 1.
    /// @param  _phase1StartTimestamp The timestamp to start the Premint Phase 1.
    /// @param  _merkleRoot The MerkleRoot of the Phase1 whitelist to mint from.
    /// @dev Will set the default royalty to 0 for Premint Phase1.
    function setPhaseOneMintValues(uint _phase1StartTimestamp, bytes32 _merkleRoot) external onlyOwner {
        status = Status.Phase1; 
        presaleMintPrice = premintPhase1Price;
        maxItemsPerTx = phase1MaxMint;
        phase1StartTimestamp = _phase1StartTimestamp;
        merkleRoot = _merkleRoot;
        _setDefaultRoyalty(royaltyReceiver, 0);
    }

    /// @notice Used to set the Status to Premint Phase 2.
    /// @param  _phase2StartTimestamp The timestamp to start the Premint Phase 2.
    /// @param  _merkleRoot The MerkleRoot of the Phase2 whitelist to mint from.
    /// @dev Will set the default royalty to 7,5% for Premint Phase2.
    function setPhaseTwoMintValues(uint _phase2StartTimestamp, bytes32 _merkleRoot) external onlyOwner phaseOneIsOpen {
        ...
        presaleMintPrice = premintPhase2Price;
        maxItemsPerTx = phase2MaxMint;
        phase2StartTimestamp = _phase2StartTimestamp;
        merkleRoot = _merkleRoot;
        _setDefaultRoyalty(royaltyReceiver, defaultRoyalty);
    }

    /// @notice Used to set the Status to Premint Phase 3.
    /// @param  _phase3StartTimestamp The timestamp to start the Premint Phase 3.
    /// @param  _merkleRoot The MerkleRoot of the Phase3 whitelist to mint from.
    /// @dev Will set the default royalty to 7,5% for Premint Phase3.
    function setPhaseThreeMintValues(uint _phase3StartTimestamp, bytes32 _merkleRoot) external onlyOwner phaseTwoIsOpen {
        ...
        presaleMintPrice = premintPhase3Price;
        maxItemsPerTx = phase3MaxMint;
        phase3StartTimestamp = _phase3StartTimestamp;
        merkleRoot = _merkleRoot;
        _setDefaultRoyalty(royaltyReceiver, defaultRoyalty);
    }

    /// @notice Used to set the Status to Public mint.
    /// @param  _publicStartTimestamp The timestamp to start the Public minting.
    /// @param  _merkleRoot The MerkleRoot of the Phase2 whitelist to mint from.
    /// @dev Will set the default royalty to 7,5% for Public mint.
    function setPublicMintValues(uint _publicStartTimestamp, bytes32 _merkleRoot) external onlyOwner phaseThreeIsOpen {
        status = Status.Public;
        maxItemsPerTx = publicMaxMint;
    ...
    /// @dev Withdraw the contract balance to the recipient's address.
    ...
    /// @param _tokenId The token id to retrieve the metadata from.
