...
library TradeLib {
    ...
    function checkFinalizeAuctionPreconditions(address cancellationsAddress, GUTradeManager manager, TradeLib.FinalizeAuction calldata data) external view {
        ...
        require(!(GUCancellations(cancellationsAddress).cancelledBidsAndOffers(data.bidSignature)),"Offer cancelled");
    ...
    {
        data.offer.counter ? require(data.seller == data.offerer, "Unauthorized") : require(data.seller == msg.sender, "Unauthorized");
        require(manager.authorizedSigners(data.authorized), "Signer not authorized");
        require(!(GUCancellations(cancellationsAddress).cancelledListings(data.sale.listingId)), "Listing cancelled");
        require(!(GUCancellations(cancellationsAddress).cancelledBidsAndOffers(data.offerSignature)), "Offer cancelled");
        require(block.timestamp < data.sale.expiry + 3 minutes, "Listing expired");
        require(data.nftData.royalty <= manager.maxRoyaltyFee(), "Royalty too high");
        require(data.auth.fee <= manager.maxPlatformFee(), "Platform fee too high");
    ...
    function verifyBidSignature(address signer, BidData memory bid, bytes memory signature) public pure returns (bool)
    {
    ...
    {
        bytes memory toHash = abi.encodePacked(offer.to, offer.listingId, offer.amount, offer.expiry, offer.counter);
        bytes32 _hash = keccak256(toHash);
        return recoverSigner(getEthSignedMessageHash(_hash), signature) == signer;
    }

    function verifyAuthBidOfferSignature(address signer, bytes memory message, bytes memory signature) public pure returns (bool)
    {
    ...
    {
        bytes memory toHash = abi.encodePacked(nft.creator, nft.nftId, nft.cid, nft.supply, nft.royalty, nft.nftContract);
        bytes32 _hash = keccak256(toHash);
        return recoverSigner(getEthSignedMessageHash(_hash), signature) == signer;
    }
...
contract GUCollections is ERC1155Supply, ERC1155URIStorage {
    ...
    event TraderAuthorized(address traderAddress);
    event TraderDisauthorized(address traderAddress);
    event AdminSafeChanged(address safeAddress);
    event Mint(uint indexed id, address indexed to, uint amount);
...
// Owner of this software is GloballyUnited
// SPDX-License-Identifier: UNLICENSED
pragma solidity ^0.8.0;
...
contract GUCancellations {
    ...
    mapping(bytes => bool) public cancelledBidsAndOffers;

    event ListingCancelled(uint indexed id);
    event OfferCancelled(bytes indexed offerId);
    ...
    function cancelOffer(address offerer, TradeLib.OfferData calldata offer, bytes calldata offerSignature, address authorized, bytes calldata authSignature) external
    {
        require(offerer == msg.sender, "You are not the offerer");
        require(!cancelledBidsAndOffers[offerSignature], "Offer already cancelled");
        require(TradeLib.verifyOfferSignature(offerer,offer,offerSignature), "Invalid offerer's signature");
        require(TradeLib.verifyAuthBidOfferSignature(authorized, offerSignature, authSignature), "Invalid auth signature");
        cancelledBidsAndOffers[offerSignature] = true;
        emit OfferCancelled(offerSignature);
    }

    function cancelBid(address bidder, TradeLib.BidData calldata bid, bytes calldata bidSignature, address authorized, bytes calldata authSignature) external
    {
        require(bidder == msg.sender, "You are not the offerer");
        require(!cancelledBidsAndOffers[bidSignature], "Offer already cancelled");
        require(TradeLib.verifyBidSignature(bidder, bid, bidSignature), "Invalid offerer's signature");
        require(TradeLib.verifyAuthBidOfferSignature(authorized, bidSignature, authSignature), "Invalid auth signature");
        cancelledBidsAndOffers[bidSignature] = true;
        emit OfferCancelled(bidSignature);
    }
}
