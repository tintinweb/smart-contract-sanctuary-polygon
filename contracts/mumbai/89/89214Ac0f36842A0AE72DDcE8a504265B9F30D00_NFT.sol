...
contract NFT is SustainSigner, OwnableUpgradeable, ERC721Upgradeable, VRFConsumerBase{
    ...
    event minted (address _from, address  _to, uint ids);
  ...
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
        ...
        uint tokenTypes = requestToTokenTypeMap[requestId];

            // uint number = randomNumberGenerator(randomness,);
            uint tokenId = getTokenNumber(tokenTypes ,randomness);
            tokentier[tokenId] = tokenTypes;
            tokenTaken[tokenId] = true;
            
            token.transferFrom(requestToUserMap[requestId],address(this),tokentierToPrice[tokenTypes]);
    ...
    function mintNFts(uint[] memory tokenTypes) public  returns (bytes32){
        require(LINK.balanceOf(address(this)) >= fee, "Not enough LINK - fill contract with faucet");
        for(uint i=0; i<tokenTypes.length;i++){
        bytes32 data = requestRandomness(keyHash, fee);
        requestToTokenTypeMap[data] = tokenTypes[i];
...
mapping(address=>uint[]) public nftIDs;
    ...
    function getTokenNumber(uint tokenTier, uint _randomNumber) public view returns (uint) {
        uint randomNumber;
    ...
    //         uint NFTS = getRewards(tokenIds,sustain.userAddress)/tokentierToPrice[_tokenTier[0]];
            
    //         lastClaimTime[sustain.tokenId] = block.timestamp;
    //         mintNFts(sustain,tokenTier);
    //     }
