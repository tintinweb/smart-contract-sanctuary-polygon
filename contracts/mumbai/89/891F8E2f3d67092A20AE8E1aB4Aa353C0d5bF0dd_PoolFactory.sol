...
contract PoolFactory is ReentrancyGuard {
    ...
    event PoolADD(address lpToken, address pool);
    ...
    function addPool(
    ...
    ) external onlyOwner nonReentrant returns (LPStaking pool) {
        pool = new LPStaking(
            _lpToken,
            _owner,
            _rewards,
        ...
        stakingPool[_lpToken] = address(pool);

        emit PoolADD(_lpToken, address(pool));
    }
}
...
contract LPStaking is ReentrancyGuard {
    ...
    function harvest() external nonReentrant paused pausedStatus(2) {
        _getReward();
    }
    ...
    function emergencyWithdraw() external nonReentrant paused pausedStatus(3) {
