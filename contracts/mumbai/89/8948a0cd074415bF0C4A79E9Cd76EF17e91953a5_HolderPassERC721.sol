...
contract HolderPassERC721 is ERC721, AccessControl, Ownable{
    ...
    function getHolderList(uint256 mitamaTokenId) public view returns (address[] memory) {
        if(!hasGold[mitamaTokenId]) revert InvalidTokenId();
     ...
     * Update baseURI
     */
    function setImageDirURI(string memory _imageDirURI) public onlyRole(MINTER_ROLE) {
        imageDirURI = _imageDirURI;
    }
    ...
    function getSilverPassExpireCount(uint256 tokenId) public view returns (uint256) {
        ...
        uint256 expireCount = diff % 6 == 0 ? 6 : diff % 6;
        return expireCount;
    } 
    ...
    function tokenURI(uint256 tokenId) override(ERC721) public view returns (string memory) {
        ...
        string memory json;
        string memory description = "Permanent royalty rights for Mitama ecosystem participants.";
        string memory base64_json;
        // GoldPass
        if(tokenId < 10000){
            imageURI = string(abi.encodePacked(imageDirURI, "holderpass-gold.jpg"));
            pass_name = string(abi.encodePacked("Gold HolderPass #",Strings.toString(tokenId)));
        ...
        // SilverPass
        imageURI = string(abi.encodePacked(imageDirURI, "holderpass-silver.jpg"));
        pass_name = string(abi.encodePacked("Silver HolderPass #",Strings.toString(tokenId % 10000)));
        string memory expireCount = Strings.toString(getSilverPassExpireCount(tokenId));
        json = string(abi.encodePacked(
            '{"name":"',
            pass_name,
            '","symbol":"',
            ...
            '","image":"',
            imageURI,
            '","attributes":[{"display_type":"number","trait_type":"ExpireCount","value":',
            expireCount,
            '}]}'
        ));
        base64_json = Base64.encode(bytes(json));
