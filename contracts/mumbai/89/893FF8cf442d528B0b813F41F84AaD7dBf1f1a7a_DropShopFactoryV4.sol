...
import "./DropShopNFTERC1155.sol";

contract DropShopFactoryV4 {
    address immutable contractImplementation;

    event NewContractCreated(address contractAddress, address contractImplementation, bytes selector);

    constructor() {
        contractImplementation = address(new DropShopNFTERC1155());
    }

    function createNFTContract(string memory name_, string memory symbol_, string memory uri_, string memory _contractURI, address minterRole, address owner_, uint256 maxTotalSupply_, uint256 maxTokenId_) external returns (address) {
       bytes memory selector = abi.encodeWithSelector(DropShopNFTERC1155(address(0)).initialize.selector, name_, symbol_, uri_, _contractURI, minterRole, owner_, maxTotalSupply_, maxTokenId_ );
        ERC1967Proxy proxy = new ERC1967Proxy(
            contractImplementation,
            selector
        );
        emit NewContractCreated(address(proxy), contractImplementation, selector );
...
contract  DropShopNFTERC1155 is
    ...
    UUPSUpgradeable
{
        ...
        public
    {
        ...
        _setupContractId("DropShopNFTERC1155");
 ...
 /**
    ...
    */
    ...
    /**
     * @notice See definition of `_mint` in ERC1155Upgradeable contract
     * @dev This implementation only allows admins to mint tokens
     * but can be changed as per requirement
     */
    function mint(
        address account,
        string calldata metadataIpfsHash_,
        uint256 amount,
        bytes calldata data
    ) external only(MINTER_ROLE) {
        _tokenIdCounter.increment();
        uint256 tokenId = _tokenIdCounter.current();
        require( ((tokenId > uint(0)) && (tokenId <= uint(MAX_TOKEN_ID))), "INVALID_TOKEN_ID");
        require(((amount > uint(0)) && (amount == restTotalSupply(tokenId, amount))) , "INVALID_TOKEN_AMOUNT");
         ipfsHashPerTokenId[tokenId]=metadataIpfsHash_;
        _mint(account, tokenId, amount, data);
        address payable wallet = payable(account);
        setRoyalties(tokenId, wallet, 500);
    }
