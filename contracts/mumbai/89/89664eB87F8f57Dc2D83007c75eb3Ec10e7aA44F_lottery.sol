...
contract lottery is VRFConsumerBaseV2 {
    ...
    address s_owner;
    uint256 public fees = 0.02 ether; //mremember to remove public
    address better;
    uint256 betNumber;
    uint8[] rewardsArray = [0, 0, 2, 0, 3, 0];
    //event to. emit the the random number, address, counter
    event EventTogetResult(
        address better,
        uint256 counter,
    ...
    function fulfillRandomWords(
        ...
        verifyWinner(isWinner);
    }

    function verifyWinner(bool _isWinner) internal {
        emit EventTogetResult(
            better,
            requestCounter,
            s_randomWords,
            betNumber,
