...
contract Celaris is  ERC721Royalty,  Ownable, ReentrancyGuard, UpdatableOperatorFilterer, EIP712 {
    ...
    mapping(uint256 => uint256) public tokenNounceTransfer;
   ...
   function mints(address to,uint256[] memory tokenId) external {
    ...
    function checkRedeemsignature(uint256 tokenId, bytes memory signature) public view returns (bool) {
    ...
    function checkTransfersignature(uint256 tokenId, bytes memory signature, address from, address to) public view returns (bool) {
        bytes32 messageHash = keccak256(abi.encodePacked(address(this), tokenId, tokenNounceTransfer[tokenId],from, to, block.chainid));
        bytes32 signedMessageHash = ECDSA.toEthSignedMessageHash(messageHash);
        address signer = ECDSA.recover(signedMessageHash, signature);
        return signer == ownerOf(tokenId);
    }
    ...
    function redeemGasLess(uint256 tokenId, bytes memory signature) public {
        ...
        require(checkRedeemsignature(tokenId, signature) == true, "Bad signature");
        redeemedTimestamp[tokenId] = block.timestamp;
    }
    ...
    function transferGasLess(address from, address to, uint256 tokenId, bytes memory data, bytes memory signature) public virtual  {
        require(checkTransfersignature(tokenId, signature, from, to) == true, "Bad signature");
        tokenNounceTransfer[tokenId] = tokenNounceTransfer[tokenId] + 1;
