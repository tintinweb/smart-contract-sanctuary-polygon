...
interface RulesInterface{
    ...
    function prizeCheck (uint pos) external view returns (uint[] calldata);
}
...
contract ElegantEV1 is Ownable, AccessControl {
    ...
    address public racerAddress = 0x31030Daf8e8F7D6862f191BbF9dB2Cafaf52F58A;

    //peanuts
    address public tokenAddress = address(0x78331328604a43B5eBD7a624BD30B1272e836E17); 
    IERC20 rewardToken = IERC20(tokenAddress);
    //Racers
    address public erc721Contract = address(0x31030Daf8e8F7D6862f191BbF9dB2Cafaf52F58A);
    ...
    constructor() {
        ...
        teamInfo[msg.sender].percentage = 25;
    }
    ...
    function addRacer(uint _tokenID) internal { 
    ...
    function signUp (uint[] calldata _racers) public payable{
        uint _count = _racers.length;
        uint money = _count.mul(rulesByRace[currentEvent].matPrice);
        uint nutso = _count.mul(rulesByRace[currentEvent].nutPrice);
        ...
        for(uint i=0; i<_count; i++){
            addRacer(_racers[i]);
            if (rulesByRace[currentEvent].conCheck != address(0)){
                bool answer = RulesInterface(rulesByRace[currentEvent].conCheck).tokenCheck(_racers[i],rulesByRace[currentEvent].conValue);
                require(answer == true, "Token does not meet the event requirements.");
            }
        }
        totalEarned = totalEarned.add(money);
        totalNuts = totalNuts.add(nutso);
    }
    ...
    function returnSteps(uint _num, uint _grid) public view returns(uint){
        uint _steps = 0;
        uint _racer = racersByRace[_num][_grid];
        uint _rem = 0;
        uint fHundo = 500;
        
        _rem = fHundo.mod(perfByRacer[_racer].firstLeg);
        if (_rem > 0){_rem = 1;}
                
        _steps = fHundo.div(perfByRacer[_racer].firstLeg) + _rem;
        _rem = fHundo.mod(perfByRacer[_racer].secondLeg);
        if (_rem > 0){_rem = 1;}
                
        _steps = _steps + fHundo.div(perfByRacer[_racer].secondLeg) + _rem;
    ...
    function returnRace(uint _num) public view returns(uint[] memory){
        ...
        for (uint j=0; j<_size; j++){
            _gogo = 0;
            _c = 0;
            for (uint h=0; h<_steps.length; h++){
    ...
    function setEvent(uint _event) public {
        require(teamInfo[msg.sender].member == true, "Caller is not a member of the team.");
        currentEvent = _event;
     }
    ...
    function nextRace() public {
        ...
        if (currentRace < (raceNum-1)){
            ...
            uint[] memory positions = returnRace(currentRace);
            uint winnerToken = positions[0];
            address winner = perfByRacer[winnerToken].user;
            uint payOut = rulesByRace[currentEvent].matPrice.mul(5);
            uint payNut = rulesByRace[currentEvent].nutPrice.mul(5);
    ...
    function buildRace() internal {
        ...
        uint racing = 0;
        uint rMax = rulesByRace[currentEvent].gridSize;
        if (total >= rMax){
            racing = rMax;
        } else {
            racing = total;
        ...
        for(i; i<racing; i++){ //go through every racer
            ...
            uint _racer = currentlyStaked[_r];

            racersByRace[raceNum].push(_racer);   
            
            uint _nonce = block.timestamp.sub(deployT)+currentlyStaked.length;
