...
contract ShipFuser is Managable, Pausable, ReentrancyGuard {
    ...
    function fuse(uint256[] memory _botIds, address _token) whenNotPaused external nonReentrant returns(uint256 totalShipsFused) {
        require(_token != ZERO_ADDRESS, "zero address token");
        
        (uint totalShips, uint _tokenPrice, uint _oilPrice) = _fuse(_botIds, _token);
    ...
    function fuseWithMATIC (uint256[] memory _botIds) whenNotPaused external payable nonReentrant returns(uint256 totalShipsFused) {
        ...
        (uint totalShips, uint _tokenPrice, uint _oilPrice) = _fuse(_botIds, ZERO_ADDRESS);
    ...
    ) external whenNotPaused nonReentrant returns(uint256 totalShipsFused) {
        require(_token != ZERO_ADDRESS, "zero address token");        
    ...
    function _fuse(uint256[] memory _botIds, address _token) internal returns(uint totalShipsFused, uint _tokenPrice, uint _oilPrice) {
        ...
        ( _tokenPrice, _oilPrice) = getFusePrices(totalShips, _token);
        ...
        return (totalShips, _tokenPrice, _oilPrice);
    }
