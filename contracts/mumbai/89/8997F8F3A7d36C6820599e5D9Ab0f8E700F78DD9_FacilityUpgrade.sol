...
contract FacilityUpgrade is IFacilityUpgrade, IRNGConsumer, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) external override {
        ...
        cooldownEndTimestamp[upgradeOrder.player] = 
            block.timestamp + cooldownTimePerLevel * assetLevel[upgradeOrder.facilityTokenId];

        emit FacilityUpgradeFinished(upgradeOrder.player, upgradeOrder.baseOrder.orderId, upgradedFacilityId, cooldownEndTimestamp[msg.sender]);
    }
    ...
    function claimOrder(uint256 _orderId) public override {
        ...
        emit ClaimOrder(msg.sender, _orderId, reqId);
    }
...
interface IFacilityUpgrade is ITrader {
    ...
    event FacilityUpgradeFinished(address indexed player, uint256 indexed orderId, uint256 upgradedFacilityId, uint256 cooldownEndTimestamp);
    event ClaimOrder(address indexed player, uint256 indexed orderId, uint256 requestId);
