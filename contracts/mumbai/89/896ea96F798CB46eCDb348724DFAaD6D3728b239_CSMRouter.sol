...
contract CSMRouter is ICSMRouter, AccessControl, ReentrancyGuard {
    ...
    function isApprovedAsset(uint16 chainId_, uint256 assetId_) public view returns (bool) {
        return _approvedAsset[chainId_][assetId_] != address(0);
    }

    function isApprovedAsset(uint16 chainId_, address assetAddress_) public view returns (bool) {
    ...
    function getApprovedAssetId(address assetAddress_, uint16 chainId_) public view returns (uint256) {
    ...
    function lzReceive(
        ...
        require(_approvedRouters[_srcChainId] == address(_srcAddress), "SRC_CHAIN_NOT_APPROVED");
        (
            address sender,
            uint16 srcChainId,
            uint16 dstChainId,
    ...
    function toggleAssetAndChain(
        ...
        if (add_) {
            ...
            // require(_crossChainAsset[chainId_][assetId_].nativeAssetAddress != assetAddress_, "ASSET_EXISTS_3");
