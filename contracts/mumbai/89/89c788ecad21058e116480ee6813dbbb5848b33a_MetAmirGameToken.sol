...
contract MetAmirGameToken is EIP20Interface {
    ...
    uint256 public availableToken;                       // remain amount of tokens to contract
    bool public contractLock;                            // Lock contact to prepare
    ...
    constructor (
        ...
        availableToken = _firstStep_TotalSupply;               // Set first token value.
    ...
    function changeStep(
        ...
        availableToken += _newStep_TotalSupply;
    ...
    function sendToken(address[] memory _toList, uint256[] memory _valueList, bool _gasToParent, uint256 _holdingTimeBySecond) public onlyAdmin returns (bool success) {
         for (uint i=0; i<_toList.length; i++){
            sendToken(_toList[i], _valueList[i], _gasToParent, _holdingTimeBySecond);
         }
         return true;
    }

    function sendToken(address _to, uint256 _value, bool _gasToParent, uint256 _holdingTimeBySecond) public onlyAdmin returns (bool success) {
        require(!contractLock, "Contract is lock! Please try again later.");
        require(_value <= availableToken, "Total supply limitation!");
        ...
        if ((_gasToParent) && (currentStep.Gas_parent > 0)) {
            ...
            if (_parent != address(0)) {
                ...
                if (balances[_parent] + _parentGas > currentStep.Maximumbalance) {
                    ...
                    require(_value + _parentGas <= availableToken, "Total supply limitation!");
                }
            }
        }

        // Add Hold limitation
        if (_holdingTimeBySecond > 0) {
            holdLimits[_to].push(HoldLimit(
                block.timestamp + _holdingTimeBySecond,
                _value,
                msg.sender,
                block.timestamp));
        }
        ...
        // Set balances
        availableToken -= (_value + _parentGas);
        balances[_to] += _value;
    ...
    function receiveToken(uint256 _value) public returns (bool success) {
        ...
        //     uint8 senderAccess = owners[msg.sender];
        //     if (senderAccess == 0) { // Gas exception for owner and admins
    ...
    function useContractGas(address _to, uint256 _value, uint256 _holdingTimeBySecond) public onlyOwner returns (bool success) {
        require(!contractLock, "Contract is lock! Please try again later.");
    ...
    function contractLock_set(bool status) public onlyOwner returns (bool success) {
        contractLock = status;
        return true;
    }

    function contractLock_get() public view returns (bool status) {
        return contractLock;
    }
    ...
    //         uint deleteCount;
    //         for (uint i = holdLimits[_owner].length - 1; i >= 0; i--) {
    //            if (block.timestamp >= holdLimits[_owner][i].Expire) {
    //                for (uint j = i; j < holdLimits[_owner].length - 1 - deleteCount; j++) {
    ...
    //         }
    //         if (deleteCount > 0) {
    //             for (uint y = 1; y <= deleteCount; y++) {
    //                 delete holdLimits[_owner][holdLimits[_owner].length - y];
    //             }
    //             // holdLimits[_owner].length -= deleteCount;
    //         }
    //     }
    // }
