...
contract Pot {
    ...
    uint256 private createClaimAllowTime = 840; //5702400000; // 66 days
    ...
    function getLifeTime() public view returns (uint256) {
        // require(timeUntilExpiry > block.timestamp, "You cannot get lifeTime because this is close bidding!");
...
contract ControlPot {
    ...
    event Deployed(address, address, address);
    ...
    function createPot(uint256 _potTokenIndex, uint256 _potAmount, uint256 _bidTokenIndex, bidPrice memory _bid, address[] memory _toAddress, uint256[] memory _toPercent, expiryTimeInfoVal memory _expirationTime, bool _priorityPool, uint256 _toPotFee, uint256 _toPreviousFee) external returns (address pair) {
        ...
        require(bidPercent == (percent - toOwnerFee - _toPotFee - _toPreviousFee), "Fee is not 100%!");
        ...
        cValue.toPotFee = _toPotFee;
        cValue.toPreviousFee = _toPreviousFee;

        bytes32 salt = keccak256(abi.encodePacked(tokenList[_potTokenIndex], _potAmount, tokenList[_bidTokenIndex], _bid.variable1, _toAddress, _toPercent, cValue.expiryTime.startTime, cValue.expiryTime.decreaseBy, cValue.expiryTime.minimumTime, _priorityPool, _toPreviousFee));
        assembly {
            pair := create2(0, add(bytecode, 32), mload(bytecode), salt)
        }
        allTournaments.push(pair);
        Pot(pair).initialize(cValue);
        emit Deployed(tokenList[_potTokenIndex], tokenList[_bidTokenIndex], pair);
        bidPercent = 0;
    }
}