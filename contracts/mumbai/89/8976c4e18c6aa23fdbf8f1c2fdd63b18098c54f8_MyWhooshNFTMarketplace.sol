...
/* MyWhoosh NFT Marketplace
    ...
    & support Royalty
*/
contract MyWhooshNFTMarketplace is Ownable {
    IMyWhooshNFTFactory private immutable MyWhooshNFTFactory;
    ...
    constructor(
        ...
        IMyWhooshNFTFactory _MyWhooshNFTFactory
    ) {
        
        MyWhooshNFTFactory = _MyWhooshNFTFactory;
    }

    modifier isMyWhooshNFT(address _nft) {
        require(MyWhooshNFTFactory.isMyWhooshNFT(_nft), "not MyWhoosh NFT");
        _;
    }
    ...
    function listNft(
        ...
        address _payToken,
        uint256 _price
    ) external isMyWhooshNFT(_nft) isPayableToken(_payToken) {
    ...
    function buyNFT(
        ...
        uint256 royaltyFee = MyWhooshNFTFactory.getMyWhooshNFTRoyality(_nft);

        address royaltyRecipient = MyWhooshNFTFactory.getMyWhooshNFTRoyalityRecipient(_nft);
        ...
        isListedNFT(_nft, _tokenId)
    {
        ...
        uint256 royaltyFee = MyWhooshNFTFactory.getMyWhooshNFTRoyality(offer.nft);

        IERC20 payToken = IERC20(offer.payToken);
  
        uint256 royaltyTotal = calculateRoyalty(royaltyFee, offerPrice);

        address royaltyRecipient = MyWhooshNFTFactory.getMyWhooshNFTRoyalityRecipient(offer.nft);
        ...
        // Transfer to seller
        payToken.transfer(list.seller, totalPrice - royaltyTotal);
    ...
    function resultAuction(address _nft, uint256 _tokenId) external {
        ...
        payToken.transfer(auction.creator, totalPrice - royaltyTotal);
   ...
   // @calculateRoyalty 
        ...
        returns (uint256)
    {
        ...
        return (_price * _royalty) / 10000; //its devided by 10000 to saving from math error 
    }
    // @getListed Nfts 
   ...
   // check ERC20 token listed for trade 
    ...
    function addPayableToken(address _token) external onlyOwner {
     ...
     // withdraw any token if its stuck in contract 
    function royaltyWithdraw(address _token, uint256 _amount) external onlyOwner
    {
