...
contract WildGamesThreeTest is Ownable {
    IERC20 paymentToken;
    ...
    event UserEnteredGame(address indexed user, uint256 indexed betIndex, uint256 indexed gameIndex, address[3] participants);
    event GameFinished(uint256 indexed betIndex, uint256 betValue, uint256 indexed gameIndex, address looser, address[3] participants);

    constructor() {
        paymentToken = IERC20(0xEA548e0d48255Ae04EFaD40127352aEfc5A51785);
        
        wildGamesVault = 0xddF056C6C9907a29C3145B8C6e6924F9759103E4;
        amountGamesUntilExtraGame = 3;
    ...
    function createGame(uint256 _betValue) public onlyOwner {
        ...
        address[3] memory playersArr;
        uint256[3] memory playersNotesArr;
    ...
    function viewPlayersByGame(uint _indexGame) public view returns(address[3] memory) {
        Game storage currentGame = AllGames[_indexGame];
 ...
 ////////////////////////////////////////////
    ...
    function drawProcess(uint _indexGame) internal {
        ...
        uint payoutForWinner = (currentGame.betValue * 140) / 100; //80%
