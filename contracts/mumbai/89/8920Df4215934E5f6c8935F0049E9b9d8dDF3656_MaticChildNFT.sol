...
/**
 * @title Matic Custom Child ERC721 Token
 *
 * @notice Custom child contract should follow certain guidelines before being deployed on the child chain.
 *
 * @notice `deposit` method should be present in your custom child contract.
 *      This function is called by the ChildChainManagerProxy contract whenever a deposit is initiated
 *      from the root chain. This deposit function internally mints the token on the child chain.
 *
 * @notice `withdraw` method should be present in your custom child contract. It can be called to burn your tokens
 *      on the child chain. Burning is the first step of your withdrawal process. This withdraw function will
 *      internally burn the token on the child chain.
 *
 * @notice These rules need to be followed to maintain proper balance of assets between two chains.
 *
 * @notice Note: No token minting in constructor of child token contract.
 */
interface ChildERC721 {
	/**
	...
	/**
	 * @notice Keeps track of withdrawn tokens (tokens which exist on L1);
	 *      since withdrawn tokens exist on L1, they cannot be minted on L2
	 *
	 * @param tokenId tokenId to check the withdrawn status (L1 existence) for
	 */
...
import "../interfaces/MaticSpec.sol";
import "alethea-contracts/contracts/token/RoyalNFT.sol";

/**
 * @title Matic Child NFT
 *
 * @notice Polygon mintable ERC721 token (asset),
 *      see https://docs.polygon.technology/docs/develop/ethereum-polygon/mintable-assets
 */
contract MaticChildNFT is ChildERC721, RoyalNFT {
	/**
	 * @inheritdoc TinyERC721
	 */
	uint256 public constant override TOKEN_UID = 0x883747860db533378d921c661076ce84cb9c6d22c6e299e89a20d213206292cf;

	/**
	 * @dev Constructs/deploys Matic Child NFT instance
	...
	/**
	 * @inheritdoc ChildERC721
	 */
	mapping(uint256 => bool) public override withdrawnTokens;

	/**
	 * @inheritdoc ChildERC721
	 */
	function deposit(address user, bytes calldata depositData) public {
		// decode token IDs to deposit (mint)
		uint256[] memory tokenIds =
			// single token is passed as uint256 and occupies one slot of calldata (32 bytes)
			depositData.length == 32? new uint256[](abi.decode(depositData, (uint256))):
			// batch of tokens is passed as uint256 array
			abi.decode(depositData, (uint256[]));

		// deposit tokens one by one:
		for (uint256 i; i < tokenIds.length; i++) {
			// mint next token (role ROLE_TOKEN_CREATOR required)
			mint(user, tokenIds[i]);
			// mark minted token as deposited (not withdrawn)
	...
	function withdraw(uint256 tokenId) public {
		// burn token (role ROLE_TOKEN_DESTROYER required)
		burn(tokenId);
