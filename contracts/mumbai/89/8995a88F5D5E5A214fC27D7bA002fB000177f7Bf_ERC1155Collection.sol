...
contract ERC1155Collection is ERC1155, AccessControl, Pausable, ERC1155Burnable, ERC1155Supply, Ownable{
    ...
    function setSupplyLimit(uint256 id, uint256 limit) external onlyRole(OPERATOR_ROLE){
        require(limit > 0, "DustlandCollection: cannot set limit to 0");
        require(_supplyLimit[id] == 0, "DustlandCollection: cannot update supply limit");
        require(limit >= totalSupply(id) , "DustlandCollection: limit is less than current supply");
    ...
    {
        _safeDeliverFrom(from, tos, ids, amounts, data);
    }

    function _safeDeliverFrom(
        address from,
        address[] memory tos,
        uint256[] memory ids,
        uint256[] memory amounts,
        ...
        require(length == ids.length && length == amounts.length,  "DustlandCollection: tos, ids and amounts length mismatch");
        ...
        if (from == address(0)) {
            for (uint256 i = 0; i < ids.length; ++i) {
                ...
                if(_supplyLimit[id] != 0 && totalSupply(id) > _supplyLimit[id]){
                    revert("DustlandCollection: excess supply limit");
                }
            }
        }
        if (to == address(0)) {
