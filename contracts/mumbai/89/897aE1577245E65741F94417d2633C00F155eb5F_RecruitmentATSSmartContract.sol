...
contract RecruitmentATSSmartContract {
    ...
    function applyJob(
        ...
        applicationsByEmployer[jobs[jobId].employerAddress].push(
            Application(
                applicantAddress,
                jobs[jobId].employerAddress,
                jobId,
        ...
        applicationsByApplicant[applicantAddress].push(
            Application(
                applicantAddress,
                jobs[jobId].employerAddress,
                jobId,
        ...
        emit NewJobApplication(applicantAddress, jobId);
    }

    // Function to accept an application
    function acceptApplication(uint jobId, address applicantAddress) public {
        // Ensure the job exists
        require(jobs[jobId].salary > 0, 'Job does not exist.');
        ...
        for (
            uint i = 0;
            i < applicationsByEmployer[jobs[jobId].employerAddress].length;
            i++
        ) {
            if (
                applicationsByEmployer[jobs[jobId].employerAddress][i].jobId ==
                jobId
            ) {
                applicationsByEmployer[jobs[jobId].employerAddress][i]
                    .accepted = true;
            }
        }
        ...
        for (
            uint i = 0;
            i < applicationsByApplicant[applicantAddress].length;
            i++
        ) {
            if (applicationsByApplicant[applicantAddress][i].jobId == jobId) {
                applicationsByApplicant[applicantAddress][i].accepted = true;
            }
        }
    ...
    function rejectApplication(uint jobId, address applicantAddress) public {
        // Ensure the job exists
        require(jobs[jobId].salary > 0, 'Job does not exist.');
