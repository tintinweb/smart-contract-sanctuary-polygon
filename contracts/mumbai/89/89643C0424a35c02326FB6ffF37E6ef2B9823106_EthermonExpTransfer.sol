...
// File: contracts/EthermonExpTransfer.sol
...
contract EthermonExpTransfer is BasicAccessControl {
    ...
    event EventEthermonExpTransfer(uint64 monster_id, address trainer);
    ...
    //_from is sacrifice mon.
    function transferExp(uint64 _from, uint64 _to) external onlyModerators {
        ...
        (
            ...
            obj1.lastClaimIndex,
            obj1.createTime
        ) = data.getMonsterObj(_from);

        //If mon isn't burned already
        ...
        //Increase experience of the mon.
        data.increaseMonsterExp(obj.monsterId, obj1.exp);

        // Sacrificing the mon. (Burning)
        data.removeMonsterIdMapping(obj1.trainer, _from);

        emit EventEthermonExpTransfer(_from, address(0));
        emit EventEthermonExpTransfer(_to, obj.trainer);
    }
}