...
library Errors {
    ...
    error SttrCampaignNotClosed();
    error SttrCampaignNotClosedNorNotFunded();
    error SttrCampaignNotSucceededNorFunded();
    ...
    error ProjWithdrawAmountTooHigh();
...
contract VCProject is Initializable {
    ...
    function getAmountToRaise() external view returns (uint256) {
        return _amountToRaise;
    }

    // this function only works if the lab/researcher is only allowed to create one campaign at the time
    // it only considers the amount raised through campaigns, if someone transfers directly it will not apear here
    function getRaisedAmount() public view returns (uint256 raisedAmount) {
    ...
    function startCampaign(
        ...
        _onlyStarter();
        if (_target > _amountToRaise - getRaisedAmount()) {
            revert Errors.ProjCampaignTargetTooHigh();
        }
    ...
    // how we are going to handle the $ received from the marketplace if the seller decides to support the Campaign?
    ...
    // Lab/Researcher would be able to withdraw more than what they raised with each Campaign if someone transfers directly
    function labWithdraw(
        uint256 _amountToWithdraw,
        IERC20 _currency,
        address _receiver
    ) external {
        _onlyStarter();
        uint256 balanceAvailable = _currency.balanceOf(address(this)) - _totalCampaignsBalance[_currency];
        if (_amountToWithdraw > balanceAvailable) {
            revert Errors.ProjWithdrawAmountTooHigh();
        }
        if (!_currency.transfer(_receiver, _amountToWithdraw)) {
            revert Errors.ProjERC20TransferError();
        }
    }
...
contract VCStarter {
    ...
    event SttrSetMaxCampaignOffset(uint256 maxCampaignOffset);
    ...
    event SttrProjectCreated(address indexed lab, address indexed project, uint256 amountToRaise);
    ...
    // event SttrCampaignForcedNotFunded(address indexed lab, address indexed project, uint256[] campaignId);
    ...
    event SttrLabWithdrawal(address indexed lab, address indexed project, IERC20 indexed currency, uint256 amount);
    ...
    address private _projectTemplate;
    address private _admin;
    address private _pool;
    IPoCNft private _poCNft;

    /// @notice The list of laboratories
    mapping(address => bool) private _isWhitelistedLab;
    mapping(address => bool) private _isWhitelistedResearcher;
    mapping(address => address) private _projectToLab;
    mapping(IERC20 => bool) private _allowedCurrencies;

    uint256 private _minCampaignDuration;
    uint256 private _maxCampaignDuration;
    uint256 private _maxCampaignOffset;
    uint256 private _minCampaignTarget;
    uint256 private _maxCampaignTarget;
    uint256 private _softTargetBps;

    uint256 private constant _FEE_DENOMINATOR = 10_000;

    /// @notice amount of seconds to wait for lab operation
    uint256 private _backersTimeout = 15 * 24 * 60 * 60; // 15 days
    ...
    modifier onlyLabOwner(address _project) {
    ...
    // check multiple events inside tests
    // function forceNotFundedCampaigns(address project, uint256[] calldata campaignIds) external onlyAdmin {
    //     if (_projectToLab[project] == address(0)) {
    //         revert Errors.SttrUnexistentProject();
    //     }

    //     uint256 numberOfCampaigns = VCProject(project).getNumberOfCampaigns();
    //     uint256[] memory closedCampaigns = new uint256[](campaignIds.length);

    //     for (uint256 i = 0; i < campaignIds.length; i++) {
    //         if (campaignIds[i] < numberOfCampaigns) {
    //             CampaignStatus status = getCampaignStatus(_project, _campaignId);
    //             if (status == CampaignStatus.ACTIVE) {
    //                 VCProject(project).setCampaignNotFunded(campaignIds[i]);
    //                 closedCampaigns[i] = (campaignIds[i]);
    //             }
    //         }
    //     }
    //     emit SttrCampaignForcedNotFunded(_projectToLab[project], project, closedCampaigns);
    // }
    ...
    // function setMaxCampaignOffset(uint256 maxCampaignOffset) external onlyAdmin {
    //     if (maxCampaignOffset == _maxCampaignOffset) {
    //         revert Errors.SttrMaxCampaignOffsetError();
    //     }
    //     _maxCampaignOffset = maxCampaignOffset;
    //     emit SttrSetMaxCampaignOffset(_maxCampaignOffset);
    // }
    ...
    // only whitelisted labs?
    function createProject(address _lab, uint256 _amountToRaise) external returns (address newProject) {
        _onlyAdmin();
        newProject = Clones.clone(_projectTemplate);
        VCProject(newProject).init(address(this), _lab, _amountToRaise);
        _projectToLab[newProject] = _lab;
        emit SttrProjectCreated(_lab, newProject, _amountToRaise);
    }
    ...
    function startCampaign(
        address _project,
        uint256 _target,
        // uint256 _offset // pause this feature for the moment
        uint256 _duration
    ) external onlyWhitelistedLab onlyLabOwner(_project) returns (uint256) {
        ...
        // if (_offset > _maxCampaignOffset) {
        //     revert Errors.SttrCampaignOffsetError();
        // }
        uint256 softTarget = (_target * _softTargetBps) / _FEE_DENOMINATOR;
        uint256 startTime = block.timestamp; // + _offset;
        uint256 endTime = startTime + _duration;
        uint256 backersDeadline = endTime + _backersTimeout;
    ...
    function fundCampaign(
        ...
        (uint256 amountToCampaign, uint256 amountToPool, bool isFunded) = VCProject(_project).getFundingAmounts(
            _campaignId,
            _amount
        );
        ...
        VCProject(_project).fundCampaign(_campaignId, _currency, msg.sender, amountToCampaign);
        emit SttrCampaignFunding(lab, _project, _campaignId, _currency, msg.sender, amountToCampaign);
        ...
        if (isFunded) {
            emit SttrCampaignFunded(lab, _project, _campaignId);
        }
    }
    ...
    function backerMintPoCNft(
        ...
        CampaignStatus newStatus = getCampaignStatus(_project, _campaignId);

        if (newStatus != CampaignStatus.FUNDED && newStatus != CampaignStatus.SUCCEEDED) {
            revert Errors.SttrCampaignNotSucceededNorFunded();
        }
    ...
    function backerWithdrawDefeated(
        ...
        if (newStatus == CampaignStatus.NOTFUNDED) {
        ...
        } else if (newStatus == CampaignStatus.DEFEATED) {
            uint256 amount = VCProject(_project).validateMint(_campaignId, _currency, msg.sender);
            _poCNft.mint(msg.sender, amount);
            emit SttrBackerMintPoCNft(_projectToLab[_project], _project, _campaignId, _currency, amount);
        } else {
            revert Errors.SttrCampaignNotClosedNorNotFunded();
        }
    }
    ...
    function labCampaignWithdraw(
        ...
        uint256 _campaignId,
        IERC20 _currency
    ) external onlyLabOwner(_project) {
        CampaignStatus newStatus = getCampaignStatus(_project, _campaignId);

        if (newStatus != CampaignStatus.FUNDED) {
            revert Errors.SttrCampaignNotFunded();
        }

        (uint256 withdrawAmount, bool campaignSucceeded) = VCProject(_project).labCampaignWithdraw(
            _campaignId,
            _currency
        );
        emit SttrLabCampaignWithdrawal(msg.sender, _project, _campaignId, _currency, withdrawAmount);
        if (campaignSucceeded) {
            emit SttrCampaignSucceded(_projectToLab[_project], _project, _campaignId);
        }
    }
    ...
    function transferUnclaimedFunds(
        ...
        if (newStatus != CampaignStatus.DEFEATED) {
            revert Errors.SttrCampaignNotClosed();
        }

        uint256 amountToPool = VCProject(_project).transferUnclaimedFunds(_campaignId, _currency, _pool);
        emit SttrUnclaimedFundsTransferredToPool(
            _projectToLab[_project],
            _project,
            _campaignId,
            _currency,
            amountToPool
    ...
    function labWithdraw(
        ...
        VCProject(_project).labWithdraw(_withdrawAmount, _currency, msg.sender);
        emit SttrLabWithdrawal(msg.sender, _project, _currency, _withdrawAmount);
    }
    ...
    function isWhitelistedLabByProject(address[] memory _projects) external view returns (bool[] memory) {
        bool[] memory isWhitelisted = new bool[](_projects.length);
        for (uint256 i = 0; i < _projects.length; i++) {
            address lab = _projectToLab[_projects[i]];
            isWhitelisted[i] = _isWhitelistedLab[lab];
        }
        return isWhitelisted;
        // if needed, here we can also check if the project is active
    }

    function isWhitelistedLab(address[] memory _labs) external view returns (bool[] memory) {
        bool[] memory isWhitelisted = new bool[](_labs.length);
        for (uint256 i = 0; i < _labs.length; i++) {
            isWhitelisted[i] = _isWhitelistedLab[_labs[i]];
        }
        return isWhitelisted;
    }
