...
contract BidAndCollect is ERC721("B&C", "B&C"), IERC721Receiver {
   
    address coin_address = 0x62A8DE6B2FB6679F16377bB819Dba67239eaC719;
    address btc_address = 0xFA2C7442364AA0a5630d85e2A268b9B01185348F;
    IERC20 coin = IERC20(coin_address);

    IERC20 wbtc = IERC20(btc_address);
address usdc_agg= 0x572dDec9087154dC5dfBB1546Bb62713147e0Ab0;
address wbtc_agg= 0x007A22900a3B98143368Bd5906f8E17e9867581b;
    struct Storage {
        uint256[] members;
    ...
    AggregatorV3Interface awbtc =
        AggregatorV3Interface(wbtc_agg);
    AggregatorV3Interface usdc =
        AggregatorV3Interface(usdc_agg);
    ...
    function join(uint128 hash) public {
        ...
        wbtc.transferFrom(msg.sender, address(this), ps.collateralValue);
    ...
    function defaultfn(
        ...
        for (uint256 j = 0; j < s.currentMembers; j++) {
            ...
            if (
                ...
                dft =
                    (s.bd[k].bidPlacedValue *awbtc_decimals) /
    ...
    function collectBidValue(uint128 hash, uint256 uid) public {
        ...
        coin.transfer(msg.sender, (197 * val) / 200);
...
uint256 awbtc_decimals= 10**awbtc.decimals();
uint256 usdc_decimals= 10**usdc.decimals();
  
    function getBTCPrice() public view returns (uint256) {

        (,int price,,,)= awbtc.latestRoundData();
        (,int price2,,,)=usdc.latestRoundData();
        return uint((price*int(usdc_decimals/awbtc_decimals))/(price2/int(usdc_decimals)));
