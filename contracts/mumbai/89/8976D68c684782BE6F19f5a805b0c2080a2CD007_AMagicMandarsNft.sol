...
contract    AMagicMandarsNft  is  ERC1155D
{
    ...
    //constructor(string memory newUri) ERC1155D("") 
    ...
    function    uri(uint tokenId) public view virtual override returns (string memory) 
    {
        ...
        // /* pour voir les tokens uniquement minté */ require(tokenId<=generatedTokenCount, "Id??");
        ...
        nLeaf = getRand(rndSeed);   rndSeed++;

        nftContent = string(abi.encodePacked(nftContent,
                                        "\"},{\"trait_type\":\"Complexity\",\"value\":\"",(30+(idx%70)).toString(),
                                        "0\"},{\"trait_type\":\"Energy\",\"value\":\"",((16+(nLeaf%185))*10).toString(),
                                        "\"}"));

        //-----

        uint darkMode   = ((getRand(rndSeed+5) & 127) > 64) ? 1:0;  rndSeed++;
        ...
        finalSVG = string(abi.encodePacked(finalSVG, svgDEF1c, svgDEF2, svgDEF3, blendFxs[darkMode], svgDEF3b, svgCageForces[rndSeed&7]));
                        ...
                        external payable
    {
        ...
        require(proposedHashes[hash256]!=1, "DoubleH");
        
        require(keccak256(abi.encodePacked( nonce,
                                            quantity,
                                            affWallet,
                                            msg.sender)
                )==proposedHash, "Havoc");

        require(uint256(s) < ecLimit, "EC Error");
    ...
    //=============================================================================
    function    whitelistMint(bytes32   proposedHash, uint8 v, bytes32 r, bytes32 s,
                              uint      nonce,
                              uint      quantity, 
                              address   affWallet, 
    ...
    //=============================================================================
    function    isWhitelisted(bytes32[] calldata merkleProof, address wallet) public view returns(bool)
    {
        return merkleProof.verify(whitelistMerkleRoot, keccak256(abi.encodePacked(wallet)));
    }
    //=============================================================================
    ...
    //=============================================================================
    //=============================================================================
    function    payAffiliateNftFees(address affiliateWallet, uint totalAmount, uint quantitySold, uint pricePerNft)   internal returns(uint)
    {
        if (affiliateWallet==address(0x0))      return totalAmount;     // not a good index!!!

        uint affiliateAmount = (totalAmount * affiliatePercentInM100) / 100;
        totalAmount         -= affiliateAmount;

        (bool sent,) = affiliateWallet.call{value: affiliateAmount}("");    require(sent, "Failed");

        TAffiliate storage affStat = affiliatesStats[affiliateWallet];

        affStat.mintCount   += 1;
        affStat.nftCount    += quantitySold;
        affStat.totalAmount += totalAmount;
        affStat.revenue     += affiliateAmount;

        emit PayAffiliateNftFees(affiliateWallet, affiliateAmount, totalAmount, quantitySold, pricePerNft);

        return totalAmount;
    }
    //=============================================================================
    function    getAffiliateStat(address affiliateWallet) external view returns(TAffiliate memory)
    {
        return affiliatesStats[affiliateWallet];
    }
    //=============================================================================
    //=============================================================================
    function    setSigningEntity(uint payload, uint k) external  onlyOwner
    {
