...
// Fourth Version (2022-4-1)
contract NewFactory is OwnableUpgradeable {
  address public immutable TIMELOCK_IMPL;

  address private newTokenBaseDeployer; // staking erc20 tokens to mint PASS
  address private newNftBaseDeployer; // staking erc721 tokens to mint PASS
  address private newFixedPeriodDeployer; // pay erc20 tokens to mint PASS in a fixed period with linearly decreasing price
  address private newFixedPriceDeployer; // pay erc20 tokens to mint PASS with fixed price
  ...
  constructor(
    ...
    timelock = _timelock;
    newTokenBaseDeployer = _tokenBaseDeployer;
    newNftBaseDeployer = _nftBaseDeployer;
    newFixedPeriodDeployer = _fixedPeriodDeployer;
    newFixedPriceDeployer = _fixedPriceDeployer;
    _setPlatformParms(_platform, _platformRate);

    TIMELOCK_IMPL = address(new Timelock());
  }

  event NewTokenBaseDeploy(
  ...
  function newTokenBaseDeploy(
    address _erc20,
    uint256 _rate,
    address _timelock,
    bytes calldata _newParameter
  ) public {
    INewTokenBaseDeployer factory = INewTokenBaseDeployer(newTokenBaseDeployer);
    //return the address of deployed NFT PASS contract
    address addr = factory.newDeployTokenBase(
      _timelock == address(0) ? cloneTimelock() : _timelock,
      _erc20,
      _rate,
      _newParameter
    );
    emit NewTokenBaseDeploy(addr, _erc20, _rate, _newParameter);
  }

  function newNftBaseDeploy(
    address _erc721,
    address _timelock,
    bytes calldata _newParameter
  ) public {
    INewNFTBaseDeployer factory = INewNFTBaseDeployer(newNftBaseDeployer);
    address addr = factory.newDeployNFTBase(
      _timelock == address(0) ? cloneTimelock() : _timelock,
      _erc721,
      _newParameter
    );
    emit NewNFTBaseDeploy(addr, _erc721, _newParameter);
  }

  function newFixedPeriodDeploy(
    address _erc20,
    ...
    address addr = INewFixedPeriodDeployer(newFixedPeriodDeployer).newDeployFixedPeriod(
      _timelock == address(0) ? cloneTimelock() : _timelock,
      _erc20,
      platform,
      _receivingAddress,
  ...
  function newFixedPriceDeploy(
    ...
    INewFixedPriceDeployer factory = INewFixedPriceDeployer(newFixedPriceDeployer);
    address addr = factory.newDeployFixedPrice(
      _timelock == address(0) ? cloneTimelock() : _timelock,
      _erc20,
      platform,
      _receivingAddress,
