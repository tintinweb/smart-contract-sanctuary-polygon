...
contract Oracle is Ownable {
    ...
    function totalInfos(address _account) external view returns (uint256, address, uint256, uint256, uint256, uint256, uint256)
    {
        return(levelsLength[_account], owner(), _totalSupply, _depositedAmount[_account],_rewardAmount[_account], _leftPartnersAmount[_account], _rightPartnersAmount[_account] );
    }

    function poolInfos() external view returns (uint256, uint256, uint256, uint256)
    {
        return(Pool1Amount, Pool2Amount, Pool3Amount, Pool4Amount);
    }
