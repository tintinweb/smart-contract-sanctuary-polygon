...
contract EmpatikaEDU is ERC1155, AccessControl, IERC721Mintable {
    bytes32 public constant MANAGER_ROLE = keccak256("MANAGER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    uint256 public currentEventTokenId = 0;

    mapping(uint256 => uint128) private _supplies;

    // "https://edu.empatika.com/api/tokens/metadata/{id}"
    constructor(string memory uri_, uint256 initEventTokenId_)
        ERC1155(uri_)
    {
    ...
    function setSupply(uint256 tokenId_, uint128 supply_)
        external
        onlyRole(MANAGER_ROLE)
    {
        ...
        returns (uint256)
    {
        ...
        require(_supplies[currentEventTokenId] > 0, "No supply");

        _supplies[currentEventTokenId] -= 1;
