...
// ReentrancyGuard, 
contract PhroToken is ERC20Burnable, Ownable, AccessControl
{
    ...
    string private constant ZERO_ADDRESS = "Cannot be zero address";
    ...
    uint256 public constant _TIMELOCK_YEAR = 365 days; // 1 year in days
    uint256 public constant _TIMELOCK_6MONTHS = _TIMELOCK_YEAR / 2; // 6 months in days
    ...
    modifier isPermittedOperator() {
        require(hasRole(PharoConstants.OPERATOR_ROLE, msg.sender) || owner() == msg.sender,
                "Not an approved minter");
        _;
    }
        ...
        ERC20Burnable()
    {
        ...
        _mint(0x1DE9264aaf8e59368d20D75BA9F97DFe5D9FEf41, 10000000 ether); // Aphilos
        _mint(0x3f15B8c6F9939879Cb030D6dd935348E57109637, 10000000 ether); // Jax
    }

    function setupMinterRole(address minter) public onlyOwner {
        _setupRole(MINTER_ROLE, minter);
    }

    /// @notice Creates `_amount` token to `_to`. Must only be called by PermittedMinter
    // nonReentrant
    function mintTokens(address _to, uint256 _amount) public isPermittedMinter {
    ...
    function burn(uint256 _amount) public virtual override {
        require(
            ...
            "PharoToken :: Not enough balance to burn."
        );
    ...
    function mint(uint256 amount) public virtual onlyOwner {
        balances[TreasuryAddress] = balances[TreasuryAddress].add(amount);
    ...
    function mintForContract(
        ...
        uint256 amount
    ) external isPermittedMinter {
        // must assign any contract needing to mint PHRO tokens the MINTER_ROLE
        // PharoStakePool so far is one
    ...
    function burnFrom(address account, uint256 amount) public virtual override {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(
            amount,
            "PharoToken:: burn amount exceeds allowance"
        );
    ...
    function permit(
        ...
        require(
            ...
            "PHRO :: past the fucking deadline bro"
        );
        ...
        require(
            signatory != address(0),
            "PHRO :: permit: invalid signature ~ zero address"
        );
        require(
            signatory == owner,
            "PHRO :: permit: unauthorized ~ owner not allowed"
        );
    ...
    */
    function lock(bytes32 _reason, uint256 _amount, uint256 _time) public onlyOwner returns(bool)
    {
