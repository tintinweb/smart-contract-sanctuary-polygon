...
contract Marketplace is Ownable {
    ...
    mapping(uint256 => address) public listingToBuyer;
...
function buy(uint256 tokenId) external {
    ...
    listingToBuyer[tokenId] = msg.sender; // store the buyer's address
...
function releasePayment(uint256 tokenId) external onlyOwner {
    ...
    address buyer = listingToBuyer[tokenId]; // Use the stored buyer's address
    uint256 price = listing.price;
    uint256 fee = (price * feePercentage) / FEE_DENOMINATOR;

    nft.safeTransferFrom(seller, buyer, tokenId); // Transfer NFT ownership to the buyer
    ...
    delete tokenIdToListing[tokenId];
    delete listingToBuyer[tokenId];

    emit SaleCompleted(tokenId, buyer, seller, price); // Emit event with correct buyer and price
}
