...
contract LottoGame is VRFConsumerBaseV2 {
    ...
    function buyTickets(string calldata _referrerUsername, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket memory) {
        ...
        if(!Vault(vault).hasUsername(msg.sender)){
            Vault(vault).setUsername(msg.sender, _referrerUsername);
        }
    ...
    function batchBuyTickets(uint256 _numberOfTickets, string calldata _referrerUsername, uint8 _v, bytes32 _r, bytes32 _s) external returns(Ticket[] memory) {
        require(block.timestamp >= startTime, "Game not started yet");
        bool hasSignedConsent = Vault(vault).hasSignedConsent(msg.sender);
        if(!hasSignedConsent){
            bytes memory prefix = "\x19Ethereum Signed Message:\n32";
