...
contract DecentralizedGreenEnery is Context, IERC20, Ownable {  
    ...
    uint256 public sellPrice = 0.00000000002 ether;
    ...
    */
    ...
    function buy(uint256 tokenAmount)public payable  {
        ...
        uint256 cost = tokenAmount * buyPrice;
        uint256 _marketing = tokenAmount.mul(_marketingPer).div(100);
        uint256 _autoBurn = tokenAmount.mul(_autoBurnPer).div(100);
        uint256 _reward = tokenAmount.mul(_RewardPer).div(100);
        uint256 netAmount = tokenAmount.sub(_autoBurn).sub(_marketing).sub(_reward);
    ...
    function calculateTokens() public payable returns (uint256) {
    uint256 price = msg.value * buyPrice;
    uint256 tokens = price / 1 ether; // Assuming 1 token = 1 ether
    return tokens;
}
