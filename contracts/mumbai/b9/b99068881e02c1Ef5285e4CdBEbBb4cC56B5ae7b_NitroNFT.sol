 ...
 * @author NitroLeague.
 */
contract NitroNFT is ERC721Enumerable, MetaOwnable, Mintable, ClaimContext {
    using Counters for Counters.Counter;
    string private baseURI;

    string[] public tokenTypes;
    mapping(uint => uint) public nextTokenIdByType;

    event TokenTypeAdded(string indexed tokenName);
    event TokenTypeRemoved(string indexed tokenName);
    ...
    function safeMint(address _to, uint tokenTypeIndex) public onlyOwner {
        _safeMint(_to, nextTokenIdByType[tokenTypeIndex]);
        setNextTokenId(tokenTypeIndex);
    }
    ...
    function addTokenType(string calldata tokenName) external onlyOwner {
        tokenTypes.push(tokenName);
        setNextTokenId(tokenTypes.length - 1);
        emit TokenTypeAdded(tokenName);
    }

    function removeTokenType(uint index) external onlyOwner {
        /**
         *
         * 0.json => Civic #0
         * 1.json => Corolla #1
         ...
         * 3.json => Mehran #3
         */
        uint lastTokenType = tokenTypes.length - 1;
        require(index <= lastTokenType, "invalid index");

        string memory tokenTypeName = tokenTypes[index];
        tokenTypes[index] = tokenTypes[lastTokenType];
        tokenTypes.pop();
        nextTokenIdByType[lastTokenType] = 0;

        emit TokenTypeRemoved(tokenTypeName);
    }

    function getIndexOfTokenType(string calldata tokenName)
        external
        view
        returns (uint tokenTypeIndex)
    {
        uint tokenTypesLength = tokenTypes.length;

        for (uint i = 0; i < tokenTypesLength; i++)
            if (
                keccak256(abi.encodePacked(tokenName)) ==
                keccak256(abi.encodePacked(tokenTypes[i]))
            ) return i;

        revert("Not found");
    }

    function getNumberOfTokenTypes() external view returns (uint) {
        return tokenTypes.length;
    }

    function setNextTokenId(uint tokenTypeIndex) internal {
        uint length = tokenTypes.length;
        uint nextTokenId = nextTokenIdByType[tokenTypeIndex] + length;
        while (_exists(nextTokenId)) nextTokenId += (length - 1);

        nextTokenIdByType[tokenTypeIndex] = nextTokenId;
    }
}
