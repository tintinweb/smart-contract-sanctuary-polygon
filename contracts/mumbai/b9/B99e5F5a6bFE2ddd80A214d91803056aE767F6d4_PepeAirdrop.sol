...
contract PepeAirdrop is Ownable {

    IERC20 public pepeToken;
    uint256 public amount = 1000000000000000000 wei;

    constructor(address _pepeAddress) {
        pepeToken = IERC20(_pepeAddress);
    }

    function airdrop(address[] memory _wallets) external onlyOwner {
        for (uint256 i = 0; i < _wallets.length; i++) {
            pepeToken.transferFrom(owner(), _wallets[i], amount);
        }
    }
