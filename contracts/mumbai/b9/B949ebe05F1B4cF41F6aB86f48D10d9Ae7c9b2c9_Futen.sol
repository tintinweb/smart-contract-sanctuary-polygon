...
// Part: OpenZeppelin/[email protected]/Multicall
...
// Part: OpenZeppelin/[email protected]/ERC20Votes
    ...
    Multicall
{
    ...
    uint256 public constant initialSupply = 100_000_000e18;
    ...
    event GovernorChanged(address from, address to);

    constructor() ERC20('Futen', 'FUT') ERC20Permit('Futen') {
    ...
    function setGovernance(address newGovernance) public onlyRole(GOVERNOR_ROLE) {
        _revokeRole(GOVERNOR_ROLE, governance);
        _revokeRole(SNAPSHOT_ROLE, governance);
        _revokeRole(PAUSER_ROLE, governance);

        _grantRole(GOVERNOR_ROLE, newGovernance);
        _grantRole(SNAPSHOT_ROLE, newGovernance);
        _grantRole(PAUSER_ROLE, newGovernance);

        emit GovernorChanged(governance, newGovernance);
        governance = newGovernance;
    }

    function mint(address to, uint256 amount) public onlyRole(GOVERNOR_ROLE) {
        require(to != address(0), 'Futen: cannot transfer to the zero address');
        _mint(to, amount);
    }
    ...
    function rescueTokens(
        ...
        // todo: this may not be needed if deployed on polygon.
        if (token == address(0)) {
            // ether
            uint256 totalBalance = address(this).balance;
            balance = balance == 0 ? totalBalance : Math.min(totalBalance, balance);
            require(balance > 0, 'Futen: cannot transfer 0 balance');
            to.transfer(balance);
        } else {
            // erc20
