...
contract EggAuction is BaseAuctionReceiver, ReentrancyGuard {
    ...
    constructor(address accessControl, address eggToken, address coinContract, uint fees100) 
    BaseAuctionReceiver(accessControl, eggToken, coinContract, fees100) {
    }

    function weiMinPrice() public override virtual pure returns (uint) {
    ...
    whenLocked(tokenId) {
        require(amount >= priceOf(tokenId), 
            "EggAuction: incorrect amount sent to the contract");
        require(holderOf(tokenId) != _msgSender(), 
            "EggAuction: a token holder cannot buy own token. Use the method withdraw instead.");
