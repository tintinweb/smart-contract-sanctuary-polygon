...
interface IPositionV2 {
    ...
    function createPositionAndTradeV2(address tokenAddress,address targetToken, uint256 amount, address userAddress) external 
    returns(uint256,uint256, uint256,uint256, address[] memory);

    function closePositionV2(address asset, address targetAsset, uint256 targetAmount) external
    returns(uint256, uint256);

    function liquidateV2(address asset, uint256 targetAmount, address[] memory path) external 
    returns(uint256, uint256);

    function getCurrentPriceV2(address[] memory path) external view returns (uint256);
...
interface IPositionV3 {
    ...
    function createPositionAndTradeV3(address tokenAddress,address targetToken, uint256 amount, address userAddress) external 
    returns(uint256,uint256, uint256,uint256, address[] memory);

    function closePositionV3(address asset, address targetAsset, uint256 targetAmount) external
    returns(uint256, uint256);

    function liquidateV3(address asset, uint256 targetAmount, address[] memory path) external 
    returns(uint256, uint256);

    function getCurrentPriceV3(address[] memory path) external view returns (uint256);
...
import "./interface/IPositionV2.sol";
import "./interface/IPositionV3.sol";
// SPDX-License-Identifier: UNLICENSED
...
contract Position is ReentrancyGuard, Ownable, PositionStorage {
    ...
    function createPositionAndTrade(address tokenAddress, address targetToken, uint256 amount, string memory version) external{
       uint256 marginUsed;
       uint256 loanUsed;
       uint256 result;
       uint256 tradePriceAfterTrade;
       address[] memory path = new address[](2);
       if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV2")))) {
           (marginUsed, loanUsed, result, tradePriceAfterTrade, path) = IPositionV2(IAdminFunctions(adminContract).getPositionV2Contract()).createPositionAndTradeV2(tokenAddress, targetToken, amount, msg.sender);
       }
       else if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV3")))) {
            (marginUsed, loanUsed, result, tradePriceAfterTrade, path) = IPositionV3(IAdminFunctions(adminContract).getPositionV3Contract()).createPositionAndTradeV3(tokenAddress, targetToken, amount, msg.sender);
       }
    ...
    function closePosition(uint256 positionId) public {
        string memory version = swapName[positionId];
        ...
        uint256 price;
        if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV2")))) {
           (_result, price) = IPositionV2(IAdminFunctions(adminContract).getPositionV2Contract()).closePositionV2(positionInfo.asset,  positionInfo.targetAsset, positionInfo.targetAmount);
        }
        else if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV3")))) {
            (_result, price) = IPositionV3(IAdminFunctions(adminContract).getPositionV3Contract()).closePositionV3(positionInfo.asset,  positionInfo.targetAsset, positionInfo.targetAmount);
        }
    ...
    function liquidate(uint256 positionId) external {
        ...
        if(positionInfo.status == 0 && getCurrentPrice(positionId) <  positionInfo.liquidationPrice) {
             if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV2")))) {
               (_result, price) = IPositionV2(IAdminFunctions(adminContract).getPositionV2Contract()).liquidateV2(positionInfo.asset ,positionInfo.targetAmount, path);
             }
             else if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV3")))) {
                (_result, price) = IPositionV3(IAdminFunctions(adminContract).getPositionV3Contract()).liquidateV3(positionInfo.asset ,positionInfo.targetAmount, path);
    ...
    function getCurrentPrice(uint256 _positionId) public view returns (uint256) {
        string memory version = swapName[_positionId];
        ...
        uint256 price;
       if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV2")))) {
           (price) = IPositionV2(IAdminFunctions(adminContract).getPositionV2Contract()).getCurrentPriceV2(path);
       }
       else if(keccak256(abi.encodePacked((version))) == keccak256(abi.encodePacked(("uniswapV3")))) {
            (price) = IPositionV3(IAdminFunctions(adminContract).getPositionV3Contract()).getCurrentPriceV3(path);
       }
...
contract PositionStorage {
    ...
    mapping (uint256 => string) public swapName;
