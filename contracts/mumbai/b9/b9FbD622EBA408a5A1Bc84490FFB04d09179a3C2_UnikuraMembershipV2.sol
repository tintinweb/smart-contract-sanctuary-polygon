...
contract UnikuraMembershipV2 is ERC1155Upgradeable, OwnableUpgradeable {
    ...
    modifier onlyOneToken(
        ...
        if (to != owner()) {
            ...
            require(
                balanceOf(to, GOLD) == 0,
                "You already own a membership token"
            );
        }
        _;
    }
     ...
     * @param _contractUri The contract URI used for OpenSea metadata.
     */
    function initialize(
        string memory uri,
        ...
        UnikuraMembershipStorage.layout()._contractUri = _contractUri;
    }
    ...
    function ownsMembership(address account) public view returns (bool) {
        return (balanceOf(account, GOLD) > 0);
    }
    ...
    /**
     * @notice Returns the contract URI used for OpenSea metadata.
     * @dev This function is marked as `view` and does not modify the contract state.
     * @return A string representing the contract URI.
     */
    function contractURI() public view returns (string memory) {
        return UnikuraMembershipStorage.layout()._contractUri;
    }
    ...
    function setContractURI(string calldata newContractUri) external onlyOwner {
        ...
        UnikuraMembershipStorage.layout()._contractUri = newContractUri;
