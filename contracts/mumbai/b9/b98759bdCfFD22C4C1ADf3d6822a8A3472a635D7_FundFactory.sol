...
contract Fund is Ownable, ReentrancyGuard {
    ...
    uint256 public ratio; //gets divided by divisibleRatio
    uint256 public divisibleRatio = 1000000000000000000;
    uint256 public buyPercent = 10; //give as it will be divided by 1000
    address private maskerKey;
    ...
    function setBaseData(
        ...
        _end = false;
        _claim = false;
        isAdmin[_masterKey] = true;
        isAdmin[manager] = true;
        maskerKey = _masterKey;
    ...
    function updateStart(bool _isStarted)external onlyAdmin {
        _start = _isStarted;
    }

    function updateBuyPercent(uint256 _buyPercent) external {
        require(msg.sender == maskerKey, "Only admin can change this");
        require(
            _start == false,
            "Pool already started!"
        );
    ...
    function updateEnd(bool _isEnd)external onlyAdmin {
        _end = _isEnd;
    }

    function updateClaim(bool _isClaim)external onlyAdmin {
        _claim = _isClaim;
    }
    ...
    function claimableAmount(address user) external view returns (uint256) {
        uint256 restAmount = (claimable[user] * 1000) / (buyPercent + 1000);
        return (restAmount * ratio)/divisibleRatio;
    }

    function withdrawToken() external onlyOwner {
        require(_end == true, "Pool has not yet ended");
        token.safeTransfer(
            msg.sender,
    ...
    function withdrawWei(uint256 amount) public payable onlyOwner nonReentrant {
        ...
        uint256 amountWithoutInvestorFee = (amount * 1000) / (buyPercent + 1000);
        uint256 buyFee = amount - amountWithoutInvestorFee;
        require(
        ...
        .getFeeInfo();
        uint256 fee = (amountWithoutInvestorFee * feePercent) / 1000;
        uint256 restAmount = amountWithoutInvestorFee - fee;
        payable(feeRecipient).sendValue(fee + buyFee);
    ...
    function withdrawWeiToken(uint256 amount) external onlyOwner {
        ...
        uint256 fee = (amountWithoutInvestorFee * feePercent) / 1000;
        uint256 restAmount = amountWithoutInvestorFee - fee;
        IERC20(weiToken).safeTransfer(feeRecipient, fee + buyFee);
        IERC20(weiToken).safeTransfer(msg.sender, restAmount);
    }

    function claim() external {
        require(
            _end == true && _claim == true,
            "claiming not allowed yet"
        );
        require(claimable[msg.sender] > 0, "nothing to claim");

        uint256 restAmount = (claimable[msg.sender] * 1000) / (buyPercent + 1000);

        uint256 amount = (restAmount * ratio)/divisibleRatio;
    ...
    function buyWithEth() public payable {
        require(weiToken == address(0), "It's not eth-buy pool!");
        checkBeforeBuy();
        uint256 restAmount = (msg.value * 1000) / (buyPercent + 1000);
        require(restAmount >= minWei, "amount too low");
        uint256 amount = (restAmount * ratio)/divisibleRatio;
        require(
        ...
        uint256 totalClaimed = (claimable[msg.sender] * 1000) / (buyPercent + 1000);

        require(
            totalClaimed + restAmount <= maxWei,
    ...
    function buy(uint256 weiAmount) public {
        require(weiToken != address(0), "It's not token-buy pool!");
        checkBeforeBuy();
        uint256 restAmount = (weiAmount * 1000) / (buyPercent + 1000);
...
contract FundFactory is Initializable, OwnableUpgradeable, IPoolFactory {
    using SafeERC20 for IERC20;
    ...
    function createPool(uint256 poolId) external returns (address) {
        ...
        pools[poolId] = address(fund);
        isPool[address(fund)] = true;
        ...
        emit PoolCreated(poolId, address(fund), msg.sender);
