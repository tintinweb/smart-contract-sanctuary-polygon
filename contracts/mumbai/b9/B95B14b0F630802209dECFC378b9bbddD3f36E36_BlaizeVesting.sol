...
contract BlaizeVesting is OwnableUpgradeable {
    struct Beneficiary {
        uint256 amount;
    ...
    /// @dev Total amount of vesting tokens.
    ...
    /// @dev vestingToken token address.
    ...
    error InitialUnlockPercentageExceeds100(uint256 _initialUnlockPercentage, uint256 _hundredPercent);
    ...
    error VestingNotActive(uint256 start, uint256 end, uint256 current);

    /// @dev Revert if claim is not over.
    error VestingNotOver(uint256 end, uint256 current);
     ...
     */
    event VestingPeriodUpdated(uint256 indexed _startTimestamp, uint256 indexed _endTimestamp);

    event InitialUnlockPercentageUpdated(uint256 _initialUnlockPercentage);
    ...
    event VestingPeriodsUpdated(uint256 _vestingPeriods);

    /**
     * @dev Emitted when remaining tokens are withdrawn.
     */
    event RemainingVestingTokenWithdrawal(address indexed _to, uint256 indexed _amount);
    ...
    /**
     * @dev Check if the vesting is active.
     */
    modifier whenVestingActive() {
        if (!isVestingActive()) {
            revert VestingNotActive(startTimestamp, endTimestamp, block.timestamp);
        }
        _;
    }

    /**
     * @dev Check if action happens after the vesting is over.
     */
    modifier whenVestingEnded() {
        if (endTimestamp > block.timestamp) {
            revert VestingNotOver(endTimestamp, block.timestamp);
        }
        _;
    }
    ...
    function claim() external whenVestingActive onlyBeneficiary(msg.sender) {
    ...
    function initVesting(
        ...
        if (totalVestingAmount != 0 || endTimestamp != 0) revert AlreadyInitialized();
        ...
        _setInitialUnlockPercentage(_initialUnlockPercentage);

        // fill beneficiaries
        for (uint256 i = 0; i < _beneficiaries.length; i++) {
            _addBeneficiary(_beneficiaries[i], _amounts[i]);
        }

        // transfer vesting tokens to this contract
        uint256 balance = vestingToken.balanceOf(address(this));
        if (balance < totalVestingAmount) {
            uint256 amount = totalVestingAmount - balance;
    ...
    // set all variables to zero, so that the contract can be re-initialized
    function resetVesting() public onlyOwner {
        totalVestingAmount = 0;
        startTimestamp = 0;
        endTimestamp = 0;
        vestingPeriods = 0;
        initialUnlockPercentage = 0;
        vestingToken = IERC20Upgradeable(address(0));

        for (uint256 i = 0; i < beneficiariesAddresses.length; i++) {
            address beneficiary = beneficiariesAddresses[i];
            beneficiaries[beneficiary] = Beneficiary(0, 0, 0, 0);
        }
        beneficiariesAddresses = new address[](0);

        emit VestingReset();
    }

    function restartVesting(
        uint256 _startTimestamp,
        uint256 _endTimestamp,
        IERC20Upgradeable _vestingToken,
        uint256 _vestingPeriods,
    ...
    //  */
    // function setVestingToken(IERC20Upgradeable _vestingToken) external onlyOwner {
    //     _setVestingToken(_vestingToken);
    // }
    ...
    function calculateClaimAmount(
    ...
    ) public view onlyBeneficiary(_beneficiary) returns (uint256 claimAmount) {
        Beneficiary storage beneficiary = beneficiaries[_beneficiary];

        if (block.timestamp <= startTimestamp || beneficiary.amount == beneficiary.claimed) {
            return 0;
        }
        ...
        if (vestedPeriods >= vestingPeriods) {
            ...
            claimableAmount = beneficiary.initialAmount + (vestedPeriods * beneficiary.vestingAmount);
        }
        console.log(" ---- claimableAmount: %s", claimableAmount);
        claimAmount = claimableAmount - beneficiary.claimed;
        console.log(" ---- claimAmount: %s", claimAmount);
        if (claimAmount > beneficiary.amount)
    ...
    function getAllVestingData()
        ...
        returns (
            ...
            Beneficiary[] memory _beneficiariesData
        )
    {
        ...
        _beneficiariesAddresses = beneficiariesAddresses;
        _beneficiariesData = new Beneficiary[](beneficiariesAddresses.length);
        for (uint256 i = 0; i < beneficiariesAddresses.length; i++) {
            _beneficiariesData[i] = beneficiaries[beneficiariesAddresses[i]];
        }
    }

    // _______________ Internal functions _______________

    function _addBeneficiary(address _beneficiary, uint256 _amount) internal notZeroAddress(_beneficiary) {
        if (vestingPeriods == 0) revert VestingPeriodNotSet();
        if (_amount == 0) revert ZeroVestingAmount(_beneficiary);
        if (beneficiaries[_beneficiary].amount != 0) revert BeneficiaryAlreadyExists(_beneficiary);

        uint256 initialAmount = (_amount * initialUnlockPercentage) / (100 * PERCENTAGE_MULTIPLIER);

        beneficiaries[_beneficiary] = Beneficiary({
            amount: _amount,
            claimed: 0,
            initialAmount: initialAmount,
            vestingAmount: (_amount - initialAmount) / vestingPeriods
        });

        beneficiariesAddresses.push(_beneficiary);
    ...
    function _setStartTimestamp(uint256 _startTimestamp) internal {
        ...
        emit VestingPeriodUpdated(_startTimestamp, endTimestamp);
    }
    ...
    function _setEndTimestamp(uint256 _endTimestamp) internal {
        ...
        emit VestingPeriodUpdated(startTimestamp, _endTimestamp);
    }
    ...
    function _setInitialUnlockPercentage(uint256 _initialUnlockPercentage) internal {
        if (_initialUnlockPercentage > 100 * PERCENTAGE_MULTIPLIER)
            revert InitialUnlockPercentageExceeds100(_initialUnlockPercentage, 100 * PERCENTAGE_MULTIPLIER);
        initialUnlockPercentage = _initialUnlockPercentage;
        emit InitialUnlockPercentageUpdated(_initialUnlockPercentage);
    }

    function _setVestingPeriods(uint256 _vestingPeriods) internal {
        bool isMoreThanOnePeriod = _vestingPeriods > 1;
        if (!isMoreThanOnePeriod) revert LessThanTwoVestingPeriods(_vestingPeriods);
        ...
        emit VestingPeriodsUpdated(_vestingPeriods);
    }
}
