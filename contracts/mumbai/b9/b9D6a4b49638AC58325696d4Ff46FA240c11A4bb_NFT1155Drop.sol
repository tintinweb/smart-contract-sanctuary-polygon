    ...
    ReentrancyGuard
{
    ...
    function createClaim(
        ...
        uint256 m_length = _claimees.length;
        require(
            PNDC_ERC1155(_collection).balanceOf(msg.sender, _tokenId) >=
                _amount * m_length
        );
        require(_time > 0);

        // Loop through the array of claimees and create a claim for each one
        for (uint256 i = 0; i < m_length; i++) {
            address claimee = _claimees[i];

            // Check that the claimee is not the sender
            require(claimee != msg.sender);
            // Check that the claimee has less than 10 existing claims
            require(s_userClaims[claimee].length <= 10);

            // Create a new claim for the current claimee
            ...
            s_userClaims[claimee].push(m_newClaim);
            emit ClaimCreated(
                msg.sender,
                claimee,
                _tokenId,
