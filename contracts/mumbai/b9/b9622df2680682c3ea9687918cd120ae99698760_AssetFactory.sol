    ...
    */
    function claimRewards()
        ...
        returns (uint256)
        {
            ...
            for (uint256 s = 0; s < numberOfPools; s += 1){
                ...
                if (LPTokenTotalSupply >0){
                    if (poolAddress == IPTPoolAddress){
                        rawRewards = LPRewardTokenNumber.mul(4*5).mul(LPTokenBalance).div(LPTokenTotalSupply).div(numberOfPools+3).div(10);
                        
                    }
                    else{
                        rawRewards = LPRewardTokenNumber.mul(4).mul(LPTokenBalance).div(LPTokenTotalSupply).div(numberOfPools+3).div(10);
                        
                    }    
                    uint256 boostFactor = 1000 + (1500 * LPTokenTotalSupply * veISS) / (LPTokenBalance * veSupply);
            ...
            voteMachine.addRewardPointsDAO(msg.sender,veISS);
            voteMachine.addTotalRewardPointsDAO(veISS);
