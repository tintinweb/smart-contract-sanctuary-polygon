...
import "./RouterERC20UpgradableOld.sol";
...
contract ERC20SafeUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    // bytes32 public constant VOYAGER_DEPOSIT_HANDLER = keccak256("VOYAGER_DEPOSIT_HANDLER");
    // bytes32 public constant VOYAGER_EXECUTE_HANDLER = keccak256("VOYAGER_EXECUTE_HANDLER");
    // bytes32 public constant VOYAGER_UTILS = keccak256("VOYAGER_UTILS");
    // bytes32 public constant RESOURCE_SETTER = keccak256("RESOURCE_SETTER");

    // modifier onlyVoyagerHandlers() {
    //     require(
    //         hasRole(VOYAGER_DEPOSIT_HANDLER, _msgSender()) ||
    //             hasRole(VOYAGER_EXECUTE_HANDLER, _msgSender()) ||
    //             hasRole(VOYAGER_UTILS, _msgSender()),
    //         "only voyager handlers"
    //     );
    //     _;
    // }

    function __ERC20SafeUpgradeable_init() internal onlyInitializing {
        __Context_init_unchained();
        __AccessControl_init_unchained();
    }

    function __ERC20SafeUpgradeable_init_unchained() internal onlyInitializing {}

    /// @notice Used to transfer tokens into the safe to fund proposals.
    /// @notice Only callable by an address that currently has the ERC20_Handler role.
    /// @param tokenAddress Address of ERC20 to transfer.
    /// @param owner Address of current token owner.
    /// @param amount Amount of tokens to transfer.
    function fundERC20(address tokenAddress, address owner, uint256 amount) public onlyRole(ERC20HANDLER_ROLE) {
    ...
    /// @param recipient Address to transfer tokens to.
    /// @param amount Amount of tokens to transfer.
    function releaseERC20(address tokenAddress, address recipient, uint256 amount) public onlyRole(ERC20HANDLER_ROLE) {
        IERC20Upgradeable erc20 = IERC20Upgradeable(tokenAddress);
    ...
    function mintERC20(address tokenAddress, address recipient, uint256 amount) public onlyRole(ERC20HANDLER_ROLE) {
        RouterERC20UpgradableToken erc20 = RouterERC20UpgradableToken(tokenAddress);
    ...
    function burnERC20(address tokenAddress, address owner, uint256 amount) public onlyRole(ERC20HANDLER_ROLE) {
        RouterERC20UpgradableToken erc20 = RouterERC20UpgradableToken(tokenAddress);
    ...
    function _safeTransfer(IERC20Upgradeable token, address to, uint256 value) internal {
    ...
    /// @param to Address to transfer token to
    /// @param value Amount of token to transfer
    function _safeTransferFrom(IERC20Upgradeable token, address from, address to, uint256 value) internal {
        _safeCall(token, abi.encodeWithSelector(token.transferFrom.selector, from, to, value));
    }

    /// @notice used for approval of ERC20s safely
    /// @param tokenAdd Token address to transfer
    /// @param spender Address of the spender
    /// @param value Amount of token to allow
    function _safeApprove(address tokenAdd, address spender, uint256 value) internal {
        IERC20Upgradeable token = IERC20Upgradeable(tokenAdd);
        _safeCall(token, abi.encodeWithSelector(token.approve.selector, spender, value));
    }
    ...
    ERC20SafeUpgradeable
{
    ...
    function initialize(
        address _voyagerDepositHandler,
        address _voyagerExecuteHandler,
        address _voyagerUtils
        ...
        _setupRole(RESOURCE_SETTER, _msgSender());
        _setupRole(ERC20HANDLER_ROLE, _voyagerUtils);
        _setupRole(ERC20HANDLER_ROLE, _voyagerDepositHandler);
        _setupRole(ERC20HANDLER_ROLE, _voyagerExecuteHandler);
    }
    ...
    function setEthHandler(IEthHandler ethHandler) external onlyRole(RESOURCE_SETTER) {
        _ethHandler = ethHandler;
    }

    function giveAllowance(address token, address spender, uint256 amount) external onlyRole(ERC20HANDLER_ROLE) {
    ...
    //     bool _isFeeEnabled,
    //     address _feeManager
    // ) external onlyRole(VOYAGER_DEPOSIT_HANDLER) {
    ...
    /// @notice Used to stake native tokens into the LP.
    /// @dev Can only be called by ERC20Handler contract
    /// @param depositor Address of the depositor
    /// @param tokenAddress Address of the ERC20 token
    /// @param amount Amount of tokens to be staked
    ...
    /// @notice Unstake the native tokens from LP.
    /// @dev Can only be called by ERC20Handler contract.
    /// @param unstaker Address of the account who is willing to unstake.
    /// @param tokenAddress staking token of which liquidity needs to be removed.
    /// @param amount Amount that needs to be unstaked.
    ...
    function unstakeETH(
        ...
        _ethHandler.withdraw(WETH, amount, unstaker);
    ...
    function withdrawWETH(
        ...
        IWETH(WETH).transfer(address(_ethHandler), amount);
        _ethHandler.withdraw(WETH, amount, recipient);
    }

    /// @notice Sets liquidity pool owner for an existing LP.
    /// @dev Can only be set by the Voyager Utils contract
    ...
    function _setLiquidityPool(
        ...
        require(_contractToLP[contractAddress] == address(0), "pool already set");
    ...
    /// @param lpAddress Address of LP contract.
    function resetLP(address contractAddress, address lpAddress) external virtual onlyRole(RESOURCE_SETTER) {
    ...
    ERC20Upgradeable
{
    ...
    /// All two of these values are immutable: they can only be set once during
    /// construction.
    function __RouterERC20Upgradable_init(string memory name_, string memory symbol_, uint8 decimals_) internal {
        __Context_init_unchained();
        __AccessControl_init();
        __Pausable_init();
        __ERC20_init_unchained(name_, symbol_);
    ...
    function __RouterERC20Upgradable_init_unchained(
        string memory name_,
