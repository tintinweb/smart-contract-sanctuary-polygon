...
contract Signatures {
    ...
    bytes32 internal constant EIP712_NAME_HASH = keccak256("GOKDTest");
    ...
    event LogPost(address user, uint256 time);
    event LogMint(address user, uint256 time);
    ...
    constructor(uint256 _fee, address _gelatoRelay) {
        fee = _fee;
    ...
    function post(
        address user,
        DataTypes.PostData calldata _vars,
                ...
                _calculateDigest(
                    keccak256(
                        abi.encode(
                            ...
                            sig.deadline,
                            _vars.profileId,
                            keccak256(bytes(_vars.contentURI)),
                            _vars.collectModule,
                            keccak256(_vars.collectModuleInitData),
                            _vars.referenceModule,
                            keccak256(_vars.referenceModuleInitData)
                        )
                    )
                ),
                user,
        ...
        _post(_vars);
        _transfer(payable(gelatoRelay), ETH, fee);
        emit LogPost(user, block.timestamp);
    }

    function mint(address user, EIP712Signature calldata sig)
        external
        onlyGelatoRelay
    {
        unchecked {
        ...
        _mint(user);
        _transfer(payable(gelatoRelay), ETH, fee);
        emit LogMint(user, block.timestamp);
    }

    function _post(DataTypes.PostData calldata _vars) internal {
        _vars;
    }
