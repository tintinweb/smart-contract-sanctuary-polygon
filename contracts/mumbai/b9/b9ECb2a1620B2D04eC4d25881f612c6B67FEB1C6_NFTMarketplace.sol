...
contract NFTMarketplace is IERC721Receiver {
    ...
    function end(uint256 _nftId) external {
        ...
        idMarketItem[_nftId].highestBidder.bidder = payable(address(0));
        idMarketItem[_nftId].highestBidder.bidValue = 0;
        idMarketItem[_nftId].seller = payable(address(0));
        idMarketItem[_nftId].initialPrice = 0;
        idMarketItem[_nftId].endAt = 0;
    ...
    function getAllListed() external view returns (MarketItem[] memory) {
        MarketItem[] memory listed = new MarketItem[](marketItemIds.length);
        for (uint256 i = 0; i < marketItemIds.length; i++) {
            listed[i] = idMarketItem[i];
        }
        return listed;
    }
}
