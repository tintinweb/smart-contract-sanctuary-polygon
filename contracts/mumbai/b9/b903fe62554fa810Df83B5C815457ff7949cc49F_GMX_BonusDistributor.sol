...
contract GMX_BonusDistributor is GMX_IRewardDistributor, GMX_ReentrancyGuard, GMX_Governable {
    ...
    function setBonusMultiplier(uint256 _bonusMultiplierBasisPoints) external onlyAdmin {
        ...
        GMX_IRewardTracker(rewardTracker).updateRewards();
    ...
    function tokensPerInterval() public view override returns (uint256) {
        uint256 supply = GMX_IERC20(rewardTracker).totalSupply();
    ...
    function distribute() external override returns (uint256) {
        ...
        uint256 balance = GMX_IERC20(rewardToken).balanceOf(address(this));
        if (amount > balance) { amount = balance; }

        GMX_IERC20(rewardToken).safeTransfer(msg.sender, amount);
