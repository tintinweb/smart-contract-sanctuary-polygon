...
contract Item is ERC1155, Ownable{
    ...
    function uri(uint256 _tokenId) public view virtual override returns (string memory) {
        ...
        string memory _tokenURI = string(abi.encodePacked(super.uri(_tokenId), "?id=", firebaseID));
        return _tokenURI;
    }
    ...
    function mint(uint256 _fuseBlockId, address _receiver, string memory _itemUUID, uint256 _quantity, uint256 _auraAmount) public onlyFuseBlock{
        ...
        items[tokenId] = ItemInfo({itemUUID: _itemUUID, auraAmount: _auraAmount, tokenURI: uri(tokenId)});
