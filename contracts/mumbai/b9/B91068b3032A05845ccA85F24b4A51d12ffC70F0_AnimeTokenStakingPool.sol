 ...
 *              _
 *             |  0 (i = 0)
 *      ΔTi := |  the ith duration (in seconds) between init, Stake or Unstake events(i = 1 .. n-1)
 *             |_ a duration from the last event (i = n)
 *      Tk - SUM(ΔTk | i = 1..k) after inital time
 *      x(t) - staking amount at t after initial time
 ...
 * (Thm. 2) reward = x(T1)  * SUM(ΔTi / S(Ti) | i = 1..k1) * R / D
 *                   x(Tk1) * SUM(ΔTi / S(Ti) | i = k1+1.. k2) * R / D
 *                   ...+ x(Tkm) *SUM(ΔTi / S(Ti) | i = km+1 + 1..n) * R / D
 *          reward = x(T1) * RPT(k1) + x(Tk1) * (RPT(k2) - RPT(k1)) ... +  x(Tk_m) * (RPT(n) - RPT(k_m))
 *
 * Comment:
 ...
 *      RPT_k is stored as rewardPerTokenPayed[user] to compute reward using Thm. 1
 *      RPT_n is stored as rewardPerToken to compute RPT_{n+1} later
 *
 */
...
contract AnimeTokenStakingPool is Ownable {
  ...
  mapping(uint256 => uint256) public minRewardOfType;
  mapping(uint256 => uint256) public maxSerialsOfType;
  mapping(uint256 => mapping(address => bool)) public mintedByUserForType;

  // token_id serial
  mapping(uint256 => uint256) public nonceOfType;
  ...
  // Let, reward for t<=Tkv be claimed, then
  ...
  // reward + x(Tk_m) * (RPT(n) - RPT(k_m)) <= targetReward
  // RPT(n) <= (targetReward - reward) / x(Tk_m) + RPT(k_m)
  function estimateTime(uint256 _type, address account)
    internal
    view
    returns (uint256)
  {
    uint256 targetReward = minRewardOfType[_type];
    uint256 balance = _balances[account];
    uint256 reward = rewards[account];
    if (balance == 0 || targetReward <= reward) {
      return 0;
    }
    uint256 targetRPT = (targetReward - reward) /
      (balance / 1e18) +
      userRewardPerTokenPaid[account];

    return
      ((targetRPT - rewardPerToken()) * _totalSupply) /
      1e18 /
      rewardRate +
      lastUpdateTime;
  }

  function estimateDuration(uint256 _type, address account)
    external
    view
    returns (uint256)
  {
    uint256 estimatedTime = estimateTime(_type, account);
    if (estimatedTime <= block.timestamp) {
      return 0;
    }
    return estimatedTime - block.timestamp;
  }
  ...
  function claimNft(uint256 _type) private {
    // e.g.) token_id: 1_000000000001
    require(
      mintedByUserForType[_type][msg.sender] == false,
      "AnimeBankStakingPool: Sender has already claimed"
    );
    uint256 tokenId = _type * 1e9 + nonceOfType[_type];
    mintedByUserForType[_type][msg.sender] = true;
    IERC721Mintable(productNft).mint(msg.sender, tokenId);
    ++nonceOfType[_type];
  }

  function buyNft(uint256 _type) external updateReward(msg.sender) {
    require(initialize == true, "AnimeBankStakingPool: not initialized");
    require(
      nonceOfType[_type] > 0,
      "AnimeBankStakingPool: Owner has not set nonce of the type"
    );
    require(
      nonceOfType[_type] <= maxSerialsOfType[_type],
      "AnimeBankStakingPool: Sold out"
    );

    uint256 reward = rewards[msg.sender];
    uint256 minReward = minRewardOfType[_type];
    require(
      minReward <= reward,
      "AnimeBankStakingPool: Reward has not rearched to buy nft"
    );

    uint256 price = priceOfType[_type];
    require(
      price <= reward,
      "AnimeBankStakingPool: price is higher than reward"
    );

    // consume reward and claim nft
    if (price > 0) {
      rewards[msg.sender] = reward - price;
      IERC20(stakingToken).transfer(owner(), price);
  ...
  function setPriceOfType(uint256 _type, uint256 _price) external onlyOwner {
    priceOfType[_type] = _price;
  }

  function setNonceOfType(uint256 _type, uint256 _nonce) external onlyOwner {
    nonceOfType[_type] = _nonce;
  }

  function setMaxSerialsOfType(uint256 _type, uint256 _maxSerial)
    external
    onlyOwner
  {
    require(_maxSerial < 1e12, "AnimeBankStakingPool: Max Serial is too large");
    maxSerialsOfType[_type] = _maxSerial;
  }

  function setMinRewardOfType(uint256 _type, uint256 _minReward)
    external
    onlyOwner
  {
    minRewardOfType[_type] = _minReward;
  }
...
interface IAnimeTokenStakingPool {
  ...
  function estimateDuration(uint256 targetReward, uint256 balance) external;
