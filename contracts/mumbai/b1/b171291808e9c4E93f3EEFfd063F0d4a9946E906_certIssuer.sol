...
contract certIssuer{
    ...
    function addCert(string memory  _hash, string memory _aluno, string memory _matricula, string memory _curso , string memory _instituicao, string memory _data, string memory _link, string memory _cpf) public {
    ...
    function printAllCert() public view returns (Dados[] memory) {
       ...
       function certExist(string memory _hash) public view returns (uint) {
        for (uint i = 0; i <= count; i++){
            Dados storage dados = Dado[i];
            if (compareStrings(dados.Hash, _hash) == true) {
                return i; 
            } 
        }
        return count+1;
   ...
   function getNumCert() public view returns (uint) {
        ...
        function getCertByHash(string memory _hash) public view returns (Dados[] memory) {
