...
contract ERC721TokenVault is ERC20Upgradeable, ERC721HolderUpgradeable,ReentrancyGuardUpgradeable {
    ...
    function initialize(address _curator, address _token, uint256 _id, uint256 _supply, uint256 _listPrice, uint256 _fee, string memory _name, string memory _symbol,uint256 _ratio) external initializer {
...
contract ERC721VaultFactory is Ownable, Pausable {
  ...
  constructor(address _settings,address _tokenCenter,address _hsnlNFT) {
    hsnlNFT=_hsnlNFT;
    settings = _settings;
    tokenCenter = _tokenCenter;
    logic = address(new ERC721TokenVault(_settings,_tokenCenter));
    vaultCount=1;
  }
  ...
  /// @return the ID of the vault
   function mint(string memory _name, string memory _symbol, address _token, uint256 _id, uint256 _supply, uint256 _listPrice, uint256 _fee,uint256 _ratio) external whenNotPaused returns(uint256) {
    require(_token == hsnlNFT,"Error: Invalid NFT");
    
    bytes memory _initializationCalldata =
      abi.encodeWithSignature(
        "initialize(address,address,uint256,uint256,uint256,uint256,string,string,uint256)",
          msg.sender,
          _token,
          _id,
          _supply,
