...
import { IGiveawayUpgradeable } from "./interfaces/IGiveawayUpgradeable.sol";
import { IERC721Upgradeable } from "./interfaces/IERC721Upgradeable.sol";
import { TreasuryUpgradeable } from "./internal-upgradeable/TreasuryUpgradeable.sol";
import { VerifierGatewayUpgradeable } from "./internal-upgradeable/VerifierGatewayUpgradeable.sol";
...
import { NestedUniqueCheckingUpgradeable } from "./internal-upgradeable/NestedUniqueCheckingUpgradeable.sol";
...
contract GiveawayUpgradeable is
    IGiveawayUpgradeable,
    ReentrancyGuardUpgradeable,
    TreasuryUpgradeable,
    VerifierGatewayUpgradeable,
    UniqueCheckingUpgradeable,
    NestedUniqueCheckingUpgradeable
{
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor() {
        _disableInitializers();
    ...
    function initialize(
        ...
        __VerifierGateway_init_unchained(verifier_);
        __Treasury_init_unchained(assets_, vaults_);
    }

    function withdraw(Proof calldata proof_, WithdrawInput calldata withdrawInput_) external override {
        if (msg.sender != withdrawInput_.recipient) revert Giveaway__NotAuthorized();

        if (!_isValidRoot(withdrawInput_.root)) revert Giveaway__InvalidRoot();

        if (!_verifyProof(proof_, withdrawInput_)) revert Giveaway__InvalidProof();

        _setUsed(withdrawInput_.nullifierHash);

        _setAddressUsed(withdrawInput_.recipient, withdrawInput_.campaignId);

        SafeTransferLib.safeTransferFrom(
            withdrawInput_.asset,
            vaults[withdrawInput_.asset],
            withdrawInput_.recipient,
            withdrawInput_.value
        );
        require(
            IERC721Upgradeable(withdrawInput_.asset).ownerOf(withdrawInput_.value) == withdrawInput_.recipient,
            "!OWNER"
        );
...
import { IVerifierGatewayUpgradeable } from "./IVerifierGatewayUpgradeable.sol";

interface IGiveawayUpgradeable is IVerifierGatewayUpgradeable {
    error Giveaway__InvalidProof();
    error Giveaway__InvalidRoot();
    error Giveaway__NotAuthorized();
...
interface IVerifierGatewayUpgradeable {
    struct Proof {
        uint256[2] a;
        uint256[2][2] b;
        uint256[2] c;
...
abstract contract NestedUniqueCheckingUpgradeable {
    using BitMaps for BitMaps.BitMap;

    mapping(address => BitMaps.BitMap) private _isAddressUsed;

    function _setAddressUsed(address account_, uint256 uid_) internal {
        if (_isAddressUsed[account_].get(uid_)) revert AddressAlreadyUsed();
        _isAddressUsed[account_].set(uid_);
    }

    function _used(address account_, uint256 uid_) internal view returns (bool) {
        return _isAddressUsed[account_].get(uid_);
    }

    function isClaimed(address account_, uint256 uid_) external view returns (bool) {
        return _used(account_, uid_);
    }
}
...
abstract contract TreasuryUpgradeable is BaseUpgradeable {
    using BitMaps for BitMaps.BitMap;
    ...
    mapping(uint256 => CampaignInfo) public treasuries;
    mapping(address => address) public vaults;
    BitMaps.BitMap private _campaignIds;

    function __Treasury_init(address[] calldata assets_, address[] calldata vaults_) internal onlyInitializing {
        __Treasury_init_unchained(assets_, vaults_);
    }
    ...
    function setVaults(address[] calldata assets_, address[] calldata vaults_) external onlyRole(OPERATOR_ROLE) {
        _setVaults(assets_, vaults_);
    }

    function _setVaults(address[] calldata assets_, address[] calldata vaults_) private {
        uint256 length = assets_.length;
        if (length != vaults_.length) revert Treasury__LengthMismatch();

        for (uint256 i; i < length; ) {
            vaults[assets_[i]] = vaults_[i];
            unchecked {
                ++i;
            }
        }
    ...
    function setCampaigns(uint256[] calldata campaignIds_) external onlyRole(OPERATOR_ROLE) {
        uint256 length = campaignIds_.length;

        for (uint i = 0; i < length; ) {
            _campaignIds.set(campaignIds_[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function isValidCampaign(uint256 campaignId_) external view returns (bool) {
        return _campaignIds.get(campaignId_);
    }
...
import { IVerifier } from "../interfaces/IVerifier.sol";
import { IVerifierGatewayUpgradeable } from "../interfaces/IVerifierGatewayUpgradeable.sol";
import { BaseUpgradeable } from "./BaseUpgradeable.sol";
import { OPERATOR_ROLE } from "../libraries/Constants.sol";
import { BitMapsUpgradeable } from "../libraries/BitMapsUpgradeable.sol";

abstract contract VerifierGatewayUpgradeable is IVerifierGatewayUpgradeable, BaseUpgradeable {
    using BitMapsUpgradeable for BitMapsUpgradeable.BitMap;

    IVerifier public verifier;
    BitMapsUpgradeable.BitMap internal _roots;

    function __VerifierGateway_init(IVerifier verifier_) internal onlyInitializing {
        __VerifierGateway_init_unchained(verifier_);
    }

    function __VerifierGateway_init_unchained(IVerifier verifier_) internal onlyInitializing {
        _setVerifier(verifier_);
    }

    function _verifyProof(Proof calldata proof_, WithdrawInput calldata withdrawInput_) internal view returns (bool) {
        return
    ...
    function _setVerifier(IVerifier verifier_) private {
        if (address(verifier_) == address(0)) revert Verifier__ZeroAddress();
        emit VerifierUpdated(verifier, verifier_);
        verifier = verifier_;
    }

    function isValidRoot(uint256 uid_) external view returns (bool) {
        return _roots.get(uid_);
    }

    function _isValidRoot(uint256 uid_) internal view returns (bool) {
        return _roots.get(uid_);
    }
