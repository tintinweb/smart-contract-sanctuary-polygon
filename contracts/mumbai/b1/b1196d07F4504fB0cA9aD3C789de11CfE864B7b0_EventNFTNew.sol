...
contract EventNFTNew is Ownable, ERC721URIStorage {
    ...
    function getAllEventDetails() public view returns (Sale[][] memory) {
        uint length = dailyEvents.length;
    Sale[][] memory allSales = new Sale[][](length);
    
    for (uint i = 0; i < length; i++) {
        DailyEvent storage dailyEventData = dailyEvents[i];
        uint saleLength = dailyEventData.sales.length;
        Sale[] memory sales = new Sale[](saleLength);
        
        for (uint j = 0; j < saleLength; j++) {
            sales[j] = dailyEventData.sales[j];
        }
