  ...
  UUPSUpgradeable
{
  ...
  function initialize(string memory baseURI_) public initializer {
    ...
    //__PaymentSplitter_init(address(this), 100);
  ...
  /// @param amount The amount of the new token minted to the new token holder
  event TokenMinted(address indexed to, uint256 indexed id, uint256 amount);

  /// @notice Set the base URI of a token
  /// @param newuri The base token URI. For instance: https://token-cdn-domain/{id}.json
  ...
  /// @param id The type id of the token to mint
