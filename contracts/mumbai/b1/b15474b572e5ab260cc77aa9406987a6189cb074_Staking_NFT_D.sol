...
// File: contracts2/staking2_zanovo.sol
...
contract Staking_NFT_D is Ownable, ReentrancyGuard, Pausable {
    ...
    IERC20 public immutable miningEnergyToken;
    IERC20 public immutable rewardsToken;
    IERC721 public immutable nftCollection;

    uint256 constant SECONDS_IN_MINUTE = 60;
    uint256 constant MINING_ENERGY_COST = 10;
    ...
    constructor(IERC721 _nftCollection, IERC20 _rewardsToken, IERC20 _miningEnergyToken) {
        nftCollection = _nftCollection;
        rewardsToken = _rewardsToken;
        miningEnergyToken = _miningEnergyToken;
    }

    function stake(uint256[] calldata _tokenIds) external whenNotPaused {
        Staker storage staker = stakers[msg.sender];

        require(miningEnergyToken.balanceOf(msg.sender) >= MINING_ENERGY_COST, "Not enough energy tokens to start mining!");
        miningEnergyToken.safeTransferFrom(msg.sender, address(this), MINING_ENERGY_COST);
        ...
        if (staker.stakedTokenIds.length > 0) {
            ...
            staker.energyLeft = MINING_ENERGY_COST;
        }
    ...
    function calculateRewards(address _stakerAddress) private view returns (uint256) {
        ...
        uint256 energyConsumed = block.timestamp - staker.timeOfLastUpdate > staker.energyLeft ? staker.energyLeft : block.timestamp - staker.timeOfLastUpdate;
        uint256 minutesElapsed = energyConsumed / SECONDS_IN_MINUTE;
    ...
    function userStakeInfo(address _user)
        public
        view
        returns (uint256[] memory _stakedTokenIds, uint256 _availableRewards, uint256 _energyLeft)
    {
        return (stakers[_user].stakedTokenIds, availableRewards(_user), stakers[_user].energyLeft);
    }
