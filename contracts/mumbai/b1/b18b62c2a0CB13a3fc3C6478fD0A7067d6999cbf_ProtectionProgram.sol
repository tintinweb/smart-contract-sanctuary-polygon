...
contract ProtectionProgram is IProtectionProgram, OwnableUpgradeable {
    ...
    mapping(address => EnumerableSet.UintSet) private nftOwnerToNftNumbers;
    ...
    function setBankerRewardPerSecond(uint256 _bankerRewardPerSecond) external override onlyOwner {
        require(_bankerRewardPerSecond > 0, "PP: invalid bankers reward");
    ...
    function setTaxPercent(uint128 _taxPercent) external override onlyOwner {
        require(_taxPercent > 0 && _taxPercent < _getDecimals(), "PP: invalid percent value");
    ...
    function add(uint256[] calldata _nums) external override {
        require(_nums.length > 0, "PP: array is empty");
        ...
        for (uint256 i = 0; i < _nums.length; i++) {
            ...
            nftOwnerToNftNumbers[msg.sender].add(_num);

            CreatureType _creatureType = _creatureContract.getCreatureType(_num);

            if (_creatureType == CreatureType.Rebel) {
                uint256 _tenureScore = _creatureContract.getCreatureTenure(_num);
            ...
            } else if(_creatureType == CreatureType.Banker) {
    ...
    function withdraw(uint256[] calldata _nums) external override {
        _claim(_nums, true);
    }
    ...
    function getNftsByOwner(address _address, uint256 _offset, uint256 _limit) external view override returns(uint256[] memory) {
        return nftOwnerToNftNumbers[_address].part(_offset, _limit);
    }
        ...
        for (uint256 i = 0; i < _nums.length; i++) {
            if (_isNumsRebel[i]) {
                if (_isWithdraw) {
                    ...
                    nftOwnerToNftNumbers[msg.sender].remove(_nums[i]);
                }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < _nums.length; i++) {
            if (i > 0) require(_nums[i] > _nums[i - 1], "PP: invalid sequence of numbers in the array");
            ...
            if (msg.sender == nftToBankerStake[_nums[i]].owner) {
                ...
                revert("PP: nft is not on the contract or caller is not a token owner");
            }
        }
    ...
    function _calculateBankerReward(
        ...
        if (_isWithdraw)
            require(
                ...
                "PP: wait until the lockout period is over"
            );
...
interface IProtectionProgram is ICreatureType {
    ...
    /// @param _limit Nfts amount in array.
    function getNftsByOwner(address _address, uint256 _offset, uint256 _limit) external view returns(uint256[] memory);
}
...
interface ICreature is ICreatureType, IERC721EnumerableUpgradeable {
    ...
    /**
     * @notice Add base info about NFT, can be called only from `Generator` contract
     * @param _id NFT ID
     * @param _gen NFT gen
     * @param _tenure NFT tenure score
     */
    function addCreatureInfo(uint256 _id, uint8 _gen, uint8 _tenure) external;

    /**
     * @notice Return tenure score of NFT
     * @param _id NFT ID
     */
    function getCreatureTenure(uint256 _id) external view returns (uint256);

    /**
     * @notice Return NFT type of NFT
     * @param _id NFT ID
     */
    function getCreatureType(uint256 _id) external view returns (CreatureType);
}
...
interface ICreatureType {
    enum CreatureType { Undefined, Banker, Rebel }
}
...
contract Creature is ICreature, ERC721EnumerableUpgradeable, OwnableUpgradeable {
    mapping(uint256 => CreatureInfo) public creatures;

    /// @dev `Generator` contract address
    address public generatorAddress;

    /// @dev Base URI
    string public baseURI;

    uint256 public totalHumals;
    uint256 public totalBankers;
    ...
    function setGeneratorAddress(address _generatorAddress) external override onlyOwner {
        generatorAddress = _generatorAddress;
    }
    ...
    function safeMint(address _to, uint256 _id) external override {
        require(generatorAddress == msg.sender, "Creature: invalid caller");

        _safeMint(_to, _id);
    }

    function addCreatureInfo(uint256 _id, uint8 _gen, uint8 _tenure) external override {
        require(generatorAddress == msg.sender, "Creature: invalid caller");

        CreatureType _type;
        if (_tenure == 0) {
            _type = CreatureType.Banker;
            totalBankers++;
        } else {
            _type = CreatureType.Rebel;
            totalHumals++;
        }

        creatures[_id] = CreatureInfo(_gen, _tenure, _type);
    }

    function getCreatureTenure(uint256 _id) external view override returns (uint256) {
        return creatures[_id].tenure;
    }

    function getCreatureType(uint256 _id) external view override returns (CreatureType) {
        return creatures[_id].creatureType;
    }
