...
contract Prediction is Ownable, Pausable, ReentrancyGuard {
     ...
     */
    function betBear(uint256 _amount) external whenNotPaused nonReentrant notContract {
        ...
        _updateUserRounds(msg.sender, epoch);
     ...
     */
    function betBull(uint256 _amount) external whenNotPaused nonReentrant notContract {
        uint256 epoch = currentEpoch;
        require(_bettable(epoch), "Round not bettable");
        require(_amount >= minBetAmount, "Bet amount must be greater than minBetAmount");
        ...
        for (uint i; i < epochs.length; ++i) {
            ...
            if (rounds[epochs[i]].oracleCalled) {
                ...
                if (round.closePrice > round.lockPrice) {
                    // Bull wins
                    addedReward = round.rewardAmount * ledger[epochs[i]][msg.sender].bullAmount / round.rewardBaseCalAmount;
                } else if (round.closePrice < round.lockPrice) {
                    // Bear wins
                    addedReward = round.rewardAmount * ledger[epochs[i]][msg.sender].bearAmount / round.rewardBaseCalAmount;
                }
            }
    ...
    function claimable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            round.oracleCalled &&
            !betInfo.claimed &&
            ((round.closePrice > round.lockPrice && betInfo.bullAmount > 0) ||
            (round.closePrice < round.lockPrice && betInfo.bearAmount > 0));
    }
    ...
    function refundable(uint256 epoch, address user) public view returns (bool) {
        ...
        return
            ...
            (betInfo.bullAmount > 0 || betInfo.bearAmount > 0);
    }
    ...
    function getPriceFromOracle() public view returns (uint80, int256) {
        ...
        // return (uint80(currentEpoch), int256(block.timestamp));    // temp
    }
    ...
    /**
     * @notice Update userRounds
     * If it is new epoch, add it to userRounds
     */
    function _updateUserRounds(address _user, uint256 _epoch) internal {
        uint256 userRoundsLength = userRounds[_user].length;
        for (uint i; i < userRoundsLength; ++i) {
            if (userRounds[_user][i] == _epoch) {
                return;
            }
        }

        userRounds[_user].push(_epoch);
    }
