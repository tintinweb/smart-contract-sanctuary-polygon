...
contract StrangerWorlds is ERC721A, Ownable{
    ...
    uint256 public maxPresaleSupply = 3;
    uint256 public maxPublicSupply = 3;
    uint256 public maxTeamSuply = 1;
    uint256 internal maxPublicSaleMint = 2;
    uint256 internal maxMintPerTransaction = 2;
    uint256 internal maxPresaleMint = 2;
    uint256 internal tokenId;
    address team = 0x007bEf5FC4eAdCD0F8FD3d2aa656cAdbB523d341;
    ...
    function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable{
        ...
        require(addressMintedBalance[msg.sender] + _quantity <= maxPresaleMint, " Presale mint is 2 tokens only.");
        require(totalSupply() + _quantity <= maxPresaleSupply, "Max supply for presale exceeded");
        addressMintedBalance[msg.sender] += _quantity;
        _safeMint(_account, _quantity);
        if (maxPresaleSupply - totalSupply() == 0){
            isPresale = false;
        }
    }
    ...
    function publicSaleMint(address _account, uint _quantity) external payable{
        ...
        require(addressMintedBalance[msg.sender] + _quantity <= maxPresaleMint, " Public mint is 2 tokens only.");
        _safeMint(_account, _quantity);
    }
