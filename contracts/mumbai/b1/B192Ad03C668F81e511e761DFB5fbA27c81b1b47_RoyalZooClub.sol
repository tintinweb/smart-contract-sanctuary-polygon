...
contract RoyalZooClub is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public tokenPrice = 1000000000000000000; // 13 MATIC
    ...
    event RoyalZooClubMinted(uint256 tokenId, address owner);

    constructor() ERC721("RoyalZooClub", "RZClub") {}
    ...
    function withdraw() public payable onlyOwner {
     ...
     function setNftPrice(uint256 _tokenprice) external onlyOwner {
    ...
    function mintRoyalZooClub(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit RoyalZooClubMinted(id, msg.sender);
            }
        }
    }

    function presaleRoyalZooClub(uint256 numberOfTokens) external payable {
