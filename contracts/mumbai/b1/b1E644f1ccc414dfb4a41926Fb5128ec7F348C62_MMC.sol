...
contract MMC is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("mmc", "MMC") AccessControl(){
        _mint(owner, 650000 * 10 ** 18);
        ...
        coinAmountPerRewardUnit = 69 * 10 ** 18; // 69 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 41142; // 24 hours per reward unit , may be changed later on
        welcomeBonusAmount = 420 * 10 ** 18; // 420 tokens welcome bonus, only paid once per tokenId
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "MMC: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "MMC: update to zero address not possible");
