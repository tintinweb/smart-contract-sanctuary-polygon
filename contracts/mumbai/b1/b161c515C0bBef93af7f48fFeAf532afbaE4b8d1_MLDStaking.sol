...
contract MLDStaking is Ownable {
    ...
    modifier withdrawCheck(uint256 _index) {
        require(_index < userDeposits[msg.sender].length, "Invalid Index");
        _;
    }
    ...
    function _deleteUserDeposit(address _user, uint _index) internal {
    ...
    function unstake(uint256 _index) external withdrawCheck(_index) {
        ...
        _deleteUserDeposit(user, _index);
    ...
    function preMatureWithdraw(uint256 _index) external withdrawCheck(_index) {
        ...
        userTotalDeposits[user].totalMLDStaked -= stakedMLD;
        userTotalDeposits[user].totalLordStaked -= _lordStaked;
    ...
    function getUserUnclaimedId(
        address _user
    )
        public
        view
        returns (uint256[] memory maturedIds, uint256[] memory preMaturedIds)
    {
        DepositInfo memory userInfo;
        uint totalUsersDeposit = userDeposits[_user].length;
        maturedIds = new uint[](totalUsersDeposit);
        preMaturedIds = new uint[](totalUsersDeposit);
        for (uint256 i = 0; i < totalUsersDeposit; i++) {
            userInfo = userDeposits[msg.sender][i];
            if (block.timestamp >= userInfo.maturity) maturedIds[i] = i;
            else preMaturedIds[i] = i;
        }
    }
