 ...
 * @dev Contract for minting the exclusive mintpasses for Monster Racing League
 * @author Phat Loot DeFi Developers
 * @custom:version v1.0
 * @custom:date 19 May 2022
 */
contract MintpassMinter is AccessControl, Withdrawable {
    bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");

    Mintpass private immutable _mintpass = Mintpass(address(0x05017Ab278444F767B9C8D1eA90F1600A4979b3e)); // Mumbai
    IERC20 private immutable _usdc = IERC20(address(0x025208D62e86A443AE9e17B0922607Fe2A0cDF93)); // Mumbai USDC mock

    bool public mintActive = true; // TODO: Set back to false when on Polygon

    uint256 public mintingFee = 300 * 10**6; // 300 USDC (with 6 decimals)
    ...
    constructor() {
        ...
        _allowList[msg.sender] = true; // TODO: Remove on Polygon
    }
    ...
    function safeMint() public payable returns (uint256 tokenId) {
    ...
    function safeMintBatch(uint256 amount) external payable {
        if(!mintActive) revert MintNotActive();
        if(!isAllowedToMint(msg.sender)) revert NotOnTheAllowlist();
