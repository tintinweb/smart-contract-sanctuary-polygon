...
contract VlinNFTMarketplace is Pausable, Ownable, ReentrancyGuard {
    ...
    function listNft(
        ...
        uint256 _tokenId,
        uint256 _price
    ) external isVlinNFT(_nft) whenNotPaused nonReentrant {
    ...
    function buyNFT(
        ...
        (address royaltyRecipient ,uint256 royaltyFee) = nft.royaltyInfo(_tokenId, listedNft.price);
    ...
    function cancelOfferNFT(address _nft, uint256 _tokenId)
        external
        isOfferredNFT(_nft, _tokenId, msg.sender) whenNotPaused nonReentrant
    {
        ...
        nonReentrant
    {
        ...
        //IVlinNFT vlinNFT = IVlinNFT(offer.nft);
        (address royaltyRecipient, uint256 royaltyFee) = IVlinNFT(offer.nft).royaltyInfo(_tokenId, totalPrice);
    ...
    function createAuction(
        ...
        uint256 _startTime,
        uint256 _endTime
    ) external  isNotAuction(_nft, _tokenId) whenNotPaused nonReentrant{
        IERC721 nft = IERC721(_nft);
        require(nft.ownerOf(_tokenId) == msg.sender, "not nft owner");
    ...
    function cancelAuction(address _nft, uint256 _tokenId)
        external
        isAuction(_nft, _tokenId) whenNotPaused nonReentrant
    {
        AuctionNFT memory auction = auctionNfts[_nft][_tokenId];
    ...
    function bidPlace(
        address _nft,
        uint256 _tokenId
    ) external payable isAuction(_nft, _tokenId) whenNotPaused nonReentrant{
        require(
    ...
    function resultAuction(address _nft, uint256 _tokenId) external whenNotPaused {
        ...
        (address royaltyRecipient, uint256 royaltyFee) = vlinNft.royaltyInfo(_tokenId, totalPrice);
        

        //if no bids placed ????
        //add code to return NFT to Owner
