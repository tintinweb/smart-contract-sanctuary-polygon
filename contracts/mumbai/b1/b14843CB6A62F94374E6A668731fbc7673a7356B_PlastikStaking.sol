...
contract PlastikStaking is Ownable, ReentrancyGuard {
    ...
    event AddPool(uint256 _minDeposit, uint256 _maxDeposit, uint256 _limitStaking, uint64 _APR, uint128 _period);
    ...
    function initialize(uint256 _rewardPerBlock, uint256 _startBlock, uint256 _bonusEndBlock, uint256 _poolLimitPerUser, address _admin) external onlySystemAdmin {
    ...
    function addPool(uint256 _minDeposit, uint256 _maxDeposit, uint256 _limitStaking, uint64 _APR, uint128 _period) external onlySystemAdmin {        
        ...
        emit AddPool(_minDeposit, _maxDeposit, _limitStaking, _APR, _period);
    }
    ...
    function poolInformation(uint256 _pid) external view returns (uint256, uint128, uint64) {
        return (stakeAmout(_pid), poolInfo[_pid].period, poolInfo[_pid].APR);
    }

    function stakeAmout(uint256 _pid) public view returns(uint256) {
        return userInfoLongTerm[_pid][msg.sender].amount;
    }
    ...
    function totalLongTermStaked(uint256 _pid) public view returns(uint256) {
        return poolInfo[_pid].currentStaking;
    }
    function systemAdmin() external view returns(bool) {
    ...
    function earned(uint256 _pid, address _user) public view returns (uint256) {
        ...
        if (uint128(block.timestamp) >= user.timeStake.add(pool.period)) {
            ...
            return (((endUserStakedTime).mul(pool.APR)).div(SECONDS_IN_YEAR)).div(10000);
        }
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            if (pending > 0) {
                plastik.safeTransfer(address(msg.sender), pending);
            }
        }
