...
contract Comments is Ownable {
  ...
  function createRegistryTable(address tablelandRegistry) public onlyOwner returns (uint256) {
    _tableland = ITablelandTables(tablelandRegistry);
    _createRegistryTable();
    return _registryTableId;
  }

  function createCommentsTable(address tablelandRegistry) public onlyOwner returns (uint256) {
    _tableland = ITablelandTables(tablelandRegistry);
    _createCommentsTable();
    return _commentsTableId;
  }

  function addCommentSql(uint256 id, uint256 created_at, string memory authorAddress, string memory body, uint256 contract_id, uint256 token_id, uint256 parent_id) public returns (string memory) {
  ...
  function addCollectionSql(uint256 id, string memory contractAddress, uint256 chain_id, string memory collectionName, string memory collectionSymbol) public returns (string memory) {
    string.concat(
      "INSERT INTO ",
      _registryTable,
  ...
  function _createRegistryTable() internal {
    ...
    string memory tableColumns = "(id integer NOT NULL, address text NOT NULL, chain_id integer NOT NULL, name text, symbol text);";
