...
import "../coins/MentoraWellPlayedToken.sol";
...
import "../dataProviders/Price.sol";

contract Vendor is
    Ownable,
    PriceConsumerMaticDollar,
    ReentrancyGuard,
    AccessControl
{
    //EVENTOS
    ...
    event FailWithdraw(uint256 value, address sender);
    event ClainUserFailIndex(address account, uint256[] orders, uint256 value);
    event InsufficientTokens(
        uint indexed vendorBalance,
        uint indexed totalValue
    );
    ...
    //pricefortokens
    function Tokens(uint256 amount, bytes32 batch)
        public
        view
        returns (uint256)
    {
        ...
        if (batch == WL) {
            ...
            return
                (amount * SafeCast.toUint256(tokenPerMatic(BATCH2))) / 10**18;
        } else if (batch == BATCH3) {
            return
                (amount * SafeCast.toUint256(tokenPerMatic(BATCH3))) / 10**18;
        } else {
    ...
    function BuyPix(address _account, uint256 _value)
        public
        onlyRole(BUYPIXROLE)
    {
    ...
    function convertMethod(bytes32 _method)
        public
        pure
    ...
    function getAccountOrdens(address _account)
        public
        view
    ...
    function claim() public nonReentrant {
        ...
        if (vendorBalance >= amountToBuy) {
            emit InsufficientTokens(vendorBalance, amountToBuy);
        }
        require(
        ...
        bool sent = MWPToken.transfer(msg.sender, amountToBuy);
        //Caso falhe lançar um evento
        if (sent == false) {
            emit ClainUserFailIndex(msg.sender, _orders, amountToBuy);
        }
