...
contract Trident is ERC20, Ownable, ReentrancyGuard{    
  ...
  function _tokenAmount(uint256 amount_) private pure returns (uint256) {
     uint256 _amount;
      if(amount_ <= (1 * 10**17)){
          _amount = amount_;
          ...
          _amount = amount_ * 3;
      } else if(amount_ > (1*10**18)){	
          _amount = amount_ * 3 + amount_ * (amount_ / 10**18) * 75 / 100;
      }
