...
contract FlipDropsV2 is ERC721Base, LazyMint, Drop {
    ...
    ERC721Base("FlipDropsV2", "FKD2", _royaltyRecipient, _royaltyBps){}
    ...
    function _beforeClaim(
        ...
        require(presentInAllowList(_receiver), "You are not in the allow list");
    }

    function addToAllowList(address _address) public {
        if(!presentInAllowList(_address)) allowList.push(_address);
    }

    function presentInAllowList(address _address) public view returns (bool){
        for (uint i = 0; i < allowList.length; i++) {
            if (_address == allowList[i]) return true;
        }
        return false;
    }
