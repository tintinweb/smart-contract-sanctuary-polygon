...
import {LRaceUtils} from "./LRaceUtils.sol";

// Define our contract, inheriting from multiple other contracts including Ownable and UUPSUpgradeable
    ...
    IERC165
{
    ...
    event LogFeeData(address indexed router, uint256 fee, uint256 blance);

    // Can consider making mutable up until mainnet.
    IRouterClient public router;

    // Selector for the destination chain (in the CCIP protocol)
    uint64 destinationChainSelector;

    // Mapping to hold token mappings between chains
    mapping(address => address) public tokenMapping;

    mapping(uint64 => bytes) public s_chains;

    // Current feeToken
    IERC20Upgradeable public s_feeToken;

    event TransferCompleted(bytes32 messageId);
    event PayFeeContext(LRaceUtils.SPayFeeContext payFeeContext);
    ...
    function setFeeToken(address _feeToken) public onlyOwner {
        s_feeToken = IERC20Upgradeable(_feeToken);
    }
    ...
    function ccipReceive(
        ...
        // @todo receive data as LRaceUtils.SClaimRewardReqBody
        LRaceUtils.SPayFeeContext memory sPayFeeContext = decodeData(
            message.data
        );
        emit PayFeeContext(sPayFeeContext);
        emit MessageReceived(message.messageId);
    }

    // // 1 ccip send with ERC20
    // // want to do: USDC move to Avalanche from Polygon
    // function transferTokenPayableWithSwap(
    //     address receiver,
    //     address token,
    //     uint256 amount,
    //     uint64 destChainSelector
    // ) public payable {
    //     // @todo exchange msg.value to ERC20(USDC) by UniSwap
    //     // something
    //     // transferToken();
    // }

    // 2 ccip send with native token (optional. if possible)
    // want to do: Native token(MATIC) move to Avalanche from Polygon
    function transferTokenPayable(
        ...
        // something
        // @todo send data with msg.value to another chain
        // transferToken();
    }

    function decodeData(
        bytes memory data
    ) internal pure returns (LRaceUtils.SPayFeeContext memory) {
        (
        ...
        LRaceUtils.SPayFeeContext memory payFeeContext = LRaceUtils
            .SPayFeeContext(raceId, replicantId, strategy, exchangeRate);
        return payFeeContext;
    }
    ...
    function transferTokenPayableWithSwap(
        ...
        address token,
        uint256 amount,
        uint64 destChainSelector // LRaceUtils.SClaimRewardReqBody memory context
    ) public payable {
        // @todo exchange msg.value to ERC20(USDC) by UniSwap
        // something
        // transferToken();
    ...
    function transferTokenInternal(
        ...
        IERC20Upgradeable sourceToken = IERC20Upgradeable(token);
        require(
            sourceToken.balanceOf(msg.sender) >= amount,
        ...
        tokenAmounts[0] = Client.EVMTokenAmount(address(token), amount);
        ...
        IERC20Upgradeable feeToken = IERC20Upgradeable(address(s_feeToken));
        emit LogFeeData(address(router), fee, feeToken.balanceOf(msg.sender));
        ...
        emit TransferCompleted(messageId);
    }

    function transferTokenWithPayFeeContext(
        address receiver,
        address token,
        uint256 amount,
        uint64 destChainSelector,
        LRaceUtils.SPayFeeContext memory context
    ) public payable {
        bytes memory data = abi.encode(context);
        transferTokenInternal(receiver, token, amount, destChainSelector, data);
    }
    ...
    modifier validChain(uint64 chainSelector) {
        require(s_chains[chainSelector].length > 0, "Invalid chain");
        _;
    }
