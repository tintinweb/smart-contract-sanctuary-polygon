...
contract Operations is VRFConsumerBaseV2, ERC2771Recipient {
    ...
    mapping(uint256 => BetSlip) private s_betSlipIdToBetSlip;
    ...
    function placeBet(
        ...
        uint256 totalStake,
        uint256 totalPayout,
        bytes32[6] calldata betDetails,
        bool callOracle
    ) public {
        if (s_placeBetKey[betDetails[5]] == true) {
        ...
        s_betSlipId++;
        s_placeBetKey[betDetails[5]] = true;
        ...
        s_betSlipIdToBetSlip[s_betSlipId] = BetSlip(
            s_betSlipId,
            _msgSender(),
            token,
            totalStake,
    ...
    function settleBet(
        ...
        uint256 stake,
        uint256 payout,
        bytes32[] calldata betInfo
    ) public onlySettleSigner {
        BetSlip storage betSlip = s_betSlipIdToBetSlip[betSlipId];
        if (betSlipId != betSlip.betSlipId || betSlipId == 0) revert Operations__InvalidBetId();
        betSlip.allBetsStatus = betInfo[3];
        if (settleStatus == 1) {
    ...
    function getBet(uint256 betId) public view returns (BetSlip memory) {
        return s_betSlipIdToBetSlip[betId];
    }
}
//