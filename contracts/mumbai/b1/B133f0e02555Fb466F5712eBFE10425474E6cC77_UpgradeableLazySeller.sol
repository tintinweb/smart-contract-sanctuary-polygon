...
import "./AuthVoting.sol";

contract UpgradeableLazySeller is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, EIP712("Lazy-Domain", "1"), AccessControlUpgradeable, Upgradeable, AuthVoting {
    struct ListingVoucher {
        uint listingId;
        uint price;
        string uri;
        bytes signature;
    ...
    ListingVoucher[] public vouchers;
    ...
    function initialize(address payable _admin, address payable seller, string memory _collectionName) initializer public {
        __ERC721_init("ListingToken", "LST");
    ...
    function mintNFT(address _to, ListingVoucher calldata _voucher) public {
        address signer = _verify(_voucher);
        if(hasRole(MINTER_ROLE, signer)){
            
            _safeMint(_to, _voucher.listingId);
            _setTokenURI(_voucher.listingId, _voucher.uri);

            createBallot(vouchers.length + 1);
            vouchers.push(_voucher);
    ...
    function _hash(ListingVoucher calldata voucher) internal view returns(bytes32){
    ...
    function _verify(ListingVoucher calldata voucher) internal view returns(address){
    ...
    // Voting functions
    function createBallot(uint proposalIndex) public override{}

    function voteFor(address payable authenticator, uint index) public override{}

    function voteAgainst(address payable authenticator, uint index) public override{}

    function verifiedAuthenticator(address payable authenticator) public view override returns(bool) {}

    // Only allow this statement to be carried out when there are enough votes in favor of 
    // authenticating. Can be called automatically when the condition is met. 
    function authenticate(uint index, bool verdict) public override{}
}
