 ...
 */
    ...
    event NewAccountCreated(string identity, address indexed  _owner, uint256 tokenId);
    ...
    function setAuthorisation(bytes32 profileId, address target, bool _isAuthorised) external {
    ...
    function createNewAccount(string memory accountName) public returns (AccountInfo memory) {
        AccountInfo memory Info = _createNewAccount(msg.sender);
        setOptionalName(msg.sender, Info.tokenId, accountName);
        return Info;

    }

    function createNewAccount() public returns (AccountInfo memory) {
        AccountInfo memory Info = _createNewAccount(msg.sender);
        return Info;
    }

    function _createNewAccount(address accountOwner) internal returns (AccountInfo memory) {
        uint256 tokenId = _register(accountOwner, true);
        string memory _ident = abi.decode(abi.encodePacked(tokenId, accountOwner), (string));
        setIdent(tokenId, _ident);
        bytes32 metasId = setNewMetaDatas(tokenId, accountOwner);
        AccountInfo memory Info = setAccountInfoRecord(tokenId, _ident, accountOwner, metasId);
        emit NewAccountCreated(_ident, accountOwner, tokenId);
    ...
    function myAccountInfo() external view returns(AccountInfo memory Info) {
        Info = _info[msg.sender];
    }
    ...
    returns (bool)
    {
        return (interfaceID == ERC721_ID || 
        interfaceID == RECLAIM_ID || 
        interfaceID == REPUTATION_ID || 
        interfaceID == IDENT_INTERFACE_ID ||
        interfaceID == METADATA_INTERFACE_ID || interfaceID == METAVALUE_INTERFACE_ID ||
        interfaceID == NAME_INTERFACE_ID ||
        interfaceID == PROFILE_INTERFACE_ID ||
        super.supportsInterface(interfaceID));
    }
...
abstract contract BaseUserRegistry is ERC721, Ownable {
    ...
    constructor(bytes32 rootNode, address _uds) ERC721("AKX3 ECOSYSTEM USER","AKXU"){
        uds = UDS(_uds);
    ...
    function register(address _owner) internal returns (uint256) {
        uint tokenId = index++;
        return _register(_owner, true);
    }


    function _register(address _owner, bool update) internal  returns (uint256) {
        uint tokenId = index++;
...
abstract contract IdentResolver is BaseResolver {
   ...
   mapping(uint256 => string) private _idents; // external user identifier for ie database
    ...
    function setIdent(uint256 tokenId, string memory identifier) public {
