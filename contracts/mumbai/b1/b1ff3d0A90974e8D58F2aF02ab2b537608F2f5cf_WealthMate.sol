...
contract WealthMate {
  ...
  // State Variables
  address public bankOwner; // address of the owner of the bank
  string public bankName; // name of the bank
  ...
  event Withdraw(address indexed customerAddress, uint withdrawAmount, uint newBalance); // emitted when a customer withdraws tokens
  event Transfer(address from, address to, uint transferAmount); // emitted when a customer transfers tokens to another account

  // MODIFIERS
  modifier onlyBankOwner() { // modifier to restrict certain functions to only be callable by the owner of the bank
    require(msg.sender == bankOwner, "You're not the owner of the bank"); _;
  }

  // Constructor
  constructor() {
    bankOwner = msg.sender; // set the owner of the bank to the address that deployed the contract
  }
  ...
  function depositToken() external payable returns (uint) {
    ...
    customerBalances[msg.sender] = customerBalances[msg.sender].add(msg.value);
  ...
  function withdrawToken(uint withdrawAmount) external returns(uint) {
    ...
    customerBalances[msg.sender] = customerBalances[msg.sender].sub(withdrawAmount);
  ...
  function transferToken(address to, uint transferAmount) external payable returns(bool) {
    ...
    // Use SafeMath's sub function to prevent underflow
    customerBalances[msg.sender] = customerBalances[msg.sender].sub(transferAmount);
    ...
    customerBalances[to] = customerBalances[to].add(transferAmount);
