...
contract InsideOut is InsideOutInterface {
  ...
  /**
   * @dev Whitelist of addresses that can pre-mint.
   */
  ...
  constructor(string memory _initBaseURI, string memory _initNotRevealedURI)
    payable
    InsideOutInterface(_initBaseURI, _initNotRevealedURI)
  {}
   ...
   */
  function getAllWhitelist() external returns (address[] memory) {
  ...
  /**
   * @dev Allows a new address to be whitelisted.
   * @param _newWhitelistAddresses The list of addresses to whitelist.
   */
  function addWhitelistAddresses(address[] memory _newWhitelistAddresses)
    external
    onlyOwner
  {
    if (whitelistedAddresses.length > 0) {
      address[] memory tempWhitelistAddresses = whitelistedAddresses;
      for (uint256 i = 0; i < tempWhitelistAddresses.length; i++) {
        whitelisted[tempWhitelistAddresses[i]] = false;
      }
    }

    for (uint256 i = 0; i < _newWhitelistAddresses.length; i++) {
      whitelisted[_newWhitelistAddresses[i]] = true;
    }

    whitelistedAddresses = _newWhitelistAddresses;
  }

  /**
   * @dev Toggles the mint to be whitelist mint only.
   */
  function turnOffPremint() external onlyOwner {
    isPremint = false;
  }
  ...
  /**
   * @dev Updates the base URI to the original image.
   */
  ...
  function withdrawFunds() external payable onlyOwner {
    // 0xF83C26B36f0a64aD1444f932D3145E8cd8396bE2 // Daniel
    (bool os, ) = payable(address(0xF3bD1bc39dB223b6F33F27728C861350239bA5F3)) //test
