...
contract STUDIOMARKET is ReentrancyGuard, ERC721URIStorage, Ownable {
  ...
  constructor() ERC721('X-STUDIO Final', 'XSTF') {}
  ...
  /* Updates the trading price of the contract */
  function updateTradingPrice(uint256 _tradeFee) public onlyOwner {
    //   require(owner == msg.sender, "Only marketplace owner can update listing price.");
    tradeFee = _tradeFee;
  }

  /* Returns the listing price of the contract */
  function getTradeingPrice() public view returns (uint256) {
    return tradeFee;
  }
  ...
  function createMarketItem(
    ...
    idToMarketItem[tokenId] = MarketItem(tokenId, payable(msg.sender), payable(address(this)), payable(msg.sender), price, royalty, false);

    _transfer(msg.sender, address(this), tokenId);
    emit MarketItemCreated(tokenId, msg.sender, address(this), msg.sender, price, royalty, false);
  }
  ...
  function takeCommission(
    ...
    uint256 royaltyFee = (amountPaid * royaltyPercent) / 100;
  ...
  function resellToken(uint256 tokenId, uint256 price) public payable nonReentrant {
    ...
    require(price > 0, 'New Price must be at least 1 wei');
    if (listingPrice > 0) {
      require(msg.value == listingPrice, 'Price must be equal to listing price');
    }
  ...
  function createMarketSale(uint256 tokenId) public payable {
    ...
    takeCommission(seller, creator, owner(), price, tradeFee, royaltyFee);
  }
