...
abstract contract OnChainMetadata {
    struct Metadata {
        ...
        mapping(bytes32 => bytes[]) data;           // key => value
        mapping(bytes32 => uint256) valueCount;     // key => number of value
    }
    ...
    /**
     * @dev Get the value of token metadata key
    ...
    /**
     * @dev Get the first value of token metadata key
     * @param tokenId the token identifier
     * @param key the token metadata key
     */
