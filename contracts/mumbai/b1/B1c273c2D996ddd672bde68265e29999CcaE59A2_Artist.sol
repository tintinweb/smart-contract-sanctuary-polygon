...
contract Artist is ERC721Upgradeable, IERC2981Upgradeable, OwnableUpgradeable {
    ...
    // platform address => platformFee
    mapping(address => uint256) public depositPlatformFee;
    ...
    function buyEdition(uint256 _editionId) external payable {
        ...
        require(msg.value % 100 == 0, "minimum ammount paid is 100wei");
        ...
        uint256 platformFee = (msg.value *
            editions[_editionId].commissionPercentage) / 100;

        uint256 funding = msg.value - platformFee;

        depositedForEdition[_editionId] += funding;

        depositPlatformFee[platform] = platformFee;
    ...
    function withdrawPlatformFee(address payable _platform) public {
        _platform.transfer(depositPlatformFee[_platform]);
    }
...
// File @openzeppelin/contracts/pr
