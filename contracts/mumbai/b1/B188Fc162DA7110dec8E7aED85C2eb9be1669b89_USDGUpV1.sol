    ...
    UUPSUpgradeable
{
    ...
    function deposit(uint256 _sendAmount, uint8 _fieldIndex) external {
        ...
        //need approve first
        USDT.safeTransferFrom(msg.sender, address(this), _sendAmount);
    ...
    function createOrder(
        ...
        uint256 _orderRevenue = (_sendAmount *
            revenueRate *
            (_fieldIndex + 1)) / 10;
        uint256 _releasPerDay = _orderRevenue /
    ...
    function createMarketOrder(
        ...
        uint256 _orderRevenue = (_depositValue *
            revenueRate *
            (_fieldIndex + 1)) / 10;
        uint256 _releasPerDay = _orderRevenue /
            (baseReleaseDays * (_fieldIndex + 1));
        ...
        for (uint256 _index = _startIndex; _index < _endIndex + 1; _index++) {
            ...
            if (orderToSwap.state == 0) {
                ...
                uint256 _swapValue = orderToSwap.swapValue -
                    _swapFeeForPlatform -
                    _swapFeeForParent;
                USDT.transfer(_applier, _swapValue);
