...
interface IAdminIdentityInit is IIdentity{
    ...
    function setSigRequirement(uint256 _sigReq) external;
    function setExecutionExpiry(uint256 _expiry) external;
    function verifyApproval(bytes32 _key, bytes32[] memory _approvals) external pure returns(bool);
}
...
contract TokenFactory is AccessControl, Initializable{
    ...
    function setImplementation (
        address _identityRegistry, address _claimTopicsRegistry,
         ...
         address _implAuthorityAdminIdentity) public onlyRole(Owner_Role){
    ...
    function deployAdminIdentity(address _initialManagementKey, uint256 executionBuffer, uint256 reqSig, address tokenizationAgent , address _issuer, address _token) public onlyRole(Owner_Role) returns (address) {
        address adminIdentityProxy = address(new IdentityProxy());

        (bool success,) = adminIdentityProxy.call(abi.encodeWithSelector(0x3659cfe6, implAuthorityAdminIdentity));
        require(success," Upgrade failed");
        success = false;
    
        (success,) = adminIdentityProxy.call(abi.encodeWithSelector(0x7a1ac61e, address(this), executionBuffer, reqSig));
        ...
        MultiSigContracts[_token] = adminIdentityProxy;
    ...
    function createToken(
        ...
        address _issuer, address _transferAgent, address _tokenizationAgent) external onlyRole(Owner_Role){
    ...
    function setRolesOnToken(address _userAddress, bytes32[] memory roles, address _token) public onlyRole(Owner_Role){
        IToken token = IToken(_token);
        token.setRolesToTokenAgent(_userAddress, roles);
    }

    function revokeRolesOnToken(address _userAddress, bytes32[] memory roles, address _token) public onlyRole(Owner_Role){
        IToken token = IToken(_token);
    ...
    function addMultiSigSigner(address _signer, address _token)public onlyRole(Owner_Role){
        address multiSig = getMultiSig(_token);
    ...
    // function removeMultiSigSigner(address _signer, address _token)public onlyRole(Owner_Role){
    //     address multiSig = getMultiSig(_token);
    //     IAdminIdentityInit(multiSig).addKey(keccak256(abi.encode(_signer)), 2, 1);
    // }

    function setSigRequirementOnMultiSig(uint256 _sigReq, address _token) public onlyRole(Owner_Role){
        address multiSig = getMultiSig(_token);
        IAdminIdentityInit(multiSig).setSigRequirement(_sigReq);
    }

    function getMultiSig(address _token) public view returns(address){
        return MultiSigContracts[_token];
    }
