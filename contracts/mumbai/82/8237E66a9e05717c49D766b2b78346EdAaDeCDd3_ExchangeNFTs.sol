...
contract ExchangeNFTs is IExchangeNFTs, ERC721HolderUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize(address _config, address _owner) public initializer {
    ...
    function readyToSellTokenTo(
        ...
        require(_msgSender() == IERC721Upgradeable(_nftToken).ownerOf(_tokenId), 'A1'); // Only Token Owner can sell token
        require(_price != 0, 'P1'); // Price must be granter than zero
    ...
    function setCurrentPrice(
        ...
        require(_price != 0, 'P2'); // Price must be granter than zero
        _asksMaps[_nftToken][_quoteToken].set(_tokenId, _price);
        emit Ask(_nftToken, _msgSender(), _tokenId, _quoteToken, _price);
    }
    ...
    function cancelSellToken(address _nftToken, uint256 _tokenId) public override nonReentrant {
        require(tokenSellers[_nftToken][_tokenId] == _msgSender(), 'A3'); // Only Seller can cancel sell token
    ...
    function bidTokenTo(
        ...
        config.checkEnableTrade(_nftToken, _quoteToken);
        require(_price != 0, 'P3'); // Price must be granter than zero
        require(_asksMaps[_nftToken][_quoteToken].contains(_tokenId), 'Token not in sell book');
    ...
    function updateBidPrice(
        ...
        require(_price != 0, 'P4'); // Price must be granter than zero
    ...
    function sellTokenTo(
        ...
        config.checkEnableTrade(_nftToken, _quoteToken);
        require(_asksMaps[_nftToken][_quoteToken].contains(_tokenId), 'Token not in sell book');
        require(tokenSellers[_nftToken][_tokenId] == _msgSender(), 'A5');  // Only Bidder can update the bid price
    ...
    function cancelBidToken(
        ...
        require(_userBids[_nftToken][_quoteToken][_msgSender()].contains(_tokenId), 'A6'); // Only Bidder can cancel the bid
        // find  bid and the index
