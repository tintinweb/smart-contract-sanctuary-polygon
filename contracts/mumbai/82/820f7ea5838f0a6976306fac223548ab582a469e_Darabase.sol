...
contract Darabase is NFTContract{
    ...
    function renewRental(
        ...
        require(rental.startTime<rental.endTime, "end time should be greater than start time");
        // validateSellSignature(sellOrder, sellSignature);

        Rental memory rentals = getRentalDetails(pdrId, rental.rentalId);
        uint256 numberOfRentals = rentals.amount;
        ...
        uint remaining = doPlatformPayment(msg.sender, totalPayment, sellOrder.payment, platformPayment);
        doPaymentTransfer(msg.sender, remaining, sellOrder.maker, sellOrder.payment, true);
        updateRental(pdrId, rental.rentalId, rental.startTime, rental.endTime);
