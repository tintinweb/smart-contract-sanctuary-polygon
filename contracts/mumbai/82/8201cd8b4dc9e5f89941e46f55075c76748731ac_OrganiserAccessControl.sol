...
contract OrganiserAccessControl is IOrganiserAccessControl, AccessControl {
    ...
    modifier onlyAdmin() {
        if (!this.isAdminRole(msg.sender)) {
    ...
    modifier onlyAdminOrOganiserApprover() {
        if (!this.isAdminRole(msg.sender) && !this.isOrganiserApproverRole(msg.sender)) {
            revert OnlyAdminOrOrganiserApproverAllowed(msg.sender);
        }
        _;
    }
    ...
    modifier checkOrganiserNotExist() {
        if (this.isOrganiserExist(msg.sender)) {
            revert OrganiserExist(msg.sender);
        }
        _;
    }
    ...
    function isOrganiserExist(address organiser) external view override returns (bool) {
        return organisers[organiser] == Status.NONE ? false : true;
    }

    function addAdminRole(address account) external override onlyAdmin {
        grantRole(DEFAULT_ADMIN_ROLE, account);
    }

    function addOrganiserApproverRole(address account) external override onlyAdmin {
    ...
    function approveOrganiser(address organiser) external override onlyAdminOrOganiserApprover {
        if (organisers[organiser] != Status.PENDING) {
            revert OrganiserNotInPendingStatus(organiser);
        }
    ...
    function rejectOrganiser(address organiser) external override onlyAdminOrOganiserApprover {
        if (organisers[organiser] != Status.PENDING) {
            revert OrganiserNotInPendingStatus(organiser);
        }
        organisers[organiser] = Status.BLOCKED;

        emit ChangeOrganiserStatus(organiser, msg.sender, Status.PENDING, Status.BLOCKED);
    }

    function blockOrganiser(address organiser) external override onlyAdminOrOganiserApprover {
        if (organisers[organiser] != Status.APPROVED) {
            revert OrganiserNotInApprovedStatus(organiser);
        }
        organisers[organiser] = Status.BLOCKED;
    ...
    function unblockOrganiser(address organiser) external override onlyAdminOrOganiserApprover {
        if (organisers[organiser] != Status.BLOCKED) {
            revert OrganiserNotInBlockedStatus(organiser);
        }
        organisers[organiser] = Status.APPROVED;
...
interface IOrganiserAccessControl is IAccessControl {
    ...
    /// @param account The account address that is checked
    error OnlyAdminAllowed(address account);

    /// @dev Error thrown when only admin or organiser approver allowed to execute the transaction
    /// @param account The account address that is checked
    error OnlyAdminOrOrganiserApproverAllowed(address account);

    /// @dev Error thrown when organiser already exist
    /// @param organiser The address of the organiser that is checked
    error OrganiserExist(address organiser);

    /// @dev Error thrown when organiser is not in pending status
    /// @param organiser The address of the organiser that is checked
    error OrganiserNotInPendingStatus(address organiser);

    /// @dev Error thrown when organiser is not in approved status
    /// @param organiser The address of the organiser that is checked
    error OrganiserNotInApprovedStatus(address organiser);

    /// @dev Error thrown when organiser is not in blocked status
    /// @param organiser The address of the organiser that is checked
    error OrganiserNotInBlockedStatus(address organiser);

    /// @dev Get organiser status
    /// @param account Organiser address for which is getting status
    function organisers(address account) external view returns (Status);

    /// @dev Create a bool check to see if a account address has the role admin
    function isAdminRole(address account) external view returns (bool);

    /// @dev Create a bool check to see if a account address has the role organiser approver
    function isOrganiserApproverRole(address account) external view returns (bool);

    /// @dev Check if the organiser does not exist
    function isOrganiserExist(address organiser) external view returns (bool);

    /// @dev Add a user address as a admin
    function addAdminRole(address account) external;

    /// @dev Add a user address as a organiser approver
    function addOrganiserApproverRole(address account) external;

    /// @dev Apply for approve
    function applyForApprove() external;

    /// @dev Approve organiser that could create events
    function approveOrganiser(address organiser) external;

    /// @dev Reject organiser (it's not possible to create new events)
    /// @param organiser The organiser address that is rejecting
    function rejectOrganiser(address organiser) external;

    /// @dev Block organiser (it's not possible to create new events)
    /// @param organiser The organiser address that is blocking
    function blockOrganiser(address organiser) external;

    /// @dev Unblock organiser
    /// @param organiser The organiser address that is unblocking
