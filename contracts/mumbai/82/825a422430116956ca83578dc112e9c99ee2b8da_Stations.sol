...
contract Stations is Initializable, OpenSeaCollection, ERC1155PausableUpgradeable, WheelcoinPausable {
    ...
    modifier requireValidMetadata(MetadataParams memory _params) {
        require(_params.valid(), "Stations: invalid metadata provided");
        _;
    }
...
contract OpenSeaCollection is Initializable, OwnableUpgradeable, WheelcoinRoles, IERC2981Upgradeable {
    ...
    modifier nonEmptyString(string memory _input) {
        require(bytes(_input).length != 0, "OpenSeaCollection: can't be empty");
        _;
    }

    function __OpenSeaCollection_init(ContractParams memory _params) public onlyInitializing {
        require(_params.valid(), "OpenSeaCollection: invalid contract params");
    ...
    function setName(string memory _name) public onlyRole(ADMIN_ROLE) nonEmptyString(_name) {
        contractMetadata.name = _name;
    }

    function setDescription(string memory _description) public onlyRole(ADMIN_ROLE) nonEmptyString(_description) {
        contractMetadata.description = _description;
    }

    function setImage(string memory _image) public onlyRole(ADMIN_ROLE) nonEmptyString(_image) {
        contractMetadata.imageURL = _image;
    }

    function setFeeRecipient(address _recipient) public onlyRole(ADMIN_ROLE) {
        require(_recipient != address(0), "OpenSeaCollection: can't be empty address");
        contractMetadata.feeRecipient = _recipient;
    }

    function setSellerFeeBasisPoints(uint256 _fee) public onlyRole(ADMIN_ROLE) {
        require(_fee != (0), "OpenSeaCollection: can't be 0");
...
library StationsMetadataGenerator {
    ...
    function _webURIData(bytes memory _json) internal pure returns (string memory) {
        return string(abi.encodePacked("data:application/json;base64,", Base64.encode(_json)));
    }

    function _jsonMetadata(MetadataParams memory params) internal pure returns (bytes memory) {
    ...
    function _propertiesString(MetadataProps memory props) internal pure returns (string memory) {
...
library ContractMetadataGenerator {
    ...
    //TODO: revert instead of return bool(?)
    function valid(ContractParams memory _params) internal pure returns (bool) {
        ...
        if (bytes(_params.imageURL).length == 0) {
            return false;
        }
    ...
    function _contractMetadata(ContractParams memory params) internal pure returns (bytes memory) {
        bytes memory _json = abi.encodePacked(
            "{",
            '"name":"',
            params.name,
