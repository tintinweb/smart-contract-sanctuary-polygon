...
contract PolygonBSG {
    ...
    uint256 public boosterIncomePool;
    ...
    function _distributeDeposit(uint256 _amount) private {
        ...
        uint256 diamondpoolIncome = _amount.mul(diamondIncomePoolPercents).div(baseDivider);
        diamondIncomePool = diamondIncomePool.add(diamondpoolIncome);
        uint256 more1kPool = _amount.mul(more1kIncomePoolPercents).div(baseDivider);
        more1kIncomePool = more1kIncomePool.add(more1kPool);
        boosterIncomePool = boosterIncomePool.add(more1kIncomePool);
    }
    ...
    function _calCurStaticRewards(address _user) public view returns(uint256, uint256) {
        ...
        uint256 _capital = userRewards.capitals;
        uint256 totalRewards;
        if(getMember(_user) > 0){
            totalRewards = _capital.mul(boostercycleRewardPercents).div(baseDivider);
        } else {
            totalRewards = _capital.mul(normalcycleRewardPercents).div(baseDivider);
        }

        if(_capital.mul(limitProfit).div(baseDivider) > userInfo[_user].totalRevenue) {
            totalRewards = 0;
        }
    ...
    function _calCurDynamicRewards(address _user) public view returns(uint256, uint256) {
        ...
        if(_member >= 2){
            ...
            teamReward = getActiveDirectReward(_user);
        }
    ...
    function transferBylockusdt(address _receiver, uint256 amount) external {
        uint256 _amount = amount.mul(1e6);
        require(_amount >= minDeposit.div(2) && _amount.mod(minDeposit.div(2)) == 0, "amount err");
    ...
    function getActiveDirectNumbers(address _user) public view returns(uint256) {
    ...
    function getActiveDirectReward(address _user) public view returns(uint256) {
        address[] storage _teams = myTeamUsers[_user];
        uint256 teamLength = _teams.length;
        uint256 teamReward = 0;
        OrderInfo[] storage myOrderList = orderInfos[_user];
        uint256 _mylastAmount = 0;
        if(myOrderList.length > 0){
            _mylastAmount = myOrderList[myOrderList.length - 1].amount;
        }
        for (uint256 i = 0; i < teamLength; i++) {
            OrderInfo[] storage _order = orderInfos[myTeamUsers[_user][i]];
            if(_order.length > 0){
                ...
                if(_deadline > block.timestamp){
                    //active
                    uint256 _teammemberamount = _finalOrder.amount > _mylastAmount ? _mylastAmount : _finalOrder.amount;
    ...
    function getActiveDirectDeposit(address _user) public view returns(uint256) {
        address[] storage _teams = myTeamUsers[_user];
        uint256 teamLength = _teams.length;
        uint256 teamTotalDeposit = 0;
        for (uint256 i = 0; i < teamLength; i++) {
            OrderInfo[] storage _order = orderInfos[myTeamUsers[_user][i]];
            if(_order.length > 0){
                OrderInfo storage _finalOrder = _order[_order.length - 1];
                ...
                if(_deadline > block.timestamp){
                    ...
                    teamTotalDeposit = teamTotalDeposit.add(_teammemberamount);
                }
            }
        }

        return teamTotalDeposit;
    }
    ...
    function checkDiamondMembership(address _user) public view returns(uint256){
        ...
        if(_orderLength > 0){
            ...
            _directSize = getActiveDirectDeposit(_user);
    ...
    function getMember(address _user) public view returns(uint256) {
        uint256 _capital = rewardInfo[_user].capitals;
        uint256 _member = 0;
        if(getActiveDirectNumbers(_user) >= 2 && getActiveDirectDeposit(_user) >= _capital.mul(2)){
            _member = 1;
        }else{
            _member = 0;
        }
