...
contract AuctionStorageV2 is Initializable {
    ...
    mapping(bytes => AuctionStructure.auctionStruct) private auctionIdToAuction;
    ...
    modifier onlyAuctionPure() {
        require(auctionPureAddress == msg.sender, "InvalidOwner");
        _;
    }

    function setStateAuction(
        bytes calldata _auctionId,
        AuctionStructure.StateOfAution _state
    ) external onlyOwner {
        require(
            auctionIdToAuction[_auctionId].state !=
                AuctionStructure.StateOfAution.DONE,
            "InvalidState"
        );
        auctionIdToAuction[_auctionId].state = _state;
    }

    function setAuctionPure(address _auctionPureAddress) external onlyOwner {
        require(_auctionPureAddress != address(0), "InvalidAddress");
        auctionPureAddress = _auctionPureAddress;
    }
    ...
    {
        AuctionStructure.auctionStruct storage _auction = auctionIdToAuction[
            _auctionId
        ];

        AuctionStructure.infoAuction memory _auctionInstance = AuctionStructure
            .infoAuction(
                _auction.autionId,
                _auction.tokenId,
                _auction.state,
    ...
    {
        AuctionStructure.auctionStruct
    ...
    function getInforOfferAuction(
        ...
        AuctionStructure.subOffer memory subOfferInstance = auctionInstance
            .offers
            .subOffers[_subOfferId];
        AuctionStructure.infoOfferAuction memory infoInstance = AuctionStructure
            .infoOfferAuction(
                auctionInstance.offers.tokenId,
    ...
    {
        AuctionStructure.Offer storage _offerInstance = auctionIdToAuction[
            _auctionId
        ].offers;

        uint256 length = auctionIdToAuction[_auctionId].offerIds.length;
        AuctionStructure.userFund[]
            memory userFunds = new AuctionStructure.userFund[](length);
        ...
        for (uint256 i = 0; i < length; ) {
            if (
                _offerInstance
                    ...
                    .state ==
                AuctionStructure.StateOfOffer.CANCELLED &&
                _offerInstance
                    .subOffers[auctionIdToAuction[_auctionId].offerIds[i]]
                    .currency ==
                ...
                address maker = _offerInstance
                    .subOffers[auctionIdToAuction[_auctionId].offerIds[i]]
                    .maker;
                AuctionStructure.userFund memory instance = AuctionStructure
                    .userFund(
                        maker,
        ...
        onlyAuctionPure
    {
        ...
        AuctionStructure.Offer storage offerInstance = auctionIdToAuction[
            _auctionId
        ].offers;
        ...
        for (uint256 i = 0; i < offerIds.length; ) {
            ...
            if (user != _exceptionist) {
                offerInstance.subOffers[offerIds[i]].state = AuctionStructure
                    .StateOfOffer
                    .CANCELLED;
            }
            unchecked {
    ...
    ) external onlyAuctionPure {
        AuctionStructure.auctionStruct
            storage auctionInstance = auctionIdToAuction[_auctionId];
        auctionInstance.autionId = _auctionId;
        auctionInstance.tokenId = _tokenId;
        auctionInstance.state = AuctionStructure.StateOfAution.ACTIVE;
        auctionInstance.owner = _ownerOfListing;
        auctionInstance.currency = _currency;
    }
    ...
    {
        AuctionStructure.auctionStruct
            storage auctionInstance = auctionIdToAuction[_params.indexId];
        auctionInstance.offers.saveOffer(_params);
        auctionInstance.offerIds.push(_params.subOfferId);
        auctionInstance.userToBidnumber[_params.maker] = _params.amount;
        auctionInstance.offerIdToIndex[_params.subOfferId] =
    ...
    function acceptOfferAuction(
        ...
        AuctionStructure.auctionStruct
            storage auctionInstance = auctionIdToAuction[_auctionId];
        auctionInstance.state = AuctionStructure.StateOfAution.DONE;
        auctionInstance.offers.subOffers[_subOfferId].state = AuctionStructure
            .StateOfOffer
            .DONE;
    }
    ...
    function cancelOfferAuction(
        ...
        require(auctionInstance.offerIds.length != 0, "InvalidOfferAmount");
    ...
    function expiredListing(bytes[] memory listingIds)
        external
        onlyAuctionPure
    {
        for (uint256 i = 0; i < listingIds.length; ) {
            auctionIdToAuction[listingIds[i]].state = AuctionStructure
                .StateOfAution
                .EXPIRED;
        ...
        returns (address, bytes memory)
    {
        ...
        require(auctionInstance.userToBidnumber[winner] != 0, "InvalidWinner");
