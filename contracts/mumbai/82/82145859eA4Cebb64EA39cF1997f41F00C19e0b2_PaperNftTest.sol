...
contract PaperNftTest is ERC721, Ownable {
  IPaperKeyManager paperKeyManager;

  uint256 supply = 0;

  constructor(address _paperKeyAddress) ERC721("PaperNftTest", "PNT") {
    paperKeyManager = IPaperKeyManager(_paperKeyAddress);
  }
  ...
  function mint(
    ...
    bytes32 _nonce,
    bytes calldata _signature
  ) external onlyPaper(keccak256(abi.encode(_receiver)), _nonce, _signature) {
    supply += 1;
    _safeMint(_receiver, supply);
  }

  function registerPaper(address _paperKey) external onlyOwner {
