            ...
            return
                string(
                    abi.encodePacked(
                        base,
                        "/",
                        encode(bytes.concat((_tokenURIs[tokenId])))
                    )
                );
...
interface IVoiceA is IERC721AUpgradeable {
  function mintOne(address to, bytes32 tokenURI, uint256 holdDuration) external;

  function mintBatch(address to, bytes32[] calldata tokenURIs, uint256 holdDuration) external;
