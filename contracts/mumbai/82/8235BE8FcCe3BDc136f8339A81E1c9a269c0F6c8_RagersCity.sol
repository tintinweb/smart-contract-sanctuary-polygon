...
// @title:  
// @desc:   
// @artist: https://twitter.com/
// @team:   https://twitter.com/
// @team:   https://twitter.com/
// @author: https://twitter.com/antoine_adx
...
contract RagersCity is IRagersCity, Ownable, SignatureMint {
    using ECDSA for bytes32;

    // -------- Minter --------
    uint256 public constant MAX_SUPPLY = 5556;
    uint256 public constant MAX_PUBLIC_SUPPLY = 5334;
    uint256 public constant MAX_WHITELIST_SUPPLY = 1556;
    uint256 public constant MAX_OG_SUPPLY = 223;

    uint256 public totalOGSupply = 0;
    uint256 public publicCost = 0.14 ether;
    uint256 public whitelistFirstCost = 0.1 ether;
    uint256 public whitelistSecondCost = 0.12 ether;
    ...
    // -------- ECDSA Whitelisting --------
    //The signature previously generated for the whitelist.
    address private whitelistSigner;


    // ======== CONSTRUCTOR =========
    constructor(address _metadata) ERC721A("Ragers City", "RC") {
    ...
    // -------- ECDSA Signer --------
    // function setWhitelistSigner(address _signer) external override onlyOwner {
    //     whitelistSigner = _signer;
    //     emit WhitelistSignerUpdated(whitelistSigner);
    // }

    // -------- Minting --------
    // function isAddressWhitelisted(bytes calldata _signature, bool _hasFreeMint) public view override returns (bool) {
    //    // Verify the _signature provided, which the client has given us through our website.
    //    return whitelistSigner == keccak256(
    //         abi.encodePacked(
    //             "\x19Ethereum Signed Message:\n32",
    //             bytes32(abi.encode(msg.sender, (_hasFreeMint ? 1 : 0)))
    //         )
    //     ).recover(_signature);
    // }
    ...
    function mintWhitelist(
        ...
        require(msg.value == _getWhitelistCost(_mintAmount, _hasFreeMint, _balance), "Insufficient funds!");
    ...
    modifier requiresPublic(uint256 _mintAmount) {
        ...
        require(_mintAmount < maxMintAmountPerTx, "Invalid mint amount!");
        require(totalSupply() + _mintAmount < MAX_PUBLIC_SUPPLY, "Max public supply exceeded!");
        require(msg.sender == tx.origin);
        require(
    ...
    function mintOG(
        ...
        // Free mint
        if (_hasFreeMint) { // freeMintClaimed ?
            if (isOG(_signature, _hasFreeMint) && _getAux(msg.sender) == 0) {
                require(totalOGSupply + 1 < MAX_OG_SUPPLY, "Max OG supply exceeded!");
                unchecked {
                    _mintAmount++;
                    totalOGSupply++;
                }
    ...
    function _getWhitelistCost(uint256 _mintAmount, bool _hasFreeMint, uint256 _balance) internal view override returns (uint256) {
        if (_hasFreeMint && _balance > 0) {
            _balance -= 1;
        }
        return _balance == 0 ?
                _mintAmount != 0 ? 
    ...
    function getCost(uint256 _mintAmount, bool _hasFreeMint) external view override returns (uint256) {
        if (whitelistOnly) {
            uint256 _balance = balanceOf(msg.sender);
            return _getWhitelistCost(_mintAmount, _hasFreeMint, _balance);
        } else {
            return publicCost;
        }
    }
...
abstract contract SignatureMint is Ownable {
    ...
    function isOG(bytes calldata signature, bool _hasFreeMint) public view returns(bool) {
        require(whitelistSigningKey != address(0), "OG whitelist not enabled");
        // Verify EIP-712 signature by recreating the data structure
        // that we signed on the client side, and then using that to recover
        // the address that signed the signature for this data.
        bytes32 digest = keccak256(
...
abstract contract IRagersCity is ERC721A, IERC2981 {
    ...
    function setWhitelistSigner(address _signer) external virtual {}
   ...
   function mintWhitelist(
    ...
    function setPaused() external virtual {}
    ...
    function getCost(uint256 _mintAmount, bool _hasFreeMint)
        external
        view
        virtual
        returns (uint256)
