...
contract DigCryptoVIP is AccessControl{
    using SafeERC20 for IERC20;
    
    mapping(address => vip) public vipData;
    
    address public TKN = 0x2523cCC751CFd372b3a4c9CF2538Fc4C565E2044;
    address public BUSD = 0x03401e701Ca700a32c9dfdd6631787888B5DE85e;
    ...
    constructor(IPancakeRouter01 _router,IERC20 _token,address payable _rewardPool){
        router = _router;
        token = _token;
    ...
    function addVipPrice(uint256 busdValue) public onlyRole(VIP_ROLE){
        vipPrices.push(busdValue);
    }
    function updateVipPrice(uint256 tier, uint256 busdValue) public onlyRole(VIP_ROLE){
    ...
    function setRewardPool(address payable _rewardPool) public onlyRole(VIP_ROLE){
        rewardPool = _rewardPool;
    }
    function setRewardTKN(address tkn) public onlyRole(VIP_ROLE){
        TKN = tkn;
    }
    function setRewardBUSD(address busd) public onlyRole(VIP_ROLE){
        BUSD = busd;
    }
    function setRewardWbnb(address Wbnb) public onlyRole(VIP_ROLE){
        WBNB = Wbnb;
    }
}
