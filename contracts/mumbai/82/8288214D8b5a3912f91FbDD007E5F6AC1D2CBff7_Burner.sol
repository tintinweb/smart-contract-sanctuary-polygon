...
import "./interface/token/IUtilityToken.sol";
import "./interface/IBurner.sol";
import "./error/Errors.sol";
import "hardhat/console.sol";

contract Burner is IBurner, Ownable, Base {
    IToken public token;
    IUtilityToken public uToken;

    // TODO: Define the tokenIds somewhere else (Enum?)
    uint256[] public utilityTokenIds = [0, 1, 2];

    constructor(address token_, address utilityToken_)
        checkNonZeroAddress(token_)
        checkNonZeroAddress(utilityToken_)
    {
        token = IToken(token_);
        uToken = IUtilityToken(utilityToken_);
    }

    function burn(uint256 amount, address to)
        external
        checkNonZeroValue(amount)
    {
        if (to == address(0)) {
            to = msg.sender;
        }
        ...
        uint256[] memory amounts = new uint256[](utilityTokenIds.length);
        uint256[] memory ids = utilityTokenIds;
        for (uint256 i = 0; i < amounts.length; i++) {
            amounts[i] = amount;
        }

        uToken.mintBatch(to, ids, amounts, "");
    }

    //TODO add method to update the utilityTokenIds
}
