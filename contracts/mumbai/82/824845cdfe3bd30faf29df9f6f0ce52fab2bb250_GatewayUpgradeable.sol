    ...
    ReentrancyGuardUpgradeable
{
    ...
    bytes32 constant IRECEIVE_METHOD_NAME =
    0x6952656365697665000000000000000000000000000000000000000000000000;
    ...
    error ExecuteCallsRevert(bool execFlag, bytes execData);
    ...
    function updateValset(
        ...
        Utils.Signature[] calldata _sigs
        ...
        SignatureUtils.checkValidatorSignatures(_currentValset, _sigs, digest, Utils.constantPowerThreshold);
    ...
    function setCurrentVersion(uint256 newVersion) external onlyRole(DEFAULT_ADMIN_ROLE) {
        currentVersion = newVersion;
    }

    function setBridgeFees(
        uint256 _iSendDefaultFee
    ...
    function setVault(address _vaultAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        // "empty address" => "C02"
        require(_vaultAddress != address(0), "C02");
        vault = AssetVault(_vaultAddress);
    }

    function setDappMetadata(string memory feePayerAddress) external payable nonReentrant returns (uint256) {
        // "fees too low" => "C03"
        require(msg.value >= iSendDefaultFee, "C03");
        eventNonce++;
        emit SetDappMetadataEvent(eventNonce, toBytes(msg.sender), chainId, feePayerAddress);
        return eventNonce;
    }

    // TODO: Memory or calldata check gas
    function iSend(
        uint256 version,
        uint256 routeAmount, 
        ...
        if (routeAmount > 0) {
            ...
            // TODO: Can we remove
            require(keccak256(routeRecipient) != 0xc5d2460186f7233c927e7db2dcc703c0e500b653ca82273b7bfad8045d85a470, 
                "C04"
            );
    ...
    function checkCheckpointAndVerifySigs(
        ...
        SignatureUtils.checkValidatorSignatures(_valsetArgs, _sigs, digest, Utils.constantPowerThreshold);
    }
    ...
    function iReceive(
        ...
        // TODO: Change individual fields to the whole payload 
        bytes memory encodedABI = abi.encode(
            ...
            requestPayload.requestPacket,
            requestPayload.isReadCall
        );

        checkCheckpointAndVerifySigs(_currentValset, _sigs, encodedABI);

        require(keccak256(bytes(chainId)) == keccak256(bytes(requestPayload.destChainId)), "wrong dest ID");

        // "cross-chain request message already handled" => "C06"
        require(nonceExecuted[requestPayload.srcChainId][requestPayload.requestIdentifier] == false, "C06");
        ...
        (bytes memory handler, bytes memory packet) = abi.decode(
            requestPayload.requestPacket,
            (bytes, bytes)
        );
        ...
        //         this.executeHandlerCalls.selector,
        //         requestPayload.requestSender,
        //         requestPayload.srcChainId,
        //         handler,
        //         packet,
        ...
        address handlerAddr = toAddress(handler);
        bool execFlag = isContract(handlerAddr);

        if (execFlag) {
            if (!requestPayload.isReadCall) {
                (execFlag, execData) = handlerAddr.call(
                    abi.encodeWithSelector(
                ...
                (execFlag, execData) = handlerAddr.call(packet);
            }
        }
        ...
        console.log("gas left handle request: %s", gasLeft - gasleft());
    }
    ...
    //     bool isReadCall
    // ) external isSelf returns (bool, bytes memory) {
    //     bytes memory execData;
    //     address handlerAddr = toAddress(handler);
    //     bool execFlag = isContract(handlerAddr);
    ...
    //             );
    //         } else {
    //             (execFlag, execData) = handlerAddr.call(payload);
    //         }
    //     }

    //     if (!execFlag) {
    //         revert ExecuteCallsRevert({ execFlag: execFlag, execData: execData });
    //     }

    //     return (execFlag, execData);
    // }

    // TODO: gas left remove
    function iAck(
        ...
        uint256 gasLeft = gasleft();
        // chainId, chainType data is present inside the signature validation
        // valsetArgs validation will be handled by the makeCheckPoint function
        require(
            nonceExecuted[crossChainAckPayload.destChainId][
        ...
        bool success;
        bytes memory data;
        success = isContract(handler);
        if (success) {
            (success, data) = handler.call(
                abi.encodeWithSelector(
                    IDapp.iAck.selector,
                    crossChainAckPayload.requestIdentifier,
                    crossChainAckPayload.execFlag,
        ...
        console.log("gas left ack: %s", gasLeft - gasleft());
    }

    // function executeCrossTalkAck(
    //     uint64 requestIdentifier,
    //     bytes memory requestSender,
    //     bool execFlag,
    //     bytes memory execData
    // ) external isSelf {
    //     address handler = toAddress(requestSender);
    //     bool success;
    //     bytes memory data;
    //     success = isContract(handler);
    //     if (success) {
...
library ValsetUpdate {
    function validateValsetPower(Utils.ValsetArgs calldata _newValset) public pure {
        ...
        for (uint64 i = 0; i < _newValset.powers.length; i++) {
            ...
            if (cumulativePower > Utils.constantPowerThreshold) {
                break;
            }
        }
        ...
        if (cumulativePower <= Utils.constantPowerThreshold) {
            revert Utils.InsufficientPower({
                ...
                powerThreshold: Utils.constantPowerThreshold
            });
        }
    }
