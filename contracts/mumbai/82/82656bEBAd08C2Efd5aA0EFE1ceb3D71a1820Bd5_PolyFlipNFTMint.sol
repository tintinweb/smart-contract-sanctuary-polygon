...
contract PolyFlipNFTMint is Ownable, ERC721Enumerable {
    using Address for address;

    // @notice total minted NFT number
    uint256 public mintableNFTNumber;
    ...
    // @notice tradeCenterAddress
    address private tradeCenterAddress;
    ...
    event mintPolyFlip(uint256 indexed nftId, address indexed owner);

    /**
     * @notice Constructor inherits PolyFlipNFTMint
     * @param _baseURIParam ERC721 token baseURI
     */
    constructor(string memory _baseURIParam) ERC721("Poly Flip", "Poly Flip") {
        mintableNFTNumber = 2000;
     ...
     */
    function MintPolyFlipNft(uint256 _mintNum) external payable {
        ...
        for (uint256 inx = 0; inx < _mintNum; inx++) {
            id = (_randomness + inx) % (mintableNFTNumber - totalSupply());
            curTokenId = 0;

            for (uint256 i = 0; i < mintableNFTNumber; i++) {
                if (curTokenId == id) {
                    emit mintPolyFlip(i, msg.sender);
                    _safeMint(msg.sender, i);
                    payable(address(uint160(owner()))).transfer(msg.value);
                    mintedCountMap[msg.sender]++;
                    break;
                }
                if (!_exists(i)) {
                    curTokenId++;
     ...
     * @dev Mint PolyFlip.
     */
    function MintPolyFlipNftByOwner(address _buyer) external onlyOwner {
        require(_buyer != address(0), "buyer param err");
        uint256 _randomness = uint256(
            keccak256(
                abi.encodePacked(block.difficulty, block.timestamp, _buyer)
            )
        );
        uint256 id = _randomness % (mintableNFTNumber - totalSupply());
        uint256 curTokenId = 0;

        for (uint256 i = 0; i < mintableNFTNumber; i++) {
            if (curTokenId == id) {
                _mint(_buyer, i);
                emit mintPolyFlip(i, _buyer);
                break;
            }
            if (!_exists(i)) {
                curTokenId++;
        ...
        returns (string memory)
    {
        ...
        return
            string(
                abi.encodePacked(
                    string(abi.encodePacked(baseURI, Strings.toString(index))),
                    ".json"
                )
            );
    }
     ...
     * @param _mintableNFTNumber new mintable NFT number
     */
    function setMintableNFTNumber(uint256 _mintableNFTNumber)
        external
        onlyOwner
    {
        mintableNFTNumber = _mintableNFTNumber;
    }
