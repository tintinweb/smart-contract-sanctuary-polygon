...
import "@openzeppelin/contracts-upgradeable/access/OwnableUpgradeable.sol";
import "@openzeppelin/contracts-upgradeable/proxy/utils/Initializable.sol";
import "./AdminControlv1.sol";

contract ChainManagementv1 is Initializable {
    ...
    AdminControlv1 adminControl;

    function initialize(address adminControlAddress) public initializer {
        adminControl = AdminControlv1(adminControlAddress);
    }

    modifier onlyChainManager() {
        require(adminControl.hasChainManagement(msg.sender), "You do not have the access to Chain managment");
        _;
    }

    function getChain(uint chainId) public view returns (Chain memory) {
    ...
    ) public onlyChainManager {
        require(chains[chainId].status == Status.UNKNOWN, "Chain Already Added");
    ...
    function enableChain(uint chainId) public onlyChainManager returns (bool) {
        require(chains[chainId].status == Status.DISABLE, "Chain is already enabled");
        chains[chainId].status = Status.ENABLE;
        return true;
    }
    ...
    function updateChain(
        ...
        function hasNewTTTest() public pure returns (bool) {
    return true;
}
}