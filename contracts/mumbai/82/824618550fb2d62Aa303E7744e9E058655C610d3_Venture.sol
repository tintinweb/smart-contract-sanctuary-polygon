...
contract Venture is AragonApp {
    //is OracleClient
    using SafeMath256 for uint256;
    using SafeMath16 for uint16;
    using ArrayUtils2 for address[];
    // bytes32 public constant FUNDRAISING_ROLE = keccak256("FUNDRAISING_ROLE");
    bytes32 public constant FUNDRAISING_ROLE =
        0x9cdf7b88bc507be882f65d229d9ca8dc02786dbf7cea65d38672b821ee56a125;
    ...
    struct DeployParams {
        ...
        string _communityTokenSymbol;
        string _governanceTokenName;
        string _governanceTokenSymbol;
        string _id;
        address[] _communityMembers;
        uint256[] _communityStakes;
        address[] _governanceMembers;
        uint256[] _governanceStakes;
        uint64[3] _votingSettings;
        address[] _tokenRequestAcceptedDepositTokens;
        string _syndicate_aggreement_link;
        uint64 _syndicate_setup_fee_percent;
        uint64 _syndicate_distribution_fee_percent;
        uint64 _token_start_price;
        string _reference;
        string _description;
    ...
    event Contribution(
        ...
        MiniMeToken addressContributeToken,
    ...
    function initialize(
        ...
        console.log("initialize Venture");
        console.log("RewardAPP");
        console.log(_wallets[3]);
        console.logBytes32(FUNDRAISING_ROLE);
    ...
    function contribute(
        MiniMeToken _addressContributeToken,
    ...
    function stopFundraising() external auth(FUNDRAISING_ROLE) {
        require(!locked, "LOCKED");
        ...
        for (uint256 j = 0; j < successTokenList.length; j++) {
            MiniMeToken token = MiniMeToken(successTokenList[j]);
            console.log(
                "::SUCCESS AMOUNT",
    ...
    function distributeProjectToken(
        uint256 _amount,
        uint256 _startUser
    ) external auth(FUNDRAISING_ROLE) {
        require(locked, "NOT YET LOCKED");
        require(projectToken != address(0), "NOT SET PROJECT TOKEN");
        MiniMeToken _token = MiniMeToken(projectToken);
    ...
    function setCapAmount(uint256 _amount) external auth(FUNDRAISING_ROLE) {
        capAmount = _amount;
    }

    function setProjectTokenPrice(
        uint16 _price
    ) external auth(FUNDRAISING_ROLE) {
    ...
    function calculateSuccessFeeAmountTokenForType2(
        uint256 _amount
    ) public view returns (uint256 successFeeAmountToken) {
        uint256 profitPerToken = currentTokenPrice.sub(baseTokenPrice);
    ...
    function removePrivelegedUsers(
        address[] _address
    ) external auth(FUNDRAISING_ROLE) {
        uint256 arrayLen = _address.length;
        for (uint256 i = 0; i < arrayLen; ++i) {
    ...
    function setProjectToken(address _token) external auth(FUNDRAISING_ROLE) {
