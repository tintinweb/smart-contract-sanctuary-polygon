...
contract RaffleMainProject is RaffleVRF{
    ...
    /* Raffle Variables */
    uint256 private ticketCost = defaultTicketCost;
    uint256 private prize = defaultPrize;
    uint256 private softcap = defaultSoftcap;
    //Default
    uint256 private defaultTicketCost = 0.01 ether;
    ...
    /* CONSTRUCTOR */
    constructor(address vrfCoordinatorV2, bytes32 keyHash, uint64 subscriptionId, uint32 callbackGasLimit)
    ...
    */
    ...
    function closingTime() public isAtStage(RaffleStage.Open) returns (uint256) {
    ...
    */
    function createRaffle(uint256 feeToUpdate,
                          ...
                          uint256 newSoftcap,
                          uint256 timeToClose)
        safetyLock isAtStage(RaffleStage.Closed) public onlyAdmins {       
        raffleId++;
        lastTimeStamp = block.timestamp;

        if (feeToUpdate != defaultTicketCost) {ticketCost = feeToUpdate;}
    ...
    */
    ...
    require(block.timestamp < raffleClosing, "Check the closing time!");
        ...
        require(msg.value == (ticketCost * numberOfTickets), "Incorrect amount sent");

        entryCount[msg.sender] += numberOfTickets;
    ...
    function drawResult(uint256 requestId) public returns (address) {
    ...
    */
    ...
    function calculateRefundAmount(address participant) internal view safetyLock isAtStage(RaffleStage.Canceled) returns (uint256) {
        ...
        for (uint256 i = 0; i < players.length; i++) {
            ...
            if (player == participant && numberOfTickets > 0) {totalAmountSpent += (numberOfTickets * ticketCost);}
        }
