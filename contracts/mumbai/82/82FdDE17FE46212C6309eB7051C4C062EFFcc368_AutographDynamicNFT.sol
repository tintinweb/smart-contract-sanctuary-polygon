...

{
    ...
    */
        ...
        external
    {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            ...
            /**
            * @dev assigns default NFT metadata
            **/
            ...
            metadata[newItemId].traits = metadata[0].traits;
            metadata[newItemId].values = metadata[0].values;
    ...
    */
        ...
        returns(bool success)
    {
        ...
        for (uint i = 0; i <= _tokenIds.current() + 1; i++) {
            metadata[i].title = meta[0];
            metadata[i].description = meta[1];
            metadata[i].image_url = meta[2];
            metadata[i].animation_url = meta[3];
            metadata[i].external_url = meta[4];
            metadata[i].traits = traits;
    ...
    /**
    * @dev Set metadata for list by array of tokenIds
    */
    function setMetadataByIds
    (
        ...
        returns(bool success)
    {
        ...
        for (uint i = 0; i < tokenIds.length; i++) {
            metadata[tokenIds[i]].title = meta[0];
            metadata[tokenIds[i]].description = meta[1];
            metadata[tokenIds[i]].image_url = meta[2];
            metadata[tokenIds[i]].animation_url = meta[3];
            metadata[tokenIds[i]].external_url = meta[4];
            metadata[tokenIds[i]].traits = traits;
            metadata[tokenIds[i]].values = values;
        }
    ...
    */
        ...
        returns (string memory)
    {
        ...
        string memory name = concatenate(metadata[tokenId].title, tokenIdString);
    ...
    */
        ...
        returns(string memory)
    {
        ...
        for (uint t = 0; t < metadata[_tokenId].traits.length; t++) {
            string memory trait = string(abi.encodePacked('{"trait_type":"', metadata[_tokenId].traits[t], '","value":"', metadata[_tokenId].values[t], '"}'));
            if(t != metadata[_tokenId].traits.length - 1) {
