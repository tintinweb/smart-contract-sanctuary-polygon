...
// File contracts/AccountAbstraction/Lamport/Constants.sol
...
abstract contract LamportBaseA is Pausable, BaseAccount {
	using Address for address payable;

	mapping(bytes32 => Constants.PublicKeyHashUsageStatus) publicKeyHashes;
    ...
    */
	function isRedeemable(bytes32 publicKeyHash) public view returns (bool) {
		return publicKeyHashes[publicKeyHash] == Constants.PublicKeyHashUsageStatus.POSTED;
	}
	...
	modifier redeemLamport(
		...
		require(Constants.verify_u256(uint256(keccak256(prepackedMessageData)), signature, publicKey), "LB:E9"); // LamportBaseA: Signature not valid

		_removeSinglePublicKeyHash(publicKeyHash);
		emit Constants.RemovedPublicKeyHashes(liveKeyCount);

		_;

		if (liveKeyCount < Constants.DEFAULT_MINIMAL_LIVE_KEYS) {
			_pause();
		}
	}
    ...
    */
	function _addSinglePublicKeyHash(bytes32 publicKeyHashToAdd) internal {
		require(publicKeyHashes[publicKeyHashToAdd] == Constants.PublicKeyHashUsageStatus.UNUSED, "LB:E10"); // LamportBaseA: Public Key Hash Already Exists

		publicKeyHashes[publicKeyHashToAdd] = Constants.PublicKeyHashUsageStatus.POSTED;
		liveKeyCount++;
	}
    ...
    */
	function _addPublicKeyHashes(bytes32[] memory publicKeyHashesToAdd) internal {
		unchecked {
			...
			emit Constants.AddedPublicKeyHashes(liveKeyCount);
		}
	}
    ...
    */
	function _removeSinglePublicKeyHash(bytes32 publicKeyHashToRemove) internal {
		require(publicKeyHashes[publicKeyHashToRemove] == Constants.PublicKeyHashUsageStatus.POSTED, "LB:E12"); // LamportBaseA: Public Key Hash Not Posted Or Already Redeemed

		publicKeyHashes[publicKeyHashToRemove] = Constants.PublicKeyHashUsageStatus.REDEEMED;
		liveKeyCount--;
	}
    ...
    IERC1363Spender
{
    ...
    function initialize(
        ...
        // console.log("VVVVVVV");
        // // console.logBytes4(type(IERC777Recipient).interfaceId); 
        // // console.logBytes4(type(IERC165).interfaceId); 
        // console.logBytes4(type(IERC777Sender).interfaceId);
        // console.logBytes4(type(IERC1271).interfaceId);
        // console.log("^^^^^^^");
    ...
    function _validateAndUpdateNonce(
        ...
        console.log("Nonce (source a) is %s", userOp.nonce);
        console.log("Nonce (source b) is %s", _nonce);
    ...
    function _validateSignature(
       ...
       console.log("Nonce is %s", userop.nonce); 
        ...
        console.logBytes(ecdsaSig);

        if (owner != hash.recover(ecdsaSig))
            return SIG_VALIDATION_FAILED;

        if (!isRedeemable(selectedKey)) {
            validationData = SIG_VALIDATION_FAILED;
            return validationData;
        }

        bool isValidSig = Constants.verify_u256(
            // uint256(userOpHash),
            uint256(hash),
            sig,
            publicKey
        );
        console.log("isValidSig: %s", isValidSig);
    ...
    function execute(
        ...
        console.log("STUB: inside execute");
        _requireFromEntryPoint();
...
contract LamportAccountFactory {
    ...
    function createAccount(address owner,uint256 salt, bytes32[] memory firstLamportKeys) public returns (LamportAccount ret) {
        ...
        ret = LamportAccount(payable(new ERC1967Proxy{salt : bytes32(salt)}(
