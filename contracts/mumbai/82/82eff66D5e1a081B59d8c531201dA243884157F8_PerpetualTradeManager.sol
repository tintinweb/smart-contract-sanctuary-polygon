...
contract PerpetualTradeManager is PerpetualTradeFunctions, IFunctionList, IPerpetualTradeManager {
     ...
     */
    function trade(Order calldata _order) external override {
        require(_order.traderAddr == msg.sender, "sender should be set in an order");
...
contract PerpetualTradeFunctions is PerpetualRebalanceFunctions, PerpetualHashFunctions {
     ...
     */
    function _trade(IPerpetualOrder.Order memory _order, bytes32 _digest)
        internal
    ...
    function _doOpeningTradeActions(
        ...
        int128 maxTradeDelta = _getTradeLogic().getMaxSignedTradeSizeForPos(
            _perpetual.id,
            _fTraderPos,
            _order.fAmount
        );
        ...
        require(
            _order.fAmount.abs() < maxTradeDelta.abs(),
