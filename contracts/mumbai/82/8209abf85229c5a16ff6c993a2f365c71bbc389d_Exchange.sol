...
contract Exchange {
    ...
    function isVerifiedOrder(Order memory order, Sign calldata sign) internal view {
        bytes32 hash = keccak256(abi.encodePacked(this, order.seller, order.buyer, order.erc20Address, order.nftAddress, order.amount, order.tokenId, order.tokenId,sign.nonce));
    ...
    */
    ...
    function createNFT(address nftAddress, string memory uri, uint256 supply, uint96 fee, Sign calldata sign) external payable {
        ...
        (uint256 tokenId, bool mint) = IERC1155(nftAddress).mint(uri, supply, fee);
