...
contract ZipMexVestingV1 is Initializable, OwnableUpgradeable{
    ...
    mapping(uint => mapping(address => Beneficiary)) private beneficiaries;
    ...
    */
    modifier nameDoesNotExist(string memory _name) {
        require( 
            ...
            "ZipMexVesting: name already exists");
        _;
    }
    ...
    */
    modifier tokenAmountNotZero(uint _tokenAmount) {
        require(
            _tokenAmount > 0,
            "ZipMexVesting: zero token amount"
        );
        _;
    }
    ...
    */
    modifier onlyBeneficiary(uint _poolIndex) {
        require(
            beneficiaries[_poolIndex][msg.sender].status,
            "ZipMexVesting: not whitelisted"
        );
        _;
    }
    ...
    */
        ...
        tokenAmountNotZero(totalPoolTokenAmount)
    {
        require(
            ...
            "ZipMexVesting: unmatched release array lengths"
        );
        uint totalPercent = 0;
    ...
    */
        ...
        tokenAmountNotZero(tokenAmount)
    {
        ...
        if(beneficiaries[poolIndex][address_].investment == 0){
        ...
        beneficiaries[poolIndex][address_].investment += tokenAmount;
    ...
    */
        ...
        onlyOwner
    {
        require(
            ...
            "ZipMexVesting: address and investment array sizes are different."
            );
    ...
    */
    ...
    {
        beneficiaries[poolIndex][beneficiary].status = true;
    }
    ...
    */
    ...
    {
        require(
            !beneficiaries[poolIndex][msg.sender].rewarded,
            "ZipMexVesting: all releases claimed"
        );
        uint unlockedTokens;
        uint releaseTime;
        ...
        require(
            ...
            "ZipMexVesting: no pending releases"
        );

        require(
            block.timestamp >= releaseTimeInSeconds,
            "ZipMexVesting: time for next release not reached yet."
        );

        require(
            unlockedTokens > 0, 
            "ZipMexVesting: no claimable tokens in this schedule."
        );
        require(
            unlockedTokens <= TOKEN.balanceOf(address(this)),
            "ZipMexVesting: not enough tokens in the contract."
        );
        if(beneficiaries[poolIndex][msg.sender].currentIndex > 
            vestingPools[poolIndex].releaseTimes.length){
                beneficiaries[poolIndex][msg.sender].rewarded = true;
            }
        poolInvestments[poolIndex].totalTokensClaimed += unlockedTokens;
        beneficiaries[poolIndex][msg.sender].claimed += unlockedTokens;
        beneficiaries[poolIndex][msg.sender].currentIndex += 1;

        require(TOKEN.transfer(msg.sender, unlockedTokens),
            "ZipMexVesting: token transfer failed"
        );
    ...
    */
    ...
    {
        return(beneficiaries[poolIndex][address_]);
    }
    ...
    */
        ...
        returns (uint, uint)
    {
        ...
        uint currentVestingIndex = beneficiaries[poolIndex][msg.sender].currentIndex;
        ...
        nextReleaseTime = vestingPool.releaseTimes[currentVestingIndex];
        nextReleaseAmount = (beneficiaries[poolIndex][address_].investment*multiplier)/10000;
