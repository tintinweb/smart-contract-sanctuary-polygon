...
contract Exchange {
    ...
    event FundsLoaded(address indexed from, uint USDTamount, uint MATICamount); // emits when funds successfully loaded
    ...
    function loadFunds(uint MATICToReceive, uint USDTToReceive) public {
        ...
        if (MATICToReceive > 0) {
            bool MATICTransferred = MATIC.transfer(address(this), MATICToReceive); // it makes transfer tokens from user's balance to contract balance and return bool of result
            if (MATICTransferred) {
        ...
        if (USDTToReceive > 0) {
            bool USDTTransferred = USDT.transfer(address(this), USDTToReceive); // it makes transfer tokens from user's balance to contract balance and return bool of result
            if (USDTTransferred) {
                // if transfer return true (it success) contract add it to user balance
        ...
        emit FundsLoaded(msg.sender, USDTToReceive, MATICToReceive); // emits event with data when funds loaded
    }
