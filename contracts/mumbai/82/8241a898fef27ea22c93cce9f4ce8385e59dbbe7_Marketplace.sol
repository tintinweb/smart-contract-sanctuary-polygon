    ...
    UUPSUpgradeable
{
    ...
    event WithdrawCommissionFee(address _owner, uint256 _amount);
    ...
    function withdrawCommissionFee() public onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit WithdrawCommissionFee(owner, address(this).balance);
    }
    ...
    function firstListMarketItem(uint256 _tokenId, uint256 _price, uint256 _creatorEarning) public {
        ...
        require(_creatorEarning <= 100, "CREATOR_EARNING_OVER_LIMIT");
    ...
    function saleMarketItem(uint256 _tokenId) public payable {
        ...
        if (_oc.creatorEarning > 0) {
            _creatorEarning = _omi.price.mul(_oc.creatorEarning).div(1000);
