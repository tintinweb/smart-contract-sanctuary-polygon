...
/**
 * @author Linum Labs
 * @title CollectionFactory smart contract that is responsible for creating new ERC1155 collections.
 */
contract CollectionFactory is Ownable {
    ...
    /// Addresses of all collections that have been minted so far
    address[] public collectionAddresses;

    /// Check if the collection address is of Bullion collection
    mapping(address collectionAddress => bool isBullionCollection)
        public isBullionCollection;
    ...
    /**
     * @notice Emit when a collection is successfully created
    ...
    /**
     * @notice Create a collection and deploy the Collection contract accordingly
     * @dev Only the contract owner can call this function
     * @param collectionHash Collection hash that stores the collection metadata
     * @param baseURI Collection base URI
     */
    function createCollection(
        string calldata collectionHash,
        string calldata baseURI
    ) external onlyOwner {
        if (bytes(collectionHash).length == 0) revert CannotBeEmptyString();
        if (bytes(baseURI).length == 0) revert CannotBeEmptyString();

        /// Deploy a new Collection contract
        Collection collection = new Collection(collectionHash, baseURI);
        /// Transfer ownership of the new Collection contract to this contract owner
        collection.transferOwnership(msg.sender);
        /// Mark that the newly created Collection contract is from Bullion and add it to collectionAddresses
        isBullionCollection[address(collection)] = true;
        collectionAddresses.push(address(collection));

        emit CollectionCreated(address(collection), collectionHash, baseURI);
    }
}
