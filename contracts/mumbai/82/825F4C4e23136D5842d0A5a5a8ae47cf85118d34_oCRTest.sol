...
contract oCRTest is 
    ...
    uint256 public minHolder;
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIdTracker;

    struct ConfCost{
        uint256 oCRGenerateCost;
        uint256 oCRScrambleCost;
        uint16  oCRMultiple;
        uint16  oCRMinHolder;
        uint16  oCRScramblePoint;    
        }
    ...
    // Mapping oCRGenerate -> can _mintRubik if oCRGenerate.amount > oCRGenerate.scramble
    mapping(address => oCRGenerate) public generate;
    ConfCost public confcost;

    constructor() ERC721("oCRTest", "oCRT") {
        maxSupply = 10000;
    }
    ...
    modifier mintVerify(uint16 _mintAmount, bool _generate ) {
        require(totalSupply() + _mintAmount < maxSupply, "Max supply exceeded!");
        if(_msgSender()!= owner() ){
            require( balanceOf(msg.sender) >= confcost.oCRMinHolder , "inValid MinHolder!");
            if(_generate == false){
                require(generate[_msgSender()].scramble + confcost.oCRMultiple <= generate[_msgSender()].amount , "inValid mint point!");
                require(msg.value >= confcost.oCRScrambleCost * _mintAmount, "Insufficient funds!");
            }else{
                require(msg.value >= confcost.oCRGenerateCost * _mintAmount, "Insufficient funds!");
            }
        }
        _;
    }

    function _mintRubik(string memory _name, string memory _scramble) public mintVerify(1, false) payable {
        _timestamp = block.timestamp;
        rubiks[_tokenIdTracker.current()] = Rubik(string(abi.encodePacked("oCR#",Strings.toString(_tokenIdTracker.current())," ", _name)), _scramble, Strings.toString(_timestamp), _msgSender());
        isDna[Strings.toString(_timestamp)] = Dna(true);
    ...
    function _mintRandom(uint8  _itemsamount, uint8  _notationsamount) public mintVerify(_itemsamount, true) payable {
        for (uint i=0; i < _itemsamount; i++) {
            string memory _scramble = GetOcrTest.randomNotations(_tokenIdTracker.current(), i, _notationsamount);
            string memory dna = Strings.toString(block.timestamp);
            require(isDna[dna].exist!=true, "Dna exists!");            
            rubiks[_tokenIdTracker.current()] = Rubik(string(abi.encodePacked("oCR#",Strings.toString(_tokenIdTracker.current()))), _scramble, dna, _msgSender());
            isDna[dna] = Dna(true);
            _safeMint(_msgSender(), _tokenIdTracker.current());
            _setTokenRoyalty(_tokenIdTracker.current(), _msgSender(), 500);
            _tokenIdTracker.increment();            
        ...
        if(_msgSender()!= owner() ){
            generate[_msgSender()].amount = generate[_msgSender()].amount + (_itemsamount * confcost.oCRScramblePoint);            
            (bool success, ) = payable(owner()).call{value: address(this).balance}("");
            require(success);
        }
    }
...
library GetOcrTest {
    function randomNotations(uint256 tokenId, uint256 itemAmount, uint256 notationAmount) public view returns (string memory notationstr) {
        ...
        for (uint256 i = 0; i < notationAmount; i++) {
            notations[i] = uint256(keccak256(abi.encodePacked(block.timestamp,tokenId,itemAmount, i)))%12;
            notationstr = string(abi.encodePacked(notationstr,notationsArr[notations[i]]));
        }        
        return (notationstr);
    }  

    function getRubiks(uint256 _tokenId) public pure returns (string memory) {
        ...
        pathSvg = string(abi.encodePacked(pathSvg,"<text xml:space='preserve' style='font-size:73.5032px;fill:#f2f2f2;stroke:#f2f2f2;stroke-width:6.12527' x='77.858833' y='283.40219' id='text823'><tspan id='tspan821' x='77.858833' y='283.40219' style='fill:#f2f2f2;stroke:#f2f2f2;stroke-width:6.12527'>Cocacoba ",Strings.toString(_tokenId),"</tspan></text>"));
