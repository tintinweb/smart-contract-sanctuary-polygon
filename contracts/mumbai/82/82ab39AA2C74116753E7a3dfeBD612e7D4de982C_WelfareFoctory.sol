...
contract Lottery is IRandomConsumer, Ownable {
    ...
    event Bonus(uint256 phase, address lottery, uint256 bonus);
    ...
    function setBitMask(uint8 _bits)
        ...
        isState(LotteryState.Open)
    {
        require(_bits % 4 == 0, "Invalid bytes");
        _BITPOS = _bits;
    ...
    function _payoutBonus(uint256 _winningNum) private {
        ...
        for (uint256 e = 0; e < len; e++) {
            ...
            for (uint256 n = 0; n < entry.numbers.length; n++) {
                ...
                while (bits > 0) {
                    ...
                    if (ml == wl) {
                        entry.winnings = entry.winnings.add(1);
                    }
                    mLot = mLot >> 4;
                    wLot = wLot >> 4;
                    bits = bits - 4;
        ...
        if (balance > 0) {
            ...
            for (uint256 index = 0; index < len && balance > 0; index++) {
                ...
                Entry memory entry = entries[star];
                uint256 _bonus = 0;
                if (entry.winnings > 0 && bonus[entry.winnings] > 0) {
                    _bonus = bigBonus.mul(bonus[entry.winnings]).div(1 ether);
                    if (_bonus > 0) {
                        entry.bonus = _bonus;
                        entries[star] = entry;
        ...
        for (uint256 index = 0; index < len; index++) {
            ...
            if (entry.bonus > 0 && !entry.finished) {
                ...
                emit Bonus(phase, address(this), _bonus);
            }
        }
    }
