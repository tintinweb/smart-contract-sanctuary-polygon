...
contract NftDuel is VRFConsumerBaseV2, ERC721Holder, Ownable {
    enum CoinSide {
        HEAD,
        TAIL
    }
    ...
    bytes32 public claimFeesMerkleRoot;
    bytes32 public joinFeesMerkleRoot;
    bytes32 public clMaxGasKeyHash;
    VRFCoordinatorV2Interface public clCoordinator;
    uint64 public clSubscriptionId;
    uint32 public clCallbackGasLimit = 100000;
    ...
    constructor(
        ...
        bytes32 _joinFeesMerkleRoot,
        bytes32 _claimFeesMerkleRoot
    ) VRFConsumerBaseV2(coordinatorAddress)
    {
        ...
        joinFeesMerkleRoot = _joinFeesMerkleRoot;
        claimFeesMerkleRoot = _claimFeesMerkleRoot;
    }

    /**
     * PUBLIC API
     **/
    function getAllGames(address projectAddress) public view returns (Game[] memory) {
        Game[] memory gameList = new Game[](gameCountPerProject[projectAddress]);

        for (uint i = 0; i < gameCountPerProject[projectAddress]; i++) {
            gameList[i] = projectGames[projectAddress][i];
        }
    ...
    function getGamesBatch(address projectAddress, uint256 startIndex, uint256 batchSize) public view returns (Game[] memory) {
        require(startIndex < gameCountPerProject[projectAddress], "Start index out of bounds");
        require(batchSize > 0, "Batch size must be greater than zero");
        Game[] memory gameList = new Game[](batchSize);
    ...
    function createGame(address nftContractAddress, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        require(isContractEnabled, "Game is currently disabled.");
        requireFee(feesMerkleProof, nftContractAddress, requiredFeeInWei, joinFeesMerkleRoot);
        require(verifyNftPower(nftPowerMerkleProof, nftId, nftPower, nftPowerMerkleRoots[nftContractAddress]));
    ...
    function joinGame(address nftContractAddress, uint256 gameId, uint256 nftId, uint256 nftPower, uint256 requiredFeeInWei, bytes32[] calldata feesMerkleProof, bytes32[] calldata nftPowerMerkleProof) public payable {
        require(isContractEnabled, "Game is currently disabled.");
        ...
        // TODO: Make sure that it's actually more gas efficient to update memory object properties
        // and assigning the whole object to storage instead of updating 2 storage object properties
        // If this optimization is good then update also fulfillRandomWords function
        // games[gameId].status = GameStatus.InProgress;
        // games[gameId].player2Bet = PlayerBet(msg.sender, nftId, selectedSide);
        Game memory game = projectGames[nftContractAddress][gameId];
        game.status = GameStatus.InProgress;
        game.player2 = Player(msg.sender, nftId, nftPower);
        projectGames[nftContractAddress][gameId] = game;
    ...
    function leaveGame(address nftContractAddress, uint256 gameId) public {
        ...
        IERC721 nftContract = IERC721(projectGames[nftContractAddress][gameId].nftContractAddress);
    ...
    function claimPrize(address nftContractAddress, uint256 gameId, uint256 requiredFeeInWei, bytes32[] calldata merkleProof) public payable {
        ...
        requireFee(merkleProof, nftContractAddress, requiredFeeInWei, claimFeesMerkleRoot);
    ...
    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        ...
        uint256 totalNftPower = game.player1.nftPower + game.player2.nftPower;
        Player memory winner = _randomWords[0] % totalNftPower < game.player1.nftPower
            ? projectGames[nftContractAddress][gameId].player1
            : projectGames[nftContractAddress][gameId].player2;
        projectGames[nftContractAddress][gameId].winner = winner;

        emit GameCompleted(nftContractAddress, gameId, winner.nftId);
    }
    ...
    function requireFee(bytes32[] calldata merkleProof, address nftContractAddress, uint256 requiredFeeInWei, bytes32 merkleRoot) private view {
        require(verifyRequiredFee(merkleProof, nftContractAddress, requiredFeeInWei, merkleRoot), "INVALID_PROOF");
        require(msg.value >= requiredFeeInWei, "Provided fee is too low.");
    }

    function verifyRequiredFee(bytes32[] calldata merkleProof, address nftContractAddress, uint256 requiredFeeInWei, bytes32 merkleRoot) private pure returns (bool) {
        bytes32 leaf = keccak256(abi.encode(nftContractAddress, requiredFeeInWei));
        return MerkleProof.verify(merkleProof, merkleRoot, leaf);
    }

    function verifyNftPower(bytes32[] calldata merkleProof, uint256 nftId, uint256 nftPower, bytes32 merkleRoot) private pure returns (bool) {
    ...
    function setClaimFeesMerkleRoot(bytes32 merkleRoot) external onlyOwner
    {
        claimFeesMerkleRoot = merkleRoot;
    }

    function setJoinFeesMerkleRoot(bytes32 merkleRoot) external onlyOwner
    {
        joinFeesMerkleRoot = merkleRoot;
    }

    function setNftPowerMerkleRoot(address nftProjectAddress, bytes32 merkleRoot) external onlyOwner
    {
        nftPowerMerkleRoots[nftProjectAddress] = merkleRoot;
    }
