...
contract MBKeeper is Ownable {
    ...
    function setMBStorageAddress(address _newMBStorageAddress)
        public
        onlyOwner
    {
    ...
    function setMBAdminAddress(address _newMBAdminAddress)
        public
        onlyOwner
    {
    ...
    function performUpkeep(bytes calldata performData) external {
        ...
        if (performData.length == 0) {
            ...
            uint256 removedCount = mbAdmin.purgeExpiringItems();
            if (removedCount < _count) emit SomethingWrong();
        } else {
            if (!mbStorage.KeeperAutoSync()) revert KeeperSyncingDisabled();
            if (block.timestamp - lastSync < mbStorage.syncInterval())
                revert TooSoon();
...
interface IMBStorage {
    ...
    // [1, 0, 0, 3, "0x9f3814f57a2390e29bbc15f33554b62da9a7923a", 0, "0x7cBCCC4a1576d7A05eB6f6286206596BCBee14aC", 5, 1, 0, false]
    struct Item {
        //CB: Probably needs reorg to pack data
        uint256 ref;
        uint256 pool;
        chType onChainType;
    ...
    // [true, "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6",1,0,0,0,0,0]
    struct MBPool {
        //CB: Probably needs reorg to pack data
        bool active;
        address mbContract;
    ...
    function defaultMBcontractAddress() external view returns (address);

    function defaultItemsContractAddress() external view returns (address);

    function defaultItemsVaultAddress() external view returns (address);
    ...
    function nitConditions() external view returns (address);
    ...
    function PoolsConditions(uint256 _pool)
        external
        view
    ...
    function getPoolConditionsCount(uint256 _pool)
        external
        view
        returns (uint256);
    ...
    function PoolItems(uint256 _pool, uint256 _index)
        external
        view
        returns (uint256);
    ...
    function oldItems(uint256 _pool, uint256 _index)
        external
        view
        returns (uint256);
    ...
    function itemWinners(uint256 _itemRef, uint256 _index)
        external
        view
        returns (address);
    ...
    function getUserWonItemsCount(address _user)
        external
        view
        returns (uint256);

    function userWonItems(address _user, uint256 _index)
        external
        view
        returns (uint256);

    function itemWinsOfUserbyRef(uint256 _ref, address _user)
        external
        view
        returns (uint256);
    ...
    function lastOpenedBox(uint256 _pool, address _user)
        external
        view
        returns (uint256);
    ...
    function setUserLastOpened(uint256 _pool, address _user) external;

    function incrementMBopens(uint256 _pool, address _user) external;
}
