...
contract iNFTspaceSale is Ownable, ERC721Holder, ERC1155Holder, ReentrancyGuard {
    ...
    event SetPlatformRecipient(address payable recipient);
    event SetSellingPeriod(uint256 sellingPeriod);
    event RegOperator(address operator);
    event RemoveOperator(address operator);
    event SetPlatformFeeRatio(uint256 platformFeeRatio);

    constructor(address  payable _platformRecipient, uint256 _platformFeeRatio){
        require(_platformRecipient != address(0), "iNFTSpaceSale:Invalid platform recipient address");
    ...
    modifier onlyOperator() {
        require(operators[_msgSender()], "iNFTspaceSale: caller is not the operator");
        _;
    }
    ...
    function list(
        ...
        require(_saleTokenValidityById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");

        SaleToken memory saleToken = getSaleTokenById(_saleTokenId);
        require(_payCurrencyValidityById(_saleTokenId, _currencyId), "iNFTspaceSale:currency must supported");
        ...
        if(saleToken.standard == SALE_TOKEN_ERC721) { // ERC721
            ...
            require(iSaleToken.ownerOf(_tokenId) == msg.sender, "iNFTspaceSale:sale token is not owned by msg.sender");
    ...
    function updateList(
        ...
        require(_saleTokenExistsById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");
        ...
        _sellingNFTs[_saleTokenId][_assetId] = Selling(s.assetId, _saleTokenId, s.currencyId, s.price, s.saler, _specificBuyer, s.tokenId, _startTime, _stopTime, s.salerListNonce);

        emit UpdateListForSell(s.saler, s.assetId, _saleTokenId, _specificBuyer, _startTime, _stopTime);
    }

    function unList(uint256 _saleTokenId, uint256 _assetId) external {
        require(_saleTokenExistsById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");
    ...
    function buy(uint256 _saleTokenId, uint256 _assetId) external payable nonReentrant{
    ...
    function salerListNonce(address _saler) external view returns (uint256) {
        return _salerListNonce[_saler];
    }

    function sellingNFTsLength() external view returns (uint256) {
        return _sellingNFTsId.length;
    }

    function sellingNFTs(uint256 _index) external view returns (Selling memory) {
        return _sellingNFTs[_sellingNFTsId[_index].saleTokenId][_sellingNFTsId[_index].assetId];
    }

    function sellingNFTsId(uint256 _index) external view returns (SellingId memory) {
        return _sellingNFTsId[_index];
    }

    function sellingNFT(uint256 _saleTokenId, uint256 _assetId) external view returns (Selling memory) {
        return _sellingNFTs[_saleTokenId][_assetId];
    }

    function supportSaleTokensLength() external view returns (uint256) {
        return _supportSaleTokens.length;
    }

    function supportSaleTokens(uint256 _index) external view returns (SaleToken memory) {
        return _supportSaleTokens[_index];
    }

    function payCurrenciesLengthBySaleTokenId(uint256 _saleTokenId) external view returns (uint256) {
        return _supportCurrencies[_saleTokenId].length;
    }

    function payCurrenciesBySaleTokenId(uint256 _saleTokenId, uint256 _index) external view returns (PayCurrency memory) {
    ...
    function getSaleTokenByAddress(address _saleToken) external view returns (SaleToken memory) {
        for(uint i=0; i<_supportSaleTokens.length; i++){
    ...
    function getPayCurrencyByAddress(uint256 _saleTokenId, address _currency) external view returns (PayCurrency memory) {
        for(uint i=0; i<_supportCurrencies[_saleTokenId].length; i++){
    ...
    function setPlatformRecipient(address payable _recipient) external onlyOwner{
        require(_recipient != address(0), "iNFTSpaceSale:Invalid recipient address");
        platformRecipient = _recipient;

        emit SetPlatformRecipient(_recipient);
    }

    function setSellingPeriod(uint256 _sellingPeriod) external onlyOperator {
        require(_sellingPeriod > 0, "iNFTSpaceSale:Invalid selling period");
        maxSellingPeriod = _sellingPeriod;

        emit SetSellingPeriod(_sellingPeriod);
    }
    ...
    function setPlatformFeeRatio(uint256 _platformFeeRatio) external onlyOperator{
        platformFeeRatio = _platformFeeRatio;

        emit SetPlatformFeeRatio(_platformFeeRatio);
    }

    function regSaleToken(address _saleToken, uint256 _standard) external onlyOperator{
    ...
    function disableSaleToken(uint256 _saleTokenId) external onlyOperator{
        require(_saleTokenExistsById(_saleTokenId), "iNFTSpaceSale:saleToken must add yet");
    ...
    function enableSaleToken(uint256 _saleTokenId) external onlyOperator{
        require(_saleTokenExistsById(_saleTokenId), "iNFTSpaceSale:saleToken must add yet");
    ...
    function regPayCurrency(uint256 _saleTokenId, address _currency) external onlyOperator {
        require(_saleTokenValidityById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");
        require(!_payCurrencyExistsByAddress(_saleTokenId, _currency), "iNFTSpaceSale:currency already add yet");
    ...
    function disablePayCurrency(uint256 _saleTokenId, uint256 _currencyId) external onlyOperator {
        require(_saleTokenValidityById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");
        require(_payCurrencyExistsById(_saleTokenId, _currencyId), "iNFTSpaceSale:currency must add yet");
    ...
    function enablePayCurrency(uint256 _saleTokenId, uint256 _currencyId) external onlyOperator{
        require(_saleTokenValidityById(_saleTokenId), "iNFTspaceSale:saleToken must add yet");
        require(_payCurrencyExistsById(_saleTokenId, _currencyId), "iNFTSpaceSale:currency must add yet");
    ...
    function _payCurrencyValidityById(uint256 _saleTokenId, uint256 _currencyId) internal view returns (bool) {
        for(uint i=0; i<_supportCurrencies[_saleTokenId].length; i++){
            if(_supportCurrencies[_saleTokenId][i].id == _currencyId && _supportCurrencies[_saleTokenId][i].validity ){
                return true;
            }
        }
        return false;
    ...
    function _saleTokenValidityById(uint256 _saleTokenId) internal view returns (bool) {
        for(uint i=0; i<_supportSaleTokens.length; i++){
            if(_supportSaleTokens[i].id == _saleTokenId && _supportSaleTokens[i].validity){
                return true;
            }
        }
        return false;
