...
contract WasteTrader is OwnableUpgradeable, ReentrancyGuardUpgradeable, IWasteToCash {
    ...
    mapping(address => Order[]) private new_wasteOrders;
    ...
    address pixt;
    address beneficiary;
    uint pixtSpeedupSplitBps;
    uint pixtSpeedupCost;
    ...
    function placeWasteOrder(uint256 _wasteAmount) external nonReentrant {
        if (s_gwsTime > 0) {
            Order[] memory wasteOrders = new_wasteOrders[msg.sender];
    ...
    function createNewOrder(uint256 _wasteAmount) internal {
        ...
        new_wasteOrders[msg.sender].push(Order({
            orderId: orderId,
    ...
    function _removeOrder(uint _index) internal {
        if (_index < new_wasteOrders[msg.sender].length -1)
            new_wasteOrders[msg.sender][_index] = new_wasteOrders[msg.sender][new_wasteOrders[msg.sender].length-1];
        new_wasteOrders[msg.sender].pop();
    }

    /// @inheritdoc ITrader
    function claimOrder(uint _orderId) external override {
        Order[] storage wasteOrders = new_wasteOrders[msg.sender];
    ...
    /// @inheritdoc ITrader
    function getOrders(address _player) external view returns (Order[] memory) {
        Order[] memory wasteOrders = new_wasteOrders[_player];
        return wasteOrders;
    }
}
