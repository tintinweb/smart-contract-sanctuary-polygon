...
import "./TCC.sol";
import "./interfaces/ITBE.sol";
import {Project} from "./ThalloStructs.sol";
...
/**
 *@title ThalloBridgeRegistry
 *@notice Contract for creating projects and vintages under those projects
 */
contract ThalloBridgeRegistry is UUPSUpgradeable, OwnableUpgradeable {
    uint24 public currentProjectId;
    address public thalloBridgeEvents;

    mapping(uint24 => Project) public projects;
    mapping(bytes32 => bool) public projectRegistered;
    mapping(address => bool) public registryCustodian;

    event NewProject(Project newProject);
    event NewVintage(address tcc, uint24 indexed projectId, uint16 vintage);
    ...
    function initialize(address _owner, address _thalloBridgeEvents) external initializer {
        __Ownable_init();
        transferOwnership(_owner);
        thalloBridgeEvents = _thalloBridgeEvents;
        registryCustodian[_owner] = true;
    }
    ...
    modifier onlyCustodian() {
        require(registryCustodian[msg.sender], "TBR:NC");
        _;
    }

    function addCustodian(address _who) external onlyOwner {
        registryCustodian[_who] = true;
        emit CustodianAdded(_who);
    }

    function removeCustodian(address _who) external onlyOwner {
        registryCustodian[_who] = false;
        emit CustodianRemoved(_who);
    }

    /**
     *@notice Creates a new project
     *@param _newProject Project Struct. See ThalloStructs.sol for details.
     *@dev Registering the project with the hash of the registry+registryId prevents adding duplicate projects
     */
    function createProject(Project memory _newProject) external onlyCustodian {
        bytes32 projKey = keccak256(abi.encodePacked(_newProject.registry, _newProject.registryProjectId));
        require(!projectRegistered[projKey], "TBR:CP:PE");
        unchecked {
            ++currentProjectId;
        }
        _newProject.id = currentProjectId;
        projects[_newProject.id] = _newProject;
        projectRegistered[projKey] = true;
        emit NewProject(projects[_newProject.id]);
    }

    /**
     *@notice Creates a new vintage for a project
     *@param _projectId The id of the project
     *@param _vintage The vintage year
     *@param _symbol The symbol for the TCC contract token
     *@param _name The name for the TCC contract token
     *@param _metadata Any additional metadata for the vintage
     *@dev This does not mint any tokens on chain, it only deploys the TCC contract for the vintage
     */
    function createVintage(
        ...
        require(_projectId <= currentProjectId, "TBR:CV:PDNE");
        require(ITBE(thalloBridgeEvents).getVintageAddress(vintageKey) == address(0), "TBR:CV:VE");
        TCC newTcc = new TCC();
        ERC1967Proxy proxy = new ERC1967Proxy(
            address(newTcc),
            abi.encodeWithSelector(
                TCC(address(0)).initialize.selector,
                owner(),
                thalloBridgeEvents,
                _projectId,
                _vintage,
                projects[_projectId].registry,
                _name,
                _symbol
            )
        );
        ITBE(thalloBridgeEvents).setVintageAddress(vintageKey, address(proxy));
        ITBE(thalloBridgeEvents).setVintageMetadata(vintageKey, _metadata);
        emit NewVintage(address(proxy), _projectId, _vintage);
    }

    /**
     *@notice Edits the data stored for a particular project
     *@param _editedProject Project struct with new project details
     *@dev Intended only for use when the underlying registry changes their identification standards
     */
    function editProject(Project memory _editedProject) external onlyCustodian {
        projects[_editedProject.id] = _editedProject;
    }
...
/**
 *@title TCC
 *@notice Contract representing a tokenized carbon credit for a project & vintage, custodied by Thallo
 */
contract TCC is ERC20Upgradeable, UUPSUpgradeable, OwnableUpgradeable {
    ...
    event RequestUncustody(address requestor, uint256 quantity);
    ...
    modifier onlyBridge() {
        require(thalloBridgeEvents == msg.sender, "TCC:NB");
        _;
    }

    /**
     *@notice Issue vintage tokens to a custodian address
     *@param _to Thallo custodian address to issue tokens to
     *@param _quantity Number of tokens to issue
     *@dev Amount must start as a whole number of credits
     */
    function issueTokens(address _to, uint256 _quantity) external onlyBridge {
        require(_quantity >= 1 ether && _quantity % 1 ether == 0, "TCC:IT:NWN");
        _mint(_to, _quantity);
    }

    /**
     *@notice Retire vintage tokens and call the bridge to mint a proof of retirement NFT
     *@param _to The address to mint the proof of retirement NFT to -- this allows any address to retire tokens on behalf of another address
     *@param _quantity Number of tokens to retire
     *@dev Amount must be a whole number of credits
     */
    function retire(address _to, uint256 _quantity) external {
        require(_quantity >= 1 ether && _quantity % 1 ether == 0, "TCC:R:NWN");
        _burn(msg.sender, _quantity);
        ITBE(thalloBridgeEvents).retireVintageTokens(projectId, registry, vintage, _quantity, _to);
    }

    /**
     *@notice Request an uncustody of vintage tokens
     *@param _quantity Number of tokens to uncustody
     *@dev The requestor must have an account on the corresponding registry. We escrow tokens and do not burn right away for this reason.
     */
    function requestUncustody(uint256 _quantity) external {
        require(_quantity >= 1 ether && _quantity % 1 ether == 0, "TCC:RU:NWN");
        _transfer(msg.sender, address(this), _quantity);
        escrowedUncustody[msg.sender] += _quantity;
        emit RequestUncustody(msg.sender, _quantity);
    }

    /**
     *@notice Finalize the uncustody request of tokens and burn them
     *@dev The original requestor will be confirmed to have an account on the corresponding registry and the tokens will be transfered to them there.
     */
    function finalizeUncustody(address _requestor, uint256 _quantity) external onlyBridge {
        require(escrowedUncustody[_requestor] >= _quantity, "TCC:FU:NEE");
        escrowedUncustody[_requestor] -= _quantity;
        _burn(address(this), _quantity);
    }

    /**
     *@notice Return tokens to the account that requested uncustody
     *@dev If the requestor does not have an account on the corresponding registry, the tokens will be returned as the request cannot be completed.
     */
    function returnUncustody(address _requestor) external onlyBridge {
        uint256 quantity = escrowedUncustody[_requestor];
        escrowedUncustody[_requestor] = 0;
        _transfer(address(this), _requestor, quantity);
    }
}
...
interface ITBE {
    ...
    function setVintageMetadata(bytes32 _vintageKey, string memory _metadata) external;
}
