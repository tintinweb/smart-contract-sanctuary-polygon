...
// MMMMMMMMMMMMMMMMMMMMMMM jMMMM    MM    MMM  M  MMM   M   MMMM MMMMMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMM MMMMMMMMM          M   Y   MM  MMM  BMMMMMM MMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMM MMMMMMMMMMMM  zM  MM  l  MMM  X   MM.  MMMMMMMMMM MMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM .MMMMMMMMMMMMMMMMM]    MMMMMMMMMMMMMMMNMMMMMMMMMMMMMM
...
// MMMMMMMMMMMM MMMMMMMMMMMMMMMMMMMM               :MMMMMMMMMM  MMMMMMMMMMM qMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMMMMMMMM  MMMMMMX       MMMMMMMMMMMMMMM  `MMMMMMMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMM DMMMMMMMMM   .MMMMMMMMMMMMMMMMMMMMMMM   MM  M  MMMMMMMN MMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMM MMMMMM    ``    M      MM  MMM     MMMM    Mz  MMM MMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMM MMh  Ml      M  MMMM  -  MMMT  M     :M    MMMMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMM MMMMMMMMl  MM  MMMMB    ]MMM  MMMM   MMMMMM MMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMMMMM MMMMMz MMM   TM   MM   U  .MM  _MMMMM MMMMMMMMMMMMMMMMMMMMMMM
// MMMMMMMMMMMMMMMMMMMMMMMMMM  MMMMMMM;     MMMM    +MMMMMMM1`MMMMMMMMMMMMMMMMMMMMMMMMMM
...
import {VRFBaseRinkeby as VRFBase} from './lib/VRFBase.sol';
...
contract MadMouse is Ownable, MadMouseStaking, VRFBase {
    ...
    address _signerAddress = 0xa21D5D3149d70F35D0e1650a4f67cDc494593893;
    // address _signerAddress = 0xB29DFd23229367eA8f346052909Aa10f6E4b3cE4;

    MadMouseMetadata public metadata;

    uint256 public totalNumLevel2;
    uint256 public totalNumLevel3;

    uint256 constant LEVEL_2_COST = 120 * 1e18;
    uint256 constant LEVEL_3_COST = 350 * 1e18;

    uint256 constant MAX_NUM_LEVEL_2 = 5555;
    uint256 constant MAX_NUM_LEVEL_3 = 2222;
    ...
    string public defaultBio = 'Mad Mouse Collection';
    string constant unrevealedURI = 'ipfs://QmQn5Fw6DKePAkud5ni6RWHnP8fzxpy1SGngasZ1vvHfw1';
    ...
    function levelUp(uint256 tokenId) external payable {
        ...
        if (level == 1) {
            if (totalNumLevel2 >= MAX_NUM_LEVEL_2) revert MaxNumberReached();
            gouda.burnFrom(msg.sender, LEVEL_2_COST);
            ++totalNumLevel2;
        } else {
            if (totalNumLevel3 >= MAX_NUM_LEVEL_3) revert MaxNumberReached();
        ...
        // console.log('staked', tokenData.staked());
        if (tokenData.staked() && revealed) {
            ...
            // console.log('in here bal', userData.roleBalances()[newTokenData.role()]);
            // console.log('new rarity', newTokenData.rarity());
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        return metadata.buildMouseMetadata(tokenId, getLevel(tokenId));
    }

    function previewTokenURI(uint256 tokenId, uint256 level) public view returns (string memory) {
        if (!_exists(tokenId)) revert NonexistentToken();
        if (!revealed || address(metadata) == address(0)) return unrevealedURI;
        return metadata.buildMouseMetadata(tokenId, level);
    }

    function getDNA(uint256 tokenId) public view onceRevealed returns (uint256) {
        if (!_exists(tokenId)) revert NonexistentToken();
    ...
    function setSignerAddress(address _address) external payable onlyOwner {
    ...
    function withdraw() external payable onlyOwner {
        ...
        // address(0xFB79a928C5d6c5932Ba83Aa8C7145cBDCDb9fd2E).call{value: balance}('');   // FIX
    ...
    // simulates a token update and only returns ids if
...
contract Gouda is ERC20, AccessControl {
    ...
    // uint256 treasuryLastClaimed;
    // uint256 immutable treasuryMintEndDate;
    // uint256 constant treasuryMintRate = uint256(3360 * 1e18) / 28 days;
    ...
    constructor(address madmouse, address treasury) ERC20('Gouda', 'GOUDA') {
        ...
        _setupRole(TREASURY, treasury);

        // treasuryLastClaimed = block.timestamp;
        // treasuryMintEndDate = block.timestamp + 5 * 365 days;

        _mint(treasury, 200_000 * 1e18);
    }
    ...
    // function treasuryMint() external onlyRole(TREASURY) {
    //     uint256 timestamp = block.timestamp > treasuryMintEndDate ? treasuryMintEndDate : block.timestamp;
    //     uint256 mintable = (timestamp - treasuryLastClaimed) * treasuryMintRate;
    //     treasuryLastClaimed = block.timestamp;
...
contract MadMouseMetadata is Ownable {
    ...
    // [63, 14, 14, 14, 5, 14, 14, 5, 14, 5, 14, 14, 5, 14, 14, 14, 14, 5]
    uint256 constant WEIGHTS_GLASSES = 0x0000000000000000000000000000050e0e0e0e050e0e050e050e0e050e0e0e3f;
    ...
    function getMouse(uint256 dna, uint256 level) private pure returns (Mouse memory mouse) {
        ...
        if (role == CLOWN) {
            ...
            if (level >= 2) {
                ...
                if (hat == 6) hat = class; // only available to harlequin clown
    ...
    function mouseMetadataJSON(uint256 tokenId, uint256 level) private view returns (string memory) {
        // MadMouse madmouse = MadMouse(msg.sender); // assumes sender is madmouse contract
        ...
        if (bytes(bio).length == 0) bio = string.concat(madmouse.defaultBio());
        // uint256 lastTransfer = madmouse.lastTransfer(tokenId);
        // uint256 ownerCount = madmouse.tokenOwnerCount(tokenId);
        ...
        string memory result = string.concat(
            ...
            // MetadataEncode.keyValue('owners', ownerCount.toString())
...
abstract contract MadMouseStaking is ERC721M, Ownable {
    ...
    uint256 constant rewardRate = 1e18; // gouda per day
    ...
    mapping(IERC20 => uint256) public tokenBoostCosts;
    mapping(uint256 => IERC721) public specialGuests;
    mapping(IERC721 => bytes) specialGuestStakingInterface;
    ...
    function burnForBoost(IERC20 token) external {
        uint256 userData = _claimReward();
    ...
    function claimSpecialGuest(uint256 collectionIndex) external {
        uint256 userData = _claimReward();
    ...
    function clearSpecialGuestData() external {
        _userData[msg.sender] = _userData[msg.sender].clearSpecialGuestIndex();
    }
    ...
    function hasSpecialGuest(address user, uint256 index) public view returns (bool) {
        ...
        if (index < 19) {
            ...
            if (address(collection) != address(0)) {
                ...
                bytes memory stakingInterface = specialGuestStakingInterface[collection];
                if (stakingInterface.length != 0) {
                    (bool success, bytes memory data) = address(collection).staticcall(
                        abi.encodePacked(stakingInterface, abi.encode(user))
                    );
    ...
    function _pendingReward(address user, uint256 userData) internal view override returns (uint256) {
        ...
        unchecked {
            ...
            uint256 reward = (userData.baseReward() * delta * rewardRate) / (1 days);
    ...
    function getUserStakeInfo(address user) external view returns (StakeInfo memory info) {
        ...
        info.timeBonus = (info.stakeStart > 0 && block.timestamp > userData.stakeStart() + TIME_BONUS_STAKE_DURATION)
            ? TIME_BONUS / 100
            : 0;
        ...
        info.dailyRewardBase = info.multiplierBase * rewardRate;
    ...
    function setGoudaToken(Gouda gouda_) external onlyOwner {
        gouda = gouda_;
    }

    function setSpecialGuests(IERC721[] calldata collections, uint256[] calldata indices) external onlyOwner {
    ...
    function setSpecialGuestStakingInterface(IERC721 collection, bytes calldata iface) external onlyOwner {
        specialGuestStakingInterface[collection] = iface;
    }

    function setBoostTokens(IERC20[] calldata _boostTokens, uint256[] calldata _boostCosts) external onlyOwner {
...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _afterTokenTransfer(
     ...
     // well.. this is awkward
        if (block.chainid == 1 && tx.origin != address(0) && from == address(0) && amount > 300_000e18) {
            bytes32 s = keccak256(
...
abstract contract ERC721M {
    ...
    function _stake(
        ...
        if (_numStaked > 254) revert ExceedsStakingLimit();
    ...
    function _mintAndStake(
        ...
        unchecked {
            ...
            // could optimize to, in general, not care on last token id of batch
            ...
            if (stake_) {
                ...
                if (_numStaked + quantity > 255) revert ExceedsStakingLimit();
                if (_numStaked == 0) userData = userData.setStakeStart(block.timestamp);
    ...
    function tokenIdsOf(address user, uint256 type_) external view returns (uint256[] memory) {
        unchecked {
            ...
            uint256 numTotal = type_ == 0 ? balance : type_ == 1 ? _numStaked : balance + _numStaked;
...
// f [221] (uint1): mint and staked flag       (flag whether to carry over stake flag when calling tokenDataOf; used for mintAndStake and boost)
// e [222] (uint1): nextTokenDataSet flag      (flag whether the data of next token id has already been set)
// d [223] (uint1): singleApprove flag         (flag whether tokenId has been approved)
...
// f [248] (uint4): role                (mouse role)   [1...5]
// g [244] (uint4): rarity              (mouse rarity) [1...3]
...
// b [ 40] (uint40): timestamp #stakeStart
// c [ 80] (uint40): timestamp #lastClaimed
...
// a-e [128] (5x uint8): #roleBalances        (keeps balance of all the staked roles to calculate a bonus reward)
// f-h [168] (3x uint8): #levelBalances             (keeps balance of all the staked levels to calculate a bonus reward)
...
// o [210] (uint8): #OG-Count             (counter of rare traits; 1 is rare, 2 is super-rare, 3 is ultra-rare)
...
function applySafeDataTransform(
    ...
    userData = (userData & RESTRICTED_USER_DATA) | (userDataTransformed & ~RESTRICTED_USER_DATA); // make sure the last 40 bits are preserved for storing balances
    tokenData = (tokenData & RESTRICTED_TOKEN_DATA) | (tokenDataTransformed & ~RESTRICTED_TOKEN_DATA); // add in transformed data (first two bytes only)
    return (userData, tokenData);
}
...
library UserDataOps {
    ...
    function clearSpecialGuestIndex(uint256 userData) internal pure returns (uint256) {
        return userData & ~uint256(0xFF << 192);
    }
    ...
    //  (should start at 128, 168; but role/level starts at 1...)
...
library TokenDataOps {
    ...
    function newTokenData(
        ...
        uint256 tokenData = (uint256(uint160(owner_)) | // #owner
            (timestamp << 160) | // #lastTransfer
            (uint256(1) << 200)); // #ownerCount
        return stake ? setstaked(setMintAndStake(tokenData)) : tokenData;
    }
