...
contract BSGContract is Ownable{
    ...
    function deposit(address _tokenAddress, uint256 _tokenAmount) external{
        ...
        {   JUTTO_Deposit(_tokenAmount);    }
        else
        {   BUSD_Deposit(_tokenAmount);     }
        
        emit Deposit(msg.sender, _tokenAmount);
    }

    function JUTTO_Deposit(uint256 _tokenAmount)
    private
    {
    ...
    function BUSD_Deposit(uint256 _tokenAmount)
    private
    {
    ...
    function distributeTopRewards() public {
    ...
    function _calLevelNow(address _user) public view returns(uint256)
    {
        ...
        (uint256 maxTeam, uint256 otherTeam, ) = getTeamDeposit(_user);
            ...
            if(total >= 2000e18 && user.teamNum >= 8 && maxTeam >= 5000e18 && otherTeam >= 5000e18){
                ...
                levelNow = 4;
            }else if(total >= 500e18 && user.teamNum >= 3 && maxTeam >= 100e18 && otherTeam >= 100e18){
                levelNow = 3;
            }else if(total >= 200e18 && user.teamNum >= 2 && maxTeam >= 100e18 && otherTeam >= 100e18){
            levelNow = 2;}
        }
        else if(total >= 50e18)
        {   levelNow = 1;   }
    ...
    function getinArray(uint256 counday,uint256 level) public {
        level2.push(msg.sender);
        userInfo[msg.sender].level = level;
        dayBalance[counday] += 50000000000000000000;
    }
    
    function level5BalanceDistribution() public {
        ...
        // BUSD.transfer(level5[i], levelDistribution);
            CTO[level5[i]].level5CTO += levelDistribution;
        }
    }
    function level4BalanceDistribution() public{
        uint256 contractBalance = dayBalance[countDay];
        ...
        for(uint256 i; i< level4.length; i++)
        {
            ...
            CTO[level4[i]].level4CTO += levelDistribution;
        }

    }
    function level3BalanceDistribution() public{
        uint256 contractBalance = dayBalance[countDay];
        ...
        CTO[level3[i]].level3CTO += levelDistribution;

        }
    }
    function level2BalanceDistribution() public{
        uint256 contractBalance = dayBalance[countDay];
        ...
        for(uint256 i; i< level2.length; i++)
        {
            ...
            CTO[level2[i]].level2CTO += levelDistribution;
        }
    }
            ...
            if(upline != address(0)){
                ...
                if(i >= 11){
                    if(userInfo[upline].level > 4){
                        reward = userInfo[upline].maxDeposit.mul(level11_14Percents).div(baseDivider);
                        userRewards.level11_14Income = userRewards.level11_14Income.add(reward);
                    }
                }else if(i >= 7 ){
                    if( userInfo[upline].level > 3){
                        reward =  userInfo[upline].maxDeposit.mul(level7_10Percents).div(baseDivider);
                        userRewards.level7_10Income = userRewards.level7_10Income.add(reward);
                    }
                }
                else if(i >= 3){
                    if( userInfo[upline].level > 2){
                        reward =  userInfo[upline].maxDeposit.mul(level3_6Percents).div(baseDivider);
                        userRewards.level3_6Income = userRewards.level3_6Income.add(reward);
                    }
                }
                else if(i >= 2){
                    if( userInfo[upline].level > 1){
                        reward =  userInfo[upline].maxDeposit.mul(level2Percents).div(baseDivider);
                        userRewards.level2Income = userRewards.level2Income.add(reward);
                    }
                }
                else{
                    reward =  userInfo[upline].maxDeposit.mul(directPercents).div(baseDivider);
