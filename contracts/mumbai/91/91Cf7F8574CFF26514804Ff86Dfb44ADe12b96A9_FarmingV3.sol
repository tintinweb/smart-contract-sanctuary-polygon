...
contract FarmingV3 is IFarming, Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  ...
  */
  ...
  function depositUniV3(
    ...
    INonfungiblePositionManager(pool.token).safeTransferFrom(msg.sender, address(this), _tokenId);
  ...
  */
  ...
  function withdrawUniV3(uint _tokenId, bool _claimRewards) external virtual returns (uint) {
    address _userAddress = msg.sender;
    (IUniswapV3Pool uniPool, , , ) = NFTPositionInfo.getPositionInfo(
      factory,
      nonfungiblePositionManager,
