    ...
    VRFConsumerBaseV2
{
    ...
    function addSubcollection(
        ...
        require(bytes(params.defaultTokenCID).length != 0, "Missing default token CID");
        require(bytes(params.upgradedTokenCID).length != 0, "Missing default token CID");
    ...
    function upgradeToken(
        ...
        _setTokenURI(tokenId, string(abi.encodePacked(_baseURL, upgradedTokenCID, "/", tokenId.toString(), ".json")));
    ...
    function tokenURI(
        ...
        string memory defaultTokenCID = collection.params.defaultTokenCID;
        if (!collection.isRevealed) {
            return string(abi.encodePacked(_baseURL, defaultTokenCID));
        }
        if (bytes(tokenCID).length > 0) {
            return string(abi.encodePacked(_baseURL, tokenCID));
        }
        return
            bytes(collection.params.baseCID).length > 0
                ? string(abi.encodePacked(_baseURL, baseCID, "/", tokenId.toString(), ".json"))
                : string(abi.encodePacked(_baseURL, defaultTokenCID));
    }
     ...
     */
    function _setTokenURI(uint256 tokenId, string memory tokenCID) private {
