...
contract Trade {
    ...
    constructor (uint8 _BuyerFeePermille, uint8 _sellererFeePermille, uint8 _swappingFee, TransferProxy _transferProxy) {
    ...
    function buyAsset(Order calldata order, bool _import, Sign calldata sign) external returns(Fee memory) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
    ...
    function executeBid(Order calldata order, bool _import, Sign calldata sign) external returns(bool) {
        require(!usedNonce[sign.nonce],"Nonce : Invalid Nonce");
        usedNonce[sign.nonce] = true;
        Fee memory fee = getFees(order, _import);
        verifyBuyerSign(order.buyer, order.tokenId, order.amount, order.erc20Address, order.nftAddress, order.qty, sign);
