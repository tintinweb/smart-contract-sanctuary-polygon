 ...
 * Has functions to create package and update package.
...
contract Packages is Ownable, AccessControl {
    ...
    uint256 public totalAdminPortion;
    ...
    event AdminPortionWithdrawn(address adminAddress, uint256 withdrawAmount);

    /**
     * @dev Set the address of interface balance vault, voting, staking.
    ...
    /**
    * @dev Allow when package created.
    */
    modifier packageCreated(string calldata _packageId) {
        require(
            bytes(packageByPackageId[_packageId].packageId).length > 0,
            "[Package.packageCreated] Package not create"
        );
        _;
    }
    ...
    function createPackage(
        ...
        uint256 price = (_votePortion + _commentPortion + _stakePortion + _adminPortion);
        packageByPackageId[_packageId] = Package(
            _packageId,
            _votePortion,
            _commentPortion,
            _stakePortion,
    ...
    function updatePackage(
        ...
        uint256 _stakePortion,
        uint256 _adminPortion
    ) external onlyRole(WORKER) packageCreated(_packageId) {
        require(
            bytes(packageByPackageId[_packageId].packageId).length != 0,
    ...
    function approvePackage(
        ...
        if(package.stakePortion > 0) {
            stakingLevel.addPoolReward(_userAddress, package.stakePortion);
        }
        // Admin
        if(package.adminPortion > 0) {
            balanceVault.payWithUpo(_userAddress, package.adminPortion);
            totalAdminPortion += package.adminPortion;
        }

        emit PackageApproved(_packageId, _contestId, _projectId, _userAddress);
    }

    function getPackageInfo(string calldata _packageId)
        external
        view
        returns (
            Package memory
    ...
    {
        return packageByPackageId[_packageId];
    }
    ...
    /**
     * @dev Withdraw admin portion stored.
     */
    function adminPortionWithdraw() public onlyRole(WORKER) {
        uint256 withdrawAmount = totalAdminPortion;
        require(
            withdrawAmount > 0,
            "[StakingPayment.adminPortionWithdraw] Insufficient admin portion to withdraw"
        );
        totalAdminPortion = 0;
        balanceVault.transferUpoToAddress(msg.sender, withdrawAmount);

        emit AdminPortionWithdrawn(msg.sender, withdrawAmount);
    }
}
...
interface IVoting{
    function addContestEntry(string calldata _contestId, string calldata _projectId, address _userAddress, uint256 _entryVoteReward, uint256 _entryCommentReward) external;
}
