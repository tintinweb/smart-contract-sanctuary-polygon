...
//SilverCard
abstract contract BundotMembershipSilverCard is ERC721Burnable, Ownable, ReentrancyGuarded{
...
contract RamperBridgeforBundotSilverCard721 is IRamperBridgeInterface721 {

BundotMembershipSilverCard public silvercardaddress;

constructor(BundotMembershipSilverCard _existingcontractaddress){
    silvercardaddress = _existingcontractaddress;
}
...
function price() external view returns (uint256)
{
    return silvercardaddress.getPrice();
}

function mint(address _userWallet, uint256 _quantity) override external payable {
        require(_quantity == 1);
        silvercardaddress.mintToken{value:msg.value}(_userWallet);
    }
...
/// @notice Helper Transfer function to allow multiple tokens to be transferred at once
    function safeBulkTransferFrom(address _from, address _to, uint256[] memory _tokenIds) external{
        for(uint256 i;i < _tokenIds.length; i++)
               silvercardaddress.safeTransferFrom(_from,_to,_tokenIds[i]);
    } 
