...
contract HSTswap {
    ...
    function swap(address _from, address _to, uint256 _amount) public security{
        ...
        if(coins[_to].isNative==true){
            ...
            scaledAmount = _amount.mul(coins[_from].sp).div(100);
        }

        if(_to==bnb){
            require(scaledAmount<=address(this).balance,"Insufficient coins!");
            BEP20(_from).transferFrom(msg.sender,address(this),_amount);
            slippage(_to,scaledAmount.mul(slippageCharge).div(100));
            msg.sender.transfer(scaledAmount.mul(100-slippageCharge).div(100));
        }
        else{
            ...
            slippage(_to,scaledAmount.mul(slippageCharge).div(100));
            BEP20(_to).transfer(msg.sender,scaledAmount.mul(100-slippageCharge).div(100));
        }
    ...
    function slippage(address _coin, uint256 _amount) internal{
        uint256 _slip = _amount.div(5);
        if(_coin==bnb){
            slipper1.transfer(_slip);
            slipper2.transfer(_slip);
            slipper3.transfer(_slip);
            slipper4.transfer(_slip);
            slipper5.transfer(_slip);
        }
        else{
            BEP20(_coin).transfer(slipper1,_slip);
            BEP20(_coin).transfer(slipper2,_slip);
            BEP20(_coin).transfer(slipper3,_slip);
            BEP20(_coin).transfer(slipper4,_slip);
            BEP20(_coin).transfer(slipper5,_slip);
        }
    }
