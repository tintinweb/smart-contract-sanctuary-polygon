...
contract Zekra {
    ...
    struct InfoZekr{
        uint  UniqId;//شناسه کاربر
        uint8 cZekr;//شمارنده ذکر
        uint8 cPay;//پرداخت چندم
        uint8 constNumberZekr;//تعداد قابل شمارش
        uint  startTime;//زمان شروع
        uint8  cPeriod;//روز چندم
    ...
    function placeOrder(string calldata _zekrCode) payable external {
        require(msg.value >= 50, "Payment amount must be greater than 0");
