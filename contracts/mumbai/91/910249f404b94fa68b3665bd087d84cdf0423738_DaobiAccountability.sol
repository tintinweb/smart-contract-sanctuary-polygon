...
contract DaobiAccountability is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint public constant DAY_IN_SECONDS = 1000;    //86400 #CHANGE BEFORE PRODUCTION!!
    ...
    function mint(address _to, address _target) private {
        ...
        _setTokenURI(tokenId,URIcontract.generateURI(_target, grudgeBook[_target].accuser, daobi.chancellor(), uint16(grudgeBook[_target].supporters.length), uint32(block.timestamp % 2**32), generateSupporterString(_target) ));
    }
...
interface IDaobiAccountabilityURIs {
  ...
  function generateURI ( address _target, address _accuser, address _chancellor, uint16 _numSupporters, uint32 _timestamp, string memory _supporterList) external view returns ( string memory );
