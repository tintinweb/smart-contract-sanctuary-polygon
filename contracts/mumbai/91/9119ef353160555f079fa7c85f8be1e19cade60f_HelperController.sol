...
contract ERC20DB is   Ownable  , AccessControl , Pausable{
    ...
    mapping ( uint256 => address[] ) private Coins ; 
    // 1 for staking 
    // 2 for debtbased
    // 3 for DAO
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function addCoinsForOtherPlatform ( uint256 id , address coinName  )  onlyContractOrAdmin whenNotPaused public
    {
        //if 
        if ( checkCoinExistAlready ( id ,coinName ) == false  && getERC20Info (coinName).enabled == true  ) 
        {
            Coins[id].push(coinName);
        }
    }

    function getCoingForOtherPlatforms ( uint256 id ) public view returns (  ERC20Coin[] memory  )
    {
        ERC20Coin[] memory result = new ERC20Coin[]( Coins[id].length );
        for ( uint256 i = 0 ; i <  Coins[id].length ; i++  )
        {
            result[i] = getERC20Info( Coins[id][i] ) ;
        }
        return result;
    }

    function removeCoinForOtherPlatforms (  uint256 id , address coinAddress  ) onlyContractOrAdmin whenNotPaused public returns ( bool )
    {
        bool result = false ; 
        for ( uint256 i = 0 ; i < Coins[id].length ; i++ )
        {
            if ( Coins[id][i] == coinAddress) 
            {
                Coins[id][i] = Coins[id][ Coins[id].length.sub(1) ] ; 
                Coins[id].pop();
                result = true ; 
                break ; 
            }
    ...
    function checkCoinExistAlready (  uint256 id , address coinAddress  ) public view returns ( bool )
    {
        bool result = false ; 
        for ( uint256 i = 0 ; i < Coins[id].length ; i++ )
        {
    ...
    function getStakeCoin () public view returns (  ERC20Coin memory )
    {
...
// File: contracts/HelperController.sol
...
contract HelperController is   Ownable  , AccessControl , Pausable{
  ...
  /// @dev Add an account to the admin role. Restricted to admins.
    ...
    function abortProposal ( uint256 index  ) public   whenNotPaused    returns (  uint256 )
    {
        require( msg.sender == proposaldb.getProposal(index).applicant , "E1");        
        return proposaldb.abortProposal(index);
    }



    // function updateProposal ( uint256 index , ProposalDB.Proposal memory proposal ) public whenNotPaused
    // {
    //     require( msg.sender == proposaldb.getProposal(index).applicant , "E1");        
    //     proposaldb.updateProposal( index , proposal    );
    // }

    // function changeSubmitDate ( uint256 index , uint256 submitDate  ) public    returns (  uint256 )
    // {
    //     require( msg.sender == proposaldb.getProposal(index).applicant , "E1");        
    //     return proposaldb.changeSubmitDate( index , submitDate ) ;
    // }
    ...
    function getInvestors( uint256 page , uint256  size) public view returns ( InvestorDB.Investor[] memory  )
    {
        return investordb.getInvestors( page , size );
    }
    ...
    // function getBankAccountByAddress(   uint256 page , uint256  size , address applicant ) public view returns ( uint256[] memory )
    // {
    //     return  bankdb.getBankAccountByAddress( page , size  , applicant) ; 
    ...
    // function getProposalFundInfo( uint256 index ) public view returns ( BankDB.ProposalFundInfo memory )
    // {
    //     return bankdb.getProposalFundInfo(index);
    // }


    function getProposalsProfitReturn( uint256 index ,  uint256 page , uint256  size) public view returns ( ProposalDB.returnProfit[] memory  )
    {
        return proposaldb.getProposalsProfitReturn( index , page ,size );
    }


    function getProposals( uint256 page , uint256  size) public view returns ( ProposalDB.Proposal[] memory  )
    {
        return proposaldb.getProposals(   page ,size );

    }

    function getProposal( uint256 index) public view returns ( ProposalDB.Proposal memory  )
    {
        return  proposaldb.getProposal(index);

    }

    function getProposalReturnProfitById ( uint256 index , uint256 paymentID) public  view returns  ( ProposalDB.returnProfit memory )
    {
        return proposaldb.getProposalReturnProfitById( index , paymentID )  ;
    } 
    ...
    function getBankAccountByProposal(   uint256 page , uint256  size , uint256 proposalID ) public view returns ( uint256[] memory )
    {
        ...
        return  bankdb.getBankAccountByProposal( page , size , proposalID  ) ;

    }

    function getBankAccountInfoByProposal(   uint256 page , uint256  size , uint256 proposalID ) public view returns ( BankDB.BankAccount[] memory )
    {
        //require ( page > 0 && size > 0  , "page/size shoud not be 0");

        return  bankdb.getBankAccountInfoByProposal( page , size , proposalID  ) ;

    }

    function getBankAccountInfoByAddress( uint256 page , uint256  size ,  address applicant ) public view returns ( BankDB.BankAccount[] memory )
    {

        return  bankdb.getBankAccountInfoByAddress( page , size , applicant  ) ;
        

    }


    function getBankAccountByAddressAndProposalId ( address applicant , uint256 proposalId ) public view returns ( BankDB.BankAccount memory )
    {
        return  bankdb.getBankAccountByAddressAndProposalId( applicant , proposalId  ) ;
    }
    ...
    function getPremiumProposals (  uint256 page , uint256  size  ) public view returns ( ProposalDB.Proposal[] memory  )
    {

        require ( page > 0 && size > 0  , "page/size != 0");
        ProposalDB.Proposal[] memory  result  = new  ProposalDB.Proposal[] (  size );
        uint256[] memory premiumList = proposaldb.getPremiumList() ;
        uint256 counter = 1 ;  

        uint256 index = 0 ; 
        for ( uint256 i = 0 ; i < proposaldb.getPremiumList().length  ; i++ )
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if ( proposaldb.getProposal( premiumList[i] ).exist == true && proposaldb.getProposal( premiumList[i] ).premium == true && proposaldb.getProposal( premiumList[i] ).enabled == true  )
            {

                if (counter > page.sub(1).mul(size) &&  counter <= page.mul(size) )
                {
                    ProposalDB.Proposal memory temp = proposaldb.getProposal( premiumList[i]  ) ;
                    result[index] = temp ;
                    index = index.add(1);   
    ...
    function getFundableProposals (  uint256 page , uint256  size  ) public view returns ( ProposalDB.Proposal[] memory  )
    {
        require ( page > 0 && size > 0  , "page/size != 0");
        ProposalDB.Proposal[] memory  result  = new  ProposalDB.Proposal[] (  size );
        uint256 counter = 1 ;  
        ...
        for ( uint256 i = 1 ; i <= proposaldb.getProposalCount()  ; i++ )
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if ( proposaldb.getProposal( i ).exist == true && proposaldb.getProposal( i ).didPass == true && proposaldb.getProposal( i ).Locked >= 1 &&  proposaldb.getProposal( i ).Locked <= 2 && proposaldb.getProposal( i ).enabled == true  )
            {

                if (counter > page.sub(1).mul(size) &&  counter <= page.mul(size) )
                {
                    ProposalDB.Proposal memory temp = proposaldb.getProposal( i  ) ;
                    result[index] = temp ;
                    index = index.add(1);
    ...
    function getSuccessfullyFundedProposals (  uint256 page , uint256  size  ) public view returns ( ProposalDB.Proposal[] memory  )
    {
        require ( page > 0 && size > 0  , "page/size != 0");
        ProposalDB.Proposal[] memory  result  = new  ProposalDB.Proposal[] (  size );
        uint256 counter = 1 ;  
        ...
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if ( proposaldb.getProposal( i ).exist == true && proposaldb.getProposal( i ).paid == true &&  proposaldb.getProposal( i ).fundReturned == true && proposaldb.getProposal( i ).enabled == true  )
            {
    ...
    function getApplicantProposal (  uint256 page , uint256  size , address applicant  ) public view returns ( ProposalDB.Proposal[] memory  )
    {
        require ( page > 0 && size > 0  , "page/size != 0");
        ProposalDB.Proposal[] memory  result  = new  ProposalDB.Proposal[] (  size );
        uint256 counter = 1 ;  
        ...
        {
            //UserStake storage temp = this.StakeUserInfo(msg.sender , ( page.sub(1) ).mul(size).add(i) ) ;
            if ( proposaldb.getProposal( i ).exist == true && proposaldb.getProposal( i ).applicant == applicant && proposaldb.getProposal( i ).enabled == true  )
            {
    ...
    function getCoingForOtherPlatforms ( uint256 id ) public view returns (  ERC20DB.ERC20Coin[] memory  )
    {
        return erc20db.getCoingForOtherPlatforms( id );
    }
