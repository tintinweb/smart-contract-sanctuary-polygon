...
contract GasMonster is ERC721A, ChainlinkInfrastructureClient, TokenURIResolver, PausableMint, Withdrawable, Royalty {
	...
	function mintOnPreSale(uint256 quantity, bytes memory signature) external payable {
		require(block.timestamp > preSaleTimestamp, 'Pre-sale is not started yet');
	...
	function mintOnPublicSale(uint256 quantity) external payable {
		require(block.timestamp > publicSaleTimestamp, 'Public sale is not started yet');
	...
	function mintDragonGirlWithDiscount(uint256 boyDragonId, uint256 quantity) external payable {
		require(block.timestamp >= timestampOfSexChangeAtMint, 'Mint of girl dragons is not started yet');
