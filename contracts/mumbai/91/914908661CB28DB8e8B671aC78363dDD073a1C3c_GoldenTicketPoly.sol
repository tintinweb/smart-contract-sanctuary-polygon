...
import "IExtraData.sol";
// needs poly bridge mintable 1155 contract to mint on mainnet
contract GoldenTicketPoly is ERC1155(""), Ownable {
	...
	IConsumables immutable CONSUMABLES;

	IExtraData public extraData;
	uint256 public itemIndex;
	uint256 public amountToBurn = 10;
	uint256 public kongiumAmountToBurn = 20000;
	mapping(uint256 => string) public uris;

	constructor(address _consumables, uint256 _index, address _extra) {
		CONSUMABLES = IConsumables(_consumables);
		itemIndex = _index;
		extraData = IExtraData(_extra);
	}
	...
	function updateBaseUri(string memory _uri_) external onlyOwner {
		_uri = _uri_;
	}

	function setUri(uint256 _id, string memory _uri_) external onlyOwner {
		uris[_id] = _uri_;
	}
	...
	function setAmountToBurn(uint256 _amount) external onlyOwner {
		amountToBurn = _amount;
	}

	function setKongiumAmountToBurn(uint256 _amount) external onlyOwner {
		kongiumAmountToBurn = _amount;
	}

	function mintGolden(address _to, uint256 _amount) external onlyOwner {
		_mint(_to, 1, _amount, "");
	}

	function exchange() external {
		CONSUMABLES.safeTransferFrom(msg.sender, BURN_ADDRESS, itemIndex, amountToBurn, "");
		extraData.decItem(KONGIUM_ID, msg.sender, kongiumAmountToBurn);
		this.safeTransferFrom(address(this), msg.sender, 1, 1, "");
	}
	...
	function onERC1155Received(address _operator, address _from, uint256 _id, uint256 _value, bytes calldata _data) external returns(bytes4) {
		return GoldenTicketPoly.onERC1155Received.selector;
	}
