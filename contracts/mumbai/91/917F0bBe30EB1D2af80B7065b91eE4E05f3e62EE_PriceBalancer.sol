...
contract PriceBalancer is Ownable {
    ...
    function getInputAmount(
        ...
        uint112 scaleUp = 10 ** 18;
        return
            operationType == OperationType.BUY
                ? uint256(y - (sqrt((targetPrice * x * y) / scaleUp) * scaleUp) / targetPrice)
                : uint256((sqrt((targetPrice * x * y) / scaleUp) * scaleUp) / targetPrice - y);
    }

    function sqrt(uint112 x) public pure returns (uint112 y) {
        uint112 z = (x + 1) / 2;
        y = x;
        while (z < y) {
            y = z;
