...
contract MultiTaskFactory {
    ...
    function createMultiTask(
        ...
        _contracts.push(addr);
        _userContracts[msg.sender].push(addr);
        _userContracts[_contractor.account].push(addr);
        _userContracts[_referee.account].push(addr);
        return newContract;
    }
    ...
    /**
     * @dev Returns an array of all the MultiTask contracts that userAddr is involved in.
     * @return An array of all the MultiTask contracts that userAddr is involved in.
     */
    function userContracts(address userAddr) public view returns(address[] memory) {
        return _userContracts[userAddr];
    }
}
