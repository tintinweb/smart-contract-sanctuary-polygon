...
contract Testcon is Ownable, ERC721A, NonblockingReceiver {
 ...
 constructor(
    uint256 maxBatchSize_,
    uint256 currentIndex_,
    uint256 MaxTokenNum_,
    address _layerZeroEndpoint
  ) ERC721A("Testcon", "Testcon", maxBatchSize_, currentIndex_, MaxTokenNum_) {
        MaxMintPerTx = maxBatchSize_;
        MAX_MINT_ETHEREUM = MaxTokenNum_;
        _owner = msg.sender;
  ...
  //upgrade traits
   function upgrade(address owner,  uint256 tokenId, uint256 newtokenId) public returns (bool) {
     require(UpgradeIsActive, "Compose is not active at the moment");
     require(ERC721A.ownerOf(tokenId) == owner, "You do not have this cat");
    ...
    function mint(uint8 numTokens) external payable {
        ...
        require(numTokens > 0 && numTokens <= MaxMintPerTx, "Can not mint more than MaxMintPerTx");
