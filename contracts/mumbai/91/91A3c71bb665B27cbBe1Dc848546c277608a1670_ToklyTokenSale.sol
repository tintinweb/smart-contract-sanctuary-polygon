...
import "../utils/ToklyAllowlist.sol";
 ...
 * @author Khiza DAO
 * @dev ToklyTokenSale is a base contract for a token sale. It's created using the ToklyFactory.
 */
    ...
    AmountLimits
{
    ...
    event BuyTokens(address buyer, uint amountToPay, uint amountToSell);
        ...
        amountCompliance(_amountToBuy)
    {
        ...
        if (allowlistEnabled) {
            _subtractAllowlistLimit(_msgSender(), _amountToBuy);
        }
    ...
    /**
     * @dev Verifies if the contract has enough tokens to sell for the transaction and returns the amount to pay. Reverts if amount to pay is zero.
     * @return _amountToPay The amount of tokens to pay for the amount of tokens to buy
     */
    ...
    struct ToklyTokenSaleInfo {
        ...
        uint minAmount;
        uint maxAmount;
        uint tokenToSellDecimals;
        bool allowlistEnabled;
        uint startsAt;
        uint endsAt;
        uint releasesAt;
        uint totalReservedAmount;
        string toklyVersion;
    ...
    /**
     * @dev Returns all contract info in a single call
     * @return
     */
    function getAll() external view returns (ToklyTokenSaleInfo memory) {
        return ToklyTokenSaleInfo(
            owner(),
            paused,
            address(tokenToSell),
            address(tokenToPay),
...
contract ToklyAllowlist is Ownable {
    ...
    event AddressAddedToAllowlist(address _address, uint _limit);
    event AddressRemovedFromAllowlist(address _address);
    ...
    EnumerableMap.AddressToUintMap private _allowlistLimits;

    // === Modifiers ===

    modifier allowlistCompliance(uint amountToBuy) {
        if (
            allowlistEnabled && getAllowlistLimitFor(_msgSender()) < amountToBuy
    ...
    /**
     * @dev Subtracts from the limit for a given address.
     * @param _address address to subtract limit from
     * @param _amount amount to subtract from limit
     */
    function _subtractAllowlistLimit(
        address _address,
        uint _amount
    ) internal {
        uint currentLimit = _allowlistLimits.get(_address);
        _allowlistLimits.set(_address, currentLimit - _amount);
    }

    /**
     * @dev Sets the limit for a given address.
     * @param _address address to set limit for
    ...
    function setAllowlistAddress(
        ...
        if (_limit > 0) {
            _allowlistLimits.set(_address, _limit);
            emit AddressAddedToAllowlist(_address, _limit);
        } else {
            _allowlistLimits.remove(_address);
            emit AddressRemovedFromAllowlist(_address);
        }
    }

    /**
     * @dev Returns the limit for a given address.
     * @param _address Address to check
     */
    function getAllowlistLimitFor(address _address) public view returns (uint) {
        (,uint limit) = _allowlistLimits.tryGet(_address);
        return limit;
    }

    /**
     * @dev Returns the list of addresses on the allowlist as a list of AllowlistEntry structs.
     * @dev This is used by the frontend to display the allowlist.
     * @return Array of AllowlistEntry structs
     */
    function getAllowlist() public view returns (AllowlistEntry[] memory) {
        AllowlistEntry[] memory result = new AllowlistEntry[](
            _allowlistLimits.length()
        );

        for (uint i = 0; i < _allowlistLimits.length(); i++) {
            (result[i]._address, result[i]._limit) = _allowlistLimits.at(i);
        }
    ...
    /**
     * @dev Enables owner to set multiple addresses at once
     * @param _list Array of AllowlistEntry structs to set
     */
    function setMultiAllowlist(AllowlistEntry[] memory _list) public onlyOwner {
        for (uint i = 0; i < _list.length; i++) {
            setAllowlistAddress(_list[i]._address, _list[i]._limit);
        }
    }
}
...
contract ReleaseLock is Ownable {
    ...
    EnumerableMap.AddressToUintMap private _reservedClaims;
    ...
    function _setReservedClaims(address _address, uint _amount) internal {
        totalReservedAmount -= getClaimFor(_address);
        _reservedClaims.set(_address, 0);
        _addReservedClaims(_address, _amount);
    }

    function _addReservedClaims(address _address, uint _amount) internal {
        uint currentReservedAmount = getClaimFor(_address);
        _reservedClaims.set(_address, currentReservedAmount + _amount);
    ...
    function getClaimFor(address _address) public view returns (uint) {
        (, uint value) = _reservedClaims.tryGet(_address);
        return value;
    }
    ...
    function isClaimCompliant(
        address _address,
        uint _amount
    ) public view returns (bool) {
        return (getClaimFor(_address) >= _amount);
    }

    function getAllClaims() public view returns (ClaimEntry[] memory) {
        ClaimEntry[] memory result = new ClaimEntry[](_reservedClaims.length());

        for (uint i = 0; i < _reservedClaims.length(); i++) {
            (result[i]._address, result[i]._amount) = _reservedClaims.at(i);
        }
    ...
    modifier claimCompliance(address _address, uint _amount) {
        if (!isClaimCompliant(_address, _amount))
            revert NotEnoughReserved(
                _address,
                _amount,
                getClaimFor(_address)
            );
        uint currentReservedAmount = getClaimFor(_address);
        _reservedClaims.set(_address, currentReservedAmount - _amount);
        totalReservedAmount -= _amount;
...
/**
 * @title AmountLimits
 * @dev This contract allows to set min and max values for an amount.
 * @author Khiza DAO
 */
contract AmountLimits is Ownable {
    ...
    /**
     * @dev Emitted when the new amount is invalid. Eg. minAmount > maxAmount
     * @param _amount The new amount
     * @param _bound The bound that was violated
     */
    error InvalidAmount(uint256 _amount, uint256 _bound);

    /**
     * @dev Emitted when the amount is not in the range of minAmount and maxAmount
     * @param _amount The amount
     * @param _minAmount The min allowed amount at the moment of the transaction
     * @param _maxAmount The max allowed amount at the moment of the transaction
     */
    ...
    function setMinAmount(uint256 _value) public onlyOwner {
        if (_value > maxAmount) revert InvalidAmount(_value, maxAmount);
        minAmount = _value;
        emit MinAmountChanged(_value);
    }

    function setMaxAmount(uint256 _value) public onlyOwner {
        if (_value < minAmount) revert InvalidAmount(_value, minAmount);
        maxAmount = _value;
        emit MaxAmountChanged(_value);
    }
