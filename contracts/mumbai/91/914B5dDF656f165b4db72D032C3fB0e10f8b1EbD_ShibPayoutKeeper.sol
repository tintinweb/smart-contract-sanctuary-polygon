...
contract ShibPayoutKeeper is KeeperCompatibleInterface, Ownable {
    ...
    address private _coverContractAddress;
    ...
    event PayoutExecuted(uint256 atRoundId, uint256 atPrice);
    event Debug();

    constructor(address coverContractAddress, address priceFeedAddress) {
        lastTimeStamp = block.timestamp;
        _coverContractAddress = coverContractAddress;
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        // Lets get the latest round data
        (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredInRound) = _tokenPriceFeed.latestRoundData();      
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        // (uint256 roundId, uint256 answer, uint256 updatedAt) = performData;

        // emit PayoutExecuted(roundId, uint256(answer));
        emit Debug();
    }
}
