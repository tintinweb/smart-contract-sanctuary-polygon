...
contract PIXTerritoryStaking is OwnableUpgradeable, ReentrancyGuardUpgradeable, ERC721HolderUpgradeable, IPIXTerritoryStaking {
    ...
    function getStakedNFTs(address account) external override view returns(uint256[] memory) {
        uint256 len;
        for (uint256 i; i < stakedNFTs[account].length; i += 1) {
            if (stakers[stakedNFTs[account][i]] != account) continue;
            len += 1;
        ...
        uint256 ind;
        uint256[] memory stakedNFTsMem = new uint256[](len);
        for (uint256 i; i < stakedNFTs[account].length; i += 1) {
            if (stakers[stakedNFTs[account][i]] != account) continue;
            stakedNFTsMem[ind] = stakedNFTs[account][i];
            ind += 1;
        }
        return stakedNFTsMem;
    }
