...
contract ArtByCodersMintPass is ERC1155, Ownable, Pausable, ERC1155Burnable, ERC1155Supply {
    string public name;
    string public symbol;
    constructor() ERC1155("") {
    name = "ArtByCodersMintPass";
    symbol = "MPWL";
    }

uint256 public mintRate = 1 ether;    
//uint256 public mintRateWL = 1 ether; 
uint256[] public minted = [0,0];
uint256[] public supplies = [0, 10000000];
...
require(!stopped, "WL Mint offer finished! you can mint MintPass at regular Price ");
_mint(msg.sender, id, amount, "");
}

//MintWL @ 1 Matic , 
/* function MintOnlyWL() payable public
{
//require(id > 0, "Token doesn't exist");
require(msg.value >= mintRateWL * mintAmount, "insufficient funds");
//holder of WLWinner NFT
require(!stopped, "WL Mint offer finished! you can mint MintPass at regular Price ");
_mint(msg.sender, 1, mintAmount, "");
    ...
    function mintOnlyOwner(address account, uint256 amount, bytes memory data)
        public
        onlyOwner
    {
...
// send to many transfer only
  function bulkAirdropNFTs(address from, address[] calldata _to, uint256[] calldata _id) public onlyOwner{
    require(_to.length == _id.length, "Receivers and IDs are different length");
    for (uint256 i = 0; i < _to.length; i++) {
      _safeTransferFrom(from, _to[i], _id[i], 1, "");
    }
  }
