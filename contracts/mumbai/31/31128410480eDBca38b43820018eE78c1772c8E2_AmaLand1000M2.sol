...
contract AmaLand1000M2 is DefaultOperatorFilterer, ERC721A, Ownable {
  using Address for address;
  using Strings for uint256;
  ...
  uint256 private _royaltyMobiupCrowd = 7;  
  uint256 private _royaltyFund = 1;  
  uint256 private _royaltyArtist = 0;  
  uint256 private _royaltyCreator = 85;  
  ...
  address private wArtist = 0x99BF57549e83ffA0e8aE16caF1626B44ACf6838e;
  address private wCreator = 0x99BF57549e83ffA0e8aE16caF1626B44ACf6838e;
  uint256 royaltiesSpender;

  uint256 private _royaltyFundMarket = 1;  
  uint256 private _royaltyArtistSecondMarket = 0;
  uint256 private _royaltyCreatorSecondMarket = 9;
  ...
  uint256 private _nftEtherValueWaitList = 0.01 ether;  

  mapping(uint256 => string) private cids;

  uint256 private initSaleWaitList = 1685566994;
  uint256 private endSaleWaitList = 1685566994;
  uint256 private startSalePublic = 1685566994;

  event _transferSend(address _from, address _to, uint _amount);

  constructor(    
    bytes32 _root    
  ) ERC721A("AmaLand1000M2", "LAND2") {    
    root = _root;
    _contractUri = "";
    ...
    function split(uint256 _mintAmount, address payable _recommendedBy, uint256 _indicationType ) public payable{
      ...
      uint256 splitMobiupCrowd =  (msg.value * _royaltyMobiupCrowd) / 100;

      uint256 eachAmount = splitMobiupCrowd / 2;    

      payable(wMobiup).transfer(eachAmount);
      payable(wCrowd).transfer(eachAmount);
