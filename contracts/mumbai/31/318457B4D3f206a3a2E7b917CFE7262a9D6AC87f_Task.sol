...
interface ITask {
    function createTask(address _worker, address _token, uint _amount) external;
...
contract Task is Verifier, Ownable, ITask {
    ...
    function createTask(
        ...
        // 抛出事件
        emit TaskCreated(taskId, _worker, _token, _amount);
    }
    ...
    function withdraw(
        ...
        if ((signer != task.worker && signer != task.issuer) || signer == msg.sender)
    ...
    function getTaskInfo(uint256 taskId) external view override returns(TaskInfo memory){
        return tasks[taskId];
    }
}
