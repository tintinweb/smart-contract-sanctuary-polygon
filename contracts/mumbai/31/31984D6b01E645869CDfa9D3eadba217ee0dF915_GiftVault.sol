...
/// @title GiftVault - customized escrow wallet for GiftBox
/// @dev tokenIn removed as actual transfer occur in GiftBox
/// @dev token tracking removed as GiftVault use same token tracking with GiftBox
contract GiftVault is HSystemChecker, Multicall, ERC1155Holder {
    ...
    constructor(address systemCheckerContractAddress, address itemFactoryContractAddress)
        HSystemChecker(systemCheckerContractAddress)
    {
        _itemFactoryContractAddress = itemFactoryContractAddress;
        _token = IERC1155(itemFactoryContractAddress);
    }

    /// @notice Remove token from the vault
    ...
    function tokenOut(
        ...
        require(
            _token.balanceOf(address(this), tokenId) >= amount,
            'GV 102 - Invalid vault balance'
        );
        _token.safeTransferFrom(address(this), to, tokenId, amount, '');
    }

    /// @notice Batch remove token from the vault
    /// @param to - Account to transfer to
    /// @param tokenIds - token Id array to remove
    /// @param tokenAmounts - token Amounts array to remove
    function batchTokenOut(
        address to,
        ...
        for (uint256 i; i < tokenIds.length; ) {
            require(
                _token.balanceOf(address(this), tokenIds[i]) >= tokenAmounts[i],
                'GV 102 - Invalid vault balance'
            );
        ...
        _token.safeBatchTransferFrom(address(this), to, tokenIds, tokenAmounts, '');
    }
