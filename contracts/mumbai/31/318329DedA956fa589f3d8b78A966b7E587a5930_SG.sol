...
contract SG is ERC1155, Ownable, Reclaimable, IERC2981 {
    ...
    function isHolder(address holder, uint256 catOrRatId) public view returns (bool) {
        if (catsAddress.balanceOf(holder, catOrRatId) > 0) return true;
        if (ratsAddress.balanceOf(holder, catOrRatId) > 0) return true;
    ...
    function mint(uint256[] calldata ids, uint256[] calldata amounts, uint256 catOrRatId) external payable {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            ...
            if (current >= 6 && current <= 10) require(isHolder(msg.sender, catOrRatId), "Can't mint holder bundle");
    ...
    function platformMint(address to, uint256[] calldata ids, uint256[] calldata amounts, uint256 catOrRatId) external {
        require(block.timestamp > startTime && block.timestamp <= endTime, "Out of timeframe");
        require(msg.sender == minter, "Not the minter");

        for (uint256 i = 0; i < ids.length; i++) {
            if (ids[i] >= 6 && ids[i] <= 10) require(isHolder(to, catOrRatId), "Not eligible to mint");
        }
    ...
    function setCollections(address cats, address rats, address[] calldata erc721s) external onlyOwner {
