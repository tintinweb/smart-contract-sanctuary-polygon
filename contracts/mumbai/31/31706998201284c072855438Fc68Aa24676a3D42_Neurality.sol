...
contract Neurality is ERC721Enumerable, Ownable {
    ...
    uint16 public constant TOTAL_MINT_AMOUNT = 7777;
    string public mintType = "wl1";
    uint16 public mintAmountPerMintType = 1777;
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0) && to != address(0)) {
            if (mintedTimestamp[from] > 0) {
                require(
                    block.timestamp - mintedTimestamp[from] >= 20131200,
    ...
    function privateMint(bytes32[] calldata _merkleProof) public payable {
        ...
        if (
            ...
            require(totalSupply() <= 1777, "1st private sale is finished.");
        } else if (
            keccak256(abi.encodePacked((mintType))) ==
            keccak256(abi.encodePacked(("wl2")))
        ) {
            //  2nd private sale
            require(totalSupply() <= 3777, "2nd private sale is finished.");
        }
