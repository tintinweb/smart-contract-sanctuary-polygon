...
contract DefiLottery is ERC721A, PaymentSplitter, Ownable, ReentrancyGuard {
    ...
    uint public saleStartTime = 1655841642;
    ...
    constructor(string memory _theBaseURI) ERC721A("DefiLottery", "DFL") PaymentSplitter(_team, _teamShares) {
    ...
    function saleMint(address _account, uint256 _ammount) external payable nonReentrant {
        ...
        if(currentTime() >= saleStartTime + 1 weeks) {
    ...
    function getBalanceForTheWinner() public view returns(uint) {
        return address(this).balance;
    }

    function getWinnerAddress()public view returns(address) {
        uint myNumber;
        require(sellingStep == Steps.SoldOut, "Sorry, sale has not finished.");
    ...
    function withdrawToWinner(address payable _to, uint _amount)external payable nonReentrant{
        uint myNumber;
        address winnerAddress = ownerOf(myNumber);
        myNumber = uint(keccak256(abi.encodePacked(msg.sender, block.timestamp, myNumber))) % totalSupply();
        require(msg.sender == _to);
        require(sellingStep == Steps.SoldOut, "Sorry, sale has not finished.");
        require(_to == winnerAddress, "Not enought funds");
