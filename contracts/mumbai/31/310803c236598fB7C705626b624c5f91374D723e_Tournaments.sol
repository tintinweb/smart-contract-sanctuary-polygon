...
contract Tournaments is Initializable {
    ...
    function getPlayerGameState(uint256 i)
        external
        view
        returns (
            string memory,
    ...
    function exitGame(uint256 i) external {
        ...
        // TODO: check
        delete currPlayers[i][playerNum[i][msg.sender] - 1];
        currPlayers[i][playerNum[i][msg.sender] - 1] = currPlayers[i][
            games[i].size - 1
        ];
        delete currPlayers[i][games[i].size - 1];
