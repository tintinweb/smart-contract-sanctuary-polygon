...
contract CooperationSelling is Pausable {
    ...
    uint256 maxRelease = 2000000 * 1e6;
    uint256 restRelease = 2000000 * 1e6;
    ...
    {
        return (swapRate, swapRate_USDT, swapRate_WETH, maxRelease, restRelease);
    }
    ...
    function second_withdraw() external {
        require(block.timestamp >= Locks[msg.sender].since + 182.5 days, 'Not 6 months later');
        uint256 amount = Locks[msg.sender].second;
        token.transfer(msg.sender, amount);
        Locks[msg.sender].second = 0;
        emit Withdraw(msg.sender, amount);
    }

    function third_withdraw() external {
        require(block.timestamp >= Locks[msg.sender].since + 365 days, 'Not 12 months later');
        uint256 amount = Locks[msg.sender].third;
        token.transfer(msg.sender, amount);
        Locks[msg.sender].third = 0;
        emit Withdraw(msg.sender, amount);
    }

    //edit the unlock rate
    function setLocks(uint256 _uint) internal {
        Locks[msg.sender].first += (_uint * 2) / 10;
        Locks[msg.sender].second += (_uint * 3) / 10;
        Locks[msg.sender].third += (_uint * 5) / 10;
        if (Locks[msg.sender].since == 0) {
            Locks[msg.sender].since = block.timestamp;
        }
    }

    function getLocks(address _address) external view returns (Lock memory) {
        return Locks[_address];
    }

    function distrubute(uint256 _amount) internal {
        token.transfer(msg.sender, Locks[msg.sender].first);
        Locks[msg.sender].first = 0;
