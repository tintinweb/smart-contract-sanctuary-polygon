...
contract BitcoinSuckers is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer, ERC721AQueryable {
    ...
    mapping(address => uint) public suckToken;

    constructor(address[] memory _team, uint[] memory _teamShares, string memory _baseURI) ERC721A("Bitcoin Suckers", "BS")
    ...
    function suck(uint nftID) public {
        require(ownerOf(nftID) == msg.sender, "Not the owner");
        // SUCKY SUCKY
        suckToken[msg.sender] += 100;
        if (suckToken[msg.sender] % 500 == 0) {
            // NUT
            _mint(msg.sender, 1);
        }
    }
