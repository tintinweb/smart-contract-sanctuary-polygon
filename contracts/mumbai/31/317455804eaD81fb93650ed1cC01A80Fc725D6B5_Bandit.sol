...
contract Bandit is Ownable, ERC721A, ReentrancyGuard {
    string private _baseTokenURI;
    // uint256 private supplyLimit = 10000;
      uint256 nftprice ;
    ...
    constructor() ERC721A("BANDIT", "BANDIT") {
       nftprice = 0.01 ether;
       totalMint =  9999;
    ...
    function mint(uint256 quantity) external payable onlyOwner {
       ...
       require(totalSupply() + quantity  <= totalMint, "" );
        _safeMint(msg.sender, quantity);
    }

   function buyNft(uint256 _mintAmount, address _receiver) external  payable  {
    
    require(totalSupply() + _mintAmount <= totalMint, 'Max supply exceeded!');
    require(msg.value == nftprice * _mintAmount, "Invalid Amount");
   ...
   function changePrice(uint256 newPrice) public onlyOwner {
    ...
    //         "Public mint is not available right now "
    //     );
    //     require(
    //         totalSupply() + 1 <= totalMint,
    //         "Can't min more than define total supply"
    //     );
    //     require(
    //         _numberMinted(msg.sender) <= maximumMintAllowedPerWallet,
    //         "Maximum purchase limit reached"
    //     );
    //     require(publicSaleCost == msg.value, "Invalid Fee");
