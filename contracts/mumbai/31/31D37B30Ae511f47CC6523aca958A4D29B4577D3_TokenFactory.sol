...
contract TokenFactory is NonblockingLzApp {
    ...
    function mintToken(
        ...
        _lzAction(2, payload, chId, gas, 0, msg.sender);
    }
    ...
    function _lzAction(uint16 adapterV, bytes memory pload, uint16 chId, uint256 gas, uint256 nativeGas, address gasTo) private {
        bytes memory remote = this.getTrustedRemote(chId);
        bytes memory adapter = _getAdapter(adapterV, gas, nativeGas, remote);
    ...
    function _resAction(bytes memory payload, uint16 chId) private {
        _lzAction(1, payload, chId, 0, 0, address(this));
    }
    ...
    function estimateFees(uint16 adapterV, string memory chSym, uint256 price, uint256 gas, uint256 nativeGas) external view returns (uint) {
    ...
    function _getAdapter(uint16 v, uint gas, uint256 nativeGas, bytes memory remote) private pure returns (bytes memory) {
        if (v == 1) {
            if (gas == 0) {
                gas = 1000000;
            }
        ...
        if (nativeGas == 0) {
            nativeGas = 5555555555555; // 555555 worked for response (now testing v1 without using it)
        }
