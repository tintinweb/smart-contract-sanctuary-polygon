 ...
 */
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App, __with_semver(uint8(1), uint8(0), uint8(3)) {
    using Strings for uint256;
...
contract ERC721BaseSale is App {
    ...
    function _claim(bytes32 tierValue) internal {
        if (totalClaimsLimit != 0) {
            require(totalClaimsLimit > totalClaimed, Exceptions.UNAVAILABLE_ERROR);
            totalClaimed++;
        }
        if (claimLimit != 0) {
            require(claims[_msgSender()] > claimLimit, Exceptions.UNAVAILABLE_ERROR);
        }
        if (tierPerWalletLimit[tierValue] != 0) {
            require(
                tierPerWalletLimit[tierValue] > tierPerWalletClaimed[_msgSender()][tierValue],
                Exceptions.UNAVAILABLE_ERROR
            );
    ...
    function getLeftClaimsForTier(bytes32 tierValue) public view returns (uint256) {
        ...
        if (totalClaimsLimit != 0) {
            if (totalClaimsLimit > totalClaimed) {
                uint256 rest = totalClaimsLimit - totalClaimed;
    ...
    function getLeftClaimsForWallet(address wallet, bytes32 tierValue) public view returns (uint256) {
        ...
        if (perWalletLimit != 0) {
            ...
            require(perWalletLimit >= claimed, Exceptions.UNAVAILABLE_ERROR);
...
contract ERC721OpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(1), uint8(1)) {
 ...
 */
contract Kernel is Initializable, AppManager, IKernel, __with_semver(uint8(1), uint8(0), uint8(5)) {
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    ...
    function deployAndConfigure(
        ...
        address appImplAddress = _updateManager.getLastAppCode(id);
        address appProxyAddress = address(new AppProxy(appImplAddress));
        _setApp(id, appProxyAddress, true);
        (bool success, ) = appProxyAddress.call(initializeAppData);
        require(success, Exceptions.INVALID_INITIALIZATION_ERROR);
    ...
    __with_semver(uint8(1), uint8(0), uint8(2))
{
    ...
    constructor(address nativeTokenWrapperAddress_) {
