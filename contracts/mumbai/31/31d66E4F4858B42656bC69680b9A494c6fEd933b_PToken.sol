...
contract PToken is VersionedInitializable, IPToken {
  ...
  function claimPrxy(address user, uint256 amount) external virtual override onlyPool {
    ...
    require (_userState[user].prxyBalance >= claimPrxyAmount, Errors.INSUFFICIENT_USER_PRXY_BALANCE);
    if(_userState[user].prxyBalance - claimPrxyAmount < WadRayMath.RAY) claimPrxyAmount = _userState[user].prxyBalance;
    require(_prxyTotalBalance >= claimPrxyAmount, Errors.INSUFFICIENT_TOTAL_PRXY_BALANCE);
    _userState[user].prxyBalance -= claimPrxyAmount;
    _prxyTotalBalance -= claimPrxyAmount;
