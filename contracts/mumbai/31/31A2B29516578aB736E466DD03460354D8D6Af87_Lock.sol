...
contract OnChainMusicMetadata is MusicStructs {
        ...
        returns (string memory)
    {
        ...
        for (uint256 i = 0; i < credits[trackNumber].length; i++) {
            ...
            _string = string(abi.encodePacked(_string, _getString(credits[trackNumber][i].name)));
        ...
        returns (string[47] memory)
    {
        ...
        values[36] = _getString(
            songs[_tokenId].songPublishingData.releaseDate
        );
        values[37] = _getString(
            songs[_tokenId].songPublishingData.recordLabel
        );
