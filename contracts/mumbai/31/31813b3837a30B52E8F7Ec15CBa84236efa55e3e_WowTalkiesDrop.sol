  ...
  ERC1155BurnableUpgradeable
{
  ...
  function batchMint(
    ...
    for (uint256 i = 0; i < _addresses.length; i++) {
      super._mint(_addresses[i], _tokenId, _amount, '0x');
      saleItems[_tokenId].sold += _amount;
      boughtQuantities[_addresses[i]][_tokenId] += _amount;
    }
  }
  ...
  function setMaxSupply(uint256 _tokenId, uint256 _newSupply) public adminOnly {
    require(
  ...
  function setTokenPrice(uint256 _tokenId, uint256 _newPrice) public adminOnly {
  ...
  // ) external adminOnly {
  //     require(from.length == amount.length, "Different length");
  //     for (uint i = 0; i < from.length; i++) {
  //         super._burn(from[i], id, amount[i]);
  //     }
  // }
