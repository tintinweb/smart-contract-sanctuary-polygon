...
contract Trial is ERC20, AccessControl {
    ...
    constructor() ERC20("Trial", "TRL") {
        ...
        domainSeparator = keccak256(
            abi.encode(DOMAIN_TYPE_HASH, keccak256(bytes("Pegaxy|Trial")), getChainId(), address(this))
        );
    }

    function addSigner (address signer) public onlyRole(OWNER_ROLE) {
        require(signer != address(0), "Trial: signer is invalid");
        require(signers[signer] == false, "Trial: signer is exists");
        signers[signer] = true;
    ...
    function removeSigner (address signer) public onlyRole(OWNER_ROLE) {
        require(signer != address(0), "Trial: signer is invalid");
        require(signers[signer] == true, "Trial: signer is not exists");
    ...
    function claim(address player, string memory txId, uint256 amount, uint8 v, bytes32 r, bytes32 s) public {
        require(player != address(0), "Trial: Invalid claimer");
        require(amount > 0, "Trial: Invalid amount");

        bytes32 structHash = keccak256(abi.encode(CLAIM_TYPE_HASH, player, keccak256(bytes(txId)), amount));
        bytes32 digest = keccak256(abi.encodePacked("\x19\x01", domainSeparator, structHash));
        address signatory = ecrecover(digest, v, r, s);

        require(signers[signatory], "Trial: Signer is not valid");
        require(claimTransactions[txId] == 0, "Trial: transaction is claimed");
