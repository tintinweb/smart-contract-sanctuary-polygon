    ...
    ReentrancyGuard
{
    ...
    function mint(uint[] memory ids) public onlyOneBlock nonReentrant
    {
        /**
        * @dev image hardcode
        */
        ...
        */
        ...
        { 
            ...
            bool status = _mint(ids[i], metadata);
            if(status)
            {
                numbers[j] = ids[i];
                j++;
    ...
    function _mint(uint itemId, string memory _tokenURI) internal returns(bool) 
    {
        bool add = false;
        if(tokenCount < quant)
        {
