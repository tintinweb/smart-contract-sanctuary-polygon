...
contract ERC20ForwardRequestTypes {
    /*allow the EVM to optimize for this,ensure that you try to order your storage variables and struct members such that they can be packed tightly*/
...
contract ERC20Forwarder is Ownable, ERC20ForwarderStorage {
   ...
   uint public BasisPoint;
    constructor(uint _basisPoints) Ownable(){
    ...
    function initialize(
        ...
        require(
            !initialized,
            "ERC20 Forwarder: contract is already initialized"
        );
        require(
    ...
    function setTrustedForwarder(address payable _forwarder)
        external
        onlyOwner
    {
        require(
...
// Designed to enable capturing token fees charged during the execution
     ...
     */
     function SetBasisPoint(uint _bp)public onlyOwner
     {
         require(_bp > 0);
        BasisPoint=_bp;
     }
    ...
    function executeEIP712(
        ...
        (success, ret) = BiconomyForwarder(forwarder).executeEIP712(
            req,
            domainSeparator,
            sig
        );
        ...
        uint256 charge = _transferHandler(
            req,
            initialGas.add(baseGas).add(transferHandlerGas).sub(postGas)
        );
    ...
    function permitAndExecuteEIP712(
        ...
        uint256 charge = _transferHandler(
            req,
            initialGas.add(baseGas).add(transferHandlerGas).sub(postGas)//overall extemated gas
        );
        emit FeeCharged(req.from, charge, req.token);
    }
    ...
    function executePersonalSign(
        ...
        uint256 initialGas = gasleft();
        (success, ret) = BiconomyForwarder(forwarder).executePersonalSign(
            req,
            sig
        );
        uint256 postGas = gasleft();
    ...
    function _transferHandler(
        ...
        charge = req
            .tokenGasPrice
            .mul(executionGas).mul(BasisPoint).div(10000);
            console.log(charge);
