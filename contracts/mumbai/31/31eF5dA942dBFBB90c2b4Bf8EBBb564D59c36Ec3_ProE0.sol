 ...
 * @notice This is the contract of the presale of the Pro NFT
 */
contract ProE0 is ERC721, ERC721Enumerable, Pausable, AccessControl, ERC721Burnable, ReentrancyGuard {
    // Token counter
    ...
    // URI of the tokens
    string public baseURI = "http://vegabuild.es/mcg_tokens/2.json";
    ...
    // Events
    event ProMinted(uint256 indexed tokenId, address to);
     ...
     */
    constructor(address selectedStablecoin, uint256 newPrice) ERC721("ProEd0", "PRE0") {
    ...
    function setNewStableCoin(address newStablecoin) external onlyRole(ADMIN_ROLE) {
        require(newStablecoin != address(0), "PRE0: StableCoin address shouldn't be zero address");
    ...
    function setNewPrice(uint256 newPrice) external onlyRole(ADMIN_ROLE) {
        require(newPrice > 0, "PRE0: Price must be greater than 0 wei");
        price = newPrice;
    } 
    ...
    function setChildContractAddress(address contractAddress) external onlyRole(ADMIN_ROLE) {
        require(contractAddress != address(0), "PRE0: Zero address shouldn't be child contract");
    ...
    function setCounterBreeding(uint256 id, address owner) external {
        require(msg.sender == _child, "PRE0: Caller isnt the child contract");
        require(owner == ownerOf(id), "PRE0: The sender isnt the owner of the token");
        _breedingCounter[id]++;
    }
    ...
    /**
     * @notice Set a new URI for Pro ED0 NFT
    ...
    /**
     * @notice Inherit function which returns the URI of Pro ED0 NFT
     * @dev ALL the tokens of Pro ED0 have ONE SINGLE URI
     * @param tokenId The token ID
     * @return baseURI the baseURI of Pro ED0
     */
    ...
    function setNewPrivateSell(address beneficiary, uint8 quantity) external onlyRole(ADMIN_ROLE) {
        require(_tokenIdCounter.current() + (_soldPrivately + quantity) <= MAX_SUPPLY, "PRE0: Max supply reached");
        require(beneficiary != address(0), "PRE0: Address shouldn't be zero");
    ...
    function doPrivateSell() external whenNotPaused nonReentrant {
        require(msg.sender != address(0), "PRE0: Address shouldn't be zero");
        require(_privateSell[msg.sender] > 0, "PRE0: Beneficiary isn't into private sell");
        require(_endOfPS, "PRE0: Presale is not over yet");

        for(uint8 i = 0; i < _privateSell[msg.sender]; i++){
            uint256 tokenId = _tokenIdCounter.current();
            _tokenIdCounter.increment();
            emit ProMinted(tokenId, msg.sender);
    ...
    /**
     * @notice This function mints one Pro NFT from ed0
     * @param to The address of minter
     */
    function mintPro(address to) external nonReentrant whenNotPaused {
        require(to != address(0), "PRE0: Address shouldn't be zero");
        require(IERC20(stableCoin).balanceOf(to) >= price, "USDC: Insufficient funds");
        
        // Mint
        uint256 tokenId = _tokenIdCounter.current();
        require(tokenId < MAX_SUPPLY - _soldPrivately, "PRE0: Max supply reached");
        _tokenIdCounter.increment();     

        emit ProMinted(tokenId, to);
