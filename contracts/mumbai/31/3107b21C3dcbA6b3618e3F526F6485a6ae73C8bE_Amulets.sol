...
//// Used for gas-less meta transactions for transfers51
...
contract Amulets is ERC721Upgradeable, ERC721EnumerableUpgradeable, PausableUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    * @dev 3 = Open Mint
    */
    function mintAmulet() private whenNotPaused nonReentrant {

        if (openMint == 1){
            require(whiteList[tx.origin].isOnWhaleList == true, "Open to Whitelists Whales Only");
            require(whiteList[tx.origin].amountMinted < maxWhaleList, "Max mint for Whitelist Whales");
        } else if (openMint == 2){
            require(whiteList[tx.origin].isOnList == true || whiteList[tx.origin].isOnWhaleList == true, "Open to All Whitelisters");
            require(
                (whiteList[tx.origin].amountMinted < maxWhaleList && whiteList[tx.origin].isOnWhaleList) ||
                (whiteList[tx.origin].amountMinted < maxWhiteList && whiteList[tx.origin].isOnList)
        ...
        // Increment
        minted++;
        whiteList[tx.origin].amountMinted++;

        // Mint the token
        _safeMint(tx.origin, minted);

        // Emit to the logs for external reference
        emit logMintToken(tx.origin, minted);
    }
