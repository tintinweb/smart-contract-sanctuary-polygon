...
contract NelNelDeDe is ERC1155, Ownable, ERC1155Supply {
...
//for random id based 
  ...
  // //for random id based 
  // function checkRandom() internal returns (uint256) {
  //     uint r = setRandom();
  //     if (lastRandomNumber ==0){ lastRandomNumber= r; return r ;} 
  ...
  // }
    ...
    // function multiBlockRandomGen(uint seed, uint size) public view returns (uint) {
    //     uint n = 0;
    //     for (uint i = 0; i < size; i++){
    //         if (uint(keccak256( abi.encodePacked(blockhash(block.number-i-1), seed ,  block.timestamp)))%2==0)
    //             n += 2**i;
    //     }
    //     return n;
    // }
