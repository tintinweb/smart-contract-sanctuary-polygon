...
import {DerbyStarsHorseV1Impl} from "../horse/DerbyStarsHorseV1Impl.sol";
import {DerbyStarsSoulV1Storage} from "./DerbyStarsSoulV1Storage.sol";
...
import {IDerbyStarsSoulV1} from "./IDerbyStarsSoulV1.sol";
import {IDerbyStarsBeacon} from "../../interfaces/IDerbyStarsBeacon.sol";

contract DerbyStarsSoulV1Impl is ERC721EnumerableImpl, IDerbyStarsSoulV1 {
    // TODO: change according to spec
    uint256 public constant crtBreedingFee = 1e18;
    // TODO: change according to spec
    uint256 public constant runBreedingFee = 1e18;
    address public immutable crtErc20;
    address public immutable runErc20;
    address public immutable dao;
    IDerbyStarsHorseV1 public immutable dsHorse;
    ...
    constructor(
        ...
        runErc20 = runErc20_;
        dao = dao_;
        dsHorse = IDerbyStarsHorseV1(dsHorse_);
    ...
    function breed(uint256 motherTokenId, uint256 fatherTokenId) public {
        require(
            msg.sender == IERC721(address(dsHorse)).ownerOf(motherTokenId),
            "1. verify that msg.sender owns motherTokenId"
        );
        Properties memory mprop = dsHorse.getProperty(motherTokenId);
        require(
        ...
        dsHorse.adjustFatherBreedingCount(fatherTokenId);
        Properties memory fprop = dsHorse.getProperty(fatherTokenId);
        uint256 breedingCRT = BreedingCalculatorV1.calculateBreedingFee(fprop, dsData);
        if (msg.sender != IERC721(address(dsHorse)).ownerOf(fatherTokenId)) {
            // send 85%
            ERC20(crtErc20).transferFrom(
                msg.sender,
                IERC721(address(dsHorse)).ownerOf(fatherTokenId),
                breedingCRT * 85 / 100
            );
        ...
        ERC20(runErc20).transferFrom(msg.sender, dao, breedingRUN);

        // 5. increase breeding counts of motherTokenId and fatherTokenId
        dsHorse.incrementMotherBreedingCount(motherTokenId);
        dsHorse.incrementFatherBreedingCount(fatherTokenId);
        // 6. mint new token to msg.sender
        uint256 newTokenId = totalSupply() + 1; // start from id 1
        DerbyStarsSoulV1Storage.Slot storage s = DerbyStarsSoulV1Storage.load();
        s.birthTimestamp[newTokenId] = block.timestamp;
        s.motherTokenIds[newTokenId] = motherTokenId;
        s.fatherTokenIds[newTokenId] = fatherTokenId;
        ...
        s.rarity[newTokenId] = rarity_sum;
    }

    function burn(uint256 tokenId) public override {
        require(msg.sender == address(dsHorse));
        _burn(tokenId);
    }
    ...
    function motherTokenIds(uint256 tokenId)
        ...
        returns (uint256)
    {
        return DerbyStarsSoulV1Storage.load().motherTokenIds[tokenId];
    }
    ...
    function fatherTokenIds(uint256 tokenId)
        ...
        returns (uint256)
    {
        return DerbyStarsSoulV1Storage.load().fatherTokenIds[tokenId];
    }
    ...
    {
        return DerbyStarsSoulV1Storage.load().birthTimestamp[tokenId];
    }

    function name() public pure override returns (string memory) {
        return "DerbyStarsSoul";
    }
...
library DerbyStarsSoulV1Storage {
    bytes32 private constant STORAGE_SLOT =
        bytes32(uint256(keccak256("derbystars.soul.v1")) - 1);

    struct Slot {
        mapping(uint256 => uint256) rarity;
        mapping(uint256 => uint256) motherTokenIds;
        mapping(uint256 => uint256) fatherTokenIds;
        mapping(uint256 => uint256) birthTimestamp;
    }
