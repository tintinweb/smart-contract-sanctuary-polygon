...
contract TokenSwap is Ownable, Pausable, ReentrancyGuard {
    // Global Variables
    ERC20 public TOKEN;
    uint256 public baseRate = 92000000;
    uint256 public rateFloor = baseRate * 90 / 100; // 10% lower than the base rate
    ...
    receive() external payable whenNotPaused nonReentrant {
        ...
        (
            ...
            uint256 EthToRefund
        ) = _calculateAmountReceived(msg.value, currentRate);
        ...
        (bool sent, ) = owner().call{value: msg.value - EthToRefund}("");
    ...
    function getRate() public view returns (uint256) {
        uint256 timeSinceStart = block.timestamp - contractStartTime;
        uint256 rateDecrease = (timeSinceStart + totalEthReceived) * baseRate / (1 ether);
        uint256 newRate = baseRate + baseRate / 10 - rateDecrease; // 10% more tokens for initial investors

        return newRate > rateFloor ? newRate : rateFloor;
    }
    ...
    function _calculateAmountReceived(
        ...
        if (amountToReceive > tokenBalance) {
            ...
            EthToRefund = _amount - (tokenBalance / _rate);
        }
    }
