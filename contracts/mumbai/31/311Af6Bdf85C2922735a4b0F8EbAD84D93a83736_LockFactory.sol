        ...
        if (isPaymentToken[address(paymentToken)]) {
            ...
            if (tokenDecimals >= EXP_VALUE) {
                fixedTokenAmount =
                    ...
                    (tokenDecimals / EXP_VALUE) * priceFeedDecimals / uint256(tokenUsdPrice);
            } else {
                fixedTokenAmount =
                    deployFeeFixedAmount /
                    (EXP_VALUE / tokenDecimals) * priceFeedDecimals / uint256(tokenUsdPrice);
            }
        } else {
            if (tokenDecimals >= EXP_VALUE) {
                percentageTokenAmount =
                    ((lockedAmount * deployFeePercentageAmount) / EXP_VALUE) *
    ...
    function updateFeedsAndTokens(
        ...
        for (uint256 i; i < _deployFeeTokens.length; i++) {
            require(
                _deployFeeTokens[i] != address(0) &&
                    _tokenPriceFeeds[i] != address(0) &&
                    _ids[i] < _deployFeeTokens.length - 1,
