...
import {Sync, IAggregator} from "./interfaces/IAggregator.sol";

error EmptySources();
error TokenDoesNotExist();
error TransferNotAllowed();

contract CategoryCustomBOB is Initializable, ERC721Upgradeable, AccessControlUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
    // Roles
    bytes32 public constant UPGRADER_ROLE = keccak256("UPGRADER_ROLE");

    // SourceIDs
    bytes32 public constant SOURCE_ID_BAB = keccak256("BAB");

    string private _baseTokenURI;
    IAggregator private _aggregator;

    uint256 public syncTTL;
    ...
    function setSyncTTL(uint256 _syncTTL) external onlyRole(UPGRADER_ROLE) {
        syncTTL = _syncTTL;
    }
    ...
    function _exists(address owner) internal view returns (bool) {
        Sync memory sync = _aggregator.isSynced(uint256(SOURCE_ID_BAB), owner);

        return sync.payload > 0 && sync.timestamp > block.timestamp - syncTTL;
    }
}
