...
/// @title AmmFactoryStrategy
/// @author The Siren Devs
/// @notice Factory contract responsible for AMM creation
contract AmmFactoryStrategy is OwnableUpgradeable, Proxiable {
    ...
    function createAmm(
        ...
        amms[assetPair].push(address(newAmm));
...
interface IMinterAmm {
    ...
    function addPermissionedUsers(address _userAddress, bool _permissioned)
        external;

    function setPermissionedAmm(bool _whiteListed) external;
}
...
interface IAddressesProvider {
    ...
    function setSeriesDeployer(address ammStrategyFactory) external;
