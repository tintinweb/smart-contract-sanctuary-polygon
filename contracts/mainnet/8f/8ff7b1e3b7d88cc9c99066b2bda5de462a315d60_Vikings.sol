...
contract Vikings is ERC721Enumerable, Pausable, Ownable {
    ...
    uint256 constant public MAX_SUPPLY = 21006;
    ...
    modifier checkTypeMint(uint256 _type, uint256 _amount) {
        ...
        require(MAX_SUPPLY >= totalSupply() + _amount, "Vikings: Mint error, max supply reached");
    ...
    function setMintType(uint256 _mintType, uint256 _totalSupply, uint256 _initialIndex, uint256 _startTime, uint256 _endTime, uint256 _strength, uint256 _price) external onlyOwner {
        require(totalTypes > _mintType, "Vikings: Error nonexistent type of mint");
    ...
    function saleMint(uint256 _type, uint256 _amount) external checkTypeMint(_type, _amount) {
        ...
        if (block.timestamp > whiteListStartTime && block.timestamp <= whiteListEndTime) {
            ...
            require(whiteListAmountLeft[_msgSender()] >= _amount, "Vikings: The amount execced the the whitelist user allowance");
