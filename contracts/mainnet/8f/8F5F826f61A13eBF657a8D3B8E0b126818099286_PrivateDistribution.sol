...
contract PrivateDistribution is Ownable {
    ...
    IERC20 private _routeToken;
    address[] public investors;

    enum AllocationType { SEED }
    ...
    function withdrawTokens(AllocationType _allocationType) external onlyInvestor(_allocationType) initialized() {
        ...
        _routeToken.safeTransfer(_msgSender(), tokensAvailable);
    ...
    function removeInvestor(address _investor, uint256 _allocationType) external onlyOwner() {
        require(_investor != address(0), "invalid address");
        AllocationType allocationType = AllocationType(_allocationType);
        Investor storage investor = investorsInfo[allocationType][_investor];
        uint256 allocation = investor.tokensAllotment;
        require(allocation > 0, "the investor doesnt exist");
        ...
        returns (uint256 availablePercentage)
    {
        ...
        uint256 initialCliff = _initialTimestamp + 1 days;
        uint256 vestingDuration = _initialTimestamp + 1461 days;
        uint256 noOfRemaingDays = 1461;

        if (currentTimeStamp > _initialTimestamp) {
            if (allocationType == AllocationType.SEED) {
                uint256 initialReleasePercentage = 0;
                ...
                } else if (currentTimeStamp >= vestingDuration) {
