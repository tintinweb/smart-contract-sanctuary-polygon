...
contract Rail is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 250 * 10**6 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = 'Rail Token';
    string private _symbol = 'RAIL';
    ...
    uint constant MEV_COOLDOWN_TIME = 60 minutes;
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if (_cooldownWhitelist[sender] != true) {
            require(_cooldowns[sender] <= uint32(block.timestamp), "Please wait 1 hour before transferring or selling your tokens.");
        }
