...
// File contracts/interfaces/IAavegotchiFacet.sol
...
// File contracts/OriumAavegotchiLending.sol
...
contract OriumAavegotchiLending is OwnableUpgradeable {
    ...
    modifier onlyAllowedAddresses() {
        require(_allowedAddresses[msg.sender], "Only Gelato can invoke this function");
        _;
    }
    ...
    function initialize(
        ...
        _tokens = tokens;
        _allowedAddresses[gelatoAddress] = true;
    ...
    function updateAllowedAddresses(address[] memory allowedAddresses_, bool[] memory _isAllowed) external onlyOwner {
        require(allowedAddresses_.length == _isAllowed.length, "OriumAavegotchiLending: Invalid input");

        for(uint256 i = 0; i < allowedAddresses_.length; i++) {
            _allowedAddresses[allowedAddresses_[i]] = _isAllowed[i];
        }
    }
    ...
    function getAavegotchiState(uint32[] memory tokenIds) public view returns (bool[] memory isAavegotchisLent, bool[] memory isAavegotchisListed){
