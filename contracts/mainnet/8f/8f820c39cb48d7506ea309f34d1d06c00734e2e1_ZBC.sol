...
contract ZBC is Setting, ZBCCompound {
    ...
    function setZBCWallet(address zbcWallet_) external {
        if (sys.zbcWallet == address(0)) {
            sys.zbcWallet = zbcWallet_;
    ...
    function _distribute(uint256 amount) private {
        ...
        IWallet(sys.zbcWallet).withdrawBatch(sysAddr.usdt,feeAccounts,amount);
    }
    ...
    function rewardAmount(uint256 startBlock,uint256 activeBlock,uint256 lastRewardBlock,
        ...
        for (uint256 i=1; i <= nums+1; i++) {
            ...
            if (nowBlock >= point) {
                ...
                uint256 modNumber = epochTs % dayBlocks;
                if (modNumber > 0) {
                    uint256 profit = compound128(totalReward,nSubsidy,1);
                    uint256 dayReward = profit.sub(totalReward);
                    uint256 avgReward = dayReward.div(dayBlocks);
                    totalReward = totalReward.add(avgReward.mul(modNumber));
                }else {
...
interface IWallet {
    ...
    function getNFTOwners() external view returns(UserInfo[] memory);
}
