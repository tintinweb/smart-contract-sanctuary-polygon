...
contract RubicViaAggregator is ReentrancyGuard, AccessControl, Pausable {
    ...
    modifier onlyAdmin() {
        require(isAdmin(msg.sender), 'Caller is not in admin role');
        _;
    }

    modifier onlyManager() {
        require(isManager(msg.sender) || isAdmin(msg.sender), 'Caller is not in manager role');
        _;
    }
    ...
    constructor(
        ...
        for (uint256 i; i < length; ++i) {
            require(_crossChains[i] != address(0));
            whitelist.add(_crossChains[i]);
        }
    ...
    function viaCall(
        ...
        require(!whitelist.contains(crossChain), 'address is not in whitelist');

        uint256 _allowance = inputToken.allowance(address(this), crossChain);
        if (_allowance < totalInputAmount) {
            if (_allowance == 0) {
                inputToken.safeApprove(crossChain, type(uint256).max);
            } else {
                try inputToken.approve(crossChain, type(uint256).max) returns (bool res) {
                    require(res == true, 'approve failed');
                } catch {
                    inputToken.safeApprove(crossChain, 0);
                    inputToken.safeApprove(crossChain, type(uint256).max);
                }
            }
        }
    ...
    function viaCallWithNative(
        ...
        Address.functionCallWithValue(crossChain, data, inputAmount);
    }
    ...
    function addToWhitelist(address[] memory _crossChains) external onlyManager {
        for (uint256 i; i < _crossChains.length; ++i) {
            require(_crossChains[i] != address(0));
            whitelist.add(_crossChains[i]);
        }
    }

    function removeFromWhitelist(address[] memory _crossChains) external onlyManager {
        for (uint256 i; i < _crossChains.length; ++i) {
            require(_crossChains[i] != address(0));
            whitelist.remove(_crossChains[i]);
        }
    }

    function getWhitelist() public view returns (address[] memory _crossChains) {
