...
contract InsidersPass is InsidersPassAuction  {
    ...
    //Mumbai Testnet Aggregator: 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada
    //Polygon mainnet Aggregator: 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0    
     
       AggregatorV3Interface internal priceFeed = AggregatorV3Interface(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0);
         function getConvertedAmount(uint usdAmount) public view returns (uint256) {
        return (((1 ether) * usdAmount) / (getLatestPrice()));
    }
    ...
    function getUsdAmount(uint _usdAmount) public view returns (uint256) {
        return (((1 ether) * _usdAmount) / (getLatestPrice()));
    }
    ...
    //For an event scheduled from 5-20 jan
    //_eventStartDate: 1672858800
    //_eventEndDate: 1674154800

    //Renting Start & End time 12-15 December
    //1672858800
    //1672729834
    ...
    //[[1,1000000000000000000,[1674154800] ,100,"http://URI",0, 2]]

    // 13 & 14 November
    //[[1,1000000000000000000,[1672513200,1672945200] ,100,"http://URI",0, 2]]
    ...
    //eventID => PromoCode => fixAmountDiscount
    mapping(uint64 =>mapping(string => uint32)) public fixedDiscount;

     mapping(uint64 =>mapping(string => uint32[])) promoValidityDates;
    ...
    constructor() ERC1155("") {
     ...
     struct PromoCodes {
        ...
        uint64 eventIDforPromo;
        uint32 promoStartDate;
        uint32 promoEndDate;
        uint32 fixedDiscount;
        uint32 maxUsers;
        uint32 claimedUsers;
    ...
    function checkPrice(uint _price) internal view {
        ...
        
    {
        ...
        require((ticketTypeDetails[ticketID].copiesSold + noOfCopies) <= ticketTypeDetails[ticketID].totalCopies, "Copies unavailable");
        //Admin will mint Tickets freely in case of fiat payments
        if(_msgSender() != owner()){
             if(keccak256(abi.encodePacked(_promoCode)) != keccak256(abi.encodePacked(""))){
                require(promoCodeDetails[_promoCode].claimedUsers < promoCodeDetails[_promoCode].maxUsers, "Users limit reached");
                require(promoCodeDetails[_promoCode].promoStartDate < block.timestamp && promoCodeDetails[_promoCode].promoEndDate > block.timestamp, "Not Valid");
                require(promoCodeDetails[_promoCode].eventIDforPromo == ticketTypeDetails[ticketID].eventIDforTicket, "Promo Not Valid");
                if(promoCodeDetails[_promoCode].discountIsPercentage){
                    uint newDiscountedAmount =ticketTypeDetails[ticketID].ticketFixPrice - ((promoCodeDetails[_promoCode].discountPercentage/100)*(ticketTypeDetails[ticketID].ticketFixPrice)) ;
                    if(promoCodeDetails[_promoCode].maxDiscountAmount > newDiscountedAmount){
                       checkPrice(getUsdAmount((newDiscountedAmount*noOfCopies)));
                        promoCodeDetails[_promoCode].claimedUsers++;
                    }
                        
                    else{
                       checkPrice(getUsdAmount((promoCodeDetails[_promoCode].maxDiscountAmount*noOfCopies)));
                        promoCodeDetails[_promoCode].claimedUsers++;
                    }
                        
                }
                else if(!promoCodeDetails[_promoCode].discountIsPercentage){
                     uint newDiscountedAmount =ticketTypeDetails[ticketID].ticketFixPrice - (promoCodeDetails[_promoCode].fixedDiscount);
                     if(promoCodeDetails[_promoCode].maxDiscountAmount > newDiscountedAmount){
                          checkPrice(getUsdAmount((newDiscountedAmount*noOfCopies)));
                         promoCodeDetails[_promoCode].claimedUsers++;
                     }
                 ...
                 checkPrice(getUsdAmount(ticketTypeDetails[ticketID].ticketFixPrice*noOfCopies));
             }
    ...
    function addPromoCodes( 
       ...
       PromoCodes[] memory _promoCodes
            ...
            eventIsRegistered(_eventID);
                ...
                for(uint i = 0; i <  _promoCodes.length; i++){
                    promoCodeDetails[promoCodes[i]] = _promoCodes[i];
                }
        }
        ...
        returns (bool)
    {
        ...
        require(
            balanceOf(_user, ticketID) > 0 || Ticket[_user][ticketID].Exists == true, "You dont have this ticket"
        );
        bool response;
        for (
            uint8 i = 0;
    ...
    InsidersPassRoyalties //is SpaceERC20
{
    ...
    function eventIsNotPaused(uint64 _eventID) internal view {
        require(eventIsPaused[_eventID] == false, "Event is Paused");
    }

    //To check if even already exists or not
    function eventIsRegistered(uint64 _eventID) internal view {
        require(eventExists[_eventID] == true, "Event not exist");
    }

    //To check if even already exists or not
    function TicketExists(uint256 ticketID) internal view {
        require(ticketExists[ticketID] == true, "Ticket not exist");
    }
    ...
    function buyPackageForFixedPrice(
        ...
        setApprovalForAll(address(this), true);
        emit packageSold(packageID, "Sold for Fixed Price");
    }
    ...
    function placePackageForTimedAuction(
        ...
        emit availableForAuction(newPackageID, " Accepting Bids");
    }
    ...
    function placePackageForFixedPrice(
        ...
        emit availableForFixPrice(newPackageID, "Available for Fixed Price");
    }
    ...
    //     uint256 packageFixPrice
    // ) external {
    //     require(_endTime > block.timestamp, "End Time > Current Time");
    //     checkPackageConditions(_eventID, _ticketIDs, ticketCopiesCount);
    //     packagesCount++;
    //     uint256 newPackageID = packagesCount;
    //     CurrentStatus = saleTypeChoice(1);
    //     placePackageForSale(
    //         newPackageID,
    //         _ticketIDs,
    //         ticketCopiesCount,
    ...
    //         0
    //     );
    //     emit packageCreated(newPackageID, ticketsPackage[newPackageID]);
    //     emit availableForRent(newPackageID, "Available for Rent");
    // }

    //PENDING YET
    // function borrowPackage(
    //     address to,
    //     uint256 packageID,
    //     uint256 _eventID
    // ) external payable {
    ...
    //         "Insufficient Amount"
    //     );
    //     ticketsPackage[packageID].salestatus = saleTypeChoice(3);
    //     for (
    //         uint256 i = 0;
    //         i < ticketsPackage[packageID].ticketIDs.length;
    //         i++
    ...
    //         eventDetails[_eventID].servicePercentage
    //     );
    //     emit packageSold(packageID, "Package Borrowed");
    //     setApprovalForAll(address(this), true);
    // }
    ...
    //             ""
    //         );
    //         emit packageSold(packageID, "Sold Claimed by Original Owner");
    //abc
    //     }
    // }
        ...
        returns (bool accepted)
    {
        ...
        if (result == false) {
            ...
            emit packageSold(packageID, "Sold for Timed Auction");
