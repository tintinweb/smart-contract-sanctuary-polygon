 ...
 *
 */
abstract contract ERC1155Roles is AccessControlUpgradeable

{
  ...
  function __ERC1155Roles_init() internal {
...
/////////////////////////////////////
/**
 * @title [ERC1155 Non-Fungible Token implementation]
 * @dev see https://eips.ethereum.org/EIPS/eip-1155
 * @author Virtualness
 *
 */

contract ERC1155Virtualness is  
    ERC1155Roles, 
    OwnableUpgradeable, 
    UUPSUpgradeable, 
    ...
    event Supply(uint256 indexed tokenId, uint256 indexed supply);
    event Creators(uint256 indexed tokenId, LibPart.Part[] indexed creators);
    ...
    */
    function initialize(string memory _contratctUri, string memory _name, string memory _symbol) external initializer {
        __ERC1155_init("");
        __ERC1155Roles_init();
        __UUPSUpgradeable_init();
        __Ownable_init();
        __EIP712_init("ERC1155Virtualness", "1.0");
        contractURI_ = _contratctUri;
        name = _name;
        symbol = _symbol;
    }
    ...
    * @param _supply          Token ID to mint
    * 
    */
    function mint(
        uint256 _supply,
        ...
        require(_creator != address(0), "ERC1155: mint to the zero address");
        require(supply[_tokenId] == 0, "TokenId already minted");

        address sender = _msgSender();
        LibPart.Part[] memory creatorsOfToken = new LibPart.Part[](1);
        
        creatorsOfToken[0]= LibPart.Part({
            account: payable(_creator),
            value: uint96(_supply)
        });

        _saveSupply(_tokenId, _supply);
        _setURI(_tokenId, _tokenURI);
        _setTokenRoyalty(_tokenId, _creator, _royalityFee);
        _saveCreators(_tokenId, creatorsOfToken);
        
        mint_(sender, _tokenId, _supply, "");

    }

    /**
    * @dev Mints some amount of tokens to an address
    * @param data          Mint1155Data
    * @param _to          Token ID to mint
    ...
    */
    function lazyMint(
        ...
        require(data.creators.length == data.signatures.length, "Invalid signature length");
        ...
        for (uint i = 0; i < data.creators.length; ++i) {
                ...
                if (creator != sender) {
                    bytes32 hash = _hashToSign(structHash);
                    signer = ECDSAUpgradeable.recover(hash, data.signatures[i]);
                    require(signer == owner(), "Creator signature is invalid");
                }
        }
        ...
        if (supply[data.tokenId] == 0) {
            ...
            _setTokenRoyalty(data.tokenId, data.royalties[0].account, data.royalties[0].value);
            _saveCreators(data.tokenId, data.creators);
            _setURI(data.tokenId, data.tokenURI);
        }

        require(from == owner() || hasRole(MINTER_ROLE, signer), "wrong order maker");
        
        mint_(_to, data.tokenId, _amount, "");
    ...
    /**
    * @dev Returns the creator address of given token ID. 
    */
    ...
    /**
    * @dev Returns the Contract URI set via {_setContractURI}. 
    */
    ...
    */
    ...
    /**
    * @dev Burn the tokenId
    * 
    * @param account owner of the token Id
    * @param id of token in existence
    * @param value no of token to burn
    */
    function burn(
        address account,
        uint256 id,
    ...
    * 
    * @param account owner of the token Id
    * @param ids of tokens in existence
    * @param values no of tokens to burn
    */
    function burnBatch(
        address account,
        uint256[] memory ids,
    ...
    function supportsInterface(bytes4 interfaceId) 
        ...
        view 
        virtual 
        override(ERC2981Upgradeable, ERC1155Upgradeable, AccessControlUpgradeable) 
        returns (bool) 
    {
     ...
     * @dev Transfers ownership of the contract to a new account (`newOwner`).
     * Can only be called by the current owner.
     * Assign OWNER_ROLE to new owner
     */
