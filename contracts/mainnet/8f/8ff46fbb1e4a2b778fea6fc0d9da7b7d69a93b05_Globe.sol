...
contract Globe is ERC721, ISnowV1Program, LogisticVRGDA {
  ...
  constructor()
    ERC721("Globe", "GLOBE")
    LogisticVRGDA(1, 0.31e18, toWadUnsafe(MAX_SUPPLY), 0.1e18)
  {
    spriteIndex = 14;
  ...
  function run() public baller {
    ...
    SNOW.storeProgram(owned, address(this));
  ...
  function tokenURI(uint256) public view virtual override returns (string memory) {
    return string(abi.encodePacked(BASE_URI, uint2str(owned)));
  }
  ...
  /// @notice Pulls the snow token and sets the clawback
  function pull(address from, uint256 tokenId) external {
    SNOW.safeTransferFrom(from, address(this), tokenId);
    clawbacks[from][tokenId] = msg.sender;
    owned = tokenId;
  }
