...
contract evolvingnft5 is ERC1155Drop {
    uint256 public TYPE_1000_SUPPLY_MAX = 0;
    uint256 public TYPE_2500_SUPPLY_MAX = 0;
    uint256 public TYPE_5000_SUPPLY_MAX = 0;
    uint256 public TYPE_10000_SUPPLY_MAX = 0;
    ...
    function setMaxSupply(uint256 tokenId, uint256 maxSupply) public onlyOwner {
        if (tokenId == 0) {
            TYPE_1000_SUPPLY_MAX = maxSupply;
        } else if (tokenId == 1) {
            TYPE_2500_SUPPLY_MAX = maxSupply;
        } else if (tokenId == 2) {
            TYPE_5000_SUPPLY_MAX = maxSupply;
        } else if (tokenId == 3) {
            TYPE_10000_SUPPLY_MAX = maxSupply;
        } else {
            revert("Invalid token ID");
        }
    }

    function getMaxTokenSupply(uint256 _tokenId) public view returns (uint256) {
    ...
    function _beforeClaim(
        ...
        if (totalSupply[_tokenId] + _quantity > getMaxTokenSupply(_tokenId)) {
            revert("Not enough tokens left in supply");
        }
        if (_quantity > getMaxTokenSupply(_tokenId) - totalSupply[_tokenId]) {
            revert("Requested quantity exceeds maximum supply of token");
        }
    }
    ...
    modifier nonReentrant() {
        require(!rentrancy_lock, "Reentrant call.");
        rentrancy_lock = true;
        _;
        rentrancy_lock = false;
    }

    //need to test if this works as internal
    function mintclaim(
        address _receiver,
        uint256 _tokenId,
        uint256 _quantity
        ...
        require(
            (totalSupply[_tokenId] + _quantity) <= getMaxTokenSupply(_tokenId),
            " not enough supply to mint token"
        );
    ...
    function evolveToken(uint256 _tokenId) public {
        uint256 amountOfTokens = balanceOf[msg.sender][_tokenId];

        if (_tokenId == 0) {
            require(amountOfTokens >= 5, "Not enought token 0 to evolve");
            // Burn 5 Token 0 and mint 1 Token 2
            _burn(msg.sender, 0, 5);
            mintclaim(msg.sender, 2, 1);
        }

        if (_tokenId == 1) {
            require(amountOfTokens >= 2, "Not enought token 1 to evolve");
            // Burn 2 Token 1 and mint 1 Token 2
            _burn(msg.sender, 1, 2);
            mintclaim(msg.sender, 2, 1);
        }

        if (_tokenId == 2) {
            require(amountOfTokens >= 3, "Not enought token 2 to evolve");

            // Burn 3 Token 2 and mint 1 Token 3
            _burn(msg.sender, 2, 3);
            mintclaim(msg.sender, 3, 1);
        }
    }
}