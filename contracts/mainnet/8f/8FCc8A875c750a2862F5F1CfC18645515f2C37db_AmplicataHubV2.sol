 ...
 * @title AmplicataHub
 */
contract AmplicataHubV2 is OwnableUpgradeable {
    ...
    /**
     * @notice Deposit approved funds to profile wallet 
     *
     * @param profileId Promoter profile id
     * @param amount amount to deposit
     */        
    function deposit(uint128 profileId, uint128 amount) public {
        if (amount == 0) 
    ...
    /**
     * @notice Withdraw available funds from wallet to profile owner 
     *
     * @param profileId Promoter profile id
     * @param amount amount to withdraw
     */ 
    function withdraw(uint128 profileId, uint128 amount) external { 
    ...
    /**
     * @notice Create promotion
     *
     * @param profileId Promoter profile id
     * @param pubId Promoter publication id
     * @param dataLayerId Promoter publication id 
     * @param promotionData Promotion structure
     * @param serviceSignature Promotion serviceSignature 
     */ 
    function createPromotion(uint128 profileId, uint128 pubId, uint128 dataLayerId, DataTypes.Promotion calldata promotionData, bytes calldata serviceSignature) public {
        _onlyProfileOwner(profileId);

        if (!_isSignatureValid(keccak256(abi.encode(profileId, pubId, dataLayerId, promotionData)), serviceSignature, config.serviceWallet)) // Service signature invalid
            revert ErrorCode(DataTypes.ErrorCode.mirror_serviceSignature);
        ...
        if (promotionData.duration == 0) {
            promotion.duration = config_.maxDuration; 
        } else {
            promotion.duration = promotionData.duration; 
            if (promotion.duration > config_.maxDuration) // Duration too long
                revert ErrorCode(DataTypes.ErrorCode.createPromotion_maxDuration);
        }                      
        
        // Profile Eligibility
        promotion.profileEligibility = promotionData.profileEligibility;        
        
        if (promotion.profileEligibility.minPostsToMirrorsRatio > 100) // Bad min posts to mirrors ratio
            revert ErrorCode(DataTypes.ErrorCode.createPromotion_minPostsToMirrorsRatio);

        if (promotion.profileEligibility.maxMirrors > config_.maxMirrors) // Bad min posts to mirrors ratio
            revert ErrorCode(DataTypes.ErrorCode.createPromotion_maxMirrors);
        
        if (promotion.profileEligibility.mirrorsForLastDuration != 0) { 
        ...
        // Reward Parameters        
        promotion.rewardParameters = promotionData.rewardParameters;
        
        // publish duration
        if (promotion.rewardParameters.publishDuration != 0) {
            if (promotion.rewardParameters.publishDuration < config.minPublishDuration) // Bad min posts to mirrors ratio
                revert ErrorCode(DataTypes.ErrorCode.createPromotion_minPublishedDuration);            
            if (promotion.rewardParameters.publishDuration > config.maxPublishDuration) // Bad max posts to mirrors ratio
                revert ErrorCode(DataTypes.ErrorCode.createPromotion_maxPublishedDuration);
        }  

        // receive duration      
        if (promotion.rewardParameters.commentsReceived != 0 || promotion.rewardParameters.likesReceived != 0) {
            if (promotion.rewardParameters.receiveDuration < config.minReceiveDuration) // Bad min c/l receive duration
                revert ErrorCode(DataTypes.ErrorCode.createPromotion_minCommentsAndLikesReceiveDuration);            
            if (promotion.rewardParameters.receiveDuration > config.maxReceiveDuration) // Bad max c/l receive duration
        ...
        for (uint256 i = 0; i < rewardTiersLength;) {
            ...
            promotion.rewardTiers.push(rewardTier);            
            unchecked { i++; }
        }
                
        _emitEvent(DataTypes.EventCode.PROMO_CREATE, abi.encode(profileId, pubId, dataLayerId, promotion));
	}
        
    /**
     * @notice Update promotion 
     *
     * @param profileId Promoter profile id
     * @param pubId Promoter publication id
     * @param dataLayerId Promoter publication id
     * @param promotionData New budget
     */
    function updatePromotion(uint128 profileId, uint128 pubId, uint128 dataLayerId, DataTypes.Promotion calldata promotionData) external {
        _onlyProfileOwner(profileId);
    ...
    /**
     * @notice Remove promotion
     *
     * @param profileId Promoter profile id
     * @param pubId Promoter publication id
     * @param dataLayerId Promoter dataLayer id
     */
    function removePromotion(uint128 profileId, uint128 pubId, uint128 dataLayerId) external {
        _onlyProfileOwner(profileId);
    ...
    /**
     * @notice Mirror publication by influencer
     *
     * @param mirrorData Input params
     * @param serviceSignature Service wallet signature of encoded mirrorData
     */
    function applyPromotion(DataTypes.MirrorData calldata mirrorData, bytes calldata serviceSignature) external returns (DataTypes.Mirror memory) {
        uint128 profileId = mirrorData.profileId;
        _onlyProfileOwner(profileId);
        
        if (!_isSignatureValid(keccak256(abi.encode(mirrorData)), serviceSignature, config.serviceWallet)) // Service signature invalid
            revert ErrorCode(DataTypes.ErrorCode.mirror_serviceSignature);
        ...
        // check influencer met profile age condition
        if (promotion.profileEligibility.minAge != 0) {
            uint256 profileCreationTime = lensHub.mintTimestampOf(profileId);
            
            if (block.timestamp - profileCreationTime < promotion.profileEligibility.minAge) // Too young
                revert ErrorCode(DataTypes.ErrorCode.mirror_minAge);
        }
        ...
        if (mirrorData.rewardTierIdx_ >= promotion.rewardTiers.length) // Assigned reward tier not exist
            revert ErrorCode(DataTypes.ErrorCode.mirror_rewardTierIdx);
        mirror.reward = promotion.rewardTiers[mirrorData.rewardTierIdx_].amount;
        mirror.serviceFee = mirror.reward * config.serviceFee / feeBase;
    ...
    function distribute(DataTypes.Distribution[] calldata items) external returns (uint8[] memory result) {       
        ...
        for (uint256 i = 0; i < items.length;) {
            ...
            if (mirror.pubId == 0) { // Mirror not found
                result[i] = 1;
                continue; 
            } 
           
            if (mirror.reward == 0) { // Already rewarded
                result[i] = 2;
                continue;
            }                
            ...
            if (lensHub.exists(profileId)) {                
                ...
                if (item.isReward) {
                    ...
                    paymentToken.safeTransfer(lensHub.ownerOf(profileId), mirror.reward); 
    ...
    /**
     * @notice Set contract configuration values. 
     * @dev Only owner can call this method.
     * All variables will be overwritten with new values. 
     * In case of changing single variable provide old values for all else variables. 
     *
     * @param configData config structure
     */
    function setConfig(DataTypes.Config memory configData) public onlyOwner {   
        config = configData;
        
        if (config.serviceWallet == address(0)) //Bad service wallet
            revert ErrorCode(DataTypes.ErrorCode.setConfig_serviceWallet); 
        
        if (config.serviceFee > 50) //Bad service fee, 5% max
            revert ErrorCode(DataTypes.ErrorCode.setConfig_serviceFee);       
        
        if (config.minReward < feeBase) //Min reward lower than fee base
            revert ErrorCode(DataTypes.ErrorCode.setConfig_feeBase);
               
        if (config.maxRewardTiers < 1 && config.maxRewardTiers > 30) // Bad max reward tires 1-30
            revert ErrorCode(DataTypes.ErrorCode.setConfig_maxRewardTiers);                 
                
        if (config.maxDuration < 60 days && config.maxDuration > 120 days) //Bad max duration 60-120 days
            revert ErrorCode(DataTypes.ErrorCode.setConfig_maxDuration);
        
        if (config.minPublishDuration < 1 hours) // Bad min publish duration 1h-max
            revert ErrorCode(DataTypes.ErrorCode.setConfig_minPublishDuration); 
                
        if (config.maxPublishDuration <= config.minPublishDuration || config.maxPublishDuration > 60 days) //Bad max publish duration min-60d
            revert ErrorCode(DataTypes.ErrorCode.setConfig_maxPublishDuration);
        
        if (config.minReceiveDuration < 1 hours) // Bad min receive duration 1h-max
            revert ErrorCode(DataTypes.ErrorCode.setConfig_minReceiveDuration);
        
        if (config.maxReceiveDuration <= config.minReceiveDuration || config.maxReceiveDuration > 60 days) //Bad max receive duration min-60d
            revert ErrorCode(DataTypes.ErrorCode.setConfig_maxReceiveDuration);

        if (config.maxMirrors > 150) //Bad max duration 60-120 days
            revert ErrorCode(DataTypes.ErrorCode.setConfig_maxMirrors);

        if (config.mirrorsForLastDuratioWeeks > 10) //Bad max duration 60-120 days
            revert ErrorCode(DataTypes.ErrorCode.setConfig_mirrorsForLastDuratioWeeks);
             
        _emitEvent(DataTypes.EventCode.CONFIG, abi.encode(config));                   
	}

    /**
     * @notice Recover stucked tokens in contract. 
     * @dev Only owner can call this method.
     * If token = 0 address eth will be recovered
     * If token = payment token valid burned profile id must be provided
     *
     * @param token address of token to recover (set 0 for eth currency)
    ...
    function recover(address token, uint128 amount) external onlyOwner {
        ...
        if (token == address(0)) {
            ...
            IERC20Upgradeable(token).safeTransfer(config.serviceWallet, amount);
		} 
    ...
    /**
     * @dev Performs ECDSAUpgradeable signature check 
     */
    function _isSignatureValid(bytes32 dataHash, bytes calldata signature, address signer) internal pure returns (bool) {
