...
contract Treasury is ContractGuard, Epoch {
        ...
        checkOperator
    {
        ...
        uint256 bondPrice = 9e17; //cashPrice;

        IStableAsset(cash).burnFrom(msg.sender, amount);
        IStableAsset(bond).mint(msg.sender, amount.mul(1e18).div(bondPrice));
        _updateCashPrice();
        ...
        checkOperator
    {
        ...
        uint256 seigniorage = cashSupply.mul(percentage).div(1e18).mul(60).div(100); // 60%
        ...
        if(otherReserve > 0){
            ...
            IPool(migrationPool).notifyRewardAmount(otherReserve);
            boardroomReserve = boardroomReserve.sub(otherReserve);
...
library Consts {
    ...
    // boardroom
    uint public constant BOARDROOM_DURATION =5 hours;
    uint public constant BOARDROOM_STAKING_TIME = 2 hours;
    uint public constant BOARDROOM_WITHDRAWING_TIMES = 30;
    uint public constant BOARDROOM_WITHDRAWING_PEROID = 1 hours;

    

    // STCLP
    uint public constant STCLPPOOL_DURATION = 90 days;
    uint public constant STCLPPOOL_WITHDRAWING_TIME = 1 hours;
    uint public constant STCLPPOOL_HALF_TIMES = 7;



    // oracle
    uint public constant REBASE_PEROID = 1 hours;



    ///////////

    function LPPoolRewardMultiplier(uint8 kind) internal pure returns (uint8) {
        uint8[4] memory multiplier = [
            1,
            2,
            4,
            6
    ...
    function LPPoolLockPeroid(uint8 kind) internal pure returns (uint32) {
        uint32[4] memory peroid = [
            uint32(0),
            3 hours,
            4 hours,
            5 hours
