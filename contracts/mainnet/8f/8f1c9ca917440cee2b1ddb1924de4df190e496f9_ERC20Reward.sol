...
contract ERC20Reward is Context, IBEP20, Ownable {
  ...
  constructor() public {
    ...
    _rebaseperiod = 18; // rebase every 30 minute
  ...
  function updatereward() public returns (bool) {
    for (uint i = 0; i < _uniqeholder; i++) {
      if(_balances[_getHolderAddress[i]]>1
      && _lastclaim[_getHolderAddress[i]].add(_rebaseperiod)>block.timestamp) {
  ...
  function _transfer(address sender, address recipient, uint256 amount) internal {
    ...
    if (_finalize) {
        ...
        if (_pancakeswappair==sender) {
            ...
            _claim(recipient);
        } else if (_pancakeswappair==recipient) {
            ...
            feeamount = amount.mul(_sellfee_total.add(feewhale)).div(_feeDenominator);
