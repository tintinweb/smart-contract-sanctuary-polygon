...
contract AdManager is Initializable {
    ...
    event campaignEvent(address _owner, address _campaign, Reward _reward, string _metadata, string _secret_metadata, bool _active);
    ...
    function addCampaign(
          ...
          string memory _secret_metadata,
          bool _active,
        ...
        emit campaignEvent(msg.sender, campaigns[msg.sender][campaigns[msg.sender].length - 1], _reward, _metadata, _secret_metadata, _active);
    ...
    function setCampaign(
        ...
        c.setSecretMetadata(_secret_metadata);
        c.setActive(_active);
        ...
        c.setBroker(_broker);
        emit campaignEvent(msg.sender, _address, _reward, _metadata, _secret_metadata, _active);
    }
...
contract Campaign is Initializable {
    ...
    function initialize(Reward memory _reward, string memory _metadata, string memory _secret_metadata, bool _active, bool _payImpressionRewards, bool _thirdPartyValidation, address _oracle, address _accounts, address _broker, address _owner) public initializer {
     ...
     */
    function processEngagements(string[] memory _tokens, uint256[] memory _amount) external {
        if ((msg.sender == owner || msg.sender == broker) && active == true) {
            ...
            for (uint i = 0; i < _tokens.length; i++) {
                ...
                rewards[i] = _amount[i] * reward.userReward;
                rewards[i] = _amount[i] * reward.publisherReward;
                rewards[i] = _amount[i] * reward.brokerReward;
    ...
    function getSecretMetadata() public view returns (string memory) {
    ...
    function setSecretMetadata(string memory _metadata) public {
        if (msg.sender == adManager) {
...
contract EngageToken is ERC20, AccessControlEnumerable {
    ...
    function setMinterTotal(address minter, uint256 total) external onlyRole(GOVERNMENT_ROLE) {
        require(hasRole(MINTER_ROLE, minter), "not minter");
    ...
    function mint(address account, uint256 amount) external onlyRole(MINTER_ROLE) returns (bool) {
        ...
        require(amount <= minterSupply[msg.sender].max, "ERC20: mint exceeds limit");
        require(minterSupply[msg.sender].total + amount <= minterSupply[msg.sender].cap, "ERC20: mint exceeds cap");
    ...
    function burn(address account, uint256 amount) external onlyRole(MINTER_ROLE) returns (bool) {
        ...
        require(minterSupply[msg.sender].total - amount > 0, "ERC20: burn exceeds limit");
        require(totalSupply() + amount > MIN_SUPPLY);
