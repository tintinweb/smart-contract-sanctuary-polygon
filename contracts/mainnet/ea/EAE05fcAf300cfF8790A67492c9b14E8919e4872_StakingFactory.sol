...
contract StakingFactory is Ownable {
    ...
    constructor(address _stakingToken, uint _stakingGenesis) Ownable() {
        require(
            ...
            "StakingFactory::constructor: genesis too soon"
        );
    ...
    function deploy(
        ...
        require(info.Staking == address(0), "StakingFactory::deploy: already deployed");

        info.Staking = address(
            new Staking(/*_rewardsDistribution=*/ address(this), rewardToken, stakingToken)
        );
    ...
    function update(
        ...
        StakingInfo storage info = StakingInfoByRewardToken[rewardToken];
        require(info.Staking != address(0), "StakingFactory::update: not deployed");
    ...
    function notifyRewardAmounts() public onlyOwner {
        require(
            ...
            "StakingFactory::notifyRewardAmounts: called before any deploys"
        );
    ...
    function notifyRewardAmount(address rewardToken) public onlyOwner {
        require(block.timestamp >= stakingGenesis, "StakingFactory::notifyRewardAmount: not ready");

        StakingInfo storage info = StakingInfoByRewardToken[rewardToken];
        require(info.Staking != address(0), "StakingFactory::notifyRewardAmount: not deployed");
        ...
        if (info.rewardAmount > 0 && info.duration > 0) {
            ...
            require(
                IERC20(rewardToken).transfer(info.Staking, rewardAmount),
                "StakingFactory::notifyRewardAmount: transfer failed"
            );
            Staking(info.Staking).notifyRewardAmount(rewardAmount, duration);
        }
    }
