...
// TODO also add a cancel function for the GET foundation! 
// TODO add a delete function for if they want to break association
// TODO add a emergency function to change state of structs (and so forth) only callable by the DAO

/**
 * ClearingHouse contract takes custody of pallets. This is done by registering an 'Offering', which entails that a Teller Loan Bid is couples with a pallet as 'combined offering'. 
 * Take note that the Teller state is 'leading'. This contract should only change the state or owner of a pallet, after consulting the Teller contract and checking if the action fits in the flow.
 * This is why this contract is called a clearing house, it moves assets according to orders or information external from this contract.
 */
