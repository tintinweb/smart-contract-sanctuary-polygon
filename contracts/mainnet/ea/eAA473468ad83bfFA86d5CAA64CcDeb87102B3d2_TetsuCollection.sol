...
contract TetsuCollection is Ownable, DefaultOperatorFilterer, AccessControl, ERC721Enumerable, ERC721URIStorage, ERC721Royalty, CantBeEvil(LicenseVersion.PERSONAL_NO_HATE) {
    constructor(string memory _name, string memory _symbol, address[] memory admin, address walletToMintTo, uint256 mintAmount, uint256 _maxAmount, address royaltyRecipient, uint96 basisPoints, string[] memory _tokenURI)
    ERC721(_name, _symbol) {

        for (uint256 i = 0; i < admin.length; i++) {
            _grantRole(DEFAULT_ADMIN_ROLE, admin[i]);
        }

        for (uint256 i = 0; i < mintAmount; i++) {
            mint(walletToMintTo, _tokenURI[i]);
        }

        _setDefaultRoyalty(royaltyRecipient, basisPoints);
        maxAmount = _maxAmount;
    }

    uint256 maxAmount;

    function mint(address addr, string memory _tokenURI) public onlyRole(DEFAULT_ADMIN_ROLE) {
        
        if (maxAmount != 0) {
            require(totalSupply() < maxAmount, "All NFTs have been minted!");
        } 
    ...
    function supportsInterface(bytes4 interfaceId) public view override(AccessControl, ERC721, ERC721Enumerable, ERC721Royalty, CantBeEvil) returns(bool)  {
