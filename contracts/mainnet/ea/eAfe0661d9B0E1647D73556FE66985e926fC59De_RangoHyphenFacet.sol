...
contract RangoHyphenFacet is IRango, ReentrancyGuard, IRangoHyphen {
    ...
    /// @param _address The address of whitelist contracts for bridge
    ...
    function doHyphenBridge(
        ...
        require(hyphenAddress != LibSwapper.ETH, 'Hyphen address not set');
        require(block.chainid != dstChainId, 'Cannot bridge to the same network');

        if (token == LibSwapper.ETH)
            require(amount <= msg.value, "Insufficient ETH sent to RangoHyphen");
        else
            SafeERC20.safeTransferFrom(IERC20(token), msg.sender, address(this), amount);

        if (token == address(0)) {
            IHyphenBridge(hyphenAddress).depositNative{ value: amount }(receiver, dstChainId, "Rango");
            emit HyphenNativeDeposit(dstChainId, receiver, amount);
        } else{
            LibSwapper.approve(token, hyphenAddress, amount);
            IHyphenBridge(hyphenAddress).depositErc20(dstChainId, token, receiver, amount, "Rango");
            emit HyphenERC20Deposit(dstChainId, token, receiver, amount);
        }
    }
