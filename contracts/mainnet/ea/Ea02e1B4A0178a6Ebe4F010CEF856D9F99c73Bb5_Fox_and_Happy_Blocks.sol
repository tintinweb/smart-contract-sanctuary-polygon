...
contract Fox_and_Happy_Blocks is ERC721A, DefaultOperatorFilterer, Ownable, ReentrancyGuard {
    uint256 public MAX_SUPPLY = 10;

    
    uint256 public FHB_SUPPLY = 9;
    
    bool public IS_FHB_ACTIVE = false;

    
    uint256 public FHB_TX_LIMIT = 1;

    
    uint256 public FHB_WALLET_LIMIT = 1;

    
    uint256 public FHB_PRICE = 0 ether;

    
    uint256 public FHB_COUNT = 0;

    

    
    mapping(address => uint) public FHB_Minted;

    bool public _revealed = false;

    string private baseURI = "";
    string private preRevealURI = "https://ipfs.w3bmint.xyz/ipfs/QmWHwhGpP1dSBaQeAxZhFwPwJZugjLfBXqdceodiwdoDEY";
    ...
    constructor() ERC721A("Fox_and_Happy_Blocks", "FHB") {
    ...
    function FHBMint(address to, uint256 numberOfTokens) external isSecured(FHB_PRICE * numberOfTokens) payable {
        validateMint(IS_FHB_ACTIVE, FHB_COUNT, FHB_SUPPLY, FHB_Minted[msg.sender], FHB_WALLET_LIMIT, FHB_TX_LIMIT, numberOfTokens, true);
        addressBlockBought[to] = block.timestamp;
        FHB_Minted[to] += numberOfTokens;
        FHB_COUNT += numberOfTokens;
    ...
    function toggleFHBMintStatus() external onlyOwner {
        IS_FHB_ACTIVE = !IS_FHB_ACTIVE;
    }

    // SUPPLY CHANGERS
    
    function setFHBSupply(uint256 _supply) external onlyOwner {
        FHB_SUPPLY = _supply;
    }
    ...
    function setFHBPrice(uint256 _price) external onlyOwner {
        FHB_PRICE = _price;
    }
