...
contract Semantic is ISemantic, AccessControl,ERC721 {
    ...
    function claim(ClaimOperateData[] memory claimOperateDatas) external {
        for (uint256 i = 0; i < claimOperateDatas.length; i++) {
            ...
            string memory rdf = generateRdf(operateData.subject.classId,operateData.subject.value,operateData.predicateId,operateData.object);
            ...
            uint256 subjectId = generateSubjectId(operateData.subject.classId,operateData.subject.value);
            Triple memory triple = Triple(subjectId,operateData.predicateId,operateData.object);
            _mint(msg.sender,Strings.toHexString(msg.sender),1,triple);
