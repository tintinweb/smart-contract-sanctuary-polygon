...
contract RewardSharks is Ownable, ReentrancyGuard, AccessControl {
    ...
    function updateRewardAddress(IERC20 _rewardTokenAddress)
        external
        onlyOwner
    {
        rewardTokenAddress = _rewardTokenAddress;
    ...
    function ifBoostTypeAvailable(uint256 _boostType)
        public
        view
        returns (bool)
    {
    ...
    function userBoostAvailabitiy(uint256 _boostType, address _address)
        internal
        returns (bool)
    {
    ...
    function returnSingleSharkData(uint256 _tokenID)
        external
        view
        returns (Sharks memory)
    {
        return getSharks[_tokenID];
    }

    function returnOwnerAllSharkData(address _address)
        external
        view
        returns (
            Sharks[] memory,
        ...
        )
    {
        ...
        for (uint256 i = 0; i < ownerTokenList[_address].length; i++) {
            ...
            _boostCounter += getSharks[ownerTokenList[_address][i]]
                .activeBoost
                .length;
        }
        ...
        for (uint256 i = 0; i < ownerTokenList[_address].length; i++) {
            ...
            for (uint256 j = 0; j < _sharkTemp.activeBoost.length; j++) {
                _boosts[currentBoostCounter] = getBooster[
                    _sharkTemp.activeBoost[j]
                ];
        ...
        Boosts[] memory _boostsAvailable = new Boosts[](
            _tempAvailableBoosts.length
        );
        uint256 counter2 = 0;
    ...
    function getOwnerList(address _address)
        external
        view
    ...
    function getActiveBoost(uint256 _tokenId, uint256 activeBoostNumber)
        external
        view
        returns (uint256)
    {
    ...
    function activateBooster(
        ...
        require(hasRole(BOOST_ROLE, msg.sender), "Caller is not a booster");
        require(
            userBoostAvailabitiy(_boostType, _address),
    ...
    function checkFixedBooster(uint256 _tokenId) public view returns (uint256) {
        ...
        uint256 timeDiffMonth = (block.timestamp - _shark.stakingTimeStamp) /
            30 days;
        rewardAdder = rewardAdder + timeDiffMonth.mul(5);
        return (rewardAdder > 100 ? 100 : rewardAdder);
    }

    function calculateTotalRewards(uint256 _tokenId)
        public
        view
        returns (uint256)
    {
        ...
        uint256 rewardPercent = checkFixedBooster(_tokenId);
        // S2 Booster
        if (_shark.shiver) {
        ...
        for (uint256 i = 0; i < activeBoosters.length; i++) {
            ...
            if (_boost.expireTimeStamp > block.timestamp) {
                rewardPercentS3 =
                    rewardPercentS3 +
                    _boost.boostAmountPercentage;
            }
        }
        rewardPercent =
            rewardPercent +
            (rewardPercentS3 > 100 ? 100 : rewardPercentS3);
    ...
    function claimRewards(uint256 _tokenId) public {
        ...
        uint256 rewardPerMinute = tokenIdReward[_tokenId]
            .mul(10**18)
            .div(24)
            .div(60);
        uint256 timeElapsed = (block.timestamp - _shark.lastClaimTimeStamp) /
            1 minutes;
        uint256 amountReward = timeElapsed
            .mul(rewardPerMinute)
            .mul(100 + rewardPercent)
            .div(100);
    ...
    modifier onlyNFT() {
        require(
            ...
            "Can be only accessed by Raffle Contract"
        );
        _;
    }
    ...
    modifier onlyDutchRaffle() {
        require(
            msg.sender == dutchRaffle,
            "Can be only accessed by Dutch Raffle Contract"
        );
        _;
    }
    ...
    function updateTotalLockTokens(uint256 _amount, address _address)
        external
        onlyDutchRaffle
    {
    ...
    function removeTokenFromOwnerList(uint256 _tokenId) internal {
        uint256[] storage listOfTokens = ownerTokenList[
            getSharks[_tokenId].ownerAddress
        ];
        uint256 index = 0;
        bool flag = false;
    ...
    function makeShiver(uint256[] memory listOfTokens, address _address)
        external
    {
        require(hasRole(BOOST_ROLE, msg.sender), "Caller is not a booster");
        require(
            listOfTokens.length == 5,
            "Shiver can be only made with 5 Tokens"
        );
        for (uint256 i = 0; i < 5; i++) {
            Sharks memory _shark = getSharks[listOfTokens[i]];
            require(
                _shark.shiver == false,
                "Shiver already activated for this token"
            );
            require(
                _shark.ownerAddress == _address,
                "The address passed is not owner"
            );
    ...
    function updateAllRewards(
        ...
        require(hasRole(BOOST_ROLE, msg.sender), "Caller is not a booster");
        require(
            _rewards.length == _tokenIds.length,
