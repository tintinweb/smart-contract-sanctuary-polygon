...
contract Reward is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
  ...
  /** @dev administrative function to manage weekly state. must be called prior to batchTransfer weekly. Only the admin is allowed to force an update; the oracle can only advance the week.
   * @param forceUpdate boolean to force an update, ignoring the date check
   */
  function updateWeek(bool forceUpdate) external {
    if (!hasRole(DEFAULT_ADMIN_ROLE, msg.sender) && (!hasRole(ORACLE_ROLE, msg.sender) || forceUpdate)) {
      revert("Missing required role");
    }
