...
contract KaliLinux is ERC721, PaymentSplitter {
    ...
    constructor(address[] memory _payees, uint256[] memory _shares)
        ERC721("Linux Layer", "LINUX")
        PaymentSplitter(_payees, _shares)
    {}

    function mint() external payable {
        if (totalSupply + 1 > 5014) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked(
                    "ipfs://QmUjKKKKVY9QA3F23sKm23rqAVVvsdnzA5efUEDtVLsUTT/",
