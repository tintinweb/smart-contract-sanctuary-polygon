...
contract MTDoge is IERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint public subTime = 1 days;
    uint public rate = 128;
    uint256 public mkFee = 500;
    ...
    function _updateBal(address owner, uint time) internal {
        ...
        if (bal > 0) {
            ...
            if (bal > updatedBal) {
                ...
                _balances[fundAddress2] = _balances[fundAddress2].add(ba);
                emit Transfer(owner, fundAddress2, ba);
            }
        } else {
            lastAddLqTimes[owner] = time;
        }
    ...
    function setFundAddress2(address addr1) external onlyOwner {
        fundAddress2 = addr1;
        exemptFee[addr1] = true;
    }
