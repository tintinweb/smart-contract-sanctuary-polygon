...
abstract contract BaseDoubleJumpRateModel is Ownable, InterestRateModelI {
  ...
  /**
   * @notice The aproximate number of blocks per day that is assumed by the interest rate model
   */
  uint256 private immutable _blocksPerDay;
  ...
  constructor(uint256 blocksPerYear_, uint256 blocksPerDay_) {
  ...
  function blocksPerDay() external view override returns (uint256) {
...
contract DoubleJumpRateModel is BaseDoubleJumpRateModel {
   ...
   * @param blocksPerDay_ Aproximation of blocks per day based on the chain's block time
   */
  constructor(
    uint256 baseRatePerYear_,
    uint256 multiplierPerYear_,
  ...
  ) BaseDoubleJumpRateModel(blocksPerYear_, blocksPerDay_) {
