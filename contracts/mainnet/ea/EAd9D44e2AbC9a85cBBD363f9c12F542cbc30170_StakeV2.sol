...
import { VariableDataV2 } from "./VariableDataV2.sol";

contract StakeV2 is ReentrancyGuardUpgradeable, ERC2771Recipient, VariableDataV2 {
    ...
    function staketokens(
        ...
        require(bytes(stakeType).length == 3, "StakeV1: Invalid stake type");
        require(
            keccak256(abi.encodePacked((stakeType))) == keccak256(abi.encodePacked(("PRJ"))),
            "StakeV1: Invalid stake type"
        );
        require(bytes(projectId).length != 0, "StakeV1: Invalid stake project id");
        ...
        emit StakeTokens(_msgSender(), stakeIndex, projectId, stakeType, quantity, escrowAccount, block.timestamp);
    ...
    function unstaketokens(uint256 stakeId, uint256 quantity) external nonReentrant returns (bool) {
        ...
        require(quantity > 0, "StakeV1: Unstake quantity should be greater than zero");
