...
contract Pools is Ownable, Pausable, ReentrancyGuard {
    ...
    CrystalShard private token;

    constructor(address tokenAddress) {
        token = CrystalShard(tokenAddress);
    }

    function setAddress(address tokenAddress) external onlyOwner {
        token = CrystalShard(tokenAddress);
    }

    function getPools() public view returns (Pool[] memory) {
        Pool[] memory ret = new Pool[](totalPools);
    ...
    function addPool(string memory name, uint256 total) external onlyOwner {
        uint256 id = totalPools;
        pools[id] = Pool({
                id: id,
                balance: 0,
                total: total,
    ...
    function withdraw(address to, uint256 amount) external onlyOwner {
        require(token.balanceOf(address(this)) >= amount, "Not enough to withdraw.");
        token.transferFrom(address(this),to, amount);
    }

    function pool(uint256 id, uint256 amount) public {
        Pool memory _pool = pools[id];
        require(!_pool.hidden, "Pool is not available.");
        require(token.balanceOf(_msgSender()) >= amount, "You do not have enough Tokens.");
        token.transferFrom(_msgSender(),address(this), amount);
        pools[id] = Pool({
                id: id,
                balance: _pool.balance + amount,
                total: _pool.total,
                name: _pool.name,
                hidden: _pool.balance + amount >= _pool.total
        });
    }
}
