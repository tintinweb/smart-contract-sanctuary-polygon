...
contract FinancialManager is Initializable, AccessControlUpgradeable {
    ...
    function depositBalance() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < tokens.length; i++) {
            ...
            if (tokenBalances[i] != 0) {
                ...
                IERC20Upgradeable(tokens[i]).safeApprove(address(strategy), depositAmounts[i]);
                IERC20Upgradeable(tokens[i]).safeTransfer(treasury, treasuryAmount);
            }
        }
    ...
    function yield(address _underlying) external onlyRole(MULTISIG_ROLE) {
        bool validUnderlying = false;
        for (uint256 i = 0; i < tokens.length; i++) {
            if (tokens[i] == _underlying) {
                validUnderlying = true;
                break;
            }
        }
        require(validUnderlying, "Invalid token");
        ...
        IERC20Upgradeable(_underlying).safeTransfer(multisigUser, balance);
    ...
    function supplyRewardsForPlayoffAndMegaLeague() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        IERC20Upgradeable(playoffRewardToken).safeTransfer(address(fantasyLeague), totalRewardPlayoff);
        ...
        IERC20Upgradeable(playoffRewardToken).safeTransfer(megaLeague, restOfBalance);
