...
// File: piggybees/extensions/with_limited_supply.sol
...
// File: piggybees/extensions/pseudo_randomly_assigned.sol
...
contract piggybs is ERC721Enumerable, RandomlyAssigned, Ownable {
    using Strings for uint256;

    uint256 public currentPrice = 10000000000000000; //Price in NATIVE CURRENCY
    uint256 public valueMultiple = 100;
    uint256 public constant maxSupply = 64;
    address public foundationPublicWallet;
    uint256 public foundationPercent = 20;
    string metadataBaseUri;
    string contractMetaData;

    mapping(uint256 => uint256) public initialDeposit;
    ...
    ) ERC721("Piggybees", "PGBS") RandomlyAssigned(maxSupply, 1) {
        metadataBaseUri = _metadataBaseUri;
        contractMetaData = _contractMetaData;
        foundationPublicWallet = foundationWallet;
    }

    function updateCurrentPrice(uint256 newPrice) public onlyOwner {
        currentPrice = newPrice;
    }

    function updateFoundationPercent(uint256 newPercent) public onlyOwner {
        foundationPercent = newPercent;
    }
    ...
    function totalSupply()
        public
        view
        override(WithLimitedSupply, ERC721Enumerable)
        returns (uint256)
    {
    ...
    function createNewPiggyb(uint256 unlockTimestamp)
        public
        payable
        ensureAvailabilityFor(1)
    {
        ...
        initialDeposit[tokenId] = msg.value;
        locked[tokenId] = msg.value - (currentPrice);
        unlockTime[tokenId] = unlockTimestamp;

        uint256 foundationTax = (currentPrice / 100) * foundationPercent;
        locked[tokenId] = currentPrice - foundationTax;
        payable(foundationPublicWallet).transfer(foundationTax);
    }

    function deposit(uint256 tokenId) public payable {
        require(msg.value > 0);

        funds[tokenId] += msg.value;
    }

    function burn(uint256 tokenId) public {
        require(msg.sender == ownerOf(tokenId));
        if (funds[tokenId] > 0) {
            if (unlockTime[tokenId] > block.timestamp) {
                payable(msg.sender).transfer(funds[tokenId]);
                payable(foundationPublicWallet).transfer(locked[tokenId]);
            } else {
                payable(msg.sender).transfer(funds[tokenId] + locked[tokenId]);
            }
            locked[tokenId] = 0;
            funds[tokenId] = 0;
        }
    ...
    {
        uint256 maxlevel = (initialDeposit[tokenId] * valueMultiple);
        uint256 steplevel = maxlevel / 10;
        uint256 nlevel = funds[tokenId] < maxlevel
            ? funds[tokenId] / steplevel
            : 10;
        return
            string(
                abi.encodePacked(
