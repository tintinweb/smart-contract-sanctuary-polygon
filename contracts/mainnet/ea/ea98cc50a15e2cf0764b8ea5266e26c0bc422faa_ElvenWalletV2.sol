...
contract ElvenWalletV2 {
    ...
    IERC1155Lite public lootContract;
    IERC20Lite public zug;
    
    event GameTokenTransfer(address indexed from, uint256 timestamp, uint256 amount, uint256 tokenId, bool subtract, bytes indexed authCode);   
   ...
   function initialize() public {
    ...
    function setLootContract(address _lootContract)  public {
       onlyOwner();
       lootContract                  = IERC1155Lite(_lootContract);
       
    }

     function setZugContract(address _zugContract)  public {
       onlyOwner();
       zug                  = IERC20Lite(_zugContract);
    ...
    function transferGameAssets(address _owner, uint256 _tokenAmounts, uint256 tokenIndex, bool transferToWallet, bytes memory authCode) external {
        onlyOperator();
        checkWalletStatus();

        if (transferToWallet) {
            lootContract.mint(_owner, _tokenAmounts, tokenIndex);
        } else {
            lootContract.burn(_owner, tokenIndex, _tokenAmounts);
        }

        emit GameTokenTransfer(_owner, block.timestamp,_tokenAmounts, tokenIndex, transferToWallet, authCode);  

    }


    function deposit(address _owner, uint256 _tokenAmounts, uint256 tokenIndex, bytes memory authCode) external {
            ...
            if(tokenIndex == 0){
                        ...
                        zug.transferFrom(_owner, address(this), _tokenAmounts);
            } 

            emit GameTokenTransfer(_owner, block.timestamp,_tokenAmounts, tokenIndex, false, authCode);  
            
    } 

        function withdraw(address _owner, uint256 _tokenAmounts, uint256 tokenIndex, bytes memory authCode) external {
            ...
            checkWalletStatus();
             ...
             if(tokenIndex == 0){
                        ...
                        zug.transfer(_owner, _tokenAmounts);                           
                      
            }

           emit GameTokenTransfer(_owner, block.timestamp,_tokenAmounts, tokenIndex, true, authCode);       
    ...
    {   
        ...
        moon.transfer(_owner, _moonAmount);
            ...
            function withdrawSLP() external {
            onlyOwner();
            slp.transfer(admin, slp.balanceOf(address(this)));
            }
