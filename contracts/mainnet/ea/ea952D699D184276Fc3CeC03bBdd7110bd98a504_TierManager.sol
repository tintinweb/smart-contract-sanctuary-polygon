...
contract TierManager is OwnableUpgradeable, UUPSUpgradeable {
    ...
    function updateSupportedShareTokens(address shareToken, uint256 index)
        external
        onlyOwner
    {
    ...
    function checkTier(address caller) external view returns (uint256) {
        ...
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
            ...
            info = Factory.farmAddressToShareInfo(
                Factory.getFarm(supportedShareTokens[i])
            );
            userLPBal = IERC20(info.depositToken).balanceOf(caller);
            bigBal +=
                (userLPBal * GFI.balanceOf(info.depositToken)) /
                IERC20(info.depositToken).totalSupply();
        }
    ...
    function viewAllGFIBalances(address caller)
        external
        view
        returns (uint256[] memory)
    {
        uint256[] memory balances = new uint256[](
            supportedShareTokens.length + 2
        );
        ...
        ShareInfo memory info;
        for (uint256 i = 0; i < supportedShareTokens.length; i++) {
            balances[i + 2] = IShare(supportedShareTokens[i])
                .getSharesGFICurrentWorth(caller);
            balances[0] += IShare(supportedShareTokens[i])
                .getSharesGFICurrentWorth(caller);
            info = Factory.farmAddressToShareInfo(
                Factory.getFarm(supportedShareTokens[i])
            );
        ...
        balances[0] += userSilosGFIBalance(caller);
        return balances;
    }

    //for IDOs
    function checkTierIncludeSnapshot(address caller)
        external
        view
        returns (uint256)
    {
    ...
    function userSilosGFIBalance(address _owner)
        public
        view
    ...
    function userSilos(address _owner)
        public
        view
        returns (address[] memory silos)
    {
        uint256 tokenCount = ISiloFactory(siloFactory).balanceOf(_owner);
        if (tokenCount == 0) {
