...
contract HTokenLens is HTokenLensI {
  ...
  function getFrontendMarketData(
    ...
    uint256 blocksPerDay = InterestRateModelI(interestRateModelAddress).blocksPerDay();
    uint256 blocksPerYear = InterestRateModelI(interestRateModelAddress).blocksPerYear();
    {
      // avoiding stack too deep
      HTokenI htoken = _hToken;
      uint256 supplyApy = (((pow(((supplyRate * blocksPerDay) / 1e18) + 1, 365))) - 1) * 100;
      uint256 borrowApy = (((pow(((borrowRate * blocksPerDay) / 1e18) + 1, 365))) - 1) * 100;

      return (
        supplyApy,
        borrowApy,
        (supplyRate * blocksPerYear * 100) / 1e18,
        (borrowRate * blocksPerYear * 100) / 1e18,
