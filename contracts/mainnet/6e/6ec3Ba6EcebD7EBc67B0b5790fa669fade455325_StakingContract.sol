    ...
    PausableUpgradeable
{
    ...
    // usdc token address
    address internal constant im_usdc =
        0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    // bal token address
    address internal constant im_bal =
        0x9a71012B13CA4d3D0Cdc72A177DF3ef03b0E76A3;
    // mai token address
    address internal constant im_mai =
        0xa3Fa99A148fA48D14Ed51d610c367C61876997F1;
    // dai token address
    address internal constant im_dai =
        0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063;
    // usdt token address
    address internal constant im_usdt =
        0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    // balancer address
    address internal constant im_balancer =
        0xBA12222222228d8Ba445958a75a0704d566BF2C8;
    // balancer merkle address
    address internal constant im_merkle =
        0x0F3e0c4218b7b0108a3643cFe9D3ec0d4F57c54e;
    // quickswap router address
    address internal constant im_qs =
    ...
    function setApprovalsInternal() internal {
        ...
        IERC20Upgradeable(im_dai).approve(im_balancer, MAX_INT);
        
        //approve balancer to take USDC
        IERC20Upgradeable(im_usdc).approve(im_balancer, MAX_INT);

        //approve balancer to take USDT
        IERC20Upgradeable(im_usdt).approve(im_balancer, MAX_INT);
    ...
    modifier onlyApproved() {
        require(
            msg.sender == s.owner || s.approvedUsers[msg.sender],
            "onlyApproved: not allowed"
        );
        _;
    }
    ...
    function updateCreator(address _creator) public {
        require(
            msg.sender == s.contractCreator,
            "Can only be called by creator"
        );
        s.contractCreator = _creator;
    }

    function joinPoolController(uint256 _dai) public onlyApproved {
        joinPool(_dai);
    }

    function joinPool(uint256 _dai) internal {
        ...
        //once we join the pool, must stake BPSP tokens on balancer gaguge to get rewards
        uint256 bpspBalance = IERC20Upgradeable(s.BPSP).balanceOf(
            address(this)
        );
        BalancerGauge(s.balancerGauge).deposit(bpspBalance);
    }

    function leavePoolExactAmount(uint256 _dai) internal {
        BalancerGauge(s.balancerGauge).withdraw(_dai);
        ...
        bytes memory userDataEncoded = abi.encode(
            2,
            amountsOut,
            IERC20Upgradeable(s.BPSP).balanceOf(address(this))
        );
