...
contract RidersFactory is VRFConsumerBaseV2, Ownable  
{
  ...
  function purchase(uint8 mintingPack, string memory referral) external payable {
    ...
    uint _DevMaticPart = devPerc * maticFeeDollars * uint(tokenQuantity) * (10 ** 18) * (10 ** 6) / (uint(priceOracleInstance.getLatestPrice()) * maticOverallPerc); // devPerc is 10 and maticOverallPerc is 15 initially
    ...
    bool sent = rdkToken.transferFrom(msg.sender, rewardPoolAddress, tokenPart * tokenRewardPoolPerc / tokenOverallPerc); // tokenRewardPoolPerc is 75 originally; tokenOverallPerc is 85 originally
