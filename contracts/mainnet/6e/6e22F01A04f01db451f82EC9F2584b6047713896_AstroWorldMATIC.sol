...
contract AstroWorldMATIC is ERC721, Ownable {
    ...
    //stores info if contract is paused or not
    bool private paused;

    //price to mint in usd with 8 decimals
    ...
    constructor() ERC721("Lifetime Pass", "LP") public {
        ...
        _currentBaseURI = "ipfs://QmPHy5nk1CoVWmE4WirXK5q1hUS8nm1Kka3ywWcqQaMDx1";
    }
    ...
    function getPriceMATIC() public view returns(uint256) {
          int price = getMATICtoUSD();
          return (mintPrice * 1000000000000000000) / uint256(price);
    }
    ...
    function mint() public payable {
        ...
        uint256 price = getPriceMATIC();
        require(price <= msg.value, "Insufficient funds to mint!");

        _tokenIds.increment();
        uint256 riderId = _tokenIds.current();
        _mint(msg.sender, riderId);

        _withdraw(payable(ownert), msg.value); 
    }

    //shows the amount of funds in the account
    ...
    //activate or deactivate pause
    function togglePause() public onlyOwner {
        paused = !paused;
    }

    //allow owner to change the price to mint
    function changeMintPrice(uint256 newPrice) public onlyOwner {
        mintPrice = newPrice;
    }

    //change the reciever of the funds
    function changeOwnert(address newAddress) public onlyOwner {
        require(newAddress != address(0), "Cannot be Null address!");
        ownert = newAddress;
    }
