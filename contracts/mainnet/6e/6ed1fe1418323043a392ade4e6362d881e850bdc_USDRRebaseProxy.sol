...
contract USDRRebaseProxy is AddressAccessor {
    bytes32 public constant POOL_MANAGER_ROLE = bytes32(keccak256("POOL_MANAGER"));
    ...
    constructor(address _poolManager) {
        ...
        _setRoleAdmin(POOL_MANAGER_ROLE, POOL_MANAGER_ROLE);
    }
        ...
        returns (uint256 amount)
    {
        ...
        for (uint256 i = pools_.length; i > 0; ) {
            ...
            _tryAutoBribe(_voter, IPair(pools_[i]), usdr);
        }
    }
    ...
    function _autoBribeOrReinvest(
        ...
        if (amount > 0) {
            if (tokenAddress == autoBribeTokenAddress) {
                token.safeApprove(bribeAddress, amount);
                IBribe(bribeAddress).notifyRewardAmount(tokenAddress, amount);
            } else {
