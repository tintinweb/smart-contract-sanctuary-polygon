...
contract Analytics is Ownable, ReentrancyGuard {
    ...
    function setFileBaseData(
        ...
        // string.concat(namespace, _clusterDns, nmtype);
        bytes memory strKey = bytes.concat(namespace, _clusterDns, nmtype);
        // bytes32(abi.encodePacked(strKey));
        clusterFileBaseInfo[bytes32(abi.encodePacked(strKey))].cid = cid;
        // clusterFileBaseInfo[namespace].cid = cid;
        // clusterFileBaseInfo[namespace].nmtype = nmtype;
        // clusterFileBaseInfo[namespace].cluster = _clusterDns;
    }

    // function getCid(
    //     bytes32[] memory _clusterDns,
    //     string memory namespace,
    //     string memory nmtype
