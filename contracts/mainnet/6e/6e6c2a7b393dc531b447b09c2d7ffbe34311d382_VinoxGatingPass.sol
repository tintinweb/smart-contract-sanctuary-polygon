 ...
 */
abstract contract GatingStorage {

    uint256 public constant MINT_START = 1666530000;
    event NFTWhiteListed(address indexed player);
...
import "../storage/GatingStorage.sol";
import "../../../tokens/ERC721ABase.sol";

error WalletNotWhiteListed();
error InvalidGatingPrice();
error InvalidPassQuantity();
error GatingWithdrawFailed();
error MintingNotStarted();
error MintingFinished();
error OnlyOneFreeMintAllowed();

contract VinoxGatingPass is ERC721ABase, UUPSUpgradeable, ERC2981, GatingStorage {

    constructor(string memory name_, string memory symbol_, string memory baseUrl_) {
        _disableInitializers();
    }

    function _authorizeUpgrade(address) internal override onlyRole(GOVERNOR_ROLE) {}

    function initialize(string memory _name, string memory _symbol, string memory _baseUrl, string memory _contractUri, address _owner) initializerERC721A initializer public {
        super._initialize(_name, _symbol, _baseUrl, _contractUri, _owner);
        metadataURI = _baseUrl;
    }

    function contractURI() public pure returns (string memory) {
        return "https://metadata.vitidiary.com/gating-metadata";
    }
    ...
    function checkWhitelist(address player) public view returns (bool) {
        return eapWhitelist[player];
    }

    function updateFreeMintingStatus(bool status) external {
        freeMintingClosed = status;
    }

    function batchCheckWhitelist(address[] memory players) public view returns (bool[] memory) {
        bool[] memory ret = new bool[](players.length);
        for (uint i = 0; i < players.length; ++i) {
            ret[i] = eapWhitelist[players[i]];
        }
        return ret;
    }

    function batchWhitelist(address[] memory players) external onlyOwner { 
        for (uint i = 0; i < players.length; ++i) {
            whitelist(players[i]);
        }
    }

    function openPublicSale(bool openOrClose) external onlyOwner {
        publicSaleOpened = openOrClose;
    }

    function whitelist(address player) public onlyOwner {
       eapWhitelist[player] = true;
    }

    function setDefaultRoyaltyInfo(address receiver, uint96 feeBasisPoints) external onlyOwner {
        _setDefaultRoyalty(receiver, feeBasisPoints);
    }

    function setTokenRoyaltyInfo(uint256 tokenId, address receiver, uint96 feeBasisPoints) external onlyOwner {
    ...
    /**
     * @dev Mints Gating Pass for the given wallet address and transfer to the sender
     *
     * Requirements:
     *
    ...
    function mint(uint256 quantity) external payable {
        // we only issue 1855 gating passes.
        if (totalSupply() + quantity > 1855) {
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721ABase, ERC2981) returns (bool) {
