...
contract RentalProtocol is IRentalProtocol, AccessControl, ERC721Holder, ReentrancyGuard, EIP712 {
    ...
    function rent(
        ...
        for (uint256 i = 0; i < offer.nfts.length; i++) {
            ...
            uint64 basisPoints = nft.basisPoints;
        ...
        return
            _hashTypedDataV4(
                keccak256(
                    abi.encode(
                        keccak256(
                            "RentalOffer(address maker,address taker,NFT[] nfts,address feeToken,uint256 feeAmount,uint256 nonce,uint256 deadline)NFT(address token,uint256 tokenId,uint64 duration,uint64 basisPoints)"
                        ),
                        offer.maker,
                        offer.taker,
                        nftsHash,
    ...
    function hashNFTs(NFT[] memory nfts) internal pure returns (bytes32 hash) {
        ...
        for (uint256 i = 0; i < arrayLength; i++) {
            hashArray[i] = keccak256(
                abi.encode(
                    keccak256("NFT(address token,uint256 tokenId,uint64 duration,uint64 basisPoints)"),
                    nfts[i].token,
                    nfts[i].tokenId,
                    nfts[i].duration,
                    nfts[i].basisPoints
