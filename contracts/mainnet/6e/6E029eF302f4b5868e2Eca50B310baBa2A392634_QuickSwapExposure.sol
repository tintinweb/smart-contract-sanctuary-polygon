...
contract QuickSwapExposure is TroveHelpers, ILeverageStrategy, CurveHelpers {
  ...
  ) CurveHelpers(_curveRouter, _clp, _cpool) {
  ...
  function test(
    ...
    // deposit arth.usd
    // arth.approve(address(arthUsd), amount);
    // arthUsd.deposit(amount);
...
contract CurveHelpers {
  ...
  function _sellARTHusdForExact(
    ...
    arthUsd.approve(address(curveRouter), arth.balanceOf(address(this)));

    // curveRouter.exchange_underlying(pool, 2, 0, amountInMax.div(2), amountUSDCOut);
    // curveRouter.exchange_underlying(pool, 0, 3, arthUsd.balanceOf(address(this)), amountUSDCOut);
