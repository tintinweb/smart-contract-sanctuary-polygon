...
contract Factory is IFactory {
    ...
    ILif3PriceOracle public lif3PriceOracle;
    ...
    constructor(
        ...
        ILif3PriceOracle _lif3PriceOracle
    ) public {
        admin = _admin;
        reservesAdmin = _reservesAdmin;
        bDeployer = _bDeployer;
        cDeployer = _cDeployer;
        lif3PriceOracle = _lif3PriceOracle;
    ...
    function initializeLendingPool(address uniswapV2Pair) external {
        ...
        require(!lPool.initialized, "LIF3: ALREADY_INITIALIZED");

        require(
            lPool.collateral != address(0),
            "LIF3: COLLATERALIZABLE_NOT_CREATED"
        );
        require(
            lPool.borrowable0 != address(0),
            "LIF3: BORROWABLE0_NOT_CREATED"
        );
        require(
            lPool.borrowable1 != address(0),
            "LIF3: BORROWABLE1_NOT_CREATED"
        );

        (, , , , , bool oracleInitialized) =
            lif3PriceOracle.getPair(uniswapV2Pair);
        if (!oracleInitialized) lif3PriceOracle.initialize(uniswapV2Pair);
    ...
    function _setPendingAdmin(address newPendingAdmin) external {
        require(msg.sender == admin, "LIF3: UNAUTHORIZED");
    ...
    function _acceptAdmin() external {
        require(msg.sender == pendingAdmin, "LIF3: UNAUTHORIZED");
    ...
    function _setReservesPendingAdmin(address newReservesPendingAdmin)
        external
    {
        require(msg.sender == reservesAdmin, "LIF3: UNAUTHORIZED");
    ...
    function _acceptReservesAdmin() external {
        require(msg.sender == reservesPendingAdmin, "LIF3: UNAUTHORIZED");
