...
contract ZapAnalyzer {
    ...
    struct LiquidityPath {
        ...
        int24 tickLower;
        int24 tickUpper;
        address wrapperHelper; // Required if lpType == (Arrakis || Gamma)
    }
        ...
        returns (SwapReturns memory returnValues)
    {
        ...
        if (params.liquidityPath.liquidityType == LiquidityType.V2) {
            ...
            vars.arrakisPool = MathHelper.getArrakisPool(
                vars.uniV3Pool,
                IArrakisFactoryV1(params.liquidityPath.wrapperHelper)
            );
            MathHelper.SwapRatioParams memory swapRatioParams = MathHelper.SwapRatioParams({
                inputToken: vars.inputToken,
                inputAmount: params.inputAmount,
            ...
            MathHelper.SwapRatioParams memory swapRatioParams = MathHelper.SwapRatioParams({
                ...
                gammaUniProxy: params.liquidityPath.lpRouter,
                gammaHypervisor: params.liquidityPath.wrapperHelper
            });
            (returnValues.swapToToken0, returnValues.swapToToken1) = MathHelper.getSwapRatio(swapRatioParams);
        }
...
library MathHelper {
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        (vars.underlying0, vars.underlying1) = getLPAddRatio(
            ...
            swapRatioParams.gammaUniProxy,
            swapRatioParams.gammaHypervisor
        );

        if (swapRatioParams.uniV3Factory != address(0)) {
    ...
    function getLPAddRatio(
        ...
        } else if (gammaUniProxy != address(0)) {
            (uint256 amountStart, uint256 amountEnd) = UniProxy(gammaUniProxy).getDepositAmount(
                gammaHypervisor,
                token0,
                10**ERC20(token0).decimals()
            );
            amount0 = 10**ERC20(token0).decimals();
