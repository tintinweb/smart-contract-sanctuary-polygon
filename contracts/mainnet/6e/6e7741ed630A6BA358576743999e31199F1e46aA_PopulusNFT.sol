...
contract PopulusNFT is ERC4973, Ownable {
    ...
    constructor() ERC4973("PopulusNFT", "POPULUS", "0.0.1") {}

    function setBaseURI(string memory customBaseURI_) public onlyOwner {
        _customBaseURI = customBaseURI_;
    }

    function setURI(uint256 tokenId, string memory customURI_)
        public
        onlyOwner
    {
    ...
    function setGovernanceValue(uint256 tokenId, uint16 governanceIncrement)
        public
        onlyOwner
    {
        _setGovernanceValue(tokenId, governanceIncrement);
    }
    ...
    // }
    function viewGovernance(uint256 tokenId)
        internal
        view
        virtual
        returns (uint16)
    {
        return governanceValue(tokenId);
    }

    function viewTotalGovernance()
        internal
        view
        virtual
        returns (uint256)
...
contract ERC4973 is EIP712, ERC165, IERC721Metadata, IERC4973 {
    ...
    mapping(uint256 => uint16) private _governanceValues;
    ...
    function governanceValue(uint256 tokenId)
        public
        view
        virtual
        returns (uint16)
    {
        require(_exists(tokenId), "tokenURI: token doesn't exist");
        return _governanceValues[tokenId];
    }
    ...
    function _setGovernanceValue(uint256 tokenId, uint16 governanceIncrement)
        internal
        virtual
    {
        require(_exists(tokenId), "Governance set: nonexistent token");
        _totalGovernance = _totalGovernance + governanceIncrement;

        _governanceValues[tokenId] =
            _governanceValues[tokenId] +
            governanceIncrement;
    }
    ...
    function _burn(uint256 tokenId) internal virtual {
        ...
        _balances[owner] -= 1;
        _totalGovernance -= _governanceValues[tokenId];
        delete _owners[tokenId];
        delete _tokenURIs[tokenId];
        delete _governanceValues[tokenId];
