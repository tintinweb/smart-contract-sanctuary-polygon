...
contract Vote {
  ...
  address payable[] likeVoters;
  address payable[] voters;

  string alreadyVotedError = "You already voted";
  string notEnoughTokensError = "Need 1 CHIN tokens to vote";
  ...
  function hasTokens(address voter) private view returns (bool) {
    return _token.balanceOf(voter) >= 1 * 10**18;
  }
  ...
  function getLikePercentage() public view returns (uint256) {
    return (likeVoters.length * 100) / getNumVoters();
  }
}