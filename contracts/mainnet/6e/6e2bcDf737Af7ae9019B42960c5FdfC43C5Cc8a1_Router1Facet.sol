...
library LibGmx {
    ...
    function swapGmx(Hop memory h) internal {
        h.path[0].approve(h.addr, h.amountIn);
        IRouter(h.addr).swap(h.path, h.amountIn, 0, address(this));
    }
}
...
library LibTrident {
    ...
    function getTridentPath(address[] memory path, bytes[] memory poolDataList)
        private
        view
        returns (ITridentRouter.Path[] memory tridentPath)
    {
        uint256 pl = path.length;
        uint256 pdl = poolDataList.length;
        ...
        for (uint256 i = 0; i < pdl; ) {
            ...
            tridentPath[i].data = encodeTridentData(path[i], i == pdl - 1 ? address(this) : tridentPath[i].pool, false);
        ...
        if (h.path.length == 2) {
            ITridentRouter(h.addr).exactInputSingle(
                ITridentRouter.ExactInputSingleParams({
                    ...
                    tokenIn: h.path[0],
                    data: encodeTridentData(h.path[0], address(this), false)
                })
            );
        } else {
            ITridentRouter(h.addr).exactInput(
                ITridentRouter.ExactInputParams({
                    ...
                    amountIn: h.amountIn,
                    amountOutMinimum: 0,
                    path: getTridentPath(h.path, h.poolDataList)
                })
            );
        }
    }
...
library LibWoofi {
    ...
    function swapWoofi(Hop memory h) internal {
        h.path[0].approve(h.addr, h.amountIn);
        IWooPPV2(h.addr).swap(h.path[0], h.path[1], h.amountIn, 0, address(this), msg.sender);
    }
}
