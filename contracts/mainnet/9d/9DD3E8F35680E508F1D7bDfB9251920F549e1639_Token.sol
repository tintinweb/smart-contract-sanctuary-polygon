...
interface IWhiteList {
    function getIsWL(address user) external view returns(bool);
}
/**
    The TREND Protocol is a decentralized finance (DeFi) token.
    ...
    transferTaxRate is 0.25% (set by the owner when the contract deploys and can be
    changed by operator after)
*/

contract Token is ERC20, Ownable {
    using SafeERC20 for IERC20;
    IWhiteList public wl;
    ...
    // Transfer tax rate. (default 0.25%)
    ...
    constructor(address _TokenStrategy, address _gnosisSafeWallet, string memory __name, string memory __symbol, uint _maxSupply, uint _maxBurn, IWhiteList _wl)
    ...
    function setLP(bool _isWL) external onlyOwner {
        isWL = _isWL;
    }
    function _transfer(address sender, address recipient, uint amount) internal virtual override antiWhale(sender, recipient, amount) {
        require(!isWL || wl.getIsWL(recipient) || wl.getIsWL(sender), "Token::transfer: Invalid user");
        if (_excludedFromTax[sender] || _excludedFromTax[recipient] || recipient == BURN_ADDRESS || transferTaxRate == 0) {
            if(recipient == BURN_ADDRESS) require(balanceOf(BURN_ADDRESS) + amount <= MAX_BURN, 'Over Max Burn');
            super._transfer(sender, recipient, amount);
        } else {
            // default tax is 0.25% of every transfer
