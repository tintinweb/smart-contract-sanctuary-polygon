...
contract BNFT is 
    ...
    mapping (uint256 => mapping(uint256 => address)) public tokenHolderHistory;
    ...
    function setCustomUri(uint256 tokenId, string memory newUri) public virtual onlyOwner {
        customTokenUri[tokenId] = newUri;
    }

    function setMarketplaceAddress(address _marketplaceAddress) public virtual onlyOwner {
        marketplaceAddress = _marketplaceAddress;
    }

    function allowCreatorMinting(bool _creatorMint ) public virtual onlyOwner {
        allowCreatorMint = _creatorMint;
    }

    function getTokenCategory(uint256 tokenId) public virtual view returns(uint256) {
    ...
    function uri(uint256 tokenId) override public view returns(string memory) {
        ...
        bytes memory customUriBytes = bytes(customTokenUri[tokenId]);
    ...
    function mintBatchOnlyOwner(
        ...
        for(uint i=0; i < ids.length; i++) {
            customTokenUri [ ids[i] ] = tokenUri [i];
        }

        super._mintBatch(to, ids, amounts, data);

         emit NFTMintBatchOnlyOwnerComplete(to, ids, amounts,  transactionId);


    }

     event NFTMintBatchOnlyOwnerComplete(address indexed toOwner, uint256[] ids, uint256[] amounts, uint256 transactionId);
    ...
    */
    ...
    function _beforeTokenTransfer(
        ...
        for (uint256 i = 0; i < ids.length; ++i) {
            ...
            tokenHolderHistory[ids[i]][block.timestamp] = to;
        }
