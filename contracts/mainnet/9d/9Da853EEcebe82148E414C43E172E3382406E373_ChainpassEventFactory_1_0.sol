...
import "./interfaces/IChainpassSplits.sol";
...
/*
 *  ChainpassEventFactory V1.0
 *
 *  A factory contract for launching events within the ChainpassProtocol.  Events
 *  are a combination of an ERC-721 ChainpassTickets contract and a revenue sharing
 *  ChainpassSplits contract.  Events can only be launched within the Chainpass
 *  platform and so the createEvent() method can only be called by the Chainpass wallet.
 */
contract ChainpassEventFactory_1_0 is MetaTxMixin, ReentrancyGuard {
    event EventCreated(address eventHost, address ticketContractAddress, address splitsContractAddress);
    ...
    /*
     *  Creating and fetching events.
     */
    mapping(address => bool) public eventInfoByTicketContractAddress;
    address[] public eventTicketContractAddresses;

    function createEvent(address eventHost) external nonReentrant {
        // Only the Chainpass wallet can create events because we need to enforce that all events
        // are launched within the Chainpass platform.
        require(_msgSender() == chainpassWallet(), "Caller must be Chainpass Wallet.");

        // Ship splits contract.
        address splitsContractAddress = address(new ChainpassProxy(
            registry().getSplitsBaseContractAddress(),
            abi.encodeWithSignature("initialize()")
        ));

        // Determine the revenue splits initially using the default fee.
        uint32 defaultFee = registry().getChainpassDefaultFee();

        address[] memory allocatedAddress = new address[](2);
        allocatedAddress[0] = chainpassWallet();
        allocatedAddress[1] = eventHost;

        uint32[] memory allocatedShares = new uint32[](2);
        allocatedShares[0] = defaultFee;
        allocatedShares[1] = 10_000 - defaultFee;

        // Set the allocations on the splits contract.
        IChainpassSplits(splitsContractAddress).setAllocations(
            allocatedAddress,
            allocatedShares
        );

        // Transfer ownership of the splits contract to the Chainpass wallet.
        IOwnable(splitsContractAddress).transferOwnership(chainpassWallet());

        // Ship ChainpassTickets ERC-721 contract and also transfer ownership to Chainpass.
        address chainpassTickets = address(new ChainpassProxy(
            registry().getTicketsBaseContractAddress(),
            abi.encodeWithSignature("initialize(address,address)", chainpassRegistry, splitsContractAddress)
        ));
        IOwnable(chainpassTickets).transferOwnership(chainpassWallet());

        // Save the event information.
        eventInfoByTicketContractAddress[chainpassTickets] = true;
        eventTicketContractAddresses.push(chainpassTickets);

        emit EventCreated(eventHost, chainpassTickets, splitsContractAddress);
    }
...
interface IChainpassSplits {
    function setAllocations(address[] calldata addrs, uint32[] calldata shares) external;
}
 ...
 *  ChainpassProxy
 *
 *  A simple proxy implementation used within the Chainpass Protocol.
 */
