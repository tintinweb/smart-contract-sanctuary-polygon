...
contract MaticStaker {
	...
	uint256 constant public WITHDRAW_MIN_AMOUNT = 10 ether; // 10 MATIC
	...
	uint256 constant public WITHDRAW_STEP = TIME_STEP / 6;
	...
	constructor(address payable pWallet) {
		require(!isContract(pWallet));
		projectWallet = pWallet;
		startUNIX = 1649628000; //Sunday, April 10, 2022 10:00:00 PM GMT+0000
	...
	function invest(address referrer, uint8 plan) public payable {
		...
		require(msg.value >= INVEST_MIN_AMOUNT, "the min amount is 5 matic");
	...
	function reinvest(uint256 amount) internal {
		require(block.timestamp > startUNIX, "not luanched yet");
		require(amount >= INVEST_MIN_AMOUNT, "the min amount is 5 matic");
	...
	function withdraw() public {
		require(block.timestamp > startUNIX, "not luanched yet");
		User storage user = users[msg.sender];
		require( (user.checkpoint + WITHDRAW_STEP) < block.timestamp ,"only each 4 hours" );
		(uint256 totalAmountU, uint256 totalAmountL) = getUserDividends(msg.sender);

		require((totalAmountL + totalAmountU) >= WITHDRAW_MIN_AMOUNT, "min withdraw is 10 MATIC");
...
/* © 2022 by MaticStaker. All rights reserved. */