...
contract PolyflipLottery is ReentrancyGuard, Random {
    ...
    event LotteryCreated(uint indexed lotteryId, string title, uint endSale, uint indexed maxTicketsSellable, Price[] prices, uint[] prizes, uint startSale, address prizesToken);
    ...
    function createLottery(string memory _title, Price[] memory _prices, uint _maxTicketsSellable, uint[] memory _prizes, address _prizesToken, uint _endsale, uint _startSale) external onlyOwner {
        ...
        require(overallPrize <= balanceAvailable(_prizesToken), "Not enough funds in contract");
        lockedBalance[_prizesToken] += overallPrize;

        emit LotteryCreated(lotteryId, _title, _endsale, _maxTicketsSellable, _prices, _prizes, _startSale, _prizesToken);
    }
    ...
    function balanceAvailable(address token) view public returns (uint){
        if (token == address(0)) {
            return address(this).balance - lockedBalance[token];
        }
        else {
            return IERC20(token).balanceOf(address(this)) - lockedBalance[token];
        }        
    }
