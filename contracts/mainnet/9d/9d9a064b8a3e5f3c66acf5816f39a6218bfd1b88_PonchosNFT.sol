...
import {BaseERC1155} from "../BaseOverrides/BaseERC1155.sol";
import {MarketLibV2} from "../libs/MarketLibV2.sol";
import {QurableRegistryManagedV2} from "../v2/QurableRegistryManagedV2.sol";
import {IERC1155Minter} from "../IERC1155Minter.sol";
import {IHasVersion} from "../IHasVersion.sol";

import "hardhat/console.sol";

/// @custom:security-contact [email protected]
contract PonchosNFT is
    Initializable,
    BaseERC1155,
    ...
    IHasVersion
{
    ...
    mapping(uint256 => uint256) private _tokenIdCounterByCollection;
    mapping(uint256 => string) private _tokenURIs;
    mapping(uint256 => uint256) private _baseTokenURIsIndex;
    mapping(uint256 => uint256) private isFrozenTokenURI;
    mapping(uint256 => uint256) private _burnedTokenCounterByCollection;
    ...
    function uri(uint256 tokenId) public view override returns (string memory) {
        require(exists(tokenId), "InvalidTokenId");

        return _uriUnsafe(tokenId);
    }

    function exists(uint256 tokenId) public view returns (bool) {
        uint256 collectionId = tokenId -
            (tokenId % MarketLibV2.MAX_ALLOWED_MINTING_COUNT);
        uint256 indexInCollection = tokenId %
            MarketLibV2.MAX_ALLOWED_MINTING_COUNT;

        return _tokenIdCounterByCollection[collectionId] > indexInCollection;
    }
    ...
    function totalSupplyByCollection(uint256 collectionId)
        public
        view
        returns (uint256)
    {
        return
            _tokenIdCounterByCollection[collectionId] -
            _burnedTokenCounterByCollection[collectionId];
    }

    function freezeAllTokenURIs(uint256 collectionId)
        external
        onlyOperatorOrOwner
    {
        ...
        for (
            uint256 index = 0;
            index < _tokenIdCounterByCollection[collectionId];
            index++
        ) {
            uint256 tokenId = collectionId + index;
            isFrozenTokenURI[tokenId] = 1;
            emit PermanentURI(_uriUnsafe(tokenId), tokenId);
        }
    }
    ...
    function setTokenURIs(
        ...
        for (uint256 index = 0; index < tokenIds_.length; index++) {
            ...
            string memory tokenURI = tokenURIs_[index];

            require(exists(tokenId), "InvalidTokenId");
            require(bytes(tokenURI).length > 0, "InvalidTokenURI");
            require(isFrozenTokenURI[tokenId] == 0, "TokenURIFreezed");
    ...
    function operatorBurnBatch(
        ...
        require(
            _tokenIdCounterByCollection[collectionId] -
                _burnedTokenCounterByCollection[collectionId] >=
                ids.length,
            "InvalidAmountToBurnInCollection"
        );

        _burnedTokenCounterByCollection[collectionId] = ids.length;
    ...
    function mint(
        ...
        bool freezeTokenURI
    ) external override onlyOperatorOrOwner whenNotPaused returns (uint256) {
        require(to != address(0), "InvalidTo");
        require(bytes(tokenURI).length > 0, "InvalidTokenURI");
        require(
            collectionId >= MarketLibV2.MAX_ALLOWED_MINTING_COUNT,
            "InvalidTokenId"
        );
        require(
            _tokenIdCounterByCollection[collectionId] <
        ...
        uint256 newTokenId = collectionId +
            _tokenIdCounterByCollection[collectionId];

        _tokenURIs[newTokenId] = tokenURI;

        _tokenIdCounterByCollection[collectionId] += 1;

        _mint(from, to, newTokenId, 1, "");

        if (freezeTokenURI) {
            isFrozenTokenURI[newTokenId] = 1;
    ...
    function mintMultiple(
        ...
        require(
            _tokenIdCounterByCollection[collectionId] + amount <
                MarketLibV2.MAX_ALLOWED_MINTING_COUNT,
            "MaxMintingReached"
        );
        require(
        ...
        uint256[] memory amountsById = new uint256[](amount);
        uint256 startTokenIdCounter = _tokenIdCounterByCollection[collectionId];
        _tokenIdCounterByCollection[collectionId] += amount;
        ...
        for (uint256 index = 0; index < ids.length; index++) {
            ...
            ids[index] = newTokenId;
            amountsById[index] = 1;

            _baseTokenURIsIndex[newTokenId] = baseTokenURIIndex;
            _balances[newTokenId] = to;
        }

        address operator = _msgSender();
        emit TransferBatch(operator, from, to, ids, amountsById);
    ...
    function _uriUnsafe(uint256 tokenId) internal view returns (string memory) {
    ...
    IERC1155MetadataURIUpgradeable
{
    ...
    function _burn(
        ...
        require(_balances[id] == from, "ERC1155: burn amount exceeds balance");
