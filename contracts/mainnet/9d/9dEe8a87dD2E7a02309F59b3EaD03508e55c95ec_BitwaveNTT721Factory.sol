...
/// @return newContractAddress The address of the deployed contract.
    function deployNewCert(string memory _name, string memory _symbol) public returns (address) {
        BitwaveNTT721 newBitwaveCert = new BitwaveNTT721(
            _name,
            _symbol);
...
contract BitwaveNTT721 is ERC721, Ownable {
    ...
    function bulkMint(address[] memory to, string[] memory mintURI) public onlyOwner {
        for (uint i = 0; i < to.length; i++) {
            uint256 tokenId = currentTokenId();
        _mint(to[i], tokenId);
        _tokenURI[tokenId] = mintURI[i];
        }
    }

    /// @notice Overrides the ERC721 transferFrom function to add NTT functionality.
    /// @notice NTT transfers are only possible via the owner of the contract.
    ...
    function mint(address to, string memory mintURI) public onlyOwner {
        uint256 tokenId = currentTokenId();
        _mint(to, tokenId);
        _tokenURI[tokenId] = mintURI;
    }
