...
contract GovernorContractAggregate is GovernorContractProposal,Initializable
{
    /// @notice Delay before voting shoud start.
    /// @dev Delay before voting shoud start.
    GovernorContract governance;
    ...
    // @notice Initializer.
    /// @dev Initializer.
    /// @param _governance Governace Contract Address.
    function initialize(
        GovernorContract _governance
        )
        initializer public
    {
    ...
    /// @param groupNameBytes group for Vote on proposal.
    /// @param stateCode Proposal State.
    /// @param votesFor - Number of tokens voted  "for".
    /// @param votesAgainst - Number of tokens voted  "agains".
    /// @param votesQuorum - Number of tokens needed to quorum. It is sum of voting power, beforere vote is stated, form all accounts * 0.5.
    /// @param userHasVoted - If user voted for this proposal.
    /// @param snapshotBlockNumber - Block Start for voting.
    /// @param deadlineBlockNumber - Block End for voting.
    /// @param userVotingPower - Voting power of account on block, when voting is started.
    struct ProposalAggregate {
        uint256 id;
        string hash;
        uint8 typ;
        address owner;
        bytes32 groupNameBytes;
        GovernorUpgradeable.ProposalState stateCode;
        uint256 votesFor;
   ...
   /// @notice Structure of single proposal pagination with aggregate data.
    /// @dev Structure of single proposal pagination with aggregate data.
    struct ProposalAggregatePagination {
        uint256 total;
        uint256 per_page;
        uint256 last_page;
        uint256 current_page;
        ProposalAggregate[] data;
    }
    ...
    returns (ProposalAggregate memory) {
        uint256 proposalsIndex = governance.getProposalIndex(proposalID);
        (string memory hash,uint8 typ,address owner,bytes32 groups) = governance.getSingleProposal(proposalID);
        (uint256 votes_for,uint256 votes_against,) = governance.proposalVotes(proposalID);
        return ProposalAggregate(
            ...
            votes_for,
            votes_against,
            governance.quorumPerGroup(proposalID),
            governance.hasVoted(proposalID,msg.sender),
            governance.proposalSnapshot(proposalID),
            governance.proposalDeadline(proposalID),
            governance.getVotes(msg.sender,governance.proposalSnapshot(proposalID))
            );
    ...
    view
    returns (ProposalAggregate[] memory) {
        Proposal[] memory proposals  = governance.getAllProposals();
        ProposalAggregate [] memory  proposal_aggregate = new ProposalAggregate[](proposals.length);

        for(uint256 i=0;i<proposals.length;i++){
            proposal_aggregate[i]  = getSingleProposalAggregate(proposals[i].proposalID);
        }
        return proposal_aggregate;
    }
    ...
    /// @notice Structure of single proposal pagination with aggregate data.
    /// @dev Structure of single proposal pagination with aggregate data.
    struct ProposalAggregatePagination2 {
        ProposalAggregate[] data;
    }

    /// @notice Get Aggregate Info for all Proposal.
    /// @dev Get Aggregate Info for all Proposal.
    /// @param first Per Page.
    /// @param offset Offset.
    /// @return ProposalAggregatePagination Data pagination: 
    // @return total Total of items without paginate.
    // @return per_page First parameter value.
    // @return last_page Total_items / first parameter.
    // @return current_page Currnet Page. 
    // @return data List of Agregated data:
    // @param id Proposal ID.
    // @param hash IPFS CID of Proposal.
    // @param typ Proposal Type: 0 - blog, 1 - integration.
    // @param owner Accout sended Proposal.
    ...
    // @param deadlineBlockNumber - Block End for voting.
    // @param userVotingPower - Voting power of account on block, when voting is started.
    function getAllProposalAggregatePaginate(uint256 first, uint256 offset)
    public
    view
    returns (ProposalAggregatePagination memory) {
        require(first>0, "Wrong argument: first.");
        uint256 total_internal= governance.getProposalsLength();
        Proposal[] memory proposals  = governance.getAllProposals();
        

        if (total_internal == 0){
            ProposalAggregate [] memory  proposal_aggregate_empty = new ProposalAggregate[](0);
            return ProposalAggregatePagination(0,0,0,0, proposal_aggregate_empty);
        }


        uint256 last_page_internal=0;
        if(total_internal != (total_internal*10**2)/(first*10**2)*first)
        {
            last_page_internal = (total_internal*10**2)/(first*10**2)+1;
        }
        else{
            last_page_internal = (total_internal*10**2)/(first*10**2);
        }

        uint256 current_page_internal=0;

        if(offset == 0)
        {
            current_page_internal = 1;
        }
        else{
            current_page_internal = (offset*10**2)/(first*10**2)+1;
        }

        
        uint256 size = 0;
        if(total_internal > current_page_internal*first+offset)
        {
            size = first;
        }
        else
        ...
        for(uint256 i=0;i<size;i++){
            proposal_aggregate[i]  = getSingleProposalAggregate(proposals[i+total_internal-(total_internal-offset)].proposalID);
            //proposal_aggregate[i]  = getSingleProposalAggregate(proposals[1].proposalID);
        }
        //pagination.data = proposal_aggregate;
        //ProposalAggregatePagination memory pagination = ProposalAggregatePagination(proposal_aggregate,total_internal);
        return (ProposalAggregatePagination(total_internal,first,last_page_internal,current_page_internal,proposal_aggregate));
    }
    /*
    ...
    // @return current_page Currnet Page. 
    // @return data List of Agregated data.
    function getAllProposalAggregatePaginate2(uint256 first, uint256 offset)
    public
    view
    returns (uint256,uint256,uint256,uint256, ProposalAggregatePagination2 memory) {
        require(first>0, "Wrong first.");
        uint256 total_internal= proposals.length;
        

        if (total_internal == 0){
            pagination.data = new ProposalAggregate[](0);
            return (0,0,0,[] );
        }
        ...
        ProposalAggregate [] memory  proposal_aggregate;
        uint256 size = 0;
        if(total_internal > size)
        {
            size = current_page_internal*first;
        }
        else
        {
            size = total_internal+first-current_page_internal*first;
        }
        proposal_aggregate = new ProposalAggregate[](size);

        //ProposalAggregate [] memory  proposal_aggregate = new ProposalAggregate[](total-(offset-1)*per_page);
       

        for(uint256 i=0;i<size;i++){
            proposal_aggregate[i]  = getSingleProposalAggregate(proposals[i+offset+first].proposalID);
        }
        //pagination.data = proposal_aggregate;
        //ProposalAggregatePagination memory pagination = ProposalAggregatePagination(proposal_aggregate,total_internal);
        return (total_internal,first,last_page_internal,current_page_internal,ProposalAggregatePagination2(proposal_aggregate));
    }
