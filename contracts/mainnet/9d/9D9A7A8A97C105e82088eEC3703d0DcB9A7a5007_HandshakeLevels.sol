...
contract HandshakeLevels is Ownable {
    ...
    function getDexGoNFT() public view returns (address) {
        return dexGoNFT;
    }
    function setDexGoNFT(address _dexGoNFT) public onlyOwner {
        dexGoNFT = _dexGoNFT;
    }
    ...
    constructor(address _dexGoNFT) {
        dexGoNFT = _dexGoNFT;
        percentPerLevelWei[0] = 0.08 ether;
        percentPerLevelWei[1] = 0.07 ether;
        percentPerLevelWei[2] = 0.06 ether;
        percentPerLevelWei[3] = 0.05 ether;
        percentPerLevelWei[4] = 0.05 ether;
        percentPerLevelWei[5] = 0.04 ether;
    }
    ...
    function setHandshake(address wallet, address referrer) public returns (uint, uint, bool, uint) {
        require(msg.sender == dexGoNFT || msg.sender == owner() || msg.sender == dexGo,'HandshakeLevels: unauthorized sender');
