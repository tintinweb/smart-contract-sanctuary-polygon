  ...
  */
   ...
   */
    ...
    function getLimits() external override view returns(uint128 dailyLimit, uint128 globalLimit,uint128 userDailyLimit, uint128 userGlobalLimit){
        dailyLimit = addressBook.getDailyLimit();
        globalLimit = addressBook.getGlobalLimit();
        userDailyLimit = addressBook.getUserDailyLimit();
        userGlobalLimit = addressBook.getUserGlobalLimit();
    }

    function getUserTotalOustanding(uint _nfcsId) public override view returns(uint){
        address[] memory investors = addressBook.addressList(investorsAddressLookupCategory);
        address[] storage usersAddresses = _tokenBundle[_nfcsId];
            ...
            for(uint j = 0; j < usersAddresses.length; j++){
                ...
                oustandingBalance += 
                    IERC20PaymentStandard(investors[i])
                        .getNFCSTotalOustanding(_nfcsId);
            }
        }
    ...
    function getGlobalTotalOustanding() public override view returns(uint){
        address[] memory investors = addressBook.addressList(investorsAddressLookupCategory);
        uint oustandingBalance = 0;
        // for each investor
        for(uint i = 0; i < investors.length; i++){
                //update outstanding global balance  at that investor
                oustandingBalance += 
                    IERC20PaymentStandard(investors[i])
                        .getTotalOustanding();
