...
if (block.timestamp > inicio[msg.sender] + upT)    {return 1;}      }
    ...
    function sync(Event[] memory _events) public hasFarm returns (Farm memory) {
        ...
        uint balance = token.balanceOf(msg.sender)-15;
    ...
    function receiveReward() private hasFarm {
    ...
    function stake(address resourceAddress, uint amount) private {
    ...
    function createRecipe(address tokenAddress, Cost[] memory costs) private {
    ...
    function createResource(address resourceAddress, address requires) private {
    ...
    function craft(address recipeAddress, uint amount) private {
    ...
    function mintNFT(address recipeAddress, uint tokenId) private {
        Material memory material = materials[recipeAddress];
    ...
    function getRecipe(address recipeAddress) private view returns (Recipe memory recipe) {
        return recipes[recipeAddress];
    }

    function getResource(address resourceAddress) private view returns (Resource memory resource) {
