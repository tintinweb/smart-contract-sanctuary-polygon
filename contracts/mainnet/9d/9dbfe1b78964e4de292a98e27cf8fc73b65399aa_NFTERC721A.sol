...
contract NFTERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint public SalePrice = 25 ether;

    address public addressOW = 0x0A24FCA368cfc1151EAc81CE7d08873CC068A5A7;

    mapping(address => uint) public amountNFTsperWallet;

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, string memory _baseURI) ERC721A("The Gaming Club", "TGC")
    ...
    function Mint(uint _quantity) external payable callerIsUser {
        ...
        require(amountNFTsperWallet[msg.sender] + _quantity <= 1, "Solo es posible mintear 1 NFT por cuenta");
        require(totalSupply() + _quantity <= MAX_SUPPLY, "Maximo Supply alcanzado");
    ...
    function releaseAll() external {
        require(msg.sender == addressOW, "Only Owner can release");
