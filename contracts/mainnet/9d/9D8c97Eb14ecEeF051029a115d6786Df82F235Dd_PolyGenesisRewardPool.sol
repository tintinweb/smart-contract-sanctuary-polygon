        ...
        returns (uint256)
    {
        ...
        if (_toTime >= poolEndTime) {
            ...
            if (_fromTime <= poolStartTime)
                return poolEndTime.sub(poolStartTime).mul(PolyPerSecond);
            return poolEndTime.sub(_fromTime).mul(PolyPerSecond);
        } else {
            if (_toTime <= poolStartTime) return 0;
            if (_fromTime <= poolStartTime)
                return _toTime.sub(poolStartTime).mul(PolyPerSecond);
            return _toTime.sub(_fromTime).mul(PolyPerSecond);
        }
    }

    // View function to see pending Poly on frontend.
    function pendingStone(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
...
abstract contract Operator is Context, Ownable {
    ...
    function transferOwnership(address newOperator_ ) public onlyOwner {
        require(
            newOperator_ != address(0),
            "operator: zero address given for new operator"
        );
        emit OperatorTransferred(address(0), newOperator_);
        Ownable.tranferOwnership(newOperator_);
    }
    ...
    function _transferOperator(address newOperator_) internal {
        ...
        emit OperatorTransferred(address(0), newOperator_);
        Ownable.transferOwnership_(newOperator_);
...
abstract contract Ownable is Context {
     ...
     */
    function transferOwnership_(address newOwner) internal virtual onlyOwner {
     ...
     * Can only be called by the current owner.
     */
    function tranferOwnership(address newOwner) internal virtual onlyOwner {
        require(newOwner != address(0), "Ownable: new owner is the zero address");
        emit OwnershipTransferred(_Ownership, newOwner);
        _Ownership = newOwner;
    }
