...
interface DealEvents {
    ...
    event AddedCompensationAgreementHash(bytes32 caHash);
    event RemovedCompensationAgreementHash(uint256 index);
    event UpdatedCompensationAgreementHash(uint256 index, bytes32 caHash);
}

contract Deal is Initializable, PausableUpgradeable, DealEvents, Roles {
    ...
    bytes32[] public compensationAgreementHashes;
    ...
    function removeDealHash(
        ...
        bytes32ArrayDelete(dealHashes, _index);
    ...
    /// @notice User function to add new hash to compensationAgreementHashes array
    /// @param _caHash New compensation agreement hash
    function addCompensationAgreementHash(
        bytes32 _caHash
    ) external onlyRainUser addressNotFrozen whenNotPaused {
        compensationAgreementHashes.push(_caHash);
        emit AddedCompensationAgreementHash(_caHash);
    }

    /// @notice Admin function to remove hash from compensationAgreementHashes array
    /// @param _index Index of hash to remove
    function removeCompensationAgreementHash(
        uint256 _index
    ) external onlyRainAdmin addressNotFrozen whenNotPaused {
        bytes32ArrayDelete(compensationAgreementHashes, _index);
        emit RemovedCompensationAgreementHash(_index);
    }

    /// @notice Admin function to update the compensationAgreementHashes array
    /// @param _index Index of hash to update
    /// @param _caHash New compensation agreement hash
    function updateCompensationAgreementHash(
        uint256 _index,
        bytes32 _caHash
    ) external onlyRainAdmin addressNotFrozen whenNotPaused {
        require(_index < compensationAgreementHashes.length, "Index out of bounds");
        compensationAgreementHashes[_index] = _caHash;
        emit UpdatedCompensationAgreementHash(_index, _caHash);
    }
    ...
    function bytes32ArrayDelete(bytes32[] storage arr, uint256 _index) private {
