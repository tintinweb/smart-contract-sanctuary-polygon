...
import "./interfaces/IMeshSingle.sol";

contract StratMeshSingle is Ownable, ReentrancyGuard, Pausable {
    ...
    address public wantAddress; // ibToken
    address public token0Address; //
    address public token1Address; //
    address public earnedAddress; // Meshswap Protocol
    address public uniRouterAddress; // Meshswap Router
    ...
    function deposit(address _userAddress, uint256 _wantAmt) public onlyOwner nonReentrant whenNotPaused returns (uint256) {
        ...
        uint256 entranceFee = _wantAmt.mul(entranceFeeFactorMax.sub(entranceFeeFactor)).div(entranceFeeFactorMax);
        IERC20(wantAddress).safeTransfer(rewardsAddress, entranceFee);

        uint256 sharesAdded = _wantAmt.sub(entranceFee);

        if (wantLockedTotal > 0 && sharesTotal > 0) {
            sharesAdded = sharesAdded.mul(sharesTotal).div(wantLockedTotal);
        }
    ...
    function earn() public whenNotPaused nonReentrant {
        ...
        // Harvest farm tokens
        IMeshSingle(wantAddress).claimReward();
        ...
        if (earnedAddress != token0Address) {
            ...
            _safeSwap(uniRouterAddress, earnedAmt, slippageFactor, earnedToToken0Path, address(this), block.timestamp.add(600));
        }
        ...
        if (token0Amt > 0) {
            ...
            IMeshSingle(wantAddress).depositToken(token0Amt);
        }
    ...
    function setEarnedToMSDPath(address[] memory _earnedToMSDPath) public onlyAllowGov {
        earnedToMSDPath = _earnedToMSDPath;
    }

    function setMSDAddress(address _MSDAddress) public onlyAllowGov {
        MSDAddress = _MSDAddress;
    }
