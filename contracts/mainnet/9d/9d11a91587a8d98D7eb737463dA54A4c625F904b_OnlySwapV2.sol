...
contract OnlySwapV2 {
    ...
    function internalSwapTokensForTokens(address tokenA,address tokenB,uint amountA,bool takefee) internal returns(uint,uint) {
    ...
    function bestPathSwapTokensForTokens(address tokenA,address tokenB,uint amountA,uint slippage) public payable {
        ...
        (uint amount,uint price)=internalSwapTokensForTokens(tokenA,tokenB,amountA,true);
    ...
    function swapTokensForTokens(address router,address tokenA,address tokenB,uint amountA,uint32 version) internal returns(uint amountOut) {
        ...
        amountOut=IERC20(tokenB).balanceOf(address(this));
        //refund token
        uint tokenBalance=IERC20(tokenA).balanceOf(address(this));
        if(tokenBalance>0){
            IERC20(tokenA).transfer(msg.sender,tokenBalance);  
        }
    }
