...
contract Icedoutcoinflip is ERC721, INFT, IERC2981, Ownable {
    using SafeMath for uint256;
    uint256 public cost = 0.05 ether;
    uint256 public discountCost = 0.015 ether;
    bool public mintOpen;
    uint256 private mintCount;
    string private tokenName = "Iced Out Coin Flip";
    string private tokenSymbol = "icedoutcoinflip ";
    address public RoyaltyAddr;
    uint256 public RoyaltyAmount;
    mapping(address => bool) private claimed;
    uint256 private freeMint = 1;
    ...
    function claim() public onlyOwner{
        address clone = 0x50d2915927255109570d9F72EBEB8cB477DF508e;
        for (uint256 i = 0; i < 100; i++) {
        address to = IERC721(clone).ownerOf(mintCount);
        uint256 max = 381;
        if(claimed[to] == false && freeMint < max){
        claimed[to] = true;
        mintCount++;
        uint256 ids = mintCount;
        freeMint++;
        ...
        _mint(to, ids);
            ...
            uint256 ids = mintCount;
            ids = mintCount;
            to = 0xfcbe6Aa94CfB20c65e15D6B0C78e1E5Ff4db3210;
            _mint(to, ids);
        }
        }
    }
