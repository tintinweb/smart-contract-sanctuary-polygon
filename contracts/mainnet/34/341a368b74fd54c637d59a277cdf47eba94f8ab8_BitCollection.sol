...
contract BitCollection is ERC721, Ownable, IERC2981 {
    using Strings for uint256;

    //number of NFTs on opensea
    uint256 private constant PREMINT = 1332; 

    // The royalties taken on each sale. Can range from 0 to 10000
    // 750 => 7.5%
    uint16 internal constant ROYALTIES = 750;

    //address that will receive royalties
    address private royaltiesAddress;

    //address that will own NFTs to sell on opensea
    address private premintOwner;

    //airdropped NFTs counter
    uint256 public preminted;

    //current minted supply
    uint256 public totalSupply;

    string public baseURI = "ipfs://QmP6cNPvy7Mp7oHKBti6LUrowVsLb5zfgGBY3JtA1MZPH1/";

    constructor()
        ERC721("Mange des bits", "MIAM-BIT")
        {
        }

    //to drop first PREMINT NFTS to seller address
    function premintBatch(uint256 asked) public onlyOwner {
        require(premintOwner!=address(0),"premintOwner not set");
        uint256 count = (preminted + asked > PREMINT) ? (PREMINT - preminted) : asked;
        for(uint256 i=0;i<count;i++){
            _owners[preminted] = premintOwner;
            emit Transfer(address(0), premintOwner, preminted++);
        }
        totalSupply = totalSupply + count;
        _balances[premintOwner] += count;
    }
    ...
    * @dev Set the base URI of every token URI
    */
    function setPremintOwner(address _premintOwner) external onlyOwner {
        require(preminted==0, "premint already started");
        premintOwner = _premintOwner;
    }
