...
// File: sSlice.sol
  ...
  contract slice is ERC20 {
      ...
      constructor (string memory name, string memory symbol) ERC20(name, symbol) {
        ...
        vTotalSupply = 438000000000 * 10 ** uint(decimals());
      ...
      require (challengers[msg.sender][_addr] != _addr && m.whitelisted!=false, "Alr rep");
      ...
      uint indx = index[_addr];
      Minters storage m = mintersDB[indx];
      require (msg.sender != _addr, "NotAllowed");
      require (challengers[msg.sender][_addr] != _addr, "Alr rep");
      require (m.goodCount<maxGoodCount, "maxGoodCount");
      require (m.reported>0, "NotNeeded");
      ...
      returns (uint8, bytes32, bytes32)
    {
      ...
      function SliceMinting() external onlyWhitelisted {
          require(vSupply>=vTotalSupply, "notyetminted");
      ...
      function vSliceMinting_ExW() external onlyWhitelisted {
              require(vSupply<= vTotalSupply, "already minted");
      ...
      function stakeVSlice_h56(uint _amount, address _addr) external {
          uint indx = index[_addr];
          Minters storage m = mintersDB[indx];
          require (vSliceBalances[_addr] >= _amount && m.whitelisted!=false, "2much");
      ...
      function unstakeVSlice_Hha(uint _amount, address _addr) external  {
          uint indx = index[_addr];
          Minters storage m = mintersDB[indx];
          require(stakedVSlices[_addr]>=_amount && m.whitelisted!=false, "2much");
          stakedVSlices[_addr] -= _amount;
          vSliceBalances[_addr] += _amount;
      ...
      function setGoodP (uint _p) external onlyAdmin{
      goodPrice = _p;
    }
      function getGoodP () view external returns (uint) {
      return goodPrice;
        }
      function updateWenBlock(uint _count) external onlyAdmin {
      wenBlock = _count;
    }
      function countMinters() view external returns(uint) {
      return mintersDB.length;
      }
      function updateMaxGoodCount(uint _count) external onlyAdmin{
      maxGoodCount = _count;
    }
      function sliceParams () view external returns (uint, uint, uint, uint, uint, uint, address, uint) {
      return (vSupply, vTotalSupply, wenBlock, maxGoodCount, mintingSpeed, goodPrice, signer, pw[listAdmin]);
      }
      event Chat(address indexed _from, address indexed _to, string message);
      function sendMessage(address _to, string calldata message) external {
        emit Chat(msg.sender, _to, message);
      }
      //make withdrawal of Reputation fee
