...
// File: contracts/DoodlePalUtility.sol


pragma solidity ^0.8.2;


contract DoodlePalUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressDoodlePalNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitDoodlePalNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountDoodlePalNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersDoodlePalNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedDoodlePalNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogDoodlePalNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserDoodlePalNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedDoodlePalNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserDoodlePalNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioDoodlePalNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedDoodlePalNFT (address owner, uint256 amount);
    event NFTStakedDoodlePalNFT (address owner, uint256 tokenId);
    event NFTUnstakedDoodlePalNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressDoodlePalNFT, address _collectionAddressNFTv2) ERC20("DoodlePalUtility", "DPU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressDoodlePalNFT = _collectionAddressDoodlePalNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitDoodlePalNFT = 5 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountDoodlePalNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserDoodlePalNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserDoodlePalNFT[owner];
    }
    ...
    function pendingRewardsDoodlePalNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogDoodlePalNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitDoodlePalNFT;
    ...
    function stakeDoodlePalNFT(uint256 tokenId) public {
        IERC721(collectionAddressDoodlePalNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressDoodlePalNFT).ownerOf(tokenId) == address(this),
            "DoodlePalUtility: Error while transferring token");
        StakeInfo storage info = stakeLogDoodlePalNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserDoodlePalNFT[_msgSender()] == 0){
            amountOfStakersDoodlePalNFT += 1;
        }
        tokensStakedByUserDoodlePalNFT[_msgSender()] += 1;
        tokensStakedDoodlePalNFT += 1;
        stakePortfolioByUserDoodlePalNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserDoodlePalNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioDoodlePalNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedDoodlePalNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountDoodlePalNFT);
            welcomeBonusCollectedDoodlePalNFT[tokenId] = true;
        }

        emit NFTStakedDoodlePalNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchDoodlePalNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeDoodlePalNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchDoodlePalNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeDoodlePalNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeDoodlePalNFT(uint256 tokenId) public {
        if(pendingRewardsDoodlePalNFT(_msgSender(), tokenId) > 0){
            harvestDoodlePalNFT(tokenId);
        }
        StakeInfo storage info = stakeLogDoodlePalNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressDoodlePalNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressDoodlePalNFT).ownerOf(tokenId) == _msgSender(),
            "DoodlePalUtility: Error while transferring token");
        if(tokensStakedByUserDoodlePalNFT[_msgSender()] == 1){
            amountOfStakersDoodlePalNFT -= 1;
        }
        tokensStakedByUserDoodlePalNFT[_msgSender()] -= 1;
        tokensStakedDoodlePalNFT -= 1;
        stakePortfolioByUserDoodlePalNFT[_msgSender()][indexOfTokenIdInStakePortfolioDoodlePalNFT[tokenId]] = 0;
        emit NFTUnstakedDoodlePalNFT(_msgSender(), tokenId);
    }
    ...
    function harvestDoodlePalNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogDoodlePalNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsDoodlePalNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedDoodlePalNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchDoodlePalNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserDoodlePalNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestDoodlePalNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitDoodlePalNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitDoodlePalNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountDoodlePalNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountDoodlePalNFT = coinAmount;
    }
    ...
    function setCollectionAddressDoodlePalNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressDoodlePalNFT = newAddress;
    }
