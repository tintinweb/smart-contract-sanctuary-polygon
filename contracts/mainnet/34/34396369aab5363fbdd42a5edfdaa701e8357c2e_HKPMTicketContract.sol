...
/// @title Contract for HKPM Open Ceremony Commemorative Ticket
...
contract HKPMTicketContract is ERC721, Ownable {
    uint256 public constant MAX_SUPPLY = 1180;
    string public baseURI;
    uint256 public totalSupply;

    mapping(bytes32 => bool) public tidMinted;

    constructor() ERC721("HKPM Open Ceremony Commemorative Ticket", "HKPMTIX") {
        baseURI = "https://dot.dttd.group/HKPMTIX/";
    }
    ...
    modifier tidCheck(bytes32 tid) {
        require (tidMinted[tid] == false, "tid already used");
        _;
    }

    function airdrop(bytes32 tid, address _receiver, uint256[] calldata token_ids) external onlyOwner tidCheck(tid) maxSupplyCheck(token_ids.length) {
        tidMinted[tid] = true;
        totalSupply += token_ids.length;
        for (uint256 i = 0; i < token_ids.length; i++) {
            require(!_exists(token_ids[i]), "Token already minted");
            _mint(_receiver, token_ids[i]);
        }
    }
}
