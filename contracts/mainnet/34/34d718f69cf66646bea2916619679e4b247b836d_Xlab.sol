...
contract Xlab is Ownable {
    ...
    function _createProduct( uint productId, string memory title, uint woodId ) private {        
        Product memory _product = Product(productId, title, woodId);
        products[productId] = _product;
        productscount++;
        emit NewProduct(productId, title);
    }

    function _createOrder(uint _id, uint _rank, uint _prevState, uint _currentState, string memory _commit, string memory _url, string memory _photo, Product memory _product) private {        
        Order memory _order = Order(_id, _rank, _prevState, _currentState, 0, '', _commit, _url, _photo, _product);
        orders[_id] = _order;
        orderscount++;
    ...
    function createOrder(uint _id, uint _rank, uint _prevState, uint _currentState, string memory _commit, string memory _url, string memory _photo, Product memory _product) public payable {
        _createOrder(_id, _rank, _prevState, _currentState, _commit, _url, _photo, _product);        
    }

    function createProduct(uint productId, string memory title, uint woodId) public payable {
        _createProduct(productId, title, woodId);        
    }
