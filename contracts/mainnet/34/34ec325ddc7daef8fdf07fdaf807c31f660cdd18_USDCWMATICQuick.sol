...
contract USDCWMATICQuick is CoreStrategyAave {
    ...
    function balancePendingHarvest() public view override returns (uint256) {
        uint256 dQuickPending =
            IStakingDualRewards(farmMasterChef).earnedA(address(this)).add(
        ...
        total_wmatic = total_wmatic.add(
            IStakingDualRewards(farmMasterChef).earnedB(address(this))
        );
    ...
    function _withdrawFarm(uint256 _amount) internal override {
        if (_amount > 0) IStakingDualRewards(farmMasterChef).withdraw(_amount);
    }
