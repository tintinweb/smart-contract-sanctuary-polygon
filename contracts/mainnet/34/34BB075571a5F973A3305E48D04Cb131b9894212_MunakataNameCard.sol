...
contract MunakataNameCard is ERC1155, AccessControl, Ownable {
    using Strings for uint256;

    bytes32 public constant ADMIN = "ADMIN";
    string public baseURI;
    string public baseExtension;
    mapping(uint256 => mapping(address => bool)) public mintedAddresses;
    ...
    function airdrop(uint256 _tokenId, address[] calldata _addresses) external onlyRole(ADMIN) {
        for (uint256 i = 0; i < _addresses.length; i++) {
            if (mintedAddresses[_tokenId][_addresses[i]] == false) {
                _mint(_addresses[i], _tokenId, 1, "");
                mintedAddresses[_tokenId][_addresses[i]] = true;
            }
        }
    }
