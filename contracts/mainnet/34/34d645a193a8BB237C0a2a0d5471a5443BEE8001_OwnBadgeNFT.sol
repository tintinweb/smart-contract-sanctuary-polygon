...
contract OwnBadgeNFT is ERC721, EIP712MetaTransaction("Own Badge Minter Test", "1"), Ownable {
    ...
    string badgeURI = "https://own-badge.s3.amazonaws.com/badge/";
    string public baseExtension = "";

    event User(uint256 timestamp, address user);
    event Mint(uint256 timestamp, address user, uint256 tokenId);

    constructor() public ERC721("Own Badge Test", "OwnBadge") {}

    function mintToCaller(address caller) 
        public
        returns (uint256)
    {
        require(isWhitelisted[caller], "Own: caller not authorised"); // Makes sure that only the discord members can mint the badge
        require(isMinted[caller] == false, "Own: NFT already minted");
        _tokenIds.increment();
    ...
    function setWhitelist(address[] memory users) external onlyOwner {
        for (uint256 i = 0; i<users.length; i++) {
            if(isWhitelisted[users[i]]==false) {
                isWhitelisted[users[i]] = true;
            }
        }
    }

    function isAddressWhitelisted(address user) public view virtual returns (bool) {
        return isWhitelisted[user];
    }

    function hasUserMinted(address user) public view virtual returns (bool) {
        return isMinted[user];
    }

    // internal
    function _badgeURI() internal view virtual returns (string memory) {
        return badgeURI;
    }

    function setbadgeURI(string memory _newBadgeURI) public onlyOwner {
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        string memory currentBadgeURI = _badgeURI();
        
        return bytes(currentBadgeURI).length > 0
        ? string(abi.encodePacked(currentBadgeURI, Strings.toString(tokenId), baseExtension))
        : "";
     }
