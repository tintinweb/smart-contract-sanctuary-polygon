...
contract MarketPlaceV2 {
    ...
    function insertNftId(uint p_nftID) private {
        bool exist=false;
        for(uint i=0;i<_allIdsNFT.length;i++){
            if(_allIdsNFT[i]==p_nftID){
                exist=true;
                break;
            }
        }
    ...
    function deleteOpenSale(uint p_nftID) private {
        ...
        id_openSales[_openSales[last]]=i;
        _openSales.pop();
    ...
    function _openSale(uint256 p_nftID, uint256 p_price, address token) private {
            ...
            s_sales[p_nftID].push(Sale(msg.sender,Status.open,p_price,block.timestamp,token,getName(token),getSymbol(token),getDecimals(token)));
    ...
    function cancelSale(uint256 p_nftID) public whenNotPaused securityFrontRunning(p_nftID) {
        ...
        s_sales[p_nftID].push(Sale(last.owner,Status.cancelled,last.price,block.timestamp,last.token,last.name,last.symbol,last.decimals));
    ...
    function buy(uint256 p_nftID) public whenNotPaused securityFrontRunning(p_nftID) {
        ...
        require(ERC20(last.token).transferFrom(msg.sender, last.owner, last.price), "Error transfer token - price");
        require(ERC20(last.token).transferFrom(msg.sender, devsAddress, (last.price * fee) / 100 ), "Error transfer fee"); // fee 3%
        s_NFTs.transferFrom(last.owner, msg.sender, p_nftID);
        s_sales[p_nftID].push(Sale(msg.sender,Status.executed,last.price,block.timestamp,last.token,last.name,last.symbol,last.decimals));
        deleteOpenSale(p_nftID);
    }
    function getPriceWithFee(uint256 p_nftID) public view returns (uint) {
        Sale memory last=getSale(p_nftID);
        require(last.status == Status.open, "Is not Open");
        return last.price + ((last.price * fee) / 100);
    }
