...
contract BrewlabsSwapFeeManager is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function getPoolFeInfo(address pair) external view returns (PoolFeInfo memory) {
        return
            PoolFeInfo(
                pools[pair].token0,
                pools[pair].token1,
    ...
    function _claim(address pair) internal existPair(pair) {
        require(block.timestamp > pools[pair].timeToOpen, 'BrewlabsFeeManager: UNABLE TO CLAIM BEFORE OPEN TIME');
