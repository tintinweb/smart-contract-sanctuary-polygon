...
contract FeeManagerGenericUpgradeable is Initializable, ContextUpgradeable, AccessControlUpgradeable {
    ...
    function setFee(
        uint8 destinationChainID,
        address feeTokenAddress,
        uint256 feeFactor, // Price factor
        uint256 bridgeFee,
        uint256 gasLimit, // Default
        uint256 gasPrice, // Default
        uint8 decimals,
        bool accepted
    ) public isHandler {
        ...
        fees._gasPrice = gasPrice;
        fees._gasLimit = gasLimit;
    ...
    /**
        ...
        @param  feeTokenAddress address of the fee token.
    */
    function getFee(uint8 destinationChainID, address feeTokenAddress) public view virtual returns ( uint256 , uint256 , uint256 , uint256 , uint8  ) {
        require(_fees[destinationChainID][feeTokenAddress].accepted, "FeeManager: fees not set for this token");
        Fees storage fees = _fees[destinationChainID][feeTokenAddress];
        return ( fees._feeFactor , fees._bridgeFee , fees._gasLimit , fees._gasPrice , fees._decimals );
    }
    ...
    */
    function getFeeSafe(uint8 destChainId, address feeToken) public view virtual returns (uint256, uint256 , uint256 , uint256  , uint8 , bool) {
        Fees storage fees = _fees[destChainId][feeToken];
        return ( fees._feeFactor , fees._bridgeFee , fees._gasLimit , fees._gasPrice , fees._decimals , fees.accepted );
    }
