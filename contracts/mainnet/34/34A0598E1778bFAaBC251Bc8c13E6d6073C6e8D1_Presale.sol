...
contract Presale is AccessControl {
    ...
    uint256 unitPrice = 90 ether;
    uint256 public startTime = 1667498400;

    event Buy(address indexed user, uint256 count);
    ...
    function buy(
        ...
        require(msg.value >= price(count), 'insufficient fund');
        users[to] += count;
        emit Buy(to, count);
    }
