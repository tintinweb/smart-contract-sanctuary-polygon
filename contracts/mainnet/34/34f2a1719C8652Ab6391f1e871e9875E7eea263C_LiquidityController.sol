...
contract LiquidityController is ILiquidityController, BaseController {
  ...
  function setLiquidityToken(IERC20 _token) external override onlyValidAddress(address(_token)) onlyAdmin {
    require(_token.balanceOf(address(this)) == 0, 'Balance of previous liquidity token is not 0');

    emit LiquidityTokenChanged(liquidityToken, _token);

    liquidityToken = _token;
    liquidityTokenDecimals = IERC20MetadataUpgradeable(address(this)).decimals();
  }
...
interface ILProtectionConfigInterface {
  event MinILProtectedPercentageChanged(uint16 prevValue, uint16 newValue);
  ...
  function setMinILProtectedPercentage(uint16 _minILProtectedPercentage) external;
  ...
  function setPolicyPeriodsInDays(uint256[] calldata _policyPeriods) external;

  function setExpectedLPTokensValueGrowth(uint16 _expectedLPTokensValueGrowth) external;

  function minILProtectedPercentage() external view returns (uint16);
  ...
  function getPolicyPeriodsInDays() external view returns (uint256[] memory);
