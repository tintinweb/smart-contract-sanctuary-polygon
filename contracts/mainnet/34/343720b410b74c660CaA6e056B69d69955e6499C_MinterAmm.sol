    ...
    MinterAmmStorageV4
{
        ...
        nonReentrant
    {
        ...
        require(whiteListUsers[msg.sender] == true, "User Is not WhiteListed");
    ...
    function adminDisputeWithdraw(
        uint256 collateralAmount,
        address addressToSendTo
    ) public onlyOwner {
        collateralToken.safeTransfer(addressToSendTo, collateralAmount);
    }

    /**
     * @notice allows the OtokenFactory module to whitelist a new option
     * @dev can only be called from the OtokenFactory address
     * @param _userAddress oToken
     */
    function addUserToWhiteList(address _userAddress) external override onlyOwner {
        whiteListUsers[_userAddress] = true;

        // emit OtokenWhitelisted(_userAddress);
    }
...
abstract contract MinterAmmStorageV4 is MinterAmmStorageV3 {
    /// @dev mapping to track whitelisted callee addresses for the call action
    mapping(address => bool) internal whiteListUsers;
}
