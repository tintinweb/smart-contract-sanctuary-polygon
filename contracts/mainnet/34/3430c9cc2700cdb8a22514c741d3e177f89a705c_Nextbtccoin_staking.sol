...
contract Nextbtccoin_staking is ERC20,Initializable {
	using SafeMath for uint256;
    ...
    function _calculateseedReward(address userAddress) internal returns (uint256){
    ...
    function _calculatedailyReward(address user_address) internal returns(uint256){
        uint256 rewardUser;
            ...
            if (user.checkpoint < finish) {
				...
				uint256 to = finish < block.timestamp ? finish : block.timestamp;
                    ...
                    if (from < to) {
                        ...
                        FinalAmount= check_balance(user_address,_FinalAmount);
