...
contract TemaNFT is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        uint256 rand = random(uint2str(tokenId));
        string memory svg = generateSVG(rand);
                ...
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"',
                                "Tema | Crypto hub NFT", // You can add whatever name here
                                '", "description":"Ya HASBULA)", "attributes":"", "image":"',imageURI,'"}'
                            )
                        )
                    )
                )
    ...
    require(numClaimed >= 0 && numClaimed <= 10000, "invalid claim");
    _safeMint(_msgSender(), numClaimed + 1);
    numClaimed += 1;
    }
    
    constructor() ERC721("TemaNFT", "Tema") Ownable() {}
}