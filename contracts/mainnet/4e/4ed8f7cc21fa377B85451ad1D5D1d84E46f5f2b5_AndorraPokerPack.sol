...
contract AndorraPoker is ERC721Linkable {
    event NewParticipant(address indexed participant, uint256 tokenId);

    IERC721 immutable public MRC; // = IERC721(0xeF453154766505FEB9dBF0a58E6990fd6eB66969);
    ERC20 immutable public USDC; // = IERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    IOracle immutable public ORCL; // = Oracle(0x73366Fe0AA0Ded304479862808e02506FE556a98);
    ...
    modifier onlyHolder(uint256 tokenId, address wallet) {
        require(MRC.ownerOf(tokenId) == wallet, "Not holder");
        _;
    }
    ...
    modifier onlyAdmin() {
        require(
            isAdmin[msg.sender] == true || msg.sender == owner,
            "Not admin"
        );
        _;
    }
    ...
    ) ERC721Linkable(name_, symbol_, MRC_) {
        owner = tx.origin;
    ...
    function _mintProcess(
        uint256 tokenId,
        address wallet
    ) private notPaused onlyHolder(tokenId, wallet) {
        require(registered[tokenId] == false, "Token already registered");
        require(players.length < maxPlayers, "All slots have been filled");

        registered[tokenId] = true;
        players.push(tokenId);
        _safeMint(wallet, players.length);

        _approve(address(this), players.length);
        this.linkToken(players.length, tokenId);

        emit NewParticipant(wallet, tokenId);
    }

    function mint(uint256 tokenId) public {
        USDC.transferFrom(msg.sender, owner, getPriceUSD());
        _mintProcess(tokenId, msg.sender);
    }

    function freeMintTo(uint256 tokenId, address holder) public onlyAdmin {
        _mintProcess(tokenId, holder);
    }
    ...
    function getPriceUSD() public view returns (uint256) {
        uint256 toUSD = (price * uint256(ORCL.latestAnswer())) / (10 ** ORCL.decimals());
        return toUSD;
    }
    ...
    function setMaxPlayers(uint256 maxPlayers_) public onlyOwner {
    ...
    function setAdmin(address admin_, bool adminBool) public onlyOwner {
        isAdmin[admin_] = adminBool;
    }
...
import "@openzeppelin/contracts/token/ERC20/ERC20.sol";
import "./AndorraPoker.sol";

contract AndorraPokerPack {
    ERC20 immutable USDC; // = ERC20(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174);
    IOracle immutable public ORCL; // = Oracle(0x73366Fe0AA0Ded304479862808e02506FE556a98);
    AndorraPoker immutable ADP;
    ...
    function mint(uint256 tokenId) public {
        USDC.transferFrom(msg.sender, owner, getPriceUSD());
        ADP.freeMintTo(tokenId, msg.sender);
    }

    function freeMintTo(uint256 tokenId, address wallet) public onlyOwner {
        ADP.freeMintTo(tokenId, wallet);
    }
