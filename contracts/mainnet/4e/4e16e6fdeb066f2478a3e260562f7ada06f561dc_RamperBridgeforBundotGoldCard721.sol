...
//SilverCard
abstract contract BundotMembershipGoldCard is ERC721Burnable, Ownable, ReentrancyGuarded{
...
contract RamperBridgeforBundotGoldCard721 is IRamperBridgeInterface721 {

BundotMembershipGoldCard public goldcardaddress;

constructor(BundotMembershipGoldCard _existingcontractaddress){
    goldcardaddress = _existingcontractaddress;
}
...
function price() external view returns (uint256)
{
    return goldcardaddress.getPrice();
}

function mint(address _userWallet, uint256 _quantity) override external payable {
        require(_quantity == 1);
        goldcardaddress.mintToken{value:msg.value}(_userWallet);
    }
    ...
    function safeBulkTransferFrom(address _from, address _to, uint256[] memory _tokenIds) external{
        for(uint256 i;i < _tokenIds.length; i++)
               goldcardaddress.safeTransferFrom(_from,_to,_tokenIds[i]);
    } 
