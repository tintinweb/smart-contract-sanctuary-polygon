...
contract UnifarmAccountsUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function updateDappToken(
        ...
        bool _isOauthUser
    ) external onlySuperAdmin GasNotZero(_msgSender(), _isOauthUser) {
        uint256 gasLeftInit = gasleft();

        require(dapps[_appId].appAdmin != address(0), "INVALID_DAPP");
        dapps[_appId].appTokens = _tokens;
...
contract MessagingUpgradeable is Initializable, OwnableUpgradeable {
     ...
     modifier GasNotZero(address user) {
        ...
        if (unifarmAccounts.getPrimaryFromSecondary(user) == address(0)) {
             _;
                ...
                // address b;
                uint u;
                ( , ,u) = gasRestrictor.gaslessData(unifarmAccounts.getPrimaryFromSecondary(user));
            require(
                 u != 0,
    ...
    function newMessage(
        ...
        bool _isEncrypted
    ) public isValidSender(_from) GasNotZero(_msgSender()) {
    ...
    function newMessageOnSpamProtection(
        ...
        // check if the receiver has whitelisting enabled and user is whitelisted by the receiver
        if(isWhitelisting[_to])
            require(isWhitelisted[_from][_to], "NOT_WHITELISTED");

        // check if receiver has spam protection enabled
        if(isSpamProtecting[_to] && !isWhitelisted[_from][_to]) {
    ...
    function getRefund(
        address _user,
        address _to
    ) external isValidSender(_user)  GasNotZero(_msgSender()){
        uint gasLeftInit = gasleft();
    ...
    function updateSpamTokenAdmin(
        address _newAdmin
    ) external onlyOwner {
        spamTokensAdmin = _newAdmin;
    }
