    ...
    Pausable
{
    ...
    address[] private _team = [0x563cB1729C1B06E07567482702a3EB2E9cc5fe86];
    ...
    bool private _stakingActivated = false;


    /**
     * @notice The staked NFT's and their stake information
     */
    ...
    ) ERC721("OyabunGenesis", "OYA") PaymentSplitter(_team, _teamShares) {
        transferOwnership(msg.sender);
        sellingStep = Steps.Before;
        _baseURIVariable = _theBaseURI;
        notRevealedURI = _notRevealedURI;
        _royaltiesReceiver = _initialRoyaltiesReceiver;
        _nftIdCounter.increment();

        // Init the price of the nft in sale
        priceSale[1] = 297 ether;
        priceSale[2] = 247.5 ether;
        priceSale[3] = 198 ether;

        // // Gift 50 NFT to an address
        // address accountGifted = 0xd5552D43Fb973B12f03a8289D4b713e87Ec8a018;
        // for (uint256 i = 0; i < 50; i++) {
        //     uint supply = totalSupply();
        //     require(supply + 1 <= MAX_SUPPLY, "Sold out");
        //     _safeMint(accountGifted, supply + 1);
        // }
    }
    ...
    modifier whenStakingNotPaused() {
        require(_stakingActivated, "Staking not started");
        _;
    }

    function setStakingStatus(bool _status) public onlyOwner{
        _stakingActivated = _status;
    }

    function stakeNFT(uint256 _tokenId) public whenNotPaused whenStakingNotPaused {
        _stakeNFT(_tokenId);
    }

    /**
     * @notice Use to stake a NFT
     * @param _tokenId the identifier of the NFT you want to stake
     */
    function _stakeNFT(uint256 _tokenId) internal {
        // Verify that the token is owned by the caller
        require(_isApprovedOrOwner(msg.sender, _tokenId));
        // Burn the token
        _burn(_tokenId);
        // Add the token to the stakes mapping
    ...
    function unstakeNFT(uint256 _tokenId) public whenNotPaused whenStakingNotPaused {
        _unstakeNFT(_tokenId);
    }

    /**
     * @notice Use to unstake a NFT
     * @param _tokenId the identifier of the NFT you want to unstake
     */
    function _unstakeNFT(uint256 _tokenId) internal
    {
        // Verify that the stake exists
        require(stakes[_tokenId].instanciated, "The stake does not exist");
        // Verify that the token is owned by the caller
        require(
            stakes[_tokenId].owner == msg.sender,
            "You are not the owner of the stake"
        );
        // Remove the token from the stakes mapping
        Stake memory stake = stakes[_tokenId];
        delete stakes[_tokenId];
        // Add the token to the owner's balance
        _safeMint(stake.owner, _tokenId);
    }

    function unstakeAll() onlyOwner external {
        for(uint256 i = 0; i < _nftIdCounter.current(); i++) {
            if (stakes[i].instanciated) {
                // Remove the token from the stakes mapping
                Stake memory stake = stakes[i];
                delete stakes[i];
                // Add the token to the owner's balance
                _safeMint(stake.owner, i);
            }
        }
    }

    /**
     * @notice Return the royalties receiver
     *
    ...
    function setRoyaltiesReceiver(address newRoyaltiesReceiver)
        ...
        onlyOwner
    {
        require(newRoyaltiesReceiver != _royaltiesReceiver); // not the same address
     ...
     *
     * @param _whitelist The whitelist address
     *
     **/
    ...
    function addBulkWhitelist(address[] memory _whitelists) external onlyOwner {
        for (uint256 i = 0; i < _whitelists.length; i++) {
            whitelisted.push(_whitelists[i]);
        }
    } 

    /**
    * @notice Update all whitelists
    *
    * @param _whitelists Array of whitelist address
    *
    **/
    function setWhitelists(address[] memory _whitelists) external onlyOwner {
        whitelisted = _whitelists;
    }

    /**
    * @notice Get all whitelists
    *
    * @return Return all the whitelists
    *
    **/
     ...
     *
     * @param _number The number of NFTS you want to update
     * @param _priceSale The new price of one NFT for the sale
     **/
    function changePriceSale(uint _number, uint _priceSale) external onlyOwner {
        priceSale[_number] = _priceSale;
    }
    ...
    /**
     * @notice Return URI of the NFTs
     *
     * @return The URI of the NFTs
     **/
    function _baseURI() internal view virtual override returns (string memory) {
        require(revealed == true, "The base URI has not been revealed yet");
        return _baseURIVariable;
    }
    ...
    function setUpSale() external onlyOwner {
        // require(
        //     sellingStep == Steps.Presale,
        //     "You can fast forward to the sale, please do the presale before."
        // );
     ...
     *
     **/
    // function presaleMint() external payable nonReentrant whenNotPaused {
    //     //Are we in Presale ?
     ...
     **/
    function saleMint(uint256 _ammount)
        external
        payable
        nonReentrant
        whenNotPaused
    {
        //Is this user on the whitelist ?
        require(isWhiteListed(), "Not on the whitelist");
        // Check if the _ammount is not greater than the last priceSale
        uint price;
        if (_ammount > 3) {
            price = priceSale[3];
        } else {
            price = priceSale[_ammount];
        }
        ...
        require(
            ...
            "You can't mint more than 3 NFT's"
        );
        // Verify that the user don't mint more than 3 NFTS in total
        require(
            nftsPerWallet[msg.sender] + _ammount <= max_mint_allowed,
            "You can't mint more than 3 NFT's per wallet"
        );
        //If the user try to mint any non-existent token
        require(
            _nftIdCounter.current() + _ammount - 1 <= MAX_SUPPLY,
        ...
        if (_nftIdCounter.current() + _ammount - 1 == MAX_SUPPLY) {
    ...
    function gift(address _account) external onlyOwner {
        uint supply = _nftIdCounter.current();
        require(supply <= MAX_SUPPLY, "Sold out");
        _safeMint(_account, supply);
        _nftIdCounter.increment();
    }

    function setCancelWhitelist(bool _status) external onlyOwner {
        cancelWhitelist = _status;
    }
