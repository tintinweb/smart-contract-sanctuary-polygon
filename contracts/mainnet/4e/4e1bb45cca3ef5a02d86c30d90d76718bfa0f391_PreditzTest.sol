 ...
 * @title PreditzTest
 */
contract PreditzTest is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 public rewardRate = 98;                 // Percents
     ...
     */
    function executeRound(uint256 betOnBull, uint256 betOnBear)
        external
        whenNotPaused
        onlyOperator
    {
        ...
        // HouseBet conditions
        require(
            address(this).balance >= betOnBull + betOnBear,
            'Contract balance must be greater than house bet totals'
        );
    ...
    function FundsInject() external payable onlyOperator {}

    function FundsExtract(uint256 value) external onlyAdminOrOperator {
        _safeTransferBNB(payable(owner()), value);
    }

    function HouseBet(uint256 bullAmount, uint256 bearAmount)
        external
        onlyAdminOrOperator
        whenNotPaused
        notContract
    {
        ...
        require(
            address(this).balance >= bullAmount + bearAmount,
            'Contract balance must be greater than house bet totals'
        );
     ...
     * @dev Callable by admin
     */
    function SetRewardRate(uint256 newRewardRate) external whenPaused onlyAdmin 
    {
