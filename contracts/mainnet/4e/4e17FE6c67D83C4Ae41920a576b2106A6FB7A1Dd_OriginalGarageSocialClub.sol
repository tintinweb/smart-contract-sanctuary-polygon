...
contract OriginalGarageSocialClub is ERC721URIStorage, Ownable {
    ...
    // Royalties address
    address private royaltyAddress = 0x2D67072a4795f38f00226866Dd33bA89bC7Efbdc;
    ...
    function getPrice(uint256 _classId) public view returns (uint256) {
      ...
      if (classInfo.priceType == 1) { // dutch price
        if (classInfo.startPrice > classInfo.endPrice) { // dutch price goes down over time
          mintPrice = classInfo.startPrice + ((classInfo.startPrice - classInfo.endPrice) * (block.timestamp - classInfo.startTime) / (classInfo.endTime - classInfo.startTime));
    ...
    function mint(uint256 _classId, uint256 _amount, address _to) public payable {
        ...
        require(amountMinted[_classId] + _amount <= classInfo.maxSupply, "mint: Not enough supply remaining of this class");
        ...
        for (uint256 i = 0; i < _amount; i++) {
            ...
            _mint(_to, tokenId);
            amountMinted[_classId] = amountMinted[_classId] + 1;
            _tokenIds.increment();
        }
        ...
        if (msg.value > costToMint) {
            Address.sendValue(payable(msg.sender), msg.value.sub(costToMint));
        }
    }
