...
contract IncomeRouter is Ownable {
    ...
    function share(
        ...
        // We do not need
        // to sort the waterfall when we are trying to
        // mint an NFR with ID 0 as we know that
        // there will always only be one NFR when minting
        // the first NFR.
        if (s_nfrToBePaid == 0) {
            s_nfrToBePaid = nfrId;
        } else {
            ...
            if (s_waterfall[currNFRId] == 0) {
                if (s_nfrs[currNFRId].maturityDate > maturityDate) {
                    s_waterfall[nfrId] = currNFRId;
                    s_waterfall[currNFRId] = 0;
                } else {
