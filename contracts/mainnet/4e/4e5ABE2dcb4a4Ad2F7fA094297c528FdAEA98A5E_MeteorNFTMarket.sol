...
// flattened :  Monday, 17-Jul-23 05:01:17 UTC
...
contract MeteorNFTMarket is Context, AccessControl {
    ...
    constructor(address _treasury, address _nft, address _usdt, address _mto) {
        ...
        _grantRole(SETPRICE_ROLE, msg.sender);
    }
    ...
    function setMTOPrice(uint256 _price) public onlyRole(SETPRICE_ROLE) {
        MTOPriceByUSDT = _price;
    }

    function setRebate(uint256 _rate) public onlyRole(DEFAULT_ADMIN_ROLE) {
    ...
    function buy(uint8 nftType, bool payMTO, address referrer) external {
        ...
        require(sender != referrer, "MeteorNFTMarket: referrer cant be self");
        ...
        if (sale.price > 0) {
            if (payMTO) {
                uint256 amt = (sale.price * 10 ** 12 * PriceDenominator) /
                    MTOPriceByUSDT;
