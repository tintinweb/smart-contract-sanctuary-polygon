...
contract AaveLendingContract is Ownable {
    ...
    function claim(address assetAddress) external {
        ...
        if(getDateTimeNowInSeconds().sub(investment.lastUpdateDateInSecond) < 10)// 24 hr _ 86400
        { 
        ...
        uint256 masterWalletAmount = (rewards.mul(feePercentage)).div(100);
    ...
    function withdraw (address assetaddress) public {
        ...
        uint256 wamount = withdrawFromLending(assetaddress, investment.totalInvestment);
        if(wamount > 0){ 
