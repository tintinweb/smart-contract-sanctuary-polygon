    ...
    ERC1155NonTransferableUpgradeable
{
    ...
    /// @notice Manual migration of existing Kudos' community data
    /// @dev Temporary function that allows for a manual conversion of the community data for the original Kudos tokens. We do this manually and in a
    ///      centralized way (ie trusting the contract owners to do the right thing) for now.
    ///
    ...
    /// @param tokenIds                   Token IDs to migrate the data
    /// @param prevCommunityDiscordIds    The array of unique Discord server IDs
    /// @param newCommunityUniqIds        The array of unique community IDs associated with the community registry
    function backdoorConversionOfCommunityData(
        uint256[] memory tokenIds,
        string[] memory prevCommunityDiscordIds,
        string[] memory newCommunityUniqIds
    ) public onlyOwner whenPaused {
        for (uint256 i = 0; i < tokenIds.length; i++) {
            require(ICommunityRegistry(communityRegistryAddress).doesCommunityExist(newCommunityUniqIds[i]), "Community uniqId does not exist in community registry");
            require(compareStringsbyBytes(tokenIdToKudosContainer[tokenIds[i]].DEPRECATED_communityDiscordId, prevCommunityDiscordIds[i]), "The stored discord ID is different from what is passed in");
            tokenIdToKudosContainer[tokenIds[i]].communityUniqId = newCommunityUniqIds[i];
        }
    }
