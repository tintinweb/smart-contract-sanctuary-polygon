...
import "../apps/AdminSales/AdminSales.sol";
...
//    }
    ...
    function _deployAdminSales(Kernel kernel) internal {
        address adminSalesCode = updateManager.getLastAppCode(AppsIds.ADMIN_SALES);
        AdminSales adminSales = AdminSales(
            kernel.deployApp(
                AppsIds.ADMIN_SALES, adminSalesCode,
                abi.encodeCall(AdminSales.initialize, (kernel))
            )
        );
        ...
        kernel.addPermission(
            AppsIds.SUDO,
            AppsIds.ADMIN_SALES,
            AdminSales(adminSales).CONTROLLER()
        );
        kernel.addPermission(
            AppsIds.ADMIN_CONTROLLER,
            AppsIds.ADMIN_SALES,
