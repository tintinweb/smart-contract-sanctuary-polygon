...
contract Consensus is 
    ...
    string private uriForFetchingOnly = "https://gateway.pinata.cloud/ipfs/QmPaF5xY6KyPBoHgGa2GjxF9CPW3LBaDuefuCnyYo5Ca4p";
    string private uriForFetchingOnlyLegendary = "https://gateway.pinata.cloud/ipfs/QmNi5Bc93JUcHR3LtpQNdJYXGsbEgBe83L58Kmbtbn2LfQ";

    mapping(uint256 => bool) public legendary;

    string public artworkBackupTx;
    string public artworkLegendaryBackupTx;

    string public artworkTitle = unicode"Consensus #1 by Hardpulz";
    string public artworkDescription = unicode"Tickets & NFT légendaires de la première édition de Consensus organisée par Hardpulz le 17 juin 2022 à Paris";

    bool public lock = false;

    event BackupUploaded(string backupData);

    constructor() ERC721("Consensus #1 NFT", "CHP1") {
        _setDefaultRoyalty(0xBcf8Ac5B94A120f51B6eb37c0abf6D78C160E978, 500);
    }

    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        require(_exists(tokenId), "ERC721Metadata: URI query for nonexistent token");
        if (legendary[tokenId]) {
            return uriForFetchingOnlyLegendary;
        } else {
    ...
    function updateUriForFetchingOnlyLegendary(string calldata _uriForFetchingOnlyLegendary) external onlyOwner {
        uriForFetchingOnlyLegendary = _uriForFetchingOnlyLegendary;
    }

    function setArtworkLegendaryBackupTx(string calldata _tx) external onlyOwner {
        require(bytes(artworkLegendaryBackupTx).length == 0, "Backup locked.");
        artworkLegendaryBackupTx = _tx;
    }
    ...
    function mintLegendary(address[] calldata _owners) external onlyOwner {
        require(!lock, "Soldout");

        uint256 token_id = totalSupply() + 1;

        for (uint256 i = 0; i < _owners.length; i++) {
            legendary[token_id] = true;
            _safeMint(_owners[i], token_id);
            _addPermanentTokenURI(token_id, 'https://gateway.pinata.cloud/ipfs/QmZUgaSixivnVu43K732qhHorY8nF87zQ6G5eNvWuRqznM');
            _addPermanentTokenURI(token_id, 'http://arweave.net/UAkXjN8vQKhU2LtyVjOS-IejFWtuy7jYQl7v-cT-DBQ');
            _setPermanentTokenProof(token_id, "501c0320e31b1877e96064321f66694b639e0e66b58965498da748e73baa87af");
            token_id++;
        }
    }

    function mint(address[] calldata _owners) external onlyOwner {
        require(!lock, "Soldout");

        uint256 token_id = totalSupply() + 1;

        for (uint256 i = 0; i < _owners.length; i++) {
            legendary[token_id] = false;
            _safeMint(_owners[i], token_id);
            _addPermanentTokenURI(token_id, 'https://gateway.pinata.cloud/ipfs/QmaNDZpDqXmZztaqGpDwyrzJ7FY4v6jrkye74hkPTZBd4m');
            _addPermanentTokenURI(token_id, 'http://arweave.net/YPpM6oVL5YsnvYrjBxD9ZmYTSWGNY-Jm3x8vdz2v8wE');
            _setPermanentTokenProof(token_id, "b3467f1dbda8c6dc886d1d355a72f9fe4dd22944181f4efd104cbbf01789bfce");
            token_id++;
        }
    }
