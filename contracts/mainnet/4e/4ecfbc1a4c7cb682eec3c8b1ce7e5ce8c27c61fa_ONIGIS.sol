...
contract ONIGIS is ERC721r {
    ...
    uint256 public constant vipIds = 111;
    ...
    function purchase(uint256 numOfNft) external payable{
        ...
        if(!isVip[msg.sender]){
            if(totalSupply() + numOfNft > 1000) revert MaxSupplyReached();
        }
        uint256 fee = numOfNft * price;
        _mintRandom(msg.sender, numOfNft);
        (bool success, ) = payable(teamVault).call{value: fee}("");
    ...
    function rescueETH() external onlyOwner{
        (bool success, ) = payable(teamVault).call{value: address(this).balance}("");
        if(!success) revert FailedForward();
    }
}