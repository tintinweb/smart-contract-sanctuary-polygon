...
contract Marketplace is Initializable, Pausable, AccessControlUpgradeable {
    ...
    function initialize(address _nft) public initializer {
        ...
        foundationFee = 200; // 2% to foundationAddress (only initial sale)
        adminFee = 800; // 8% to adminAddress on initial sale

        adminFeeOnResell = 250; // 2.5% to adminAddress on resell
        creatorFee = 750; // 7.5% creator (init artist) on resell (from this 7.5% we also pay collaborators fees)
