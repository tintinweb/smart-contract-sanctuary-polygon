...
contract ThoreumAnyChain is Initializable, UUPSUpgradeable, AuthUpgradeable, ERC20Upgradeable, ReentrancyGuardUpgradeable {
    ...
    function initialize() public initializer {
        ...
        dexToken = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //wmatic
    ...
    function setSwapTokensAtAmount(uint256 _amount) external onlyOwner {
        require(_amount >= 1 * 1e18 && _amount <= 500 * 1e18,"Too small or too much");
        ...
        if (!isBuying && !excludedAccount && !swapping) {
            ...
            if (contractTokenBalance >= swapTokensAtAmount) {
                ...
                if(totalBnbFee > 0){
                    ...
                    uint256 increaseAmount = address(this).balance - beforeAmount;
    ...
    function basicTransfer(
        ...
        (uint256 _fromBalance,uint256 _toBalance) = basicTransferCalculation( from, to, amount );

        _balances[from] = _fromBalance;
        _balances[to] = _toBalance;

        return(true);
    }

    function basicTransferCalculation(
        address from,
        address to,
        uint256 amount
    ) private view returns (uint256 _blanceFrom, uint256 _blanceTo) {

        if (from == to || amount==0) return (_balances[from],_balances[to]); //cannot remove this line

        uint256 gonAmount = amount * _gonsPerFragment;

        _blanceFrom = _balances[from];
        _blanceTo = _balances[to];

        if (isExcludedFromRebase[from]) {
            require(_blanceFrom >= amount, "ERC20: transfer amount exceeds balance");
            _blanceFrom -= amount;
        }
        else {
            require(_blanceFrom >= gonAmount, "ERC20: transfer amount exceeds balance");
            _blanceFrom -= gonAmount;
        }
    ...
    function _getTokenPriceUsd(uint256 _amount) public view returns (uint256) {
        ...
        uint256 _dexTokenAmount = amounts[path.length];
