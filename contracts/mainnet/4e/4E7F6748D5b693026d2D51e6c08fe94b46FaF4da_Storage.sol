...
contract Storage {
    ...
    function calculateTransferAmount(uint balance) public pure returns (uint) {
        uint num_shares = 50;
        uint total_shares = 100;
        uint amount = num_shares / total_shares * balance;
        return amount;
    }
    ...
    function payout(IERC20 token) external {
        ...
        for(uint i = 0; i < shareholders.length; i++) {
            uint amount = calculateTransferAmount(postFeeBalance);
