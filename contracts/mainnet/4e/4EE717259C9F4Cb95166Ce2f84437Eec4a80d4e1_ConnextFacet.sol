...
contract ConnextFacet is ReentrancyGuard {
    using SafeERC20 for IERC20;

    IConnext private immutable connext;
    IWrappedEth private immutable weth;

    constructor(IConnext _connext, IWrappedEth _weth) {
        connext = _connext;
        weth = _weth;
    }

    function bridgeToConnext(ConnextDescription memory _cnDesc) external payable nonReentrant {
        LibPlexusUtil._isTokenDeposit(_cnDesc.srcToken, _cnDesc.amount);
        _cnDesc.amount = LibPlexusUtil._fee(_cnDesc.srcToken, _cnDesc.amount);
        _connextStart(_cnDesc);
    }

    function swapAndBridgeToConnext(SwapData calldata _swap, ConnextDescription memory _cnDesc) external payable nonReentrant {
        _cnDesc.amount = LibPlexusUtil._fee(_cnDesc.srcToken, LibPlexusUtil._tokenDepositAndSwap(_swap));
        _connextStart(_cnDesc);
    }

    function _connextStart(ConnextDescription memory _cnDesc) internal {
        bool isNotNative = !LibPlexusUtil._isNative(_cnDesc.srcToken);
        if (isNotNative) {
            IERC20(_cnDesc.srcToken).safeApprove(address(connext), _cnDesc.amount);
            connext.xcall(
                _cnDesc.domainId,
                _cnDesc.recipient,
                _cnDesc.srcToken,
            ...
            IERC20(address(weth)).safeApprove(address(connext), _cnDesc.amount);
            _cnDesc.srcToken = address(weth);
            connext.xcall(
                _cnDesc.domainId,
                _cnDesc.recipient,
                _cnDesc.srcToken,
        ...
        emit LibData.Bridge(msg.sender, _cnDesc.dstChainId, _cnDesc.srcToken, _cnDesc.amount, "Connext");
    }
}
...
library LibData {
    ...
    event Bridge(address user, uint64 chainId, address srcToken, uint256 fromAmount, string bridge);
