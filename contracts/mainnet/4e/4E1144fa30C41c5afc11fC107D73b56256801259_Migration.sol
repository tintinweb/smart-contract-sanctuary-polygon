...
contract Migration is OwnableUpgradeable, PausableUpgradeable {
    ...
    mapping(address => uint256[]) internal trollMigratedPlayers;
    ...
    function migrateTroll(uint256 _tokenId) public {
        ...
        trollMigratedPlayers[_msgSender()].push(_tokenId);

        // Mint a new token for the sender
        trollContract.migrationMint(_tokenId, _msgSender());
    }
    ...
    function getMigratedTrolls(address _owner) public view returns (uint256[] memory) {
        return trollMigratedPlayers[_owner];
    }
...
interface ITroll {
    function migrationMint(uint256 _tokenId, address _receiver) external;
}
