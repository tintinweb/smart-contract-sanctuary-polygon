...
contract ShipFuser is Managable, Pausable, ReentrancyGuard {
    ...
    event EarlyClaimFuseAmounts (uint256 indexed _earlyClaimId, address indexed _owner, uint256 _oilClaim, uint256 _bitsClaim);
    ...
    function fuseWithEarlyClaim(
        ...
        if (_rewarder1.addr == BITS_ADDRESS){
            if(_earlyClaim.parts.length == 1){
                emit EarlyClaimFuseAmounts(_earlyClaim.id, msg.sender, 0, _earlyClaim.parts[0].amountClaim);
            } else {
                emit EarlyClaimFuseAmounts(_earlyClaim.id, msg.sender, _earlyClaim.parts[1].amountClaim, _earlyClaim.parts[0].amountClaim);
            }
        } else {
            if(_earlyClaim.parts.length == 1){
                emit EarlyClaimFuseAmounts(_earlyClaim.id, msg.sender, _earlyClaim.parts[0].amountClaim, 0);
            } else {
                emit EarlyClaimFuseAmounts(_earlyClaim.id, msg.sender, _earlyClaim.parts[0].amountClaim, _earlyClaim.parts[1].amountClaim);                
            }
        }
