...
contract LOKGoods is ERC721Enumerable, Membership {
    event Mint(address indexed to, uint256 indexed id);

    constructor(string memory name, string memory symbol) ERC721(name, symbol) {//임시
        setBaseURI("https://lok-nft.leagueofkingdoms.com/api/goods/");
    }
    ...
    function mints(address[] memory tos, uint256[] memory tokenIds) public onlyMinter {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            mint(tos[i], tokenIds[i]);
        }
    }
    function mintsTo(address to, uint256[] memory tokenIds) public onlyMinter {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            mint(to, tokenIds[i]);
        }
    }
    ...
    function burns(uint256[] memory tokenIds) public onlyMinter  {
        for (uint256 i = 0; i < tokenIds.length; ++i) {
            _burn(tokenIds[i]);
        }
    }
