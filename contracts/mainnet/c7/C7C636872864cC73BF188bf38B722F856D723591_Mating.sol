...
contract Mating is OwnableUpgradeable {
    ...
    //upgrade
    mapping(address => uint) public referBox;
    mapping(address => uint) public referMatting;
    uint public mattingExp;
    ...
    function setMattingExp(uint exp_) external onlyOwner {
        mattingExp = exp_;
    }
    ...
    function checkMattingReward(address addr) internal {
        ...
        referMatting[invitor]++;
        if (referMatting[invitor] >= 5) {
            item.mint(invitor, 15, 1);
            referMatting[invitor] = 0;
    ...
    function mating(uint myTokenId, uint targetTokenID) external {
        ...
        BVG.safeTransferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), bvgCost);
        BVT.safeTransferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), bvtCost);
        stable.addStableExp(msg.sender, mattingExp);
        ...
        if (userUploadList[rec].length > 0 && _index != 99) {
    ...
    function coutingCost(address addr, uint tokenId) public view returns (uint bvg_, uint bvt_){
        ...
        uint _matingTime = matingTime[tokenId] - excessTimes[tokenId];
        return (mattingCostBVG[_matingTime] * rate / 100, mattingCostBVT[_matingTime] * rate / 100);
    }
