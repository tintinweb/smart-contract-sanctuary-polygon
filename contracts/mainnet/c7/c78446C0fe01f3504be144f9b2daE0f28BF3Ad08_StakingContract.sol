...
contract StakingContract is AccessControl, Multicall {

    using SafeERC20 for IERC20;

    IAaveLendingPoolAddressesProvider public iAaveLendingPoolAddressesProvider;
    IAaveLendingPool public iAaveLendingPool;
    ...
    mapping(IERC20 => IERC20) public lookUpUnderlying; // bearing <> underlying
    mapping(IERC20 => IERC20) public lookUpAtoken; // underlying <> bearing

    modifier sync(IERC20 toSync) {
        require(whitelistedTokens[toSync] == true, "token not supported by protocol");
        if (lookUpUnderlying[toSync] != IERC20(address(0x0))) {
            toSync = lookUpUnderlying[toSync];
        }
        IERC20 _aToken = toSync;
        if (lookUpAtoken[toSync] != IERC20(address(0x0))) {
            _aToken = lookUpAtoken[toSync];
        }

        uint8 decimalsAtoken = IERC20Extended(address(_aToken)).decimals();
        uint8 decimalsUtoken = IERC20Extended(address(toSync)).decimals();

        if (_aToken.balanceOf(address(this)) * 10 ** decimalsUtoken > totalTokensDeposited[toSync] * 10 ** decimalsAtoken) {
            uint256 _surplus = (_aToken.balanceOf(address(this)) * 10 ** decimalsUtoken - totalTokensDeposited[toSync] * 10 ** decimalsAtoken) / 10 ** decimalsAtoken;
    ...
    modifier updateDebt(IERC20 toSync, address toUpdate) {
        ...
        if (deposited[toUpdate][toSync] > 0) {
            uint256 reduction = (deposited[toUpdate][toSync] * (block.timestamp - lastDebtInteraction[toUpdate][toSync])) / 86400;
    ...
    constructor(IERC20 _shardToken, uint256 _mdm, address provider) {
        shardToken = _shardToken;
        _setupRole(ADMIN_ROLE, _msgSender());
        _setRoleAdmin(ADMIN_ROLE, ADMIN_ROLE);
        maxDebtModifier = _mdm;
        lendingPoolAddressesProvider = provider;
        iAaveLendingPoolAddressesProvider = IAaveLendingPoolAddressesProvider(lendingPoolAddressesProvider);
        referral = 0;
    ...
    function deposit(IERC20 toDeposit, uint256 amount) external sync(toDeposit) updateDebt(toDeposit, _msgSender()) {
        ...
        if (lookUpUnderlying[toDeposit] != IERC20(address(0x0))) {
            toDeposit = lookUpUnderlying[toDeposit];
        }
        else { // convert asset to it's yield bearing form using Aave protocol
            address lendingPool = iAaveLendingPoolAddressesProvider.getLendingPool();
            iAaveLendingPool = IAaveLendingPool(lendingPool);
            toDeposit.approve(lendingPool, amount);
            iAaveLendingPool.deposit(address(toDeposit), amount, address(this), referral);
        }
    ...
    function withdraw(IERC20 toWithdraw, uint256 amount) external updateDebt(toWithdraw, _msgSender()) {
        ...
        if (lookUpUnderlying[toWithdraw] != IERC20(address(0x0))) {
            bookkeeping = lookUpUnderlying[toWithdraw];
        }
        ...
        if (toWithdraw == bookkeeping) {
            address lendingPool = iAaveLendingPoolAddressesProvider.getLendingPool();
            iAaveLendingPool = IAaveLendingPool(lendingPool);
            iAaveLendingPool.withdraw(address(toWithdraw), amount, address(this));
        }
    ...
    function lockPartlyAndReceive(IERC20 tokenToLock, uint256 shardsToCredit) external updateDebt(tokenToLock, _msgSender()) {
        if (lookUpUnderlying[tokenToLock] != IERC20(address(0x0))) { 
            tokenToLock = lookUpUnderlying[tokenToLock];
        }
    ...
    function harvestSurplus(IERC20 lockedToken) external updateDebt(lockedToken, _msgSender()) {
        if (lookUpUnderlying[lockedToken] != IERC20(address(0x0))) { 
            lockedToken = lookUpUnderlying[lockedToken];
        }
    ...
    function repayPartlyAndUnlock(IERC20 tokenToUnlock, uint256 amount) external updateDebt(tokenToUnlock, _msgSender()) {
        if (lookUpUnderlying[tokenToUnlock] != IERC20(address(0x0))) { 
            tokenToUnlock = lookUpUnderlying[tokenToUnlock];
        }
    ...
    function getIncentives(address controller, address recipient, address[] calldata assets) onlyRole(ADMIN_ROLE) external {
        iAaveIncentivesController = IAaveIncentivesController(controller);
        uint256 balance = iAaveIncentivesController.getRewardsBalance(assets, address(this));
        iAaveIncentivesController.claimRewards(assets, balance, recipient);
    }

    function whitelistToken(IERC20 aToken, IERC20 underlyingAsset) onlyRole(ADMIN_ROLE) external {
        whitelistedTokens[aToken] = true;
        whitelistedTokens[underlyingAsset] = true;
        lookUpUnderlying[aToken] = underlyingAsset;
        lookUpAtoken[underlyingAsset] = aToken;
    }
