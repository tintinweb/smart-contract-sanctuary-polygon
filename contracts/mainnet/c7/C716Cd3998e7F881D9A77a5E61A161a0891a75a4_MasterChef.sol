...
// The Axo Garden is a fork of MasterChef by SushiSwap
...
// will be transferred to a governance smart contract once AXO is sufficiently
...
contract MasterChef is Ownable {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of AXOs
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accAXOPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accAXOPerShare` (and `lastRewardBlock`) gets updated.
    ...
    // set a max axo per second, which can never be higher than 1 per second
    uint256 public constant maxAxoPerBlock = 1e18;
    ...
    function pendingAXO(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            uint256 booReward = multiplier.mul(axoPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accAXOPerShare = accAXOPerShare.add(booReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        axo.mint(devaddr, booReward.div(10));
        axo.mint(address(this), booReward);

        pool.accAXOPerShare = pool.accAXOPerShare.add(booReward.mul(1e12).div(lpSupply));
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function safeAXOTransfer(address _to, uint256 _amount) internal {
        uint256 booBal = axo.balanceOf(address(this));
        if (_amount > booBal) {
            axo.transfer(_to, booBal);
        } else {
...
contract AxoToken is ERC20Permit, Ownable {

    constructor() ERC20("AxoToken", "AXO") ERC20Permit("AxoToken")
    {

    }

    uint256 constant private _maxTotalSupply = 25000000e18;
