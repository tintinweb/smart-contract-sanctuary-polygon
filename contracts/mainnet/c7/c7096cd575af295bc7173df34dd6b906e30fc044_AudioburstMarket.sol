...
contract AudioburstNFT is ERC721 {
    ...
    address public collection_creator;
    ...
    event CollectionCreatorUpdated(address newCreator);
	 ...
	 */
    function setCollectionURI(string memory newURI) external collectionCreator {
        collection_uri = newURI;
        emit CollectionUriUpdated(newURI);
    }
    function setName(string memory newname) external collectionCreator {
        collection_name = newname;
        emit CollectionNameUpdated(newname);
    }
    function setPublic(bool bPublic) external collectionCreator {
        isPublic = bPublic;
	 ...
	 */
    function addItem(string memory _tokenURI, uint256 royalty) external collectionCreator returns (uint256) {
    ...
    function setTokenURI(uint256 _tokenId, string memory _newURI) external creatorOnly(_tokenId)
    {
    ...
    /**
     * @dev Require _msgSender() to be the collection creator
     */
    modifier collectionCreator() {
        require(collection_creator == _msgSender(), "The caller is unauthorized!");
        _;
    }
    /**
    ...
    modifier creatorOnly(uint256 _id) {
        require(
            Items[_id].creator == _msgSender(), "ERC721Tradable#creatorOnly: ONLY_CREATOR_ALLOWED"
        );
        _;
    }
    // transfer collectoin creator
    function transferCreator(address _newCreator) external collectionCreator {
        collection_creator = _newCreator;
        emit CollectionCreatorUpdated(_newCreator);
    }
}
...
contract AudioburstMarket is Ownable, ERC721Holder {
    using SafeMath for uint256;
    using Address for address;
    ...
    event SetWhitelist(address account, bool value);

    // governance token for Polygon: 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619
    // governance token for Mumbai: 0x879bad9DcD7e7f79B598a632103984FC090DA00D (Test Token TT from WERT)
    constructor () {
        governanceToken = IERC20(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
        swapFeeAddress = msg.sender;
        whitelist[msg.sender] = true;
    }

    function setFee(uint256 _swapFee, address _swapFeeAddress) external onlyOwner {
        swapFee = _swapFee;
        swapFeeAddress = _swapFeeAddress;
    }

    function createCollection(string memory _name, string memory _uri, bool bPublic) external isWhitelist returns(address collection) {
        bytes memory bytecode = type(AudioburstNFT).creationCode;
        ...
        IAudioburstNFT(collection).initialize(_name, _uri, msg.sender, bPublic);
        collections.push(collection);
        emit CollectionCreated(collection, msg.sender, _name, _uri, bPublic);
    }

    function list(address _collection, uint256 _token_id, uint256 _price) OnlyItemOwner(_collection, _token_id) external {
        require(_price > 0, "invalid price");
        IAudioburstNFT nft = IAudioburstNFT(_collection);
        ...
        pairs[currentPairId].royalty = nft.royalties(_token_id);
		pairs[currentPairId].owner = msg.sender;
		pairs[currentPairId].price = _price;
        pairs[currentPairId].bValid = true;

        emit ItemListed(currentPairId, _collection, _token_id, _price, pairs[currentPairId].creator, msg.sender, pairs[currentPairId].royalty);
    }
    ...
    function delist(uint256 _id) external {
        ...
        IAudioburstNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pairs[_id].token_id);
    ...
    function buy(uint256 _id) external ItemExists(_id) {
        ...
        uint256 creatorAmount = totalAmount.mul(pair.royalty).div(PERCENTS_DIVIDER);
        uint256 sellerAmount = totalAmount.sub(feeAmount).sub(creatorAmount);
        // transfer governance token to fee address
        require(governanceToken.transferFrom(msg.sender, swapFeeAddress, feeAmount), "failed to transfer Admin fee");
        // transfer governance token to creator as royalty
        require(governanceToken.transferFrom(msg.sender, pair.creator, creatorAmount), "failed to transfer creator fee");
        // transfer governance token to owner
        require(governanceToken.transferFrom(msg.sender, pair.owner, sellerAmount), "failed to transfer to owner");
        
        // transfer NFT token to buyer
        IAudioburstNFT(pairs[_id].collection).safeTransferFrom(address(this), msg.sender, pair.token_id);
        pairs[_id].bValid = false;
    ...
    function buyWithCard(address buyer, uint256 _id) external payable ItemExists(_id) {
        require(pairs[_id].bValid, "invalid Pair id");
        require(pairs[_id].owner != buyer, "owner can not buy");
        ...
        // transfer NFT token to buyer
        IAudioburstNFT(pairs[_id].collection).safeTransferFrom(address(this), buyer, pair.token_id);
        
        pairs[_id].bValid = false;
        totalEarning = totalEarning.add(totalAmount);
        totalMarketCount = totalMarketCount.add(1);
        
        emit Swapped(buyer, pair);
    }

    modifier OnlyItemOwner(address _collection, uint256 _tokenId) {
        IAudioburstNFT collectionContract = IAudioburstNFT(_collection);
    ...
    modifier isWhitelist() {
        require(whitelist[msg.sender], "Caller is unauthorized to create a collection");
        _;
    }
    function setWhitelist(address _account, bool _value) external onlyOwner {
        whitelist[_account] = _value;
        emit SetWhitelist(_account, _value);
    }
}