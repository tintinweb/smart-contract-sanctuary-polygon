...
// File: contracts/NewtestTranzacto.sol
...
using SafeERC20 for IERC20;
    ...
    struct sellerTranzacto{
        address sellerID;
        string sellerName;
        string sellerAddressLa;
        string sellerAddressLo;
        uint256 salesMade;
    }
    ...
    sellerTranzacto[] public USDTsellers;
    ...
    mapping(address => USDTrequests) public USDTrequest;

/* Events */
   event changeOwner(address OldOwner, address NewOwner);
   event returnMatic(address To, uint256 Amount);
   event ERC20Withdraw(address To, uint256 Amount);
   event sellCreated(address Buyer, uint256 Invoice);
   event paidFor(uint256 Invoice, string Item, IERC20 Currency, uint256 Amount);
   event invoiceRejected(uint256 Invoice, address Seller, address Buyer);
   event USDTrequested(address Buyer, address Seller, uint256 Amount);
   event USDTexchanged(address Sender, address Receiver, uint256 Amount);
   event requestDeclined(address Seller, address Buyer);
...
/*. @dev: Change the contract owner */
    ...
    function setBurnPercentage(uint256 _newPercentage) external onlyOwner{
        toBurn = _newPercentage;
    }
    function buyERR(address _user, uint256 _amount, bool _isUSDT) public {
        uint256 slippage = 1;
        if(_isUSDT != true){
           USDT.transferFrom(_user, address(this), _amount); 
        }
        (uint256 receivedQuoteAmount,) = IDODOV2(dodoV2Pool).querySellBase(_user, _amount);
        uint256 minReturnAmount = receivedQuoteAmount.mul(100 - slippage).div(100); 
        address[] memory dodoPairs = new address[](1); //one-hop
        dodoPairs[0] = dodoV2Pool;
        uint256 directions = 0;
        uint256 deadline = block.timestamp + 60 * 10;
        _generalApproveMax(address(USDT), dodoApprove, _amount);
        uint256 returnAmount = IDODOProxy(dodoProxy).dodoSwapV2TokenToToken(
            address(USDT),
            address(ERR),
            _amount,
        ...
        ERR.safeTransfer(deadAddress, returnAmount);
    }
    function addSeller(string memory _name, string memory _addressLa, string memory _addressLo, address _seller, uint256 pA, bool _receiveERR) external onlyOwner{
        approvedSeller[_seller].sellerName = _name;
        approvedSeller[_seller].sellerAddressLa = _addressLa;
        approvedSeller[_seller].sellerAddressLo = _addressLo;
    ...
    function addUSDTseller(string memory _name, string memory _addressLa, string memory _addressLo, address _seller) external onlyOwner{
        USDTsellers.push(sellerTranzacto(_seller, _name, _addressLa, _addressLo, 0));
        isSeller[_seller] = true;
    }
    function getUSDTsellers() public view returns (sellerTranzacto[] memory){
        return USDTsellers;
    }
    ...
    function forceRequest(address _requester) external onlyOwner{
        USDTrequest[_requester].status = true;
    }
    //called by user who want to buy USDT for cash
    function requestUSDT(address _seller, uint256 _amount) external{
        if(USDTrequest[msg.sender].requestID > 0){
            require(USDTrequest[msg.sender].status == true, "you have a pending request");
        }
        require(isSeller[_seller] == true, "not authorized seller");
        USDTrequest[msg.sender].seller = _seller;
        USDTrequest[msg.sender].amount = _amount;
        USDTrequest[msg.sender].status = false;
        USDTrequest[msg.sender].requestID++;
        emit USDTrequested(msg.sender, _seller, _amount);
    }
    //called by seller to deposit USDT for sale in tranzacto contract
    function depositUSDT(uint256 _amount) external{
        require(isSeller[msg.sender] == true, "not authorized seller");
        //require approval from USDT smart contract
        USDT.transferFrom(msg.sender, address(this), _amount);
        //USDT.transfer(feeReceiver, fees);
        balanceUSDT[msg.sender] += _amount;//.sub(fees);
        amountUSDT += _amount;//_amount.sub(fees);
    }
    //called by seller to withdraw his/her USDT from tranzacto contract
    function withdrawUSDT(uint256 _amount) external{
        require(isSeller[msg.sender] == true, "not authorized seller");
        require(balanceUSDT[msg.sender] >= _amount, "not enough balance");
        amountUSDT -= _amount;
        balanceUSDT[msg.sender] -= _amount;
        USDT.transfer(msg.sender, _amount);
    }
    //called by seller to approve request and send USDT to buyer
    function exchangeUSDT(address _receiver) external nonReentrant{
        uint256 _amount = USDTrequest[_receiver].amount;
        uint256 _toBurn;
        if(toBurn != 0){
            _toBurn = fees.mul(toBurn).div(100);
            buyERR(msg.sender, _toBurn, true);
        } 
        require(isSeller[msg.sender] == true, "not authorized seller");
        require(USDTrequest[_receiver].seller == msg.sender, "sale not approved");
        require(balanceUSDT[msg.sender] >= _amount,"over balance");
        balanceUSDT[msg.sender] -= _amount;
        amountUSDT -= _amount;
        USDT.transfer(_receiver, _amount.sub(fees));
        USDT.transfer(feeReceiver, fees.sub(_toBurn));
        USDTrequest[_receiver].status = true;
        USDTrequest[_receiver].successCount++;
        emit USDTexchanged(msg.sender, _receiver, _amount);
    }
    //called by seller to decline the sale of USDT
    function declineUSDT(address _requester) external{
        require(isSeller[msg.sender] == true, "not authorized seller");
        require(USDTrequest[_requester].seller == msg.sender, "you are not the seller");
        USDTrequest[_requester].seller = address(0);
        USDTrequest[_requester].amount = 0;
        USDTrequest[_requester].status = true;
        emit requestDeclined(msg.sender, _requester);
    }
    // First the seller creates order for the buyer
    function createSell(address _buyer, string memory _what, IERC20 _currency, uint256 _amount) isCurrency(_currency) external nonReentrant returns(uint256 _invoice){
        require(isSeller[msg.sender] == true, "Seller not Approved");
        invoice ++;
        paying[_buyer][invoice].invoice = invoice;
        ...
        emit sellCreated(_buyer, invoice);
        return(invoice);
    }
    // The buyer get the invoice number from the seller then pays for the seller
    function payFor(uint256 _invoice) external nonReentrant{
        ...
        uint256 _toBurn = fees.mul(toBurn).div(100);
        ...
        if(feeAddress.balanceOf(msg.sender) < fees){
                ERR.transferFrom(msg.sender, deadAddress, feesInERR);
        }
        else{
            buyERR(msg.sender, _toBurn, false);
            feeAddress.transferFrom(msg.sender, feeReceiver, fees.sub(_toBurn));
        }
    ...
    function rejectPay(uint256 _invoice) external{
        require(paying[msg.sender][_invoice].isPending == true, "Invoice is ethier paid or rejected");
        address _seller = paying[msg.sender][_invoice].seller;
        paying[msg.sender][_invoice].isPending = false;
        paying[msg.sender][_invoice].status = "Rejected";
        emit invoiceRejected(_invoice , _seller, msg.sender);
    }
    //called by authorized seller to withdraw monthly amount
...
/*  @dev: Withdrwa Polygon ERC20 tokens!*/
    function withdrawalERC20(IERC20 _token, address _to, uint256 _amount, uint8 decimal) external onlyOwner() {
        uint256 dcml = 10 ** decimal;
        uint256 amount = _amount*dcml;
        if(_token == ERR){
            uint256 allowedAmount = _token.balanceOf(address(this)).sub(amountERR);
            require(allowedAmount >= amount, "can only withdraw what is not for sellers");
        }
        else if(_token == USDT){
            uint256 allowedAmount = _token.balanceOf(address(this)).sub(amountUSDT);
            require(allowedAmount >= amount, "can only withdraw what is not for sellers");
        }
        else{
            require(_token.balanceOf(address(this)) >= amount,"Balanace"); //No token balance available
        }
        require(_to != address(0), "Zero Address");
        emit ERC20Withdraw(_to, amount);
...
/*  @dev: Withdrwa Matic token!*/
    function withdrawalMatic(uint256 _amount, uint8 decimal, address to) external onlyOwner() {
        uint256 dcml = 10 ** decimal;
        uint256 amount = _amount*dcml;
        require(address(this).balance >= amount,"Balanace"); //No matic balance available
        require(to != address(0), "Zero Address");
        
        emit returnMatic(to, amount);
