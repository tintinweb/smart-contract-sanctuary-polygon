...
import '../cva/interface/IWETH.sol';

contract SimpleTokenSwapV2 is Ownable {
  ...
  function crossChainBridge(
    ...
    if (_token == address(WETH) && IERC20(_token).balanceOf(address(this)) < _amount) {
      require(address(this).balance >= _amount, 'no eth');
    ...
    (bool success, ) = _to.call{value: nativeAmount}(txData);
    require(success);
  }
}