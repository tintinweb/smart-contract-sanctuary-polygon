...
contract DataStore is Initializable, NonblockingLzApp {
    struct Message {
        uint256 id;
        address vault;
        uint256 balance;
    ...
    mapping(address => Message) public childMessageOnChain; // Latest message of child onchain
    ...
    bytes[] public onChainMessages;

    event OnChainMessageReceived(uint16 _srcChainId, address _srcAddress, uint64 _nounce, bytes message);
    event OnChainVaultNotFound(address _vault, bytes message);

    modifier onlyGovernorOrOffchainManager() {
        require(msg.sender == governor() || isOffchainManager[msg.sender], "!Governor || Offchain Mgr");
        _;
    }

    function initialize(address _endpoint) external onlyGovernor initializer {
        lzEndpoint = ILayerZeroEndpoint(_endpoint);
    }

    function setEpsilon(uint256 _timeEpsilonBps, uint256 _tvlEpsilonBps, uint256 _balanceEpsilonBps) external onlyGovernor {
    ...
    function writeOffchain(address _vault, address _childVault, uint256 _balance, uint256 _tvl, uint256 _chainId, uint256 _timestamp, uint8 _source) external onlyGovernorOrOffchainManager {
        require(_vaultExist(_vault), "!vault");
        ...
        childMessagesOffChain[_childVault].push(childMessageOffChain[_childVault]);
    }

    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nounce, bytes memory _payload) internal override {
        // use assembly to extract the address from the bytes memory parameter
        address sendBackToAddress;
        assembly {
            sendBackToAddress := mload(add(_srcAddress, 20))
        }
        onChainMessages.push(_payload);
        emit OnChainMessageReceived(_srcChainId, sendBackToAddress, _nounce, _payload);

        (address[] memory _vaults, uint256[] memory _balances, uint256[] memory _tvls, uint256 _actualChainId, uint256 _recieved_on) = abi.decode(
            _payload,
            (address[], uint256[], uint256[], uint256, uint256)
        );

        for (uint256 i = 0; i < _vaults.length; i++) {
            childMessageOnChain[_vaults[i]] = Message(
                childMessageOnChain[_vaults[i]].id + 1,
                _vaults[i],
                _balances[i],
                _tvls[i],
                _actualChainId,
            ...
            childMessagesOnChain[_vaults[i]].push(childMessageOnChain[_vaults[i]]);
        }
    }
}
 ...
 */
abstract contract LzApp is Governable, ILayerZeroReceiver, ILayerZeroUserApplicationConfig {
    ...
    function setTrustedRemote(uint16 _srcChainId, bytes calldata _path) external onlyGovernor {
        trustedRemoteLookup[_srcChainId] = _path;
        emit SetTrustedRemote(_srcChainId, _path);
    }

    function setTrustedRemoteAddress(uint16 _remoteChainId, bytes calldata _remoteAddress) external onlyGovernor {
    ...
    function setPrecrime(address _precrime) external onlyGovernor {
        precrime = _precrime;
        emit SetPrecrime(_precrime);
    }

    function setMinDstGas(uint16 _dstChainId, uint16 _packetType, uint _minGas) external onlyGovernor {
