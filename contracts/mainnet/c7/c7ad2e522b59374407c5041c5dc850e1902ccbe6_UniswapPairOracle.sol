...
contract UniswapPairOracle is Ownable {
    ...
    function twap(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        uint8 _token0MissingDecimals = 24 - (ERC20(token0).decimals());
        uint8 _token1MissingDecimals = 24 - (ERC20(token1).decimals());
        if (token == token0) {
            amountOut =
    ...
    function spot(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        uint8 _token0MissingDecimals = 24 - (ERC20(_token0).decimals());
        uint8 _token1MissingDecimals = 24 - (ERC20(_token1).decimals());
        uint256 _price = 0;
        if (token == _token0) {
