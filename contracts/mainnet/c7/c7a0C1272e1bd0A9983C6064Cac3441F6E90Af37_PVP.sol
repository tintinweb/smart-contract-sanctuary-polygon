...
contract PVP is VRFConsumerBaseV2, Ownable {
  ...
  // mapping(uint => uint256[]) private s_randomWords; // TODO: to delete before going prod. Helpful for testing
  ...
  function register(uint32 _riderId, uint _raceId) external payable{
    require(msg.value == radikalStoreInstance.getPvpChainLinkFee(), "PVP: IF");
    ...
    internal override
  {
    ...
    // s_randomWords[_raceId] = randomWords; // TODO: TO BE DELETED before going prod. Helpful for testing
  ...
  // TODO: DELETE
  // function getRandoms(uint raceID, uint mod) external view returns (uint256[] memory) {
  //   if(mod == 0) {
  //     return s_randomWords[raceID];
  //   } else {
  //     uint[] memory arr = new uint[](s_randomWords[raceID].length);
  //     for(uint i = 0; i < s_randomWords[raceID].length; i++) {
  //       arr[i] = (s_randomWords[raceID][i] % mod);
  //     }
  //     return arr;
  //   }
  // }
