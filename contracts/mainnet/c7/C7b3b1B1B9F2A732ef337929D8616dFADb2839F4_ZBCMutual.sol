...
contract ZBCMutual is IMutual, RiskOrder{
    ...
    uint256[6] private levelMaxDeposit = [0,500e6,1000e6,1500e6,2000e6,2500e6];
    uint256[6] private levelMinDeposit = [0,100e6,500e6,1000e6,1500e6,2500e6];
    ...
    modifier onlyIDO() {
        require(msg.sender == sys.idoAddr, "only ido");
        _;
    }
    ...
    constructor(address usdt_, address ido_, address zbc_, address nft_, address router_){
        sys.idoAddr = ido_;
        sys.zbcAddr = zbc_;
        sys.nftAddr = nft_;
        sys.routerAddr = router_;
    ...
    function depositByBorrow(uint256 _amount, address lender_) external onlyRegister {
        ...
        bool success = usdt.transferFrom(msg.sender, address(this), payAmt);
        require(success,"transferFrom failed");
    ...
    function _withdraw(address addr_,address to_) private {
        ...
        if (pendingAmount > 0) {
            usdt.transfer(to_,pendingAmount);
        }
    }
    ...
    function _isLending(address addr,uint256 borrowAmt) private view returns(bool) {
        return IZBC(sys.nftAddr).balanceOf(addr) > 0 && lenders[addr].balance >= borrowAmt && addr != address(0);
    }

    function _isLender(address addr) private view returns(bool) {
        return IZBC(sys.nftAddr).balanceOf(addr) > 0 && addr != address(0);
    }
    ...
    function _burnZBC(address addr,uint256 amount,bool isSafe) private {
        ...
        IZBC(sys.zbcAddr).burnFrom(addr,zbcAmount);
    }
    ...
    function _unfreezeCapitalOrReward(address _userAddr, uint256 _amount, bool isBorrow) private returns(bool isUnFreeze,uint256 newAmount) {
        ...
        uint256 addFreeze = dayPerCycle + orderInfos[_userAddr].length * timeStep;
        if(addFreeze > maxAddFreeze) {
            addFreeze = maxAddFreeze;
        }
        uint256 unfreezeTime = block.timestamp + addFreeze;
        (,bool isRisk,) = userTotalRevenue(_userAddr);
        OrderInfo memory orderIn = OrderInfo(_userAddr,_amount, block.timestamp, unfreezeTime, false, isRisk);
        ...
        if (block.timestamp < order.endTime) {
            ...
            uint256 freeAmount515 = 0;
            if (_amount >= ri.freezeTotalLevel515) {
                freeAmount515 = ri.unfreezeLevel515;
            }else {
        ...
        if (!isStaticRisk && !order.isRisk) {
            ri.staticReward += _amount * realPercents / base100;
            ri.split += _amount * splitPercents / base100;
            ri.totalRevenue += _amount;
        }
    ...
    function userTotalRevenue(address _userAddr) private view returns(uint256 totalRevenue,bool isRisk,bool isStaticRisk) {
        ...
        ri.totalRevenue < ri.freezeCapitals || (!risk.riskLevelNext && user.riskNum >= risk.riskNum)) {
            isRisk = false;
        }else {
            isRisk = true;
        }
    ...
    function getLeader(address addr) external view returns(Lender memory) {
        ...
        lender.nftNums = IZBC(sys.nftAddr).balanceOf(addr);
    ...
    function getSys() external view returns(SysInfo memory) {
        ...
        sy.usdtAddr = address(usdt);
        sy.balance = usdt.balanceOf(address(this));
    ...
    function USDTToZBCAmount(uint256 _amount) public view returns(uint256){
        ...
        path[0] = sys.usdtAddr;
        path[1] = sys.zbcAddr;
        return IZBC(sys.routerAddr).getAmountsOut(_amount,path)[1];
    }
    ...
    function getLender10() external view returns(address[] memory lenders10) {
        ...
        for (uint i=0; i<lenderArr.length; i++) {
            Lender memory lender = Lender({
                addr:  lenderArr[i] ,
                nftNums: IZBC(sys.nftAddr).balanceOf(lenderArr[i]),
                isLender: false,
                balance: 0
            });
            arr[i] = lender;
