...
contract BulkGrinder is Ownable {
    ...
    function bulkGrind (uint256[] memory itemIDs, IGrindHelper.Ingredient[] memory ingredients) external {
        ...
        for (uint i = 0; i < resources.length; i ++) {
            uint256 profit = IERC20(resources[i]).balanceOf(address(this)).sub(balances[i]);
            uint256 taxes = profit.mul(1000).div(100);
            uint256 leftover = profit.sub(taxes);
            IERC20(resources[i]).transferFrom(address(this), msg.sender, leftover);
            IERC20(resources[i]).transferFrom(address(this), owner(), taxes);
        }
    } 
