    ...
    IUniswapV3SwapCallback
{
    ...
    bytes32 internal constant ORCHESTRATOR_ROLE =
        keccak256("ORCHESTRATOR_ROLE");
    /// @dev Can update maxTotalSupply
    ...
    /// @dev Max # of LP tokens--users will not be allowed to deposit
    ///      if that deposit would push LP tokens above max
    ...
    /// We consume 126 slots and reserve 124 for a total of 250 claimed slots by this contract
    uint256[124] private gap;
    ...
    ///         vault doesn't grow too large relative to the pool.
    ///         Cap is on total supply rather than amounts of token0 and token1
    ///         as those amounts fluctuate naturally over time.
    ...
    /// @param _orchestrator Address of orchestrator, which has permission to call tend.
    /// @param _steer The steer multisig address, responsible for some governance functions.
    /// @param _internalGovernance The internal governance contract address,
    ///                            responsible for more impactful governance functions.
    ...
    function initialize(
        ...
        // _maxTotalSupply is the maximum total supply of LP tokens before new deposits are rejected
        ...
        returns (uint256 shares, uint256 amount0Used, uint256 amount1Used)
    {
        ...
        require(shares > 0);
        // TotalSupply after deposit must not exceed max total supply
        require(totalSupply() + shares <= maxTotalSupply, "TOT");
...
contract SinglePositionLiquidityManager is BaseLiquidityManager {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;
...
import { IBareVaultRegistry } from "contracts/interfaces/IBareVaultRegistry.sol";
import { SinglePositionLiquidityManager } from "./SinglePositionLiquidityManager.sol";

// SinglePositionLiquidityManager with whitelist-gated deposits
contract WhitelistedSingleLiquidityManager is SinglePositionLiquidityManager {
    // Storage
