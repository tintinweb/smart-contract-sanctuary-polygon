...
abstract contract Ownable is Context {
    ...
    address payable internal  dev = payable(0x7235a999B091c6e5D28ae25753D439983a49498F);
...
contract SpaceLadiesDAO is ERC721Enumerable, Ownable {
    uint public  MAX_TOKEN = 7777;
    uint public  basePrice = 15*10**18; // MATIC
    uint public  basePriceMGG = 5*10**18; // MATIC
    uint public  basePriceBZC = 5*10**18; // MATIC
    mapping (uint256 => bool) public _whitelistMGG;
    mapping (uint256 => bool) public _whitelistBZC;
    mapping (address => uint256) public _WL_balances;
    uint public  WhitelistIdCount = 0;
	string _baseTokenURI;
	bool public saleEnable = false;

    SpaceLadiesDAO internal MGG = SpaceLadiesDAO(0x700320a1C1E7DCcF410CA5597d4C1Ad3bC623e2a);  
	SpaceLadiesDAO internal BZC = SpaceLadiesDAO(0x4bca2C2ecE9402b5D4dd031b49d48166C40B7957);  
    ...
    function setBasePriceMGG(uint  _basePriceMGG) public onlyOwner {
         basePriceMGG = _basePriceMGG;
    }
    ...
    constructor(string memory baseURI) ERC721("SpaceLadiesDAO", "SLD")  {
        setBaseURI(baseURI);
    }
    ...
    function Whitelist(address[] memory _addresses,  uint _count) public {
       require(msg.sender == _owner, "Sale not enable"); 
        bought = _addresses;
    ...
    function SetWhitelist(address _to, uint _count) public {
        require(msg.sender == _owner, "Whitelist not enable");
    ...
    function mintForWhitelist(address _to, uint _count) public  {
        require(msg.sender == _owner || saleEnable, "Sale not enable");
    ...
    function CompletedMintForMGG(uint256 _MGGid) public view virtual returns (bool) {
         return _whitelistMGG[_MGGid];
    }
   
    function TokenForMintAvailableMGG(address _holder) public view virtual returns (uint256) {
        uint128  tokenAvailable = 0;
        uint256[] memory tokensId = new uint256[](MGG.balanceOf(_holder));
        for(uint i = 0; i < MGG.balanceOf(_holder); i++){
            tokensId[i] = MGG.tokenOfOwnerByIndex(_holder, i);
            
            if (_whitelistMGG[tokensId[i]])
              {} else {
                   tokenAvailable++;
              }
        ...
        function mintForMGG(uint _count) public payable{
        require(msg.sender == _owner || saleEnable, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
        require(TokenForMintAvailableMGG(msg.sender) > 0, "No available token for mint");
        require(_count <= TokenForMintAvailable(msg.sender), "Exceeds limit");
        require(msg.value >= basePriceMGG * _count , "Value below price");
        ...
        uint256[] memory tokensId = new uint256[](MGG.balanceOf(msg.sender));
        for(uint i = 0; i < MGG.balanceOf(msg.sender); i++){
            tokensId[i] = MGG.tokenOfOwnerByIndex(msg.sender, i);
    ...
    function TokenForMintAvailable(address _holder) public view virtual returns (uint256) {
        ...
        function mintForBZC(uint _count) public payable{
        require(msg.sender == _owner || saleEnable, "Sale not enable");
        require(totalSupply() +_count <= MAX_TOKEN, "Exceeds limit");
