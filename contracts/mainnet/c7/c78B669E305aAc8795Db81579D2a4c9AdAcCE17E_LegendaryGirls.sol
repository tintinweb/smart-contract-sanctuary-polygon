...
contract LegendaryGirls is ERC721Enumerable, Ownable {
    ...
    // Current price.
    uint256 public CURRENT_PRICE = 123 * 10 **18;
     ...
     */
    constructor()
        ERC721("LegendaryGirls", "LegendaryGirls")
    {}
     ...
     * Pause sale if active, make active if paused
     */
    function changePrivateSaleStatus(bool newState) public onlyOwner {
        isWhitelistSale = newState;
    }

    function whitelistAccount(address[] memory accounts, bool status) public onlyOwner
    {
        for(uint256 index =0;index<accounts.length;index++)
        {
         whitelist[accounts[index]] = status; 
        }
    }


    /**
     * GiveAway LegendaryGirls
     */
    function giveAway(address account,uint256 numberOfTokens) public payable onlyOwner {
        require(
            totalSupply().add(numberOfTokens) <= MAX_TOKENS,
            "Purchase would exceed max supply of LegendaryGirls"
        );
        ...
        for (uint256 i = 1; i <= numberOfTokens; i++) {
            ...
            if (tokenId <= MAX_TOKENS) {
                ...
                emit TokenMinted(tokenId, account, first_encounter);
            }
        }
    }
     ...
     * Mint LegendaryGirls
     */
    function mintLegendaryGirls(uint256 numberOfTokens) public payable {
        ...
        if(isWhitelistSale)
        {
            require(whitelist[msg.sender],"You are Not WhiteListed, Please Contact to Owner");
        }
        uint256 first_encounter = block.timestamp;
        uint256 tokenId;
