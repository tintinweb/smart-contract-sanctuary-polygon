  ...
  ReentrancyGuardUpgradeable
{
  ...
  bool public isRepeat;  // Whether to limit the user to repeat mint  true:not  false: is(default)
  ...
  function mint() public nonReentrant returns (uint256 tokenId) {
    ...
    if (isRepeat){
        require(!hasNoMint[_msgSender()], "The address has already been minted");
        _safeMint(_msgSender(), tokenId); // mint PASS to user address
        hasNoMint[_msgSender()] = true;   // record that the address has been minted
  ...
  function addWhite(address[] memory _addWhite) public onlyReceivingAddress returns (bool) {
  ...
  function delWhite(address[] memory _delWhites) public onlyReceivingAddress returns (bool) {
