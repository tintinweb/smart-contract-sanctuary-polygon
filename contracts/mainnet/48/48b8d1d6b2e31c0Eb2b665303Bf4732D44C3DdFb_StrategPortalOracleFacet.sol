...
contract StrategPortalOracleFacet is UsingDiamondOwner {
    ...
    function whitelistOracle(bool _enable, address _addr) external onlyOwner {
        ...
        emit OracleWhitelistChanged(_enable, _addr);
    }
    ...
    function getOracleRates(
        ...
        for (uint i = 0; i < ratesLength; i++) {
            rates[i] = LibOracle.getRate(_froms[i], _to[i]);
        }
    ...
    function updateOracleRates(
        ...
        for (uint i = 0; i < _rates.length; i++) {
            LibOracle.setRate(_froms[i], _to[i], _rates[i]);
            emit OracleRateChanged(_froms[i], _to[i], _rates[i]);
        }
    }
}
...
library LibOracle {
    ...
    struct OracleStorage {
        ...
        mapping(address => mapping(address => OracleEntry)) entries;
    }
    ...
    function getRate(address _from, address _to) internal view returns (uint256) {
        return oracleStorage().entries[_from][_to].rate;
    }
    ...
    function setRate(
        ...
        store.entries[_from][_to].rate = _rate;
    }
}
