...
contract airdrop is Initializable, OwnableUpgradeable {
    ...
    function goAirdrop(
        ...
        //检查输入的人数，跟发送数量数组的人数是否一致
        require(_users.length == _amts.length, "input err");

        //计算总共需要空投数量
        uint256 allAmt = 0;
        for (uint256 i = 0; i < _amts.length; i++) {
            allAmt = allAmt + _amts[i];
        }
        ...
        for (uint256 j = 0; j < _amts.length; j++) {
            IERC20Upgradeable(_erc20).safeTransfer(_users[j], _amts[j]);
        }
