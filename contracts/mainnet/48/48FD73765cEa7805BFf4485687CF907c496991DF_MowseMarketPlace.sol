...
contract MowseMarketPlace is Initializable, Payments, ReentrancyGuardUpgradeable, OwnableUpgradeable, IERC721ReceiverUpgradeable{
    /////////////////////////////////////////////////////////////////////////
    // using libraries
    /////////////////////////////////////////////////////////////////////////   
     ...
     */
    function initialize(address _iMarketSettings, address _iERC721CreatorRoyalty) initializer  public {
        require(
    ...
    function grantTokenListRole(address _account) external {
        require(iMarketplaceSettings.checkModeratorRole(msg.sender)!= false || msg.sender == owner(),"Caller is Neither Admin Nor Moderator");
        require(tokenListRole[_account] != true,"Address already has Token List Role");
    ...
    function createMarketItem(
        ...
        require(tokenListRole[msg.sender] == true,"Should have tokenListRole to list NFT ");
    ...
    {   
        ...
        IERC721Upgradeable(nftAddress).transferFrom(address(this), msg.sender, tokenId);
    ...
    {   
        ...
        uint price = tokenMarketData.price;

        if(tokenMarketData.owner != msg.sender && tokenMarketData.seller != msg.sender){
            revert("transferMarketSale: Wrong item or NFT not listed");
        }

        else if(tokenMarketData.seller == msg.sender && IERC721Upgradeable(nftContract).ownerOf(tokenId) == address(this)){
        ...
        else if(tokenMarketData.owner == msg.sender && IERC721Upgradeable(nftContract).isApprovedForAll(msg.sender,address(this)) !=true){
            revert("Not approved to MP");
        }
        else if(tokenMarketData.owner == msg.sender && IERC721Upgradeable(nftContract).isApprovedForAll(msg.sender,address(this)) ==true){
            require(ERC721Upgradeable(nftContract).ownerOf(tokenId) != address(this),"Error");
            IERC721Upgradeable(nftContract).safeTransferFrom(msg.sender, to, tokenId);
            tokenMarketData.owner = payable(to);
        emit MarketItemTransferred(
            itemId,
            nftContract,
     ...
     */
    function _marketPlaceStillOwnsTheToken(address nftContract, uint itemId) 
    ...
    *@return itemId nftContract seller price sold owner 
    */   
