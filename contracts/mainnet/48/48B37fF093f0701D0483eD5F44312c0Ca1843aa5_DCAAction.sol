...
contract DCAAction is BaseSiloAction {
    using SafeERC20 for IERC20;

    address private constant GRAVITY_ROUTER =
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],uint[5],bytes[],uint";
    ...
    {
        require(tokenA != tokenB, "DCA: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB
            ? (tokenA, tokenB)
            : (tokenB, tokenA);
        require(token0 != address(0), "DCA: ZERO_ADDRESS");
    }

    function swapWithUniSubRoute(
        SubRoute memory subRoute,
        uint256 inputAmount,
        address to
    ) private returns (uint256 outputAmount) {
        (address token0, ) = sortTokens(subRoute.tokenIn, subRoute.tokenOut);

        (uint256 reserve0, uint256 reserve1, ) = IUniswapV2Pair(subRoute.pool)
            .getReserves();

        (uint256 reserveIn, uint256 reserveOut) = subRoute.tokenIn == token0
            ? (reserve0, reserve1)
            : (reserve1, reserve0);

        uint256 amountOut = getAmountOut(inputAmount, reserveIn, reserveOut);

        (uint256 amount0Out, uint256 amount1Out) = subRoute.tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));

        IUniswapV2Pair(subRoute.pool).swap(
            amount0Out,
            amount1Out,
            to,
            new bytes(0)
        ...
        if (subRoute.router == GRAVITY_ROUTER) {
            amountOut = (amountOut * 9995) / 10000;
        }

        return amountOut;
    }

    function swapWithUniRoutes(SubRoute[] memory subRoutes, uint256 inputAmount)
        private
        returns (uint256 outputAmount)
    {
        uint256 swapAmount = inputAmount;

        SubRoute memory firstPool = subRoutes[0];

        SafeERC20.safeTransfer(
            IERC20(firstPool.tokenIn),
            firstPool.pool,
            swapAmount
        );

        for (uint256 i = 0; i < subRoutes.length; ) {
            address to = i < subRoutes.length - 1
                ? subRoutes[i + 1].pool
                : address(this);
            if (swapAmount == 0) {
                revert("zero swap output");
            }

            swapAmount = swapWithUniSubRoute(subRoutes[i], swapAmount, to);
    ...
    ) private returns (uint256[5] memory outputAmounts) {
        uint256[5] memory inputAmounts = abi.decode(inputData, (uint256[5]));

        address[5] memory _inputs;
        address[5] memory _outputs;
        uint256[5] memory _investAmounts;
        bytes[] memory subRoutesData;
        uint256 trigger;

        (_inputs, _outputs, _investAmounts, subRoutesData,trigger) = abi.decode(
            configuration,
            (address[5], address[5], uint256[5], bytes[],uint256)
        );

        if(trigger > 0 && (_inputs[0] == GFI || _outputs[0] == GFI)){
            if(claimGFIReward()){
                return outputAmounts;
            }
        }

        bool autoDCA = ISilo(address(this)).siloDelay() == 0;

        for (uint256 i = 0; i < subRoutesData.length; ) {
            if (_inputs[i] == address(0) || inputAmounts[i] == 0) {
                unchecked {
                    i++;
                }
                continue;
            ...
            SubRoute[] memory subRoutes = abi.decode(
                subRoutesData[i],
                (SubRoute[])
            );

            if (autoDCA) {
                outputAmounts[i] = swapWithUniRoutes(
                    subRoutes,
                    inputAmounts[i]
                );
            } else {
                if (inputAmounts[i] < _investAmounts[i]) {
                    unchecked {
                        i++;
                    }
                    continue;
    ...
    function enter(
        ...
        outputAmounts = swapAction(implementation, configuration, inputData);
    }
    ...
    function claimGFIReward() private returns(bool result){
    ...
    {
        (_inputs, _outputs, _investAmounts, subRoutes, trigger) = abi.decode(
            configData,
            (address[5], address[5], uint256[5], bytes[], uint256)
        );
    }
        ...
        returns (bool)
    {
        ...
        address[5] memory _outputs;
        uint256[5] memory _investAmounts;
        bytes[] memory _subRoutesData;

        (_inputs, _outputs, _investAmounts, _subRoutesData,) = abi.decode(
            configData,
            (address[5], address[5], uint256[5], bytes[],uint256)
        );
        ...
        if (_subRoutesData.length > 5) {
            return false;
        }

        if (_inputs[0] == address(0) || _outputs[0] == address(0)) {
            return false;
        }

        if (_investAmounts[0] == 0) {
            return false;
        }

        for (uint256 i; i < 5; ) {
            if (_inputs[i] != address(0)) {
                if (_investAmounts[i] == 0) {
                    return false;
                }
            }
            unchecked {
        ...
        returns (bool)
    {
        ...
        (_inputs, _outputs, _investAmounts, , trigger) = abi.decode(
            configuration,
            (address[5], address[5], uint256[5], bytes[], uint256)
        );
        ...
        for (uint256 i = 0; i < 5; ) {
            ...
            if (IERC20(_inputs[i]).balanceOf(msg.sender) >= _investAmounts[i]) {
                return true;
            }
            unchecked {
                i++;
        ...
        returns (bool)
    {
        ...
        (_inputs, , _investAmounts, , ) = abi.decode(
            configuration,
            (address[5], address[5], uint256[5], bytes[], uint256)
        );
        ...
        )
    {
        ...
        pendingReward = gfiGov.pendingEarnings(msg.sender);
    }
}
