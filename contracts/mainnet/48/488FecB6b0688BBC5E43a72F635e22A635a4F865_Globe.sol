...
contract Globe is ERC721, ISnowV1Program, LogisticVRGDA {
  ...
  /// @notice Allows the snow erc721 token to be clawed back for emergency redemption
  /// @notice Maps the address from to the token id to the operator address
  mapping(address => mapping(uint256 => address)) public clawbacks;
  ...
  /// @notice Give the snow token emergency ability to claw back the token
  function onERC721Received(
    address operator,
    address from,
    uint256 tokenId,
    ...
    clawbacks[from][tokenId] = operator;
    return ERC721TokenReceiver.onERC721Received.selector;
  }

  /// @notice Claw back the snow token
  function clawback(address from, uint256 tokenId) external {
    IERC721 token = IERC721(from);
    if (clawbacks[from][tokenId] != msg.sender) revert InvalidOwner();
    clawbacks[from][tokenId] = address(0);
