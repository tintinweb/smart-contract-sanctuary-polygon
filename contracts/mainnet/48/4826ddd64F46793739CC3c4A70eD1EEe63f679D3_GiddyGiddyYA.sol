...
contract GiddyGiddyYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function sharesToValue(uint256 shares) public view override returns (uint256) {
    ...
    uint256 pending = rewards > COMPOUND_THRESHOLD ? rewards - (rewards * GIDDY_CONFIG.earningsFee() / BASE_PERCENT) : 0;
  ...
  function compound() public override whenNotPaused {
    ...
    if (earnings > COMPOUND_THRESHOLD) {
      earnings = deductFeeGiddy(earnings, earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT);
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    uint256 giddyValue = swapTokensSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, address(this), value);
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant{
    ...
    uint256 usdcValue = swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, address(this), value);
