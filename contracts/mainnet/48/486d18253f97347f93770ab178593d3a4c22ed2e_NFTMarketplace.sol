...
// File: contracts/dweb3marketplace.sol
...
contract NFTMarketplace {
  ...
  function TransferInt(address a, address b, uint256 amount) internal{
    IERC20(a).transferFrom(
      msg.sender,
                b,
                amount
            );
  ...
  function fillOffer(uint _offerId, address Erc, uint price, uint uid) public payable {
   ...
   if(_offer.isErc20 == true){
        ...
        //begining of Royalties and dweb3 commision
        if(UserTracker[uid] == 1){

              if(royal.royalty == 0){
                  TransferInt(Erc, _offer.user, price);
                  // TokenTracker[_offer.Erc20Token][royal.nftowner] += (price * royal.royalty) / 100;
                
              }else{
                // TokenTracker[_offer.Erc20Token][_offer.user] = price;
                 uint256 ra = (price / 100) * royal.royalty;
                  TransferInt(Erc, royal.nftowner, ra );
                  
                  // TokenTracker[_offer.Erc20Token][_offer.user] += (price * leftFund) / 100;
                    uint256 la = price - ra;
                   TransferInt(Erc, _offer.user, la );
              }//end of royal
            
        }else{
              if(royal.royalty == 0){
                uint256 Chargeuser = (price / 1000) / 25;
                TransferInt(Erc, creator, Chargeuser);
                
                uint256 leftCharged = price - Chargeuser;
                TransferInt(Erc, _offer.user, leftCharged);
                  // TokenTracker[_offer.Erc20Token][royal.nftowner] += 
                
              }else{
                  // userFunds[royal.nftowner] = price;
                // TokenTracker[_offer.Erc20Token][royal.nftowner] += 
                  uint256 r = (price / 100 ) * royal.royalty ; //10
                 TransferInt(Erc, royal.nftowner, r); ///royalty

                  uint creatorBalance = (price / 1000) * 25; //
                  TransferInt(Erc, creator, creatorBalance); 
                  
                  uint256 n = price - (r + creatorBalance);
                  TransferInt(Erc, _offer.user, n);
                // TokenTracker[_offer.Erc20Token][creator] += 
              }//end of royal
  ...
  function Getercbalance(address a, address b) public view returns (uint256) {
       return TokenTracker[a][b];
  }
