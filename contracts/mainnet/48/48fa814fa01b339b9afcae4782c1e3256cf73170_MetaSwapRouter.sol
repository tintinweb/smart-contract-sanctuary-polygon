...
library Constants {
    ...
    address internal constant ORIGINAL_MSG_SENDER = address(1);

    /// @dev Used as a flag for identifying direct message sender, saves gas by sending more 0 bytes
    address internal constant DIRECT_MSG_SENDER = address(2);
}
...
contract MetaSwapRouter is ReentrancyGuard, Pausable, MulticallExtended, SelfPermit {
    ...
    function swap(
        ...
        if (recipient == Constants.ORIGINAL_MSG_SENDER) recipient = sender;
        else if (recipient == Constants.DIRECT_MSG_SENDER) recipient = msg.sender;
