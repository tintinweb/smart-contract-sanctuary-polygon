...
contract LandVendingFacet {
    ...
    function beginKeystoneToLand(uint256 price, uint8 landType, uint256 slippage) external returns (uint256, string memory){
    ...
    function terminusAddress() external view returns (address) {
        return LibLandVending.terminusAddress();
    }

    function setTerminusAddress(address _terminusAddress) external {
        LibDiamond.enforceIsContractOwner();
        LibLandVending.setTerminusAddress(_terminusAddress);
    }
    ...
    function setOwedRBW(uint256 amount) external {
        LibDiamond.enforceIsContractOwner();
        LibLandVending.setOwedRBW(amount);
    }
    ...
    function getOwedRBW() external view returns(uint256) {
        return LibLandVending.getOwedRBW();
    }
...
library LibLandVending {
    ...
    function setOwedRBW(uint256 amount) internal {
        landVendingStorage().landVendingRBWCost = amount;
    }
    ...
    function getRarityByLandType(uint8 landType) private returns(uint256) {
        if(landType == 1) {
    ...
    function getOwedRBW() internal view returns(uint256) {
        return landVendingStorage().landVendingRBWCost;
    }
