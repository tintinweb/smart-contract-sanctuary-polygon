...
contract SettersFacet is Modifiers {
    ...
    function setCurvePools(address[] memory lpTokens, address[] memory pools) external onlyOwner {
    ...
    function setAaveV2Tokens(
        address[] memory markets,
        address[] memory aTokens,
        address[] memory vTokens
    ) external onlyOwner {
        require(markets.length == aTokens.length && markets.length == vTokens.length, '!LENGTH');
        for (uint256 i = 0; i < markets.length; i++) {
            s.aTokensV2[markets[i]] = aTokens[i];
            s.vTokensV2[markets[i]] = vTokens[i];
        }
    }

    function setAaveV3Tokens(
        address[] memory markets,
        address[] memory aTokens,
        address[] memory vTokens
    ) external onlyOwner {
        require(markets.length == aTokens.length && markets.length == vTokens.length, '!LENGTH');
        for (uint256 i = 0; i < markets.length; i++) {
            s.aTokensV3[markets[i]] = aTokens[i];
            s.vTokensV3[markets[i]] = vTokens[i];
        }
    }
}
