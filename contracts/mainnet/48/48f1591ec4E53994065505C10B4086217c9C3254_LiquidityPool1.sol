...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.1/contracts/utils/introspection/IERC1820Registry.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.1/contracts/token/ERC777/IERC777Sender.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.1/contracts/token/ERC777/IERC777Recipient.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.1/contracts/token/ERC777/IERC777.sol
...
// File: https://github.com/OpenZeppelin/openzeppelin-contracts/blob/release-v4.1/contracts/token/ERC777/ERC777.sol
...
contract LiquidityPool1 is IERC777Recipient, DSMath, ReentrancyGuard {
    ...
    event Bought(address indexed buyer, uint256 amount, uint256 amountOut, uint256 maticPrice);
    event Sold(address indexed seller, uint256 tokenAmount, uint256 maticOut, uint256 tokenPrice);
    ...
    require(reserve1 > 0 && reserve0 > 0, "AMM: INSUFFICIENT_LIQUIDITY");
    uint256 tokenToMatic = DSMath.wdiv(reserve1, reserve0);
    return tokenToMatic;
    }

    function sell(address payable seller, uint256 tokenAmount) public payable nonReentrant {
      prevalidateTrading(tokenAmount);
      uint256 maticOut = DSMath.wmul(getmaticToToken(), tokenAmount);
      deliverMatic(seller, maticOut);
      emit Sold(seller, tokenAmount, maticOut, gettokenToMatic());
      reserve1 = add(reserve1, tokenAmount);
      reserve0 = sub(reserve0, maticOut);
    }

      function prevalidateTrading(uint256 tokenAmount) internal view {
         require(tokenIn == token1, "AMM: INVALID_TOKEN");
         require(tokenAmount >= 30 ether);
    }

    function buy(address buyer, uint256 amount) public payable nonReentrant{
      require(amount > 1, "AMM: INSUFFICIENT_SWAP_AMOUNT");
      uint256 amountOut = DSMath.wmul(gettokenToMatic(), amount);
      deliverTokens(buyer, amountOut);
      emit Bought(buyer, amount, amountOut, getmaticToToken());
      reserve0 = add(reserve0, amount);
      reserve1 = sub(reserve1, amountOut);
    }

    function deliverTokens(address buyer, uint256 amountOut) internal {
        ERC777(token1).transfer(buyer, amountOut);
    }
