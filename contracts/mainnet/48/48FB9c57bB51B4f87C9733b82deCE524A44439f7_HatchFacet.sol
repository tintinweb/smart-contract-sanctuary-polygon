...
contract HatchFacet {
    ...
    function _finishHatchingEgg(
        ...
        require(
            ds.erc721_owners[_tokenId] != address(0),
            "HatchFacet: Non-existent token"
        );
    ...
    function beginHatchingEgg(uint256 _tokenId) external {
        LibDiamond.enforceCallerOwnsNFT(_tokenId);
        uint256 dna = LibUnicornDNA._getDNA(_tokenId);
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();

        require(
            dna > 0,
            "HatchFacet: No DNA found for this id"
        );

        require(
            LibUnicornDNA._getVersion(dna) == LibUnicornDNA._targetDNAVersion(),
            "HatchFacet: Invalid DNA version"
        );

        require(
            LibUnicornDNA._getLifecycleStage(dna) == LibUnicornDNA.LIFECYCLE_EGG,
            "HatchFacet: Only Eggs can be hatched"
        );

        require(
            LibIdempotence._getGenesisHatching(_tokenId) == false,
            "HatchFacet: Unicorn is already hatching or evolving"
        );
        ...
        if(origin) {
            ...
            ds.rngNonce =  uint(keccak256(abi.encodePacked(ds.erc721_tokenURIs[_tokenId], _tokenId)));
        } else {
            //  Code path moved to HatchingFacet
            revert("HatchFacet: beginHatchingEgg - invalid code path");   //  Not implemented - RS
        }
        ...
        for (uint256 i = 1; i <= 6; ++i) {
            ...
            majorGenes[i] = ds.bodyPartInheritedGene[parts[i]];
        ...
        dna = LibUnicornDNA._setBodyPart(dna, ds.bodyPartLocalIdFromGlobalId[parts[1]]);
        dna = LibUnicornDNA._setFacePart(dna, ds.bodyPartLocalIdFromGlobalId[parts[2]]);
        dna = LibUnicornDNA._setHornPart(dna, ds.bodyPartLocalIdFromGlobalId[parts[3]]);
        dna = LibUnicornDNA._setHoovesPart(dna, ds.bodyPartLocalIdFromGlobalId[parts[4]]);
        dna = LibUnicornDNA._setManePart(dna, ds.bodyPartLocalIdFromGlobalId[parts[5]]);
        dna = LibUnicornDNA._setTailPart(dna, ds.bodyPartLocalIdFromGlobalId[parts[6]]);
    ...
    function attemptGeneUpgrade(uint256 _geneId, uint256 _geneDominance) internal returns (uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        if(ds.geneTierById[_geneId] >= 6) return _geneId;
        uint256 rand = LibRNG.getCheapRNG(100);
        if(rand <= ds.geneUpgradeChances[ds.geneTierById[_geneId]][_geneDominance]) {
            return ds.geneTierUpgradeById[_geneId];
        }
        return _geneId;
    }
