...
contract Titanium300 {
    ...
    address public id1=0x47A1F9fF7e11f96EE5013917cF52f565F34850d2;


    uint256 public lastDistribute;
    
    address owner=0x0C0C2019B6B77443B16def1AFfb773117cA525a1;
    address deductionWallet=0x9b2C6F5D3F09E41Dc8047FD2c6c3c0760f697726;
    ...
    function buyNewLevel(uint8 level) external {
        ...
        if(users[msg.sender].activeLevels[level])
        {
            ...
            users[msg.sender].totalDeposit +=packagePrice[level];
        }
        if(status)
        {
            ...
            dailyPayoutOf(users[msg.sender].referrer);
            ...
            if(order.isactive){
                if(block.timestamp>order.deposit_time){
                    ...
                    if(order.payouts<max_payout){
                        ...
                        if(userLayerDayDirect[i+1][_user]>4)
                            userLayerDayDirect[i+1][_user]=userLayerDayDirect[i+1][_user]-5;
