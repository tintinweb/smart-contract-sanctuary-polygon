  ...
  */
  ...
  /**
   Set the base debt of a vault.
    @dev only the vaultsCore module can call this function
    @param _vaultId Vault ID of which the base debt will be updated
    @param _newBaseDebt the new base debt of the vault.
  */
  function setBaseDebt(uint256 _vaultId, uint256 _newBaseDebt) external override onlyInceptionCore {
    require(vaultExists(_vaultId), "IV105");
    InceptionVault storage _vault = _vaults[_vaultId];
...
@param _id the ID of the vault
  ...
  */
  function vaultDebt(uint256 _vaultId) external view override returns (uint256) {
    return _a.ratesManager().calculateDebt(_vaults[_vaultId].baseDebt, _inceptionVaultsCore.getCumulativeRate());
  }
  ...
  */
  function vaultExists(uint256 _id) public view override returns (bool) {
    return _vaults[_id].owner != address(0);
  }
  ...
  */
  function getVault(uint256 _id) external view override returns (InceptionVault memory) {
    return _vaults[_id];
  }
  ...
  function getInceptionVaultsCore() external view override returns (IInceptionVaultsCore) {
  ...
  function getBaseDebt() external view override returns (uint256) {
    return _baseDebt;
  }
}
