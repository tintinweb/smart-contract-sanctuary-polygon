...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "drivergoal: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "drivergoal: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "drivergoal: the user cannot buy his own offer");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "drivergoal: only the original seller can cancel his sales");
    ...
    function bid(uint256 tokenId, uint256 qty, uint256 price) external nonReentrant {
        ...
        require(qty>0, "drivergoal: qty has to be positive");
        require(price>0, "drivergoal: price has to be positive");
        require(qty <= totalSupply(tokenId), "drivergoal: not enough items for sale");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "drivergoal: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "drivergoal: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "drivergoal: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "drivergoal: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "drivergoal: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract ERC1155drivergoal is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "ERC1155drivergoal-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "drivergoal: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "drivergoal: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "drivergoal: Wrong NFT ID");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "drivergoal: wrong signature");
    }
