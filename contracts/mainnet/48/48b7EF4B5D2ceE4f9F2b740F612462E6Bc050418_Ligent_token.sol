...
=== 'Ligent' Token contract with following features ===
    => ERC20 Compliance
...
-------------------------------------------------------------------
 Copyright (c) 2022 onwards Ligent
-------------------------------------------------------------------
*/ 
...
contract Ligent_token is owned {
    
    using SafeMath for uint256;

    string constant private _name = "Ligent";
    string constant private _symbol = "LGNT";
    ...
    function repurches(address _from, uint256 _value) external returns(bool) {

        require(msg.sender == minerContract, 'Invalid caller');
        //checking of enough token balance is done by SafeMath
        _balanceOf[_from] = _balanceOf[_from].sub(_value);  // Subtract from the sender
        _balanceOf[msg.sender] = _balanceOf[msg.sender].add(_value);    // Updates on contract
    
        return true;
    }

    function burn_internal(uint256 _value, address _from) external returns (bool success) {

        //checking of enough token balance is done by SafeMath
        require(msg.sender == minerContract, 'Invalid caller');
        _balanceOf[_from] = _balanceOf[_from].sub(_value);  // Subtract from the sender
        _balanceOf[msg.sender] = _balanceOf[msg.sender].add(_value);
        emit Transfer(msg.sender, address(0), _value);
        return true;
    }
