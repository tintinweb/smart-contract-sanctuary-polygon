...
contract Silo is Initializable, KeeperCompatibleInterface{
    ...
    {   
        if(siloDelay != 0 && deposited){//If delay is zero, maintenance is only conditional based
        ...
        if(status == Statuses.UNWIND){
            ...
            if(!upkeepNeeded){
                (,,uint availableBlock) = getWithdrawLimitSiloInfo();
                if(block.number >= availableBlock){
                    upkeepNeeded = true;
                }
            }
        }else{
    ...
    function performUpkeep(bytes calldata performData) external override onlySiloManager {
        ...
        if(keeperExit){
            if(deposited){ //if there is money in the strategy then remove it
                _exitStrategy(0);
            }
            if(status != Statuses.UNWIND){
            ...
            _runStrategy();
            if(status == Statuses.DORMANT){//not enough funds in the silo to reinvestment, so set status to paused
    ...
    function setStrategy(address[4] memory _inputs, bytes[] memory _configurationData, address[] memory _implementations) external onlyFactory{
        ...
        require(status == Statuses.PAUSED, "Call exitSiloStrategy(id, 0) before changing strategy");
    ...
    function pause() external onlyFactory {
        ...
        require(status == Statuses.MANAGED, "Cannot pause a silo that is not managed");
        status = Statuses.PAUSED;
    }
    
    //user could flip this to managed without setting a strategy, but UI is only set up for new silos to have a strategy
    function unpause() external onlyFactory {
        require(status == Statuses.PAUSED, "Cannot unpause a silo that is not paused");
        status = Statuses.MANAGED;
    }
    ...
    function _investSilo() internal returns(uint[4] memory amounts){
        ...
        for(uint i=0; i<4; i++){
            ...
            if(depositTokens[i] != address(0)){
                ...
                if(tokenAmount > 0 && tokenAmount >= Factory.tokenMinimum(depositTokens[i])){
                    ...
                    if(isNew){//track if a silo has ever had anything deposited into it
                        isNew = false;
                    }
                }
            }
    ...
    function _exitStrategy(uint _requestedOut) internal{
        ...
        if(!hasStrategyBalance()){
            ...
            if(isLimit && _requestedOut == 0 && status != Statuses.UNWIND){
                status == Statuses.UNWIND;
            }
        }
    }

    function getWithdrawLimitSiloInfo() public view returns(bool, uint amount,uint availableBlock){
        for(uint i=0; i<strategy.length; i++){
            ( amount,  availableBlock) = IAction(strategy[i].implementation).withdrawLimit(strategy[i].configurationData);
            if(amount > 0 || availableBlock > 0){
                return (true,amount,availableBlock);
            }
        }
        return (false,0,0);
    }
    ...
    function hasStrategyBalance() public view returns(bool){
        ...
        for(uint i=0; i<strategy.length; i++){
            ...
            strategyBalance = action.showBalances(address(this),  strategy[i].configurationData);
