...
contract HOMBondingCalculator is IBondingCalculator {
    ...
    function getKValue( address _pair ) public view returns( uint k_ ) {
        ...
        uint lptoken = IERC20( _pair ).decimals();
        // uint decimals = token0.add( token1 ).sub( IERC20( _pair ).decimals() );
        uint decimals = lptoken.sub( token0.add(token1) ); 
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == HOM ) {
            reserve = reserve1;
        } else {
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** IERC20( HOM ).decimals() ) ).div( getTotalValue( _pair ) );
    }
}