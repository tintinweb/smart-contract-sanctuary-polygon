    ...
    DefaultOperatorFilterer
{
    ...
    uint256 private constant MIN_GAS_TO_TRANSFER_AND_STORE = 200_000;
    ...
    constructor(address _lzEndpoint) ERC721("MASTER", "MASTER") ONFT721Core(1, _lzEndpoint) {}
    ...
    function init(bytes calldata data) public payable override {
        ...
        minGasToTransferAndStore = MIN_GAS_TO_TRANSFER_AND_STORE;
        name = _name;
        symbol = _symbol;
        baseURI = baseURI_;
    }
    ...
    function _creditTo(uint16, address _toAddress, uint256 _tokenId) internal virtual override {
        address ownerNFT = _ownerOf[_tokenId];
        bool exists = ownerNFT != address(0);
        require(!exists || ownerNFT == address(this));
        if (!exists) {
