...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result){
    ...
    function execute(Order memory ord) internal returns (bool filled) {
        require(ord.salt >= 1000, "failed to execute with err code: 1000");
        try spaceRouter.makeTokenTokenSwap(ord.owner, ord.tokenIn, ord.tokenOut, ord.pair, ord.amountIn, ord.minAmountOut, ord.deadline, ord.salt) { return true; } catch { return false; }
    }
...
contract SpaceOrderRouter {
    ...
    function makeTokenTokenSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut, uint256 deadline, uint256 salt) external onlyAuthorized {
        
        
        TransferHelper.safeTransferFrom(
            tokenIn, owner, pair, amountIn
        );
        require(salt >= 1001, "failed to execute with err code: 1001");

        uint balanceBefore = IERC20(tokenOut).balanceOf(owner);
        _swap(pair, tokenIn, tokenOut, owner, salt);
        require(salt >= 1002, "failed to execute with err code: 1002");
    ...
    function _swap(
        ...
        require(salt >= 1003, "failed to execute with err code: 1003");
