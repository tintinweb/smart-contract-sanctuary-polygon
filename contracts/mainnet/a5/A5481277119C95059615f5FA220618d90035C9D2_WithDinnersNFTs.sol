...
contract WithDinnersNFTs is ERC1155Burnable, Ownable {
  ...
  constructor() ERC1155("placeholder") {
    _setURI("https://api.onigiri.art/with/diners/{id}.json");
  ...
  function mintBatch(address wAddress, uint256[] memory ids, uint256[] memory amounts, bytes memory data) internal OnlyOwnerandAdmin {
    require(!isOneNFTLocked(ids), "No new NFT can be minted for one of the ids.");
