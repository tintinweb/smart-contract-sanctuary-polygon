...
contract KaliDAOrelayer is Multicallable {
    IKaliRelay private immutable factory;

    constructor(IKaliRelay _factory) payable {
        factory = _factory;
    }
    ...
    function cancelProposal(IKaliRelay dao, uint256 proposal) external payable {
        dao.cancelProposal(proposal);
    }

    function processProposal(IKaliRelay dao, uint256 proposal) external payable {
