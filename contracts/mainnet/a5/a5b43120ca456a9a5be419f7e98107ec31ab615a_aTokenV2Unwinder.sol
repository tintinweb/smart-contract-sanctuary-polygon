...
contract aTokenV2Unwinder is BaseDepositUnwinder {
        ...
        returns (uint256 underlyingAmnt)
    {
        ...
        require(IAToken(token).balanceOf(address(this)) > 0, "no token is held by unwinder");
        ...
        uint256 underlyingAmntBefore = IERC20MetadataUpgradeable(underlying).balanceOf(beneficiary);

        // redeem all aToken in this contract
        uint256 amountWithdrawn = ILendingPool(lpAddrProvider.getLendingPool()).withdraw(
            underlying,
            type(uint256).max,
            beneficiary
        );
        require(amountWithdrawn > 0, "aTokenV2Unwind: Failed to withdraw from AaveV2");
        // check if all aTokens are minted
        require(aToken.balanceOf(address(this)) == 0, "aTokenV2Unwind: Failed to withdraw all tokens from AaveV2");

        underlyingAmnt = IERC20MetadataUpgradeable(underlying).balanceOf(beneficiary) - underlyingAmntBefore;
        require(underlyingAmnt > 0, "aTokenV2Unwind: no underlying has been redeemed");

        emit DepositTokenUnwound(token, underlying, amountWithdrawn, underlyingAmnt);
    }

    function exchangeRate(address token) external view override returns (uint256) {
        require(underlyingToken[token] != address(0), "token is not supported");
        return
            (1e18 * 10**(IERC20MetadataUpgradeable(underlyingToken[token]).decimals())) /
            10**(IAToken(token).decimals()); // 1:1 pegged according to Aave doc
    }
}
