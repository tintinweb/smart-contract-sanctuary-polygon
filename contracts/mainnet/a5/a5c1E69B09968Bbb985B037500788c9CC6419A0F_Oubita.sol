...
// File: contracts/NFToubitaMatic2.sol
...
contract Oubita is ERC721, Ownable {
   ...
   //uint256 public NFTperAddressLimit= 20;
    ...
    uint256 limit_matic_whitelist = 200000000000000;
    uint256 limit_matic_special = 1500000000000000;
    uint256 limit_matic_normal = 2000000000000000;

    uint256 limit_eth_whitelist = 200000000000000; 
    uint256 limit_eth_special = 1500000000000000;
    uint256 limit_eth_normal = 2000000000000000;
    ...
    constructor(
        ...
        tokenContract = 0x54EAE8D0434f409A51c627546dd83CFBA59958a7;
    ...
    function mint(uint tipo, uint _mintAmount,uint256 _cost) public payable{
        ...
        if(msg.sender != owner()){
            ...
            if(onlyWhitelisted == true){
                    ...
                    multiTokensTransfer(pagos, tipo, _mintAmount, counter, _cost);
    ...
    function multiTokensTransfer(address _to, uint _type, uint _amount, uint _counter, uint256 _cost) public payable{
        if(_type == 1){
            if(onlyWhitelisted == true){
                require(_cost >= limit_matic_whitelist, "Wrong data");
                require(msg.value >= _cost * _amount, "You dont have enough balance");
            }else if(_counter < specialSale){
                require(_cost >= limit_matic_special, "Wrong data");
                require(msg.value >= _cost * _amount, "You dont have enough balance");
            }else if(_counter >= specialSale){
                require(_cost >= limit_matic_normal, "Wrong data");
                require(msg.value >= _cost * _amount, "You dont have enough balance");
            }
            ...
            if(onlyWhitelisted == true){
                ...
                uint256 precio = limit_eth_whitelist * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter < specialSale){
                
                uint256 precio = limit_eth_special * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }else if(_counter >= specialSale){
                
                uint256 precio = limit_matic_normal * _amount;
                IERC20(tokenContract).transferFrom(msg.sender, _to, precio);
            }
        }
    } 
    ...
    returns (string memory)
  {
    ...
    function setLimiMatic_whitelist (uint256 _amount) public onlyOwner{
        limit_matic_whitelist = _amount;
    }

    function setLimiMatic_special (uint256 _amount) public onlyOwner{
        limit_matic_special = _amount;
    }

    function setLimiMatic_normal (uint256 _amount) public onlyOwner{
        limit_matic_normal = _amount;
    }

    //-- ETH

    function setLimiEth_whitelist (uint256 _amount) public onlyOwner{
        limit_eth_whitelist = _amount;
    }

    function setLimiEth_special (uint256 _amount) public onlyOwner{
        limit_eth_special = _amount;
    }

    function setLimiEth_normal (uint256 _amount) public onlyOwner{
