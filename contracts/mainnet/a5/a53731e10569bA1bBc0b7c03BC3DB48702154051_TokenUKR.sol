...
contract TokenUKR is ERC20Std, Governance {
    ...
    function _donate(address token, uint256 amount) internal returns (uint256 halfAmountOfUKR) {
        ...
        uint256 u4Redeem = usdtAmount.mul(25).div(10 ** 2);//25% for Redeem
        _balance4Redeem = _balance4Redeem.add(u4Redeem);

        uint256 u2DAO = usdtAmount.sub(u4Redeem);//75% for DAO
    ...
    function safeWithdrawAllUSDT() onlyGovernance external {
        USDT.safeTransfer(_governance, USDT.balanceOf(address(this)));
    }
}