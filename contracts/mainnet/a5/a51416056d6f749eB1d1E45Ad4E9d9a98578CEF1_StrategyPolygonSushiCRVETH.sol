...
contract StrategyPolygonSushiCRVETH is StratManager, FeeManager {
    ...
    constructor(
        address _want, //0x396e655c309676caf0acf4607a868e0cded876db 
        uint256 _poolId, // 9 
        address _vault, // 0x405b4C2F047c80643389830304a928Ba89d32E5F
        address _unirouter, // 0x1b02da8cb0d097eb8d57a175b88c7d8b47997506 
        address _keeper, // 0xA1Ab2C87f842F864458464a34f66C76BdD7d4d6a
        address _strategist, // 0xA1Ab2C87f842F864458464a34f66C76BdD7d4d6a
        address _CashFeeRecipient // 0xA1Ab2C87f842F864458464a34f66C76BdD7d4d6a
    ) StratManager(_keeper, _strategist, _unirouter, _vault, _CashFeeRecipient) {
        want = _want;
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (tx.origin == owner() || paused()) {
            ...
            IERC20(want).transfer(vault, wantBal.sub(withdrawalFeeAmount));
        }
    }
    ...
    function chargeFees() internal {
        ...
        if (maticToOutput > 0) {
            IUniswapV2Router02(unirouter).swapExactTokensForTokens(maticToOutput, 0, maticToOutputRoute, address(this), block.timestamp);
        }
        
        uint256 toMatic = IERC20(output).balanceOf(address(this)).mul(45).div(1000);
        IUniswapV2Router02(unirouter).swapExactTokensForTokens(toMatic, 0, outputToMaticRoute, address(this), block.timestamp);

        uint256 maticBal = IERC20(matic).balanceOf(address(this));

        uint256 callFeeAmount = maticBal.mul(callFee).div(MAX_FEE);
        IERC20(matic).transfer(msg.sender, callFeeAmount);

        uint256 CashFeeAmount = maticBal.mul(CashFee).div(MAX_FEE);
        IERC20(matic).transfer(CashFeeRecipient, CashFeeAmount);

        uint256 strategistFee = maticBal.mul(STRATEGIST_FEE).div(MAX_FEE);
        IERC20(matic).transfer(strategist, strategistFee);
    }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapV2Router02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp0Route, address(this), block.timestamp);
        }

        if (lpToken1 != output) {
            IUniswapV2Router02(unirouter).swapExactTokensForTokens(outputHalf, 0, outputToLp1Route, address(this), block.timestamp);
        }
    ...
    function _giveAllowances() internal {
        IERC20(want).approve(minichef, uint256(type(uint256).max));
        IERC20(output).approve(unirouter, uint256(type(uint256).max));
        IERC20(matic).approve(unirouter, uint256(type(uint256).max));

        IERC20(lpToken0).approve(unirouter, 0);
        IERC20(lpToken0).approve(unirouter, uint256(type(uint256).max));

        IERC20(lpToken1).approve(unirouter, 0);
        IERC20(lpToken1).approve(unirouter, uint256(type(uint256).max));
    }

    function _removeAllowances() internal {
        IERC20(want).approve(minichef, 0);
        IERC20(output).approve(unirouter, 0);
        IERC20(matic).approve(unirouter, 0);
        IERC20(lpToken0).approve(unirouter, 0);
        IERC20(lpToken1).approve(unirouter, 0);
    }
}