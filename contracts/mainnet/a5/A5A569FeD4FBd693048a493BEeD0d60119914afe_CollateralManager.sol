    ...
    ICollateralManager
{
    ...
    */
    function updateCollateralNonce(
        ...
        //require(_collateralNonce > collateralNonce, "C32");  //TODO: uncomment this when in production 
    ...
    ITFM
{
    ...
    */
    function novate(NovateParams calldata _params) external {
        ...
        // Deal with all the cases in which novation can take place
        {
            if (
                ...
                // Initiator has to be - (thisStrategy.alpha == targetStrategy.omega) || (thisStrategy.omega == targetStrategy.alpha)
                require(
                    (thisStrategy.alpha == targetStrategy.omega) ||
                        (thisStrategy.omega == targetStrategy.alpha),
                    "S36 - A"
                ...
                // Initiator has to be - (thisStrategy.alpha == targetStrategy.alpha) || (thisStrategy.omega == targetStrategy.omega)
                require(
                    (thisStrategy.alpha == targetStrategy.alpha) ||
                        (thisStrategy.omega == targetStrategy.omega),
                    "S36 - B"
        ...
        Utils.checkNovationSignatures(
            _params,
            initiator,
            (initiator == thisStrategy.alpha)
                ? thisStrategy.omega
                : thisStrategy.alpha,
            (initiator == targetStrategy.alpha)
                ? targetStrategy.omega
    ...
    function novateFinalise(
        ...
        Strategy storage targetStrategy = strategies[targetStrategyID];

        // Update strategy nonces
        {
            strategyNonce[thisStrategyID]++;
            strategyNonce[targetStrategyID]++;
        }

        int256 thisAmplitude = thisStrategy.amplitude;
        int256 targetAmplitude = targetStrategy.amplitude;
        ...
        if (Utils.abs(thisAmplitude) < Utils.abs(targetAmplitude)) {
            ...
            collateralManager.reallocatePortionNoCollateralCheck(
                (initiator == targetStrategyAlpha)
                    ? targetStrategyOmega
                    : targetStrategyAlpha, // Corresponds to person who is not the initiator in targetStrategy
                targetStrategyID,
                thisStrategyID,
                thisAmplitude.abs(),
                targetAmplitude.abs(),
            ...
            // Dellocate initiator's collateral from one of targetStrategy.
            collateralManager.reallocateAllNoCollateralCheck(
            ...
            if (initiator == thisStrategyAlpha) {
                thisStrategyAlpha = (initiator == targetStrategyAlpha)
                    ? targetStrategyOmega
                    : targetStrategyAlpha;
            } else {
                thisStrategyOmega = (initiator == targetStrategyAlpha)
                    ? targetStrategyOmega
                    : targetStrategyAlpha;
            }
            ...
            collateralManager.reallocatePortionNoCollateralCheck(
                ...
                targetStrategyID, // Corresponds to AC.
                targetAmplitude.abs(),
                thisAmplitude.abs(),
                thisStrategy.basis
            );
            ...
            if (initiator == targetStrategyOmega) {
                targetStrategyOmega = (initiator == thisStrategyOmega)
                    ? thisStrategyAlpha
                    : thisStrategyOmega;
            } else {
                targetStrategyAlpha = (initiator == thisStrategyOmega)
                    ? thisStrategyAlpha
                    : thisStrategyOmega;
            }
            ...
            collateralManager.reallocateAllNoCollateralCheck(
                initiator, // Corresponds to B.
                targetStrategyID, // Corresponds to deleted strategy.
                0,
                thisStrategy.basis
            );
            ...
            thisStrategy.transferable = newTransferable;
            if (initiator == thisStrategyAlpha) {
                strategies[thisStrategyID].alpha = (initiator ==
                    targetStrategyOmega)
                    ? targetStrategyAlpha
                    : targetStrategyOmega;
            } else {
                strategies[thisStrategyID].omega = (initiator ==
                    targetStrategyOmega)
                    ? targetStrategyAlpha
                    : targetStrategyOmega;
            }
            delete strategies[targetStrategyID]; // Delete original BC strategy.
            delete strategyNonce[targetStrategyID]; // Delete strategyNonce for strategy BC
        }
    }
...
library Utils {
    ...
    */
    function checkNovationSignatures(
        ...
        address _initiator,
        address _thisStrategyNotInitiator,
        address _targetStrategyNotInitiator,
