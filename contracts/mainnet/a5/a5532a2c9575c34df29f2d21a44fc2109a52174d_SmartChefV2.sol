...
contract SmartChefV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public constant MAX_WITHDRAW_FEE_PERIOD = 100 days; // 100 days
    ...
    constructor(IERC20 _stakeToken, uint _stakingEndBlock, uint _limitPerUser, address _treasury) {
    ...
    function unstake(uint _amount) external nonReentrant {
        ...
        if (block.timestamp < (user.lastDepositedTime + withdrawFeePeriod)) {
            uint256 currentWithdrawFee = stakedAmount[msg.sender] * withdrawFee / 10000;
            stakeToken.safeTransfer(treasury, currentWithdrawFee);
            stakedAmount[msg.sender] = stakedAmount[msg.sender]- currentWithdrawFee;
            totalStakedSupply -= currentWithdrawFee;
        }
        _updateRewardDebt(msg.sender);
