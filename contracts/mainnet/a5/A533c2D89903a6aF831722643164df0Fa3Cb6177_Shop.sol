...
contract Shop is ReentrancyGuard, Ownable {
    ...
    // DropNo => user address => token id => purchased number
    ...
    modifier whenNotPaused() {
        require(!isPaused, "Selling currently on hold");
        _;
    }
    ...
    function purchase(
        ...
        require(
            userLimits[dropNo][_beneficiary][_tokenId] + _count <= item.limit,
        ...
        IERC1155(_nftAddress).safeTransferFrom(item.owner, _beneficiary, _tokenId, _count, "");
        ...
        userLimits[dropNo][_beneficiary][_tokenId] =
            userLimits[dropNo][_beneficiary][_tokenId] +
            _count;

        _usedNonces[_beneficiary][_nonce] = true;

        emit ItemPurchased(_nftAddress, _beneficiary, _tokenId, _count);
    }
    ...
    function createItem(
        ...
        require(
            IERC1155(_nftAddress).balanceOf(_itemsStorage, _tokenId) >= _amount,
            "createItem: storage doesn't have enought balance"
        );
        require(
            IERC1155(_nftAddress).isApprovedForAll(_itemsStorage, address(this)),
            "createItem: not approved"
        );
        ...
        emit ItemCreated(_nftAddress, _itemsStorage, _amount, _limit, _price, _tokenId);
    }
    ...
    function cancelItem(address _nftAddress, uint256 _tokenId) external onlyOwner {
        ...
        uint256 i = _findIndexInShopItems(_nftAddress, _tokenId);
    ...
    function updateItemPrice(
        ...
        uint256 i = _findIndexInShopItems(_nftAddress, _tokenId);
        _shopItems[dropNo][i].price = _price;
    ...
    function updateItemAmount(
        ...
        uint256 i = _findIndexInShopItems(_nftAddress, _tokenId);
        _shopItems[dropNo][i].amount = _amount;
    }
    ...
    function updateItemLimit(
        ...
        uint256 i = _findIndexInShopItems(_nftAddress, _tokenId);
        _shopItems[dropNo][i].limit = _limit;
    }
    ...
    function _findIndexInShopItems(address _nftAddress, uint256 _tokenId) private returns (uint256) {
        for (uint256 i = 0; i < _shopItems[dropNo].length; i++) {
            if (
                _shopItems[dropNo][i].nftAddress == _nftAddress &&
                _shopItems[dropNo][i].tokenId == _tokenId
        ...
        require(false, "Can't find item in the shop");
    }
