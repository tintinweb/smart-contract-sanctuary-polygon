...
contract Ftech {
    ...
    modifier onlyViewer {
        require(msg.sender == Viewer, "Not a user");
        _;
    }
    ...
    function reg(address _user, address _referrer ,uint256 _level) private {
        ...
        for(uint256 i=0; i<_level; i++) {
            ...
            address _upline = userInfo[_user].PFIUpline[i];
            for(uint256 j=0; j<11; j++) {
                if(_upline != address(0) && _upline != address(this)) {
                    rewardInfo[_upline].PFI[i] += PFIDist[i];
                    _upline = userInfo[_upline].PFIUpline[i];
                } else {
                    break;
                }
            }
