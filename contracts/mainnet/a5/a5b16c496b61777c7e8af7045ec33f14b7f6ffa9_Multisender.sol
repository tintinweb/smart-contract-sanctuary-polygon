...
contract Multisender is Initializable, OwnableUpgradeable, UUPSUpgradeable  {
    ...
    event AdminChanged(address indexed sender, bool state);
    constructor() {
    ...
    function setDistributionWallet( address _walletAddress) public onlyOwner {
        distributionWallet = _walletAddress ;
    }
    ...
    function multiSendTokenDifferentAmount(
        ...
        uint256[] memory _amounts
            ...
            "This address can not send"
        );
        require(_recipients.length == _amounts.length, 'error size');
        uint256 total = 0;
        for (uint256 i = 0; i < _recipients.length; i++) {
            IERC20(token).safeTransferFrom(distributionWallet , _recipients[i], _amounts[i]);
     ...
     uint256 sendAmount = _recipients.length.sub(1).mul(_amounts);
        for (uint256 i = 0; i < _recipients.length; i++) {
            IERC20(token).safeTransferFrom(distributionWallet ,_recipients[i], _amounts);

        }
        emit MultiSend(msg.sender, token, sendAmount);
    }

        function multiSendDiffNativeCoin(address payable[] memory _recipients, uint256[] memory _amounts) public payable {
        require(owner() == msg.sender || adminList[msg.sender] == true,"address cannot mint");
        require(_recipients.length == _amounts.length, 'error size');
    ...
    function addNRemoveAdmin(address[] memory _addr, bool _flag) external onlyOwner {

        if (_flag) {
            // Add admin addresses
            for (uint256 i = 0; i < _addr.length; i++) {
                adminList[_addr[i]] = _flag;
                // Check if the address is already in the adminAddresses array
                ...
                if (!duplicate) {
                    ...
                    emit AdminChanged(_addr[i], _flag);
                }
            }
        } else {
            ...
            for (uint256 i = 0; i < _addr.length; i++) {
                adminList[_addr[i]] = _flag;
                emit AdminChanged(_addr[i], _flag);
                // Find the address in the adminAddresses array and remove it
                for (uint256 j = 0; j < adminAddresses.length; j++) {
                    if (adminAddresses[j] == _addr[i]) {
