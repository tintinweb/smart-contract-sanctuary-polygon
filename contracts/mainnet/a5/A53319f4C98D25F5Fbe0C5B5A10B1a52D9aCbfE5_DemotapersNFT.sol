...
interface IERC721Metadata is IERC721, IERC2981 {
...
contract DemotapersNFT is ERC165, Ownable, IERC721Metadata, IERC721Enumerable {
    ...
    string private constant NAME = 'Demotapers NFT';

    string private constant SYMBOL = 'DMTPR NFT';

    string private _baseURI;

    EnumerableMap.UintToAddressMap private _tokenOwners;  // tokenId => owner
    ...
    event RoyaltyNumeratorSet(uint256 value);
    uint256 public constant ROYALTY_DENOMINATOR = 10000;  // 100% = 10000
    uint256 public constant MAX_ROYALTY_DENOMINATOR = 1000;  // royalty cannot be set higher than 10%
    uint256 public royaltyNumerator;
    
    constructor() {
        // double-check the validity of "magic" hashes
        // this is a cheap check and we will be sure that all values are correct
        assert(_INTERFACE_ID_ERC721 == type(IERC721).interfaceId);
        assert(_INTERFACE_ID_ERC721_METADATA == type(IERC721Metadata).interfaceId);
        assert(_INTERFACE_ID_ERC721_ENUMERABLE == type(IERC721Enumerable).interfaceId);
        assert(_ERC721_RECEIVED == bytes4(keccak256("onERC721Received(address,address,uint256,bytes)")));
    ...
    /**
     * @notice set royalty numerator
     * @param value new value (denominated by 10000)
     */
    function setRoyaltyNumerator(uint256 value) external onlyOwner {
        require(value <= MAX_ROYALTY_DENOMINATOR, "wrong value");
        royaltyNumerator = value;
        emit RoyaltyNumeratorSet(value);
    }
    ...
    */
    ...
    function tokenByIndex(uint256 index) external view returns (uint256) {
        (uint256 tokenId, /*address owner*/) = _tokenOwners.at(index);
        return tokenId;
    }
    ...
    */
    ...
    /**
     * @dev Function mints NFT to owner address
     *
     * @param tokenAmount number of NFTs to mint.
    */
    function batchMint(uint tokenAmount) external onlyOwner returns (uint) {
        require(tokenAmount > 0, "Tokens amount cannot be 0");
        require(tokenAmount <= 100, "Can only mint 100 tokens at a time");

        uint first = totalSupply();
    ...
    */
    ...
    function royaltyInfo(
      ...
      returns (
        ...
        receiver = owner();
        royaltyAmount = salePrice * royaltyNumerator / ROYALTY_DENOMINATOR;
    }
}
...
contract ERC165 is IERC165 {
    bytes4 private constant _INTERFACE_ID_ERC165 = bytes4(keccak256('supportsInterface(bytes4)')); // 0x01ffc9a7
