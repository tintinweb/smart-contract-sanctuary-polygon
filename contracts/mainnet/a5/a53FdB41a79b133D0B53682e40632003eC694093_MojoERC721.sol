...
contract MojoERC721 is PlanetMojoERC721Base {
    constructor(
        ...
        PlanetMojoERC721Base(_owner, _name, _symbol, _baseUri, _openSeaProxyRegistryAddress) {
    }
}
...
abstract contract PlanetMojoERC721Base is ERC721MintManagement, CustomRoyalties, OwnableWithTempOwnership {
    ...
    constructor(
        ...
        ERC721Tradable(_name, _symbol, _openSeaProxyRegistryAddress) 
     ...
     */
    function setMinters(address[] memory minters, bool[] memory trusted) public onlyOwner {
        _setMinters(minters, trusted);
    }
    ...
    //  */
    // function setTokenIdCounter(uint256 tokenIdCounter) public override onlyOwner {
    //     _setTokenIdCounter(tokenIdCounter);
    // }
    ...
    function supportsInterface(bytes4 interfaceId) public view override(ERC721, IERC165) returns (bool) {
        return CustomRoyalties._supportsInterface(interfaceId) 
 ...
 */
abstract contract ERC721MintManagement is ERC721Tradable, IMintable {
    mapping(address => bool) private minters;

    /**
     * @dev Allow or disallow given accounts to mint new tokens
     */
    function _setMinters(address[] memory _minters, bool[] memory trusted) public {
     ...
     */
    function mintTo(address to) public override onlyMinters {
        _mintTo(to);
    }
     ...
     */
    function mintById(address to, uint256 tokenId) public override onlyMinters {
     ...
     */
    function mintToWithTokenURI(address to, string memory tokenURI) public override onlyMinters { 
        _mintToWithTokenURI(to, tokenURI);
    }

    /**
     * @notice Mints a token with a given ID to a given address and sets a tokenURI for that token
     */
    function mintByIdWithTokenURI(address to, uint256 tokenId, string memory tokenURI) public override onlyMinters {
        _mintByIdWithTokenURI(to, tokenId, tokenURI);
    }

    /**
     * @dev Mints a many tokens to many accounts
     * Can specify a different number of tokens for each account
     */
    function mintMany(address[] memory recipients, uint256[] memory tokenCounts) public override onlyMinters {
     ...
     */
    function mintManyByIds(address[] memory recipients, uint256[][] memory tokenIds) public override onlyMinters {
        for(uint256 i = 0; i < recipients.length; i++) {
            uint256 cnt = tokenIds[i].length;

            for(uint256 j = 0; j < cnt; j++) {
                _mintById(recipients[i], tokenIds[i][j]);
            }
        }
    }
...
abstract contract CustomRoyalties is RoyaltiesV2, IERC2981 {
    ...
    function _supportsInterface(bytes4 interfaceId) public pure returns (bool) {
...
abstract contract OwnableWithTempOwnership is Ownable {
    ...
    function setTempOwner(address account) public onlyOwner {
        //If a temporary owner is already set, disallow setting a new one
        //Need to call revokeTempOwner() first
        if(rootOwner != address(0)) {
            revert TempOwnerAlreadySet();
        }
    ...
    function revokeTempOwner() public {
        ...
        transferOwnership(rootOwner);
        rootOwner = address(0);
    }

    function tempOwner() public view returns (address) {
        return rootOwner != address(0) 
            ? owner()
            : address(0);
    }
 ...
 */
abstract contract ERC721Tradable is ERC721, RelayRecipientAndContextMixin, NativeMetaTransaction {
     ...
     */
    function _setTokenIdCounter(uint256 _tokenIdCounter) internal virtual {
     ...
     */
    function _mintToWithTokenURI(address _to, string memory _tokenURI) internal virtual {
        uint256 tokenId = _mintTo(_to);
    ...
    * @dev Mints a token with a given ID to an address
    */
    function _mintById(address _to, uint256 _tokenId) internal virtual {
        _safeMint(_to, _tokenId);
        totalSupply++;
    }

    /**
     * @dev Mints a token with a given ID to an address with a tokenURI.
     */
    function _mintByIdWithTokenURI(address _to, uint256 _tokenId, string memory _tokenURI) internal virtual {
        _mintById(_to, _tokenId);
...
abstract contract RelayRecipient {
    ...
    /**
     * @dev Require a function to be called through a trusted forwarder only
     */
