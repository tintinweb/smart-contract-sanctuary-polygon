...
interface IBIT is IERC20, IBASE, ISTAKEBIT, IORACLE, IBORROW {}

interface IUSDE is IERC20, IBASE, ISTAKEUSDE {}
...
contract BIT is IBIT, IOWNER {
    ...
    constructor(
        ...
        _initContract(NAME, SYMBOL, SYMBOL_OF, DECIMAL, START, MAXCAP);
    }
    ...
    function and(address token) external virtual returns (bool) {
        ...
        if (_isAdmin() && isCtr_ && _isWZero(and())) {
            USDE = IUSDE(token);
            emit And(token);
            return true;
        }
    ...
    function mint(address to, uint256 bitWei)
        ...
        returns (bool)
    {
        uint256 newCap_ = _totalSupply() + bitWei;
    ...
    function transferFrom(
        ...
        return _approve(from, _sender(), _allowance(from, _sender()) - bitWei);
    }
    ...
    function stake(
        ...
        bool before = _usdeOf(_sender()) >= usdeWei && _isAnd();
        if (before == true && USDE.stake(pool, usdeWei, month) == true) {
    ...
    function borrow(uint256 bitWei) external virtual override returns (bool) {
        ...
        if (_hasWei(_sender(), bitWei) && _isAnd()) {
        ...
        returns (bool)
    {
        ...
        if (canSwap_ && _hasWei(_sender(), bitWei)) {
            ...
            _price_s(bitWei);
            _transfer(_sender(), _contract, bitWei);
            return true;
        }
        return false;
    }
    ...
    function _redeem(address token, uint256 tkWei) internal returns (bool) {
        // deposit some token to usde
        IERC20 _token = IUSDE(token);

        address fr_ = _sender();
        address ctr_ = _contract;
        // Before tx need approve from _token.approve(ctr_, tkWei);

        uint256 redeemWei = _toBit(_SYMBOLOF(token), tkWei, _decimalOf(token));
        bool isOfToken = _ofToken(token, fr_) >= tkWei && tkWei > 0;
        bool isBitWei = redeemWei > 0 && _maxCap >= _totalSupply() + redeemWei;
    ...
    function _swap(address toToken, uint256 bitWei) internal returns (bool) {
        // withDraw from usde to some token;
    ...
    function _approve(
        ...
        require(_isWallet(owner_), "BIT: approve from the zero address");
        require(_isWallet(spender), "BIT: approve to the zero address");
    ...
    function _payment(uint256 usdeWei) internal returns (bool) {
        address from_ = _sender();
        ...
        if (isusdeWeis && _burnBy(usdeWeis) == true) {
            uint256 bit_pay = (asset_ * usdeWeis) / borrowOf_;
            _beforeTransfer(from_, from_, bit_pay);
            _fromContract(from_, bit_pay);
            _decBorrow(from_, bit_pay, usdeWeis);
            return true;
        }
        return false;
    }

    function _fromContract(address to, uint256 bitWei) internal {
        if (_hasWei(_contract, bitWei)) {
                _transfer(_contract, to, bitWei);
            } else {
                _mint(to, bitWei);
            }
    }

    function _mintBy(uint256 usdeWei) internal returns (bool) {
        return USDE.mintBy(usdeWei) == true;
    }

    function _burnBy(uint256 usdeWei) internal returns (bool) {
        return USDE.burnBy(usdeWei) == true;
    }

    function _mint(address to, uint256 bitWei) internal returns (bool) {
        if (!_isWallet(to)) {
            return false;
        }
        _beforeTransfer(_contract, to, bitWei);
        ...
        _emitTransfer(_contract, to, bitWei);
        return true;
    }

    function _burn(address from, uint256 bitWei) internal returns (bool) {
    ...
    function _price_s(uint256 bitWei) internal {
        uint256 bit_usd_Wei = _oracleUsdWei(_symbol);
    ...
    function _incTotalExchange(uint256 bitWei) internal {
        _totalExchange += bitWei;
    }

    function _incLock(address locker_, uint256 bitWei) internal {
        _totalLocking += bitWei;
        _locksOf[locker_] += bitWei;
        _transfer(locker_, _contract, bitWei);
        emit Locking(locker_, bitWei, _unLockAt(locker_));
    }

    function _decLock(address locker_, uint256 bitWei) internal {
        _totalLocking -= bitWei;
        _locksOf[locker_] -= bitWei;
        _fromContract(locker_, bitWei);
    ...
    function _incBalance(address wallet, uint256 bitWei) internal {
        _balances[wallet] += bitWei;
    }

    function _decBalance(address wallet, uint256 bitWei) internal {
        _balances[wallet] -= bitWei;
    }
    ...
    // function _fee_swap(uint256 bitWei) internal view returns (uint256) {
    ...
    function _usdeOf(address addr) internal view returns (uint256) {
    ...
    function _getStake(address wallet) internal view returns (IStake memory) {
        ...
        address pool = USDE.poolOf(wallet);
    ...
    function _initStart(uint256 START) internal {
        _chainOf = "MATIC";
        _FEE_SWAP = "FEE_SWAP";
    ...
    function _oracleStart(uint256 START) internal {
