...
contract UAEDfinanceProtocol {
    ...
    bytes4 public constant flashLoanSig =
        bytes4(abi.encodeWithSelector(this.flashLoan.selector));
    ...
    constructor(address _uaedContract) {
        ...
        _payDebtSig = bytes4(
            abi.encodeWithSignature("_payDebt(uint8,address)")
        );
        _liquidateSig = bytes4(
            abi.encodeWithSignature("_liquidate(address,uint8,address)")
        );
        ...
        tokenAddress = [
            ...
            0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6, // WBTC   1
            address(0), // ETH    2
            0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619, // WETH   3
            0x53E0bca35eC356BD5ddDFebbD1Fc0fD03FaBad39, // LINK   4
            0xD6DF932A45C0f255f85145f286eA0b292B21C90B, // Aave   5
            0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270, // WMATIC 6
            0x172370d5Cd63279eFa6d502DAB29171933a610AF, // CRV    7
            0x0b3F868E0BE5597D5DB7fEB59E1CADBb0fdDa50a, // SUSHI  8
            0xb33EaAd8d922B1083446DC23f610c2567fB5180f, // UNI    9
            0x6f8a06447Ff6FcF75d803135a7de15CE88C1d4ec // SHIBA  10
        ];
    ...
    fallback(
        bytes calldata _data
    ) external payable onlyOnFlashloan returns (bytes memory) {
        require(
            getSelector(_data) == flashLoanSig,
            "only for flashLoan ETH receive"
        );
    }
    ...
    function onTokenApproval(
        ...
        if (funcSig == _payDebtSig) {
            ...
            (address _user, uint8 _assetId) = abi.decode(
                _data[4:],
                (address, uint8)
            );
    ...
    ) external onlyProtocolRequestor {
    ...
    function addAssetAsCollateral(
        ...
        assert(
            tokenAddress.length == assetN &&
                tokenDecimals.length == assetN &&
                collateralFactor.length == assetN
        );
    }
    ...
    */
    ...
    function mintByCollateral(
        ...
        require(
            _assetId != 0 && _assetId != 2 && _assetId < assetN,
            "wrong assetId"
        );
        require(_percentage < collateralFactor[_assetId], "wrong percentage");
        require(
            !pledgor[msg.sender][_assetId].isPledgedBefore,
            "Repetitious asset"
        );
    ...
    function _mintByCollateral(
        address _user,
        uint _amount,
        uint8 _assetId,
        uint8 _percentage
    ) private notPausedAsCollateral(_assetId) {
        uint mintAmountNumerator = _amount *
            uaedFinancePrices.getPriceInUSD(_assetId) *
            _percentage *
            10 ** (tokenDecimals[0]); // 0 is UAED assetId
        uint mintAmountDenominator = uaedFinancePrices.getPriceInUSD(0) *
            10 ** (tokenDecimals[_assetId] + 2); // +2 : _percentage decimals
    ...
    function _sendAssetToPool(
        address _from,
        uint8 _assetId,
        uint _amount
    ) private {
        IERC20(tokenAddress[_assetId]).transferFrom(
            _from,
            address(uaedFinancePool),
            _amount
        );
    }
    ...
    function getDebtState(
        ...
        uint interest = ((block.timestamp - mintedTime) *
            interestRatePerHour *
            mintedAmount) / (3600 * 10 ** 8);
        uint debtAmount = interest + mintedAmount; // debtAmount in UAED

        // I => indexed
        // N => Numerator & D => Denominator
        // values below are in UAED
        uint collateralValueN = securityPledgedAmount *
            uaedFinancePrices.getPriceInUSD(_assetId) *
            10 ** (tokenDecimals[0]);
        uint collateralValueD = uaedFinancePrices.getPriceInUSD(0) *
            10 ** (tokenDecimals[_assetId]);
        uint collateralValue = collateralValueN / collateralValueD; // collateralValue in UAED
        uint IcollateralValue = (collateralValue * collateralFactor[_assetId]) /
            1e2; // 1e2 : collateralFactor decimals
    ...
    function _liquidate(
        ...
        ) = getDebtState(_user, _assetId); // _collateralValue is in UAED
        ...
        if (_debtAmount >= _collateralValue) {
            ...
            uint X = (100 * liquidatorBonusPercentage) /
                (100 - collateralFactor[_assetId]);
            ...
            if (surplusCollateralValue < liquidatorMinBounus) {
                ...
                liquidatorBonus = liquidatorLinearBonus < liquidatorMinBounus
                    ? liquidatorMinBounus
                    : liquidatorLinearBonus;
            }
            ...
            uint liquidatorPortion = ((_debtAmount + liquidatorBonus) *
                _pledgedAmount) / _collateralValue;
            ...
            if (_pledgedAmount > liquidatorPortion) {
                _sendAssetFromPool(
                    _user,
                    _assetId,
                    _pledgedAmount - liquidatorPortion
                );
            }
        }
    ...
    function getFlashLoanedETH() external payable onlyOnFlashloan {}
    ...
    function _ETHflashLoan(uint _amount) private nonReentrant {
        ...
        require(
            IFlashLoanReceiver(msg.sender).executeOperation(flashLoanFee, 2),
            "flashLoan failed"
        ); // 2 is ETH assetId
        _sendAssetToPool(_amount);
        require(address(this).balance == 0, "excessive returned ETH");
        require(
            address(uaedFinancePool).balance == poolBalance,
            "insufficient returned ETH"
        );
    }
