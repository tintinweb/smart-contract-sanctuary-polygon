...
contract DVFCrossSwap is Swap {
  ...
  function initialize(address admin, address paraswap, address paraswapTransferProxy, address relayer) external initializer {
    __DVFAccessControl_init(admin);
    __Swap_init(paraswap, paraswapTransferProxy, relayer);
  }
}
...
abstract contract Swap is Storage, UserWallet {
 ...
 bytes32 public constant _XCHAIN_TRANSACTION_TYPEHASH =
   keccak256("XChain(address user,bytes32 swapGateway,address[] tokensFrom,address[] tokensTo,uint256[] amountsFrom,uint256[] minAmountsTo,uint256 nonce,uint256 deadline,uint256 chainId)");

  struct XChainTransaction {
    address user;
    bytes32 swapGateway;
    address[] tokensFrom;
    address[] tokensTo;
  ...
  function executeXChainTransactionWithSignature(
    address vm,
    XChainTransaction memory xchainTransaction,
    bytes32 swapId,
    bytes memory signature,
    bytes calldata data
  ) external payable onlyRole(OPERATOR_ROLE) withUniqueId(swapId) {
    ensureDeadline(xchainTransaction.deadline);

    verifyXChainTransactionSignature(xchainTransaction, signature);

    (uint256[] memory amountsToUser, uint256[] memory amountsToFee) = 
      performXChainTransaction(
        vm,
        data,
        xchainTransaction
      );
    ...
    bool fundsBridged = xchainTransaction.swapGateway == DEVERSIFI_GATEWAY;
    if (fundsBridged) {
      transfer(xchainTransaction.user, xchainTransaction.tokensTo, address(this), amountsToUser);
    }

    emit SwapPerformedV2(swapId, xchainTransaction.swapGateway, xchainTransaction.user, xchainTransaction.tokensFrom,
     xchainTransaction.amountsFrom, xchainTransaction.tokensTo, amountsToUser, amountsToFee, fundsBridged);
  }

  function executeXChainTransactionWithSelfLiquidity(
    address vm,
    XChainTransaction memory xchainTransaction,
    bytes32 swapId,
    bytes calldata data
    ...
    transfer(address(this), xchainTransaction.tokensFrom, xchainTransaction.user, xchainTransaction.amountsFrom);
    ...
    emit SwapPerformedV2(swapId, xchainTransaction.swapGateway, xchainTransaction.user, xchainTransaction.tokensFrom,
     xchainTransaction.amountsFrom, xchainTransaction.tokensTo, amountsToUser, amountsToFee, false);
  ...
  function balancesOf(address[] memory tokens) internal view returns (uint256[] memory amounts) {
    amounts = new uint256[](tokens.length);
    for(uint i=0; i<tokens.length;i++)
    {
      amounts[i] = IERC20Upgradeable(tokens[i]).balanceOf(address(this));
    }
    return amounts;
  }

  function performXChainTransaction(
    address vm,
    bytes calldata data,
    XChainTransaction memory xChainTransaction
  ) private returns(uint256[] memory amountsToUser, uint256[] memory amountsToFee) {

    amountsToUser = balancesOf(xChainTransaction.tokensTo);
    amountsToFee = new uint256[](amountsToUser.length);
    uint256[] memory tokensFromAmount = new uint256[](xChainTransaction.tokensFrom.length);
    // Transfer tokens to the VM
    for(uint i=0; i<xChainTransaction.tokensFrom.length; i++)
    {
      tokensFromAmount[i] = IERC20Upgradeable(xChainTransaction.tokensFrom[i]).balanceOf(address(this));
      _decreaseBalance(xChainTransaction.tokensFrom[i], xChainTransaction.user, xChainTransaction.amountsFrom[i]);
      IERC20Upgradeable(xChainTransaction.tokensFrom[i]).safeTransfer(vm, xChainTransaction.amountsFrom[i]);
      tokensFromAmount[i] = tokensFromAmount[i] - IERC20Upgradeable(xChainTransaction.tokensFrom[i]).balanceOf(address(this));
    }

    // execute all vm calldata
    Relayer(relayer).relay{value:msg.value}(vm, data);

    // credit for the balance differences of the specified tokens
    for(uint i=0; i<xChainTransaction.tokensTo.length; i++)
    {
      amountsToUser[i] = IERC20Upgradeable(xChainTransaction.tokensTo[i]).balanceOf(address(this)) - amountsToUser[i];
      amountsToFee[i] = amountsToUser[i] - xChainTransaction.minAmountsTo[i];
      ...
      amountsToFee[i] = amountsToFee[i] > xChainTransaction.maxFeesTo[i] ? xChainTransaction.maxFeesTo[i] : amountsToFee[i];
      _increaseBalance(xChainTransaction.tokensTo[i], xChainTransaction.user, amountsToUser[i]);
      _increaseBalance(xChainTransaction.tokensTo[i], address(this), amountsToFee[i]);
 
      require(tokensFromAmount[i] <= xChainTransaction.amountsFrom[i], "HIGHER_THAN_AMOUNT_FROM");
      require(amountsToUser[i] >= xChainTransaction.minAmountsTo[i], "LOWER_THAN_MIN_AMOUNT_TO");
    }
  }
  ...
  function verifyXChainTransactionSignature(
    XChainTransaction memory xChainTransaction,
    bytes memory signature
  ) private {
    require(xChainTransaction.nonce > userNonces[xChainTransaction.user], "NONCE_ALREADY_USED");
    require(xChainTransaction.chainId == block.chainid, "INVALID_CHAIN");

    bytes32 structHash = _hashTypedDataV4(keccak256(
      abi.encode(
        _XCHAIN_TRANSACTION_TYPEHASH,
        xChainTransaction.user,
        xChainTransaction.swapGateway,
        keccak256(abi.encodePacked(xChainTransaction.tokensFrom)),
        keccak256(abi.encodePacked(xChainTransaction.tokensTo)),
        keccak256(abi.encodePacked(xChainTransaction.amountsFrom)),
        keccak256(abi.encodePacked(xChainTransaction.minAmountsTo)),
        xChainTransaction.nonce,
        xChainTransaction.deadline,
        xChainTransaction.chainId
      )
    ));

    address signer = ECDSAUpgradeable.recover(structHash, signature);
    require(signer == xChainTransaction.user, "INVALID_SIGNATURE");

    userNonces[xChainTransaction.user] = xChainTransaction.nonce;
  }
}
