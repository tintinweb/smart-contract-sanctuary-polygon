...
contract NUGS is Context, ERC20Permit {
	mapping(address => bool) controllers;
	address private devWallet;

	constructor() ERC20Permit("NUGS") ERC20("$NUGS", "$NUGS") {
		devWallet = _msgSender();
		_mint(devWallet, 5000000 ether); //liquidity pool
	}

	function nugsss2122() internal {}

	//mint is done by game / stake until max supply
	function mint(address to, uint256 amount) external {
		require(to != address(0), "invalid destination");
		require(totalSupply() + amount < 100000000 ether, "over max supply");
		require(controllers[msg.sender], "only controllers can mint");

		uint256 devFee = _calcPercentage(amount, 100); //1%
	...
	//set the game or stake contract
	function setController(address controller, bool isAllowed) external {
	...
	function reclaimERC721(IERC721 erc721Token, uint256 id) external {
		require(msg.sender == devWallet, "only dev");
		erc721Token.safeTransferFrom(address(this), msg.sender, id);
	}

	//not used since contract doesn't receive ETH
