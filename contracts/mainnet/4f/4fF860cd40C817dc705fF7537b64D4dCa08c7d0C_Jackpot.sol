 ...
 */
contract Jackpot is IJackpot, AccessControl, TransferHelper {
    // token => balance
    mapping(address => uint256) public totalBalances;
    // user => (token => balance)
    mapping(address => mapping(address => uint256)) public jackpotBalances;
    ...
    function bet(
        ...
        jackpotBalances[owner][tokenAddress] += ownerAmount;
        jackpotBalances[bookmaker][tokenAddress] += bookmakerAmount;
        totalBalances[tokenAddress] += ownerAmount + bookmakerAmount;

        transferERC20From(tokenAddress, owner, jackpot, ownerAmount);
        transferERC20From(tokenAddress, bookmaker, jackpot, bookmakerAmount);
    ...
    function distribute(JackpotDistributer calldata distributer)
        ...
        returns (bool)
    {
        address tokenAddress = distributer.tokenAddress;
        address jackpot = distributer.jackpot;
        address owner = distributer.owner;
        address bookmaker = distributer.bookmaker;
        uint256 mintValue = distributer.mintValue;
        uint256 faceValue = distributer.faceValue;
        uint256 reward = distributer.reward;
        ...
        //WIN or WIN-Part
        if (reward > mintValue) {
            jackpotBalances[owner][tokenAddress] += reward - mintValue;
        }
        //LOST or LOST-Part
        else if (reward < mintValue) {
            jackpotBalances[owner][tokenAddress] -= mintValue - reward;
        }
        jackpotBalances[bookmaker][tokenAddress] -= faceValue - mintValue;

        if (faceValue > reward) {
            uint256 transferAmount = faceValue - reward;
            totalBalances[tokenAddress] -= transferAmount;
            transferERC20(tokenAddress, bookmaker, transferAmount);
        ...
        returns (bool)
    {
        ...
        address jackpot = distributer.jackpot;
        address owner = distributer.owner;
        address seller = distributer.seller;
        uint256 mintValue = distributer.mintValue;
        ...
        jackpotBalances[seller][tokenAddress] -= mintValue;
        jackpotBalances[owner][tokenAddress] += mintValue;
        return true;
    }
        ...
        returns (bool)
    {
        ...
        uint256 jackpotBalance = jackpotBalances[owner][tokenAddress];
        if (jackpotBalance < amount) {
            revert JackpotBalanceError(jackpot, amount, jackpotBalance);
        }
        jackpotBalances[owner][tokenAddress] -= amount;
        totalBalances[tokenAddress] -= amount;

        transferERC20(tokenAddress, owner, amount);
