 ...
 *        - Only the top voted `maxPoolsLength` pools will be voted on every week (this is due to a max vote count per NFT in Dystopia)
    ...
    ProxyImplementation
{
    ...
    // Internal helpers
    IVlPen internal vlPen;
    IVe internal ve;
    IVoterProxy internal voterProxy;
    IController internal controller;

    // Timestamps
    uint256 public dystopiaVotesLastCleared;
    ...
    function voteWeightTotalByAccount(address accountAddress)
        ...
        returns (uint256)
    {
        return vlPen.lockedBalanceOf(accountAddress);
    }
    ...
    function vote(
        ...
        // Fetch user's vlPen balance and use this as maximum user weight
    ...
    /**
     * @notice Clears votes on Dystopia for Penrose, only once per epoch
    ...
    function clearVotesOnDystopia() external {
        ...
        require(
            dystopiaVotesLastCleared + window < nextEpoch(),
            "Epoch vote already cleared"
        );

        // TODO: need to check if bribes are claimed

        dystopiaVotesLastCleared = block.timestamp;

        // Voting with empty weights will reset Penrose's vote on Dystopia
     ...
     * @dev This number is important as Dystopia has intensive gas constraints when voting
     */
