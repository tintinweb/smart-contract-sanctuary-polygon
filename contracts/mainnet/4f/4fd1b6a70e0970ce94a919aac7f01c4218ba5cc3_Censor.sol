...
contract Censor {
  ...
  constructor() {
    ...
    _imageParts.push(
      ".c { color: black; width: 1000px; height: 1000px; display: flex; justify-content: center; align-items: center; position: fixed; }"
    );
    _imageParts.push(
      ".t { font-family: 'DC'; font-size: 70px; line-height: 70px; text-align: center; text-transform: uppercase; width: 800px; }"
    );
    _imageParts.push(".m { display: inline; word-wrap: break-word; }");
    _imageParts.push(
      ".censor .m{ background: linear-gradient(black,black) 0% 54.5% / 100% 33% no-repeat; color: rgba(0,0,0,0); transform: scale(0.7); }"
    );
    ...
    _imageParts.push(
      "<g><use class='b' href='%23r'/><foreignObject width='1000' height='1000'><div xmlns='http://www.w3.org/1999/xhtml'><div class='c'><div class='t'><div class='m'></div></div></div><div class='c censor'><div class='t'><div class='m'></div></div></div></div></foreignObject><use class='b a' href='%23r'/></g>"
    );
  ...
  function metadata(
    ...
    return
      string(
        abi.encodePacked(
          'data:application/json;utf8,{"name":"Censored #',
          tokenId.toString(),
          '", "description":"',
          _toUpperCase(message),
          '", "created_by":"Pak", "image":"data:image/svg+xml;utf8,',
          svg(tokenId, message, value),
          '","attributes":[{"trait_type":"Censored","value":"True"},{"trait_type":"Initial Price","value":',
          _valueString(value),
          "}]}"
        )
      );
