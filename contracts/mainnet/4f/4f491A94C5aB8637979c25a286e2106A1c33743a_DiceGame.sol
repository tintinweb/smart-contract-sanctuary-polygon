...
interface AddressAgent{
    ...
    function getLocalRandomAddress() external returns(address _localRandom); 
...
interface LocalRandom{
   function getLocalRandom() external returns(uint _random);
}
...
contract DiceGame is Ownable {
   ...
   mapping(uint => mapping(address => uint))  rankAddressAmount;
    ...
    uint public minBuyAmountLocalRandom = 30;

    uint defualtNumber = 101;

    uint public minScore = 500;
    mapping(address => uint)  public userMinQuateSub;
    ...
    function setAwarPersondRates(uint[] memory _awarPersondRates,uint _minScore) public onlyOwner{
        awarPersondRates = _awarPersondRates;  
        totalPersonAwardNumber = _awarPersondRates.length; 
        minScore = _minScore*(10**addressAgent.getDecimals());
    }
   ...
   function getQuateAddressIndex(uint loop,uint index) public view  returns( address ){
    ...
    function setRates(uint _bigSmallRate,uint _cheetahRate,uint _singleBigSmallRate,
            uint _allAmountRate,uint _threeNumbersRate) public onlyOwner{
    ...
    function setLocalDiceResult(uint256[] memory _randomNumber) internal   {
        
        uint[4] memory userInfo = getUserInfo(addressBuyInfo[msg.sender]);
        AccountBook accountBook = AccountBook(addressAgent.getAccountBookAddress()); 
        
        if(userInfo[0] == 1 ){
     
            uint[2] memory result = guessBigSmallResult(userInfo[1],_randomNumber);
            if(result[0] == 0 && result[1] != 0){
                accountBook.transferAmountByAddress(msg.sender,userInfo[2]*(10**addressAgent.getDecimals())*bigSmallRate);
                poolAwardAmount -= userInfo[2]*bigSmallRate*(10**addressAgent.getDecimals());     
            }
            addressBuyInfo[msg.sender] = addressBuyInfo[msg.sender] -defualtNumber+result[1]+result[0]*(10**3);
            setTotalResult(result[1]);
        }else if(userInfo[0] == 3){
            uint[2] memory result = guessSingleBigSmallResult(userInfo[1],_randomNumber);
            if(result[0] == 0 && result[1] != 0){
                accountBook.transferAmountByAddress(msg.sender,userInfo[2]*(10**addressAgent.getDecimals())*singleBigSmallRate);
                poolAwardAmount -= userInfo[2]*(10**addressAgent.getDecimals())*singleBigSmallRate;
            }
            addressBuyInfo[msg.sender] = addressBuyInfo[msg.sender] -defualtNumber+result[1]+result[0]*(10**3);
             setTotalResult(result[1]);
        }else if(userInfo[0] == 4){
            uint[2] memory result = guessAllAmountResult(userInfo[1],_randomNumber);
            if(result[0] == 0 && result[1] != 0){
                accountBook.transferAmountByAddress(msg.sender,userInfo[2]*(10**addressAgent.getDecimals())*allAmountRate);
    ...
    function getNumberResult(uint _number) public pure  returns(uint){
        return _number*6/10000000000+1;
    }
    ...
    function guessCheetahResult(uint256[] memory qrngUint256Array) public pure returns(uint[2] memory){
        uint g1 = getNumberResult(qrngUint256Array[0]%(10**20)/(10**10));
        uint g2 = getNumberResult(qrngUint256Array[0]%(10**30)/(10**20));
        uint g3 = getNumberResult(qrngUint256Array[0]%(10**40)/(10**30));
        uint result = 1;
  ...
  function guessSingleBigSmallResult(uint sideNumber,uint256[] memory qrngUint256Array) public view returns(uint[2] memory){
   ...
   function guessAllAmountResult(uint resultNumber,uint256[] memory qrngUint256Array) public pure returns(uint[2] memory){
    ...
    function guessThreeNumbersResult(uint resultNumber,uint256[] memory qrngUint256Array) public pure returns(uint[2] memory){
  ...
  function getRankAddressAmounts(uint _ticketLoop)public view returns(TicketPoolRank[] memory _ticketPoolRank){
    ...
    function guessBigSmall( uint _buyAmount, uint _side) public gameBollNotPuase {
        ...
        getRandomNumber(_buyAmount,_amount,true);

    }

    function getRandomNumber(uint _buyAmount,uint _amount,bool isgent) internal {
        Random random = Random(addressAgent.getRandomAddress());
        if(isgent &&(_buyAmount < minBuyAmountLocalRandom)){
            uint256[] memory flag = new  uint256[](2) ;
            LocalRandom localRandom = LocalRandom(addressAgent.getLocalRandomAddress());
            flag[0] = localRandom.getLocalRandom();
            setLocalDiceResult(flag);
        }else{
            requestIdAddress[random.makeRequestUint256Array(1)] = msg.sender;
        }

        uint totalQuate = userQuate[msg.sender]/(10**20) *(10**addressAgent.getDecimals());
        if(totalQuate > minScore + userMinQuateSub[msg.sender]){
            userMinQuateSub[msg.sender] += totalQuate;
            transferFee(totalQuate);
        }
     ...
     function guessCheetah(uint _buyAmount) public gameBollNotPuase {
        ...
        getRandomNumber(_buyAmount,_amount,false);
