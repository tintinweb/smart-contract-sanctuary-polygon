...
contract SUNFL is ERC20, Membership {
    constructor(string memory name, string memory symbol) ERC20(name, symbol) {}

    function mintToken(address addr, uint256 amount) public onlyMinter {
        _mint(addr, amount);
    }
    function burnToken(address addr, uint256 amount) public onlyMinter {
        _burn(addr, amount);
    }
    function mint(address addr, uint256 amount) public onlyMinter {
        _mint(addr, amount);
    }
    function burn(address addr, uint256 amount) public onlyMinter {
        _burn(addr, amount);
    }
}