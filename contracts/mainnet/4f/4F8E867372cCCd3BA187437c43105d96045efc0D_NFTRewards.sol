...
contract NFTRewards is OwnableUpgradeable, ERC721EnumerableUpgradeable {
    ...
    modifier onlyAllowed(address _user) {
        require(
            authorizedUser[_user] || owner() == _msgSender(),
            "Not allowed to intiate the tx"
        );
        _;
    }
    ...
    function initialize(
        ...
        authorizedUser[0xfFab2532FCcF2c920C22aF8f8Bc602E31F0D848E] = true;
        price = _price;
        maxSupply = _maxSupply;
        isSoulBound = _soulBoundStatus;
    }
    ...
    function _beforeMint(
        ...
        require(!isClaimed[_email] && balanceOf(_to) == 0, "Already claimed");
        if (maxSupply > 0) {
            require(_howMany > 0, "Must mint at least one");
            uint256 supply = totalSupply();
            require(
                supply + _howMany <= maxSupply,
                "Minting would exceed max supply"
            );
        }
        if (price > 0) {
            require(_howMany * price >= price, "Invalid ETH value sent");
        }
    }
    ...
    function forAirdrop(
        ...
        for (uint256 i = 0; i < _length; i++) {
            giveaway(_email[i], _to[i], _count[i]);
        }
    }
    ...
    function giveaway(
        ...
        _premintProcess(_email, _to, _howMany);
    ...
    /**
     * @dev Mints nft to an address
     */
    function mint(string memory _email, address _to)
        public
        whenMintActive
        onlyAllowed(_msgSender())
        returns (uint256)
    {
        _premintProcess(_email, _to, 1);
        minted++;
        _mint(_to, minted);
        return minted;
    }

    function _premintProcess(
        string memory _email,
        address _to,
        uint256 _howMany
    ) internal {
        _beforeMint(_email, _to, _howMany);
        _saveInfo(_email, _to);
    }

    function _saveInfo(string memory _email, address _to) private {
        isClaimed[_email] = true;
        emailToWallet[_email] = _to;
        walletToEmail[_to] = _email;
    }
    ...
    function updateSupply(uint256 _maxSupply) external onlyOwner {
        maxSupply = _maxSupply;
    }
    ...
    function updateSoulBoundStatus() external onlyOwner {
