...
import './libraries/DistributionMapping.sol';
import './libraries/VestingCalculator.sol';
import './interfaces/ICertificate.sol';
import './libraries/Distributions.sol';
...
contract StockOptions is ReentrancyGuardUpgradeable {
    ...
    /**
     * @notice Distribution added event
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @param _address Address of the new participant
     * @param issueDate Issue date of the distribution plan as timestamps
     * @param tokenAmount Token amount to be vested
     * @param intervals Distribution intervals
     ...
     * @notice Distribution removed from the token pool event
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @param _address Address of the participant
     */
    ...
    /**
     * Participant distribution plan terminated
     * @notice Participant distribution plan termination from the token pool event
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution plan identifier
     * @param _address Address of the participant
     * @param tokensTransfered Tokens transfered to the participant
     * @param tokensReverted Tokens revered back
     */
    event DistributionTerminated(
        uint256 tokenPoolId,
        uint256 distributionId,
        address indexed _address,
    ...
    /**
     * @notice Distribution paused event
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @param _address Address of the participant
     */
    event DistributionPaused(uint256 tokenPoolId, uint256 distributionId, address _address);

    /**
     * @notice Distribution resumed event
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @param _address Address of the participant
     */
    event DistributionResumed(uint256 tokenPoolId, uint256 distributionId, address _address);
     ...
     */
    event TokensClaimed(uint256 tokenPoolId, address indexed _address, uint256 amount);

    mapping(uint256 => TokenPools.TokenPool) private tokenPools;
    // mapping(uint256 => mapping(uint256 => DistributionMapping.Map)) distributions;
    mapping(uint256 => DistributionMapping.Map) distributions;
    ...
    using DistributionMapping for DistributionMapping.Map;
    ...
    function createTokenPool(
        ...
        require(!tokenPools[tokenPoolId].exists, 'Token pool with provided ID already exists');
     ...
     */
    function createAdmin(uint256 tokenPoolId, address _address) external onlyAdmin(tokenPoolId) {
     ...
     */
    function removeAdmin(uint256 tokenPoolId, address _address) external onlyAdmin(tokenPoolId) {
    ...
    /**
     * @notice Add distribution plan for participant
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identificator
     * @param _address Address of the participant
     * @param issueDate Date when distribution plan starts as timestamp
     * @param tokenAmount Token amount to be vested
     * @param intervals Distribution intervals
    ...
    function createDistribution(
        ...
        require(
            !distributions[tokenPoolId].get(distributionId).exists,
            'Distribution with provided ID already exists'
        );
        ...
        uint256 certificateId = certificate.print(_address, uri);

        (uint256[] memory intervalsEndDate, uint256[] memory intervalsAmount) = intervals.serialize(
            tokenAmount
        );

        distributions[tokenPoolId].set(
            distributionId,
        ...
        tokenPools[tokenPoolId].reservedTokens = tokenPools[tokenPoolId].reservedTokens.add(
            tokenAmount
        );
    ...
    function removeDistribution(
        ...
        require(distributions[tokenPoolId].get(distributionId).exists, 'No distribution');
        require(
            distributions[tokenPoolId].get(distributionId).claimedTokens == 0,
            'Claimed tokens'
        );

        tokenPools[tokenPoolId].reservedTokens = tokenPools[tokenPoolId].reservedTokens.sub(
            distributions[tokenPoolId].get(distributionId).tokenAmount
        );

        certificate.burn(_address, distributions[tokenPoolId].get(distributionId).certificateId);

        distributions[tokenPoolId].remove(distributionId);

        emit DistributionRemoved(tokenPoolId, distributionId, _address);
    }
    ...
    function pauseDistribution(
        ...
        distributions[tokenPoolId].get(distributionId).pause(pauseDate);

        emit DistributionPaused(tokenPoolId, distributionId, _address);
    }

    /**
     * @notice Un-pause participant distribution plan
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @param _address Address of the participant
     * @param resumeDate Resume date as timestamp
     */
    function resumeDistribution(
        ...
        distributions[tokenPoolId].get(distributionId).resume(resumeDate);

        emit DistributionResumed(tokenPoolId, distributionId, _address);
    }
     ...
     * @param uri Certificate file URI
     */
    function terminateDistribution(
        uint256 tokenPoolId,
        uint256 distributionId,
        address _address,
        ...
        require(distributions[tokenPoolId].get(distributionId).exists, 'No distribution');
        require(
            distributions[tokenPoolId].get(distributionId).lastPausedAt == 0,
            'Distribution paused'
        );

        _terminateDistribution(tokenPoolId, distributionId, _address, terminationDate, uri);
    }

    /**
     * @notice Get distribution
     * @param tokenPoolId Token pool identifier
     * @param distributionId Distribution identifier
     * @return Distributions
     */
    function getDistribution(uint256 tokenPoolId, uint256 distributionId)
        external
        view
        onlyAdmin(tokenPoolId)
        returns (Distribution memory)
    {
        Distributions.Distribution storage distribution = distributions[tokenPoolId].get(
            distributionId
        );

        require(distribution.exists, 'No distribution');

        return _getDistribution(tokenPoolId, distributionId, distribution);
    }
     ...
     */
    function getDistributions(uint256 tokenPoolId)
        external
        view
        onlyAdmin(tokenPoolId)
        returns (Distribution[] memory)
    {
        uint256 len = distributions[tokenPoolId].size();

        if (len == 0) {
            return new Distribution[](0);
        }

        Distribution[] memory _distributions = new Distribution[](len);

        for (uint256 i; i < distributions[tokenPoolId].size(); i++) {
            uint256 key = distributions[tokenPoolId].getKeyAtIndex(i);

            _distributions[i] = _getDistribution(
                tokenPoolId,
                key,
                distributions[tokenPoolId].get(key)
            );
        }
    ...
    function getMyDistribution(uint256 tokenPoolId, uint256 distributionId)
        external
        view
        returns (Distribution memory)
    {
        require(
            distributions[tokenPoolId].get(distributionId)._address == msg.sender,
            'Wrong address'
        );
    ...
    /**
     * @notice Get participant distribution schedule
     * @param tokenPoolId Token pool identifier
     * @return Distribution plans
     */
    function getMyDistributions(uint256 tokenPoolId) external view returns (Distribution[] memory) {
        uint256 len;
        for (uint256 i; i < distributions[tokenPoolId].size(); i++) {
            uint256 key = distributions[tokenPoolId].getKeyAtIndex(i);
            if (distributions[tokenPoolId].get(key)._address == msg.sender) {
                len++;
            }
        }

        Distribution[] memory _distributions = new Distribution[](len);
        uint256 internalCounter;

        for (uint256 i; i < distributions[tokenPoolId].size(); i++) {
            uint256 key = distributions[tokenPoolId].getKeyAtIndex(i);

            if (distributions[tokenPoolId].get(key)._address == msg.sender) {
                _distributions[internalCounter] = _getDistribution(
                    tokenPoolId,
                    key,
                    distributions[tokenPoolId].get(key)
                );
     ...
     */
    function getMyClaims() external view returns (Claims.Claim[] memory) {
    ...
    function claimTokens(
        ...
        require(
            distributions[tokenPoolId].get(distributionId).terminatedDate == 0,
        ...
        uint256 amount = calculateClaimableTokens(tokenPoolId, distributionId);

        require(amount > 0, 'Nothing to claim');
        require(
            distributions[tokenPoolId].get(distributionId).canClaim(block.timestamp),
            'Can not claim'
        );

        _claimTokens(tokenPoolId, distributionId, msg.sender, block.timestamp, amount, _uri);
    }

    function _terminateDistribution(
        uint256 tokenPoolId,
        uint256 distributionId,
        address _address,
        uint256 terminationDate,
        string memory uri
    ) private returns (uint256 distributionTokensToClaim, uint256 distributionTokensReverted) {
        require(
            terminationDate > distributions[tokenPoolId].get(distributionId).issueDate,
            'Termination date before issue date'
        );

        uint256 _distributionTokensToClaim;

        if (distributions[tokenPoolId].get(distributionId).canClaim(terminationDate)) {
            // get tokens that can be vested with termination date
            uint256 claimableTokens = distributions[tokenPoolId]
                .get(distributionId)
                .getClaimableTokens(terminationDate);
            // get claimed tokens
            uint256 claimedTokens = distributions[tokenPoolId].get(distributionId).claimedTokens;
            // calculate tokens that are claimable
            _distributionTokensToClaim = claimableTokens.sub(claimedTokens);
        }

        if (_distributionTokensToClaim > 0) {
            _claimTokens(
                tokenPoolId,
                distributionId,
                _address,
                terminationDate,
                _distributionTokensToClaim,
                uri
            );
        }

        uint256 _distributionTokensReverted = distributions[tokenPoolId]
            .get(distributionId)
            .tokenAmount
            .sub(distributions[tokenPoolId].get(distributionId).claimedTokens);

        tokenPools[tokenPoolId].reservedTokens = tokenPools[tokenPoolId].reservedTokens.sub(
            _distributionTokensReverted
        );

        distributions[tokenPoolId].get(distributionId).terminatedDate = terminationDate;

        emit DistributionTerminated(
            tokenPoolId,
            distributionId,
            _address,
            _distributionTokensToClaim,
        ...
        return (_distributionTokensToClaim, _distributionTokensReverted);
    }

    function _getDistribution(
        uint256 tokenPoolId,
        uint256 distributionId,
        Distributions.Distribution storage distribution
    ) private view returns (Distribution memory) {
        return
            Distribution(
                distribution._address,
                distribution.issueDate,
                distribution.tokenAmount,
                distribution.terminatedDate,
                distribution.getIntervals(),
                distribution.claimedTokens,
                calculateClaimableTokens(tokenPoolId, distributionId),
                distribution.certificateId
            );
    }

    function calculateClaimableTokens(uint256 tokenPoolId, uint256 distributionId)
        private
        view
        returns (uint256)
    {
        uint256 claimableTokens = distributions[tokenPoolId].get(distributionId).getClaimableTokens(
            block.timestamp
        );

        if (claimableTokens < distributions[tokenPoolId].get(distributionId).claimedTokens) {
            return 0;
        }

        return claimableTokens.sub(distributions[tokenPoolId].get(distributionId).claimedTokens);
    }
    ...
    function _claimTokens(
        ...
        distributions[tokenPoolId].get(distributionId).claimedTokens = distributions[tokenPoolId]
            .get(distributionId)
            .claimedTokens
            .add(amount);

        tokenPools[tokenPoolId].claimedTokens = tokenPools[tokenPoolId].claimedTokens.add(amount);

        claims[_address].push(Claims.Claim(distributionId, claimDate, amount));

        tokenPools[tokenPoolId].reservedTokens = tokenPools[tokenPoolId].reservedTokens.sub(amount);

        balances[tokenPoolId].transfer(address(this), _address, amount);

        setParticipantCertificate(tokenPoolId, distributionId, _address, uri);
    ...
    function setParticipantCertificate(
        ...
        require(distributions[tokenPoolId].get(distributionId).exists, 'No distribution');
        require(
            distributions[tokenPoolId].get(distributionId)._address == _address,
            'Wrong address'
        );

        uint256 certificateId = distributions[tokenPoolId].get(distributionId).certificateId;
        require(certificateId != 0, 'No certificate');

        certificate.setUri(certificateId, _uri);
    }

    function resetUri(uint256 tokenPoolId, uint256 certificateId) external onlyAdmin(tokenPoolId) {
        string memory uri = certificate.uri(certificateId);

        if (!uri.contains('ipfs://') && !uri.contains('metadata.json')) {
            string memory newUri = string(abi.encodePacked('ipfs://', uri, '/metadata.json'));

            certificate.setUri(certificateId, newUri);
        }
    }
}
...
library DistributionMapping {
    // Iterable mapping from address to uint;
    struct Map {
        uint256[] keys;
        mapping(uint256 => Distributions.Distribution) values;
    ...
    function get(Map storage map, uint256 key)
        internal
        view
        returns (Distributions.Distribution storage)
    {
        return map.values[key];
    }

    function getKeyAtIndex(Map storage map, uint256 index) internal view returns (uint256) {
    ...
    function set(
        Map storage map,
        uint256 key,
        Distributions.Distribution memory val
    ) internal {
        if (map.inserted[key]) {
            map.values[key] = val;
        } else {
...
interface ICertificate is IERC1155MetadataURIUpgradeable {
    ...
    function burn(address from, uint256 distributionId) external;
...
library Distributions {
    ...
    function getClaimableTokens(Distributions.Distribution storage self, uint256 claimDate)
        internal
        view
        returns (uint256)
    {
        ...
        return VestingCalculator.calculateClaimableTokens(claimDate, self.issueDate, intervals);
    }

    function getIntervals(Distributions.Distribution storage self)
        internal
        view
    ...
    function canClaim(Distributions.Distribution storage self, uint256 atDate)
        internal
        view
        returns (bool)
    {
    ...
    function pause(Distributions.Distribution storage self, uint256 pauseDate) internal {
        require(self.lastPausedAt == 0, 'Already paused');
        self.lastPausedAt = pauseDate;
    }

    function resume(Distributions.Distribution storage self, uint256 resumeDate) internal {
        require(self.lastPausedAt > 0, 'Not paused');

        uint256 pausedFor = resumeDate.sub(self.lastPausedAt);
...
library Intervals {
    ...
    function serialize(Intervals.Interval[] memory self, uint256 tokenAmount)
        internal
        pure
        returns (uint256[] memory intervalsEndDate, uint256[] memory intervalsAmount)
    {
...
library StringExtension {
    function contains(string memory self, string memory what) internal pure returns (bool) {
        bytes memory whatBytes = bytes(what);
        bytes memory whereBytes = bytes(self);
