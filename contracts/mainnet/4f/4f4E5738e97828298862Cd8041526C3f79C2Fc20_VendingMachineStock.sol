...
contract VendingMachineStock {
  ...
  constructor(address _vm) {
    vm = VendingMachine(_vm);
  }

  function paginatedInventory(uint16 page, uint16 pageSize) public view returns (Asset[] memory) {
    VendingMachine.Asset[] memory original = vm.paginatedInventory(page, pageSize);
    Asset[] memory parsed = new Asset[](original.length);

    for (uint256 i = 0; i < parsed.length; i++) {
      VendingMachine.Asset memory asset = original[i];
      string memory metadata;
      if (asset.assetType == 721) {
        Type721 c = Type721(asset.assetAddress);
        metadata = c.tokenURI(asset.tokenId);
      } else {
        Type1155 c = Type1155(asset.assetAddress);
        metadata = c.uri(asset.tokenId);
      }
      parsed[i] = Asset(
        asset.assetAddress,
        asset.assetType,
