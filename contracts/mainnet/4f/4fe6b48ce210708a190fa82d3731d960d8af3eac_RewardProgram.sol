...
// File: DelegatorRewards/openzeppelin-solidity/contracts/GSN/Context.sol
...
// File: DelegatorRewards/openzeppelin-solidity/contracts/access/Ownable.sol
...
// File: DelegatorRewards/openzeppelin-solidity/contracts/utils/Address.sol
...
// File: DelegatorRewards/openzeppelin-solidity/contracts/math/SafeMath.sol
...
// File: DelegatorRewards/openzeppelin-solidity/contracts/token/ERC20/IERC20.sol
...
// File: DelegatorRewards/openzeppelin-solidity/contracts/token/ERC20/SafeERC20.sol
...
// File: DelegatorRewards/RewardProgram.sol
...
contract RewardProgram is Ownable {
    ...
    uint256 public shortRewardTimeoutPeriod = 28 days;
    ...
    event RequestReward(address target, uint256 amount, bool isFullWithdrawal);
    event ReceiveReward(address target, uint256 amount, bool indexed isWithdrowedByNominator);
    ...
    function calculateShortAmount(uint256 amount) internal view returns (uint256) {
        return amount.mul(shortRewardPercentage).div(10);
    }

    function getRequestAmountWithError(address target, uint256 item)
        internal
        view
        returns (
            uint256 amount,
    ...
    function transferOrApproveRewardToAddress(
        address target,
        uint256 amount,
        bool isWithdrowedByNominator,
        bool isApprove,
        ...
        receivedRewards[target].push(
            AmountWithTime(amount, block.timestamp, isWithdrowedByNominator, isFullWithdrawal)
        );
        if (isApprove) {
    ...
    function receiveResidue(
        ...
        (
            ...
            bool isWithdrowedByNominator,
            bool isFullWithdrawal
        ) = getRequestAmountWithError(target, item);
    ...
    function receiveReward(uint256 item) public {
    ...
    function requestTokensByMerkleProof(
        bytes32[] memory _proof,
        uint256 _amount,
        bool _isFullWithdrawal
    ) public isCancelable returns (bool) {
        require(lastRewardRequestTime[msg.sender] < merkleRootLastUpdateTime, "already withdrawn in this period");
        require(_proof.length < 20, "proof too large");
