...
contract STOA is Initializable, ReentrancyGuardUpgradeable, ISTOA, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    /// @notice Indexing of accepted limits
    mapping(uint256 => bool) public acceptedDurations;
    ...
    struct Period {
        ...
        CountersUpgradeable.Counter bronze;
        CountersUpgradeable.Counter silver;
        CountersUpgradeable.Counter gold;
        CountersUpgradeable.Counter userCount;
    }
    ...
    /// @notice Initializes the STOA instance, stores user performance and period/reward settings
    ...
    function initialize(
        ...
        Period memory _currentPeriod;
        _currentPeriod.index = 1;
        _currentPeriod.start = block.timestamp;
        _currentPeriod.end = block.timestamp + periodDuration;

        _currentPeriod.totalSupply = rewardsForNextPeriod;
        _currentPeriod.rewardsPaid = 0;
        _currentPeriod.baseRate = _calculateRewardsRateForPeriod(rewardsForNextPeriod, rewardRates[0], periodDuration);
        _currentPeriod.stakeRate = _calculateRewardsRateForPeriod(rewardsForNextPeriod, rewardRates[1], periodDuration);
        _currentPeriod.accomplishmentRewards = rewardsForNextPeriod.mul(rewardRates[2]).div(1e18);
        _currentPeriod.commitmentRewards = rewardsForNextPeriod.mul(rewardRates[3]).div(1e18);

        _currentPeriod.userCount = currentPeriod.userCount;
    ...
    /// @notice Emitted when meditation durations are updated
    event AcceptedDurationsUpdated(uint256[] durations, bool added);
    ...
    function _updatePeriodExpires() internal {
        ...
        uint256 rewardsForNextPeriod = _calculateRewardForPeriod();
        CountersUpgradeable.Counter memory bronze;
        CountersUpgradeable.Counter memory silver;
        CountersUpgradeable.Counter memory gold;
    ...
    function addMeditationLimits(uint256[] memory _limitsInSeconds) external onlyRole(CONFIG_ROLE) {
        uint256 len = _limitsInSeconds.length;
        uint256[] memory cached = _limitsInSeconds;
        for (uint256 i = 0; i < len; i++) {
            acceptedDurations[cached[i]] = true;
        }
        emit AcceptedDurationsUpdated(cached, true);
    }

    function removeMeditationLimits(uint256[] memory _limitsInSeconds) external onlyRole(CONFIG_ROLE) {
        uint256 len = _limitsInSeconds.length;
        uint256[] memory cached = _limitsInSeconds;
        for (uint256 i = 0; i < len; i++) {
            acceptedDurations[cached[i]] = false;
        }
        emit AcceptedDurationsUpdated(cached, false);
    }
    ...
    function updateTreasury(address newTreasury) external onlyRole(CONFIG_ROLE) {
        treasury = newTreasury;
        emit AddressUpdated("Treasury", newTreasury);
    }

    function updateRewarder(address newRewarder) external onlyRole(CONFIG_ROLE) {
        rewarder = newRewarder;
        emit AddressUpdated("Rewarder", newRewarder);
    }
    ...
    modifier meditationWithinLimits(uint256 duration) {
        ...
        require(acceptedDurations[duration], "Invalid meditation period duration");
        _;
    }
