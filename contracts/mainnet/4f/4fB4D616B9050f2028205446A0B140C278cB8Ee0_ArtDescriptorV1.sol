...
import "./ArtDescriptor.sol";
import "@openzeppelin/contracts/utils/Strings.sol";

contract ArtDescriptorV1 is ArtDescriptor {
    struct Color {
        string value;
        string name;
    }
    ...
    function tokenURI(uint256 tokenId, uint256 seed) external pure override returns (string memory) {
        ...
        string memory description = 'ChainPeople';
            ...
            abi.encodePacked(
                ...
                Base64.encode(
                    bytes(
                        abi.encodePacked(
                            '{',
                            '"name":"ChainPeople #', tokenId.toString(), '",',
    ...
    function getColor(uint256 seed) private pure returns (Color memory) {
        ...
        if (seed == 22) {
            return Color("#9b19f5", "Purple");
        }
        if (seed == 23) {
            return Color("#dc0ab4", "Magenta");
        }
        if (seed == 24) {
            return Color("#f46a9b", "Cyclamen");
        }
        if (seed == 25) {
            return Color("#bd7ebe", "Violet");
        }
        if (seed == 26) {
            return Color("#fdcce5", "Rose");
        }
        if (seed == 27) {
            return Color("#FCE74C", "Gargoyle");
        }
        if (seed == 28) {
            return Color("#eeeeee", "Gray");
        }
        if (seed == 29) {
            return Color("#7f766d", "Silver");
        }
