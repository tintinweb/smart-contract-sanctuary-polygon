    ...
    CrossmintMintAPI
{
    ...
    /**
     * @dev Set the baseURI of the token
     * @param _baseURI The new name of the token.
     */
    function setBaseURI(
    ...
    /**
     * @dev Set a custom tokenURI for a token that will override the baseURI
     ...
     */
    function mintTo(address recipient) public onlyRole(DEFAULT_ADMIN_ROLE) {
        require(hasSetBaseURI, "BaseURI must be set to mint without tokenURI");

        currentId.increment();
        _mint(recipient, currentId.current());
    }

    /**
     * @dev Mint a token that has a custom tokenURI which overrides the baseURI
     * @param recipient The address to receive the newly minted token.
    ...
    function tokenURI(
        ...
        // If there is no specific URI for this token, return the base URI + tokenId
        if (bytes(_tokenURI).length == 0) {
            return string(abi.encodePacked(baseURI, tokenId.toString()));
        }

        // If there is a specific URI for this token, return it
        return _tokenURI;
    }
