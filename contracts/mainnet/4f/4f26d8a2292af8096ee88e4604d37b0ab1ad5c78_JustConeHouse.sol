...
contract JustConeHouse is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0.5 ether;
    uint256 public maxMints = 155;
    ...
    constructor()
        ERC721("Just Cone House", "Coneheads")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 155) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiciaekdilhvkkxfsket63citg5ze55f3r726cp7reaeslui4sle3u/", Strings.toString(tokenId), ".json")
            );
    }
