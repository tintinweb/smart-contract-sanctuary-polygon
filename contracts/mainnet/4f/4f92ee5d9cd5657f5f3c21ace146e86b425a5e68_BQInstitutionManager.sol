...
function signCertificates( uint[] memory certificatesIds) public   requireSignCertPermission{
    for(uint i = 0; i < certificatesIds.length; i++) {
...
function setCertificateVisibility(  uint certificateId, bool showCertificateContent) public  {
    require(usersList[msg.sender].canCreateCertificates || ownerOf(certificateId) == msg.sender, "Unauthorized");
