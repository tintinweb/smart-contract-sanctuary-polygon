...
import {ITroopFactory} from "./interfaces/ITroopFactory.sol";
import {ITroop} from "./interfaces/ITroop.sol";
import {Troop} from "./Troop.sol";
import {IFundDeployer} from "./enzyme/IFundDeployer.sol";
import {EnzymeUtils} from "./enzyme/EnzymeUtils.sol";
import {ToggleableAdmin} from "./utils/ToggleableAdmin.sol";

contract TroopFactory is ITroopFactory, Ownable, ToggleableAdmin {
    /// @notice LensHub
    address public immutable lensHub;
    /// @notice ApprovalFollowModule that is already whitelisted by LensHub
    address public immutable approvalFollowModule;
    /// @notice Count of how many Troops have been deployed (by this factory)
    uint256 public troopCount;
    /// @notice TroopFactory troop deployment index => Troop contract mapping
    mapping(uint256 => Troop) public troops;
    /// @notice Enzyme's Fund Deployer, semi-permanent (?)
    address public enzymeFundDeployer;
    /// @notice Troop implementation contract
    address public troopImplementation;
    /// @notice Troop token implementation contract
    address public tokenImplementation;
    /// @notice Timelock implementation contract
    address public timelockImplementation;
    /// @notice TroopGovernor implementation contract
    address public troopGovernorImplementation;
    /// @notice Enzyme policy: Restrict accounts that can deposit to vault/mint shares
    ///     TODO: Inject this
    address public constant ALLOWED_DEPOSIT_RECIPIENTS_POLICY =
        0xE1853502e2eA2b7C14C5E89169C63065f5A459fF;
    /// @notice Enzyme policy: Restrict accounts that receive vault share transfers
    ///     TODO: Inject this
    address public constant ALLOWED_SHARES_TRANSFER_RECIPIENTS_POLICY =
        0x3B6913A8ED4595919A6b4A9022208ceDE20194BD;
    /// @notice Enzyme fee: Entrance tax, paid directly to the protocol (us)
    address public constant ENTRANCE_RATE_DIRECT_FEE =
        0x88c9a11c7bB8bC274388d0db864ab87C14fb78B8;

    constructor(
        address lensHubProxyAddress,
        address approvalFollowModuleAddress,
        address enzymeFundDeployer_,
        address troopImplementation_,
        address tokenImplementation_,
        address timelockImplementation_,
        address troopGovernorImplementation_
    ) {
        lensHub = lensHubProxyAddress;
        approvalFollowModule = approvalFollowModuleAddress;
        enzymeFundDeployer = enzymeFundDeployer_;
        troopImplementation = troopImplementation_;
        tokenImplementation = tokenImplementation_;
        timelockImplementation = timelockImplementation_;
        troopGovernorImplementation = troopGovernorImplementation_;
    }

    /// @notice Setup Lens profile for a Troop
    /// @param handle Lens handle e.g. "TroopOfApes.lens"
    /// @param to address to mint Lens ProfileNFT to
    function createLensProfile(
        string memory handle,
        address to
    ) private returns (uint256) {
        // Caution: Must be whitelisted by deployed Lens Hub
        DataTypes.CreateProfileData memory profileData;
        profileData.to = to;
        profileData.handle = handle;
        profileData.imageURI = ""; // TODO
        // TODO: This should be the whitelisted ApprovalFollowModule
        profileData.followModule = address(approvalFollowModule);
        profileData.followModuleInitData = bytes("");
        profileData.followNFTURI = "";
        return ILensHub(lensHub).createProfile(profileData);
    }

    /// @notice Create a Troop contract that controls a Lens profile and an Enzyme vault
    /// @param troopHandle_ Handle that will be used for creating Lens profile
    function createTroop(
        string memory troopHandle_,
        string memory troopName_,
        string memory troopSymbol_,
        uint256 actionTimelockSeconds_,
        uint256 votingDelay_,
        ...
        // Mint Lens profile to self, we'll transfer this to the deployed Troop later
        uint256 lensProfileId = createLensProfile(troopHandle_, address(this));
        return
            createTroopWithExistingLensProfile(
                lensProfileId,
                troopName_,
                troopSymbol_,
                actionTimelockSeconds_,
    ...
    /// @notice Create a Troop contract that controls a Lens profile and an Enzyme vault.
    ///     The profile NFT must have this contract as an approved operator.
    /// @param lensProfileId Lens profile ID that the Troop should assume control of
    /// @param troopName Name of this Troop
    /// @param troopSymbol Symbol for this Troop vault's ERC-20 token
    /// @param actionTimelockSeconds Timelock duration (seconds) before a finalised proposal can be executed
    /// @param votingDelay Interval of time (seconds) before a proposal becomes active
    /// @param votingPeriod Interval of time (seconds) when Troop members can vote on an active proposal
    /// @param quorumNumerator The value x in q = x/100, where q determines the minimum fraction of FollowNFT
    ///     holders that a vote must reach in order for a proposal to reach quorum.
    /// @param denominationAsset Address of ERC-20 to use as the denomination asset for the underlying
    ///     Enzyme vault.
    /// @param sharesActionTimelock Minimum duration (seconds) that any member must wait in between any
    ///     interactions with the Enzyme vault.
    function createTroopWithExistingLensProfile(
        uint256 lensProfileId,
        string memory troopName,
        string memory troopSymbol,
        uint256 actionTimelockSeconds,
        uint256 votingDelay,
        uint256 votingPeriod,
        uint256 quorumNumerator,
        ...
        // Deploy Troop proxy
        ERC1967Proxy troopProxy = new ERC1967Proxy(
            troopImplementation,
            bytes("")
        );
        Troop troop = Troop(address(troopProxy));
        // Set ownership of Lens profile to newly-deployed Troop by transferring the Profile NFT
        IERC721 profileNFT = IERC721(address(lensHub));
        profileNFT.transferFrom(
            profileNFT.ownerOf(lensProfileId),
            address(troop),
            lensProfileId
        );

        // Initialise Troop proxy
        ITroop.GovernorParams memory governorParams = ITroop.GovernorParams({
            actionTimelockSeconds: actionTimelockSeconds,
            votingDelay: votingDelay,
            votingPeriod: votingPeriod,
            quorumNumerator: quorumNumerator,
            timelockImplementation: timelockImplementation,
            troopGovernorImplementation: troopGovernorImplementation
        });
        ITroop.VaultParams memory vaultParams = ITroop.VaultParams({
            enzymeFundDeployer: enzymeFundDeployer,
            denominationAsset: denominationAsset,
            sharesActionTimelock: sharesActionTimelock,
            feeManagerConfigData: encodeFeeManagerConfigData(),
            policyManagerConfigData: encodePolicyManagerConfigData(
                address(troop)
            )
        });
        troop.init(
        ...
        emit TroopDeployed(troopCount, address(troop));

        troops[troopCount] = troop;
        return (troopCount++, address(troop));
    }

    /// @notice Encode configs for Enzyme fees manager
    /// @return encoded config data to pass into Enzyme's fund deployer
    function encodeFeeManagerConfigData() private view returns (bytes memory) {
        address[] memory fees = new address[](1);
        fees[0] = ENTRANCE_RATE_DIRECT_FEE; // Troop global protocol fee
        bytes[] memory feeConfigs = new bytes[](1);
        feeConfigs[0] = EnzymeUtils.encodeEntranceFeeConfig(50, address(this));
        return abi.encode(fees, feeConfigs);
    }

    /// @notice Encode configs for Enzyme policy manager such that only the given Troop
    ///     address can deposit or mint/transfer shares
    ///     TODO: Could probably move this offchain to save gas
    /// @param troop Address of Troop
    /// @return encoded config data to pass into Enzyme's fund deployer
    function encodePolicyManagerConfigData(
        address troop
    ) private pure returns (bytes memory) {
        address[] memory allowedAddresses = new address[](1);
        allowedAddresses[0] = address(troop);
        address[] memory policies = new address[](2);
        policies[0] = ALLOWED_DEPOSIT_RECIPIENTS_POLICY;
        policies[1] = ALLOWED_SHARES_TRANSFER_RECIPIENTS_POLICY;
        bytes[] memory policyConfigs = new bytes[](2);
        policyConfigs[0] = EnzymeUtils.encodeAddressListRegistryPolicyConfig(
            allowedAddresses
        );
        policyConfigs[1] = EnzymeUtils.encodeAddressListRegistryPolicyConfig(
            allowedAddresses
        );
        // https://github.com/enzymefinance/protocol/blob/v4/contracts/release/extensions/policy-manager/PolicyManager.sol#L145
        return abi.encode(policies, policyConfigs);
    }

    /// @notice Set the Enzyme's Fund Deployer, semi-permanent (?)
    /// @param enzymeFundDeployer_ New enzymeFundDeployer
    function setEnzymeFundDeployer(
        address enzymeFundDeployer_
    ) external onlyOwner {
        enzymeFundDeployer = enzymeFundDeployer_;
    }

    /// @notice Set the Troop implementation contract
    /// @param troopImplementation_ New troopImplementation
    function setTroopImplementation(
        address troopImplementation_
    ) external onlyOwner {
        troopImplementation = troopImplementation_;
    }

    /// @notice Set the Troop token implementation contract
    /// @param tokenImplementation_ New tokenImplementation
    ...
    /// @notice Set the Timelock implementation contract
    /// @param timelockImplementation_ New timelockImplementation
    function setTimelockImplementation(
        address timelockImplementation_
    ) external onlyOwner {
        timelockImplementation = timelockImplementation_;
    }

    /// @notice Set the TroopGovernor implementation contract
    /// @param troopGovernorImplementation_ New troopGovernorImplementation
    function setTroopGovernorImplementation(
        address troopGovernorImplementation_
    ) external onlyOwner {
        troopGovernorImplementation = troopGovernorImplementation_;
    }
...
library EnzymeUtils {
    ...
    function encodeEntranceFeeConfig(uint256 rateBps, address recipient)
        external
        pure
        returns (bytes memory)
    {
...
contract ToggleableAdmin {
    /// @notice Determines whether the the guard is enabled or not
    bool public isAdminGuardEnabled;
    /// @notice Whitelisted admins
    mapping(address => bool) public isAdmin;

    constructor() {
        isAdminGuardEnabled = true;
    ...
    modifier onlyAdmin() {
        require(
            !isAdminGuardEnabled || isAdmin[msg.sender],
    ...
    /// @param isAuthorised `true` to authorise account as an admin
    function setAdmin(address guy, bool isAuthorised) public onlyAdmin {
        isAdmin[guy] = isAuthorised;
    }

    /// @notice Toggle whether the admin guard is enabled or not
    /// @param isEnabled `true` to enable admin guard, `false` otherwise
    function setAdminGuard(bool isEnabled) public onlyAdmin {
        isAdminGuardEnabled = isEnabled;
    }
}
...
contract ForceSendEther {
    ...
    function forceSendEther(address payable target) external {
        selfdestruct(target);
    }
}