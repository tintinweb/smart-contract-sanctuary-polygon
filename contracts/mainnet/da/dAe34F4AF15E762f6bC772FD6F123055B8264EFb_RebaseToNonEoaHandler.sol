...
contract RebaseToNonEoaHandler is Initializable, Governable {
    ...
    mapping(address => uint) public gaugeCashAmount;
    mapping(address => uint) public bribeCashAmount;
    ...
    function addSatinContract(address _satinPoolAddress, uint256 _guageShare, uint256 _bribeShare, uint256 _treasuryShare) external onlyGovernor {
        ...
        if (_gaugeAddress == IVoter(satinVoter).SATIN_CASH_LP_GAUGE()) {
    ...
    function process() external onlyVaultOrGovernor {
        for (uint256 i = 0; i < allContracts.length; i++) {
            ...
            if (_contractInfo.isSupported && _contractInfo.isSatin) {
                ...
                uint _gaugeShare = gaugeCashAmount[_contractInfo.gauge];
                ...
                uint _bribeShare = bribeCashAmount[_contractInfo.bribe];
    ...
    function _allowFeeGauge(address _recipient, uint256 _skimmableCashAmount, uint256 _share) internal returns (uint _shareAmount) {
        _shareAmount = (_skimmableCashAmount * _share) / BASE_FEE;
        if (SATIN_CASH_LP_GAUGE == _recipient) {
            uint satinCashLPVeShare = IVoter(satinVoter).calculateSatinCashLPVeShare(_shareAmount);
    ...
    function _allowFeeBribe(address _recipient, uint256 _skimmableCashAmount, uint256 _share) internal returns (uint _shareAmount) {
        _shareAmount = (_skimmableCashAmount * _share) / BASE_FEE;
...
interface IVoter {
    ...
    function SATIN_CASH_LP_GAUGE() external view returns (address);
