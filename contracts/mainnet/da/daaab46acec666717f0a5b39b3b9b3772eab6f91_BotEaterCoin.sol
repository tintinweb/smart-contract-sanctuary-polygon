...
contract BotEaterCoin is Context, IERC20Metadata, ERC20, Ownable {
    ...
    mapping(address => bool) public walletIsLP;
  

    constructor(address feesWallet) ERC20("Bot Eater", "EATBOTS") {
     
        
        _feesWallet = feesWallet;
   
        _mint(_msgSender(), 420_069_000_000_000 ether);
       
    }


    function setWalletIsLP(address[] memory addresses) public onlyOwner {
      for (uint i=0; i<addresses.length; i++) {
        walletIsLP[addresses[i]] = true;
      }
    }
    ...
    function isBuy(address _from) internal view returns(bool) {
        ...
        return walletIsLP[_from] == true;
    }

    function isSale(address _to) internal view returns(bool) {
        return walletIsLP[_to] == true;
    }
