...
contract Vault is Ownable, Signers, VerifySigEIP712 {
    ...
    {
        (
            IClipperExchangeInterface exchange,address user,IERC20 srcToken,IERC20 dstToken,uint256 input,
            uint256 output,uint256 goodUntil,bytes32 r,bytes32 vs
        ) = abi.decode(_data[4:],
                (
    ...
    function fillOrderRFQTo(uint minOut, bytes calldata _data) external payable {
        (OrderLib.OrderRFQ memory order, , , ) = abi.decode(_data[4:], (OrderLib.OrderRFQ, bytes, uint256, address));

        _isNativeSwap(IERC20(order.makerAsset), order.makingAmount);

        (bool succ, bytes memory _data) = address(ROUTER).call{value: msg.value}(_data);
        if (succ) {
            (uint256 filledMakingAmount, uint256 filledTakingAmount, ) = abi.decode(_data, (uint256, uint256, bytes32));
            require(filledTakingAmount >= minOut);

            emit Swap(msg.sender, order.makerAsset,order.takerAsset, filledMakingAmount, filledTakingAmount);
        } else {
            revert();
        }
    }

    function fillOrderRFQToBridge(uint minOut, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        (OrderLib.OrderRFQ memory order, , , ) = abi.decode(_data[4:], (OrderLib.OrderRFQ, bytes, uint256, address));
        ...
        if (succ) {
            (uint256 filledMakingAmount, uint256 filledTakingAmount, ) = abi.decode(_data, (uint256, uint256, bytes32));
            require(filledTakingAmount >= minOut);
            _isNotNativeBridge(IERC20(order.takerAsset), filledTakingAmount, bDesc);
        } else {
            revert();
        }
    }

    // spentAmount 와 desc.amount차이가 있을경우 차이만큼 유저에게 재전송필요.
    ...
    function unoBridge(
        ...
        (IERC20 srcToken, uint256 amount, uint256 b, uint256[] memory c) = abi
            .decode(_data[4:], (IERC20, uint256, uint256, uint256[]));
    ...
    function viewSwap(bytes calldata _data) external view returns (bytes4 selector) {
        bytes4 selector = abi.decode(_data[:4],(bytes4));
    }
    ...
    function relayUno(uint256 minOut,address toToken,bytes calldata _data) external onlyOwner {
        (address user,IERC20 srcToken,uint256 amount,uint256 b,uint256[] memory c) = abi.decode(_data[4:],(address, IERC20, uint256, uint256, uint256[]));
