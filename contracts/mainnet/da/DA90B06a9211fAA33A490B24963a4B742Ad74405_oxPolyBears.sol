...
// File: contracts/oxPolyBears.sol
...
contract oxPolyBears is ERC721A, Ownable{

    string public baseURI;
    uint256 constant public MAX_SUPPLY = 7777;
    uint256 public freeMintPerWallet = 15;
    ...
    function mint(uint256 _amount) external payable publicSaleActive {
        ...
        require(totalSupplyPublic + _amount <= MAX_SUPPLY, "Not enough mints left");

        uint256 userMintsTotal = _numberMinted(msg.sender);
 
    if (msg.sender != owner()) {
        require(_amount <= maxMintAmount, "Too many mints per tx");
        if (!isFreeMintActive) {
            uint256 price = cost;
            ...
            require(userMintsTotal + _amount <= freeMintPerWallet, "Max mint limit");
            totalSupplyFree += _amount;  
        }
        } else if (isFreeMintActive) {
            totalSupplyFree += _amount;
        }
   ...
   function setFreeMintActive(bool _state) public onlyOwner {
    ...
    function setPerFreeMintAmount(uint256 _newmaxFreeAmount) public onlyOwner {
        freeMintPerWallet = _newmaxFreeAmount;
    }
