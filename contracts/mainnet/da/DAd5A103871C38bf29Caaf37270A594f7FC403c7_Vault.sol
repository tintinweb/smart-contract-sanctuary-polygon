    ...
    function swapBridge(uint minOut, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        ...
        if (succ) {
            ...
            (bool success, bytes memory _da) = address(BRIDGE).delegatecall(
                abi.encodeWithSignature("send(address,address,uint256,uint64,uint64,uint32)",bDesc.receiver,address(desc.srcToken),desc.amount,bDesc.dstChainId,bDesc.nonce,bDesc.maxSlippage)
            );
