...
contract MaxityMarketPlace is Ownable,Pausable ,IMaxityMarketPlace{
    ...
    event TokenIdSaled(address indexed _token,uint256 indexed _tokenid,address  buyer,address  seller,uint256 price);
    ...
    function sellByFixPrice(address _token,uint256[] memory _tokenids,uint256 _price) external  returns (uint) {
            
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].saler==address(0x0),'already saled');
            SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
            if(IERC721(_token).ownerOf(_tokenid)==address(this)){//first mint
                saleInfo.saler = IMaxityMetadata(_token).ngo();//
            }else{
                IERC721(_token).safeTransferFrom(msg.sender, address(this), _tokenid);
                saleInfo.saler = msg.sender;
            }
            // 
            saleInfo.isAuction = false;
            saleInfo.fixprice = _price;
            emit TokenIdOnSale(_token,_tokenid,saleInfo.saler,_price);
        }
        return length;
    }

    function auctionMax721(address _token,uint256[] memory _tokenids,uint base_price,uint incre_price,uint deadline) external returns (uint amountU){
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].saler==address(0x0),'already saled');
            ...
            emit TokenIdOnAuction(_token,_tokenid,saleInfo.saler,base_price,incre_price,deadline);
        }
        return length;
    }

    function buy(address _token,uint256 _tokenid) public {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.saler!=address(0x0),'not on saled');
        ...
        tradeDone(_token,_tokenid,saleInfo.fixprice,msg.sender);
    }
   ...
   function delist(address _token,uint256 _tokenid) public override {
        ...
        require(IERC721(_token).ownerOf(_tokenid)==address(this),'token not in this market');
        require(!saleInfo.isAuction,'token is on auction');
        require(saleInfo.saler==msg.sender,'only saler can delist');
        
        
        IERC721(_token).transferFrom(address(this), saleInfo.saler, _tokenid);
        saleInfo.saler = address(0x0);
        emit TokenIdDelist(_token,_tokenid,msg.sender);

    }

    function bid(address _token,uint256 _tokenid,uint256 amount) public returns (bool){
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.saler!=address(0x0),'not on saled');
        require(IERC721(_token).ownerOf(_tokenid)==address(this),'token not in this market');
        ...
        if(checkAuctionDone(_token,_tokenid)){
            ...
            if(amount>=nextbidPrice){
                ...
                saleInfo.price_offer = msg.sender;
                saleInfo.cur_bid_price = amount;
                emit BidOnTokenId(_token,_tokenid,msg.sender,amount);
                return true;
            }else{
                return false;
            }
    ...
    function tradeDone(address _token,uint256 _tokenid,uint amount,address buyer) internal {
        ...
        if(saleInfo.saler==IMaxityMetadata(_token).ngo()){//first sell
            ...
            TransferHelper.safeTransfer(ut, saleInfo.saler,remainAmount);
        } 
        IERC721(_token).transferFrom(address(this), buyer, _tokenid);
        saleInfo.saler = address(0x0);
        emit TokenIdSaled(_token,_tokenid, buyer,saleInfo.saler,amount);
    }
...
interface IMaxityMarketPlace{
    ...
    function bid(address _token,uint256 _tokenid,uint256 amount) external returns (bool);
