  ...
  function concludeBetting(bool startNew) external onlyOwner {
    ...
    if(startingPrice>price){
      ...
      for(uint256 i=0;i<playersFor.length;i++){
        recentWinnersWonAmount.push(((pool*((playersForPayedAmount[i]*1e18)/amountBetFor))/1e20)*95);// minus commision(used mainly for paying gas for transfering rewards to winners).
        playersFor[i].transfer(recentWinnersWonAmount[i]);
      }
    }else{
      recentWinners = playersAgainst;
      for(uint256 i=0;i<playersAgainst.length;i++){
        recentWinnersWonAmount.push(((pool*((playersAgainstPayedAmount[i]*1e18)/amountBetAgainst))/1e20)*95);// minus commision(used mainly for paying gas for transfering rewards to winners).
