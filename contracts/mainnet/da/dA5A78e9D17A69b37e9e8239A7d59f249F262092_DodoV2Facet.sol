...
contract DodoV2Facet is IDodoV2 {
    function swapDodoV2(Hop calldata h) external {
...
import {Hop} from "../LibRouter.sol";

interface IDodoV2 {
    function swapDodoV2(Hop memory h) external;
}
...
library LibRouter {
    ...
    function swap(SwapArgs memory swapArgs) internal returns (uint256 amountOut) {
        ...
        for (ss.i; ss.i < ss.hopsLength; ) {
            ...
            (bool success, ) = address(facet).delegatecall(ammCall);
            if (!success) {
                revert RouterAmmCallFailed();
            }
