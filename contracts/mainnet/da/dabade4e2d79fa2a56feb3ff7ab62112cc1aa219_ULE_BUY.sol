...
contract ULE_BUY is Ownable {
    ...
    constructor
    (IBEP20 _ULE_, IPancakeRouter01 _Router,address _LpReceiver_, address _swapContractAddress)
    {
       ULE = _ULE_;
       Router = _Router;
       WETH = Router.WETH();
    ...
    function _ULE_buyRouter(uint256 _tokens)
    public 
    payable
    {
        ...
        ULE.transfer(swapContractAddress, HalfToken);
        pool = true;
        }
        count++;
        if(pool) {
