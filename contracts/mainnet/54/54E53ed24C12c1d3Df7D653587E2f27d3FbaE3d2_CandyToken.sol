...
contract AntiBotHelper is Ownable {
    ...
    uint256 internal _txLimit = 50000 ether; // 50,000
    uint256 internal _holdLimit = 500000 ether; // 500,000
...
contract CandyToken is AntiBotHelper, FeeHelper, ERC20, Operatable {
    ...
    function _transfer(
        ...
        // This one will not work in case of token-buying, so we need to have another validation to check if it is not token-buying tx
        // by checking msg.sender not in candy pair
        if (
            !_inSwap &&
            feeApplied &&
            !_isSelfPair[from] &&
...
contract FeeHelper is Ownable {
    ...
    constructor() {
        ...
        _tokenFees[TX_CASE.TRANSFER].treasuryFee = 400;
        _tokenFees[TX_CASE.TRANSFER].burnFee = 0;

        _tokenFees[TX_CASE.BUY].marketingFee = 0;
        _tokenFees[TX_CASE.BUY].treasuryFee = 0;
        _tokenFees[TX_CASE.BUY].burnFee = 0;

        _tokenFees[TX_CASE.SELL].marketingFee = 400;
        _tokenFees[TX_CASE.SELL].treasuryFee = 400;
