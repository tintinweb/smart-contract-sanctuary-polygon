...
interface IHouse {
    function placeBet(uint amount, uint coins, uint agentID, string memory tokenString, IERC20 token) payable external;
    function settleBet(address player, uint amount, uint agentID, uint coins, bool win, IERC20 token) external returns(uint);
...
abstract contract Manager is Ownable {
    ...
    uint public minBetAmount = 10000000000000000 wei;
    uint public maxBetAmount = 200 ether;
    uint public maxCoinsBettable = 5;
   ...
   // Events
    event BetPlaced(uint indexed betId, address indexed player, uint amount, uint indexed coins, uint choice, string tokenString, uint agentID);
    event BetSettled(uint indexed betId, address indexed player, uint amount, uint indexed coins, uint choice, uint outcome, uint winAmount, string tokenString, uint agentID, uint256 randomNumber, address houseAddress);

    // Setter

    function setToken(string memory _tokenSymbol, IERC20 _token) external admin {
        betTokens[_tokenSymbol] = _token; 
    }

    function setMaxCoinsBettable(uint _maxCoinsBettable) external admin {
        maxCoinsBettable = _maxCoinsBettable;
    }

    function setMinBetAmount(uint _minBetAmount) external admin {
        minBetAmount = _minBetAmount;
    }

    function setMaxBetAmount(uint _maxBetAmount) external admin {
...
contract Coinflipper is ReentrancyGuard, Random, Manager {
     function placeBet(uint betChoice, uint coins, uint agentID, string memory tokenString, uint amount) external payable nonReentrant {
        ...
        IERC20 token = betTokens[tokenString];
        if(address(token) == address(0))
        {
            uint maticAmount = msg.value;
            require(maticAmount >= minBetAmount && maticAmount <= maxBetAmount, "Bet amount not within range");
            house.placeBet{value: msg.value}(maticAmount, coins, agentID, tokenString, token);
        }
        else{
            require(token.balanceOf(msg.sender) >= amount, "Your token balance not enough");
            SafeERC20.safeTransferFrom(token, msg.sender, houseAddress, amount);
            house.placeBet(amount, coins, agentID, tokenString, token);
        }
        
        bytes32 requestId = requestRandomness(keyHash, chainlinkFee);
        betMap[requestId] = betId;

        emit BetPlaced(betId, msg.sender, amount, coins, betChoice, tokenString, agentID);   
        bets.push(Bet({
            coins: uint8(coins),
    ...
    function settleBet(bytes32 requestId, uint256 randomNumber) private nonReentrant {
        ...
        bool win = choice == outcome;
        uint256 ranNum = randomNumber;

        bet.isSettled = true;
        bet.outcome = uint40(outcome);
        bet.randomNumber = ranNum;
        uint agentID = bet.agentID;
        string memory tokenString = bet.tokenString;
        IERC20 token = bet.token;

        uint winAmount = house.settleBet(player, amount, agentID, coins, win, token);

        bet.winAmount = uint128(winAmount);

        emit BetSettled(betId, player, amount, coins, choice, outcome, winAmount, tokenString, agentID, ranNum, houseAddress);
    }
}