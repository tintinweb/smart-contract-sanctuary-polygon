...
contract METAHUBToken is IBEP20, TokenAuth {
  string public constant name = "TEST Finance";
  string public constant symbol = "TMEN";
...
//  }
  ...
  function updateTaxWhitelist(address _user, IMENToken.TaxType _type, uint _percentage, bool _whitelisted) external onlyMn {
    require(_percentage <= 100000, "percentage invalid");
  ...
  function _calculateTax(address _from, address _to, uint _value) private returns (uint) {
    ...
    if (isBuyingToken) {
      WhitelistTax storage userWhitelistTax = whitelistTax[_to][IMENToken.TaxType.Buy];
      if (userWhitelistTax.status) {
        return userWhitelistTax.percentage;
      }
    ...
    if (_from == address(vault)) {
      WhitelistTax storage userWhitelistTax = whitelistTax[_to][IMENToken.TaxType.Claim];
      if (userWhitelistTax.status) {
        return userWhitelistTax.percentage;
      }
    ...
    if (isSellingToken) {
      WhitelistTax storage userWhitelistTax = whitelistTax[_to][IMENToken.TaxType.Sell];
      if (userWhitelistTax.status) {
        return userWhitelistTax.percentage;
      }
