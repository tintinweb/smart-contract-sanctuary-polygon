...
import "./SandollMinter.sol";
import "./Whitelist.sol";

contract SandollFont is ERC721URIStorage, MintController, Whitelist {

    using Address for address;
    address public _reciever;
    event Received (address indexed sender, uint256 value);

    constructor(address recipient, uint256 price, string memory name, string memory symbol) ERC721(name, symbol) MintController(recipient) {
        _mintPrice = price;
    }

    function mint(address recipient, uint256 tokenId, string memory tokenURI) override external onlyMinter {
        _mint(recipient, tokenId);
        _setTokenURI(tokenId, tokenURI);
        emit Minted(address(_minter), recipient, tokenId, tokenURI);
    }

    function safeTransferFrom(address from, address to, uint256 tokenId) public override {
        require(address(0) != to, "To address should not be null.");
        require(address(_minter).isContract(), "Minter is not contract.");
        require(tokenId > 0, "tokenId should be grater than zero.");
        super.safeTransferFrom(from, to, tokenId);
        SandollMinter(address(_minter)).move(from, to, tokenId);
    }
    ...
    function approveMint(uint256 count) override external payable {
        require(onlyWhiteListed(_msgSender()), "Sender is not included in whitelist.");
        require(address(_reciever) != address(0), "Receiver should not be null.");
        require(_mintPrice >= 0, "Mint price should be more than 0.");
        require(count >= 1 && count <= 10, "Mint count is not normal.");
        require(msg.value == _mintPrice * count, "Not enough balance.");
        require(address(_msgSender()).balance > _mintPrice * count, "Insufficient balance.");
        require(address(_minter).isContract(), "Minter is not contract.");

        ListType listType = _getListType();
        if(getEnabled() && listType == ListType.OG) {
            uint256 balance = this.balanceOf(_msgSender());
            if(balance >= 10) { revert(); }
        }
        
        if(count == 10 && _mintPrice > 0) { count = count + 1; }
        (bool success,) = _reciever.call{ value: msg.value }("");
        require(success, "Transfer failed.");

        for (uint i = 0; i < count; i++) {
            uint256 tokenId = SandollMinter(address(_minter)).reserveMint(_msgSender(), false, 0);
            emit MintApproved(_msgSender(), address(_minter), tokenId, count, false);
        }
    }

    function approveCombine(uint256 idx) override external {
        require(onlyWhiteListed(_msgSender()), "Sender is not included in whitelist.");
        require(address(_minter).isContract(), "Minter is not contract.");
        _setApprovalForAll(_msgSender(), address(_minter), true);
        uint256 tokenId = SandollMinter(address(_minter)).reserveMint(_msgSender(), true, idx);
        emit MintApproved(_msgSender(), address(_minter), tokenId, 1, true);
    }

    function decombine(uint256 tokenId) external {
        require(tokenId > 0, "TokenId should be grater than 0.");
        require(address(_minter).isContract(), "Minter is not contract.");
        SandollMinter(address(_minter)).decombine(_msgSender(), tokenId);
        _burn(tokenId);
        emit DecombineApproved(_msgSender(), tokenId);
    }

    function setListType(ListType value) external onlyOwner {
        _type = ListType(value);
    }
    ...
    function addWhiteList(address[] memory addresses) public onlyOwner returns (address[] memory) {
        return _addWhiteList(addresses);
    }

    function delWhiteList(address[] memory addresses) public onlyOwner returns (address[] memory) {
        return _delWhiteList(addresses);
    }

    function removeAllList() public onlyOwner returns (address[] memory) {
        return _removeAllList();
    }

    function setEnabled(bool enabled) public onlyOwner returns (bool) {
        return _setEnabled(enabled);
    }

    function setReciever(address recipient) public onlyOwner {
        _reciever = recipient;
    }
}
...
contract Whitelist {
    ...
    function onlyWhiteListed(address _caller) view internal returns (bool) {
        if(!_enabled) { return true; }
        bool listed = false;
        for (uint i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i] != _caller) { continue; }
            listed = true;
            break;
        }
        return listed;
    ...
    function _getListType() view public returns (ListType) {
        return _type;
    }

    function _addWhiteList(address[] memory addresses) internal returns (address[] memory) {
        for (uint i = 0; i < addresses.length; i++) {
            if(exist(addresses[i])) { continue; }
            _whitelist.push(addresses[i]);
        }
        return _whitelist;
    }

    function _delWhiteList(address[] memory addresses) internal returns (address[] memory) {
        for (uint i = 0; i < addresses.length; i++) {
            if(exist(addresses[i])) {
                _remove(addresses[i]);
            }
        }
        return _whitelist;
    ...
    function _remove(address _address) internal {
        for (uint i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i] != _address) { continue; }
            delete _whitelist[i];            
    ...
    function _removeAllList() internal returns (address[] memory) {
        delete _whitelist;
        return _whitelist;
    }

    function _setEnabled(bool enabled) internal returns (bool) {
        _enabled = enabled;
        return _enabled;
    }
    ...
    function exist(address _address) view public returns (bool) {
        bool _ok = false;
        for (uint i = 0; i < _whitelist.length; i++) {
            if(_whitelist[i] != _address) { continue; }
            _ok = true;
    ...
    function getEnabled() view public returns (bool) {
        return _enabled;
    }
}
