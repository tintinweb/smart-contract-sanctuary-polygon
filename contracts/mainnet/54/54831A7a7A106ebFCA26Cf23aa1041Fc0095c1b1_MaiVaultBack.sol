...
contract MaiVaultBack is BaseSiloAction{
    ...
    constructor(string memory _name, address _siloFactory, address[4] memory _inputs, address[4] memory _outputs, address _qiVault){
        name = _name;
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        if(storedConfig.length > 0){//if config is set in strategy use it
            (input,,maiVault) = abi.decode(storedConfig, (address[4],address[4],address));
        }
        else{
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address _qiVault) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _qiVault);
    }
