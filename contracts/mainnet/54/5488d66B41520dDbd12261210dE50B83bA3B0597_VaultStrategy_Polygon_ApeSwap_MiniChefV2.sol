    ...
    IVaultStrategy
{
    ...
    Reward public reward; //base reward
    Reward[] public additionalRewards; //optional additional rewards
    ...
    function setRewardSwapPath(SwapPath memory _swapPath) external
    {
        reward.swap = _swapPath;
    }

    function setAdditionalRewardSwap(uint256 _index, SwapPath memory _swapPath) external
    {
        //check
        requireValidRewardIndex(_index);

        //set
        additionalRewards[_index].swap = _swapPath;
    }

    function removeAdditionalReward(uint256 _index) external
    {
        //check
        requireValidRewardIndex(_index);

        //clone
        if (_index != additionalRewards.length - 1)
        {
            additionalRewards[_index] = additionalRewards[additionalRewards.length - 1];
        }        

        //remove
        additionalRewards.pop();        
    }

    function addAdditionalReward(Reward memory _reward) external
    {
    ...
    function rewardsContainsDepositToken() external view override returns (bool)
    {
        ...
        for (uint256 n = 0; n < additionalRewards.length; n++)
        {
            if (additionalRewards[n].token == depositToken)
            {
                return true;
            }
        }
    ...
    {
        require(_index < additionalRewards.length, "Invalid reward index");
    }
}
    ...
    ML_TransferHelper
{
    ...
    function compound(address _user) public override returns (bool)
    {
        //harvest all rewards from pool
        harvest();

        //convert additional rewards into reward
        swapAdditionalRewardsToReward();
    ...
    function swapReward(Reward memory _reward) private
    {
        //check balance
        uint256 balance = _reward.token.balanceOf(address(this));
        if (balance == 0)
        {
            return;
        }
    ...
    function swapAdditionalRewardsToReward() private
    {
        for (uint256 n = 0; n < additionalRewards.length; n++)
        {
            swapReward(additionalRewards[n]);
        }
    }
    ...
    VaultStrategyV2
{
    ...
    SwapPath public rewardToToken0; //path to swap main reward to Token0
    SwapPath public rewardToToken1; //path to swap main reward to Token1
    ...
    function setRewardToLPSwap(bool _token0, SwapPath memory _swapPath) public
    {
        //check
        require(isLPToken, "Deposit is not LP");
        require(_swapPath.path[0] == address(rewardToken()), "Start not reward token");
        require(
            _swapPath.path[_swapPath.path.length - 1] == (_token0
                ? ITokenPair(address(depositToken)).token0()
                : ITokenPair(address(depositToken)).token1()
            ),
            "End not part of LP token"
        );

        //set
        if (_token0)
        {
            rewardToToken0 = _swapPath;
        }
        else
        {
            rewardToToken1 = _swapPath;
        }
    }
    ...
    function swapRewardToDeposit() internal virtual override
    {
        ...
        if (token1 != rewardToken())
        {
            swap(
                halfReward,
                rewardToToken1.router, 
                rewardToToken1.path
            );
        }

        //add liquidity
        IUniRouterV2(reward.swap.router).addLiquidity(
            address(token0),
            address(token1),
    ...
    VaultStrategy_swap_UniSwapV2
{
    ...
    )
    {
        ...
        reward = createReward(REWARD_TOKEN, ROUTER);
        //additionalRewards.push(createReward(ADDITIONAL_REWARD_TOKEN, ROUTER));
    }    
    ...
    ///@inheritdoc VaultStrategy_deposit_LP
    function swapRewardToDeposit() internal override(VaultStrategyV2, VaultStrategy_deposit_LP)
    {
        super.swapRewardToDeposit();     
    }
}