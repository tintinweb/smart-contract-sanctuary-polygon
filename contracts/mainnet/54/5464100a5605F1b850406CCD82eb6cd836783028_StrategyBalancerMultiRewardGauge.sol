...
contract StrategyBalancerMultiRewardGauge is StratFeeManagerInitializable {
    ...
    function initialize(
        bytes32[] calldata _balancerPoolIds,
    ...
    function addLiquidity() internal {
        if (input != native) {
            uint256 nativeBal = IERC20(native).balanceOf(address(this));
            UniswapV3Utils.swap(uniV3router, nativeToWantPath, nativeBal);
        }
