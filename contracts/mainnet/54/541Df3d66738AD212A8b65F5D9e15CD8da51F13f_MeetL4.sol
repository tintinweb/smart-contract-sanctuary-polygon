...
//This Contract is a ERC721 OpenZeppelin NFT Implementation
//It uses the Ownable and Counter extensions

contract MeetL4 is ERC721, Ownable {

    //imported from OpenZeppelin
    using Counters for Counters.Counter;
    using Strings for uint256;

    //Keeps track of the tokenIds, so we can see whats the lastly minted nft, and which number the new one will have
    Counters.Counter private _tokenIds;

    //Gives the NFTs their Metadata
    mapping (uint256 => string) private _tokenURIs;


    //This Constructor is executed once, when the contract is deployed

    constructor() ERC721("Tokenshelf", "TSHELF") {
    }
    ...
    returns (uint256)
    {
        require( balanceOf(recipient) < 1,"This Person already got a MeetL4 - NFT");
        _tokenIds.increment();
