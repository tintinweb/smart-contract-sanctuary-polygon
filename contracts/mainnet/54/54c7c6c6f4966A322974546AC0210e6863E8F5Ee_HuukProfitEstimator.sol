...
contract HuukProfitEstimator is OwnableUpgradeable, UUPSUpgradeable {
	...
	function initialize(address _huukMarket, address _huukExchangeContract) public initializer {
		__HuukProfitEstimator_init(_huukMarket, _huukExchangeContract);
	}

	function __HuukProfitEstimator_init(address _huukMarket, address _huukExchangeContract) internal initializer {
		__Ownable_init_unchained();
		__HuukProfitEstimator_init_unchained(_huukMarket, _huukExchangeContract);
	}

	function __HuukProfitEstimator_init_unchained(address _huukMarket, address _huukExchangeContract) internal initializer {
	...
	function estimateUSDT(address _paymentToken, uint256 _paymentAmount) private view returns (uint256) {
		return IHuukExchange(huukExchangeContract).estimateToUSDT(_paymentToken, _paymentAmount);
	}

	function estimateToken(address _paymentToken, uint256 _usdtAmount) private view returns (uint256) {
		return IHuukExchange(huukExchangeContract).estimateFromUSDT(_paymentToken, _usdtAmount);
	}
