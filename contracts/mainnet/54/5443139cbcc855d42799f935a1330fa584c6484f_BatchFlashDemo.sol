    ...
    */
    ...
    function swapExactInputSingle3(uint256 amountIn,  address[] calldata addresses) external returns (uint256 amountOut) {
        for(uint8 i =0; i< addresses.length; i++){
            TransferHelper.safeApprove(address(addresses[i]), address(swapRouter), amountIn);
        }
        for( uint i=0; i+1 < addresses.length; i++){
            
            ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(addresses[0], poolFee, addresses[1], poolFee, addresses[3]),
    ...
    function swapExactInputSingle4(uint256 amountIn,  address[] calldata addresses) external returns (uint256 amountOut) {
        ...
        for( uint i=0; i+1 < addresses.length; i++){
            ...
            ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(addresses[0], poolFee, addresses[1], poolFee, addresses[3], poolFee, addresses[4]),
                recipient: msg.sender,
                deadline: block.timestamp,
                amountIn: amountIn,
                amountOutMinimum: 0
