...
contract SiloFactory is Ownable, ERC721Enumerable {
    ...
    mapping(uint256 => address[4]) public strategyInputs;
    ...
    address public defaultRecipient =
        0x41a0672cA96dA8A256147F350397297B951fAfa0; //fee manager
    ...
    event SiloDeposit(uint256 siloID, uint256[4] amount);
    ...
    event SiloExit(uint256 siloID, uint256 amount, uint8 exitType);

    event SiloStatusUpdate(uint256 siloID, uint8 status);
    ...
    function adjustDefaultFeeList(uint256[4] memory _feeList)
        external
        onlyOwner
    {
    ...
    function adjustFeeList(uint256[4] memory _feeList, address _action)
        external
        onlyOwner
    {
    ...
    function adjustRecipient(address _recipient, address _action)
        external
        onlyOwner
    {
    ...
    function adjustHighRiskActions(address _action, bool _state)
        external
        onlyOwner
    {
    ...
    function enterSiloStrategy(uint256[4] memory amount, uint256 siloID)
        external
        isOwner(siloID)
    {
    ...
    function withdrawTokensFromSilo(uint256 siloID, address[] memory tokens)
        external
        isOwner(siloID)
    {
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function exitSiloStrategy(uint256 siloID, uint256 _requestedOut)
        external
        isOwner(siloID)
    {
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function exitStrategyAndWithdraw(uint256 siloID, uint256 _requestedOut)
        external
        isOwner(siloID)
    {
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function adjustSiloDelay(uint256 siloID, uint256 _newDelay)
        external
        isOwner(siloID)
    {
        ISilo silo = ISilo(siloMap[siloID]);
    ...
    function nameSilo(uint256 _siloId, string memory _name)
        external
        isOwner(_siloId)
    {
    ...
    /***************************************external view *************************************/
    function getFeeInfo(address _action)
        external
        view
    ...
    function getFeeInfoNoTier(address _action)
        external
        view
    ...
    function isSiloManager(address _silo, address _manager)
        external
        view
        returns (bool)
    {
    ...
    function getStrategyInputs(uint256 _id)
        public
        view
    ...
    function getStrategyActions(uint256 _id)
        public
        view
    ...
    function getStrategyConfigurationData(uint256 _id)
        public
        view
    ...
    function _setActionStack(
        ...
        require(
            ...
            "Gravity Finance: no approved manager"
        );
        ...
        if (silo.highRiskAction()) {
            ...
            require(
                ...
                "small upkeep balance for high risk strategies"
            );
        }
    }
        ...
        if (enforceBeforeTransfer && to != address(0)) {
            ...
            if (from != address(0)) {
                ...
                require(
                    ...
                    "Caller not approved to send silos to recipient"
                );
            }
            require(
            ...
            require(
                balanceOf(to) < maxSilosPerUser,
                "Gravity Finance: over max number of silos"
            );
