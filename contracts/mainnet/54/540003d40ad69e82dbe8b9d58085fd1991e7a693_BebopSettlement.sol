...
abstract contract BebopSigning {
    ...
    bytes32 public constant AGGREGATED_ORDER_TYPE_HASH = keccak256(abi.encodePacked(
        "Aggregate(uint256 expiry,address taker_address,address[] maker_addresses,uint256[] maker_nonces,address[][] taker_tokens,address[][] maker_tokens,uint256[][] taker_amounts,uint256[][] maker_amounts,address[] receivers,bool[] using_contract)"
    ));

    bytes32 public constant PARTIAL_AGGREGATED_ORDER_TYPE_HASH = keccak256(abi.encodePacked(
        "Partial(uint256 expiry,address taker_address,address maker_address,uint256 maker_nonce,address[] taker_tokens,address[] maker_tokens,uint256[] taker_amounts,uint256[] maker_amounts,address receiver)"
    ));
    ...
    function assertAndInvalidateMakerOrders(
        ...
        address nativeToken = Transfer.NATIVE_TOKEN;
        for (uint256 i; i < numMakerSigs; ++i) {
            ...
            Order.Partial memory partial_order = Order.Partial(
                ...
                new address[](order.taker_tokens[i].length),
                new address[](order.maker_tokens[i].length),
                order.taker_amounts[i],
                order.maker_amounts[i],
                order.receivers[i]
            );
            for (uint j; j < partial_order.taker_tokens.length; ++j) {
                if (order.taker_tokens[i][j] == nativeToken) {
                    partial_order.taker_tokens[j] = wrappedNativeToken;
                } else {
                    partial_order.taker_tokens[j] = order.taker_tokens[i][j];
                }
            }
            for (uint j; j < partial_order.maker_tokens.length; ++j) {
                if (order.maker_tokens[i][j] == nativeToken) {
                    partial_order.maker_tokens[j] = wrappedNativeToken;
                } else {
                    partial_order.maker_tokens[j] = order.maker_tokens[i][j];
                }
            }
            bytes32 partial_hash = hashPartialOrder(partial_order);
            validateSignature(maker_address, partial_hash, makerSigs[i].signature, true);
    ...
    function assertAndInvalidateAggregateOrder(
        ...
        assertAndInvalidateMakerOrders(order, makerSigs, wrappedNativeToken);
...
abstract contract BebopTransfer {
    ...
    function makerTransferFunds(
        ...
        for (uint j; j < tokensNum; ++j) {
            ...
            address receiver = to;
            if (token == Transfer.NATIVE_TOKEN) {
    ...
    function callPermitForTokens(
        ...
        uint deadline,
        Signature.TokenPermit[] memory permits
    ) internal {
        for (uint i; i < permits.length; ++i) {
            Signature.TokenPermit memory permit = permits[i];
            (bytes32 r, bytes32 s, uint8 v) = Signature.getRsv(permit.signatureBytes);
...
contract BebopSettlement is IBebopSettlement, BebopSigning, BebopTransfer, ReentrancyGuard {
    ...
    function SettleAggregateOrder(
        ...
        bytes32 h = assertAndInvalidateAggregateOrder(order, takerSig, makerSigs, WRAPPED_NATIVE_TOKEN);

        uint numMakerSigs = makerSigs.length;
        Transfer.NativeTokens memory nativeTokens = Transfer.NativeTokens({receiver: address(0), toTaker: 0, toMakers: 0});
        ...
        for (uint i; i < numMakerSigs; ++i) {
            ...
            for (uint k; k < order.taker_tokens[i].length; ++k) {
                if (order.taker_tokens[i][k] == Transfer.NATIVE_TOKEN) {
        ...
        for (uint i; i < makerSigs.length; ++i) {
            ...
            for (uint k; k < order.taker_tokens[i].length; ++k) {
                if (order.taker_tokens[i][k] == Transfer.NATIVE_TOKEN) {
                    // Accumulating taker's number of native tokens
                    nativeTokens.toMakers += order.taker_amounts[i][k];
                    pendingTransfersToMakers[pendingTransfersLen++] = Transfer.Pending(
                        WRAPPED_NATIVE_TOKEN, order.maker_addresses[i], order.taker_amounts[i][k]
                    );
...
/// @title IPermit2 (This is renamed ISignatureTransfer.sol - https://github.com/Uniswap/permit2/blob/main/src/interfaces/ISignatureTransfer.sol)
