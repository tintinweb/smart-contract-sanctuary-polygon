 ...
 */
contract BettingV3 is Ownable, AccessControl, ReentrancyGuard, Pausable {
    ...
    modifier tournamentCreated(string calldata _tournamentId) {
        require(
            bytes(tournamentByTournamentId[_tournamentId].tournamentId).length > 0,
            "[BettingV3.tournamentCreated] Tournament not created yet"
        );
        _;
    }
    modifier tournamentEntryExist(string calldata _tournamentId, string calldata _teamId) {
        require(
            tournamentByTournamentId[_tournamentId].entryIdByTeamId[_teamId] != 0,
            "[BettingV3.tournamentEntryExist] Tournament entry not exist"
        );
        _;
    }
    modifier beforeTournamentBettingClosed(string calldata _tournamentId) {
        require(
            !tournamentByTournamentId[_tournamentId].isClose,
            "[BettingV3.beforeTournamentBettingClosed] Tournament betting period closed"
        );
        _;
    }
    modifier afterTournamentWinnerUpdated(string calldata _tournamentId) {
        require(
            bytes(tournamentByTournamentId[_tournamentId].winner).length > 0,
            "[BettingV3.afterTournamentWinnerUpdated] Tournament winner not been updated yet"
        );
        _;
    }
    ...
    receive() external payable {
        revert("[BettingV3] Revert receive function.");
    }

    fallback() external payable {
        revert("[BettingV3] Revert fallback function.");
    }
     ...
     */
    function createBettingTournament(string calldata _tournamentId, string[] calldata _teamIds, address _tournamentOwner)
        public
        whenNotPaused
        onlyRole(WORKER)
    {
        require(
            bytes(tournamentByTournamentId[_tournamentId].tournamentId)
                .length == 0,
            "[BettingV3.createBettingTournament] Tournament already created"
        );
        require(
            _teamIds.length > 1,
            "[BettingV3.createBettingTournament] Invalid teamIds length"
        );
        ...
        for (uint256 i = 0; i < _teamIds.length; i++) {
            ...
            tournament.entryIdByTeamId[_teamIds[i]] = tournament.entryCount;
            tournament.entryByEntryId[tournament.entryCount].teamId = _teamIds[i];
        }
     ...
     */
    function batchCreateBettingTournament(TournamentArgs[] calldata _tournamentArgsArr)
        external
        whenNotPaused
        onlyRole(WORKER)
    {
        require(
            ...
            "[BettingV3.batchCreateBettingTournament] Invalid tournamentArgs array"
        );
        for (uint256 i = 0; i < _tournamentArgsArr.length; i++) {
            if(bytes(tournamentByTournamentId[_tournamentArgsArr[i].tournamentId].tournamentId).length != 0)
                revert InvalidTournamentArr(i, "Tournament already created");
            if(_tournamentArgsArr[i].teamIds.length <= 1)
                revert InvalidTournamentArr(i, "Invalid teamIds length");

            createBettingTournament(_tournamentArgsArr[i].tournamentId, _tournamentArgsArr[i].teamIds, _tournamentArgsArr[i].tournamentOwner);
        }
    }
     ...
     * @param _teamIds - list of team id generated from backend.
     */
    function addBettingTournamentEntries(string calldata _tournamentId, string[] calldata _teamIds)
        external
        whenNotPaused
        onlyRole(WORKER)
        ...
        beforeTournamentBettingClosed(_tournamentId)
    {
        require(
            _teamIds.length > 0,
            "[BettingV3.addBettingTournamentEntries] Invalid entiryIds length"
        );

        Tournament storage tournament = tournamentByTournamentId[_tournamentId];
        for (uint256 i = 0; i < _teamIds.length; i++) {
            require(
                tournament.entryIdByTeamId[_teamIds[i]] == 0,
                "[BettingV3.addBettingTournamentEntry] Some team already exist in entry list"
            );
     ...
     */
    function placeTournamentBet(string calldata _tournamentId, string calldata _teamId, uint256 _betAmount)
        external
        whenNotPaused
        tournamentCreated(_tournamentId)
        ...
        beforeTournamentBettingClosed(_tournamentId)
    {
        require(
            _betAmount > 0,
            "[BettingV3.placeTournamentBet] Invalid bet amount"
        );
        // stack too deep
        {
            Tournament storage tournament = tournamentByTournamentId[_tournamentId];
     ...
     * @param _tournamentId - tournament id generated from backend.
     */
    function closeBettingTournament(string calldata _tournamentId)
        public
        whenNotPaused
        onlyRole(WORKER)
        tournamentCreated(_tournamentId)
     ...
     * @param _tournamentId - tournament id generated from backend.
     */
    function claimTournamentReward(string calldata _tournamentId)
        external
        nonReentrant
        tournamentCreated(_tournamentId)
        afterTournamentWinnerUpdated(_tournamentId)
    {
        ...
        require(
            reward > 0,
            "[BettingV3.claimTournamentReward] User not have claimable reward for this tournament"
        );
     ...
     */
    function batchClaimTournamentReward(string[] calldata _tournamentIds)
        external
        nonReentrant
    {   
        require(
            _tournamentIds.length > 0,
            "[BettingV3.batchClaimTournamentReward] Invalid tournamentId array"
        );
     ...
     * @param _tournamentId - tournament id generated from backend.
     */
    function tournamentFeesWithdraw(string calldata _tournamentId)
        external
        nonReentrant
        tournamentCreated(_tournamentId)
        afterTournamentWinnerUpdated(_tournamentId)
    {
        ...
        require(
            ...
            "[BettingV3.tournamentFeesWithdraw] Only tournament owner can withdraw fees"
        );
        uint256 tournamentFees = tournament.tournamentFees;
        require(
            tournamentFees > 0,
            "[BettingV3.tournamentFeesWithdraw] Tournament owner not have withdrawable fees for this tournament"
        );
     ...
     * @param _tournamentIds - array of tournament id generated from backend.
     */
    function batchTournamentFeesWithdraw(string[] calldata _tournamentIds)
        external
        nonReentrant
    {   
        require(
            _tournamentIds.length > 0,
            "[BettingV3.batchTournamentFeesWithdraw] Invalid tournamentId array"
        );
        for (uint256 i = 0; i < _tournamentIds.length; i++) {
            Tournament storage tournament = tournamentByTournamentId[_tournamentIds[i]];
     ...
     * @param _tournamentId - tournament id generated from backend.
     */
    function adminRewardWithdraw(string calldata _tournamentId)
        external
        nonReentrant
        onlyRole(WORKER)
        tournamentCreated(_tournamentId)
        afterTournamentWinnerUpdated(_tournamentId)
    {
        ...
        require(
            ...
            "[BettingV3.adminRewardWithdraw] Admin cannot withdraw when there is bet on winner"
        );
        require(
            tournament.tournamentClaimAmount == 0,
            "[BettingV3.adminRewardWithdraw] Admin cannot withdraw more than once"
        );

        uint256 reward = tournament.tournamentBetAmount;
        require(
            reward > 0,
            "[BettingV3.adminRewardWithdraw] Tournament have no reward to be withdrawed"
        );
    ...
    function adminServiceFeesWithdraw() external nonReentrant onlyRole(WORKER) {
        require(
            serviceFees > 0,
            "[BettingV3.adminServiceFeeWithdraw] Contract have no service fees to be withdrawed"
        );
     ...
     */
    function safeUpoTransfer(address _userAddress, uint256 _upoAmount) internal {
     ...
     */
    function getUserTournamentClaimableReward(string memory _tournamentId, address _userAddress)
        public
        view
        returns (uint256 claimableReward)
    {
        ...
        claimableReward = tournament.userClaimAmount[_userAddress] == 0?
            (tournament.rewardPerBet * entry.userBetAmount[_userAddress]) / REWARDPRECISION : 0;
    }
     ...
     * @param _tournamentIds - array of tournament id generated from backend.
     */
     function getUserTournamentClaimableRewardList(string[] memory _tournamentIds, address _userAddress)
        external
        view
        returns (uint256[] memory claimableRewardList)
    {
        claimableRewardList = new uint256[](_tournamentIds.length);
        for (uint256 i = 0; i < _tournamentIds.length; i++) {
            claimableRewardList[i] = getUserTournamentClaimableReward(_tournamentIds[i], _userAddress);
        }
    }
     ...
     * @param _userAddress - user address.
     */
    function getUserBetInfo(string calldata _tournamentId, address _userAddress)
        external
        view
        returns (
     ...
     */
    function setTournamentWinner(string calldata _tournamentId, string calldata _teamId)
        external
        whenNotPaused
        onlyRole(WORKER)
        tournamentCreated(_tournamentId)
        tournamentEntryExist(_tournamentId, _teamId)
    {
        ...
        require(
            ...
            "[BettingV3.setTournamentWinner] Tournament already have winner"
        );
        ...
        // not deduct fee if 
        // 1) all user bet on winner side
        // 2) fee deduction will make user loss from bet correctly
        if(winner.entryBetAmount == tournament.tournamentBetAmount || tournament.tournamentBetAmount < (winner.entryBetAmount + _serviceFees + _tournamentFees)) {
            _tournamentFees = 0;
            _serviceFees = 0;
        } else {
        ...
        emit TournamentWinnerUpdated(_tournamentId, _teamId, _tournamentFees, _serviceFees);
    }
     ...
     * @param _tournamentOwner - tournament owner address
     */
    function setTournamentOwner(string calldata _tournamentId, address _tournamentOwner)
        external
        whenNotPaused
        onlyRole(WORKER)
        tournamentCreated(_tournamentId)
        ...
        onlyRole(WORKER)
    {
        require(
            ...
            "[BettingV3.setTournamentFeesPercentage] Invalid tournament fees percentage"
        );
        ...
        onlyRole(WORKER)
    {
        require(
            ...
            "[BettingV3.setServiceFeesPercentage] Invalid service fees percentage"
        );
     ...
     */
    function pauseBettingV3() external onlyOwner {
        _pause();
    }
     ...
     */
    function unpauseBettingV3() external onlyOwner {
        _unpause();
    }
}
