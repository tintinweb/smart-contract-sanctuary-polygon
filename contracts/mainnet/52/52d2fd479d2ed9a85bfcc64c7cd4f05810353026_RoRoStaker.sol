 ...
 *
 * BLKZally, the non-standard {decreaseAllowance} and {increaseAllowance}
...
interface IMaster{
    function userInfo(uint, address) external view returns (uint,uint,uint,uint,uint,uint,bool,bool);
}




contract RoRoStaker is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        bool isExists;
        bool isClaimed;
        bool isPendingClaimed;
        bool isDepositClaimed;
    }
   ...
   // uint256 public intervalBlock = 86400;
    ...
    constructor() public   {
        devaddr = 0x36d18DBF8e1F083822e1fe30d03AEbF0CB703160;
        feeAddress = 0x36d18DBF8e1F083822e1fe30d03AEbF0CB703160;
        startBlock = 41520870;
    }
...
//add new pool _allocPoint - Multiplier , _withUpdate - true, _isFixed - , _emisson - per block reward, _interval - if fixed interval reward, _poolLimit - stak token count
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, bool _withUpdate, IERC20 _rwdToken,uint256 _lockedPeriod,bool _isFixed,uint256 _emisson,uint256 _interval,uint256 _poolLimit) public  {
...
//Edit pool
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, bool _withUpdate,IERC20 _rwdToken,uint256 _lockedPeriod,uint256 _interval,uint256 _poolLimit) public onlyOwner {
        require(_depositFeeBP <= 10000, "set: invalid deposit fee basis points");
        if (_withUpdate) {
            massUpdatePools();
        }
        if(!poolInfo[_pid].isFixed)
        ...
        poolInfo[_pid].poolLimit =(_poolLimit);
    ...
    function pendingReward(uint256 _pid, address _user) external view returns (uint256) {
        ...
        return user.pendingRewards.add(pool.isFixed ? getFixedReturns(_pid,_user) : getAPRPending(_pid,_user));
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if(_amount > 0) {
            if(pool.poolLimit > 0)
                 require(pool.depositedAmount.add(_amount) <= pool.poolLimit, "Pool Limit Reached !");
            
            pool.lpToken.safeTransferFrom(address(msg.sender), address(this), _amount);
            
       
            if(pool.depositFeeBP > 0 && !(!user.isDepositClaimed)){
                uint256 depositFee = _amount.mul(pool.depositFeeBP).div(10000);
                pool.lpToken.safeTransfer(feeAddress, depositFee);
                user.amount = user.amount.add(_amount).sub(depositFee);
                pool.depositedAmount = pool.depositedAmount.add((_amount).sub(depositFee));
            }else{
        ...
        if(!user.isPendingClaimed){
                user.isPendingClaimed = true;
        }
        if((!user.isDepositClaimed)){
            user.isDepositClaimed = true;
        }
    ...
    function getFixedReturns(uint256 _pid,address _user) public view returns (uint256){
        ...
        if(pool.lockedPeriod == 0)
         ...
         Reward = (user.amount.mul(pool.emissionPerBlock).div(10**20)).mul(multiplier).mul(pool.allocPoint.div(100)); // TostPerBlock = 20 i.e 0.2 %
        }
        return Reward;
    }
    ...
    function claimFixed(address _user,uint256 pid) internal{
        ...
        if(claimAmount > 0) {
            uint256 claimedAmount = safeRewardTransfer(_user, claimAmount,pid);
            user.pendingRewards = claimAmount.sub(claimedAmount);
    ...
    // Safe rwd transfer function, just in case if rounding error causes pool to not have enough Tokens.
