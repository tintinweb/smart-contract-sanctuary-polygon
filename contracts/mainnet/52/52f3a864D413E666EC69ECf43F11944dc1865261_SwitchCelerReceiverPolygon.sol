...
contract SwitchCelerReceiverPolygon is SwitchPolygon {
        ...
        returns (IMessageReceiverApp.ExecutionStatus)
    {
        ...
        if (m.bridgeToken == m.dstToken) {
            ...
            if (m.paraswapUsageStatus ==  uint(DataTypes.ParaswapUsageStatus.None)) { // when case not using paraswap
                _executeWithDistribution(m, _amount);
            } else if (m.paraswapUsageStatus ==  uint(DataTypes.ParaswapUsageStatus.OnDestChain)) { // When case using paraswap
