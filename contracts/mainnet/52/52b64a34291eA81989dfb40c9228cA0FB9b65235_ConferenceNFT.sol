...
contract ConferenceNFT is ERC721URIStorage, Ownable {
    ...
    constructor(bytes32 merkleRoot_, uint256 maxTickets_) public ERC721("Digital Gold Conference NFT", "DGC") {
      merkleRoot = merkleRoot_;
      maxTickets = maxTickets_;
    }

    function claim(bytes32[] calldata merkleProof) external {
        require(!isClaimed[msg.sender], 'Contract: NFT already claimed.');
        require(!isPaused, 'Contract: Claiming is paused.');

        uint256 tokenId = _tokenIds.current();
        require(tokenId < maxTickets, 'Contract: All tickets have been minted.');

        // Verify the merkle proof.
        bytes32 node = keccak256(abi.encodePacked(msg.sender));
        require(MerkleProof.verify(merkleProof, merkleRoot, node), 'Contract: Invalid proof.');
    ...
    function ownerClaim(address to) external onlyOwner {
        uint256 tokenId = _tokenIds.current();
        require(tokenId < maxTickets, 'Contract: All tickets have been minted.');
