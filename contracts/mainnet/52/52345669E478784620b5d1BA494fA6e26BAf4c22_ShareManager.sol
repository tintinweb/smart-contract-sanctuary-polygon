...
contract ShareManager is BaseContract {
  ...
  function checkin() external {
    ...
    uint stTokenBalance = stToken.balanceOf(msg.sender);
    ...
    if (stTokenBalance > 0) {
      user.stTokenStocked += stTokenBalance;
      config.totalSTTokenStocked[checkinRound] += stTokenBalance;
      _takeToken(stToken, stTokenBalance);
    }
    emit CheckedIn(msg.sender, checkinRound, nftBalance, stTokenBalance, block.timestamp);
  }

  function claim(uint _round) external {
    require(block.timestamp - (config.zeroTimestamp + _round * CHECKIN_INTERVAL * config.secondsInADay) >= config.secondsInADay, "ShareManager: please come back later");
