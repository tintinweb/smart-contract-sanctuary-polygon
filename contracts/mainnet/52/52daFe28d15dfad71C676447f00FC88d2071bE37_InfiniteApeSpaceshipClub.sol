...
contract InfiniteApeSpaceshipClub is ERC721A, Ownable {
  ...
  function addRandEquity(uint256 _quantity) internal {
    uint _tokenIDStart = totalSupply();
  
    for (uint256 i = 0; i < _quantity; i++) {
      uint _Equity = getRandEquity(_tokenIDStart + i);
	  IASCEquity[_tokenIDStart + i] = _Equity;
	  totalIASCEquity += _Equity;
    }
  }

  function getRandEquity(uint256 _tokenID) public view returns (uint256) {
    bytes memory seed = abi.encodePacked(block.timestamp.add(_tokenID));
    uint RandNO = rand(seed, 1, 100000);
