...
// File contracts/MultiNFTGenesisToken.sol

// Updated to start tokenId from 1

pragma solidity ^0.8.11;

contract MULTINFT_MEMBERSHIP is ERC721A {
    ...
    string  private _URIPrefix = "https://multinft.mypinata.cloud/ipfs/QmXsSWPEebEsKEfoB5UMEhdFKwd7kdjb7xXcYK7pKqGg65/";
    uint256 private _userMintLimit = 5;
    uint256 private _state = 0; // 0: paused, 1: standard mint, >= 2: special sales

    constructor()
        ERC721A("MultiNFT Membership", "MNFT")
    {
        _contractOwner = msg.sender;
        _mintERC2309(msg.sender, 400);
    }
    ...
    function specialSale(uint256 quantity) external payable {
        ...
        require(currentState > 1, "No undergoing special sale");
        require(msg.value >= quantity * price, "Not enough ether for minting");
        require(_currentIndex - 1 + quantity <= _maxSupply, "Mint quantity exceeds max supply");
        require(balanceOf(msg.sender) + quantity <= _userMintLimit, "Mint limit by user reached");
        emit SpecialSaleClaim(msg.sender, uint8(quantity), uint8(currentState));
    }
    ...
    function mint(uint256 quantity) external payable {
        require(_state == 1, "Mint disabled");
        require(_currentIndex - 1 + quantity <= _maxSupply, "Mint quantity exceeds max supply");
        require(balanceOf(msg.sender) + quantity <= _userMintLimit, "Mint limit by user reached");
        require(msg.value >= quantity * price, "Not enough ether for minting");
    ...
    function setState(uint256 state_) external payable onlyOwner {
        _state = state_;
    }

    function setPrice(uint256 price_) external payable onlyOwner {
        price = price_;
    }

    function setUserMintLimit(uint256 userMintLimit_) external payable onlyOwner {
        _userMintLimit = userMintLimit_;
    }
