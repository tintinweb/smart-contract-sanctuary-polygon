...
library AmysStakingCo {
    ...
    function getMCPoolData(address chef) external view returns (ChefType chefType, address[] memory lpTokens, uint256[] memory allocPoint) {

        uint len = IMasterHealer(chef).poolLength();
        if (len == 0) revert PoolLengthZero(chef);

        chefType = identifyChefType(chef);
        if (chefType == ChefType.UNKNOWN) revert UnknownChefType(chef);

        lpTokens = new address[](len);
        allocPoint = new uint256[](len);

        if (chefType == ChefType.MASTERCHEF) {
            for (uint i; i < len; i++) {
        ...
        } else if (chefType == ChefType.MINICHEF) {
            for (uint i; i < len; i++) {
    ...
    function identifyChefType(address chef) public view returns (ChefType chefType) {
        ...
        if (!success && checkMasterChef(chef)) {
            return ChefType.MASTERCHEF;
        }
        
        return ChefType.UNKNOWN;
    }
