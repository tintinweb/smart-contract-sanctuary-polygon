...
//** Author DanishGakhar :  Marketplace Base Contract 2023.04 */
...
import "./interfaces/IMRConfig.sol";

/// The contract is abstract so it cannnot be deployed.
abstract contract OOMContractBase is Ownable, ReentrancyGuard, IERC721Receiver {
    enum SellMode {
        FixedPrice,
        Auction
    }
    ...
    function _checkRoyalties(address _contract) internal view returns (bool) {
        bool success = ERC721(_contract).supportsInterface(
    ...
    function _addListing(
        ...
        require(_fromAddress != address(0), "Invalid from address");
        // Fixed price no need to check start time and duration.
        if (_sellMode != SellMode.FixedPrice) {
            require(
                _startTime >= block.timestamp,
                "Listing auction start time past already"
            );
            require(
                _duration >= mrConfig.getMinDuration() &&
                    _duration <= mrConfig.getMaxDuration(),
                "Invalid duration"
            );
        }
        ...
        _depositNft(_fromAddress, _hostContract, _tokenId, _copies);
        ...
        for (uint256 index = 0; index < _copies; ++index) {
            ...
            listingRegistry[listingId].seller = _fromAddress;
            ...
            addrListingIds[_fromAddress].push(listingId);
    ...
    function _placeBid(
        ...
        require(
            lst.sellMode == SellMode.Auction,
            "Can only bid for listing on auction"
        );
        require(
            block.timestamp >= lst.startTime,
        ...
        require(_fromAddress != lst.seller, "Bidder cannot be seller");
        ...
        biddingRegistry[biddingId].bidder = _fromAddress;
        ...
        addrBiddingIds[_fromAddress].push(biddingId);
    ...
    function _deduceRoyalties(
        ...
        if (royaltiesAmount > 0) {
            uint256 royaltyFee = (royaltiesAmount *
                mrConfig.getRoyalityFeeRate()) / FEE_RATE_BASE;
            if (royaltyFee > 0) {
                _transferToken(
                    msg.sender,
                    mrConfig.getRevenueAddress(),
                    royaltyFee
                );
    ...
    function _buyNow(
        ...
        uint256 price,
        bool _isGl
    ) internal returns (address hostContract) {
        listing storage lst = listingRegistry[listingId];
        require(
            lst.sellMode != SellMode.Auction,
            "Auction not support buy now"
        );
        require(_fromAddress != lst.seller, "Buyer cannot be seller");
        ...
        if (_isGl == true) {
            ...
            _transferNft(_fromAddress, lst.hostContract, lst.tokenId);
        } else {
            _transferToken(_fromAddress, lst.seller, sellerAmount);
            _transferNft(_fromAddress, lst.hostContract, lst.tokenId);
        }

        emit BuyNow(listingId, _fromAddress, price);
    ...
    function _claimNft(
        ...
        emit ClaimNFT(bid.listingId, biddingId, _fromAddress);

        _removeListing(bid.listingId, lst.seller);
        _removeBidding(biddingId, bid.bidder);
    }
    
    function updateFeeStatus(bool _status) public onlyOwner {
        isFeeEnable = _status;
    }
    ...
    function listingOfAddr(address addr)
        public
        view
     ...
     */
    function biddingOfAddr(address addr)
        public
        view
        returns (bytes32[] memory)
    {
    ...
    function _removeItemFromArray(
        bytes32 listingId,
        bytes32[] storage arrayOfIds
    ) internal {
     ...
     */
    function removeListing(address _fromAddress, bytes32 listingId)
        external
        nonReentrant
        onlyOwner
    {
        ...
        if (lst.sellMode != SellMode.FixedPrice) {
            require(lst.biddingId == 0, "Bidding is placed for this listingId");
        }
        require(lst.seller == _fromAddress, "Only seller can remove");
    ...
    function _depositNft(
        ...
        if (
            IERC165(_hostContract).supportsInterface(type(IERC721).interfaceId)
        ) {
            ...
            IERC165(_hostContract).supportsInterface(type(IERC1155).interfaceId)
        ) {
    ...
    /*
     * @notice Process fee which is the revenue
...
//** Author DanishGakhar :  Marketplace) Contract 2023.04 */

pragma solidity ^0.8.0;

import "./interfaces/IERC2981.sol";
import "./OOMContractBase.sol";

contract OOMMarketPlace is OOMContractBase {
    constructor(
        address _owner,
        IMRConfig _mrConfig
    ) OOMContractBase(_owner , _mrConfig) {}
    ...
    function placeBid(bytes32 listingId) external payable nonReentrant {
        _placeBid(msg.sender, listingId, msg.value);
    }
    function placeBid(address _fromAddress, bytes32 listingId) external payable nonReentrant onlyOwner {
        _placeBid(_fromAddress,listingId, msg.value);
    }
    ...
    function buyNow(bytes32 listingId) external payable nonReentrant {
        ...
        // uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        require(msg.value >= price, "Not enough funds to buy");

        _buyNow(msg.sender,listingId, price, false);
    }  
    function buyNowGL(address _fromAddress, bytes32 listingId) external payable nonReentrant onlyOwner{ // only opererator
        uint256 price = getPrice(listingId);
        // For BNB, the value need to larger then the price + fee
        // uint256 fee = (price * feeRate) / FEE_RATE_BASE;
        require(msg.value >= price, "Not enough funds to buy");

        _buyNow(_fromAddress,listingId, price, true);
    ...
    function claimNft(bytes32 biddingId) external payable nonReentrant {
        ...
        _claimNft(msg.sender,biddingId, bid, lst);
    }
    function claimNftGassLess(address _fromAddress, bytes32 biddingId) external payable nonReentrant onlyOwner {
        bidding storage bid = biddingRegistry[biddingId];
        require(bid.bidder == _fromAddress, "Only bidder can claim NFT");
        ...
        _claimNft(_fromAddress,biddingId, bid, lst);
    }
