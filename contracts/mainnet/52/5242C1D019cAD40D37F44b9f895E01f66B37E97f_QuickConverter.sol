...
contract QuickConverter is Ownable {
    ...
    //0xCf0B86f9944A60A0ba22b51a33C11d9E4dE1CE9F
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(
            token != quick && token != wmatic && token != bridge,
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == quick) {
                ...
                quickOut = _toQUICK(wmatic, amount);
            } else {
            ...
            quickOut = _toQUICK(
                wmatic,
                _swap(token1, wmatic, amount1, address(this)).add(amount0)
            );
        } else if (token1 == wmatic) {
            // eg. USDT - ETH
            quickOut = _toQUICK(
                wmatic,
                _swap(token0, wmatic, amount0, address(this)).add(amount1)
            );
        } else {
            // eg. MIC - USDT
