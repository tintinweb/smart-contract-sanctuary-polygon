...
contract Swap is SwapInterface, Initializable, AdapterManage, Constants, PausableUpgradeable, ReentrancyGuardUpgradeable  {    

    PerformerInterface public performer;
    
    using TransferHelper for address;

    function initialize(address performerContract) external initializer {
        __Ownable_init();
        __ReentrancyGuard_init();
        __Pausable_init();
        performer = PerformerInterface(performerContract);
    }
...
abstract contract AdapterManage is AdapterManageInterface, OwnableUpgradeable {
    ...
    function removeAdapter(uint256 index) external override onlyOwner {
        ...
        require(adapter._router != address(0), "Index not exists");
...
library AdapterSet {
     ...
     * already present.
     */
    function add(Set storage set, Adapter memory value) public returns (bool) {
     ...
     * present.
     */
    function remove(Set storage set, Adapter memory value) public returns (bool) {
     ...
     */
    function contains(Set storage set, Adapter memory value) public view returns (bool) {
     ...
     */
    function at(Set storage set, uint256 index) public view returns (Adapter memory) {
     ...
     */
    function values(Set storage set) public view returns (Adapter[] memory) {
        return set._values;
    }
}
...
abstract contract FeeTo is FeeToInterface, Constants, Ownable {
    ...
    function setFeeTo(address payable newFeeTo) public override onlyOwner {
        require(newFeeTo != address(0), "Error: feeTo Cant be zero");
        require(feeTo != newFeeTo, "ERROR: Address Cant be Same");
    ...
    function setFeeRate(uint newRate) external override onlyOwner {
        require(feeRate != newRate, "ERROR, new rate Cant be same");
    ...
    function distributorETH(address payable recipient, uint256 balance) internal {
        require(balance > 0, "eth can be zero");

        uint feeAmount = balance.mul(feeRate).div(1000);
        feeTo.sendValue(feeAmount);
        earned[ETH] = earned[ETH].add(feeAmount);
