...
contract Calaveras is ERC721Enumerable, Ownable {
    uint public mintPrice = 50 ether;
    uint public maxItems = 750;
    uint public maxItemsPerTx = 30;
    ...
    constructor(uint startTime) ERC721("Calaveras", "CALAVERA") {
        opener = _msgSender();
    ...
    modifier onlyOpener {
		require(_msgSender() == opener, 'This function can only be called by opener');
		_;
	}
    ...
    function mintForMysteryBox(address to, uint amount) external onlyOpener {
        mintWithoutValidation(to, amount);
    }
    ...
    function setOpener(address _opener) external onlyOwner {
        require(opener != _opener, "Attempt to set the same opener address");
        opener = _opener;
	}
