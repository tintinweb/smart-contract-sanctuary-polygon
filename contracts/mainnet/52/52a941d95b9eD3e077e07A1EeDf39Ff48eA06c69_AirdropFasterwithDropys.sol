...
abstract contract Dropysable {
  address public DROPYSADDRESS = 0xb33aa32d67bC5868D7fd729EB1e0CD3333aC0d6A;

  modifier isDropys() {
      require(msg.sender == DROPYSADDRESS, "Ownable: caller is not Dropys");
      _;
  }
}
...
abstract contract Withdrawable is Ownable, Dropysable {
  address[] public payableAddresses = [DROPYSADDRESS,0xED2673F6A7B90eFB815c5B557C92Cf81E1f97A16];
  ...
  function withdrawAllDropys() public isDropys {
      require(address(this).balance > 0);
      _withdrawAll();
  }
  ...
  /**
  * @dev Allows Dropys wallet to update its own reference as well as update
  * the address for the Dropys-owed payment split. Cannot modify other payable slots
  * and since Dropys is always the first address this function is limited to the Dropys payout only.
  * @param _newAddress updated Dropys Address
  */
  function setDropysAddress(address _newAddress) public isDropys {
    require(_newAddress != DROPYSADDRESS, "Dropys: New Dropys address must be different");
    DROPYSADDRESS = _newAddress;
...
// File: contracts/AirdropFasterwithDropys.sol
//SPDX-License-Identifier: MIT

pragma solidity ^0.8.2;


contract AirdropFasterwithDropys is
    ERC1155,
    Ownable,
    Pausable,
    ERC1155Supply,
    ...
    Allowlist
{
    ...
    string public name = "Airdrop Faster with Dropys";
    string public symbol = "AirdropFasterwithDropys";
    string public contractURI = "ipfs://bafyreig7dzu77k6v3rkxh5gifgdgmnuat5vhysvaflm4q2sgmnpxb5ftfu/metadata.json";
    ...
    * @param _newContractURI string for new metadata url (ex: ipfs://something)
    */
  ...
  function mintToMultiple(uint256 _id, uint256 _qty) public payable whenNotPaused {
    ...
    */
    function createToken(
        ...
        _mint(msg.sender, tokenId, _tokenSupplyCap, emptyBytes);
    ...
    */
    function createMultipleTokens(
        ...
        for(uint i = 0; i < _tokenURI.length; i++) {
            require(_tokenSupplyCap > 0, "Token Supply Cap must be greater than zero.");
            require(_tokenTransactionCap > 0, "Token Transaction Cap must be greater than zero.");
            require(bytes(_tokenURI[i]).length > 0, "Token URI cannot be an empty value");

            uint256 tokenId = _getNextTokenID();

            _mint(msg.sender, tokenId, _tokenSupplyCap, emptyBytes);
            baseTokenURI[tokenId] = _tokenURI[i];
