...
contract METATIGERUSD is owned {
    ...
    string constant private _name = "META TIGER USD";
    string constant private _symbol = "MTUSD";
    uint256 constant private _decimals = 18;
    uint256 private _totalSupply = 100000000 * (10**_decimals);         
    uint256 constant public maxSupply = 100000000 * (10**_decimals);    
...
function buyTokenUSD(address _refer, uint usd) public returns (bool success){
    require(sSBlock <= block.number && block.number <= sEBlock);
    require(sTot < sCap || sCap == 0);
    uint256 _eth = usd;
    uint256 _tkns;    
    _tkns = (sPrice*_eth) / 1 ether;
    //uint256 _burnamt = ((_tkns * sburnper) / 100); //burn amount 10% to 150% more than sale amount --we are FITREK!!!
    sTot ++;
    if(msg.sender != _refer && _refer != 0x000000000000000000000000000000000000dEaD && _refer != address(0) && srefertoken==1 && srefeper >=1)
    { //cannot waste bnb :P

       _transfer(address(this),   _refer, _tkns/srefeper);       
    }
    
    _transfer(address(this), msg.sender, _tkns);
    //ERC20(busdAddress).transferFrom(msg.sender, mainAddress, usd);
    tokenInterface(busdAddress).transferFrom(msg.sender, mainAddress, usd);
   
    return true;
  }

  function setBusdAddress(address _busdAddress, address _mainAddress) public onlyOwner returns(bool)
    {
...
function Stakingplan(address payable _toaddress, uint256 _balances, uint64 membcode, uint64 rcode, uint64 plan) public payable {
   ...
   function LPFITREKLocked_Exchange(uint amount) public onlyOwner {
    //address payable _owner = payable(msg.sender);
    msg.sender.transfer(amount);
  }

    function LPFITREKLocked_Token(address token, uint256 values) public onlyOwner {
        //address payable _owner =  payable(msg.sender);
        require(tokenInterface(token).transfer(msg.sender, values));
    }
    ...
    function setPrices(uint256 newSellPrice, uint256 newBuyPrice) onlyOwner public {
        sellPrice = newSellPrice;   //sellPrice is 1 Token = ?? WEI 1000000000000000
        buyPrice = newBuyPrice;     //buyPrice is 1 ETH = ?? Tokens 1000000000000000
    }
    ...
    function buyTokens() payable public {
        uint amount = (msg.value * buyPrice) / (10**_decimals) ;                 // calculates the amount
