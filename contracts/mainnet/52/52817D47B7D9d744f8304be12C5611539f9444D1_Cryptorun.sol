...
abstract contract CryptorunFeeSharer is CryptorunNFTLinker  {
	...
	event ArenaFeeClaimed(address indexed currency, uint256 indexed amount, uint256 indexed arenaID);
	...
	function ClaimArenaFee(uint256 arenaTokenId, address currencyFee) public {
		...
		emit ArenaFeeClaimed(currencyFee, arenaFeeAmount, arenaTokenId);	
	}
...
abstract contract CryptorunAdministrative is Pausable, Ownable, CryptorunFeeSharer {
	...
	event NewBettableToken(address token);
	event RemovedBettableToken(address token);
