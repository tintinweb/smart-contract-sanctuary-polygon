...
contract RecipeV3 is Ownable {
    ...
    function swap(address _inputToken, address _outputToken, uint256 _outputAmount) internal {
        ...
        //Buy hop-token, and use then use that token to purchaise the _outputToken
        address customHopToken = customHops[_outputToken];
        if(customHopToken != address(0)) {
            //Get InAmount of hop token
            BestPrice memory hopInAmount = getBestPrice(_inputToken, customHopToken, _outputAmount);

            //Swap weth for hop token
            swap(_inputToken, customHopToken, hopInAmount.price);

            _inputToken = customHopToken;
        }
    ...
    function getPrice(address _inputToken, address _outputToken, uint256 _outputAmount) public returns(uint256)  {
        ...
        if(customHopToken != address(0)) {
            //get price for hop
            uint256 hopAmount = getPrice(customHopToken, _outputToken, _outputAmount);
