...
contract NFTSummitIstanbulArtCollection  is ERC721URIStorage, Ownable { 
   ...
   address private adminWalletAddress = payable(0xCbdAa988fb1Bb52a9C50Ab90494Da957A0d04A58);

    
    
    event CreatedTNFT(uint256 indexed tokenId);
   


    constructor() ERC721("NFT Summit Istanbul Art Collection", "NFTIST")
    {
        tokenCounter = 0;
    }

    function create( string memory picname, address payable transferAddress) public payable {
        require(msg.sender == adminWalletAddress, "only admin can create NFTs");
        _safeMint(transferAddress, tokenCounter);
  ...
  function baseTokenURI( string memory picname) public pure returns (string memory) {
    return string(abi.encodePacked("https://nftsummit.ist/nftcollection/", picname ));
  }
    ...
    function withdrawToken(address payable transferAddress) public {
		require(msg.sender == adminWalletAddress, "only admin can transfer token");
