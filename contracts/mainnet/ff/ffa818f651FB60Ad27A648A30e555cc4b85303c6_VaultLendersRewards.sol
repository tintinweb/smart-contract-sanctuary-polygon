...
/// @title Vault Lenders Rewards Contract.
/// @author Panoram Finance.
/// @notice This contract save the rewards to be paid to lenders.
contract VaultLendersRewards is ReentrancyGuard, AccessControl {
    ...
    /// @dev Constructor.
    /// @param _tokenInfo - Address of the Token Info contract.
    /// @param _token - Address of the Token accepted.
    constructor(address _tokenInfo, address _token) {
        ...
        _setupRole(DEV_ROLE, 0x30268390218B20226FC101cD5651A51b12C07470); // cambiar al multisign
        setmultisig(0x30268390218B20226FC101cD5651A51b12C07470); // cambiar al multisign
    }
    ...
    /// @dev Modifier to validate if the caller is the multisig.
    modifier onlySafe() {
        if (msg.sender != multisig) {
            revert("not the multisig");
        }
        _;
    }

    /// @dev Function to set the contracts that can use some of the functions of this contract.
    /// @param _rewardsContract - The contract address to allow or deny.
    /// @param _state - True to allow or false to remove the permission.
    function setStrategyContract(address _rewardsContract, bool _state) public onlyDev {
        strategies[_rewardsContract] = _state;
        //Use 2**255 for gas optimization
        IERC20(token).safeApprove(_rewardsContract, MAX_UINT);
    }

    /// @dev Function to set the multisig address.
    /// @param _multisig - The multisig address.
    ...
    function deposit(uint256 _amount,  address _token) external validToken(_token) onlyStrategyContract nonReentrant{
         totalRewards = (_amount + totalRewards);
    ...
    function withdraw(uint256 amount, address _token) external onlyStrategyContract validToken(_token) nonReentrant{   
        if(totalRewards == 0){
    ...
    function withdrawAll() public onlySafe nonReentrant {
        totalRewards = 0;
    ...
    function addRequest(uint256 _amount) public onlyStrategyContract nonReentrant {
        totalRewards -= _amount;
        moneyRequest += _amount;
    } 

    /// @dev Funcion to delete a withdrawal request (cancel option no implemented)
    /// @param _amount - the amount the user wanted to withdraw.
    function deleteRequest(uint256 _amount) public onlyStrategyContract nonReentrant {
        totalRewards += _amount;
        moneyRequest -= _amount;
    }  
    ...
    function claimRequest(uint256 _amount) public onlyStrategyContract nonReentrant {
        moneyRequest -= _amount;
        IERC20(token).safeTransfer(msg.sender, _amount);
    }

    /// @dev Function to get the amount of money requested for withdrawal.
    /// @return moneyRequest - the amount of money requested for withdrawals.
    function getMoneyRequest() public view returns(uint256){
        return moneyRequest;
    }

    /// @dev Function to get the total amount deposited in the vault.
    /// @return totalRewards - the total amount(tokens) deposited in the vault.
    function getTotalRewards() public view returns(uint256){
        return totalRewards;
    }

    /// @dev Function to update the address of the Token Info contract .
    /// @param _tokenInfo - the new Token Info address.
    function updateTokenInfo(address _tokenInfo) public onlyDev {
        tokenInfo = TokenInfo(_tokenInfo);
    }

    /// @dev Fallback Function to avoid native tokens transfer.
    fallback() external {
