...
contract CubixSellPack is Ownership {
    ...
    mapping(uint => uint256) public packBought;
    ...
    constructor(address _cubixaddress, address _cubixPackSellV1){          
        ...
        PKGDetail.push(pkgDetail("Starter",30,30 * cubixperusd * decimalsValue, 1,1,10500)); 
        PKGDetail.push(pkgDetail("Bronze",100,100 * cubixperusd * decimalsValue,6,1,71500));   
        PKGDetail.push(pkgDetail("Silver",250,250 * cubixperusd * decimalsValue,11,1,51000));       
        PKGDetail.push(pkgDetail("Gold",500,500 * cubixperusd * decimalsValue,25,1,40000));
        PKGDetail.push(pkgDetail("Platinum",1000,1000 * cubixperusd * decimalsValue,57,1,10000));
        PKGDetail.push(pkgDetail("Diamond",2500,2500 * cubixperusd * decimalsValue, 153,1,5000)); 
        PKGDetail.push(pkgDetail("MegaPack",5000,5000 * cubixperusd * decimalsValue,343,1,0)); 
    ...
    function register(address referrerAddress, address placement, uint256 usdAmount, uint pkgIndex, uint _position, bool isUSDT) external returns(bool)
    {
        ...
        if(PKGDetails.buyLimit > 0){
            require(PKGDetails.buyLimit > packBought[pkgIndex], 'Pack sold out');
        }
    ...
    function setPackBought(uint index, uint256 _boughtPkgs) external onlyAuthorized {
            packBought[index] = _boughtPkgs;
    }
    ...
    function updatePkg(uint _index, string calldata _text, uint256 _amount, uint256 _nfts, uint256 _sportsId,uint256 _buylimit) external onlyOwner {
        pkgDetail storage PKGDetails = PKGDetail[_index];
        ...
        PKGDetails.buyLimit = _buylimit;
    }
    function addPkg(string calldata _text, uint256 _amount, uint256 _nfts, uint256 _sportsId,uint256 _buylimit) external onlyOwner {
        PKGDetail.push(pkgDetail(_text,_amount,_amount * cubixperusd * decimalsValue,  _nfts, _sportsId,_buylimit));  
    }
