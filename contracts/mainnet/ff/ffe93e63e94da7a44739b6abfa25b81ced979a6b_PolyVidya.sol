...
contract PolyVidya is ReentrancyGuard, UtilityToken{
    ...
    address public vidyaWethLp;
    uint256 public ratio = 1157407407407407; 

    IERC20 public _tokenLP;

    uint256 public halvingAmount = 50000 ether;
    uint256 public rateReduction = 1;

    constructor()UtilityToken("PolyVidya", "pV", address(0)){
        _tokenLP= IERC20(address(0xf52dD4063D034e50F56CD3d37c6816498F0E8B70));
        vidyaWethLp = address(0xf52dD4063D034e50F56CD3d37c6816498F0E8B70);
    }

    mapping(address => Provider) public _provider;
    ...
    function depositToken(uint256 amount, uint256 daoId)external nonReentrant{
   
        require(_tokenLP.balanceOf(msg.sender) >= amount, "Not enough Balance");
        Provider storage p = _provider[msg.sender];
        _tokenLP.safeTransferFrom(msg.sender, address(this), amount);
            
        if(p.timeOfLastClaim!=0){
            _claim(msg.sender, daoId);
            uint256 a =((p.timeOfDeposit*p.totalDeposited) + (amount*block.timestamp)) / (amount + p.totalDeposited);
            p.timeOfDeposit = a; 
    ...
    function _claim(address user, uint256 daoId)internal {

        uint256 c = block.timestamp;
        Provider storage p = _provider[user];
        uint256 amount = claculateAmount(p,user, c, daoId);
        p.timeOfLastClaim = c;
        p.totalClaimed += amount;
        _mint(user, amount);
    ...
    function claculateAmount(Provider memory user, address _user, uint256 c, uint256 daoId)internal view returns(uint256){
        uint256 b = bonus(_user, daoId)+1;
        return ((b * (c-user.timeOfLastClaim) * user.totalDeposited)/ ratio)/rateReduction;
    }

    function claim( uint256 daoId)external nonReentrant{
        _claim( msg.sender, daoId);
    }
    ...
    function withdraw( uint256 daoId, uint256 amount)external nonReentrant{
        uint256 pV = polyVidyaOwed(msg.sender, amount, daoId);
        Provider storage p = _provider[msg.sender];
        ...
        if(p.totalDeposited == amount){
            ...
            uint nTC = (p.totalClaimed *amount)/p.totalDeposited;
            p.totalDeposited -= amount;
            if(p.totalClaimed > nTC){p.totalClaimed -= nTC;}
            else{p.totalClaimed = 0;}
            _claim( msg.sender, daoId);
        }

        _tokenLP.safeTransfer(msg.sender, amount);
    }

    function polyVidyaOwed( address user, uint256 amount, uint256 daoId)public view returns(uint256 pV){
        ...
        pl = pl*amount*220752000;
        Provider memory p = _provider[user];
    ...
    function userData(address user, uint256 daoId)public view returns(ProviderData memory){

        ProviderData memory P;
        uint256 c = block.timestamp;

        Provider memory provider =_provider[user];

        uint256 claimAmount = claculateAmount(provider, user, c, daoId);
        uint256 pVDue = polyVidyaOwed(user, provider.totalDeposited, daoId);
        P = ProviderData(claimAmount, pVDue, provider);
