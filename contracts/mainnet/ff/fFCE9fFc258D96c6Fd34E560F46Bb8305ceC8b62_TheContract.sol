...
contract TheContract is ERC721Enumerable, ERC721OpenSeaGassLess, ERC721Batch, ERC721Royalty, Ownable, ReentrancyGuard {
  enum MintPhase {
    PreSale,
    Sale,
    PostSale
  ...
  uint256 public constant RESERVES = 1;
  uint256 public constant MAX_SUPPLY = 1193;
  uint256 public constant PRICE_IN_WEI = 177 ether;
  ...
  ) ERC721("The Contract", "CONTRACT") ERC721OpenSeaGassLess(proxyRegistryAddress_) {
  ...
  function mint(uint64 amount) public payable callerIsUser {
    require(_mintPhase == MintPhase.Sale, "Sale not open");
    require(_tokenId + amount <= MAX_SUPPLY, "Exceeds max supply");
    require(MAX_PER_TX >= amount, "Exceeds max per tx");
    require(amount * PRICE_IN_WEI == msg.value, "Invalid funds provided");
