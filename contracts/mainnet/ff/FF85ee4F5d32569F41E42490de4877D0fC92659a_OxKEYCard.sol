...
contract OxKEYCard is ERC1155Supply, Ownable {
  ...
  **/
  function setURI(
    ...
    require(_id <= _currentTokenID, "nonexistent token");
    tokenURIs[_id] = _uri;
  }

  /**
   * @dev return a token's URI
  **/
  function uri(
    uint256 _id
  ...
  /**
    * @dev Airdrop one token to some addresses
    * @param _receivers   Addresses of the future owners of the token
    * @param _id          Token ID to airdrop
  */
  function airdrop(
    ...
    require(_id <= _currentTokenID, "nonexistent token");
    require(_receivers.length > 0, "_receivers length is 0");
    for (uint256 i = 0; i < _receivers.length; ++i) {
      _mint(_receivers[i], _id, 1, "0x00");
    }
  }
  ...
  */
  ...
  /**
    * @dev Burn some amount of tokens of an address
