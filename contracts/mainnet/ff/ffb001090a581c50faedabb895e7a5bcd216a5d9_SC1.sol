...
contract SC1 is Ownable, IAxelarExecutable {
    ...
    address public _EABCDADDRESS = 0x685D185F35790213A56Aec9fCcdA097Ac055b0D7; //testnet
    address public _PANCAKEROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;  //testnet
    // address public _PANCAKEROUTER = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;  //mainnet
    address public _SC2ADDRESS = 0x03A24DCcaD5C8d16c6a9E29701df77AAC4e0aD65;
    address public _SC3ADDRESS = 0x33a1a829B6177FF907F834cae582D74BdDf3785c;
    ...
    uint256 public test1 = 0;
    uint256 public test2 = 0;
    ...
    function _execute (
        ...
        crossChainBuyOrder(amount, balance, sourceChain_, sourceAddress_, walletAddress );
    }

    function callContract (string memory order, string memory _destinationChain, string memory _destinationAddress, address _userWallet, uint256 _amount, uint256 gasfee) internal {
        bytes memory payload = abi.encode(order, _amount, _userWallet);
        if(gasfee > 0) {
    ...
    function crossChainBuyOrder(uint256 _amount, uint256 balance, string memory destinationChain, string memory destinationAddress, address userWallet) public{
        uint256 BUSD_decimal = IBEP20(_BUSDADDRESS).decimals();
        uint256 eABCD_decimal = IBEP20(_EABCDADDRESS).decimals();
        uint256 priceFromOracle = ISC3(_SC3ADDRESS).getAssetPrice();
        uint256 feeForBuyer = _calFee(balance);
        uint256 tken_amount = _amount;

        callContract("buyOrder", destinationChain,destinationAddress, userWallet, tken_amount * (100 * 1000 - feeForBuyer) * (10 ** eABCD_decimal) / (10 ** BUSD_decimal)/  1000 / 100 / priceFromOracle, 100000000000000);

        swapBUSDForTHALES( tken_amount * feeForBuyer * buybackFee / 1000 / 100 / 100);
        IBEP20(_BUSDADDRESS).transfer(_SC2ADDRESS, tken_amount * feeForBuyer * (100 - buybackFee) / 1000 / 100 / 100);
