...
contract TokenSwap is Ownable {
    ...
    // QUICK token address
    IERC20 public immutable halal;

    // xQUICK token address
    IERC20 public immutable halalX;
     ...
     */
    event XHalalToHalal(uint256 halalAmount, uint256  halalxAmount, address indexed account);
    ...
    constructor (
        ...
        require(address(_halal) != address(0), "Invalid address");
        require(address(_halalX) != address(0), "Invalid address");
    ...
    function halalToHalalX(uint256 halalXAmount) public {
       
        halalX.safeTransferFrom(msg.sender, DEAD, halalXAmount);

        uint256 halalAmount = halalXAmount * SWAP_RATIO;
        halal.safeTransfer(msg.sender, halalAmount);

        emit XHalalToHalal(halalAmount, halalXAmount, msg.sender);
    }
    ...
    function withdrawTokens(address tokenAddress, uint256 amount) public onlyOwner {
        if(tokenAddress == address(halalX)) {
            require(
