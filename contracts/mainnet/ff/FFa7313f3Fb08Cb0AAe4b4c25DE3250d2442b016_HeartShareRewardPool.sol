...
contract HeartShareRewardPool {
    ...
    // The time when heartShare mining starts.
    uint256 public poolStartTime;

    // The time when heartShare mining ends.
    uint256 public poolEndTime;

    uint256 public sharesPerSecond = 0.00009384384 ether; // 3000 heartShare / (0
    ...
    constructor(
        ...
        if (_heartShare != address(0)) heartShare = IERC20(_heartShare);
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "HeartShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "HeartShareRewardPool: existing pool?");
        }
    }
    ...
    // View function to see pending HeartShare on frontend.
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            ...
            uint256 _heartShareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accTokensPerShare = accTokensPerShare.add(_heartShareReward.mul(1e18).div(tokenSupply));
        }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _heartShareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accTokensPerShare = pool.accTokensPerShare.add(_heartShareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    // Deposit LP tokens.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (_pending > 0) {
                safeHeartShareTransfer(_sender, _pending);
    ...
    // Safe heartShare transfer function, just in case if rounding error causes pool to not have enough heartShare.
    function safeHeartShareTransfer(address _to, uint256 _amount) internal {
        uint256 _heartShareBal = heartShare.balanceOf(address(this));
        if (_heartShareBal > 0) {
            if (_amount > _heartShareBal) {
                heartShare.safeTransfer(_to, _heartShareBal);
            } else {
                heartShare.safeTransfer(_to, _amount);
            }
        }
    }
    ...
    function governanceRecoverUnsupported(IERC20 _token, uint256 amount, address to) external onlyOperator {
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (heartShare or lps) if less than 90 days after pool ends
            require(_token != heartShare, "heartShare");
            uint256 length = poolInfo.length;
            for (uint256 pid = 0; pid < length; ++pid) {
