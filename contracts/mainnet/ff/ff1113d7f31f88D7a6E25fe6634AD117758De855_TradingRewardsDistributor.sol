...
// File: contracts/TradingRewardsDistributor.sol
 ...
 * @notice It distributes DGM tokens with rolling Merkle airdrops.
 */
contract TradingRewardsDistributor is Pausable, ReentrancyGuard, Ownable, MinterRole {
    using SafeERC20 for IERC20;

    uint256 public constant BUFFER_ADMIN_WITHDRAW = 3 days;

    IERC20 public immutable dgameToken;
    ...
    // Total amount claimed by user (in DGM)
    ...
    /**
     * @notice Constructor
     * @param _dgameToken address of the DGame token
     */
    constructor(address _dgameToken) {
        dgameToken = IERC20(_dgameToken);
        _pause();
    }
    ...
    function claim(uint256 amount, bytes32[] calldata merkleProof) external whenNotPaused nonReentrant {
        ...
        dgameToken.safeTransfer(msg.sender, adjustedAmount);
     ...
     */
    function updateTradingRewards(bytes32 merkleRoot, uint256 newMaximumAmountPerUser) external onlyMinter {
    ...
    /**
     * @notice Transfer DGM tokens back to owner
    ...
    function withdrawTokenRewards(uint256 amount) external onlyOwner whenPaused {
        ...
        dgameToken.safeTransfer(msg.sender, amount);
