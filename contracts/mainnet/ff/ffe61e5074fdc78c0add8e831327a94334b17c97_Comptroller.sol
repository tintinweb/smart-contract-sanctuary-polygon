...
contract Comptroller is ComptrollerV3Storage, ComptrollerInterface, ComptrollerErrorReporter, Exponential {
    ...
    function exitMarket(address cTokenAddress) external returns (uint) {
        ...
        if (storedList.length == 0) {
            if(allBorrowers.length > 1){
                uint256 exiterIndex = borrowerIndexes[msg.sender]; // the index of the borrower which is exiting.
                address lastEl = allBorrowers[allBorrowers.length - 1]; // the last item in borrowers list

                allBorrowers[exiterIndex] = lastEl; // Copy last item in list to location of item to be removed
                borrowerIndexes[lastEl] = exiterIndex; // Set borrower index of moved item to correct index
            }
    ...
    function mintAllowed(address cToken, address minter, uint mintAmount) external returns (uint) {
        ...
        if (supplyCap != 0) {
            ...
            require(nextTotalUnderlyingSupply < supplyCap, "supply cap reached");
        }
    ...
    function liquidateBorrowAllowed(
        ...
        if (isDeprecated(CToken(cTokenBorrowed))) {
            require(borrowBalance >= repayAmount, "repay > total borrow");
        } else {
    ...
    function _addRewardsDistributor(address distributor) external returns (uint) {
        ...
        // Check marker method
        require(RewardsDistributorDelegate(distributor).isRewardsDistributor(), "!isRewardsDistributor");

        // Check for existing RewardsDistributor
        for (uint i = 0; i < rewardsDistributors.length; i++) require(distributor != rewardsDistributors[i], "rd already added");
    ...
    function _supportMarket(CToken cToken) internal returns (uint) {
        ...
        require(cToken.isCToken(), "!isCToken");

        // Check cToken.comptroller == this
        require(address(cToken.comptroller()) == address(this), "CToken has diff comptroller");
    ...
    function _setMintPaused(CToken cToken, bool state) public returns (bool) {
        require(markets[address(cToken)].isListed, "cant pause an unlisted market");
        require(msg.sender == pauseGuardian || hasAdminRights(), "only pause guardian or admin");
    ...
    function _beforeNonReentrant() external {
        require(markets[msg.sender].isListed, "Cmp_b:not_listed");
        require(_notEntered, "reenter assets");
        _notEntered = false;
    }
    ...
    function _afterNonReentrant() external {
        require(markets[msg.sender].isListed, "Cmp_a:not_listed");
        _notEntered = true; // get a gas-refund post-Istanbul
    }
}
