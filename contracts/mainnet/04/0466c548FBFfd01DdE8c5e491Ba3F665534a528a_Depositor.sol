...
contract Depositor is AragonApp {
    ...
    bytes32 constant public SET_TOKEN_REQUEST_ROLE = keccak256("SET_TOKEN_REQUEST_ROLE");
    bytes32 constant public SET_WEEZICORE_ROLE = keccak256("SET_WEEZICORE_ROLE");
    bytes32 constant public FINALISE_TOKEN_REQUEST_ROLE = keccak256("FINALISE_TOKEN_REQUEST_ROLE");
    string private constant ERROR_EXPIRED_CREATE_REQUEST_DATA = "EXPIRED_CREATE_REQUEST_DATA";
    string private constant ERROR_ADDRESS_NOT_CONTRACT = "TOKEN_REQUEST_ADDRESS_NOT_CONTRACT";
    string private constant ERROR_ETH_TRANSFER_FAILED = "SERVICE_FEE_ETH_TRANSFER_FAILED";
    string private constant ERROR_TOKEN_TRANSFER_REVERTED = "SERVICE_FEE_TOKEN_TRANSFER_REVERTED";
    ...
    event TokenRequestFinalised(uint256 requestId, address requester, address depositToken, uint256 depositAmount, address requestToken, uint256 requestAmount, address serviceAddress, uint256 serviceFee);

    modifier withValidData(
        FinaliseTokenRequestParams params
    ) {
        ...
        bytes32 dataHash = keccak256(
            abi.encodePacked(params._tokenRequestId, params._serviceAddress, params._serviceFee, params._timestamp)
        );

        require(
            weeziCore.isValidSignature(
                dataHash,
                params._signature
            ),
            "INVALID_SIGNATURE"
    ...
    */
    function initialize(address _tokenRequest, address _weeziCore) external onlyInit {
    ...
    */
    function setTokenRequest(address _tokenRequest) external auth(SET_TOKEN_REQUEST_ROLE) {
        require(isContract(_tokenRequest), ERROR_ADDRESS_NOT_CONTRACT);
    ...
    */
    function setWeeziCore(address _weeziCore) external auth(SET_WEEZICORE_ROLE) {
        require(isContract(_weeziCore), ERROR_ADDRESS_NOT_CONTRACT);
    ...
    */
    function finaliseTokenRequest(
       FinaliseTokenRequestParams params
        ...
        nonReentrant
    {
        require(params._tokenRequestId < tokenRequest.nextTokenRequestId(), ERROR_NO_REQUEST);
        (address requesterAddress, address depositToken, uint256 depositAmount, uint256 requestAmount) = tokenRequest.getTokenRequest(params._tokenRequestId);

        tokenRequest.finaliseTokenRequest(params._tokenRequestId);

        if (depositAmount > 0) {
            require(depositAmount > params._serviceFee, ERROR_SERVICE_FEE_TOO_HIGH );

            if (params._serviceFee > 0) {
                Vault(tokenRequest.vault()).transfer(depositToken, params._serviceAddress, params._serviceFee);
            }
        }
        
        emit TokenRequestFinalised(params._tokenRequestId, requesterAddress, depositToken, depositAmount, address(tokenRequest.tokenManager().token()), requestAmount, params._serviceAddress, params._serviceFee);
    }
}
