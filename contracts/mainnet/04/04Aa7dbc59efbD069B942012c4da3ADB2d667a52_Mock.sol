...
contract Mock {
    ...
    function check(
        ...
        emit Address3(argsLiFiData.receivingAssetId, argsPrepareArgs.invariantData.receivingAssetId, tokenIn);
        if(argsLiFiData.receivingAssetId != argsPrepareArgs.invariantData.receivingAssetId){
            emit String("LiFiAdapter: NOT argsLiFiData.receivingAssetId == argsPrepareArgs.invariantData.receivingAssetId");
        }
        if (argsLiFiData.receivingAssetId != tokenIn) {
            emit String("LiFiAdapter: NOT receivingAssetId == tokenIn");
        }
    }
}
