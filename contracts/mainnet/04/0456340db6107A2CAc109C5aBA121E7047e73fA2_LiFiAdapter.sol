...
contract LiFiAdapter is AdapterBase {
    ...
    function getSlice(bytes memory array, uint256 begin, uint256 end) public pure returns (bytes memory) {
    ...
    function _executeCall(
        ...
        require( // selector == sendArgs.lifiTxData[:4]
            (ILiFi(target).startBridgeTokensViaNXTP.selector[0] == sendArgs.lifiTxData[0]) &&
            (ILiFi(target).startBridgeTokensViaNXTP.selector[1] == sendArgs.lifiTxData[1]) &&
            (ILiFi(target).startBridgeTokensViaNXTP.selector[2] == sendArgs.lifiTxData[2]) &&
            (ILiFi(target).startBridgeTokensViaNXTP.selector[3] == sendArgs.lifiTxData[3])
            , "LiFiAdapter: wrong selector, only startBridgeTokensViaNXTP is supported");  // only startBridgeTokensViaNXTP is supported currently
        bytes memory lifiTxDataNoSelector = getSlice(sendArgs.lifiTxData, 4, sendArgs.lifiTxData.length);  // sendArgs.lifiTxData[4:]
        (
            _ILiFi.LiFiData memory argsLiFiData,
            ITransactionManager.PrepareArgs memory argsPrepareArgs
        ) = abi.decode(lifiTxDataNoSelector, (_ILiFi.LiFiData, ITransactionManager.PrepareArgs));

        require(argsLiFiData.receivingAssetId == argsPrepareArgs.invariantData.receivingAssetId, "LiFiAdapter: NOT argsLiFiData.receivingAssetId == argsPrepareArgs.invariantData.receivingAssetId");
        require(argsLiFiData.receivingAssetId == tokenIn, "LiFiAdapter: NOT receivingAssetId == tokenIn");
        require(argsLiFiData.amount == argsPrepareArgs.amount, "LiFiAdapter: NOT argsLiFiData.amount == argsPrepareArgs.amount");
        require(argsLiFiData.amount >= amountIn, "LiFiAdapter: NOT argsLiFiData.amount >= amountIn");
