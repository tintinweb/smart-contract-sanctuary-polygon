    ...
    NativeMetaTransaction
{
    ...
    event ClaimIdWhitelisted(string claimId, address toAddress, uint32 amount);
    event ClaimIdMinted(string claimId, address toAddress, uint32 amount);

    uint256 private _nextTokenId = 0;
    uint32 private _whitelistedTotal = 0;
    string private _btURI;
    bool private _disablePseudoRandomReveal;
    ...
    // 0 means collection is NOT revealed, other numbers mean it is
    uint256 public startingIndex = 0;

    // the version of the recipient (the collection contract)
    ...
    // proof that metadata is unchanged (deterministically created MD5 hash of the metadata json)
    ...
    constructor(
        ...
        // if true startIndex is 1
        bool __disablePseudoRandomReveal
    ) ERC721(_name, "") {
        supplyCap = _supplyCap;
        metadataProofHash = _metadataProofHash;
        _cURI = __cURI;
        _disablePseudoRandomReveal = __disablePseudoRandomReveal;
    ...
    function whitelistClaimAmount(
        ...
        for (uint256 i = 0; i < customerAddressesArr.length; i++) {
    ...
    function whitelistedClaimIdForAddress(address customerAddress, string memory claimId)
        public
        view
        returns (bool)
    {
    ...
    function whitelistedClaimIdAmount(string memory claimId) public view returns (uint32) {
        return whitelistedClaimIds[claimId];
    }

    function mintedClaimIdAmount(string memory claimId) public view returns (uint32) {
    ...
    function canMintAmount(address customerAddress, uint32 amount) public view returns (bool) {
    ...
    function mint(uint32 amount, string memory claimId) external {
        ...
        require(totalSupply() + amount <= supplyCap, "collection minting cap reached");
        ...
        require(mintedClaimIds[claimId] == 0, "NovelCollection: claim ID has already been minted");
     ...
     */
    function reveal(string memory baseTokenURI) external onlyOwner returns (bool) {
        if (isRevealed()) {
            return false;
        }
        ...
        // Prevent sequence from possibly matching one to one
        if (startingIndex == 0) {
            startingIndex = 1;
        }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if (!isRevealed()) {
            return string(abi.encodePacked(_cURI, "/", tokenId.toString(), "/placeholder.json"));
        }

        // offset of 1 because token IDs start at 1, not 0
        uint256 trueIndex = ((startingIndex + tokenId - 1) % supplyCap) + 1;
        return string(abi.encodePacked(_btURI, "/", trueIndex.toString(), ".json"));
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(IERC165, ERC721Enumerable, ERC2981)
        returns (bool)
    {
        return
...
contract PaymentSplitter is Context, Ownable {
    ...
    event PaymentReceivedFallback(address from, uint256 amount);
    ...
    function release(address account) public virtual {
        ...
        address payable accountAddress = payable(account); //// Converting to payable

        Address.sendValue(accountAddress, payment);
        emit PaymentReleased(accountAddress, payment);
    }
    ...
    // internal private logic to check the pending balance of an account for both ERC20 and Native Token
    ...
    // To check the remaining Payment of each user's Native token

    function remainingPayment(address account) public view returns (uint256) {
        return
            ((address(this).balance + totalReleased()) * _shares[account]) /
            _totalShares -
            released(account);
    }

    // To check the remaining Payment of each user's ERC20 Tokens

    function remainingPayment(IERC20 token, address account) public view returns (uint256) {
        return
            ((token.balanceOf(address(this)) + totalReleased(token)) * _shares[account]) /
            _totalShares -
            released(token, account);
    }

    /// ADD Payee with shares
    ...
    /// ADD distribute all Native Tokens

    function distributeAll() public onlyOwner {
        for (uint256 i = 0; i < _payees.length; i++) {
            if (remainingPayment(_payees[i]) > 0) {
                release(_payees[i]);
            }
        }
    }

    /// ADD distribute all ERC20 Tokens

    function distributeAll(IERC20 token) public onlyOwner {
        for (uint256 i = 0; i < _payees.length; i++) {
            if (remainingPayment(token, _payees[i]) > 0) {
