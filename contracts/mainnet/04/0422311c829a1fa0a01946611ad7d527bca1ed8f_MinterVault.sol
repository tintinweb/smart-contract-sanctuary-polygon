...
// File contracts/NFTFairVaultBase.sol
...
/**
 * @title NFTFairVaultBase
 * @notice NFTFair Vaults are responsible for fairly distributing NFTs, a.k.a. Fairdrop, this is all made possible by Conveyor. The Vault contract achieves this by inheriting the ConveyorBase contract.
 * The NFTVaultBase contract provides the essential features to receive and organize NFTs by assigning categories.
 */
contract NFTFairVaultBase is INFTFairVault, ConveyorBase, IERC721Receiver, ERC165, IERC1155Receiver {
    ...
    mapping(address => mapping(uint256 => uint256)) internal _userClaimedBalance;
     ...
     */
    function claimed(address _user, uint256 _category) public view virtual override returns (uint256[] memory) {
        return _userClaimedInfo[_user][_category];
    }
     ...
     */
    function claimedCount(address _user, uint256 _category) public view virtual override returns (uint256) {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            sum += _userClaimedBalance[_user][ids[i]];
        }
        return sum;
    }
     ...
     */
    function getClaimableIds(uint256 _category) public view virtual override returns (uint256[] memory) {
        CategoryInfo storage category = _categories[_category];
     ...
     */
    function claimableCount(uint256 _category) public view virtual override returns (uint256) {
        CategoryInfo storage category = _categories[_category];
     ...
     */
    function getCategoryPurchaseLimit(uint256 _category) public view virtual override returns (uint256) {
        CategoryInfo storage category = _categories[_category];
        return category.purchaseLimit;
    }
     ...
     */
    function getCategoryPrice(uint256 _category) public view virtual override returns (uint256) {
        CategoryInfo storage category = _categories[_category];
        return category.price;
    }
     ...
     * @return {category.nft}
     */
    function getCategoryNft(uint256 _category) public view virtual override returns (address) {
        CategoryInfo storage category = _categories[_category];
        return category.nft;
    }
     ...
     * @return {category.ids}
     */
    function getCategoryNftIds(uint256 _category) public view virtual override returns (uint256[] memory) {
        CategoryInfo storage category = _categories[_category];
    ...
    // === INITIALIZER ===
    constructor(address _forwarder, address _treasury) ConveyorBase(_forwarder) {
        treasury = _treasury;
    }
     ...
     */
    function configurePayment(address _newToken, uint256[] memory _prices) external virtual override onlyOwner {
     ...
     */
    function recover(uint256[] memory _categoryArr, address _recipient) external virtual override onlyOwner {
     ...
     */
    function revenuePayout(address _beneficiary, address _token) external onlyOwner {
     ...
     */
    function _updateCategoryPricing(uint256[] memory _prices) internal virtual {
...
// File contracts/templates/MinterVault.sol
...
/**
 * @title MinterVault
 * @notice This vault does not store pre-minted NFTs. It mints NFTs on the spot as users call `claim()`
 */
contract MinterVault is NFTFairVaultBase {
    using EnumerableSet for EnumerableSet.UintSet;
    using Counters for Counters.Counter;

    constructor(address _forwarder, address _treasury) NFTFairVaultBase(_forwarder, _treasury) {}

    // === EVENT LOGGER ===
    event NFTClaimed(address indexed _claimer, address _nft, uint256 _tokenId);
    event ERC1155Claimed(address indexed _claimer, address _erc1155, uint256 _tokenId, uint256 _amount);

    // === OVERWRITES ===
    function recover(uint256[] memory _categoryArr, address _recipient) external override onlyOwner {
        revert("NFTFairVaultError: Cannot recover NFTs from a MinterVault");
    }
    ...
    function onERC721Received(
        ...
        revert("NFTFairVaultError: This vault does not accept inbound NFTs");
    }
    ...
    function getCategoryNftIds(uint256 _category) public view override returns (uint256[] memory) {
        ...
        uint256[] memory claimable_ids = category.claimable_ids.values();
        uint256[] memory claimed_ids = category.ids.values();

        uint256[] memory ret = new uint256[](claimable_ids.length + claimed_ids.length);

        uint256 length = 0;
        for (uint256 i = 0; i < claimable_ids.length; i++) {
            ret[i] = claimable_ids[i];
        }

        length = claimable_ids.length;
        for (uint256 j = 0; j < claimed_ids.length; j++) {
            uint256 id = claimed_ids[j];
            if (category.claimable_ids.contains(id)) {
                break;
            }
            ret[length] = id;
            length += 1;
        ...
        uint256[] memory ret2 = new uint256[](length);
    ...
    function setCategoryIds(
        ...
        if (category.tokenType == TokenType.ERC721) {
            ...
            for (uint256 i = 0; i < _ids.length; i++) {
                ...
                uint256 issued = category.claimable_id_values[_id] + category.id_values[_id];
    ...
    function claim(
        ...
        uint256 _deadline,
        bytes calldata _sig
    ) external onlyConveyor {
        CategoryInfo storage category = _categories[_category];
        bytes memory data = _extractData();
        uint256 seed = abi.decode(data, (uint256));
        uint256 id = _getIdFromSeed(seed, category);
        ...
        if (category.tokenType == TokenType.ERC721) {
            ...
            uint256 availableToClaimCount = claimableCount(_category);
            require(availableToClaimCount > 0, "NFTFairVaultError: There are no NFTs available for this category");
            _userClaimedInfo[_msgSender()][_category].push(id);
            category.ids.add(id);
            category.claimable_ids.remove(id);
            IERC721Mintable(category.nft).safeMint(_msgSender(), id, "");
            emit NFTClaimed(_msgSender(), category.nft, id);
        } else if (category.tokenType == TokenType.ERC1155) {
            ...
            category.ids.add(id);
            category.id_values[id] = category.id_values[id] + amount;
            require(
                category.claimable_id_values[id] >= amount,
                "NFTFairVaultError: Insufficient token balance to mint"
            );
            category.claimable_id_values[id] -= amount;
            if (category.claimable_id_values[id] == 0) {
                category.claimable_ids.remove(id);
            }
            if (_userClaimedBalance[_msgSender()][id] == 0) {
                _userClaimedInfo[_msgSender()][_category].push(id);
            }
            _userClaimedBalance[_msgSender()][id] = _userClaimedBalance[_msgSender()][id] + 1;
            IERC1155Mintable(category.nft).safeMint(_msgSender(), id, amount, "");
            emit ERC1155Claimed(_msgSender(), category.nft, id, amount);
        }
    }
}