...
// File: contracts/DeadHandzLab.sol
...
/// @title Dead Handz Lab
/// @author Aric Kuter
/// @dev All function calls are currently implemented without events to save on gas
contract DeadHandzLab is ERC721Enumerable, Ownable {
    using Strings for uint256;
    using Counters for Counters.Counter;

    Counters.Counter private _tokenIdCounter;
    Counters.Counter private _dateCounter;

    ERC721Enumerable private tartarusContract;
    ...
    /// @notice Map date index to tokenId to minted bool
    mapping(uint256 => mapping(uint256 => bool)) public idMinted;
    /// @notice map index to date
    ...
    constructor(
        ...
        mintDates[0] = 1651395600;
        mintDates[1] = mintDates[0] + 2592000;
        mintDates[2] = mintDates[1] + 2592000;
        mintDates[3] = mintDates[2] + 2592000;
        mintDates[4] = mintDates[3] + 2592000;
    ...
    function redeem(
        ...
        require(block.timestamp > mintDates[0], "Lab is closed");
        uint256 _tokenId = _tokenIdCounter.current();
        require(_tokenId + 1 < MAX_SUPPLY, "Supply limit reached");
        require(isValidAccessMessage(_spice, _signature), "Invalid signature");

        while (block.timestamp > mintDates[_dateCounter.current() + 1]) {
            _dateCounter.increment();
        }

        for (uint256 i = 0; i < mintIds.length; i++) {
            require(idMinted[_dateCounter.current()][mintIds[i]] == false);
            idMinted[_dateCounter.current()][mintIds[i]] = true;
            _safeMint(msg.sender, _tokenId + i);
            _tokenIdCounter.increment();
        }
    }

    function isRedeemable(uint256[] memory mintIds)
        public
        view
        returns (bool[] memory)
    {
        bool[] memory idClaimable = new bool[](mintIds.length);
        for (uint256 i = 0; i < mintIds.length; i++) {
            if (idMinted[_dateCounter.current()][mintIds[i]] == false) {
                idClaimable[i] = true;
            } else {
                idClaimable[i] = false;
            }
     ...
     */
    function isValidAccessMessage(bytes memory _spice, bytes memory _signature)
        public
        view
        returns (bool)
    {
        (bytes32 r, bytes32 s, uint8 v) = splitSignature(_signature);
        bytes32 messageHash = getMessageHash(address(this), _spice);
        bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);

        return owner() == ecrecover(ethSignedMessageHash, v, r, s);
    }

    function getMessageHash(address _contractAddr, bytes memory _spice)
        public
        pure
        returns (bytes32)
    {
        return keccak256(abi.encodePacked(_contractAddr, _spice));
    }
