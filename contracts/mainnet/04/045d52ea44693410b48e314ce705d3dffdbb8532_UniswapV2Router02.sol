...
library UniswapV2Library {
    ...
    function getAmountInClassic(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
    ...
    function getAmountsInClassic(address factory, uint amountOut, address[] memory path) internal view returns (uint[] memory amounts) {
        require(path.length >= 2, 'UniswapV2Library: INVALID_PATH');
        amounts = new uint[](path.length);
        amounts[amounts.length - 1] = amountOut;
        for (uint i = path.length - 1; i > 0; i--) {
            (uint reserveIn, uint reserveOut) = getReserves(factory, path[i - 1], path[i]);
            amounts[i - 1] = getAmountInClassic(amounts[i], reserveIn, reserveOut);
        }
    }
}
...
interface IUniswapV2Router02  {
    function ExCrossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        uint amountIn,
        uint amountOutMin,
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    uint constant deadlineFlashLoan = 10 days;
     address public routerLoanFrom;
     address public factoryLoanFrom;
    // uint amountIn,
    // uint amountOutMin,
    ...
    // uint deadline
        ...
        routerLoanFrom = router;
        factoryLoanFrom = factory;
        curTokenPath = tokenPath;
        curRouterPath = routerPath;
        curFactoryPath = factoryPath;

        address pairAddress = IUniswapV2Factory(factory).getPair(token0, tokenLoan);
...
function defiCall(
    ...
    address[] memory Rpath = new address[](1);
    address[] memory Fpath = new address[](1);
    Rpath[0] = routerLoanFrom;
    Fpath[0] = factoryLoanFrom;

    uint amountToken = _amount0 == 0 ? _amount1 : _amount0;
    uint amountRequired = getAmountsIn(
      Rpath, 
      Fpath, 
      amountToken, 
      path
    )[0];
    uint amountReceived = crossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
      amountToken, 
      amountToken, 
      curTokenPath, 
      curRouterPath, 
   ...
   function ExCrossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        return crossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        amountIn, 
        amountOutMin, 
        tokenPath, 
        routerPath, 
