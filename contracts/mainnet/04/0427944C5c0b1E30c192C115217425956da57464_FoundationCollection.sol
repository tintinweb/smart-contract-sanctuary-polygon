...
contract FoundationCollection is ICollection, AccessControl, Ownable {
  ...
  *
  */
  uint256 public buyBackValue = 10000000000000000000000000; // 10.000.000 EFT 
...
/**
  ...
  *
  */
uint public marketplaceSellerFeePercent = 5;
/**
  * @notice Stores the Marketplace Fee.
  *
  */
uint public marketplaceBuyerFeePercent = 5;
...
/**
  ...
  *
  */
uint32 public freeMintCount = 0;

/**
  * @notice Stores Total Mint Count.
  *
  */
uint16 public totalMintedCountForJoker = 169;

/**
  * @notice Stores Gained Jokers count.
  *
  */
uint8 public jokersCount = 15;
...
/**
   ...
   */
 function depositForFreeMint(uint32 _mintCount, uint256 _value) public  {
  ...
  function upgradeCards(uint160[] calldata _tokenIds, uint8 _targetCardType, uint8 _collectionId,  Coupon memory _coupon, uint _discount, uint256 _value) public  validCardType(_targetCardType) {
    ...
    
    {
      ...
      if(upgradecost.payValue > 0){
        bool success = eft.transferFrom(msg.sender, daoAddress, upgradecost.payValue);
...
/**
	...
	*/
  function setMarketplaceSellerFeePercent(uint _marketplaceSellerFeePercent) external onlyOwner {
     require(_marketplaceSellerFeePercent >=0 && _marketplaceSellerFeePercent <=100, "incorrect percent");
  ...
  /**
	...
	*/
  function setMarketplaceBuyerFeePercent(uint _marketplaceBuyerFeePercent) external onlyOwner {
        require(_marketplaceBuyerFeePercent >=0 && _marketplaceBuyerFeePercent <=100, "incorrect percent");
...
contract TreasuryReserve is AccessControl, Ownable, ReentrancyGuard{
  ...
  function liquidityDeposit(uint256 _value) public {
   ...
   function withdraw(address _to, uint _amount) external  onlyOwner nonReentrant{
    require(_to != address(0), "zero address not accepted");
    require( _amount <= eft.balanceOf(address(this)), "not enough funds");
    bool success = eft.transfer(_to, _amount);
