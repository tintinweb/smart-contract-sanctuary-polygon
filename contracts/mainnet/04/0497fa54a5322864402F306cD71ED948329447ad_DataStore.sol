...
contract DataStore is Initializable, NonblockingLzApp {
    ...
    function writeOnchain(address _childVault, uint256 _balance, uint256 _tvl, uint256 _chainId, uint256 _timestamp, uint8 _source) public onlyGovernorOrOffchainManager {
        childMessageOnChain[_childVault] = Message(
            childMessageOnChain[_childVault].id + 1,
            _childVault,
            _balance,
            _tvl,
            _chainId,
        ...
        childMessagesOnChain[_childVault].push(childMessageOnChain[_childVault]);
    }
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _nounce, bytes memory _payload) internal override {
        ...
        (address[] memory _vaults, uint256[] memory _balances, uint256[] memory _tvls, uint256 _actualChainId, uint256 _asOf) = abi.decode(
            _payload,
            (address[], uint256[], uint256[], uint256, uint256)
        );

        for (uint256 i = 0; i < _vaults.length; i++) {
            writeOnchain(_vaults[i], _balances[i], _tvls[i], _actualChainId, _asOf, 3);
        }
    }
}
