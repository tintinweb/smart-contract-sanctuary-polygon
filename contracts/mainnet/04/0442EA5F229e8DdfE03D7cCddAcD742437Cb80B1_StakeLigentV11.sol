...
modifier ownerOnly() {
   ...
   function initialize(address payable _ownerWallet,address _creatorAddress) public {
    ...
    function stake(address _referrer,uint _amount) public payable {
...
function stakeToken(address _referrer,uint _amount,uint _token) public{
...
user.withdrawnPASSIVERoi = current_Total_Roi();
uint totalwithdrawToken = calculateTokenRecived(value);
...
function currentRoi() public view returns(uint current_roi){
  ...
  function getDepositInfo(address _user,uint index) public view returns(
    ...
    function changeTokenPrice(uint usdValue) public ownerOnly {
        tokenPrice = usdValue;
    ...
    function calculateTokenRecived(uint usd_value) public view returns(uint _tokens) {
        uint usd = usd_value;
        _tokens = (usd.div(tokenPrice*1e8)).div(1e8);
    }
    function calculateUSdRecived(uint _token) public view returns(uint usd) {
        usd = (_token.mul(tokenPrice)).div(1e8);
    }

    function changePlanValue(uint _usdAmount) public adminOnly{
        INVEST_MIN_AMOUNT = _usdAmount;
    }
    
     function changeMinWithdraw(uint _usdAmount) public adminOnly{
        minWithdraw = _usdAmount;
    }

    function burn(uint _value) public adminOnly {
        token.burn(_value);
    }
}
