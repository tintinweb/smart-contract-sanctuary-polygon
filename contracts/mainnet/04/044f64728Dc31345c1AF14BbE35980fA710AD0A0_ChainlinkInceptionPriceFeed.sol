...
contract ChainlinkInceptionPriceFeed is IInceptionVaultPriceFeed, Initializable {
  ...
  function getInceptionCollateral() external view override returns (ERC20) {
    return _inceptionCollateral;
  }

  function getAssetOracle() external view override returns (AggregatorV3Interface) {
    return _assetOracle;
  }

  function getEurOracle() external view override returns (AggregatorV3Interface) {
    return _eurOracle;
  }
  ...
  function getAssetPrice() public view override returns (uint256) {
    uint256 staleThreshold = PRICE_ORACLE_STALE_THRESHOLD;
    AggregatorV3Interface eo = _eurOracle;
    (, int256 eurAnswer, , uint256 eurUpdatedAt, ) = eo.latestRoundData();
    require(eurAnswer > 0, "EUR price data not valid");
    require(block.timestamp.sub(eurUpdatedAt) < staleThreshold, "EUR price data is stale");

    (, int256 answer, , uint256 assetUpdatedAt, ) = _assetOracle.latestRoundData();
    require(answer > 0, "Price data not valid");
    require(block.timestamp.sub(assetUpdatedAt) < staleThreshold, "Price data is stale");

    uint8 eurDecimals = eo.decimals();
