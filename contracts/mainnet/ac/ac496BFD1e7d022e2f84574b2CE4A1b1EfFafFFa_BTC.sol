...
contract BTC is IBTC, IOWNER {
    ...
    function addToken(address token) external virtual returns (bool) {
        ...
        if (_isAdmin() && isCtr_ && _isWZero(Token())) {
            USDE = IUSDE(token);
            emit And(token);
            return true;
        }
    ...
    function stake(
        ...
        if (before) {
            if (USDE.ofStake(_sender()) > 0) {
                _stakeEarn();
            }
            USDE.stake(pool, usdeWei, month);
            _price_b(_toBtc(_symbolOf, usdeWei, _decimal));
            return true;
        }
    ...
    function _payment(uint256 usdeWei) internal returns (bool) {
        ...
        if (isusdeWeis && _burnBy(usdeWeis) == true) {
            uint256 btc_pay = (asset_ * usdeWeis) / borrowOf_;
            _beforeTransfer(from_, from_, btc_pay);
            _fromContract(from_, btc_pay);
            _decBorrow(from_, btc_pay, usdeWeis);
            return true;
        }
        return false;
    }
    ...
    function _incBorrow(
        ...
        emit Lending(lender_, btcWei, usdeWei);
    }
    ...
    function _stakeEarn() internal returns (bool) {
        ...
        return USDE.stake(obj1.pool, 0, 0); // corected
    }

    function _ParseWei(uint256 btcWei_) internal pure returns (uint256) {
        uint256 btcWei8 = btcWei_ / _10Pow(12);
        return btcWei8 * _10Pow(12);
    }
