...
import "../../pool-components/slim-pool/PoolRankedLogicUnblockable.sol";
import "../../interfaces/IAuctionPoolSlimPure.sol";

contract AuctionPoolRankedUnblockable is
  IAuctionPoolSlim,
  PoolRankedLogicUnblockable
{
  /**
  ...
  {
    // This pool doesn't enforce minBids against faceValue because it is enforcing required fees protection on "setRanks"
  ...
  function tryExtendRound(
    ...
    if (bidsForExtension != 0) {
      ...
      if (roundBaseEndTime - periodOfExtension < block.timestamp) {
        require(
          roundBaseEndTime + roundExtension[_roundId] >= block.timestamp,
...
import "../storage/PoolRankedUnblockableStorage.sol";
import "../structs/SlimBidStruct.sol";

abstract contract PoolRankedLogicUnblockable is
  PoolBaseLogicSlim,
  PoolRankedEvent,
  PoolRankedUnblockableStorage
{
  using SafeERC20Upgradeable for IERC20Upgradeable;
      ...
      if (_roundId > roundCount) {
        ...
        if (_initTime > 0) {
          ...
          if (settlementTime[_roundId] > 0) {
            ...
            if (
              _initTime + roundDuration + roundExtension[_roundId] >
              block.timestamp
            ) {
  ...
  function finalize(
    ...
    uint256 _reindexerIndex = bidListSlotsDataReindexer[_bidListId];
    unchecked {
      uint256 _safeLimit = validBids[_reindexerIndex].length >=
      ...
      for (i = finalizedBidsCount[_bidListId]; i < _safeLimit; i++) {
        ...
        if (SlotsData[_reindexerIndex][_slotIndex] < 2) {
          ...
          if (!bidHasRank[_reindexerIndex][_bidId]) {
            require(
              _bidAmount < bidAmounts[_bidListId][lastFedRankedBid[_bidListId]],
              "Fed ranks wrong. Reset."
            );
          }
        } else {
          require(
            !bidHasRank[_reindexerIndex][_bidId] ||
              lastLivingBid[_bidListId] == _bidId,
            "Burnt bids can't have a rank"
          );
        }
      }
      ...
      if (_safeLimit == validBids[_reindexerIndex].length) {
        ...
        if (_noneRankRewardFunds > 0) // for minbids 0 profit pool
        {
          IERC20Upgradeable(factory.feeToken()).safeTransfer(
            ...
            (_noneRankRewardFunds) / decimalsDelta
          );
        }
  ...
  function feedRanks(
    uint256 _roundId,
    uint256[] memory _orderedRankedBids
  ) external onlyOperator {
    ...
    if (lastFedRankedBid[_bidListId] == 0) {
      if (validBids[_reindexerIndex].length != 0) {
        if (livingBids[_bidListId] == 0) {
          ...
          require(
            _orderedRankedBids[0] == lastLivingBid[_bidListId],
            "Last living winner scenario unmet"
          );
        }
      } else {
    ...
    unchecked {
      for (uint256 i = 0; i < _orderedRankedBids.length; ++i) {
        BidInfoSlim memory _theBid = bidsList[_bidListId][
          _orderedRankedBids[i]
        ];
        require(
          bidAmounts[_bidListId][lastFedRankedBid[_bidListId]] >
            bidAmounts[_bidListId][_orderedRankedBids[i]] ||
        ...
        bidAtRank[_reindexerIndex][_currentFedRanksCount] = _orderedRankedBids[
          i
        ];
        ...
        if (
          rankRatio[_currentFedRanksCount] > 0 &&
          rankRequiredBids[_currentFedRanksCount] <=
          valuedBidsLength[_bidListId]
        ) {
          _bidReward =
            (rankRatio[_currentFedRanksCount] * faceValue) /
            FULLRATIOSHARE;
          auctionRankRatio[_reindexerIndex][_currentFedRanksCount] = rankRatio[
            _currentFedRanksCount
          ];
    ...
    require(
      livingBids[_bidListId] >= _currentFedRanksCount,
      "More ranks than living bids"
    );
    require(_currentFedRanksCount <= rankCount, "More ranks than possible");
    //Must be conducted before finalization

    if (
      _currentFedRanksCount == livingBids[_bidListId] ||
      _currentFedRanksCount == rankCount
    ) {
  ...
  function setRanks(
    ...
    uint256[] memory _ranksRatio
  ) external onlyOperator {
    require(_ranks.length == _ranksRatio.length);
    ...
    require(_ranks[0] == rankCount + 1, "Order mismatch");
    uint256 _tempCumulativeRanksRatioAddition;
    uint256 _helpCalc;
    unchecked {
      for (uint256 i = 0; i < _ranksRatio.length; ++i) {
        if (i != _ranksRatio.length - 1) {
          require(_ranks[i] + 1 == _ranks[i + 1], "Ranks must be ordered");
        }
        rankRatio[_ranks[i]] = _ranksRatio[i];

        if (_ranksRatio[i] > 0) {
          emit RankSet(_ranks[i], _ranksRatio[i]);
          if (
            _tempCumulativeRanksRatioAddition == 0 && cumulativeRanksRatio == 0
          ) //first rank's required amount is min bids
          {
            require(
              (minBids * bidFee >=
                (_ranksRatio[i] * faceValue) / FULLRATIOSHARE),
              "Not enough fees"
            );
            rankRequiredBids[_ranks[i]] = minBids;
            requiredBidsSum = minBids; //This will also reset 'requiredFeeSum' on rank reset
          } else {
            _helpCalc =
              (_ranksRatio[i] * faceValue * 15) /
              (FULLRATIOSHARE * 10); //require additional 150% bidFees to unlock
            requiredBidsSum +=
              _helpCalc /
              bidFee +
              (_helpCalc % bidFee != 0 ? 1 : 0);
            rankRequiredBids[_ranks[i]] = requiredBidsSum;
          }
        } //No need to set "rankRequiredFee" back to 0 on reset ranks because it's not logic interfering if those values remain

        _tempCumulativeRanksRatioAddition += _ranksRatio[i];
      }
      cumulativeRanksRatio += _tempCumulativeRanksRatioAddition;
    }
  ...
  //Flat extension logic (extensionStep is ignored due override)
  function tryExtendRound(
    uint256 _roundId,
    uint256 _bidCount
  ) internal override {
  ...
  //Dictate if a a certain rank was unlocked
  function isRankUnlocked(uint256 _roundId) internal view returns (bool) {}
}
...
contract PoolRankedStorage {
    ...
    //rank -> ratio (0...FULLRATIOSHARE) where FULLRATIOSHARE is 100%
...
import "./PoolRankedStorage.sol";

contract PoolRankedUnblockableStorage is PoolRankedStorage {
  /* UNBLOCKABLE RANKS LOGIC VAR */
  /// @notice Dictates the required funds from bids feed to unlock a rank
  mapping(uint256 => uint256) public rankRequiredBids;

  /// @notice Helps set the value of "rankRequiredFee" during rank setting
  uint256 public requiredBidsSum;
}
