...
//    }
    ...
    function freeMultiMint(uint amount_, uint heroId_, uint rarity_) whenMergerActive onlyEOA external {
        if (amount_ == 0 || amount_ > 10) {
            revert WrongAmount();
        }
        if (mintIndexPublicSale + amount_ > MAX_MINT) {
...
//        worldCupAddress = _worldCupAddress;
    ...
    function addMerge(uint tokenIdA, uint tokenIdB, uint price) payable onlyEOA whenMergerActive external {
        ...
        if (tokenIdA == tokenIdB || rarityMap[tokenIdA] != rarityMap[tokenIdB] || rarityMap[tokenIdA] >= maxRarity || rarityMap[tokenIdA] == 1) {
