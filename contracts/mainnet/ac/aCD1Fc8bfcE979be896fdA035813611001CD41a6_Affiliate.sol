...
contract Affiliate is Ownable {
    ...
    function splitFeeWithAffiliate(
        ...
        if (affiliateOwner != address(0)) {
            ...
            uint256 affiliatePortion = _feeAmount.sub(protocolFeePortion);
            uint256 protocolFeePortionMaxCalc = returnFeeAfterAffiliateAllocation(
                    _maxFeeCalculation
                );
            uint256 affiliatePortionMaxCalc = _maxFeeCalculation.sub(
            ...
            if (affiliatePortion > affiliatePortionMaxCalc) {
                _payToken.safeTransferFrom(
                    _feeSender,
                    _feeReceipient,
                    _feeAmount.sub(affiliatePortionMaxCalc)
                );
                _payToken.safeTransferFrom(
                    _feeSender,
                    affiliateOwner,
        ...
        if (affiliateOwner != address(0)) {
            ...
            if (affiliatePortion > affiliatePortionMaxCalc) {
                (bool platformTransferSuccess, ) = payable(_feeReceipient).call{
                    value: _feeAmount.sub(affiliatePortionMaxCalc)
                }("");
    ...
    function returnFeeAfterAffiliateAllocation(uint256 _amount)
        ...
        returns (uint256)
    {
        uint256 affiliateAllocation = _amount.mul(affiliateFee).div(10000);
        uint256 amountAfterFee = _amount.sub(affiliateAllocation);
