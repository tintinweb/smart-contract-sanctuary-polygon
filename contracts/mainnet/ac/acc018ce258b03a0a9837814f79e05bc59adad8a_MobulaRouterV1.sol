...
contract MobulaRouterV1 {
    ...
    function mobulaSwapSupportingFee(
        address factory,
        address[] memory path,
        address to
    ) internal {
        for (uint256 i = 0; i < path.length - 1; i++) {
            ...
            {
                (uint256 reserve0, uint256 reserve1, ) = pair.getReserves();
                (uint256 reserveInput, uint256 reserveOutput) = pair.token0() ==
                    path[i]
                    ? (reserve0, reserve1)
                    : (reserve1, reserve0);
                amountInput =
    ...
    function multiSwapExactTokensForETHSupportingFee(
        address factory,
        uint256 amountIn,
        uint256 amountOutMin,
        address[] calldata path,
        ...
        (bool sent, ) = payable(to).call{
            value: address(this).balance - contractBalanceBefore
        }("");
    ...
    function multiSwapExactETHForTokens(
        ...
        require(
            ...
            "MobulaRouter: Deposited ether amount too low."
        );
