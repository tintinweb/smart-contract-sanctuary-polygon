...
contract ERC721WithPermit is ERC721, EIP712Base, ERC2981, AccessControl, Pausable, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _tokenIdCounter;

  /// @dev The contract-level metadata (for Opensea standards)
  string public contractURI;

  /// @dev for platform fee
  struct PermitDetail {
    uint256 deadline;
    uint8 v;
    bytes32 r;
  ...
  // Mapping token address to boolean
  mapping(address => bool) private _allowedTokensForTransfer;

  // Array of allowed tokens for transfer
   ...
   */
  event PurchaseBlocks(address indexed seller, address indexed buyer, uint256[] tokenIds, string[] tokenUris);
  ...
  constructor(
      ...
      _setDefaultRoyalty(msg.sender, _royaltyBps_);
    }
  ...
  function mintSingleBlockWithPermit(
    address to,
    ...
    PermitDetail calldata permitDetail,
    PlatformFeeDetail calldata platformFeeDetail
  ) external whenNotPaused {
    verifyMintSig(
      to,
      seller,
      permitDetail.deadline,
      mintSignature
    );

    if (value != 0) {
      _verifyTokenForTransfer(tokenAddress);
      ...
      IERC20WithPermit(tokenAddress).transferFrom(to, seller, value);
    }

    if (platformFeeDetail.amount != 0) {
      // For platform fee
      verifyTransferPermit(
        to,
        platformFeeDetail.amount,
        platformFeeDetail.deadline,
        platformFeeDetail.v,
        platformFeeDetail.r,
        platformFeeDetail.s,
        tokenAddress
      ...
      IERC20WithPermit(tokenAddress).transferFrom(to, platformFeeDetail.recipient, platformFeeDetail.amount);
    }
  ...
  function verifyMintSig(
    ...
    require(block.timestamp <= deadline, "Artifract|Ely: INVALID_DEADLINE");
    ...
    require(hasRole(MINTER_ROLE, signer), "Artifract|Ely: INVALID_SIGNATURE");
  }
  ...
  function verifyTransferPermit(
    ...
    IERC20WithPermit(tokenAddress).permit(
      owner,
      address(this),
      value,
      deadline,
  ...
  /**
    * @dev Add `tokenaddress` to allowed tokens for transfer
    *
    */
  function addAllowedTokenForTransfer(address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
      _allowedTokensForTransfer[tokenAddress] = true;
      _allowedTokensList.push(tokenAddress);
  }

  /**
    * @dev Remove `tokenaddress` to allowed tokens for transfer 
    *
    */
  function removeAllowedTokenForTransfer(uint index, address tokenAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
      require(_allowedTokensList[index] == tokenAddress, 'Artifract|Ely: TOKEN_INDEX_ADDRESS_MISMATCH');
      _allowedTokensForTransfer[tokenAddress] = false;
      _allowedTokensList[index] = _allowedTokensList[_allowedTokensList.length - 1];
      _allowedTokensList.pop();
  }

  /**
    * @dev Verify if a `token address` is allowed
    *
    */
  function _verifyTokenForTransfer(address tokenAddress) internal view {
      require(_allowedTokensForTransfer[tokenAddress], "Artifract|Ely: TOKEN_NOT_ALLOWED");
  }

  /**
    * @dev Returns true if a `token address` is allowed for token transfer, else, false
    *
    */
  function isAllowedTokenForTransfer(address tokenAddress) external view returns (bool) {
    return _allowedTokensForTransfer[tokenAddress];
  }

  /**
    * @dev Returns all allowed tokens for transfer
    *
    */
  function getAllowedTokensForTransfer() public view returns (address[] memory) {
    return _allowedTokensList;
  }

  /**
    * @dev Updates the tokenUri of the token with given`tokenId`
    *
    */
