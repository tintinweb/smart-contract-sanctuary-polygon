...
contract GammaLPFarmV1 is BaseSiloAction {
    ...
    constructor(string memory _name,address _factory) {
        name = _name;
        metaData = "address[5],address[5],uint256[2],uint256,address[2],address[][2],address";
        usesTakeFee = true;
    ...
    function prepareShareBoth(
        ...
        address uniProxy = IHypervisor(pos).whitelistedAddress();
        (uint256 test1Min, uint256 test1Max) = IUniProxy(uniProxy)
            .getDepositAmount(
                pos,
        ...
        if (deposit1 >= test1Min && test1Max >= deposit1) {
            ...
            if (deposit1 < test1Min) {
                uint256 mAmount0;
                {
                    (uint256 test0Min, uint256 test0Max) = IUniProxy(uniProxy)
                        .getDepositAmount(
                            pos,
                            address(IHypervisor(pos).token1()),
                            deposit1
                        );
                    mAmount0 = (test0Min + test0Max) / 2;
                }
    ...
    function prepareShareToken1(
        ...
        uint256 price = getUniPrice(QUICK_V2_ROUTER, path, deposit1);
        ...
        if (price > 0) {
            ...
            if (result) {
                amounts[0] = swapOuts[swapOuts.length - 1];
                amounts[1] = deposit1 - swapAmount;
            }
        } else {
            price = getV3Price(QUOTER_ADDRESS, token1, token0, deposit1);
    ...
    function prepareShareToken0(
        ...
        uint256 price = getUniPrice(QUICK_V2_ROUTER, path, deposit0);
        ...
        if (price > 0) {
            ...
            if (result) {
                amounts[1] = swapOuts[swapOuts.length - 1];
                amounts[0] = deposit0 - swapAmount;
            }
        } else {
            price = getV3Price(QUOTER_ADDRESS, token0, token1, deposit0);
    ...
    function getGamamaLp(
        ...
        share = IUniProxy(uniProxy).deposit(
            amounts[0],
            amounts[1],
            address(this),
            pos,
    ...
    function getRewarders(
        uint pid
    ) internal view returns (address[2] memory rewarders) {
        IMasterChef masterChef = IMasterChef(GammaMasterChef1);
        for (uint i = 0; i < 2; i++) {
            try masterChef.getRewarder(pid, i) returns (address rewarder) {
                rewarders[i] = rewarder;
            } catch {}
        }
    }
    ...
    function actionValid(
        ...
        // return true; //second bool overwritten to logically account for the end block
         return (ISiloFactory(factory).actionValid(address(this)), true); 
    }
...
interface IDragonLair {
    ...
    function dQUICKForQUICK(
        uint256 _dQuickAmount
    ) external view returns (uint256 quickAmount_);
...
import {ActionBalance} from "../../../interfaces/IAction.sol";
...
abstract contract BaseSiloAction {
    ...
    // function actionValid(bytes memory) external view virtual returns (bool) {
    //     return true; //second bool can be overwritten by individual actions
    // }

    function actionValid(
        bytes memory
    ) external view virtual returns (bool, bool) {
        return (ISiloFactory(factory).actionValid(address(this)), true); //second bool can be overwritten by individual actions
    }
    ...
    function showFee(
        ...
        nameOfFee = feeName;
        if (usesTakeFee) {
            fees = ISiloFactory(factory).getFeeInfoNoTier(
                _action
            );
        }
    }
    ...
    function _takeFee(
        ...
        try
            ISiloFactory(ISilo(address(this)).factory()).getFeeInfo(_action)
        returns (uint256 _fee, address _recipient) {
            fee = _fee; // Last one is the outToken
        ...
        if (fee > 0) {
            uint256 feeToTake = (_gains * fee) / FEE_DECIMALS;

            remaining = _gains - feeToTake;
                  
           (uint256 share, address referrer) =  ISiloReferral(ISiloFactory(ISilo(address(this)).factory()).referral()).siloReferralInfo(
                address(this)
            );

            if (share > 0 && referrer != address(0)) {
                uint256 shareAmount = (feeToTake * share) / FEE_DECIMALS;
