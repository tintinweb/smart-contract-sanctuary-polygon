...
contract EmbrCheckout is Ownable {
    ...
    event StableCoinAddressSet(address stableCoinAddress, bool flag);

    /**
     * @param _dexRouterAddresses router addresses of DEX
     * @param _stableCoinAddresses stable coin addresses
     */
    constructor(
        ...
        uint256 i;
        for (i = 0; i < _dexRouterAddresses.length; i++) {
            require(
                _dexRouterAddresses[i] != address(0),
                "EmbrCheckout: DEX Router address can't be 0"
            );
            dexRouters[_dexRouterAddresses[i]] = true;
        }
        for (i = 0; i < _stableCoinAddresses.length; i++) {
            require(
                _stableCoinAddresses[i] != address(0),
                "EmbrCheckout: DEX Router address can't be 0"
            );
            stableCoins[_stableCoinAddresses[i]] = true;
        }
    }
    ...
    function swapExactETHForTokens(
        ...
        // Check zero address
        require(to != address(0), "EmbrCheckout: To address can't be 0");
        require(
        ...
        require(msg.value > 0, "EmbrCheckout: Value can't be 0!");
        ...
        if (customAvaxDexRouters[dexRouterAddress]) {
            if (isFeeToken) {
                ...
                return
                    IJoeRouter02(dexRouterAddress).swapExactAVAXForTokens{
                        value: amountToSwap
                    }(amountOutMinNew, path, to, deadline);
            }
        } else {
            if (isFeeToken) {
                ...
                return
                    IUniswapV2Router02(dexRouterAddress).swapExactETHForTokens{
                        value: amountToSwap
                    }(amountOutMinNew, path, to, deadline);
            }
        }
    ...
    function swapExactTokensForETH(
        ...
        // Check path length is at least 2
        require(path.length >= 2, "EmbrCheckout: Path is wrong!");
        // Check if the exchange supported on our Swap
        require(
            dexRouters[dexRouterAddress],
            "EmbrCheckout: DEX is not supported by us!"
        ...
        if (
            IERC20(tokenAddress).allowance(address(this), dexRouterAddress) <
            amountIn
        ) {
        ...
        if (customAvaxDexRouters[dexRouterAddress]) {
            if (isFeeToken) {
                ...
                IJoeRouter02(dexRouterAddress).swapExactTokensForAVAX(
                    amountIn,
                    amountOutMin,
                    path,
                    address(this),
            ...
            if (isFeeToken) {
                ...
                IUniswapV2Router02(dexRouterAddress).swapExactTokensForETH(
                    amountIn,
                    amountOutMin,
                    path,
                    address(this),
        ...
        uint256 feeAmount = ((nowBalance - prevBalance) * fee) / 1e4;
        bool success = false;
        (success, ) = payable(to).call{
            value: (nowBalance - prevBalance - feeAmount)
        }("");
    ...
    function swapExactTokensForTokens(
        ...
        require(
            stableCoins[sellTokenAddress] || stableCoins[buyTokenAddress],
            "EmbrCheckout: One of the token should be stable coin"
        );
        // Transfer tokens from msg.sender to this
        ...
        // Check allowance
        if (
            IERC20(sellTokenAddress).allowance(
                address(this),
                dexRouterAddress
            ) < amountIn
        ) {
            IERC20(sellTokenAddress).approve(
                dexRouterAddress,
                IERC20(sellTokenAddress).totalSupply()
            );
        }

        if (stableCoins[sellTokenAddress]) {
            takeFeeAndSwap(
                amountIn,
                amountOutMin,
                path,
    ...
    function swapAndTakeFee(
        ...
        uint256 prevTokenBalance = IERC20(path[path.length - 1]).balanceOf(
            address(this)
        );
        if (customAvaxDexRouters[dexRouterAddress]) {
            if (isFeeToken) {
                ...
                IJoeRouter02(dexRouterAddress).swapExactTokensForTokens(
                    amountIn,
                    amountOutMin,
                    path,
                    address(this),
        ...
        uint256 nowTokenBalance = IERC20(path[path.length - 1]).balanceOf(
            address(this)
        );
        IERC20(path[path.length - 1]).safeTransfer(
            to,
            ((nowTokenBalance - prevTokenBalance) * (1e4 - fee)) / 1e4
        );
    }

    function takeFeeAndSwap(
        uint256 amountIn,
        uint256 amountOutMin,
        address[] calldata path,
        address to,
        ...
        uint256 amountInNew = (amountIn * (1e4 - fee)) / 1e4;
        uint256 amountOutMinNew = (amountOutMin * (1e4 - fee)) / 1e4;
        if (customAvaxDexRouters[dexRouterAddress]) {
            if (isFeeToken) {
                IJoeRouter02(dexRouterAddress)
    ...
    function setStableCoinAddress(address _stableCoinAddress, bool _flag)
        external
        onlyOwner
    {
        // Check zero address
        require(
            ...
            "EmbrCheckout: Stable coin address can't be 0"
        );
        stableCoins[_stableCoinAddress] = _flag;
        emit StableCoinAddressSet(_stableCoinAddress, _flag);
    }
    ...
    function setFee(uint256 newFee) external onlyOwner {
        require(
            newFee <= 10000,
            "EmbrCheckout: Fee can't be bigger than 10000(100%)!"
        );
        fee = newFee;
        ...
        onlyOwner
    {
        require(
            recipient != address(0),
            "EmbrCheckout: recipient address can't be 0"
        );
        require(
    ...
    function withdrawToken(
        ...
        require(
            ...
            "EmbrCheckout: tokenAddress can't be 0"
        );
        require(
            IERC20(tokenAddress).balanceOf(address(this)) >= amount,
            "EmbrCheckout: Not enough tokens"
        );
