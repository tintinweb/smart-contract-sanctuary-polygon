...
contract GammaGiddyWethStrategy is GiddyStrategyV2, Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable {
  uint256 constant private BASE_PERCENT = 1e6;
  uint constant private COMPOUND_THRESHOLD_GIDDY = 10e18;
  uint256 constant private PID = 1;
  ...
  address constant private POS = 0x2D08b4B4C74d0B2f4144aE7bd86eE40FB654acef;
  ...
  function compound(SwapInfo[] calldata swaps) external override onlyVault returns (uint256 staked) {
    ...
    if (swaps[0].amount > 0) {
      amounts[0] += swaps[0].amount;
    }
    if (swaps[1].amount > 0) {
      amounts[1] += GiddyLibraryV2.routerSwap(vault.config().swapRouter(), swaps[1], address(this), address(this), WETH_TOKEN);
    }
    amounts[0] = deductEarningsFee(GIDDY_TOKEN, amounts[0]);
  ...
  function withdraw(uint256 staked) external override nonReentrant onlyVault returns (uint256[] memory amounts) {
    ...
    SafeERC20Upgradeable.safeTransfer(IERC20Upgradeable(GIDDY_TOKEN), address(vault), amounts[0]);
  ...
  function depositLP(uint256[] memory amounts) private returns (uint256 staked) {
    SafeERC20Upgradeable.safeApprove(IERC20Upgradeable(GIDDY_TOKEN), POS, amounts[0]);
