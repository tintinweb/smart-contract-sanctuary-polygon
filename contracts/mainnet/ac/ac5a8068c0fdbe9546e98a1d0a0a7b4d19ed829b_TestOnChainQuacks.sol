...
contract TestOnChainQuacks is ERC721A, ReentrancyGuard, Ownable,  DefaultOperatorFilterer , IERC4906 {
    using Strings for uint256;
    ...
    uint256 public maxPurchase = 130;
    uint256 public maxSupply = 26000;
    mapping(uint256 => uint256) public tokenHue;
    mapping(uint256 => uint256) public tokenSat;
    mapping(uint256 => uint256) public tokenLum;
    ...
    mapping(uint256 => uint256) public tokenIdBurnedBy;
    mapping(uint => string) public levelBaseTokenURIs;
    ...
    constructor() ERC721A("TEST On Chain Quacks", "QUACKS") {
    }
    ...
    external payable  {
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            require(ownerOf(tokenIds[i]) == msg.sender, "NOT_TOKEN_OWNER");
            require(tokenIdLevels[tokenIds[i]] == tokenIdLevels[_tokenID], "Tokens are not right level");
        ...
        for (uint256 i; i < tokenIds.length; i++) {
            if (tokenIds[i] != _tokenID) {
                ...
                tokenIdBurnedBy[tokenIds[i]] = _tokenID;
            
            }
            else {
                tokenIdLevels[tokenIds[i]] += 1;
            }
            
        }
        tokenHue[_tokenID] = ((_tokenID * tokenIdLevels[_tokenID] * 1111 ) % 360);
        tokenSat[_tokenID] = 30 + ((_tokenID * tokenIdLevels[_tokenID] * 1111 ) % 50);
        tokenLum[_tokenID] = 80 - ((_tokenID * tokenIdLevels[_tokenID] * 1111 ) % 50);
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"On Chain Duck Race", "image":"',
                                image,
                                '", "description": "An all on Chain NFT which can Burn and Combine to change colors and gain a level, on Chain Duck Races by Quacks QQ","attributes": [{"trait_type": "Metadata","value": "OnChain"},{"trait_type": "Utility","value": "Combine And Burn"},{"trait_type": "Token Level","value":',
                                Strings.toString(tokenIdLevels[id]),
                                '},',
