...
import "./UniversePairVaultStorage.sol";

contract UniversePairVaultDelegate is
    Ownable,
    UpgradeableProxy,
    UniversePairVaultBaseStorage
{
    constructor(
        address _vaultAddress,
        address payable _timelock,
        bytes memory _data
    ) UpgradeableProxy(_vaultAddress, _data) {
        timelock = TimelockController(_timelock);
    }

    function upgrade(address _vaultAddress) external {
        require(_vaultAddress != address(0), "ZERO");
        require(msg.sender == address(timelock), "forbidden");
        _upgradeTo(_vaultAddress);
    }
}
    ...
    IVaultEvents
{
    ...
    function getUserBals(address user)
        external
        view
    ...
    function getBals(uint256 share0, uint256 share1)
        external
        view
        returns (uint256, uint256);
    ...
    function withdraw(uint256 share0, uint256 share1)
        external
        returns (uint256, uint256);
 ...
 */
import "@openzeppelin/contracts/access/TimelockController.sol";
import "../interfaces/IUniversePairVaultConfig.sol";
import "../interfaces/PositionNFTForPair.sol";
import "../interfaces/IUniswapV3Staker.sol";
import "../interfaces/IUToken.sol";
import "../interfaces/IPositionNFTManager.sol";

contract UniversePairVaultBaseStorage {
    // @dev UNIVERSE VERSION   1 - Single Share Token   2 - Double Share Token
    uint8 public constant UNIVERSE_VAULT_VERSION = 2;
    TimelockController public timelock;
}

contract UniversePairVaultStorage is UniversePairVaultBaseStorage {
    address operator;
    IUToken public token0;
    IUToken public token1;
    IUToken shareToken;
    ...
    PositionNFTForPair.Position[] public positionList;

    IUniversePairVaultConfig config;

    IPositionNFTManager nftManager;
}
...
interface IVaultEvents {
    ...
    event CollectFees(uint256 feesFromPool0, uint256 feesFromPool1);
...
contract UToken is UNTERC20 {
    ...
    constructor(
        ...
        uint8 _decimals
    ) UNTERC20(_name, _symbol, _decimals) {
        _owner = ownerAddress;
    }
...
interface IUniversePairVaultConfig {
    struct PairVaultParam {
    ...
    function changePairVaultParam(
        uint8 _performanceFee,
        uint24 _diffTick,
        address _vaultAddress
    ) external;

    function pairVaultParam(address vaultAddress)
        external
        view
        returns (PairVaultParam memory param);

    function performanceFee() external view returns (uint8);

    function diffTick() external view returns (uint24);
    ...
    function findActiveIncentive(address vaultAddress, address poolAddress)
        external
        view
        returns (IUniswapV3Staker.IncentiveKey[] memory activeList);

    /// @notice Find the Incentive
    /// @param vaultAddress Address of Universe V3 Vault
    function getIncentiveList(address vaultAddress)
        external
        view
...
import "@openzeppelin/contracts/math/SafeMath.sol";
import "./IUniswapV3Staker.sol";
import "./IUniversePairVaultConfig.sol";
import "./IncentiveId.sol";

library PositionNFTForPair {
    using SafeMath for uint256;

    INonfungiblePositionManager public constant positionManager =
        INonfungiblePositionManager(0xC36442b4a4522E871399CD717aBDD847Ab11FE88);
    IUniswapV3Staker public constant v3Staker =
    ...
    function nftStakeAll(
        IUniversePairVaultConfig config,
        address nftManagerAddress,
        Position memory pos
    ) internal {
        //get active Incentive List
        IUniswapV3Staker.IncentiveKey[] memory activeList = config
            .findActiveIncentive(address(this), pos.poolAddress);
        if (activeList.length > 0) {
            if (pos.liquidity > 0) {
                positionManager.safeTransferFrom(
                    ...
                    address(v3Staker),
                    pos.tokenId,
                    abi.encode(activeList)
                );
            } else {
                positionManager.approve(nftManagerAddress, pos.tokenId);
            }
        } else {
            positionManager.approve(nftManagerAddress, pos.tokenId);
        }
    ...
    function nftStake(
        ...
        (address owner, , , ) = v3Staker.deposits(pos.tokenId);
        // already stake nft
        if (owner == address(this)) {
            v3Staker.stakeToken(key, pos.tokenId);
        } else {
            nftStakeAll(config, nftManagerAddress, pos);
        }
    }

    // @dev stake to Incentive
    function nftUnStakeAll(IUniversePairVaultConfig config, uint256 tokenId)
        internal
    {
        (address owner, , , ) = v3Staker.deposits(tokenId);
        if (owner == address(this)) {
            IUniswapV3Staker.IncentiveKey[] memory activeList = config
                .getIncentiveList(address(this));
            for (uint256 i = 0; i < activeList.length; i++) {
                (, uint128 liquidity) = v3Staker.stakes(
                    tokenId,
                    IncentiveId.compute(activeList[i])
                );
                if (liquidity > 0) {
...
interface IPositionNFTManager {
    ...
    function getTotalAmounts(uint256 tokenId, uint8 _performanceFee)
        external
        view
        returns (uint256, uint256);
    ...
    function burn(uint256 tokenId, uint128 _liquidity)
        external
        returns (
            uint256 amount0,
            uint256 amount1,
    ...
    function burnAll(uint256 tokenId)
        external
        returns (
            uint256 amount0,
            uint256 amount1,
