...
import {IUniswapHelper, Ticks} from '../interfaces/IUniswapHelper.sol';
...
contract Strategy is BaseStrategy, IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    int24 public tickWidth = 1000;
    int24 public tickRebalanceDeviation = 200;
    ...
    uint256 public wethDust = 1e18;

    event Rebalance(int24 tick, int24 newTickLower, int24 newTickUpper, uint256 fees, uint256 il);

    constructor(address _vault, address _uniswapHelper) public BaseStrategy(_vault) {
    ...
    function name() external view override returns (string memory) {
        return 'Strategy Uniswap V3 USDC-wETH';
    }

    function estimatedTotalAssets() public view override returns (uint256) {
        return balanceOfWant().add(valueOfWeth()).add(balanceOptimistic());
    }

    function estimatedTotalAssetsSafe() public view returns (uint256) {
        return balanceOfWant().add(valueOfWethSafe()).add(balanceSafe());
    }
    ...
    function valueOfWeth() public view returns (uint256) {
        return wethToWant(balanceOfWeth(), false);
    }

    function valueOfWethSafe() public view returns (uint256) {
        return wethToWant(balanceOfWeth(), true);
    }

    function principalValue() public view returns (uint256) {
        (uint256 usdcBalance, uint256 wethBalance, uint128 _liquidity) = principal();
        if (_liquidity == 0) return 0;
        uint256 wethInUsdc = wethToWant(wethBalance, false);
        return wethInUsdc.add(usdcBalance);
    }
    ...
    function getTotalInvestedAssets() public view returns (uint256, uint256) {
        (uint256 usdcBalance, uint256 wethBalance, ) = principal();
        (uint256 usdcInLimitOrder, uint256 wethInLimitOrder, ) = limitOrderPosition();
        return (usdcBalance.add(usdcInLimitOrder), wethBalance.add(wethInLimitOrder));
    }
    ...
    {
        return uniHelper.getPosition(baseTicks);
    }
    ...
    {
        return uniHelper.getPosition(limitTicks);
    }
    ...
    function estimatedFees() public view returns (uint256) {
        (uint256 wantFee, uint256 wethFee) = uniHelper.estimatedFees(baseTicks, currentTick());
        // Compute current fees earned
        return wantFee.add(wethToWant(wethFee, false));
    }

    function estimatedHarvest() public view returns (int256) {
        return int256(estimatedFees() - estimatedImpermanentLoss());
    }
    ...
    function adjustPosition(uint256 _debtOutstanding) internal override {
        ...
        if (wantBal > wantDust) {
            uint256 wantToSwap = _getWantSwapAmount(wantBal);
            if (wantToSwap > wantDust) _swapToWeth(wantToSwap);

            _addLiquidity(balanceOfWant(), balanceOfWeth());

            _setLimitOrder();
        }
    }

    function rebalance(int24 _tickLower, int24 _tickUpper) public onlyKeepers {
        require(
        ...
        uint256 initialETA = estimatedTotalAssetsSafe();

        (uint256 fees0, uint256 fees1) = _unwindAllPositions();

        baseTicks.lowerTick = _tickLower;
        baseTicks.upperTick = _tickUpper;
        ...
        emit Rebalance(
            ...
            _tickLower,
            _tickUpper,
            fees0.add(wethToWant(fees1, false)),
            estimatedImpermanentLoss()
        );

        _addLiquidity(wantBalance, wethBalance);

        _setLimitOrder();

        _enforceSlippage(initialETA, estimatedTotalAssetsSafe(), slippageMax);
    }

    function autoRebalance() external onlyKeepers {
        int24 tickFloor = uniHelper.floor(currentTick());
        int24 tickCeil = tickFloor + tickSpacing;
        rebalance(tickFloor - tickWidth, tickCeil + tickWidth);
    }

    function _getWantSwapAmount(uint256 _wantBal) internal view returns (uint256) {
        (uint256 usdcBal, uint256 wethBalance, ) = principal();
        uint256 wethPercentage = 5e17;
        if (usdcBal > 0 || wethBalance > 0) {
            uint256 wethInUSDC = wethToWant(wethBalance, false);
            wethPercentage = wethInUSDC.mul(1e18).div((usdcBal).add(wethInUSDC));
        }
        return _wantBal.mul(wethPercentage).div(1e18);
    }
    ...
    function liquidateAllPositions() internal override returns (uint256) {
        uint256 etaBefore = estimatedTotalAssetsSafe();
        _unwindAllPositions();
    ...
    function _withdrawFromLimitOrder(uint256 _amountNeeded) internal {
        ...
        _burnAndCollectLiquidity(_liquidity, limitTicks);
    ...
    function _withdrawFromPrincipal(uint256 amountNeeded) internal {
        (, , uint128 _principalLiquidity) = principal();
        if (_principalLiquidity == 0) return;

        uint256 fraction = amountNeeded.mul(1e18).div(principalValue());

        uint256 _liquidityNeeded = uint256(_principalLiquidity).mul(fraction).div(1e18);

        uint128 liquidityToBurn = uint128(Math.min(_liquidityNeeded, _principalLiquidity));
    ...
    function _swapNeededWeth(uint256 _wantNeeded) internal {
        ...
        if (wethToSell > 0) _swapToWant(wethToSell);
    }
    ...
    function _swapToWeth(uint256 _amountIn) internal {
        uint256 wantBalBefore = balanceOfWant();
        uint256 toSwap = Math.min(_amountIn, balanceOfWant());
        if (toSwap > wantDust) {
            _swap(int256(toSwap), true);
            _enforceSlippage(toSwap, wantBalBefore.sub(balanceOfWant()), slippageMax);
        }
    }
        ...
        returns (uint256 wantBalance, uint256 wethBalance)
    {
        ...
        if (wantBalance < wantDust) return (wantBalance, wethBalance.div(2));
        if (wethBalance < wethDust) return (wantBalance.div(2), wethBalance);
    }

    function _unwindPosition(Ticks memory _tick) internal returns (uint256, uint256) {
        (uint128 _liquidityLO, , ) = uniHelper.position(_tick);
    ...
    function _unwindAllPositions() internal returns (uint256, uint256) {
        ...
        return (_fees0.add(_fees0L), _fees1.add(_fees1L));
    }

    function getBidAsk() internal view returns (Ticks memory bid, Ticks memory ask) {
        int24 tickFloor = uniHelper.floor(currentTick());
        int24 tickCeil = tickFloor + tickSpacing;
    ...
    function setTicks() internal {
        int24 tickFloor = uniHelper.floor(currentTick());
        int24 tickCeil = tickFloor + tickSpacing;
        baseTicks = Ticks({lowerTick: tickFloor - tickWidth, upperTick: tickCeil + tickWidth});
    }

    function _setLimitOrder() internal {
        (uint256 _wantBalance, uint256 _wethBalance) = getLimitOrderAmounts();
        (Ticks memory bid, Ticks memory ask) = getBidAsk();

        uint128 bidLiquidity = uniHelper.liquidityForAmount0(bid, _wantBalance);
        uint128 askLiquidity = uniHelper.liquidityForAmount1(ask, _wethBalance);
    ...
    function _addLiquidity(uint256 amount0, uint256 amount1) internal returns (uint256, uint256) {
        uint128 _liquidity = uniHelper.liquidityForAmounts(baseTicks, amount0, amount1);
        return _mintLiquidity(_liquidity, baseTicks);
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        (uint256 _amount0Min, uint256 _amount1Min) =
            uniHelper.amountsForLiquidity(_liquidity, _ticks);
        if (_liquidity > 0) {
            mintStorage = true;
    ...
    function _collectTradingFees() internal returns (uint256 fee0, uint256 fee1) {
        (uint128 _liquidityP, , ) = uniHelper.position(baseTicks);
        if (_liquidityP > 0) {
            return _burnAndCollectLiquidity(0, baseTicks);
        }
    }
    ...
    function _getBalance(uint256 _fees, bool _oracle) internal view returns (uint256) {
        (uint256 totalUsdcBalance, uint256 totalWethBalance) = getTotalInvestedAssets();
    ...
    function setTickParams(int24 _lowerTick, int24 _upperTick) external onlyGovernance {
    ...
    function prepareMigration(address _newStrategy) internal override {
        ...
        uint256 wethBalance = balanceOfWeth();
        if (wethBalance > 0) {
            wETH.transfer(_newStrategy, wethBalance); 
        }
    }
...
interface IUniswapHelper {
    ...
    function liquidityForAmount0(Ticks memory _ticks, uint256 _amount0)
        external
        pure
        returns (uint128);

    function liquidityForAmount1(Ticks memory _ticks, uint256 _amount1)
        external
        pure
        returns (uint128);

    function amountsForLiquidity(uint128 _liquidity, Ticks memory _ticks)
        external
        view
    ...
    function rawPosition(Ticks memory _tick)
        external
        view
        returns (
            uint128 liquidity,
    ...
    function position(Ticks memory _tick)
        external
        view
        returns (
            uint128 liquidity,
    ...
    function floor(int24 tick) external view returns (int24);

    function estimatedFees(Ticks memory _ticks, int24 _currentTick)
        external
        view
        returns (uint256, uint256);

    function getPosition(Ticks memory _ticks)
        external
        view
        returns (
            uint256 usdcAmount,
