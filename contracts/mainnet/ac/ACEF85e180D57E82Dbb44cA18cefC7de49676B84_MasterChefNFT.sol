...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    uint256 public lastBanksyDistroTime;

    uint256 public pendingUsd;

    uint256 public pendingBanksy;
    ...
    event SetBanksyAddress(address banksyAddress);
    ...
    ///@dev sell all of a current type of token for usd. and distribute on a drip.
    function getBanksyRelease(uint256 totalBanksyLockup) external onlyRole(OPERATOR_ROLE) returns (uint256) {
        require(banksyAddress != address(0));

        uint256 banksyBalance = IERC20(banksyAddress).balanceOf(address(this));
        if (pendingBanksy + totalBanksyLockup > banksyBalance)
            return 0;

        uint256 banksyAvailable = banksyBalance - pendingBanksy - totalBanksyLockup;

        // only provide a drip if there has been some blocks passed since the last drip
        uint256 timeSinceLastDistro = block.timestamp > lastBanksyDistroTime ? block.timestamp - lastBanksyDistroTime : 0;

        // We distribute the banksy assuming the old banksy balance wanted to be distributed over distributionTimeFrame blocks.
        uint256 banksyRelease = (timeSinceLastDistro * banksyAvailable) / distributionTimeFrame;

        banksyRelease = banksyRelease > banksyAvailable ? banksyAvailable : banksyRelease;

        lastBanksyDistroTime = block.timestamp;
        pendingBanksy = pendingBanksy + banksyRelease;

        return banksyRelease;
    }
    ...
    function transferBanksyToOwner(address ownerAddress, uint256 amount) external onlyRole(OPERATOR_ROLE) {
        require(banksyAddress != address(0));

        uint256 banksyBalance = IERC20(banksyAddress).balanceOf(address(this));
        if (banksyBalance < amount)
            amount = banksyBalance;

        require(IERC20(banksyAddress).transfer(ownerAddress, amount), "E1");

        /// avoid negative result
        if (amount > pendingBanksy)
            amount = pendingBanksy;

        pendingBanksy = pendingBanksy - amount;

        emit BanksyTransferredToUser(ownerAddress, amount);
    }

    function setBanksyAddress(address newBanksyAddress) external onlyRole(DEFAULT_ADMIN_ROLE) {
        banksyAddress = newBanksyAddress;

        emit SetBanksyAddress(banksyAddress);
    }
...
contract MasterChefNFT is Ownable, ReentrancyGuard, ERC721Holder {
    ...
    struct UserInfo {
        ...
        uint256 extraBanksyRewardDebt;
        uint256 banksyRewardLockup;
        uint256 usdRewardLockup;
        uint256 extraBanksyRewardLockup;
    ...
    uint256 public constant banksyMaximumSupply = 500 * (10 ** 3) * (10 ** 18); // 500,000 banksyToken
    ...
    // Total usd collected
    uint256 public totalExtraBanksyCollected;

    // USD per share
    uint256 public accDepositUsdRewardPerShare;

    // Extra Banksy per share
    uint256 public accDepositExtraBanksyRewardPerShare;
    ...
    // Total extra Banksy Lockup
    uint256 public totalExtraBanksyLockup;
    ...
    function deposit(uint256 pid, uint256 amount) external nonReentrant {
        ...
        if (amount > 0) {
            ...
            if (pool.depositFeeBP > 0) {
                ...
                // send 3% to banksydao finance
                IERC20(pool.lpToken).safeTransfer(feeAddress, devDepositFee);
                // send 1% to treasury
                IERC20(pool.lpToken).safeTransfer(treasuryUtil, treasuryDepositFee);
        ...
        if (pid == ownershipPid) {
            ...
            user.extraBanksyRewardDebt = ((user.amount * accDepositExtraBanksyRewardPerShare) / 1e24);
        }
    ...
    function emergencyWithdraw(uint256 pid) external nonReentrant {
        ...
        user.extraBanksyRewardDebt = 0;
        user.extraBanksyRewardLockup = 0;
    ...
    ///@return pending Extra Banksy.
    function pendingExtraBanksy(address userAddress) external view returns (uint256) {
        UserInfo storage user = userInfo[0][userAddress];

        return ((user.amount * accDepositExtraBanksyRewardPerShare) / 1e24) + user.extraBanksyRewardLockup - user.extraBanksyRewardDebt;
    }
    ...
    function _updatePool(uint256 pid) internal {
        ...
        if (poolInfo[ownershipPid].totalLocked > 0) {
            ...
            uint256 extraBanksyRelease = treasuryDAO.getBanksyRelease(totalExtraBanksyLockup);
            accDepositExtraBanksyRewardPerShare = accDepositExtraBanksyRewardPerShare + ((extraBanksyRelease * 1e24) / poolInfo[ownershipPid].totalLocked);
            totalExtraBanksyCollected = totalExtraBanksyCollected + extraBanksyRelease;
        }
    ...
    function _payPendingBanksy(uint256 pid) internal {
        ...
        uint256 pendingExtraBanksyToken;
        if (pid == ownershipPid) {
            pendingUsdToken = ((user.amount * accDepositUsdRewardPerShare) / 1e24) - user.usdRewardDebt;
            pendingExtraBanksyToken = ((user.amount * accDepositExtraBanksyRewardPerShare) / 1e24) - user.extraBanksyRewardDebt;
        }
        ...
        if (canHarvest(pid, msg.sender)) {
            ...
            if (pid == ownershipPid) {
                ...
                if (pendingExtraBanksyToken > 0 || user.extraBanksyRewardLockup > 0) {
                    uint256 extraBanksyRewards = pendingExtraBanksyToken + user.extraBanksyRewardLockup;
                    treasuryDAO.transferBanksyToOwner(msg.sender, extraBanksyRewards);
                    if (user.extraBanksyRewardLockup > 0) {
                        totalUsdLockup = totalUsdLockup - user.extraBanksyRewardLockup;
                        user.extraBanksyRewardLockup = 0;
                    }
                }
            }
        } else if (pendingBanksyToken > 0 || pendingUsdToken > 0 || pendingExtraBanksyToken > 0) {
            ...
            if (pid == ownershipPid) {
                ...
                if (pendingExtraBanksyToken > 0) {
                    user.extraBanksyRewardLockup = user.extraBanksyRewardLockup + pendingExtraBanksyToken;
                    totalExtraBanksyLockup = totalExtraBanksyLockup + pendingExtraBanksyToken;
                }
            }
        }
