...
contract AiPolyFinance is Context, IERC20, IERC20Metadata, Ownable {

    //TODO remove
    mapping (address => bool) private _isWhitelisted;
    address[] private _confirmedWhites;
    ...
    string private _name = 'AiPoly Finance';
    string private _symbol = 'AIPO';
    ...
    function _transfer(address sender, address recipient, uint256 amount) private {
        ...
        if(sender != owner() && recipient != owner()) {
          ...
          require((_isWhitelisted[recipient] || _isWhitelisted[sender]), 'Only whitelisted addresses allowed here!');//TODO remove
        }
    ...
    function _whitelistAddress(address account) external onlyOwner() { //TODO remove
        require(!_isWhitelisted[account], "Account is already whitelisted");
        _isWhitelisted[account] = true;
        _confirmedWhites.push(account);
    }

    function _removeFromWhitelist(address account) external onlyOwner() { //TODO remove
        require(_isWhitelisted[account], "Account is not whitelisted");
        for (uint256 i = 0; i < _confirmedWhites.length; i++) {
            if (_confirmedWhites[i] == account) {
                _confirmedWhites[i] = _confirmedWhites[_confirmedWhites.length - 1];
                _isWhitelisted[account] = false;
                _confirmedWhites.pop();
                break;
            }
        }
    }
