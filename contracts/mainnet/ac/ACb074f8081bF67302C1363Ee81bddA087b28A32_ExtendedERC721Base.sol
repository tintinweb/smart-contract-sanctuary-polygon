...
/**
 * @notice Contract extended from ERC721Base. It implements:
 * setTokenUri function: it allows setting a full url to a given tokenid.
 * setBaseTokenUri function: it allows setting a base uri for all tokens. In case of its existence,
   it takes precedence over the full url maping.
*/
contract ExtendedERC721Base is ERC721Base, PermissionsEnumerable, PrimarySale, PlatformFee {
    using TWStrings for uint256;
    ...
    /**
     * @notice baseURI that takes precedence over fullURI as a tokenURI in case of its existence
     */
    ...
    /**
     * @notice Thrown if trying to access a non existing token
     */
    error TokenNotFound();
    ...
    /**
     * @notice Emitted when ownership is updated
     */
    ...
    /**
     * @dev function responsible for setting a new tokenURI given a tokenId
     * it overrides the behaviour from ERC721Base contract so fullURI is managed by this contract
    ...
    */
    function _setTokenURI(uint256 _tokenId, string memory _tokenURI) internal override {
        require(
            ...
            "Not allowed to set token uri"
        );
        fullURI[_tokenId] = _tokenURI;
    }

    /// @dev Returns whether a token can be minted in the given execution context.
    function _canMint() internal override view returns (bool) {
     ...
     * Otherwise, it gets the value from fullURI
     * @param _tokenId - token id
    */
    ...
    */
     ...
     * It allows empty values in case we want to fetch the tokenURI back from the fullURI mapping
    ...
    */
    function setTokensURIs(uint256[] memory _tokenIds, string[] memory _tokenURIs) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(_tokenIds.length == _tokenURIs.length, "The number of tokenIds and tokenURIs needs to match");
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            if (_tokenIds[i] >= nextTokenIdToMint()) {
                revert TokenNotFound();
            }
    ...
    /**
     * @notice function responsible for setting new contract owner.
     * It emits an OwnershipTransferred that can be caught by OpenSea
