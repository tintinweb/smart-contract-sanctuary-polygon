...
// File contracts/distribution/HeimShareRewardPool.sol
...
contract HeimShareRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 accHeimSharePerShare; // Accumulated tSHAREs per share, times 1e18. See below.
    ...
    uint256 public tSharePerSecond = 0.001564058 ether; // 50000 heimshare / (370 days * 24h * 60min * 60s)
    ...
    modifier onlyOperator() {
        require(operator == msg.sender, "HeimShareRewardPool: caller is not the operator");
        _;
    }
    ...
    function checkPoolDuplicate(IERC20 _token) internal view {
        ...
        for (uint256 pid = 0; pid < length; ++pid) {
            require(poolInfo[pid].token != _token, "HeimShareRewardPool: existing pool?");
        }
    }
    ...
    function pendingShare(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 accHeimSharePerShare = pool.accHeimSharePerShare;
        uint256 tokenSupply = pool.token.balanceOf(address(this));
        if (block.timestamp > pool.lastRewardTime && tokenSupply != 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            accHeimSharePerShare = accHeimSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        return user.amount.mul(accHeimSharePerShare).div(1e18).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        if (totalAllocPoint > 0) {
            uint256 _generatedReward = getGeneratedReward(pool.lastRewardTime, block.timestamp);
            uint256 _bshareReward = _generatedReward.mul(pool.allocPoint).div(totalAllocPoint);
            pool.accHeimSharePerShare = pool.accHeimSharePerShare.add(_bshareReward.mul(1e18).div(tokenSupply));
        }
        pool.lastRewardTime = block.timestamp;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 _pending = user.amount.mul(pool.accHeimSharePerShare).div(1e18).sub(user.rewardDebt);
            if (_pending > 0) {
                safeHeimShareTransfer(_sender, _pending);
        ...
        user.rewardDebt = user.amount.mul(pool.accHeimSharePerShare).div(1e18);
    ...
    function safeHeimShareTransfer(address _to, uint256 _amount) internal {
