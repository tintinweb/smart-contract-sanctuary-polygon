...
contract BalancerAaveUsdYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private BB_AM_USDC_TOKEN = 0xF93579002DBE8046c43FEfE86ec78b1112247BB8;
  address constant private BB_AM_USD_TOKEN = 0x48e6B98ef6329f8f0A30eBB8c7C960330d648085;
  address constant private BB_AM_USD_GAUGE_TOKEN = 0x1c514fEc643AdD86aeF0ef14F4add28cC3425306;
  ...
  address constant private BALANCER_REWARDS = 0x1c514fEc643AdD86aeF0ef14F4add28cC3425306;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IERC20(BB_AM_USD_GAUGE_TOKEN).balanceOf(address(this));
  }
  ...
  function compound() public override whenNotPaused {
    ...
    if (earnings > COMPOUND_THRESHOLD) {
      ...
      earnings = GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, WMATIC_TOKEN, USDC_TOKEN, address(this), earnings);
  ...
  function joinPool(uint256 usdcValue, bool query) private returns(uint256 amountOut) {
    ...
    swaps[0].poolId = 0xf93579002dbe8046c43fefe86ec78b1112247bb8000000000000000000000759;
    swaps[0].assetInIndex = 0;
    swaps[0].assetOutIndex = 1;
    swaps[0].amount = usdcValue;

    swaps[1].poolId = 0x48e6b98ef6329f8f0a30ebb8c7c960330d64808500000000000000000000075b;
    swaps[1].assetInIndex = 1;
    swaps[1].assetOutIndex = 2;

    address[] memory assets = new address[](3);
    assets[0] = USDC_TOKEN;
    assets[1] = BB_AM_USDC_TOKEN;
    assets[2] = BB_AM_USD_TOKEN;
    ...
    if (query) {
      amountOut = uint256(IBalancer(BALANCER_POOL).queryBatchSwap(IBalancer.SwapKind.GIVEN_IN, swaps, assets, funds)[0]);
    }
    else {
      int256[] memory limits = new int256[](3);
      limits[0] = int256(usdcValue);
      IBalancer(BALANCER_POOL).batchSwap(IBalancer.SwapKind.GIVEN_IN, swaps, assets, funds, limits, block.timestamp + 1200);
      amountOut = IERC20(BB_AM_USD_TOKEN).balanceOf(address(this));
    }
  }
  ...
  function join(uint256 usdcValue) private returns(uint256 shares) {
    ...
    uint256 value = joinPool(usdcValue, false);
    shares = contractShares == 0 ? value * INIT_SHARES : value * contractShares / getContractBalance();
    if (!IERC20(BB_AM_USD_TOKEN).approve(BALANCER_REWARDS, value)) {
      revert();
    }
    IRewards(BALANCER_REWARDS).deposit(value);
  }

  function exitPool(uint256 value, bool query) private returns (uint256 amountOut) {
    IBalancer.BatchSwapStep[] memory swaps = new IBalancer.BatchSwapStep[](2);
    swaps[0].poolId = 0x48e6b98ef6329f8f0a30ebb8c7c960330d64808500000000000000000000075b;
    swaps[0].assetInIndex = 0;
    swaps[0].assetOutIndex = 1;
    swaps[0].amount = value;

    swaps[1].poolId = 0xf93579002dbe8046c43fefe86ec78b1112247bb8000000000000000000000759;
    swaps[1].assetInIndex = 1;
    swaps[1].assetOutIndex = 2;

    address[] memory assets = new address[](3);
    assets[0] = BB_AM_USD_TOKEN;
    assets[1] = BB_AM_USDC_TOKEN;
    assets[2] = USDC_TOKEN;
    ...
    if (query) {
      IBalancer(BALANCER_POOL).queryBatchSwap(IBalancer.SwapKind.GIVEN_IN, swaps, assets, funds);
    }
    else {
      int256[] memory limits = new int256[](3);
      limits[0] = int256(value);
      IBalancer(BALANCER_POOL).batchSwap(IBalancer.SwapKind.GIVEN_IN, swaps, assets, funds, limits, block.timestamp + 1200);
      amountOut = IERC20(USDC_TOKEN).balanceOf(address(this));
    }
  }
...
interface IBalancer {
  ...
  function batchSwap(SwapKind kind, BatchSwapStep[] memory swaps, address[] memory assets, FundManagement memory funds, int256[] memory limits, uint256 deadline) external returns (int256[] memory assetDeltas);
}
