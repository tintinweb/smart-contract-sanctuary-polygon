...
contract FOMOBeta is Ownable {
    ...
    uint256 public newCycleGracePeriod = 60;
    uint256 public timerIncrement = 15;
    uint256 public fee = 50;
    uint256 public rewardPercentage = 700;
    uint256 public percentageDivider = 1000;

    constructor(address initWallet, uint256 startTime) {
        creatorWallet = initWallet;
    ...
    function fomo() public payable {
        ...
        if (currentCountDown <= block.timestamp) {
            ...
            if (depositsPerCycle == 1) {
                ...
                // Transfer Reward
                uint256 reward = (address(this).balance - msg.value) * rewardPercentage / percentageDivider;
                (bool rewardPayout, ) = payable(lastDeposit).call{value: reward}("");
                require(rewardPayout, "Reward payout error.");
            }
    ...
    function timeLeft() public view returns (uint256) {
        if (currentCountDown <= block.timestamp) {
            ...
            return currentCountDown - block.timestamp;
        }
    }

    function setCreatorWallet(address newWallet) public onlyOwner {
