...
contract BeefyUniV2ZapUpdated {
    ...
    // IF any tokenB is NATIVE, send value in tokenBParamter
    function beefInDual(address beefyVault, uint256 tokenAmountOutMin, address tokenInA, address tokenInB, uint256 tokenInAAmount, uint256 tokenInBAmount, bool isNative) external {
        require(tokenInAAmount >= minimumAmount, 'Beefy: Insignificant input amount');
        require(tokenInBAmount >= minimumAmount, 'Beefy: Insignificant input amount');

        if(isNative){
            IERC20(tokenInB).safeTransferFrom(msg.sender, address(this), tokenInBAmount);
        }

        require(IERC20(tokenInA).allowance(msg.sender, address(this)) >= tokenInAAmount, 'Beefy: Input token is not approved');

        if(!isNative){
            require(IERC20(tokenInB).allowance(msg.sender, address(this)) >= tokenInBAmount, 'Beefy: Input token is not approved');
        }
        
        IERC20(tokenInA).safeTransferFrom(msg.sender, address(this), tokenInAAmount);
        IERC20(tokenInB).safeTransferFrom(msg.sender, address(this), tokenInBAmount);

        _stakeDual(beefyVault, tokenInA, tokenInB, tokenInAAmount, tokenInBAmount);
    ...
    function _stakeDual(address beefyVault, address tokenInA, address tokenInB, uint256 tokenAAmount, uint256 tokenBAmount) private {
        (IBeefyVaultV6 vault, IUniswapV2Pair pair) = _getVaultPair(beefyVault);
        _approveTokenIfNeeded(tokenInA, address(router));

        _approveTokenIfNeeded(tokenInB, address(router));
        (,, uint256 amountLiquidity) = router
            .addLiquidity(tokenInA, tokenInB, tokenAAmount, tokenBAmount, 1, 1, address(this), block.timestamp);
    ...
    function _approveTokenIfNeeded(address token, address spender) private {
        if (IERC20(token).allowance(address(this), spender) == 0) {
            IERC20(token).safeApprove(spender, uint256(type(uint256).max));
        }
    }
