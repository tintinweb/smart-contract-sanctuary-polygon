...
contract Treasury is Ownable, ReentrancyGuard {
    ...
    uint public VAULT_FEE = 53 ether;
    ...
    */
    ...
    /**
     * @dev Function to get Game by ID
    ...
    function UpdateGame(uint _gameId, uint _result) TreasuryOwner() nonReentrant external returns(uint) {
        ...
        if (_result == 0) {
            ...
            if (TAKE_FEE) {
                ...
                // Add to FeeVaultBalance to keep track of Fees that stays in the Treasury contract
                FeeVaultBalance += Games[_gameId].Wager * LOSE_FEE / TOTAL_PERCENTAGE;
            }
        } else if (_result == 2) {
    ...
    */
    function DistributeFeesBackToVaults() nonReentrant onlyOwner() external {
        require(TAKE_FEE == true, "Error: Take Fee is not enabled");
        require(FeeVaultBalance > 0, "Error: Fee balance is 0");
        require(address(this).balance > 0, "Error: Treasury balance is 0");
        require(address(this).balance >= FeeVaultBalance, "Error: Treasury balance is less than fee balance");

        uint feeBalance = FeeVaultBalance;
        FeeVaultBalance = 0;
        _takeGameFee(feeBalance, false, true);

        emit DistributedFeeBackToVaults(TeamVaultBalance, NFTVaultBalance);
    }
    ...
    */
    ...
    function _takeGameFee(uint _wager, bool _winFee, bool _insideDistribution) private returns(uint) {
        ...
        if (!_insideDistribution) {
            if (_winFee) {
                fee = _wager * WIN_FEE / TOTAL_PERCENTAGE;
            } else {
                fee = _wager * LOSE_FEE / TOTAL_PERCENTAGE;
            }
        } else {
            fee = _wager;
        }
