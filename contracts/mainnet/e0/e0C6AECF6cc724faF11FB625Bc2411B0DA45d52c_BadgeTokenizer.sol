...
contract BadgeTokenizer is ERC721, Pausable, Ownable2Step, ERC721Burnable {
    ...
    mapping(uint256 => string) private addedAttributes;
    ...
    mapping(uint256 => string[]) private attachments;
    ...
    constructor() ERC721("badgeNFT by Badgify", "bNFT") {
        admin = msg.sender;
    }
    ...
    function fee(uint256 _tfee, uint256 _gfee) external onlyOwner {
        tFees = _tfee; // @note ether value
        gFees = _gfee; // @note percent
    }

    function txfees() public view returns (uint256) {
    ...
    function setBadgeInfo(
        ...
        require(
            authorized(msg.sender, tokenId),
            "BADGIFY: Sender is not authorized"
        );
    ...
    function authorized(
        ...
        bool status;
        if (distributedTokens[tokenId]) {
            status = issuers[tokenId][ownerOf(tokenId)] == addr;
        } else {
            status = ownerOf(tokenId) == addr;
        }
        return status;
    }
    ...
    function setDesignInfo(
        ...
        if (designMaker[designId] != address(0)) {
            require(
    ...
    function singleMint(
        ...
        require(msg.value >= txfees(), "BADGIFY: Tokenizer fee not met");
        require(
            !registrar.blacklisted(msg.sender),
            "BADGIFY: Sender is blacklisted"
        );
    ...
    function multiMint(
        ...
        require(
            msg.value >= txfees().mul(addr.length),
        ...
        if (msg.value > 0) {
            badgeBalances[0] = badgeBalances[0].add(msg.value); // @note badge collect fees
        }
        if (msg.sender != admin) {
    ...
    function viewIssuer(
        ...
        return issuers[tokenId][receiver];
    }
            ...
            string(
                abi.encodePacked(
                    ...
                    Base64.encode(
                        bytes(
                            abi.encodePacked(
                                '{"name":"badgeNFT","description":"Digital badges on the blockchain","image":"',
                                themer.cover(address(this)),
                                '","external_link":"',
                                themer.link(),
                                '","fee_recipient":""}'
                            )
    ...
    function addAttribute(uint256 tokenId, string memory _attributes) external {
        ...
        addedAttributes[tokenId] = _attributes;
    }
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    registrar.viewNamespace(
                        viewIssuer(tokenId, ownerOf(tokenId))
                    ),
    ...
    function revoke(uint256 tokenId) external {
        ...
        if (daoActivated) {
            require(
                verifier.revokers(msg.sender),
                "BADGIFY: Sender is not a badge revoker"
            );
            require(
                verifier.flaggedBadge(tokenId),
                "BADGIFY: Badge is not flagged at all"
            );
        } else {
    ...
    function viewBadgeBalance(uint256 tokenId) external view returns (uint256) {
        return badgeBalances[tokenId];
    }

    function addBadgeBalance(uint256 tokenId) external payable noReentrant {
        require(_exists(tokenId));
        require(msg.value > 0, "BADGIFY: Invalid input value");
        uint256 processingFee = msg.value.mul(gFees).div(100);
        require(
            processingFee.add(badgeBalances[tokenId]) <= msg.value,
            "BADGIFY: Integer overflow"
        );
    ...
    function redeemBadgeBalance(
        ...
        require(badgeBalances[tokenId] > 0, "BADGIFY: Badge has no balance");

        uint256 currentBalance = badgeBalances[tokenId];
        require(
            address(this).balance > currentBalance,
            "BADGIFY: Badge has no balance"
        );
        badgeBalances[tokenId] = 0;

        (bool redeemed, ) = admin.call{value: currentBalance}("");
        require(redeemed, "BADGIFY: Failed to redeem balance");
    }

    function attachToToken(uint256 tokenId, string memory data) external {
        require(authorized(msg.sender, tokenId), "BADGIFY: Not Authorized");
        attachments[tokenId].push(data);
    }
...
/**
 * @dev Interface of the BadgeVerifier.
 */
interface IBadgeVerifier {
    function version() external pure returns (string memory);

    function revokers(address) external view returns (bool);

    function flaggedBadge(uint256) external view returns (bool);
}
