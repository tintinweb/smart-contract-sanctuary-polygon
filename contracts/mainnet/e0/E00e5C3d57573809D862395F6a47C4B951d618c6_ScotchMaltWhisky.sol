...
contract ScotchMaltWhisky is Ownable, ContextMixin, ERC721 {
    ...
    function mintOne(address to, string memory uri) public {
        require(
            msg.sender == owner() || founders[msg.sender] || manager[msg.sender],
            "NOT ALLOWED"
        );
    ...
    function setManager(address _admin, bool status) external onlyOwner {
        manager[_admin] = status;
...
abstract contract Ownable is Context {
    ...
    event Founder(address indexed from, address indexed founder);
    ...
    modifier onlyOwner() {
        require(
            _msgSender() == owner() || founders[_msgSender()] == true,
    ...
    function addFounder(address admin) public virtual onlyOwner {
        founders[admin] = true;
        emit Founder(msg.sender, admin);
    }
