...
contract g_Power is owned {
    ...
    mapping(uint => poolInfo[6]) public poolInfos;
    ...
    function process(uint _poolType, uint _quantity, address _ref) internal returns(bool) {
        ...
        for (uint i=0;i<5;i++) {
            if(i != _poolType) {
                poolInfos[lpi + 1][i].totalPoolAmount = poolInfos[lpi][i].totalPoolAmount;
                poolInfos[lpi + 1][i].totalPoolGain += amt * 4/5 * j/100;
                lastPoolIndex[i]++;
            }
            j = j + 5;
        }
    ...
    function withdrawValue(uint _poolType,uint upToIndex) public view returns(uint) {
        uint lpi = lastPoolIndex[_poolType];
        investInfo memory temp = investInfos[msg.sender][_poolType];
    ...
    function withdrawFounderValue(uint upToIndex) public view returns(uint) {
        uint _poolType = 5;
        uint lpi = founderLastPoolIndex;
        investInfo memory temp = investInfos[msg.sender][_poolType];
