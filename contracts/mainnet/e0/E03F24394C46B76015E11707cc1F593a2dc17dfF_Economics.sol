...
contract Economics is IEconomics, AuthModifiers, OwnableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function reserveFuelPrimarySale(
        ...
        uint256 _fuel = _reserveFuelForRate(
            integrator,
            _ticketActions,
            integrator.rates.minFeePrimary,
            integrator.rates.maxFeePrimary,
            integrator.rates.primaryRate
        );

        emit FuelReservedPrimary(integrator.index, uint32(_ticketActions.length), _fuel);
        return _fuel;
    }
    ...
    function reserveFuelSecondarySale(
        ...
        uint256 _fuel = _reserveFuelForRate(
            integrator,
            _ticketActions,
            integrator.rates.minFeeSecondary,
            integrator.rates.maxFeeSecondary,
            integrator.rates.secondaryRate
        );

        emit FuelReservedSecondary(integrator.index, uint32(_ticketActions.length), _fuel);
        return _fuel;
    }
    ...
    */
    ...
    function updateDynamicRates(uint32 _integratorIndex, DynamicRates calldata _dynamicRates)
        ...
        onlyIntegratorAdmin
    {
        integratorData[_integratorIndex].rates = _dynamicRates;
    ...
    function enableIntegratorBilling(uint32 _integratorIndex) external onlyIntegratorAdmin {
        integratorData[_integratorIndex].isBillingEnabled = true;
    ...
    function disableIntegratorBilling(uint32 _integratorIndex) external onlyIntegratorAdmin {
        integratorData[_integratorIndex].isBillingEnabled = false;
