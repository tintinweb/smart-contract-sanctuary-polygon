...
/// @title Hop Facet
contract HopFacet is IMiraidon, SwapperV2, ReentrancyGuard, Validatable {
    /// Storage ///
    bytes32 internal constant NAMESPACE = keccak256("com.miraidon.facets.hop");
    ...
    /// @notice Initialize local variables for the Hop bridge
    ...
    function initHop(Config[] calldata configs) external {
        ...
        for (uint256 i = 0; i < configs.length; i++) {
            require(configs[i].bridge != address(0), GenericErrors.E14);
    ...
    function registerBridge(address assetId, address bridge) external {
        ...
        require(bridge != address(0), GenericErrors.E14);
    ...
    /// @notice Bridges token via Hop protocol
    ...
    /// @notice Contains the business logic for the bridge via hop protocol
    /// @param _bridgeData the core information needed for bridging
    /// @param _hopData data specific to Hop protocol
    function _startBridge(
        ...
        _hopData.amountOutMin = LibFeeCollect.fix(_hopData.amountOutMin);
