...
contract ReadHelper {
    ...
    function getActiveSubscriptions(address addr) public view returns (uint256[] memory) {

        uint256 totalProfiles = profiles.totalProfiles();
        uint256 subCount = 0;
        bool[] memory status = new bool[](totalProfiles);
        
        for (uint256 i = 0; i < totalProfiles; i++) {
            if (!subFactory.isSubscriptionProfileCreated(i)) { status[i] = false; continue; }
        ...
        uint256[] memory active = new uint256[](subCount);
        subCount = 0;
        
        for (uint256 i = 0; i < totalProfiles; i++) {
            if (status[i] == true) { active[subCount] = i; subCount++; }
        }
    ...
    function getProfiles(uint256[] memory profileList) public view returns (address[] memory, address[] memory, string[] memory) {
        ...
        for (uint256 i = 0; i < profileList.length; i++) {
            (address _o, address _r, string memory _c) = profiles.getProfile(profileList[i]);
            owners[i] = _o;
            receivers[i] = _r;
            cids[i] = _c;
        }
