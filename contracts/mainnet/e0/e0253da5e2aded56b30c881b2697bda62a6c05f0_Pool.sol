...
contract Pool is Ownable, IERC721Receiver, VRFConsumerBase {
  ...
  // init
  constructor(address _babies, address _mutants, address _frost, uint256 _frostRate, uint256 _chanceOfStealingNFT, uint256 _chanceOfStealingClaimed, uint256 _chanceOfDying)  VRFConsumerBase(0x3d2341ADb2D31f1c5530cDC622016af293177AE0, 0xb0897686c545045aFc77CF20eC7A532E3120E0F1) { 
  ...
  function addMutantsToPool(address account, uint16[] calldata tokenIds) external {
    require(account == _msgSender(), "DONT GIVE YOUR TOKENS AWAY");
    frost.transferFrom(msg.sender, DEAD_ADDRESS, (100 * tokenIds.length) * 1 ether);

    for (uint i = 0; i < tokenIds.length; i++) {
        uint current = tokenIds[i];
        require(mutantsContract.ownerOf(current) == _msgSender(), "AINT YO TOKEN");
        mutantsContract.transferFrom(_msgSender(), address(this), current);
        _mutantsOfOwner[msg.sender].push(uint16(current));
        pool[address(mutantsContract)][current] = Stake({
          owner: account,
          tokenId: uint16(current),
