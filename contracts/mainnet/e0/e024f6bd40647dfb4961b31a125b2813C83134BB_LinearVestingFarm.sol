...
contract LinearVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    /**
     * @notice function is setting initial state of contract
     *
     * @param _vestedToken - address of token that is vested
     * @param _startTime - when vesting is starting
     * @param _endTime - when is ending
     * @param _earlyClaimAvailablePer - percentage for claiming after start
     * @param _beforeStartClaimPer - percentage for claiming before start
     * @param _initialFundPercent - initial percent for funding farm
     * @param _farmImplementation - address of farms implementation
     */
    function initialize(
        ...
        nonReentrant
    {
        ...
        bool flag = false;
        if(partialFunding){
            if(!_sufficientFunds(rewardAmount)){
                isActive = false;
                emit InsufficientFunds(
                    rewardAmount.sub(vestedToken.balanceOf(address(this))),
                    address(this),
                    isActive
                );
                flag = true;
