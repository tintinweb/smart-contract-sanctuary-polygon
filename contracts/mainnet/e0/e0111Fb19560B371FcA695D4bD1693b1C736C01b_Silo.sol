...
contract Silo is Initializable, KeeperCompatibleInterface {
    ...
    event StrategyFailed(uint256 i);
    ...
    modifier onlySiloManager() {
        require(
            ISiloManagerFactory(Factory.managerFactory()).isManager(msg.sender),
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        if (status == Statuses.UNWIND) {
            ...
            if (siloDelay != 0 && deposited && status == Statuses.MANAGED) {
        ...
        onlySiloManager
    {
        ...
        if (keeperExit) {
            ...
            //     //not enough funds in the silo to reinvestment, so set status to paused
    ...
    function ExitSilo(address caller) external onlyFactory {
        ...
        uint256 balance;
        IERC20 token;
        for (uint256 i = 0; i < tokensInPlay.length; i++) {
    ...
    function setStrategy(
        ...
        require(
            status == Statuses.PAUSED,
            "Call exitSiloStrategy(id, 0) before changing strategy"
        );
        require(!deposited, "Silo has funds deposited in it");
        require(
        ...
        for (uint256 i = 0; i < _implementations.length; i++) {
            ...
            require(
                ...
                "Actions have different output/input lengths"
            );
            for (uint256 j = 0; j < actionInput.length; j++) {
                require(
                    actionInput[j] == actionOutput[j],
                    "Actions input/output addresses do not match"
                );
            }
            ...
            if (
                !highRiskAction && Factory.highRiskActions(_implementations[i])
            ) {
                highRiskAction = true;
            }
        ...
        (bool isLimit, , , ) = getWithdrawLimitSiloInfo();
    ...
    function adjustStrategy(
        ...
        require(
            ...
            "Proposed Configuration Data is not valid"
        );
    ...
    function pause() external onlyFactory {
        ...
        require(
            status == Statuses.MANAGED,
            "Cannot pause a silo that is not managed"
        );
        status = Statuses.PAUSED;
    }
    ...
    function unpause() external onlyFactory {
        require(
            status == Statuses.PAUSED,
            "Cannot unpause a silo that is not paused"
        );
        status = Statuses.MANAGED;
    }
    ...
    function setActive() external onlyFactory {
        require(
            ...
            "Cannot set active a silo that is not unwind"
        );
        status = Statuses.MANAGED;
    }
    ...
    /****************************Internal Functions*****************************/
    function _investSilo() internal returns (uint256[4] memory amounts) {
        address[4] memory depositTokens = abi.decode(
            configurationData,
            (address[4])
        );
        for (uint256 i = 0; i < 4; i++) {
            ...
            if (depositTokens[i] != address(0)) {
                ...
                if (
                    tokenAmount > 0 &&
                    tokenAmount >= Factory.tokenMinimum(depositTokens[i])
                ) {
    ...
    function _runStrategy() internal {
        uint256[4] memory amounts = _investSilo();
    ...
    function _exitStrategy(uint256 _requestedOut) internal {
        ...
        uint256[4] memory amounts = [_requestedOut, 0, 0, 0];
        ...
        // if(_requestedOut == 0){
        //     //strategy is only exited if owner is withdrawing everything!
        //     deposited = false;
        // }
    }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < strategy.length; i++) {
            (currentBalance,possibleWithdraw, availableBlock) = IAction(strategy[i].implementation)
                .withdrawLimit(strategy[i].configurationData);
            if (possibleWithdraw > 0 || availableBlock > 0) {
                return (true,currentBalance, possibleWithdraw, availableBlock);
            }
        }
        return (false, 0, 0, 0);
    }

    // function hasStrategyBalance() public view returns (bool) {
    //     IAction action;
    //     ActionBalance memory strategyBalance;

    //     for (uint256 i = 0; i < strategy.length; i++) {
    //         action = IAction(strategy[i].implementation);
    //         strategyBalance = action.showBalances(
    //             address(this),
    //             strategy[i].configurationData
    //         );
    //         if (strategyBalance.collateral > 0) {
    //             return true;
    //         }
    //     }
    //     return false;
    ...
    function _setConfigData(address[4] memory _input, address[4] memory _output)
        internal
    {
