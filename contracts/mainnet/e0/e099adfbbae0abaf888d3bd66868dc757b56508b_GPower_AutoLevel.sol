...
contract GPower_AutoLevel is Ownable {
   ...
   // uint public adminDistPart = 1 * ( 10 ** 18 );
    ...
    function Team_registrationExt(address userAddress, address referreraddress) external {
   ...
   //     netTotalUserWithdrawable[owner] += adminDistPart;
    ...
    function Team_ReEntry(address userAddress) public returns(bool)
    {
        require(userInfos[userAddress].joined, "register first");

        require(lastReJoinTime[userAddress] + nextReJoinWait <= now, "please wait time little more");
        //this saves gas while using this multiple times
        tokenDai.transferFrom(msg.sender, address(this), 15 * (10 ** 18));
        
        require(payForLevelOne(userAddress),"pay for level fail");

        updateNPayAutoPool(1,userAddress);

        lastReJoinTime[userAddress] = now;
        return true;
    }
    ...
    function payForLevelOne(address _user) internal returns (bool){
        ...
        for(uint i=0;i<7;i++)
        {
            ...
            LevelIncome[ref] = (LevelIncome[ref] + pay[i]);
        }               
        return true;
    }
    ...
    function updateNPayAutoPool(uint _level,address _user) internal returns (bool)
    {
        ...
        for(uint i=0;i<7;i++)
        {
            ...
            if(refcount>=dCount[i])
            {
                ...
                AutopoolIncome[usr] = (AutopoolIncome[usr] + amount);
            }
            else
            {
