...
contract OfferDex is Ownable, DexStorage {
 ...
 event OfferAccept(uint256 tokenId, address collection, uint256 amount, address from, address to, uint256 currencyType, string fromOwnerId, bytes32 hashOffer);
  ...
  function acceptOffer(address[2] memory participants /** offerer,collection */, uint256[4] memory nums /** tokenId, price, currencyType, expiry */, uint8 v /** offerer V */,
        bytes32[2] memory rs /** offerer r & s */) public {

          // caller should be seller OR tokenId's owner
          require(INFT(participants[1]).ownerOf(nums[0]) == msg.sender || _saleTokens[nums[0]][participants[1]].seller == msg.sender || _auctionTokens[nums[0]][participants[1]].seller == msg.sender, "unauthorized to accept offer");
          // construct offer
          Offer memory offer = Offer(participants[0],participants[1], nums[0], nums[1], nums[2], nums[3]);
          // construct Offerrer Sig
          Sig memory offerrerSig = Sig(v, rs[0], rs[1]) ;
            // validate offer
            // verify offer signed by buyer & not expired
              bytes32 hashOffer = requireValidOffer(offer, offerrerSig);
          // process the offer
          acceptedOffer[hashOffer] = true;
            // deduct amount of given token from buyer & send to seller after platform %age
            // transfer tokenId to buyer/offerrer
            processOffer(offer, nums[0], participants[1], hashOffer);

  }
/**
     * @dev Assert an offer is valid and return its hash
     * @param offer Offer to validate
     * @param sig ECDSA signature
     */
    function requireValidOffer(Offer memory offer, Sig memory sig)
        internal
        view
        returns (bytes32)
    {
        bytes32 hash = hashToSign(offer);
        require(validateOffer(hash, offer, sig),"invalid offer");
        return hash;
    }
/**
     * @dev Hash an offer, returning the hash that a client must sign, including the standard message prefix
     * @param offer Offer to hash
     * @return Hash of message prefix and offer hash per Ethereum format
     */
    function hashToSign(Offer memory offer)
        internal
        pure
        returns (bytes32)
    {
        bytes32 hash = keccak256(abi.encodePacked(offer.maker,
                                        offer.collection,
                                        offer.tokenId,
                                        offer.price,
                                        offer.currencyType,
...
/**
     * @dev Validate a provided signed offer, hash, and signature.
     * @param hash Offer hash (already calculated, passed to avoid recalculation)
     * @param offer Offer to validate
     * @param sig ECDSA signature
     */
    function validateOffer(bytes32 hash, Offer memory offer, Sig memory sig) 
        internal
        view
        returns (bool)
    {
  
        /* Offer must have valid parameters. */
        if (!validateOfferParameters(offer)) {
            return false;
        }

        /* Offer must have not been already accepted. */
        if (acceptedOffer[hash]) {
            return false;
        }

        /* or (b) ECDSA-signed by maker. */
        // if (ecrecover(hash, sig.v, sig.r, sig.s) == offer.maker) {
        //     return true;
        // }
...
/**
     * @dev Validate offer parameters (does *not* check signature validity)
     * @param offer Offer to validate
     */
    function validateOfferParameters(Offer memory offer)
        internal
        view
        returns (bool)
    {
      ...
      if(offer.collection == address(0)){
        return false;
      }
      // TODO: price & currencyType checks
    ...
    // it's a function to fetch hash of offer, which it to be signed by offerer and store in Database.
    function hashToSign_(address[2] memory participants, uint256[4] memory nums) public view returns( bytes32 ) {
      
      return keccak256(abi.encodePacked(participants[0],participants[1],nums[0],nums[1],nums[2],nums[3]));
    }
    // it's a utility function of cross-verify if offer is signed by offerer correctly.
    function validateOffer_(bytes32 hash, address[2] memory participants, uint256[4] memory nums, uint8 v, bytes32[2] memory rss) public view returns( bool ) {
      bytes32 hash2 = keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", hash));
      return validateOffer(hash2, Offer(participants[0],participants[1],nums[0],nums[1],nums[2],nums[3]), Sig(v, rss[0], rss[1]));

    }


  function processOffer(Offer memory offer, uint256 tokenId, address collection, bytes32 hashByte ) internal {   
      // offer storage temp = _offers[tokenId][collection]._offer[index];
      Offer memory temp = offer; // just to keep below logic untouched.
   ...
   (uint256 mainPerecentage, uint256 authorPercentage, address blindRAddress) = getPercentages(tokenId, collection);
      token = ALIA;
      if(_saleTokens[tokenId][collection].seller == nonCryptoNFTVault || msg.sender == nonCryptoNFTVault){
        _nonCryptoWallet[_nonCryptoOwners[tokenId][collection]].Alia += SafeMath.mul(SafeMath.div(price,1000), mainPerecentage);
      }
      ...
      if(blindRAddress == address(0x0)){
         blindRAddress = _tokenAuthors[tokenId][collection]._address;
            ...
            // as tokens in offer are not escrowed so should use transferfrom
            token.transferFrom(temp.maker, platform, SafeMath.mul(SafeMath.div(price,1000), platformPerecentage));
        ...
        if( collection == address(XNFT)  || collectionsWithRoyalties[collection]) {
          blindRAddress = _tokenAuthors[tokenId][collection]._address;
            ...
            token.transferFrom(temp.maker, blindRAddress, SafeMath.mul(SafeMath.div(price,1000), authorPercentage));
         
        }
       
           token.transferFrom(temp.maker, msg.sender,SafeMath.mul(SafeMath.div(price,1000), mainPerecentage));
        ...
        if(auct.seller == msg.sender) {
         ...
         if(auct.currencyType < 3){
           ...
           token.transfer(auct.bidder, price); 
            ...
            delete _allowedCurrencies[tokenId][collection][0];
            delete _allowedCurrencies[tokenId][collection][1];
            delete _allowedCurrencies[tokenId][collection][2];
            delete _allowedCurrencies[tokenId][collection][3];
            delete  _saleTokens[tokenId][collection];
        }
        } 
        string memory ownerId;
        if(msg.sender == nonCryptoNFTVault){
          ownerId = _nonCryptoOwners[tokenId][collection];
          delete _nonCryptoOwners[tokenId][collection];
        }
      INFT(collection).transferFrom(owner, temp.maker, tokenId);
      
     emitOfferEvent( temp, tokenId, collection, ownerId, hashByte);
      
  }
  function emitOfferEvent(Offer memory temp, uint256 tokenId, address collection, string memory ownerId, bytes32 hashByte) internal {
     emit OfferAccept(tokenId, collection, temp.price, msg.sender, temp.maker, temp.currencyType, ownerId, hashByte);
  }
