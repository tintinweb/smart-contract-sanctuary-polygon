...
interface IMaiVaultV2 {
    ...
    function payBackToken(uint256 vaultID, uint256 amount,uint256 _front) external;
...
*/
contract ZapMaiVaultV2Front is BaseSiloAction {
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],address,uint[3],uint";
    ...
    function enter(
        ...
        uint256 vaultId;
        {
            ...
            (, output, maiVault, ratioRange, position) = abi.decode(
                configuration,
                (address[5], address[5], address, uint256[3], uint256)
            );

            outputAmounts[position] = IERC20Decimals(output[position])
        ...
        if (inputAmounts[0] > 0) {
            depositCollateral(maiVault, vaultId, inputAsset, inputAmounts[0]);
        }

        uint256 debt = IMaiVaultV2(maiVault).vaultDebt(vaultId);

        //repay debt
        if (debt > 0 && inputAmounts[4] > 0) {
            if (inputAmounts[4] > debt) {
                //just repay the debt
                payBackToken(maiVault, vaultId, debt);
            } else {
        ...
        if (inputAmounts[4] == 0) {
            ...
            IERC20Decimals miMatic = IERC20Decimals(
                IMaiVaultV2(maiVault).mai()
            );
            ...
            uint256 collateralValue = ((
                ...
                (10 ** IMaiVaultV2(maiVault).priceSourceDecimals()); //in terms of collateral decimals

            uint256 debtCeil = IMaiVaultV2(maiVault).getDebtCeiling();

            collateralValue =
                ((10 ** miMatic.decimals()) * collateralValue) /
                (10 ** collateral.decimals()); //convert to miMatic decimals
            debt = IMaiVaultV2(maiVault).vaultDebt(vaultId); //in terms of miMatic decimals

            if (debt > 0) {
                if (inputAmounts[0] > 0) {
                    //calculate amount to borrow
                    uint256 idealDebt = ((10 ** miMatic.decimals()) *
                        collateralValue) / ratioRange[1];

                    if (idealDebt > debt) {
                        uint256 amountToBorrow = idealDebt - debt;
                        if (amountToBorrow <= debtCeil) {
                            IMaiVaultV2(maiVault).borrowToken(
                                vaultId,
                                amountToBorrow,
                                0
                            );
                            outputAmounts[0] = amountToBorrow;
                        } else if (debtCeil > 0 && amountToBorrow > debtCeil) {
                            IMaiVaultV2(maiVault).borrowToken(
                                vaultId,
                                debtCeil,
                                0
                            );
                            outputAmounts[0] = debtCeil;
                        }
                    }
                } else {
                    uint256 CDratio = ((10 ** miMatic.decimals()) *
                        collateralValue) / debt; //10**18 is to match zeroes in ratioRange

                    if (CDratio < ratioRange[0]) {
                        //need to tell next farm to withdraw tokens to pay back loan
                        //calculate amount to repay.
                        uint256 idealDebt = ((10 ** miMatic.decimals()) *
                            collateralValue) / ratioRange[1]; //in miMatic decimals
                        outputAmounts[4] = debt - idealDebt;
                        ...
                        if (idealDebt > debt) {
                            ...
                            if (amountToBorrow <= debtCeil) {
                                ...
                                debtCeil > 0 && amountToBorrow > debtCeil
                            ) {
                                IMaiVaultV2(maiVault).borrowToken(
                                    vaultId,
                                    debtCeil,
                ...
                if (idealDebt > 0) {
                    ...
                    } else if (debtCeil > 0 && debtCeil < idealDebt) {
                        IMaiVaultV2(maiVault).borrowToken(vaultId, debtCeil, 0);
                        outputAmounts[0] = debtCeil;
                    }
                }
            }
            ...
            if (inputAmounts[4] > 0) {
                ...
                if (vaultDebt > 0) {
                    if (inputAmounts[4] > vaultDebt) {
                        //just repay the debt
                        miMatic.approve(maiVault, vaultDebt);
                        vault.payBackToken(vaultId, vaultDebt, 0);
                    } else {
                        miMatic.approve(maiVault, inputAmounts[4]);
                        vault.payBackToken(vaultId, inputAmounts[4], 0);
                    }
                }
            }

            //calculate how much collateral to remove from the vault
            if (
                inputAmounts[0] == 0 ||
                inputAmounts[0] >= vault.vaultCollateral(vaultId)
            ) {
                //trying to remove ALL collateral
                outputAmounts[4] = 2 ** 256 - 1; //ask next action to remove ALL stake
            } else {
                uint256 debtCeil = vault.getDebtCeiling();

                //check the Collateral to Debt Ratio
                IERC20Decimals collateral = IERC20Decimals(vault.collateral());
                IPriceFeed oracle = IPriceFeed(vault.ethPriceSource());
                uint256 collateralValue = ((vault.vaultCollateral(vaultId) -
                    inputAmounts[0]) * oracle.latestAnswer()) /
                    (10 ** vault.priceSourceDecimals()); //in terms of collateral decimals
                if (collateralValue > 0) {
                    collateralValue =
                        ((10 ** miMatic.decimals()) * collateralValue) /
                        (10 ** collateral.decimals()); //convert to miMatic decimals
                    uint256 idealDebt = (10 ** miMatic.decimals() *
                        collateralValue) / ratioRange[1];
                    uint256 debt = vault.vaultDebt(vaultId); //in terms of miMatic decimals
                    if (idealDebt > debt) {
                        //want to take out more of a loan
                        outputAmounts[0] = idealDebt - debt;
                        if (
                            outputAmounts[0] > 0 && outputAmounts[0] <= debtCeil
                        ) {
                            // vault.borrowToken(vaultId, outputAmounts[0]);
                        } else if (debtCeil != 0) {
                            outputAmounts[0] = debtCeil;
                            // vault.borrowToken(vaultId, vault.getDebtCeiling());
                        }
                    } else if (idealDebt < debt) {
    ...
    function depositCollateral(
        ...
        IERC20Decimals(inputAsset).approve(zapper, amount);

        try
            IThreeStepQiZappah(zapper).beefyZapToVault(
                amount,
                vaultId,
                inputAsset,
                vault.collateral(),
    ...
    function validateConfig(
        ...
        uint256[3] memory ratioRange;
        uint256 position;
        (input, , maiVault, ratioRange, position) = abi.decode(
            configData,
            (address[5], address[5], address, uint256[3], uint256)
        );
        // IMaiVaultV2 vault = IMaiVaultV2(maiVault);

        if (input[position] != address(0)) {
            //make sure position spot is free
            return false;
        }
    ...
    function checkMaintain(
        ...
        if (storedConfig.length > 0) {
            //if config is set in strategy use it
            (, , maiVault, ratioRange) = abi.decode(
                storedConfig,
                (address[5], address[5], address, uint256[3])
            );
        } else {
            (, , maiVault, ratioRange) = abi.decode(
                configuration,
        ...
        IPriceFeed oracle = IPriceFeed(vault.ethPriceSource());
        uint256 collateralValue = (vault.vaultCollateral(vaultId) *
            oracle.latestAnswer()) / (10 ** vault.priceSourceDecimals()); //in terms of collateral decimals
        collateralValue =
            ((10 ** miMatic.decimals()) * collateralValue) /
            (10 ** collateral.decimals()); //convert to miMatic decimals
        uint256 debt = vault.vaultDebt(vaultId); //in terms of miMatic decimals
        if (debt > 0) {
            uint256 CDratio = ((10 ** miMatic.decimals()) * collateralValue) /
                debt;
            if (CDratio < ratioRange[0]) {
                // need to repay the debt
                return (true, 2);
            } else if (CDratio > ratioRange[2]) {
                //minimum debt ceiling value
                //check how much would be borrowed
                uint256 amountToBorrow = ((10 ** miMatic.decimals()) *
                    collateralValue) /
                    ratioRange[1] -
                    debt;
            ...
            if (collateralValue > 0 && vault.getDebtCeiling() > 0) {
                return (true, 4);
            }
            return (false, 4);
        }
    ...
    function showBalances(
        ...
        if (vault.balanceOf(_silo) == 0) {
            ...
            vaultId = vault.tokenOfOwnerByIndex(_silo, 0);
        }
        return
            ActionBalance({
                collateral: vault.vaultCollateral(vaultId),
                debt: vault.vaultDebt(vaultId),
                collateralToken: inputAsset,
                debtToken: output[0],
                collateralConverted: 0,
                collateralConvertedToken: address(0),
                lpUnderlyingBalances: "",
        ...
        )
    {
        ...
        if (vault.balanceOf(_silo) > 0) {
            ...
            IPriceFeed oracle = IPriceFeed(vault.ethPriceSource());
            collateral = vault.vaultCollateral(vaultId);
            uint256 collateralValue = (collateral * oracle.latestAnswer()) /
                (10 ** vault.priceSourceDecimals()); //in terms of collateral decimals
            collateralValue =
                ((10 ** miMatic.decimals()) * collateralValue) /
                (10 ** IERC20Decimals(vault.collateral()).decimals()); //convert to miMatic decimals
            debt = vault.vaultDebt(vaultId); //in terms of miMatic decimals
            if (debt > 0) {
                cdr = ((10 ** miMatic.decimals()) * collateralValue) / debt;
            }
        }
    }
}
