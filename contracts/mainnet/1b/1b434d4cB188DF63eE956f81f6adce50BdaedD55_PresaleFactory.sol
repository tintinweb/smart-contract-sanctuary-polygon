    ...
    OwnableUpgradeable
{
    ...
    function create(
    ...
    ) external returns (address _contratAddress) {
        require(
            contractsList[_presaleName] == address(0),
            "PSF: peoject already exists."
        );
        ...
        _contratAddress = address(newContract);
    ...
    function getContract(string memory _preSaleName)
        external
        view
        returns (address _contratAddress)
    {
        _contratAddress = contractsList[_preSaleName];
    }
}
...
contract Presale is IPresale, PresaleTypes, Ownable, ReentrancyGuard {
    ...
    function withdrawInvestedToken() external {
        ...
        require(
            state == PresaleState.FAILURE,
            "the user is not OUT or the presale did not touch softcap"
        );
