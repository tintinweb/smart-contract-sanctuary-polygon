...
contract TomiDNSMarketplace is ERC721Holder, ReentrancyGuard, Ownable {
    ...
    event PartnerClaimed(uint256 partnerId , uint256 _tokenId, uint256 amount , string label , string tld , address claimedBy);
    ...
    function setOnAuction(
        ...
        auction.tld = _tld;
        auction.raisedAmount = auction.mintAmount.sub(auction.alreadyClaimed);
    ...
    function claim(uint256 _tokenId) external nonReentrant {
        ...
        if (isFirstBid) {
            uint256 treasuryAmount = auction.raisedAmount;
    ...
    function getMinterClaimReward(uint256 id) view public returns(uint256){
        uint256 _tokenId = partnerNFT.getDnsOfNFT(id);
        Auction memory auction = getAuctions[_tokenId];
        Bidding memory bidding = getBiddings[_tokenId];
        return bidding.amount.mul(auction.minterClaimPercentage).div(100).sub(auction.alreadyClaimed);
    }

     function MinterClaimReward(uint256 id) public nonReentrant {
        uint256 toClaim = getMinterClaimReward(id);
        uint256 _tokenId = partnerNFT.getDnsOfNFT(id);
        ...
        emit PartnerClaimed(id, _tokenId, toClaim, auction.label, auction.tld , sendTo);
    }

    function MinterClaimRewardMultiple(uint256[] memory ids) public {
        for(uint256 i = 0 ; i < ids.length ; i++){
            MinterClaimReward(ids[i]);
        }
    }
