...
// ------------------------------------------
// -- Powered By SBG - https://sbgworld.io --
// ------------------------------------------
...
contract EBLivePremios is ERC721 {
    ...
    function MintUrl(string memory _Url) public onlyCreator whenNotPaused  {
        _safeMint(_Creator, COUNTER);
        SetTokenURI(COUNTER, _Url);
        COUNTER++;
    }

    function SafeMint(
        address to
    ) public onlyCreator whenNotPaused returns (uint256) {
    ...
    modifier onlyCreator() {
        require(
            (_Creator == _msgSender() || _Admin == _msgSender()),
            "Ownable: caller is not the Creator or Admin"
        );
        _;
    }
