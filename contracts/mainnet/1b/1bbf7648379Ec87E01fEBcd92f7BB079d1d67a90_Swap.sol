...
contract Swap is BaseContract {
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    if (_autoStake) {
      if(lsd.isQualifiedForTaxDiscount(msg.sender) && menToken.getLsdDiscountTaxPercentages(IMENToken.TaxType.Buy) > 0) {
        taxAmount = taxAmount * (DECIMAL3 - menToken.getLsdDiscountTaxPercentages(IMENToken.TaxType.Buy)) / DECIMAL3;
