...
,ERC721Fees
{
   ...
   function _createAuction(uint256 _tokenId, uint256 _minValue,uint _auctionTime) internal virtual {
...
contract METAHIPER is Market {

    uint256 _BOX_Open_Price = 0.01 ether; 

    address payable _OpenBoxOwner; 
    ...
    event ImmediatelySell_Auction(address indexed owner, uint256 price, uint256 nftID);
    ...
    constructor() ERC721("METAHIPER", "MTH") {
    ...
    function mInt_Multi(address _toAddress, string[] memory _tokenURI, bool _isReveal) public returns (uint256[] memory){   
        ...
        uint256[] memory _listTokenIds = new uint256[](_Count); 
    ...
    function createAuction(uint256 _tokenId, uint256 _minValue,uint _auctionTime)  public virtual {
      require(_ownerOf(_tokenId) == msg.sender);
      require(hasAuction(_tokenId) == false);
    ...
    function immediatelySell_Auction(uint256 _tokenId, uint256 _price) public  {     
        require(_ownerOf(_tokenId) == msg.sender);
        require(hasAuction(_tokenId) == false);
        require(listedMap[_tokenId] == false); 
        price[_tokenId] = _price;
        listedMap[_tokenId] = true;      
        emit ImmediatelySell_Auction(msg.sender, _price, _tokenId);  
    }
   ...
   function SetReveal(uint256 _tokenId , string memory _tokenURI) external {    
       require(msg.sender == _Administrator || msg.sender == _owner);  
       require(reveal[_tokenId] == false,  "Already reveal....." ); 
       reveal[_tokenId] = true;  
       _setTokenURI(_tokenId, _tokenURI);   
   }
   ...
   function OpenBox()  external payable returns (uint256){   
       ...
       require(_nCount > 0, "Not enough quantity."); 
       ...
       uint256[] memory _listTokenIds = new uint256[](_ableCount); 
   ...
   function GetOpenBoxPrice() public view returns(uint256){
       return _BOX_Open_Price;
   }
}