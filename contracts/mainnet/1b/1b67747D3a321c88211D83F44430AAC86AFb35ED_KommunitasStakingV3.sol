  ...
  IKommunitasStakingV3
{
  ...
  uint256 public minPrivatePartner;
  uint256 public minGetKomV; // min kom staked to receive komVToken
  uint256 public minLockIndexGetGiveaway; // min lock index to be choosen to join free token giveaway
  uint256 public apy_d2;
  ...
  function init(
    ...
    address _komVToken,
    uint256 _apy_d2,
    // uint256[] calldata _lockPeriodInDays,
    uint256[] calldata _lockPeriodInSeconds,
    uint256[] calldata _multiplier,
    uint256[] calldata _feeInPercent_d2
    ...
    require(
      _lockPeriodInSeconds.length == _multiplier.length &&
      _lockPeriodInSeconds.length == _feeInPercent_d2.length,
      "misslength"
    );
    ...
    apy_d2 = _apy_d2;
    lockNumber = _lockPeriodInSeconds.length;

    uint256 i = 0;
    do {
      lock[i] = Lock({
        // lockPeriodInSeconds: _lockPeriodInDays[i] * 86400,
        lockPeriodInSeconds: _lockPeriodInSeconds[i],
      ...
      ++i;
    } while(i < _lockPeriodInSeconds.length);

    minStaking = 100 * 1e8; // 100 komToken
    minPrivatePartner = 500 * 1e8; // 500 komToken
    minGetKomV = 3000 * 1e8; // 3000 komToken
    minLockIndexGetGiveaway = _lockPeriodInSeconds.length - 1; // last lock index
  }

  function _authorizeUpgrade(address newImplementation) internal virtual override proxied {}

  function totalPendingReward() external view returns(uint256 total){
    for(uint256 i=0; i<lockNumber; ++i){
      total += lock[i].pendingReward;
    }
  }

  function totalKomStaked() external view returns(uint256 total){
    for(uint256 i=0; i<lockNumber; ++i){
      total += lock[i].komStaked;
    }
  }

  function stakerLength() external view returns(uint256 length){
    length = staker.length;
  }

  function locked(uint256 _lockIndex) external view returns(
    // uint256 lockPeriodInDays,
    uint256 lockPeriodInSeconds,
    uint256 multiplier,
    uint256 feeInPercent_d2,
    ...
    lockPeriodInSeconds = lock[_lockIndex].lockPeriodInSeconds;
    multiplier = lock[_lockIndex].multiplier;
  ...
  function userStakedLength(address _staker) external view returns(uint256 length){
    length = staked[_staker].length;
  }

  function getStakedDetail(
    address _staker,
    uint256 _userStakedIndex
  ...
  function getTotalWithdrawableTokens(address _staker) external view returns (uint256 withdrawableTokens) {
    for(uint256 i = 0; i < staked[_staker].length; ++i){
  ...
  function getTotalLockedTokens(address _staker) external view returns (uint256 lockedTokens) {
    for (uint256 i = 0; i < staked[_staker].length; ++i) {
      if (staked[_staker][i].endedAt >= block.timestamp) {
  ...
  function getUserNextUnlock(address _staker) external view returns (
  ...
  function getTotalStakedAmountBeforeDate(uint256 _beforeAt) external view returns(uint256 totalStaked){
    for(uint256 i=0; i<staker.length; ++i){
      for(uint256 j=0; j<staked[staker[i]].length; ++j){
  ...
  function calculateReward(
    ...
    uint256 effectiveAPY = (lockDetail.multiplier * apy_d2 * lockDetail.lockPeriodInSeconds * 1e10) / (yearInSeconds * 10); 
    reward = (effectiveAPY * _amount) / 1e14;
  }
  ...
  function stake(
    ...
    if(staked[sender].length == 0){
      staker.push(sender);
      stakerIndex[sender] = staker.length - 1;
    }
  ...
  function unstake(
    ...
    if(block.timestamp > stakeDetail.endedAt){
      ...
      if(stakeDetail.amount > _amount){
        ...
        if(staked[_staker][_userStakedIndex].amount >= minPrivatePartner) privatePartnerStakedAmount -= remainderAmount;
  ...
  function _stake(
    ...
    staked[_sender].push(Stake({
      ...
      userStakedIndex: staked[_sender].length,
      compoundType: _compoundType,
      amount: _amount,
      reward: reward,
      stakedAt: block.timestamp,
      endedAt: block.timestamp + lock[_lockIndex].lockPeriodInSeconds
    }));
    ...
    emit Staked(
      ...
      reward,
      block.timestamp,
      block.timestamp + lock[_lockIndex].lockPeriodInSeconds
    );
  }
  ...
  function _unstake(
    ...
    if(staked[_sender].length == 0){
      uint256 indexToDelete = stakerIndex[_sender];
  ...
  function toggleTrustedForwarder(address _forwarder) external onlyOwner {
    isTrustedForwarder[_forwarder] = !isTrustedForwarder[_forwarder];
  }

  function setMin(
    uint256 _minStaking,
    uint256 _minPrivatePartner,
    uint256 _minGetKomV,
    uint256 _minLockIndexGetGiveaway
  ) external onlyOwner {
  ...
  function setPeriodInSeconds(
    uint256 _lockIndex,
    uint256 _newLockPeriodInDays
  ) external onlyOwner {
    require(lockNumber > _lockIndex, "bad");
    // lock[_lockIndex].lockPeriodInSeconds = _newLockPeriodInDays * 86400;
    lock[_lockIndex].lockPeriodInSeconds = _newLockPeriodInDays;
  }

  function setPenaltyFee(
    uint256 _lockIndex,
    uint256 _feeInPercent_d2
  ) external onlyOwner {
    require(lockNumber > _lockIndex, "bad");
    lock[_lockIndex].feeInPercent_d2 = _feeInPercent_d2;
  }

  function setAPY(uint256 _apy_d2) external onlyOwner {
    require(apy_d2 != _apy_d2, "bad");
    apy_d2 = _apy_d2;
  }
