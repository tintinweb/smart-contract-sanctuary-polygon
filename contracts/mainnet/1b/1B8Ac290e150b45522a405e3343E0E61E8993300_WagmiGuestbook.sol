...
contract WagmiGuestbook is ERC721Enumerable {
  struct Gm {
    uint256 timestamp;
    address sender;
  }
  ...
  Gm[] public gms;
  mapping(address => Gm[]) public gmsByAddress;
  mapping(address => uint256) public gmsByAddressCount;
  mapping(address => uint256) public lastGmTimestamp;

  event GmSent(Gm, uint256 tip);

  constructor() ERC721('Guestbook by wagmi', 'GM') {}

  function gm() external payable {
    require(
      balanceOf(msg.sender) == 0 ||
        block.timestamp - lastGmTimestamp[msg.sender] > 86400,
    ...
    gms.push(gm_);
    gmsByAddress[msg.sender].push(gm_);
    gmsByAddressCount[msg.sender]++;
    lastGmTimestamp[msg.sender] = block.timestamp;

    emit GmSent(gm_, msg.value);
      ...
      bytes(
        string(
          abi.encodePacked(
            ...
            toString(gmsByAddressCount[ownerOf(tokenId)]),
            '" }, { "trait_type": "lastGm", "display_type": "date", "value": ',
            toString(lastGmTimestamp[ownerOf(tokenId)]),
            ' }]}'
          )
        )
      )
