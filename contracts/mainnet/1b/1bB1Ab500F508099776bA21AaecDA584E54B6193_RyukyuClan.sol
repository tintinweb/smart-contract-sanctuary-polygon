...
contract RyukyuClan is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0.001 ether;
    uint256 public maxMints = 75;
    ...
    constructor()
        ERC721("Ryukyu Clan", "RYKU")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 75) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifzcbwzsacceojttm2hd6kflw75ba7amxdb5jaxu3wjnrq7ltttmy/", Strings.toString(tokenId), ".json")
            );
    }
