...
contract Silo is Initializable, KeeperCompatibleInterface {
  ...
  function setStrategyCategory(uint256 _strategyCategory) external onlyFactory {
      ...
      if (
        ...
        if (upkeepNeeded) {
          for (uint256 i; i < strategy.length; ) {
            upkeepNeeded = IAction(strategy[i].implementation).checkUpkeep(
      ...
      if (!upkeepNeeded) {
        ...
        for (uint256 i; i < strategy.length; ) {
          if (i == 0 || (i != 0 && deposited)) {
            upkeepNeeded = IAction(strategy[i].implementation).checkMaintain(
              strategy[i].configurationData
            );
          }
  ...
  function performUpkeep(bytes calldata performData)
    ...
    onlySiloManager
  {
    (bool keeperExit, uint256 task) = abi.decode(performData, (bool, uint256));
    if (task == 2) {
  ...
  function showActionStackValidity() external view returns (bool, bool) {
    ...
    for (uint256 i; i < strategy.length; ) {
      ...
      (tmpTeam, tmpLogic) = IAction(strategy[i].implementation).actionValid(
        strategy[i].configurationData
      );
      if (!tmpTeam) {
        team = tmpTeam;
  ...
  function setStrategy(
    ...
    for (uint256 i; i < _implementations.length; ) {
      ...
      if (storedConfig.length > 0) {
        ...
        if (
          !IAction(_implementations[i]).validateConfig(_configurationData[i])
        ) {
          string memory errorMessage = string(
            abi.encodePacked(
  ...
  function _investSilo() internal returns (uint256[4] memory amounts) {
    ...
    for (uint256 i; i < 4; ) {
      ...
      if (depositTokens[i] != address(0)) {
        ...
        if (tokenAmount > 0 && tokenAmount >= tokenMinimum[depositTokens[i]]) {
  ...
  function _runStrategy() internal {
    ...
    for (uint256 i; i < strategy.length; ) {
      ...
      if (!success) {
        string memory errorMessage = string(
          abi.encodePacked("Strategy Failed At: ", Strings.toString(i))
        );
        revert(errorMessage);
      }
      inputData = result;
  ...
  function _exitStrategy(uint256 _requestedOut) internal {
    ...
    for (uint256 i; i < strategy.length; ) {
      ...
      if (!success) {
        string memory errorMessage = string(
          abi.encodePacked("Withdraw Failed At: ", Strings.toString(i))
        );
        revert(errorMessage);
      }
      exitData = result;
