...
contract testing is Context, IERC20, Ownable {
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!(_isExcludedFromFee[from] || _isExcludedFromFee[to]) ) {            
            ...
            if(contractETHBalance > 10000000000000000) { //change to eth
    ...
    function whitelistAddress(address _addr,bool _bool) public onlyOwner{    //add or remove address from whitelist
        _isExcludedFromFee[_addr] = _bool;
    }

    receive() external payable {}
    
    function transferERC20(IERC20 token, uint256 amount) public onlyOwner{ //function to transfer stuck erc20 tokens
