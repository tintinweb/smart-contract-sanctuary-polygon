...
/**
 * @title Finder
 * @notice Provides addresses and bytecode(of some contracts) of contracts implementing certain interfaces.
 */
contract Finder is IFinder, StandardAccessControlEnumerable {
    //----------------------------------------
    // Storage
    //----------------------------------------
    /// @notice maps bytes32 encoding of the Contract name to the contract address
    mapping(bytes32 => address) public interfacesImplemented;

    /// @notice maps bytes32 encoding of the Contract name to the contract bytecode
    ...
    /**
     * @notice Updates the addresses of the contracts that implements `interfaceName`.
     * @param interfacesNames bytes32 of the interfaces names that is either changed or registered.
     * @param implementationAddresses addresses of the implementation contracts.
     */
    function changeBulkImplementationAddress(
        bytes32[] calldata interfacesNames,
        address[] calldata implementationAddresses
    ) external onlyMaintainer {
        uint arrLength = implementationAddresses.length;
        _require(
            interfacesNames.length == arrLength,
            Errors.ARRAY_LENGTH_MISMATCH
        );
        for (uint i; i < arrLength; i++) {
            interfacesImplemented[interfacesNames[i]] = implementationAddresses[
                i
            ];
            emit InterfaceImplementationChanged(
                interfacesNames[i],
