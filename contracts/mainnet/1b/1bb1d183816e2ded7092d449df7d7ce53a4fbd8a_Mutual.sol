...
contract Mutual is FDFRisk{
    ...
    function setRiskLevel(uint256 index, uint256 open, uint256 start, uint256 next, uint256 close) external {
        _setRiskLevel(index,open,start,next,close);
    }

    function initialize(address usdtAddr,address admin,
        address sys,address ins,address found, address market, uint256 timesDay) public initializer {

        FDFRisk.initialize();

        unlocked = 1;

        invitePercents = [500, 100, 200, 300, 100, 200, 200, 200, 100, 100];
        levelMaxDeposit = [500e6,1000e6,1500e6,2000e6,2500e6];
        levelMinDeposit = [100e6,500e6,1000e6,1500e6,2500e6];
        levelTeam = [0, 20, 40, 60, 120];
        levelInvite = [0, 5000e6, 10_000e6, 15_000e6, 50_000e6];
    ...
    function register(address ref_) external{
        ...
        require(rewardInfo[ref_].freezeCapitals >0,"ref freezeCapitals is zero");
...
contract FDFRisk is Initializable{
    ...
    function _setRiskLevel(uint256 index, uint256 open, uint256 start, uint256 next, uint256 close) internal {
        require(index < riskLevels.length, "index out of range");
        riskLevels[index].open = open;
        riskLevels[index].start = start;
        riskLevels[index].next = next;
        riskLevels[index].close = close;
    }
