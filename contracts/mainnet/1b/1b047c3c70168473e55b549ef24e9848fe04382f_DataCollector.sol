...
contract DataCollector is DefaultAccessControl {
    ...
    constructor(address _usdc, IUniswapV3Factory _factory, address owner) DefaultAccessControl(owner) {
        usdc = _usdc;
        factory = _factory;
    }
    ...
    function updatePoolFee(address token, uint24 fee) external {
        _requireAdmin();
        poolFee[token] = fee;
    }
    ...
    function collect(Request[] memory requests) external view returns (Response[] memory pulseVaultsResponses) {
        ...
        for (uint32 index = 0; index < requests.length; ++index) {
            ...
            for (uint32 i = 0; i < pulseVaultsResponses[index].tokens.length; i++) {
                ...
                if (token == usdc) {
                    pulseVaultsResponses[index].priceToUSDCX96[i] = Q96;
                } else {
                    IUniswapV3Pool pool = IUniswapV3Pool(factory.getPool(usdc, token, poolFee[token]));
                    ...
                    pulseVaultsResponses[index].priceToUSDCX96[i] = priceX96;
                }
            }
        }
    }
