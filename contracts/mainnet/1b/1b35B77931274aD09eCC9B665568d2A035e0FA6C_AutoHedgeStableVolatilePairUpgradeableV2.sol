	...
	UniswapV2ERC20Upgradeable
{
	...
	function withdraw(uint256 liquidity, address to)
		external
		override
		nonReentrant
		returns (uint256 amountStableToUser)
	{
		...
		uint256 code = _tokens.cUniLp.redeemUnderlying(amountUniLp);
		require(
			code == 0,
	...
	IAutoHedgeStableVolatilePairUpgradeableV2Event
{
	...
	function depositBatch(
		...
		address referrer,
		bool getVolFromUser
	) external returns (uint256 amountAhLp);
	 ...
	 */
	function withdraw(uint256 liquidity, address to) external returns (uint256);
