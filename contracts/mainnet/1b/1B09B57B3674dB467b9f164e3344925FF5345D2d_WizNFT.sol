...
contract WizNFT is Ownable, ERC721A {
    ...
    function mint(bytes calldata proof) public payable {
        ...
        if (_msgSender() != owner()) {
            require(
                isValidProof(_msgSender(), proof),
                "User has no valid proof"
            );
            require(isPublicSale, "Sale has not started");
            require(
                mintedPerAddress[_msgSender()] + 1 <= maxPerWallet,
                "Cannot exceed max mint per wallet"
            );
    ...
    function mintFromAltar(address a, uint256 quantity) public onlyAltars {
    ...
    function isValidProof(address a, bytes memory proof)
        internal
        returns (bool)
    {
