...
contract Rewarder {
    ...
    function handleReward(uint256 claimedEpoch,
        address factory,
        ...
        if(rebaseRate != 0) {
            ...
            rewardReferrers(handler, factory, balanceToMint, rightUpTaxRate, protocolTaxRate, taxDivisor);
        }
    }

    function handleSelfTax(address handler, address factory, uint256 balance, uint256 protocolTaxRate, uint256 divisor) internal {
        address owner = IReferralHandler(handler).ownedBy();
        ITaxManager taxManager =  getTaxManager(factory);
    ...
    function handleRightUpTax(address handler,address factory, uint256 balance, uint256 taxRate, uint256 protocolTaxRate,  uint256 divisor) internal {
        ...
        IReferralHandler(handler).mintForRewarder(taxManager.getRightUpTaxPool(), protocolTaxed);
    }
    function rewardReferrers(address handler, address factory, uint256 balanceDuringRebase, uint256 rightUpTaxRate, uint256 protocolTaxRate, uint256 taxDivisor) internal {
        ...
        IReferralHandler(handler).mintForRewarder(maintenancePool, protocolMaintenanceAmount);
        ...
        if(referral[1] != address(0)) {
            ...
            IReferralHandler(handler).mintForRewarder(referral[1], firstReward);
            }
            ...
            if(referral[2] != address(0)) {
                ...
                IReferralHandler(handler).mintForRewarder(referral[2], secondReward);
                }
                ...
                // Block Scoping to reduce local Variables spillage
                    ...
                    IReferralHandler(handler).mintForRewarder(referral[3], thirdReward);
                    }
                    ...
                    if(referral[4] != address(0)) {
                        ...
                        IReferralHandler(handler).mintForRewarder(referral[4], fourthReward);
                        }
                    }
                }
            }
        ...
        IReferralHandler(handler).mintForRewarder(rewardPool, rewardPoolAmount);
        ...
        IReferralHandler(handler).mintForRewarder(devPool, leftOverTax.div(2));
        IReferralHandler(handler).mintForRewarder(revenuePool, leftOverTax.div(2));
        }
    }
}