...
interface IveFXS {
    ...
    function user_point_history__ts(address _addr, uint256 _idx) external  returns (uint256);
    ...
    function totalFXSSupply() external  returns (uint256);
    function totalFXSSupplyAt(uint256 _block) external  returns (uint256);
    function changeController(address _newController) external;
    function token() external  returns (address);
    function supply() external  returns (uint256);
    function locked(address addr) external  returns (LockedBalance memory);
    function epoch() external  returns (uint256);
    function point_history(uint256 arg0) external  returns (int128 bias, int128 slope, uint256 ts, uint256 blk, uint256 fxs_amt);
    function user_point_history(address arg0, uint256 arg1) external  returns (int128 bias, int128 slope, uint256 ts, uint256 blk, uint256 fxs_amt);
    function user_point_epoch(address arg0) external  returns (uint256);
    function slope_changes(uint256 arg0) external  returns (int128);
    function controller() external  returns (address);
    function transfersEnabled() external  returns (bool);
    function emergencyUnlockActive() external  returns (bool);
    function name() external  returns (string memory);
    function symbol() external  returns (string memory);
    function version() external  returns (string memory);
    function decimals() external  returns (uint256);
    function future_smart_wallet_checker() external  returns (address);
...
interface IFraxGaugeController {
    ...
    function n_gauges() external  returns (int128);
    function gauge_type_names(int128) external  returns (string memory);
    function gauges(uint256) external  returns (address);
    ...
    function last_user_vote(address, address) external  returns (uint256);
    function points_weight(address, uint256)
        external
        
        returns (Point memory);
    function time_weight(address) external  returns (uint256);
    function points_sum(int128, uint256) external  returns (Point memory);
    function time_sum(uint256) external  returns (uint256);
    function points_total(uint256) external  returns (uint256);
    function time_total() external  returns (uint256);
    function points_type_weight(int128, uint256)
        external
        
        returns (uint256);
    function time_type_weight(uint256) external  returns (uint256);

    // Getter functions
    function gauge_types(address) external  returns (int128);
    function gauge_relative_weight(address) external  returns (uint256);
    function gauge_relative_weight(address, uint256) external  returns (uint256);
    function get_gauge_weight(address) external  returns (uint256);
    function get_type_weight(int128) external  returns (uint256);
    function get_total_weight() external  returns (uint256);
    function get_weights_sum_per_type(int128) external  returns (uint256);
    ...
    function global_emission_rate() external  returns (uint256);
...
interface IFraxGaugeFXSRewardsDistributor {
  ...
  function curator_address() external  returns(address);
  function currentReward(address gauge_address) external  returns(uint256 reward_amount);
  function distributeReward(address gauge_address) external returns(uint256 weeks_elapsed, uint256 reward_tally);
  function distributionsOn() external  returns(bool);
  function gauge_whitelist(address) external  returns(bool);
  function is_middleman(address) external  returns(bool);
  function last_time_gauge_paid(address) external  returns(uint256);
  function nominateNewOwner(address _owner) external;
  function nominatedOwner() external  returns(address);
...
library Address {
     ...
     * _Available since v3.3._
     */
    function functionStaticCall(address target, bytes memory data, string memory errorMessage) internal  returns (bytes memory) {
...
contract FraxUnifiedFarmTemplate is Owned, ReentrancyGuard {
    ...
    function isTokenManagerFor(address caller_addr, address reward_token_addr) public  returns (bool){
    ...
    function lastTimeRewardApplicable() internal  returns (uint256) {
        return Math.min(block.timestamp, periodFinish);
    }

    function rewardRates(uint256 token_idx) public  returns (uint256 rwd_rate) {
    ...
    function rewardsPerToken() public  returns (uint256[] memory newRewardsPerTokenStored) {
    ...
    function earned(address account) public  returns (uint256[] memory new_earned) {
    ...
    function getRewardForDuration() external  returns (uint256[] memory rewards_per_duration_arr) {
    ...
    function totalLiquidityLocked() external  returns (uint256) {
        return _total_liquidity_locked;
    }

    // Total locked liquidity / LP tokens
    function lockedLiquidityOf(address account) external  returns (uint256) {
        return _locked_liquidity[account];
    }

    // Total combined weight
    function totalCombinedWeight() external  returns (uint256) {
        return _total_combined_weight;
    }

    // Total 'balance' used for calculating the percent of the pool the account owns
    // Takes into account the locked stake time multiplier and veFXS multiplier
    function combinedWeightOf(address account) external  returns (uint256) {
        return _combined_weights[account];
    }

    // Calculated the combined weight for an account
    function calcCurCombinedWeight(address account) public virtual  
        returns (
    ...
    function lockMultiplier(uint256 secs) public  returns (uint256) {
        return Math.min(
            lock_max_multiplier,
    ...
    function userStakedFrax(address account) public  returns (uint256) {
        return (fraxPerLPStored * _locked_liquidity[account]) / MULTIPLIER_PRECISION;
    }

    function proxyStakedFrax(address proxy_address) public  returns (uint256) {
    ...
    function maxLPForMaxBoost(address account) external  returns (uint256) {
    ...
    function minVeFXSForMaxBoost(address account) public  returns (uint256) {
        return (userStakedFrax(account) * vefxs_per_frax_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function minVeFXSForMaxBoostProxy(address proxy_address) public  returns (uint256) {
        return (proxyStakedFrax(proxy_address) * vefxs_per_frax_for_max_boost) / MULTIPLIER_PRECISION;
    }

    function getProxyFor(address addr) public  returns (address){
    ...
    function veFXSMultiplier(address account) public  returns (uint256 vefxs_multiplier) {
...
interface IScaledBalanceToken {
   ...
   **/
  function getScaledUserBalanceAndSupply(address user) external  returns (uint256, uint256);
...
interface ILendingPool {
  ...
  // function getConfiguration ( address asset ) external  returns ( tuple );
  // function getReserveData ( address asset ) external  returns ( tuple );
  ...
  // function getUserConfiguration ( address user ) external  returns ( tuple );
...
contract FraxUnifiedFarm_PosRebase is FraxUnifiedFarmTemplate {
    ...
    function fraxPerLPToken() public  override returns (uint256) {
    ...
    function calcCurCombinedWeight(address account) public override 
        returns (
            uint256 old_combined_weight,
            uint256 new_vefxs_multiplier,
            uint256 new_combined_weight
    ...
    function currLiqIdx() public  returns (uint256) {
    ...
    function lockedStakesOf(address account) external  returns (LockedStake[] memory) {
        return lockedStakes[account];
    }

    // Returns the length of the locked stakes for a given account
    function lockedStakesOfLength(address account) external  returns (uint256) {
        return lockedStakes[account].length;
    }

    // // All the locked stakes for a given account [old-school method]
    // function lockedStakesOfMultiArr(address account) external  returns (
    ...
    function _getStake(address staker_address, bytes32 kek_id) internal  returns (LockedStake memory locked_stake, uint256 arr_idx) {
