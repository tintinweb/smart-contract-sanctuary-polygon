...
contract BeefyQI is ERC20, ReentrancyGuard, QiManager {
    ...
    function withdraw(uint256 _amount) public {
        require(_amount <= balanceOfWant(), "Not enough QI");
    ...
    function harvestAndDepositQI() public { 
        if (totalQI() > 0) {
            if (balanceOfWant() - outstandingReward() > requiredReserve()) {
                ...
                eQI.enter(availableBalance, block.timestamp + MAX_LOCK);
            } else {
                eQI.enter(0, block.timestamp + MAX_LOCK);
            }
        }
    ...
    function qiPower() public view returns (uint256) {
    ...
    function lockInfo() external view returns (uint256 endBlock, uint256 blocksRemaining) {
        (, endBlock) = eQI.userInfo(address(this));
        blocksRemaining = endBlock > block.timestamp ? endBlock - block.timestamp : 0;
    }
...
contract QiManager is Ownable, Pausable, IERC1271 {
    using SafeERC20 for IERC20;
   ...
   constructor(
    ...
    /**
    * Will give us an opportunity to vote via snapshot
    */
