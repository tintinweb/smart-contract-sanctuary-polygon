...
import { RouterStorage } from "../libraries/RouterStorage.sol";
import { LibSrcCrossSwap } from "../libraries/LibSrcCrossSwap.sol";
...
contract SrcCrossSwapFacet {
     ...
     */
    function initCrossSwap(LibSrcCrossSwap.CrossSwapArgs calldata crossSwapArgs, LibSrcCrossSwap.SrcSwapArgs calldata srcSwapArgs)
        external
        payable
    {
        ...
        if (crossSwapArgs.connectorToken == crossSwapArgs.srcToken) {
            IERC20(crossSwapArgs.srcToken).safeTransferFrom(msg.sender, s.connectorTokenHolder[block.chainid], crossSwapArgs.srcAmount);
            ...
            LibSrcCrossSwap._executeSwap(crossSwapArgs.srcToken, crossSwapArgs.srcAmount, srcSwapArgs);
        ...
        emit LibSrcCrossSwap.SrcCrossSwapV2(
            msg.sender,
            ...
            // emit current nonce and then increase
            s.nonce++,
            "bitoftrade"
        );
    }
...
library LibSrcCrossSwap {
     ...
     * @param swapArgs.value amount of the native token to be used in swap
     * @param swapArgs.callData encoded calldata of the swap
     * @param swapArgs.provider swap provider
     * @param swapArgs.provider swap provider approve proxy
     */
    function _executeSwap(
        address srcToken,
        uint256 srcAmount,
        SrcSwapArgs calldata swapArgs
    ) internal {
        if (srcToken != NATIVE_TOKEN) {
            ...
            IERC20(srcToken).approve(swapArgs.approveProxy, srcAmount);
        }
...
struct RouterStorage {
    ...
    // TODO: remove deprecated multichain before prod release
