...
// File: contracts/CollectionRoles.sol
...
contract Collection is ERC721Enumerable {
    ...
    function grantMinterRole(address account) public onlyAdmin {
        if (!minters[account]) {
            ...
            minterAddresses.push(account);
            emit RoleGranted(account); // Emitir el evento RoleGranted
        }
    }
    ...
    function transferOwnership(address newOwner) public onlyAdmin {
        require(newOwner != address(0), "Invalid new owner");
        minters[newOwner] = true; // Agregar el nuevo owner como minter
        admin = newOwner; // Actualizar la dirección del administrador
    }

    function getMinterAddresses() public view returns (address[] memory) {
...
interface ICollectionFactory {
    ...
    function createCollection(string memory name, string memory image, string memory symbol) external returns (address);
    function getCollectionAddresses() external view returns (address[] memory);
    function getCollectionData(address collectionAddress) external view returns (CollectionData memory);
    function getAllCollectionData() external view returns (CollectionData[] memory);
    function getCollectionCreatorAddresses(uint256 startIndex, uint256 pageSize) external view returns (address[] memory);
}

contract CollectionFactory is ICollectionFactory {
    CollectionRoles public collectionRoles;

    mapping(address => CollectionData) private collectionDataMap;
    address[] private collections;

    event CollectionCreated(address indexed collectionAddress, address indexed owner, string name, string image, string symbol);

    constructor(address collectionRolesAddress) {
        collectionRoles = CollectionRoles(collectionRolesAddress);
    }

    function createCollection(string memory name, string memory image, string memory symbol) public override returns (address) {
        require(collectionRoles.collectionCreators(msg.sender), "Caller is not a collection creator");

        Collection newCollection = new Collection(name, image, symbol, msg.sender);
        collections.push(address(newCollection));
        collectionDataMap[address(newCollection)] = CollectionData(address(newCollection), msg.sender, name, image, symbol);

        emit CollectionCreated(address(newCollection), msg.sender, name, image, symbol);

        return address(newCollection);
    }

    function getCollectionAddresses() public view override returns (address[] memory) {
        return collections;
    }

    function getCollectionData(address collectionAddress) public view override returns (CollectionData memory) {
        return collectionDataMap[collectionAddress];
    }

    function getAllCollectionData() public view override returns (CollectionData[] memory) {
        CollectionData[] memory allCollectionData = new CollectionData[](collections.length);
        for (uint256 i = 0; i < collections.length; i++) {
            address collectionAddress = collections[i];
            allCollectionData[i] = getCollectionData(collectionAddress);
        }
        return allCollectionData;
    }

    function getCollectionCreatorAddresses(uint256 startIndex, uint256 pageSize) public view override returns (address[] memory) {
    return collectionRoles.getCollectionCreatorAddresses(startIndex, pageSize);
    }
}