...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    function _safeMintBatch(address to, uint256 quantity) internal virtual {
        require(to != address(0), "ERC721: mint to the zero address");
		uint256 startTokenId = totalSupply();
		require(!_exists(startTokenId), "ERC721: token already minted");
		
        for (uint256 i = 0; i < quantity; i++) {
           _beforeTokenTransfer(address(0), to, startTokenId + i);
           _holderTokens[to].add(startTokenId + i);
           _tokenOwners.set(startTokenId + i, to);
...
/**
 * @title Shadow AzukiApe contract
 * @dev Extends ERC721 Non-Fungible Token Standard basic implementation
 */
contract ShadowAzukiApe is ERC721, Ownable {
    using SafeMath for uint256;

    string public SSAPE_PROVENANCE = "";
    ...
    constructor(string memory name, string memory symbol, uint256 maxNftSupply) ERC721(name, symbol) {
        ...
        REVEAL_TIMESTAMP = block.number + (86400 * 9);
    }
    ...
    function reserveApes() public onlyOwner {        
        _safeMintBatch(msg.sender, 30);
    }
	
    function reserveSSApes(uint numberOfTokens) public onlyOwner {        
		_safeMintBatch(msg.sender, numberOfTokens);
    }

    function reserveSSApesTo(address _To, uint numberOfTokens) public onlyOwner {        
		_safeMintBatch(_To, numberOfTokens);
    }
	
    function reserveInternalTo(address _To, uint numberOfTokens) internal {        
        _safeMintBatch(_To, numberOfTokens);
    }
	
    function reserveSSApesBatch(address[] memory _deliveryAddrs, uint numberOfTokens) public onlyOwner {
        uint _addressAmount = _deliveryAddrs.length;
        for(uint i = 0; i < _addressAmount; i++) {
			_safeMintBatch(_deliveryAddrs[i], numberOfTokens);
        }
    }
    ...
    function setProvenanceHash(string memory provenanceHash) public onlyOwner {
        SSAPE_PROVENANCE = provenanceHash;
    }
