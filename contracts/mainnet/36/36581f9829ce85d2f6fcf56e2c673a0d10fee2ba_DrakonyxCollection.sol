    ...
    OwnableUpgradeable
{
   ...
   /**
    ...
    function upgradeCards(
        ...
        require(msg.value >= upgradecost.payValue, "not e-gh");
     ...
     */
     function forceRemoveCommit(address _address) external onlyRole(SUPPORT) {
        if (
            !commits[_address].revealed &&
            commits[_address].amount > 0
        ) {
            if(commits[_address].cost == 0){
                freeMintCount += commits[_address].amount;
            }else{
                gametreasury.refundCommit(_address, commits[_address].cost);
            }
            commits[_address].revealed = true;
            commits[_address].amount = 0;
            commits[_address].cost = 0;
            commits[_address].partnerFee = 0;
            commits[_address].collectionMintFee = 0;
        }
    }
