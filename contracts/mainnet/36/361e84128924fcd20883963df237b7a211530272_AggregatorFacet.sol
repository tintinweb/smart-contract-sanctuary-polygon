...
import {AppStorage, DataTransferType} from "../../libraries/LibMagpieAggregator.sol";
...
import {DataTransferInArgs, DataTransferInProtocol, DataTransferOutArgs, TransferKey} from "../data-transfer/LibCommon.sol";
...
library LibAggregator {
    ...
    function swapIn(SwapInArgs memory swapInArgs) internal returns (uint256 amountOut) {
        ...
        TransferKey memory transferKey = dataTransferIn(
    ...
    function bridgeIn(BridgeInArgs memory bridgeInArgs) internal returns (uint64) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        bytes4 selector = hex"cc4f18b4";
        ...
        (bool success, bytes memory data) = address(facet).delegatecall(bridgeInCall);
        if (!success) {
    ...
    function dataTransferIn(DataTransferInArgs memory dataTransferInArgs) internal returns (TransferKey memory) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        bytes4 selector = hex"b9031d91";
        address facet = ds.selectorToFacetAndPosition[selector].facetAddress;
        bytes memory dataTransferInCall = abi.encodeWithSelector(selector, dataTransferInArgs);
        (bool success, bytes memory data) = address(facet).delegatecall(dataTransferInCall);
        if (!success) {
            revert AggregatorDataTransferInCallFailed();
        }

        return abi.decode(data, (TransferKey));
    }
