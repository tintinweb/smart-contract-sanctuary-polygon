...
contract Multisender {
    ...
    modifier ERC20Helper(address[] memory recipients, uint256 amount, address tokenAddress) {
        require(recipients.length <= 50, "Too many inputs");
        require(amount > 0);
        require(recipients.length > 0, "Can't airdrop tokens to 0 recipients");
    ...
    function batchSendERC20(address[] memory recipients, uint256 amount, address tokenAddress) 
        ERC20Helper(recipients, amount, tokenAddress) public {
            IERC20 token = IERC20(tokenAddress);
    ...
    modifier nftHelper(address tokenAddress, address[] memory recipients, uint256[] memory tokenIds) {
        require(recipients.length <= 50, "Too many inputs");
        require(recipients.length > 0, "Can't airdrop tokens to 0 recipients");
    ...
    function batchSendNFT(address tokenAddress, address[] memory recipients, uint256[] memory tokenIds)
        nftHelper(tokenAddress, recipients, tokenIds) public {
