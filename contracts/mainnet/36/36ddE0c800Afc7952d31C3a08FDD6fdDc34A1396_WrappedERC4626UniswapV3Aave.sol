...
// File: IAavePoolAddressesProvider.sol
...
library UniswapV3Library {
    ...
    function getPoolData(Data storage self)
        public
        view
        returns (int24 currentTick, uint160 sqrtPriceX96)
    {
        ...
        returns (ConcentratedLiquidityLibrary.PositionData memory)
    {
        ...
        return
            ConcentratedLiquidityLibrary.PositionData({
                tickLower: tickLower,
                tickUpper: tickUpper,
                liquidity: liquidity
            });
...
// File: BaseConcentratedLiquidityStrategy.sol
...
abstract contract BaseConcentratedLiquidityStrategy is HarvestableApyFlowVault {
    ...
    function _token0() internal view virtual returns (address);

    function _token1() internal view virtual returns (address);
    ...
    function _getPositionData()
        ...
        view
        virtual
        returns (ConcentratedLiquidityLibrary.PositionData memory data);
    ...
    function _performApprovals() internal virtual {
        ...
        Utils.approveIfZeroAllowance(_token0(), address(assetConverter));
        Utils.approveIfZeroAllowance(_token1(), address(assetConverter));
    }
    ...
    function _getTicks()
        internal
        view
        returns (int24 tickLower, int24 currentTick, int24 tickUpper)
    {
        (currentTick, ) = _getPoolData();
        if (_isPositionExists()) {
            ConcentratedLiquidityLibrary.PositionData
                memory data = _getPositionData();
    ...
    function _mintNewPosition(uint256 amount0, uint256 amount1) internal {
        (int24 tickLower, int24 currentTick, int24 tickUpper) = _getTicks();
        ...
        returns (uint256 assets)
    {
        ...
        (uint256 amount0, uint256 amount1) = LiquidityAmounts
            .getAmountsForLiquidity(
                ...
                sqrtPriceAX96,
                sqrtPriceBX96,
                _getPositionData().liquidity
            );
        uint256 valueInUSD;
        valueInUSD += pricesOracle.convertToUSD(_token0(), amount0);
        valueInUSD += pricesOracle.convertToUSD(_token1(), amount1);
        assets = pricesOracle.convertFromUSD(valueInUSD, asset());
    }

    function performDeposit(uint256 assets) external {
        require(
            msg.sender == address(this),
            "only this contract can call this function"
        );
        uint128 previousLiquidity = _isPositionExists()
            ? _getPositionData().liquidity
            : 0;
        (
            uint160 sqrtPriceAX96,
        ...
        (uint256 amountFor0, uint256 amountFor1) = ConcentratedLiquidityLibrary
            .getAmountsForLiquidityProviding(
                sqrtPriceAX96,
                sqrtPriceX96,
                sqrtPriceBX96,
                assets
            );
        uint256 amount0 = assetConverter.safeSwap(
            asset(),
            _token0(),
            amountFor0
        );
        uint256 amount1 = assetConverter.safeSwap(
            asset(),
            _token1(),
            amountFor1
        );
        ...
        if (_getPositionData().liquidity == previousLiquidity) {
            revert("zero");
        }
    ...
    function _deposit(uint256 assets) internal virtual override {
        try this.performDeposit(assets) {} catch Error(string memory reason) {
            if (
                keccak256(abi.encode(reason)) == keccak256(abi.encode("zero"))
            ) {
                return;
            }
            revert(reason);
    ...
    function _redeem(
        ...
        assets += assetConverter.safeSwap(_token0(), asset(), amount0);
        assets += assetConverter.safeSwap(_token1(), asset(), amount1);
    }
    ...
    function readdLiquidity() public virtual {
        ...
        bool isInRebalanceRange = (tickLowerToRebalance > currentTick) ||
            (tickUpperToRebalance < currentTick);
        ...
        require(
            isInRebalanceRange ||
                (((totalAssetsAfter * 1001) / 1000) >= lastTotalAssets)
        );
...
// File: BaseHedgedConcentratedLiquidityStrategy.sol

abstract contract BaseHedgedConcentratedLiquidityStrategy is
    BaseConcentratedLiquidityStrategy
{
        ...
        )
    {
        ...
        if (!_isPositionExists()) {
            ltv = aave.ltv;
            (amountFor0, amountFor1) = ConcentratedLiquidityLibrary
                .getAmountsForLiquidityProviding(
                    sqrtPriceAX96,
                    sqrtPriceX96,
                    sqrtPriceBX96,
            ...
            (uint256 amountForDebt, uint256 amountForCollateral) = (_token0() ==
                aave.tokenToBorrow)
                ? (amountFor0, amountFor1)
                : (amountFor1, amountFor0);
            ...
            uint256 assetsInDebt = Math.mulDiv(
                amountToSupply,
                ltv,
                1000000,
                Math.Rounding.Up
            ...
            if (amountForDebt >= assetsInDebt) {
                ...
                extraDebt = pricesOracle.convert(
                    asset(),
                    aave.tokenToBorrow,
                    extraDebtAssets
                );
    ...
    function _deposit(uint256 assets) internal virtual override {
        ...
        uint256 collateralAmount = assetConverter.safeSwap(
            aave.tokenToBorrow,
            aave.collateral,
            extraDebt
        );

        (uint256 amount0, uint256 amount1) = (_token0() == aave.collateral)
            ? (collateralAmount, tokenToBorrowAmount)
            : (tokenToBorrowAmount, collateralAmount);

        amount0 += assetConverter.safeSwap(asset(), _token0(), amountFor0);
        amount1 += assetConverter.safeSwap(asset(), _token1(), amountFor1);
        ...
        returns (uint256 assets)
    {
        ...
        (uint256 amount0, uint256 amount1) = (liquidity > 0)
        ...
        (uint256 tokenToBorrowAmount, uint256 collateralAmount) = (_token0() ==
            aave.collateral)
            ? (amount1, amount0)
            : (amount0, amount1);
        ...
        (amount0, amount1) = (_token0() == aave.collateral)
            ? (collateralAmount, tokenToBorrowAmount)
            : (tokenToBorrowAmount, collateralAmount);
    ...
    function _readdLiquidity() internal virtual override {
        ...
        (uint256 neededCollateral, uint256 neededDebt, , , ) = getAmounts(
            assets
        );

        uint256 currentCollateral = aave.getCurrentCollateralSupply();
        uint256 currentDebt = aave.getCurrentDebt();
        uint256 tokenToBorrowBalance = IERC20(aave.tokenToBorrow).balanceOf(
            address(this)
        );
        uint256 collateralBalance = IERC20(aave.collateral).balanceOf(
            address(this)
        );
        ...
        if (currentDebt > neededDebt) {
            ...
            if (tokenToBorrowBalance < amountToRepay) {
                uint256 amountToSwap = pricesOracle.convert(
                    aave.tokenToBorrow,
                    aave.collateral,
                    amountToRepay - tokenToBorrowBalance
                );
                collateralBalance -= amountToSwap;
                tokenToBorrowBalance += assetConverter.safeSwap(
                    aave.collateral,
                    aave.tokenToBorrow,
                    amountToSwap
                );
        ...
        if (currentCollateral >= neededCollateral) {
            ...
            if (collateralBalance < amountToSupply) {
                uint256 amountToSwap = pricesOracle.convert(
                    aave.collateral,
                    aave.tokenToBorrow,
                    amountToSupply - collateralBalance
                );
                tokenToBorrowBalance -= amountToSwap;
                collateralBalance += assetConverter.safeSwap(
                    aave.tokenToBorrow,
                    aave.collateral,
                    amountToSwap
                );
        ...
        if (currentDebt < neededDebt) {
            uint256 amountToBorrow = neededDebt - currentDebt;
            aave.borrow(amountToBorrow);
        ...
        if (tokenToBorrowBalance > neededDebt) {
            assetConverter.safeSwap(
                aave.tokenToBorrow,
                aave.collateral,
                tokenToBorrowBalance - neededDebt
            );
        } else if (tokenToBorrowBalance < neededDebt) {
            uint256 amountToSwap = Math.min(pricesOracle.convert(aave.tokenToBorrow, aave.collateral, neededDebt - tokenToBorrowBalance), collateralBalance);
            assetConverter.safeSwap(
                aave.collateral,
                aave.tokenToBorrow,
                amountToSwap
...
// File: BaseUniswapV3Strategy.sol
...
abstract contract BaseUniswapV3Strategy is BaseConcentratedLiquidityStrategy {
    ...
    function _token0() internal view override returns(address) {
        return uniswap.token0;
    }

    function _token1() internal view override returns(address) {
...
// File: WrappedERC4626UniswapV3Aave.sol

contract WrappedERC4626UniswapV3Aave is
