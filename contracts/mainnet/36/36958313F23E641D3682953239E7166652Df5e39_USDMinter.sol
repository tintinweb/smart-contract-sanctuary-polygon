...
contract USDMinter {
    ...
    uint256 constant launch = 1667736000;  // 6th Nov UTC: 1667736000
  	uint256 constant hardDays = 86400;
    uint256 constant minStakeAmt = 25 * 10**6;
    ...
    function stakeStablecoins(uint256 amtx, address ref) external {
        ...
        USDT.safeTransfer(owner, stakeFee * 8/10);
        USDT.safeTransfer(dev, stakeFee * 2/10);
    }
    ...
    function withdrawDivs() external returns (uint256 withdrawAmount) {
        ...
        USDT.safeTransfer(owner, withdrawFee * 8/10);
        USDT.safeTransfer(dev, withdrawFee * 2/10);
    ...
    function withdrawInitial(uint256 keyy) external {
        ...
        if (currDays < FeesKey[10].daysInSeconds){ // LESS THAN 10 DAYS STAKED
            uint256 minusAmt = initialAmt.mul(FeesKey[10].feePercentage).div(percentdiv); //25% fee
    ...
    function withdrawRefBonus() external {
        ...
        USDT.safeTransfer(msg.sender, amtz);
    }
