...
contract WPNS is ERC721,ERC721Enumerable,Ownable,VRFConsumerBase {

// ################################################################################
// TEST
// ################################################################################
    function test_doCleanup() external onlyAdmin {
        // cleanup tier data
        for (uint i=0; i<weaponTiers.length; i++) {
            delete wpnTierMintIndex[i+1];
            for (uint j=1; j<=weaponTiers[i].typeCnt; j++)
                for (uint k=1; k<=weaponTiers[i].subtypeCnt; k++)
                    delete wpnTTSMintIndex[(i+1)*10000+j*100+k];
            weaponTiers.pop();
        }
        // cleanup token data
        for (uint i=1; i<=maxTokenId; i++) {
            _burn(i);
            delete tokenData[i];
            delete tokenGameExp[i];
            delete tokenTradeExp[i];
        }
        // cleanup other data
        delete maxTokenId;
    }
    function setRndNumVrf(uint256 _value) external onlyAdmin {
        _rndNumVrf = _value;
    }
// ################################################################################
    ...
    event CreateWeapon(
        ...
        uint256 innovation,
        address recipient);
    event IncreaseAttribs(uint256 tokenId, uint256 sec, uint256 ano, uint256 inn);
    ...
    event UpdateWeaponTierParameter(uint256 tierLvl, string parameterName, uint256 parameterValue);
    ...
    mapping (uint256 => uint256) public tokenGameExp;
    mapping (uint256 => uint256) public tokenTradeExp;
    ...
    uint256 public maxTokenId; // auto-incremental

    address public admin;
    address public burner;
    address public superAdmin;

    uint16 public maxMintBatch; // max amount of cards to be minted in one call

    uint8 public autoRandomRequest; // createWeapons requests randomness
    bool public vrf;
...
// ################################################################################
/*
addWeaponTier(2,1,4,100);
address[] memory recipients_ = new address[](5);
recipients_[0] = 0x5B38Da6a701c568545dCfcB03FcB875f56beddC4;
recipients_[1] = 0xAb8483F64d9C6d1EcF9b849Ae677dD3315835cb2;
recipients_[2] = 0x4B20993Bc481177ec7E8f571ceCaE8A9e22C02db;
recipients_[3] = 0x78731D3Ca6b7E34aC0F824c42a7cC18A495cabaB;
recipients_[4] = 0x617F2E2fD72FD9D5503197092aC168c91465E7f2;
createWeapons(1,1,1,recipients_);
createWeapons(1,2,1,recipients_);
*/
// ################################################################################

    }

    /// @notice add weapon tier
    function addWeaponTier
    (
        uint256 _typeCnt, //2 ( 1-melee 2-ranged )
        uint256 _subtypeCnt, // 1 (1-1 sword, 2-1 gun)
        uint256 _categoryCnt, // 4 ()
        uint256 _mintCap
    )
        public
        onlySuperAdmin
    ...
    /// @notice Create weapon
    /// @param _type (1-melee, 2-ranged...)
    /// @param _recipients Address of the card recipients
    function createWeapons(
        ...
        onlyAdmin
    {
        uint256 randomNumberBase_ = (vrf ? _rndNumVrf : block.timestamp);
        require(randomNumberBase_ > 0 || !vrf,"CRE-05");
        require(_recipients.length <= maxMintBatch,"CRE-10");
        require(_tierLvl > 0 && _tierLvl <= weaponTiers.length,"CRE-20");    
        require(_type > 0 && _type <= weaponTiers[_tierLvl-1].typeCnt,"CRE-30");
        require(_subtype > 0 && _subtype <= weaponTiers[_tierLvl-1].subtypeCnt,"CRE-40");
        require(wpnTierMintIndex[_tierLvl]+_recipients.length <= weaponTiers[_tierLvl-1].mintCap,"CRE-50");
        for (uint256 i = 0; i < _recipients.length; i++) {
            require(_recipients[i] != address(0),"CRE-60");
        }
        uint256 randomNumber_;
        uint256 attribs_;
        uint256 category_;
        uint256 tokenId_ = maxTokenId;
        uint256 serialNo_ = wpnTTSMintIndex[_tierLvl*10000+_type*100+_subtype];
        address recipient_;
        ...
        for (uint256 i=0; i < _recipients.length; i++) {
            ...
            randomNumber_ = uint256(keccak256(abi.encodePacked(randomNumberBase_, tokenId_, recipient_)));
        ...
        //
        maxTokenId = tokenId_;
        wpnTierMintIndex[_tierLvl] += _recipients.length;
        wpnTTSMintIndex[_subtype+_type*100+_tierLvl*10000] += _recipients.length;
        if (vrf && autoRandomRequest > 0) requestRndNumVrf(autoRandomRequest);
    }

    /// @notice Burn weapon token
    function burnWeapon(uint256 _tokenId, address _owner)
        external
        onlyBurner
    {
        require(_owner == ownerOf(_tokenId),"BRN-01");
        require(
            _owner == msg.sender ||
            getApproved(_tokenId) == msg.sender ||
            isApprovedForAll(_owner, msg.sender),"BRN-02");
        // !!!
        TokenData memory tokenData_ = getTokenData(_tokenId);
        uint256 sumAttribs_;
        _burn(_tokenId);
    ...
    /// @notice increase attributes for a token
        ...
        onlyAdmin
    {
        ...
        emit IncreaseAttribs(_tokenId, sec_, ano_, inn_);
    }

    /// @notice get all attributes for tokenId
    function getAttribs(uint256 _tokenId)
        public
        view
        returns(uint256 _sec,uint256 _ano,uint256 _inn)
    { 
        uint256 tokenData_ = tokenData[_tokenId];
        _sec = uint256(uint8(tokenData_>>64));
        _ano = uint256(uint8(tokenData_>>72));
        _inn = uint256(uint8(tokenData_>>80));
    }
    ...
    {
        uint256 tokenData_ = tokenData[_tokenId];
        _tierLvl = uint256(uint8(tokenData_));
        _type = uint256(uint8(tokenData_>>8));
        _subtype = uint256(uint8(tokenData_>>16));
        _category = uint256(uint8(tokenData_>>24));
        _serialNo = uint256(uint32(tokenData_>>32));
    }
    ...
    /// @notice change mint cap
    function changeMintCap(uint256 _tierLvl, uint256 _newCap) external onlySuperAdmin{
        require(_tierLvl > 0 && _tierLvl <= weaponTiers.length,"CAP-01");
        require(_newCap >=  weaponTiers[_tierLvl-1].mintCap, "CAP-02");
        WeaponTier storage weaponTier_ = weaponTiers[_tierLvl-1];
        weaponTier_.mintCap = uint32(_newCap);
        emit UpdateWeaponTierParameter(_tierLvl, "mintCap", _newCap);
    }

    /// @notice Increase weapon type count
    function increaseWeaponTypeCount(uint256 _tierLvl) external onlySuperAdmin{
        ...
        WeaponTier storage weaponTier_ = weaponTiers[_tierLvl-1];
        weaponTier_.typeCnt++;
        emit UpdateWeaponTierParameter(_tierLvl, "typeCnt", weaponTier_.typeCnt);
    }

    /// @notice Increase weapon subtype count
    function increaseWeaponSubtypeCount(uint256 _tierLvl) external onlySuperAdmin{
        ...
        emit UpdateWeaponTierParameter(_tierLvl, "subtypeCnt", weaponTier_.subtypeCnt);
    }

    /// @notice Increase weapon category count
    function increaseWeaponCategoryCount(uint256 _tierLvl) external onlySuperAdmin{
        ...
        emit UpdateWeaponTierParameter(_tierLvl, "categoryCnt", weaponTier_.categoryCnt);
    }

    /// @notice Set max mint batch
    function setMaxMintBatch(uint16 _newMaxMintBatch) external onlyAdmin {
        maxMintBatch = _newMaxMintBatch;
    }

    /// @notice Admin function to fix possible discrepancy with tokenId
    ...
    /// @notice provides array of tokenIds assigned to specified address
    ...
    function setAutoRandomRequest(uint256 _newStatus) external onlySuperAdmin {
        require(_newStatus < 3, "ARR-01");
        autoRandomRequest = uint8(_newStatus);
        emit SetAutoRandomRequest(_newStatus);
    }

    function toggleSafeMinting() external onlySuperAdmin {
    ...
    function changeBurner(address _newBurner)
        ...
        onlySuperAdmin
    {
        require(_newBurner != address(0) && _newBurner != burner,"ADM-01");
        burner = _newBurner;
    ...
    function setTradeExp(uint256 _tokenId, uint256 _amount) external onlyAdmin {
        tokenTradeExp[_tokenId] = _amount;
    }

    function increaseTradeExp(uint256 _tokenId, uint256 _amount) external onlyAdmin {
        tokenTradeExp[_tokenId] += _amount;
    }
    ...
    function requestRndNumVrf(uint256 _requestType) public onlyAdmin returns (bytes32 requestId) {
    ...
    /// @notice needed for ERC721Enumerable
    ...
    /// @notice override of internal getter for base URI
