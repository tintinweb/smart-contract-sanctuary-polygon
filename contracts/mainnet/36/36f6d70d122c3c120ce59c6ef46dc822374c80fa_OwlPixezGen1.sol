...
contract OwlPixezGen1 is ERC721, ERC721Burnable, Ownable, EIP712, ERC721Votes {
    using Counters for Counters.Counter;
    using Strings for uint256;
    ...
    uint nonce;
    mapping (uint256 => uint256) tokenPickMapping;
    uint256 tokensAvailable = 8000;
    uint256 public maxMintAmount = 10;

    constructor() ERC721("OwlPixez Gen 1", "OWLPx") EIP712("OwlPixez Gen 1", "1") {}

    function _baseURI() internal pure override returns (string memory) {
        return "ipfs://bafybeigsft6psf7prefpjlz2g4lqch5z3ajypruq26yi2hnwlycbn3bxti/";
    }

    function contractURI() public pure returns (string memory) {
        return "ipfs://bafkreif4n4yyhxa6ioapn6576mu2mmah3dbczqsgpk2xnkyvsprwthuszy";
    }

    function random() internal returns (uint) {
       uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp, block.difficulty, nonce))) % 8000;
    ...
    function reward(address _to) public onlyOwner{
        require(tokensAvailable > 0, "No more tokens available.");
        uint256 randomPick = random() % tokensAvailable;
        uint256 pickedRandomToken = tokenPickMapping[randomPick];
        if (pickedRandomToken == 0) {
            pickedRandomToken = randomPick + 1;
        }
        
        uint256 newTokenId = pickedRandomToken - 1;
        _mint(_to, newTokenId);
        _supply.increment();
        tokensAvailable--;
        uint256 lastToken = tokenPickMapping[tokensAvailable];
        if (lastToken == 0) {
    ...
    function withdraw() external onlyOwner {
        ...
        //Gnosis Safe
        payable(0x86dac72b20191122223a77B5AD658D2808e0b96C).transfer(address(this).balance);
