...
contract Polystone is ERC20Burnable, Operator {
    using SafeMath8 for uint8;
    using SafeMath for uint256;

    // Initial distribution for the first 24h genesis pools
    uint256 public constant INITIAL_GENESIS_POOL_DISTRIBUTION = 25000 ether;
    // Initial distribution for the day 2-5 STONE-MATIC LP -> STONE pool
    ...
    function _getEPolyPrice() internal view returns (uint256 _polyPrice) {
        try IOracle(polyOracle).consult(address(this), 1e18) returns (
            uint144 _price
        ) {
            return uint256(_price);
        } catch {
            revert("POLYSTONE: failed to fetch POLYSTONE price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _polyPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (
                ...
                if (_polyPrice >= taxTiersTwaps[tierId]) {
                    require(
    ...
    function setPolyOracle(address _polyOracle) public onlyOperatorOrTaxOffice {
        require(
            _polyOracle != address(0),
    ...
    /**
     * @notice Operator mints POLYSTONE to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of POLYSTONE to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentTombPrice = _getEPolyPrice();
