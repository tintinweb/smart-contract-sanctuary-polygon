...
contract AuraDogsNFT is ERC721, Ownable {
  ...
  uint256 public whitelistPrice = 255000000000000000000 wei; // 200 USD in MATIC
  uint256 public preSalePrice = 318000000000000000000 wei; // 250 USD in MATIC
  uint256 public publicSalePrice = 382000000000000000000 wei; // 300 USD in MATIC
  ...
  modifier whitelistMintCompliance(uint256 _mintAmount) {
    ...
    require(supply.current() + _mintAmount <= 1000, "Max supply exceeded!");
    _;
  }

  modifier presaleMintCompliance(uint256 _mintAmount) {
    require(_mintAmount > 0, "Invalid mint amount!");
    require(supply.current() + _mintAmount <= 2000, "Max supply exceeded!");
    _;
  }
  ...
  constructor(bytes32 _root) ERC721("AuraDogs", "AD") {
      root = _root;
  }
  ...
  function setSales(bool _whitelist, bool _presale, bool _publicsale) public onlyOwner {
  ...
  function whitelistMint(uint256 _mintAmount, bytes32[] memory proof) public payable whitelistMintCompliance(_mintAmount) {
    require(claimed[msg.sender] == false, "Already claimed");
    claimed[msg.sender] = true;
    require(MerkleProof.verify(proof, root, toBytes32(msg.sender)), "Address not whitelisted");
    require(!paused, "The contract is paused!");
    require(whitelist, "Whitelist off!");
  ...
  function mint(uint256 _mintAmount) public payable mintCompliance(_mintAmount) {
    require(!paused, "The contract is paused!");
    require(publicsale, "Public Sale off!");
    require(!_blacklist[msg.sender], "Blacklisted address");
