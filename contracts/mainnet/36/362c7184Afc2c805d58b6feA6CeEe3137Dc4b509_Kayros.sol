...
import {IKayros} from '../interfaces/IKayros.sol';
...
import {KayrosNFTBase} from './base/KayrosNFTBase.sol';
import {KayrosMultiState} from './base/KayrosMultiState.sol';
import {KayrosStorage} from './storage/KayrosStorage.sol';
 ...
 */
contract Kayros is KayrosNFTBase, VersionedInitializable, KayrosMultiState, KayrosStorage, IKayros {
 ...
 * @author Lens Protocol
 *
 * @notice This is the interface for the Kayros contract, the main entry point for the Lens Protocol.
 * You'll find all the events and external functions, as well as the reasoning behind them here.
 */
interface IKayros {
    /**
     * @notice Initializes the Kayros NFT, setting the initial governance address as well as the name and symbol in
...
library Events {
     ...
     *
     * @param hub The Kayros contract address used.
     * @param timestamp The current block timestamp.
     */
...
import {IKayrosNFTBase} from '../../interfaces/IKayrosNFTBase.sol';
import {Errors} from '../../libraries/Errors.sol';
import {DataTypes} from '../../libraries/DataTypes.sol';
import {Events} from '../../libraries/Events.sol';
 ...
 */
abstract contract KayrosNFTBase is ERC721Enumerable, IKayrosNFTBase {
 ...
 *
 * @notice This is an abstract contract that implements internal Kayros state setting and validation.
 *
 * whenNotPaused: Either publishingPaused or Unpaused.
 * whenPublishingEnabled: When Unpaused only.
 */
abstract contract KayrosMultiState {
 ...
 * @author Lens Protocol
 *
 * @notice This is an abstract contract that *only* contains storage for the Kayros contract. This
 * *must* be inherited last (bar interfaces) in order to preserve the Kayros storage layout. Adding
 * storage variables should be done solely at the bottom of this contract.
 */
abstract contract KayrosStorage {
...
interface IFollowModule {
    ...
    /**
     * @notice Processes a given follow, this can only be called from the Kayros contract.
     *
     ...
     * only be called from the Kayros contract.
     *
...
interface ICollectModule {
     ...
     * @param pubId The associated publication's Kayros publication ID.
     ...
     *
     * @param referrerProfileId The Kayros profile token ID of the referrer's profile (only different in case of mirrors).
     * @param collector The collector address.
     * @param profileId The token ID of the profile associated with the publication being collected.
     * @param pubId The Kayros publication ID associated with the publication being collected.
...
contract FollowNFTProxy is Proxy {
    ...
    constructor(bytes memory data) {
        HUB = msg.sender;
        IKayros(msg.sender).getFollowNFTImpl().functionDelegateCall(data);
    }

    function _implementation() internal view override returns (address) {
        return IKayros(HUB).getFollowNFTImpl();
    }
}
