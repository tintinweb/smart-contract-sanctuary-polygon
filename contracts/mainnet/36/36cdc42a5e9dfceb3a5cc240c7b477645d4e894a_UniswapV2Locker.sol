...
contract UniswapV2Locker is Ownable, ReentrancyGuard {
    ...
    EnumerableSet.AddressSet allowedUniswapFactories;
    ...
    struct TokenLock {
        ...
        address owner;
        string logoUrl; // Logo URL for the pair;
    }
    ...
    constructor(address _uniswapFactory) public {
        devaddr = msg.sender;
        gFees.referralPercent = 0; // 25% = 250
        gFees.ethFee = 0; // 1 eth = 1e18
        gFees.secondaryTokenFee = 0; // 1 token = 1e18
        gFees.secondaryTokenDiscount = 0; // 20% = 200
        gFees.liquidityFee = 0; // 1% = 10
        gFees.referralHold = 0; // 10 eth = 10e18
        gFees.referralDiscount = 0; // 10% = 100
        allowedUniswapFactories.add(_uniswapFactory);
    }

    function toggleUniswapFactory(address _uniswapFactory, uint8 add)
        public
        onlyOwner
    {
        if (add == 0) {
            allowedUniswapFactories.remove(_uniswapFactory);
        } else {
            allowedUniswapFactories.add(_uniswapFactory);
        }
    }
     ...
     * @param _fee_in_eth fees can be paid in eth or in a secondary token with a discount on univ2 tokens
    ...
    function lockLPToken(
        ...
        require(
            allowedUniswapFactories.contains(lpair.factory()),
            "INVALID_UNISWAP_FACTORY"
        );
        address factoryPairAddress = IUniFactory(lpair.factory()).getPair(
            lpair.token0(),
            lpair.token1()
        );
    ...
    /**
     * @notice change the logo url of a lock
     */
    ...
    function getUniswapFactoriesLength() external view returns (uint256) {
        return allowedUniswapFactories.length();
    }
}