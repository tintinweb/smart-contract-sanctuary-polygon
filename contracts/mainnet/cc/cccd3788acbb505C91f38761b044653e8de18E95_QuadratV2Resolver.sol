...
import {IQuadratV2Resolver} from "./interfaces/IQuadratV2Resolver.sol";
import {
    IUniswapV3Factory
} from "@uniswap/v3-core/contracts/interfaces/IUniswapV3Factory.sol";
import {IQuadratV2Helper} from "./interfaces/IQuadratV2Helper.sol";
...
import {
    ...
    RangeWeight,
    Rebalance
} from "./structs/SQuadratV2.sol";
import {hundredPercent} from "./constants/CQuadratV2.sol";

/// @title QuadratV2Resolver helpers that resolve / compute payloads for QuadratV2 calls
contract QuadratV2Resolver is IQuadratV2Resolver {
    IUniswapV3Factory public immutable factory;
    IQuadratV2Helper public immutable helper;

    constructor(IUniswapV3Factory factory_, IQuadratV2Helper helper_) {
        factory = factory_;
        helper = helper_;
    }
    ...
    /// @param vaultV2_ Quadrat V2 vault.
    /// @return rebalanceParams payload to send to rebalance
    /// function on Quadrat V2 contract.
...
import {IQuadratV2} from "./IQuadratV2.sol";
import {RangeWeight, Rebalance} from "../structs/SQuadratV2.sol";

interface IQuadratV2Resolver {
    function standardRebalance(
        RangeWeight[] memory rangeWeights_,
        IQuadratV2 vaultV2_
    ) external view returns (Rebalance memory rebalanceParams);
...
import {Amount} from "../structs/SQuadratV2Helper.sol";

interface IQuadratV2Helper {
    function totalUnderlyingWithFeesAndLeftOver(IQuadratV2 vault_)
        external
        view
        returns (UnderlyingOutput memory underlying);

    function totalUnderlyingWithFees(IQuadratV2 vault_)
        external
        view
        returns (
            uint256 amount0,
    ...
    function totalUnderlying(IQuadratV2 vault_)
        external
        view
...
library Underlying {
        ...
        )
    {
        ...
        IQuadratV2 quadratV2 = IQuadratV2(underlyingPayload_.self);

        (uint256 fee0After, uint256 fee1After) = subtractAdminFees(
            fee0,
            fee1,
            quadratV2.managerFeeBPS()
        );
        ...
        amount0 +=
            ...
            quadratV2.managerBalance0();
        amount1 +=
            fee1After +
            IERC20(underlyingPayload_.token1).balanceOf(
                underlyingPayload_.self
            ) -
            quadratV2.managerBalance1();
    }
        ...
        )
    {
        ...
        if (current0_ == 0 && current1_ > 0) {
            ...
            require(amount0Mint > 0 && amount1Mint > 0, "QuadratV2: mint 0");

            mintAmount = amount0Mint < amount1Mint ? amount0Mint : amount1Mint;
        } else {
            revert("QuadratV2: panic");
        }
...
import {Range} from "./SQuadratV2.sol";
