...
contract BEARETH1X_Treasury is ContractGuard {
    ...
    //eth price oracle
    AggregatorV3Interface public priceFeed = AggregatorV3Interface(0xF9680D99D6C9589e2a93a78A04A279e509205945);
    ...
    function getIndexPrice(uint _currentEthPrice, uint _initialEpochEthPrice, uint _initialTokenIndexPrice) internal pure returns (uint) {
        uint256 temp1 = _currentEthPrice;
        uint256 temp2 = _initialEpochEthPrice.mul(2);
        if(temp2 < temp1){
            return _initialTokenIndexPrice.div(100);
        }else{
            uint256 IndexPrice = (temp2-temp1).mul(_initialTokenIndexPrice).div(_initialEpochEthPrice);
            return IndexPrice;
        }
    }
    ...
    function allocateSeigniorage() external onlyOneBlock checkCondition checkEpoch checkOperator {
        ...
        if (previousEpochTokenTwapPrice > currentEpochTokenIndexPrice.mul(tokenPriceCeiling).div(10000)) {
            uint256 _percentage = (previousEpochTokenTwapPrice - currentEpochTokenIndexPrice).div(5);
    ...
    function boardroomRemoveAddressBlacklist(address _address) external onlyOperator {
        IBoardroom(boardroom).removeAddressBlacklist(_address);
    }

    function boardroomBlacklistFundWithdraw(address _address, address _to) external onlyOperator {
        IBoardroom(boardroom).blacklistFundWithdraw(_address, _to);
    }
