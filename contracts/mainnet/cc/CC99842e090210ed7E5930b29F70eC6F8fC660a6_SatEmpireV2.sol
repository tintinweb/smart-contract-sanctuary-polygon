...
/// @dev This contract defines the core logic of the EMPIREv2 token on a satellite chain
contract SatEmpireV2 is Controller {
    ...
    mapping (address => bool) private _excludedTradeLimits;
    ...
    constructor (address eRouter) {
        ...
        address _pair = IEmpireFactory(router.factory())
    ...
    /// @notice Receives `amount` EMPIRE from a cross chain transfer
    ...
    /// @notice Initiates cross chain transfer of `amount` EMPIRE to another chain
    /// @dev Called by bridge when user has calls function lock() on LockBurnBridge
