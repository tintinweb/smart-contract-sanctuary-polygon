...
contract VibeFactory {
    ...
    BoringFactory public immutable factory;
    ...
    event ERC20Created(address sender, address proxy);
    event ERC721Created(address sender, address proxy);
    event ERC1155Created(address sender, address proxy);
     ...
     */
    constructor(address _vibeERC20Impl, address _vibeERC721Impl, address _vibeERC1155Impl, address _royaltyReceiverImpl, address _NFTMintSale, address _NFTMintSaleMultiple, BoringFactory _simpleFactory) {
    ...
    function createRoyaltyReceiver(uint256[] calldata recipientBPS_, address[] calldata recipients_) external {
        bytes memory data = abi.encode(recipientBPS_, recipients_);
        address proxy = factory.deploy(royaltyReceiverImpl, data, false);
        RoyaltyReceiver(proxy).transferOwnership(msg.sender);
    }

    function createNFTMintSale(string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint16 derivativeRoyaltyRate, bool isDerivativeAllowed, uint64 maxMint_, uint32 beginTime_, uint32 endTime_, uint128 price_, IERC20 paymentToken_) external {
        bytes memory data = abi.encode(symbol, name, baseURI, royaltyReceiver_, royaltyRate_, derivativeRoyaltyRate, isDerivativeAllowed, maxMint_, beginTime_, endTime_, price_, paymentToken_);
        address proxy = factory.deploy(nftMintSale, data, false);
        RoyaltyReceiver(proxy).transferOwnership(msg.sender);
    }

    function createNFTMintSaleMultiple(string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint16 derivativeRoyaltyRate, bool isDerivativeAllowed, uint32 beginTime_, uint32 endTime_, TierInfo[] memory tiers_, IERC20 paymentToken) external {
        bytes memory data = abi.encode(symbol, name, baseURI, royaltyReceiver_, royaltyRate_, derivativeRoyaltyRate, isDerivativeAllowed, beginTime_, endTime_, tiers_, paymentToken);
    ...
    function createERC20(string memory name, string memory symbol) external {
        ...
        VibeERC20(proxy).transferOwnership(msg.sender);

        emit ERC20Created(msg.sender, proxy);
    }

    function createERC721(string memory name, string memory symbol, string memory baseURI) external {
        
        bytes memory data = abi.encode(name, symbol, baseURI);
        address proxy = factory.deploy(vibeERC721Impl, data, false);
        
        VibeERC721(proxy).transferOwnership(msg.sender);

        emit ERC721Created(msg.sender, proxy);
    }

    function createERC1155(string memory uri) external {
        bytes memory data = abi.encode(uri);
        address proxy = factory.deploy(vibeERC1155Impl, data, false);
        
        VibeERC1155(proxy).transferOwnership(msg.sender);

        emit ERC1155Created(msg.sender, proxy);
    }
}
...
contract RoyaltyReceiver is Ownable {
    ...
    event LogDistributeETH(address indexed recipient, uint256 amount);
    ...
    function distributeERC20(ERC20 token) external {
    ...
    function distributeETH() external {
        ...
        for (uint256 i; i < recipientBPS.length; i++ ) {
            uint256 amount = totalAmount * recipientBPS[i] / BPS;
            SafeTransferLib.safeTransferETH(recipients[i], amount);
            emit LogDistributeETH(recipients[i], amount);
        }
...
contract NFTMintSale is Ownable {
    ...
    BoringFactory public immutable vibeFactory;
    ...
    function init(bytes calldata data) public payable {
        (string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint16 derivativeRoyaltyRate, bool isDerivativeAllowed, uint64 maxMint_, uint32 beginTime_, uint32 endTime_, uint128 price_, IERC20 paymentToken_) = abi.decode(data, (string, string, string, address, uint16, uint16, bool, uint64, uint32, uint32, uint128, IERC20));
