...
interface IConvexBooster {
  ...
  function poolInfo(
    uint256 pid
  ) external view returns (address lpToken, address gauge, address crvRewards, bool shutdown, address factory);
}
...
contract StrategyConvex is StratManagerUpgradeable, DynamicFeeManager {
  ...
  event ChargedFees(uint256 callFees, uint256 strategistFees);
  ...
  function initialize(
    ...
    CommonAddresses memory commonAddresses
  ) public initializer {
    __StratManager_init(
      commonAddresses._keeper,
      commonAddresses._strategist,
      commonAddresses._unirouter,
      commonAddresses._vault,
      commonAddresses._dysonFeeRecipient
    );
  ...
  function swapRewardsToNative() internal {
    for (uint i; i < rewardsV3.length; ++i) {
      uint bal = IERC20Upgradeable(rewardsV3[i].token).balanceOf(address(this));
      if (bal >= rewardsV3[i].minAmount) {
        UniV3Actions.swapV3WithDeadline(unirouterV3, rewardsV3[i].toNativePath, bal);
      }
    }
    for (uint i; i < rewards.length; ++i) {
      uint bal = IERC20Upgradeable(rewards[i].token).balanceOf(address(this));
  ...
  function chargeFees(address callFeeRecipient) internal {
    uint256 generalFeeAmount = (IERC20Upgradeable(native).balanceOf(address(this)) * feeOnProfits) / 1000;
    ...
    emit ChargedFees(callFeeAmount, strategistFeeAmount);
  }
  ...
  function addLiquidity() internal {
    ...
    if (depositToken != native) {
      if (nativeToDepositPath.length > 0) {
        UniV3Actions.swapV3WithDeadline(dystRouter, nativeToDepositPath, nativeBal);
      } else {
        IUniswapRouterETH(dystRouter).swapExactTokensForTokens(
          nativeBal,
          0,
      ...
      depositBal = IERC20Upgradeable(depositToken).balanceOf(address(this));
    } else {
  ...
  function addRewardV2(address _router, address[] calldata _rewardToNativeRoute, uint _minAmount) external onlyOwner {
    ...
    IERC20Upgradeable(token).approve(_router, 0);
    IERC20Upgradeable(token).approve(_router, type(uint).max);
  }
  ...
  function addRewardV3(bytes memory _rewardToNativePath, uint _minAmount) public onlyOwner {
    ...
    IERC20Upgradeable(token).approve(unirouterV3, 0);
    IERC20Upgradeable(token).approve(unirouterV3, type(uint).max);
  }
  ...
  function _giveAllowances() internal {
    IERC20Upgradeable(want).approve(address(booster), type(uint).max);
    IERC20Upgradeable(native).approve(dystRouter, type(uint).max);
    IERC20Upgradeable(depositToken).approve(pool, type(uint).max);
    if (zap != address(0)) IERC20Upgradeable(depositToken).approve(zap, type(uint).max);
  }

  function _removeAllowances() internal {
    IERC20Upgradeable(want).approve(address(booster), 0);
    IERC20Upgradeable(native).approve(dystRouter, 0);
    IERC20Upgradeable(depositToken).approve(pool, 0);
    if (zap != address(0)) IERC20Upgradeable(depositToken).approve(zap, 0);
  }
...
import "../interfaces/IUniswapRouterV3WithDeadline.sol";
