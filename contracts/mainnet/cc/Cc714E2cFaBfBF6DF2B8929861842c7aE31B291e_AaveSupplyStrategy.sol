...
contract AaveSupplyStrategy is InitializableAbstractStrategy, CurveExchange {
    ...
    function collectRewardTokens() external override onlyHarvester nonReentrant {
        console.log("Aave Strategy: NO_EXPLICIT_REWARDS");
    }
    ...
    function _withdrawAsset(address _asset, uint256 _amountInUsd) internal returns (uint256) {
        ...
        require(IERC20(_asset).balanceOf(address(this)) >= _inTokenAmount, "AaveStrategy - LOW_BAL_IN_TOKEN");
    ...
    // @dev This function will withdraw the token0 amount from the startegy pool
    function _directWithdraw(uint256 _amountOfToken0) internal {
        if (_amountOfToken0 > lpBalance()) {
            storedATokenBalance = 0;
            pool.withdraw(address(token0), type(uint256).max, address(this));
        } else {
            storedATokenBalance -= _amountOfToken0;
            pool.withdraw(address(token0), _amountOfToken0, address(this));
        }
    }
}
