    ...
    UUPSUpgradeable
{
    ...
    function send(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            if (token == address(0)) {
                ...
                IERC20Upgradeable(token).transferFrom(msg.sender, users[i], amounts[i]);
            }
        }
    }
    ...
    function sendERC1155(
        ...
        for (uint256 i = 0; i < users.length; i++) {
            IERC1155Upgradeable(token).safeTransferFrom(msg.sender, users[i], id, amounts[i], "0x00");
        }
    }
