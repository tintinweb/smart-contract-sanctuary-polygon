...
contract DirectSwap is FeeModel, IRouter {
    ...
    function directCurveV1Swap(Utils.DirectCurveV1 memory data) external payable {
        ...
        if (address(fromToken) == ETH_IDENTIFIER && data.needWrapNative == true) {
            IWETH(weth).deposit{ value: fromAmount }();
        }

        if (!data.isApproved) {
            if (data.needWrapNative == true) {
                Utils.approve(data.exchange, weth, fromAmount);
            } else {
                Utils.approve(data.exchange, fromToken, fromAmount);
            }
        }
        ...
        if (data.swapType == Utils.CurveSwapType.EXCHANGE_UNDERLYING) {
            amountOut = ICurvePool(data.exchange).exchange_underlying(data.i, data.j, fromAmount, 1);
        } else {
            if (address(fromToken) == ETH_IDENTIFIER && data.needWrapNative == false) {
                amountOut = ICurveEthPool(data.exchange).exchange{ value: fromAmount }(data.i, data.j, fromAmount, 1);
            } else {
                amountOut = ICurvePool(data.exchange).exchange(data.i, data.j, fromAmount, 1);
            }
        }
    ...
    function directCurveV2Swap(Utils.DirectCurveV2 memory data) external payable {
        ...
        uint256 amountOut;
        if (data.swapType == Utils.CurveSwapType.EXCHANGE_UNDERLYING) {
            amountOut = ICurveV2Pool(data.exchange).exchange_underlying(data.i, data.j, fromAmount, 1);
        } else if (data.swapType == Utils.CurveSwapType.EXCHANGE_GENERIC_FACTORY_ZAP) {
            amountOut = IGenericFactoryZap(data.exchange).exchange(data.poolAddress, data.i, data.j, fromAmount, 1);
        } else {
            if (address(fromToken) == ETH_IDENTIFIER && data.needWrapNative == false) {
                amountOut = ICurveV2EthPool(data.exchange).exchange{ value: fromAmount }(
                    data.i,
                    data.j,
                    fromAmount,
                    1,
                ...
                amountOut = ICurveV2Pool(data.exchange).exchange(data.i, data.j, fromAmount, 1);
            }
        }
