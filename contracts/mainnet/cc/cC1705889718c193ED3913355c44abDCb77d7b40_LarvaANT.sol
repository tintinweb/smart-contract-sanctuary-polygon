...
contract LarvaANT is ERC721, Ownable {
    uint256 public immutable HATCH_DURATION = 3 minutes;
    uint256 public immutable PORTION_FEE = 80;
    uint256 public immutable MAX_GENESIS_MINT = 10000;
    uint256 public immutable LARVA_PRICE = 0.0008 ether;
    ...
    constructor() ERC721("LarvaANT","LarvaANT") {
       admin = msg.sender;     
    }
    ...
    function getLarvae(address _user) public view returns(uint256[] memory) {
        ...
        uint256[] memory larvas = new uint256[](_balance);
        larvas = playerToLarvae[_user];
        return larvas;
    }

    function getHatchersLength (address _user) public view returns (uint256 hatchersLength) {
        ...
        for (uint256 i = 0; i < larvaeList.length; i++){
            ...
            hatchersLength = _elapsedTime > HATCH_DURATION ? hatchersLength++ : hatchersLength;
        }
    }

    function genesisMint(uint256 amount) public payable {
        require(amount * LARVA_PRICE == msg.value, "Incorrect amount.");
        require(genesisCounter + amount < MAX_GENESIS_MINT, "No more larvae left.");  
        for (uint i = 0; i < amount; i++){
            idToSpawnTime[counter] = block.timestamp;
            idToResource[counter] = 0;
            _mint(msg.sender,counter);
            playerToLarvae[msg.sender].push(counter); 
            counter++;
            genesisCounter++;
        }
    }
    ...
    function burn(address _user, uint256 _id) public {
        ...
        for (uint i = 0; i < playerToLarvae[_user].length - 1; i++) {
