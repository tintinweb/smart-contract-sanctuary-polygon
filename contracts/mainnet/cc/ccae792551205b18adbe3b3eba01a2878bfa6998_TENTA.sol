...
contract TENTA is ERC20, AccessControl, ERC721Holder {
	...
	mapping (address => mapping(uint256 => StakeInfoC4)) public stakeLogC4;
	...
	mapping (address => uint256) public tokensStakedByUserC4;
	...
	mapping (address => uint256[]) public stakePortfolioByUserC4;
	...
	mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioC4;
	...
	event RewardsHarvestedC4 (address owner, uint256 amount);
	...
	event NFTStakedC4 (address owner, uint256 tokenId);
	...
	event NFTUnstakedC4 (address owner, uint256 tokenId);

    constructor() ERC20("TENTA", "TENTA")AccessControl(){
        owner = 0x4aCdB47f872b97BA9b8FEa3DB79af976764de23c;
        collectionAddressC1 = 0x47ab7cd7A8df3b816F6994c4093b416c5BD881F6;
		...
		collectionAddressC4 = 0x0000000000000000000000000000000000000000;
        _mint(owner, 90000 * 10 ** 18);
		...
		stakingStartBlockC4 = block.number;
        coinAmountPerRewardUnitC1 = 1 * 10 ** 18;
        coinAmountPerRewardUnitC2 = 1 * 10 ** 18;
        coinAmountPerRewardUnitC3 = 1 * 10 ** 18;
		coinAmountPerRewardUnitC4 = 1 * 10 ** 18;
        numberOfBlocksPerRewardUnitC1 = 3600;
        numberOfBlocksPerRewardUnitC2 = 3600;
        numberOfBlocksPerRewardUnitC3 = 3600;
		numberOfBlocksPerRewardUnitC4 = 3600;
    }
	...
	function stakedNFTSByUserC4(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserC4[owner];
    }
	...
	function pendingRewardsC4(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfoC4 memory infoC4 = stakeLogC4[owner][tokenId];

        if(infoC4.lastHarvestBlock < stakingStartBlockC4 || infoC4.currentlyStaked == false) {
            return 0;
        }
        uint256 blocksPassedSinceLastHarvest = block.number - infoC4.lastHarvestBlock;
        if (blocksPassedSinceLastHarvest < numberOfBlocksPerRewardUnitC4 * 2) {
            return 0;
        }
        uint256 rewardAmount = blocksPassedSinceLastHarvest / numberOfBlocksPerRewardUnitC4 - 1;
        return rewardAmount * coinAmountPerRewardUnitC4;
    }
    ...
    function stakeC1(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddressC1).ownerOf(tokenId) == address(this),
            "TENTA: Error while transferring token");
	...
	function stakeC4(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        IERC721(collectionAddressC4).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressC4).ownerOf(tokenId) == address(this),
            "TENTA: Error while transferring token");
        StakeInfoC4 storage infoC4 = stakeLogC4[_msgSender()][tokenId];
        infoC4.stakedAtBlock = block.number;
        infoC4.lastHarvestBlock = block.number;
        infoC4.currentlyStaked = true;
        if(tokensStakedByUserC4[_msgSender()] == 0){
            amountOfStakersC4 += 1;
        }
        tokensStakedByUserC4[_msgSender()] += 1;
        tokensStakedC4 += 1;
        stakePortfolioByUserC4[_msgSender()].push(tokenId);
        uint256 indexOfNewElementC4 = stakePortfolioByUserC4[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioC4[tokenId] = indexOfNewElementC4;
	...
	function stakeBatchC4(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
    ...
    function harvestC1(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC1 storage infoC1 = stakeLogC1[_msgSender()][tokenId];
        uint256 rewardAmountInUnites2 = pendingRewardsC1(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInUnites2 > 0) {
            require(supply + rewardAmountInUnites2 <= maxSupply, "Maximum Supply limit exceeded");
            infoC1.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInUnites2);
            emit RewardsHarvestedC1(_msgSender(), rewardAmountInUnites2);
        }
    }
    function harvestC2(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC2 storage infoC2 = stakeLogC2[_msgSender()][tokenId];
        uint256 rewardAmountInUnites2 = pendingRewardsC2(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInUnites2 > 0) {
            require(supply + rewardAmountInUnites2 <= maxSupply, "Maximum Supply limit exceeded");
            infoC2.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInUnites2);
            emit RewardsHarvestedC2(_msgSender(), rewardAmountInUnites2);
        }
    }
    function harvestC3(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC3 storage infoC3 = stakeLogC3[_msgSender()][tokenId];
        uint256 rewardAmountInUnites2 = pendingRewardsC3(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInUnites2 > 0) {
            require(supply + rewardAmountInUnites2 <= maxSupply, "Maximum Supply limit exceeded");
            infoC3.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInUnites2);
            emit RewardsHarvestedC3(_msgSender(), rewardAmountInUnites2);
        }
    }
	function harvestC4(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC4 storage infoC4 = stakeLogC4[_msgSender()][tokenId];
        uint256 rewardAmountInUnites2 = pendingRewardsC4(_msgSender(), tokenId);
        uint256 supply = totalSupply();
        if(rewardAmountInUnites2 > 0) {
            require(supply + rewardAmountInUnites2 <= maxSupply, "Maximum Supply limit exceeded");
            infoC4.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInUnites2);
            emit RewardsHarvestedC4(_msgSender(), rewardAmountInUnites2);
        }
    }
	...
	function harvestBatchC4(address user) external payable{
        uint256[] memory tokenIds = stakePortfolioByUserC4[user];
	...
	function unstakeC4(uint256 tokenId) public {
        if(pendingRewardsC4(_msgSender(), tokenId) > 0){
            harvestC4(tokenId);
        }
        StakeInfoC4 storage infoC4 = stakeLogC4[_msgSender()][tokenId];
        infoC4.currentlyStaked = false ;
        IERC721(collectionAddressC4).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressC4).ownerOf(tokenId) == _msgSender(),
            "TENTA: Error while transferring token");
        if(tokensStakedByUserC4[_msgSender()] == 1){
            amountOfStakersC4 -= 1;
        }
        tokensStakedByUserC4[_msgSender()] -= 1;
        tokensStakedC4 -= 1;
        stakePortfolioByUserC4[_msgSender()][indexOfTokenIdInStakePortfolioC4[tokenId]] = 0;
	...
	function unstakeBatchC4(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
	...
	function setNumberOfBlocksPerRewardUnitC4(uint256 numberOfBlocksC4) external onlyRole(CONTRACT_ADMIN_ROLE){
        numberOfBlocksPerRewardUnitC4 = numberOfBlocksC4;
    }
    ...
    function setCoinAmountPerRewardUnitC4(uint256 coinAmountC4) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitC4 = coinAmountC4;
    }

    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "TENTA: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "TENTA: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "TENTA: update to zero address not possible");
        collectionAddressC3 = newAddressC3;
    }
	
	function setCollectionAddressC4(address newAddressC4) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC4 != address(0), "TENTA: update to zero address not possible");
        collectionAddressC4 = newAddressC4;
    }
	...
	function setStakingStartBlockC4(uint256 newStakingStartBlockC4) external onlyRole(CONTRACT_ADMIN_ROLE){
        stakingStartBlockC4 = newStakingStartBlockC4;
    }
}