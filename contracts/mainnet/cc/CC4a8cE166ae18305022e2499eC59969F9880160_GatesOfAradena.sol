	...
	ReentrancyGuard
{
	...
	uint256 public immutable COMMON_MAX_EARLY_ACCESS_SUPPLY;
	...
	constructor(
		...
		COMMON_MAX_EARLY_ACCESS_SUPPLY = _commonMaxEarlyAccessSupply;
	...
	function _validatePurchaseWindows(uint256 _commonTokenId, uint256 _amount) internal {
		...
		if (block.timestamp >= goldPassWindowOpens) {
			...
			require(
				_validateDiamondPass(_commonTokenId, _amount) > 0,
				"Diamond passes all redeemed"
			);
	...
	modifier validateEarlyAccessPurchaseAmount(uint256 _tokenId, uint256 _amount) {
		require(
			_tokenId + _amount <= COMMON_MAX_EARLY_ACCESS_SUPPLY,
