...
contract NFT1 is ERC721Enumerable, Ownable {
    ...
    uint16 public constant maxSupply = 10050;
    uint16 nextToken;
    ...
    function mint(address _to, uint256 _mintAmount) public payable notPaused {
        ...
        if (msg.sender != owner()) {
            if (isPremiumWhitelisted[_to]) {
                require(
                    _mintAmount == 1,
                    "You can only have one whitelisted mint"
                );
                require(
                    msg.value >= pWCost * _mintAmount,
                    "PremiumMint: Not enough ether"
                );
                ...
                require(
                    msg.value >= wCost * _mintAmount,
                    "WhitelistedMint: Not enough ether"
                );
                isWhitelisted[_to] = false;
            } else {
                require(
                    msg.value >= cost * _mintAmount,
                    "Mint: Not enough ether"
                );
            }
        }
    ...
    function setPremiumWhitelist(address[] memory _addresses, bool _bool)
        public
        onlyOwner
    {
    ...
    /// @dev argument should look like this: ["0x00", "0x00"]
    function setWhitelist(address[] memory _addresses, bool _bool)
        public
        onlyOwner
    {
        for (uint256 i; i < _addresses.length; i++) {
