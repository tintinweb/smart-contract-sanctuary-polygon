...
contract NFT is ERC1155SupplyUpgradeable, OwnableUpgradeable {
    ...
    function _getIdToMint(uint256 randomness) private view returns (uint256) {
        uint256[] memory raritySum = new uint256[](4);
        for (uint256 i = 1; i < 4; i++) {
            raritySum[i] = raritySum[i - 1] + rarities[i] * (maxSupplies[i] - totalSupply(i));
        }

        uint256 rand = randomness % raritySum[3];
        for (uint256 i = 1; i < 4; i++) {
            if (rand < raritySum[i]) {
                return i;
            }
