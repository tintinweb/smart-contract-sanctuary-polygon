...
contract KnowledgeToken is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) s_tokenToPublication;
    mapping(uint256 => string) s_tokenUri;

    // fees
    uint16 s_adminFee = 50;
    mapping(address => uint256) s_writerFeesAccrued;
    mapping(address => uint256) s_writerFeesWithdrawn;

    struct KnowledgeInfo {
        uint256 readDuration;
    }
