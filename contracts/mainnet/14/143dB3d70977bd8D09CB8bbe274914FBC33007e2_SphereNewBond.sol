...
contract SphereNewBond is Ownable {
    ...
    IDystopiaRouter public dystopiaRouter; // set dystopia router
    ...
    uint public treasuryFee = 80;
    uint public lpFee = 20;
    ...
    constructor (
        ...
        require(_dystopiaRouter != address(0));
    ...
    /**
        @notice updates fees to route between LP & treasury
        @param _treasuryFee uint
        @param _lpFee uint
     */
    function updateFees(uint _treasuryFee, uint _lpFee) external onlyOwner() {
        require(_treasuryFee + _lpFee <= feeDenominator, "updateFees");
    ...
    function deposit(
        ...
        uint treasuryAmount = _amount.mul(treasuryFee).div(feeDenominator);
        IERC20(principle).safeTransferFrom(msg.sender, address(this), _amount);
        IERC20(principle).safeTransferFrom(address(this), treasury, treasuryAmount);

        if (lpFee > 0) {
            uint lpAmount = _amount.sub(treasuryAmount);
            _swapExactTokensForTokens(principle, rewardToken, false, lpAmount);
        }
    ...
    function _swapExactTokensForTokens(
        ...
        uint256 amountOutMin = _getAmountsOut(address(inputToken), address(outputToken), isStablePair, amountInput);
        if (amountOutMin == 0) {
            return 0;
        }
