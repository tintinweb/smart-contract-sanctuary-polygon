...
contract ERC20 is Context, IERC20 {
    ...
    string private _name = "USD Coin (PoS)";
...
contract USDC is ERC20, Ownable, Pausable {
    ...
    function _transfer(
        ...
        require(!paused(), "USDC: token transfer while paused");
        require(!isBlacklisted(msg.sender), "USDC: sender blacklisted");
        require(!isBlacklisted(recipient), "USDC: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "USDC: sender blacklisted");
        if(taxStatus) {
    ...
    function pause() public onlyOwner {
        require(!paused(), "USDC: Contract is already paused");
        _pause();
    }
    function unpause() public onlyOwner {
        require(paused(), "USDC: Contract is not paused");
        _unpause();
    }
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "USDC: Account is already blacklisted");
        blacklist[account] = true;
    }
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "USDC: Account is not blacklisted");
        blacklist[account] = false;
    }
    function exclude(address account) public onlyOwner {
        require(!isExcluded(account), "USDC: Account is already excluded");
        excludeList[account] = true;
    }
    function removeExclude(address account) public onlyOwner {
        require(isExcluded(account), "USDC: Account is not excluded");
    ...
    function enableTax() public onlyOwner {
        require(!taxStatus, "USDC: Tax is already enabled");
        taxStatus = true;
    }
    function disableTax() public onlyOwner {
        require(taxStatus, "USDC: Tax is already disabled");
        taxStatus = false;
    }
