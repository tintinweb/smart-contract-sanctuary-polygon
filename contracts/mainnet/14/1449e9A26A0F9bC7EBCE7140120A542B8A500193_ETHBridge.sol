...
// File: EnvoyNew/Polygon/ETHBridge.sol
...
contract ETHBridge is IVoyBridge, Pausable, AccessControl {
    address public voyToken;

    bytes32 public constant OPERATOR_ROLE = keccak256("OPERATOR_ROLE");

    event SwapStarted(address _user, uint256 _amount);
    event SwapReturned(address _user, uint256 _amount, string _ethTxHash);
    ...
    function returnSwap(address _user, uint256 _amount, string memory _ethTxHash) external onlyRole(OPERATOR_ROLE) whenNotPaused {
        ERC20(voyToken).transfer(_user, _amount);
        emit SwapReturned(_user, _amount, _ethTxHash);
    }
