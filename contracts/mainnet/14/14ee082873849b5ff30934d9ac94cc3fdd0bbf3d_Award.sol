...
interface IAwardData {
    ...
    function SetAwardPools(uint256 _id, AwardPool[] memory _pools, address _creator) external;
...
contract Award is Permission {
    ...
    function CreateAwardPools(string memory _appId, uint256 _id, IAwardData.AwardPool[] memory _pools, uint256 _preGas) public payable onlyAdmin(_appId) {
        ...
        awardData.SetAwardPools(_id, _pools, msg.sender);
    }


    function ExecuteAward(string memory _appId, uint256 _id, address[][] memory _receivers) public {
        require(isOperator(_appId), "Only operator");
        ...
        for (uint i = 0; i < pools.length; i++) {
            require(_receivers[i].length <= pools[i].dropCount, "Receivers exceeded");
            if (pools[i].isNFT) {
                ...
                if (pools[i].isETH) {
                    for (uint j = 0; j < _receivers[i].length; j++) {
                        payable(_receivers[i][j]).transfer(pools[i].awardCount.div(_receivers[i].length));
                    }
                } else {
                    for (uint j = 0; j < _receivers[i].length; j++) {
                        pools[i].token.transfer(_receivers[i][j], pools[i].awardCount.div(_receivers[i].length));
                    }
                }
            }
        }
