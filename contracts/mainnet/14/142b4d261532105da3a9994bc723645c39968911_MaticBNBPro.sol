...
contract MaticBNBPro {
    ...
    ****************************************/
	...
	uint256 constant public minInvestmentAmount = 8 ether;
	...
	uint[5] public ref_bonuses = [14,8,7,6,5];
    
    uint256[5] public defaultPackages = [8 ether,16 ether,35 ether,75 ether,150 ether];
    ...
    constructor() public {
		primaryAdmin = 0x307CeedbcfC4B8B71045263965cC019ECb36F8d8;
  ...
  function _refPayout(address _addr, uint256 _amount) internal {
    ...
    function invest(address referrer) public payable {
		...
		require(msg.value >= minInvestmentAmount,'Min invesment 8 MATIC');
  ...
  function _usersDownlineIncomeDistribution(address _user, uint256 _Amount) internal {
      ...
      uint256 DistributionPayment = _Amount.mul(40).div(100);
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
      ...
      if( (TotalDeposit >=defaultPackages[1] && TotalDeposit < defaultPackages[1])){
          reivest = 55;
          withdrwal = 45;
      }
      else if(users[_user].refs[0] >=1 && (TotalDeposit >=defaultPackages[2] && TotalDeposit < defaultPackages[3])){
          reivest = 50;
          withdrwal = 50;
      }else if(users[_user].refs[0] >=2 && (TotalDeposit >=defaultPackages[3] && TotalDeposit < defaultPackages[4])){
          reivest = 40;
          withdrwal = 60;
