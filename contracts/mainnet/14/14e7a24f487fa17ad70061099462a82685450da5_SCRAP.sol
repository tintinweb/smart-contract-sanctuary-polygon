...
contract SCRAP is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("SCRAP", "SCRAP") AccessControl(){
        ...
        coinAmountPerRewardUnit = 8 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 41142; // 12 hours per reward unit , may be changed later on
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "SCRAP: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "SCRAP: update to zero address not possible");
