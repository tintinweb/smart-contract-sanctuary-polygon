...
contract Auction is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function createAuction(
        ...
        */
        require(_startTimestamp > block.timestamp, "invalid start time");
    ...
    function placeBid(
        ...
        require(_isContract(msg.sender) == false, "no contracts permitted");
        ...
        whenNotPaused
    {
        ...
        require(
            block.timestamp > _endTime && (block.timestamp - _endTime >= 43200),
        ...
        nonReentrant
    {
        ...
        uint256 erc2981feeAmount = 0;
        if (IERC165(_nftAddress).supportsInterface(INTERFACE_ID_ERC2981)) {
            address erc2981Receiver;
            (
            erc2981Receiver,
            erc2981feeAmount
            ) = IERC2981(_nftAddress).royaltyInfo(_tokenId, payAmount);

            if (erc2981feeAmount > 0) {
                if (erc2981feeAmount > payAmount) {
                    erc2981feeAmount = payAmount;
                }

                payAmount = payAmount - erc2981feeAmount;
    ...
    function updateAuctionStartTime(
        ...
        require(auction.startTime + 60 > block.timestamp, "auction already started");
     ...
     */
    function updatePlatformFee(uint256 _platformFee, address payable _platformFeeRecipient) external onlyOwner {
        platformFeeRecipient = _platformFeeRecipient;
