...
contract ILProtectionController is ILProtectionControllerInterface, BaseController {
  ...
  uint8 public constant MAX_PROTECTIONS_IN_UPKEEP_NUM = 255;
  ...
  function buyProtection(
    ...
    require(_lpTokensWorthAtBuyTimeUSD > 0, '_lpTokensWorthAtBuyTimeUSD value must be larger than 0');
  ...
  function closeProtections(uint256[] memory _protectionsIds) public override {
    ...
    for (uint256 i; i < _protectionsIds.length; i++) {
      ...
      if (protectionDetails.protectionEndTimestamp <= block.timestamp) {
        ...
        require(protectionWithMetadata.maxAmountToBePaid <= collateral, 'maxAmountToBePaid is larger then collateral');

        require(
          protectionDetails.lpTokensWorthAtBuyTimeUSD <= totalLPTokensWorthAtBuyTimeUSD,
          'lpTokensWorthAtBuyTimeUSD is larger then totalLPTokensWorthAtBuyTimeUSD'
        );
  ...
  function setILProtectionNFT(ILProtectionNFTInterface _newInstance)
    external
    override
    onlyAdmin
    onlyValidAddress(address(_newInstance))
  {
    emit ILProtectionNFTChanged(protectionNFT, _newInstance);

    protectionNFT = _newInstance;
  }
  ...
  function setMaxILProtectedPercentage(uint16 _maxILProtectedPercentage) external override onlyAdmin {
    require(openProtectionsIds.length == 0, 'Cannot change maxILProtectedPercentage with existing open protections');

    protectionConfig.setMaxILProtectedPercentage(_maxILProtectedPercentage);
  }

  function setMaxProtectionsInUpkeep(uint8 _maxProtectionsInUpkeep) external override onlyAdmin {
    require(
      _maxProtectionsInUpkeep > 0 && _maxProtectionsInUpkeep <= MAX_PROTECTIONS_IN_UPKEEP_NUM,
  ...
  function getCollateralRatioComponents() public view override returns (uint256, uint256) {
    return (collateral, liquidityController.liquidity());
  }
  ...
  function calculateParameterizedPremium(
    ...
    uint256 _liquidity,
    uint16 _maxILProtectedPercentage,
  ...
  function calculatePremiumAndMaxAmountToBePaid(
    ...
    uint256 maxAmountToBePaid = calcEstimatedAmountToBePaid(
      ...
      protectionConfig.maxILProtectedPercentage()
    );
  ...
  function calcMaxValueOfTokensWorthToProtect() public view override returns (uint256) {
    ...
    uint256 denominator = (tokensValueGrowth * MAX_PERCENTAGE) /
      (MAX_PERCENTAGE - protectionConfig.maxILProtectedPercentage()) -
      tokensValueGrowth;

    return ((liquidityController.liquidity() - collateral) * MAX_PERCENTAGE) / denominator;
  }

  function calcAmountToBePaidWithProtectionId(uint256 _protectionId) public view override returns (uint256) {
    require(openProtectionsWithMetadata[_protectionId].exists, "Protection is not open or doesn't exist");
  ...
  function calcAmountToBePaid(
    ...
    return (lpTokensWorthIfHeldUSD * _impermanentLoss) / MAX_PERCENTAGE;
  }
  ...
  function calcEstimatedAmountToBePaid(
    ...
    uint256 estimatedTokensWorthAtEnd = (_lpTokensWorthAtBuyTimeUSD * _expectedLPTokensValueGrowth) / MAX_PERCENTAGE;

    return
      (estimatedTokensWorthAtEnd * MAX_PERCENTAGE) / (MAX_PERCENTAGE - _impermanentLoss) - estimatedTokensWorthAtEnd;
  }
    ...
    )
  {
    ...
    uint16 maxImpermanentLoss = protectionConfig.maxILProtectedPercentage();
...
interface ILProtectionControllerInterface is IBaseController, KeeperCompatibleInterface {
  ...
  event ILProtectionNFTChanged(ILProtectionNFTInterface prevValue, ILProtectionNFTInterface newValue);
  ...
  function setILProtectionNFT(ILProtectionNFTInterface _newInstance) external;
  ...
  function setMaxILProtectedPercentage(uint16 _maxILProtectedPercentage) external;
  ...
  function getCollateralRatioComponents() external view returns (uint256, uint256);
    ...
    return
      uint16(
        (PRBMathUD60x18.SCALE - (2 * PRBMathUD60x18.sqrt(p).div(p + PRBMathUD60x18.SCALE))) /
          (PRBMathUD60x18.SCALE / maxPercentage)
      );
  }
}
...
import '../../cvi-contracts/CVIOracle.sol';
...
interface ILProtectionConfigInterface {
  ...
  event MaxILProtectedPercentageChanged(uint16 prevValue, uint16 newValue);
  ...
  event FeePercentageChanged(uint16 prevValue, uint16 newValue);
  ...
  function maxILProtectedPercentage() external view returns (uint16);
