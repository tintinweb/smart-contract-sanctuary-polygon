...
abstract contract BaseUniversalRegistrarController is Initializable {
    ...
    // Amount of shares of an individual.
    mapping(bytes32 => mapping(address => uint256)) private _shares;

    // Amount of Ether already released to an individual.
    mapping(bytes32 => mapping(address => uint256)) private _released;
    ...
    modifier authorized(bytes32 node) {
        require(msg.sender == registryPayee() || msg.sender == _payee[node], "Caller is not authorized!");
        _;
    }
    ...
    /**
     * @dev Getter for the amount of shares held by an individual
     */
    function shares(bytes32 node, address account) public view returns (uint256) {
        return _shares[node][account];
    }

    /**
     * @dev Getter for the amount of Ether already released to an individual
     */
    function released(bytes32 node, address account) public view returns (uint256) {
        return _released[node][account];
    }
    ...
    */
    function releasable(bytes32 node, address account) public view returns (uint256) {
        uint256 totalReceived = balance(node) + totalReleased(node);
        return _pendingPayment(node, account, totalReceived, released(node, account));
    }

    /**
     * @dev Triggers a transfer to an account for the amount of Ether they are owed, according to their
     * percentage of the total shares and their previous withdrawals.
    */
    function release(bytes32 node) public virtual authorized(node) {
        address payable account;
        uint256 payment;

        if (_payee[node] == address(0)) {
            account = payable(registryPayee());

            payment = balance(node);
        } else if(!(_payee[node] == address(0))) {
            account = payable(msg.sender);

            require(_shares[node][account] > 0, "Account has no shares");

            payment = releasable(node, account);
        }

        require(payment != 0, "account is not due payment");

        _released[node][account] += payment;
        _totalReleased[node] += payment;
        _balances[node] -= payment;

        AddressUpgradeable.sendValue(account, payment);
        emit PaymentReleased(node, account, payment);
    }
    ...
    function _pendingPayment(
        ...
        return (totalReceived * _shares[node][account]) / _totalShares - alreadyReleased;
    }
    ...
    */
    function setPayee(bytes32 node, address account, uint256 shares_) external onlyRegistry {
        _payee[node] = account;
        setShares(node, account, shares_);
    ...
    */
    function setShares(bytes32 node, address account, uint256 shares_) public onlyRegistry {
        require(account == _payee[node] || account == registryPayee(), "Invalid account!");

        if(account == registryPayee()) {
            _shares[node][account] = totalShares();
        } else {
            _shares[node][account] = shares_;
            _shares[node][registryPayee()] = totalShares() - shares_;
        }
    }
}
...
contract UniversalRegistrarController is Ownable, BaseUniversalRegistrarController {
    ...
    event NameRegistered(bytes32 indexed tld, string name, bytes32 indexed label, address indexed owner, uint cost);
    ...
    function registerWithConfig(bytes32 node, string memory name, address owner,
        ...
        uint cost = _consumeCommitment(node, name, commitment);
