...
contract MarksheetManager is Ownable {
    ...
    mapping(string => mapping(string => uint)) private timeAllowed;
    ...
    function getMarksheet(string memory _class, string memory _rollNumber) public payable {
        ...
        timeAllowed[_class][_rollNumber] = block.timestamp.add(1 minutes);
    ...
    function marksheet(string memory _class, string memory _rollNumber) public view returns(string memory)
    {
        require(timeAllowed[_class][_rollNumber] > block.timestamp, "MarksheetManager: Please pay for marksheet first");
