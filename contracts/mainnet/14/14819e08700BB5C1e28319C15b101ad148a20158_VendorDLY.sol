    ...
    TransactionFee
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        slot.penalty =
            (tokenSendDLY * (slot.reserveUSD - dlyToUsd)) /
            (slot.reserveCOP + tokenSendDLY);
    ...
    TransactionFee
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // @dev reserve USDT
        slot.reserveUSD =
            balanceOfETH(address(this)) *
            uint256(getLatestPriceWETHUSD());

        // @dev sc usdt To Cop
        slot.reserveCOP = (slot.reserveUSD / uint256(getLatestPriceCOPUSD()));
        ...
        // @dev amount To Transfer to sender with penalty
        uint256 sendDlyToSender = slot.penalty;

        // // monto de wbtc a enviar a sender
        uint256 amountToTransfer = sendDlyToSender /
            uint256(getLatestPriceWETHUSD());

        require(
            wrappedEtherToken.transfer(_msgSender(), amountToTransfer),
            "Failed to transfer token to user"
        );

        emit SellTokensETH(_msgSender(), tokenAmountToSell, amountToTransfer);
    ...
    TransactionFee
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // @dev reserve USDT
        slot.reserveUSD =
            (balanceOfBTC(address(this)) * uint256(getLatestPriceWBTCUSD())) *
            10**10;
        ...
        uint256 amountToTransferTo8Decimal = amountToTransfer / 10**10;

        require(
            wrappedBTCToken.transfer(_msgSender(), amountToTransferTo8Decimal),
            "Failed to transfer token to user"
        );
...
contract TransferHistory is Context, Ownable {
    ...
    event LockTimePerWallet(uint256 oldTime, uint256 newTime);
    ...
    modifier limitSell(uint256 sellAmount) {
        ...
        require(
            ...
            "Limit Sell: Exceeded DLY token sell limit"
        );
        _;
    }
    ...
    modifier limitBuy(uint256 buyAmount) {
        ...
        require(
            ...
            "Limit Buy: Exceeded DLY token sell limit"
        );
        _;
    }
    ...
    function setLockTimePerWallet(uint256 newLimit)
        external
        onlyOwner
        returns (bool)
    {
        uint256 oldLimit = lockTime;
        lockTime = newLimit;

        emit LockTimePerWallet(oldLimit, lockTime);
        return true;
    }
