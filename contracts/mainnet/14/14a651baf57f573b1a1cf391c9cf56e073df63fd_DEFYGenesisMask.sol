...
contract DEFYGenesisMask is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  address payable commissions = payable(0x5D77494107F0349B222c57B1591e80DfF49B6E6D);
...
function MINT_1_DGM_FOR_500_MATIC(uint256 _mintAmount) public payable {
    require(!paused, "the contract is paused");
    uint256 supply = totalSupply();
    require(_mintAmount > 0, "need to mint at least 1 NFT");
    require(_mintAmount <= maxMintAmount, "max mint amount per session exceeded");
    ...
    function WKaptainKrank_Holders(address payable[] memory _recipients) public payable onlyOwner {
    uint256 i = 0;
    uint256 supply = totalSupply();
    address rec;
