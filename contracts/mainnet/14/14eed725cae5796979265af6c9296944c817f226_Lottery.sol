...
contract Lottery is ReentrancyGuard {
    IERC20 public constant BTC_TOKEN = IERC20(0x1BFD67037B42Cf73acF2047067bd4F2C47D9BfD6);
    uint256 public ticketPrice = 0.00017 ether; // 0.00017 btc
    uint256 public maxTickets = 10000; // maximum tickets per lottery
    uint256 public ticketCommission = 0.00004 ether; // 0.00004 btc commition per ticket
...
function BuyTickets(address referral, uint256 _amount) public nonReentrant {
    ...
    require(
        BTC_TOKEN.transferFrom(msg.sender, address(this), totalCostWithCommissions),
    ...
    if (referral != address(0)) {
        require(
            BTC_TOKEN.transfer(referral, referralCommission),
            "Token transfer failed"
        );
    }
    ...
    if (ticketCommission > 0) {
        require(
            BTC_TOKEN.transfer(lotteryOperator, ticketCommission),
            "Token transfer failed"
        );
    }
}
...
function Verifications(address winner) public isOperator {
    ...
    BTC_TOKEN.transfer(winner, reward2Transfer);
}
    ...
    for (uint256 i = 0; i < tickets.length; i++) {
        ...
        tickets[i] = address(0);
        BTC_TOKEN.transfer(to, ticketPrice);
    }
    delete tickets;
}
