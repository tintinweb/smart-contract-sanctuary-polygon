...
contract PositionManager is BasePositionManager {
    ...
    function executeIncreasePositions(bytes32[] calldata _keys, address payable _executionFeeReceiver) external onlyOrderKeeper {
        for (uint i = 0; i < _keys.length; i++) {
            bytes32 key = _keys[i];

            try IPositionRouter(positionRouter).executeIncreasePosition(key, _executionFeeReceiver) returns (bool _wasExecuted) {
                if (!_wasExecuted) { break; }
            } catch {
                // wrap this call in a try catch to prevent invalid cancels from blocking the loop
                try IPositionRouter(positionRouter).cancelIncreasePosition(key, _executionFeeReceiver) returns (bool _wasCancelled) {
    ...
    function executeIncreasePosition(bytes32 _key, address payable _executionFeeReceiver) external onlyOrderKeeper {
        try IPositionRouter(positionRouter).executeIncreasePosition(_key, _executionFeeReceiver) returns (bool _wasExecuted) {
            if (!_wasExecuted) { return; }
        } catch {
            // wrap this call in a try catch to prevent invalid cancels from blocking the loop
            try IPositionRouter(positionRouter).cancelIncreasePosition(_key, _executionFeeReceiver) returns (bool) {
            } catch {}
        }
    }

    function executeDecreasePositions(bytes32[] calldata _keys, address payable _executionFeeReceiver) external onlyOrderKeeper {
        for (uint i = 0; i < _keys.length; i++) {
            bytes32 key = _keys[i];

            try IPositionRouter(positionRouter).executeDecreasePosition(key, _executionFeeReceiver) returns (bool _wasExecuted) {
                if (!_wasExecuted) { break; }
            } catch {
                // wrap this call in a try catch to prevent invalid cancels from blocking the loop
                try IPositionRouter(positionRouter).cancelDecreasePosition(key, _executionFeeReceiver) returns (bool _wasCancelled) {
                    if (!_wasCancelled) { break; }
                } catch {}
            }
        }
    ...
    function executeDecreasePosition(bytes32 _key, address payable _executionFeeReceiver) external onlyOrderKeeper {
        try IPositionRouter(positionRouter).executeDecreasePosition(_key, _executionFeeReceiver) returns (bool _wasExecuted) {
            if (!_wasExecuted) { return; }
        } catch {
            // wrap this call in a try catch to prevent invalid cancels from blocking the loop
            try IPositionRouter(positionRouter).cancelDecreasePosition(_key, _executionFeeReceiver) returns (bool) {
            } catch {}
        }
    }
    ...
    function liquidatePositions(
        ...
        for (uint i = 0; i < _accounts.length; i++) {
            ...
            try IVault(_vault).liquidatePosition(_accounts[i], _collateralTokens[i], _indexTokens[i], _isLongs[i], _feeReceiver) {} catch {}
