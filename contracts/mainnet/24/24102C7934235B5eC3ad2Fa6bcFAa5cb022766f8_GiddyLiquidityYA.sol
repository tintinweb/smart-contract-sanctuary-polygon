...
contract GiddyLiquidityYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external pure override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = shares + fap;
    priceImpact = 48500;
  }
  ...
  function withdrawGiddyCalc(uint256 shares, uint256 fap) external pure override returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = shares + fap;
    priceImpact = 48500;
  }
  ...
  function usdcToGiddy(uint256 amount) public view returns(uint256)
  {
  ...
  function usdcToGiddyLP(uint256 value) public view returns (uint256) {
    ...
    return (IUniswapV2Pair(GIDDY_USDC_PAIR).totalSupply() * value) / resUsdc;
  }

  function giddyLPToUsdc(uint256 value) public view returns (uint256) {
    (uint256 resUsdc,,) = IUniswapV2Pair(GIDDY_USDC_PAIR).getReserves();
    return (resUsdc * value) / IUniswapV2Pair(GIDDY_USDC_PAIR).totalSupply();
  }
