...
contract OptimisticOracleTest is Ownable {
    ...
    string private constant QUERY_TEMPLATE =
        "[TEST] Q:Are the results of the snapshot proposals in space https://snapshot.org/#/hw.alexanderem49.eth match vote results received in Request Transaction? Proposals MUST be created and closed before the Request Transaction block timestamp. Received data is emitted in the VoteResultReceived event in Request Transaction. Proposal category from its topic has to be received as well. Vote percentages are submitted as integers with 2 decimals (10000 - 100.00%, 1234 - 12.34%). Snapshot proposal ids: ";
    ...
    function submitVotes(VoteResult[] memory results) external onlyOwner {
        ...
        bytes32[] memory voteIds = new bytes32[](resultsLength);

        for (uint256 i = 0; i < resultsLength; i++) {
            voteIds[i] = results[i].voteId;

            emit VoteResultReceived(
                results[i].voteId,
                results[i].voteCategory,
                results[i].options,
                results[i].votePercentage
            );
        }
        bytes memory ancillaryData = createQuery(voteIds, resultsLength);
    ...
    function createQuery(
        bytes32[] memory voteIds,
        uint256 length
    ) private pure returns (bytes memory) {
        bytes memory query = abi.encodePacked(QUERY_TEMPLATE);

        for (uint256 i = 0; i < length; i++) {
            if (length == 1 || length - 2 == i) {
                query = abi.encodePacked(
                    query,
                    Strings.toHexString(uint256(voteIds[i])),
                    ". "
                );
                break;
            }
