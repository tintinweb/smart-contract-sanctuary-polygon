...
// File contracts/erc721/ContextMixin.sol
...
// File contracts/erc721/EIP712Base.sol
...
// File contracts/erc721/NativeMetaTransaction.sol
...
// File contracts/erc721/ERC721Core.sol

contract ERC721Core is ERC721, ContextMixin, NativeMetaTransaction, Ownable {
    using SafeMath for uint256;
    ...
    constructor(string memory name, string memory symbol, string memory tokenURIPrefix, string memory tokenURISuffix, uint256 maxSupply_) ERC721(name, symbol) {
    ...
    function mint(address to) public onlyOwner {
        require(_nextTokenId.current() < _maxSupply, "Can't minting over max supply");
        _safeMint(to, _nextTokenId.current());
        _nextTokenId.increment();
    }

    function mintBatch(address[] calldata tos) public onlyOwner {
        require((_nextTokenId.current() + tos.length) <= _maxSupply, "Can't minting over max supply");
        for (uint256 i = 0; i < tos.length; ++i) {
            _safeMint(tos[i], _nextTokenId.current());
            _nextTokenId.increment();
        }
    }
    ...
    function mintableSupply() external view returns (uint256) {
        if (_nextTokenId.current() < _maxSupply) {
            return _maxSupply - _nextTokenId.current();
        }
        return 0;
    }
    ...
    function isApprovedForAll(address owner, address operator) override public view returns (bool) {
        // NOTE: https://github.com/divergencetech/ethier/blob/main/contracts/thirdparty/opensea/OpenSeaGasFreeListing.sol
        // NOTE: https://docs.opensea.io/docs/polygon-basic-integration
        ...
        if (registry != address(0) && operator == registry) {
            return true;
        } else {
...
// File contracts/HorseOwners.sol

contract HorseOwners is ERC721Core {
    /* solhint-disable no-empty-blocks */
    constructor() ERC721Core(
        "711.0 Horse Owners",
        "HOR",
        "https://bafybeibechntqjsqk2qmc4uwljown4flws5uk3g2hw3namqfekfk5oo2q4.ipfs.nftstorage.link/json/",
        ".json",
        712
    ) { }
