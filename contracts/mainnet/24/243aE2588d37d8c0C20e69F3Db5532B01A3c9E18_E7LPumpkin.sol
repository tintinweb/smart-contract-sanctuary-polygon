...
contract E7LPumpkin is ERC721Linkable, ERC721Enumerable {
    using Strings for uint256;
    ...
    uint256[6] private probability = [1, 2, 2, 3, 3, 3];
    ...
    function _random(uint256 tokenId) private view returns (uint) {
        return uint(keccak256(abi.encodePacked(blockhash(25839542), tokenId)))%probability.length;
    }
        ...
        returns (string memory)
    {
        ...
        uint256 noRandom = _random(tokenId);

        return bytes(baseURI).length > 0
            ? string(abi.encodePacked(baseURI, probability[noRandom].toString(), baseExtension))
            : "";
    }
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal virtual override(ERC721Enumerable, ERC721Linkable) {
        ERC721Linkable._beforeTokenTransfer(from, to, tokenId, batchSize);
        ERC721Enumerable._beforeTokenTransfer(from, to, tokenId, batchSize);
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721Enumerable, ERC721Linkable) returns (bool) {
        return ERC721Enumerable.supportsInterface(interfaceId) || ERC721Linkable.supportsInterface(interfaceId);
    }
}
