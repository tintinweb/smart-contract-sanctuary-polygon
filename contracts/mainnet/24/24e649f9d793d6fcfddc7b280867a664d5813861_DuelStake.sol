...
contract DuelStake is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    uint256 public tokenPerSecond = 1e13;
    ...
    constructor(
        ...
        poolInfo.push(
            PoolInfo({
                ...
                accTokenPerShare: 0,
                totalAllocation: _endTime.sub(_startTime).mul(tokenPerSecond),
                totalReward: 0
            })
        );
        totalAllocPoint = 1;

        emit CreatePool(address(_stakingToken), address(mintable? _mintableRewardToken : _rewardToken), _endTime.sub(_startTime).mul(tokenPerSecond));
    }
    ...
    function setTokenPerSecond(uint256 _amount) external onlyOwner {
        ...
        require(_amount >= 1 * 1e13, "DuelStaking: Min per time 1 token");
    ...
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        ...
        if(mintable)
        {
            require(mintableRewardToken.transfer(_to, amount), "DuelStake: safeTokenTransfer error");
        }
        else
        {
            require(rewardToken.transfer(_to, amount), "DuelStake: safeTokenTransfer error");
        }
