...
import "./interfaces/IStrategyRebalanceStakerAlgebra.sol";
 ...
 * It also includes functions for determining tick values based on given inputs.
 */
contract AlgebraBalanceCalculator {
  IAlgebraCalculator public algebraCalculator = IAlgebraCalculator(0x87dF6956C24c38e0E986cfa907E2734761D15FE7);

  /**
   * @notice  Calculates balances of assets
   * @dev     Calcuates how much token0 / token1 exists in the contract
   * @param   _dysonStrategy Dyson strategy to calculate balances for
   * @return  _a0Expect  amount in token0 that is to be expected
   * @return  _a1Expect  amount in token1 that is to be expected
   */
  function getLiquidity(IStrategyRebalanceStakerAlgebra _dysonStrategy)
    public
    view
    returns (uint256 _a0Expect, uint256 _a1Expect)
  {
    IAlgebraPool _pool = IAlgebraPool(_dysonStrategy.pool());
    uint256 _liquidity = _dysonStrategy.liquidityOfPool();
    (uint160 _sqrtRatioX96, , , , , , ) = _pool.globalState();
    (_a0Expect, _a1Expect) = algebraCalculator.amountsForLiquidity(
      _sqrtRatioX96,
      uint128(_liquidity),
      _dysonStrategy.tick_lower(),
      _dysonStrategy.tick_upper()
    );
    _a0Expect += (IERC20(address(_dysonStrategy.token0())).balanceOf(address(this)));
    _a1Expect += (IERC20(address(_dysonStrategy.token1())).balanceOf(address(this)));
  }
}
...
interface IStrategyRebalanceStakerAlgebra {
  ...
  function ALGEBRA_ROUTER() external view returns (address);

  function MAX_PERFORMANCE_TREASURY_FEE() external view returns (uint256);

  function adjustHarvesters(address[] memory _harvesters, bool[] memory _values) external;

  function algebraEternalFarming() external view returns (address);

  function algebraFarmingCenter() external view returns (address);
  ...
  function getLiquidity() external view returns (uint256 _a0Expect, uint256 _a1Expect);
  ...
  function outputToAssetBonusRewardToken(uint256) external view returns (address);

  function outputToAssetRewardToken(uint256) external view returns (address);
  ...
  function setTickRangeMultiplier(int24 _tickRangeMultiplier, int24 _innerTickRangeMultiplier) external;
