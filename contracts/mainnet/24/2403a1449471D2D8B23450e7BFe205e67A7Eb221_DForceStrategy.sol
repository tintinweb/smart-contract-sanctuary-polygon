...
contract DForceStrategy is UpgradeableBase, IStrategy {
    ...
    function setRewardsToStrategyToken(
        ...
        require(path[0] == rewardsToken, "DF7");
        require(
            path[length - 1] == IiToken(strategyXToken).underlying(),
            "DF8"
        );
    ...
    function setStrategyTokenToBLID(
        ...
        uint256 length = path.length;
        require(length >= 2, "DF6");
        require(path[0] == IiToken(strategyXToken).underlying(), "DF7");
        require(path[length - 1] == blid, "DF8");
     ...
     * @param amount amount of min storage available for token using
     */
     ...
     * @param _borrowRateMin borrowLimit min : decimals = 18
     * @param _borrowRateMax borrowLimit max : deciamls = 18
     */
    function setRebalanceParameter(
        uint256 _borrowRateMin,
        uint256 _borrowRateMax
    ) external onlyOwner onlyStrategyPaused {
        ...
        onlyStrategyPaused
    {
        ...
        ILogic(_logic).addXTokens(IiToken(_xToken).underlying(), _xToken);
    ...
    function useToken() external override {
        ...
        address _strategyToken = IiToken(_strategyXToken).underlying();
    ...
    function rebalance() external override {
        ...
        uint256 borrowRate = (xTokenInfo.borrowAmount * BASE) /
            xTokenInfo.borrowLimit;
    ...
    function destroyAll() external override onlyOwnerAndAdmin {
        ...
        // RewardsTokenPrice Kill Switch
        bool rewardsTokenPriceKill = _rewardsPriceKillSwitch(
            strategyStatistics,
            _rewardsToken
        );
    ...
    function claimRewards() public override onlyOwnerAndAdmin {
        ...
        if (diff > 0 && rewardsTokenPriceKill == false) {
            ILogic(_logic).swap(
                swapRouter_RewardsToStrategyToken,
                IERC20MetadataUpgradeable(_rewardsToken).balanceOf(_logic),
                uint256(diff),
        ...
        // swap Rewards to BLID
        if (rewardsTokenPriceKill == false) {
            uint256 amountRewardsToken = IERC20MetadataUpgradeable(
                _rewardsToken
            ).balanceOf(_logic);
            ILogic(_logic).swap(
        ...
        onlyMultiLogicProxy
    {
        ...
        address _logic = logic;
        uint8 _circlesCount = circlesCount;
        require(token == IiToken(_strategyXToken).underlying(), "DF13");
    ...
    function destructCircles(
        ...
        for (uint256 i = 0; i < iterateCount; ) {
            ...
            uint256 repayAmount = IERC20Upgradeable(
                IiToken(xToken).underlying()
            ).balanceOf(_logic);
