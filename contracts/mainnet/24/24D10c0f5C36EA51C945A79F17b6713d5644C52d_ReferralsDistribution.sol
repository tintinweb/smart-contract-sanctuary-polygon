...
contract ReferralsDistribution is Initializable {
  ...
  mapping(address => mapping(address => AssetRewards)) private userRewards;
  ...
  function initialize(address _rewardToken, address _addressesProvider) public initializer {
  ...
  function setRewards(address[] memory users, address asset, uint256[] memory referralRewards, uint256[] memory cashbackRewards) external onlySuperOracle returns (uint256) {
    require(users.length == referralRewards.length && users.length == cashbackRewards.length, "Array lengths mismatch");
    uint256 totalRewards;
    for (uint256 i = 0; i < users.length; i++) {
      if (userRewards[users[i]][asset].referralRewards == 0 && userRewards[users[i]][asset].cashbackRewards == 0) {
        userAssets[users[i]].push(asset);
      }

      userRewards[users[i]][asset].referralRewards += referralRewards[i];
      userRewards[users[i]][asset].cashbackRewards += cashbackRewards[i];
      totalRewards += referralRewards[i] + cashbackRewards[i];
    }
    rewardToken.safeTransferFrom(msg.sender, address(this), totalRewards);
    return totalRewards;
  }

  function claimReward(address asset, RewardType rewardType) external returns (uint256) {
    AssetRewards storage rewards = userRewards[msg.sender][asset];
    uint256 rewardAmount;

    if (rewardType == RewardType.Referral) {
      rewardAmount = rewards.referralRewards;
      rewards.referralRewards = 0;
    } else if (rewardType == RewardType.Cashback) {
      rewardAmount = rewards.cashbackRewards;
      rewards.cashbackRewards = 0;
    } else if (rewardType == RewardType.All) {
      rewardAmount = rewards.referralRewards + rewards.cashbackRewards;
      rewards.referralRewards = 0;
      rewards.cashbackRewards = 0;
    } else {
    ...
    // Clear asset from userAssets if both rewards are 0
    if (rewards.referralRewards == 0 && rewards.cashbackRewards == 0) {
      _removeAsset(msg.sender, asset);
    }
  ...
  function claimAllRewards() external {
    ...
    uint256 i = 0;
    while (i < userAssets[msg.sender].length) {
      address asset = userAssets[msg.sender][i];
      AssetRewards storage rewards = userRewards[msg.sender][asset];
      totalRewards += rewards.referralRewards + rewards.cashbackRewards;
      rewards.referralRewards = 0;
      rewards.cashbackRewards = 0;
  ...
  function getUserRewardPerAsset(address user, address asset) external view returns (AssetRewards memory) {
    AssetRewards memory rewards = userRewards[user][asset];
    return rewards;
  }

  function getUserRewards(address user) external view returns (address[] memory, AssetRewards[] memory) {
    address[] memory assets = userAssets[user];
    AssetRewards[] memory rewards = new AssetRewards[](assets.length);
    for (uint256 i = 0; i < assets.length; i++) {
      rewards[i] = userRewards[user][assets[i]];
    }
    return (assets, rewards);
  }

  function emergencyWithdraw(address token, uint256 amount) external onlySuperOracle {
    IERC20(token).transfer(msg.sender, amount);
  }

  function setRewardToken(address _rewardToken) external onlySuperOracle {
    rewardToken = IERC20(_rewardToken);
  }

  function _removeAsset(address user, address asset) private {
