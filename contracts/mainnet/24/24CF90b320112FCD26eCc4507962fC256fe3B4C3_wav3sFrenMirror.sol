...
contract wav3sFrenMirror {
    ...
    modifier onlyWav3sHub() {
        require(msg.sender == s_wav3sHub, 'Errors.Only wav3sHub can call this function.');
        _;
    }
    ...
    function processMirror(
        ...
        s_appToCurrencyToFees[appAddress][currency] += dataByFrenFundedPublication[pubId]
            .feePerMirror;
    ...
    function fundMirror(
        ...
        dataByFrenFundedPublication[pubId].feePerMirror = (((fees * 2) / (5 + appFee)) /
            (dataByFrenFundedPublication[pubId].budget / reward));
    ...
    function getPubData(string memory pubId) public view returns (FrenFundedPostData memory) {
        // Get budget for this publication
     ...
     */
    function withdrawMirrorBudget(string memory pubId /*, uint256 amount*/) public stopInEmergency {
    ...
    function withdrawPub(string memory pubId) public onlyInEmergency onlyWav3sHub {
        // Check pubid validity
        require(
            bytes(pubId).length != 0,
    ...
    function withdrawAppFee(address appAddress) public onlyInEmergency onlyWav3sHub {
    ...
    function payCurrencyEmergency(address appAddress, address currencyAddress) internal {
        if (s_appToCurrencyToFees[appAddress][currencyAddress] > 0) {
            IERC20(currencyAddress).safeTransferFrom(
                address(this),
                msg.sender,
            ...
            emit Events.wav3sFrenMirror__EmergencyAppFeeWithdraw(
                appAddress,
                currencyAddress,
                s_appToCurrencyToFees[appAddress][currencyAddress]
            );
    ...
    function fundMirrorInEmergency(
        ...
        // Check if the budget is enough for the reward
        require(
            dataByFrenFundedPublication[pubId].reward <= dataByFrenFundedPublication[pubId].budget,
        ...
        address currencyAddress = dataByFrenFundedPublication[pubId].currencyAddress;

        // Transfer funds from the budget owner to wav3s contract
        IERC20(currencyAddress).safeTransferFrom(msg.sender, address(this), budget);
