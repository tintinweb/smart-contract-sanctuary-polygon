...
contract Cubix is Ownership {
    ...
    constructor(address _feeAddress, address _companyAddress, address _cubixaddress, address _usdt){
        ...
        PKGDetail.push(pkgDetail("MegaPack",5000,343,1)); 
        PKGDetail.push(pkgDetail("Limited Edition",7500,650,1)); 
    }
    ...
    function register(address referrerAddress, address placement, uint256 usdAmount, uint pkgIndex, uint _position, bool isUSDT) external returns(bool)
    {
        ...
        require(UserInfo[placement].uID > 0 || placement == defaultreferrer,"Incorrect placement");
        ...
        emit JoinEV(msg.sender, UserInfo[msg.sender].uID, referrerAddress, UserInfo[referrerAddress].uID, placement, UserInfo[placement].uID, tokenAmount, usdAmount, tokenPrice, _position, block.timestamp);                                 
    ...
    {      
        ...
        if(referrerAddress == address(0) || referrerAddress == _user)
        {
            referrerAddress = defaultreferrer ;
        }
        ...
        emit JoinEV(_user, UserInfo[_user].uID, referrerAddress, UserInfo[referrerAddress].uID, placement, UserInfo[placement].uID, tokenAmount, usdAmount, tokenPrice, _position, block.timestamp);                                 
    ...
    {          
        ...
        emit DepositEV(_user, tokenAmount, usdAmount, tokenPrice, block.timestamp);        
        return true;
    }
    ...
    function updateCubixPrice(uint256 usdPrice) external onlyAuthorized
    {
