...
contract DystopiaStrategy is InitializableAbstractStrategy, DystopiaExchange, BalancerExchange  {
    ...
    function _deposit(address _asset, uint256 _amount)  internal {
        ...
        if (token1Balance > minThresholds[1]) {
            amount0From1 = _onSwap(address(token1), address(token0), token1Balance);
        }
        ...
        _swap(address(token0), address(token1), amountToken0ToSwap);
        ...
        returns (uint256 balance)
    {
        ...
        uint256 PSBalanceFromToken0;
        if ( (address(token0) != address(primaryStable))  ) {
            if (token0Balance > minThresholds[0]) {
                PSBalanceFromToken0 = _onSwap(address(token0), address(primaryStable), token0Balance);
                // console.log("Token0 swap -  PSBalanceFromToken0 ", PSBalanceFromToken0);
            }
        } else {
            PSBalanceFromToken0 += token0Balance;
        }

        uint256 PSBalanceFromToken1;
        if ( (address(token1) != address(primaryStable))  ) {
            if (token1Balance > minThresholds[1]) {
                PSBalanceFromToken1 = _onSwap(address(token1), address(primaryStable), token1Balance);
                // console.log("Token1 swap -  PSBalanceFromToken1 ", PSBalanceFromToken1);
            }
        } else {
            PSBalanceFromToken1 += token1Balance;
        }
        uint256 total = PSBalanceFromToken0 + PSBalanceFromToken1;
        if (address(token0) != address(primaryStable) && address(token1) != address(primaryStable)) {
            total += primaryStable.balanceOf(address(this));
        }
         // console.log("PSBalanceFromToken0: ", PSBalanceFromToken0);
         // console.log("PSBalanceFromToken1: ", PSBalanceFromToken1);
        return total;
    }
    ...
    function _swapAssetsToPrimaryStable() internal {
        if ( (address(token0) != address(primaryStable)) && (token0.balanceOf(address(this)) > minThresholds[0]) )  {
            ...
            _swap(address(token0), address(primaryStable), token0.balanceOf(address(this)));
        }
        if ( (address(token1) != address(primaryStable)) && (token1.balanceOf(address(this)) > minThresholds[1])  )  {
             // console.log("Swapping token1");
            _swap(address(token1), address(primaryStable), token1.balanceOf(address(this)));
        }
    }
    function _swapPrimaryStableToToken0() internal {
        uint256 primaryStableBalance = primaryStable.balanceOf(address(this));
        if ((address(primaryStable) != address(token0)) && (primaryStableBalance > minThresholds[2])) {
            _swap(address(primaryStable), address(token0), primaryStableBalance);
        }
    }

    function _swap(address from, address to, uint256 balance) internal returns (uint256) {
        return swap(
            balancerVault,
            poolId,
    ...
    function _onSwap(address from, address to, uint256 balance) internal view returns (uint256) {
        return onSwap(
            balancerVault,
            poolId,
            IVault.SwapKind.GIVEN_IN,
