...
// File: BarPool/interfaces/IVBar.sol
...
// File: BarPool/interfaces/IBoostContract.sol
...
// File: BarPool/interfaces/IMasterChefV2.sol
...
// File: BarPool/contracts/utils/Address.sol
...
// File: BarPool/contracts/token/ERC20/IERC20.sol
...
// File: BarPool/contracts/token/ERC20/utils/SafeERC20.sol
...
// File: BarPool/contracts/utils/Context.sol
...
// File: BarPool/contracts/security/Pausable.sol
...
// File: BarPool/contracts/access/Ownable.sol
...
contract BarPool is Ownable, Pausable {
    ...
    event NewVBarContract(address VBar);
     ...
     * @param _pid: bar pool ID in MasterChefV2
     */
    constructor(
        IERC20 _token,
    ...
    function init(IERC20 dummyToken) external onlyOwner {
        ...
        masterchefV2.deposit(barPoolPID, balance);
        emit Init();
    }
    ...
    /**
     * @notice Checks if the msg.sender is either the bar owner address or the operator address.
     */
    modifier onlyOperatorOrBarOwner(address _user) {
        require(msg.sender == _user || msg.sender == operator, "Not operator or bar owner");
        _;
    }
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IBoostContract(boostContract).onBarPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    /**
     * @notice Unlock user bar funds.
     * @dev Only possible when contract not paused.
     * @param _user: User address
     */
    function unlock(address _user) external onlyOperatorOrBarOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
    ...
    /**
     * @notice Deposit funds into the Bar Pool.
     * @dev Only possible when contract not paused.
    ...
    function depositOperation(
        ...
        if (VBar != address(0)) {
            IVBar(VBar).deposit(_user, _amount, _lockDuration);
        }
        ...
        user.barAtLastUserAction = (user.shares * balanceOf()) / totalShares - user.userBoostedShare;
    ...
    /**
     * @notice Withdraw funds from the Bar Pool.
    ...
    function withdrawOperation(uint256 _shares, uint256 _amount) internal {
        ...
        if (VBar != address(0)) {
            IVBar(VBar).withdraw(msg.sender);
        }
    ...
    /**
     * @notice Harvest pending BAR tokens from MasterChef
     */
    function harvest() internal {
        uint256 pendingBar = masterchefV2.pendingBar(barPoolPID, address(this));
        if (pendingBar > 0) {
            uint256 balBefore = available();
            masterchefV2.withdraw(barPoolPID, 0);
    ...
    /**
     * @notice Set VBar Contract address
     * @dev Callable by the contract admin.
     */
    function setVBarContract(address _VBar) external onlyAdmin {
        require(_VBar != address(0), "Cannot be zero address");
        VBar = _VBar;
        emit NewVBarContract(VBar);
    }
    ...
    /**
     * @notice Withdraw unexpected tokens sent to the Bar Pool
     */
    ...
    function calculatePerformanceFee(address _user) public view returns (uint256) {
        ...
        if (user.shares > 0 && !user.locked && !freePerformanceFeeUsers[_user]) {
            uint256 pool = balanceOf() + calculateTotalPendingBarRewards();
     ...
     * @return Returns total pending bar rewards
     */
    function calculateTotalPendingBarRewards() public view returns (uint256) {
        uint256 amount = masterchefV2.pendingBar(barPoolPID, address(this));
        return amount;
    }

    function getPricePerFullShare() external view returns (uint256) {
        return totalShares == 0 ? 1e18 : (((balanceOf() + calculateTotalPendingBarRewards()) * (1e18)) / totalShares);
    }
