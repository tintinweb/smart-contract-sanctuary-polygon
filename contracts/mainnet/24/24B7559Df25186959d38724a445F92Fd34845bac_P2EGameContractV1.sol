...
contract P2EGameContractV1 is Initializable {
    ...
    function initialize(address _token, address _dev) external initializer {
        ceoAddress = msg.sender;
        smartContractAddress = address(this);
        devAddress = _dev;
        Token = IERC20Upgradeable(_token); //TOken Contract Address
        withdrawFee = 1;
        depositFee = 1;
    }
    ...
    function depositToken(uint256 amount) public {
        require(initialized);
        uint256 toPlayer = (amount * (100 - depositFee)) / 100;
    ...
    function emergencyWithdrawToken(address _adr) public {
        require(msg.sender == ceoAddress, "Error: Caller Must be Ownable!!");
        uint256 bal = IERC20Upgradeable(_adr).balanceOf(address(this));
        IERC20Upgradeable(_adr).transfer(msg.sender, bal);
    } 
    ...
    function withdrawToken(uint256 amount) public {
        ...
        uint256 toPlayer = (amount * (100 - withdrawFee)) / 100;
        uint256 fee = (amount * withdrawFee) / 100;

        IERC20Upgradeable(Token).transfer(account, toPlayer);
