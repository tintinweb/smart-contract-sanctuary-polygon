...
contract Harmonize is TuningEngine {    
    ...
    function encodedTokenImage(uint256 tokenId, Seed memory flower) public view returns (bytes memory) {  
        return abi.encodePacked("data:image/svg+xml;base64,",Base64.encode(Renderable(
            assignments[tokenId].exists ? 
            renderers[assignments[tokenId].key].renderer: 
            renderers[BASE_RENDER_KEY].renderer
        ).svg(flower)));        
    }
    ...
    function contractURI() public view virtual override returns (string memory) { 
        ...
        bytes memory dataURI = abi.encodePacked(
            '{',
                ...
                ',"image":"',string(Renderable(renderers[BASE_RENDER_KEY].renderer).svg(Seed(226,90,52,4,0,0,0,0))),'"',
        ...
        return flower.pick == TETRAD ? 
            ...
            flower.pick == TETRAD ? 
                ...
                flower.negated > 0 ? 
                    flower.light-15:
                    flower.light, 5)):
            monochrome(self,Color(flower.hue, 
                flower.muted > 0 ? 
                    flower.sat-30:
                    flower.sat,
                flower.negated > 0 ? 
