    ...
    AccessControlEnumerableUpgradeable
{
    ...
    uint64 public constant MAX_FIRST_OFFERING_SUPPLY = 100;
    ...
    // treasury contract address for verify
    ...
    function initialize() public initializer {
        ...
        __ERC721_init_unchained("MyFantasyWar", "MFWNFT");
    ...
    function mintDrop() public payable {
        ...
        uint256 tokenId = dropList[currentSession][randomPosition];
        _safeMint(_msgSender(), tokenId);
        ...
        tokens[0] = tokenId;
        IMarketplace(marketplaceAddress).setLegend(tokens, true, address(this));
    }

    function mintHero(address _user, uint256 tokenId) public onlyOwner {
        _mint(_user, tokenId);
    }

    function mintChestHero(address _user, uint256[] calldata _tokenIds)
        external
    {
        require(_msgSender() == treasuryContractAddress, "INVALID_TREASURY");
        require(_tokenIds.length <= 10, "INVALID_AMOUNT");
    ...
    function mintBox(address _user, uint256 _tokenId) external {
        require(
            whitelistBoxMinters[_msgSender()].total > 0,
            "You are not minter"
        );
        require(
            whitelistBoxMinters[_msgSender()].total >
                whitelistBoxMinters[_msgSender()].totalMinted,
            "You minted all"
        );
        require(
            whitelistBoxMinters[_msgSender()].startBlock <= block.number,
            "Mint has not started"
        );
        require(
            whitelistBoxMinters[_msgSender()].endBlock >= block.number,
            "Mint has ended"
        );
        whitelistBoxMinters[_msgSender()].totalMinted = whitelistBoxMinters[
            _msgSender()
        ].totalMinted.add(1);
    ...
    function setTreasuryContract(address _treasury) external onlyOwner {
        ...
        treasuryContractAddress = _treasury;
    }
     ...
     */
    function whitelist(address _minter, uint256 tokenId) public {
        require(hasRole(OPERATOR_ROLE, msg.sender), "Caller is not a operator");
        require(
    ...
    function updateMarketplace(address _address) external onlyOwner {
