...
abstract contract SignerRole is Context, Ownable {
  ...
  function isSigner(address account) public view returns (bool) {
    require(account != address(0), "SignerRole: account is the zero address");
    return _signers[account] == true;
  }

  function addSigner(address account) public onlyOwner {
    require(!isSigner(account), "Roles: account already has role");
    _addSigner(account);
  }

  function removeSigner(address account) public onlyOwner {
    require(isSigner(account), "SignerRole: account does not have role");
...
import "@openzeppelin/contracts/utils/cryptography/ECDSA.sol";
import "@openzeppelin/contracts/utils/cryptography/draft-EIP712.sol";
import "../lib/contracts/SignerRole.sol";
import "../lib/contracts/PauserRole.sol";
import "../token/CarbonToken.sol";

/**
 * @title Seller
 * Seller - a sales contract for CryptoArte non-fungible tokens 
 */
contract Seller is Ownable, SignerRole, PauserRole, Pausable, EIP712 {
  ...
  */
  ...
  whenNotPaused {
    ...
    require(isSigner(signer), "Signature invalid or unauthorized");

    // make sure that the redeemer is paying enough to cover the buyer's cost
    require(msg.value >= voucher.minPrice, "Insufficient funds to redeem");

    // transfer tokenы to buyer
  ...
  /**
   * @notice Redeems an MintVoucher for an actual lazy minted NFT.
   */
  function mintNFT(
  ...
  whenNotPaused {
    // make sure signature is valid and get the address of the signer
    address signer = _verifyMint(voucher);
    // check if voucher has been expired
    require(voucher.expiredAt == 0 || voucher.expiredAt >= block.timestamp, "Voucher has been expired");
    // check if voucher already used
    require(_usedVouchers[voucher.voucherId] == false, "Voucher already used");
    // make sure that the signer is contract's owner
    require(isSigner(signer), "Signature invalid or unauthorized");
    ...
    _token.mint(voucher.to, voucher.tokenId, 1, voucher.tokenURI);
    ...
    if (msg.value > 0) {
      payable(owner()).transfer(msg.value);
      emit Withdraw(address(owner()), msg.value, address(this).balance);
    }
  }

  function safeBatchMint(address _to, uint256[] calldata _ids, string[] calldata _uris) public onlySigner {
    batchMint(_to, _ids, _uris);
  }

  function batchMint(address _to, uint256[] calldata _ids, string[] calldata _uris) internal {
    for (uint256 i = 0; i < _ids.length; ++i) {
      // mint token to buyer
      _token.mint(_to, _ids[i], 1, _uris[i]);
    }
  }
  ...
  public
  onlySigner {
    _token.safeBatchTransferFrom(_from, _to, _ids, _values, "0x01");
  }
  ...
  */
  ...
  function _hash(NFTVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("NFTVoucher(string voucherId,uint256[] tokenIds,uint256[] counts,uint256 minPrice,address buyer,uint256 expiredAt)"),
      keccak256(abi.encodePacked(voucher.voucherId)),
      keccak256(abi.encodePacked(voucher.tokenIds)),
      keccak256(abi.encodePacked(voucher.counts)),
      voucher.minPrice,
      voucher.buyer,
      voucher.expiredAt
    )));
   ...
   */
  function _hashMint(MintVoucher calldata voucher) internal view returns (bytes32) {
    return _hashTypedDataV4(keccak256(abi.encode(
      keccak256("MintVoucher(string voucherId,address to,uint256 tokenId,string tokenURI,uint256 minPrice,uint256 expiredAt)"),
      keccak256(abi.encodePacked(voucher.voucherId)),
      voucher.to,
      voucher.tokenId,
   ...
   * @dev Will revert if the signature is invalid. Does not verify that the signer is authorized to transfer NFTs.
   */
  function _verifyMint(MintVoucher calldata voucher) internal view returns (address) {
    bytes32 digest = _hashMint(voucher);
    return ECDSA.recover(digest, voucher.signature);
  }
}
