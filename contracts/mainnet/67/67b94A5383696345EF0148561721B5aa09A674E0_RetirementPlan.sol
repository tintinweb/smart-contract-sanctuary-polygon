...
/// @title A token distributor for retired pizzerias
/// @author Radikal Riders
/// @notice This contract distributes tokens to owners of retired pizzerias
/// @dev Only Radikal Ridkers team can use this contract
contract RetirementPlan is Ownable {
  ...
  constructor (
    address _pizzeriasAddress,
    address _rdkTokenAddress) {
    pizzeriasInstance = ICollectionPizzerias(_pizzeriasAddress);
    rdkToken = ERC20RDK(_rdkTokenAddress);
  }
  
  event pensionPayed(address indexed user, uint distributedTokens, uint totalDistributed, uint timeStamp);
  
  /// @notice Send tokens to owners of retired pizzerias
  /// @dev The quantity of tokens sent to an owner are proportional to the tip power of the pizzeria of that owner
  /// @custom:frequency ideally this function should be executed once a month by the Radikal team and their are in charge of keeping this with sufficient token funds before distributing these
  function pensionPay() external onlyOwner {
    uint[] memory retiredPizzerias = pizzeriasInstance.getRetiredPizzeriasList();
    uint totalTipPower = pizzeriasInstance.getTotalRetiredPizzeriasTP();
    uint retiredPizzeriasLenght = retiredPizzerias.length;
    require(retiredPizzeriasLenght > 0, "Retirement Plan: there are no retired pizzerias");
    uint tokensToDistribute = rdkToken.balanceOf(address(this));
    bool sent;
    address _user;
    uint _tokensToDistribute;
    for(uint i = 0; i < retiredPizzeriasLenght; i++) {
      _user = pizzeriasInstance.extOwnerOf(retiredPizzerias[i]);
      _tokensToDistribute = tokensToDistribute * pizzeriasInstance.getTipPower(retiredPizzerias[i]) / totalTipPower;
      sent = rdkToken.transfer(_user, _tokensToDistribute);
      require(sent, "Retirement Plan: rdk transfer failed");
      emit pensionPayed(_user, _tokensToDistribute, tokensToDistribute, block.timestamp);
    }
  }
