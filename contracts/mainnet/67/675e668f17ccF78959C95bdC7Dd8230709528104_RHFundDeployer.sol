...
contract RHFundDeployer is Ownable, IRHFundDeployer {
    ...
    mapping(uint256 => address) internal _funds;
    mapping(address => bool) internal _fundDeployed;

    /**
     * @dev new fund deployed event
    ...
    // event NewFundCounter(uint256 newCounter);
    ...
    * @param _deplFacAddr deployer facility address contract to add
    */
    ...
    * @param _wlAddr WL deployer contract address to add
    */
    ...
    * @return wlDeplContract address of WL deployer contract
    */
    ...
    /**
    * @dev set deployed fund contract counter. Emergency only function (onlyOwner)
    * @param _newValue new counter value
    */
    function setFundCounter(uint256 _newValue) external override onlyOwner {
        fundCounter = _newValue;
        // emit NewFundCounter(fundCounter);
    }

    /**
    * @dev get deployed fund contract counter
    * @return fundCounter number of deployed fund contracts
    */
    function getDeployedFundCounter() external override view returns(uint256) {
        return fundCounter;
    }

    /**
    * @dev get deployed fund contract address as an item of an array
    * @param idx element index in funds array
    * @return _funds[idx] idx-th fund contract address
    */
    function getDeployedFundsAddress(uint256 idx) external override view returns(address) {
        return _funds[idx];
    }

    /**
    * @dev check if an fund contract address was deployed by this deployer
    * @param _fundAddr address to check
    * @return _fundDeployed[_fundAddr] true if fund address was deployed, otherwise false
    */
    function isFundDeployed(address _fundAddr) external override view returns (bool) {
        return _fundDeployed[_fundAddr];
    }

    /**
    * @dev add deployed fund contract address to internal variables
    * @param newFundToAdd fund contract address to add
    */
    function addFundContractAddress(address newFundToAdd) internal {
        _funds[fundCounter] = newFundToAdd;
        fundCounter = fundCounter.add(1);
        _fundDeployed[newFundToAdd] = true;
    }

    /**
    * @dev deploy a new fund contract, add its address to internal variables (onlyDeplFacility)
    ...
    */
    function deployFund(address _initialOwner,
            ...
            string memory _physicalAddressOfOperation) external override onlyDeplFacility returns (address) {
        RHFund newFund = new RHFund(_initialOwner, address(assetDeplContract), address(wlDeplContract),
                _name, _vatNumber, _companyRegNumber, _stateOfIncorporation, _physicalAddressOfOperation);
        addFundContractAddress(address(newFund));
        newFund.transferOwnership(_initialOwner);
        emit FundCreated(address(newFund), _name);
