...
contract BearMaintenanceFees is Ownable {
    ...
    uint public monthlyTributeFee = 1 ether;
    ...
    constructor(address _babyBears, address _honeyContract, address _charity, address _rewardPool, address _treasuryPool, uint _deploymentTime) {

        babyBears = IBabyBearsContract(_babyBears);
    ...
    function claimRewards(uint[] calldata ids, uint percentToCharity) external {
        ...
        for(uint i = 0; i < ids.length; i++) {
            ...
            require(babyBears.ownerOf(id) == msg.sender, "Reward claimer is not the owner of the NFT");
    ...
    function setLastClaimed(TimeData[] calldata timeDataArray) external {
        ...
        for(uint i = 0; i < timeDataArray.length; i++) {
            ...
            if (data.isMint) {
                ...
                require(babyBears.ownerOf(data.id) != address(0), "Token doesn't exist");
                ...
                if(data.time < lastClaimedTimestamp[data.id] || lastClaimedTimestamp[data.id] == 0) {
                    continue;
                }
            }
    ...
    */
    function setInitialEmissionRates(uint[] calldata _ranges, uint[] calldata _rates) onlyOwner external {

        require(setup, "Can only be called in setup");

        require(_ranges.length == _rates.length, "rates and ranges should be the same length");

        emissionRates.push(EmissionRate(_rates, _ranges, deploymentTime));
