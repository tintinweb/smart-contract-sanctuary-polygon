...
// import {IDGameToken} from "./interfaces/IDGameToken.sol";


/**
 * @title TokenDistributor
 * @notice It handles the distribution of DGM token.
 * It auto-adjusts block rewards over a set number of periods.
 */
contract TokenDistributor is ReentrancyGuard {
    using SafeERC20 for IERC20;
    // using SafeERC20 for IDGameToken;
    ...
    /**
     * @notice Constructor
     * @param _dgameToken DGM token address
     * @param _tokenRewards DGM token rewards address
    ...
    constructor(
        ...
        // uint256 nonCirculatingSupply = IDGameToken(_dgameToken).SUPPLY_CAP() -
        //     IDGameToken(_dgameToken).totalSupply();
     ...
     * @param amount amount to deposit (in DGM)
     */
    function deposit(uint256 amount) external nonReentrant {
        ...
        // Transfer DGM tokens to this contract
        dgameToken.safeTransferFrom(msg.sender, address(this), amount);
    ...
    function withdraw(uint256 amount) external nonReentrant {
        ...
        // Transfer DGM tokens to the sender
    ...
    function withdrawAll() external nonReentrant {
        ...
        // Transfer DGM tokens to the sender
        dgameToken.safeTransfer(msg.sender, amountToTransfer);
    ...
    function _updatePool() internal {
        ...
        if (tokenRewardForStaking > 0) {
            ...
            // bool mintStatus = dgameToken.mint(address(this), tokenRewardForStaking);
            // if (mintStatus) {
            //     accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            // }
            // dgameToken.mint(tokenSplitter, tokenRewardForOthers);

            bool mintStatus = tokenRewards.mint(address(this), tokenRewardForStaking);
            if (mintStatus) {
                accTokenPerShare = accTokenPerShare + ((tokenRewardForStaking * PRECISION_FACTOR) / totalAmountStaked);
            }
            tokenRewards.mint(tokenSplitter, tokenRewardForOthers);
        }
...
/**
 * @title TokenSplitter
 * @notice It splits DGM to team/treasury/trading volume reward accounts based on shares.
 */
contract TokenSplitter is Ownable, ReentrancyGuard {
    ...
    // Total DGM tokens distributed across all accounts
     ...
     * @param _dgameToken address of the DGM token
     */
    constructor(
    ...
    /**
     * @notice Release DGM tokens to the account
    ...
    function releaseTokens(address account) external nonReentrant {
        ...
        uint256 totalTokensReceived = dgameToken.balanceOf(address(this)) + totalTokensDistributed;
        ...
        dgameToken.safeTransfer(account, pendingRewards);
    ...
    /**
     * @notice Retrieve amount of DGM tokens that can be transferred
     * @param account address of the account
     */
...
// File: contracts/StakingPoolForUniswapV2Tokens.sol
 ...
 * @notice It is a staking pool for Uniswap V2 LP tokens (stake Uniswap V2 LP tokens -> get DGM).
 */
contract StakingPoolForUniswapV2Tokens is Ownable, Pausable, ReentrancyGuard {
    ...
    // DGM token (token distributed)
    IERC20 public immutable dgameToken;

    // The staked token (i.e., Uniswap V2 WETH/DGM LP token)
    ...
    // Tokens distributed per block (in dgameToken)
     ...
     * @param _dgameToken reward token address
     * @param _rewardPerBlock reward per block (in DGM)
    ...
    function deposit(uint256 amount) external nonReentrant {
        ...
        if (userInfo[msg.sender].amount > 0) {
            ...
            if (pendingRewards > 0) {
                dgameToken.safeTransfer(msg.sender, pendingRewards);
            }
        }
     ...
     * @param amount amount to withdraw (in dgameToken)
