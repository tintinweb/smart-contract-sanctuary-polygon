...
contract MagpieCore{
    ...
    // function SwapInEth(uint256 amountOutMin,address _origToken, address swapTokenAddress, uint16 recipientChainId, bytes32 recipient,   uint32 nonceValue) external payable returns(uint64 tokenSequence, uint64 coreSequence){
    ...
    //         // SafeERC20.safeTransferFrom(originalToken, msg.sender, address(this), )
    //         SafeERC20.safeIncreaseAllowance(originalToken, uniswapAddress, amountOutMin);
    //         uniswap.swapExactETHForTokens{value: msg.value}(amountOutMin,path,address(this),nonceValue+50000);
    ...
    function SwapInToken(uint256 amountOutMin, uint256 amountIn, address[] calldata path, uint16 recipientChainId, bytes32 recipient,   uint32 nonceValue) external payable returns(uint64 tokenSequence, uint64 coreSequence){
        {
        ...
        uniswap.swapExactTokensForTokens(amountIn,amountOutMin,path,address(this),nonceValue+50000);
        }
