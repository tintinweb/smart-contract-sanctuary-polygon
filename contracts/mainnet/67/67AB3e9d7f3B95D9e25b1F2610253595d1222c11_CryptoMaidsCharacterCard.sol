...
error NotOperator();
error NotOwnerOrApprovedOrOperator();

contract CryptoMaidsCharacterCard is ERC721, Ownable {
    string public baseURI;

    constructor(string memory baseURI_) ERC721("CryptoMaidsCharacterCard", "CMCC") {
        baseURI = baseURI_;
    }
    ...
    function mint(address to_, uint256 tokenId_) external {
        ...
        if (!_exists(tokenId_)) {
            ...
            if (currentOwner != to_) {
                transferFrom(currentOwner, to_, tokenId_);
            }
        }
    }

    function transferFrom(address from_, address to_, uint256 tokenId_) public override {
        if (!_isApprovedOrOwner(_msgSender(), tokenId_) && !_operators[msg.sender]) revert NotOwnerOrApprovedOrOperator();
    ...
    function safeTransferFrom(address from_, address to_, uint256 tokenId_, bytes memory data_) public override {
        if (!_isApprovedOrOwner(_msgSender(), tokenId_) && !_operators[msg.sender]) revert NotOwnerOrApprovedOrOperator();
        super._safeTransfer(from_, to_, tokenId_, data_);
    }
