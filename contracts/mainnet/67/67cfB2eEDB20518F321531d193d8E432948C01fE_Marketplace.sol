...
contract Marketplace is BaseContract {
  ...
  event OrderExtended(address indexed nftContractAddress, uint indexed tokenId, address user, uint price, uint weekNumber, address paymentAddress, uint expiredTime, uint takeOrderPrice, uint timestamp);
  ...
  function setNFTListing(
    ...
    uint paymentLength = _payment._paymentAddresses.length;
    require(
      _payment._paymentAddresses.length == paymentLength &&
      _payment._poolAddresses.length == paymentLength &&
      _payment._isUniSwapV3.length == paymentLength &&
      _payment._isR0DivR1.length == paymentLength &&
      _payment._isStableCoin.length == paymentLength,
    ...
    require(_checkPaymentAddressesNotHasZeroAddress(_payment._paymentAddresses, _payment._isStableCoin), "Marketplace: payment addresses invalid");

    nftListings[_nftContractAddress] = NftListing(
      _owner,
      _uri,
      _payment._paymentAddresses,
      _payment._poolAddresses,
      _payment._isUniSwapV3,
      _payment._isR0DivR1,
      true,
      _payment._isStableCoin
    );
  ...
  function takeOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external validOrder(_nftContractAddress, _tokenId) {
    ...
    (uint currentPrice, uint takeUSDPrice) = _getCurrentPrice(_nftContractAddress, _tokenId, _weekNumber, _paymentAddress);
  ...
  function extendOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external {
    ...
    uint currentPrice = order.takeOrderPrice * _weekNumber;
    ...
    emit OrderExtended(_nftContractAddress, _tokenId, msg.sender, currentPrice, _weekNumber, _paymentAddress, expiredTime, order.takeOrderPrice, block.timestamp);
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint, uint) {
    Order storage order = orders[_nftContractAddress][_tokenId];
    NftListing storage nftListing = nftListings[_nftContractAddress];
    ...
    if (nftListing.isStableCoin[indexPayment]) {
      currentPrice = order.orderType == IMarketPlace.OrderType.installmentRental
        ...
        : currentPrice;
      return (currentPrice, takeUSDPrice);
    }
    ...
    if (nftListing.isUniSwapV3[indexPayment]) {
      ...
      currentPrice = currentPrice * DECIMAL9 / tokenPrice * (10 ** (decimalTokenOut - decimalTokenIn));
    }
