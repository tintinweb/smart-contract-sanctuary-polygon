...
interface InterestRateModel {
   ...
   * @param cash The total cash of the underlying asset in the OToken
   * @param borrows The total borrows of the underlying asset in the OToken
   * @param reserves The total reserves of the underlying asset in the OToken
 ...
 *         of the KYC registry.
 */
contract KYCRegistryClient is IKYCRegistryClient {
  ...
  function _setKYCRegistry(address _kycRegistry) internal {
    require(_kycRegistry != address(0), "Registry Address cannot be zero");
 ...
 */
contract KYCRegistryClientConstructable is KYCRegistryClient {
  /**
   * @notice Constructor
   *
   ...
   */
  constructor(address _kycRegistry, uint256 _kycRequirementGroup) public {
...
import "../ondo/kyc/KYCRegistryClientConstructable.sol";
import "../ondo/kyc/ISanctionsList.sol";

/**
 * @title Ondo's OToken Contract
 * @notice Base for OTokens that have sanctions & KYC checks (oUSDC)
 * @author Compound Protocol
 */
contract OToken is
  EIP20Interface,
  Exponential,
  ...
  KYCRegistryClientConstructable
{
  ...
  constructor(
    ...
    address kycRegistry,
    uint256 kycRequirementGroup
  ) internal KYCRegistryClientConstructable(kycRegistry, kycRequirementGroup) {
    // Set admin to admin_
    admin = admin_;
  ...
  function mintFresh(address minter, uint mintAmount) internal returns (uint) {
     ...
     *  On success, the oToken holds an additional mintAmount of cash.
   ...
   * @param redeemTokensIn The number of oTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
   * @param redeemAmountIn The number of oTokens to redeem into underlying (only one of redeemTokensIn or redeemAmountIn may be zero)
   * @return uint 0=success, otherwise a failure (see ErrorReporter.sol for details)
   */
  ...
  function liquidateBorrowInternal(
    ...
    return
      liquidateBorrowFresh(msg.sender, borrower, repayAmount, oTokenCollateral);
  }
  ...
  function liquidateBorrowFresh(
    ...
    if (seizeTokens > oTokenCollateral.balanceOf(borrower)) {
      return
        fail(
    ...
    uint seizeError = oTokenCollateral.seize(liquidator, borrower, seizeTokens);
