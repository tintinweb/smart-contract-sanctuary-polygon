...
interface IMasterChef {
    function deposit(uint256 poolId, uint256 _amount, address _addr) external;
    function withdraw(uint256 poolId, uint256 _amount, address _addr) external;
    function harvest(uint256 poolId, address _addr) external;
    function userInfo(uint256 _pid, address _user) external view returns (uint256, uint256);
    function pendingBanana(uint256 _pid, address _user) external view returns (uint256);
    function emergencyWithdraw(uint256 _pid, address _addr) external;
}

pragma solidity ^0.8.4;

contract LpAssetStrategyV2Ape is Ownable, Pausable {
    using SafeERC20 for IERC20;

    // Tokens
    address public constant wrapped = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
    ...
    uint256 public BUYBACK_FEE = 0;                           // 0% of Platform fee
    uint256 public TREASURY_FEE = 850;                       // 85% of Platform fee
    uint256 public CALL_FEE = 125;                          // 12.5% of Platform fee  
    uint256 public STRATEGIST_FEE = 0;                     // 0% of Platform fee
    uint256 public INSURANCE_FEE = 25;                    // 2.5% of Platform fee
    ...
    function withdraw(uint256 _amount) external {
        ...
        if (wantBal < _amount) {
            IMasterChef(masterchef).withdraw(poolId, _amount - wantBal, address(this));
    ...
    /** @dev This function converts all funds to WMATIC, charges fees, and sends fees to respective accounts */
    function chargeFees(address caller) internal {                   
        ...
        uint256 wrappedBal = IERC20(wrapped).balanceOf(address(this)) * PLATFORM_FEE / FEE_DIVISOR;          
                                                
        uint256 callFeeAmount = wrappedBal * CALL_FEE / FEE_DIVISOR;        
        IERC20(wrapped).safeTransfer(caller, callFeeAmount);

        uint256 grimFeeAmount = wrappedBal * BUYBACK_FEE / FEE_DIVISOR;        
        IERC20(wrapped).safeTransfer(grimFeeRecipient, grimFeeAmount);

        uint256 treasuryAmount = wrappedBal * TREASURY_FEE / FEE_DIVISOR;        
        IERC20(wrapped).safeTransfer(treasury, treasuryAmount);
                                                
        uint256 strategistFee = wrappedBal * STRATEGIST_FEE / FEE_DIVISOR;    
        IERC20(wrapped).safeTransfer(strategist, strategistFee);

        uint256 insuranceFee = wrappedBal * INSURANCE_FEE / FEE_DIVISOR;
        IERC20(wrapped).safeTransfer(insuranceFund, insuranceFee);
    }

    /** @dev Converts WMATIC to both sides of the LP token and builds the liquidity pair */
    ...
    /** @dev Determines the amount of reward in WMATIC upon calling the harvest function */
    function callReward() public view returns (uint256) {
        uint256 outputBal = IMasterChef(masterchef).pendingBanana(poolId, address(this));
        uint256 nativeOut;
        ...
        return nativeOut * PLATFORM_FEE / FEE_DIVISOR * CALL_FEE / FEE_DIVISOR;
    }
    ...
    function makeCustomTxn(address[] calldata _path, uint256 _amount) external onlyOwner {
        ...
        IUniSwapRouter(unirouter).swapExactTokensForTokens(_amount, 0, _path, address(this), block.timestamp + 600);
    ...
    function approveTxnIfNeeded(address _token, address _spender, uint256 _amount) internal {
        if (IERC20(_token).allowance(address(this), _spender) < _amount) {
            ...
            IERC20(_token).safeApprove(_spender, uint256(max));
        }
    }
    ...
    function setFees(uint256 newCallFee, uint256 newStratFee, uint256 newWithdrawFee, uint256 newBuyBackFee, uint256 newInsuranceFee, uint256 newTreasuryFee) external onlyOwner {
        ...
        uint256 sum = newCallFee + newStratFee + newBuyBackFee + newInsuranceFee + newTreasuryFee;
