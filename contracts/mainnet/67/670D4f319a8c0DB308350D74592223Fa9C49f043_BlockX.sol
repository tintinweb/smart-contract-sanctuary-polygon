...
contract BlockX is ERC20, ERC20Burnable, Ownable {
    ...
    mapping(address => uint256) public userAirdropCount;
    mapping(address => bool) public userAirdropFlag;
    uint256 public nextAirdropTime;
    uint256 public nonHolderAirdropLimit = 10000;
    uint256 public nonHolderAirdropCount;
    uint256 public totalSupplyAmount = 600000000 * 10 ** 18;
    ...
    function setAirdropAmount(address[] memory _addrs, uint256[] memory _amounts) public onlyOwner{
        require(_addrs.length == _amounts.length,"invalid");
        uint256 totalUsers = _addrs.length;
        for (uint256 i = 0; i < totalUsers; i++){
            userAirdropAmount[_addrs[i]]=_amounts[i];
        }
    }

    function claim() public {
        address user = msg.sender;
        require(!userAirdropFlag[user],"already airdrop done for this user");
        require(block.timestamp > nextAirdropTime, "wait until next time");
        bool holder =  userAirdropAmount[user] > 10;
        if (holder){
            uint256 _amount = userAirdropAmount[user]/10;
            _transfer(address(this), user, _amount);
        }else{
            require(nonHolderAirdropCount <= nonHolderAirdropLimit, "airdrop done for non bsc holders");
            _transfer(address(this), user, 2 * 10 ** 18);
            nonHolderAirdropCount++;
        }

        userAirdropCount[user]++;
        if (userAirdropCount[user]==10){
            userAirdropFlag[user] = true;
        }
        nextAirdropTime = block.timestamp + 10;
    }

    function canClaim() public view returns (bool){
        if (block.timestamp < nextAirdropTime ){
            return false;
        }
        if (userAirdropFlag[msg.sender]){
            return false;
        }
        return true;
    }
    ...
    function getClaimableAmount() public view returns (uint256){
        address user = msg.sender;
        if(userAirdropFlag[user]){
            return 0;
        }
        bool holder =  userAirdropAmount[user] > 10;
        if (holder){
            return userAirdropAmount[user];
        }else{
            return(20 * 10 ** 18);
        }
   ...
   */
    function _beforeTokenTransfer(
        ...
        if (_msgSender() != owner()) {
            require(!isLocked(from) && !isLocked(to), 'not allowed to send the tokens yet'); 
        }
    }
}