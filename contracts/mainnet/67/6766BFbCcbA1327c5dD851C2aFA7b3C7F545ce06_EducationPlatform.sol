...
contract EducationPlatform is Ownable {
    ...
    function register() public{
        require(!isUserRegistered[_msgSender()]);
        isUserRegistered[_msgSender()] = true;

        emit UserRegistration(_msgSender());
    }
    ...
    function startRound(uint _startTimeStapm,uint _timeInHours, uint _roundRevardsPoints) external onlyOwner {
        uint _hourInSecconds = 60*60;
        require(!round.roundPlanned, "Round already planned");
        require(block.timestamp <= _startTimeStapm && (_startTimeStapm - block.timestamp)<= _hourInSecconds*24*31);
        uint _endTime = _startTimeStapm + _timeInHours * _hourInSecconds;
    ...
    function donateInUSDT(uint _id, uint _amount) public{
        ...
        if(round.roundPlanned && round.startTime < block.timestamp && block.timestamp < round.endTime && !round.isUserDonatedToExpertInRound[_msgSender()][_id]){
            round.totalVotes++;
    ...
    function transferTokensToExpert(uint _id) public onlyOwner{
        require(round.roundPlanned && round.endTime < block.timestamp, "Round not finished yet");
    ...
    function getRoundStatus() external view returns(string memory){
        if(round.endTime <= block.timestamp && round.roundPlanned){
            return('round ended');
        }
        if(round.startTime <= block.timestamp &&  round.endTime >= block.timestamp && round.roundPlanned){
            return('round now');
        }
    ...
    event UserRegistration(address _userAddress);
}