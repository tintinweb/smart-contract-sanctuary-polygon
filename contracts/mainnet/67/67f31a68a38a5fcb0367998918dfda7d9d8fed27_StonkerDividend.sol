...
//mainnet
        ...
        USDCContract internal USDC_ = USDCContract(0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174); //mainnet
        ...
        function setAllowClaim(bool allow_) public onlyOwner {
            allowClaim=allow_;
        }
        ...
        //function getExcluded() public onlyOwner view returns()
        function getTotalDividendHistoryIndex() public view returns(uint256[] memory){
          return dividendHistoryIndex;
        }

        function getTotalDividendHistory(uint256 timestamp_) public view returns(uint256){
          return dividendHistory[timestamp_];
        }
        ...
        function getClaimableDividend(address addr_) public view returns(uint256){
         ...
         function isAddressExcluded(address addr_) public view returns(bool){
          return isExcluded[addr_];
        }
        ...
        function getLastDisbursementTime() public view returns(uint256){
          return lastDisbursementTime;
        }
        ...
        function claim() public {
          ...
          require(allowClaim,"claim is not allowed");
          require(dividends[msg.sender]>0,"zero dividend");
          //uint256 amountToClaim = dividends[msg.sender];
          
          USDC_.transfer(msg.sender, dividends[msg.sender]);
         
          claimedDividends[msg.sender]+=dividends[msg.sender];
          claimDetail memory cd;
          cd.claimTime=block.timestamp;
          cd.claimAmount=dividends[msg.sender];
          claimedDividendHistory[msg.sender].push(cd);
          totalClaimedDividends+=dividends[msg.sender];
        ...
        */
        function distributeDividends(address [] calldata addr_, uint256 [] calldata dividends_, uint256 [] calldata stonkerId_, uint256 totalDividend_) onlyOwner public {
          uint256 tax = (15*totalDividend_)/100;
          USDC_.transfer(stonkerTreasury, tax);
          USDC_.transfer(stonkerTax, tax);
          for(uint256 j=0;j<dividends_.length;j++){
                dividends[addr_[j]]+=dividends_[j];
                dividendDetail memory dd;
                dd.stonkerId = stonkerId_[j];
                dd.distributionTime = block.timestamp;
                dd.distributionAmount = dividends_[j];
                distributedDividends[addr_[j]].push(dd);
              } 

          lastDisbursementTime = block.timestamp;
          dividendHistory[lastDisbursementTime]=totalDividend_;
          totalDistributed += totalDividend_;
          dividendHistoryIndex.push(lastDisbursementTime);
        }

        function distributeDividend(address addr_, uint256 amount) public onlyOwner{
            dividends[addr_]+=amount;
            dividendDetail memory dd;
            dd.distributionTime = block.timestamp;
            dd.distributionAmount = amount;
            distributedDividends[addr_].push(dd);
            totalDistributed += amount;
        }

        function getDistributableDividend() public view returns(uint256){
          return getStonkerUSDCBalance() - totalDistributed;
        }

         function getUnclaimedDividend() public view returns(uint256){
          return totalDistributed - totalClaimedDividends;
        }
