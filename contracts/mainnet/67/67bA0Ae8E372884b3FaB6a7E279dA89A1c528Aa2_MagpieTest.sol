...
contract MagpieTest is ReentrancyGuard, Ownable, Multicall, IMagpieTest {
    ...
    function swap(bytes[] calldata hops, bytes calldata params) external payable nonReentrant override returns (uint256 amountOut) {
        Swap memory s = Swap({
            assetsLength: params.toUint8(1),
            i: 0,
            fromIsNative: false,
            toIsNative: false,
            hopsLength: hops.length,
            amountIn: params.toUint256(3),
            accAmountIn: 0,
            lastAmountOut: 0,
            fromAssetAddress: params.toAddress(90),
            toAssetAddress: params.toAddress(111)
        });
        ...
        for (s.i; s.i < s.hopsLength; s.i++) {
            ...
            h.amountIn = (h.fromAssetAddress == s.fromAssetAddress ? params.toUint256(h.amountInIndex + 1) : s.lastAmountOut);
            h.toAssetAddress = params.toAddress(h.toAssetIndex + 1);
        ...
        if (amountOut < params.toUint256(36) || amountOut == 0) {
            revert InsufficientOutputAmount();
        }

        s.toAssetAddress.transfer(payable(params.toAddress(69)), amountOut);
    }
}
...
library LibBytes4 {
    ...
    function getPayload(bytes4 selector, bytes memory hop, bytes memory params) internal pure returns (bytes memory) {
        ...
        assembly {
            ...
            let endOfHopDefinitions := add(hopDefinition, sub(hopLength,6))
            let paramIndex := 0
            ...
            {
                hopDefinition := add(hopDefinition, 2)
            }
            {
                paramIndex := add(
                    paramValues, 
                    /* Param index: 2 bytes shifted to right */ shr(240, mload(hopDefinition))
                )
                mstore(
                    position, 
                    /* Param value */
                ...
                position := add(position, /* Size of param bytes to get: 1 byte shifted to right */ shr(248, mload(sub(paramIndex, 1))))
            }
