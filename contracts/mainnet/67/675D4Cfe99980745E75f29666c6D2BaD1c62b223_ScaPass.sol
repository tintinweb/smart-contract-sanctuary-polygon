...
contract ScaPass is ERC1155, Ownable {

    using Strings for uint256;

    string public constant NAME = "ScaPass";
    ...
    uint256[3] public maxSupply = [150,30,20];
    uint256[3] public maxMintPerWallet = [1,1,1];
    uint256[3] public totalMinted = [0,0,0];
    uint256[3] public mintPrice = [0,0,10000 ether];
    bytes32[2] public merkleRoot;

    uint256 public startTime = 1654531200;
    uint256 public endTime = 1654876800;
 
    bool public paused = false;
    bool public isPublisSale = false;

    address public testTokenAddress = address(0);

    constructor(string memory _baseURI_, address _testTokenAddress) ERC1155(_baseURI_) {
    ...
    function mint(uint256 _amount, uint256 _tokenId, bytes32[] calldata _proof) public onlyNotPaused {
        ...
        if(_tokenId == 3) {
            require(IERC20(testTokenAddress).balanceOf(msg.sender) >= mintPrice[2] * _amount, 'SCA value is too low');
            IERC20(testTokenAddress).transferFrom(msg.sender, address(this), mintPrice[2] * _amount);
        }

        for(uint256 i = 0; i < _amount; i++) {
            super._mint(msg.sender, _tokenId, 1, "");   
        }
    ...
    function withdrawToken() public onlyOwner {
        uint256 _balance = IERC20(testTokenAddress).balanceOf(address(this));
        IERC20(testTokenAddress).transfer(msg.sender, _balance);
    }
}
