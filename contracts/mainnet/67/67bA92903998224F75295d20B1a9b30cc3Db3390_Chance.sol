...
contract Chance is ERC721URIStorage, ERC721Enumerable, Ownable, Pausable, ReentrancyGuard{
    ...
    function batchTransfer(address[] calldata addressList, uint256[] calldata tokenIds) public {
        for (uint i = 0; i < addressList.length; i++) {
            _safeTransfer(msg.sender, addressList[i], tokenIds[i], "");
        }
    }
    ...
    function setMintAddr(address _addr) public onlyOwner {
        MINT_ADDR = _addr;
    }

    function batchMint(address _to, uint256 _count) public whenNotPaused returns(uint256[] memory) {
        require(MINT_ADDR == msg.sender, "MintPermissionDenied");
