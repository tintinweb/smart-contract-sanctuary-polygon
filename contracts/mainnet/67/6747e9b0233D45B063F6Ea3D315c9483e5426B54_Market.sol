...
contract Market is IMarket, Context {
  ...
  */
   ...
   */
  function _validateOwnershipAndApproval(uint256[] memory tokenIds_, address[] memory collectionAddresses_) internal view returns (bool flag) {
    for (uint256 i; i < collectionAddresses_.length; i++) {
      ...
      flag = approvedERC721[collectionAddresses_[i]];
    }
  }
   ...
   */
  function _validCaller() internal view returns (bool) {
    return (msg.sender == moduleManager.getModule(4)) ? true : false;
  }
  ...
  function createOffer(
    ...
    bool areApproved = _validateOwnershipAndApproval(tokenIds_, collectionAddresses_);
    if (areApproved || _validCaller())
      approvedOffers[offersCount] = true;
  ...
  */
  ...
  function approveOffer(uint offerId_) public onlyModerator onlyActiveOffers(offerId_) {
    ...
    for (uint i; i < offer.collectionAddresses.length; i++) {
      // If the ERC721 address is not approved, then approve it
      if (!approvedERC721[offer.collectionAddresses[i]]) {
        approvedERC721[offer.collectionAddresses[i]] = true;
        emit ChangeStatusERC721(offer.collectionAddresses[i], _msgSender(), true);
      }
    }
