...
contract McVaultControllerProxy is Ownable, IManagedVaultController {
    using SafeERC20 for IERC20;

    IManagedVaultController public controller;

    constructor(IManagedVaultController _controller) public {
        controller = _controller;
    }

    /* ========== VIEW FUNCTIONS ========== */

    function canCreateVault(address _factory, address _user) external override view returns (bool) {
        return controller.canCreateVault(_factory, _user);
    }

    function canDeposit(address _factory, address _user) external override view returns (bool) {
        return controller.canDeposit(_factory, _user);
    }

    function futureCheck(address _factory, address _user, bytes memory userData) external override view returns (bool) {
        return controller.futureCheck(_factory, _user, userData);
    }

    /* ========== OWNER FUNCTIONS ========== */

    function setController(IManagedVaultController _address) external onlyOwner {
        controller = _address;
    }
}