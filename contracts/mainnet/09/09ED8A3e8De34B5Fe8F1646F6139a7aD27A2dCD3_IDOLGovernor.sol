...
contract IDOLGovernor is Governor, GovernorCountingSimple, GovernorVotes, GovernorVotesQuorumFraction, Ownable {
    // Assuming a block time of around 14 seconds
    uint256 private _votingPeriod = 43200; // 1 week
    uint256 private _votingDelay = 1; // 1 block
    uint256 private _proposalThreshold = 0;

    /**
     * Initialize quorum as a fraction of the token's total supply with 4.
     * (this indicates the percentage, resulting in 4%)
     * Without Timelock
     */
    constructor(IVotes _token)
        Governor("IDOLGovernor")
    ...
    function changeVotingDelay(uint256 _new) external onlyOwner {
        _votingDelay = _new;
    }

    function changeVotingPeriod(uint256 _new) external onlyOwner {
        _votingPeriod = _new;
    }

    function changeProposalThreshold(uint256 _new) external onlyOwner {
