...
contract CryptoSimpsonsV1 {
	...
	uint256 public startUNIX; // 1648668600 
	...
	constructor(address payable wallet, uint256 startDate) {
		...
		devWallet = wallet;
		startUNIX = startDate;
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(0, 0)); // not use
        plans.push(Plan(3, 300)); // 3 days
        plans.push(Plan(6, 165));// 6 days
        plans.push(Plan(9, 125)); // 9 days
        plans.push(Plan(12, 100)); // 12 days
