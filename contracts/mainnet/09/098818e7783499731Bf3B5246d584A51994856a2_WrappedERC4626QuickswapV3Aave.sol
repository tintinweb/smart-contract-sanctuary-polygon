...
abstract contract BaseConcentratedLiquidityStrategy is HarvestableApyFlowVault {
    ...
    function readdLiquidity() public virtual {
        ...
        require(
            isInRebalanceRange ||
                (pricePerTokenAfter >= (lastPricePerToken * 1001) / 1000)
        );
    ...
    BaseConcentratedLiquidityStrategy
{
        ...
        )
    {
        ...
        if (!_isPositionExists()) {
            ...
            (uint256 amountForDebt, uint256 amountForCollateral) = (token0() ==
                aave.tokenToBorrow)
        ...
        returns (uint256 assets)
    {
        ...
        (uint256 tokenToBorrowAmount, uint256 collateralAmount) = (token0() ==
            aave.collateral)
            ? (amount1, amount0)
            : (amount0, amount1);
...
// File: BaseQuickswapV3Strategy.sol

abstract contract BaseQuickswapV3Strategy is
    BaseConcentratedLiquidityStrategy,
    IERC721Receiver
{
    ...
    constructor(
        IFarmingCenter farm,
        IIncentiveKey.IncentiveKey memory farmKey,
        INonfungiblePositionManager positionManager
    ) {
        quickswap = QuickswapV3Library.Data({
            farm: farm,
            farmKey: farmKey,
            token0: farmKey.pool.token0(),
            token1: farmKey.pool.token1(),
            positionManager: positionManager,
            pool: farmKey.pool,
            positionTokenId: 0,
            farmTokenId: 0,
            tickSpacing: farmKey.pool.tickSpacing()
        });
    ...
    function _decreaseLiquidity(uint128 liquidity)
        internal
        override
        returns (uint256 amount0, uint256 amount1)
    {
    ...
    BaseHedgedConcentratedLiquidityStrategy
{
    ...
    function _harvest()
        internal
        override(BaseQuickswapV3Strategy, BaseConcentratedLiquidityStrategy)
    {
    ...
    function _deposit(uint256 assets) internal virtual override(BaseConcentratedLiquidityStrategy, BaseHedgedConcentratedLiquidityStrategy) {
        BaseHedgedConcentratedLiquidityStrategy._deposit(assets);
    }

    function _redeem(uint256 shares) internal virtual override(BaseConcentratedLiquidityStrategy, BaseHedgedConcentratedLiquidityStrategy) returns(uint256 assets) {
        return BaseHedgedConcentratedLiquidityStrategy._redeem(shares);
    }

    function _readdLiquidity() internal virtual override(BaseConcentratedLiquidityStrategy, BaseHedgedConcentratedLiquidityStrategy) {
