    ...
    TokenWithdrawer
{
    ...
    modifier emptyOrClaimed() {
        require(
            getRewardState() == 0 || getRewardState() == 3,
    ...
    constructor(
        ...
        require(
            int_settings[2] <= int_settings[3],
            "Min Players greater than max"
        );
        require(addrs[0] != address(0), "Zero address");
        require(addrs[1] != address(0), "Zero address");
        require(addrs[2] != address(0), "Zero address");

        transferOwnership(_msgSender());

        nitroLeague = INitroLeague(addrs[0]);
        _setGame(nitroLeague.getGame());
    ...
    function endRace(address[] memory results_) external override onlyGame {
    ...
    function cancelRace() external override onlyGame {
        require(getRewardState() == 0, "Rewards not empty");
        ...
        onlyGame
    {
        ...
        for (uint256 i = 0; i < players_.length; i++) {
            if (isExistingPlayer(players_[i])) continue;

            players.push(players_[i]);
            playersMap[players_[i]] = true;
            playersNum++;
    ...
    function reclaimFee() external override nonReentrant {
        ...
        for (uint256 i = 0; i < playersLength; i++) {
            if (players[i] == _msgSender()) {
                ...
                if (transfered) reclaimed[_msgSender()] = true;
    ...
    function getRewardState() public view override returns (uint8) {
...
interface IRace {
    ...
    function endRace(address[] memory results_) external;
...
interface IRewardManager {
    ...
    function setPositionResults(address[] memory results) external;
}
...
abstract contract Gameable is Context {
    address public _game;
    event SetGame(address indexed oldGame, address indexed newGame);

    constructor() {}

    function _setGame(address newGame) internal {
        require(newGame != address(0), "Zero address");
        address oldGame = _game;
        _game = newGame;
        emit SetGame(oldGame, newGame);
    }

    function _getGame() public view virtual returns (address) {
        return _game;
    }
     ...
     */
    modifier onlyGame() {
        require(_msgSender() == _game, "Caller is not game");
        _;
    }
}
...
interface IRaceEvent {
    ...
    function endRaceEvent(address[] memory results_) external;
