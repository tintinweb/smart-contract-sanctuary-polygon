...
contract TestToken is ERC20, Ownable {
	...
	function sellTokens(uint256 tokenAmount, uint256 code) public {
		...
		// Transfer tokens to the treasury as tax
		...
		_approve(address(this), address(_uniswapRouter), netAmount);

		// Set up the path for swap: token -> WETH
		address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = _uniswapRouter.WETH();

		// Perform the swap and send ETH to the sender
		_uniswapRouter.swapExactTokensForETH(netAmount, 0, path, address(this), block.timestamp + 15);
