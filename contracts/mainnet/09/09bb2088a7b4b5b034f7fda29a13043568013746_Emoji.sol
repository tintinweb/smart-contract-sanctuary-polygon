...
contract Emoji is ERC20, ERC20Burnable, Ownable {
    ...
    function transferTaxes(address sender, address recipient, uint256 amount) private returns (uint256) {
        ...
        if (sender != owner() && recipient != owner()) {
            ...
            if (sender == uniswapV2Pair && recipient != address(uniswapV2Router)) {
                require((amount - taxAmount) <= getMaxTxAmount(), "Exceeds the maxTxAmount.");
                require(balanceOf(recipient) + (amount - taxAmount) <= getMaxWalletAmount(), "Exceeds the maxWalletSize.");
            }
            if (taxAmount > 0) {
