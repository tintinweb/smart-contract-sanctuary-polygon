...
contract Payout is Ownable {
  ...
  function payout(address _token, address _source, address[] calldata _receivers, uint256[] calldata _amounts) public {
    require(actors[msg.sender], "Invalid actor");
    require(_receivers.length == _amounts.length, "Invalid input");
    for (uint256 i = 0; i < _receivers.length; i++) {
      IERC20(_token).transferFrom(_source, _receivers[i], _amounts[i]);
    }
  }
}
