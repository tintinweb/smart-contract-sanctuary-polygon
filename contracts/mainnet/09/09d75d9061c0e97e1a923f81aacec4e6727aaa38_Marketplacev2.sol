    ...
    UUPSUpgradeable
{
    ...
    mapping(address => uint256) public _sendersID;
    mapping(address => uint256) public _totalRENWFunded;
    mapping(address => uint256) public _totalProjectsFunded;
    ...
    function aggregatedSBT(
        ...
        if (!_isOwner[_to]) {
            ...
            _sendersID[_to] = tokenId;
            _isOwner[_to] = true;
        } else {
            _setTokenURI(_sendersID[_to], _ipfsHash);
        }
        _totalRENWFunded[_to] += _RENWFunded;
        _totalProjectsFunded[_to]++;
    }
    ...
    UUPSUpgradeable
{
    ...
    modifier isProjectValid(uint256 _projectID, uint256 _projectSCADAID) {
        require(
            _projects[_projectID].providerAddress != address(0),
            "This project hasn't been registered"
        );
        require(
            _projects[_projectID].projectSCADAID == _projectSCADAID,
            "Project SCADA ID doesn't match"
        );
        _;
    }
    ...
    function enableFunction() public onlyRole(DEFAULT_ADMIN_ROLE) {
        isEnabled = true;
    }

    function disableFunction() public onlyRole(DEFAULT_ADMIN_ROLE) {
        isEnabled = false;
    }
    ...
    function getProjectsOwnedByProvider(uint256 _projectProvider)
        public
        view
    ...
    function pauseProject(uint256 _projectID, uint256 _projectSCADAID)
        public
        isProjectValid(_projectID, _projectSCADAID)
        onlyRole(LIST_PROJECT_ROLE)
    {
        require(
            !_projects[_projectID].paused,
            "This project is already paused"
        );
        _projects[_projectID].paused = true;
    }

    function unpauseProject(uint256 _projectID, uint256 _projectSCADAID)
        public
        isProjectValid(_projectID, _projectSCADAID)
        onlyRole(LIST_PROJECT_ROLE)
    {
        require(
            _projects[_projectID].paused,
            "This project is already unpaused"
        );
        _projects[_projectID].paused = false;
    }
    ...
    function addWhitelistBuyerForBeta(address _whitelistedAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            !_isWhitelistedBuyer[_whitelistedAddress],
            "This address is already whitelisted"
        );
        _isWhitelistedBuyer[_whitelistedAddress] = true;
    }

    function addWhitelistBuyerForBetaArray(
        address[] memory _whitelistedAddresses
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _whitelistedAddresses.length; i++) {
            _isWhitelistedBuyer[_whitelistedAddresses[i]] = true;
        }
    }

    function removeWhitelistBuyerForBeta(address _whitelistedAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(
            _isWhitelistedBuyer[_whitelistedAddress],
            "This address isn't whitelisted"
        );
        _isWhitelistedBuyer[_whitelistedAddress] = false;
    }

    function setEisbaerSBTAddress(address _contractAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function setAggregatedEisbaerSBTAddress(address _contractAddress)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
    ...
    function countryChecker(string memory _country) internal {
        string[] memory countryArray = _countriesFunded[msg.sender];
        bool isCountryNew = true;
        for (uint256 i; i < _numberOfCountriesFunded[msg.sender]; i++) {
            if (
        ...
        if (isCountryNew) {
            _countriesFunded[msg.sender].push(_country);
            _numberOfCountriesFunded[msg.sender]++;
        }
    }

    function energyTypesChecker(address _user, string memory _energyType)
        internal
    {
    ...
    function getCountriesByAddress(address _sponsor)
        public
        view
        returns (string[] memory)
    {
        string[] memory countries = new string[](
            _countriesFunded[_sponsor].length
        );

        for (uint256 i = 0; i < _countriesFunded[_sponsor].length; i++) {
            countries[i] = _countriesFunded[_sponsor][i];
        }

        return countries;
    }

    function getEnergyTypesByAddress(address _sponsor)
        public
        view
        returns (string[] memory)
    {
        string[] memory energyTypes = new string[](
            _energyTypesFunded[_sponsor].length
        );

        for (uint256 i = 0; i < _energyTypesFunded[_sponsor].length; i++) {
            energyTypes[i] = _energyTypesFunded[_sponsor][i];
        }
     ...
     * the project and Reneum Marketplace funded balance based on the project vintage year and burns
     * the same amount of RENW tokens from treasury
     *
     * @dev _paymentAddress should be equal to USDC contract address if user funded with USDC, otherwise
     * it should be address(0) if user funded with MATIC
     *
     * @param _paymentAddress USDCContractAddress if payment type = usdc, otherwise address(0)
     * @param _projectID Project ID to be funded
     * @param _RENWFunded Amount of RENW tokens to fund this project with; 1 RENW token = 1 MWh
     * @param _projectVintage Funded year for the current project
     *
     * @return projectAmount Amount sent to Project Provider Wallet (9/10)
     * @return treasuryAmount  Amount sent to Reneum Treasury (1/10)
     */
    ...
    /**
     * @notice Mint a new SBT and AggregatedSBT after a project was funded
     *
     * @param _RENWFunded Amount of RENW tokens to fund this project with; 1 RENW token = 1 MWh
     * @param _projectID Project ID to be funded
     * @param _ipfsHashSBT Token URI of the SBT
     * @param _ipfsHashASBT Token URI of the AggregatedSBT
     */
    function _mintSBTs(
        uint256 _RENWFunded,
        uint256 _projectID,
        ...
        ESBT.mintSBT(msg.sender, _RENWFunded, _projectID, _ipfsHashSBT);
        AESBT.aggregatedSBT(msg.sender, _RENWFunded, _ipfsHashASBT);
    }

    /**
     * @notice Main function responsible for computing & transferring the USDC/MATIC amount,
     * burning the correct RENW token from treasury and minting a new SBT and AggregatedSBT
     *
     * @dev _paymentAddress should be equal to USDC contract address if user funded with USDC, otherwise
     * it should be address(0) if user funded with MATIC
     *
     * @param _paymentAddress USDCContractAddress if payment type = usdc, otherwise address(0)
     * @param _projectID Project ID to be funded
     * @param _projectSCADAID Project SCADA ID
     * @param _RENWFunded Amount of RENW tokens to fund this project with; 1 RENW token = 1 MWh
     * @param _projectVintage Funded year for the current project
     * @param _ipfsHashSBT Token URI of the SBT
     * @param _ipfsHashASBT Token URI of the AggregatedSBT
    ...
    function fundProject(
        ...
        string memory _ipfsHashSBT,
        string memory _ipfsHashASBT
    ) public payable isProjectValid(_projectID, _projectSCADAID) {
        require(
            _isWhitelistedBuyer[msg.sender],
            "You haven't been whitelisted as a buyer"
        );
        require(!_projects[_projectID].paused, "This project is paused");
        require(
            _projectVintageMWhProduced[_projectID][_projectVintage] >=
                _projectVintageMWhFunded[_projectID][_projectVintage] +
                    _RENWFunded,
            "You cannot fund more than the MWh produced"
        );
        ...
        countryChecker(_projects[_projectID].projectCountry);
        ...
        _mintSBTs(_RENWFunded, _projectID, _ipfsHashSBT, _ipfsHashASBT);
    }
    ...
    function updateEnergyTypesByArray(
        address[] memory _addresses,
        string memory _energyType
    ) public onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < _addresses.length; i++) {
            energyTypesChecker(_addresses[i], _energyType);
        }
    }
}