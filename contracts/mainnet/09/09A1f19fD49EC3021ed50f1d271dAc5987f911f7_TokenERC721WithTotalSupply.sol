...
contract TokenERC721WithTotalSupply is
Initializable,
IThirdwebContract,
IOwnable,
IRoyalty,
...
ITokenERC721
{
    ...
    function initialize(
        ...
        _setupRole(ADMIN_TRANSFER_ROLE, _defaultAdmin);
    ...
    function mintTo(address _to, string calldata _uri, uint256 tokenId) external onlyRole(MINTER_ROLE) returns (uint256) {
        // `_mintTo` is re-used. `mintTo` just adds a minter role check.
        return _mintTo(_to, _uri, tokenId);
    }
    ...
    function _mintTo(address _to, string calldata _uri, uint256 tokenIdToMint) internal returns (uint256) {
    ...
    function transferFromPermit(address owner, address to, uint256 tokenId, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        _permit(owner, msg.sender, tokenId, deadline, v, r, s);
        _transfer(owner, to, tokenId);
    }

    function permit(address owner, address spender, uint tokenId, uint deadline, uint8 v, bytes32 r, bytes32 s) external {
        _permit(owner, spender, tokenId, deadline, v, r, s);
    }

    function _permit(address owner, address spender, uint tokenId, uint deadline, uint8 v, bytes32 r, bytes32 s) private {
        ...
        require(recoveredAddress != address(0) && recoveredAddress == owner, 'ERC721: INVALID_SIGNATURE');
        _approve(spender, tokenId);
    }

    function deidWithdraw(address owner, address to, uint256 tokenId) external{
        require(hasRole(ADMIN_TRANSFER_ROLE, msg.sender), "ERC721: INVALID_ADMIN_TRANSFER_ROLE");
        _transfer(owner, to, tokenId);
    }

    function setTokenURI(uint256 _tokenId, string calldata _uri) external {
        require(hasRole(DEFAULT_ADMIN_ROLE, msg.sender), "ERC721: INVALID_DEFAULT_ADMIN_ROLE");
        uri[_tokenId] = _uri;
    }
}
