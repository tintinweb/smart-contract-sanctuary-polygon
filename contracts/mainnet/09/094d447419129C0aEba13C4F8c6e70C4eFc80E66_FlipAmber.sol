...
contract FlipAmber {
    ...
    constructor() {
        _owner = msg.sender;
        _amberToken = IERC20(0x8865BC57c58Be23137ACE9ED1Ae1A05fE5c8B209);
    }
    ...
    function bet(uint256 betAmount) external gasLimit(200000) {
        ...
        // Вызов approve для разрешения контракту Flipcoin на снятие токенов с вашего кошелька
        _amberToken.approve(address(this), betAmount);
    ...
    function depositFunds(uint256 amount, bytes calldata signature) external gasLimit(100000) {
        require(amount > 0, "Amount must be greater than zero");
        require(_verifySignature(amount, signature), "Invalid signature");

        // Вызов approve для разрешения контракту Flipcoin на снятие токенов с вашего кошелька
        _amberToken.approve(address(this), amount);
    ...
    function withdrawFunds(uint256 amount, bytes calldata signature) external onlyOwner {
        require(amount > 0, "Amount must be greater than zero");
        require(_verifySignature(amount, signature), "Invalid signature");
    ...
    function _verifySignature(uint256, bytes memory) private pure returns (bool) {
