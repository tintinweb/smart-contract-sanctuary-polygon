...
contract RuffRaffle is Ownable {
    ...
    function participateInRaffle(uint256 raffleId, uint256 ticketsToBuy) external payable onlyActiveRaffle(raffleId) {
        RaffleInfo storage raffle = raffles[raffleId];

        require(msg.value == ticketsToBuy.mul(raffle.ticketPrice), "Incorrect value sent.");

        uint256 availableTickets = raffle.totalTickets.sub(raffle.ticketsBought[msg.sender]);
        require(availableTickets >= ticketsToBuy, "Not enough available tickets.");

        if (raffle.ticketsBought[msg.sender] == 0) {
            raffle.participants.push(msg.sender);
        }

        raffle.ticketsBought[msg.sender] = raffle.ticketsBought[msg.sender].add(ticketsToBuy);

        emit RaffleParticipated(raffleId, msg.sender, ticketsToBuy);
    }

    function depositTokens(address tokenAddress, uint256 amount) external {
        require(tokenAddress != address(0), "Invalid token contract address.");
        ...
        supportedTokens[msg.sender][tokenAddress] = supportedTokens[msg.sender][tokenAddress].add(amount);
    ...
    function withdrawTokens(address tokenAddress, uint256 amount) external {
        ...
        uint256 availableAmount = supportedTokens[msg.sender][tokenAddress];
        require(availableAmount >= amount, "Not enough tokens available for withdrawal.");

        IERC20 token = IERC20(tokenAddress);

        supportedTokens[msg.sender][tokenAddress] = supportedTokens[msg.sender][tokenAddress].sub(amount);
    ...
    function withdrawAllTokens() external {
        require(userRaffles[msg.sender].length == 0, "Cannot withdraw tokens while participating in an active raffle.");
        ...
        for (uint256 i = 0; i < userRaffles[msg.sender].length; i++) {
            ...
            uint256 availableAmount = supportedTokens[msg.sender][tokenContract];
            if (availableAmount > 0) {
                tokenCount = tokenCount.add(availableAmount);
                delete supportedTokens[msg.sender][tokenContract];
            }
        }

        require(tokenCount > 0, "No tokens available for withdrawal.");
        ...
        for (uint256 i = 0; i < userRaffles[msg.sender].length; i++) {
            ...
            uint256 availableAmount = supportedTokens[msg.sender][tokenContract];
            if (availableAmount > 0) {
                supportedTokens[msg.sender][tokenContract] = supportedTokens[msg.sender][tokenContract].sub(availableAmount);

                token = IERC20(tokenContract);
                require(token.transfer(msg.sender, availableAmount), "Token transfer failed.");
            }
        }
    ...
    function removeRaffle(uint256 raffleId) external onlyRaffleCreator(raffleId) {
        ...
        require(raffle.isActive, "Raffle is already inactive.");
    ...
    function sendNFT(uint256 raffleId, address recipient) external onlyRaffleCreator(raffleId) {
        RaffleInfo storage raffle = raffles[raffleId];

        require(raffle.isActive, "Raffle is not active.");
        require(recipient != address(0), "Invalid recipient address.");

        IERC721 nft = IERC721(raffle.nftContract);

        nft.transferFrom(address(this), recipient, raffle.nftTokenId);

        emit NFTSent(raffleId, recipient, raffle.nftTokenId);
    }
    ...
    function getTicketsBought(uint256 raffleId, address participant) external view returns (uint256) {
        require(raffleExists[raffleId], "Raffle does not exist.");

        RaffleInfo storage raffle = raffles[raffleId];

        return raffle.ticketsBought[participant];
    }
    ...
    function setServiceFeePercentage(uint256 percentage) external onlyOwner {
        require(percentage <= 100, "Service fee percentage should be less than or equal to 100.");
    ...
    function isRaffleActive(uint256 raffleId) external view returns (bool) {
        return raffleExists[raffleId] && raffles[raffleId].isActive;
    }
}