...
contract DAO_RulesV2 {
    ...
    uint32 TimeLocker=604800; // 604800
    ...
    // 1) LES ASSOCIATIONS CARITATIVES SONT CHOISIES SELON LE CHOIX DE LA COMMUNAUTE
    // 2) LES REWARDS SERONT ENVOYES APRES LE JEU ET UTILISABLE DIRECTEMENT
...
interface IGameContract
{
    ...
    function Profil(address)external view returns(uint64,uint);
    function Level() external view returns(level calldata);

    // onlyOwner
    function transferOwnership(address) external;
    function ChangeLevel(level calldata L) external;
    function ChangeIERC20(IERC20Charity) external;
    function withdraw() external;
    function gameOver(string calldata _time, uint _milliseconds, uint _score, address _from, string calldata _email) external;

    event GameOver(uint _gameNumber, string _time, uint _milliseconds, uint _score, address _from, string _email);
}
