...
contract LotteryPool {
    ...
    mapping(address => mapping(IERC20 => uint256)) public erc20Bank;
    ...
    modifier onlyAdmin(string memory _appId, address _sender){
        bool isAdmin = quiz.checkAdmin(_appId, _sender);
    ...
    mapping(uint256 => address) public lotteryCreator;
    ...
    onlyAdmin(_appId, msg.sender) {
        ...
        lotteries[_lotteryId] = Lottery(_rewardToken, _amounts, _fixedNum, _proportionNum, _amount, false, false, true);
        erc20Bank[msg.sender][_rewardToken] = erc20Bank[msg.sender][_rewardToken].add(_amount);
        lotteryCreator[_lotteryId] = msg.sender;
    ...
    onlyAdmin(_appId, msg.sender) {
        ...
        lotteries[_lotteryId] = Lottery(IERC20(address(0)), _amounts, _fixedNum, _proportionNum, _amount, true, false, true);
        ethBank[msg.sender] = ethBank[msg.sender].add(msg.value);
        lotteryCreator[_lotteryId] = msg.sender;
    }
    ...
    function drawALotteryByIndex(uint256 _lotteryId, uint256 _index) public onlyOperator {
        ...
        lotteries[_lotteryId].over = true;
        if (lotteries[_lotteryId].amounts[_index] == 0) {
            return;
        }
    ...
    function _drawALotteryByIndex(uint256 _lotteryId, uint256 _index, bool isFixNum) internal {
        ...
        address creator = lotteryCreator[_lotteryId];

        if (lotteries[_lotteryId].isEth) {
            require(ethBank[creator] >= lotteries[_lotteryId].amounts[_index], "creator's eth not enough");
            ethBank[creator] = ethBank[creator].sub(lotteries[_lotteryId].amounts[_index]);
            _ethLottery(lotteries[_lotteryId].amounts[_index], lotteryResults[_lotteryId][_index]);
        } else {
            require(erc20Bank[creator][lotteries[_lotteryId].token] >= lotteries[_lotteryId].amounts[_index], "creator's eth not enough");
            erc20Bank[creator][lotteries[_lotteryId].token] = erc20Bank[creator][lotteries[_lotteryId].token].sub(lotteries[_lotteryId].amounts[_index]);
            _erc20Lottery(lotteries[_lotteryId].token, lotteries[_lotteryId].amounts[_index], lotteryResults[_lotteryId][_index]);
        }
    }

    function getLotteries(uint256[] memory _ids) public view returns (Lottery[] memory){
        Lottery[] memory details = new Lottery[](_ids.length);
        for (uint i = 0; i < _ids.length; i++) {
            details[i] = lotteries[_ids[i]];
        }
        return details;
    }
