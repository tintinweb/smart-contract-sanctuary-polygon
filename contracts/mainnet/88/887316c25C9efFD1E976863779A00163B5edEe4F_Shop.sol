...
contract Shop is ReentrancyGuard, AccessControl {
  ...
  function whitelistNeeded(
    ...
    return
      ...
      whitelist.whitelistNeeded(collectionAddress, productIds);
  }
  ...
  function purchase(
    address collectionAddress,
    ProductOrder[] calldata productOrders
  ) external payable {
    //Safety checks performed in next method call
    purchaseWithWhitelist(collectionAddress, productOrders, new bytes32[](0));
  }

  function purchaseWithWhitelist(
    address collectionAddress,
    ProductOrder[] calldata productOrders,
    ...
    for (uint256 i = 0; i < productOrders.length; i++) {
      ProductOrder calldata productOrder = productOrders[i];

      require(
        collection.products(productOrder.productId).forSale,
...
interface Whitelist {
  ...
  function whitelistNeeded(address collection, uint256[] calldata productIds)
    external
    view
    returns (bool);
}
