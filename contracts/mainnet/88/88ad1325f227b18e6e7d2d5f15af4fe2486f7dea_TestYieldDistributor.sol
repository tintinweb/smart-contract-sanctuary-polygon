...
/*
	Test Yield Distributor

	This contract has no public methods (except getters)

	Yield Distributor takes addresses to claim assets from to save on gas cost (and it should be
	executed by automated backend).
	Addresses should set up allowance with appropriate functions beforehand.
*/
contract TestYieldDistributor is AccessControlUpgradeable, SafeAllowanceResetUpgradeable {
    using SafeMathUpgradeable for uint256;
    using SafeERC20Upgradeable for IERC20Upgradeable;
	...
	// distributed to HolyTreasury and operational cashflow. Remainder is net customer profit.
    ...
    ///////////////////////////////////////////////////////////////////////////
    // intrastructure and fee setters
    ///////////////////////////////////////////////////////////////////////////
	...
	function redeemSingleAddress(address _address) public {
		...
		IERC20Upgradeable token = IERC20Upgradeable(IAssetPoolV3(addressPool).getBaseAsset());
	...
	function distributeYield(IERC20Upgradeable _token, uint256 _yieldAmount) internal {
		...
		uint256 amountPool = _yieldAmount;

		if (amountTreasury > 0 && addressTreasury != address(0)) {
			_token.transfer(addressTreasury, amountTreasury);
            amountPool = amountPool.sub(amountTreasury);
		}
		if (amountOperations > 0 && addressOperations != address(0)) {
			_token.transfer(addressOperations, amountOperations);
            amountPool = amountPool.sub(amountOperations);
		}

        resetAllowanceIfNeeded(_token, addressPool, amountPool);

        // Asset Pool transfers token by itself
		IAssetPoolV3(addressPool).harvestYield(amountPool);
