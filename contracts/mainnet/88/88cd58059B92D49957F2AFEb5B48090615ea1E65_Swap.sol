 ...
 * deployment size.
 */
contract Swap is OwnerPausableUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        address _lpToken = address(new LPToken());
    ...
    function getTokensArray() public view returns (address[] memory) {
        address[] memory tokensArray = new address[](swapStorage.pooledTokens.length);
...
library SwapUtils {
    ...
    function _calculateWithdrawOneToken(
        ...
        uint256 dySwapFee = currentY.sub(newY).div(self.tokenPrecisionMultipliers[tokenIndex]).sub(
            dy
        );
    ...
    function swap(
        ...
        {
            IERC20Upgradeable tokenFrom = self.pooledTokens[tokenIndexFrom];
    ...
    function addLiquidity(
        ...
        IERC20Upgradeable[] memory pooledTokens = self.pooledTokens;
    ...
    function skim(Swap storage self, address _to) external {
        IERC20Upgradeable[] memory pooledTokens = self.pooledTokens;
        uint256[] memory balances = self.balances;
        for (uint256 i = 0; i < pooledTokens.length; i++) {
            uint amount = pooledTokens[i].balanceOf(address(this)) - balances[i];
            pooledTokens[i].safeTransfer(_to, amount);
        }
    }
    ...
    function withdrawAdminFees(Swap storage self, address to) external {
        IERC20Upgradeable[] memory pooledTokens = self.pooledTokens;
        for (uint256 i = 0; i < pooledTokens.length; i++) {
            IERC20Upgradeable token = pooledTokens[i];
