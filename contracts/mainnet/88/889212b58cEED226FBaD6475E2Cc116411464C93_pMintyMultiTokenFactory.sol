...
contract pMintyMultiTokenFactory is Ownable {
    ...
    function number_of_projects() external view returns (uint256) {
        return projects.length;
    }

    function updateSale(address __saleContract) external onlyAuth {
        _saleContract = __saleContract;
    }

    function updateProxy(INewentProxy __registry)  external onlyAuth {
         _registry = __registry;
    }
...
contract pMintyMultiToken is ContextMixin, ERC1155Supply, NativeMetaTransaction, IERC2981 {
    ...
    event LocksUpdated(locking[]  _locs);
    ...
    function updateLocks(locking[] memory _locs) external {
         require ( msgSender() == deployer ||  auth[msgSender()]  , "Unauthorised");
          locs = _locs;
          emit LocksUpdated(_locs);
    }
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        ...
        uint256 _royaltyAmount = (salePrice * royaltyPerMille) / 1000;
    ...
    function _split(uint256 amount) internal {
        bool sent;
        uint256 _total;
        PoolEntry[] memory p = resalePools[0];
        for (uint256 j = 0; j < p.length; j++) {
            uint256 _amount = amount * p[j].share / 1000;
            if (j == p.length-1) {
                _amount = amount - _total;
            } else {
                _total += _amount;
            }
            ( sent, ) = p[j].beneficiary.call{value: _amount}(""); // don't use send or xfer (gas)
    ...
    function _split20(uint256 amount, address token) internal {
        ...
        for (uint256 j = 0; j < p.length; j++) {
            ...
            IERC20(token).transfer(p[j].beneficiary,_amount); 
        }
    }
