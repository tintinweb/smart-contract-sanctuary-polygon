...
contract RubicLiFiProxy is MultipleTransitToken, OnlySourceFunctionality {
    ...
    function lifiCall(BaseCrossChainParams calldata _params, bytes calldata _data)
        ...
        whenNotPaused
    {
        accrueFixedCryptoFee(); // collect fixed fee
        IERC20Upgradeable(_params.srcInputToken).safeTransferFrom(msg.sender, address(this), _params.srcInputAmount);

        uint256 _amountIn = accrueTokenFees(_params.integrator, _params.srcInputAmount, 0, _params.srcInputToken);
        ...
        require(
            (balanceBefore - IERC20Upgradeable(_params.srcInputToken).balanceOf(address(this))) == _amountIn,
            'different amount spent'
        );
        emit LiFiRequest(
    ...
    function lifiCallWithNative(BaseCrossChainParams calldata _params, bytes calldata _data)
        ...
        whenNotPaused
    {
        uint256 _amountIn = accrueTokenFees(_params.integrator, accrueFixedCryptoFee(), 0, address(0)); // amountIn - fixedFee - commission
    ...
    function _calculateFee(
        ...
        (_totalFee, _RubicFee) = OnlySourceFunctionality._calculateFee(_integrator, _amountWithFee, 0);
    }
