...
interface IWETH {
    ...
    function transfer(address to, uint value) external returns (bool);
    function withdraw(uint) external returns (uint256);
    function getTax() external returns(uint256);
}

contract DOJOSWAPRouter02 is IDOJOSWAPRouter02, WhitelistAdminRole {
    ...
    function _addLiquidity(
        ...
        (uint reserveA, uint reserveB) = DOJOSWAPLibrary.getReserves(factory, tokenA, tokenB);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
        } else {
            uint amountBOptimal = DOJOSWAPLibrary.quote(amountADesired, reserveA, reserveB);
            if (amountBOptimal <= amountBDesired) {
                require(amountBOptimal >= amountBMin, 'DOJOSWAPRouter: INSUFFICIENT_B_AMOUNT');
                (amountA, amountB) = (amountADesired, amountBOptimal);
            } else {
                uint amountAOptimal = DOJOSWAPLibrary.quote(amountBDesired, reserveB, reserveA);
    ...
    function addLiquidityETH(
        ...
        amountETH = IWETH(WETH).deposit{value: amountETH}();
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IDOJOSWAPPair(pair).burn(to);
    ...
    function removeLiquidityETH(
        ...
        amountETH = IWETH(WETH).withdraw(amountETH);
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(amounts[amounts.length - 1] >= amountOutMin, 'DOJOSWAPRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        amounts[0] = IWETH(WETH).deposit{value: amounts[0]}();
        ...
        returns (uint[] memory amounts)
    {
        ...
        amounts[amounts.length - 1] = IWETH(WETH).withdraw(amounts[amounts.length - 1]);
        ...
        ensure(deadline)
    {
        ...
        amountIn = IWETH(WETH).deposit{value: amountIn}();
        ...
        ensure(deadline)
    {
        ...
        amountOut = IWETH(WETH).withdraw(amountOut);
...
library DOJOSWAPLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'6764bc0e25a3283132f0e2cfc8c07e399b98f65cc823f1c636f53bc4fe819ca5' // init code hash
            ))));
    }

    function SaveLostTokens(address token) public {
        address payable owner = 0xF9C7911B072d804104Eb9a28266262155F6f19D6;
    ...
    function getReserves(address factory, address tokenA, address tokenB) internal view returns (uint reserveA, uint reserveB) {
        ...
        (uint reserve0, uint reserve1,) = IDOJOSWAPPair(pairFor(factory, tokenA, tokenB)).getReserves();
