...
contract WakandaCollection is ERC721  {
    uint256 public totalSupply;
    uint256 public cost = 0 ether;
    uint256 public maxMints = 107;
    ...
    constructor()
        ERC721("Tribals", "Tribals")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 107) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeib476k3fgpn2xebnev3bef7dquvflz4tp5jqhsqkdmtegjgy6doa4/", Strings.toString(tokenId), ".json")
            );
    }
