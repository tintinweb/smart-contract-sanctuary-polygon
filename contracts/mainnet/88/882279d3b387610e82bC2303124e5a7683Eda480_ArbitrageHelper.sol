...
contract ArbitrageHelper is Initializable, IArbitrageHelper {
    ...
    function unwrapLP(ICurveLP lpTokenPool, uint256 amount, address destToken) public {
        address token0 = lpTokenPool.coins(0);
        address token1 = lpTokenPool.coins(1);
        uint128 destTokenIndex = token0 == destToken
            ? 0
            : token1 == destToken ? 1 : 2;
        console.log('destTokenIndex', destTokenIndex);

        console.log(amount, destTokenIndex, 1);
        // console.log(IERC20(0x6c3F90f043a72FA612cbac8115EE7e52BDe6E490).balanceOf(msg.sender));
        // we should receive anything
        lpTokenPool.remove_liquidity_one_coin(amount, int128(destTokenIndex), 1);
    }
    ...
    function swapWithFlashLoan(
        ...
        address flashLoanBaseToken // if it's address(0) => we use WETH
    ) public {
        address pool = getPool(tokenB, flashLoanBaseToken == address(0) ? WETH : flashLoanBaseToken);
...
interface ICurveLP {
    function add_liquidity(uint256[3] memory amounts, uint256 minMintAmount) external;
    ...
    function calc_add_one_coin(uint256[3] memory amounts, bool deposit) external view returns (uint256);
}
