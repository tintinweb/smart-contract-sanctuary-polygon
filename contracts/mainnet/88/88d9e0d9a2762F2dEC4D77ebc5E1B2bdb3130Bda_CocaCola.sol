  ...
  PausableUpgradeable
{
  ...
  function setURI(string memory newuri) external {
    require(hasRole(MINTER_ROLE, msg.sender), "Caller is not setURI authorised");
    _setURI(newuri);
  }
  ...
  function safeTransferFrom(
    ...
    require(hasRole(TRANSFER_ROLE, msg.sender), "Caller does not have transfer role");
    _safeTransferFrom(from, to, id, amount, data);
  }

  function setApprovalForAll(address operator, bool approved) public virtual override {
    require(operator.code.length == 0, "approval can't be set on a smart contract");
