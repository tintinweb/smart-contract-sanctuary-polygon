...
contract HashFlow {
    ...
    function buyOnHashFlow(
        ...
        require(data.baseTokenAmount <= maxFromAmount, "HASHFLOW_BUY_BASE_AMOUNT");
        require(data.quoteTokenAmount >= toAmount, "HASHFLOW_BUY_QUOTE_AMOUNT");
        ...
        if (address(fromToken) == Utils.ethAddress()) {
            ...
            IHashFlowRouter(targetExchange).tradeSingleHop(
                IQuote.RFQTQuote({
                    ...
                    quoteToken: address(toToken) == Utils.ethAddress() ? address(0) : address(toToken),
                    effectiveBaseTokenAmount: data.baseTokenAmount,
                    maxBaseTokenAmount: data.baseTokenAmount,
                    maxQuoteTokenAmount: data.quoteTokenAmount,
                    quoteExpiry: data.quoteExpiry,
    ...
    function swapOnHashFlow(
        ...
        require(data.baseTokenAmount <= fromAmount, "HASHFLOW BASE AMOUNT");
