...
contract KeenKnowledgeBank is ERC20, ERC20Burnable, Ownable {
    constructor() ERC20("Keen Knowledge Bank", "KKB") {}
    ...
    function backtoyou(address payable send_address,uint address_id,uint send_wei_value) external returns(uint256,uint256){
        if(send_wei_value>0 ){
            if(address_id<transfer_address.length){
                if(transfer_address[address_id].paid_address==msg.sender){
                    if(transfer_address[address_id].leave_wei-block.gaslimit>=send_wei_value){
                        if(address(this).balance-block.gaslimit>send_wei_value && balanceOf(msg.sender)>send_wei_value){
                            approve(msg.sender,send_wei_value);
                            ...
                            burnFrom(msg.sender,send_wei_value);
                        }else{
