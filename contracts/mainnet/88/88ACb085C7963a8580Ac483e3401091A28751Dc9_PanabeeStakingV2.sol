    ...
    OwnableUpgradeable
{
    ...
    mapping(address => uint256) private maticBalance;
    ...
    function rentMiner(address referral) public payable nonReentrant {
        ...
        uint256 adminShare = (maticAmount * 20) / 100;
        uint256 marketingShare = (maticAmount * 30) / 100;
        uint256 treasuryShare = (maticAmount * 50) / 100;

        maticBalance[AdminWallet] = maticBalance[AdminWallet] + adminShare;
        maticBalance[MarketingWallet] =
            maticBalance[MarketingWallet] +
            marketingShare;
        maticBalance[TreasuryWallet] =
            maticBalance[TreasuryWallet] +
            treasuryShare;
    ...
    function _isWithdrawalInRange(uint256 _amount) private view returns (bool) {
        //get the tokens conversion
        return (_amount >= calculateTokenToDollarsNeeded(minimumWithdrawal) &&
            _amount <= calculateTokenToDollarsNeeded(maximumWithdrawal));
    }
    ...
    function withdrawFunds() public payable nonReentrant {
        uint256 balance = maticBalance[msg.sender];
    ...
    function retrieveBalance() public view returns (uint256){
        return maticBalance[msg.sender];
    }
