...
import "./interfaces/IJamonSwapLottery.sol";

interface IERC20Burn is IERC20 {
    function burn(uint256 amount) external;
}

/** @title JamonSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally with Chainlink VRF.
 */
contract JamonSwapLottery is
    ReentrancyGuard,
    IJamonSwapLottery,
    Ownable,
    Pausable
{
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInJamon;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] jamonPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInJamon;
        uint32 finalNumber;
    }
     ...
     * @param _jamonTokenAddress: address of the JAMON token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _jamonTokenAddress, address _randomGeneratorAddress) {
        jamonToken = IERC20Burn(_jamonTokenAddress);
        randomGenerator = IRandomNumberGenerator(_randomGeneratorAddress);

        // Initial config
        config.maxBuyOrClaimTickets = 100;
        config.burnFee = true;
        ...
        // Init operator for start lottery
        ...
        nonReentrant
    {
        ...
        require(
            _ticketNumbers.length <= config.maxBuyOrClaimTickets,
            "Too many tickets"
        );
        ...
        // Calculate number of JAMON to this contract
        uint256 amountCakeToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInJamon,
            _ticketNumbers.length
        );

        // Transfer jamon tokens to this contract
        require(
            jamonToken.transferFrom(
                address(msg.sender),
                address(this),
        ...
        _lotteries[_lotteryId].amountCollectedInJamon += amountCakeToTransfer;
    ...
    function claimTickets(
    ...
    ) external override whenNotPaused notContract nonReentrant {
        require(_ticketIds.length == _brackets.length, "Not same length");
        require(_ticketIds.length != 0, "Length must be > 0");
        require(
            _ticketIds.length <= config.maxBuyOrClaimTickets,
            "Too many tickets"
        );
        require(
        ...
        jamonToken.transfer(msg.sender, rewardInCakeToTransfer);
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInJamon) *
        ...
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].jamonPerBracket[j] =
                ...
                // A. No JAMON to distribute, they are added to the amount for burn and withdraw to governor address
            } else {
                _lotteries[_lotteryId].jamonPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId]
            .amountCollectedInJamon - amountToShareToWinners);

        if (config.burnFee) {
            jamonToken.burn(amountToWithdrawToTreasury);
        } else {
            // Transfer JAMON to governor address for fund link contracts
            jamonToken.transfer(treasuryAddress, amountToWithdrawToTreasury);
        }
    ...
    {
        require(
            _lotteries[lotteryId.current()].status == Status.Claimable,
        ...
        nonReentrant
    {
        ...
        require(
            ...
            "Transfer JAMON error"
        );
        _lotteries[_lotteryId].amountCollectedInJamon += _amount;
     ...
     * @param _priceTicketInJamon: price of a ticket in JAMON
    ...
    function startLottery(
        ...
        require(_treasuryFee <= MAX_BURN_FEE, "Burn fee too high");
        ...
        lotteryId.increment();
        uint256 currentLotteryId = lotteryId.current();

        _lotteries[currentLotteryId] = Lottery({
            status: Status.Open,
            startTime: block.timestamp,
            endTime: _endTime,
            priceTicketInJamon: _priceTicketInJamon,
            ...
            amountCollectedInJamon: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(jamonToken), "Cannot be JAMON token");
        ...
        onlyOwner
    {
        ...
        config.maxBuyOrClaimTickets = _maxNumberTicketsPerBuy;
    }

    /**
     * @notice Set operator and treasury
     * @dev Only callable by owner
     ...
     * @param _priceTicket price of a ticket (in JAMON)
    ...
    function maxNumberTicketsPerBuyOrClaim() external view returns (uint256) {
        return config.maxBuyOrClaimTickets;
    }
    ...
    {
        return (
            uint256(_lotteries[_lotteryId].status),
            _lotteries[_lotteryId].startTime,
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].jamonPerBracket[_bracket];
        } else {
            return 0;
        }
    }
    ...
    function setBurnFee(bool _set) external onlyOwner {
        config.burnFee = _set;
    }
