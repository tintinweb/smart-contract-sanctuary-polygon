...
contract NFTFanGroww is ReentrancyGuard, ERC721URIStorage, Ownable{
 ...
 function updateTokenURI(uint256 tokenId,string memory uri) public returns(bool){
        require(_exists(tokenId), "OZ:2022");
        _tokenIDURI[tokenId] = uri;
        return true;
    }
    ...
    function listNFT(uint256 tokenId, uint256 price) public { // to make tradable
    ...
    function unlistNFT(uint256 tokenId) public { // to make non tradable
    ...
    function transferNFT(address recieverAddress, uint256 tokenId, uint256 gotTransferFee) public payable nonReentrant{
        ...
        require(MarketItemDatabase[tokenId].forSale == false, "ERR:13");//those nft which has forssale=false, only it could be transfered
