...
interface ITicketMarketplace {
    ...
    function eventIdOfTicketId(uint256 ticketId) external view returns (uint256);
}
...
contract Admin is Initializable, OwnableUpgradeable, PausableUpgradeable {
    ...
    function createEvent(
        ...
        if (tickets.length > 0) {
            ITicketMarketplace(ticketMarketplaceAddress).publishTicketsForOrganizer(eventId, organizer, tickets, memberships);
        }
    ...
    function forceMetadataCancelation(string calldata entityType, uint256 id) external {
        ...
        if (keccak256(abi.encodePacked(entityType)) == keccak256('TICKET')) {
            eventId = ITicketMarketplace(ticketMarketplaceAddress).eventIdOfTicketId(id);
        }
