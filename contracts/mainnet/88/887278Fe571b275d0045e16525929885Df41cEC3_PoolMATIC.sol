...
library WethUtils {
    ...
    // IERC20 public constant MMF = IERC20(0x22a31bD4cB694433B6de19e0aCC2899E553e9481); //MMF live token
    IERC20 public constant MMF = IERC20(0x141AAB05a67e2fbe2FC6884ce8A106e451d19B83); //MMF Test token

    // IERC20 public constant MUSD = IERC20(); //MUSD token
    // IERC20 public constant MMO = IERC20(); //MMO token
...
// File contracts/PoolMATIC.sol

pragma solidity 0.8.4;

contract PoolMATIC is Ownable, ReentrancyGuard {
    using SafeERC20 for IERC20;
    ...
    function mint(uint256 _minXTokenOut) external payable nonReentrant {
        ...
        if (_yTokenOutTwap > 0 && _wethSwapIn > 0) {
            IERC20(WethUtils.weth).safeIncreaseAllowance(address(swapStrategy), _wethSwapIn);
    ...
    function transferToTreasury(uint256 _amount) internal {
        ...
        if (_amount > 0) {
            IERC20(WethUtils.weth).safeTransfer(treasury, _amount);
        }
    }
