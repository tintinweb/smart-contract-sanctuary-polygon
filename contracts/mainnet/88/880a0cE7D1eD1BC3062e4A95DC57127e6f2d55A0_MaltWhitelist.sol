...
contract MaltWhitelist is Permissions {
  ...
  mapping(address => uint256) public addressAllocation;
  ...
  constructor(
    ...
    _adminSetup(msg.sender);
  }

  function addWhitelistedAddress(address _address, uint256 allocation)
    external
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    addressAllocation[_address] = allocation;
  }
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    ...
    for (uint256 i; i < length; ++i) {
      addressAllocation[_addresses[i]] = _allocations[i];
    }
  }
  ...
  function unlockStart()
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(!startUnlocked, 'Already started!');
    startUnlocked = true;
  }
  ...
  function unlockEnd()
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(startUnlocked, 'Not started yet');
    require(!endUnlocked, 'Already ended!');
    endUnlocked = true;
  }

  function buy(uint256 _amount) public {
    require(startUnlocked, 'Not yet started');
    require(!endUnlocked, 'Already ended');
    require(addressAllocation[msg.sender] > 0, 'Insufficient allocation');
    require(
      totalPurchased[msg.sender] + _amount <= addressAllocation[msg.sender],
      'you have reached your allocation limit'
    );
    ...
    uint256 purchased = balanceAfter - initialBalance;
    totalPurchased[msg.sender] += purchased;
    totalRaise += purchased;
  }

  function buyForAccount(uint256 _amount, address account) public {
    require(startUnlocked, 'Not yet started');
    require(!endUnlocked, 'Already ended');
    require(addressAllocation[account] > 0, 'Insufficient allocation');
    require(
      totalPurchased[account] + _amount <= addressAllocation[account],
      'you have reached your allocation limit'
    );

    if (totalPurchased[account] == 0) {
  ...
  function distributeFunds()
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(endUnlocked, "Not ended yet");
    require(!finalized, "Already finalized");
    require(liquidityExtension != address(0), "Liquidity Extension unset");
    require(swingTrader != address(0), "Swing trader unset");
    require(address(malt) != address(0), "Malt unset");

    // Just using Malt here is safe as no malt exists in circulating
    // so a griefing attack of sending Malt here
    ...
    uint256 requirement = requiredMalt();

    if (maltBalance > requirement) {
      maltBalance = requirement;
    }

    malt.safeApprove(address(router), maltBalance);
    rewardToken.safeApprove(address(router), maltBalance);
    ...
    (,, uint256 liquidity) = router.addLiquidity(
      ...
      maltBalance,
      maltBalance,
      // slippage doesn't matter as the pool is locked
      // to only this contract at this point
      0,
      0,
      address(this),
      block.timestamp
    ...
    // LE gets 40%
    uint256 liquidityExtensionCut = rewardBalance * 40 / 100;
    uint256 swingTraderCut = rewardBalance - liquidityExtensionCut;

    rewardToken.safeTransfer(liquidityExtension, liquidityExtensionCut);
    rewardToken.safeTransfer(swingTrader, liquidityExtensionCut);

    finalized = true;
  }

  function bondAccounts(
    uint256 startIndex,
    uint256 endIndex
  )
    public
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    ...
    require(address(lpToken) != address(0), "LP == addr(0)");
    require(address(bonding) != address(0), "Bonding == addr(0)");

    uint256 lpBalance = lpToken.balanceOf(address(this));
    lpToken.safeApprove(address(bonding), lpBalance);

    uint256 totalLength = accounts.length;

    require(startIndex < totalLength, "Start out of bounds");
    ...
    for (uint256 i = startIndex; i < endIndex; ++i) {
      if (executed[accounts[i]]) {
        continue;
      }
      address account = accounts[i];
      uint256 lpShare = totalLPTokens * totalPurchased[account] / totalRaise;

      bonding.bondToAccount(account, poolId, lpShare);

      executed[accounts[i]] = true;
    }

    lpToken.safeApprove(address(bonding), 0);
  }

  function requiredMalt() public view returns(uint256) {
    return totalRaise / 2;
  }
  ...
  function setMaltAddress(address _malt)
    external
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_malt != address(0), "Cannot be addr(0)");
    malt = IERC20(_malt);
  }
  ...
  function setLpToken(address _lpToken)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_lpToken != address(0), "Cannot be addr(0)");
    lpToken = IERC20(_lpToken);
  }

  function setBondingContract(address _bonding)
    external
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_bonding != address(0), "Cannot be addr(0)");
  ...
  function setLiquidityExtension(address _liquidityExtension)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_liquidityExtension != address(0), "Cannot be addr(0)");
    liquidityExtension = _liquidityExtension;
  }
  ...
  function setSwingTrader(address _swingTrader)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_swingTrader != address(0), "Cannot be addr(0)");
  ...
  function setRouter(address _router)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_router != address(0), "Cannot be addr(0)");
    router = IUniswapV2Router02(_router);
  }
  ...
  function setRewardToken(address _rewardToken)
    ...
    onlyRoleMalt(ADMIN_ROLE, "Must have admin privs")
  {
    require(_rewardToken != address(0), "Cannot be addr(0)");
    rewardToken = IERC20(_rewardToken);
  }
