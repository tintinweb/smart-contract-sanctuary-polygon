...
// File: contracts/interfaces/IBarSwapLottery.sol

pragma solidity ^0.8.4;

interface IBarSwapLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in BAR per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BAR token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInBar: price of a ticket in BAR
...
// File: contracts/BarSwapLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title BarSwap Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BarSwapLottery is ReentrancyGuard, IBarSwapLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBar = 50 ether;
    uint256 public minPriceTicketInBar = 0.005 ether;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBar;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] barPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBar;
        uint32 finalNumber;
    }
     ...
     * @param _barTokenAddress: address of the BAR token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _barTokenAddress, address _randomGeneratorAddress) {
        barToken = IERC20(_barTokenAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BAR to this contract
        uint256 amountBarToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBar,
            _ticketNumbers.length
        );

        // Transfer bar tokens to this contract
        barToken.safeTransferFrom(address(msg.sender), address(this), amountBarToTransfer);

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBar += amountBarToTransfer;
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBarToTransfer
        uint256 rewardInBarToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBarToTransfer += rewardForTicketId;
        }

        // Transfer money to msg.sender
        barToken.safeTransfer(msg.sender, rewardInBarToTransfer);

        emit TicketsClaim(msg.sender, rewardInBarToTransfer, _lotteryId, _ticketIds.length);
    }
        ...
        nonReentrant
    {
        ...
        uint256 amountToShareToWinners = (
            ((_lotteries[_lotteryId].amountCollectedInBar) * (10000 - _lotteries[_lotteryId].treasuryFee))
        ) / 10000;

        // Initializes the amount to withdraw to treasury
        uint256 amountToWithdrawToTreasury;

        // Calculate prizes in BAR for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].barPerBracket[j] =
                ...
                // A. No BAR to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].barPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury += (_lotteries[_lotteryId].amountCollectedInBar - amountToShareToWinners);

        // Transfer BAR to treasury address
        barToken.safeTransfer(treasuryAddress, amountToWithdrawToTreasury);
    ...
    function injectFunds(uint256 _lotteryId, uint256 _amount) external override onlyOwnerOrInjector {
        ...
        barToken.safeTransferFrom(address(msg.sender), address(this), _amount);
        _lotteries[_lotteryId].amountCollectedInBar += _amount;
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBar >= minPriceTicketInBar) && (_priceTicketInBar <= maxPriceTicketInBar),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBar: _priceTicketInBar,
            discountDivisor: _discountDivisor,
            rewardsBreakdown: _rewardsBreakdown,
            treasuryFee: _treasuryFee,
            barPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            countWinnersPerBracket: [uint256(0), uint256(0), uint256(0), uint256(0), uint256(0), uint256(0)],
            firstTicketId: currentTicketId,
            firstTicketIdNextLottery: currentTicketId,
            amountCollectedInBar: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount) external onlyOwner {
        require(_tokenAddress != address(barToken), "Cannot be BAR token");
    ...
    /**
     * @notice Set BAR price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBar: minimum price of a ticket in BAR
     * @param _maxPriceTicketInBar: maximum price of a ticket in BAR
     */
    function setMinAndMaxTicketPriceInBar(uint256 _minPriceTicketInBar, uint256 _maxPriceTicketInBar)
        external
        onlyOwner
    {
        require(_minPriceTicketInBar <= _maxPriceTicketInBar, "minPrice must be < maxPrice");

        minPriceTicketInBar = _minPriceTicketInBar;
        maxPriceTicketInBar = _maxPriceTicketInBar;
    }
     ...
     * @param _priceTicket price of a ticket (in BAR)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].barPerBracket[_bracket];
        } else {
            return 0;
        }
    }
