...
contract GeosWheel is VRFConsumerBase, GeosVault {
    ...
    event RewardGiven(address indexed nftOwner, uint8 rewardPackage, uint256 tokenId, uint256 tokenAmout, bool successfull);
    ...
    function _checkPackageChances() internal view {
        uint256 _totalChance;
        for(uint8 i=0; i<packagesLength; i++){
            _totalChance += _packageChance[i];
        }
        require(_totalChance == packageLen, "GeopolyWheel: Total Chance is not equal to packageLen");
    }

    function changePackageLength(uint256 _packagelen) public onlyAuthorized {
        require(_packagelen != 0, "GeopolyWheel: Package Length cannot be 0");
    ...
    function _getRandomExpansion(uint256 n) internal view returns (uint256[] memory expandedValues) {
        expandedValues = new uint256[](n);
        for (uint256 i = 0; i < n; i++) {
            expandedValues[i] = uint256(keccak256(abi.encode(mainRandomness, i)));
        }
    ...
    function getRewardChances() public view returns(uint256[] memory rc){ 
        rc = new uint256[](packagesLength);
        for(uint8 i=0; i<packagesLength; i++){
            rc[i] = _packageChance[i];             
        }
    }
    ...
    function _getTokenIdReward(RewardPackage rt) internal view returns(uint256){
        ...
        revert("GeopolyWheel: No NFT for such prize");
    } 

    function giveReward(address to) internal  {
        (RewardPackage packageType, uint256 amount, uint256 tokenId, bool success) = _getReward();
        if(success){
            if(amount != 0){
                _sendTokens(to, amount);
            }else {
                _sendNFTs(to, tokenId);
            }
        ...
        require(rewardTracker <= packageLen, "GeopolyWheel: No more rewards, please wait for a refill");
        }
        emit RewardGiven(to, uint8(packageType), tokenId, amount, success);
    }

    function _getReward() internal view returns(RewardPackage _rewardPackage, uint256 _rewardAmount ,uint256 _rewardToken, bool _rewardable){
        uint256 _rr = _getRandomRewardIncrementer(rewardTracker);
        for(uint256 i=_rr; i == 1; i--){
            if(_packageChance[uint8(i)] > 0){
                _rewardPackage = RewardPackage(i);
                break;
            }
        }
        _rewardable = _rewardPackage == RewardPackage(0) ? false : true;
        if (!_rewardable){
            return(_rewardPackage, 0,0,false);
        }else{
            if(_rewardPackage <= RewardPackage(uint8(5)) && (_rewardPackage > RewardPackage(0))){
                _rewardAmount = _getTokenAmountReward(_rewardPackage);
            }else if (_rewardPackage <= RewardPackage(uint8(10)) && (_rewardPackage > RewardPackage(0))){
                _rewardToken = _getTokenIdReward(_rewardPackage);
            }
        }
    }
}