...
import {IModuleV2} from "./interfaces/IModuleV2.sol";
...
import {Order} from "./structs/Order.sol";

contract GelatoOrdersVault {
    ...
    event LogDeposit(bytes32 indexed key, Order order, uint256 amountIn);
    ...
    modifier onlyGelato() {
        require(GELATO == msg.sender, "GelatoOrdersVault: onlyGelato");
        _;
    }
    ...
    function depositNative(Order calldata _order) external payable {
    ...
    function depositToken(Order calldata _order) external {
        require(
            _order.inputToken != NATIVE,
    ...
    function cancelOrder(Order calldata _order) external {
        require(
    ...
    function fill(Order calldata _order, bytes calldata _auxData)
        external
        onlyGelato
    {
        bytes32 key = keyOf(_order);
        ...
        delete deposits[key];
        bool isNative = _order.inputToken == WRAPPED_NATIVE ||
            _order.inputToken == NATIVE;
        if (isNative) {
            ...
            SafeERC20.safeTransfer(IERC20(WRAPPED_NATIVE), pool, amount - fee);
        } else if (
            _order.outputToken == WRAPPED_NATIVE || _order.outputToken == NATIVE
        ) {
            SafeERC20.safeTransfer(IERC20(_order.inputToken), pool, amount);
        } else {
            ...
            SafeERC20.safeTransfer(IERC20(_order.inputToken), handler, fee);
        }

        uint256 bought = IModuleV2(_order.module).execute(_order, _auxData);
        if (isNative) payable(relayer).sendValue(fee);
        emit LogFill(key, _order.owner, bought, _auxData);
    }
    ...
    function canFill(
        ...
        return
            ...
            IModuleV2(_order.module).canExecute(
    ...
    function keyOf(Order calldata _order) public pure returns (bytes32) {
...
interface IModuleV2 {
    function execute(Order calldata _order, bytes calldata _auxData)
        external
