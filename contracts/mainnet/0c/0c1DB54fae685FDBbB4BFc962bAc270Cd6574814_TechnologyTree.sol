...
contract TechnologyTree is OwnableUpgradeable {
    ...
    function buyTec(uint ID, uint amount) external {
        ...
        if(userTec[msg.sender][ID] * 1e18 + amount >= 1000e18){
            amount = 1000e18 - userTec[msg.sender][ID] * 1e18;
        }
    ...
    function buyTecBatch(uint[] memory ids, uint[] memory amounts) external {
        ...
        for (uint i = 0; i < ids.length; i ++) {
            ...
            if(userTec[msg.sender][ids[i]] * 1e18 + tempAmount >= 1000e18){
                tempAmount = 1000e18 - userTec[msg.sender][ids[i]] * 1e18;
            }
            total += tempAmount;
