...
// File: contracts/baihu.sol

// Created by STEVE
// BAIHU

contract BAIHU is ERC721A, Ownable {
  ...
  uint256 public maxMintAmountFree = 4;
  ...
  constructor() ERC721A("BAIHU", "BAI") {
    setNotRevealedUri("ipfs://QmPGz7yzRMTU48hqrrVk4iDAxJSnei7ADUKMdpkQnsq2zt/hidden.json");
  }
  ...
  function mint(uint256 _amount) external payable {
        ...
        if(totalSupply() <= FREE_MAX_SUPPLY && (_mintedFreeAmount[msg.sender] + _amount <= maxMintAmountFree)){
            require(_amount <= MAX_PER_TX_FREE, "Excess max per free tx");
            _mintedFreeAmount[msg.sender] += _amount;
        }else{
            require(_amount <= MAX_PER_TX, "Excess max per paid tx");
