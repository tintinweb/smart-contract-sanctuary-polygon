...
contract Presale is ReentrancyGuard, Ownable {
    ...
    function claim() external nonReentrant {
        require(block.timestamp > endTime + 90 days, "Claiming not available yet");
        ...
        if (lastClaim[msg.sender] > endTime) {
            ...
            timePassed = block.timestamp - endTime - 90 days; // time since end of sale (minus 3 months)
        }
    ...
    if(block.timestamp < endTime + 90 days) {
        return endTime + 90 days;
    } else {
        uint256 timeSinceCliff = block.timestamp - (lastClaim[user] + 90 days);
        uint256 monthsSinceCliff = timeSinceCliff / 30 days;
        return (lastClaim[user] + 90 days) + (monthsSinceCliff * 30 days);
    }
}
