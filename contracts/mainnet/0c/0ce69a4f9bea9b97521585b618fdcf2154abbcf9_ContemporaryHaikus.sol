...
contract whitelistCheck is EIP712{
    ...
    string private constant SIGNING_DOMAIN = "HAIKU123!";
...
contract ContemporaryHaikus is ERC721Enumerable, Ownable , whitelistCheck{
    using Strings for uint;
    using ECDSA for bytes32;

    uint public ContemporaryHaikus_PRESALE = 1; //presale
    uint public ContemporaryHaikus_WALLET_PRESALE_LIMIT = 10; //ContemporaryHaikusperwalletInWholeDuration
    uint public ContemporaryHaikus_WALLET_PUBLICSALE_LIMIT = 20; //ContemporaryHaikusperwalletInWholeDuration
    uint public ContemporaryHaikus_MAX_COUNT = 1400; //maxSupply
    uint public totalTokensSoldInPresale;
    uint public preSaleAmountMinted;

    uint public ContemporaryHaikus_MAINSALE_PRICE = 1.0 ether; //priceInMainSale
    uint public ContemporaryHaikus_PRESALE_PRICE = 0.5 ether; //priceInPreSale
    ...
    string private _tokenBaseURI = "https://gateway.pinata.cloud/ipfs/QmR8ZKHrw4HbcMv9qpy1XTCNYLfaxuJXMi6FFPDN9zo5xA/";
    string private constant Sig_WORD = "private";
    address private _signerAddress = 0x81B26E81989eE95A7a375Ae03FabbCAb2b620360;
    ...
    constructor() ERC721("Contemporary Haikus", "ContemporaryHaikus") {}

    function setContemporaryHaikus_MAX_COUNT(uint _maxCount) external onlyOwner{
        ContemporaryHaikus_MAX_COUNT = _maxCount;
    }

    function setContemporaryHaikus_PRESALE(uint _presale) external onlyOwner{
        ContemporaryHaikus_PRESALE = _presale;
    }

    function setContemporaryHaikus_WALLET_PRESALE_LIMIT(uint _presaleLimit) external onlyOwner{
        ContemporaryHaikus_WALLET_PRESALE_LIMIT = _presaleLimit;
    }

    function setContemporaryHaikus_WALLET_PUBLICSALE_LIMIT(uint _publicSaleLimit) external onlyOwner{
        ContemporaryHaikus_WALLET_PUBLICSALE_LIMIT = _publicSaleLimit;
    }

    function setContemporaryHaikus_MAINSALE_PRICE(uint _mainSalePrice) external onlyOwner{
        ContemporaryHaikus_MAINSALE_PRICE = _mainSalePrice;
    }

    function setContemporaryHaikus_PRESALE_PRICE(uint _presalePrice) external onlyOwner{
        ContemporaryHaikus_PRESALE_PRICE = _presalePrice;
    }
    ...
    function gift(address[] calldata receivers) external onlyOwner {
        require(totalSupply() + receivers.length <= ContemporaryHaikus_MAX_COUNT, "EXCEED_MAX");
    ...
    function buy(uint tokenQuantity) external payable {
        ...
        require (tokenQuantity <= ContemporaryHaikus_WALLET_PUBLICSALE_LIMIT);
        require (saleListPurchase[msg.sender] + tokenQuantity <= ContemporaryHaikus_WALLET_PUBLICSALE_LIMIT, "MAX MINT COMPLETE");
        require (totalSupply() + tokenQuantity <= (ContemporaryHaikus_MAX_COUNT - preSaleAmountMinted), "EXCEED_MAX");
        require (ContemporaryHaikus_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
    ...
    function founderMint(uint tokenQuantity) external onlyOwner {
        require(totalSupply() + tokenQuantity <= ContemporaryHaikus_MAX_COUNT, "EXCEED_MAX");
        for(uint i = 0; i < tokenQuantity; i++) {
        _safeMint(msg.sender, totalSupply() + 1);
        }
    }
    ...
    function presaleBuy( Whitelist memory whitelist, uint tokenQuantity) external payable{
        require(presaleLive, "PRESALE_CLOSED");//ContemporaryHaikus_WALLET_PRESALE_LIMIT
        require(getWhitelist(whitelist), "DIRECT_MINT_DISALLOWED");
        require(whitelist.userAddress == msg.sender);
        require(preSaleAmountMinted + tokenQuantity <= ContemporaryHaikus_PRESALE, "EXCEED_PRIVATE");
        require(presalerListPurchases[msg.sender] + tokenQuantity <= ContemporaryHaikus_WALLET_PRESALE_LIMIT, "EXCEED_ALLOC");
        require(ContemporaryHaikus_MAINSALE_PRICE * tokenQuantity <= msg.value, "INSUFFICIENT_ETH");
