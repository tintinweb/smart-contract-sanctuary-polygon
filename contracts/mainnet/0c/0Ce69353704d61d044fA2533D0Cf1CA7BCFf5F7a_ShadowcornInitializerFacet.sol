...
contract ShadowcornInitializerFacet {
    struct InitParams {
        address gameBank;
        address UNIMAddress;
        address RBWAddress;
    ...
    function init(InitParams memory params) external {
        ...
        LibShadowcorn.setGameBank(params.gameBank);
        LibShadowcorn.setUNIMAddress(params.UNIMAddress);
        LibShadowcorn.setRBWAddress(params.RBWAddress);
        LibShadowcorn.setTerminusAddress(params.terminusAddress);
        LibShadowcorn.setCommonEggPoolId(params.commonEggPoolId);
        LibShadowcorn.setRareEggPoolId(params.rareEggPoolId);
        LibShadowcorn.setMythicEggPoolId(params.mythicEggPoolId);
    }

    function configureForPolygonMainnet() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.configureForPolygonMainnet();
    }

    function configureForMumbaiTestnet() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.configureForMumbaiTestnet();
    }

    function configureForLocalhost() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.configureForLocalhost();
    }
    
    function loadConfigContent() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.loadContent();
    }

    function configureNFTImages() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.configureNFTImages();
    }

    function configureLoadFirstNames() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.loadFirstNames();
    }

    function configureLoadLastNames() external {
        LibShadowcorn.enforceIsContractOwner();
        LibEnvironmentConfig.loadLastNames();
    }
