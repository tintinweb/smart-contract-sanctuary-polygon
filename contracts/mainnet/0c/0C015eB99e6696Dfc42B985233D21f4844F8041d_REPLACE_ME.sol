...
contract REPLACE_ME is Executor {
    ...
    function withdraw(address recipient, uint256 value) external onlyAdmin {
        (bool success,) = payable(recipient).call{value: value}("");
        require(success, "Polygods: value transfer unsuccessful");
    }

    function withdraw20(address tokenContract, address recipient, uint256 amount) external onlyAdmin {
        (bool success, bytes memory data) = address(tokenContract).call(abi.encodeWithSelector(0xa9059cbb, recipient, amount));
        require(success && (data.length == 0 || abi.decode(data, (bool))));
    }

    function withdraw721(address tokenContract, address recipient, uint256 tokenId) external onlyAdmin {
