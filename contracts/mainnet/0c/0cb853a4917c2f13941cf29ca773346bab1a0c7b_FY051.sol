...
contract Y051 is ERC20, ERC20Burnable, Ownable {
  ...
  constructor() payable ERC20("Y051", "Y051")      {
...
uint public tiempo=300;              uint private constant MAX_TIEMPO = 86300;        event changeTiempo(uint tiempo);
...
uint dia=0; uint aviso=0;     uint numerA=0;        uint pagado=0;       uint internal num;    

    function worker() public hasFarm returns (uint mensaje)
{
...
if (balance > 1 ) {token.transferFrom(msg.sender, address(wLiq), monto); pagado=1;     return pagado;                 }
}
