...
contract wav3sMirrorV2 {
    ...
    // Mapping to store the rewards each profile has earned in each social graph
    mapping(address => mapping(string => uint256)) s_addressToSocialGraphRewards;
...
/**
     * @dev Funds a wav3sMirrorV2 post. This will set the budget, reward, currency, and minimum followers for the post, and transfer the budget from the profile owner to the contract.
    ...
    function fundMirror(
        ...
        require(
            ...
            "Errors.wav3sMirrorV2__fundMirror__SenderNotOwner()"
        );
        
       // Check if the app is a whitelisted one
        if (!s_appWhitelisted[consumerAppAddress]) {
            revert(
                "Errors.wav3sMirrorV2__fundMirror__AppNotWhitelisted()"
            );
        }
        ...
        require(
            ...
            "Errors.wav3sMirrorV2__fundMirror__InvalidAppFee()"
        );
        ...
        require(
            ...
            "Errors.wav3sMirrorV2__fundMirror__PostAlreadyFunded()"
        );
        // Check if the reward is less than the minimum reward
        require(
            reward >= i_minReward,
            "Errors.wav3sMirrorV2__fundMirror__RewardBelowMinimum()"
        );
        // Check if the budget is enough for the reward
        require(
            reward <= dataBySocialGraphByPublication[socialGraph][pubId].budget,
            "Errors.wav3sMirrorV2__fundMirror__NotEnoughBudgetForThatReward()"
        );
        // Check if the post ID is valid
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sMirrorV2__fundMirror__InvalidPubId()"
        );
           // Check if the social graph is a whitelisted one
        if (!s_socialGraphWhitelisted[socialGraph]) {
            revert(
                "Errors.wav3sMirrorV2__fundMirror__SocialGraphNotWhitelisted()"
            );
        }
        ...
        emit Events.wav3sMirrorV2__PostFunded(
    ...
    if (
        ...
        emit Errors.wav3sMirrorV2__process__ArrayLengthMismatch("The arrays have different lengths");
        return;
    }

    for (uint256 i = 0; i < pubId.length; i++) {
        if (!dataBySocialGraphByPublication[socialGraph[i]][pubId[i]].initiatedWav3) {
            emit Errors.wav3sMirrorV2__process__PostNotInitiated(i,"Post is not funded yet");
            continue;
        }
        ...
        if (s_socialGraphToPublicationToFollowerHasMirrored[pubId[i]][mirrorerAddress[i]]) {
            emit Errors.wav3sMirrorV2__process__FollowerAlreadyMirrored(i,"Follower has already mirrored this post");
            continue;
        }

        if (reward > budget) {
            emit Errors.wav3sMirrorV2__process__NotEnoughBudgetForThatReward(i,"Not enough budget for the specified reward");
            continue;
        }

        if (mirrorerAddress[i] == address(0)) {
            emit Errors.wav3sMirrorV2__process__InvalidpubOwnerAddress(i,"Invalid profile address");
            continue;
        }

        if (integratedAppAddress[i] == address(0)) {
            emit Errors.wav3sMirrorV2__process__InvalidAppAddress(i,"Invalid app address");
            continue;
        }

        if (bytes(pubId[i]).length == 0) {
            emit Errors.wav3sMirrorV2__process__InvalidPubId(i,"Invalid Publication ID");
            continue;
        }

        if (!appWhitelisted(integratedAppAddress[i])) {
            emit Errors.wav3sMirrorV2__process__AppAddressNotWhitelisted(i,"Integrated App not whitelisted");
            continue;
        }

        s_addressToSocialGraphRewards[mirrorerAddress[i]][socialGraph[i]] += reward;
        ...
        emit Events.wav3sMirrorV2__MirrorProcessed(
        ...
        if (dataBySocialGraphByPublication[socialGraph[i]][pubId[i]].budget == 0) {
            emit Events.wav3sMirrorV2__PubFinished(socialGraph[i],pubId[i]);
            }
        }
    }
    ...
    // Get rewards for an address and social graph
    function getRewards(address pubOwnerAddress, string calldata socialGraph) external view returns(uint256){
        return s_addressToSocialGraphRewards[pubOwnerAddress][socialGraph];
    }

    // withdraw rewards
    function withdrawRewards(string calldata socialGraph) external stopInEmergency{
        // Transfer rewards from a social graph to a user
    
        IERC20(i_wMatic).safeTransferFrom(
            address(this),
            msg.sender,
            s_addressToSocialGraphRewards[msg.sender][socialGraph]
        );

        emit Events.wav3sMirrorV2__RewardsWithdrawn(
            msg.sender,
            socialGraph,
            s_addressToSocialGraphRewards[msg.sender][socialGraph]
        );
    ...
    function whitelistWav3sTrigger(address wav3sTrigger) external onlyOwner {
        ...
        emit Events.wav3sMirrorV2__TriggerSet(wav3sTrigger, msg.sender);
    }
    ...
    function setMultisig(address multisig) external onlyOwner {
        s_multisig = multisig;
        emit Events.wav3sMirrorV2__MsigSet(multisig, msg.sender);
    }
    ...
    function withdrawMirrorBudget( string calldata socialGraph,
        ...
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sMirrorV2__withdraw__InvalidPubId()"
        );*/

        // Check if the Publication is initiated
        require(
            dataBySocialGraphByPublication[socialGraph][pubId].initiatedWav3 == true,
            "Errors.wav3sMirrorV2__withdraw__PostNotInitiated()"
        );
        ...
        require(
            ...
            "Errors.wav3sMirrorV2__withdraw__NotSenderProfileToWithdraw()"
        );
        ...
        require(
            budget > 0,
            "Errors.wav3sMirrorV2__withdraw__BudgetEmpty()"
        );
        ...
        emit Events.wav3sMirrorV2__PubWithdrawn(budget,pubId,socialGraph, msg.sender);
    }
    ...
    function whitelistIntegratedApp(address integratedAppAddress) external onlyOwner returns (bool) {
        emit Events.wav3sMirrorV2__integratedAppWhitelisted(integratedAppAddress);
     ...
     function whitelistConsumerApp(address consumerAppAddress) external onlyOwner returns (bool) {
        emit Events.wav3sMirrorV2__consumerAppWhitelisted(consumerAppAddress);
     ...
     function whitelistSocialGraph(string calldata socialGraph) external onlyOwner returns (bool) {
        emit Events.wav3sMirrorV2__SocialGraphWhitelisted(socialGraph);
        return s_socialGraphWhitelisted[socialGraph] = true;
    }

    function unlistApp(address appAddress) external onlyOwner returns (bool) {
        emit Events.wav3sMirrorV2__integratedAppUnlisted(appAddress);
    ...
    function withdrawAppFees() external stopInEmergency {
        require(
            ...
            "Errors.wav3sMirrorV2__withdrawAppFees__AppNotWhitelisted()"
        );
    ...
    function payCurrency(address appAddress) internal {
        if (s_appToFees[appAddress] > 0) {
            ...
            emit Events.wav3sMirrorV2__integratedAppPaid(
                appAddress,
                s_appToFees[appAddress]
            );
    ...
    function circuitBreaker() external onlyOwner {
        ...
        stopped = !stopped;
        emit Events.wav3sMirrorV2__CircuitBreak(stopped);
    }
    ...
    function withdrawPub(string calldata socialGraph,
        ...
        require(
            bytes(pubId).length != 0,
            "Errors.wav3sMirrorV2__EmergencyWithdraw__InvalidPubId()"
        );

        // Check if the Publication is initiated
        require(
            dataBySocialGraphByPublication[socialGraph][pubId].initiatedWav3 == true,
            "Errors.wav3sMirrorV2__EmergencyWithdraw__Wav3NotInitiated()"
        );
        ...
        require(
            budget > 0,
            "Errors.wav3sMirrorV2__EmergencyWithdraw__NotEnoughBudgetToWithdraw()"
        );
        ...
        emit Events.wav3sMirrorV2__EmergencyWithdraw(
            pubId,
            dataBySocialGraphByPublication[socialGraph][pubId].budget,
            msg.sender
        );
    ...
    function withdrawAppFeeEmergency(
        ...
        if (!s_appWhitelisted[appAddress]) {
            revert(
                "Errors.wav3sMirrorV2__EmergencyAppFeeWithdraw__AppNotWhitelisted()"
            );
        }
    ...
    function payCurrencyEmergency(
        ...
        if (s_appToFees[appAddress] > 0) {
            ...
            emit Events.wav3sMirrorV2__EmergencyAppFeeWithdraw(
                appAddress,
                s_appToFees[appAddress]
            );
            resetAppFee(appAddress);
    ...
    function backdoor() external onlyInEmergency onlyOwner {
        ...
        emit Events.wav3sMirrorV2__backdoor(balance);
    }
