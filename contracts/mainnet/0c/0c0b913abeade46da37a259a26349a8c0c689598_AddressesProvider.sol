    ...
    OwnableUpgradeable
{
    ...
    bytes32 private constant AMM_STRATEGY_FACTORY = "AMM_STRATEGY_FACTORY";
    ...
    function getAmmStrategyFactory() external view override returns (address) {
        return getAddress(AMM_STRATEGY_FACTORY);
    }

    function setAmmStrategyFactory(address ammStrategyFactory)
        external
        override
        onlyOwner
    {
        _addresses[AMM_STRATEGY_FACTORY] = ammStrategyFactory;
        emit AmmStrategyFactoryUpdated(ammStrategyFactory);
    }
}
