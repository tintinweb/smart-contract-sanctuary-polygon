...
contract MarketPlace is AccessHelper, ReentrancyGuard, MarketPlaceEvents {
    ...
    // user => token => number of sell orders
    ...
    function listAvailableTokensForSale(
        ...
        for (uint256 i = 0; i < contractCounter.current(); i++) {
            ...
            FundingTerms fundingTerms = FundingTerms(
    ...
    function createBid(
        ...
        require(_pricePerToken != 0, "price per token gt 0");

        require(_quantity > 0, "quantaty cannot be 0");
        require(_fullPrice > 0, "_fullPrice cannot be 0");

        bool isCorrectPrice = validateOrder(
            _quantity,
            _fullPrice,
            _pricePerToken,
            18
        );
        require(isCorrectPrice == true, "prices are not correct");

        FundingTerms fundingTerms = FundingTerms(_fundingTermsAddress);
    ...
    function sell(
        ...
        IERC20(_payToken).transfer(bid.buyer, totalPrice - platformFeeTotal);
    ...
    function getActiveOrders(
        ...
        uint256 numActiveAsks = userAsksCounter[user][address(this)];
        uint256 numActiveBids = userBidsCounter[user][address(this)];
        uint256 totalOrders = numActiveAsks + numActiveBids;
        Order[] memory orders = new Order[](totalOrders);
        uint256 orderIndex = 0;

        for (uint256 i = 0; i < numActiveAsks; i++) {
            Order storage order = askOrders[user][i];
            if (!order.sold) {
        ...
        for (uint256 i = 0; i < numActiveBids; i++) {
            Order storage order = bidOrders[user][i];
            if (!order.sold) {
                orders[orderIndex] = order;
                orderIndex++;
            }
    ...
    function validateOrder(
        ...
        uint256 usdcDecimals = 6; // USDC has 6 decimal places
        uint256 pricePerTokenInUsdc = pricePerToken /
            (10 ** (tokenDecimals - usdcDecimals));
        uint256 totalFullPriceInUsdc = fullPrice;
        uint256 expectedFullPriceInUsdc = pricePerTokenInUsdc * quantity;
        return totalFullPriceInUsdc == expectedFullPriceInUsdc;
    }
}
