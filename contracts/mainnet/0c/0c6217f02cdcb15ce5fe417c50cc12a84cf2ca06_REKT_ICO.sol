...
contract REKT_ICO is Ownable {    
    ...
    uint256 public CLAIM_INTERVAL = 1 weeks;
    ...
    function claimPrivate() external {
        ...
        require(MAP_CLAIM_DATETIME_PRIVATE[msg.sender] <= block.timestamp, "Should wait until next claim date");
        ...
        MAP_CLAIM_DATETIME_PRIVATE[msg.sender] = block.timestamp + CLAIM_INTERVAL;
    }

    function claimWhitelist() external {
        require(ICO_ENDED , "ICO is not finished");

        require(MAP_CLAIM_DATETIME_WHITELIST[msg.sender] <= block.timestamp, "Should wait until next claim date");
        ...
        MAP_CLAIM_DATETIME_WHITELIST[msg.sender] = block.timestamp + CLAIM_INTERVAL;
