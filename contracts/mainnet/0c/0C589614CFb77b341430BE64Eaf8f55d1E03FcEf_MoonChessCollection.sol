    ...
    ERC1155Supply
{
    ...
    string private uriPrefix =
        "ipfs://Qmem7BVgwH9TA4EwZnocskEeKTo73nFKFkdhSPEZL5NUyf/";
    string private uriSuffix = ".json";

    uint256 public price = 55 ether;
    uint256 public supply = 250;
    uint256 public maxSupply = 5000;
    uint256 private collectionsReleased = 4;

    uint256 teamAllocation = 500;
    ...
    function mint(uint256 id, uint256 amount) external payable {
        ...
        require(totalSupply(id) + amount <= supply, "Max supply reached");
        require(
            msg.value >= amount * price,
            "Not enough value sent for your mint!"
        );
        require(id > 0 && id <= collectionsReleased, "TokenId nonexistent");
        ...
        payable
    {
        ...
        for (uint256 i = 0; i < ids.length; i++) {
            require(
                totalSupply(ids[i]) + amounts[i] <= supply,
            ...
            s += amounts[i];
            require(
                ids[i] > 0 && ids[i] <= collectionsReleased,
                "TokenId nonexistent"
            );
        }
        require(msg.value >= price * s, "Not enough value sent for your mint!");
    ...
    // Increse suplly, but not more than maxSupply
