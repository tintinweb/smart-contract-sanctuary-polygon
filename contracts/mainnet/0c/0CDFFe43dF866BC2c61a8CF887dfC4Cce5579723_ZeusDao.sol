...
contract ZeusDao is Context, IERC20, Ownable {
    ...
    uint256 private _tTotal = 5000 * 10**9;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "ZeusDao v2";
    string private _symbol = "Zeus";
    uint8 private _decimals = 9;
    
    address public deadAddress = 0x000000000000000000000000000000000000dEaD;
    address public marketingWallet = 0x38B2714a1c582FAef4Cfd8DbA9b9Ad0e0BaEE463;
    ...
    uint256 public minimumTokensBeforeSwap = 5000 * 10**9;
    uint256 public _maxBuyTxAmount = 2500 * 10**9;
    uint256 public _maxSellTxAmount = 1500 * 10**9;
    uint256 public maxWalletToken = 1000 * (10**9);
    ...
    function _transfer(address from, address to, uint256 amount) private 
    {
     ...
     function swapTokensForUsdt(uint256 tokenAmount) private {
        ...
        path[2] = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F; // USDT cotract address
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private 
    {
        if(recipient==uniswapV2Pair && !_isExcludedFromFee[sender])
        {
            ...
            setAllFees(_saleTaxFee, _saleMarketingFee);
    ...
    function setAllFees(uint256 taxFee, uint256 marketingFee) private {
        _taxFee = taxFee;
        _marketingFee = marketingFee;
    }
    
    
    function setFees(uint256 taxFee, uint256 marketingFee) 
    external onlyOwner() {
        setAllFees(taxFee, marketingFee);
        _previousTaxFee = taxFee;
        _previousMarketingFee = marketingFee;
    }
    
    
    function setSaleFees(uint256 taxFee, uint256 marketingFee) 
    external onlyOwner() {
        _saleTaxFee = taxFee;
...
event CoolDownPeriodUpdated(address indexed operator, uint256 newCoolDownPeriod);
...
function checkForCoolDown(address from, address to) public
{
    ...
    {
        uint256 coolDownTimestamp = lastTxnTimeStamp[from].add(sellCooldownperiod);

        require(coolDownTimestamp < block.timestamp, "Wait till one hour Cool Down Pd");    
