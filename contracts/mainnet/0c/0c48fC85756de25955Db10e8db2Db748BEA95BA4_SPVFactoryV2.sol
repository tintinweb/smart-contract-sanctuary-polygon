...
import "./lib/ISPVContract.sol";

// uint256: as long as we have only one (unpacked) variable in Ctor, we don't need to reduce the integer size because it won't save any gas
type PermissionSetIdType is uint256;
...
contract ERC20SPV2 is ERC20Burnable, ISPVContract, PermissionsAware {
    bytes32 public constant PROP_SPV_NAME = keccak256("PROP_SPV_NAME");
    bytes32 public constant PROP_DENOMINATION = keccak256("PROP_DENOMINATION");
    bytes32 public constant PROP_VENTURE_PARTNER = keccak256("PROP_VENTURE_PARTNER");
    ...
    event PermissionSetIdChanged(
        PermissionSetIdType indexed oldPermissionSetId,
        PermissionSetIdType indexed newPermissionSetId
    );
    event SetRoleIdOverwritten(uint256 roleId, bool overwrite);

    mapping(bytes32 => string) private _properties;

    PermissionSetIdType private _permissionSetId;

    mapping(uint256 => bool) _overwrittenRoleIds;
    ...
    constructor(
        ...
        PermissionSetIdType permissionSetId_,
        string memory name_,
        string memory symbol_,
        string memory spvName_,
        string memory venturePartner_,
        string memory denomination_
    ) ERC20(name_, symbol_) PermissionsAware(permissions_) {
        _permissionSetId = permissionSetId_;
        
        _properties[PROP_SPV_NAME] = spvName_;
        _properties[PROP_VENTURE_PARTNER] = venturePartner_;
    ...
    function isRoleIdOverwritten(uint256 roleId) external view returns (bool) {
        return _overwrittenRoleIds[roleId];
    }

    function setRoleIdOverwrite(uint256 roleId, bool overwrite) external {
    ...
    function getPermissionSetId() external view returns (PermissionSetIdType) {
        return _permissionSetId;
    }

    function setPermissionSetId(PermissionSetIdType permissionSetId) external onlyRole(TOKEN_ROLE_ADMIN) {
        if(PermissionSetIdType.unwrap(_permissionSetId) == PermissionSetIdType.unwrap(permissionSetId)) {
            revert ErrPermissionSetIDWasAlreadySet();
        }

        PermissionSetIdType oldPermissionSetId = _permissionSetId;
        ...
        for (uint256 i = 0; i < 4; ++i) {
            if (PermissionSetIdType.unwrap(oldPermissionSetId) > 0) {
    ...
    function transformedRoleId(PermissionSetIdType permissionSetId_, uint256 roleId) public pure returns (uint256) {
        return PermissionSetIdType.unwrap(permissionSetId_) * PERMISSION_ID_DELTA + roleId;
    }
    ...
    function setProperty(bytes32 key, string memory val_) external onlyRole(TOKEN_ROLE_ADMIN) {
     ...
     */
    function mint(address to, uint256 amount) public onlyRole(TOKEN_ROLE_MINTER) {
        _mint(to, amount);
    }

    /* function increaseAllowanceWithBeneficiary(address spender, address beneficiary, uint256 addedValue) public override returns (bool) {
        address owner = _msgSender();
        // _approve(owner, spender, allowance(owner, spender) + addedValue);
        return true;
    } */
    ...
    function _beforeTokenTransfer(
        ...
        if (from != address(0)) {
            ...
            if (!_hasRole(TOKEN_ROLE_TRANSFERER, from)) {
                revert(
                    string(
                        abi.encodePacked(
                            "ERC20SPV2: account ",
        ...
        if (to != address(0) && _hasRole(TOKEN_ROLE_IS_BLACKLISTED, to)) {
            revert(
                string(abi.encodePacked("ERC20SPV2: account ", Strings.toHexString(uint160(to), 20), " is blacklisted"))
            );
        }

        if (!_hasRole(TOKEN_ROLE_IS_WHITELISTED, to)) {
            revert(
                string(
                    abi.encodePacked("ERC20SPV2: account ", Strings.toHexString(uint160(to), 20), " is not verified")
                )
            );
        }
...
import "./ERC20SPV2.sol";

contract SPVFactoryV2 is PermissionsAware {
    event SpvIssued(
        address indexed result,
        PermissionSetIdType indexed permissionSetId,
        string name,
        string symbol,
        string spvName
    );
    ...
    function create(
        PermissionSetIdType permissionSetId,
        string calldata name,
        string calldata symbol,
        string calldata spvName,
        string calldata venturePartner,
        string calldata denomination
    ) external {
        _checkRole(TOKEN_ROLE_DEPLOYER);

        ERC20SPV2 token = new ERC20SPV2(
            _permissions,
            permissionSetId,
            name,
            symbol,
        ...
        emit SpvIssued(address(token), permissionSetId, name, symbol, spvName);
    }
...
interface ISPVContract is IERC20 {
