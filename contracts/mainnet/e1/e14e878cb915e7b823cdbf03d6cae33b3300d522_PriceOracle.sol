...
contract PriceOracle is Governable {
    ...
    function calculateAssetPrice(address _asset) public view returns (uint256) {
        ...
        if(tokenType == AssetType.Token) {
        ...
        } else if(tokenType == AssetType.LinearOrStableBalancer) {
            // Check if it is a boosted pool.
            bool isBoosted;
            try IBalancerPool(_asset).getMainToken() {
                isBoosted = true;
            } catch {
                isBoosted = false;
            }
            ...
            uint256 price;
            if(isBoosted) {
                price = (IBalancerPool(_asset).getRate() * calculateAssetPrice(IBalancerPool(_asset).getMainToken())) / 1e18;
            } else {
                (IERC20[] memory tokens,,) = IBalancerVault(IBalancerPool(_asset).getVault()).getPoolTokens(IBalancerPool(_asset).getPoolId());

                price = (IBalancerPool(_asset).getRate() * calculateAssetPrice(address(tokens[0]))) / 1e18;
            }

            return price;
        } else if(tokenType == AssetType.BelugaVault) {
            return (IVault(_asset).getPricePerFullShare() * calculateAssetPrice(IVault(_asset).underlying())) / 1e18;
        }
            ...
            if(isStableswapLP) {
                ...
                try IBalancerPool(_token).getVault() {
                    ...
                    try IRewardGauge(_token).voting_escrow() {
                        ...
                        // If it is not a reward gauge, then we must check if it is a cToken.
                        try ICToken(_token).comptroller() {
                            ...
                            try IVaultYearnLike(_token).pricePerShare() {
                                ...
                                try IBalancerPool(_token).getRate() {
                                    return AssetType.LinearOrStableBalancer;
                                } catch {
                                    try IVault(_token).getPricePerFullShare() {
                                        return AssetType.BelugaVault;
                                    } catch {
