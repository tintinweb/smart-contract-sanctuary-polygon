...
contract BondDepository is WarpBase {
     ...
     *  @return uint
     */
    function redeem(uint256 _tokenId, address _stake) external returns (uint256) {
     ...
     *  @param _recipient address (The receiving address)
     *  @param _staking address (The address of the staking portal to stake in)
     *  @param _amount uint (The amount to stake in the portal)
     *  @return uint
     */
    function stakeOrSend(
        ...
        if (_staking == address(0)) {
            ...
            if (useHelper) {
                ...
                IStakingHelper(stakingHelper).stake(_amount, _recipient, _staking);
            } else {
                IERC20Upgradeable(WARP).approve(_staking, _amount);
                IStaking(_staking).stake(_amount, _recipient);
            }
        }
        return _amount;
    }
