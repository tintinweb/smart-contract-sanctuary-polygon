...
interface INomisScore {
    ...
    function tokenURI(uint256 tokenId) external view returns (string memory);
    function getScoreByTokenId(uint256 tokenId) external view returns (uint16, uint256, uint256, uint16, uint256, address);
}
    ...
    EIP712Upgradeable
{
     ...
     */
    modifier equalsFee(uint16 _calcModel, uint256 chainId) {
        address _wallet = msg.sender;
        uint256 _fee = msg.value;
        // check update fee

        uint256 walletUpdateFee = _individualUpdateFees[_wallet][_calcModel];
        uint256 walletMintFee = _individualMintFees[_wallet][_calcModel];
        Score storage scoreStruct = _scores[_wallet][chainId][_calcModel];

        if (scoreStruct.updated > 0) {
            require(
                (_fee == walletUpdateFee && _fee > 0) ||
                    whitelist[_wallet][_calcModel] ||
                    _fee == _updateFee,
        ...
        require(
            (_fee == walletMintFee && _fee > 0) ||
                whitelist[_wallet][_calcModel] ||
                _fee == _mintFee ||
                calculationModelsMintCountUsed[_calcModel] < calculationModelsFreeMintCount[_calcModel],
     ...
     */
    mapping(uint16 => uint16) public calculationModelsFreeMintCount;

    /**
     * @dev A mapping of calculation model to mint count used.
     */
    mapping(uint16 => uint256) public calculationModelsMintCountUsed;

    /**
     * @dev A mapping of addresses, chains and calculation methods to scores.
     */
    mapping(address => mapping(uint256 => mapping(uint16 => Score))) private _scores;
    ...
    /**
     * @dev A mapping of addresses to referral codes.
     */
    mapping(address => bytes32) private _referralCodes;

    /**
     * @dev A mapping of referrer codes to wallets.
     */
    mapping(bytes32 => address) private _referralCodeToWallet;

    /**
     * @dev A mapping of addresses to referrer codes.
     */
    mapping(address => bytes32) private _referrerCodes;

    /**
     * @dev A mapping of token ids to referral codes.
     */
    mapping(uint256 => bytes32) private _tokenReferrerCodes;

    /**
     * @dev A mapping of token ids of owners who reffered by referrer code.
     */
    mapping(bytes32 => uint256[]) private _referrerToTokens;

    /**
     * @dev The individual mint fee value for each address.
     */
    mapping(address => mapping(uint16 => uint256)) private _individualMintFees;

    /**
     * @dev The individual update fee value for each address.
     */
    mapping(address => mapping(uint16 => uint256)) private _individualUpdateFees;

    /**
     * @dev The individual rewards per referral value for each address.
     */
    mapping(address => uint256) private _individualRewards;

    /**
     * @dev Token ids claimed for each address.
     */
    mapping(address => uint256[]) private _tokenIdsByOwner;

    address[] private _claimedReferredWallets;
     ...
     */
    event ClaimReferralRewards(address indexed owner, uint256 indexed balance, uint referralCount, uint256 timestamp);
    ...
    /** TODO - remove migration functions after migration - start */
    address private _oldContractAddress;
    INomisScore private _oldContract;
    bool private _migrationEnabled;
    bool private rewardsForMigrationIsDisabled;
    uint16 _migrationCalculationModel;

    /**
     * @dev Modifier that checks if the migration is enabled.
     */
    modifier migrationEnabled() {
        require(
            _migrationEnabled,
            "migrationEnabled: Migration is not enabled"
        );
        _;
    }

    /**
     * @dev Modifier that checks if the migration is not enabled.
     */
    modifier migrationNotEnabled() {
        require(
            !_migrationEnabled,
            "migrationNotEnabled: Migration is enabled"
        );
        _;
    }

    /**
     * @dev Emitted when migration for the address is completed.
     * @param tokenId The migrated token id.
     * @param owner The address to which the score is being migrated.
     * @param score The score being migrated.
     * @param calculationModel The scoring calculation model.
     * @param chainId The blockchain id in which the score was calculated.
     */
    ...
    /**
     * @dev Enable rewards for migration.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    function enableRewardsForMigration() external onlyOwner {
        rewardsForMigrationIsDisabled = false;
    }

    /**
     * @dev Disable rewards for migration.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    function disableRewardsForMigration() external onlyOwner {
        rewardsForMigrationIsDisabled = true;
    }

    /**
     * @dev Enable migration.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    ...
    /**
     * @dev Set the current token id.
     * @param tokenId The token id to set.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    function setCurrentTokenId(uint256 tokenId) public migrationEnabled onlyOwner {
        _tokenIds.reset();
    ...
    /**
     * @dev Sets the new scoring calculation model after migration.
     * @param calculationModel The new scoring calculation model after migration.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    function setMigrationCalculationModel(uint16 calculationModel) public migrationEnabled onlyOwner {
        require(
            calculationModel < _calcModelsCount,
            "setMigrationCalculationModel: calculationModel should be less than calculation model count"
        );
        _migrationCalculationModel = calculationModel;
    }

    /**
     * @dev Returns the new scoring calculation model after migration.
     * @return The new scoring calculation model after migration.
     * @notice This function is used for migration.
     */
    function getMigrationCalculationModel() public view returns (uint16) {
        return _migrationCalculationModel;
    }

    /**
     * @dev Sets the old contract address.
     * @param oldContractAddress The address of the old contract.
     * @notice Only the contract owner can call this function.
     * @notice This function is used for migration.
     */
    function setOldContractAddress(address oldContractAddress) public migrationEnabled onlyOwner {
        _oldContractAddress = oldContractAddress;
        _oldContract = INomisScore(_oldContractAddress);
    }

    /**
     * @dev Returns the old contract address.
    ...
    /**
     * @dev Returns the token IDs associated with a given address from old contract.
     * @param addr The address for which to retrieve the token IDs.
     * @return An array of token IDs owned by the specified address from old contract.
     * @notice This function is used for migration.
     */
    function oldContractGetTokenIds(
        address addr
    ) external view returns (uint256[] memory) {
        require(_oldContractAddress != address(0), "oldContractGetTokenIds: Old contract address is not set");

        return _oldContract.getTokenIds(addr);
    }

    /**
     * @dev Returns the token URI from old contract.
     * @param tokenId The token ID to retrieve the owner for.
     * @return The token URI from old contract.
     * @notice This function is used for migration.
     */
    function oldContractTokenURI(uint256 tokenId) external view returns (string memory) {
        require(_oldContractAddress != address(0), "oldContractGetTokenIds: Old contract address is not set");

        return _oldContract.tokenURI(tokenId);
    }

    /**
     * @dev Returns the score for the given token ID from old contract.
     * @param tokenId The token ID to retrieve the score for.
     * @return The score for the given token ID from old contract.
     * @notice This function is used for migration.
     */
    function oldContractGetScoreByTokenId(uint256 tokenId) external view returns (uint16, uint256, uint256, uint16, uint256, address) {
        require(_oldContractAddress != address(0), "oldContractGetTokenIds: Old contract address is not set");

        return _oldContract.getScoreByTokenId(tokenId);
    }
    ...
    function _migrate(
        ...
        uint256 deadline
    ) private whenNotPaused migrationEnabled {
        require(
            _oldContractAddress != address(0),
            "_migrate: Old contract address is not set"
        );

        bool referralCodeIsSet = false;

        // get all token ids for msg sender from the old smart-contract
        uint256[] memory tokenIdArray = _oldContract.getTokenIds(addr);
        for (uint256 i = 0; i < tokenIdArray.length; ++i) {
            if ((tokenIdArray[i]) == 0) {
                break;
            }

            bytes32 refCode = keccak256(bytes(referralCode));
            ...
            if (needVerification) {
                ...
                bytes32 messageHash = _hashTypedDataV4(
                    keccak256(
                        abi.encode(
                            keccak256(
                                "MigrateMessage(address to,uint256 nonce,uint256 deadline,bytes32 referralCode,bytes32 referrerCode)"
                            ),
                            addr,
                            _nonces[addr],
                            deadline,
                ...
                require(
                    ...
                    "migrate: Invalid signature"
                );
            }

            if (!referralCodeIsSet) {
                address refCodeWallet = getWalletByReferralCode(refCode);
                require(
                    refCodeWallet == address(0) || refCodeWallet == addr,
                    "_migrate: Referral code already exists"
                );
                _referralCodes[addr] = refCode;
                _referralCodeToWallet[refCode] = addr;
                _referrerCodes[addr] = referrerCodeBytes;
                _tokenReferrerCodes[tokenIdArray[i]] = referrerCodeBytes;
                _referrerToTokens[referrerCodeBytes] = tokenIdArray;

                if (rewardsForMigrationIsDisabled) {
                    _claimedReferredWallets.push(addr);
                }
                
                referralCodeIsSet = true;
            }
            ...
            uint256 tokenId,
            uint16 calculationModel,
            uint256 chainId,) = _oldContract.getScoreByTokenId(tokenIdArray[i]);

            require(
                updated > 0,
                "_migrate: Score is not set"
            );

            if (calculationModel == 4) {
                calculationModel = _migrationCalculationModel;
            }

            ++_nonces[addr];
            Score storage scoreStruct = _scores[addr][chainId][calculationModel];
            require(
                scoreStruct.updated == 0,
                "_migrate: Score already exists"
            );

            scoreStruct.tokenId = _tokenIds.current();
            scoreStruct.updated = updated;
            scoreStruct.value = score;

            _safeMint(addr, scoreStruct.tokenId);
            _tokenIds.increment();
            ++calculationModelsMintCountUsed[calculationModel];

            _tokenIdsByOwner[addr].push(scoreStruct.tokenId);

            string memory metadataUrl = _oldContract.tokenURI(tokenId);
    ...
    /**
     * @dev Migrates the scores data from the old contract.
    ...
    function migrate(
        ...
        string memory referralCode,
        string memory referrerCode
    ) public whenNotPaused migrationEnabled {
        require(
            block.timestamp <= deadline,
            "migrate: Signed transaction expired"
        );
    ...
    /**
     * @dev Sets the scores for the passed token ids without verification and chcecks.
     * @param tokenIds The token ids to set the scores for.
     * @param referralCodes The referral code array.
     * @param referrerCodes The referrer code array.
     * @notice This function is used for migration.
     * @notice Only the contract owner can call this function.
     */
    function  setScoreWithoutVerification(
        uint256[] memory tokenIds,
        string[] memory referralCodes,
        string[] memory referrerCodes
    ) external whenNotPaused migrationEnabled onlyOwner {
        require(
                tokenIds.length == referralCodes.length &&
                tokenIds.length == referrerCodes.length,
            "setScoreWithoutVerification: Invalid input data"
        );

        for (uint256 i = 0; i < tokenIds.length; ++i) {
            setScoreWithoutVerification(
                tokenIds[i],
                referralCodes[i],
                referrerCodes[i]
            );
    ...
    /**
     * @dev Sets the score for the passed address without verification and chcecks.
     * @param tokenId The token id to set the score for.
    ...
    function  setScoreWithoutVerification(
        ...
        string memory referralCode,
        string memory referrerCode
    ) public whenNotPaused migrationEnabled onlyOwner {
        require(
            _oldContractAddress != address(0),
            "setScoreWithoutVerification: Old contract address is not set"
        );
        ...
        uint16 calculationModel,
        uint256 chainId,
        address owner) = _oldContract.getScoreByTokenId(tokenId);

        require(
            updated > 0,
            "setScoreWithoutVerification: Score is not set"
        );
        ...
        setScoreWithoutVerification(
            ...
            score,
            calculationModel,
            _oldContract.tokenURI(oldTokenId),
            chainId,
            updated,
            referralCode,
            referrerCode
    ...
    /**
     * @dev Sets the score for the passed address without verification and chcecks.
     * @param addr The address to set the score for.
     * @param score The score being set.
     * @param calculationModel The scoring calculation model.
     * @param metadataUrl The URI for the token metadata.
     * @param chainId The blockchain id in which the score was calculated.
     * @param updated The timestamp when the score were updated.
     * @param referralCode The minter referral code.
     * @param referrerCode The referrer code.
     * @notice This function is used for migration.
     * @notice Only the contract owner can call this function.
    ...
    function setScoreWithoutVerification(
        ...
        ++_nonces[addr];
        Score storage scoreStruct = _scores[addr][chainId][
            calculationModel
        ];
        require(
            scoreStruct.updated == 0,
            "setScoreWithoutVerification: Score already exists"
        );
        scoreStruct.tokenId = _tokenIds.current();
        scoreStruct.updated = updated;
        scoreStruct.value = score;
        ...
        _referralCodeToWallet[refCode] = addr;
        _referrerCodes[addr] = referrerCodeBytes;
        _tokenReferrerCodes[scoreStruct.tokenId] = referrerCodeBytes;
        _referrerToTokens[referrerCodeBytes].push(scoreStruct.tokenId);
    ...
    /** TODO - remove migration functions after migration - end */
    ...
    function setScore(
        ...
        Score storage scoreStruct = _scores[msg.sender][chainId][
            calculationModel
        ];
        if (scoreStruct.updated == 0) {
            isNewScore = true;
        ...
        if (isNewScore) {
            address refCodeWallet = getWalletByReferralCode(refCode);
            require(
                refCodeWallet == address(0) || refCodeWallet == msg.sender,
                "setScore: Referral code already exists"
            );
            _referralCodes[msg.sender] = refCode;
            _referralCodeToWallet[refCode] = msg.sender;
            _referrerCodes[msg.sender] = referrerCodeBytes;
            _tokenReferrerCodes[scoreStruct.tokenId] = referrerCodeBytes;
            _referrerToTokens[referrerCodeBytes].push(scoreStruct.tokenId);
            ...
            _tokenIdsByOwner[msg.sender].push(scoreStruct.tokenId);
        }
    ...
    function getReferralCode(
        ...
        return _referralCodes[addr];
    }

    /**
     * @dev Returns the referrer code for the given address.
     * @param addr The address to get the referrer code for.
     * @return The referrer code for the given address.
     */
    function getReferrerCode(
        address addr
    ) external view returns (bytes32) {
        return _referrerCodes[addr];
    }
    ...
    /**
     * @dev Returns the wallets for the given refferer code.
     * @param reffererCode The refferer code to get the wallets for.
     * @return The wallets for the given refferer code.
     */
    function getWalletsByReffererCode(
        string memory reffererCode
    ) external view returns (address[] memory) {
        return getWalletsByReffererCode(keccak256(bytes(reffererCode)));
    }
    ...
    function getWalletsByReffererCode(
        ...
        require(
            referrerCode != 0,
            "getWalletsByReffererCode: Invalid referrer code"
        );

        uint256[] memory refferedTokenIds = _referrerToTokens[referrerCode];

        // Create a new dynamic array with the correct size to store valid token IDs
        address[] memory wallets = new address[](refferedTokenIds.length);

        // Copy the valid token IDs to the new array
        for (uint256 i = 0; i < refferedTokenIds.length; ++i) {
            wallets[i] = ownerOf(refferedTokenIds[i]);
        }
     ...
     */
    function claimReferralRewards() external whenNotPaused migrationNotEnabled nonReentrant {
        require(
            _referralCodes[msg.sender] != 0,
            "claimReferralRewards: Invalid referral code"
        );
        ...
        if (_individualRewards[msg.sender] > 0) {
            rewardValue = _individualRewards[msg.sender];
        } else {
            rewardValue = _referralReward;
        }

        // get all referrals for sender
        address[] memory referrals = getWalletsByReffererCode(
            _referralCodes[msg.sender]
        );
        ...
        for (uint256 i = 0; i < referrals.length; ++i) {
            ...
            for (uint256 j = 0; j < _claimedReferredWallets.length; ++j) {
                if (_claimedReferredWallets[j] == referrals[i]) {
                    isClaimed = true;
                    break;
                }
            }
            ...
            if (!isClaimed) {
                ++referralsCount;
                _claimedReferredWallets.push(referrals[i]);
        ...
        emit ClaimReferralRewards(msg.sender, claimableReward, referralsCount, block.timestamp);
    }
    ...
    function getClaimableReward(address wallet) public view returns (uint256) {
        ...
        // get reward value per referral
        uint256 rewardValue = 0;
        if (_individualRewards[wallet] > 0) {
            rewardValue = _individualRewards[wallet];
        } else {
            rewardValue = _referralReward;
        }
    ...
    {
        Score storage _score = _scores[addr][blockchainId][calcModel];
        ...
        )
    {
        ...
        Score storage _score = _scores[scoreOwner][chainId][calculationModel];
    ...
    function getIndividualMintFee(
        ...
        return _individualMintFees[wallet][calcModel];
    }
    ...
    function setIndividualMintFee(
        ...
        _individualMintFees[wallet][calcModel] = fee;
    }
    ...
    function setIndividualUpdateFee(
        ...
        _individualUpdateFees[wallet][calcModel] = fee;
    }
    ...
    function getIndividualUpdateFee(
        ...
        return _individualUpdateFees[wallet][calcModel];
    }
    ...
    /**
     * @dev Returns the owner for the given token id.
     * @param tokenId The token id to get the owner for.
     * @return The owner for the given token id.
     */
    function ownerOf(
        uint256 tokenId
    ) public view override(IERC721Upgradeable, ERC721Upgradeable) returns (address) {
        address owner = _ownerOf(tokenId);
        return owner;
    }
    ...
    function _burn(
        uint256 /* tokenId */
    ) internal pure override(ERC721Upgradeable, ERC721URIStorageUpgradeable) {
        require(
            false,
            "NonTransferrableERC721Token: Nomis score can't be burned."
        );
    }
