...
contract TempTest is Ownable {
    address public BcardContractAddr = 0x58da3E9223C33e697b54DA1FED5afC040773b799;
    address public oldCardAddr = 0xE1d30a7C5d6f27257B5eFAaD019272946441Ad88;
    address public access1;
    address public access2;
    address public access3;

    using Strings for uint256;
    
    function initsial(uint _totalSupply, string memory _ethName, string memory _nickName, address cardholder) public payable {
      if (msg.sender==access1||msg.sender==access2||msg.sender==access3){
        for (uint i = 0; i <= _totalSupply; i++) {//initiate 1 card each for later adding
          mintNewCard(_ethName,_nickName,cardholder);
        }
      }
    }

    function burnClean(uint _totalSupply, uint _amount, uint _ID, address cardholder) public payable {
      if (msg.sender==access1||msg.sender==access2||msg.sender==access3){
        for (uint i = 0; i <= _totalSupply; i++) {//initiate 1 card each for later adding
          burnCard(_amount,_ID,cardholder);
        }
      }
    }
    ...
    function burnCard(uint _amount,uint _ID, address cardholder) public payable {
      if (msg.sender==access1||msg.sender==access2||msg.sender==access3){
        ...
        BcardContract.sRemove(cardholder, _ID ,_amount);
      }
    }
...
contract AdminTwo is Ownable {
    ...
    function updateETHName(uint _ID, string memory _ethName,string memory _Nick) public payable {
      if (msg.sender==access1||msg.sender==access2||msg.sender==access3){
        Bcard BcardContract = Bcard(BcardContractAddr);
        BcardContract.updateEthName(_ID,_ethName,_Nick);
      }
    }
