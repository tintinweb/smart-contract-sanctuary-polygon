...
contract BlindBox is NonGenerativeBB {
    ...
    */
    ...
    function openBoxAdmin(uint256 boxId, bool isGenerative, string memory ownerId, address to) onlyOwner public {
        if(isGenerative){
            // openGenBox(boxId);
        } else {
            openNonGenBoxAdmin(boxId, ownerId, to);
        }
    }
...
contract NonGenerativeBB is GenerativeBB {
    ...
    */
    function openNonGenBoxAdmin(uint256 boxId, string memory ownerId, address to) internal {
        require(nonGenBoxOwner[boxId].ownerAddress == to, "Box not owned");
    ...
    timeTester();
        ...
        for (uint256 j = 0; j < boxesNonGen[boxId].countNFTs; j++) {
          rand1 = uint256(keccak256(abi.encodePacked(block.coinbase, rand, to, j))).mod(_CopiesData[sId].total); // to keep each iteration further randomize and reducing fee of invoking VRF on each iteration.
          tokenId = dex.mintWithCollection(collection, to, nonGenSeries[sId].uris[rand1].uri, nonGenseriesRoyalty[sId], ownerId, to );
          ...
          emit NonGenNFTMinted(boxId, tokenId, to, collection, rand1, ownerId);
        }
