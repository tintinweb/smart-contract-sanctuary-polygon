...
contract Crowdfunding is Pausable, ReentrancyGuard {
    ...
    uint256 public totalFeeContributed;
    ...
    event SetTaxFee(uint256 tax);
    event RefundFees(uint256 startIndex, uint256 endIndex);
    event Invest(
        address investor,
        uint256 amountInvested,
    ...
    constructor(
        ...
        USDT = IERC20(_USDT);
        token = IERC20(_token);
        accessControl = IAccessControl(_whitelistAddress);
        priceFeed = AggregatorV3Interface(
            0x73366Fe0AA0Ded304479862808e02506FE556a98 // EUR / USD mainnet
        );
    }
        ...
        onlyRole(WHITELISTED_ROLE)
    {
        ...
        uint256 fee = (usdtContribution * (taxFee * 10)) / 1000;
        uint256 value = usdtContribution + fee;
        ...
        _updateValues(amountInDecimals, usdtContribution, fee);
        emit Invest(msg.sender, usdtContribution, _securityAmount, fee);
    }

    // Add investment manually
    function addInvestmentManually(address _account, uint256 _tokenAmount)
        external
        onlyRole(MANAGER_ROLE)
    {
        ...
        uint256 securityPurchased = getRequiredAmountOfSecurityFor(
            _tokenAmount
        );
        uint256 fee = 0;
        uint256 rest = restToSell();

        require(securityPurchased > 0, "Cannot buy 0 token.");
        require(rest > 0, "The hardcap has been reached.");
        require(securityPurchased <= rest, "Not enough token to sell.");
        require(
            accessControl.hasRole(WHITELISTED_ROLE, _account),
            "Account is not whitelisted."
        );

        uint256 amountInDecimals = _tokenAmount * 10**6;
        investorMap.addInvestment(
            _account,
            amountInDecimals,
            securityPurchased,
        ...
        uint256 securityDecimalLess = securityPurchased /
            (10**IERC20Metadata(address(token)).decimals());
        _updateValues(securityPurchased, amountInDecimals, fee);
        emit Invest(_account, amountInDecimals, securityDecimalLess, fee);
    }
    ...
    function refund() external whenNotPaused nonReentrant {
        ...
        uint256 investedAmount = investorMap.getTokenAmountFor(msg.sender);
        uint256 taxFeeFor = investorMap.getTaxFee(msg.sender);
        uint256 v = investedAmount + taxFeeFor;

        USDT.safeTransfer(msg.sender, v);
        investorMap.reinitInvestment(msg.sender);
        emit Refund(msg.sender, investedAmount);
    }
    ...
    function distributeTokensAfterCrowdsale(
        ...
        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            ...
            uint256 amount = investorMap.getSecurityTokenAmountFor(key);
            token.safeTransfer(key, amount);
            investorMap.reinitInvestment(key);
            investorsAddress[i] = key;
            investedAmounts[i] = amount;
        }
        emit Distribute(investorsAddress, investedAmounts);
    }
    ...
    function refundHoldersWithFee(uint256 _startIndex, uint256 _endIndex)
        external
        onlyRole(MANAGER_ROLE)
    {
        require(_isFinalized, "The sale is not finalized.");
        ...
        for (uint256 i = _startIndex; i <= _endIndex; i++) {
            address key = investorMap.getKeyAtIndex(i);
            uint256 amountToRefund = getRemainingFeeFor(key);
            USDT.safeTransfer(key, amountToRefund);
            investorMap.reinitTaxFee(key);
        }
        emit RefundFees(_startIndex, _endIndex);
    }

    function _updateValues(
        uint256 _securitySold,
        uint256 _usdtContribution,
        uint256 _feeContribution
    ) private {
        totalSecuritySold += _securitySold;
        totalUSDTContribution += _usdtContribution;
        totalFeeContributed += _feeContribution;
    }

    // Setters
    function setSaleDates(uint256 _startDate, uint256 _endDate)
        external
        onlyRole(MANAGER_ROLE)
    {
    ...
    function setTaxFee(uint256 _value) external onlyRole(MANAGER_ROLE) {
        require(_value != taxFee, "TaxFee has already this value");
        taxFee = _value;
        emit SetTaxFee(_value);
    }
    ...
    function setHardCap(uint256 _hardCap) external onlyRole(MANAGER_ROLE) {
        require(_hardCap > 0, "HardCap cannot be equal to zero.");
        hardCap = _hardCap;
    ...
    // Getters
    function restToSell() public view returns (uint256) {
        return hardCap - totalSecuritySold;
    }

    function getRemainingFeeFor(address _investorAddress)
        public
        view
        returns (uint256)
    {
        uint256 val = investorMap.getTaxFee(_investorAddress);
        uint256 contractBalance = getContractBalance();
        uint256 r = (val / totalFeeContributed) * contractBalance;
        return r;
    }
    ...
    function getTaxFeeFor(address _investorAddress)
        external
        view
        returns (uint256)
    {
        return investorMap.getTaxFee(_investorAddress);
    }
    ...
    function getRequiredAmountOfTokensFor(uint256 _securityTokens)
        ...
        returns (uint256)
    {
        uint256 amount = (_securityTokens * uint256(getLatestPrice())) / 100;
        return amount;
    }

    function getRequiredAmountOfSecurityFor(uint256 _tokens)
        public
        view
        returns (uint256)
    {
        uint256 amount = (_tokens * 10**8) / uint256(getLatestPrice());
