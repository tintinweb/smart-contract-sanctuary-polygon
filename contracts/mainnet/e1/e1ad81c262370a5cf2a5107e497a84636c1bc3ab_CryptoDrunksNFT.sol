...
contract CryptoDrunksNFT is ERC1155, Ownable {
    using Counters for Counters.Counter;
    Counters.Counter private _tokenIds;
    uint256 public mintMax = 1013;
    uint256 public mintFee = 88 ether;

    string public name = "CryptoDrunks";

    constructor() ERC1155("") {}
    //"ipfs://QmRsQQ5aYRBHRyCtZNNV2tmLvSC66SrdesugauaYP589sN/{id}.json"
    ...
    function mintOne(address account) payable public {
        require(msg.value >= mintFee, "Insufficient fee!");
        uint256 newItemId = _tokenIds.current();
        require(newItemId < mintMax, "No more, no more!");
        _tokenIds.increment();
        _mint(account, newItemId, 1, "");
    }

    function mintBatch(address to, uint256 count) public onlyOwner {
        require((_tokenIds.current()+count) <= mintMax, "No more available.");
