...
contract GSWForwarder is Initializable {
    ...
    event Executed(address indexed gswAddress, address indexed source, bytes metadata);

    /// @notice emitted if one of the actions in GSW.cast() fails
    event ExecuteFailed(address indexed gswAddress, address indexed source, bytes metadata, string reason);
    ...
    function execute(
        ...
        if (success_ == true) {
            emit Executed(address(gsw_), source_, metadata_);
        } else {
            emit ExecuteFailed(address(gsw_), source_, metadata_, revertReason_);
        }
    }
