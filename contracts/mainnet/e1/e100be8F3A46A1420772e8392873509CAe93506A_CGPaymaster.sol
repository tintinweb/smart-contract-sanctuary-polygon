...
// File: contracts/openGSN/utils/MinLibBytes.sol
...
// File: contracts/openGSN/utils/GsnUtils.sol
...
// File: contracts/openGSN/interfaces/IRelayRecipient.sol
...
// File: contracts/openGSN/interfaces/IStakeManager.sol
...
// File: contracts/openGSN/forwarder/IForwarder.sol
...
// File: contracts/openGSN/utils/GsnTypes.sol
...
// File: contracts/openGSN/utils/GsnEip712Library.sol
...
// File: contracts/openGSN/interfaces/IRelayHub.sol
...
// File: contracts/openGSN/interfaces/IPaymaster.sol
...
// File: contracts/openGSN/BasePaymaster.sol
...
// File: contracts/Paymaster/CGPaymaster.sol
...
contract CGPaymaster is Ownable, BasePaymaster {
  uint256 public inactiveDuration;
  address public target;

  uint256 public maximumRecipientDeposit;  //  used in RelayHub, can be updated.


  mapping(address => uint256) public lastTxTimestamp;
  mapping(bytes4 => bool) public permitMethodSignature;


  /**
   * @dev Constructor.
   * @param _target Smart Contract address, that should be passed as a recipient.
   * @param _inactiveDuration Duration for inactive period.
   */
  constructor(address _target, uint256 _inactiveDuration) {
    target = _target;
  ...
  function preRelayedCall(GsnTypes.RelayRequest calldata relayRequest, bytes calldata signature, bytes calldata approvalData, uint256 maxPossibleGas) external override virtual returns (bytes memory context, bool revertOnRecipientRevert) {
    ...
    require(block.timestamp >= lastTxTimestamp[relayRequest.request.from] + inactiveDuration, 'Too early');
    lastTxTimestamp[relayRequest.request.from] = block.timestamp;

    bytes4 sig = GsnUtils.getMethodSig(relayRequest.request.data);
    require(permitMethodSignature[sig], "Unpermitted sig");

    return ("", true);
  }

  function postRelayedCall(bytes calldata context, bool success, uint256 gasUseWithoutPost, GsnTypes.RelayData calldata relayData) external override virtual {
    (context, success, gasUseWithoutPost, relayData);
  }

  /**
   * @dev Updates duration for inactive period.
   * @param _inactiveDuration Duration period.
   */
  function updateInactiveDuration(uint256 _inactiveDuration) external onlyOwner {
    inactiveDuration = _inactiveDuration;
  }

  /**
   * @dev Updates target Smart Contract address.
   * @param _target Smart Contract address, that should be passed as a recipient.
   */
  function updateTargetAddress(address _target) external onlyOwner {
    target = _target;
  }

  /**
   * @dev Updates method signatures permitted to be used.
   * @param _permit Wheter permit or not.
   * @param _methodSignatures List of method signatures.
   */
  function updatePermitMethodSignature(bool _permit, bytes4[] memory _methodSignatures) external onlyOwner {
    for (uint256 i = 0; i < _methodSignatures.length; i++) {
      permitMethodSignature[_methodSignatures[i]] = _permit;
    }
  }

  /**
   * @notice Check this value in RelayHub Smart Contract.
   * @dev Updates maximumRecipientDeposit.
   * @param _amount Amount to be used.
   */
  function updateMaximumRecipientDeposit(uint256 _amount) external onlyOwner {
    maximumRecipientDeposit = _amount;
  }
  ...
  receive() external override payable {
    require(address(relayHub) != address(0), "relay hub address not set");
    require(maximumRecipientDeposit > 0, "wrong max depo");

    uint256 totalAmount = msg.value;

    while (totalAmount > 0) {
      uint256 transferAmount = (totalAmount < maximumRecipientDeposit) ? totalAmount : maximumRecipientDeposit;
      relayHub.depositFor{value:transferAmount}(address(this));
      totalAmount -= transferAmount;
    }
  }
 
  /**
   * @dev Destroys Smart Contract & transfers balance to owner.
   */
  function kill() external onlyOwner {
    uint256 balance = relayHub.balanceOf(address(this));
    if (balance > 0) {
      relayHub.withdraw(balance, payable(owner()));
    }
