...
// File contracts/IterativeVestingFarm.sol
...
contract IterativeVestingFarm is OwnableUpgradeable, ReentrancyGuardUpgradeable{
    ...
    // Time when farm is ending with vesting
    uint256 public endTime;
    // 1/vestingPortion of totalUserRewards
    uint256 public nVestingPortions;
    // On how many days is portion distributed
    uint256 public distributionIntervalDays;
    // Array of all reward times during factory
    uint256[] public vestingTimes;
    // Address of token that is vested
    IERC20 public vestedToken;
    // Total rewards that needs to be paid out
    uint256 public totalRewards;
    // Total rewards that we owe to the users
    ...
    // Mapping with users last withdrawals
    mapping(address => uint256) public userLastWithdrawnPortionNumber;
    // Did user collected first reward or not
    mapping(address => bool) public withdrawFirstPortion;
    // How many tokens we gave to user
    ...
    // If this farm has ability to be funded partially
    bool public partialFunding;
    // Initial percentage of total rewards that
    // farm must be funded with,at the beginning
    uint256 public initialFundPercent;
    ...
    event InsufficientFunds(
        uint256 indexed mising,
        address indexed farmAddress,
        bool indexed isActive
    );

    /**
     * @notice function is initializing contract
     *
     ...
     * @param _earlyClaimAvailablePer - percentage of claiming if claiming all early,
     * @param _farmImplementation - address of farm implementation
        ...
        initializer
    {
        ...
        require(
            _nVestingPortions > 0,
            "Portion of distribution has to be over 0"
        );
        require(
        ...
        require(
            _beforeStartClaimPer >= 0 && _beforeStartClaimPer <= 100,
            "Claim percentage should be above 0 and below 100"
        );
        require(
            _initialFundPercent >= 5 && _initialFundPercent <= 100,
            "Initial Fund Percent should be above 5 and up to 100"
        );

        uint256 distributionPeriod;
        if(_nVestingPortions == 1){
            distributionPeriod = 1;
        }
        else{
            require(
                _distributionIntervalDays > 0,
                "Delay between two vesting days needs to be over 0"
            );

            distributionPeriod = _distributionIntervalDays;
        }

        vestedToken = IERC20(_vestedToken);
        startTime = _startTime;
        nVestingPortions = _nVestingPortions;
        distributionIntervalDays = distributionPeriod;
        endTime = startTime +
            ((nVestingPortions - 1) * distributionIntervalDays * 24 * 60 * 60);
        isActive = false;
        earlyClaimAvailablePercent = _earlyClaimAvailablePer;
        claimAvailableBeforeStartPercent = _beforeStartClaimPer;
        initialFundPercent = _initialFundPercent;
        if(initialFundPercent == 100){
            partialFunding = false;
        }
        else{
            partialFunding = true;
        }
        ...
        uint256 rewardTime;
        for(uint256 i = 0; i < nVestingPortions; i++){
            if(i == 0){
                rewardTime = startTime;
                vestingTimes.push(rewardTime);
            }
            else{
                rewardTime = rewardTime +
                    (distributionIntervalDays * 24 * 60 * 60);
                vestingTimes.push(rewardTime);
            }
        }
        ...
        onlyOwner
    {
        ...
        for(uint256 i = 0 ; i < _rewards.length; i++){
            ...
            if(r.amount > 0 && !isFarmUser[r.user]){
                ...
                noOfUsers++;
                totalUserPayouts[r.user] = 0;
                userLastWithdrawnPortionNumber[r.user] = 0;
                withdrawFirstPortion[r.user] = false;
        ...
        onlyOwner
    {
        require(
            !isActive,
            "Iterative farm is activated, you can't remove user"
        );
        require(
        ...
        onlyOwner
    {
        if(!partialFunding){
            ...
            uint256 minNeededAmount = totalRewards
                .mul(initialFundPercent)
                .div(100);
            if(totalRewardsPendingBalance < minNeededAmount){
                minNeededAmount = totalRewardsPendingBalance;
            }

            require(
                _amount >= minNeededAmount,
                "Amount must be over initialFundPercent of what has left to be paid"
            );

            uint256 balanceBefore = vestedToken.balanceOf(address(this));
            ...
            uint256 balanceAfter = vestedToken.balanceOf(address(this));
            uint256 fundAmount = balanceAfter.sub(balanceBefore);

            require(
                fundAmount >= minNeededAmount,
                "Amount funded must be over initialFundPercent of what has left to be paid"
            );
        }
    ...
    {
        if(nVestingPortions != 1){
            uint256 rewardPerPortion = (totalRewards / nVestingPortions);
            uint256 nPortionsVested = (
                (
                    (
                        block.timestamp - startTime
                    ) / 86400
                ) / distributionIntervalDays
            );

            if(nPortionsVested > nVestingPortions - 1){
                nPortionsVested = nVestingPortions - 1;
            }

            nPortionsVested += 1;
            uint256 totalRewardsUnlocked = rewardPerPortion * nPortionsVested;
            ...
            return (0, totalRewards, totalWithdrawn, totalBurned);
        }
    }
     ...
     *
     * @return the percentage available to claim
     */
     ...
     *
     * @param user - address of user
     * @param frontEnd - flag who calls function
     *
     ...
     * @return nPortionsVested - how much user earned portions
     */
        ...
        )
    {
        ...
        if(userTotalRewards == userTotalPayouts){
            return(0, 0, 0, 0, nVestingPortions);
        }
        else{
            uint256 amountEarned;
            uint256 nPortionsVested;
            (amountEarned, nPortionsVested) = _withdrawCalculation(user, frontEnd);

            uint256 claimPercent = _getClaimPercent();

            uint256 totalLeftLockedForUser = userTotalRewards
                .sub(userTotalPayouts)
                .sub(amountEarned);
            uint256 burnPercent = 100 - claimPercent;
            uint256 claimAmountFromLocked = totalLeftLockedForUser
                .mul(claimPercent).div(100);
     ...
     * @param frontEnd - flag who calls function
     *
     * @return (amountEarned, nPortionsVested)
     * the amount earned and number of portions vested
     */
        ...
        returns (uint256, uint256)
    {
        if(!_vestingHasStarted()){
            ...
            uint256 amountEarned;
            uint256 rewardPerPortion = (
                totalUserRewards[address(user)] / nVestingPortions
            );
            uint256 nPortionsVested = (
                (
                    (
            ...
            if(!frontEnd){
                if(withdrawFirstPortion[address(user)]){
                    require(
                        userLastWithdrawnPortionNumber[address(user)] < nPortionsVested,
                        "User withdrew everything available at this moment"
                    );
                }
            }
            ...
            amountEarned = (
                (
                    (nPortionsVested - userLastWithdrawnPortionNumber[address(user)])
                ) * (totalUserRewards[address(user)] / nVestingPortions)
            );

            if(!withdrawFirstPortion[address(user)]){
                amountEarned = amountEarned + rewardPerPortion;
            }
        ...
        nonReentrant
    {
        ...
        bool flag = false;
        if(partialFunding){
            if(!_sufficientFunds(totalLeftLockedForUser + amountEarned)){
                isActive = false;
                //emit event
                uint256 totalAmount = totalLeftLockedForUser + amountEarned;
                emit InsufficientFunds(
                    totalAmount.sub(vestedToken.balanceOf(address(this))),
                    address(this),
                    isActive
                );
                flag = true;
        ...
        if(!flag){
            ...
            if (amountEarned >= 0){
                userLastWithdrawnPortionNumber[address(msg.sender)] = nPortionsVested;
                if(!withdrawFirstPortion[address(msg.sender)]){
                    withdrawFirstPortion[address(msg.sender)] = true;
                }

                totalRewardsPendingBalance = totalRewardsPendingBalance
                .sub(amountEarned + totalLeftLockedForUser);
                //transfer funds
                vestedToken.safeTransfer(
                    address(msg.sender),
                    amountEarned + claimAmountFromLocked
                );
                ...
                totalWithdrawn += (amountEarned + claimAmountFromLocked);
                // Update total withdrawn
    ...
    /**
     * @notice function is giving back rewards
     */
    function withdraw()
        external
        nonReentrant
    {
        uint256 amountEarned;
        uint256 nPortionsVested;
        (amountEarned, nPortionsVested) = _withdrawCalculation(address(msg.sender), false);

        bool flag = false;
        if(partialFunding){
            if(!_sufficientFunds(amountEarned)){
                isActive = false;
                emit InsufficientFunds(
                    amountEarned.sub(vestedToken.balanceOf(address(this))),
                    address(this),
                    isActive
                );
                flag = true;
        ...
        if(!flag){
            ...
            if (amountEarned > 0){
                ...
                totalUserPayouts[address(msg.sender)] += amountEarned;
                totalRewardsPendingBalance -= amountEarned;
                vestedToken.safeTransfer(address(msg.sender), amountEarned);
                emit RewardPaid(address(msg.sender), amountEarned);
        ...
        onlyOwner
    {
        ...
        isActive = false;
        emit EmergencyWithdraw(asset, amount, address(msg.sender));
    }
}