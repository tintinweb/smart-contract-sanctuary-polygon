...
contract Internals is Variables, Events {
    ...
    */
    function updateStorage(
        ...
        totalSupply_ = _rawSupply[token_].mul(newSupplyExchangePrice_).div(1e18);
        totalBorrow_ = _rawBorrow[token_].mul(newBorrowExchangePrice_).div(1e18);
        uint utilization_ = totalBorrow_.mul(10000).div(totalSupply_);
...
contract ProtocolModule is Internals {
    ...
    */
    ...
    /**
    * @dev transfer revenue from Liquidity to Treasury
    * @param tokens_ address of tokens.
    * @return amts_ amts of token that got transferred
    */
    function collect(
        address[] memory tokens_
    ) public nonReentrant returns (uint[] memory amts_) {
        require(_treasury != address(0), "setup-treasury-first");
        uint length_ = tokens_.length;
        require(length_ > 0, "");
        amts_ = new uint[](length_);
        for (uint i = 0; i < length_; i++) {
            (,,,, uint totalSupply_, uint totalBorrow_) = updateStorage(tokens_[i], 0, 0);
            IERC20 tokenContract_ = IERC20(tokens_[i]);
            uint balanceOf_ = tokenContract_.balanceOf(address(this));
            amts_[i] = balanceOf_.sub(totalSupply_.sub(totalBorrow_));
            tokenContract_.safeTransfer(_treasury, amts_[i]);
        }
        emit collectLog(tokens_, amts_);
    }
...
contract Events {
    ...
    event collectLog(address[] tokens_, uint[] amts_);
