...
abstract contract CurveExchange {
    ...
    function swap(address _curvePool, address _fromToken, address _toToken, uint256 _amount, address _oracleRouter) internal returns (uint256) {
        _checkPoolBalances(_curvePool);
        IERC20(_fromToken).approve(_curvePool, _amount);
    ...
    function onSwap(address _curvePool, address _fromToken, address _toToken, uint256 _amount) internal view returns (uint256) {
        _checkPoolBalances(_curvePool);
        uint256 _returned = IStableSwapPool(_curvePool).get_dy_underlying(_getTokenIndex(_fromToken), _getTokenIndex(_toToken), _amount);
