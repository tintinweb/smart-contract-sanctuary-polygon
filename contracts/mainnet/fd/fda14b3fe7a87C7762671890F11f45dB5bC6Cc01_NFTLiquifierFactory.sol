...
import "./NFTLiquifier.sol";

contract NFTLiquifierFactory is BoringOwnable {
    NFTLiquifier[] public ownerLiquifier;
    NFTLiquifier[] public publicLiquifier;
    NFTLiquifier public immutable liquifierMaster;

    constructor (NFTLiquifier _liquifierMaster){
        liquifierMaster = _liquifierMaster;
    }

    function makeLiquifier(
        IERC721 _tokenContract,
        bytes32 _merkleRoot,
        string memory _ipfsURL,
        string memory symbol
    ) external onlyOwner {
        NFTLiquifier liquifier = NFTLiquifier(liquifierMaster.deploy(address(liquifierMaster), abi.encode(_tokenContract, _merkleRoot, _ipfsURL, symbol), true));
        ownerLiquifier.push(liquifier);
    }

    function makeLiquifierPublic (
        IERC721 _tokenContract,
        bytes32 _merkleRoot,
        string memory _ipfsURL,
        string memory symbol
        ...
        NFTLiquifier liquifier = NFTLiquifier(liquifierMaster.deploy(address(liquifierMaster), abi.encode(_tokenContract, _merkleRoot, _ipfsURL, symbol), true));
        publicLiquifier.push(liquifier);
    }

    function transferOwnership(NFTLiquifier[] calldata liquifiers, address _newOwner) external onlyOwner {
        uint i;
        uint length = liquifiers.length;
        for (; i < length;) {
            liquifiers[i].transferOwnership(_newOwner, true, false);
            unchecked {
                ++i;
            }
        }
    ...
    function setFee(NFTLiquifier[] calldata liquifiers, uint256 fee, address payable feeRecipient) external onlyOwner {
        uint i;
        uint length = liquifiers.length;
        for (; i < length;) {
            liquifiers[i].setFee(fee, feeRecipient);
            unchecked {
                ++i;
            }
        }
    ...
    function collectFee(NFTLiquifier[] calldata liquifiers) external {
        uint i;
        uint length = liquifiers.length;
        for (; i < length;) {
            liquifiers[i].collectFees();
            unchecked {
                ++i;
            }
        }
