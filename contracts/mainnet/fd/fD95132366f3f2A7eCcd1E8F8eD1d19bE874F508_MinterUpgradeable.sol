...
contract MinterUpgradeable is IMinter, OwnableUpgradeable {
    ...
    uint public constant MAX_TEAM_RATE = 100; // 10%

    uint public constant WEEK = 86400 * 7; // allows minting once per week (reset every Thursday 00:00 UTC)
    uint public weekly; // represents a starting weekly emission of 2.6M ASCENT (ASCENT has 18 decimals)
    ...
    function initialize(    
        ...
        _ascent = IAscent(IVotingEscrow(__ve).token());
        ...
        weekly = 3_000_000 * 1e18; // represents a starting weekly emission of 3M ASCENT (ASCENT has 18 decimals)
        isFirstMint = true;
    ...
    function _initialize(
        ...
        if(max > 0){
            _ascent.mint(address(this), max);
            _ascent.approve(address(_ve), type(uint).max);
        ...
        _rewards_distributor.setLastTokenTime(active_period + WEEK);
    }
    ...
    function circulating_supply() public view returns (uint) {
        return _ascent.totalSupply() - _ascent.balanceOf(address(_ve));
    }
    ...
    function calculate_rebate(uint _weeklyMint) public view returns (uint) {
        uint _veTotal = _ascent.balanceOf(address(_ve));
        uint _ascentTotal = _ascent.totalSupply();
        
        uint lockedShare = (_veTotal) * PRECISION  / _ascentTotal;
    ...
    function update_period() external returns (uint) {
        ...
        if (block.timestamp >= _period + WEEK && _initializer == address(0)) { // only trigger if new week
            ...
            uint _balanceOf = _ascent.balanceOf(address(this));
            if (_balanceOf < _required) {
                _ascent.mint(address(this), _required - _balanceOf);
            }

            require(_ascent.transfer(team, _teamEmissions));
            
            require(_ascent.transfer(address(_rewards_distributor), _rebase));
            _rewards_distributor.checkpoint_token(); // checkpoint token balance that was just minted in rewards distributor
            _rewards_distributor.checkpoint_total_supply(); // checkpoint supply

            _ascent.approve(address(_voter), _gauge);
...
interface IRewardsDistributor {
    ...
    function setLastTokenTime(uint _time) external;
}
