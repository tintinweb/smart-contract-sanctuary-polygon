...
// File: contracts/ERC20ReflectionToken.sol
...
contract ERC20ReflectionToken is Context, IERC20, Ownable {
    ...
    constructor (string memory name,string memory symbol, uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 burnFee,address devAddress, uint256 setAmount) public payable {
        _name = name;
        _symbol = symbol;
        _taxFee = taxFee;
        ...
        _tTotal = setAmount.mul(1000000000);
...
contract Laboratory {
    ...
    ERC20ReflectionToken[] public contracts;
    ...
    constructor (address ELXRAddress, address _DAOTreasury) public payable {
        ELXR = IERC20(ELXRAddress);
    ...
    function deployNewERC20Token(string calldata name,string calldata symbol, uint256 taxFee, uint256 liquidityFee, uint256 devFee, uint256 burnFee,address devAddress, uint256 setAmount) public payable returns (address) {
        require(ELXR.transferFrom(msg.sender, DAOTreasury, 1 * 10**18));
        ERC20ReflectionToken t = new ERC20ReflectionToken(name,symbol,taxFee,liquidityFee,devFee,burnFee,devAddress,setAmount);
        contracts.push(t);
