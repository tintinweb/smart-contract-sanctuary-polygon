...
contract SavageStock is ISavageStock, Operator {
  ...
  uint256 private _stripeTokenDecimals;
  ...
  function getSaleTokenAmount(uint256 saleId, address tokenIn) public view returns (uint256) {
    ...
    if (tokenIn == _intermediateStablecoin) return price;
    else if (tokenIn == _stripeToken) return (price * 10 ** _stripeTokenDecimals) / 10 ** _stablecoinDecimals;
  ...
  constructor(
    ...
    _stablecoinDecimals = IERC20Decimals(intermediateStablecoin_).decimals();
    _stripeTokenDecimals = IERC20Decimals(stripeToken_).decimals();
    _minimumPrice = 10 ** _stablecoinDecimals;
  ...
  function addPaymentToken(address token) external onlyOwner override(ISavageStock) returns (bool) {
  ...
  ) external onlyOwnerOrOperator override(ISavageStock) returns (uint256[] memory tokenIds, uint256 saleId) {
  ...
  function finishAuction(uint256 saleId) external override(ISavageStock) returns (bool) {
    ...
    if (bidderExist) {
      ...
      if (sale_.tokenIn == _stripeToken) {
        amountOut = (amountOut * 10 ** _stripeTokenDecimals) / 10 ** _stablecoinDecimals;
        amountOut = _collateralPool.swap(this_, amountOut);
      } else {
        amountOut = _swap(_getOutPath(_savageToken), amountOut, this_, false);
      }
  ...
  function removePaymentToken(address token) external onlyOwner override(ISavageStock) returns (bool) {
  ...
  function saleNFT(
    ...
    uint256 startTimestamp,
    uint256 endTimestamp
  ) external onlyOwnerOrOperator override(ISavageStock) returns (uint256 saleId) {
  ...
  function stopSale(uint256 saleId) external override(ISavageStock) returns (bool) {
    ...
    if (sale_.bidder != zero) {
      ...
      if (sale_.tokenIn == _intermediateStablecoin || sale_.tokenIn == _stripeToken) {
        ...
        bool toMATIC = sale_.tokenIn == _wrappedToken && sale_.isNative;
        _swap(_getOutPath(sale_.tokenIn), amountOut, sale_.bidder, toMATIC);
      }
      sale_.bidder = zero;
    }
    uint256 count = sale_.tokenIds.length();
  ...
  function _bid(
    ...
    require(bidder == caller || caller == operator() || caller == owner(), "Invalid caller");
    require(saleId < _salesCount, "Sale is not exist");
    Sale storage sale_ = _sales[saleId];
    ...
    if (tokenIn != _intermediateStablecoin && tokenIn != _stripeToken) {
      stableAmount = _swap(_getInPath(tokenIn, _intermediateStablecoin), amountIn, address(this), false);
    } else if (tokenIn == _stripeToken) {
      stableAmount = (stableAmount * 10 ** _stablecoinDecimals) / 10 ** _stripeTokenDecimals;
    }
  ...
  function _buy(
    ...
    address caller = msg.sender;
    address this_ = address(this);
    require(buyer == caller || caller == operator() || caller == owner(), "Invalid caller");
    require(saleId < _salesCount, "Sale is not exist");
    Sale storage sale_ = _sales[saleId];
    require(sale_.active, "Sale not active");
    ...
    uint256 stableAmount = amountIn;
    {
      ...
      if (tokenIn == _savageToken) {
        requiredStableAmount = requiredStableAmount * (100 - _savageDiscountPercent) / 100;
      } else if (tokenIn == _stripeToken) {
        stableAmount = (stableAmount * 10 ** _stablecoinDecimals) / 10 ** _stripeTokenDecimals;
      }
      success = stableAmount >= requiredStableAmount;
    ...
    if (tokenIn == _stripeToken) {
      ...
      amountOut = _swap(_getInPath(tokenIn, _savageToken), amountOut, this_, false);
    }
  ...
  function _swap(address[] memory path, uint256 amount, address to, bool toMATIC) private returns (uint256 result) {
    ...
    if (toMATIC) {
      ...
      result = _router.swapExactTokensForTokens(
        amount,
        1,
        path,
        to,
