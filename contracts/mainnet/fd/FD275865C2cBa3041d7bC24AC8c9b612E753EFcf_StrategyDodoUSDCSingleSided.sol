...
contract StrategyDodoUSDCSingleSided is StrategyBase {
    ...
    )
    {
        ...
        IERC20(dodo).approve(usdc_dodo_lp_token, uint256(- 1));
        ...
        IERC20(want).approve(usdc_dodo_lp_token, uint256(- 1));
        ...
        IERC20(usdc).approve(usdc_dodo_lp_token, uint256(- 1));

        IERC20(usdc_dodo_lp_token).approve(dodo_approve, uint256(- 1));
        IERC20(usdc_dodo_lp_token).approve(rewards, uint256(- 1));
        IERC20(usdc_dodo_lp_token).approve(dodoSwap, uint256(- 1));
        IERC20(usdc_dodo_lp_token).approve(erc20mine, uint256(- 1));
        IERC20(usdc_dodo_lp_token).approve(usdc_dodo_lp_token, uint256(- 1));
        ...
        IERC20(dodo_dodo_usdt_lp).approve(usdc_dodo_lp_token, uint256(- 1));
    }
    ...
    function swapAssetToUsdc() public {
        // swap Dodo to USDC
    ...
    function stakeUsdcToDodo() public {

        // Adds in liquidity for DODO/USDC
        uint256 _usdc = IERC20(usdc).balanceOf(address(this));

        IERC20(usdc).safeApprove(dodoSwap, 0);
        IERC20(usdc).safeApprove(dodoSwap, _usdc);
    ...
    function stakeUsdcIntoMineRewards() public {
        ...
        // check for usdc LP
        uint256 _usdc = IERC20(usdc_dodo_lp_token).balanceOf(address(this));

        if (_usdc > 0) {
            IERC20(usdc_dodo_lp_token).safeApprove(rewards, 0);
            IERC20(usdc_dodo_lp_token).safeApprove(rewards, _usdc);
