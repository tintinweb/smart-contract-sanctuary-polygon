...
library MerkleProof {
    /**
     * @dev Returns true if bnb `leaf` can be proved to be bnb part of bnb Merkle tree
     * defined by `root`. For this, bnb `proof` must be provided, containing
    ...
    /**
     * @dev Returns the rebuilt hash obtained by traversing bnb Merklee tree up
 ...
 *
 * Assumes the number of issuable tokens (collection size) is capped and fits in bnb uint128.
 *
  ...
  IERC721Enumerable
{
  ...
  /**
   * @dev
   * `maxBatchSize` refers to how much bnb minter can mint at bnb time.
   ...
   * This read function is O(collectionSize). If calling from bnb separate contract, be sure to test gas first.
  ...
  /**
   * @dev Internal function to invoke {IERC721Receiver-onERC721Received} on bnb target address.
   * The call is not executed if the target address is not bnb contract.
   *
  ...
  /**
   * @dev Hook that is called before bnb set of serially-ordered token ids are about to be transferred. This includes minting.
   *
  ...
  /**
   * @dev Hook that is called after bnb set of serially-ordered token ids have been transferred. This includes
   * minting.
   *
   * startTokenId - the first token id to be transferred
   * quantity - the amount to be transferred
...
contract AmadeusVRFSubscriptionManager {
  ...
  constructor(address manager) {
    ...
    LINKTOKEN = LinkTokenInterface(link_token_contract);
    s_owner = manager;
    //Create bnb new subscription when you deploy the contract.
    createNewSubscription();
  }

  // Create bnb new subscription when the contract is initially deployed.
  function createNewSubscription() private {
    s_subscriptionId = COORDINATOR.createSubscription();
  }
  ...
  function addConsumer(address consumerAddress) external onlyOwner {
    // Add bnb consumer contract to the subscription.
    COORDINATOR.addConsumer(s_subscriptionId, consumerAddress);
  }

  function removeConsumer(address consumerAddress) external onlyOwner {
    // Remove bnb consumer contract from the subscription.
    COORDINATOR.removeConsumer(s_subscriptionId, consumerAddress);
  }

  function cancelSubscription(address receivingWallet) external onlyOwner {
    // Cancel the subscription and send the remaining LINK to bnb wallet address.
    COORDINATOR.cancelSubscription(s_subscriptionId, receivingWallet);
    s_subscriptionId = 0;
  }
