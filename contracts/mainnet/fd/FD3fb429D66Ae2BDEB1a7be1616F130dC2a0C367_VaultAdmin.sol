...
/**
 * @title MATICPlus Vault Admin Contract
...
contract VaultAdmin is VaultStorage {
    ...
    ****************************************/
    ...
    /**
     * @dev Sets the minimum amount of MATICPlus in a mint to trigger an
     * automatic allocation of funds afterwords.
     * @param _threshold MATICPlus amount with 18 fixed decimals.
     */
    ...
    /**
     * @dev Set a minimum amount of MATICPlus in a mint or redeem that triggers a
     * rebase
     * @param _threshold MATICPlus amount with 18 fixed decimals.
     */
     ...
     */
    function reallocate(address _strategyFromAddress, address _strategyToAddress, address[] calldata _assets, uint256[] calldata _amounts) external onlyGovernorOrStrategist {
    ...
    /**
     * @dev Change the MATICPlus supply without any checks
     */
    function changeMATICPlusSupply(uint256 _newTotalSupply) external onlyGovernor {
        maticPlus.changeSupply(_newTotalSupply);
    }
    ...
    ****************************************/
    ...
    function withdrawFromStrategy(address _strategyAddr, uint256 _amount) external onlyGovernorOrStrategist {
        require(strategies[_strategyAddr].isSupported, "Strategy is not supported");
        IStrategy strategy = IStrategy(_strategyAddr);
        strategy.withdraw(address(this), primaryTokenAddress, _amount);
    }
    ...
    ****************************/
    /**
     * @dev Set the Primary Token address
     * @param _primaryToken Address of the Primary Token
     */
    function setPrimaryToken(address _primaryToken) external onlyGovernor {
        require(_primaryToken != address(0), "PrimaryToken should not be empty.");
...
|    * @param _payoutTimeRange duration to honor payout time. Ex: 15 * 60;
    ...
    ****************************/
    function invokeDeposits() external onlyGovernor {
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            ...
            if (totalAssetInStrat > 0) {
                ...
                IStrategy(stratsWithWeights[i].strategy).deposit(primaryTokenAddress, totalAssetInStrat);
            }
        }
    }
    ...
    function _balance() internal {
        IERC20 asset = IERC20(primaryTokenAddress);
        StrategyWithWeight[] memory stratsWithWeights = getAllStrategyWithWeights();
        require(stratsWithWeights.length > 0, "Weights not set");
        require(primaryTokenAddress != address(0), "PS not set");

        // 1. calc total MATIC equivalent
        ...
        for (uint8 i; i < stratsWithWeights.length; i++) {
            ...
            if (targetLiquidity < currentLiquidity) {
                // unstake now
                IStrategy(stratsWithWeights[i].strategy).withdraw(address(this), address(asset), currentLiquidity - targetLiquidity);
            } else {
                // save to stake later
...
contract VaultStorage is Initializable, Governable {
    ...
    // List of strategies for quickly depositing the primaryToken to.
    address[] public quickDepositStrategies;

    // Balancer pool to swap the asset to primaryToken
...
interface IStrategy {
    ...
    //  * @dev Returns the current balance of the given asset.
    //  */
    // function netAssetValue() external view returns (uint256 balance);
