...
contract LicencseToken is owned {
    ...
    uint256 count = 0;
    uint256 level = 1; 
    uint256 licensePrice = 600;
    uint256 awardTotal = licensePrice/4;
    ...
    constructor(address _sendToken, address _masterAddress) {
        _usdc = _sendToken;
        USDC = IERC20(_usdc);
    ...
    // licensing logic
    function giveLicense(address _account) onlyOwner public {
    ...
    function licensePriceTransfer (address _from) public {
        uint256 licenseTransferPrice = licensePrice * 3 / 4;
        USDC.transferFrom(_from, _master, licenseTransferPrice * 10 ** 18);
        USDC.transferFrom(_from, address(this), awardTotal * 10 ** 18);
    }
    ...
    function invite (address _senderAddress, address _receiverAddress) public {
        datas.push(SenderData({
            dataId: i, 
    ...
    function _getReceiveAddress (address _receiveAddress) public {
        count = 0;
        for( uint256 j = 0 ; j < datas.length ; j ++ ) {
            if( datas[j].receiverAddress == _receiveAddress ) {
                sender[level] = datas[j].senderAddress;
                level = level + 1;
                count = count + 1;
            }
        }
    ...
    function makingTree (address _receiveAddress) public {
        _getReceiveAddress(_receiveAddress);
        while( count != 0 ) {
            _getReceiveAddress(sender[level-1]);
        }
    }

    function reward () public {
        uint256 sum = 0;
        for(uint256 awardId = 1; awardId < level ; awardId ++) {
            div = 50;
            if( awardId == 1){
                div = 10;
            } else if( awardId == 2){
            ...
            USDC.transfer(sender[awardId], (awardTotal/div) * 10 ** 18);
            sum = sum + (awardTotal/div);
        }
        USDC.transfer(_master, (awardTotal - sum) * 10 ** 18);
        level = 1;
    }

    function _mint(address _account) onlyOwner internal returns (uint256 tokenId) {
        // create new token
