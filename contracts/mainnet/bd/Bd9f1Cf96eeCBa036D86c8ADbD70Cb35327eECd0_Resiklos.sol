...
contract Resiklos is ERC721A, Ownable {
    ...
    uint256 public MAX_LEGENDARY_SUPPLY = 1000;
    uint256 public MAX_RARE_SUPPLY = 3000;
    uint256 public MAX_COMMON_SUPPLY = 6000;

    uint256 public MAX_COMMON_MINT = 0; 
    uint256 public MAX_RARE_MINT = 0; 
    uint256 public MAX_LEGENDARY_MINT = 0; 
    uint256 public MAX_WALLET_MINT = 5; 

    uint256 public LEGENDARY_PRICE = 30 ether;
    uint256 public RARE_PRICE = 20 ether;
    uint256 public COMMON_PRICE = 10 ether;
    ...
    constructor() ERC721A("Resiklos", "RSK") { }
    ...
    function rskMint(bytes32[] memory _merkleProof, string calldata rarity, uint256 _quantity)  external payable correctState(SaleState.RSKWL) {
        ...
        if (keccak256(abi.encodePacked(rarity)) == keccak256(abi.encodePacked("legendary"))) {
            ...
            require((getRareMintedSupply() + _quantity) <= MAX_RARE_SUPPLY, "Beyond max rare supply.");
            if (MAX_RARE_MINT > 0) {
            ...
            require((getCommonMintedSupply() + _quantity) <= MAX_COMMON_SUPPLY, "Beyond max common supply.");
    ...
    function devMint(uint256 legendary, uint256 rare, uint256 common) external onlyOwner {
        ...
        require((getLegendaryMintedSupply() + legendary) <= MAX_LEGENDARY_SUPPLY, "Beyond max legendary supply.");
        require((getRareMintedSupply() + rare) <= MAX_RARE_SUPPLY, "Beyond max rare supply.");
        require((getCommonMintedSupply() + common) <= MAX_COMMON_SUPPLY, "Beyond max common supply.");
    ...
    function getLegendaryMintedSupply() internal view virtual returns (uint256) {
        return legendaryMintedSupply;
    }

    function getRareMintedSupply() internal view virtual returns (uint256) {
        return rareMintedSupply;
    }

    function getCommonMintedSupply() internal view virtual returns (uint256) {
    ...
    function withdrawMoney() external onlyOwner {
        ...
        payable(0x0CbBFbE47945289A559E45c67C2708764cC973a9).transfer(balance);
    }
}