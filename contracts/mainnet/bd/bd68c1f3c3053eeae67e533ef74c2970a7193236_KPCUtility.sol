...
// File: contracts/KPCUtility.sol


pragma solidity ^0.8.2;


contract KPCUtility is ERC20, AccessControl, ERC721Holder {
    bytes32 public constant CONTRACT_ADMIN_ROLE = keccak256("CONTRACT_ADMIN_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");

    address public collectionAddressKaratePandaClubNFT;
    address public collectionAddressNFTv2;

    uint256 public numberOfBlocksPerRewardUnit;

    uint256 public coinAmountPerRewardUnitKaratePandaClubNFT;
    uint256 public coinAmountPerRewardUnitNFTv2;

    uint256 public welcomeBonusAmountKaratePandaClubNFT;
    uint256 public welcomeBonusAmountNFTv2;

    uint256 public amountOfStakersKaratePandaClubNFT;
    uint256 public amountOfStakersNFTv2;

    uint256 public tokensStakedKaratePandaClubNFT;
    ...
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogKaratePandaClubNFT;

        /// owner => tokenId => StakeInfo
    mapping (address => mapping(uint256 => StakeInfo)) public stakeLogNFTv2;

    /// owner => #NFTsStaked
    mapping (address => uint256) public tokensStakedByUserKaratePandaClubNFT;
    ...
    mapping (uint256 => bool) public welcomeBonusCollectedKaratePandaClubNFT;

        /// tokenId => true/false (true if welcome bonus has been collected for a specific tokenId)
    mapping (uint256 => bool) public welcomeBonusCollectedNFTv2;

    /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserKaratePandaClubNFT;

        /// owner => list of all tokenIds that the user has staked
    mapping (address => uint256[]) public stakePortfolioByUserNFTv2;

    /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioKaratePandaClubNFT;

        /// tokenId => indexInStakePortfolio
    mapping(uint256 => uint256) public indexOfTokenIdInStakePortfolioNFTv2;

    event RewardsHarvestedKaratePandaClubNFT (address owner, uint256 amount);
    event NFTStakedKaratePandaClubNFT (address owner, uint256 tokenId);
    event NFTUnstakedKaratePandaClubNFT (address owner, uint256 tokenId);
    ...
    constructor(address owner, address _collectionAddressKaratePandaClubNFT, address _collectionAddressNFTv2) ERC20("KaratePandaClubUtility", "KPCU") AccessControl(){
        _mint(owner, 100000 * 10 ** 18);
        _setupRole(DEFAULT_ADMIN_ROLE, owner);
        _setupRole(CONTRACT_ADMIN_ROLE, owner);
        _setupRole(BURNER_ROLE, owner);
        _setupRole(MINTER_ROLE, owner);
        collectionAddressKaratePandaClubNFT = _collectionAddressKaratePandaClubNFT;
        collectionAddressNFTv2 = _collectionAddressNFTv2;
        contractCreationBlock = block.number;
        coinAmountPerRewardUnitKaratePandaClubNFT = 15 * 10 ** 18; // 5 ERC20 coins per rewardUnit, may be changed later on
        ...
        welcomeBonusAmountKaratePandaClubNFT = 300 * 10 ** 18; // 300 tokens welcome bonus, only paid once per tokenId
        welcomeBonusAmountNFTv2 = 600 * 10 ** 18; // 600 tokens welcome bonus, only paid once per tokenId
    }

    function stakedNFTSByUserKaratePandaClubNFT(address owner) external view returns (uint256[] memory){
        return stakePortfolioByUserKaratePandaClubNFT[owner];
    }
    ...
    function pendingRewardsKaratePandaClubNFT(address owner, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLogKaratePandaClubNFT[owner][tokenId];
        ...
        return rewardAmount * coinAmountPerRewardUnitKaratePandaClubNFT;
    ...
    function stakeKaratePandaClubNFT(uint256 tokenId) public {
        IERC721(collectionAddressKaratePandaClubNFT).safeTransferFrom(_msgSender(), address(this), tokenId);
        require(IERC721(collectionAddressKaratePandaClubNFT).ownerOf(tokenId) == address(this),
            "KPCUtility: Error while transferring token");
        StakeInfo storage info = stakeLogKaratePandaClubNFT[_msgSender()][tokenId];
        info.stakedAtBlock = block.number;
        info.lastHarvestBlock = block.number;
        info.currentlyStaked = true;
        if(tokensStakedByUserKaratePandaClubNFT[_msgSender()] == 0){
            amountOfStakersKaratePandaClubNFT += 1;
        }
        tokensStakedByUserKaratePandaClubNFT[_msgSender()] += 1;
        tokensStakedKaratePandaClubNFT += 1;
        stakePortfolioByUserKaratePandaClubNFT[_msgSender()].push(tokenId);
        uint256 indexOfNewElement = stakePortfolioByUserKaratePandaClubNFT[_msgSender()].length - 1;
        indexOfTokenIdInStakePortfolioKaratePandaClubNFT[tokenId] = indexOfNewElement;
        if(!welcomeBonusCollectedKaratePandaClubNFT[tokenId]) {
            _mint(_msgSender(), welcomeBonusAmountKaratePandaClubNFT);
            welcomeBonusCollectedKaratePandaClubNFT[tokenId] = true;
        }

        emit NFTStakedKaratePandaClubNFT(_msgSender(), tokenId);
    }
    ...
    function stakeBatchKaratePandaClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            stakeKaratePandaClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeBatchKaratePandaClubNFT(uint256[] memory tokenIds) external {
        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            unstakeKaratePandaClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function unstakeKaratePandaClubNFT(uint256 tokenId) public {
        if(pendingRewardsKaratePandaClubNFT(_msgSender(), tokenId) > 0){
            harvestKaratePandaClubNFT(tokenId);
        }
        StakeInfo storage info = stakeLogKaratePandaClubNFT[_msgSender()][tokenId];
        info.currentlyStaked = true;
        IERC721(collectionAddressKaratePandaClubNFT).safeTransferFrom(address(this), _msgSender(), tokenId);
        require(IERC721(collectionAddressKaratePandaClubNFT).ownerOf(tokenId) == _msgSender(),
            "KPCUtility: Error while transferring token");
        if(tokensStakedByUserKaratePandaClubNFT[_msgSender()] == 1){
            amountOfStakersKaratePandaClubNFT -= 1;
        }
        tokensStakedByUserKaratePandaClubNFT[_msgSender()] -= 1;
        tokensStakedKaratePandaClubNFT -= 1;
        stakePortfolioByUserKaratePandaClubNFT[_msgSender()][indexOfTokenIdInStakePortfolioKaratePandaClubNFT[tokenId]] = 0;
        emit NFTUnstakedKaratePandaClubNFT(_msgSender(), tokenId);
    }
    ...
    function harvestKaratePandaClubNFT(uint256 tokenId) public {
        StakeInfo storage info = stakeLogKaratePandaClubNFT[_msgSender()][tokenId];
        uint256 rewardAmountInERC20Tokens = pendingRewardsKaratePandaClubNFT(_msgSender(), tokenId);
        if(rewardAmountInERC20Tokens > 0) {
            info.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInERC20Tokens);
            emit RewardsHarvestedKaratePandaClubNFT(_msgSender(), rewardAmountInERC20Tokens);
        }
    }
    ...
    function harvestBatchKaratePandaClubNFT(address user) external {
        uint256[] memory tokenIds = stakePortfolioByUserKaratePandaClubNFT[user];

        for(uint currentId = 0; currentId < tokenIds.length; currentId++) {
            if(tokenIds[currentId] == 0) {
                continue;
            }
            harvestKaratePandaClubNFT(tokenIds[currentId]);
        }
    }
    ...
    function setCoinAmountPerRewardUnitKaratePandaClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        coinAmountPerRewardUnitKaratePandaClubNFT = coinAmount;
    }
    ...
    function setWelcomeBonusAmountKaratePandaClubNFT(uint256 coinAmount) external onlyRole(CONTRACT_ADMIN_ROLE){
        welcomeBonusAmountKaratePandaClubNFT = coinAmount;
    }
    ...
    function setCollectionAddressKaratePandaClubNFT(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "VIBE: update to zero address not possible");
        collectionAddressKaratePandaClubNFT = newAddress;
    }
