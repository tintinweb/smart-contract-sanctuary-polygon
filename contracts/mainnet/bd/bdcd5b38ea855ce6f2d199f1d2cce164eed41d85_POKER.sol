...
contract POKER is ERC20Burnable, Ownable {
    using SafeMath for uint256;
    ...
    address public _feeAccount = 0xa07dAe8B97e81446608876EAFA76c77e10196a7C;
    ...
    constructor(
        ...
        address owner
    ) ERC20('Poker Dogs', 'POKER') {
        require(owner != address(0), 'incorrect owner address');
        if (_msgSender() != owner) {
    ...
    function updateTaxInfo(address feeAccount, uint256 feePercent) external onlyOwner{
        require(feeAccount != address(0),'fee account from the zero address');
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal override {
        ...
        if (isExisted(sender) || _feePercent == 0) {
            ...
            uint256 _fee = amount.mul(_feePercent).div(100);
