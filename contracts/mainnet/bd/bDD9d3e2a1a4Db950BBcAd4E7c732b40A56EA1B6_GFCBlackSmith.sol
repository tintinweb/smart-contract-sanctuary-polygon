...
contract GFCBlackSmith is OwnableUpgradeable, WhitelistUpgradeable, VRFConsumerBaseV2 {
    ...
    event WeaponReversed(address indexed account, uint256 tokenIds, uint256 indexed transactionId);
    ...
    function smith(uint256 transactionId) public {
        ...
        if(fInfo.forgeType == 1) {
            ...
            uint256[] memory tokenIds = mintPrevTier(fInfo.category, fInfo.tier, transactionId);
        }else {
            revert();
        }
    ...
    function mintPrevTier(uint256 category, uint256 tier, uint256 transactionId) internal returns (uint256[] memory) {
        tier--;
        uint256[] memory randNums = expand(getResult(msg.sender), weaponForgeRate[tier]);
        uint256[] memory weaponIds = new uint256[](randNums.length);
        for (uint8 i; i < randNums.length; i ++) {
            uint256 weaponId;
            if(category == 1){
                uint256 weaponType = randNums[i] % meleeWeaponCount[tier];
                weaponId = category*10000 + tier*1000 + weaponType; 
            } else if (category == 2) {
                uint256 weaponType = randNums[i] % rangedWeaponCount[tier];
                weaponId = category*10000 + tier*1000 + weaponType; 
            } else if (category == 3) {
                uint256 weaponType = randNums[i] % meleeP2eWeaponCount[tier];
                weaponId = category*10000 + tier*1000 + weaponType;
            } else if (category == 4) {
                uint256 weaponType = randNums[i] % rangedP2eWeaponCount[tier];
                weaponId = category*10000 + tier*1000 + weaponType;
            }
            WeaponMock(genesisWeaponAddress).mintWeapon(msg.sender, weaponId, 1);
            emit WeaponReversed(msg.sender, weaponId, transactionId);
            weaponIds[i] = weaponId;
        }
        return weaponIds;
    }
