...
contract AncientFarmer is ERC721Enumerable, EIP712, Ownable {
    ...
    //VOUCHER STORAGE
    mapping(bytes => bool) public voucherUsed;
    ...
    constructor() ERC721 ("AncientFarmer", "AFR") EIP712(SIGNING_DOMAIN_NAME, SIGNING_DOMAIN_VERSION){
        isPublicMint = true;
        baseURI = "https://www.api.ancientsociety.io/reveal/metadata/farmer";
        foundersWallet = 0x02367e1ed0294AF91E459463b495C8F8F855fBb8;
        signer = 0x72f42ECa685185Fcfe0f0644E5ddf8d799659AF6;

        managers[msg.sender] = true;
    }

    modifier onlyMinter(address player, uint256 _numberOfTokens){
        require(isPublicMint, "Mint not open");
        require(_numberOfTokens > 0, "At least 1 nft");
        require(_numberOfTokens <= mintAllowance[player], "Not allowed so many nfts");
        require(nftMinted[player] + _numberOfTokens <= mintAllowance[player], "Minted already");
        _;
    }
    ...
    function increaseAllowance(uint256 id, address spender, uint256 tokens, uint256 blockNumber, bytes memory signature) external {
        require(!voucherUsed[signature], "voucher already used");
        require(check(id, spender, tokens, blockNumber, signature) == signer, "Voucher not valid");
        require(tokens > mintAllowance[spender], "new allowance must be greater than previous one");
        
        mintAllowance[spender] = tokens;
        voucherUsed[signature] = true;
    }

    function increaseAllowanceManager(address spender, uint256 tokens) external onlyManager(msg.sender){
        mintAllowance[spender] = tokens;
    }
    ...
    function setPublicMint(bool isPublicMint_) external onlyManager(msg.sender){
    ...
    //FOUNDERS WALLET
    function setFoundersWallet(address newFoundersWallet) external onlyManager(msg.sender){
        foundersWallet = newFoundersWallet;
    }


    //MINTING FUNCTIONS
    function mint(address player, uint256 _numberOfTokens) external onlyMinter(player, _numberOfTokens){
        for (uint256 i = 0; i < _numberOfTokens; i++) {
            emit Mint(player, totalSupply());
