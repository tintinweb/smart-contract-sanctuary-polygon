...
import "./DungeonCrawlingItemContracts.sol";

contract DungeonCrawlingItem is Initializable, DungeonCrawlingItemContracts {

    function initialize() external initializer {
        DungeonCrawlingItemContracts.__DungeonCrawlingItemContracts_init();
    }

    function mint(address _to, uint256 _id, uint256 _amount) external requiresRole(MINTER_ROLE) {
        _mint(_to, _id, _amount, "");
    }

    function mintBatch(address _to, uint256[] calldata _ids, uint256[] calldata _amounts) external requiresRole(MINTER_ROLE) {
        _mintBatch(_to, _ids, _amounts, "");
    }

    function burn(address _from, uint256 _id, uint256 _amount) external requiresRole(BURNER_ROLE) {
        _burn(_from, _id, _amount);
    }

    function burnBatch(address _from, uint256[] calldata _ids, uint256[] calldata _amount) external requiresRole(BURNER_ROLE) {
        _burnBatch(_from, _ids, _amount);
    }

    function noApprovalSafeTransferFrom(address _from, address _to, uint256 _id, uint256 _amount) external requiresRole(NO_APPROVAL_TRANSFER_ROLE) {
        _safeTransferFrom(_from, _to, _id, _amount, "");
    }

    function noApprovalSafeBatchTransferFrom(address _from, address _to, uint256[] calldata _ids, uint256[] calldata _amounts) external requiresRole(NO_APPROVAL_TRANSFER_ROLE) {
        _safeBatchTransferFrom(_from, _to, _ids, _amounts, "");
    }

    function airdrop(uint256 _id, uint256[] calldata _amounts, address[] calldata _recipients) external requiresRole(MINTER_ROLE) {
        require(_id > 0 && _amounts.length == _recipients.length && _recipients.length > 0, "Bad inputs given");

        for(uint256 i = 0; i < _recipients.length; i++) {
            require(_amounts[i] > 0, "Bad amount");
            _mint(_recipients[i], _id, _amounts[i], "");
        }
    }
...
import "./DungeonCrawlingItemState.sol";

abstract contract DungeonCrawlingItemContracts is Initializable, DungeonCrawlingItemState {

    function __DungeonCrawlingItemContracts_init() internal initializer {
        DungeonCrawlingItemState.__DungeonCrawlingItemState_init();
    }
}
...
import "./IDungeonCrawlingItem.sol";
import "../../shared/UtilitiesUpgradeable.sol";
import "../../shared/ERC1155OnChainUpgradeable.sol";

abstract contract DungeonCrawlingItemState is IDungeonCrawlingItem, ERC1155OnChainUpgradeable {

    event OwnershipTransferred(address indexed previousOwner, address indexed newOwner);

    bytes32 constant MINTER_ROLE = keccak256("MINTER");
    bytes32 constant BURNER_ROLE = keccak256("BURNER");
    bytes32 constant NO_APPROVAL_TRANSFER_ROLE = keccak256("NO_APPROVAL_TRANSFER");

    function __DungeonCrawlingItemState_init() internal initializer {
...
import "./BBase64.sol";
import "./IERC1155OnChainUpgradeable.sol";

abstract contract ERC1155OnChainUpgradeable is Initializable, ERC1155Upgradeable, IERC1155OnChainUpgradeable, UtilitiesUpgradeable {
    ...
    function __ERC1155OnChainBase_init() internal onlyInitializing {
    ...
    function _beforeTokenTransfer(
        ...
        require(!paused(), "ERC1155OnChainUpgradeable: Cannot transfer while paused");
    }

    function supportsInterface(bytes4 interfaceId) public view virtual override(IERC165Upgradeable, AccessControlEnumerableUpgradeable, ERC1155Upgradeable) returns (bool) {
