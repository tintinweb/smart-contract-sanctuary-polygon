...
/**
 * @title AutoCompound Curve pools
 * @notice strategy for auto-compounding on pools using curve
 * @author YieldWolf
 */
contract ACCurve is AutoCompoundStrategy {
    using SafeERC20 for IERC20;

    ICurveLiquidityPool public liquidityPool;
    IERC20 public liquidityReceipt;
    IERC20 public liquidityToken;
    uint256 public liquidityIndex;

    event SetLiquidityInfo(address liquidityToken, uint256 liquidityIndex);
    ...
    function _totalStaked() internal pure override returns (uint256 amount) {
        // noop
    }

    function _farmHarvest() internal override {
        ICurveGauge(address(stakeToken)).claim_rewards();
    }
    ...
    function earn(address _bountyHunter) external override onlyOwner returns (uint256 bountyReward) {
        ...
        if (liquidityToken != earnToken) {
            _safeSwap(earnAmount, swapPath[address(earnToken)][address(liquidityToken)], address(this), false);
        }

        uint256 liquidityInAmount = liquidityToken.balanceOf(address(this));
        liquidityToken.safeIncreaseAllowance(address(liquidityPool), liquidityInAmount);
        uint256[4] memory amounts;
        amounts[liquidityIndex] = liquidityInAmount;
        liquidityPool.add_liquidity(amounts, 0);

        uint256 receiptAmount = liquidityReceipt.balanceOf(address(this));
        liquidityReceipt.safeIncreaseAllowance(address(stakeToken), receiptAmount);
        ICurveGauge(address(stakeToken)).deposit(receiptAmount);

        return bountyReward;
    }

    function setLiquidityInfo(
        ICurveLiquidityPool _liquidityPool,
        IERC20 _liquidityToken,
        uint256 _liquidityIndex,
        address[] calldata _earnToLiquidityTokenPath
        ...
        liquidityReceipt = IERC20(ICurveGauge(address(stakeToken)).lp_token());
        if (address(earnToken) != address(liquidityToken)) {
            require(_earnToLiquidityTokenPath[0] == address(earnToken), 'setLiquidityInfo: INVALID_PATH');
            require(
                _earnToLiquidityTokenPath[_earnToLiquidityTokenPath.length - 1] == address(liquidityToken),
                'setLiquidityInfo: INVALID_PATH'
            );
            swapPath[address(earnToken)][address(liquidityToken)] = _earnToLiquidityTokenPath;
        }
        emit SetLiquidityInfo(address(liquidityToken), liquidityIndex);
    }
}
