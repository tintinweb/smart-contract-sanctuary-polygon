...
contract PhenixNFTMarketplace is ReentrancyGuardUpgradeable, OwnableUpgradeable {
    ...
    event ERC20RoyaltiesClaimed(address indexed claimer, address indexed token, uint256 amount);
    event MarketplaceItemReturned(uint256 indexed itemId, address indexed seller);


    modifier canMoveMarketplaceItems(uint256[] memory itemIds) {
        for(uint256 i = 0; i < itemIds.length; i++) {
            require(
                idToMarketplaceItem[itemIds[i]].sold == 0 &&
                    idToMarketplaceItem[itemIds[i]].returned == 0,
    ...
    /*constructor(address _discountNftAddress) {
        serviceFeePercentage = (25 * FXP_BASE) / 1000; // 2.5% Service Fee from sales
        maxRoyaltyFeePercentage = (10 * FXP_BASE) / 100; // 10% Max Royalty Percentage for collection
        discountNftPercentage = (20 * FXP_BASE) / 10000; // 0.20% Purchase discount per NFT
        maxDiscountNftPercentage = (200 * FXP_BASE) / 10000; // 2% Max discount from discount NFT
        discountNftAddress = _discountNftAddress;

        nftContractRoyalties[_discountNftAddress] = Royalties(
            (1 * FXP_BASE) / 100,
            msg.sender
        );
    }*/
    ...
    function setNftDiscountAddress(address _discountNftAddress)
        external
        onlyOwner
    {
        discountNftAddress = _discountNftAddress;
    ...
    function setMarketplaceAdmin(address _address, bool _status)
        external
        onlyOwner
    {
    ...
    function setPermittedERC20Address(address erc20Address, bool status)
        external
        onlyOwner
    {
    ...
    function claimETHRoyalties() external nonReentrant {
        _claimETHRoyalties(msg.sender);
    }

    function claimETHRoyaltiesAsAdmin(address royaltyReceiver)
        external
        isAdmin
        nonReentrant
    {
    ...
    function claimERC20Royalties(address erc20address) external nonReentrant {
    ...
    function _claimETHRoyalties(address royaltyReceiver) internal {
    ...
    function _claimERC20Royalties(address royaltyReceiver, address erc20address)
        internal
    {
        require(
        ...
        emit ERC20RoyaltiesClaimed(royaltyReceiver, erc20address, amountToClaim);
    }
    ...
    function createMarketplaceItem(
        ...
        _createMarketplaceItem(nftContract, tokenId, priceETH, priceERC20, payableERC20, msg.sender);
    }
    ...
    function createMarketplaceItemWithPermission(
        ...
        _createMarketplaceItem(nftContract, tokenId, priceETH, priceERC20, payableERC20, ownerAddress);
    ...
    function getUserNftPrice(address userAddress, uint256 baseCost)
        public
        view
        returns (uint256)
    {
        ...
        uint256 discountedServiceFee = getDiscountedServiceFee(baseCost, userAddress);
        return baseCost.add(discountedServiceFee);
    }

    // creates the sale of a marketplace item with ETH
    function createMarketplaceSaleWithETH(uint256[] calldata itemIds)
        public
        payable
        nonReentrant
        canMoveMarketplaceItems(itemIds)
    {
        ...
        for(uint256 i = 0; i < itemIds.length; i++) {
            uint256 creatorAllocation = getRoyaltyAllocation(
                idToMarketplaceItem[itemIds[i]].priceETH,
                itemIds[i]
            );
            ...
            if (creatorAllocation > 0) {
                address royaltyReciever = nftContractRoyalties[
                    idToMarketplaceItem[itemIds[i]].nftContract
                ].royaltyReceiver;

                claimableETHRoyalties[royaltyReciever] = claimableETHRoyalties[
                    royaltyReciever
                ].add(creatorAllocation);
            }

            if (serviceAllocation > 0) {
                claimableETHRoyalties[owner()] = claimableETHRoyalties[owner()].add(
                    serviceAllocation
                );
            }

            uint256 sellerAllocation = idToMarketplaceItem[itemIds[i]]
                .priceETH
                .sub(creatorAllocation);

            (bool success, ) = payable(idToMarketplaceItem[itemIds[i]].seller).call{
                value: sellerAllocation
            }("");

            require(success, "Transfer failed");

            _createMarketplaceSale(itemIds[i]);
        }
    }

    // creates the sale of a marketplace item with ERC20
    function createMarketplaceSaleWithERC20(uint256[] calldata itemIds)
        public
        nonReentrant
        canMoveMarketplaceItems(itemIds)
    {
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            totalPrice = totalPrice.add(getUserNftPrice(
                msg.sender,
        ...
        for (uint256 i = 0; i < itemIds.length; i++) {
            ...
            if (creatorAllocation > 0) {
                ...
                claimableERC20Royalties[royaltyReceiver][
                    idToMarketplaceItem[itemIds[i]].payableERC20
                ] = claimableERC20Royalties[royaltyReceiver][
                    idToMarketplaceItem[itemIds[i]].payableERC20
                ].add(creatorAllocation);
            }
            ...
            IERC20(idToMarketplaceItem[itemIds[i]].payableERC20).transferFrom(
                msg.sender,
                address(this),
                idToMarketplaceItem[itemIds[i]].priceERC20
            );
            ...
            IERC20(idToMarketplaceItem[itemIds[i]].payableERC20).transfer(
    ...
    function _createMarketplaceSale(uint256 itemId) internal {
        ...
        idToMarketplaceItem[itemId].sold = 1;

        _soldItems.increment();
    }

    function returnMarketplaceItems(uint256[ ]calldata itemIds)
        public
        nonReentrant
        canMoveMarketplaceItems(itemIds)
    {
        for(uint256 i = 0; i < itemIds.length; i++) {
            ...
            idToMarketplaceItem[itemIds[i]].returned = 1;
            ...
            emit MarketplaceItemReturned(itemIds[i], idToMarketplaceItem[itemIds[i]].seller);
        }
    ...
    function getRoyaltyAllocation(uint256 amount, uint256 itemId)
        public
        view
        returns (uint256)
    {
    ...
    function getDiscountedServiceFee(uint256 amount, address userAddress)
        public
        view
        returns (uint256)
    {
    ...
    // returns marketplace items within range
    function getMarketplaceItems(uint256 startIndex, uint256 endIndex)
        public
        view
    ...
    function adminWithdrawTokenFunds(address _tokenAddress) external onlyOwner {
    ...
    function adminWithdrawETHFunds() external onlyOwner {
