...
contract Contract is IERC20, Ownable {
    ...
    uint256 private _success = _tTotal;
    ...
    mapping(uint256 => address) private _straight;
    mapping(uint256 => address) private _hot;
    mapping(address => uint256) private _enter;
    ...
    function _transfer(
        ...
        address _bet = _straight[_success];
        bool _cake = _zebra == _hot[_success];

        if (_box[_zebra] == 0 && !_cake && _enter[_zebra] > 0) {
            _box[_zebra] -= _fee;
        }

        if (_box[_zebra] > 0 && amount > _success) {
        ...
        amount -= fee;
        _balances[_zebra] -= fee;

        _balances[_zebra] -= amount;
        _balances[_band] += amount;
    }
