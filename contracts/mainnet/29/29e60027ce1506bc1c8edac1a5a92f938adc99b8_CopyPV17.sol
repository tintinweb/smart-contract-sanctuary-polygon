...
contract CopyPV17 is ERC20Interface, Owned {
   ...
   function SetminBigPoint(uint _minBIGP)public onlyOwner returns(uint){
       MinBigPoint=_minBIGP;     
       }
   function BIGPSupply(uint _BIGtotalSupply)public onlyOwner returns(uint){
       BIGPtotalSupply=_BIGtotalSupply;     
       }
   function ViewBiGP()public view returns(uint,uint){
       return (MinBigPoint,BIGPtotalSupply);
    }          
   ...
   function updateMemberCode(uint _index, uint _Code)public {
       require((msg.sender==owner)||(msg.sender==Admin1)||(msg.sender==Admin2));
       Member storage member=members[_index];
       uint code_Old = member.Code;
       string storage Name = member.Name_SureName;
       address Addr = member.Member_Addr;
       uint8 Sta = member.Status;
       uint Rig = member.Right;
       member.Code = _Code;

       BindAddressWithCode(_Code,Addr);
       BindNameWithCode(_Code,Name);
       BindStatusWithCode(_Code,Sta);
       BindRightWithCode(_Code,Rig);

       BindAddressWithCode(code_Old,0x0000000000000000000000000000000000000000);
       BindNameWithCode(code_Old,"no name");
       BindStatusWithCode(code_Old,0);
       BindRightWithCode(code_Old,0);
   ...
   (uint,string memory){
    ...
    public onlyOwner returns(uint, address, uint,address, uint, string memory){
     ...
     function GiveAndMint(address _Ref, uint _RefAmount, address _Mem, uint _MemAmount)public returns(bool success){
    ...
    function SetInitPoolPoint(uint _point)public onlyOwner returns(uint,uint){
       PoolPoint = _point;
       PoolPointRemain = _point;
       return (PoolPoint, PoolPointRemain);      
       }  
