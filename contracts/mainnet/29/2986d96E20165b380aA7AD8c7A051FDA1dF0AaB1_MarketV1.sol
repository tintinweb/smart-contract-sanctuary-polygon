...
contract MarketV1 is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable, OwnableUpgradeable {
    ...
    modifier onlyCollection(address nft) {
        require(collections[nft] == true, "Collection is not allowed");
        _;
    }
    ...
    function addCollection(
        ...
        collections[nft] = status;
    }

    function initVariables(address custodial, address _newEmotion) external onlyOwner nonReentrant {
        _treasury = custodial;
        _emotion = _newEmotion;
    }
    ...
    /// @param nftContract - address of the collection contract
    ...
    function listNFT(
        ...
        onlyOwner
        onlyCollection(msg.sender)
        onlyStudent(userId, tokenId)
        onlyAllowedToken(IERC20(sellingToken))
    { 
        require(price > 0, "Price must be not zero");
        uint256 creator = _creators[tokenId];
        uint256 listingId = nextListingId.current();
        listingIdToItem[listingId] = MarketItem(
            listingId,
            tokenId,
    ...
    function buyItem(uint256 listingId) external nonReentrant whenNotPaused {
        ...
        IERC20 workingToken = IERC20(currentListing.sellingToken);
        IERC721 workingNft = IERC721(currentListing.nftContract);
        uint256 creator = _creators[currentListing.tokenId];
        if (currentListing.sellingToken == _emotion){
            workingToken.feeLessTransfer(msg.sender, _treasury, currentListing.price);
            IUA(_treasury).splitRoyalty(currentListing.sellingToken, currentListing.seller, creator, transferAmount, feeAmount);
        } else {
            workingToken.transferFrom(msg.sender, _treasury, currentListing.price);
            IUA(_treasury).splitRoyalty(currentListing.sellingToken, currentListing.seller, creator, transferAmount, feeAmount);
        }
        ...
        emit itemSold(
            ...
            creator,
            currentListing.price,
            currentListing.sellingToken
        );
    }

    /// @dev listingId is being pulled from subgraph
    function buyItemInternal(uint256 listingId, uint256 buyer) external onlyOwner nonReentrant {
        require(
            listingIdToItem[listingId].seller != 0,
            "Listing does not exist"
        );
        ...
        uint256 creator = _creators[currentListing.tokenId];
        IUA(_treasury).transferSale(currentListing.sellingToken, buyer, currentListing.seller, creator, transferAmount, feeAmount);
    ...
    /// @param _price - new price of selected item
...
interface IUA {
    ...
    function transferSale(address token, uint256 from, uint256 to, uint256 creator, uint256 amount, uint256 fee) external;

    function pay(address nft, uint256 userId, uint256 amount) external;

    function splitRoyalty(address token, uint256 userId, uint256 creator, uint256 amount, uint256 fee) external;
}
