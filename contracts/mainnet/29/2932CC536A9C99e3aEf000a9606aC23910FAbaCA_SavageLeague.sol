...
contract SavageLeague is ERC721A, Ownable {

    uint256 price = 120 ether;

    uint256 public constant MAX_SUPPLY = 10000;

    address feeAddress = address(0x14bb28857da0EfBABF1EcC34ED64d6F236c1c5DC);

    constructor() ERC721A("Savage League", "SAVAGE"){}

    function mint(address to, uint256 quantity) public payable {
        _verifyPurchase(quantity);
        _mint(to, quantity);
        uint256 weiAmount = msg.value;

        uint256 feeAmount = weiAmount * 11 / 100;
        uint256 transferAmount = weiAmount - feeAmount;
    ...
    function burn(uint256 tokenId) public{
        require(isApprovedOrOwner(msg.sender, tokenId), "ERC721A: caller is not token owner nor approved");
        _burn(tokenId);
    }
    ...
    function _verifyPurchase(uint256 quantity) private view {
        require(
