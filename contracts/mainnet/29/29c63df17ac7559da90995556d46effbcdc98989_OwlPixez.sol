...
abstract contract ERC721Tradable is ERC721, ContextMixin, NativeMetaTransaction, Ownable {
    ...
    function random() internal returns (uint) {
        uint randomnumber = uint(keccak256(abi.encodePacked(block.timestamp,
                block.difficulty, nonce))) % MaxToken;
        nonce++;
        return randomnumber;
    }
    ...
    function mint(address _to) public onlyOwner {
        uint256 currentTokenId = random();
        if(currentTokenId == 0 ){
            currentTokenId = 1;
        }
        require(!_exists(currentTokenId), "id not available");
        _safeMint(_to, currentTokenId);
    }

    function mintTo(address _to) public onlyPresaleWhitelisted {
        uint256 currentTokenId = random();
        if(currentTokenId == 0 ){
            currentTokenId = 1;
        }
    ...
    function AddUser (address addrs) public onlyOwner {
        presaleWhitelist[addrs] = true;
...
contract OwlPixez is ERC721Tradable {


    constructor(address _proxyRegistryAddress)
        ERC721Tradable("OwlPixez", "OWLPx", _proxyRegistryAddress)
    {}
