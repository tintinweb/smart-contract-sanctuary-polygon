...
/// @title The non-presale factory of recipe
/// @author Radikal Riders
/// @notice This contract allows to buy packs of recipe
/// @dev This contract channels all the functional/business rules needed to mint recipe
contract RecipesFactory is VRFConsumerBaseV2, Ownable  {
  ...
  // TipPower value of common recipes
  uint16 commonTP = 50;
  ...
  event Minted(address indexed user, uint16[] tipPower);
  event Referred(address indexed user, string referral, uint token, uint matic);
  ...
  /// @param mintingPack Pack of nfts to mint. 0: 1 recipe | 1: 5 recipe | 2: 10 recipe | 3: 20 recipe
  function purchase(uint8 mintingPack, string memory referral) external payable {
    // pay 25 $ 
      // require 15 % in Matic
    ...
    require(userToMintingState[msg.sender] == MintingState.TO_OPEN || userToMintingState[msg.sender] == MintingState.COMPLETED, "Collection Recipes: already opened");
    ...
    uint _DevMaticPart = 10 * 375 * uint(tokenQuantity) * (10 ** 18) * (10 ** 6) / (uint(priceOracleInstance.getLatestPrice()) * 15);
    // Fee to maintain Radikal subscription in chainlink paying to generate random numbers
    uint chainlinkFeeMatic = radikalStoreInstance.getRecipeQuantityToChainLinkFee(tokenQuantity);
    require(msg.value >= (maticPart + chainlinkFeeMatic), "Collection Recipes: Matic insufficient funds");
    devTeamAddress.transfer(_DevMaticPart);
    liquidityAddress.transfer(maticPart - (_DevMaticPart));
    chainLinkRDKFeeAddress.transfer(chainlinkFeeMatic);
    payable(msg.sender).transfer(msg.value - ((_DevMaticPart) + (maticPart - (_DevMaticPart)) + chainlinkFeeMatic));
    bool sent = rdkToken.transferFrom(msg.sender, rewardPoolAddress, tokenPart * 75 / 85);
    require(sent, "Collection Recipes: rdk transfer failed");
    sent = rdkToken.transferFrom(msg.sender, retirementPlanAddress, tokenPart - (tokenPart * 75 / 85));
    require(sent, "Collection Recipes: rdk transfer failed");
    emit Referred(msg.sender, referral, tokenPart, maticPart);
    _requestRandomWords(tokenQuantity * 2);
  }
  ...
  /// @notice Mints a pack of recipe
  /// @dev Applies business rules to set Recipe Attributes and their token URI and delegate minting to CollectionRecipes
  function mint() external {
    require(userToMintingState[msg.sender] == MintingState.TO_REVEAL, "Collection Recipes: there are no NFT to reveal yet");
    ...
    uint16[] memory _eventTipPower = new uint16[](_randomWordsLength/2);
    for (uint i = 0; i < _randomWordsLength; i+= 2) {
      // Range of probability and URI number for the first random number
      (uint rangeResult, uint URINumber) = _getProbabilityRange(
        (_randomWords[i] % 1000) + 1,
        radikalStoreInstance.getRegularMintingProbs()
      );
      // Gets the Bottom Tip Power. Remember: Final Tip Power = Bottom Tip Power + (Random number inside a range for that tip Power) 
      uint16 recipeBottomAttribute = radikalStoreInstance.getRecipeAttribute(rangeResult);
      // Gets Random number inside a range tip power above
      uint16 recipeRandomTP = recipeBottomAttribute == commonTP ? 0 : uint16((_randomWords[i+1] % radikalStoreInstance.getRecipeTPRange(recipeBottomAttribute)));
      // Get the token URI for the NFT to mint
      string memory tokenURI = _getTokenURI(URINumber, radikalStoreInstance.getRecipesBaseURI());
      recipeInstance.mint(msg.sender, tokenURI, recipeRandomTP + recipeBottomAttribute);
      _eventTipPower[((_randomWordsLength - i)/2) - 1] = recipeRandomTP + recipeBottomAttribute;
    } 
    emit Minted(msg.sender, _eventTipPower);
    // updates the minting state of the user so that it can buy more recipe
  ...
  /// @notice Define price oracle interface
  /// @dev Should only be updated if liquidity is included in different pools or exchanges
  /// @param _commonTP MarketPlaceRadikals contract address
  function setCommonTP(uint16 _commonTP) external onlyOwner {
    commonTP = _commonTP;
  }
  ...
  function _getProbabilityRange(uint _randomNumber, uint[] memory recipeTypeProbability) internal pure returns (uint rangeResult, uint URINumber){
    ...
    return (rangeResult, URINumber); // URINumber can only go from 0 to 4. 5 is only available in the presale
  }
