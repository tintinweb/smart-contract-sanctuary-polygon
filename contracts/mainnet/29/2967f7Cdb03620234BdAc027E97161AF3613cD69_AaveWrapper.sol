...
/**
 * @notice Aave v2 wrapper contract. For explantion of the Aave calls please refer to https://docs.aave.com/developers/v/2.0/the-core-protocol/lendingpool
 */
contract AaveWrapper is AbstractWrapper {
  ILendingPool public aavePool = ILendingPool(0x7d2768dE32b0b80b7a3454c06BdAc94A69DDc7A9);
  ...
  /**
   * @notice Calls Aave v2 functions after requiest
   *
   * @param _action Aave action, wrappers available for deposit, setUserUseReserveAsCollateral, withdraw, borrow, repay.
   * @param _params Encoded parameters required by the given function.
   */
  function performAction(string calldata _action, bytes calldata _params) external override verifyCaller(msg.sender) {
    if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('deposit')))) {
      address asset = ByteUtils.bytesToAddress(_params, 0);
      uint256 amount = ByteUtils.bytesToUint(_params, 20, 32);
      address behalfOf = address(proxy);

      proxy.approveBalance(IERC20(asset), amount, address(aavePool));
      proxy.forward(address(aavePool), abi.encodeWithSignature('deposit(address,uint256,address,uint16)', asset, amount, behalfOf, uint16(0)));
      proxy.approveBalance(IERC20(asset), 0, address(aavePool));
    } else if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('setUserUseReserveAsCollateral')))) {
      address asset = ByteUtils.bytesToAddress(_params, 0);
      bool useAsCollateral = ByteUtils.bytesToBool(_params, 20, 0);

      proxy.forward(address(aavePool), abi.encodeWithSignature('setUserUseReserveAsCollateral(address,bool)', asset, useAsCollateral));
    } else if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('withdraw')))) {
      address asset = ByteUtils.bytesToAddress(_params, 0);
      uint256 amount = ByteUtils.bytesToUint(_params, 20, 32);
      address to = address(proxy);

      proxy.forward(address(aavePool), abi.encodeWithSignature('withdraw(address,uint256,address)', asset, amount, to));
    } else if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('borrow')))) {
      address asset = ByteUtils.bytesToAddress(_params, 0);
      uint256 amount = ByteUtils.bytesToUint(_params, 20, 32);
      uint256 interestRateMode = ByteUtils.bytesToUint(_params, 20 + 32, 32);
      address onBehalfOf = address(proxy);

      proxy.forward(
        address(aavePool),
        abi.encodeWithSignature('borrow(address,uint256,uint256,uint16,address)', asset, amount, interestRateMode, uint16(0), onBehalfOf)
      );
    } else if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('repay')))) {
      address asset = ByteUtils.bytesToAddress(_params, 0);
      uint256 amount = ByteUtils.bytesToUint(_params, 20, 32);
      uint256 rateMode = ByteUtils.bytesToUint(_params, 20 + 32, 32);
      address onBehalfOf = address(proxy);

      proxy.approveBalance(IERC20(asset), amount, address(aavePool));
      proxy.forward(address(aavePool), abi.encodeWithSignature('repay(address,uint256,uint256,address)', asset, amount, rateMode, onBehalfOf));
      proxy.approveBalance(IERC20(asset), 0, address(aavePool));
    } else if (keccak256(abi.encodePacked((_action))) == keccak256(abi.encodePacked(('liquidationCall')))) {
      address collateral = ByteUtils.bytesToAddress(_params, 0);
      address debt = ByteUtils.bytesToAddress(_params, 20);
      address user = ByteUtils.bytesToAddress(_params, 20 + 20);
      uint256 debtToCover = ByteUtils.bytesToUint(_params, 20 + 20 + 20, 32);
      bool receiveAToken = ByteUtils.bytesToBool(_params, 20 + 20 + 20 + 32, 0);

      proxy.approveBalance(IERC20(debt), debtToCover, address(aavePool));
      proxy.forward(
        address(aavePool),
        abi.encodeWithSignature('liquidationCall(address,address,address,uint256,bool)', collateral, debt, user, debtToCover, receiveAToken));
      proxy.approveBalance(IERC20(debt), 0, address(aavePool));
    } else {
      revert OPERATION_FAILED(address(aavePool), _action, 'unknown action');
    }
  }
}
...
/**
 * @notice taken from @aave/protocol-v2/contracts/interfaces/ILendingPool.sol
 */
