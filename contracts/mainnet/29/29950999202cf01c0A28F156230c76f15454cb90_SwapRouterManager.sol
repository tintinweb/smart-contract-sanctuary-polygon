...
contract SwapRouterManager is Ownable {
    ...
    function simpleSwap(
    ...
    ) public payable coverUp(masterInput) returns (uint256 receivedAmount) {
        transferTokensIfNeeded(data.fromToken, data.fromAmount, data.permit);
        ...
        Utils.transferTokens(
            data.toToken,
            payable(msg.sender),
            Utils.tokenBalance(data.toToken, address(this))
        );
    }
    ...
    function transferTokensIfNeeded(
        address token,
        uint256 amount,
        bytes memory permit
    ) private {
