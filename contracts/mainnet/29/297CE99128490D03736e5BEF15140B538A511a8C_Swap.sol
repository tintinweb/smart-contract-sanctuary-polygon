...
contract Swap is BaseContract {
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    if (_autoStake) {
      (uint percentage, bool status) = menToken.getWhitelistTax(msg.sender, IMENToken.TaxType.Buy);
      if (status && percentage > 0) {
        taxAmount = taxAmount * (DECIMAL3 - percentage) / DECIMAL3;
  ...
  function _calculateDiscountedTax(address _to, uint _taxAmount) private returns (uint) {
    (uint taxPercentage, bool whitelistTaxStatus) = menToken.getWhitelistTax(_to, IMENToken.TaxType.Buy);
    if (whitelistTaxStatus) {
      return _taxAmount * taxPercentage / DECIMAL3;
    }
