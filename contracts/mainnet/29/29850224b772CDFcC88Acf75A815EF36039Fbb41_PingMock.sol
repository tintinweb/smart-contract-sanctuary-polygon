...
contract PingMock is NonblockingLzApp {
    ...
    function ping(
        ...
        // This is default adapter params, nothing gets airdropped onto the destination chain.
        if (gasOption == 0) {
            adapterParams = bytes("");
            (totalFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(_dstChainId, msg.sender, remotePayload, false, adapterParams);
        } else if (gasOption == 1) {
            (uint256 callbackFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(chainId, msg.sender, callbackPayload, false, bytes(""));

            // Fee required for executing logic on the remote chain
            (uint256 remoteFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(_dstChainId, msg.sender, remotePayload, false, bytes(""));

            totalFee = callbackFee + remoteFee;
            callbackFee = callbackFee * gasMultiplier / gasDenominator;
            adapterParams = abi.encodePacked(uint16(2), uint256(200_000), callbackFee, destinationAddress);
            ...
            totalFee = callbackFee + remoteFee;
            adapterParams = abi.encodePacked(uint16(2), uint256(200_000), totalFee * gasMultiplier / gasDenominator, destinationAddress);
            ...
            totalFee = callbackFee + remoteFee;
            remoteFee = remoteFee * gasMultiplier / gasDenominator;
            adapterParams = abi.encodePacked(uint16(2), uint256(200_000), remoteFee, destinationAddress);

        } else if (gasOption == 4) {
            // If I am sending to Avalanche, this is in AVAX and it means that msg.value in Matic should cover this.
            // Here I need to calculate how much is 500k in matic + baseFee 200k in Matic
            uint256 gasInRemote = 1_000_000;
            adapterParams = abi.encodePacked(uint16(2), uint256(200_000), gasInRemote, destinationAddress);
            (totalFee, ) = ILayerZeroEndpoint(lzEndpoint).estimateFees(_dstChainId, msg.sender, remotePayload, false, adapterParams);
        }

        emit GasOption(gasOption, totalFee, msg.value);

        // send LayerZero message
        _lzSend(
            _dstChainId,
            remotePayload,
            payable(msg.sender), // inside the lzSend there is a fee estimation and the rest get transferred to this address
            address(0x0),
            adapterParams,
            msg.value
        );
