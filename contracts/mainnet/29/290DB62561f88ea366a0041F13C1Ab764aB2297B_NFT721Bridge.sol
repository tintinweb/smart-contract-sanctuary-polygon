...
contract NFT721Bridge is Initializable, ReentrancyGuard, NonblockingLzApp {
    ...
    mapping(address => bool) public mintBurnToken;
    ...
    function _nonblockingLzReceive(uint16 _srcChainId, bytes memory _srcAddress, uint64 _sequence, bytes memory _payload) internal override {
        ...
        if (transfer.tokenChain == chainId) {
            if (mintBurnToken[transfer.tokenAddress]) {
                ZKBridgeErc721(transfer.tokenAddress).zkBridgeMint(transfer.to, transfer.tokenID, transfer.uri);
            } else {
    ...
    function setWrappedAsset(uint16 _nativeChainId, address _nativeContract, address _wrapper, bool isMintBurn) external onlyOwner {
        ...
        mintBurnToken[_wrapper] = isMintBurn;
    }
