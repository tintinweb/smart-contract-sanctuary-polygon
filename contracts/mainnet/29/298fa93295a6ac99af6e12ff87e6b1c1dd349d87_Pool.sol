...
// Addresses for withdraw function (dev = me)
  ...
  constructor(address _gold, address _traits, uint256 _maxTokens) ERC721("Desert Clash", 'Desert Clash') { 
    gold = GOLD(_gold);
  ...
  function mint(uint256 amount, bool stake) external payable whenNotPaused {
    ...
    for (uint i = 0; i < amount; i++) {
      ...
      if (!stake || recipient != _msgSender()) {
        ...
        addToMapping(recipient, minted);
      } else {
        _safeMint(address(pool), minted);
        addToMapping(address(pool), minted);
        tokenIds[i] = minted;
       }
      _minted[msg.sender]++;
      tokenMinted(minted);
  ...
  function addToMapping(address account, uint256 tokenId) internal {
    uint len = tokensOfOwner[account].length;
    tokensOfOwner[account][len] = uint16(tokenId);
  }
  ...
  function camelsOfOwner(address account) external view returns(uint16[] memory tokenIds) {
    for (uint i; i < tokensOfOwner[account].length; i++) {
      if(tokenTraits[tokensOfOwner[account][i]].isCamel == true) {
        tokenIds[tokenIds.length] = tokensOfOwner[account][i];
      }
    }
    return tokenIds;
  }
  
  function banditsOfOwner(address account) external view returns(uint16[] memory tokenIds) {
    for (uint i; i < tokensOfOwner[account].length; i++) {
...
contract Pool is IPool, Ownable, IERC721Receiver, Pausable, VRFConsumerBase {
   ...
   */
  constructor(address _camelit, address _gold)  VRFConsumerBase(0x3d2341ADb2D31f1c5530cDC622016af293177AE0, 0xb0897686c545045aFc77CF20eC7A532E3120E0F1) { 
    camelit = Camelit(_camelit);
    gold = GOLD(_gold);
  }
  /** READ ONLY */
  function CamelsOfOwner(address account) external view returns(uint16[] memory) {
    uint len;
    uint16[] memory temp = new uint16[](tokensOfOwner[account].length);
    for (uint i; i < tokensOfOwner[account].length; i++) {
      if(isCamel(tokensOfOwner[account][i])) {
  ...
  function claimableGOLDForCamels(uint16[] calldata tokenIds) external view returns(uint256 claimable, uint256 withTax) {
    for (uint i = 0; i < tokenIds.length; i++) {
      require(camelit.ownerOf(tokenIds[i]) == address(this), "Token not staked");
      Stake memory stake = pool[tokenIds[i]];
      require(isCamel(tokenIds[i]), "Not a camel");
      claimable += figureOutOwedForCamel(stake);
    }
  withTax = claimable - (15 * claimable) / 100;
  }
  function claimableGOLDForBandits(uint16[] calldata tokenIds) external view returns(uint256 claimable) {
    for (uint i = 0; i < tokenIds.length; i++) {
      require(camelit.ownerOf(tokenIds[i]) == address(this), "Token not staked");
      Stake memory stake = pool[tokenIds[i]];
      require(!isCamel(tokenIds[i]), "Not a bandit");
      claimable += figureOutOwedForBandit(stake);
    }
  }
  /** STAKING */
  ...
  function claimManyFromPool(uint16[] calldata tokenIds, bool unstake) external whenNotPaused _updateEarnings {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      if (isCamel(tokenIds[i]))
        _claimCamelFromPool(tokenIds[i], unstake);
      else
        owed += _claimBanditFromPool(tokenIds[i], unstake);
      if(unstake) {
        uint len = tokensOfOwner[msg.sender].length;
        uint pos;
        for(uint x; x < len; x++) {
          if(tokensOfOwner[msg.sender][x] == tokenIds[i]) {
            pos = x;
            break;
          }
          else {continue;}
        }
        tokensOfOwner[msg.sender][pos] = tokensOfOwner[msg.sender][len - 1];
  ...
  function figureOutOwedForCamel(Stake memory stake) internal view returns(uint256 owed) {
  ...
  function figureOutOwedForBandit(Stake memory stake) internal view returns(uint256 owed) {
   ...
   */
  function _claimCamelFromPool(uint256 tokenId, bool unstake) internal {
    Stake memory stake = pool[tokenId];
    require(camelit.ownerOf(tokenId) == address(this), "AINT A PART OF THE POOL");
    require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
    attemptStealing(tokenId, unstake);
  }
    function attemptStealing(uint tokenId, bool unstake) internal {
        bytes32 requestId = requestRandomness(keyHash, fee);
        pending[requestId] = forVRF(tokenId, unstake);
    }
  function fulfillRandomness(bytes32 requestId, uint256 randomness) internal override {
    uint256 tokenId = pending[requestId].tokenId;
    Stake memory stake = pool[tokenId];
    bool unstake = pending[requestId].unstake;
    uint256 owed = figureOutOwedForCamel(stake);
    uint256 randomValue = randomness; 
    uint256 numDays = (block.timestamp - stake.value) / 1 days;
    numDays = (numDays > 9) ? 9 : numDays;
    uint256 probStealing = 55 - (5 * numDays);
    bool stolen = (randomValue % 100) < probStealing;
    if(!stolen || totalBanditStaked == 0) {
    ...
    if (unstake) {
      camelit.safeTransferFrom(address(this), stake.owner, tokenId, ""); // send back Camel
      delete pool[tokenId];
      totalCamelStaked -= 1;
    } else {
      pool[tokenId] = Stake({
  ...
  function _claimBanditFromPool(uint256 tokenId, bool unstake) internal returns (uint256 owed) {
    ...
    require(camelit.ownerOf(tokenId) == address(this), "AINT A PART OF THE POOL");
    require(stake.owner == _msgSender(), "SWIPER, NO SWIPING");
    owed = figureOutOwedForBandit(stake);
    if (unstake) {
