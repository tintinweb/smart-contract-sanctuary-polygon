...
contract Fight {
    ...
    constructor(address _ff, address _dev, address _dev2, uint _dscs,uint _dscs2, uint _spotResPrice, address _fightAdmin, address _tc, address _influ2, address _charity, address _lsp) {
        ...
        require(extention <=3600 && duration<= 604800 && duration >= 21600, '');
  ...
  //flip
   ...
   //Vote 2
    ...
    require ((infs+us+cs) == 100 && (_iscs+_uscs) == 64 && srt >= 300 && fs <=100 && scb<=100 && (created+spotBusyTime) > block.timestamp, '');
    flipperShare = fs;
    influ2Share = infs;
    spotCashBack = scb;
    usersSlice = us;
