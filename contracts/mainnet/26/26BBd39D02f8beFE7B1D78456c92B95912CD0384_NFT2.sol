...
interface Interface {
    ...
    function viewActiveTokens(address _address) external view returns (uint256);

    function subtractTokens(address _address, uint256 _tokens) external;
}
...
contract NFT2 is ERC721Enumerable, Ownable {
    ...
    function mint(address _to, uint256 _mintAmount) public payable notPaused {
        ...
        for (uint256 i = 0; i < _mintAmount; i++) {
            uint256 tempMint = nextMint;
            nextMint++;
            _safeMint(_to, tempMint);
        }
    }
    ...
    function viewMyTokens(address _address) public view returns (uint256) {
        uint256 _active = Interface(parentContract).viewActiveTokens(_address);
        uint256 _allocated = Interface(parentContract).viewAllocatedTokens(
            _address
        );
        uint256 _subtracted = Interface(parentContract).viewsubtractedTokens(
            _address
        );
    ...
    function withdraw() public onlyOwner {
        (bool oc, ) = payable(owner()).call{value: address(this).balance}("");
        require(oc);
    }
}