...
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    function usersSilosFilterCategory(address _user, uint _category, bool _filter) external view returns(uint[] memory){
        uint count;
        ISilo silo;
        if(_filter){
        ...
        uint[] memory silos = new uint[](count);
        count = 0;
        for(uint i=0; i<balanceOf(_user); i++){
            silo = ISilo(siloMap[tokenOfOwnerByIndex(_user, i)]);
            if(!_filter || silo.strategyCategory() == _category){
                silos[count] = tokenOfOwnerByIndex(_user, i);
                count+=1;
            }
        }
        return silos;
    ...
    function usersSilosAddressesFilterCategory(address _user, uint _category, bool _filter) external view returns(address[] memory){
        uint count;
        ISilo silo;
        if(_filter){
            for(uint i=0; i<balanceOf(_user); i++){
