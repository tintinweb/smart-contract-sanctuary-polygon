...
contract CuteCorals is ERC721A, Ownable, ERC2981, DefaultOperatorFilterer {
    using Strings for uint256;
    ...
    uint256 public wlLimit = 3;
    uint256 public max_per_wallet = 3000;
    ...
    mapping(address => bool) public mint3free2_claimed;
    mapping(address => bool) public mint7free4_claimed;

    constructor(
        string memory _initBaseURI,
        string memory _initNotRevealedUri,
        string memory _contractURI
    ) ERC721A("Cute Corals", "CC") {
    ...
    function mint(uint256 quantity) public payable {
        ...
        if (msg.sender != owner()) {
            require(public_mint_status, "Public mint is paused");
            require(
    ...
    function mint3free2() public payable {
        require(totalSupply() + 3 <= MAX_SUPPLY, "Not enough tokens left");

        if (msg.sender != owner()) {
            require(!paused, "the contract is paused");
            require(
                mint3free2_claimed[msg.sender] == false,
                "'Mint 3 Free 2', Already Claimed"
            );
            require(msg.value >= (publicSaleCost * 1), "Not enough ether sent");
        }
        _safeMint(msg.sender, 3);
        publicmint_claimed[msg.sender] = publicmint_claimed[msg.sender] + 3;
        mint3free2_claimed[msg.sender] = true;
    }

    function mint7free4() public payable {
        require(totalSupply() + 7 <= MAX_SUPPLY, "Not enough tokens left");

        if (msg.sender != owner()) {
            require(!paused, "the contract is paused");
            require(
                mint7free4_claimed[msg.sender] == false,
                "'Mint 7 Free 4', Already Claimed"
            );
            require(msg.value >= (publicSaleCost * 3), "Not enough ether sent");
        }
        _safeMint(msg.sender, 7);
        publicmint_claimed[msg.sender] = publicmint_claimed[msg.sender] + 7;
        mint7free4_claimed[msg.sender] = true;
    }
        ...
        payable
    {
        ...
        require(wl_mint_status, "Whitelist mint is paused");
        require(whitelist_claimed[msg.sender] + quantity <= wlLimit,"Per wallet whitelist limit reached");
    ...
    function setWlLimit(uint256 _wlLimit) public onlyOwner {
        wlLimit = _wlLimit;
    }
