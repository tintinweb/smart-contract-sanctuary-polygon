...
contract FirstApproval is Ownable, ERC20Capped, ERC20Burnable, ERC20Permit {
    ...
    bool public mintingDisabledForever = false;  // when this flag is set, no new tokens can be minted

    constructor(
        string memory _name,
        string memory _symbol,
        uint256 _cap  // expected to be 300 * 1e6 * 1e18
    )
        ERC20(_name, _symbol)
        ERC20Capped(_cap)
        ERC20Permit(_name)
    ...
    function mint(address account, uint amount) onlyOwner external {
        require(!mintingDisabledForever, "minting disabled forever");
