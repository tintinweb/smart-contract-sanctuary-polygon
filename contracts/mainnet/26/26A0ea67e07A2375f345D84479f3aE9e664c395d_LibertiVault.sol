...
import "./interfaces/ILibertiSwap.sol";
    ...
    ReentrancyGuardUpgradeable
{
    ...
    address private immutable delegatedSwap;
    address private immutable delegatedPriceFeed;
    ISanctionsList private immutable sanctionsList;

    uint8 private assetDecimals;
    uint8 private otherDecimals;
    ...
    uint256 private constant MAX_TOTAL_ASSETS = 1_000_000_000 * 1e18;

    constructor(
        address _delegatedPriceFeed,
        address _delegatedSwap,
        address _sanctionsList
    ) {
        _disableInitializers();
        delegatedPriceFeed = _delegatedPriceFeed;
        delegatedSwap = _delegatedSwap;
    ...
    function swapExactTokensForTokens(
        ...
        (bool success, bytes memory returndata) = delegatedSwap.delegatecall(
            abi.encodeCall(
                ILibertiSwap.swapExactTokensForTokens,
                (amount, token0, token1, receiver)
            )
        );
        if (!success) {
    ...
    function rebalance(
        ...
        require(newInvariant <= 10_000, "!invariant");
        invariant = newInvariant; // For subsequent deposits
        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = delegatedSwap.delegatecall(
            abi.encodeCall(ILibertiSwap.swap, (minOut, data))
        );
        if (!success) {
            // solhint-disable-next-line no-inline-assembly
            assembly {
    ...
    // round down the value of the user deposit, round up the tvl of the vault
    // price feeds of assets and other token must have the same number of decimals
    function _previewDeposit(uint256 assets) private view returns (uint256, uint256) {
        uint256 supply = totalSupply();
        if (0 == assets || 0 == supply) {
            return (assets, 0);
        }
        address _asset = asset();
        uint256 keepAssets = assets.mulDiv(invariant, 10_000);
        uint256 sellAssets = assets - keepAssets;
        ILibertiPriceFeed priceFeed = ILibertiPriceFeed(delegatedPriceFeed);
        uint256 assetPrice = priceFeed.getPrice(_asset);
        uint256 depositAssetInUsd = assetPrice.mulDiv(
            keepAssets,
            10**assetDecimals,
            MathUpgradeable.Rounding.Down
        );
        uint256 depositOtherInUsd = 0;
        uint256 otherPrice = priceFeed.getPrice(other);
        if (0 != sellAssets) {
            uint256[] memory amountsOut = ILibertiSwap(delegatedSwap).getAmountsOut(
                sellAssets,
                _asset,
                other
            );
            depositOtherInUsd = otherPrice.mulDiv(
                amountsOut[1],
                10**otherDecimals,
                MathUpgradeable.Rounding.Down
            );
        }
        uint256 nav = assetPrice.mulDiv(
            IERC20Metadata(_asset).balanceOf(address(this)),
            10**assetDecimals,
            MathUpgradeable.Rounding.Up
        );
        nav += otherPrice.mulDiv(
        ...
        return (
            supply.mulDiv(
                depositAssetInUsd + depositOtherInUsd,
                nav,
                MathUpgradeable.Rounding.Down
            ),
            sellAssets
    ...
    function _deposit0(uint256 assets, address receiver)
        internal
        nonReentrant
        whenNotPaused
        returns (uint256)
    {
        require(!sanctionsList.isSanctioned(msg.sender), "!sanction");
        require(assets <= maxDeposit(receiver), "ERC4626: deposit more than max");
        (uint256 shares, uint256 soldAssets) = _previewDeposit(assets);
        if (0 < soldAssets) {
            swapExactTokensForTokens(soldAssets, asset(), other, address(this));
        }
        return shares;
    }

    function deposit(uint256 assets, address receiver) public override returns (uint256) {
        uint256 shares = _deposit0(assets, receiver);
        ...
        )
    {
        ...
        uint256 assets = IERC20Metadata(asset_).balanceOf(address(this));
        uint256 assetsAmount = assets.mulDiv(shares, supply, MathUpgradeable.Rounding.Down);
        uint256 others = IERC20Metadata(other).balanceOf(address(this));
        if (0 == others) {
            return (assetsAmount, assetsAmount, 0);
        }
        uint256 othersAmount = others.mulDiv(shares, supply, MathUpgradeable.Rounding.Down);
        uint256[] memory amountsOut = ILibertiSwap(delegatedSwap).getAmountsOut(
            othersAmount,
            other,
            asset_
        );
        return (assetsAmount + amountsOut[1], assetsAmount, othersAmount);
    }
    ...
    function _redeem0(uint256 shares, address receiver)
        internal
        nonReentrant
        returns (uint256, uint256)
    {
        (uint256 assets, uint256 assetsAmount, uint256 otherReturn) = _previewRedeem(shares);
        if (0 < otherReturn) {
            swapExactTokensForTokens(otherReturn, other, asset(), receiver);
        }
        return (assets, assetsAmount);
    }
    ...
    function redeem(
        ...
        (uint256 assets, uint256 assetsAmount) = _redeem0(shares, receiver);
        _withdraw(_msgSender(), receiver, owner_, assetsAmount, shares);
        return assets;
    }
...
interface ILibertiSwap {
    ...
    function swap(uint256 minOut, bytes calldata data) external returns (bytes memory);
}
