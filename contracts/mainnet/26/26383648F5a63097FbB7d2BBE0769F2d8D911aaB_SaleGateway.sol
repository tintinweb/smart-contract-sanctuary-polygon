  ...
  ISaleGateway
{
  ...
  DstChain[] public override allDstChains; // all chains supported

  mapping(uint240 => uint256) public override dstChainIndex;
  ...
  /**
   * @dev Get total number of chains supported
   */
  function allDstChainsSupportedLength() external view virtual override returns (uint256) {
    return allDstChains.length;
  }

  function setRemote(
    uint240[] calldata _chainIDs,
    uint16[] calldata _lzChainIDs,
    address[] calldata _saleGateways
  ) external virtual onlyOwner {
    require(_chainIDs.length == _lzChainIDs.length && _chainIDs.length == _saleGateways.length, 'bad');

    for (uint8 i = 0; i < _chainIDs.length; ++i) {
      if (_chainIDs[i] == uint240(block.chainid)) continue;

      dstChainIndex[_chainIDs[i]] = allDstChains.length;
      allDstChains.push(DstChain({chainID: _chainIDs[i], lzChainID: _lzChainIDs[i], saleGateway: _saleGateways[i]}));

      this.setTrustedRemoteAddress(_lzChainIDs[i], abi.encodePacked(_saleGateways[i]));
    }
  }
  ...
  function _nonblockingLzReceive(
    ...
    (uint240 chainID, address targetSale, uint256 amountIn, address user) = abi.decode(
      _payload,
      (uint240, address, uint256, address)
    );

    uint240 knownChainID = allDstChains[dstChainIndex[chainID]].chainID;

    require(chainID == knownChainID, '!chainID');

    IPublicGovSale(targetSale).buyToken(chainID, amountIn, user);
  }

  function pushForceRemoteData(bytes memory _payload) external virtual onlyOwner {
    _nonblockingLzReceive(0, new bytes(0), 0, _payload);
  }

  /**
   * @dev Toggle lzReceive pause
   */
