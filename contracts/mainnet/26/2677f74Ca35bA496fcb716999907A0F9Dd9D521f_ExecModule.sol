...
contract ExecModule is Module
{
	...
	address public immutable DEFAULT_WHITELIST;

	constructor(address _DEFAULT_WHITELIST)
	{
		DEFAULT_WHITELIST = _DEFAULT_WHITELIST;
	}
	...
	function action_multicall(address[] calldata _modules, uint256[] calldata _values, bytes[] calldata _calldatas) external nonReentrant onlyInstalled returns (bytes[] memory _returndatas)
	{
		_returndatas = new bytes[](_calldatas.length);
		Data storage _data = data[msg.sender];
		...
		if (_whitelist != address(0)) {
			for (uint256 _i = 0; _i < _modules.length; _i++) {
				...
				_returndatas[_i] = IFractions(msg.sender).__externalCall(_module, _values[_i], _calldata);
			}
		} else {
			for (uint256 _i = 0; _i < _modules.length; _i++) {
				_returndatas[_i] = IFractions(msg.sender).__externalCall(_modules[_i], _values[_i], _calldatas[_i]);
			}
		}
		return _returndatas;
	}
	...
	function estimateAction(address payable _fractions, address _module, uint256 _value, bytes calldata _calldata, uint256 _deadline) external returns (bytes memory _returndata)
	{
		bytes32 _actionId = IFractions(_fractions).calcActionId(_module, _value, _calldata, _deadline);
		IFractions(_fractions).__scheduleAction(_actionId, block.timestamp);
		uint256 _startGas = gasleft();
		_returndata = executeAction(_fractions, _module, _value, _calldata, _deadline);
		uint256 _endGas = gasleft();
		revert(string(abi.encodePacked(this.estimateAction.selector, _startGas - _endGas, _returndata)));
	}

	function executeAction(address payable _fractions, address _module, uint256 _value, bytes calldata _calldata, uint256 _deadline) public nonReentrant installedOn(_fractions) returns (bytes memory _returndata)
	{
	...
	function executeAction(address payable _fractions, address _module, uint256 _value, bytes calldata _calldata) external singleOwner(_fractions) returns (bytes memory _returndata)
	{
		bytes32 _actionId = IFractions(_fractions).calcActionId(_module, _value, _calldata, block.timestamp);
		IFractions(_fractions).__scheduleAction(_actionId, block.timestamp);
		return executeAction(_fractions, _module, _value, _calldata, block.timestamp);
	}
...
import { IFractionsCore } from "./IFractionsCore.sol";
import { ActionScheduler } from "./ActionScheduler.sol";

interface IFractions is IFractionsCore, IERC20Metadata, IERC20Permit, IERC721Receiver, IERC1155Receiver
{
	// AccessControlled
	function accessController() external view returns (address _accessController);

	// Extensible
	function extension() external view returns (address _extension);

	// ActionScheduler
	function actions(bytes32 _actionId) external view returns (ActionScheduler.ActionInfo memory _info);
	...
	// ERC20Votes
	function checkpoints(address _account, uint32 _pos) external view returns (ERC20Votes.Checkpoint memory _checkpoint);
	function numCheckpoints(address _account) external view returns (uint32 _count);
...
{
	function __setAccessController(address _accessController) external;
	function __setExtension(address _extension) external;
...
abstract contract ActionScheduler
{
	enum ActionState { Default, Scheduled, Executing, Executed, Invalidated }
	...
	bytes32 private constant ACTION_TYPEHASH = keccak256("Action(address module,uint256 value,bytes calldata,uint256 deadline)");

	mapping(bytes32 => ActionInfo) public actions;

	function calcActionId(address _module, uint256 _value, bytes memory _calldata, uint256 _deadline) public view returns (bytes32 _actionId)
	{
		return _hashTypedDataV4(keccak256(abi.encode(ACTION_TYPEHASH, _module, _value, _calldata, _deadline)));
	}

	function _hashTypedDataV4(bytes32 _data) internal view virtual returns (bytes32 _hash);

	function _scheduleAction(bytes32 _actionId, uint256 _available) internal
	{
		ActionInfo storage _info = actions[_actionId];
		require(_info.state == ActionState.Default, "illegal state");
		_info.state = ActionState.Scheduled;
		_info.available = _available;
		_info.block = block.number;
		emit ActionScheduled(_actionId, _available);
	}

	function _unscheduleAction(bytes32 _actionId) internal
	{
		ActionInfo storage _info = actions[_actionId];
		require(_info.state == ActionState.Scheduled, "illegal state");
		_info.state = ActionState.Default;
		_info.available = 0;
		_info.block = 0;
		emit ActionUnscheduled(_actionId);
	}

	function _executeAction(address _module, uint256 _value, bytes memory _calldata, uint256 _deadline) internal returns (bytes memory _returndata)
	{
		require(block.timestamp <= _deadline, "expired action");
		bytes32 _actionId = calcActionId(_module, _value, _calldata, _deadline);
		ActionInfo storage _info = actions[_actionId];
		require(_info.state == ActionState.Scheduled, "illegal state");
		require(block.timestamp >= _info.available, "unavailable action");
		_info.state = ActionState.Executing;
		_info.available = 0;
		(bool _success, bytes memory _result) = _module.call{value: _value}(_calldata);
		require(_success, string(_result));
		_info.state = ActionState.Executed;
		emit ActionExecuted(_actionId);
		return _result;
	}

	function _invalidateAction(bytes32 _actionId) internal
	{
		ActionInfo storage _info = actions[_actionId];
		ActionState _state = _info.state;
		require(_state == ActionState.Default || _state == ActionState.Scheduled, "invalid state");
		_info.state = ActionState.Invalidated;
		_info.available = 0;
		emit ActionInvalidated(_actionId);
	}

	event ActionScheduled(bytes32 indexed _actionId, uint256 indexed _available);
	event ActionUnscheduled(bytes32 indexed _actionId);
	event ActionInvalidated(bytes32 indexed _actionId);
	event ActionExecuted(bytes32 indexed _actionId);
}
