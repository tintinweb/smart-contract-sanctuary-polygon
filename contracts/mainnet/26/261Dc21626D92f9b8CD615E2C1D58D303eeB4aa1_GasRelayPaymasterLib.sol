...
contract FundDeployer is IFundDeployer, IMigrationHookHandler, GasRelayRecipientMixin {
    ...
    function setComptrollerLib(
        address _comptrollerLib
    )
        external
        onlyOwner // pseudoConstant(getComptrollerLib())
    {
    ...
    function setProtocolFeeTracker(
        address _protocolFeeTracker
    )
        external
        onlyOwner // pseudoConstant(getProtocolFeeTracker())
    {
    ...
    function setVaultLib(
        ...
        external
        onlyOwner // pseudoConstant(getVaultLib())
    {
        vaultLib = _vaultLib;
    ...
    function cancelMigration(
        address _vaultProxy,
        bool _bypassPrevReleaseFailure
    ) external onlyMigratorNotRelayable(_vaultProxy) {
    ...
    function getReconfigurationRequestForVaultProxy(
        address _vaultProxy
    ) public view returns (ReconfigurationRequest memory reconfigurationRequest_) {
    ...
    function hasReconfigurationRequest(
        address _vaultProxy
    ) public view override returns (bool hasReconfigurationRequest_) {
    ...
    function isAllowedBuySharesOnBehalfCaller(
        address _who
    ) public view override returns (bool isAllowed_) {
...
contract ComptrollerLib is IComptroller, IGasRelayPaymasterDepositor, GasRelayRecipientMixin {
    ...
    function __assertSharesActionNotTimelocked(
        address _vaultProxy,
        address _account
    ) private view {
    ...
    function __hasPendingMigrationOrReconfiguration(
        address _vaultProxy
    ) private view returns (bool hasPendingMigrationOrReconfiguration) {
        return
            IDispatcher(getDispatcher()).hasMigrationRequest(_vaultProxy) ||
    ...
    function setAutoProtocolFeeSharesBuyback(
        bool _nextAutoProtocolFeeSharesBuyback
    ) external onlyOwner {
    ...
    function __assertPermissionedVaultAction(
        address _caller,
        IVault.VaultAction _action
    ) private view {
        bool validAction;
    ...
    function __calcExternalPositionValue(
    ...
    ) external onlyOwner returns (uint256 sharesReceived_) {
    ...
    // REDEEM SHARES From Fund Manager
    function redeemSharesOnBehalf(
        address _shareHolder,
        uint256 _sharesQuantity,
        address[] calldata _payoutAssets,
        uint256[] calldata _payoutAssetPercentages
    ) external locksReentrance onlyOwner returns (uint256[] memory payoutAmounts_) {
        require(
    ...
    // REDEEM Shares In Kind From Fund Manager
        ...
        returns (address[] memory payoutAssets_, uint256[] memory payoutAmounts_)
    {
        ...
        emit SharesRedeemed(_recipient, _recipient, sharesToRedeem, payoutAssets_, payoutAmounts_);
    ...
    function setGasRelayPaymaster(
        address _nextGasRelayPaymaster
    ...
    function getLastSharesBoughtTimestampForAccount(
        address _who
    ) public view returns (uint256 lastSharesBoughtTimestamp_) {
