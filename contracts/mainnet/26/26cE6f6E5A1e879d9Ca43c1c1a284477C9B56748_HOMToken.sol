...
contract HOMToken is BEP20, MinterRole {
    ...
    mapping (address => uint256) private buyfirsttime;
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if(reserves >= 10  || reserves < 2)
            {
                ...
                if(automatedMarketMakerPairs[recipient] && sellFee > 0) {
                    ...
                    if(block.timestamp < buyfirsttime[sender] + ( 86400 * 30 *20 ))  //20 months
                    {
                ...
                //Sell Token
                if(automatedMarketMakerPairs[recipient] && sellFee > 0) {
                    feeAmount = amount.div(reserves);
                    sendAmount = amount.sub(feeAmount);
                    uint256 tokenamount = balanceOf(sender);  // token amount in sender
                    if(block.timestamp < buyfirsttime[sender] + ( 86400 * 30 *20 ))
                    {
                        if(selltime[sender] == 0)
                        {
                            require(amount < ( tokenamount / 20), "You can sell the 5% of your tokens only"); 
