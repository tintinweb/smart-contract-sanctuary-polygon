...
abstract contract App is UpgradeableApp, Initializable, ERC2771ContextUpgradeable {
...
contract ERC721PaymentOpenSale is App, __with_semver(uint8(1), uint8(0), uint8(0)) {
    using SafeERC20 for IERC20;
    string public constant BUY_LIMIT_ERROR = "BUY_LIMIT_ERROR";
    string public constant NULL_STABLECOIN_ADDRESS = "NULL_STABLECOIN_ADDRESS";
    string public constant UNSUPPORTED_TOKEN_ADDRESS = "UNSUPPORTED_TOKEN_ADDRESS";
    string public constant NOT_FOUND_PRICE_FOR_TIER = "NOT_FOUND_PRICE_FOR_TIER";
    string public constant INSUFFICIENT_AMOUNTS_OF_FUNDS = "INSUFFICIENT_AMOUNTS_OF_FUNDS";
    ...
    mapping(bytes32 => uint256) public tierPrices; // stored in DAI
    ...
    event SetClaimLimit(uint64 claimLimit_, bytes32 tierValue);
    event SetActive(bool setValue);
    event SetStableCoinAddress(address stableCoinAddress_);
    event InitERC721PaymentOpenSale(bytes32[] tiers, uint256[] claimLimits, uint256[] tierPrices_);
    ...
    function initialize(
        ...
        require(
            tierPrices_.length == tiers.length && tierPrices_.length == claimLimits.length,
            Exceptions.VALIDATION_ERROR
        );
        __App_init(kernel);
        for (uint256 i; i != tiers.length; ++i) {
            claimLimitByTier[tiers[i]] = claimLimits[i];
            tierPrices[tiers[i]] = tierPrices_[i];
        }
        _setStableCoinAddress(stableCoinAddress_);

        emit InitERC721PaymentOpenSale(tiers, claimLimits, tierPrices_);
    }

    function buyPrice(address tokenAddress, bytes32 tierValue)
        external
        view
        returns (uint256 stablePrice, uint256 tokenPrice)
    {
        return _buyPrice(tokenAddress, tierValue);
    }

    function _buyPrice(address tokenAddress, bytes32 tierValue)
        internal
        view
        returns (uint256 stablePrice, uint256 tokenPrice)
    {
        uint256 cost = tierPrices[tierValue];
        require(cost != 0, NOT_FOUND_PRICE_FOR_TIER);
        address oracleAddress = kernel.getOracleAddress();
        require(UniswapV3Oracle(oracleAddress).isWhitelistTokenAddress(tokenAddress), UNSUPPORTED_TOKEN_ADDRESS);
        uint256 price = UniswapV3Oracle(oracleAddress).getPrice(stableCoinAddress, tokenAddress);
    ...
    function buy(
        ...
        require((++claims[tierValue]) <= claimLimitByTier[tierValue], BUY_LIMIT_ERROR);
        address oracleAddress = kernel.getOracleAddress();
        require(UniswapV3Oracle(oracleAddress).isWhitelistTokenAddress(tokenAddress), UNSUPPORTED_TOKEN_ADDRESS);

        // Get price and transfer ERC20 token to Treasury
        (, uint256 costInTokens) = _buyPrice(tokenAddress, tierValue);
        IERC20 token = IERC20(tokenAddress);
        uint256 balanceOfSender = token.balanceOf(_msgSender());
        require(balanceOfSender >= costInTokens, INSUFFICIENT_AMOUNTS_OF_FUNDS);
        _transferToTreasury(tokenAddress, costInTokens);

        // transfer NFT to user
        address appAddress = kernel.getAppAddress(AppsIds.ERC721);
        ERC721Properties(appAddress).mint(to, tierValue);
        emit OnBuy(to, appAddress, tierValue, costInTokens, tokenAddress, "SALE", __semver());
    }
    ...
    function setClaimLimit(uint64 claimLimit_, bytes32 tierValue) external requirePermission(CONTROLLER) {
        claimLimitByTier[tierValue] = claimLimit_;
        emit SetClaimLimit(claimLimit_, tierValue);
    }

    function setStableCoinAddress(address stableCoinAddress_) external requirePermission(CONTROLLER) {
        _setStableCoinAddress(stableCoinAddress_);
    }

    function _setStableCoinAddress(address stableCoinAddress_) internal requirePermission(CONTROLLER) {
        require(stableCoinAddress_ != address(0) && stableCoinAddress_.code.length > 0, NULL_STABLECOIN_ADDRESS);
        stableCoinAddress = stableCoinAddress_;
        emit SetStableCoinAddress(stableCoinAddress_);
    }
    ...
    function getLeftClaimsForTier(address userAddress, bytes32 tierValue) external view returns (int256) {
        if (claimLimitByTier[tierValue] == 0) {
            return -1;
        }
        return int256(claimLimitByTier[tierValue]) - int256(claims[tierValue]);
    }

    function _transferToTreasury(address tokenAddress_, uint256 costInTokens_) internal {
        address wallet = kernel.getAppAddress(AppsIds.TREASURY);
        require(wallet != address(0x00), Exceptions.NO_TREASURY_ADDRESS);
        IERC20 token = IERC20(tokenAddress_);
        token.safeTransferFrom(_msgSender(), wallet, costInTokens_);
    }
}
...
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App, __with_semver(uint8(1), uint8(0), uint8(0)) {
    using Strings for uint256;
...
library AppsIds {
    ...
    bytes32 constant ERC721_PAYMENT_OPEN_SALE = keccak256(abi.encodePacked("ERC721_PAYMENT_OPEN_SALE"));
}
...
interface IOracle {
    function getPrice(address tokenAddressA, address tokenAddressB) external view returns (uint256 price);
}
...
library UniswapV3Math {
    function getSqrtTwapX96(IUniswapV3Pool pool, uint32 twapInterval) internal view returns (uint160 sqrtPriceX96) {
        if (twapInterval == 0) {
            ...
            sqrtPriceX96 = getSqrtRatioAtTick(int24((tickCumulatives[1] - tickCumulatives[0]) / int32(twapInterval)));
        }
    }
...
{
    using Strings for uint256;
    string public constant LIQUIDITY_POOL_NOT_FOUND = "LIQUIDITY_POOL_NOT_FOUND";
    uint24 public feeTier;
    address public factoryAddress;
    uint32 twapInterval;
    mapping(address => bool) internal _whitelistTokens;
    ...
    constructor(
        ...
        address[] memory whitelistTokensAddress
    ) {
        feeTier = feeTier_;
        factoryAddress = factoryAddress_;
        twapInterval = twapInterval_;
        factory = IUniswapV3Factory(factoryAddress);
        for (uint256 i; i != whitelistTokensAddress.length; ++i) {
            _whitelistTokens[whitelistTokensAddress[i]] = true;
        }
    }
    ...
    function getPrice(address tokenAddressA, address tokenAddressB) external view returns (uint256 price) {
        ...
        address poolAddress = factory.getPool(tokenAddressA, tokenAddressB, feeTier);
        require(poolAddress != address(0) && poolAddress.code.length > 0, LIQUIDITY_POOL_NOT_FOUND);
        IUniswapV3Pool pool = IUniswapV3Pool(poolAddress);
        uint160 sqrtPriceX96 = UniswapV3Math.getSqrtTwapX96(pool, twapInterval);
    ...
    function setFeeTier(uint24 feeTier_) external onlyOwner {
        feeTier = feeTier_;
    }

    function isWhitelistTokenAddress(address tokenAddress) external view returns (bool) {
        return _whitelistTokens[tokenAddress];
    }
}