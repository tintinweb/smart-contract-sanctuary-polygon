...
contract UniswapHelper is IUniswapV3MintCallback, IUniswapV3SwapCallback {
    ...
    int24 public tickWidth = 2000; // for 2200 r = 1.2461 for 4000 r = 1.4918 for 1800 r = 1.1972
    int24 public tickRebalanceDeviation = 0; // 0 for lower bound as we rebalance the upper with the collateral ratio
    uint256 public percentFactor = 2500; // In this particular moment base.token0 = balanceOfDebtToken()
    ...
    constructor(address _uniV3Pool, address _vault) public {
        ...
        TOKEN_1 = vault.token();
        baseTicks = _getInitialTicks(); // NOTE ticks in that moment, after some time these are no longer valid
    }
    ...
    function balanceOfToken0() public view returns (uint256) {
        return IERC20(TOKEN_0).balanceOf(address(this)); // Matic
    }
    ...
    function rebalanceTrigger() public view returns (bool) {
        ...
        if (base.token0 < lendHelper.balanceOfDebtToken()) return true;
        if (_calcToken0Percent() < percentFactor) return true; // 2500 percentFactor
    ...
    function token0Price(bool _oracle) public view returns (uint256) {
        (, int24 tick, , , , , ) = uniV3Pool.slot0();
        uint256 price = _oracle ? aaveOracle.getAssetPrice(address(TOKEN_0)).div(100) : calcTickPrice(tick);
        return price;
    }
    ...
    function unwindAllAndSetTicks(int24 _newLowerTick, int24 _newUpperTick) external onlyAuthorized {
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) IERC20(TOKEN_0).safeTransfer(address(uniV3Pool), uint256(amount0Delta));
        if (amount1Delta > 0) IERC20(TOKEN_1).safeTransfer(address(uniV3Pool), uint256(amount1Delta));
        swapStorage = false;
    }
    ...
    function _setOrders() internal {
        ...
        _balanceProportion(baseTicks); // Here we check amounts and swap accordingly
        uint256 wethAmount = balanceOfToken0(); // Matic
        uint256 wantAmount = balanceOfToken1();
        _addLiquidity(wethAmount, wantAmount, baseTicks);
        _sendFundsToStrategy();
    }
    ...
    {
        (uint256 _amount0Min, uint256 _amount1Min) = _amountsForLiquidity(_liquidity, _ticks);
        (uint256 burned0, uint256 burned1) = uniV3Pool.burn(_ticks.lowerTick, _ticks.upperTick, _liquidity);
    ...
    function _sendFundsToStrategy() internal {
        uint256 token0 = balanceOfToken0(); // Matic
    ...
    /**
     * @dev Computes the token0 and token1 value for a given amount of liquidity.
     */
    ...
    /**
     * @dev Gets the liquidity for the available amounts of Token0 and Token1.
     */
    ...
    function _balanceProportion(Ticks memory _ticks) internal {
        // Initial case or case when tick has crossed the lower bound -> we swap half of the Matic
        if (balanceOfToken1() < wantDust) {
            _swap(balanceOfToken0().div(2), true);
        }
    ...
    function _estimateFees(Ticks memory _ticks, int24 _currentTick) internal view returns (uint256, uint256) {
        (
            uint128 liquidity,
            uint256 feeGrowthInside0Last,
            uint256 feeGrowthInside1Last,
        ...
        uint256 token1Fee =
            _computeFeesEarned(_currentTick, false, feeGrowthInside1Last, _ticks, liquidity) + owed1;
    ...
    function _calcToken0Percent() internal view returns (uint256) {
        Position memory base = _getPosition(baseTicks);
        if (base.liquidity == 0) return 0;
        uint256 token0In1 = token0To1(base.token0, false);
        uint256 principalInWant = token0In1.add(base.token1);
        return token0In1.mul(basisOne).div(principalInWant);
    }
    ...
    function setPercentFactor(uint256 _percentFactor) external onlyAuthorized {
        percentFactor = _percentFactor;
    }
}
