...
contract UnifarmAccountsUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function _isValidSender(address from) internal view {
        require(
            _msgSender() == from ||
    ...
    function _superAdminOrDappAdminOrAddedAdmin(bytes32 appID) internal view {
        address appAdmin = getDappAdmin(appID);
        require(
            _msgSender() == owner() ||
                _msgSender() == getSecondaryWalletAccount(owner()) ||
                _msgSender() == appAdmin ||
                _msgSender() == getSecondaryWalletAccount(appAdmin) ||
                accountRole[_msgSender()][appID] == 2,
            "INVALID_SENDER"
        );
    }
    ...
    function _superAdminOrDappAdminOrSendNotifRole(bytes32 appID) internal view {
        address appAdmin = getDappAdmin(appID);
        require(
            _msgSender() == owner() ||
                _msgSender() == getSecondaryWalletAccount(owner()) ||
                _msgSender() == appAdmin ||
                _msgSender() == getSecondaryWalletAccount(appAdmin) ||
                accountRole[_msgSender()][appID] == 1,
            "INVALID_SENDER"
        );
    }
    ...
    function addNewDapp(
        ...
        require(_appScreenshots.length < 7, "SURPASSED IMAGE LIMIT");
        ...
        GasNotZero(_msgSender(), isOauthUser) 
    {
        ...
        require(_appScreenshots.length < 7, "SS_LIMIT_EXCEED");
    ...
    // mapping(address => mapping(bytes32 => bool)) public hasPreviouslySubscribed;
    ...
    function subscribeToDapp(
        ...
        // // if ebook exists and user has not previously subscribed to the app
        // if(subscriptionStatus && bytes(ebooks[appID].title).length != 0 && ebooks[appID].isSendNotif 
        //     && !hasPreviouslySubscribed[user][appID] && dapps[appID].credits != 0) {
        //     _sendAppNotification(appID, user, message, buttonName, cta, isEncrypted);
        //     // _sendAppNotification(appID, user, notifParams[0], notifParams[1], notifParams[2], isEncrypted);

        //     hasPreviouslySubscribed[user][appID] = true;
        // }
    ...
    function updateEbook(
        ...
        bool _isAuthUser
    ) external superAdminOrDappAdminOrAddedAdmin(_appId) GasNotZero(_msgSender(), _isAuthUser) {
        uint256 gasLeftInit = gasleft();
	...
	//     string[] memory _files,  // [_assetFile, _assetSampleFile, _coverImage]
	//     bool _isSendNotif,
	//     bool _isShowApp,
	//     string[] memory _about,     // [_aboutCompany, _aboutImage]
    //     bool _isAuthUser
    // ) external superAdminOrDappAdmin(_appId) GasNotZero(_msgSender(), _isAuthUser) {
    //     uint256 gasLeftInit = gasleft();

    //     require(dapps[_appId].appAdmin != address(0), "INVALID DAPP ID");
    //     require(bytes(_title).length != 0, "EMPTY_TITLE");
    //     EbookDetails memory ebookDetails = EbookDetails({
    //         title: _title,
	//         summary: _summary,
	//         assetFile: _files[0],
	//         assetSampleFile: _files[1],
	//         coverImage: _files[2],
	//         isSendNotif: _isSendNotif,
	//         isShowApp: _isShowApp,
	//         aboutCompany: _about[0],
	//         aboutImage: _about[1]
    //     });
    //     ebooks[_appId] = ebookDetails;
    ...
    function sendAppNotification(
        ...
        require(isSubscribed[_appId][walletAddress], "NOT_SUBSCRIBED");
...
contract MessagingUpgradeable is Initializable, OwnableUpgradeable {
    ...
    function getSpamProtectionTokens() public view returns (address[] memory) {
        return spamProtectionTokens;
    }

    function getUserSpamTokens(
        address _user
    ) public view returns (SpamProtectionToken[] memory) {
        return userSpamTokens[_user];
    }
