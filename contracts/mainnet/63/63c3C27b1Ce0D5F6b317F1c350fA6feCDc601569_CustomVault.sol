...
/// Takes erc20(Jtry) as underlying token and mints X-tokens(xJTRY) . jTry and xJTRY is 1:1
    ...
    StandardAccessControlEnumerableUpgradeable
{
    ...
    /// @notice Role required by MarketplaceBorrower to borrow and repay xTokens against underlying
    ...
    /// @notice erc20 token that will be staked as underlying toke, (jtry in our case)
    address public stakeToken;

    /// @notice x-token to mint against underlying erc20 token
    ...
    event LentToMarketplaceBorrower(address mpBorrower, uint amount);
    event RepayedFromMPBorrower(address mpBorrower, uint amount);

    /**
     * @notice Initializes the contract.
     * @param _stakeToken erc20token(jtry) to be used as underlying token
     * @param _xToken address of xToken(xJtry) that will be minted or burned on _stakeToken deposit or withdrawal
     * @param _manager address Manager.sol
     */
    function initialize(
        address _stakeToken,
        address _xToken,
    ...
    /**
     * @notice transferFrom jTry and mints xJTRY to the caller
     * @param assets number of Jtry user wants to deposit or required xJtry
     * @return amount of xJtry minted to the caller
     */
    ...
    /**
     * @notice transferFrom jTry from caller and mints xJTRY to the receiver
     * @param assets number of Jtry user wants to deposit or required xJtry
     * @param receiver reciever of xJTRY
     * @return amount of xJtry minted to the reciever
     */
    function deposit(
        uint256 assets,
        address receiver
    ...
    /**
     * @notice helper to mint xJtry
     * @param _to address of xJTRY receiver
     * @param _amount amount of xJTRY to be minted to _to
     */
    ...
    /**
     * @notice helper to butn xJtry
     * @param _from address of which xJTRY are to be burned
     * @param _amount amount of xJTRY to be burned from _from
     */
    ...
    /**
     * @notice helper to butn xJtry
     * @param amountToWithdraw amount of underlying token to be withdrawed
     * @param receiver address to receive underlying erc20(jtry) tokens
     * @return amount of underlying erc20 token(jtry) withdrew
     */
    function withdraw(
        uint256 amountToWithdraw,
        address receiver
    ) public virtual returns (uint) {
        xToken.safeTransferFrom(_msgSender(), address(this), amountToWithdraw);
        _burn(_msgSender(), amountToWithdraw);
        MintableBurnableSyntheticTokenPermit(stakeToken).safeTransfer(
            receiver,
            amountToWithdraw
        );
        emit Withdraw(msg.sender, receiver, amountToWithdraw, amountToWithdraw);
    ...
    /**
     * @notice for Marketplace borrower to get Jtry in exchange of xJTRY
     * @param amount of Jtry asked by MarketplaceBorrower
     */
    function borrow(uint amount) external onlyMarketplaceBorrowerRole {
        ...
        emit LentToMarketplaceBorrower(msg.sender, amount);
    }

    /**
     * @notice for Marketplace borrower to get xJTRY in exchange of jTry
     * @param recepient reciveer of xJTRY
     * @param amount of xJTRY requested by MarketplaceBorrower
     */
    function repay(
        address recepient,
        uint amount
        ...
        emit RepayedFromMPBorrower(msg.sender, amount);
    }
