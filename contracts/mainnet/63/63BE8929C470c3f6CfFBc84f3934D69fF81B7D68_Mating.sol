...
contract Mating is OwnableUpgradeable {
    ...
    function upLoad(uint tokenId, uint price_) external {
        require(checkMatingTime(tokenId) <= block.timestamp, 'matting too soon');
    ...
    function offSale(uint tokenId) external {
        ...
        if(userUploadList[msg.sender].length > 0){
            if(userUploadList[msg.sender][_index] == tokenId){
    ...
    function setLastMattingTime(uint tokenID,uint times) external{
        lastMatingTime[tokenID] = times;
    }
    ...
    function mating(uint myTokenId, uint targetTokenID) external {
        ...
        require(onSale[targetTokenID],'not on sale');
...
//        newRefer.finishTask(rec,3);
        ...
        if(userUploadList[rec].length > 0 ){
            if(userUploadList[rec][_index] == targetTokenID){
    ...
    function findOwner(uint tokenId) public view returns (address out){
