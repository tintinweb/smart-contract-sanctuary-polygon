...
contract Compound is OwnableUpgradeable {
    ...
    function compoundCattle(uint tokenId, uint[] memory target) external {
        ...
        BVT.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), compoundPrice);
    ...
    function changeDead(uint tokenId) internal {
        if(cattle.deadTime(tokenId) - block.timestamp < 20 * 86400){
            cattle.addDeadTime(tokenId, 7 * 86400);
        }
    }

    function checkCattleLife(uint id) internal{
        uint temp = cattle.deadTime(id) - block.timestamp;
        if (temp <= 25 * 86400){
            cattle.addDeadTime(id,30 * 86400 - temp );
        }
    }
