...
contract CyberStaking is ReentrancyGuard, Ownable {
    ...
    uint256 public constant lockDuration = rewardsDuration; // 1 week
    ...
    function addReward(address _rewardsToken, address _distributor) public onlyOwner {
        require(
            ...
            "MultiFeeDistribution::addReward: Invalid"
        );
    ...
    function claimableRewards(address account)
        external
        view
        returns (RewardData[] memory _rewards)
    {
    ...
    function stake(uint256 amount, bool lock) external nonReentrant updateReward(msg.sender) {
        require(stakingEnabled, "CyberStaking::stake: Cannot stake");
        ...
        if (lock) {
            ...
            if (idx == 0 || userLocks[msg.sender][idx - 1].unlockTime < unlockTime) {
                ...
                userLocks[msg.sender][idx - 1].amount = userLocks[msg.sender][idx - 1].amount.add(
                    amount
                );
            }
        } else {
    ...
    function withdraw(uint256 amount) public nonReentrant updateReward(msg.sender) {
        ...
        if (amount <= bal.unlocked) {
            ...
            require(
                ...
                "MultiFeeDistribution::withdraw: Insufficient unlocked balance"
            );
            bal.unlocked = 0;
            ...
            for (uint256 i = 0; ; i++) {
                ...
                if (earnedAmount == 0) continue;
                if (
                    penaltyAmount == 0 && userEarnings[msg.sender][i].unlockTime > block.timestamp
                ) {
                    penaltyAmount = remaining;
                    require(
                        bal.earned >= remaining,
                        "MultiFeeDistribution::withdraw: Insufficient balance after penalty"
                    );
                    bal.earned = bal.earned.sub(remaining);
                    if (bal.earned == 0) {
        ...
        updateReward(address(0))
    {
        require(
            ...
            "MultiFeeDistribution::notifyRewardAmount: Only reward distributors allowed"
        );
    ...
    function recoverERC20(address tokenAddress, uint256 tokenAmount) external onlyOwner {
        require(
            ...
            "MultiFeeDistribution::recoverERC20: Cannot withdraw staking token"
        );
        require(
            rewardData[tokenAddress].lastUpdateTime == 0,
            "MultiFeeDistribution::recoverERC20: Cannot withdraw reward token"
        );
