...
contract TokenityCollection is
    Ownable,
    ERC721A,
    Withdraw,
    ReEntrancyGuard,
    ...
    Oracle
{
    ...
    constructor() ERC721A("TokenityCollection", "TKC") { }
    ...
    /// @notice Mint NFT with native token
    /// @dev Standard mint function with token nativo
    /// 430000000000000000
    /// @param _pairId Pair id
    function mintNativeToken(uint256 _pairId)
        external
        payable
        noReentrant
        returns (bool)
    {
        require(
            msg.value > 0,
            "Mint Native Token: Send BNB to buy some tokens"
        );

        /// @dev get the pair
        Pair storage pair = _PairstoSwap[_pairId];
        require(pair.active, "Mint Native Token: Pair is not active");

        /// @dev check pair is native
        require(pair.isNative, "Mint Native Token: Pair is not native");
        ...
        /// @dev Validate supply
        require(
            supply.add(_quantity) <= MAX_SUPPLY,
            "mintNativeToken: Can't mint more than max supply"
        );

        /// @dev max tokens for wallet
        require(
            _quantity > 0 && _quantity <= MAX_MINT_PER_TX,
            "Mint Native Token: Can only mint between 1 and 5000 tokens at once"
        );

        /// @dev Transfer NFT from contract to user wallet
        _safeMint(_msgSender(), _quantity);

        return true;
    }

    /// @notice buyWitkCustomToken function
    /// @dev Pay mint with custom token
    /// @param _amountTokens        Amount send for mint NFT
    /// _pair                       Pair of the token to buy
    function mintWithCustomToken(uint256 _pairId, uint256 _amountTokens)
        external
        noReentrant
        returns (uint256, uint256)
    {
        ...
        /// @dev Transfer token to the sender  =>  transfer from tokenA to contract
        require(
        ...
        /// @dev check if the user has enought tokens to buy the amount of token he wants to buy
        /// @dev and delete decimals from calculeted amount
        uint256 _quantity = valueInUsd.div(pair.price);
        ...
        require(
            supply.add(_quantity) <= MAX_SUPPLY,
            "Buy With Custom Token: Can't mint more than max supply"
        );

        /// @dev max tokens for wallet
        require(
            _quantity > 0 && _quantity <= MAX_MINT_PER_TX,
            "Buy With Custom Token: Can only mint between 1 and 5000 tokens at once"
        );

        /// @dev Transfer NFT from contract to user wallet
        _safeMint(_msgSender(), _quantity);

        return (_quantity, supply);
    }
...
contract Withdraw is Ownable {
    // @dev Withdrawal $NATIVE ONLY ONWER
    // @dev Allow the owner of the contract to withdraw NATIVE Owner
    function withdrawNativeOwner(uint256 amount, address to)
        external
        payable
        onlyOwner
        returns (bool)
    {
        require(
            ...
            "withdrawNativeOwner: Failed to transfer token to fee contract"
        );
        return true;
    }
