...
// File: contracts_ETH/main/VaultApeV2.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract VaultApeV2;
*/
...
contract VaultApeV2 is Ownable {
    using SafeERC20 for IERC20;
    ...
    Admin public admin;
    constructor (address _admin, address _safeHouse, address _ape) payable {
        ...
        admin = Admin(_admin);
        ape = _ape;
        apeRouter = IApeRouter(_ape);
        factory = apeRouter.factory();
        WETH = apeRouter.WETH();
    ...
    function addLiquidity(
       ...
       ERC20(tokenA).approve(ape,  amountADesired);
       ERC20(tokenB).approve(ape,  amountBDesired);
       (amountA,amountB,liquidity) = apeRouter.addLiquidity(tokenA,tokenB,amountADesired,amountBDesired,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    }
    ...
    function addLiquidityETH(
        ...
        ERC20(token).approve(ape, amountTokenDesired);
        require (address(this).balance >= amountETHDesired, "Formation.Fi: balance limit");
        (amountToken,amountETH,liquidity)= apeRouter.addLiquidityETH{value: amountETHDesired}(token,amountTokenDesired,amountTokenMin, amountETHMin, safeHouse, block.timestamp + duration);
    }
    ...
    function removeLiquidity(
       ...
       ERC20(_pair).approve(ape, liquidity);
      (amountA,amountB)= apeRouter.removeLiquidity(tokenA,tokenB,liquidity,amountAMin,amountBMin, safeHouse, block.timestamp + duration);
    ...
    function removeLiquidityETH(
        ...
        ERC20(_pair).approve(ape, liquidity);
        (amountToken,amountETH)= apeRouter.removeLiquidityETH(token, liquidity, amountTokenMin, amountETHMin, safeHouse, block.timestamp + duration);
