...
library QuickLibrary {
    ...
    function quote(uint amountA, uint reserveA, uint reserveB) internal pure returns (uint amountB) {
        require(amountA > 0, 'Quickswap: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'Quickswap: INSUFFICIENT_LIQUIDITY');
        amountB = amountA.mul(reserveB) / reserveA;
    }

    // given an input amount of an asset and pair reserves, returns the maximum output amount of the other asset
    function getAmountOut(uint amountIn, uint reserveIn, uint reserveOut) internal pure returns (uint amountOut) {
        require(amountIn > 0, 'Quickswap: INSUFFICIENT_INPUT_AMOUNT');
    ...
    function getAmountIn(uint amountOut, uint reserveIn, uint reserveOut) internal pure returns (uint amountIn) {
        require(amountOut > 0, 'Quickswap: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'Quickswap: INSUFFICIENT_LIQUIDITY');
...
contract QuickswapScamCheckBot {

    address internal constant ROUTER_ADDRESS = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address internal constant FACTORY_ADDRESS = 0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32;

    uint constant MAX_UINT = 2**96 - 1;
    ...
    function checkInternalFee(address tokenAddress) public payable authorized {
        ...
        amounts = QuickLibrary.getAmountsOut(FACTORY_ADDRESS, msg.value, path);
