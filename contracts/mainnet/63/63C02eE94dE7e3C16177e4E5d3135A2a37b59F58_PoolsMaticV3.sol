...
interface IPoolsMatic {
    function currentReward(uint pid, address user) external view returns(uint);
    function userTotalLock(address user) external view returns(uint);
    function volumeOntree(address user) external view returns(uint);
    function childs(address user) external view returns(uint, uint);
    function users(address user, uint pid) external view returns(uint, uint, uint, uint);
    function userClaimed(address user, uint pid, uint index) external view returns(uint, uint, uint, uint);
...
contract PoolsMaticV3 is Ownable, ReentrancyGuard {
    IPoolsMatic poolMatic = IPoolsMatic(0x849ACE7457cae40cd9B0e2C253bdb23357ecb523);
    ...
    mapping(address => uint) public userTotalTicket; // user => total ticket for mine TREND
    ...
    function setIsLP() external onlyGnosisSafe {
        isLP = !isLP;
    }
    function setMM(address mm) external onlyGnosisSafe {
        MM = mm;
    }
    function setTREND(address trend) external onlyGnosisSafe {
        require(!isLP, "PoolsMaticV3::setTREND: Disable add LP first");
        TREND = trend;
    }
    ...
    function claimReward(uint pid) public nonReentrant {
        ...
        if(reward > 0) {
            ...
            uint currentInterest = u.isMine ? p.currentInterestWithMine : p.currentInterest;
            userClaimed[_msgSender()][pid].push(Claim(getDays(), reward, users[_msgSender()][pid].totalLock, currentInterest));
    ...
    function getChildren(address user) public view returns (uint direct, uint downLine) {
        (direct, downLine) = poolMatic.childs(user);
    ...
    function getVolumeOnTre(address user) public view returns (uint volumn) {
        volumn = volumeOntree[user] + poolMatic.volumeOntree(user);
    }
    function getUserTotalLock(address user) public view returns (uint lock) {
        lock = userTotalLock[user] + poolMatic.userTotalLock(user);
    }
    ...
    function addLP() internal {
        remainComm[MM] += msg.value * 5 / 100;
        uint addAmount = msg.value * 20 / 100;
        if(!isLP) addLPAmount += addAmount;
        else {
            addAmount += addLPAmount;
            addLPAmount = 0;
            uint buyBack = addAmount / 2;
            address[] memory path = new address[](2);
            path[0] = wBnb;
            path[1] = TREND;
            uint[] memory amounts = IPancakeRouter(pancakeRouter).getAmountsOut(buyBack, path);
            pancakeRouter.swapExactETHForTokensSupportingFeeOnTransferTokens{value: buyBack}(amounts[1], path, address(this), block.timestamp);

            IERC20(TREND).approve(address(pancakeRouter), amounts[1]);
            pancakeRouter.addLiquidityETH{value: amounts[0]}(TREND, amounts[1], amounts[1]*9/100, amounts[0], gnosisSafe, block.timestamp);
    ...
    function deposit(uint pid, bool isMine) external payable {
        ...
        if(pid > 0 && isMine) {
            uint _t = msg.value;
            for(uint j = 1; j < pid; j++) {
                _t += _t * (j-1) / 4;
            }
            userTotalTicket[_msgSender()] += msg.value / _min ;
            totalTicket += userTotalTicket[_msgSender()];
        }
