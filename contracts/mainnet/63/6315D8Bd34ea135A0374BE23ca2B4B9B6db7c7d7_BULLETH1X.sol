...
contract BULLETH1X is ERC20Burnable, Operator {
    ...
    bytes4 private constant FUNC_SELECTOR_GET_INDEX_PRICE= bytes4(keccak256("getRealtimeTokenIndexPrice()"));
    bytes4 private constant FUNC_SELECTOR_GET_TWAP_PRICE= bytes4(keccak256("getTokenUpdatedPrice()"));
    ...
    function _getTokenTwapPrice() internal view returns (uint256 _tokenPrice) {
    ...
    function _getTokenRealtimeIndexPrice() internal view returns (uint) {
        return ITreasury(treasury).getRealtimeTokenIndexPrice();
    }

    function _getTokenRealtimePrice() internal view returns (uint) {
    ...
    function setBurnRate(uint256 _burnRate) external onlyOwner {
        require(_burnRate >= 0 && _burnRate <= 10000, "out of range");
        burnRate = _burnRate;
    }
    ...
    function calculateTax(uint amount) public returns (uint tax) {
        uint256 AfterSwapPrice = _getAfterSwapPrice(amount);
        IndexPrice = _getTokenRealtimeIndexPrice();
    ...
    //calculate tax 
    function _calculateTax(uint amount, uint _AfterSwapPrice, uint _IndexPrice) public pure returns (uint tax) {
    ...
    function calculateReward(uint amount) public returns (uint reward) {
        RealtimePrice = _getTokenRealtimePrice();
        IndexPrice = _getTokenRealtimeIndexPrice();
    ...
    //calculate reward
    function _calculateReward(uint amount, uint _RealtimePrice, uint _IndexPrice) public pure returns (uint reward) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if(_isContract(to)){
            if(autoCalculateTax && !isAddressExcluded(to)){
 ...
 /*       if(!autoCalculateTax){
            ...
            if(!isAddressExcluded(to)){
                if(_isContract(to)){
                    uint256 tax = 0;
                    AfterSwapPrice = _getAfterSwapPrice(amount);
                    if(AfterSwapPrice < IndexPrice){
                        tax = calculateTax(amount, AfterSwapPrice, IndexPrice);
                        amount = amount.sub(tax);
                    }
                    if(tax > 0){
                        super._transfer(from, treasury, tax);
