...
contract SwapGateway is ISwapGateway, UpgradeableBase {
    ...
    mapping(address => uint8) swapRouterVersion; // 0: unregistered, 2: Pancakeswap/UniswapV2, 3: UniswapV3
    ...
    function quoteExactInput(
        ...
        if (amountIn > 0) {
            ...
            if (version == 2) {
                address[] memory _path = new address[](path.length);
                for (i = 0; i < path.length; ) {
                    _path[i] = path[i] == ZERO_ADDRESS ? _wETH : path[i];
                    unchecked {
                        ++i;
                    }
                }
