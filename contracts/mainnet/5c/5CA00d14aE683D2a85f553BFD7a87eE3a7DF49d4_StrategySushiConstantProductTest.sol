...
contract StrategySushiConstantProductTest is StratFeeManager, GasFeeThrottler {
    ...
    function assignParams(address[] memory _route, address[] memory _pools, ITridentRouter.ExactInputParams storage _params) internal {
        ...
        for (uint i; i < poolLength;) {
            _params.path.push(ITridentRouter.Path(_pools[i], abi.encode(_route[i], address(this), true)));
            unchecked { ++i; }
        }
