...
//////////////////////////////////////////////////////////////////////////////////////////////////////
// @title: 0xLarvaLads
// On chain PFP collection of 5k unique profile images with the following properties:
...
//   - all 5,000 OnChain Larva Lads are unique
...
contract xLarvaLads is ERC721Enumerable, ReentrancyGuard, Ownable {
    ...
    function randomxLarvaLad(uint256 tokenId) internal view returns (xLarvaObject memory) {
        
        xLarvaObject memory xlarvaLad;
        xlarvaLad.baseColor = getBaseColor(tokenId);
        xlarvaLad.layerThree = getLayerThree(tokenId);
        xlarvaLad.layerFour = getLayerFour(tokenId);
        xlarvaLad.layerFive = getLayerFive(tokenId);
        xlarvaLad.layerSix = getLayerSix(tokenId);

        return xlarvaLad;
    }
    
    function getTraits(xLarvaObject memory xlarvaLad) internal view returns (string memory) {
        
        string[20] memory parts;
        
        parts[0] = ', "attributes": [{"trait_type": "Type","value": "';
        if (xlarvaLad.layerThree == 3) {
            parts[1] = 'Zombie"}, {"trait_type": "Mouth","value": "Zombie"},'; 
        }
        if (xlarvaLad.layerThree == 4) {
            parts[2] = 'Alien"}, {"trait_type": "Mouth","value": "Alien"},'; 
        }
        if (xlarvaLad.layerThree == 5) {
            parts[3] = 'Ape"}, {"trait_type": "Mouth","value": "Ape"},'; 
        }
        if (xlarvaLad.layerThree < 3 || xlarvaLad.layerThree > 5) {
            parts[4] = 'Normal"}, {"trait_type": "Mouth","value": "';
            parts[5] = thirdNames[xlarvaLad.layerThree];
            parts[6] = '"},';
        }
        parts[7] = ' {"trait_type": "Eyewear","value": "';
        parts[8] = fourthNames[xlarvaLad.layerFour];
        parts[9] = '"}, {"trait_type": "Headwear","value": "';
        parts[10] = fifthNames[xlarvaLad.layerFive];
        parts[11] = '"}, {"trait_type": "Accessory","value": "';
        parts[12] = sixthNames[xlarvaLad.layerSix];
        parts[13] = '"}], ';
    ...
    function getSVG(xLarvaObject memory xlarvaLad) internal view returns (string memory) {
        string[9] memory parts;

        parts[0] = '<svg id="x" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="xMinYMin meet" viewBox="0 0 30 30"><path fill="#ffd430" d="M0 0h30v30H0z"/>';
        parts[1] = '<path fill="';
        parts[2] = baseColors[xlarvaLad.baseColor];
        ...
        parts[4] = thirdLayers[xlarvaLad.layerThree];
        parts[5] = fourthLayers[xlarvaLad.layerFour];
        parts[6] = fifthLayers[xlarvaLad.layerFive];
        parts[7] = sixthLayers[xlarvaLad.layerSix];
    ...
    function tokenURI(uint256 tokenId) override public view returns (string memory) {
        xLarvaObject memory xlarvaLad = randomxLarvaLad(tokenId);
        string memory json = Base64.encode(bytes(string(abi.encodePacked('{"name": "0xLarva Lad #', toString(tokenId), '", "description": "0xLarva Lads. The artwork and metadata are fully on-chain and were randomly generated at mint."', getTraits(xlarvaLad), '"image": "data:image/svg+xml;base64,', Base64.encode(bytes(getSVG(xlarvaLad))), '"}'))));
    ...
    constructor() ERC721("0xLarva Lads", "0xLARVA") Ownable() {}
}