...
contract StupidWars{
    ...
    modifier onlyPlayer() {
        bool  checkIfPlayerInList = false;
        uint256 node;
        if(playerList.nodeExists(uint256(uint160(msg.sender)))){
            checkIfPlayerInList = true;
        }
        else{
            checkIfPlayerInList = false;
        }
        require(checkIfPlayerInList, "Permission denied.");
        _;
    }
    ...
    StructuredLinkedList.List playerList;
    address public owner;
    bytes32 private EthmessageHash;
   
    constructor() payable{
        owner = msg.sender;
        EthmessageHash = 0xdbefb542f18f140edfb526ebffd37a552a2b069a116d71f8c8a8444b1077d633;
    }

    receive() external payable{}

    function withdraw(uint256 money) onlyPlayer external{
        require(address(this).balance >= money, "Rejected");
        address payable Receiver = payable(msg.sender);
        Receiver.transfer(money);
        removePlayer();
    }

    function ownerWithdraw(uint amount) onlyOwner external{
        address payable Receiver = payable(msg.sender);
        Receiver.transfer(amount);
    }


    function addPlayer(bytes memory _sig) public payable{
        if(recover(EthmessageHash, _sig) == owner)
            playerList.pushBack(uint256(uint160(msg.sender)));
        else
            emit error("Invalid Operation");
    }

    function removePlayer() internal returns(uint){
        return playerList.remove(uint256(uint160(msg.sender)));
    }

    function recover(bytes32 _ethSignedMassageHash, bytes memory _sig)
        public pure returns(address)
    {
        (bytes32 r, bytes32 s, uint8 v) = _split(_sig);
        return ecrecover(_ethSignedMassageHash, v, r, s);
    }   
