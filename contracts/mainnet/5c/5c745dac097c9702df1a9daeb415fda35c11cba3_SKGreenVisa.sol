...
contract SKGreenVisa is ERC721Enumerable, Ownable {
    using Strings for uint256;

    uint256 private constant MAX_SUPPLY = 1000;
    uint256 private constant MAX_PER_MINT = 50;

    string private baseURI;

    constructor(string memory initBaseURI)
        ERC721("Savanna Kidz Green Resident Visa", "SKGRV")
    {
        baseURI = initBaseURI;
    }
    ...
    function mint(address[] calldata addresses, uint256 amount)
        external
        onlyOwner
    {
        uint256 totalAmount = addresses.length * amount;
        require(totalAmount > 0, "Need at least 1 token");
        require(totalAmount <= MAX_PER_MINT, "50 tokens per call max");
        require(
