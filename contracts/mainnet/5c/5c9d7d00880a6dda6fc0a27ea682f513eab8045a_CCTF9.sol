...
contract CCTF9 {
  ...
  modifier onlyActive {
    require(block.timestamp >= volStart && !paused, "CCTF not started or paused");
    _;
  }
  ...
  function setCCTFStatus(bool started) external onlyAdmin {
    paused = !started;
    if(volStart == 0) { 
      volStart = block.timestamp;
      emit CCTFStarted(volStart);
    }
  }

  function setVolStartNow() external onlyAdmin {
      volStart = block.timestamp;
  }

  function setPause(bool _val) external onlyAdmin {
      paused = _val;
  }
...
////////// Submit flags
    ...
    function SubmitFlag(bytes32 _message, bytes memory signature, uint256 _submitFor) external{
...
////////// Check status, scores, etc
  ...
  // Return player count (verified/unverified/banned)
  // Return 
}