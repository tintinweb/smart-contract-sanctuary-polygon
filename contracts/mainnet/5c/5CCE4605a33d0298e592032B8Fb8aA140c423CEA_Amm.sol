...
contract Amm is IAmm, PerpFiOwnableUpgrade, BlockContext {
    ...
    event QuoteAssetRepeg(uint256 quoteAssetReserve, uint256 baseAssetReserve);
    event BaseAssetRepeg(uint256 quoteAssetReserve, uint256 baseAssetReserve);
    event KRepeg(uint256 quoteAssetReserve, uint256 baseAssetReserve);
    ...
    modifier onlyRepegBot() {
        require(repegBot == _msgSender() || owner() == _msgSender(), "caller is not repegBot");
        _;
    }
    ...
    IERC20 public deductionToken;
    Decimal.decimal public deductionPercent;
    Decimal.decimal public deductionThreshold;
    ...
    function setQuoteAssetReserve(uint256 _reserve) public onlyRepegBot {
        quoteAssetReserve = Decimal.decimal(_reserve);
        emit QuoteAssetRepeg(quoteAssetReserve.toUint(), baseAssetReserve.toUint());
    }

    function setBaseAssetReserve(uint256 _reserve) public onlyRepegBot {
        baseAssetReserve = Decimal.decimal(_reserve);
        emit BaseAssetRepeg(quoteAssetReserve.toUint(), baseAssetReserve.toUint());
    }

    function setK(uint256 _multiplier) public onlyOwner {
        quoteAssetReserve = quoteAssetReserve.mulD(Decimal.decimal(_multiplier));
        baseAssetReserve = baseAssetReserve.mulD(Decimal.decimal(_multiplier));
        emit KRepeg(quoteAssetReserve.toUint(), baseAssetReserve.toUint());
    }

    function setRepegBot(address _repegBot) public onlyOwner {
        repegBot = _repegBot;
    }
    ...
    function setDeductionToken(IERC20 _token) public onlyOwner {
        deductionToken = _token;
    }

    function setDeductionCriteria(Decimal.decimal memory _deductionPercent, Decimal.decimal memory _deductionThreshold)
        public
        onlyOwner
    {
        deductionPercent = _deductionPercent;
        deductionThreshold = _deductionThreshold;
    }

    /**
     * @notice calculate total fee (including toll and spread) by input quoteAssetAmount
     * @param _quoteAssetAmount quoteAssetAmount
     * @param _deductFees true if want to have a fee discount
     * @return total tx fee
     */
    function calcFee(
        Decimal.decimal calldata _quoteAssetAmount,
        address _trader,
        bool _deductFees
    ) external view override returns (Decimal.decimal memory, Decimal.decimal memory) {
        if (_quoteAssetAmount.toUint() == 0) {
            return (Decimal.zero(), Decimal.zero());
        }
        Decimal.decimal memory _tollRatio = tollRatio;
        Decimal.decimal memory _spreadRatio = spreadRatio;
        if (_deductFees) {
            require(address(deductionToken) != address(0), "deduction token not set");
            require(deductionPercent.toUint() != 0, "deduction fees not set");
            if (deductionToken.balanceOf(_trader) >= deductionThreshold.toUint()) {
                _tollRatio = tollRatio.mulD(Decimal.one().subD(deductionPercent));
                _spreadRatio = spreadRatio.mulD(Decimal.one().subD(deductionPercent));
            }
        }
        return (_quoteAssetAmount.mulD(_tollRatio), _quoteAssetAmount.mulD(_spreadRatio));
    }
    ...
    function setQuoteAsset(IERC20 _asset) public onlyOwner {
        quoteAsset = _asset;
    }
}
...
interface IAmm {
    ...
    function calcFee(
        ...
        address _trader,
        bool _deductFees
    ) external view returns (Decimal.decimal memory, Decimal.decimal memory);
...
contract TraderWallet {
    ...
    function openPosition(
        ...
        clearingHouse.openPosition(_amm, _side, _quoteAssetAmount, _leverage, _minBaseAssetAmount, _deductFees);
    }
    ...
    function closePosition(Amm _amm, bool _deductFees) external {
        clearingHouse.closePosition(_amm, Decimal.zero(), _deductFees);
    }
    ...
    function multiActions(
        ...
        executeAction(_action1, _amm, _side, _quoteAssetAmount, _leverage, _baseAssetAmountLimit, _trader, _deductFees);
        if (_setRestriction) {
            clearingHouse.mockSetRestrictionMode(_amm);
        }
        executeAction(_action2, _amm, _side, _quoteAssetAmount, _leverage, _baseAssetAmountLimit, _trader, _deductFees);
    }
    ...
    function executeAction(
        ...
        if (_action == ActionType.OPEN) {
            clearingHouse.openPosition(_amm, _side, _quoteAssetAmount, _leverage, _baseAssetAmountLimit, _deductFees);
...
contract ClearingHouse is DecimalERC20, OwnerPausableUpgradeSafe, ReentrancyGuardUpgradeSafe, BlockContext {
     ...
     * init margin ratio:
     * - set at 10%
     * - if you have 100 collateral, open a position worth 1000 i.e 10x lev
     *   so 100 / 1000
     * maintenance margin ratio:
     * - set at 6.25%
     * - % margin you should maintain
     * - with 100 collateral, and total size 1000, your margin is 10%
     * - if size reduces 6.25%, you will be liquidated
     * liquidation fee ratio:
     * - % of fee to go to liquidators
     */
    function initialize(
...
contract ChainlinkPriceFeed is IPriceFeed, PerpFiOwnableUpgrade, BlockContext {
    ...
    bytes32 internal _specId;
    uint256 public lastRequestTimestamp;
    mapping(bytes32 => AmmRequester) public requests;
    uint256 public requestInterval;
    uint256 internal _price;
    uint256 internal _payment;
    address internal _uc;
    address public oracleBot;
    address public manualBot;
    mapping(bytes32 => uint256) public manualPrice;
    enum PriceMode {
        Oracle,
        Manual
    }
    ...
    function initialize(address uc_) public initializer {
        __Ownable_init();
        _uc = uc_;
    }

    modifier onlyOracleBot() {
        require(msg.sender == oracleBot || msg.sender == owner(), "caller != oracleBot");
        _;
    }

    modifier onlyManualBot() {
        require(msg.sender == manualBot || msg.sender == owner(), "caller != oracleBot");
        _;
    }
    ...
    function addRequester(
        ...
        requests[priceFeedKey] = AmmRequester(specId_, payment_, _assetId, "", "");
    }

    function updatePriceOracle(bytes32 priceFeedKey) public onlyOracleBot {
        requests[priceFeedKey].previousRequestId = requests[priceFeedKey].latestRequestId;
        bytes32 specId = requests[priceFeedKey].specId;
        uint256 payment = requests[priceFeedKey].payment;
        uint256 assetId = requests[priceFeedKey].assetId;
        (bool success, bytes memory data) = _uc.call(
            abi.encodeWithSignature("requestPrice(bytes32,uint256,uint256)", specId, payment, assetId)
        );
        require(success, "requestPrice failed");
        bytes32 requestId = abi.decode(data, (bytes32));
        requests[priceFeedKey].latestRequestId = requestId;
    }

    function updatePriceManual(bytes32 priceFeedKey, uint256 _price) public onlyManualBot {
        manualPrice[priceFeedKey] = _price;
    }

    function getPrice(bytes32 _priceFeedKey) public view override returns (uint256 price) {
        if (priceMode == PriceMode.Manual) {
            price = manualPrice[_priceFeedKey];
        } else {
            require(requests[_priceFeedKey].latestRequestId != "", "no requestId");
            price = _getPrice(requests[_priceFeedKey].latestRequestId);
            if (price == 0) {
                price = _getPrice(requests[_priceFeedKey].previousRequestId);
            }
        }
    }

    function _getPrice(bytes32 _requestId) internal view returns (uint256 price) {
        (bool success, bytes memory data) = _uc.staticcall(
            abi.encodeWithSignature("requestIdResult(bytes32)", _requestId)
        );
        require(success, "requestIdResult failed");
        price = abi.decode(data, (uint256));
    }

    function getTwapPrice(bytes32 _priceFeedKey, uint256 _interval) public view override returns (uint256) {
        return getPrice(_priceFeedKey);
    }

    function setSpecId(bytes32 specId_) public onlyOwner {
        _specId = specId_;
    }

    function setPayment(uint256 payment_) public onlyOwner {
        _payment = payment_;
    }
    ...
    function setPriceMode(PriceMode _priceMode) public onlyOwner {
        priceMode = _priceMode;
    }

    function setUC(address uc_) public onlyOwner {
        _uc = uc_;
    }
}
