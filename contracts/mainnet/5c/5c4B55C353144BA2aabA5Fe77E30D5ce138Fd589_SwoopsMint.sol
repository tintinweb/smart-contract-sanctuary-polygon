    ...
    Ownable
{
    ...
    constructor(string memory baseURI) ERC721("Clams", "CLAMS") {
        _uri = baseURI;
    }
    ...
    /// Set the single baseURI for all tokens to use.
    ...
    /// Pause all EIP-721 contract operations.
    /// @dev only callable by the contract owner.
    function pause() public onlyOwner {
        _pause();
    }

    /// Unpause the EIP-721 contract operations.
    /// @dev only callable by the contract owner.
    function unpause() public onlyOwner {
        _unpause();
    }

    /// Mint a Swoops token to the supplied address.
    /// @dev only callable by the contract owner.
    /// @param to The address where the newly created token should be sent.
    function safeMint(address to) public onlyOwnerOrMintingContract {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        _safeMint(to, tokenId);
    }

    /// Set the contract that is allowed to call this contract directly to mint.
    /// @dev only callable by the contract owner.
    /// @param contractAddress The new address of the contract that is allowed to invoke this EIP-721.
    function setMintingContract(address contractAddress) public onlyOwner {
        _mintingContract = contractAddress;
    }

    /// Set the default royalty amount for transactions exchanging tokens for currency. NOTE: defining
    ...
    /// @notice No-op. Overriding renounceOwnership() to prevent the ability 
    ///         to renounce ownership of this contract.
    /// @dev callable only by the contract owner, but does nothing.
...
contract SwoopsMint is Ownable {
    ...
    /// @notice Withdraw
    /// @dev callable only by the contract owner, but does nothing.
    /// @param token only by the contract owner, but does nothing.
    function withdrawToken(address token) public onlyOwner returns (bool) {
        ...
        return foreignToken.transfer(msg.sender, amount);
    }
