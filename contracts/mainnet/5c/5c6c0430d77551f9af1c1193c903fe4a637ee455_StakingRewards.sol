...
contract StakingRewards is Ownable {
    ...
    function rewardPerToken(uint256 userTime) public view returns (uint) {
        ...
        return
            ...
            ((block.timestamp - lastUpdateTime - userTime) * rewardRate * 1e18) ;
    }

    function earned(address account) public view returns (uint) {
        return
            ((_balances[account] *
                (rewardPerToken(userLastUpdateTime[account]) - userRewardPerTokenPaid[account])) / 1e18) +
            rewards[account];
    }

    modifier updateReward(address account) {
        rewardPerTokenStored = rewardPerToken(userLastUpdateTime[account]);
