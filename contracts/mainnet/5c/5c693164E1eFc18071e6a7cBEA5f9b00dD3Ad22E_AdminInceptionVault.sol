...
contract AdminInceptionVault is IAdminInceptionVault, BoringOwnable, Initializable, ReentrancyGuard {
  ...
  EnumerableSet.AddressSet private _collaterals;
  ...
  **/
  function depositETH() external payable override onlyOwner {
    IVaultsCore vaultsCore = _a.core();
    _addCollateralType(address(vaultsCore.WETH()));
    vaultsCore.depositETH{ value: msg.value }();
  }

  /**
    Wraps ETH and deposits WETH into the vault of the msg.sender as collateral and borrows the specified amount of 
    tokens in WEI
  ...
  **/
  function depositETHAndBorrow(uint256 _borrowAmount) external payable override onlyOwner {
    IVaultsCore vaultsCore = _a.core();
    _addCollateralType(address(vaultsCore.WETH()));
    vaultsCore.depositETHAndBorrow{ value: msg.value }(_borrowAmount);
  }

  /**
    Deposit an ERC20 token into the vault of the msg.sender as collateral and borrows the specified amount of 
    tokens in WEI
  ...
  **/
  function depositAndBorrow(
    ...
    IERC20 collateralType = IERC20(_collateralType);
    IVaultsCore vaultsCore = _a.core();
    _addCollateralType(_collateralType);
    collateralType.safeTransferFrom(msg.sender, address(this), _depositAmount);
    collateralType.safeIncreaseAllowance(address(vaultsCore), _depositAmount);
    vaultsCore.depositAndBorrow(_collateralType, _depositAmount, _borrowAmount);
  }
  ...
  **/
  function deposit(address _collateralType, uint256 _amount) external override onlyOwner {
    IERC20 collateralType = IERC20(_collateralType);
    IVaultsCore vaultsCore = _a.core();
    collateralType.safeTransferFrom(msg.sender, address(this), _amount);
    collateralType.safeIncreaseAllowance(address(vaultsCore), _amount);
    _addCollateralType(_collateralType);
    vaultsCore.deposit(_collateralType, _amount);
  }
  ...
  **/
  function borrow(uint256 _vaultId, uint256 _amount) external override onlyOwner {
  ...
  **/
  function withdraw(uint256 _vaultId, uint256 _amount) external override onlyOwner {
    ...
    a_.core().withdraw(_vaultId, _amount);
    IERC20 collateralType = IERC20(a_.vaultsData().vaultCollateralType(_vaultId));
    collateralType.safeTransfer(msg.sender, _amount);
  }

  /**
    Withdraws ETH from a vault.
    @dev Only the owner of a vault can withdraw ETH from it.
    `withdrawETH()` will fail if it would bring the vault below the minimum collateralization threshold.
    @param _vaultId the ID of the vault from which to withdraw the collateral.
    @param _amount the amount of ETH to be withdrawn in WEI. 
   */
  function withdrawETH(uint256 _vaultId, uint256 _amount) external override onlyOwner nonReentrant {
    _a.core().withdrawETH(_vaultId, _amount);
  ...
  */
  function claimMimo() external override {
    IDebtNotifier debtNotifier_ = _debtNotifier;
    for (uint256 i; i < _collaterals.length(); i++) {
      IGenericMiner supplyMiner = IGenericMiner(
        address(debtNotifier_.collateralSupplyMinerMapping(_collaterals.at(i)))
      );
  ...
  /**
    ...
    @dev This function can only be called by the InceptionVaultsCore.
  ...
  */
  function transferPar(uint256 _amount, address _to) external override onlyOwner {
    IERC20 stablex = IERC20(address(_a.stablex()));
    stablex.safeTransfer(_to, _amount);
  }
  ...
  function getDebtNotifier() external view override returns (IDebtNotifier) {
    return _debtNotifier;
  }

  function getWeth() external view override returns (IWETH) {
    return _weth;
  }

  function getMimo() external view override returns (IERC20) {
    return _mimo;
  }

  function getInceptionCore() external view override returns (IInceptionVaultsCore) {
    return _inceptionCore;
  }

  function getCollaterals() external view override returns (bytes32[] memory) {
    return _collaterals._inner._values;
  }

  function _addCollateralType(address _collateralType) internal {
    if (!_collaterals.contains(_collateralType)) {
      _collaterals.add(_collateralType);
    }
  }
}
...
interface IInceptionVaultsCore {
  ...
  event Initialized(VaultConfig vaultConfig);
