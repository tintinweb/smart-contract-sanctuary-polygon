 ...
 */
contract FirstSaleJenCi is Ownable {
  ...
  uint256 private PERCENT_PROMOTER = 20;
  uint256 private PERCENT_OPENART = 20;
  uint256 private PERCENT_CREATOR = 60;
  ...
  function getProfits(address[] memory currencies) external onlyMembers nonReentrant {
    ...
    for (uint256 i; i < currencies.length; ) {
      ...
      if (PROMOTER != address(0)) _transfer(currencies[i], PROMOTER, amount1);
      if (OPENART != address(0)) _transfer(currencies[i], OPENART, amount2);
      if (CREATOR != address(0)) _transfer(currencies[i], CREATOR, amount3);
