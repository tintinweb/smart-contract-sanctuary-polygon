...
contract MagpieCore is ReentrancyGuardUpgradeable, OwnableUpgradeable, UUPSUpgradeable, Pausable, IMagpieCore {
    ...
    mapping(IMagpieDataTransfer.DataTransferType => mapping(uint8 => mapping(uint64 => bool))) public sequencesV2;
    ...
    function _getWrapSwapConfig(IMagpieRouter.SwapArgs memory swapArgs, bool transferFromSender)
        private
        view
        returns (WrapSwapConfig memory wrapSwapConfig)
    {
    ...
    function _wrapSwap(IMagpieRouter.SwapArgs memory swapArgs, WrapSwapConfig memory wrapSwapConfig)
        private
        ...
        )
    {
        ...
        (depositAmount, coreSequence, tokenSequence) = IMagpieBridge(config.magpieBridgeAddress).bridgeIn{
            value: bridgeFee
        }(args.bridgeType, args.dataTransferType, args.payload, amountOut, toAssetAddress, msg.sender);
    ...
    function swapOut(SwapOutArgs calldata args)
        ...
        returns (uint256[] memory amountOuts)
    {
        (IMagpieDataTransfer.ValidationOutPayload memory payload, uint64 coreSequence) = IMagpieDataTransfer(
            config.magpieDataTransferAddress
        ).getPayload(args.bridgeArgs.encodedVmCore);

        require(
            !sequencesV2[payload.dataTransferType][payload.senderNetworkId][coreSequence],
            "MagpieCore: already used sequence"
        );

        sequencesV2[payload.dataTransferType][payload.senderNetworkId][coreSequence] = true;
