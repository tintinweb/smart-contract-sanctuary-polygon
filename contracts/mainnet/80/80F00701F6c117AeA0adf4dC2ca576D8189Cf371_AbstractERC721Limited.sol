...
interface IAbstractToken {
  event Reify(AbstractTokenMessage message);
  event Dereify(AbstractTokenMessage message);
...
contract AbstractERC721 is IERC165, AbstractToken, IAbstractERC721, ERC721 {
  constructor(
    string memory _name,
  ...
  function id(AbstractTokenMessage calldata message) public pure returns (uint256) {
    ...
    // metadata for ERC721 tokens includes the id and uri
    return uint256(bytes32(message.meta[0:32]));
  }

  function uri(AbstractTokenMessage calldata message) public pure returns (string calldata) {
    require(_validMeta(message.meta), 'invalid metadata');
    return string(message.meta[32:]);
  }

  // transforms token from message to contract
  function _reify(AbstractTokenMessage calldata message) internal override(AbstractToken) virtual {
    // no permission checking! Callers must validate the message!
    _mint(message.owner, id(message));
  }

  // transforms token from contract to message
...
import { IAbstractToken, AbstractTokenMessage } from '../interfaces/IAbstractToken.sol';
import { AbstractToken } from './AbstractToken.sol';
import { AbstractERC721 } from './AbstractERC721.sol';

contract AbstractERC721Limited is AbstractERC721 {
  ...
  constructor(
    ...
    address _signer,
    uint256 _max
  ) AbstractERC721(_name, _symbol, _signer) {
    max = _max;
  }
    
  function _reify(AbstractTokenMessage calldata message) override internal {
    // checks
...
abstract contract AbstractToken is IAbstractToken, Ownable, GenericEIP712 {
    ...
    bytes32 internal constant TYPE_HASH = keccak256("AbstractTokenMessage(uint256 chainId,address implementation,address owner,uint256 nonce,bytes meta)");
