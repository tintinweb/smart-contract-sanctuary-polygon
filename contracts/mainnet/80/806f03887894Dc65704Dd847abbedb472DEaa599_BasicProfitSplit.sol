...
contract BasicProfitSplit is Initializable,PausableUpgradeable{
    ...
    function withdrawProfits() public{
        ...
        for(uint256 i = 0; i < revenueTokens.length; i++){
            IERC20 token = IERC20(revenueTokens[i]);
            uint256 balance = token.balanceOf(address(this));
            uint256 percentage = 1000 / recipients.length; 

            for(uint256 j = 0; j < recipients.length; j++){
                token.transfer(recipients[j], balance.mul(percentage).div(1000));
            }
        }            
    }
}
