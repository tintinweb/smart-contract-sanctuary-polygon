...
contract GopherMinesStaking is IERC721Receiver, Ownable {
    ...
    struct StakedNFT {
        ...
        uint256 unStakeTimestamp; // the time that the NFT would be unsaked
    }
...
//map all added nfts
    mapping(string => uint) public nftIdToDUration;
    mapping(bytes32 => uint) public stakeDuration;
    ...
    function _generateStakeId(
        ...
        /**
     * @dev allows adds an nft and duation to stake to the contract
     *
     * Requirements:
     *
    ...
    function addNFT( string memory _id,  uint _duration) public onlyOwner {
        nftIdToDUration[_id] = _duration;
    }
    ...
    function stake(
        ...
        stakedNFTs[stakeId] = StakedNFT({
            ...
            tokenId: tokenId,
            unStakeTimestamp: block.timestamp + _duration
        });

        // add the stake ID to the user's tracking
        userStakes[_msgSender()].add(stakeId);

        // add the duration to the stake ID
        stakeDuration[stakeId] = _duration;
    ...
    function unstake(bytes32 stakeId) public {
        ...
        if(owner() != _msgSender()){
        require(stakedNFTs[stakeId].unStakeTimestamp < block.timestamp, "stake period not over");
        }
     ...
     */
    function permittedNfts() public view returns (address[] memory) {
