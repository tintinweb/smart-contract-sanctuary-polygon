...
contract PriceOracleKeeper is KeeperCompatibleInterface {
    ...
    event FailedSettlementPrice(address underlyingToken, address priceToken, uint256 settlementTimestamp);
    ...
    function isClaimRequiredAmm(address amm) 
        public view returns(bool claimRequired) 
    {
        ...
        claimRequired = false; 
        if (amm != address(0)) {
            uint64[] memory allSeriesCall = IMinterAmm(amm)
                .getAllSeries();
