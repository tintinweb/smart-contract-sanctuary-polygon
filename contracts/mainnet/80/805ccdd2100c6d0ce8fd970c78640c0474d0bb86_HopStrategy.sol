...
interface IJar {
    function token() external view returns (IERC20); //this should be changed to `want()` to match IStrategy
...
contract HopStrategy is BaseStrategyStakingRewards {
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if(amountHarvested > 0) {
            ISwapRouter.ExactInputSingleParams memory params = ISwapRouter.ExactInputSingleParams(harvestedToken, underlying, address(this), now.add(60), amountHarvested, 1, 0);
            ISwapRouter(currentRouter).exactInputSingle(params);
        }
