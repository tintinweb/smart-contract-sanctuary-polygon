...
import "./interfaces/IStrategAssetBuffer.sol";
/**
 * @dev Implementation of the ERC4626 "Tokenized Vault Standard" as defined in
 * https://eips.ethereum.org/EIPS/eip-4626[EIP-4626].
 *
 ...
 * _Available since v4.7._
 */
abstract contract StrategERC4626Upgradeable is Initializable, ERC20Upgradeable, IERC4626Upgradeable {
    using MathUpgradeable for uint256;

    IStrategAssetBuffer private _buffer;
     ...
     * @dev Set the underlying asset contract. This must be an ERC20-compatible contract (ERC20 or ERC777).
     */
    function __ERC4626_init(IERC20Upgradeable asset_, address buffer_) internal onlyInitializing {
        __ERC4626_init_unchained(asset_, buffer_);
    }

    function __ERC4626_init_unchained(IERC20Upgradeable asset_, address buffer_) internal onlyInitializing {
        (bool success, uint8 assetDecimals) = _tryGetAssetDecimals(asset_);
        _underlyingDecimals = success ? assetDecimals : 18;
        _asset = asset_;
        _buffer = IStrategAssetBuffer(buffer_);
    }
    ...
    function _deposit(address caller, address receiver, uint256 assets, uint256 shares) internal virtual {
        ...
        _buffer.putInBuffer(address(_asset), assets);
    ...
    function _withdraw(
        ...
        // shares are burned and after the assets are transferred, which is a valid state.
        _burn(owner, shares);
        SafeERC20Upgradeable.safeTransferFrom(_asset, address(_buffer), receiver, assets);
 ...
 */
contract StrategVault is ERC20Upgradeable, ERC20PermitUpgradeable, StrategERC4626Upgradeable, StrategVaultManagement, ReentrancyGuard {
    using SafeERC20 for IERC20;

    struct LargeWithdrawParams {
        address user;
        uint256 amount;
    }

    uint256 constant BUFFER_DERIVATION = 500;

    address immutable buffer;
    ...
    uint256 private bufferPercent;
    uint256 private lastFeeHarvestIndex;
    uint256 private nativeTVL;
    uint256 private lastNativeTVLUpdate;

    uint256 private stratBlocksLength;
    mapping(uint256 => address) private stratBlocks;

    uint256 private harvestBlocksLength;
    mapping(uint256 => address) private harvestBlocks;

    uint256 private oracleBlocksLength;
    mapping(uint256 => address) private oracleBlocks;
    ...
    event StrategyInitialized(
        ...
        address[] _oracleBlocksIndex,
        bytes[] _oracleBlocksParameters
    );
     ...
     * @param _bufferPercent Percentage of the buffer
     * @param _creatorFees Creator fees where 100% is 10000
     * @param _harvesterFees Harvester fees where 100% is 10000
     */
    function initialize(
        address _owner,
        address _erc3525,
        ...
        address _asset,
        MIDDLEWARE_STRATEGY _strategy,
        uint256 _bufferPercent,
        uint256 _creatorFees,
        uint256 _harvesterFees
    ) external initializer {
        __ERC20_init(_name, _symbol);
        __ERC20Permit_init(_name);
        __ERC4626_init(IERC20Upgradeable(_asset), buffer);
        __StrategVaultManagement_init(
            _erc3525,
            _strategy,
            _creatorFees,
            _harvesterFees
        ...
        erc3525 = _erc3525;
        creatorFees = _creatorFees; // 10000 = 100%
        harvestFees = _harvesterFees; // 10000 = 100%
        bufferPercent = _bufferPercent; // 10000 = 100%
        
        lastFeeHarvestIndex = 10000;
        factory = msg.sender;

        if(_strategy == MIDDLEWARE_STRATEGY.WHITELISTED) {
            isWhitelisted[_owner] = true;
        }

        IERC20(_asset).safeApprove(buffer, type(uint256).max);
    }
    ...
    function setStrat(
        ...
        uint256 _blocksToVerifyLength = _stratBlocks.length + _harvestBlocks.length + _oracleBlocks.length;
        ...
        stratBlocksLength = uint256(_stratBlocks.length);
        ...
        harvestBlocksLength = uint256(_harvestBlocks.length);

        for (uint256 i = 0; i < _oracleBlocks.length; i++) {
            oracleBlocks[i] = _oracleBlocks[i];
            _blocksToVerify[_blocksToVerifyCurrentIndex] = _oracleBlocks[i];
            _blocksToVerifyCurrentIndex = _blocksToVerifyCurrentIndex + 1;

            LibBlock.setupOracleBlockData(i, _oracleBlocksParameters[i]);
        }
        oracleBlocksLength = uint256(_oracleBlocks.length);
    ...
    {
        uint256 _stratBlocksLength = stratBlocksLength;
        uint256 _harvestBlocksLength = harvestBlocksLength;
        uint256 _oracleBlocksLength = oracleBlocksLength;
        ...
        for (uint256 i = 0; i < _stratBlocksLength; i++) {
            ...
            _stratBlocksParameters[i] = LibBlock.getStrategyStorageByIndex(i);
        }
        ...
        for (uint256 i = 0; i < _oracleBlocksLength; i++) {
            _oracleBlocks[i] = oracleBlocks[i];
            _oracleBlocksParameters[i] = LibBlock.getOracleStorageByIndex(i);
        }
    }

    function partialStrategyEnter(
        uint256 _from,
        uint256[] memory _dynParamsIndex, 
        bytes[] memory _dynParams
    ) external onlyPositionManager {
        _storeDynamicParams(_dynParamsIndex, _dynParams);
        for (uint256 i = _from; i < stratBlocksLength; i++) {
            LibBlock.executeStrategyEnter(stratBlocks[i], i);
        }
        _purgeDynamicParams(_dynParamsIndex);
    }

    function partialStrategyExit(
        uint256 _to,
        uint256[] memory _dynParamsIndex, 
        bytes[] memory _dynParams
    ) external onlyPositionManager {
        ...
        for (uint256 i = 0; i < stratBlocksLength; i++) {
            if(revertedIndex - i < _to) break;
            uint256 index = revertedIndex - i;
            LibBlock.executeStrategyExit(stratBlocks[index], index, 10000);
        }
        _purgeDynamicParams(_dynParamsIndex);
    }

    function _fetchNativeTVL(bool _force) internal returns (uint256) {
        if(lastNativeTVLUpdate == block.timestamp && !_force)
            return nativeTVL;
    ...
    function _getNativeTVL() internal view returns (uint256) {
        ...
        IStrategOracleBlock.OracleResponse memory _tmp;
        ...
        if (oracleBlocksLength == 0) {
            ...
            _tmp = LibBlock.getOracleExit(_tmp, oracleBlocks[0], 0);
        } else {
            uint256 revertedIndex = oracleBlocksLength - 1;
            for (uint256 i = 0; i < oracleBlocksLength; i++) {
                uint256 index = revertedIndex - i;
                _tmp = LibBlock.getOracleExit(_tmp, oracleBlocks[index], index);
            }
        }
        ...
        tvl += IERC20(_asset).allowance(buffer, address(this));
        return tvl;
    }
    ...
    function _exitStrategy(
        ...
        if (oracleBlocksLength == 1) {
            LibBlock.executeStrategyExit(stratBlocks[0], 0, _percent);
        } else {
            uint256 revertedIndex = stratBlocksLength - 1;
            for (uint256 i = 0; i < stratBlocksLength; i++) {
                uint256 index = revertedIndex - i;
                if(i == 0)
                    LibBlock.executeStrategyExit(stratBlocks[index], index, _percent);
                else 
                    LibBlock.executeStrategyExit(stratBlocks[index], index, 10000);
            }
        }
    ...
    function _harvestFees() private {
        ...
        uint256 _creatorFee = creatorFees;
        uint256 _harvestFee = harvestFees;
        ...
        lastFeeHarvestIndex =
            currentVaultIndex -
            ((lastFeeHarvestIndexDiff * (_creatorFee + harvestFees + _protocolFee)) / 10000);
        ...
        _asset.approve(erc3525, creatorFeeAmount);
        ...
        returns (uint256)
    {
        ...
        _incrementValueDeposited(sender, assets);
    ...
    function withdraw(
        ...
        uint256 shares = previewWithdraw(assets);
        _withdraw(_msgSender(), receiver, owner, assets, shares);
        _decreaseValueDeposited(_msgSender(), assets);
    ...
    function withdrawalRebalance(
        ...
        IERC20 _asset = IERC20(asset());
        uint256 _percent = _amount * 10000 / _fetchNativeTVL(false);
        /**
         * Exit strategy funds
         */
        _exitStrategy(_percent);
    ...
    /**
     * @dev Buffer rebalancing proccess
     */
    function _rebalance(
        uint256[] memory _dynParamsIndexEnter, 
        bytes[] memory _dynParamsEnter,
        ...
        uint256 currentBufferPercent = _bufferSize * 10000 / _nativeTVL;
        /**
         * Buffer oversized
         */
        if(currentBufferPercent > bufferPercent + BUFFER_DERIVATION) {
            _storeDynamicParams(_dynParamsIndexEnter, _dynParamsEnter);
            uint256 amountToDeposit = _nativeTVL * (currentBufferPercent - bufferPercent) / 10000;
        ...
        if(currentBufferPercent < bufferPercent - BUFFER_DERIVATION) {
            _storeDynamicParams(_dynParamsIndexExit, _dynParamsExit);
            _exitStrategy(bufferPercent - currentBufferPercent);
            uint256 amountToBuffer = _nativeTVL * (bufferPercent - currentBufferPercent) / 10000;
            IStrategAssetBuffer(buffer).putInBuffer(asset(), amountToBuffer);
            _storeDynamicParams(_dynParamsIndexEnter, _dynParamsEnter);
            _enterInStrategy();
        }
    ...
    function emergencyExecution(
        ...
        for (uint i = 0; i < _targets.length; i++) {
            ...
            if (!success) 
               revert EmergencyExecutionReverted(_targets[i], _data);
        }
    }
}
...
import "./interfaces/IStrategVault.sol";
import "./interfaces/IStrategERC3525.sol";
...
abstract contract StrategVaultManagement {
    ...
    uint256 immutable MIN_CREATOR_FEE = 100;
    uint256 immutable MIN_HARVEST_FEE = 50;
    uint256 immutable MAX_CREATOR_FEE = 2500;
    uint256 immutable MAX_HARVEST_FEE = 500;

    enum MIDDLEWARE_STRATEGY {
        PUBLIC,
        WHITELISTED,
        HOLDER
    }
    ...
    MIDDLEWARE_STRATEGY public strategy;
    uint256 private limitMode;

    bool timelockEnabled; // is timelock enabled
    uint256 timelockDuration; // timelock duration in second
    mapping(address => uint256) timelocks; // last deposit timestamp

    uint256 userMinDeposit; // minimum deposit from user if he want to enter in a strategy
    uint256 userMaxDeposit; // maximum deposit from user if he want to enter in a strategy

    uint256 vaultMinDeposit; // minimum deposit
    uint256 vaultMaxDeposit; // maximum deposit 
    ...
    event MiddlewareInit(address _erc3525, MIDDLEWARE_STRATEGY strategy);
    event NewTimelockParams(bool enabled, uint256 duration);
    event NewDepositLimits(uint256 minUserDeposit, uint256 maxUserDeposit,uint256 minVaultDeposit, uint256 maxVaultDeposit);
    event NewHoldingParams(address token, uint256 holdAmount);
    event NewWhitelistedAddresses(address[] addresses);
    event RemoveWhitelistedAddresses(address[] addresses);
    event NewFeeParams(uint256 creatorFees, uint256 harvestFees);
     ...
     * @param _erc3525 Address of vault's ERC3525
    ...
    function __StrategVaultManagement_init(
        ...
        _verifyFeeParams(_creatorFees, _harvestFees);

        strategy = _strategy;
        erc3525 = _erc3525;
        creatorFees = _creatorFees;
        harvestFees = _harvestFees;
        emit MiddlewareInit(_erc3525, _strategy);
    }
    ...
    function _applyMiddleware(uint256 _amount, uint256 _vaultTvl) internal view {
        MIDDLEWARE_STRATEGY s = strategy;

        //Check tvl limits
        _applyTvlLimits(_amount, _vaultTvl);

        //Execute middle strategy
        if(s == MIDDLEWARE_STRATEGY.PUBLIC)
            return;
        if(s == MIDDLEWARE_STRATEGY.WHITELISTED) {
            _applyWhitelisted();
        }
        if(s == MIDDLEWARE_STRATEGY.HOLDER) {
            _applyHolder();
        }
    } 
    ...
    function _applyTvlLimits(uint256 _amount, uint256 _vaultTvl) internal view {
        ...
        if(mode % 2 == 1) {
            ...
            if(userDeposited == 0) {
                ...
                if(userDeposited + _amount < minDeposit) revert MinDepositNotReached();
            }
        }

        //Vault max limit set
        if(mode >= 4) 
            if(_vaultTvl + _amount > vaultMaxDeposit) revert MaxVaultDepositReached();

        //User max limit set
        if(mode % 4 > 1) 
            if(userDeposited + _amount > userMaxDeposit) revert MaxUserDepositReached();
    }
    ...
    function _applyTimelock() internal view {
        ...
        if(timelocks[msg.sender] + timelockDuration > block.timestamp) revert TimelockNotReached();
    }
    ...
    function setDepositLimits(
        ...
        if(_minUserDeposit > 0)
            mode += 1;

        if(_maxUserDeposit > 0)
            mode += 2;
        
        if(vaultMaxDeposit > 0)
            mode += 4;
    ...
    function whitelist(address[] memory addresses) external onlyOwner {
        ...
        emit NewWhitelistedAddresses(addresses);
    }

    /**
     * @notice Removes the given addresses from the whitelist.
     * @param addresses Array of addresses to be removed from the whitelist
     */
    function unwhitelist(address[] memory addresses) external onlyOwner {
        for (uint i = 0; i < addresses.length; i++) 
            isWhitelisted[addresses[i]] = false;
        
        emit RemoveWhitelistedAddresses(addresses);
    }
     ...
     */
    function setTimelockParams(bool _enabled, uint256 _duration) external onlyOwner {
        timelockEnabled = _enabled;
        timelockDuration = _duration;

        emit NewTimelockParams(_enabled, _duration);
    }
    ...
    function _resetTimelock(address _user) internal {
        if(timelockEnabled)
            timelocks[_user] = block.timestamp;
    }
     ...
     */
    function setHoldingParams(address _token, uint256 _amount) external onlyOwner {
        token = IERC20(_token);
        holdAmount = _amount;

        emit NewHoldingParams(_token, _amount);
    }
     ...
     */
    function setFeeParams(uint256 _creatorFees, uint256 _harvestFees) external onlyOwner {
        _verifyFeeParams(_creatorFees, _harvestFees);
        creatorFees = _creatorFees;
        harvestFees = _harvestFees;

        emit NewFeeParams(_creatorFees, _harvestFees);
    }

    function _verifyFeeParams(uint256 _creatorFees, uint256 _harvestFee) internal pure {
        if(_creatorFees < MIN_CREATOR_FEE && _creatorFees > MAX_CREATOR_FEE) revert BadCreatorFee();
        if(_harvestFee < MIN_HARVEST_FEE && _harvestFee > MAX_HARVEST_FEE) revert BadHarvestFee();
    }
...
interface IStrategyBlockRegistry {
    ...
    function addBlocks(address[] memory _blocks, string[] memory _names, string[] memory _ipfsHash) external;
...
interface IStrategVaultFactory {
    ...
    event NewVaultImplementation(
        uint256 indexed version,
        address implementation
    );

    event NewERC2535Implementation(
        uint256 indexed version,
        address implementation
    );
...
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "../interfaces/IStrategOracleBlock.sol";
import "hardhat/console.sol";
...
library LibBlock {
    ...
    bytes32 constant ORACLE_BLOCKS_STORAGE_POSITION = keccak256("oracle.blocks.strateg.io");
    ...
    function executeStrategyEnter(address _block, uint256 _index) internal {
        (bool success, bytes memory _data) = _block.delegatecall(
        ...
        if (!success) 
            revert StrategyEnterReverted(_block, _index, _data);
    }
    ...
    function executeStrategyExit(address _block, uint256 _index, uint256 _percent) internal {
        ...
        if (!success) 
            revert StrategyExitReverted(_block, _index, _data);
    }
    ...
    function executeHarvest(address _block, uint256 _index) internal {
        ...
        if (!success) 
            revert HarvestReverted(_block, _index, _data);
    }

    /**
     * Oracle part
     */
    function oracleBlocksStorage() internal pure returns (BlocksStorage storage ds) {
        bytes32 position = ORACLE_BLOCKS_STORAGE_POSITION;
        assembly {
            ds.slot := position
        }
    }

    function getOracleStorageByIndex(uint256 _index) internal view returns (bytes memory) {
        BlocksStorage storage store = oracleBlocksStorage();
        return store.storagePerIndex[_index];
    }

    function setupOracleBlockData(uint256 _index, bytes memory _data) internal {
        BlocksStorage storage store = oracleBlocksStorage();
        store.storagePerIndex[_index] = _data;
    }

    function getOracleEnter(IStrategOracleBlock.OracleResponse memory _before, address _block, uint256 _index) internal view returns (IStrategOracleBlock.OracleResponse memory) {
        return IStrategOracleBlock(_block).oracleEnter(_before, getOracleStorageByIndex(_index));
    }

    function getOracleExit(IStrategOracleBlock.OracleResponse memory _before, address _block, uint256 _index) internal view returns (IStrategOracleBlock.OracleResponse memory) {
        return IStrategOracleBlock(_block).oracleExit(_before, getOracleStorageByIndex(_index));
    }

    /**
     * Oracle response functions
     */
    function findTokenAmount(address _token, IStrategOracleBlock.OracleResponse memory _res) internal pure returns (uint256) {
    ...
    function addTokenAmount(address _token, uint256 _amount, IStrategOracleBlock.OracleResponse memory _res) internal pure returns (IStrategOracleBlock.OracleResponse memory) {
        for (uint i = 0; i < _res.tokens.length; i++) {
            if(_res.tokens[i] == _token) {
    ...
    function removeTokenAmount(address _token, uint256 _amount, IStrategOracleBlock.OracleResponse memory _res) internal pure returns (IStrategOracleBlock.OracleResponse memory) {
        for (uint i = 0; i < _res.tokens.length; i++) {
            if(_res.tokens[i] == _token) {
                _res.tokensAmount[i] -= _amount;
                return _res;
