...
contract UniV2AndV3PriceGetter {
    ...
    address public constant FACTORYV3 = 0x1F98431c8aD98523631AE4a59f267346ea31F984; //FactoryV3
    ...
    //Token addresses
    address constant WETH = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2;
    address constant USDC = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174; //0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48;
    address constant USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F; //0xdAC17F958D2ee523a2206206994597C13D831ec7;
    address constant DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063; //0x6B175474E89094C44Da98b954EedeAC495271d0F;
    ...
    function getETHPriceV2() public view returns (uint256 price, uint256 wbnbTotal) {
        ...
        if (USDC < WETH) (wbnbReserve0, udscReserve) = (udscReserve, wbnbReserve0);
        (uint256 wbnbReserve2, uint256 usdtReserve, ) = IApePair(pairFor(WETH, USDT)).getReserves();
        if (USDT < WETH) (wbnbReserve0, usdtReserve) = (usdtReserve, wbnbReserve0);

        wbnbTotal = wbnbReserve0 + wbnbReserve1 + wbnbReserve2;
        uint256 usdTotal = daiReserve + udscReserve + usdtReserve;

        price = (usdTotal * PRECISION) / wbnbTotal;
    }
    ...
    function pairFor(
        ...
        pair = IUniswapV3Factory(FACTORYV3).getPool(tokenA, tokenB, fee);
    }

    function pairFor(address tokenA, address tokenB) private view returns (address pair) {
        pair = IUniswapV2Factory(FACTORYV2).getPair(tokenA, tokenB);
    }
}
