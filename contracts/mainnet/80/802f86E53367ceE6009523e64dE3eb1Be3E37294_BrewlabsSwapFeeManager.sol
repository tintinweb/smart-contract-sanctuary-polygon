...
contract BrewlabsSwapFeeManager is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 private constant MAX_LP_FEE = 200; // 2%
    uint256 private constant MAX_BREWLABS_FEE = 10; // 0.1%
    uint256 private constant MAX_TOKEN_OWNER_FEE = 1000; // 10%
    uint256 private constant MAX_STAKING_FEE = 1000; // 10%
    uint256 private constant MAX_REFERRAL_FEE = 1000; // 10%
    ...
    modifier onlyPair(address pair) {
        require(msg.sender == pair, 'BrewlabsFeeManager: ONLY LP PAIR CAN ACCESS');
        _;
    }
    ...
    function getPoolFeeInfo(address pair) external view returns (PoolFeeInfo memory) {
        return
            PoolFeeInfo(
                pools[pair].token0,
                pools[pair].token1,
                pools[pair].tokenOwner,
                pools[pair].referrer,
                pools[pair].feeDistribution
            );
    }
    ...
    function lpMinted(
        ...
        address token1,
        address pair
    ) external validPair(pair, token0, token1) onlyPair(pair) {
    ...
    function lpTransferred(
        ...
        require(lpBalanceBeforeTransfer > lpBalanceAfterTransfer, 'BrewlabsFeeManager: INVALID TRANSFER');
        {
            ...
            address _to = to;
            address _pair = pair;
            uint256 movingReward0 = pools[_pair]
                .lpStakingInfo
                .rewardsTable0[_from]
                .mul(lpBalanceBeforeTransfer - lpBalanceAfterTransfer)
                .div(lpBalanceBeforeTransfer);
            pools[_pair].lpStakingInfo.rewardsTable0[_from] -= movingReward0;
            pools[_pair].lpStakingInfo.rewardsTable0[_to] += movingReward0;
            uint256 movingReward1 = pools[_pair]
                .lpStakingInfo
                .rewardsTable1[_from]
                .mul(lpBalanceBeforeTransfer - lpBalanceAfterTransfer)
                .div(lpBalanceBeforeTransfer);
            pools[_pair].lpStakingInfo.rewardsTable1[_from] -= movingReward1;
            pools[_pair].lpStakingInfo.rewardsTable1[_to] += movingReward1;
        }
    }
    ...
    function claimAll(address[] calldata brewlabs_pairs) external nonReentrant {
        ...
        for (uint256 i; i < brewlabs_pairs.length; i++) {
            _claim(brewlabs_pairs[i]);
        }
    }

    function _setFeeDistribution(address pair, bytes memory feeDistribution) internal {
        (uint256 _lpFee, uint256 _brewlabsFee, uint256 _tokenOwnerFee, uint256 _stakingFee, uint256 _referralFee) = abi
            .decode(feeDistribution, (uint256, uint256, uint256, uint256, uint256));
        require(_lpFee <= MAX_LP_FEE, 'BrewlabsFeeManager: INVALID LP FEE VALUE');
        require(_brewlabsFee <= MAX_BREWLABS_FEE, 'BrewlabsFeeManager: INVALID BREWLABS FEE VALUE');
        ...
        if (_stakingFee > 0 && IBrewlabsPair(pair).stakingPool() == address(0)) {
            revert('BrewlabsFeeManager: STAKING POOL NOT DEFINED YET');
        }

        uint256 _totalFee = _lpFee + _brewlabsFee + _tokenOwnerFee + _stakingFee + _referralFee;
        IBrewlabsPair(pair).setFeePercent(uint16(_totalFee * 100));
    ...
    function _claimLPFee(address pair, address to) internal {
        _updateStakedLP(pair, to);
        (RewardAmount memory lpRewards, ,) = pendingRewards(pair, to);
        if (lpRewards.amount0 > 0 || lpRewards.amount1 > 0) {
            _updateLPRewardsTable(pair);
        ...
        returns (RewardAmount memory lpRewards, RewardAmount memory ownerRewards, RewardAmount memory referralRewards)
    {
        address token0 = pools[pair].token0;
        address token1 = pools[pair].token1;
        uint256 lpBalance = IBrewlabsPair(pair).balanceOf(claimer);
        ...
        if (lpBalance > 0) {
            ...
            uint256 leftOver1 = balance1 - lpStakingInfo.accRewards1;
            {
                ...
                lpRewards = lpStakingInfo.lpSupply == 0
                    ? RewardAmount(0, 0)
                    : RewardAmount(
                        lpStakingInfo.rewardsTable0[_claimer] + leftOver0.mul(lpBalance).div(lpStakingInfo.lpSupply),
                        lpStakingInfo.rewardsTable1[_claimer] + leftOver1.mul(lpBalance).div(lpStakingInfo.lpSupply)
                    );
            }
        }
        if (claimer == pools[pair].tokenOwner) {
            ownerRewards = RewardAmount(
                pools[pair].balanceOfTokenOwner[token0],
                pools[pair].balanceOfTokenOwner[token1]
            );
        }
        if (claimer == pools[pair].referrer) {
            referralRewards = RewardAmount(
                pools[pair].balanceOfReferral[token0],
                pools[pair].balanceOfReferral[token1]
            );
        }
    }
}
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"bd469e65b7b5ff7ee645ff5bb70fb02fb316e278345dd78d40abc850bc450473" // init code hash
                    )
                )
            )
        );
