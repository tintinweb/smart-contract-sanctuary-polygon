...
// File contracts/FriendsOfXEN.sol
...
contract FriendsOfXEN is Context, IERC20, IERC20Metadata, Ownable {
    ...
    constructor() {
        _name = "Friends of XEN";
        _symbol = "FoXEN";
        _fee = 15;
        _feeExcluded[_msgSender()] = true;
        _feeAddress = _msgSender();
        _mint(_msgSender(), 100000000000 ether);
    }
    ...
    function _transfer(
        ...
        unchecked {
            ...
            if (feeAmount > 0) _balances[_feeAddress] += feeAmount;
        }

        emit Transfer(from, to, transferAmount);
        if (feeAmount > 0) emit Transfer(from, _feeAddress, feeAmount);
