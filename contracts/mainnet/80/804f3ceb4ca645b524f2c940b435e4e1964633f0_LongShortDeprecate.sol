...
import "../../interfaces/ISyntheticTokenOriginal.sol";
import "../../interfaces/IStaker.sol";
import "../../interfaces/ILongShortOriginal.sol";
import "../../interfaces/IYieldManagerOriginal.sol";
import "../../../interfaces/IOracleManager.sol";
import "../../../abstract/AccessControlledAndUpgradeable.sol";
import "../../../GEMS.sol";

interface IStakerExit {
  function withdrawAllUser(uint32 marketIndex, address user) external;
}
...
contract LongShortDeprecate is ILongShortOriginal, AccessControlledAndUpgradeable {
    ...
    virtual
  {
    ...
    ISyntheticTokenOriginal(syntheticTokens[marketIndex][true]).mint(
      PERMANENT_INITIAL_LIQUIDITY_HOLDER,
      initialMarketSeedForEachMarketSide
    );
    ISyntheticTokenOriginal(syntheticTokens[marketIndex][false]).mint(
      PERMANENT_INITIAL_LIQUIDITY_HOLDER,
      initialMarketSeedForEachMarketSide
    );
  ...
  */
  ...
  /// @param - The market for which to execute the function for.
  /// @param - A scalar base e18 for the funding rate.
  /// @param - Side with more liquidity.
  /// @param - Side with less liquidity.
  ...
  function _claimAndDistributeYieldThenRebalanceMarket(uint32 marketIndex)
    internal
    virtual
    returns (uint256 longValue, uint256 shortValue)
  {
    ...
    // uint256 treasuryYieldPercent_e18 = 0; // Rounding errors were giving over-flow - setting this to 99.999999999999999% (very close to 100%) to avoid this.
    uint256 treasuryYieldPercent_e18 = 99999999999999999; // Rounding errors were giving over-flow - setting this to 99.999999999999999% (very close to 100%) to avoid this.
  ...
  function _updateSystemStateInternal(uint32 marketIndex)
    ...
    requireMarketExists(marketIndex)
  {
    int256 newAssetPrice = assetPrice[marketIndex];
...

    {
      ...
      ) = _claimAndDistributeYieldThenRebalanceMarket(marketIndex);

      uint256 syntheticTokenPrice_inPaymentTokens_long = _getSyntheticTokenPrice(
        newLongPoolValue,
        ISyntheticTokenOriginal(syntheticTokens[marketIndex][true]).totalSupply()
      );
      uint256 syntheticTokenPrice_inPaymentTokens_short = _getSyntheticTokenPrice(
        newShortPoolValue,
        ISyntheticTokenOriginal(syntheticTokens[marketIndex][false]).totalSupply()
      );
  ...
  /// @param - An uint32 which uniquely identifies a market.
  /// @param - Amount of payment tokens in that token's lowest denominationfor which to mint synthetic assets at next price.
  /// @param - Whether the mint is for a long or short synth.
    ...
    gemCollecting
  {
    ...
    ISyntheticTokenOriginal(syntheticTokens[marketIndex][isLong]).transferFrom(
      msg.sender,
      address(this),
      tokens_redeem
    );
  ...
  /// @param - An uint32 which uniquely identifies a market.
  /// @param - Amount in wei of synthetic tokens to shift from the one side to the other at the next oracle price update.
  /// @param - Whether the token shift is from long to short (true), or short to long (false).
  ...
  function _executeOutstandingNextPriceMints(
    ...
    if (currentPaymentTokenDepositAmount > 0) {
      ...
      ISyntheticTokenOriginal(syntheticTokens[marketIndex][isLong]).transfer(
        user,
  ...
  function _executeOutstandingNextPriceTokenShifts(
    ...
    if (syntheticToken_toShiftAwayFrom_marketSide > 0) {
      ...
      require(
        ISyntheticTokenOriginal(syntheticTokens[marketIndex][!isShiftFromLong]).transfer(
          user,
  ...
  function _handleChangeInSyntheticTokensTotalSupply(
    ...
    if (changeInSyntheticTokensTotalSupply > 0) {
      ISyntheticTokenOriginal(syntheticTokens[marketIndex][isLong]).mint(
        address(this),
        uint256(changeInSyntheticTokensTotalSupply)
      );
    } else if (changeInSyntheticTokensTotalSupply < 0) {
      ISyntheticTokenOriginal(syntheticTokens[marketIndex][isLong]).burn(
        uint256(-changeInSyntheticTokensTotalSupply)
      );
    }
  }
  ...
  */
  ...
  // This function isn't gas optimized - it is CORRECTNESS & SIMPLICITY optimised.
  //    For example - this function updates system state more than it needs to. However that isn't expensive since much of its core has been stripped out.
  function exitEverything(uint32 marketIndex) public {
    IStakerExit(staker).withdrawAllUser(marketIndex, msg.sender);

    // LONG
    uint256 tokens_redeem_long = IERC20(syntheticTokens[marketIndex][true]).balanceOf(msg.sender);
    if (tokens_redeem_long > 0) {
      _redeemNextPrice(marketIndex, tokens_redeem_long, true);
    }

    // SHORT
    uint256 tokens_redeem_short = IERC20(syntheticTokens[marketIndex][false]).balanceOf(msg.sender);
    if (tokens_redeem_short > 0) {
      _redeemNextPrice(marketIndex, tokens_redeem_short, false);
    }

    // Settle everything for the user!
    _updateSystemStateInternal(marketIndex);
    _executeOutstandingNextPriceSettlements(msg.sender, marketIndex);
  }

  function exitEverythingMulti(uint32[] calldata marketIndecies) public {
    for (uint256 index = 0; index < marketIndecies.length; index++) {
      exitEverything(marketIndecies[index]);
    }
  }
}
