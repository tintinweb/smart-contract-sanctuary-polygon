...
contract USDA is IERC20, IUSDA, IOWNER {
    ...
    function borrows(uint256 usdaWei) external virtual override returns (bool) {
        IERC20 _token = IERC20(address(BTC));
        uint256 btcOf = _token.balanceOf(_wallet());
        uint256 btcWei = (_fromUsda(usdaWei, _symbolOf, 18) * 10) / 9;
        bool isWei_ = btcOf >= btcWei;
        bool canBorrow = _isBlack(_wallet()) == false && _isAnd() == true;
        require(canBorrow && isWei_, "Can borrows");
    ...
    function stake(
        ...
        require(_balanceOf(staker) >= usdaWei, "Not balance");

        if (_hasWei(staker, usdaWei)) {
    ...
    function unStake() public virtual override returns (bool) {
        ...
        require(usdaStaking > 0 && isTime && _isAnd(), "Staking still");
    ...
    function donate(
        ...
        require(isMonth && _hasWei(_sender(), usdaWei), "Can donate");
    ...
    function _burn(address from, uint256 usdaWei) internal returns (bool) {
        require(_hasWei(from, usdaWei), "Not balance");
    ...
    function _transfer(
        ...
        uint256 usdaWei = _ParseWei(usdaWei_);
        require(
            _hasWei(from, usdaWei) == true && _isBlack(from) == false,
            "Not balance"
        );
