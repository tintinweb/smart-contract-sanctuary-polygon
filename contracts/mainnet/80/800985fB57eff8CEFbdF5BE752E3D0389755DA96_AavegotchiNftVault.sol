...
contract AavegotchiNftVault is OriumNftVault {
    ...
    function endRental(address _nftAddress, uint32 _tokenId) external onlyScholarshipManager {
        LibAavegotchiNftVault.endRental(_nftAddress, _tokenId, address(factory));
    }
...
library LibAavegotchiNftVault {
    ...
    function endRentalAndRelist(
        ...
        if (_data.length == 0) {
            IGotchiLendingFacet(_rentalImplementation).claimAndEndAndRelistGotchiLending(_tokenId);
        } else {
        ...
        )
    {
        ...
        require(
            shares.length == IOriumFactory(_factory).getPlatformSharesLengthOfEventId(_platform, 1),
