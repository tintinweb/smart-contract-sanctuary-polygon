...
/**
 * @dev Implementation of the BeefyAvatars art storage contract. It contains the image data and metadata for all traits.
 * The storage is separate from the rest of the logic due to contract deploy size constrains. Data could have been added after deploy, 
 * but we wanted to keep everything in the constructor to increase auditability by less technical users.
 */
contract Art {
    struct Trait {
        string traitId;
    ...
    constructor () {
        ...
        art.body.push(Trait("body", "BDSM", "QmVWTMTLve4sf425rFcrRZEQ79hRLtRgrtNTEMgZZubvm5", 4));
        ...
        art.body.push(Trait("body", "Chicken suit", "QmNzvoa8SkyybiEzMod4xChmGQ3hhVwFJ1YB1Aevo6ifWj", 23));
        ...
        art.body.push(Trait("body", "Blue jacket", "QmP5kJ7jg3sQW96p94iTiTzfWgg4m75GrVzoU4ChmuGGmP", 46));
    ...
    /**
     * @dev Returns data and metadata for all traits of a cow based on its dna. The dna is derived from 
     * a random number delivered by Chainlink VRF.
     * @param dna Array of 7 numbers from 0 to 999
     */
    function cow(uint16[7] memory dna) external view returns (Trait[7] memory) {
        return [
            _cowBackground(dna[0]),
            _cowBody(dna[1]),
            _cowHair(dna[2]),
            _cowEyes(dna[3]),
            _cowAccsBody(dna[4]),
    ...
    /**
     * @dev Returns the correct background info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowBackground(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct body info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowBody(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct hair info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowHair(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct eyes info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowEyes(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct body accesory info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowAccsBody(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct face accesory info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowAccsFace(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
    ...
    /**
     * @dev Returns the correct hat info for a given gene. Genes should be random numbers returned by the VRF.
     * @param gene number from 0 to 999
     */
    function _cowHat(uint16 gene) internal view returns (Trait memory) {
        uint16 current = 0;
