...
contract NFTERC721A is Ownable, ERC721A, PaymentSplitter {
    ...
    uint private constant MAX_PUBLIC = 1800;
    uint private constant MAX_GIFT = 0;

    uint public wlSalePrice = 1 ether;
    uint public publicSalePrice = 3 ether;

    bytes32 public merkleRoot;

    uint public saleStartTime = 1667482505;

    mapping(address => uint) public amountNFTsperWalletWhitelistSale;

    uint private teamLength;

    constructor(address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRoot, string memory _baseURI) ERC721A("Skelbones", "SKEL")
