...
contract Polytomic {
    ...
    function _calculateWeeklyReward(uint256 totalDays) private view returns(uint256){
        uint256 rewardUser;
        for(uint256 i = 0; i < investors.length; i++){
            User storage user = users[investors[i]];
    ...
    function updateReward(uint256 totalDays) external onlyInitiator{
        ...
        uint256 rewardLength = _calculateWeeklyReward(totalDays);
