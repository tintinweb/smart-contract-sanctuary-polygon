...
contract Dali_NFT_Contract is Ownable, Pausable, ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable {
    ...
    function mintNFT(address tokenAddress, uint256 nftAmount, uint256 tokenAmount) public {
        require(mintStarted == true, "NFT: Mint not started yet!");
        require(tokenAmount >= mintPrice * nftAmount, "NFT: Insufficient mint fees!");
        TransferHelper.safeTransferFrom(tokenAddress, address(msg.sender), owner(), tokenAmount);
        for (uint256 i = 0; i < nftAmount; i++) {
            internalMint(msg.sender);
        }
    }   
