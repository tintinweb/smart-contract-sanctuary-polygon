...
/**
 *Submitted for verification at hecoinfo.com on 2022-05-12
*/
...
contract D1verseDex {
    ...
    function checkOrderAndNFT(
        ...
        if (order.payment_token == address(0)) {
            approved_amount = taker.balance;
        } else {
            approved_amount = IERC20(order.payment_token).allowance(taker, address(this));
        }
        require(approved_amount >= order.fixed_price, "Taker's balance or approved-amount is not enough");
