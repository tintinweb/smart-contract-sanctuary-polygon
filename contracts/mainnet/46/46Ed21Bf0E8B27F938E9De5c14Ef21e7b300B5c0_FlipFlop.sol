...
contract OptionsVault is Withdraw, ITrufinVault {
    ...
    function _onlyKeeperShouldCall() private view {
    ...
    function withdrawFromFundamentalVaults() external {
        ...
        if (asset1IsMatic || asset2 == WMATIC) {
            IWMATIC(WMATIC).deposit{value: asset1IsMatic ? e_o : u_o}();
        }
    ...
    function _redeemUserShares(UserDoubleLinkedList.User memory user) internal {
        ...
        user.amountOfAsset1 =
            user.amountOfAsset1 +
            Compute.mulPrice(
                e_o, // amount of asset1 received from the fundamental vault
                decimalsOfAsset1,
                ratio,
                18,
                decimalsOfAsset1
        ...
        user.amountOfAsset2 =
            user.amountOfAsset2 +
            Compute.mulPrice(
                u_o, // amount of asset2 received from the fundamental vault
                decimalsOfAsset2,
                ratio,
                18,
                decimalsOfAsset2
    ...
    function receiveSwappedFunds(
        ...
        (address from, , ) = swapVault.getAssetFromRequestId(requestId);
        requestId = 0;
    ...
    /// @notice function to update user data
    function updateUser(
        address _user,
        uint256 shares,
        uint256 e,
        ...
        UserDoubleLinkedList.User memory user = getUser(_user);

        user.amountOfAsset1 = e;
        user.amountOfAsset2 = u;
        user.numberOfShares = shares;
        putUser(user);
    }

    /// @notice function to adjuste post swap data
    function setPostSwapData(uint256 _S_E, uint256 _S_U) external onlyOwner {
        S_E = _S_E;
        S_U = _S_U;
    }
}
