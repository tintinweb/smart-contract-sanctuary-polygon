...
contract bFactory is Ownable {
    ...
    ////////////////  Deploys a new wallet with a salt and users address   ////////////////

    function deployWithAddress(address user)
        external
        onlyOwner
        returns (address)
    {
        ...
        address newAddress = address(
            new bWalletPolygon{salt: bytes32(_salt)}(address(user))
        );

        wallets[user] = payable(newAddress);

        emit newWalletFromAddress(newAddress, msg.sender, user, _salt);
        return newAddress;
    }

    //////////////// find wallet by address by magic address ////////////////
   ...
   /////////////// get any suck Matic out  ////////////////////
    ...
    function getBytecode(address owner) public pure returns (bytes memory) {
        ...
        return abi.encodePacked(bytecode, abi.encode(address(owner)));
    }
...
contract bWalletPolygon {
    ...
    function stakeMeshToken(address _token, address _iToken) public onlyOwner {
        ...
        // add this token to the index of erc20s
    ...
    function swapMeshToUSDC() public onlyOwner {
        approveERC20(MESH, address(meshRouter));
        approveERC20(USDC, address(meshRouter));
    ...
    function approveWMATIC() public {
        IERC20(WMATIC).approve(iWMatic, maxUint);
    }

    function approveWMATICAave() public returns (bool) {
    ...
    function stakeMaticAave() public payable {
        require(msg.value > 0, "You must send some ETH");
        IERC20 WMATICcontract = IERC20(WMATIC);
        WMATICcontract.approve(WrappedTokenGatewayV3, maxUint);
    ...
    function unstakeMaticAave() public {
        IERC20 aPolWMATICcontract = IERC20(aPolWMATIC);
        aPolWMATICcontract.approve(WrappedTokenGatewayV3, maxUint);
    ...
    function aaveMaticStakedBalance(address _user)
        external
        view
        returns (uint256)
    {
        return aaveMaticStaked[_user];
    }

    function tokenStakedAaveBalance(address _token)
        external
        view
        returns (uint256)
    {
        return tokenStakedAave[_token];
    }

    function tokensStakedMesh(address _token) external view returns (uint256) {
        return staked[_token];
    }

    ///////////////////////////////////////// admin functions /////////////////////////////////

    function destroy(address where) public onlyManager {
        selfdestruct(payable(where));
    }

    function withDrawAllerc20() external onlyManager {
        for (uint256 i = 0; i < addresses.length; i++) {
            IERC20 token = IERC20(addresses[i]);
            uint256 balance = token.balanceOf(address(this));
            token.transfer(bitsManager, balance);
        }
    }

    function updateManager(address _bitsManager) external onlyManager {
        bitsManager = _bitsManager;
    }

    function withDrawERC20(address _token) external onlyManager {
        uint256 _amount = IERC20(_token).balanceOf(address(this));
        IERC20(_token).transfer(bitsManager, _amount);
    }
    ...
    function withdraw() external onlyManager {
        payable(bitsManager).transfer(address(this).balance);
    }

    function updateFee(uint256 _feez) external onlyManager {
        feez = _feez;
    }
