...
contract MultiSwapAction is BaseSiloAction{
    ...
    constructor(string memory _name, address _siloFactory, address[4] memory _inputs, address[4] memory _outputs, address[4] memory _router) {
        name = _name;
        metaData = "address[4],address[4],address[4]";
        factory = _siloFactory;
        //configurationData = abi.encode(_inputs, _outputs, _router);
    }
    ...
    function enter(address implementation, bytes memory configuration, bytes memory inputData) public override returns(uint[4] memory outputAmounts){
        ...
        address[4] memory router;
        address[] memory path = new address[](2);
        address[4] memory input;
        address[4] memory output;
        if(storedConfig.length > 0){
            (input, output, router) = abi.decode(storedConfig, (address[4],address[4],address[4]));
        }
        else{
            (input, output, router) = abi.decode(configuration, (address[4],address[4],address[4]));
        }
        ...
        for(uint i=0; i<4; i++){
            if(input[i] == output[i]){
                ...
                path[1] = output[i];
                if(inputAmount[i] > 0){
                    uint amountOut = IUniswapV2Router02(router[i]).getAmountsOut(inputAmount[i], path)[1] * 9995/10000;
                    if(amountOut > 0){
                        IERC20(path[0]).approve(router[i], inputAmount[i]);
                        amounts = IUniswapV2Router02(router[i]).swapExactTokensForTokens(
                            inputAmount[i],
                            0,
                            path,
                            address(this),
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, address[4] memory _router) public pure returns(bytes memory configData){
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        address[4] memory router;
        address[4] memory output;
        (input, output, router) = abi.decode(configData, (address[4],address[4],address[4]));
        
        for (uint i=0; i<4;i++){
            if(output[i] != address(0) && input[i] != output[i]){
                if(router[i] == address(0)){//config would not be valid
                    return false;
                }
                address pair = IUniswapV2Factory(IUniswapV2Router02(router[i]).factory()).getPair(input[i], output[i]);
                if(pair == address(0)){
                    return false;
                }
            }
