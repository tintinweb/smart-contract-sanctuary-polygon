...
contract KURTH is IBEP20, Auth {

    address WBNB = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    string constant _name = "KURTH";
    string constant _symbol = "KURTH"; 
   ...
   uint256 public _maxBuyTxAmount = (_totalSupply / 100) * 1;
    ...
    uint256 public _maxWalletToken = (_totalSupply / 100) * 1; //1% of supply
    ...
    constructor () Auth(msg.sender) {
        ...
        autoLiquidityReceiver = 0xE0B0A12760bA0a877EdC109AB6722b3e0b51ddE4;
    ...
    function setmaxWalletToken(uint256 maxWalletToken) external onlyOwner() {
        require(maxWalletToken != _maxWalletToken, "New max tx amount is same as the current");
        _maxWalletToken = maxWalletToken;
    }
    
        function setmaxBuyTxAmount(uint256 maxBuyTxAmount) external onlyOwner() {
        require(maxBuyTxAmount != _maxBuyTxAmount, "New max tx amount is same as the current");
        _maxBuyTxAmount = maxBuyTxAmount;
    }
        function setmaxSellTxAmount(uint256 maxSellTxAmount) external onlyOwner() {
        require(maxSellTxAmount != _maxSellTxAmount, "New max tx amount is same as the current");
    ...
    function swapBack() internal swapping {
        ...
        uint256 amountToLiquify = tokensToSell / (totalFee) * (liquidityFee);
        ...
        uint256 totalBNBFee = totalFee - (liquidityFee);
        ...
        uint256 amountBNBDevFee = amountBNB * (DevFee) / (totalBNBFee);

        try distributor.deposit{value: amountBNBReflection}() {} catch {}
        (bool CharitySuccess,) = payable(0xE0B0A12760bA0a877EdC109AB6722b3e0b51ddE4).call{value: amountBNBCharity, gas: 30000}("");
        require(CharitySuccess, "receiver rejected ETH transfer");
        (bool DevFeeSuccess,) = payable(0xa9170949ecEfe8aDE10F1EADf69Da7DB24Ddfe94).call{value: amountBNBDevFee, gas: 30000}("");
        require(DevFeeSuccess, "receiver rejected ETH transfer");
    ...
    function clearStuckBalance(uint256 amountPercentage) external authorized {
        ...
        payable(0xE0B0A12760bA0a877EdC109AB6722b3e0b51ddE4).transfer(amountBNB * amountPercentage / 100);
    }
