...
contract Reselling is Ownable, Auctions, Pausable{
    ...
    modifier isValidTicketContract(){
        require(address(_ticketsERC1155Contract) != address(0), "Reselling: Ticket contract is null");
        _;
    }
    ...
    function createEventPackageWithFixedPrice(EventPkg calldata pkg) public whenNotPaused isValidMainContract isValidTicketContract {
        isValidEventPkg(pkg);
        isEligibleForPkgCreation(pkg);
    ...
    function createEventPackageWithAuction(EventPkg calldata pkg, Auction calldata auction) public whenNotPaused isValidMainContract isValidTicketContract {
        isValidEventPkg(pkg);
        isEligibleForPkgCreation(pkg);
    ...
    function buyEventPkgWithFixedPrice(uint eventPkgId) public whenNotPaused isValidMainContract isValidTicketContract payable{
        checkEventPkgExists(eventPkgId);
    ...
    function endAuctionForEventPkg(uint eventPkgId) public whenNotPaused isValidMainContract isValidTicketContract {
        checkEventPkgExists(eventPkgId);
        require(msg.sender == _eventPkgDetails[eventPkgId].seller, "Reselling: Not an owner of package");
        checkUserBalance(_eventPkgDetails[eventPkgId].seller, _eventPkgDetails[eventPkgId].ticketTypeIds, _eventPkgDetails[eventPkgId].copies);
