...
contract PublicGovSale {
    ...
    function initialize(
        ...
        for(uint32 i=1; i<=4; i++){
            ...
            if(i < 4) booster[i].end = booster[i].start + 900;
    ...
    function moveFee() external {
        require(payment.balanceOf(address(this)) >= revenue && boosterProgress() == 4 && !isMoved && (msg.sender == factory.savior() || msg.sender == owner), "bad");
    ...
    function refund() external {
        ...
        uint128 amount = (getTotalPurchase(msg.sender) * price * _refund_d2) / 10000;
