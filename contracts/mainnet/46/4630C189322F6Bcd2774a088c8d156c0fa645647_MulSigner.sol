...
interface IERCMulSigner {
    ...
    event OpposedMulSigner(address sender, uint txHash, string txId);
...
interface IERCSignerManage {
    ...
    event AjuestThreshold(address signer,uint newThreshold,uint oldThreshold);
    ...
    //设置多签账号
     ...
     function adjustSingerThreshold(address singer, uint256 newThreshold) external ;
    ...
    IERCSignerManage
{
    ...
    function submitMulSigner(
        ...
        if (op == IERCMulSigner.SignerOp.Approve) {
          ...
          emit ApproveMulSigner(msg.sender, txHash, mtx.txId);
           ...
           emit OpposedMulSigner(msg.sender, txHash, mtx.txId);
        }
    ...
    function completeExpireTx(address sender) private {
      ...
      function adjustSingerThreshold(address singer, uint256 newThreshold) external  override onlyAdmin {
        IERCSignerManage.Signer storage Signer = Signers[singer];
        require(Signer.Signers.length > 2, "Signer is empty");
        require(Signer.threshold > 2, "Threshold is zero");
        require(Signer.Signers.length >= Signer.threshold, "Threshold is bigger than Signer");
        require(Signer.Signers.length <= 10, "Signer is bigger than 10");
        require(newThreshold >=2 && newThreshold <=Signer.Signers.length, "Threshold is misvalue");
        Signer.threshold = newThreshold;
        emit AjuestThreshold(singer, newThreshold, Signer.threshold);
    }
