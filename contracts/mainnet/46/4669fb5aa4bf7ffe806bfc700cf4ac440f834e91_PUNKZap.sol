...
// File: contracts/interfaces/IPUNKFactory.sol


pragma solidity 0.6.12;

interface IPUNKFactory {
    function createPool(
        IERC20 tokenA,
        IERC20 tokenB,
        uint32 ampBps,
...
// File: contracts/interfaces/IPUNKPool.sol
...
interface IPUNKPool {
    ...
    function factory() external view returns (IPUNKFactory);

    function kLast() external view returns (uint256);
}

// File: contracts/libraries/PUNKLibrary.sol
...
library PUNKLibrary {
    ...
    {
        require(tokenA != tokenB, "PUNKLibrary: IDENTICAL_ADDRESSES");
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(address(token0) != address(0), "PUNKLibrary: ZERO_ADDRESS");
    }
        ...
        )
    {
        ...
        uint256 vReserve0;
        uint256 vReserve1;
        (reserve0, reserve1, vReserve0, vReserve1, feeInPrecision) = IPUNKPool(pool).getTradeInfo();
    ...
    function getReserves(
        ...
        (IERC20 token0, ) = sortTokens(tokenA, tokenB);
        (uint256 reserve0, uint256 reserve1) = IPUNKPool(pool).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, "PUNKLibrary: INSUFFICIENT_AMOUNT");
        require(reserveA > 0 && reserveB > 0, "PUNKLibrary: INSUFFICIENT_LIQUIDITY");
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, "PUNKLibrary: INSUFFICIENT_INPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > 0, "PUNKLibrary: INSUFFICIENT_LIQUIDITY");
        ...
        require(reserveOut > amountOut, "PUNKLibrary: INSUFFICIENT_LIQUIDITY");
    }
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, "PUNKLibrary: INSUFFICIENT_OUTPUT_AMOUNT");
        require(reserveIn > 0 && reserveOut > amountOut, "PUNKLibrary: INSUFFICIENT_LIQUIDITY");
...
// File: contracts/periphery/PUNKZap.sol
...
contract PUNKZap {
    ...
    mapping(IPUNKFactory => bool) public whitelistedFactories;
    address public immutable weth;

    event ConfigMasterUpdated(address oldConfigMaster, address newConfigMaster);
    event FactoryAdded(IPUNKFactory factory);
    event FactoryRemoved(IPUNKFactory factory);

    modifier onlyConfigMaster() {
        require(msg.sender == configMaster, "PUNKZap: Forbidden");
        _;
    }

    modifier whitelistedFactory(IPUNKFactory _factory) {
        require(whitelistedFactories[_factory], "PUNKZap: Forbidden");
        _;
    }

    modifier ensure(uint256 deadline) {
        require(deadline >= block.timestamp, "PUNKZap: Expired");
        _;
    }

    constructor(IPUNKFactory _factory, address _weth) public {
    ...
    function addFactory(IPUNKFactory _factory) external onlyConfigMaster {
        whitelistedFactories[_factory] = true;
        emit FactoryAdded(_factory);
    }

    function removeFactory(IPUNKFactory _factory) external onlyConfigMaster {
    ...
    function zapInEth(
        ...
        lpQty = IPUNKPool(pool).mint(to);
        require(lpQty >= minLpQty, "IPUNKRouter: INSUFFICIENT_MINT_QTY");
    }
    ...
    function calculateZapOutAmount(
        ...
        amountOut += PUNKLibrary.getAmountOut(
            amountIn,
            data.rIn,
            data.rOut,
            data.vIn,
    ...
    function calculateSwapAmounts(
        ...
        require(factory.isPool(tokenIn, tokenOut, pool), "INVALID_POOL");
        (uint256 rIn, uint256 rOut, uint256 vIn, uint256 vOut, uint256 feeInPrecision) = PUNKLibrary
            .getTradeInfo(pool, tokenIn, tokenOut);
        amountSwap = _calculateSwapInAmount(rIn, rOut, vIn, vOut, feeInPrecision, userIn);
        amountOutput = PUNKLibrary.getAmountOut(amountSwap, rIn, rOut, vIn, vOut, feeInPrecision);
    }
    ...
    function _swap(
        ...
        (IERC20 token0, ) = PUNKLibrary.sortTokens(tokenIn, tokenOut);
        (uint256 amount0Out, uint256 amount1Out) = tokenIn == token0
            ? (uint256(0), amountOut)
            : (amountOut, uint256(0));
        IPUNKPool(pool).swap(amount0Out, amount1Out, to, new bytes(0));
    }
    ...
    function _zapOut(
        ...
        uint256 amountIn;
        {
            ...
            (uint256 amount0, uint256 amount1) = IPUNKPool(pool).burn(address(this));
            (IERC20 token0, ) = PUNKLibrary.sortTokens(tokenIn, tokenOut);
        ...
        uint256 swapAmount;
        {
            (
                ...
                uint256 vOut,
                uint256 feeInPrecision
            ) = PUNKLibrary.getTradeInfo(pool, tokenIn, tokenOut);
            swapAmount = PUNKLibrary.getAmountOut(amountIn, rIn, rOut, vIn, vOut, feeInPrecision);
        }
        ...
        )
    {
        ...
        (data.rIn, data.rOut, data.vIn, data.vOut, data.feeInPrecision) = PUNKLibrary.getTradeInfo(
            pool,
            tokenIn,
            tokenOut
        );
        uint256 totalSupply = _calculateSyncTotalSupply(factory, IPUNKPool(pool), data);
        bool isAmpPool = (IPUNKPool(pool).ampBps() != 10000);
        // calculate amountOut
