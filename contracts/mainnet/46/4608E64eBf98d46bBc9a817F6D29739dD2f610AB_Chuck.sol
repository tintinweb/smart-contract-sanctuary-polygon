...
contract Chuck is Initializable, ERC721EnumerableUpgradeable , AccessControlUpgradeable {
    ...
    function initialize(address payable minter, string memory domain, uint256 supply) public initializer {
        __ERC721_init("GITEX Official NFT 2022", "GITEX22");
    ...
    function _baseURI() internal view virtual override returns (string memory) {
        return string(abi.encodePacked(domainName, "/contract/", uint160(address(this)).toHexString(), "/token/"));
    }
    ...
    function mint(address to) public payable onlyRole(MINTER_ROLE) {
        require(totalSupply() < maxSupply, "Every token has been minted");
        require(hasRole(MINTER_ROLE, msg.sender), "Only contract owner can mint");
        require(balanceOf(to) == 0, "You already own a token on this contract");
        uint256 tokenId = uint256(keccak256(abi.encodePacked(address(this), totalSupply())));
