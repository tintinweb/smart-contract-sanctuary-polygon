...
contract BulkAirdrop is ReentrancyGuard {
    ...
    function bulkAirdropERC20(IERC20 _token, address[] calldata _to, uint256[] calldata _value) external onlyOwner nonReentrant {
        require(_to.length == _value.length, "BA: _to & _value length mismatch");
        // Removed the code to check the allowance to save on the gas
        for(uint256 i; i < _to.length;){
            require(_token.transferFrom(msg.sender, _to[i], _value[i]), "BA: Transfer Failed");
            emit Airdrop(msg.sender, _to[i], _value[i]);
            unchecked {
                ++i;
            }
        }
    ...
    function withdrawERC20(IERC20 _token) external onlyOwner nonReentrant {
        ...
        require(_token.transfer(admin, balance));
    }
}