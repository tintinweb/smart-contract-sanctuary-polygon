...
contract MaxityMarketPlace is Ownable,Pausable ,IMaxityMarketPlace,IERC721Receiver{
    ...
    function sellByFixPrice(address _token,uint256[] memory _tokenids,uint [] memory _prices,address _seller) external  returns (uint) {
            
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].seller==address(0x0),"This token is no longer for sale");
    ...
    function onERC721Received( address _operator, address _from, uint256 _tokenId, bytes calldata data ) public override returns (bytes4) {
        return this.onERC721Received.selector;
    }

    function auctionMax721(address _token,uint256[] memory _tokenids,uint[] memory base_prices,uint [] memory incre_prices,uint[] memory starttimes,uint[] memory deadlines,address _seller) external returns (uint amountU){
        uint256 length = _tokenids.length;
        for (uint256 pid = 0; pid < length; ++pid) {
            uint256 _tokenid=_tokenids[pid];
            require(marketUnits[_token][_tokenid].seller==address(0x0),"This token is no longer for sale");
    ...
    function auctionOnFixPrice(address _token,uint _tokenid,uint _price,address price_offer) public override {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller!=address(0x0),"This token is not for sale");
        require(IERC721(_token).ownerOf(_tokenid)==address(this),"This token is not on the marketplace");
        require(!saleInfo.isAuction,"This token is on auction");
     ...
     function agreeAuctionOnFixPrice(address _token,uint _tokenid) public override {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
        require(saleInfo.seller!=address(0x0),"This token is not for sale");
        require(IERC721(_token).ownerOf(_tokenid)==address(this),"This token is not on the marketplace");
        require(!saleInfo.isAuction,"This token is on auction");
   ...
   function delist(address _token,uint256 _tokenid) public override {
    ...
    function tradeDone(address _token,uint256 _tokenid,uint amount,address buyer,TradeMode mode) internal {
        SaleInfo storage saleInfo = marketUnits[_token][_tokenid];
