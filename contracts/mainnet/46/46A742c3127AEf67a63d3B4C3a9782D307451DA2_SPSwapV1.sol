...
library LibSwap  {
    ...
    */
    ...
    /**
    * @dev Token bundle allowe
    */
    enum BundleAllowed {
        ONLY_ERC721,
        ERC20_ONLY_ONE_SIDE,
        ERC20_BOTH_SIDES_WITH_NFT,
        ALL
    }
...
contract SPSwapV1 is Ownable, Pausable, ReentrancyGuard, ValidatorSwap {
    ...
    /**
     * @dev Mapping of swapHash => bool indicate if swap hash been cancelles
     */
    mapping(bytes32 => bool) private _cancelled;

    /**
     * @dev Token bundle allowed. View TokenType in LibSwap 
     */
    LibSwap.BundleAllowed private _bundleAllowed;
    ...
    function cancelSwap(LibSwap.Swap calldata swap, bytes calldata signature) 
        ...
        LibSwap.SwapInfo memory swapInfo = _getSwapInfo(swap);
        validateCancelSwap(swap, signature, swapInfo);
        _cancelled[swapInfo.swapHash] = true;
     ...
     /**
     * @dev Get swap status from hash
     * @param swapHash Swap hash
     * @return swapStatus the status os swap
     */
    function getSwapStatus(bytes32 swapHash) 
        external view returns (LibSwap.SwapStatus) {

        if(_filled[swapHash] == true) {
            return LibSwap.SwapStatus.FILLED;
        }
        else if (_cancelled[swapHash] == true) {
            return LibSwap.SwapStatus.CANCELLED;
        }
        else {
            return LibSwap.SwapStatus.UNDEFINED;
        }
    }
    ...
    /**
     * @dev Set token bundle allowed
     */
    function setBundleAllowd(LibSwap.BundleAllowed bundleAllowed) public onlyOwner {
        _bundleAllowed = bundleAllowed;
    }
    ...
    function _dispatchTransferFrom(LibSwap.TokenData[] calldata tokenData, address from, address to) 
        ...
        for (uint256 i = 0; i < tokenData.length; ++i) {
            ...
            else if (tokenData[i].tokenType == LibSwap.TokenType.ERC1155) {
                IERC1155(tokenData[i].tokenContract).safeTransferFrom(from, to, tokenData[i].tokenId, 
                    tokenData[i].amount, "");
            }
            else if (tokenData[i].tokenType == LibSwap.TokenType.ERC20) {
                IERC20(tokenData[i].tokenContract).safeTransferFrom(from, to, tokenData[i].amount);
            }
        }
    }
     ...
     */
    function _getCancelled() internal view override 
     ...
     */
    function _getFilled() internal view override returns (mapping(bytes32 => bool) storage) {
        return _filled;
    }

     /**
     * @dev See {ValidatorSwap-getBundleAllowed}.
     */
    function _getBundleAllowed() internal view override returns (LibSwap.BundleAllowed){
        return _bundleAllowed;
    }
     ...
     */
    function _getSwapInfo(LibSwap.Swap calldata swap)
        private
        view
...
abstract contract ValidatorSwap {
     ...
     */
    function _getCancelled() internal virtual view returns (mapping(bytes32 => bool) storage);

    /**
     * @dev Return swap filled status
     * @return  filled mapping of filled swap
     */
    function _getFilled() internal virtual view returns (mapping (bytes32 => bool) storage);

    /**
     * @dev Return token bundle allowed
     * @return  token bundle allowed
     */
    function _getBundleAllowed() internal virtual view returns (LibSwap.BundleAllowed);
    ...
    function validateFillSwap(
        ...
        _isValidBundle(swap.makerTokenData, swap.takerTokenData);
    }
    ...
    function _validateCommonSwap(
        ...
        _isCancelled(swapInfo.swapHash);
    ...
    function _isCancelled(bytes32 swapHash) private view {
        require(_getCancelled()[swapHash] ==  false, "The swap has already been cancelled");

    }

    function _isFilled(bytes32 swapHash) private view {
        require(_getFilled()[swapHash] ==  false, "The swap has already been filled");
    ...
    function _isValidTokenData(LibSwap.TokenData calldata tokenData, address owner) private view {
        ...
        _isValidTokenDataERC1155(tokenData, owner);
        _isValidTokenDataERC20(tokenData, owner);
    ...
    function _isValidTokenDataERC721(LibSwap.TokenData calldata tokenData, address owner) private view {
        if(tokenData.tokenType == LibSwap.TokenType.ERC721) {
            ...
            require(ERC165Checker.supportsInterface(tokenData.tokenContract, type(IERC721).interfaceId), 
    ...
    function _isValidTokenDataERC1155(LibSwap.TokenData calldata tokenData, address owner) private view {
        if(tokenData.tokenType == LibSwap.TokenType.ERC1155) {
            require(tokenData.amount > 0, "Amount must be greater than 0");

            require(ERC165Checker.supportsInterface(tokenData.tokenContract, type(IERC1155).interfaceId), 
                "Contract not supported");

            require(IERC1155(tokenData.tokenContract).balanceOf(owner, tokenData.tokenId) >= tokenData.amount, 
                    "Insufficient token balance");

            require(IERC1155(tokenData.tokenContract).isApprovedForAll(owner, address(this)),
                    "SP Swap is not approved for this token");
        }
    }

     function _isValidTokenDataERC20(LibSwap.TokenData calldata tokenData, address owner) private view {
        if(tokenData.tokenType == LibSwap.TokenType.ERC20) {
            require(tokenData.amount > 0, "Amount must be greater than 0");
            require(tokenData.tokenId == 0, "Token id must be 0");

            require(IERC20(tokenData.tokenContract).balanceOf(owner) >= tokenData.amount, 
                    "Insufficient token balance");

            require(IERC20(tokenData.tokenContract).allowance(owner, address(this)) >= tokenData.tokenId,
                    "SP Swap is not approved for this token amount");
        }
    }

    function _isValidBundle(LibSwap.TokenData[] calldata makerTokenData, LibSwap.TokenData[] calldata takerTokenData)
        private view {
            bool _existERC20InMakerData = false;
            bool _existERC20InTakerData = false;
            bool _existNFTInMakerData = false;
            bool _existNFTInTakerData = false;
            
            for(uint256 i = 0; i < makerTokenData.length; i++) {
                if(makerTokenData[i].tokenType == LibSwap.TokenType.ERC20) {
                    _existERC20InMakerData = true;
                }
                else{
                    _existNFTInMakerData = true;
                }

                if(_existERC20InMakerData && _existNFTInMakerData) {
                    break;
                }
            }

            for(uint256 i = 0; i < takerTokenData.length; i++) {
                if(takerTokenData[i].tokenType == LibSwap.TokenType.ERC20) {
                    _existERC20InTakerData = true;
                }
                else {
                    _existNFTInTakerData = true;
                }

                if(_existERC20InTakerData && _existNFTInTakerData) {
                    break;
                }
            }

            LibSwap.BundleAllowed _bundleAllowed = _getBundleAllowed();

            if(_bundleAllowed == LibSwap.BundleAllowed.ONLY_ERC721) {
                require(!_existERC20InMakerData && !_existERC20InTakerData, "ERC20 not allowed in bundle.");
            }
            else if(_bundleAllowed == LibSwap.BundleAllowed.ERC20_ONLY_ONE_SIDE) {
                require((_existNFTInMakerData || _existNFTInTakerData) &&
                        ( 
                            (!_existERC20InMakerData && !_existERC20InTakerData) ||
                            (_existERC20InMakerData && !_existERC20InTakerData) ||
                            (_existERC20InTakerData && !_existERC20InMakerData)
                        ), 
                        "ERC20 only allowed in one side.");
            }
            else if(_bundleAllowed == LibSwap.BundleAllowed.ERC20_BOTH_SIDES_WITH_NFT) {
                require(_existNFTInMakerData || _existNFTInTakerData, "There has to be at least one NFT.");
            }
    }
}