...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function _swap(SwapArgs memory swapArgs, bool transferFromSender) internal returns(uint256[] memory amountOuts) {
    ...
    if (toAssetId.isNative()) {
      address wrappedToAssedId = swapArgs.getToAssetId();
      wrappedToAssedId.increaseAllowance(config.weth, amountOut);
  ...
  function swap(SwapArgs calldata swapArgs) external payable returns(uint256[] memory amountOuts) {
    amountOuts = _swap(swapArgs, true);

    emit Swapped(swapArgs, amountOuts, msg.sender);
  }

  function swapIn(SwapInArgs calldata args) external payable override returns(uint256[] memory amountOuts, uint64, uint64) {
    ...
    toAssetId.increaseAllowance(config.tokenBridgeAddress, amountOut);
    uint64 tokenSequence = IWormhole(config.tokenBridgeAddress).transferTokens(
      toAssetId,
      amountOut,
...
library LibBytes {
  ...
  function parse(bytes memory self) internal pure returns (IMagpieCore.ValidationOutPayload memory payload) {
    ...
    payload.recipientChainId = self.toUint16(i);
    i += 2;
