...
contract LaTest is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    LaTestDividendTracker public dividendTracker;
    ...
    constructor() ERC20("LaTest", "Ltest") {
        ...
        teamWallet = payable(0x4A7Ccd75a4cE2F7BD39547e33a7E6584f5542557);
        ...
        dividendTracker = new LaTestDividendTracker(payable(this), router, 0x1D2F0da169ceB9fC7B3144628dB156f3F6c60dBE,
                                                   "LaTestTRACKER", "LtestTRACKER");
...
contract LaTestDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor(address payable mainContract, address router, address token, string memory _name,
        ...
        LaTestContract = LaTest(mainContract);
    ...
    function withdrawDividend() public pure override {
        require(false, "withdrawDividend disabled. Use the 'claim' function on the main LaTest contract.");
    }
    ...
    function _reinvestDividendOfUser(address account) private returns (uint256) {
        ...
        if (_withdrawableDividend > 0) {
            ...
            path[1] = address(LaTestContract);

            uint256 prevBalance = 
            LaTestContract.balanceOf(address(this));
            ...
            {
                uint256 received = LaTestContract.balanceOf(address(this)).sub(prevBalance);
                if (received > 0) {
                    success = true;
                    LaTestContract.transfer(account, received);
                } 
                else {
                    success = false;
                }
