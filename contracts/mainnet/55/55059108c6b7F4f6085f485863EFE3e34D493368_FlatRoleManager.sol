...
contract FlatRoleManager is IFlatRoleManager, BaseOwnable {
    ...
    /// @notice Add new roles
    /// @dev Only owner can process roles, the role has to be new roles.
    ///      On success, `RoleAdded` event will be fired
    /// @param _roles the role to be added
    ...
    /// @notice Grant roles to a delegates
    /// @dev Granting a role to a delegate will give delegate permission to call
    ///      contract functions associated with the role. Only owner can process
    ///      the roles. On success, `RoleGranted` event would be fired and
    ///      possibly `DelegateAdded` as well if this is the first role being
    ///      granted to the delegate.
    /// @param _roles the role to be granted
    /// @param _delegates the delegate to be granted role
    function grantRoles(bytes32[] calldata _roles, address[] calldata _delegates) external onlyOwner {
        require(_roles.length > 0 && _roles.length == _delegates.length, "invalid inputs");
    ...
    /// @notice Revoke roles from delegates
    ...
    ///      Only owner can process the roles. `RoleRevoked` event would
    ///      be fired and possibly `DelegateRemoved` as well if this is the
    ///      last role the delegate owns.
    /// @param _roles the roles to be granted
    /// @param _delegates the delegates to be granted role
    function revokeRoles(bytes32[] calldata _roles, address[] calldata _delegates) external {
        ...
        for (uint256 i = 0; i < _roles.length; i++) {
            ...
            // need to make sure `RoleRevoked` is fired before `DelegateRemoved`
