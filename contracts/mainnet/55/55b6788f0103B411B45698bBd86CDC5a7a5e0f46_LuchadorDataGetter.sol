...
contract LuchadorDataGetter {
    ...
    function getLuchadoresData(uint256 startIndex, uint256 endIndex) public view returns (Luchador.LuchadorData[] memory) {
        ...
        for (uint i = startIndex; i < endIndex; i++) {
            luchadores[i - 1].tokenURI = luchador.tokenURI(i);
            luchadores[i - 1].attributes = luchador.getAllLuchadorAttributes(i);
            luchadores[i - 1].location = luchador.luchadoresLocation(i);
            luchadores[i - 1].currentTrainerOwner = luchador.luchadoresCurrentTrainerOwner(i);
            luchadores[i - 1].locationStartTime = luchador.luchadoresLocationStartTime(i);
            luchadores[i - 1].weight = luchador.luchadoresWeight(i);
            luchadores[i - 1].wins = luchador.luchadoresWins(i);
            luchadores[i - 1].losses = luchador.luchadoresLosses(i);
            luchadores[i - 1].name = luchador.luchadoresName(i);
            luchadores[i - 1].bio = luchador.luchadoresBio(i);
            luchadores[i - 1].price = luchador.luchadoresPrice(i);
            luchadores[i - 1].index = luchador.luchadoresIndex(i);
            luchadores[i - 1].lastSalePrice = luchador.luchadoresLastSalePrice(i);
            luchadores[i - 1].saleStatus = luchador.luchadoresSaleStatus(i);
        }
