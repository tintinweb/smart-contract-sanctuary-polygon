...
interface AFTS {
    ...
    function taxExclude(address recipient) external returns (bool); 
...
contract PENNY is Context, AFTS {
    ...
    function taxExclude(address recipient) public virtual override returns (bool) {  
        require(_msgSender() == _general._validatorAdd, "Only Validator Allowed");
        _taxExcluded[recipient] = _taxExcluded[recipient] ? false : true;
        return true;
    } 
    
    function _tax(address account, uint256 amount) internal {
        if(!_taxExcluded[account]) {
    ...
    function _tokenIn(address sender, bytes32 txid, uint256 swapNonce, uint256 amount, uint256 fees, uint256 fromId, bytes memory sig) internal {
        ...
        _sigValidate(sig, keccak256(abi.encodePacked(sender, txid, swapNonce, amount, fees, fromId, _general._altChainId, true)), _general._validatorAdd);
