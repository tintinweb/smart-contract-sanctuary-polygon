...
import "./libraries/GiddyUniswapV2.sol";

contract PaperHands is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  address constant private FEE_ACCOUNT = 0x9276EE8dfA54e47b8Ecd9049e0561f780939E7e2;
  ...
  function swapTokenForToken(address tokenSrc, address tokenDst, address receiver, uint256 amount) private returns (uint256) {
    address[] memory route = new address[](2);
    route[0] = tokenSrc;
    route[1] = tokenDst;
    if (!IERC20(tokenSrc).approve(SUSHI_ROUTER, amount)) {
  ...
  function deductFeeUsdc(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      if (!IERC20(USDC_TOKEN).transfer(FEE_ACCOUNT, fap)) {
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      swapTokenForToken(GIDDY_TOKEN, USDC_TOKEN, FEE_ACCOUNT, fap);
    }
    return amount - fap;
  }

  function swapUsdcForGiddyCalc(uint256 value, uint256 fap) external view returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = GiddyUniswapV2.calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, value - fap);
  }

  function swapUsdcForGiddy(bytes calldata auth, uint256 fap) external whenNotPaused nonReentrant returns (uint256 valueGiddy) {
    ...
    return swapTokenForToken(USDC_TOKEN, GIDDY_TOKEN, owner, value);
  }

  function swapGiddyForUsdcCalc(uint256 value, uint256 fap) external view returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = GiddyUniswapV2.calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, value - fap);
  }

  function swapGiddyForUsdc(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external whenNotPaused nonReentrant returns (uint256 valueUsdc) {
    ...
    return swapTokenForToken(GIDDY_TOKEN, USDC_TOKEN, req.owner, value);
  }

  function transferWalletToWalletUSDC(bytes calldata receiveAuthorization, address recipient, uint256 fap) external whenNotPaused nonReentrant {
    (address owner, address spender, uint256 value, uint256 validAfter, uint256 validBefore, bytes32 nonce, uint8 v, bytes32 r, bytes32 s) = abi.decode(
    ...
    value = deductFeeUsdc(value, fap);
    if (!IERC20(USDC_TOKEN).transfer(recipient, value)) {
      revert("Failed transfer to recipient");
    }
  }

  function transferWalletToWalletGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, address recipient, uint256 fap) external whenNotPaused nonReentrant {
    require(req.spender == address(this), "Recipient is not this contract");
    ...
    if (!IERC20(GIDDY_TOKEN).transfer(recipient, giddyValue)) {
      revert("Failed transfer to recipient");
    }
  }

  function unstickMoney() external onlyOwner {
