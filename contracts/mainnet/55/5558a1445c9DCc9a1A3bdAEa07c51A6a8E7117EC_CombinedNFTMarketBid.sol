...
interface MarketItemHolder {
    ...
    function fetchMarketItem(uint itemId) external view returns (MarketItem memory);

    function createMarketItem(address sellerAddress, bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) external returns (MarketItem memory);

    function updateMarketItem(uint256 itemId, uint256 price) external returns (MarketItem memory);

    function cancelMarketItem(uint256 itemId) external returns (MarketItem memory);

    function createMarketSale(uint256 itemId, uint256 remainAmount) external returns (MarketItem memory);
    ...
    function fetchMarketBidItem(uint itemId) external view returns (MarketBidItem memory);

    function createMarketBidItem(address bidderAddress, bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) external returns (MarketBidItem memory);

    function updateMarketBidItem(uint256 itemId, uint256 price) external returns (MarketBidItem memory);

    function cancelMarketBidItem(uint256 itemId) external returns (MarketBidItem memory);

    function createMarketAccept(uint256 itemId, uint256 remainAmount) external returns (MarketBidItem memory);

    function testSenderManager() external view returns (address);
...
contract CombinedNFTMarketBid is ReentrancyGuard, Ownable {
    using SafeMath for uint256;
    using Address for address;

    address public _marketItemHolderAddress;

    MarketItemHolder marketItemHolder;
    ...
    constructor(uint256 fee, address feeRecipient, address marketItemHolderAddress) {
        _marketplaceFee = fee;
        _marketplaceFeeRecipient = payable(feeRecipient);
        _marketItemHolderAddress = marketItemHolderAddress;
        marketItemHolder = MarketItemHolder(marketItemHolderAddress);
        _managers[msg.sender] = true;
    }


    event MarketItemCancelled(uint256 itemId);
    event MarketItemSold(address indexed buyer, uint256 indexed itemId, uint256 amount, MarketItemHolder.State state);
    ...
    event MarketBidItemAccepted(address indexed acceptor, uint256 indexed itemId, uint256 amount, MarketItemHolder.BidState state);
    ...
    function setMarketItemHolderAddress(address marketItemHolderAddress) public onlyOwner {
        _marketItemHolderAddress = marketItemHolderAddress;
        marketItemHolder = MarketItemHolder(marketItemHolderAddress);
    }
    ...
    function createMarketItem(bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        ...
        MarketItemHolder.MarketItem memory item = marketItemHolder.createMarketItem(msg.sender, isErc721, nftContract, tokenId, price, amount, erc20Address);
    ...
    function emitUpdateMarketItem(uint256 itemId) public onlyOwner {
        MarketItemHolder.MarketItem memory item = marketItemHolder.fetchMarketItem(itemId);
        require(item.state == MarketItemHolder.State.INITIATED, "invalid state");
    ...
    function updateMarketItem(uint256 itemId, uint256 price) public payable nonReentrant {
        ...
        item = marketItemHolder.updateMarketItem(itemId, price);
        emit MarketItemEdited(
            itemId,
            item.isErc721,
            item.nftContract,
    ...
    function cancelMarketItem(uint256 itemId) public virtual {
        ...
        require(item.state == MarketItemHolder.State.INITIATED, "invalid state");
        require(item.seller == msg.sender || msg.sender == owner(), "invalid owner");
        item = marketItemHolder.cancelMarketItem(itemId);
    ...
    function createMarketSale(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        ...
        item = marketItemHolder.createMarketSale(itemId, remainAmount);

        if (item.erc20Address == address(0)) {
            require(msg.sender.balance >= priceTotal.add(fee), "Insufficient balance");
            ...
            require(token.balanceOf(msg.sender) >= priceTotal.add(fee), "Insufficient balance");
    ...
    function fetchMarketItem(uint itemId) public view returns (MarketItemHolder.MarketItem memory) {
        return marketItemHolder.fetchMarketItem(itemId);
    }
    ...
    function createMarketBidItem(bool isErc721, address nftContract, uint256 tokenId, uint256 price, uint256 amount, address erc20Address) public payable nonReentrant {
        ...
        MarketItemHolder.MarketBidItem memory item = marketItemHolder.createMarketBidItem(msg.sender, isErc721, nftContract, tokenId, price, amount, erc20Address);
    ...
    function emitUpdateMarketBidItem(uint256 itemId) public onlyOwner {
        MarketItemHolder.MarketBidItem memory item = fetchMarketBidItem(itemId);
        require(item.state == MarketItemHolder.BidState.INITIATED, "invalid state");
    ...
    function updateMarketBidItem(uint256 itemId, uint256 price) public payable nonReentrant {
        ...
        item = marketItemHolder.updateMarketBidItem(itemId, price);
        emit MarketBidItemEdited(
            itemId,
            item.isErc721,
            item.nftContract,
    ...
    function cancelMarketBidItem(uint256 itemId) public virtual {
        ...
        require(item.state == MarketItemHolder.BidState.INITIATED, "invalid state");
        require(item.bidder == msg.sender || msg.sender == owner(), "invalid owner");
        marketItemHolder.cancelMarketBidItem(itemId);
    ...
    function createMarketAccept(uint256 itemId, address erc20Address, uint256 amount) public payable nonReentrant {
        ...
        uint256 fee = priceTotal.mul(_marketplaceFee).div(10000);
        uint256 remainAmount = item.amount.sub(amount);
        item = marketItemHolder.createMarketAccept(itemId, remainAmount);
        IERC20 token = IERC20(item.erc20Address);
        require(token.balanceOf(item.bidder) >= priceTotal.add(fee), "Insufficient balance");
    ...
    function fetchMarketBidItem(uint itemId) public view returns (MarketItemHolder.MarketBidItem memory) {
        return marketItemHolder.fetchMarketBidItem(itemId);
    }
    ...
    function testSenderManager() external view returns (address) {
        return marketItemHolder.testSenderManager();
    }
}