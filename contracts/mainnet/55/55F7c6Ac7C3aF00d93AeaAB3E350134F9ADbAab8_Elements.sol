...
contract Elements is ERC721Enumerable, Ownable {
 ...
 /*  function getParents( string memory _elementId) public view  virtual     returns (string  memory)
    {
         bytes1   elementId= bytes1( bytes(_elementId) );
         string memory parameters=string(abi.encodePacked("parents:"));
          for(uint i=0;i<elements[elementId].parents.length;i++){
            parameters= string( abi.encodePacked (parameters, elements[elementId].parents[i],",") ) ;
        }
         return  parameters ;
    } */
    ...
    function addNftElement(
        ...
        for(uint i=0;i<_parents.length;i++){
            parents[i]= (bytes1( bytes(_parents[i]) ));
        }
