...
contract BuyMeSomeGas is IBase,  GelatoRelayContext {
    ...
    error BuyMeSomeGas__buy_deadline();
    error BuyMeSomeGas__buy_invalidFeeToken();
    error BuyMeSomeGas__buy_feeTooHigh();
    error BuyMeSomeGas__buy_invalidPermitValue();
    error BuyMeSomeGas__getPermitNonce_invalidToken();
    error BuyMeSomeGas__getTokenDS_invalidToken();

    event Buy(address indexed buyer, uint256 amountInBuy, uint256 amountOut, uint256 fee, address indexed token);

    constructor(address _buyer, address _usdc, address _dai) {
      buyer = IBuyer(_buyer);
      usdc = _usdc;
      dai = _dai;
    }

    function buy(uint256 _amountIn, uint256 _amountOutMin, uint256 _maxFee, uint _deadline, PermitSign calldata _permitSig, PermitSign calldata _unPermitSig) external onlyGelatoRelay {
        if (block.timestamp > _deadline) revert BuyMeSomeGas__buy_deadline();

        address feeToken = _getFeeToken();
        uint256 fee = _getFee();

        if (feeToken != usdc && feeToken != dai) revert BuyMeSomeGas__buy_invalidFeeToken();
        if (fee > _maxFee) revert BuyMeSomeGas__buy_feeTooHigh();

        if (feeToken == usdc) {
          ERC20Permit(feeToken).permit(
            _permitSig.owner,
            address(this),
            _amountIn,
            _deadline,
          ...
          // DAI permit is different: (holder, spender, nonce, expiry, allowed, v, r, s)
          IDai(feeToken).permit(
            _permitSig.owner,
            address(this),
            getPermitNonce(feeToken, _permitSig.owner),
            _deadline,
            true,
            _permitSig.v,
            _permitSig.r,
        ...
        ERC20Permit(feeToken).safeTransferFrom(_permitSig.owner, address(this), _amountIn);
        ...
        // Buy me some gas
        uint256 amountInBuy = _amountIn - fee;

        // We need to so this way if we want to have multiple buyers connectors/adapters
        bytes memory payload = abi.encodeWithSelector(IBuyer.buyGas.selector, feeToken, amountInBuy, _amountOutMin, _deadline, _permitSig);
        bytes memory returnData = address(buyer).functionDelegateCall(payload);

        (uint256 amountOut) = abi.decode(returnData, (uint256));

        emit Buy(_permitSig.owner, amountInBuy, amountOut, fee, feeToken);
    }

    function getAmountOut(address _feeToken, uint256 _amountIn) external view returns(uint256) {
        return buyer.getAmountOut(_feeToken, _amountIn);
    }

    // TODO - Improve this
    function getPermitNonce(address _feeToken, address _owner) public view returns(uint256) {
      if (_feeToken == usdc) {
        return ERC20Permit(usdc).nonces(_owner);
      }

      if (_feeToken == dai) {
        try IDai(_feeToken).nonces(_owner) returns (uint256 nonce)
        {
            return nonce;
        }
        catch {
          try IDai(_feeToken).getNonce(_owner) returns (uint256 nonce)
          {
              return nonce;
          }
          catch {
            revert BuyMeSomeGas__getPermitNonce_invalidToken();
          }
        }
      }
    ...
    // TODO - Improve this
    function getTokenDS(address _feeToken) public view returns(bytes32) {
      if (_feeToken == usdc) {
        return ERC20Permit(usdc).DOMAIN_SEPARATOR();
      }

      if (_feeToken == dai) {
        try IDai(_feeToken).DOMAIN_SEPARATOR() returns (bytes32 ds)
        {
            return ds;
        }
        catch {
          try IDai(_feeToken).getDomainSeperator() returns (bytes32 ds)
          {
              return ds;
          }
          catch {
            revert BuyMeSomeGas__getTokenDS_invalidToken();
          }
        }
      }
