...
contract GovernanceAutomator is KeeperCompatibleInterface {
    ...
    constructor(
        ...
        s_proposalStartingIndex = (_proposalStartingIndex > 0)
            ? _proposalStartingIndex
            : 1; // proposals start at 1.
        ...
        returns (bool upkeepNeeded, bytes memory performData)
    {
        ...
        for (uint256 i = newStartingIndex; i <= proposalCount; i++) {
            ...
            (, address proposer, , , , , , , ) = s_governanceContract.proposals(
                i
            );
            ...
            if (state == GovernorAlpha.ProposalState.Succeeded) {
                ...
                s_governanceTokenContract.getPriorVotes(
                    proposer,
    ...
    function performUpkeep(bytes calldata performData) external override {
        //Decode performData
        (Action performAction, uint256 proposalIndex) = abi.decode(
            performData,
            (Action, uint256)
        );

        //Check state of proposal at index
        GovernorAlpha.ProposalState state = s_governanceContract.state(
            proposalIndex
        );

        //Revalidate state and action of provided index
        if (
            performAction == Action.QUEUE &&
            state == GovernorAlpha.ProposalState.Succeeded
        ) {
            s_governanceContract.queue(proposalIndex);
        } else if (
            performAction == Action.EXECUTE &&
            ...
            uint256 newStartingIndex = findStartingIndex();
            require(
                newStartingIndex > s_proposalStartingIndex,
                "No update required"
            );
    ...
    function findStartingIndex() public view returns (uint256 index) {
        ...
        for (uint256 i = pendings_proposalStartIndex; i <= proposalCount; i++) {
            uint256 state = uint256(s_governanceContract.state(i));
            if (
                state == uint256(GovernorAlpha.ProposalState.Pending) ||
                state == uint256(GovernorAlpha.ProposalState.Active) ||
                state == uint256(GovernorAlpha.ProposalState.Succeeded) ||
                state == uint256(GovernorAlpha.ProposalState.Queued) ||
                i == proposalCount
            ) {
