...
contract DFC is ERC721Enumerable, Ownable {
    using Strings for uint256;

    string baseURI; // this is the URI pointing to the real NFT arts on IPFS
    string public baseExtension = ".json";
    uint256 public cost = 100 ether; // This is the initial price set by the owner to mint each NFT.
    uint256 public maxSupply = 6779; // Total number of NFT that are available to mint.
    uint256 public maxMintAmount = 5; // The max amount of NFTs that can be mint at a time.
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (whitelistOnly) {
            require(isWhitelisted(msg.sender), "Address is not on whitelisted!");
        }
