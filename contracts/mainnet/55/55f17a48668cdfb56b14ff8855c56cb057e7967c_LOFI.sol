...
contract LOFI is ERC20, AccessControl, ERC721Holder {
    ...
    constructor() ERC20("LOFI", "LOFI")AccessControl(){
        owner = 0xe367A56aFe769172cFBAC6cA2302b53A70687594;
        collectionAddressC1 = 0x1D8fBf2E6EE2fd658D246c89989955822212C604;
    ...
    function stakeC1(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddressC1).ownerOf(tokenId) == address(this),
            "LOFI: Error while transferring token");
    ...
    function harvestC1(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC1 storage infoC1 = stakeLogC1[_msgSender()][tokenId];
        uint256 rewardAmountInLOFI = pendingRewardsC1(_msgSender(), tokenId);
        if(rewardAmountInLOFI > 0) {
            infoC1.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInLOFI);
            emit RewardsHarvestedC1(_msgSender(), rewardAmountInLOFI);
        }
    }
    function harvestC2(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC2 storage infoC2 = stakeLogC2[_msgSender()][tokenId];
        uint256 rewardAmountInLOFI = pendingRewardsC2(_msgSender(), tokenId);
        if(rewardAmountInLOFI > 0) {
            infoC2.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInLOFI);
            emit RewardsHarvestedC2(_msgSender(), rewardAmountInLOFI);
        }
    }
    function harvestC3(uint256 tokenId) public {
        require(!paused, "The contract is paused");
        StakeInfoC3 storage infoC3 = stakeLogC3[_msgSender()][tokenId];
        uint256 rewardAmountInLOFI = pendingRewardsC3(_msgSender(), tokenId);
        if(rewardAmountInLOFI > 0) {
            infoC3.lastHarvestBlock = block.number;
            _mint(_msgSender(), rewardAmountInLOFI);
            emit RewardsHarvestedC3(_msgSender(), rewardAmountInLOFI);
        }
    }
    ...
    function setCollectionAddressC1(address newAddressC1) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC1 != address(0), "LOFI: update to zero address not possible");
    ...
    function setCollectionAddressC2(address newAddressC2) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC2 != address(0), "LOFI: update to zero address not possible");
    ...
    function setCollectionAddressC3(address newAddressC3) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddressC3 != address(0), "LOFI: update to zero address not possible");
