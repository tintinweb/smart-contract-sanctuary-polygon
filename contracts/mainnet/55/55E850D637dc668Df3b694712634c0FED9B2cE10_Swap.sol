...
contract Swap is BaseContract {
  ...
  function swapUSDForToken(uint _usdAmount, ISwap.PaymentCurrency _paymentCurrency, bool _autoStake) external returns (uint) {
    ...
    netAmount = _calculateTax(msg.sender, tokenAmount);
  ...
  function _calculateTax(address _to, uint _value) private returns (uint) {
    if (menToken.getWhitelistTax(_to, IMENToken.TaxType.Claim)) {
      return 0;
    }

    uint baseTax = _value * 1000 / taxManager.totalTaxPercentage();
    (uint[] memory sharkCheckpoints, uint[] memory sharkTaxPercentages ) = menToken.getSharkInfo();

    if (sharkCheckpoints.length > 0) {
      (, uint totalClaimed) = vault.getUserInfo(_to);
      uint checkpointIndex;
      for(uint i = 0; i < sharkCheckpoints.length; i++) {
        if (totalClaimed >= sharkCheckpoints[i]) {
          checkpointIndex = i;
        }
      }
      if (checkpointIndex > 0) {
        return baseTax * (DECIMAL3 - sharkTaxPercentages[checkpointIndex]) / DECIMAL3;
      }
    }

    if(lsd.isQualifiedForTaxDiscount(_to) && menToken.lsdDiscountTaxPercentages(IMENToken.TaxType.Claim) > 0) {
      return baseTax * (DECIMAL3 - menToken.lsdDiscountTaxPercentages(IMENToken.TaxType.Claim)) / DECIMAL3;
    }
