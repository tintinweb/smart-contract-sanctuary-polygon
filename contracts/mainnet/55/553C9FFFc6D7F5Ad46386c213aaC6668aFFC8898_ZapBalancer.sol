...
import "../../BIFI/utils/BalancerUtils.sol";
import "../../BIFI/interfaces/beethovenx/IBalancerVault.sol";


/// Support zap in and out of vault with tokens: WETH, tokens in the balancer pool.
/// Tested with stMatic-Matic pool
/// Please verify it with other pools before using with those pools.
contract ZapBalancer {
    ...
    function beefInETH (address beefyVault, bytes32 inputSwapPoolId, bytes32 wantPoolId, address inputTokenToBalancerPool) external payable {
        require(msg.value >= minimumAmount, "Insignificant input amount");
        IWETH(WETH).deposit{value: msg.value}();
        uint256 nativeBal = IWETH(WETH).balanceOf(address(this));
        _swapAndStake(beefyVault, inputSwapPoolId, wantPoolId, inputTokenToBalancerPool, WETH, nativeBal);
    }

    /// This function only supports zap in the inputTokenToBalancerPool or WNATIVE 
    function beefIn (address beefyVault, bytes32 inputSwapPoolId, bytes32 wantPoolId, address inputTokenToBalancerPool, address tokenIn, uint256 tokenInAmount) public {
        require(tokenInAmount >= minimumAmount, "Insignificant input amount");
        require(IERC20(tokenIn).allowance(msg.sender, address(this)) >= tokenInAmount, "Input token not approved");

        IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), tokenInAmount);

        _swapAndStake(beefyVault, inputSwapPoolId, wantPoolId, inputTokenToBalancerPool, tokenIn, tokenInAmount);    
    }

    /// This function accepts either balancer vault input token or WETH.
    /// Deposit balancer vault required input token to balancer to get lp (want) token
    /// If inp 
    function _swapAndStake(address beefyVault, bytes32 inputSwapPoolId, bytes32 wantPoolId, address inputTokenToBalancerPool, address providedToken, uint256 providedAmount) private {
        require(providedToken == WETH || providedToken == inputTokenToBalancerPool, "input token not supported");
        // in this case provided token must be WETH and diff from inputTokenToBalancerPool 
        // we will swap the provided token to the inputToken
        if (providedToken != inputTokenToBalancerPool) {
            balancerSwap(inputSwapPoolId, providedToken, inputTokenToBalancerPool, providedAmount);
        }

        IBeefyVault vault = IBeefyVault(beefyVault);
        (address want,) = IBalancerVault(router).getPool(wantPoolId); 

        uint256 inputBal = IERC20(inputTokenToBalancerPool).balanceOf(address(this));
        _approveTokenIfNeeded(address(inputTokenToBalancerPool), router);
        balancerSwap(wantPoolId, inputTokenToBalancerPool, want, inputBal);

        // Now deposit want token to vault
        ...
        // just in case pools cannot swap all provided tokens
        address[] memory tokenReturn;
        if (providedToken != inputTokenToBalancerPool) {
            tokenReturn = new address[](2);
            tokenReturn[0] = providedToken;
            tokenReturn[1] = inputTokenToBalancerPool;
        } else {
            tokenReturn = new address[](1);  
            tokenReturn[0] = inputTokenToBalancerPool;
        }       
        _returnAssets(tokenReturn); 
    }

    function beefOut(address beefyVault, bytes32 inputSwapPoolId, bytes32 wantPoolId, address inputTokenToBalancerPool, address tokenOut, uint256 withdrawAmount) external {
        require(tokenOut == WETH || tokenOut == inputTokenToBalancerPool, "tokenOut not supported");
        IBeefyVault vault = IBeefyVault(beefyVault);
        (address want,) = IBalancerVault(router).getPool(wantPoolId); 
        ...
        // swap want to inputTokenToBalancerPool
        uint256 wantBal = IERC20(want).balanceOf(address(this));
        _approveTokenIfNeeded(address(want), router);
        balancerSwap(wantPoolId, want, inputTokenToBalancerPool, wantBal);

        if (tokenOut != inputTokenToBalancerPool) {
            uint256 tokenOutBal = IERC20(tokenOut).balanceOf(address(this));
            _approveTokenIfNeeded(address(tokenOut), router);
            balancerSwap(inputSwapPoolId, inputTokenToBalancerPool, tokenOut, tokenOutBal);

        } 
        address[] memory tokenReturn = new address[](2);
        tokenReturn[0] = inputTokenToBalancerPool;
        tokenReturn[1] = tokenOut;
        _returnAssets(tokenReturn);
    }

    function balancerSwap(bytes32 inputSwapPoolId, address _tokenIn, address _tokenOut, uint256 _amountIn) internal returns (uint256) {
        IBalancerVault.SingleSwap memory singleSwap = IBalancerVault.SingleSwap(inputSwapPoolId, IBalancerVault.SwapKind.GIVEN_IN, _tokenIn, _tokenOut, _amountIn, "");
        IBalancerVault.FundManagement memory funds = IBalancerVault.FundManagement(address(this), false, payable(address(this)), false);
        return IBalancerVault(router).swap(singleSwap, funds, 1, block.timestamp);
    }
