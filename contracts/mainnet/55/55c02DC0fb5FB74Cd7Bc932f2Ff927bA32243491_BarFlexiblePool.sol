...
// File: BarFlexiblePool/interfaces/IBarPool.sol
...
interface IBarPool {
    struct UserInfo {
        ...
        uint256 barAtLastUserAction; // keep track of bar deposited at the last user action.
...
// File: BarFlexiblePool/contracts/utils/Address.sol
...
// File: BarFlexiblePool/contracts/token/ERC20/IERC20.sol
...
// File: BarFlexiblePool/contracts/token/ERC20/utils/SafeERC20.sol
...
// File: BarFlexiblePool/contracts/utils/Context.sol
...
// File: BarFlexiblePool/contracts/security/Pausable.sol
...
// File: BarFlexiblePool/contracts/access/Ownable.sol
...
// File: BarFlexiblePool/BarFlexiblePool.sol
...
contract BarFlexiblePool is Ownable, Pausable {
    using SafeERC20 for IERC20;

    struct UserInfo {
        uint256 shares; // number of shares for a user
        uint256 lastDepositedTime; // keeps track of deposited time for potential penalty
        uint256 barAtLastUserAction; // keeps track of bar deposited at the last user action
        uint256 lastUserActionTime; // keeps track of the last user action time
    }

    IERC20 public immutable token; // Bar token
    IBarPool public immutable barPool; //Bar pool
    ...
    //When call barpool.withdrawByAmount function,there will be a loss of precision, so need to withdraw more.
    uint256 public withdrawAmountBooster = 10001; // 1.0001
    uint256 public performanceFee = 200; // 2%
    uint256 public withdrawFee = 10; // 0.1%
    uint256 public withdrawFeePeriod = 72 hours; // 3 days

    event DepositBar(address indexed sender, uint256 amount, uint256 shares, uint256 lastDepositedTime);
    ...
    /**
     * @notice Constructor
     * @param _token: Bar token contract
     * @param _barPool: BarPool contract
    ...
    constructor(
        ...
        // Infinite approve
        IERC20(_token).safeApprove(address(_barPool), type(uint256).max);
    }
    ...
    /**
     * @notice Deposits funds into the Bar Flexible Pool.
     * @dev Only possible when contract not paused.
     * @param _amount: number of tokens to deposit (in BAR)
     */
    function deposit(uint256 _amount) external whenNotPaused {
        ...
        if (user.shares > 0) {
            ...
            uint256 earnAmount = totalAmount - user.barAtLastUserAction;
            ...
            if (currentPerformanceFee > 0) {
                ...
                if (_amount >= currentPerformanceFee) {
                    ...
                    barPool.withdrawByAmount(withdrawAmount);
        ...
        user.barAtLastUserAction = (user.shares * balanceOf()) / totalShares;
        user.lastUserActionTime = block.timestamp;

        emit DepositBar(msg.sender, _amount, currentShares, block.timestamp);
    }

    /**
     * @notice Withdraws funds from the Bar Flexible Pool
    ...
    /**
     * @notice Withdraws from Bar Pool without caring about rewards.
     * @dev EMERGENCY ONLY. Only callable by the contract admin.
     */
    function emergencyWithdraw() external onlyAdmin {
        require(staking, "No staking bar");
        staking = false;
        barPool.withdrawAll();
    }

    /**
     * @notice Withdraw unexpected tokens sent to the Bar Flexible Pool
     */
     ...
     * @dev It includes tokens held by the contract and held in BarPool
     */
    function balanceOf() public view returns (uint256) {
        (uint256 shares, , , , , , , , ) = barPool.userInfo(address(this));
        uint256 pricePerFullShare = barPool.getPricePerFullShare();

        return token.balanceOf(address(this)) + (shares * pricePerFullShare) / 1e18;
    }

    /**
     * @notice Deposits tokens into BarPool to earn staking rewards
     */
    function _earn() internal {
        uint256 bal = available();
        if (bal > 0) {
            barPool.deposit(bal, 0);
        }
    }
}