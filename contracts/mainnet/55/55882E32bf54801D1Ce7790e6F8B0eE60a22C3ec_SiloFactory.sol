...
contract SiloFactory is Ownable, ERC721Enumerable{
     ...
     */
    function fullSend(address[4] memory _inputs, address[] memory _actions, bytes[] memory _configurationData) external{
        uint siloId = createSilo(address(this));
        setActionStack(siloId, _inputs, _actions, _configurationData);
        safeTransferFrom(address(this), msg.sender, siloId);
    }
    ...
    function createSilo(address recipient) public returns(uint){
        ...
        siloCount+=1;//increment before so there is no zero id silo
