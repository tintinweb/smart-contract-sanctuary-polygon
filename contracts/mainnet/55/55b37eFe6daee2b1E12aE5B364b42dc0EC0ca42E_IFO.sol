...
contract IFO is ReentrancyGuard {
    ...
    uint256 upperLimit = 800 * 1e6;
    uint256 lowerLimit = 200 * 1e6;
    ...
    constructor(IERC20 _lpToken, IERC20 _offeringToken) {
        ...
        // IFO starts on: Wed Jun 29 2022 12:00:00 GMT+0000
        startTime = 1656504000;

        // IFO ends on: Thu Jul 07 2022 08:00:00 GMT+0000
        endTime = 1657180800;

        // Total offer amount: 1.8bil
        offeringAmount = 1800000000 * 10 ** 18;

        // Total raise amount to 1.44mil USDC
        raisingAmount = 1440000 * 10 ** 6;
    ...
    function setFloorPrice(uint256 _newAmount) public onlyAdmin {
        floorPrice = _newAmount;
    }
    ...
    function deposit(uint256 _amount) public {
        ...
        require(_amount >= lowerLimit, "Amount transfer less than 200$");
        require(_amount <= upperLimit, "Amount transfer greater than 800$");
        require(
            (records[msg.sender].add(_amount)) <= upperLimit,
            "Upper limit for account reached"
        );

        lpToken.transferFrom(address(msg.sender), address(this), _amount);
        offeringToken.transfer(address(msg.sender), _amount.div(floorPrice).mul(1e12).mul(1e18));
