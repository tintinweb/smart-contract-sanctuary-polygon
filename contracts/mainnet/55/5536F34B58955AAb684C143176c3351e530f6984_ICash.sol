...
contract DividendDistributor is IDividendDistributor {
    ...
    modifier onlyAuth() {
        require(msg.sender == _authorized); _;
    }
    ...
    function setContractAddress(address tokenAddr) external onlyAuth {
        require(msg.sender == _authorized, "UNAUTHORIZED");
        _token = address(tokenAddr);
    }

    function setAuthorizedAddress(address authAddr) external onlyAuth {
        require(msg.sender == _authorized, "UNAUTHORIZED");
        _authorized = address(authAddr);
    }

    function rescueStuckTokens(address _tok, address payable recipient, uint256 amount) public payable onlyAuth {
        require(msg.sender == _authorized, "UNAUTHORIZED");
    ...
    function rescueStuckNative(address payable recipient) public payable onlyAuth {
        require(msg.sender == _authorized, "UNAUTHORIZED");
    ...
    function changeRouter(address _newRouter) external onlyAuth {
