...
contract Factory is Ownable, Helper {
    using SafeERC20 for IERC20;
    ...
    function createERC20DAO(
        ...
        if (_gnosisAddress == address(0))
            revert AddressInvalid("_gnosisAddress", _gnosisAddress);
        ...
        address _daoAddress = address(
            new ProxyContract(ERC20Implementation, owner, data)
        );
    ...
    function createERC721DAO(
        ...
        address owner = owner();
        address _daoAddress = address(
            new ProxyContract(ERC721Implementation, owner, data)
        );
    ...
    /// @dev Function to update distribution amount
    ...
    /// @dev Function to update price per token
    ...
    function ifTokenGatingApplied(address _daoAddress) private view {
        ...
        for (uint i; i < conditions.length; ) {
            if (uint8(conditions[i].operator) == 0) {
                if (uint8(conditions[i].comparator) == 0) {
                    require(
                        IERC20(conditions[i].tokenA).balanceOf(msg.sender) >
                            conditions[i].value[0],
                        "Insufficient balance"
                    );
                    require(
                        IERC20(conditions[i].tokenB).balanceOf(msg.sender) >
                            conditions[i].value[1],
                        "Insufficient balance"
                    );
                } else if (uint8(conditions[i].comparator) == 1) {
                    require(
                        IERC20(conditions[i].tokenA).balanceOf(msg.sender) <
                            conditions[i].value[0],
                        "Insufficient balance"
                ...
                if (uint8(conditions[i].comparator) == 0) {
                    ...
                    require(
                        IERC20(conditions[i].tokenA).balanceOf(msg.sender) ==
                            conditions[i].value[0] ||
                            IERC20(conditions[i].tokenB).balanceOf(
                                msg.sender
                            ) ==
                            conditions[i].value[1],
                        "Insufficient balance"
