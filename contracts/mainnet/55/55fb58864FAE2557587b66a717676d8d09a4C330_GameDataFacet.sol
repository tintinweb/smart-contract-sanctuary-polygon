...
contract GameDataFacet is ChainlinkClient{
   ...
   event ScoreUpdated(string gameId);
    ...
    function initializeGameData() external {
        if(initialized != true){
            ...
            c.linkTokenAddress = 0xb0897686c545045aFc77CF20eC7A532E3120E0F1;
            c.oracle = 0x7420463A58A987BD884221320192340240483415;
            setChainlinkToken(c.linkTokenAddress);
            c._LINK = IERC20(c.linkTokenAddress);
    ...
    function getGameFromMap(string memory _gameId) external view returns(BigBoardManagerLib.GameData memory game){
...
import "./BoardToken.sol";
import "./GameDataFacet.sol";
import "./BigBoardManagerLib.sol";

contract BoardManagerFacet {
...
contract Board is Ownable{
    ...
    BigBoardManagerLib.GameData public game;    
    uint256[4] homeQ;
    uint256[4] awayQ;
    ...
    BigBoardManagerLib.Winner[] public winners;
    ...
    function buySquare(BigBoardManagerLib.Cell[] memory potentialSquarez, string memory tag) external payable {
        
        require(boardStatus != BoardStatus.CANCELED, "CANCELED.");
        require(boardStatus != BoardStatus.SOLDOUT, "SOLDOUT.");
        require(buyerSquareCount[msg.sender] < 7,"MAXED OUT.");
        //BigBoardManagerLib.Square memory selectedSquare = squareBuyerz[row][cell]; 
        // if selectedSquare is not already purchased 
        for (uint256 index = 0; index < potentialSquarez.length; index++) {
            if(squareBuyerz[potentialSquarez[index].row][potentialSquarez[index].cell].purchaser == address(0x0)){
            // is available
            require(feeToken.transferFrom(msg.sender, address(this), boardFee), "Fee required.");
            squareBuyerz[potentialSquarez[index].row][potentialSquarez[index].cell].purchaser = payable(msg.sender);
            squareBuyerz[potentialSquarez[index].row][potentialSquarez[index].cell].squareTag = tag;
            squareBuyerz[potentialSquarez[index].row][potentialSquarez[index].cell].row = potentialSquarez[index].row;
            squareBuyerz[potentialSquarez[index].row][potentialSquarez[index].cell].cell = potentialSquarez[index].cell;
            buyerSquareCount[msg.sender] = buyerSquareCount[msg.sender] + 1;
            availableSquarezCount--;
            emit SquarePurchased(msg.sender, potentialSquarez[index].row, potentialSquarez[index].cell);
    ...
    function selectWinners() public adminOrHostOnly {
        ...
        gameDataFacet = GameDataFacet(admin);
        BigBoardManagerLib.GameData memory curGame = gameDataFacet.getGameFromMap(gameId);
        for (uint256 index = 0; index < 4; index++) {
            uint fRow = downIndexes[curGame.score_away_by_period[index] % 10];
            uint fCell = acrossIndexes[curGame.score_home_by_period[index] % 10];
            uint bRow = downIndexes[curGame.score_home_by_period[index] % 10];
            uint bCell = acrossIndexes[curGame.score_away_by_period[index] % 10];

            BigBoardManagerLib.Winner memory fWinner;
            fWinner.row = fRow;
            fWinner.col = fCell;
            fWinner.winner = squareBuyerz[fRow][fCell].purchaser;
            fWinner.fPrize = forwardPrize;
            winners.push(fWinner);
            BigBoardManagerLib.Winner memory bWinner;
            bWinner.row = bRow;
            bWinner.col = bCell;
            bWinner.winner = squareBuyerz[bRow][bCell].purchaser;
            bWinner.bPrize = backwardPrize;
    ...
    function collectPrize() public winnerOnly {
        require(boardStatus == BoardStatus.WINNERSSELECTED, 'No Winners Selected');
        ...
        if(prizeCollectedByWinner[msg.sender]){
            ...
            for (uint256 i = 0; i < winners.length; i++) {
                if(winners[i].winner == msg.sender){
                    total += winners[i].fPrize;
                    total += winners[i].bPrize;
                }
            }
            
            //uint256 total = winners[msg.sender].totalFWinnings + winners[msg.sender].totalBWinnings;
            if(total > 0 ){
                //
    ...
    function isWinner(address _addy) internal view returns(bool winner){
        winner = false;
        for (uint256 index = 0; index < 8; index++) {
            if(winners[index].winner == _addy){
                winner = true;
                return winner;
            }
        }
...
import { Board } from "./BoardManagerFacet.sol";
import "./BoardToken.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
...
library BigBoardManagerLib {
    ...
    struct ChainlinkNode {
        ...
        address linkTokenAddress;// = 0x326C977E6efc84E512bB9C30f76E30c160eD06FB;// LINK token address change to 0xb0897686c545045afc77cf20ec7a532e3120e0f1 for mainnet
    ...
    function getGameFromScores(string memory _gameId) internal view returns(BigBoardManagerLib.GameData storage){
        Factory storage f = getFactory();        
         
        return f.scores[_gameId];
    }
    ...
    function isAirdopEnabled() internal view returns(bool){
       return getFactory().airdropEnabled;
    }
