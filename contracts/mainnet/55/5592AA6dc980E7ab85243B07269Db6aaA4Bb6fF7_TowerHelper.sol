...
interface IFarm {
    ...
    function cubePerSecond() external view returns (uint256);

    function totalAllocPoint() external view returns (uint256);
}

interface ICubeStake {
    ...
    function epoch() external view returns (EpochDistribution memory);
}

contract TowerHelper is TowerProtocol {
    IERC20 public twr;
    IERC20 public cube;
    IERC20 public usdc;
    ITwapOracle public twrTwap;
    ITwapOracle public cubeTwap;
    IPriceOracle public oracle;
    IBankSafe public safe;
    ICubeStake public stake;
    IFarm public farm;
    IUniswapV2Pair public twrPair;
    IUniswapV2Pair public cubePair;
    address public dustbin;
    address public treasury;

    function setContracts1(
        address _twr,
        address _cube,
        address _usdc,
        address _twrTwap,
        ...
        cube = IERC20(_cube);
        usdc = IERC20(_usdc);
        twrTwap = ITwapOracle(_twrTwap);
        cubeTwap = ITwapOracle(_cubeTwap);
        oracle = IPriceOracle(_priceOracle);
        safe = IBankSafe(_bankSafe);
    }

    function setContracts2(
        address _cubeStake,
        address _farm,
        address _twrPair,
        address _cubePair,
        ...
        stake = ICubeStake(_cubeStake);
        farm = IFarm(_farm);
        twrPair = IUniswapV2Pair(_twrPair);
    ...
    function twrInfo() public view returns (TokenInfo memory tokenInfo) {
        tokenInfo = TokenInfo({
            ts: _currentBlockTs(),
            price: oracle.towerPrice(),
            updatedAt: twrTwap.blockTimestampLast(),
            totalSupply: twr.totalSupply(),
            marketCap: (twr.totalSupply() * oracle.towerPrice()) /
                TOWER_PRECISION
        });
    }

    function cubeInfo() public view returns (TokenInfo memory tokenInfo) {
        tokenInfo = TokenInfo({
            ts: _currentBlockTs(),
            price: oracle.cubePrice(),
            updatedAt: cubeTwap.blockTimestampLast(),
            totalSupply: cube.totalSupply(),
            marketCap: (cube.totalSupply() * oracle.cubePrice()) /
                TOWER_PRECISION
        });
    }

    function safeBalance() public view returns (uint256 balance) {
        balance = safe.globalCollateralBalance();
    }

    function stakeBalance() public view returns (uint256 balance) {
        uint256 _stakingCube = cube.balanceOf(address(stake));
        uint256 _price = oracle.cubePrice();
        balance = (_stakingCube * _price) / CUBE_PRECISION;
    }

    function twrPrice() public view returns (uint256 price) {
        uint112 _resv0;
        uint112 _resv1;
        (_resv0, _resv1, ) = twrPair.getReserves();

        uint256 _price = twrPair.token0() == address(twr)
            ? (_resv1 * TOWER_PRECISION) / _resv0
            : (_resv0 * TOWER_PRECISION) / _resv1;

        price = (oracle.collatPrice() * _price) / PRICE_PRECISION;
    }

    function cubePrice() public view returns (uint256 price) {
        uint112 _resv0;
        uint112 _resv1;
        (_resv0, _resv1, ) = cubePair.getReserves();

        uint256 _price = cubePair.token0() == address(cube)
            ? (_resv1 * CUBE_PRECISION) / _resv0
            : (_resv0 * CUBE_PRECISION) / _resv1;

        price = (oracle.collatPrice() * _price) / PRICE_PRECISION;
    }

    function lpPrice(address _lpAddr) public view returns (uint256 price) {
        IUniswapV2Pair _lpPair = IUniswapV2Pair(_lpAddr);
        price = 0;

        if (_lpPair.token0() == address(usdc)) {
            (uint256 _resv, , ) = _lpPair.getReserves();
            price =
                (_resv * oracle.collatPrice() * 2 * MISSING_PRECISION) /
                _lpPair.totalSupply();
        } else if (_lpPair.token1() == address(usdc)) {
            (, uint256 _resv, ) = _lpPair.getReserves();
            price =
                (_resv * oracle.collatPrice() * 2 * MISSING_PRECISION) /
                _lpPair.totalSupply();
        } else if (_lpPair.token0() == address(twr)) {
            (uint256 _resv, , ) = _lpPair.getReserves();
            price = (_resv * twrPrice() * 2) / _lpPair.totalSupply();
        } else if (_lpPair.token1() == address(twr)) {
            (, uint256 _resv, ) = _lpPair.getReserves();
            price = (_resv * twrPrice() * 2) / _lpPair.totalSupply();
        }
    }
    ...
    function farmBalance() public view returns (uint256 balance) {
        (, balance) = farmInfo();
    }
    ...
    function farmInfo()
        public
        view
        returns (FarmInfo[] memory infos, uint256 tvl)
    {
        uint256 _cubePrice = cubePrice();
        uint256 _cubePerSecond = farm.cubePerSecond();
        uint256 _totalAllocPoint = farm.totalAllocPoint();
        uint256 _poolLength = farm.poolLength();
        infos = new FarmInfo[](_poolLength);

        for (uint256 i = 0; i < _poolLength; i++) {
            IFarm.PoolInfo memory _info = farm.poolInfo(i);
            address _lpAddr = farm.lpToken(i);
            uint256 _lpPrice = lpPrice(_lpAddr);
            uint256 _lpStaked = IERC20(_lpAddr).balanceOf(address(farm));
            uint256 _tvl = (_lpStaked * _lpPrice) / TOWER_PRECISION;
            tvl = tvl + _tvl;

            infos[i] = FarmInfo({
                farmAddr: address(farm),
                id: i,
                lpAddr: _lpAddr,
                rewardToken: address(cube),
                tvl: _tvl,
                lpPrice: _lpPrice,
                lpStaked: _lpStaked,
                rewardPrice: _cubePrice,
                rewardPerSecond: (_cubePerSecond * _info.allocPoint) /
                    _totalAllocPoint
            });
        }
    }

    function borrowedBalance() public view returns (uint256 value) {
        value =
            (cubePair.balanceOf(dustbin) * lpPrice(address(cubePair))) /
            TOWER_PRECISION;
    }

    function ownedBalance() public view returns (uint256 value) {
        value =
            (cubePair.balanceOf(treasury) * lpPrice(address(cubePair))) /
            TOWER_PRECISION;
    }
    ...
    function info()
        ...
        view
        returns (
            TokenInfo memory twrInfoOut,
            TokenInfo memory cubeInfoOut,
            uint256 tvl,
            uint256 farmTvl,
            uint256 stakeTvl,
            uint256 safeTvl,
    ...
    {
        twrInfoOut = twrInfo();
        cubeInfoOut = cubeInfo();
        farmTvl = farmBalance();
        stakeTvl = stakeBalance();
        safeTvl = safeBalance();
        tvl = farmTvl + stakeTvl + safeTvl;
    ...
    function stakeInfo()
        ...
        returns (
            uint256 totalSupply,
            uint256 staked,
            uint256 cubePriceOut,
            uint256 cubePerShare,
            uint256 cubePerSec
        )
    {
        totalSupply = cube.totalSupply();
        staked = cube.balanceOf(address(stake));
        cubePriceOut = cubePrice();
        cubePerShare = stake.cubePerShare();
        cubePerSec = stake.epoch().stepAmount;
    }
}
