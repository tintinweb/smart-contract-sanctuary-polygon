...
/**
// @title Ondefy Router
// @notice Act as the registry for DEX aggregator adapters
// @dev Admin functions ony called by owner
// @author Ondefy
*/
contract Router is Ownable {
    ...
    */
    function callAdapter(uint256 _adapterIndex, address _inputToken, uint256 _inputTokenAmount, address _outputToken, bytes memory _swapCallData) public payable {
        require(adapters[_adapterIndex].deployedContract != address(0), "Adapter not deployed");
        require(adapters[_adapterIndex].isActivated, "Adapter is not activated");
    ...
    */
    function activateAdapter(uint256 index) public onlyOwner {
        require(adapters[index].deployedContract != address(0), "Adapter not deployed");
        require(!adapters[index].isActivated, "Already activated");
    ...
    */
    function deactivateAdapter(uint256 index) public onlyOwner {
        require(adapters[index].deployedContract != address(0), "Adapter not deployed");
        require(adapters[index].isActivated, "Already deactivated");
    ...
    // @param _deployedContract address of the deployed contract
    // @param _isActivated true for activating right away, false otherwise
    // @dev no adapter must be already present at given index
     */
    function addAdapter(uint256 index, address _deployedContract, bool _isActivated) public onlyOwner {
        require(adapters[index].deployedContract == address(0), "Adapter already exists at given index");
    ...
    */
    function setFeeRate(uint256 index, uint8 _feeRate) public onlyOwner {
        AdapterBase(payable(adapters[index].deployedContract)).setFeeRate(_feeRate);
    }
    ...
    */
    function rescueFunds(uint256 index, address token, address recipient, uint256 amount) public onlyOwner {
