...
contract KishSwapRouter is IKishSwapRouter, Ownable, Permitable {
  ...
  function _addLiquidity(
    ...
    (uint256 reserveA, uint256 reserveB) = IKishSwapFormula(formula).getReserves(pair, tokenA, tokenB);
    if (reserveA == 0 && reserveB == 0) {
      (amountA, amountB) = (amountADesired, amountBDesired);
    } else {
      uint256 amountBOptimal = IKishSwapFormula(formula).quote(amountADesired, reserveA, reserveB);
      if (amountBOptimal <= amountBDesired) {
        require(amountBOptimal >= amountBMin, "Router: INSUFFICIENT_B_AMOUNT");
        (amountA, amountB) = (amountADesired, amountBOptimal);
      } else {
        uint256 amountAOptimal = IKishSwapFormula(formula).quote(amountBDesired, reserveB, reserveA);
  ...
  function createPair(
    ...
    address pair = IKishSwapFactory(factory).createPair(tokenA, tokenB, tokenWeightA, swapFee);
    _addLiquidityToken(pair, tokenA, tokenB, amountA, amountB, 0, 0);
    liquidity = IKishSwapPair(pair).mint(to);
  }
  ...
  function createPairETH(
    ...
    address pair = IKishSwapFactory(factory).createPair(token, WETH, tokenWeight, swapFee);
  ...
  function _swap(
    ...
    for (uint256 i = 0; i < path.length; i++) {
      IKishSwapPair pairV2 = IKishSwapPair(path[i]);
    ...
    for (uint256 i; i < path.length; i++) {
      ...
      address currentOutput;
      {
        (address output, uint256 reserveInput, uint256 reserveOutput, uint32 tokenWeightInput, , uint32 swapFee) =
          IKishSwapFormula(formula).getFactoryReserveAndWeights(factory, path[i], tokenIn, dexIds[i]);
        uint256 amountInput = IERC20(tokenIn).balanceOf(path[i]).sub(reserveInput);
        amountOutput = IKishSwapFormula(formula).getAmountOut(amountInput, reserveInput, reserveOutput, tokenWeightInput, 100 - tokenWeightInput, swapFee);
        currentOutput = output;
      }

      IKishSwapPair pair = IKishSwapPair(path[i]);
  ...
  function _validateAmountOut(
    ...
    amounts = IKishSwapFormula(formula).getFactoryAmountsOut(factory, tokenIn, tokenOut, amountIn, path, dexIds);
  ...
  function _validateAmountIn(
    ...
    amounts = IKishSwapFormula(formula).getFactoryAmountsIn(factory, tokenIn, tokenOut, amountOut, path, dexIds);
  ...
  function _removeLiquidity(
    ...
    require(IKishSwapFactory(factory).isPair(pair), "Router: Invalid pair");
    IKishSwapPair(pair).transferFrom(msg.sender, pair, liquidity);
    // send liquidity to pair
    (uint256 amount0, uint256 amount1) = IKishSwapPair(pair).burn(to);
    (address token0, ) = IKishSwapFormula(formula).sortTokens(tokenA, tokenB);
  ...
  function removeLiquidityWithPermit(
  ...
  ) external virtual override ensure(deadline) returns (uint256 amountA, uint256 amountB) {
    {
      ...
      IKishSwapPair(pair).permit(msg.sender, address(this), value, deadline, v, r, s);
    }
    (amountA, amountB) = _removeLiquidity(pair, tokenA, tokenB, liquidity, amountAMin, amountBMin, to);
  }
