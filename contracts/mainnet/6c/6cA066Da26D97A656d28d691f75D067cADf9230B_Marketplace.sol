...
contract Marketplace is BaseContract {
  ...
  function makeOrder(
    ...
    if (_orderType == IMarketPlace.OrderType.renting) {
      require(_duration > 0, "Marketplace: duration must be > 0");
    } else if (_orderType == IMarketPlace.OrderType.installmentRental) {
      require(_weekNumber > 0, "Marketplace: weekNumber must be > 0");
    }
