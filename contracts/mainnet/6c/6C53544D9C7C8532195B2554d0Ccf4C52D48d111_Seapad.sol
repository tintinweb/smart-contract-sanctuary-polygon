...
contract Seapad is VRFV2WrapperConsumerBase, ChainlinkClient {
    ...
    function openPack(uint256 projectid, uint256 packSoldID_) public payable {
        ...
        uint256 amountRemaining = msg.value - ethToPay;

        //refund any remaining eth from this transaction to the sender.
        if (amountRemaining > 0) {
            payable(tx.origin).transfer(amountRemaining);
        }
    ...
    */
    ...
    function addAlreadyMinted(uint256 projectID, uint256[] calldata itemIDs)
        public
        onlyOwner
    {
        isValidProject(projectID);
        Projects storage project = projects[projectID];
        //only the contract can call this or the seapad owner.
        for (uint256 i = 0; i < itemIDs.length; i++) {
            uint256 randomResult = itemIDs[i];
            uint256 itemsAmounts = project.itemsLeft;
            if (randomResult == 0) {
                randomResult = 1;
            }
            require(project.items[randomResult] == 0, "Item already minted");
            project.items[randomResult] = itemsAmounts;
            project.itemsLeft--;
        }
        require(
            project.itemsLeft % project.amountToPayOutPerPack == 0,
            "Not divisable"
        ); //not divisible
    }
    ...
    function createToken(
        ...
        if (customContract != address(0)) {
            require(cce[customContract], "6"); //invalid custom contract //needs to be enabled by admin
    ...
    function createTokenPart2(
        ...
        //get the packBackgroundImageUrl link extension
        project.pricePerPack = pricePerPack;
