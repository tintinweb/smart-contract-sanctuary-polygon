    ...
    ERC721WithPermitUpgradable
{
    ...
    function buy(
        ...
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_], referral_, referralBonus);
    ...
    function batchMint(uint256 typeNFT_, uint256 quantity_, address recipient_) external onlyRole(MINTER_ROLE) {
        _setSold(typeNFT_, quantity_);
        uint256 tokenId = _idCounter;
        emit Registered(recipient_, typeNFT_, tokenId, quantity_, _sold[typeNFT_], address(0), 0);
        _batchProcess(
            recipient_,
            tokenId,
            quantity_,
