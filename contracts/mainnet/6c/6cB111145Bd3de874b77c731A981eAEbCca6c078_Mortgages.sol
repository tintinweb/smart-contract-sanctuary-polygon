...
contract Mortgages is AccessControl, ReentrancyGuard, IMortgageInterest, IERC721Receiver {
    ...
    address public guardian = 0x988F94C0Ef61815AacD775789Af297713408D3B8; // relayer mumbai: 0x1921a154365A82b8d54a3Cb6e2Fd7488cD0FFd23;
    ...
    address vaultRewards, address RewardsLenders, address _control){
    ...
    function openMortgage(address collection, uint256 loan, uint256 period,address token) public validToken(token) NotPaused returns(uint256 id){
    ...
    function batchMortgage(address collection, uint256 amount,uint256[] calldata loan, uint256[] calldata period,address token) public validToken(token) NotPaused{
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
    ...
    function calcsDeposits(uint256 loan,uint256 downPay, uint256 _price, address token) private returns(uint256 debt){
        LocalMortgage memory locals;
        LocalVaults memory vaults;
    ...
    function openPresaleMortgage (address collection/* , address user */, uint256 loan,uint256 period,address token) public NotPaused returns(uint256 id) {
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
        LocalVaults memory vaults;
    ...
    uint256 period,address token) public NotPaused returns(uint256 id){
     LocalMortgage memory locals;
     LocalMortgage2 memory locals2;
     LocalMortgage3 memory locals3;
     LocalVaults memory vaults;
    ...
    function seizeNFT(uint256 idMortgage,address collection, address user, uint256 tokenId) public NotPaused onlyRelayer{
