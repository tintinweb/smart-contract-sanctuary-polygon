...
contract Swap{
    ...
    function getPriceSell(address _sender, uint256 _amount, uint256 _slippage) public view returns(uint256 _Price){
        (uint256 receivedBaseAmount,) = IDODOV2(dodoV2Pool).querySellQuote(_sender, _amount);
        uint256 minReturnAmount = receivedBaseAmount.mul(100 - _slippage).div(100);
        return(minReturnAmount);
    }
    function getPriceBuy(address _sender, uint256 _amount, uint256 _slippage) public view returns(uint256 _Price){
        (uint256 receivedQuoteAmount,) = IDODOV2(dodoV2Pool).querySellBase(_sender, _amount);
        uint256 minReturnAmount = receivedQuoteAmount.mul(100 - _slippage).div(100);
        return(minReturnAmount);
    }

    address dodoV2Pool = 0x9723520d16690075e80cd8108f7C474784F96bCe; //ISLAMI pool on DODO
    //address fromToken = 0x9c891326Fd8b1a713974f73bb604677E1E63396D; //Polygon ISLAMI
    ...
    function useDodoSwapV2(address fromToken, address toToken, uint256 _amount, uint256 _slippage, uint256 directions) internal {
        ...
        if(directions == 0){
            minAmount = getPriceBuy(msg.sender, _amount, slippage);
        }
        else{
            minAmount = getPriceSell(msg.sender, _amount, slippage);
        }
...
*/
    ...
    function ISLAMIprice() public view returns(uint256 _price){
        _price = getPriceSell(address(this), 10000000, 1);
        return(_price);
    }
    function swapISLAMI(uint256 _type, uint256 _amount, uint256 _slippage) external{
        string memory Type;
        address _fromToken;
        address _toToken;
        uint256 amount;
        ...
        for(uint i = 0; i < OrderDetails.length; i++){
            if(OrderDetails[i].sB == msg.sender){
                ...
                //if(block.timestamp < p2p[msg.sender].orderLife.sub(410)){ // (45 hours){
                    monopoly[msg.sender] = block.timestamp.add(1 hours);
                //}
                break;
            }
        }
