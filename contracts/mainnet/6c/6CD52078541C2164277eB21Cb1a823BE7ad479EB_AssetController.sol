...
contract AssetController is Operator, IAssetController {
    ...
    address public collateralPolicy;
    address public pool;
    ...
    function initialize(
        ...
        require(_collateralPolicy != address(0), "Invalid Address");
        require(_pool != address(0), "Invalid Address");
        require(_share != address(0), "Invalid Address");
        require(_router != address(0), "Invalid Address");

        treasury = _treasury;
        collateralPolicy = _collateralPolicy;
        pool = _pool;
        xShare = _share;
        router = _router;
        usdc = _usdc;
    ...
    function addAsset(address _asset, address _collateral, address _oracle, bool _isStable, uint256 _price_band, uint256 _init_tcr, uint256 _init_ecr) public onlyOperator {
        ...
        ICollateralPolicy(collateralPolicy).addCollateralPolicy(assetCount, _price_band, missingDecimals, _init_tcr, _init_ecr);
    ...
    function setCollateral(address _collateral, uint256 _assetId) public onlyOperator {
        ...
        uint256 missingDecimals = uint256(18).sub(ERC20(_collateral).decimals());
        ICollateralPolicy(collateralPolicy).setMissingDecimals(missingDecimals, _assetId);
    }
