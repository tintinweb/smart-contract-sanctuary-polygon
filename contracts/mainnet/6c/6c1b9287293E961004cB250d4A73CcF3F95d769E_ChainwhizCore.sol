...
contract ChainwhizCore is ReentrancyGuard {
    ...
    event UnstakeAmountSet(address publisher, string bountyUrl);
    ...
    /// @param _bountyUrl the github url
    /// @param _description the bounty given to solver
    /// @param _evaluationCriteria the bounty given to solver
    ...
    function postBounty(
        ...
        emit BountyPosted(msg.sender, _bountyUrl, _bountyTitle, _tags);
        //To avoid stack-too-deep error
        emit AdditionalBountyDetail(
            msg.sender,
            _bountyUrl,
            _description,
            _evaluationCriteria
    ...
    function postSolution(
        ...
        require(_publisherAddress != msg.sender, "POST_SOLUTION_A");
        ...
        emit SolutionSubmitted(_solutionLink, _publisherAddress, _bountyUrl);
        return true;
    }
    ...
    function stakeVote(
        ...
        Bounty storage bounty = bountyDetail[_publisherAddress][_bountyUrl];
        require(
            bounty.solverRewardAmount != 0 && bounty.isCommunityVote,
            "STAKE_VOTE_B"
        );
        ...
        if (bounty.questionStatus == QuestionStatus.Solve)
            bounty.questionStatus = QuestionStatus.Vote;
        //Check if voter has alreay voted in any solution
        bool voterVoted = _checkAlreadyVoted(bounty.voterAddress, msg.sender);
        ...
        //emit event
        emit VoteStaked(_bountyUrl, _solutionLink, msg.sender, msg.value);
    }
    ...
    function setUnstakeAmount(
        ...
        require(
            bounty.endVoteTime <= block.timestamp && !bounty.isUnstakeSet,
            "SET_UNSTAKE_B"
        );
    ...
    function payBackPublisher(
        ...
        if (flag) {
            _transferFunds(
                ...
                bounty.solverRewardAmount + bounty.communityVoterRewardAmount,
                isMatic,
                bounty.tokenName
            );
        }
