...
contract Kernel is Initializable, AppManager, IKernel, __with_semver(uint8(1), uint8(1), uint8(0)) {
    ...
    //TODO remove after migration
    function migrateSales(bytes32 [] memory tierValues) external requirePermission(KERNEL_ADMIN){
        ...
        _addPermission(AppsIds.KERNEL, AppsIds.SALES_CONTROLLER, 0);
        address openSale = _appInfo[AppsIds.ERC721_OPEN_SALE].addr;

        if(openSale!=address(0)){

            bool [] memory activity = new bool[](tierValues.length);
            for(uint i; i<tierValues.length; i++){
                if(ERC721BaseSale(openSale).tierPrices(tierValues[i])>0){
                    activity[i] = true;
                } else {
                    activity[i] = false;
                }
            ...
            _addPermission(AppsIds.KERNEL, AppsIds.ERC721_OPEN_SALE, 0);
            ERC721BaseSale(openSale).setSalesController(address(salesController));

            SalesController(salesController).executeSingle(openSale, abi.encodeCall(ERC721BaseSale.setTimeOfSale, (1669030634, 1984649834))); //// from 21.11.2022 to 21.11.2032
            SalesController(salesController).executeSingle(openSale, abi.encodeCall(ERC721BaseSale.setTiersActive, (tierValues, activity)));

            _removePermission(AppsIds.KERNEL, AppsIds.ERC721_OPEN_SALE, 0);
        }
        ...
        if(whiteList!=address(0)){
            bool [] memory activity = new bool[](tierValues.length);
            for(uint i; i<tierValues.length; i++){
                if(ERC721BaseSale(whiteList).tierPrices(tierValues[i])>0){
                    activity[i] = true;
                } else {
                    activity[i] = false;
                }
            ...
            _addPermission(AppsIds.KERNEL, AppsIds.ERC721_WHITELIST_SALE, 0);
            ERC721BaseSale(whiteList).setSalesController(address(salesController));
            SalesController(salesController).executeSingle(whiteList, abi.encodeCall(ERC721BaseSale.setTimeOfSale, (1669030634, 1984649834))); // from 21.11.2022 to 21.11.2032
            SalesController(salesController).executeSingle(whiteList, abi.encodeCall(ERC721BaseSale.setTiersActive, (tierValues, activity)));
            _removePermission(AppsIds.KERNEL, AppsIds.ERC721_WHITELIST_SALE, 0);
        }
        _removePermission(AppsIds.KERNEL, AppsIds.SALES_CONTROLLER, 0);
    }
