...
contract raffleLiquidityPool is Ownable {
    ...
    // EVENTS
    event UpdateUserBalance (address indexed _user, uint256 indexed _raffleID, uint256 _oldBalance, uint256 _newBalance);
    event UpdateRaffleBalance (uint256 indexed _raffleID, uint256 indexed _oldBalance, uint256 indexed _newBalance);
    event UpdateRaffleStatus (uint256 indexed _raffleID, uint8 indexed _status);
    event NewRaffleAddress (address indexed _oldRaffleAddress, address indexed _newRaffleAddress);
    event NewUSDTAddress (address indexed _oldUSDTAddress, address indexed _newUSDTAddress);
    event NewLiquidityPoolAddress (address indexed _oldLiquidityPoolAddress, address indexed _newLiquidityPoolAddress);
    event NewPremiumLiquidityPoolAddress (address indexed _oldPremiumLiquidityPoolAddress, address indexed _newPremiumLiquidityPoolAddress);
    event NewLiquidityPoolPercentage (uint256 indexed _oldLiquidityPoolPercentage, uint256 indexed _newLiquidityPoolPercentage);
    event NewPremiumLiquidityPoolPercentage (uint256 indexed _oldPremiumLiquidityPoolPercentage, uint256 indexed _newPremiumLiquidityPoolPercentage);
    ...
    // ##### PUBLIC #####
    function setOwner (address _withdrawUser) public {
        require(!ownerSetted, "Owner already setted");
        _transferOwnership(msg.sender);
        withdrawUser = _withdrawUser;
        ownerSetted = true;
    }
    ...
    // ##### VIEW #####
    function getRafflesStatus (uint256 [] memory _raffles) public view returns (uint256 [] memory, address [] memory) {
        uint256 length_ = _raffles.length;

        uint256 [] memory status_ = new uint256[](length_);
        address [] memory winner_ = new address[](length_);

        for (uint256 i = 0; i < length_; i++) {
            uint256 raffle_ = _raffles[i];
            status_[i] = raffleStatus[raffle_];
            winner_[i] = raffleInstance.getRaffleWinner(raffle_);
        }
    ...
    // ##### ONLY OWNER #####
    function withdraw (address _to, uint256 [] memory _raffleIDs) public {
        require(msg.sender == withdrawUser, "User not approved");
    ...
    function setWithdrawUser (address _newWithdrawUser) public {
        require(msg.sender == withdrawUser, "User not approved");
        withdrawUser = _newWithdrawUser;
    }
    // ###################
}