...
contract HOMToken is BEP20, MinterRole {
    ...
    mapping (address => uint256) public buyfirsttime;
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
        ...
        if (sender == owner() || recipient == owner() || isExcludedFromFee[sender] || isExcludedFromFee[recipient]) {
            ...
            if( buyfirsttime[sender] == 0)
            {
                buyfirsttime[sender] = block.timestamp;
            }
            ...
            if(automatedMarketMakerPairs[recipient] && sellFee > 0) {
                ...
                if(block.timestamp < buyfirsttime[sender] + ( 86400 * 30 *20 ))  //20 months
                {
                    if(selltime[sender] == 0)
                    {
                        ...
                        selltime[sender] = buyfirsttime[sender] + ((( block.timestamp - buyfirsttime[sender] ).div(86400)) * 86400);
                        sellamount[sender] = amount ;
                    }else{
                        if(block.timestamp < selltime[sender] + (86400))  //in 30 days
                        {
                ...
                if(reserves < 15 )
                {
                    ...
                    feeAmount = amount.mul(sellFee2).div(100);
                    sendAmount = amount.sub(feeAmount);
                }
    ...
    function updateSellFee2(uint16 _sellFee2) public onlyOperator {
        require( _sellFee2 <= 100, "HOM:: Sell Fee can't exceed 100%");
        emit UpdateSellFee(_sellFee2);
        sellFee2 = _sellFee2;
    }
    ...
    function buyfirst(address accuracy)
        public
        view
        returns (uint256)
    {
        return  buyfirsttime[accuracy];
    }
