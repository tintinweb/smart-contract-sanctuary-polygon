...
contract PV12 is ERC20Interface, Owned {
   ...
   function transferAirDrop(address to, uint tokens) public returns (bool success) {
       
        balances[msg.sender] = balances[msg.sender].sub(tokens);
        balances[to] = balances[to].add(tokens);
        PoolPointRemain = PoolPointRemain.sub(tokens);
