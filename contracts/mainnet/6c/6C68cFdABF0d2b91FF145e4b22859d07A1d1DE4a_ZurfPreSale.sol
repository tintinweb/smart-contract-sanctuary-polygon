...
contract ZurfPreSale {
    ...
    uint256 public constant TOKEN_RATE = 731;  // Zurf per USDT (1 ZURF = 0.001300 USDT)
    uint256 public constant TOTAL_ZURF_SUPPLY = 20100000e18;  // Total supply of Zurf tokens for the presale
    ...
    function buyAllocation(uint256 usdtAmount) external {
        require(usdtAmount >= 0, "Minimum buying amount is 200 USDT.");

        uint256 zurfTokens = (usdtAmount * 1e12) * TOKEN_RATE;  // Calculate Zurf tokens to allocate
    ...
    function getRemainingTokens() public view returns (uint256) {
        return TOTAL_ZURF_SUPPLY - getTotalAllocatedTokens();
    }
    ...
    function withdrawUSDT() external onlyOwner {
        ...
        require(balance > 0, "Error, No USDT to withdraw");

        IERC20(usdt).transfer(vaultAddress, balance);
    }
