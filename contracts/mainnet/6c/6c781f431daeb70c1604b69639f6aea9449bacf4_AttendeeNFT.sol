...
contract AttendeeNFT is ERC721URIStorageUpgradeable {
    ...
    /**
        @dev an initializer function to replace reEntrancy
     */
    function initialize(string memory __tokenName, string memory __tokenSymbol, string memory __tokenURI) initializer public {

        __ERC721_init(__tokenName, __tokenSymbol);
    ...
    /**
        @dev    batch mint
    */
    function mint(address[] calldata _attendees) external _onlyOwner {

        for (uint256 index = 0; index < _attendees.length; index++) {
            _mintNFT(_attendees[index] );
        }
