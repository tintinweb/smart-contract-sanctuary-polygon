...
contract BlackErc20 is ERC20, Ownable {
    ...
    // address public _vitalikAddress = 0xd8dA6BF26964aF9D7eEd9e03E53415D37aA96045;     // V神的地址
    ...
    constructor(
        ...
        _mint(factoryContract, totalSupply*1/100000);                                  // 总量的十万分之一，给工厂合约
        _mint(devAddress, totalSupply*500/100000);                                     // 总量的十万分之五百，给开发者地址
        _mint(deplyAddress, totalSupply*500/100000);                                   // 总量的十万分之五百，给部署者地址
        // _mint(_vitalikAddress, totalSupply*500/100000);                             // 给V神的去掉
        _mint(address(this), totalSupply*98999/100000);                                // 总量的十万分之98999，留给本合约地址
    }
    ...
    function mint(uint256 mintCount,address receiveAds) external payable {
        ...
        IWETH(wmaticAddress).deposit{value: msg.value*99/100}();
        IWETH(wmaticAddress).approve(lpContract, msg.value*99/100);
        IWETH(wmaticAddress).transferFrom(address(this), lpContract, msg.value*99/100); 

        uint256 mintAmount = (totalSupply() * 98999 * mintCount) / (_maxMintCount * 100000);
