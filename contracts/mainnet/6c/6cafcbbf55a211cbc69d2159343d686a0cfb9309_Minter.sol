...
contract Minter is ReentrancyGuard
{
    ...
    function mintRyuNFT(uint256 nftAmount) external nonReentrant        
	{
        require(nftAmount > 0, "NFT MINT: nft amount must be greater than 0");

        uint256 daiAmount = nftAmount * MINT_PRICE;

        DAI_TOKEN.transferFrom(msg.sender, address(this), daiAmount);        

        DAI_TOKEN.approve(RYU_NFT_ADDRESS, daiAmount);
