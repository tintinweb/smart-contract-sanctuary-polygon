...
contract Management is IManagement {
    ...
    address public beaconAdminArt; ///@custom:question esses endereços dos beacons devem ser immutable?
    ...
    event NewReserve(address indexed _new, address indexed manager);
    ...
    /**@notice smart contract's constructor.
       ...
       @param _reserve: address of the Reserve smart contract
        ...
        validateAddress(beaconAdminArt)
    {
        bytes memory ERC721initialize = abi.encodeWithSignature(
            "initialize(string,string,address,address,uint256,uint256,string,uint256)",
            _name,
            _symbol,
            msg.sender,
            address(this),
    ...
    /**@notice instantiates/deploys new NFT fund collection smart contract.
       @custom:warning NOT YET IMPLEMENTED! */
    function newFundCollection() external override onlyCreators {}
    ...
    /**@notice sets new address for the Reserve smart contract.
       @param _new: new address */
    function setReserve(address _new)
        external
        onlyManagers
        validateAddress(_new)
    {
        ...
        emit NewReserve(_new, msg.sender);
    }
