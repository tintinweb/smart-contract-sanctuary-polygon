...
contract SpaceStaking is Initializable, ReentrancyGuardUpgradeable, OwnableUpgradeable, AccessControlEnumerableUpgradeable {
    ...
    public 
    {
        ...
        for(uint32 idx=0;idx<length;idx++) {
            ...
            nftContract.safeTransferFrom(sender, address(this), tokenId, 1, "");
            count += 1;
    ...
    nonReentrant 
    {
        ...
        for(uint32 idx=0;idx<length;idx++) {
            uint256 tokenId = tokenIds[idx];
            nftContract.safeTransferFrom(address(this), recipient, tokenId, 1, "");
    ...
    function claimableItems(address staker, uint256[] memory signedTokenIds)
    internal
    view
    returns(ClaimableItem[] memory items)
    {
        uint256[] storage stakedTokenIds = stakedNftsEntries[staker];
        uint32 length = uint32(stakedTokenIds.length);
        items = new ClaimableItem[](length);
        for(uint32 idx=0; idx < length; idx++) {
            uint256 tokenId = stakedTokenIds[idx];
            items[idx].tokenId = tokenId;
            items[idx].perDay = 50e18;
            items[idx].hasPair = false;

            for(uint256 idx2 = 0;idx2<signedTokenIds.length; idx2++) {
                if(signedTokenIds[idx2] == tokenId) {
                    items[idx].hasPair = true;
                    items[idx].perDay = 100e18;
                    break;
                }
            }
        }
    ...
    // TODO: test
    function removeStakenNftEntries(address staker, uint256 tokenId)
    internal 
    {
        uint256[] storage stakedTokenIds = stakedNftsEntries[staker];
        uint32 length = uint32(stakedTokenIds.length);
    ...
    internal
    {
        ...
        require(stakedEffectiveTime[sender] < nonce, "ERR_WRONG_NONCE");
        require(nonce + 3600 > block.timestamp, "ERR_OLD_NONCE");
    ...
    view
    public
    returns(uint256 amount, uint256 amountPerDay, uint256 effectiveTime, uint32 pairs, uint32 singles, ClaimableItem[] memory items) 
    {
        ...
        for(uint32 idx=0; idx < length; idx++) {
            ...
            if(stakedBy[signedTokenId] == sender) {
                pairs+=1;
            }
        }

        singles = stakedNftsCount[sender] - pairs;
        uint256 pairsAmount = pairs * rewardPerSecond();
        uint256 singlesAmount = singles * rewardPerSecond() / 2;
        amountPerDay = (pairs * rewardPerDay + singles * rewardPerDay / 2);
        amount = (pairsAmount + singlesAmount) * elapsedTime;
        if(actualBalance < amount) {
            amount = actualBalance / 10000 * uint32(singlesAmount / (amount / 10000)) + 
                actualBalance / 10000 * uint32(pairsAmount / (amount / 10000));
            // TODO: test
            effectiveTime = stakedEffectiveTime[sender] + elapsedTime / 10000 * (actualBalance / (amount / 10000));
        } else {
            effectiveTime = block.timestamp;
        }
        items = claimableItems(sender, signedTokenIds);
    }
    ...
    /*
    * @dev return staked token ids
    */
    function retrieve(address owner, uint256 offset, uint256 limit)
    public 
    view
    returns(uint256[] memory items, uint256 left)
    {
        if(offset + limit > stakedNftsEntries[owner].length) {
            // TODO: test
            limit = stakedNftsEntries[owner].length - offset;
        }
        items = new uint256[](limit);

        for(uint256 idx=0; idx < limit; idx++) {
            items[idx] = stakedNftsEntries[owner][offset + idx];
        }

        if(stakedNftsEntries[owner].length > offset + limit) {
            // TODO: test
            left = stakedNftsEntries[owner].length - offset - limit;
        } else {
            left = 0;
        }
    }
