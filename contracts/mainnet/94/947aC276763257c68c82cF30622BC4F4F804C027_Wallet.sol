...
interface ISwapToken{
    ...
    function getAggregatorAddress(address tokenAddress) external view returns (address);
}
...
/************************************************************************************ WALLET-FUNCTIONS ******************************************************************************************/
   ...
   */
   function update( address[] memory _tokenAddresses, uint[] memory _tokenAmounts, uint[] memory _amountsOut) external onlyClampContract{
       sortTokens(_tokenAddresses, _tokenAmounts, _amountsOut);
       uint _indexIdentifier = uint(keccak256(abi.encodePacked(_tokenAddresses)));
        if(userIndexes[_indexIdentifier].Exists == true){
            for(uint i=0;i<_tokenAddresses.length;i++){
                userIndexes[_indexIdentifier].TokenHoldings[i] += _amountsOut[i];
            }
        }else{
            userIndexes[_indexIdentifier] = Index(true, _tokenAddresses, _amountsOut);           
        }
        address _wmaticAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; //main-net address -> should be a part of definedTokens and corresponding aggregaator address in SWAPTOKEN contract
        emit MultSwap(Transaction.TransactionStatus.MultiSwap,
        _tokenAddresses,
        _tokenAmounts,
        _amountsOut,
        getPrice(_wmaticAddress)
        );
    }
    /*
    ...
    */
function internalSwap(uint _indexIdentifier, uint _percentage, address _returnBaseToken) external onlyOwner  {
    require(userIndexes[_indexIdentifier].Exists, "CLAMPV1: INDEX-NOT-FOUND");
    require(_percentage > 0 && _percentage <= 1000, "CLAMPV1: INVALID-PERCENTAGE");
    require(SWAPTOKEN.isDefinedToken(_returnBaseToken), "CLAMPV1: INVALID-BASE-TOKEN");
    ...
    uint[] memory amountsOut = new uint[](n);

    for (uint i = 0; i < n; i++) {
        swapAmounts[i] = indexToUpdate.TokenHoldings[i].mul(_percentage).div(1000);
        ...
        if (_tokenAddrs[i] == _returnBaseToken) {
            ...
            amountsOut[i] = swapExactInputSingle(_returnBaseToken, _tokenAddrs[i], swapAmounts[i]);
        }
    }
    ...
    int256 swappedTokenPrice = getPrice(_returnBaseToken);
    ...
    */
    function externalMultiWithdraw(uint _indexIdentifier, uint _percentage) external onlyOwner {
        require(userIndexes[_indexIdentifier].Exists, "CLAMPV1: INDEX-NOT-FOUND");
        require(_percentage > 0 && _percentage < 1001, "CLAMPV1: INVALID-PERCENTAGE"); //should be greater than 5% or 10% 
        ...
        for (uint i = 0; i < n; i++) {
            IERC20 token = IERC20(indexToUpdate.TokenAddresses[i]);
            token.transfer(Owner, swapAmounts[i]);
        }
        
        int[] memory _priceOfWithdrawTokens = new int[](n);
        for (uint i = 0; i < n; i++) {
            address tempAddress = indexToUpdate.TokenAddresses[i];
            _priceOfWithdrawTokens[i] = getPrice(tempAddress);
        }  
        ...
        if(_percentage == 1000){
            ...
            for(uint i=0;i<n;i++){
                userIndexes[_indexIdentifier].TokenHoldings[i] -= swapAmounts[i];
            }
        }  
    ...
    */
    function withdrawReseveBaseTokens(address _token, uint _amount) external onlyOwner {
        require(_token != address(0), "token address cannot be 0");
        require(_amount <= reserveBaseTokenBalances[_token], "amount must be less than or equal to the balance of the token");
...
/******************************************************************************************* HELPERS ********************************************************************************************/
    function _removeIndex(uint _indexIdentifier) private{
        require(userIndexes[_indexIdentifier].Exists == true,'CLAMPV1: NO-INDEX-EXISTS');
        delete userIndexes[_indexIdentifier];
    }
    function getIndexVlaues(uint _indexIdentifier) public view returns(uint[] memory, address[] memory){
       require(userIndexes[_indexIdentifier].Exists == true,'CLAMPV1: NO-INDEX-EXISTS');
       return (userIndexes[_indexIdentifier].TokenHoldings,userIndexes[_indexIdentifier].TokenAddresses);
    }
    function sum(uint[] memory arr) private pure returns (uint) {
        uint result = 0;
    ...
    function getPrice(address _tokenAddress) private view returns (int) {
        require(SWAPTOKEN.isDefinedToken(_tokenAddress), "Token not in Defined set array");
        address aggregatorAddress = SWAPTOKEN.getAggregatorAddress(_tokenAddress);
        require(aggregatorAddress != address(0), "Aggregator not found for the token");
        AggregatorV3Interface priceFeed = AggregatorV3Interface(aggregatorAddress);        
        (,int price,,,) = priceFeed.latestRoundData();
        return price;
    }
    
    //will change this to private later
    function transferTokensToEOA(address _tokenAddress, address _recipient, uint256 _amount ) private {
        IERC20 token = IERC20(_tokenAddress);
        token.transfer(_recipient, _amount);
    }
    function sortTokens(address[] memory _tokenAddresses, uint256[] memory _tokenAmounts,uint[] memory _amountsOut) private pure {
        require(_tokenAddresses.length > 1 && _tokenAddresses.length < 16,'CLAMP: Index Tokens number mis-match');
        for (uint i = 0; i < _tokenAddresses.length - 1; i++) {
            ...
            if (minIndex != i) {
                (_tokenAddresses[i], _tokenAddresses[minIndex]) = (_tokenAddresses[minIndex], _tokenAddresses[i]);
                (_tokenAmounts[i], _tokenAmounts[minIndex]) = (_tokenAmounts[minIndex], _tokenAmounts[i]);
                (_amountsOut[i], _amountsOut[minIndex]) = (_amountsOut[minIndex], _amountsOut[i]);
            }
        }
    }
/************************************************************************************************************************************************************************************************/
