...
|    * @param _payoutTimeRange duration to honor payout time. Ex: 15 * 60;
    ...
    */
    function _payout() internal {
        ...
        // Print cashtotalsupply and vault.checkBalnace()
        uint256 _t = cash.totalSupply();
        uint256 _lv = IVault(address(this)).checkBalance();
        // Print
        console.log("Cash Total Supply: %s", _t);
        console.log("Vault Balance: %s", _lv);
        IHarvester(harvesterAddress).harvestAndDistribute();
        console.log("After harvesting");
        _t = cash.totalSupply();
        _lv = IVault(address(this)).checkBalance();
        // Print
        console.log("Cash Total Supply: %s", _t);
        console.log("Vault Balance: %s", _lv);
