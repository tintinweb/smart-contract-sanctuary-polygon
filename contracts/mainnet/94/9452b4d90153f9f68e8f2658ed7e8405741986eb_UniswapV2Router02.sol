...
library TransferHelper {
    ...
    function safeTransferFrom(address token, address from, address to, uint value) internal {
        ...
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'TransferHelper: TRANSFER_FROM_FAILEDXXX');
    }
...
interface IUniswapV2Router02  {
    function crossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external //returns (uint amountOutReal) 
    ;
    function flashLoanToCrossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        address routerLoan, 
        address factoryLoan, 
        address routerReim, 
        address factoryReim, 
...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    address curRouterLoan;
    address curFactoryLoan;
    ...
    function flashLoanToCrossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        address routerLoan, 
        address factoryLoan, 
        address routerSuffix, 
        address factorySuffix, 
        address tokenReim,
        address tokenLoan,
        uint amountLoan,
        address[] calldata tokenPath,
        ...
        curRouterPath.push(routerSuffix);
        curFactoryPath.push(factorySuffix);
        curDeadline = deadline;
        curRouterLoan = routerLoan;
        curFactoryLoan = factoryLoan;

        address pairAddress = IUniswapV2Factory(factoryLoan).getPair(tokenReim, tokenLoan);
...
function defiCall(
    ...
    address takerAddr = address(this);
    uint balanceBefore = IERC20(curTokenPath[curTokenPath.length-1]).balanceOf(takerAddr);
    IUniswapV2Router02(address(this)).crossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
      amountLoan, 
      amountLoan, 
      curTokenPath, 
      curRouterPath, 
    ...
    uint balanceAfter = IERC20(curTokenPath[curTokenPath.length-1]).balanceOf(takerAddr);
    uint amountReceived = balanceAfter - balanceBefore;
    require(amountReceived > 0, 'amountReceived_ZERO');

    address[] memory path = new address[](2);
    path[0] = curTokenPath[curTokenPath.length-1];
    path[1] = curTokenPath[curTokenPath.length-2];
    uint amountReim = IUniswapV2Router01(curRouterLoan).getAmountsIn(
        amountLoan, 
        path
    )[0];
  ...
  function crossSwapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        address to,
        uint deadline
    ) external virtual override ensure(deadline) //returns (uint amountOutReal) 
    {
