...
contract MidnightRaffles is VRFConsumerBase {
    ...
    function setMaxMultiplier(uint256 newMultiplier) external onlyDeployer {
        maxMultiplier = newMultiplier;
    }

    function getMaxMultiplier() public view returns (uint256) {
    ...
    function enterCollabRaffle(uint256 xTokers) public payable {
        ...
        require(
            (currentEntries[msg.sender] <= maxEntries[msg.sender]),
            "You don't own enough of the Collab Project's NFTs!."
        );
        for (uint256 i = 0; i < xTokers; i++) {
            pId = count++;
            playerTickets[msg.sender].push(pId);
    ...
    function getMaxEntriesTMTC(address userWallet) public view returns (uint256) {
        return TMTC(TokerAddy).balanceOf(userWallet) * maxMultiplier;
    }

    function getMaxEntriesCollab(address userWallet) public view returns (uint256) {
        return TMTC(raffleCollab).balanceOf(userWallet) * maxMultiplier;
    }

    function getMaxEntriesTotal(address userWallet) public view returns (uint256) {
        return (TMTC(TokerAddy).balanceOf(userWallet) + TMTC(raffleCollab).balanceOf(userWallet)) * maxMultiplier;
    }

     function getCollabAddress() public view returns (address) {
        return (raffleCollab);
    }  
