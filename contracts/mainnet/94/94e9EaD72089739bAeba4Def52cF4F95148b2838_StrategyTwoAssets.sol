...
abstract contract BaseStrategyMasterChef is BaseStrategy {
    ...
    function getHarvestable() external override view returns (uint256) {
        return IStakingRewards(rewards).earnedA(address(this));
    }
    ...
    function deposit(uint256 _amount) internal override {
        ...
        if (_amount > 0) {
            ...
            IStakingRewards(rewards).stake(_amount);
        }
    }
...
abstract contract StrategyFarmTwoAssets is BaseStrategyMasterChef {
    ...
    // Dragon's lair
    address public constant dragonslair = 0xf28164A485B0B2C90639E47b0f377b4a438a16B1;
    // Reward token address
    address public constant rewardToken0Addr = 0x831753DD7087CaC61aB5644b308642cc1c33Dc13;
    address public constant rewardToken1Addr = 0x2e1AD108fF1D8C782fcBbB89AAd783aC49586756;
    // One or more farm tokens
    address public constant farmToken0Addr = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    address public constant farmToken1Addr = 0x2e1AD108fF1D8C782fcBbB89AAd783aC49586756;
    // Fee token
    address public constant feeTokenAddr = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
    // Routers
    ...
    uint256 public kogefarmFeeAmountBps = 290;
    // The minimum amount (in bps) KogeFarm will harvest from the reward token
    uint256 public harvestCutoffBps = 10**13;
    uint256 public harvestCutoffBps2 = 10**17;
    uint256 public harvestCutoffBps3 = 10**17;
    // The reward --> fee token path
    address[] public rewardToFeeTokenPath;
    // Assuming one or more reward --> farm token paths
    address[] public reward0ToFarmToken0Path;
    address[] public reward1ToFarmToken0Path;
    address[] public farmToken0ToFarmToken1Path;
        ...
        )
    {
        ...
        reward0ToFarmToken0Path = new address[](2);
        reward0ToFarmToken0Path[0] = rewardToken0Addr;
        reward0ToFarmToken0Path[1] = farmToken0Addr;

        reward1ToFarmToken0Path = new address[](2);
        reward1ToFarmToken0Path[0] = rewardToken1Addr;
        reward1ToFarmToken0Path[1] = farmToken0Addr;

        farmToken0ToFarmToken1Path = new address[](4);
        farmToken0ToFarmToken1Path[0] = farmToken0Addr;
        farmToken0ToFarmToken1Path[1] = farmToken1Addr;
    }
    ...
    function set_harvestCutoff2(uint256 newCutoff) external onlyOwner() {
        harvestCutoffBps2 = newCutoff;
        emit SetHarvestCutoff(harvestCutoffBps);
    }

    function set_harvestCutoff3(uint256 newCutoff) external onlyOwner() {
        harvestCutoffBps3 = newCutoff;
        emit SetHarvestCutoff(harvestCutoffBps);
    }
    ...
    function collectFee(uint256 _rewardBalance) internal {
        ...
        // if(rewardToken1Addr != feeTokenAddr) {
    ...
    function harvest() public override nonReentrant {
        ...
        uint256 _reward0Balance = IERC20(rewardToken0Addr).balanceOf(address(this));
        if (_reward0Balance > harvestCutoffBps){
            swapAmount = calculateSwapAmount(rewardToken0Addr,10000);
            _swapUniswapWithPathForFeeOnTransferTokens(reward0ToFarmToken0Path, swapAmount, swapRouter0Addr);
        }
        // Figure out how much reward we collected
        uint256 _reward1Balance = IERC20(rewardToken1Addr).balanceOf(address(this));
        if (_reward1Balance > harvestCutoffBps2){
            swapAmount = calculateSwapAmount(rewardToken1Addr,10000);
            _swapUniswapWithPathForFeeOnTransferTokens(reward1ToFarmToken0Path, swapAmount, swapRouter0Addr);
        }

        uint256 _farm0Balance = IERC20(farmToken0Addr).balanceOf(address(this));
        if (_reward1Balance > harvestCutoffBps3){
          // Collect fee if not at 0
          if (kogefarmFeeAmountBps > 0) {
              collectFee(_farm0Balance);
          }
          // Swap reward for farm tokens (want)
          // Calculate the amount to swap for the farm token.
          swapAmount = calculateSwapAmount(farmToken0Addr,5000);
          // Swap the required amount of reward token using the designated swap path
          _swapUniswapWithPathForFeeOnTransferTokens(farmToken0ToFarmToken1Path, swapAmount, swapRouter0Addr);
          // Add liquidity
...
contract StrategyTwoAssets is StrategyFarmTwoAssets {
    // Token addresses
    address public FARM_MASTER_CHEF = 0x1661C61b66A2A2C54970F271f2B86556cdf18B97;
    address public LP_TOKEN = 0x17a7829cc1167ecdA8b9668414a5405050846F8A;
    constructor()
        public
