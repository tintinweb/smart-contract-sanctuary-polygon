...
contract Collection721Impl is ERC721Upgradeable, OwnableUpgradeable, AccessControlUpgradeable, PausableUpgradeable {
    ...
    function mintNFT(string memory _metaData, bytes32[] memory proof) public whenNotPaused payable {
        ...
        require(totalMinted.add(1) <= maxSupply, "2");
        ...
        if (phase.qtyAvailable > 0) {
            require(phase.totalMinted.add(1) <= phase.qtyAvailable, "6");
        }

        //Price checks
        uint currentPrice = phase.price;
        require(msg.value >= currentPrice.mul(1), "7");

        //Permissions Checks
        if (phase.id != 0) {
            if (phase.restrictedMerkleRoot > 0) {
                bool verified = verify(phase.restrictedMerkleRoot, _leaf(msg.sender), proof);
                require(verified, "8");
            }
        }
    ...
    function mintNFTMulti(string[] memory _metaData, bytes32[] memory proof) public whenNotPaused payable {
        //Basic checks
        uint totalMinted = _tokenIds.current();
        require(totalMinted.add(_metaData.length) <= maxSupply, "2");
        ...
        DROPPHASE memory phase = currentPhase;
        if (phase.qtyAvailable > 0) {
            require(phase.totalMinted.add(_metaData.length) <= phase.qtyAvailable, "6");
        }

        //Price checks
        uint currentPrice = phase.price;
        require(msg.value >= currentPrice.mul(_metaData.length), "7");
    ...
    function airDrop(address[] memory accounts, string[] memory metaURIs) public whenNotPaused {
        ...
        uint totalMinted = _tokenIds.current();
        require(totalMinted.add(accounts.length) <= maxSupply, "11");
    ...
    function setRevealed(bool _revealed, string memory _revealURI) public whenNotPaused {
        require(hasRole(PROJECT_ADMIN, msg.sender), "9");
        revealed = _revealed;
        baseTokenURI = _revealURI;
    }
