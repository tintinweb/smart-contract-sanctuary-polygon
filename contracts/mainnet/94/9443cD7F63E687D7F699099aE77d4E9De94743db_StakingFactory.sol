...
contract StakingFactory {
    ...
    function initialize (address lp, uint256 amount, uint256 duration) public {
        ...
        address stakingPool = address(new TokenRewards(escrowToken, lp, duration));
...
contract TokenRewards is LPTokenWrapper, IRewardDistributionRecipient, StakingWhitelistable {
    ...
    constructor (address _token, address _lp, uint256 _duration) LPTokenWrapper(_lp) public {
      snx = IERC20(_token);
        ...
        updateReward(address(0))
    {
        ...
        if (block.timestamp >= periodFinish) {
            rewardRate = reward.div(poolDuration);
        } else {
            uint256 remaining = periodFinish.sub(block.timestamp);
            uint256 leftover = remaining.mul(rewardRate);
            rewardRate = reward.add(leftover).div(poolDuration);
        }
        lastUpdateTime = block.timestamp;
        periodFinish = block.timestamp.add(poolDuration);
