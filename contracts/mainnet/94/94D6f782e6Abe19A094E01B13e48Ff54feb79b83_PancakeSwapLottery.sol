    ...
    EIP712MetaTransaction
{
    ...
    EnumerableSetUpgradeable.AddressSet private _totalAmountPlayers;

    mapping(uint256 => mapping(uint256 => uint256))
        private _resellerLotteryticketCounts;

    EnumerableSetUpgradeable.AddressSet private _resellerAddress;
    //EnumerableSetUpgradeable.UintSet private _resellerIds;
    struct Reseller {
        uint256 resellerId;
    ...
    event AddReseller(address indexed resellerAddress, uint256 resellerId);
    ...
    function buyTickets(
        ...
        _totalAmountPlayers.add(_msgSender());
        ...
        nonReentrant
    {
        ...
        _totalFeeAmount += amountToWithdrawToTreasury;
        ...
        uint256 _resellerTotalAmount = amountToWithdrawToTreasury -
            amountToWithdrawToTeam;
        ...
        for (uint256 i = 0; i < _resellerAddress.length(); i++) {
            uint256 resellerid = _resellers[_resellerAddress.at(i)].resellerId;
            _resellers[_resellerAddress.at(i)].amount +=
                (_resellerLotteryticketCounts[_lotteryId][resellerid] /
                    _lotteryTotalCount) *
                _resellerTotalAmount;
        }
    ...
    function getTotalPlayers() external view onlyOperator returns (uint256) {
        return _totalAmountPlayers.length();
    }
    ...
    function addReseller(address _address, uint256 _id) external onlyOperator {
        require(
