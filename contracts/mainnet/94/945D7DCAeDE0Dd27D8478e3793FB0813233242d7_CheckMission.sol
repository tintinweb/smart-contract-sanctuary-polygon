...
contract CheckMission is OwnableUpgradeable {
    ...
    function initialize() public initializer {
        ...
        stable = IStable(0x936eD7414ec0999f3D5F8A10bd7a0DCCA180B688);
        cattle = ICOW(0x904ff0644C6254DB78c6a8F2f1e53ec3053Dc142);
        milk = IMilk(0x28449c11a56FF30B8406BE2040e1DEfAfb9A4561);
        fight = IFight(0xFc8c9779A8Fb4290156697bE024d3fC928FE0434);
        claim = IClaim(0x46979Ec6E39913a20fFCae9857023Ba350741097);
    }

    function checkUserIsQualified(address addr) public view returns (bool){
        bool[] memory res = checkUserAllMission(addr);
    ...
    function checkUserAllMission(address addr) public view returns (bool[] memory){
        bool[] memory list = new bool[](4);

        (,,bool done,,) = claim.userInfo(addr);
        if (done) {
            list[0] = true;
        }
        {
            uint[]memory cowList = stable.checkUserCows(addr);
            if (cowList.length != 0) {
                for (uint i = 0; i < cowList.length; i++) {
                    if (!cattle.isCreation(cowList[i]) && cattle.getAdult(cowList[i])) {
                        list[1] = true;
                        break;
                    }
                }
        ...
        (,uint milkRew) = milk.userInfo(addr);
        if (milkRew >= 200 ether) {
            list[3] = true;
        }
        (,uint fightRew) = fight.userInfo(addr);
        if (fightRew >= 1000 ether) {
            list[4] = true;
        }
        return list;
    }
