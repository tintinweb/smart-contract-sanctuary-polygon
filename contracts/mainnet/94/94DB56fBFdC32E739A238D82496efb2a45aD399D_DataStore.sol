...
contract DataStore is Initializable, NonblockingLzApp {
    ...
    function childBalance(address _childVault) external view returns (uint256) {
        _checkEpsilon(_childVault);
        return childMessageOffChain[_childVault].balance;
    }
    function childTvl(address _childVault) external view returns (uint256) {
        _checkEpsilon(_childVault);
        return childMessageOffChain[_childVault].tvl;
    }
    ...
    function writeOnchain(address _childVault, uint256 _balance, uint256 _tvl, uint256 _chainId, uint256 _timestamp, uint8 _source) internal {
