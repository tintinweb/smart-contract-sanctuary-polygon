 ...
 */
contract HedgeCoreUpgradable is
	IHedgeCore,
	Initializable,
	UUPSUpgradeable,
	PausableUpgradeable,
	...
	HedgeCoreStorage
{
	...
	modifier onlyBeforeLockedTime() {
		require(block.timestamp <= hedgeInfo.rebaseEndTime - resctrictedPeriod, "HC:RESTRICTED");
		_;
	}
	 ...
	 * @dev This function cannot be called again after init gnessis hedge. (in case we leave it uninitialized during furture upragde)
	...
	function initGnesisHedge(
		...
		// 1. restricted period. defualt is 4 hours (can be modified no bigger than 8hours)
		resctrictedPeriod = 14400;

		// 2 price fetch
		// 2.1 set price ratio
		priceRatio = 10000; // default is 10%
		// 2.2 fetch inital prices for vsq and hegeTarget token
		(hedgeInfo.hedgeTokenPrice, hedgeInfo.hedgeTargetPrice) = _fetchPrice();

		// 3 fetch inital rebase info
	...
	function depositFor(
		...
		uint256 hgeAmount_,
		uint256 levAmount_
	) public override nonReentrant onlyInitialized whenNotPaused onlyBeforeLockedTime isEligibleSender {
	 ...
	 */
	function withdrawAllWhenPaused() public override nonReentrant onlyInitialized whenPaused isEligibleSender {
		// 1. cal hge and lev balance
	...
	function sponsorDeposit(uint256 amount_) public override nonReentrant onlyInitialized whenNotPaused isEligibleSender {
	 ...
	 */
	function updatePriceRatio(uint256 newRatio_) external onlyDAO {
	...
	/**
	 * @dev [onlyDAO] range 0-8hours => deposit available window (8hours - 0 hours)
	...
	function _withdrawTo(
		...
		uint256 left;
		if (deposited > hgeAmount_ + levAmount_) {
			left = deposited - (hgeAmount_ + levAmount_);
		}
	...
	function _sponsorWithraw(uint256 amount_) private {
		...
		// 3. emit events
		emit Logger(hgeToken.index(), levToken.index(), hgeRebases.length, levRebases.length);
		emit SponsorWithdrawn(msg.sender, amount_);
	}
	...
	function _startNewHedge() private {
		...
		bool isLong;
		// 1.1 get the original price
		uint256 winningPrice = (hedgeInfo.hedgeTokenPrice * PRICE_PRECISION) / hedgeInfo.hedgeTargetPrice;
		// 1.2 multiply price by price ratio
		winningPrice += (winningPrice * priceRatio) / RATIO_PRECISION;
		// 1.3 lev win if currPrice >= winningPrice else hedge win
		if (((currHedgeTokenPrice * PRICE_PRECISION) / curHedgeTargetPrice) >= winningPrice) {
			isLong = true;
		}
		...
		uint256 sTokenBalance = sToken.balanceOf(address(this));
	 ...
	 * @param isLong_ true: soft lev win. false: soft hge win
	 * @param amountRebased_ actual rebase rebase reward being distributed to user
	 * @param rebaseTotalAmount_ totalRebase rewards
	 */
	function _rebaseHedgeToken(
		bool isLong_,
		uint256 amountRebased_,
		uint256 rebaseTotalAmount_
	) private {
		...
		currRebase.totalRebasedAmount = rebaseTotalAmount_;
	...
	/**
	 * @dev log only. do not touch any core logic
	 * @param isLong_ winning side
	 */
	function _storeLog(bool isLong_) private {
		Log memory log;
		log.isLong = isLong_;
		if (isLong_) {
	...
	function hedgeCoreStatus() external view override returns (bool isUnlocked_) {
		isUnlocked_ = (block.timestamp <= hedgeInfo.rebaseEndTime - resctrictedPeriod) ? true : false;
	}
	...
	function earnedProfit(address user_) external view returns (uint256 earnedProfit_) {
		...
		if (totalBalance >= userDepositedAmount) {
			// in case of underflow since user might transfer token themselves
			earnedProfit_ = totalBalance - userDepositedAmount;
		}
	}

	/**
	 * @notice view the expected lev winning price for next round
	 * @dev expect the lev winning price = (hedgeTokenPrice/hedgeTargetPrice)*(1 + ratio(%))
	 * @return price_ the price with precision (1E8)
	 */

	function priceAfterRatio() external view returns (uint256 price_) {
		// 1. get the original price
		price_ = (hedgeInfo.hedgeTokenPrice * PRICE_PRECISION) / hedgeInfo.hedgeTargetPrice;
		// 2. multiply price by price ratio
		price_ += (price_ * priceRatio) / RATIO_PRECISION;
	}
	...
	function fetchLastNRoundsResults(uint256 n_) external view returns (bool[] memory results_) {
		...
		for (uint256 i = 0; i < logs.length; i++) {
			results_[i] = logs[logs.length - n_ + i].isLong;
		}
	}
...
contract HedgeCoreStorage {
	...
	// tokens
	IERC20 public sToken; // the underlying token which user can deposit and withdraw
	...
	address public hedgeTarget; // the token which VSQ is hedge/leverage against to
	IHedgeCore.HedgeInfo public hedgeInfo; // store important game related data
