...
contract CC_Heroes is ERC721, ERC2981{
    ...
    event LevelUp(uint256 indexed tokenId, uint32 level);
    ...
    function mintTo(address to, address royaltyReceiver, uint32 hero, uint32 level, uint32 skin) public onlyOwnerOrAuthorizedUser{
        tokenIdCounter.increment();
        _mint(to, tokenIdCounter.current());

        _heroes[tokenIdCounter.current()] = Hero(hero, level, skin);
    ...
    function initialMintBatch(uint256 count, address royaltyReceiver, uint32 hero, uint32 level, uint32 skin) public onlyOwner{
        for(uint i=0;i<count;i++){
            mintTo(msg.sender, royaltyReceiver, hero, level, skin);
        }
    }
    ...
    function setLevel(uint256 tokenId, uint32 level) public onlyOwnerOrAuthorizedUser {
        require(_exists(tokenId), "Hero: token id does not exists");
