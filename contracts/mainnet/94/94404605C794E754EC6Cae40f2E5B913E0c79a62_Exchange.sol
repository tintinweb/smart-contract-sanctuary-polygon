...
contract Exchange is OwnableUpgradeable {
    ...
    event AllListingsCanceled   (uint256 tokenId, address tokenOwner);
    ...
    bytes32 public constant ORDER_TYPEHASH = keccak256(
        "Order(address maker,bool isErc20Offer,uint256 tokenId,address currencyContract,uint256 amount,uint256 nonce,uint256 tokenNonce,uint256 listingTime,uint256 expirationTime,uint256 salt)"
    );
    ...
    function initialize(address _tokenAddress, uint256 _chainId) public initializer {
    ...
    function cancelAllListings(uint256 tokenId) public {
        ...
        tokenNonce[tokenId]++;
        emit AllListingsCanceled(tokenId, msg.sender);
    }
        ...
        reentrancyGuard
    {
        ...
        require(firstOrder.tokenNonce < tokenNonce[firstOrder.tokenId], "First order token nonce is less");
        ...
        uint256 royalty = (firstOrder.amount * royaltyBasisPoints) / royaltyPercentageDenominator;
        uint256 orderAmount = firstOrder.amount - royalty;
        ...
        if (firstOrder.currencyContract == address(0)) {
            ...
            if(royalty > 0 && royaltyAddress != address(0)){
                (bool success,) = address(uint160(firstOrder.maker)).call{value: royalty}("");
                require(success, "native token transfer failed.");
            }
            if(orderAmount > 0) {
                (bool success,) = address(uint160(firstOrder.maker)).call{value: orderAmount}("");
                require(success, "native token transfer failed.");
            }
        }else{
            ...
            if(royalty > 0 && royaltyAddress != address(0))
                require(paymentContractAddress.transferFrom(secondOrder.maker, royaltyAddress, royalty));
            if(orderAmount > 0)
