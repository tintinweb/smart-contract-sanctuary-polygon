...
contract StandardUncappedNFTCollection is ERC721URIStorage, Operatorable, ReentrancyGuard {
    ...
    function _popId(address from, uint256 tokenId) internal {
        uint256[] storage collectionIds = collectionIds[from];
        for (uint256 i = 0; i < collectionIds.length; i++) {
            if (collectionIds[i] == tokenId) {
