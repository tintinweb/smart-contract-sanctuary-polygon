...
contract ML_AddLiquidity is ML_TransferHelper
{
    ...
    ) internal returns (AddLiquidityResult memory)
    {
        ...
        result.t0Unused = result.t0After - result.t0Before;
        result.t1After = t1.balanceOf(address(this));
        result.t1Unused = result.t1After - result.t1Before;
...
contract ServiceWithReferral
{
    ...
    uint256 private referralGas = 50000;
    ...
    function _setReferralGas(uint256 _gas) internal
    {
    ...
    function _handleFeeETH(uint256 _referralID, uint256 _amount) internal returns (uint256)
    {
        (address referrer, uint256 referralFee, uint256 amount) = _getReferralFeeInfo(_referralID, _amount);
        if (referralFee != 0)
        {
            (bool success, ) = payable(referrer).call{ value: referralFee, gas: referralGas }("");
    ...
    function _handleFeeToken(uint256 _referralID, IERC20 _token, uint256 _amount) internal returns (uint256)
    {
        (address referrer, uint256 referralFee, uint256 amount) = _getReferralFeeInfo(_referralID, _amount);
        if (referralFee != 0)
        {
    ...
    ServiceWithReferral
{
    ...
    {
        //check
        _validAmount(msg.value);
        require(_path[0] == address(wrappedCoin), "From token is not wETH");

        //take fees        
        (uint256 amount, uint256 fee) = takeFees(msg.value);
        if (fee > 0)
        {
            _handleFeeETH(_referralID, fee);
        }
    ...
    ) external whenNotPaused validAmount(_amountIn) nonReentrant
    {
        ...
        //take fees
        (uint256 amount, uint256 fee) = takeFees(swapResult.swapped);
        if (fee > 0)
        {
            _handleFeeETH(_referralID, fee);
        }   
    ...
    ) external whenNotPaused validAmount(_amountIn) nonReentrant
    {
        ...
        //take fees
        (uint256 amount, uint256 fee) = takeFees(_amountIn);
        if (fee > 0)
        {
            _handleFeeToken(_referralID, IToken(_path[0]), fee);
        } 
    ...
    ) external payable whenNotPaused nonReentrant
    {
        //check
        _validAmount(msg.value);
        require((_swapInfoOut.pair.token0() == address(wrappedCoin)
                || _swapInfoOut.path0[0] == address(wrappedCoin)),
            "Token0 path start is not wETH");
        require((_swapInfoOut.pair.token1() == address(wrappedCoin)
                || _swapInfoOut.path1[0] == address(wrappedCoin)),
            "Token1 path start is not wETH");   
        ...
        //send to user
        _swapInfoOut.pair.safeTransfer(msg.sender, addResult.lpAdded);
    }    
    ...
    {
        //init
        IToken from = IToken(_swapInfoOut.path0.length == 0
            ? _swapInfoOut.pair.token0()
            : _swapInfoOut.path0[_swapInfoOut.path0.length - 1]);
        ...
        if (fee > 0)
        {
            _handleFeeToken(_referralID, from, fee);
        }
    ...
    {
        //check
        require((_swapInfoIn.pair.token0() == address(wrappedCoin)
                || _swapInfoIn.path0[_swapInfoIn.path0.length - 1] == address(wrappedCoin)),
            "Token0 path end is not wETH");
        require((_swapInfoIn.pair.token1() == address(wrappedCoin)
                || _swapInfoIn.path1[_swapInfoIn.path1.length - 1] == address(wrappedCoin)),
            "Token1 path end is not wETH");
        ...
        //take fees
        (uint256 amount, uint256 fee) = takeFees(swapped);
        if (fee > 0)
        {
            _handleFeeETH(_referralID, fee);
        }   
    ...
    {
        //init
        IToken to = IToken(_swapInfoIn.path0.length == 0
            ? _swapInfoIn.pair.token0()
            : _swapInfoIn.path0[_swapInfoIn.path0.length - 1]);
        ...
        if (fee > 0)
        {
            _handleFeeToken(_referralID, to, fee);
        }   
    ...
    {
        //take fees
        (uint256 amount, uint256 fee) = (_takeFee ? takeFees(_amountIn) : (_amountIn, 0));
        if (fee > 0)
        {
            _handleFeeToken(_referralID, IToken(_path[0]), fee);
        }
    ...
    ) private returns (uint256)
    {
        ...
        require(to0 == to1, "Token0 / Token1 path end mismatch");
        require(from0 == _swapInfoIn.pair.token0(), "Token0 path start not matching pair");
        require(from1 == _swapInfoIn.pair.token1(), "Token0 path start not matching pair");
    ...
    ) private returns (ML_AddLiquidity.AddLiquidityResult memory)
    {
        ...
        require(from0 == from1, "Token0 / Token1 path start mismatch");
        require(to0 == _swapInfoOut.pair.token0(), "Token0 path start not matching pair");
        require(to1 == _swapInfoOut.pair.token1(), "Token0 path start not matching pair");
        ...
        if (_refund)
        {
            if (addResult.t0Unused > 0)
            {
                IToken(to0).safeTransfer(msg.sender, addResult.t0Unused);
            }
            if (addResult.t1Unused > 0)
            {
                IToken(to1).safeTransfer(msg.sender, addResult.t1Unused);
            }
        }
    ...
    function takeFees(uint256 _amount) private view returns (uint256 amountAfterFee, uint256 fee)
    {   
    ...
    {
        (uint256 amount, ) = takeFees(super.getSwapEstimate(_amountIn, _router, _path));
        return amount;
    }
