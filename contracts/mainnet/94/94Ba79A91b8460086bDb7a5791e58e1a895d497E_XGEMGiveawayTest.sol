...
import "../security/Permissions.sol";
...
contract XGEMGiveawayTest is ReentrancyGuard, Pausable, WhiteListV2, RegistryClient {
    using SafeERC20 for IERC20;
    event LookUpAddrSuccess(string name, address addr);
    event LookUpAddrReq(string name);
    ...
    mapping(address => ClaimLog) private claimLog;
    address public treasuryWallet;
    string private xgemContractID;
    string public whiteListID;

    constructor(
        address _treasuryWallet,
        address _regAddr,
        string memory _xgemContractID,
        string memory _whiteListID
    ) {
        ...
        require(_treasuryWallet != address(0), "0 address for treasury wallet");
        require(!compare(_xgemContractID, ""), "xgem contract id empty");
        require(!compare(_whiteListID, ""), "whitelist id empty");
        treasuryWallet = _treasuryWallet;
        xgemContractID = _xgemContractID;
        whiteListID = _whiteListID;
    }

    // Get claimed amount
    function getClaimLog(address _user) external view returns (ClaimLog memory) {
        return claimLog[_user];
    }

    // Set treasury wallet
    function setTreasuryWallet(address _treasuryWallet) external onlySU {
        require(_treasuryWallet != address(0), "0 address for treasury wallet");
        treasuryWallet = _treasuryWallet;
    }
    ...
    function getXGEMContract1() public {
        emit LookUpAddrReq(xgemContractID);
        address xgemContractAddress = lookupContractAddr(xgemContractID);
        emit LookUpAddrReq(xgemContractID);

        require(
            xgemContractAddress != address(0),
            concatenate(">>address not found in registry for: ", xgemContractID)
        );

        emit LookUpAddrSuccess(xgemContractID, xgemContractAddress);
    }

    function getXGEMContract2() public view returns (address) {
        address xgemContractAddress = lookupContractAddr(xgemContractID);
        return xgemContractAddress;
    }

    function getXGEMContract() private view returns (IERC20) {
        address xgemContractAddress = lookupContractAddr(xgemContractID);
        ...
        return IERC20(xgemContractAddress);
    }

    /* Submit a merkle tree proof for being in the whitelist and claim the approved XGEM */
    function claimXGEM(bytes32[] memory proof, string memory leafSource)
        external
        whenNotPaused
        nonReentrant
    {
        ...
        bool isOnWhiteList;
        uint256 amount;
        (isOnWhiteList, amount) = isAllowed(whiteListID, proof, leafSource);
        // on WL, the amount is in ether units and mul by 1 ether
        // is needed to transfer exact amount on WL
        amount = amount * 1 ether;
        require(isOnWhiteList, "not in WL");
        require(claimLog[msg.sender].amount == 0, "claimed already");
        IERC20 xgemContract = getXGEMContract();
        claimLog[msg.sender] = ClaimLog(amount, block.timestamp);
        uint256 selfBalance = xgemContract.balanceOf(address(this));
        require(selfBalance > amount, "insufficient balance");
        bool transferStatus = xgemContract.transfer(msg.sender, amount);
        require(transferStatus, "xgem transfer failed");
    }
    ...
    // Transfer contract's XGEMs to treasury wallet
    function transferXGEM() external nonReentrant onlySU {
        IERC20 xgemContract = getXGEMContract();
        uint256 balance = xgemContract.balanceOf(address(this));
        require(balance > 0, "insufficient balance");
        xgemContract.safeTransfer(treasuryWallet, balance);
    }
}
...
/**
 * @dev roles used in Ethlas contracts (based on access control support by Openzeppelin)
 */
abstract contract Permissions is AccessControlEnumerable {
    // administers payementProcessorStorage
    bytes32 public constant PAY_SUBMITTER_ROLE = keccak256("PAY_SUBMITTER_ROLE");
    bytes32 public constant PAY_APPROVER_ROLE = keccak256("PAY_APPROVER_ROLE");
    // administers contract directory
    bytes32 public constant SU_ROLE = keccak256("SU_ROLE");
    bytes32 public constant READINESS_ROLE = keccak256("READINESS_ROLE");
    bytes32 public constant ORACLE_RNG_ROLE = keccak256("ORACLE_RNG_ROLE");
    ...
    modifier onlyOracleRNG() {
        require(hasRole(ORACLE_RNG_ROLE, msg.sender), "Caller is not ORACLE_RNG_ROLE");
        _;
    }
    ...
    modifier onlyMinter() {
        require(hasRole(MINTER_ROLE, msg.sender), "Caller is not MINTER_ROLE");
        _;
    }

    modifier onlyPayApprover() {
        require(hasRole(PAY_APPROVER_ROLE, msg.sender), "Not in PAY_APPROVER_ROLE");
        _;
    }

    modifier onlyPaySub() {
        require(hasRole(PAY_SUBMITTER_ROLE, msg.sender), "Not in PAY_SUBMITTER_ROLE");
        _;
    }

    modifier onlyReadiness() {
        require(hasRole(READINESS_ROLE, msg.sender), "Caller is not in READINESS_ROLE");
        _;
    }
}
