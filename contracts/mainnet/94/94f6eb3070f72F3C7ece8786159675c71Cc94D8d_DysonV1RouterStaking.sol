...
import {IGauge, IDysonV1RouterStaking} from "./interfaces/IDysonV1RouterStaking.sol";
import {IDysonVaultV1} from "./interfaces/IDysonVaultV1.sol";
...
contract DysonV1RouterStaking is
    IDysonV1RouterStaking,
    Initializable,
    PausableUpgradeable,
    OwnableUpgradeable
{
    ...
    /// @notice addLiquidity adds liquidity to DysonVaultV1 pool of interest (mints LP tokens)
    /// @param pool address of DysonVaultV1 pool to add liquidity to
    ...
    /// @param receiver account to receive minted DysonVaultV1 tokens
    /// @return amount0 amount of token0 transferred from msg.sender to mint `mintAmount`
    /// @return amount1 amount of token1 transferred from msg.sender to mint `mintAmount`
    /// @return mintAmount amount of DysonVaultV1 tokens minted and transferred to `receiver`
    ...
    /// @notice addLiquidityAndStake same as addLiquidity except Dyson LP token
    /// is immediately staked in corresponding LiquidityGauge. New param:
    /// @param gauge the address of the LiquidityGauge corresponding to the DysonVaultV1
    ...
    /// @notice addLiquidityETHAndStake same as addLiquidityETH except Dyson LP token
    /// is immediately staked in corresponding LiquidityGauge. New param:
    /// @param gauge the address of the LiquidityGauge corresponding to the DysonVaultV1
    ...
    /// @notice removeLiquidity removes liquidity from a DysonVaultV1 pool and burns LP tokens
    /// @param burnAmount The number of DysonVaultV1 tokens to burn
    ...
    /// @notice removeLiquidityAndUnstake same as removeLiquidity except Dyson LP token
        ...
        )
    {
        ...
        address pool = gauge.staking_token();
        (amount0, amount1, liquidityBurned) = IDysonVaultV1(pool).burn(
            burnAmount,
            receiver
        );
        require(
        ...
        )
    {
        ...
        IERC20 token0 = IDysonVaultV1(pool).token0();
        IERC20 token1 = IDysonVaultV1(pool).token1();
        ...
        )
    {
        ...
        (uint256 amount0In, uint256 amount1In, uint256 _mintAmount) = pool
...
import {IDysonV1RouterBase} from "./IDysonV1RouterBase.sol";
import {IGauge} from "./IGauge.sol";

interface IDysonV1RouterStaking is IDysonV1RouterBase {
    function addLiquidityAndStake(
        IGauge gauge,
        uint256 amount0Max,
        uint256 amount1Max,
...
import {IDysonVaultV1} from "./IDysonVaultV1.sol";

interface IDysonV1RouterBase {
    function addLiquidity(
        IDysonVaultV1 pool,
        uint256 amount0Max,
        uint256 amount1Max,
