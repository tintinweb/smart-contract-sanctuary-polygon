...
contract TokenCondensor is BaseSiloAction{
        ...
        for(uint i=0; i<4; i++){
            ...
            if(input[index] == output[index]){
                ...
                if(path[0] == path[1]){
                    ...
                    amountToSwap += amount[index];
                    if(amountToSwap > 0){
                        uint amountOut = IUniswapV2Router02(router[index]).getAmountsOut(amountToSwap, path)[1] * 9995/10000;
                        if(amountOut > 0){
                            IERC20(path[0]).approve(router[index], amountToSwap);
                            amounts = IUniswapV2Router02(router[index]).swapExactTokensForTokens(
                                amountToSwap,
                                0,
                                path,
                                address(this),
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        if(input[0] != output[0]){
            ...
            if(router[0] == address(0)){
                return false;
            }
            address pair = IUniswapV2Factory(IUniswapV2Router02(router[0]).factory()).getPair(input[0], output[0]);
            if(pair == address(0)){
                return false;
            }
        }
        ...
        for (uint i=0; i<3;i++){
            index = 3-i;
            if(input[index] != output[index]){//then we are trying to make a swap!
                if(router[index] == address(0)){//make sure router is set
                return false;
                }
                //check the pair
                address pair = IUniswapV2Factory(IUniswapV2Router02(router[index]).factory()).getPair(input[index], input[index-1]);
                if(pair == address(0)){
                    return false;
                }
            }
