...
import "../../Impl/BhavishPredictionStorage.sol";
import "@openzeppelin/contracts/access/AccessControl.sol";
import "@openzeppelin/contracts/utils/Address.sol";

contract BhavisPredictionWrapper is AccessControl {
    ...
    struct BetArray {
        IBhavishPrediction.BetInfo betInfo;
        uint256 roundId;
    }

    mapping(bytes32 => mapping(bytes32 => BhavishPredictionStorage)) public bhavishPredictionStorage;
    ...
    function addBhavishPredictionStorage(
        BhavishPredictionStorage[] memory _bhavishPredictionStorage,
        bytes32[] memory _underlying,
        bytes32[] memory _strike
    ) public onlyAdmin(msg.sender) {
        require(_bhavishPredictionStorage.length == _underlying.length, "Invalid array length");
        require(_strike.length == _underlying.length, "Invalid array length");
        for (uint256 i = 0; i < _bhavishPredictionStorage.length; i++) {
            require(address(_bhavishPredictionStorage[i]).isContract(), "Not a contract");
            bhavishPredictionStorage[_underlying[i]][_strike[i]] = _bhavishPredictionStorage[i];
        }
    }

    function removeBhavishPredictionStorage(
        bytes32 _underlying,
        bytes32 _strike,
        BhavishPredictionStorage _bhavishPredictionStorage
    ) public onlyAdmin(msg.sender) {
        require(bhavishPredictionStorage[_underlying][_strike] == _bhavishPredictionStorage, "Address not present");
        delete bhavishPredictionStorage[_underlying][_strike];
    }

    function fetchUserRoundDetais(
        uint256 _from,
        uint256 _to,
        address _userAddress,
        bytes32 _underlying,
        bytes32 _strike
    ) public view returns (BetArray[] memory betArray) {
        BhavishPredictionStorage pred = bhavishPredictionStorage[_underlying][_strike];
        require(address(pred) != address(0), "Asset not supported");
        uint256 latestRoundId = pred.latestRoundId();
        require(_to <= latestRoundId, "Index out of bound");
        require(_from < _to, "From < To");

        betArray = new BetArray[](_to - _from);

        for (uint256 i = 0; i < betArray.length; i++) {
            BetArray memory arr;
            arr.roundId = pred.userRounds(_userAddress, _from + i);
            arr.betInfo = pred.getBetInfo(arr.roundId, _userAddress);
            betArray[i] = arr;
        }
    }
}
