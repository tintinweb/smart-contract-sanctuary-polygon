...
contract SweetKitties is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.2 ether;
    uint256 public maxMints = 109;
    ...
    constructor()
        ERC721("Sweet Kitties", "SK")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 109) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeid5gzkujs3nwt26mujlvhxdtsqonemp3nmcamuoa4pyr4lsg6jdlm/", Strings.toString(tokenId), ".json")
            );
    }
