...
contract Semantic is ISemantic, AccessControl,ERC721 {
    ...
    //主语 
    mapping(uint256 => string) public subjectIdForDataMap; 
    mapping(string => uint256) public subjectDataForIdMap;
    ...
    mapping(uint256 => mapping(uint256=>mapping(string=>uint256))) public tokenDataForIdMap;

    mapping(uint256 => Trida) public tokenIdForDataMap;
    ...
    string public constant PREDICATE_REMOVED = "PREDICATE_REMOVED";
     ...
     * @param classes class
     */
    function addClasss(address owner, Class[] memory classes) external {
     ...
     */
    function addPredicates(address owner, Predicate[] memory predicateInfos)
        external
    {
     ...
     */
    function adminMint(AdminOperateData[] memory operateDatas) external {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            AdminOperateData memory operateData = operateDatas[i];
            require(operateData.predicateId < predicateSize,"predicate undefined");
            uint256 subjectId = subjectDataForIdMap[operateData.subject.value];
            if(subjectId == 0){
                subjectSize++;
                subjectDataForIdMap[operateData.subject.value] = subjectSize;
                subjectIdForDataMap[subjectSize] = operateData.subject.value;
            }
            Trida memory trida = Trida(subjectId,operateData.predicateId,operateData.object);
            _mint(msg.sender, trida);
        }
    }
     ...
     * @param operateDatas 操作数据
     */
    function mint(UserOperateData[] memory operateDatas) external {
 ...
 /**
     ...
     * @param operateDatas 操作数据
     */
    function adminBurn(AdminOperateData[] memory operateDatas) external {
     ...
     * @param operateDatas 操作数据
     */
    function burn(UserOperateData[] memory operateDatas) external {
        // for (uint256 i = 0; i < operateDatas.length; i++) {
    ...
    function _mint(address owner, Trida memory trida) private {
        uint256 tokenId = totalSupply;
        tokenDataForIdMap[trida.subjectId][trida.predicateId][trida.object] = tokenId;
        tokenIdForDataMap[tokenId] = trida;

        _balanceOf[owner] = _balanceOf[owner] + 1;
        tokenHolderMap[tokenId] = owner;

        totalSupply++;
        emit Mint(msg.sender, owner, tokenId, trida.subjectId,trida.predicateId,trida.object);
...
interface ISemantic {
    ...
    struct AdminOperateData {
        Subject subject;
        uint256 predicateId;
        string object;
    }

    struct UserOperateData {
        address owner;
        uint256 predicateId;
        string object;
    }
     ...
     * @param owner 所属者，e.g: 0xa5eb265c9814c898a4a9aA3c8542edEE09897566
     ...
     * @param subjectId 主语id
     * @param predicateId 谓语id
     * @param object 宾语   
     */
    event Mint(
        address indexed operator,
     ...
     * @param owner 所属者，e.g: 0x854C2B6Cf940959b71fAC98f69598D4002b7525F
     * @param tokenId schemaId，e.g: 100
     * @param rdfOriginalData schema的原始数据，
     ...
     * @param predicateInfos 谓词
     */
    function addPredicates(address owner, Predicate[] memory predicateInfos) external;
