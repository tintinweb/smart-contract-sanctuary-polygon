...
contract ControllerFactory is Owned {
address factory;
//WETH MATIC MAINNET
IERC20 wrappedToken = IERC20(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270);
IPancakeFactory _pancakeFactory = IPancakeFactory(0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32);
...
function createLiquidityController(RootedToken root, EliteToken elite, EliteFloorCalculator calc, RootedTransferGate gate) public onlyFactory() returns (LiquidityController) {
    return new LiquidityController(_router, wrappedToken, root, elite, calc, gate);
}

function createTokenTimelock() public onlyFactory() returns (TokenTimelock) {
    return new TokenTimelock();
}
}
