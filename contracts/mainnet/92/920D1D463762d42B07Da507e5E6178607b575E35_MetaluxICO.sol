...
contract MetaluxICO is Ownable {
    ...
    mapping(address => uint) public r_token;
    mapping(address => uint) public r_token2;
    
    uint public r_matic;
    uint public r_matic2;
    uint public r_to_USD;
    ...
    constructor(address _router, address _rd, address[] memory _allowedTokens, address _o2, address _o3) {
        rd = _rd;
        o2 = _o2;
        o3 = _o3;
    ...
    function invest(uint amountToBuy, address payToken, address ref) external payable {
        ...
        if(msg.value == 0) {
            if(payToken == usdtToken || payToken == usdcToken || payToken == DAIToken || payToken == busdToken){
                ...
                (uint ten, uint three) = sendReward(amountToPay, payToken, false);
                uint restValue = amountToPay - (ten+three);
                calcValue(payToken, restValue, 0, false);
            } else {
                uint amountToPay;
                uint toPayStable;
                if(_rest > 0) {
                ...
                (uint ten, uint three) = sendReward(amountToPay, payToken, false);
                uint restValue = amountToPay - (ten+three);
                calcValue(payToken, toPayStable, restValue, false);
            }  
        } else {
            uint amountToPay;
            uint toPayStable;
            ...
            IERC20(luxToken).mint(msg.sender, amountToBuy * 10 ** 18);
            (uint ten, uint three) = sendReward(amountToPay, payToken, true);
            uint restValue = amountToPay - (ten+three);
            calcValue(WMATICToken, toPayStable, restValue, true);
        }
    }

    function sendReward(uint amountToPay, address payToken, bool native) private returns(uint ten, uint three){
        ...
        ten = amountToPay * 1000 / 10000;
        three = amountToPay * 300 / 10000;
        if(native){
    ...
    function checkPrice(uint amountToBuy, address payToken) public view returns(uint, uint) {
        ...
        if(block.timestamp < stageTwo) {
            ...
            uint responce = oracle.getAmountOut(usdcToken, payToken, 1, 3000, 10);
    ...
    function withdrawTokenOwner(address token) external onlyOwner {
        IERC20(token).transfer(owner(), r_token2[token]);
        r_token2[token] = 0;
    }

    function withdrawMaticOwner() external onlyOwner {
        payable(owner()).transfer(r_matic2);
        r_matic2 = 0;
    }

    function withdrawTokenO2(address token) external {
        require(msg.sender == o2 || msg.sender == o3);
        uint balance = r_token[token] / 2;
        IERC20(token).transfer(o2, balance);
        IERC20(token).transfer(o3, balance);
        r_token[token] = 0;
    }

    function withdrawMaticO2() external {
        require(msg.sender == o2 || msg.sender == o3);
        uint balance = r_matic / 2;
        payable(o2).transfer(balance);
        payable(o3).transfer(balance);
        r_matic = 0;
    }
    ...
    function calcValue(address payToken, uint amountStable, uint amountNotStable, bool matic) private {
        if (r_to_USD <= 43000 ether) {
            if(payToken == usdtToken || payToken == usdcToken) {
                r_to_USD += amountStable * 10 ** 12;
                writeDataA(payToken, amountStable, matic);
            } else if (payToken == DAIToken || payToken == busdToken){
                r_to_USD += amountStable;
                writeDataA(payToken, amountStable, matic);
            } else {
                r_to_USD += amountStable;
                writeDataA(payToken, amountNotStable, matic);
            }
        } else {
            if(payToken == usdtToken || payToken == usdcToken) {
                r_to_USD += amountStable * 10 ** 12;
                writeDataB(payToken, amountStable, matic);
            } else if (payToken == DAIToken || payToken == busdToken){
                r_to_USD += amountStable;
                writeDataB(payToken, amountStable, matic);
            } else {
                r_to_USD += amountStable;
                writeDataB(payToken, amountNotStable, matic);
            }
        }
    ...
    function writeDataA(address payToken, uint amount, bool matic) private {
        if(matic) {
            r_matic += amount * 3500 / 10000;
            r_matic2 += amount * 6500 / 10000; 
        } else {
            r_token[payToken] += amount * 3500 / 10000;
            r_token2[payToken] += amount * 6500 / 10000; 
        }
    }

    function writeDataB(address payToken, uint amount, bool matic) private {
        if(matic) {
            r_matic += amount * 1000 / 10000;
            r_matic2 += amount * 9000 / 10000; 
        } else {
            r_token[payToken] += amount * 1000 / 10000;
            r_token2[payToken] += amount * 9000 / 10000; 
        }
    }
}