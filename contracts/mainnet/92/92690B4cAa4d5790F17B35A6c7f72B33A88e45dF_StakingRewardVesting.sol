...
contract StakingRewardVesting is VestingHub {
    ...
    uint256 internal _LOCK_DURATION = 2 weeks;
    uint256 internal _INITIAL_REALEASE_BUFFER = 1 weeks;
...
contract VestingHub is Ownable, ReentrancyGuard {
    ...
    uint256 public initialRealeseBuffer;
    ...
    constructor(
        ...
        initialRealeseBuffer = initialRealeseBuffer_;
        transferOwnership(0x6FA6DA462CBA635b0193809332387cDC25Df3e8D);
    }
    ...
    function getReleasedTokensAtTimestamp(
        ...
        if (timestamp < start + initialRealeseBuffer) {
            return 0;
        }

        if (timestamp < start + lockDuration) {
            uint256 initialTokens = (_allocatedTokens[beneficiary] *
                initialReleasePercentage) / 100;

            uint256 claimedTokens = getClaimedTokens(beneficiary);

            return initialTokens - claimedTokens;
        }
    ...
    function updateLockDuration(uint256 _lockedDuration) external onlyOwner {
