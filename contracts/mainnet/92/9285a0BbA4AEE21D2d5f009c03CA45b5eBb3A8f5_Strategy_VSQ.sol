...
/*
    This strategy is for POLY network.
    VESQ token uses for staking.
    MATIC -> WMATIC -> WETH -> DAI -> VESQ routing
    Token Helper = "Staked VSQ (sVSQ)"
*/
contract Strategy_VSQ is Ownable {
    ...
    uint256 public pendingFeeWETH;
    uint256 public pendingFeeDAI;
    uint256 public pendingFeeVESQ;
    address public yelLiquidityRouter = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff;
    address public router = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506; // SushiSwapRouter
    address public routerFee = 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506;
    address public lpToken = 0x5Cf66CeAf7F6395642cD11b5929499229edEF531; // VESQ - 9 dec : Dai - 18 dec
    address public vault;
    address public mainToken = 0xbb0FDc2Fe6D3cEB6Bf76Cc955173a07EDbF57494; // sVSQ decimals 9
    address public WETH = 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619;
    address public DAI = 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063; // token1 - decimals 18
    address public VESQ = 0x29F1e986FCa02B7E54138c04C4F503DdDD250558; // token0 - decimals 9

    // ------------> Important constants <--------------
    address public constant USDT = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
    address public constant YELtoken = 0xD3b71117E6C1558c1553305b44988cd944e97300;
    address public constant wrappedToken = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address public constant stakingHelper = 0x493Fdb9ddFd51873a878494B0E4d858D6DEc57E9; // stake
    address public constant staking = 0x2F3E9e54bD4513D1B49A6d915F9a83310638CFC2; // unstake
    // ------------>        END          <--------------
    ...
    function earn() public returns(uint256 _balance) {
        _balance = getBalanceOfToken(VESQ);
        if(_balance > 0) {
            _approveToken(VESQ, stakingHelper);
            IStakingHelper(stakingHelper).stake(_balance, address(this));
            emit Earn(_balance);
        }
    }
    //  -------------------------> END <---------------------------
    ...
    function emergencyWithdraw(address _receiver) public onlyVaultOrOwner {
        ...
        _transfer(VESQ, _receiver, getBalanceOfToken(VESQ));
        _transfer(DAI, _receiver, getBalanceOfToken(DAI));
    ...
    function deposit() public payable onlyVault returns (uint256) {
        ...
        _approveToken(VESQ, router);
        _approveToken(DAI, router);
        _approveToken(WETH, router);
        _checkPrices();
        _swapExactETHForTokens(WETH, address(this).balance);
        _swapExactTokenForToken(router, WETH, DAI, getBalanceOfToken(WETH));
        _swapExactTokenForToken(router, DAI, VESQ, getBalanceOfToken(DAI));
        _updateLastPrices();
    ...
    function depositAsMigrate() public onlyVault {
        ...
        _approveToken(DAI, router);
        _approveToken(WETH, router);
        _swapExactTokenForToken(router, wrappedToken, WETH, getBalanceOfToken(wrappedToken));
        _swapExactTokenForToken(router, WETH, DAI, getBalanceOfToken(WETH));
        _swapExactTokenForToken(router, DAI, VESQ, getBalanceOfToken(DAI));
        earn();
    }
    ...
    function getBalanceOfToken(address _token) public view returns (uint256) {
        ...
        if(_token == VESQ) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeVESQ;
        }
        if(_token == DAI) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeDAI;
        }
        if(_token == WETH) {
            return IERC20(_token).balanceOf(address(this)) - pendingFeeWETH;
        }
    ...
    function setLP(address _address) public onlyVaultOrOwner {
        require(_address != address(0), "The address can not be zero address");
        require(VESQ == IPair(_address).token0() && DAI == IPair(_address).token1(), "Tokens don't match");
    ...
    function takeFee(uint256 _total) external onlyVault {
        uint256 _amount = _calculateAmountFee(_total);
        if(_amount > 0 && getBalanceOfToken(mainToken) <= _amount)
            _withdrawFromStaking(_amount);

        _amount = getBalanceOfToken(VESQ) + pendingFeeVESQ;
        bool result = _swapExactTokenForToken(router, VESQ, DAI, _amount);
        if(result) {
            emit TakeFee(_amount);
            pendingFeeVESQ = 0;
        } else {
            pendingFeeVESQ = _amount;
        }

        _amount = getBalanceOfToken(DAI) + pendingFeeDAI;
        result = _swapExactTokenForToken(router, DAI, WETH, _amount);
        if(result) {
            emit TakeFee(_amount);
            pendingFeeDAI = 0;
        } else {
            pendingFeeDAI = _amount;
        }

        _amount = getBalanceOfToken(WETH) + pendingFeeWETH;
        result = _swapExactTokenForToken(router, WETH, wrappedToken, _amount);
        if(result) {
            emit TakeFee(_amount);
            pendingFeeWETH = 0;
        } else {
            pendingFeeWETH = _amount;
        }

        _amount = getBalanceOfToken(wrappedToken) + pendingFee;
        result = _swapExactTokenForToken(router, wrappedToken, USDT, _amount);
        if(result) {
            emit TakeFee(_amount);
            pendingFee = 0;
        } else {
    ...
    function _swapTokenToWrapped(uint256 _total, uint256 _percent) internal {
        ...
        _swapExactTokenForToken(router, VESQ, DAI, getBalanceOfToken(VESQ));
        _swapExactTokenForToken(router, DAI, WETH, getBalanceOfToken(DAI));
        _swapExactTokenForToken(router, WETH, wrappedToken, getBalanceOfToken(WETH));
    }
    ...
    function _getToken0Price() public view returns (uint256) {
        ...
        path[0] = DAI;
        path[1] = VESQ;
        return 1e27 / (_getAmountsOut(router, 10 * 1e18, path) / 10);
    }
    ...
    function _getToken1Price() public view returns (uint256) {
        ...
        path[0] = VESQ;
        path[1] = DAI;
        return 1e27 / (_getAmountsOut(router, 10 * 1e9, path) / 10);
    }

    function _getWethPerDaiPrice() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = WETH;
        path[1] = DAI;
        return 1e36 / _getAmountsOut(router, 1e18, path);
    }

    function _getWrappedPerWethPrice() public view returns (uint256) {
        address[] memory path = new address[](2);
        path[0] = wrappedToken;
        path[1] = WETH;
        return 1e36 / (_getAmountsOut(router, 100 * 1e18, path) / 100);
    }

    function _getSimpleTCI(uint256 _amount) internal view returns (uint256 tCI) {
    	uint256 amountDAI = _getToken0Price() * _amount / 1e9;
    	uint256 amountWETH = _getWethPerDaiPrice() * amountDAI / 1e18;
        tCI += _getWrappedPerWethPrice() * amountWETH / 1e18;
    ...
    function _getSimpleTC() internal view returns (uint256 TC) {
        ...
        if (_balance > 0) {
            path[0] = VESQ;
            path[1] = DAI;
            amountDAI = _getAmountsOut(router, _balance, path);
        }

        _balance = getBalanceOfToken(DAI) + amountDAI;
        if (_balance > 0) {
	        path[0] = DAI;
	        path[1] = WETH;
	        amountWETH = _getAmountsOut(router, _balance, path);
	    }

	    _balance = getBalanceOfToken(WETH) + amountWETH;
        if (_balance > 0) {
	        path[0] = WETH;
	        path[1] = wrappedToken;
