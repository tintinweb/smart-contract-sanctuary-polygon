...
contract ORIGINAL_NFT_FACTORY is ERC721Enumerable, Ownable {
    ...
    uint256 public LAUNCH_MAX_SUPPLY = 20;    // max launch supply
    uint256 public LAUNCH_SUPPLY = 0;        // current launch supply

    address public LAUNCHPAD = address(0);   // current launch supply
    ...
    function setMaxLaunchpadSupply(uint256 _launch_max_supply) public {
        LAUNCH_MAX_SUPPLY = _launch_max_supply;
    }

    function setLaunchpadSupply(uint256 _launch_supply) public {
        LAUNCH_SUPPLY = _launch_supply;
    }

    function setLaunchpadContract(address _launchpad) public {
    ...
    ) ERC721("ORIGINAL NFT Factory", "ORNFT") {
    ...
    function burn(uint256 tokenId) external {
        require(isBurnEnabled, "ORNFT: burning disabled");
        require(
            _isApprovedOrOwner(msg.sender, tokenId),
            "ORNFT: burn caller is not owner nor approved"
        );
        _burn(tokenId);
        _tokenIds.decrement();
    }

    // MINTER ORIGINAL NFT FACTORY
