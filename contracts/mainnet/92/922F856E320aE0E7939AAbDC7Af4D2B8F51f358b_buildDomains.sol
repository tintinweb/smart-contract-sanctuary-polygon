...
contract buildDomains is ERC721URIStorage, Pausable{
...
//<text x="32.5" y="220" font-size="23" fill="url(#g1)" filter="url(#A)" font-family="Plus Jakarta Sans,DejaVu Sans,Noto Color Emoji,Apple Color Emoji,sans-serif" font-weight="bold">';
  ...
  address[2000] private whiteUser;

  //register event
  event registEvent(
      string _registName,
      uint256 _registId,
      address _registAddress
  );
  //record event
  event recordEvent(
      string recordName,
  ...
  constructor(string memory _tld) payable ERC721("krypectro Name Service", "build") {
   ...
   function register(string memory name) public payable{
      ...
      uint256 _price = setPrice(name);
      //Check whether the IP address is whitelisted
      if(whitelist[msg.sender]==true){
        //It's important to limit the number of times 
        (bool sent, ) = (msg.sender).call{value: thisWhitePrice}("");
        require(sent,"This whiteAddress not enough Matic paid");
      ...
      emit registEvent(name,newRecordId,msg.sender);
  ...
  function removeWhitelist(address _newEntry) public {
    require(whitelist[_newEntry]==true,"not a whitelist");
  ...
  function setRecord(string memory name, string memory record) public{
      ...
      uint256 getDomainForId=getNameToTokenId(name);
      require(msg.sender == ERC721.ownerOf(getDomainForId),"Record error!");
      records[name] = record;
      emit recordEvent(name,record,msg.sender);
  }
  ...
  function getDomainAddress(string memory name) public view returns (address) {
      //Check that the owner is the transaction sender
      return ERC721.ownerOf(getNameToTokenId(name));
  }
  ...
  function getNameToTokenId(string memory name) public view returns(uint){
  ...
  function setPrice(string memory name) public pure returns(uint) {
  ...
  function whitePrice(string memory name) public pure returns(uint) {
    uint len = StringUtils.strlen(name);
    require(len > 0 && 3<=len && len<=30);
   if (len == 3) {
      return 0; //0matic. 
  ...
  function setDeadline(uint deadNumber) external onlyOwner{
    require(deadNumber==77,"Destruction of failure!");
