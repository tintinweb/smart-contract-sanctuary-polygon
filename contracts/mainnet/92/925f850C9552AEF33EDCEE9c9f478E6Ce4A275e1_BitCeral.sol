...
contract BitCeral is ReentrancyGuard {
 ...
 modifier onlyOwner {
    ...
    _giveAway = (GIVEAWAY_ADDRESS);
        ...
        MIN_REF_DEPOSIT_FOR_BONUS = 7410000000000000; // 200 dolar
    }

function setBTC(address newBTC) public onlyOwner {
    BTC = newBTC;
}
...
function setGiveawayAddress(address newGiveawayAddress) public onlyOwner {
 ...
 function user(address adr) public view returns (Baker memory) {
    ...
    function buyBeans(address ref, uint256 _amount) public {
        ...
        IERC20(BTC).transferFrom(msg.sender, address(this), _amount);
    ...
    function sendFees(uint256 totalFee, uint256 giveAway) private {
        ...
        IERC20(BTC).transfer(_dev, dev);
        IERC20(BTC).transfer(_marketing, marketing);
        IERC20(BTC).transfer(_ceo, ceo);

        if (giveAway > 0) {
            IERC20(BTC).transfer(_giveAway, giveAway);
        }
    }
    ...
    function eat() public nonReentrant {
        ...
        IERC20(BTC).transfer(msg.sender, bnbToEat);
