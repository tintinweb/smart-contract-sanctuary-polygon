...
contract LXYArt is ERC721, ERC721Royalty, ERC721Enumerable, ERC721URIStorage, ERC721Burnable, ERC721Pausable, Ownable, AccessControl, ReentrancyGuard {
    using Address for address;
    ...
    address private beneficiaryAddress = 0x7D7EFd156CBcfBC2F149E1D47eCBbE2f3f9122E8;
    address public feeToken = address(0x0);
    mapping(address => uint16) public shareFree;
    address[] public freeAddr;
    ...
    function setShareFree(
        address _freeAddr,
        uint16 _count
    ) external onlyRole(MINTER_ROLE) {
        shareFree[_freeAddr]=shareFree[_freeAddr]+_count;
        freeAddr.push(_freeAddr);
    }
    ...
    function safeMint(
        ...
        uint96 royaltyFee
    ) external payable nonReentrant noSaleOut {
        if (feeToken == address(0)) {
            require(msg.value >= _MINT_FEE || hasRole(MINTER_ROLE,_msgSender()) || shareFree[_msgSender()]>0, "insufficient fee");
        }else{
            if(!hasRole(MINTER_ROLE,_msgSender()) && shareFree[_msgSender()]==0){
                IERC20(feeToken).transferFrom(msg.sender, address(this), _MINT_FEE);
                emit Payed(msg.sender, feeToken, _MINT_FEE, block.timestamp);
            }
        }
    ...
    function batchMint(
        ...
        if (feeToken == address(0)) {
            require(msg.value == tokenAmount * _MINT_FEE || hasRole(MINTER_ROLE,_msgSender()), "insufficient fee");
        }else{
            if(!hasRole(MINTER_ROLE,_msgSender()) && shareFree[_msgSender()]==0){
                IERC20(feeToken).transferFrom(msg.sender, address(this), tokenAmount * _MINT_FEE);
                emit Payed(msg.sender, feeToken, tokenAmount * _MINT_FEE, block.timestamp);
            }
        }
        for (uint256 i; i < tokenAmount; i++) {
            _mint(to, tokenURIList[i], royaltyFee);
        }
    }
