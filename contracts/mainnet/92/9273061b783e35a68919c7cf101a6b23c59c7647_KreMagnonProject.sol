...
contract KreMagnonProject is ERC721A, ERC2981, OperatorFilterer, Ownable {
    using ECDSA for bytes32;

    bool public operatorFilteringEnabled;
    mapping(bytes4 => bool) public functionLocked;

    uint256 MAX_MINTS = 498;
    uint256 MAX_SUPPLY = 498;
    ...
    constructor(address _royaltyReceiver, uint96 _royaltyFraction)
        ERC721A("Crazy Kre-Magnon Club", "KREMA")
    {
        setHiddenMetadataUri(
            "https://kremagnonproject.netlify.app/server/asset/metadata/"
        );
        ...
        returns (uint256[] memory)
    {
        ...
        while (
            ownedTokenIndex < ownerTokenCount && currentTokenId <= MAX_MINTS
        ) {
