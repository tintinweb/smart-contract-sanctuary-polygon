...
contract UnoFarmApeswap is Initializable, ReentrancyGuardUpgradeable {
     ...
     * {lpPair} - Token that the strategy maximizes. The same token that users deposit in the {MiniApe}.
    ...
    function initialize(address _lpPair, address _assetRouter) external initializer {
        ...
        IERC20(lpPair).approve(address(MiniApe), type(uint256).max);
        IERC20(lpPair).approve(address(apeswapRouter), type(uint256).max);
