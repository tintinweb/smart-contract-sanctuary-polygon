...
contract Silo is Initializable, KeeperCompatibleInterface, IERC721Receiver {
    ...
    modifier onlySiloManager() {
        require(
            ISiloManagerFactory(ISiloFactory(factory).managerFactory()).isManager(msg.sender),
            "not manager"
        );
        _;
    }
    ...
    function setMinGravityReward(uint256 _reward) external {
        require(
            msg.sender == ISiloFactory(factory).ownerOf(SILO_ID) || msg.sender == factory,
            "wrong caller"
        );
    ...
    function setStrategy(
        ...
        for (uint256 i; i < _implementations.length; ) {
            ...
            if (
                !highRiskAction && ISiloFactory(factory).highRiskActions(_implementations[i])
            ) {
                highRiskAction = true;
            }
    ...
    function adjustStrategy(
        ...
        if (!highRiskAction && ISiloFactory(factory).highRiskActions(_implementation)) {
            highRiskAction = true;
        }
