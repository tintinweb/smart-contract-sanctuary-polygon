...
contract GiddyGiddyYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  uint256 constant private SWAP_FEE = 3000;
  ...
  UniswapV2Giddy private uniswapGiddy;
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, USDC_TOKEN, GIDDY_TOKEN, SWAP_FEE, value - fap);
  }
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    (amountOut, priceImpact) = calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, SWAP_FEE, sharesToValue(shares));
    amountOut -= fap;
  }
...
contract UniswapV2Giddy {
  ...
  function calcSwap(address router, address src, address dst, uint256 swapFee, uint256 inBefore, uint256 inAfter) public view returns (uint256 outBefore, uint256 outAfter) {
    ...
    outAfter = (outAfter * (BASE_PERCENT - swapFee)) / BASE_PERCENT;
  }

  function calcPrice(address router, address[] memory route, uint256 swapFee, uint256 value) public view returns (uint256 amountOut, uint256 priceImpact) {
    uint256 outBefore = value;
    amountOut = value;
    for (uint16 i = 0; i < (route.length - 1); i++) {
      (outBefore, amountOut) = calcSwap(router, route[i], route[i + 1], swapFee, outBefore, amountOut);
    }
  ...
  function calcPriceSimple(address router, address src, address dst, uint256 swapFee, uint256 value) public view returns (uint256 amountOut, uint256 priceImpact) {
    address[] memory route = new address[](2);
    route[0] = src;
    route[1] = dst;
    (amountOut, priceImpact) = calcPrice(router, route, swapFee, value);
  }
