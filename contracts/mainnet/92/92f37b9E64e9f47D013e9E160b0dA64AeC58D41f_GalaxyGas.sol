...
// File: contracts/GGAS.sol


pragma solidity ^0.8.4;


contract GalaxyGas is ERC20, AccessControl, ERC721Holder {
    using Strings for uint256;
    ...
    constructor(address owner, address _collectionAddress) ERC20("GalaxyGas", "GGAS") AccessControl(){
        ...
        coinAmountPerRewardUnit = 2 * 10 ** 18; // 8 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 30; // 12 hours per reward unit , may be changed later on
        welcomeBonusAmount = 150 * 10 ** 18; // 170 tokens welcome bonus, only paid once per tokenId
    }
    ...
    */
    ...
    function pendingRewards(address targetAddress, uint256 tokenId) public view returns (uint256){
        StakeInfo memory info = stakeLog[targetAddress][tokenId];
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "GGAS: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "GGAS: update to zero address not possible");
