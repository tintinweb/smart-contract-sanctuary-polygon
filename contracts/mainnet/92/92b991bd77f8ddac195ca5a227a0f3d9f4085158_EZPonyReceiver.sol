...
contract EZPonyReceiver is IERC721Receiver, Ownable {
	...
	address public zedRunAddress = 0xA5F1Ea7DF861952863dF2e8d1312f7305dabf215;
	...
	function onERC721Received(
		...
		if (msg.sender == address(zedRunAddress)) {
			tokenIds.increment();
	...
	function withdrawToOwnerByAdmin(uint256 _tokenId) external onlyOwner {
		...
		require(!ezpTokens[_tokenId].isSale, "Token is on sale now.");
		IERC721(zedRunAddress).safeTransferFrom(address(this), ezpToken.owner, ezpToken.nftId);
	...
	function transferToBuyer(uint256 _tokenId, address _buyer) external onlyOwner {
		...
		IERC721(zedRunAddress).safeTransferFrom(address(this), _buyer, ezpToken.nftId);
		delete ezpTokens[_tokenId];
	}
	...
	function buy(uint256 _tokenId) external {
		...
		IERC721(zedRunAddress).safeTransferFrom(address(this), msg.sender, ezpTokens[_tokenId].nftId);
		delete ezpTokens[_tokenId];
	}
