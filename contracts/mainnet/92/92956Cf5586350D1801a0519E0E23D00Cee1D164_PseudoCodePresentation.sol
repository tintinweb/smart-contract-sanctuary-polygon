...
contract PseudoCodePresentation is IPresentation {
    ...
    function _getFontSource() internal pure returns (string memory) {
        return
    ...
    function _getBase64Svg(
        ...
        string memory tokenIdString = _toHexStringUpperCase(
            Strings.toHexString(tokenId, 8)
        );
        string memory formattedMorning = _formatEncryptedText(morning);
        string memory formattedAfternoon = _formatEncryptedText(afternoon);
        string memory formattedNight = _formatEncryptedText(night);
        ...
        svg = abi.encodePacked(
            svg,
            "</tspan><tspan x='94' dy='1em'>GET secret</tspan><tspan x='94' dy='1em'>IV = ",
            tokenIdString,
            "</tspan><tspan x='94' dy='1em'>morning = ",
            formattedMorning,
            "\u2026</tspan><tspan x='94' dy='1em'>afternoon = ",
            formattedAfternoon,
            "\u2026</tspan><tspan x='94' dy='1em'>night = ",
            formattedNight,
            "\u2026</tspan><tspan x='94' dy='1em'>IF decrypting morning by AES256 succeeds</tspan><tspan x='130' dy='1em'>PRINT decrypted morning</tspan><tspan x='94' dy='1em'>IF decrypting afternoon by AES256 succeeds</tspan><tspan x='130' dy='1em'>PRINT decrypted afternoon</tspan><tspan x='94' dy='1em'>IF decrypting night by AES256 succeeds</tspan><tspan x='130' dy='1em'>PRINT decrypted night</tspan></text></svg>"
        );
    ...
    function metadata(
        ...
        bytes memory metadataJson = abi.encodePacked(
            '{"name":"',
            _metadataName(tokenId, day),
            '","description":"',
            description,
            '","image":"data:image/svg+xml;base64,',
            _getBase64Svg(tokenId, day, morning, afternoon, night),
    ...
    function _metadataName(uint256 tokenId, string memory day)
        internal
        pure
    ...
    function _formatEncryptedText(string memory value)
        internal
        pure
        returns (string memory)
    {
        return
            _toHexStringUpperCase(
                _addHexPrefix(
                    _getSlice(0, 32, _padStart(32, _removeHexPrefix(value)))
                )
            );
    }

    function _toHexStringUpperCase(string memory value)
        internal
        pure
        returns (string memory)
    {
    ...
    function _removeHexPrefix(string memory value)
        internal
        pure
        returns (string memory)
    {
        bytes memory valueBytes = bytes(value);
        if (valueBytes[0] != "0" || valueBytes[1] != "x") return value;
        uint256 valueBytesLength = valueBytes.length;

        bytes memory padBytes = new bytes(valueBytesLength - 2);
        for (uint256 i = 0; i < valueBytesLength - 2; i++) {
            padBytes[i] = valueBytes[i + 2];
        }

        return string(padBytes);
    }

    function _addHexPrefix(string memory value)
        internal
        pure
        returns (string memory)
    {
        ...
        padBytes[0] = "0";
        padBytes[1] = "x";
        for (uint256 i = 2; i < valueBytesLength + 2; i++) {
            padBytes[i] = valueBytes[i - 2];
        }
    ...
    function _padStart(uint256 length, string memory value)
        internal
        pure
        returns (string memory)
    {
        uint256 valueBytesLength = bytes(value).length;
        if (valueBytesLength >= length) return value;

        uint256 padlength = length - valueBytesLength;

        bytes memory padBytes = new bytes(length);
        bytes memory valueBytes = bytes(value);
        for (uint256 i = 0; i < length; i++) {
            if (i < padlength) {
                padBytes[i] = "0";
            } else {
                padBytes[i] = valueBytes[i - padlength];
            }
        }
        return string(padBytes);
    }
    ...
    function _getSlice(
        ...
        uint256 targetBytesLength = end - begin;
        bytes memory slicedBytes = new bytes(targetBytesLength);
        bytes memory valueBytes = bytes(value);
        for (uint256 i = 0; i < targetBytesLength; i++) {
            slicedBytes[i] = valueBytes[i + begin];
        }
