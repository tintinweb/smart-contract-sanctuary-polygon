    ...
    EIP712Upgradeable
{
    ...
    function initialize() public initializer {
        __Ownable_init();
        __Pausable_init();
        __EIP712_init("Beta704CapeCoral", "1");

        _name = "Beta 704 Cape Coral";
        _symbol = "704CapeCoral";
    ...
    function delegate(address delegatee) public virtual override {
        require(balanceOf(delegatee) > 0, "Not a DAO member");
    ...
    function mint(address to, uint256 amount) external virtual onlyOwner {
        require(_totalSupply + amount <= 1000, "Can not mint above 1000");
        _mint(to, amount);
    }

    function mintBatch(address[] memory account, uint256[] memory amount)
        external
        virtual
        onlyOwner
    {
        require(account.length == amount.length, "length mismatch");

        for (uint256 i = 0; i < account.length; i++) {
            require(
                totalSupply() + amount[i] <= 1000,
                "Can not mint above 1000"
            );
    ...
    function burnBatch(address[] memory account, uint256[] memory amount)
        external
        virtual
        onlyOwner
    {
    ...
    function setNewToken(address _newToken) external virtual onlyOwner {
        require(_newToken != address(0), "Zero address");
        newToken = IERC20Upgradeable(_newToken);
    }

    function claimNewToken() external virtual {
        require(address(newToken) != address(0), "New token not set");

        uint256 claimableAmount = balanceOf(msg.sender);

        _burn(msg.sender, claimableAmount);
        newToken.safeTransfer(msg.sender, claimableAmount);

        emit Claim(msg.sender, claimableAmount);
    }

    function airdropNewToken(
        address[] calldata account,
        uint256[] calldata amount
    ) external virtual onlyOwner {
        for (uint256 i = 0; i < account.length; i++) {
            _burn(account[i], amount[i]);
            newToken.safeTransfer(account[i], amount[i]);

            emit Airdrop(account[i], amount[i]);
        }
    }
