...
contract NFT is ERC721URIStorage, Ownable {
    ...
    {
        require(
            ownerOf(tokenId) != deadAddress,
            "ERC721: query token URI of a nonexistent token"
        );
        if (revealOnSale && ownerOf(tokenId) == owner()) {
            return "";
        }
    ...
    function setRevealOnSale(bool _revealOnSale) public onlyAuthorized {
        revealOnSale = _revealOnSale;
    }
}
