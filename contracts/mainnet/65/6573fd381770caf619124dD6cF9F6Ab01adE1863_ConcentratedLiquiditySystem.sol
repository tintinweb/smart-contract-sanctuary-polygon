...
contract ConcentratedLiquiditySystem is BaseContract {
  ...
  function addLiquidityUSDTAndDAI(uint _usdAmount, uint _lpTokenBurnPercentage) external onlyContractCall {
    require(_lpTokenBurnPercentage <= 100 * DECIMAL3, "ConcentratedLiquiditySystem: burn percentage invalid");
    require(usdtToken.balanceOf(address(this)) >= _usdAmount, "ConcentratedLiquiditySystem: contract insufficient usdt balance");
    require(daiToken.balanceOf(address(this)) >= _usdAmount, "ConcentratedLiquiditySystem: contract insufficient dai balance");
    uint lpBalanceBefore = lpToken.balanceOf(address(this));
    uniswapV2Router.addLiquidity(
      address(daiToken),
      address(usdtToken),
  ...
  function addLiquidityUSDTAndUSDC(uint _usdAmount, uint _lpTokenBurnPercentage) external onlyContractCall {
    require(_lpTokenBurnPercentage <= 100 * DECIMAL3, "ConcentratedLiquiditySystem: burn percentage invalid");
    require(usdtToken.balanceOf(address(this)) >= _usdAmount, "ConcentratedLiquiditySystem: contract insufficient usdt balance");
    require(usdcToken.balanceOf(address(this)) >= _usdAmount, "ConcentratedLiquiditySystem: contract insufficient dai balance");
    uint lpBalanceBefore = lpToken.balanceOf(address(this));
    uniswapV2Router.addLiquidity(
      address(usdcToken),
      address(usdtToken),
  ...
  function addLiquidityUSDTAndMSTToken(uint _usdAmount, uint _lpTokenBurnPercentage) external onlyContractCall {
    require(_lpTokenBurnPercentage <= 100 * DECIMAL3, "ConcentratedLiquiditySystem: burn percentage invalid");
    uint lpBalanceBefore = lpToken.balanceOf(address(this));
    uniswapV2Router.addLiquidity(
      0x763bed355ec82450b5DC1503709eF73be0573850,
      address(usdtToken),
      _usdAmount,
      _usdAmount,
      0,
  ...
  function convertUsdcOrDaiToUsdt(uint _usdAmount, bool isUsdcToken) external onlyMn {
    ...
    require(usdToken.balanceOf(address(this)) >= _usdAmount, "ConcentratedLiquiditySystem: contract insufficient balance");
    address[] memory path = new address[](2);
  ...
  function _initDependentContracts() override internal {
    ...
    IBEP20 MSTToken = IBEP20(0x763bed355ec82450b5DC1503709eF73be0573850);
    MSTToken.approve(address(uniswapV2Router), type(uint).max);
  }
}
