...
contract RunbitWallet is AccessControl {
    ...
    function updateTokens(uint256 newIndex) external onlyRole(OPERATE_ROLE) {
        ...
        for (uint i = currentIndex; i < newIndex; ++i) {
            ...
            if (tokenId !=0 && _gets(tokenId) == 0) {
                ...
                _set(tokenId, dayEarn * (meta.durability / 100) / 10**14);
                sumIncrease += dayEarn;
    ...
    // for tokenburn
    function amendIndex(uint256 newIndex) external onlyRole(OPERATE_ROLE) {
        currentIndex = newIndex;
    }
    ...
    function mint(address to, uint256 amount) external onlyRole(OPERATE_ROLE) {
        require(minted <= maxSupply, "Exceed max mint");
    ...
    function addShoesCollection(IRunbitCollection.ShoesCollection memory colection) external onlyRole(OPERATE_ROLE) {
        uint256 earn = uint256(colection.meta.specialty + colection.meta.comfort + colection.meta.aesthetic)
                       * (colection.meta.durability / 100) * rate;
        uint256 priceEarn = colection.price1 / earn;
        require(priceEarn >= minPrice, "price too small!");
