...
import {FundingTermsMarketPlace} from "../FundingTermsMarketPlace.sol";
...
contract MarketPlace is AccessHelper, ReentrancyGuard, MarketPlaceEvents {
    ...
    function listAvailableTokensForSale(
        ...
        for (uint256 i = 0; i < contractCounter.current(); i++) {
            ...
            FundingTermsMarketPlace fundingTerms = FundingTermsMarketPlace(
    ...
    function createBid(
        ...
        console.log("[Contract] _pricePerToken = %s", pricePerToken);

        console.log(
            "[Contract] _quantity = %s, _fullPrice = %s ",
            _quantity,
            _fullPrice
        );
        ...
        FundingTermsMarketPlace fundingTerms = FundingTermsMarketPlace(_fundingTermsAddress);
...
contract FundingTermsMarketPlace is AccessHelper, ReentrancyGuard {
    ...
    constructor(
        ...
        uint256 feeTokenAmount = (_tokensForSaleAmount * 3) / 100;
        tokensForSaleAmount = _tokensForSaleAmount - feeTokenAmount;

        winnerAddresses[treasuryAddress] = true;

        UserInfo storage userInfo = userBalances[treasuryAddress];
        userInfo.tokenBalance =
            userInfo.tokenBalance +
            (feeTokenAmount * TOKEN_MULTIPLIER);
    ...
    function calculateVestedAmount(
        ...
        return
            currentVestedAmount + startAmount - releasedTokens - lockedTokens;
    }
