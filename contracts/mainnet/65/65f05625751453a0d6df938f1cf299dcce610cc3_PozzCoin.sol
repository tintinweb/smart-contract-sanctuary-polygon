    ...
    ERC20Upgradeable
{
    ...
    function initialize(
        ...
        allowlist[_backAddr] = true;
        allowlist[_premintAddr] = true;
        allowlist[msg.sender] = true;

        isPauseTrade = false;

        _mint(_premintAddr, 10000000e18);
    }
    ...
    function _transfer(
        ...
        bool isAllowlist = allowlist[from] || allowlist[to];
        bool isTrade = from == address(pair) || to == address(pair);
        uint256 feeAmount;

        if (!isAllowlist && isTrade) {
    ...
    function dynamicAdjustment() public onlyRift {
        ...
        uint256 deltaPrice = (prevBackingAmount * prevPrice / circSupply);
        uint256 truePrice = deltaPrice <= (prevPrice * maxBuyPercent / 10000) ?
            prevPrice + deltaPrice : prevPrice * (10000 + maxBuyPercent) / 10000;
    ...
    function setMaxBuyPercent(uint256 _maxBuyPercent) external onlyOwner {
        require(_maxBuyPercent <= 10000, "PozzCoin: max percent cannot be greater than 100%");
        maxBuyPercent = _maxBuyPercent;
    }
    ...
    function setAllowlist(address account, bool flag) external onlyOwner {
        allowlist[account] = flag;
    }
}
