...
contract PublicPokerClub is ERC721Linkable {
    ...
    // need to maintain the same interface as the original contract
    function mint(uint256) public notPaused {
        USDC.transferFrom(msg.sender, EddieRivers, price);

        supply++;
        participants.push(supply);
        _safeMint(msg.sender, supply);

        emit NewParticipant(msg.sender, supply);
    }
