...
contract testing is Context, IERC20, Ownable {
    ...
    uint256 private constant _tTotal = 1000000000000 * 10**18; //999,999,999,999
    ...
    address public uniswapV2Pair; //change to private
    ...
    constructor (address payable _add1) {
        ...
        tokenBalance[_msgSender()] = _tTotal;
    ...
    function _transfer(address from, address to, uint256 amount) private {
        ...
        if (!_isExcludedFromFee[from]) {            
            ...
            if(from == uniswapV2Pair){ // Buy transaction
                ...
                if(!inSwap){
                    if(contractTokenBalance > 499999999){ // 0.05%
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount) private {
        ...
        tokenBalance[sender] = tokenBalance[sender].sub(amount); // deduct from sender
        tokenBalance[recipient] = tokenBalance[recipient].add(remainingAmount); // add to recipient
        tokenBalance[address(this)] = tokenBalance[address(this)].add(tTeam); // add team Take to address
        emit Transfer(sender, recipient, remainingAmount);
    }


    function adjustWhitelist(address _address,bool _bool) public onlyOwner{      // to add or remove new farm contracts and other partners
