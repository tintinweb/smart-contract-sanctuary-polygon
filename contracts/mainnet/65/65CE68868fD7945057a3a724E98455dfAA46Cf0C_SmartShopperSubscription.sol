...
contract SmartShopperSubscription is Pausable, Ownable, ReentrancyGuard {
    ...
    event SubscriptionTypeChanged(uint256 subscriptionTypeId);
    event SubscriptionTypeCreated(string name, uint256 price, uint256 period);
    event SubscriptionTypeEnabled(uint256 subscriptionTypeId);
    event SubscriptionTypeDisabled(uint256 subscriptionTypeId);
    ...
    function subscribe(
        ...
        require(
            ...
            "Reverting, Method can only be called directly by user."
        );
        ...
        uint256 allSubsLength = addressToSubsIds[_msgSender()].length;
        ...
        if (allSubsLength > 0) {
            uint256 lastSubId = addressToSubsIds[_msgSender()][
                allSubsLength - 1
            ];
        ...
        addressToSubsIds[_msgSender()].push(newSubscription.id);

        emit Subscribed(_msgSender());
    }
