...
contract StakingRewards {
    ...
    uint256 public minimumClaim = 500;
    ...
    function stake(uint256 _amount) external updateReward(msg.sender) {
        require(_amount > 0, "Cant Stake 0 Tokens");
    ...
    function setRewardRate(uint256 _amount) public {
        require(msg.sender == admin, "Access Deny!");
        rewardRate = _amount;
    }

    function setMinimumClaim(uint256 _amount) public {
        require(msg.sender == admin, "Access Deny!");
        minimumClaim = _amount;
    }

    function withdraw(uint256 _amount) external updateReward(msg.sender) {
        require(_amount > 0, "Cant Withdraw 0 Tokens");
    ...
    function getReward() external updateReward(msg.sender) {
        ...
        uint256 dexBalance = rewardsToken.balanceOf(address(this));
        require(
            reward >= minimumClaim * 10**18,
            "Minimum 500 VRTX Tokens needed to claim"
        );
