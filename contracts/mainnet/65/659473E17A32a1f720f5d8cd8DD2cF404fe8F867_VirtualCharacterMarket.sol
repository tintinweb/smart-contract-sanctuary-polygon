...
contract VirtualCharacterMarket is Whitelist {
    ...
    constructor(
        ...
        treasury = _treasury;
        price = _price;
        priceForWL = _priceForWL;

        active = true;
    }

    function buy(uint256 _items) external isActive {
        _buy(msg.sender, _items);
    }
    ...
    function setPriceForWL(uint256 _price)
        external
    ...
    function setTreasury(address _treasury)
        ...
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        require(_treasury != address(0), "Black hole set?");
    ...
    function calculatePrice(address _account, uint256 _items)
        public
        view
        returns (uint256)
    {
        return (isMember(_account)) ? priceForWL * _items : price * _items;
    }

    function _buy(address _account, uint256 _items) internal {
        require(_items != 0, "Buy nothing?");

        uint256 amount = calculatePrice(_account, _items);

        require(amount != 0, "Missing price");

        IERC20(token).safeTransferFrom(msg.sender, treasury, amount);
        orderId++;
        emit Purchased(orderId, msg.sender, _items, amount);
    }
...
contract Whitelist is ExceptionalList {
...
library ArrayUtils {
     ...
     */
    function hasDuplicate(uint256[] calldata _collection)
        internal
        pure
        returns (bool)
    {
...
abstract contract ExceptionalList is RBAC {
     ...
     */
    function addMembers(address[] calldata _accounts)
        external
        onlyRole(EL_MANAGER_ROLE)
    {
     ...
     */
    function removeMembers(address[] calldata _accounts)
        external
        onlyRole(EL_MANAGER_ROLE)
    {
        uint256 l = _accounts.length;
        uint256 i;
        for (i; i < l; i++) {
            require(isMember(_accounts[i]), "Whitelist: Address is no member");
