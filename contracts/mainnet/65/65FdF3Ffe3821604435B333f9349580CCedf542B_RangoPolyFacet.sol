...
contract RangoPolyFacet is IRango, ReentrancyGuard, IRangoPoly {
    ...
    /// @dev The poly bridge part is handled in the RangoPoly.sol contract
    ...
    function polyBridge(
        ...
        doPolyBridge(bridgeRequest, request.toToken, out);
    }
    ...
    function polyBridge(
        ...
        doPolyBridge(bridgeRequest, fromToken, amount);
    }
    ...
    function doPolyBridge(
        ...
        if (token == LibSwapper.ETH)
            require(amount <= msg.value, "Insufficient ETH sent to RangoPoly");

        if (token == LibSwapper.ETH) {
            IPolyBridge(s.polyWrapperAddress).lock{ value: amount }(
                token,
                dstChainId,
                receiver,
                amount,
            ...
            IPolyBridge(s.polyWrapperAddress).lock{ value: msg.value }(
                token,
                dstChainId,
                receiver,
                amount,
...
library LibSwapper {
    ...
    function callSwapsAndFees(SwapRequest memory request, Call[] calldata calls) private returns (bytes[] memory) {
        ...
        // this is only used for calculating the fees.
        ...
        if (request.affiliateIn > 0) {
            ...
            require(request.affiliateIn <= maxAffiliate, 'Requested affiliate fee exceeded max threshold');
