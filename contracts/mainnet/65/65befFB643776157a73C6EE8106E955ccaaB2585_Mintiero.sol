...
contract Mintiero is ERC1155, Ownable {
    string public name;
    ...
    function setAvailableQuantity(uint256 tokenId, uint256 quantity) public onlyOwner {
    ...
    function mint(address _userWallet, uint256 _quantity, uint256 _tokenId) external payable {
        ...
        tokenInstance.transferFrom(msg.sender, address(this), price(_tokenId));

        availableQuantities[_tokenId] -= _quantity;
    ...
    function availableTokens(address, uint256 _tokenId) public view returns (uint256 quantity) {
        if (availableQuantities[_tokenId] < MAX_TOKENS_PER_TXN) {
            return availableQuantities[_tokenId];
        }
        return MAX_TOKENS_PER_TXN;
    }

    function price(uint256 _tokenId) public view returns (uint256) {
        return tokenPrices[_tokenId] / 10 ** 11;
    }
}