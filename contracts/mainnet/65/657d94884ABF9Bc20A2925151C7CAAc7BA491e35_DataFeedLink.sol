...
contract DataFeedLink is ChainlinkClient {
    ...
    // from previous
    mapping(address => string) public game;
    mapping(address => string) public win;
    mapping(address => bool) public qualifiedUsers;
    address[] public qualifiedAddresses;
    

    event NewGame(address sender, string game);
    event NewWin(address sender, string win);
    event QualifiedUser(address user);
    // from previous
    ...
    constructor() {
        ...
        setChainlinkOracle(0xA98FEd1e6E59fB7Cf6b35fD240Cd72EBf2DA7d66);
    }
    ...
    /* ========== ADD VALUE ========== */
        function addString(string memory _game, string memory _win) public payable {
        require(msg.value > 0, "You must send some ether to add a message.");
        game[msg.sender] = _game;
        win[msg.sender] = _win;
        bytes32 hashedWin = keccak256(abi.encodePacked(_win));
        if (hashedWin == keccak256(abi.encodePacked("win_lakers"))) {
            qualifiedUsers[msg.sender] = true;
            emit QualifiedUser(msg.sender);
            qualifiedAddresses.push(msg.sender);
        }
        emit NewGame(msg.sender, _game);
        emit NewWin(msg.sender, _win);
    }
