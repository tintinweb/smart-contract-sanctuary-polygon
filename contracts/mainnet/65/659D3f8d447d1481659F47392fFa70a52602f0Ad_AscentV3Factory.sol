...
/// @title Canonical AscentExchange V3 factory
/// @notice Deploys AscentExchange V3 pools and manages ownership and control over pool protocol fees
contract AscentV3Factory is IAscentV3Factory {
    /// @inheritdoc IAscentV3Factory
    ...
    function createPool(
        ...
        pool = IAscentV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IAscentV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IAscentV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IAscentV3Pool(pool).setLmPool(lmPool);
    }
}
