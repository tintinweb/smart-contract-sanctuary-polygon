...
contract XenBoxUpgradeable is ERC721Upgradeable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function _batchRankAndReward(uint256 start, uint256 end, uint256 term) internal {
        ...
        unchecked {
            for (uint256 i = start; i < end; ++i) {
                IXenBoxImpl(proxyAddress(i, _codehash)).rankAndReward(term);
            }
        }
    }
    ...
    function proxyAddress(uint256 index, bytes32 _codehash) public view returns (address) {
        return address(uint160(uint256(keccak256(abi.encodePacked(bytes1(0xff), address(this), index, _codehash)))));
    }

    function userMints(uint256 tokenId) public view returns (IXen.MintInfo memory) {
        return IXen(xenAddress).userMints(proxyAddress(tokenMap[tokenId].start, codehash));
    }
    ...
    function changeNotCheckRefer() external onlyOwner {
        notCheckRefer = !notCheckRefer;
    }
}