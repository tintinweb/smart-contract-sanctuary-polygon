...
contract GainsStrategy is InitializableAbstractStrategy, BalancerExchange   {
    ...
    address public balancerVault; // [IMP] Balancer Vault is not being used currently, but Curve is.
    ...
    function setSwappingPool(address _pool) external onlyGovernor {
        ...
        returns (uint256)
    {
        ...
        if (daiAmount >  minThresholds[1]) {
            return ICurvePool(balancerVault).get_dy_underlying(
                _getCoinIndex(address(token0)),
                _getCoinIndex(address(primaryStable)),
                daiAmount
            );
        }
        return 0;
    ...
    function _swapAssetToPrimaryStable() internal {
        if ( (address(token0) != address(primaryStable)) && (token0.balanceOf(address(this)) > 0) )  {
            uint256 _min_dy = _oracleToken0ToPrimaryStable(token0.balanceOf(address(this)));
            token0.approve(address(balancerVault), token0.balanceOf(address(this)));
            ICurvePool(balancerVault).exchange_underlying(
                _getCoinIndex(address(token0)),
                _getCoinIndex(address(primaryStable)),
    ...
    function _swapPrimaryStableToToken0() internal {
        ...
        if (address(primaryStable) != address(token0)) {
            uint256 _min_dy = _oraclePrimaryStableToToken0(primaryStable.balanceOf(address(this)));
            primaryStable.approve(address(balancerVault), primaryStable.balanceOf(address(this)));
            ICurvePool(balancerVault).exchange_underlying(
                _getCoinIndex(address(primaryStable)),
                _getCoinIndex(address(token0)),
                primaryStable.balanceOf(address(this)),
                _min_dy.mulTruncate(
                    uint256(1e18) - MAX_SLIPPAGE
                )
            );
    ...
    function _getCoinIndex(address _asset) internal view returns (uint256) {
        for (uint256 i = 0; i < 5; i++) {
            if (ICurvePool(balancerVault).underlying_coins(i) == _asset) {
                return i;
            }
        }
