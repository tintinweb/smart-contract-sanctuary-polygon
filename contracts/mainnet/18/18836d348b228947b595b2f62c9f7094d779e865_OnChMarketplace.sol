...
import "./IHerityNFT.sol";
import "./IHerityNFTExtended.sol";
...
contract OnChMarketplace is Initializable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    address public holderTokenAddress;
    uint256 public holderDiscount; // Discount percentage 0.5
    uint256 public holderMinimum; // Discount from


    function initialize(address eternalStorageAddress, address royaltyOrchestratorAddress) public payable initializer {
    ...
    function initializeV4(address _holderTokenAddress, uint256 _holderDiscount, uint256 _holderMinimum) public reinitializer(4) {
        holderTokenAddress = _holderTokenAddress;
        holderDiscount = _holderDiscount;
        holderMinimum = _holderMinimum;
    }
    ...
    struct SellOrder {
        ...
        address nftHighestBidderAddress;
        uint listingTime;
        uint expirationTime;
        uint amount;
    ...
    */
    function createSellOrder(
        ...
        uint sellOrderId,
        uint collectionId
    ) public nonReentrant whenNotPaused isMarketplaceApproved(contractAddress) returns (bool) {
        SellOrder memory existingMarketItem = _retrieveOrder(contractAddress, tokenId, msg.sender, sellOrderId);

        if (tokenId == 0) {
            // Logic for minting item on the spot
            if (nftKind == SaleKindInterface.NFTKind.ERC721) {
                tokenId = IHerityNFT(contractAddress).mintFor(msg.sender, collectionId);
            }

            if (nftKind == SaleKindInterface.NFTKind.ERC1155) {
                tokenId = IHerityNFTExtended(contractAddress).mintFor(msg.sender, amount, collectionId);
            }
        }
        ...
        if (saleKind == SaleKindInterface.SaleKind.EnglishAuction) {
            require(_isAuctionOngoing(expirationTime), "OnCh: Expiration time can either be 0 or in the future");
            require(paymentToken != address(0), 'OnCh: Auctions can only be started with ERC20 payments');
        }
    ...
    function cancelSellOrder(
        ...
        uint256 tokenId,
        uint sellOrderId
    ) notZeroAddress(contractAddress) nonReentrant whenNotPaused payable public returns (bool) {
        SellOrder memory marketItem = _retrieveOrder(contractAddress, tokenId, msg.sender, sellOrderId);
        bytes32 orderHash = _computeOrderHashId(contractAddress, tokenId, msg.sender, sellOrderId);
        ...
        _updateOrderStatus(contractAddress, tokenId, msg.sender, sellOrderId, SaleKindInterface.SellOrderStatus.Cancelled, address(0));
    ...
    function createBuyOrder(address contractAddress,
        ...
        uint256 amount,
        address paymentToken,
        uint sellOrderId) notOwnItem(sellerAddress) notZeroAddress(contractAddress) nonReentrant whenNotPaused payable public returns (bool) {
        SellOrder memory sellOrder = _retrieveOrder(contractAddress, tokenId, sellerAddress, sellOrderId);
        ...
        if (sellOrder.saleKind == SaleKindInterface.SaleKind.EnglishAuction) {
            require(price >= sellOrder.price, 'OnCh: The offer must be at least equal to the starting price');
            ...
            _updateOrderHighestBidder(sellOrder.contractAddress, sellOrder.tokenId, sellOrder.seller, msg.sender, price, sellOrderId);
        }
        ...
        if (sellOrder.saleKind == SaleKindInterface.SaleKind.FixedPrice) {
            ...
            _completeSellOrder(sellOrder, msg.sender, price, amount, sellOrderId);
        }
    ...
    function acceptHighestBid(address contractAddress, uint256 tokenId, uint sellOrderId) nonReentrant public returns (bool)  {
        SellOrder memory sellOrder = _retrieveOrder(contractAddress, tokenId, msg.sender, sellOrderId);

        require(sellOrder.contractAddress != address(0), 'OnCh: Market item not found');
        require(sellOrder.saleKind != SaleKindInterface.SaleKind.FixedPrice, 'OnCh: Can only accept on auctions');
        require(sellOrder.nftHighestBidderAddress != address(0), 'OnCh: No bids were placed');
        require(sellOrder.status != SaleKindInterface.SellOrderStatus.Completed, 'OnCh: Auction already ended');


        require(IERC20(sellOrder.paymentToken).allowance(sellOrder.nftHighestBidderAddress, address(this)) > sellOrder.nftHighestBid, 'OnCh: Marketplace is not authorized to spend the amount the bidders behalf');
        require(IERC20(sellOrder.paymentToken).balanceOf(sellOrder.nftHighestBidderAddress) > sellOrder.nftHighestBid, 'OnCh: Bidder doesnt have enough tokens');

        _completeSellOrder(sellOrder, sellOrder.nftHighestBidderAddress, sellOrder.nftHighestBid, 1, sellOrderId);

        return true;
    }

    function updateSellOrderDetails(address contractAddress, uint256 tokenId, uint expirationTime, uint256 price, uint sellOrderId) nonReentrant whenNotPaused public returns (bool)   {
        SellOrder memory sellOrder = _retrieveOrder(contractAddress, tokenId, msg.sender, sellOrderId);
        ...
        if (sellOrder.saleKind == SaleKindInterface.SaleKind.FixedPrice) {
            require(expirationTime == 0, 'OnCh: Expiration time can only be changed on auctions');
        }

        _updateOrderDetails(contractAddress, tokenId, msg.sender, expirationTime, price, sellOrderId);

        emit SellOrderUpdated(contractAddress, msg.sender, tokenId, expirationTime, price, sellOrderId);
        return true;
    }


    function _completeSellOrder(SellOrder memory sellOrder, address buyer, uint256 price, uint256 amount, uint sellOrderId) internal {
        bytes32 orderHash = _computeOrderHashId(sellOrder.contractAddress, sellOrder.tokenId, sellOrder.seller, sellOrderId);

        _updateOrderStatus(sellOrder.contractAddress, sellOrder.tokenId, sellOrder.seller, sellOrderId, SaleKindInterface.SellOrderStatus.Completed, buyer);
    ...
    function _paySellerAndTakePlatformFees(SellOrder memory sellOrder, address buyer, uint256 price) internal {
        (address itemRoyaltyAddress, uint itemRoyaltyPercent, address platformFeeAddress, uint platformFeePercent) = _getMarketplaceFeeInformation(sellOrder.contractAddress, sellOrder.tokenId);

        uint256 platformFeeAmount = platformFeeAddress != address(0) ? price.mul(platformFeePercent).div(1000) : 0;
        uint256 creatorRoyaltyAmount = itemRoyaltyAddress != address(0) ? price.mul(itemRoyaltyPercent).div(1000) : 0;
        uint256 sellerAmount = price - platformFeeAmount - creatorRoyaltyAmount;
        uint256 buyerAmount = 0;

        if (platformFeeAmount > 0 && holderTokenAddress != address(0) && IERC20(holderTokenAddress).balanceOf(msg.sender) > holderMinimum) {
            platformFeeAmount = platformFeeAmount - price.mul(holderDiscount).div(1000);
            buyerAmount = price.mul(holderDiscount).div(1000);
        }
        ...
        if (paymentToken == address(0)) {
            ...
            if (buyerAmount > 0) {
                payable(msg.sender).transfer(buyerAmount);
            }
            if (creatorRoyaltyAmount > 0) {
                payable(itemRoyaltyAddress).transfer(creatorRoyaltyAmount);
            }
        } else {
            IERC20(paymentToken).transferFrom(buyer, sellOrder.seller, sellerAmount);
            if (creatorRoyaltyAmount > 0) {
                IERC20(paymentToken).transferFrom(buyer, itemRoyaltyAddress, creatorRoyaltyAmount);
            }
            // We don't need to do anything if buyer gets a discount
            if (platformFeeAmount > 0) {
                IERC20(paymentToken).transferFrom(buyer, platformFeeAddress, platformFeeAmount);
            }
        }

        emit RoyaltiesPayout(
            sellOrder.contractAddress,
            sellOrder.tokenId,
    ...
    function _getMarketplaceFeeInformation(address contractAddress, uint256 tokenId) public view returns (address, uint256, address, uint256) {
        return RoyaltyOrchestrator.getItemFees(contractAddress, tokenId);
    }
    ...
    function _updateOrderHighestBidder(address contractAddress, uint256 tokenId, address seller, address bidderAddress, uint256 price, uint sellOrderId) internal {
        bytes32 orderHash = _computeOrderHashId(contractAddress, tokenId, seller, sellOrderId);
    ...
    function _updateOrderStatus(address contractAddress, uint256 tokenId, address seller, uint256 sellOrderId, SaleKindInterface.SellOrderStatus status, address buyer) internal {
        bytes32 orderHash = _computeOrderHashId(contractAddress, tokenId, seller, sellOrderId);
    ...
    function _updateOrderDetails(address contractAddress, uint256 tokenId, address seller, uint expirationTime, uint256 price, uint sellOrderId) internal {
        bytes32 orderHash = _computeOrderHashId(contractAddress, tokenId, seller, sellOrderId);

        EternalStorage.setUint(keccak256(abi.encodePacked(orderHash, 'expirationTime')), expirationTime);
        EternalStorage.setUint(keccak256(abi.encodePacked(orderHash, 'price')), price);
    }
    ...
    */
    function _storeOrder(SellOrder memory marketItem) internal returns (bytes32) {
        bytes32 orderHash = _computeOrderHashId(marketItem.contractAddress, marketItem.tokenId, marketItem.seller, marketItem.sellOrderId);
        ...
        EternalStorage.setAddress(keccak256(abi.encodePacked(orderHash, 'nftHighestBidderAddress')), marketItem.nftHighestBidderAddress);
        ...
        EternalStorage.setUint(keccak256(abi.encodePacked(orderHash, 'sellOrderId')), marketItem.sellOrderId);
    ...
    */
    function _retrieveOrder(address contractAddress, uint256 tokenId, address seller, uint sellOrderId) public view returns (SellOrder memory) {
        bytes32 orderHash = _computeOrderHashId(contractAddress, tokenId, seller, sellOrderId);
        ...
        return SellOrder(
            ...
            uint256(EternalStorage.getUint(keccak256(abi.encodePacked(orderHash, 'nftHighestBid')))),
            EternalStorage.getAddress(keccak256(abi.encodePacked(orderHash, 'nftHighestBidderAddress'))),
            ...
            EternalStorage.getUint(keccak256(abi.encodePacked(orderHash, 'sellOrderId')))
        );
    }

    function _computeOrderHashId(address contractAddress, uint256 tokenId, address seller, uint sellOrderId) public pure returns (bytes32) {
        return keccak256(abi.encodePacked(contractAddress, tokenId, seller, sellOrderId));
    }
}
...
interface IHerityNFT {
  ...
  function getTokenCollectionId ( uint256 tokenId ) external view returns ( uint256 );
  function isApprovedForAll ( address owner, address operator ) external view returns ( bool );
  function isWhitelisted ( address addressToCheck ) external view returns ( bool );
  function mint (  ) external returns ( uint256 );
  function mintFor ( address ownerAddress, uint256 collectionId ) external returns ( uint256 );
  ...
  function tokenIdToCollectionIdMapper ( uint256 ) external view returns ( uint256 );
...
interface IHerityNFTExtended {
  ...
  function mintFor ( address ownerAddress, uint256 amount, uint256 collectionId ) external returns ( uint256 );
  function name (  ) external view returns ( string memory );
  function owner (  ) external view returns ( address );
  function removeFromWhitelist ( address allowedAddress ) external;
  function removeFromWhitelistBulk ( address[] memory allowedAddresses ) external;
  ...
  function setApprovalForAll ( address operator, bool approved ) external;
  function setBaseURI ( string memory baseURI_ ) external returns ( bool );
  function setUnWithdrawableToken ( address token ) external;
  function supportsInterface ( bytes4 interfaceId ) external view returns ( bool );
  function symbol (  ) external view returns ( string memory );
  function tokenIdToCollectionIdMapper ( uint256 ) external view returns ( uint256 );
...
interface IOnChRoyaltyOrchestrator {
  function addInternalContract ( address nftContractAddress ) external returns ( bool );
  function addToWhitelist ( address allowedAddress ) external;
  function addToWhitelistBulk ( address[] memory allowedAddresses ) external;
  ...
  function collectionRoyaltyPercentage ( uint256 ) external view returns ( address creatorAddress, uint256 percentage, uint256 platformPercentage );
  function contractRoyaltyPercentage ( address ) external view returns ( address creatorAddress, uint256 percentage, uint256 platformPercentage );
  function creators ( uint256 ) external view returns ( address );
  function creatorsLength (  ) external view returns ( uint256 );
  function getItemFees ( address _contractAddress, uint256 _tokenId ) external view returns ( address, uint256, address, uint256 );
  function internalContracts ( address ) external view returns ( bool );
  function isWhitelisted ( address addressToCheck ) external view returns ( bool );
  ...
  function removeFromWhitelist ( address allowedAddress ) external;
  function removeFromWhitelistBulk ( address[] memory allowedAddresses ) external;
  function removeInternalContract ( address nftContractAddress ) external returns ( bool );
  function renounceOwnership (  ) external;
  function setItemFees ( address _creatorAddress, address _contractAddress, uint256 _tokenId, uint256 _collectionId, uint8 royaltyPercentage, uint256 platformRoyaltyPercentage ) external returns ( bool );
  function setUnWithdrawableToken ( address token ) external;
  function transferOwnership ( address newOwner ) external;
  function withdrawResidualErc20 ( address token, address to ) external;
}
