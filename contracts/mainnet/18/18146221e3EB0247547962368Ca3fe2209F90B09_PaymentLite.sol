...
contract PaymentLite is Ownable, Pausable {
    using SafeERC20 for IERC20;

    /// @notice ERC20 token contract used as currency for protocol payments.
    IERC20 public immutable paymentToken;

    /**
     * @notice Emitted when `payer` makes a new deposit of `amount`
     * protocol-currency tokens to topup its owned funds.
     */
     ...
     * @param _paymentToken Address of the ERC20 contract used as currency
     */
    constructor(address _ownerAddress, address _paymentToken) {
    ...
    /**
     * @notice Deny clients from making any new deposits.
     * Needed in case of contract migration.
     */
    function pausePayment() external onlyOwner {
        _pause();
    }

    /**
     * @notice Unpause contract and allow clients to make deposits again.
     * Used to recover from an accidental pausing.
     */
    function unpausePayment() external onlyOwner {
        _unpause();
    }

    /**
     * @notice Destroy contract and send all stored native coins to dest
     * @param dest Destination address for native tokens (should not be any, but just in case)
     */
    function destroyPayment(address payable dest) external onlyOwner {
        selfdestruct(dest);
    }
}
