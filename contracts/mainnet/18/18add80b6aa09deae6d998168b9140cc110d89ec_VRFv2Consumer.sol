...
event ConfirmedStats(uint _requestId);
...
function fulfillRandomWords(
  ...
  emit ConfirmedStats(requestNumber);
}


function queryStatsById(uint256 _requestId) external view returns (uint256 [] memory) 
  {
    return s_requestIndexToRandomWords[_requestId];
