...
contract Offer is Initializable, IOffer , ReentrancyGuardUpgradeable, OwnableUpgradeable, EIP712Upgradeable {
  ...
  function acceptOffer(
    IOffer.Offer calldata _offer,
    bytes calldata _signature) external nonReentrant escrowSet {
  ...
  function acceptCollectionOffer(
    IOffer.Offer calldata _offer,
    bytes calldata _signature) external nonReentrant escrowSet {
    address bidder = ECDSAUpgradeable.recover(_hashCollectionOffer(_offer), _signature);
  ...
  function acceptCounterOffer(
    ...
    address seller = ECDSAUpgradeable.recover(_hashCounterOffer(_counterOffer), _signature);

    require(seller == _counterOffer.seller, 'invalid signature');
    require(IERC721(_counterOffer.tokenAddress).ownerOf(_counterOffer.tokenId) == _counterOffer.seller, "seller not own nft");

    // transfer nft to bidder
    IERC721(_counterOffer.tokenAddress).transferFrom(_counterOffer.seller, msg.sender, _counterOffer.tokenId);

    _payout(_counterOffer.tokenAddress, _counterOffer.price, msg.sender, _counterOffer.seller);
  ...
  function _payout(address _tokenAddress, uint256 _price, address _bidder, address _owner) internal {
    ...
    if (address(royaltyRegistry) != address(0)) {
      ...
      if (collectionPayee != address(0) && collectionRoyalty > 0) {
        ...
        escrow.completePayment(_bidder, collectionPayee, collectionRevenue);
  ...
  function _hashCollectionOffer(IOffer.Offer calldata _offer) private view returns (bytes32) {
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256("CollectionOfferParams(address tokenAddress,uint256 price,address bidder)"),
            _offer.tokenAddress,
            _offer.price,
            _offer.bidder
          )
  ...
  function _hashCounterOffer(IOffer.CounterOffer calldata _counterOffer) private view returns (bytes32) {
    return
      _hashTypedDataV4(
        keccak256(
          abi.encode(
            keccak256("CounterOfferParams(address tokenAddress,uint256 tokenId,uint256 price,address seller)"),
            _counterOffer.tokenAddress,
            _counterOffer.tokenId,
            _counterOffer.price,
            _counterOffer.seller
