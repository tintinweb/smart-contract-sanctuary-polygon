...
contract PriceOracle {
    ...
    /**
        Works For MATIC, Regular Tokens And Surge Tokens
     */
    function priceOf(address token) public view returns (uint256) {
        return token == WETH ? priceOfMATIC() : priceOfToken(token);
    }
    ...
    function priceOfToken(address token) public view returns (uint256) {
        ...
        return( ( p1 * priceOfMATIC()) / p0);
    }

    function priceOfMATIC() public view returns (uint256) {
        address token = 0x2791Bca1f2de4661ED88A30C99A7a9449Aa84174;
        address LP = IUniswapV2Factory(router.factory()).getPair(token, router.WETH());
        uint256 amt0 = IERC20(token).balanceOf(LP); // 6 decimals
        uint256 amt1 = IERC20(router.WETH()).balanceOf(LP); // 18 decimals
        return ( amt0 * 10**12 / amt1);
    }
