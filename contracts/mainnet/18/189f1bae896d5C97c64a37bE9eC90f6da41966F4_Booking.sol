...
library Wei {
  function fromEthers(uint256 value) public pure returns (uint256) {
    uint256 weiExponential = 10 ** 18;
    return value * weiExponential;
  }

  function fromFeed(
    AggregatorV3Interface priceXUsdFeed,
    AggregatorV3Interface priceEurUsdFeed,
    uint256 value
  ) public view returns (int256) {
    uint8 _decimals = 8;
    int256 price = getDerivedPrice(priceXUsdFeed, priceEurUsdFeed, _decimals);
    uint256 precision = 10 ** (_decimals - 2);
    uint256 priceWithPrecision = value * precision;
    return int256(fromEthers(priceWithPrecision)) / price;
  }
  ...
  function getDerivedPrice(
    ...
    int256 basePrice = getAggregatorPrice(priceXUsdFeed, decimals);
    int256 quotePrice = getAggregatorPrice(priceEurUsdFeed, decimals);
  ...
  function scalePrice(
    ...
    if (priceDecimals < decimals) {
      int256 precision = int256(10 ** uint256(decimals - priceDecimals));
      return price * precision;
    } else if (priceDecimals > decimals) {
      int256 precision = int256(10 ** uint256(priceDecimals - decimals));
      return price / precision;
    } else {
      return price;
    }
