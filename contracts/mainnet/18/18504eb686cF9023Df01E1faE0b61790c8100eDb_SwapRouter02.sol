...
abstract contract V2DfynRouter is IDfynRouterV2, DfynImmutable {
    ...
    function swapCallBack(
        ...
        if (isExactInput) {
            if (vault.balanceOf(tokenIn, data.payer) < amountIn)
                vault.deposit{value: tokenIn == USE_NATIVE ? amountIn : 0}(
                    tokenIn,
                    data.payer,
                    msg.sender,
                    amountIn,
            ...
            else vault.transfer(tokenIn, data.payer, msg.sender, amountIn);
        } else {
            // either initiate the next swap or pay
            if (data.path.hasMultiplePools()) {
                data.path = data.path.skipToken();
                exactOutputInternal(amountIn, msg.sender, unWrapVault, data);
            } else {
    ...
    PeripheryImmutableStateV1
{
    ...
    ///  IPeripheryPaymentsExtended
    function wrapETHLegacy(uint256 value, bool v1) public {
