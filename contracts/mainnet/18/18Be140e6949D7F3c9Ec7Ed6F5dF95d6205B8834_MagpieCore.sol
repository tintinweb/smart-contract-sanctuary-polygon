...
contract MagpieCore is ReentrancyGuard, Ownable, Pausable, IMagpieCore {
  ...
  function swapOut(SwapOutArgs calldata args)
    ...
    returns (uint256[] memory amountOuts)
  {
    (IWormholeCore.VM memory vm, bool valid, ) = IWormholeCore(
    ...
    if (payload.useTokenBridge == true) {
      ...
      (, bool status) = IHyphenLiquidityPool(config.hyphenLiquidityPool)
      .checkHashStatus(
        payload.fromAssetId,
        payload.amountIn,
      ...
      require(status, 'Transaction Not Processed');
    }
    ...
    swapArgs.routes[0].amountIn -= payload.destGasTokenAmount;
