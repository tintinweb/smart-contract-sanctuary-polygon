...
contract singletonDepositManager{
	...
	function sweep(address token, bytes32 salt) external{
		...
		sweeper(addy).sweep(token, payable(msg.sender));
	}

	function calculateDepositAddress(address sender, bytes32 salt) external view returns (address){
		bytes32 realsalt = keccak256(abi.encode(sender, salt));
		return computeAddress(realsalt, codehash);
	}
}
contract singletonDepositAddress is sweeper{
	...
	function sweep(address token, address payable to) external override{
		require(msg.sender == owner, "You are not the owner.");
		if(token == 0x0000000000000000000000000000000000000000){
			selfdestruct(to); //Don't worry, we'd be regenerated next time.
		} else{
			safeTransfer(IERC20(token), to, IERC20(token).balanceOf(address(this)));
		}
