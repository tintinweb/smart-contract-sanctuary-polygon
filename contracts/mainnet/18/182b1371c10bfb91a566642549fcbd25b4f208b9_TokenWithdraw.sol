...
/**
 * @title WYZ20 interface (compatible with ERC20 interface)
...
contract TokenWithdraw is Ownable, ECVerify { 
   using SafeMath for uint256;
    ...
    event WithdrawULE(address  User, uint256 Amount, uint256 Timestamp);

    event WithdrawWYZ(address user, uint256 amount); 
    ...
    function withdraw(address _tokenAddress, uint256 _amount, uint256 _nonce, bytes memory signature) external {   
        ...
        usedHash[hash] = true;   
        IWYZ20(_tokenAddress).transfer(msg.sender, _amount);   
        emit WithdrawULE(msg.sender, _amount, block.timestamp);   
    }  
 
     /**  
     * @dev Change Signature Address  
     */  
    ...
    function withdraw(uint256 _amount) payable public onlyOwner{
        ...
        emit WithdrawWYZ(msg.sender, _amount);
    }
