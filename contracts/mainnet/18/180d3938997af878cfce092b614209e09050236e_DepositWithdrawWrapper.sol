...
import "../vaults/interfaces/IDepositFacet.sol";
import "../vaults/interfaces/IWithdrawFacet.sol";
...
contract DepositWithdrawWrapper {
    ...
    function deposit(
        ...
        (lpAmount, actualTokenAmounts) = IDepositFacet(vault).proportionalDeposit(depositTokenAmounts, minLpAmount);
    ...
    function withdraw(
        ...
        uint256[] memory withdrawedTokenAmounts = IWithdrawFacet(vault).proportionalWithdraw(
            lpAmount,
            new uint256[](1)
        );
...
interface ICommonFacet {
    struct Storage {
        IBaseOracle.SecurityParams[] vaultTokensSecurityParams;
        IBaseOracle.SecurityParams[] allTokensSecurityParams;
        LpToken lpToken;
        IOracle oracle;
        address[] tokens;
...
interface IDepositFacet {
    struct Storage {
        mapping(address => mapping(address => uint256)) oneSidedDeposits;
        mapping(address => EnumerableSet.AddressSet) oneSidedDepositors;
        uint256 auctionGap;
        uint256 duration;
        uint256 startCoefficientX96;
        uint256 endCoefficientX96;
        mapping(address => uint256) auctionStartTimestamp;
        mapping(address => bool) isAuctionStarted;
        EnumerableSet.AddressSet supportedDepositTokens;
        mapping(address => uint256) minDepositAmounts;
    }
    ...
    function oneSidedDeposit(address token, uint256 amount) external;

    function pushOneSidedDeposits(address token, IManualVaultFacet.Call[] calldata calls) external;
}
...
interface IWithdrawFacet {
    struct Storage {
        mapping(address => mapping(address => uint256)) oneSidedWithdrawals;
        mapping(address => EnumerableSet.AddressSet) oneSidedWithrawers;
        uint256 auctionGap;
        uint256 duration;
        uint256 startCoefficientX96;
        uint256 endCoefficientX96;
    ...
    function registerWithdrawal(uint256 lpAmount, address token) external;

    function closeWithdrawal(address token) external;

    function pushWithdrawals(address token, IManualVaultFacet.Call[] calldata calls) external;
}
...
interface IManualVaultFacet {
    ...
    function externalCall(
        IManualVaultFacet.Call[] calldata calls
    ) external returns (IManualVaultFacet.Response[] memory responses);
}
