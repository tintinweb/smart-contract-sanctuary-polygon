...
contract Donsy is Ownable, ERC721A, PaymentSplitter, DefaultOperatorFilterer {
    ...
    uint public MAX_PER_WALLET_PUBLIC = 15;
    uint public MAX_PER_WALLET_WL = 5;


    uint public wlSalePrice = 25 ether;
    uint public publicSalePrice = 28 ether;
    ...
    constructor(uint96 _royaltyFeesInBips, address[] memory _team, uint[] memory _teamShares, bytes32 _merkleRootWL, string memory _baseURI) ERC721A("Donsy", "DONSY")
   ...
   function whitelistMint(address _account, uint _quantity, bytes32[] calldata _proof) external payable callerIsUser {
    ...
    function batchGift(address[] memory _to, uint256[] memory _amounts)
        external
        onlyOwner
    {
        ...
        require(
            totalSupply() + totalAmountToMint <= MAX_SUPPLY,
            "Exceeds max Supply"
        );
