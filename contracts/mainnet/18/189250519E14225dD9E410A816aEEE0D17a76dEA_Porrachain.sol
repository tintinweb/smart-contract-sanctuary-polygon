...
contract Porrachain {
     ...
     */
    function removeBet(bytes32 hash) payable public {
        Match storage _match = matches[hash];
        if (!_match.exists) {
            revert BetContract__MatchNotFound();
        }
     ...
     * @param hash Match unique identifier
     */
    function removeBetWhenMatchSuspended(bytes32 hash) payable public {
        Match storage _match = matches[hash];
        if (!_match.exists) {
            revert BetContract__MatchNotFound();
        }
     ...
     * @param matchHash identifier for a match
     */
    function removeMatchValidator(bytes32 matchHash) public {
        Match storage _match = matches[matchHash];
        if (!_match.exists) {
            revert BetContract__MatchNotFound();
        }
     ...
     * @param matchHash identifier for a match
     */
    function removeValidatorWhenMatchSuspended(bytes32 matchHash) public {
        Match storage _match = matches[matchHash];
        if (!_match.exists) {
            revert BetContract__MatchNotFound();
        }
