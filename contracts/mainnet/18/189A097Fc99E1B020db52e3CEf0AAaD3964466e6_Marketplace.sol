...
contract Marketplace is BaseContract {
  ...
  function setNFTListing(
    ...
    bool[] calldata _isUniSwapV3,
    bool[] calldata _isR0DivR1
  ) external onlyMn {
    uint paymentLength = _paymentAddresses.length;
    require(
      _paymentAddresses.length == paymentLength &&
      _poolAddresses.length == paymentLength &&
      _isUniSwapV3.length == paymentLength &&
      _isR0DivR1.length == paymentLength,
