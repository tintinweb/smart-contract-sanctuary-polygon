...
/**
   @title AccessControlList contract
...
contract AccessControlList is AccessControlEnumerable {
    ...
    mapping(bytes32 => bool) public customRoles;
    ...
    /**
       @notice Add new access control role
       @dev  Caller must have DEFAULT_ADMIN_ROLE
    */
    function addNewRole(string calldata roleName)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        bytes32 roleId = keccak256(abi.encodePacked(roleName));
        require(!customRoles[roleId], "Role added");
        customRoles[roleId] = true;
    }

    /**
       @notice Add new operator to access role
       @dev  Caller must have DEFAULT_ADMIN_ROLE
    */
    function addToRole(string calldata roleName, address operator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        bytes32 roleId = keccak256(abi.encodePacked(roleName));
        require(customRoles[roleId], "Role is invalid");
        _setupRole(roleId, operator);
    }
    ...
    /**
       ...
       @dev  Caller must have DEFAULT_ADMIN_ROLE
    */
    function removeFromRole(string calldata roleName, address operator)
        external
        onlyRole(DEFAULT_ADMIN_ROLE)
    {
        bytes32 roleId = keccak256(abi.encodePacked(roleName));
        require(customRoles[roleId], "Role is invalid");
        _revokeRole(roleId, operator);
    }
}
