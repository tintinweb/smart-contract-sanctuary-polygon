 ...
 * @author @ThePlagueNFT
 * @notice Acorne E2E NFT. L2 Polygon based state sync for engagement commissions
 */
contract AcorneChild is
    FxBaseChildTunnel,
    IAcorneCommon,
    ERC2771Context,
    ...
    ReentrancyGuard
{
    ...
    bytes32 public constant COMMISSION_MANAGER =
        keccak256("COMMISSION_MANAGER");

    /// @dev Token id to commission info
    ...
    constructor(
        ...
        _setupRole(COMMISSION_MANAGER, _msgSender());
    }
    ...
    /**
     * @notice Adds a commission value for a particular token id
     * @param _tokenId The token id
     * @param _amount The amount of commission in wei
     */
    function addCommission(
        uint256 _tokenId,
        uint256 _amount
    ) external onlyRole(COMMISSION_MANAGER) {
        commissions[_tokenId].value += _amount;
        emit CommissionAdded(_tokenId, _amount);
    }

    /**
     * @notice Removes a commission value for a particular token id
     * @param _tokenId The token id
     * @param _amount The amount of commission in wei
     */
    function removeCommission(
        ...
        commissions[_tokenId].value -= _amount;
        emit CommissionRemoved(_tokenId, _amount);
    }
    ...
    /**
     * @notice Verify commission and sync back to ETH
     * @dev Generates withdrawal proof that needs to be processed on ETH side
     * @param _tokenId The token id
     */
    function verifyCommission(
        ...
        uint256 commissionValue = commissions[_tokenId].value;
        require(
            _amount <= commissionValue,
            "Verification amount is larger than commissions allowance"
        );
        // Update working commissions
        commissions[_tokenId].value -= _amount;
        commissions[_tokenId].lastClaimedAt = block.timestamp;
        commissions[_tokenId].lastClaimedAddress = _msgSender();
        // Send sync up to parent
        _sendMessageToRoot(
            abi.encode(COMMISSION_CLAIM, abi.encode(_tokenId, _amount))
        );
        emit CommissionVerified(_tokenId, _amount);
    }

    // ========================================
    // FX-Portal Integrations
    // ========================================
    ...
    /**
     * @dev Persists the tracking data
     * @param _address The user address
     * @param _tokenId The token id
     */
    function _mintSync(address _address, uint256 _tokenId) internal {
        // Add baseline working commission to token id
