...
abstract contract SignatureAction is EIP712, ISignatureAction {
    ...
    function _encodeRequest(GenericRequest calldata _req) internal pure returns (bytes memory) {
        return
            abi.encode(TYPEHASH, _req.validityStartTimestamp, _req.validityEndTimestamp, _req.uid, keccak256(_req.data));
    }
}
...
import "@thirdweb-dev/contracts/extension/SignatureAction.sol";

contract InfiniPack is ERC1155Drop, SignatureAction {
    using TWStrings for uint256;

    address private sysAddress; // Address to get signature from
    ...
    function _isAuthorizedSigner(address _signer) internal view virtual override returns (bool) {
        return _signer == sysAddress;
    }

    function changeSysAddress(address _sysAddress) external onlyOwner {
        require(_sysAddress != address(0), "_sysAddress != address(0)");
        sysAddress = _sysAddress;
    }

    function updateBaseURI(uint256 _tokenId, string calldata _baseURI) external onlyOwner {
        uint256 batchId;
    ...
    function uri(uint256 _tokenId) public view virtual override returns (string memory) {
        string memory batchUri = _getBaseURI(_tokenId);
        return string(abi.encodePacked(batchUri, _tokenId.toString(), _metaDataExtension));
    }

    // Get Meta data extension
    function getMetaDataExtension() public view returns (string memory) {
        return _metaDataExtension;
    }

    // Set Meta data extension Important! -> Will be the same for every token ID
    function setMetaDataExension(string calldata newExtension) external onlyOwner {
        _metaDataExtension = newExtension;
    }

    // Override the burn function to take a GenericRequest and a signature
    function burnWithSignature(GenericRequest calldata _req, bytes calldata _signature)
        external
        returns (address signer)
    {
        address caller = msg.sender;
        (address owner, uint256 tokenId, uint256 amount) = extractBurnData(_req);

        require(caller == owner || isApprovedForAll[owner][caller], "Unapproved caller");
        require(balanceOf[owner][tokenId] >= amount, "Not enough tokens owned");
    ...
    function extractBurnData(GenericRequest calldata _req)
        public
        pure
        returns (address _owner, uint256 _tokenId, uint256 _amount)
    {
        (_owner, _tokenId, _amount) = abi.decode(_req.data, (address, uint256, uint256));
        require(_owner != address(0), "Owner address cannot be 0");
        require(_amount != 0, "Amount cannot be 0");
        return (_owner, _tokenId, _amount);
    }

    function burn(address _owner, uint256 _tokenId, uint256 _amount) external pure override {
        revert("Function not available call burnWithSignature");
    }
}