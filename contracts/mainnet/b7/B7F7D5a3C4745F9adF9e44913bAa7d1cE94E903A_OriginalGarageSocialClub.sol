    ...
    function getPrice(uint256 _classId) public view returns (uint256) {
      ...
      if (classInfo.priceType == 1) { // dutch price
        if (classInfo.startPrice > classInfo.endPrice) { // dutch price goes down over time
          ...
          if (mintPrice > classInfo.startPrice) {
            mintPrice = classInfo.startPrice;
          } else if (mintPrice < classInfo.endPrice) {
            mintPrice = classInfo.endPrice;
          }
          ...
          if (mintPrice > classInfo.endPrice) {
            mintPrice = classInfo.endPrice;
          } else if (mintPrice < classInfo.startPrice) {
            mintPrice = classInfo.startPrice;
          }
        }
      }
