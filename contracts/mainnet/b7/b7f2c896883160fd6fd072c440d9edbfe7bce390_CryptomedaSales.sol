...
contract CryptomedaSales is Ownable {
  ...
  event ChangeLootboxContractAddress(address lootboxContractAddress);
  ...
  error LootboxError(uint256 _lootboxId);
    ...
    onlyAdmin
  {
    ...
    if (_lootboxID > 0) {
      if (_getLootboxStatus(_lootboxID)!=1)
        revert LootboxError(_lootboxID);
    }
    //
  ...
  function addSharedCap (string calldata _name, uint256 _cap, uint256 _maxSalesCnt) external onlyAdmin {
    require(_maxSalesCnt >= 2 && _maxSalesCnt <= 12, "Shared cap is for 2-12 sales");
    ...
    payable
  {
    ...
    if (sale_.lootboxID > 0) {
      ...
      if (lbAssetId_.length == 0)
        revert LootboxError(sale_.lootboxID);
    } else {
  ...
  {
    LootboxInterface = Lootbox(lootboxContractAddress);
    return LootboxInterface.openLootbox(_lootboxId, _buyer, _quantity);
  }
  ...
  {
    LootboxInterface = Lootbox(lootboxContractAddress);
    return LootboxInterface.getLootboxStatus(_lootboxId);
  }
