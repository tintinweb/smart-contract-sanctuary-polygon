...
contract EggMarket is BaseAccessControl, ReentrancyGuard, Pausable {
    ...
    function setPresalePrice(DragonInfo.Types drgType, uint newValue) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged(string(
            abi.encodePacked("presalePrices.", Strings.toString(uint(drgType)))
    ...
    function setPresaleMaxBuyCount(DragonInfo.Types drgType, uint newValue) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged(string(
            abi.encodePacked("presaleMaxBuyCounts.", Strings.toString(uint(drgType)))
        ), previousValue, newValue);
    }
    ...
    function allowPresale(DragonInfo.Types drgType, uint totalAllowedPresaleCnt, uint psPrice) 
    ...
    function setPublicPrice(DragonInfo.Types drgType, uint newValue) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged(string(
            abi.encodePacked("publicPrices.", Strings.toString(uint(drgType)))
            ), previousValue, newValue);
    }
    ...
    function setMaxBuyCount(DragonInfo.Types drgType, uint newValue) external onlyRole(CFO_ROLE) {
        ...
        emit ValueChanged(string(
            abi.encodePacked("maxBuyCount.", Strings.toString(uint(drgType)))
            ), previousValue, newValue);
    }
    ...
    function buy(DragonInfo.Types drgType, address to, uint count) 
    external 
    payable 
    nonReentrant 
    whenNotPaused {
    ...
    function claim(DragonInfo.Types drgType, address to, uint count) external nonReentrant {
...
contract EggToken is ERC721Burnable, BaseAccessControl {
    ...
    string public constant BAD_ARGS_ERROR = "EggToken: wrong arguments";
    string public constant NOT_ENOUGH_PRIVILEGES_ERROR = "EggToken: not enough privileges";
    string public constant EGG_EXISTS_ERROR = "EggToken: ID already exists";
    ...
    function currentEggsCount() public view returns(uint) {
        return _eggCounts[DragonInfo.Types.Unknown]+
            _eggCounts[DragonInfo.Types.Epic20] + 
            _eggCounts[DragonInfo.Types.Legendary];
    }
    ...
    function mint(address to, DragonInfo.Types _dragonType) external returns (uint) {
        require(currentEggsCount() < totalSupply(), SUPPLY_EXCEEDED_ERROR);
