...
contract ERC20Disperser is Ownable {
  /// @notice The address of token to be dispersed
  ...
  function addBalances(
    ...
    require(
      IERC20(token).balanceOf(_owner) >= _totalTokenAmount,
  ...
  function removeBalances(address[] calldata _targets) external onlyOwner {
    ...
    for (uint256 i = 0; i < _targets.length; i++) {
      ...
      bool sent = IERC20(token).transfer(_owner, _balance);
  ...
  function claim() external {
    ...
    bool sent = IERC20(token).transfer(msg.sender, _userBalance);
    require(sent, "ERC20Disperser: Couldn't send tokens to you.");
  ...
  function recoverTokens() external onlyOwner {
    uint256 _amount = IERC20(token).balanceOf(address(this)) - leftToBeClaimed;
    require(_amount > 0, "ERC20Disperser: No tokens to be recovered.");

    bool sent = IERC20(token).transfer(owner(), _amount);
    require(sent, "ERC20Disperser: Couldn't send tokens to you.");
  }
