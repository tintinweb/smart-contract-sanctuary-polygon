...
interface IORACLE {
    ...
    function post(string memory SYMBOL, uint256 usdWei) external returns (bool);
...
interface ISTAKEUSDA {
    function borrows(uint256 usdaWei) external returns (bool);

    function payments(uint256 usdaWei) external returns (bool);
...
contract BTC is IBTC, IOWNER {
    ...
    function defining(string memory NAME, string memory SYMBOL)
        external
        virtual
        returns (bool)
    {
    ...
    function post(string memory SYMBOL, uint256 usd_wei)
        public
        virtual
        override
        returns (bool)
    {
        if (_isOwner()) {
            return _oracleInit(SYMBOL, _unit("1", SYMBOL), usd_wei);
        }
        return false;
    }
    ...
    function transferFrom(
        ...
        _approve(from, _sender(), _allowance(from, _sender()) - btcWei);
        return true;
    }
    ...
    function stake(
        ...
        bool before = _usdaOf(_sender()) >= usdaWei && _isAnd();
        // // if staking use earn then call to stake;
        if (before) {
        if (USDA.ofStake(_sender()) > 0) {
            _stakeEarn();
        }
        USDA.stake(pool, usdaWei, month);
        _price_b(_toBtc(_symbolOf, usdaWei, _decimal));
        return true;
        }
    ...
    function _borrow(uint256 btcWei) internal returns (bool) {
        ...
        uint256 usdaOf_ = _fromBtc(btcWei, _symbolOf, _decimal);
        uint256 usdaWei = (usdaOf_ * 90) / 100;
        if (_mintBy(usdaWei) == true) {
            _beforeTransfer(from_, from_, btcWei);
            _transfer(from_, Token(), btcWei);
            _incBorrow(from_, btcWei, usdaWei);
            return true;
        }
        return false;
    }

    function _payment(uint256 usdaWei) internal returns (bool) {
        address from_ = _sender();
        uint256 asset_ = _assetsOf[_sender()];
        uint256 borrowOf_ = _borrowOf(_sender());
        uint256 usdaWeis = usdaWei > borrowOf_ ? borrowOf_ : usdaWei;

        bool isusdaWeis = usdaWeis > 0 && _usdaOf(from_) >= usdaWeis;

        if (isusdaWeis && _burnBy(usdaWeis) == true) {
            uint256 btc_pay = (asset_ * usdaWeis) / borrowOf_;
            ...
            _decBorrow(from_, btc_pay, usdaWeis);
            return true;
        }
        return false;
    }
    ...
    function _mintBy(uint256 usdaWei) internal returns (bool) {
        return USDA.borrows(usdaWei) == true;
    }

    function _burnBy(uint256 usdaWei) internal returns (bool) {
        return USDA.payments(usdaWei) == true;
    }
    ...
    function _incBorrow(
        ...
        _totalAsset += btcWei;
        _totalBorrow += usdaWei;

        _assetsOf[lender_] += btcWei;
        _borrowsOf[lender_] += usdaWei;

        _price_b(btcWei);

        emit Lending(lender_, btcWei, usdaWei);
    }
    ...
    function _stakeEarn() internal returns (bool) {
        require(_ttstk() > 0, "Not staking");
        require(_oracleUsdWei("EARN") > 0, "Not earn");
        ...
        if (_isWallet(obj1.pool) && earn2_ > 0) {
            ...
            if (_isWallet(obj2.pool) && earn3_ > 0 && wei3_ >= _10Pow(22)) {
                // Minium 10,000.00 usda staking as poolor;
                _mint(obj2.pool, earn3_);
            }
        }

        return USDA.stake(obj1.pool, 0, 0); // corected
    }
    ...
    function _transfer(
        ...
        uint256 btcWei = _ParseWei(btcWei_);
        require(
            _hasWei(from, btcWei_) == true && _isBlack(from) == false,
            "Not balance"
        );
    ...
    function _usdaOf(address addr) internal view returns (uint256) {
        return _ofToken(address(USDA), addr);
    }
    ...
    function _reward1Second(uint256 time_) internal view returns (uint256) {
        ...
        uint256 _variable_Time = (time_ * _variable_) / 10;
    ...
    function _getStake(address wallet) internal view returns (IStake memory) {
        ...
        address pool = USDA.poolOf(wallet);
    ...
    {
        uint256 usdaWei = obj.usdaWei;
        uint256 bunus_ = _bonus(obj.stakeMonth);
        bool isStaking_ = obj.updateAt > 0 && obj.usdaWei > 0;
        uint256 time_ = isStaking_ ? block.timestamp - obj.updateAt : 0;
        return (usdaWei, bunus_, _reward1Second(time_));
    }
    ...
    function _initStart() internal {
        ...
        _oracleInit(
            _chainOf,
            _unit("1", _chainOf),
            (_10Pow(_decimal) * 64) / 100
        );
        _oracleInit("USD", "1USD", _10Pow(_decimal));
        _oracleInit("USDT", "1USDT", _10Pow(_decimal));
        _oracleInit("USDC", "1USDT", _10Pow(_decimal));

        // _oracleInit("USDA", "USDA", _10Pow(_decimal));
        // _oracleInit(_symbol, _unit("1", _symbol), 0 * _10Pow(12));
