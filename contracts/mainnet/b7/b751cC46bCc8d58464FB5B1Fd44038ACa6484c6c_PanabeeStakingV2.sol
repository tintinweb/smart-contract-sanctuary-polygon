...
contract PanabeeStakingV2 is PanabeeStaking {

     function initialize(
        address _maticPriceToBNB
    ) external initializer {

        priceFeed = AggregatorV3Interface(_maticPriceToBNB);
    }


    function setPriceFeed(address _maticPriceToBNB) public initializer {
        priceFeed = AggregatorV3Interface(_maticPriceToBNB);
    }
    ...
    OwnableUpgradeable
{
    ...
    //mainnet address: 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0;
    address private MATIC_TO_USD;
    uint256 private minimumWithdrawal;
    uint256 private maximumWithdrawal;
    uint256 private maticDeposit;
    ...
    function initialize(
        ...
        priceFeed = AggregatorV3Interface(_maticPriceToBNB);
        MATIC_TO_USD = _maticPriceToBNB;
    }

    //user functions
    function rentMiner(address referral) public payable nonReentrant {
        //convert the tokenAmount to the 9 decimal places
        //tokenAmount is coming in as a 1eth 1e18
        //get value of token needed
        require(_checkIfParametersAreSetUp(), "set up parameters");
        ...
        require(_isDepositEnough(msg.value), "you need to enough Matic");
        ...
        //transfer the Matic to the wallet we need
        uint256 maticAmount = msg.value;
        payable(AdminWallet).transfer((maticAmount * 20) / 100); //20%
        payable(MarketingWallet).transfer((maticAmount * 30) / 100); //30%
        payable(TreasuryWallet).transfer((maticAmount * 50) / 100); //50%
    ...
    function feedMiner(uint256 _minerIndex) public nonReentrant {
        ...
        require(
            !_checkIfMinerExpired(_user.miners[_minerIndex]),
            "Miner has expired"
        );
        bool canFeed = _checkMinerCanFeed(
            _user.miners[_minerIndex].minerLastFed
        );
        ...
        _user.miners[_minerIndex].miningStatus = true;
        //update the last feed time.
        _user.miners[_minerIndex].minerLastFed = block.timestamp;
    ...
    function claimReward() public nonReentrant {
        ...
        for (uint256 i = 0; i < minersLength; i++) {
            ...
            if (canWithdraw) {
                ...
                if (daysOfMinning > 0) {
                    ...
                    uint256 _dailyReturn = miner.dailyPercent;
                    if (miner.dailyPercent == 0) {
                        _dailyReturn = dailyPercentage;
                    }
                    uint256 minerReturnsAmount = (_dailyReturn *
                        miner.rentFee *
                        daysOfMinning *
                        BASE) / 100;
        ...
        //check if the amount is within the minimum or maximum
        require(
            _isWithdrawalInRange(amountToTransfer),
            "not up to minimum or exceeds maximum withdrawal"
        );
    ...
    function setMinimumWithrawalAmount(uint256 _amount) public onlyOwner {
        minimumWithdrawal = _amount;
    }

    function setMaxmimumWithdrawalAmount(uint256 _amount) public onlyOwner {
        maximumWithdrawal = _amount;
    }

    function getMinandMaxWithdrawal() public view returns (uint256, uint256) {
        return (minimumWithdrawal, maximumWithdrawal);
    }

    function setMaticValueForRenting(uint256 _value) public onlyOwner {
        maticDeposit = _value;
    }

    function getMaticValueForRenting() public view returns (uint256) {
        return maticDeposit;
    }

    function _isDepositEnough(uint256 _amount) private view returns (bool) {
        return _amount == maticDeposit;
    }

    function _isWithdrawalInRange(uint256 _amount) private view returns (bool) {
        return (_amount >= minimumWithdrawal && _amount <= maximumWithdrawal);
    }

    function _checkIfParametersAreSetUp() private view returns (bool) {
        return (minimumWithdrawal != 0 &&
            maximumWithdrawal != 0 &&
            maticDeposit != 0 &&
            rentMinerFee != 0 &&
            feedMinerFee != 0 &&
