...
import "@openzeppelin/contracts-0.8/token/ERC721/extensions/ERC721URIStorage.sol";
import "@openzeppelin/contracts-0.8/token/ERC721/extensions/ERC721Royalty.sol";
import "@openzeppelin/contracts-0.8/utils/Counters.sol";
import "../utils/AccessProtected-0.8.sol";

/// @title ArcadeNetwork Partners NFT
/// @author ArcadeNetwork
/// @notice ERC721 NFT Contract for Partners Collection
contract ArcPartnerNFT is ERC721URIStorage, AccessProtected, ERC721Royalty {
    ...
    function mint(address recipient, string memory _tokenURI)
        ...
        returns (uint256)
    {
        require(!_hashes[_tokenURI], "NFT for tokenURI already minted");
    ...
    /// @param recipient NFT recipient address
    /// @param _tokenURI NFT URI batch
    function mintBatch(address[] memory recipient, string[] memory _tokenURI)
        external
        onlyAdmin
        returns (uint256[] memory mintedTokenIds)
    {
        require(
            recipient.length == _tokenURI.length,
            "recipient and tokenURI arrays must be the same length"
        );
        mintedTokenIds = new uint256[](recipient.length);
        for (uint256 i = 0; i < recipient.length; i++) {
            mintedTokenIds[i] = (mint(recipient[i], _tokenURI[i]));
        }
    ...
    /// @dev expose setDefaultFee
    /// @param receiver address
    /// @param feeNumerator uint256
