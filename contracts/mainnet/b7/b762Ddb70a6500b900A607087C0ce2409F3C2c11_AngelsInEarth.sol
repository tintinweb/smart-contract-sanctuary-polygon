...
contract AngelsInEarth is ERC721 {
    uint256 public totalSupply;
    uint256 public cost = 0.29 ether;
    uint256 public maxMints = 47;
    ...
    constructor()
        ERC721("Angels in Earth", "AIE")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 48) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiabng7rdku44ncskrph3ypuqvl4psoeccmdkiffxkzx7tidkanuoq/", Strings.toString(tokenId), ".json")
            );
    }
