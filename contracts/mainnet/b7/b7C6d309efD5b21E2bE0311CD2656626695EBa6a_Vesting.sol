...
contract Vesting is VestingWallet {
    ...
    */
    constructor(address safe_, address registry_, bytes memory signature_)
        VestingWallet(safe_, uint64(block.timestamp), uint64(block.timestamp + 60 * 24 * 365 * 2)) {
        registry = IRegistry(registry_);
        registry.register(Constants.VESTING, signature_);
    }
    /**
