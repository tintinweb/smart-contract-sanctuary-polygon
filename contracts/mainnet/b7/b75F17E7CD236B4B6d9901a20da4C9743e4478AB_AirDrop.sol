...
contract AirDrop is AccessControl {
  ...
  {
    return (users[_user].eligible, users[_user].claimed, users[_user].amount);
  }
    ...
    onlyRole(DEFAULT_ADMIN_ROLE)
  {
    require(
      ...
      "Array length should be same for both address and amount"
    );
    for (uint256 i = 0; i < _users.length; i++) {
      users[_users[i]] = User(true, false, _amounts[i]);
    }
  }

  function claim() external {
    require(
      users[_msgSender()].eligible,
      "You are not eligible for this airdrop"
    );
    require(
      !users[_msgSender()].claimed,
      "You have already claimed your tokens"
    );
    users[_msgSender()].claimed = true;
    token.transfer(_msgSender(), users[_msgSender()].amount);
  }

  function claimTokenBack() external onlyRole(DEFAULT_ADMIN_ROLE) {
