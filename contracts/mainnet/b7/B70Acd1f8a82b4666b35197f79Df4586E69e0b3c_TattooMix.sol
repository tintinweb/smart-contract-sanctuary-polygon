...
contract TattooMix is ERC721 {
    ...
    constructor()
        ERC721("Tattoo Mix", "TTMX")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 39) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeiamapjarljoey6sjntkardqb6p34xsvdvid5m5v2glzf2nescpvlm/", Strings.toString(tokenId), ".json")
            );
    }
