...
contract Leverage is Ownable {
    ...
    function deleverage(
        ...
        if (test_step == 1) return;
        emit Deleverage(
            IERC20Detailed(ADDR_arth).balanceOf(address(this)),
            IERC20Detailed(_asset).balanceOf(address(this))
        );
        // Send deleveraged asset to user.
        uint256 balanceRemainAsset = IERC20Detailed(ADDR_arth).balanceOf(
            address(this)
        );
        ...
        balanceRemainAsset = IERC20Detailed(_asset).balanceOf(address(this));
        if (balanceRemainAsset > 0) {
            IERC20Detailed(ADDR_arth).transfer(msg.sender, balanceRemainAsset);
        }
