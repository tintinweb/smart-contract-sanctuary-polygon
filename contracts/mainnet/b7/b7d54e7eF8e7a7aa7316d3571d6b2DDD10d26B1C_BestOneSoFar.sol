...
// File: contracts/BestOneSoFar.sol
...
contract BestOneSoFar is ERC721A, IERC2981,ReentrancyGuard,Ownable {
    //set variables
    ...
    bool public _activePublicSale = false;
    bool public _activePresaleSale = false;
    
    //mappings for counters
    mapping(address => uint8) public _publicCounter;    
    mapping(address => uint8) public _presaleCounter;    
    mapping(address => uint256) public _ownerCounter;     

    //wallet for on cahin royalties
    address public royaltyWallet = 0x8AB5496a45c92c36eC293d2681F1d3706eaff85D;
    ...
    modifier isCorrectPayment(uint256 pricePerNFT, uint256 numberOfTokens) {
        require(
            pricePerNFT * numberOfTokens == msg.value,
            "Incorrect funds"
        );
        _;
    }
    ...
    function publicMint (uint8 quantity)
        ...
        payable
        callerIsUser   
        isCorrectPayment(price, quantity)  
        nonReentrant   
    {       
        require(_activePublicSale, "Public mint is not yet active");
        require(_publicCounter[msg.sender] + quantity <= maxQtyPerMember, "Exceeds Max Per Wallet Address");
    ...
    function presaleMint (uint8 quantity, bytes32[] calldata _merkleProof)
        external
        payable
        callerIsUser  
        isCorrectPayment(price, quantity)     
        nonReentrant   
    {       
        require(_activePresaleSale, "Whitelist mint is not yet active");
        require(_presaleCounter[msg.sender] + quantity <= maxQtyPerMember, "Exceeds Max Per Wallet Address");
        require(quantity > 0, "Must mint more than 0 tokens");
        require(totalSupply() + quantity <= maxSupply, "Sold out");
        require(quantity <= maxQtyPerTransaction, "Exceeds max per transaction");        
        require(price * quantity >= msg.value, "Incorrect funds");
        ...
        payable(_splitter).transfer(msg.value);        
        _presaleCounter[msg.sender] = _presaleCounter[msg.sender] + quantity;        
    }     
