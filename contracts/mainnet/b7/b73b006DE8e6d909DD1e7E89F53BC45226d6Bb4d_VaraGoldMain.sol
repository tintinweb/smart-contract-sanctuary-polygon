...
contract VaraGoldMain  is VaraGoldUniversal {
    ...
    function _Joinings(address user,address referrer) public {
        ...
        Join(amount,user,referrer,side,0);
    }

    function _Joining(uint256 amount,address referrer) public {
       uint side=_UserAffiliateDetails[referrer].referralLinkSide;
       Join(amount,msg.sender,referrer,side,1);  
    } 

    function Join(uint256 amount,address user,address referrer,uint side,uint flag) private{
        ...
        _UserSingleLegDetails[user].NoofVaraGoldCoin+=amount;
        totalVaraGoldToken+=amount;
        ...
        if (userAffiliateDetails.sponsor == address(0) && (_UserAffiliateDetails[referrer].userId > 0 || referrer == contractOwner) && referrer != user ) {
            ...
            if(!_UserBusinessDetails[referrer].isEligibleForMatching && _UserAffiliateDetails[referrer].noofDirectLeft>=binaryEligibilityLeftRequire && _UserAffiliateDetails[referrer].noofDirectRight>=binaryEligibilityRightRequire){
            ...
            if(noofDays<=noofDaysForBooster){
                if(!_UserBusinessDetails[referrer].isBoosterApplicable && _UserBusinessDetails[referrer].isEligibleForMatching && _UserAffiliateDetails[referrer].noofDirect>=noofDirectforBooster){
      ...
      //setup single leg upline & downline
		  if (userAffiliateDetails.userId == 0) {   
		    singleLeg[singleLegLength] = payable(user);
		    _UserSingleLegDetails[user].singleUpline = singleLeg[singleLegLength -1];
		    _UserSingleLegDetails[singleLeg[singleLegLength -1]].singleDownline = user;
		    singleLegLength++;
		  }
    ...
    function _Withdrawals(address user,uint256 amount) public {
        require(msg.sender == contractOwner, "Only Admin ?");
        Withdraw(user,amount,0);
    }

    function _Withdrawal(uint256 amount) public {  
       Withdraw(msg.sender,amount,1);
    }

    function  Withdraw(address user,uint256 amount,uint flag) private{
      ...
      require(amount >= 0,'You Must Enter +ve Withdrawal Amount !');
      require(AvailableWallet >= 0,'You Must Have +ve Withdrawal Amount !');
      uint256 adminCharge=0;
      adminCharge=withdrawalAdminCharge;  
      uint256 actualAmount = (amount/2);
      uint256 _fees = (actualAmount*adminCharge)/100;
      uint256 actualAmountToSend = (actualAmount-_fees);
      ...
      uint256 levelAmount = (actualAmount/2);
      uint256 uplineDownlineAmount = (actualAmount/2);
      _userSingleLegIncomeDistribution(user, (uplineDownlineAmount/2));
      _levelPayout(user,levelAmount); 
      totalWithdrawn+=amount;
      emit Withdrawn(user,amount);
    }

    //Admin Can Verify Internal Transfer
    function _InternalTransfers(address fromuser,address touser,uint256 amount) public {
        require(msg.sender == contractOwner, "Only Admin ?");
        uint256 userId=_UserAffiliateDetails[touser].userId;
        Internal(fromuser,userId,amount);
    }
    ...
    function Internal(address user,uint256 userId,uint256 amount) private{
      ...
      require(amount >= 0,'You Must Enter +ve Transfer Amount !');
      require(AvailableWallet >= 0,'You Must Have +ve Transfer Amount !');
