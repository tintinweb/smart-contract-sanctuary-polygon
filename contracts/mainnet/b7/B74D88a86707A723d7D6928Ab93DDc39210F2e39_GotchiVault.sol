...
struct AppStorage {
    ...
    //the Vault's split of alchemica profits, in %
    uint8 alchemicaFee;
...
contract GotchiVault is IERC173, Initializable,PausableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function setAlchemicaFee(uint8 _newAlchemicaFee) public onlyOwner{
        s.alchemicaFee = _newAlchemicaFee;
    }

    function getAlchemicaFee() public view returns(uint8){
        return s.alchemicaFee;
    }
    ...
    function depositERC721(address _tokenAddress, uint256[] calldata _tokenId) public whenNotPaused {
        require(_tokenId.length <= 10, "depositErc721");
    ...
    function lockTimeRemaining(uint256 _tokenId) public view returns(uint256 _timeRemaining){

        uint256 timeElapsed = (block.timestamp - s.gotchiMapping[_tokenId].lastLocked);
        
        _timeRemaining = s.gotchiMapping[_tokenId].lockPeriod - timeElapsed;
    }
    ...
    function setRentalSettings(
        ...
        for(uint256 i = 0; i < _tokenIds.length; i++){
            ...
            require(_revenueSplit[2] == s.alchemicaFee,"setRentalSettings");
    ...
    function modifyGuildSettings(
        ...
        require(msg.sender == s.guilds[_guild].approved || msg.sender == s.contractOwner);
...
contract vGHST is Initializable, ERC20Upgradeable, PausableUpgradeable {
    ...
    function enter(uint256 _amount) public whenNotPaused returns(uint256)  {
        ...
        //todo: once amGHST goes live, will need to deposit to amGHST and stake amGHST for frens
    ...
    function leave(uint256 _share) public {
        if(_share>0){
            ...
            if(ghstAmount > IERC20Upgradeable(ghstAddress).balanceOf(address(this))){
                ...
                //todo: once amGHST is live, will need to also withdraw from Aave
            }
            _burn(msg.sender, _share);
    ...
    //todo: once we are holding amGHST, will need to convert amGHST to GHST in this calculation
        ...
        address _weth) public onlyOwner
    {
        ...
        uni = _uni;
        weth = _weth;
        //todo: when amGHST live, need to add appropriate approvals -- Aave will need to take GHST, Aavegotchi will need to take amGHST
    }
    ...
    function harvestMaticIntoToken() public onlyApproved() {
        ...
        if(_wmaticBalance > 2) {
            ...
            // todo: once amGHST staking is live, need to deposit amGHST into aavegotchi for frens
        }
    }*/
