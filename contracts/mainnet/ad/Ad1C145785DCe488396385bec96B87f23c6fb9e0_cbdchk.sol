...
contract cbdchk is ERC721, Ownable {

    uint256 constant public MAX_LANDS = 321832;

    string public baseTokenURI = "ipfs://QmUNLLsPACCz1vLxQVkXqqLX5R1X345qqfHbsf67hvA3Nn/";
    ...
    address proxyRegistryAddress; //opensea

    event LandMinted(address account, uint256 tokenId);

    constructor() ERC721("cbd chk", "cbdchk") {
    ...
    function mintNFTWhitelist(uint256 amount, bytes32 signatureR, bytes32 signatureVS) public payable {
        require(checkWhitelist, "Public mode");
    ...
    function mintNFT(uint256 amount) public payable {
        require(!checkWhitelist, "Whitelist mode");
    ...
    function _mintNFT(uint256 amount, bytes32 signatureR, bytes32 signatureVS) internal {
        ...
        require(_tokenIdCounter + amount <= MAX_LANDS, "MAX_LANDS limited");
        ...
        for (uint256 i=0; i<amount; i++) {
            ...
            emit LandMinted(msg.sender, newItemId);
        }
    }
    ...
    function airdrop(address[] memory addrs) public onlyOwner {
        ...
        for (uint256 i=0; i<amount; i++) {
          uint256 newItemId = start + i + 1;
          _mint(addrs[i], newItemId);

          emit LandMinted(addrs[i], newItemId);
        }
    }
