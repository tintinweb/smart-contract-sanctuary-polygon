...
contract AaveV2V3Adapter is CustodianStorage, IAdapter {
    ...
    function collectRewards() public returns (uint256 collectedAmount) {
        ...
        if(newYieldFactor > lastYieldFactor) collectedAmount = (yieldProviderBalance * (newYieldFactor - lastYieldFactor)) / RAY;
...
contract PoolCustodian is CustodianStorage, AccessControl, IPoolCustodian {
    ...
    constructor(
        ...
        (bool success, bytes memory returnData) = _adapter.delegatecall(abi.encodeWithSignature('supportsToken(address)', _yieldProvider));
    ...
    function switchYieldProvider(address newAdapter, address newYieldProvider) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(IAdapter(newAdapter).supportsInterface(type(IAdapter).interfaceId));
        (bool success, bytes memory returnData) = newAdapter.delegatecall(abi.encodeWithSignature('supportsToken(address)', newYieldProvider));
        require(success && abi.decode(returnData, (bool)));

        collectRewards();
        uint256 balanceToSwitch = depositedBalance;
        (success, ) = adapter.delegatecall(abi.encodeWithSignature('empty()'));
        require(success);
