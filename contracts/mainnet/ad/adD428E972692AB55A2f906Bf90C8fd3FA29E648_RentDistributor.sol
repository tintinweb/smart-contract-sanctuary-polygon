 ...
 * @notice To redeem property rent reward tokens(vtry) to USDC/Jtry.
 */
    ...
    ReentrancyGuardUpgradeable
{
    ...
    /// @notice percentage of fees to charge from user when redeeming rent
    uint256 public feePercentage; // 100 is 1%; 10 is 0.1%

    /// @notice Representation of 100%. Equals t0 10000 that means 100 = 1%
    uint256 public PERCENTAGE_BASED_POINT;
    ...
    event LiquidityWithdrawed(address indexed to, uint amount);
    event RentRedeemed(address indexed recipient, uint amount, address token);
    ...
    function initialize(address _finder) public initializer nonReentrant {
        ...
        _grantRole(MAINTAINER_ROLE, msg.sender); // chaning here made it work
    }
     ...
     * @param amount amount of property rent tokens willing to pay for reedem
     * @param tokenOut token currency to get, jtry or USDC
     * @param recipient receiver of the redeemed amount
     * @param _metaTxArgs to perform Permit on property reward token
    ...
    function redeemWithPermit(
        ...
        uint minTargetAmount // in case of output token is JTRY
    ) external returns (uint) {
        address vTRY = IFinder(finder).getVTRY();
        IMetaTx(vTRY).permit(
            _metaTxArgs.owner,
            _metaTxArgs.spender,
            _metaTxArgs.value,
    ...
    function redeem(
        ...
        address vTRY = IFinder(finder).getVTRY();
        address tokenWhitelist = IFinder(finder).getTokensWhitelistAddress();
        address USDC = IFinder(finder).getUSDC();
        _require(
            ITokensWhitelist(tokenWhitelist).isOnWhitelist(tokenOut),
            Errors.TOKEN_NOT_WHITELISTED
        );
        IERC20(vTRY).safeTransferFrom(_msgSender(), address(this), amount);
        ERC20Burnable(vTRY).burn(amount);
        uint amountInUSDC = calcVtryToUSD(amount, vTRY);
        uint256 toTransfer = amountInUSDC - _calcFees(amountInUSDC);
        if (tokenOut == USDC) {
            IERC20(USDC).safeTransfer(recipient, toTransfer);
            emit RentRedeemed(recipient, toTransfer, USDC);
            return toTransfer;
        } else {
            address oclrAddress = IFinder(finder).getOclrAddress();
            IERC20(USDC).safeIncreaseAllowance(oclrAddress, toTransfer);

            IOCLRouter.DexSwapArgs memory _swapArgs;
            _swapArgs._quoteCurrency = synthereumLiqPoolAddress;
            _swapArgs._origin = USDC;
            _swapArgs._target = tokenOut;
            _swapArgs._originAmount = toTransfer;
            _swapArgs._minTargetAmount = minTargetAmount;
            _swapArgs._deadline = block.timestamp + (60 * 30);
            _swapArgs._receipient = recipient;
            uint tokensRecieved = IOCLRouter(oclrAddress).mintOrRedeemOnJaris(
                _swapArgs
            );
            emit RentRedeemed(recipient, tokensRecieved, tokenOut);
            return tokensRecieved;
        }
    }

    /**
     * @notice For admin to withdraw USDC liquidity.
    ...
    function withdrawLiquidity(
        ...
        emit LiquidityWithdrawed(to, amount);
    }
     ...
     * @return fees to be charged on input amount
     */
    ...
    /**
     * @notice This uses JTRY feed fo the conversion as we consider VTRY and JTRY 1:1
     * @param _vTryAmount amount of vTry tokens
     * @param _vTry address of vTRY
     * @return amount of USDC equivalent to inputted vTRY amount
     */
    function calcVtryToUSD(
        uint _vTryAmount,
        address _vTry
    ) internal view returns (uint) {
        uint vTRYRateChainLink = getTokenFromChainlink(_vTry);
        return _vTryAmount.wadMul(vTRYRateChainLink) / (10 ** 12);
    }

    /**
     * @param _token address of the tokens
     * @return latest price from the chainlink
     */
    function getTokenFromChainlink(
        address _token
