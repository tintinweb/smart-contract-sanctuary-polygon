...
contract GammaWmaticUsdtNarrowQuery is GiddyQueryV2, Initializable, OwnableUpgradeable {
  ...
  address constant private POS = 0x598cA33b7F5FAB560ddC8E76D94A4b4AA52566d7;
  ...
  function getDepositRatios() external view override returns (uint256[] memory ratios) {
    ...
    (ratios[0], ratios[1]) = IUniProxy(IHypervisor(POS).whitelistedAddress()).getDepositAmount(POS, WMATIC_TOKEN, 1000e18);
    ratios[1] = (ratios[0] + ratios[1]) / 2;
    ratios[0] = (1000e6 * RATIO_PERCENT) / (1000e6 + ratios[1]);
    ratios[1] = (ratios[1] * RATIO_PERCENT) / (1000e6 + ratios[1]);    
  }
