...
contract DubVestingFund is Ownable{
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    // Address of dub token
    IERC20 public immutable dub;
    ...
    // Num of stages
    uint8 public constant NUM_STAGES = 10;
     ...
     * @param _dub Address of dub Token that will be locked on contract.
    ...
    constructor (
        ...
        require(_dub != address(0), "Illegal address");
    ...
    function getAvailableTokensToRelease () public view returns (uint256 tokensToSend) {
        ...
        if (tokensUnlockedPercentage >= 100) {
            tokensToSend = dub.balanceOf(address(this));
        } else {
    ...
    function initVestingStages () internal {
        stages[0].date = 1656630060;
        stages[1].date = 1659308460;
        stages[2].date = 1661986860;
        stages[3].date = 1664578860;
        stages[4].date = 1664578860;
        stages[5].date = 1669852860;
        stages[6].date = 1672531260;
        stages[7].date = 1675209660;                        
        stages[8].date = 1677628860;
        stages[9].date = 1680303660;
     ...
     */
    function Digup () external {
        // solhint-disable-next-line not-rely-on-time
        require(
            block.timestamp >= stages[0].date,
            "Dub Vesting: current time is before first release time"
        );
        ...
        require(tokensToSend > 0, "Dub Vesting: no tokens to release");

        sendTokens(tokensToSend);
    }
    
    function emergencyWithdraw(IERC20 token, address recipient, uint256 amount) external onlyOwner {
    ...
    //         .call{value: token.safeTransfer(msg.sender, balance)}("");
    //     require(success);
    // }
    ...
    function setInitialTokensBalance () private {
        initialTokensBalance = dub.balanceOf(address(this));
    }
    ...
    function sendTokens (uint256 tokensToSend) private {
        ...
        dub.safeTransfer(destinationAddress, tokensToSend);
        // Raising event
