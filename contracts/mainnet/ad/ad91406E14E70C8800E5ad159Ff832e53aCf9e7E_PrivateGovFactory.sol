...
interface IPrivateGovFactory{
    ...
    function createProject(uint256, uint128, uint128, uint128, uint128, uint128[2] calldata, address, address) external returns (address);
...
import "./PrivateGovSale.sol";

contract PrivateGovFactory is IPrivateGovFactory{
    ...
    function createProject(
        ...
        project = address(new PrivateGovSale());

        allProjects.push(project);
        
        PrivateGovSale(project).initialize(
            _calculation,
            _start,
            _sale,
            _target,
...
contract PrivateGovSale{
    ...
    function initialize(
        ...
        for(uint32 i=1; i<=2; i++){
            ...
            booster[i].end = booster[i].start + 900;
     ...
     */
    function isEligible(address _user, uint32 _boosterRunning) public view returns (bool){
    ...
    function moveFund(uint16 _percent_d2, bool _devAddr, address _target) external {
        ...
        if(_devAddr){
            TransferHelper.safeTransfer(address(payment), factory.devAddr(), amount);
        } else{
    ...
    function moveFee() external {
        require(payment.balanceOf(address(this)) >= revenue && block.timestamp >= booster[2].start && !isMoved && (msg.sender == factory.savior() || msg.sender == owner), "bad");

        isMoved = true;
        buyEnded = true;

        TransferHelper.safeTransfer(address(payment), factory.devAddr(), revenue);
    }
    ...
    function refund() external {
        ...
        uint128 amount = (getTotalPurchase(msg.sender) * price * _refund_d2) / 1e22; // 1e18 (token decimal) + 1e4 (percent 2 decimal)
     ...
     * @param _fee_d2 Fee project percent in each rounds in 2 decimal
     */
    function setFee_d2(uint128[2] calldata _fee_d2) external onlyOwner {
        require(uint128(block.timestamp) < booster[1].start, "bad");
