...
{    
    ...
    function poolInfo(uint256 _pid) external view returns (uint128 accBananaPerShare, uint64 lastRewardTime, uint64 allocPoint);		
    ...
    ML_TransferHelper
{
    ...
    ///@notice total balance of strategy + pool
    ...
    ML_RouterSwap_UniSwapV2
{
    ...
    {        
        try IUniRouterV2(_router).getAmountsOut(_amount, _path) returns (uint256[] memory out)
        {
    ...
    VaultStrategy_LP_UniSwapV2
{
    ...
    function poolTotalAllocPoints() public view virtual override(IVaultPoolInfo, VaultStrategy_withPoolInfo) returns (uint256)
    {
        return IMiniChefV2_Polygon_ApeSwap(poolProvider).totalAllocPoint();
    }

    function poolAllocPoints() public view virtual override(IVaultPoolInfo, VaultStrategy_withPoolInfo) returns (uint256)
    {
        (,, uint64 allocPoint) = IMiniChefV2_Polygon_ApeSwap(poolProvider).poolInfo(poolID);
        return uint256(allocPoint);
    }

    function poolTotalRewardEmission() public view virtual override(IVaultPoolInfo, VaultStrategy_withPoolInfo) returns (uint256)
    {
        return IMiniChefV2_Polygon_ApeSwap(poolProvider).bananaPerSecond();
    }
    ...
    function poolBlockOrTime() public view virtual override(IVaultPoolInfo, VaultStrategy_withPoolInfo) returns (bool)
    {
        return true; //seconds
    }
