...
contract Marketplace is BaseContract {
  ...
  function extendOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external {
    ...
    uint currentPrice = _convertUsdToTokenByDecimals(
      _paymentAddress,
    ...
    emit OrderExtended(_nftContractAddress, _tokenId, msg.sender, currentPrice, _weekNumber, _paymentAddress, expiredTime, order.takeOrderPrice, discountPrice, block.timestamp);
  }
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint, uint, uint) {
    ...
    if (payments[_paymentAddress].isStableCoin) {
      currentPrice = order.orderType == IMarketPlace.OrderType.installmentRental
        ? _weekNumber * currentPrice
        : currentPrice;
      return (_convertUsdToTokenByDecimals(_paymentAddress, currentPrice), takeUSDPrice, discountPrice);
    }
    ...
    if (payments[_paymentAddress].isUniSwapV3) {
      ...
      currentPrice = currentPrice * (10 ** decimalTokenOut) / tokenPrice;
    } else {
      ...
      currentPrice = _convertUsdToTokenByDecimals(_paymentAddress, currentPrice) * DECIMAL9 / tokenPrice;
    }
