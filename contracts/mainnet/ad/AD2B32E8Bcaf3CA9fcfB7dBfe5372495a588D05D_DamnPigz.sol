...
contract DamnPigz is ERC721ARoyalty, Ownable, ReentrancyGuard {
    ...
    constructor(string memory _metadataUri, address _treasureAccount) ERC721A("Damn Pigiz", "DAMPNFT") {
        setBaseURI(_metadataUri);
        _setDefaultRoyalty(_treasureAccount, uint96(800));
    }

    function mint(uint32 _amount) external payable nonReentrant {
        require(!paused, "The contract is paused!");
        require(
            _amount > 0 && _amount <= maxMintAmountPerTx,
            "Invalid mint amount!"
        );
