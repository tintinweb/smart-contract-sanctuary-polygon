...
contract BecomeTheCitizen is TokenAccessControl {
    ...
    function createBatchTokens(uint16 count) public ifNotPaused onlyContractOwner {
        for(uint i=0;i<count;i++){
            createToken();
        }
    }

    function createToken() public ifNotPaused onlyContractOwner returns (uint256 tokenId) {
        this.generateDigits();
        ...
        _transfer(address(0), msg.sender, lastIndex);
