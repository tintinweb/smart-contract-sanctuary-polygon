...
contract NFTMUT is ReferralNFT, TierNFT {

    constructor()
        AvatarNFT(
            17.87 ether, // starting price
            2750, // total
            0, // reserved
            20, // max tokens per mint
            "https://metadata-ak-dev.vercel.app/api/nftcars/",
            "NFT MUT", "NFT MUT"
        )
    ...
    function initTiers(uint8) internal override {
        // This is in MATIC
        _priceByTier[TierId.wrap(uint8(Tier.Basic))] = 17.87 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Advanced))] = 89.35 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Pro))] = 178.7 ether;
        _priceByTier[TierId.wrap(uint8(Tier.GT))] = 446.75 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Supersport))] = 893.5 ether;
        _priceByTier[TierId.wrap(uint8(Tier.Unlim))] = 1429.6 ether; 

		
        // inverted!
        _tierRanges[TierId.wrap(uint8(Tier.Unlim))] = Range(0, 799);
        _tierRanges[TierId.wrap(uint8(Tier.Supersport))] = Range(800, 1499);
        _tierRanges[TierId.wrap(uint8(Tier.GT))] = Range(1500, 1999);
        _tierRanges[TierId.wrap(uint8(Tier.Pro))] = Range(2000, 2399);
        _tierRanges[TierId.wrap(uint8(Tier.Advanced))] = Range(2400, 2649);
        _tierRanges[TierId.wrap(uint8(Tier.Basic))] = Range(2650, 2749);
    }
