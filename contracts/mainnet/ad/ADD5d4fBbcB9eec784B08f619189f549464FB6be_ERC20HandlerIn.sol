...
contract ERC20HandlerIn is
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override {
		...
		amount = (amount * 10**IERC20Metadata(tokenAddress).decimals()) / 10**srcTokenDecimals;

		if(tokenAddress == address(_WETH) && !isWrapped){
...
contract HandlerHelpersCustom is
     ...
     */
    function setBurnable(address contractAddress) external override {
        _onlyBridge();
