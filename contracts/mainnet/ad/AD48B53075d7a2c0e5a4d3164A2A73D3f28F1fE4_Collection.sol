...
// File: contracts/Collection1.sol
...
contract TempTest  {
    address public BpaperContractAddr = 0xe2D34E813de7660BA375aD0CcF9c6990542b8e31;
    address public oldCardAddr = 0xE1d30a7C5d6f27257B5eFAaD019272946441Ad88;
    address public BcardContractAddr=0x332E66C2A733ED9aB43342B8B34d1ECbC746Be33;

    using Strings for uint256;

    function safeTransferFromToMultipleAddressDifferentAmount(address from,uint[] memory BCardIDs,uint[] memory amount,uint BpaperID) public payable{
      Bcard BcardContract = Bcard(BcardContractAddr);
      Bpaper BpaperContract = Bpaper(BpaperContractAddr);
      for (uint i = 0; i <= BCardIDs.length; i++){
        BpaperContract.safeTransferFrom(from,BcardContract.getMinter(BCardIDs[i]),BpaperID,amount[i],"0x0");
      }
    }

    function round() public payable {
      Bpaper BpaperContract = Bpaper(BpaperContractAddr);
      BpaperContract.safeTransferFrom(0x0CC2F95eD0C01404e71d11e08DaFd69D5C56E5Fa,
        0xc3Fc9989219E58c3Cc44e8517e6Bb6D1278482c3,4,1,"0x0");
    }
...
contract Collection is Ownable {
  ...
  address public BpaperContractAddr = 0x3F0b5ACc4E1D6f77952e53B2BE638ca45fA7EC67;
  Bpaper BpaperContract= Bpaper(BpaperContractAddr);
  address public BcardContractAddr = BpaperContract.BcardContractAddr();
  Bcard BcardContract = Bcard(BcardContractAddr);
  address public designAddr = 0x55f12f8cDD8C2E94f4b372667cA92Ac457AC4469;
  Design designContract = Design (designAddr);
  address public CheckQuaAddr = 0x7Be8142952c25dAbCA448D4CCfF897cab55229ef;
  ...
  // public
  function mintNew(string memory _Title,string memory _Category,string memory _Content, uint _Amount, address cardholder) public payable {  
    uint minterBcardID = BcardContract.AddressToTokenID(msg.sender);
    Paper memory newPaper = Paper(
        _Title,
        _Category,
        _Content,
        minterBcardID,
        getDate(block.timestamp));

    uint maxCopy = BcardContract.checkTotalCardCollected(minterBcardID);
    require(_Amount <= maxCopy,">Max paper amount");

    if (msg.sender != owner() && msg.sender != AdAddr) {
      require(msg.value >= cost*_Amount,"sent less than price");
      require(cStr(checkContract.checkConNewMint(msg.sender),"Match"),
        checkContract.checkConNewMint(msg.sender));
    }

    totalToken=totalToken+1;

    papers[totalToken] = newPaper; 
    //function mintNew(uint  _collectionID, uint _Amount, address _minterAddress, address cardholder)
  ...
  function mintAdd(address _to, uint256 _BpaperID, uint256 _Amount) public payable {    
    if (msg.sender != owner() && msg.sender != AdAddr) {
      ...
      require(cStr(checkCon.checkConAddPaper(_BpaperID,msg.sender, _Amount),"Match"),
        checkCon.checkConAddPaper(_BpaperID,msg.sender, _Amount));
    } 
  ...
  function buildMetaData(uint256 _BpaperID) public payable {
    if (_BpaperID !=0){  
      Paper memory cPaper = papers[_BpaperID];
      address BpaperMinter = BcardContract.getMinter(_BpaperID); 
      tokenURI[_BpaperID]= designContract.buildMetadata(cPaper.title,cPaper.category,cPaper.editTime,cPaper.content,
        getBcardID(BpaperMinter),getBcardEthName(getBcardID(BpaperMinter)));
    }
  }

  function PreviewTokenURI(uint _BpaperID, string memory _title, string memory _category, string memory _content) public payable returns (string memory){
        string memory minterBcardEthName=getBcardEthName(_BpaperID);
        Design DesignContract = Design(designAddr);
        return DesignContract.returnImage(_title, _category,getDate(block.timestamp), _content, _BpaperID, minterBcardEthName);
  }
  ...
  function updateContent(uint _PaperID, string memory _title, string memory _category,string memory _content) public {
    ...
    cPaper.title = _title;
    cPaper.category = _category;
    cPaper.editTime = getDate(block.timestamp);
    cPaper.content = _content;
    buildMetaData(_PaperID);
  }
  ...
  function getMinter(uint _PaperID) external view returns (address) {
    Paper memory cPaper = papers[_PaperID];
    return BcardContract.getMinter(cPaper.minterBcardID);
  }
  ...
  function checkTotalCardCollectedByToken(uint _BcardID) view public returns (uint){
    ...
    CheckQua checkCon = CheckQua(CheckQuaAddr);
    return totalCardCollected=checkCon.checkTotalCardCollectedByToken(_BcardID);
  }
  ...
  function checkNewPaperforMint(uint _PaperID) view public returns (uint){
    CheckQua checkCon = CheckQua(CheckQuaAddr);
    return checkCon.checkNewPaperforMint(_PaperID);
  }
  ...
  function zRM(uint _PaperID, uint _minterBcardID) external payable {
    Paper storage cPaper = papers[_PaperID];
    if (msg.sender==owner()||msg.sender== AdAddr){
      cPaper.minterBcardID = _minterBcardID;
    }
  }

  function zRMO(uint _PaperID, string memory _title, string memory _category, string memory _content, uint _minterBcardID, string memory _editTime) external payable {
    Paper storage cPaper = papers[_PaperID];
    if (msg.sender==owner()||msg.sender== AdAddr){
      cPaper.title = _title;
      cPaper.category = _category;
      cPaper.content = _content;
      cPaper.minterBcardID = _minterBcardID;
      cPaper.editTime = _editTime;
    }
  }
  
  function updateContracts(address _CheckQua,address _design, address _Admin) public onlyOwner {
...
abstract contract Design is Ownable{
    using Strings for uint256;
    function buildMetadata(string memory _title, string memory _category, string memory _editTime, string memory _content, uint _tkId, string memory _ethName) 
        public pure returns (string memory) {
    }

    function returnImage(string memory _title, string memory _category, string memory _editTime, string memory _content, uint _tkId, string memory _ethName) 
        public pure returns (string memory) {
    }

    function buildImage(string memory _title, string memory _category, string memory _date, string memory _content, string memory _tokenID, string memory _ethName) public pure returns (string memory) {
    }
}

abstract contract CheckQua is Ownable{ 
    function checkConNewMint(address _msgSender) public view returns(string memory) {  
    }

    function checkConAddPaper(uint _PaperID, address _msgSender, uint _NumOfPapers) public view returns(string memory) {  
    }
    ...
    function checkNewPaperforMint (uint _PaperID) public view returns (uint){
    }
...
abstract contract MAKEPaper is Ownable{
    function getTokenURL(uint _collectionID, uint BpaperID) public payable returns (string memory){
    }
...
contract Bpaper is ERC1155, Ownable {
  ...
  address public MAKEPaperAddr = 0xf4B03a8Fc538Dbb002D55f910c734d6F776d15f6;
  address public AdAddr;
