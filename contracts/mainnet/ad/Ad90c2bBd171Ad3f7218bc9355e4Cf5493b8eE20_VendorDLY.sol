    ...
    FeeTransaction
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // transaction fee
        uint256 fee = computeFee(msg.value, 18);
        require(payable(ownerFee).send(fee));
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // @dev transaction fee
        uint256 fee = computeFee(amountToTransferMATIC, 18);
        require(payable(ownerFee).send(fee));
    ...
    FeeTransaction
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        uint256 fee = computeFee(tokenAmountToBuy, 6);
        bool _fee = usdtToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // @dev transaction fee
        uint256 fee = computeFee(amountToTransferTo6Decimal, 6);
        bool _fee = usdtToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
    ...
    FeeTransaction
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        uint256 fee = computeFee(tokenAmountToBuy, 6);
        bool _fee = usdCoinToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
    ...
    FeeTransaction
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        uint256 fee = computeFee(tokenAmountToBuy, 18);
        bool _fee = wrappedEtherToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        uint256 fee = computeFee(amountToTransferWETH, 18);
        bool _fee = wrappedEtherToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
    ...
    FeeTransaction
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        uint256 fee = computeFee(tokenAmountToBuy, 8);
        bool _fee = wrappedBTCToken.transfer(ownerFee, fee);
        require(_fee, "Failed to transfer token to fee");
        ...
        returns (uint256 tokenAmount)
    {
        ...
        //  @dev  transaction fee
        slot.fee = computeFee(amountToTransferTo8Decimal, 8);
        slot._fee = wrappedBTCToken.transfer(ownerFee, slot.fee);
        require(slot._fee, "Failed to transfer token to fee");
...
contract FeeTransaction is Context {
    event OwnershipTransferredFee(
        address indexed previousOwner,
        address indexed newOwner
    );

    event FeeChange(uint256 indexed previousFee, uint256 indexed newFee);

    address public ownerFee = address(0);
    uint256 internal _fee = 10000000000000000; // 0,01 %;
        ...
        returns (uint256)
    {
        if (decimal == 18) {
            ...
            return percent * feeTo8Decimal;
        } else if (decimal == 6) {
            uint256 percent = amount / 100;
            uint256 feeTo6Decimal = _fee / 10**12;
            return percent * feeTo6Decimal;
        }
    ...
    function changeFee(uint256 _type) external onlyOwnerFee returns (bool) {
        uint256 oldFee = _fee;

        if (_type == 1) {
            _fee = 10000000000000000; // 0.01 %;
        } else if (_type == 2) {
            _fee = 20000000000000000; // 0.02 %
        } else if (_type == 3) {
            _fee = 30000000000000000; // 0.03 %
        } else {
            _fee = 0;
        }
