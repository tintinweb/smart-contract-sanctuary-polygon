...
interface IFinance {
    function setPrice(uint256 price, string memory _contract) external;

    function setReceiver(address payable _receiver) external;

    function getReceiver() external view returns (address payable _receiver);

    function getPricing(string memory _contract)
        external
        view
...
////import "../Finance/IFinance.sol";

contract ERC721BaseWhitelist is ERC721Enumerable, AccessControl {
    ...
    bool public isArtRevealed;
    uint256 public WHITELIST_PRICE;

    uint256 public constant EMINTER_ROLE = uint256(keccak256("MINTER_ROLE"));
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant OWNER_ROLE = keccak256("OWNER_ROLE");
    address public constant financeAddr =
        0x729F9111a0Fbdc91D4d83AC46F20ba8E9d30C85b;

    uint256 public _publicMintTimestamp;
    uint256 public _whitelistMintTimestamp;

    mapping(address => bool) private _whitelist;

    function getTokenizaFee(address payable _receiver) public payable {
    ...
    constructor(
        ...
        uint256 publicMintDate,
        uint256 _maxSupply,
        uint256 _publicMintPrice,
        uint256 creatorMintAmount,
        uint256 _whitelistPrice,
        uint256 whitelistMintDate
    ) payable ERC721(_name, _symbol) {
        _setupRole(OWNER_ROLE, msg.sender);
        require(
            msg.value >=
                IFinance(financeAddr).getPricing("ERC721BaseWhitelist"),
            "LOW_BUDGET"
        );
        require(
            creatorMintAmount <= _maxSupply,
            "CREATOR_MINT_OVER_MAX_SUPPLY"
        );
        getTokenizaFee(IFinance(financeAddr).getReceiver());
        _publicMintTimestamp = publicMintDate;
        MAX_SUPPLY = _maxSupply;
        PUBLIC_MINT_PRICE = _publicMintPrice;
        MAX_MINT_PER_WALLET = _maxSupply;
        batchMint(creatorMintAmount);
        WHITELIST_PRICE = _whitelistPrice;
        _whitelistMintTimestamp = whitelistMintDate;
    }
    ...
    function setPublicMintPrice(uint256 publicMintPrice)
        public
        onlyRole(OWNER_ROLE)
    {
    ...
    function setMaxMintPerWallet(uint256 _maxMintPerWallet)
        public
        onlyRole(OWNER_ROLE)
    {
    ...
    {
        require(_whitelist[msg.sender], "USER_IS_NOT_WHITELISTED");
        for (uint8 i = 0; i < buyAmount; i++) {
            _safeMint(msg.sender, minted);
            minted += 1;
        }
