...
// File: contracts/LibERC20Token.sol
...
contract CurveModule is ISlingshotModule {
    ...
    int128 i = int128(int256(_i));
    int128 j = int128(int256(_j));

    // IERC20(_poolTokens[_i]).approve(_poolAddress, _amountIn);
    
    uint256 _amountIn = IERC20(_poolTokens[_i]).balanceOf(address(this));
    IERC20(_poolTokens[_i]).approveIfBelow(_poolAddress, _amountIn);
    // TransferHelper.safeTransfer(coins[_j], msg.sender, _amountIn);
    // dx = IERC20(iToken).balanceOf(address(this));
    


    ICurvePool(_poolAddress).exchange_underlying(0, 1, _amountIn, _dy );
