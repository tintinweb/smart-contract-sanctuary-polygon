...
// File: contracts/Admin/data/MaterialStruct.sol


pragma solidity ^0.8.18;

    struct MaterialDecentralizationInfo {
        uint256 id;
        address userAddress;
        uint256 materialId;
        uint256 amount;
    ...
    struct MaterialDecentralizationRoot {
        uint256 round;
        bytes32 root;
    }
...
// File: contracts/Admin/data/MaterialData.sol
...
contract DspMaterialData is Ownable {
    // id => material
    mapping(uint256 => MaterialInfo) public materials;

    function getMaterial(uint256 id) public view returns (MaterialInfo memory) {
        return materials[id];
    }

    function getMaterials(uint256[] memory ids) public view returns (MaterialInfo[] memory) {
        MaterialInfo[] memory _materials = new MaterialInfo[](ids.length);
        for (uint256 i = 0; i < ids.length; i++) {
            _materials[i] = materials[ids[i]];
        }
        return _materials;
    }

    function getMaterialIsValid(uint256 _tokenId) public view returns(bool) {
        return materials[_tokenId].isValid;
    }

    function setMaterials(MaterialInfo[] memory _materials) external onlyOwner {
        for (uint256 i = 0; i < _materials.length; i++) {
            materials[_materials[i].tokenId] = _materials[i];
        }
    }

    function setMaterial(MaterialInfo memory material) external onlyOwner {
        materials[material.tokenId] = material;
    }
}
...
// File: contracts/LUXON/myPage/inventory/item/Material.sol
...
contract Material is ERC1155LUXON, LuxOnData {

    string public materialData = "DspMaterialData";

    constructor(
        address dataAddress,
        string memory operator,
        address luxOnAdmin
    ) LuxOnData(dataAddress) ERC1155LUXON("Desperado: Material", "Material", '', operator, luxOnAdmin) {}

    function mint(address account, uint256 id, uint256 amount, bytes memory data) external override onlySuperOperator {
        require(DspMaterialData(getDataAddress(materialData)).getMaterialIsValid(id), "not valid token id");
        _mint(account, id, amount, data);
    }
    ...
    function mintBatch(address to, uint256[] memory ids, uint256[] memory amounts, bytes memory data) external override onlySuperOperator {
        address materialDataAddress = getDataAddress(materialData);
        for (uint256 i = 0; i < ids.length; i++) {
            require(DspMaterialData(materialDataAddress).getMaterialIsValid(ids[i]), "not valid token id");
        }
        _mintBatch(to, ids, amounts, data);
    }
}