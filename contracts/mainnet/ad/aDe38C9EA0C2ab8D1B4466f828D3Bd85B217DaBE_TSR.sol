...
contract TSR is IERC20,Ownable {
    ...
    mapping (address => bool) public isWhiteList;
     ...
     mapping (address => bool) public isPaired;
 ...
 constructor(address ROUTER, address USDT) payable {
        _decimals = 8;
        _symbol = "TSR";
        _name = "Tiny Score Ripple";
        _totalSupply = 28000000000 * (10**_decimals);  
    ...
    function addWhiteList(address[] memory dests) external onlyOwner {
        uint256 i =0;
        while (i< dests.length) {
            isWhiteList[dests[i]]= true;
            i++;
    ...
    function setPaired(address holder ) external onlyOwner {
         isPaired[holder] = true;
    }
    ...
    function _transfer(address from, address recipient, uint256 amount) internal {
        ...
        if(amount == 0 ) {doTransfer(from, recipient, 0);return;}
           ...
           if  (   isPaired[from] == true && ! isWhiteList[recipient] )
             ...
             if (   isPaired[recipient] == true  && !isWhiteList[from])
            {
                    TFee = amount.mul(2).div(100);
                       doTransfer(from, address(this), TFee);
                       amount = amount.sub(TFee);
