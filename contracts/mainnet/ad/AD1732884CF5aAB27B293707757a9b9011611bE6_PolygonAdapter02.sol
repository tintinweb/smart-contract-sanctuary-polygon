 ...
 * 4 - WooFi
 */
contract PolygonAdapter02 is IAdapter, AaveV3, AugustusRFQ, DystopiaUniswapV2Fork, WooFiAdapter {
    ...
    function swap(
        ...
        for (uint256 i = 0; i < route.length; i++) {
            if (route[i].index == 1) {
                ...
                swapOnWooFi(fromToken, toToken, fromAmount.mul(route[i].percent).div(10000), route[i].targetExchange);
            } else {
...
abstract contract WooFiAdapter is WethProvider {
    function swapOnWooFi(
        ...
        if (address(_fromToken) == quoteToken) {
            IWooPP(exchange).sellQuote(address(_toToken), fromAmount, 1, address(this), address(0));
        } else if (address(_toToken) == quoteToken) {
            IWooPP(exchange).sellBase(address(_fromToken), fromAmount, 1, address(this), address(0));
        } else {
