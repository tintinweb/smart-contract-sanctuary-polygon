...
contract SiloFactoryUIHelper is Ownable{
    ...
    function showAllBalances(uint siloId) external view returns(address[] memory, uint[] memory){
        ISilo silo = ISilo(SiloFactory.siloMap(siloId));
        IAction action;
        (address[] memory actions, bytes[] memory configData) = silo.viewStrategy();
        ...
        address[] memory tokens = new address[](index);
        uint[] memory balances = new uint[](index);
