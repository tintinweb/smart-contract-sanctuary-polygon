...
contract USD_FORSAGE is Ownable {
    ...
    function buyNewLevel(uint8 matrix, uint8 level) external {
      ...
      if (matrix == 2){
            ...
            require(!users[msg.sender].x6Matrix[level].buyslot, "this level upgrade movement"); 
    ...
    function buyNewLevelAutoUpgrade(uint8 matrix, uint8 level) external {
      ...
      if (matrix == 2){
            ...
            require(users[msg.sender].x6Matrix[level].buyslot, "this level upgrade movement"); 
            require(users[msg.sender].x6Matrix[level].buylevel == level, "this level Not upgrade movement"); 
    ...
    function updateX6ReferrerSecondLevel(address userAddress, address referrerAddress, uint8 level, bool pay) private {
        ...
        && (users[referrerAddress].x6Matrix[level].reinvestCount==0)){
              users[referrerAddress].x6Matrix[level+1].buyslot=true;
              users[referrerAddress].x6Matrix[level+1].buylevel = level+1;

             if(users[referrerAddress].x6Matrix[level].secondLevelReferrals.length == 3){
                users[referrerAddress].x6Matrix[level+1].buyslot=false;
             }
    ...
    function sendETHDividends(address userAddress, address _from, uint8 matrix, uint8 level, bool pay) private {
       ...
       if(pay==false)
       {
            ...
            if (!tokenUSDC.transferFrom(address(this), receiver, levelPrice[level])) {
            tokenUSDC.transferFrom(address(this), owner, levelPrice[level]);
        }
       }
