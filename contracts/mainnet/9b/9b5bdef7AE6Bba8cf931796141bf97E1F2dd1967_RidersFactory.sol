...
contract RidersFactory is VRFConsumerBaseV2, Ownable  
{
  ...
  function purchase(uint8 mintingPack, string memory referral) external payable {
    ...
    require(sent, "Collection Riders: rdk transfer failed");
    ...
    _requestRandomWords(tokenQuantity * 3);
  }
  ...
  function mint() external {
    ...
    for(uint i = 0; i < _randomWordsLength; i += 3) {
      ...
      (string memory currentBaseURI, string memory currentImageURI) = radikalStoreInstance.getRidersURIs(attributes.pizzaQuantity);
      ...
      eventAttributes[((_randomWordsLength - i)/3) - 1] = _mint(msg.sender, (_randomWords[i + 2]), motorBike, _tokenURI, _imageURI, attributes);
    }
    emit Minted(msg.sender, eventAttributes);
  ...
  function _mint(address user, uint _randonNumber3, bytes12 _motorBike, string memory _tokenURI, string memory _imageURI, IRadikalStore.RiderAttributes memory attributes) internal returns(ICollectionRiders.RidersAttributes memory riderAttributes){
    ...
    // ICollectionRiders.RidersAttributes memory 
