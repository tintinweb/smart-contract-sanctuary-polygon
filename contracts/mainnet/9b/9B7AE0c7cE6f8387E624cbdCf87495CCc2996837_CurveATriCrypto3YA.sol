...
contract CurveATriCrypto3YA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant deductEarningsFee() {
    require(req.spender == address(this));
  ...
  function postOwnerUpgrade() public onlyOwner {
    uint256 balance = IERC20(CRV_USDBTCETH_TOKEN).balanceOf(address(this));
    if (!IERC20(CRV_USDBTCETH_TOKEN).approve(REWARD_POOL, balance)) {
      revert();
    }
    IRewardsGauge(REWARD_POOL).deposit(balance);
  }
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) private returns (uint256) {
    if (fap > 0) {
      GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, feeAccount, fap);
    }
    return amount - fap;
  }
  ...
  modifier deductEarningsFee() {
    ...
    if (earnings > 0) {
      uint256 feeTri = earnings * EARNINGS_FEE / BASE_PERCENT;
      ...
      if (pendingTriFees >= COMPOUND_TRI_THRESHOLD) {
        ...
        if (!IERC20(USDC_TOKEN).transfer(feeAccount, IERC20(USDC_TOKEN).balanceOf(address(this)))) {
          revert();
        }
        pendingTriFees = 0;
      }
      ...
      if (value >= COMPOUND_CRV_THRESHOLD) {
        ...
        value = deductFeeUsdc(value, value * EARNINGS_FEE / BASE_PERCENT);
...
interface ICurveCryptoFiveWaySwap {
  function add_liquidity(uint256[5] calldata _amounts, uint256 _min_mint_amount) external;
  function remove_liquidity(uint256 _amount, uint256[5] calldata _min_amounts) external;
  function remove_liquidity_one_coin(uint256 _token_amount, uint256 i, uint256 _min_amount) external;
  function calc_token_amount(uint256[5] calldata _amounts, bool _is_deposit) external view returns ( uint256 );
  function calc_withdraw_one_coin(uint256 token_amount, uint256 i) external view returns ( uint256 );
}

interface ICurveCryptoFourWaySwap {
  function add_liquidity(uint256[4] calldata _amounts, uint256 _min_mint_amount) external;
  function remove_liquidity(uint256 _amount, uint256[4] calldata _min_amounts) external;
  function remove_liquidity_one_coin(uint256 _token_amount, uint256 i, uint256 _min_amount) external;
  function calc_token_amount(uint256[4] calldata _amounts) external view returns ( uint256 );
  function calc_withdraw_one_coin(uint256 token_amount, uint256 i) external view returns ( uint256 );
}
