...
contract g_Power is owned {
    ...
    function withdrawValue(address _user, uint _poolType,uint upToIndex) public view returns(uint) {
        uint lpi = lastPoolIndex[_poolType];
        investInfo memory temp = investInfos[_user][_poolType];
    ...
    function withdrawFounderValue(address _user, uint upToIndex) public view returns(uint) {
        uint _poolType = 5;
        uint lpi = founderLastPoolIndex;
        investInfo memory temp = investInfos[_user][_poolType];
