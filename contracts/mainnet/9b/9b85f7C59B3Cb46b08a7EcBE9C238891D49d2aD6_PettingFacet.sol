...
import {vGHST} from "../../vGHST.sol";
import "@openzeppelin/contracts-upgradeable/security/PausableUpgradeable.sol";

interface gotchiWorld{
...
contract PettingFacet is Modifiers{
    function migrate() public whenNotPaused onlyApproved{

        //local variables -- only live in the scope of this function
        address oriumContract =  0xb4Cd476E1aFb22089A35a1102Eae208F15eCDc79;
        address fireballPetter = 0xe3601aa8774CD35Bd07ddeFa8aA1b893bd0B9ED3;
        address fireballContract = 0x72EAd8783cd9024161DE7BA91e0E9FB552652739;
        address gotchiworldAddress = 0x8c41492CD9F15D0cA22003aEcb4B00d02E4ce8e6;

        //set pet operator for fireball.gg autopet and orium autopet
        //orium
        IAavegotchi(im_diamondAddress).setPetOperatorForAll(oriumContract,true);
        //fireball
        IAavegotchi(im_diamondAddress).setPetOperatorForAll(fireballPetter,true);

        //withdraw GHST from Ceza's bot
        gotchiWorld(gotchiworldAddress).unstakeGhst();

        //approve fireball.gg to take GHST
        GHST(im_ghstAddress).approve(fireballContract,100 ether);

        //turn on autopet for fireball.gg
        fireball(fireballContract).subscribe();

        //turn on autopet for orium
        orium(oriumContract).enablePetOperator();
    }
}
