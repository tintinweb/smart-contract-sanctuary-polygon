...
contract RwardOneRandomCollection is ERC721r, EIP2981PerTokenRoyalties, Ownable {
  using Counters for Counters.Counter;
  Counters.Counter private _totalCount;
  bool public isMintEnabled;
  string private tokenBaseUri;
  string private collectionID;
  uint256 private limitPerWallet;
  address private admin;

  //events
  event MintEvent(uint256 indexed tokenID, string collID, string message);
  ...
  constructor(string memory _name, string memory _symbol, string memory _baseUri, uint256 _supply, uint256 _limitPerWallet, string memory _collectionID) ERC721r(_name, _symbol, _supply) {
  ...
  function _enableMinting() private {
  ...
  function replaceAdmin(address _newAdmin) external onlyAdmin {
    ...
    require(admin != _newAdmin, 'New admin is the same as the current admin');
    _replaceAdmin(_newAdmin);
  }
  
  function mintToken(address nftOwner) external payable onlyOwner returns (uint256) {
    require(isMintEnabled, "Minting is not open.");
    require(!(limitPerWallet >= 1) || balanceOf(nftOwner) < limitPerWallet, "Limit per wallet reached");


    uint256[] memory newTokenIDs = _mintRandom(msg.sender, 1);
    uint256 newTokenID = newTokenIDs[0];
    _setTokenRoyalty(newTokenID, owner());

    emit MintEvent(newTokenID, collectionID, "An NFT was minted successfully");
...
abstract contract EIP2981PerTokenRoyalties is IERC2981  {
    // The constant value for royalty.
    uint256 private constant _FIXED_ROYALTY = 750;
    ...
    function _setTokenRoyalty(
        ...
        _royalties[tokenId] = RoyaltyInfo(recipient, uint24(_FIXED_ROYALTY));
    }
