...
contract StarNFTV4 is Ownable, ERC165, IERC721, IERC721Metadata, IStarNFT {
    ...
    */
    ...
    function transferFrom(
        ...
        address to,
        uint256 tokenId
    ) public override onlyTransferable {
    ...
    * This internal function is equivalent to {safeTransferFrom}, and can be used to
    e.g.
    * implement alternative mechanisms to perform token transfer, such as signaturebased.
    *
    * Requirements:
    *
    * - `from` cannot be the zero address.
    * - `to` cannot be the zero address.
    * - `tokenId` token must exist and be owned by `from`.
    * - If `to` refers to a smart contract, it must implement {IERC721ReceiveronERC721Received}, which is called upon a safe transfer.
    *
    * Emits a {Transfer} event.
    */
