...
contract gammaUWZap {
    ...
    function goOut (address _vault, uint256 withdrawAmount) external { //TODO: WITHDRAW TWO TOKENS ?
        ...
        if (pair.token0() != WETH && pair.token1() != WETH) { //TODO: CHECK ThiS CONDITION
            return _removeLiqudity(address(pair), msg.sender, withdrawAmount);
        }
        _removeLiqudity(address(pair), address(this), withdrawAmount);
    ...
    function _removeLiqudity(address pair, address to, uint withdrawAmount) private {
        // IERC20(pair).safeTransfer(pair, IERC20(pair).balanceOf(address(this)));
        // (uint256 amount0, uint256 amount1) = IUniswapV2Pair(pair).burn(to);
        uint256[4] memory minIn = [uint256(0), uint256(0), uint256(0), uint256(0)];
        IMasterChef(pair).withdraw(withdrawAmount, msg.sender, address(this), minIn);
    ...
    function _swapAndStake(address _vault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        (uint256 amountNeededUsdcMin,)= routerLiq.getDepositAmount(address(pair), path[1], swapedAmounts[1]);

        _approveTokenIfNeeded(path[0], address(pair)); 
        _approveTokenIfNeeded(path[1], address(pair)); 
        uint256[4] memory minIn = [uint256(0), uint256(0), uint256(0), uint256(0)];
        uint256 amountLiquidity = routerLiq
              .deposit(amountNeededUsdcMin, swapedAmounts[1], address(this), address(pair), minIn);
