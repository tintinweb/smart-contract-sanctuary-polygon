...
contract BifiMaxiYA is BaseYA, UniswapV2Giddy, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  uint256 constant private COMPOUND_THRESHOLD = 10e18;
  address constant private BIFI_TOKEN = 0xFbdd194376de19a88118e84E279b977f165d01b8;
  address constant private BEEFY_REWARD_POOL = 0xDeB0a777ba6f59C78c654B8c92F80238c8002DD2;
  ...
  function getContractBalance() public view override returns (uint256) {
    return IBeefyRewardPool(BEEFY_REWARD_POOL).balanceOf(address(this));
  }

  function getContractRewards() public view override returns (uint256) {
    return IBeefyRewardPool(BEEFY_REWARD_POOL).earned(address(this)) + IERC20(WMATIC_TOKEN).balanceOf(address(this));
  }
  ...
  function compound() public override whenNotPaused {
    IBeefyRewardPool(BEEFY_REWARD_POOL).getReward();
    uint256 earnings = IERC20(WMATIC_TOKEN).balanceOf(address(this));
    if (earnings > COMPOUND_THRESHOLD) {
      earnings = deductFeeWmatic(earnings, earnings * GIDDY_CONFIG.earningsFee() / BASE_PERCENT);

      address[] memory route = new address[](3);
      route[0] = WMATIC_TOKEN;
      route[1] = USDC_TOKEN;
      route[2] = BIFI_TOKEN;
      earnings = swapTokens(SUSHI_ROUTER, route, GIDDY_CONFIG.feeAccount(), earnings);

      emit Compound(earnings, lastCompoundTime, getContractBalance());
      if (!IERC20(BIFI_TOKEN).approve(BEEFY_REWARD_POOL, earnings)) {
        revert();
      }
      IBeefyRewardPool(BEEFY_REWARD_POOL).stake(earnings);
      lastCompoundTime = block.timestamp;
    }
  }
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[0] = USDC_TOKEN;
    route[1] = BIFI_TOKEN;
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, route, value - fap);
  }
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    route[0] = USDC_TOKEN;
    route[1] = BIFI_TOKEN;
    uint256 bifiValue = swapTokens(SUSHI_ROUTER, route, address(this), value);

    uint256 shares = contractShares == 0 ? bifiValue * INIT_SHARES : bifiValue * contractShares / getContractBalance();
    userShares[owner] += shares;
    contractShares += shares;

    if (!IERC20(BIFI_TOKEN).approve(BEEFY_REWARD_POOL, bifiValue)) {
        revert();
    }
    IBeefyRewardPool(BEEFY_REWARD_POOL).stake(bifiValue);
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    route[1] = USDC_TOKEN;
    route[2] = BIFI_TOKEN;
    value = swapTokens(SUSHI_ROUTER, route, address(this), value);

    uint256 shares = contractShares == 0 ? value * INIT_SHARES : value * contractShares / getContractBalance();
    userShares[req.owner] += shares;
    contractShares += shares;

    if (!IERC20(BIFI_TOKEN).approve(BEEFY_REWARD_POOL, value)) {
        revert();
    }
    IBeefyRewardPool(BEEFY_REWARD_POOL).stake(value);
  
    emit Deposit(req.owner, giddyToUsdc(value));
  }
  ...
  function withdrawCalc(uint256 shares, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[0] = BIFI_TOKEN;
    route[1] = USDC_TOKEN;
    (amountOut, priceImpact) = calcPrice(SUSHI_ROUTER, route, sharesToValue(shares));
    amountOut -= fap;
  }
  ...
  function withdraw(uint256 shares, uint256 fap) external override whenNotPaused nonReentrant{
    ...
    IBeefyRewardPool(BEEFY_REWARD_POOL).withdraw(value);
  ...
  function deductFeeWmatic(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      address[] memory route = new address[](2);
      route[0] = WMATIC_TOKEN;
      route[1] = USDC_TOKEN;
