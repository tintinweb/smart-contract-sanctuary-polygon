...
contract SmartbidNFTExclusive is  Ownable{
    ...
    function getUri(uint256 pid, uint256 tokenOwned) public view returns(bool WIHave, string memory tok, string memory tokS) {
        ...
        uint256 lent = info.URIRequireFromNFTs.length;
        for (uint256 i; i < lent;) {
            string memory tokenSet = IERC1155(info.NFTContractAddress).setUri(info.URIRequireFromNFTs[i]);
            if (keccak256(abi.encodePacked(tokenSet)) == keccak256(abi.encodePacked(token))) {
                WIHave = true;
                tok = token;
                tokS = tokenSet;
                break;
    ...
    function DistributeRewardsWithOther3(uint256 pid) public {
        ...
        for (uint256 j = 0; j < temporaryOtherLast3Address.length; j++) {
            ...
            emit luckyWinner(wallet, pid, forRandUser, block.timestamp);
        }        
    ...
    function distributeRewardsWithRandomness(uint256 pid) public returns (uint256[] memory expandedValues) {
        ...
        for (uint256 i = 0; i < bid.numberOfRandomAddress; i++) {
            ...
            emit luckyWinner(_wallet, pid, forRandUser, block.timestamp);            
        }     
    ...
    function getSetURI(uint256 indexedOfID) external view returns(uint256[] memory) {
        return (request_token_info[indexedOfID].URIRequireFromNFTs);
    }
