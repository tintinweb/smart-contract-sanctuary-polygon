...
contract UniV3Rebalancer is OwnableUpgradeable, IUniV3Rebalancer {
  ...
  function thresholdRebalancePools() public onlyBenevolent {
    ...
    for (uint256 i = 0; i < dysonPools.length; i++) {
      ...
      dysonPool.tickLowerThreshold = _lower + ((currentTick - _lower) * (THRESHOLD_MULTI_MAX - thresholdMulti));
      dysonPool.tickUpperThreshold = _upper - ((_upper - currentTick) * (THRESHOLD_MULTI_MAX - thresholdMulti));
