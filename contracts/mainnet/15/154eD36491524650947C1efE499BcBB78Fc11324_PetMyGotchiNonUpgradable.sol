...
import "./interfaces/IAavegotchiFacet.sol";

contract PetMyGotchiNonUpgradable is Ownable, Multicall{
    ...
    uint256 public maxPetAtOnce;
    uint256 public maxLentPetCount; // To get the maximum number of listings
    ...
    event ParentAdded(address _parent, bool _custom);
    event ParentRemoved(address _parent, bool _custom);
    event ParentBanned(address _parent);
    event ParentUnbanned(address _parent);
    event PricePerPetPerDayUpdated(uint256 _pricePerPetPerDay);
    event PricePerPetPerDayForParentUpdated(uint256 _pricePerPetPerDay);
    event BalanceDeposited(address _parent, uint256 _amount);
    event BalanceWithdrawn(address _parent, uint256 _amount);
    event FeeCharged(uint256 _amount);
    ...
    function getPricePerPetPerDayForParent(address _parent) public view returns(uint256){
        uint _pricePerPetPerDay = pricePerPetPerDay;
        uint _parentPricePerPetPerDay = parent_pricePerPetPerDay[_parent];

        if(_parent == owner()){
            _pricePerPetPerDay = 0;
        }
        else if(_parentPricePerPetPerDay > 0 && _pricePerPetPerDay > _parentPricePerPetPerDay){
            _pricePerPetPerDay = _parentPricePerPetPerDay;
        }
    ...
    function setPricePerPetPerDay(uint256 _pricePerPetPerDay) public onlyOwner{
        ...
        emit PricePerPetPerDayUpdated(_pricePerPetPerDay);
    }
    ...
    function setCustomPricePerPetPerDayForParent(address _parent, uint256 _pricePerPetPerDay) public onlyOwner{
        ...
        emit PricePerPetPerDayForParentUpdated(_pricePerPetPerDay);
    }
    ...
    function addParent() external payable{
        require(! EnumerableSet.contains(parents, _msgSender()), "Already exists!");
        require(! EnumerableSet.contains(banned_parents, _msgSender()), "Banned!");

        uint256 _pricePerPetPerDay = getPricePerPetPerDayForParent(_msgSender());

        require(parent_balance[_msgSender()] >= _pricePerPetPerDay || msg.value >= _pricePerPetPerDay, "Please send some funds!");

        EnumerableSet.add(parents, _msgSender());
        parent_balance[_msgSender()] += msg.value;

        emit ParentAdded(_msgSender(), false);
    }
    ...
    function addCustomParent(address _parent) public onlyOwner payable{
        ...
        emit ParentAdded(_parent, true);
    }

    function removeParent() public {
        require(EnumerableSet.contains(parents, _msgSender()), "Does not exists!");

        EnumerableSet.remove(parents, _msgSender());
        withdrawBalanceFor(_msgSender());

        emit ParentRemoved(_msgSender(), false);
    }
    ...
    function removeCustomParent(address _parent) public onlyOwner{
        ...
        emit ParentRemoved(_parent, true);
    }
    ...
    function childrenOf(address _parent) public view returns(uint256[] memory){
        uint256[] memory _childrenIds = new uint256[](countChildrenOf(_parent));
        ...
        for(uint i = 0; i < _ownedIds.length; i++){
            _childrenIds[_counter] = uint256(_ownedIds[i]);
            _counter++;
        }

        for(uint j = 0; j < _lendings.length; j++){
            _childrenIds[_counter] = uint256(_lendings[j].erc721TokenId);
            _counter++;
        }
    ...
    function nextPetTimeForChildrenOf(address _parent) public view returns(uint256){
        uint256[] memory children = childrenOf(_parent);
    ...
    function depositFor(address _parent) public payable{
        ...
        emit BalanceDeposited(_parent, msg.value);
    }

    function whenNextDepositIsRequired() public view returns (uint256){
        return whenNextDepositIsRequiredFor(_msgSender());
    }
    ...
    function whenNextDepositIsRequiredFor(address _parent) public view returns (uint256){
        ...
        uint _pricePerPetPerDay = getPricePerPetPerDayForParent(_parent);
    ...
    function isThisParentOwnsThisChild(address _parent, uint256 _child) public view returns(bool){
        bool status = false;

        if(IAavegotchiFacet(aavegotchiFacetAddress).isAavegotchiLent(uint32(_child))){
            status = IAavegotchiFacet(aavegotchiFacetAddress).getGotchiLendingFromToken(uint32(_child)).lender == _parent;
        }
        else{
    ...
    function getParentOfThisChild(uint256 _child) public view returns(address){
        ...
        return IAavegotchiFacet(aavegotchiFacetAddress).ownerOf(_child);
    }

    function _canInteractWith(uint256 _child, uint256 _balance, uint256 _pricePerPetPerDay, bool _checkPetTime) public view returns(bool){
        bool _canInteract = _child > 0
            && _balance >= _pricePerPetPerDay
            && IAavegotchiFacet(aavegotchiFacetAddress).getAavegotchi(_child).status == 3
            && IAavegotchiFacet(aavegotchiFacetAddress).isPetOperatorForAll(getParentOfThisChild(_child), gelatoContractAddress);

        if(_canInteract && _checkPetTime){
            uint256 _lastInteracted = IAavegotchiFacet(aavegotchiFacetAddress).getAavegotchi(_child).lastInteracted;
    ...
    function countTakingCareOf() public view returns(uint256){
        uint256 _counter = 0;
        ...
        for (uint256 i = 0; i < EnumerableSet.length(parents); i++) {
            ...
            if(parentExists(_parent)){
                uint256[] memory _pets = childrenOf(_parent);
                uint256 _balance = parent_balance[_parent];
                uint256 _pricePerPetPerDay = getPricePerPetPerDayForParent(_parent);

                if(_pets.length > 0){
                    for(uint256 j = 0; j < _pets.length; j++){
                        if (_canInteractWith(_pets[j], _balance, _pricePerPetPerDay, false)) {
                            _balance -= _pricePerPetPerDay;
                            _counter++;
                        }
                    }
    ...
    function takingCareOf() public view returns(uint256[] memory){
        uint256 _counter = countTakingCareOf();
    ...
    function interactable() public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < EnumerableSet.length(parents); i++) {
            ...
            if(_pets.length > 0){
                for(uint256 j = 0; j < _pets.length; j++){
                    if (_canInteractWith(_pets[j], _balance, _pricePerPetPerDay, true)) {
                        _balance -= _pricePerPetPerDay;
                        _counter++;
    ...
    function interactableWithTheChildrenOf(address _parent) public view returns (uint256[] memory) {
        ...
        if(_pets.length > 0){
            ...
            for(uint256 i = 0; i < _pets.length; i++){
                if (_canInteractWith(_pets[i], _balance, _pricePerPetPerDay, true)) {
                    _balance -= _pricePerPetPerDay;
                    _counter++;
                }
            }
    ...
    function countTakingCareOfChildrenOf(address _parent) public view returns (uint256) {
        uint256 _counter = 0;
        ...
        if(parentExists(_parent)){
            ...
            if(_pets.length > 0){
                uint256 _balance = parent_balance[_parent];
                for(uint256 i = 0; i < _pets.length; i++){
                    if (_canInteractWith(_pets[i], _balance, _pricePerPetPerDay, false)) {
                        _balance -= _pricePerPetPerDay;
                        _counter++;
                    }
                }
    ...
    function takingCareOfChildrenOf(address _parent) public view returns (uint256[] memory) {
        uint256 _counter = countTakingCareOfChildrenOf(_parent);
    ...
    function withdraw(uint256 _amount) public {
        withdrawAmountFor(_msgSender(), _amount);
    }
    ...
    function chargeDailyFee(uint256[] calldata ids) external {
        ...
        uint _idsLength = ids.length;

        require(_idsLength > 0, "Invalid call");

        uint _totalFeeCharged = 0;
        ...
        for (uint256 i = 0; i < _idsLength; i++) {
            ...
            if (_pricePerPetPerDay > 0 && parent_balance[_parent] >= _pricePerPetPerDay) {
                ...
                _totalFeeCharged += _pricePerPetPerDay;
            }
        }

        feeEarned += _totalFeeCharged;

        lastFeeChargedAt = block.timestamp;

        emit FeeCharged(_totalFeeCharged);
    }
