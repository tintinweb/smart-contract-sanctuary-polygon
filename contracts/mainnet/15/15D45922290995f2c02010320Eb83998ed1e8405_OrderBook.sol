...
contract OrderBook is ReentrancyGuard, IOrderBook {
    ...
    event CreateDecreaseOrderWithPath(
        address indexed account,
        uint256 orderIndex,
        address[] path
    );
        ...
        )
    {
        ...
        return (order.path[0], order.collateralDelta, order.indexToken, order.sizeDelta, order.isLong, order.triggerPrice, order.triggerAboveThreshold, order.executionFee);
    }
    ...
    function createDecreaseOrder(
        ...
        if (_withdrawETH) {
            require(_path[_path.length - 1] == weth, "OB: invalid _path");
        }

        _createDecreaseOrder(msg.sender, _path, _collateralDelta, _indexToken, _sizeDelta, _isLong, _triggerPrice, _triggerAboveThreshold, _minOut, _withdrawETH);
    }
    ...
    function _createDecreaseOrder(
        ...
        emit CreateDecreaseOrder(_account, _orderIndex, _path[0], _collateralDelta, _indexToken, _sizeDelta, _isLong, _triggerPrice, _triggerAboveThreshold, msg.value);
        if (order.path.length > 1) {
            emit CreateDecreaseOrderWithPath(_account, _orderIndex, _path);
        }
    }
        ...
        if(orderListSize>0){
            ...
            for (
                ...
                if (order.orderType == OrderType.SWAP){
                    ...
                    (uint256 size, , , , , , , ) = IVault(vault).getPosition(decreaseOrder.account, decreaseOrder.path[0], decreaseOrder.indexToken, decreaseOrder.isLong);
                    if(size>0){
