...
contract GammaUsdcGiddyYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function compound() public override whenNotPaused {
    ...
    if (value > COMPOUND_THRESHOLD_GIDDY) {
      ...
      (uint x, uint y) = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).getDepositAmount(USDC_GIDDY_POS, GIDDY_TOKEN, 1000e18);
      x = (x + y) / 2;
      ...
      value = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).deposit(y, x, address(this), USDC_GIDDY_POS, [uint(0), uint(0), uint(0), uint(0)]);
  ...
  function depositCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (uint x, uint y) = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).getDepositAmount(USDC_GIDDY_POS, USDC_TOKEN, 1e6);
    x = (x + y) / 2;
  ...
  function deposit(bytes calldata auth, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    x = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).deposit(x, y, address(this), USDC_GIDDY_POS, [uint(0), uint(0), uint(0), uint(0)]);
  ...
  function depositGiddyCalc(uint256 value, uint256 fap) external view override returns (uint256 amountOut, uint256 priceImpact) {
    value -= fap;
    (uint x, uint y) = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).getDepositAmount(USDC_GIDDY_POS, GIDDY_TOKEN, 1e18);
    x = (x + y) / 2;
  ...
  function depositGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, uint256 fap) external override whenNotPaused nonReentrant {
    ...
    x = IUniProxy(IHypervisor(USDC_GIDDY_POS).whitelistedAddress()).deposit(y, x, address(this), USDC_GIDDY_POS, [uint(0), uint(0), uint(0), uint(0)]);
    if (!IERC20(USDC_GIDDY_POS).approve(MASTER_CHEF, x)) {
      revert("STAKE_APPROVE");
    }
