...
contract ProxyAction is Ownable {
    ILensHub private immutable _lensHub;
    mapping(address => bool) private _isAllowed;

    constructor(address lensHub, address[] memory allowed) {
        _lensHub = ILensHub(lensHub);
        for (uint256 i = 0; i < allowed.length; i++) {
            _isAllowed[allowed[i]] = true;
        }
    }

    modifier onlyAllowed() {
        require(_isAllowed[msg.sender] == true, "not allowed");
        _;
    }
    ...
    function grantAllowance(
        ...
        emit AllowanceGranted(tokenAddress, spender, amount);
    }
    ...
    function toggleAllowed(address allow, bool allowed) external onlyOwner {
        _isAllowed[allow] = allowed;
    }

    function collectFor(
        CollectForRequest[] memory request
    ) external onlyAllowed {
        for (uint256 i = 0; i < request.length; i++) {
            uint256 tokenId = _lensHub.collect(
                request[i].profileId,
                request[i].pubId,
                request[i].data
            );
            address collectNft = _getCollectNft(
                request[i].profileId,
                request[i].pubId
            );
            IERC721(collectNft).safeTransferFrom(
                address(this),
                request[i].mintFor,
                tokenId
            );
    ...
    function _getCollectNft(
        uint256 profileId,
        uint256 pubId
    ) internal view returns (address) {
        DataTypes.PubType pubType = _lensHub.getPubType(profileId, pubId);
        if (pubType == DataTypes.PubType.Mirror) {
            (uint256 pointedProfileId, uint256 pointedPubId) = _lensHub
                .getPubPointer(profileId, pubId);
            return _lensHub.getCollectNFT(pointedProfileId, pointedPubId);
        } else {
            return _lensHub.getCollectNFT(profileId, pubId);
        }
    }

    function followFor(
        uint256[] calldata profileIds,
        address[] calldata mintFor,
        bytes[] calldata datas
    ) external onlyAllowed {
        uint256[] memory tokenIds = _lensHub.follow(profileIds, datas);

        for (uint256 i = 0; i < tokenIds.length; i++) {
            address followNft = _lensHub.getFollowNFT(profileIds[i]);
            IERC721(followNft).safeTransferFrom(
                address(this),
                mintFor[i],
                tokenIds[i]
            );
