...
contract Ryu is ERC721URIStorage, ReentrancyGuard
{
    //Constants
    string private constant NAME = "RYU NFT";
    string private constant SYMBOL = "RYU";
    string private constant TOKEN_URI = "ipfs://QmXTpswzFSVGkBuWKDH3ZB2J6ekHkaDRHS32qQauRsQVJd";
    ...
    uint256 private constant STAKING_BLOCK_RANGE = 10512000;
    uint256 private constant UNIT_WEI = 1000000000000000000;
    uint256 private constant UPDATE_COOLDOWN_BLOCKS = 1200;
	...
	address private constant MANA_TOKEN_ADDRESS = 0xA1c57f48F0Deb89f569dFbE6E2B7f46D33606fD4;
    ...
    ManaToken       private constant MANA_TOKEN = ManaToken(MANA_TOKEN_ADDRESS);    
    ...
    address[] private CRV_MANA_PAIR = [CRV_TOKEN_ADDRESS, MATIC_TOKEN_ADDRESS, MANA_TOKEN_ADDRESS];
    ...
    {
        return MANA_TOKEN.balanceOf(address(this));
    }
    ...
    {
        require(crvAmount > 0, "NFT: CRV amount cannot be 0");
    ...
    function updateRewardsFund() private
    {
        ...
        if (elapsedBlocks > UPDATE_COOLDOWN_BLOCKS)
        {
            ...
            //Swap CRV for MANA
            crvAmount = CRV_TOKEN.balanceOf(address(this));
            
            if (crvAmount > 0)
            {
                address[] memory crvManaPairMemory = CRV_MANA_PAIR;
            
                CRV_TOKEN.approve(QUICKSWAP_ROUTER_ADDRESS, crvAmount);
                QUICKSWAP_ROUTER.swapExactTokensForTokens(crvAmount, 0, crvManaPairMemory, address(this), block.timestamp + SWAP_WAITING_SECONDS);
            }
    ...
    {
        require(_totalSupply < MAX_SUPPLY, "NFT: Max supply is 1000000000");    
    
        DAI_TOKEN.transferFrom(msg.sender, address(this), MINT_PRICE);
        ...
        _setTokenURI(_totalSupply, TOKEN_URI);
        
        //Update staking block
    ...
    function computeUserReward() public view returns (uint256)
    {
    	...
    	uint256 nftAmount = balanceOf(msg.sender);    
        uint256 blocksStaking = computeBlocksStaking();
        uint256 userReward = 0;        
        
        if ((nftAmount > 0) && (blocksStaking > 0))
	    {
		    ...
		    if (nftAmount < _totalSupply)
		        userDepositRatio = nftAmount * UNIT_WEI / _totalSupply;
    ...
    function harvest() public nonReentrant
    {
        ...
        require(nftAmount > 0, "NFT: Nft amount cannot be 0");

        uint256 blocksStaking = computeBlocksStaking();

        require(blocksStaking > HARVEST_COOLDOWN_BLOCKS, "NFT: harvest cooldown in progress");
        ...
        MANA_TOKEN.transfer(msg.sender, userReward);
    }
    
    function getStakingBlock() external view returns (uint256)
    {
        uint256 stakingBlock = _stakingBlock[msg.sender];
    ...
    function computeBlocksStaking() public view returns (uint256)
    {
        ...
        if (nftAmount > 0)
            blocksStaking = block.number - _stakingBlock[msg.sender];
