...
// File: IEurtUsdCurve.sol

interface IEurtUsdCurve {
    ...
    function remove_liquidity(
        uint256 _amount,
        uint256[4] calldata min_mint_amount,
        address _receiver
    ) external;
...
// File: BaseImpulseMultiStrategy.sol
...
contract BaseImpulseMultiStrategy is IImpulseMultiStrategy, AccessControlUpgradeable, ReentrancyGuardUpgradeable {
    ...
    /// @param _router Address of new swap router.
    ...
    // Transfer want tokens autoFarm -> strategy
    function depositInUnderlying(uint256[] calldata _amounts) external override virtual nonReentrant onlyRole(STRATEGIST_ROLE) returns (uint256) {
    ...
    function _validateToken(address _underlying) internal virtual view returns (bool) {
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
...
// File: ImpulseCurveEurtUsd.sol

contract ImpulseCurveEurtUsd is BaseImpulseMultiStrategy {
    using SafeERC20 for IERC20;
    using SafeCast for uint256;
    using SafeCast for int128;
    using SafeCast for int256;

    uint256 public constant UNDERLYING_COUNT = 4;
    ...
    // Transfer want tokens autoFarm -> strategy
    /// @notice Do not receive payment in EURT token.
    function depositInUnderlying(uint256[] calldata _amounts) external override virtual nonReentrant onlyRole(STRATEGIST_ROLE) returns (uint256) {
        require(_amounts[0] == 0, "Cannot deposit in EURT");
        require(_amounts.length == underlyingTokens.length, "deposit, wrong amounts");
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            if (_amounts[i] != 0) {
                IERC20(underlyingTokens[i]).safeTransferFrom(_msgSender(), address(this), _amounts[i]);
    ...
    /// @param _wantAmt Shares amount.
    /**
     * @return Array with 4 elements which represent want tokens price in underlyings.
     * First element is EURT.
     * Second element is DAI amount.
     * Third element us USDC amount.
     * Fourth element is USDT amount.
     */
    function wantPriceInUnderlying(uint256 _wantAmt) public view override returns (uint256[] memory) {
        ...
        address crvPool = IEurtUsdCurve(targetPoolProtocol).pool();

        // Calculate amount of EURT
        result[0] = (_wantAmt * IPool(crvPool).balances(0) / totalSupply);
        // Calculate amounts of stablecoins
        uint256 stableLPAmount = (_wantAmt * IPool(crvPool).balances(1)) / totalSupply;
        address stableLP = IPool(crvPool).coins(1);
        address stableLPPool = address(0x445FE580eF8d70FF569aB36e80c647af338db351); // Address of pool for stableLP token
        uint256 stableLPTotalSupply = IERC20(stableLP).totalSupply();
        for (uint256 i = 0; i < 3; i++) {
            uint256 balance = IPool(stableLPPool).balances(i);
            result[i + 1] = stableLPAmount * balance / stableLPTotalSupply;
        }
    ...
    function wantPriceInUsd(uint256 _wantAmt) external view virtual override returns (uint256) {
        ...
        uint256 wantPrice = 0;
        // Calculate price of EURT
        uint256 eurtPrice = IEurtUsdCurve(targetPoolProtocol).get_dy_underlying(0, 2, 10**6);
        wantPrice += (underlyingAmounts[0] * eurtPrice / 10**6) * 10**12; // UERT has 6 decimals
        // Price of stables
        for (uint256 i = 1; i < UNDERLYING_COUNT; i++) {
    ...
    function _swapAllUnderlyingToWant() internal virtual override returns (uint256) {
        ...
        IEurtUsdCurve(targetPoolProtocol).add_liquidity(amounts, 0);
    ...
    function _swapAllWantTolUnderlying(address _receiver) internal virtual override {
        ...
        if (wantAmount > 0) {
            uint256[UNDERLYING_COUNT] memory amountsAtricrypto;
            IERC20(wantToken).safeApprove(targetPoolProtocol, wantAmount);
            IEurtUsdCurve(targetPoolProtocol).remove_liquidity(wantAmount, amountsAtricrypto, _receiver);
        }
    }
    ...
    function _swapAllWantToOneUnderlying(address _receiver, address _underlying) internal virtual override {
        ...
        if (wantAmount > 0) {
            IERC20(wantToken).safeApprove(targetPoolProtocol, wantAmount);
            IEurtUsdCurve(targetPoolProtocol).remove_liquidity_one_coin(wantAmount, index, 0, _receiver);
        }
    }

    /// @notice Swap rewards tokens to underlying tokens.
    /// @param _rewardAmts Amount of rewards token.
    function _swapRewardsToUnderlings(uint256[] memory _rewardAmts) internal virtual override {
        uint256 u = 2; // Swap rewards to USDC

        for (uint256 i = 0; i < rewardTokens.length; i++) {
            if (_rewardAmts[i] > DUST) {
                rewardTokens[i].safeApprove(router, 0);
                rewardTokens[i].safeApprove(router, _rewardAmts[i]);
                _swapTokens(swapRewardRoutes[address(rewardTokens[i])][address(underlyingTokens[u])], _rewardAmts[i]);
            }
        }
    }

    /// @notice Loop is started from index 1 to exclude EURT. Cannot withdraw in EURT.
    function _validateToken(address _underlying) internal override view returns (bool) {
        for (uint256 i = 1; i < underlyingTokens.length; i++) {
            if (_underlying == address(underlyingTokens[i])) {
                return true;
            }
        }
    ...
    function _verifySetup() internal virtual override {
        ...
        for (uint256 i = 0; i < underlyingTokens.length; i++) {
            ...
            require(IEurtUsdCurve(targetPoolProtocol).underlying_coins(i) == address(underlyingTokens[i]), "Wrong underlying");
        }
    }
}