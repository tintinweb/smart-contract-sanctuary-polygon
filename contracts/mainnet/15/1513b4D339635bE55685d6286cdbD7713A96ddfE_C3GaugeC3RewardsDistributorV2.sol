...
// File: contracts/Staking/C3/IStakingRewardsDualV5.sol
...
// File: contracts/Curve/IC3GaugeC3RewardsDistributor.sol


pragma solidity >=0.6.11;

interface IC3GaugeC3RewardsDistributor {
  function acceptOwnership() external;
...
// File: contracts/Curve/IC3GaugeController.sol
...
// File: contracts/Curve/C3MiddlemanGauge.sol
...
// ======================== C3MiddlemanGauge ========================
// ====================================================================
// Looks at the gauge controller contract and pushes out C3 rewards once
...
contract C3MiddlemanGauge is Owned, ReentrancyGuard {
    using SafeMath for uint256;
...
// File: contracts/Curve/C3GaugeC3RewardsDistributorv2.sol
...
// ==================================================================== 
// Looks at the gauge controller contract and pushes out C3 rewards once
// a week to the gauges (farms) // Allows contract to override rewardRate on staking gauges
// Original pulled from Frax Finance: https://github.com/FraxFinance
// Frax Finance: https://github.com/FraxFinance
// C3GaugeC3RewardsDistributor is a fork of Frax version with variables renamed to C3
//  "'can I copy your homework?'/ 'yeah just change it up a bit so it doesn't look obvious you copied' / 'ok'
...
contract C3GaugeC3RewardsDistributorV2 is Owned, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeERC20 for ERC20;
    ...
    IC3GaugeController public gauge_controller;
    ...
    constructor (
        ...
        gauge_controller = IC3GaugeController(_gauge_controller_address);
    ...
    /* ========== MUTATIVE FUNCTIONS ========== */
    // Callable by curator
    function distributeRewardAndUpdateRate(address gauge_address, uint256 nonC3Rate) public isDistributing nonReentrant onlyByOwnerOrCuratorOrGovernance returns (uint256 weeks_elapsed, uint256 reward_tally) {
        ...
        if (is_middleman[gauge_address]){
            ...
            C3MiddlemanGauge(gauge_address).pullAndBridge(reward_tally);
        }
        else {
            ...
            // Adjust RewardRates accordingly, on per week basis
            uint256 newRewardRate = reward_tally.div(ONE_WEEK);
            IStakingRewardsDualV5(gauge_address).setRewardRates(newRewardRate, nonC3Rate, true );
        }
