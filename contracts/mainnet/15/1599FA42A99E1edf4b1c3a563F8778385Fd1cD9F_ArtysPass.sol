...
contract ArtysPass is ERC721, AccessControl, Pausable, Royalty {
    ...
    function mintNft(
        ...
        if (walletMintCount[_to] + _amount > maxMintPerWallet)
            revert MaxMintWallet();

        // +1 cause we start at id 1
        if (_tokenIdCounter.current() + _amount > maxSupply + 1)
    ...
    function __mint(address _to) internal {
        uint256 tokenId = _tokenIdCounter.current();
        _tokenIdCounter.increment();
        setRoyaltyInfoForToken(tokenId, _to, 500);
    ...
    // we set the royalties to the original minter. 5%
    function setRoyaltyReceiver(
        uint256 _id,
        address _newOwner
    ) public onlyRole(ADMIN_ROLE) {
        setRoyaltyInfoForToken(_id, _newOwner, 500);
    }
