...
contract BerryToken is Context, IERC20, Ownable {
    ...
    // address private _charityWalletAddress =  replacewithcommunitywallet;
    address private _charityWalletAddress = 0x4348Bd9f3ea0B88510F0Ff75028D11EDAbFFaB01;
    address private _taxWalletAddress = 0x4348Bd9f3ea0B88510F0Ff75028D11EDAbFFaB01;
                                             
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 15000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;

    string private _name = "Berry";
    ...
    function setTaxAddress(address _account) external onlyOwner() {
        require (_account != address(0), "Tax fee address couln't be address(0)");
        _taxWalletAddress = _account;
    }
    ...
    function _tokenTransfer(address sender, address recipient, uint256 amount,bool takeFee) private {
        ...
        if (_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferFromExcluded(sender, recipient, amount.sub(taxAmount));
        } else if (!_isExcluded[sender] && _isExcluded[recipient]) {
            _transferToExcluded(sender, recipient, amount.sub(taxAmount));
        } else if (!_isExcluded[sender] && !_isExcluded[recipient]) {
            _transferStandard(sender, recipient, amount.sub(taxAmount));
        } else if (_isExcluded[sender] && _isExcluded[recipient]) {
            _transferBothExcluded(sender, recipient, amount.sub(taxAmount));
        } else {
        ...
        removeAllFee();
        _transferStandard(sender, _taxWalletAddress, taxAmount);
        restoreAllFee();
