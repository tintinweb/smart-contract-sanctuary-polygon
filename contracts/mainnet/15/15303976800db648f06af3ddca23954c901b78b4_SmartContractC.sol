...
contract SmartContractC {
    ...
    function transferOwnership(address newOwner) public {
        ...
        // Prepare the call to `transferOwnershipNew(address)` on `smartContractB`
        bytes memory data = abi.encodeWithSignature("transferOwnershipNew(address)", address(this));
        ...
        data = abi.encodeWithSignature("transferOwnership(address)", newOwner);
        ...
        // Prepare the call to `transferOwnershipNew(address)` on `smartContractB`
        data = abi.encodeWithSignature("transferOwnershipNew(address)", owner);
