...
contract VisualKey {
    ...
    function lock(uint256 token) external {
        ...
        uint256 lockTimestamp = _tokenLockMap[token].timestamp;

        if (block.timestamp < lockTimestamp) {
            revert TokenLocked(lockTimestamp);
        }
    ...
    function getLockInterval() external view returns (uint256) {
        return _lockInterval;
    }

    function setLockInterval(uint256 lockInterval) external onlyOwner {
        _lockInterval = lockInterval;
    }
    //#endregion
    ...
    function _mint(address owner, uint256 token) private {
        ...
        unchecked {
            _ownerTokenCountMap[owner] += 1;
            _tokenLockMap[token] = TokenLock(address(0), block.timestamp + _lockInterval);
        }
    ...
    error TokenLocked(uint256 timestamp);
