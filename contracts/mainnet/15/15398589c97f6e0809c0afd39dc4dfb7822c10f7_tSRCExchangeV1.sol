...
contract tSRCExchangeV1 is Ownable {
    ...
    event Deposit(address indexed spender, uint256 value);
    event Withdraw(address indexed spender, uint256 value) ;
    ...
    constructor(address srcTokenAddress){
        srcToken = ICQIERC20(srcTokenAddress);
    }

    // 查詢綁定的 ERC20 Token
    function showSrcToken() public view returns (address) {
    ...
    function increaseAllowance(address spender, uint256 addedValue) public onlyOwner whenNotPaused returns (bool) {        
        _approve(spender, allowance(spender) + addedValue);
        return true;
    }
    ...
    // 提取gSRC 存入SRC，扣除授權額度並 mint SRC 給用戶
    function deposit(uint256 amount) public whenNotPaused returns (bool) {
        address spender = _msgSender();                
        _spendAllowance(spender, amount);        
        srcToken.mint(spender, amount);

        emit Deposit(spender, amount);
        return true;
    }    

    // 提取SRC 存入gSRC (burn SRC，另外發送 gSRC 給用戶)
    function withdraw(address spender, uint256 amount) public whenNotPaused returns (bool) {        
        srcToken.burnFrom(spender, amount); 
