...
contract PositionBalancer is IERC721Receiver, IUniswapPositionBalancerResolver, OpsReady {
    ...
    onlyOpsOrOwner(rebalanceRequest.tokenId)
  {
    ...
    (
      uint256 amount0PositionBalance,
      uint256 amount1PositionBalance
    ) = collectAllFeesAndLiquidityForRebalance(rebalanceRequest.tokenId, deposit);
    ...
    // calculate the optimal amount of tokens to buy and add to the new position
    (
      uint256 amount0ToBuy,
      uint256 amount1ToBuy,
      uint256 optimalAmount0,
      uint256 optimalAmount1
    ) = getAmountsToBuyForNewLiquidityPosition(
        deposit,
        rebalanceRequest,
        amount0PositionBalance,
        amount1PositionBalance
      );
  ...
  function getAmountsToBuyForNewLiquidityPosition(
    ...
    RebalanceRequest memory rebalanceRequest,
    uint256 amount0PositionBalance,
    uint256 amount1PositionBalance
  )
    public
    ...
    )
  {
    IUniswapV3Pool pool = IUniswapV3Pool(
      uniswapV3Factory.getPool(deposit.token0, deposit.token1, deposit.fee)
    );
    (uint160 sqrtPriceX96, , , , , , ) = pool.slot0();
    int24 tick = TickMath.getTickAtSqrtRatio(sqrtPriceX96);

    (optimalAmount0, optimalAmount1) = LiquidityAmounts.getAmountsForLiquidity(
      TickMath.getSqrtRatioAtTick(tick),
      TickMath.getSqrtRatioAtTick(rebalanceRequest.newLowerTick),
      TickMath.getSqrtRatioAtTick(rebalanceRequest.newUpperTick),
      deposit.liquidity
    );

    amount0ToBuy = optimalAmount0 > amount0PositionBalance
      ? optimalAmount0 - amount0PositionBalance
      : 0;
    amount1ToBuy = optimalAmount1 > amount1PositionBalance
      ? optimalAmount1 - amount1PositionBalance
      : 0;
  }
