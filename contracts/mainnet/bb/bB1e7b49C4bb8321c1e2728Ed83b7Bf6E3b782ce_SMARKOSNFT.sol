...
contract SMARKOSNFT is ERC721Royalty, Ownable, ContextMixin {

    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public MAX_QTY = 10000;

    uint256 public price = 130 ether;

    uint96 public royaltyPer = 750;

    bool public _saleStatus;

    mapping(address=>bool) public isAirdropUsers;
    mapping(address=>uint) public isWhitelisted;
    mapping(address=>uint) public isAllowedPublicSale;

    constructor() ERC721("SMARKOS","SMK") {
        _setBaseURI("ipfs://bafybeigxux2krimqnaspwj4c2rjmts4e5mr36d565d7mljeylvdiuy5cda/");
    }
    ...
    function mintNFT(uint numberOfTokens) public payable saleFlip {
        ...
        require(isAllowedPublicSale[msg.sender].add(numberOfTokens) <=5,"User exceed max limit");
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            _safeMint(msg.sender, mintIndex+1);
            _setTokenRoyalty(mintIndex+1, msg.sender, royaltyPer);
        }
        isAllowedPublicSale[msg.sender] += numberOfTokens;
    }

    function mintWhiteListUserNFT(uint numberOfTokens) public payable {
        require(totalSupply().add(numberOfTokens) <= MAX_QTY, "Purchase would exceed max supply of NFTs");
        require(price.mul(numberOfTokens) <= msg.value, "Ether value sent is not correct");
        require(isWhitelisted[msg.sender].add(numberOfTokens) <= 3, "User exceed Max limit");
        for(uint i = 0; i < numberOfTokens; i++) {
            uint mintIndex = totalSupply();
            _safeMint(msg.sender, mintIndex+1);
            _setTokenRoyalty(mintIndex+1, msg.sender, royaltyPer);
        }
        isWhitelisted[msg.sender] += numberOfTokens;
    }

    function airdrop() public {
        require(totalSupply().add(3) <= MAX_QTY, "Purchase would exceed max supply of NFTs");
        require(isAirdropUsers[msg.sender],"User is not allowed for Airdrop!");
        uint mintIndex = totalSupply();
        _safeMint(msg.sender, mintIndex+1);
        _setTokenRoyalty(mintIndex+1, msg.sender, royaltyPer);
        isAirdropUsers[msg.sender] = false;
    }

    function addAirdropUsers(address[] memory _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            isAirdropUsers[_users[i]] = true;
        }
    }

    function addWhitelistUsers(address[] memory _users) public onlyOwner {
        for (uint i = 0; i < _users.length; i++) {
            isWhitelisted[_users[i]] = 0;
        }
    }
