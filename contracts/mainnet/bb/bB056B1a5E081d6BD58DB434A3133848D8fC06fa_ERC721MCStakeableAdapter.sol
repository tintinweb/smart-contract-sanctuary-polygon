...
contract ERC721MCStakeableAdapter is IMissionControlStakeable, OwnableUpgradeable, ERC721HolderUpgradeable {
    ...
    event ParametersUpdated(address _stakeable, uint _id, uint _ev, uint _cap, bool _isBase);


    mapping(uint => uint) seeds;
    mapping(address => mapping(uint => uint)) tokenIds;
    uint public resourceId;
    uint public resourceEV;
    uint public resourceCap;
    int public maxCutoff;
    address missionControl;
    IERC721 erc721Token;
    bool isBaseVal;
    int constant MAX_SCALE = 1e6;
    int constant MAX_HALF_SCALE = 5e5;
    int256 constant MAX_LOG2_E = 1_442695;
    uint constant PROBA_RES = 1000;
    uint constant CUTOFF_COEF = 115293;
    uint constant CUTOFF_INTERCEPT = 17322;
    ...
    * @param _resourceEV Expected value for poisson distribution over 24 hours
    * @param _resourceCap The maximum amount of resources that can occur "on" a token at once
    */
    function initialize(uint _resourceId, uint _resourceEV, uint _resourceCap, bool _isBase) external initializer {
        resourceId = _resourceId;
        resourceEV = _resourceEV;
    ...
    function __update_cutoff() private {
        if (resourceEV > 0) {
            maxCutoff = int((CUTOFF_COEF * resourceCap) / resourceEV + CUTOFF_INTERCEPT);
        }
    }
    ...
    */
    ...
    function setResourceParameters(uint _resourceId, uint _resourceEV, uint _resourceCap, bool _isBase) external onlyOwner {
        ...
        isBaseVal = _isBase;
        __update_cutoff();
        emit ParametersUpdated(address(this), resourceId, resourceEV, resourceCap, isBaseVal);
    }
    ...
    function reset(address _userAddress, uint _nonce) external override onlyMissionControl {
    ...
    /// @inheritdoc IMissionControlStakeable
    function checkTile(address _userAddress, uint _nonce) external override onlyMissionControl view returns (uint, uint){
        return __checkTile(tokenIds[_userAddress][_nonce]);
    }

    /// @inheritdoc IMissionControlStakeable
    function onCollect(address _userAddress, uint _nonce) external override onlyMissionControl returns (uint, uint) {
        uint tokenId = tokenIds[_userAddress][_nonce];
        (uint amount, uint id) = __checkTile(tokenId);
        seeds[tokenId] = block.timestamp;
        return (amount, id);
    }

    /// @inheritdoc IMissionControlStakeable
    function isBase(uint _tokenId) view external override returns (bool){return isBaseVal;}

    /// @inheritdoc IMissionControlStakeable
    function isRaidable(uint _tokenId) view external override returns (bool _isRaidable){
        _isRaidable = true;
    }

    /// @inheritdoc IMissionControlStakeable
    function onStaked(
        address _currentOwner, uint _tokenId, uint _nonce, address _underlyingToken, uint _underlyingTokenId) external override onlyMissionControl {
    ...
    /// @inheritdoc IMissionControlStakeable
    function onUnstaked(address _newOwner, uint _nonce) external override onlyMissionControl {
        uint _tokenId = tokenIds[_newOwner][_nonce];
    ...
    function __checkTile(uint tokenId) internal virtual view returns (uint, uint){
        if (resourceEV == 0) {
            return (0, resourceId);
        }
        int localSeed = int(seeds[tokenId]);
        require(localSeed > 0, "MCS_ADAPTER: No such token staked");
        int blockTime = int(block.timestamp);
        if (blockTime - localSeed >= maxCutoff) {
            return (resourceCap, resourceId);
        }
        uint ret = 0;
        int T = _getT(tokenId, localSeed);
        while (localSeed + T < blockTime && ret < resourceCap) {
            ret++;
            localSeed = localSeed + T;
            T = _getT(tokenId, localSeed);
        }
        return (ret, resourceId);
    }

    /// @inheritdoc IMissionControlStakeable
    function checkTimestamp(address _userAddress, uint _nonce) external view override onlyMissionControl returns (uint _timestamp){
        _timestamp = seeds[tokenIds[_userAddress][_nonce]];
    }

    /// @inheritdoc IMissionControlStakeable
    function timeLeft(address _userAddress, uint _nonce, uint _tokenId) external view override onlyMissionControl returns(int _timeLeft){
        int localSeed = int(seeds[tokenIds[_userAddress][_nonce]]);
        require(localSeed > 0, "MCS_ADAPTER: No such token staked");
        _timeLeft = -1;
    }
    ...
    */
    ...
    function _getT(uint tokenId, int seed) view internal returns (int T){
        require(resourceEV > 0, "PIX: Expected value is zero");
        T = _max_div(int(DAY_IN_SEC) * MAX_SCALE, int(resourceEV) * MAX_SCALE) * _exponential_2(tokenId, seed) / (MAX_SCALE * MAX_SCALE);
    }

    function _uniform(uint tokenId, int seed) pure internal returns (uint uniform_sample){
        uniform_sample = uint(keccak256(abi.encode(tokenId, seed))) % PROBA_RES;
    }

    function _exponential_2(uint tokenId, int seed) pure internal returns (int256 exponential_sample){
        exponential_sample = - _max_ln(
            MAX_SCALE - _max_div(
            MAX_SCALE * int(_uniform(tokenId, seed)),
            MAX_SCALE * int(PROBA_RES)
        ));
    }
