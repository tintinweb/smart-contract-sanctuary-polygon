...
contract TreasuryDAO is ReentrancyGuard, AccessControl {
    ...
    function convertDepositFeesToUSD(address token, uint256 tokenType, uint256 amount) public  onlyRole(OPERATOR_ROLE) {
        ...
        uint256 holdingLiquidity = amount * 250 / 10000;
        uint256 holdingOwnership = amount * 750 / 10000;
        ...
        if (token == address(wnativeToken)) {
            _swapTokensForEth(token, holdingOwnership);
            return;
        }
         
        // uint256 usdValue = getTokenUSDValue(amount, token, tokenType, false, address(usdCurrency));
