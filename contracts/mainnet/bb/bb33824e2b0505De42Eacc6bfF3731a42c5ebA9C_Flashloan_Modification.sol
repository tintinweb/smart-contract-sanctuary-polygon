...
contract Flashloan_Modification is IFlashloan, DodoBase, FlashloanValidation, Withdraw {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    event SentProfit(address recipient, uint256 profit);
    event SwapFinished(address token, uint256 amount);
    event TokenApprovalStatus(address token, address to, bool status); // NEW: event to track token approval status
    ...
    checkParams(params)
{
    ...
    require(
        IERC20(loanToken).balanceOf(address(this)) >= params.loanAmount,
        "Flash loan failed"
    );
}
    ...
    function approveToken(
        ...
        bool approvalSuccess = IERC20(token).approve(to, amountIn); // NEW: check approval status
        emit TokenApprovalStatus(token, to, approvalSuccess); // NEW: emit event to track approval status
        require(approvalSuccess, "Token approval failed"); // NEW: add a require check
    }
}
