    ...
    Ownable
{
    ...
    uint256 public _maxMintPerAddress = 36;
    uint256 public _maxNFTPerAddress = 36;
    ...
    uint256 private _numItemsAllocated;
    ...
    /// Mints _amount of token to owner address
        ...
        whenNotPaused
    {
        ...
        require(setNumItemsAllocated(_optionId), "allocation failed");
        require(canMint(_numItemsAllocated), "supply limit reached");
        require(canMintAndOwn(_toAddress, _numItemsAllocated));
        require(checkValue(msg.value, _numItemsAllocated));
    ...
    /// Internal function to allocate requested items for minting
    /// @param _optionId bulk minting option id
    /// @return bool true if the number was set successfully, false if not
    function setNumItemsAllocated(uint256 _optionId) internal returns (bool) {
        if (_optionId >= NUM_OPTIONS) {
            return false;
        }
        if (_optionId == COMMON_OPTION) {
            _numItemsAllocated = NUM_COMMON_OPTION;
        } else if (_optionId == UNCOMMON_OPTION) {
            _numItemsAllocated = NUM_UNCOMMON_OPTION;
        } else if (_optionId == RARE_OPTION) {
            _numItemsAllocated = NUM_RARE_OPTION;
        } else if (_optionId == EPIC_OPTION) {
            _numItemsAllocated = NUM_EPIC_OPTION;
        } else if (_optionId == LEGENDARY_OPTION) {
            _numItemsAllocated = NUM_LEGENDARY_OPTION;
        }
        return true;
    }

    /// Checks if tx exceeds total supply
    /// @param _amount of item to mint
    /// @return bool true if requested amount can be minted, false if it exceeds max supply
    function canMint(uint256 _amount) public view returns (bool) {
        return totalSupply() <= (MAX_SUPPLY - _amount);
    }
    ...
    /// @param requestedNumItems number of requested NFTs for bulk minting
    /// @return bool true if minting and owning limits not exceeded, otherwise return false
    function canMintAndOwn(address _toAddress, uint256 requestedNumItems)
        public
        view
        returns (bool)
    {
        require(
            (addressMintedBalance[_toAddress] + requestedNumItems) <=
                _maxMintPerAddress,
            "mint limit exceeded"
        );
        require(
            (balanceOf(_toAddress) + requestedNumItems) <= _maxNFTPerAddress,
            "allowed amount exceeded"
        );
        return true;
    }
    ...
    /// @param items number of NFTs requested for bulk minting
    function checkValue(uint256 value, uint256 items)
        internal
        view
        returns (bool)
    {
    ...
    function _beforeTokenTransfer(
        ...
        if (to != owner()) {
            require(
                balanceOf(to) + 1 <= _maxNFTPerAddress,
                "receiver exceeds allowed amount"
            );
        }
    }
    ...
    /// If allowed, override isApprovedForAll to whitelist user's marketplace proxy accounts to enable gas-less listings and easy trading
    function isApprovedForAll(address owner, address operator)
        ...
        returns (bool)
    {
        if (_proxyRegistryAddress == operator && _secAllowIsApprovedForAll) {
            return true;
        }
