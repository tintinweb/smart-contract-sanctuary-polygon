...
abstract contract RouterCrossTalk is Context, iRouterCrossTalk, ERC165 {
  ...
  modifier isLinkSet(uint8 _chainID) {
    require(
      ...
      "RouterCrossTalk : Cross Chain Contract to Chain ID set"
    );
    _;
  }
  ...
  function routerSend(
    ...
    emitCrossTalkEvent(destChainId, _selector, _data, hash);
    
    return (true, hash);
  }

  function emitCrossTalkEvent(uint8 destChainId, bytes4 selector, bytes memory data, bytes32 hash) private{
    emit CrossTalkSend(
