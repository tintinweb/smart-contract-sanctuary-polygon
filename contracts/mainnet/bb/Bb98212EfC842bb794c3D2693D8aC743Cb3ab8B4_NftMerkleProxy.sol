...
contract NftMerkleProxy is MerkleAllowList, NftTrustedConsumers, Pausable, Ownable {
    address payable public payeeAddress;
    IMinter private minterAddress;
    uint256 public fee = 0.004 ether;
    constructor(address payable _payeeAddress, address payable _minterAddress, bytes32 merkleRoot) MerkleAllowList(merkleRoot) {
        require(_payeeAddress != address(0x0), "payeeAddress Need a valid address");
        require(_minterAddress != address(0x0), "minterAddress Need a valid address");
        minterAddress = IMinter(_minterAddress);
        payeeAddress = _payeeAddress;
    }

    function setMinter(address _minterAddress) public onlyOwner {
        require(_minterAddress != address(0x0), "Need a valid address");
        minterAddress = IMinter(_minterAddress);
    }
    ...
    function mint(address to) public payable onlyPublicSale whenNotPaused {
        require(msg.value >= fee, "Payable must be at least the fee");
        minterAddress.mint(to);
    }
    ...
    function proxyMint(address to) public payable whenNotPaused {
        require(isTrusted(_msgSender()), "msg sender is not Trusted");
        minterAddress.mint(to);
    }

    function ownerMint(address to) public payable onlyOwner whenNotPaused {
         minterAddress.mint(to);
    }
...
abstract contract NftTrustedConsumers {
