...
contract StakingContract is AccessControl, Multicall {
    ...
    modifier sync(IERC20 toSync) {
        ...
        if (totalTokensDeposited[toSync] > 0) {
            if (toSync.balanceOf(address(this)) > totalTokensDeposited[toSync]) {
                ...
                totalTokensDeposited[toSync] = toSync.balanceOf(address(this));
            }
        }
        _;        
    }
    ...
    function collectTreasury(IERC20 toCollect, address recipient) onlyRole(ADMIN_ROLE) external sync(toCollect) {
        uint256 amount = deposited[treasury][toCollect];
        require(amount > 0, "treasury is empty");
        toCollect.safeTransfer(recipient, amount);
        deposited[treasury][toCollect] -= amount;
    }
