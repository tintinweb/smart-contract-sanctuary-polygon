...
contract PaperHands is ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  mapping(address => bool) private approvedSenders;
  ...
  function deductFeeGiddy(uint256 amount, uint256 fap) internal returns (uint256) {
    if (fap > 0) {
      GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, FEE_ACCOUNT, fap);
    }
    return amount - fap;
  }

  function transferUsdcCalc(uint256 value, uint256 fap, bool swapToGiddy) external view returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = value - fap;
    priceImpact = 0;
    if (swapToGiddy) {
  ...
  function transferUsdc(bytes calldata receiveAuthorization, address recipient, uint256 fap, bool swapToGiddy) external whenNotPaused nonReentrant returns (uint256 valueUsdc) {
    ...
    require(spender == address(this) && owner == _msgSender(), "Failed owner/spender check");
    ...
    require(value > 0, "Input amount to small");

    owner = recipient; //avoids stack to deep error
    if (swapToGiddy) {
      address[] memory route = new address[](2);
      route[0] = USDC_TOKEN;
      route[1] = GIDDY_TOKEN;
      return GiddyUniswap.swapTokens(SUSHI_ROUTER, route, owner, value);
    }
    else {
      require(recipient != _msgSender(), "Invalid transfer");
  ...
  function transferGiddyCalc(uint256 value, uint256 fap, bool swapToUsdc) external view returns (uint256 amountOut, uint256 priceImpact) {
    amountOut = value - fap;
    priceImpact = 0;
    if (swapToUsdc) {
      (amountOut, priceImpact) = GiddyUniswap.calcPriceSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, amountOut);
    }
  }

  function transferGiddy(ApproveWithAuthorization.ApprovalRequest calldata req, bytes calldata sig, address recipient, uint256 fap, bool swapToUsdc) external whenNotPaused nonReentrant returns (uint256 valueUsdc) {
    require(req.spender == address(this) && req.owner == _msgSender(), "Failed owner/spender check");
    ...
    require(value > 0, "Input amount to small");
    if (swapToUsdc) {
      return GiddyUniswap.swapTokensSimple(SUSHI_ROUTER, GIDDY_TOKEN, USDC_TOKEN, recipient, value);
    }
    else {
      require(recipient != _msgSender(), "Invalid transfer");
      if (!IERC20(GIDDY_TOKEN).transfer(recipient, value)) {
        revert("Failed transfer to recipient");
      }
      return value;
    }
  ...
  function transferErc20Token(address recipient, address tokenAddress, uint256 value) external whenNotPaused nonReentrant {
    require(value > 0, "Input amount too small");
    require(recipient != _msgSender(), "Invalid transfer");

    if (IERC20(tokenAddress).allowance(_msgSender(), address(this)) < value) {
      revert("Allowance too low");
    }

    if (!IERC20(tokenAddress).transferFrom(_msgSender(), address(this), value)) {
      revert("Failed transfer to contract");
    }

    if (!IERC20(tokenAddress).transfer(recipient, value)) {
      revert("Failed transfer to recipient");
    }
  }
