...
contract PAPC is ERC20, Ownable, ReentrancyGuard { 
    ...
    constructor() ERC20("Polygon Ape Punks Club Token", "PAPC") { 
        uint256 initialSupply = 500000000000 * 10**decimals(); 
        _mint(msg.sender, initialSupply); 
        setTaxAddress(0x000000000000000000000000000000000000dEaD);
        setTaxRate(0); // 0% tax rate by default
    }

    function setTaxAddress(address _taxAddress) public onlyOwner {
        require(_taxAddress != address(0), "Invalid address");
        taxAddress = _taxAddress;
        emit TaxAddressChanged(_taxAddress);
    }

    function setTaxRate(uint256 _taxRate) public onlyOwner {
        require(_taxRate <= 8, "Tax rate should be less than or equal to 8");
        taxRate = _taxRate;
