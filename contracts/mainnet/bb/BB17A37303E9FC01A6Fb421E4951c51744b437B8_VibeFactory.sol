...
contract VibeFactory is BoringBatchable {
    ...
    function createNFTMintSale(string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint16 derivativeRoyaltyRate, bool isDerivativeAllowed, uint64 maxMint_, uint32 beginTime_, uint32 endTime_, uint128 price_, IERC20 paymentToken_) external {
        ...
        {
            RoyaltyInformation memory royalty = RoyaltyInformation( royaltyReceiver_, royaltyRate_,  derivativeRoyaltyRate,  isDerivativeAllowed);
            data = abi.encode(abi.encode(symbol, name, baseURI), royalty, maxMint_, beginTime_, endTime_, price_, paymentToken_, msg.sender);
        }
    ...
    function createNFTMintSaleMultiple(string memory symbol, string memory name, string memory baseURI, address royaltyReceiver_, uint16 royaltyRate_, uint16 derivativeRoyaltyRate, bool isDerivativeAllowed, uint32 beginTime_, uint32 endTime_, TierInfo[] memory tiers_, IERC20 paymentToken) external {
        ...
        {
            RoyaltyInformation memory royalty = RoyaltyInformation( royaltyReceiver_, royaltyRate_,  derivativeRoyaltyRate,  isDerivativeAllowed);
            data = abi.encode(abi.encode(symbol, name, baseURI), royalty, beginTime_, endTime_, tiers_, paymentToken, msg.sender);
        }
...
contract NFTMintSale is Ownable {
    ...
    function init(bytes calldata data) public payable {
        (bytes memory dataNFT, RoyaltyInformation memory royalty, uint64 maxMint_, uint32 beginTime_, uint32 endTime_, uint128 price_, IERC20 paymentToken_, address owner_) = abi.decode(data, (bytes, RoyaltyInformation, uint64, uint32, uint32, uint128, IERC20, address));
        ...
        address proxy = vibeFactory.deploy(masterNFT, dataNFT, false);

        vibeFactory.transferOwnership(proxy, address(this));

        nft = VibeERC721(proxy);
        nft.setMinter(address(this), true);
        nft.setRoyalty(royalty.royaltyReceiver_, royalty.royaltyRate_, royalty.derivativeRoyaltyRate, royalty.isDerivativeAllowed);
...
contract NFTMintSaleMultiple is Ownable {
    ...
    function init(bytes calldata data) external {
        (bytes memory dataNFT, RoyaltyInformation memory royalty, uint32 beginTime_, uint32 endTime_, TierInfo[] memory tiers_, IERC20 paymentToken_, address owner_) = abi.decode(data, (bytes, RoyaltyInformation, uint32, uint32, TierInfo[], IERC20, address));
        require(nft == VibeERC721(address(0)), "Already initialized");
        ...
        address proxy; 
        {
            proxy = vibeFactory.deploy(masterNFT, dataNFT, false);
        }
