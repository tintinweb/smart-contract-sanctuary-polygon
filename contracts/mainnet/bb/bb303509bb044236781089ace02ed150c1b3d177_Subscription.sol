...
contract Box is Ownable {
   ...
   mapping(address => uint8) public requestCount;
   ...
   function getAllFile() external view returns (File[] memory) {
   ...
   function getUsedStorage() external view returns (uint256) {
       return usedStorage;
   }

   // Function to get total available storage of box
   function getTotalStorage() external view returns (uint256) {
       return totalStorage;
   }
   ...
   function checkAllowedStatus(address user) external view returns (bool) {
   ...
   function requestAccessForBox() external {
       require(
           isRequested[msg.sender].isRequested == false,
   ...
   function requestAccept(address user) external onlyOwner {
   ...
   function requestAutoAccept(address user) external callbyFactoryContract {
       allowedUsers[user] = true;
       isRequested[user].isRequested = false;
   }

   // Function to Reject Request
   function requestReject(address user) external callbyFactoryContract {
       require(isRequested[user].isRequested, "user not Request for Access");
       isRequested[user].isRequested = false;
   }
...
interface box {
   function requestReject(address user) external;

   function requestAutoAccept(address user) external;
...
contract Subscription is Ownable {
   ...
   constructor() {
       packages.push(subscriptionPlans(5876000000000000000, 10 * 1024));
       packages.push(subscriptionPlans(11750000000000000000, 25 * 1024));
       packages.push(subscriptionPlans(14700000000000000000, 50 * 1024));
   }
   ...
   function updateSubscriptionPlan(
       ...
       packages[index].storageAvailable = newStorage * 1024;
   }
   ...
   function createNewSubscriptionPlan(uint256 newPrice, uint256 newStorage)
       ...
       onlyOwner
   {
       packages.push(subscriptionPlans(newPrice, newStorage * 1024));
   }

   // Function to Remove subscription plan
   function removeSubscriptionPlan(uint256 index) external onlyOwner {
       packages[index] = packages[packages.length - 1];
       packages.pop();
   }

   // Function to return total subscription plans
   function totalSubscriptionPlans() external view returns (uint256) {
   ...
   function subscribe(uint256 package) external payable returns (address) {
       require(
   ...
   function autoAllowAccessForSpecificBox(address boxAddress, address user)
       external
       AllowedForBoxCall
   {
       box(boxAddress).requestAutoAccept(user);
   }

   // Function to Auto Approve Request for specific box
   function rejectAllowAccessForSpecificBox(address boxAddress, address user)
       external
       AllowedForBoxCall
   {
       box(boxAddress).requestReject(user);
   }

   // Function to Allow Users to call requestReject Function of specific box
   function allowToCallBoxFunctions(address allow) external onlyOwner {
       isAllowForBoxCall[allow] = true;
   }

   // Function to disAllow Users to call requestReject Function of specific box
   function disAllowToCallBoxFunctions(address allow) external onlyOwner {
