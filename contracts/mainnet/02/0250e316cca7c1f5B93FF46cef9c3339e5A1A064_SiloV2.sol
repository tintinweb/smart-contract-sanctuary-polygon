...
contract SiloV2 is Initializable, KeeperCompatibleInterface, IERC721Receiver {
    ...
    modifier onlySiloManager() {
        require(
            ISiloManagerFactory(ISiloFactory(factory).managerFactory())
                .checkManager(
                    ISiloFactory(factory).ownerOf(SILO_ID),
                    msg.sender,
                    autoType
                ),
            "not manager"
    ...
    function exitSilo(address caller) external onlyFactory {
        ...
        if (maticBalance > 0) {
            (bool success, ) = payable(caller).call{value: maticBalance}("");
