...
contract CampaignContract is ICampaignContract, Ownable, Initializable, ERC1155Holder {
    ...
    modifier validClaimCreator(string memory _campaignId) {
        require(!isGoalReached(_campaignId),"CampaignContract::fundCampaign: Campaing is Goal Reached");
        _;
    }
    ...
    **/
    function claimFractionCreator(string memory _campaignId, address _creatorWallet, uint256 _fractionCount ) override external onlyOwner validClaimCreator(_campaignId){
        Campaign memory _camp = idToCampaign[_campaignId];
        require(_fractionCount > 0 && _camp.remainFractions > _fractionCount, "CampaignContract::claimFractionCreator:Over limit");

        SJFractions _sjfractions = SJFractions(fractionContract);
        uint256 itemId = _sjfractions.getItemId(_campaignId);
        _sjfractions.safeTransferFrom(address(this), _creatorWallet, itemId, _fractionCount, "0x00");

        //Emit event claim
        emit ClaimCampaign(block.timestamp, _fractionCount,_creatorWallet,"creator_wallet","",_campaignId);
