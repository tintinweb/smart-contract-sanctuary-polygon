...
contract SharedPigeon{
    ...
    constructor (address coinAddress, address chatAddress){
        _owner = msg.sender;
	...
	function connectToChat(address sender) public {
		require(msg.sender == _toAddress[sender], "Do not have invite");
		if (!IChat(chat).isContact(sender, _toAddress[sender])){
			IChat(chat).addContactByPigeon(sender, _toAddress[sender]);
		}
		if (!IChat(chat).isContact(_toAddress[sender], sender)){
			IChat(chat).addContactByPigeon(_toAddress[sender], sender);
		}
	}
	...
	function hasFlown(address sender) public view returns (bool flown){
		return _hasFlown[sender];
	}
