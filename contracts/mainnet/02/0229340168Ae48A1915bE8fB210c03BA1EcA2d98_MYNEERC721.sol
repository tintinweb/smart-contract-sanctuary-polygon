...
// File: contracts/SignatureHandler.sol


pragma solidity ^0.8.0;


abstract contract SignatureHandler is EIP712 {
  struct Request {
  ...
  bytes32 private constant TYPEHASH =
    keccak256(
      "Request(uint256 validityStartTimestamp,uint256 validityEndTimestamp)"
    );

  constructor() EIP712("SignatureHandler", "1") {}
  ...
  function _processRequest(
    ...
    emit RequestExecuted(msg.sender, signer, _req);
  }
  ...
  function _recoverAddress(
    ...
    return
      _hashTypedDataV4(keccak256(_encodeRequest(_req))).recover(_signature);
  }
...
contract MYNEERC721 is Ownable, ERC721, SignatureHandler {
  ...
  function mint(address recipient, string memory _tokenURI) private {
    _safeMint(recipient, _tokenId);
    ownershipRecord[recipient].push(
      TokenMetaData(_tokenId, block.timestamp, _tokenURI)
    );
    tokenUrl[_tokenId] = _tokenURI;
    _tokenId = _tokenId + 1;
  }
