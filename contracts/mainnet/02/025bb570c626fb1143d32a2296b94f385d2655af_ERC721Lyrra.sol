...
library LibPart {
    ...
    function hashData(Part memory part) internal returns (bytes32) {
        
        bytes memory message = abi.encode(TYPE_HASH, part.account, part.value);
       
        bytes32 hash = keccak256(message);
        emit PartData(message,hash);
        return hash;
...
library Sku{
    ...
    struct SkuInfo{
        ...
        //bytes signature; // 其他数据hash后进行签名
        uint8 v;
        bytes32 r;
        bytes32 s;
        //AssetType assetType;  // asset type enum AssetType {ETH, ERC20}
 
    }

    //event SkuHash(bytes32 hash,bytes message);
...
abstract contract ERC721Lazy is OwnableUpgradeable,IERC721LazyMint, ERC721Upgradeable, Mint721Validator, RoyaltiesV2Upgradeable, RoyaltiesV2Impl,SignerRole,ERC2981 {
    ...
    event TranferFee(address owner,uint256 tokenId,address token,uint256 platformFee, address skuPlatformAccount,uint256 fansFee, address skuFansAccount,uint256 other,address to);
    ...
    function setPlatformFee(address payable recipient,uint32 value) public onlyOwner{
        ...
        require(value != 0, "platform fee value  be positive");
   ...
   function setFansFee(address payable recipient,uint32 value) public onlyOwner{
        require(recipient != address(0x0), "Recipient  be present");
        require(value != 0, "fans fee value  be positive");
    ...
    //event payMintAndTransferHash(bytes message1,bytes message,address signer );
    // pay mint
    function payMintAndTransfer(uint256 tokenId, address to,string memory tokenURI,Sku.SkuInfo memory skuInfo, uint8 v, bytes32 r, bytes32 s) public payable override { 
          ...
          /*bytes memory message = abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this,tokenId,to, keccak256(bytes(tokenURI)),hash)));

          address signer = ecrecover(keccak256(abi.encodePacked("\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(this,tokenId,to, keccak256(bytes(tokenURI)),hash)))), v, r, s);
          emit payMintAndTransferHash(abi.encodePacked(this,tokenId,to, keccak256(bytes(tokenURI)),hash),message,signer);
          */
   ...
   function transfer(address token, uint value, address from, address to) internal {
    ...
    event TranferFee2(uint256 price ,address token);
    function tranferFee(uint256 price ,address token,address to,uint256 tokenId)  public{
          ...
          emit TranferFee(owner,tokenId,token,platformFee,skuPlatformFee.account,
             fansFee,skuFansFee.account,other,to );
