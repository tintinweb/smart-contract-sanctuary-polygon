...
import "../interfaces/3rd-party/IAavePool.sol";
...
contract ArbitrageHelper is Initializable, IArbitrageHelper {
    ...
    function initialize(address _exchange, address _aavePool) public initializer {
        EXCHANGE = IExchange(_exchange);
        AAVE_POOL = IAavePool(_aavePool);
    }

//    // @notice Can use address(this) instead of recipient.
...
//    }
    ...
    function executeOperation(address asset, uint256 amount, uint256 fee, address sender, bytes calldata data) external {
        ...
        // console.log("before safeTransfer 2");
        ...
        console.log('to arbitrage bot', swapProfit - tokenBLeft - fee);
        ...
        console.log('to approve', decoded.tokenBAmount + fee);
        // Send tokenBToRepay (Repay borrow)
        TransferHelper.safeApprove(decoded.tokenB, address(AAVE_POOL), 2**256 - 1);
        console.log(decoded.tokenB);
        console.log(IERC20(decoded.tokenB).balanceOf(address(this)));
        console.log(IERC20(decoded.tokenB).allowance(address(this), address(AAVE_POOL)));
        console.log(decoded.tokenBAmount + fee);
        // TransferHelper.safeTransfer(decoded.tokenB, address(AAVE_POOL), decoded.tokenBAmount + fee);
    }
