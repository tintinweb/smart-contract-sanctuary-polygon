...
contract DirectStrat is OwnableUpgradeable {
  ...
  function initialize(uint256 _amountDlyPerSale, uint256 _minDiff)
    external
    initializer
  {
    __Ownable_init();
  ...
  function triggerJava() external {
    (uint256 lastSale, , ) = JAVA_VENDOR.getLastSell(address(this));
    require(block.timestamp > lastSale + 1 days, 'CANT SWAP YET');
  ...
  function triggerPegdex() external {
    ...
    uint256 amountUsdtReceived = PEDGEX.swapCOPForTokens(
      USDT,
      amountDlyPerSale,
      minUsdtToReceive
    );
