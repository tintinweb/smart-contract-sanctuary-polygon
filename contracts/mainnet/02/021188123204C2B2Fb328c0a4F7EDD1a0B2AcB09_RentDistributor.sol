    ...
    ReentrancyGuardUpgradeable
{
    ...
    function redeem(
        ...
        require(
            ITokenWhitelist(tokenWhitelist).isOnWhitelist(tokenOut),
            "Token not supported"
        );
        IMetaTx(vTRY).permit(
            _metaTxArgs.owner,
        ...
        if (tokenOut == USDC) {
            ...
            uint tokensRecieved = IOCLRouter(oclrAddress).swap(
                USDC,
                USDC,
                tokenOut,
                toTransfer,
    ...
    function updateVTRY(address _vTRY) external onlyMaintainer {
        vTRY = _vTRY;
    }

    function updateUSDC(address _USDC) external onlyMaintainer {
        USDC = _USDC;
    }
