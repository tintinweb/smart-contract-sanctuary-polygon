...
contract ASCIITest is ERC721, Ownable, ContextMixin {
    ...
    constructor() ERC721("ASCIITest", "ETG"){}
    ...
    function calcTotal(uint count) public view returns(uint) {
        require(saleStatus != SaleStatus.PAUSED, "ASCIITest: Sales are off");
    ...
    function mint(uint count) external payable {
        require(saleStatus != SaleStatus.PAUSED, "ASCIITest: Sales are off");
        require(_tokenIds.current() + count <= COLLECTION_SIZE, "ASCIITest: Number of requested tokens will exceed collection size");
        
        
        require(msg.value >= calcTotal(count), "ASCIITest: Ether value sent is not sufficient");
