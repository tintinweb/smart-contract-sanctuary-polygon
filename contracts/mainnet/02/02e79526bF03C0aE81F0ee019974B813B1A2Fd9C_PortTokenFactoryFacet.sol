...
contract PortToken is ERC20, IPortToken {
    // using Address for address;

    address public override controller; // Controller contract address
    address public override manager ; // Manager of contract
    ...
    function changeController(address _newController) external override onlyController {
        controller = _newController;
    }

    function controllerMint(address _to, uint256 _amt) external override onlyController {
        ITransferHookReceiver(controller).onTokenTransfer(address(this), address(0), _to, _amt);
        _mint(_to, _amt);
    }

    function controllerBurn(address _from, uint256 _amt) external override onlyController {
    ...
    ) external onlyController override returns (bytes memory returndata) {
