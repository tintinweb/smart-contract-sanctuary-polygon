...
contract Sharpe is IVault,IUniswapV3MintCallback,IUniswapV3SwapCallback,ERC20,ReentrancyGuard{
    ...
    function withdraw(uint256 shares,uint256 amount0Min,uint256 amount1Min,address to
        ...
        if (amount0Min > 0 && amount1Min == 0){
            _zappTokens(amount1, true, to);
            if (amount0 > 0) token0.safeTransfer(to, amount0);
        }
        else if (amount1Min > 0 && amount0Min == 0){
            _zappTokens(amount0, false, to);
    ...
    function _zappTokens(uint256 amount, bool output, address receiver) internal{
        if (output){
