...
contract DividendPayingToken is BEP20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public immutable BUSD = address(0xdAb529f40E671A1D4bF91361c21bf9f0C9712ab7); //BUSD matic mainnet
...
contract wJindoChainCoin is BEP20, LockToken {
    using SafeMath for uint256;
    ...
    wJindoChainCoinDividendTracker public dividendTracker;

    address public deadWallet = 0x000000000000000000000000000000000000dEaD;

    address public immutable BUSD = address(0xdAb529f40E671A1D4bF91361c21bf9f0C9712ab7); //BUSD matic mainnet

    uint256 public _totalSupply = 21_000_000 * (10**18);
    uint256 public swapTokensAtAmount = 100_000 * (10**18); //50 millions. 
    uint256 public _maxTxAmount = _totalSupply.mul(2).div(100);//2%

    uint256 public BUSDRewardsFee = 1;
    ...
    address payable public _marketingWalletAddress = payable(0xABaa93A2C320b78423bAF49Aa6f7E6b87F0d8FB3);
    address payable public _buybackWalletAddress = payable(0x1234D8ebf8D83fBd305B1AF0369e8dcD4A62C9cF);
    ...
    constructor() BEP20("wJindoChainCoin", "wJCC") 
    {
        dividendTracker = new wJindoChainCoinDividendTracker();
    ...
    {
        require(newAddress != address(dividendTracker), "wJindoChainCoin: The dividend tracker already has that address");
        wJindoChainCoinDividendTracker newDividendTracker = wJindoChainCoinDividendTracker(payable(newAddress));
        require(newDividendTracker.owner() == address(this), "wJindoChainCoin: The new dividend tracker must be owned by the wJindoChainCoin token contract");
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "wJindoChainCoin: The router already has that address");
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "wJindoChainCoin: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "wJindoChainCoin: The PanBUSDSwap pair cannot be removed from automatedMarketMakerPairs");
        _setAutomatedMarketMakerPair(pair, value);
    }
    

    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "wJindoChainCoin: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "wJindoChainCoin: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "wJindoChainCoin: Cannot update gasForProcessing to same value");
    ...
    function _transfer(address from, address to, uint256 amount) internal open(from, to) override 
    {
        ...
        !automatedMarketMakerPairs[from] && from != owner()) 
        {
            ...
            uint256 swapableFees = totalFees.sub(BUSDRewardsFee); // marketingFee+buybackFee+liquidityFee.
...
contract wJindoChainCoinDividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("wJindoChainCoin_Dividend_Tracker", "wJindoChainCoin_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 2000_000 * (10**18); //must hold 2 Millions+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "wJindoChainCoin_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "wJindoChainCoin_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main wJindoChainCoin contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "wJindoChainCoin_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "wJindoChainCoin_Dividend_Tracker: Cannot update claimWait to same value");
