...
contract SignedStake is ISignedStake, TokensRecoverable
{
    ...
    function checkTokenClaims() private returns (PayOutInfo memory) {
        ...
        for (uint i = 0; i < tokenList.length; i++)
        {
            ...
            if (availableTokenClaims[msg.sender][tokenContract] > 0)
            {
                //add token contract to array
                tokenContracts.push(tokenContract);
                //add amount to array
                tokenAmounts.push(availableTokenClaims[msg.sender][tokenContract]);
            } 
            //_payoutInfo.tokenAddresses.push(availableTokenClaims[msg.sender][tokenContract]);
        }
