...
/// @author belbix
abstract contract BalancerUniversalStrategyBase is ProxyStrategyBase {
  using SafeERC20 for IERC20;
  ...
  address public constant VAULT_BB_T_USD_ENTER_TOKEN = 0xae646817e458C0bE890b81e8d880206710E3c44e;
  ...
  function initializeStrategy(
    ...
    IERC20(_getPoolAddress(poolId_)).safeApprove(gauge_, type(uint).max);
  ...
  function _liquidateRewards(bool silently) internal {
    ...
    if (_isCompound) {
      ...
      uint toPool = IERC20(VAULT_BB_T_USD_ENTER_TOKEN).balanceOf(address(this));
      if (toPool != 0) {
        ...
        _balancerJoin(tokenAssets, BB_T_USD_POOL_ID, VAULT_BB_T_USD_ENTER_TOKEN, toPool);
      }
