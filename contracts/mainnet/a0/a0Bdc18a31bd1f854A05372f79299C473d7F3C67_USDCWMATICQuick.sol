...
contract USDCWMATICQuick is CoreStrategyAave {
    using SafeERC20 for IERC20;
    ...
    constructor(address _vault)
        ...
        CoreStrategyAave(
            ...
            CoreStrategyAaveConfig(
                ...
                0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270, // short -> WMATIC
                0x6e7a5FAFcec6BB1e78bAE2A1F0B612012BF14827, // wantShortLP -> USDC/WMATIC 
                ...
                0x14977e7E263FF79c4c3159F497D9551fbE769625, // farmMasterChef -> IStakingDualRewards
                0, // farmPid
    ...
    function balancePendingHarvest() public view override returns (uint256) {
        uint256 dQuickPending = IStakingDualRewards(farmMasterChef).earnedA(address(this)).add(farmToken.balanceOf(address(this)));
        uint256 quickAmount = dragonLair.QUICKForDQUICK(dQuickPending).add(quick.balanceOf(address(this)));
        ...
        total_wmatic = total_wmatic.add(IStakingDualRewards(farmMasterChef).earnedB(address(this)));
    ...
    function _depositLp() internal override {
        ...
        IStakingDualRewards(farmMasterChef).stake(lpBalance);
    }
    ...
    function _sellHarvestWant() internal override {
        ...
        if (quickBalance > 0) {
            router.swapExactTokensForTokens(
                quickBalance,
                0,
                getTokenOutPath(address(quick), address(weth)),
                address(this),
                now
            );
        }
        ...
        if (wethBalance > 0) {
            router.swapExactTokensForTokens(
                wethBalance,
                0,
                getTokenOutPath(address(weth), address(want)),
                address(this),
                now
            );
        }
    ...
    function _withdrawFarm(uint256 _amount) internal override {
        if(_amount > 0)
            IStakingDualRewards(farmMasterChef).withdraw(_amount);
    }

    function claimHarvest() internal override {
        IStakingDualRewards(farmMasterChef).getReward();
    }
    
    function countLpPooled() internal view override returns (uint256) {
        return IStakingDualRewards(farmMasterChef).balanceOf(address(this));
    }
