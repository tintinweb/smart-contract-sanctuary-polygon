    ...
    IERC721ReceiverUpgradeable
{
     ...
     * @param _sourceToken Address of token being used to purchase the pool token.
     * @param _poolToken Address of pool token being used.
     * @return path of addresses to be used as the path for the swap.
     */
        ...
        returns (address[] memory path)
    {
        ...
        if (_poolToken == BCT && _sourceToken == USDC) {
            // If the source is USDC and retiring BCT, route through KLIMA.
            path = new address[](3);
            path[0] = _sourceToken;
            path[1] = KLIMA;
            path[2] = _poolToken;
        } else if (_sourceToken == KLIMA || _sourceToken == USDC) {
            // If the source is KLIMA or USDC do a direct swap, else route through USDC.
            path = new address[](2);
            path[0] = _sourceToken;
            path[1] = _poolToken;
        } else if (_poolToken == BCT && _sourceToken != BCT) {
            // Route non-USDC sources through USDC then KLIMA for BCT.
            path = new address[](4);
            path[0] = _sourceToken;
            path[1] = USDC;
            path[2] = KLIMA;
    ...
    function mintToucanCertificate(
        ...
        ) = IKlimaCarbonRetirements(retirementStorage).getRetirementIndexInfo(
                _beneficiary,
                _index
            );
