...
contract TestSwapV2 is Ownable {
  ...
  function wrappedSwapExactTokensForTokens(
    ...
    IUniswapV2Router02(_router).swapExactTokensForTokens(
      _amountIn.mul(fee) / 100,
      _amountOutMin.mul(fee) / 100,
      _addresses,
      _to,
      _deadline
      );
    ...
    function wrappedSwapETHForExactTokens(
      ...
      IUniswapV2Router02(_router).swapExactETHForTokens{value: msg.value.mul(fee) / 100}(
        _amountOut.mul(fee) / 100,
        _addresses,
        _to,
        _deadline
      );
    ...
    function getAmountOutMin(
      ...
      IUniswapV2Router02(_router).getAmountsOut(_amountIn.mul(fee) / 100, path);
