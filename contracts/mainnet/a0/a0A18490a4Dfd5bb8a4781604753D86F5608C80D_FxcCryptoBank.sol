...
contract FxcCryptoBank  {
    ...
    uint256 private constant maxDepositGrowth = 1000e18;
    uint256 private constant maxDeposit = 2000e18;
    uint256 private constant freezeIncomePercents = 5000;
    ...
    uint256[2] private associatemanagerPercents = [200, 300];
    uint256[2] private managerPercents = [400, 300];
    uint256[15] private RMPercents = [200, 100, 100, 100, 100, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50];
    uint256 private constant managerPoolPercents = 50;
      uint256 private constant RMPoolPercents = 50;
    ...
    event DepositBySeedFund(address user, uint256 amount);
    event TransferBySeed(address user, address receiver, uint256 amount);
    event Withdraw(address user, uint256 withdrawable);

    constructor(address _daitAddr)   {
        DAI = IERC20(_daitAddr);
       
          feeReceivers[0] = address(0x730DaB488b43f5B5D7d2Ac2B7F7a8b046555f742);   //// Creater Fund
          feeReceivers[1] = address(0x29979E066eA75DfF9fC0EDA5763218Bc0e014C18);   //// Developer Fund 
          feeReceivers[2] = address(0x56221Dd3E7D85Bd1735955eCAcD99588602602f7);   //// Reserve fund
 ...
 function _updateReferInfo(address _user, uint256 _amount) private {
     ...
     function _updateLevel(address _user) private {
        ...
        if(levelNow > user.level){
            user.level = levelNow;
              if(levelNow == 2){        
                associatemanagerUsers.push(_user);
            }
              if(levelNow == 3){        
...
function _calLevelNow(address _user) private view returns(uint256) {
  ...
  function getTeamDeposit(address _user) public view returns(uint256, uint256, uint256){
   ...
   function _deposit(address _user, uint256 _amount) private {
     ...
     function _distributeDeposit(uint256 _amount) private {
        ...
        DAI.transfer(feeReceivers[0], _amount.mul(100).div(baseDivider));
        DAI.transfer(feeReceivers[1], _amount.mul(200).div(baseDivider));
        DAI.transfer(feeReceivers[2], _amount.mul(200).div(baseDivider));
       uint256 manager = _amount.mul(managerPoolPercents).div(baseDivider);
        managerPool = managerPool.add(manager); 
        uint256 rm = _amount.mul(RMPoolPercents).div(baseDivider);
        RMPool = RMPool.add(rm); 
 ...
 function _distributeManagerPool() private {
        ...
        for(uint256 i = 0; i < managerUsers.length; i++){
            if(userInfo[managerUsers[i]].level == 3 && userInfo[managerUsers[i]].isactive == true){
       ...
       function _distributeRMPool() private {
        uint256 rmCount;
        for(uint256 i = 0; i < RMUsers.length; i++){
            if(userInfo[RMUsers[i]].level == 4 && userInfo[RMUsers[i]].isactive == true){
                rmCount = rmCount.add(1);
            }
        }
        if(rmCount > 0){
            uint256 reward = RMPool.div(rmCount);
            uint256 totalReward;
            for(uint256 i = 0; i < RMUsers.length; i++){
                if(userInfo[RMUsers[i]].level == 4 && userInfo[RMUsers[i]].isactive == true){
                    rewardInfoPool[RMUsers[i]].RM = rewardInfoPool[RMUsers[i]].RM.add(reward);
                    userInfo[RMUsers[i]].totalRevenue = userInfo[RMUsers[i]].totalRevenue.add(reward);
                    totalReward = totalReward.add(reward);
                }
            }
            if(RMPool > totalReward){
                RMPool = RMPool.sub(totalReward);
            }else{
                RMPool = 0;
            }
        }
 ...
 function _distributeGMPool() private {
        ...
        for(uint256 i = 0; i < GMUsers.length; i++){
            if(userInfo[GMUsers[i]].level == 5 && userInfo[GMUsers[i]].isactive == true){
  ...
  function depositBySeedFund(uint256 _amount) external {
        require(_amount >= minDeposit && _amount.mod(minDeposit) == 0, "amount err");
        require(_amount <= maxDepositGrowth , "less before 1");     
        ...
        emit DepositBySeedFund(msg.sender, SafeMath.mul(_amount,2));
    }

    function transferBySeed(address _receiver, uint256 _amount) external {
        require(_amount >= minDepositGrowth && _amount.mod(minDepositGrowth) == 0, "amount err");
        uint256 splitLeft = getCurSplit(msg.sender);
        require(splitLeft >= _amount, "insufficient income");
        rewardInfo[msg.sender].splitDebt = rewardInfo[msg.sender].splitDebt.add(_amount);
        rewardInfo[_receiver].split = rewardInfo[_receiver].split.add(_amount);
        emit TransferBySeed(msg.sender, _receiver, _amount);
    }
    ...
    function getssociatemanagerusersLength() external view returns(uint256) {
        return associatemanagerUsers.length;
    }
    ...
    function getRMusersLength() external view returns(uint256) {
        return RMUsers.length;
    }
 ...
 function getActiveUpline(address _user) public view returns(bool) {
    ...
    function _calCurDynamicRewards(address _user) private view returns(uint256, uint256) {
       ...
       totalRewards = totalRewards.add(userRewardsf.GM.add(userRewardsf.manager).add(userRewardsf.RM));
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(baseDivider);
        uint256 withdrawable = totalRewards.sub(splitAmt);
        return(withdrawable, splitAmt);
    }
            ...
            if(upline != address(0)){
              ...
              if(i==0 && idstatus==true){
                ...
                if(userInfo[upline].level > 1){
                      reward = newAmount.mul(associatemanagerPercents[i - 1]).div(baseDivider);
                ...
                if(userInfo[upline].level > 2 && i < 5 && idstatus==true){
                  ...
                  upRewards.level4Freezed = upRewards.level4Freezed.add(reward);
                    ...
                    reward = newAmount.mul(RMPercents[i - 5]).div(baseDivider);
               ...
               _updatestatus(upline);
                ...
                RewardInfo storage upRewards = rewardInfo[upline];
                  ...
                  if(i > 0 && i < 3 && userInfo[upline].level > 1 && idstatus==true){
                    if(upRewards.level3Freezed > 0){
                        uint256 level3Reward = newAmount.mul(associatemanagerPercents[i - 1]).div(baseDivider);
                ...
                if(i >= 5 && userInfo[upline].level > 3  && idstatus==true){
                    if(upRewards.level5Left > 0){
                        uint256 level5Reward = newAmount.mul(RMPercents[i - 5]).div(baseDivider);
