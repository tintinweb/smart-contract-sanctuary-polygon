...
contract UHOMToken is BEP20, MinterRole {
    ...
    uint256 public minSwapAmount = 100000000;   //0.1 UHOM tokens
    ...
    /**z
     * @notice Constructs the UHOM token contract.
     */
    constructor(address _treasury, address _lpcontract) public BEP20("UHOMToken", "UHOM") {
        operator = _msgSender();
    ...
    /// @dev overrides transfer function to meet tokenomics of UHOM
    ...
    function transferOperator(address newOperator) public onlyOperator {
        require(newOperator != address(0), "UHOM::transferOperator: new operator is the zero address");
    ...
    function updateBuyFee(uint16 _buyFee) public onlyOperator {
        require( _buyFee <= 100, "UHOM:: Biu Fee can't exceed 100%" );
        emit UpdateBuyFee(_buyFee);
        buyFee = _buyFee;
    }

    function updateSellFee(uint16 _sellFee) public onlyOperator {
        require( _sellFee <= 100, "UHOM:: Sell Fee can't exceed 100%");
    ...
    function updateFeeWallet(address _treasury) public onlyOperator {
        require( _treasury != address(0), "UHOM:: tax fee address is zero" );
