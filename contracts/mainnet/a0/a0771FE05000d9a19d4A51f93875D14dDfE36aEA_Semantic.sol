...
contract Semantic is ISemantic, AccessControl,Semantic721 {
    ...
    mapping(uint256 => Turtle) public tokenData; //todo ==>tokenIdForTurtleMap
    ...
    function soulMint(SoulOperateData[] memory operateDatas) external {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ...
            require(
                tokenDataForId[triple.subjectId][triple.predicateId][triple.object] == 0,
                "already exist"
            );
    ...
    function claim(ClaimOperateData[] memory operateDatas) external {
        for (uint256 i = 0; i < operateDatas.length; i++) {
            ...
            string memory signOriginalData = string.concat(operateData.expireTime.toString(),operateData.subjectId.toString(),
            operateData.predicateId.toString(),operateData.object);
            ...
            Triple memory triple = Triple(operateData.subjectId,
                operateData.predicateId,
                operateData.object);
            _mint(msg.sender, subjectIdForData[operateData.subjectId].value, subjectIdForData[operateData.subjectId].classId, triple);
        }
    }
    ...
    function _mint(
        ...
        tokenDataForId[triple.subjectId][triple.predicateId][triple.object] = tokenId;
    ...
    function _mintTurtle(
        ...
        uint256 subjectId = turtle.subjectId;
        string memory rdf = "";
    ...
    function generateRdf(
        ...
        if (
            predicateIdForData[predicateId].predicateType == FieldType.STRING
        ) {
        ...
        if (subjectId > 0) {
            require(
                _isSubjectOwner(subjectId) ||
                    tokenDataForId[subjectId][OWNER_PREDICATE_ID][ALL_PERMISSION] > 0,
                "not have permission"
            );
        }
    }
    ...
    function _isSubjectOwner(uint256 subjectId) internal virtual returns (bool) {
        return
