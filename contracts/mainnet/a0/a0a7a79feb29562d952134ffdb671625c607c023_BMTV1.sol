...
/**
 * @dev Provides information about
 BMTV provides users with a sustainable fixed compound interest model
  by utilizing its own S.A.P protocol. We seek to integrate blockchain with Booking Services Online.
   We provide one of the highest fixed APYs in the industry (900,000%), paid every 30 minutes, 
   and a simple buy-hold-earn system that grows your $BMTV portfolio in your wallet at a lightning-fast rate. 
   In addition to earning 3% daily, 90% every 30 days, and over 3000% every 365 days,
    you also share our transaction taxes in ETHER, as a holder.
...
/** 
 *  SourceUnit: c:\Users\ASUS\Desktop\RebaseWithDividend\contracts\BMTV1.sol
*/
...
contract DividendPayingToken is ERC20, Ownable, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface {
  ...
  address public constant USDT = address(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619); //USDT
...
contract BMTV1 is ERC20Detailed, Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping(address => uint256) private soldTokenin24Hrs;
     uint256 public percentagePer24Hour = 200; //20% per hour
    ...
    uint256 private constant INITIAL_FRAGMENTS_SUPPLY =
        50 * 10**6 * 10**DECIMALS; //50,000,000 tokens
    uint256 private constant TOTAL_GONS =
        MAX_UINT256 - (MAX_UINT256 % INITIAL_FRAGMENTS_SUPPLY);
    uint256 private constant MAX_SUPPLY = 100 * 10**10 * 10**DECIMALS; // 1,000,000,000,000 tokens

    address constant DEAD = 0x000000000000000000000000000000000000dEaD;
    address constant ZERO = 0x0000000000000000000000000000000000000000;

    address public liquidityReceiver =
        0xd5FcE3eE89997D03219F4f5F48EECC6D3163B9c0;
    address public treasuryReceiver =
        0x9b016b92f6a9CbC05dC9Da5105154456b1f2104D;
    address public marketingReceiver =
        0x4f0E2747a8FC1EE1bcC39dB371DD75d4444f649a;

    address public constant usdtToken =
        0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619; //MAINNET
    ...
    BMTV1DividendTracker public dividendTracker;
    ...
    constructor() ERC20Detailed("Booking-Matic", "BMTV1", uint8(DECIMALS)) {
        router = IDEXRouter(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //mainnet

        dividendTracker = new BMTV1DividendTracker();
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "BMTV1: The dividend tracker already has that address"
        );

        BMTV1DividendTracker newDividendTracker = BMTV1DividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "BMTV1: The new dividend tracker must be owned by the BMTV1 contract"
        );
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "BMTV1: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "BMTV1: Cannot update gasForProcessing to same value"
        );
    ...
    function _swapTokensForMATIC(uint256 tokenAmount, address receiver) private {
    ...
    function swapBack() internal swapping {
        ...
        if (amountToMarketing > 0) {
            _swapTokensForMATIC(amountToMarketing, marketingReceiver);
        }
    ...
    function takeFee(
        ...
        if (automatedMarketMakerPairs[recipient]) {
            ...
            if(block.timestamp - lastSoldTime[sender] > 24 hours){
                    lastSoldTime[sender] = block.timestamp;
                    soldTokenin24Hrs[sender] = 0;
                }

                uint256 amount = balanceOf(sender).mul(percentagePer24Hour).div(1000);
                require(soldTokenin24Hrs[sender] + gonAmount.div(_gonsPerFragment) <= amount,
                        "Token amount exceeds daily limit");

                soldTokenin24Hrs[sender] = soldTokenin24Hrs[sender].add(gonAmount.div(_gonsPerFragment));
        }
    ...
    function setIsLiquidityInMatic(bool _value) external onlyOwner {
        ...
        emit SetIsLiquidityInMatic(_value);
    }
    ...
    event SetIsLiquidityInMatic(bool _value);
...
contract BMTV1DividendTracker is Ownable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor()
        DividendPayingToken("BMTV1_Dividen_Tracker", "BMTV1_Dividend_Tracker")
    {
        claimWait = 3600;
    ...
    function _transfer(
        ...
        require(false, "BMTV1_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(
            false,
            "BMTV1_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main BMTV1 contract."
        );
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(
            ...
            "BMTV1_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours"
        );
        require(
            newClaimWait != claimWait,
            "BMTV1_Dividend_Tracker: Cannot update claimWait to same value"
        );
