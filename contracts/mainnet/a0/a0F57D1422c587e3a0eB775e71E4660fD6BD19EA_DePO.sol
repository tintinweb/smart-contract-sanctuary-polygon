...
abstract contract DePOCurrencyDictionary is DePOCountryDictionary {
    ...
    mapping(uint256 => Counters.Counter) private _currencyCounter;
    ...
    function addCurrency(uint256 countryId, string memory name) public onlyOwner() returns (uint256){
        _currencyCounter[countryId].increment();
        uint256 currencyId = _currencyCounter[countryId].current();
...
abstract contract DePOOfferManage is DePODictionary {
    ...
    validateOfferDependencies(countryId, paymentMethodIds, currencyId)
    returns(Offer memory) {
        Token memory token = getToken(tokenId);
        require(token.available, "Token is not available");
        require(amount >= limitBottom, "Amount is lower than bottom limit");
        require(limitTop >= limitBottom, "Top limit should be more or equal to bottom limit");
    ...
    modifier validateOfferDependencies(
        uint256 countryId,
        uint256[] memory paymentMethodIds,
        uint256 currencyId
    ) {
