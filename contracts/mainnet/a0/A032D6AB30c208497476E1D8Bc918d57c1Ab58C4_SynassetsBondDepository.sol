...
contract SynassetsBondDepository is Ownable {
    ...
    struct Bond {
        uint payout; // SYNASSETS remaining to be paid
    ...
    function deposit(
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 SYNASSETS ( underflow protection )
        ...
        /**
            ...
            approved and
            deposited into the treasury, returning (_amount - profit) SYNASSETS
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao
            IERC20( SYNASSETS ).safeTransfer( DAO, fee );
        }
    ...
    function maxPayout() public view returns ( uint ) {
        return IERC20( SYNASSETS ).totalSupply().mul( terms.maxPayout ).div( 100000 );
    }
    ...
    /**
     *  @notice calculate current ratio of debt to SYNASSETS supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {
        uint supply = IERC20( SYNASSETS ).totalSupply();
    ...
    /**
     *  @notice calculate amount of SYNASSETS available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or SYNASSETS) to the DAO
     *  @return bool
     */
    function recoverLostToken( address _token ) external returns ( bool ) {
        require( _token != SYNASSETS );
