...
contract PublicGovSale is Initializable, IPublicGovSale, PausableUpgradeable, OwnableUpgradeable {
  ...
  function buyToken(uint240 _chainID, uint256 _amountIn, address _user) external virtual {
    ...
    chainSold[_chainID] += tokenReceivedFinal;
    ...
    if (sender == address(saleGateway)) {
      ...
      chainUsersBought[_chainID][index] += amountInFinal;
      chainUsersFee[_chainID][index] += feeCharged;
      chainUsersReceived[_chainID][index] += tokenReceivedFinal;
    } else {
