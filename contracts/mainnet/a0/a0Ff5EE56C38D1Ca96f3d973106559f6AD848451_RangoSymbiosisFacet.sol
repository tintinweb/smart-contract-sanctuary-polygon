...
contract RangoSymbiosisFacet is IRango, ReentrancyGuard, IRangoSymbiosis {
    ...
    address private symbiosisMetaRouter;
    /// @notice The address of symbiosis meta router gateway contract
    address private symbiosisMetaRouterGateway;
    ...
    /// @param metaRouter address of Symbiosis MetaRouter 
    /// @param metaRouterGateway address of Symbiosis MetaRouterGateway
    constructor(address metaRouter, address metaRouterGateway) {
    ...
    function messageReceive(
        ...
        (address receivedToken, uint dstAmount, LibInterchain.OperationStatus status) = LibInterchain.handleDestinationMessage(token, amount, receivedMessage);
    ...
    function doSymbiosisBridge(
        ...
        require(symbiosisMetaRouter != LibSwapper.ETH, 'Symbiosis meta router address not set');
        require(symbiosisMetaRouterGateway != LibSwapper.ETH, 'Symbiosis meta router gateway address not set');
        require(token != LibSwapper.ETH, 'Symbiosis contract handles only ERC20 tokens');

        SafeERC20.safeTransferFrom(IERC20(token), msg.sender, address(this), amount);
        LibSwapper.approve(token, symbiosisMetaRouterGateway, amount);
    ...
    function updateSymbiosisAddressInternal(address metaRouter, address metaRouterGateway) private {
        ...
        symbiosisMetaRouter = metaRouter;
        symbiosisMetaRouterGateway = metaRouterGateway;
