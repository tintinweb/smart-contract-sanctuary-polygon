...
contract BetCoin is Context, IERC20, Ownable {
    ...
    uint256 public _maxOutAmount = 20000 * 10**_decimals;    // 20k  - Initial max sell / WITHDRAW by user
    ...
    string private constant _name = "MXBBB";
    string private constant _symbol = "MXBBBCOIN";
    ...
    /*  1% rfi/holders, 4% mkt, 5% liquidity, 0% burn  = 10% */
    ...
    address public walletGameAddress;
    address public wallet_presale;    
    bool public Trading = false;
    bool inSwapAndLiquify;
    bool private _EnableTransferFrom = true;
    bool public swapAndLiquifyEnabled = true;


    event SwapAndLiquifyEnabledUpdated(bool enabled);
    event LiquidityAdded(uint256 e_tokenAmount, uint256 e_bnbAmount);
    event EventSetWalletPreSale( address e_wallet_presale);
    event EventSetWalletMarketing( address e_marketingAddress);
    event EventSetBuyRates(uint256 e_rfi, uint256 e_marketing, uint256 e_liquidity, uint256 e_burn);
    event EventSetSellRates(uint256 e_rfi, uint256 e_marketing, uint256 e_liquidity, uint256 e_burn);
    ...
    constructor () {
        ...
        //  Polygon Mainnet - QuickSwapRouter: https://polygonscan.com/address/0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff
            IQuickswapV2Router02 _QuickSwapV2Router = IQuickswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); // mainnet
        //  Mumbai Testnet - QuickSwapRouter: https://mumbai.polygonscan.com/address/0x8954AfA98594b838bda56FE4C12a09D7739D179b
        //  IQuickswapV2Router02 _QuickSwapV2Router = IQuickswapV2Router02(0x8954AfA98594b838bda56FE4C12a09D7739D179b); // Testnet
        ...
        marketingAddress = payable(0xAdcddde4D6307A13946d3c3bE1B2Ed0eE2323f0a); // wallet address that will receive funds from fees
        ...
        _isExcluded[quickswapV2Pair] = true;
        _excluded.push(quickswapV2Pair);
    ...
    function setWalletPreSale(address account) public onlyOwner {
        ...
        emit EventSetWalletPreSale( wallet_presale);
    }    
    ...
    function setSellRates(uint256 rfi, uint256 marketing, uint256 liquidity, uint256 burn) public onlyOwner {
        ...
        sellRates.burn = burn;
        emit EventSetSellRates(sellRates.rfi, sellRates.marketing, sellRates.liquidity, sellRates.burn);
    }

    function setMarketingAddress(address payable  _marketingAddress) public onlyOwner {
        marketingAddress = _marketingAddress;
        emit EventSetWalletMarketing(marketingAddress);
    }
 ...
 function _transfer(address from, address to, uint256 amount) private {
		...
		if(_EnableTransferFrom == true){
			...
			if (contractBalance.lp_balance>= numTokensToSwap && !inSwapAndLiquify && from != quickswapV2Pair && swapAndLiquifyEnabled) {
			    swapAndLiquify(numTokensToSwap);
		    }

		    if (contractBalance.marketing_balance>= numTokensToSwap && !inSwapAndLiquify && from != quickswapV2Pair && swapAndLiquifyEnabled) {
		...
		if(_EnableTransferFrom == false){
			// If any holder tries to make a sale or transfer before enabling the contract, the transferred tokens are burned. This function will be disabled after launch
    ...
    function swapAndSendToMarketing(uint256 tokenAmount) private lockTheSwap {
        ...
        if(allowance(address(this), address(QuickSwapV2Router)) < tokenAmount) {
            _approve(address(this), address(QuickSwapV2Router), ~uint256(0));
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 bnbAmount) private {
        ...
        QuickSwapV2Router.addLiquidityETH{value: bnbAmount}(
            address(this),
            tokenAmount,
            0,
            0,
