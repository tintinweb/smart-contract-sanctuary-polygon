...
contract LuckyMint is Pausable, AccessControl{
    ...
    function getDollars(uint dollars) public view returns(uint256){
    ...
    function BuyTickets(uint tickets) public payable {
        oneDollar = getDollar();
        require(msg.value >= (oneDollar*tickets - 100000), "Price should be greater than a dollar for each ticket");
        require(block.timestamp < lotteryPeriod, "Lottery Period Ended: No More buying allowed");
        require(block.timestamp > startDay, "Lottery Not Started: Wait for the lottery to start");
