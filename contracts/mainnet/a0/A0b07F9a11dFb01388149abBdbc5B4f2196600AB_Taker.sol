 ...
 * ensures fee is swapped to native token (ETH,MATIC) at fill
 * swapping to native is optional, otherwise dstToken is sent to owner
 * All Fee struct properties are optional
 * The Fee struct can be moved offchain to save gas (~100k), but added here to simplify taker offchain implementation
 */
contract Taker is Ownable {
    ...
    mapping(uint256 => Fee) public fees; // id => Fee
    ...
    function bid(
        ...
        fees[id] = fee;
        TWAP(twap).bid(id, exchange, data, fee.dstTokenAmount);
    }

    function fill(uint256 id) external onlyOwner {
        Fee memory fee = fees[id];
        OrderLib.Order memory o = TWAP(twap).order(id);

        TWAP(twap).fill(id);

        if (fee.exchange != address(0)) {
            ERC20(o.ask.dstToken).safeIncreaseAllowance(fee.exchange, type(uint256).max);
            IExchange(fee.exchange).swap(ERC20(o.ask.dstToken).balanceOf(address(this)), fee.minAmountOut, fee.data);
        }
