...
interface IStarNFT {
    ...
    // mint
    function mint(address account, uint256 powah) external payable returns (uint256);
    function mintBatch(address account, uint256 amount, uint256[] calldata powahArr) external payable returns (uint256[] memory);
...
contract StarNFTV3 is ERC721, IStarNFT, Ownable {
    ...
    {
        if(msg.sender != owner()){
            require(msg.value >= 5 ether || minters[msg.sender] == true);
            }
        _starCount++;
    ...
    ) external payable override onlyMinter returns (uint256[] memory) {
