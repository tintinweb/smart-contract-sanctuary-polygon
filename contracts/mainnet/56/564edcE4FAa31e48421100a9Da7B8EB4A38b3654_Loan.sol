...
contract Loan is AccessProtected, ReentrancyGuard, BaseRelayRecipient, IERC721Receiver, EIP712Base, Pausable {
    ...
    constructor(
        ...
        require(_treasuryPercentage <= 10000, "Percentage cannot be greater than 100%");
        token = _token;
    ...
    function updateMinTimePeriod(uint256 _timePeriod) external onlyAdmin {
        require(_timePeriod > 0, "Incorrect time period");
        require(_timePeriod < maxLoanPeriod, "time period greater than maxLoanPeriod");
    ...
    function cancelLoan(uint256 _loanId) external nonReentrant whenNotPaused {
        ...
        require(loanItems[_loanId].owner != address(0), "Loan item doesn't exist");
        require(!loanItems[_loanId].canceled, "Loan already canceled");
        require(
            _sender == loanItems[_loanId].owner || _sender == loanItems[_loanId].loanee,
            "Sender is not the owner, or loanee"
        );
        if (loanItems[_loanId].upfrontFee != 0 && loanItems[_loanId].endTime > block.timestamp) {
            require(_sender == loanItems[_loanId].loanee, "only loanee can cancel loan when fee is greater than 0");
        }
    ...
    function emergencyWithdrawal(
        ...
        for (uint256 i = 0; i < _nftIds.length; i++) {
            require(nftOnLoan[_nftAddresses[i]][_nftIds[i]] == false, "NFT is on loan");
            address nftOwner = IERC721(_nftAddresses[i]).ownerOf(_nftIds[i]);
