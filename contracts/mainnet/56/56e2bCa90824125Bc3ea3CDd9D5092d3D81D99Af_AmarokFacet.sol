...
import {LibBridge, UserBridgeData, PaymentParameters, ILiFi, ILiFiDiamond, LibSwap, AmarokData, Storage, PaymentFunctionParameters} from "../Libraries/LibBridge.sol";
...
contract AmarokFacet {
    ...
    event PaymentBridgeSubscription(
        ...
        PaymentFunctionParameters indexed info
    );
    ...
    function paymentWithBridgeWithAmarok(
        PaymentFunctionParameters memory paymentParams,
        address user,
        ...
        emit PaymentBridgeSubscription(user, paymentParams);
    }
...
import {LibBridge, PaymentParameters, LibSwap, PaymentFunctionParameters} from "./LibBridge.sol";
...
struct PaymentFunctionParameters {
    uint256 destChainId;
    uint256 subscriptionManagerId;
    uint8 subscriptionId;
    uint256 priceInSubToken;
...
library LibBridge {
    ...
    function tokenPayment(
        ...
        require(
            getStorage()
            .users[paymentParams.destChainId][
                paymentParams.subscriptionManagerId
            ][user].subscriptionLimit >= paymentParams.priceInSubToken,
            "Amount too high"
        );
        ...
        getStorage()
        .users[paymentParams.destChainId][paymentParams.subscriptionManagerId][
            user
        ].nextPaymentTime =
            block.timestamp +
            getStorage()
    ...
    function verifyRenew(
        ...
        require(
            ...
            .users[paymentParams.destChainId][
                paymentParams.subscriptionManagerId
            ][user].nextPaymentTime < block.timestamp,
        ...
        return
            string(
                abi.encodePacked(
                    ...
                    Strings.toString(originChainId),
                    "\nDest Chain:",
