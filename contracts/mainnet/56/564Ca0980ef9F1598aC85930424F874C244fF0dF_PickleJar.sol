...
contract PickleJar is ERC721, AdminGame {
    ...
    IPickleEscape pickleEscape;

    constructor(string memory baseURI) ERC721("PickleJar", "JAR"){
        baseTokenURI = baseURI;
    }

    function setPickleEscapeGameContract(address _pickleEscape) public onlyOwner {
        setAdminAddress(address(pickleEscape), false);
        pickleEscape = IPickleEscape(_pickleEscape);
        setAdminAddress(_pickleEscape, true);
    }

    function mint(address _to) public onlyAdmin {

        require(_to != address(0), "TE");

        uint256 tokenId = totalSupply;

        require(!_exists(tokenId), "TAM");

        _owners[uint32(tokenId)] = _to;
        _balances[_to] += 1;
    ...
    function burn(uint256 _tokenId) public onlyAdmin {
        ...
        delete _owners[uint32(_tokenId)];

        emit Transfer(owner, address(0), _tokenId);
    }

    function airdrop(address[] memory _to) public onlyAdmin{
    ...
    function walletOfOwner(address _owner) external view virtual returns (uint32[] memory) {
        ...
        for (uint32 tokenId = 0; tokenId < totalSupply; tokenId++) {
    ...
    function balanceOfMany(address[] memory owners) public view returns(uint256[] memory) {
    ...
    function setTransfer(bool _transferEnable) public onlyOwner {
    ...
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override {
        require(transferEnable == true, "Transfers are disable");
        pickleEscape.transfer(from, to, tokenId);
    }
}
