...
contract PlayerFacet is Modifiers {
    ...
    function fixLootboxesByPlayer(address playerAddress) external onlyGuildAdminOrGallion playerExists(playerAddress) {
        uint256[] memory totalMintedLoootboxesByPlayer = s.totalMintedLoootboxesByPlayer[playerAddress];
        uint256[] memory totalOpenedLoootboxesByPlayer = s.totalOpenedLoootboxesByPlayer[playerAddress];
        s.totalMintedLoootboxesByPlayer[playerAddress] = new uint256[](s.rarities.length + 1);
        s.totalOpenedLoootboxesByPlayer[playerAddress] = new uint256[](s.rarities.length + 1);
        for (uint i = 0; i < s.rarities.length + 1; i++) {
            s.totalMintedLoootboxesByPlayer[playerAddress][i] = totalMintedLoootboxesByPlayer[i];
            s.totalOpenedLoootboxesByPlayer[playerAddress][i] = totalOpenedLoootboxesByPlayer[i];
        }
    }
