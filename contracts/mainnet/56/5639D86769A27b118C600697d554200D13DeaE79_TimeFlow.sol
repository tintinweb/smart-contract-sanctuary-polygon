...
contract TimeFlow is Ownable,ReentrancyGuard{
    ...
    function senderStake(string calldata contentName)external lockState nonReentrant{
        ...
        require(doThisTransfer[msg.sender][contentName]!=true,"The initiator has already contracted the transaction!"); 
    ...
    function pointStake(string calldata contentName)external lockState nonReentrant{
        ...
        require(getTime(contentName)>=600,"time error"); 
        require(userBailMoney[msg.sender][contentName]==0&&receiverIfStaking[msg.sender][contentName]==false,"The user has transferred the corresponding mone to the contract"); 
    ...
    //The sender needs to get back the original money(failed to pass general examination or passed disciplinary matters)
    function sendWithdraw(string calldata contentName)external nonReentrant{
        ...
        require(msg.sender==getSource(contentName),"Sender error"); 
        require(pointState(contentName)!=true,"trade error"); 
        require(getTime(contentName)==0,"time error"); 
        require(doThisTransfer[msg.sender][contentName],"The initiator has already contracted the transaction!"); 
    ...
    //The recipient of the transaction reclaiming the pledged margin (passed the general examination and failed the disciplinary matters)
    function pointWithdraw(string calldata contentName)external nonReentrant{
        ...
        IERC20 thisToken=IERC20(tradeToken(contentName));
        require(userBailMoney[msg.sender][contentName]>0,"The user not transferred the corresponding mone to the contract"); 
    ...
    //Execute the corresponding token time stream (pass the general review and fail the disciplinary transaction)
    function doTimeFlow(string calldata contentName)external nonReentrant{
        ...
        require(pointState(contentName),"The sender or receiver does not agree to the transaction!");
        require(getTime(contentName)==0,"time <= end time"); 
    ...
    //The initiator draws liquidated damages (failed to pass the general examination and passed the disciplinary matters)
