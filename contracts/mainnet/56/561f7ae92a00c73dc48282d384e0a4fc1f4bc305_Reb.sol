...
contract Reb is ERC20PresetMinterPauser {
    constructor () ERC20PresetMinterPauser("Rebellion DAO Token", "REB") {
    ...
    function batchMint(address[] memory _to_list, uint256[] memory _values) public {
        require(_to_list.length == _values.length);
        for (uint i = 0; i < _to_list.length; i++) {
            mint(_to_list[i], _values[i]);
        }
    }
}