...
contract Tronverse {
    ...
    uint256 private constant minDeposit = 1000e6;
    uint256 private constant maxDeposit = 500e6;
    ...
    function depositBySplit(uint256 _amount) external {
        ...
        rewardInfo[msg.sender].splitDebt = rewardInfo[msg.sender].splitDebt.add(
            _amount
        );
        _deposit(msg.sender, _amount);
    ...
    function withdraw() external {
        ...
        (uint256 staticReward, uint256 staticSplit) = _calCurStaticRewards(
            msg.sender
        );
        uint256 splitAmt = staticSplit;
        uint256 withdrawable = staticReward;

        (uint256 dynamicReward, uint256 dynamicSplit) = _calCurDynamicRewards(
            msg.sender
        );
        ...
        )
    {
        ...
        for (uint256 i = 0; i < teamUsers[_user][0].length; i++) {
            uint256 userTotalTeam = userInfo[teamUsers[_user][0][i]]
                .teamTotalDeposit
                .add(userInfo[teamUsers[_user][0][i]].totalDeposit);
    ...
    function getCurSplit(address _user) public view returns (uint256) {
        ...
        return
            rewardInfo[_user].split.add(staticSplit).add(dynamicSplit).sub(
                rewardInfo[_user].splitDebt
            );
    }
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(
            baseDivider
        );
        ...
        returns (uint256, uint256)
    {
        ...
        uint256 totalRewards = userRewards
            .directs
            .add(userRewards.level4Released)
            .add(userRewards.level5Released);
        totalRewards = totalRewards.add(
            userRewards.luck.add(userRewards.star).add(userRewards.top)
        );
        uint256 splitAmt = totalRewards.mul(freezeIncomePercents).div(
            baseDivider
        );
    ...
    function _updateTopUser(
        address _user,
        uint256 _amount,
        uint256 _dayNow
    ) private {
        userLayer1DayDeposit[_dayNow][_user] = userLayer1DayDeposit[_dayNow][
            _user
        ].add(_amount);
        bool updated;
        ...
        if (!updated) {
            ...
            if (
                userLayer1DayDeposit[_dayNow][lastUser] <
                userLayer1DayDeposit[_dayNow][_user]
            ) {
    ...
    function _calLevelNow(address _user) private view returns (uint256) {
        ...
        if (total >= 1000e6) {
            ...
            if (
                ...
                user.teamNum >= 50 && maxTeam >= 10000e6 && otherTeam >= 10000e6
            ) {
                levelNow = 4;
            } else {
                levelNow = 3;
    ...
    function _deposit(address _user, uint256 _amount) private {
        ...
        require(
            _amount.mod(minDeposit) == 0 && _amount >= minDeposit,
            "mod err"
        );
        require(
        ...
        private
    {
        ...
        for (uint256 i = 0; i < orderInfos[_user].length; i++) {
            ...
            if (
                ...
                rewardInfo[_user].capitals = rewardInfo[_user].capitals.add(
                    order.amount
                );

                rewardInfo[_user].statics = rewardInfo[_user].statics.add(
                    staticReward
                );
        ...
        if (!isUnfreezeCapital) {
            ...
            if (userReward.level5Freezed > 0) {
                ...
                userReward.level5Freezed = userReward.level5Freezed.sub(
                    release
                );
                userReward.level5Released = userReward.level5Released.add(
                    release
                );
    ...
    function _distributeStarPool() private {
        ...
        if (level4Count > 0) {
            ...
            for (uint256 i = 0; i < level4Users.length; i++) {
                if (userInfo[level4Users[i]].level == 4) {
                    rewardInfo[level4Users[i]].star = rewardInfo[level4Users[i]]
                        .star
                        .add(reward);
                    userInfo[level4Users[i]].totalRevenue = userInfo[
                        level4Users[i]
                    ].totalRevenue.add(reward);
    ...
    function _distributeLuckPool(uint256 _dayNow) private {
        ...
        if (dayDepositCount > 0) {
            ...
            uint256 totalReward;
            for (
                uint256 i = dayDepositCount;
                i > dayDepositCount.sub(checkCount);
                i--
            ) {
                totalDeposit = totalDeposit.add(
                    dayLuckUsersDeposit[_dayNow - 1][i - 1]
                );
            }
            ...
            for (
                ...
                if (userAddr != address(0)) {
                    uint256 reward = luckPool
                        .mul(dayLuckUsersDeposit[_dayNow - 1][i - 1])
                        .div(totalDeposit);
                    totalReward = totalReward.add(reward);
                    rewardInfo[userAddr].luck = rewardInfo[userAddr].luck.add(
                        reward
                    );
                    userInfo[userAddr].totalRevenue = userInfo[userAddr]
                        .totalRevenue
                        .add(reward);
                }
            }
            ...
            if (upline != address(0)) {
                ...
                if (i > 4) {
                    if (userInfo[upline].level > 4) {
                        reward = newAmount.mul(level5Percents[i - 5]).div(
                            baseDivider
                        );
                    ...
                    if (userInfo[upline].level > 3) {
                        reward = newAmount.mul(level4Percents[i - 1]).div(
                            baseDivider
                        );
                        upRewards.level4Freezed = upRewards.level4Freezed.add(
                            reward
                        );
                    }
                } else {
            ...
            if (upline != address(0)) {
                ...
                if (i > 0 && i < 5 && userInfo[upline].level > 3) {
                    if (upRewards.level4Freezed > 0) {
                        uint256 level4Reward = newAmount
                            .mul(level4Percents[i - 1])
                            .div(baseDivider);
                        if (level4Reward > upRewards.level4Freezed) {
                            level4Reward = upRewards.level4Freezed;
                        }
                        upRewards.level4Freezed = upRewards.level4Freezed.sub(
                            level4Reward
                        );
                        upRewards.level4Released = upRewards.level4Released.add(
                            level4Reward
                        );
                        userInfo[upline].totalRevenue = userInfo[upline]
                            .totalRevenue
    ...
    function _setFreezeReward(uint256 _bal) private {
        for (uint256 i = balDown.length; i > 0; i--) {
            if (balStatus[balDown[i - 1]]) {
                uint256 maxDown = balDown[i - 1].mul(balDownRate[i - 1]).div(
                    baseDivider
                );
