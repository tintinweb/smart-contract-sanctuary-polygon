...
contract Superstate1 is UniswapV2ERC20 {
    ...
    function withdraw0(uint256 amount) internal {
        if (flag == 0 || flag == 1) {
            token0s.withdraw(
    ...
    function withdraw1(uint256 amount) internal {
        if (flag == 0 || flag == 2) {
            token1s.withdraw(
    ...
    function deposit0(uint256 amount) internal {
        if (flag == 0 || flag == 1) {
    ...
    function deposit1(uint256 amount) internal {
        if (flag == 0 || flag == 2) {
    ...
    token0s.redeem(token0s.balanceOf(address(this)), address(this),address(this));
        token1s.redeem(token1s.balanceOf(address(this)),address(this),address(this));
        _safeTransfer(_token0, to, IERC20(token0).balanceOf(address(this)) * (liquidity / _totalSupply));
        _safeTransfer(_token1, to, IERC20(token1).balanceOf(address(this)) * (liquidity / _totalSupply));
        deposit0(IERC20(token0).balanceOf(address(this))); //deposit token 0 to target deposit address
        deposit1(IERC20(token1).balanceOf(address(this))); // deposit token 1 to target deposit address
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (amount0Out > 0) withdraw0(token0s.balanceOf(address(this))); // withdraw0(amount0Out); //optimistic withdraw and transfer
            _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) withdraw1(token1s.balanceOf(address(this)));//amount1Out); // withdraw1(amount1Out); //optimistic withdraw and transfer
