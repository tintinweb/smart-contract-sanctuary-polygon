...
interface IMaxityMarketPlaceV3{
    ...
    function bid(address _token,uint256 _tokenid,uint256 amount,address to) external;

    function auctionOnFixPrice(address _token,uint _tokenid,uint _price,address price_offer,uint deadline)  external returns(uint);
    
    // function agreeAuctionOnFixPrice(address _token,uint _tokenid) external;

    function agreeAuctionOnFixPrice(address _token,uint _tokenid, uint _offeridx) external;
