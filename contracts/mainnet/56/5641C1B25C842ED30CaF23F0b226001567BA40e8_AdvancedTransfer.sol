...
Note: X 1,2,3,4,5 are up to 5 recipient addresses
*/
...
contract AdvancedTransfer is BaseSiloAction {
    address private constant ERC20_LINK_ADDRESS =
        0x53E0bca35eC356BD5ddDFebbD1Fc0fD03FaBad39; //Mainnet ERC20 _LINK_ADDRESS
    address private constant ERC677_LINK_ADDRESS =
        0xb0897686c545045aFc77CF20eC7A532E3120E0F1;

    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],uint[4],address[4],address[],address";
    ...
    function transferAction(
...

        {
            ...
            address _oracle;
            (input, ratio, recipients, _path, _oracle) = abi.decode(
                configuration,
                (address[5], uint256[4], address[4], address[], address)
            );
            info = ActionInfo({
                inputs: input,
                outputs: output,
                ...
                ratio: ratio,
                path: _path,
                oracle: IExchangeRateOracle(_oracle),
                alpha: 0,
                beta: 0,
                upkeep: 0,
...

        {
            ...
            for (uint256 i; i < 4; ) {
                ...
                if (info.recipients[i] == address(0)) {
                    ...
                    IAction(implementation).getIsSilo(info.recipients[i])
                ) {
                    //sending to a silo
                ...
                if (potentialSilo == address(0)) {
                    ...
                    address[5] memory siloTokens = ISilo(potentialSilo)
                        .getInputTokens();
                    for (uint256 j; j < 5; ) {
        ...
        if (
            info.upkeep > 0 &&
            info.path.length > 1 &&
            info.siloManager != address(0)
        ) {
            address[] memory path = new address[](info.path.length + 1);

            for (uint256 index; index < info.path.length; ) {
        ...
        for (uint256 i; i < 4; ) {
            ...
            if (
                ...
                if (info.outputs[i] == info.inputs[0]) {
                    ...
                    outputAmounts[i] =
                        (amounts[0] * info.ratio[i]) /
                        info.alpha;
                } else if (info.outputs[i] != ERC20_LINK_ADDRESS) {
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _ratio, _recipients, _path, _oracle);
    }
    ...
    {
        (inputs, ratio, recipients, path, oracle) = abi.decode(
            configData,
            (address[5], uint256[4], address[4], address[], address)
        );
    }
