...
contract RangoHop is IRangoHop, BaseProxyContract {
    ...
    mapping(address => bool) public hopBridges;
    ...
    function initialize(address _nativeWrappedAddress, address[] calldata _addresses) public initializer {
        BaseProxyStorage storage baseStorage = getBaseProxyContractStorage();
    ...
    function addHopBridges(address[] calldata _addresses) public onlyOwner {
        addHopBridgesInternal(_addresses);
    }
    ...
    function removeHopBridges(address[] calldata _addresses) external onlyOwner {
        for (uint i = 0; i < _addresses.length; i++) {
            delete hopBridges[_addresses[i]];
        }

        emit HopBridgesRemoved(_addresses);
    }

    /// @inheritdoc IRangoHop
    function hopBridge(
        HopRequest memory _request,
        address _fromToken,
        uint _amount
    ) external override payable whenNotPaused nonReentrant {
        require(hopBridges[_request.bridgeAddress], "Requested hop address not whitelisted");
        if (_fromToken == NULL_ADDRESS) {
            require(msg.value >= _amount, "Insufficient ETH sent for bridging");
        } else {
            SafeERC20Upgradeable.safeTransferFrom(IERC20Upgradeable(_fromToken), msg.sender, address(this), _amount);
            approve(_fromToken, _request.bridgeAddress, _amount);
        }

        IHop hop = IHop(_request.bridgeAddress);
        if (_request.actionType == HopActionType.SWAP_AND_SEND) {
            hop.swapAndSend{value: msg.value}(
                _request.chainId,
                _request.recipient,
                _amount,
                _request.bonderFee,
                _request.amountOutMin,
                _request.deadline,
                _request.destinationAmountOutMin,
                _request.destinationDeadline
            );
        } else if (_request.actionType == HopActionType.SEND_TO_L2) {
