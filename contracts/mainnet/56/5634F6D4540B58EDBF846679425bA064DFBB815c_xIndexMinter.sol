...
//        if (_useNative) {
//            collateral_price = getNativePrice(_cid);
//        } else {
//            collateral_price = getLpPrice(_cid);
//        }
        _xIndex_amount = _amount.mul(collateral_price).div(PRICE_PRECISION).mul(collateral.nativeMissingDecimals);
    }
    ...
    function convertNativeToLp(uint256 _cid, uint256 _amount) public returns (uint256 _lpAmount) {
        Collateral memory _collateral = collateralList[_cid];
    ...
    // Mint function process
    // calculate input_amount_value
    // calculate output xindex amount based on input_amount_value
    // transfer amount in
    // mint xIndex
}
