 ...
 */
contract ERC721Properties is ERC721Upgradeable, OwnableUpgradeable, App, __with_semver(uint8(1), uint8(0), uint8(2)) {
    using Strings for uint256;
    ...
    function mint(address to, bytes32 tierValue) external requirePermission(CONTROLLER) {
        ...
        if (randomShuffleMintByTier || randomMintByTier) {
            if (address(vrfCoordinator) != address(0)) {
                ...
                uint256 rand = Pseudorand.pseudorand(abi.encodePacked(to));
...
contract ERC721PaymentOpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(0), uint8(0)) {
    ...
    {
        require(
            UniswapV3Oracle(_oracleAddress).isWhitelistTokenAddress(tokenAddress),
            Exceptions.UNSUPPORTED_TOKEN_ADDRESS
        );
        uint256 cost = tierPrices[tierValue];
        require(cost != 0, Exceptions.NOT_FOUND_PRICE_FOR_TIER);

        require(tokenSaleAddress != address(0), Exceptions.BASIC_TOKEN_ADDRESS_NOT_SET);
        uint256 price = UniswapV3Oracle(_oracleAddress).getTradePrice(tokenSaleAddress, tokenAddress);
    ...
    function buy(
        ...
        (, uint256 costInBuyTokens) = getPrice(tokenBuyAddress, tierValue);
        require(costInBuyTokens != 0, Exceptions.VALIDATION_ERROR);

        _claim();
        address nativeTokenWrapperAddress = UniswapV3Oracle(_oracleAddress).nativeTokenWrapperAddress();
        if (nativeTokenWrapperAddress != tokenBuyAddress) {
        ...
        address appAddress = kernel.getAppAddress(AppsIds.ERC721);
        ERC721Properties(appAddress).mint(to, tierValue);
        _afterBuy(to, appAddress, tierValue, costInBuyTokens, "ERC20_SALE", __semver());
    }
    ...
    function _tokenTransferFromToTreasury(
        ...
        address treasury = kernel.getAppAddress(AppsIds.TREASURY);
...
contract ACL is BaseStorage, IACL {
    ...
    /**
     * @dev Internally calcualtes indexes: row defines by dividing index on ACL_SLOT_SIZE;
     * column defines by dividing withount remainder index % ACL_SLOT_SIZE.
    ...
    __with_semver(uint8(1), uint8(0), uint8(0))
{
    ...
    address public constant nativeTokenWrapperAddress = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;

    uint32 private _twapInterval;
    mapping(address => uint24) private _whitelistTokens;
    ...
    function initialize(
        ...
        _twapInterval = twapInterval_;
        _transferOwnership(owner_);
    }

    function upgrade(address newImpl) external onlyOwner {
        _getImplementationSlot().value = newImpl;
    }
    ...
    function setWhitelistTokenAddress(address[] calldata tokenAddress_, uint24[] calldata fee_) external onlyOwner {
        require(tokenAddress_.length == fee_.length, Exceptions.VALIDATION_ERROR);
        for (uint256 i; i != tokenAddress_.length; ++i) {
            _whitelistTokens[tokenAddress_[i]] = fee_[i];
        }
    }

    function removeWhitelistTokenAddress(address[] calldata tokenAddress_) external onlyOwner {
        for (uint256 i; i != tokenAddress_.length; ++i) {
            delete _whitelistTokens[tokenAddress_[i]];
        }
    }

    function isWhitelistTokenAddress(address tokenAddress) external view returns (bool) {
        return (_whitelistTokens[tokenAddress] != 0) ? true : false;
    }

    function getTradePrice(address tokenSalePrice, address tokenBuyPrice) external view returns (uint256 price) {
        uint24 basicFeeA = _whitelistTokens[tokenSalePrice];
        uint24 basicFeeB = _whitelistTokens[tokenBuyPrice];
        require(basicFeeA != 0 && basicFeeB != 0, Exceptions.VALIDATION_ERROR);
        ...
        address poolAddress = uniswapFactory.getPool(
            tokenSalePrice,
            tokenBuyPrice,
            (basicFeeA > basicFeeB) ? basicFeeA : basicFeeB
        );
    ...
    function _getTradePrice(address uniswapV3Pool, address tokenBuyPrice) internal view returns (uint256 price) {
        uint160 sqrtPriceX96;
        ...
        if (_twapInterval == 0) {
            ...
            sqrtPriceX96 = UniswapMath.getSqrtRatioAtTick(
                int24((tickCumulatives[1] - tickCumulatives[0]) / int32((_twapInterval)))
            );
        }
...
import "../apps/ERC721Sale/ERC721PaymentOpenSale.sol";
import "./Models.sol";
import "../libraries/Semver.sol";
    ...
    __with_semver(uint8(1), uint8(1), uint8(0))
{
    ...
    function deploy(
        ...
        for (uint256 i = 0; i < modules.length; i++) {
            ...
            } else if (module == Models.AdditionalModules.PaymentOpenSale) {
                _deployPaymentOpenSale(kernel, erc721, deploymentSettings.paymentOpenSaleSettings);
            }
        }
        ...
        returns (AdminController)
    {
        ...
        AdminController admin = AdminController(
            kernel.deployApp(
                ...
                adminCode,
                abi.encodeCall(
                    AdminController(adminCode).initialize,
                    (kernel, adminSetting.admins, adminSetting.creator)
                )
            )
        );
    ...
    function _deployPaymentOpenSale(
        Kernel kernel,
        ERC721Properties erc721,
        Models.SaleSettings calldata paymentOpenSaleSettings
    ) internal returns (ERC721PaymentOpenSale) {
        address paymentOpenSaleCode = updateManager.getLastAppCode(AppsIds.ERC721_PAYMENT_OPEN_SALE);
        ERC721PaymentOpenSale paymentOpenSale = ERC721PaymentOpenSale(
            kernel.deployApp(
                AppsIds.ERC721_PAYMENT_OPEN_SALE,
                paymentOpenSaleCode,
                abi.encodeCall(ERC721PaymentOpenSale(paymentOpenSaleCode).initialize, (kernel))
            )
        );
        ...
        kernel.addPermission(
            AppsIds.SUDO,
            AppsIds.ERC721_PAYMENT_OPEN_SALE,
            ERC721PaymentOpenSale(paymentOpenSale).CONTROLLER()
        );
        kernel.addPermission(
            AppsIds.ADMIN_CONTROLLER,
            AppsIds.ERC721_PAYMENT_OPEN_SALE,
        ...
        kernel.addPermission(AppsIds.ERC721_PAYMENT_OPEN_SALE, AppsIds.ERC721, ERC721Properties(erc721).CONTROLLER());

        paymentOpenSale.setPaymentPolicy(paymentOpenSaleSettings.tiersValues, paymentOpenSaleSettings.tiersPrices);
        paymentOpenSale.setActive(true);

        paymentOpenSale.setTokenSaleAddress(paymentOpenSaleSettings.tokenSaleAddress);

        return paymentOpenSale;
    }
}
...
library Models {
    ...
    struct DeploymentSettings {
        ...
        SaleSettings paymentOpenSaleSettings;
    }
