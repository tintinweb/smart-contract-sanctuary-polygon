...
// File: contracts/helpers/interfaces/ICLOUTERC20.sol

pragma solidity ^0.8.4;

interface ICLOUTERC20 {
...
// File: contracts/regular/LimelightTreasury.sol
...
contract LimelightTreasury is Ownable {
  ...
  address public sCLOUT;
  uint256 public sCLOUTQueue; // Delays change to sCLOUT address
  ...
  /**
        @notice allow approved address to deposit an asset for CLOUT
        @param _amount uint
  ...
  function deposit(
    ...
    // mint CLOUT needed and store amount of rewards for distribution
    send_ = value.sub(_profit);
    IERC20Mintable(CLOUT).mint(msg.sender, send_);
  ...
  /**
        @notice allow approved address to burn CLOUT for reserves
        @param _amount uint
        @param _token address
     */
  function withdraw(uint256 _amount, address _token) external {
    ...
    uint256 value = valueOf(_token, _amount);
    ICLOUTERC20(CLOUT).burnFrom(msg.sender, value);
  ...
  function incurDebt(uint256 _amount, address _token) external {
    ...
    uint256 maximumDebt = IERC20(sCLOUT).balanceOf(msg.sender); // Can only borrow against sCLOUT held
  ...
  /**
        @notice allow approved address to repay borrowed reserves with CLOUT
        @param _amount uint
     */
  function repayDebtWithCLOUT(uint256 _amount) external {
    require(isDebtor[msg.sender], "Not approved as a Debtor");

    ICLOUTERC20(CLOUT).burnFrom(msg.sender, _amount);

    debtorBalance[msg.sender] = debtorBalance[msg.sender].sub(_amount);
    totalDebt = totalDebt.sub(_amount);

    emit RepayDebt(msg.sender, CLOUT, _amount, _amount);
  }
  ...
  function mintRewards(address _recipient, uint256 _amount) external {
    ...
    IERC20Mintable(CLOUT).mint(_recipient, _amount);
  ...
  function excessReserves() public view returns (uint256) {
    return totalReserves.sub(IERC20(CLOUT).totalSupply().sub(totalDebt));
  }
  ...
  /**
        @notice returns CLOUT valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
  ...
  {
    if (isReserveToken[_token]) {
      // convert amount to match CLOUT decimals
      value_ = _amount.mul(10**IERC20(CLOUT).decimals()).div(
    ...
    returns (bool)
  {
    ...
    } else if (_managing == MANAGING.SCLOUT) {
      // 9
      sCLOUTQueue = block.number.add(blocksNeededForQueue);
    } else return false;
