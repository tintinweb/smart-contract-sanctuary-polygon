    ...
    UUPSUpgradeable
{
    ...
    function getStakingReward(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 rewardInToken, uint256 rewardInUSDT) {
        rewardInToken = _getStakingReward(_userAddress, _stakingID);
        rewardInUSDT = _tokenToUSDT(rewardInToken);
    }

    function getStakingRewardAll(
        address _userAddress
    ) external view returns (uint256 stakingRewardInToken, uint256 stakingRewardInUSDT) {
        ...
        for (uint8 i; i < stakingIDLength; i++) {
            stakingRewardInToken += _getStakingReward(
                _userAddress,
                stakingIDs[i]
            );
        }

        stakingRewardInUSDT = _tokenToUSDT(stakingRewardInToken);
    }
    ...
    function getUserStakingAPY(
        ...
        userStakingAPY = userCurrentRewardRate;
    ...
    function getUserRewardClaimedByStakingID(
        address _userAddress,
        uint256 _stakingID
    ) external view returns (uint256 rewardClaimedInToken, uint256 rewardClaimedInUSDT) {
        Account storage userAccount = user[_userAddress][_stakingID];
        rewardClaimedInToken = userAccount.rewardClaimed;
        rewardClaimedInUSDT = _tokenToUSDT(rewardClaimedInToken);
    }
    ...
    function getUserTotalRewardClaimed(
        ...
        returns (
            ...
            uint256 totalRewardClaimedInUSDT,
        ...
        )
    {
        ...
        totalRewardClaimedInUSDT = _tokenToUSDT(totalRewardClaimed);
    ...
    function _tokenToUSDT(uint256 _tokenValue) private view returns (uint256) {
        return _tokenValue * 10 ** IERC20_EXTENDED(_usdtContract).decimals() / IPresale(_presaleContract).getPricePerUSD();
    }
