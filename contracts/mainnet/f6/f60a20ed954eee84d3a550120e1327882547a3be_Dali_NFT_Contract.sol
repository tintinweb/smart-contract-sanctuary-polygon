...
contract Dali_NFT_Contract is Ownable, Pausable, ERC721, ERC721Enumerable, ERC721URIStorage, ERC721Burnable {
    ...
    constructor(string memory _name, string memory _symbol, address _token, uint256 _mintPrice)
    ...
    function mintNFT(uint256 nftAmount, uint256 tokenAmount) public payable {
        ...
        TransferHelper.safeTransferFrom(token, msg.sender, owner(), tokenAmount);
        for (uint256 i = 0; i < nftAmount; i++) {
            internalMint(msg.sender);
        }
    }   
