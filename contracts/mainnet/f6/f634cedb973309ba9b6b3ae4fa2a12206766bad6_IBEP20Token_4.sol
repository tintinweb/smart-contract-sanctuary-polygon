...
contract IBEP20Token_4 is Context, IBEP20, Ownable {
    ...
    function updateFee(uint256 _txFee) public onlyOwner {
        require(_txFee < 100);
        _TAX_FEE = _txFee * 100;
    ...
    function _transfer(
        ...
        bool takeFee = true;
        if (_isExcluded[recipient] || _isExcluded[sender] || _TAX_FEE == 0) {
            takeFee = false;
        }
    ...
    function _getTBasics(uint256 tAmount, uint256 taxFee)
        private
        view
        returns (uint256)
    {
    ...
    function getTTransferAmount(uint256 tAmount, uint256 tFee)
        private
        pure
        returns (uint256)
    {
    ...
    function _getRTransferAmount(uint256 rAmount, uint256 rFee)
        private
        pure
        returns (uint256)
    {
