...
library Parsing {
  function getTokenId(bytes calldata blob) internal pure returns (uint256) {
    ...
    require(index >= 0, "ERC720_METADATA: Separator must exist");
...
contract CakedApes is ERC721Upgradeable, AuthorizableUpgradeable {
  using StringsUpgradeable for uint256;

  uint256 public supply;
  string public baseTokenURI;
  address payable public paymentSplitter;
  address payable public osSplitter;
  ...
  function init(
    ...
    address payable _osSplitter
  ) public initializer {
    __ERC721_init(_data.name, _data.symbol);
    __Ownable_init();
    supply = _data.supply;
    for (uint256 i = 0; i < _data.authorizedAddresses.length; i++) {
      authorize(_data.authorizedAddresses[i], true);
    }
    authorize(_seller, true);
    authorize(_owner, true);
    baseTokenURI = _data.baseTokenURI;
    paymentSplitter = _paymentSplitter;
    osSplitter = _osSplitter;
    seller = _seller;
    transferOwnership(_owner);
  }

  function updateSeller(address _newSeller) external onlyOwner {
    authorize(seller, false);
    authorize(_newSeller, true);
    seller = _newSeller;
  }

  function setPaymentSplitter(address payable _paymentSplitter)
    external
    onlyOwner
  {
    paymentSplitter = _paymentSplitter;
  ...
  function setOSPaymentSplitter(address payable _osSplitter)
    external
    onlyOwner
  {
    osSplitter = _osSplitter;
    ...
    returns (string memory)
  {
    require(
      _exists(_tokenId),
      "ERC721_METADATA: URI query for nonexistent token"
    );
  ...
  function mintFor(
    ...
    require(_quantity == 1, "MINT: invalid quantity");
