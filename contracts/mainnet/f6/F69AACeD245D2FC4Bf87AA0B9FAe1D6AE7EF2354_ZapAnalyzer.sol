...
library MathHelper {
    ...
    function getWeightedPrice(ZapAnalyzer.SwapPath[] memory fullPath) internal view returns (uint256 weightedPrice) {
        ...
        for (uint256 i = 0; i < fullPath.length; i++) {
            ...
            } else if (path.swapType == ZapAnalyzer.SwapType.ALGEBRA) {
                for (uint256 index = 0; index < path.path.length - 1; index++) {
                    weightedPrice =
                        (weightedPrice *
                            pairTokensAndValue(
    ...
    function pairTokensAndValue(
        ...
        if (algebra) {
            tokenPegPair = IAlgebraFactory(uniV3Factory).poolByPair(token0, token1);
        } else {
            tokenPegPair = IUniswapV3Factory(uniV3Factory).getPool(token0, token1, fee);
        }
