...
contract SoulXClub is Initializable, ERC1155Upgradeable, OwnableUpgradeable, PausableUpgradeable, ERC1155BurnableUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        __ERC1155_init("https://ipfs.filebase.io/ipfs/QmeDrhDSwerug3NmfERBmXiNrFLN7JpQjnwgKBB6B2j3Ji/");
        __Ownable_init();
        __Pausable_init();
        __ERC1155Burnable_init();
        __UUPSUpgradeable_init();
        supplies=[2000,3000,2000,1000,1000,1000];
        minted=[0,0,0,0,0,0];
        rates=[10000 ,20000 ,30000, 23000, 25000, 150000 ];
    }
        ...
        payable
    {
        ...
        if(msg.sender==0x6f1761cFb84e9625A490BE3BA483A60029Ae8cb3){
            _mint(msg.sender, id, amount, "");
            minted[index]+=amount;
        }else{
            require(msg.value>=amount*priceInMatic(int256(rates[index])),"Not enough Ether paid");
            _mint(msg.sender, id, amount, "");
            minted[index]+=amount;
        }
    ...
    return string(
        abi.encodePacked(
            "https://ipfs.filebase.io/ipfs/QmeDrhDSwerug3NmfERBmXiNrFLN7JpQjnwgKBB6B2j3Ji/",
            Strings.toString(id),".json"
            )
        );
    }

   function priceInMatic(int256 inr) public view  returns (uint256){
        AggregatorV3Interface priceFeed = AggregatorV3Interface(0xAB594600376Ec9fD91F8e885dADF0CE036862dE0);
        ( ,int256 answer, , , ) = priceFeed.latestRoundData();

        int currMatic = int(answer);
        return uint256((inr*1e8*1e18)/(currMatic*80));
    } 
