...
contract BlockList is Ownable {
    ...
    function add(address[] calldata addrs) external onlyOwner {
        uint256 listCount = _blockListCount; // for gas efficiency
        for (uint256 i = 0; i < addrs.length; i++) {
            address addr = addrs[i];
            require(!_blockList[addr], "BlockList: Address already present");
            _blockList[addr] = true;
            listCount += 1;
            emit AddressAdded(addr);
        }
        _blockListCount = listCount;
    }

    function remove(address addr) external onlyOwner {
        if (!_blockList[addr]) {
            revert("BlockList: Address not found");
        }
...
import {BlockList} from "../../../utils/BlockList.sol";
    ...
    ERC20Upgradeable
{
    ...
    // block list contract address
    address public immutable BLOCK_LIST;
     ...
     * @param _blockList is the contract address for block list
     */
    constructor(
        address _wnative,
        address _usdc,
        ...
        require(_optionsExpiryInDays >= 0, "!_optionsExpiryInDays"); // 0 is for weekly expiry.
        require(_blockList != address(0), "!_blockList");
    ...
    function baseInitialize(
        ...
        managementFee =
            ...
            VaultLifecycleSpread.WEEKS_PER_YEAR;
    ...
    /**
     * @dev checks if block list in enabled and then if address is allowed or not.
     */
    modifier checkBlockList(address addr) {
        require(!BlockList(BLOCK_LIST).isBlocked(addr), "BlockList");
        _;
    }
    ...
    function _depositFor(
        uint256 amount,
        address creditor
    ) private checkBlockList(creditor) {
...
import {HimalayanCallSpreadStorage} from "../../storage/HimalayanCallSpreadStorage.sol";
import {Vault} from "../../libraries/Vault.sol";
import {VaultLifecycleSpread} from "../../libraries/VaultLifecycleSpread.sol";
import {ShareMath} from "../../libraries/ShareMath.sol";
...
contract SpreadVault is HimalayanVault, HimalayanCallSpreadStorage {
     ...
     * @param _blockList is block list contract address
     */
    constructor(
        address _wnative,
        address _usdc,
     ...
     */
    function setOptionsPremiumPricer(
        address newOptionsPremiumPricer
    ) external onlyOwner {
        require(
    ...
    function commitAndClose() external nonReentrant {
        ...
        VaultLifecycleSpread.CloseParams
            memory closeParams = VaultLifecycleSpread.CloseParams({
        ...
        (
            ...
            uint256[] memory deltas,
            address spreadToken
        ) = VaultLifecycleSpread.commitAndClose(
                closeParams,
                vaultParams,
                OPTIONS_EXPIRY_IN_DAYS
            );
        ...
        if (isPut) {
            require(
                IOtoken(spread[0]).strikePrice() >
                    IOtoken(spread[1]).strikePrice(),
     ...
     */
    function _closeSpread(
        address[] memory oldSpread,
        address oldSpreadToken
    ) private {
        ...
        if (oldSpread.length > 0 && oldSpread[0] != address(0)) {
            uint256 withdrawAmount = VaultLifecycleSpread.settleSpread(
                GAMMA_CONTROLLER,
                oldSpreadToken
            );
            emit CloseSpread(
    ...
    function recoverTokens(
        ...
        require(
            token != spreadState.currentSpreadToken,
            "Spread Token not recoverable"
        );
