...
/// @title Staking $BIM
/// @author BIM
/// @notice Allows you to stake your $BIM
contract BimStaking is Ownable {

    IERC20 public immutable bimToken;
    ...
    constructor(address _bimToken) {
        bimToken = IERC20(_bimToken);
    }
    ...
    function stake(uint256 _amount) external {
        bimToken.transferFrom(msg.sender, address(this), _amount);
    ...
    function withdraw(uint256 _amount) external {
        ...
        require(bimToken.balanceOf(address(this)) >= _amount, "contract balance too low");

        balanceOf[msg.sender] = balance - _amount;
        stakeTime[msg.sender] = block.timestamp;

        totalStaked = (totalStaked + rewards) - _amount;
        bimToken.transfer(msg.sender, _amount);
    ...
    function withdrawBalance() external {
        ...
        require(bimToken.balanceOf(address(this)) >= withdrawAmount, "contract balance too low");

        balanceOf[msg.sender] = 0;
        totalStaked -= balance;

        bimToken.transfer(msg.sender, withdrawAmount);
    }
    ...
    /// @notice Admin withdraw $BIM
    /// @param _amount The amount to withdraw
    /// @param _recipient The recipient of the $BIM
    function adminWithdraw(uint256 _amount, address _recipient) external onlyOwner{
        bimToken.transfer(_recipient, _amount);
    }
