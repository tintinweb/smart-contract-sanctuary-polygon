...
contract AddressItems {
    ...
    function addTokenToAddressItems(address tokenOwner, uint32 tokenId) public {
        _addressItems[tokenOwner].tokens.insert(tokenId);
        // _addressItems[tokenOwner].tokens.orderedInsert(tokenId);
    }

    function removeTokenFromAddressItems(address tokenOwner, uint32 tokenId) public {
        AddressItem storage addressItem = _addressItems[tokenOwner];
        uint32 index = addressItem.tokens.findIndex(tokenId);
        addressItem.tokens.orderedRemove(index);
    }

    function transferTokenInAddressItems(address from, address to, uint32 tokenId) public {
...
library Array {
    function insert(uint32[] storage arr, uint32 value) internal {
        arr.push(value);
    }

    //ToDo: fix it
    function orderedInsert(uint32[] storage arr, uint32 value) internal {
        arr.push(0);
        bool broken = false;
        for (uint32 i = uint32(arr.length - 1); i > 0; i--) {
            if (arr[i] > value) {
                arr[i] = arr[i - 1];
            } else {
                arr[i] = value;
    ...
    function remove(uint32[] storage arr, uint32 index) internal {
        require(arr.length > 0, "Can't remove from empty array");
        arr[index] = arr[arr.length - 1];
        arr.pop();
    }

    function orderedRemove(uint32[] storage arr, uint32 index) internal {
        require(arr.length > 0, "Can't remove from empty array");
        for (uint32 i = index; i < arr.length - 1; i++) {
            arr[i] = arr[i + 1];
        }
        arr.pop();
    }

    function findIndex(uint32[] storage arr, uint32 value) internal view returns (uint32) {
    ...
    AddressItems
{
    ...
    function _mint(address to) private returns (uint32) {
        uint32 tokenId = uint32(_tokenIdCounter.current());
    ...
    function transferFrom(address from, address to, uint256 tokenId) public virtual override {
        transferTokenInAddressItems(from, to, uint32(tokenId));
    ...
    function setURIPostfix(string memory uriPostfix) external onlySuperOwnerOrPermittedOwner {
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        return
            _isCommonTokenUri
                ...
                : string.concat(_uri, tokenId.toString(), _uriPostfix);
    }

    function contractURI() public view returns (string memory) {
        return string.concat(_uri, "contract", _uriPostfix);
    }
    ...
    function _addSessionItem(address tokenOwner, uint32 tokenCount, bytes32 sessionHash) private {
        ...
        _sessionItems[sessionHash] = SessionItem(tokenOwner, tokenCount);
    }
