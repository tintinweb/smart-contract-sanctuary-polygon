...
contract vvGHST is Initializable, ERC20Upgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function enter(uint256 _amount) public whenNotPaused returns(uint256){
        ...
        if (totalShares == 0 || totalTokenLocked == 0) { // If no vGHST exists, mint it 1:1 to the amount put in
            ...
            uint256 vvGHSTAmount = _amount.mul(totalShares).div(totalTokenLocked);
    ...
    // claim vGhst by burning vGHST -- this function is largely from QiDao code
    // we charge a 0.5% fee on withdrawal
    function leave(uint256 _share) public {
        if(_share>0){

            //the amount of vGHST the user is owed
            uint256 vghstAmount = convertVVGHST(_share);
            ...
            uint256 newCDR = newCollateralValueTimes100.div(debtValue);

            //if this withdrawal puts us below our minimum CDR, unwind some of the strategy
            if(newCDR < s.targetCDRLow){
                ...
                //repay the debt -- this will charge 0.5%.  todo: need to deal with this
                QiVault(s.vaultAddress).payBackToken(s.vaultID, maiToPayBack);
            }

            //withdraw vGHST to the contract
            QiVault(s.vaultAddress).withdrawCollateral(s.vaultID, vghstAmount);
            ...
            uint256 vQi_feeAmount = vghstAmount.mul(s.withdrawalFeeBP).div(10000); 

            s.totalFeesCollected += vGHST(im_vGHST).convertVGHST(vghst_feeAmount);

            IERC20Upgradeable(im_vGHST).transfer(s.owner, vghst_feeAmount.mul(100).div(400));
            IERC20Upgradeable(im_vGHST).transfer(s.contractCreator, vghst_feeAmount.mul(100).div(400));

            IERC20Upgradeable(im_vQi).transfer(s.owner, vQi_feeAmount.mul(100).div(400));
            IERC20Upgradeable(im_vQi).transfer(s.contractCreator, vQi_feeAmount.mul(100).div(400));

            //convert half the fee to GHST to send it to the vGHST contract
            vGHST(im_vGHST).leave(vghst_feeAmount.mul(200).div(400));
            uint256 ghst_balance = IERC20Upgradeable(im_ghst).balanceOf(address(this));
            IERC20Upgradeable(im_ghst).transfer(im_vGHST, ghst_balance);
            ...
            IERC20Upgradeable(im_vQi).transfer(msg.sender, vQiToClaim.sub(vQi_feeAmount));
    ...
    function rebalanceVault() public{
        ...
        uint256 CDR = collateralValueTimes100.div(debtValue);

        //if CDR > target, that means we can afford to take out more debt
        if(CDR > s.targetCDRHigh){
    ...
    function convertVVGHST(uint256 _share) public view returns(uint256 _vghst){
        if(_share > 0){
            ...
            _vghst = _share.mul(totalTokenLocked).div(totalShares);
        }
        else return 0;
    ...
    //todo:
    function compoundBalancerRewards() public onlyApproved{

        //claim rewards

        //track the airdropped amount of Qi
    ...
    function vQiClaimable(address _user) public view returns(uint256 totalQi) {
        ...
        for(uint256 i = 1; i <= s.currentSnapshotId.current(); i++){
            ...
            //and the size of the corresponding airdrop
            uint256 airdrop = s.qiAirdrops[i];

            //the user is entitled to a proportional amount of the airdrop,
            //based on the percentage of the total supply he possessed
