...
contract CryptoGodsTruBadgerMinter is AccessControl {
    using ECDSA for bytes32;
    bytes32 public constant ADMIN_ROLE = keccak256("ADMIN_ROLE");

    uint256 public unitPrice = 318 ether; //TODO unit price for whitelist?
    uint256 public startTime = 1686547678;
    uint256 public whitelistStartDate = block.timestamp;
    uint8 public maxPerUser = 20;
    uint256 public maxCap = 1200;
    address public signer;
    mapping(address => uint256) public usersMinted;

    IMRC721 public nftContract;

    constructor(address nftContractAddress, address _signer) {
    ...
    function privateMint(
        ...
        require(
            _count + usersMinted[_to] <= mintableAmount,
            "exceeded balance"
        );
    ...
    function updateSigner(address _signer) public onlyRole(ADMIN_ROLE) {
        signer = _signer;
    }
    ...
    function updateWhitelistStartDate(
        uint256 time
    ) public onlyRole(ADMIN_ROLE) {
        whitelistStartDate = time;
    }
    ...
    function updateNftContrcat(
        IMRC721 _newAddress
    ) public onlyRole(ADMIN_ROLE) {
        nftContract = IMRC721(_newAddress);
    }

    function updateMaxCap(uint256 _val) public onlyRole(ADMIN_ROLE) {
        maxCap = _val;
    }
