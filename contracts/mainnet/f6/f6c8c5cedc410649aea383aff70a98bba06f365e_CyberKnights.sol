...
contract CyberKnights is ERC721Enumerable, Ownable {
    ...
    function airdropToWallets(address[] memory airdropWallets) external onlyOwner {
        require(pause == false, "The contract is paused!"); //  IS THE CONTRACT PAUSED
        for(uint256 i = 0; i < airdropWallets.length; i++) {
            address wallet = airdropWallets[i];
            if(blacklist[wallet] == false){
                uint256 supply = totalSupply(); // GET THE TOTAL SUPPLY
                if(supply + 1 > maxSupply) 
                    return;
                    
                if(onPresale == true) 
                    availableMaxSupplyOnPresale = availableMaxSupplyOnPresale - 1;
                else 
                    availableMaxSupplyAfterPreSale = availableMaxSupplyAfterPreSale - 1; 
                allowNftTransfer[supply + 1] = true;
    ...
    function _transfer(address from, address to, uint256 tokenId) internal virtual override {
        ...
        require(allowNftTransfer[tokenId] == true || from == address(0), "Please allow the transfer of your NFT!");
