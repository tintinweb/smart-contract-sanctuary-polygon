...
contract WProfile is IBBPermissionsV01 {
    ...
    constructor(address initialOwner, address initialReceiver, string memory cid, bool fromExisting, uint256 existingProfileId) {
        factory = WProfileFactory(msg.sender);
        if (fromExisting) {
            profileId = existingProfileId;
        } else {
            profileId = profiles.createProfile(initialOwner, initialReceiver, cid);
        }
    }
...
contract WProfileFactory is ERC721, ERC721Burnable {
    ...
    function createProfile(address initialReceiver, string memory cid) public returns (address) {
        WProfile newWProfile = new WProfile(msg.sender, initialReceiver, cid, false, 0);
    ...
    function mintForExistingProfile(uint256 profileId) external returns(address) {
        ...
        WProfile newWProfile = new WProfile(msg.sender, receiver, cid, true, profileId);
