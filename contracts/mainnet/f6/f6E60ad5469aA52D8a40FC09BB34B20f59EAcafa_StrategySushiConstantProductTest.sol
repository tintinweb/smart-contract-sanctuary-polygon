...
contract StrategySushiConstantProductTest is StratFeeManager, GasFeeThrottler {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        if (rewardBal > 0 && reward != native) {
            ...
            ITridentRouter(unirouter).exactInput(_rewardToNative);
        }
        ...
        if (outputBal > 0) {
            ...
            ITridentRouter(unirouter).exactInput(_outputToNative);
        }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != native) {
            ...
            ITridentRouter(unirouter).exactInput(_nativeToLp0);
        }

        if (lpToken1 != native) {
            ITridentRouter.ExactInputParams memory _nativeToLp1 = params.nativeToLp1;
            _nativeToLp1.amountIn = nativeHalf;
            ITridentRouter(unirouter).exactInput(_nativeToLp1);
        }

        ITridentRouter.TokenInput[] memory tokens = new ITridentRouter.TokenInput[](2);
        uint256 lp0Bal = IERC20(lpToken0).balanceOf(address(this));
        uint256 lp1Bal = IERC20(lpToken1).balanceOf(address(this));
        tokens[0] = ITridentRouter.TokenInput(lpToken0, false, lp0Bal);
        tokens[1] = ITridentRouter.TokenInput(lpToken1, false, lp1Bal);
