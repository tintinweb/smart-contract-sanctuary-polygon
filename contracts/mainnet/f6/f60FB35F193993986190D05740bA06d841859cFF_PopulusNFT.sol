...
contract PopulusNFT is ERC4973, Ownable {
    ...
    // }
    function _viewGovernance(uint256 tokenId)
        internal
        view
        virtual
        returns (uint16)
    {
        return _getGovernanceValue(tokenId);
    }
}
...
contract ERC4973 is EIP712, ERC165, IERC721Metadata, IERC4973 {
        ...
        virtual
    {
        require(
            _exists(tokenId),
            "Governance set: nonexistent token"
        );
        _governanceValues[tokenId] =
            _governanceValues[tokenId] +
            governanceIncrement;
    }
     function _getGovernanceValue(uint256 tokenId)
        public
        view
        virtual
        returns (uint16)
