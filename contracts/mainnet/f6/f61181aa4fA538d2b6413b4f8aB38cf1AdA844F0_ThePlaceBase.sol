...
import "../hootbase/utils/HootCrypto.sol";
import "../hootbase/base/common/HootBase.sol";
import "../hootbase/base/erc721/HootERC721A.sol";

abstract contract CombinationInterface {
    function mintCombination(address address_, uint256[] calldata tokenIds_, uint256[] calldata goodsIds_)
        public
        virtual
        returns (uint256);
}
...
contract ThePlaceBase is HootCrypto, HootBase ,HootERC721A {

    event SetGoodsConfig(uint256[] goodsIDs_, GoodsConfig[]  cfgs_);
    event SetDefaultURI(bytes32 uri);
    event UserMint(address _recipient, uint256  goodsID_, uint256  tokenID_);
    event UserBatchMint(address _recipient, uint256[] goodsIDs_, uint256[] tokenIDs_);


    // Optional mapping for token URIs
    mapping(uint256 => bytes32) private _tokenHashes; // tokenid => uri_hash
    bytes32 public _defaultURI;

    uint256 public _maxSupply;
    uint256 public _batchMintLimit;
    struct GoodsConfig {
        bytes32 uriHash;
        uint256 maxSupply;
        uint256 price;
    ...
    mapping(uint256 => GoodsConfig) private _goodsCfg;   // goods_id => GoodsConfig
    mapping(uint256 => uint256) private _goodsTotalMint; // goods_id => TotalMint
    mapping(uint256 => uint256) private _tokenIDGoods;   // token_id => goods_id
    mapping(address => bool) private _combinationContracts;
    constructor(string memory name_, string memory symbol_)  HootERC721A(name_, symbol_){
    }

    function setGoodsConfigBatch(uint256[] calldata goodsIDs_, GoodsConfig[] calldata cfgs_) external atLeastManager {
        require(goodsIDs_.length > 0, "the goods id must have be");
        require(goodsIDs_.length == cfgs_.length, "The lengths must be equal");
        for (uint256 i=0;i<goodsIDs_.length;i++){
            _goodsCfg[goodsIDs_[i]] = cfgs_[i];
        }
        
        emit SetGoodsConfig(goodsIDs_, cfgs_);
    }

    function getGoodsConfigBatch(uint256[] calldata goodsIDs_) public view  returns(GoodsConfig[] memory){
        GoodsConfig[] memory cfgs = new GoodsConfig[](goodsIDs_.length);
        for (uint256 i = 0;i<goodsIDs_.length;i++){
            cfgs[i] = _goodsCfg[goodsIDs_[i]];
        }
        return cfgs;
    }

    function setBatchMintLimit(uint256  batchMintLimit_) external atLeastManager {
        require(batchMintLimit_ > 0, "the batchMintLimit id must have be");
        _batchMintLimit = batchMintLimit_;
    }

    function setMaxSupply(uint256  maxSupply_) external atLeastManager {
        require(maxSupply_ > 0, "the max supply id must be have");
        _maxSupply = maxSupply_;
    }

    /**
 * @notice setDefaultURI is used to set the default uri.
     * @param uri_ the default uri to set.
     */
    function setDefaultURI(bytes32 uri_) external atLeastManager {
        _defaultURI = uri_;
        emit SetDefaultURI(uri_);
    }
  ...
  |__________________________________*/
    ...
    function checkGoodsAndReturnPrice(uint256 goodsID_) internal view returns (uint256){
        GoodsConfig memory goodsCfg_ = _goodsCfg[goodsID_];

        require(goodsCfg_.maxSupply > 0, "goods max supply is not exist");
        require(goodsCfg_.uriHash != "", "goods uri hash is not exist");

        uint256 totalMinted_ = _goodsTotalMint[goodsID_];
        require(totalMinted_ < goodsCfg_.maxSupply , "goods max supply exceeded");
        require(goodsCfg_.enable, "goods is not enabled");
        return goodsCfg_.price;
    }

    function mintNFT(uint256 goodsID_) external payable callerIsUser nonReentrant {
        require(goodsID_ > 0, "The goodsID_ must be have");
        require(_maxSupply >_totalMinted()+1,"max supply exceeded");
        uint256 price = checkGoodsAndReturnPrice(goodsID_);
        require(price <= msg.value, "pay value is insufficient");

        _mint(_msgSender(),  1);
        uint256 newTokenID = _totalMinted();
        _tokenIDGoods[newTokenID] = goodsID_;
        refundExcessPayment(msg.value);
        emit UserMint(_msgSender(), goodsID_, newTokenID);
    }

    function mintBatchNFT(uint256[] calldata goodsIDs_) external payable callerIsUser nonReentrant {
        require(goodsIDs_.length > 0,"The goodsID_ must be have");
        require(goodsIDs_.length <= _batchMintLimit, "out of batch mint limit");
        require(_maxSupply >_totalMinted()+goodsIDs_.length,"max supply exceeded");

        uint256 amount = 0;
        uint256 count =goodsIDs_.length;
        uint256 startTokenID = _totalMinted()+1;
        uint256[] memory newTokenIDs = new uint256[](goodsIDs_.length);
        for (uint256 i = 0; i<count; i++){
            amount += checkGoodsAndReturnPrice(goodsIDs_[i]);
            _tokenIDGoods[startTokenID + i] = goodsIDs_[i];
            newTokenIDs[i] = startTokenID + i;
        }
        require(amount <= msg.value, "pay value is insufficient");

        _mint(_msgSender(), count);
        refundExcessPayment(amount);

        emit UserBatchMint(_msgSender(), goodsIDs_, newTokenIDs);
    }

    function selfMintNFT(address _recipient, uint256 goodsID_, bytes32 uriHash_) external  nonReentrant atLeastManager {
        require(uriHash_ != "","The uri hash_ must be have");
        require(_maxSupply >_totalMinted()+1,"max supply exceeded");
        checkGoodsAndReturnPrice(goodsID_);

        _mint(_recipient, 1);
        // set token uri
        _tokenHashes[_totalMinted()] = uriHash_;
        _tokenIDGoods[_totalMinted()] = goodsID_;
    }

    function selfMintBatchNFT(address _recipient, uint256[] calldata goodsIDs_, bytes32[] calldata uriHashes_) external  nonReentrant atLeastManager {
        require(uriHashes_.length == goodsIDs_.length, "The lengths must be equal");
        require(uriHashes_.length > 0, "The uri hashes must be have");
        require(uriHashes_.length <= _batchMintLimit, "out of batch mint limit");
        require(_maxSupply >_totalMinted()+uriHashes_.length,"max supply exceeded");

        uint256 count =uriHashes_.length;
        uint256 startTokenID = _totalMinted()+1;
        for (uint256 i = 0;i<count;i++){
            checkGoodsAndReturnPrice(goodsIDs_[i]);
            _tokenIDGoods[startTokenID + i] = goodsIDs_[i];
            _tokenHashes[startTokenID + i] = uriHashes_[i];
        }

        _mint(_recipient, count);
    }
    ...
    function tokenURI(uint256 tokenId_) public view override returns (string memory){
        ...
        if (tokenHash == "") {
            tokenHash = _defaultURI;
            uint256 goodsID_ = _tokenIDGoods[tokenId_];
            if (goodsID_ != 0){
                GoodsConfig memory goodsCfg_ = _goodsCfg[goodsID_];
                if (goodsCfg_.uriHash != ""){
                    tokenHash = goodsCfg_.uriHash;
                }
            }
        }
    ...
    function setTokenHash(uint256 tokenId_, bytes32 uriHash) external  nonReentrant atLeastManager {
        require(tokenId_>0, "tokenId_ must be have");
        require(uriHash != "", "uriHash must be have");
        _tokenHashes[tokenId_] = uriHash;
    }

    function setTokenHashBatch(uint256[] calldata tokenIds_, bytes32[] calldata uriHashes_) external  nonReentrant atLeastManager {
        require(uriHashes_.length == tokenIds_.length, "The tokenIds_ must be equal");
        require(uriHashes_.length > 0, "The uri hashes must be have");
        for (uint256 i = 0;i<uriHashes_.length;i++){
            _tokenHashes[tokenIds_[i]] = uriHashes_[i];
        }
    }
    ...
    |               Combination         |
    |__________________________________*/ 
    function setCombinationContract(address contractAddr_, bool enable) external atLeastManager {
        _combinationContracts[contractAddr_] = enable;
    }
    // combine
    function combine(address contractAddr_, uint256[] calldata tokenIds_) external callerIsUser nonReentrant returns (uint256) {
        require(_combinationContracts[contractAddr_], "unsupported combination contract");

        uint256[] memory goodsIds = new uint256[](tokenIds_.length);
        for(uint256 i=0; i<tokenIds_.length; i++){
            require(ownerOf(tokenIds_[i]) == _msgSender(), "caller is not owner");
            goodsIds[i] = _tokenIDGoods[tokenIds_[i]];
            _burn(tokenIds_[i]);
        }
        CombinationInterface combinationContract = CombinationInterface(contractAddr_);
        return combinationContract.mintCombination(_msgSender(), tokenIds_, goodsIds);
    }
}
