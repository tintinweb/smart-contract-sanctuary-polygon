...
contract ApeironPlanet is ERC721, ERC721Enumerable, Ownable, ERC721Pausable {
    ...
    uint256 public MAX_NFT = 12000;
    uint256 public MAX_BY_MINT = 10;
	
	//Inital Minting price 50 Matic /mint
    ...
    constructor(string memory baseURI, address payable creator,uint256 _count) ERC721("ApeironPlanet", "APEP") {
        setBaseURI(baseURI);
    ...
    function mintByCreator(address _to, uint256 _count) public payable onlyOwner {
        uint256 total = _totalSupply();
        require(total + _count <= MAX_NFT, "Max limit");
        require(total <= MAX_NFT, "Sale end");
        for (uint256 i = 0; i < _count; i++) {
    ...
    function updateMaxNFT(uint maxNFT) public onlyOwner {
        MAX_NFT = maxNFT;
    }
    
    function updateMaxByMint(uint maxByMint) public onlyOwner {
