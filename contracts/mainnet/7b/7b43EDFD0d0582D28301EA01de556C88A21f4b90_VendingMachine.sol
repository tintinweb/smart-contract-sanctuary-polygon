    ...
    EIP712MetaTransaction("VendingMachine", "1")
{
    ...
    // (1-2] billion = karma dinos on ethereum
    ...
    // Returns zero if all breed charges are used.
    ...
    function breed(address _breedee,
        ...
        if ((_priceFossil - daoFossil - devFossil - burnedFossil) > 0) {
        ...
        if ((_priceBone - daoBone - devBone - burnedBone) > 0) {
            require(boneContract.transferFrom(msgSender(), _breedee,
    ...
    function setStaticFossilFee(uint256 _fee) external onlyRole(PRIV_MANAGE) {
        fossilFee = _fee;
    }
