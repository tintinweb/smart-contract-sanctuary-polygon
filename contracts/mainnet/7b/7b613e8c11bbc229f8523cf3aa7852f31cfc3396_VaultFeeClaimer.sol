...
contract VaultFeeClaimer is Module {
    address strategy;

    constructor(address _owner, address _strategy) {
        bytes memory initializeParams = abi.encode(_owner, _strategy);
    ...
    function withdrawFee() external {
        exec(
            strategy,
            0,
            abi.encodePacked(bytes4(keccak256("withdrawFees()"))),
            Enum.Operation.Call
        );
    }
    ...
    function setUp(bytes memory initializeParams) public override initializer {
        __Ownable_init();
        (address _owner, address _strategy) = abi.decode(
            initializeParams,
            (address, address)
        );
