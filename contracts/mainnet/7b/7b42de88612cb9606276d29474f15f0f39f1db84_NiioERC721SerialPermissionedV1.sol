...
RolesUpgradeable {
    ...
    modifier capApprove(uint256 tokenId) {
        ...
        require(uint8(tokenPermission) < 2, "Approve operation is not allowed");
        _;
    }
    ...
    override {
        if (from != address(0)) {//not a mint operation
            ...
            if (to == address(0)) {//burn operation
                require(uint8(tokenPermission) < 3, "Burn operation is not allowed");
            } else {//transfer operation
                if (uint8(tokenPermission) > 1) {
                    require(tokenPermission != PermissionStatus.Documentation_Only, "Transfer operation is not allowed");
                    //cases of Private_Trading & Transfer_Only, allowing only to owner or niio-admin
                    address owner = ERC721Upgradeable.ownerOf(tokenId);
                    require(msg.sender == owner || hasRole(ADMIN_ROLE, msg.sender), "Transfer operation is not allowed");
                }
            }
        }
    ...
    function getPermissions(uint256 id)
    external
    view
    ...
    pure
    returns (string memory) {
        if (PermissionStatus.Default_Public_Trading == permission) return "Default_Public_Trading";
        if (PermissionStatus.Public_Trading == permission) return "Public_Trading";
        if (PermissionStatus.Private_Trading == permission) return "Private_Trading";
        if (PermissionStatus.Documentation_Only == permission) return "Documentation_Only";
        if (PermissionStatus.Transfer_Only == permission) return "Transfer_Only";
        revert("Invalid permission value");
    }
