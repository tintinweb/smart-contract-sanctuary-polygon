...
contract ValuesStakingDistributor is Ownable {
    ...
    function setEpochLength(uint256 _newEpochLength) external onlyOwner {
        nextEpochTime = _newEpochLength;
    }

    function setValues(address _newValues) external onlyOwner {
        VALUES = _newValues;
    }
    ...
    function distribute() external returns (bool) {
        if (nextEpochTime <= block.timestamp) {
            ...
            for (uint256 i = 0; i < info.length; i++) {
                if (info[i].rate > 0) {
                    IERC20(VALUES).transferFrom( // send from treasury
                        treasury,
                        info[i].recipient,
    ...
    function nextRewardAt(uint256 _rate) public view returns (uint256) {
        return IERC20(VALUES).totalSupply().mul(_rate).div(1000000);
    }
