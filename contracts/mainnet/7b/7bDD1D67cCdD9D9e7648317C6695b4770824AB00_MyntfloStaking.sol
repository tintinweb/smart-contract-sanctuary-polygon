...
contract MyntfloStaking is ReentrancyGuard, ERC2771Context {
    ...
    mapping(address => mapping(uint256 => uint256)) public timeOfLastUpdate;
    ...
    function stake(uint256 _tokenId, address _tokenContract) external nonReentrant {
        ...
        uint256 lastUpdate = timeOfLastUpdate[_tokenContract][_tokenId];
        if(lastUpdate == 0) {
            // set staked time for this token to prevent rewards reset when transfering and re-staking
