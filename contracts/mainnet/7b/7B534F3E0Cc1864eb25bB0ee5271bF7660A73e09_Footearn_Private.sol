...
import "./Footearn_Ref.sol";

contract Footearn_Private is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    IERC20 public usdtToken;
    IERC20 public busdToken;
    IERC20 public fenToken;
    ...
    mapping(address => bool) public whitelistWiner;
    ...
    uint256 public startDateRandom;
    uint256 public endDate;
    uint256 public countBuy;
    uint256 public maxBuyWiner;
    uint256 public maxBuyRandom;
    ...
    function initialize(
        ...
        uint256 _Rate,
        uint256 _StartDate,
        uint256 _StartDateRandom,
        uint256 _startClaim,
        uint256 _EndDate,
        uint256 _TotalTokenSale,
        uint256 _MaxBuyWiner,
        uint256 _MaxBuyRandom
    ) public onlyOwner {
        usdtToken = IERC20(_USDT);
        busdToken = IERC20(_BUSD);
        fenToken = IERC20(_FenToken);
        rate = _Rate;
        startDate = _StartDate;
        startClaim = _startClaim;
        endDate = _EndDate;
        totalToken = _TotalTokenSale;
        maxBuyWiner = _MaxBuyWiner;
        maxBuyRandom = _MaxBuyRandom;
        startDateRandom = _StartDateRandom;
        usdtToken.approve(
            address(this),
    ...
    function buy(uint256 amount, address currency) public nonReentrant {
        uint256 currentBuy = totalBuy[msg.sender] / rate;
        uint256 nextBuy = currentBuy + amount;
        if (whitelistWiner[msg.sender] == true) {
            require(
                startDate <= block.timestamp && endDate >= block.timestamp,
                "Error: Invalid Time"
            );
            require(
                amount <= maxBuyWiner + maxBuyRandom &&
                    nextBuy <= maxBuyWiner + maxBuyRandom,
                "Error: Invalid balance of buy"
            );
        } else {
            require(
                startDateRandom <= block.timestamp &&
                    endDate >= block.timestamp,
                "Error: Invalid Time"
            );
            require(
                amount <= maxBuyRandom && nextBuy <= maxBuyRandom,
                "Error: Invalid balance of buy"
            );
        }

        if (currency == address(usdtToken)) {
            require(
        ...
        } else if (currency == address(busdToken)) {
            require(
            ...
            revert("Error: Do not support currency");
        }
    ...
    function harvest() public {
        ...
        fenToken.safeTransfer(msg.sender, getAmount);
    ...
    function setStartClaim(uint256 time) public onlyOwner {
        startClaim = time;
    }

    function withdrawFund(address fundAddress) public onlyOwner {
        uint256 totalFundUSDT = usdtToken.balanceOf(address(this));
        uint256 totalFundBUSD = busdToken.balanceOf(address(this));
        usdtToken.safeTransferFrom(address(this), fundAddress, totalFundUSDT);
        busdToken.safeTransferFrom(address(this), fundAddress, totalFundBUSD);
    }

    function addWhitelistWiner(address[] memory _userAddress) public onlyOwner {
        for (uint256 index = 0; index < _userAddress.length; index++) {
            whitelistWiner[_userAddress[index]] = true;
        }
    }

    function removeWhitelistWiner(address[] memory _userAddress)
        public
        onlyOwner
    {
        for (uint256 index = 0; index < _userAddress.length; index++) {
            whitelistWiner[_userAddress[index]] = false;
        }
    }
}
...
contract Footearn_Ref is Ownable, ReentrancyGuard {
    /** ================State================*/
    using SafeERC20 for IERC20;
    IERC20 public usdtToken;
    IERC20 public poToken;

    mapping(address => address) listRef;
    mapping(address => bool) listLeader;
    mapping(address => uint256) totalRefOfUser;

    mapping(string => uint256) totalCurrency;
    ...
    function calculateFundRef(
        ...
        if (
            ...
            if (rfL1 != address(0)) {
                totalCurrency[concatenate(typeCurrency, sender)] += this
                    .calculatePercent(amount, 500);
            }
            if (rfL2 != address(0)) {
                totalCurrency[concatenate(typeCurrency, sender)] += this
                    .calculatePercent(amount, 300);
            }
            if (rfL3 != address(0)) {
                totalCurrency[concatenate(typeCurrency, sender)] += this
                    .calculatePercent(amount, 200);
            }
        } else {
            if (rfL1 != address(0)) {
                totalCurrency[concatenate(typeCurrency, sender)] += this
    ...
    function getTotalToken(address _typeCurrency)
        public
        view
        returns (uint256)
    {
        return totalCurrency[concatenate(_typeCurrency, msg.sender)];
    }

    function getTotalRef(address userAddress) public view returns (uint256) {
        return totalRefOfUser[userAddress];
    }

    function claimToken(uint256 amount, address _typeCurrency) public {
        require(
            amount <= totalCurrency[concatenate(_typeCurrency, msg.sender)],
            "Error: Invalid amount"
        );
        require(amount > 1 ether, "Error: Invalid fee");

        totalCurrency[concatenate(_typeCurrency, msg.sender)] -= amount;
        usdtToken.approve(
            address(this),
            115792089237316195423570985008687907853269984665640564039457584007913129639935
        );
        usdtToken.transfer(msg.sender, amount - 1 ether);
        usdtToken.transfer(feeAddress, 1 ether);
    }

    function concatenate(address _typeCurrency, address _sender)
        public
        pure
        returns (string memory)
    {
        return string(abi.encodePacked(_typeCurrency, _sender));
    }
}
