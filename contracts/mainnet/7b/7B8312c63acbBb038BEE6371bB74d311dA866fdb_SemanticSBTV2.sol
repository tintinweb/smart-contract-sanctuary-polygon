 ...
 */
contract SemanticSBTV2 is Ownable,Initializable, ERC165, IERC721, IERC721Metadata, ISemanticSBT, ISemanticMetadata {
    ...
    // Mapping of subjectIndex to (Mapping of address to true/false)
    mapping(uint256 => mapping(address => bool)) _subjectOwner;
    ...
    // Mapping from predicate name to Predicate
    mapping(string => Predicate) private _predicate;
    // Mapping from predicate Id to predicate name
    mapping(uint256 => string) private _predicateName;
    // Predicate size
    uint256 public predicateSize;

    // All object value of int type
    uint256[] _intO;
    // All object value of string type
    string[] _stringO;
    // All object value of address type
    address[] _addressO;
    ...
    function initialize(
        ...
        AddPredicate[] memory predicates_
    ...
    function subject(uint256 index) public view returns(string memory subjectValue,string memory className_){
        ...
        className_ = className[_subjects[index].cIndex];
    }

    /**
     * @dev Get token rdf .
     */
    function rdfOf(uint256 tokenId) public view returns (string memory) {
    ...
    function _addClass(string[] memory classList) internal {
        for (uint256 i = 0; i < classList.length; i++) {
            ...
            classSize++;
            _classIndex[className_] = classSize;
            className[classSize] = className_;
        }
    }
     ...
     */
    function _addPredicate(AddPredicate[] memory predicates) internal {
        for (uint256 i = 0; i < predicates.length; i++) {
            AddPredicate memory predicate = predicates[i];
            require(
                keccak256(abi.encode(predicate.name)) !=
                keccak256(abi.encode("")),
                "Predicate cannot be empty"
            );
            predicateSize++;
            _predicate[predicate.name] = Predicate(
                predicateSize,
                predicate.name,
                predicate.fieldType
            );
            _predicateName[predicateSize] = predicate.name;
        }
    }

    function _checkPredicate(uint256 pIndex)internal view {
        require(pIndex > 0 && pIndex <= predicateSize, "predicate not exist");
    }
    ...
    function addSubject(string memory value, string memory className_) external onlyMinter returns(uint256 sIndex) {
        ...
        _subjectOwner[sIndex][msg.sender] = true;
    }



    function predicateOf(string memory predicateName_) external view returns (uint256 predicateIndex,FieldType fieldType) {
        Predicate memory predicate_ = _predicate[predicateName_];
        predicateIndex = predicate_.index;
        fieldType = predicate_.fieldType;
    }

    function predicateName(uint256 pIndex) external view returns (string memory) {
        return _predicateName[pIndex];
    }
    ...
    function _addIntPO(uint256[] storage pIndex, uint256[] storage oIndex, IntPO[] memory intPOList) internal {
        for(uint256 i = 0 ; i < intPOList.length; i++){
            ...
            _checkPredicate(intPO.pIndex);
            uint256 _oIndex = _intO.length;
            _intO.push(intPO.o);
    ...
    function _addStringPO(uint256[] storage pIndex, uint256[] storage oIndex,  StringPO[] memory stringPOList) internal {
        for(uint256 i = 0 ; i < stringPOList.length; i++){
            ...
            _checkPredicate(stringPO.pIndex);
    ...
    function _addAddressPO(uint256[] storage pIndex, uint256[] storage oIndex,  AddressPO[] memory addressPOList) internal {
        for(uint256 i = 0 ; i < addressPOList.length; i++){
            ...
            _checkPredicate(addressPO.pIndex);
            uint256 _oIndex = _addressO.length;
            _addressO.push(addressPO.o);
    ...
    function _addSubjectPO(uint256[] storage pIndex, uint256[] storage oIndex,  SubjectPO[] memory subjectPOList) internal {
        for(uint256 i = 0 ; i < subjectPOList.length; i++){
            ...
            _checkPredicate(subjectPO.pIndex);
    ...
    function _addBlankNodePO(uint256[] storage pIndex, uint256[] storage oIndex,  BlankNodePO[] memory blankNodePOList) internal {
        for(uint256 i = 0 ; i < blankNodePOList.length; i++){
            ...
            require(blankNodePO.pIndex <= predicateSize, "predicate not exist");
    ...
    function _buildRDF(SPO memory spo) internal view returns (string memory _rdf){
        ...
        for(uint256 i =0; i < spo.pIndex.length; i++){
            Predicate memory p = _predicate[_predicateName[spo.pIndex[i]]];
    ...
    function _buildS(SPO memory spo) internal view returns (string memory){
        string memory _className = spo.sIndex == 0 ? SOUL_CLASS_NAME : className[_subjects[spo.sIndex].cIndex];
    ...
    function _buildIntRDF(uint256 pIndex, uint256 oIndex) internal view returns (string memory){
        Predicate memory predicate = _predicate[_predicateName[pIndex]];
        string memory p = string.concat(PROPERTY_PREFIX, predicate.name);
        string memory o = _intO[oIndex].toString();
    ...
    function _buildAddressRDF(uint256 pIndex, uint256 oIndex) internal view returns (string memory){
        Predicate memory predicate = _predicate[_predicateName[pIndex]];
        string memory p = string.concat(PROPERTY_PREFIX, predicate.name);
        string memory o = string.concat(ENTITY_PREFIX, SOUL_CLASS_NAME, CONCATENATION_CHARACTER, _addressO[oIndex].toHexString());
        return string.concat(p, BLANK_SPACE, o);
    }
    ...
    function _buildSubjectRDF(uint256 pIndex, uint256 oIndex) internal view returns (string memory){
        Predicate memory predicate = _predicate[_predicateName[pIndex]];
        string memory _className = className[_subjects[oIndex].cIndex];
        string memory p = string.concat(PROPERTY_PREFIX, predicate.name);
    ...
    function _buildBlankNodeRDF(uint256 pIndex, uint256 oIndex) internal view returns (string memory){
        ...
        string memory _rdf = BLANK_NODE_START_CHARACTER;
       ...
       for(uint256 i =0; i < blankPList.length; i++){
            Predicate memory _p = _predicate[_predicateName[blankPList[i]]];
        ...
        return string.concat(p, BLANK_SPACE, _rdf, TURTLE_END_SUFFIX);
    }
   ...
   function mint(address account, uint256 sIndex, IntPO[] memory intPOList, StringPO[] memory stringPOList,
    ...
    function burn(address account, uint256 id) external onlyMinter {
        ...
        emit CreateSBT(msg.sender, account, id, _rdf);
    }
...
interface ISemanticMetadata is IERC721Metadata {
