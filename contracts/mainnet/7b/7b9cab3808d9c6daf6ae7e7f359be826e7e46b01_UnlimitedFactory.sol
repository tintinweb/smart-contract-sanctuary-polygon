...
import {Unlimited} from "./Unlimited.sol";

contract Strict is  Unlimited {
  uint256 endTime;
  uint256 maxSupply;
  ...
  constructor(
    ...
    uint256 _endTime,
    uint256 _maxSupply
  ) Unlimited(name, symbol, _baseURI, _startTime, _admin) {
    endTime = _endTime;
 ...
 * @notice  Unlimited is a simple ERC721 token.
 */
...
import {UnlimitedSupply} from "./UnlimitedSupply.sol";
import {UnlimitedTime} from "./UnlimitedTime.sol";
import {Strict} from "./Strict.sol";

contract UnlimitedFactory {
  event CollectionCreated(address indexed createdBy, address indexed collectionAddress);
  
  function createUnlimitedSupplyCollection(
    string memory name,
    string memory symbol,
    ...
    address admin,
    uint256 endTime
  ) public returns (UnlimitedSupply collection) {
    collection = new UnlimitedSupply(
      name,
      symbol,
      baseURI,
      startTime,
  ...
  function createUnlimitedTimeCollection(
    string memory name,
    string memory symbol,
    string memory baseURI,
    uint256 startTime,
    address admin,
    uint256 maxSupply
  ) public returns (UnlimitedTime collection) {
    collection = new UnlimitedTime(
      name,
      symbol,
      baseURI,
      startTime,
  ...
  function createStrictCollection(
    string memory name,
    string memory symbol,
    string memory baseURI,
    uint256 startTime,
    ...
    uint256 endTime,
    uint256 maxSupply
  ) public returns (Strict collection) {
    collection = new Strict(
      name,
      symbol,
      baseURI,
      startTime,
...
contract UnlimitedSupply is Unlimited {
  // Only limitation for this the end time
  uint256 endTime;
...
contract UnlimitedTime is Unlimited {
  // Only limitation for this collection is
  // the max supply 
  uint256 maxSupply;
  ...
  function mint(address _to) external override checkSupply {
    unchecked {
      _mint(_to, totalSupply++);
    }
  }
