...
contract MetaOlympic is ERC20Burnable, Ownable {
    using SafeMath for uint256;
    ...
    constructor(
        ...
        uint256 feePercent,
        address owner
    ) ERC20('Meta Olympic', 'METY') {
        require(
            feeAccount != address(0),
            'Meta Olympic: fee account from the zero address'
        );
        require(feePercent <= PRECISION, 'Meta Olympic: incorrect fee percent');
        require(owner != address(0), 'Meta Olympic: incorrect owner address');
        if (_msgSender() != owner) {
    ...
    function _transfer(
        ...
        require(sender != address(0), 'Meta Olympic: transfer from the zero address');
        require(recipient != address(0), 'Meta Olympic: transfer to the zero address');
