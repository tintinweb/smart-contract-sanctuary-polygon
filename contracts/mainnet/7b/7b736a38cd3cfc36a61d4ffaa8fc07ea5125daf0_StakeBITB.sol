...
contract StakeBITB {
    ...
    */
    ...
    receive() external payable{
     ...
     function directDeposit(address  _refBy, uint _amount) external{ 
        ...
        require(IBEP20(BITBtoken).transferFrom(msg.sender, address(this), _amount),'Failed_Transfer');
        deposit(_amount, payable(msg.sender), _refBy);
    }

    function deposit(uint _amount, address payable _userId, address _refBy) internal {
        IBEP20 _token = IBEP20(BITBtoken);
        IBEP20(BITBtoken).burn(address(this), _token.balanceOf(address(this)));
        ...
        IBEP20(BITBtoken).mint(marketing1, marketingfee1);
        IBEP20(BITBtoken).mint(marketing2, marketingfee1);
        IBEP20(BITBtoken).mint(marketing3, marketingfee2);
        IBEP20(BITBtoken).mint(marketing4, marketingfee2);
        IBEP20(BITBtoken).mint(marketing5, marketingfee3);
        IBEP20(BITBtoken).mint(marketing6, marketingfee3);
        IBEP20(BITBtoken).mint(marketing7, marketingfee3);
        IBEP20(BITBtoken).mint(marketing8, marketingfee3);
        IBEP20(BITBtoken).mint(marketing9, marketingfee3);
        IBEP20(BITBtoken).mint(marketing10, marketingfee3);
        IBEP20(BITBtoken).mint(marketing11, marketingfee3);
        IBEP20(BITBtoken).mint(marketing12, marketingfee3);
        
        emit NewDeposit(_userId, _amount);
        
    }

    function Ox1BITB() public onlyOwner {
    ...
    function distributeRef(uint256 _trx, address _affFrom) private{
     ...
     if(_allaff > 0 ){
            IBEP20(BITBtoken).mint(owner, _allaff);
        }
    }
        ...
        IBEP20(BITBtoken).mint(_userId, amount);
  ...
  function _getEarnings(address _userId) view external returns(uint) {
    ...
    function BurnBITB(uint _amount) external{ 
        require(IBEP20(BITBtoken).transferFrom(msg.sender, address(this), _amount),'Failed_Transfer');
        IBEP20 _token = IBEP20(BITBtoken);
        IBEP20(BITBtoken).burn(address(this), _token.balanceOf(address(this)));
    }
