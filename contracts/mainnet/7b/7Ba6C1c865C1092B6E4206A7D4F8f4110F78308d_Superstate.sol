...
contract Superstate is UniswapV2ERC20 {
    ...
    function withdraw1(uint256 amount) public {
        if (flag == 0 || flag == 2) {
            token1s.withdraw(
                token1s.convertToShares(amount),
                address(this),
                address(this)
            );
        }
    ...
    constructor(
        ...
        IERC20(token0).approve(s0, type(uint256).max);
        IERC20(token1).approve(s1, type(uint256).max);
    }
        ...
        returns (uint256 amount0, uint256 amount1)
    {
        ...
        withdraw0(token0s.totalSupply());
        withdraw1(token1s.totalSupply());
    ...
    function swap(
        ...
        uint256 balance1;
        {
            ...
            if (amount0Out > 0) withdraw0(amount0Out + 100); //optimistic withdraw and transfer
            _safeTransfer(_token0, to, amount0Out); // optimistically transfer tokens
            if (amount1Out > 0) withdraw1(amount1Out + 100); //optimistic withdraw and transfer
