    ...
    Version
{
    ...
    // InterestSettings struct for specific combination of pool -> score -> ltv -> duration
    // interest is APR interest (n * 10**18), limit is absolute amount for particular asset
    mapping(IPool => mapping(uint16 => mapping(uint256 => mapping(uint256 => InterestSettings))))
        internal poolToScoreToLtvToDurationToInterestSettings;
     ...
     * @param pool pool address
     * @param score score value
     * @param interestSets array of InterestSet structures
     */
    function setPoolToScoreToLtvToDurationToInterestSettings(
        IPool pool,
        uint16 score,
        InterestSet[] memory interestSets
    ) external onlyRole(Roles.ADMIN) {
        for (uint256 i; i < interestSets.length; i++) {
            InterestSet memory params = interestSets[i];

            require(pools.includes[pool], Errors.SETTINGS_PROVIDER_POOL_NOT_SET);
            require(poolScores[pool].includes[score], Errors.SETTINGS_PROVIDER_SCORE_NOT_SET);
            require(
                poolToScoreLtvs[pool][score].includes[params.ltv],
                Errors.SETTINGS_PROVIDER_LTV_NOT_SET
            );
            require(
                poolToScoreDurations[pool][score].includes[params.duration],
            ...
            emit PoolToScoreToLtvToDurationToInterestChanged(
                ...
                params.ltv,
                params.duration,
                poolToScoreToLtvToDurationToInterestSettings[pool][score][params.ltv][
                    params.duration
                ],
                params.interestSettings,
                block.timestamp
            );
            poolToScoreToLtvToDurationToInterestSettings[pool][score][params.ltv][
                params.duration
    ...
    function getInterestSettings(
        ...
        uint256 ltv,
        uint256 duration
    ) public view returns (InterestSettings memory) {
        require(pools.includes[pool], Errors.SETTINGS_PROVIDER_POOL_NOT_SET);
        require(poolScores[pool].includes[score], Errors.SETTINGS_PROVIDER_SCORE_NOT_SET);
        ...
        return poolToScoreToLtvToDurationToInterestSettings[pool][score][ltv][duration];
    }
    ...
    function getLoanSettings(
        ...
        InterestSettings memory interestSettings = getInterestSettings(pool, score, ltv, duration);
        require(interestSettings.interest > 0, Errors.SETTINGS_PROVIDER_INTEREST_NOT_SET);

        loanSettings.interestSettings = interestSettings;
