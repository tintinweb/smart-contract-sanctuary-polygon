...
// File @openzeppelin/contracts-upgradeable/utils/AddressUpgradeable.so[email protected]
    ...
    ReentrancyGuardUpgradeable
{
    ...
    function getContent(uint tokenId) public view returns(string memory){
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        _requireMinted(tokenId);
        string memory baseURI = _baseURI();
        return bytes(baseURI).length > 0 ? string(abi.encodePacked(baseURI, contents[tokenId])) : contents[tokenId];
    }

    function mint(
        address to,
        string calldata content
    ) public onlyAdmin(MINTER_ROLE) nonReentrant {
        require(
    ...
    function batchMint(
        ...
        for(uint i = 0; i < count; ++i){
            ...
            _mint(to[i], tokenId);
            _tokenIdTracker.increment();
            contents[tokenId] = content[i];
            emit Mint(_msgSender(), to[i], tokenId, content[i]);
        }
    }

    function pause() public onlyAdmin(PAUSER_ROLE) {
        _pause();
    }

    function unpause() public onlyAdmin(PAUSER_ROLE) {
        _unpause();
    }
