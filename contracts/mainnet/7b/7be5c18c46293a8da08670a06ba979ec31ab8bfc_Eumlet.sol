...
contract Eumlet is Ownable, ReentrancyGuard {
    ...
    uint128 public constant AUTORESUME = 30 days;
        ...
        exist(userToIds[msg.sender].streamId, streamId)
    {
        ...
        if (stream.paused && stream.start + AUTORESUME <= block.timestamp) {
            _resume(msg.sender, streamId, AUTORESUME);
        }
        ...
        exist(userToIds[user].streamId, streamId)
    {
        ...
        if (stream.paused && stream.start + AUTORESUME <= block.timestamp) {
            // resume without change status
            stream.lastClaimed += AUTORESUME;
            stream.end += AUTORESUME;
        } else {
        ...
        exist(userToIds[user].streamId, streamId)
    {
        ...
        uint128 timeFromPause = uint128(block.timestamp) -
            userToStreams[user][streamId].start;
        _resume(
            user,
            streamId,
            timeFromPause < AUTORESUME ? timeFromPause : AUTORESUME
        );
    }
        ...
        exist(userToIds[user].streamId, streamId)
    {
        ...
        if (
            userToStreams[user][streamId].paused &&
            userToStreams[user][streamId].start + AUTORESUME <= block.timestamp
        ) {
            _resume(user, streamId, AUTORESUME);
        }
        // copy to memory only after resume (if needed)
        ...
        )
    {
        ...
        address token;
        if (stream) {
            uint256 end = userToStreams[user][id].end;
            if (userToStreams[user][id].paused) {
                end += AUTORESUME;
            }
            require(end + limit < block.timestamp, "Come back later");
     ...
     * if paused and not autoresume return only part unlocked before pause
     * if paused and autoresume return the same as without pause
     */
        ...
        returns (uint256 amount, uint128 numSteps)
    {
        ...
        uint128 to;
        if (stream.paused) {
            if (stream.start + AUTORESUME > block.timestamp) {
                to = stream.start;
            } else {
                to = block.timestamp - AUTORESUME >= stream.end
                    ? stream.end
                    : uint128(block.timestamp) - AUTORESUME;
            }
        } else {
            to = block.timestamp >= stream.end
                ? stream.end
                : uint128(block.timestamp);
        }
        numSteps = _getNumSteps(stream.lastClaimed, to, stream.step);
