...
contract Del1155 is ERC1155, Pausable, ReentrancyGuard, AccessControlEnumerable {
    ...
    mapping (uint256 => Token) tokens; /* token_id -> token */
    ...
    event TokenUpdated(uint256 indexed token_id, string name, uint256 price, uint256 max);
    event SlotUsed(uint256 slotId, uint256 tokenId, uint256 amount, address minter);

    constructor(string memory _uri, address payable _beneficiary, address _signer, address[] memory admins, IERC20 _paymentToken) ERC1155(_uri) {
        ...
        emit SignerChanged(_signer);
         ...
         setToken(1, "Stock Option", 1e6, 10860); // Price: 1 USDC (6 decimals), Max supply: 10860
    }
    ...
    function setToken(uint256 id, string memory _name, uint256 _price, uint256 _max) public onlyRole(DEFAULT_ADMIN_ROLE) {
        Token storage token = tokens[id];
        token.name = _name;
        token.price = _price;
        token.max = _max;
        emit TokenUpdated(id, _name, _price, _max);
    }
    ...
    function mint(uint256 amount, uint256 slotId, uint256 tokenId, uint256 validUntil, bytes memory signature) external nonReentrant whenNotPaused {
        // Check signature.
        ...
        require(token.supply + amount <= token.max, "Cannot mint over maximum");

        // Check price.
        if (token.price > 0) {
            paymentToken.safeTransferFrom(_msgSender(), address(this), amount * token.price);
        }
    ...
    function batchMint(address[] memory accounts, uint256[] memory tokenIds, uint256[] memory amounts) external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        for (uint256 i = 0; i < accounts.length; i++) {
            ...
            require(token.supply <= token.max, "Cannot mint over maximum amount of token");
    ...
    function totalSupply(uint256 id) external view returns (uint256) {
        Token storage token = tokens[id];
        require(token.max > 0, "No such tokenId");
        return token.supply;
    }
    ...
    // @notice Send all of the payment currency to predetermined beneficiary. Anyone can pay gasses for release.
    function release() external {
        paymentToken.safeTransferFrom(address(this), beneficiary, paymentToken.balanceOf(address(this)));
    }
