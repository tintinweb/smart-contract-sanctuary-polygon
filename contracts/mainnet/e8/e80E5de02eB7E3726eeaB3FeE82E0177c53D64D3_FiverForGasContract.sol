...
contract FiverForGasContract {
    ...
    function FiverForGas(uint[] calldata toChainIds,address receiver) whenNotPaused public returns (uint [] memory) {
        ...
        for (uint i=0; i<toChainIds.length; i++) {
            ...
            abi.encodeWithSignature("sendFiverrGasTokenViaCallAnyCall(uint256,uint256,address,address)"
            ,block.chainid,toChainIds[i],msg.sender,receiver),
            address(this),
            toChainIds[i]
            );
    ...
    event sendFiverrGasTokenEvent(uint indexed _chainAId,uint _chainBId,address indexed _chainAsender,address indexed _chainBreceiver,uint gasTokenSent);
    
    function sendFiverrGasTokenViaCallAnyCall(uint _chainAId,uint _chainBId,address _from,address payable _to) onlyAnyCall public payable {
        ...
        emit sendFiverrGasTokenEvent(_chainAId,_chainBId,_from,_to,FiverrGasToken);
        // if failed do a callback to refund source chain
    }

    event Refund5Dai(uint indexed failedChainid,address indexed failedContract,address indexed refundReceiver);
    ...
    function anyFallback(address _chainbContract,bytes calldata _originalData) onlyAnyCall public {
        ...
        (,uint _failedChainid,address _from,) = abi.decode(_originalData[4:], (uint,uint,address, address));
        uint fiver=5 ether;

        //refund 5 dai
        TransferHelper.safeTransfer(stableCoinAddress,_from,fiver);
        emit Refund5Dai(_failedChainid,_chainbContract,_from);
        // if failed do a callback to refund source chain
    }
