...
contract Withdrawal is AragonApp {
    ...
    event WithdrawSuccess(WithdrawalParams params);
    ...
    ) external auth(SET_TOKEN_MANAGER_ROLE) {
        require(isContract(_tokenManager), ERROR_TOKEN_MANAGER);
     ...
     */
    function setWeeziCore(
        address _weeziCore
    ) external auth(SET_WEEZICORE_ROLE) {
        require(isContract(_weeziCore), ERROR_WEEZI_CORE);
...
function toString(bytes memory data) public pure returns(string memory) {
        ...
        nonReentrant
    {
        ...
        require(
            tokenManager.spendableBalanceOf(/*msg.sender*/ params._sender) >= params._amount,
            string(abi.encodePacked(ERROR_INSUFFICIENT_BALANCE, toString(/*msg.sender*/ params._sender),toString(tokenManager.spendableBalanceOf(/*msg.sender*/ params._sender))))
        );
        ...
        tokenManager.burn(/*msg.sender*/ params._sender, params._amount);

        emit WithdrawSuccess(params);
    }
}
