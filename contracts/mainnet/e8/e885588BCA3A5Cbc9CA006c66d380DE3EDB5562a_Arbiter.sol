...
contract Arbiter is IArbiter, BastionConnector, AutomationCompatible {
    ...
    function getJob(uint256 _index) external view returns (
        address _flFarm,
        address _flPool,
        address _token0,
        address _token1,
    ...
    function getJobConfig(uint256 _index) external view returns (
    ...
    function pushArbiterJob(
        ...
        (address _token0, address _token1) = 
            (IFlashLiquidityPair(_flashPool).token0(), IFlashLiquidityPair(_flashPool).token1());
        ...
        if (
            address(priceFeeds[_token0]) == address(0) || 
            address(priceFeeds[_token1]) == address(0)
        ) {
    ...
    function _getPrices(address _token0, address _token1) 
        internal 
        view
    ...
    function performUpkeep(bytes calldata performData) external override {
        ...
        if (_profit < (_call.zeroToOne ? _reserve0 : _reserve1) / jobsConfig[_job.flPool].reserveToProfitRatio) {
        ...
        if (!_call.tokenInIsRewardToken) {
            ...
            (_reserve0, _reserve1) = _call.zeroToOne ? (_reserve0, _reserve1) : (_reserve1, _reserve0);
