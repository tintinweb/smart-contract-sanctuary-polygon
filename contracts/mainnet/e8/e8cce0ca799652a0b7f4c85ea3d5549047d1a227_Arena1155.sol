...
contract Arena1155 is Owned(msg.sender) {
    ...
    uint256 constant public NON_FUNGIBLE_ID_FLAG = 1 << 255;
    ...
    // support rake adjustments in 0.5% increments
    uint256 constant rakeCalculationDenominator = 200;
    ...
    /// @notice all unclaimed funds earned from item sales
    mapping(address => uint256) public itemSaleProceeds;

    /// @dev base URI for all token metadata
    string private _baseMetadataURI;
 
    struct fungibleCollection {
        address collectionManager; 
        uint40 maxSupply;
        uint40 costGwei;
        uint8 adminRake; // in 0.5% increments. i.e 100 is a 50% cut
        bool purchaseable;
    }
    /// @notice collection details for fungible collections
    mapping(uint128 => fungibleCollection) public fungibles;

    /// @notice track the total minted for a given fungible token
    mapping(uint128 => uint256) public fungibleMints;

    struct nonFungibleCollection{
        address collectionManager;
        uint24 maxSupply;
        uint48 costGwei;
        uint8 adminRake; // in 0.5% increments. i.e 100 is a 50% cut
    ...
    mapping(uint128 => nonFungibleCollection) public nonFungibles;

    /// @dev contains additional derived fields useful for consumers
    struct nonFungibleCollectionExternal {
        address collectionManager;
        uint32 maxSupply;
        uint48 costGwei;
        uint8 adminRake;
    ...
    function createNonFungibleCollection(
        ...
        nonFungibleCollection memory collection = nonFungibleCollection(
            collectionManager,
            maxSupply,
            costGwei,
            adminRake,
        ...
        nonFungibles[collectionID] = collection;
    }


    /// @dev arena admin or collectionManagers can directly mint from collections they control
    /// @param to target address for tokens
    /// @param collectionID ID of an existing non-fungible collection
    /// @param amount number of tokens to mint in target collection
    function directMintNonFungible(address to, uint128 collectionID, uint amount) external {

        // must be admin or collectionManager
        nonFungibleCollection memory collection = nonFungibles[collectionID];
        if (!(msg.sender == owner || msg.sender == collection.collectionManager)) revert("UNAUTHORIZED");

        if (amount > 1) {
            _batchMintNonFungible(to, collectionID, amount);
        } else {
            _mintNonFungible(to, collectionID);
        }
    }
    ...
    // slightly more efficient than calling _mintNonFungible multiple times
    // when you need to mint multiple items in the same collection to a given user
    function _batchMintNonFungible(
        address to,
        uint128 collectionID,
        uint amount
    ) private {
    ...
    function getNonFungibleCollection(uint128 collectionID) public view returns (nonFungibleCollectionExternal memory) {
        if (collectionID & NON_FUNGIBLE_COLLECTION_FLAG == 0) revert invalidNonFungibleID();

        nonFungibleCollection storage deets = nonFungibles[collectionID];
        return nonFungibleCollectionExternal(
            deets.collectionManager,
            deets.maxSupply,
            deets.costGwei,
            deets.adminRake,
            deets.purchaseable,
    ...
    function createFungibleCollection(
        uint128 collectionID,
        address collectionManager,
        uint40 maxSupply,
        uint40 costGwei,
        ...
        fungibleCollection memory td = fungibleCollection(collectionManager, maxSupply, costGwei, adminRake, purchaseable);
        fungibles[collectionID] = td;
    }

    function directMintFungible(address to, uint128 collectionID, uint amount) external {

        // must be admin or collectionManager
        fungibleCollection memory collection = fungibles[collectionID];
        if (!(msg.sender == owner || msg.sender == collection.collectionManager)) revert("UNAUTHORIZED");

        _mintFungible(to, collectionID, amount);
    }
    ...
    function _mintFungible(
        ...
        if (collectionID & NON_FUNGIBLE_COLLECTION_FLAG != 0) revert invalidFungibleID();
        if (fungibleMints[collectionID] + amount > fungibles[collectionID].maxSupply) revert cannotMintOverMaxSupply();

        uint256 id = uint256(collectionID) << 128;
        fungibleBalances[to][id] += amount;
        fungibleMints[collectionID] += amount;
    ...
    function getFungibleDetails(uint128 collectionID) public view returns (fungibleCollection memory) {
        if (collectionID & NON_FUNGIBLE_COLLECTION_FLAG != 0) revert invalidFungibleID();
        ...
        returns (uint256[] memory balances)
    {
        ...
        unchecked {
            for (uint256 i = 0; i < addrs.length; ++i) {
                ...
                if (ids[i] & NON_FUNGIBLE_ID_FLAG != 0) {
    ...
    function safeTransferFrom(
        ...
        if (id & NON_FUNGIBLE_ID_FLAG != 0) { // Non-Fungible
            uint128 collectionID = uint128(id >> 128);
            ...
            nonFungibleCollection memory collection = nonFungibles[collectionID];
            if (collection.soulbound) revert cannotTransferSoulbound();
    ...
    function purchaseItem(uint128 collectionID, uint24 amount) public payable {
        if (amount == 0) revert("invalid amount to purchase");

        // non-fungible
        if (collectionID & NON_FUNGIBLE_COLLECTION_FLAG != 0) {
            nonFungibleCollection memory collection = nonFungibles[collectionID];
            if (!collection.purchaseable) revert("item not purchaseable");

            // free item vs. ones with a cost. Always mint 1 to prevent bulk minting free items
            if (collection.costGwei == 0) {
                if (msg.value != 0) revert("invalid purchase fee");
                _mintNonFungible(msg.sender, collectionID);
            } else {
                if (msg.value != uint256(collection.costGwei) * 1e9 * amount) revert("invalid purchase fee");

                uint256 adminPortion = calculateAdminPortion(collection.adminRake, msg.value);
                uint256 managerPortion = msg.value - adminPortion;
                itemSaleProceeds[owner] += adminPortion;
                itemSaleProceeds[collection.collectionManager] += managerPortion;

                if (amount > 1) {
                    _batchMintNonFungible(msg.sender, collectionID, amount);
                } else {
                    _mintNonFungible(msg.sender, collectionID);
                }
            }
            ...
            if (collection.costGwei == 0) {
                if (msg.value != 0) revert("invalid purchase fee");
                _mintFungible(msg.sender, collectionID, 1);
            } else {
                if (msg.value != uint256(collection.costGwei) * 1e9 * amount) revert("invalid purchase fee");
                ...
                _mintFungible(msg.sender, collectionID, amount);
            }
        }
    }

    function claimSaleProceeds() public {
        uint256 balance = itemSaleProceeds[msg.sender];
        if (balance == 0) revert("no balance to claim");

        itemSaleProceeds[msg.sender] = 0;
    ...
    function calculateAdminPortion(uint256 managerCut, uint256 amount) public pure returns (uint256) {
        return managerCut * amount / rakeCalculationDenominator;
    }
