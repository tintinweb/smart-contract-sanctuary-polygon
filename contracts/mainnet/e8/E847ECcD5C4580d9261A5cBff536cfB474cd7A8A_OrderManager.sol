...
contract OrderManager is UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    address[] public marketplaceRoyaltyAddress;
    uint256[] public marketplaceRoyaltyFee;
    ...
    function initialize(address _signer) public initializer {
        ...
        marketplaceRoyaltyAddress = [address(0), address(0)];
        marketplaceRoyaltyFee = [0, 0];
    }
    ...
    */    
    function createOrder(Order memory order) external returns (bool) {
        ...
        if (order._contractType == 0) {
            require(
                msg.sender == IERC721Upgradeable(order._nftAddress).ownerOf(order._tokenId)
            );
            require(IERC721Upgradeable(order._nftAddress).isApprovedForAll(msg.sender, address(this)), "Should be approved");
        } else if (order._contractType == 1) {
            require(
                IERC1155Upgradeable(order._nftAddress).balanceOf(
                    msg.sender,
                    order._tokenId
                ) >= order._amount
            );
            require(IERC1155Upgradeable(order._nftAddress).isApprovedForAll(msg.sender, address(this)), "Should be approved");
        }
    ...
    */
    function cancelOrder(Order memory order) external nonReentrant returns (bool) {
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (orders[hash]._currencyTokenAddress == address(0)) {
            ...
            // Native token
            require(
                IERC20Upgradeable(orders[hash]._currencyTokenAddress).allowance(
                    msg.sender,
                    address(this)
                ) >= order._price * amount,
                "Invalid Allow"
            ...
            IERC20Upgradeable(orders[hash]._currencyTokenAddress).safeTransferFrom(
                msg.sender,
                address(this),
                order._price * amount
            );
    ...
    * @param order Order detail
    */
    function refundBid(Order memory order) external onlyOwner nonReentrant {
        bytes32 hash = hashOrder(order);
        ...
        if (order._currencyTokenAddress == address(0)) {
            ...
            IERC20Upgradeable(orders[hash]._currencyTokenAddress).safeTransfer(
                orders[hash]._buyer,
    ...
    * @param order Order detail
    */
    function placeBid(Order memory order) external payable nonReentrant returns (bool) {
        bytes32 hash = hashOrder(order);
        require(existOrder[hash] == true, "Order does not exist");
        require(
            msg.sender != orders[hash]._creator,
        ...
        if (orders[hash]._buyPrice != orders[hash]._price) {
            if (order._currencyTokenAddress == address(0)) {
                ...
                IERC20Upgradeable(order._currencyTokenAddress).safeTransfer(
                    order._buyer,
                    orders[hash]._buyPrice * orders[hash]._amount
                );
            }
    ...
    */
    function makeOffer(
        ...
        if (_offerCurrencyTokenAddress == address(0)) {
            ...
            require(
                IERC20Upgradeable(_offerCurrencyTokenAddress).allowance(
                    msg.sender,
                    address(this)
                ) >= _price * _amount,
                "Invalid Allow"
            ...
            IERC20Upgradeable(_offerCurrencyTokenAddress).safeTransferFrom(
                msg.sender,
                address(this),
                _price * _amount
            );
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (token._contractType == 0) {
            require(
                msg.sender == IERC721Upgradeable(token._address).ownerOf(token._tokenId), "You are not nft owner"
            );
        } else if (token._contractType == 1) {
            require(
                IERC1155Upgradeable(token._address).balanceOf(msg.sender, token._tokenId) >=
    ...
    */
        ...
        returns (bool)
    {
        ...
        if (offer._offerCurrencyTokenAddress == address(0)) {
            ...
            IERC20Upgradeable(offer._offerCurrencyTokenAddress).safeTransfer(
                offer._offerAddress,
                offer._offerPrice * offer._offerAmount
            );
        }
    ...
    {
        bytes32 hash = hashOrder(order);
        Order memory _order = orders[hash];

        require(existOrder[hash] == true, "Order does not exist");
        require(_order._amount >= amount, "Insufficient NFT Order Balance");
        require(
            getOrderState(_order) == OrderState.FINISHED,
            "Order must be finished"
        );
        verifySignature(signature, _order._nftAddress, royalty_addresses, royalty_bips);

        cancelledOrFinalized[hash] = true;
        existOrder[hash] = false;

        if (_order._creator == _order._buyer) {
            emit NFTY_OrderFinalized(order, amount);
            return true;
        }

        uint256 price = _order._buyPrice * amount;

        withdrawToken(
            Token(_order._contractType, _order._nftAddress, _order._tokenId),
            _order._creator,
            _order._buyer,
            amount
        );
        ...
        _withdrawFunds(
            ...
            _order._creator,
            price,
            _order._currencyTokenAddress,
            royalty_addresses,
            royalty_bips
        );

        _order._amount -= amount;
        
        if (_order._amount > 0) {
            bytes32 newHash = hashOrder(_order);
            orders[newHash] = _order;
    ...
    function withdrawToken(
        ...
        if (_token._contractType == 0) {
            IERC721Upgradeable(_token._address).transferFrom(_from, _to, _token._tokenId);
        } else if (_token._contractType == 1) {
            IERC1155Upgradeable(_token._address).safeTransferFrom(
                _from,
                _to,
                _token._tokenId,
                _amount,
    ...
    function _withdrawFunds(
        ...
        if (royalty_length > 0) {
            ...
            //transfer Token
                ...
                for (uint8 i = 0; i < royalty_length; i++) {
                    ...
                    if (royalty_address != address(0)) {
                        IERC20Upgradeable(_currencyTokenAddress).safeTransfer(
                            royalty_address,
                            (_price * royalty_fee) / BASE_BIPS
                        );
                        total_royalty_fee += (_price * royalty_fee) / BASE_BIPS;
        ...
        } else if (IERC165Upgradeable(_nftAddress).supportsInterface(type(IERC2981Upgradeable).interfaceId)) {
            (address receiver, uint256 royalty_amount) = IERC2981Upgradeable(_nftAddress).royaltyInfo(_tokenId, _price);
            total_royalty_fee = _takeMarketplaceFee(_currencyTokenAddress, _price);
            // Transfer eth
            if (_currencyTokenAddress == address(0)) {
    ...
    function _takeMarketplaceFee(address _currency, uint256 _price) private returns(uint256 total_royalty_fee) {
        ...
        // token
            for (uint8 i = 0; i < 2; i++) {
                if (marketplaceRoyaltyAddress[i] != address(0)) {
                    uint256 fee = (_price * marketplaceRoyaltyFee[i]) / BASE_BIPS;
                    IERC20Upgradeable(_currency).safeTransfer(
