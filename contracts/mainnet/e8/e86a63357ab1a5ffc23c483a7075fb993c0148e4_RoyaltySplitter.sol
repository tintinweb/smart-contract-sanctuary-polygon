...
{ 
  ...
  IERC20Upgradeable[] private _erc20Accepted;
  ...
  mapping(IERC20Upgradeable => uint256) private _erc20TotalAccountedFor;
  mapping(IERC20Upgradeable => uint256) private _erc20TotalUnaccountedReleased;
  mapping(IERC20Upgradeable => mapping(address => uint256)) private _erc20AccountedFor;
  mapping(IERC20Upgradeable => mapping(address => uint256)) private _erc20UnaccountedReleased;
  mapping(IERC20Upgradeable => mapping(address => uint256)) private _erc20AccountedReleased;
  ...
  function initialize(
    ...
    uint256[] memory shares_,
    address admin_
  ) public payable initializer validRecipients(recipients_, shares_) {
    __ReentrancyGuard_init();
    _setupRole(DEFAULT_ADMIN_ROLE, admin_);
    _setupRole(MANAGER_ROLE, admin_);
   ...
   */
  function release(IERC20Upgradeable token, address account) 
    external nonReentrant
  {
    //get the amount accounted for
   ...
   */
  function accept(IERC20Upgradeable token) external onlyRole(MANAGER_ROLE) {
  ...
  function batchUpdate(
    address[] memory recipients, 
    uint256[] memory shares_
  ) external validRecipients(recipients, shares_) onlyRole(MANAGER_ROLE) {
    //account for the unaccounted for
    _accountFor();
   ...
   */
  function _accountedFor(IERC20Upgradeable token, address account) 
   ...
   */
  function _unaccountedFor(IERC20Upgradeable token, address account) 
    internal view returns(uint256) 
  {
   ...
   */
  function _totalAccountedFor(IERC20Upgradeable token) 
    internal view returns(uint256) 
  {
   ...
   */
  function _totalUnaccountedFor(IERC20Upgradeable token) 
    internal view returns(uint256) 
  {
  ...
  function _accountFor() internal {
    ...
    for (uint256 j = 0; j < _erc20Accepted.length; j++) {
      //get erc20 token
      IERC20Upgradeable token = _erc20Accepted[j];
      //get erc20 balance
      balance = (
   ...
   * unaccounted balance to an account vault
   */
  function _accountFor(IERC20Upgradeable token, address account, uint256 balance) 
    internal 
  {
    uint256 unaccounted = _account(
      balance,
...
interface IRoyaltySplitter {
   ...
   */
  function accept(IERC20Upgradeable token) 
    external;
