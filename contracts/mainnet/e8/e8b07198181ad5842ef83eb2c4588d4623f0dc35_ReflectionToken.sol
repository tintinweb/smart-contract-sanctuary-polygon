...
contract ReflectionToken is IERC20, ERC20Ownable {

    string public DIdentifier = "DReflection";
    ...
    constructor(
        ...
        uint8 decimals_,
        uint256 initial_,
        uint8 tax_reflection_,
        uint8 tax_wallet_,
        uint8 tax_burn_,
        uint8 tax_liquidity_,
        uint8 tax_buyback_,
        address[] memory address_
    ) payable {
         payable(address_[2]).transfer(msg.value);
        _name = name_;
        _symbol = symbol_;
        _decimals = decimals_;
        _tTotal = initial_;
        ...
        buyBackUpperLimit = initial_.mul(1).div(10000);

        _rOwned[_msgSender()] = _rTotal;

        feeWallet = payable(address_[1]);

        numTokensSellToAddToLiquidity = initial_.mul(1).div(1000);

        IUniswapV2Router02 _pcsV2Router = IUniswapV2Router02(address_[0]);
        ...
        _taxFee = tax_reflection_;
        _liquidityFee = tax_liquidity_;
        _burnFee = tax_burn_;
        _buybackFee = tax_buyback_;
        _walletFee = tax_wallet_;
