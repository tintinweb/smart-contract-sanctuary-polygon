...
contract TheLegion is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("The Legion", "LGN")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 82) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeifasxs2tfg6gfd5agpmkltwrvotfmb4axgczk3xftkmwrkbz7ixay/", Strings.toString(tokenId), ".json")
            );
    }
