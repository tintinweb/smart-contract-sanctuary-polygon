...
contract Exchange is SuAuthenticated, IExchange {
    ...
    function unwrapLP(ICurveLP lpTokenPool, address lpToken, uint256 amount, address requiredToken, uint16 nCoinsLp) public returns (UnwrapData[] memory unwrapData) {
        ...
        uint256[3] memory amounts = lpTokenPool.remove_liquidity(amount, minAmounts);
    ...
    function removeLiquidityTest(
        ...
        TransferHelper.safeTransferFrom(token2sell, msg.sender, address(this), amount);
        address lpTokenPool = lpTokenToPool[token2sell];

        IERC20(token2sell).approve(lpTokenPool, amount);
        uint256[3] memory minAmounts;
        minAmounts[0] = 0;
        minAmounts[1] = 0;
        minAmounts[2] = 0;
        console.log(address(this));
        console.log(token2sell);
        console.log(lpTokenPool);
        console.log(amount);
        console.log(IERC20(token2sell).balanceOf(address(this)));
        console.log(IERC20(token2sell).allowance(address(this), lpTokenPool));
        uint256[3] memory amounts = ICurveLP(lpTokenPool).remove_liquidity(amount, minAmounts);
        console.log("amounts", amounts[0], amounts[1], amounts[2]);
    }
