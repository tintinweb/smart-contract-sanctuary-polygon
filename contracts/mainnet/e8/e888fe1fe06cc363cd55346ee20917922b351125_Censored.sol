...
contract Censored is ReentrancyGuard, AdminControl, ERC721 {
   ...
   */
  function activate(
    uint256 messageStartTime_,
    uint256 messageEndTime_,
   ...
   */
  function message(string memory message_) external payable nonReentrant {
    require(
      (block.timestamp >= messageStartTime && block.timestamp <= messageEndTime && _tokenIndex >= 2) ||
        (msg.sender == owner() && _tokenIndex > 0),
      "Cannot message"
    );
