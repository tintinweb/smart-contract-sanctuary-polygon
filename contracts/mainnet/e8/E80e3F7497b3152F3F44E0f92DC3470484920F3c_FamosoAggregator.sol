...
contract FamosoAggregator {
  ...
  event Swap(address indexed token1, uint256 amount1, address indexed token2, uint256 amount2);
  constructor() {
    owner = msg.sender;
  }

  function setFee(uint256 _fee) external admin {
    Fee = _fee;
  }
  ...
  function swap(
    ...
    returnAmount = returnAmount.mul(10000 - Fee).div(10000);
    ...
    emit Swap(address(fromToken), amount, address(destToken), returnAmount);
  }
  ...
  function swapMulti(
    ...
    emit Swap(address(tokens[0]), amount, address(tokens[tokens.length - 1]), returnAmount);
  }
