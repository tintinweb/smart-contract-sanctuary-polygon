    ...
    Ownable
{
    ...
    ) ERC721("OYABUN Yakuza", "OYABUN_YAKUZA") {
    ...
    function setRoyaltyPercent(uint256 newPercent) public onlyOwner {
        require(newPercent < maxPercent, "Yakuza: percent must be less than max");
        royaltyPercent = newPercent;
    }

    function getOwnedTokens(address user, uint256 offset, uint256 count) public view returns(uint256[] memory) {
        require(offset + count <= ownedTokens[user].length, "Yakuza: user haven't enough tokens");
    ...
    function mint(address to) public {
        require(hasRole(MINTER_ROLE, _msgSender()), "Yakuza: must have minter role to mint");

        _mint(to, _startId + _tokenIdTracker.current());
    ...
    function mintBulk(address[] calldata to, uint256[] calldata ids) public {
        require(to.length == ids.length, "Yakuza: array must be same length");
        for(uint256 i = 0; i < to.length; i += 1) {
            require(ids[i] < _startId, "Yakuza: must be old token");
            _mint(to[i], ids[i]);
        }
    }
    ...
    function pause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Yakuza: must have pauser role to pause");
        _pause();
    }
    ...
    function unpause() public {
        require(hasRole(PAUSER_ROLE, _msgSender()), "Yakuza: must have pauser role to unpause");
        _unpause();
    }
