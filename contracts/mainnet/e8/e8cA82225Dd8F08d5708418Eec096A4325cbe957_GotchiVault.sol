...
contract GotchiVault is IERC173, Initializable,PausableUpgradeable, IERC721ReceiverUpgradeable, IERC1155ReceiverUpgradeable {
    ...
    function getRentalSettings(uint256 _tokenId) public view returns(RentalSettings memory){
    ...
    function addGotchiLending(uint32[] calldata _tokenIds) public onlyApproved{
        ...
        RentalSettings memory defaultSettings = s.defaultSettings;
        
        for(uint256 i = 0; i < _tokenIds.length; i++){

            if(!isPaused(_tokenIds[i])){

                Gotchi memory thisGotchi = s.gotchiMapping[_tokenIds[i]];
