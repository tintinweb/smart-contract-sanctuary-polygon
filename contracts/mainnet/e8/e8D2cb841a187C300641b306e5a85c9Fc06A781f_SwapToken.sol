...
contract SwapToken is ReentrancyGuard {
    ...
    function addInDefinedTokenWithAggregator(
        ...
        require(!definedTokens[_tokenAddress], "CLAMP-V1: TOKEN ADDRESS EXISTS"); // Check that the address is not already in the mapping
    ...
    function removeFromDefinedTokenWithAggregator(
        ...
        require(definedTokens[_tokenAddress],"CLAMP-V1: TOKEN ADDRESS DOES NOT EXISTS"); // Check that the address is in the mapping
    ...
    function MaticToMultiSwapWithReceiver( address[] memory _tokenAddresses, uint256[] memory _tokenAmounts, uint256 _amountTotal, address _receiver ) public payable nonReentrant {
        require(
            _tokenAddresses.length > 1 && _tokenAddresses.length < 16,
            "CLAMP-V1: INDEX-LENGTH-INVALID"
        );
        require(
            _tokenAddresses.length == _tokenAmounts.length,
            "CLAMP-V1: Mismatched token addresses and amounts"
        );
        require(msg.value > 0, "CLAMP-V1: MSG.VALUE CONDITION FAILED - 1"); //will change msg.value in future
        require(
            msg.value == amountWithFee(_amountTotal),
            "CLAMP-V1: MSG.VALUE CONDITION FAILED - 2"
        );
        ...
        require(
            ...
            "CLAMP-V1: NOT DEFINED AND UNIQUE"
        );
...
contract Wallet is Initializable {
    ...
    */
    ...
    address public CLAMP_V1_SWAPTOKEN;
    ...
    */
    ...
    /**************************************************************************************** MODIFIERS *********************************************************************************************/
    modifier onlyOwner() {
        require(msg.sender == Owner, "CLAMP-V1: NOT-WALLET-OWNER");
        _;
    }
    modifier onlyClampContract() {
        require(msg.sender == CLAMP_V1_SWAPTOKEN, "CLAMP-V1: ERROR-UPDATE");
        _;
    }
   ...
   */
    ...
    */
    ...
    function internalSwap(
        ...
        require(
            ...
            "CLAMP-V1: INDEX-NOT-FOUND"
        );
        require(
            _percentage > 0 && _percentage <= 1000,
            "CLAMP-V1: INVALID-PERCENTAGE"
        );
        require(
            SWAPTOKEN.isDefinedToken(_returnBaseToken),
            "CLAMP-V1: INVALID-BASE-TOKEN"
        );
    ...
    */
    ...
    function getAmountOutMin(
        ...
        int256 price = getDerivedPrice(_tokenIn, _tokenOut, 8);
        if (price <= 0) {
            revert("CLAMP-V1: INVALID-ORACLE-PRICE");
        }
        uint256 tokenIndecimal = IERC20(_tokenIn).decimals();
        uint256 tokenOutdecimal = IERC20(_tokenOut).decimals();
        uint256 amountOut;
