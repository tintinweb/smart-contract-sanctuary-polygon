    ...
    function swap(SwapArgs memory swapArgs, bool transferToRecipient) internal returns (uint256 amountOut) {
        ...
        for (ss.i; ss.i < ss.hopsLength; ) {
            ...
            if (ss.i == ss.hopsLength - 1) {
                amountOut = hopToAssetAddress.getBalanceOf(ss.recipient) - startingBalance;
...
import {LibSimulator} from "../LibSimulator.sol";

contract SimulatorFacet is ISimulator {
    ...
    function simulateSwap(SwapArgs calldata swapArgs) external override returns (uint256 amountOut) {
        ...
        amountOut = LibSimulator.simulateSwap(swapArgs);
    ...
    function simulateTransfer(
        ...
        LibPauser.enforceIsNotPaused();
        LibGuard.enforcePreGuard();
        amountOut = LibSimulator.simulateTransfer(swapArgs, shouldTransfer, useTransferFrom);
        LibGuard.enforcePostGuard();
    }
}
...
library LibSimulator {
    ...
    function simulateSwap(SwapArgs memory swapArgs) internal returns (uint256 amountOut) {
        ...
        if (fromAssetAddress == toAssetAddress) {
            ...
            amountOut = LibRouter.swap(swapArgs, false);
        }
    ...
    function simulateTransfer(
        ...
        if (shouldTransfer) {
            if (useTransferFrom) {
                s.deposits[toAssetAddress] += amountOut;
                s.depositsByUser[toAssetAddress][msg.sender] += amountOut;
                toAssetAddress.approve(address(this), amountOut);
