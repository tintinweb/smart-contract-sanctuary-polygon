...
contract Banka {
    ...
    function profit(address addr) public view returns(uint) {
        if (games[totalGames].timeEnd < block.timestamp) {
            return games[totalGames].lastPlayer == addr ? ((games[totalGames].totalInPot + winners[addr])/100)*95 : (winners[addr]/100)*95;
        }
        else {
            return (winners[addr]/100)*95;
        }
    }
    function makeBet(uint amount) public {
        require(amount > 0, "your bet need to be more than 0");
        if (games[totalGames].timeEnd < block.timestamp) {
            winners[games[totalGames].lastPlayer] = winners[games[totalGames].lastPlayer] + games[totalGames].totalInPot;
            IERC20(ERC20).transferFrom(msg.sender, address(this), amount);
            totalGames++;
            games.push(Game(block.number, block.timestamp + 100, amount, amount, msg.sender));
            ...
            IERC20(ERC20).transferFrom(msg.sender, address(this), amount);
            games[totalGames] = Game(games[totalGames].blockStarted, block.timestamp + 100, games[totalGames].totalInPot + amount, amount, msg.sender);
            emit newBet(msg.sender, amount, block.timestamp);
        }
    }
    function withdraw() public {
        ...
        if (msg.sender == games[totalGames].lastPlayer) {
            winners[games[totalGames].lastPlayer] = winners[games[totalGames].lastPlayer] + games[totalGames].totalInPot;
            IERC20(ERC20).transfer(msg.sender, (winners[msg.sender]/100)*95);
            IERC20(ERC20).transfer(owner, (winners[msg.sender]/100)*5);
