...
contract SimplePresaleContract is Ownable {

    bool public isPaused = true;
    uint public maxBuyAmount; //In whole Ether
    uint public minBuyAmount; //In whole Ether
    uint public tokensPerEth;
    uint ethDecimals = 18;

    uint8 tokenDecimals;
    NGonToken public tokenForSale;


    constructor(NGonToken _tokenAddress, uint _tokensPerEth, uint _minBuyAmount, uint _maxBuyAmount){
        tokensPerEth = _tokensPerEth;
        maxBuyAmount = _maxBuyAmount * 1 ether;
        minBuyAmount = _minBuyAmount * 1 ether;
        tokenForSale = _tokenAddress;
        tokenDecimals = tokenForSale.decimals();
    }

    function setPauseState() public onlyOwner{
        if (isPaused == true) {
            isPaused = false;
        } else {
            isPaused = true;
    ...
    function purchase() public payable {
        require(!isPaused, "Presale isn't active yet or has ended already");
        require(msg.value <= maxBuyAmount, "That amount is above the maximum purchase limit");
        require(msg.value >= minBuyAmount, "That amount is below the minimum purchase limit");
        require(tokenForSale.balanceOf(msg.sender) < maxBuyAmount, "You already have the maximum purchase amount for this sale");
        uint leftInPresale = availableToBuy();
        address recipient = msg.sender;
        uint dec = ethDecimals - tokenDecimals;
        uint256 amountOfToken = (tokensPerEth * msg.value) / 10**dec;
        require(leftInPresale >= amountOfToken, "Contract hasn't enough balance left. Try buying less");

        tokenForSale.transfer(recipient, amountOfToken);
    }
    ...
    function availableToBuy() public view returns (uint) {
        uint available = tokenForSale.balanceOf(address(this));
        return available;
    }

    function setMaxAndMinAmounts(uint _maxBuyAmount, uint _minBuyAmount) public onlyOwner {
        maxBuyAmount = _maxBuyAmount * 1 ether;
        minBuyAmount = _minBuyAmount * 1 ether;
    }

    function setRate(uint _tokensPerEth) public onlyOwner {
        tokensPerEth = _tokensPerEth;
    }
