...
contract Sec is ISec, UUPSUpgradeable, Initializable {
    ...
    mapping(address => OwnerConfig) public _ownerConfig;
    ...
    function rmGdAccount(address gdAccount) public {
        ...
        if (_ownerConfig[sender]._guarding.length > 1) {
            address[] memory newGdList = new address[](
                _ownerConfig[sender]._guarding.length - 1
            );
            uint256 newIndex = 0;
            for (
                uint256 i = 0;
                i < _ownerConfig[sender]._guarding.length;
                i++
            ) {
                if (_ownerConfig[sender]._guarding[i] != gdAccount) {
                    newGdList[newIndex] = _ownerConfig[sender]._guarding[i];
                    newIndex++;
                }
            }
            _ownerConfig[sender]._guarding = newGdList;
            caGuarding[sender][gdAccount] = false;
            _whoIProtect[gdAccount].protectStatus[sender] = 2;
            emit RmGD(sender, gdAccount, block.timestamp);
        } else {
            require(dg == gdAccount, "need one caGuarding");
            _ownerConfig[sender]._guarding = new address[](1);
            _ownerConfig[sender]._guarding[0] = dg;
            caGuarding[sender][gdAccount] = false;
    ...
    mapping(uint256 => bool) public retrieveNonce;
    ...
    function retrieve(
        ...
        if (_ownerConfig[anNewOwner]._guarding.length < 1) {
            ...
            for (uint256 i = 0; i < oldConfig._guarding.length; i++) {
                if (!isGuarding(anNewOwner, oldConfig._guarding[i])) {
                    _ownerConfig[anNewOwner]._guarding.push(
                        oldConfig._guarding[i]
                    );
                    _whoIProtect[oldConfig._guarding[i]]._eoa.push(anNewOwner);
                    _whoIProtect[oldConfig._guarding[i]].protectStatus[
                        anNewOwner
        ...
        for (uint256 i = 0; i < len; i++) {
            ...
            caGuarding[anNewOwner][oldConfig._caAddress[i]] = true;
            caGuarding[anOldOwner][oldConfig._caAddress[i]] = false;
    ...
    function checkRetieve(address anOldOwner, address anNewOwner) public {
        ...
        if (_ownerConfig[anNewOwner]._guarding.length < 1) {
            emit RetrieveDebug(anOldOwner, anNewOwner, 0, 0);
            for (uint256 i = 0; i < oldConfig._guarding.length; i++) {
                _ownerConfig[anNewOwner]._guarding.push(oldConfig._guarding[i]);
                _whoIProtect[oldConfig._guarding[i]]._eoa.push(anNewOwner);
                _whoIProtect[oldConfig._guarding[i]].protectStatus[
            ...
            for (uint256 i = 0; i < oldConfig._guarding.length; i++) {
                emit RetrieveDebug(anOldOwner, anNewOwner, 1, i);
                if (!isGuarding(anNewOwner, oldConfig._guarding[i])) {
                    _ownerConfig[anNewOwner]._guarding.push(
                        oldConfig._guarding[i]
                    );
        ...
        for (uint256 i = 0; i < len; i++) {
            emit RetrieveDebug(anOldOwner, anNewOwner, 2, i);
            _anOwner[oldConfig._caAddress[i]] = anNewOwner;
            _ownerConfig[anNewOwner]._caAddress.push(oldConfig._caAddress[i]);
            caGuarding[anNewOwner][oldConfig._caAddress[i]] = true;
            caGuarding[anOldOwner][oldConfig._caAddress[i]] = false;
