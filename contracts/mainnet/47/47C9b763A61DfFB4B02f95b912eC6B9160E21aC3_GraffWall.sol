...
contract GraffWall is Ownable, GroupedURIs, ERC721Burnable {
    using Strings for uint256;
    ...
    /// @notice returns contract total supply
    function totalSupply() public view returns (uint256) {
        return lastTokenId - burned;
    }
    ...
    {
        ownerOf(tokenId);
        return groupBaseURI[tokenGroup[tokenId]];
    }
    ...
    /// @notice Allows an owner to burn their token
    ...
    /// @notice allows owner to mint `uri` (or current group uri) to recipients
    /// @param uri the new URI if new group
    /// @param recipients the recipients to mint to
    function mintBatch(string calldata uri, address[] calldata recipients)
        external
        onlyOwner
    {
        // if there is a uri, create a new Group
        if (bytes(uri).length > 0) {
            _incrementGroup("", uri);
        }

        uint256 tokenId = lastTokenId + 1;
        uint256 groupId = currentGroupId;
        uint256 length = recipients.length;

        for (uint256 i; i < length; i++) {
            _mint(recipients[i], tokenId + i);
            _setTokenGroup(tokenId + i, groupId);
        }
