...
// File: CapsuleContract_0.sol

pragma solidity >=0.8.0 <0.9.0;



contract CapsuleContract_0 is ICapsuleContract, AccessControl{
    ...
    uint256 public constant endAt=99165078720000;

    /// @dev Maximum number of capsules this capsule machine can generate.
    uint256 public constant maxCapsuleNumber=1000;
    ...
    function createCapsule(address caller,bool triple) external override onlyELFCore returns(uint256[] memory, uint256){
        require(caller==admin,NO_PERMISSION);
        uint256 count=1;
        if (triple){
            count=3;
        }
