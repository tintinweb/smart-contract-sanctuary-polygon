	...
	NativeMetaTransaction
{
	...
	IRaffler private _raffler;
	string private _baseTokenURI;
	uint256 private immutable _cap;
	address[] private _proxies;

	event RafflerHasChanged(address to);
	...
	constructor(
		...
		require(cap > 0, "ERC721Rafflable: max supply is 0");
	...
	function isApprovedForAll(
		...
		for (uint i = 0; i < _proxies.length && i < 5; i++) {
			if (_operator == _proxies[i]) {
				return true;
			}
		}
	...
	function allowProxy(address proxy) external onlyOwner {
		// OpenSea: 0x58807baD0B376efc12F5AD86aAc70E78ed67deaE
		_proxies.push(proxy);
	}

	function allowedProxy(uint index) external view onlyOwner returns (address) {
	...
	function mint(uint256 amount) public onlyOwner {
		require(totalSupply() + amount <= maxSupply(), "ERC721Rafflable: max supply reached");
		...
		override(ERC721)
	{
		...
		if (totalSupply() > 0 && _raffler != IRaffler(address(0))) {
			...
			_raffler.addTicket(tokenId, uint256(seed) % totalSupply() + 1); 
			_raffler.tryRaffleDraw(seed);
		}
	}
...
interface IRaffler {
	...
	event RaffleTokenTransfer(address winner, address token, uint256 amount);
	function addTicket(uint256 tokenId, uint256 ticketId) external returns (bool);
	function tryRaffleDraw(bytes32 seed) external returns (bool);
}
