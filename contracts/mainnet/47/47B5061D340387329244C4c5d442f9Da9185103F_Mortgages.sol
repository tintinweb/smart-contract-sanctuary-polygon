...
contract Mortgages is AccessControl, ReentrancyGuard {
    ...
    event releaseEvent(
        uint256 idMortgage, address wallet, address collection, uint256 nftId, string status, uint256 quantity
    );
    event seized(
        uint256 idMortgage, address wallet, address collection, uint256 nftId, string status, uint256 quantity
    );
    ...
    constructor(
        ...
        address _referalsControl,
        address _market,
        address _token,
        address foundersRewards,
        address RewardsLenders,
        address _membershipVault,
        address _referalVault,
        address _membershipValuation,
    ...
    function openMortgage(
        ...
        uint256 period,
        address token
    ) public validToken(token) nonReentrant NotPaused returns (uint256 id) {
        ...
        //Create and wrap NFT
        INFTMarket(market).mintingMortgage(
            locals._collection, locals2.tokenId, address(this), locals3.holder, _quantity, locals.price
        );
        IWrapMembership(locals.wrapContract).wrapNFT(locals3.holder, locals2.tokenId, _quantity);
        IMembership(locals._collection).safeTransferFrom(
            address(this), locals.wrapContract, locals2.tokenId, _quantity, ""
        );
        ...
        IMortgageControl(mortgageControl).addIdInfo(id, locals3.holder);
        IMortgageControl(mortgageControl).updateLastTimeCalc(locals3.holder, id, block.timestamp);

        emit mortgageEvent(
            id, locals3.holder, locals._collection, locals.wrapContract, locals2.tokenId, locals2.debt, _quantity
        );
    }

    function openManualMortgage(
        address collection,
        uint256 tokenId,
        uint256 loan,
        uint256 _quantity,
        ...
        address _sponsor,
        address _owner
    ) public validToken(token) onlyManual nonReentrant NotPaused returns (uint256 id) {
        LocalMortgage memory locals;
        LocalMortgage2 memory locals2;
        LocalMortgage3 memory locals3;
        ...
        if (_sponsor != address(0)) {
            //Verify if the user has already been sponsored
        ...
        //Create and wrap NFT
        INFTMarket(market).mintingMortgage(
            locals._collection, locals2.tokenId, address(this), _owner, locals3._quantity, locals.price
        );
        IWrapMembership(locals.wrapContract).wrapNFT(_owner, locals2.tokenId, locals3._quantity);
        IMembership(locals._collection).safeTransferFrom(
            address(this), locals.wrapContract, locals2.tokenId, locals3._quantity, ""
        );
        //Register information
        IMortgageControl(mortgageControl).addRegistry(
            id,
        ...
        emit mortgageEvent(
            id, _owner, locals._collection, locals.wrapContract, locals2.tokenId, locals2.debt, locals3._quantity
        );
    }

    function openPresaleMortgage(
        address collection,
        address _owner,
        uint256 tokenId,
        uint256 loan,
        ...
        locals2.debt = submitFees(token, locals.value, locals3._loan);
        //Create and wrap NFT
        INFTMarket(market).mintingPresaleMortgage(
            locals._collection, locals2.tokenId, address(this), locals3.holder, _quantity, locals.price
        );
        IWrapMembership(locals.wrapContract).wrapNFT(locals3.holder, locals2.tokenId, _quantity);
        IMembership(locals._collection).safeTransferFrom(
    ...
    function openManualPresaleMortgage(
        address collection,
        uint256 tokenId,
        uint256 loan,
        uint256 _quantity,
    ...
    function mortgagePaid(uint256 idMortgage, address collection, address user, uint256 tokenId, uint256 _quantity)
        public
        nonReentrant
        NotPaused
    {
    ...
    function seizeNFT(uint256 idMortgage, address collection, address user, uint256 tokenId, uint256 _quantity)
        public
        NotPaused
        onlyRelayer
    {
    ...
    function verifySponsor(address _sponsor, address _sponsored) private returns (bool) {
        ...
        if (referalsControl.getReferredStatus(_sponsored)) {
            //Verify if the user has already been sponsored
    ...
    function permissions(
        address _token,
        address _foundersRewards,
        address _RewardsLenders,
        address _membershipVault,
        address _referalVault
    ) public onlydev validToken(_token) {
