...
contract ViaRouter is Ownable, ReentrancyGuard {
    ...
    using Transfers for Transfers.Token;
    ...
    struct Call {
        address adapter;
        Transfers.Token fromToken;
        Transfers.Token toToken;
        bytes args;
    }
    ...
    event CallForAnotherChain(Call call);
    ...
    function initializeExecution(
        ...
        address fromToken = calls[0].fromToken.token;
    ...
    function _execute(
        ...
        require(
            calls[0].fromToken.chainId == block.chainid,
            "invalid calls[0].chainid"
        );
        ...
        bool skipFlag = false;
        Transfers.Token memory fromToken = calls[0].fromToken;
        Transfers.Token memory toToken = calls[0].fromToken;

        for (uint8 i = 1; i < calls.length; i++) {
            require(
                calls[i].fromToken.equals(calls[i].toToken),
                "Non continuous swap path"
            );
        }
        ...
        for (uint8 i; i < calls.length; i++) {
            ...
            if (calls[i].fromToken.chainId == block.chainid && !skipFlag) {
                uint256 toTokenAmount = Transfers.getBalance(toToken);

                // Use delegate call through adapter
                require(adapters[call.adapter], "Not an adapter");
                call.adapter.functionDelegateCall(
                ...
                if (i == calls.length - 1) {
                    if (receiver != address(0)) {
                        ...
                        emit TransferToReceiver(
                            receiver,
                            toToken.token,
                            toTokenAmount
                        );
                    } else {
                        require(toTokenAmount == 0, "Should specify receive");
                    }
                }
            } else {
                // do we need to pass all chain?
                skipFlag = true;
                emit CallForAnotherChain(call);
            }
        }
