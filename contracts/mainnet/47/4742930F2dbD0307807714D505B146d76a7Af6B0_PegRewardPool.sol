...
contract PegRewardPool is Operator, ReentrancyGuard {
    ...
    address public yw31Receipt = address(0xfA65b4726637d53D169ae1c865D6848Fd7353C4E);
    ...
    event Initialized(uint256 initTime);

    modifier onlyTreasuryOrOperator() {
        require(msg.sender == treasury || msg.sender == operator(), "Not allowed");
        _;
    }
    ...
    function initialize(
        ...
        require(_treasury != address(0), "!token");
    ...
    function getSpaceUsdcLpOwned(address _user) public view returns (uint256 _lp_owned, uint256 _receipt_staked) {
        ...
        _receipt_staked = user.amount;
        IYWReceipt receipt_token = IYWReceipt(yw31Receipt);
    ...
    function getTotalLpStaked() public view returns (uint256) {
        IYWReceipt receipt_token = IYWReceipt(yw31Receipt);
        uint256 yw31_bal = IERC20(yw31Receipt).balanceOf(address(this));
        uint256 total_SpaceUsdcLp_Staked = receipt_token.totalStakeTokens();
        uint256 total_receipt_supply = receipt_token.totalSupply();
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 0) {
                IERC20 spaceLp = IERC20(spaceUsdcLp);
                if (_taxAmount > 0) spaceLp.safeTransferFrom(msg.sender, daoFund, _taxAmount);

                spaceLp.safeTransferFrom(msg.sender, address(this), _amount_post_fee);
                // Deposit SpaceLP to YW Vault
                uint256 receipt_token_before_deposit = IERC20(yw31Receipt).balanceOf(address(this));
                spaceLp.safeApprove(yieldWolfVault, 0);
                spaceLp.safeApprove(yieldWolfVault, _amount_post_fee);
                vault.deposit(vaultPid, _amount_post_fee);
                uint256 receipt_token_after_deposit = IERC20(yw31Receipt).balanceOf(address(this));
                uint256 amount_receipt_to_stake = receipt_token_after_deposit.sub(receipt_token_before_deposit);

                user.amount = user.amount.add(amount_receipt_to_stake);
    ...
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        UserInfo storage user = userInfo[_pid][_sender];
        if (_pid == 0) {
            (uint256 _lpOwned,) = getSpaceUsdcLpOwned(_sender);
            require(_amount <= _lpOwned, "withdraw: not good");
        } else {
        ...
        if (_amount > 0) {
            ...
            if (_pid == 0) {
                ...
                spaceLp.safeApprove(yieldWolfVault, 0);
                spaceLp.safeApprove(yieldWolfVault, _amount);
                uint256 receipt_bal_before_withdraw = pool.token.balanceOf(address(this));
                vault.withdraw(vaultPid, _amount);
                uint256 receipt_bal_after_withdraw = pool.token.balanceOf(address(this));
                uint256 receipt_burnt = receipt_bal_before_withdraw.sub(receipt_bal_after_withdraw);
                user.amount = user.amount.sub(receipt_burnt);
                spaceLp.safeTransfer(_sender, _amount.sub(_taxAmount));
                if (_taxAmount > 0) spaceLp.safeTransfer(daoFund, _taxAmount);
    ...
    function addReward(uint256 _amount) external onlyTreasuryOrOperator {
