...
contract TTClub {
    ...
    uint8 public constant MANAGERS_AMOUNT = 6;
    ...
    constructor(address[MANAGERS_AMOUNT] memory _managers) {
        managers = _managers;
    }
    ...
    function addDiamonds(address ref) public payable {
        ...
        if (ref != address(0)) {
            clubs[ref].diamonds += (diamonds * 6) / 100;
            clubs[ref].bucks += (diamonds * 100 * 6) / 100;
