...
interface IRoyaltyRegistry {
    ...
    function setRoyalty(address _nft_address, address payable _payoutAddress, uint256 _payoutBasisPoints) external;
    function getRoyaltyPayoutAddress(address _nft_address) external view returns (address payable);
    function getRoyaltyPayoutRate(address _nft_address) external view returns (uint256);
}
...
interface extraERC1155{
    function minted_records(uint256 id) external view returns (bool);
    function safe_mint_ext(address account, uint256 id) external;
    function safeTransferFrom_ext(address from, address to, uint256 id, uint256 amount, bytes memory data) external;
}

contract NFTGT_MarketplaceV1 is Ownable, Pausable, ReentrancyGuard {
    ...
    uint256 private _makerFeepayoutBasisPoints = 250; // check.. _makerFeePerMille
    uint256 private _maxpayoutBasisPoints = 1500; //check..payoutBasisPoints _maxRoyaltyPerMille

    bytes32 private EIP712_DOMAIN_TYPE_HASH = keccak256("EIP712Domain(string name,string version)");
    bytes32 private DOMAIN_SEPARATOR = keccak256(abi.encode(
            EIP712_DOMAIN_TYPE_HASH,
            keccak256(bytes("NFTGT")),
            keccak256(bytes("1"))
        ));
    ...
    */ // offchain list then buyer buy on-chain

    function set_marketplace_fee(uint256 _feeBasisPoint) public onlyOwner {
        require(_feeBasisPoint <= _maxpayoutBasisPoints, "Fee is too High");
        _makerFeepayoutBasisPoints = _feeBasisPoint; 
    }
    ...
    function fillSellOrder(
        ...
        require (address(buyer) == address(msg.sender), "Only buyer can fillSellOrder");
    ...
    */ // someone offer to buy that nft then the owner sell it
    function fillBuyOrder(
        ...
        require (address(seller) == address(msg.sender), "Only seller can fillBuyOrder");
        _checkValidERC20Payment(buyer, price, paymentERC20);
    ...
    */
    ...
    // TBD get royalty from ERC2981 standard
    /*
    * @dev Sets the royalty as an int out of 10000 that the creator should receive and the address to pay.
    */ //_makerFeepayoutBasisPoints
    function setRoyalty(address contractAddress, address payable _payoutAddress, uint256 _payoutBasisPoints) external {
        require(_payoutBasisPoints <= _maxpayoutBasisPoints, "Royalty must be between 0 and 15%");
        ...
        royaltyRegistry.setRoyalty(contractAddress, _payoutAddress, _payoutBasisPoints);
    }
    ...
    */ //_makerFeepayoutBasisPoints
    function _sendETHPaymentsWithRoyalties(address contractAddress, address payable finalRecipient) internal {
        uint256 royaltyPayout = (royaltyRegistry.getRoyaltyPayoutRate(contractAddress) * msg.value) / 10000;
        uint256 makerPayout = (_makerFeepayoutBasisPoints * msg.value) / 10000;
    ...
    function _sendERC20PaymentsWithRoyalties(
        ...
        uint256 royaltyPayout = (royaltyRegistry.getRoyaltyPayoutRate(contractAddress) * price) / 10000;
        uint256 makerPayout = (_makerFeepayoutBasisPoints * price) / 10000;
    ...
    function _checkValidERC20Payment(address buyer, uint256 price, address paymentERC20) internal view {
        ...
        require(IERC20(paymentERC20).balanceOf(buyer) >= price, "Buyer has an insufficient balance of the ERC20.");
        require(
    ...
    */
    ...
    function _transferNFT(address contractAddress, uint256 tokenId, address seller, address buyer, uint256 quantity) internal {
        if (contractAddress.supportsInterface(InterfaceId_ERC721)) {
            ...
            ///  Transfer ///
            /////////////////
            if (extraERC1155(contractAddress).minted_records(tokenId) == true){

                try extraERC1155(contractAddress).safeTransferFrom_ext(seller, buyer, tokenId, quantity, "") {
                } catch {
                    erc1155.safeTransferFrom(seller, buyer, tokenId, quantity, "");
                }

            } else {
                extraERC1155(contractAddress).safe_mint_ext(seller, tokenId);
                erc1155.safeTransferFrom(seller, buyer, tokenId, quantity, "");
