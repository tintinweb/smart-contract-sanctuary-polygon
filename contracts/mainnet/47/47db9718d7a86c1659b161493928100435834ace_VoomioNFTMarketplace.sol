...
contract ERC721 is Context, ERC165, IERC721, IERC721Metadata {
    ...
    /**
     * @dev See {IERC721-safeTransferFrom}ownerOf.
     */
    function safeTransferFrom(
        address from,
        address to,
...
contract VoomioNFT is ERC721, ERC721URIStorage, ERC721Burnable, Ownable {
    ...
    function getnftstatus(uint256 _tokenid) public view returns (ListedToken memory) {
    ...
    function ApproveMarketplace(address contractAddress )
    public onlyOwner
    {
...
// File contracts/VoomioNFTMarketplace.sol
...
contract VoomioNFTFactory {
    ...
    Counters.Counter private collectionaddressCounter;
    

    mapping(uint256 => address) private numberToColeection;
    ...
    function createNFTCollection(
        ...
        // uint totalItemCount = collectionaddressCounter.current();
        uint256 _tokenIdCounter = collectionaddressCounter.current();
         collectionaddressCounter.increment();
        numberToColeection[_tokenIdCounter]= address(nft);
    ...
    function getallCollections() external view returns (address[] memory) {
            uint totalItemCount = collectionaddressCounter.current();
            address[] memory tokens = new address[](totalItemCount);
             for(uint i=0;i<totalItemCount;i++)
                        {
                            // currentId = i + 1;
                            address  currentItem = numberToColeection[i];
...
contract VoomioNFTMarketplace is Ownable, ReentrancyGuard {
    ...
    //     emit ListedNFT(_nft, _tokenId, address(0), _price, msg.sender);
    // }
    ...
    function buyBulkbyeth(BULKBUYLIST[] memory listttt, address _buyerAddress) 
        external
        payable {
            // BULKBUYLIST[] memory data= listttt;
            require(
                _buyerAddress == msg.sender,
                "buyeradddress must be equal to msg.sender"
            );
             for (uint i=0; i<listttt.length; i++) {
            BULKBUYLIST memory item = listttt[i];
            // this.buyByETH(item.NFT,item.tokenId);
            ListNFT storage listedNft = listNfts[item.NFT][item.tokenId];
            require(
                listedNft.seller != address(0) && !listedNft.sold,
                "not listed"
            );
