...
contract OneSplit is Initializable, IOneSplit, OneSplitRoot, UUPSUpgradeable, AccessControlUpgradeable {
    ...
    function swap(
        ...
        _swapFloor(fromToken, destToken, confirmed, flags, _data);
        ...
        uint256 userBalanceOld = destToken.universalBalanceOf(msg.sender);
        destToken.universalTransfer(msg.sender, returnAmount);
        uint256 userBalanceNew = destToken.universalBalanceOf(msg.sender);
    ...
    function swapMulti(
        ...
        for (uint256 i = 1; i < tokens.length; i++) {
            ...
            _swapFloor(tokens[i - 1], tokens[i], returnAmount, flags[i - 1], _data[i-1]);
        ...
        uint256 userBalanceOld = tokens[tokens.length - 1].universalBalanceOf(msg.sender);
        tokens[tokens.length - 1].universalTransfer(msg.sender, returnAmount);
        uint256 userBalanceNew = tokens[tokens.length - 1].universalBalanceOf(msg.sender);
        require(userBalanceNew-userBalanceOld >= minReturn, "ERROR:1003");
