...
contract SwapRouterV1 is Ownable {
    ...
    function toggleSwap() public onlyOwner {
        ...
        emit SwapStateChanged(swapEnabled);
    }
    
    function swapToExactOutputPermit(uint256 amountInMaximum, uint256 amountOut, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (uint256 amountIn) {
        gpo.permit(msg.sender, address(this), amountInMaximum, deadline, v, r, s);
        return swapToExactOutput(amountInMaximum, amountOut, deadline);
    }

    function swapToExactOutput(uint256 amountInMaximum, uint256 amountOut, uint256 deadline) public returns (uint256 amountIn) {
    ...
    function swapToExactInputPermit(uint256 amountIn, uint256 amountOutMinimum, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (uint256 amountOut) {
        gpo.permit(msg.sender, address(this), amountIn, deadline, v, r, s);
        return swapToExactInput(amountIn, amountOutMinimum, deadline);
    }

    function swapToExactInput(uint256 amountIn, uint256 amountOutMinimum, uint256 deadline) public returns (uint256 amountOut) {
    ...
    function swapFromExactOutputPermit(uint256 amountInMaximum, uint256 amountOut, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (uint256 amountIn) {
        IERC20Permit(gpo.addrUSDC()).permit(msg.sender, address(this), amountInMaximum, deadline, v, r, s);
        return swapFromExactOutput(amountInMaximum, amountOut, deadline);
    }

    function swapFromExactOutput(uint256 amountInMaximum, uint256 amountOut, uint256 deadline) public returns (uint256 amountIn) {
        require(amountInMaximum > 0 && amountOut > 0);
        require(swapEnabled);
        ...
        if (maxBuyTradeSize >= amountInMaximum) {
            ...
            revert("GP-SR: Order above buy trade size");
        }
    }

    function swapFromExactInputPermit(uint256 amountIn, uint256 amountOutMinimum, uint256 deadline, uint8 v, bytes32 r, bytes32 s) external returns (uint256 amountOut) {
        IERC20Permit(gpo.addrUSDC()).permit(msg.sender, address(this), amountIn, deadline, v, r, s);
        return swapFromExactInput(amountIn, amountOutMinimum, deadline);
    }

    function swapFromExactInput(uint256 amountIn, uint256 amountOutMinimum, uint256 deadline) public returns (uint256 amountOut) {
        require(amountIn > 0 && amountOutMinimum > 0);
        require(swapEnabled);
