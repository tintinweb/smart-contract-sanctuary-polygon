    ...
    ReentrancyGuard
{
        ...
        )
    {
        ...
        if (paymentToken != token0) {
            ...
            (bool success, bytes memory resultData) = hyperDex.call{value:msg.value}(callData1);
            if (!success) {
        ...
        if (paymentToken != token1) {
            ...
            (bool success, bytes memory resultData) = hyperDex.call{value:msg.value}(callData2);
            if (!success) {
                _revertWithData(resultData);
            }
        ...
        (amount0, amount1, mintAmount, liquidityMinted) = IHyperLPool(hyperpool).mint(amount0, amount1, _msgSender());

        emit Minted(_msgSender(), mintAmount, amount0, amount1, liquidityMinted);
