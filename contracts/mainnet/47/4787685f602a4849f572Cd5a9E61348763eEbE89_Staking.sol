...
//executeables and shit
    ...
    /*\
    set minimum of available withdraw before chainlink automation executes
    \*/
    function setMinForExecution(uint _amount) public onlyOwner returns(bool) {
        minForExecution = _amount;
        return minForExecution == _amount;
    }
...
// view & misc and shit
    ...
    function getIdOf(address _addr) external view returns(uint id) {
        for(uint i=lookFrom; i < runningCount; i++) {
            if(stakeInfo[i].owner == _addr && !stakeInfo[i].closed) {
                id = i;
                break;
            }
        }
    ...
    get reward of id
    \*/
    function getRewardOf(uint _id) public view returns(uint) {
        return getStaked(_id) * _getBalance() / LPAddr.balanceOf(address(this));
    }


    /*\
    get unlock time stamp of id
    \*/
    function getEndOf(uint _id) public view returns(uint) {
        return stakeInfo[_id].unlock;
    }
