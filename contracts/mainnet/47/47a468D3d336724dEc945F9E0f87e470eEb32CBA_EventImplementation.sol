...
contract EventImplementation is IEventImplementation, EventERC721Upgradeable, AuthModifiers {
    ...
    /**
     * @notice Returns a ticket's status of claim
     * @dev Claims may happen once anytime after mint given the ticket is valid
     * @param _tokenId ticket identifier and ERC721 tokenId
     * @return _status claim status
     */
    ...
    /**
     * @notice Sets `isClaimed` to true or false
     * @dev This edits the ticket specific bit-field (tokenData.booleanFlags) by calling _setBoolean
     * @param _tokenId ticket identifier and ERC721 tokenId
     * @param _status claim status
     */
    ...
    /**
     * @notice Mints NFTs as a batch from ticketActions
     * @param _ticketActions array of TicketAction structs for which mints are performed
     * @param _balanceUpdates array of BalanceUpdates struct used to update an owner's balance
     */
    function _mintBatch(TicketAction[] calldata _ticketActions, BalanceUpdates[] calldata _balanceUpdates) internal {
    ...
    /**
     * @notice Transfers NFTs as a batch from ticketActions
     * @param _ticketActions array of TicketAction structs for which transfers are performed
     */
    function _transferFromBatch(TicketAction[] calldata _ticketActions) internal {
        for (uint256 i = 0; i < _ticketActions.length; i++) {
            _transfer(ownerOf(_ticketActions[i].tokenId), _ticketActions[i].to, _ticketActions[i].tokenId);
        }
    }
    ...
    function _primarySale(TicketAction[] calldata _ticketActions, BalanceUpdates[] calldata _balanceUpdates) internal {
        _mintBatch(_ticketActions, _balanceUpdates);
    ...
    function _claim(TicketAction[] calldata _ticketActions) internal {
        ...
        _transferFromBatch(_ticketActions);
