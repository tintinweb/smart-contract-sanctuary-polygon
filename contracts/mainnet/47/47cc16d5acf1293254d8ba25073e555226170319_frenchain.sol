...
// KEK CHAIN  // Blockchain && Domains(.kek)
   ...
   //https://t.me/frenchain
   //frenchain.com/
   //domains.frenchain.com/
...
{
modifier M() {require(msg.sender == multisig);_;}
address public multisig;
address[] public Payees;
bool public Paying;
constructor(address deployer){multisig = deployer;}
receive() external payable {}
fallback() external payable {}
function SetPayees(address[] memory addresslist) external M{require(addresslist.length != 0); Payees = addresslist;}
function ChangeMultiSig(address newsig) M external{multisig = newsig;}
function Payout() public 
{
require(!Paying);
Paying = true;
uint256 balance = address(this).balance;
uint256 division = balance / Payees.length;
for(uint256 x; x<Payees.length;x++)
{
if(payable(Payees[x]).send(division)){}
}
Paying = false; 
}
}

contract frenchain is Context, IERC20, Ownable { 
    ...
    */
    ...
    string private _name = "FrenChain"; 
    ...
    */
    ...
    constructor () {
        Paysplitter PaySplitContract = new Paysplitter(msg.sender);
        Wallet_Dev = payable(address(PaySplitContract));
