...
contract ERC20Claimer is Base {
    ...
    mapping(address => ClaimInfo) internal claimInfo;
    mapping(address => mapping(uint256 => bool)) public claimed;

    IERC20Mintable public claimToken;

    constructor(address settingsAddress, address claimTokenAddress) public Base(settingsAddress) {
        claimToken = IERC20Mintable(claimTokenAddress);
    }
    ...
    function setClaimInfo(
        ...
        uint256 rewardsPerItem,
        uint256 endBlockNumber
    ) external onlyConfigurator(msg.sender) {
        require(endBlockNumber > block.number, "invalid_end_block_number");
        claimInfo[token] = ClaimInfo({
            endBlockNumber: endBlockNumber,
            rewardsPerItem: rewardsPerItem
        });

        emit ClaimInfoSet(token, rewardsPerItem, endBlockNumber);
    }

    function claim(address token) external {
        require(claimInfo[token].endBlockNumber > block.number, "!claim_enabled");
        require(claimInfo[token].rewardsPerItem > 0, "invalid_token");

        uint256 balance = IERC721Enumerable(token).balanceOf(msg.sender);
        require(balance > 0, "!account_token_balance");
        uint256 totalRewards = 0;
        for (uint256 id = 0; id < balance; id++) {
            uint256 tokenId = IERC721Enumerable(token).tokenOfOwnerByIndex(msg.sender, id);
            if (!claimed[token][tokenId]) {
                totalRewards = totalRewards.add(claimInfo[token].rewardsPerItem);
                claimed[token][tokenId] = true;
            }
        }
        require(totalRewards > 0, "!account_rewards");
        IERC20Mintable(claimToken).mint(msg.sender, totalRewards);

        emit TokendsClaimed(msg.sender, token, balance, totalRewards);
    }

    /* View Functions */

    function getClaimInfo(address token)
        external
        view
        returns (uint256 endBlockNumber, uint256 rewardsPerItem)
    {
        return (claimInfo[token].endBlockNumber, claimInfo[token].rewardsPerItem);
    }

    function getPendingRewards(address token, address account)
        external
        view
        returns (uint256 pendingRewards)
    {
        if (
            claimInfo[token].endBlockNumber <= block.number || claimInfo[token].rewardsPerItem == 0
        ) {
            return 0;
        }
        uint256 balance = IERC721Enumerable(token).balanceOf(account);
        for (uint256 id = 0; id < balance; id++) {
            uint256 tokenId = IERC721Enumerable(token).tokenOfOwnerByIndex(account, id);
            if (!claimed[token][tokenId]) {
                pendingRewards = pendingRewards.add(claimInfo[token].rewardsPerItem);
            }
        }
    }
    ...
    event ClaimInfoSet(address token, uint256 rewardsPerItem, uint256 endBlockNumber);

    event TokendsClaimed(
        address account,
        address token,
