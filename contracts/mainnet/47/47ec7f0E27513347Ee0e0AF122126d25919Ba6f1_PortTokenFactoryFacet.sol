...
struct PortTokenFactoryStorage {
    address[] portTokens;
}

contract PortTokenFactoryFacet {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("portfolio.tokenfactory");

    function diamondStorage() internal pure returns (PortTokenFactoryStorage storage ds) {
    ...
    function _createPortToken(
        address manager,
        string calldata _name,
        string calldata _symbol
    ) internal returns (address) {
        PortToken token = new PortToken(address(this), manager, _name, _symbol);

        address addr = address(token);

        PortTokenFactoryStorage storage ds = diamondStorage();
        ds.portTokens.push(addr);

        emit CreatedPortToken(addr, manager, _name, _symbol);
        return addr;
    }

    function createPortToken(string calldata _name, string calldata _symbol) external returns (address) {
        return _createPortToken(msg.sender, _name, _symbol);
    }

    function _createAndInitPortToken(
        address _manager,
        string calldata _name,
        string calldata _symbol,
        ...
        address portTokenAddr = _createPortToken(_manager, _name, _symbol);

        LibFee.setStreamingFee(portTokenAddr, _streamingFee);
        LibFee.setSuccessFee(portTokenAddr, _successFee);

        for (uint256 i = 0; i < _trackedTokens.length; i++) {
            LibTrackedToken.addTrackedToken(portTokenAddr, _trackedTokens[i], _amounts[i]);
        }

        require(LibPriceInterface.checkUtilityTokenShare(_trackedTokens, _amounts), "The share of utility token is too small");

        return portTokenAddr;
    }
    ...
    function createAndInitPortToken(
        ...
        return _createAndInitPortToken(msg.sender, _name, _symbol, _streamingFee, _successFee, _trackedTokens, _amounts);
    }
    ...
    function createAndInitPortTokenFromRatios(
        ...
        return _createAndInitPortToken(msg.sender, _name, _symbol, _streamingFee, _successFee, _trackedTokens, amounts);
    }

    function portTokensLength() external view returns (uint256) {
        PortTokenFactoryStorage storage ds = diamondStorage();
        return ds.portTokens.length;
    }
...
library LibFee {
    bytes32 constant DIAMOND_STORAGE_POSITION = keccak256("portfolio.streamingfee");

    uint16 public constant MIN_SUCCESS_FEE = 0;
    uint16 public constant MAX_SUCCESS_FEE = 300; //30%
    uint16 public constant MIN_STREAMING_FEE = 10; //1%
    uint16 public constant MAX_STREAMING_FEE = 500; //50%
    ...
    function diamondStorage() internal pure returns (FeeStorage storage ds) {
        bytes32 position = DIAMOND_STORAGE_POSITION;
        assembly {
            ds.slot := position
        }
    ...
    function streamingFeeDataForToken(address _addr) internal view returns (StreamingFeeData storage) {
        FeeStorage storage ds = diamondStorage();
    ...
    function setStreamingFee(address _addr, uint16 _value) internal {
        require(_value <= MAX_STREAMING_FEE && _value >= MIN_STREAMING_FEE, "Invalid fee");
    ...
    function setSuccessFee(address _addr, uint16 _value) internal {
        require(_value <= MAX_SUCCESS_FEE && _value >= MIN_SUCCESS_FEE, "Invalid fee");
...
library LibPriceInterface {
    ...
    function getTokenPrice(address _addr, uint256 _amount) internal view returns (uint256 price) {
        ...
        if (os.feedType == 1) {
            ...
            price = (IOffchainOracle(os.feed).getRate(IERC20(_addr), IERC20(ds.baseToken), true) * _amount) / (10**ERC20(_addr).decimals());
        }
    }
    ...
    function getTokenForPrice(address _addr, uint256 _amount) internal view returns (uint256 price) {
        ...
        if (os.feedType == 1) {
            ...
            price = (IOffchainOracle(os.feed).getRate(IERC20(ds.baseToken), IERC20(_addr), true) * _amount) / (10**ERC20(ds.baseToken).decimals());
        }
    }
