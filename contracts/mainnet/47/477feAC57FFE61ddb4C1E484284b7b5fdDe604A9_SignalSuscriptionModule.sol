...
contract SignalSuscriptionModule is ModuleBase, Ownable, ReentrancyGuard {
    ...
    event SetPlatformAndPlatformFee(ISubscribeFeePool _subscribeFeePool,uint256 _fee,address _platform_vault,uint256 _warningLine,uint256 _unsubscribeLine,address _mirrorToken);
    ...
    function setPlatformAndPlatformFee(
        ...
        unsubscribeLine = _unsubscribeLine;
         mirrorToken=_mirrorToken;
        emit SetPlatformAndPlatformFee(_subscribeFeePool,_fee,_platform_vault,_warningLine,_unsubscribeLine,_mirrorToken);
    }
    ...
    function handleFee(
        ...
        if (nextBalance > preBalance) {  
            ...
            uint256 platformFeeBalance = strategistFeeBalance.preciseMul(platformFee);    
    ...
    function handleResetFee( IJasperVault _target,address _token,uint256 _amount) external nonReentrant onlyManagerAndValidSet(_target){
           if(_amount>0){
             IERC20(_token).approve(address(subscribeFeePool), _amount);
             subscribeFeePool.desposit(_token, address(_target), _amount);
           }
    }
    function desposit(
        IJasperVault _jasperVault,
