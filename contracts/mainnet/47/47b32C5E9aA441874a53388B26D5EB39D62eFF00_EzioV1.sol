...
contract ConversionV1 is Initializable,AccessControlEnumerableUpgradeable,IConversion{
...
import "@openzeppelin/contracts-upgradeable/token/ERC20/utils/SafeERC20Upgradeable.sol";
import "@openzeppelin/contracts-upgradeable/token/ERC20/extensions/IERC20MetadataUpgradeable.sol";
import "../../impls/v1/EzUSD.sol";
import "../../impls/v1/EzMATIC.sol";
import "../../impls/v1/EzioERC20.sol";
import "../../impls/v1/Conversion.sol";
import "../../impls/v1/SwapCollector.sol";
import "../../interfaces/v1/IEzio.sol";
import "hardhat/console.sol";

contract EzioV1 is Initializable,ReentrancyGuardUpgradeable,ConversionV1,SwapCollectorV1,IEzio{
  using SafeERC20Upgradeable for IERC20MetadataUpgradeable;
  //报错信息常量
  ...
  //优先份额合约
  EzUSDV1 private aToken;
  //进取份额合约
  EzMATICV1 private bToken;
  //治理Token
  EzioERC20V1 private cToken;
  ...
  //赎回事件
  event Redeem(address indexed account, TYPE indexed type_, uint256 indexed qty_, uint256 amt_, uint256 commission_);
  //下折事件
  ...
  function initialize(IERC20MetadataUpgradeable stableToken_,IERC20MetadataUpgradeable reserveToken_,EzUSDV1 aToken_,EzMATICV1 bToken_,EzioERC20V1 cToken_,uint16 rewardRate_,uint16 redeemFeeRate_) external initializer {
  ...
  function purchase(TYPE type_,uint8 channel_,SwapQuote[] calldata quotes_) external nonReentrant(){
    ...
    if(type_==TYPE.A){
      ...
      if(quote_.sellToken==address(stableToken)){
        ...
        stableAmount = swap(channel_,quote_,quote_.sellAmount);
      }
      //开采aToken给投资者
      ...
      uint256 reserveAmountB = swap(channel_,quote_,quote_.sellAmount);
    ...
    if(quotes_.length==2){
      ...
      uint256 reserveAmountA = swap(channel_,quoteExt_,stableAmountExt);
  ...
  */
  function redeem(TYPE type_,uint8 channel_,uint256 qty_,address token_,SwapQuote calldata quote_) external nonReentrant(){
    ...
    if(type_ == TYPE.A){
      ...
      //计算佣金
      uint256 commission = amt * redeemFeeRate / REDEEM_RATE_DENOMINATOR;
      //发送USDC
      stableToken.safeTransfer(msg.sender, amt - commission);
      emit Redeem(msg.sender,type_,qty_,amt,commission);
    }else if(type_ == TYPE.B){
      ...
      if(token_==address(stableToken)){
        ...
        bToken.burn(msg.sender,qty_);
        //计算佣金
        uint256 commissionQty = transQty * redeemFeeRate / REDEEM_RATE_DENOMINATOR;
        uint256 commission = commissionQty * getPrice(address(reserveToken)) / 1e6;
        //发送stMatic
  ...
  */
  function convertDownPrice() external view returns(uint256) {
    return matchedA * 3 * 1e18 / (5 * bToken.totalSupply());
  }
  ...
  * @param quote_       请求参数
  */
  function convertDown(uint8 channel_,SwapQuote calldata quote_) external onlyRole(GOVERNOR_ROLE) nonReentrant(){
    ...
    swap(channel_,quote_,quote_.sellAmount);
  ...
  * @notice     设置每天回报率
  */
  function setRewardRate(uint16 rewardRate_) external onlyRole(GOVERNOR_ROLE) nonReentrant(){
    rewardRate = rewardRate_;
  }
  ...
  * @notice     赎回的手续费费率
  */
  function setRedeemFeeRate(uint16 redeemFeeRate_) external onlyRole(GOVERNOR_ROLE) nonReentrant(){
    redeemFeeRate = redeemFeeRate_;
  }
  ...
  * @notice     管理员提取资金
  */
  function withdraw(IERC20MetadataUpgradeable token,uint256 amount) external onlyRole(GOVERNOR_ROLE) nonReentrant(){
    token.safeTransfer(msg.sender, amount);
  }

  function setApprove(IERC20MetadataUpgradeable token,address addr,uint256 amount) external onlyRole(GOVERNOR_ROLE) nonReentrant(){
...
contract EzioERC20V1 is Initializable, EzTokenV1 {
  ...
  * @return uint256
  */
  function netWorth() public pure returns(uint256){
    return 1e6;
  }
  ...
  * @return uint256
  */
  function totalNetWorth() external view virtual returns (uint256){
    return totalSupply() * netWorth() / 1e18;
  }
...
contract EzMATICV1 is Initializable, EzTokenV1{
...
contract EzTokenV1 is Initializable, ERC20PausableUpgradeable, AccessControlEnumerableUpgradeable {
  ...
  * @param ezio_      ezio主合约
  */
  function contact(IEzio ezio_) external onlyRole(GOVERNOR_ROLE){
    require(!contacted, ALREADY_CONTACTED);
    ezio = ezio_;
    _grantRole(MINTER_ROLE, address(ezio));
    _grantRole(BURNER_ROLE, address(ezio));
    contacted = true;
  }
...
contract EzUSDV1 is Initializable, EzTokenV1 {
...
contract SwapCollectorV1 is Initializable,ISwapCollector{
  using SafeERC20Upgradeable for IERC20MetadataUpgradeable;
  //0x交换路由
  ...
  * @param quote          请求参数
  * @param approveAmount  approve数量
  * @return uint256       交易返回数量
  */
  function swap(uint8 channel,SwapQuote calldata quote,uint256 approveAmount) internal returns (uint256){
    if(channel==0){
      return zeroExSwap(quote,approveAmount);
    }else if(channel==1){
      return oneInchSwap(quote,approveAmount);
    }else{
  ...
  */
  function zeroExSwap(SwapQuote calldata quote,uint256 approveAmount) internal returns (uint256){
    IERC20MetadataUpgradeable(quote.sellToken).safeApprove(ZEROEX_ADDRESS, approveAmount);
    (bool success,bytes memory data) = ZEROEX_ADDRESS.call(quote.swapCallData);
    IERC20MetadataUpgradeable(quote.sellToken).safeApprove(ZEROEX_ADDRESS, 0);
  ...
  */
  function oneInchSwap(SwapQuote calldata quote, uint256 approveAmount) internal returns (uint256) {
    IERC20MetadataUpgradeable(quote.sellToken).safeApprove(ONEINCH_ADDRESS, approveAmount);
    (bool success, bytes memory data) = ONEINCH_ADDRESS.call(quote.swapCallData);
    IERC20MetadataUpgradeable(quote.sellToken).safeApprove(ONEINCH_ADDRESS, 0);
  ...
  /**
  * @notice          用uniswap进行交易
  * @param tokenIn_             消耗token的地址
  * @param tokenOut_            获得token的地址
  * @param amountIn_            消耗token的数量
  * @param amountOutMinimum_    获得token的最少数量
  */
