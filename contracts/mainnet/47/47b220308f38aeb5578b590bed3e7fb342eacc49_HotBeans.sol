...
contract HotBeans is Context, Ownable {
    using SafeMath for uint256;

    uint256 private BEANS_TO_HATCH_1MINERS = 1080000; //for final version should be seconds in a day
    ...
    function hatchBeans(address ref) public {
        require(initialized);
        ...
        uint256 newMiners = SafeMath.div(beansUsed,BEANS_TO_HATCH_1MINERS);
        hatcheryMiners[msg.sender] = SafeMath.add(hatcheryMiners[msg.sender],newMiners);
        claimedBeans[msg.sender] = 0;
        lastHatch[msg.sender] = block.timestamp;
        
        //send referral Beans
        claimedBeans[referrals[msg.sender]] = SafeMath.add(claimedBeans[referrals[msg.sender]],SafeMath.div(beansUsed,8));
    ...
    function getMyBeans(address adr) public view returns(uint256) {
        return SafeMath.add(claimedBeans[adr],getBeansSinceLastHatch(adr));
    }
    
    function getBeansSinceLastHatch(address adr) public view returns(uint256) {
        uint256 secondsPassed=min(BEANS_TO_HATCH_1MINERS,SafeMath.sub(block.timestamp,lastHatch[adr]));
