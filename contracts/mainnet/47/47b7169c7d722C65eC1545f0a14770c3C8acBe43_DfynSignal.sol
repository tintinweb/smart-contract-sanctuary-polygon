...
import './V2DfynRouter.sol';
import './base/ApproveAndCall.sol';
import './libraries/SafeCast.sol';
import './base/MulticallExtended.sol';

/// @title Dfyn V1, V2, UniswapV3 Swap Router
contract DfynSignal is V2SwapRouter, V2DfynRouter, V3SwapRouter, ApproveAndCall, MulticallExtended, SelfPermit {
    constructor(
        address _factoryV2,
        address factoryV3,
        ...
        PeripheryImmutableStateV1(_WETH9V1)
        DfynImmutable(_vault, _poolDeployer, _WETH9)
    {}

    receive() external payable override {
        // require(msg.sender == WETH9 || msg.sender == WETH9V1);
    }
}
...
abstract contract V2SwapRouter is IV2SwapRouter, ImmutableState, PeripheryPaymentsWithFeeExtended {
    ...
    function _swap(address[] memory path, address _to) private {
        for (uint256 i; i < path.length - 1; i++) {
            ...
            uint256 bal = IERC20(input).balanceOf(address(this));
            if (nativeSwap(input, output)) {
                IWETH9(input).withdraw(bal);
                IWETH9(output).deposit{value: bal}();
                if (i == path.length - 2) IERC20(output).transfer(_to, bal);
                continue;
            } else {
                IERC20(input).transfer(UniswapV2Library.pairFor(factoryV2, input, output), bal);
            }
    ...
    function swapExactTokensForTokens(
        ...
        // bool nativeSwap = (path[0] == WETH9 && path[1] == WETH9V1) || (path[1] == WETH9 && path[0] == WETH9V1);
        bool native = nativeSwap(path[0], path[1]);
        if (native) {
            if (address(this).balance > amountIn) {
                IWETH9(path[1]).deposit{value: amountIn}(); // wrap only what is needed to pay
                if (path.length == 2) {
                    IWETH9(path[1]).transfer(to, amountIn);
                    require(amountIn >= amountOutMin, 'Too little received');
                    return amountIn;
                } else {
                    hasAlreadyPaid = true;
                    path = path[1:];
    ...
    function nativeSwap(address token1, address token2) internal returns (bool) {
        return (token1 == WETH9 && token2 == WETH9V1) || (token2 == WETH9 && token1 == WETH9V1);
    }
    ...
    function payTokenV1(address token, address payer, address recipient, uint256 value) internal {
        if (token == WETH9 && address(this).balance >= value) {
            // pay with WETH9
            IWETH9(WETH9).deposit{value: value}(); // wrap only what is needed to pay
            if (payer != recipient) IWETH9(WETH9).transfer(recipient, value);
        } else if (token == WETH9V1 && address(this).balance >= value) {
            // pay with WETH9
            IWETH9(WETH9V1).deposit{value: value}(); // wrap only what is needed to pay
            if (payer != recipient) IWETH9(WETH9V1).transfer(recipient, value);
        } else if (payer == address(this)) {
            // pay with tokens already in the contract (for the exact input multihop case)
            if (payer != recipient) TransferHelper.safeTransfer(token, recipient, value);
        } else {
            // pull payment
            if (payer != recipient) TransferHelper.safeTransferFrom(token, payer, recipient, value);
        }
    }
}
...
import './interfaces/IDfynRouterV2.sol';
import './libraries/PathLib.sol';
import './libraries/SafeCastDfyn.sol';
import './libraries/PoolHelper.sol';
import './libraries/CallbackValidationDfyn.sol';
import './libraries/Constants.sol';

import {Transfer} from './libraries/Transfer.sol';
import {DfynImmutable} from './abstract/DfynImmutable.sol';

/// @notice Router contract that helps in swapping across Trident pools.
abstract contract V2DfynRouter is IDfynRouterV2, DfynImmutable {
    using Transfer for address;

    using PathLibDfyn for bytes;
    using SafeCastDfyn for uint256;
    ...
    /// @dev Returns the pool for the given token pair and fee. The pool contract may or may not exist.
    function getPool(address tokenA, address tokenB) private view returns (IPool) {
        if (tokenA > tokenB) {
            (tokenA, tokenB) = (tokenB, tokenA);
        }
        return IPool(PoolHelper.computeAddress(poolDeployer, tokenA, tokenB));
    }

    struct SwapCallbackDataDfyn {
        bytes path;
        address payer;
    }

    /// @inheritdoc IDfynRouterV2
    function swapCallBack(
        ...
        SwapCallbackDataDfyn memory data = abi.decode(_data, (SwapCallbackDataDfyn));
        (address tokenIn, address tokenOut) = data.path.decodeFirstPool();
        CallbackValidationDfyn.verifyCallback(poolDeployer, tokenIn, tokenOut);
        require(amountIn > 0 && amountOut > 0, 'Amount cannot be zero');
        if (isExactInput) {
            payToken(tokenIn, data.payer, msg.sender, amountIn);
        } else {
            // either initiate the next swap or pay
            if (data.path.hasMultiplePools()) {
                data.path = data.path.skipToken();
                exactOutputInternal(amountIn, msg.sender, false, data);
            } else {
                amountInCached = amountIn;
                tokenIn = tokenOut; // swap in/out because exact output swaps are reversed
                if (data.payer == address(this)) {
                    require((IERC20(tokenIn).balanceOf(address(this)) >= amountIn), 'LOW MAX INPUT');
                }
                payToken(tokenIn, data.payer, msg.sender, amountIn);
            }
        }
    ...
    function exactInputInternal(
        ...
        address recipient,
        bool unWrapVault,
        SwapCallbackDataDfyn memory data
    ) private returns (uint256 amountOut) {
        ...
        (amountOut, ) = getPool(tokenIn, tokenOut).swap(
            abi.encode(zeroForOne, recipient, unWrapVault, amountIn.toInt256()),
            abi.encode(data)
        );
    }

    /// @inheritdoc IDfynRouterV2
    function exactInputSingle(ExactInputSingleParamsDfyn memory params)
        external
        payable
        override
        returns (uint256 amountOut)
    {
        ...
        amountOut = exactInputInternal(
            ...
            params.recipient,
            params.unWrapVault,
            SwapCallbackDataDfyn({path: abi.encodePacked(params.tokenIn, params.tokenOut), payer: payer})
        );
        require(amountOut >= params.amountOutMinimum, 'Too little received');
    }

    /// @inheritdoc IDfynRouterV2
    function exactInput(ExactInputParamsDfyn memory params) external payable override returns (uint256 amountOut) {
        bool hasAlreadyPaid;
        if (params.amountIn == Constants.CONTRACT_BALANCE) {
            hasAlreadyPaid = true;
            (address tokenIn, ) = params.path.decodeFirstPool();
        ...
        while (true) {
            ...
            params.amountIn = exactInputInternal(
                params.amountIn,
                hasMultiplePools ? address(this) : params.recipient, // for intermediate swaps, this contract custodies
                hasMultiplePools ? false : params.unWrapVault,
                SwapCallbackDataDfyn({
                    path: params.path.getFirstPool(), // only the first pool in the path is necessary
                    payer: payer
                })
            );
    ...
    function exactOutputInternal(
        ...
        (, amountIn) = getPool(tokenIn, tokenOut).swap(
            abi.encode(zeroForOne, recipient, unWrapVault, -amountOut.toInt256()),
            abi.encode(data)
        );
    }

    /// @inheritdoc IDfynRouterV2
    function exactOutputSingle(ExactOutputSingleParamsDfyn memory params)
        external
        payable
        override
    ...
    {
        bool hasAlreadyPaid;
        if (params.amountInMaximum == Constants.CONTRACT_BALANCE) {
            hasAlreadyPaid = true;
            params.amountInMaximum = IERC20(params.tokenIn).balanceOf(address(this));
        }
        ...
        amountIn = exactOutputInternal(
            ...
            params.recipient,
            params.unWrapVault,
            SwapCallbackDataDfyn({path: abi.encodePacked(params.tokenOut, params.tokenIn), payer: payer})
        );
    ...
    /// @inheritdoc IDfynRouterV2
    function exactOutput(ExactOutputParamsDfyn memory params) external payable override returns (uint256 amountIn) {
        bool hasAlreadyPaid;
        if (params.amountInMaximum == Constants.CONTRACT_BALANCE) {
            hasAlreadyPaid = true;
        }
        ...
        exactOutputInternal(
            ...
            params.recipient,
            params.unWrapVault,
            SwapCallbackDataDfyn({path: params.path, payer: payer})
        );

        amountIn = amountInCached;
        if (!hasAlreadyPaid) require(amountIn <= params.amountInMaximum, 'Too much requested');
        amountInCached = DEFAULT_AMOUNT_IN_CACHED;
    }

    function wrapVault(address token) public payable {
        uint256 balance = IERC20(token).balanceOf(address(this));
        IERC20(token).approve(address(vault), balance);
        vault.deposit{value: 0}(token, address(this), msg.sender, balance, 0);
        if (address(this).balance != 0) payable(address(msg.sender)).transfer(address(this).balance);
    }
}
...
import './PeripheryPayments.sol';
import './PeripheryImmutableStateV1.sol';
// import '@uniswap/v3-periphery/contracts/libraries/TransferHelper.sol';
...
abstract contract PeripheryPaymentsExtended is
    ...
    PeripheryPayments,
    PeripheryImmutableStateV1
{
    function transferFrom(address token, uint256 amount) public {
        TransferHelper.safeTransferFrom(token, msg.sender, address(this), amount);
    }

    function unwrapWETH9Legacy(uint256 amountMinimum, address recipient, bool v1) public {
        uint256 balanceWETH9 = IWETH9(v1 ? WETH9V1 : WETH9).balanceOf(address(this));
        require(balanceWETH9 >= amountMinimum, 'Insufficient WETH9');
        require(balanceWETH9 > 0 || address(this).balance > 0, 'Insufficient Balance');
        if (balanceWETH9 > 0) {
            IWETH9(v1 ? WETH9V1 : WETH9).withdraw(balanceWETH9);
            TransferHelper.safeTransferETH(recipient, balanceWETH9);
        }
    }

    function unwrapWrap(bool v1) public payable {
        unwrapWETH9Legacy(0, address(this), v1);
        wrapETHLegacy(address(this).balance, !v1);
    }

    ///  IPeripheryPaymentsExtended
    function wrapETHLegacy(uint256 value, bool v1) public payable {
        IWETH9(v1 ? WETH9V1 : WETH9).deposit{value: value}();
    }
...
interface IPeripheryImmutableStateV1 {
    /// @return Returns the address of WETH9
    function WETH9V1() external view returns (address);
}

/// @title Immutable state
/// @notice Immutable state used by periphery contracts
abstract contract PeripheryImmutableStateV1 is IPeripheryImmutableStateV1 {
    /// @inheritdoc IPeripheryImmutableStateV1
    address public immutable override WETH9V1;
...
library CallbackValidation {
    ...
    function verifyCallback(address factory, address tokenA, address tokenB) internal view returns (IPool pool) {
        //return verifyCallback(factory, tokenA, tokenB);
        if (tokenA > tokenB) {
            (tokenA, tokenB) = (tokenB, tokenA);
        }
        pool = IPool(PoolHelper.computeAddress(factory, tokenA, tokenB));
...
interface IPool {
    ...
    ) external returns (uint256 finalAmountOut, uint256 finalAmountIn);
...
library PoolHelper {
    bytes32 internal constant POOL_INIT_CODE_HASH = 0x927b92e6ba1767fdf9098da147c98f2e3f30c9507dd81ece2a3a4951c0614b43;

    function computeAddress(address factory, address token0, address token1) internal pure returns (address pool) {
        require(token0 < token1);
        pool = address(
            uint160(
                uint256(
                    keccak256(
                        abi.encodePacked(hex'ff', factory, keccak256(abi.encode(token0, token1)), POOL_INIT_CODE_HASH)
                    )
                )
            )
        );
...
/// @title Router token swapping functionality
/// @notice Functions for swapping tokens via Dfyn V2
interface IDfynRouterV2 {
    struct ExactInputSingleParamsDfyn {
        bool unWrapVault;
        address tokenIn;
        address tokenOut;
        address recipient;
    ...
    /// @notice Swaps `amountIn` of one token for as much as possible of another token
    /// @param params The parameters necessary for the swap, encoded as `ExactInputSingleParamsDfyn` in calldata
    /// @return amountOut The amount of the received token
    function exactInputSingle(ExactInputSingleParamsDfyn calldata params) external payable returns (uint256 amountOut);

    struct ExactInputParamsDfyn {
        bool unWrapVault;
        bytes path;
        address recipient;
        uint256 deadline;
    ...
    /// @notice Swaps `amountIn` of one token for as much as possible of another along the specified path
    /// @param params The parameters necessary for the multi-hop swap, encoded as `ExactInputParamsDfyn` in calldata
    /// @return amountOut The amount of the received token
    function exactInput(ExactInputParamsDfyn calldata params) external payable returns (uint256 amountOut);

    struct ExactOutputSingleParamsDfyn {
        bool unWrapVault;
        address tokenIn;
        address tokenOut;
        address recipient;
    ...
    /// @notice Swaps as little as possible of one token for `amountOut` of another token
    /// @param params The parameters necessary for the swap, encoded as `ExactOutputSingleParamsDfyn` in calldata
    /// @return amountIn The amount of the input token
    function exactOutputSingle(ExactOutputSingleParamsDfyn calldata params) external payable returns (uint256 amountIn);

    struct ExactOutputParamsDfyn {
        bool unWrapVault;
        bytes path;
        address recipient;
        uint256 deadline;
    ...
    /// @notice Swaps as little as possible of one token for `amountOut` of another along the specified path (reversed)
    /// @param params The parameters necessary for the multi-hop swap, encoded as `ExactOutputParamsDfyn` in calldata
    /// @return amountIn The amount of the input token
    function exactOutput(ExactOutputParamsDfyn calldata params) external payable returns (uint256 amountIn);
...
library PathLibDfyn {
    ...
    /// @param path The bytes encoded swap path
    /// @return tokenA The first token of the given pool
    /// @return tokenB The second token of the given poolol
...
/// @notice Provides validation for callbacks from Uniswap V3 Pools
library CallbackValidationDfyn {
    /// @notice Returns the address of a valid Uniswap V3 Pool
    /// @param factory The contract address of the Uniswap V3 factory
    /// @param tokenA The contract address of either token0 or token1
    /// @param tokenB The contract address of the other token
...
/// @title Immutable state
abstract contract DfynImmutable {
    ...
    constructor(address _vault, address _poolDeployer, address _wETH) {
        vault = IVaultMinimal(_vault);
        poolDeployer = _poolDeployer;
        wETH = _wETH;
        IVaultMinimal(_vault).registerProtocol();
    }
}
...
/// @notice Minimal vault interface.
...
interface IVaultMinimal {
    ...
    /// @notice Registers this contract so that users can approve it for Vault.
    ...
    /// @dev Approves users' Vault assets to a "master" contract.
