...
// File src/interfaces/trident-router.sol
...
// File src/strategies/polygon/strategy-stargate-farm-base.sol
pragma solidity ^0.8.6;
...
abstract contract StrategyStargateFarmBase is StrategyBase {
    ...
    // Token addresses
    address public constant stg = 0x2F6F07CDcf3588944Bf4C42aC74ff24bF56e7590;
    ...
    address public constant tridentRouter =
        0xc5017BE80b4446988e8686168396289a9A62668E;
    address public constant pool2 = 0xcA5953773602e8C789f0635F40e05e816165B85c;
    address public constant bento = 0x0319000133d3AdA02600f0875d2cf03D442C3367;

    address public am3CrvPool = 0x445FE580eF8d70FF569aB36e80c647af338db351;
    ...
    mapping(address => int128) public am3CrvTokensOrder;
    ...
    constructor(
        ...
        IERC20(stg).approve(bento, type(uint256).max);
        IERC20(want).approve(starchef, type(uint256).max);
        IBentoBox(bento).setMasterContractApproval(
            address(this),
            tridentRouter,
            true,
            0,
    ...
    function harvest() public override {
        ...
        bytes memory data = abi.encode(stg, address(this), true);
        ITridentRouter.Path[] memory path = new ITridentRouter.Path[](1);
        path[0] = ITridentRouter.Path({pool: pool2, data: data});
        ITridentRouter(tridentRouter).exactInputSingleWithNativeToken(
        ...
        // If diff underlying, swap USDC -> underlying
        if (underlying != usdc) {
            uint256 _base = IERC20(usdc).balanceOf(address(this));
            ICurvePool_0_2_4(am3CrvPool).exchange_underlying(
                1,
                am3CrvTokensOrder[underlying],
                _base,
                0
            );
        }
...
// File src/strategies/polygon/stargate/strategy-stargate-usdc.sol
// License-Identifier: MIT
pragma solidity ^0.8.6;

contract StrategyPolyStargateUsdc is StrategyStargateFarmBase {
  // Token addresses
  address public constant _lp = 0x1205f31718499dBf1fCa446663B532Ef87481fe1;
  address public constant _underlying = usdc;
  address public _baseToken = usdc;
  ...
  {
    IERC20(usdc).approve(starRouter, type(uint256).max);
  }

  // **** Views ****

  function getName() external pure override returns (string memory) {
    return "StrategyPolyStargateUsdc";
  }
}