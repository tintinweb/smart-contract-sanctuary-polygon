...
contract BattleContract is Ownable, ReentrancyGuard {  
    ...
    */
    ...
    function sendUpdate(uint256 _id, uint256 _health, uint256 _damage, uint256 _experience, uint256 _level, uint256 _skillPoints, uint256 _charges) internal {
      nftInterface  i = nftInterface(nftAddress); 
      i.updateCharecter(_id, _health, _damage, _experience, _level, _skillPoints, _charges);
    }
    ...
    */
    ...
    function _fight(uint256 self, uint256 target) 
        ...
        characterOf(self)  {
            ...
            if (won) {
                ...
                getStats(self);
                sendUpdate(self, _health, _damage, _experience + 1, _level, _skillPoints, _charges - 1);
                if (_experience >= 100){
    ...
    function _onFightLost(uint256 id) private {
        //Chareceter looses hp
        getStats(id);
        uint newHealth = _health -50;
        sendUpdate(id, newHealth, _damage, 0, _level, _skillPoints, _charges - 1);
        //Burn NFT if hp below zero
        if (newHealth <= 0){
            //Burn NFT
            //transfer(msg.sender, burnAddress, id);
            emit Death(msg.sender, id);
        }
    ...
    function setBattleToken(address _address) public onlyOwner{
        battleWagerToken = IERC20(0xeb8f08a975Ab53E34D8a0330E0D34de942C95926);
