...
contract StrategyCommonSatinWrapperStakerLP is StrategyCommonSolidlyStakerLP {
  ...
  function addLiquidity() internal override {
    ...
    if (lp0Amt > peg) {
      ...
      uint256 lp0Decimals = 10 ** IERC20Extended(requiredToken).decimals();
      uint256 lp1Decimals = 10 ** IERC20Extended(gaugeStaker).decimals();
      uint256 out0 = (ISolidlyRouter(dystRouter).getAmountsOut(lp0Amt, outputToSpSatinRoute)[
        outputToSpSatinRoute.length
      ] * 1e18) / lp0Decimals;
      uint256 out1 = lp1Amt;
...
contract StrategyCommonSolidlyStakerLP is StratManagerUpgradeable, DynamicFeeManager {
  ...
  function chargeFees(address callFeeRecipient) internal {
    ...
    if (strategistFeeAmount > 0) {
     ...
     console.log("veSwapper", veSwapper);
