...
// File: contracts/interfaces/gravity/IWETH.sol
...
// File: contracts/interfaces/gravity/IGFIRouterV2.sol


pragma solidity ^0.8.19;

interface IGFIRouterV2 {
...
// File: contracts/interfaces/gravity/IGovernanceV2.sol
...
interface IGovernanceV2 {
    ...
    function feeBalance(address) external view returns (uint256);

    function feeLedger(address) external view returns (FeeLedger memory);
    ...
    function claimBTC(uint256) external;

    function updateFee(address _address) external returns (uint256);

    function depositFee(uint256, uint256) external;
}

// File: contracts/interfaces/common/IGFITierChecker.sol
...
// File: contracts/interfaces/gmx/IGMXStrategy.sol
...
// File: contracts/interfaces/gmx/IGravityVault.sol



pragma solidity ^0.8.19;

interface IGravityVault {
...
// File: contracts/interfaces/gmx/IGMXTracker.sol
...
// File: contracts/interfaces/mvx/IMVXRouter.sol
...
// File: contracts/common/StratFeeManagerInitializable.sol
    ...
    PausableUpgradeable
{
    ...
    // managerFee => managerFeeRecipient, partnerFee => partnerFeeRecipient,
    // The rest goes into Vault (compound)
    struct HarvestFees {
        uint256 managerFee;
        uint256 partnerFee;
    }
    ...
    modifier onlyManager() {
        require(msg.sender == owner() || msg.sender == operator, "!manager");
        _;
    }

    function getHarvestFees() external view returns (HarvestFees memory) {
        return harvestFees;
    }

    function getWithdrawalFees() external view returns (WithdrawalFees memory) {
        return withdrawalFees;
    }

    function getFeeCollectors() external view returns (FeeCollectors memory) {
    ...
    function getAllWithdrawalFeePerTier(
        ...
        uint256[] memory feesPerTier = new uint256[](tierCount);

        for (uint i = 0; i < tierCount; i++) {
            feesPerTier[i] = withdrawalFeePerTier[i];
        }
    ...
    function setFeeCollectors(
        ...
        feeCollectors = FeeCollectors(managerFeeRecipient, partnerFeeRecipient);

        emit SetFeeCollectors(managerFeeRecipient, partnerFeeRecipient);
    }
    ...
    function setHarvestFees(
        ...
        harvestFees = HarvestFees(managerFee, partnerFee);
    ...
    function setWithdrawalFees(
        ...
        withdrawalFees = WithdrawalFees(managerFee, partnerFee);
    ...
    function setTierChecker(address _tierChecker) external onlyManager {
        tierChecker = _tierChecker;
    }
...
// File: contracts/StrategyGFI.sol
...
contract StrategyGFI is StratFeeManagerInitializable, UUPSUpgradeable {
    ...
    address public gfiGov;
    address public gfiRouter;
    ...
    function initialize(
        ...
        __StratFeeManager_init(_tierChecker, _vault);
    ...
    function withdraw(uint256 _amount, uint256 _feeAmount) external {
        require(msg.sender == vault, "!vault");

        uint256 b = balanceOfWant();

        if (_amount > 0) {
            if (b > _amount) {
                b = _amount;
            }
            IERC20(want).safeTransfer(vault, b);
            b = balanceOfWant();
        }
        ...
        chargeWithdrawalFees(b);

        emit Withdraw(balanceOf());
    }

    //send fee (MVLP) to fee collectors
    function chargeWithdrawalFees(uint256 _feeAmount) internal {
        if (_feeAmount > 0) {
            uint256 managerFee = (_feeAmount * withdrawalFees.managerFee) /
                FULL_PERCENT;
            uint256 partnerFee = (_feeAmount * withdrawalFees.partnerFee) /
                FULL_PERCENT;
    ...
    function getWithdrawalFeeAmount(
        uint256 _withdrawAmount,
        address _caller
    ) external view returns (uint256) {
        uint256 _withdrawalFee = getWithdrawalFee(_caller);
        return ((_withdrawAmount * _withdrawalFee) / FULL_PERCENT);
    }
    ...
    function _harvest() internal whenNotPaused {
        IGovernanceV2(gfiGov).claimFee();

        uint256 wethBal = IERC20(native).balanceOf(address(this));
        if (wethBal > 0) {
            IWETH(native).withdraw(wethBal);
        }
    ...
    // performance fees
    function chargeHarvestFees() internal {
        uint256 feeBal = address(this).balance;

        uint256 managerFee = (feeBal * harvestFees.managerFee) / FULL_PERCENT;
        uint256 partnerFee = (feeBal * harvestFees.partnerFee) / FULL_PERCENT;
        ...
        emit ChargedHarvestFees(managerFee, partnerFee);
    }

    // buy more GFI with the MATIC earned as fees
    ...
    function rewardsAvailable() public view returns (uint256) {
        return IGovernanceV2(gfiGov).pendingEarnings(address(this));
    }
    ...
    function setWant(address _want) external onlyManager {
        want = _want;
    }

    function setNative(address _native) external onlyManager {
        native = _native;
    }

    function setGFIGov(address _gfiGov) external onlyManager {
        gfiGov = _gfiGov;
    }

    function setGFIRouterV2(address _router) external onlyManager {
        gfiRouter = _router;
    }
