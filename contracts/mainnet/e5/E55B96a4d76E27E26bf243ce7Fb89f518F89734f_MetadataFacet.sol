...
contract MetadataFacet {
    ...
    function getLandMetaData(uint256 _tokenId) external view returns (
        ...
        tokenURI = ds.erc721_tokenURIs[_tokenId];
        uint256 dna = ds.land_dna[_tokenId];
    ...
    function getAllLandIdsForOwner(address _owner) external view returns (uint256[] memory) {
        ...
        for(uint256 i = 0; i < balance; ++i) {
            ids[i] = ds.erc721_ownedTokens[_owner][i];
        }
        return ids;
    }
    ...
    function getLandsByOwner(address _owner, uint32 _pageNumber) external view returns (
        ...
        for(uint i = 0; i < count; ++i) {
            ...
            tokenIds[i] = tokenId;
            landTypes[i] = ds.landTypeByTokenId[tokenId];
            names[i] = LibLandNames._getFullName(tokenId);
            gameLocked[i] = LibLandDNA._getGameLocked(ds.land_dna[tokenId]);
        }
    }
}
