...
contract Ticket is ERC721URIStorage, Governable, ReentrancyGuard {
    ...
    constructor(address _governance, uint256 _maxTicketCount, string memory _eName) Governable(_governance) ERC721(unicode"İşSanat Metaverse Sergi Bileti", "ISSANAT") {
    ...
    */
    function createTicket(address _user, string memory _tokenURI) external nonReentrant onlyGovernance returns (uint256 ticketId) {
        return _createTicket(_user, _tokenURI);
    }

    /**
     * @notice Creates a ticket.
     * @param _users The user to mint ticket.
     * @param _tokenURIs The token uri.
    */
    function createTickets(address[] memory _users, string[] memory _tokenURIs) external nonReentrant onlyGovernance {
        require(_users.length == _tokenURIs.length, "length mismatch");
        for (uint256 i = 0; i < _users.length; i++) {
            _createTicket(_users[i], _tokenURIs[i]);
        }
    }
     ...
     * @return ticketId The ticket id.
    */
    function _createTicket(address _user, string memory _tokenURI) internal returns (uint256 ticketId) {
