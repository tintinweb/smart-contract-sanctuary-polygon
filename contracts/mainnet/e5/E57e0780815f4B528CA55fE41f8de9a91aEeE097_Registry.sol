...
//
/*
u32 or bytepack terms;
string to bytes32 for the ipfs hash;
remove mapping to store sigs AND validate on-chain;
*/
...
contract Registry is Initializable, OwnableUpgradeable, UUPSUpgradeable, EIP712Upgradeable, IERC2771Recipient {
    ...
    mapping(bytes32 => AgreementTerms) public agreementTerms;
    ...
    function addAgreementTerm(
        string memory _name,
        string memory _description
    ) external onlyOwner {
        //It's just a hash of the description
        bytes32 termHash = keccak256(abi.encodePacked(_description));
        agreementTerms[termHash] = AgreementTerms(_name, _description);
    }

    function addAgreementTerms(
        AgreementTerms[] calldata _terms
    ) external onlyOwner {
        unchecked{

            for(uint i; i < _terms.length; ++i) {
                bytes32 termHash = keccak256(abi.encodePacked(_terms[i].description));
                agreementTerms[termHash] = _terms[i];
            }
        }
    }
    ...
    /*
    function counterSignBulk(Agreements[] memory agreements, bytes calldata signature) {
            bytes32 digest = ..... eip 712 typed array
            verify notary, might need to verify promisor individually, 
            verify bulk array for offerer
    }
    */
