...
abstract contract RWAHub is IRWAHub, ReentrancyGuard, AccessControlEnumerable {
    ...
    checkRestrictions(msg.sender)
  {
    ...
    uint256 depositValueAfterFees = amount - feesInCollateral;
   ...
   * @dev We do not take fees for direct deposits
   */
  function addProof(
    bytes32 txHash,
    address user,
    ...
    depositIdToDepositor[txHash] = Depositor(user, amount, 0);
    emit DepositProofAdded(txHash, user, amount, timestamp);
  }
   ...
   * @param depositValueAfterFees The deposit value for the new entry
