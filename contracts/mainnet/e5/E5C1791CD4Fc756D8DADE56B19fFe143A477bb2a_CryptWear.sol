...
contract CryptWear is ERC721, Ownable, ReentrancyGuard {
    ...
    function setMintSettings(
        ...
        require(_maxGiveaway < 101, "max giveaway < 101");
    ...
    function mintPresale(address to, uint256 amount, bytes32[] calldata proof)
    external
    canMint(amount, MintType.PRESALE) nonReentrant
    {
    ...
    function mintWhitelist(address to, uint256 amount, bytes32[] calldata proof)
    external
    payable
    canMint(amount, MintType.WHITELIST_N_PUBLIC)
    isCorrectPayment(mintSettings.whitelistSalePrice, amount)
    nonReentrant
    {
        // Throw if address has already claimed tokens
        require(!claimedWhitelist[msg.sender], "NFT is already claimed by this wallet");

        // Verify merkle proof, or revert if not in tree
        bytes32 leaf = keccak256(abi.encodePacked(to, amount));
        bool isValidLeaf = MerkleProof.verify(proof, whitelistMerkleRoot, leaf);
        require(isValidLeaf, "not part of Merkle tree");

        claimedWhitelist[to] = true;
        for (uint256 i = 0; i < amount; i++) {
    ...
    nonReentrant
    {
        require(mintSettings.giveawayCounter++ < mintSettings.maxGiveaway, "maxGiveaway reached!");
