...
import "./tokens/interfaces/IDIAT.sol";
import "./tokens/interfaces/IsDIAT.sol";
import "./bonds/interfaces/IBondCalculator.sol";
...
contract DiatomTreasuryV2 is DiatomAccessControlled, ITreasury {
    /* ========== DEPENDENCIES ========== */
    ...
    /**
     * @notice allow approved address to deposit an asset for DIAT
    ...
    function deposit(
        ...
        // mint DIAT needed and store amount of rewards for distribution
        send_ = value.sub(_profit);
        DIAT.mint(msg.sender, send_);
    ...
    /**
     * @notice allow approved address to burn DIAT for reserves
     * @param _amount uint256
     * @param _token address
     */
    function withdraw(uint256 _amount, address _token) external override {
        ...
        uint256 value = tokenValue(_token, _amount);
        DIAT.burnFrom(msg.sender, value);
    ...
    /**
     * @notice mint new DIAT using excess reserves
    ...
    function mint(address _recipient, uint256 _amount) external override {
        ...
        DIAT.mint(_recipient, _amount);
        emit Minted(msg.sender, _recipient, _amount);
    }

    /**
     * DEBT: The debt functions allow approved addresses to borrow treasury assets
     * or DIAT from the treasury, using sDIAT as collateral. This might allow an
     * sDIAT holder to provide DIAT liquidity without taking on the opportunity cost
    ...
    function incurDebt(uint256 _amount, address _token) external override {
        uint256 value;
        if (_token == address(DIAT)) {
            require(permissions[STATUS.DIATDEBTOR][msg.sender], notApproved);
            value = _amount;
        } else {
        ...
        sDIAT.changeDebt(value, msg.sender, true);
        require(sDIAT.debtBalances(msg.sender) <= debtLimit[msg.sender], "Treasury: exceeds limit");
        totalDebt = totalDebt.add(value);

        if (_token == address(DIAT)) {
            DIAT.mint(msg.sender, value);
            diatDebt = diatDebt.add(value);
        } else {
            totalReserves = totalReserves.sub(value);
            IERC20(_token).safeTransfer(msg.sender, _amount);
        }
    ...
    function repayDebtWithReserve(uint256 _amount, address _token) external override {
        ...
        IERC20(_token).safeTransferFrom(msg.sender, address(this), _amount);
        uint256 value = tokenValue(_token, _amount);
        sDIAT.changeDebt(value, msg.sender, false);
    ...
    /**
     * @notice allow approved address to repay borrowed reserves with DIAT
     * @param _amount uint256
     */
    function repayDebtWithDIAT(uint256 _amount) external {
        require(
            permissions[STATUS.RESERVEDEBTOR][msg.sender] || permissions[STATUS.DIATDEBTOR][msg.sender],
            notApproved
        );
        DIAT.burnFrom(msg.sender, _amount);
        sDIAT.changeDebt(_amount, msg.sender, false);
        totalDebt = totalDebt.sub(_amount);
        diatDebt = diatDebt.sub(_amount);
        emit RepayDebt(msg.sender, address(DIAT), _amount, _amount);
    }
    ...
    function enable(
        ...
        if (_status == STATUS.SDIAT) {
            sDIAT = IsDIAT(_address);
        } else {
    ...
    function execute(uint256 _index) external {
        ...
        if (info.managing == STATUS.SDIAT) {
            // 9
            sDIAT = IsDIAT(info.toPermit);
        } else {
     ...
     * @return bool
     */
    function isReserveToken(address token) external view override returns (bool) {
        return permissions[STATUS.RESERVETOKEN][token];
    }
    ...
    function excessReserves() public view override returns (uint256) {
        return totalReserves.sub(DIAT.totalSupply().sub(totalDebt));
    }

    /**
     * @notice returns DIAT valuation of asset
     * @param _token address
     * @param _amount uint256
     * @return value_ uint256
     */
    function tokenValue(address _token, uint256 _amount) public view override returns (uint256 value_) {
        value_ = _amount.mul(10**IERC20Metadata(address(DIAT)).decimals()).div(10**IERC20Metadata(_token).decimals());
    ...
    function baseSupply() external view override returns (uint256) {
        return DIAT.totalSupply() - diatDebt;
    }
}
