...
contract UniswapPairOracle is Ownable {
    ...
    constructor(IUniswapV2Pair _pair) {
        ...
        require(reserve0 != 0 && reserve1 != 0, "UniswapPairOracle::constructor: NO_RESERVES");
    }
    ...
    function twap(address token, uint256 pricePrecision) external view returns (uint256 amountOut) {
        ...
        if (token == token0) {
            ...
            amountOut = amountOut.mul(_token1MissingDecimals);
        } else {
            amountOut = price1Average.mul(pricePrecision).decode144();
            amountOut = amountOut.mul(_token0MissingDecimals);
        }
    }

    /// Get token spot price by a specific precision.
    function spot(address token, uint256 pricePrecision) external view returns (uint256) {
        require(token == token0 || token == token1, "UniswapPairOracle::spot: Invalid pair");
        (uint256 _reserve0, uint256 _reserve1, ) = pair.getReserves();
        require(_reserve0 > 0 && _reserve1 > 0, "UniswapPairOracle::spot: No reserves");
        uint8 _token0MissingDecimals = 18 - (ERC20(token0).decimals());
        uint8 _token1MissingDecimals = 18 - (ERC20(token1).decimals());
        uint256 _price = 0;
        if (token == token0) {
    ...
    function update() external {
        ...
        require(timeElapsed >= period, "UniswapPairOracle::update: PERIOD_NOT_ELAPSED");
    ...
    function currentCumulativePrices(IUniswapV2Pair _pair)
        internal
        view
        returns (
    ...
    {
        /// The current block timestamp within the range of uint32, i.e. [0, 2**32 - 1]
