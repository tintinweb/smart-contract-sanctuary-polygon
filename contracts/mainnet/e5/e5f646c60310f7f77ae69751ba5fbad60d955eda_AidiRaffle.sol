...
// Creating Rafflee 
  ...
  function enterRaffle(bytes32 _id, uint256 _numEntries) external {
    ...
    if (_raffle.entryFee > 0) {
      ...
      require( approvedValue >= _raffle.entryFee * _numEntries, "Not approved!");
      _entryToken.safeTransferFrom(
        msg.sender,
        address(this),
        _raffle.entryFee.mul(_numEntries)
      );
  ...
  function _randomIndexArray(uint256 randomValue, uint256 length, uint256 range) internal pure returns (uint256[] memory randomArray) {
    ...
    for (uint256 i = 0; i < length; i++) {
        randomArray[i] = uint256((uint256(keccak256(abi.encode(randomValue, i))) % range) + 1);
    }
    return randomArray;
  }
