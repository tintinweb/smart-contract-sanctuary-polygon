...
contract JibberShapes3 is ERC721A, Pausable {
 ...
 struct PresaleConfig {
    ...
    ) ERC721A("JibberShapes3", "JBSH3", _maxBatchSize, _maxTotalSupply) 
    {
        require(
            (_maxGiftSupply <= _maxTotalSupply),
            "maxGiftSupply exceeds maxTotalSupply"
        );
        require(
            (_maxBatchSize <= _maxTotalSupply),
            "maxBatchSize exceeds maxTotalSupply"
        );
    ...
    function setUpPresale(uint256 _duration, uint256 _maxCount ) external onlyOwner {
        require( _duration > 0,
            "Presale duration must be greater than 0"
        );
        require(_maxCount <= maxBatchSize ,
            "maxCount is higher than maxBatchSize"
        );
    ...
    function promoAdminMint(uint256 _amount) external onlyOwner whenNotPaused
    {
        require(
    ...
    function promoUserMint(address[] calldata _users) external onlyOwner whenNotPaused
    {
        require(
    ...
    function presaleMint(uint256 _amount) internal {
        ...
        require(
            ...
            "Max presale count exceeded"
        );

        require(
            (price * _amount) <= msg.value,
            "Ether value sent not enough"
        );
    ...
    function saleMint(uint256 _amount) internal {
        ...
        require(_saleConfig.startTime > 0, "Sale not started yet");
        require(
        ...
        require(
            ...
            "Max count per transaction exceeded"
        );
