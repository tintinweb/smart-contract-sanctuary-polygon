...
contract VaultV2 is IVault, ERC20UpgradeSafe, ReentrancyGuardUpgradeSafe, OwnableUpgradeSafe {
  /// @notice Libraries
  ...
  function initialize(
    ...
    tinyShare = uint256(10) ** (_decimals - 1);
  }
  ...
  function _deposit(uint256 amountToken) internal {
    ...
    require(totalSupply() > tinyShare, "Vault::deposit:: no tiny shares");
  }
  ...
  function withdraw(uint256 share) external override accrue(0) nonReentrant {
    ...
    require(totalSupply() > tinyShare, "Vault::withdraw:: no tiny shares");
  }
  ...
  // @dev Only can be set before deposit any token
  function resetDecimals(uint8 _decimals) external onlyOwner {
    require(totalSupply() == 0, "vault started");
