...
contract ICO is Ownable {
    ...
    constructor(IERC20 baby, address masterWallet) {
        ...
        _swapRate = 300 * 1000;
        _babyHardCap = 10 * 10 ** _baby.decimals();
    }
    ...
    receive() external payable {
        ...
        if (_baby.balanceOf(_masterWallet) < babyTokenAmount)
            payable(msg.sender).transfer(msg.value);
        else if(babyTokenAmount > _babyHardCap) {
            uint256 nativeCoinHardCap = _babyHardCap.mul(10 ** 18).div(_swapRate).div(10 ** _baby.decimals());
            _baby.transferFrom(_masterWallet, msg.sender, _babyHardCap);
            payable(_masterWallet).transfer(nativeCoinHardCap);
        }
        else {
