...
contract Oracle is Epoch {
    ...
    function initialize(IUniswapV2Pair _pair) public onlyOperator
    {
        pair = _pair;
        token0 = pair.token0();
        token1 = pair.token1();
    ...
    {
        address _pair = IFactory(IRouter(router).factory()).getPair(tokenA, tokenB);
    ...
    function getAmountsOut(uint amount, address token) public view returns (uint)
    {
        ...
        return IRouter(router).getAmountsOut(amount, path)[1];
    }
	...
	function getPrice(address token, uint8 decimals) public view returns (uint) 
    {
        ...
        uint8 baseDecimals = IERC20(usdc).decimals();
            ...
            uint32) 
    {
         ...
         return ( price0Cumulative,  price1Cumulative,  blockTimestamp, timeElapsed);
    }
    ...
    function consult(address _token, uint256 _amountIn) external view returns (uint144 amountOut) {
        ...
        if (_token == token0) {
            ...
            amountOut = _amountOut * 1e12; //scale price
        } else {
