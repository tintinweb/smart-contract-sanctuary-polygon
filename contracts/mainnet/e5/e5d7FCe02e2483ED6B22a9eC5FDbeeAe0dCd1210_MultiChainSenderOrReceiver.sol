...
contract MultiChainSenderOrReceiver is BoringOwnable, IReceiver {
    ...
    mapping (IERC721 => IOriginal.Original) public cloneToOriginal;
    mapping (uint32 => mapping (IERC721 => IERC721)) public originalToClone;
    ...
    function sendNFT (IERC721 nft, uint256 id, uint256 chainId, address recipient, uint256 amountOnDst) external payable {
        require(msg.sender == nft.ownerOf(id), "Not Owner Of NFT");
        IOriginal.Original memory original = cloneToOriginal[nft];
        ...
        if (address(original.original) == address(0)) {
            original = IOriginal.Original(nft, uint32(block.chainid));
            nft.transferFrom(msg.sender, address(this), id);
            adapter.send{value: msg.value}(chainId, original, id, nft.name(), nft.symbol(), nft.tokenURI(id), recipient,  royalty, amountOnDst);
        } else {
    ...
    function receiveNFT (address nft, uint32 originChainId, string calldata name, string calldata symbol, string calldata tokenURI, uint256 tokenId, address recipient, IReceiver.RoyaltyData calldata royalty) external override {
        require(msg.sender == address(adapter), "Not from Adapter");

        if (originChainId == block.chainid) {
            IERC721(nft).transferFrom(address(this), recipient, tokenId);
        } else {
            IERC721 clone = originalToClone[originChainId][IERC721(nft)];
            if(address(clone) == address(0)) {
                ...
                IOriginal.Original memory original = IOriginal.Original(IERC721(nft), originChainId);
                originalToClone[originChainId][IERC721(nft)] = clone;
                cloneToOriginal[clone] = original;
                if (royalty.royaltyReceiver != address(0)) {
...
import {IOriginal} from "./interfaces/IERC721.sol";
...
interface IAdapter {
    ...
    function send (uint256 chainId, IOriginal.Original memory original, uint256 tokenId, string calldata name, string calldata symbol, string calldata tokenURI, address recipient, IAdapter.RoyaltyData calldata royalty, uint256 amountOnDst) external payable;
}
...
interface IReceiver {
    ...
    function receiveNFT (address nft, uint32 originChainId, string calldata name, string calldata symbol, string calldata tokenURI, uint256 tokenId, address recipient, RoyaltyData calldata royalty) external;
}
...
contract MultiChainAdapter is IAdapter, Ownable, ILayerZeroReceiver {
    ...
    function send(uint256 chainId, IOriginal.Original memory original, uint256 tokenId, string calldata name, string calldata symbol, string calldata tokenURI, address recipient, IAdapter.RoyaltyData calldata royalty, uint256 amountOnDst) external payable override {
        require(msg.sender == address(receiver), "Not from Receiver");

        bytes memory payload;
        {
            payload = abi.encode(original, tokenId, name, symbol, tokenURI, recipient, royalty);
        }

        uint16 chainIdLz = connections[chainId];
        require(chainIdLz != 0, "No LZ Connection");
    ...
    function lzReceive(uint16 _srcChainId, bytes memory _srcAddress,uint64 _nonce, bytes calldata _payload) external override {
        require(msg.sender == address(endpoint), "Sender not Endpoint");
        address fromAddress;
        assembly {
        ...
        (IOriginal.Original memory original, uint256 tokenId, string memory name, string memory symbol, string memory tokenURI, address recipient, IReceiver.RoyaltyData memory royalty) = abi.decode(_payload, (IOriginal.Original, uint256, string, string, string, address, IReceiver.RoyaltyData));

        receiver.receiveNFT(address(original.original), original.originChainId, name, symbol, tokenURI, tokenId, recipient, royalty);
    }
