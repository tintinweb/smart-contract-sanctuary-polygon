...
abstract contract RouterCrossTalk is Context , iRouterCrossTalk, ERC165 {
    ...
    function fetchExecutes( bytes32 hash ) external override view returns( executesStruct memory ) {
    ...
    /// @param _gasLimit Gas provided for cross chain send.
    /// @param _gasPrice GasPrice provided for cross chain send.
    function routerSend( uint8 destChainId , bytes4 _selector , bytes memory _data , uint256 _gasLimit , uint256 _gasPrice ) internal isLinkUnSet( destChainId ) returns (bool success , bytes32 hash) {
        bytes32 hash = _hash(address(this),Chain2Addr[destChainId], handler.fetch_chainID(), destChainId, _selector, _data);
        bytes memory data = abi.encode( _selector , _data );
        uint64 nonce = handler.genericDeposit(destChainId , data, hash , _gasLimit , _gasPrice , feeToken );
        executes[hash] = executesStruct( destChainId , nonce );
        emit CrossTalkSend( handler.fetch_chainID() ,destChainId , Chain2Addr[destChainId] , hash );
        return ( true , hash );
    }

    function routerSync(uint8 srcChainID , address srcAddress , bytes memory _data , bytes32 hash ) external override isLinkSync( srcChainID , srcAddress ) isHandler returns ( bool , bytes memory ) {
        uint8 cid = handler.fetch_chainID();
        ( bytes4 _selector , bytes memory data ) = abi.decode( _data , ( bytes4 , bytes ));
        bytes32 Dhash = _hash(Chain2Addr[srcChainID],address(this), srcChainID , cid, _selector, data);
        require( Dhash == hash , "RouterSync : Valid Hash" );
        ( bool success , bytes memory _returnData ) = _routerSyncHandler( _selector , data );
        emit CrossTalkReceive( srcChainID , cid , srcAddress , hash );
        return ( success , _returnData );
    }

    function routerReplay( bytes32 hash , uint256 _gasLimit , uint256 _gasPrice ) internal {
        handler.replayGenericDeposit(executes[hash].chainID , executes[hash].nonce , _gasLimit , _gasPrice );
    }
    ...
    function _hash(address _srcAddress , address _destAddress , uint8 _srcChainID ,uint8 _destChainId , bytes4 _selector , bytes memory _data) internal pure returns (bytes32) {
...
interface iGenericHandler {
    ...
    /// @param _gasPrice GasPrice Specified for the contract function
    /// @param _feeToken Fee Token Specified for the contract function
    function genericDeposit( uint8 _destChainID, bytes memory _data, bytes32 _hash, uint256 _gas, uint256 _gasPrice , address _feeToken) external returns ( uint64 );
...
interface iRouterCrossTalk is IERC165 {
    ...
    event CrossTalkSend(uint8 indexed sourceChain , uint8 indexed destChain ,  address destinationAddress , bytes32 _hash );
    ...
    /// @param _hash Hash of the data sent.
    event CrossTalkReceive(uint8 indexed sourceChain , uint8 indexed destChain , address sourceAddress ,  bytes32 _hash );
    ...
    function routerSync(uint8 srcChainID , address srcAddress , bytes calldata _data , bytes32 hash ) external returns ( bool , bytes memory );
    ...
    function fetchExecutes( bytes32 _hash ) external view returns( executesStruct memory );
}
