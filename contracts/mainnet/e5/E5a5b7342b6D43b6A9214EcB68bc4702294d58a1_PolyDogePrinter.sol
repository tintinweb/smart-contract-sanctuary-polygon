...
// Dependency file: contracts/baby/PolyDogePrinterDividendTracker.sol
    ...
    DividendPayingTokenOptionalInterface
{
    ...
    function distributePolyDogeDividends(uint256 amount) public onlyOwner {
...
contract PolyDogePrinterDividendTracker is OwnableUpgradeable, DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    function withdrawDividend() public pure override {
        require(
            false,
            "Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main PolyDogePrinter contract."
        );
    }
...
// Root file: contracts/baby/PolyDogePrinter.sol
...
// import "contracts/interfaces/IUniswapV2Factory.sol";
// import "contracts/interfaces/IUniswapV2Router02.sol";
// import "contracts/baby/PolyDogePrinterDividendTracker.sol";
// import "contracts/BaseToken.sol";

contract PolyDogePrinter is ERC20, Ownable, BaseToken {
    using SafeMath for uint256;
    ...
    PolyDogePrinterDividendTracker public dividendTracker;
    ...
    constructor(
        ...
        dividendTracker = PolyDogePrinterDividendTracker(
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(
            ...
            "PolyDogePrinter: The dividend tracker already has that address"
        );

        PolyDogePrinterDividendTracker newDividendTracker = PolyDogePrinterDividendTracker(
            payable(newAddress)
        );

        require(
            newDividendTracker.owner() == address(this),
            "PolyDogePrinter: The new dividend tracker must be owned by the PolyDogePrinter token contract"
        );
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(
            ...
            "PolyDogePrinter: The router already has that address"
        );
    ...
    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(
            ...
            "PolyDogePrinter: Account is already the value of 'excluded'"
        );
        _isExcludedFromFees[account] = excluded;
        ...
        onlyOwner
    {
        require(
            pair != uniswapV2Pair,
            "PolyDogePrinter: The PanPolyDogeSwap pair cannot be removed from automatedMarketMakerPairs"
        );
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(
            ...
            "PolyDogePrinter: Automated market maker pair is already set to that value"
        );
        automatedMarketMakerPairs[pair] = value;
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(
            ...
            "PolyDogePrinter: gasForProcessing must be between 200,000 and 500,000"
        );
        require(
            newValue != gasForProcessing,
            "PolyDogePrinter: Cannot update gasForProcessing to same value"
        );
    ...
    function swapAndSendToFee(uint256 tokens) private {
        uint256 initialPolyDogeBalance = IERC20(rewardToken).balanceOf(
            address(this)
        );

        swapTokensForPolyDoge(tokens);
        uint256 newBalance = (IERC20(rewardToken).balanceOf(address(this))).sub(
            initialPolyDogeBalance
        );
    ...
    function swapTokensForPolyDoge(uint256 tokenAmount) private {
    ...
    function swapAndSendDividends(uint256 tokens) private {
        ...
        if (success) {
            dividendTracker.distributePolyDogeDividends(dividends);
