...
contract LSD is BaseContract {
  ...
  function initialize() public initializer {
    BaseContract.init();
    config.minAmountForTaxDiscount = 10_000e6;
  ...
  function burn(uint _stAmount) external {
    ...
    _recheckQualifiedForTaxDiscount();
  }
  ...
  function _recheckQualifiedForTaxDiscount() private {
    if (users[msg.sender].qualifyForDiscountUntil <= block.timestamp) {
      users[msg.sender].qualifyForDiscountUntil = 0;
      return;
    }

    bool isFailed = true;

    for (uint i = 0; i < users[msg.sender].burnableAt.length; i++) {
      if (users[msg.sender].burnableAmount[i] >= config.minAmountForTaxDiscount) {
        isFailed = false;
        break;
      }
    }
