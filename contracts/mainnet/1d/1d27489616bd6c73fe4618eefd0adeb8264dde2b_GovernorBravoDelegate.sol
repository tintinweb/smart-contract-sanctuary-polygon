...
contract GovernorBravoDelegate is Initializable,UUPSUpgradeable,GovernorBravoDelegateStorageV2, GovernorBravoEvents {
    ...
    /**
      ...
      */
    function initialize(address timelock_, address uRvlt_, uint votingPeriod_, address treasury_, uint proposalPeriod_, uint rvltPriceProposal_, address _adminOwner, uint _governanceStartTime) public initializer{
        ...
        admin = timelock_;
        treasury = treasury_;
        governanceStartTime = _governanceStartTime;
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        ...
        require(keccak256(abi.encodePacked(signatures[0])) == keccak256(abi.encodePacked("_setInvesteeDetails(address,uint256)")), "GovernorBravo::propose: invalid proposal");
        ...
        uRvlt.updateLockInfo(msg.sender, startTime);
    ...
    /**
      * @notice update min proposal value to create Proposal
      * @param _rvltPriceProposal latest value of RVLT Proposal
      */
    function updateMinUSDValue(uint _rvltPriceProposal) external {
        require(msg.sender == adminOwner, "Invalid msgSender address");
        rvltPriceProposal = _rvltPriceProposal;
    }

    /**
      * @notice renounce admin address
      */
    function renounceAdmin() external {
        require(msg.sender == adminOwner, "Invalid msgSender address");
        adminOwner = address(0);
    }
    ...
    function cancel(uint proposalId) external {
        ...
        uRvlt.updateLockInfo(msg.sender, 0);
    ...
    function state(uint proposalId) public view returns (ProposalState) {
        ...
        } else if (proposal.forVotes <= proposal.againstVotes || add256(proposal.forVotes, proposal.againstVotes) < quorumVotes) {
    ...
    /**
      * @notice Cast votes in batch for a proposal
      * @param proposalId The id of the proposal in array to vote on
      * @param support The support value in array for the vote. 0=against, 1=for, 2=abstain
      */
    function batchCastVote(uint[] memory proposalId, uint8[] memory support) external {
        require(proposalId.length == support.length, "GovernorBravo:batchCastVote: Invalid data");
        for(uint i=0; i<proposalId.length; i++) {
            emit VoteCast(msg.sender, proposalId[i], support[i], castVoteInternal(msg.sender, proposalId[i], support[i]), "");
        }
    }
    ...
    /**
      ...
      */
    function batchCastVoteBySig(uint[] memory proposalId, uint8[] memory support, uint8 v, bytes32 r, bytes32 s) external {
        require(proposalId.length == support.length, "GovernorBravo:batchCastVote: Invalid data");
        for(uint i=0; i<proposalId.length; i++) {
            castVoteBySig(proposalId[i], support[i], v, r, s);
        }
    }
    ...
    function castVoteInternal(address voter, uint proposalId, uint8 support) internal returns (uint256) {
        ...
        receipt.votes = votes;
        uRvlt.depositCultmanders(voter);
