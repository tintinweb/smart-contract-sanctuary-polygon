...
import "./BitwaveNTT721SVG.sol";


/// @title A factory contract for NTT contracts.
/// @author Bitwave
/// @author Inish Crisson
/// @notice Now with support for fallback functions.
contract BitwaveNTT721SVGFactory {
...
/// @notice Deploys a new SVG based NTT contract. 
/// @return newContractAddress The address of the deployed contract.
    function deployNewSvgCert(
        string memory _name,
        ...
        NFTSVG.NftInfo memory _nftInfo = NFTSVG.NftInfo(
            _titleLineOne,
            _titleLineTwo,
            _titleLineThree,
            _colourOne,
        ...
        BitwaveNTT721SVG newBitwaveCert = new BitwaveNTT721SVG(
            _name,
            _symbol,
            _nftInfo);
        newBitwaveCert.transferOwnership(msg.sender);
...
/// @title NFTSVG
/// @notice Provides a function for generating an SVG associated with a certification NFT, and then encoding this.
library NFTSVG {
    ...
    function generateSVG(NftInfo memory params, uint256 tokenId, string memory name) internal pure returns (string memory svg) {
        ...
        svg = string(
            abi.encodePacked(
                ...
                params.titleLineOne,
                '</text><text y="110px" x="32px" fill="white" font-family="\'Courier New\', monospace" font-weight="200" font-size="36px">',
                params.titleLineTwo,
...
/// @title A modified nontransferable ERC721 token.
/// @author Inish Crisson
/// @author Jesse Farese
/// @author Kunz Mainali
/// @dev This contract is a modified version of the ERC721 token.
/// @dev It is used to produce SVG based NTTs.
contract BitwaveNTT721SVG is ERC721, Ownable {

    /// @notice Constructor, sets name, symbol.
    constructor(
        string memory _name,
        string memory _symbol,
        NFTSVG.NftInfo memory _nftInfo
        ) ERC721(_name, _symbol) {
            _localNftInfo = _nftInfo;
        }

    /// @notice iterating uint for tokenId creation.
    uint256 internal _currentIndex;

    /// @notice Local NFT info: colours, titles, description.
    NFTSVG.NftInfo internal _localNftInfo;
    ...
    function burn(uint256 tokenId) public onlyOwner {
        _burn(tokenId);
        delete tokenHolderNames[tokenId];
    }
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        return NFTSVG.constructURI(name(), _localNftInfo.description, NFTSVG.generateSVG(_localNftInfo, tokenId, tokenHolderNames[tokenId]));
    }
}
