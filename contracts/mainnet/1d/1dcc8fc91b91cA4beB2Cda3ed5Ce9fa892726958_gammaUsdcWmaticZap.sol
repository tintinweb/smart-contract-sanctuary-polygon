...
contract gammaUsdcWmaticZap is Ownable {
    ...
    constructor(address _router, address _WETH, address _yeller, address _priceFeedNative) {
    ...
    function _addLiqudity(address _pair, address[] memory _path, uint256[] memory _swapedAmounts) private returns(uint256) {
        address routerLiquidity = IUniswapV2Pair(_pair).whitelistedAddress();

        uint middleAmountUsdc = getLiqAmount(address(_pair), _path[1], _swapedAmounts[1], routerLiquidity);

        _approveTokenIfNeeded(_path[0], address(_pair)); 
        _approveTokenIfNeeded(_path[1], address(_pair)); 
        uint256[4] memory minIn = [uint256(0), uint256(0), uint256(0), uint256(0)];

        uint256 amountLiquidity = IUniswapV2Router02(routerLiquidity)
            .deposit(_swapedAmounts[1], middleAmountUsdc, address(this), address(_pair), minIn);
    ...
    function _swapAndStake(address _vault, uint256 tokenAmountOutMin, address tokenIn) private {
        ...
        uint256 swapAmountIn = _getSwapAmount(fullInvestment, reserveB, reserveA);
        ...
        uint amountLiquidity = _addLiqudity(address(pair), path, swapedAmounts);
    ...
    function getLiqAmount(address _pair, address _path, uint _swapedAmounts, address _routerLiquidity) internal view returns(uint) {
        (uint256 amountNeededUsdcMin, uint256 amountNeededUsdcMax)= IUniswapV2Router02(_routerLiquidity).getDepositAmount(_pair, _path, _swapedAmounts);
