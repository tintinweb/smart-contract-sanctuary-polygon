...
contract VestingV2 is VestingAbstract, AccessControl {
     ...
     */
    struct InfoVesting {
        uint128 amountDistribution;
        uint128 amountReceived;
        uint128 amountDistributed;
        uint128 price;
    ...
    event SetVesting(address account, uint256 amount);
    ...
    {
        _infoRound.amountDistribution = tokenDistribution;
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        _setupRole(ADMIN_ROLE, msg.sender);
    }

    function setVesting(address account, uint256 amount)
        external
        onlyRole(ADMIN_ROLE)
    {
        _infoRound.amountDistributed += uint128(amount);
        UserInfo storage user = _mapUserInfo[account];
        user.amountDistributed += uint128(amount);
        require(
            _infoRound.amountDistributed <= _infoRound.amountDistribution,
            "Maximum tokens sold"
        );

        emit SetVesting(account, amount);
    }
    ...
    function claim() external {
        ...
        IERC20(_basicInfo.token).safeTransfer(msgSender, availableReward);
        ...
        returns (uint256 AvailableReward)
    {
        return
            _calcAvailableReward(
                _mapUserInfo[account].amountDistributed,
     ...
     */
    function getInfoRound()
        external
        view
        returns (InfoVesting memory infoRound)
    {
        return _infoRound;
    }
