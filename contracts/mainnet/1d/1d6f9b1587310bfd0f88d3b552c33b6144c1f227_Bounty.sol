...
contract Bounty is ChainlinkClient {
    LinkTokenInterface private s_link;
    ...
    uint256 public executeDeadline;
    ...
    constructor(uint256 _durationInDays) {
        ...
        // Polygon Mainnet
        // https://polygonscan.com/token/0xb0897686c545045aFc77CF20eC7A532E3120E0F1
        address linkToken = 0xb0897686c545045aFc77CF20eC7A532E3120E0F1;
        s_link = LinkTokenInterface(linkToken);
        setChainlinkToken(linkToken);

        // https://polygonscan.com/address/0xbac0638773782e9130DfeAF59a5b27bBc49a7007
        setChainlinkOracle(0xbac0638773782e9130DfeAF59a5b27bBc49a7007);
    }

    function isFinished() private view returns (bool) {
        return appealDeadline > 0 && block.timestamp > appealDeadline;
    }

    function contribute() public payable {
        require(!isFinished(), "Bounty finished");
        require(deadline > block.timestamp, "Bounty has ended");
    ...
    function reserve() public payable {
        require(deadline > block.timestamp, "Bounty has ended");
        require(!isFinished(), "Bounty finished");
        require(executeDeadline < block.timestamp, "Already reserved");
        require(
            msg.value >= 10 * 10**18,
            "You need to deposit at least 10 MATIC to reserve"
        );

        contributions[msg.sender] += msg.value;
        raisedAmount += msg.value;

        executeDeadline = block.timestamp + 4 hours;
        executor = msg.sender;
    }

    function declare() public {
        require(deadline > block.timestamp, "Bounty has ended");
        require(executor == msg.sender, "Only executor can declare");
        require(executeDeadline > block.timestamp, "Execution deadline passed");
        require(!isFinished(), "Bounty finished");

        uint256 payment = 15 * LINK_DIVISIBILITY / 100;
        require(s_link.balanceOf(address(this)) > payment, "Too little LINK");

        Chainlink.Request memory req = buildChainlinkRequest(
            "8ed704f0e2e7441181ba6a36b30b4215",
            address(this),
            this.appeal.selector
        );

        req.add("get", "http://37.28.176.111:81/");

        sendChainlinkRequest(req, payment);

        winner = msg.sender;
        appealDeadline = block.timestamp + 10 minutes;
    }

    function appeal(bytes32 _requestId, uint256)
        public
        recordChainlinkFulfillment(_requestId)
    {
        delete winner;
        delete appealDeadline;
    }

    function getReward() public {
        require(raisedAmount > 0, "Reward taken already");
        require(winner == msg.sender, "Only winner can get reward");
        require(isFinished(), "Bounty not finished yet");
