...
contract RebaseToNonEoaHandler is Initializable, Governable {
    ...
    function addSatinContract(address _satinPoolAddress, uint256 _guageShare, uint256 _bribeShare, uint256 _treasuryShare) external onlyGovernor {
    ...
    function _calculate4poolSkimmable(address _4PoolAddress) internal view returns (uint256 _skimmableCashAmount) {
        ...
        uint cashReserve = I4Pool(_4PoolAddress).getTokenBalance(tokenIndex);
        _skimmableCashAmount = cash.balanceOf(_4PoolAddress) - cashReserve;
    }

    function _calculateSkimmable(
        address _contractAddress
    ) internal view returns (address _skimmableToken, uint256 _skimmableTokenAmount, uint256 _skimmableCashAmount) {
        ...
        if (address(_token0) == address(cash)) {
            ...
            _skimmableTokenAmount = IERC20(_token1).balanceOf(_contractAddress) - _reserve1;
        } else if (address(_token1) == address(cash)) {
            _skimmableToken = _token0;
            _skimmableTokenAmount = IERC20(_token0).balanceOf(_contractAddress) - _reserve0;
            _skimmableCashAmount = cash.balanceOf(_contractAddress) - _reserve1;
        }
    }

    function _allowFee(address _recipient, uint256 _skimmableCashAmount, uint256 _share) internal returns (uint _shareAmount) {
