...
contract BEER is ERC20, AccessControl, ERC721Holder {
    ...
    constructor(address owner, address _collectionAddress) ERC20("Beer", "BEER") AccessControl(){
        ...
        coinAmountPerRewardUnit = 50 * 10 ** 18; // 50 ERC20 coins per rewardUnit, may be changed later on
        numberOfBlocksPerRewardUnit = 43200; // 24 hours per reward unit , may be changed later on
    }
    ...
    function stake(uint256 tokenId) public {
        ...
        require(IERC721(collectionAddress).ownerOf(tokenId) == address(this),
            "BEER: Error while transferring token");
    ...
    function setCollectionAddress(address newAddress) external onlyRole(CONTRACT_ADMIN_ROLE){
        require (newAddress != address(0), "BEER: update to zero address not possible");
