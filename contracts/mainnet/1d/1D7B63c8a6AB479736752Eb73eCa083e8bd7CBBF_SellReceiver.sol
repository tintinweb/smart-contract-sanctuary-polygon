...
contract SellReceiver {
    ...
    // Events
    event Trigger();
    event SetPercentage(uint256 treasury_, uint256 staking_, uint256 yieldFarm_);
    event SetAddresses(address treasury_, address staking_, address yieldFarm_);
    event SetMinimumToTrigger(uint256 newMin);
    ...
    function trigger() external {
        ...
        uint256 partYield = balance * yieldFarmPercent / 100;
        uint256 partBurn = balance - ( partStaking + partTreasury + partYield );
    ...
    function setPercentages(uint treasury_, uint staking_, uint yieldFarm_) external onlyOwner {
        ...
        emit SetPercentage(treasury_, staking_, yieldFarm_);
    }
    ...
    function setAddresses(address treasury_, address staking_, address yieldFarm_) external onlyOwner {
        ...
        require(goodAddress(yieldFarm_) == true, 'Invalid Address!');

        treasury = treasury_;
        staking = staking_;
        yieldFarm = yieldFarm_;

        emit SetAddresses(address(treasury), address(staking), address(yieldFarm));
    }
