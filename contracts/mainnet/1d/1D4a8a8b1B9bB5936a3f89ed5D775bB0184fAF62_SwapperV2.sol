...
// File: contracts/Token/Math/PRBMath.sol
...
// File: contracts/Token/Math/PRBMathSD59x18.sol
...
contract SwapperV2 {
    ...
    function getTokenAPriceV3(address router,address tokenA,address tokenB,uint amountA) public view returns(uint){
        ...
        uint32[] memory sec=new uint32[](2);
        sec[0]=1;sec[1]=3;
        (int56[] memory ticks,)=IDEXPair(ad).observe(sec);
        int  tick=(ticks[0]-ticks[1]);
        if(tick<0){tick=-1*tick;}
        uint price=uint(PRBMathSD59x18.toInt(PRBMathSD59x18.pow(PRBMathSD59x18.div(10001, 10000), PRBMathSD59x18.div(tick, 2))));
        return tokenA < tokenB ? amountA*price : (amountA/price);
    }
    ...
    function bestPathSwapTokensForTokens(address tokenA,address tokenB,uint amountA,uint slippage) public payable {
        ...
        if(wmatic==tokenB){
            IERC20(matic).withdraw(amountTokenB);
