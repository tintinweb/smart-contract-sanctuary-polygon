...
/// @title X's Dice Roll game
/// @notice The game is played with a 6 sided dice. The game's goal is to guess whether the lucky number will be  your chosen number
/// @author Anush GS
contract DiceRoll is VRFConsumerBaseV2, ReentrancyGuard {
    /// @notice Enum to represent the faces of a dice.
    enum DiceFace {
        One,
        Two,
        Three,
    ...
    /// @notice Enum to track dice roll status
    enum BetStatus {
        Active,
        Loss,
        Win,
    ...
    /// @notice Full dice roll bet information struct.
    ...
    /// @param timesstamp The time stamp of dice rolled
    /// @param choice The The chosen dice face
    /// @param status The current bet status
    /// @param flipped bool to identify dice rolled
    ...
    /// @param  minStake The minimum stake allowed on Dice Roll
    /// @param maxStake The maximum stake allowed on Dice Roll
    struct StakeLimits {
        uint256 minStake;
        uint256 maxStake;
    }
    ...
    /// @notice Maps chainlink requestId to RolledDice structure
    ...
    /// @notice Owner of Dice Roll Game
    ...
    /// @notice Emitted when dice is rolled
    ...
    event DiceRolled(uint256 indexed requestId, address indexed userAddress, address indexed token, uint256 odds, uint256 stake, uint256 payout, DiceFace choice, BetStatus status);

    /// @notice Emitted when dice roll is settled
    /// @param requestId Used to match randomness requests with their fullfillment order
    /// @param userAddress address of the bettor of the bettor
    /// @param token address of the token
    /// @param status The current bet status
    ...
    event DiceRollSettled(uint256 indexed requestId, address indexed userAddress, address indexed token, BetStatus status, uint256 randomWord);

    /// @notice Reverting error when method is not called by the owner
    error DiceRoll__OnlyOwnerMethod();

    /// @notice Reverting error when dice roll is called anb betallowed is false
    error DiceRoll__GameDisabled();

    /// @notice Reverting error when address is invalid
    error DiceRoll__IncorrectAddress();

    /// @notice Reverting error when stake less than limit
    ...
    /// @notice Modifier to restrict access to the owner of the DiceRoll contract.
    ...
    /// @notice Transfers the ownership of the DiceRoll contract to a new address.
    ...
    /// @param _newOwner The address of the new owner.
    /// @dev Internal function to transfer ownership of the DiceRoll contract.
    ...
    /// @dev Only the owner of the DiceRoll contract can call this function.
    ...
    function setOdds(address token, uint8 odds) public onlyDiceRollOwner {
    ...
    ///      Emits the DiceRolled event with the bet information.
    function rollDice(address token, uint256 stake, DiceFace choice) external nonReentrant isGameAllowed {
        ...
        s_requestIdToDiceRoll[requestId] = RolledDice(msg.sender, token, provider, s_tokenToOdds[token], stake, payout, 6, block.timestamp, choice, BetStatus.Active, true);
        emit DiceRolled(requestId, msg.sender, token, s_tokenToOdds[token], stake, payout, choice, BetStatus.Active);
        if (tips > 0) {
    ...
    /// @dev This function is called internally to settle a rolled dice  bet based on the received random words.
    ...
    ///      Determines the result of the dice roll based on the random word.
    ///      Updates the bet status and random word in the RolledDice struct.
    ///      Emits the DiceRollSettled event with the updated bet information.
    ...
    function fulfillRandomWords(uint256 requestId, uint256[] memory randomWords) internal override {
        ...
        if (rolledDice.stake > i_core.getUserStaked(rolledDice.userAddress, rolledDice.token)) revert DiceRoll__InsuffcientUserStake();
        ...
        emit DiceRollSettled(requestId, rolledDice.userAddress, rolledDice.token, rolledDice.status, randomWord);
        if (rolledDice.choice == result) {
            i_core.handleStakes(rolledDice.userAddress, rolledDice.token, rolledDice.stake, 0);
            i_core.handlePayout(rolledDice.provider, rolledDice.token, rolledDice.payout, 0);
            i_core.handleBalance(rolledDice.userAddress, rolledDice.token, rolledDice.stake + rolledDice.payout, 1);
        } else {
            i_core.handleStakes(rolledDice.userAddress, rolledDice.token, rolledDice.stake, 0);
            i_core.handlePayout(rolledDice.provider, rolledDice.token, rolledDice.payout, 0);
    ...
    ///      Reverts if the `rolledDice.rolled` is false, indicating an invalid `requestId`.
    ///      Reverts if the sender is not the bettor of the bet.
    ///      Reverts if the bet status is not `Active`.
    ///      Reverts if the time since the dice roll is not greater than the bet timelock duration.
    ...
    ///      Emits the `DiceRollSettled` event with the updated bet status.
    ...
    function suspendBet(uint256 requestId) external nonReentrant {
        RolledDice storage rolledDice = s_requestIdToDiceRoll[requestId];
        if (!rolledDice.rolled) revert DiceRoll__RequestIdNotFound();
        if (rolledDice.userAddress != msg.sender) revert DiceRoll__NotBettor();
        if (rolledDice.status != BetStatus.Active) revert DiceRoll__OnlyActiveSuspend();
        if (!((block.timestamp - rolledDice.timestamp) > s_tokenToBetTimelock[rolledDice.token])) revert DiceRoll__Timelock();
        if (rolledDice.stake > i_core.getUserStaked(rolledDice.userAddress, rolledDice.token)) revert DiceRoll__InsuffcientUserStake();
        if (rolledDice.stake > i_core.getTotalStakes(rolledDice.token)) revert DiceRoll__InsuffcientStakes();
        if (rolledDice.payout > i_core.getProviderPayout(rolledDice.provider, rolledDice.token)) revert DiceRoll__InsuffcientProviderPayouts();
        if (rolledDice.payout > i_core.getTotalPayout(rolledDice.token)) revert DiceRoll__InsuffcientPayouts();
        rolledDice.status = BetStatus.Suspend;
        emit DiceRollSettled(requestId, rolledDice.userAddress, rolledDice.token, rolledDice.status, 6);
        i_core.handleStakes(rolledDice.userAddress, rolledDice.token, rolledDice.stake, 0);
        i_core.handlePayout(rolledDice.provider, rolledDice.token, rolledDice.payout, 0);
        i_core.handleBalance(rolledDice.userAddress, rolledDice.token, rolledDice.stake, 1);
    ...
    /// @notice Retrieves the address of the Dice Roll owner.
    /// @return owner The address of the Dice Roll owner.
    ...
    function getDiceRollOdds(address token) public view returns (uint8) {
    ...
    /// @notice Retrieves the RolledDice struct associated with a given requestId.
    /// @param requestId The ID of the request.
    /// @return coinFlip The RolledDice struct associated with the requestId.
