...
// File: contracts/FBBlockchainExperiences-ERC1155.sol
pragma solidity ^0.8.0;

contract FBBlockchainExperiences is ERC1155, Ownable {
    ...
    constructor(uint[] memory _maxAmounts, string[] memory _uris) ERC1155("") {
        require(_maxAmounts.length == _uris.length, "max amounts and uris not same length");
        name = "FB Blockchain Experiences";
        symbol = "FBBE";
        for (uint i=0; i<_maxAmounts.length; i++) {
            typeProps[i] = TypeProp(0, _maxAmounts[i], _uris[i]);
            numTypes++;
        }
    }

    function addType(uint _maxAmount, string memory _uri) external onlyOwner {
        typeProps[numTypes] = TypeProp(0, _maxAmount, _uri);
        numTypes++;
    }
