...
contract ZeroXAdapter is AdapterBase {
  ...
  constructor(address _registry, address _zeroXRouter) AdapterBase(_registry) {
    zeroXRouter = _zeroXRouter;
    feeRate = 1;
  }

  function setZeroXRouter(address _zeroXRouter) public onlyRegistry {
    zeroXRouter = _zeroXRouter;
  }
  ...
  */
  ...
  ) public payable virtual override onlyRegistry {
    require(_inputToken != address(0) && _outputToken != address(0), "INVALID_ASSET_ADDRESS");
    bool success;
    bytes memory result;
    if (_inputToken != NATIVE) {
        require(IERC20(_inputToken).balanceOf(address(this)) >= _inputTokenAmount, "UNAVAIBLE_FUNDS");
...
abstract contract AdapterBase {
  ...
  event FeeRateChange(uint8 indexed newFeeRate);
  ...
  */
  modifier onlyRegistry {
    require(registry == msg.sender, "UNAUTHORIZED");
    _;
  }
  ...
  */
  function computeFee(uint256 amount) public view returns (uint) {
    return (50 + (amount * feeRate)) / 100;
  }

  // onlyRegistry functions 

  /**
  // @dev set fee rate
  // @param _feeRate fee rate in percentage
  */
  function setFeeRate(uint8 _feeRate) public onlyRegistry {
    feeRate = _feeRate;
    emit FeeRateChange(_feeRate);
  }

  /**
  // @dev transfer funds to the registry
  ...
  */
  ...
  // @dev set a new value for registry
  // @param _newGovernance address of the new governance contract
  */
  function transferGovernance(address _newGovernance) public onlyRegistry {
    require(_newGovernance != address(0), "ZERO_ADDRESS_FORBIDDEN");
    registry = _newGovernance;
