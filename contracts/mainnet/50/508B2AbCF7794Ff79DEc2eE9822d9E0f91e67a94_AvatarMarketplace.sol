...
interface IAvatar {
    ...
    function getMultiplier(uint256 avatarId) external view returns (uint256);
...
contract AvatarMarketplace is ReentrancyGuard, PresaleTax {
    ...
    mapping(bytes32 => AvatarType) private avatarTypes;

    function buyAvatar(bytes32 typeId) external nonReentrant {
        _buyAvatar(typeId);
    }

    function buyAvatar(bytes32 typeId, uint256 quantity) external nonReentrant {
    ...
    function getTypeInfo(bytes32 id) external view returns (AvatarType memory) {
        return avatarTypes[id];
    }

    //---------------------------------------------------//

    function _buyAvatar(bytes32 typeId) private status {
    ...
    function _removeType(bytes32 typeId) private {
        avatarTypes[typeId] = AvatarType(bytes32(0), 0, 0, "");
    }
