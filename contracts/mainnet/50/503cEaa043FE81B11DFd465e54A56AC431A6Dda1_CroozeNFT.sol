    ...
    OperatorFiltererUpgradeable
{
    ...
    mapping(address => string) marketplaces;
    ...
    modifier priceMatch(uint256 payAmount) {
        require(payAmount == price(), "money was not mached");
        _;
    }
    ...
    function initialize(
        ...
        __ERC2981_init();
        __OperatorFilterer_init(address(0), false);
        ...
        fee = fee_;
        _setDefaultRoyalty(royaltyReceiver, fee);
    }
    ...
    function mint_(
        ...
        _mint(to, newItemId);
        _setTokenRoyalty(newItemId, royaltyReceiver, fee);
    ...
    function customMint(
        ...
        return mint_(to, _tokenURI);
    }

    function generatorMint(
        address to,
        string memory _tokenURI,
        address payToken,
        uint256 amount
    ) external supportTokens(payToken) priceMatch(amount) returns (uint256) {
        IERC20Upgradeable(payToken).transferFrom(to, owner(), amount);
        return mint_(to, _tokenURI);
    }
    ...
    function updateOperator(
        ...
        if (filtered) {
            marketplaces[operator] = name;
        }
        registry.updateOperator(address(this), operator, filtered);
    }
    ...
    function setCreatorFee(uint96 newFee) external onlyOwner {
        require(newFee >= 0 && newFee < 5000, "fee should be from 0 to 50");
        fee = newFee;
    }
