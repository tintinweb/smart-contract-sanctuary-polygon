...
contract SimpleStreamManager is SuperAppBase, Initializable {
    ...
    event PaymentWithdrawalSuccessful(uint256 amount);
    ...
    function withdrawPayments(uint256 _amount) external {
        ...
        if (_amount == type(uint256).max)
            _amount = paymentToken.balanceOf(address(this));

        if (!(paymentToken.transfer(msg.sender, _amount)))
            revert PaymentWithdrawalFailed(_amount);
        
        emit PaymentWithdrawalSuccessful(_amount);
    }
