...
contract MetAmirGameToken is EIP20Interface {
    ...
    constructor () public {
        ...
        symbol = "MetAmir";                              // Set the symbol for display purposes.
        ...
        Token_OnHand = totalSupply;                     // Set first token value.
        contract_GasPercent = 3;                        // Default Gas of contract.
        contract_MaxBalance = totalSupply * 3 / 100;    // Maximum Tokens in per wallet.
        contract_ParentPercent = 3;

        // Settings
        steps[1].HoldTime = 31536000;
        steps[1].OnHand = totalSupply * 3 / 100;

        steps[2].HoldTime = 7776000;
        steps[2].OnHand = totalSupply * 1 / 100;

        steps[3].HoldTime = 15552000;
        steps[3].OnHand = totalSupply * 3 / 100;
    }
    ...
    function owner_del(address _ownerAddress) public onlyOwner returns (bool success) {
        ...
        require(_ownerAddress != msg.sender, "Self deleting?!");
        if (owners[_ownerAddress] > 0) {
            // Delete access after 10 days.
            require(owners[msg.sender] + 864000 < block.timestamp, "Access denied!"); 
