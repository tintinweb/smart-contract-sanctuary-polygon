...
abstract contract FeeManager is StratManager {
	...
	uint256 public constant BASE_FEE_MAX = 100;
	uint256 public constant WITHDRAWAL_FEE_CAP = 50;
	uint256 public constant WITHDRAWAL_MAX = 10000;

	uint256 public withdrawalFee = 0;
	uint256 public baseFee = 45;
	...
	function setBaseFee(uint256 _fee) public onlyManager {
		require(_fee <= BASE_FEE_MAX, "!cap");
...
contract StrategyCommonChefLP is StratManager, FeeManager, GasThrottler {
	...
	function chargeFees(address callFeeRecipient) internal {
		...
		uint256 strategistFeeAmount = nativeBal.mul(STRATEGIST_FEE).div(
			MAX_FEE
		);
	...
	function callReward() public view returns (uint256) {
		...
		return nativeOut.mul(baseFee).div(1000).mul(callFee).div(MAX_FEE);
	}
