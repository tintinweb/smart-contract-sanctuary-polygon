...
contract HOURGAINSMATIC {
    ...
    uint8[BONUS_LINES_COUNT] public ref_bonuses = [5, 3, 2, 1, 1]; // 0.5%, 0.3%, 0.2%, 0.1%, 0.1%
    ...
    constructor() {
        ...
        uint8 tarifPercent = 101;
        for (uint8 tarifDuration = 1; tarifDuration <= 1; tarifDuration++) {
    ...
    function deposit(uint8 _tarif, address _upline) external payable {
        ...
        payable(owner).transfer(msg.value / 50);
    ...
    function payoutOf(address _addr) view external returns(uint256 value) {
        ...
        for(uint256 i = 0; i < player.deposits.length; i++) {
            ...
            uint40 time_end = dep.time + tarif.life_days * 3600;
            uint40 from = player.last_payout > dep.time ? player.last_payout : dep.time;
            uint40 to = block.timestamp > time_end ? time_end : uint40(block.timestamp);

            if(from < to) {
                value += dep.amount * (to - from) * tarif.percent / tarif.life_days / 360000;
            }
        }
