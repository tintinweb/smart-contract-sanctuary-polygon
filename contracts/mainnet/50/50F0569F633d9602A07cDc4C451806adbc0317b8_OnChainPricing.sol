...
contract OnChainPricing is UUPSUpgradeable, OwnableUpgradeable {
    ...
    function updateDmm(
        ...
        if (pool != address(0)) {
            ...
            IERC20[] memory tokenPath = new IERC20[](2);
            tokenPath[0] = IERC20(token);
            tokenPath[1] = IERC20(pairToken);

            uint256[] memory amounts = IDMMExchangeRouter(KYBER_DMM_ROUTER)
                .getAmountsOut(1e18, pools, tokenPath);

            require(amounts[1] > 0, "wrong pool");
        }
