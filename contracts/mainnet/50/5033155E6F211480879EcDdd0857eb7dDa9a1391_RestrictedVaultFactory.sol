...
contract VaultFactory is Ownable {
  ...
  /// @notice Default deposit tolerance value to set on adapter when deploying
  ...
  event VaultCreated(uint256 vaultId, uint256 indexed vaultType, address _adapter, address indexed creator, address indexed vault);
  ...
  /// @param adapterType Type ID of adapter
  /// @param base Address of base asset
  /// @param creator Address of creator
  /// @param adapter Address of adapter
  event AdapterCreated(uint256 id, uint256 indexed adapterType, address base, address indexed creator, address indexed adapter);
  ...
  /// @param adapterType ID of adapter type to use
  /// @param base Base address for the adapter
  ...
  function createAdapter(
    ...
    bytes memory bytecode = adapterTypeByteCode[adapterType];
    ...
    IAdapter(adapterAddress).initialize(adapterId, base, defaultDepositTolerance, data);

    AdapterInfo memory ai = AdapterInfo({creatorAddress: msg.sender, addr: adapterAddress, adapterTypeId: adapterType});
    deployedAdapterInfo[adapterId] = ai;
    deployedAdapterAddrToId[adapterAddress] = adapterId;

    emit AdapterCreated(adapterId, adapterType, base, msg.sender, adapterAddress);
  }
  ...
  /// @notice Set new default deposit tolerance to be set on newly deployed adapters
  ...
  function setDefaultDepositTolerance(uint256 _defaultDepositTolerance) external onlyOwner {
    require(_defaultDepositTolerance != 0, "NZI");
