...
contract WidoCallExecutor is AccessControl {
    ...
    constructor(address _widoRouter, address _widoTokenManager, address _externalCallAdapter) {
    ...
    function call(
        ...
        uint256 _txGas,
        bytes memory _data
    ) external onlyAdapter returns (bool _result) {
        IERC20 token = IERC20(_token);
        uint256 amount = token.balanceOf(address(this));

        // create approve to allow the target contract to spend tokens.
        if (amount > 0) {
            _customApprove(token, widoTokenManager, amount);
        }

        widoRouter.call(_data);
        _result = true;
        ...
        if (amount > 0) {
            ...
            _customApprove(token, widoTokenManager, 0);
        }
    }
