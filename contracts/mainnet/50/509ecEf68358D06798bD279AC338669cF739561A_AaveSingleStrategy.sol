...
/**
 * @title Aave Strategy
 * @notice Investment strategy for investing stablecoins via Aave Strategy
 */
...
contract AaveSingleStrategy is InitializableAbstractStrategy {
    using StableMath for uint256;
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    IERC20 public token0;
    IERC20 public _amToken;

    IPool pool;

    function initialize(
        address _platformAddress, // aave provider
    ...
    function withdraw(address _beneficiary, address _asset, uint256 _amount) external override onlyVaultOrGovernor nonReentrant {
        require(_asset == address(token0), "Token not compatible.");
        _amToken.approve(address(pool), _amount);
        uint256 withdrawAmount = pool.withdraw(_asset, _amount, address(this));

        token0.safeTransfer(_beneficiary, withdrawAmount);
    }
    ...
    function withdrawAll() external override onlyVaultOrGovernor nonReentrant {
        uint256 _bal = _amToken.balanceOf(address(this));
        _amToken.approve(address(pool), _bal);
        pool.withdraw(address(token0), _bal, address(this));
    ...
    function _checkBalance() internal view returns (uint256) {
        return token0.balanceOf(address(this)) + _amToken.balanceOf(address(this));
    }
