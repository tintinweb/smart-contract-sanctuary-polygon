...
contract RewardPool {
    ...
    /**
     * Gives unlimited allowance of a token to the protocol-caller.
     * @param token_ token address
     */
    function giveAllowance(address token_) external {
        require(msg.sender == protocol, "caller-not-protocol");
        approve(TokenInterface(token_), msg.sender, type(uint256).max);
    }

    /**
     * @param protocol_ address of protocol contract
     */
