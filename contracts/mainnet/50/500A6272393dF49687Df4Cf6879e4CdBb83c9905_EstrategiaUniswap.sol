...
contract EstrategiaUniswap is Ownable {
    ...
    IUniswapV2Pair public lpToken = IUniswapV2Pair(0x2cF7252e74036d1Da831d11089D326296e64a728);
    IUniswapV2Router public router = IUniswapV2Router(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
    IStakingRewards public staking = IStakingRewards(0xAFB76771C98351Aa7fCA13B130c9972181612b54);
    ...
    address public wMatic = address(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270); // Nativo
    address public tokenRegalo = address(0xf28164A485B0B2C90639E47b0f377b4a438a16B1);

    address[] public tokenParRuta = [token, par];
    address[] public parTokenRuta = [par, token];
    address[] public regaloTokenRuta = [tokenRegalo, wMatic, token];
    ...
    constructor(address _vault) {
        ...
        require(token == lpToken.token0(), 'no es USDC');
        require(par == lpToken.token1(), 'no es USDT');
     ...
     */
    function gananciaEstimada() public view returns (uint256) {
        uint256 tokensRegalo = staking.earned(address(this));
        if (tokensRegalo == 0) return 0;
        uint256[] memory ganancias = router.getAmountsOut(tokensRegalo, regaloTokenRuta);
        return ganancias[ganancias.length - 1];
    }
    ...
    /**
     * @dev Devuelve el balance de lpTokens en la estrategia.
     */
    function balanceDeLpTokens() public view returns (uint256) {
        uint256 lpTokenContrato = lpToken.balanceOf(address(this));
        uint256 lpTokenStaking = staking.balanceOf(address(this));
        return lpTokenContrato + lpTokenStaking;
    }
    ...
    function balanceDesplegado() public view returns (uint256 balance) {
        (uint256 reservaA, uint256 reservaB, ) = lpToken.getReserves();
        uint256 totalSupply = lpToken.totalSupply();
        uint256 lpBalance = balanceDeLpTokens();
        balance = ((reservaA + reservaB) * lpBalance) / totalSupply;
    }
    ...
    function retirarEstrategia() external {
        ...
        _removerLiquidez(balanceDeLpTokens());
    ...
    function _darLiquidez() internal {
        router.addLiquidity(
            ...
            par,
            balanceDeToken(),
            IERC20(par).balanceOf(address(this)),
            0,
            0,
            address(this),
            block.timestamp
     ...
     */
    function _removerLiquidez(uint256 lpTokens) internal {
        router.removeLiquidity(token, par, lpTokens, 0, 0, address(this), block.timestamp);
    }

    /**
     * @dev Cambia el el token regalo por USDC.
     */
    function _venderReward() internal {
        uint256 tokenRegaloBal = IERC20(tokenRegalo).balanceOf(address(this));
    ...
    function _retirarAlgo(uint256 cantidad) internal {
        uint256 cantidadEnLpToken = (cantidad * balanceDeLpTokens()) / balanceDesplegado();
        staking.withdraw(cantidadEnLpToken);
        _removerLiquidez(cantidadEnLpToken);
        _venderPar();
    }
    ...
    function _darPermisos() internal {
        ...
        IERC20(tokenRegalo).safeApprove(address(router), MAX);
        lpToken.approve(address(staking), MAX);
    }
    ...
    function _removerPermisos() internal {
        ...
        IERC20(tokenRegalo).safeApprove(address(router), 0);
        lpToken.approve(address(staking), 0);
    }
...
/**
 * @dev Interface of Uniswap's IUniPair.
 */
