...
abstract contract CommentableERC721 is ERC721Pausable {

    // Define a struct for comments
    struct Comment {
        address commenter;
        string text;
        uint256 likeCount;
    ...
    // Define a mapping from token ID to the total number of likes
    mapping(uint256 => uint256) private _tokenLikeCount;

    // Define a mapping from token ID to an array of comments
    mapping(uint256 => Comment[]) private _tokenComments;

    // Define a mapping from token ID to a mapping of comment ID to whether the comment has been liked by a given address
    mapping(uint256 => mapping(uint256 => mapping(address => LikeType))) private _commentLikes;

    // Define a mapping from token ID to whether the token has been liked by a given address
    mapping(uint256 => mapping(address => LikeType)) private _tokenLikes;

    // Event emitted when a token is liked
    event TokenLiked(uint256 indexed tokenId, address indexed liker);

    // Event emitted when a new comment is added
    event CommentAdded(uint256 indexed tokenId, uint256 indexed commentId, address indexed commenter, string text);

    // Event emitted when a comment is liked
    event CommentLiked(uint256 indexed tokenId, uint256 indexed commentId, address indexed liker);

    // Function to add a comment to a token
    function addComment(uint256 tokenId, string memory commentText) public {
        // Ensure the caller owns the token
        require(ownerOf(tokenId) == msg.sender, "CommentableERC721: Only token owner can add comment");

        // Add the comment to the token's array of comments
        _tokenComments[tokenId].push(Comment(msg.sender, commentText, 0));

        // Emit the CommentAdded event
        emit CommentAdded(tokenId, _tokenComments[tokenId].length - 1, msg.sender, commentText);
    }

    // Function to get the comments for a token
    function getComments(uint256 tokenId) public view returns (Comment[] memory) {
        return _tokenComments[tokenId];
    }

    // Function to check if a comment has been liked by a given address
    function isCommentLiked(uint256 tokenId, uint256 commentId, address liker) public view returns (LikeType) {
        return _commentLikes[tokenId][commentId][liker];
    }

    // Function to like a token
    function likeToken(uint256 tokenId) public {
        // Ensure the caller does not own the token
        require(ownerOf(tokenId) != msg.sender, "CommentableERC721: Token owner cannot like own token");

        // Ensure the caller has not already liked the token
        require(_tokenLikes[tokenId][msg.sender] != LikeType.Like, "CommentableERC721: Token already liked");

        _tokenLikeCount[tokenId]++;
        if(_tokenLikes[tokenId][msg.sender] == LikeType.Unlike)
            _tokenLikeCount[tokenId]++;
        // Mark the token as liked by the caller
        _tokenLikes[tokenId][msg.sender] = LikeType.Like;
        // Emit the TokenLiked event
        emit TokenLiked(tokenId, msg.sender);
    }

    // Function to unlike a token and update the total number of likes
    function unlikeToken(uint256 tokenId) public {
        // Ensure the caller does not own the token
        require(ownerOf(tokenId) != msg.sender, "CommentableERC721: Token owner cannot unlike own token");

        // Ensure the caller has already liked the token
        require(_tokenLikes[tokenId][msg.sender] != LikeType.Unlike, "CommentableERC721: Token already unliked");

        // Decrement the total number of likes for the token
        _tokenLikeCount[tokenId]--;
        if(_tokenLikes[tokenId][msg.sender] == LikeType.Like)
            _tokenLikeCount[tokenId]--;

        // Mark the token as unliked by the caller
        _tokenLikes[tokenId][msg.sender] = LikeType.Unlike;
    ...
    function likeComment(uint256 tokenId, uint256 commentId) public {
        // Ensure the caller owns the token
        require(ownerOf(tokenId) == msg.sender, "CommentableERC721: Only token owner can like comment");

        // Ensure the comment exists
        require(commentId < _tokenComments[tokenId].length, "CommentableERC721: Invalid comment ID");

        // Ensure the caller has not already liked the comment
        require(_tokenLikes[tokenId][msg.sender] != LikeType.Like, "CommentableERC721: Comment already liked");

        if(_tokenLikes[tokenId][msg.sender] == LikeType.Unlike) {
            _tokenComments[tokenId][commentId].likeCount++;
        }
        _tokenComments[tokenId][commentId].likeCount++;
        // Mark the comment as liked by the caller
        _commentLikes[tokenId][commentId][msg.sender] = LikeType.Like;
        // Emit the CommentLiked event
        emit CommentLiked(tokenId, commentId, msg.sender);
    }

    // Function to unlike a comment
    function unlikeComment(uint256 tokenId, uint256 commentId) public {
        // Ensure the comment exists
        require(commentId < _tokenComments[tokenId].length, "CommentableERC721: Invalid comment ID");
        LikeType likeType = _commentLikes[tokenId][commentId][msg.sender];
        // Ensure the caller has already liked the comment
        require(likeType != LikeType.Unlike, "CommentableERC721: Comment Unliked");
        _tokenComments[tokenId][commentId].likeCount--;

        if(likeType == LikeType.Like) {
            _tokenComments[tokenId][commentId].likeCount--;
        }
        // Mark the comment as unliked by the caller
        _commentLikes[tokenId][commentId][msg.sender] = LikeType.Unlike;
    }

    // Function to get the total number of likes for a token
    function getTokenLikeCount(uint256 tokenId) public view returns (uint256) {
        return _tokenLikeCount[tokenId];
    }
}

import "./Vendor.sol";
import "./IsAdmin.sol";
import "./CommentableERC721.sol";
import "./IArtaNFTFactory.sol";

pragma solidity ^0.6.6;
pragma experimental ABIEncoderV2;

contract ArtaNFT is CommentableERC721, IsAdmin, Ownable {
    string _name;
    string _symbol;
    ...
    function mint(bytes32[] memory merkleProof) public payable {
        mint(_msgSender(), merkleProof);
    }

    function mint(address mintTo, bytes32[] memory merkleProof) public payable {
        require(
    ...
    function bulkMint(uint256 quantity, address mintTo, bytes32[] memory merkleProof) public payable {        
        require(
            quantity > 0,
            "quantity error"
        );
        for(uint256 i = 0; i < quantity; i++){
            mint(mintTo, merkleProof);
        }
    }
