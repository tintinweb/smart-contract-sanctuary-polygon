...
contract StrategyNewQuick is StratFeeManagerInitializable {
    ...
    address public constant quickRouter = 0xf5b509bB0909a69B1c207E495f687a596C168E12;
    ...
    function initialize(
        ...
        require(_nativeToWantRoute[0] == native, "toWant[0] != output");
        nativeToWantPath = AlgebraUtils.routeToPath(_nativeToWantRoute);
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        UniswapV3Utils.swap(unirouter, outputToNativePath, IERC20(output).balanceOf(address(this)));
    ...
    function swapNativeToWant() internal {
        if (want != native) {
            uint256 nativeBal = IERC20(native).balanceOf(address(this));
            AlgebraUtils.swap(quickRouter, nativeToWantPath, nativeBal);
        }
    }
    ...
    function outputToWant() public view returns (address[] memory) {
        return AlgebraUtils.pathToRoute(nativeToWantPath);
    }

    function _giveAllowances() internal {
        IERC20(want).safeApprove(rewardPool, type(uint).max);
        IERC20(output).safeApprove(unirouter, type(uint).max);
        IERC20(native).safeApprove(quickRouter, type(uint).max);
    }

    function _removeAllowances() internal {
        IERC20(want).safeApprove(rewardPool, 0);
        IERC20(output).safeApprove(unirouter, 0);
        IERC20(native).safeApprove(quickRouter, 0);
    }
