    ...
    EIP712Upgradeable
{
     ...
     * @param addrs The address array to set the scores for.
     * @param referralCodes The minter referral code array.
     * @param referrerCodes The referrer code array.
     * @notice This function is used for migration.
     * @notice Only the contract owner can call this function.
     */
    function migrateWithoutVerification(
        address[] calldata addrs,
        string[] calldata referralCodes,
        string[] calldata referrerCodes
    ) public onlyOwner migrationEnabled nonReentrant {
        require(
                addrs.length == referralCodes.length &&
                addrs.length == referrerCodes.length,
            "migrateWithoutVerification: Invalid input data"
        );

        for (uint256 i = 0; i < addrs.length; ++i) {
            migrateWithoutVerification(
                addrs[i],
                referralCodes[i],
                referrerCodes[i]
            );
    ...
    /**
     * @dev Migrates the scores data from the old contract wiyhout verification.
     * @param addr The address to set the scores for.
    ...
    function _migrate(
        ...
        uint256 deadline
    ) private whenNotPaused migrationEnabled nonReentrant {
        require(
    ...
    function migrate(
        ...
        string memory referralCode,
        string memory referrerCode
    ) public whenNotPaused migrationEnabled nonReentrant {
        require(
    ...
    function setScore(
        ...
        string memory referralCode,
        string memory referrerCode
    ) external payable whenNotPaused nonReentrant equalsFee(calculationModel, chainId) {
