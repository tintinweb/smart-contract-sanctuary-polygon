...
contract MaiVaultFront is BaseSiloAction {
    ...
    function enter(
        ...
        uint256 vaultId;
        {
            ...
            (input, output, maiVault, ratioRange, position) = abi.decode(
                configuration,
                (address[4], address[4], address, uint256[3], uint256)
            );
            //carry through 1,2
            if (position == 1) {
            ...
            outputAmounts[position] = _takeFee(
                implementation,
                outputAmounts[position],
                output[position]
            );
        }
        ...
        //repay debt
        if (inputAmounts[3] > 0) {
            if (inputAmounts[3] > vault.vaultDebt(vaultId)) {
                //just repay the debt
        ...
        if (debt > 0) {
            uint256 CDratio = ((10**miMatic.decimals()) * collateralValue) /
                debt; //10**18 is to match zeroes in ratioRange
            ...
            //borrow initial loan
            uint256 idealDebt = ((10**miMatic.decimals()) * collateralValue) /
                ratioRange[1];
    ...
    function exit(
        ...
        if (
            ...
            outputAmounts[3] = 2**256 - 1; //ask next action to remove ALL stake
        } else {
            //check the Collateral to Debt Ratio
            IPriceFeed oracle = IPriceFeed(vault.ethPriceSource());
            uint256 collateralValue = ((vault.vaultCollateral(vaultId) -
            ...
            if (idealDebt > debt) {
                ...
                outputAmounts[0] = idealDebt - debt;
                if (outputAmounts[0] <= vault.getDebtCeiling()) {
        ...
        returns (bool)
    {
        ...
        if (storedConfig.length > 0) {
            ...
            (, , maiVault, ratioRange) = abi.decode(
                storedConfig,
                (address[4], address[4], address, uint256[3])
            );
        } else {
            (, , maiVault, ratioRange) = abi.decode(
                configuration,
        ...
        returns (ActionBalance memory)
    {
        ...
        uint256 vaultId;
        address[4] memory output;
        (input, output, maiVault, , ) = abi.decode(
            _configurationData,
            (address[4], address[4], address, uint256[3], uint256)
        );
