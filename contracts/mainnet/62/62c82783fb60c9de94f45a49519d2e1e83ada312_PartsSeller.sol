...
contract PartsSeller is WarpBase {
    using EnumerableSetUpgradeable for EnumerableSetUpgradeable.AddressSet;
    using SafeERC20Upgradeable for IERC20;

    //** ====== Events ====== *//
    event PartSold(address to, address principle, uint256 value);
    ...
    mapping(address => uint256) principleMultiplier;

    /** ====== Initialize ====== */
    function initialize(
        address _fund,
        address _treasury,
        address _starshipPartsControl,
        address _warpUtils,
        address _warp
    ) public initializer {
        __WarpBase_init();
        ...
        fund = _fund;
        warpUtils = _warpUtils;
        treasury = _treasury;
        starshipPartsControl = _starshipPartsControl;
        growthPct = 10;
        warp = _warp;
    }
    ...
    function buyPart(
        ...
        // Get value from treasury
        uint256 value = ITreasury(treasury).valueOf(_principle, _amount);

        // Transfer from sender, then deposit to treasury and sendout grwoth fee
        IERC20(_principle).safeTransferFrom(msg.sender, address(this), _amount);

        // Burn the warp or deposit
        if (_principle == warp) {
            IERC20Burnable(warp).burn(_amount);
        } else {
            IERC20(_principle).approve(address(treasury), _amount);
            ITreasury(treasury).deposit(_amount, _principle, value);
            ITreasury(treasury).withdrawGrowthFee(_amount / growthPct, _principle);
        }

        if (_part != PartType.FUEL) {
            // Mint a repair kit to msg sender (0 = repair kit index);
            IWarpUtilities(warpUtils).mint(0, 1, msg.sender);
        }

        // Build part
        uint256 actualValue = (value * principleMultiplier[_principle]) / 100;
        IStarshipPartsControl(starshipPartsControl).buildSpecificPart(
            msg.sender,
            actualValue,
            _part
        );
        emit PartSold(msg.sender, _principle, value);
    }

    /** @dev set allowed principle
        @param _principle {address}
        @param _allowed {bool}
        @param _multiplier {uint256}
     */
    function setAllowedPrinciples(
        address _principle,
        bool _allowed,
        uint256 _multiplier
    ) external onlyOwner {
        if (_allowed) {
            ...
            principleMultiplier[_principle] = _multiplier;
        } else principles.remove(_principle);
    }

    /** @dev View function to get principles for dApp */
    function getPrinciples() external view returns (Principle[] memory) {
        Principle[] memory _items = new Principle[](principles.length());

        uint256 count = 0;
        for (uint256 i = 0; i < principles.length(); i++) {
            _items[count] = Principle({
                token: principles.at(i),
                multiplier: principleMultiplier[principles.at(i)]
            });
            count++;
        }
