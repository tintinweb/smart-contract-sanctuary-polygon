...
contract Reward is Initializable, AccessControlUpgradeable, UUPSUpgradeable {
   ...
   */
  function initialize(address tokenAddress, address vehicleIdProxyAddress, address sanctionsContractAddress) external initializer {
    ...
    require(vehicleIdProxyAddress != address(0), "vehicleNftAddress is an invalid zero address");
  ...
  function batchTransfer(address[] calldata users, uint256[] calldata values, uint256[] calldata vehicleIds) external onlyRole(ORACLE_ROLE) {
    ...
    for (uint256 i = 0; i < arrayLength; i++) {
      vehicleOwner = vehicleIdProxy.ownerOf(vehicleIds[i]);
