...
abstract contract ERC20Bannable is ERC20, Ownable {
    ...
    function lockUser(address user) external onlyOwner {
        require(!isUserLocked[user], 'already locked');
        isUserLocked[user] = true;
    }

    function unlockUser(address user) external onlyOwner {
        require(isUserLocked[user], 'already unlocked');
    ...
    function _beforeTokenTransfer(
        ...
        require(!isUserLocked[from], 'user locked');
    }
}
...
contract GOAT is ERC20Bannable {
    constructor(
        string memory name,
