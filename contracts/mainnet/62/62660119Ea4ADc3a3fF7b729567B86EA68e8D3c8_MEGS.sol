...
contract MEGS is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20("Meta Game Swap Token", "MEGS") {
    ...
    function getLockedPerAgreement(address walletAddress, uint index) public view returns (uint256) {
        ...
        if (block.timestamp > currentVest.vestStart) {
            if (((block.timestamp - currentVest.vestStart) / 86400) <= currentVest.vestPeriod) {
                unLockedAmount = currentVest.totalAmount * ((block.timestamp - currentVest.vestStart) / 86400) / currentVest.vestPeriod;
