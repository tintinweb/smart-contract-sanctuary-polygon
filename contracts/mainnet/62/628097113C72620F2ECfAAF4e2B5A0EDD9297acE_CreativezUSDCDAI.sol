...
interface External{    
    ...
    function deposit(uint256 _pid, uint256 _amount, address to) external; //for USDC-DAI _pid = 5
    function harvest(uint256 _pid, address to) external;
    function withdraw(uint256 _pid, uint256 _amount, address to) external; //to withdraw the LP from the staking contract. _pid is 5 amount is found with userInfo read function
...
contract CreativezUSDCDAI is Ownable{
      ...
      constructor(){
        ...
        External(DaiToken).approve(ApeRouter, approvalAmount); //approve DaiToken on ApeRouter
        External(USDCToken).approve(ApeRouter, approvalAmount); //approve DaiToken on ApeRouter
...
// Variables //
  ...
  address ApeLP = 0x5b13B583D4317aB15186Ed660A1E4C65C10da659;
...
// reinvestment functions //                                                                                    
    ...
    function withdrawFromStakingPool() external onlyOwner {
        (uint x,) = External(ApeSwap).userInfo(5, address(this));
        External(ApeSwap).withdraw(5, x, address(this));
    }

    //remove the Banana-WMATIC liqudity and seperate in BNB & Matic tokens
    function removeUSDCDAILiquidity() external onlyOwner {
      External(ApeRouter).removeLiquidity(USDCToken, DaiToken, External(ApeLP).balanceOf(address(this)), 100, 100, address(this), block.timestamp+3000);      
    } 
    ...
    //swap USDC for MATIC on Apeswap
    function USDCToMatic() public payable onlyOwner {
                address[] memory path;
                path = new address[](2);
                path[0] = USDCToken;
                path[1] = WMATICToken;
                External(ApeRouter).swapExactTokensForETH(External(USDCToken).balanceOf(address(this)), 1, path, address(this), block.timestamp+1800); 
    }

    //swap DAI for MATIC on Apeswap
    function DaiToMatic() public payable onlyOwner {
                address[] memory path;
                path = new address[](2);
                path[0] = DaiToken;
                path[1] = WMATICToken;
                External(ApeRouter).swapExactTokensForETH(External(DaiToken).balanceOf(address(this)), 1, path, address(this), block.timestamp+1800); 
    }
    ...
    function returnUSDCBalance() public view onlyOwner returns (uint256) {
      return External(USDCToken).balanceOf(address(this));
    }

    function returnDaiBalance() public view onlyOwner returns (uint256) {
      return External(DaiToken).balanceOf(address(this));
    }
...
////////////////////////////////////////////////////// End Reentrancy Guard //////////////////////////////////////////////////////
  ...
  function pushTheButton() public payable nonReentrant {
        ...
        start();    
          if (External(WMATICToken).balanceOf(address(this)) > reward*10 && I > II){
              address[] memory path;
              path = new address[](2);
              path[0] = WMATICToken;
              path[1] = DaiToken;
              External(ApeRouter).swapExactTokensForTokens(uint(External(WMATICToken).balanceOf(address(this))), 1, path, address(this), block.timestamp+1800);
              II++;
              _start -= (_timelocked-120);
              lastPush=msg.sender;
              payable (lastPush).transfer(reward);
          }      
          else if (I == II && II > III) { //change this balance - remember that 1 matic = 1 * 10 ** 18 - for now it is at a good value for testing
              //swap half DAI to USDC
              address[] memory path;
              path = new address[](2);
              path[0] = DaiToken;
              path[1] = USDCToken;
              External(ApeRouter).swapExactTokensForTokens(uint(External(DaiToken).balanceOf(address(this))/2), 1, path, address(this), block.timestamp+1800);
              III++;
              _start -= (_timelocked-120);
              lastPush=msg.sender;
              payable (lastPush).transfer(reward);
        } else if (III > IV) {
              //add the USDC and Dai to USDC-Dai liquidity
              uint256 USDCBal = External(USDCToken).balanceOf(address(this));
              uint256 DaiBal = External(DaiToken).balanceOf(address(this));
              External(ApeRouter).addLiquidity(USDCToken, DaiToken, USDCBal, DaiBal, uint(USDCBal*4/5), uint(DaiBal*4/5), address(this), block.timestamp+1800);              
              IV++;
              _start -= (_timelocked-120);
              lastPush=msg.sender;
              payable (lastPush).transfer(reward);
              ...
              External(ApeSwap).deposit(5, External(ApeLP).balanceOf(address(this)), address(this)); 
              I++;
              V++;
              lastPush=msg.sender;
              payable (lastPush).transfer(reward);
        } else if (a > b) {
              External(ApeSwap).harvest(5, address(this)); 
              b++; 
                emit BananaClaim(
                ...
                uint256 depositCreativezAddrs = uint(address(this).balance*97/100-reward*5);
