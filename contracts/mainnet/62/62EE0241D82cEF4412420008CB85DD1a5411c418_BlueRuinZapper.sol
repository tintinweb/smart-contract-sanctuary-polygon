...
contract BlueRuinZapper is Owned, TokensRecoverable, ReentrancyGuard
{
    ...
    address public dMagicLPAddress; // LP address of dMagic-axMatic
    ...
    constructor(IAxMatic _axMatic, IERC20 _dmagic, address _LPAddress, IERC20 _NightBane, IaxMatic_Direct _axMaticDirect, IDragonZapper _dragonDirect, IERC31337 _LPNFTWrapper, IERC1155 _ERC1155Token, uint _erc1155TokenID, address _dMagicLPAddress)
    {
        axMatic = _axMatic;
    ...
    function estimateBuyLPFromMatic(uint256 _matic_amount) external view returns(uint256){
       ...
       // liquidity = Math.min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1);
        uint256 amt1NBane = maticToBuy.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve0DM.add(maticToBuy));
        uint256 amt2NBane = dMagicAmt.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve1DM.sub(dMagicAmt));

        uint nBaneAmt;
        if(amt1NBane>amt2NBane) nBaneAmt = amt2NBane;
        else nBaneAmt = amt1NBane;
        ...
        uint256 amt2 = dMagicAmtForLP.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0);
        uint256 amt1 = nBaneAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1);
    ...
    function estimateBuyLPFromDmagic(uint256 _dmagic_amount) external view returns(uint256){
        ...
        uint256 amt1NBane = maticAmount.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve0DM.sub(maticAmount));
        uint256 amt2NBane = dMagicToBuy.mul(IUniswapV2Pair(dMagicLPAddress).totalSupply()).div(reserve1DM.add(dMagicToBuy));
        ...
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 
        uint256 amt2 = dMagicToBuy.mul(2).mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0);
        uint256 amt1 = nBaneAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1);
    ...
    function estimateBuyLPFromNight(uint256 _night_amount) external view returns(uint256){
        uint256 nightToBuy = _night_amount.div(2);
        (uint256 reserve0, uint256 reserve1,) = IUniswapV2Pair(LPAddress).getReserves(); 

        address[] memory path = new address[](2);
        path[0] = address(NightBane);
        path[1] = address(dmagic);
        (uint256[] memory amounts) = uniswapV2Router.getAmountsOut(nightToBuy, path);
        uint256 dMagicAmt = amounts[1];

        uint256 amt2 = dMagicAmt.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve0.sub(dMagicAmt));
        uint256 amt1 = nightToBuy.mul(IUniswapV2Pair(LPAddress).totalSupply()).div(reserve1.add(nightToBuy));
