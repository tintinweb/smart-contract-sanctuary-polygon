...
contract GameOwner is Ownable {
    mapping(address => bool) public gameRoles;

    function addGameRole(address _game) public onlyOwner {
        gameRoles[_game] = true;
    }

    function gameAddGameRole(address _game) public onlyGame {
        gameRoles[_game] = true;
    }

    function gameRemoveGameRole(address _game) public onlyGame {
    ...
    modifier onlyGame() {
        require(gameRoles[_msgSender()] == true, "You are not the game");
        _;
    }
}
...
/**
 * Data Storage contract for Bumpkin NFTs
 * Refer to metadata endpoint for licence details
 */
contract Bumpkin is
    ERC721Enumerable,
    ERC721Pausable,
    ...
    GameOwner
{
    ...
    function setBaseURI(string memory uri) external onlyGame {
        baseURI = uri;
    }
    ...
    function gameMint(address account) external onlyGame returns (uint256) {
        uint256 tokenId = ++id;
    ...
    function gameApprove(address to, uint256 tokenId) external onlyGame {
        _approve(to, tokenId);
    }
    ...
    function pause() external onlyGame {
        _pause();
    }

    function unpause() external onlyGame {
        _unpause();
    }
    ...
    function deleteDefaultRoyalty() external onlyGame {
