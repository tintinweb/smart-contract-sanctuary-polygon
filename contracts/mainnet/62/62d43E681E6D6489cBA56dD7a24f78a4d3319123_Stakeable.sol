...
interface tokenI {
        function proxyMint(address _token, address _to, uint256 _amount) external;
    }
...
contract Stakeable is IERC721Receiver, Ownable, ReentrancyGuard {
    ...
    address public proxyAddress = 0x8E664FCc6A2fC0DAD067B73FbCf37aAB636dD212;
    address public erc721Contract = 0xd9C1c47c3Ccf1B2c882dEA3C28b976aF7727149A;
    ...
    function setRentalFee(uint256 _tokenID, uint _fee) public {
        require(nftByID[_tokenID].user == msg.sender, "That NFT does not belong to you.");
        nftByID[_tokenID].rentalFee = _fee;
    }

    function hireToken(uint256 _tokenID) public {
        require(nftByID[_tokenID].rentalFee > 0, "That NFT does not appear to be available.");
        uint _fee = nftByID[_tokenID].rentalFee;
        require(rewardToken.transferFrom(msg.sender, address(this), _fee), "You need to pay to use this NFT.");
        nftByID[_tokenID].coffer = nftByID[_tokenID].coffer.add(_fee);
    }
    ...
    function receiveReward(uint256 _tokenID) nonReentrant public {
        ...
        if (rewardToken.balanceOf(address(this)) <= earned) {
            tokenI(proxyAddress).proxyMint(tokenAddress, address(this), 50000000000000000000000);
        }
