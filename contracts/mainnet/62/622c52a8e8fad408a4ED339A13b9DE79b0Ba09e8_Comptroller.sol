  ...
  ExponentialNoError
{
  ...
  function exitMarket(address oTokenAddress) external returns (uint) {
    ...
    (uint oErr, uint tokensHeld, uint amountOwed, ) = oToken.getAccountSnapshot(
      msg.sender
    );
  ...
  function liquidateBorrowAllowed(
    ...
    if (
      !markets[oTokenBorrowed].isListed || !markets[oTokenCollateral].isListed
    ) {
      return uint(Error.MARKET_NOT_LISTED);
    }
  ...
  function seizeAllowed(
    ...
    if (
      !markets[oTokenCollateral].isListed || !markets[oTokenBorrowed].isListed
    ) {
      return uint(Error.MARKET_NOT_LISTED);
    }
  ...
  function getAccountLiquidity(
    ...
    (
      ...
      uint liquidity,
      uint shortfall
    ) = getHypotheticalAccountLiquidityInternal(account, OToken(0), 0, 0);
  ...
  function _setCollateralFactor(
    ...
    if (
      newCollateralFactorMantissa != 0 && oracle.getUnderlyingPrice(oToken) == 0
    ) {
      return
        fail(
          Error.PRICE_ERROR,
