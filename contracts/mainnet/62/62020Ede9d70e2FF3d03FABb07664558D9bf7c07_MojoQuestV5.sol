...
contract MojoQuestV5 is Initializable, UUPSUpgradeable, OwnableUpgradeable, ReentrancyGuardUpgradeable, VRFConsumerBaseUpgradeable{
    ...
    // owner function to manually adjust if necessary
    function adjustBoost(uint256 tokenId, address _contract, uint8 level) external onlyOwner{
        mojoBoost[tokenId][_contract] = level;
    }

    function adjustLocation(uint8 locationId, uint8 mojo, uint8 cost, uint8[] calldata rewards, uint16[] calldata chance) external onlyOwner{
        locations[locationId] = Location({minMojo: mojo, hghCost: cost, reward: [rewards[0], rewards[1], rewards[2], rewards[3]], rewardChance: [chance[0], chance[1], chance[2], chance[3]]});
    }
    ...
    function sendToQuest(uint256 _locationId, uint256 _tokenOne, address _tokenOneContract, uint256 _tokenTwo, address _tokenTwoContract) external nonReentrant{
        ...
        if(_tokenOneContract == eclAddress){
            require(_tokenOne != _tokenTwo, "Can't send the same ECL questing with themselves");
        }
    ...
    function getWinner(uint256 currentQuest, uint256 seed) internal view returns (uint8){
        uint16 rollOne = uint16(_rand(currentQuest, seed) % IMaticMike(questToParticipants[currentQuest][0]._contract).getPowerLevel(questToParticipants[currentQuest][0].tokenId));
        uint16 rollTwo = uint16(_rand(currentQuest+1, seed) % IMaticMike(questToParticipants[currentQuest][1]._contract).getPowerLevel(questToParticipants[currentQuest][1].tokenId));
    ...
    function getQuestInfo(uint256 tokenId, address tokenContract) public view returns(uint8, uint256){
        uint256 timeRemaining = block.timestamp - tokenToTimer[tokenId][tokenContract];
        if(timeRemaining == block.timestamp){
            timeRemaining = 0;
        }

        if(tokenContract == eclAddress){
            return(eclQuestsPerDay - questsLeft(tokenId,tokenContract), timeRemaining);
        }
        else{
            return(mmQuestsPerDay - questsLeft(tokenId, tokenContract), timeRemaining);
        }
    }
...
interface IHgh is IERC20 {
    ...
    function expansionGetTokensStaked(address expansion, address staker) external view returns(uint256[] memory);
}
