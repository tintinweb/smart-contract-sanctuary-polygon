...
import { IPonder } from "../Interfaces/IPonder.sol";
...
contract GenericSwapFacet is IPonder, ReentrancyGuard, SwapperV2, Validatable {
    /// External Methods ///
    ...
    function swapTokensGeneric(
        ...
        emit PonderGenericSwapCompleted(
            _transactionId,
            _integrator,
            _referrer,
            _receiver,
...
interface IPonder {
    ...
    event PonderTransferStarted(IPonder.BridgeData bridgeData);

    event PonderTransferCompleted(
    ...
    event PonderGenericSwapCompleted(
        bytes32 indexed transactionId,
        string integrator,
        string referrer,
        address receiver,
...
abstract contract ReentrancyGuard {
    ...
    bytes32 private constant NAMESPACE =
        keccak256("com.Ponder.reentrancyguard");
...
contract Validatable {
    modifier validateBridgeData(IPonder.BridgeData memory _bridgeData) {
    ...
    modifier noNativeAsset(IPonder.BridgeData memory _bridgeData) {
    ...
    modifier onlyAllowSourceToken(
        IPonder.BridgeData memory _bridgeData,
        address _token
    ) {
    ...
    modifier containsSourceSwaps(IPonder.BridgeData memory _bridgeData) {
    ...
    modifier doesNotContainSourceSwaps(IPonder.BridgeData memory _bridgeData) {
        if (_bridgeData.hasSourceSwaps) {
            revert InformationMismatch();
        }
        _;
...
library LibAllowList {
    ...
    bytes32 internal constant NAMESPACE =
        keccak256("com.Ponder.library.allow.list");
