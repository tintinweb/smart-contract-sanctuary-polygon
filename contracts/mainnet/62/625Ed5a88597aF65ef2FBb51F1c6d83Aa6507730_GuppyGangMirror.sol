...
import "../lib/ERC721/ERC721Mirror.sol";

contract GuppyGangMirror is ERC721Mirror {
    constructor() ERC721Mirror("Guppy Gang (Mirror)", "GUPPY") {}
}
...
/**
 * @title ERC721Mirror
 * @dev ERC721 contract with several extensions implemented to allow for mirroring cross-chain
 * @author phatmo.eth
 * @custom:version 1.0
 * @custom:date 18 July 2022
 */
abstract contract ERC721Mirror is
    AccessControl,
    Ownable,
    ERC721,
    ERC721Enumerable,
    ...
    ERC721TimeTrackable
{
    ...
    function flipLock(uint256 tokenId) external onlyRole(STAFF_ROLE) {
        _flipLock(tokenId);
    }
    ...
    function flipAddressLock(address _address) external onlyRole(STAFF_ROLE) {
    ...
    function safeMint(address to, uint256 tokenId) external onlyRole(MINT_ROLE) {
    ...
    function _beforeTokenTransfer(
        ...
        address to,
        uint256 tokenId
    ) internal override(ERC721, ERC721Enumerable, ERC721TimeTrackable) {
        if (!hasRole(TRANSFER_ROLE, to)) {
