    ...
    ICollateralManager
{
    ...
    /**
        ...
        @param _wrappedNativeCoin the wrapped version of native coin
    */
    function initialize(
        address _owner,
        ...
        IWrappedNativeCoin _wrappedNativeCoin
        ...
        personalPools[TreasuryAddress] = payable(new PersonalPool());
        personalPools[_InsuranceFundAddress] = payable(new PersonalPool());
    ...
    */
    ...
    function postNative() external payable {
        ...
        address personalPool = _getOrCreatePersonalPool(msg.sender);
        require(wrappedNativeCoin.transfer(personalPool, msg.value));
    ...
    */
    function withdraw(
        ...
        require(
            !isNative || (_basisAddress == address(wrappedNativeCoin)),
            "C40"
        );
    ...
    */
    function allocateCollateral(uint256 _toStrategyID, uint256 _amount)
        external
    {
        _allocateCollateralUser(msg.sender, _toStrategyID, _amount, (TFM(TFMAddress).getStrategy(_toStrategyID)).basis);
    }
    ...
    function _autoCollateraliseStrategy(
        ...
        require(
            unallocatedCollateral[_user][_basis] +
                ...
                _requiredCollateral,
            "C15" // "_user has not deposited enough to cover collateral"
        );
    ...
    */
    function combineExecute(CombineRequest memory _req) external isTFM {
        ...
        lockParticleMass(_req.initiator, _req.basis, _req.particleMass);

        // Lock particle mass for counterparty.
        lockParticleMass(_req.sender, _req.basis, _req.particleMass);

        // Move all allocated collateral of both parties to thisStrategy as tarstrategies will be removed.
        reallocateAllNoCollateralCheck(
            _req.sender,
            _req.tarstrategiesID,
            _req.thisStrategyID,
            _req.basis
        );
        reallocateAllNoCollateralCheck(
    ...
    */
    function peppermintExecute(PeppermintRequest memory _req) external isTFM {
        ...
        _autoCollateraliseStrategy(
            _req.alpha,
            _req.strategyID,
            alphaCollateralRequirement - premiumAlpha,
            _req.basis
        );
        _autoCollateraliseStrategy(
            _req.omega,
            _req.strategyID,
            omegaCollateralRequirement - premiumOmega,
            _req.basis
        );
...
import {Utils} from "../libraries/Utils.sol"; // @review: all functions are public, turning a few to internal can actually improve code size
    ...
    ITFM
{
    ...
    event ReallocatedCollateral(uint256 _fromStrategyID, uint256 _toStrategyID);
    ...
    event Pepperminted(uint256 _strategyID);
    event Transferred(
        uint256 _thisStrategyID,
        address _targetUser,
        int256 _premium
    );
    event Combined(uint256 _thisStrategyID, uint256 _tarstrategiesID);
    event Novated(uint256 _thisStrategyID, uint256 _tarstrategiesID);
    event Exercised(uint256 _strategyID);
    event Liquidated(uint256 _strategyID);
    event Annihilated(uint256 _strategyID);
    ...
    */
    ...
    /**
        ...
        @return Strategy struct corresponding to _strategyID.
    */
    /* function strategies(uint256 _strategyID)
        external
        view
        returns (Strategy memory)
    {
    ...
    */
    function reallocateCollateral(
        ...
        emit ReallocatedCollateral(_fromStrategyID, _toStrategyID);
    }
    ...
    /**
        ...
        @param _collateralParams the collateral parameters of the strategy to be minted (SpearmintCollateralParams)
    ...
    */
    ...
    function _checkAndCreateStrategy(
        CollateralParamsFull calldata _collateralParams,
        bytes calldata _signature,
        address _alpha,
        address _omega,
    ...
    */
    ...
    /**
        ...
        @param _params is the input struct for teh function, containing:
            ...
            tarstrategiesID the ID of the other strategy to combine with (this strategy 
            will be deleted)
    */
    function combine(CombineParams calldata _params) external {
        Strategy storage thisStrategy = strategies[_params.thisStrategyID];
        Strategy storage tarstrategies = strategies[_params.tarstrategiesID];
        ...
        address targetOmega = tarstrategies.omega;
        //check both alpha & omega have signed, return who signed first
        address initiator = Utils.checkCombineSignatures(
            ...
            thisOmega,
            strategyNonce[_params.thisStrategyID],
            strategyNonce[_params.tarstrategiesID],
            collateralManager.collateralNonce(),
            tarstrategies.alpha,
            targetOmega
        )
        ...
        address counterparty = initiator == thisAlpha ? thisOmega : thisAlpha;
        ...
        delete strategies[_params.tarstrategiesID]; // Delete tarstrategies.
        delete strategyNonce[_params.tarstrategiesID]; //Delete tarstrategies Nonce

        emit Combined(_params.thisStrategyID, _params.tarstrategiesID);
    }
    ...
    */
    function novate(NovateParams calldata _params) external {
        ...
        require(Utils.wavefnEq(thisStrategy.phase, tarstrategies.phase), "S35"); // "strategies are not compatible"
        ...
        {
            if (
                (thisStrategy.amplitude < 0 && tarstrategies.amplitude < 0) ||
                (thisStrategy.amplitude > 0 && tarstrategies.amplitude > 0)
            ) {
                // Initiator has to be - (thisStrategy.alpha == tarstrategies.omega) || (thisStrategy.omega == tarstrategies.alpha)
                require(
                    (thisStrategy.alpha == tarstrategies.omega) ||
                        (thisStrategy.omega == tarstrategies.alpha),
                    "S36 - A"
                ); // Novation: Initiator invalid for when amplitude product is less than 0
                initiator = (thisStrategy.alpha == tarstrategies.omega)
                    ? thisStrategy.alpha
                    : thisStrategy.omega;
            } else if (
                (thisStrategy.amplitude < 0 && tarstrategies.amplitude > 0) ||
                ...
                require(
                    (thisStrategy.alpha == tarstrategies.alpha) ||
                        (thisStrategy.omega == tarstrategies.omega),
                    "S36 - B"
                ); // Novation: Initiator invalid for when amplitude product is greater than 0
                initiator = (thisStrategy.alpha == tarstrategies.alpha)
                    ? thisStrategy.alpha
                    : tarstrategies.omega;
            }
        }
        ...
        emit Novated(_params.thisStrategyID, _params.tarstrategiesID);
    }
    ...
    function novateFinalise(
        ...
        Strategy storage thisStrategy = strategies[thisStrategyID];
        Strategy storage tarstrategies = strategies[tarstrategiesID];

        // Update strategy nonces
        {
            strategyNonce[thisStrategyID]++;
            strategyNonce[tarstrategiesID]++;
        }

        int256 thisAmplitude = int256((thisStrategy.amplitude).abs());
        int256 targetAmplitude = int256((tarstrategies.amplitude).abs());
        address thisStrategyAlpha = thisStrategy.alpha;
        address tarstrategiesAlpha = tarstrategies.alpha;
        address thisStrategyOmega = thisStrategy.omega;
        address tarstrategiesOmega = tarstrategies.omega;

        bool newTransferable = thisStrategy.transferable ||
            tarstrategies.transferable;
        ...
        if (thisAmplitude < targetAmplitude) {
            ...
            collateralManager.reallocatePortionNoCollateralCheck(
                (initiator == tarstrategiesAlpha)
                    ? tarstrategiesOmega
                    : tarstrategiesAlpha, // Corresponds to person who is not the initiator in tarstrategies
                tarstrategiesID,
                thisStrategyID,
                uint256(thisAmplitude),
                uint256(targetAmplitude),
                thisStrategy.basis
            );
            // Dellocate initiator's collateral from thisStrategy.
            collateralManager.reallocateAllNoCollateralCheck(
                initiator, // Corresponds to B.
                thisStrategyID, // Corresponds to AC.
                0,
                thisStrategy.basis
            );
            ...
            if (initiator == thisStrategyAlpha) {
                thisStrategy.alpha = (initiator == tarstrategiesAlpha)
                    ? tarstrategiesOmega
                    : tarstrategiesAlpha;
            } else {
                thisStrategy.omega = (initiator == tarstrategiesAlpha)
                    ? tarstrategiesOmega
                    : tarstrategiesAlpha;
            }

            targetAmplitude = targetAmplitude - thisAmplitude;
            tarstrategies.amplitude = (tarstrategies.amplitude < 0)
                ? (-targetAmplitude)
                : (targetAmplitude);
            ...
            collateralManager.reallocatePortionNoCollateralCheck(
                (initiator == thisStrategyOmega)
                    ? thisStrategyAlpha
                    : thisStrategyOmega, // Corresponds to A.
                thisStrategyID, // Corresponds to AB.
                tarstrategiesID, // Corresponds to AC.
                uint256(targetAmplitude),
                uint256(thisAmplitude),
                thisStrategy.basis
            );
            ...
            // Update the params of the strategies.
            tarstrategies.transferable = newTransferable;
            if (initiator == tarstrategiesOmega) {
                tarstrategies.omega = (initiator == thisStrategyOmega)
                    ? thisStrategyAlpha
                    : thisStrategyOmega;
            } else {
                tarstrategies.alpha = (initiator == thisStrategyOmega)
                    ? thisStrategyAlpha
                    : thisStrategyOmega;
            }
            ...
            thisStrategy.amplitude = (thisStrategy.amplitude < 0)
                ? (-thisAmplitude)
                : (thisAmplitude);
        } else if (Utils.abs(thisAmplitude) == Utils.abs(targetAmplitude)) {
            ...
            collateralManager.reallocateAllNoCollateralCheck(
                initiator, // Corresponds to B.
                tarstrategiesID, // Corresponds to deleted strategy.
                0,
                thisStrategy.basis
            );
            ...
            collateralManager.reallocateAllNoCollateralCheck(
                (initiator == tarstrategiesAlpha)
                    ? tarstrategiesOmega
                    : tarstrategiesAlpha, // Corresponds to C.
                tarstrategiesID, // Corresponds to deleted strategy.
                thisStrategyID, // Corresponds to AC.
                thisStrategy.basis
            );
            ...
            thisStrategy.transferable = newTransferable;
            if (initiator == thisStrategyAlpha) {
                thisStrategy.alpha = (initiator == tarstrategiesOmega)
                    ? tarstrategiesAlpha
                    : tarstrategiesOmega;
            } else {
                thisStrategy.omega = (initiator == tarstrategiesOmega)
                    ? tarstrategiesAlpha
                    : tarstrategiesOmega;
            }
            delete strategies[tarstrategiesID]; // Delete original BC strategy.
            delete strategyNonce[tarstrategiesID]; // Delete strategyNonce for strategy BC
        }
    }
    ...
    */
    function exercise(
        ...
        emit Exercised(_paramsID.strategyID);
    }
    ...
    */
    function liquidate(
        ...
        //for this function we need a strict equivalent, because we should use LAST collateral requirements;
        ...
        bool omegaAmountPositive = _lParams._transferredCollateralOmega +
            ...
            0;
        /// @review move the utils check and the require checks in later stages to library

        require(
            collateralNonce == _collateralParams.collateralNonce,
        ...
        strategy.amplitude = _collateralParams.amplitude;
        ...
        emit Liquidated(_lParams._strategyID);
    }
}
...
//WETH, WMATIC, WAVAX has same interface (for needed functions)
interface IWrappedNativeCoin {
    /**
     * @notice wrap deposited native coin into WNativeCoin 
     */
    function deposit() external payable;

    /**
     * @notice withdraw native coin from contract
     * @dev Unwrap from WNativeCoin into native coint
     * @param wad amount WNativeCoin to unwrap and withdraw
     */
    function withdraw(uint256 wad) external;

    /**
     * @notice Returns the WNativeCoint balance of @param account
     */
    function balanceOf(address account) external view returns (uint256);
     ...
     * @param wad amount of WNativeCoin
     * @return True if tx succeeds, False if not
     */
    function approve(address guy, uint256 wad) external returns (bool);

    /**
     * @notice transfer WNativeCoint
...
library Utils {
    ...
    */
    ...
    function checkWeb2Signature(
        ...
        bytes32 msgHash = keccak256(onlyCollateralParams ? 
            abi.encode(
                _collateralParams.expiry,
                _collateralParams.alphaCollateralRequirement,
                _collateralParams.omegaCollateralRequirement,
    ...
    /**
        ...
        @param _tarstrategiesNonce nonce for tarstrategies i.e., second strategy
        produce _signature, false in any other case
    */
    function checkNovationSignatures(
        NovateParams calldata _params,
        Strategy calldata thisStrategy,
        Strategy calldata tarstrategies,
        uint256 _thisStrategyNonce,
        uint256 _tarstrategiesNonce,
        uint256 _collateralNonce
    ) external view {
        address _initiator;
        bool _thisStrategyTransferable = thisStrategy.transferable;
        bool _tarstrategiesTransferable = tarstrategies.transferable;
        {
            if (
                (thisStrategy.amplitude < 0 && tarstrategies.amplitude < 0) ||
                (thisStrategy.amplitude > 0 && tarstrategies.amplitude > 0)
                ...
                _initiator = (thisStrategy.alpha == tarstrategies.omega)
                    ? thisStrategy.alpha
                    : thisStrategy.omega;
            } else if (
                (thisStrategy.amplitude < 0 && tarstrategies.amplitude > 0) ||
                ...
                _initiator = (thisStrategy.alpha == tarstrategies.alpha)
                    ? thisStrategy.alpha
                    : tarstrategies.omega;
            }
        }

        address _thisStrategyNotInitiator = (_initiator == thisStrategy.alpha)
            ? thisStrategy.omega
            : thisStrategy.alpha;
        address _tarstrategiesNotInitiator = (_initiator ==
            tarstrategies.alpha)
            ? tarstrategies.omega
            : tarstrategies.alpha;
        ...
        require(
            _params.thisStrategyNonce == _thisStrategyNonce &&
                _params.tarstrategiesNonce == _tarstrategiesNonce,
        ...
        if (_thisStrategyTransferable && !_tarstrategiesTransferable) {
            require(
                isValidSigner(
                    calculatedHash,
                    _params.sig3,
                    _tarstrategiesNotInitiator
                ),
                "A29-a"
        ...
        else if (!_thisStrategyTransferable && _tarstrategiesTransferable) {
            require(
                isValidSigner(
                    calculatedHash,
                    _params.sig2,
    ...
    function checkTransferUserSignaturesAndParams(
        ...
        require(expiry > block.timestamp, "S1"); // "strategy must be active"
    ...
    /**
        ...
        @param thisStrategy the ID of one of the strategies to compare
        @param tarstrategies the ID of the other strategy to compare against
    */
    function strategiesCompatible(
        ...
        require(
            thisStrategy.bra == tarstrategies.bra &&
                thisStrategy.ket == tarstrategies.ket &&
                thisStrategy.basis == tarstrategies.basis,
            "S33" // "strategies must have the same bra, ket, and basis"
        );

        /// @review move to library
        require(
            thisStrategy.expiry == tarstrategies.expiry,
    ...
    */
    function checkCollateralRequirements(
        ...
        checkWeb2Signature(collateralParams, strategy, _signature, collateralNonce, web2Address, false);
    }
}
