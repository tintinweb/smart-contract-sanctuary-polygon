    ...
    Helper
{
    ...
    function updateProposalAndExecution(
        address _contract,
        bytes memory _data
    ) external onlyGnosisOrDao(factoryAddress, address(this)) nonReentrant {
        require(
            _contract == address(this) || _contract == factoryAddress,
            "Invalid address"
        );
    ...
    function sendCustomToken(
        ...
        for (uint256 i; i < leng; ) {
            if (
                ...
                IERC20(_customTokenAddress).safeTransferFrom(
                    Factory(factoryAddress)
                        .getDAOdetails(address(this))
                        .gnosisAddress,
                    _addresses[i],
                ...
                IERC20(_customTokenAddress).safeTransfer(
                    _addresses[i],
                    _amountArray[i]
                );
            }
    ...
    function airDropToken(
        ...
        if (!(_ownersAirdropFees < FLOAT_HANDLER_TEN_4))
        ...
        if (_ownersAirdropFees == 0) {
            ...
            uint256 _ownerShare = ((_airdropAmount) * (_ownersAirdropFees)) /
                (FLOAT_HANDLER_TEN_4);
            if (
                Factory(factoryAddress)
                    .getDAOdetails(address(this))
                ...
                IERC20(_airdropTokenAddress).safeTransferFrom(
                    Factory(factoryAddress)
                        .getDAOdetails(address(this))
                        .gnosisAddress,
                    erc20DaoDetails.ownerAddress,
                    _ownerShare
                );
            } else {
                require(
                    IERC20(_airdropTokenAddress).balanceOf(address(this)) >=
                        _airdropAmount,
                    "Insufficient funds"
                );
                IERC20(_airdropTokenAddress).safeTransfer(
                    erc20DaoDetails.ownerAddress,
                    _ownerShare
                );
            }
            _updatedAmount = _airdropAmount - _ownerShare;
        }
        ...
        for (uint256 i = 0; i < leng; ) {
            uint256 _dropAmount;
            _dropAmount =
                (balanceOf(_members[i]) *
                    toUniform(_updatedAmount, _airdropTokenAddress)) /
                (totalSupply());
            if (
                Factory(factoryAddress)
                    .getDAOdetails(address(this))
                ...
                IERC20(_airdropTokenAddress).safeTransferFrom(
                    Factory(factoryAddress)
                        ...
                        .gnosisAddress,
                    _members[i],
                    fromUniform(_dropAmount, _airdropTokenAddress)
                );
            } else {
                IERC20(_airdropTokenAddress).safeTransfer(
                    _members[i],
    ...
    Helper
{
    ...
    function updateProposalAndExecution(
        address _contract,
        bytes memory _data
    ) external onlyGnosisOrDao(factoryAddress, address(this)) {
        require(
            _contract == address(this) || _contract == factoryAddress,
            "Invalid address"
        );
