...
contract ERC1155 is IERC1155MetadataURI  {
    ...
    // Used as the URI for all token types by relying on CID substitution, e.g. ipfs://{cid}
    ...
    function mint(uint256 amount, string memory cid)
        public
        returns (uint256)
    {
        require(owner == msg.sender, "Not contract owner!");
        ...
        _metadataHashes[tokenId] = cid;
        _mint(msg.sender, tokenId, amount, "");
        return tokenId;
    }


    function   supportsInterface  (bytes4 interfaceId)  override 
        public
        view
        virtual
        returns (bool)
    ...
    function uri(uint256 id) override public view returns (string memory)  {
        return
            _concat(_uriPrefix, _metadataHashes[id]);
    }

    function balanceOf(address account, uint256 id) override
        public
        view
        returns (uint256)
    {
    ...
    function balanceOfBatch(address[] memory accounts, uint256[] memory ids) override
        public
        view
        returns (uint256[] memory)
    {
    ...
    function  isApprovedForAll(address account, address operator) override
        public
        view
        returns (bool)
    {
