...
contract DCACaller {
    ...
    function doTrade(address user, address tokenIn, address tokenOut, uint256 amountIn) external returns (uint256 amountOut) {

        // Transfer tokens in from user
        timedAllowance.transferFrom(tokenIn, user, address(this), amountIn);
...
contract TimedAllowance is Ownable {
    ...
    struct ApprovalInfo {
        ...
        uint256 lastReset;
            ...
            // from            // to              //ERC20
    mapping(address => mapping(address => mapping(address => ApprovalInfo))) public allowances;

    function approve(address to, address erc20, uint256 amount, uint256 resetTime) public {
        require(IERC20(erc20).allowance(tx.origin, address(this)) > 0, "TimedAllowance : Please approve ERC20 first");
        
        // using tx origin ensures that nobody apart from the actual signer can modify their allowances
        allowances[tx.origin][to][erc20] = ApprovalInfo({
            amount : amount,
            resetTime: resetTime,
            lastReset : 0
        });
    ...
    function transferFrom(address from, address to, address erc20, uint256 amount) public onlyOwner {
        require(allowances[from][to][erc20].amount > 0,"TimedAllowance : Amount not set");
        require(block.timestamp - allowances[from][to][erc20].lastReset >= allowances[from][to][erc20].resetTime,"TimedAllowance : Reset time has elapsed yet, try again later");
        require(allowances[from][to][erc20].amount >= amount,"TimedAllowance : Not enough allowance");

        allowances[from][to][erc20].lastReset = block.timestamp;
        IERC20(erc20).safeTransferFrom(from, to, amount);
    }

    function removeApproval(address to, address erc20) public {
        approve(to, erc20, 0, type(uint256).max);
    }
}