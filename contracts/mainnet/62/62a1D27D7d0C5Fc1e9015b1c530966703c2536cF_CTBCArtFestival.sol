...
import "./interfaces/ICTBCArtFestival.sol";
...
contract CTBCArtFestivalStorage is Object {
    mapping(uint256 => ObjectInfo) public mintParameters;
...
contract CTBCArtFestival is 
    ICTBCArtFestival, 
    CTBCArtFestivalStorage,
    Ownable,
    ReentrancyGuard,
    ERC721A,
    ERC721AQueryable,
    ...
    // Get the mint info of a token.
    function getTokenMintInfo(uint256 tokenId) 
        external
        view 
        override
        returns(ObjectInfo memory tokenMintInfo)
    {
        if (!_exists(tokenId)) {
        ...
        for (uint256 curr = tokenId; ; curr--) {
            ObjectInfo memory currObject = mintParameters[curr];
            if (currObject.mintTime != uint256(0)) {
                return currObject;
            }
        }
        revert TokenNotExist();
        ...
        onlyAuthorized
    {
        ...
        mintParameters[currentIndex] = ObjectInfo(block.timestamp, block.number);
        _safeMint(to, quantities);
        emit MintTokens(to, quantities, totalSupply(), block.timestamp, block.number);
    }
        ...
        nonReentrant
    {
        ...
        uint256 currentIndex = totalSupply().sub(1);
        mintParameters[currentIndex] = ObjectInfo(block.timestamp, block.number);
        _safeMint(msg.sender, quantities);
        emit MintTokens(msg.sender, quantities, totalSupply(), block.timestamp, block.number);
    }
...
interface ICTBCArtFestival is Object {
  ...
  // Get the mint info of a token.
  function getTokenMintInfo(uint256 tokenId) external view returns(ObjectInfo memory tokenMintInfo);
  ...
  event MintTokens(address owner, uint256 quantity, uint256 totalSupply, uint256 mintTimestamp, uint256 mintBlockNumber);
