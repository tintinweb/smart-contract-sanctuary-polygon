 ...
 */
contract RaiderZapper is Ownable, IRaiderZapper {
  ...
  function zapOutToken(
    ...
    // remove from LP
    (uint256 token0Amount, uint256 token1Amount) = IUniswapRouterETH(routerAddr)
      .removeLiquidity(
        token0,
        token1,
        amount,
  ...
  function zapAcross(
    ...
    IUniswapRouterETH(_toRouter).addLiquidity(
      pair.token0(),
      pair.token1(),
      amt0,
      amt1,
  ...
  function swapFromNative(
    ...
    IUniswapRouterETH(routerAddr).swapExactETHForTokens{ value: msg.value }(
      0,
      path,
      _recipient,
      block.timestamp
  ...
  function approveNewInputToken(address token, address router)
    external
    onlyOwner
  {
  ...
  function _swapTokenToLP(
    ...
    if (
      ...
      uint256 liquidity;
      (, , liquidity) = IUniswapRouterETH(routerAddr).addLiquidity(
        _from,
        other,
    ...
    )
  {
    ...
    IUniswapRouterETH router = IUniswapRouterETH(routerAddress);
