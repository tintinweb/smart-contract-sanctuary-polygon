...
contract FelinosMatic is Ownable {
    ...
    function getRefUser(address _user) public view returns(uint[maxPool][REFERRAL_LENGTH]  memory) {
        uint[maxPool][REFERRAL_LENGTH] memory result;
        for(uint i = minPool; i <= maxPool; i++) {
            for(uint j = 0; j < REFERRAL_LENGTH; j++) {
