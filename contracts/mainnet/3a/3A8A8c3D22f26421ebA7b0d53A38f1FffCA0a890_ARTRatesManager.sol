...
contract ARTRatesManager is Ownable {
    ...
    event UpdatedRate(address owner, string currency, uint256 rate);
    event ReadAllRates(address owner, string[] currency, uint256[] rate);

    function updateRate(string memory currencyISO, uint rate) public onlyOwner {
        if (isSupportedCurrency(currencyISO)) {
            rates[currencyISO] = rate;
            emit UpdatedRate(msg.sender, currencyISO, rate);
        }
    }

    function getRateByCurrency(string memory currency) public onlyOwner view returns (uint256) {
    ...
    function calculateART(string[] memory currencies, uint256[] memory values) public returns (uint256) {
        ...
        for (uint256 index = 0; index < currencies.length; index++) {
            ...
            if (isSupportedCurrency(currency)) {
                ...
                uint256 artTokenAmount = (value * rate) / (10 ** DIGITS);
