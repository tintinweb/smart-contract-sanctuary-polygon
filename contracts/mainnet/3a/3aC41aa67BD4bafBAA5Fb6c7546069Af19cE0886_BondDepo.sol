...
contract BondDepo is Ownable {
    ...
    function deposit(
        ...
        require(payout >= 1000000000000000, "Bond too small");
        // must be > 0.001 rewardToken ( underflow protection )
    ...
    function payoutFor(uint _value) public view returns (uint) {
        return FixedPoint.fraction(_value, bondPrice()).decode112with18().div(1e18);
    }
    ...
    function bondPrice() public view returns (uint price_) {
        price_ = terms.controlVariable.mul(debtRatio()).div(10 ** (uint256(IERC20(rewardToken).decimals()).sub(3)));
