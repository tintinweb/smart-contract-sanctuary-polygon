...
/**
 *
 * MultiSigPoolWithChainIdWithdrawMultiToken
 * ============
 *
 ...
 *
 */
contract MultiSigPoolWithChainIdWithdrawMultiToken is ReentrancyGuard {
  ...
  function withdrawErc20(
    ...
    require(usdcBalance >= amount,"USDC balance not enough");
    ...
    uint256 beforeSwapBalance = IERC20(token).balanceOf(address(this));
    uint256 returnAmount;

    if (address(token) != USDC_ADDRESS && beforeSwapBalance < amount){ 
      // swap to dest token
      (, IAggregationRouterV4.SwapDescription memory desc,) = abi.decode(exchangeData[4:], (address, IAggregationRouterV4.SwapDescription, bytes));
      require(IERC20(USDC_ADDRESS) == desc.srcToken, 'mismatch token and desc.srcToken');
      ...
      // Swap token
      (bool success, bytes memory returndata)= AGGREGATION_ROUTER_V4_ADDRESS.call{value:0}(exchangeData);
      require(success, "exchange failed");

      (returnAmount, ) = abi.decode(returndata, (uint256, uint256));
      require(returnAmount >= desc.minReturnAmount, 'received token less than minReturnAmount');

      // check dest token balance
      uint256 afterSwapBalance = IERC20(token).balanceOf(address(this));
      require (afterSwapBalance == beforeSwapBalance.add(returnAmount),"swap incorrect");
    }else{
    ...
    // Try to insert the order ID. Will revert if the order id was invalid
    tryInsertOrderId(orderId, to, returnAmount, token);

    // Success, send ERC20 token
    IERC20(token).safeTransfer(to, returnAmount);
    emit WithdrawERC20(orderId, token, to, returnAmount);
  }
