...
pragma solidity 0.6.12;
contract Ant is ERC20Burnable, Operator {
    ...
    // Initial distribution for the day 2-5 ANT-WETH LP -> ANT pool
    uint256 public constant INITIAL_ANT_POOL_DISTRIBUTION = 672000 ether;
    // Distribution for airdrops wallet
    uint256 public constant INITIAL_AIRDROP_WALLET_DISTRIBUTION = 48000 ether;
    ...
    /**
     * @notice Constructs the ANT ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("ant.money", "ANT") {
        // Mints 1 ANT to contract creator for initial pool setup
    ...
    function _getAntPrice() internal view returns (uint256 _antPrice) {
        try IOracle(antOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("Ant: failed to fetch ANT price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _antPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_antPrice >= taxTiersTwaps[tierId]) {
    ...
    function setAntOracle(address _antOracle) public onlyOperatorOrTaxOffice {
        require(_antOracle != address(0), "oracle address cannot be 0 address");
        antOracle = _antOracle;
    }
    ...
    /**
     * @notice Operator mints ANT to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of ANT to mint to
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentAntPrice = _getAntPrice();
            currentTaxRate = _updateTaxRate(currentAntPrice);
            if (currentAntPrice < burnThreshold) {
                burnTax = true;
            }
        }
    ...
    function distributeReward(
        ...
        require(_antPool != address(0), "!_antPool");
        require(_airdropWallet != address(0), "!_airdropWallet");
        rewardPoolDistributed = true;
        _mint(_genesisPool, INITIAL_GENESIS_POOL_DISTRIBUTION);
        _mint(_antPool, INITIAL_ANT_POOL_DISTRIBUTION);
