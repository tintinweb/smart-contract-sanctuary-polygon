...
error InsuficientFunds(uint256 expected, uint256 received);

contract Sender is BaseRecoverableContract {
    uint256 private constant _BATCH_SIZE_LIMIT = 800;
    ...
    function batchTransfer(address[] memory accounts, uint256 value) external payable {
        if (accounts.length > _BATCH_SIZE_LIMIT) revert BatchSizeTooLarge(_BATCH_SIZE_LIMIT, accounts.length);
        if (msg.value < value * accounts.length) revert InsuficientFunds(value * accounts.length, msg.value);

        for (uint256 i; i < accounts.length; ) {
            if (accounts[i] == address(0)) revert AddressIsEmpty();
