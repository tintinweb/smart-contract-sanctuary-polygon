...
// File: contracts/Ninya2/ninya.sol
...
contract ITokenized is NinfyDropsInterface, ERC721, Ownable {
    ...
    uint256 public override mintLimit = 1;
    ...
    function withdraw() external onlyOwner override  {
        ...
        devWallet.transfer(contractBalance.mul(devSaleShare).div(100));
    ...
    function buy(uint256 _amount) external override payable {
        ...
        require(saleActive, "NiftyDropPlatform::buy: Sale is not active.");

        require(_amount <= mintLimit, "NiftyDropPlatform::buy: Too many tokens for one transaction.");

        require(msg.value >= mintPrice.mul(_amount), "NiftyDropPlatform::buy: Insufficient payment.");



        if (!smartContractAllowance) {

            require(tx.origin == _msgSender(), "NiftyDropPlatform::buy: Smart contracts are not allowed to buy.");
