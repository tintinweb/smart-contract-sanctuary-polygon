...
contract TechnologyTree is OwnableUpgradeable {
    ...
    function buyTec(uint ID, uint amount) external {
        ...
        if(userTec[msg.sender][ID] + amount >= 500e18){
            amount = 500e18 - userTec[msg.sender][ID] ;
        }
        BVT.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), amount);
    ...
    function buyTecBatch(uint[] memory ids, uint[] memory amounts) external {
        ...
        for (uint i = 0; i < ids.length; i ++) {
            ...
            if(userTec[msg.sender][ids[i]] + tempAmount >= 500e18){
                tempAmount = 500e18 - userTec[msg.sender][ids[i]];
            }
            total += tempAmount;
            userTec[msg.sender][ids[i]] += tempAmount;
        ...
        BVT.transferFrom(msg.sender, address(0x000000000000000000000000000000000000dEaD), total);
