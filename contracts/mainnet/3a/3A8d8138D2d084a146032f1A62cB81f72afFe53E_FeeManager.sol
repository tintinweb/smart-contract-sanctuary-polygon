...
contract FeeManager is Ownable, IFeeManager {
    ...
    /// @notice Fees Information for Implementation
    struct ImplementationFeeInfo {
    ...
    mapping(address => ImplementationFeeInfo) public implementationFeeInfos;
    ...
    ) external payable override returns (uint256 fetchedFees) {
        fetchedFees = _fetchFees(_implementationAddress, false, 0);
    }
    ...
    function _fetchFees(
        ...
        bool _exactFees,
        uint256 _feeAmount
    ) private returns (uint256 fetchedFees) {
        ImplementationFeeInfo memory feeInfo = implementationFeeInfos[
            _implementation
        ];
    ...
    function fetchExactFees(
        ...
        fetchedFees = _fetchFees(_implementationAddress, true, _feeAmount);
    }

    /**
     * @notice Updates or Create a new Information for a Implementation Contract
     */
    function updateImplementaionFeesInfo(
        address _implementation,
        uint256 _feeAmount,
        address _feeToken
    ) external onlyOwner {
        ImplementationFeeInfo storage feeInfo = implementationFeeInfos[
            _implementation
        ];
        feeInfo.feeAmount = _feeAmount;
        feeInfo.feeToken = _feeToken;
        feeInfo.isFeeDistributor = true;
        emit FeesUpdated(_implementation, _feeAmount, _feeToken);
    }
    ...
    {
        if (_token == address(0)) {
            feeBeneficiary.transfer(_amount);
        } else {
    ...
    function getImplementationFeeInfo(
        ...
        address _factoryCaller,
        bytes memory _encodedData
    ) external view override returns (uint256 _feeAmount, address _feeToken) {
        ImplementationFeeInfo memory feeInfo = implementationFeeInfos[
