...
import "../base/NewFixedPrice.sol";
import "../util/OwnableUpgradeable.sol";
import "../interfaces/INewFixedPriceDeployer.sol";

contract NewFixedPriceDeployer is INewFixedPriceDeployer, OwnableUpgradeable {
  address public immutable FIXEDPRICE_IMPL;

  constructor() {
    __Ownable_init(msg.sender);
    FIXEDPRICE_IMPL = address(new NewFixedPrice());
  }

  function newDeployFixedPrice(
    address _timelock,
    address _erc20,
  ...
  ReentrancyGuardUpgradeable
{
  ...
  event BaseURIFrozen();
  event UpdateWhitelistState(bool state);
  event UpdateMintDeadlineTime(uint256 _newTimestamp);
  ...
  uint256 public repeatMintNum;  //Number of single repeats
  uint256 public mintDeadlineTime; // default 0, does not open,
  bool public isNotWhitelist;      // Whether to enable the whitelist mode
  address public userAddress;      // User deployment contract address
  address public whitelsitAddress; // white address

  string public _name;
  string public _symbol;
  string public _bURI;

  // Record whitelisted users
  mapping(address => bool) public _whiteLists;

  // Record mint number whether the user has mint
  mapping(address => uint256) public mintNumber;
  ...
  function initialize(
    ...
    (repeatMintNum,mintDeadlineTime,isNotWhitelist,whitelsitAddress,_name,_symbol,_bURI) = abi.decode(_newParameter,(uint256,uint256,bool,address,string,string,string));
    require(repeatMintNum > 0, "Must be greater than 0");
    if (mintDeadlineTime > 0) {
       require(mintDeadlineTime > block.timestamp, "Must be greater than the current timestamp");
    }
  ...
  modifier onlyUserAddress() {
    require(userAddress == _msgSender(), "Ownable: caller is not the user address");
    _;
  }

  modifier onlyWhitelsitAddress() {
    require(whitelsitAddress == _msgSender(), "Ownable: caller is not the whitelsit address");
    _;
  }

  // only user address can set Whitelist
  function setEnableWhitelist() public onlyUserAddress {
    require(!isNotWhitelist, "The whitelist function is enabled");
    isNotWhitelist = !isNotWhitelist;
    emit UpdateWhitelistState(true);
  }

  // only ReceivingAddress can set Mint Deadline Time
  function setMintDeadlineTime(uint256 _deadline) public onlyUserAddress {
    require(_deadline > block.timestamp, "Must be greater than the current timestamp");
    mintDeadlineTime = _deadline;
    emit UpdateMintDeadlineTime(_deadline);
  }

     // batch add white user
  function addWhite(address[] memory _addWhite) public onlyWhitelsitAddress returns (bool) {
      for (uint i = 0; i < _addWhite.length; i++) {
          require(_addWhite[i] != address(0), "addWhite: _addWhite is the zero address");
          _whiteLists[_addWhite[i]] = true;
      }
      return true;
    }

  // batch delete white user
  function delWhite(address[] memory _delWhites) public onlyWhitelsitAddress returns (bool) {
      for (uint i = 0; i < _delWhites.length; i++) {
          require(_delWhites[i] != address(0), "delWhite: _delWhite is the zero address");
          _whiteLists[_delWhites[i]] = false;
      }
      return true;
    }
  ...
  function mint() public nonReentrant returns (uint256 tokenId) {
    ...
    require(mintNumber[_msgSender()] < repeatMintNum, "Exceeding the limit of mint number");
    //  whiteLists mode
    if (isNotWhitelist) {
        require(_whiteLists[_msgSender()], "Not a whitelist user");
    }
    // cutoff mode
    if (mintDeadlineTime > 0 ){
      require(block.timestamp < mintDeadlineTime, "Over mint deadline");
    }
    ...
    mintNumber[_msgSender()] += 1;   // add number
  ...
  function mintEth() public payable nonReentrant returns (uint256 tokenId) {
    ...
    // Accept the assets
    (bool result, ) = receivingAddress.call{value: rate}("");
...
interface INewFixedPriceDeployer {
