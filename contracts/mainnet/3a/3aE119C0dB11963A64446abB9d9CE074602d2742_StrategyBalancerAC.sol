...
// TODO: rename output to reward1
// TODO: hoist reward2 and reward2ToNativeRoute, uniRouter variable
// TODO: write remaining comments

/// @dev Auto Compounding Strategy
contract StrategyBalancerAC is Initializable, StratManagerUpgradeableCommon, DynamicFeeManager {
    ...
    bytes32[] public outputToNativeRoutePids;
    bytes32[] public outputToLp0RoutePids;
    bytes32[] public outputToLp1RoutePids;
     ...
     * @param _routers [Dystopia router, Uniswap router]
     * @param _keeper Keeper
     * @param _strategist Strategist
     * @param _feeRecipient Fee Recipient receives a portion of fee
     * @param _outputToNativeRoute Array of address. Path along which to swap along
     * @param _reward2ToNativeRoute Array of address. Path along which to swap along
     * @param _outputToLp0Route Array of address. Path along which to swap along
     * @param _outputToLp1Route Array of address. Path along which to swap along
     * @param _routePids Array of (array of bytes32).
                         It is of the form [_outputToNativeRoutePids[], _outputToLp0RoutePids[], _outputToLp1RoutePids[]]
     ...
     */
    function __StrategyBalancerAC_init(
        address _want,
        address _rewardPool,
        address _vault,
        ...
        __StratManager_init_unchained(_keeper, _strategist, _routers[0], _vault, _feeRecipient);
        __DynamicFeeManager_init_unchained();
        __StrategyBalancerAC_init_unchained(_want, _rewardPool, _routers[1], _outputToNativeRoute, _reward2ToNativeRoute, _outputToLp0Route, _outputToLp1Route, _routePids);
    }

    function __StrategyBalancerAC_init_unchained(
        address _want,
        address _rewardPool,
        address _uniRouter,
        address[] memory _outputToNativeRoute,
        ...
        outputToNativeRoutePids = _routePids[0];
        outputToLp0RoutePids = _routePids[1];
        outputToLp1RoutePids = _routePids[2];

        require(outputToNativeRoute.length == outputToNativeRoutePids.length + 1, "route length != routePids length + 1");
        require(outputToLp0Route.length == outputToLp0RoutePids.length + 1, "route length != routePids length + 1");
        require(outputToLp1Route.length == outputToLp1RoutePids.length + 1, "route length != routePids length + 1");
    ...
    /// @dev function that is called before each deposit
    ...
    /** @dev currently assumes one of the pool tokens is the native token,
             so the native can now be fed directly for adding liquidity without having to swap it
     */
    function _harvest(address callFeeRecipient) internal whenNotPaused {
        IRewardPool(rewardPool).claim_rewards();
        uint256 reward2Balance = IERC20Upgradeable(reward2).balanceOf(address(this));

        uint256 nativeBalanceBefore = IERC20Upgradeable(native).balanceOf(address(this));
        // reward2 to native
        uint24 poolFee = 3000; // for standard non-correlated pools like ETH/DAI
        swapUniswap(reward2Balance, reward2ToNativeRoute, poolFee, block.timestamp + 30 seconds);
        ...
        if (outputBal > 0) {
            chargeFees(
                callFeeRecipient,
                (nativeBalanceAfter - nativeBalanceBefore) * feeOnProfits / 1000
            );
            addLiquidity();
    ...
    function chargeFees(address callFeeRecipient, uint256 nativeFeeFromReward2) internal {
        ...
        if (generalFeeOnProfits > 0) {
            if (output != native) {
                ...
                swap(generalFeeOnProfits, outputToNativeRoute, outputToNativeRoutePids);
        ...
        generalFeeAmount += nativeFeeFromReward2;
     ...
     */
    function swap(uint256 amount, address[] memory route, bytes32[] memory routePids) internal {
    ...
    /// @dev assuming last elm of maxAmountsIn corresponds to the pool receipt token [as is the case with stMATIC/MATIC pool]
    /// @dev userData: [EXACT_TOKENS_IN_FOR_BPT_OUT, amountsIn, minimumBPT]
    function addLiquidity() internal {
        uint256 outputHalf = IERC20Upgradeable(output).balanceOf(address(this)) / 2;

        if (lpToken0 != output) {
            swap(outputHalf, outputToLp0Route, outputToLp0RoutePids);
        }

        if (lpToken1 != output) {
            swap(outputHalf, outputToLp1Route, outputToLp1RoutePids);
        }
        ...
        for (uint256 j = 0; j < assets.length; j++) {
            if (address(assets[j]) == lpToken0) {
                maxAmountsIn[j] = lp0Bal;
            } else if (address(assets[j]) == lpToken1) {
                maxAmountsIn[j] = lp1Bal;
            }
        }

        uint256[] memory amountsIn = new uint256[](maxAmountsIn.length - 1);
        for (uint256 j = 0; j < maxAmountsIn.length - 1; j++) {
            amountsIn[j] = maxAmountsIn[j];
        }
    ...
    * @param amount Amount that is to be swapped
    * @param route Array of address; route along which to swap
    * @param deadline Timestamp (in seconds) beyond which the transaction will fail
    */
    function swapUniswap(uint256 amount, address[] memory route, uint24 poolFee, uint256 deadline) internal {
        if (amount == 0) return;
    ...
    function _giveAllowances() internal {
        ...
        IERC20Upgradeable(output).safeApprove(router, 0);
        IERC20Upgradeable(output).safeApprove(router, type(uint256).max);
...
contract StratManagerUpgradeableCommon is Initializable, OwnableUpgradeable, PausableUpgradeable {
  ...
  function __StratManager_init(
    ...
    __Ownable_init_unchained();
    __Pausable_init_unchained();
    __StratManager_init_unchained(_keeper, _strategist, _router, _vault, _feeRecipient);
  }
