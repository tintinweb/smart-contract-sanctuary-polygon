...
contract Vault is Ownable, Signers {
    ...
    function swapRouter(bytes calldata _data) external payable {
        abi.decode(_data[4:], (IClipperExchangeInterface, address, IERC20, IERC20, uint256, uint256, uint256, bytes32, bytes32));
    }
    ...
    function fillOrderTo ( uint minOut, bytes calldata _data) external payable {
        (OrderLib.Order memory _order, bytes memory sig, bytes memory a, uint256 make, uint256 take, uint256 b, address user) =
        abi.decode(_data[4:], (OrderLib.Order, bytes, bytes, uint256, uint256, uint256, address));
        {
        bool isNotNative = !_isNative(IERC20(_order.makerAsset));

        if (isNotNative) {
            IERC20(_order.makerAsset).transferFrom(msg.sender, address(this), _order.makingAmount);
        ...
        (bool succ, bytes memory _data) = address(ROUTER).call{value: msg.value}(_data);
        if(succ) {
            (uint256 actualMakingAmount, uint256 actualTakingAmount,) = abi.decode(_data, (uint256, uint256, bytes32));
    ...
    function fillOrderToBridge ( uint minOut, bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        (OrderLib.Order memory _order, bytes memory sig, bytes memory a, uint256 make, uint256 take, uint256 b, address user) =
        abi.decode(_data[4:], (OrderLib.Order, bytes, bytes, uint256, uint256, uint256, address));
        ...
        if(succ) {
            (uint256 actualMakingAmount, uint256 actualTakingAmount,) = abi.decode(_data, (uint256, uint256, bytes32));
            _isNotNativeBridge(IERC20(_order.makerAsset),_order.makingAmount,bDesc);
            // emit Swap(msg.sender, _order.makerAsset, _order.takerAsset, actualMakingAmount, actualTakingAmount);
        } else {
            revert();
        }
    ...
    function swapBridge( bytes calldata _data, BridgeDescription calldata bDesc) external payable {
        (address _c, SwapDescription memory desc, bytes memory _d) = abi.decode(_data[4:], (address, SwapDescription, bytes));
    ...
    function relayV3(uint minOut, address srcToken, address toToken, bytes calldata _data) external onlyOwner {
        ( address user, uint256 amount, uint256 b, uint256[] memory c) = abi.decode(_data[4:], ( address, uint256, uint256, uint256[]));
    ...
    function relayUno(uint minOut, address toToken, bytes calldata _data) external onlyOwner {
        (address user, IERC20 srcToken, uint256 amount, uint256 b, bytes32[] memory c) = abi.decode(_data[4:], (address, IERC20, uint256, uint256, bytes32[]));
