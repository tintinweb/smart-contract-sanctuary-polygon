...
contract MoreNFTCollection is IMintableCollection, ERC721Enumerable, ERC2981, AccessControl, Ownable {
    ...
    /**
    * @dev tokens are minted sequentially, bounds included.
    */
    function bulkMint(address _to, uint256 _fromTokenId, uint256 _toTokenId) external onlyRole(MINTER_ROLE) {
        require(_fromTokenId > 0 && _fromTokenId <= _toTokenId, "Provide consistent token bounds and ID");
        require(_toTokenId <= cap, "Can not mint beyond the cap");
        for (uint256 _i = _fromTokenId; _i <= _toTokenId; _i++) {
            _mint(_to, _i);
        }
    }
