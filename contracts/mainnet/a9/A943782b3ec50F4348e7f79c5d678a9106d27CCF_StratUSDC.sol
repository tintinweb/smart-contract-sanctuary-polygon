...
contract StratUSDC {
  ...
  function swapJava(uint256 amountDlyPerSale, uint256 minUsdcToReceive)
    external
    onlyAdmin
    returns (uint256 amountUsdcReceived)
  {
    // Amount of USDC received in swap
    IERC20(DLYCOP).safeApprove(address(JAVA_VENDOR), amountDlyPerSale);
    JAVA_VENDOR.sellUSDC(amountDlyPerSale);

    amountUsdcReceived = IERC20(USDC).balanceOf(address(this));

    // Check that enough funds were sent
    require(amountUsdcReceived >= minUsdcToReceive, 'INSUFFICIENT_OUTPUT');

    // Send USDC to admin
    IERC20(USDC).safeTransfer(admin, amountUsdcReceived);
  }

  function swapPegdex(uint256 amountDlyPerSale, uint256 minUsdcToReceive)
    external
    onlyAdmin
    returns (uint256 amountUsdcReceived)
  {
    ...
    amountUsdcReceived = PEDGEX.swapCOPForTokens(
      USDC,
      amountDlyPerSale,
      minUsdcToReceive,
      block.timestamp + 1
