    ...
    ML_TransferHelper
{
    ...
    {
        //get data
        (uint256 cShares, uint256 pShares) = vault.getTotalPayoutAndCompoundShares();
        uint256 tShares = cShares + pShares;

        //check amount
        uint256 amount = (balanceOfReward() * pShares) / tShares;
        if (amount == 0)
        {
            return;
        }
    ...
    {
        //TODO: check restrictions to prevent rug or maybe completly remove
    ...
    ML_RouterSwap_UniSwapV2
{
    ...
    {
        uint256[] memory out = IUniRouterV2(_router).getAmountsOut(
            _amount,
            _path
        );
