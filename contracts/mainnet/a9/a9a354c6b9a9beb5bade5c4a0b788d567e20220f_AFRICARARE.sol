...
contract AFRICARARE is ERC721, Ownable, Pausable {
    ...
    //URI of the mask NFT metadata
    string public maskMetadata = "https://bafkreidjm55katsnqaxge43aml2yb4wf7tsxcgvdqrqqb7jper5ricjebi.ipfs.nftstorage.link/";
    ...
    //Constructor
    constructor() ERC721("Africarare Mask Collection", "gRARE") {
    }
    ...
    */
    function mintSingleMaskNFT(string memory URI) internal onlyOwner {
    ...
    */
    function mintToAddressSingleMaskNFT(string memory URI, address receiverAddress) internal onlyOwner {
        uint256 tokenId = numMinted;
        require(!_exists(tokenId), "That token ID has already been minted. Please try again.");
        ++numMinted;
    ...
    */
    function mintBatchOfMaskNFT(uint256 numToMint) external onlyOwner {
        for (uint i=0; i < numToMint; i++) {
            mintSingleMaskNFT(maskMetadata);
        }
    }
    ...
    */
    function mintToAddressBatchOfMaskNFT(uint256 numToMint, address receiverAddress) external onlyOwner {
        for (uint i=0; i < numToMint; i++) {
            mintToAddressSingleMaskNFT(maskMetadata, receiverAddress);
        }
    }
