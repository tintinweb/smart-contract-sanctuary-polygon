...
contract RUFFRAFFLE {
    ...
    function withdrawTokens(address tokenAddress, address recipient, uint256 amount) external onlyAdmin {
        ...
        require(amount > 0, "Amount should be greater than zero.");
        require(amount <= supportedTokens[msg.sender][tokenAddress], "Insufficient token balance.");
    ...
    function withdrawAllTokens(address recipient) external onlyAdmin {
        ...
        payable(recipient).transfer(totalBalance);
    ...
    function getSupportedTokensBalance(address user) external view returns (uint256[] memory) {
        uint256 tokenCount = 0;
