...
contract UniswapV3Oracle is Ownable {
    ...
    uint24[] internal defaultFeeTiers = [500, 3000, 10000];
    ...
    function getEstimatedAmountOut(uint128 _amountIn, address _tokenIn, address _tokenOut, uint32 _period) external view returns (uint256 _quoteAmount, address[] memory _queriedPools) {
        _queriedPools = _getQueryablePoolsForTiers(_tokenIn, _tokenOut, _period);
        _quoteAmount = _quote(_amountIn, _tokenIn, _tokenOut, _queriedPools, _period);
    }

    function _quote(uint128 _amountIn, address _tokenIn, address _tokenOut, address[] memory _pools, uint32 _period) internal view returns (uint256 _quoteAmount) {
        ...
        return OracleLibrary.getQuoteAtTick(_weightedTick, _amountIn, _tokenIn, _tokenOut);
    }

    function _getQueryablePoolsForTiers(address _tokenIn, address _tokenOut, uint32 _period) internal view returns (address[] memory _queryablePools) {
        address[] memory _existingPools = _getPoolsForTiers(_tokenIn, _tokenOut, defaultFeeTiers);
    ...
    function _getPoolsForTiers(address _tokenIn, address _tokenOut, uint24[] memory _feeTiers) internal view returns (address[] memory _pools) {
        _pools = new address[](_feeTiers.length);
        uint256 _validPools;
        for (uint256 i; i < _feeTiers.length; i++) {
            address _pool = PoolAddress.computeAddress(uniswapFactory, PoolAddress.getPoolKey(_tokenIn, _tokenOut, _feeTiers[i]));
    ...
    function changeUniswapFactory(address _uniswapFactory) external onlyOwner {
        uniswapFactory = _uniswapFactory;
    }
    ...
    function addNewFeeTier(uint24 _feeTier) external onlyOwner {
        require(IUniswapV3Factory(uniswapFactory).feeAmountTickSpacing(_feeTier) != 0, 'BadFee');
        for (uint256 i; i<defaultFeeTiers.length; i++) require(defaultFeeTiers[i] != _feeTier, 'TierExists');
        
        defaultFeeTiers.push(_feeTier);
    }
}
