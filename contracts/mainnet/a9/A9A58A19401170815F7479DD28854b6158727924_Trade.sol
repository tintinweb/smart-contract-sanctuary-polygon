...
contract Trade {
    ...
    //Pre-sale whitelist user
    mapping(address => bool) private whitelistedWallet;

    uint256 public mintingFee = 600 * 10**18;
    ...
    modifier onlySigner() {
        require(owner == msg.sender, "caller is not the signer");
        _;
    }
    ...
    function changeSigner(address newSigner) external onlySigner returns (bool) {
        require(
            newSigner != address(0),
