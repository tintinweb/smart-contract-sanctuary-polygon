...
contract BscBank {
    ...
    event LogRefund(address _to, address _token, string _symbol, uint256 _value, uint256 _nonce);
...
// File contracts/BridgeBankPausable.sol
    ...
    PausableUpgradeable
{
    ...
    bytes32 public constant REFUND_HASH =
        keccak256("REFUND(address _recipient,address _tokenAddress,string _symbol,uint256 _amount,uint256 _nonce)");
    ...
    function burn(
        ...
        uint256 fee = getBridgeFee(msg.sender);
    ...
    function refund(
        ...
        require(isValidThreshold(_signatures.length), "Signature: Invalid Length");
        bytes32 hash = getRefundHash(_recipient, _tokenAddress, _symbol, _amount, _nonce);
        address[1000] memory listValidatorsSigned;
        for (uint256 i = 0; i < _signatures.length; i++) {
            bytes memory _signature = _signatures[i];
            address signer = _getSigner(hash, _signature);
