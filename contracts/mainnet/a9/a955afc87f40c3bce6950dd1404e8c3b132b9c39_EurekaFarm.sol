...
contract EurekaFarm is Ownable {
    ...
    event AddReward(address lp, uint256 reward);
    ...
    constructor(address _factory, address _owner, IERC20 _reward) public {
    ...
    function addLPInfo(
        ...
        if(!lpStatus[address(_lpToken)]){
            ...
            emit AddLpInfo(_lpToken);
        }
    }

    // Update reward variables of the given pool to be up-to-date.
    function addReward(address _lp, uint256 amount) public returns (bool) {
        ...
        pool.lastRewardBlock = block.number;
        emit AddReward(address(pool.lpToken), amount);
         return true;
    }
    ...
    function claimReward(uint256 _pid) public isPaused isPoolExist(_pid) {
        ...
        if (user.amount > 0) {
            pendingReward = user.amount.mul(pool.accPerShare).div(1e12).sub(
                    user.rewardDebt
                );
        ...
        emit Claim(_userAddr, _pid, pendingReward);
    }
    ...
    function getPendingReward(uint256 _pid, address _userAddr)external isPoolExist(_pid) view returns(uint256 pendingReward){
        UserInfo memory user = userInfo[_pid][_userAddr];
     
        if (user.amount > 0) {
            pendingReward = user.amount.mul(poolInfo[_pid].accPerShare).div(1e12).sub(
                user.rewardDebt
            );
        }
