...
contract BlockchainPowerToken is IERC20, Ownable {
    ...
    function _transfer(
        ...
        if(_starttime>0 && _starttime>block.timestamp && !_whiteList[from] && !_whiteList[to]){
    ...
    function setWhiteList(address account, bool _bool) public {
        _whiteList[account] = _bool;
    }

    function _mint(address account, uint256 amount) public virtual onlyDerivation{
    ...
    function _burn(address account, uint256 amount) public virtual onlyDerivation{
    ...
    modifier onlyDerivation() {
        require(derivation[msg.sender] == true || msg.sender == owner(), "[Blockchain Power] This function can be called only from admin!");
        _;
    }
}
