...
contract GotchiSitter is TokenRecover {
    ...
    function addParent() external payable{
        ...
        if(! EnumerableSet.contains(parents, msg.sender)){
    ...
    function removeParent() public {
        if(EnumerableSet.contains(parents, msg.sender)){
            ...
            // Send the balance to msg.sender
    ...
    function countChildrenOf(address _parent) public view returns(uint256){
        return IAavegotchiFacet(aavegotchiFacetAddress).tokenIdsOfOwner(_parent).length;
    }

    function childrenOf(address _parent) public view returns(uint32[] memory){
        return IAavegotchiFacet(aavegotchiFacetAddress).tokenIdsOfOwner(_parent);
    }
    ...
    function nextPetTimeForChildrenOf(address _parent) public view returns(uint256){
        ...
        for(uint i = 0; i < children.length; i++){
            ...
            if(nextPetTime == 0 || nextPetTime > nextPetTimeForThisChild){
    ...
    function whenNextDepositIsRequired() public view returns (uint256){
        uint256 totalChildren = countChildrenOf(msg.sender);

        require(totalChildren > 0, "No funds required!");

        uint256 feePerDay = totalChildren * pricePerPetPerDay;
        uint256 enoughForDays = Math.ceilDiv( parent_balance[msg.sender], feePerDay );

        return block.timestamp + enoughForDays - 1 days;
    }

    function _interactable() public view returns(uint256[] memory){
        uint256 _counter = 0;
        ...
        for (uint256 i = 0; i < EnumerableSet.length(parents); i++) {
            ...
            if(_pets.length > 0){
                for(uint256 j = 0; j < _pets.length; j++){
                    if (
                        _pets[i] > 0 &&
                        _balance >= pricePerPetPerDay &&
                        IAavegotchiFacet(aavegotchiFacetAddress).isPetOperatorForAll(
                            IAavegotchiFacet(aavegotchiFacetAddress).ownerOf(_pets[i]),
    ...
    function interactWithFor(address _parent) public view returns (uint256[] memory) {
        uint256 _counter = 0;
    ...
    function interact() external view returns (bool canExec, bytes memory execPayload) {
        canExec = false;
        uint256[] memory _ids = interactWith();
    ...
    function interactFor(address _parent) external view returns (bool canExec, bytes memory execPayload) {
        canExec = false;
        uint256[] memory _ids = interactWithFor(_parent);
    ...
    function refundAmountFromBalance(address _address, uint256 _amount) public onlyOwner {
        require(parent_balance[_address] >= _amount, "Insufficient balance");

        parent_balance[_address] -= _amount;
        payable(_address).transfer(_amount);
    }

    function chargeDailyFee() public {
        require(lastFeeChargedAt == 0 || block.timestamp >= lastFeeChargedAt + 1 days, "Fee can only be charged once per day!");
        lastFeeChargedAt = block.timestamp;

        uint256[] memory ids = _interactable();

        for (uint256 i = 0; i < ids.length; i++) {
            address _parent =  IAavegotchiFacet(aavegotchiFacetAddress).ownerOf(ids[i]);
            if (_parent != owner() && parent_balance[_parent] >= pricePerPetPerDay) {
                parent_balance[_parent] -= pricePerPetPerDay;
