...
contract KyberUSDCMAIStrategy is InitializableAbstractStrategy, IERC721Receiver {
    ...
    function initialize(
        ...
        minThresholds.push(10**5);
        minThresholds.push(10**17);
    }
    ...
    function getRatios() public view override returns (uint256, uint256) {
        if (tokenId == 0) {
            ...
            uint256 token1Equivalent = OracleLibrary.getQuoteAtTick(currentTick, sqrtPriceX96, 10 ** 8, address(token0), address(token1));
    ...
    function _deposit(address[] memory, uint256[] memory _amounts, bool) internal {
    ...
    function _mint(uint256 _amount0, uint256 _amount1) internal returns (uint128) {
        ...
        // 0.01 % diff on both sides
        int256 diff = (((int256(nearestCurrentTick) * 1) / 10000) / uniswapV3Pool.tickDistance()) * uniswapV3Pool.tickDistance();
        ...
        console.logInt(nearestCurrentTick);
        console.logInt(range0);
        console.logInt(range1);
        console.logInt(_previousTick);
        console.logInt(_nextTick);
        INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams(
            ...
            _amount0,
            _amount1,
            0,// OvnMath.subBasisPoints(_amount0, 500), // slippage 5%
            0,// OvnMath.subBasisPoints(_amount1, 500), // slippage 5%
            address(this),
            block.timestamp + 600
        );
    ...
    function _withdraw(address _beneficiary, address[] memory _assets, uint256[] memory _amounts) internal {
        ...
        _transferAllTokens(returnedAmount0, returnedAmount1);
        _sendBack(_beneficiary, _amounts[0], _amounts[1]);
    }
    function _transferAllTokens(uint256 _a0, uint256 _a1) internal {
        ...
        try uniswapPositionManager.burnRTokens(burnParams) {} catch Error(string memory reason) {
            console.log("BRN_F", reason);
        }
        uniswapPositionManager.transferAllTokens(address(token0), _a0, address(this));
        uniswapPositionManager.transferAllTokens(address(token1), _a1, address(this));
    }
    ...
    */
    function _collectRewards() internal returns (uint256, uint256) {
        ...
        try elasticMiner.harvestMultiplePools(_getId(), _datas) {} catch Error(string memory reason) {
            console.log("H_F", reason);
        }
        uint256 rewardBalance = KNC.balanceOf(address(this));
        console.log("KNC", rewardBalance);
        if (rewardBalance <= minThresholds[0]) {
            return (0, 0);
        }

        KNC.approve(address(swapper), rewardBalance);
        uint256 rewardBalanceInWMATIC = swapper.swapCommon(address(KNC), address(wMATIC), rewardBalance.subBasisPoints(100));
        (uint256 _swapPart0, uint256 _swapPart1) = _rewardSwapDistribution(rewardBalanceInWMATIC);
        wMATIC.approve(address(swapper), rewardBalanceInWMATIC);
        uint256 swappedT0amount = 0;
        uint256 swappedT1amount = 0;
        if (_swapPart0 > minThresholds[0]) {
            swappedT0amount = swapper.swapCommon(address(wMATIC), address(token0), _swapPart0);
        }
        if (_swapPart1 > minThresholds[0]) {
            swappedT1amount = swapper.swapCommon(address(wMATIC), address(token1), _swapPart1);
        }
    ...
    */
    ...
    function _getId() internal view returns (uint256[] memory) {
