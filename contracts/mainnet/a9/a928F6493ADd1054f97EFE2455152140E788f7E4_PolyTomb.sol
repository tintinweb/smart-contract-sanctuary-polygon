...
contract PolyTomb is ERC20Burnable {
    ...
    // Have the rewards  distributed to the pools
    ...
    /**
     * @notice Constructs the PEG ERC-20 contract.
     */
    constructor(uint256 _taxRate, address _taxCollectorAddress) public ERC20("PEG", "PEG") {
        // Mints 1 PEG to contract creator for initial pool setup
    ...
    function _getPEGTombPrice() internal view returns (uint256 _pegTokenPrice) {
        try IOracle(pegTokenOracle).consult(address(this), 1e18) returns (uint144 _price) {
            return uint256(_price);
        } catch {
            revert("PEGTomb: failed to fetch PEG price from Oracle");
        }
    }

    function _updateTaxRate(uint256 _pegTokenPrice) internal returns (uint256) {
        if (autoCalculateTax) {
            for (uint8 tierId = uint8(getTaxTiersTwapsCount()).sub(1); tierId >= 0; --tierId) {
                if (_pegTokenPrice >= taxTiersTwaps[tierId]) {
    ...
    function setPEGTombOracle(address _pegTokenOracle) public onlyOperatorOrTaxOffice {
        require(_pegTokenOracle != address(0), "oracle address cannot be 0 address");
    ...
    /**
     * @notice Operator mints PEG to a recipient
     * @param recipient_ The address of recipient
     * @param amount_ The amount of PEG to mint to
     * @return whether the process has pegTokenn done
     */
    ...
    function transferFrom(
        ...
        if (autoCalculateTax) {
            uint256 currentPEGTombPrice = _getPEGTombPrice();
            currentTaxRate = _updateTaxRate(currentPEGTombPrice);
            if (currentPEGTombPrice < burnThreshold) {
                burnTax = true;
            }
        }
