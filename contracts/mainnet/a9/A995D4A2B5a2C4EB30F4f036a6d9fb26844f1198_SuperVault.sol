...
contract SuperVault is AccessControl, Initializable {
  ...
  address public constant oneInch = 0x11111112542D85B3EF69AE05771c2dCCff4fAa26;
  address public constant paraswapProxy = 0x216B4B4Ba9F3e719726886d34a177484278Bfcae;
  ...
  ///@param aggregator Enum representing the aggregator to be used for selling PAR, either OneInch or Paraswap
  function leverage(
    address asset,
    ...
    bytes memory leverageParams = abi.encode(parToSell, dexTxData, aggregator);
  ...
  function rebalanceOperation(
    ...
    (uint256 vaultId, address toCollateral, uint256 parAmount, bytes memory dexTxData, Aggregator aggregator) = abi
      .decode(params, (uint256, address, uint256, bytes, Aggregator));
    aggregatorSwap(aggregator, fromCollateral, amount, dexTxData);
  ...
  function rebalance(
    ...
    bytes memory rebalanceParams = abi.encode(vaultId, toCollateral, parAmount, dexTxData, aggregator);
  ...
  function emptyVaultOperation(
    ...
    (uint256 vaultId, bytes memory dexTxData, Aggregator aggregator) = abi.decode(params, (uint256, bytes, Aggregator));

    aggregatorSwap(aggregator, vaultCollateral, amount, dexTxData); // swap assets for par to repay back loan
  ...
  ///@param aggregator Aggregator to use for swapping the vault collateral for par
  function emptyVault(
    uint256 vaultId,
    ...
    bytes memory emptyVaultParams = abi.encode(vaultId, dexTxData, aggregator);
  ...
  function leverageSwap(bytes memory params, IERC20 token) internal {
    (uint256 parToSell, bytes memory dexTxData, Aggregator aggregator) = abi.decode(
      params,
      (uint256, bytes, Aggregator)
    );
    token.approve(address(a.core()), 2**256 - 1);
    a.core().depositAndBorrow(address(token), token.balanceOf(address(this)), parToSell);
    IERC20 par = IERC20(a.stablex());
    aggregatorSwap(aggregator, par, parToSell, dexTxData);
  }

  ///@notice Helper function to approve and swap an asset using an aggregator
  ///@param aggregator The aggregator to use to swap
  ...
  function aggregatorSwap(
    ...
    if (aggregator == Aggregator.ONEINCH) {
      token.approve(oneInch, amount);
      oneInch.call(dexTxData);
    } else if (aggregator == Aggregator.PARASWAP) {
      token.approve(paraswapProxy, amount);
      paraswap.call(dexTxData);
    }
  }
