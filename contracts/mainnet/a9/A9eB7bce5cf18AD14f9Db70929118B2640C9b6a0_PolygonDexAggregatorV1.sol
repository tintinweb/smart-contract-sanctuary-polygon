...
import "./PolygonUniV2Dex.sol";
import "./PolygonUniV3Dex.sol";
...
contract PolygonDexAggregatorV1 is DelegateInterface, Adminable, DexAggregatorInterface, PolygonUniV2Dex, PolygonUniV3Dex {
    ...
    function sell(address buyToken, address sellToken, uint sellAmount, uint minBuyAmount, bytes memory data) external override returns (uint buyAmount){
        ...
        else if (data.toDex() == DexData.DEX_POLYGON_UNIV3) {
    ...
    function updateV3Observation(address desToken, address quoteToken, bytes memory data) external override {
        if (data.toDex() == DexData.DEX_POLYGON_UNIV3) {
...
contract PolygonUniV3Dex is IUniswapV3SwapCallback {
    using SafeMath for uint;
...
contract PolygonUniV2Dex {
    ...
    function getUniV2ClassPair(address tokenA, address tokenB, IUniswapV2Factory factory) internal view returns (address pair){
        ...
        if (address(factory) == 0xc35DADB65012eC5796536bD9864eD8773aBc74C4) {
            // sushi swap
        ...
        } else if (address(factory) == 0x5757371414417b8C6CAad45bAeF941aBc7d3Ab32){
            // quick swap
            pair = address(uint(keccak256(abi.encodePacked(
                    hex'ff',
                    factory,
