...
abstract contract ERC20Capped is ERC20, Teams {
    ...
    function setSupplyCap(uint256 _newCap) public onlyTeamOrOwner {
        require(_newCap >= ERC20.totalSupply(), "ERC20Capped: New cap must be greater than or equal to current minted supply.");
        _cap = _newCap;
    }
...
abstract contract Feeable is Teams {
  ...
  function getPrice(uint256 _count) public view returns (uint256) {
    require(_count >= 1 ether, "getPrice: _count must be at least 1. Input expects wei, not ether.");
...
contract ERC20Plus is 
    ...
    */
    function adminMint(address _to, uint256 _qty) public onlyTeamOrOwner{
        require(_qty >= 1 ether, "Must mint at least 1 token.");
        require(canMintAmount(_qty), "ERC20Capped: Minting over the capped supply is not allowed.");
        _mint(_to, _qty);
    }
    ...
    */
    function mintMany(address _to, uint256 _amount) public payable {
        ...
        require(_amount >= 1 ether, "Must mint at least 1 token.");
        require(inPublicMint(), "Public minting is not enabled currently.");
        require(canMintBatch(_amount), "ERC20MintCaps: Amount is over allowed batch size");
        require(canMintAmount(_amount), "ERC20Capped: Minting over the capped supply is not allowed.");
        require(canAccountMintAmount(_to, _amount), "ERC20MintCaps: Account has minted maximum allocation.");
        require(msg.value == getPrice(_amount), "Feeable: Value below required mint fee for amount");
    ...
    */
    function mintManyAL(address _to, uint256 _amount, bytes32[] calldata _merkleProof) public payable {
        require(onlyERC20MintingMode == false, "Only minting with ERC-20 tokens is enabled.");
        require(_amount >= 1 ether, "Must mint at least 1 token");
        require(inAllowlistMint(), "Allowlist minting is closed");
