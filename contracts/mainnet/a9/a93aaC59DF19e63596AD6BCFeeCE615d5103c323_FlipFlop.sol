...
contract OptionsVault is Withdraw, ITrufinVault {
        ...
        returns (address _toContinue)
    {
        ...
        while (user.user != ZERO_ADDRESS && i < size) {
            if (whiteListedUsers[user.user]) _calculateS_E_S_U(user);
            user = getNextUser();
            unchecked {
                i++;
            }
    ...
    function _redeemUserShares(UserDoubleLinkedList.User memory user) internal {
        ...
        (lock1, lock2) = FlipFlopLib.calcLockedAmount(
            user,
            minDepositOfAsset1,
            minDepositOfAsset2
        );
    ...
    function receiveSwappedFunds(
        ...
        tempSwap.asset = from;
        tempSwap.A_S = tempSwap.A - _A; // A_S is in asset from
        bool isAsset1 = asset1 == from;

        tempSwap.S = getInternalSwapRate(poolId);

        uint256 amountAsset1;
        uint256 amountAsset2;
        ...
        if (isAsset1) {
            ...
            uint256 one = 10**18;
            tempSwap.S = one.wadDiv(tempSwap.S);
    ...
    /// @notice In case when compute A is 0, so send to swap vault fails
    function skipSwap(
        address from,
        uint256 _S // asset 1 to asset 2
    ) external {
        _onlyKeeperShouldCall();
        swap.asset = from;
        swap.S = _S;
    ...
    /**
     * @notice Changes the flipflop state
     * @param newContractState is the new state of the contract
     */
    function updateState(FlipFlopStates newContractState) external {
        _onlyKeeperShouldCall();
...
contract Deposit is UserDoubleLinkedList {
    ...
    function depositMATIC() external payable {
        ...
        if (!whiteListedUsers[msg.sender]) whiteListedUsers[msg.sender] = true;
    }
    ...
    function depositMATICFor(address _toUser) external payable {
        ...
        if (!whiteListedUsers[_toUser]) whiteListedUsers[_toUser] = true;
    }
    ...
    function depositFor(address _forUser, uint256 _asset1Amount) external {
        ...
        if (!whiteListedUsers[_forUser]) whiteListedUsers[_forUser] = true;
    }
...
library FlipFlopLib {
    ...
    function _computeUserValues(
        ...
        if (temp.asset == assetDetails.asset1) {
            ...
            precisionWindow = Compute.scaleDecimals(
                10,
                assetDetails.decimalsOfAsset1,
                18
            );
            ...
            if (Compute.wadMul(e, temp.S) > u) {
                ...
                uint256 stack = temp.S1.wadMul(temp.A_S);
                {
                    ...
                    if (assetDetails.S_E < temp.A) {
                        if ((temp.A - assetDetails.S_E) < precisionWindow)
            ...
            if (temp.S.wadMul(e) > u) {
...

                {
                    ...
                    if (assetDetails.S_U < temp.A) {
                        if ((temp.A - assetDetails.S_U) < precisionWindow)
