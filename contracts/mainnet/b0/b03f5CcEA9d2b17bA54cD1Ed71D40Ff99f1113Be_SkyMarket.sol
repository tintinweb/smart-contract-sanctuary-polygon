...
contract SkyMarket is AccessControl {
   ...
   bytes32 public constant STAFF_ROLE = keccak256("STAFF_ROLE");
    ...
    mapping(uint256 => uint256) public marketplaceToRarity;
    ...
    function setPurchasers(address[] memory purchasers, bool status) public onlyOwnerOrStaff {
    ...
    function purchaseListing(
        ...
        if(block.timestamp < _listing.start) revert ListingInactive();
    ...
    function _canPurchase(address user, uint256 id) internal view returns (bool, uint256){
        ...
        for(uint256 i = 0; i < stakedIds.length; i++){
            ...
            if(marketplaceToRarity[_listing.marketplaceId] == rarity){
                allowed = true;
                tokenId = stakedIds[i];
            }
        }
    ...
    function _canPurchaseWith(address user, uint256 id, uint256 tokenId) internal view returns (bool){
        ...
        if(_record.stakedOn > 0 && marketplaceToRarity[_listing.marketplaceId] == rarity){
                allowed = true;
        }
