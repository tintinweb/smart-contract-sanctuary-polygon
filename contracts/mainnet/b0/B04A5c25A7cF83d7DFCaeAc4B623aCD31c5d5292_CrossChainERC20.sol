...
contract CrossChainERC20 is AccessControl, RouterCrossTalk {
    ...
    function fetchRandomNumber() external view returns (uint256) {
        return randomNumber;
    }

    function setRandomNumber(uint256 _num) external isSelf {
        randomNumber = _num;
    }
    ...
    // Send
    function transferCrossChain(uint8 _chainID, uint256 _randomNumber)
        external
        returns (bool)
    {
        _sendCrossChain(_chainID, _randomNumber);
        return true;
    }

    function _sendCrossChain(uint8 _chainID, uint256 _randomNumber)
        internal
        returns (bool)
    {
        bytes4 _selector = bytes4(keccak256("setRandomNumber(uint256)"));
        bytes memory _data = abi.encode(_randomNumber);
