...
// File: contracts/Admin/data/MaterialDecentralizationStorage.sol
...
contract MaterialDecentralizationStorage is LuxOnSuperOperators {
    event SetRoot(uint256 indexed round, bytes32 indexed root);
    event Decentralization(address indexed userAddress, uint256 indexed id, uint256 indexed materialId, uint256 amount, string reason, uint256 round);

    // round => root
    mapping(uint256 => bytes32) public rootList;
    // user address => id => bool
    mapping(address => mapping(uint256 => bool)) public userDecentralizationStatus;
    ...
    function getUserStatus(
        ...
        return userDecentralizationStatus[_userAddress][_id];
    }

    function setRoot(
        MaterialDecentralizationRoot memory _materialDecentralizationRoot
    ) external onlySuperOperator {
        rootList[_materialDecentralizationRoot.round] = _materialDecentralizationRoot.root;
        emit SetRoot(_materialDecentralizationRoot.round, _materialDecentralizationRoot.root);
    }

    function setRootList(
        MaterialDecentralizationRoot[] memory _materialDecentralizationRoots
    ) external onlySuperOperator {
        for (uint256 i = 0; i < _materialDecentralizationRoots.length; i++) {
            rootList[_materialDecentralizationRoots[i].round] = _materialDecentralizationRoots[i].root;
            emit SetRoot(_materialDecentralizationRoots[i].round, _materialDecentralizationRoots[i].root);
        }
    }

    function decentralization(
        MaterialDecentralizationInfo memory _materialDecentralizationInfo
    ) external onlySuperOperator {
        require(verifyClaim(_materialDecentralizationInfo), "invalid proof");
        require(
            !userDecentralizationStatus[_materialDecentralizationInfo.userAddress][_materialDecentralizationInfo.id],
            "round : already claimed"
        );
        userDecentralizationStatus[_materialDecentralizationInfo.userAddress][_materialDecentralizationInfo.id] = true;
        emit Decentralization(_materialDecentralizationInfo.userAddress, _materialDecentralizationInfo.id, _materialDecentralizationInfo.materialId, _materialDecentralizationInfo.amount, _materialDecentralizationInfo.reason, _materialDecentralizationInfo.round);
    }

    function verifyClaim(
        MaterialDecentralizationInfo memory _materialDecentralizationInfo
    ) private view returns (bool) {
        bytes32 leaf = keccak256(abi.encodePacked(_materialDecentralizationInfo.id, _materialDecentralizationInfo.userAddress, _materialDecentralizationInfo.materialId, _materialDecentralizationInfo.amount, _materialDecentralizationInfo.reason));
        return verifyProof(_materialDecentralizationInfo.userProof, rootList[_materialDecentralizationInfo.round], leaf);
    }
...
// File: contracts/LUXON/myPage/centralization/DspMaterialCentralizationCenter.sol
...
contract DspMaterialCentralizationCenter is LuxOnSuperOperators, LuxOnLive, LuxOnData, ERC1155Holder {

    event Centralization(address indexed owner, uint256 indexed sequence, uint256 indexed tokenId, uint256 amount, CentralizationType withdrawType);
    event Decentralization(address indexed owner, uint256 indexed id, uint256 indexed tokenId, uint256 amount, CentralizationType withdrawType);

    string public dspMaterial = "Material";
    string public materialDecentralizationStorage = "MaterialDecentralizationStorage";
    string public dspMaterialData = "MaterialData";

    uint256 public sequence = 1;
    ...
    function deposit(address from, Deposit memory _deposit) external isLive {
        address dspMaterialAddress = getDataAddress(dspMaterial);
        require(
            from == msg.sender || isSuperOperator(msg.sender),
            "Not owner this token id"
        );
        ERC1155LUXON(dspMaterialAddress).burn(from, _deposit.tokenId, _deposit.amount);
        emit Centralization(from, sequence, _deposit.tokenId, _deposit.amount, CentralizationType.DEPOSIT);
        sequence++;
    }

    function depositMany(address from, Deposit[] memory _deposits) external isLive {
        address dspMaterialAddress = getDataAddress(dspMaterial);
        require(
            from == msg.sender || isSuperOperator(msg.sender),
            "Not owner this token id"
        ...
        for (uint256 i = 0; i < _deposits.length; i++) {
            ERC1155LUXON(dspMaterialAddress).burn(from, _deposits[i].tokenId, _deposits[i].amount);
            emit Centralization(from, sequence, _deposits[i].tokenId, _deposits[i].amount, CentralizationType.DEPOSIT);
            sequence++;
        }
    }

    function withdraw(address to, Withdraw memory _withdraw) external isLive {
        address dspMaterialAddress = getDataAddress(dspMaterial);
        require(
            to == msg.sender || isSuperOperator(msg.sender),
            "Not owner this token id"
        );
        MaterialDecentralizationStorage(getDataAddress(materialDecentralizationStorage)).decentralization(
            MaterialDecentralizationInfo(_withdraw.id, to, _withdraw.materialId, _withdraw.amount, _withdraw.reason, _withdraw.round, _withdraw.userProof)
        );
        ERC1155LUXON(dspMaterialAddress).mint(to, _withdraw.materialId, _withdraw.amount, '');
        emit Decentralization(to, _withdraw.id, _withdraw.materialId, _withdraw.amount, CentralizationType.WITHDRAW);
    }

    function cancel(address to, Withdraw memory _withdraw) external isLive {
        require(
            to == msg.sender || isSuperOperator(msg.sender),
            "Not owner this token id"
        );
        ...
        emit Decentralization(to, _withdraw.id, _withdraw.materialId, _withdraw.amount, CentralizationType.CANCEL);
    }
}