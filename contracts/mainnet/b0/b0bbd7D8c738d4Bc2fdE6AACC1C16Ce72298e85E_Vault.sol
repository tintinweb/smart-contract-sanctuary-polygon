...
abstract contract GameRole is Context {
    ...
    modifier onlyOwner() {
        require(
            owner() == _msgSender(),
            "Vault GameRole: caller is not the owner"
        );
        _;
    }
     ...
     */
    modifier onlyGame() {
        require(_isGame[_msgSender()], "Vault GameRole: caller is not a game");
        _;
    }
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(
            newOwner != address(0),
            "Vault GameRole: new owner is the zero address"
        );
    ...
    function addGame(address newGame) public onlyOwner {
        ...
        require(!_isGame[newGame], "Vault GameRole: this address is game");
    ...
    function removeGame(address game) public onlyOwner {
        require(_isGame[game], "Vault GameRole: this address is not game");
...
abstract contract VaultBase is GameRole, ReentrancyGuard {
    bool internal _gameIsLive = false;
    string internal _name;

    // Modifiers
    modifier onlyLive() {
        require(_gameIsLive, "Vault ERR: Vault is not live");
        _;
    }
    ...
    function receiveETH() external payable onlyLive onlyGame nonReentrant returns (bool) {
        return true;
    }
    ...
    function transfer(
        ...
        address _to,
        uint256 _amount
    ) external onlyLive onlyGame nonReentrant returns (bool) {
        require(_amount > 0, "Vault ERR: Invalid zero amount");
        if (_token == address(0)) {
...
contract Vault is VaultBase {
    constructor() {
        _name = "Game Vault - crystal.network";
        _gameIsLive = true;
    }
}