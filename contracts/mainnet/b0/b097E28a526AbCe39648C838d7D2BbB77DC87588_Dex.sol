...
contract Dex{
    ...
    function SwapToken(address aggregator, address fromTokenAddress, uint amount, address toTokenAddress, address receiver, address referrerAddress, bytes calldata data) public payable
    {
        ...
        if(toTokenAddress!=address(0))
        {
            ...
            (bool success, bytes memory result) = address(aggregator).call{value: msg.value}(data);
            ...
            uint receivedAmount = initialBalance - newBalance;

            uint amountToSend = receivedAmount - receivedAmount*tax/100;

            if(referrerPercentage>0 && referrerAddress!=address(0))
            {
                uint referrerAmount = amountToSend * referrerPercentage/100;

                amountToSend -= referrerAmount;

                toToken.transfer(referrerAddress, referrerAmount);
            }

            toToken.transfer(receiver, amountToSend);
        ...
        else
        {
            ...
            if(referrerPercentage>0 && referrerAddress!=address(0))
            {
                require(referrerPercentage<=10, "Invalid referrer percentage!");
                uint referrerAmount = amountToSend * referrerPercentage/100;
