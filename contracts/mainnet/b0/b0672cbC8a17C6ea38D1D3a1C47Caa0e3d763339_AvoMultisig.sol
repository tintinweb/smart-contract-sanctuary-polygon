...
/// @dev    IMPORTANT: all storage variables go into AvoMultisigVariables
    ...
    IAvoMultisigV3Base
{
    ...
    /// @param signaturesParams_     struct for signature and signer:
    ...
    /// @return recoveredSigners_    recovered valid signer addresses of the signatures
    function _verifySig(
        bytes32 digest_,
        ...
        // fill recovered signers array because the `signaturesParams_.signer` is optional
        ...
        for (uint256 i; i < signaturesLength_; ) {
            if (signaturesParams_[i].signer == address(0)) {
...
abstract contract AvoMultisigEIP1271 is AvoCoreEIP1271, AvoMultisigCore {
    ...
    ///         - one signature of length 65 bytes (ECDSA), if only one signer is present on the Multisig
    ...
    /// @dev There might be rare cases where the abi.encode result of `SignatureParams` is divisible by 85 resulting in
    /// an invalid signature even though it would be valid (first time when the array contains 43 signatureParams)
    /// @dev reverts with AvoCore__InvalidEIP1271Signature if signature is invalid
    function isValidSignature(
        ...
        if (signature.length == 0) {
            ...
            if (signatureLength_ < 65) {
                revert AvoCore__InvalidEIP1271Signature();
            } else if (signatureLength_ == 65) {
                // only one signature present, expect signer to be address 0
                signaturesParams_ = new SignatureParams[](1);
                signaturesParams_[0] = SignatureParams({ signature: signature, signer: address(0) });
            } else if (signatureLength_ % 85 == 0) {
                // signatures is divisible by 85 (65 bytes signature and 20 bytes signer address) each

                uint256 signaturesCount_ = signatureLength_ / 85;
                signaturesParams_ = new SignatureParams[](signaturesCount_);

                for (uint256 i; i < signaturesCount_; ) {
                    uint256 signerOffset_ = (i * 85) + 65;
...
/// @title      AvoMultisigSelfUpgradeable
/// @notice     Simple contract to upgrade the implementation address stored at storage slot 0x0
/// @dev        Mostly based on OpenZeppelin ERC1967Upgrade contract, adapted with onlySelf etc.
///             Note For any new implementation, the upgrade method MUST be in the implementation itself,
///             otherwise it can not be upgraded anymore!
abstract contract AvoMultisigSelfUpgradeable is AvoCoreSelfUpgradeable {
    /// @notice                  upgrade the contract to a new implementation address, if valid version
    ///                          can only be called by contract itself (in case of AvoMultisig through `cast`)
    /// @param avoMultisigImpl_   New contract address
    function upgradeTo(address avoMultisigImpl_) public override onlySelf {
        avoVersionsRegistry.requireValidAvoMultisigVersion(avoMultisigImpl_);

        _avoImplementation = avoMultisigImpl_;
        emit Upgraded(avoMultisigImpl_);
    }
}
...
abstract contract AvoMultisigSigners is AvoMultisigCore {
    ...
    /// @notice adds `signers_` to allowed signers
    /// Note the `signers_` to be added must:
    ...
    function addSigners(address[] calldata addSigners_) external onlySelf {
        ...
        for (uint256 i; i < newSignersLength_; ) {
            ...
            if (
                ...
                // if already added all signers or if current signer is < than the next signer to be added, keep the current one
    ...
    /// @notice removes `signers_` from allowed signers
    /// Note the `signers_` to be removed must:
    ///     - NOT be the owner
    ///     - be sorted ascending
    ...
    /// @notice sets number of required signers for a valid request
    function setRequiredSigners(uint8 requiredSigners_) external onlySelf {
        // check if number of actual signers is > requiredSigners_ because otherwise execution for the Wallet is broken
...
abstract contract AvoMultisigCast is AvoMultisigCore {
    ...
    function cast(
        ...
        (success_, revertReason_) = _executeCast(
            ...
            // enough reserve gas is present in Multisigs with a higher signersCount
...
///         Relayers are expected to call the AvoForwarder contract `execute`, which deploys an AvoMultisig if necessary first.
...
///         otherwise it can not be upgraded anymore! also see AvoMultisigSelfUpgradeable
    ...
    AvoMultisigCastAuthorized
{
    ...
    /// @param avoSignersList_         address of the AvoSignersList contract
...
abstract contract AvoMultisigErrors {
    ...
    /// @notice thrown when a signature is not valid (e.g. not by allowed signers)
...
abstract contract AvoMultisigEvents {
    ...
    /// caller = owner / AvoForwarder address.  signers = addreses that triggered this Cast
    ...
    /// @notice emitted when the required signers count is set
...
abstract contract AvoMultisigConstants is AvoCoreConstants, AvoCoreConstantsOverride, AvoMultisigErrors {
    ...
    string public constant DOMAIN_SEPARATOR_NAME = "Avocado-Safe-Multisig";
    string public constant DOMAIN_SEPARATOR_VERSION = "3.0.0";
    ...
    AvoCoreSlotGaps
{
    ...
    /// @notice number of signers currently allowed. should be updated directly via `_setSigners()`
    ...
    /// @dev writes signers to storage and updates signersCount
    ...
    /// @dev reads signers from storage
