...
import {IERC721EnumerableUpgradeable} from
    "openzeppelin-contracts-upgradeable/contracts/interfaces/IERC721EnumerableUpgradeable.sol";
...
import {NFTV1} from "medallion/brevity/token/NFTV1.sol";

import {IProductV1} from "./IProductV1.sol";
import {ProductV1} from "./ProductV1.sol";
import {ProductV1Types} from "./ProductV1Types.sol";

contract ProductV1Shadow is ProductV1 {
    /// @notice The product to shadow
    address private _shadow;

    /// @notice `shadowBlocked` reverts in a standard way for blocked methods
    modifier shadowBlocked() {
        if (_shadow != address(0)) {
            revert("Shadow: method blocked");
        }
        _;
    }

    /// @notice `shadowRequired` reverts if the contract doesn't have a shadow
    modifier shadowRequired() {
        require(_shadow != address(0), "Shadow: no shadow");
        _;
    }
    ...
    /// @notice `setShadow` set the shadow contract address
    /// @param shadow the shadow contract address
    function setShadow(address shadow) public onlyRole(ADMIN_ROLE) {
        require(IERC165(shadow).supportsInterface(type(INFTHistoryV1).interfaceId), "Shadow: invalid shadow");
        require(IERC165(shadow).supportsInterface(type(IERC721Enumerable).interfaceId), "Shadow: invalid shadow");
        require(IERC165(shadow).supportsInterface(type(IERC721).interfaceId), "Shadow: invalid shadow");
        _shadow = shadow;
    }
    ...
    /// @dev shadowed
    function numTransfers(uint256 tokenId) public view override shadowRequired returns (uint256) {
        return INFTHistoryV1(_shadow).numTransfers(tokenId);
    }

    /// @inheritdoc INFTHistoryV1
    /// @dev shadowed
    function issuedAt(uint256 tokenId) public view override shadowRequired returns (uint256) {
        return INFTHistoryV1(_shadow).issuedAt(tokenId);
    }

    /// @inheritdoc INFTHistoryV1
    /// @dev shadowed
    function transferredAt(uint256 tokenId, int256 index) public view override shadowRequired returns (uint256) {
        return INFTHistoryV1(_shadow).transferredAt(tokenId, index);
    }

    /// @inheritdoc IERC721EnumerableUpgradeable
    /// @dev shadowed
    function tokenOfOwnerByIndex(address owner, uint256 index) public view override shadowRequired returns (uint256) {
        return IERC721Enumerable(_shadow).tokenOfOwnerByIndex(owner, index);
    }

    /// @inheritdoc IERC721EnumerableUpgradeable
    /// @dev shadowed
    function totalSupply() public view override shadowRequired returns (uint256) {
        return IERC721Enumerable(_shadow).totalSupply();
    }

    /// @inheritdoc IERC721EnumerableUpgradeable
    /// @dev shadowed
    function tokenByIndex(uint256 index) public view override shadowRequired returns (uint256) {
        return IERC721Enumerable(_shadow).tokenByIndex(index);
    }
    ...
    function balanceOf(address owner)
        ...
        returns (uint256)
    {
        return IERC721(_shadow).balanceOf(owner);
    }
    ...
    function ownerOf(uint256 tokenId)
        ...
        returns (address)
    {
        return IERC721(_shadow).ownerOf(tokenId);
    }

    /* ------------------------------ OVERRIDE ------------------------------ */

    /// @inheritdoc ProductV1
    function isEligible(address /* recipient */ ) public pure override returns (bool) {
        return false;
    }

    /// @notice `tokenVariant` deterministically maps tokens to variants
    /// @dev Reverts for non-existant tokens
    /// @param tokenId The tokenId
    /// @return The token variant
    function tokenVariant(uint256 tokenId) public view override returns (bytes32) {
        uint256 issuedAt_ = issuedAt(tokenId);
        uint256 randomIndex = uint256(keccak256(abi.encodePacked(issuedAt_, tokenId))) % totalVariants();

        return variantByIndex(randomIndex).id;
    }

    /// @inheritdoc ProductV1
    function totalSupply(bytes32 variantId) public view override returns (uint256) {
        uint256 totalSupply_ = totalSupply();
        if (totalVariants() == 1) {
            return totalSupply_;
        }

        uint256 variantSupply_;
        for (uint256 i = 0; i < totalSupply_; i++) {
            if (tokenVariant(tokenByIndex(i)) == variantId) {
                variantSupply_++;
            }
        }
        return variantSupply_;
    }

    /// @inheritdoc ProductV1
    function variantInventory(bytes32 variantId) public view override returns (ProductV1Types.Inventory memory) {
        uint256 total = totalSupply(variantId);

        return ProductV1Types.Inventory({availableInventory: 0, totalSupply: total});
    }

    /// @inheritdoc NFTV1
    /// @dev shadowed to make tokenURI work
    function _checkTokenExists(uint256 tokenId) internal view override shadowRequired {
        ownerOf(tokenId);
    }
    ...
    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function amendVariantInventory(bytes32 variantId, uint256 newInventory)
        public
        override
        onlyRole(ADMIN_ROLE)
        shadowBlocked
    ...
    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function freezeSupply() public override onlyRole(OPERATOR_ROLE) shadowBlocked {}

    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function freezeSupply(bytes32 variantId) public override onlyRole(OPERATOR_ROLE) shadowBlocked {}

    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function issue(address recipient, bytes32 variantId)
        public
        override
    ...
    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function issueMany(address[] calldata recipients, bytes32 variantId)
        public
        override
        onlyRole(OPERATOR_ROLE)
        shadowBlocked
    ...
    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function issueRandom(address recipient, uint256 seed)
        public
        override
        onlyRole(OPERATOR_ROLE)
        shadowBlocked
    ...
    /// @inheritdoc ProductV1
    /// @dev shadow blocked
    function setIssueLimit(uint256 newIssueLimit) public override onlyRole(OPERATOR_ROLE) shadowBlocked {}

    /// @inheritdoc NFTV1
    /// @dev shadow blocked
    function _issue(address recipient) internal override shadowBlocked returns (uint256) {}

    /// @inheritdoc ERC721Upgradeable
    /// @dev shadow blocked
    function _beforeTokenTransfer(address from, address to, uint256 tokenId) internal override shadowBlocked {}

    /// @inheritdoc NFTV1
    /// @dev shadow blocked
    function setRoyaltyProvider(address newRoyaltyProvider) public override onlyRole(ADMIN_ROLE) shadowBlocked {}
...
import {IRoyaltyProviderV1} from 'medallion/brevity/royalty/IRoyaltyProviderV1.sol';
import {IRoyaltySubscriberV1} from 'medallion/brevity/royalty/IRoyaltySubscriberV1.sol';
    ...
    IRoyaltySubscriberV1
{
    ...
    /// @notice the address of the royalty provider (if any) for this contract's tokens
    address private _royaltyProvider;
    ...
    /// @inheritdoc IRoyaltySubscriberV1
    function setRoyaltyProvider(address newRoyaltyProvider) public virtual onlyRole(ADMIN_ROLE) {
        if (newRoyaltyProvider != address(0)) {
            require(
                IERC165(newRoyaltyProvider).supportsInterface(type(IRoyaltyProviderV1).interfaceId),
                "NFT: invalid royalty provider"
            );
        }
        address oldRoyaltyProvider = _royaltyProvider;
        _royaltyProvider = newRoyaltyProvider;

        emit RoyaltyProviderUpdated(oldRoyaltyProvider, newRoyaltyProvider);
    }
    ...
    /// @inheritdoc INFTHistoryV1
    function numTransfers(uint256 tokenId) public virtual view returns (uint256) {
        return _transferredAt[tokenId].length;
    }

    /// @inheritdoc INFTHistoryV1
    function issuedAt(uint256 tokenId) public virtual view returns (uint256) {
    ...
    /// @inheritdoc INFTHistoryV1
    function transferredAt(uint256 tokenId, int256 index) public virtual view returns (uint256) {
    ...
    function royaltyInfo(uint256 tokenId, uint256 salePrice)
        ...
        returns (address receiver, uint256 royaltyAmount)
    {
        if (_royaltyProvider == address(0)) {
            return (address(0), 0);
        }
        IRoyaltyProviderV1 royaltyProvider = IRoyaltyProviderV1(_royaltyProvider);

        return royaltyProvider.royaltyInfo(address(this), tokenId, salePrice);
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlSubscriberV1, IERC165)
        returns (bool)
    {
        return
            ...
            interfaceId == type(IRoyaltySubscriberV1).interfaceId ||
    ...
    function _checkTokenExists(uint256 tokenId) internal virtual view {
    ...
    function _issue(address recipient) internal virtual returns (uint256) {
...
interface IProductV1 {
    ...
    /// @notice Issues multiple tokens for a specific variant
    /// @dev Reverts if any individual issue fails, e.g. due to eligibility
    /// @param recipients The token recipients
    /// @param variantId The variant id
    /// @return uint256 The new tokenIds
    function issueMany(address[] calldata recipients, bytes32 variantId) external returns (uint256[] memory);
    ...
    /// @notice Check if the recipient is eligible to be issued a product
    /// @return boolean Whether the recipient is eligible
    function isEligible(address recipient) external view returns (bool);
...
contract ProductV1 is Initializable, NFTV1, IProductV1 {
    using StringsUpgradeable for uint256;

    /// @notice The variant metadata base URI
    string private _variantBaseURI;

    /// @notice list of all variants
    ProductV1Types.Variant[] private _variants;

    /// @notice mapping of variantId to 1-indexed position in list of variants
    mapping(bytes32 => uint256) private _variantsIndex;

    /// @notice mapping of tokenId to variantId
    mapping(uint256 => bytes32) private _tokenVariants;

    /// @notice mapping of variantId to tokenIds
    mapping(bytes32 => uint256[]) private _variantTokens;

    /// @notice mapping of tokenId to index in variant token list
    mapping(uint256 => uint256) private _tokenVariantIndex;

    /// @notice Limit the number of product which can be issued to an address
    uint256 private _issueLimit = 0;
    ...
    /// @notice Initializes the product implementation
    /// @param name_ The ERC721 name
    /// @param symbol_ The ERC721 symbol
    /// @param contractURI_ The contract metadata URI
    /// @param tokenURI_ The token metadata URI
    /// @param variantBaseURI_ The variant metadata base URI
    /// @param accessControlProvider_ The access control provider address
    /// @param variants_ List of variants to create
    function __ProductV1_init(
        string calldata name_,
        string calldata symbol_,
        string calldata contractURI_,
        string calldata tokenURI_,
        string calldata variantBaseURI_,
        address accessControlProvider_,
        ProductV1Types.Variant[] memory variants_
    ) public virtual initializer {
        __NFTV1_init(name_, symbol_, contractURI_, tokenURI_, accessControlProvider_);
        _variantBaseURI = variantBaseURI_;

        // build variants data set
        for (uint256 i = 0; i < variants_.length; i++) {
            if (variants_[i].id == 0x0) {
                break;
            }

            require(variants_[i].initialInventory >= 0, 'Product: invalid inventory');
            _variants.push(variants_[i]);
            _variantsIndex[variants_[i].id] = i + 1;
        }
        require(_variants.length > 0, 'Product: no variants provided');
    }
    ...
    function amendVariantBaseURI(string calldata newVariantBaseURI) public onlyRole(ADMIN_ROLE) {
        _variantBaseURI = newVariantBaseURI;
    }

    /// @notice Amends the total inventory for a variant
    /// @dev Only callable by ADMIN_ROLE
    /// @dev Be careful, downstream systems may not be aware of the change
    /// @dev The new inventory amount must be at least the current supply
    /// @param variantId The variant to amend inventory for
    /// @param newInventory The new inventory amount for the variant
    function amendVariantInventory(bytes32 variantId, uint256 newInventory) public virtual onlyRole(ADMIN_ROLE) {
        uint256 index = _variantIndex(variantId);
        uint256 supply = totalSupply(variantId);
        require(newInventory >= supply, 'Product: new inventory too low');
        _variants[index].initialInventory = newInventory;
    }

    /* ------------------------------ OPERATOR ------------------------------ */

    /// @inheritdoc IProductV1
    function freezeSupply() public virtual onlyRole(OPERATOR_ROLE) {
        for (uint256 i = 0; i < _variants.length; i++) {
            freezeSupply(_variants[i].id);
        }
    }

    /// @inheritdoc IProductV1
    function freezeSupply(bytes32 variantId) public virtual onlyRole(OPERATOR_ROLE) {
        uint256 index = _variantIndex(variantId);
        uint256 supply = totalSupply(variantId);
        _variants[index].initialInventory = supply;
    }

    /// @inheritdoc IProductV1
    function issue(address recipient, bytes32 variantId) public virtual onlyRole(OPERATOR_ROLE) returns (uint256) {
        require(variantInventory(variantId).availableInventory > 0, 'Product: product unavailable');

        uint256 tokenId = _issue(recipient);
        _assignVariant(tokenId, variantId);
    ...
    function issueMany(address[] calldata recipients, bytes32 variantId)
        ...
        returns (uint256[] memory)
    {
        uint256 inventory = variantInventory(variantId).availableInventory;
        require(inventory >= recipients.length, 'Product: product unavailable');
        uint256[] memory tokenIds = new uint256[](recipients.length);
        for (uint256 i = 0; i < recipients.length; i++) {
            uint256 tokenId = _issue(recipients[i]);
            _assignVariant(tokenId, variantId);
            tokenIds[i] = tokenId;
        }
    ...
    /// @inheritdoc IProductV1
    function issueRandom(address recipient, uint256 seed) public virtual onlyRole(OPERATOR_ROLE) returns (uint256) {
        require(productInventory().availableInventory > 0, 'Product: product unavailable');

        uint256 tokenId = _issue(recipient);
        _assignRandomVariant(tokenId, seed);

        return tokenId;
    }

    /// @inheritdoc IProductV1
    function setIssueLimit(uint256 newIssueLimit) public virtual onlyRole(OPERATOR_ROLE) {
        _issueLimit = newIssueLimit;
    }

    /* ------------------------------- PUBLIC ------------------------------- */

    /// @inheritdoc IProductV1
    function isEligible(address recipient) public virtual view returns (bool) {
        if (_issueLimit == 0) {
            return true;
        }

        return balanceOf(recipient) < _issueLimit;
    }

    /// @inheritdoc IProductV1
    function productInventory() public view returns (ProductV1Types.Inventory memory) {
        ProductV1Types.Inventory memory productInventory_;
        for (uint256 i = 0; i < _variants.length; i++) {
            ProductV1Types.Inventory memory variantInventory_ = variantInventory(_variants[i].id);
            productInventory_.availableInventory += variantInventory_.availableInventory;
            productInventory_.totalSupply += variantInventory_.totalSupply;
        }

        return productInventory_;
    }

    /// @inheritdoc IProductV1
    function tokenVariant(uint256 tokenId) public virtual view returns (bytes32) {
        return _tokenVariants[tokenId];
    }

    /// @inheritdoc IProductV1
    function totalVariants() public view returns (uint256) {
        return _variants.length;
    }

    /// @inheritdoc IProductV1
    function totalSupply(bytes32 variantId) public virtual view returns (uint256) {
        return _variantTokens[variantId].length;
    }

    /// @inheritdoc IProductV1
    function variantByIndex(uint256 index) public view returns (ProductV1Types.Variant memory) {
        require(index >= 0 && index < _variants.length, 'Product: variant not found');

        return _variants[index];
    }

    /// @inheritdoc IProductV1
    function variantInventory(bytes32 variantId) public virtual view returns (ProductV1Types.Inventory memory) {
        uint256 index = _variantIndex(variantId);
        ProductV1Types.Variant memory variant_ = _variants[index];

        uint256 total = totalSupply(variant_.id);
        uint256 available = variant_.initialInventory - total;

        return ProductV1Types.Inventory({availableInventory: available, totalSupply: total});
    }

    /// @inheritdoc IProductV1
    function variantURI(bytes32 variantId) public view virtual override returns (string memory) {
        return string(abi.encodePacked(_variantBaseURI, variantId));
    }

    /// @inheritdoc ERC721Upgradeable
    /// @dev Replaces static Product token URI with the usual dynamic one
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
    ...
    /// @notice Assigns variant to tokenId
    /// @dev Does not check if the token or variant exist
    /// @param tokenId The tokenId
    /// @param variantId The variantId
    function _assignVariant(uint256 tokenId, bytes32 variantId) internal {
        _tokenVariants[tokenId] = variantId;
        _tokenVariantIndex[tokenId] = _variantTokens[variantId].length;
        _variantTokens[variantId].push(tokenId);
    }

    /// @notice Assigns random available variant to tokenId
    /// @dev Does not check if the token exists
    /// @param tokenId The tokenId
    /// @return variantId The assigned variant ID
    function _assignRandomVariant(uint256 tokenId, uint256 seed) internal returns (bytes32) {
        bytes32 variantId;
        uint256 numVariants = totalVariants();

        // Single variant isn't random
        if (numVariants == 1) {
            variantId = _variants[0].id;
        } else {
            // Get available inventory and pick a random item
            ProductV1Types.Inventory memory inventory_ = productInventory();
            uint256 random = uint256(keccak256(abi.encodePacked(block.number, block.timestamp, seed))) %
                inventory_.availableInventory;

            // Find variant for random item
            uint256 counter = 0;
            for (uint256 i = 0; i < numVariants; i++) {
                counter += variantInventory(_variants[i].id).availableInventory;
                if (counter > random) {
                    variantId = _variants[i].id;
                    break;
                }
            }
        }
    ...
    function _authorizeIssue(address recipient) internal virtual override {
        require(isEligible(recipient), 'Product: unauthorized');
    }

    /// @notice Get variant index
    /// @dev Reverts if variant not found
    /// @param variantId The variantIdentifier
    function _variantIndex(bytes32 variantId) internal view returns (uint256) {
        require(_variantsIndex[variantId] > 0, 'Product: variant not found');

        return _variantsIndex[variantId] - 1;
    }
    ...
    IOwnableV1
{
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual returns (bool) {
        return
            interfaceId == type(IAccessControlSubscriberV1).interfaceId || interfaceId == type(IOwnableV1).interfaceId;
    }
...
/// @notice Generic interface to provide EIP-2981 royalties for any token contract
interface IRoyaltyProviderV1 {
    /// @notice Calculate EIP-2981 royalty info for a given sale
    /// @dev Should throw when token address is not supported by the provider
    /// @param token_ The address of the token contract being sold
    /// @param tokenId_ The specific token being sold
    /// @param salePrice_ The undenominated sale price
    /// @return royaltyRecipient The recipient of the royalty fee
    /// @return royaltyAmount The royalty fee to collect
    function royaltyInfo(address token_, uint256 tokenId_, uint256 salePrice_)
        external
        view
...
/// @notice Interface for token contract subscribing to royalty provider
interface IRoyaltySubscriberV1 {
    /// @notice Emitted when the royalty provider is updated
    event RoyaltyProviderUpdated(address indexed from, address indexed to);

    /// @notice Set the royalty provider for this contract
    /// @dev Throws if `newRoyaltyProvider` doesn't implement `IRoyaltyProviderV1`
    /// @param newRoyaltyProvider The royalty provider address
    function setRoyaltyProvider(address newRoyaltyProvider) external;
}
