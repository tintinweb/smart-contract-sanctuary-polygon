...
abstract contract AnycallClientBase is PausableControlWithAdmin {
    ...
    modifier onlyExecutor() {
        require(msg.sender == address(executor), "AnycallClient: not authorized executor");
        _;
    }
    ...
    constructor(address _admin, address _callProxy) PausableControlWithAdmin(_admin) {
        ...
        executor = IExecutor(IAnycallV6Proxy(_callProxy).executor()); 
    }
...
contract AnyswapTokenAnycallClient is AnycallClientBase {
    ...
    // Change vault of a controlled token 
     function changeVault(address token, address newVault) public onlyAdmin returns (bool) {
        require(newVault != address(0), "Client: address(0x0)");
        return IAnyswapToken(token).changeVault(newVault);
    }
    ...
    {
        (address _from,,) = executor.context();
