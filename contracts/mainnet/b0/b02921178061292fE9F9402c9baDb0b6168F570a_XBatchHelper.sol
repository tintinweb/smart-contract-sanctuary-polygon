...
contract XBatchHelper {
  ...
  address public XPAddress = 0xf1b5AF2bd94dDfB696B54DaBb06118A79C7460bc;
  IXP MYXP = IXP(XPAddress);

  

  function batchUpdateScore(ScoreUpdate[] memory scoreBatch) external {
    for (uint256 i = 0; i < scoreBatch.length; i++) {
      (bool success, bytes memory data) = XPAddress.call(
        abi.encodeWithSignature(
          "updateScore(bytes32,bytes32,string,string,address)",
          scoreBatch[i]._updateId,
          scoreBatch[i]._projectId,
          scoreBatch[i]._actionName,
          scoreBatch[i]._scoreType,
          scoreBatch[i]._targetWallet
        )
      );
