...
/**
 * @title Restaurant Relief Oracle
 * @dev Stores information about the nominated restaurants, votes, and money
 * raised.
 */
contract RestaurantReliefOracle is PublicOracle, IRestaurantReliefOracle {
    ...
    /**
     * @dev returns the number of restaurants that have been registered.
     * @dev use with `getRestaurantAtIndex()` to iterate.
     */
    ...
    /**
     * @dev returns the restaurant at the index.
     * @dev use with `getRestaurantCount()` to iterate.
     */
    function getRestaurantAtIndex(uint256 index)
        public
        view
    ...
    /**
     * @dev returns the restaurant for an id.
     * @param restaurantId the primary key from the offchain database.
     */
    function getRestaurantById(uint256 restaurantId)
        public
        view
    ...
    /**
     * @dev register a new restaurant or update an existing one.
     * @dev if `restaurant.restaurantId` matches a restaurant that has already
     * been registered, it will be updated with the new data.
     * @param restaurant the restaurant data
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - the `restaurant.restaurantId` MAY refer to an existing restaurant. In
    ...
    /**
     * @dev register and/or update a batch of restaurants.
     * @dev if any `restaurantId` matches a restaurant that has already been
     * registered, it will be updated with the new data.
     * @param newRestaurants the restaurant data
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - the `restaurantId` for any element MAY refer to an existing
     *     restaurant. In this case, the name and location will be updated, but
    ...
    /**
     * @dev record a vote for a restaurant
     ...
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `restaurantId` MUST refer to a registered restaurant
     * - `amountRaised` MAY be bytes32(0), in which case the value is not
     *     updated for the restaurant.
     ...
     *     previous restaurant and added to the new one.
     */
    function recordVoteAndProgress(
        ...
        _verifyRestaurant(restaurantId);
    ...
    /**
     * @dev record batch of votes for a restaurant
     * @param _tokenIds the NFT casting the vote
     * @param _restaurantIds the primary key from the offchain database.
     ...
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `_tokenIds` and `_restaurantIds` MUST be the same length
     * - `_amountsRaised` MUST either be an empty array or be the same length
     *     as `_tokenIds`
     * - Every element of `restaurantIds` MUST refer to a registered restaurant
     * - `_amountsRaised` MAY be an empty array, in which case the values are
     *     not updated for any restaurant.
     * - Elements of `amountRaised` MAY be bytes32(0), in which case the value
     *     is not updated for the restaurant at the same index.
     * - `updatedProgress` MAY be an empty string, in which case the value is
     *     not updated.
    ...
    function recordVotesAndProgress(
        ...
        for (uint256 i = 0; i < _restaurantIds.length; i++) {
            _verifyRestaurant(_restaurantIds[i]);
     ...
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `_amountsRaised` and `_restaurantIds` MUST be the same length
     * - Every element of `restaurantIds` MUST refer to a registered restaurant
     * - Elements of `amountRaised` MAY be bytes32(0), in which case the value
     *     is not updated for the restaurant at the same index.
     * - `updatedProgress` MAY be an empty string, in which case the value is
    ...
    function updateFundraisingTotals(
        ...
        require(
            _amountsRaised.length == 0 ||
                _restaurantIds.length == _amountsRaised.length,
            "array length mismatch"
        );

        for (uint256 i = 0; i < _restaurantIds.length; i++) {
            _verifyRestaurant(_restaurantIds[i]);
            _setAmountRaised(restaurants[_restaurantIds[i]], _amountsRaised[i]);
        }
        _setProgress(_updatedProgress);
    }

    /**
     * @dev sets the vote totals to 0 for all restaurants.
    ...
    /**
     * @dev returns the restaurant struct nominated by `tokenId`.
     */
    function restaurantNominatedBy(uint256 tokenId)
        public
        view
    ...
    /**
     * @dev returns the restaurant struct voted by `tokenId`.
     */
    function restaurantVotedBy(uint256 tokenId)
        public
        view
    ...
    function _addRestaurant(RestaurantInfo memory restaurant) internal {
        if (restaurantIds.contains(restaurant.restaurantId)) {
    ...
    function _updateRestaurant(RestaurantInfo memory restaurant) internal {
        RestaurantInfo storage existing = restaurants[restaurant.restaurantId];
        existing.restaurantId = restaurant.restaurantId;
        existing.restaurantName = restaurant.restaurantName;
        existing.restaurantName2 = restaurant.restaurantName2;
        existing.location = restaurant.location;
    }

    function _verifyRestaurant(uint256 restaurantId) internal view {
        require(
            restaurantId != 0 && restaurantIds.contains(restaurantId),
            "invalid restaurant"
        );
    }
    ...
    function _recordVote(
        ...
        if (existingVote != 0 && restaurantIds.contains(existingVote)) {
    ...
    function _recordVoteAndProgress(
        ...
        _setAmountRaised(restaurant, amountRaised);
    }
