...
/**
 * @title Atlantis World Sale v2 nft holder contract
 * @notice Contract can be used for claiming the nfts during the club sale v1. The contract allows minting in 1,2 or 3 sets of batches.abi
 * @author Rachit Anand Srivastava
 */

contract PostClubSaleContract is Ownable, Pausable {
    /// @notice The starting token Id available
    uint256 tokenId = 1529;

    /// @notice The mint price for a key = 0.22 ETH
    uint256 public MINT_PRICE = (22 * 1e18) / 100;

    /// @notice KEYS Contract and WETH Contract
    IERC721 keysContract;
    IERC20 weth;

    /// @notice For assigning an address the right to withdraw funds
    address private targetAddress;

    address publicVerificationAddress;
    ...
    event MINT_NFT(uint256 numberOfNfts, address minter);

    /**
     * @param startingTokenId The starting token id issues by the contract
     * @param _keysContract The address of the keys contract
     * @param _WETH The address of the keys contract
     * @param _publicVerificationAddress The verification address used to check the signing address
     * @param _treasuryAddress Atlantis World treasury address
     */
    constructor(
        uint256 startingTokenId,
        IERC721 _keysContract,
        IERC20 _WETH,
        address _publicVerificationAddress,
        address _treasuryAddress
    ) {
        tokenId = startingTokenId;
        keysContract = _keysContract;
        weth = _WETH;
        publicVerificationAddress = _publicVerificationAddress;
        treasuryAddress = _treasuryAddress;
    }

    /// @notice Returns the avalaible nfts
    function availableNFTs(address addr) public view returns (uint256) {
        return 3 - alreadyMinted[addr];
    }
    ...
    /**
     * @param numberOfNfts The total number of nfts to be minted -> range [1,3]
     * @param nonce The nonce returned by the backend
     * @param signature The signature signed and return from our atlantis world server
     */
    function mint(
        ...
        uint256 available = availableNFTs(msg.sender);

        require(available >= numberOfNfts, "Not enough nfts available");

        require(
            weth.transferFrom(
                msg.sender,
                treasuryAddress,
                numberOfNfts * MINT_PRICE
            ),
            "Insufficient Funds"
        );

        for (uint256 x = 1; x <= numberOfNfts; x++) {
            keysContract.safeTransferFrom(address(this), msg.sender, tokenId++);
        }

        alreadyMinted[msg.sender] += numberOfNfts;

        emit MINT_NFT(numberOfNfts, msg.sender);
    }
     ...
     */
    function selfTransferKeys(address _treasuryAddress)
        external
        onlyOwner
        whenNotPaused
    {
