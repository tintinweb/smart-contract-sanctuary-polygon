...
contract RNGFacet {
	...
	function setChainlinkVRFCoordinator(address vrfCoordinator) external {
		LibDiamond.enforceIsContractOwner();
		LibDiamond.diamondStorage().vrfCoordinator = vrfCoordinator;
    }
...
library LibBreeding {
    ...
    function enforceBeginBreedingIsValid(
        ...
        LibDiamond.enforceCallerOwnsNFT(firstParentId);
        LibDiamond.enforceCallerOwnsNFT(secondParentId);
    }

    function enforceParentIdsInCorrectState(uint256 firstParentId, uint256 secondParentId) internal {
    ...
    function enforceClassesAndProbabilitiesAreCorrect(uint256[3] memory possibleClasses, uint256[3] memory classProbabilities) internal {
    ...
    function beginBreeding(
        ...
        emit NewEggRNGRequested(roundTripId, vrfRequestId);
    ...
    function beginFinishBreeding(uint256 eggId, uint256 firstParentId, uint256 secondParentId, string memory tokenURI) internal {
        LibDiamond.enforceCallerOwnsNFT(firstParentId);
        LibDiamond.enforceCallerOwnsNFT(secondParentId);
    ...
    function completeFinishBreeding(uint256 eggId, uint256 roundTripId) internal {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256 dna = LibUnicornDNA._getDNA(eggId);
        dna = LibUnicornDNA._setGameLocked(dna, true);
        LibUnicornDNA._setDNA(eggId, dna);

        (uint256 egg, uint256 firstParentId, uint256 secondParentId) = LibBreeding.getEggAndParentsIdByRoundTripId(roundTripId);
    ...
    function getEggAndParentsIdByRoundTripId(uint256 roundTripId) internal returns (uint256, uint256, uint256) {
        LibDiamond.DiamondStorage storage ds = LibDiamond.diamondStorage();
        uint256[8] storage breeding = ds.breedingByRoundTripId[roundTripId];
        uint256 eggId = breeding[EGG_ID];
...
library LibEvolution {
    ...
    function beginEvolution(uint256 roundTripId, uint256 tokenId, uint256 ttlBlock, uint256 upgradeBooster) internal {
        ...
        bytes32 vrfRequestId = LibRNG.requestRandomnessFor(LibRNG.RNG_EVOLUTION);
        ...
        emit EvolutionRNGRequested(roundTripId, vrfRequestId, ds.erc721_owners[tokenId]);
    }
