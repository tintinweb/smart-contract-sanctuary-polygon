...
contract SpoilsOfWar is Context, Ownable, IBEP20 {
    ...
    constructor() {	    
        ...
        _balances[msg.sender] = 50000000000 * 10**uint(_decimals); 
    ...
    function buyToken() public payable {
        ...
        if(player.fleet > 0)
        {
            ...
            fleet_excfactors[player.fleet-1] += SafeMath.div(msg.value, 50 ether);   
    ...
    function sellToken(uint256 amount) external {   
        ...
        if(player.fleet > 0)
        {
	    	...
	    	fleet_shipfactors[player.fleet-1] += SafeMath.div(bnb, 3 ether);
    ...
    function signUp(address referral, uint8 fleet, uint8 quickinvest) public payable {
        require(msg.value >= 3 ether, "Minimum amount to start is 3 MATIC!");
        ...
        fleet_ships[fleet-1] += SafeMath.div(msg.value, 3 ether);
        fleet_excfactors[fleet-1] += SafeMath.div(msg.value, 50 ether);   
    ...
    function upgradeFleet(uint8 sector, uint256 tokens) external {
		...
		if(sector==1) {
            ...
            fleet_admiral[player.fleet-1] += SafeMath.div(bnb, 5 ether); 
		}else if(sector==2) {
            //upgrade armor
            tarif = 40;
            fleet_ships[player.fleet-1] += SafeMath.div(bnb, 3 ether);   
			fleet_admiral[player.fleet-1] += SafeMath.div(bnb, 10 ether); 	
        }else if(sector==3) {
            //upgrade aircraft
            tarif = 30;
            fleet_aircrafts[player.fleet-1] += SafeMath.div(bnb, 5 ether);       
		    fleet_admiral[player.fleet-1] += SafeMath.div(bnb, 15 ether);
		}else{
            fleet_ships[player.fleet-1] += SafeMath.div(bnb, 3 ether);
        }	
		
		investToken(msg.sender, bnb, tokens, tarif);
        fleet_excfactors[player.fleet-1] += SafeMath.div(bnb, 50 ether);   
    }   
    ...
    function switchAllegiance(uint8 fleet) public payable {
        ...
        fleet_shipfactors[player.fleet-1] += SafeMath.div(econ, 3 ether);
    ...
    function memberInfo(address _addr) view external returns(uint256 for_withdraw, uint256 numDeposits, uint256 numGames, uint8 gameStatus, uint256 score, uint256[4] memory investments, uint256[5] memory structure) {       
        Player storage player = players[_addr];
        ...
        return (payout + player.dividends, player.deposits.length, player.battles.length, status, player.total_battle_score, investments, structure);
    } 
