 ...
 *
 */
contract MultiSigPoolWithdrawMultiToken is ReentrancyGuard {
  ...
  function deposit(
    ...
    if (address(token) == USDC_ADDRESS){   // deposit USDC 
      ...
      bytes4 exchangeSelector = abi.decode(exchangeData[0:4], (bytes4));
      require(exchangeSelector == IAggregationRouterV4.swap.selector, 'invalid swap selector');
  ...
  function withdrawErc20(
    ...
    if (token != USDC_ADDRESS && tokenBalance < amount){ 
      // swap to dest token
      (, IAggregationRouterV4.SwapDescription memory desc,) = abi.decode(exchangeData[4:], (address, IAggregationRouterV4.SwapDescription, bytes));
      require(USDC_ADDRESS == address(desc.srcToken), 'mismatch token and desc.srcToken');
      require(token == address(desc.dstToken), 'invalid desc.dstToken');
      require(amount == desc.amount, 'mismatch amount and desc.amount');
      require(to == desc.dstReceiver, 'invalid desc.dstReceiver');
      ...
      // Swap token
      (bool success, )= AGGREGATION_ROUTER_V4_ADDRESS.call(exchangeData);
      require(success, "exchange failed");
    }else{
      returnAmount = amount;
      IERC20(token).safeTransfer(to, returnAmount);
    }
