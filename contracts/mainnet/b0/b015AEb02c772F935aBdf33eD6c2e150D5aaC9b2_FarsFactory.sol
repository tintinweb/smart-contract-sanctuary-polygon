...
contract FarsFactory is IFactory {
  ...
  function setPauser(address _pauser) external {
    require(msg.sender == pauser, "FarsFactory: Not pauser");
    pendingPauser = _pauser;
  }

  function acceptPauser() external {
    require(msg.sender == pendingPauser, "FarsFactory: Not pending pauser");
  ...
  function pairCodeHash() external pure override returns (bytes32) {
    return keccak256(type(FarsPair).creationCode);
  }
  ...
  external override returns (address pair) {
    require(tokenA != tokenB, 'FarsFactory: IDENTICAL_ADDRESSES');
    (address token0, address token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
    require(token0 != address(0), 'FarsFactory: ZERO_ADDRESS');
    require(getPair[token0][token1][stable] == address(0), 'FarsFactory: PAIR_EXISTS');
    // notice salt includes stable as well, 3 parameters
    bytes32 salt = keccak256(abi.encodePacked(token0, token1, stable));
    (_temp0, _temp1, _temp) = (token0, token1, stable);
    pair = address(new FarsPair{salt : salt}());
