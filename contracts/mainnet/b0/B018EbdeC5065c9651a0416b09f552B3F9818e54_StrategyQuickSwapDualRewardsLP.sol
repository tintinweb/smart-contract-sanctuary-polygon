...
import "../../interfaces/strategy/IStakingDualRewards.sol";
import "../../interfaces/strategy/IDragonsLair.sol";
...
contract StrategyQuickSwapDualRewardsLP is StrategyManager, FeeManager, GasThrottler {
    ...
    event HarvesterPaid(address indexed harvester, uint256 amount);
    event ProfitCollected(address indexed token, uint256 amount);
    ...
    constructor(
        ...
        address _unirouter,
        address _keeper,
        address _profitFeeCollector,
        address[] memory _outputToNativeRoute,
        address[] memory _nativeToLp0Route,
        address[] memory _nativeToLp1Route
    ) public StrategyManager(_keeper, _unirouter, _vault, _profitFeeCollector) {
        want = _want;
    ...
    function chargeFees(address callFeeRecipient) internal {
        uint256 _outputToNative = IERC20(output).balanceOf(address(this));
        if (_outputToNative > 0) {
        ...
        uint256 nativeBal = IERC20(native).balanceOf(address(this)).mul(performanceFee).div(10000);

        uint256 harvesterFeeAmount = nativeBal.mul(harvesterFee).div(MAX_FEE);
        IERC20(native).safeTransfer(callFeeRecipient, harvesterFeeAmount);
        emit HarvesterPaid(callFeeRecipient, harvesterFeeAmount);

        uint256 performanceFeeAmount = nativeBal.mul(MAX_FEE.sub(harvesterFee)).div(MAX_FEE);
        IERC20(native).safeTransfer(profitFeeCollector, performanceFeeAmount);
        emit ProfitCollected(native, performanceFeeAmount);
    }
    ...
    function addLiquidity() internal {
        ...
        if (lpToken0 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, nativeToLp0Route, address(this), block.timestamp);
        }

        if (lpToken1 != output) {
            IUniswapRouterETH(unirouter).swapExactTokensForTokens(outputHalf, 0, nativeToLp1Route, address(this), block.timestamp);
        }
    ...
    function callReward() public view returns (uint256) {
        ...
        uint256 nativeOut;
        if (outputBal > 0) {
            try IUniswapRouterETH(unirouter).getAmountsOut(outputBal, outputToNativeRoute) returns (uint256[] memory amountOut) {
        ...
        return nativeOut.mul(performanceFee).div(10000).mul(harvesterFee).div(MAX_FEE);
    }
...
contract GasThrottler {
    ...
    address public gasprice = address(0xa152599e3Eb2bcbE242ee15420B5116EfdCe4127);
...
contract StrategyManager is Ownable, Pausable {
    /**
     * @dev Strategy Contracts:
     * {keeper} - Address to manage the strategy.
     * {vault} - Address of the vault that controls the strategy's funds.
     * {unirouter} - Address of exchange to execute swaps.
     * {profitFeeCollector} - Address of exit fee's collector
     */
    address public keeper;
    address public unirouter;
    address public vault;
    address public profitFeeCollector;
     ...
     * @param _profitFeeCollector address for collecting profit's fee
     */
    constructor(
        address _keeper,
        address _unirouter,
        ...
        vault = _vault;
        profitFeeCollector = _profitFeeCollector;
    }
    ...
    /**
     * @dev Updates profit fee recipient.
     * @param _collector new exit fee recipient address.
     */
    function setProfitFeeCollector(address _collector) external onlyOwner {
        profitFeeCollector = _collector;
    }
...
abstract contract FeeManager is StrategyManager {
    /// @dev Fee variables
    uint256 public performanceFee = 500;
    uint256 public harvesterFee = 100;

    /// @dev Fee cap
    uint256 public constant MAX_PERFORMANCE_FEE = 2000;

    uint256 public constant MAX_FEE = 10000;
    uint256 public constant MAX_HARVESTER_FEE = 100; // BP based on performanceFee
    ...
    function setHarvesterFee(uint256 _fee) external onlyManager {
        require(_fee <= MAX_HARVESTER_FEE, "!cap");
        harvesterFee = _fee;
    }
}
