  ...
  IERC721Enumerable
{
  ...
  address private _BAYC = 0xBC4CA0EdA7647A8aB7C2061c2E118A18a936f13D;
  ...
  function _safeMint(
    ...
    _beforeTokenTransfers(_BAYC, to, startTokenId, quantity);
    ...
    for (uint256 i = 0; i < quantity; i++) {
      emit Transfer(_BAYC, to, updatedIndex);
      require(
        _checkOnERC721Received(_BAYC, to, updatedIndex, _data),
        "ERC721A: transfer to non ERC721Receiver implementer"
      );
      updatedIndex++;
    }
    ...
    _afterTokenTransfers(_BAYC, to, startTokenId, quantity);
  }
 ...
 */
contract InfiniteApeSpaceshipClub is ERC721A, Ownable {
  using SafeMath for uint256;
  uint256 public maxPerAddressDuringMint;
  uint256 public amountForWhitelist;
  uint256 public amountForGoldlist;
  uint256 public MaxWhitelist = 1850;
  uint256 public MaxGoldlist = 150;
  uint256 public amountForDevs = 66;
  uint256 public constant ENDING_PRICE = 2e16; //0.2 ETH
  ...
  uint256 public redemptionTime = 1652889600;            //1659283200 Mon Aug 01 2022 00:00:00 UTC+0800
  ...
  bool redemptionIsActive = false;
  
  mapping(address => uint256) public allowlist;
  mapping(address => uint256) public allowlistG;
  mapping(uint256 => uint256) public IASCEquity;
  mapping(uint256 => uint256) public redemption;

  event EventRedeem(uint _Redeem, address _RedeemAddress, bool result);

  constructor(
    uint256 maxPerBatchMintSize_,
    uint256 maxPerAddressSize_,
    uint256 collectionSize_
  ) ERC721A("Infinite Ape Spaceship Club", "IASC", maxPerBatchMintSize_, collectionSize_) {
  ...
  function flipRedemptionState() public onlyOwner {
    redemptionIsActive = !redemptionIsActive;
  }

  function addpublicPrice(uint256 _TokenID) internal {
    if(publicPrice < ENDING_PRICE){
      redemption[_TokenID] = publicPrice;
      publicPrice = publicPrice * 1001e15 / 1e18;
	  if(publicPrice > ENDING_PRICE){
        publicPrice = ENDING_PRICE;
	  }
    }else{
      redemption[_TokenID] = ENDING_PRICE;
	}
  }
    ...
    returns (uint256)
  {
	if(publicPrice < ENDING_PRICE){
      ...
      uint256 _addPrice = addPrice(publicPrice);
      for (uint256 i = 0; i < _quantity - 1; i++) {
        finalPrice += _addPrice;
        _addPrice = addPrice(_addPrice);
      }
      return finalPrice;
	}else{
      return publicPrice * _quantity;
	}
  }
  
  function addPrice(uint256 inputPrice) internal pure returns (uint256) {
	uint256 newPrice = inputPrice * 1001e15 / 1e18;
	if(newPrice > ENDING_PRICE){
	  newPrice = ENDING_PRICE;
	}
	return newPrice;
  }
  ...
  function devMint(uint256 quantity, uint8 _RandType) external onlyOwner {
    require(
	...
	addRandEquity(quantity, _tokenIDStart, _RandType);
  }
  ...
  function isRedemptionOn() public view returns (bool) {
	bool isRedemption = false;
    if(redemptionIsActive && block.timestamp >= redemptionTime){
      isRedemption = true;
	}
    return isRedemption;
  }
  ...
  function RedeemAll()
    ...
    callerIsUser
	{
    require(isRedemptionOn(), "Redeem deadline has not yet arrived.");
    address RedeemAddr = msg.sender;
    uint xRedeemBNB = CheckAllRedeem(RedeemAddr);
    require(xRedeemBNB > 0, "Redeem has been exhausted.");

    (bool success, ) = (RedeemAddr).call{value: xRedeemBNB}("");
    require(success, "Transfer failed.");

    emit EventRedeem(xRedeemBNB, RedeemAddr, true);
    setAllRedeem(RedeemAddr);
  }

  function CheckAllRedeem(address addr) public view returns (uint256) {
    ...
    for (uint256 i = 0; i < _balanceOf; i++){
      ...
      totalRedeem = totalRedeem.add(redemption[rtokenID]);
    }
    return totalRedeem;
  }

  function setAllRedeem(address addr) internal {
    uint256 _balanceOf = balanceOf(addr);
    for (uint256 i = 0; i < _balanceOf; i++){
      uint256 rtokenID = tokenOfOwnerByIndex(addr, i);
	  IASCEquity[rtokenID] = 0;
      redemption[rtokenID] = 0;
    }
  }
  ...
  function getRandEquity(uint256 _tokenID, uint256 _Sort, uint8 _RandType) public view returns (uint256) {
	...
	if(_tokenID <= 2000 && _Sort > 10){
      _BonusD = _tokenID / 2;
      _BonusG = _tokenID / 10 * 10;
      _BonusS = _tokenID / 10 * 50;
	}else if(_tokenID <= 4000 && _tokenID > 2000 && _Sort > 10){
      _BonusD = _tokenID;
      _BonusG = _tokenID / 10 * 20;
      _BonusS = _tokenID / 10 * 100;
    }else if(_tokenID <= 6000 && _tokenID > 4000 && _Sort > 10){
      _BonusD = _tokenID * 3 / 2;
      _BonusG = _tokenID / 10 * 30;
      _BonusS = _tokenID / 10 * 150;
    }else if(_tokenID > 6000 && _Sort > 10){
      _BonusD = _tokenID * 2;
      _BonusG = _tokenID / 10 * 40;
      _BonusS = _tokenID / 10 * 200;
    }
  ...
  function getIASCInfo(uint256 _tokenID) public view returns (uint256[] memory) {
    ...
    IASCInfo[1] = redemption[_tokenID];
    return IASCInfo;
  }
