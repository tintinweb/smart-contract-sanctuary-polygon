    ...
    MinterAmmStorageV4
{
    ...
    //Emitted when AddressesProvider is updated
    event AddUserToWhiteList(address userAddress);

    //Emitted when AddressesProvider is updated
    event SetWhiteListed(bool permissioned);
     ...
     */
    function addUserToWhiteList(address _userAddress)
        external
        override
        onlyOwner
    {
        ...
        emit AddUserToWhiteList(_userAddress);
    }

    /**
     * @notice allows the OtokenFactory module to whitelist a new option
     * @dev can only be called from the OtokenFactory address
     * @param _whiteListed set to if amm is whitelisted or not
     */
    function setWhiteListedAmm(bool _whiteListed) external override onlyOwner {
        isWhiteListed = _whiteListed;

        emit SetWhiteListed(_whiteListed);
    }
}
