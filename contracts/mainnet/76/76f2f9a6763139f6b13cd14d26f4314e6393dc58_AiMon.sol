...
contract AiMon is ERC721ABurnableUpgradeable, OwnableUpgradeable {
    ...
    function mintTo(uint8 quantity, address receiver) external payable {
        ...
        pmon.transferFrom(msg.sender, address(this), quantity);
    }
        ...
        returns (string memory)
    {
        ...
        if (random == 0) {
            return
                string(
                    abi.encodePacked(
                        'data:application/json;utf8,{"status" : "random number pending"}'
                    )
                );
        } else {
    ...
    function _createCreature(
        ...
        // Guidance
        newCreature.guidance = uint8((guidanceRandomNumber % 18) + 3); // [3, 20]

        // Denoising
        newCreature.denoising = uint8(denoisingRandomNumber % 2); // [0, 1]
