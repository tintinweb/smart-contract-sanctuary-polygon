...
interface MCCrosschainServices{
...
contract BaseLevel is IBaseLevel, OwnableUpgradeable {
    ...
    function getBaseLevel(address _user) public view returns (uint _level){
        ...
        if(superAppAddress == address(0) || superTokenAddress == address(0) || superTokenLiteAddress == address(0)) revert BaseLevel__SuperAddressNotSetUp();
        uint constantLevel = 6;
        if(MCCrosschainServices(crosschainServicesAddress).getNewlandsGenesisBalance(_user) != 0){
            return genesisLevel;
        }
        ...
        if (sfLevel != 0 || sfLevelLite != 0) return (uint(uint96(sfLevel + sfLevelLite)) / superFluidPerLevel) + constantLevel;
    ...
    /// @inheritdoc IBaseLevel
    function setOrderCapacity(address _trader, uint _fromLevel, uint _toLevel, uint _additionalOrders) override external onlyOwner {
    ...
    /// @inheritdoc IBaseLevel
    function setSuperAppAddress(address _superAppAddress) override external onlyOwner {
        superAppAddress = _superAppAddress;
    }

    /// @inheritdoc IBaseLevel
    function setSuperTokens(address _superToken, address _superTokenLite) override external onlyOwner{
        superTokenAddress = _superToken;
        superTokenLiteAddress = _superTokenLite;
    }

    /// @inheritdoc IBaseLevel
    function setSuperFluidPerLevel(uint _superFluidPerLevel) override external onlyOwner {
