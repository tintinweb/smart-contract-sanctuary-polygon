...
abstract contract Manager is Ownable {
    ...
    // Events
    event BetPlaced(uint indexed betId, address indexed player, uint amount, uint choice, bool isBonus, uint note);
    event BetSettled(uint indexed betId, address indexed player, uint amount, uint choice, uint outcome, uint winAmount);
    ...
    function amountToWinnableAmount(uint _amount, uint multiplier) internal view returns (uint) {
...
contract Crash is ReentrancyGuard, Manager, IGame {
    function placeBet(uint multiplierChoice, uint bonus, uint note) external payable nonReentrant {
        ...
        uint winnableAmount = amountToWinnableAmount(amount, multiplierChoice);
        ...
        emit BetPlaced(betId, msg.sender, amount, multiplierChoice, isBonus, note);   
        bets.push(Bet({
    ...
    function _settleBet(uint betId, uint256 randomNumber) private nonReentrant {
        ...
        emit BetSettled(betId, player, amount, multiplierChoice, multiplierOutcome, winAmount);
    }
    ...
    function refundBet(uint betId) external nonReentrant {
        ...
        uint winnableAmount = amountToWinnableAmount(amount, bet.choice);
