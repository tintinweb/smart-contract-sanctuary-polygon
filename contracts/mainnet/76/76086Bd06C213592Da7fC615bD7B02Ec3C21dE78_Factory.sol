    ...
    AccessControl
{
    ...
    function _prepaidPostpaid(LoanLib.Loan memory _details) internal {
        uint256 totalAmt = _details.borrowerShare + _details.foundationShare; 

        if (_details.isPrepaid) {
            Paza.transferFrom(_details.bank, defiWallet, (totalAmt * defiSharePercent) / PCT_BASE);

            Paza.transferFrom(
                _details.bank,
                cefiWallet,
                ((totalAmt) * cefiSharePercent) / PCT_BASE
            );
            ...
            Paza.safeTransfer(defiWallet, (totalAmt * defiSharePercent) / PCT_BASE);
            Paza.safeTransfer(
                pools[_details.bank],
                _details.borrowerShare
            );
