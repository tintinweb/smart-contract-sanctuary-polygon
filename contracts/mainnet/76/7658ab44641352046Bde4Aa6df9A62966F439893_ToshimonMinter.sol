...
contract ToshimonMinter is
  ERC1155,
  AccessControlMixin
{
  using SafeMath for uint256;
  using Strings for string;
  ToshimonMinterOld public toshimonMinterOld;
  ...
  constructor(
    ...
    toshimonMinterOld = ToshimonMinterOld(0xd2d2a84f0eB587F70E181A0C4B252c2c053f80cB);
    _setupRole(DEFAULT_ADMIN_ROLE, _msgSender());
    _setupRole(MINTER_ROLE, _msgSender());
    _setupRole(WHITELIST_ADMIN_ROLE, _msgSender());
    _contractURI = 'https://api.toshimon.io/cards/';
  }
  ...
  function uri(uint256 _id) public  view virtual override returns (string memory) {
    require(maxSupply(_id) > 0, 'ERC721Tradable#uri: NONEXISTENT_TOKEN');
    return string(abi.encodePacked(contractURI(), uint2str(_id)));
  }
 ...
 function setContractURI(string memory newURI) public only(WHITELIST_ADMIN_ROLE) {
  ...
  external
    only(MINTER_ROLE) {
    uint256 _amount = toshimonMinterOld.balanceOf(_account, _id);
    require(_amount > 0, "no cards");
    toshimonMinterOld.burn(_account, _id, _amount );
    mint(_account, _id, _amount, '');
  }
