    ...
    IAffiliate
{
    ...
    /**
     * @notice Owner: Set `maxReinforcementShare_` as the new maximum amount of reserves that can be locked by all bets
     *         for the same condition.
     */
    function changeMaxReinforcementShare(uint64 maxReinforcementShare_)
        external
        onlyOwner
    {
        ...
        emit MaxReinforcementShareChanged(maxReinforcementShare_);
    }
...
interface IBetExpress is IBet, IOwnable {
    ...
    event MaxReinforcementShareChanged(uint64 newMaxReinforcementShare);
