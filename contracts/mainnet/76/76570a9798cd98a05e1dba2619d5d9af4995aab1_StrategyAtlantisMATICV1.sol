...
abstract contract StrategyBase {
    ...
    //Sushi
    address public sushiRouter = 0x93bcDc45f7e62f89a8e901DC4A0E2c6C427D9F25;//comethswap
...
interface IComptroller {
    ...
    function atlantisAccrued(address) external view returns (uint256);
    ...
    function claimAtlantis(address holder, address[] calldata aTokens) external;
...
contract StrategyAtlantisMATICV1 is StrategyBase, Exponential {
    // constants
    address public constant comptroller = 0x8F85EE1c0a96734cb76870106DD9C016db6dE09A;
    address public constant aMATIC = 0xA65722af4957CeF481Edb4cB255f804DD36E8aDc;
    address public constant ATLX = 0x0b68782eFF3177f1F9240B64A7e2F8E0497e2454;

    // Require some buffer between market collateral factor and strategy's collateral factor when leveraging
    ...
    public StrategyBase(WMATIC, _governance, _strategist, _controller, _timelock)
    {
        ...
        IERC20(want).safeApprove(WMATIC, uint256(-1));
        IERC20(want).safeApprove(aMATIC, uint256(-1));
    }
    ...
    function getName() external override pure returns (string memory) {
        return "StrategyAtlantisMATICV1";
    }

    function getSuppliedView() public view returns (uint256) {
        (, uint256 cTokenBal, , uint256 exchangeRate) = ICToken(aMATIC).getAccountSnapshot(address(this));
        (, uint256 bal) = mulScalarTruncate(Exp({mantissa: exchangeRate}), cTokenBal);
        return bal;
    }

    function getBorrowedView() public view returns (uint256) {
        return ICToken(aMATIC).borrowBalanceStored(address(this));
    }
    ...
    function getMarketColFactor() public view returns (uint256) {
        (, uint256 colFactor) = IComptroller(comptroller).markets(aMATIC);
        return colFactor;
    }
    ...
    // To safeguard with enough buffer, we divide the borrow rate by 2 (half days in one year) which indicates allowing 48 hours response time
    ...
    function getCompAccrued() public returns (uint256) {
        uint256 accrued = IComptroller(comptroller).atlantisAccrued(address(this));
        return accrued;
    }
    ...
    function getSupplied() public returns (uint256) {
        return ICToken(aMATIC).balanceOfUnderlying(address(this));
    }

    function getBorrowed() public returns (uint256) {
        return ICToken(aMATIC).borrowBalanceCurrent(address(this));
    }
    ...
    function leverageUntil(uint256 _supplyAmount, uint256 _maxLeverage, uint256 _unleveragedSupply) public onlyKeepers {
        ...
        while (supplied < _supplyAmount) {
            ...
            require(ICEther(aMATIC).borrow(_borrowAndSupply) == 0, '!borrowInLeverage');
    ...
    function deleverageUntil(uint256 _supplyAmount) public onlyKeepers {
        ...
        do {
            ...
            require(ICEther(aMATIC).redeemUnderlying(_redeemAndRepay) == 0, "!redeemInDeleverageUntil");
            ICEther(aMATIC).repayBorrow{value: _redeemAndRepay}();
    ...
    function harvest() public override onlyBenignCallers {
        address[] memory ctokens = new address[](1);
        ctokens[0] = aMATIC;
        IComptroller(comptroller).claimAtlantis(address(this), ctokens);
        
        // convert reward to WMATIC(want)
        uint256 _wantBal = balanceOfWant();
        uint256 _atlx = IERC20(ATLX).balanceOf(address(this));
        if (_atlx > 0) {
            _swapUniswap(ATLX, want, _atlx);
        }
        ...
        if (buybackEnabled == true && _buybackAmount > 0) {
            buybackAndNotify(WMATIC, _buybackAmount);
    ...
    function deposit() public override{
        ...
        if (_want > 0) {
            ...
            ICEther(aMATIC).mint{value: _want}();
            require(IERC20(aMATIC).balanceOf(address(this)) > 0, "!deposit");
        }
    }
    ...
    function _withdrawSome(uint256 _amount) internal override returns (uint256){
        ...
        require(ICToken(aMATIC).getCash() >= _redeem, "!cash-liquidity");
        ...
        // Redeems underlying
        require(ICToken(aMATIC).redeemUnderlying(_redeem) == 0, "!redeemInWithdrawSome");
    ...
    function withdraw(IERC20 _asset) external override returns (uint256 balance) {
        require(aMATIC != address(_asset), "!cToken");
