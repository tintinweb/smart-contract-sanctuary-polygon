...
contract ERC1155Marketplace is ERC1155YuzuInternal, ReentrancyGuard {
    ...
    {
        require(_commissionRate + _royaltiesCommissionRate < 100, "XPNFT: total commissions should be lower than 100");
        admin = _admin;
    ...
    function sell(uint256 tokenId, uint256 qty, uint256 price, address wallet) external {
        ...
        // Limit the sell to owned items, the user cannot sell items do not have
        require(balanceOf(_msgSender(), tokenId) >= qty, "XPNFT: you do not have enough tokens to sell");
    ...
    function buy(uint256 tokenId, uint256 index, uint256 qty) payable external nonReentrant {
        ...
        // validate user
        require(openSales[tokenId][index].user != _msgSender() , "XPNFT: the user cannot buy his own offer");

        uint256 total = qty * openSales[tokenId][index].price;

        // require enough funds
        require(msg.value >= total, "XPNFT: payment is too low");
    ...
    function cancelSale(uint256 tokenId, uint256 index) external {
        ...
        require(openSales[tokenId][index].user == _msgSender(), "XPNFT: only the original seller can cancel his sales");
    ...
    function cancelBid(uint256 tokenId, uint256 index) external nonReentrant {
        ...
        // Only the original bidder can cancel his bids
        require(openOffers[tokenId][index].user == _msgSender(), "XPNFT: only the original bidder can cancel his bids");
    ...
    function acceptBid(uint256 tokenId, uint256 index, uint256 qty) external nonReentrant {
        ...
        // validate user
        require(openOffers[tokenId][index].user != _msgSender() , "XPNFT: the user cannot accept his own bid");
    ...
    function callOptionalReturn(IERC1155 token, bytes memory data) private {
        ...
        require(address(token).isContract(), "XPNFT: call to non-contract");

        // solhint-disable-next-line avoid-low-level-calls
        (bool success, bytes memory returndata) = address(token).call(data);
        require(success, "XPNFT: low-level call failed");

        if (returndata.length > 0) { // Return data is optional
            // solhint-disable-next-line max-line-length
            require(abi.decode(returndata, (bool)), "XPNFT: ERC20 operation did not succeed");
        }
    }
 ...
 */
contract XP1155NativeMarket is ERC1155Marketplace, EIP712 {
    string private constant SIGNING_DOMAIN = "XPNFT-Voucher";
    ...
    function redeem(
        ...
        require(chainId == getChainId(), "XPNFT: Wrong chain");
        require(vouchersMintedQty[id] + qtyToMint <= qty, "XPNFT: the qty exceeds the amount available for minting");
        require(nftId == 0 || exists(nftId), "XPNFT: Wrong NFT ID");

        // require enough funds
        require(msg.value >= qtyToMint * price, "XPNFT: payment is too low");
    ...
    function verify(
        ...
        require(recoveredSigner == signer, "XPNFT: wrong signature");
    }
