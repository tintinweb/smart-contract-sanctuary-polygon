	...
	UniswapV2ERC20Upgradeable
{
	...
	function _mintLiquidity(
		...
		uint256 _totalSupply = totalSupply;
		uint256 liquidity;
		if (_totalSupply < MINIMUM_LIQUIDITY || increaseFactor == 0) {
			liquidity = Maths.sqrt(amountStable * amountVol);
			if (_totalSupply < MINIMUM_LIQUIDITY) {
				liquidity = liquidity - (MINIMUM_LIQUIDITY - _totalSupply);
				_mint(address(this), MINIMUM_LIQUIDITY - _totalSupply); // permanently lock the first MINIMUM_LIQUIDITY tokens
			}
		} else {
