...
/**
 * @title AcornTokenRewardVault
 * @notice Profit sharing pool on the ATR tokens deposited in stake. It distributes the tokens received based on the number of tokens staked in each wallet.
 */
contract AcornTokenRewardVault is ReentrancyGuard, Pausable, Ownable {
    //---------- Libraries ----------//
    ...
    //---------- Contracts ----------//
    IERC20 public immutable ATR; // ATR token contract.
    ...
    //---------- Constructor ----------//
    constructor(address ATR_) {
        ATR = IERC20(ATR_);
        validTokens.add(ATR_);
        treasury = msg.sender;
    }
    ...
    function _recalculateBalances() internal virtual {
        ...
        for (uint256 i = 0; i < tokensCount; i++) {
            ...
            uint256 balance = token == address(ATR)
        ...
        returns (bool)
    {
        ...
        bool success = ATR.transferFrom(wallet_, address(this), amount_);
        ...
        returns (uint256)
    {
        ...
        uint256 balance = token_ == address(ATR)
    ...
    function stake(uint256 amount_) external whenNotPaused nonReentrant {
        ...
        require(
            ATR.allowance(_msgSender(), address(this)) >= amount_,
            "Amount not allowed"
        );
    ...
    function safeUnStake() external whenPaused nonReentrant {
        ...
        uint256 stakedBal = stakeHolders[_msgSender()].stakedBal;
        delete stakeHolders[_msgSender()];
        require(ATR.transfer(_msgSender(), stakedBal), "Transfer error");
        totalStaked = totalStaked.sub(stakedBal);
        totalHolders.decrement();
    }
    ...
    function setTokenList(address token_, bool add_) external onlyOwner {
        require(
            token_ != address(0) && token_ != address(ATR),
            "Invalid address"
        );
        if (add_) {
