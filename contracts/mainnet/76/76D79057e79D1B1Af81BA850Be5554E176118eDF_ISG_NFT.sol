...
library AppConsts {
    ...
    string  public constant ISG_NFT_CONTRACT_NAME = "ISG_NFT";
    bytes32 public constant ISG_NFT_CONTRACT_ID = keccak256(abi.encodePacked(ISG_NFT_CONTRACT_NAME));

    bytes32 public constant     ROLE_MINTER = keccak256(abi.encodePacked('ROLE_MINTER'));
    bytes32 public constant     ROLE_ERC20MINTER = keccak256(abi.encodePacked('ROLE_ERC20MINTER'));
...
abstract contract AppModuleUpgradeable is ModuleUpgradeable, IApp {
    ...
    modifier    onlyMinter() {
        _checkRole(AppConsts.ROLE_MINTER);
        _;
    }

    modifier    onlyOperator() {
        _checkRole(AppConsts.ROLE_OPERATOR);
        _;
    }
...
//import "./meta-transactions/ContentMixin.sol";
import "../base/meta-transactions/NativeMetaTransaction.sol";
 ...
 */
abstract contract ERC721TradableUpgradeable is ERC721Upgradeable, NativeMetaTransaction {
    using SafeMath for uint256;
    ...
    function baseTokenURI() virtual public view returns (string memory); // Removed pure modifier

/*    function tokenURI(uint256 _tokenId) override public view returns (string memory) {  // Removed pure modifier
...
/*    function _msgSender()
        ...
        returns (address sender)
    {
//	console.log("OVERRIDEN _msgSender call");
...
abstract contract ISG_NFT_Base is AppModuleUpgradeable {
	...
	return AppConsts.ISG_NFT_CONTRACT_ID;
    }

    function getName() override view public returns(string memory) {
	return AppConsts.ISG_NFT_CONTRACT_NAME;
    }
 ...
 * Ver: 2.4 / 190223
 */
contract ISG_NFT is ReentrancyGuardUpgradeable, ERC721EnumerableUpgradeable, ERC721TradableUpgradeable, ISG_NFT_Base {
    using Counters for Counters.Counter;

    string public base_token_URI;

    mapping(uint256 => uint256) public last_transfer_timestamp; //v2.0

    mapping(address => bool) public free_minted; //v2.0

    mapping(uint256 => uint256) public scores; //v2.2

    string private _tokenName; //v2.3

    string private _tokenSymbol; //v2.3

    uint256 private _freeMintCap; //v2.4

    uint256[250] private __gap;

    event BaseURI(string uri);
    event Name(string name);
    event Symbol(string symbol);
    event FreeMintCap(uint256 cap);

    function initialize() public virtual initializer {
	__ISG_NFT_init();
    }

    function __ISG_NFT_init() internal onlyInitializing {
	ReentrancyGuardUpgradeable.__ReentrancyGuard_init();
	ERC721EnumerableUpgradeable.__ERC721Enumerable_init();
	ERC721TradableUpgradeable.__ERC721Tradable_init("ISG Membership ticket", "ISGMem", address(0));
	_tokenName = super.name();
	_tokenSymbol = super.symbol();
	ISG_NFT_Base.__Base_init();
    }
    ...
    function defaultTokenURI() public virtual view returns (string memory) {
	return tokenURI(0);
    }

    function tokenURI(uint256 _tokenId) override virtual public view returns (string memory) { //v2.2
	require(exists(_tokenId), "ERC721Metadata: URI query for nonexistent token");
//	return string(abi.encodePacked(base_token_URI,'/',Strings.toString(_tokenId)));
	string memory	token_URI = LibString.replace(base_token_URI, '<token-id>', Strings.toString(_tokenId));
			token_URI = LibString.replace(token_URI, '<user-addr>', 
	    Strings.toHexString(uint160(
		ownerOf(
		    _tokenId
		)
	    ),20));
    ...
    function contractURI() public virtual view returns (string memory) {
        return base_token_URI;
    }
    ...
    function setName(string calldata name) public onlyOperator {
	_tokenName = name;

	emit Name(name);
    }

    function setSymbol(string calldata symbol) public onlyOperator {
	_tokenSymbol = symbol;

	emit Symbol(symbol);
    }

    function setFreeMintCap(uint256 _cap) public onlyOperator { // v2.4
	_freeMintCap = _cap;

	emit FreeMintCap(_cap);
    }

    function mintTo(address _to, uint8 count) public onlyMinter nonReentrant {
	for(uint i=0;i<count;i++)
	    super.mintTo(_to);
    }

    function mintTo(address _to) override public onlyMinter nonReentrant {
	super.mintTo(_to);
    }

    function mintFree(bytes calldata) public nonReentrant { // v2.1
	require(totalSupply() < _freeMintCap,'Free mint cap exceeded'); // v2.4
	address _to = _msgSender();
	require(!free_minted[_to],"ISG_NFT: An account cannot have more than single free mint");
	free_minted[_to] = true;
	super.mintTo(_to);
    }
    ...
    function totalSupply() public view override(ERC721TradableUpgradeable,ERC721EnumerableUpgradeable) virtual returns (uint256) {
	return ERC721EnumerableUpgradeable.totalSupply();
    }

    function _msgSender() internal override(ContextUpgradeable,ModuleUpgradeable) view virtual returns (address) {
	return ModuleUpgradeable._msgSender();
    }

    function getRoleOperator() public pure returns(bytes32){
	return AppConsts.ROLE_OPERATOR;
    }
    ...
    function supportsInterface(bytes4 interfaceId)
        ...
        view
        virtual
        override(ERC721EnumerableUpgradeable, ERC721Upgradeable, ModuleUpgradeable)
        returns (bool)
    {
	return
	...
	last_transfer_timestamp[tokenId] = block.timestamp;
