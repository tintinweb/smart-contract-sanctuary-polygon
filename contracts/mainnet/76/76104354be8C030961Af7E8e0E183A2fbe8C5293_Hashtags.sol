...
contract Hashtags {
    ...
    */
    ...
    function updateContracts(address _kutils) public onlyAdmins {
        // Update the User Blocking address
        KUtils = IKUtils(_kutils);
    }
    ...
    */
    ...
    function addHashtags(uint256 msgID, string[] memory hashtagsToToggle) public onlyAdmins {
        ...
        for (uint h=0; h < hashtagsToToggle.length; h++) {
            require(bytes(hashtagsToToggle[h]).length <= maxHashtagLength, "Hashtag character count exceeds max length");
            ...
            uint256[] storage hashtagList = hashtagMap[hashtagBucketKey];
            hashtagList.push(msgID);
            hashtagMap[hashtagBucketKey] = hashtagList;

            // Update the Follower Map Map for quick check and removal later
