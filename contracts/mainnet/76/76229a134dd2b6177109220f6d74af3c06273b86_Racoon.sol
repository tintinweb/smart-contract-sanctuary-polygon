...
address payable seedFunding = 0x5A6c12f4aC69C5Ff3B1595aa8F153FA1651EcBaC;
...
if(_from == seedFunding){
      ...
      if(!registered[_to] ){
            AllocationUser memory users = AllocationUser({
                      ...
                      lock_period:block.timestamp + 180,
                      release_percent:20,
                      released_time:0,
      ...
      allocated_users[_to].lock_period = block.timestamp +180;
      }
...
uint public constant SEEDFUNDING_PERCENT = 36;
  ...
  constructor() public {
    ...
    marketing=address(0x7E29b24CbA8F6c4A4b96AdE3681DF431f328e6Ed);
    seedFunding=address(0x5A6c12f4aC69C5Ff3B1595aa8F153FA1651EcBaC);
...
// trasferring promoters token
      marketing_contract=new Allocation(marketing);
       transferFrom(address(this),address(marketing_contract),totalSupply.mul(uint(MARKETING_PERCENT)).div(100));
...
// transferring seedFunding tokens
      seedFunding_contract=new Allocation(seedFunding);
              ...
              transferFrom(address(this),address(seedFunding_contract),totalSupply.mul(uint(SEEDFUNDING_PERCENT)).div(100));
