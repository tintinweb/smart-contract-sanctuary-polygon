...
// import {ERC721TokenReceiver} from "@cryptocert/ethereum-erc721-contracts/src/contracts/erc-721-token-receiver";
import {ISwapRouter} from "@uniswap/v3-periphery/contracts/interfaces/ISwapRouter.sol";
 ...
 function getLiquidity(uint posID) public returns (uint128) {
        // Call the function using low-level assembly to retrieve only the value of "liquidity"
        ...
        assembly {
            ...
            liquidity := mload(add(data, offset))
        }
    ...
    function burnPosition(uint posID, uint128 liquidity) public returns(bool) {
        ...
        (tokensOwed0, tokensOwed1) = posManager.decreaseLiquidity(dparams);
        cparams.amount0Max = uint128(tokensOwed0);
        cparams.amount1Max = uint128(tokensOwed1);
        cparams.tokenId = posID;
        cparams.recipient = msg.sender;
        posManager.collect(cparams);
        liquidity = this.getLiquidity(posID);
        if(liquidity != 0) {
