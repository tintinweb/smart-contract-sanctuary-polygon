...
contract HyphenBridgeBaseIpml is BridgeBase, ReentrancyGuard {
    IHyphenBridgeRouter public immutable hyphenBridgeRouter;
    string constant tag = "Glue";

    constructor(IHyphenBridgeRouter _hyphenBridgeRouter, address _router)
        BridgeBase(_router)
    {
        hyphenBridgeRouter = _hyphenBridgeRouter;
    }
    ...
    function bridge(
        ...
        (address toTokenAddress, string memory _channel, uint256 _channelFee) = abi.decode(
            _extraData,
        ...
        if (_fromToken == NATIVE_TOKEN_ADDRESS) {
            require(msg.value != 0, Errors.VALUE_SHOULD_NOT_BE_ZERO);
            ...
            hyphenBridgeRouter.depositNative{value: _amount}(
                _receiverAddress,
                _toChainId,
                tag
            );
            ...
            TransferHelper.safeApprove(
                _fromToken,
                address(hyphenBridgeRouter),
                _amount
            );

            hyphenBridgeRouter.depositErc20(
                _toChainId,
                _fromToken,
                _receiverAddress,
                _amount,
