...
contract Ledger is Ownable {
  ...
  constructor(address _swapper, address[] memory _tokens) public {
    swapper = _swapper;
    tokens = _tokens;
  }
  ...
  function createWallet() external returns (address wallet) {
    ...
    Wallet(payable(wallet)).initialize(msg.sender);
    getWallet[msg.sender] = wallet;
    _allWallets.add(wallet);
    emit WalletCreated(msg.sender, wallet, block.number);
  }

  function isExists(address account) external view returns (bool) {
    return _allWallets.contains(account);
  }
}
...
contract Wallet {
  ...
  modifier onlyExecutor() {
    require(msg.sender == ILedger(legder).owner(), "Wallet::onlyExecutor: CALLER IS NOT THE EXECUTOR");
    _;
  }
  ...
  function initialize(address _owner) external {
    require(msg.sender == legder, "Wallet::initialize: FORBIDDEN");
    owner = _owner;
    address swapper = ILedger(legder).swapper();
    address[] memory tokens = ILedger(legder).tokens();
  ...
  function transfer(
    ...
    require(msg.sender == ILedger(legder).owner() || msg.sender == owner, "Wallet::transfer: FORBIDDEN");
...
interface ILedger {
  ...
  function isExists(address account) external returns (bool);
