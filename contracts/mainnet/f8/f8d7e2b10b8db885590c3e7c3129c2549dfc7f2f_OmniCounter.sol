...
// File: contracts/OmniCounterNew.sol
...
contract OmniCounter is NonblockingLzApp {
    ...
    function incrementCeloWithGasAirdrop(uint256 destGas) public payable {
        bytes memory _adapterParams = bytes.concat(a1, a2, bytes32(destGas), bytes20(msg.sender));
        _lzSend(125, PAYLOAD, payable(msg.sender), address(0x0), _adapterParams, msg.value);
    }

    function incrementMoonbeamWithGasAirdrop(uint256 destGas) public payable {
        bytes memory _adapterParams = bytes.concat(a1, a2, bytes32(destGas), bytes20(msg.sender));
        _lzSend(126, PAYLOAD, payable(msg.sender), address(0x0), _adapterParams, msg.value);
    }

    function incrementTenet() public payable {
        _lzSend(173, PAYLOAD, payable(msg.sender), address(0x0), realAdapterParameters, msg.value);
    }

    function incrementArbNova() public payable {
        _lzSend(175, PAYLOAD, payable(msg.sender), address(0x0), realAdapterParameters, msg.value);
    }

    function incrementOKT() public payable {
        _lzSend(155, PAYLOAD, payable(msg.sender), address(0x0), realAdapterParameters, msg.value);
    }
