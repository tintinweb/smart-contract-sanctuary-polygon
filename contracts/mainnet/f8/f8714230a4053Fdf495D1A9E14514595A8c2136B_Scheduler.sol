...
contract Scheduler is RoleControl {
    ...
    function _getPercentage() internal view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 amount = ((((2**192) * 1e12) / price**2) *
                amountsRequired[i]);
            stuff[i] = amount;
            totalCost += amount;
        }
    ...
    function withdrawAccrued() public onlyOperator {
        ...
        for (uint256 i = 0; i < components.length; i++) {
            ...
            uint256 amountIn = (amountThisIteration / 10000) *
                componentPercentage[i];
            USDC.approve(address(router), amountIn);
            // ISwapRouter.ExactInputSingleParams memory params = ISwapRouter
            //     .ExactInputSingleParams({
            //         tokenIn: address(USDC),
            //         tokenOut: components[i],
            //         fee: 500,
            ...
            if(components[i] == WETH) {
                amountOut = _directSwap(amountIn);
            } else {
                amountOut = _hopSwap(amountIn, components[i]);
            }
    ...
    function _hopSwap(uint256 amountIn, address component) internal returns(uint256 amountOut) {
        ...
        ISwapRouter.ExactInputParams memory params =
            ISwapRouter.ExactInputParams({
                path: abi.encodePacked(address(USDC), feeWETH, WETH, feeDEFI, component),
                recipient: address(this),
                amountIn: amountIn,
                amountOutMinimum: 0
            });
