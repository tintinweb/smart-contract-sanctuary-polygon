...
import "@thirdweb-dev/contracts/openzeppelin-presets/token/ERC20/utils/SafeERC20.sol";
...
contract PET is ERC721Base {
    ...
    /**
     * @dev Public sale minting function
     */

    function mintPet(uint256 numPets) external whenPublicSaleActive {
        require(numPets > 0, "Must mint at least one pet");
        require(
            numPets <= maxMintPerTx,
            "numPets should not exceed maxMintPerTx"
        );
        ...
        emit PublicSaleMint(msg.sender, numPets);
        _safeMint(msg.sender, numPets);
    }
    ...
    function mintTo(
        ...
        uint256 mintPrice = getMintPrice();
        IERC20(tokenContract).transferFrom(_to, address(owner()), mintPrice);
    ...
    function batchMintTo(
        ...
        emit PublicSaleMint(_to, _quantity);
