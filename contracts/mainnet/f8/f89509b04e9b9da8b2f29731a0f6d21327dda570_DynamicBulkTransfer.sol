...
contract DynamicBulkTransfer is Ownable{
   ...
   function updateComssionAmount(uint256 _commissionAmount) public onlyOwner {
       commissionAmount = _commissionAmount;
   }
    
    // bulktransfer method
    function makeTransfer(address payable[] memory  addressArray, uint256[] memory  amountArray, address contractAddress)
     public payable{
    ...
    for(uint256 i=0; i<amountArray.length;i++){
      ...
      if(msg.value < commissionAmount){
        require(false, "insufficient amount for commission");
    }
