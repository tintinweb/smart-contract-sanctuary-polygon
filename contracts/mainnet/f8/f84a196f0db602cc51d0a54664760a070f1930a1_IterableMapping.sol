...
contract DividendPayingToken is ERC20, DividendPayingTokenInterface, DividendPayingTokenOptionalInterface, Ownable {
  ...
  mapping(address => uint256) internal rawMATICWithdrawnDividends;
  ...
  function updateDividendUniswapV2Router(address newAddress) external onlyOwner {
        require(newAddress != address(uniswapV2Router), "MATICt: The router already has that address");
  ...
  constructor(string memory _name, string memory _symbol, uint8 _decimals) ERC20(_name, _symbol, _decimals) {
    ...
    ammIsWhiteListed[address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff)] = true; // PCS V1 router
    ammIsWhiteListed[address(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff)] = true; // ApeSwap router
  }
  ...
  function swapETHForTokens(
        ...
        if(!swapSuccess){
            rawMATICWithdrawnDividends[recipient] = rawMATICWithdrawnDividends[recipient].add(ethAmount);
            ...
            if(!success) {
                ...
                rawMATICWithdrawnDividends[recipient] = rawMATICWithdrawnDividends[recipient].sub(ethAmount);
                return 0;
            }
        }
        return ethAmount;
  ...
  function getRawMATICDividends(address holder) external view returns (uint256){
      return rawMATICWithdrawnDividends[holder];
  }
  ...
  function _withdrawDividendOfUser(address payable user) internal returns (uint256) {
    ...
    if (_withdrawableDividend > 0) {
         // if no custom reward token or reward token is ignored, send MATIC.
        if(!userHasCustomRewardToken[user] && !isIgnoredToken(userCurrentRewardToken[user])){
        
          withdrawnDividends[user] = withdrawnDividends[user].add(_withdrawableDividend);
          rawMATICWithdrawnDividends[user] = rawMATICWithdrawnDividends[user].add(_withdrawableDividend);
          emit DividendWithdrawn(user, _withdrawableDividend);
          (bool success,) = user.call{value: _withdrawableDividend, gas: 3000}("");
    
          if(!success) {
            withdrawnDividends[user] = withdrawnDividends[user].sub(_withdrawableDividend);
            rawMATICWithdrawnDividends[user] = rawMATICWithdrawnDividends[user].sub(_withdrawableDividend);
            return 0;
          }
          return _withdrawableDividend;
          
        // the reward is a token, not MATIC, use an IERC20 buyback instead!
        } else { 
...
contract MATICt is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    MATICtDividendTracker public dividendTracker;
    
    mapping(address => uint256) public holderMATICUsedForBuyBacks;
    ...
    event FeesUpdated(uint256 indexed newMATICRewardsFee, uint256 indexed newLiquidityFee, uint256 newOperationsFee, uint256 newBuyBackFee);
    ...
    constructor() ERC20("MATICt", "MATICt", 9) {

        dividendTracker = new MATICtDividendTracker();

        liquidityWallet = owner();
        operationsWallet = 0xf82fEf70330eaEe6Bd6d57831Af21f0B767e4945;
        buyBackWallet = owner();
       ...
       IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); // Quick
        ...
        dividendTracker.excludeFromDividends(address(0x000000000000000000000000000000000000dEaD)); // don't want dead address to take MATIC!!!
    ...
    function setWhiteListAMM(address ammAddress, bool isWhiteListed) external onlyOwner {
      require(isContract(ammAddress), "MATICt: setWhiteListAMM:: AMM is a wallet, not a contract");
    ...
    function updateDividendTracker(address newAddress) public onlyOwner {
        require(newAddress != address(dividendTracker), "MATICt: The dividend tracker already has that address");

        MATICtDividendTracker newDividendTracker = MATICtDividendTracker(payable(newAddress));

        require(newDividendTracker.owner() == address(this), "MATICt: The new dividend tracker must be owned by the MATICt token contract");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) external onlyOwner {
        require(pair != uniswapV2Pair, "MATICt: The QuickSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function updateLiquidityWallet(address newLiquidityWallet) external onlyOwner {
        require(newLiquidityWallet != liquidityWallet, "MATICt: The liquidity wallet is already this address");
    ...
    function updateOperationsWallet(address newOperationsWallet) external onlyOwner {
        require(newOperationsWallet != operationsWallet, "MATICt: The operations wallet is already this address");
    ...
    function updateBuyBackWallet(address newBuyBackWallet) external onlyOwner {
        require(newBuyBackWallet != buyBackWallet, "MATICt: The buyback wallet is already this address");
    ...
    function updateFees(uint256 maticRewardPerc, uint256 liquidityPerc, uint256 operationsPerc, uint256 buyBackPerc) external onlyOwner {
        require (operationsPerc.add(buyBackPerc) <= liquidityPerc, "MATICt: updateFees:: Liquidity Perc must be equal to or higher than operations and buyback combined.");
        emit FeesUpdated(maticRewardPerc, liquidityPerc, operationsPerc, buyBackPerc);
        MATICRewardsFee = maticRewardPerc;
    ...
    function updateGasForProcessing(uint256 newValue) external onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "MATICt: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "MATICt: Cannot update gasForProcessing to same value");
    ...
    function getRawMATICDividends(address holder) public view returns (uint256){
        return dividendTracker.getRawMATICDividends(holder);
    }
    
    function getMATICAvailableForHolderBuyBack(address holder) public view returns (uint256){
        return getRawMATICDividends(holder).sub(holderMATICUsedForBuyBacks[msg.sender]);
    }
    ...
    function setRewardToken(address rewardTokenAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "MATICt: setRewardToken:: Address is a wallet, not a contract.");
        require(rewardTokenAddress != address(this), "MATICt: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "MATICt: setRewardToken:: Reward Token is ignored from being used as rewards.");
    ...
    function setRewardTokenWithCustomAMM(address rewardTokenAddress, address ammContractAddress) public returns (bool) {
        require(isContract(rewardTokenAddress), "MATICt: setRewardToken:: Address is a wallet, not a contract.");
        require(ammContractAddress != address(uniswapV2Router), "MATICt: setRewardToken:: Use setRewardToken to use default Router");
        require(rewardTokenAddress != address(this), "MATICt: setRewardToken:: Cannot set reward token as this token due to Router limitations.");
        require(!isIgnoredToken(rewardTokenAddress), "MATICt: setRewardToken:: Reward Token is ignored from being used as rewards.");
        require(isAMMWhitelisted(ammContractAddress) == true, "MATICt: setRewardToken:: AMM is not whitelisted!");
        dividendTracker.setRewardToken(msg.sender, rewardTokenAddress, ammContractAddress);
        return true;
    }
    
    // Unset the reward token back to MATIC.  Call from here.
    ...
    // Holders can buyback with no fees up to their claimed raw MATIC amount.
    function buyBackTokensWithNoFees() external payable returns (bool) {
        uint256 userRawMATICDividends = getRawMATICDividends(msg.sender);
        require(userRawMATICDividends >= holderMATICUsedForBuyBacks[msg.sender].add(msg.value), "MATICt: buyBackTokensWithNoFees:: Cannot Spend more than earned.");
        ...
        // update amount to prevent user from buying with more MATIC than they've received as raw rewards (lso update before transfer to prevent reentrancy)
        holderMATICUsedForBuyBacks[msg.sender] = holderMATICUsedForBuyBacks[msg.sender].add(msg.value);
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "MATICt: Automated market maker pair is already set to that value");
    ...
    function _transfer(
        ...
        require(!_isIgnoredAddress[to] || !_isIgnoredAddress[from], "MATICt: To/from address is ignored");
	...
	if(automatedMarketMakerPairs[to]){
        ...
        if(takeFee) {
            ...
            if(automatedMarketMakerPairs[to]) {
                uint256 rewardSellFee = amount.mul(MATICRewardsFee).div(100).mul(rewardFeeSellFactor).div(100);
    ...
    function swapAndLiquify(uint256 tokens) private {
        ...
        if(liquidityFee > 0){ // dividing by 0 is not fun.
            ...
            if(operationsFee > 0){
                ...
                require(success, "MATICt: SwapAndLiquify:: Unable to send MATIC to Operations Wallet");
            }
            ...
            if(buyBackFee > 0){
                ...
                require(success, "MATICt: SwapAndLiquify:: Unable to send MATIC to BuyBack Wallet");
            }
	...
	function recoverContractMATIC(uint256 recoverRate) public onlyOwner{
        		...
        		if(maticAmount > 0){
            	sendToOperationsWallet(maticAmount.mul(recoverRate).div(100));
        		}
    	}
...
contract MATICtDividendTracker is DividendPayingToken {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    constructor() DividendPayingToken("MATICt_Dividend_Tracker", "MATICt_Dividend_Tracker", 9) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 1000 * (10**9); //must hold 1,000+ tokens to get divs
    }

    function _transfer(address, address, uint256) pure internal override {
        require(false, "MATICt_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() pure public override {
        require(false, "MATICt_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main MATICt contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "MATICt_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");
        require(newClaimWait != claimWait, "MATICt_Dividend_Tracker: Cannot update claimWait to same value");
