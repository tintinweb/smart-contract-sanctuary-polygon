...
// File: contracts/5_ChildComplex.sol
...
contract ERC1155Token  is ERC1155,Ownable {
    ...
    modifier checkMinting(uint256 _tokenId, address to) {
        require(
    ...
    function createToken(uint256 _tokenId, bool _limited, uint128 _limit) external onlyOwner {
        require(
            tokenIdToToken[_tokenId].tokenLimit == 0,
            "Token already exists"
        );
        Token memory token = Token(_limited, _limit, 0);
        tokenIdToToken[_tokenId] = token;
    }

    function mintTokenItem(address to, uint256 tokenId) public onlyOwner {
        _mint(to, tokenId, 1, '');
    }

    function batchMintTokenItem(address[] memory to, uint256 tokenId, bytes memory data) public onlyOwner {
    ...
    function setURI(uint256 _tokenId, string memory _uri) internal onlyOwner {
    ...
    function tokenMinter(uint256 _tokenId) public view returns (uint128) {
