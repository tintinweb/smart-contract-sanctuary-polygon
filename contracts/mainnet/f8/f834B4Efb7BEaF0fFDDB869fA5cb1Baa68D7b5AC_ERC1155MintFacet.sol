...
/// @author Nypox
contract ERC1155MintFacet is Modifiers {
...
/// @dev Mints `order.amount` tokens of token type `order.id` to `order.to`.
    function ERC1155Mint(
        ...
        require(
            ...
            "1155F: INVALID_MARKET_SIGNATURE"
        );

        AssetVersion version = LibNFT.getAssetVersion(order.token);

        LibNFT.checkAssetVersionIsDefault(AssetStandard.ERC1155, version);

        LibNFT.mintERC1155(order, version, false); 
    }

/// @dev Batched version of {ERC1155Mint}.
    function ERC1155MintBatch(
        ...
        LibNFT.mintERC1155Batch(order, version, false); 
    }
}
