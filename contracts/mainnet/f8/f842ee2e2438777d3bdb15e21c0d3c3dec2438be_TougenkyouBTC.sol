...
import { ERC721BridgeBRCTokenURL, IERC721 } from "ERC721BridgeBRC/src/extensions/ERC721BridgeBRCTokenURL.sol";
...
contract TougenkyouBTC is AccessControl, ERC2981, ERC721BridgeBRCTokenURL, ERC721Enumerable {
    ...
    mapping(BTCTicketID => mapping(address => uint256)) private _whiteListClaimed;

    constructor(address _underlyingToken, address _bridge) ERC721BridgeBRCTokenURL("TougenkyouBTC", "momoBTC", IERC721(_underlyingToken), _bridge) {
        _setDefaultRoyalty(_msgSender(), 750);
        _grantRole(DEFAULT_ADMIN_ROLE, _msgSender());
        _grantRole(MINTER_ROLE, _msgSender());
        _setBaseURI("ar://KSXteJIOs5zVu5A5KiQ_bYiF0c68sedCKY7csYf4RXw/", 1);
    ...
    function tokenURI(uint256 tokenId) public view virtual override(ERC721BridgeBRCTokenURL, ERC721) returns (string memory) {
        return ERC721BridgeBRCTokenURL.tokenURI(tokenId);
    }
    ...
    function publicMint(address _to, uint256 _mintAmount) external payable callerIsUser whenMintable {
        if (_mintAmount > depositLength()) revert MaxSupplyOver();
    ...
    function preMint(uint256 _mintAmount, uint256 _presaleMax, bytes32[] calldata _merkleProof, BTCTicketID ticket) external payable whenMintable {
        ...
        uint256 claimed = _whiteListClaimed[ticket][msg.sender] + _mintAmount;
        ...
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _whiteListClaimed[ticket][msg.sender]++;
            _mintAt(msg.sender);
        }
    }

    function ownerMint(address _address, uint256 _tokenId) external onlyRole(MINTER_ROLE) {
        require(!renounceOwnerMintFlag, "owner mint renounced");
        _safeMint(_address, _tokenId);
    }

    function ownerBatchMint(address _address, uint256 _mintAmount) external onlyRole(MINTER_ROLE) {
        require(!renounceOwnerMintFlag, "owner mint renounced");
        for (uint256 i = 1; i <= _mintAmount; i++) {
            _mintAt(_address);
        }
    ...
    function setBaseURI(string memory metadata, uint256 _maxRange) external virtual onlyRole(DEFAULT_ADMIN_ROLE) {
        _setBaseURI(metadata, _maxRange);
    }
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, ERC721Enumerable, ERC2981, AccessControl) returns (bool) {
        return ERC721.supportsInterface(interfaceId) || ERC721Enumerable.supportsInterface(interfaceId) || ERC2981.supportsInterface(interfaceId) || AccessControl.supportsInterface(interfaceId);
    }
