...
contract PIXFixedSale is PIXBaseSale, EIP712Upgradeable {
    ...
    */
    ...
    /** @notice cancel sale request by contract owner
     *  @param _saleId Sale id to cancel
     */
    function adminSaleCancel(uint256 _saleId) external onlyOwner {
        FixedSaleInfo storage _saleInfo = saleInfo[_saleId];
    ...
    */
    ...
    function withdraw(address _walletAddress, address _currency) external payable onlyOwner {
        if (_currency == address(0)) {
            (bool callSuccess,) = payable(_walletAddress).call{value : address(this).balance}("");
            if (!callSuccess) revert WithdrawalFailed();
        } else {
