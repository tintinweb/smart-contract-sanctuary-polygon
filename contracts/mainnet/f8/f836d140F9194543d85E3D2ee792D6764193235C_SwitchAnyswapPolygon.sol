...
contract SwitchAnyswapPolygon is SwitchPolygon {
    ...
    function setAnyswapRouter(address _newAnyswapRouter) external onlyOwner {
        anyswapRouter = _newAnyswapRouter;
    }
    ...
    function transferByAnyswap(TransferArgsAnyswap calldata transferArgs) external payable {
        ...
        if (!isNative) IERC20(underlyingToken).universalTransferFrom(msg.sender, address(this), transferArgs.amount);

        if (isNative) {
            IAnyswapV4Router(anyswapRouter).anySwapOutNative{ value: transferArgs.amount }(
            ...
            if (approvedAmount < transferArgs.amount) {
                IERC20(underlyingToken).safeIncreaseAllowance(anyswapRouter, transferArgs.amount - approvedAmount);
            }
        ...
        _emitCrossChainSwapRequest(transferArgs, bytes32(0), msg.sender, DataTypes.SwapStatus.Succeeded);
    }
    ...
    function _emitCrossChainSwapRequest(TransferArgsAnyswap calldata transferArgs, bytes32 transferId, address sender, DataTypes.SwapStatus status) internal {
