...
contract NFT721Bridge is Initializable, OwnableUpgradeable, IZKBridgeReceiver, ReentrancyGuard {
    ...
    function transferNFT(address _token, uint256 _tokenId, uint16 _recipientChain, address _recipient) public payable nonReentrant returns (uint64 sequence) {
        ...
        sequence = _logTransfer(Transfer(tokenAddress, tokenChain, symbol, name, _tokenId, uriString, _recipient, _recipientChain), msg.value);

        emit TransferNFT(sequence, _token, _tokenId, _recipientChain, msg.sender, _recipient);
    }

    function _logTransfer(Transfer memory _transfer, uint256 _callValue) internal returns (uint64 sequence) {
        ...
        sequence = zkBridge.send{value : _callValue}(_transfer.toChain, dstAddress, payload);
    }

    function zkReceive(uint16 _srcChainId, address _srcAddress, uint64 _sequence, bytes calldata _payload) external nonReentrant override {
        require(msg.sender == address(zkBridge), "Not From ZKBridgeEntrypoint");
        require(trustedRemoteLookup[_srcChainId] == _srcAddress, "invalid emitter");
        _completeTransfer(_srcChainId, _sequence, _payload);
    }

    function _completeTransfer(uint16 _srcChainId, uint64 _sequence, bytes calldata _payload) internal {
