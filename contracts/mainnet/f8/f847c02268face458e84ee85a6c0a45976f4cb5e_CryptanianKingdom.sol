...
// File: contracts/CryptanianKingdom.sol
...
// File: contracts/RevolutionPunk.sol
    ...
    Ownable
{
    ...
    uint256 public maxPublicMintAmmount = 1000;

    Counters.Counter public tokenSupply;
    Counters.Counter public publicMintCount;
    ...
    uint256 public NFT_price = 2 ether; 
    uint256 public NFT_price_for_3 = 3.5 ether; 
    ...
    uint256 public reveal_time = 1650042265;
  

    constructor(
    ) ERC721("Cryptanian Kingdom", "CK") {
        setNotRevealedUri("ipfs://QmPzaCpGxNZwjuypS8wMMXabzGQXKL7yRECiPCm3vjhSuF/");
        setBaseUri("ipfs://QmX6LrPcGBHehtAX2tTxcQ8A3FFYGfPW2fEnf52NmUUNza/");
    }
    ...
    function mintPresale() external payable{
        ...
        _mintLoop(_msgSender(), 3);
    }
    
    // Public Mint

    function publicMint(uint256 ammountMint) public payable {
        uint256 getFreeMint = ammountMint / 2;

        require(msg.value == NFT_price * ammountMint, "Sent Amount Wrong");
        require(ammountMint.add(getFreeMint) + publicMintCount.current() <= maxPublicMintAmmount, "Public mint reached");
         //  require current spply < max supply
        require(tokenSupply.current() + ammountMint.add(getFreeMint) <= MAX_SUPPLY, "Max Supply Reached");
        require(ammountMint <= 50, "Max 50 Allowed.");

        for (uint256 i = 0; i < ammountMint.add(getFreeMint); i++) {
            publicMintCount.increment();
            tokenSupply.increment();
            _safeMint(_msgSender(), tokenSupply.current());
        }
    }
    
    //Owner Mint Functions

    function mintDev(uint256 ammountMint) public onlyOwner {
        _mintLoop(_msgSender(), ammountMint);
    }


    function setMaxPublicMintAmmount(uint256 ammountMint) public onlyOwner {
        maxPublicMintAmmount = ammountMint;
    }
        ...
        returns (string memory)
    {
        ...
        if(block.timestamp < reveal_time){
            return 
                string(abi.encodePacked(notRevealedURI, Strings.toString(_tokenId), ".json"));
        }
        return
    ...
    function increaseMaxSupply(uint256 temp) external onlyOwner {
     ...
     function _mintLoop(address _receiver, uint256 _mintAmount) internal {
        //  require current spply < max supply
        require(tokenSupply.current() + _mintAmount <= MAX_SUPPLY, "Max Supply Reached");
        require(_mintAmount <= 50, "Max 50 Allowed.");

        for (uint256 i = 0; i < _mintAmount; i++) {
            tokenSupply.increment();
            _safeMint(_receiver, tokenSupply.current());
        }
    }
}