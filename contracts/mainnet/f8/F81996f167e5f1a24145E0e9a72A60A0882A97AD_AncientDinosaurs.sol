...
contract AncientDinosaurs is ERC721, Ownable, ERC721Enumerable {
    using SafeMath for uint256;
    using Strings for uint256;

    uint256 public constant tokenPrice = 90000000000000000000; // 0.19 Matic //  90000000000000000000 // 90 Matic
    uint256 public constant presaleTokenPrice = 45000000000000000000; // 0.09 Matic // 45000000000000000000 // 45 Matic
    ...
    uint256 public devReserve = 141;

    event DinosaurMinted(uint256 tokenId, address owner);

    constructor() ERC721("Ancient Dinosaurs", "AD") {}
    ...
    function mintDinosaur(uint256 numberOfTokens) external payable {
        ...
        for (uint256 i = 0; i < numberOfTokens; i++) {
            ...
            if (totalSupply() < MAX_TOKENS) {
                ...
                emit DinosaurMinted(id, msg.sender);
            }
        }
    }

    function presaleDinosaur(uint256 numberOfTokens) external payable {
        ...
        require(
            msg.value >= presaleTokenPrice.mul(numberOfTokens),
            "Ether value sent is not correct"
        );
        ...
        returns (string memory)
    {
        ...
        if (isRevealed == false) {
            return
                "ipfs://QmRsV8duYdSJdPFnihj8SXjSW12YnCPWyzS7BsMDDyk8KL/1.json";
        }
