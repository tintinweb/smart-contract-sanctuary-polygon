...
contract LuffyMarket is Ownable, ERC721Holder, ReentrancyGuard {
    ...
    function purchaseWithFixedPrice(
        ...
        try nftInstance.safeTransferFrom(thisSale.seller, msg.sender, tokenId) {} catch (
            bytes memory _error
        ) {
    ...
    function finishAuction(
        ...
        if (thisAuction.highestBidder != address(0) && thisAuction.winPrice > 0) {
            if (thisAuction.tokenPayment == address(0)) {
                ...
                payable(msg.sender).transfer(thisAuction.winPrice - royalFee - ethFee); //keep ethFee in marketplace
            } else {
                if (thisAuction.isBurn == true) {
                    uint256 burnFee = (winPrice * thisAuction.burnRate) / 10 ** 3;
                    royalFee = (winPrice * thisAuction.royalRate) / 10 ** 3;
                    if (burnFee > 0) {
                    ...
                    require(
                        token.transfer(msg.sender, (winPrice - royalFee - burnFee)),
                        "Transfer token to seller fail"
                    );
                } else {
                    getFee = setFee[thisAuction.tokenPayment];
                    tokenFee = (winPrice * getFee) / feeDecimal;
                    royalFee = (winPrice * thisAuction.royalRate) / 10 ** 3;
                    if (royalFee > 0) {
                        require(
                            token.transfer(thisAuction.ownerCollection, royalFee),
                    ...
                    require(
                        token.transfer(msg.sender, (winPrice - tokenFee)), //keeping tokenFee in Contract
                        "Transfer token to seller fail"
                    );
                }
            }

            try nftInstance.safeTransferFrom(address(this), highestBidder, tokenId) {} catch (
                bytes memory _error
            ) {
                emit ErrorLog(_error);
            ...
            try nftInstance.safeTransferFrom(address(this), msg.sender, tokenId) {} catch (
                bytes memory _error
            ) {
                emit ErrorLog(_error);
                revert("FINISH AUCTION FAIL");
