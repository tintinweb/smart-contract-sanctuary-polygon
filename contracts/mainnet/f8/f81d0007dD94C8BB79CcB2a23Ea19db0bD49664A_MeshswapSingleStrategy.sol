...
import {ISinglePool} from "../../connectors/meshswap/single/ISinglePool.sol";

import "hardhat/console.sol";

contract MeshswapSingleStrategy is InitializableAbstractSingleStrategy {
     ...
     * well within that abstraction.
     ...
     @param _mesh_contracts Meshswap related contracts
    ...
    function initialize(
        ...
        address[] calldata _mesh_contracts, 
        ...
        MESH = IERC20(_rewardTokenAddresses[0]);

        swapper = ISwapper(_essentials[0]);
        oracleRouter = IOracle(_essentials[1]);

        pool = ISinglePool(_mesh_contracts[0]);
    ...
    function _withdraw(uint256 _amount) external onlyVault nonReentrant {
    ...
    function _withdrawAll() internal {
        pool.withdrawTokenByAmount(lpBalance());
    }
    function withdrawAll() external override onlyVault nonReentrant {
        _withdrawAll();
        token0.safeTransfer(vaultAddress,
    ...
    function balance() public override view returns (uint256) {
        ...
        if (lpBalance() > 0) {
            ...
            b0 = b0.add(exchangeRateStored.mul(lpBalance()).div(1e18));
        }
        return b0;
    }
    ...
    function _collectRewards() internal returns (uint256) {
        pool.claimReward();
        uint256 meshBalance = MESH.balanceOf(address(this));
        console.log("RewardCollection - MESH Balance: ", meshBalance);

        uint256 rewardInVaultToken = 0;
        if (meshBalance > minThresholds[0]) {
            uint256 previous = vaultAsset.balanceOf(address(this));
            MESH.approve(address(swapper), meshBalance);
            swapper.swapCommon(address(MESH), address(vaultAsset), meshBalance);
