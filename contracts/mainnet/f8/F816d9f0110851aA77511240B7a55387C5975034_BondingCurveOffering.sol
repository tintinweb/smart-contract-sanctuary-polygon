...
contract BondingCurveOffering is BondingCurve {
    ...
    function Purchase(uint256 _amount) public validBuy(_amount) {
        ...
        usdt.transfer(wallet, tax(_amount));
    ...
    {
        uint256 _tax_rate = taxRatio * (rate / MAX_WEIGHT);
        return _tax_rate * _amount;
    }
...
contract BondingCurve is BancorFormula, Ownable, MaxGasPrice {
    ...
    constructor(uint256 _supply, uint256 _reserveRatio) {
    ...
    {
        return
            calculateSaleReturn(supply, balance, uint32(reserveRatio), _amount);
    }
