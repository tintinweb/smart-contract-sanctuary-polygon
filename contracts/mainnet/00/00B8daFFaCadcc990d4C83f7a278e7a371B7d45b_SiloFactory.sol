...
contract SiloFactory is Ownable, ERC721Enumerable{
    ...
    function usersSilosFilterCategory(address _user, uint _category, bool _filter) external view returns(uint[] memory, address[] memory, string[] memory){
        ...
        if(_filter){
            for(uint i=0; i<balanceOf(_user); i++){
                silo = ISilo(siloMap[tokenOfOwnerByIndex(_user, i)]);
        ...
        for(uint i=0; i<balanceOf(_user); i++){
            ...
            if(!_filter || silo.strategyCategory() == _category){
                ids[count] = tokenOfOwnerByIndex(_user, i);
    ...
    function getUpkeepBalance(address _user) external view returns(uint96){
        return ISiloManagerFactory(managerFactory).getUpkeepBalance(_user);
    }

    function managerApproved(address _user) external view returns(bool){
        return ISiloManagerFactory(managerFactory).managerApproved(_user);
    }

    function managerExists(address _user) external view returns(bool){
        address manager = ISiloManagerFactory(managerFactory).userToManager(_user);
    ...
    //Think this should return the current strategy too
