 ...
 * @title C250Gold
 */
contract C250Gold is AccessControl, ERC20, Ownable, ReentrancyGuard {
        ...
        )
    {
        ...
        if (users[userID].classicIndex > 0) {
    ...
    function processChangeWalletRequest(uint256 userID) external {
        require(isUserAdmin(msg.sender), "Access Denied");

        if (userAddresses[userID] == address(0)) {
            require(
                changeWalletRequests[userID].approvals.length >= 3,
                "Not allowed"
            );
        } else {
            require(
                changeWalletRequests[userID].approvals.length >= 5,
                "Not allowed"
            );
        }
    ...
    address presaleTicket1;

    function setPresaleTicket1(address addr) external onlyOwner {
        require(!live, "Already launched");

        presaleTicket1 = addr;
    }

    address presaleTicket2;

    function setPresaleTicket2(address addr) external onlyOwner {
        require(!live, "Already launched");
    ...
    function claimPresale() external {
        ...
        uint256 amount1 = IERC20(presaleTicket1).balanceOf(msg.sender);
        if (amount1 > 0) {
            IERC20(presaleTicket1).transferFrom(
                msg.sender,
                address(this),
                amount1
            );
        ...
        uint256 amount2 = IERC20(presaleTicket2).balanceOf(msg.sender);
        if (amount2 > 0) {
            IERC20(presaleTicket2).transferFrom(
                msg.sender,
                address(this),
                amount2
            );
        ...
        presaleBalance[msg.sender] = presaleBalance[msg.sender].add(amount);
    }

    function transferInternal(address to, uint256 amount) external {
        address sender = msg.sender;
        require(
            presaleBalance[sender] >= amount,
            "Insufficient presale balance"
        );

        _burn(sender, amount);
        _mint(to, amount);

        presaleBalance[sender] = presaleBalance[sender].sub(amount);
        presaleBalance[to] = presaleBalance[to].add(amount);
    }
    ...
    function importClassicAccount(
        ...
        if (users[id].classicIndex == 0) {
            classicIndex = classicIndex.add(1);
            users[id].classicIndex = classicIndex;
        }

        if (addr != address(0)) {
            userAccounts[addr].push(id);
        }

        if (referralID > 0 && users[referralID].classicIndex > 0) {
            users[referralID].referrals.push(id);
        }
    }

    function withdraw(uint256 userID) external nonReentrant {
    ...
    function upgradeToPremium(uint256 userID, uint256 random) external {
        ...
        if (accountIsInPremium(user.referralID)) {
            referralEarner = userAddresses[user.referralID];
        }
        sendPayout(referralEarner, amountFromDollar(UPGRADE_FEE.div(2)));
        ...
        onlyOwner
    {
        ...
        if (!matrices[options.userID][options.part].registered) {
            premiumCounter = premiumCounter.add(1);
        }
    ...
    uint256[] public CLASSIC_REFERRAL_PERCENTS = [10, 7, 5];
    ...
    bytes32 public constant USER_ADMIN_ROLE = keccak256("USER_ADMIN");
    ...
    constructor(
        ...
        classicIndex++;
        users[1].classicIndex = classicIndex;
        users[1].classicCheckpoint = timeProvider.currentTime();
    }
    ...
    function isUserAdmin(address account) public view virtual returns (bool) {
        return hasRole(USER_ADMIN_ROLE, account);
    }

    /// @dev Add an account to the admin role. Restricted to admins.
    function addUserAdmin(address account) external virtual onlyAdmin {
        grantRole(USER_ADMIN_ROLE, account);
    }

    /// @dev Remove an account from the user role. Restricted to admins.
    function removeUserAdmin(address account) external virtual onlyAdmin {
        revokeRole(USER_ADMIN_ROLE, account);
    }
    ...
    function register(
        ...
        uint256 uplineID,
        address addr
    ) public nonReentrant validReferralID(referralID) {
    ...
    function buildPremiumConfig() private {
        ...
        levelConfigurations[18] = LevelConfig(18000000 * 1e18, 4, 16);
