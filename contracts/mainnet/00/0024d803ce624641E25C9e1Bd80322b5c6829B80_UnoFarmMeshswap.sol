...
contract UnoFarmMeshswap is Initializable, ReentrancyGuardUpgradeable, IUnoFarmMeshswap  {
	 ...
	 * {lpPool} - Token that the strategy maximizes.
	...
	function initialize(address _lpPool, address _assetRouter) external initializer {
		...
		rewardToken = IExchangeMeshwap(_lpPool).mesh();
		tokenA = IExchangeMeshwap(lpPool).token0();
		tokenB = IExchangeMeshwap(lpPool).token1();
		...
		IERC20(lpPool).approve(address(MeshswapRouter), type(uint256).max);
	...
	function getTotalDeposits() external view returns (uint256 _totalDeposits) {
		if (totalDeposits > 0) {
			_totalDeposits = IExchangeMeshwap(lpPool).balanceOf(address(this));
		}
	}
