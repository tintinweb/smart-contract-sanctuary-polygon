...
import "../QuickMai/V3Router/libraries/TickMath.sol";
import "../QuickMai/V3Router/libraries/FullMath.sol";
...
contract GammaMaiLpFarm is BaseSiloAction {
    ...
    address private constant MAI_USDC_ADDRESS =
    ...
    constructor(string memory _name, address _siloFactory) {
        name = _name;
        metaData = "address[5],address[5],uint,uint,address";
        factory = _siloFactory;
        usesTakeFee = true;
        feeName = "Gamma Qi Harvests";
    }
    ...
    function _act(
        ...
        outputAmounts[1] = inputAmounts[2];

        (, , , uint256 pid, address pos) = abi.decode(
            configuration,
            (address[5], address[5], uint256, uint256, address)
        );
        ...
        if (
            inputAmounts[0] == 0 &&
            inputAmounts[4] == 0 &&
            farm.deposited(pid, address(this)) > 0
        ) {
            uint256 reward = farm.pending(pid, address(this));
            ...
            if (inputAmounts[0] > 0) {
                ...
                uint256 swapAmount = _getSwapAmountForShare(
                    inputAmounts[0],
                    pos
                );
                ...
                if (swapAmount > 0) {
                    ...
                    uint256 shares = IUniProxy(UniProxy_Address).deposit(
                        amounts[1],
                        inputAmounts[0] - amounts[0],
                        address(this),
                        pos,
                    ...
                    uint256 reward = IERC20(QI_ADDRESS).balanceOf(
                        address(this)
                    );

                    farm.deposit(pid, shares);

                    reward =
                        IERC20(QI_ADDRESS).balanceOf(address(this)) -
                        reward;
                ...
                if (deposited > 0) {
                    ...
                    if (inputAmounts[4] == 2 ** 256 - 1) {
                        ...
                        removeShare = deposited;
                    } else {
                        removeShare = getRemoveShareAmount(
                            inputAmounts[4],
                            pos
                        );
                        removeShare = removeShare > deposited
                            ? deposited
                            : removeShare;
                    }
                    ...
                    if (removeShare > 0) {
                        ...
                        farm.withdraw(pid, removeShare);

                        reward =
                            IERC20(QI_ADDRESS).balanceOf(address(this)) -
                            reward;

                        (uint256 amount0, uint256 amount1) = IHypervisor(pos)
                            .withdraw(
                                removeShare,
                                address(this),
                                address(this),
    ...
    function getRemoveShareAmount(
        uint256 amount,
        address pos
    ) internal view returns (uint256 removeShare) {
        IUniswapV2Pair quickMaiLP = IUniswapV2Pair(MAI_USDC_ADDRESS);
        ...
        removeShare = getShareAmountFromSingleAsset(
            resA,
            resB,
            total0,
            total1,
    ...
    function convertSingleAsset(
        ...
        uint256 balance1 = IERC20(MAI_ADDRESS).balanceOf(address(this));
    ...
    function createConfig(
        ...
        configData = abi.encode(_inputs, _outputs, _trigger, _pid, _pos);
    }
    ...
    function getShareAmountFromSingleAsset(
        uint256 resA,
        uint256 resB,
        uint256 total0,
        uint256 total1,
        uint256 singleAmountB,
        uint256 totalShareSupply
    ) internal pure returns (uint256 decreaseShare) {
        uint256 k = ((resA * 1000)) +
            (((resB - singleAmountB) * 997) * total0) /
            total1;

        uint256 sss = sqrt(
            k ** 2 +
                (((4 * 1000 * resA * singleAmountB * 997)) * total0) /
                total1
        );

        decreaseShare = ((sss - k) * totalShareSupply) / total0 / 2 / 997;
    }
}
