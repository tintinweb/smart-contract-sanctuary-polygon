...
contract AutoMeshedV2 is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        // We do some fancy math here. Basically, any point in time, the amount of MSD
        // entitled to a user but is pending to be distributed is:
        //
        //   amount = user.shares / sharesTotal * wantLockedTotal
        //   pending reward = (amount * pool.accMSDPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws want tokens to a pool. Here's what happens:
        //   1. The pool's `accMSDPerShare` (and `lastRewardBlock`) gets updated.
    ...
    uint256 public startBlock = 99999999;
    ...
    function initialize(address _MSD, uint256 _startBlock) public onlyOwner {
    ...
    // Update the given pool's MSD allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending MSD on frontend.
    ...
    // Want tokens moved from user -> MSDProtocol (MSD allocation) -> Strat (compounding)
    ...
    // Safe MSD transfer function, just in case if rounding error causes pool to not have enough
