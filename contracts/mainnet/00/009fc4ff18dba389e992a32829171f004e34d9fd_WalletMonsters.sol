...
contract WalletMonsters is ERC1155, Ownable, IERC1155Receiver{
   ...
   address reserves; //devs
   address escrowedValut; //contract
   address payable public FULLSTACKCALI;
   uint256 amountBurned = 0;
   uint256 walyAmount = 50000;
   string public name;
   ...
   constructor() ERC1155("https://p7vqo9fd8fws.usemoralis.com/{id}.json") {
...
function trainerMarket(uint256 itemId, uint256 amount) public  {
        ...
        require(itemId < 4, "only eDEF/eATK");
  ...
  function useWaly(address token,  uint tokenId) public {
       ...
       require(balanceOf(FULLSTACKCALI, tokenId) >= 1, "faucet is empty");
 
       IERC20(token).transferFrom(msg.sender, reserves, walyAmount *(10**18));
   ...
   function faucet() payable public returns(uint UUJIs){
       require(balanceOf(FULLSTACKCALI, 0) >= 50, "faucet is empty");
       require(msg.value >= faucetPrice, "Not enough for tx");
 ...
 function setWalyAmount(uint _cost) public onlyOwner returns(uint){
           walyAmount = _cost;
   ...
   function slay(address account, uint256 amount, uint256 enjimonId) public
   {
       ...
       require(enjimonId >= 6, "only $Enjimon");
       require(balanceOf(msg.sender, enjimonId) > 0, "No $Enjimon");
   ...
   function feed(uint256 tokenId) public {
       ...
       require(enjimon.lastMeal + enjimon.endurance > block.timestamp, "Immutable Death");
       require(uujiBalance > (50 + (27 + enjimon.level)), "Not enough UUJI");
   ...
   function tradeEnjimon(address to, uint256 tokenId) public {
       ...
       require(tokenId >= 6, "invalid ID");
       require(_hold[msg.sender][tokenId] == 0, "remove from marketplace!");
   ...
   {
       require(enjimonId >= 6, "only name $Enjimon!");
       require(balanceOf(msg.sender, enjimonId) > 0, "only give your $Enjimon nickname!");
   ...
   function buyItem(uint256 id) payable public ItemExist(id) IsForSale(id) {
           ...
           require(balanceOf(msg.sender, 0) >= 10, "10 UUJI min.");
 
         
           if(itemsForSale[id].tokenId >= 5)
           {
               ...
               emit itemSold(id, msg.sender, 1 ,itemsForSale[id].askingPrice);
               emit burnedCount(0, amountBurned, block.timestamp);
