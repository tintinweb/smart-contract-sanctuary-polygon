    ...
    AdminAccessControl
{
    ...
    constructor(string memory _baseURIInput, uint256 _maxSupply)
        ERC721("Pepper Attack", "PEPPER")
    {
    ...
    {
        require(
            _maxSupply >= _tokenIdTracker.current(),
            "PEPPER::MAX_SUPPLY_MUST_BE_GREATER_THAN_CURRENT_INDEX"
        );
        maxSupply = _maxSupply;
    ...
    {
        require(_qty > 0, "PEPPER::QUANTITY_MUST_BE_GREATER_THAN_ZERO");
        require(
            _receiverAddress != address(0),
            "PEPPER::CANNOT_MINT_TO_BLACKHOLE"
        );
        require(maxSupply > _tokenIdTracker.current(), "PEPPER::NO_MORE_TOKEN");
        require(
            _qty + _tokenIdTracker.current() <= maxSupply,
            "PEPPER::MINTING_WILL_EXCEED_MAX_SUPPLY"
        );

        uint256[] memory mintedTokenIds = new uint256[](_qty);
        // Mint
    ...
    function batchTransferToSingleAddress(
        address _receiver,
        uint256[] memory _tokenIds
    ) external returns (bool) {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            safeTransferFrom(_msgSender(), _receiver, tokenId);
        }

        return true;
    }

    function batchBurn(uint256[] memory _tokenIds) external returns (bool) {
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            uint256 tokenId = _tokenIds[i];
            burn(tokenId);
        }
