...
contract PredictionOpsManager is Ownable {
    ...
    constructor(IOddzPrediction[] memory _oddzPrediction) {
        for (uint256 i = 0; i < _oddzPrediction.length; i++) {
            setPredicitionMarket(_oddzPrediction[i]);
        }
    }

    function setPredicitionMarket(IOddzPrediction _oddzPredicition) public onlyOwner {
    ...
    function execute() public {
        for (uint256 i = 0; i < predictionMarkets.length; i++) {
            ...
            if (block.timestamp > round.roundEndTimestamp && round.roundState != IOddzPrediction.RoundState.ENDED) {
    ...
    function canPerformTask() external view returns (bool canPerform) {
        for (uint256 i = 0; i < predictionMarkets.length; i++) {
            ...
            canPerform =
                ...
                round.roundState != IOddzPrediction.RoundState.ENDED;
