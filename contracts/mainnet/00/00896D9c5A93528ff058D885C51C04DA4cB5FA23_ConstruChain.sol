/*
Developed by balder.world
t.me/construchainglobal
...
contract ConstruChain is ERC20, ERC20Burnable, Ownable, DividendTrackerFunctions {
    
    uint256 public swapThreshold;
    
    uint256 private _marketingPending;
    uint256 private _shareholderPending;
    ...
    uint16[3] public shareholderFees;
    ...
    event shareholderAddressUpdated(address shareholderAddress);
    event shareholderFeesUpdated(uint16 buyFee, uint16 sellFee, uint16 transferFee);
    event shareholderFeeSent(address recipient, uint256 amount);
    ...
    constructor()
        ERC20(unicode"ConstruChain", unicode"CTO") 
    {
        address supplyRecipient = 0xfa7f3dd603c59FCb1774D4705D9a42e38346ae23;
        
        updateSwapThreshold(50000 * (10 ** decimals()));

        marketingAddressSetup(0x82f65032C6aa20002c3F22b571fae61255327B4A);
        marketingFeesSetup(500, 500, 0);

        shareholderAddressSetup(0xE0Fccad08c608C8B1Ed45BA56405E1d0591C780f);
        shareholderFeesSetup(200, 200, 0);

        _deployDividendTracker(43200, 5000 * (10 ** decimals()), 0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
        ...
        excludeFromLimits(shareholderAddress, true);

        updateTradeCooldownTime(5);

        _mint(supplyRecipient, 100000000 * (10 ** decimals()));
        _transferOwnership(0xfa7f3dd603c59FCb1774D4705D9a42e38346ae23);
    }
    ...
    function getAllPending() public view returns (uint256) {
        return 0 + _marketingPending + _shareholderPending + _rewardsPending;
    }
    ...
    function marketingFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        ...
        totalFees[0] = 0 + marketingFees[0] + shareholderFees[0] + rewardsFees[0];
        totalFees[1] = 0 + marketingFees[1] + shareholderFees[1] + rewardsFees[1];
        totalFees[2] = 0 + marketingFees[2] + shareholderFees[2] + rewardsFees[2];
        require(totalFees[0] <= 2500 && totalFees[1] <= 2500 && totalFees[2] <= 2500, "TaxesDefaultRouter: Cannot exceed max total fee of 25%");

        emit marketingFeesUpdated(_buyFee, _sellFee, _transferFee);
    }

    function shareholderAddressSetup(address _newAddress) public onlyOwner {
        shareholderAddress = _newAddress;

        excludeFromFees(_newAddress, true);

        emit shareholderAddressUpdated(_newAddress);
    }

    function shareholderFeesSetup(uint16 _buyFee, uint16 _sellFee, uint16 _transferFee) public onlyOwner {
        shareholderFees = [_buyFee, _sellFee, _transferFee];
        ...
        emit shareholderFeesUpdated(_buyFee, _sellFee, _transferFee);
    }
    ...
    function _transfer(
        ...
        if (!_swapping && !AMMPairs[from] && canSwap) {
            ...
            if (false || _marketingPending > 0 || _shareholderPending > 0) {
                uint256 token2Swap = 0 + _marketingPending + _shareholderPending;
                bool success = false;
                ...
                uint256 shareholderPortion = coinsReceived * _shareholderPending / token2Swap;
                if (shareholderPortion > 0) {
                    (success,) = payable(address(shareholderAddress)).call{value: shareholderPortion}("");
                    require(success, "TaxesDefaultRouterWalletCoin: Fee transfer error");
                    emit shareholderFeeSent(shareholderAddress, shareholderPortion);
                }
                _shareholderPending = 0;
        ...
        if (!_swapping && amount > 0 && to != address(routerV2) && !isExcludedFromFees[from] && !isExcludedFromFees[to]) {
            ...
            if (txType < 3) {
                ...
                _shareholderPending += fees * shareholderFees[txType] / totalFees[txType];
