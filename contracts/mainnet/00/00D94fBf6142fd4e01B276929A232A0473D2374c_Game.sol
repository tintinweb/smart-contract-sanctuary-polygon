...
contract GameBase is Ownable {
    ...
    function gameInfo() public view returns (uint256[5] memory) {
        return [bets.length, _totalSupply, _totalInvested, _totalDivested, address(this).balance];
    }
...
contract Game is ReentrancyGuard, GameBase {
    ...
    function initialze(uint256 _fund) external payable onlyOwner {
        require(_totalSupply == uint256(0), "Can not initialze");
        require(msg.value == uint256(_fund+1), "Need 1Ether and 1 Wei for initialization");

        _totalInvested += uint256(_fund+1);
