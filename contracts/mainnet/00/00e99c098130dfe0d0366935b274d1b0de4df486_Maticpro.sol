...
contract Maticpro {
    ...
    ****************************************/
	...
	uint256 constant public adminCharge = 10; // 10%;
	...
	uint[5] public ref_bonuses = [10,7,8,11,14];
    
    uint256[5] public defaultPackages = [10 ether,30 ether,75 ether,150 ether,300 ether];
    
    mapping(uint256 => address payable) public singleLeg;
    uint256 public singleLegLength;

    uint[5] public requiredDirect = [1,1,2,2,2];
    ...
    constructor() public {
		primaryAdmin = 0xCEdF97589B1c0102B3557EF3211f16A036a3b4c7;
  ...
  function getEligibleWithdrawal(address _user) public view returns(uint8 reivest, uint8 withdrwal){  
      ...
      if(users[_user].refs[0] >=2 && (TotalDeposit >=defaultPackages[2] && TotalDeposit < defaultPackages[3])){
          reivest = 50;
          withdrwal = 50;
      }else if(users[_user].refs[0] >=4 && (TotalDeposit >=defaultPackages[3] && TotalDeposit < defaultPackages[4])){
          reivest = 40;
          withdrwal = 60;
