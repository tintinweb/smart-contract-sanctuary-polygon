...
contract DividendPayingToken is ERC20, IDividendPayingToken, IDividendPayingTokenOptional {
    ...
    address public adminAddress = 0xE81251Aa4CdeEB2a4e7044f421f2a75A7177cE82; //contract wallet
...
contract Flux1 is ERC20, Ownable {
    using SafeMath for uint256;
    ...
    uint256 public buyBackUpperLimit = 1 * 10 ** 18;        //1 MATIC

    // Minimum MATIC balance before buyback IF lower than this number no buyback
    uint256 public minimumBalanceRequired = 1 * 10 ** 18;   //1 MATIC
    ...
    constructor() ERC20("Flux1", "Flux1") {
        a80DividendTracker = new A80DividendTracker();
        b20DividendTracker = new B20DividendTracker();
        
        contractWallet = 0xE81251Aa4CdeEB2a4e7044f421f2a75A7177cE82;
        marketingWallet = 0xd45b21ddFe055eF997D41a025461AacB8e774830;
        a80DividendToken = 0x3E61d4D16d1250646106F4fF6A0f2C3a396C72E9; //Alpha
        b20DividendToken = 0x664fC1e7c522d73f7b57aC61845Fc617476ae512; //Ever

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff); //Quick
        ...
        */
        _mint(deadAddress, 2 * (10**9) * (10**18));
        _mint(contractWallet, 8 * (10**9) * (10**18));
    }
    ...
    function afterPreSale() external onlyOwner {
        ...
        b20DividendRewardsBuyFee = 3;
        marketingBuyFee = 2;
        buyBackAndLiquidityBuyFee = 1;
        
        a80DividendRewardsSellFee = 3;
        b20DividendRewardsSellFee = 6;
        marketingSellFee = 5;
        buyBackAndLiquiditySellFee = 1;
        ...
        swapTokensAtAmount = 3000000000 * (10**18);
        maxBuyTransactionAmount = 100000000000000 * (10**18);
        maxSellTransactionAmount = 500000000000 * (10**18);
        maxWalletToken = 10000000000000000 * (10**18);
    }
    ...
    function updateA80DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newA80DividendTracker.owner() == address(this), "must be owned by Baby Bonfire");
    ...
    function updateB20DividendTracker(address newAddress) external onlyOwner {
        ...
        require(newB20DividendTracker.owner() == address(this), "must be owned by Baby Bonfire");
    ...
    function _transfer(
        ...
        require(!isBlacklisted(from), "Flux1: sender blacklisted");
        require(!isBlacklisted(to), "Flux1: recipient blacklisted");
        require(!isBlacklisted(tx.origin), "Flux1: sender blacklisted");
        ...
        if (
            ...
            if (!swapping && contractTokenBalance >= swapTokensAtAmount) {
                ...
                if (buyBackAndLiquifyEnabled) {
                    if(buyBackMode){
                        swapTokensForMATIC(contractTokenBalance.mul(buyBackAndLiquiditySellFee).div(totalSellFees));
                    }else{
            ...
            if (!swapping && buyBackAndLiquifyEnabled && buyBackMode) {
                ...
                uint256 buyBackBalanceMatic = address(this).balance;
                if (buyBackBalanceMatic >= minimumBalanceRequired && amount >= minimumSellOrderAmount) {
                    swapping = true;

                    if (buyBackBalanceMatic > buyBackUpperLimit) {
                        buyBackBalanceMatic = buyBackUpperLimit;
                    }

                    buyBackAndBurn(buyBackBalanceMatic.div(10**2));
    ...
    function enableBlacklist(address account) public onlyOwner {
        require(!blacklist[account], "Flux1: Account is already blacklisted");
    ...
    function disableBlacklist(address account) public onlyOwner {
        require(blacklist[account], "Flux1: Account is not blacklisted");
...
contract A80DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("A80_Dividend_Tracker", "A80_Dividend_Tracker", 0x3E61d4D16d1250646106F4fF6A0f2C3a396C72E9, tx.origin) {
        claimWait = 3600;
...
contract B20DividendTracker is DividendPayingToken, Ownable {
    ...
    constructor() DividendPayingToken("B20_Dividend_Tracker", "B20_Dividend_Tracker", 0x664fC1e7c522d73f7b57aC61845Fc617476ae512, tx.origin) {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 200000 * (10**18); //must hold 10000+ tokens
    }
