...
contract Liquidity is UniV3QuoterCore, Ownable {
    ...
    constructor(INonfungiblePositionManager _nonfungiblePositionManager, address _TREND, address _WMATIC, uint24 _poolFee, address _factory) {
        nonfungiblePositionManager = _nonfungiblePositionManager;
        TREND = _TREND;
        poolFee = _poolFee;
        factory = _factory;
        TransferHelper.safeApprove(_TREND, address(uniV3), 21000000 ether);
        TransferHelper.safeApprove(_WMATIC, address(uniV3), 1000000000 ether);
    }
    ...
    /// For this example we are providing 1000 TREND and 1000 WMATIC in liquidity
    ...
    )
    {
        ...
        TransferHelper.safeTransferFrom(TREND, msg.sender, address(this), amount0ToMint);
        TransferHelper.safeTransferFrom(WMATIC, msg.sender, address(this), amount1ToMint);

        // Approve the position manager
        TransferHelper.safeApprove(TREND, address(nonfungiblePositionManager), amount0ToMint);
        ...
        // Note that the pool defined by TREND/WMATIC and fee tier 0.3% must already be created and initialized in order to mint
        ...
        if (amount0 < amount0ToMint) {
            TransferHelper.safeApprove(TREND, address(nonfungiblePositionManager), 0);
            uint256 refund0 = amount0ToMint - amount0;
            TransferHelper.safeTransfer(TREND, msg.sender, refund0);
        }
    ...
    function quoteExactInputSingle() public returns(uint amountOut) {
        TREND2USDT = quote.quoteExactInputSingle(TREND, USDT, poolFee, 1 ether, 0);
        amountOut = TREND2USDT;
    }
...
//        }
    ...
    function getStuck() external onlyOwner {
