...
// File: contracts/TrashPandazClubSteaking.sol


pragma solidity 0.8.11;

/* TrashPandazClub Steaking 🥩 */
...
contract TrashPandazClubSteaking is ERC721Holder, ReentrancyGuard, Ownable, Pausable {
    ...
    function rewardPerToken() public view returns (uint256) {
        ...
        return
            rewardPerTokenStored +
                ((rewardRate * (getMultiplier() / 100)) * (block.timestamp - lastUpdateTime) * 1e18 / totalSupply) ; //(getMultiplier() / 100)
    }
    ...
    function walletHoldsToken(address _contract, uint256 _id) internal view returns (bool) {
        IERC1155 token = IERC1155(_contract);
        //use msg.sender instead of _owner to get the user wallet address
        return token.balanceOf(msg.sender, _id) > 0; 
    }
    ...
    function getMultiplier() public view returns (uint256){ 
        ...
        for (uint i = 0; i < SM_Contracts.length; i++) {
            if ( walletHoldsToken(SM_Contracts[i], SM_TokenIds[i]) ) {
