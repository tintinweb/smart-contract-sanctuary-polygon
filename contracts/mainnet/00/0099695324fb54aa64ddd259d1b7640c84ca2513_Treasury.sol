...
contract Treasury is ContractGuard, Initializable, OwnableUpgradeable {
    ...
    modifier checkEpoch() {
        ...
        epoch = epoch.add(1);
        epochSupplyContractionLeft = (getHOSTPrice() > HOSTPriceCeiling)
            ? 0
            : getHOSTCirculatingSupply().mul(maxSupplyContractionPercent).div(
                10000
            );
    }
    ...
    modifier checkOperator() {
        require(
            ...
            "Treasury: need more permission from HOST"
        );
        require(
            IBasisAsset(bbond).operator() == address(this),
            "Treasury: need more permission from bbond"
        );
        require(
            IBasisAsset(bshare).operator() == address(this),
            "Treasury: need more permission from bshare"
        );
        require(
            Operator(boardroom).operator() == address(this),
            "Treasury: need more permission from boardroom"
        );
        _;
    }
    ...
    function getBurnableHOSTLeft()
        public
        view
        returns (uint256 _burnableHOSTLeft)
    {
        uint256 _HOSTPrice = getHOSTPrice();
        if (_HOSTPrice <= HOSTPriceOne) {
            uint256 _HOSTSupply = getHOSTCirculatingSupply();
            uint256 _bondMaxSupply = _HOSTSupply.mul(maxDebtRatioPercent).div(
                10000
            );
            uint256 _bondSupply = IERC20(bbond).totalSupply();
            if (_bondMaxSupply > _bondSupply) {
                uint256 _maxMintableBond = _bondMaxSupply.sub(_bondSupply);
                uint256 _maxBurnableHOST = _maxMintableBond.mul(_HOSTPrice).div(
                    HOSTPriceOne
                );
                _burnableHOSTLeft = Math.min(
    ...
    function getBondPremiumRate() public view returns (uint256 _rate) {
        uint256 _HOSTPrice = getHOSTPrice();
        if (_HOSTPrice > HOSTPriceCeiling) {
            uint256 _HOSTPricePremiumThreshold = HOSTPriceOne
                .mul(premiumThreshold)
                .div(100);
            if (_HOSTPrice >= _HOSTPricePremiumThreshold) {
                //Price > 1.10
                uint256 _premiumAmount = _HOSTPrice
                    .sub(HOSTPriceOne)
                    .mul(premiumPercent)
                    .div(10000);
    ...
    function setHOSTPriceCeiling(uint256 _HOSTPriceCeiling)
        external
        onlyOperator
    {
        require(
            _HOSTPriceCeiling >= HOSTPriceOne &&
                _HOSTPriceCeiling <= HOSTPriceOne.mul(120).div(100),
            "out of range"
        ); // [$1.0, $1.2]
    ...
    {
        require(
            _premiumThreshold >= HOSTPriceCeiling,
            "_premiumThreshold exceeds HOSTPriceCeiling"
        );
        require(
    ...
    function getHOSTCirculatingSupply() public view returns (uint256) {
        ...
        for (
            ...
            balanceExcluded = balanceExcluded.add(
                HOSTErc20.balanceOf(excludedFromTotalSupply[entryId])
            );
        }
        return totalSupply.sub(balanceExcluded);
    }

    function buyBonds(uint256 _HOSTAmount, uint256 targetPrice)
        external
        isHuman
        onlyOneBlock
        checkCondition
        checkOperator
    {
        ...
        require(
            _HOSTAmount <= epochSupplyContractionLeft,
        ...
        require(
            newBondSupply <= HOSTSupply.mul(maxDebtRatioPercent).div(10000),
            "over max debt ratio"
        );
        ...
        checkOperator
    {
        ...
        require(
            IERC20(HOST).balanceOf(address(this)) >= _HOSTAmount,
            "Treasury: treasury has no more budget"
        );

        seigniorageSaved = seigniorageSaved.sub(
            Math.min(seigniorageSaved, _HOSTAmount)
        );
    ...
    function _calculateMaxSupplyExpansionPercent(uint256 _HOSTSupply)
        internal
        returns (uint256)
    {
        ...
        checkOperator
    {
        ...
        if (epoch < bootstrapEpochs) {
            ...
            _sendToBoardroom(
                HOSTSupply.mul(bootstrapSupplyExpansionPercent).div(10000)
            );
        } else {
            if (previousEpochHOSTPrice > HOSTPriceCeiling) {
                ...
                uint256 _percentage = previousEpochHOSTPrice
                    .sub(HOSTPriceOne)
                    .mul(1000);
                uint256 _savedForBond;
                uint256 _savedForBoardroom;
                uint256 _mse = _calculateMaxSupplyExpansionPercent(HOSTSupply)
                    .mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
                if (
                    ...
                    uint256 _seigniorage = HOSTSupply.mul(_percentage).div(
                        1e18
                    );
    ...
    function _simulateMaxSupplyExpansionPercent(uint256 _HOSTSupply)
        internal
        view
        returns (uint256)
    {
    ...
    function simulateSeigniorage()
        public
        view
        returns (uint256 _savedForBond, uint256 _savedForBoardroom)
    {
        uint256 _previousEpochHOSTPrice = getHOSTPrice();
        uint256 HOSTSupply = getHOSTCirculatingSupply().sub(seigniorageSaved);
        if (epoch < bootstrapEpochs) {
            // 28 first epochs with 4.5% expansion
            _savedForBoardroom = HOSTSupply
                .mul(bootstrapSupplyExpansionPercent)
                .div(10000);
        } else {
            if (_previousEpochHOSTPrice > HOSTPriceCeiling) {
                // Expansion ($TOMB Price > 1 $FTM): there is some seigniorage to be allocated
                uint256 bondSupply = IERC20(bbond).totalSupply();
                uint256 _percentage = _previousEpochHOSTPrice
                    .sub(HOSTPriceOne)
                    .mul(1000);
                uint256 _mse = _simulateMaxSupplyExpansionPercent(HOSTSupply)
                    .mul(1e14);
                if (_percentage > _mse) {
                    _percentage = _mse;
                }
    ...
    function setExcludedFromTotalSupply(address[] memory _excluded)
        external
        onlyOperator
    {
