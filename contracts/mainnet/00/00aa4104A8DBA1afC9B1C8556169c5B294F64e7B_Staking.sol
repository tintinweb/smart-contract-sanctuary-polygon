...
contract Staking is AccessControl {
    ...
    mapping (address => bool) internal _allowedYieldAddress;
    ...
    function addDepositProfile (
        ...
        uint256 lockedAmount = _getLockedAmount(
            uintData[0],
    ...
    function _getLockedAmount(
        ...
        if (depositContractAddress == yieldContractAddress) {
        ...
        (bool success, bytes memory data) = yieldContractAddress.call(callData);
        if (!success) lp = false;
        else (token0) = abi.decode(data, (address));
        callData = abi.encodeWithSignature(
            'token1()'
        );
        (success, data) = yieldContractAddress.call(callData);
        if (!success) lp = false;
        else (token1) = abi.decode(data, (address));
        if (
            lp &&  (
                token0 == depositContractAddress && _allowedYieldAddress[token1]
                    || token1 == depositContractAddress && _allowedYieldAddress[token0]
            )
        ) {
    ...
    function setAllowedYield (
        address contractAddress,
        bool allowed
    ) external onlyManager returns (bool) {
        _allowedYieldAddress[contractAddress] = allowed;
        return true;
    }
    ...
    function getAllowedYield (
        address contractAddress
    ) external view returns (bool) {
        return _allowedYieldAddress[contractAddress];
    }
