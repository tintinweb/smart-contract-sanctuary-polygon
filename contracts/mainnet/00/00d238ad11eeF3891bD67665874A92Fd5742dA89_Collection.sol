...
import "./utils/CollectionWhitelist.sol";



abstract contract ERC2981 is IERC2981, ERC165,ReentrancyGuard {
    struct RoyaltyInfo {
        address receiver;
    ...
    event RoyaltyInfoReset(uint256 tokenId);
    ...
    function _setTokenRoyalty(
        ...
        require(
            feeNumerator <= maxRoyalty,
            "Collection ERC2981:royality must be less than max limit"
        );
        require(
            feeNumerator <= _feeDenominator(),
            "Collection ERC2981: royalty fee will exceed salePrice"
        );
        require(
            receiver != address(0),
            "Collection ERC2981: Invalid parameters"
        );
        _tokenRoyaltyInfo[tokenId] = RoyaltyInfo(receiver, feeNumerator);
        emit RoyaltyInfoSet(tokenId, receiver, feeNumerator);
    } 
...
contract Collection is ERC2771Context, ERC721URIStorage, ERC2981, Whitelist {
    ...
    /**
     * @dev Mapping from token to initial owner.
     */
    ...
    /**
     *@dev modifer to check only initial owner can set royality
     */
    ...
    function changeLimitOfRoyality(uint256 _limit) external onlyOwner {
        require(
            ...
            "Collection: royalty must be less than equal to 100%"
        );
        uint256 _oldLimit = maxRoyalty;
        require(_limit != _oldLimit, "Collection: limit must be different");
        maxRoyalty = _limit;
    ...
    function batchMint(
        ...
        require(
            _numberOfToken == _tokenURI.length,
            "Collection: _tokenURI lenght is not valid"
        );
     ...
     */
    function mint(address _to, string memory _tokenURI) public onlyAuthorized nonReentrant {
        require(_to != address(0), "Collection: _to address not valid");
        counter++;
        uint256 tokenId = counter;
        totalSupply = totalSupply + 1;
        initialOwner[tokenId] = _to;
        bool isAdded = _holderTokens[_to].add(tokenId);
        require(isAdded, "Collection: _holderTokens contains invalid token");
    ...
    function burn(uint256 _tokenId) external {
        ...
        bool isRemoved = _holderTokens[tokenOwner].remove(_tokenId);
        require(isRemoved,"Collection: invalid token id");
    ...
    function approve(address to, uint256 tokenId) public override {
        if (to.isContract()) {
            require(whitelisted(to), "Collection: Only whitelist is allowed");
        }
        ...
        override
    {
        if (operator.isContract()) {
            require(
                ...
                "Collection: Only whitelist is allowed"
            );
        }
    ...
    function transferFrom(
        ...
        bool isRemoved = _holderTokens[from].remove(tokenId);
        bool isAdded = _holderTokens[to].add(tokenId);
        require(isRemoved && isAdded, "Collection: invalid token id");
...
import "./CollectionAuthorizable.sol";
...
contract Authorizable is Ownable {
    ...
    modifier onlyAuthorized() {
        require(
            ...
            "Collection Authorizable: caller is not the SuperAdmin or Admin"
        );
        _;
    }
    ...
    function addAuthorized(address _toAdd) external onlyOwner {
        require(
            _toAdd != address(0),
            "Collection Authorizable: _toAdd isn't vaild address"
        );
        require(
            !authorized[_toAdd],
            "Collection Authorizable: _toAdd is already added"
        );
        ...
        onlyOwner
    {
        require(
            _toRemove != address(0),
            "Collection Authorizable: _toRemove isn't vaild address"
        );
        require(
            adminList[_index] == _toRemove,
            "Collection Authorizable: _index isn't valid index"
        );
