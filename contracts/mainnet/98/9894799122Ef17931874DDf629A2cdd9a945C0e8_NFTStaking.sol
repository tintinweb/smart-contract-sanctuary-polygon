...
//CYBER TOKENS REWARDS
interface EvolvedRewards {
...
contract NFTStaking is Ownable, IERC721Receiver {
  ...
  function stake(uint256[] calldata tokenIds) external {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      vault[tokenId] = Stake({
        ...
        lastClaimed: uint256(block.timestamp)
      });
    }
  }
  ...
  function _claim(address account, uint256[] calldata tokenIds, bool _unstake) internal {
    uint256 tokenId;
    uint256 earned = 0;
    uint256 totalRewardTime = 0;
    uint256 totalStakeTime = 0;
    for (uint i = 0; i < tokenIds.length; i++) {
      tokenId = tokenIds[i];
      Stake memory staked = vault[tokenId];
      require(staked.owner == account, "not an owner");
      totalRewardTime = block.timestamp - staked.lastClaimed;
      totalStakeTime = block.timestamp - staked.timestamp;
      earned = (250 ether * totalRewardTime) / 86400 ;
      vault[tokenId] = Stake({
        owner: account,
        tokenId: uint24(tokenId),
    ...
    if (_unstake) {
       require(totalStakeTime >= 2629743,"You can't unstake before the locktime." );
        token.mint(account, earned);
        _unstakeMany(account, tokenIds);
    }
    else if (earned > 0) {
      require(totalRewardTime >= 604800,"You can't claim the rewards before the locktime." );
      token.mint(account, earned);
    }
  ...
  function earningInfo(address account, uint256[] calldata tokenIds) external view returns (uint256[1] memory info) {
    ...
    for (uint i = 0; i < tokenIds.length; i++) {
      ...
      Stake memory staked = vault[tokenId];
      require(staked.owner == account, "not an owner");
      uint256 earnTotalTime = block.timestamp - staked.lastClaimed;
      earned = (250 ether * earnTotalTime) / 86400;
    }
