 ...
 * @author AlloyX
 */
contract StakeDesk is IStakeDesk, AdminUpgradeable {
  ...
  // vault address=> govToken address
  mapping(address => address) public govTokenMap;

  using ConfigHelper for AlloyxConfig;

  event Stake(address _vaultAddress, address _staker, uint256 _amount);
  event Unstake(address _vaultAddress, address _unstaker, uint256 _amount);
  event ClearAllStake(address _vaultAddress);
  event AlloyxConfigUpdated(address indexed who, address configAddress);
  event SetGovTokenAddress(address indexed who, address vaultAddress, address govTokenAddress);
   ...
   */
  function setGovTokenForVault(address _vaultAddress, address _govTokenAddress) external override notPaused {
    require(msg.sender == config.managerAddress(), "must be called by manager");
    govTokenMap[_vaultAddress] = _govTokenAddress;
    emit SetGovTokenAddress(msg.sender, _vaultAddress, _govTokenAddress);
  }
   ...
   */
  function addPermanentStakeInfo(address _account, uint256 _amount) external override notPaused isVault {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    config.getPermanentStakeInfo().addStake(vaultAddress, _account, _amount);
    IMintBurnableERC20(govTokenAddress).mint(_account, _amount);
    emit Stake(vaultAddress, _account, _amount);
  }
   ...
   */
  function subPermanentStakeInfo(address _account, uint256 _amount) external override notPaused isVault {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    config.getPermanentStakeInfo().removeStake(vaultAddress, _account, _amount);
    IMintBurnableERC20(govTokenAddress).burn(_account, _amount);
    emit Unstake(vaultAddress, _account, _amount);
  }
   ...
   * @param _amount the amount the message sender intending to stake in
   */
  function addRegularStakeInfo(address _account, uint256 _amount) external override notPaused isVault {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    config.getRegularStakeInfo().addStake(vaultAddress, _account, _amount);
    IMintBurnableERC20(govTokenAddress).mint(_account, _amount);
    emit Stake(vaultAddress, _account, _amount);
  }
   ...
   * @param _amount the amount the message sender intending to unstake
   */
  function subRegularStakeInfo(address _account, uint256 _amount) external override notPaused isVault {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    config.getRegularStakeInfo().removeStake(vaultAddress, _account, _amount);
    IMintBurnableERC20(govTokenAddress).burn(_account, _amount);
    emit Unstake(vaultAddress, _account, _amount);
  }
   ...
   */
  function getRegularStakerProrataGain(address _staker, uint256 _gain) external view override returns (uint256) {
    address vaultAddress = msg.sender;
    uint256 stakerTemporalStake = config.getRegularStakeInfo().receiverTemporalStake(vaultAddress, _staker);
    uint256 vaultTemporalStake = config.getRegularStakeInfo().vaultTemporalStake(vaultAddress);
    if (vaultTemporalStake == 0) {
      return 0;
    }
    return stakerTemporalStake.mul(_gain).div(vaultTemporalStake);
  }
   ...
   */
  function getPermanentStakerProrataGain(address _staker, uint256 _gain) external view override returns (uint256) {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    uint256 stakerTemporalStake = config.getPermanentStakeInfo().receiverTemporalStake(vaultAddress, _staker);
    uint256 vaultTemporalStake = config.getPermanentStakeInfo().vaultTemporalStake(vaultAddress);
    if (vaultTemporalStake == 0) {
      return 0;
    }
    return stakerTemporalStake.mul(_gain).div(vaultTemporalStake);
   ...
   */
  function clearStakeInfoAfterClaiming(address _staker) external override isVault {
    address vaultAddress = msg.sender;
    config.getPermanentStakeInfo().resetStakeTimestampWithTemporalStake(vaultAddress, _staker, 0);
    config.getRegularStakeInfo().resetStakeTimestampWithTemporalStake(vaultAddress, _staker, 0);
  }

  // TODO: need to change this because high gas
  /**
   * @notice Unstake all the regular and permanent stakers and burn all govTokens
   */
  function unstakeAllStakersAndBurnAllGovTokens() external override notPaused isVault {
    address vaultAddress = msg.sender;
    address govTokenAddress = govTokenMap[vaultAddress];
    address[] memory allRegularStakers = config.getRegularStakeInfo().getAllStakers(vaultAddress);
    for (uint256 i = 0; i < allRegularStakers.length; i++) {
      IMintBurnableERC20(govTokenAddress).burn(allRegularStakers[i], IMintBurnableERC20(govTokenAddress).balanceOf(allRegularStakers[i]));
    }
    config.getRegularStakeInfo().removeAllStake(vaultAddress);
    address[] memory allPermanentStakers = config.getPermanentStakeInfo().getAllStakers(vaultAddress);
    for (uint256 i = 0; i < allPermanentStakers.length; i++) {
      IMintBurnableERC20(govTokenAddress).burn(allPermanentStakers[i], IMintBurnableERC20(govTokenAddress).balanceOf(allPermanentStakers[i]));
    }
    config.getPermanentStakeInfo().removeAllStake(vaultAddress);
    emit ClearAllStake(vaultAddress);
  }

  /**
   * @notice Get the voting power of offchain governance
   * @param _staker the staker to calculate the gain to whom the gain is entitled
   */
  function getVotingPower(address _staker) external view returns (uint256) {
    uint256 regularStakes = config.getRegularStakeInfo().totalStakeForUser(_staker);
    uint256 permanentStakes = config.getPermanentStakeInfo().totalStakeForUser(_staker);
    uint256 stakerAlyxBalance = config.getAlyx().balanceOf(_staker);
    return regularStakes.add(permanentStakes).add(stakerAlyxBalance);
  }
}
