...
library Address {
    ...
    function functionCallWithValue(
        ...
        return functionCallWithValue(target, data, value, "low-level call failed");
    }
...
abstract contract Ownable is Context {
    ...
    function transferOwnership(address newOwner) public virtual onlyOwner {
        require(newOwner != address(0), "New owner 0 address");
...
abstract contract ERC2981 is IERC2981, ERC165 {
    ...
    function _setDefaultRoyalty(address receiver, uint96 feeNumerator) internal virtual {
        require(feeNumerator <= _feeDenominator(), "royalty>salePrice");
    ...
    function _setTokenRoyalty(
        ...
        require(feeNumerator <= _feeDenominator(), "royalty will>salePrice");
...
contract MafiaTownSale is ERC721A("MafiaTown.wtf", "MT"), Ownable, ERC721AQueryable, ERC2981 {
    ...
    function purchaseTokensFree(uint256 _howMany) external saleActive callerIsUser mintLimit(_howMany) tokensAvailable(_howMany) {
        require(_totalMinted() < freeMint, "Max free reached");
    ...
    modifier saleActive() {
        require(block.timestamp > saleActiveTime, "come back later");
        _;
    }
    ...
    modifier tokensAvailable(uint256 _howMany) {
        require(_howMany <= maxSupply - totalSupply(), "we are sold out");
        _;
    }

    modifier priceAvailable(uint256 _howMany) {
        require(msg.value == _howMany * itemPrice, "send the exact amount of ETH");
        _;
    }
