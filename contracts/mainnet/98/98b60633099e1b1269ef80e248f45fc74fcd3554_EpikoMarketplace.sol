...
interface IMarket{
    ...
    event PlaceBid(address nftAddress, address bidder, uint256 price, uint256 tokenId);
    event MarketItemCreated(address indexed nftAddress, address indexed seller, uint256 price, uint256 indexed tokenId, uint256 quantity);
    event Buy(address indexed seller, address bidder, uint256 indexed price, uint256 indexed tokenId, uint256 quantity);
    event AuctionCreated(address indexed nftAddress, uint256 indexed tokenId, address indexed seller, uint256 price, uint256 quantity, uint256 startTime, uint256 endTime);
    event CancelBid(uint256 indexed tokenId, address indexed seller);
}   
...
contract EpikoMarketplace is IMarket,Ownable{
    ...
    uint256 public _buyTax  = 110;//divide by 100
    uint256 public _sellTax = 110;//divide by 100
    ...
    modifier onlySellerOrOwner (address nftAddress, uint256 tokenId, address user, uint256 saleType) {
        if(saleType == 1){
            Sale storage sale = nftSaleItem[nftAddress][user][tokenId];
            require((sale.seller == msg.sender) || owner() == msg.sender, "Market: Only seller or owner can cancel sell");
        }else{
            Auction storage auction = nftAuctionItem[nftAddress][user][tokenId];
            require(auction.seller == msg.sender || owner() == msg.sender, "Market: only seller or owner can cancel sell");
        }
        _;
    }
    ...
    mapping(address => mapping(address => mapping(uint256 => Auction))) public nftAuctionItem;
    /// @dev Mapping from Nft contract to tokenId to Sale structure
    mapping(address => mapping(address => mapping(uint256 => Sale))) public nftSaleItem;
    ...
    function mint(
        ...
        require(royaltyFraction <= PERCENTAGE_DENOMINATOR, "Market: invalid royaltyFraction provided");
    ...
    function sellitem(
        ...
        if(IERC721Minter(nftAddress).supportsInterface(ERC721InterfaceId)){           
            ...
            require(IERC721(nftAddress).ownerOf(tokenId) == msg.sender, "Market: not an nft owner");
            
            _addItemtoSell(nftAddress, tokenId, price, 1 , seller);
            
            emit MarketItemCreated(nftAddress, seller, price, tokenId, amount);
            ...
            require(IERC1155(nftAddress).balanceOf(msg.sender,tokenId) >= amount, "Market: not enough balance");
    ...
    function buyItem(
        ...
        require(quantity <= sale.quantity, "Market: not enough quantity avaialble");
        require(seller != msg.sender, "Market: Owner not Allowed");
        ...
        if(IERC721(nftAddress).supportsInterface(ERC721InterfaceId)){
            ...
            uint256 totalNftValue = price.mul(quantity);
            ...
            emit Buy(seller, msg.sender, price, tokenId, sale.quantity);
            ...
            // sellItem.onSell = false;
            emit Buy(seller, msg.sender, price, tokenId, quantity);
    ...
    function createAuction(
        ...
        emit AuctionCreated(nftAddress,tokenId, seller, basePrice, amount, startTime, endTime);
    ...
    function placeBid(
        ...
        emit PlaceBid(nftAddress, msg.sender, price, tokenId);
    ...
    function approveBid(
        ...
        if(IERC721(nftAddress).supportsInterface(ERC721InterfaceId)){
            ...
            emit Buy(auction.seller, msg.sender, bidderValue, tokenId, auction.quantity);
            ...
            auction.sold = true;
            itemIdOnAuction[nftAddress][auction.seller][tokenId] = false;
            emit Buy(auction.seller, bidder, bidderValue, tokenId, auction.quantity);
    ...
    function cancelSell(
        ...
        require(!nftSaleItem[nftAddress][msg.sender][tokenId].sold, "Market: NFT Sold");
        
        delete nftSaleItem[nftAddress][msg.sender][tokenId];
        itemIdOnSale[nftAddress][msg.sender][tokenId] = false;
    }
    ...
    function cancelBid(
        ...
        require(tokenId > 0, "Market: not valid id");
        require(nftAuctionItem[nftAddress][msg.sender][tokenId].endTime > block.timestamp, "Market: Auction ended");
        require(fundsByBidder[nftAddress][tokenId][msg.sender] > 0, "Market: not bided on auction");

        // delete bidAndValue[nftAddress][tokenId];

        _removeBid(nftAddress, seller, tokenId, msg.sender);

        emit CancelBid(tokenId,seller);
    }
    ...
    function setSellTax(
        ...
        require(percentage <= PERCENTAGE_DENOMINATOR, "Market: percentage must be less than 100");
        _sellTax = percentage;
    }
    ...
    function _transferTokens(
        ...
        omiToken.transfer(_seller, amountForSeller - royaltyAmount);

        if(royaltyReceiver != address(0)){
            omiToken.transfer(royaltyReceiver, royaltyAmount);
        }
    }
    ...
    function _tokenDistribute(
        ...
        if(royaltyReceiver != address(0)){
            omiToken.transfer(_seller, _amount);
        }
    ...
    function _removeBid(address nftAddress, address seller, uint256 tokenId, address _bidder) internal {
    ...
    function _addItemtoSell(
        ...
        Sale storage sale = nftSaleItem[nftAddress][_seller][tokenId];
        ...
        itemIdOnSale[nftAddress][_seller][tokenId] = true;
    ...
    function revokeAuction(address nftAddress, uint256 tokenId) external {
        ...
        require(itemIdOnAuction[nftAddress][msg.sender][tokenId], "Market: NFT not on auction");
        require(nftAuctionItem[nftAddress][msg.sender][tokenId].endTime < block.timestamp, "Auction is not ended");
        require(nftAuctionItem[nftAddress][msg.sender][tokenId].highestBid.bid == 0, "Revoke not Allowed");
