...
contract DividendDistributor is IDividendDistributor {
    ...
    IERC20 RWRD = IERC20(0x7ceB23fD6bC0adD59E62ac25578270cFf1b9f619);
...
contract Rabat is ERC20, Auth {
    ...
    constructor() Auth(msg.sender) ERC20("Rabat", "RABAT") {
        _totalSupply = 100 * 1e18; // 18 decimals

        _maxTxAmount = (_totalSupply * 2) / 100; // 2%
        _maxWalletToken = (_totalSupply * 3) / 100; // 3%
        swapThreshold = (_totalSupply / (1000)) * (1); // 0.1%
        ...
        setIsFeeExempt(msg.sender, true);
        setIsFeeExempt(address(this), true);
        setIsFeeExempt(address(0xdead), true);

        setIsTxLimitExempt(msg.sender, true);
        setIsTxLimitExempt(address(this), true);
        setIsTxLimitExempt(address(0xdead), true);
        setIsTxLimitExempt(address(pair), true);

        setIsDividendExempt(address(this), true);
        setIsDividendExempt(address(0xdead), true);
        setIsDividendExempt(address(pair), true);
    ...
    function _transfer(
        ...
        // require trading openned or authorization
        require(tradingOpen || authorizations[from], "Trading Off");

        bool buy = (from == pair);
        bool sell = (to == pair);

        if (!authorizations[from] && !authorizations[to] && to != address(0xdead)) {
            if ((sell || buy) && !isTxLimitExempt[from]) { // maxTx 
                require(amount <= _maxTxAmount, "Amount exceeds the maxTx");
            } 
            if(!(sell) && !isTxLimitExempt[to]) {
    ...
    function setMaxTx(uint multiplier) external onlyOwner {
        _maxTxAmount = (_totalSupply.mul(multiplier)).div(100);
    }

    function setMaxWallet(uint multiplier) external onlyOwner {
        _maxWalletToken = (_totalSupply.mul(multiplier)).div(100);
    }
