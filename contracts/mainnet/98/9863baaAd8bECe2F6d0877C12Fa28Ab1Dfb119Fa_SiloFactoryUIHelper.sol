...
contract SiloFactoryUIHelper is Ownable {
    ...
    */
    ...
    //     uint count;
    //     string memory name = SiloFactory.catalogue(_category)[_strategyTypeIndex];
    //     ISilo silo;
        ...
        )
    {
        ...
        inputs = SiloFactory.strategyInputs(id);
        actions = SiloFactory.strategyActions(id);
        configurationData = SiloFactory.strategyConfigurationData(id);
    }
    ...
    */
        ...
        returns (string[] memory names, uint256[] memory categories)
    {
        ...
        for (uint256 i = 0; i < availableCategories.length; i++) {
            _names = SiloFactory.catalogue(i);
