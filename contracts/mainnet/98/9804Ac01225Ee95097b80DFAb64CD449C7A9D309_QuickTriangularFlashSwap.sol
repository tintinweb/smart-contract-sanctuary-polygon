...
contract QuickTriangularFlashSwap {
    ...
    // Declare a mapping to store the latest prices of USDT, MATIC, and LINK
    ...
    function checkProfitability(
        ...
        uint256 tradeValue = calculateTradeValue(maticAmount, linkAmount);

        // Check if the trade is profitable
        return tradeValue > tradeCost;
        }

    function calculateTradeValue(uint256 maticAmount, uint256 linkAmount) public view returns (uint256) {
        // Get the current prices of MATIC and LINK in USDT
        uint256 maticPrice = getMaticPrice();
        uint256 linkPrice = getLinkPrice();

        // Calculate the value of the trade in USDT
        return maticAmount * maticPrice + linkAmount * linkPrice;
    }

    // Declare the getMaticPrice() function
    ...
    function startArbitrage(address _tokenBorrow, uint256 _amount) external {
        ...
        IERC20(MATIC).safeApprove(address(QUICKSWAP_ROUTER), MAX_INT);
    ...
    function quickswapCall(
        ...
        uint256 trade2AcquiredCoin = placeTrade(LINK, MATIC, trade1AcquiredCoin);
        uint256 trade3AcquiredCoin = placeTrade(MATIC, USDT, trade2AcquiredCoin);
