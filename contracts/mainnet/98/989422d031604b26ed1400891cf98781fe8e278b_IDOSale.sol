...
contract IDOSale is AccessControl, Pausable, ReentrancyGuard {
    ...
    event Claimed(address indexed sender);
   ...
   function depositTokens(uint256 amount) external onlyOperator whenNotPaused {
    ...
    require(ido.approve(address(this), amount), "IDOSale: APPROVAL_FAILED");
    require(ido.transferFrom(_msgSender, address(this), amount), "IDOSale: TRANSFER_FAILED");
    ...
    uint256 purchaseTokenAmount = amount*2/10000000000000;
    require(purchaseTokenAmount <= purchaseToken.balanceOf(_msgSender()), "IDOSale: INSUFFICIENT_FUNDS");
    require(purchaseToken.approve(address(this), purchaseTokenAmount), "IDOSale: APPROVAL_FAILED");
    ...
    function claim(uint256 amount) external nonReentrant whenNotPaused {
        ...
        purchasedAmounts[_msgSender()] = purchasedAmounts[_msgSender()]-amount;
    ...
    function sweepICO(address to, uint256 amount) external onlyOwner {
        require(endTime <= block.timestamp, "IDOSale: SALE_NOT_ENDED");
        ido.safeTransfer(to, amount);
    }
}