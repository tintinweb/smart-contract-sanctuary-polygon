...
contract Market is IMarket, Ownable, NFTAssist, ReentrancyGuard {
    ...
    function getOffer(uint256 offerId) public view returns (OfferOutput memory) {
        ...
        OfferOutput memory offerOutput = OfferOutput(_offers[offerId]);
        offerOutput.offer.nft.tokenURI = _tokenURI(
            offerOutput.offer.nft.nftContract,
            offerOutput.offer.nft.tokenId
        );
        return offerOutput;
    }
    ...
    function offersStatus(
        ...
        require(offset < _offers.length, 'Not existed lotId');

        OfferOutput[] memory offersOutput = new OfferOutput[](limit);
        uint256 inIndex = 0;
        uint256 outIndex = 0;

        while (offset + inIndex < offersOutput.length && outIndex < limit) {
