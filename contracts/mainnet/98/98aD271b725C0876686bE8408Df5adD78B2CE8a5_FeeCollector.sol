...
contract FeeCollector is AccessControl {
    ...
    IERC721DividendPaying public nftGov;
    address public systemFeeAddr;
    ...
    constructor(
        ...
        address _systemFeeAddr
    ) {
        _setupRole(DEFAULT_ADMIN_ROLE, msg.sender);
        distribution = 50e18;  // 50%

        nftGov = IERC721DividendPaying(_nftGov);
        rewardToken = IERC20(_rewardToken);
        systemFeeAddr = _systemFeeAddr;
    }
    function setSystemFeeAddr(address _systemFeeAddr) external onlyAdmin {
            systemFeeAddr = _systemFeeAddr;
    }
    function setDistribution(uint256 _distribution) external onlyAdmin {
        require(distribution != _distribution, "same value");
        distribution = _distribution;        
    }
    function claim() external {
        uint256 totalAmount = rewardToken.balanceOf(address(this));
        require(totalAmount > 0, "nothing to be claimed");
        uint256 feeAmount = totalAmount.mul(distribution)
                .div(PERCENT_PRECISION.mul(100));
        IERC20(rewardToken).safeTransfer(systemFeeAddr, feeAmount);
        rewardToken.approve(address(nftGov), totalAmount.sub(feeAmount));
        nftGov.distributeDividends(totalAmount.sub(feeAmount));
    }

    function withdraw(address _token, address _to) external onlyAdmin {
