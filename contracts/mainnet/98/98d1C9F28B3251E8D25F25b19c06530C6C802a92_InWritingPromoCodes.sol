...
contract InWritingPromoCodes is Ownable{
    address InWriting_address = 0xd9Bd0a41aA6fadC0E9Ea5Eabc4d816c2ac277Ce7;
    ...
    function add_promo(string memory promo_code, uint256 num, uint8 discount) public onlyOwner {
        promoCodes[promo_code] = num;
        promoDiscount[promo_code] = discount;
    }

    function add_affiliate(string memory affiliate_code, address addr, uint8 discount, uint8 commission) public onlyOwner {
        require(!affiliateCodeUsed[affiliate_code], "affiliate code already in use");
    ...
    function get_promo_remaining_uses(string memory promo_code) public view returns (uint256) {
        return promoCodes[promo_code];
    }

    function get_promo_discount(string memory promo_code) public view returns (uint8) {
        return promoDiscount[promo_code];
    }

    function get_affiliate_used(string memory affiliate_code) public view returns (bool) {
        return affiliateCodeUsed[affiliate_code];
    }

    function get_affiliate_address(string memory affiliate_code) public view returns (address) {
        return affiliateAddress[affiliate_code];
    }

    function get_affiliate_discount(string memory affiliate_code) public view returns (uint8) {
        return affiliateDiscount[affiliate_code];
    }

    function get_affiliate_commission(string memory affiliate_code) public view returns (uint8) {
        return affiliateCommission[affiliate_code];
    }

    function get_price_with_promo(string memory promo_code) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - promoDiscount[promo_code])) / 100;
    }

    function get_price_with_affiliate(string memory affiliate_code) public view returns (uint256) {
        return (write.get_minting_cost() * (100 - affiliateDiscount[affiliate_code])) / 100;
    }
    ...
    function set_become_affiliate_commission(uint8 commission) public onlyOwner{
    ...
    function mint_promo_NFT(string memory str, string memory promo_code) public payable returns (uint256) {
        bytes32 h = keccak256(abi.encodePacked(msg.sender, promo_code));
        require(!promoCodeUsed[h], "this promotional code has been used for this address already");
        require(promoCodes[promo_code] > 0, "this promotional code has expired");
        uint256 cost = (write.get_minting_cost() * (100 - promoDiscount[promo_code])) / 100;
        require(msg.value >= cost, string(abi.encodePacked("payment not sufficient, this promotional code is only ", Strings.toString(promoDiscount[promo_code]), "%")));
    ...
    function mint_and_send_affiliate_NFT(string memory str, string memory affiliate_code, address addr) public payable returns (uint256) {
        require(affiliateCodeUsed[affiliate_code], "this affiliate code does not exist");
        uint256 cost = ((write.get_minting_cost() * (100 - affiliateDiscount[affiliate_code])) / 100);
        require(msg.value >= cost, string(abi.encodePacked("payment not sufficient, this affiliate code is only ", Strings.toString(affiliateDiscount[affiliate_code]), "%")));

        uint256 tokenId =  write.mint_NFT{value: write.get_minting_cost()}(str);
        write.transferFrom(address(this), addr, tokenId);

        payable(affiliateAddress[affiliate_code]).transfer(cost * affiliateCommission[affiliate_code] / 100);
        if (msg.value > cost) {
            payable(msg.sender).transfer(msg.value - cost);
        }
    ...
    function mint_affiliate_NFT(string memory str, string memory affiliate_code) public payable returns (uint256) {
