...
contract DDFAExecuter {
    ...
    function swapExactInputSingle(address _tokenIn, address _tokenOut, uint24 _fee, uint _amountIn)
        private
    ...
    function _tradeOnSushi(uint256 amountIn, uint256 amountOutMin, uint256 deadline, address _tokenIn, address _tokenOut) private {
        TransferHelper.safeApprove(_tokenIn, address(sushiRouter), amountIn);
    ...
    function _flashLoanCallBack(address sender, uint256, uint256, bytes calldata data) internal {
        ...
        uint256 amountOut = swapExactInputSingle(loanToken, tokenOut, poolFee, uint(loanAmount));
