...
contract ORL_StakingEngine is Ownable {
    ...
    uint8[25] memory raceCounts; 
        ...
        do{
            ...
            uint16 _slot = slotByRacer[currentWeek][_con][_temp.id];

            if (_slot != 0)  revert alreadyRegistered();
            
                ++_funchecks.tempstaked;
                _slot = _funchecks.tempstaked;
                slotByRacer[currentWeek][_con][_temp.id] = _slot;
     ...
     function returnRacers() external view returns(performance[] memory){
        performance[] memory _racers = new performance[](currentlyStaked[currentWeek]);

        for (uint16 i; i < currentlyStaked[currentWeek];){
  ...
  /*   function returnTokens(uint16[] memory _tokenID) public {
 
        uint16[] memory _racers = _tokenID;
        for( uint i = 0; i < _racers.length; i ++ ) {
            uint16 _tok = _racers[i];
            performance memory staking = perfByRacer[currentWeek][_tok];
            ...
            perfByRacer[currentWeek][_tok].eventWanted = 100;
        }
    }*/
