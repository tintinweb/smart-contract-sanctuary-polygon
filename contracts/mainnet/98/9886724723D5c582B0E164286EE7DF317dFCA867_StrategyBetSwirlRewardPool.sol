...
import "../../interfaces/betswirl/IRewardPool.sol";
import "../Common/StratFeeManager.sol";
import "../../utils/GasFeeThrottler.sol";

contract StrategyBetSwirlRewardPool is StratFeeManager, GasFeeThrottler {
    ...
    function chargeFees(address callFeeRecipient) internal {
        ...
        uint256 forFees = IERC20(output).balanceOf(address(this)) * fees.total / DIVISOR;

        uint256 callFeeAmount = forFees * fees.call / DIVISOR;
        IERC20(output).safeTransfer(callFeeRecipient, callFeeAmount);

        uint256 beefyFeeAmount = forFees * fees.beefy / DIVISOR;
        IERC20(output).safeTransfer(beefyFeeRecipient, beefyFeeAmount);

        uint256 strategistFeeAmount = forFees * fees.strategist / DIVISOR;
        IERC20(output).safeTransfer(strategist, strategistFeeAmount);
    ...
    function rewardsAvailable() public view returns (uint256) {
        return IRewardPool(rewardPool).earned(address(this), output);
    }
