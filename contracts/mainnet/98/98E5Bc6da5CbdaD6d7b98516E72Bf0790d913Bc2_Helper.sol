...
contract PoptionDeployer {
    function deploy(
        ...
        address _oracle,
        uint256 _settleTime,
        uint128[STRIKE_NUM] memory strikes_
    ) external returns (Poption) {
        return new Poption(_token, _oracle, _settleTime, strikes_);
    }
}
...
contract Helper {
    ...
    function deploy(
        ...
        uint128[STRIKE_NUM] calldata strikes,
        uint128[3] calldata swapArgs,
        bool isCash,
        uint128 amount,
        uint128[STRIKE_NUM] memory poolInit
    ) external {
