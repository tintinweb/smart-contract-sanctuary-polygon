...
contract Market is IMarket, Ownable, NFTAssist, ReentrancyGuard {
    ...
    {
        require(offset < _offers.length, 'Not existed offerId');

        Offer[] memory offers_ = new Offer[](limit);
        for (uint256 i = 0; i < limit; i++) {
            if (i + offset >= _offers.length) break;
            offers_[i] = _offers[i + offset];
        }
        return offers_;
    }
...
interface IMarket is INFTAssist {
    ...
    /// @param offset start index
    /// @param limit length of array
    function offers(uint256 offset, uint256 limit) external view returns (Offer[] memory);
