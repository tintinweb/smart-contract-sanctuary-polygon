...
contract Buk is ERC1155, IERC1155Receiver, Ownable, ReentrancyGuard{
    ...
    constructor(string memory NAME, address _treasury, address _signer, address _currency, address bot ) 
    ERC1155(NAME){
       treasury = _treasury;
       signer = _signer;
       checkOutBot = bot; 
    ...
    function withdrawTokens(address token, address wallet, uint256 amount) external onlyOwner{
    ...
    function updateManagerWhitelist(address manager, bool isWhitelisted) external onlyOwner{
    ...
    function updateUri(uint256 tokenId, string memory tokenUri) external{
        ...
        idToHotel[bookingDetails[tokenId].hotelId].hotelManager,"Access Denied");
    ...
    function bookRoom(Booking[] memory booking,address user, bytes memory signature, string memory nonce) external {                 
        require(!usedNonce[nonce], "Nonce used");
        require(
        matchSigner(
            hashTransaction(user, nonce),
            signature
        ),
        "Not allowed to lock"
        );
        ...
        for(uint256 i=0; i< totalNfts; i++){
           ...
           // uint256 bukFee  = (booking[i].base_price * _bukCommission / 10000);
            uint256 bukActualTotal = ((booking[i]._baseprice * platformFee / 10000))- booking[i].discount;
            uint256 HotelAmount = TotalAmount - bukActualTotal;
            IERC20(currency).transferFrom(msg.sender, treasury, bukActualTotal);
            IERC20(currency).transferFrom(msg.sender, address(this), HotelAmount);
    ...
    function cancelBookingUser(uint256 id, address user, string memory  nonce, bytes memory signature) external {
        ...
        uint256 bukComm = ((bookingDetails[id]._baseprice * platformFee / 10000))- bookingDetails[id].discount; // bukFee and discounts deducted from Total paid amount 
        ...
        if( owner() == msg.sender || msg.sender == idToHotel[bookingDetails[id].hotelId].hotelManager){
            ...
            address HotelTreasury = idToHotel[bookingDetails[id].hotelId].hotelTreasury;    
    ...
    function preCheckIn(uint256 id, string memory  nonce, bytes memory signature) external {
        require(balanceOf(msg.sender,id) > 0 || owner() == msg.sender,"Access Denied,Only Nft Owner or Contract owner" );  
        require(!usedNonce[nonce], "Nonce used");
        require(
        matchSigner(
    ...
    function checkOut(uint256 id, address user) external {
        require(owner() == msg.sender || msg.sender == checkOutBot,
        "Access Denied" );  
        require(block.timestamp > bookingDetails[id].time + 79200 + checkOutTime,"CheckOut window is not Opened");  
        uint256 itemId = BukMarket(marketplaceContract).getTokenToItem(id);
        if(itemId != 0){
            BukMarket(marketplaceContract).endSale(itemId);
        }
        ...
        uint256 bukComm = ((bookingDetails[id]._baseprice * platformFee / 10000))- bookingDetails[id].discount; // bukFee and discounts deducted from Total paid amount 
        uint256 ActualAmount = TotalPaidAmount - bukComm;// Actual Amount went to Hotel that user paid 
        IERC20(currency).transfer(idToHotel[bookingDetails[id].hotelId].hotelTreasury, 
        ActualAmount);
        checkedOut[id] = true;
        transferBlocked[id] = true;
    ...
    function getNFTsToCheckout() external view returns(uint256[] memory ids){
        uint totalNfts = tokenCount.current();
        uint itemCount = 0;
        uint currentIndex = 0;

        for (uint i = 0; i < totalNfts; i++) {
            if ( nftCheckedOut[i] == false &&
   ...
   function safeTransferFrom(
        ...
        require(block.timestamp < bookingDetails[id].time - checkInTime,"Transfer has been blocked");
    ...
    function safeBatchTransferFrom(
        ...
        for(uint256 i =0; i<total;i++){
            require(block.timestamp < bookingDetails[ids[i]].time - checkInTime,"Transfer has been blocked");
