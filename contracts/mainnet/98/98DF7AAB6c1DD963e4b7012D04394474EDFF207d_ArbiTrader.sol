...
contract ArbiTrader is IFlashLoanSimpleReceiver, OwnableUpgradeable, AbstractTrader{
    ...
    function executeOperation(
        ...
        console.log('calling execute ops...');
    ...
    function _executeOperation(Strategy memory _strategy) internal {
        uint256 previousAmountOut;

        for(uint8 i = 0; i < _strategy._strategyLength; i++){

            PoolOperation memory _operation = _decodeOperation(_strategy._ops[i]);

            address poolAddress = _operation._pool;
            address tokenTransfer;
            uint256 transferAmount;
            uint256 currentAmountOut = _operation._amount0Out == 0 ? _operation._amount1Out : _operation._amount0Out;
            require(_strategy._loanAsset == _operation._token, "Loan Asset != Operation token");

            if(i == 0){
                transferAmount = _strategy._loanAmount;
                previousAmountOut = currentAmountOut;
            }else{
                transferAmount = previousAmountOut;
            }

            require( IERC20(_operation._token).transfer( poolAddress , transferAmount),  "Transfer to pool failed!" );
            IUniswapV2Pair(poolAddress).swap(_operation._amount0Out, _operation._amount1Out, address(this), "0x");
            // uint256 usdtBalance = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F).balanceOf(address(this));
            // console.log('swap result === ', usdtBalance);
            // Update currentamountIn for next swap as is latest amountOut
            previousAmountOut = currentAmountOut;
        }
    }
    ...
    */
    ...
    function _decodeOperation(bytes memory operation) internal returns(PoolOperation memory _operation){
       (
        _operation._pool,
        _operation._token,
        _operation._amount0Out,
        _operation._amount1Out) = abi.decode(operation,(address, address, uint256,uint256));
    }
