...
import "contracts/utils/TransparentUpgradeableProxy.sol";
import "contracts/utils/ImplementationStorage.sol";
...
/// @title PartyVotingWindow Fabric
contract PartyVotingWindowFabric is
    Access,
    ImplementationStorage
{
    ...
    /// @param implementationAddress implementation of the partyVotingWindow address
    ...
    constructor(
        ...
        __ImplementationStorage_init_unchained(implementationAddress);
    }

    function _deployProxy(address upgradeAdmin) internal returns(address) {
        bytes memory _data = new bytes(0);
        return address(new TransparentUpgradeableProxy(implementation, upgradeAdmin, _data));
    }
    ...
    function _init(
        ...
        PartyVotingWindow(result).initialize({
            ownerValue: instanceOwner
        });
    }

    /// Create new contract instance
    /// @param upgradeAdmin upgradeAdmin to set on instance
    /// @param instanceOwner instanceOwner to set on instance
    function create(
        address upgradeAdmin,
        address instanceOwner
    ) public returns(address result) {
        result = _deployProxy(upgradeAdmin);
        _init(
            result,
            instanceOwner
        );
...
abstract contract ImplementationStorage is Access {
    using Utils for address;
    address public implementation;
    event ImplementationSet(address indexed implementation);

    function __ImplementationStorage_init_unchained(address implementationAddress) internal {
        implementation = implementationAddress.ensureNotZero();
    }

    /// @notice Set new "implementation" setting value (only contract owner may call)
    /// @param implementationAddress new setting value
    function setImplementation(address implementationAddress) public onlyOwner {
        implementation = implementationAddress.ensureNotZero();
        emit ImplementationSet(implementationAddress);
    }
}
