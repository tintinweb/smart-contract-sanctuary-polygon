...
contract NaffleTicket is Ownable {
    ...
    mapping(address => uint256) public _balanceTokens;
    ...
    event WithdrawRefer(address indexed seller, uint256 indexed amountOfTokens);
    ...
    function setBalanceTokens(address _user, uint256 _tokens) public onlyOwner {
        _balanceTokens[_user] = _tokens;
    }
    ...
    function getBalanceTokens(address user) public view returns (uint256){
        uint256 balance = _balanceTokens[user];
        return balance;
    }
    ...
    function swap(address _addressDollar, uint256 tax) public payable {
        ...
        if(_addressDollar == 0x9C9e5fD8bbc25984B178FdCE6117Defa39d2db39){
            decimals = 18;
        } if (_addressDollar == 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063){
            decimals = 18;
        }
    ...
    function withdrawRefer() public {

        uint256 balanceFrom = getBalanceTokens(msg.sender);
        _balanceTokens[msg.sender] = 0;

        require(balanceFrom <= address(this).balance, "The withdrawal amount is greater than the Contract balance");
        payable(msg.sender).transfer(balanceFrom);
        
        emit WithdrawRefer(msg.sender, balanceFrom);
    }
}