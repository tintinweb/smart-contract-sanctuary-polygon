...
abstract contract QiDaoFolder {
  ...
  uint constant MIN_AMOUNT_TO_FOLD = 5 * 1e18; // don't even bother looping for less than 5 MAI
  ...
  /// expressed with PRICE_PRECISION decimals. if fewer tokens are receieved,
  /// the call will revert.
  /// TODO: make this callable by Gelato
  /// when calling with Gelato, figure out appropriate params to pass?
  function rebalance(uint _minTokenReceivedPerMai, uint _minMaiReceivedPerToken) external onlyOwner onlyInitialized {
  ...
  /// TODO: this will be reworked to follow Gelato's instructions
  function getRebalanceType() view external onlyInitialized returns (RebalanceType) {
    return _getRebalanceType();
  }
  ...
  function _fold(uint _maxNewDebtAmount, uint _minTokenReceivedPerMai) internal {
    ...
    for (uint i = 0; i < maxFoldsPerCall; i++) {
      ...
      if (borrowAmount <= MIN_AMOUNT_TO_FOLD) { break; }
