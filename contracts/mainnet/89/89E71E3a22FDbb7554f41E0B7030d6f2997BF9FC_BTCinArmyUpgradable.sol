    ...
    UUPSUpgradeable
{
    ...
    struct Account {
        ...
        uint256[] globalRewardPaid;
        address[] globalRewardPaidBy;
        uint256[] globalRewardPaidTimestamp;
    }
    ...
    function initialize() external initializer {
        ...
        //Polygon Network
        tokenContract = 0x32F81F5fa147027F990b6a35e14b41b5990fCE1a;
    ...
    function getUserTotalTeamBusiness(
        address _address
    ) external view returns (uint256 userTotalTeamBusiness) {
        Account storage userAccount = accounts[_address];
        uint256 userRefereeLength = userAccount.referee.length;

        for (uint256 i; i < userRefereeLength; i++) {
            Account storage userRefereeAccount = accounts[
                userAccount.referee[i]
            ];
            userTotalTeamBusiness += userRefereeAccount.totalBusiness;
        }
    }
    ...
    function getUserGlobalRewardPaid(
        address _address
    ) external view returns (uint256 userGlobalRewardPaid) {
        Account storage userAccount = accounts[_address];
        uint256 length = userAccount.globalRewardPaid.length;
        for (uint256 i; i < length; i++) {
            userGlobalRewardPaid += userAccount.globalRewardPaid[i];
        }
    }
    ...
    function getUserGlobalIdCount(
        ...
        Account storage userAccount = accounts[_address];
        globalIdCount = userAccount.globalId.length;
    }
    ...
    function _payReward(uint256 _value, address _user) private {
        ...
        bool payGlobalReward = userReferrerAccount.payGlobalReward;
        ...
        if (payGlobalReward) {
            ...
            globalAddressAccount.globalRewardPaid.push(rewardValue);
            globalAddressAccount.globalRewardPaidBy.push(_user);
            globalAddressAccount.globalRewardPaidTimestamp.push(
                block.timestamp
            );
        ...
        userReferrerAccount.payGlobalReward = !payGlobalReward;
