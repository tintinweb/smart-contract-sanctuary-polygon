    ...
    ReentrancyGuardUpgradeable
{
    ...
    mapping(uint256 => address) public _lockProviders;

    error NFT_ZeroAddress();
    error NFT_Locked(uint256 tokenId);

    event NFTMinted(address indexed to, uint256 tokenId);
    event NFTLocked(uint256 tokenId, address indexed lockProvider);
    event NFTUnlocked(uint256 tokenId);

    modifier onlyNotLocked(uint256 firstTokenId_, uint256 batchSize_) {
        for (uint256 i = firstTokenId_; i < firstTokenId_ + batchSize_; ) {
            if (_lockProviders[i] != address(0)) {
                revert NFT_Locked(i);
            }
    ...
    function initialize(
        ...
        if (amberfiStorageAddress_ == address(0)) {
    ...
    function _beforeTokenTransfer(
        ...
        onlyNotLocked(firstTokenId, batchSize)
    {
    ...
    function lock(uint256 tokenId_, address lockProvider_)
        external
        nonReentrant
    {
        if (lockProvider_ == address(0)) {
            revert NFT_ZeroAddress();
        }

        if (msg.sender == ownerOf(tokenId_)) {
            _lockProviders[tokenId_] = lockProvider_;
        }

        emit NFTLocked(tokenId_, lockProvider_);
    }

    function unlock(uint256 tokenId_) external nonReentrant {
        if (msg.sender == ownerOf(tokenId_)) {
            delete _lockProviders[tokenId_];
        }

        emit NFTUnlocked(tokenId_);
    }

    function setLockProvider(uint256 tokenId_, address newLockProvider_)
        external
        nonReentrant
    {
        if (msg.sender == ownerOf(tokenId_)) {
            _lockProviders[tokenId_] = newLockProvider_;
        }
    }

    function revokeLockProvider(uint256 tokenId_) external nonReentrant {
        if (msg.sender == ownerOf(tokenId_)) {
            delete _lockProviders[tokenId_];
        }
    }
