...
contract ImmutableCreate2Factory {
   ...
   */
    function safeCreate2AndTransferOwner(
        bytes32 salt,
        ...
        if(isProxy){
            bytes memory payload2 = abi.encodeWithSignature("transferProxyOwnership(address)",msg.sender);
            deploymentAddress.call(payload2);
        }else{
            bytes memory payload = abi.encodeWithSignature("transferOwnership(address)",msg.sender);
            deploymentAddress.call(payload);
        }
