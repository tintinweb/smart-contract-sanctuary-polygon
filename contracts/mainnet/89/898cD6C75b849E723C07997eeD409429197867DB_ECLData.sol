...
contract ECLData is Initializable, UUPSUpgradeable, OwnableUpgradeable{
    // Trait structure
    struct Trait{
        string traitName;
        string traitType;
    ...
    */
        ...
        returns (string memory)
    {
        ...
        if(danceoffAddress != nullAddress){
            ...
            IDanceOffLegacy.Winner[] memory Winners = IDanceOffLegacy(danceoffAddress).getPlacementsByToken(_tokenId);
            entered = IDanceOffLegacy(danceoffAddress).getRumblesEntered(_tokenId).length;
        ...
        returns (string memory)
    {
        ...
        // external contract call for boss loot and graphics as well as any other background graphics
        if(xxlAddress != nullAddress){
            IMikeExtended.Traits[] memory XxlTraits = IMikeExtended(xxlAddress).getAllPlayerLoot(_tokenId);
        ...
        svgString = string(
            abi.encodePacked(
                '<svg id="ecl-svg" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMinYMin meet" viewBox="0 0 24 24"> ',
                svgString,
