...
contract CustomerRouter is IRouter, Ownable {
  ...
  function changeManagerRouter(address newManagerRouter) external {
    IManagerRouter mr = IManagerRouter(newManagerRouter);
    require(mr.isManagerOwner(_msgSender()), "Only ManagerRouter owner can change manager address!");
    _managerRouter = IManagerRouter(newManagerRouter);
  }
...
// SPDX-License-Identifier: ENERGY8
interface IManagerRouter {
  ...
  function isManagerOwner(address adr) external returns (bool);
}
