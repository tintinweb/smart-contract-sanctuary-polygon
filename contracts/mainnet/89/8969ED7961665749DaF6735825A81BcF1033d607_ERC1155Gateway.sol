...
import {IMyERC1155} from "./interfaces/IMyERC1155.sol";
...
import {IERC721LockedERC1155} from "./interfaces/IERC721LockedERC1155.sol";
import {IERC1155Gateway} from "./interfaces/IERC1155Gateway.sol";
import {EmergencyTokenRecoveryUpgradeable} from "../protocol/EmergencyTokenRecoveryUpgradeable.sol";

import {IERC1155ReceiverUpgradeable} from "@openzeppelin/contracts-upgradeable/token/ERC1155/IERC1155ReceiverUpgradeable.sol";


contract ERC1155Gateway is 
  Initializable,
  ...
  EmergencyTokenRecoveryUpgradeable
{
  ...
  // IERC721LockedERC1155 public ERC721LockedERC1155;

  // address internal _ERC721LockedERC1155ImplAddress;

  /**
   * @notice erc1155NftContract => wrapperContract
   * @notice Each erc1155 nft contract has its own corresponding wrapper
   * @dev Multiple erc1155 nft contracts may share a single nft wrapper
   * @dev Each nft wrapper represents a lendingpool liquidity strategy (baseLTVAsCollateral , liquidationThreshold , liquidationBonus ...)
   * @dev The purpose of the wrapper is to make erc1155 nft look like erc721 for use within the protocol
   */
  mapping(address => IERC721LockedERC1155) public nftWrappers;

  address[] public allNftWrappers;
  ...
  function __ERC1155Gateway_init(
    address addressProvider,
    address wethGateway
    // address ERC721LockedERC1155Impl
    // address _MyERC1155,
    // address _ERC721LockedERC1155
  ) public initializer {
    __ERC721Holder_init();
    ...
    // _ERC721LockedERC1155ImplAddress = ERC721LockedERC1155Impl;

    // MyERC1155 = IMyERC1155(_MyERC1155);
    // ERC721LockedERC1155.registerProxy();
    // proxy = ERC721LockedERC1155.proxyInfo(address(this));
    
    // ERC721LockedERC1155 = IERC721LockedERC1155(_ERC721LockedERC1155);

    // IERC721Upgradeable(address(ERC721LockedERC1155)).setApprovalForAll(address(_getLendPool()), true);
    // IERC721Upgradeable(address(ERC721LockedERC1155)).setApprovalForAll(address(_wethGateway), true);
  }
  ...
  //   return INFTOracle(_addressProvider.getNFTOracle());
  // }
  ...
  //   return IBNFTRegistry(_addressProvider.getBNFTRegistry());
  // }
  ...
  // struct LendingPoolConfigParams{
  //   uint ltv;
  ...
  //   uint redeemFine;
  //   uint nftRedeemThreshold;
  // }

  // /**
  //  * @notice This function is the factory to create ERC721LockedERC1155
  //  * @param name wrap nft name (ex. Wrapped Landlord)
  //  * @param symbol wrap nft symbol (ex. WLandlord)
  //  */
  // function createNftWrappers(
  //   string memory name,
  //   string memory symbol
  //   // uint oracleType,
  //   // LendingPoolConfigParams calldata lendingPoolConfigParams
  // ) external nonReentrant onlyOwner{
  //   // deploy wrapper
  //   address nftWrapperProxy = address(
  //       new ERC1967Proxy(
  //         _ERC721LockedERC1155ImplAddress,
  //         abi.encodeWithSelector(
  //             bytes4(keccak256(bytes("__ERC721LockedERC1155_init(string,string)"))),
  //             name, 
  //             symbol
  //         )
  //       )
  //     );
  //   // IERC721LockedERC1155(nftWrapperProxy).__ERC721LockedERC1155_init(name , symbol);
  //   IERC721LockedERC1155(nftWrapperProxy).transferOwnership(owner());

  //   allNftWrappers.push(nftWrapperProxy);
  // }

  /**
   * @notice Register nftWrapper to the gateway
   */
  function registerNftWrappers(
    address nftWrapper
  ) external nonReentrant onlyOwner {
    require(_requireAddressIsNftWrapper(nftWrapper) , "nftWrapper invalid");
    IERC721Upgradeable(nftWrapper).setApprovalForAll(address(_getLendPool()) , true);
    IERC721Upgradeable(address(nftWrapper)).setApprovalForAll(address(_wethGateway), true);

    allNftWrappers.push(nftWrapper);
  }

  /**
   * @notice register erc1155 contract address to the wrapper
   * @notice a wrapper can wrap several erc721 contracts
   */
  function registerErc1155ContractForWrapper (
    address nftWrapper,
    address[] calldata erc1155NftAddresses
  ) external nonReentrant onlyOwner {
    require(_requireAddressIsNftWrapper(nftWrapper) , "nftWrapper invalid");

    for(uint i = 0; i<erc1155NftAddresses.length; i++){
      address erc1155NftAddress = erc1155NftAddresses[i];
      require(_requireAddressIsErc1155(erc1155NftAddress) , "erc1155NftAddress invalid");

      nftWrappers[erc1155NftAddress] = IERC721LockedERC1155(nftWrapper);
      IERC721LockedERC1155(nftWrapper).registerErc1155Contract(erc1155NftAddress);
      IMyERC1155(erc1155NftAddress).setApprovalForAll(nftWrapper , true);
    }

  }

  function _requireAddressIsErc1155(address erc1155NftAddress) internal view returns(bool){
    // TODO
    return true;
  }

  function _requireAddressIsNftWrapper(address nftWrapper) internal view returns(bool){
    // TODO
    return true;
  }

  /**
   * @notice To get the contract address and tokenId of the wrapper nft
   * @param _ERC1155Token The underlying erc1155 nft info
   */
  function getWrapErc721TokenId(ERC1155Token calldata _ERC1155Token) public view override returns(address wrapperNftAddress, uint wrapTokenId){
    IERC721LockedERC1155 nftWrapperInterface = nftWrappers[address(_ERC1155Token.erc1155Contract)];
    wrapperNftAddress = address(nftWrapperInterface);
    wrapTokenId = nftWrapperInterface.getErc721TokenId(_ERC1155Token);
  }
  

  /**
   * @notice This function will wrap erc1155 nft to erc721 nft and deposit it to the pool
   * @return After the deposit user can call getWrapErc721TokenId function to get the wrap erc721 new tokenId
   */
  function _deposit1155(ERC1155Token calldata _ERC1155Token) internal returns (bool) {
    ILendPoolLoan cachedPoolLoan = _getLendPoolLoan();

    uint256 loanId = cachedPoolLoan.getCollateralLoanId(address(_ERC1155Token.erc1155Contract) , _ERC1155Token.erc1155TokenId);
    if (loanId != 0) {
      return false;
    }

    // In erc1155 contract, we use balanceOf instead of ownerOf to check if the user own the nft
    uint balance = IMyERC1155(_ERC1155Token.erc1155Contract).balanceOf(_msgSender() , _ERC1155Token.erc1155TokenId);
    require(balance >= 1 , "not owner of tokenId");
    // require(owner == _msgSender(), "ERC1155Gateway: not owner of _ERC1155Token.erc1155TokenId");

    // This erc1155 nft must be registered in eRC721LockedERC1155 (registerErc1155Contract)
    _ERC1155Token.erc1155Contract.safeTransferFrom(
      _msgSender(), 
      // address(nftWrappers[address(_ERC1155Token.erc1155Contract)]),
      address(this),
      _ERC1155Token.erc1155TokenId, 
      1,
      ""
    );

    nftWrappers[address(_ERC1155Token.erc1155Contract)].depositERC1155(
        _ERC1155Token, 
        address(this) ,
        ""
      );
  ...
  function borrow(
    ...
    _deposit1155(_ERC1155Token);

    (address wrapperNftAddress, uint wrapTokenId) = getWrapErc721TokenId(_ERC1155Token);

    cachedPool.borrow(reserveAsset, amount, wrapperNftAddress ,wrapTokenId, onBehalfOf, referralCode);
  ...
  function batchBorrow(
    ...
    ERC1155Token[] calldata _ERC1155Tokens,
    address onBehalfOf,
    uint16 referralCode
  ) external override nonReentrant {
    require(_ERC1155Tokens.length == reserveAssets.length, "inconsistent reserveAssets length");
    require(_ERC1155Tokens.length == amounts.length, "inconsistent amounts length");

    _checkValidCallerAndOnBehalfOf(onBehalfOf);

    ILendPool cachedPool = _getLendPool();

    for (uint256 i = 0; i < _ERC1155Tokens.length; i++) {
      _deposit1155(_ERC1155Tokens[i]);

      (address wrapperNftAddress, uint wrapTokenId) = getWrapErc721TokenId(_ERC1155Tokens[i]);

      cachedPool.borrow(reserveAssets[i], amounts[i], wrapperNftAddress, wrapTokenId, onBehalfOf, referralCode);
  ...
  function _withdraw1155(
    ...
    (address wrapperNftAddress, uint wrapTokenId) = getWrapErc721TokenId(_ERC1155Token);
    IERC721LockedERC1155 nftWrapperInterface = IERC721LockedERC1155(wrapperNftAddress);

    address owner = nftWrapperInterface.ownerOf(wrapTokenId);
    require(owner == _msgSender(), "ERC1155Gateway: caller is not owner");
    require(owner == onBehalfOf, "ERC1155Gateway: onBehalfOf is not owner");

    nftWrapperInterface.returnToERC1155(ERC1155Token(_ERC1155Token.erc1155Contract , _ERC1155Token.erc1155TokenId) , address(this) , "");

    IMyERC1155(address(_ERC1155Token.erc1155Contract)).safeTransferFrom(address(this) , onBehalfOf , _ERC1155Token.erc1155TokenId , 1, "");
  }
  ...
  function repay(
    ...
    return _repay(_ERC1155Token, amount);
  }
    ...
    returns (uint256[] memory, bool[] memory)
  {
    ...
    uint256[] memory repayAmounts = new uint256[](_ERC1155Tokens.length);
    bool[] memory repayAlls = new bool[](_ERC1155Tokens.length);

    for (uint256 i = 0; i < _ERC1155Tokens.length; i++) {
      (repayAmounts[i], repayAlls[i]) = _repay(_ERC1155Tokens[i], amounts[i]);
    }
  ...
  function _repay(
    ...
    ILendPoolLoan cachedPoolLoan = _getLendPoolLoan();
    (address wrapperNftAddress, uint wrapTokenId) = getWrapErc721TokenId(_ERC1155Token);
    uint256 loanId = cachedPoolLoan.getCollateralLoanId(wrapperNftAddress, wrapTokenId);
    address borrower = cachedPoolLoan.borrowerOf(loanId);

  {

    require(loanId != 0, "ERC1155Gateway: no loan with such _ERC1155Token.erc1155TokenId");
  ...
  {
    (uint256 paybackAmount, bool burn) = cachedPool.repay(wrapperNftAddress, wrapTokenId, amount);

    if (burn) {
      require(borrower == _msgSender(), "ERC1155Gateway: caller is not borrower");
      _withdraw1155(_ERC1155Token, borrower);
    }
  ...
  function auction(
    ...
    cachedPool.auction(wrapperNftAddress, wrapTokenId, bidPrice, onBehalfOf);
  }
  ...
  function redeem(
    ...
    uint256 paybackAmount = cachedPool.redeem(wrapperNftAddress, wrapTokenId, amount);
  ...
  function liquidate(
    ...
    DataTypes.LoanData memory loan = cachedPoolLoan.getLoan(loanId);
    require(loan.bidderAddress == _msgSender(), "ERC1155Gateway: caller is not bidder");

    if (amount > 0) {
      IERC20Upgradeable(loan.reserveAsset).transferFrom(msg.sender, address(this), amount);
    }

    uint256 extraRetAmount = cachedPool.liquidate(wrapperNftAddress, wrapTokenId, amount);

    _withdraw1155(_ERC1155Token, loan.bidderAddress);
  ...
  function borrowETH(
    ...
    _wethGateway.borrowETH(amount, wrapperNftAddress, wrapTokenId, onBehalfOf, referralCode);
  }
  ...
  function batchBorrowETH(
    ...
    address[] memory nftAssets = new address[](_ERC1155Tokens.length);
    uint[] memory nftTokenIds = new uint[](_ERC1155Tokens.length);
    for (uint256 i = 0; i < _ERC1155Tokens.length; i++) {
      _deposit1155(_ERC1155Tokens[i]);
      (nftAssets[i], nftTokenIds[i]) = getWrapErc721TokenId(_ERC1155Tokens[i]);
    }

    _wethGateway.batchBorrowETH(amounts, nftAssets, nftTokenIds, onBehalfOf, referralCode);
  }
  ...
  function repayETH(
    ...
    (uint256 paybackAmount, bool burn) = _repayETH(_ERC1155Token, amount, 0);
    ...
    returns (uint256[] memory, bool[] memory)
  {
    ...
    for (uint256 i = 0; i < _ERC1155Tokens.length; i++) {
      (repayAmounts[i], repayAlls[i]) = _repayETH(_ERC1155Tokens[i], amounts[i], allRepayAmount);
  ...
  function _repayETH(
    ...
    (, vars.repayDebtAmount) = cachedPoolLoan.getLoanReserveBorrowAmount(loanId);
    if (amount < vars.repayDebtAmount) {
      vars.repayDebtAmount = amount;
    }

    require(msg.value >= (accAmount + vars.repayDebtAmount), "msg.value is less than repay amount");

      
    vars.borrower = cachedPoolLoan.borrowerOf(loanId);
    require(vars.borrower == _msgSender(), "ERC1155Gateway: caller is not borrower");

    {
      (vars.paybackAmount, vars.burn) = _wethGateway.repayETH{value: vars.repayDebtAmount}(
        wrapperNftAddress,
        wrapTokenId,
        amount
      );
      ...
      if (vars.burn) {
        _withdraw1155(_ERC1155Token, vars.borrower);
      }
      
      return (vars.paybackAmount, vars.burn);
    }
  ...
  function auctionETH(
    ...
    _wethGateway.auctionETH{value: msg.value}(wrapperNftAddress, wrapTokenId, onBehalfOf);
  }
  ...
  function redeemETH(
    ...
    uint256 paybackAmount = _wethGateway.redeemETH{value: msg.value}(wrapperNftAddress, wrapTokenId, amount);
  ...
  function liquidateETH(
    ...
    uint256 extraAmount = _wethGateway.liquidateETH{value: msg.value}(wrapperNftAddress, wrapTokenId);
...
import "./IERC721Views.sol";
import "./IERC1155Token.sol";

interface IERC721LockedERC1155 is IERC721Views , IERC1155Token {
...
interface IERC1155Gateway is IERC1155Token{

    function getWrapErc721TokenId(
        ERC1155Token calldata _ERC1155Token
    ) external view returns(address wrapperNftAddress, uint wrapTokenId);
