...
interface INFTStake {
    ...
    function initialize(address _defaultAdmin, string memory _contractURI, address[] memory _trustedForwarders, address _rewardToken, address _stakingToken, uint256 _timeUnit, uint256 _rewardsPerUnitTime) external;
...
interface ITokenStake {
    ...
    function depositRewardTokens(uint256 _amount) external payable;
    function withdrawRewardTokens(uint256 _amount) external;
    function initialize(address _defaultAdmin, string memory _contractURI, address[] memory _trustedForwarders, address _rewardToken, address _stakingToken, uint256 _timeUnit, uint256 _rewardRatioNumerator, uint256 _rewardRatioDenominator) external;
    function grantRole(bytes32 role, address account) external;
...
contract BlackErc20 is ERC20, Ownable {
    ...
    constructor(
        ...
        // 代币分配情况：
        // factoryContract: 15.01% (含stakePool 15%)
        // fundAddress:     0.05%
        // deplyAddress:    0.05%
        // this:            84.89%
        _maxPro = 10000 - (1 + 5 + 1505);
        _mint(factoryContract, totalSupply * 1501 / 10000);
        _mint(fundAddress, totalSupply * 5 / 10000);
        _mint(deplyAddress, totalSupply * 5 / 10000);
        _mint(address(this), totalSupply * _maxPro / 10000);
    }
    ...
    function mint(uint256 mintCount, address receiveAds) external payable {
        ...
        IWETH(wmaticAddress).deposit{value: msg.value}();
        IWETH(wmaticAddress).approve(lpContract, msg.value);
        IWETH(wmaticAddress).transferFrom(address(this), lpContract, msg.value);
        
        uint256 mintAmount = (totalSupply() * (_maxPro - 1500) * mintCount) / (_maxMintCount * 10000);
    ...
    function setSPContract(address sp) external onlyOwner {
        require(spContract == address(0), "SP contract already set");
        spContract = sp;
    }

    function fundAward() external {
        uint256 balance = address(this).balance;
        require(balance > 0, "Contract has no ETH balance.");
        address payable sender = payable(_fundAddress);
