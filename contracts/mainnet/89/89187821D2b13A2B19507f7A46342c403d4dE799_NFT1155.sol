...
contract NFT1155 is ERC1155Upgradeable, AccessControlUpgradeable {
    ...
    function safeTransferFrom(
        ...
        uint256 amount,
        bytes memory data
    ) public override onlyTransferRole {
    ...
    modifier onlyTransferRole() {
        require(hasRole(TRANSFER_ROLE, msg.sender), "no transfer");
        _;
    }

    function GrantTransferRole(address account) external onlyAdmin {
        grantRole(TRANSFER_ROLE, account);
    }

    function RevokeTransferOperator(address account) external onlyAdmin {
