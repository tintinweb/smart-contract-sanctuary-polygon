...
library FlipFlopLib {
    ...
    function _computeUserValues(
        ...
        (uint256 locked1, uint256 locked2) = calcLockedAmount(
            user,
            assetDetails.minDepositOfAsset1,
            assetDetails.minDepositOfAsset2
        );
        uint256 e = user.amountOfAsset1 - locked1;
        uint256 u = user.amountOfAsset2 - locked2;
        ...
        if (temp.asset == assetDetails.asset1) {
            ...
            if (Compute.wadMul(e, temp.S) > u) {
                ...
                user.amountOfAsset2 = u + (((e - UbyS).wadMul(stack)) >> 1);
            } else {
                uint256 uDivS;
                uint256 eMulS;
                ...
                // e1=e+0.5*(u/S-e)
                user.amountOfAsset1 = e + ((uDivS - e) >> 1);
                // u1=u-0.5*(u-e*S)
                user.amountOfAsset2 = u - ((u - eMulS) >> 1);
            }
        } else {
            temp.A = Compute.scaleDecimals(
                temp.A,
            ...
            if (temp.S.wadMul(e) > u) {
                // e1=e-0.5*(e-u/S)
                user.amountOfAsset1 = e - ((e - u.wadDiv(temp.S)) >> 1);
                // u1=u+0.5*(e*S-u)
                user.amountOfAsset2 = u + ((e.wadMul(temp.S) - u) >> 1);
            } else {
                ...
                user.amountOfAsset1 = e + (((u - EtoU).wadMul(stack)) >> 1);

                uint256 multiplyingFactor = assetDetails.S_U +
                    temp.A_S -
                    temp.A;
                ...
                user.amountOfAsset2 =
                    u -
                    ((
                        ((u - EtoU).wadMul(multiplyingFactor)).wadDiv(
                            assetDetails.S_U
                        )
                    ) >> 1);
            }
...
contract Storage is OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    uint256 internalSwapAsset1; // TODO: remove before redeployment
    uint256 internalSwapAsset2; // TODO: remove before redeployment

    uint256 internal S_E;
    uint256 internal S_U;

    // Amount of asset1 without current deposit, i.e. what was left in the vault from the previous vault
    uint256 internal e_V;
    // Amount of asset2 without current deposit, i.e. what was left in the vault from the previous vault
