...
contract VenusMeCE is ERC721, ERC721Royalty, ERC721Enumerable, Ownable {
    ...
    constructor(string memory baseURI) ERC721("VenusMeCE", "VenusMeCE") {
        setBaseURI(baseURI);
    ...
    // Override so we have unlimited NFTs repeating each 10.000 mints
    /**
        ...
        returns (string memory)
    {
        ...
        return
            bytes(baseURI).length > 0
                ? string(abi.encodePacked(baseURI, Strings.toString(imageId)))
                : "";
    }
    ...
    // everyone able to read a smart contract, can verify that this is the only
    ...
    // ------------------------------------------------------------
    // Unfortunately the Giving Block does not accept native MATIC token,
    // so on Polygon blockchain we cannot handle donations in a trustless
    // manner. We will have to move the donated MATIC to Ethereum blockchain
    // before donating.
    // ------------------------------------------------------------
