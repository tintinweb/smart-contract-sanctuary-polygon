...
contract EvolutionFacet {
    ...
    function beginEvolutionWithSignature(uint256 roundTripId,
        uint256 tokenId,
        uint256 owedRBW,
        uint256 owedUNIM,
        uint256 upgradeBooster,
        ...
        bytes32 hash = beginEvolutionGenerateMessageHash(roundTripId, tokenId, owedRBW, owedUNIM, upgradeBooster, bundleId, blockDeadline);
        ...
        LibEvolution.beginEvolution(roundTripId, tokenId, blockDeadline, upgradeBooster);
    ...
    function retryEvolutionWithSignature(
        ...
        bytes32 hash = retryEvolutionGenerateMessageHash(roundTripId, bundleId, blockDeadline);
        address gameServer = LibDiamond.gameServer();
        require(
            SignatureChecker.isValidSignatureNow(gameServer, hash, signature),
    ...
    function finishEvolutionGenerateMessageHash(
        ...
        uint256 blockDeadline
         ...
         bytes32 structHash = keccak256(
            abi.encode(
                keccak256(
                    "finishEvolutionPayload(uint256 roundTripId, string memory tokenURI, uint256 bundleId, uint256 blockDeadline)"
                ),
                roundTripId,
                tokenURI,
                bundleId,
    ...
    function finishEvolutionWithSignature(
        ...
        bytes32 hash = finishEvolutionGenerateMessageHash(roundTripId, tokenURI, bundleId, blockDeadline);
        address gameServer = LibDiamond.gameServer();
        require(
            SignatureChecker.isValidSignatureNow(gameServer, hash, signature),
        ...
        bytes32 vrfRequestId = LibEvolution.getVRFRequestId(roundTripId);
        uint256 tokenId = LibEvolution.getTokenId(vrfRequestId);
        LibEvolution.finishEvolution(roundTripId, tokenURI, vrfRequestId, tokenId);
    }
