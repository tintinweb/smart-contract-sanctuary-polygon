...
// File: Heavenly Empire/heavenlyempire.sol
...
contract HeavenlyEmpire is ERC721, IERC2981, ReentrancyGuard, Ownable {
    ...
    constructor(string memory _customBaseURI) ERC721("Heavenly Empire", "HE") {
        customBaseURI = _customBaseURI;
    }

    function mint(string memory tokenURI_, uint256 count) public payable nonReentrant onlyOwner {
      ...
      for (uint256 i = 0; i < count; i++) {
        ...
        if (bytes(tokenURI_).length > 0) {
          setTokenURI(totalSupply(), tokenURI_);
        }
      }
    }
    ...
    uint256 public minimumPrice = 1 * 10 ** 18; // minimum price for token sales (1 MATIC)
    bool public isPricingEnabled = false; // boolean variable to enable/disable pricing

    event TokenPriceSet(uint256 indexed tokenId, uint256 price);

    function setTokenPrice(uint256 tokenId, uint256 price) public {
        require(_exists(tokenId), "Token does not exist");
        require(msg.sender == ownerOf(tokenId), "Only token owner can set price");
        require(price >= minimumPrice, "Price must be greater than or equal to minimum price");
        require(isPricingEnabled, "Pricing is disabled");
    ...
    function buyToken(uint256 tokenId) public payable nonReentrant {
        require(_exists(tokenId), "Token does not exist");
        require(tokenPrice[tokenId] > 0, "Token is not for sale");
        require(isPricingEnabled, "Pricing is disabled");
        address payable seller = payable(ownerOf(tokenId));
        _transfer(seller, msg.sender, tokenId);
        seller.transfer(msg.value);
        tokenPrice[tokenId] = 0; // reset token price to zero after sale
    }

    function setMinimumPrice(uint256 price) public onlyOwner {
        require(isPricingEnabled, "Pricing is disabled");
        minimumPrice = price;
    }

    function setIsPricingEnabled(bool enabled) public onlyOwner {
        isPricingEnabled = enabled;
    }

    function getPricingStatus() public view returns (bool) {
    ...
    function trackTokenOwnership(uint256 tokenId) public {
        require(_exists(tokenId), "Token does not exist");
        tokenOwnerMap[tokenId] = ownerOf(tokenId);
    }
