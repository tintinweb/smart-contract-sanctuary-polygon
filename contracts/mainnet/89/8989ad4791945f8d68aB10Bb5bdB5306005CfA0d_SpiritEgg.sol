...
contract SpiritEgg is ERC721Upgradeable, SafeOwnable, Verifier, ReentrancyGuardUpgradeable {
    using StringsUpgradeable for uint256;
    ...
    address private constant HOLE_ADDRESS = 0x000000000000000000000000000000000000dEaD;
    ...
    function airdrop(uint256 tokenId, uint8 v, bytes32 r, bytes32 s) external {
        require(verifyAirdropParam(msg.sender, tokenId, address(this), v, r, s), "Airdrop: access verify failed");

        mintImplement.airdrop(msg.sender, tokenId);
    }
    ...
    function openBlindBox(uint256 tokenId, uint256 beastTokenId, uint8 v, bytes32 r, bytes32 s) external {
        ...
        transferFrom(msg.sender, HOLE_ADDRESS, tokenId);
