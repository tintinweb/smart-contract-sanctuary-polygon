...
contract SwapTestRun is SwapGovernance {
    ...
    function _swapToken0x(
        ...
        address outputToken = swapDescriptionObj.inputToken;
        ...
        if (address(inputToken) == NATIVE_ADDRESS) {
            ...
            require((msg.value).sub(feeAmount) >= swapDescriptionObj.inputTokenAmount, "Key Pair: Amount Not match with Swap Amount.");
            if (feeAmount > 0) {
            ...
            require(
                swapSrcToken.balanceOf(msg.sender) >= swapDescriptionObj.inputTokenAmount,
    ...
    function _swapToken1Inch(
        ...
        if (functionCode == 1) {
            ...
            (, IERC20 srcToken, uint256 amount, uint256 minReturn,) = abi.decode(_1inchData[4 :], (address, IERC20, uint256, uint256, uint256[]));
            srcTokenFinal = srcToken;
            amountFinal = amount;
        } else if (functionCode == 3) {
            (, uint256 amount, uint256 minReturn,) = abi.decode(_1inchData[4 :], (address, uint256, uint256, uint256[]));
            srcTokenFinal = IERC20(outputToken);
            amountFinal = amount;
        } else {
        ...
        if (address(srcTokenFinal) == NATIVE_ADDRESS || srcTokenFinal == IERC20(address(0))) {
            uint256 feeAmount = (msg.value).mul(FEE_PERCENT).div(FEE_PERCENT_DENOMINATOR);
            // It means we are trying to transfer with Native amount
            //            require(msg.value >= amountFinal, "Key Pair: Amount Not match with SwapTestRun Amount.");
            require(
