...
contract SwapMatrix is Context, ReentrancyGuard {
    ...
    constructor(address addressWMATIC, ISwapRouter _swapRouter) {
        WMATIC = IWMATIC(addressWMATIC);
    ...
    function swapETH(address _tokenOut, uint24 feeTier) external payable nonReentrant {
        ...
        uint256 fee = valueCoin * feeSwap / 1e6;
        uint256 amountIn = valueCoin - fee;
        shareFeeETH(fee);
    ...
    function swapToken(address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _feeTier) external nonReentrant {
        ...
        uint256 fee = _amountIn * feeSwap / 1e6;
        uint256 amountIn = _amountIn - fee;
        shareFee(_tokenIn, who, fee);
        
        swap(who, who, _tokenIn, _tokenOut, amountIn, _feeTier);
    }

    function swapETHTo(address _sendTo, address _tokenOut, uint24 feeTier) external payable nonReentrant {
        address who = _msgSender();
    ...
    function swapTokenTo(address _sendTo, address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _feeTier) external nonReentrant {
        address who = _msgSender();
    ...
    function swap(address who, address _sendTo, address _tokenIn, address _tokenOut, uint256 _amountIn, uint24 _feeTier) internal {
    ...
    function shareFeeETH(uint256 _amount) internal {
        uint256 shareOwner1 = _amount * shareOwner / 100;
        uint256 shareOwner2 = _amount - shareOwner1;
        Address.sendValue(payable(payeesOwner[0]), shareOwner1);
        Address.sendValue(payable(payeesOwner[1]), shareOwner2);
    }

    function shareFee(address _token, address who, uint256 _amount) internal {
