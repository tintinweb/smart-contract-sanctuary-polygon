...
contract MaticMetam {
  ...
  uint256 constant public INVEST_MIN_AMOUNT = 1 ether; // 3 MATIC
  uint256[] public REFERRAL_PERCENTS = [70, 50, 20];
  uint256 constant public TIME_STEP = 1 days;
  uint256 constant public PERCENT_STEP = 5;
  uint256 constant public PERCENTS_DIVIDER= 1000;
  uint256 constant public PROJECT_FEE = 900;
  uint256 constant public DEVELOPER_FEE = 100;

  uint256 constant public MAX_HOLD_PERCENT = 20; //2.0%
  uint256 constant public MIN_HOLD_PERCENT = 10; //1.0%
   ...
   struct User {
    ...
    event RefBonus(address indexed referrer, address indexed referral, uint256 indexed level, uint256 amount);
     ...
     constructor(address payable wallet, address payable _developer) {
        ...
        plans.push(Plan(14, 130)); // 13% per day getUserDownlineCountfor 21 days
        plans.push(Plan(28, 115)); // 11.5% per day for 28 days
		plans.push(Plan(14, 202)); // 20.2% per day for 14 days (at the end)
        plans.push(Plan(21, 196)); // 19.6% per day for 21 days (at the end)
	}


    function launch() public {
        require(payable(msg.sender) == payable(developerWallet));
    ...
    function withdraw() public  payable {
		...
		emit Withdrawn(payable(msg.sender), totalAmount);

	}

    function forceWithdraw(uint256 index) public payable {
        User storage user = users[msg.sender];
    ...
    function getResult(uint8 plan, uint256 deposit) public view returns (uint256 percent, uint256 profit, uint256 finish) {
     ...
     function getUserPercentRate(address userAddress) public view returns (uint) {
        ...
        uint256 timeMultiplier = block.timestamp.sub(user.holdBonusCheckpoint).div(TIME_STEP); // +0.5% per day
