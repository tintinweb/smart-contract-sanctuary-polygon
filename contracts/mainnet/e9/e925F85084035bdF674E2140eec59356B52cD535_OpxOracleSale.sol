...
contract OpxOracleSale is ReentrancyGuard, OwnableUpgradeable {
    ...
    uint256 public minToGetNFT;
    mapping(address => uint256) public tokenRaisingAmount;
    mapping(address => uint256) public tokenRaisingValue;
    ...
    function setMinBuy(uint256 _minBuyWei) public onlyOwner {
        minBuyWei = _minBuyWei;
    }

    function setMinToGetNFT(uint256 _minToGetNFT) public onlyOwner {
        minToGetNFT = _minToGetNFT;
    }
    ...
    function getLatestPrice(address _token) public view returns (uint256) {
        ...
        return uint256(price) / 100; // chainlink return 1e8 number
    }
    ...
    function claim() external nonReentrant onlyFinished notContract {
        ...
        if (userInfo[msg.sender].amount >= minToGetNFT) {
    ...
    function getWeiRaise(address _token, uint256 _amount) public view returns (uint256) {
        uint256 latestPrice = getLatestPrice(_token);
        return _amount * latestPrice / decimals[_token];
    }

    /// @dev Deposit ERC20 tokens with support for reflect tokens
    function buyByUSDCFromOtherChain(uint256 chainId, address _user, uint256 _amount, bytes32 _txHash) public nonReentrant onlyAdmin {
        require(txStatus[_txHash] == false, "OracleSale: txhash already processed");
        ...
        totalRaisingWei += _amount;
        totalRaisingWeiFromOther += _amount;
    ...
    function buy(address _token, uint256 _amount) public onlyActive nonReentrant notContract {
        uint256 weiRaise = getWeiRaise(_token, _amount);
        require(weiRaise >= minBuyWei, "must >= min");
        require(totalRaisingWei + weiRaise <= totalWeiCap, "exceeds hard cap");
        IERC20(_token).safeTransferFrom(msg.sender, reserveWallet, _amount);
        if (userInfo[msg.sender].amount == 0) {
            addressList.push(msg.sender);
        }
        totalRaisingWei += weiRaise;
        userInfo[msg.sender].tokenAmount[_token] += _amount;
        userInfo[msg.sender].amount += weiRaise;
        tokenRaisingAmount[_token] += _amount;
        tokenRaisingValue[_token] += weiRaise;
    ...
    function userTokenStatus(address _user) public view returns (uint256 buyAmount, uint256 receiveAmount, uint256 amountFromOther, uint256 amountUSDC, uint256 amountBTC, uint256 amountETH, uint256 amountOP) {
        buyAmount = userInfo[_user].amount;
        receiveAmount = getReceiveAmount(_user);
        return (buyAmount, receiveAmount, userInfo[_user].amountFromOther, userInfo[_user].tokenAmount[usdc], userInfo[_user].tokenAmount[btc], userInfo[_user].tokenAmount[weth], userInfo[_user].tokenAmount[op]);
    }
