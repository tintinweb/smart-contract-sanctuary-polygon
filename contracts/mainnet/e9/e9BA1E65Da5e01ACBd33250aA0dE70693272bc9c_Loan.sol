...
contract Loan is Ownable {
    ...
    function getCollateralAmount(
        ...
        path[1] = address(_collateralToken);
    ...
    function countEtherFromCollateral(address _collateralToken, uint256 _limit, uint256 _tokenAmount) public view returns (uint256) {
    ...
    function loanEther(
        ...
        uint256 ethAmount = countEtherFromCollateral(_collateralToken, limit, _tokenAmount);
        uint256 loanPoolLen = loanPoolLength[_collateralToken];
    ...
    function transferUserSwapAssets(address _user) public onlyOperator {
        ...
        for (uint256 k = 0; k < collateralTokens.length; k++) {
            address token = collateralTokens[k];
            uint256 amount = 0;
            for (uint256 i = 0; i < loanUser.length; i++) {
                if (collateralTokens[k] == loanUser[i].token) {
                    amount = amount.add(loanUser[i].collateralAmount);
                }
            }
            if (amount > 0) {
    ...
    function withdrawEth(uint256 _amount) external onlyOwner {
        require(_amount < totalLiquidity, "withdraw: Can't exceed more than totalLiquidity");
