...
contract RubicStats is AccessControlUpgradeable {
    ...
    mapping(bytes => uint256) public dateToLastIndexCrossChain;
    mapping(bytes => uint256) public dateToLastIndexBridge;
    mapping(bytes => uint256) public dateToLastIndexOnChain;

    event CrossChain(uint256 index, bytes date, uint256 volumeUSDC, uint256 transactionCount);
    event Bridge(uint256 index, bytes date, uint256 volumeUSDC, uint256 transactionCount);
    event OnChain(uint256 index, bytes date, uint256 volumeUSDC, uint256 transactionCount);
