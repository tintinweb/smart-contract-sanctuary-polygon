...
contract EVEOTCStakes is Owner, ReentrancyGuard {
     ...
     */
    constructor(IERC20 _erc20, address _owner) Owner(_owner) {
        asset = _erc20;
    }

    /**
     * Returns if the user has an active discount
     */
    function active(address user) external view returns (bool _is_active) {
    ...
    function addPlan(
        ...
        plans.push(Plan(_token_amount, _smart_discount_seller, _smart_discount_buyer, _option_discount_seller, _option_discount_buyer, _discount_duration, true));
    }
    ...
    */ 
    ...
    function startLock(uint256 _plan) external nonReentrant {
        ...
        ledger[msg.sender] = Record(block.timestamp, plans[_plan].token_amount, _plan);
     ...
     */
    function stakingSet(IERC20 _asset) external isOwner {
        asset = _asset;
    }
