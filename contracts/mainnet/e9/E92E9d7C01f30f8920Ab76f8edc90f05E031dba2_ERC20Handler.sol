...
contract ERC20Handler is IDepositExecute, HandlerHelpers, ERC20Safe {
    ...
    */
    ...
    function executeProposal(bytes32 resourceID, bytes calldata data) external override onlyBridge {
        ...
        if(tokenAddress == _networkToken) {
            IWrappedToken(tokenAddress).withdraw(amount);
            _safeTransferNetworkToken(address(recipientAddress), amount);
        } else {
            if (_burnList[tokenAddress]) {
    ...
    function _safeTransferNetworkToken(address to, uint256 value) internal {
        (bool success,) = to.call{value : value}(new bytes(0));
        require(success, 'Network token transfer failed');
    }

    receive() external payable {
        require(msg.sender == _networkToken, "Not network token");
    }
}
