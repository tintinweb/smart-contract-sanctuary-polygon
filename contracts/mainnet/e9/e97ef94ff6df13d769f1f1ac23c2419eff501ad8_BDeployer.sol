...
contract BDeployer is IBDeployer {
    ...
    bytes32 public constant INIT_CODE = keccak256(abi.encodePacked(type(Borrowable).creationCode));
...
contract BAllowance is PoolToken, BStorage {
    ...
    function _checkBorrowAllowance(
        ...
        if (spender != owner && _borrowAllowance != uint256(-1)) {
            require(_borrowAllowance >= value, "Lif3: BORROW_NOT_ALLOWED");
