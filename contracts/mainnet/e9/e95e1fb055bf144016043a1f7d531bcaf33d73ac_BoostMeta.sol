...
*/
    ...
    function regUser(uint _referrerID ) public returns(bool) 
    {
        ...
        userInfos[userAddressByID[_referrerID][0]][0].directCount += 3;
    ...
    function BuyULP() public returns(bool){
        ...
        userInfos[userAddressByID[_referrerID][0]][0].directCount += 2;
    ...
    function timeRemains(address _user) public view returns(uint, uint)
    {
        uint a;
        uint b;
        if( nextJoinPending[_user] == 0 || lastJoinTime[_user] + 48 hours < now) 
        {
            a = 0;
        }
        else
        {
            a = lastJoinTime[_user] - now;
        }       
        if( nextReJoinPending[_user] == 0  || lastReJoinTime[_user] + 3 hours < now) 
        {
            b = 0;
        }
        else
        {
            b = lastReJoinTime[_user] - now ;
        }  
        return (a,b);
    }
