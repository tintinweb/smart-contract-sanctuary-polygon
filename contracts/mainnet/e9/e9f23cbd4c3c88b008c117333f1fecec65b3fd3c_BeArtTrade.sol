...
contract BeArtTrade is Initializable, OwnableUpgradeable {
    event OrderPlace(
        address indexed from,
        uint256 indexed tokenId,
    ...
    function initialize() public initializer {
        __Ownable_init();
        serviceValue = 1750000000000000000;
        sellervalue = 1750000000000000000;
        deci = 18;
        publicMint = true;
        _tid = 1;
    }
    ...
    address public BeArt721;
    address public BeArt1155;
    uint256 deci;
    uint256 public _tid;
    ...
    function saleToken(
        ...
        if (ids[3] == 721) {
            IBEP721Upgradeable(_conAddr).safeTransferFrom(
                from,
                msg.sender,
                ids[0]
            );
            ...
            if (IBEP1155Upgradeable(_conAddr).balanceOf(from, ids[0]).sub(ids[2]) == 0) {
                if (order_place[from][ids[0]].price > 0) {
                    delete order_place[from][ids[0]];
                }
            }
            IBEP1155Upgradeable(_conAddr).safeTransferFrom(
                from,
                msg.sender,
                ids[0],
                ids[2],
    ...
    function _saleToken(
        ...
        address create;
        if(BeArt721    ==  _conAddr || BeArt1155    ==  _conAddr){
        if (ids[3] == 721) {
            (create, ldatas[0]) = BeArtCollection721(BeArt721)
                .getCreatorsAndRoyalty(ids[0]);
        } else {
            (create, ldatas[0]) = BeArtCollection1155(BeArt1155)
                .getCreatorsAndRoyalty(ids[0]);
        }
        }
    ...
    function acceptBId(
        ...
        if (ids[3] == 721) {
            ...
            if (order_place[msg.sender][ids[0]].price > 0) {
             ...
             if (
                IBEP1155Upgradeable(_conAddr).balanceOf(msg.sender, ids[0]).sub(ids[2]) == 0
            ) {
                if (order_place[msg.sender][ids[0]].price > 0) {
                    delete order_place[msg.sender][ids[0]];
                }
    ...
    function minting(
        ...
        if (datas[1] == 721) {
            BeArtCollection721(BeArt721).mint(
                ipfsmetadata,
                msg.sender,
                datas[2],
                id_
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], BeArt721, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        } else {
            BeArtCollection1155(BeArt1155).mint(
                ipfsmetadata,
                msg.sender,
                datas[0],
                datas[2],
            ...
            if (datas[3] > 0) {
                orderPlace(id_, datas[3], BeArt1155, datas[1], "mint");
            } else {
                emit OrderPlace(msg.sender, id_, datas[3]);
            }
        }
    ...
    function setCollectionAddress(address _BeArt721, address _BeArt1155)
        public
        onlyOwner
    {
        BeArt721 = _BeArt721;
        BeArt1155 = _BeArt1155;
    }
    function SetRoyalty(uint96 _royPer) public onlyOwner{
        BeArtCollection721(BeArt721).setRoyaltyPBEPentage(_royPer);
        BeArtCollection1155(BeArt1155).setRoyaltyPBEPentage(_royPer);
    }
    function changeCollectionOwner(address to) public {
        BeArtCollection721(BeArt721).changeCollectionOwner(to);
        BeArtCollection1155(BeArt1155).changeCollectionOwner(to);
    }
    function _adminBurn(uint256 tokenId, uint256 amount, address from) public onlyOwner{
        BeArtCollection721(BeArt721).adminBurn(tokenId);
        BeArtCollection1155(BeArt1155).adminBurn(tokenId,amount,from);
    }
}