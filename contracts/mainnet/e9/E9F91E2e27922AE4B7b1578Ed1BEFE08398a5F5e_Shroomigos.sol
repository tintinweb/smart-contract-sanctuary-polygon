...
contract Shroomigos is ERC721 , DefaultOperatorFilterer {
    uint256 public totalSupply;
    uint256 public cost = 0.05 ether;
    uint256 public maxMints = 8000;
    ...
    constructor()
        ERC721("shroomigos", "SHRM")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 6666) revert SoldOut();
        if(cost < threshold ) {
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeicvmmq5vyul2szeiyhovfqwactuoymrhf6wmtdr7lq3idnszvstqa/", Strings.toString(tokenId), ".json")
            );
    }
