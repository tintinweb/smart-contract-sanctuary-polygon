...
contract GiddyGiddyYA is BaseYA, ReentrancyGuardUpgradeable, OwnableUpgradeable, PausableUpgradeable, ERC2771ContextUpgradeable {
  ...
  function calcPrice(address router, address src, address dst, uint256 value) internal view returns (uint256 amountOut, uint256 priceImpact) {
    ...
    route[0] = src;
    route[1] = dst;
    amountOut = IUniswapV2Router01(router).getAmountsOut(value, route)[1];
    uint256 dstReserve = IUniswapV2Router01(router).getAmountsOut(999999999999999999999999999999999999999999999999, route)[1];
    route[0] = dst;
    route[1] = src;
    uint256 srcReserve = IUniswapV2Router01(router).getAmountsOut(999999999999999999999999999999999999999999999999, route)[1];

    priceImpact = (value * dstReserve * BASE_PERCENT) / srcReserve;
    priceImpact /= dstReserve - ((srcReserve * dstReserve) / (srcReserve + value));
