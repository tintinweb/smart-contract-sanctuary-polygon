...
contract DragonLairV1 is BaseSiloAction {
    ...
    function enter(
    ...
    ) public override returns (uint256[5] memory) {
        uint256[5] memory inputAmounts = abi.decode(inputData, (uint256[5]));

        address[5] memory inputs;

        (inputs) = abi.decode(configuration, (address[5]));

        uint256 index = 1;

        for (; index < 5; ) {
            if (inputs[index] != address(0) && inputAmounts[index] > 0) {
                _swapTokens(
                    inputs[index],
                    QUICK_ADDRESS,
                    inputAmounts[index],
        ...
        uint256 quick = IERC20(QUICK_ADDRESS).balanceOf(address(this));

        if (quick > 0) {
            SafeERC20.safeApprove(IERC20(QUICK_ADDRESS), dQUICK_ADDRESS, quick);
            IDragonLair(dQUICK_ADDRESS).enter(quick);
        }
    }
    ...
    function exit(
        ...
        uint256 dQquick = IERC20(dQUICK_ADDRESS).balanceOf(address(this));

        if (dQquick == 0) {
            return outputAmounts;
        }

        IDragonLair(dQUICK_ADDRESS).leave(dQquick);

        uint256 quick = IERC20(QUICK_ADDRESS).balanceOf(address(this));

        _takeFee(implementation, quick, QUICK_ADDRESS);
    }

    function createConfig(
        address[5] memory _inputs
    ) public pure returns (bytes memory configData) {
        configData = abi.encode(_inputs);
    }
    ...
    function showBalances(
        ...
        uint256 quick = IDragonLair(dQUICK_ADDRESS).QUICKBalance(_silo);
        return
            ActionBalance({
                collateral: quick,
                debt: 0,
                collateralToken: QUICK_ADDRESS,
                debtToken: address(0),
    ...
    function validateConfig(
        ...
        address[5] memory inputs = abi.decode(configuration, (address[5]));

        if (inputs[0] != QUICK_ADDRESS) {
            return false;
        }
