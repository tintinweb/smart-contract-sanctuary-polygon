...
// File: contracts_ETH/main/StakingMiniApeV2.sol
...
/** 
* @author Formation.Fi.
* @notice Implementation of the contract StakingMiniApeV2.;
*/
interface IRewarder {
...
contract StakingMiniApeV2 is Ownable {
    address public safeHouse;
    address public master;
    MasterApeV2 public masterApeV2;
    Admin public admin;
    constructor (address _admin, address _safeHouse, address _master)  {
        ...
        safeHouse = _safeHouse;
        admin = Admin(_admin);
        masterApeV2 = MasterApeV2(_master);
    ...
    function stake(
        ...
        ERC20(_lpToken).approve(master, amount);
        masterApeV2.deposit(_pid, amount, address(this));
    }
    ...
    function unstake (
        ...
        masterApeV2.withdraw(_pid, amount, address(this));
    }


    // **** CLAIM BANANA  REWARDS ****
    function claimRewards(
        uint _pid
    ) external onlyManager {
        masterApeV2.harvest(_pid, address(this));
    }
    // ****withdraw And CLAIM BANANA  REWARDS ****
    function unstakeAndClaim (
        uint _pid,
        uint amount
    ) external onlyManager  {
        masterApeV2.withdrawAndHarvest(_pid, amount, address(this));
    }
    
    function getBananaRewards(
        uint256 _pid) 
         external view returns (uint256 _pending){
         _pending = masterApeV2.pendingBanana(_pid, address(this));
    } 
