...
contract ItemNFT is BaseNFT {
  ...
  function mintUserItems(address[] calldata _owners, uint8[] calldata _genes, uint16[] calldata _classIds, uint[] calldata _price) onlyMintAdmin external {
    for(uint i = 0; i < _owners.length; i++) {
      address owner = _owners[i];
      uint16 classId = _classIds[i];
      uint8 gene = _genes[i];
      uint price = _price[i];
      mintItem(owner, gene, classId, price, i);
    }
  }
