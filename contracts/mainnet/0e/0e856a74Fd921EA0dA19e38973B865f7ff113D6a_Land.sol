    ...
    Ownable
{
    ...
    uint256 private _chickenOwnerSaleStart = FAR_FUTURE;
    uint256 private _chickenOwnerSaleEnd = FAR_FUTURE;
    bool private _isPublicSaleSet = false;
    ...
    address public immutable chickenContract;
    ...
    constructor(
        ...
        ERC721("Chicken Derby Land NFT", "LND")
        RandomlyAssigned(MAX_SUPPLY, NUMBER_OF_RESERVED_LANDS)
    {
    ...
    function startChickenOwnerSale(uint256 _daysAfter) public onlyOwner {
        _chickenOwnerSaleStart = block.timestamp;
        _chickenOwnerSaleEnd = _chickenOwnerSaleStart + _daysAfter * 1 days;
    }

    function setPublicSale(bool _state) public onlyOwner {
        require(
            block.timestamp > _chickenOwnerSaleEnd,
            "Land: Still in presale"
        );
        _isPublicSaleSet = _state;
    }
    ...
    function isChickenOwnerSaleActive() public view returns (bool) {
        return (block.timestamp >= _chickenOwnerSaleStart &&
            block.timestamp <= _chickenOwnerSaleEnd);
    }

    function isPublicSaleActive() public view returns (bool) {
        return block.timestamp > _chickenOwnerSaleEnd && _isPublicSaleSet;
    }
        ...
        ensureAvailabilityFor(_mintAmount)
    {
        if (isChickenOwnerSaleActive())
            require(
                IERC721(chickenContract).balanceOf(_msgSender()) > 0,
                "Land: Not chicken owner"
            );
        else if (!isPublicSaleActive()) revert Errors.PublicSaleNotActive();
        if (_mintAmount == 0) revert Errors.InvaildAmount();
...
library Errors{
    ...
    error InvaildAmount();
    error NotOwnerOfChicken();
    error ChickenOwnerSaleNotActive();
