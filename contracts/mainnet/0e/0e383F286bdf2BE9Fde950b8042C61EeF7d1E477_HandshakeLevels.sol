...
contract HandshakeLevels is Ownable {
    ...
    function _distributeMoney(address sender, uint value, bool isIOS, address token) private returns (uint) {
        address[] memory friends;
        uint friendsCount;
        uint valueForFriendTotal = 0;
        if (!isIOS) {
            (friends, friendsCount) = getHandshakes(sender);
            // invitation bonus not send for iOS due charge back
            uint valueForFriend;
            if (friends[0] != address(0)) {
                valueForFriend = value * getPercentPerInvitationBonusWei() / 1 ether;
                //slither-disable-next-line unchecked-lowlevel
                if (token != address(0)) IERC20(token).transferFrom(sender, friends[0], valueForFriend);
            ...
            for(uint8 x=1;x<friendsCount;x++) {
                if (friends[x] != address (0) && block.timestamp - IDexGoStorage(storageContract).getLatestPurchaseTime(friends[x]) < 60 * 60 * 24 * 30) { // must make at least one purchase per months to receive reward
                    ...
                    //slither-disable-next-line unchecked-lowlevel
                    if (token != address(0)) IERC20(token).transferFrom(sender, friends[x], valueForFriend);
        ...
        if (IDexGoNFT(IDexGoStorage(storageContract).getNftContract()).getIsIGO() != true) {
            //slither-disable-next-line unchecked-lowlevel
            if (token != address(0)) IERC20(token).transferFrom(sender, IDexGoStorage(storageContract).getDexGo(), partForRewards);
            else Address.sendValue(payable(IDexGoStorage(storageContract).getDexGo()), partForRewards);
        } else partForRewards = 0;

        uint valueForTeam = (value - valueForFriendTotal - partForRewards) ;
        //slither-disable-next-line unchecked-lowlevel
        if (token != address(0)) IERC20(token).transferFrom(sender, IDexGoStorage(storageContract).getAccountTeam1(), valueForTeam * 33 / 100);
        ...
        //slither-disable-next-line unchecked-lowlevel
        if (token != address(0)) IERC20(token).transferFrom(sender, IDexGoStorage(storageContract).getAccountTeam2(), valueForTeam * 33 / 100);
        else Address.sendValue(payable(IDexGoStorage(storageContract).getAccountTeam2()), valueForTeam * 33 / 100);
        valueForTeam = valueForTeam - valueForTeam * 33 / 100;
        //slither-disable-next-line unchecked-lowlevel
        if (token != address(0)) IERC20(token).transferFrom(sender, owner(), valueForTeam);
        else Address.sendValue(payable(owner()), valueForTeam);
        return (value - valueForFriendTotal - partForRewards);
    }

    function distributeMoney(address sender, uint value, bool isIOS, address token) public returns (uint)  {
        ...
        if (token != address (0)) require(IDexGoStorage(storageContract).isTokenAllowed(token) == true, "NA");
        return _distributeMoney(sender, value, isIOS, token);
    }
    ...
    function updateCounterPayable() public payable nonReentrant {
        counterTotal++;
