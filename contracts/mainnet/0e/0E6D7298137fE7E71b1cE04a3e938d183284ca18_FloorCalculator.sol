...
contract FloorCalculator is TokensRecoverable
{
    ...
    // add addresses that you have just locked dmagic permanently and wont ask for drax or axMatic from pool
    ...
    function ignoredAddressesTotalBalance() public view returns (uint256)
    {
        ...
        for (uint i = 0; i < ignoredAddresses.length(); i++) {
            total = total.add(dMagic.balanceOf(ignoredAddresses.at(i)));
        }
    ...
    //  _wrappedTokens is all lp pairs axMatic<->dMagic
    //  backing token is drax
    function calculateSubFloorDRAX(IERC20[] memory wrappedTokens, IERC20 backingToken) public view returns ( uint256){
        ...
        for(uint i=0;i<uniswapV2Factories.length;i++){
            ...
            address pair = UniswapV2Library.pairFor(address(uniswapV2Factories[i]), address(dMagic), address(backingToken));
            
            uint256 freedMagic = dMagic.totalSupply().sub(dMagic.balanceOf(pair)).sub(ignoredAddressesTotalBalance()) ;

            if (freedMagic > 0) {
                uint256[] memory amountsOut = UniswapV2Library.getAmountsOut(address(uniswapV2Factories[i]), freedMagic, path);
        ...
        currentBacking=currentBacking.mul(100); // since DRAX is 100x the uniV2 pairs balance 

        if (requiredBacking >= currentBacking) { return 0; }
        
        subFloor = currentBacking.sub(requiredBacking);

        return subFloor.div(100); // convert back to uniV2 
        
    }


    // wrapped MATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270 
    // backing token is axMatic 
   
    function calculateSubFloorAXMatic(IERC20 wrappedToken, IERC20 backingToken) public view returns (uint256)
    {
        uint256 backingInPool = 0;
        uint256 sellAllProceeds = 0;
        address[] memory path = new address[](2);
        ...
        subFloor = currentBacking - requiredBacking; 
        return subFloor;           
