...
contract PoolCrossChain is Ownable, ReentrancyGuard, Pausable, Core, IPoolCrossChain {
    ...
    function withdrawFromOtherAsset(
        ...
        if (fee > 0) {
            feeCollector.collectFees(initialAsset, fee);
        }
        ...
        returns (uint256 actualToAmount, uint256 haircut)
    {
        ...
        require(router.isApprovedAsset(destinationChain, destinationAsset), "DEST_CHAIN_NOT_APPROVED");
        ...
        params.fromAsset = router.getAssetData(chainId, fromToken);
        params.toAsset = router.getAssetData(destinationChain, destinationAsset);

        (actualToAmount, haircut) = _quoteFrom(fromAsset, params.toAsset, fromAmount);
        ...
        params.toAsset.cash -= actualToAmount;
        params.toAsset.liability -= _dividend(haircut, retentionRatio);

        router.modifyCrossChainParams(
            destinationChain,
            destinationAsset,
            params.toAsset.cash,
            params.toAsset.liability
        );
        ...
        params.estimatedFee = router.estimateFee(destinationChain, params.payload);

        require(params.estimatedFee == msg.value, "NOT_ENOUGH_FEE");
        router.route{ value: msg.value }(
            destinationChain,
            params.toAsset.nativeAssetAddress,
            params.estimatedFee,
            params.payload
    ...
    function receiveSwapCrossChain(
        ...
        require(srcAssetAddress != address(0), "ZERO_SRC_ADDR");
        require(dstAssetAddress != address(0), "ZERO_DST_ADDR");
        require(dstAmount > 0, "ZERO_FROM_AMOUNT");

        ICSMCrossChainRouter.CrossChainAsset memory srcAsset = router.getAssetData(srcChain, srcAssetAddress);
        IAsset dstAsset = IAsset(dstAssetAddress);

        // normalizying decimals in case src asset decimals != dst asset decimals
        uint256 srcDecimals = srcAsset.decimals;
        uint256 dstDecimals = uint256(dstAsset.decimals());
        uint256 srcAmountNormalized;
        if (srcDecimals < dstDecimals) {
            srcAmountNormalized = dstAmount / 10**(dstDecimals - srcDecimals);
        } else if (srcDecimals > dstDecimals) {
            srcAmountNormalized = dstAmount * 10**(srcDecimals - dstDecimals);
        }

        srcAsset.cash += srcAmountNormalized;
        router.modifyCrossChainParams(
            srcChain,
            router.getApprovedAssetId(srcAssetAddress, srcChain),
            srcAsset.cash,
            srcAsset.liability
        );

        dstAsset.removeCash(dstAmount);
        dstAsset.addLiability(_dividend(haircut, retentionRatio));
        dstAsset.transferUnderlyingToken(sender, dstAmount);

        if (haircut > 0) {
            feeCollector.collectFees(dstAsset, haircut);
        }

        emit CrossChainSwap(sender, srcAssetAddress, dstAssetAddress, srcChain, dstAmount, dstAmount);
    }
