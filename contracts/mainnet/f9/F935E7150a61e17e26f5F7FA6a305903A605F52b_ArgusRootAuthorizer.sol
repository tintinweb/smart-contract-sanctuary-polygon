// sdk f6e6ca1e9383b5cd394639fbcfb333d596f4c4ce
...
contract ArgusRootAuthorizer is BaseAuthorizer, IAuthorizerSupportingHint {
    ...
    // will added into `processSet` and be invoked unconditionally.
    ...
    /// @dev Unpack the hint and do the validation.
    function _getAuthorizerFromHint(
        TransactionData calldata transaction
    ) internal view returns (address auth, bytes memory subHint) {
        bytes32 role;
        (role, auth, subHint) = _unpackHint(transaction.hint);

        // Important: Validate the hint.
        // (1) The role from hint should in RoleManager's result.
