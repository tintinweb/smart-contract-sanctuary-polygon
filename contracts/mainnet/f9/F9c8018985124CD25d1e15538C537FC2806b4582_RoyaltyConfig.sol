...
contract Rule is Ruler, Governable {
    ...
    event SetDisableNft(address token, uint256 tokenId, bool disable);
    ...
    function setDisableNft(address _token, uint256 _tokenId, bool _disable) external onlyAdmin {
        bytes32 key = keccak256(abi.encodePacked(_token, _tokenId));
        disableNft[key] = _disable;
        emit SetDisableNft(_token, _tokenId, _disable);
    }
