...
import {WhitelistWithdrawable, AccountWhitelist} from "../../core/withdraw/WhitelistWithdrawable.sol";
...
contract MultichainProtocol is IUseProtocol, Ownable, Multicall, WhitelistWithdrawable, NativeReturnMods {
    address private constant _NO_TOKEN_OVERRIDE = 0x0000000000000000000000000000000000000000;

    AccountWhitelist private immutable _multichainWhitelist;

    constructor(address withdrawWhitelist_)
        WhitelistWithdrawable(withdrawWhitelist_) {
        _multichainWhitelist = new AccountWhitelist();
    }

    function approveMultichain(address multichain_) external onlyOwner {
        _multichainWhitelist.addAccountToWhitelist(multichain_);
    }

    function revokeMultichain(address multichain_) external onlyOwner {
        _multichainWhitelist.removeAccountFromWhitelist(multichain_);
    }

    function use(UseParams calldata params_) external payable {
        require(params_.chain != block.chainid, "MC: wrong chain id");
        require(params_.account != address(0), "MC: zero receiver");
        require(params_.args.length == 41, "MC: invalid args length");
        require(params_.ins.length == 1, "MC: wrong number of ins");
        require(params_.inAmounts.length == 1, "MC: wrong number of in amounts");
        require(params_.outs.length == 1, "MC: wrong number of outs");

        address multichain = address(bytes20(params_.args[:20]));
        require(_multichainWhitelist.isAccountWhitelisted(multichain), "MC: contract not allowed");
        MultichainMethod method = MultichainMethod(uint8(bytes1(params_.args[20:21])));
        address fromTokenOverride = address(bytes20(params_.args[21:41]));

        NativeClaimer.State memory nativeClaimer;
        _hop(params_.ins[0], params_.inAmounts[0], params_.chain, params_.account, multichain, method, fromTokenOverride, nativeClaimer);
    }

    function _hop(TokenCheck calldata in_, uint256 inAmount_, uint256 chain_, address account_, address multichain_, MultichainMethod method_, address fromTokenOverride_, NativeClaimer.State memory nativeClaimer_) private returnUnclaimedNative(nativeClaimer_) {
        address fromToken = fromTokenOverride_ == _NO_TOKEN_OVERRIDE ? in_.token : fromTokenOverride_;

        TokenHelper.transferToThis(in_.token, msg.sender, inAmount_, nativeClaimer_);

        uint256 sendValue = TokenHelper.approveOfThis(in_.token, multichain_, inAmount_);
        if (method_ == MultichainMethod.Swapout) {
            IMultichain(multichain_).Swapout(inAmount_, account_);
        } else if (method_ == MultichainMethod.Transfer) {
            IMultichain(multichain_).transfer(account_, inAmount_);
        } else if (method_ == MultichainMethod.AnySwapOutUnderlying) {
            IMultichain(multichain_).anySwapOutUnderlying(fromToken, account_, inAmount_, chain_);
        } else if (method_ == MultichainMethod.AnySwapOutNative) {
            IMultichain(multichain_).anySwapOutNative{value: sendValue}(fromToken, account_, chain_);
        } else if (method_ == MultichainMethod.AnySwapOut) {
            IMultichain(multichain_).anySwapOut(fromToken, account_, inAmount_, chain_);
        } else {
            revert("MC: unexpected method");
        }
        TokenHelper.revokeOfThis(in_.token, multichain_);
    }
}