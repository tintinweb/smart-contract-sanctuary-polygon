...
contract CurveSwapRouter is ICurveSwapRouter {
  ...
  function _sellARTHUSDForExact(
    ...
    if (amountBUSDOutMin > 0) {
      zap.exchange_underlying(0, 1, amountARTHUSDInForBUSD, amountBUSDOutMin, me);
    }

    if (amountUSDCOutMin > 0) {
      zap.exchange_underlying(0, 2, amountARTHUSDInForUSDC, amountUSDCOutMin, me);
    }

    if (amountUSDTOutMin > 0) {
      zap.exchange_underlying(0, 3, amountARTHUSDInForUSDT, amountUSDTOutMin, me);
    }
  ...
  function _buyARTHUSDForExact(
    ...
    if (amountBUSDIn > 0) {
      ...
      zap.exchange_underlying(1, 0, amountBUSDIn, amountARTHUSDOutMinForBUSD, me);
    }

    if (amountUSDCIn > 0) {
      usdc.transferFrom(msg.sender, me, amountUSDCIn);
      usdc.approve(address(zap), amountUSDCIn);
      zap.exchange_underlying(2, 0, amountUSDCIn, amountARTHUSDOutMinForUSDC, me);
    }

    if (amountUSDTIn > 0) {
      usdt.transferFrom(msg.sender, me, amountUSDTIn);
      usdt.approve(address(zap), amountUSDTIn);
      zap.exchange_underlying(3, 0, amountUSDTIn, amountARTHUSDOutMinForUSDT, me);
    }
