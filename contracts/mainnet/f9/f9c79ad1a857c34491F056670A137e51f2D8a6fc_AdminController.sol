...
contract ERC721OpenSale is ERC721BaseSale, __with_semver(uint8(1), uint8(1), uint8(0)) {
    ...
    function buy(
        ...
        if (msg.value > 0 && _nativeTokenAddress == tokenBuyAddress) {
    ...
    __with_semver(uint8(1), uint8(0), uint8(2))
{
    ...
    address[] public _whitelistTokens;
    event Upgrade(address code);
    ...
    function setWhitelistTokenAddress(address[] calldata tokenAddress_) external onlyOwner {
        _whitelistTokens = tokenAddress_;
    }
    ...
    function isInWhitelistTokenAddress(address tokenAddress_) public view returns (bool) {
        for (uint256 i = 0; i < _whitelistTokens.length; i++) {
            if (_whitelistTokens[i] == tokenAddress_) return true;
        }
        return false;
    }

    function getTradePrice(address tokenSalePrice, address tokenBuyPrice) public view returns (uint256 price) {
        if (tokenSalePrice == nativeTokenAddress) tokenSalePrice = nativeTokenWrapperAddress;
    ...
    function _getTradePrice(address uniswapV3Pool, address tokenBuyPrice) private view returns (uint256 price) {
        ...
        uint160 sqrtPriceX96 = _getSqrtPriceX96(pool, _twapInterval);
