...
/// @dev Enables / disables a Merkle root `root`
    function setRoot(
        bytes32 root,
        bool enabled
    ) external {
        ...
        require(
            s.marketConfig.signers[LibCommon.msgSender()],
            "setRoot: NOT_ALLOWED"
        );

        s.accounts.roots[root] = enabled;
    }

/// @dev Sets current refill value
    function setRefillValue(
        uint256 value
    ) external onlyDiamondOwner {

        s.accounts.refillValue = value;
    }

/// @dev Prevents account refills
    function setRefillBlacklisted(
        address account,
        bool blacklisted
    ) external onlyDiamondOwner {

        s.accounts.refillBlacklist[account] = blacklisted;
    }

/// @dev Refill the caller with {Accounts-refillValue}.
    function refillNative(
        bytes32[] calldata proof,
        bytes32 root
    ) external {

        address account = LibCommon.msgSender();

        require(
            s.accounts.roots[root],
            "refillNative: INVALID_ROOT"
        );
        require(
            !s.accounts.refillBlacklist[account],
            "refillNative: NOT_ALLOWED"
        );
        require(
            ...
            "refillNative: INVALID_PROOF"
        );

        (bool sent, bytes memory data) = payable(account).call{value: s.accounts.refillValue}(""); 
        require(
            sent,
            "refillNative: SEND_CALL_FAILED"
        );
    }
}
