...
contract Snickradoodle is ERC721 , DefaultOperatorFilterer {
    ...
    constructor()
        ERC721("Snickradoodle by Ark945", "SKDA")
    {}

    function mint() external payable {
        
        if (userMints[msg.sender] >= maxMints) revert MintLimit();
        if (totalSupply + 1 > 5490) revert SoldOut();
        ...
        returns (string memory)
    {
        return
            string(
                abi.encodePacked("ipfs://bafybeig4kzs4xx62nqxkt475iw6szgflcr44dxi5thouykg2hfnukgli5i/", Strings.toString(tokenId), ".json")
            );
    }
