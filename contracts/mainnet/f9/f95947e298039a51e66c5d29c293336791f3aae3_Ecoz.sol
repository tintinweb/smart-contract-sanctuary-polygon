...
contract Ecoz is ERC721, Ownable {
    ...
    uint256 public jagSupply;
    uint256 public buckSupply;
    ...
    uint256 public costJag = 600;
    uint256 public costBuck = 480;
    uint256 public costTree = 360;
    ...
    function mintJag(address user) internal {  
        require(jagSupply + 1 <= maxJagSupply, "Over max supply");
        jagSupply++;
        weightJaguar[jagSupply] = jaguarsURI.jagRandomizer(user, jagSupply);
        balanceJaguar[user]++;
        weightOfOwner[user] = weightOfOwner[user] + weightJaguar[jagSupply];
        _mint(user, jagSupply);
        updateBreedCosts();
    }
    
    function mintBuck(address user) internal { 
        require(buckSupply + 1 <= maxBuckSupply, "Over max supply");
        buckSupply++;
        weightBushBuck[buckSupply + 1350] = bushBucksURI.buckRandomizer(user, buckSupply + 1350);
        balanceBushBuck[user]++;
        weightOfOwner[user] = weightOfOwner[user] + weightBushBuck[buckSupply + 1350];
        _mint(user, buckSupply + 1350);
        updateBreedCosts();
    }
    
    function mintTree(address user) internal {       
        require(treeSupply + 1 <= maxTreeSupply, "Over max supply");
        treeSupply++;
        weightBananaTree[treeSupply + 4050] = bananaTreesURI.treeRandomizer(user, treeSupply + 4050);
        balanceBananaTree[user]++;
        weightOfOwner[user] = weightOfOwner[user] + weightBananaTree[treeSupply + 4050];
        _mint(user, treeSupply + 4050);
        updateBreedCosts();
    }

    function mintJaguar() public {
        Thrive.burn(msg.sender, costJag * 1000000000000000000);
        mintJag(msg.sender);
    }

    function mintBushBuck() public {
        Thrive.burn(msg.sender, costBuck * 1000000000000000000);
        mintBuck(msg.sender);
    }

    function mintBananaTree() public {
        Thrive.burn(msg.sender, costTree * 1000000000000000000);
        mintTree(msg.sender);
    }
                /*----------------------//
                //   OWNER FUNCTIONS    //
    ...
    function airdrop(airdropBundle[] memory bundle) public onlyOwner {
        for (uint256 i; i < bundle.length; i++) {
            address user = bundle[i].reciever;
            for (uint8 j; j < bundle[i].numJags; j++) {
                mintJag(user);
            }
            for (uint8 k; k < bundle[i].numBucks; k++) {
                mintBuck(user);
            }
            for (uint8 l; l < bundle[i].numTrees; l++) {
                mintTree(user);
            }
        }
    }
    ...
    function upgrade(uint256 tokenID, uint8 numLevels) public {
        require(msg.sender == ownerOf(tokenID));
        if (tokenID > 0 && tokenID <= 450) {
            require(level[tokenID] + numLevels <= maxLevel);
            for(uint8 i; i < numLevels; i++) {
                Thrive.levelUpdate(msg.sender);
                weightJaguar[tokenID] = weightJaguar[tokenID] + 4;
                level[tokenID] = level[tokenID] + 1;
                Thrive.burn(msg.sender, ((costJag - (costJag % 3)) / 3) * 1000000000000000000);
            }
        }
        else if (tokenID > 1350 && tokenID <= 2250) {
            require(level[tokenID] + numLevels <= maxLevel);
            for(uint8 i; i < numLevels; i++) {
                Thrive.levelUpdate(msg.sender);
                weightBushBuck[tokenID] = weightBushBuck[tokenID] + 3;
                level[tokenID] = level[tokenID] + 1;
                Thrive.burn(msg.sender, ((costBuck - (costBuck % 3)) / 3) * 1000000000000000000);
            }
        }
        else if (tokenID > 4050 && tokenID <= 5850) {
            require(level[tokenID] + numLevels <= maxLevel);
            for(uint8 i; i < numLevels; i++) {
                Thrive.levelUpdate(msg.sender);
                weightBananaTree[tokenID] = weightBananaTree[tokenID] + 2;
                level[tokenID] = level[tokenID] + 1;
                Thrive.burn(msg.sender, ((costTree - (costTree % 3)) / 3) * 1000000000000000000);
            }
        }
        else {
            revert();
    ...
    function getBalanceJag(address user) external view returns(uint256) {
        return balanceJaguar[user];
    }

    function getBalanceBuck(address user) external view returns(uint256) {
        return balanceBushBuck[user];
    }

    function getBalanceTree(address user) external view returns(uint256) {
        return balanceBananaTree[user];
    }
        ...
        if (((jagPopulation * 2) >= buckPopulation) && ((buckPopulation * 2) >= treePopulation)) {
            ...
            for (uint8 i; i < populationRange.length; i++) {
                ...
                if (a == populationRange[i]) {
                    costJag = jagRangeOver[i];
                    break;
                }
                if (a > populationRange[11]) {
                    costJag = jagRangeOver[12];
                    break;
                }
            }
            for (uint8 j; j < populationRange.length; j++) {
                ...
                if (b == populationRange[j]) {
                    costBuck = buckRangeOver[j];
                    costTree = treeRangeUnder[j];
                    break;
                }
                if (b > populationRange[11]) {
                    costBuck = buckRangeOver[12];
                    costTree = treeRangeUnder[12];
                    break;
                }
            }
        }
            ...
            for (uint8 i; i < populationRange.length; i++) {
                if (a == populationRange[i]) {
                    costJag = jagRangeUnder[i];
                    break;
                }
                if (a > populationRange[11]) {
                    costJag = jagRangeUnder[12];
                    break;
                }
            }
            for (uint8 j; j < populationRange.length; j++) {
                if (b == populationRange[j]) {
                    costBuck = buckRangeUnder[j];
                    costTree = treeRangeOver[j];
                    break;
                }
                if (b > populationRange[11]) {
                    costBuck = buckRangeUnder[12];
                    costTree = treeRangeOver[12];
                    break;
                }
            }
        }
