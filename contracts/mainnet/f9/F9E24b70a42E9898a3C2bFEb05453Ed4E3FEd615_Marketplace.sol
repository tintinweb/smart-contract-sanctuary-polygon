...
contract Marketplace is BaseContract {
  ...
  event OrderTaken(address indexed nftContractAddress, uint indexed tokenId, address taker, uint price, IMarketPlace.OrderType orderType, address paymentAddress, uint weekNumber, uint expiredTime, uint timestamp);
  event OrderCanceled(address indexed nftContractAddress, uint indexed tokenId, uint timestamp);
  ...
  function makeOrder(
    ...
    _validatePaymentAddresses(_nftContractAddress, makeOrder._paymentAddresses);
    IERC4907 nftContract = IERC4907(_nftContractAddress);

    if (makeOrder._startingPrice != makeOrder._endingPrice) {
      require(makeOrder._actionDuration > 0, "Marketplace: actionDuration must be > 0");
    }
    ...
    if (makeOrder._orderType == IMarketPlace.OrderType.renting) {
      require(makeOrder._rentingDuration > 0, "Marketplace: rentingDuration must be > 0");
    } else if (makeOrder._orderType == IMarketPlace.OrderType.installmentRental) {
      require(makeOrder._weekNumber > 0, "Marketplace: weekNumber must be > 0");
    }

    require(orders[_nftContractAddress][_tokenId].maker == address(0), "Marketplace: NFT is being sold");
    require(makeOrder._startingPrice > 0 && makeOrder._endingPrice > 0, "Marketplace: startingPrice and endingPrice must be > 0");

    _transferNft(_nftContractAddress, _tokenId, msg.sender, address(this));

    uint rentingDuration;

    if (makeOrder._orderType != IMarketPlace.OrderType.trading) {
      rentingDuration = makeOrder._orderType == IMarketPlace.OrderType.renting
        ? makeOrder._rentingDuration
        : makeOrder._weekNumber * config.secondsInWeek;
    }

    orders[_nftContractAddress][_tokenId] = Order(
      msg.sender,
      address(0),
      makeOrder._startingPrice,
      makeOrder._endingPrice,
      makeOrder._actionDuration,
      rentingDuration,
      block.timestamp,
      makeOrder._paymentAddresses,
      makeOrder._orderType,
      IMarketPlace.OrderStatus.created,
      makeOrder._orderType == IMarketPlace.OrderType.installmentRental ? makeOrder._weekNumber : 0,
      0
    );
  ...
  function takeOrder(address _nftContractAddress, uint _tokenId, address _paymentAddress, uint _weekNumber) external validOrder(_nftContractAddress, _tokenId) {
    ...
    if (order.orderType == IMarketPlace.OrderType.trading) {
      ...
      nftContract.setUser(_tokenId, msg.sender, uint64(order.rentingDuration));
    }

    emit OrderTaken(_nftContractAddress, _tokenId, msg.sender, currentPrice, order.orderType, _paymentAddress, _weekNumber, order.rentingDuration, block.timestamp);
  }
  ...
  function cancelOrder(address _nftContractAddress, uint _tokenId) external validOrder(_nftContractAddress, _tokenId) {
    ...
    emit OrderCanceled(_nftContractAddress, _tokenId, block.timestamp);
  }
  ...
  function _validatePaymentAddresses(address _nftContractAddress, address[] calldata _paymentAddresses) private view {
    ...
    for (uint i = 0; i < _paymentAddresses.length; i += 1) {
      for (uint j = i; j < nftListing.paymentAddresses.length; j += 1 ) {
  ...
  function _getCurrentPrice(address _nftContractAddress, uint _tokenId, uint _weekNumber, address _paymentAddress) private view returns (uint) {
    ...
    if (order.orderType == IMarketPlace.OrderType.installmentRental) {
      return _weekNumber * order.endingPrice;
    }
    ...
    if (nftListing.isUniSwapV3[indexPayment]) {
      ...
      return currentPrice * (10 ** decimalTokenIn) / tokenPrice;
    } else {
      ...
      return currentPrice * DECIMAL9 / tokenPrice;
    }
  }
