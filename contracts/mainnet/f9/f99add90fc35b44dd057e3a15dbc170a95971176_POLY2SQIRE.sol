...
contract POLY2SQIRE is owned {
    ...
    constructor(address payable ownerAddress, address payable ID1address, address _magicPoolAddress) public {
        ...
        PriceOf10x[2] = 6 * 1e18;
        PriceOf10x[3] = 5 * 1e18;
        PriceOf10x[4] = 10 * 1e18;
        PriceOf10x[5] = 20 * 1e18;
        PriceOf10x[6] = 30 * 1e18;
        PriceOf10x[7] = 50 * 1e18;
        PriceOf10x[8] = 100 * 1e18;
        PriceOf10x[9] = 250 * 1e18;
        PriceOf10x[10] = 500 * 1e18;
        ...
        for(uint128 i = 0; i < 8; i++) {
            ...
            uint128 fct = (PriceOf10x[i+1] * 80)/100;
    ...
    function updateTree(address _user, uint128 a, uint128 _subLevel, uint128 _parent) internal returns(bool)
    {
        ...
        for(uint i=0; i<2;i++)
        {
            ...
            else if(cc == 6) require(payAmount(_user,_parent, a, 1), "auto level 1 payment fail");
...
*/
    ...
    function processRefPay(address _user, uint128 _level) internal returns(bool)
    {
         ...
         uint prc = (PriceOf10x[_level]*20)/100;
