...
contract SpiritBeast2Gen is ERC721Upgradeable, SafeOwnable, Verifier {
    using StringsUpgradeable for uint256;
    ...
    function verifyMintParam(address account, uint256 tokenId, uint8 v, bytes32 r, bytes32 s) private returns (bool) {
    ...
    function verifyBatchMintParam(address[] calldata accounts, uint256[] calldata tokenIds, uint8 v, bytes32 r, bytes32 s) private returns (bool) {
    ...
    function batchBurn(uint256[] calldata tokenIds) external {
        ...
        for (uint i; i < tokenIds.length; i++) {
            transferFrom(msg.sender, HOLE_ADDRESS, tokenIds[i]);
        }
    }
