    ...
    TransactionFee
{
        ...
        returns (uint256 tokenAmount)
    {
        ...
        // send fee
        uint256 _amountfee = computeFee(msg.value, 18);
        require(
...
contract TransactionFee is Context {
    ...
    uint256 internal _fee = 25000000000000000; // 0,025 %;
    ...
    // @dev calculate fee per transaction
    function computeFee(uint256 amount, uint256 decimal)
        public
        view
        returns (uint256)
    {
        if (decimal == 18) {
            uint256 percent = (amount / 100000000000000000000) * _fee;
            return percent;
        } else if (decimal == 8) {
            uint256 feeTo8Decimal = _fee / 10**10;
            uint256 percent = (amount / 10000000000) * feeTo8Decimal;
            return percent;
        } else if (decimal == 6) {
            uint256 feeTo6Decimal = _fee / 10**12;
            uint256 percent = (amount / 100000000) * feeTo6Decimal;
            return percent;
        }
