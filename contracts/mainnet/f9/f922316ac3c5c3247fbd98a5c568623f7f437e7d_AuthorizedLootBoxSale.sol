    ...
    GenericTypedMessage
{
    ...
    function updateOfferSupply(
        ...
        onlyRole(ADMIN_ROLE)
    {
        require(_offerIndex < offers.length, "Unknown offer");

        offers[_offerIndex].supply = _newSupply;
    }
        ...
        whenNotPaused
    {
        ...
        require(totalSold < supply, "Global supply exhausted");
        require(offer.sold < offer.supply, "Offer supply exhausted");
        require(bought[msg.sender] < maxBoxesPerAccount, "Max boxes per account reached");
        require(_verifySignature(msg.sender, _authorization), "Invalid authorization");
        ...
        onlyRole(ADMIN_ROLE)
    {
        ...
        require(authorizedList[_buyer], "Account not authorized");
        require(totalSold < supply, "Global supply exhausted");
        require(offer.sold < offer.supply, "Offer supply exhausted");
        require(bought[_buyer] < maxBoxesPerAccount, "Max boxes per account reached");
        require(_verifySignature(_buyer, _authorization), "Invalid authorization");

        totalSold++;
        offer.sold++;
        bought[_buyer]++;

        nft.mint(_buyer, offer.boxTier);
    }
