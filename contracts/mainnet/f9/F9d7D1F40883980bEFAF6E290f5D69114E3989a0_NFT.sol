...
contract NFT is TokenBase, ERC721Enumerable {

    string public baseUri;

    event SetBaseURI(address indexed from, string oldBaseUri, string newBaseUri);

    constructor(string memory name_, string memory symbol_, address config_, string memory baseUri_) ERC721(name_, symbol_) {
    ...
    function burn(uint256 tokenId) external {
        require(ownerOf(tokenId) == msg.sender, "NFT: access denied");
        _burn(tokenId);
    }

    function setBaseURI(string memory baseUri_) public onlyAdmin {
        emit SetBaseURI(msg.sender, baseUri, baseUri_);
        baseUri = baseUri_;
    }
