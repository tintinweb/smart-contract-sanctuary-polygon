...
interface IMeshDepositor {
    function deposit(uint256 _amount, address _stakeAddress) external;
}

interface IMeshLiquidityStaking {
...
contract AdMeshLpStrategy is BaseStrategy {

    uint256 internal constant FEE_DENOMINATOR = 10000;

    address internal constant MESH_ROUTER = 0x10f4A785F458Bc144e3706575924889954946639;
    address internal constant MESH = 0x82362Ec182Db3Cf7829014Bc61E9BE8a2E82868a;
    address internal constant ERC_FUND = 0x01fE07ce760DA7a025e4Cf3f950aE236F8e62120;
    address internal constant WMATIC = 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270;
    address internal constant ADMESH = 0x791e974FD87c8d0BCAa9aa253ff4591A5957127c;

    address internal constant AD_MESH_LP = 0xCD65DC16bC99FaE3fD3028911815E2757F5eBd62;
    address internal constant MESH_DEPOSITOR = 0x459dc0fB79653A48469F2C3c375d0A522750Dd40;

    address public tokenA;
    address public tokenB;
    address public firstAddLiqToken;
    address public secondAddLiqToken;

    uint256 public buyThreshold = 990;
    ...
    constructor(
        ...
        public
        BaseStrategy(AD_MESH_LP, ERC_FUND, MESH, MESH_ROUTER, AD_MESH_LP)
    {
        multiHarvest = 0x3355743Db830Ed30FF4089DB8b18DEeb683F8546;

        tokenA = MESH;
        tokenB = ADMESH;
        firstAddLiqToken = MESH;
        secondAddLiqToken = ADMESH;

        a_b_path = [MESH, ADMESH];

        IERC20(tokenA).safeApprove(MESH_ROUTER, uint256(-1));
        IERC20(tokenB).safeApprove(MESH_ROUTER, uint256(-1));
        IERC20(MESH).safeApprove(MESH_DEPOSITOR, uint256(-1));
    }
    ...
    function balanceOfPool() public override view returns (uint256) {
        //`balanceOf` is balanceOfWant().add(balanceOfPool()), so we want to return 0 because no tokens are in another contract
        return 0;
    }
    ...
    function harvest() public override onlyHumanOrWhitelisted nonReentrant {
        ...
        if (_harvested_balance > 0) {
            uint256 feeAmount = _harvested_balance.mul(IERCFund(strategist).getFee()).div(FEE_DENOMINATOR);
            IERC20(harvestedToken).safeTransfer(strategist, feeAmount);
        }

        uint256 meshBalance = IERC20(MESH).balanceOf(address(this));
        if (meshBalance > 0) {
            (uint meshReserve, uint adMeshReserve,) = IUniswapV2Pair(AD_MESH_LP).getReserves();

            //Swap 1/2 of MESH for adMESH if threshold of less MESH in the pool than adMESH is met or less than 2 MESH was harvested
            if(meshReserve < adMeshReserve.mul(buyThreshold).div(1000) || meshBalance.div(2) < 1e18) {
                _swapUniswapWithPathPreapproved(a_b_path, meshBalance.div(2));
            }
            else {
                //Deposit
                IMeshDepositor(MESH_DEPOSITOR).deposit(meshBalance.div(2).div(1e18).mul(1e18), address(0));
            }
        }

        //Add liquidity
        uint256 aBalance = IERC20(firstAddLiqToken).balanceOf(address(this));
        uint256 bBalance = IERC20(secondAddLiqToken).balanceOf(address(this));
        if (aBalance > 0 && bBalance > 0) {
            IUniswapRouterV2(currentRouter).addLiquidity(
                firstAddLiqToken, secondAddLiqToken,
                aBalance, bBalance,
                0, 0,
                address(this),
                now + 60
    ...
    function deposit() public override {
        //do nothing since LP tokens remain in the contract
    }

    function _withdrawSome(uint256)
        internal
        override
        returns (uint256)
    {
    ...
    function _getReward() internal {
        IMeshLiquidityStaking(rewards).claimReward();
    }
