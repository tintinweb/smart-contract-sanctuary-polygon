...
// File contracts/distribution/BeeGenesisRewardPool.sol


pragma solidity 0.6.12;
// Note that this pool has no minter key of BEE (rewards).
// Instead, the governance will call BEE distributeReward method and send reward to this pool at the beginning.
contract BeeGenesisRewardPool {
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. BEE to distribute.
        uint256 lastRewardTime; // Last time that BEE distribution occurs.
        uint16  depositFeeBP; //depositfee
        uint256 accBeePerShare; // Accumulated BEE per share, times 1e18. See below.
    ...
    // The time when BEE mining starts.
    uint256 public poolStartTime;

    // The time when BEE mining ends.
    uint256 public poolEndTime;

    // TESTNET
    // uint256 public beePerSecond = 3.0555555 ether; // 11000 BEE / (1h * 60min * 60s)
    // uint256 public runningTime = 24 hours; // 1 hours
    // uint256 public constant TOTAL_REWARDS = 11000 ether;
    // END TESTNET

    // MAINNET
    uint256 public beePerSecond = 0.15046 ether; // 10000 BEE / (24h * 60min * 60s)
    ...
    function add(
        ...
        poolInfo.push(PoolInfo({token: _token, allocPoint: _allocPoint, lastRewardTime: _lastRewardTime, accBeePerShare: 0, isStarted: _isStarted, depositFeeBP: _depositFeeBP}));
        if (_isStarted) {
            totalAllocPoint = totalAllocPoint.add(_allocPoint);
        }
    }

    // Update the given pool's BEE allocation point. Can only be called by the owner.
    ...
    // View function to see pending BEE on frontend.
 ...
 function setFeeAddress(address _feeAddress) external onlyOperator {
    ...
    function governanceRecoverUnsupported(
        ...
        if (block.timestamp < poolEndTime + 90 days) {
            // do not allow to drain core token (BEE or lps) if less than 90 days after pool ends
