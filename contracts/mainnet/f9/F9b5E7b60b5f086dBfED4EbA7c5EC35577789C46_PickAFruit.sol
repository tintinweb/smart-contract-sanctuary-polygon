...
/// @title Pick a Fruit
/// @author nifty-nft.nl for Maze de Boer
contract PickAFruit is ERC721, ERC721Enumerable, ContextMixin, NativeMetaTransaction, AccessControlMixin, IERC2981 {
  ...
  string _backgroundUnwrapped = "333333";
  string _description = "Pick a Fruit";

  string private _contractURI;

  address public beneficiary;

  event Mint(uint256 tokenId, string fruit);

  /// @dev Create the contract
  constructor() ERC721("Pick a Fruit", "PAF") {
    _initializeEIP712("Pick a Fruit");

    _grantRole(DEFAULT_ADMIN_ROLE, msg.sender);
    _grantRole(MINTER_ROLE, msg.sender);
    _grantRole(MINTER_ROLE, address(0x02657e3A709077d8116e8905B2c02e1C7968707d));
  ...
  /// @dev Mint new token
  function mint(address _to, string calldata _key) external onlyRole(MINTER_ROLE) {
    _tokenIdCounter.increment();
    uint256 _tokenId = _tokenIdCounter.current();
    _tokens[_tokenId] = _key;
    _wrapped[_tokenId] = true;
    _safeMint(_to, _tokenId);
    emit Mint(_tokenId, _key);
  }
  ...
  /// @dev Unwrap token
  function unwrap(uint256 _tokenId) external onlyRole(MINTER_ROLE) {
  ...
  /// @dev Needed for ERC721Enumerable
  ...
  /// @dev Check support for interface
