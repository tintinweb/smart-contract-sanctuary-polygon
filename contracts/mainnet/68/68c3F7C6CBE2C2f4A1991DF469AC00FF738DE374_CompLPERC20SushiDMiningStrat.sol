...
import "./fStrategy.sol";
import "./oracle/IfOracle.sol";

contract CompLPERC20SushiDMiningStrat is fStrategy {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;

    //  -------------------- CONTRACT VARIABLES --------------------
    // Tokens and Contracts to Interface with
    IMiniChefV2 internal _stakingContract; // The staking contract that the vault will compound into
    ...
    IfOracle internal _oracle; // Price oracle to use for swaps
    ...
    constructor(
        string memory _name,
        address _vaultAddr,
        address _stakingContractAddr, // Mini chef 0x0769fd68dFb93167989C6f7254cd0D766Fb2841F
        address _stakingTokenAddr, // LP WETH ZED 0x374552804F7CA26C307c8D31f4cC0d9215C87f46
        address _rewardTokenAddr, // sushitoken 0x0b3F868E0BE5597D5DB7fEB59E1CADBb0fdDa50a
        address _rewardTokenLpAddr, // Sushi Wmatic 0x597A9bc3b24C2A578CCb3aa2c2C62C39427c6a49
        address _routerAddr, // 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506
        uint _feePercent,
        ...
        )
    {
        ...
        _stakingContract = IMiniChefV2(_stakingContractAddr);
        _stakingToken = IUniswapV2Pair(_stakingTokenAddr);
        _pairedToken = IERC20(_stakingToken.token1()); // WETH
        _stakingLpBaseToken = IERC20(_stakingToken.token0()); // ZED
        _rewardToken = IERC20(_rewardTokenAddr); // Sushi
        _rewardLpToken = IUniswapV2Pair(_rewardTokenLpAddr);
        _rewardLpBaseToken = IERC20(_rewardLpToken.token1()); // WMATIC

        _router = IUniswapV2Router02(_routerAddr); // router sushiswap 0x1b02dA8Cb0d097eB8D57A175b88c7D8b47997506
        _wmaticAddr = _router.WETH();

        _oracle = IfOracle(_oracleAddr);
    ...
    function _invest(uint _amount) internal override whenNotPaused {
        _stakingContract.deposit(57, _amount, address(this));
    ...
    function _divest(uint _amount) internal override {
        _stakingContract.withdraw(57, _amount, address(this));
        ...
        onlyRole(MANAGER_ROLE)
    {
        ...
        _stakingContract.harvest(57, address(this));
        swap.rewardsToCompound = _rewardToken.balanceOf(address(this));
        ...
        if (address(_rewardToken) == address(_pairedToken)) {
            ...
            (swap.reserveA, swap.reserveB, ) = _stakingToken.getReserves();
            swap.minAmountBaseToken = _router.getAmountOut(
                swap.rewardsToCompound.mul(50 * (10**16)).div(BIG_UNIT),
                swap.reserveB,
                swap.reserveA
            );

            // Compare exchange quote with oracle to make sure it's within our tolerance
            swap.oracleAmount = _oracle.consult(
                address(_rewardToken), // address tokenIn
                swap.rewardsToCompound.mul(50 * (10**16)).div(BIG_UNIT), // uint amountIn
                address(_stakingLpBaseToken) // address tokenOut
            );

            require(
                (swap.minAmountBaseToken >=
                    swap
                        .oracleAmount
                        .mul(BIG_UNIT.sub(_dexFee).sub(_priceSlippage))
                        .div(BIG_UNIT)) &&
                    (swap.minAmountBaseToken <=
                        swap
                            .oracleAmount
                            .mul(BIG_UNIT.sub(_dexFee).add(_priceSlippage))
                            .div(BIG_UNIT)),
                "Price out of bounds."
            );
            ...
            amounts = _router.swapExactTokensForTokens(
                swap.rewardsToCompound.mul(50 * (10**16)).div(BIG_UNIT), // amount of _pairedToken in
            ...
            _swapPath.push(address(_rewardToken)); // sushi
            _swapPath.push(address(_rewardLpBaseToken)); // wmatic
            _swapPath.push(address(_pairedToken)); // weth
            _swapPath.push(address(_stakingLpBaseToken)); // zed

            // Get the current price from the exchange
            amounts = _router.getAmountsOut(swap.rewardsToCompound, _swapPath);
            swap.minAmountBaseToken = amounts[3];
            delete amounts;

            // Compare exchange quote with oracle to make sure it's within our tolerance
            swap.oracleAmount = _oracle.consult(
                address(_rewardToken), // address tokenIn
                swap.rewardsToCompound, // uint amountIn
                address(_rewardLpBaseToken) // address tokenOut
            );
            swap.oracleAmount = _oracle.consult(
                address(_rewardLpBaseToken), // address tokenIn
                swap.oracleAmount, // uint amountIn
                address(_pairedToken) // address tokenOut
            );
            swap.oracleAmount = _oracle.consult(
                address(_pairedToken),
                swap.oracleAmount,
                address(_stakingLpBaseToken)
            );
            ...
            // Execute the swap
            amounts = _router.swapExactTokensForTokens(
                swap.rewardsToCompound, // amount of _rewardToken in
                swap.minAmountBaseToken, // minAmount _stakingLpBaseToken out
                _swapPath, // _router path
                address(this), // address to send matic to (this contract)
                block.timestamp + 20 // deadline for executing the swap
            );
            swap.desiredAmountBaseToken = _stakingLpBaseToken
            ...
            //// Swap 1/2 of the stakingLpBaseToken for the paired token
            // Get the current price from the exchange
            (swap.reserveA, swap.reserveB, ) = _stakingToken.getReserves(); // Token A is the base token in all of the Pairs and Token B is the _pairedToken
            swap.minAmountPairedToken = _router.getAmountOut(
            ...
            swap.oracleAmount = _oracle.consult(
                ...
                swap.desiredAmountBaseToken, // uint amountIn
                address(_pairedToken) // address tokenOut
            );
            require(
            ...
            _router.swapExactTokensForTokens(
                swap.desiredAmountBaseToken, // amount of _stakingLpBaseToken in
                swap.minAmountPairedToken, // minimum amonOut of _pairedToken out
                _swapPath, // _router path
                address(this), // address to send matic to (this contract)
            ...
            swap.desiredAmountPairedToken = _pairedToken.balanceOf(address(this));
        }
        ...
        emit Compounded(block.timestamp, swap.rewardsToCompound);
    }
...
abstract contract fStrategy is AccessControl, ReentrancyGuard, Pausable {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    ...
    // Tokens and Contracts to Interface with
    IfVault internal immutable _vault; // The vault contract that this strategy is deployed for, immutable
