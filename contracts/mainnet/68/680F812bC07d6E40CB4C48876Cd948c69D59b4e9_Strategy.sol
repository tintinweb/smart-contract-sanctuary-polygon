 ...
 *
 *         The plan is to lend all collateral (USDC) into Aave and borrow Matic accordingly, swap half of the borrowed
 *         amount and provide liquidity with the rest of the Matic together with the swapped amounts.
 *
 *         The rebalance is done in the following situations:
 *              - Matic goes up and collat ratio is about 70%
 *              - The ticks gets close to the lower bound of the principal position
 */
...
contract Strategy is BaseStrategy, IUniswapV3SwapCallback {
    ...
    // These are variables specific to our want-Matic pair
    IUniV3 public constant uniV3Pool = IUniV3(0xA374094527e1673A86dE625aa59517c5dE346d32);
    IUniswapHelper public uniHelper;
    ILendingHelper public lendHelper;

    IERC20 public wETH = IERC20(0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270); // Matic
    ...
    // Keeper bot
    address public stupidRobot = 0x97B62feC2E893A431c395fb06fbd3a96bF0Ac13b;
    ...
    function name() external view override returns (string memory) {
        return 'Strategy Bearish Uniswap V3 USDC-MATIC v0.4';
    }
    ...
    function _rebalance(int24 _newBaseLowerTick, int24 _newBaseUpperTick) internal {
        ...
        // We collect all of our liquidity and fees from the principal
    ...
    function _withdrawSome(uint256 _initialWantNeeded) internal {
        ...
        amountNeeded = _calcAmountNeeded(_initialWantNeeded);
        if (amountNeeded == 0) return;
        // TODO: review this freeFunds(amountNeeded) ETH from base position could be less than desired
    ...
    function uniswapV3SwapCallback(
        ...
        if (amount0Delta > 0) wETH.safeTransfer(address(uniV3Pool), uint256(amount0Delta));
        if (amount1Delta > 0) want.safeTransfer(address(uniV3Pool), uint256(amount1Delta));
        swapStorage = false;
    }
...
interface ILendingHelper {
    function getCollatRatios(address token) external view returns (uint256 ltv, uint256 liquidationThreshold);
    ...
    function balanceOfDebtToken() external view returns (uint256);
