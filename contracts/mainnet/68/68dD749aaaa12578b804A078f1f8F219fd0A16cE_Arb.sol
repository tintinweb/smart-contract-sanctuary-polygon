...
contract Arb is Ownable {
	...
	function swap(address router, address _tokenIn, address _tokenOut, uint256 _amount) private {
	 ...
	 function getAmountOutMin2(address router, address _tokenIn, address _tokenOut, uint256 _amount) public view returns (uint256) {
		address[] memory path;
		path = new address[](2);
		path[0] = _tokenIn;
		path[1] = _tokenOut;
		uint256 amountOutMins = IUniswapV2Router(router).getAmountOut(_amount, uint256(uint160(_tokenIn)), uint256(uint160(_tokenOut)));
	...
	function estimateDualDexTrade2(address _router1, address _router2, address _token1, address _token2, uint256 _amount) external view returns (uint256) {
			uint256 amtBack1 = getAmountOutMin2(_router1, _token1, _token2, _amount);
			uint256 amtBack2 = getAmountOutMin2(_router2, _token2, _token1, amtBack1);
			return amtBack2;
	}
