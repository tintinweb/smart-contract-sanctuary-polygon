...
contract m00se is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  function _safemint(uint256 _mintAmount) internal
    {
        if(WalletMint[msg.sender] < freeMint) 
        {
           ...
           require(msg.value >= (_mintAmount - freeMint) * cost,"m00nm00se: Fund not enough");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
        else
        {
           require(msg.value >= _mintAmount * cost,"m00nm00se: Fund not enough");
            WalletMint[msg.sender] += _mintAmount;
           _safeMint(msg.sender, _mintAmount);
        }
    }
