...
//     function ExecuteCustomOrder(address owner,uint256 status,uint256 swapType,address pair,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute,uint256 minAmountOut,uint256 deadline) external returns (bool filled);
// }
...
contract LimitOrders is EIP712("FBS Token", "1"){
    ...
    mapping (address => bytes32[]) public addressOrdersCancel;
    ...
    function getMessageHash(Order memory ord)
        ...
        returns (bytes32)
    {
        bytes32 LIMIT_ORDER_RFQ_TYPEHASH = keccak256(
        "Order(address owner,uint256 status,uint256 swapType,address pair,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute, uint256 minAmountOut,uint256 deadline)"
    );
    ...
    // address owner,uint256 status,uint256 swapType,address pair,address tokenIn,address tokenOut,uint256 amountIn,uint256 priceExecute,uint256 minAmountOut,uint256 deadline
    function ExecuteCustomOrder(Order memory ord, bytes memory _signature) public reentrancyGuard returns (bool result, string memory message){
        require(block.timestamp <= ord.deadline, "ERC20Permit: expired deadline");
        //msgHash
        bytes32 msgHash = getMessageHash(ord);
        //getEthMsgHash
        ...
        require(signer == ord.owner, "ERC20Permit: invalid signature");
        require(ord.status == 0, "Can't executed non-pending order");
        result = false;

        for(uint i=0; i<=addressOrdersCancel[ord.owner].length; i++){
            if(msgHash == addressOrdersCancel[ord.owner][i]) {
                return (result = false,message = "Can't execute cancelled order!!!");
            } else {
                if(execute(ord)) {
                    (result = true, message = "Execute Order Success!!!");
                } else {
                    (result = false, message = "Failed to execute order!!!");
                    }
            }
        }
    ...
    function cancelOrder(Order memory ord) external  {
        ...
        addressOrdersCancel[ord.owner].push(msgHash);
    }
