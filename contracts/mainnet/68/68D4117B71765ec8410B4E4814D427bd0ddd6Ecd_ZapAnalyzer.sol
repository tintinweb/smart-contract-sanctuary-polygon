...
interface IZapAnalyzer {
    ...
    function estimateSwapReturns(SwapReturnsParams memory params)
        external
        view
        returns (SwapReturns memory returnValues, SwapRatioLocalVars memory swapVars);
}
...
contract ZapAnalyzer is IZapAnalyzer {
        ...
        returns (SwapReturns memory returnValues, SwapRatioLocalVars memory swapVars)
    {
        ...
        if (params.liquidityPath.liquidityType == LiquidityType.V2) {
            ...
            (returnValues.swapToToken0, returnValues.swapToToken1, swapVars) = getSwapRatio(swapRatioParams);
        ...
        )
    {
        ...
        // address outputToken0 = swapRatioParams.swapPath0[swapRatioParams.swapPath0.length - 1].path[
        //     swapRatioParams.swapPath0[swapRatioParams.swapPath0.length - 1].path.length - 1
        // ];
        // address outputToken1 = swapRatioParams.swapPath1[swapRatioParams.swapPath1.length - 1].path[
        //     swapRatioParams.swapPath1[swapRatioParams.swapPath1.length - 1].path.length - 1
        // ];
    ...
    function getLPAddRatio(
        ...
        if (uniV3Factory != address(0)) {
            ...
            uint256 fullInputToken = 10**IERC20Metadata(token0 < token1 ? token0 : token1).decimals();
