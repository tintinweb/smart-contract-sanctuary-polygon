...
contract BalanceTrigger is BaseSiloAction{
    ...
    function createConfig(address[4] memory _inputs, address[4] memory _outputs, uint[4] memory _triggers) public pure returns(bytes memory configData){
        configData = abi.encode(_inputs, _outputs, _triggers);
    }

    function decodeConfig(bytes memory configData) public pure returns(address[4] memory inputs, address[4] memory outputs, uint[4] memory triggers){
        (inputs, outputs, triggers) = abi.decode(configData, (address[4], address[4], uint[4]));
    }

    function checkMaintain(bytes memory configuration) public view override returns(bool){
        uint[4] memory triggerAmounts;
        address[4] memory inputs;
        (inputs,,triggerAmounts) = abi.decode(configuration, (address[4],address[4],uint[4]));
    ...
    function validateConfig(bytes memory configData) public view override returns(bool){
        ...
        uint[4] memory triggers;
        (input,output,triggers) = abi.decode(configData, (address[4],address[4],uint[4]));
        for(uint i=0; i<4; i++){
            if(input[i] != output[i]){
                return false;
            }
