...
// File: contracts/newNFTixBooth.sol
...
contract newNFTixBooth is ERC721URIStorage {
  ...
  function checkin(uint256 tokenID) public {
    ...
    string memory json = Base64.encode(
        bytes(
            string(
                abi.encodePacked(
                    '{ "name": "NFTix #', Strings.toString(tokenID),
                    '", "description": "A NFT powered ticketing system",',
                    '"traits": [{ "trait_type": "Checked In", "value": "true" }, { "trait_type": "Purchased", "value": "false" }], ',
        ...
        bytes(
            string(
                abi.encodePacked(
                    '{ "name": "NFTix #', Strings.toString(tokenID),
                    '", "description": "A NFT powered ticketing system",',
                    '"traits": [{ "trait_type": "Checked In", "value": "OUT" }, { "trait_type": "Purchased", "value": "false" }], ',
                    '"image": "data:image/svg+xml;base64,',
                    encodedImage,
                    '" }'
                )
 ...
 string[] public colori = ["White","Black","Red","Blue","Green","Yellow","Orange","Purple", "Pink","Grey"];

    function getData(uint randomNumber) public returns(string[] memory){

        uint toInsert;
        string[] memory myData = new string[](8);
        uint j = 7;
        randomNumber = randomNumber - 100000000;

        for(uint i = 0; i <8; i++){
            toInsert = randomNumber/10**j;
            myData[i] = colori[toInsert%10];
            if(j>0){
                j--;
            }
        }
  ...
  function mint() public {
    ...
    string[3] memory svg;
    string[] memory colorix = getData(198653274);
        ...
        bytes(
            string(
                abi.encodePacked(
                    ...
                    '", "description": "A NFT powered ticketing system",',
                    '"traits": [{ "trait_type": "Checked In", "value": "false" }, { "trait_type": "Purchased", "value": "false" }], ',
                    '"image": "data:image/svg+xml;base64,',
                    encodedImage,
                    '" }'
                )
