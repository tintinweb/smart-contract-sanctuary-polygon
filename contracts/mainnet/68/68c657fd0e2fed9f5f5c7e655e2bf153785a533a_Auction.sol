...
contract Auction is IAuction, Ownable, NFTAssist, ReentrancyGuard {
    ...
    function lots(uint256 offset, uint256 limit) external view returns (Lot[] memory) {
        require(offset < _lots.length, 'Not existed lotId');

        Lot[] memory lots_ = new Lot[](limit);
        for (uint256 i = 0; i < limit; i++) {
            if (i + offset >= _lots.length) break;
            lots_[i] = _lots[i + offset];
        }
        return lots_;
    }
    ...
    function userLots(
        ...
        require(_userLots[user].length > offset, 'Not existed element');
    ...
    function userBids(
        ...
        require(_userBids[user].length > offset, 'Not existed element');
