...
contract DATA is ERC20Permit {
    ...
    */
    constructor(
        ...
        _mint(registry.getEntry(Constants.GAME_TREASURY), 2_400_000_000 * 10**decimals());
        _mint(registry.getEntry(Constants.COMMUNITY_TREASURY), 600_000_000 * 10**decimals());
        _mint(registry.getEntry(Constants.VESTING), 600_000_000 * 10**decimals());
        _mint(Constants.LIQUIDITY_PROVIDER, 400_000_000 * 10**decimals());

        registry.register(Constants.DATA, nonce_, signature_);
        _approve(registry.getEntry(Constants.GAME_TREASURY), registry.getEntry(Constants.AIRDROPS), Constants.UINT256_MAX);
        _approve(registry.getEntry(Constants.COMMUNITY_TREASURY), registry.getEntry(Constants.AIRDROPS), Constants.UINT256_MAX);
    }
}
