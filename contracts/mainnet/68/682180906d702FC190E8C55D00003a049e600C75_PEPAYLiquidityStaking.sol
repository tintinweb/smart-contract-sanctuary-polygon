...
contract PEPAYFeeSplitter is Ownable, ReentrancyGuard {
    ...
    address private LP_WALLET;
    address private STAKER_WALLET;
    address private OP_WALLET;

    uint16 public LP_FEE = 600;
    uint16 public STAKER_FEE = 200;
    uint16 public OP_FEE = 100;
    uint16 public TEAM_FEE = 100;
    uint16 public constant FEE_BASE_1000 = 1000;

    uint256 public constant MIN = 100 * 10 ** 18;
    ...
    function splitFees() external nonReentrant {
        ...
        uint256 teamAmount = (contractBalance * TEAM_FEE) / FEE_BASE_1000;
        uint256 lpAmount = (contractBalance * LP_FEE) / FEE_BASE_1000;
        uint256 stakerAmount = (contractBalance * STAKER_FEE) / FEE_BASE_1000;
        uint256 opAmount = (contractBalance * OP_FEE) / FEE_BASE_1000;
        if (contractBalance > MIN) {
            sendToken(TEAM_WALLET, teamAmount);
            sendToken(LP_WALLET, lpAmount);
            sendToken(STAKER_WALLET, stakerAmount);
            sendToken(OP_WALLET, opAmount);
        }
    }
...
import {PEPAYFeeSplitter} from "./PEPAYFeeSplitter.sol";

contract PEPAYLiquidityStaking is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    PEPAYFeeSplitter FEE_ADDRESS;
    ...
    constructor(address _liquidityToken, address _rewardToken) {
    ...
    function sendToken(uint256 _value) private {
    ...
    function getReward(address _address) public view returns (uint256) {
        if (totalAmountStaked > 0) {
            uint256 contractBalance = rewardToken.balanceOf(address(this));
            uint256 feeAddressBalance = rewardToken.balanceOf(
                address(FEE_ADDRESS)
            );
            uint256 _rewardPerTokenStored = rewardPerTokenStored +
                ((contractBalance + feeAddressBalance - previusRewardTotal) *
                    (FACTOR)) /
                (totalAmountStaked);
    ...
    function getRewardPerTokenStored() public view returns (uint256) {
        uint256 contractBalance = rewardToken.balanceOf(address(this));
        uint256 feeAddressBalance = rewardToken.balanceOf(address(FEE_ADDRESS));
        if (totalAmountStaked > 0)
            return
                rewardPerTokenStored +
                ((contractBalance +
