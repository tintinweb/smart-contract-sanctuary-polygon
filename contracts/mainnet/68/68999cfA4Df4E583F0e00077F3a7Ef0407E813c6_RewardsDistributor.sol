...
contract RewardsDistributor is IRewardsDistributor, Ownable {
    ...
    uint[1000000000000000] public cnt_supply;
    ...
    function checkpoint_token() external onlyOwner {
        _checkpoint_token();
    }

    function _find_timestamp_epoch(address cnt, uint _timestamp) internal view returns (uint) {
        uint _min = 0;
        uint _max = IVotingEscrow(cnt).epoch();
        for (uint i = 0; i < 128; i++) {
            if (_min >= _max) break;
            uint _mid = (_min + _max + 2) / 2;
            IVotingEscrow.Point memory pt = IVotingEscrow(cnt).point_history(_mid);
            if (pt.ts <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    function _find_timestamp_user_epoch(
        ...
        for (uint i = 0; i < 128; i++) {
            if (_min >= _max) break;
            uint _mid = (_min + _max + 2) / 2;
            IVotingEscrow.Point memory pt = IVotingEscrow(cnt).user_point_history(tokenId, _mid);
            if (pt.ts <= _timestamp) {
                _min = _mid;
            } else {
                _max = _mid - 1;
    ...
    function cnt_for_at(uint _tokenId, uint _timestamp) external view returns (uint) {
        address cnt = voting_escrow;
        uint max_user_epoch = IVotingEscrow(cnt).user_point_epoch(_tokenId);
        uint epoch = _find_timestamp_user_epoch(cnt, _tokenId, _timestamp, max_user_epoch);
        IVotingEscrow.Point memory pt = IVotingEscrow(cnt).user_point_history(_tokenId, epoch);
    ...
    function _checkpoint_total_supply() internal {
        ...
        IVotingEscrow(cnt).checkpoint();

        for (uint i = 0; i < 20; i++) {
            if (t > rounded_timestamp) {
                break;
            } else {
                uint epoch = _find_timestamp_epoch(cnt, t);
                IVotingEscrow.Point memory pt = IVotingEscrow(cnt).point_history(epoch);
                int128 dt = 0;
                if (t > pt.ts) {
                    dt = int128(int256(t - pt.ts));
                }
                cnt_supply[t] = Math.max(uint(int256(pt.bias - pt.slope * dt)), 0);
            }
            t += WEEK;
        }
        time_cursor = t;
    ...
    function _claim(uint _tokenId, address cnt, uint _last_token_time) internal returns (uint) {
        uint user_epoch = 0;
        ...
        if (week_cursor == 0) {
            user_epoch = _find_timestamp_user_epoch(cnt, _tokenId, _start_time, max_user_epoch);
        } else {
        ...
        IVotingEscrow.Point memory user_point = IVotingEscrow(cnt).user_point_history(
            _tokenId,
            user_epoch
        );
        ...
        for (uint i = 0; i < 50; i++) {
            ...
            if (week_cursor >= user_point.ts && user_epoch <= max_user_epoch) {
                ...
                if (user_epoch > max_user_epoch) {
                    ...
                    user_point = IVotingEscrow(cnt).user_point_history(_tokenId, user_epoch);
                }
            } else {
