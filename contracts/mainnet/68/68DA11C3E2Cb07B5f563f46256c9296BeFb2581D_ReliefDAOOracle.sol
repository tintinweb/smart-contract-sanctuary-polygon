...
// File: GenericOracle.sol

/**
 * @title Generic Oracle
 * @notice stores arbitrary key-value pairs for a topic.
 * @notice like the oracles of antiquity, it is up to the petitioner to 
 *     interpret the results.
 * @dev Use a subclass. This contract has no public functions for retrieving 
 *     the data.
 */
contract GenericOracle is BaseViciContract {
    using EnumerableSet for EnumerableSet.Bytes32Set;
    using EnumerableUint256Set for EnumerableUint256Set.Uint256Set;

    bytes32 public constant ORACLE = keccak256(bytes("oracle"));

    EnumerableUint256Set.Uint256Set allTopics;
    EnumerableSet.Bytes32Set allKeys;

    mapping(uint256 => mapping(bytes32 => bytes32)) database;

    /* Enumerating functions */

    /**
     * @notice Returns the number of keys.
     * @notice use with keyAtIndex() to enumerate
     */
    function keyCount() public view returns (uint256) {
        return allKeys.length();
    }

    /**
     * @notice Returns the key at the index.
     * @notice use with keyCount() to enumerate
     */
    ...
    /**
     * @notice Returns the number of topics.
     * @notice use with topicAtIndex() to enumerate
     */
    function topicCount() public view returns (uint256) {
        return allTopics.length();
    }

    /**
     * @notice Returns the topic at the index.
     * @notice use with topicCount() to enumerate
     */
    function topicAtIndex(uint256 index) public view returns (uint256) {
        return allTopics.at(index);
    }

    /* Maintenance functions */

    /**
     * @notice adds the keys to the publically enumerable set
     * 
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role.
     */
    function defineKeys(bytes32[] memory keys) public onlyOwnerOrRole(ORACLE) {
    ...
    /**
     * @notice sets the data for the topic.
     * @notice Use this to insert a new topic or update several things about a
     *     topic at once.
     * @notice adds the topic to the publicly enumerable set if new.
     * @notice overwrites data for previously set keys.
     * @notice does not clear data for keys not included in the call.
     * 
     * @param topic identifies the topic
     * @param keys the list of keys to set
     * @param values the corresponding list of values to set
     * 
     * Requirements:
    ...
    ) public onlyOwnerOrRole(ORACLE) {
        require(
            keys.length == values.length,
            "keys and values lengths mismatch"
        );

        allTopics.add(topic);
        for (uint256 i = 0; i < keys.length; i++) {
            database[topic][keys[i]] = values[i];
        }
    }

    /**
     * @notice sets values of a key for a set of topics
     * @notice use this to push updates to lots of topics at once.
     * @notice does not add any missing topics to the enumerable list.
     * @notice adds the key to the enumerable list if missing.
     * 
     * @param topics identifies the topics
     * @param key the key to set
     * @param values the corresponding list of values to set
     * 
     * Requirements:
     * - `topics` and `values` MUST have the same length.
     * - caller MUST be the contract owner or have the ORACLE role.
     */
    function setValues(
        ...
        require(
            topics.length == values.length,
            "topics and values lengths mismatch"
        );

        allKeys.add(key);

        for (uint256 i = 0; i < topics.length; i++) {
            database[topics[i]][key] = values[i];
        }
    }

    /**
     * @notice sets or overwrites a specific value for a topic.
     * @notice use this to update a single value.
     * @notice does not add the topic to the enumerable list if missing.
     * 
     * @param topic identifies the topic
     * @param key the key to set
    ...
    /**
     * @dev returns one piece of data about a topic
     * @param topic topic identifier
     * @param key the data point
     */
    function _lookupData(uint256 topic, bytes32 key)
        internal
        view
        returns (bytes32)
    {
        return database[topic][key];
    }

    /**
     * @dev returns the requested data about a topic
     * @param topic topic identifier
     * @param keys the list of data point
     */
    function _lookupDataset(uint256 topic, bytes32[] memory keys)
        internal
        view
        returns (bytes32[] memory result)
    {
        result = new bytes32[](keys.length);
        for (uint256 i = 0; i < keys.length; i++) {
            result[i] = database[topic][keys[i]];
        }
    }
}

// File: PublicOracle.sol

/**
 * @title Public Oracle
 * @dev subclass of the GenericOracle with public lookup functions.
 */
contract PublicOracle is GenericOracle {
    /**
     * @dev returns one piece of data about a topic
     * @param topic topic identifier
     * @param key the data point
     */
    function lookupData(uint256 topic, bytes32 key)
        public
        view
        returns (bytes32)
    {
        return _lookupData(topic, key);
    }

    /**
     * @notice returns the requested data about a topic
     * @param topic topic identifier
     * @param keys the list of data point
     */
    function lookupDataset(uint topic, bytes32[] memory keys)
        public
        view
        returns (bytes32[] memory)
    {
        return _lookupDataset(topic, keys);
    }
}

// File: ReliefDAOOracle.sol

/**
 * @title Relief Dao Oracle
 * @dev Stores information about the nominated candidates, votes, and money
 * raised.
 */
contract ReliefDAOOracle is PublicOracle, IReliefDaoOracle {
    using EnumerableUint256Set for EnumerableUint256Set.Uint256Set;

    bytes32 public constant NOM_KEY = keccak256(bytes("nominated"));
    bytes32 public constant VOTE_KEY = keccak256(bytes("voted"));

    mapping(uint256 => CandidateInfo) candidates;
    EnumerableUint256Set.Uint256Set candidateIds;

    string public goal;
    string public progress = "$0";

    bool public votingOpen = false;

    /**
     * @dev returns the number of candidates that have been registered.
     * @dev use with `getCandidateAtIndex()` to iterate.
     */
    ...
    /**
     * @dev returns the candidate at the index.
     * @dev use with `getCandidateCount()` to iterate.
     */
    function getCandidateAtIndex(uint256 index)
        public
        view
        returns (CandidateInfo memory)
    {
        return candidates[candidateIds.at(index)];
    }

    /**
     * @dev returns the candidate for an id.
     * @param candidateId the primary key from the offchain database.
     */
    function getCandidateById(uint256 candidateId)
        public
        view
    ...
    /**
     * @dev register a new candidate or update an existing one.
     * @dev if `candidate.candidateId` matches a candidate that has already
     * been registered, it will be updated with the new data.
     * @param candidate the candidate data
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - the `candidate.candidateId` MAY refer to an existing candidate. In
     *     this case, the name and location will be updated, but the votes and
     *     amounts raised will not be updated.
     */
    function addCandidate(CandidateInfo memory candidate)
        public
        onlyOwnerOrRole(ORACLE)
    {
        _addCandidate(candidate);
    }

    /**
     * @dev register and/or update a batch of candidates.
     * @dev if any `candidateId` matches a candidate that has already been
     * registered, it will be updated with the new data.
     * @param newCandidates the candidate data
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - the `candidateId` for any element MAY refer to an existing
     *     candidate. In this case, the name and location will be updated, but
     *     the votes and amounts raised will not be updated.
     */
    function addCandidates(CandidateInfo[] memory newCandidates)
        public
        onlyOwnerOrRole(ORACLE)
    {
        for (uint256 i = 0; i < newCandidates.length; i++) {
            _addCandidate(newCandidates[i]);
        }
    }

    /**
     * @dev unregister a candidate
     * @param candidateId the primary key from the offchain database.
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
    ...
    /**
     * @dev record a vote for a candidate
     * @param tokenId the NFT casting the vote
     * @param candidateId the primary key from the offchain database.
     * @param amountRaised a formatted dollar amount as a string, encoded as
     *     bytes32, representing the current total amount raised for the
     *     referenced candidate.
     * @param updatedProgress a formatted dollar amount as a string
     *     representing the current total amount raised.
     * @param voteType either NOM_KEY or VOTE_KEY indicating whether this
     *     vote is a nomination or a final vote.
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `candidateId` MUST refer to a registered candidate
     * - `amountRaised` MAY be bytes32(0), in which case the value is not
     *     updated for the candidate.
     * - `updatedProgress` MAY be an empty string, in which case the value is
     *     not updated.
     * - This function MAY be called for a `tokenId` that has already recorded
     *     a vote of `voteType`. In this case, the vote is removed from the
     *     previous candidate and added to the new one.
     */
    function recordVoteAndProgress(
        uint256 tokenId,
        uint256 candidateId,
        bytes32 amountRaised,
        string memory updatedProgress,
        bytes32 voteType
    ) public onlyOwnerOrRole(ORACLE) {
        _verifyCandidate(candidateId);
        CandidateInfo storage candidate = candidates[candidateId];
        _recordVoteAndProgress(tokenId, candidate, amountRaised, voteType);
        _setProgress(updatedProgress);
    }

    /**
     * @dev record batch of votes for a candidate
     * @param _tokenIds the NFT casting the vote
     * @param _candidateIds the primary key from the offchain database.
     * @param _amountsRaised a formatted dollar amount as a string, encoded as
     *     bytes32, representing the current total amount raised for the
     *     referenced candidate.
     * @param _updatedProgress a formatted dollar amount as a string
     *     representing the current total amount raised.
     * @param _voteType either NOM_KEY or VOTE_KEY indicating whether this
     *     vote is a nomination or a final vote.
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `_tokenIds` and `_candidateIds` MUST be the same length
     * - `_amountsRaised` MUST either be an empty array or be the same length
     *     as `_tokenIds`
     * - Every element of `candidateIds` MUST refer to a registered candidate
     * - `_amountsRaised` MAY be an empty array, in which case the values are
     *     not updated for any candidate.
     * - Elements of `amountRaised` MAY be bytes32(0), in which case the value
     *     is not updated for the candidate at the same index.
     * - `updatedProgress` MAY be an empty string, in which case the value is
     *     not updated.
     * - This function MAY be called for`tokenIds` that have already recorded
     *     a vote of `voteType`. In this case, the vote is removed from the
     *     previous candidate and added to the new one.
     */
    function recordVotesAndProgress(
        uint256[] memory _tokenIds,
        uint256[] memory _candidateIds,
        bytes32[] memory _amountsRaised,
        string memory _updatedProgress,
        bytes32 _voteType
    ) public onlyOwnerOrRole(ORACLE) {
        require(
            _tokenIds.length == _candidateIds.length,
            "array length mismatch"
        );
        require(
            _amountsRaised.length == 0 ||
                _tokenIds.length == _amountsRaised.length,
            "array length mismatch"
        );
        for (uint256 i = 0; i < _candidateIds.length; i++) {
            _verifyCandidate(_candidateIds[i]);
            CandidateInfo storage candidate = candidates[_candidateIds[i]];

            if (_amountsRaised.length == 0) {
                _recordVote(_tokenIds[i], candidate, _voteType);
            } else {
                _recordVoteAndProgress(
                    _tokenIds[i],
                    candidate,
                    _amountsRaised[i],
                    _voteType
        ...
        _setProgress(_updatedProgress);
    }

    /**
     * @dev update fundraising totals without altering vote count.
     * @param _candidateIds the primary key from the offchain database.
     * @param _amountsRaised a formatted dollar amount as a string, encoded as
     *     bytes32, representing the current total amount raised for the
     *     referenced candidate.
     ...
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     * - `_amountsRaised` and `_candidateIds` MUST be the same length
     * - Every element of `candidateIds` MUST refer to a registered candidate
     * - Elements of `amountRaised` MAY be bytes32(0), in which case the value
     *     is not updated for the candidate at the same index.
     * - `updatedProgress` MAY be an empty string, in which case the value is
     *     not updated.
     */
    function updateFundraisingTotals(
        uint256[] memory _candidateIds,
        bytes32[] memory _amountsRaised,
        string memory _updatedProgress
    ) public onlyOwnerOrRole(ORACLE) {
        require(
            _amountsRaised.length == 0 ||
                _candidateIds.length == _amountsRaised.length,
            "array length mismatch"
        );

        for (uint256 i = 0; i < _candidateIds.length; i++) {
            _verifyCandidate(_candidateIds[i]);
            _setAmountRaised(candidates[_candidateIds[i]], _amountsRaised[i]);
        }
        _setProgress(_updatedProgress);
    }

    /**
     * @dev sets the vote totals to 0 for all candidates.
     * @dev you would want to do this when switching from one type of voting to
     *     another.
     *
     * Requirements:
     * - caller MUST be the contract owner or have the ORACLE role
     */
    function clearVotes() public onlyOwnerOrRole(ORACLE) {
        for (uint256 i = 0; i < candidateIds.length(); i++) {
            candidates[candidateIds.at(i)].votes = 0;
        }
    }

    /**
     * @dev returns the candidate struct nominated by `tokenId`.
     */
    function candidateNominatedBy(uint256 tokenId)
        public
        view
        returns (CandidateInfo memory)
    {
        return _candidateByKey(tokenId, NOM_KEY);
    }

    /**
     * @dev returns the candidate struct voted by `tokenId`.
     */
    function candidateVotedBy(uint256 tokenId)
        public
        view
        returns (CandidateInfo memory)
    {
        return _candidateByKey(tokenId, VOTE_KEY);
    }

    /**
     * @dev sets the fundraising goal.
     * @param updatedGoal a formatted dollar amount as a string representing
     *     the goal amount to raise.
     */
    function setGoal(string memory updatedGoal) public onlyOwnerOrRole(ORACLE) {
        goal = updatedGoal;
    }
     ...
     *     representing the current total amount raised.
     */
    function setProgress(string memory updatedProgress)
        public
        onlyOwnerOrRole(ORACLE)
    {
        _setProgress(updatedProgress);
    ...
    function _addCandidate(CandidateInfo memory candidate) internal {
    ...
    function _updateCandidate(CandidateInfo memory candidate) internal {
        CandidateInfo storage existing = candidates[candidate.candidateId];
        existing.candidateId = candidate.candidateId;
        existing.candidateName = candidate.candidateName;
        existing.candidateName2 = candidate.candidateName2;
        existing.location = candidate.location;
    }

    function _verifyCandidate(uint256 candidateId) internal view {
        require(
            candidateId != 0 && candidateIds.contains(candidateId),
            "invalid candidate"
        );
    }
    ...
    function _recordVote(
        uint256 tokenId,
        CandidateInfo storage candidate,
        bytes32 voteType
    ) internal {
        uint256 existingVote = uint256(lookupData(tokenId, voteType));
        if (existingVote != 0 && candidateIds.contains(existingVote)) {
            CandidateInfo storage r = candidates[existingVote];
            if (r.votes > 0) {
                r.votes -= 1;
            }
        }
        ...
        candidate.votes += 1;
        update(tokenId, voteType, bytes32(candidate.candidateId));
    }

    function _recordVoteAndProgress(
        uint256 tokenId,
        CandidateInfo storage candidate,
        bytes32 amountRaised,
        bytes32 voteType
    ) internal {
        _recordVote(tokenId, candidate, voteType);
        _setAmountRaised(candidate, amountRaised);
    }

    function _setAmountRaised(
        CandidateInfo storage candidate,
        bytes32 amountRaised
    ) internal {
        if (amountRaised != bytes32(0)) {
            candidate.amountRaised = amountRaised;
        }
    }

    function _setProgress(string memory updatedProgress) internal {
        if (bytes(updatedProgress).length != 0) {
            progress = updatedProgress;
        }
    }

    function _candidateByKey(uint256 tokenId, bytes32 key)
        internal
        view
        returns (CandidateInfo memory)
    {
        uint256 candidateId = uint256(lookupData(tokenId, key));
