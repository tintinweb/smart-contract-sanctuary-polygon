 ...
 */
contract NomisScore is ERC721, ERC721URIStorage, ERC721Burnable, Ownable, Pausable, EIP712 {
    ...
    */
     ...
     */
    mapping (address => mapping (uint256 => mapping (uint16 => Score))) scores;

    /**
     * @dev A mapping of token id to calculation model.
     */
    mapping(uint256 => uint16) public TokenCalcModels;
     ...
     */
    event ScoreChanged(uint256 indexed tokenId, address indexed owner, uint16 score, uint16 calculationModel, uint256 chainId);
    ...
    */
    constructor(uint256 initialFee, uint16 initialCalcModelsCount, uint16 initialfreeMintCount) ERC721("NomisScore", "NMSS") EIP712("NMSS", "0.7") {
        _tokenIds.reset();
        _mintFee = initialFee;
    ...
    function setScore(
        ...
        require(calculationModel < _calcModelsCount, "setScore: calculationModel should be less than calculation model count");

        // Verify the signer of the message
        bytes32 messageHash = _hashTypedDataV4(keccak256(abi.encode(
            keccak256("SetScoreMessage(uint16 score,uint16 calculationModel,address to,uint256 nonce,uint256 deadline,bytes32 metadataUrl,uint256 chainId)"),
            score,
            calculationModel,
            msg.sender,
            nonces[msg.sender],
        ...
        Score storage scoreStruct = scores[msg.sender][chainId][calculationModel];
        ...
        _setTokenURI(scoreStruct.tokenId, metadataUrl);
        TokenCalcModels[scoreStruct.tokenId] = calculationModel;
        TokenChains[scoreStruct.tokenId] = chainId;

        emit ScoreChanged(scoreStruct.tokenId, msg.sender, score, calculationModel, chainId);
    }
    ...
    */
    function getScore(address addr, uint256 blockchainId, uint16 calcModel) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId, address owner) {
        Score storage _score = scores[addr][blockchainId][calcModel];
    ...
    * @return owner The score owner.
    */
    function getScoreByTokenId(uint256 id) external view returns(uint16 score, uint256 updated, uint256 tokenId, uint16 calculationModel, uint256 chainId, address owner) {
        address scoreOwner = ownerOf(id);
        calculationModel = TokenCalcModels[id];
        chainId = TokenChains[id];

        Score storage _score = scores[scoreOwner][chainId][calculationModel];
    ...
    function _afterTokenTransfer(address from, address to, uint256 tokenId, uint256 batchSize) internal override (ERC721) {
        if (to == address(0))
        {
            for (uint16 i = 0; i < _calcModelsCount; i++) {
                for (uint256 j = 0; j < 9999999999999; j++) {
                    if (scores[from][j][i].tokenId == tokenId) {
                        delete scores[from][j][i];
                    }
                }
            }

            delete TokenCalcModels[tokenId];
