...
contract MissionControl is IMissionControl, OwnableUpgradeable, EIP712Upgradeable {
    ...
    mapping(address => mapping(int256 => mapping(int256 => TileElement[3]))) tiles;
    ...
    function placeNFT(PlaceOrder memory _placeOrder) external override {
        ...
        require(
            ...
            "MISSION_CONTROL: NO TILE CONTRACT STAKED"
        );
    ...
    /// @inheritdoc IMissionControl
    function removeNFTs(CollectOrder[] memory orders) external override {
        for (uint256 i; i < orders.length; i++) {
            __removeNFT(orders[i], msg.sender);
        }
    }
    ...
    function createRentTiles(
        ...
        require(_flowRate == int96(uint96(requiredFlow)), "MISSION_CONTROL: INVALID FLOW");
    }
    ...
    function updateRentTiles(
        ...
        for (uint256 i; i < vars.removeTileLength; i++) {
            ...
            require(
                tileRentalInfo[_renter][vars.removeTileItem.x][vars.removeTileItem.y].isRented == true,
                "MISSION_CONTROL: TILE IS NOT RENTED"
            );
            require(
                tileRentalInfo[_renter][vars.removeTileItem.x][vars.removeTileItem.y].rentalToken == _supertoken,
                "MISSION_CONTROL: WRONG TOKEN"
            );
        ...
        require(_flowRate == _oldFlowRate + vars.requiredFlow, "MISSION_CONTROL: INVALID FLOW");
    }

    // user stop streaming to the game
    function deleteRentTiles(address _supertoken, address _renter) external {
        require(
            msg.sender == streamer || msg.sender == address(crosschainServices),
            "MISSION_CONTROL: SENDER NOT STREAMER CONTRACT"
        );
    ...
    function toggleTile(
        ...
        require(msg.sender == tileContractLockProvider, "MISSION_CONTROL: NOT TC LOCK PROVIDER");
        TileRentalInfo storage tile = tileRentalInfo[_renter][_x][_y];
        uint256 timestamp = tile.pausedAt;

        if (_shouldPause == false) {
            TileElement[3] memory tileElements = tiles[_renter][_x][_y];
            TileElement memory tileElement = tileElements[1].nonce != 0 ? tileElements[1] : tileElements[0];
            if (tileElement.nonce != 0 && timestamp != 0) {
                if (rad == 1) IMissionControlStakeable(tileElement.stakeable).onResume(_renter, tileElement.nonce);
                if (rad > 1 && tile.isRented)
                    IMissionControlStakeable(tileElement.stakeable).onResume(_renter, tileElement.nonce);
            }
            if (rad == 1) tile.pausedAt = 0;
            if (rad > 1 && tile.isRented) tile.pausedAt = 0;
        } else {
            tile.pausedAt = block.timestamp;
        }
    }
    ...
    function _handleTileRentals(
        ...
        for (; i < length; i++) {
            ...
            if (_tileRentalInfo.pausedAt != 0) {
                ...
                if (tile.nonce != 0) IMissionControlStakeable(tile.stakeable).onResume(_renter, tile.nonce);
    ...
    {
        TileElement memory tile = tiles[args.user][args.x][args.y][__zeroOrOne(args.x, args.y, args.user)];
        return
        ...
        validCoordinates_noMaxRad(order.x, order.y, order.z)
    {
        ...
        vars.zeroOrOne = __zeroOrOne(order.x, order.y, _renter);
        vars.tokenId = tileElements[vars.zeroOrOne].tokenId;
        vars.stakeable = tileElements[vars.zeroOrOne].stakeable;
        _handleReturn(tileElements[vars.zeroOrOne], _renter);
        if (vars.zeroOrOne == 1) {
        ...
        emit NFTRemoved(
            _renter,
            vars.stakeable,
            vars.zeroOrOne == 1 ? tileElements[0].stakeable : address(0),
            vars.tokenId,
            vars.zeroOrOne == 1 ? tileElements[0].tokenId : 0,
            order.x,
            order.y
        );
    }
    ...
    function _handle_collect(HandleCollectInputs memory args)
        private
        validCoordinates(args.x, args.y, args.z)
        returns (uint256, uint256)
    {
        ...
        vars.tileElement = tiles[msg.sender][args.x][args.y][__zeroOrOne(args.x, args.y, msg.sender)];
        ...
        for (int256 x = -v.radius; x <= v.radius; x++) {
            for (int256 y = -v.radius; y <= v.radius; y++) {
                for (int256 z = -v.radius; z <= v.radius; z++) {
                    ...
                    uint256 rad = (abs(x) + abs(y) + abs(z)) / 2;
                    if (rad <= allowedRadius && rad > 0 && sum == 0) {
                        TileElement[3] memory tileElements = tiles[_renter][x][y];
                        TileElement memory tile = tileElements[1].nonce != 0 ? tileElements[1] : tileElements[0];
