...
*/
    ...
    function SaleList2(address c)public view returns(uint256[] memory)
    {
        string memory s;
        bytes memory b;
        ...
        uint256[] memory out = new uint256[](1 + l*12);
        out[nn++] = l;
	out[nn++] = UpdateTime;
        if(l>0)
        {
            ...
            {
                out[nn++] = Sale[i].id;
                out[nn++] = Sale[i].exists?1:0;
                out[nn++] = Sale[i].enabled?1:0;
                out[nn++] = Sale[i].test_mode?1:0;
                s = Sale[i].name;
                b = bytes(s);
                ...
                out[nn++] = Sale[i].cap;
                out[nn++] = Sale[i].amount[1];
                out[nn++] = Sale[i].amount[2];
                out[nn++] = Sale[i].amount[3];

                out[nn++] = uint256(uint160(Sale[i].tokens[0]));
            }
        }
        return out;
    }
