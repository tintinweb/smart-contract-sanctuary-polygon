...
contract FusionTokenRoyalty is ERC721, Ownable {
    ...
    function createItem( string memory uri, address creator, address royaltyAddress, uint96 royaltyPercentage) public onlyOwner returns (uint256) {
        _tokenIds.increment();
    ...
    function createItems(string[] memory tokenUris, address creator, address royaltyAddress, uint96 royaltyPercentage) public onlyOwner returns (uint256[] memory) {
        ...
        emit itemsCreated(newItems, tokenUris, creator, royaltyAddress, royaltyPercentage);
        return newItems;
    }
