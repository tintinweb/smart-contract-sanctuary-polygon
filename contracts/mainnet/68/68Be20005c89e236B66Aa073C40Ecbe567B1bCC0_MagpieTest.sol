...
contract MagpieTest is ReentrancyGuard, Ownable, IMagpieTest {
    ...
    modifier onlyMagpieSimulator() {
        require(msg.sender == magpieSimulatorAddress, "Only MagpieSimulator allowed");
        _;
    }
    ...
    function swap(bytes32[][] calldata hops, bytes32[] calldata params) external payable nonReentrant override returns (uint256 amountOut) {
        ...
        for (s.i; s.i < s.hopsLength; s.i++) {
            bytes memory hop = hops[s.i].concat(hops[s.i][0].toBytes().toUint8(0));
            h.paramIndex = 0;
            ...
            h.payload = hop.getHopPayload(params);

            s.fromAssetAddress.approve(amms[h.ammId].ammAddress, h.amountIn);
            bytes memory data = abi.encodeWithSelector(amms[h.ammId].selector, h.payload);

            (bool success,) = address(amms[h.ammId].ammAddress).staticcall(data);
        ...
        if (msg.sender != magpieSimulatorAddress) {
...
library LibBytes {
    ...
    function getHopPayload(bytes memory hop, bytes32[] memory params) internal pure returns (bytes memory) {
        uint256 hopLength = hop.length;
        bytes memory payload;

        if (hopLength-7 != params.length * 32) {
            revert InvalidHopParams();
        }
        ...
        assembly {
            ...
            }
            {
                mstore(
                    ...
                    mload(add(
                        paramValues, 
                        /* Param index: 2 bytes shifted to right */ mul(shr(240, mload(add(hopDefinition, 1 /* size of paramEnd */))), 32)
                    ))
                )
