...
contract IsContractBulkChecker {
  ...
  function callDetectTransferRestriction(address _contractAddress) public view returns (bool) {
    bytes4 selector = bytes4(keccak256("onERC1155Received(address,address,uint256,uint256,bytes)"));
    ...
    return bytes4(data) == selector;
  }

  function canReceiveNft(address acc) internal returns (bool) {
    if (callDetectTransferRestriction(acc)) {
      address dummyAddress = 0xFB37c2FCca2F8b61429247844ADEe1BB8f9FF224;
