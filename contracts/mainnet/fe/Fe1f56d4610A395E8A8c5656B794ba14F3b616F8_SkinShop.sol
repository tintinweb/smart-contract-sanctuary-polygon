...
contract SkinShop is OwnableUpgradeable {
    ...
    function setLevelRate(uint[] memory lists) external onlyOwner {
        levelRate = lists;
    }
    ...
    function mintRandomSkin() internal {
        uint out = rand(100);
        if (out >= 90) {
            skin.mint(msg.sender, 10009);
            emit skinReward(msg.sender, 10009);
    ...
    function checkUserSkinLevelList(address addr,uint level) public view returns(uint[] memory listss){
        uint temp = 0;
        uint[] memory skinList = skin.checkUserSkinList(addr);
        uint[] memory lists = checkSkinLevelBatch(skinList);
        for (uint i = 0; i < lists.length; i++) {
            if (lists[i] == level) {
                temp++;
            }
        }
        listss = new uint[](temp);
        for (uint i = 0; i < lists.length; i++) {
            if (lists[i] == level) {
                listss[temp - 1] = skinList[i];
                temp--;
            }
        }
    }
    function batchCompoundSkin(uint level, uint amount) external {
        require(skin.balanceOf(msg.sender) >= amount * 3, 'out of Amount');
        require(level > 0 && level <= 3, 'wrong level');
        uint temp = 0;
        uint[] memory skinList = skin.checkUserSkinList(msg.sender);
        uint[] memory lists = checkSkinLevelBatch(skinList);
        for (uint i = 0; i < lists.length; i++) {
            if (lists[i] == level) {
                temp++;
        ...
        require(temp >= amount * 3, 'not enough skin');
        uint[] memory levelList = new uint[](temp);
        for (uint i = 0; i < lists.length; i++) {
            if (lists[i] == level) {
                levelList[temp - 1] = skinList[i];
                temp--;
            }
        }
        uint burnIndex;
        uint rates = 3 * levelRate[level - 1];
        for (uint i = 0; i < amount; i++) {
            uint random = rand(100);
            skin.burn(levelList[burnIndex]);
            skin.burn(levelList[burnIndex + 1]);
            skin.burn(levelList[burnIndex + 2]);
            burnIndex += 3;
            if (random <= rates) {
                uint out = rand(100);
                if (out >= 90) {
    ...
    function compoundSuperSkin(uint[] memory skins) external {
        ...
        uint id = skin.skinIdMap(skins[0]);
        uint out = rand(100);
        if (id == 10010) {
            if (out >= 30) {
                skin.mint(msg.sender, id + 1);
                emit skinReward(msg.sender, id + 1);
                for (uint i = 0; i < skins.length; i++) {
                    skin.burn(skins[i]);
                }
            } else {
