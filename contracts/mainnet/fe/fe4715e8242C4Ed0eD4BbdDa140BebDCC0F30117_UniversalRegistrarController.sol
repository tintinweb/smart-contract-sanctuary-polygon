...
abstract contract BaseUniversalRegistrarController is Initializable {
     ...
     */
    function payee(bytes32 node) public view returns (address) {
        return _payee[node];
    }
     ...
     */
    function payeeShare(bytes32 node) public view returns (uint256) {
     ...
     */
    function registryShare(bytes32 node) public view returns (uint256) {
     ...
     */
    function payeeReleased(bytes32 node) public view returns (uint256) {
    ...
    */
    ...
    function releaseToPayee(bytes32 node) public virtual {
        ...
        AddressUpgradeable.sendValue(payable(payee_), payment);
...
contract SLDPriceOracle is Ownable {
     ...
     *                    dollar) each.
     */
    function setPrices(bytes32 node, uint256[] memory _prices) public onlyNodeOwner(node) {
        prices[node] = _prices;
...
contract UniversalRegistrarController is Ownable, BaseUniversalRegistrarController {
    ...
    mapping(bytes32 => address) public reserved;

    event NameRegistered(bytes32 indexed tld, string name, bytes32 indexed label, address indexed owner, uint cost);
    event NameReserveChanged(bytes32 indexed tld, string indexed name, address indexed account);
    ...
    function reserveName(bytes32 node, string calldata name, address account) public onlyOwner {
        bytes32 nodehash = _node(node, name);
        reserved[nodehash] = account;
        emit NameReserveChanged(node, name, account);
    }
    ...
    function ownerRegisterWithConfig(bytes32 node, string memory name, address owner,
        ...
        bytes32 commitment = makeCommitmentWithConfig(node, name, owner, secret, resolver, addr);
        _consumeCommitment(node, name, commitment);
    ...
    function _register(bytes32 node, string memory name, address owner, address resolver, address addr, uint cost) private {
        ...
        if(reserved[nodehash] != address(0)) {
            require(reserved[nodehash] == msg.sender && owner == msg.sender, "Name is Reserved!");
        }
