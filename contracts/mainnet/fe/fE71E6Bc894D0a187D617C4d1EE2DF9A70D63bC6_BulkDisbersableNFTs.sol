...
contract BulkDisbersableNFTs is Initializable, ERC1155Upgradeable, OwnableUpgradeable, ERC1155BurnableUpgradeable, ERC1155SupplyUpgradeable, UUPSUpgradeable {
  ...
  // Maintainers may update all the characteristics
  // of a token.
  ...
  function hasRole(
    ...
    returns (bool has)
  {
    uint256 gate = GATING_TYPE + (uint(role) << ROLE_BOUNDARY);
    ...
    returns (uint8 assignments)
  {
    ...
    if(!hasRole(role, user, id)) {
      uint256 token = GATING_TYPE + (uint(role) << ROLE_BOUNDARY) + id;
      mint(user, token, 1, "");
      return 1;
    }
    return 0;
    ...
    returns (uint256 tokenId)
  {
    ...
    require(
      ...
      "You must have a Creator token to change create new tokens."
    );
