...
contract LedgerInfinityPass is
    ERC721,
    ReentrancyGuard,
    AccessControl,
    Signable,
    IERC721Mintable
{
    ...
    function safeMint(address to) external onlyRole(MINTER_ROLE) {
        if (_phase == Phase.NONE) revert Errors.MintNotAvailable();
    ...
    // Function used to do minting on pre-sale phase
    function preSaleMint(
        uint256 amount,
        uint256 maxAmount,
        bytes calldata signature
    ) external payable costs(amount) phaseRequired(Phase.PRE_SALE) lock {
        _mintLogic(amount, maxAmount, signature);
    }
