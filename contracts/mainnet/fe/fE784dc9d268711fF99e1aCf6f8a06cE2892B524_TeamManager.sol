...
abstract contract StakeValidator is SeasonSync {
    ...
    function unstakePositionForcibly(uint256 _tokenId, address _user, uint256 _position) internal {
        // Getting of token's position.
        uint256 position = _position;
        require(position != 0, "Position code can't be zero");
...
abstract contract Staker is StakeValidator {
    ...
    event PlayerForciblyUnstakedWithoutTransfer(uint256 _season, address _user, uint256 _tokenId);
    ...
    function unstakePlayer(uint256 _tokenId) public addedUser(seasonId, _msgSender()) {
        deleteFromTeam(_msgSender(), _tokenId, false,0);
    ...
    /**
     * @dev Forcibly removes the player with `_tokenId` from the team of `_user` by the administrator, without transferring the player token. This helps fix the situation when the player token is burned without unstaking.
     *
     * Requirements:
     *  - The caller should have the default admin role (`DEFAULT_ADMIN_ROLE`).
     *  - The account, that owns the players, should be a user.
     ...
     * @param _tokenId   A token ID of the player to be removed from the team of its owner.
     */
    function forciblyUnstakePlayerWithoutTransfer(address _user, uint256 _tokenId,uint256 _position)
        public
        onlyRole(DEFAULT_ADMIN_ROLE)
        addedUser(seasonId, _user)
    {
        require(
            gen2PlayerToken.nftIdToImageId(_tokenId) == 0, "Token exists"
        );
        deleteFromTeam(_user, _tokenId, true, _position);
        uint256 season = seasonId;
        emit PlayerForciblyUnstakedWithoutTransfer(season, _user, _tokenId);
        emit PlayerUnstaked(season, _user, _tokenId);
    }
     ...
     */
    // prettier-ignore
    function deleteFromTeam(address _user, uint256 _tokenId, bool _force, uint256 _position) private {
        uint256 season = seasonId;
        ...
        if(_force){
            unstakePositionForcibly(_tokenId, _user, _position);
        }
        else{
