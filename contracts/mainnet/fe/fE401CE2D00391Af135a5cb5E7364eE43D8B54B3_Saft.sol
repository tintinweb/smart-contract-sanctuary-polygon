...
contract Saft is AragonApp {
    ...
    uint8 public constant AUCTION_TYPE_TOKEN = 1;
    uint8 public constant AUCTION_TYPE_EQUITY = 2;
    ...
    string private constant ERROR_PRICE_CANNOT_BE_NEGATIVE =
        "PRICE_CANNOT_BE_NEGATIVE";
    string private constant ERROR_QUANTITY_CANNOT_BE_NEGATIVE =
        "QUANTITY_CANNOT_BE_NEGATIVE";
    string private constant ERROR_AMOUNT_CANNOT_BE_NEGATIVE =
        "AMOUNT_CANNOT_BE_NEGATIVE";
    string private constant ERROR_DURATION_CANNOT_BE_NEGATIVE =
        "DURATION_CANNOT_BE_NEGATIVE";
    string private constant ERROR_DOWNPAYMENT_CANNOT_BE_NEGATIVE =
        "DOWNPAYMENT_CANNOT_BE_NEGATIVE";
    string private constant ERROR_YOU_DO_NOT_HAVE_ALLOVANCE_FOR_SO_MANY_TOKENS =
        "YOU_DO_NOT_HAVE_ALLOVANCE_FOR_SO_MANY_TOKENS";
    string private constant ERROR_AUCTION_HAS_FINISHED =
        "AUCTION_HAS_FINISHED";
    string private constant ERROR_PRICE_TOO_LOW =
        "PRICE_TO_LOW";
    string private constant ERROR_QUANTITY_TOO_LOW =
        "QUANTITY_TO_LOW";
    string private constant ERROR_AMOUNT_TOO_LOW =
        "AMOUNT_TO_LOW";
    string private constant ERROR_DOWNPAYMENT_TOO_LOW =
        "DOWNPAYMENT_TO_LOW";
    string private constant ERROR_ALREADY_APPROVED =
        "ALREADY APPROVED";
    string private constant ERROR_SHOULD_BE_APPROVED =
        "SHOULD BE APPROVED";
    string private constant ERROR_ALREADY_REJECTED =
        "ALREADY REJECTED";
    string private constant ERROR_SHOULD_BE_FINISHED =
        "SHOULD BE FINISHED";
    string private constant ERROR_ALREADY_FINISHED =
        "ALREADY FINISHED";

    TokenManager public tokenManager;

    mapping(uint256 => Bid) public bidList;
    uint256 bidListLength;

    address public owner;
    address public service; // !! NEED INIT
    address public vault; 

    uint8 public auctionType;
    uint256 public minPrice;
    uint256 public minQuantity;
    uint256 public minAmount;
    uint256 public minDownpayment;
    ...
    uint256 public blockWaitingTime = 10; // !! NEED INIT
    uint256 private finalizeBlockNumber;
    ...
    struct Bid {
        ...
        uint256 amount; // общая сумма ставки умноженная на 10^18
        uint256 completeAmount; // сумма предоплаты по ставке умноженная на 10^18
        uint256 price; // цена за единицу
        uint256 quantity; // количестов Community tokens
        bool selected;
        bool collected;
    }
    event Initialize(address user);
    event Finalize();
    event SelectBid(uint256 bidId);
    event MakeBid(
        address user,
        uint256 bidId,
        address addressToken,
    ...
    event CompleteBid(address user, uint256 bidId, uint256 amount);
    event RefundBid(address user, uint256 bidId, uint256 amount);
    event CollectFunds(uint256[] ids);
    event CollectUncompleteFunds(uint256[] ids);
    ...
    modifier onlyFromWhiteList() {
        require(whiteList.fromWhiteList(msg.sender), "NOT FROM WHITE LIST");
        _;
    }
    function initialize(
        ...
        require(_minPrice >= 0, ERROR_PRICE_CANNOT_BE_NEGATIVE);
        require(_minQuantity >= 0, ERROR_QUANTITY_CANNOT_BE_NEGATIVE);
        require(_minAmount >= 0, ERROR_AMOUNT_CANNOT_BE_NEGATIVE);
        require(_auctionDuration >= 0, ERROR_DURATION_CANNOT_BE_NEGATIVE);
        addAcceptableTokens(_acceptedDepositTokens);
        tokenManager = TokenManager(_tokenManager);
        weeziCore = IWeeziCore(cores[0]);
        whiteList = IWhiteList(cores[1]);
        vault = _vault;
        owner = msg.sender;
        ...
        // auctionEndDate = now + _auctionDuration;
        finished = false;
        approved = false;
        rejected = false;
        initialized();
    ...
    function makeBid(
        ...
        require(!finished, ERROR_AUCTION_HAS_FINISHED);
        require(approved, ERROR_SHOULD_BE_APPROVED);
        require(!rejected, ERROR_ALREADY_REJECTED);
        require(now < auctionEndDate, ERROR_AUCTION_HAS_FINISHED);
        require(acceptableTokenAdded[_addressToken], "INVALID BID TOKEN");
        require(_quantity > 0, ERROR_QUANTITY_TOO_LOW);
        require(_price >= minPrice, ERROR_PRICE_TOO_LOW);
        require(_quantity >= minQuantity, ERROR_QUANTITY_TOO_LOW);
        require(_downPayment >= minDownpayment, ERROR_DOWNPAYMENT_TOO_LOW);
        
        IERC20 _token = IERC20(_addressToken);
        uint256 _quantityForFirstStage = calculatePercentage(
            _quantity,
            _downPayment
        );
        uint256 _completeAmount = calculateAmount(
            _price,
            _quantityForFirstStage,
            2 * PRICE_DECIMALS - _token.decimals()
        );
        
        uint256 _amountForCheck = calculateAmount(
            _price,
            _quantity,
            PRICE_DECIMALS
        );
        require(_amountForCheck >= minAmount, ERROR_AMOUNT_TOO_LOW);

        uint256 _amount = calculateAmount(
            _price,
            _quantity,
            2 * PRICE_DECIMALS - _token.decimals()
        );

        //Убрано, поскольку мешает на фронте получить данные для отправки запроса на approve        
        // require(
        //     _token.allowance(msg.sender, address(this)) >= _completeAmount,
        //     ERROR_YOU_DO_NOT_HAVE_ALLOVANCE_FOR_SO_MANY_TOKENS
        // );
    ...
    function completeBid(uint256 _bidId) public {
        require(finished, ERROR_SHOULD_BE_FINISHED);
        require(approved, ERROR_SHOULD_BE_APPROVED);
        Bid storage bid = bidList[_bidId];
        require(msg.sender == bid.user, "INCORRECT USER");
        require(bid.selected, "NOT SELECTED BID");

        ERC20 _token = ERC20(bid.addressToken);
        uint256 _tokensForSecondStage = bid.amount - bid.completeAmount;

        
        if (_tokensForSecondStage > 0) {
            require(
                _token.allowance(msg.sender, address(this)) >=
                    _tokensForSecondStage,
                ERROR_YOU_DO_NOT_HAVE_ALLOVANCE_FOR_SO_MANY_TOKENS
            );
            require(
                _token.transferFrom(
        ...
        bid.completeAmount = bid.amount;

        if (bid.amount > 0) {
            if (bid.addressToken == ETH) {
                (bool success, ) = vault.call.value(bid.amount)();
                require(success, ERROR_ETH_TRANSFER_FAILED);
            } else {
                require(
                    _token.transfer(vault, bid.amount),
                    ERROR_TOKEN_TRANSFER_REVERTED
                );
            }
        }
        tokenManager.mint(msg.sender, bid.quantity);
        emit CompleteBid(msg.sender, _bidId, bid.amount - bid.completeAmount);
    }

    function refundBid(uint256 _bidId) public {
        Bid storage bid = bidList[_bidId];
        require(msg.sender == bid.user, "INCORRECT USER");
        require(!bid.selected, "SELECTED BID");
        // require(bid.completeAmount > 0, "NOTHING FOR TRANSFER");

        IERC20 _token = IERC20(bid.addressToken);
        require(
            bid.completeAmount <= _token.balanceOf(address(this)),
            "BALANCE TOO LOW"
        );

        if (bid.completeAmount > 0) {
            _token.transfer(
                msg.sender,
                bid.completeAmount
            );
        ...
        emit RefundBid(msg.sender, _bidId, bid.completeAmount);
        bid.completeAmount = 0;
    }

    function setRejected() public onlyFromWhiteList 
    {
        require(!approved, ERROR_ALREADY_APPROVED);
        require(!rejected, ERROR_ALREADY_REJECTED);
        rejected = true;
        finished = true;
        finalizeBlockNumber = block.number;
        emit Rejected();
        emit Finalize();
    }

    function setApproved() public onlyFromWhiteList 
    {
        require(!approved, ERROR_ALREADY_APPROVED);
        require(!rejected, ERROR_ALREADY_REJECTED);
    ...
    function finalize() public auth(ADMIN_ROLE) 
    {
        require(!finished, ERROR_ALREADY_FINISHED);
        require(approved, ERROR_SHOULD_BE_APPROVED);
        finished = true;
        finalizeBlockNumber = block.number;
        emit Finalize();
    ...
    function collectFunds(address _to) public auth(ADMIN_ROLE) {
        require(finished, ERROR_SHOULD_BE_FINISHED);
        
        address feeWalletAddress = weeziCore.getFeeWalletAddress();
        require(feeWalletAddress != address(0), "DON'T VALID FEE ADDRESS");
        uint256 count = 0;
        Bid storage bid;
        for (uint256 i = 0; i < bidListLength; i++) {
            bid = bidList[i];
            if (
                !bid.collected &&
                bid.selected &&
                bid.amount == bid.completeAmount
            ) {
                IERC20 _token = IERC20(bid.addressToken);
                ...
                _token.transfer(feeWalletAddress, feeAmount);
                _token.transfer(_to, bid.completeAmount.sub(feeAmount));
                bid.collected = true;
                count++;
            }
        }
        ...
        emit CollectFunds(ids);
    }

    function collectUncompleteFunds(address _to) public auth(ADMIN_ROLE) {
        require(
            block.number > finalizeBlockNumber.add(blockWaitingTime),
            "NEED WAIT"
        );
        address feeWalletAddress = weeziCore.getFeeWalletAddress();
        require(feeWalletAddress != address(0), "DON'T VALID FEE ADDRESS");
        ...
        for (uint256 i = 0; i < bidListLength; i++) {
            bid = bidList[i];
            if (!bid.collected && bid.completeAmount >0) {
                IERC20 _token = IERC20(bid.addressToken);
                uint256 feeAmount = calculatePercentage(
                    bid.completeAmount,
                    platformFee
        ...
        emit CollectUncompleteFunds(ids);
    }

    function selectBid(uint256 _bidId) public auth(ADMIN_ROLE) 
    {
        require(finished, ERROR_SHOULD_BE_FINISHED);
        Bid storage bid = bidList[_bidId];
        bid.selected = true;
        emit SelectBid(_bidId);
    }
    ...
    function calculateAmount(uint256 _price, uint256 _quantity, uint256 _decimals)
        public
        pure
        returns (uint256)
    {
        return _quantity.mul(_price).div(10**_decimals);
    }

    function getBidLength() public view returns (uint256) {
        return bidListLength;
    }

    function addAcceptableTokens(address[] _acceptedDepositTokens) internal {
        for (uint256 i = 0; i < _acceptedDepositTokens.length; i++) {
            _addAcceptableToken(_acceptedDepositTokens[i]);
        }
    }

    function _addAcceptableToken(address _token) internal {
    ...
    function addAcceptableToken(address _token) public auth(ADMIN_ROLE) {
        _addAcceptableToken(_token);
    }

    function removeAcceptableToken(address _token) external auth(ADMIN_ROLE) {
    ...
    {
        address[] memory user = new address[](bidListLength);
        address[] memory addressToken = new address[](bidListLength);
        uint256[] memory amount = new uint256[](bidListLength);
        uint256[] memory completeAmount = new uint256[](bidListLength);
        uint256[] memory price = new uint256[](bidListLength);

        uint256 c = 0;
        for (uint256 i = 0; i < bidListLength; i++) {
            Bid memory bidInfo = bidList[i];
            user[c] = bidInfo.user;
            addressToken[c] = bidInfo.addressToken;
            amount[c] = bidInfo.amount;
            completeAmount[c] = bidInfo.completeAmount;
            price[c] = bidInfo.price;
            c++;
        }

        return (user, addressToken, amount, completeAmount, price);
    }
...
interface IWhiteList {
    function addParticipants(address[] _participant) external;

    function addParticipant(address _participant) external;

    function fromWhiteList(address _participant) external view returns (bool);
