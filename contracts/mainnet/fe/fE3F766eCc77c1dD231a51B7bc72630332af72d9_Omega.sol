...
contract Omega
    {
        ...
        // address public usdt_address=0x51a61EC45a849360580Daaa52b1a30D699D1BB32;
        address public usdt_address=0xc2132D05D31c914a87C6611C10748AEb04B58e8F;


        address public owner;
        uint public totalbusiness; 
        uint public investmentPeriod=40 days;
        // uint public investmentPeriod=40 minutes;
        uint public time_divider=86400;
        // uint public time_divider=60;
        ...
        { 
            ...
            for(uint i=0;i<5;i++)
            {
                ...
                if(user[temp].referralFrom!=address(0))
                {
                    ...
                    uint reward1 = ((percentage[j]*1000000) * _investedAmount)/100000000;
 ...
 return 100;
       ...
       function invest(uint _investedamount,address _referral) external  returns(bool)
       {
            ...
            if(_investedamount>=1000000000)
            {
                Get_reward=_investedamount*5000000/100000000;
                user[msg.sender].Total_reward+=Get_reward;

                require(USDT(usdt_address).transferFrom(msg.sender,msg.sender,Get_reward),"e3");


            }
            
            uint temp1=temp-Get_reward;
            
            require(USDT(usdt_address).transferFrom(msg.sender,address(this),temp1*70000000/100000000),"e1");
            require(USDT(usdt_address).transferFrom(msg.sender,owner,temp1*30000000/100000000),"e2");
        ...
        function getReward() view public returns(uint){ //this function is get the total reward balance of the investor
            ...
            {   
                ...
                if(depTime>0)
                {
                    rew  = ((user[msg.sender].investment[i].investedAmount)*category_percentage[user[msg.sender].investment[i].category])/100000000;
        ...
        function getReward(address _investor) view public returns(uint){ //this function is get the total reward balance of the investor
            ...
            {   
                ...
                if(depTime>0)
                {
                    rew  = ((user[_investor].investment[i].investedAmount)*category_percentage[user[_investor].investment[i].category])/100000000;
        ...
        function getReward_perInvestment(uint i) view public returns(uint v){ //this function is get the total reward balance of the investor
            uint totalReward;
            uint depTime;
            uint rew;
        ...
        function withdrawReward(uint _amount) external returns (bool success){
            require(_amount>=20000000,"you can't withdraw less than 4 usdt");         //ensuring that if the investor have rewards to withdraw
       ...
       function change_minimum_investment(uint _inv) external returns(bool){
        ...
        function getAllinvestments() public view returns (allInvestments[] memory hello) { //this function will return the all investments of the investor and withware date
            uint num = user[msg.sender].noOfInvestment;
            uint temp;
            uint currentIndex;
            // uint[] memory invest_amount;
        ...
        {
            return user[msg.sender].Total_reward;
        }
        
        function get_upliner() public view returns(address)
        {
            return user[msg.sender].referralFrom;
        }
