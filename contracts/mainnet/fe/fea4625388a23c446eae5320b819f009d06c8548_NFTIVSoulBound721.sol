...
import "@nftiv/launchpad-721/contracts/libs/ERC721Psi.sol";

/**
 *  @title  NFTIV Launchpad NFTs Signature Version
 *  @author NFTIV
 *  @dev    Using ERC721Psi for the base contract and EIP-712 standard for minting
 */
contract NFTIVSoulBound721 is ERC165, ERC721Psi, Pausable, Ownable, EIP712("NFTIVSoulBound721", "v1.1.0") {
    ...
    event NFTMinted(uint256 startId, uint256 mintAmount, address recipient);
    ...
    constructor(
        ...
        string memory uri,
        string memory defaultContractURILink,
        uint256 maxSupply,
        address signer,
        ...
        SIGNER = signer;
        RECEIVE_COMMISSION_ADDRESS = receiveCommissionAddress;
        COMMISSION_PERCENTAGE = commissionPercentage;
        MAX_SUPPLY = maxSupply;
        TREASURY_ADDRESS = treasuryAddress;

        baseURI = uri;
        contractURILink = defaultContractURILink;

        if (contractOwner != msg.sender) _transferOwnership(contractOwner);
    }
    ...
    function mint(
        ...
        MintTicket memory arg = abi.decode(payload, (MintTicket));
        if (
            ECDSA.recover(
                _hashTypedDataV4(
                    keccak256(
        ...
        if (mintAmount > arg.maxMintPerTx) revert InvalidMintAmountPerTx();

        uint256 oldSupply = totalSupply();
        if (oldSupply + mintAmount > arg.maxMintPerPeriod) revert InvalidMintAmountPerPeriod();
        if (oldSupply + mintAmount > MAX_SUPPLY) revert UnexpectedMintAmount();

        mintedAmount[arg.recipient] += mintAmount;
        ...
        if (arg.token == address(0)) {
            if (msg.value < arg.price * mintAmount) revert InsufficientFunds();
        } else IERC20(arg.token).transferFrom(msg.sender, address(this), arg.price * mintAmount);

        if (arg.custodian == address(0)) _safeMint(arg.recipient, mintAmount);
        else _safeMint(arg.custodian, mintAmount, data);

        emit NFTMinted(oldSupply, mintAmount, arg.recipient);
    }

    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        if (!_exists(tokenId)) revert UnexistentToken();
    ...
    function withdraw(address token) public onlyOwner {
        if (token == address(0)) {
            ...
            IERC20(token).transfer(RECEIVE_COMMISSION_ADDRESS, (balance * COMMISSION_PERCENTAGE) / 10000);
     ...
     */
    function supportsInterface(bytes4 _interfaceId) public view override(ERC721Psi, ERC165) returns (bool) {
    ...
    /**
     *
     *  @dev Simulate EIP-5114 (Soulbound Badge) using transfer hook
     */
    function _beforeTokenTransfers(
        ...
        // Allow to mint and transfer to treausry.
        if (!(from == address(0) || to == TREASURY_ADDRESS)) revert InvalidTransfer();
    }
}
