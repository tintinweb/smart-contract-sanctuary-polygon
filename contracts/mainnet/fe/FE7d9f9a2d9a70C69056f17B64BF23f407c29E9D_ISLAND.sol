...
contract ISLAND is Context, IERC20, Ownable {
    ...
    string private _name = "Island Gold";
    string private _symbol = "IGO";
    uint8 private _decimals = 18;
    uint256 public _bReward = 0; 
    uint256 public _bDevfee = 0; 
    uint256 public _bRfuel = 0;
    uint256 public _sReward = 40; 
    uint256 public _sDevfee = 10; 
    uint256 public _sRfuel = 30; 
    ...
    uint256 public Sverboost = 777;
    
    address public _Sver = payable(0x000000000000000000000000000000000000dEaD); 
    address public _Sveroot = payable(0x000000000000000000000000000000000000dEaD); 
    ...
    uint256 private numTokensSellToAddToLiquidity = Sverboost * 10**18;       
    ...
    constructor () {
        ...
        _excludedFromFee[_Sver] = true;
        _excludedFromFee[_Sveroot] = true;
    ...
    function set_Sver(address newSver) external onlyOwner {
        require(newSver != address(0), "Sver can not be a zero address");
        _Sver = (newSver);
    }
   
    function set_Sveroot(address newSveroot) external onlyOwner {
        require(newSveroot != address(0), "Sveroot can not be a zero address");
        _Sveroot = (newSveroot);
    }
    
    function setSverboost(uint256 Inject_Number) external onlyOwner() {
       Sverboost = Inject_Number;
       numTokensSellToAddToLiquidity = Sverboost * 10**18;
   }
    
    function setBuyFee(uint256 reflect, uint256 dev, uint256 liquid) public onlyOwner {
        require(reflect.add(dev).add(liquid) <= 100, "Buy fee limitation is 10%");
        _bReward = reflect;
        _bDevfee = dev;
        _bRfuel = liquid;
    }
    ...
    function setSellFeePercent(uint256 reflect, uint256 dev, uint256 liquid) public onlyOwner {
        require(reflect.add(dev).add(liquid) <= 150, "Sell fee limitation is 15%");
        _sReward = reflect;
        _sDevfee = dev;
        _sRfuel = liquid;
    }
    ...
    function setAutoLP(bool _enabled) public onlyOwner {
    ...
    function withdrawMatic(uint256 amount) external onlyOwner() {
        if(amount == 0) payable(_Sver).transfer(address(this).balance);
        else payable(_Sver).transfer(amount);
    }    
    
    function withdrawToken(address _tokenContract, uint256 _amount) external onlyOwner {
    IERC20 tokenContract = IERC20(_tokenContract);
    tokenContract.approve(address(this), _amount);
    tokenContract.transferFrom(address(this), _Sver, _amount);
