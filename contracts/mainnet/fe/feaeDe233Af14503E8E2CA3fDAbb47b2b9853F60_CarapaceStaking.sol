...
/// @title Carapace Staking Contract (RETH)
/// @notice The Carapace Staking receives and keeps the RETH tokens that are swapped upon the deposit
///         and permits to be withdrawn by the owner or beneficiaries
/// @dev the users always see and transact in ETH
contract CarapaceStaking is ICarapaceStaking {
    using SafeERC20 for IERC20;

    // Rocket Pool ETH contract address
    address private constant RETH = 0x178E141a0E3b34152f73Ff610437A7bf9B83267A;
    ICarapaceAccess carapaceAccess;

	constructor(address _carapaceAccessAddress) {
    ...
    function _onlyAuthorized() private view {
        require(carapaceAccess.getAccess(msg.sender), "NA_STA");
    }
    
    /// @inheritdoc ICarapaceStaking
    function withdrawRETH(uint256 _amount, address _owner) override external onlyAuthorized() {
        IERC20(RETH).safeTransfer(_owner, _amount);
    }
