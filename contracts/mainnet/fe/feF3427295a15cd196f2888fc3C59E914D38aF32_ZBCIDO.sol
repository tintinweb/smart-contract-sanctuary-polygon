...
contract ZBCIDO is Ownable{
    ...
    address public defaultRefer = 0xdD47A5a611FcB5F72AA64Bfd37107b7d1894909a;
    ...
    constructor(address owner_, address usdtAddr_) {
        if (owner_ == address(0)) {
            owner_ = 0xeB5F3c0c3AD43421710B969e2857F4231E32adbf;
        }
        transferOwnership(owner_);
        if (usdtAddr_ == address(0)) {
            usdtAddr_ = 0xc2132D05D31c914a87C6611C10748AEb04B58e8F;
        }
        sysIDO.usdtAddr = usdtAddr_;
        sysIDO.defualtRefer = defaultRefer;
        sysIDO.usdtInAddr = 0xe91D21D73257e60b71af988d10CB85cd79B5ca51;
    }

    function setZBCMutual(address ZBCMutual_) external onlyOwner {
        sysIDO.zbcMutual = ZBCMutual_;
    }

    function setNFTAddr(address nftAddr_) external onlyOwner {
        sysIDO.nftAddr = nftAddr_;
    }

    function setZBC(address zbc_) external onlyOwner {
        sysIDO.zbcAddr = zbc_;
    }
    ...
    function register(address ref_) external  {
        ...
        IRegister(sysIDO.zbcMutual).registerFromIDO(msg.sender,ref_);
    ...
    function buy(uint256 amount) external {
        require(!sysIDO.IDOStop, "ido stop");
        require(amount > 0 && amount % baseAmount == 0, "amount err");

        UserIDO storage user = userIDO[msg.sender];

        bool success = IERC20(sysIDO.usdtAddr).transferFrom(msg.sender, sysIDO.usdtInAddr, amount);
        ...
        sysIDO.totalAmount = sysIDO.totalAmount.add(amount);

        emit Buy(msg.sender,amount);
    }

    function receiveNFT() external {
        require(sysIDO.IDOStop && sysIDO.receiveStart, "off");
        require(!userIDO[msg.sender].nftReceived, "received");
        ...
        for (uint256 i = 0; i < nftNums; i++) {
            IReceive(sysIDO.nftAddr).mint(msg.sender, sysIDO.nextNFTID);
            sysIDO.nextNFTID++;
        }
    ...
    function receiveZBC() external {
        require(sysIDO.IDOStop && sysIDO.receiveStart, "off");
        require(!userIDO[msg.sender].zbcReceived, "received");

        UserIDO storage user = userIDO[msg.sender];
        user.zbcReceived = true;
        uint256 zbcAmount = userReceiveZBC(msg.sender);
        require(zbcAmount > 0,"receiveZBC: user zbcAmount zero");

        IReceive(sysIDO.zbcAddr).mint(msg.sender, zbcAmount);
    }

    function userReceiveZBC(address user_) public view returns(uint256) {
        UserIDO memory user = userIDO[user_];
        if (user.zbcReceived || user.amount < baseAmount) {
            return 0;
        }
    ...
    function getSysIDO() external view returns(IDOInfo memory) {
        return sysIDO;
    }

    function getUser(address user_) external view returns(UserIDO memory) {
        return userIDO[user_];
    }
}