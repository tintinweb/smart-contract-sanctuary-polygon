...
import "./../interface/curve/ICurveSwapPolygon.sol";
import "./../interface/curve/IStrategyCurveLP.sol";


contract LiquidCZapCurvePolygon {
    ...
    function LiquidCIn (address liquidCVault, uint256 tokenAmountOutMin, address tokenIn, uint256 tokenInAmount) external {
        ...
        if (poolSize == 2) {
            ...
            ICurveSwapPolygon(pool).add_liquidity(amounts, 0, true);
        } else if (poolSize == 3) {
            uint256[3] memory amounts;
            amounts[depositIndex] = depositBal;
            if (useUnderlying) ICurveSwapPolygon(pool).add_liquidity(amounts, 0, true);
            else if (useMetapool) ICurveSwapPolygon(pool).add_liquidity(lpToken, amounts, 0);
            else ICurveSwapPolygon(pool).add_liquidity(amounts, 0);
        } else if (poolSize == 4) {
            uint256[4] memory amounts;
            amounts[depositIndex] = depositBal;
            if (useMetapool) ICurveSwapPolygon(pool).add_liquidity(lpToken, amounts, 0);
            else ICurveSwapPolygon(pool).add_liquidity(amounts, 0);
        } else if (poolSize == 5) {
            uint256[5] memory amounts;
            amounts[depositIndex] = depositBal;
            ICurveSwapPolygon(pool).add_liquidity(amounts, 0);
        }
    ...
    function LiquidCOut (address liquidCVault, uint256 withdrawAmount) external {
        ...
        uint256[] memory minAmount = new uint256[](poolSize);
        for (uint256 x=0; x<poolSize; x++) {
        ...
        uint256 amountLiquidity = IERC20(lpToken).balanceOf(address(this));
        ICurveSwapPolygon(pool).remove_liquidity(amountLiquidity, minAmount);
    ...
    function LiquidCOutAndSwap(address liquidCVault, uint256 withdrawAmount, address desiredToken, uint256 desiredTokenOutMin) external {
        ...
        uint256 amountLiquidity = IERC20(lpToken).balanceOf(address(this));
        ICurveSwapPolygon(pool).remove_liquidity_one_coin(amountLiquidity, desiredIndex, desiredTokenOutMin);
...
interface ICurveSwapPolygon {
    function remove_liquidity(uint256 token_amount, uint256[] memory min_amount) external;
