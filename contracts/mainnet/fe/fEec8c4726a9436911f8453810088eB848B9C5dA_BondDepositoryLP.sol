...
contract BondDepositoryLP is Ownable {
    ...
    function setMinPayout(uint256 _newMinPayout) public onlyPolicy() {
    ...
    function payoutFor( uint _value ) public view returns ( uint ) {
        return FixedPoint.fraction( _value, bondPrice() ).decode112with18()/1e13;
    }
    ...
    function bondPrice() public view returns ( uint price_ ) { 
        price_ = terms.controlVariable.mul( debtRatio() )/1e4;
    ...
    /**
     *  @notice converts bond price to value
     *  @return price_ uint
     */
    function bondPriceInUSD() public view returns ( uint price_ ) {
        price_ = bondPrice().mul( IBondCalculator( bondCalculator ).markdown( principle ) )/1e5;
    }
