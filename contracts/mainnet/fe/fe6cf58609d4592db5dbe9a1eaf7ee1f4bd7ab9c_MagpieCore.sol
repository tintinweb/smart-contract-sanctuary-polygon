...
contract MagpieCore{
    ...
    function swapInToken(userInput memory ui) external payable returns(uint64 tokenSequence, uint64 coreSequence){
        ...
        bytes memory encoded = abi.encodePacked(payload.targetPath, payload.targetAmountIn,  payload.targetToAddress);
    ...
    function swapOutToken(bytes memory encodedVmBridge, bytes memory encodedVmCore, uint deadline) external {
        ...
        if (valid == true){
            ...
            targetAmountsOut = uniswap.getAmountsOut(payload.targetAmountIn, payload.targetPath);
            uniswap.swapExactTokensForTokens(payload.targetAmountIn, targetAmountsOut[targetAmountsOut.length-1], payload.targetPath, payload.targetToAddress, deadline);            
        }
    }
