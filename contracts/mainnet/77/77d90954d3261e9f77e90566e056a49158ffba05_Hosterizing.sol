...
contract Hosterizing is ERC721URIStorage {
    ...
    constructor(string memory _tld) payable ERC721("Hosterizing", "HTZ") {
        owner = payable(msg.sender);
        tld = _tld;
        console.log("%s name service by Hosterizing deployed", _tld);
    }
...
if (len == 3) {
            return 5000000000000000000  * 3 ; // 5 MATIC = 5 000 000 000 000 000 000 (18 decimals). We're going with 0.5 Matic because the faucets don't give a lot 
        } else if (len == 4) {
            return 3000000000000000000 * 1 ; // To charge smaller amounts, reduce the decimals. This is 0.3
        } else {
    ...
    function register(string calldata name) public payable {
            ...
            bytes(
                string(
                    abi.encodePacked(
                        '{"name": "',
                        _name,
                        '", "description": "A domain on Hosterizing", "image": "data:image/svg+xml;base64,',
        ...
        console.log("%s domain successfully registered!", msg.sender);
