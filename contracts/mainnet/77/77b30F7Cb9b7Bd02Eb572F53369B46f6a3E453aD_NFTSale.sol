...
contract NFTSale is MultiWhitelist {
     ...
     *
     * @param nftType - Type of NFT
     * @param nextCounter - nextTokenId of NFT
     */
    function setTypeCounter(NFTType nftType, uint256 nextCounter) public onlyOwner {
        typeCounter[nftType] = nextCounter;
    }
    ...
    onlyWhitelisted
    {
        ...
        string memory tempNftId = strings.uint2str(nftId);
        string memory tempURI = URI.toSlice().concat(tempNftId.toSlice());
        string memory finalURL = tempURI.toSlice().concat(".json".toSlice());

        TOKEN.transferFrom(buyer, address(this), price);
        NFT.mint(msg.sender, nftId, finalURL);
