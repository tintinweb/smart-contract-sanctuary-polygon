...
contract UniswapTWAPAggregator is Ownable, IOracleAggregator {
    ...
    function getTokenValueOfOneNativeToken(
        ...
        require(poolFee != 0, "UniswapTWAPAggregator: token and pool fee not set");
        if(poolAddress == NATIVE_ADDRESS) {
            require(allowFixedFee, "UniswapTWAPAggregator: Shall not proceed with unreliable pool fee");
        }
        ...
        (bool success, bytes memory returndata) = address(quoter)
            .staticcall(_data);

        if (success) {
         exchangeRate = abi.decode(returndata, (uint256));
         } else {
            revert ExchangeRateQueryFailed();
         }
    }
}