...
import { IVestingHub } from "../interface/IVestingHub.sol";
import { SafeMath } from "@openzeppelin/contracts/utils/math/SafeMath.sol";
import { AggregatorV3Interface } from "@chainlink/contracts/src/v0.8/interfaces/AggregatorV3Interface.sol";

contract YPredictPublicSaleMatic is Ownable, Pausable, ReentrancyGuard {
  using SafeERC20 for IERC20;

  IERC20 public s_usdt;
  IVestingHub public s_vesting;

  address public i_matic_usd_priceFeed;
  address public i_usdt_usd_priceFeed;

  uint256 public s_usdtPrice = 38890;
  uint256 public s_minAmountToInvest = 100000000;
  uint256 public s_maxAmountToInvest = 5000000000;
  uint256 public s_tokensForSale = 18000000 ether;
  ...
  constructor() {
    ...
    if (block.chainid == 137) {
      i_matic_usd_priceFeed = 0xAB594600376Ec9fD91F8e885dADF0CE036862dE0;
      i_usdt_usd_priceFeed = 0x0A6513e40db6EB1b165753AD52E80663aeA50545;
      s_vesting = IVestingHub(0xD3C912Fa2CC79784eD40CF2F4EAE1e52BbE7005f);
      s_usdt = IERC20(0xc2132D05D31c914a87C6611C10748AEb04B58e8F);
    } else {
      i_matic_usd_priceFeed = 0xd0D5e3DB44DE05E9F294BB0a3bEEaF030DE24Ada;
      i_usdt_usd_priceFeed = 0x572dDec9087154dC5dfBB1546Bb62713147e0Ab0;
      s_vesting = IVestingHub(0x8a9eB5AfF2E941F5DF641927438bE378b42C5219);
  ...
  function _getPriceOfGivenTokenInMatic(int256 amount)
    public
    view
    returns (int256)
  {
    // 1000000 = below pricce
    int256 usdtPriceInMatic = _getDerivedPrice(
      i_usdt_usd_priceFeed,
      i_matic_usd_priceFeed,
      18
    );

    int256 priceOfTokensInUsdt = int256(multiply(uint256(amount), s_usdtPrice)); // 6 decimal answer
    ...
    return ((formmatedPriceOfTokensInUsdt * usdtPriceInMatic) / 1e18);
  }

  function buyTokensMATIC(uint256 amount)
    external
    payable
    whenNotPaused
    nonReentrant
  {
    require(
      ...
      "YPredictPrivateSale: All Tokens Solded Out"
    );

    uint256 payableAmountInMatic = uint256(
      _getPriceOfGivenTokenInMatic(int256(amount))
    );

    uint256 tokenAmount = amount * (10**18);

    require(
      msg.value >= payableAmountInMatic,
      "YPredictPrivateSale: Not enough funds to buy tokens"
    );
    ...
    require(
      s_investemetByAddress[msg.sender] + payableAmountUSDT <=
        s_maxAmountToInvest,
      "YPredictPrivateSale: Exceeding maximum limit"
    );
  ...
  function buyTokensUSDT(uint256 amount) external whenNotPaused nonReentrant {
    ...
    require(
      s_investemetByAddress[msg.sender] + payableAmount <= s_maxAmountToInvest,
      "YPredictPrivateSale: Exceeding maximum limit"
    );
  ...
  function updateVesting(IVestingHub newVesting) external onlyOwner {
    s_vesting = newVesting;
  }
...
interface IVestingHub {
  function allocateTokensManager(address benificiaries, uint256 amount)
    external;
}
