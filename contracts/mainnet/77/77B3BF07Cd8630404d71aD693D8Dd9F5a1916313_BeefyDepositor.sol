...
import "contracts/interfaces/protocols/IBeefyVault.sol";

contract BeefyDepositor is ZapDepositor {
    ...
    function depositInProtocol(address _token, uint256 _underlyingAmount)
        ...
        returns (uint256)
    {
        address beefyVault = IBTOfUnderlying[_token];
        ...
        IBeefyVault(beefyVault).deposit(uint256(_underlyingAmount)); // deposit underlying in the vault and mint IBTs to depositor.

        uint256 balanceOf_IBT = IBeefyVault(beefyVault).balanceOf(
            address(this)
        );
        require(
            balanceOf_IBT <= _underlyingAmount,
            "BeefyDepositor: balance error"
        );

        IBeefyVault(beefyVault).transfer(msg.sender, balanceOf_IBT); // transfer IBT from depositor to Zap
    ...
    function depositInProtocolFrom(
        ...
        require(
            ...
            "BeefyDepositor: Underlying Pull failed"
        ); // pull underlying tokens
