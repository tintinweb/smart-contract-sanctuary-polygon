...
error InvalidBigSigner(address signer);
...
contract AuctionSettler is AccessControl, EIP712 {
    ...
    bytes32 public constant BEST_BID_SIGNER = keccak256('BEST_BID_SIGNER');
    ...
    /// @dev check that signature matches the bid order
    modifier onlyValidBidSignature(SignedOrder calldata bid) {
        bytes32 _hash = hashOrder(bid.order);
        address signer = _hash.getSignerOfHash(bid.signature);
        if (bid.order.maker != signer) revert InvalidBigSigner(signer);
        _;
    }

    /// @dev check that bid is certified as the best bid
    modifier onlyCertifiedBestBid(
        ...
        address signer = _hash.getSignerOfHash(bestBidSignature);
        if (!hasRole(BEST_BID_SIGNER, signer)) revert InvalidBestBidProof(auction.order, bid.order);
        _;
    }
    ...
    function settleERC721Auction(
        ...
        SignedOrder calldata bid,
        LibSignature.Signature calldata bestBidSignature
    ) external onlyValidBidSignature(bid) onlyCertifiedBestBid(auction, bid, bestBidSignature) {
        LibNFTOrder.ERC721Order memory bidOrder = bid.order;
        // transfer bid amount to this contract
        bidOrder.erc20Token.safeTransferFrom(bidOrder.maker, address(this), bidOrder.erc20TokenAmount);
        // settle auction
    ...
    function settleERC721AuctionWithPermit(
        ...
        LibNFTOrder.ERC721Order memory bidOrder = bid.order;
        {
            uint256 deadline = permitData.deadline;
            uint8 v = permitData.v;
            bytes32 r = permitData.r;
            bytes32 s = permitData.s;

            IERC20 auctionERC20 = bidOrder.erc20Token;
            address bidder = bidOrder.maker;
            uint256 bidAmount = bidOrder.erc20TokenAmount;

            // transfer bid amount to this contract
            IERC20Permit(address(auctionERC20)).permit(bidder, address(this), bidAmount, deadline, v, r, s);
            auctionERC20.safeTransferFrom(bidder, address(this), bidAmount);
        }
        _settleERC721Auction(auction, bid);
    }
    ...
    function _settleERC721Auction(SignedOrder calldata auction, SignedOrder calldata bid) internal {
        ...
        auctionERC20.approve(zeroEx, bidAmount);
        ...
        transferFee(auctionOrder, bidAmount);

        // send ERC20 tokens back, if any
        uint256 balance = auctionERC20.balanceOf(address(this));
        if (balance > 0) {
            auctionERC20.safeTransfer(seller, balance);
        }
    ...
    function transferFee(LibNFTOrder.ERC721Order memory auction, uint256 bidAmount) internal {
