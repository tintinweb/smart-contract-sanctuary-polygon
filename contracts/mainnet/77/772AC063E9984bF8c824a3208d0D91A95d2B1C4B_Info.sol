...
contract Info is OwnableUpgradeable{
    ...
    function battleInfo(uint tokenId)external view returns(uint[3] memory info,bool isDead, address owner_,bool isCreation){
        ...
        info[0] = info[0] * rewRate[level] / 100;
        info[1] = info[1] * rewRate[level] / 100;
        info[2] = info[2] * rewRate[level] / 100;
    ...
    function checkBadgeInfoBatch(uint[] memory tokenIDs) public view returns(uint[] memory badgeID,uint[] memory effect){
        ...
        for(uint i = 0; i < tokenIDs.length; i ++){
            ...
            effect[i] = badge.checkBadgeEffect(tokenIDs[i]);
        }
    }
