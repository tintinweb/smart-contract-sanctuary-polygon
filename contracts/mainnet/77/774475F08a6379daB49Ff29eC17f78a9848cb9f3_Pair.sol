...
contract Pair is IPair {
    ...
    function _update0(uint amount) internal {
        ...
        uint256 _referralFee = amount * _maxRef / 100000;
        if (_referralFee > 0) {
        ...
        uint256 _stakingNftFee =  amount * PairFactory(factory).stakingNFTFee() / 100000;
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        (address _token0, address _token1) = (token0, token1);
        if (amount0In > 0) _update0(amount0In * PairFactory(factory).getFee(stable) / 100000); // accrue fees for token0 and move them out of pool
        if (amount1In > 0) _update1(amount1In * PairFactory(factory).getFee(stable) / 100000); // accrue fees for token1 and move them out of pool
        _balance0 = IERC20(_token0).balanceOf(address(this)); // since we removed tokens, we need to reconfirm balances, can also simply use previous balance - amountIn/ 100000, but doing balanceOf again as safety check
        _balance1 = IERC20(_token1).balanceOf(address(this));
    ...
    function getAmountOut(uint amountIn, address tokenIn) external view returns (uint) {
        (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
        amountIn -= amountIn * PairFactory(factory).getFee(stable) / 100000; // remove fee from amount received
