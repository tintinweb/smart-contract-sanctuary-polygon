...
contract onchainHeart is Ownable, ERC721, ERC721URIStorage {
    ...
    mapping(address => uint[]) giftsToRedeem;
    ...
    function mint(address ownerAddress, string memory name, string memory message) public payable returns (uint256) {
        ...
        giftsToRedeem[msg.sender].push(newID);
    ...
    function redeem() public returns (uint256) {
        require(giftsToRedeem[msg.sender].length > 0);
        uint totolGifts = giftsToRedeem[msg.sender].length;
        uint tokenID = giftsToRedeem[msg.sender][totolGifts-1];
    ...
    function withdrawAll(uint amount) public onlyOwner  {
      require(block.timestamp > 1645310620);
