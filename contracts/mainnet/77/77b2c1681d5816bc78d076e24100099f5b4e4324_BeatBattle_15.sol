...
contract BeatBattle_15 {
    ...
    function SetRegistrationTrue(string memory ueDate, string memory ueLink) public {
        require(_counter < 5, "BeatBattle_15::Exceeds maximum contestants");
        for (uint256 i=0; i<5; i++){
    ...
    function GetDates() public view returns (
        string memory date1, string memory date2, string memory date3, string memory date4, string memory date5
        ) {
    ...
    function GetVotingTickets () public view returns (uint256 fTickets){
        fTickets=votingTickets[msg.sender];
        return fTickets;
    }
