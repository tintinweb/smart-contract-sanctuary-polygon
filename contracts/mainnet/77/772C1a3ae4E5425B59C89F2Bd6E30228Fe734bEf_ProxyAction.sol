...
contract ProxyAction {
    ...
    function collectFor(CollectForRequest[] memory request) public { 
        for (uint256 i = 0; i < request.length; i++) {
            uint256 tokenId = _lensHub.collect(request[i].profileId, request[i].pubId, "");
            address collectNft = _lensHub.getCollectNFT(request[i].profileId, request[i].pubId);
            IERC721(collectNft).safeTransferFrom(address(this), request[i].mintFor, tokenId);
        }
    }

    function followFor(uint256[] calldata profileIds, address[] calldata mintFor, bytes[] calldata datas) public { 
