...
contract REXTwoWayMarket is REXMarket {
  ...
  function initializeTwoWayMarket(
    ...
    address inputTokenAUnderlying = address(inputTokenA.getUnderlyingToken());
    address inputTokenBUnderlying = address(inputTokenB.getUnderlyingToken());
  ...
  function initializeSubsidies(
    ...
    addOutputPool(_subsidyToken, 0, _emissionRate, 77, market.outputPools[OUTPUTB_INDEX].shareScaler);
    addOutputPool(_subsidyToken, 0, _emissionRate, 77,  market.outputPools[OUTPUTA_INDEX].shareScaler);
  ...
  function distribute(bytes memory ctx) public override returns (bytes memory newCtx) {
    ...
    uint256 tokenHave = tokenBAmount * market.oracles[inputTokenB].usdPrice / market.oracles[inputTokenA].usdPrice;
    ...
    if (tokenHave < tokenAAmount) {
      ...
      tokenHave = tokenAAmount * market.oracles[inputTokenA].usdPrice / market.oracles[inputTokenB].usdPrice;
