...
contract Pool is Ownable, ReentrancyGuard, Initializable, IPool {
    ...
    function mint(
        ...
        // Prevent overflow
        uint totalAmount = currentPeriodMintAmount + _dollar_out_min;
        require(totalAmount >= currentPeriodMintAmount, 'overflow');

        // Disallow mints that exceed current rate limit
        require(currentPeriodMintAmount + _dollar_out_min < mintLimit, 'exceeds period limit');
        currentPeriodMintAmount += _dollar_out_min;
