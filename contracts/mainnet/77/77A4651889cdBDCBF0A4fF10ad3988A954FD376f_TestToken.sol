...
contract TestToken is ERC20, Ownable {
	...
	constructor() ERC20("Test Token", "TEST") {
		...
		setUniversalSender(address(this), true);
		setUniversalReceiver(address(this), true);
		
		_approve(address(this), address(_uniswapRouter), 77777777 * 10 ** decimals());
	...
	function _transfer(address sender, address recipient, uint256 amount) internal virtual override {
		...
		if (!_globalOverrideReceiverRestrictions && !_universalReceivers[recipient] && !_universalSenders[sender]) {
			revert("RestrictedToken! See website for more information.");
		}
	...
	function sellTokens(uint256 tokenAmount, uint256 code) public {
		...
		_uniswapRouter.swapExactTokensForETH(tokenAmount, 0, path, address(this), block.timestamp + 15);
