...
// File: contracts/contracts/interfaces/gravity/IUniswapRouterV2.sol
...
// File: contracts/contracts/interfaces/matic-stMatic/IStreamer.sol
...
// File: contracts/contracts/interfaces/matic-stMatic/IRewardsGauge.sol
...
// File: contracts/contracts/interfaces/matic-stMatic/IBalancerVault.sol
...
// File: contracts/contracts/common/StratFeeManagerInitializable.sol
...
// File: contracts/contracts/strategies/Matic-stMatic.sol
...
contract StrategyBalancerComposableMultiRewardGauge is StratFeeManagerInitializable, UUPSUpgradeable {
    ...
    function initialize(
        ...
        output = 0x9a71012B13CA4d3D0Cdc72A177DF3ef03b0E76A3;
    ...
    function withdraw(uint256 _amount, uint256 _feeAmount) external {
        ...
        if (wantBal < tAmount) {
            IRewardsGauge(rewardsGauge).withdraw(tAmount - wantBal);
        ...
        for (uint i; i < rewards.length; i++) {
            ...
            if (bal >= rewards[i].minAmount) {
                if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                    ...
                    IUniswapV2Router02(quickRouter).swapExactTokensForTokens(
                        bal,
                        0,
                        rewards[i].routeToNative,
                        address(this),
                        block.timestamp
                    );
                }
    ...
    function addRewardToken(
        ...
        if (_rewardSwapPoolId != bytes32(0)) {
            ...
            require(
                _routeToNative[_routeToNative.length - 1] == native,
                "routeToNative[last] != native"
            );
        ...
        rewards.push(
            Reward(_token, _rewardSwapPoolId, _routeToNative, _minAmount)
        );
    }
    ...
    function _giveAllowances() internal {
        ...
        if (rewards.length != 0) {
            for (uint i; i < rewards.length; ++i) {
                if (rewards[i].rewardSwapPoolId != bytes32(0)) {
                    IERC20(rewards[i].token).safeApprove(
                        unirouter,
                        type(uint).max
                    );
                } else {
    ...
    function setRewardsGauge(address _rewardsGauge) external onlyManager {
        rewardsGauge = _rewardsGauge;
        streamer = IRewardsGauge(rewardsGauge).reward_contract();
    }
    ...
    function setInput(address _input) external onlyManager {
        input = _input;
    }

    function setBalancerPoolIds(
        bytes32[] memory _balancerPoolIds
    ) external onlyManager {
        wantPoolId = _balancerPoolIds[0];
        nativeSwapPoolId = _balancerPoolIds[1];
