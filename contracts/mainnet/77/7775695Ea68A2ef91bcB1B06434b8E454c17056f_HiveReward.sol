...
contract HiveReward {
   ...
   // mapping from tx index => owner => bool, to check if the owner has declined a proposed transaction
    ...
    // Mapping of each address that can claim rewards, will only be the hive nft collection initially, but other collections could be added in the future
    mapping(address => bool) CanClaimRewards;
    ...
    */
    function removeTeamMember(address _addr) external onlyAdmin {
    ...
    */
    function changeNumberConfirmations(uint _numConfirmations) external onlyAdmin {
    ...
    // Honey claiming functions

    /**
    * @dev Transfers tokens from this contract to the specified address
    * Requires the caller to be a accepted address
    * @notice Initially only the hive NFT contract will be allowed to call
    */
    function ClaimReward(uint _amount, address _address) external {

        require(CanClaimRewards[msg.sender], "Address can't claim rewards");

        ///@notice this will revert if the contract doesnt have the proper balance
        honeyToken.transfer(_address, _amount);

    }

    /**
    * @dev Set an address that is able to claim rewards from this contract
    * Requires the admin to call this function, the admin will be a multisig wallet contract
    */
    function UpdateCanClaimRewardsAddress(address _address, bool _value) external onlyAdmin {

        CanClaimRewards[_address] = _value;
