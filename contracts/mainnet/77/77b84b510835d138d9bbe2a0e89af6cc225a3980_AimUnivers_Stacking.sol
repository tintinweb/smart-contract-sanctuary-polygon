...
contract AimUnivers_Stacking {
    ...
    address public id1=0xC6EC3BeC88e073B12a474a999DC32b079F90012A;
    ...
    function dailyPayoutOf(address _user,uint level,uint256 _amount) public returns(uint,uint256){
                ...
                if(order.level==level) { 
                    if(_amount<=order.amount){
                        if(order.deposit_time+packages[level].duraion<block.timestamp)
                        {
                            ...
                            if(level>1)
                                dailypayout = _amount*dayRewardPercents*(packages[level].duraion/ timeStepdaily) / 30000;
                            else
                                dailypayout = _amount*dayRewardPercents*((block.timestamp - order.deposit_time)/ timeStepdaily) / 30000; 
                            isDeduction=0;
                        }
        ...
        return (isDeduction,dailypayout);
    }
    function stakeWithdraw(uint level,uint256 _amount) public
    {
        (uint isDeduction,uint256 dailypayout)=dailyPayoutOf(msg.sender,level,_amount);
        ...
        if(isDeduction>0 && level>1){
            tokenAPLX.transfer(msg.sender,(_amount*75/100+dailypayout));
        }
        else 
        {
