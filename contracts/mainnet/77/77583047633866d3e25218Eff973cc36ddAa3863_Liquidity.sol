...
contract Liquidity is UniV3QuoterCore, Ownable {
    ...
    constructor(ISwapRouter _swapRouter, INonfungiblePositionManager _nonfungiblePositionManager, address _TREND, uint24 _poolFee, address _factory, uint _tokenId) {
        ...
        TransferHelper.safeApprove(WMATIC, address(_swapRouter), 10000000000000000000 ether);
        TransferHelper.safeApprove(WMATIC, address(_nonfungiblePositionManager), 10000000000000000000 ether);
    }
    ...
    function _addLP(uint amountAdd0, uint amountAdd1) internal {
        ...
        nonfungiblePositionManager.increaseLiquidity{value:amountAdd0}(params);
    }
    function addLP() external payable {
        uint amountAdd1 = swapExactInputSingle(msg.value/2);
        _addLP(msg.value/2, amountAdd1);
    }

    function swapExactInputSingle1(uint256 amountIn) public payable returns (uint256 amountOut) {
        // Naively set amountOutMinimum to 0. In production, use an oracle or other data source to choose a safer value for amountOutMinimum.
        // We also set the sqrtPriceLimitx96 to be 0 to ensure we swap our exact input amount.
        ISwapRouter.ExactInputSingleParams memory params =
        ISwapRouter.ExactInputSingleParams({
    ...
    function _addLP1(uint amountAdd0, uint amountAdd1) internal {
        INonfungiblePositionManager.IncreaseLiquidityParams memory params =
        INonfungiblePositionManager.IncreaseLiquidityParams({
        tokenId: tokenIdOfPool,
        amount0Desired: amountAdd0,
    ...
    function addLP1() external payable {
        wmatic.deposit{value:msg.value}();
        uint amountAdd1 = swapExactInputSingle1(msg.value/2);
        _addLP1(msg.value/2, amountAdd1);
    }
    function getPool(
        address tokenA,
        address tokenB,
