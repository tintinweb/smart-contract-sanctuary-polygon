...
contract VoterV2 is IVoter, OwnableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function _initialize(address[] memory _tokens, address _minter) external {
        require(msg.sender == minter || msg.sender == emergencyCouncil, "Voter: Not admin");
    ...
    function setMinter(address _minter) external {
        require(msg.sender == emergencyCouncil, "Voter: Not admin");
        minter = _minter;
    }

    function setGovernor(address _governor) public {
        require(msg.sender == governor, "Voter: Not admin");
        governor = _governor;
    }
    ...
    function reset(uint _tokenId) external nonReentrant {
        require(IVotingEscrow(_ve).isApprovedOrOwner(msg.sender, _tokenId), "Voter: Not allowed action");
    ...
    function _reset(uint _tokenId) internal {
        ...
        for (uint i = 0; i < _poolVoteCnt; i ++) {
            ...
            if (_votes > 0) {
                ...
                IBribe(internal_bribes[gauges[_pool]])._withdraw(_votes, _tokenId);
    ...
    function _vote(uint _tokenId, address[] memory _poolVote, uint256[] memory _weights) internal {
        ...
        for (uint i = 0; i < _poolCnt; i++) {
            ...
            if (isGauge[_gauge]) {
                ...
                require(votes[_tokenId][_pool] == 0, "Voter: Already voted");
                require(_poolWeight != 0, "Voter: Zero vote weight");
                _updateFor(_gauge);
                ...
                IBribe(internal_bribes[_gauge])._deposit(_poolWeight, _tokenId);
                IBribe(external_bribes[_gauge])._deposit(_poolWeight, _tokenId);
    ...
    function vote(uint _tokenId, address[] calldata _poolVote, uint256[] calldata _weights) external nonReentrant {
        require(IVotingEscrow(_ve).isApprovedOrOwner(msg.sender, _tokenId), "Voter: Not allowed action");
        require(_poolVote.length == _weights.length, "Voter: Incorrect params");
        lastVoted[_tokenId] = block.timestamp;
        _vote(_tokenId, _poolVote, _weights);
    }
    ...
    function createGauge(address _pool) external returns (address) {
        ...
        if (msg.sender != governor) { // gov can create for any pool, even non-Ascent pairs
            require(isPair, "Voter: Should be pair");
            require(isWhitelisted[tokenA] && isWhitelisted[tokenB], "Voter: Not whitelisted token");
        }

        string memory _type =  string.concat("Ascent LP Fees: ", IERC20(_pool).symbol() );
    ...
    function _safeTransferFrom(address token, address from, address to, uint256 value) internal {
        require(token.code.length > 0, "Voter: Invaid token");
        (bool success, bytes memory data) =
        token.call(abi.encodeWithSelector(IERC20.transferFrom.selector, from, to, value));
        require(success && (data.length == 0 || abi.decode(data, (bool))), "Voter: Token transfer failed");
    }
    ...
    function increaseGaugeApprovals(address _gauge) external {
        require(msg.sender == emergencyCouncil, "Voter: Not admin");
        require(isGauge[_gauge] = true, "Voter: Not gauge");
