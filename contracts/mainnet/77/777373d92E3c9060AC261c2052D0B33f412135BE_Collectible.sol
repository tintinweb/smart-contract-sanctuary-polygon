...
import {AccessControlEnumerable, RBAC} from "../utils/RBAC.sol";

contract Collectible is ERC721Enumerable, ERC721Burnable, ERC721URIStorage, RBAC {
    bytes32 public constant MINTER_ROLE = keccak256(abi.encodePacked("MINTER_ROLE"));

    uint256 private _tokenIdTracker;
    string private _baseTokenURI;

    constructor(string memory name_, string memory symbol_) ERC721(name_, symbol_) RBAC(msg.sender) {}
    ...
    function mint(address to, uint256 amount) external onlyRole(MINTER_ROLE) {
        ...
        uint256 tracker = _tokenIdTracker;
        for (uint i; i < amount; ) {
            _mint(to, tracker + i);
            unchecked {
                i++;
    ...
    function tokenURI(
        uint256 tokenId
    ) public view virtual override(ERC721, ERC721URIStorage) returns (string memory uri) {
