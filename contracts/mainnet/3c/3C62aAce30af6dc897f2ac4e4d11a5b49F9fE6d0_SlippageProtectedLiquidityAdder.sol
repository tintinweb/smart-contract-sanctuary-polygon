...
contract SlippageProtectedLiquidityAdder {
    ...
    function addFunding(
        ...
        uint256[] memory minRefunds,
        uint256[] memory maxRefunds
    ) public {
        require(
            positionIds.length == minRefunds.length &&
            positionIds.length == maxRefunds.length,
            "Invalid positionIds"
        );
        ...
        for(uint256 i=0; i< length; ++i) {
            balancesBefore[i] = conditionalTokens.balanceOf(address(this), positionIds[i]);
        }

        marketMaker.addFunding(amount, distributionHint);
        
        for(uint256 i=0; i< length; ++i) {    
            uint256 refund = conditionalTokens.balanceOf(address(this), positionIds[i]) - balancesBefore[i];
            require(refund >= minRefunds[i] && refund <= maxRefunds[i], "Max slippage exceeded");
        }
    }
}
...
interface IFixedProductMarketMaker {
    function addFunding(uint256 amount, uint256[] memory distributionHint) external;
}