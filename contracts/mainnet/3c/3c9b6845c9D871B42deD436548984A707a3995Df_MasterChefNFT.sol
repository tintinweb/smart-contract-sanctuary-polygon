    ...
    function _payPendingTheEndlessSandMan(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pendingSandManToken > 0 || user.sandManRewardLockup > 0) {
                ...
                if (user.hasNFT) {
                    _payNFTBoost(_pid, msg.sender, sandmanRewards);
                    user.experience = user.experience + ((sandmanRewards * experienceRate) / 10000);
                    iFactoryNFT.setExperience(user.nftID, user.experience);
                }
            }

            if (_pid != theEndlessPID && (pendingTheEndlessToken > 0 || user.theEndlessRewardLockup > 0)) {
            ...
            if (_pid == theEndlessPID && (pendingUSDToken > 0 || user.usdRewardLockup > 0)) {
                ...
                totalUsdLockup -= pendingUSDToken;
            }
