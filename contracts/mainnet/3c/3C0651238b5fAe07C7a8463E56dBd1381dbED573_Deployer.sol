...
contract Deployer {
	address private constant dao = 0x57716b866A6e0D6534BF0d38c98C026bdfBe73b6;
	HedgeCore public hedgeCore;
	AddressProvider public addressProvider;
	OracleMaster public oracleMaster;
	HedgeToken public shortToken;
	HedgeToken public longToken;
	...
	// after deploy
	function deployAddressProvider_1() public onlyOwner {
		// deploy address provider
		addressProvider = new AddressProvider();
		addressProvider.setDAO(address(this));

	}

	function deployOracleMaster_2() public onlyOwner {
		// deploy oracle master
		oracleMaster = new OracleMaster();
		addressProvider.setOracleMaster(address(oracleMaster));
		// vesq oracle
		address vsq = 0x29F1e986FCa02B7E54138c04C4F503DdDD250558;
		oracleMaster.updateTokenOracle(vsq, 0x9a8726794E439F84eFEf388eF9b97B0C80F909ec);
		// dai chainlink oracle
		oracleMaster.updateTokenOracle(0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063, 0x8CDDe34efac46B9086f5A7DD1Ff81CC0Ef6E201b);
	}

	function deployHedgeCore_3()
		public
		onlyOwner
	{
		address svsq = 0xbb0FDc2Fe6D3cEB6Bf76Cc955173a07EDbF57494;
		hedgeCore = new HedgeCore("VSQ", address(addressProvider), svsq, 0x8f3Cf7ad23Cd3CaDbD9735AFf958023239c6A063);
		// deploy tokens
		shortToken = deployHedgeToken(
			string(abi.encodePacked("Hedge ", "VSQ")),
			string(abi.encodePacked("HGE-", "VSQ")),
			IERC20Metadata(svsq).decimals()
		);
		longToken = deployHedgeToken(
			string(abi.encodePacked("Leverage ", "VSQ")),
			string(abi.encodePacked("LEV-", "VSQ")),
			IERC20Metadata(svsq).decimals()
		);
	}

	function initAndFinal_4() public {
		hedgeCore.initGnesisHedge(address(shortToken), address(longToken));
		// transfer ownership
		addressProvider.setDAO(dao);
		addressProvider.setEmergencyAdmin(dao);
		addressProvider.transferOwnership(dao);

		// oracle master
		oracleMaster.transferOwnership(dao);
		deployer = address(0);
	}
	...
	function deployHedgeToken(
		...
		uint8 decimals_
	) private returns (HedgeToken) {
		HedgeToken token = new HedgeToken(address(addressProvider), address(hedgeCore), name_, symbol_, decimals_);
		return token;
	}
...
/// @dev Soft hedge&leverage core for vesq
contract HedgeCore is IHedgeCore, ReentrancyGuard, Pausable {
	...
	///@dev for frontend. prevent frontend rug
	function withdraw(uint256 shortAmount_, uint256 longAmount_) public override nonReentrant onlyInitialized onlyBeforeLockedTime {
	...
	function updateGaugeAndRatio(address newGauge_, uint256 ratio_) public onlyDAO {
		gauge = newGauge_;
		require(ratio_ <= uint256(20000), "HC:R  INV"); // <= 20%
		toGaugeRatio = ratio_;
	}
	...
	function _depositFor(
		...
		require(shortAmount_ + longAmount_ != 0, "HC:AMNT ZR");
		require(user_ != address(0), "HC:ADDR ZR");

		// stoken is not feeOnTransfer token so no need to check pre and post balance
		sToken.safeTransferFrom(msg.sender, address(this), shortAmount_ + longAmount_);
	...
	function _startNewHedge() private {
		require(shortToken.totalSupply() > 0 && longToken.totalSupply() > 0, "HC:B NON ZR TS");
	...
	function _initGnesisHedge(address shortToken_, address longToken_) private {
		...
		require(shortToken.core() == address(this) && longToken.core() == address(this), "HC:HT INV");
		...
		initialized = true;
		emit TokenUpdated(shortToken_, longToken_);
...
contract HedgeToken is ERC20 {
	...
	modifier onlyDAO() {
		require(msg.sender == addressProvider.getDAO(), "HT:ONLY DAO");
		_;
	}
	...
	function burn(address account_, uint256 amount_) external onlyCore {
		require(account_ != address(0), "HT:BN ZR AD");
		uint256 rawAmount = computeRawAmount(amount_, idx);
	...
	///@dev onlydao
	function updateCore(address core_) external onlyDAO {
		require(core_ != address(0), "HT:CORE ADDR ZR");
		core = core_;
	}
}
...
interface IHedgeCore {
	...
	event TokenUpdated(address indexed shortToken_, address indexed longToken_);
