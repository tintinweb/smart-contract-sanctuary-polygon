    ...
    UUPSUpgradeable
{
    ...
    function _addToWhiteList(address to) private {
    ...
    function fetchWhiteAccount(address account) external view returns (WhiteListItem memory) {
    ...
    function verifyAddress(
        ...
        bytes32 leaf = keccak256(abi.encodePacked(acccount));
        return MerkleProof.verify(merkleProofs, _merkleRoot, leaf);
    }

    function claim(bytes32[] calldata merkleProofs) external returns (uint32) {
        address sender = _msgSender();
        require(verifyAddress(sender, merkleProofs), "Invalid proof");
        _addToWhiteList(sender);
