...
// File: work/oland/web3cartoon.sol
...
contract web3cartoon is ERC1155BurnableUpgradeable,ERC1155SupplyUpgradeable,OwnableUpgradeable {
    ...
    function initialize() public initializer {
        __ERC1155_init("https://metadata.oland.info/web3cartoon/data/{id}.json");
    ...
    function symbol() public view returns (string memory) {
     ...
     function mint(
        ...
        nftIds.push(nftId);
        NFT memory nft  = NFT(nftId,nftName);
        NFTs.push(nft);
   ...
   function bulkTransfer(address[] calldata to,uint256 nftId,uint256 amount)
    public
    {
        for (uint i; i <= to.length - 1; i++) {
            safeTransferFrom(_msgSender(),to[i],nftId, amount,"");
        }
    }
     ...
     */
    function bulkTransfer1s(address[] calldata to,uint256 nftId,uint256[] calldata amount)
    public
    {
        for (uint i; i <= to.length - 1; i++) {
            safeTransferFrom(_msgSender(),to[i],nftId, amount[i],"");
        }
    }

    function bulkTransfers(address[] calldata to,uint256[] calldata _nftIds,uint256[] calldata amount)
    public
    {
        for (uint i; i <= to.length - 1; i++) {
            safeBatchTransferFrom(_msgSender() , to[i], _nftIds, amount,"");
        }
    }
