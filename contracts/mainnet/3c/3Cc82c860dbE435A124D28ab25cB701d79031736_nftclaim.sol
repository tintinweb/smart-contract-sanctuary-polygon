...
contract nftclaim is Context {
    ...
    address public authAddress=0x55dE0Fb6FacbDDB10581E4F4D22C1e96Cb3f0d3f;
    mapping(address=>mapping(uint256=>bool)) isClaimed;
    address public nftAddress=0xe6823805999b8433c67e7a43706456FBc8d72F1B;
    address public ownerAddress=0x2Aa84CDf0B815f5e5e1c8A20FD39bb3fBcc43407;
    ...
    function Claim(uint256 nftId,uint256 rankId,uint256 price,uint8 v, bytes32 r, bytes32 s) external payable
    {
        require(isClaimed[msg.sender][nftId]==false,"Allready claimed");
        string memory hash = keccak256(abi.encode(nftId, rankId,price, msg.sender)).toString();
    ...
    function setAuthAddress(address _address) external  {
        require(ownerAddress == _msgSender(), "Ownable: caller is not the owner");
        require(_address != address(0));
        authAddress = _address;
    }

    function updateNftAddress(address _newNftAddress) external  {
        require(ownerAddress == _msgSender(), "Ownable: caller is not the owner");
        nftAddress = _newNftAddress;
        emit newNftAddress(_newNftAddress,_msgSender());
    }

    function nftIsClaimed(address _address,uint256 nftId) external view returns(bool _isClaimed){
        return isClaimed[_address][nftId];
    }

    function newOwner(address _ownerAddress) external  {
        require(ownerAddress == _msgSender(), "Ownable: caller is not the owner");
        ownerAddress = _ownerAddress;
        emit _newOwner(_ownerAddress,_msgSender());
    }
