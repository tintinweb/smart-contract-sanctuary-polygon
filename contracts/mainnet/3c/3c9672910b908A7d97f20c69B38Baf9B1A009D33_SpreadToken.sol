...
contract SpreadToken is SpreadBaseToken {
    ...
    function burnAndClaim() external {
        ...
        uint256 receivedAmount = (tokenBalance *
            (asset.balanceOf(address(this)))) / (totalBalance);
        _burn(_msgSender(), tokenBalance);

        // user should see the settled amount even if it's 0.
        bool status = asset.transfer(_msgSender(), receivedAmount);
    ...
    function generateNameAndSymbol(
        ...
        spreadTokenInfo.displayStrikePrice1 = _getDisplayedStrikePrice(
            spreadTokenInfo.strikePrices[0]
        );
        spreadTokenInfo.displayStrikePrice2 = _getDisplayedStrikePrice(
            spreadTokenInfo.strikePrices[1]
        );
        spreadTokenInfo.strikeSymbol = ERC20Upgradeable(spreadTokenInfo.strike)
            .symbol();
        spreadTokenInfo.underlyingSymbol = ERC20Upgradeable(
            spreadTokenInfo.underlying
        ).symbol();

        {
            string memory collateralSymbol = ERC20Upgradeable(
                spreadTokenInfo.asset
            ).symbol();
            ...
            (
                ...
                uint256 day
                    ...
                    spreadTokenInfo.expiry
                );

            // get option type string
            (string memory typeSymbol, string memory typeFull) = _getOptionType(
                spreadTokenInfo.isPut
            );

            //get option month string
            (string memory monthSymbol, string memory monthFull) = _getMonth(
                month
            );
