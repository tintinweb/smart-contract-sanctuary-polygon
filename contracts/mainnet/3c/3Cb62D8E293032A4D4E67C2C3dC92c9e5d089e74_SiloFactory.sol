...
contract SiloFactory is Ownable, ERC721Enumerable {
  ...
  event SiloWithdraw(uint256 siloID);

  event SiloWithdrawToken(uint256 siloID, address[] tokens);

  event SiloExit(uint256 siloID, uint256 amount, uint8 exitType);

  event SiloStatusUpdate(uint256 siloID, uint8 status);

  event SiloName(uint256 siloID, string name);
  ...
  function adjustDefaultFeeList(uint256[4] memory _feeList) external onlyOwner {
  ...
  function _beforeTokenTransfer(
    ...
    if (enforceBeforeTransfer && to != address(0)) {
      ...
      if (ISilo(siloMap[tokenId]).highRiskAction()) {
        //if silo is running a high risk strategy then make sure the recievers manager has enough Link
        require(
          factory.getUpkeepBalance(to) >= factory.getMinimumUpkeepBalance(to),
          "small upkeep balance for high risk strategies"
        );
      }
    }
