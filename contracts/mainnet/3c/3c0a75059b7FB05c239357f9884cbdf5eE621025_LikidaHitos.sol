...
contract LikidaHitos is ERC1155, AccessControl, Pausable, ReentrancyGuard, ERC1155Burnable, ERC1155Supply, Ownable, ERC2981ContractWideRoyalties {

  /// The public name of this contract.
    string public constant name = "Likida Hitos";
    string public constant symbol = "LikHit";
    ...
    address private likidaAddress;
    
    mapping(address => mapping(uint256 => bool)) public tokensMintedByAddress;
    mapping(uint256 => bool) nftActive;
    ...
    constructor() ERC1155("https://tokens.likida.io/LikidaHitos/{id}.json") {
        ...
        // Token limit
        string memory collName = "Likida Hitos V1";
        addCollection(1, collName);
        setPrice(1, 0);
        setNFTState(1, true);
    }

    // Likida PS functions
    function setLikidaAddress(address _newAddress) public onlyRole(ADMIN_ROLE){
        require(_newAddress != address(0), "Cant be set to address 0");
        likidaAddress = _newAddress;
        setRoyalties(_newAddress, 500);
    }
    ...
    function setNFTState(uint256 id, bool state) public onlyRole(ADMIN_ROLE){
        nftActive[id] = state;
    }

    function addCollection(uint256 id, string memory _name) public onlyRole(ADMIN_ROLE){
        require(id > numCollections, "That collection id has allready been used");
        collectionNames[id] = _name;
        numCollections += 1;
    }

    function tokenMint(uint256 _id) public payable nonReentrant whenNotPaused {
        require(!tokensMintedByAddress[msg.sender][_id], "User allready minted this NFT");
        require(nftActive[_id], "NFT claim is inactive for that ID");
    ...
    function tokenMintBatch(uint256[] memory ids, uint256[] memory amounts) public payable nonReentrant whenNotPaused {
        for(uint256 i = 0; i < ids.length; i++){
            require(!tokensMintedByAddress[msg.sender][ids[i]], "User allready minted this NFT");
            require(nftActive[ids[i]], "NFT claim is inactive for that ID");
        }
