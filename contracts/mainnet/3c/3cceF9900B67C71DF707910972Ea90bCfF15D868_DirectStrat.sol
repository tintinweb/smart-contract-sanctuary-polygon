...
contract DirectStrat is OwnableUpgradeable {
  ...
  function triggerJava() external {
    ...
    (uint256 amount, ) = javaVendor.salesInADay(address(this));
    require(amount + amountDlyPerSale < daySellLimit, 'CANT SWAP YET');
    ...
    IERC20(DLYCOP).safeApprove(address(javaVendor), amountDlyPerSale);
    uint256 amountUsdtReceived = javaVendor.sellUSDT(amountDlyPerSale);
