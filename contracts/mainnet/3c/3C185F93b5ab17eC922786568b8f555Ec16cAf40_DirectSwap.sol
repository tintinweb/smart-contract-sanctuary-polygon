...
contract DirectSwap is FeeModel, IRouter {
    ...
    ) FeeModel(_partnerSharePercent, _maxFeePercent, _paraswapSlippageShare, _feeClaimer) {
        weth = _weth;
    }
    ...
    function directUniV3Swap(Utils.DirectUniV3 memory data) external payable {
        ...
        uint256 amountOut = ISwapRouterUniV3(data.exchange).exactInput{
            ...
            ISwapRouterUniV3.ExactInputParams({
                ...
                amountIn: fromAmount,
                amountOutMinimum: data.expectedAmount
            })
        );
        ...
        if (
            ...
            takeToTokenFeeAndTransfer(data.toToken, amountOut, beneficiary, data.partner, data.feePercent);
        } else if (amountOut > data.toAmount && !_isTakeFeeFromSrcToken(data.feePercent)) {
            ...
            Utils.transferTokens(data.toToken, beneficiary, amountOut);
        }
    ...
    function directUniV3Buy(Utils.DirectUniV3 memory data) external payable {
        ...
        uint256 amountIn = ISwapRouterUniV3(data.exchange).exactOutput{
            value: fromToken == ETH_IDENTIFIER ? data.fromAmount : 0
        }(
            ISwapRouterUniV3.ExactOutputParams({
                ...
                amountInMaximum: data.fromAmount
            })
        );
        if (fromToken == ETH_IDENTIFIER && amountIn < data.fromAmount) {
        ...
        uint256 remainingAmount = data.fromAmount.sub(amountIn);
        ...
        if (
            ...
            takeToTokenFeeAndTransfer(data.toToken, data.toAmount, beneficiary, data.partner, data.feePercent);
            Utils.transferTokens(data.fromToken, beneficiary, remainingAmount);
        } else {
            Utils.transferTokens(data.toToken, beneficiary, data.toAmount);
            if (_getFixedFeeBps(data.partner, data.feePercent) != 0 && _isTakeFeeFromSrcToken(data.feePercent)) {
                //  take fee from source token and transfer remaining token back to beneficiary
            ...
            } else if (remainingAmount.add(data.expectedAmount) > data.fromAmount) {
                takeSlippageAndTransferBuy(
                    fromToken,
                    beneficiary,
                    data.partner,
                    remainingAmount.add(data.expectedAmount),
                    data.fromAmount,
                    remainingAmount,
                    data.feePercent
                );
    ...
    function directCurveV1Swap(Utils.DirectCurveV1 memory data) external payable {
        ...
        if (data.swapType == Utils.CurveSwapType.EXCHANGE_UNDERLYING) {
            ICurvePool(data.exchange).exchange_underlying(data.i, data.j, fromAmount, data.expectedAmount);
        } else {
            if (address(fromToken) == ETH_IDENTIFIER) {
                ICurveEthPool(data.exchange).exchange{ value: fromAmount }(
                    data.i,
                    data.j,
                    fromAmount,
                    data.expectedAmount
                ...
                ICurvePool(data.exchange).exchange(data.i, data.j, fromAmount, data.expectedAmount);
            }
        }

        uint256 amountOut = Utils.tokenBalance(data.toToken, address(this));
    ...
    function directCurveV2Swap(Utils.DirectCurveV2 memory data) external payable {
        ...
        if (data.swapType == Utils.CurveSwapType.EXCHANGE_UNDERLYING) {
            ICurveV2Pool(data.exchange).exchange_underlying(data.i, data.j, fromAmount, data.expectedAmount);
        } else if (data.swapType == Utils.CurveSwapType.EXCHANGE_GENERIC_FACTORY_ZAP) {
            IGenericFactoryZap(data.exchange).exchange(
                data.poolAddress,
                data.i,
                data.j,
                fromAmount,
            ...
            if (address(fromToken) == ETH_IDENTIFIER) {
                ICurveV2EthPool(data.exchange).exchange{ value: fromAmount }(
                    data.i,
                    data.j,
                    fromAmount,
                    data.expectedAmount,
                ...
                ICurveV2Pool(data.exchange).exchange(data.i, data.j, fromAmount, data.expectedAmount);
            }
        }
    ...
    function directBalancerV2GivenInSwap(Utils.DirectBalancerV2 memory data) external payable {
        ...
        require(msg.value == (fromToken == ETH_IDENTIFIER ? data.swaps[0].amount : 0), "Incorrect msg.value");

        address payable beneficiary = data.beneficiary == address(0) ? msg.sender : data.beneficiary;

        transferTokensFromProxy(fromToken, data.swaps[0].amount, data.permit);

        if (_isTakeFeeFromSrcToken(data.feePercent)) {
            // take fee from source token
            data.swaps[0].amount = takeFromTokenFee(fromToken, data.swaps[0].amount, data.partner, data.feePercent);
        }

        if (!data.isApproved) {
            Utils.approve(data.vault, fromToken, data.swaps[0].amount);
        }

        int256[] memory deltas = IBalancerV2Vault(data.vault).batchSwap{
            value: fromToken == ETH_IDENTIFIER ? data.swaps[0].amount : 0
        }(IBalancerV2Vault.SwapKind.GIVEN_IN, data.swaps, data.assets, data.funds, data.limits, data.deadline);

        uint256 delta = uint256(-deltas[data.swaps[data.swaps.length - 1].assetOutIndex]);
        ...
        if (
            ...
            takeToTokenFeeAndTransfer(toToken, delta, beneficiary, data.partner, data.feePercent);
        } else if (delta > expectedAmount && !_isTakeFeeFromSrcToken(data.feePercent)) {
            takeSlippageAndTransferSell(
                toToken,
                beneficiary,
                data.partner,
                delta,
                uint256(-data.limits[data.swaps[data.swaps.length - 1].assetOutIndex]),
                data.feePercent
            );
        } else {
            Utils.transferTokens(toToken, beneficiary, delta);
        }
    ...
    function directBalancerV2GivenOutSwap(Utils.DirectBalancerV2 memory data) external payable {
        ...
        uint256 sentFromAmount = uint256(
            data.limits[data.swaps[data.swaps.length == 1 ? 0 : data.swaps.length - 1].assetInIndex]
        );
        uint256 expectedAmount = data.expectedAmount;

        if (fromToken == address(0)) fromToken = ETH_IDENTIFIER;
        if (toToken == address(0)) toToken = ETH_IDENTIFIER;
        require(msg.value == (fromToken == ETH_IDENTIFIER ? sentFromAmount : 0), "Incorrect msg.value");

        address payable beneficiary = data.beneficiary == address(0) ? msg.sender : data.beneficiary;

        transferTokensFromProxy(fromToken, sentFromAmount, data.permit);

        if (!data.isApproved) {
            Utils.approve(data.vault, fromToken, sentFromAmount);
        }

        int256[] memory deltas = IBalancerV2Vault(data.vault).batchSwap{
            value: fromToken == ETH_IDENTIFIER ? sentFromAmount : 0
        }(IBalancerV2Vault.SwapKind.GIVEN_OUT, data.swaps, data.assets, data.funds, data.limits, data.deadline);

        uint256 toAmount = uint256(-deltas[data.swaps[0].assetOutIndex]);
        uint256 spentFromAmount = uint256(
            deltas[data.swaps[data.swaps.length == 1 ? 0 : data.swaps.length - 1].assetInIndex]
        );

        {
            uint256 feePercent = data.feePercent;
            uint256 fromLeft = uint256(sentFromAmount).sub(spentFromAmount);

            if (
                _getFixedFeeBps(data.partner, feePercent) != 0 &&
                !_isTakeFeeFromSrcToken(feePercent) &&
                !_isReferral(data.feePercent)
            ) {
                takeToTokenFeeAndTransfer(toToken, toAmount, beneficiary, data.partner, feePercent);
                Utils.transferTokens(fromToken, beneficiary, fromLeft);
            } else {
                Utils.transferTokens(toToken, beneficiary, toAmount);
                ...
                } else if (fromLeft.add(expectedAmount) > sentFromAmount) {
                    takeSlippageAndTransferBuy(
                        fromToken,
                        beneficiary,
                        data.partner,
                        fromLeft.add(expectedAmount),
                        sentFromAmount,
                        fromLeft,
                        feePercent
                    );
    ...
    function transferTokensFromProxy(address token, uint256 amount, bytes memory permit) private {
...
contract FeeModel is AugustusStorage {
    ...
    function takeFromTokenFeeAndTransfer(
        ...
        Utils.transferTokens(fromToken, beneficiary, remainingAmount);
    }
    ...
    function _calcFixedFees(
        uint256 amount,
        uint256 fixedFeeBps
    ) private view returns (uint256 partnerShare, uint256 paraswapShare) {
    ...
    function _calcSlippageFees(
        ...
        require(feeBps + paraswapSlippageShare <= 10000, "Invalid fee percent");
        paraswapShare = slippage.mul(paraswapSlippageShare).div(10000);
