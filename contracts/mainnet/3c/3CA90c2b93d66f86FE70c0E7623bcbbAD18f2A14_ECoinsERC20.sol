...
abstract contract Ownable is Context {
    ...
    /**
     * @dev Returns if current user is an owner.
     */
...
// File: contracts\MaticNativeMetaTransaction.sol
...
// File: contracts\IECoinsERC20.sol

pragma solidity ^0.7.0;


interface IECoinsERC20 is IERC20{
...
// File: contracts\ECoinsAccessControl.sol

pragma solidity ^0.7.0;



contract ECoinsAccessControl is AccessControl, Ownable {
    
    bytes32 internal constant CONTRACT_ACCESS_ROLE = keccak256("CONTRACT_ACCESS_ROLE");
    bytes32 internal constant DEPOSITOR_ROLE = keccak256("DEPOSITOR_ROLE");
    
    constructor() 
    {
        _setRoleAdmin(DEFAULT_ADMIN_ROLE, DEFAULT_ADMIN_ROLE);
        _setRoleAdmin(CONTRACT_ACCESS_ROLE, DEFAULT_ADMIN_ROLE);
    ...
    function _checkRole(bytes32 role, address account) internal view {
        if (!hasRole(role, account) && !isOwner()) {
            revert(
                string(
                    abi.encodePacked(
...
// File: contracts\ECoinsERC20.sol
...
contract ECoinsERC20 is ContextMixin, NativeMetaTransaction, Pausable, Ownable, ECoinsAccessControl, IChildToken, IECoinsERC20, ERC20
{
    using SafeMath for uint256;

    constructor() ERC20("Emojis", "EMOJI")
    {
        _initializeEIP712("Emojis");
    ...
    function mint(address account, uint256 amount) external virtual override onlyRole(CONTRACT_ACCESS_ROLE) {
        _mint(account, amount);
    }

    function burn(address account, uint256 amount) external virtual override onlyRole(CONTRACT_ACCESS_ROLE) returns (bool) {
        uint256 decreasedAllowance = allowance(account, _msgSender()).sub(amount, "ECoinsERC20: burn amount exceeds allowance");
