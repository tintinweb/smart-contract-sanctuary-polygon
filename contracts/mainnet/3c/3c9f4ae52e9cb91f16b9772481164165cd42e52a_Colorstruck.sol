...
contract Colorstruck is ERC721, ERC721Enumerable, ERC721Burnable, Ownable {
    constructor() ERC721("Colorstruck", "CLRSTRK") {}
    uint256 public constant maxSupply = 24;
    ...
    function safeMint12(address to) public onlyOwner {
        uint totalSupply = totalSupply();
        uint currentId = totalSupply + 1;
        for (currentId = currentId; currentId <= (totalSupply + 12); currentId++) {
            safeMint(to, currentId);
        }
    }

    function _baseURI() internal override(ERC721) view virtual returns (string memory) {
        return "ipfs://QmTwaexjNobeULZhw6VLsQMwgZLAC8ZLtKCbtPJ5KA81x3/";
    }
