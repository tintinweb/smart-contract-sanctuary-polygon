 ...
 */
contract StrategAaveV3DepositBlock is IStrategStrategyBlock, IStrategOracleBlock {
    using SafeERC20 for IERC20;
    ...
    function enter(uint256 _index) external {
        ...
        IERC20(parameters.token).safeApprove(address(pool), amountToDeposit);
        pool.supply(
            parameters.token,
            amountToDeposit,
            address(this),
    ...
    function exit(uint256 _index, uint256 _percent) external {
        ...
        DataTypes.ReserveData memory collateralReserveData = pool.getReserveData(parameters.token);  
        uint256 amountToWithdraw = IERC20(collateralReserveData.aTokenAddress).balanceOf(address(this)) * _percent / 10000;
        pool.withdraw(
            parameters.token,
            amountToWithdraw,
            address(this)
    ...
    function oracleEnter(OracleResponse memory _before, bytes memory _parameters) external view returns (OracleResponse memory) {
        ...
        uint256 amountToDeposit = LibBlock.findTokenAmount(parameters.token, before) * parameters.tokenInPercent / 100;

        if(amountToDeposit == 0) {
            amountToDeposit = IERC20(parameters.token).balanceOf(before.vault);
            before = LibBlock.addTokenAmount(parameters.token, amountToDeposit, before);
        }

        DataTypes.ReserveData memory collateralReserveData = pool.getReserveData(parameters.token);  
        OracleResponse memory _after = LibBlock.removeTokenAmount(parameters.token, amountToDeposit, before);
        _after = LibBlock.addTokenAmount(collateralReserveData.aTokenAddress, amountToDeposit, _after);
    ...
    function oracleExit(OracleResponse memory _before, bytes memory _parameters) external view returns (OracleResponse memory) {
        ...
        DataTypes.ReserveData memory collateralReserveData = pool.getReserveData(parameters.token);  
        uint256 amountToWithdraw = LibBlock.findTokenAmount(collateralReserveData.aTokenAddress, before);

        if(amountToWithdraw == 0) {
            amountToWithdraw = IERC20(collateralReserveData.aTokenAddress).balanceOf(before.vault);
            before = LibBlock.addTokenAmount(collateralReserveData.aTokenAddress, amountToWithdraw, before);
        }

        OracleResponse memory _after = LibBlock.removeTokenAmount(collateralReserveData.aTokenAddress, amountToWithdraw, before);
        _after = LibBlock.addTokenAmount(parameters.token, amountToWithdraw, _after);
