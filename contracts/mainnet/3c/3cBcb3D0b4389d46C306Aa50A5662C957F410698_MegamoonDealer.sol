...
contract MegamoonDealer is Ownable, IMegamoonDealer {
    ...
    function getInjectAmount(uint256 lotteryId) private returns (uint256) {
    ...
    function injectToLottery(uint256 _lotteryId) external override returns (uint256) {
        ...
        uint256 injectAmount = getInjectAmount(currentLotteryId);
    ...
    function returnFunds(uint256 _lotteryId, uint256 _amount) external override {
        ...
        uint256 fee = _amount * processingFee / 10000;
        ...
        for ( uint256 i = 0; i < latestPoolUser; i++ ) {
            ...
            if (user.lockedAmount > 0) {
                uint256 reward = calculateReward(user.lockedAmount, user.lotteryId);
                user.stakingAmount += reward;
                user.lockedAmount = 0;
            }
        }
    ...
    function calculateReward(uint256 _amount, uint256 _lotteryId) private view returns (uint256) {
        return _amount * lotteryInjectionInfos[_lotteryId].returnAmount / lotteryInjectionInfos[_lotteryId].amount;
    }
